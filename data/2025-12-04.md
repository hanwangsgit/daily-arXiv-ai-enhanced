...
TL;DR: A self-supervised symbolic auto-encoder for diagram understanding that encodes diagrams into geometric primitives and relationships, using hierarchical reward modeling and stabilization mechanisms to achieve state-of-the-art performance on reconstruction, perception, and reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Symbolic computer vision offers interpretable diagram understanding through logical rules and structured representations, which differs fundamentally from pixel-based approaches. Current methods need better learning paradigms that can parse diagrams into geometric primitives while maintaining consistency in relationships between these elements.

Method: Proposes a self-supervised symbolic auto-encoder that encodes diagrams into structured primitives (points, lines, shapes) and their relationships, then decodes them through an executable engine. Uses Symbolic Hierarchical Process Reward Modeling with hierarchical step-level parsing rewards to enforce consistency constraints (point-on-line, line-on-shape, shape-on-relation). Introduces stabilization mechanisms to balance exploration and exploitation during diagram reconstruction, addressing vanilla RL's poor exploration in policy space.

Result: Achieves 98.2% reduction in MSE for geometric diagram reconstruction, surpasses GPT-4o by 0.6% with a 7B model on chart reconstruction, improves by +13% on MathGlance perception benchmark, and improves by +3% on MathVerse and GeoQA reasoning benchmarks.

Conclusion: The proposed neuro-symbolic system effectively integrates neural network reasoning capabilities with symbolic model interpretability through reasoning-grounded visual rewards, demonstrating superior performance across multiple diagram understanding tasks including reconstruction, perception, and reasoning.
...
Method: Propose a "drainage node" added at network output that reallocates probability mass toward uncertainty, preserving end-to-end training and differentiability, providing escape route for ambiguous/anomalous/noisy samples.

Result: Achieves up to 9% accuracy increase over existing approaches in high-noise regime on CIFAR-10/100 with instance-dependent/asymmetric noise; matches/surpasses SOTA on real-world datasets (mini-WebVision, mini-ImageNet, Clothing-1M); shows denoising effect where drainage neuron absorbs corrupt data.

Conclusion: Drainage formulation provides effective unified solution for noisy label problems, enables applications beyond classification including web-scale dataset cleaning and open-set applications.
...
TL;DR: AI-driven face restoration techniques can improve facial recognition accuracy when selectively applied, but naive application degrades performance.


<details>
  <summary>Details</summary>
Motivation: Real-world facial recognition suffers from poor accuracy due to low-quality images, non-frontal poses, and occlusions. The paper investigates whether AI-driven restoration techniques can address these challenges.
...
Result: Naive application of restoration techniques substantially degrades facial recognition accuracy. However, selective application of CFR-GAN combined with CodeFormer yields meaningful improvements in recognition performance.

Conclusion: Careful, selective application of AI-driven restoration techniques (specifically combining 2D frontalization with feature enhancement) can improve facial recognition accuracy in challenging real-world conditions, but indiscriminate use harms performance.
...
TL;DR: Flux4D: A simple, scalable, fully unsupervised framework for 4D reconstruction of large-scale dynamic scenes using 3D Gaussians with motion dynamics, achieving fast reconstruction and strong generalization without annotations or pre-trained models.
...
Method: Flux4D directly predicts 3D Gaussians and their motion dynamics using only photometric losses with "as static as possible" regularization. It learns decomposition from raw data across many scenes without pre-trained models or foundational priors.

Result: Flux4D enables efficient reconstruction within seconds, scales effectively to large datasets, and generalizes well to unseen environments including rare/unknown objects. Outperforms existing methods on outdoor driving datasets in scalability, generalization, and reconstruction quality.

Conclusion: Flux4D provides a simple, scalable, fully unsupervised solution for 4D reconstruction of large-scale dynamic scenes that overcomes limitations of existing methods and demonstrates strong performance on challenging outdoor driving scenarios.
...
### [5] [Ultra-lightweight Neural Video Representation Compression](https://arxiv.org/abs/2512.04019)
*Ho Man Kwan,Tianhao Peng,Ge Gao,Fan Zhang,Mike Nilsson,Andrew Gower,David Bull*

Main category: cs.CV

TL;DR: NVRC-Lite extends NVRC with lightweight neural representations using multi-scale feature grids and octree-based entropy coding, achieving better compression than C3 with significant speed improvements.


<details>
  <summary>Details</summary>
Motivation: Existing INR-based video codecs like NVRC show promise but have high computational complexity. Lightweight INRs exist but need improvement in both performance and practical entropy coding speed.

Method: Two key innovations: 1) Multi-scale feature grids in lightweight neural representations for better performance at low complexity, 2) Octree-based context model for entropy coding to replace slow autoregressive models.

Result: Outperforms C3 (best lightweight INR-based codec) with 21.03% BD-rate savings in PSNR and 23.06% in MS-SSIM, plus 8.4x encoding and 2.5x decoding speedup.

Conclusion: NVRC-Lite successfully combines lightweight neural representations with efficient entropy coding, making INR-based video compression more practical with better performance and faster speeds.
...
TL;DR: Zero-shot object counting using multimodal LLMs (GPT-4o and GPT-5) with only textual prompts, achieving state-of-the-art performance on FSC-147 dataset.
...
Method: Leverage multimodal LLMs (GPT-4o and GPT-5) to perform object counting in zero-shot manner using only textual prompts, without visual exemplars or annotated training data.

Result: Models achieve performance comparable to state-of-the-art zero-shot approaches on FSC-147 dataset, and in some cases even surpass them. Evaluated on both FSC-147 and CARPK datasets.

Conclusion: Multimodal LLMs can effectively perform zero-shot object counting using only textual prompts, demonstrating their potential for unsupervised counting tasks without visual exemplars.
...
TL;DR: LLM-based method infers material composition from 3D point clouds by decoupling object identity reasoning from material assignment, achieving high plausibility without task-specific training.
...
Method: Two-stage LLM approach: first stage predicts object semantics from point clouds, second stage assigns plausible materials to geometric segments conditioned on inferred semantics. Both stages operate zero-shot without task-specific training.

Result: Achieved high semantic and material plausibility across 1,000 shapes from Fusion/ABS and ShapeNet datasets, evaluated using LLM-as-a-Judge implemented in DeepEval.
...
TL;DR: Proposes a practical noise synthesis method using only one noisy image and one dark frame per ISO setting to generate realistic noise for training low-light denoisers without needing large paired datasets.


<details>
  <summary>Details</summary>
Motivation: Low-light images are very noisy, and learning-based denoisers need large paired datasets of clean-noisy images which are difficult to collect. Noise synthesis is an alternative but existing methods have limitations.

Method: Uses signal-dependent Poisson distribution for noise modeling and introduces Fourier-domain spectral sampling algorithm to accurately model signal-independent noise, requiring only one noisy image and one dark frame per ISO setting.

Result: The method generates diverse noise realizations that maintain spatial and statistical properties of real sensor noise, and leads to state-of-the-art performances on multiple low-light denoising benchmarks.

Conclusion: Proposes a general and practical noise synthesis approach that is accurate, doesn't rely on simplified parametric models or large paired datasets, and enables effective training of low-light denoisers.
...
TL;DR: PixPerfect is a pixel-level refinement framework that eliminates chromatic shifts, texture mismatches, and visible seams in LDM-based image inpainting and local editing by using discriminative pixel space, artifact simulation, and direct refinement.
...
Conclusion: PixPerfect delivers seamless, high-fidelity local edits across diverse LDM architectures and tasks, solving the pixel-level inconsistency problem that has plagued previous LDM-based editing approaches.
...
TL;DR: Two-stage deep learning pipeline (PyroFocus) for real-time wildfire detection and intensity estimation from thermal imagery, achieving good speed-accuracy tradeoffs for edge deployment.
...
Method: PyroFocus: two-stage pipeline with fire classification (CNN/Transformer models) followed by fire radiative power (FRP) regression/segmentation. Evaluated multiple deep learning architectures on NASA MASTER data.

Result: Two-stage pipeline achieves strong trade-offs between speed and accuracy, showing significant potential for real-time edge deployment in wildfire monitoring missions.

Conclusion: Proposed approach enables efficient real-time wildfire detection and intensity estimation suitable for resource-constrained onboard systems in future monitoring missions.
...
TL;DR: H²U3D is a new 3D visual question answering dataset for house-scale environments, and SpatialReasoner is an active perception framework that achieves state-of-the-art performance with efficient exploration using only 3-4 images on average.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with spatial reasoning in large-scale 3D environments, being limited to room-scale scenarios. There's a need for house-scale scene understanding capabilities.

Method: 1) Created H²U3D dataset with multi-floor environments (up to 3 floors, 10-20 rooms) using automated annotation pipeline for hierarchical representations and chain-of-thought QA pairs. 2) Proposed SpatialReasoner framework that autonomously invokes spatial tools for active 3D scene exploration. 3) Two-stage training: supervised cold start followed by reinforcement learning with adaptive exploration reward.

Result: SpatialReasoner achieves state-of-the-art performance on H²U3D, outperforming GPT-4o and Gemini-2.5-Pro. Notably achieves superior results using only 3-4 images on average vs baselines requiring 16+ images.

Conclusion: The coarse-to-fine active exploration paradigm is effective for large-scale 3D spatial reasoning, enabling efficient scene understanding with minimal visual observations while maintaining high accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Traditional HD maps are static and can become outdated, while online map construction from scratch is challenging. Navigation-grade SD maps are widely available but too coarse for direct use. The paper aims to leverage these coarse maps as priors to guide online map construction.
...
Result: On nuScenes benchmark, NavMapFusion conditioned on coarse OpenStreetMap road lines achieves 21.4% relative improvement at 100m range, with even stronger improvements at larger perception ranges, while maintaining real-time performance.
...
TL;DR: SLEDGE is a step-by-step layered design generator that models design updates as atomic changes, addressing the gap between single-step generation and the iterative nature of real design processes.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat design synthesis as single-step generation, underestimating the complexity of the creative process which is inherently iterative and step-by-step.

Method: Proposes Step-by-Step Layered Design Generation problem setting and SLEDGE model that uses multi-modal LLMs to generate designs by modeling each update as atomic, layered changes grounded in designer instructions.

Result: Exhaustive experimental analysis shows efficacy compared to state-of-the-art approaches, with introduction of new evaluation suite including dataset and benchmark.

Conclusion: The work addresses a pragmatic but under-explored research area and hopes to attract attention to step-by-step layered design generation.
...
TL;DR: ProtoEFNet is a video-based prototype learning model for continuous ejection fraction regression that learns dynamic spatiotemporal cardiac motion prototypes while maintaining accuracy comparable to black-box models.


<details>
  <summary>Details</summary>
Motivation: Traditional EF estimation requires manual tracing and expertise, making it time-consuming and variable. Current deep learning methods are black-box models with limited transparency, reducing clinical trust. Post-hoc explainability methods don't guide internal reasoning and offer limited reliability.

Method: ProtoEFNet uses video-based prototype learning for continuous EF regression, learning dynamic spatiotemporal prototypes that capture clinically meaningful cardiac motion patterns. It introduces Prototype Angular Separation (PAS) loss to enforce discriminative representations across the continuous EF spectrum.

Result: On the EchonetDynamic dataset, ProtoEFNet achieves accuracy on par with non-interpretable counterparts while providing clinically relevant insights. The PAS loss boosts performance with a 2% increase in F1 score from 77.67±2.68 to 79.64±2.10.

Conclusion: ProtoEFNet offers an interpretable alternative to black-box EF prediction models by learning clinically meaningful cardiac motion prototypes, potentially increasing clinical trust and adoption while maintaining competitive accuracy.
...
TL;DR: HalluGen is a diffusion-based framework that generates realistic hallucinations for safety-critical image restoration, enabling systematic evaluation and detection of hallucination artifacts.


<details>
  <summary>Details</summary>
Motivation: Generative models in safety-critical domains (medical imaging, industrial inspection, remote sensing) produce hallucinations that undermine reliability, but evaluating hallucinations is hindered by the circular dependency of needing labeled data that is costly and subjective to obtain.

Method: HalluGen uses diffusion-based framework to synthesize realistic hallucinations with controllable type, location, and severity, producing perceptually realistic but semantically incorrect outputs. It creates a large-scale hallucination dataset (4,350 annotated images from 1,450 brain MR images) for low-field MRI enhancement.

Result: The framework successfully generates hallucinations that significantly degrade segmentation performance (IoU drops from 0.86 to 0.36). It enables development of SHAFE metric for hallucination assessment and training of reference-free hallucination detectors that generalize to real restoration failures.

Conclusion: HalluGen and its open dataset establish the first scalable foundation for evaluating hallucinations in safety-critical image restoration, addressing a critical gap in reliable assessment of generative models in domains where hallucinations can have serious consequences.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning architectures have suboptimal inductive biases for detecting weak, spatially distributed anomalies in volumetric medical imaging. 2D/3D CNNs impose excessive locality while ViTs have diffuse global attention, both failing to capture the subtle, non-adjacent nature of early disease signals.
...
Result: Hierarchical attention models achieved 21-23% higher sensitivity and specificity in the sparse anomaly regime compared to other architectures. Mechanistic analyses revealed that hierarchical windowing produces effective receptive fields matched to the intermediate, multi-slice extent of subclinical abnormalities. Attention-distance measurements showed that required spatial integration length shifts significantly based on signal strength.

Conclusion: Hierarchical attention provides a superior and more parameter-efficient inductive bias for volumetric anomaly detection, establishing it as a principled approach for early pathological change analysis in 3D medical imaging. The findings offer design guidance for future volumetric anomaly detection systems.
...
<details>
  <summary>Details</summary>
Motivation: Most visual understanding, prediction, and generation methods operate directly on 2D observations, which leads to suboptimal performance because they ignore the underlying continuous 4D nature of the world (3D space + time).

Method: Three-stage framework: 1) Reconstruct 4D world from sparse monocular 2D frames (2D→4D), 2) Learn continuous 4D dynamics using low-rank representation and physical constraints (discrete 4D→continuous 4D), 3) Roll world forward in time and re-project to 2D at sampled times/viewpoints with spatial-temporal context awareness (4D→2D).
...
Conclusion: By modeling dynamics in the continuous 4D world rather than directly on 2D observations, SeeU enables more accurate and physically-plausible visual content generation for various applications.
...
TL;DR: Deep learning system using DenseNet169 with attention mechanisms and SVM with MobileNetV2 achieves 98% accuracy for lung cancer classification from CT scans, with interpretability via Grad-CAM and SHAP.


<details>
  <summary>Details</summary>
Motivation: Lung cancer is highly deadly worldwide, and early diagnosis is crucial for patient survival. Manual interpretation of CT scans is time-consuming and error-prone, necessitating automated systems for accurate and efficient diagnosis.

Method: Uses DenseNet169 with Squeeze-and-Excitation blocks for attention-based feature extraction, Focal Loss for class imbalance handling, and Feature Pyramid Network for multi-scale fusion. Also develops SVM model with MobileNetV2 for feature extraction. Implements Grad-CAM for CNN visualization and SHAP for SVM interpretability.
...
Conclusion: The proposed deep learning system shows potential for improving lung cancer diagnosis with high accuracy, transparency, and robustness, making it suitable for real-world medical applications.
...
TL;DR: FireSentry introduces a high-resolution wildfire dataset and FiReDiff model for fine-grained fire spread prediction, achieving state-of-the-art performance in video quality and mask accuracy.
...
Method: Created FireSentry dataset with sub-meter spatial and sub-second temporal resolution using synchronized UAVs, then developed FiReDiff - a dual-modality paradigm that first predicts future infrared video sequences, then segments fire masks based on generated dynamics.

Result: FiReDiff achieves significant improvements: 39.2% PSNR, 36.1% SSIM, 50.0% LPIPS, 29.4% FVD gains in video quality, and 3.3% AUPRC, 59.1% F1, 42.9% IoU, 62.5% MSE gains in mask accuracy.

Conclusion: FireSentry benchmark dataset and FiReDiff paradigm collectively advance fine-grained wildfire forecasting and dynamic disaster simulation, with publicly available dataset.
...
<details>
  <summary>Details</summary>
Motivation: Existing Gaussian-based methods have limitations: closed-set semantic Gaussians require 3D annotations and neglect rendering ability, while open-set methods use only 2D self-supervision leading to degraded geometry and camera-only limitations. There's a need to fully exploit Gaussian potential with open-vocabulary capabilities.

Method: Proposes Multi-Modal Gaussian Transformer that enables Gaussians to query features from diverse sensor modalities, and Shelf-Supervised Learning Paradigm that efficiently optimizes Gaussians with vision foundation model features at both 2D image and 3D scene levels.

Result: Achieves state-of-the-art zero-shot semantic occupancy prediction on Occ3D-nuScenes benchmark. Successfully evaluated on unmanned ground vehicle in diverse urban scenarios, demonstrating strong in-the-wild performance.

Conclusion: ShelfGaussian effectively bridges the gap between closed-set and open-set Gaussian methods by leveraging vision foundation models for open-vocabulary 3D scene understanding, showing strong performance across perception and planning tasks.
...
TL;DR: MOS framework addresses optical-SAR ship re-identification by reducing modality gap through denoising, alignment loss, and cross-modal data generation.
...
Method: Two core components: 1) Modality-Consistent Representation Learning (MCRL) with denoise SAR processing and class-wise modality alignment loss; 2) Cross-modal Data Generation and Feature fusion (CDGF) using brownian bridge diffusion model to synthesize cross-modal samples for feature fusion.

Result: Significant improvements on HOSS ReID dataset: +3.0% R1 accuracy (ALL to ALL), +6.2% (Optical to SAR), +16.4% (SAR to Optical), surpassing state-of-the-art methods.
...
TL;DR: The paper introduces ViDiC (Video Difference Captioning) task and ViDiC-1K dataset to evaluate MLLMs' ability to describe similarities and differences between video pairs, addressing limitations of existing image-based approaches.
...
Method: 1) Introduces ViDiC task for video difference captioning; 2) Creates ViDiC-1K dataset with 1,000 curated video pairs annotated with over 4,000 comparative checklist items across 7 categories; 3) Proposes dual-checklist framework using LLM-as-a-Judge protocol for reliable evaluation.

Result: Experiments on 19 representative multimodal models reveal significant performance gap in comparative description and difference perception abilities. ViDiC-1K serves as a challenging benchmark for video understanding.

Conclusion: ViDiC-1K lays foundation for advancing video understanding, edit awareness, and comparative reasoning in multimodal intelligence, addressing limitations of existing static image approaches.
...
Result: Achieves 52.8 mAP on ADG-Det and 73.1 mAP on IIT-Heat benchmarks with real-time performance up to 89.77 FPS (846.24 FPS for lightweight variant), demonstrating excellent accuracy-efficiency trade-off.

Conclusion: YOLOA successfully addresses the "what-where-how" challenge in affordance detection by jointly handling object detection and affordance learning with real-time capability, outperforming existing methods.
...
TL;DR: DM3D introduces a deformable Mamba architecture with adaptive serialization for point clouds, overcoming SSMs' reliance on input order through offset-guided Gaussian sequencing and achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: State Space Models (SSMs) show promise for long-sequence modeling but struggle with point clouds due to their irregular nature and reliance on input order. Existing approaches use fixed serialization strategies that can't adapt to diverse geometric structures.

Method: DM3D proposes: 1) Offset-guided Gaussian sequencing unifying local resampling and global reordering; 2) Gaussian-based KNN Resampling (GKR) for adaptive neighborhood reorganization; 3) Gaussian-based Differentiable Reordering (GDR) for end-to-end serialization optimization; 4) Tri-Path Frequency Fusion module for feature complementarity and aliasing reduction.
...
Conclusion: Adaptive serialization effectively unlocks SSMs' potential for point cloud understanding by enabling structure-aware processing of irregular geometric data.
...
Result: Foundation models excel on structured scenes (BridgeDepth: 0.23 px on ETH3D; DEFOM: 0.35-4.65 px across benchmarks), iterative methods maintain cross-domain robustness (IGEV++: 0.36-6.77 px). Critical RAFT-Stereo failure on ETH3D (26.23 px EPE, 98% error rate). DEFOM identified as optimal for forestry with superior depth smoothness, occlusion handling, and cross-domain consistency.

Conclusion: The study provides the first systematic evaluation of stereo methods for forestry applications, revealing scene-dependent performance patterns and establishing DEFOM as the gold-standard baseline for vegetation depth estimation in autonomous UAV operations.
...
TL;DR: A label-efficient hyperspectral image classification framework using frozen diffusion model features and spectral-spatial fusion, achieving SOTA performance with sparse annotations.
...
Method: Extracts low-level spatial features from early denoising timesteps of a frozen diffusion model pretrained on natural images, then uses a lightweight FiLM-based fusion module to adaptively combine these spatial features with spectral information.
...
Conclusion: Pretrained diffusion models can support domain-agnostic, label-efficient representation learning for remote sensing and broader scientific imaging tasks by providing transferable spatial features.
...
TL;DR: Proposes a novel vision-language pretraining framework for medical imaging with multi-agent data generation and ontology-based multi-aspect knowledge enhancement, achieving SOTA zero-shot performance in dermatology tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLP methods struggle with noise in web-collected medical data and complexity of unstructured long medical texts, limiting their effectiveness in medical image analysis.
...
Conclusion: The proposed framework effectively addresses data quality and text complexity challenges in medical VLP, demonstrating superior performance in dermatology applications with potential for broader medical imaging domains.
...
TL;DR: LM-CartSeg is an automatic pipeline for knee cartilage/bone segmentation, lateral/medial compartmentalization, and radiomics analysis using nnU-Net models with geometric post-processing.


<details>
  <summary>Details</summary>
Motivation: Existing knee MRI radiomics work relies on manual ROIs with poor quality control, lacking robust anatomically meaningful regions that capture both cartilage and subchondral bone.

Method: Two 3D nnU-Net models trained on SKM-TEA (138 knees) and OAIZIB-CM (404 knees), with zero-shot predictions fused and refined using geometric rules: connected-component cleaning, 10mm subchondral bone bands, and PCA/k-means for tibial lateral/medial split.

Result: Post-processing improved ASSD from 2.63 to 0.36mm and HD95 from 25.2 to 3.35mm (DSC 0.91). Zero-shot DSC on SKI-10 was 0.80. Geometric L/M rule produced stable compartments, and only 6-12% of radiomic features correlated with volume/thickness.
...
TL;DR: Unsupervised framework learns structured 3D keypoints from point clouds to condition diffusion models for shape reconstruction, improving consistency by 6% over prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised keypoint methods aren't designed for unconditional generative settings, limiting their use in modern 3D generative pipelines. There's a need to bridge this gap between unsupervised keypoint learning and generative modeling.

Method: Unsupervised framework learns spatially structured 3D keypoints from point cloud data. These keypoints serve as compact, interpretable representations that condition an Elucidated Diffusion Model (EDM) to reconstruct full shapes.
...
Conclusion: The framework successfully bridges unsupervised keypoint learning with modern 3D generative pipelines, providing structured, interpretable representations that effectively condition diffusion models for shape reconstruction while improving consistency.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models for video generation require intensive computation due to dozens of iterative steps and classifier-free guidance (CFG) doubling compute requirements. This inefficiency hinders broader adoption in downstream applications despite their superior performance.

Method: GalaxyDiT uses guidance alignment and systematic proxy selection for reuse metrics. Through rank-order correlation analysis, it identifies the optimal proxy for each video model across different model families and parameter scales to ensure optimal computational reuse.

Result: Achieves 1.87× speedup on Wan2.1-1.3B and 2.37× speedup on Wan2.1-14B with only 0.97% and 0.72% drops on VBench-2.0 benchmark. At high speedup rates, maintains superior fidelity to base model, exceeding prior SOTA approaches by 5-10 dB in PSNR.
...
Conclusion: Bridging appearance generation with 3D structure modeling through geometric regularization effectively addresses temporal consistency issues in video generation.
...
TL;DR: ThinkDeeper is a visual grounding framework for autonomous vehicles that uses a spatial-aware world model to reason about future scene states before making object localization decisions, achieving state-of-the-art performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing visual grounding methods for autonomous vehicles struggle with ambiguous, context-dependent instructions because they lack reasoning capabilities over 3D spatial relations and anticipated scene evolution. There's a need for systems that can understand complex natural language commands in dynamic driving environments.

Method: Proposes ThinkDeeper framework with two key components: 1) Spatial-Aware World Model (SA-WM) that distills current scene into command-aware latent states and rolls out future latent states for forward-looking reasoning, and 2) hypergraph-guided decoder that hierarchically fuses these states with multimodal input to capture higher-order spatial dependencies. Also introduces DrivePilot dataset created using RAG and CoT-prompted LLM pipeline.

Result: ThinkDeeper achieves #1 ranking on Talk2Car leaderboard and surpasses state-of-the-art baselines on DrivePilot, MoCAD, and RefCOCO/+/g benchmarks. Shows strong robustness in challenging scenes (long-text, multi-agent, ambiguity) and retains superior performance even when trained on only 50% of the data.

Conclusion: The ThinkDeeper framework successfully addresses visual grounding challenges in autonomous driving by incorporating future spatial reasoning through world models, demonstrating significant improvements over existing methods across multiple benchmarks while maintaining efficiency and data efficiency.
...
TL;DR: TPI (Text-Printed Image) enables effective text-centric training for LVLMs by rendering text descriptions on white canvases as synthetic images, bridging the modality gap without real images.


<details>
  <summary>Details</summary>
Motivation: Current LVLMs require costly image-text pairs for fine-tuning. Text is more available, editable, and scalable than images, but training on raw text alone yields limited gains due to the image-text modality gap.

Method: Proposes Text-Printed Image (TPI) - generating synthetic images by directly rendering textual descriptions on plain white canvases. This simple rendering projects text into image modality and can be integrated into existing LVLM training pipelines.

Result: TPI enables more effective text-centric training than diffusion model-generated synthetic images across four models and seven benchmarks. Also works as low-cost data-augmentation strategy.

Conclusion: Text-centric training has significant potential, and TPI charts a path toward fully automated data generation for LVLMs by bridging the modality gap with simple, cost-effective synthetic image generation.
...
<details>
  <summary>Details</summary>
Motivation: Infrared small target detection faces challenges due to lack of target texture and severe background clutter, where backgrounds obscure targets, requiring methods to enhance targets while suppressing backgrounds.

Method: Proposed Basis Decomposition Module (BDM) decomposes complex features into basis features to enhance information and eliminate redundancy. Extended to SD²M, SD³M, and TD²M modules. Built SD²Net (U-shaped architecture with SD²M/SD³M) for single-frame detection and STD²Net (adding TD²M for motion information) for multi-frame detection.

Result: Achieved state-of-the-art performance on both SISTD and MISTD datasets. SD²Net performed well on single-frame detection. STD²Net achieved 87.68% mIoU on MISTD datasets, significantly outperforming SD²Net's 64.97% mIoU.

Conclusion: The proposed basis decomposition approach effectively addresses infrared small target detection challenges by enhancing targets and suppressing backgrounds through feature decomposition, with temporal motion information providing significant performance gains for multi-frame detection.
...
TL;DR: AEM framework jointly models action execution and outcomes for mistake detection, using effect-aware representations and prompt-based detection to achieve SOTA performance on procedural task benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing mistake detection approaches focus only on how actions are performed, ignoring what they produce (action effects). Many errors manifest not in execution but in resulting outcomes like unintended object states or spatial arrangements.

Method: Action Effect Modeling (AEM) framework: 1) Identifies action outcomes by selecting most informative effect frames based on semantic relevance and visual quality; 2) Extracts complementary cues from visual grounding and symbolic scene graphs; 3) Aligns these in shared latent space for effect-aware representations; 4) Uses prompt-based detector with task-specific prompts aligned with intended execution semantics.
...
Result: GR-LoRA reduces diagnostic accuracy disparities by 69% while maintaining 53.15% overall accuracy on 10,000 glaucoma fundus images. Race-specific optimization achieves 60% disparity reduction. Approach requires only 0.24% trainable parameters.

Conclusion: Fairness-aware LoRA enables practical deployment of fair medical AI in resource-constrained healthcare settings by combining parameter efficiency with explicit fairness optimization, significantly reducing demographic disparities with minimal accuracy trade-off.
...
TL;DR: Proposes Object-IVQA, a new object-centric video QA benchmark for procedural understanding, and an agent framework that outperforms existing VLMs on object-level reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing action-centric methods struggle with real-world procedural flexibility where step order depends on object states. Need to shift to object-centric paradigm where actions drive state transitions.

Method: Introduces Object-IVQA benchmark with 107 instructional videos and 514 QA pairs with temporal evidence. Proposes agent framework with object-centric planning, perception, analysis and generation tools for explicit evidence retrieval and multi-hop reasoning.

Result: Existing large vision-language models struggle with object-level recognition and reasoning. The proposed framework achieves substantial improvements on the benchmark tasks.

Conclusion: Object-centric paradigm is crucial for procedural understanding. The Object-IVQA benchmark and agent framework advance object-centric reasoning in instructional videos, enabling better assistive AI for complex real-world tasks.
...
TL;DR: NAS-LoRA: A new PEFT method that integrates Neural Architecture Search with LoRA to bridge semantic gaps between SAM and specialized domains, improving adaptation while reducing training costs.


<details>
  <summary>Details</summary>
Motivation: SAM lacks spatial priors in its Transformer encoder, hindering acquisition of high-level semantic information for domain-specific tasks. Existing LoRA methods don't incorporate inductive bias needed for specialized domains like medical/agricultural imaging.

Method: NAS-LoRA adds a lightweight Neural Architecture Search block between encoder and decoder components of LoRA to dynamically optimize prior knowledge integration. Uses stage-wise optimization strategy to help ViT encoder balance weight updates and architectural adjustments.

Result: Improves existing PEFT methods while reducing training cost by 24.14% without increasing inference cost. Demonstrates potential of NAS in enhancing PEFT for visual foundation models.

Conclusion: NAS-LoRA effectively bridges semantic gap between pre-trained SAM and specialized domains by incorporating NAS into PEFT framework, enabling better adaptation to downstream tasks with reduced computational overhead.
...
TL;DR: EEA is a video agent framework that balances exploration-exploitation through semantic-guided hierarchical tree search for efficient long-form video understanding.


<details>
  <summary>Details</summary>
Motivation: Current approaches to long-form video understanding have two main problems: severe computational overhead from dense preprocessing, and ineffective balance between exploration and exploitation, leading to incomplete information coverage and inefficiency.

Method: EEA uses semantic guidance with hierarchical tree search. It autonomously discovers and updates task-relevant semantic queries, collects matching video frames as semantic anchors, preferentially explores semantically relevant frames while ensuring coverage in unknown segments, and adaptively combines VLM intrinsic rewards with semantic priors by modeling uncertainty.
...
Conclusion: EEA effectively addresses the exploration-exploitation trade-off in long-form video understanding through semantic-guided hierarchical search, achieving both better performance and computational efficiency.
...
Method: Proposes Domain-aware Prompt-driven Masked Transformer with: 1) domain-aware prompt learning for semantic alignment, 2) domain-aware contrastive learning with texture perturbation to diversify observable domains, 3) domain-robust consistency learning to minimize prediction discrepancies between original and augmented images.

Result: Demonstrates superiority and establishes new state-of-the-art performance on various DGSS benchmarks.

Conclusion: DPMFormer effectively addresses semantic misalignment in domain generalized semantic segmentation through domain-aware prompt learning and contrastive approaches, achieving robust performance across diverse domains.
...
TL;DR: A proof-of-concept framework using YOLOv8/v11 and SAM for automated dance movement analysis without specialized equipment, tested on Ghanaian AfroBeats dance video with promising detection and segmentation results.


<details>
  <summary>Details</summary>
Motivation: To explore automated dance movement analysis using contemporary computer vision techniques without requiring specialized equipment or markers, enabling quantitative assessment of dance performances.

Method: Integrated YOLOv8 and v11 for dancer detection with Segment Anything Model (SAM) for precise segmentation, enabling tracking and quantification of movements including step counting, spatial coverage patterns, and rhythm consistency measurement.

Result: Achieved ~94% detection precision and ~89% recall, with SAM segmentation reaching ~83% IoU. Analysis showed primary dancer executed 23% more steps with 37% higher motion intensity and used 42% more performance space than secondary dancers.

Conclusion: The framework demonstrates technical feasibility for automated dance analysis and establishes foundation for future systematic validation, though limited by single-video testing, lack of ground truth annotations, and no comparison with existing pose estimation methods.
...
TL;DR: CSMapping is a crowdsourced mapping system that produces accurate semantic maps and road centerlines whose quality improves with more data, using diffusion models for semantic mapping and clustering for topological mapping.
...
Method: For semantic mapping: train latent diffusion model on HD maps to learn generative prior, incorporate via constrained MAP optimization in latent space with robust initialization (vectorized mapping + diffusion inversion) and efficient optimization techniques. For topological mapping: confidence-weighted k-medoids clustering and kinematic refinement on trajectories.

Result: Achieves state-of-the-art semantic and topological mapping performance on nuScenes, Argoverse 2, and proprietary dataset, with quality consistently improving with more crowdsourced data. Thorough ablation and scalability studies demonstrate effectiveness.

Conclusion: CSMapping successfully addresses the noise robustness problem in crowdsourced mapping, producing high-quality semantic maps and road centerlines that scale effectively with data volume, enabling practical large-scale autonomous driving map construction.
...
Conclusion: The diffusion forcing-based framework, when properly tailored with the three proposed improvements, can effectively model real motion distributions and achieve superior performance for streaming human motion generation tasks.
...
TL;DR: OpenTrack3D: A mesh-free 3D instance segmentation framework using visual-spatial tracking and MLLMs for better generalization and compositional reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary 3D instance segmentation methods have two key limitations: (1) reliance on dataset-specific proposal networks or mesh-based superpoints, making them inapplicable to mesh-free environments and limiting generalization, and (2) weak textual reasoning of CLIP-based classifiers that struggle with compositional and functional user queries.

Method: OpenTrack3D uses a visual-spatial tracker to construct cross-view consistent object proposals online from RGB-D streams. It leverages 2D open-vocabulary segmentation, lifts masks to 3D point clouds using depth, extracts mask-guided instance features with DINO, and fuses visual-spatial cues for consistency. The core is mesh-free with optional superpoints refinement when mesh is available. CLIP is replaced with multi-modal large language models for better compositional reasoning.

Result: State-of-the-art performance on diverse benchmarks including ScanNet200, Replica, ScanNet++, and SceneFun3D, demonstrating strong generalization capabilities across different environments.

Conclusion: OpenTrack3D provides a generalizable and accurate framework for open-vocabulary 3D instance segmentation that works in mesh-free environments and handles complex compositional queries through MLLM integration, addressing key limitations of existing methods.
...
TL;DR: PRIS is a framework that adaptively revises prompts during inference by analyzing generated visuals, identifying failure patterns, and redesigning prompts to improve text-to-visual generation alignment.


<details>
  <summary>Details</summary>
Motivation: Current text-to-visual generation methods often fail to align with user intent on first attempts, and simply scaling the generation process (more steps/seeds) hits quality plateaus because prompts remain fixed. There's a need for adaptive prompt revision during inference.

Method: PRIS introduces an inference-time framework that: 1) Reviews generated visuals, 2) Identifies recurring failure patterns across visuals, 3) Redesigns prompts based on these patterns, and 4) Regenerates visuals with revised prompts. It uses a novel element-level factual correction verifier for fine-grained alignment assessment between prompt attributes and generated visuals.

Result: Extensive experiments on text-to-image and text-to-video benchmarks show significant improvements, including a 15% gain on VBench 2.0. The approach demonstrates that jointly scaling prompts and visuals is key to leveraging inference-time scaling laws effectively.

Conclusion: Adaptive prompt redesign during inference, combined with fine-grained alignment verification, significantly improves text-to-visual generation quality. The PRIS framework shows that scaling both prompts and visuals, rather than just generation processes, is essential for achieving better alignment with user intent.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models struggle with structured multi-step scenarios like recipe illustration, and existing methods generate fixed numbers of images regardless of actual instruction structure, lacking flexibility for variable-length recipes.
...
Conclusion: The framework enables flexible generation of coherent image sequences from arbitrary-length cooking instructions, with significant potential for applications in instructional media and procedural content creation.
...
TL;DR: V-ITI is a lightweight framework that detects visual neglect in MLLMs via head-level activation patterns and intervenes only when needed, reducing hallucinations without over-intervention.


<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from hallucinations due to visual neglect (failing to prioritize input images). Existing methods cause "over-intervention" by intervening indiscriminately, creating new hallucinations and computational overhead.
...
Conclusion: V-ITI addresses the "when to intervene" problem in hallucination mitigation, providing a lightweight, effective solution that avoids over-intervention and maintains model capabilities.
...
TL;DR: Hybrid content moderation framework for livestreaming combines supervised classification for known violations with similarity matching for novel cases, using multimodal inputs and MLLM knowledge distillation to achieve scalable detection.


<details>
  <summary>Details</summary>
Motivation: Content moderation for large-scale user-generated video platforms is challenging, especially in livestreaming where moderation must be timely, multimodal, and robust to evolving unwanted content that traditional approaches miss.
...
TL;DR: CartoMapQA is a new benchmark for evaluating Visual-Language Models' understanding of cartographic maps through 2000+ QA samples covering low-to-high-level map interpretation skills, revealing models struggle with map semantics, geospatial reasoning, and OCR errors.


<details>
  <summary>Details</summary>
Motivation: While Visual-Language Models (LVLMs) have advanced visual-text integration, their ability to interpret cartographic maps remains largely unexplored. There's a need to evaluate and improve LVLMs' map understanding capabilities for real-world applications like navigation, geographic search, and urban planning.

Method: Created CartoMapQA benchmark with over 2000 samples, each containing a cartographic map, question (open-ended or multiple-choice), and ground-truth answer. Tasks span low-, mid-, and high-level map interpretation skills including symbol recognition, embedded information extraction, scale interpretation, and route-based reasoning. Evaluated both open-source and proprietary LVLMs on this benchmark.

Result: Evaluation revealed persistent challenges: models frequently struggle with map-specific semantics, exhibit limited geospatial reasoning, and are prone to OCR-related errors. The benchmark successfully isolates these weaknesses in current LVLMs.

Conclusion: CartoMapQA provides a valuable tool for guiding future improvements in LVLM architectures and supports development of models better equipped for real-world applications requiring robust map understanding. The benchmark and data are openly available to the research community.
...
Method: Three-phase framework: 1) Fine-tune point cloud generation model for coarse object representation from text, 2) Hypergraph-based learning to refine parts as vertices, 3) Diffusion model to generate joints as graph edges.

Result: Superior performance over previous methods on PartNet-Mobility dataset, demonstrated through extensive qualitative and quantitative experiments.

Conclusion: GAOT effectively bridges the gap between text prompts and articulated 3D object generation, achieving state-of-the-art performance through its three-phase diffusion and hypergraph learning approach.
...
TL;DR: GLASTE is a new scene text editing framework that addresses inconsistency and length-insensitivity challenges by combining global contextual information with local features through a global-local structure, joint losses, and style vector representation.


<details>
  <summary>Details</summary>
Motivation: Existing scene text editing methods suffer from two major problems: 1) inconsistency between edited local patches and surrounding areas, and 2) difficulty handling significant differences in text length before and after editing.

Method: Proposes GLASTE framework with global-local combination structure, joint global and local losses, enhanced text image features, style vector representation independent of image size, and affine fusion to maintain aspect ratio when filling target text.

Result: Extensive experiments on real-world datasets show GLASTE outperforms previous methods in both quantitative metrics and qualitative results, effectively mitigating inconsistency and length-insensitivity challenges.

Conclusion: GLASTE successfully addresses key challenges in scene text editing by simultaneously incorporating global contextual information and local features, enabling consistent text style preservation and handling of varying text lengths.
...
Conclusion: Information uniqueness plays a pivotal role in token compression efficacy, and UniComp demonstrates superior performance in maximizing information fidelity under computational constraints.
...
TL;DR: CSCL is a two-stage pretraining framework that uses cross-stain contrastive learning to create universal WSI representations by aligning H&E with IHC stains, addressing inter-stain misalignment issues in computational pathology.


<details>
  <summary>Details</summary>
Motivation: Current WSI representations are limited by scarce well-aligned multi-stain datasets. Inter-stain misalignment hinders consistent patch-level features and degrades slide-level embeddings, preventing effective integration of biologically meaningful IHC information with H&E.

Method: Two-stage framework: 1) Patch-wise contrastive alignment with lightweight adapter to improve H&E-IHC compatibility, 2) Slide-level representation learning with MIL using cross-stain attention fusion and global alignment modules to integrate stain-specific features and enforce consistency across stains.
...
Conclusion: CSCL successfully addresses inter-stain misalignment and creates universal, transferable WSI representations by effectively integrating multi-stain information, with code and curated five-stain dataset publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Need for quick, easy bot detection methods that leverage human perceptual abilities (motion perception) that current AI systems struggle with, particularly for surveys and online processes.

Method: Display a 'hidden' number with same random black-and-white pixel texture as background; number becomes perceptible to humans through motion and scale differences across frames, but frame-by-frame algorithmic processing yields no meaningful signal.

Result: State-of-the-art multimodal models (GPT-5-Thinking, Gemini 2.5 Pro) fail to extract correct values even with explicit instructions; 99.5% of online survey participants (181/182) solved task with average 10.7s completion time; no negative effects on perceived ease-of-use in lab study.

Conclusion: DOT-BI effectively distinguishes humans from bots using motion perception, is user-friendly, and releases code/pre-rendered variants to facilitate adoption in surveys and online processes.
...
<details>
  <summary>Details</summary>
Motivation: Existing video frame interpolation methods struggle with fast, complex, non-linear motion patterns and fail to produce sharp, temporally consistent frames in fine-grained tasks like audio-visual synchronized interpolation. Diffusion-based approaches still have limitations covering diverse application scenarios.

Method: 1. Enhanced input design for multiple conditional modalities (text, audio, images, video). 2. Decoupled multimodal fusion mechanism that sequentially injects different conditional signals into a DiT (Diffusion Transformer) backbone. 3. Progressive multi-stage training paradigm using start-end frame difference embedding to dynamically adjust data sampling and loss weighting.
...
Conclusion: BBF successfully addresses limitations of existing interpolation methods by providing a flexible, context-aware framework that can handle complex motion patterns and audio-visual synchronization through multimodal conditioning and progressive training.
...
TL;DR: HyperRNA is a generative model using hypergraph-based encoder-decoder architecture for RNA inverse folding, outperforming existing methods in RNA sequence design.


<details>
  <summary>Details</summary>
Motivation: The RNA inverse folding problem is crucial for RNA design but challenging due to complex sequence-structure relationships. Existing methods need improvement for accurate RNA sequence generation from desired secondary structures.

Method: Three-stage framework: 1) Preprocessing constructs graph structures from RNA backbone atom coordinates using 3-bead coarse-grained representation; 2) Encoding captures higher-order dependencies via attention embedding and hypergraph-based encoder; 3) Decoding generates RNA sequences autoregressively.

Result: HyperRNA outperforms existing RNA design methods on PDBBind and RNAsolo datasets for both RNA sequence generation and RNA-protein complex sequence generation tasks.
...
TL;DR: CloseUpAvatar: Articulated human avatar representation that adapts rendering quality based on camera distance using dual-texture planes for realistic close-up views while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Previous avatar representations struggle with maintaining rendering quality across wide camera motions, especially for close-up views. There's a need for methods that can handle general camera movements while preserving detail in close-up scenarios without sacrificing performance.

Method: Represents avatar as textured planes with two sets of learnable textures (low and high-frequency). Automatically switches to high-frequency textures for close cameras and gradually reduces their impact as camera moves away. Uses limited number of primitives to maintain high FPS.

Result: Qualitative and quantitative improvements over existing methods in rendering from novel wide-range camera positions. Maintains high FPS by limiting required primitives. Tested on ActorsHQ dataset with high-resolution input images.

Conclusion: CloseUpAvatar enables realistic human avatar rendering across wider camera orientation ranges than previous approaches, with adaptive quality based on camera distance and efficient performance.
...
TL;DR: HBFormer: A Hybrid-Bridge Transformer for medical image segmentation that combines U-shaped encoder-decoder with Swin Transformer backbone and novel Multi-Scale Feature Fusion decoder to better integrate local details with global context.


<details>
  <summary>Details</summary>
Motivation: Current Vision Transformers with shifted window-based self-attention struggle to effectively fuse local details with global context, which is critical for challenging tasks like microtumor and miniature organ segmentation where both fine-grained boundary definition and broad contextual understanding are essential.

Method: HBFormer combines a classic U-shaped encoder-decoder framework with a Swin Transformer backbone for hierarchical feature extraction. The core innovation is the 'Bridge' mechanism implemented through a novel Multi-Scale Feature Fusion (MFF) decoder that uses channel and spatial attention modules constructed from dilated and depth-wise convolutions to fuse multi-scale features with global context.
...
Conclusion: HBFormer effectively addresses the limitation of existing Vision Transformers in fusing local details with global context, providing superior performance for challenging medical segmentation tasks through its hybrid architecture and sophisticated bridge mechanism.
...
TL;DR: A retrieval-augmented framework for tooth completion that uses a learnable prototype memory to provide structural priors, improving accuracy of partial dental point cloud completion without needing tooth-position labels.


<details>
  <summary>Details</summary>
Motivation: Partial dental point clouds often have large missing regions due to occlusion and limited scanning views, which bias encoder-only global features and force decoders to hallucinate structures instead of accurately completing missing areas.

Method: Proposes a retrieval-augmented framework that integrates a prototype memory into standard encoder-decoder pipelines. The model encodes partial input into a global descriptor, retrieves the nearest manifold prototype from a learnable memory, fuses it with query features through confidence-gated weighting, then decodes. The memory self-organizes into reusable tooth-shape prototypes without requiring tooth-position labels.
...
Conclusion: The proposed retrieval-augmented framework offers a simple yet effective way to exploit cross-sample regularities for improved dental point-cloud completion, providing structural priors that stabilize missing-region inference and free decoder capacity for detail recovery.
...
<details>
  <summary>Details</summary>
Motivation: While 2D foundation models have strong generalization for dynamic scene analysis, they lack 3D consistency causing spatial misalignment and temporal flickering in complex 3D environments, which is fundamental for understanding scene geometry and motion.

Method: Two-part iterative optimization framework: 1) Sequential optimization updating motion and semantic fields in stages for local consistency, 2) Global optimization jointly refining all attributes for long-term coherence. Includes 3D confidence map for dynamic motion prior adjustment, adaptive resampling for under-represented regions, and iterative semantic refinement with SAM2 prompt updates.

Result: Motion4D significantly outperforms both 2D foundation models and existing 3D-based approaches across diverse scene understanding tasks including point-based tracking, video object segmentation, and novel view synthesis.

Conclusion: Motion4D successfully integrates 2D priors into 4D Gaussian Splatting to achieve 3D-consistent dynamic scene understanding, addressing key limitations of current foundation models while maintaining their generalization capabilities.
...
TL;DR: LAMP uses LLMs as motion planners to translate natural language into 3D trajectories for objects and cameras via a motion DSL, enabling better motion control from text.


<details>
  <summary>Details</summary>
Motivation: Existing video generation interfaces have limited motion control capabilities for specifying object dynamics and camera trajectories, which are essential for creating complex cinematic scenes. Current methods lack effective ways to translate natural language descriptions into precise 3D motion specifications.

Method: LAMP leverages large language models as motion planners with a motion domain-specific language inspired by cinematography conventions. It uses LLMs' program synthesis capabilities to generate structured motion programs from natural language, which are then deterministically mapped to 3D trajectories. The authors also construct a large-scale procedural dataset pairing text descriptions with motion programs and trajectories.

Result: Experiments show LAMP achieves improved performance in motion controllability and alignment with user intent compared to state-of-the-art alternatives. It establishes the first framework for generating both object and camera motions directly from natural language specifications.

Conclusion: LAMP successfully bridges the gap between natural language descriptions and precise 3D motion control for video generation, enabling more complex and cinematic scene composition through LLM-powered motion planning.
...
TL;DR: ReCamDriving is a vision-based framework that generates novel-trajectory driving videos using 3D Gaussian Splatting (3DGS) renderings for precise camera control and geometric guidance, overcoming limitations of repair-based and LiDAR-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods have limitations: repair-based approaches fail to restore complex artifacts, while LiDAR-based methods rely on sparse and incomplete cues. There's a need for precise camera-controllable video generation with explicit geometric guidance for novel driving trajectories.

Method: Two-stage training: 1) Uses camera poses for coarse control, 2) Incorporates 3DGS renderings for fine-grained viewpoint and geometric guidance. Also introduces 3DGS-based cross-trajectory data curation to eliminate train-test gaps in camera transformations, enabling scalable multi-trajectory supervision from monocular videos.

Result: Created ParaDrive dataset with over 110K parallel-trajectory video pairs. Extensive experiments show state-of-the-art camera controllability and structural consistency in generated driving videos.

Conclusion: ReCamDriving successfully addresses limitations of previous methods by leveraging dense 3DGS renderings for explicit geometric guidance, achieving precise camera control through a two-stage training approach and novel data curation strategy.
...
### [63] [FeatureLens: A Highly Generalizable and Interpretable Framework for Detecting Adversarial Examples Based on Image Features](https://arxiv.org/abs/2512.03625)
...
### [64] [MKSNet: Advanced Small Object Detection in Remote Sensing Imagery with Multi-Kernel and Dual Attention Mechanisms](https://arxiv.org/abs/2512.03640)
*Jiahao Zhang,Xiao Zhao,Guangyu Gao*

Main category: cs.CV

TL;DR: MKSNet introduces a Multi-Kernel Selection mechanism with adaptive kernel sizing and dual attention to improve small object detection in high-resolution remote sensing imagery.


<details>
  <summary>Details</summary>
Motivation: Small object detection in remote sensing faces challenges due to information loss in deep CNN layers, spatial redundancy, and complex backgrounds that obscure small targets in high-resolution images.

Method: Proposes MKSNet with Multi-Kernel Selection mechanism using large convolutional kernels for contextual information capture and adaptive kernel sizing, plus dual attention mechanism combining spatial attention (adjusts spatial weights) and channel attention (optimizes channel selection).
...
Conclusion: MKSNet effectively handles multi-scale, high-resolution remote sensing data complexities, demonstrating innovation and superior performance in small object detection.
...
### [65] [Optical Context Compression Is Just (Bad) Autoencoding](https://arxiv.org/abs/2512.03643)
*Ivan Yee Lee,Cheng Yang,Taylor Berg-Kirkpatrick*

Main category: cs.CV

TL;DR: Vision-based text compression (like DeepSeek-OCR) shows good reconstruction but doesn't actually help language modeling; simple alternatives work better.


<details>
  <summary>Details</summary>
Motivation: To test whether vision-based context compression actually benefits language modeling, beyond just text reconstruction.
...
Result: Simple approaches match or surpass vision for text reconstruction, and outperform it for language modeling; vision-based compression fails to beat truncation.

Conclusion: The excitement around optical context compression outpaces the evidence; vision-based compression doesn't provide unique advantages for language modeling.
...
### [66] [Multi-Scale Visual Prompting for Lightweight Small-Image Classification](https://arxiv.org/abs/2512.03663)
*Salim Khazem*

Main category: cs.CV

TL;DR: MSVP introduces a multi-scale visual prompting method that improves performance on small-image benchmarks like MNIST, Fashion-MNIST, and CIFAR-10 with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Visual prompting research has focused on large Vision Transformers and high-resolution datasets, while small-image benchmarks widely used in education, prototyping, and research have been neglected. There's a need for efficient prompting methods that work well on low-resolution images.

Method: Multi-Scale Visual Prompting (MSVP) learns global, mid-scale, and local prompt maps that are fused with input images via lightweight 1×1 convolution. The method is backbone-agnostic, adds less than 0.02% parameters, and works with both CNN and Vision Transformer architectures.

Result: MSVP consistently improves performance across MNIST, Fashion-MNIST, and CIFAR-10 benchmarks using simple CNN, ResNet-18, and small Vision Transformer backbones, with negligible computational overhead. Ablation studies confirm the effectiveness of multi-scale prompting.

Conclusion: Multi-scale visual prompting provides an effective inductive bias even on low-resolution images, offering a simple, generic, and efficient solution for adapting vision models on small-image benchmarks.
...
### [67] [ToG-Bench: Task-Oriented Spatio-Temporal Grounding in Egocentric Videos](https://arxiv.org/abs/2512.03666)
*Qi'ao Xu,Tianwen Qian,Yuqian Fu,Kailing Li,Yang Jiao,Jiacheng Zhang,Xiaoling Wang,Liang He*

Main category: cs.CV

TL;DR: ToG-Bench: First task-oriented spatio-temporal video grounding benchmark for egocentric videos, requiring object localization based on intended tasks rather than descriptions, with explicit-implicit dual grounding and one-to-many grounding capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing STVG studies focus on object-centric descriptive instructions but neglect task-oriented reasoning crucial for embodied agents to accomplish goal-directed interactions. Need to bridge perception and interaction in embodied scenarios.

Method: Built benchmark on ScanNet videos with 100 annotated clips and 2,704 task-oriented grounding instructions using semi-automated pipeline combining foundation model annotation and human refinement. Introduced task-level evaluation metrics for multi-object and explicit-implicit grounding.

Result: Benchmarked 7 state-of-the-art MLLMs, revealing intrinsic challenges of task-oriented STVG and substantial performance gaps across explicit-implicit and multi-object grounding, showing difficulty in bridging perception and interaction.

Conclusion: ToG-Bench addresses limitations of existing STVG benchmarks by introducing task-oriented grounding, explicit-implicit dual grounding, and one-to-many grounding, highlighting challenges for embodied intelligence and providing evaluation framework for future research.
...
### [68] [Colon-X: Advancing Intelligent Colonoscopy from Multimodal Understanding to Clinical Reasoning](https://arxiv.org/abs/2512.03667)
*Ge-Peng Ji,Jingyi Liu,Deng-Ping Fan,Nick Barnes*

Main category: cs.CV

TL;DR: Colon-X introduces a multimodal colonoscopy initiative with ColonVQA dataset (1.1M+ VQA entries) and ColonReason reasoning dataset, plus ColonR1 model that outperforms SFT by 25.22% under data-scarce conditions.


<details>
  <summary>Details</summary>
Motivation: To advance multimodal intelligence in colonoscopy by addressing the gap between multimodal understanding and clinical reasoning, and to provide community-wide data foundation for colonoscopy AI research.

Method: 1) Built ColonVQA dataset with 1.1M+ VQA entries across 76 clinical findings and 18 tasks. 2) Evaluated 22 MLLMs for robustness. 3) Created ColonReason dataset via multi-expert debating pipeline. 4) Developed ColonR1 model with task-adaptive rewarding and gradient-stable optimization.

Result: MLLMs show poor robustness in clinical outputs. ColonR1 achieves 56.61% overall accuracy under data-scarce conditions, outperforming supervised fine-tuning by 25.22%, setting new reasoning baseline for colonoscopy analysis.

Conclusion: Colon-X establishes comprehensive multimodal datasets and introduces reasoning-centric ColonR1 model that significantly advances clinical reasoning capabilities in colonoscopy AI, with all resources publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion transformers face deployment challenges due to increasing memory footprint and inference latency as model size grows. Existing rotation-based quantization methods from LLMs have high overhead and struggle with row-wise outliers in diffusion transformers.
...
Conclusion: ConvRot effectively addresses deployment challenges of diffusion transformers through efficient 4-bit quantization, enabling practical deployment with significant performance improvements while preserving visual quality.
...
### [70] [GaussianBlender: Instant Stylization of 3D Gaussians with Disentangled Latent Spaces](https://arxiv.org/abs/2512.03683)
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-3D stylization methods require time-intensive per-asset optimization and suffer from multi-view inconsistency due to limitations of text-to-image models, making them impractical for large-scale production in game development, VR, and digital arts.
...
### [71] [Active Visual Perception: Opportunities and Challenges](https://arxiv.org/abs/2512.03687)
*Yian Li,Xiaoyu Guo,Hao Zhang,Shuiwang Li,Xiaowei Dai*

Main category: cs.CV

TL;DR: This paper provides a comprehensive overview of active visual perception - systems that dynamically engage with environments through sensing and action to modify behavior based on goals or uncertainties.


<details>
  <summary>Details</summary>
Motivation: Active visual perception offers significant advantages over passive systems by enabling dynamic interaction with environments, but faces challenges in real-time processing, decision-making, and multimodal integration that need to be addressed for broader adoption.

Method: The paper appears to be a review/survey paper that explores both opportunities and challenges in active visual perception, providing a comprehensive overview of its potential and current research landscape.

Result: The paper identifies active visual perception as a critical approach for complex environments where static sensing is insufficient, highlighting its applications in robotics, autonomous vehicles, HCI, and surveillance systems.

Conclusion: Active visual perception shows significant promise but requires overcoming challenges in real-time processing, dynamic decision-making, and multimodal integration to achieve broader adoption and realize its full potential.
...
### [72] [Out-of-the-box: Black-box Causal Attacks on Object Detectors](https://arxiv.org/abs/2512.03730)
...
<details>
  <summary>Details</summary>
Motivation: Existing adversarial perturbation methods are mostly white-box, architecture-specific, and lack explainability. There's a need for black-box attacks that reveal why attacks work, helping developers understand vulnerabilities and improve model robustness.
...
Result: BlackCAtt outperforms baseline methods significantly: 2.7× better at removing detections, 3.86× better at changing detections, and 5.75× better at triggering spurious detections on COCO dataset. Attacks are imperceptible and architecture-agnostic.

Conclusion: Causal pixel identification enables more precise, less perceptible adversarial attacks on object detectors. BlackCAtt demonstrates the power of causal analysis for creating explainable black-box attacks that work across different detector architectures.
...
### [73] [Structured Uncertainty Similarity Score (SUSS): Learning a Probabilistic, Interpretable, Perceptual Metric Between Images](https://arxiv.org/abs/2512.03701)
*Paula Seidler,Neill D. F. Campbell,Ivor J A Simpson*

Main category: cs.CV

TL;DR: SUSS is a new perceptual similarity score that models images as structured multivariate Normal distributions, trained self-supervised to align with human vision while being interpretable.
...
Method: SUSS models each image through perceptual components represented as structured multivariate Normal distributions. It's trained in a generative, self-supervised manner to assign high likelihood to human-imperceptible augmentations. The final score is a weighted sum of component log-probabilities with weights learned from human perceptual datasets. Unlike feature-based methods, it learns image-specific linear transformations of residuals in pixel space.

Result: SUSS aligns closely with human perceptual judgments, shows strong perceptual calibration across diverse distortion types, and provides localized, interpretable explanations through decorrelated residuals and sampling. It also demonstrates stable optimization behavior and competitive performance when used as a perceptual loss for downstream imaging tasks.

Conclusion: SUSS offers a novel approach to perceptual similarity that combines human alignment with interpretability, addressing limitations of both deep feature-based methods and hand-crafted measures, while enabling transparent inspection of similarity assessments.
...
TL;DR: DINO-RotateMatch combines dataset-adaptive image pairing with rotation-aware keypoint extraction for improved image matching in large-scale 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: Address challenges of image matching in large-scale 3D reconstruction from unstructured Internet images, which requires robust and scalable solutions for handling diverse orientations and large collections.

Method: Integrates dataset-adaptive image pairing using DINO for semantic retrieval, with rotation-based augmentation for orientation-dependent local features using ALIKED and Light Glue for keypoint extraction and matching.
...
### [76] [AdaptVision: Efficient Vision-Language Models via Adaptive Visual Acquisition](https://arxiv.org/abs/2512.03794)
*Zichuan Lin,Yicheng Liu,Yang Yang,Lvfang Tao,Deheng Ye*

Main category: cs.CV

TL;DR: AdaptVision is an efficient VLM that adaptively acquires visual tokens using a coarse-to-fine approach with reinforcement learning, achieving better performance with fewer tokens than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current VLMs use many visual tokens causing computational overhead, and existing efficient approaches use fixed compression ratios without adapting to task requirements. The paper aims to enable VLMs to autonomously determine the minimum visual tokens needed per sample.

Method: AdaptVision uses coarse-to-fine visual token acquisition: starts with compressed tokens from low-res images, then selectively invokes a bounding box tool to crop key regions when needed. Trained with reinforcement learning using Decoupled Turn Policy Optimization (DTPO), which separates tool learning and accuracy improvement objectives with separate advantage estimation.

Result: AdaptVision achieves superior performance across multiple VQA benchmarks while consuming substantially fewer visual tokens than state-of-the-art efficient VLM methods.

Conclusion: The proposed adaptive visual token acquisition paradigm enables VLMs to balance accuracy and efficiency effectively, outperforming fixed-ratio compression methods through autonomous determination of required visual information per sample.
...
TL;DR: PosA-VLA: A Vision-Language-Action framework that uses pose-conditioned supervision to anchor visual attention, reducing redundant actions and improving precision in embodied tasks.
...
Method: Proposes PosA-VLA framework with pose-conditioned supervision to anchor visual attention toward task-relevant regions, using pose-conditioned anchor attention mechanism to align instruction semantics with actionable visual cues. Lightweight architecture with no auxiliary perception modules needed.
...
Conclusion: PosA-VLA effectively addresses redundant action issues in VLAs through pose-conditioned attention anchoring, improving action precision and efficiency without requiring additional perception modules.
...
<details>
  <summary>Details</summary>
Motivation: Current cardiac image analysis is fragmented across separate tasks (segmentation, classification, report generation) requiring different networks and training regimes, lacking a unified framework that works across imaging modalities and datasets.
...
Result: PULSE learns task-invariant cardiac priors, generalizes robustly across datasets, and can adapt to new imaging modalities with minimal supervision, unifying pixel-to-structure-to-clinical-reasoning in one architecture.

Conclusion: PULSE moves the field toward a scalable, foundation-style cardiac analysis framework that overcomes fragmentation and enables comprehensive analysis within a single unified architecture.
...
<details>
  <summary>Details</summary>
Motivation: The two-stage learning pipeline in USL-VI-ReID introduces modality bias where modality-specific cues from single-modality training propagate into cross-modality learning, impairing identity discrimination and generalization.

Method: Proposes Dual-level Modality Debiasing Learning (DMDL) with: 1) Causality-inspired Adjustment Intervention (CAI) module using causal modeling instead of likelihood-based modeling to prevent modality-induced spurious patterns; 2) Collaborative Bias-free Training (CBT) strategy that interrupts bias propagation across data, labels, and features through modality-specific augmentation, label refinement, and feature alignment.
...
Conclusion: The proposed DMDL framework effectively addresses modality bias in USL-VI-ReID through dual-level debiasing, leading to improved modality-invariant feature learning and model generalization.
...
TL;DR: CodeVision: A code-as-tool framework for MLLMs that generates code as universal interface for image operations, improving robustness to orientation changes and enabling flexible tool composition.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs are brittle with significant performance degradation on images with simple orientation changes or natural corruptions, highlighting need for more robust tool-based reasoning beyond narrow fixed tool registries.

Method: Propose CodeVision framework where model generates code as universal interface to invoke any image operation. Two-stage training: SFT on high-quality dataset for multi-turn tool composition and error recovery, followed by RL with dense process reward function for strategic tool use.

Result: Significantly improves model performance on Qwen2.5-VL and Qwen3-VL series, fosters emergent capabilities: flexible tool composition, efficient chained execution, robust error recovery from runtime feedback.

Conclusion: CodeVision provides flexible and scalable code-as-tool framework that addresses brittleness of current MLLMs, enabling robust tool-based reasoning through code generation as universal interface for image operations.
...
TL;DR: SelfDebias is an unsupervised test-time debiasing method for diffusion models that identifies semantic clusters in embedding space and guides the diffusion process to minimize bias without requiring annotated data or external classifiers.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models trained on large-scale internet datasets inherit and reproduce societal biases present in the training data, leading to stereotypical outputs. Existing debiasing approaches often require supervised data or external classifiers, which are impractical for many applications.

Method: SelfDebias operates at inference time by identifying semantic clusters in an image encoder's embedding space and using these clusters to guide the diffusion process. It minimizes the KL divergence between the output distribution and a uniform distribution, effectively reducing bias without human supervision or external classifiers.

Result: Extensive experiments show SelfDebias generalizes across prompts and diffusion model architectures (both conditional and unconditional). It effectively debiases images along demographic dimensions while maintaining visual fidelity, and also handles abstract concepts where bias identification is challenging.

Conclusion: SelfDebias provides a practical, unsupervised solution for debiasing diffusion models that doesn't require annotated data or external classifiers, making it widely applicable across different model architectures and capable of addressing both demographic and abstract biases.
...
### [82] [BlurDM: A Blur Diffusion Model for Image Deblurring](https://arxiv.org/abs/2512.03979)
*Jin-Ting He,Fu-Jen Tsai,Yan-Tsung Peng,Min-Hung Chen,Chia-Wen Lin,Yen-Yu Lin*

Main category: cs.CV

TL;DR: BlurDM integrates blur formation process into diffusion models for dynamic scene deblurring, achieving state-of-the-art performance by simultaneously denoising and deblurring in latent space.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models for deblurring fail to leverage the intrinsic nature of the blurring process, limiting their full potential. Motion blur stems from continuous exposure, which should be properly modeled within the diffusion framework.

Method: BlurDM uses a dual-diffusion forward scheme that diffuses both noise and blur onto sharp images. During reverse generation, it employs a dual denoising and deblurring formulation. The model operates in latent space for efficient integration into deblurring networks.

Result: Extensive experiments show BlurDM significantly and consistently enhances existing deblurring methods on four benchmark datasets, demonstrating superior performance.

Conclusion: BlurDM successfully integrates blur formation into diffusion models, providing a flexible prior generation network that effectively addresses dynamic scene deblurring by modeling the intrinsic blurring process.
...
TL;DR: DIQ-H is the first benchmark for evaluating Vision-Language Model robustness under dynamic visual degradation in temporal sequences, focusing on hallucination persistence and error recovery in continuous visual streams.


<details>
  <summary>Details</summary>
Motivation: Existing VLM benchmarks focus on static, high-quality images and ignore critical failure modes in real-world deployments where transient visual corruption induces hallucinations that persist across subsequent frames in continuous visual streams under imperfect conditions.
...
Result: Experiments on 16 state-of-the-art VLMs reveal substantial robustness gaps: GPT-4o achieves only 78.5% recovery rate, while open-source models struggle with temporal consistency (<60%). UIR achieves 15.3% accuracy improvement in pseudo-ground-truth generation.

Conclusion: DIQ-H provides a comprehensive platform for evaluating VLM reliability in real-world deployments, highlighting critical robustness gaps that need to be addressed for safety-critical applications like autonomous driving.
...
TL;DR: LSRS improves VAR image generation by using rejection sampling to refine latent token maps at each scale, reducing structural errors with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: VAR models generate multiple tokens per scale in parallel, which can cause structural errors and suboptimal image quality due to error accumulation across scales.

Method: Latent Scale Rejection Sampling (LSRS) uses a lightweight scoring model to evaluate multiple candidate token maps at each scale, selecting high-quality maps to guide subsequent generation, prioritizing early scales for structural coherence.

Result: LSRS reduces VAR-d30's FID from 1.95 to 1.78 with only 1% additional inference time, and to 1.66 with 15% additional time, significantly improving generation quality with minimal overhead.
...
TL;DR: The paper introduces Text Embedding Perturbation (TEP) for test-time scaling in text-to-image diffusion models, which complements spatial noise randomness by enhancing high-frequency details in later diffusion steps.


<details>
  <summary>Details</summary>
Motivation: Current test-time scaling methods for T2I diffusion models focus on search strategies and reward models, but overlook the impact of stochastic noise characteristics. The authors aim to explore how different forms of randomness affect generation quality and diversity.

Method: The method introduces step-based text embedding perturbation that combines with spatial noise. It uses frequency-guided noise schedules and adapts perturbation intensity based on frequency-specific contributions and tolerance levels across different dimensions of the generation process.

Result: The approach shows significant improvements on multiple benchmarks with almost no additional computational cost and can be seamlessly integrated into existing test-time scaling methods.

Conclusion: Text embedding perturbation complements spatial noise randomness by enhancing high-frequency details in later diffusion steps, leading to improved generative diversity and quality in T2I diffusion models.
...
### [86] [HieroGlyphTranslator: Automatic Recognition and Translation of Egyptian Hieroglyphs to English](https://arxiv.org/abs/2512.03817)
*Ahmed Nasser,Marwan Mohamed,Alaa Sherif,Basmala Mahmoud,Shereen Yehia,Asmaa Saad,Mariam S. El-Rahmany,Ensaf H. Mohamed*

Main category: cs.CV

TL;DR: Proposes a three-stage deep learning method for automatic recognition and translation of Egyptian hieroglyphs from images to English, achieving a BLEU score of 42.2.


<details>
  <summary>Details</summary>
Motivation: Egyptian hieroglyphs present translation challenges due to their pictorial nature and multiple meanings per glyph. Deep learning offers potential for automated translation systems that could significantly impact historical and cultural studies.

Method: Three-stage approach: 1) Segmentation using Contour and Detectron2, 2) Mapping symbols to Gardiner codes, 3) Translation using CNN model. Utilized Morris Franken and EgyptianTranslation datasets.

Result: Achieved a BLEU score of 42.2, which represents significant improvement compared to previous research in hieroglyph translation.

Conclusion: The proposed deep learning method successfully addresses the challenges of hieroglyph translation, demonstrating promising results for automated ancient Egyptian text translation with potential applications in historical research and cultural preservation.
...
TL;DR: DIG is a training-free frame selection framework for long-form video understanding that adapts strategy based on query type: uniform sampling for global queries and query-aware selection for localized queries.


<details>
  <summary>Details</summary>
Motivation: Current LMMs for long-form video understanding face constraints from limited context lengths and high computational costs of processing dense video tokens. Existing query-aware frame selection methods incur significant overhead, but the paper questions whether such complex mechanisms are always necessary.

Method: Proposes DIG framework with two strategies: 1) efficient uniform sampling for global queries, and 2) specialized pipeline to extract query-relevant frames for localized queries. The approach is training-free and adapts based on query type classification.
...
Conclusion: Complex query-aware frame selection is not universally necessary - uniform sampling works well for global queries while localized queries require query-aware selection. DIG provides an adaptive, efficient solution that improves LMM performance for long-form video understanding.
...
### [88] [A Robust Camera-based Method for Breath Rate Measurement](https://arxiv.org/abs/2512.03827)
*Alexey Protopopov*

Main category: cs.CV

TL;DR: A robust video-based breath rate measurement method achieves less than 5% relative deviation from ground truth, with 0.57 RPM MAE, outperforming previous approaches and being resistant to subject movement.


<details>
  <summary>Details</summary>
Motivation: While cheap cameras enable breath rate measurement from video, existing methods either work only in ideal conditions or lack sufficient accuracy. There's a need for a more robust approach that can handle real-world conditions with subject movement.

Method: Combination of mathematical transforms applied to video footage to extract breath rate signals. Minimal hardware requirements - works with standard cameras. Tested on 14 volunteers with over 2.5 hours of video data.

Result: Less than 5% relative deviation from ground truth. Average mean absolute error of 0.57 respirations per minute, significantly better than previous works. Method is resistant to distortions from subject movement.

Conclusion: The proposed method enables accurate remote breath rate measurement without significant limitations on subject behavior, overcoming limitations of previous approaches that required near-ideal conditions.
...
TL;DR: Sketches as sequences: absolute coordinates beat relative ones, non-autoregressive decoders outperform autoregressive ones, and temporal importance depends on order and task.


<details>
  <summary>Details</summary>
Motivation: There's a gap in understanding whether treating sketches as sequences is justified and which internal orders matter most for sketch representation learning.

Method: Investigates sketch representation by comparing different sequence modeling approaches: absolute vs. relative coordinates, and autoregressive vs. non-autoregressive decoders.

Result: Absolute coordinates consistently outperform relative ones for modeling sketches as sequences. Non-autoregressive decoders beat autoregressive counterparts. Temporal importance varies based on order and task.

Conclusion: While treating sketches as sequences is valid, absolute coordinates work better than relative ones, non-autoregressive decoding is superior, and temporal relevance depends on specific order and task context.
...
### [90] [Lean Unet: A Compact Model for Image Segmentation](https://arxiv.org/abs/2512.03834)
*Ture Hassler,Ida Åkerholm,Marcus Nordström,Gabriele Balletti,Orcun Goksel*

Main category: cs.CV

TL;DR: LUnet: A lean Unet architecture with constant channel count across layers achieves comparable performance to standard Unet with 30x fewer parameters, outperforming pruning methods.


<details>
  <summary>Details</summary>
Motivation: Standard Unet architectures require large memory footprint due to iterative downsampling with channel doubling, limiting batch sizes and increasing inference latency. Channel pruning requires lengthy optimization and may not generalize well across tasks.

Method: Propose LUnet with compact, flat hierarchy where channels are NOT doubled as resolution is halved (constant channel count across layers). Skip connections allow bottleneck channels to be largely reduced. Compare with state-of-the-art pruning (STAMP) and show that random channel elimination at pruning-identified layers achieves similar performance.

Result: LUnet with over 30 times fewer parameters achieves performance comparable to conventional Unet and data-adaptively pruned networks. For same total parameters, LUnet outperforms standard Unet. Skip connections enable significant bottleneck channel reduction.

Conclusion: The final architecture structure is more crucial than channel selection strategy in pruning. Lean Unet with constant channel count provides efficient alternative to standard Unet and pruning methods, achieving similar performance with dramatically fewer parameters.
...
TL;DR: PSA introduces multi-level pooled key-value representations for sparse attention, enabling finer granularity than binary masks to reduce information loss while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Quadratic complexity of attention mechanisms limits scaling of foundation models. Current sparse attention methods use binary masks that discard entire KV blocks, causing substantial information loss under high sparsity.
...
Result: PSA consistently outperforms or achieves comparable performance to existing sparse attention baselines across video understanding and generation benchmarks, with superior efficiency-quality trade-offs while preserving contextual information and visual fidelity.

Conclusion: PSA effectively mitigates information loss in sparse attention mechanisms through multi-level pooled representations, offering a hardware-friendly solution that balances computational efficiency with performance preservation.
...
TL;DR: HP-Net: A novel heatmap pooling network for human action recognition that extracts robust, concise pooled features from videos to address redundancy, noise, and storage issues in RGB video analysis.


<details>
  <summary>Details</summary>
Motivation: Existing RGB video HAR methods suffer from information redundancy, noise susceptibility, and high storage costs. The paper aims to fully harness useful video information by extracting more efficient and robust features.

Method: Proposes HP-Net with: 1) Feedback pooling module to extract information-rich, robust, concise pooled features from human body heatmaps; 2) Spatial-motion co-learning module; 3) Text refinement modulation module to integrate pooled features with multimodal data.
...
Conclusion: HP-Net effectively addresses limitations of existing RGB video HAR methods by extracting superior pooled features and integrating multimodal data, achieving state-of-the-art performance across multiple benchmarks.
...
### [93] [Fast & Efficient Normalizing Flows and Applications of Image Generative Models](https://arxiv.org/abs/2512.04039)
*Sandeep Nagar*

Main category: cs.CV

TL;DR: This thesis presents innovations in normalizing flow efficiency (invertible convolutions, Quad-coupling, parallel inversion algorithms) and applies generative models to real-world computer vision tasks including agricultural quality assessment, geological mapping, privacy preservation, and art restoration.


<details>
  <summary>Details</summary>
Motivation: To advance the efficiency of generative models (particularly normalizing flows) and demonstrate their practical utility in solving diverse real-world computer vision challenges across multiple domains.

Method: Part 1: Six architectural improvements to normalizing flows including invertible 3x3 convolutions with mathematical proofs, Quad-coupling layers, parallel inversion algorithms, efficient backpropagation, Inverse-Flow training, and Affine-StableSR super-resolution. Part 2: Application of generative models including Conditional GANs for agricultural assessment, stacked autoencoders for geological mapping, face detection/inpainting for privacy preservation, Stable Diffusion for privacy protection, and adapted diffusion models for art restoration.

Result: Developed more efficient normalizing flow architectures with mathematical guarantees, and successfully applied generative models to practical problems: achieved good accuracy in seed purity testing, improved feature extraction for geological mapping, effective privacy preservation for autonomous driving data, and unified handling of multiple degradation types in art restoration.

Conclusion: The thesis demonstrates both theoretical advancements in normalizing flow efficiency and practical applications of generative models across diverse computer vision domains, showing the versatility and real-world utility of these approaches while addressing important challenges like computational efficiency, data scarcity, and privacy concerns.
...
TL;DR: CoDA enables dataset distillation using only off-the-shelf text-to-image models by identifying the target dataset's intrinsic core distribution and aligning generated samples with it, achieving SOTA performance without target-specific model training.


<details>
  <summary>Details</summary>
Motivation: Current dataset distillation methods have two key limitations: 1) Most diffusion-based methods require pre-training on the full target dataset, which undermines the purpose of DD and is computationally expensive. 2) Methods using general text-to-image models suffer from distributional mismatch between web-scale priors and target-specific semantics.

Method: CoDA first identifies the "intrinsic core distribution" of the target dataset using a robust density-based discovery mechanism. It then steers the generative process of an off-the-shelf text-to-image model to align generated samples with this core distribution, bridging the gap between general-purpose priors and target semantics.

Result: CoDA achieves performance on par with or superior to previous methods that require target-specific training, establishing new SOTA accuracy of 60.4% at 50 IPC on ImageNet-1K. It works effectively across all benchmarks including ImageNet-1K and its subsets.

Conclusion: CoDA successfully addresses the limitations of existing DD methods by enabling effective dataset distillation using only off-the-shelf text-to-image models, eliminating the need for expensive target-specific training while achieving superior performance through core distribution alignment.
...
TL;DR: FAMamba is a novel frequency-aware Mamba framework for traffic image restoration under adverse weather, integrating frequency guidance with sequence modeling through dual-branch feature extraction and prior-guided wavelet refinement.


<details>
  <summary>Details</summary>
Motivation: Existing traffic image restoration methods focus on spatial-domain modeling but neglect frequency-domain priors. While Mamba architecture excels at long-range dependency modeling, its potential for frequency-domain feature extraction remains unexplored, especially for adverse weather conditions in intelligent transportation systems.
...
Conclusion: FAMamba successfully integrates frequency guidance with sequence modeling, leveraging both spatial and frequency domain information for high-quality traffic image restoration with precise details, addressing a critical challenge for intelligent transportation systems.
...
TL;DR: Public release of 339 prostate biopsy whole-slide images from 185 Iraqi patients to address lack of Middle Eastern representation in pathology AI datasets and enable validation of models across diverse populations.


<details>
  <summary>Details</summary>
Motivation: Current histopathology datasets are scarce and predominantly represent Western populations, limiting the generalizability of AI models to less digitized regions like the Middle East. There's a need for diverse datasets to validate pathology AI models across globally diverse populations.

Method: Collection of 339 whole-slide images from prostate core needle biopsies of 185 consecutive patients in Erbil, Iraq. Slides were scanned using three different scanners (Leica, Hamamatsu, Grundium) and associated with Gleason scores and ISUP grades assigned independently by three pathologists. All slides were de-identified and provided in native formats without conversion.

Result: Creation of a publicly available dataset deposited in the Bioimage Archive under CC BY 4.0 license, enabling grading concordance analyses, color normalization studies, and cross-scanner robustness evaluations for pathology AI models.

Conclusion: This dataset addresses the geographic bias in existing histopathology datasets and supports the development and validation of AI models that can generalize to diverse global populations, particularly underrepresented Middle Eastern populations.
...
TL;DR: Small 5M-parameter vision transformers benefit from targeted pre-training but suffer from indiscriminate intermediate fine-tuning, which can degrade performance due to task dissimilarity.


<details>
  <summary>Details</summary>
Motivation: Transformers require large parameters and data for strong performance, but this paper explores how small-scale ViTs (5M parameters) can benefit from different training strategies to optimize performance with limited resources.

Method: Experimented with three distinct pre-training, intermediate fine-tuning, and downstream datasets and training objectives on a small 5M-parameter vision transformer to explore marginal benefits of each stage.

Result: Pre-training and fine-tuning always help but have diminishing returns; intermediate fine-tuning can actually harm downstream performance due to task dissimilarity; targeted pre-training and careful data selection are most beneficial.

Conclusion: Small-scale ViTs benefit most from targeted pre-training and careful data selection, while indiscriminate stacking of intermediate tasks wastes compute and can degrade performance.
...
TL;DR: CaravelMetrics is an automated framework for analyzing brain blood vessels using graph-based representations from 3D scans, extracting 15 features to study vascular health and aging.


<details>
  <summary>Details</summary>
Motivation: There's a need for scalable, automated tools to quantitatively analyze cerebrovascular networks from medical imaging data to support population-level studies of vascular health, aging, and disease.

Method: The framework uses skeletonization to create graph representations of blood vessels, integrates atlas-based regional parcellation, centerline extraction, and graph construction to compute 15 morphometric, topological, fractal, and geometric features at global and regional scales.
...
<details>
  <summary>Details</summary>
Motivation: Watch-and-wait surveillance for rectal cancer patients with clinical complete response needs objective methods to detect local regrowth early from follow-up endoscopy images to prevent distant metastases.

Method: Developed SSDCA combining longitudinal endoscopic images at restaging and follow-up using pretrained Swin transformers for domain-agnostic features and dual cross-attention to emphasize features from both scans without requiring spatial alignment.

Result: SSDCA achieved best performance: 81.76% balanced accuracy, 90.07% sensitivity, 72.86% specificity on held-out test set of 62 patients. Robust to artifacts and showed discriminative feature representation with maximal inter-cluster separation and minimal intra-cluster dispersion.

Conclusion: SSDCA provides accurate, robust detection of local regrowth from longitudinal endoscopic images without requiring spatial alignment, supporting watch-and-wait surveillance for rectal cancer patients.
...
<details>
  <summary>Details</summary>
Motivation: Current zero-shot video editing methods using image diffusion models rely on inter-frame correspondence in attention mechanisms, but this soft constraint is insufficient and leads to temporal inconsistency in manipulated videos.

Method: FRESCO integrates both intra-frame and inter-frame correspondence to create a more robust spatial-temporal constraint, ensuring consistent transformation of semantically similar content between frames. It goes beyond attention guidance to explicitly optimize features.

Result: The method achieves high spatial-temporal consistency with input videos, significantly enhancing visual coherence. It demonstrates effectiveness in video-to-video translation and text-guided video editing, generating high-quality, coherent videos that outperform current zero-shot methods.

Conclusion: FRESCO represents a significant advance over current zero-shot video editing methods by providing better temporal consistency through integrated spatial-temporal constraints, enabling more coherent video manipulation without additional model training.
...
Method: Models videos and 3D motions as unified token sequences using separate embedding layers to mitigate distribution gaps. Uses a novel 3D motion tokenizer with temporal expansion strategy and multiple expert decoders for reliable motion reconstruction. Integrates two distinct tasks within a single autoregressive framework.

Result: Extensive experiments demonstrate simultaneous generation of corresponding videos and motions while performing accurate motion capture. Proves effectiveness of unified modeling approach.
...
TL;DR: A framework that enhances ground-truth images for better supervision in real-world image restoration, using frequency-domain mixup and adaptive masks to improve perceptual quality without hallucinated artifacts.


<details>
  <summary>Details</summary>
Motivation: Real-world image restoration is limited by imperfect ground-truth images due to practical data acquisition constraints, creating a need for enhanced supervision beyond existing datasets.

Method: Proposes a framework with: 1) Conditional frequency mask generator to create adaptive masks, 2) Frequency-domain mixup that optimally fuses original and super-resolved ground truth components, 3) Lightweight output refinement network trained on enhanced ground truth.
...
Conclusion: The framework successfully addresses limitations of imperfect ground-truth data in real-world restoration, providing enhanced supervision that improves restoration quality while maintaining semantic fidelity.
...
TL;DR: MUT3R is a training-free framework that uses attention-derived motion cues from pretrained transformers to suppress dynamic content in early layers, improving 3D reconstruction in streaming scenarios without retraining.
...
Method: Analyze self-attention maps across transformer layers to identify dynamic regions that are naturally down-weighted. Use these attention-derived motion cues to suppress dynamic content in early transformer layers during inference via an attention-level gating module, preventing artifact propagation without retraining.

Result: Improves temporal consistency and camera pose robustness across multiple dynamic benchmarks. Stabilizes geometric reasoning in streaming scenarios by suppressing motion artifacts before they propagate through the feature hierarchy.

Conclusion: MUT3R offers a simple, training-free pathway toward motion-aware streaming reconstruction by letting pretrained transformers diagnose their own motion cues and self-correct, demonstrating that existing models already contain useful motion information that can be leveraged without additional training.
...
TL;DR: TempR1 is a temporal-aware multi-task reinforcement learning framework that enhances Multimodal Large Language Models' temporal understanding for video analysis through systematic multi-task optimization with tailored localization rewards.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches for temporal reasoning in MLLMs are limited to specific tasks and data, restricting generalization across diverse temporal understanding scenarios needed for long-form video analysis tasks like temporal localization and action detection.

Method: TempR1 uses a multi-task RL framework with curated corpus exposing models to diverse temporal structures. It builds on GRPO algorithm and categorizes temporal tasks into three correspondence types between predicted intervals and ground-truth, designing tailored localization rewards for each to capture fine-grained temporal dependencies.
...
Conclusion: TempR1 establishes a scalable and principled paradigm for temporal reasoning in MLLMs, demonstrating that systematic multi-task reinforcement learning can effectively enhance temporal comprehension for diverse video analysis applications.
...
Conclusion: UniID successfully unifies text embedding and adapter-based approaches through a principled design that enables high-fidelity face personalization with flexible text controllability, addressing the limitations of existing tuning-free methods.
...
TL;DR: DirectDrag is a mask- and prompt-free image editing framework that enables precise drag-based manipulation with minimal user input while maintaining high image fidelity and accurate point alignment.


<details>
  <summary>Details</summary>
Motivation: Existing drag-based image editing methods rely heavily on manually provided masks and textual prompts to preserve semantic fidelity and motion precision. Removing these constraints creates a trade-off: visual artifacts without masks and poor spatial control without prompts.
...
Conclusion: DirectDrag provides a novel mask- and prompt-free editing framework that enables precise and efficient manipulation with minimal user input while maintaining high image fidelity and accurate point alignment, addressing fundamental limitations in existing drag-based image editing methods.
...
TL;DR: Feed-forward 3D reconstruction models like VGGT have an implicit ability to distinguish and suppress noisy/distractor images without explicit outlier rejection mechanisms, enabling effective in-the-wild reconstruction.


<details>
  <summary>Details</summary>
Motivation: Traditional Structure-from-Motion pipelines handle noisy images through geometric verification, but feed-forward 3D reconstruction models lack these explicit mechanisms, leading to degraded performance with in-the-wild image collections containing irrelevant inputs.

Method: Analyzed VGGT model under varying proportions of synthetic distractors, identified a specific layer with natural outlier-suppressing behavior, and leveraged its discriminative internal representations for outlier-view rejection without additional fine-tuning or supervision.

Result: The implicit filtering mechanism is consistent and generalizes well across diverse scenarios, as demonstrated through extensive experiments on both controlled and in-the-wild datasets.

Conclusion: Feed-forward 3D reconstruction models possess inherent noise-filtering capabilities that can be leveraged for reliable in-the-wild reconstruction without explicit outlier rejection mechanisms or additional training.
...
TL;DR: A novel end-to-end framework that automatically learns group actions on latent image manifolds, discovering transformation-relevant structures without manual intervention using learnable binary masks and straight-through estimation.


<details>
  <summary>Details</summary>
Motivation: Prior methods for modeling group actions either operate in high-dimensional data space (making it hard to disentangle transformation-relevant subspaces) or require manual partitioning of latent variables into equivariant/invariant components, limiting robust learning of group actions in representation space.
...
Conclusion: The framework successfully learns group actions on latent image manifolds without manual intervention, automatically discovering transformation-relevant structures and enabling effective disentangled representations for various image datasets.
...
TL;DR: C3G is a feed-forward framework that estimates compact 3D Gaussians at essential spatial locations for efficient scene reconstruction and understanding from unposed sparse views, achieving better memory efficiency and feature fidelity than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current approaches using per-pixel 3D Gaussian Splatting generate excessive redundant Gaussians, causing high memory overhead and sub-optimal multi-view feature aggregation, which degrades novel view synthesis and scene understanding performance.

Method: Proposes C3G framework with learnable tokens that aggregate multi-view features through self-attention to guide Gaussian generation, ensuring each Gaussian integrates relevant visual features across views, then exploits learned attention patterns for efficient Gaussian decoding and feature lifting.

Result: Extensive experiments on pose-free novel view synthesis, 3D open-vocabulary segmentation, and view-invariant feature aggregation demonstrate effectiveness, showing superior memory efficiency and feature fidelity compared to existing methods.

Conclusion: A compact yet geometrically meaningful representation is sufficient for high-quality scene reconstruction and understanding, achieving better performance with reduced redundancy.
...
TL;DR: RELIC is a real-time interactive world model that combines long-horizon streaming, spatial memory, and user control using compressed historical latent tokens and memory-efficient distillation.


<details>
  <summary>Details</summary>
Motivation: Current interactive world models fail to simultaneously achieve real-time long-horizon streaming, consistent spatial memory, and precise user control, often sacrificing one aspect for another.

Method: Built on autoregressive video-diffusion distillation, RELIC uses compressed historical latent tokens with relative actions and camera poses in KV cache for memory. It fine-tunes a bidirectional teacher model for longer sequences and transforms it into a causal student using memory-efficient self-forcing distillation.

Result: RELIC achieves 16 FPS real-time generation with more accurate action following, stable long-horizon streaming, and robust spatial-memory retrieval compared to prior work, implemented as a 14B-parameter model trained on Unreal Engine data.

Conclusion: RELIC provides a unified framework that simultaneously addresses the three key challenges of interactive world modeling, establishing a strong foundation for next-generation interactive world models.
...
Method: Streamlines traditional structure into hierarchical end-to-end generation with two main components: 1) SLUL (Sign Language Understanding Linker) for text understanding (Prompt2Gloss, Text2Gloss) using Semantic-Aware Gloss Masking Loss, and 2) SLP-MoE hand gesture rendering expert block for Pose2Vid generation.

Result: Achieves 48.6% performance improvement compared to current state-of-the-art SLP generation methods, producing high-quality and multi-style sign language videos.

Conclusion: By redefining SLP as a simplified hierarchical end-to-end task and introducing SLUL with SAGM Loss and SLP-MoE rendering, Stable Signer significantly outperforms existing methods while reducing error accumulation in the generation pipeline.
...
<details>
  <summary>Details</summary>
Motivation: VLMs have strong visual understanding but lack precise spatial reasoning needed for embodied applications. While tool augmentation could help, current approaches rely on handcrafted prompting or fixed tool pipelines that limit optimal tool-use discovery.

Method: Double Interactive Reinforcement Learning (DIRL) with two phases: teaching phase combines demonstrations from single-tool specialists trained via interactive RL with traces from frontier models using all tools; exploration phase further refines multi-tool coordination through continued RL.

Result: SpaceTools achieves state-of-the-art performance on spatial understanding benchmarks (RoboSpatial-Home, BLINK, BOP-ASK) with +12% improvement over SFT and +16% over RL baselines on RoboSpatial, and demonstrates reliable real-world manipulation using a 7-DOF robot.

Conclusion: DIRL enables VLMs to effectively coordinate multiple tools for precise spatial reasoning, bridging the gap between qualitative visual understanding and metric spatial reasoning required for embodied applications.
...
TL;DR: PosterCopilot is a framework that improves automated graphic design by enhancing layout reasoning and enabling layer-specific iterative editing through progressive training and integration with generative models.


<details>
  <summary>Details</summary>
Motivation: Existing Large Multimodal Models for automated graphic design produce geometrically inaccurate layouts and lack the iterative, layer-specific editing capabilities required in professional workflows.

Method: A progressive three-stage training strategy: 1) Perturbed Supervised Fine-Tuning, 2) Reinforcement Learning for Visual-Reality Alignment, and 3) Reinforcement Learning from Aesthetic Feedback. Plus a complete workflow coupling the trained LMM with generative models for layer-controllable iterative editing.
...
Conclusion: PosterCopilot advances automated graphic design by addressing geometric accuracy and professional workflow needs through enhanced layout reasoning and controllable editing capabilities.
...
TL;DR: SimFlow simplifies VAE-NF training by fixing VAE encoder variance to a constant, eliminating complex noise pipelines and enabling joint training for better image generation.


<details>
  <summary>Details</summary>
Motivation: Existing Normalizing Flow methods with VAEs have two key limitations: 1) they require complex noise injection and denoising pipelines for data augmentation, and 2) they use frozen pretrained VAE encoders leading to suboptimal reconstruction and generation quality.

Method: Proposes fixing the VAE encoder's variance to a constant (e.g., 0.5) instead of predicting it. This simple modification allows the encoder to output a broader token distribution, enabling the decoder to learn reconstruction from augmented tokens without extra noise injection. It also simplifies the VAE evidence lower bound, making joint training of NF and VAE stable.

Result: SimFlow achieves gFID of 2.15 on ImageNet 256×256 generation, outperforming previous SOTA STARFlow (gFID 2.40). When combined with REPA-E representation alignment, it achieves gFID 1.91, setting new SOTA among Normalizing Flow methods.

Conclusion: Fixing VAE encoder variance to a constant provides a simple yet effective solution to two major limitations in VAE-NF training, eliminating complex noise pipelines while enabling joint optimization for superior generation quality.
...
TL;DR: Single-life learning trains vision models on one person's egocentric videos, showing models develop aligned geometric understanding across different lives, learn generalizable representations, and achieve performance comparable to diverse web data training.


<details>
  <summary>Details</summary>
Motivation: To explore whether training vision models exclusively on egocentric videos from a single individual's life can lead to effective visual representation learning, leveraging the natural multi-view consistency within one person's experiences.

Method: Train distinct vision models on egocentric videos from different individuals (single-life paradigm), use self-supervised learning on multiple viewpoints within each life, and introduce a novel cross-attention-based metric to quantify functional alignment between models.
...
TL;DR: Physics-informed ML framework for predicting steel phase transformations achieves high accuracy and computational efficiency, enabling rapid generation of continuous cooling transformation diagrams.


<details>
  <summary>Details</summary>
Motivation: Applying general-purpose ML to complex industrial materials like steel is challenging due to difficulties capturing intricate relationships between composition, processing, microstructure, and properties. There's a need for specialized frameworks that combine physical insights with ML for practical industrial applications.

Method: Developed a physics-informed continuous cooling transformation (CCT) model for steels by combining physical insights with machine learning. Trained on a dataset of 4,100 CCT diagrams and validated against literature and experimental data.

Result: The model generates complete CCT diagrams with 100 cooling curves in under 5 seconds. Achieves phase classification F1 scores above 88% for all phases and phase transition temperature MAE below 20°C for most phases (except bainite at 27°C). Shows strong generalizability across alloy steels.

Conclusion: The framework demonstrates that physics-informed ML can effectively model complex steel transformations with high accuracy and efficiency. It can be extended to establish a universal digital twin platform for heat treatment and integrated with simulation tools to accelerate materials design workflows.
...
TL;DR: LLMs used as lossless encoder-decoder for invertible problems (LCTs to HDL code) to reduce hallucinations and omissions, validated by reconstructing LCTs from generated HDL.


<details>
  <summary>Details</summary>
Motivation: To mitigate LLM drawbacks like hallucinations and omissions when generating code from specifications, especially for complex hardware design tasks like network-on-chip routers.

Method: Use LLMs as lossless encoder (LCTs → HDL) then decoder (HDL → LCTs), similar to lossless compression. Test with 7 LLMs on 2D network-on-chip router (13 units, 1500-2000 lines HDL), reconstruct LCTs from generated HDL, compare original vs reconstructed LCTs.

Result: Approach yields significant productivity improvements: confirms correctly generated logic, detects incorrectly generated logic, and helps find design specification errors.

Conclusion: Lossless encoder-decoder approach using LLMs effectively mitigates hallucinations/omissions for invertible problems, providing verification mechanism for generated code against original specifications.
...
<details>
  <summary>Details</summary>
Motivation: FL's high resource requirements exclude centers with limited computational infrastructure, widening healthcare disparities. Need for sustainable AI that reduces energy consumption while maintaining performance.

Method: Adaptive layer-freezing strategy that selectively freezes encoder weights based on monitored relative differences between rounds. Uses patience-based mechanism to freeze only when updates remain consistently minimal. Tested with different federated architectures for MRI-to-CT conversion.

Result: Reduced training time, total energy consumption, and CO2eq emissions by up to 23% compared to non-frozen counterparts. Maintained MRI-to-CT conversion performance with only small MAE variations. 3/5 architectures showed no statistically significant differences, 2 showed statistically significant improvements.

Conclusion: The approach promotes sustainable DL frameworks meeting clinical requirements while ensuring climatic, social, and economic sustainability. Lays groundwork for novel FL evaluation frameworks advancing privacy, equity, and justice in AI-driven healthcare.
...
TL;DR: PINS-CAD: A physics-informed self-supervised learning framework that predicts cardiovascular events using synthetic coronary digital twins and clinical data, outperforming traditional risk scores without requiring CFD simulations.


<details>
  <summary>Details</summary>
Motivation: Cardiovascular disease is the leading global cause of death, with coronary artery disease (CAD) being most prevalent. Current 3D coronary artery digital twin analysis relies on computationally intensive CFD simulations, limiting scalability. Data-driven approaches face challenges with scarce labeled data and lack of physiological priors.

Method: PINS-CAD uses physics-informed self-supervised learning with graph neural networks pre-trained on 200,000 synthetic coronary digital twins. The framework predicts pressure and flow guided by 1D Navier-Stokes equations and pressure-drop laws, eliminating the need for CFD or labeled data. The model is then fine-tuned on clinical data from 635 patients in the multicenter FAME2 study.

Result: PINS-CAD predicts future cardiovascular events with an AUC of 0.73, outperforming clinical risk scores and data-driven baselines. The framework generates spatially resolved pressure and fractional flow reserve curves, providing interpretable biomarkers. Physics-informed pretraining boosts sample efficiency and yields physiologically meaningful representations.

Conclusion: By embedding physical priors into geometric deep learning, PINS-CAD transforms routine angiography into a simulation-free, physiology-aware framework for scalable, preventive cardiology, enabling early risk prediction without computationally intensive CFD simulations.
...
TL;DR: Delta Sampling enables knowledge transfer between different diffusion model versions by using prediction differences from adapted models to guide new base models at inference time.
...
TL;DR: The paper analyzes token dynamics in Transformers as continuous-time systems, identifies convergence/divergence regimes based on model parameters, shows convergence harms performance, and proposes architectural refinements to mitigate convergence for both absolute and rotary positional encodings.


<details>
  <summary>Details</summary>
Motivation: To understand the dynamical properties of tokens in pre-trained Transformers and use these insights to improve Transformer architectures by addressing problematic convergence behaviors that degrade model performance.

Method: Analyzes the continuous-time limit of pre-trained Transformers as dynamical systems, characterizes asymptotic token behavior (convergence/divergence), provides parameter-based conditions for different regimes, investigates effects of absolute vs rotary positional encodings, and proposes architectural refinements based on theoretical insights.

Result: Established broader conditions for token convergence/divergence than prior work, showed convergence behavior harms model performance, found positional encoding types affect dynamical regimes, and successfully proposed refinements that mitigate convergence in both absolute and rotary positional encoding models.

Conclusion: The dynamical systems analysis provides theoretical foundations for improving Transformers; architectural refinements based on mitigating convergence behavior can enhance performance, supporting better design principles for Transformer models with different positional encoding schemes.
...
TL;DR: A hierarchical deep reinforcement learning framework for electric bus charging scheduling that minimizes costs while ensuring battery safety under real-world uncertainties.


<details>
  <summary>Details</summary>
Motivation: Electric buses integrated with renewable energy need optimized charging schedules to minimize costs while preventing battery depletion, but real-world uncertainties (PV generation, electricity prices, travel times, limited infrastructure) make this challenging.

Method: Formulate as Constrained Markov Decision Process with options; propose DAC-MAPPO-Lagrangian algorithm combining Lagrangian relaxation with Double Actor-Critic framework. High-level uses centralized PPO-Lagrangian for safe charger allocation, low-level uses MAPPO-Lagrangian for decentralized charging power decisions under CTDE paradigm.

Result: Outperforms existing baselines in both cost minimization and safety compliance while maintaining fast convergence speed, demonstrated through extensive experiments with real-world data.

Conclusion: The proposed safe hierarchical deep reinforcement learning framework effectively solves the electric bus charging scheduling problem under multi-source uncertainties, balancing operational efficiency with safety constraints.
...
TL;DR: Large-scale industrial framework for estimating heterogeneous treatment effects using experimental data from hundreds of millions of Snapchat users, enabling stable treatment effect estimation and uncovering latent user characteristics.


<details>
  <summary>Details</summary>
Motivation: Traditional treatment effect models assume uniform effects across all users, but in reality, treatment effects vary across individuals. There's a need for scalable industrial frameworks that can estimate heterogeneous treatment effects (HTE) and conditional average treatment effects (CATE) using massive experimental data to uncover previously unmeasurable user characteristics.

Method: The framework combines results across many experiments using experiment selection, base learner design, and incremental training. It leverages experimental data from hundreds of millions of Snapchat users to estimate HTE at scale.

Result: The system produces stable treatment effect estimates at scale and uncovers latent user characteristics. Applications include measuring user influenceability to ads and user sensitivity to ads. An online A/B test using influenceability scores for targeting showed improvements on key business metrics more than six times larger than typical significance thresholds.

Conclusion: The framework successfully enables large-scale HTE estimation in industrial settings, demonstrating practical value through applications like ad targeting that significantly outperform conventional approaches, highlighting the importance of personalized treatment effect estimation in real-world applications.
...
TL;DR: Two physics-inspired methods (FermiGrad and PivGa) for improving SVD-based LLM compression by optimizing layer-wise rank selection and eliminating parameter redundancy.


<details>
  <summary>Details</summary>
Motivation: LLMs are computationally expensive, and while SVD-based compression is promising, it faces practical challenges like selecting optimal layer-wise ranks and dealing with parameter redundancy in the low-rank factors.

Method: 1) FermiGrad: Gradient-descent algorithm that finds globally optimal layer-wise ranks by relaxing discrete singular-value truncation into continuous optimization using the Fermi function. 2) PivGa: Lossless compression method that exploits intrinsic gauge freedom in low-rank factor parametrization to eliminate redundancy.

Result: The paper presents two novel improvements to SVD-based LLM compression that address key practical hurdles in the compression process.

Conclusion: Physics-inspired approaches can effectively solve practical challenges in LLM compression, enabling more efficient deployment of large language models through improved SVD-based compression techniques.
...
TL;DR: AI agents in life sciences use Thompson Sampling contextual bandits to learn optimal decision-making strategies from user feedback, improving satisfaction by 15-30% without labeled data.


<details>
  <summary>Details</summary>
Motivation: Generative AI agents in life sciences need to handle diverse queries from simple factoid questions to complex mechanistic reasoning. Traditional methods using fixed rules or expensive labeled training data don't adapt to changing conditions or user preferences.

Method: Combines AWS Strands Agents with Thompson Sampling contextual bandits to enable AI agents to learn optimal decision-making strategies from user feedback alone. The system optimizes three dimensions: generation strategy selection (direct vs. chain-of-thought), tool selection (literature search, drug databases, etc.), and domain routing (pharmacology, molecular biology, clinical specialists).

Result: Demonstrates 15-30% improvement in user satisfaction compared to random baselines on life science queries. Clear learning patterns emerge after 20-30 queries. The approach requires no ground truth labels and adapts continuously to user preferences.

Conclusion: The framework provides a principled solution to the exploration-exploitation dilemma in agentic AI systems, enabling adaptive learning from user feedback without requiring expensive labeled data or fixed rules.
...
### [126] [Deep Unfolding: Recent Developments, Theory, and Design Guidelines](https://arxiv.org/abs/2512.03768)
*Nir Shlezinger,Santiago Segarra,Yi Zhang,Dvir Avrahami,Zohar Davidov,Tirza Routtenberg,Yonina C. Eldar*

Main category: cs.LG

TL;DR: Deep unfolding bridges classical optimization and machine learning by transforming iterative optimization algorithms into structured, trainable neural network architectures.


<details>
  <summary>Details</summary>
Motivation: Classical optimization methods offer interpretability and theoretical guarantees but suffer from surrogate objectives, hyperparameter tuning, and computational latency. Machine learning provides data-driven modeling but lacks structure, transparency, and efficiency for optimization-driven inference. Deep unfolding aims to bridge these two paradigms.

Method: Deep unfolding systematically transforms iterative optimization algorithms into structured, trainable ML architectures. The article presents four representative design paradigms for converting optimization solvers into ML models, along with distinctive training schemes that arise from their iterative nature.

Result: The article provides theoretical advances establishing convergence and generalization guarantees for unfolded optimizers, along with comparative qualitative and empirical studies illustrating trade-offs in complexity, interpretability, and robustness.

Conclusion: Deep unfolding emerges as a compelling framework that bridges classical optimization and machine learning, offering structured, transparent, and efficient optimization-driven inference while maintaining theoretical guarantees and data-driven adaptability.
...
### [127] [Hierarchical clustering of complex energy systems using pretopology](https://arxiv.org/abs/2512.03069)
*Loup-Noe Levy,Jeremie Bosom,Guillaume Guerard,Soufian Ben Amor,Marc Bui,Hai Tran*

Main category: cs.LG

TL;DR: This paper presents an automated method using pretopology and hierarchical classification to model and classify energy consumption profiles across distributed territories, enabling efficient building consumption management without extensive manual auditing.


<details>
  <summary>Details</summary>
Motivation: Manual case-by-case auditing of thousands of buildings for energy consumption optimization is time-consuming, expensive, and requires many qualified personnel. An automated system is needed to provide effective recommendations.

Method: Uses pretopology to model consumption profiles and develops a multi-criterion hierarchical classification algorithm based on pretopological space properties, implemented in a Python library.

Result: The algorithm successfully identifies clusters in point data sets based on spatial position and size, and achieves perfect clustering (ARI=1) on generated time series using Pearson correlation. Validated on 400 real consumption sites from a French energy company.

Conclusion: The pretopology-based hierarchical classification approach provides an effective automated solution for modeling and classifying energy consumption profiles, enabling scalable optimization of building consumption management across large distributed territories.
...
### [128] [Mixed Data Clustering Survey and Challenges](https://arxiv.org/abs/2512.03070)
*Guillaume Guerard,Sonia Djebali*

Main category: cs.LG

TL;DR: A pretopological space-based clustering method for mixed data types in big data environments, addressing challenges with heterogeneous numerical and categorical variables.


<details>
  <summary>Details</summary>
Motivation: Traditional clustering techniques struggle with mixed data types (numerical and categorical) in big data environments, creating a need for specialized methods that can handle heterogeneous data complexity while providing interpretable results.

Method: Proposes a clustering method based on pretopological spaces, which offers a mathematical framework for handling mixed data types and provides hierarchical and explainable clustering structures.

Result: Benchmarking against classical numerical clustering algorithms and existing pretopological approaches demonstrates the performance and effectiveness of the proposed method within big data contexts.

Conclusion: Pretopological space-based clustering provides a valuable approach for mixed-data clustering in big data environments, offering structured and interpretable results that support better decision-making compared to traditional methods.
...
### [129] [PretopoMD: Pretopology-based Mixed Data Hierarchical Clustering](https://arxiv.org/abs/2512.03071)
*Loup-Noe Levy,Guillaume Guerard,Sonia Djebali,Soufian Ben Amor*

Main category: cs.LG

TL;DR: Novel pretopology-based algorithm for clustering mixed data without dimensionality reduction, using customizable logical rules and adjustable hyperparameters for hierarchical clustering with superior performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Address challenges of clustering mixed data without dimensionality reduction, overcome issues with clustered data explainability, and preserve data integrity by working directly on raw data.

Method: Pretopology-based algorithm leveraging Disjunctive Normal Form to formulate customizable logical rules and adjustable hyperparameters for user-defined hierarchical cluster construction, working directly on raw mixed data.

Result: Superior performance demonstrated through hierarchical dendrogram analysis and comparative clustering metrics, accurately and interpretably delineating clusters while preserving data integrity, with robustness in constructing meaningful clusters.

Conclusion: Significant advancement in clustering mixed data through departure from traditional dimensionality reduction and innovative use of logical rules that enhance both cluster formation and clarity, improving explainability.
...
### [130] [Model-Agnostic Fairness Regularization for GNNs with Incomplete Sensitive Information](https://arxiv.org/abs/2512.03074)
*Mahdi Tavassoli Kejani,Fadi Dornaika,Jean-Michel Loubes*

Main category: cs.LG

TL;DR: A fairness regularization framework for GNNs that works with partially available sensitive attributes, addressing practical limitations of existing methods that require full sensitive attribute availability.


<details>
  <summary>Details</summary>
Motivation: GNNs can perpetuate societal biases from node features, graph structure, and message-passing mechanisms. Existing fairness-aware methods assume full availability of sensitive attributes (race, gender), which is impractical due to privacy and data collection constraints.

Method: A model-agnostic fairness regularization framework that formalizes a fairness-aware objective function integrating both equal opportunity and statistical parity as differentiable regularization terms. Designed for scenarios where sensitive attributes are only partially available.

Result: Comprehensive evaluation across five real-world benchmark datasets shows significant bias mitigation across key fairness metrics while maintaining competitive node classification performance. The framework consistently outperforms baselines in achieving favorable fairness-accuracy trade-off with minimal accuracy degradation.

Conclusion: The proposed method addresses a critical practical limitation in fairness-aware GNNs by handling partially available sensitive attributes, offering an effective solution for bias mitigation in real-world applications where complete sensitive attribute data is unavailable.
...
### [131] [Risk-Entropic Flow Matching](https://arxiv.org/abs/2512.03078)
*Vahid R. Ramezani,Benjamin Englard*

Main category: cs.LG

TL;DR: The paper proposes applying tilted (entropic) risk to Flow Matching to better capture geometric structure and rare events in data manifolds.


<details>
  <summary>Details</summary>
Motivation: Standard rectified Flow Matching uses mean squared error loss that collapses all velocity targets reaching the same space-time point into a single conditional mean, ignoring higher-order conditional information (variance, skewness, multi-modality) that encodes fine geometric structure about the data manifold and minority branches.

Method: Apply the standard risk-sensitive (log-exponential) transform to the conditional FM loss, showing the resulting tilted risk loss is a natural upper-bound on a meaningful conditional entropic FM objective. A small order expansion of the gradient yields two interpretable first-order corrections: covariance preconditioning of the FM residual, and a skew tail term that favors asymmetric or rare branches.

Result: On synthetic data designed to probe ambiguity and tails, the risk-sensitive loss improves statistical metrics and recovers geometric structure more faithfully than standard rectified FM.

Conclusion: Tilted risk provides a principled approach to enhance Flow Matching by capturing higher-order conditional information and emphasizing rare events, leading to better recovery of data manifold geometry.
...
### [132] [ALARM: Automated MLLM-Based Anomaly Detection in Complex-EnviRonment Monitoring with Uncertainty Quantification](https://arxiv.org/abs/2512.03101)
...
<details>
  <summary>Details</summary>
Motivation: In complex environments, visual anomalies can be highly contextual and ambiguous, making uncertainty quantification crucial for reliable MLLM-based visual anomaly detection systems to succeed.
...
Conclusion: ALARM provides a robust UQ-supported MLLM framework for visual anomaly detection that demonstrates strong performance and cross-domain applicability in complex environments with ambiguous anomalies.
...
### [133] [Dynamic Correction of Erroneous State Estimates via Diffusion Bayesian Exploration](https://arxiv.org/abs/2512.03102)
*Yiwei Shi,Hongnan Ma,Mengyue Yang,Cunjia Liu,Weiru Liu*

Main category: cs.LG

TL;DR: The paper proposes a diffusion-driven Bayesian exploration framework (DEPF) that enables real-time correction of early state estimation errors in emergency response applications, overcoming the Stationarity-Induced Posterior Support Invariance problem in particle filters.


<details>
  <summary>Details</summary>
Motivation: Early-stage state estimates in emergency response are critical but often based on limited or biased information, leading to severe misalignment with reality. This constrains subsequent actions and can cause catastrophic delays, resource misallocation, and human harm. Bootstrap particle filters suffer from Stationarity-Induced Posterior Support Invariance (S-PSI), where regions excluded by the initial prior remain permanently unexplorable, making corrections impossible even when new evidence contradicts current beliefs.

Method: A diffusion-driven Bayesian exploration framework that expands posterior support via entropy-regularized sampling and covariance-scaled diffusion. The method includes a Metropolis-Hastings check to validate proposals and keep inference adaptive to unexpected evidence. This approach enables principled, real-time correction of early state estimation errors.

Result: Empirical evaluations on realistic hazardous-gas localization tasks show that the approach matches reinforcement learning and planning baselines when priors are correct. It substantially outperforms classical SMC perturbations and RL-based methods under misalignment. Theoretical guarantees demonstrate that DEPF resolves S-PSI while maintaining statistical rigor.

Conclusion: The proposed diffusion-driven Bayesian exploration framework provides an effective solution for correcting early state estimation errors in high-stakes applications, overcoming the fundamental limitations of traditional particle filters while maintaining statistical rigor and outperforming existing methods under misalignment conditions.
...
### [134] [Detecting AI Hallucinations in Finance: An Information-Theoretic Method Cuts Hallucination Rate by 92%](https://arxiv.org/abs/2512.03107)
*Mainak Singha*

Main category: cs.LG

TL;DR: ECLIPSE is a framework that detects LLM hallucinations by measuring the mismatch between a model's semantic entropy and available evidence capacity, using entropy estimation via clustering and perplexity decomposition to analyze evidence utilization.


<details>
  <summary>Details</summary>
Motivation: LLMs produce fluent but unsupported answers (hallucinations), which limits their safe deployment in high-stakes domains like finance. Current approaches don't effectively measure the relationship between model uncertainty and evidence availability.
...
### [135] [E-valuator: Reliable Agent Verifiers with Sequential Hypothesis Testing](https://arxiv.org/abs/2512.03109)
*Shuvom Sadhuka,Drew Prinster,Clara Fannjiang,Gabriele Scalia,Aviv Regev,Hanchen Wang*

Main category: cs.LG

TL;DR: E-valuator converts black-box verifier scores into decision rules with provable false alarm rate control using sequential hypothesis testing and e-processes.


<details>
  <summary>Details</summary>
Motivation: Current verifiers (LLM judges, process-reward models) provide heuristic scores without statistical guarantees for deciding agent success, limiting reliable deployment of agentic AI systems.

Method: Frames trajectory evaluation as sequential hypothesis testing problem, uses e-processes to develop statistically valid sequential tests that work online at every step of agent trajectories.

Result: Outperforms other strategies across 6 datasets and 3 agents in statistical power and false alarm rate control; enables quick termination of problematic trajectories to save tokens.

Conclusion: Provides lightweight, model-agnostic framework to convert verifier heuristics into decision rules with statistical guarantees for more reliable agentic system deployment.
...
### [136] [Beyond Additivity: Sparse Isotonic Shapley Regression toward Nonlinear Explainability](https://arxiv.org/abs/2512.03112)
*Jialai She*

Main category: cs.LG

TL;DR: SISR (Sparse Isotonic Shapley Regression) is a unified nonlinear explanation framework that addresses two key challenges in Shapley values: non-additive payoff functions and computational inefficiency in high dimensions, by simultaneously learning monotonic transformations and enforcing sparsity constraints.


<details>
  <summary>Details</summary>
Motivation: Standard Shapley values assume additive worth functions, but real-world payoffs often violate this due to non-Gaussian distributions, heavy tails, feature dependence, or domain-specific loss scales, leading to distorted attributions. Additionally, achieving sparse explanations in high dimensions through dense Shapley computation followed by thresholding is computationally expensive and inconsistent.

Method: SISR simultaneously learns a monotonic transformation to restore additivity (without needing closed-form specification) and enforces an L0 sparsity constraint on the Shapley vector. The optimization uses Pool-Adjacent-Violators for efficient isotonic regression and normalized hard-thresholding for support selection, ensuring implementation ease and global convergence guarantees.

Result: SISR recovers the true transformation across various scenarios and achieves strong support recovery even with high noise. Experiments in regression, logistic regression, and tree ensembles show SISR stabilizes attributions across payoff schemes, correctly filters irrelevant features, while standard Shapley values suffer severe rank and sign distortions.

Conclusion: SISR advances nonlinear explainability by unifying transformation estimation with sparsity pursuit, providing a theoretically grounded and practical attribution framework that addresses fundamental limitations of traditional Shapley values.
...
### [137] [Temporal Graph Neural Networks for Early Anomaly Detection and Performance Prediction via PV System Monitoring Data](https://arxiv.org/abs/2512.03114)
...
<details>
  <summary>Details</summary>
Motivation: The rapid growth of solar PV systems requires advanced monitoring methods to ensure optimal operation through performance tracking and anomaly detection.

Method: Proposes a Temporal Graph Neural Network that models graph-based temporal relationships among key PV parameters (irradiance, module temperature, ambient temperature) to predict electrical power output.

Result: The study is based on data from an outdoor rooftop facility in Lyon, France, including PV power measurements and meteorological parameters.

Conclusion: The proposed Temporal GNN approach offers an advanced method for solar PV performance monitoring and anomaly detection by leveraging temporal relationships among system parameters.
...
### [138] [Real-Time Structural Health Monitoring with Bayesian Neural Networks: Distinguishing Aleatoric and Epistemic Uncertainty for Digital Twin Frameworks](https://arxiv.org/abs/2512.03115)
*Hanbin Cho,Jecheon Yu,Hyeonbin Moon,Jiyoung Yoon,Junhyeong Lee,Giyoung Kim,Jinhyoung Park,Seunghwa Ryu*

Main category: cs.LG

TL;DR: Integrated SHM framework combining PCA, Bayesian neural network, and Hamiltonian Monte Carlo for full-field strain reconstruction with uncertainty quantification from sparse sensor data.


<details>
  <summary>Details</summary>
Motivation: Need for reliable real-time analysis of sensor data in structural health monitoring with spatially resolved uncertainty quantification for trustworthy decision-making on high-value assets.

Method: Combines principal component analysis (PCA) to extract leading modes, Bayesian neural network (BNN) for mapping sparse strain measurements to full-field distributions, and Hamiltonian Monte Carlo (HMC) inference for uncertainty quantification.

Result: Validated on CFRP specimens with varying crack lengths, achieving accurate strain field reconstruction (R² > 0.9) and real-time uncertainty fields. BNN handles noisy experimental data with crack-induced strain singularities while providing explicit aleatoric and epistemic uncertainty fields.

Conclusion: The framework advances SHM toward trustworthy digital twin deployment and risk-aware structural diagnostics by enabling local diagnosis of uncertainty sources (data vs. model limitations) for reliable decision-making.
...
TL;DR: MoDE addresses catastrophic forgetting in Unified Multimodal Generative Models by decoupling modalities to prevent gradient conflicts and using knowledge distillation to preserve capabilities.


<details>
  <summary>Details</summary>
Motivation: UMGMs suffer from catastrophic forgetting when learning new tasks, especially inter-modal forgetting (across modalities) which hasn't been studied before. The paper identifies and explains this phenomenon through gradient conflict between modalities.

Method: Proposes Modality-Decoupled Experts (MoDE), a lightweight architecture that isolates modality-specific updates to mitigate gradient conflict and uses knowledge distillation to prevent forgetting while preserving pre-trained capabilities.

Result: MoDE significantly mitigates both inter- and intra-modal forgetting across diverse benchmarks, outperforming prior continual learning baselines in unified multimodal generation settings.

Conclusion: The paper successfully addresses the critical issue of catastrophic forgetting in UMGMs by decoupling modalities to prevent interference, providing an effective solution for continual learning in multimodal generative models.
...
TL;DR: ChefNMR is an AI framework that predicts molecular structures directly from 1D NMR spectra and chemical formula using atomic diffusion models, achieving over 65% accuracy on natural products.


<details>
  <summary>Details</summary>
Motivation: NMR spectroscopy is essential for determining small molecule structures but requires extensive manual interpretation by experts, creating a bottleneck in natural product discovery and drug development.

Method: Frames structure elucidation as conditional generation using atomic diffusion models built on non-equivariant transformer architecture. Trained on simulated 1D NMR spectra from over 111,000 natural products.

Result: Achieves unprecedented accuracy of over 65% in predicting structures of challenging natural product compounds, significantly advancing automated structure elucidation.

Conclusion: ChefNMR represents a major step toward automating small-molecule structure determination and demonstrates deep learning's potential to accelerate molecular discovery in natural products and therapeutics.
...
TL;DR: Unsupervised VQ-VAE framework for wastewater viral variant detection without reference genomes or labeled data, achieving high accuracy and improved clustering.


<details>
  <summary>Details</summary>
Motivation: Wastewater genomic surveillance faces computational challenges: high noise, low coverage, fragmented reads, and no variant labels. Reference-based methods struggle with novel mutations and require heavy resources.

Method: VQ-VAE with k-mer tokenization learns discrete genomic patterns without references. Extended with masked reconstruction pretraining for robustness and contrastive learning for discriminative embeddings.

Result: 99.52% mean token-level accuracy, 56.33% exact sequence match rate, 19.73% codebook utilization. Contrastive fine-tuning improved Silhouette scores by 35-42% with different embedding dimensions.

Conclusion: Reference-free VQ-VAE framework provides scalable, interpretable genomic surveillance for public health monitoring, effectively handling wastewater sequencing challenges.
...
TL;DR: Interleaved Reasoning (IR) alternates thinking with surfacing intermediate responses to reduce perceived latency and enable user feedback, with Plantain adding explicit planning as first step for ~6% accuracy improvement and 60% faster first response.


<details>
  <summary>Details</summary>
Motivation: Current reasoning models waste user time by thinking silently without feedback, unlike humans who use incremental grounding. Users can't intervene when models reason from false premises, creating frustrating experiences.

Method: Propose Interleaved Reasoning (IR) where models alternate between thinking and surfacing intermediate responses. Introduce Plantain (Plan-Thought-Answer Interleaving) with explicit step-by-step plan as first intermediate response for user feedback.

Result: Plantain yields ~6% improvement in pass@1 across math reasoning and coding benchmarks while reducing time-to-first-response by over 60% compared to think-then-answer baselines.

Conclusion: Interleaved reasoning with explicit planning enables user intervention, reduces perceived latency, and improves reasoning accuracy by allowing early correction of flawed premises.
...
TL;DR: FiRE/FiRE.1 for rare cell detection in scRNA-seq data, and Enhash for concept drift detection in streaming data - both sketching/hashing-based methods with superior performance.


<details>
  <summary>Details</summary>
Motivation: Need for fast, efficient methods to identify rare cell sub-populations in large-scale single-cell RNA sequencing data, and to detect concept drift in streaming data with resource efficiency.

Method: FiRE/FiRE.1 uses sketching-based algorithm for anomaly detection; Enhash uses projection hashing for ensemble learning to detect concept drift.
...
Conclusion: Both methods provide efficient, high-performance solutions for their respective domains - rare cell detection in scRNA-seq and concept drift detection in streaming data.
...
TL;DR: Improved policy-gradient algorithms for learning memory-based policies in partially observable environments, tested on large POMDPs including robot navigation and multi-agent problems.


<details>
  <summary>Details</summary>
Motivation: Policy-gradient methods have shown promise for memoryless policies in partially observable environments but have been less successful when memory is required, creating a need for improved algorithms for memory-based policies.

Method: Developed several improved policy-gradient algorithms for learning policies with memory in infinite-horizon settings, working directly with known environment models or via simulation when models are unavailable.

Result: Algorithms were compared on large POMDPs including noisy robot navigation and multi-agent problems, demonstrating improved performance for memory-based policies.

Conclusion: The paper presents enhanced policy-gradient methods that successfully address the challenge of learning memory-based policies in partially observable environments, advancing beyond previous limitations with memoryless approaches.
...
### [145] [A Multi-Agent, Policy-Gradient approach to Network Routing](https://arxiv.org/abs/2512.03211)
...
<details>
  <summary>Details</summary>
Motivation: Network routing is a distributed decision problem with measurable performance metrics (like packet travel time). The challenge is to enable multiple distributed agents (routers) to learn cooperative behavior that optimizes overall network performance rather than individual gains.

Method: Applied OLPOMDP (policy-gradient reinforcement learning algorithm) to simulated network routing under various network models. Used reward shaping by explicitly penalizing certain patterns of sub-optimal behavior to improve convergence.

Result: Distributed agents learned cooperative behavior without explicit inter-agent communication. They avoided individually desirable but group-detrimental behavior. Reward shaping dramatically improved convergence rate.

Conclusion: OLPOMDP is effective for network routing problems, enabling distributed learning of cooperative behavior. Reward shaping is a valuable technique for accelerating convergence in such distributed reinforcement learning scenarios.
...
### [146] [Perch 2.0 transfers 'whale' to underwater tasks](https://arxiv.org/abs/2512.03219)
*Andrea Burns,Lauren Harrell,Bart van Merriënboer,Vincent Dumoulin,Jenny Hamer,Tom Denton*

Main category: cs.LG

TL;DR: Perch 2.0, a bioacoustics foundation model trained on terrestrial species, shows strong few-shot transfer learning performance on marine mammal tasks despite minimal marine training data, outperforming other bioacoustics models.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether Perch 2.0, a bioacoustics foundation model pretrained primarily on terrestrial species (birds, mammals, amphibians, insects), can effectively transfer to marine mammal and underwater audio tasks through few-shot learning, given its limited marine training data.

Method: Used linear probing with embeddings generated from Perch 2.0 foundation model for few-shot transfer learning on marine mammal tasks. Compared performance against other pretrained bioacoustics models including multispecies whale, Perch 1.0, SurfPerch, AVES-bio, BirdAVES, and Birdnet V2.3 models.

Result: Perch 2.0 embeddings demonstrated consistently high performance for few-shot transfer learning, generally outperforming alternative embedding models on the majority of marine mammal classification tasks.

Conclusion: Perch 2.0 is recommended for developing new linear classifiers for marine mammal classification with few labeled examples, as its embeddings show superior transfer learning capabilities despite minimal marine-specific training data.
...
### [147] [SPARK: Stepwise Process-Aware Rewards for Reference-Free Reinforcement Learning](https://arxiv.org/abs/2512.03244)
*Salman Rahman,Sruthi Gorantla,Arpit Gupta,Swastik Roy,Nanyun Peng,Yang Liu*

Main category: cs.LG

TL;DR: SPARK is a three-stage framework that uses synthetic verification data from multiple models to train process reward models, enabling reference-free RL training that outperforms ground-truth methods on mathematical reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Process reward models (PRMs) are promising for reinforcement learning but limited by expensive step-level annotations or ground truth references. The paper aims to overcome these limitations by creating synthetic training data without requiring expensive human annotations.
...
Result: Achieves 67.5 F1 on ProcessBench (vs 66.4 for reference-guided training and 61.9 for GPT-4o). Using Qwen2.5-Math-7B, achieves 47.4% average accuracy across six mathematical reasoning benchmarks, outperforming ground-truth-based RLVR (43.9%).

Conclusion: SPARK enables reference-free RL training that exceeds ground-truth methods, opening new possibilities for domains lacking verifiable answers or accessible ground truth, demonstrating that synthetic verification data can surpass ground-truth supervision.
...
### [148] [Too Late to Recall: Explaining the Two-Hop Problem in Multimodal Knowledge Retrieval](https://arxiv.org/abs/2512.03276)
*Constantin Venhoff,Ashkan Khakzar,Sonia Joseph,Philip Torr,Neel Nanda*

Main category: cs.LG

TL;DR: VLMs often degrade in factual recall compared to their LLM backbones due to late entity resolution, preventing reuse of existing LLM factual recall circuits. Early entity resolution enables effective multimodal alignment.


<details>
  <summary>Details</summary>
Motivation: Many vision-language models show reduced factual recall performance compared to their original LLM backbones, raising questions about how effectively multimodal fine-tuning extends LLM mechanisms to visual inputs. The paper investigates why this degradation occurs and how to address it.

Method: Benchmarked 14 VLMs with various architectures (LLaVA, Native, Cross-Attention), sizes (7B-124B), and training setups on factual recall tasks. Used attribution patching, activation patching, and probing to analyze three high-performing and two low-performing models. Tested two recovery methods: patching entity representations from LLM backbone and chain-of-thought prompting.

Result: 11 of 14 VLMs exhibited factual recall degradation. Degraded VLMs struggle to use existing LLM factual recall circuits because they resolve entity representations too late in computation. High-performing VLMs resolve entities early enough to reuse existing mechanisms. Both recovery methods successfully improved performance.

Conclusion: Early entity resolution is critical for VLMs to effectively use preexisting LLM mechanisms. Mechanistic analysis can explain systematic failures in multimodal alignment and guide improvements in VLM design.
...
### [149] [BlendedNet++: A Large-Scale Blended Wing Body Aerodynamics Dataset and Benchmark](https://arxiv.org/abs/2512.03280)
*Nicholas Sung,Steven Spreizer,Mohamed Elrefaie,Matthew C. Jones,Faez Ahmed*

Main category: cs.LG

TL;DR: BlendedNet++ is a large-scale aerodynamic dataset and benchmark for blended wing body aircraft with 12,000+ geometries and standardized forward/inverse design tasks.


<details>
  <summary>Details</summary>
Motivation: Current machine learning-based aerodynamic surrogates are limited by scarce, field-resolved datasets, hindering progress in accurate pointwise prediction and reproducible inverse design for aircraft.

Method: Created dataset of 12,000+ unique BWB geometries with steady RANS CFD simulations providing integrated coefficients and dense surface fields. Established forward-surrogate benchmark with six model families and inverse design task using conditional diffusion model compared against gradient-based optimization and hybrid approaches.

Result: Provides comprehensive dataset with 12,490 aerodynamic results, standardized benchmarks for forward prediction and inverse design, and multi-model baselines for fair comparison across architectures and optimization paradigms.

Conclusion: BlendedNet++ offers unified forward/inverse protocol to catalyze reproducible research in field-level aerodynamics and inverse design, with resources to be released upon acceptance.
...
### [150] [Multi-Frequency Federated Learning for Human Activity Recognition Using Head-Worn Sensors](https://arxiv.org/abs/2512.03287)
*Dario Fenoglio,Mohan Li,Davide Casnici,Matias Laporte,Shkurta Gashi,Silvia Santini,Martin Gjoreski,Marc Langheinrich*

Main category: cs.LG

TL;DR: Multi-frequency Federated Learning for privacy-aware Human Activity Recognition using head-worn devices, enabling joint model learning across devices with varying sampling frequencies.


<details>
  <summary>Details</summary>
Motivation: Traditional HAR systems require centralized user data collection, raising privacy concerns. Head-worn devices present an underexplored domain compared to smartwatches/smartphones, and devices often have different sampling frequencies that need to be accommodated.

Method: Proposes multi-frequency Federated Learning approach that enables privacy-aware machine learning and joint model learning across devices with varying sampling frequencies, specifically focusing on head-worn devices like earbuds and smart glasses.

Result: The approach shows improvements on two datasets compared to frequency-specific approaches, demonstrating effectiveness in the multi-frequency FL-HAR task.

Conclusion: Multi-frequency FL shows promise for privacy-aware HAR using head-worn devices, with publicly available implementation for further research and development.
...
### [151] [ASPEN: An Adaptive Spectral Physics-Enabled Network for Ginzburg-Landau Dynamics](https://arxiv.org/abs/2512.03290)
*Julian Evan Chrisnanto,Nurfauzi Fadillah,Yulison Herry Chrisnanto*

Main category: cs.LG

TL;DR: ASPEN introduces adaptive spectral layers with learnable Fourier features to overcome PINNs' spectral bias, successfully solving stiff nonlinear PDEs like the Ginzburg-Landau equation where standard PINNs fail.


<details>
  <summary>Details</summary>
Motivation: Standard PINNs struggle with stiff, multi-scale, and nonlinear PDEs due to spectral bias in MLP architectures that prevents adequate representation of high-frequency components needed for complex physical systems.

Method: ASPEN integrates an adaptive spectral layer with learnable Fourier features at the network's input stage, allowing dynamic tuning of spectral basis during training to efficiently learn required frequency content.

Result: ASPEN successfully solves the complex Ginzburg-Landau equation with exceptional accuracy (median physics residual: 5.10×10⁻³), visually matching high-resolution ground truth, while standard PINNs catastrophically fail with non-physical oscillations.

Conclusion: By incorporating adaptive spectral basis, ASPEN provides a robust, physically-consistent solver for complex dynamical systems where standard PINNs fail, opening new possibilities for machine learning in challenging physical domains.
...
### [152] [Adaptive Regime-Switching Forecasts with Distribution-Free Uncertainty: Deep Switching State-Space Models Meet Conformal Prediction](https://arxiv.org/abs/2512.03298)
*Echo Diyun LU,Charles Findling,Marianne Clausel,Alessandro Leite,Wei Gong,Pierric Kersaudy*

Main category: cs.LG

TL;DR: Combining regime-switching models with conformal inference to provide distribution-free uncertainty quantification for time series forecasting under nonstationarity.


<details>
  <summary>Details</summary>
Motivation: Regime transitions break stationarity in time series, making calibrated uncertainty as important as point accuracy for reliable forecasting.

Method: Coupling Deep Switching State Space Models with Adaptive Conformal Inference (ACI) and aggregated ACI (AgACI), plus a unified conformal wrapper for various sequence baselines including S4, MC-Dropout GRU, sparse Gaussian processes, and change-point local models.
...
Conclusion: The proposed approach provides online predictive bands with finite-sample marginal guarantees under nonstationarity and model misspecification, making uncertainty quantification robust to regime changes.
...
### [153] [HydroDCM: Hydrological Domain-Conditioned Modulation for Cross-Reservoir Inflow Prediction](https://arxiv.org/abs/2512.03300)
*Pengfei Hu,Fan Ming,Xiaoxue Han,Chang Lu,Yue Ning,Dan Lu*

Main category: cs.LG

TL;DR: HydroDCM: A scalable domain generalization framework for cross-reservoir inflow forecasting that uses spatial metadata to guide adversarial learning and enable location-specific adaptation.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for reservoir inflow prediction suffer from domain shift when applied to different reservoirs. Conventional domain generalization techniques are limited in hydrological settings where each reservoir has unique inflow patterns and spatial metadata exerts indirect but significant influence.

Method: Proposes HydroDCM framework that uses spatial metadata to construct pseudo-domain labels for adversarial learning of invariant temporal features. During inference, lightweight conditioning layers adapt features using target reservoir's metadata, balancing domain invariance with location-specific adaptation.

Result: Outperforms state-of-the-art domain generalization baselines on 30 real-world reservoirs in the Upper Colorado River Basin under many-domain conditions while remaining computationally efficient.

Conclusion: HydroDCM effectively addresses domain shift in cross-reservoir inflow forecasting by leveraging spatial metadata for both invariant feature learning and location-specific adaptation, making it scalable and practical for many-domain hydrological systems.
...
### [154] [Robust Tabular Foundation Models](https://arxiv.org/abs/2512.03307)
*Matthew Peroni,Franck Le,Vadim Sheinin*

Main category: cs.LG

TL;DR: RTFM introduces adversarial training for tabular foundation models using synthetic data generators to emphasize challenging datasets, improving performance over baselines.


<details>
  <summary>Details</summary>
Motivation: Tabular foundation models (TFMs) can be pretrained on synthetic data, but prior work focused on high-quality priors. The authors propose using adversarial robustness perspective to adapt generators to emphasize challenging datasets for better model performance.

Method: Introduces Robust Tabular Foundation Models (RTFM), a model-agnostic adversarial training framework. Uses optimality gap measure (difference between TFM performance and best achievable performance by strong baselines) to adapt synthetic data generators to produce challenging datasets during training.

Result: Applied to TabPFN V2 classifier, RTFM improves benchmark performance with up to 6% increase in mean normalized AUC over original TabPFN and other baseline algorithms, requiring less than 100k additional synthetic datasets.

Conclusion: RTFM demonstrates a promising new direction for targeted adversarial training and fine-tuning of tabular foundation models using synthetic data alone, showing significant performance improvements with relatively small additional synthetic datasets.
...
### [155] [Retrofitting Earth System Models with Cadence-Limited Neural Operator Updates](https://arxiv.org/abs/2512.03309)
*Aniruddha Bora,Shixuan Zhang,Khemraj Shukla,Bryce Harrop,George Em. Karniadakis,L. Ruby Leung*

Main category: cs.LG

TL;DR: An operator-learning framework maps instantaneous model states to bias-correction tendencies and applies them online during ESM integration, using U-Net-based architectures to capture multiscale nonlinear features while maintaining runtime feasibility.


<details>
  <summary>Details</summary>
Motivation: Earth-system models suffer from coarse resolution, imperfect parameterizations, and uncertain initial states/forcings. Traditional bias correction via data assimilation has limited benefit in free-running models, creating a need for online bias correction during model integration.

Method: Developed two U-Net-based operator architectures (Inception U-Net and multi-scale network M&M) that combine diverse upsampling and receptive fields to capture multiscale nonlinear features. Trained on 2 years of E3SM simulations nudged toward ERA5 reanalysis, with operators mapping instantaneous model states to bias-correction tendencies applied online during integration.

Result: Both architectures outperform standard U-Net baselines in offline tests, with M&M delivering most consistent bias reductions across variables and vertical levels in online hybrid E3SM runs. ML-augmented configurations remain stable and computationally feasible in multi-year simulations.

Conclusion: The framework provides a practical pathway for scalable hybrid modeling, emphasizing long-term stability, portability, and cadence-limited updates. Demonstrates utility of expressive ML operators for learning structured, cross-scale relationships and retrofitting legacy Earth-system models.
...
### [156] [Cache What Lasts: Token Retention for Memory-Bounded KV Cache in LLMs](https://arxiv.org/abs/2512.03324)
*Ngoc Bui,Shubham Sharma,Simran Lamba,Saumitra Mishra,Rex Ying*

Main category: cs.LG

TL;DR: TRIM-KV is a novel approach that learns token importance at creation time via lightweight retention gates to optimize KV cache memory usage in LLMs, outperforming existing methods while providing interpretability benefits.


<details>
  <summary>Details</summary>
Motivation: Memory and computation bottlenecks in long-horizon LLM inference due to quadratic self-attention costs and growing KV cache. Existing solutions (quantization, offloading, heuristic eviction) have high orchestration costs or unreliable importance proxies.

Method: Learns each token's intrinsic importance at creation via lightweight retention gates that predict scalar retention scores decaying over time. Tokens with low scores are evicted when memory budget is exceeded. Trained efficiently through distillation from frozen LLM with capacity loss, requiring only gate fine-tuning.

Result: Consistently outperforms strong eviction and learnable retrieval baselines across mathematical reasoning, procedural generation, conversational long-memory, and long-context understanding benchmarks, especially in low-memory regimes. Even surpasses full-cache models in some settings, showing selective retention can serve as regularization.

Conclusion: TRIM-KV provides efficient KV cache management while offering interpretability benefits. Learned retention scores align with human intuition, recovering heuristics like sink tokens and sliding windows naturally. Retention scores also provide insights into layer- and head-specific roles, suggesting new paths toward LLM interpretability.
...
### [157] [Single-Round Scalable Analytic Federated Learning](https://arxiv.org/abs/2512.03336)
*Alan T. L. Bacellar,Mustafa Munir,Felipe M. G. França,Priscila M. V. Lima,Radu Marculescu,Lizy K. John*

Main category: cs.LG

TL;DR: SAFLe is a novel federated learning framework that achieves non-linear expressivity while maintaining single-round communication efficiency, breaking the trade-off between accuracy and communication overhead in federated learning.


<details>
  <summary>Details</summary>
Motivation: Federated Learning faces two major challenges: high communication overhead and poor performance on heterogeneous (non-IID) data. While Analytic FL (AFL) offers a single-round, data-distribution-invariant solution, it's limited to linear models. Non-linear approaches like DeepAFL regain accuracy but lose the single-round benefit, creating an undesirable trade-off.

Method: SAFLe introduces a structured head of bucketed features and sparse, grouped embeddings to achieve scalable non-linear expressivity. The key innovation is that this non-linear architecture is mathematically proven to be equivalent to high-dimensional linear regression, allowing it to be solved using AFL's single-shot, invariant aggregation law.

Result: SAFLe establishes a new state-of-the-art for analytic FL, significantly outperforming both linear AFL and multi-round DeepAFL in accuracy across all benchmarks. It demonstrates highly efficient and scalable performance for federated vision tasks.

Conclusion: SAFLe successfully breaks the trade-off between communication efficiency and accuracy in federated learning by providing a non-linear solution that maintains single-round communication benefits, offering a highly efficient and scalable framework for federated vision applications.
...
<details>
  <summary>Details</summary>
Motivation: Online A/B testing for advertising policies is resource-intensive and risky, while standard OPE methods fail in deterministic auction settings where only the highest bidder wins, creating zero exposure probabilities for non-winners.

Method: Repurpose bid landscape models to approximate propensity scores, enabling use of stable estimators like Self-Normalized Inverse Propensity Scoring (SNIPS) for counterfactual evaluation in deterministic auction environments.

Result: Achieved 92% Mean Directional Accuracy in CTR prediction on AuctionNet benchmark and real-world industrial platform data, significantly outperforming parametric baselines and aligning well with online A/B test results.

Conclusion: First practical and validated framework for reliable OPE in deterministic auction environments, providing efficient alternative to costly and risky online experiments with strong directional accuracy for deployment decisions.
...
### [159] [A2G-QFL: Adaptive Aggregation with Two Gains in Quantum Federated learning](https://arxiv.org/abs/2512.03363)
*Shanika Iroshi Nanayakkara,Shiva Raj Pokhrel*

Main category: cs.LG

TL;DR: A2G is a dual-gain framework for quantum federated learning that adapts aggregation using geometry and QoS gains to handle heterogeneous networks and quantum teleportation issues.


<details>
  <summary>Details</summary>
Motivation: Federated learning in quantum-enabled heterogeneous networks suffers from performance degradation due to uneven client quality, stochastic teleportation fidelity, device instability, and geometric mismatch between local and global models. Classical aggregation rules are inadequate for quantum federated systems.

Method: Introduces A2G (Adaptive Aggregation with Two Gains) - a dual gain framework with geometry gain for geometric blending and QoS gain derived from teleportation fidelity, latency, and instability to modulate client importance. Develops A2G update rule with convergence guarantees.

Result: A2G recovers FedAvg, QoS aware averaging, and manifold based aggregation as special cases. Experiments on quantum-classical hybrid testbed show improved stability and higher accuracy under heterogeneous and noisy conditions.

Conclusion: A2G provides an effective adaptive aggregation framework for quantum federated learning that addresses unique challenges in quantum-enabled heterogeneous networks, outperforming classical approaches.
...
### [160] [MAGE-ID: A Multimodal Generative Framework for Intrusion Detection Systems](https://arxiv.org/abs/2512.03375)
*Mahdi Arab Loodaricheh,Mohammad Hossein Manshaei,Anita Raja*

Main category: cs.LG

TL;DR: MAGE-ID is a diffusion-based multimodal generative framework that couples tabular flow features with transformed images for intrusion detection data augmentation, outperforming existing methods on fidelity, diversity, and detection performance.
...
Result: Evaluations on CIC-IDS-2017 and NSL-KDD datasets demonstrate significant improvements in fidelity, diversity, and downstream detection performance over TabSyn and TabDDPM baselines.

Conclusion: MAGE-ID effectively addresses multimodal IDS augmentation challenges, highlighting the importance of capturing cross-domain dependencies for generating realistic and diverse intrusion detection data.
...
### [161] [UniQL: Unified Quantization and Low-rank Compression for Adaptive Edge LLMs](https://arxiv.org/abs/2512.03383)
...
<details>
  <summary>Details</summary>
Motivation: Deploying LLMs on mobile platforms faces challenges due to limited memory and shared computational resources, with resource availability uncertainty caused by varying device workloads.

Method: Unified framework integrating quantization and low-rank compression for Transformers, SSMs, and hybrid models. Features efficient structured weight-sorting (20x speedup), quantization-aware SVD, state-aware weight sorting for SSMs, and fused RoPE kernel for pruned models. Performs weight-sorting, fine-tuning, and quantization in cloud single-pass workflow.
...
Conclusion: UniQL provides an effective solution for edge LLM deployment with configurable on-device pruning, significant memory/throughput improvements, and maintained accuracy across various model architectures.
...
TL;DR: A tuning-free framework for recovering jointly sparse signals using implicit regularization from overparameterization, eliminating need for parameter tuning or prior knowledge of sparsity/noise variance.


<details>
  <summary>Details</summary>
Motivation: Traditional MMV methods like M-OMP and M-FOCUSS require careful parameter tuning or prior knowledge of sparsity and noise variance, which limits practical applicability.

Method: Reparameterizes estimation matrix into factors that decouple shared row-support from individual entries, then applies gradient descent to least-squares objective. Uses small balanced initialization to create momentum-like effect that promotes row-sparse structure.

Result: Theoretical proof shows optimization dynamics guarantee convergence to idealized row-sparse solution. Empirical results demonstrate performance comparable to established methods without requiring prior information or tuning.

Conclusion: The proposed tuning-free framework successfully leverages implicit regularization to overcome limitations of traditional MMV methods, providing a practical solution for jointly sparse signal recovery without parameter tuning requirements.
...
### [163] [VS-Graph: Scalable and Efficient Graph Classification Using Hyperdimensional Computing](https://arxiv.org/abs/2512.03394)
*Hamed Poursiami,Shay Snyder,Guojing Cong,Thomas Potok,Maryam Parsa*

Main category: cs.LG

TL;DR: VS-Graph is a vector-symbolic graph learning framework that combines the efficiency of Hyperdimensional Computing with expressive power comparable to GNNs, achieving competitive accuracy with 450x faster training.


<details>
  <summary>Details</summary>
Motivation: Graph neural networks (GNNs) have high computational costs limiting scalability, while existing Hyperdimensional Computing (HDC) methods struggle to match GNN performance. There's a need for lightweight graph learning that maintains expressive power.

Method: VS-Graph introduces Spike Diffusion for topology-driven node identification and Associative Message Passing for multi-hop neighborhood aggregation in high-dimensional vector space, without gradient-based optimization or backpropagation.

Result: Achieves competitive accuracy with modern GNNs, outperforms prior HDC baseline by 4-5% on MUTAG and DD benchmarks, matches/exceeds GNN performance on several datasets with up to 450x faster training, and maintains accuracy even with hypervector dimensionality reduced to D=128.

Conclusion: VS-Graph narrows the gap between HDC efficiency and GNN expressive power, demonstrating robustness under dimension compression and enabling ultra-efficient execution on edge and neuromorphic hardware.
...
### [164] [Full-Stack Alignment: Co-Aligning AI and Institutions with Thick Models of Value](https://arxiv.org/abs/2512.03399)
...
TL;DR: The paper argues that aligning individual AI systems with their operators' intentions is insufficient for good societal outcomes, and proposes "full-stack alignment" using "thick models of value" to align both AI systems and institutions with human values.


<details>
  <summary>Details</summary>
Motivation: Current AI alignment approaches focus on aligning individual systems with operator intentions, but this fails when organizational goals conflict with broader societal values. There's a need to align both AI systems and the institutions that shape them with what people truly value.

Method: The paper proposes "thick models of value" as an alternative to traditional value representations (utility functions, preference orderings, unstructured text). These structured models enable systems to distinguish values from preferences, model social embedding of choices, and reason normatively across domains.

Result: The approach is demonstrated in five application areas: AI value stewardship, normatively competent agents, win-win negotiation systems, meaning-preserving economic mechanisms, and democratic regulatory institutions.

Conclusion: Full-stack alignment using thick models of value is necessary to ensure AI systems and institutions are aligned with human values, without imposing a particular vision of flourishing, and can address limitations of current value representation approaches.
...
### [165] [Better World Models Can Lead to Better Post-Training Performance](https://arxiv.org/abs/2512.03400)
*Prakhar Gupta,Henry Conklin,Sarah-Jane Leslie,Andrew Lee*

Main category: cs.LG

TL;DR: Explicit world-modeling objectives in Transformers improve state representations and boost reinforcement learning performance on Rubik's Cube tasks.


<details>
  <summary>Details</summary>
Motivation: To understand how explicit world-modeling objectives affect Transformers' internal representations and downstream capabilities across training stages, particularly for sequence-planning tasks.

Method: Used 2x2x2 Rubik's Cube as testbed; compared standard next-token prediction with two explicit world-modeling strategies: (i) state-prediction pretraining and (ii) joint state-prediction + next-token objective; assessed performance after Group Relative Policy Optimization (GRPO) post-training; evaluated representations with linear probes and causal interventions.
...
Conclusion: Sharpening state representations through explicit world-modeling can improve the effectiveness of post-training for sequence-planning tasks.
...
### [166] [GaussDetect-LiNGAM:Causal Direction Identification without Gaussianity test](https://arxiv.org/abs/2512.03428)
...
Method: Proves equivalence between noise Gaussianity and residual independence in reverse regression, then uses kernel-based independence tests instead of Gaussianity tests.

Result: Method maintains high consistency across diverse noise types and sample sizes while reducing tests per decision (TPD), enhancing efficiency.

Conclusion: GaussDetect-LiNGAM makes LiNGAM more accessible and reliable for real-world causal inference by replacing fragile tests with robust independence tests.
...
### [167] [Grokked Models are Better Unlearners](https://arxiv.org/abs/2512.03437)
*Yuanbang Liang,Yang Li*

Main category: cs.LG

TL;DR: Grokked models enable more efficient and stable machine unlearning with less collateral damage compared to early-stopped models.


<details>
  <summary>Details</summary>
Motivation: To investigate whether grokking (delayed generalization) helps with machine unlearning - removing influence of specific data without full retraining.
...
Result: Starting from grokked checkpoints yields: (1) more efficient forgetting (fewer updates), (2) less collateral damage (smaller performance drops), (3) more stable updates across seeds. Post-grokking models learn more modular representations with reduced gradient alignment between forget and retain subsets.

Conclusion: When a model is trained (pre- vs. post-grokking) is an orthogonal lever to how unlearning is performed, providing a practical recipe to improve existing unlearning methods without altering their algorithms.
...
### [168] [Multi-Modal Opinion Integration for Financial Sentiment Analysis using Cross-Modal Attention](https://arxiv.org/abs/2512.03464)
*Yujing Liu,Chen Yang*

Main category: cs.LG

TL;DR: Proposes a deep learning framework integrating recency and popularity opinion modalities for financial sentiment analysis using cross-modal attention, achieving 83.5% accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to effectively integrate diverse opinion modalities and capture fine-grained interactions across them for financial sentiment analysis, which is important for market forecasting and risk assessment.

Method: End-to-end deep learning framework that integrates recency modality (timely opinions) and popularity modality (trending opinions) using BERT (Chinese-wwm-ext) for feature embedding, Financial Multi-Head Cross-Attention (FMHCA) for information exchange, transformer layer for optimization, and multimodal factored bilinear pooling for sentiment classification.
...
Conclusion: The framework demonstrates potential to support more accurate financial decision-making and risk management by effectively integrating distinct opinion modalities through cross-modal attention mechanisms.
...
### [169] [Bayesian Event-Based Model for Disease Subtype and Stage Inference](https://arxiv.org/abs/2512.03467)
*Hongtao Hao,Joseph L. Austerweil*

Main category: cs.LG

TL;DR: BEBMS, a Bayesian subtype variant of event-based models, outperforms SuStaIn in disease progression modeling across ordering, staging, and subtype assignment tasks, especially under model misspecification.


<details>
  <summary>Details</summary>
Motivation: Chronic diseases progress differently across patients with structured heterogeneity (subtypes). SuStaIn has been widely used but its robustness needs evaluation. The authors aim to develop a more principled Bayesian approach and compare its performance to SuStaIn.

Method: Developed BEBMS (Bayesian Event-Based Model for Subtypes), a principled Bayesian subtype variant of event-based models. Compared BEBMS to SuStaIn using synthetic data experiments with varied levels of model misspecification, and applied both methods to real-world Alzheimer's disease data.

Result: BEBMS substantially outperformed SuStaIn across ordering, staging, and subtype assignment tasks in synthetic experiments. In real-world Alzheimer's data, BEBMS produced results more consistent with scientific consensus on disease progression than SuStaIn.

Conclusion: BEBMS is a more robust and accurate method for disease progression modeling compared to SuStaIn, providing better subtype identification and progression ordering that aligns better with established scientific understanding.
...
### [170] [SweetDeep: A Wearable AI Solution for Real-Time Non-Invasive Diabetes Screening](https://arxiv.org/abs/2512.03471)
...
TL;DR: SweetDeep is a compact neural network that uses wearable sensor data to detect type 2 diabetes with 82.5% accuracy in real-world settings.


<details>
  <summary>Details</summary>
Motivation: Current diabetes screening methods are invasive and costly, creating need for scalable, cost-effective alternatives. Wearables offer potential but prior studies were limited to controlled settings.

Method: Developed SweetDeep, a lightweight neural network (<3,000 parameters) trained on physiological and demographic data from 285 participants using Samsung Galaxy Watch 7 devices in free-living conditions over six days, with multiple 2-minute recordings per day.

Result: Achieved 82.5% patient-level accuracy (82.1% macro-F1, 79.7% sensitivity, 84.6% specificity) with 5.5% calibration error. With 10% abstention on low-confidence cases, accuracy improved to 84.5%.

Conclusion: Combining engineered features with lightweight architectures enables accurate, rapid, and generalizable type 2 diabetes detection in real-world wearable settings, offering scalable screening solution.
...
TL;DR: JPM is a probabilistic framework for modeling joint disease progression from mixed pathologies, outperforming single-disease EBMs by 21% in ordering accuracy.


<details>
  <summary>Details</summary>
Motivation: Standard event-based models assume single underlying diseases, but mixed pathologies are common in neurodegeneration, necessitating models that can handle joint disease progression.

Method: JPM treats single-disease trajectories as partial rankings and builds a prior over joint progressions, with variants including Pairwise, Bradley-Terry, Plackett-Luce, and Mallows models.

Result: All JPM variants are calibrated and achieve near-perfect separation; sharpness varies by variant. JPM improves ordering accuracy by ~21% over SA-EBM baseline in synthetic experiments.

Conclusion: JPM effectively models mixed pathologies, with Mallows variant showing consistency with prior literature on AD and VaD progression, offering improved joint disease progression modeling.
...
### [172] [ATHENA: Agentic Team for Hierarchical Evolutionary Numerical Algorithms](https://arxiv.org/abs/2512.03476)
*Juan Diego Toscano,Daniel T. Chen,George Em Karniadakis*

Main category: cs.LG

TL;DR: ATHENA is an agentic framework that automates the end-to-end computational research lifecycle using a knowledge-driven HENA loop to bridge theory-implementation gaps in SciC/SciML, achieving super-human performance with 10^-14 validation errors.


<details>
  <summary>Details</summary>
Motivation: To address the major bottleneck between theoretical conceptualization and computational implementation in Scientific Computing (SciC) and Scientific Machine Learning (SciML), where manual implementation hinders scientific discovery.

Method: ATHENA uses an agentic framework with HENA loop (Hierarchical Evolutionary Numerical Algorithms) framed as a Contextual Bandit problem. It analyzes prior trials to select structural actions from combinatorial spaces guided by expert blueprints, translates these into executable code, and generates scientific rewards. Combines hybrid symbolic-numeric workflows and allows human-in-the-loop intervention.

Result: Achieves super-human performance with validation errors of 10^-14. In SciC, autonomously identifies mathematical symmetries for exact solutions and derives stable numerical solvers. In SciML, performs deep diagnosis of ill-posed formulations and resolves multiphysics problems. Human intervention improves results by an order of magnitude.

Conclusion: ATHENA represents a paradigm shift from implementation mechanics to methodological innovation, accelerating scientific discovery by automating the computational research lifecycle while maintaining human collaboration capabilities.
...
TL;DR: MLNNs integrate deep learning with modal logic semantics, using specialized neurons for necessity and possibility operators to create differentiable logical guardrails that can enforce or learn accessibility relations between possible worlds.


<details>
  <summary>Details</summary>
Motivation: To bridge deep learning with formal reasoning about necessity and possibility, enabling neurosymbolic systems that can perform deductive reasoning while optionally learning logical structures from data.

Method: Introduces modal logical neural networks with specialized neurons for □ and ◇ operators based on Kripke semantics. The framework allows accessibility relations between possible worlds to be either user-defined or parameterized by neural networks. The end-to-end differentiable system minimizes logical contradiction loss.

Result: Demonstrated on four case studies: grammatical guardrailing, axiomatic detection of the unknown, multi-agent epistemic trust, and detecting constructive deception in natural language negotiation. Shows increased logical consistency and interpretability without changing task architecture.

Conclusion: MLNNs provide a flexible neurosymbolic framework that combines deep learning with modal logic reasoning, enabling systems to enforce or learn logical structures while maintaining interpretability and consistency.
...
### [174] [Physics-Driven Learning Framework for Tomographic Tactile Sensing](https://arxiv.org/abs/2512.03512)
*Xuanxuan Yang,Xiuyang Zhang,Haofeng Chen,Gang Ma,Xiaojie Wang*

Main category: cs.LG

TL;DR: PhyDNN: A physics-driven deep learning framework for electrical impedance tomography tactile sensing that embeds the EIT forward model into the learning objective to reduce artifacts and improve contact reconstruction accuracy.


<details>
  <summary>Details</summary>
Motivation: EIT is attractive for large-area tactile sensing due to minimal wiring and shape flexibility, but suffers from severe artifacts and inaccurate contact reconstruction due to its nonlinear inverse problem. Current deep learning approaches lack physical plausibility and generalization.

Method: PhyDNN embeds the EIT forward model directly into the learning objective, jointly minimizing discrepancy between predicted and ground-truth conductivity maps while enforcing consistency with the forward PDE. A differentiable forward-operator network approximates the nonlinear EIT response for efficient backpropagation and physics-guided training.

Result: Extensive simulations and real tactile experiments on a 16-electrode soft sensor show PhyDNN consistently outperforms NOSER, TV, and standard DNNs in reconstructing contact shape, location, and pressure distribution, with fewer artifacts, sharper boundaries, and higher metric scores.

Conclusion: PhyDNN effectively combines physics-based modeling with deep learning for high-quality tomographic tactile sensing, reducing the black-box nature of networks while improving physical plausibility and generalization capabilities.
...
### [175] [Adaptive sampling using variational autoencoder and reinforcement learning](https://arxiv.org/abs/2512.03525)
*Adil Rasheed,Mikael Aleksander Jansen Shahly,Muhammad Faisal Aftab*

Main category: cs.LG

TL;DR: Adaptive sparse sensing framework combining VAE priors with reinforcement learning for sequential measurement selection outperforms traditional compressed sensing methods.


<details>
  <summary>Details</summary>
Motivation: Existing compressed sensing methods have limitations: traditional CS uses generic bases and random measurements, optimal sensor placement uses fixed linear bases that can't adapt to nonlinear variations, and generative model-based CS still uses suboptimal random sampling.

Method: Proposes an adaptive sparse sensing framework that couples a variational autoencoder (VAE) prior with reinforcement learning to select measurements sequentially, enabling adaptive sampling patterns tailored to specific data.

Result: Experiments show the proposed approach outperforms compressed sensing (CS), optimal sensor placement (OSP), and generative model-based reconstruction from sparse measurements.

Conclusion: The adaptive framework combining VAE priors with reinforcement learning enables more efficient and higher-quality sparse sensing by adaptively selecting measurements based on learned data distributions.
...
### [176] [Parameter-Efficient Augment Plugin for Class-Incremental Learning](https://arxiv.org/abs/2512.03537)
...
<details>
  <summary>Details</summary>
Motivation: Existing CIL methods suffer from forgetting, stability-plasticity trade-offs, or require large parameter expansions. There's a need for efficient methods that can achieve high accuracy without significant parameter growth.

Method: Uses a base model trained via replay/distillation, then adds Low-Rank Adaptation (LoRA) components for each task as task-specific residuals. A lightweight weighting unit assigns importance scores to different LoRA-tuned representations during inference.

Result: On ImageNet-100, achieves 8% accuracy improvement with only 4% of ResNet-18 parameters. Surpasses state-of-the-art methods under fixed memory budget constraints.

Conclusion: DLC provides an efficient plug-and-play enhancement for CIL that significantly improves accuracy with minimal parameter overhead, demonstrating exceptional parameter efficiency.
...
### [177] [Towards Irreversible Machine Unlearning for Diffusion Models](https://arxiv.org/abs/2512.03564)
*Xun Yuan,Zilong Zhao,Jiayu Li,Aryan Pasikhani,Prosanta Gope,Biplab Sikdar*

Main category: cs.LG

TL;DR: This paper introduces DiMRA, an attack that reverses diffusion model unlearning, and proposes DiMUM, a more robust unlearning method that replaces rather than forgets targeted data.


<details>
  <summary>Details</summary>
Motivation: Safety, privacy, and copyright concerns in diffusion models necessitate machine unlearning to prevent generation of sensitive content, but current finetuning-based unlearning methods are vulnerable to reversal attacks.

Method: The paper proposes two methods: 1) DiMRA (Diffusion Model Relearning Attack) - optimizes unlearned models on auxiliary data to reverse unlearning; 2) DiMUM (Diffusion Model Unlearning by Memorization) - memorizes alternative data/features to replace targeted unlearning elements instead of forgetting them.

Result: DiMRA successfully reverses state-of-the-art finetuning-based unlearning methods, exposing their vulnerability. DiMUM demonstrates superior ability to preserve generative performance while enhancing robustness against DiMRA attacks.

Conclusion: Current finetuning-based unlearning methods for diffusion models are vulnerable to reversal attacks, necessitating more robust solutions like DiMUM that use memorization-based replacement rather than forgetting.
...
### [178] [When, How Long and How Much? Interpretable Neural Networks for Time Series Regression by Learning to Mask and Aggregate](https://arxiv.org/abs/2512.03578)
...
<details>
  <summary>Details</summary>
Motivation: Current TSER models are black boxes with poor interpretability, while existing interpretable approaches have limitations: they need concept supervision, can't capture feature interactions, lack expressiveness for complex temporal patterns, and don't scale well to multivariate data.

Method: MAGNETS learns a compact set of human-understandable concepts without annotations. Each concept corresponds to a learned, mask-based aggregation over selected input features, revealing which features drive predictions and when they matter. Predictions use transparent additive combinations of these learned concepts.
...
Conclusion: MAGNETS offers an inherently interpretable neural architecture for TSER that provides transparent decision-making through learned concepts and additive structures, addressing key limitations of both black-box models and existing interpretable approaches.
...
TL;DR: This paper provides comprehensive closed-form solutions for Gaussian optimal transport and inner product Gromov-Wasserstein alignment, addressing computational challenges in large-scale applications.


<details>
  <summary>Details</summary>
Motivation: Optimal transport and Gromov-Wasserstein alignment are computationally expensive for large-scale applications, and existing closed-form solutions are limited to specific cases (centered Gaussians). The paper aims to broaden applicability by solving open problems and providing comprehensive solutions for Gaussian distributions.

Method: The authors develop mathematical frameworks for: 1) IGW alignment between uncentered Gaussians on separable Hilbert spaces with closed-form expressions up to quadratic optimization over unitary operators, 2) fully closed-form solutions when at least one Gaussian is centered, 3) analytic solutions for IGW barycenters between centered Gaussians, 4) reduction of Gaussian multimarginal OT with pairwise quadratic costs to tractable optimization, and 5) efficient algorithms using rank-deficiency constraints.

Result: The paper provides: 1) Closed-form expression for IGW alignment between uncentered Gaussians with analytic bounds, 2) Fully closed-form solutions for centered cases, 3) Analytic solution for IGW barycenters, 4) Reduction of multimarginal OT to tractable optimization, and 5) Demonstrated utility in knowledge distillation and heterogeneous clustering on synthetic and real-world datasets.

Conclusion: The work closes several gaps in Gaussian optimal transport literature, providing comprehensive closed-form solutions that broaden applicability to large-scale problems. The results enable efficient computation for previously intractable cases and demonstrate practical utility in machine learning applications like knowledge distillation and clustering.
...
TL;DR: VesselEdge system uses federated learning and trajectory compression to extend AIS coverage for maritime situational awareness, turning vessels into mobile sensors for real-time anomaly detection over low-bandwidth connections.


<details>
  <summary>Details</summary>
Motivation: To enhance maritime situational awareness by extending AIS (Automatic Identification System) coverage, addressing limitations in existing maritime monitoring systems that may have incomplete coverage or bandwidth constraints.

Method: Combines federated learning (using M3fed model) with bandwidth-constrained trajectory compression (BWC-DR-A algorithm) to transform vessels into mobile sensors, prioritizing anomalous data transmission over low-bandwidth connections.

Result: Preliminary results show effectiveness in improving AIS coverage and situational awareness using historical data, demonstrating the system's capability for real-time anomaly detection.

Conclusion: VesselEdge provides a promising approach to maritime monitoring by leveraging federated learning and compression techniques to overcome bandwidth limitations while enhancing situational awareness through extended AIS coverage.
...
### [181] [Observation-driven correction of numerical weather prediction for marine winds](https://arxiv.org/abs/2512.03606)
*Matteo Peduto,Qidong Yang,Jonathan Giezendanner,Devis Tuia,Sherrie Wang*

Main category: cs.LG

TL;DR: Transformer-based model corrects NWP wind forecasts using sparse ocean observations, reducing GFS wind RMSE by 45% at 1-hour lead time and 13% at 48-hour lead time.


<details>
  <summary>Details</summary>
Motivation: Marine wind forecasts are crucial for navigation and energy operations but challenging due to sparse, heterogeneous ocean observations. Existing NWP models have systematic errors that could be corrected with available in-situ data.

Method: Transformer architecture that learns local correction patterns by assimilating latest observations to adjust GFS output. Uses masking and set-based attention for irregular observations, cross-attention for conditioning on observation-forecast pairs, cyclical time embeddings, and coordinate-aware location representations for single-pass inference at arbitrary coordinates.

Result: Model reduces GFS 10-meter wind RMSE at all lead times up to 48 hours: 45% improvement at 1-hour lead time, 13% improvement at 48-hour lead time. Most improvements along coastlines and shipping routes where observations are abundant. Handles heterogeneous platforms (ships, buoys, tide gauges, coastal stations) and produces both site-specific predictions and basin-scale gridded products.

Conclusion: Practical, low-latency post-processing approach that complements NWP by learning to correct systematic forecast errors using available observations, demonstrating significant improvements in marine wind forecasting accuracy.
...
### [182] [CoGraM: Context-sensitive granular optimization method with rollback for robust model fusion](https://arxiv.org/abs/2512.03610)
*Julius Lenz*

Main category: cs.LG

TL;DR: CoGraM is a multi-stage, context-sensitive optimization method for merging neural networks without retraining that addresses accuracy loss and instability issues in existing methods like weight averaging and Fisher merging.


<details>
  <summary>Details</summary>
Motivation: Existing neural network merging methods (weight averaging, Fisher merging) often lose accuracy and are unstable across different seeds, which is problematic for federated and distributed learning where retraining is not feasible.
...
Result: CoGraM significantly improves merged network performance compared to existing methods like Fisher merging, addressing their weaknesses in accuracy and stability.

Conclusion: CoGraM provides an effective optimization-based approach for neural network merging that overcomes the limitations of traditional methods, making it valuable for federated and distributed learning applications where retraining is not possible.
...
### [183] [The promising potential of vision language models for the generation of textual weather forecasts](https://arxiv.org/abs/2512.03623)
...
TL;DR: Vision language model generates Shipping Forecast text directly from video-encoded weather data, showing potential for scalable weather service innovation.


<details>
  <summary>Details</summary>
Motivation: Multimodal foundation models have promising capabilities but their application to meteorological products and services remains underdeveloped. There's a need to accelerate adoption of these technologies in weather forecasting and service delivery.

Method: Uses a vision language model to write Shipping Forecast text directly from video-encoded gridded weather data. The approach converts weather data into video format that the model can process to generate textual forecasts.

Result: Early results demonstrate promising scalable technological opportunities. The model successfully generates the iconic Shipping Forecast text from weather data, showing potential for practical application.

Conclusion: This approach can enhance production efficiency and service innovation within the weather enterprise and beyond, accelerating adoption of multimodal foundation models in meteorological applications.
...
### [184] [Conditional updates of neural network weights for increased out of training performance](https://arxiv.org/abs/2512.03653)
*Jan Saynisch-Wagner,Saran Rajendran Sari*

Main category: cs.LG

TL;DR: A method for enhancing neural network performance on out-of-distribution data using weight anomaly analysis and regression-based extrapolation.


<details>
  <summary>Details</summary>
Motivation: Address neural network performance degradation when training and application data distributions differ, including out-of-distribution problems, pattern shifts, and regime shifts common in climate sciences.

Method: Three-step approach: 1) Retrain neural network on subsets of training data to identify weight anomalies, 2) Use predictors to create regression between predictors and weight anomalies, 3) Extrapolate weights to application data to adapt the neural network.

Result: Successful demonstration in three climate science use cases showing effective temporal, spatial, and cross-domain extrapolation of neural networks.

Conclusion: The proposed method enables neural networks to better handle distribution shifts by learning how network weights should adapt based on data characteristics, providing a practical solution for real-world applications with changing data distributions.
...
### [185] [Cyclical Temporal Encoding and Hybrid Deep Ensembles for Multistep Energy Forecasting](https://arxiv.org/abs/2512.03656)
*Salim Khazem,Houssam Kanso*

Main category: cs.LG

TL;DR: A unified deep learning framework combining cyclical temporal encoding with hybrid LSTM-CNN architectures improves multistep electricity consumption forecasting across all forecast horizons.


<details>
  <summary>Details</summary>
Motivation: Accurate electricity consumption forecasting is essential for demand management and smart grid operations, but existing methods may not fully exploit temporal patterns and calendar-based features effectively.

Method: Uses cyclical temporal encoding (sine-cosine) for calendar attributes, correlation analysis for feature relevance, and a hybrid ensemble model combining LSTM, CNN, and MLP meta-learners specialized for each forecast horizon.

Result: The hybrid model achieves lower RMSE and MAE than individual architectures and prior methods across all seven forecast horizons, with consistent improvements demonstrated through ablation studies.

Conclusion: Combining cyclical temporal representations with complementary deep learning structures provides significant benefits for short-term energy forecasting, representing the first unified framework jointly evaluating temporal encodings, calendar features, and hybrid ensemble architectures.
...
### [186] [Dynamically Scaled Activation Steering](https://arxiv.org/abs/2512.03661)
*Alex Ferrando,Xavier Suau,Jordi Gonzàlez,Pau Rodriguez*

Main category: cs.LG

TL;DR: DSAS is a method-agnostic framework that dynamically scales activation steering interventions based on context, improving the trade-off between toxicity mitigation and utility preservation.


<details>
  <summary>Details</summary>
Motivation: Existing activation steering methods apply interventions uniformly across all inputs, degrading model performance when steering is unnecessary. There's a need for adaptive steering that intervenes only when undesired behavior is detected.

Method: DSAS decouples "when to steer" from "how to steer" by computing context-dependent scaling factors that selectively adjust the strength of any steering method across layers and inputs. It can be jointly optimized end-to-end with the steering function.

Result: DSAS consistently improves the Pareto front with respect to steering alone, achieving better trade-off between toxicity mitigation and utility preservation. It also works with text-to-image diffusion models for concept modulation, introduces minimal computational overhead, and improves interpretability.

Conclusion: DSAS provides a flexible, adaptive framework for activation steering that enhances existing methods by dynamically scaling interventions based on context, offering improved performance trade-offs and interpretability across different model types.
...
### [187] [Feature-aware Modulation for Learning from Temporal Tabular Data](https://arxiv.org/abs/2512.03678)
*Hao-Run Cai,Han-Jia Ye*

Main category: cs.LG

TL;DR: Proposes a feature-aware temporal modulation mechanism for tabular data that balances robustness and adaptability to temporal distribution shifts by aligning feature semantics across time.


<details>
  <summary>Details</summary>
Motivation: Temporal distribution shifts in real-world tabular data deployment create a dilemma: static models lack adaptability while adaptive models may overfit to transient patterns. Evolving feature semantics (objective and subjective meanings) introduce concept drift over time.

Method: Feature-aware temporal modulation mechanism that conditions feature representations on temporal context, modulating statistical properties like scale and skewness. Uses feature transformation strategies to mitigate discrepancies in feature representations across temporal stages.

Result: Benchmark evaluations validate the method's effectiveness in handling temporal shifts in tabular data. The approach achieves lightweight yet powerful adaptation, balancing generalizability and adaptability.

Conclusion: The proposed feature-aware temporal modulation effectively addresses temporal distribution shifts in tabular data by aligning feature semantics across time, solving the robustness-adaptability dilemma through temporal conditioning of feature representations.
...
### [188] [Quantum Topological Graph Neural Networks for Detecting Complex Fraud Patterns](https://arxiv.org/abs/2512.03696)
*Mohammad Doost,Mohammad Manthouri*

Main category: cs.LG

TL;DR: QTGNN is a quantum-topological graph neural network framework for fraud detection that combines quantum embeddings, variational graph convolutions, and topological data analysis to identify fraudulent transactions in financial networks.
...
Result: The framework demonstrates effectiveness through simulations on financial datasets (PaySim and Elliptic), benchmarking against classical and quantum baselines using metrics like ROC-AUC, precision, and false positive rate. An ablation study evaluates contributions of different components. The method provides rigorous convergence guarantees and stability of topological signatures for robust fraud detection.

Conclusion: QTGNN offers a theoretically sound, interpretable, and practical solution for financial fraud detection that bridges quantum machine learning, graph theory, and topological analysis, providing a scalable approach for large transaction networks on NISQ hardware.
...
### [189] [Unlocking the Invisible Urban Traffic Dynamics under Extreme Weather: A New Physics-Constrained Hamiltonian Learning Algorithm](https://arxiv.org/abs/2512.03744)
*Xuhui Lin,Qiuchen Lu*

Main category: cs.LG

TL;DR: A physics-constrained Hamiltonian learning algorithm detects hidden structural damage in urban transportation systems that traditional surface-level recovery indicators miss, revealing "false recovery" where traffic metrics normalize but system dynamics permanently degrade.


<details>
  <summary>Details</summary>
Motivation: Current urban transportation resilience assessment methods rely on surface-level recovery indicators that miss hidden structural damage, and cannot distinguish between true recovery and "false recovery" where traffic metrics normalize but underlying system dynamics permanently degrade.
...
Result: Analysis of London's extreme rainfall in 2021 demonstrated that while surface indicators showed full recovery, the algorithm detected 64.8% structural damage missed by traditional monitoring methods.
...
### [190] [Universally Converging Representations of Matter Across Scientific Foundation Models](https://arxiv.org/abs/2512.03750)
*Sathya Edamadaka,Soojung Yang,Ju Li,Rafael Gómez-Bombarelli*

Main category: cs.LG

TL;DR: Scientific models across different modalities (strings, graphs, 3D atomistic, proteins) learn highly aligned representations of chemical systems, suggesting convergence toward a common underlying representation of physical reality, though limitations remain for out-of-distribution data.


<details>
  <summary>Details</summary>
Motivation: To understand whether scientific models across different modalities learn similar internal representations of matter, which is essential for building reliable scientific foundation models that generalize beyond training domains. Representational convergence has been studied in language and vision but not systematically in scientific domains.

Method: Analyzed representations learned by nearly sixty scientific models spanning string-, graph-, 3D atomistic, and protein-based modalities across various chemical systems. Examined alignment between models trained on different datasets and tracked convergence in representation space as models improve in performance.

Result: Models show high representational alignment across modalities for similar chemical systems. Models trained on different datasets have highly similar representations of small molecules. Machine learning interatomic potentials converge in representation space as performance improves. Two distinct regimes identified: 1) For training-similar inputs, high-performing models align closely while weak models diverge; 2) For vastly different structures, most models collapse to low-information representations, indicating current limitations.

Conclusion: Scientific models across modalities learn aligned representations, suggesting convergence toward a common underlying representation of physical reality. However, current models remain limited by training data and inductive bias, failing to encode truly universal structure for out-of-distribution data. Representational alignment serves as a quantitative benchmark for foundation-level generality in scientific models.
...
### [191] [Origin-Conditional Trajectory Encoding: Measuring Urban Configurational Asymmetries through Neural Decomposition](https://arxiv.org/abs/2512.03755)
*Stephen Law,Tao Yang,Nanjiang Chen,Xuhui Lin*

Main category: cs.LG

TL;DR: A conditional trajectory encoder jointly learns spatial and movement representations using geometric features, addressing fragmentation in urban trajectory analysis by preserving origin-dependent asymmetries and enabling measurement of cognitive inequalities in urban navigation.


<details>
  <summary>Details</summary>
Motivation: Current urban analytics approaches suffer from fragmentation: trajectory learning ignores spatial context while spatial embedding methods miss temporal dynamics. Three key gaps exist: lack of joint spatial-temporal training, origin-agnostic treatment ignoring directional asymmetries, and over-reliance on auxiliary data rather than fundamental geometric properties.

Method: A conditional trajectory encoder framework that jointly learns spatial and movement representations using geometric features (visibility ratio and curvature). Uses bidirectional LSTM conditioned on learnable origin embeddings, decomposing representations into shared urban patterns and origin-specific signatures through contrastive learning.

Result: Results from six synthetic cities and real-world validation on Beijing's Xicheng District demonstrate that urban morphology creates systematic cognitive inequalities. The framework successfully measures cognitive asymmetries across different starting locations.

Conclusion: Provides urban planners with quantitative tools for assessing experiential equity, offers architects insights into layout decisions' cognitive impacts, and enables origin-aware analytics for navigation systems by revealing how urban morphology affects cognitive navigation experiences.
...
### [192] [DVPO: Distributional Value Modeling-based Policy Optimization for LLM Post-Training](https://arxiv.org/abs/2512.03847)
...
TL;DR: DVPO is a new RL framework for LLM post-training that combines distributional value modeling with risk-aware policy optimization to handle noisy supervision while balancing robustness and generalization.


<details>
  <summary>Details</summary>
Motivation: Real-world LLM deployment often involves noisy/incomplete supervision, which can destabilize training and harm generalization. Existing approaches (worst-case optimization like RFQI/CQL, mean-based methods like PPO/GRPO) often overlook generalization and produce overly conservative policies with uneven performance across diverse scenarios.

Method: DVPO combines conditional risk theory with distributional value modeling. It learns token-level value distributions for fine-grained supervision and applies asymmetric risk regularization: contracts lower tail to dampen noisy negative deviations while expanding upper tail to preserve exploratory diversity.

Result: DVPO consistently outperforms PPO, GRPO, and robust Bellman-based PPO under noisy supervision across extensive experiments in multi-turn dialogue, math reasoning, and scientific QA tasks.

Conclusion: DVPO shows strong potential for LLM post-training in real-world settings by better balancing robustness and generalization through its distributional value modeling and risk-aware optimization approach.
...
### [193] [Forensic Activity Classification Using Digital Traces from iPhones: A Machine Learning-based Approach](https://arxiv.org/abs/2512.03786)
*Conor McCarthy,Jan Peter van Zandwijk,Marcel Worring,Zeno Geradts*

Main category: cs.LG

TL;DR: Machine learning approach translates smartphone/wearable movement sensor data into likelihood ratios for forensic activity analysis, achieving 167/171 activity pair distinctions.


<details>
  <summary>Details</summary>
Motivation: Smartphones and smartwatches provide rich behavioral data through movement sensors, offering forensic investigators opportunities to gain insights into physical activities from digital traces.

Method: Machine learning-based approach that translates digital movement sensor traces into likelihood ratios for different physical activities, evaluated on NFI_FARED dataset containing iPhone sensor data for 19 activities.

Result: The approach produced useful likelihood ratio systems for 167 out of 171 possible activity pairings, with extensions for analyzing multiple activities simultaneously and creating activity timelines.
...
### [194] [Adaptive Identification and Modeling of Clinical Pathways with Process Mining](https://arxiv.org/abs/2512.03787)
*Francesco Vitale,Nicola Mazzocca*

Main category: cs.LG

TL;DR: Two-phase process mining method for clinical pathway modeling using conformance checking to expand knowledge base for disease variants and combinations.


<details>
  <summary>Details</summary>
Motivation: Manual modeling of clinical pathways is difficult and may not reflect actual best practices for different disease variations or combinations. There's a need for automated methods that can capture and adapt to real-world treatment patterns.

Method: Two-phase modeling using process mining: 1) Collect historical disease data to create a reference process model, 2) Compare new data against reference model using conformance checking to identify variations, and expand knowledge base with specific models for new variants or combinations.

Result: Method demonstrated on Synthea dataset for COVID-19 complications, achieving up to 95.62% AUC while maintaining 67.11% arc-degree simplicity, enabling expansion of clinical pathway knowledge base with sufficient precision.

Conclusion: The proposed process mining approach successfully automates clinical pathway modeling, allowing for dynamic expansion of treatment knowledge to accommodate disease variations and combinations while maintaining model quality and simplicity.
...
### [195] [Scalable Decision Focused Learning via Online Trainable Surrogates](https://arxiv.org/abs/2512.03861)
*Gaetano Signorelli,Michele Lombardi*

Main category: cs.LG

TL;DR: Proposes an acceleration method for Decision Focused Learning using efficient unbiased surrogate estimators to reduce costly inner solver calls while maintaining solution quality.


<details>
  <summary>Details</summary>
Motivation: Decision support systems need to solve complex optimization problems with uncertain parameters. Traditional estimators lead to suboptimal solutions, and Decision Focused Learning (using actual decision cost as loss) suffers from severe scalability issues at training time due to costly loss function evaluations.

Method: Proposes an acceleration method based on replacing costly loss function evaluations with an efficient unbiased surrogate estimator. The surrogate reduces risk of spurious local optima, provides local confidence information (enabling fallback switching), and works in black-box settings to compensate for model simplifications and account for recourse actions.
...
Conclusion: The proposed acceleration method effectively addresses scalability issues in Decision Focused Learning through efficient unbiased surrogate estimators, enabling practical application while maintaining solution quality.
...
### [196] [EfficientECG: Cross-Attention with Feature Fusion for Efficient Electrocardiogram Classification](https://arxiv.org/abs/2512.03804)
*Hanhui Deng,Xinglin Li,Jie Luo,Zhanpeng Jin,Di Wu*

Main category: cs.LG

TL;DR: This paper proposes EfficientECG, a lightweight deep learning model for ECG analysis that combines EfficientNet architecture with cross-attention feature fusion for multi-lead ECG data with demographic features.


<details>
  <summary>Details</summary>
Motivation: ECG is a valuable diagnostic tool but existing models have high misdiagnosis rates. There's a need for accurate, fast diagnostic models to reduce medical worker burden while handling complex ECG data characteristics (high-frequency, long sequences, multiple leads).

Method: Two-stage approach: 1) EfficientECG - a lightweight classification model based on EfficientNet architecture for handling high-frequency long-sequence ECG data with various lead types; 2) Cross-attention-based feature fusion model that integrates EfficientECG with additional patient features (gender, age) for multi-lead ECG analysis.
...
Conclusion: The proposed deep learning approach effectively manages and analyzes ECG data through automatic feature extraction, achieving high accuracy while reducing misdiagnosis rates and medical workload burden.
...
### [197] [Hyperdimensional Computing for Sustainable Manufacturing: An Initial Assessment](https://arxiv.org/abs/2512.03864)
*Danny Hoang,Anandkumar Patel,Ruimen Chen,Rajiv Malhotra,Farhad Imani*

Main category: cs.LG

TL;DR: HDC achieves comparable accuracy to conventional AI models while drastically reducing energy consumption (200× for training, 175-1000× for inference) and improving speed (200× training, 300-600× inference) for smart manufacturing applications.


<details>
  <summary>Details</summary>
Motivation: Smart manufacturing improves efficiency but AI models' energy demands may offset these gains. Need to compare AI models for energy consumption, accuracy, and speed in smart machining applications.

Method: Uses in-situ sensing-based prediction of geometric quality in smart machining to compare energy consumption, accuracy, and speed of common AI models. Introduces HyperDimensional Computing (HDC) as an alternative approach.

Result: HDC achieves accuracy comparable to conventional models while drastically reducing energy consumption (200× for training, 175-1000× for inference). Also reduces training times by 200× and inference times by 300-600×.

Conclusion: HDC shows great potential for energy-efficient smart manufacturing by maintaining accuracy while dramatically reducing both energy consumption and computational time compared to conventional AI models.
...
### [198] [Deep Reinforcement Learning for Dynamic Algorithm Configuration: A Case Study on Optimizing OneMax with the (1+($λ$,$λ$))-GA](https://arxiv.org/abs/2512.03805)
...
<details>
  <summary>Details</summary>
Motivation: Applying RL to Dynamic Algorithm Configuration (DAC) is challenging and requires extensive domain expertise. The paper aims to address fundamental challenges in using deep-RL algorithms for DAC, specifically for controlling population size parameters in evolutionary algorithms.

Method: Systematic analysis of DDQN and PPO for controlling population size in (1+(λ,λ))-GA on OneMax. Introduces adaptive reward shifting mechanism to address under-exploration, and explores undiscounted learning for planning horizon coverage. Analyzes hyperparameter dependencies of PPO.

Result: Identified two fundamental challenges: scalability degradation and learning instability, caused by under-exploration and planning horizon coverage. DDQN with adaptive reward shifting eliminates need for instance-specific tuning and works consistently across problem scales. PPO faces fundamental variance issues that hyperparameter optimization cannot fully resolve.

Conclusion: DDQN with adaptive reward shifting achieves state-of-the-art performance in DAC, outperforming prior approaches by several orders of magnitude while maintaining comparable performance to theoretically derived policies with much better sample efficiency.
...
### [199] [Guided Flow Policy: Learning from High-Value Actions in Offline Reinforcement Learning](https://arxiv.org/abs/2512.03973)
*Franki Nguimatsia Tiofack,Théotime Le Hellard,Fabian Schramm,Nicolas Perrin-Gilbert,Justin Carpentier*

Main category: cs.LG

TL;DR: GFP introduces a guided flow policy that selectively clones high-value actions from offline datasets using mutual guidance between flow-matching policy and distilled actor, achieving SOTA performance across 144 tasks.


<details>
  <summary>Details</summary>
Motivation: Current offline RL methods with behavior regularization fail to distinguish between high-value and low-value actions, treating all dataset actions equally regardless of their quality.

Method: GFP couples a multi-step flow-matching policy with a distilled one-step actor. The actor uses weighted behavior cloning to focus on high-value actions, while the flow policy constrains the actor to stay aligned with dataset's best transitions while maximizing critic values.
...
Conclusion: GFP's mutual guidance mechanism enables selective imitation of high-value actions while maintaining dataset alignment, overcoming limitations of indiscriminate behavior regularization in offline RL.
...
TL;DR: A cost-effective method using LLM log probabilities to continuously monitor API model changes, detecting fine-tuning steps with 1000x cheaper audits than existing methods.


<details>
  <summary>Details</summary>
Motivation: Users need consistent LLM APIs for reliable applications and reproducible research, but existing audit methods are too expensive for regular monitoring of the wide range of available LLM APIs, leaving model updates largely unmonitored in practice.

Method: Uses a simple statistical test based on average token log probabilities, requesting only a single token of output. Log probabilities are usually non-deterministic but can serve as basis for cost-effective continuous monitoring.

Result: The approach can detect changes as small as one step of fine-tuning, making it more sensitive than existing methods while being 1,000x cheaper. Introduces TinyChange benchmark to measure audit method sensitivity for small, realistic model changes.

Conclusion: LLM log probabilities provide a practical, cost-effective solution for continuous monitoring of API model consistency, addressing the gap between the need for reliable LLM services and the prohibitive costs of existing audit methods.
...
TL;DR: Proposes a semantic wireless transmission framework for 3D point clouds using DeepJSCC, where transmitter predicts weights over receiver-side semantic orthogonal feature pool for compact representation, with folding-based decoder for reconstruction.


<details>
  <summary>Details</summary>
Motivation: Widespread adoption of depth sensors has made point-cloud acquisition easier, creating need for efficient wireless transmission of 3D point cloud data with semantic understanding and bandwidth efficiency.

Method: Deep Joint Source-Channel Coding (DeepJSCC) framework with transmitter predicting combination weights over receiver-side semantic orthogonal feature pool, and folding-based decoder that deforms 2D grid into 3D while enforcing manifold continuity.

Result: Evaluated on ModelNet40 across varying SNRs and bandwidths, shows performance on par with SEPT at high bandwidth and clear gains in bandwidth-constrained regimes, with consistent improvements in both PSNR and Chamfer Distance.

Conclusion: The proposed semantic wireless transmission framework effectively enables compact representations and robust reconstruction of 3D point clouds, with ablation experiments confirming benefits of orthogonalization and folding prior.
...
### [202] [Automatic Attack Discovery for Few-Shot Class-Incremental Learning via Large Language Models](https://arxiv.org/abs/2512.03882)
*Haidong Kang,Wei Wu,Hanling Wang*

Main category: cs.LG

TL;DR: ACraft: An automated LLM-based method to generate optimal attacks for FSCIL, outperforming human-designed attacks while reducing labor costs.


<details>
  <summary>Details</summary>
Motivation: Security issues in Few-shot Class Incremental Learning (FSCIL) are understudied. Existing human-designed attacks (PGD, FGSM) either fail on base classes or require excessive expert knowledge and labor costs, necessitating specialized automated attack methods for FSCIL.

Method: Proposes ACraft method that uses Large Language Models (LLMs) to automatically discover optimal FSCIL attacks without human experts. Enhances reasoning via Proximal Policy Optimization (PPO) reinforcement learning to improve attack generation through positive feedback.

Result: ACraft significantly degrades state-of-the-art FSCIL methods' performance, dramatically outperforms human expert-designed attacks, and maintains the lowest attack costs.

Conclusion: The paper provides a holistic security study of FSCIL and demonstrates that automated LLM-based attack generation (ACraft) is more effective and cost-efficient than human-designed methods for compromising FSCIL systems.
...
### [203] [Probabilistic Foundations of Fuzzy Simplicial Sets for Nonlinear Dimensionality Reduction](https://arxiv.org/abs/2512.03899)
*Janis Keck,Lukas Silvester Barth,Fatemeh,Fahimi,Parvaneh Joharinad,Jürgen Jost*

Main category: cs.LG

TL;DR: The paper provides a probabilistic interpretation of fuzzy simplicial sets used in UMAP, showing they arise from generative models sampling Vietoris-Rips filtrations at random scales, and enables derivation of new dimensionality reduction methods.


<details>
  <summary>Details</summary>
Motivation: Fuzzy simplicial sets are widely used in dimensionality reduction (especially UMAP) but lack a clear probabilistic interpretation, detaching them from common theoretical frameworks in manifold learning.

Method: Introduces a framework explaining fuzzy simplicial sets as marginals of probability measures on simplicial sets, connecting them to probabilistic models on face posets and clarifying relationships between KL divergence and fuzzy cross-entropy.

Result: Shows that UMAP's fuzzy weights arise from a generative model sampling Vietoris-Rips filtrations at random scales, yielding cumulative distribution functions of pairwise distances, and demonstrates new embedding methods can be derived from this framework.

Conclusion: The probabilistic viewpoint provides a unified theoretical foundation for fuzzy simplicial sets, clarifies UMAP's role within this framework, and enables systematic derivation of new dimensionality reduction methods, as demonstrated with Čech filtrations and triplet sampling.
...
### [204] [Quantum-Classical Physics-Informed Neural Networks for Solving Reservoir Seepage Equations](https://arxiv.org/abs/2512.03923)
*Xiang Rao,Yina Liu,Yuxuan Shen*

Main category: cs.LG

TL;DR: Proposed DV-Circuit Quantum-Classical PINN (QCPINN) for reservoir seepage PDEs, combining classical networks with quantum core to enhance parameter efficiency and nonlinear fitting for three reservoir models.


<details>
  <summary>Details</summary>
Motivation: Traditional numerical methods have mesh-dependent errors and high computational costs, while classical PINNs struggle with parameter efficiency, high-dimensional expression, and strong nonlinear fitting for reservoir seepage PDEs.

Method: DV-Circuit QCPINN integrates classical preprocessing/postprocessing networks with a discrete variable quantum core, leveraging quantum superposition and entanglement for enhanced feature mapping while embedding physical constraints. Tested three quantum circuit topologies: Cascade, Cross-mesh, Alternate.

Result: QCPINNs achieve high prediction accuracy with fewer parameters than classical PINNs. Alternate topology performs best for heterogeneous single-phase flow and two-phase BL equations, while Cascade topology excels for compositional flow with convection-dispersion-adsorption coupling.

Conclusion: Verifies feasibility of QCPINN for reservoir engineering applications, bridging quantum computing research with industrial oil and gas engineering practice for solving complex reservoir seepage PDEs.
...
### [205] [MarkTune: Improving the Quality-Detectability Trade-off in Open-Weight LLM Watermarking](https://arxiv.org/abs/2512.04044)
...
<details>
  <summary>Details</summary>
Motivation: Open-weight language models pose challenges for watermarking because inference-time interventions can't be enforced once weights are public. Existing techniques like GaussMark require weight perturbations that reduce generation quality to achieve good detection power.

Method: MarkTune is a theoretically principled, on-policy fine-tuning framework that treats the GaussMark watermark signal as a reward while simultaneously regularizing against degradation in text quality. It performs watermark-aware weight updates within the model's representation space.

Result: MarkTune consistently improves the quality-detectability trade-off over GaussMark, pushing the frontier close to inference-time watermarking. It remains robust to paraphrasing and fine-tuning attacks, and shows strong generalization across datasets.

Conclusion: MarkTune establishes a general strategy for embedding robust, high-quality watermarks into open-weight language models, addressing the fundamental challenge of watermarking public model weights.
...
### [206] [Density-Informed VAE (DiVAE): Reliable Log-Prior Probability via Density Alignment Regularization](https://arxiv.org/abs/2512.03928)
*Michele Alessi,Alessio Ansuini,Alex Rodriguez*

Main category: cs.LG

TL;DR: DiVAE is a lightweight VAE regularizer that aligns latent prior probability with data-space density estimates, improving distributional alignment, prior coverage, and OOD uncertainty calibration.


<details>
  <summary>Details</summary>
Motivation: Standard VAEs match latents to simple priors (like Gaussian) but overlook the actual density structure in the data-space, which limits their ability to properly allocate posterior mass and handle out-of-distribution data.

Method: DiVAE adds a robust, precision-weighted penalty to the ELBO that aligns the VAE log-prior probability with log-density estimated from data. It encourages the encoder to allocate posterior mass proportionally to data-space density and nudges learnable priors toward high-density regions.
...
Conclusion: DiVAE is an effective, computationally lightweight regularizer that improves VAE performance by incorporating data-space density information into the latent prior alignment, leading to better distribution modeling and OOD handling.
...
### [207] [Fare Comparison App of Uber, Ola and Rapido](https://arxiv.org/abs/2512.04065)
*Ashlesha Gopinath Sawant,Sahil S. Jadhav,Vidhan R. Jain,Shriraj S. Jagtap,Prachi Jadhav,Soham Jadhav,Ichha Raina*

Main category: cs.LG

TL;DR: A web application that compares fares across Ola, Uber, and Rapido ride-hailing services to help users choose the most cost-effective and time-efficient ride option.


<details>
  <summary>Details</summary>
Motivation: Users face difficulties choosing the most appropriate and efficient ride-hailing service that balances cost-effectiveness with travel time. There's a need for transparency in ride-hailing services to help users make better decisions and improve their overall experience.

Method: Developed a web application with backend Python system that fetches data from ride-hailing APIs, compares fares across Ola, Uber, and Rapido for user-entered destinations, and identifies the best option. Used Android Studio emulator, Appium, and location comparison tools to access and process the data.

Result: Created a functional web application that provides fare comparison between major ride-hailing services, addresses API access challenges, and offers users the best ride option based on their destination.

Conclusion: The project successfully provides transparency in ride-hailing services, increases efficiency in ride selection, and enhances user experience by enabling informed decision-making through comprehensive fare comparisons.
...
TL;DR: This paper reviews dataset distillation techniques for text data, tracing its evolution from vision-based adaptations to specialized methods for text, highlighting key milestones and current challenges in the field.


<details>
  <summary>Details</summary>
Motivation: While dataset distillation has been extensively studied in computer vision, text dataset distillation has received less attention despite its importance. The field evolved from adapting vision methods but faced unique challenges due to text's discrete nature, leading to the need for specialized approaches and a comprehensive review of the current state.

Method: The paper conducts a systematic review of text dataset distillation methods, analyzing different distillation strategies including transformer-based approaches, discrete text generation techniques, and scaling methods for large language models. It examines the evolution from vision adaptations to specialized text methods.

Result: The review identifies key milestones in text dataset distillation: introduction of transformer-based methods, generation of discrete synthetic text, and scaling to decoder-only models with over 1B parameters. It also reveals that despite advances, the field remains maturing with several open challenges.

Conclusion: Text dataset distillation has evolved significantly but remains in a maturing phase with room for improvement in benchmarking standardization, handling text's discrete nature, managing complex tasks, and demonstrating real-world applications. The review provides a foundation for future research directions.
...
TL;DR: A training-free method for detecting LLM policy violations using whitening transformations on hidden activations, treating it as an out-of-distribution detection problem.


<details>
  <summary>Details</summary>
Motivation: Organizations need reliable policy violation detection for LLMs in sensitive domains (legal, finance, medical) where breaches cause legal/reputational risks. Existing methods (guardrails, LLM-as-judge, fine-tuning) lack robustness, have latency issues, or lack interpretability.

Method: Treats policy violation detection as OOD detection. Applies whitening transformation to decorrelate LLM hidden activations and standardize to zero mean/unit variance. Uses Euclidean norm in transformed space as compliance score. Only requires policy text and few illustrative samples.

Result: Achieves state-of-the-art results on challenging policy benchmark, surpassing existing guardrails and fine-tuned reasoning models.

Conclusion: Provides organizations with practical, statistically grounded framework for policy-aware LLM oversight, advancing deployable AI governance. Method is training-free, efficient, lightweight, and easily deployable.
...
TL;DR: PEGP integrates physics-based traffic models into Gaussian processes for improved traffic state estimation under sparse observations, outperforming pure data-driven or physics-only approaches.


<details>
  <summary>Details</summary>
Motivation: Existing TSE methods struggle with low probe-vehicle penetration and sparse observations. Pure data-driven methods lack physical interpretability and generalize poorly with sparse data, while physical models can't handle uncertainties and real-world complexity. Hybrid approaches using physics as soft constraints suffer from penalty tuning issues and poor uncertainty calibration.

Method: Developed Physics-Embedded Gaussian Process (PEGP) with two multi-output kernels based on classic traffic flow models (LWR and ARZ). Used explicit application of linearized differential operators to embed physical structure directly into the GP framework.

Result: PEGP consistently outperforms non-physics baselines on HighD and NGSIM datasets. PEGP-ARZ performs better under sparse observations, while PEGP-LWR achieves lower errors with denser observations. PEGP-ARZ residuals align closely with physics and provide calibrated uncertainty, while PEGP-LWR residuals are more orthogonal with nearly constant variance.

Conclusion: PEGP successfully integrates physical priors with uncertainty quantification for reliable traffic state estimation, addressing limitations of previous hybrid approaches and providing interpretable, calibrated uncertainty estimates.
...
TL;DR: The paper analyzes cross-entropy training dynamics on a two-layer linear neural network, proving convergence to neural collapse geometry despite non-convex optimization challenges.


<details>
  <summary>Details</summary>
Motivation: Existing theory on cross-entropy loss often relies on simplifications (squared loss or convex models) that miss essential behavior, creating a gap in understanding CE optimization dynamics in realistic non-convex settings.

Method: Analyzes a canonical two-layer linear neural network with standard-basis vectors as inputs, constructs an explicit Lyapunov function to prove global convergence, and uses Hadamard Initialization to diagonalize the softmax operator and simplify dynamics.

Result: First proof that gradient flow on cross-entropy converges to neural collapse geometry, establishes global convergence despite spurious critical points, and develops a technique (Hadamard Initialization) that enables analysis of CE dynamics beyond the specific setting.

Conclusion: The paper provides fundamental insights into cross-entropy optimization dynamics in non-convex settings, proving convergence to neural collapse and introducing a powerful analytical technique that opens pathways for broader analysis of CE training dynamics.
...
TL;DR: First DP algorithm with near-optimal privacy-utility trade-offs that can be verified cheaper than training, focusing on DP stochastic convex optimization.


<details>
  <summary>Details</summary>
Motivation: Current DP verification requires as much compute as training, making it impractical for data providers to verify privacy guarantees. There's a need for efficient verification methods that scale better than training costs.

Method: Privately minimize a series of regularized objectives using only standard DP composition bounds, achieving tight privacy-utility trade-offs while enabling cheaper verification.

Result: Achieves near-optimal privacy-utility trade-offs for DP-SCO with verification costs that scale much better than training costs, significantly reducing verification overhead on large datasets.

Conclusion: First DP-SCO algorithm with efficient verification that maintains near-optimal privacy-utility performance, enabling practical privacy verification for large-scale deployments.
...
TL;DR: Supervised learning on single-domain datasets causes domain feature collapse (I(x_d; z) = 0), explaining why state-of-the-art OOD detection methods catastrophically fail on such models.


<details>
  <summary>Details</summary>
Motivation: To explain why state-of-the-art out-of-distribution (OOD) detection methods exhibit catastrophic failure when models are trained on single-domain datasets, which has been a puzzling empirical phenomenon.

Method: Theoretical analysis using information theory, proving that supervised learning on single-domain data inevitably produces domain feature collapse (I(x_d; z) = 0). Extended analysis using Fano's inequality to quantify partial collapse. Introduced Domain Bench benchmark and validated theory through domain filtering using pretrained representations.

Result: Shows that domain feature collapse causes catastrophic OOD detection failure (e.g., only 53% FPR@95 on MNIST). Demonstrates that preserving I(x_d; z) > 0 through domain filtering resolves the failure mode, providing strong empirical evidence for the information-theoretic framework.

Conclusion: The work explains a fundamental limitation of supervised learning in narrow domains, reveals why OOD detection fails on single-domain trained models, and has broader implications for transfer learning and decisions about when to fine-tune versus freeze pretrained models.
...
TL;DR: Paper introduces discrete update rules for training neural networks instead of quantizing continuous updates, with theoretical convergence guarantees and empirical validation.


<details>
  <summary>Details</summary>
Motivation: Modern deep learning models require immense computational resources, motivating research into low-precision training. Current quantized training approaches typically rely on discretizing real-valued updates, which may not be optimal.

Method: Introduces an alternative approach where the update rule itself is discrete by design, avoiding quantization of continuous updates. Establishes convergence guarantees for a general class of such discrete schemes, and presents a multinomial update rule as a concrete example.

Result: The approach is supported by empirical evaluation, demonstrating that discrete update rules can be effective for training neural networks while being more computationally efficient.

Conclusion: This perspective opens new avenues for efficient training, particularly for models with inherently discrete structure, offering a fundamentally different approach to low-precision training compared to traditional quantization methods.
...
TL;DR: Eval Factsheets is a structured documentation framework for AI system evaluations, addressing the lack of systematic standards in benchmark documentation through a comprehensive taxonomy and questionnaire approach.


<details>
  <summary>Details</summary>
Motivation: The proliferation of benchmarks has created challenges in reproducibility, transparency, and informed decision-making. Unlike datasets and models which have documentation frameworks (Datasheets, Model Cards), evaluation methodologies lack systematic documentation standards.

Method: The authors introduce Eval Factsheets, a structured framework organized across five dimensions: Context (who/when), Scope (what), Structure (with what), Method (how), and Alignment (reliability/validity/robustness). This is implemented as a practical questionnaire with mandatory and recommended documentation elements.

Result: Through case studies on multiple benchmarks, Eval Factsheets effectively captures diverse evaluation paradigms (from traditional benchmarks to LLM-as-judge methodologies) while maintaining consistency and comparability.

Conclusion: Eval Factsheets should be incorporated into both existing and newly released evaluation frameworks to promote more transparency and reproducibility in AI system evaluations.
...
TL;DR: Training steerable AI assistant policies using self-play to manage uncertainty in ambiguous queries, optimizing for cost-penalized accuracy with customizable clarification costs.
...
Method: Use self-play with two agents (user simulator and AI assistant) to generate conversations with ambiguous queries. Train models using Reinforced Self-Training (ReST) to maximize reward defined as cost-penalized accuracy, where costs for clarification questions and generated words are provided as numerical inputs.
...
Conclusion: Self-play with cost-aware reinforcement learning enables training of flexible AI assistant policies that can adapt their uncertainty management strategies to different contextual constraints and user preferences.
...
### [217] [CFO-Robust Detection for 5G PRACH under Fading Channels: Analytical Modeling and Performance Evaluation](https://arxiv.org/abs/2512.03096)
*Daniel Alarcón-Martín,Mari Carmen Aguayo-Torres,Francisco J. Martín-Vega,Gerardo Gómez*

Main category: cs.IT

TL;DR: The paper presents a unified analytical framework for PRACH detection in 5G/6G networks, deriving optimal thresholds and detection probabilities for coherent and power combining strategies, plus a novel low-complexity CFO-aware detector.


<details>
  <summary>Details</summary>
Motivation: PRACH detection is crucial for 5G/6G initial access but suffers from impairments like high user density, large carrier frequency offset, and fast fading. Existing studies are limited to specific scenarios and lack comprehensive analytical performance characterization.

Method: Developed a unified analytical framework characterizing statistical distribution of received power delay profile under flat Rayleigh fading. Analyzed both coherent combining and power combining repetition strategies with optimal threshold derivations and closed-form detection probabilities. Also designed a novel low-complexity CFO-aware detector exploiting correlation induced by CFO across cyclic shifts.

Result: Power combining outperforms coherent combining when repetitions experience independent channels, while coherent combining can be preferable under identical channel realizations in limited settings. The proposed CFO-aware detector delivers improved robustness under severe CFO conditions.

Conclusion: The paper provides comprehensive analytical tools for PRACH detection performance evaluation and introduces a practical CFO-aware detector that enhances robustness in challenging network conditions, advancing the state-of-the-art for 5G/6G random access.
...
### [218] [Strengthening Han's Fourier Entropy-Influence Inequality via an Information-Theoretic Proof](https://arxiv.org/abs/2512.03117)
*Peijie Li,Guangyue Han*

Main category: cs.IT

TL;DR: The paper strengthens Han's Fourier entropy-influence inequality by proving it holds with optimal constants C1=C2=1 for all real-valued Boolean functions with unit L²-norm.


<details>
  <summary>Details</summary>
Motivation: To improve the constants in Han's Fourier entropy-influence inequality from C1=3+2ln2 and C2=1 to the sharp constants C1=C2=1, establishing the inequality as a fundamental structural property connecting Shannon entropy and influence.

Method: Uses a short information-theoretic proof approach to demonstrate the inequality holds with optimal constants for all real-valued Boolean functions of unit L²-norm.

Result: Proves Han's Fourier entropy-influence inequality holds with sharp constants C1=C2=1, improving upon the original constants of C1=3+2ln2 and C2=1 for {-1,1}-valued Boolean functions.

Conclusion: The strengthened inequality with optimal constants establishes the Fourier entropy-influence inequality as an elementary structural property connecting Shannon entropy and influence for real-valued Boolean functions.
...
### [219] [Multi-Source M/G/1/1 Queues with Probabilistic Preemption](https://arxiv.org/abs/2512.03241)
*Mohammad Moltafet,Hamid R. Sadjadpour,Zouheir Rezki,Marian Codreanu,Roy D. Yates*

Main category: cs.IT

TL;DR: Multi-source M/G/1/1 queue with probabilistic preemption policy for AoI optimization


<details>
  <summary>Details</summary>
Motivation: To improve Age of Information (AoI) in multi-source status update systems with limited buffer capacity (no waiting buffer) by introducing a flexible packet management policy that allows probabilistic replacement of existing packets

Method: Model as multi-source M/G/1/1 queueing system with Poisson packet generation and general service time distribution. Introduce probabilistically preemptive policy where arriving packet replaces existing packet from same source with fixed probability. Derive moment generating functions (MGFs) for AoI and peak AoI (PAoI) for each source.

Result: Successfully derived analytical expressions for MGFs of AoI and PAoI under the proposed policy. Numerical results demonstrate effectiveness of the probabilistic preemption policy in optimizing Age of Information performance.

Conclusion: The probabilistically preemptive packet management policy provides an effective approach to optimize AoI in multi-source status update systems with limited buffer capacity, offering analytical tools for performance evaluation and system design.
...
### [220] [Generalized Orthogonal Approximate Message-Passing for Sublinear Sparsity](https://arxiv.org/abs/2512.03326)
*Keigo Takeuchi*

Main category: cs.IT

TL;DR: GOAMP algorithm enables error-free sparse signal reconstruction from generalized linear measurements with sublinear sparsity, outperforming existing methods especially for ill-conditioned sensing matrices.


<details>
  <summary>Details</summary>
Motivation: Previous research assumed signal sparsity proportional to signal dimension, and AMP algorithms had poor convergence for non-Gaussian sensing matrices. There's a need for better reconstruction algorithms for signals with sublinear sparsity using various sensing matrices.

Method: Proposed Generalized Orthogonal AMP (GOAMP) with Onsager correction designed via state evolution for orthogonally invariant sensing matrices. Uses Bayesian denoisers and operates in sublinear sparsity limit where sparsity and measurement dimension grow sublinearly with signal dimension.

Result: GOAMP achieves error-free reconstruction when measurement dimension exceeds a threshold (same as AMP for Gaussian matrices) and support excludes origin neighborhood. Outperforms existing algorithms including GAMP for sublinear sparsity, especially with ill-conditioned matrices in both linear and 1-bit compressed sensing.

Conclusion: GOAMP effectively solves convergence issues of AMP for non-Gaussian sensing matrices, enabling reliable sparse signal reconstruction with sublinear sparsity across various measurement scenarios.
...
### [221] [From Reliability to Security: How RIS-Assisted Adaptive SM and SSK Enhances Wireless Systems](https://arxiv.org/abs/2512.03518)
*Chaorong Zhang,Benjamin K. Ng,Ke Wang,Hui Xu,Chan-Tong Lam*

Main category: cs.IT

TL;DR: Proposes RIS-assisted RASM and RASSK schemes that use reconfigurable intelligent surfaces to dynamically map bits at receive antennas, improving spectral efficiency and physical layer security with fewer RF chains at transmitter.


<details>
  <summary>Details</summary>
Motivation: To enhance spectral efficiency and physical layer security in wireless communications while reducing hardware costs (fewer RF chains at transmitter) and enabling green communication through efficient power usage.

Method: Two novel schemes: RIS-assisted Received Adaptive Spatial Modulation (RASM) and RIS-assisted Received Adaptive Space Shift Keying (RASSK). Both leverage RIS characteristics to dynamically map transmitting bits at receive antennas, enhancing SNR at selected antennas with minimal power.

Result: Improved spectral efficiency, lower bit error rate, enhanced physical layer security against eavesdropping, and reduced detection complexity. Simulation results show superior error performance and robustness against wiretapping.

Conclusion: RIS-assisted RASM and RASSK schemes offer reliable, green wireless communication with improved spectral efficiency and inherent physical layer security, showing strong potential for future wireless applications.
...
### [222] [Expected Confidence Dependency: A Novel Rough Set-Based Approach to Feature Selection](https://arxiv.org/abs/2512.03612)
*Saeed Rasouli,Hamid Karamikabir*

Main category: cs.IT

TL;DR: Proposes Expected Confidence Dependency (ECD), a novel accuracy-driven dependency measure for feature selection in rough set theory that uses confidence-based contributions instead of binary characterizations.


<details>
  <summary>Details</summary>
Motivation: Traditional rough set dependency measures rely on binary characterizations of conditional blocks, which may not capture nuanced relationships. There's a need for a more sophisticated, soft computing-oriented measure that can better handle uncertainty and provide more accurate feature selection.

Method: ECD assigns confidence-based contributions to individual equivalence blocks and aggregates them through a normalized expectation operator. This creates a soft computing approach that moves beyond binary classifications to capture more subtle dependencies.

Result: The paper formally establishes several desirable properties of ECD, including normalization, compatibility with classical dependency, monotonicity, and invariance under structural and label-preserving transformations.

Conclusion: ECD represents a significant advancement in rough set theory by providing a more nuanced, accuracy-driven dependency measure that maintains desirable mathematical properties while offering improved feature selection capabilities through its soft computing approach.
...
### [223] [Over-the-Air Federated Learning: Rethinking Edge AI Through Signal Processing](https://arxiv.org/abs/2512.03719)
*Seyed Mohammad Azimi-Abarghouyi,Carlo Fischione,Kaibin Huang*

Main category: cs.IT

TL;DR: This paper provides a tutorial on Over-the-Air Federated Learning (AirFL), which integrates wireless communication with distributed machine learning to enable efficient edge AI by leveraging wireless signal superposition for simultaneous communication and model aggregation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of traditional federated learning at the network edge, which suffers from high latency, bandwidth requirements, and energy consumption. AirFL aims to overcome these limitations by tightly integrating wireless signal processing with distributed machine learning.

Method: The paper presents a novel classification of AirFL into three design approaches: 1) CSIT-aware AirFL (using Channel State Information at the Transmitter), 2) blind AirFL, and 3) weighted AirFL. It provides comprehensive theoretical foundations and performance analysis for these approaches.

Result: The paper establishes AirFL as an emerging paradigm that significantly reduces latency, bandwidth, and energy consumption compared to traditional federated learning approaches by leveraging the superposition property of wireless signals for simultaneous communication and model aggregation.

Conclusion: AirFL represents a promising direction for scalable AI at the network edge, with the paper serving as a comprehensive tutorial covering theoretical foundations, practical limitations, and future research directions for this emerging field.
...
### [224] [Movable Signals with Dual-Polarized Fixed Intelligent Surfaces: Beyond Diagonal Reflection Matrices](https://arxiv.org/abs/2512.03872)
*Matteo Nerini,Bruno Clerckx*

Main category: cs.IT

TL;DR: Dual-polarized intelligent surfaces: FIS (fixed surface, adjustable frequency) outperforms RIS (adjustable surface) by at least 4x. Beyond-diagonal FIS further improves performance when transmitter/receiver polarizations differ.


<details>
  <summary>Details</summary>
Motivation: To investigate and compare the performance of different dual-polarized intelligent surface configurations for wireless systems, specifically comparing reconfigurable intelligent surfaces (RIS) with movable signals using fixed intelligent surfaces (FIS).

Method: Comparative analysis of two main approaches: 1) RIS with adjustable reflection matrices, and 2) FIS with fixed surface properties but adjustable signal frequency. Both approaches are evaluated with two types of surfaces: diagonal (with diagonal reflection matrices) and beyond-diagonal (with non-diagonal reflection matrices).

Result: Movable signals with FIS consistently outperform RIS, achieving at least a fourfold gain. Beyond-diagonal FIS provides additional performance enhancement when transmitter and receiver polarizations differ.

Conclusion: Fixed intelligent surfaces with movable signals (FIS) are superior to reconfigurable intelligent surfaces (RIS) for dual-polarized wireless systems, with beyond-diagonal FIS offering further advantages in scenarios with different transmitter and receiver polarizations.
...
### [225] [On topological and algebraic structures of categorical random variables](https://arxiv.org/abs/2512.04020)
*Inocencio Ortiz,Santiago Gómez-Guerrero,Christian E. Schaerer*

Main category: cs.IT

TL;DR: Defines a metric for categorical random variables based on entropy and symmetrical uncertainty, promotes it to quotient space, and establishes compatible monoid structure.


<details>
  <summary>Details</summary>
Motivation: To develop a proper mathematical framework for analyzing categorical random variables using information-theoretic measures, creating a metric space structure that respects equivalence classes and algebraic operations.

Method: Uses entropy and symmetrical uncertainty (SU) to define a metric for categorical random variables, then constructs a quotient space by identifying equivalent variables, and establishes a commutative monoid structure on this space.

Result: Successfully defines a metric based on entropy and SU that can be promoted to quotient space, and shows existence of natural commutative monoid structure compatible with the induced topology (continuous monoid operation).

Conclusion: Provides a rigorous mathematical framework for categorical random variables with both metric and algebraic structures, enabling topological and algebraic analysis of information-theoretic relationships.
...
### [226] [Semi-Markov Decision Process Framework for Age of Incorrect Information Minimization](https://arxiv.org/abs/2512.04077)
*Ismail Cosandal,Sennur Ulukus,Nail Akar*

Main category: cs.IT

TL;DR: The paper studies Age of Incorrect Information (AoII) for remote estimation systems with Markov sources, general channel delays, and multi-threshold transmission policies, formulating the optimization as an SMDP solved using novel DR-AMC and DR-DPH stochastic tools.


<details>
  <summary>Details</summary>
Motivation: To optimize remote estimation systems by minimizing a weighted sum of AoII-related costs and transmission costs, addressing the need for semantic-aware freshness metrics in systems with Markov sources and general channel delays.

Method: Formulate the problem as a semi-Markov decision process (SMDP) with the same state-space as the original DTMC, using a novel stochastic tool called dual-regime absorbing Markov chain (DR-AMC) and its absorption time distribution (DR-DPH) to obtain parameters for the optimum multi-threshold policy.

Result: Develop a framework to obtain the optimum multi-threshold transmission policy that minimizes the weighted sum of time average AoII/estimation costs and transmission costs in systems with Markov sources and general DPH channel delays.

Conclusion: The proposed SMDP formulation with DR-AMC/DR-DPH tools provides an effective approach for optimizing multi-threshold transmission policies in remote estimation systems with semantic-aware freshness metrics and general channel characteristics.
...
### [227] [Ultra-Strong Gradient Diffusion MRI with Self-Supervised Learning for Prostate Cancer Characterization](https://arxiv.org/abs/2512.03196)
...
TL;DR: Physics-informed self-supervised VERDICT fitting with ultra-strong gradients significantly improves prostate cancer characterization compared to conventional clinical methods.


<details>
  <summary>Details</summary>
Motivation: Current clinical dMRI metrics like Apparent Diffusion Coefficient lack specificity to prostate histology, and clinical gradient systems suffer from poor SNR at strong diffusion weightings due to prolonged echo times.

Method: Developed enhanced ssVERDICT fitting approaches using dense multilayer perceptron (Dense MLP) and convolutional U-Net architectures, benchmarking against non-linear least-squares (NLLS) fitting and Diffusion Kurtosis Imaging across clinical to ultra-strong gradient systems.

Result: Dense ssVERDICT at ultra-strong gradient outperformed NLLS VERDICT: 47% boost in median CNR, 52% reduction in inter-patient Coefficient of Variation, 50% reduction in pooled f_ic variation. Delivered highest CNR, most stable parameter estimates, and clearest tumour-normal contrast.

Conclusion: Advanced gradient systems combined with deep learning-based modelling can significantly improve non-invasive prostate cancer characterization and potentially reduce unnecessary biopsies.
...
### [228] [Quality assurance of the Federal Interagency Traumatic Brain Injury Research (FITBIR) MRI database to enable integrated multi-site analysis](https://arxiv.org/abs/2512.03202)
...
TL;DR: Researchers organized and analyzed heterogeneous TBI imaging data from FITBIR database using BIDS format, finding significant volume differences in 54 brain regions between TBI subjects and controls.


<details>
  <summary>Details</summary>
Motivation: The FITBIR database contains valuable but heterogeneous TBI imaging data that needs standardization and comprehensive analysis to understand brain changes in traumatic brain injury.

Method: Organized FITBIR structural/diffusion MRI data into Brain Imaging Data Structure (BIDS), performed quality assurance and harmonization, analyzed brain metrics using UNesT segmentations, and modeled age profiles with GAMLSS.

Result: After processing: 4,868 subjects with structural MRI, 2,666 with diffusion MRI. Found significant differences in volumes of 54 brain regions between TBI subjects and controls (q<0.05, FDR-corrected).

Conclusion: Standardizing heterogeneous TBI imaging data enables large-scale analysis revealing significant structural brain differences in TBI, providing valuable insights for TBI research and potential biomarkers.
...
### [229] [A BTR-Based Approach for Detection of Infrared Small Targets](https://arxiv.org/abs/2512.03752)
*Ke-Xin Li*

Main category: eess.IV

TL;DR: BTR-ISTD: A bilateral tensor ring decomposition method for infrared small target detection that outperforms existing methods in accuracy, background suppression, and speed.


<details>
  <summary>Details</summary>
Motivation: Existing low-rank sparse methods for infrared small target detection have high computational complexity and struggle with low-contrast targets and complex dynamic backgrounds with target-like interference.

Method: Reconstruct data into a fourth-order tensor, propose BTR-ISTD model using bilateral tensor ring decomposition to distinguish weak spatial correlations from strong temporal-patch correlations while capturing interactions between components, solved efficiently with proximal alternating minimization (PAM) framework.

Result: Experimental results show the proposed approach outperforms several state-of-the-art methods in detection accuracy, background suppression capability, and computational speed.

Conclusion: BTR-ISTD effectively addresses limitations of existing methods by leveraging tensor decomposition to handle complex infrared small target detection scenarios with improved performance and efficiency.
...
### [230] [Tada-DIP: Input-adaptive Deep Image Prior for One-shot 3D Image Reconstruction](https://arxiv.org/abs/2512.03962)
*Evan Bell,Shijun Liang,Ismail Alkhouri,Saiprasad Ravishankar*

Main category: eess.IV

TL;DR: Tada-DIP is a 3D extension of Deep Image Prior that combines input-adaptation and denoising regularization to solve 3D inverse problems, achieving performance comparable to supervised networks without training data.


<details>
  <summary>Details</summary>
Motivation: Deep Image Prior (DIP) shows promise for one-shot neural-network based image reconstruction but has limited application to 3D problems. There's a need for effective 3D DIP methods that can handle inverse problems while avoiding overfitting issues common in DIP approaches.

Method: Tada-DIP combines input-adaptation and denoising regularization to create a fully 3D DIP method. This approach enables high-quality 3D reconstructions while preventing the overfitting phenomenon that typically affects DIP methods.
...
Conclusion: Tada-DIP is an effective 3D extension of DIP that successfully addresses 3D inverse problems, avoiding overfitting while achieving reconstruction quality on par with supervised methods, demonstrating its potential for practical 3D medical imaging applications.
...
### [231] [Optimizing ISAC MIMO Systems with Reconfigurable Pixel Antennas](https://arxiv.org/abs/2512.03319)
*Ataher Sams,Yu-Cheng Hsiao,Muhammad Talha,Besma Smida,Ashutosh Sabharwal*

Main category: eess.SP

TL;DR: Proposes an ISAC MIMO framework using Reconfigurable Pixel Antennas (RPixA) with EM-domain DoF controlled by binary antenna coder switches, combining EM and digital precoding to jointly optimize sensing and communication performance.


<details>
  <summary>Details</summary>
Motivation: Integration of sensing and communication requires architectures that can flexibly exploit spatial and electromagnetic degrees of freedom, with current systems lacking sufficient EM-domain control for optimal ISAC performance.

Method: Uses Reconfigurable Pixel Antennas with binary antenna coder switch networks to introduce EM-domain DoF. Combines EM and digital precoding in a beamforming architecture. Formulates non-convex joint optimization problem to maximize sensing rate under communication rate constraints. Uses alternating optimization with genetic algorithm for port states and semi-definite relaxation for digital beamforming.

Result: The proposed EM-aware design achieves considerably higher sensing rate compared to conventional arrays and enables significant antenna reduction for equivalent ISAC performance.

Conclusion: Reconfigurable pixel antennas have potential to realize efficient and scalable EM-aware ISAC systems for future 6G networks by providing additional EM-domain degrees of freedom.
...
### [232] [A Convolutional Framework for Mapping Imagined Auditory MEG into Listened Brain Responses](https://arxiv.org/abs/2512.03458)
*Maryam Maghsoudi,Mohsen Rezaeizadeh,Shihab Shamma*

Main category: eess.SP

TL;DR: Researchers developed a CNN model that successfully maps imagined speech/music brain activity to perceived responses using MEG data from musicians, enabling potential BCI applications.


<details>
  <summary>Details</summary>
Motivation: Decoding imagined speech is challenging due to timing uncertainty and limited datasets. The study aims to bridge imagined and perceived neural responses to advance brain-computer interface technology.

Method: Used MEG data from trained musicians imagining/listening to musical/poetic stimuli. Developed sliding-window ridge regression for single-subject mapping, then created encoder-decoder CNN with subject-specific calibration layer for group-level analysis.

Result: Both imagined and perceived responses contained condition-specific information. CNN consistently outperformed null model, achieving significantly higher correlations between predicted and true listened responses for nearly all held-out subjects.

Conclusion: Imagined neural activity can be transformed into perception-like responses, establishing a foundation for future BCI applications involving imagined speech and music.
...
### [233] [Scalable Pixel-based Reconfigurable Beamforming Networks for Designing Fluid Antenna Systems](https://arxiv.org/abs/2512.03703)
*Jichen Zhang,Junhui Rao,Tianqu Kang,Zhaoyang Ming,Yijun Chen,Alikhan Umirbayev,Chi-Yuk Chiu,Ross Murch*

Main category: eess.SP

TL;DR: A scalable pixel-based reconfigurable beamforming network (PRBFN) for Fluid Antenna Systems (FAS) is proposed, enabling fast electronic reconfiguration instead of slow mechanical movement.


<details>
  <summary>Details</summary>
Motivation: Fluid Antenna Systems (FAS) are promising for 6G wireless but current mechanical implementations are too slow due to inertia limitations. There's a need for faster reconfiguration methods.

Method: The paper proposes that changing antenna position is equivalent to switching excitation current vectors in a multi-port antenna. A novel PRBFN is created by selecting required current vectors and concatenating beamforming networks, with two design examples (0.5λ and 1.5λ movements).

Result: The PRBFN-FAS provides good matching and Bessel correlation across desired bandwidth, satisfying FAS requirements. System-level experiments confirm viability in practical communication scenarios.

Conclusion: The PRBFN-FAS offers a scalable, electronically reconfigurable alternative to mechanical FAS implementations, enabling faster beamforming suitable for 6G wireless applications.
...
### [234] [Doppler Robust Vortex Wavefront Design for Integrated Sensing and Communication](https://arxiv.org/abs/2512.03802)
*Yuan Liu,Wen-Xuan Long,M. R. Bhavani Shankar,Marco Moretti,Rui Chen,Björn Ottersten*

Main category: eess.SP

TL;DR: Proposes a Doppler-robust ISAC framework using vortex modes with CDMM and VCM-EM algorithm for joint sensing and communication in dynamic scenarios.


<details>
  <summary>Details</summary>
Motivation: Doppler shifts degrade both sensing and communication in dynamic wireless systems, especially for beam-sensitive OAM wavefronts. Current ISAC systems need better Doppler robustness.

Method: Two-phase approach: 1) Sensing phase with multiple vortex modes via CDMM, using VCM-EM algorithm to jointly decode sensing matrix and estimate target parameters. 2) Communication phase with joint Tx beamforming and Rx beam steering based on estimated CSI. Analyzes pilot length trade-off.

Result: Proposed VCM-EM and ISAC designs achieve higher sensing accuracy and communication spectral efficiency than baseline schemes in dynamic scenarios.

Conclusion: The Doppler-robust ISAC framework effectively addresses Doppler-induced interference in dynamic scenes, enabling better performance for both sensing and communication through joint parameter estimation and optimized resource allocation.
...
### [235] [Exploring Syntropic Frameworks in AI Alignment: A Philosophical Investigation](https://arxiv.org/abs/2512.03048)
*Austin Spizzirri*

Main category: cs.AI

TL;DR: AI alignment should shift from encoding fixed human values to building syntropic, reasons-responsive agents through developmental processes, addressing fundamental philosophical challenges in value specification.


<details>
  <summary>Details</summary>
Motivation: Traditional content-based AI alignment approaches face structural instability due to the is-ought gap, value pluralism, and the extended frame problem, necessitating a new philosophical foundation for building aligned AI systems.

Method: Proposes a three-part philosophical framework: 1) Articulates the "specification trap" argument against content-based value specification, 2) Introduces "syntropy" as an information-theoretic framework for multi-agent alignment dynamics, and 3) Establishes functional criteria for genuine moral capacity based on compatibilist theories of guidance control.

Result: Develops a philosophical foundation for reconceiving AI alignment as architecting syntropic, reasons-responsive agents through process-based, multi-agent, developmental mechanisms rather than encoding fixed human values.

Conclusion: The paper provides a philosophical framework that generates falsifiable predictions about value emergence and moral agency in AI systems, though empirical validation remains pending in separate research.
...
### [236] [Beyond the Black Box: A Cognitive Architecture for Explainable and Aligned AI](https://arxiv.org/abs/2512.03072)
*Hu Keyi*

Main category: cs.AI

TL;DR: Weight-Calculatism: A novel cognitive architecture using atomic decomposition (Logical Atoms, Pointing, Comparison) with interpretable Weight-Calculation (Weight = Benefit * Probability) for explainable, aligned AGI.
...
Method: Deconstructs cognition into indivisible Logical Atoms and two fundamental operations (Pointing and Comparison). Uses Weight-Calculation model (Weight = Benefit * Probability) with traceable Initial Weights. Implemented via graph-algorithm-based computational engine and global workspace workflow.

Result: Achieves transparent, human-like reasoning and robust learning in unprecedented scenarios. Provides preliminary code implementation and scenario validation showing the architecture's viability for AGI.

Conclusion: Weight-Calculatism establishes a practical and theoretical foundation for building trustworthy and aligned AGI with radical explainability, intrinsic generality, and traceable value alignment.
...
### [237] [When Do Symbolic Solvers Enhance Reasoning in Large Language Models?](https://arxiv.org/abs/2512.03272)
*Zhiyuan He,Dingmin Wang*

Main category: cs.AI

TL;DR: The paper investigates when symbolic-solver-integrated approaches outperform traditional long Chain-of-Thought reasoning in LLMs, finding they help most with problems requiring limited implicit reasoning but large search spaces.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models often generate lengthy reasoning chains that incur token overhead and can lead to incorrect answers. Symbolic-solver-integrated approaches that translate reasoning tasks to executable code offer a promising alternative, but it's unclear when this approach is superior to conventional long CoT reasoning.

Method: The paper explores when symbolic-solver-integrated methods enhance conventional long CoT by analyzing different problem types. It compares LLM performance on deductive problems vs. constraint satisfaction problems, and examines the impact of declarative exemplars on model performance.

Result: Symbolic-solver-integrated methods only help when problems require limited implicit reasoning but involve ample search space. GPT-4o performs better on deductive problems with shallow reasoning depth, while symbolic solvers significantly improve LLM performance on constraint satisfaction problems requiring repeated backtracking. With declarative exemplars, even CodeLlama-13B can outperform GPT-4o on difficult Zebra puzzles.

Conclusion: The effectiveness of symbolic-solver-integrated approaches depends on problem characteristics - they excel at constraint satisfaction problems with large search spaces but limited implicit reasoning, while traditional CoT remains better for deductive problems with shallow reasoning depth.
...
### [238] [Prior preferences in active inference agents: soft, hard, and goal shaping](https://arxiv.org/abs/2512.03293)
*Filippo Torresan,Ryota Kanai,Manuel Baltieri*

Main category: cs.AI

TL;DR: Active inference agents with different preference distributions (hard/soft goals with/without goal shaping) are tested in grid world navigation, showing goal shaping improves performance but reduces exploration of environment dynamics.


<details>
  <summary>Details</summary>
Motivation: The paper addresses a gap in active inference literature regarding how preference distributions should be specified and how different specifications impact inference and learning. Previous work has given little attention to these questions despite their importance for balancing exploitation and exploration.

Method: The researchers compare four different ways of defining preference distributions: (1) hard goals vs soft goals, and (2) with vs without goal shaping (intermediate goals). They test four agents, each with one of these preference specifications, in a grid world navigation task to evaluate their performance.

Result: Goal shaping enables the best overall performance (promotes exploitation) but sacrifices learning about the environment's transition dynamics (hampers exploration). This reveals a trade-off between exploitation and exploration based on preference distribution design.

Conclusion: The specification of preference distributions significantly impacts active inference agents' behavior, with goal shaping improving task performance at the cost of reduced environmental exploration. This highlights the importance of carefully designing preference distributions to achieve desired balance between exploitation and exploration.
...
### [239] [Evaluating Generalization Capabilities of LLM-Based Agents in Mixed-Motive Scenarios Using Concordia](https://arxiv.org/abs/2512.03318)
...
TL;DR: LLM agents need better evaluation for social cooperation generalization; Concordia environment tests zero-shot mixed-motive cooperation across diverse scenarios, revealing gaps in persuasion and norm enforcement capabilities.


<details>
  <summary>Details</summary>
Motivation: LLM agents are increasingly deployed in social interactions with both human and artificial agents, but existing evaluation methods fail to measure how well their cooperative capabilities generalize to novel social situations. There's a critical need for better evaluation of LLM agents' ability to cooperate in diverse, mixed-motive environments.

Method: Introduces Concordia, a natural language multi-agent simulation environment for evaluating LLM-based agents' ability to cooperate in zero-shot, mixed-motive environments. The method tests agents' ability to identify and exploit opportunities for mutual gain across diverse partners and contexts, using the NeurIPS 2024 Concordia Contest framework with scenarios ranging from negotiation to collective action problems.

Result: Empirical results from the NeurIPS 2024 Concordia Contest reveal significant gaps between current agent capabilities and the robust generalization required for reliable cooperation. Agents particularly struggle in scenarios demanding persuasion and norm enforcement, showing limitations in adapting to novel social situations.

Conclusion: Current LLM agents lack the robust generalization capabilities needed for reliable cooperation in diverse social situations, especially in areas requiring persuasion and norm enforcement. The Concordia framework provides a valuable evaluation method for measuring general cooperative intelligence in zero-shot, mixed-motive environments.
...
### [240] [Multimodal Reinforcement Learning with Agentic Verifier for AI Agents](https://arxiv.org/abs/2512.03438)
...
TL;DR: Argos is an agentic reward system that selects optimal scoring functions to evaluate multimodal reasoning models on final answers, localization, and reasoning quality, achieving SOTA results while preventing reward hacking.
...
Method: Argos (Agentic Reward for Grounded & Objective Scoring) is a principled reward agent that selects from a pool of teacher-model derived and rule-based scoring functions to evaluate: (1) final response accuracy, (2) spatiotemporal localization of entities/actions, and (3) reasoning process quality. It's used across both supervised fine-tuning data curation and RL training.

Result: Achieves state-of-the-art results across multiple agentic tasks including spatial reasoning, visual hallucination, robotics, and embodied AI benchmarks. Demonstrates that SFT post-training alone is insufficient (agents collapse to ungrounded solutions) and that Argos reduces reward-hacking in MMRL.

Conclusion: Argos provides a principled approach to multimodal reasoning rewards that prevents reward hacking and ensures grounded solutions. The theoretical justification through pareto-optimality supports its effectiveness in selecting optimal scoring functions for different samples.
...
### [241] [Multi-Agent Reinforcement Learning with Communication-Constrained Priors](https://arxiv.org/abs/2512.03528)
*Guang Yang,Tianpei Yang,Jingwen Qiao,Yanqing Wu,Jing Huo,Xingguo Chen,Yang Gao*

Main category: cs.AI

TL;DR: Proposes a communication-constrained MARL framework that handles lossy communication by distinguishing lossy/lossless messages and quantifying their impact on global rewards.


<details>
  <summary>Details</summary>
Motivation: Real-world multi-agent systems often face lossy communication, but existing MARL methods lack scalability and robustness for complex dynamic environments. Need a generalized approach to handle varying communication conditions.

Method: 1) Proposes generalized communication-constrained model to characterize communication conditions; 2) Uses it as learning prior to distinguish lossy/lossless messages; 3) Decouples impact of lossy/lossless messages using dual mutual information estimator; 4) Introduces communication-constrained MARL framework that quantifies message impact into global reward.
...
Conclusion: The proposed framework effectively addresses lossy communication challenges in MARL, providing better scalability and robustness for real-world applications with varying communication conditions.
...
### [242] [PARC: An Autonomous Self-Reflective Coding Agent for Robust Execution of Long-Horizon Tasks](https://arxiv.org/abs/2512.03549)
...
Method: Hierarchical multi-agent architecture with task planning, execution, and self-assessment/feedback mechanisms that evaluate actions from independent context, enabling error detection and correction.

Result: Successfully reproduced materials science studies on lithium-ion conduction and alloy segregation, coordinating dozens of parallel simulations (43 hours each). In Kaggle tasks, produced solutions competitive with human-engineered baselines from minimal natural-language instructions.

Conclusion: Integrating hierarchical multi-agent systems with self-assessment and feedback enables AI systems capable of independent, large-scale scientific and analytical work, demonstrating potential for autonomous computational task execution.
...
### [243] [Reason-Plan-ReAct: A Reasoner-Planner Supervising a ReAct Executor for Complex Enterprise Tasks](https://arxiv.org/abs/2512.03560)
...
<details>
  <summary>Details</summary>
Motivation: Current autonomous agents struggle with complex enterprise tasks due to: 1) single-agent architectures causing trajectory instability in plan-execute loops, and 2) privacy requirements forcing use of local open-weight models with smaller context windows that get overwhelmed by large tool outputs.
...
Conclusion: RP-ReAct provides an effective, deployable enterprise solution by fundamentally decoupling planning from execution, enabling reliable complex task solving while addressing privacy and context window constraints.
...
### [244] [EnCompass: Enhancing Agent Programming with Search Over Program Execution Paths](https://arxiv.org/abs/2512.03571)
*Zhening Li,Armando Solar-Lezama,Yisong Yue,Stephan Zheng*

Main category: cs.AI

TL;DR: PAN programming model disentangles agent workflow logic from inference-time strategies, enabling easy experimentation with different search strategies without rewriting core code.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based agent programming approaches entangle core workflow logic with inference-time strategies (like tree search), making it hard to experiment with different strategies without rewriting code.

Method: Introduces "probabilistic angelic nondeterminism" (PAN) programming model and EnCompass framework in Python that uses decorators to compile agent workflows into search spaces, separating workflow logic from inference strategies.
...
Conclusion: PAN provides a clean separation between agent workflow and inference strategy, enabling more flexible and maintainable LLM-based agent development with easy strategy experimentation.
...
### [245] [DeepRule: An Integrated Framework for Automated Business Rule Generation via Deep Predictive Modeling and Hybrid Search Optimization](https://arxiv.org/abs/2512.03607)
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses systematic misalignment between existing theoretical models and real-world economic complexities in retail optimization. It identifies three critical gaps: 1) data modality mismatch where unstructured textual sources impede customer profiling, 2) dynamic feature entanglement challenges in modeling nonlinear price elasticity and time-varying attributes, and 3) operational infeasibility caused by multi-tier business constraints.

Method: The framework introduces a tri-level architecture: 1) Hybrid knowledge fusion engine using LLMs for deep semantic parsing of unstructured text (negotiation records, approval documents) into structured features, 2) Game-theoretic constrained optimization mechanism with bilateral utility functions to reconcile supply chain interests and encode profit redistribution as endogenous objectives, 3) Interpretable decision distillation interface using LLM-guided symbolic regression to find and optimize pricing strategies with economic priors as hard constraints.
...
Conclusion: DeepRule establishes a close-loop pipeline that unifies unstructured knowledge injection, multi-agent optimization, and interpretable strategy synthesis for real economic intelligence in retail assortment and pricing optimization.
...
### [246] [MemVerse: Multimodal Memory for Lifelong Learning Agents](https://arxiv.org/abs/2512.03627)
...
<details>
  <summary>Details</summary>
Motivation: Current AI agents lack reliable memory, leading to catastrophic forgetting, poor long-horizon reasoning, and incoherent operation in multimodal/interactive environments. There's a need for scalable memory systems that can handle multimodal experiences.

Method: MemVerse uses a model-agnostic framework with short-term memory for recent context and hierarchical knowledge graphs for long-term memory. It features continual consolidation, adaptive forgetting, bounded memory growth, and periodic distillation that compresses essential knowledge into parametric models for fast differentiable recall.
...
Conclusion: MemVerse bridges the memory gap in AI agents by providing a scalable, adaptive memory framework that supports both fast recall and structured long-term memory, addressing fundamental limitations in current agent architectures.
...
### [247] [RoCo: Role-Based LLMs Collaboration for Automatic Heuristic Design](https://arxiv.org/abs/2512.03762)
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based automatic heuristic design research often uses single-role approaches, limiting diversity and quality. There's a need for more collaborative systems that can balance exploration and exploitation while maintaining high-quality heuristic generation.
...
Result: RoCo achieves superior performance on five different combinatorial optimization problems in both white-box and black-box settings, consistently generating competitive heuristics that outperform existing methods including ReEvo and HSEvo.

Conclusion: The role-based collaborative paradigm establishes a new standard for robust and high-performing automatic heuristic design, demonstrating the effectiveness of multi-agent collaboration in balancing innovation and exploitation for heuristic generation.
...
### [248] [Omni-AutoThink: Adaptive Multimodal Reasoning via Reinforcement Learning](https://arxiv.org/abs/2512.03783)
...
<details>
  <summary>Details</summary>
Motivation: Existing Omni models show rigid reasoning behaviors - either overthinking simple problems or failing to reason when necessary. Need adaptive reasoning that adjusts depth based on task complexity.

Method: Two-stage framework: 1) Adaptive Supervised Fine-Tuning (SFT) with reasoning-augmented data for fundamental reasoning capability; 2) Adaptive Reinforcement Learning (GRPO) optimizing reasoning behaviors based on task complexity and reward feedback.

Result: Significantly improves adaptive reasoning performance compared to previous baselines. Comprehensive benchmark created spanning text-only, text-audio, text-visual, and text-audio-visual modalities.

Conclusion: Proposed framework effectively addresses rigid reasoning limitations in Omni models by enabling adaptive reasoning depth adjustment. Benchmark and code will be publicly released to advance multimodal reasoning research.
...
### [249] [A Hierarchical Tree-based approach for creating Configurable and Static Deep Research Agent (Static-DRA)](https://arxiv.org/abs/2512.03887)
*Saurav Prateek*

Main category: cs.AI

TL;DR: Static-DRA is a configurable deep research agent using tree-based workflow with Depth and Breadth parameters to balance research quality vs computational cost, achieving 34.72 score on DeepResearch Bench.
...
Method: Tree-based static workflow with hierarchical agent architecture (Supervisor, Independent, Worker agents) and user-tunable Depth and Breadth parameters for granular control over research intensity.

Result: Achieved overall score of 34.72 on DeepResearch Bench using RACE framework with depth=2, breadth=5, and gemini-2.5-pro model; increasing Depth/Breadth parameters improves research depth and evaluation scores.
...
### [250] [Autonomous Agents and Policy Compliance: A Framework for Reasoning About Penalties](https://arxiv.org/abs/2512.03931)
*Vineel Tummala,Daniela Inclezan*

Main category: cs.AI

TL;DR: A logic programming framework for policy-aware autonomous agents that reasons about penalties for non-compliance, allowing deviation when necessary for high-stakes goals while minimizing repercussions.


<details>
  <summary>Details</summary>
Motivation: Prior work focused only on compliance, but real-world scenarios sometimes require deviation from policies to achieve important goals. Additionally, modeling non-compliant behavior helps policymakers understand realistic human decision-making.
...
Result: Generates higher-quality plans that avoid harmful actions while sometimes improving computational efficiency. Framework ensures well-formed policies, accounts for policy priorities, and enhances explainability by identifying rule violations and consequences.
...
### [251] [Benchmark for Planning and Control with Large Language Model Agents: Blocksworld with Model Context Protocol](https://arxiv.org/abs/2512.03955)
*Niklas Jobs,Luis Miguel Vieira da Silva,Jayanth Somashekaraiah,Maximilian Weigand,David Kube,Felix Gehlhoff*

Main category: cs.AI

TL;DR: A benchmark with executable simulation environment for evaluating LLM-based agents in industrial automation planning, using Blocksworld problem with five complexity levels and Model Context Protocol for standardized tool interfaces.


<details>
  <summary>Details</summary>
Motivation: Industrial automation needs flexible control strategies that adapt to changing tasks/environments. LLM-based agents show potential for adaptive planning but lack standardized benchmarks for systematic comparison.

Method: Created benchmark with executable simulation environment for Blocksworld problem with five complexity categories. Integrated Model Context Protocol (MCP) as standardized tool interface to connect diverse agent architectures without implementation-specific modifications.

Result: Demonstrated benchmark's applicability with single-agent implementation, establishing quantitative metrics for comparing LLM-based planning and execution approaches.

Conclusion: The benchmark provides standardized evaluation framework for LLM-based agents in industrial automation planning, enabling systematic comparison of different approaches through MCP integration.
