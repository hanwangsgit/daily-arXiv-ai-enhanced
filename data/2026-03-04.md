...
TL;DR: A new video trajectory editing framework that uses hybrid warping with world cache for static regions and direct warping for dynamic regions, combined with history-guided autoregressive diffusion for long-term coherence, achieving SOTA on new iPhone-PTZ benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing video trajectory editing methods struggle with precise camera control and long-range consistency due to limited pose embedding capacity or single-frame warping with implicit cross-frame aggregation.

Method: 1) Hybrid warping scheme: static regions fused into world cache then rendered to target poses, dynamic regions directly warped; fusion yields globally consistent coarse frames. 2) History-guided autoregressive diffusion model processes video segments with history, with incremental world cache updates to reinforce inpainted content.

Result: Achieves state-of-the-art performance on new iPhone-PTZ benchmark with diverse camera motions and large trajectory variations, using fewer parameters than existing methods.

Conclusion: The proposed framework addresses limitations of existing VTE methods by enabling precise camera control and long-term temporal coherence through explicit cross-frame aggregation and incremental world cache updates.
...
TL;DR: Two independent agents learning world models from different viewpoints develop latent spaces that are linearly isometric, enabling seamless translation between their representations without coordination.


<details>
  <summary>Details</summary>
Motivation: To understand if decentralized vision systems learning from different viewpoints can develop compatible representations without explicit coordination or parameter sharing.

Method: Train separate agents with world models (predictive learning) from distinct viewpoints of the same environment without any parameter sharing or coordination, then analyze geometric relationships between their latent spaces.

Result: The latent spaces exhibit approximate linear isometry, enabling transparent translation between agents. This alignment allows classifier transfer without retraining, accelerates learning via distillation-like migration, and reduces compute requirements.
...
TL;DR: PDP is a prompt-decoupled framework for incremental object detection that uses dual-pool prompt decoupling to separate task-general and task-specific knowledge, plus a prototypical pseudo-label generation module to maintain supervisory consistency, achieving state-of-the-art performance on COCO and VOC benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing prompt-based incremental object detection methods suffer from prompt degradation due to prompt coupling (interference between prompts) and prompt drift (inconsistent supervision where old objects become background in new tasks).

Method: 1) Dual-pool prompt decoupling paradigm with shared pool for task-general knowledge and private pool for task-specific features. 2) Prototypical Pseudo-Label Generation (PPG) module that dynamically updates class prototypes and filters valuable pseudo-labels to maintain consistent supervision.

Result: Achieves state-of-the-art performance with 9.2% AP improvement on MS-COCO and 3.3% AP improvement on PASCAL VOC benchmarks, demonstrating effective balance between stability (remembering old classes) and plasticity (learning new classes).

Conclusion: PDP effectively addresses prompt coupling and drift issues in incremental object detection through explicit prompt decoupling and consistent supervision maintenance, showing strong potential for replay-free continual learning in object detection tasks.
...
TL;DR: AutoFFS is a data-driven framework that generates counterfactual skull morphologies for facial feminization surgery planning using adversarial attacks on sex classifiers.


<details>
  <summary>Details</summary>
Motivation: Current FFS surgical planning relies on subjective clinical assessment without quantitative anatomical guidance, creating a need for reproducible, data-driven approaches for this overlooked patient group.

Method: The framework performs deformation-based targeted adversarial attacks on an ensemble of pre-trained binary sex classifiers that have learned sexual dimorphism, transforming individual skull shapes toward target sex characteristics.

Result: Generated counterfactual skull morphologies provide quantitative foundation for preoperative planning and were validated through classifier-based evaluation and human perceptual studies confirming target sex characteristics.

Conclusion: AutoFFS offers a novel, data-driven approach to FFS planning that addresses the lack of quantitative guidance in current procedures, potentially advancing care for transgender and gender diverse patients.
...
TL;DR: HAMMER is a novel framework that uses multimodal LLMs for interaction intention-driven 3D affordance grounding, leveraging contact-aware embeddings and hierarchical cross-modal integration to accurately localize object affordances in 3D.


<details>
  <summary>Details</summary>
Motivation: Inspired by how humans identify 3D object affordances through observed interactions in images/videos and generalize this knowledge to novel objects, the authors aim to develop a framework that can similarly understand interaction intentions for 3D affordance grounding.

Method: HAMMER aggregates interaction intention from images into contact-aware embeddings, uses hierarchical cross-modal integration to combine MLLM information with 3D representations, and employs multi-granular geometry lifting to infuse spatial characteristics into intention embeddings for accurate 3D affordance localization.
...
Conclusion: HAMMER successfully leverages MLLMs for interaction intention-driven 3D affordance grounding, providing an effective framework that avoids explicit object attribute descriptions and instead focuses on understanding interaction intentions for accurate 3D affordance localization.
...
TL;DR: MERG3R is a training-free divide-and-conquer framework that enables geometric foundation models to scale beyond GPU memory limits by partitioning unordered images into overlapping subsets, reconstructing them independently, and merging results through global alignment.
...
Conclusion: MERG3R provides a model-agnostic framework that allows existing neural geometry models to overcome GPU memory limitations and scale to large image collections while maintaining or improving reconstruction quality.
...
TL;DR: The paper investigates why Video Moment Retrieval (VMR) methods, especially DETR architectures, perform poorly when trained on caption queries but tested on search queries, identifies two generalization gaps, and proposes architectural fixes to improve multi-moment search query performance.


<details>
  <summary>Details</summary>
Motivation: Existing VMR methods degrade significantly when evaluated on search queries despite being trained on caption-based queries, revealing a critical generalization problem in current architectures.

Method: Created three benchmarks by modifying textual queries in public VMR datasets (HD-EPIC, YouCook2, ActivityNet-Captions), identified language and multi-moment generalization gaps, discovered decoder-query collapse issue, and proposed architectural modifications to increase active decoder queries.

Result: The approach improves performance on search queries by up to 14.82% mAP_m, and up to 21.83% mAP_m on multi-moment search queries, demonstrating significant improvements over existing methods.

Conclusion: The paper successfully identifies and addresses key generalization challenges in VMR methods, particularly for search queries and multi-moment instances, providing both diagnostic insights and effective architectural solutions.
...
### [9] [From "What" to "How": Constrained Reasoning for Autoregressive Image Generation](https://arxiv.org/abs/2603.02712)
*Ruxue Yan,Xubo Liu,Wenya Guo,Zhengkun Zhang,Ying Zhang,Xiaojie Yuan*

Main category: cs.CV

TL;DR: CoR-Painter introduces a "How-to-What" paradigm using constrained reasoning to guide autoregressive image generation, addressing spatial ambiguity and object overlap issues through visual constraints and dual-objective optimization.


<details>
  <summary>Details</summary>
Motivation: Current autoregressive image generation methods only specify "What" details to depict by rewriting prompts, but fundamentally fail to reason about "How" to structure the overall image, leading to persistent issues like spatial ambiguity and unrealistic object overlaps.

Method: Proposes CoR-Painter framework with "How-to-What" paradigm: 1) Derives visual constraints from input prompt (spatial relationships, key attributes, compositional rules) to determine "How to draw", 2) Generates detailed description "What to draw" guided by constraints, 3) Introduces Dual-Objective GRPO strategy to optimize textual constrained reasoning and visual projection processes.
...
Conclusion: CoR-Painter successfully bridges the gap in current autoregressive image generation by introducing constrained reasoning to guide both the structural planning ("How") and detailed description ("What"), resulting in more coherent and spatially accurate image synthesis.
...
TL;DR: Proposes patient-specific radiomic feature selection using 2-stage retrieval to find complementary feature sets, outperforming top-k methods while maintaining interpretability comparable to end-to-end DL.


<details>
  <summary>Details</summary>
Motivation: Classical radiomics offers transparency but underperforms due to population-level feature sets. Top-k adaptive radiomics selects redundant features and ignores complementary interactions. Need patient-specific feature selection that finds diverse, complementary evidence while maintaining interpretability.

Method: Two-stage retrieval: 1) Randomly sample diverse candidate feature sets from large radiomic pool, 2) Rank these sets with learned scoring function to select high-performing feature set per patient. System includes feature-set scorer and classifier for final diagnosis.

Result: Outperforms top-k approach with same k values, competitive with end-to-end DL models while maintaining high transparency. Validated on ACL tear detection and KL grading for osteoarthritis. Approximates exhaustive k-feature selection.

Conclusion: Patient-specific feature-set selection via two-stage retrieval achieves diagnostic performance competitive with DL while maintaining interpretability. Generates auditable feature sets linking clinical outcomes to specific anatomical regions and radiomic families for clinician inspection.
...
TL;DR: The paper introduces Cultural Counterfactuals, a synthetic dataset of nearly 60k counterfactual images for measuring cultural biases (religion, nationality, socioeconomic status) in Large Vision-Language Models, addressing gaps in current bias research.


<details>
  <summary>Details</summary>
Motivation: Current bias studies in LVLMs focus primarily on demographic traits visible in images (race, gender), leaving cultural biases (religion, socioeconomic status) understudied due to lack of datasets with cultural context cues.

Method: Created a high-quality synthetic dataset using image-editing models to place people of different demographics into real cultural context images, generating counterfactual sets where the same person appears in multiple cultural contexts.

Result: Developed Cultural Counterfactuals dataset with nearly 60k counterfactual images that enables precise measurement of cultural context impact on LVLM outputs, demonstrating its utility for quantifying cultural biases in popular models.

Conclusion: The Cultural Counterfactuals dataset addresses a critical gap in bias research by enabling systematic measurement of cultural biases in LVLMs, which cannot be discerned from appearance alone and require cultural context cues.
...
TL;DR: A differentiable volumetric body model (KTPolyRigid) for medical imaging that improves anatomical consistency in articulated body analysis compared to surface-based approaches.


<details>
  <summary>Details</summary>
Motivation: Existing surface-based models for articulated body analysis ignore internal volumetric structures and lack anatomical consistency guarantees in deformation methods.

Method: Introduces a differentiable volumetric body model based on SMPL formulation, driven by a new Kinematic Tree-based Log-Euclidean PolyRigid (KTPolyRigid) transform that resolves Lie algebra ambiguities and encourages smooth, bijective volumetric mappings.

Result: On 53 fetal MRI volumes, KTPolyRigid yields deformation fields with significantly fewer folding artifacts, enables robust groupwise image registration, and supports label-efficient, template-based segmentation of fetal organs.

Conclusion: The framework provides a robust foundation for standardized volumetric analysis of articulated bodies in medical imaging.
...
TL;DR: TorchGeo is a PyTorch library for geospatial ML that provides specialized tools for handling large georeferenced imagery, spatial sampling, and coordinate systems, with a tutorial demonstrating water segmentation using Sentinel-2 data.


<details>
  <summary>Details</summary>
Motivation: Earth observation ML pipelines have unique challenges compared to standard computer vision: large georeferenced scenes, raster/vector labels in different coordinate systems, and need for spatially aware sampling strategies. Existing ML libraries don't handle these geospatial complexities well.

Method: TorchGeo provides PyTorch-based datasets, samplers, transforms, and pre-trained models specifically designed for geospatial data. The paper introduces a tutorial demonstrating core abstractions and an end-to-end case study on multispectral water segmentation using Sentinel-2 imagery and the Earth Surface Water dataset.

Result: The tutorial shows how to train a semantic segmentation model using TorchGeo datasets, apply it to a Sentinel-2 scene over Rio de Janeiro, and save predictions as GeoTIFF for further geospatial analysis. Two Python notebooks are provided as practical implementation guides.

Conclusion: TorchGeo simplifies geospatial ML workflows by providing domain-specific tools that handle the unique characteristics of Earth observation data, making it easier to incorporate geospatial data into machine learning pipelines.
...
TL;DR: OpenMarcie is the largest multimodal dataset for human action monitoring in manufacturing, featuring 37+ hours of data from 36 participants performing assembly tasks, benchmarked across three activity recognition tasks.


<details>
  <summary>Details</summary>
Motivation: Smart factories need worker activity recognition to optimize production, improve efficiency, and enhance worker safety. Current datasets for manufacturing environments are limited in scale and multimodal diversity.

Method: Created OpenMarcie dataset with two experimental settings: 1) 12 participants performing bicycle assembly/disassembly without fixed protocol, and 2) 25 volunteers (24 valid) assembling 3D printers with manufacturer instructions. Collected multimodal data from wearables and distributed cameras, totaling 37+ hours of egocentric/exocentric data across 8 data types and 200+ information channels.

Result: OpenMarcie is the largest multimodal dataset for manufacturing human action monitoring. It provides comprehensive data for benchmarking three tasks: activity classification, open vocabulary captioning, and cross-modal alignment.

Conclusion: OpenMarcie enables advanced research in human activity recognition for smart factories, supporting the development of AI systems that can monitor, understand, and optimize manufacturing processes through multimodal analysis of worker activities.
...
TL;DR: QuADD is a unified framework for dataset distillation that jointly optimizes both sample count and data precision under fixed bit budgets, outperforming existing methods in accuracy per bit.


<details>
  <summary>Details</summary>
Motivation: Current dataset distillation methods focus mainly on reducing sample count but overlook data precision and its impact on efficiency. There's a need for a more comprehensive approach that considers both sample reduction and precision optimization under fixed bit constraints.
...
Conclusion: QuADD provides a unified framework for optimizing both dataset compactness and precision under fixed bit budgets, demonstrating superior performance in accuracy per bit across different tasks and establishing a new benchmark for information-efficient dataset distillation.
...
### [16] [Neural Electromagnetic Fields for High-Resolution Material Parameter Reconstruction](https://arxiv.org/abs/2603.02582)
*Zhe Chen,Peilin Zheng,Wenshuo Chen,Xiucheng Wang,Yutao Yue,Nan Cheng*

Main category: cs.CV

TL;DR: NEMF is a framework for creating functional digital twins by solving the ill-posed problem of non-invasive material property inversion using systematic disentanglement of geometry, ambient field, and materials.


<details>
  <summary>Details</summary>
Motivation: Current methods like NeRF create visually rich but functionally incomplete digital twins because they lack underlying material properties (permittivity, conductivity). The challenge is acquiring this information non-invasively, which requires solving an ill-posed physical inversion problem where standard signals deeply entangle geometry, ambient field, and materials.

Method: NEMF uses a systematic disentanglement strategy: 1) leverages high-fidelity geometry from images as an anchor to resolve ambient field, 2) constrains both geometry and field using only non-invasive data, transforming the problem into a well-posed physics-supervised learning task, 3) employs a decoder guided by ambient RF signals and differentiable physical reflection models to output continuous, spatially-varying material parameter fields.

Result: Validated on high-fidelity synthetic datasets, NEMF reconstructs material maps with high accuracy and produces functional digital twins that enable high-fidelity physical simulation, moving beyond passive visual replicas.

Conclusion: NEMF advances digital twin creation from passive visual replicas to truly functional and simulatable models of the physical world by solving the non-invasive material property inversion problem through systematic disentanglement and physics-supervised learning.
...
TL;DR: TruckDrive is a new highway-scale multimodal driving dataset for long-range perception up to 1,000 meters, addressing the gap in existing datasets that only cover short urban ranges, revealing that current autonomous driving models fail at distances beyond 150 meters.


<details>
  <summary>Details</summary>
Motivation: Heavy trucks require long-range scene understanding (hundreds of meters) for safe highway autonomy due to their long braking distances, but existing driving datasets are limited to urban scenes with perception ranges under 100 meters, creating a critical gap for highway truck autonomy.

Method: Created TruckDrive dataset with specialized long-range sensor suite: seven long-range FMCW LiDARs, three high-resolution short-range LiDARs, eleven 8MP surround cameras with varying focal lengths, and ten 4D FMCW radars. Dataset includes 475K samples with 165K densely annotated frames covering up to 1,000 meters for 2D detection and 400 meters for 3D tasks.

Result: State-of-the-art autonomous driving models show severe performance degradation beyond 150 meters, with drops of 31-99% in 3D perception tasks, revealing a systematic long-range gap that current architectures and training signals cannot address.

Conclusion: There is a critical need for specialized long-range perception systems for highway truck autonomy, as current models fail at the distances required for safe heavy truck operation, highlighting the importance of the TruckDrive dataset for advancing long-range autonomous driving research.
...
TL;DR: DINOv3 vision foundation model shows strong segmentation performance but limited detection capabilities for blueberry harvesting tasks, highlighting the need for spatial modeling aligned with fruit-scale structures.


<details>
  <summary>Details</summary>
Motivation: To evaluate the practical role and performance limits of vision foundation models (specifically DINOv3) in agricultural settings, particularly for blueberry robotic harvesting visual tasks, as their effectiveness in such real-world applications remains insufficiently understood.

Method: Evaluated DINOv3 as a frozen backbone for blueberry harvesting visual tasks including fruit/bruse segmentation and fruit/cluster detection using a unified protocol with lightweight decoders. Analyzed how different tasks benefit from patch-level representations and scale with backbone size.

Result: Segmentation tasks benefit consistently from stable patch-level representations and scale with backbone size, while detection tasks are constrained by target scale variation, patch discretization, and localization compatibility. Cluster detection fails due to limitations in modeling relational targets defined by spatial aggregation.

Conclusion: DINOv3 should be viewed as a semantic backbone rather than an end-to-end task model. Its effectiveness depends on downstream spatial modeling aligned with fruit-scale and aggregation structures, providing important guidance for blueberry robotic harvesting applications.
...
TL;DR: MIRAGE framework addresses missing MRI data in Alzheimer's diagnosis by using knowledge graphs and cross-modal latent distillation instead of 3D scan synthesis.


<details>
  <summary>Details</summary>
Motivation: Multimodal AD diagnosis combining MRI and EHR is limited by frequent MRI unavailability due to cost, and synthesizing 3D scans from tabular data is technically challenging and clinically risky.

Method: Uses biomedical knowledge graph with Graph Attention Networks to map EHR variables into unified embeddings. Employs frozen pre-trained 3D U-Net decoder as auxiliary regularization with cohort-aggregated skip feature compensation to enforce structural awareness, creating "diagnostic-surrogate" representations without 3D reconstruction.

Result: Successfully bridges missing-modality gap, improving AD classification rate by 13% compared to unimodal baselines in cohorts without real MRIs.

Conclusion: MIRAGE provides a practical solution for missing MRI data in AD diagnosis by avoiding risky 3D synthesis and instead using latent distillation to create biologically plausible diagnostic representations.
...
TL;DR: ORCA is a multi-agent framework for Document Visual Question Answering that uses specialized agents coordinated through reasoning decomposition, routing, debate mechanisms, and sanity checking to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models struggle with complex reasoning and multi-step workflows in DocVQA, failing to decompose intricate questions and leverage specialized processing for different document elements.

Method: ORCA uses a reasoning agent to decompose queries into logical steps, then routes to specialized agents from a dock. It employs debate mechanisms with stress-testing, thesis-antithesis adjudication, and sanity checking for format consistency.
...
Conclusion: ORCA presents an effective multi-agent framework that addresses DocVQA limitations through strategic agent coordination and iterative refinement, achieving superior performance on complex document reasoning tasks.
...
TL;DR: Transfer learning approach improves peatland fire detection by adapting general wildfire models to peatland-specific characteristics using limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Peatland fires have distinct visual characteristics (smoldering, low flame intensity, persistent smoke, subsurface burning) that make conventional wildfire detectors ineffective, requiring domain-specific solutions.

Method: Transfer learning approach: initialize DL-based peatland fire detector with pretrained weights from conventional wildfire detection model, then fine-tune using Malaysian peatland images/videos dataset.

Result: Transfer learning significantly improves detection accuracy and robustness compared to training from scratch, especially under challenging conditions like low-contrast smoke, partial occlusions, and variable illumination.

Conclusion: The approach provides practical, scalable solution for early peatland fire detection with potential for real-time monitoring systems supporting fire prevention and environmental protection.
...
TL;DR: The paper introduces a comprehensive framework for skin tone fairness, featuring a large-scale open dataset (STW), benchmarks showing deep learning outperforms classic methods, and SkinToneNet - a fine-tuned ViT that achieves state-of-the-art generalization for fairness auditing.


<details>
  <summary>Details</summary>
Motivation: Existing fairness research lacks granular skin tone analysis due to limited datasets (Fitzpatrick scale lacks visual representativeness, small private datasets prevent reproducibility), and existing methods overlook issues like train-test leakage and dataset imbalance.

Method: 1) Created STW dataset: 42,313 images from 3,564 individuals labeled with 10-tone MST scale. 2) Benchmarked Classic Computer Vision (SkinToneCCV) vs Deep Learning approaches. 3) Proposed SkinToneNet: a fine-tuned Vision Transformer (ViT) for state-of-the-art generalization.

Result: Classic models provide near-random results, while deep learning reaches nearly annotator accuracy. SkinToneNet achieves state-of-the-art generalization on out-of-domain data, enabling reliable fairness auditing of public datasets like CelebA and VGGFace2.

Conclusion: The work provides state-of-the-art results in skin tone classification and fairness assessment, with code and data soon available. The framework enables comprehensive fairness auditing of existing datasets using granular skin tone analysis.
...
TL;DR: E2E-GNet is an end-to-end geometric deep neural network for skeleton-based human motion recognition that introduces geometric transformation and distortion-aware optimization layers to enhance discriminative power in non-Euclidean space.


<details>
  <summary>Details</summary>
Motivation: Geometric deep learning has gained attention for capturing meaningful representations in non-Euclidean spaces, but existing methods need better ways to enhance discriminative power between different motions while handling the challenges of skeleton data in non-Euclidean geometry.

Method: E2E-GNet introduces two key layers: 1) a geometric transformation layer that jointly optimizes skeleton motion sequences in non-Euclidean space and applies differentiable logarithm map activation to project them onto linear space, and 2) a distortion-aware optimization layer that limits skeleton shape distortions caused by projection to retain discriminative geometric cues.

Result: The method outperforms other approaches with lower computational cost across five datasets spanning three domains, with ablation studies demonstrating the impact of each proposed layer.

Conclusion: E2E-GNet effectively addresses the challenges of skeleton-based motion recognition in non-Euclidean space through geometric transformation and distortion-aware optimization, achieving superior performance with reduced computational requirements.
...
TL;DR: ModalPatch is a plug-and-play module that enhances multi-modal 3D object detection robustness against transient sensor failures by predicting missing features and using uncertainty-guided fusion.


<details>
  <summary>Details</summary>
Motivation: Real-world autonomous driving faces reliability challenges when sensor modalities (LiDAR/camera) temporarily drop due to hardware glitches, adverse weather, or occlusions, creating critical safety risks during simultaneous modality failures.

Method: ModalPatch uses temporal sensor data for perceptual continuity with a history-based module to predict missing features. It employs uncertainty-guided cross-modality fusion that dynamically estimates feature reliability, suppressing biased signals while reinforcing informative ones.

Result: Extensive experiments show ModalPatch consistently enhances both robustness and accuracy of state-of-the-art 3D object detectors under diverse modality-drop conditions.

Conclusion: ModalPatch provides a practical plug-and-play solution for robust multi-modal 3D object detection without requiring architectural changes or retraining, addressing critical real-world reliability challenges in autonomous driving.
...
TL;DR: WTHaar-Net replaces Hadamard Transform with Haar Wavelet Transform in hybrid quantum-classical CNNs, achieving parameter reduction while maintaining accuracy, with quantum implementation validated on IBM hardware.


<details>
  <summary>Details</summary>
Motivation: To leverage quantum computing advantages for deep learning by using structured linear transforms that can be implemented with shallow quantum circuits, while improving upon prior Hadamard-based approaches with more vision-aligned wavelet transforms.

Method: Introduces WTHaar-Net, a convolutional neural network that replaces the Hadamard Transform with Haar Wavelet Transform (HWT), which provides spatially localized, multi-resolution representations. Shows HWT admits quantum realization using structured Hadamard gates, enabling decomposition into unitary operations suitable for quantum circuits.
...
Conclusion: The Haar Wavelet Transform provides a superior alternative to Hadamard Transform for hybrid quantum-classical CNNs, offering better alignment with vision tasks while enabling efficient quantum implementation, making it suitable for near-term quantum hardware.
...
<details>
  <summary>Details</summary>
Motivation: Practical remote sensing systems often face missing modalities due to sensor failures or incomplete coverage (IMSS). Existing methods suffer from multimodal imbalance (dominant modalities suppressing fragile ones), intra-class variation across modalities, and cross-modal heterogeneity with conflicting semantic cues. Current approaches risk over-alignment, discarding modality-specific information, or imbalanced training favoring robust modalities.

Method: Proposes Semantic-Guided Modality-Aware (SGMA) framework with two plug-and-play modules: (1) Semantic-Guided Fusion (SGF) extracts multi-scale class-wise semantic prototypes, estimates per-modality robustness via prototype-feature alignment, and performs adaptive fusion weighted by robustness scores; (2) Modality-Aware Sampling (MAS) uses robustness estimations to dynamically reweight training samples, prioritizing challenging samples from fragile modalities.
...
Conclusion: SGMA effectively addresses the three key challenges of IMSS by ensuring balanced multimodal learning while reducing intra-class variation and reconciling cross-modal inconsistencies through semantic guidance, demonstrating superior performance especially for fragile modalities.
...
TL;DR: Graph-based deep learning framework compares anatomical vs functional brain parcellations for ASD classification, showing functional parcellation with GAT ensemble achieves 95% accuracy on ABIDE I dataset.


<details>
  <summary>Details</summary>
Motivation: Anatomical brain parcellations dominate rs-fMRI-based ASD classification but may fail to capture idiosyncratic connectivity patterns characteristic of ASD. Need to compare anatomical vs functionally-derived parcellation strategies.

Method: Graph-based deep learning framework comparing anatomical (AAL, 116 ROIs) and functional (MSDL, 39 ROIs) parcellations on ABIDE I dataset (400 balanced subjects). FSL preprocessing with site-stratified splits, Gaussian noise augmentation. Three-phase pipeline: baseline GCN with AAL → optimized GCN with MSDL → Graph Attention Network ensemble.

Result: Baseline GCN with AAL: 73.3% accuracy (AUC=0.74). Optimized GCN with MSDL: 84.0% accuracy (AUC=0.84). GAT ensemble: 95.0% accuracy (AUC=0.98), outperforming all recent GNN-based benchmarks on ABIDE I. 10.7-point gain from atlas substitution alone shows functional parcellation is most impactful decision.

Conclusion: Functional parcellation is crucial for capturing ASD connectivity patterns. GAT ensemble achieves state-of-the-art performance. Model decisions reflect ASD neuropathology (focus on Posterior Cingulate Cortex and Precuneus as Default Mode Network hubs) rather than acquisition artefacts.
...
TL;DR: NeighborMAE: A masked image modeling approach for Earth Observation that learns spatial dependencies by jointly reconstructing neighboring images, outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: Current masked image modeling approaches for Earth Observation overlook spatial dependencies between neighboring images, which contain rich contextual information since the Earth's surface is continuous.

Method: Proposes NeighborMAE that learns spatial dependencies through joint reconstruction of neighboring Earth Observation images. Uses a heuristic strategy to dynamically adjust mask ratio and pixel-level loss weight to maintain reconstruction challenge.
...
Conclusion: Neighboring images provide valuable information for masked image modeling in Earth Observation, and the proposed designs are effective for learning spatial dependencies.
...
TL;DR: EIMC proposes an early collaborative perception paradigm that injects lightweight collaborative voxels into local modality fusion, uses heatmap-driven consensus to identify low-confidence regions, and employs instance-centric messaging to reduce bandwidth by 88% while maintaining high detection accuracy.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal collaborative perception approaches follow a "local fusion to communication" sequence that requires high bandwidth for transmitting individual feature data before collaborative fusion, which is inefficient for autonomous driving safety applications.

Method: 1) Early collaborative paradigm with lightweight collaborative voxels injected into local modality fusion; 2) Heatmap-driven consensus protocol to identify low-confidence regions needing cooperation; 3) Top-K instance vector querying from peers for completion via cross-attention; 4) Refinement fusion collecting top-K confident instances and enhancing features via self-attention.
...
Conclusion: EIMC demonstrates that early collaboration with instance-centric messaging can significantly reduce communication overhead while maintaining high detection performance, offering a practical solution for multi-modal collaborative perception in autonomous driving.
...
TL;DR: ForestPersons is a novel large-scale dataset for under-canopy person detection in forest SAR missions, addressing limitations of aerial imagery that misses visual cues beneath dense canopy cover.


<details>
  <summary>Details</summary>
Motivation: Current UAV-based missing person detection in forests is limited because aerial perspectives often miss individuals concealed under dense canopy cover, creating a need for under-canopy visual data that better reflects actual SAR mission conditions.

Method: Created ForestPersons dataset with 96,482 images and 204,078 annotations collected under diverse environmental/temporal conditions, including bounding boxes, pose, and visibility labels for occlusion-aware analysis, representing ground-level and low-altitude perspectives.

Result: Baseline evaluations show standard object detection models trained on prior large-scale or SAR-oriented datasets perform poorly on ForestPersons, indicating existing benchmarks don't address under-canopy detection challenges.

Conclusion: ForestPersons fills a critical gap for under-canopy person detection research and provides a benchmark to advance real-world SAR capabilities, with the dataset publicly available for community use.
...
TL;DR: GAD (Generation-Assisted Discriminative) classifier combines generative and discriminative approaches for efficient and accurate closed-set action understanding in MLLMs, achieving state-of-the-art results with faster inference.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs use inefficient autoregressive generation for action classification, suffering from semantic overlap in action labels and slow inference. Discriminative classifiers are more efficient but there's potential to combine both approaches for better performance.

Method: Proposes GAD classifier that operates only during fine-tuning, preserving MLLM pretraining compatibility. It uses generative modeling to complement discriminative classifiers, combining their strengths while maintaining efficiency.
...
Conclusion: GAD successfully bridges generative and discriminative approaches, demonstrating that generative modeling can enhance discriminative classifiers for closed-set action understanding while preserving efficiency and compatibility with MLLM pretraining.
...
TL;DR: SemGS is a feed-forward framework for reconstructing generalizable semantic fields from sparse image inputs, enabling semantic scene reconstruction and novel view synthesis without scene-specific optimization.


<details>
  <summary>Details</summary>
Motivation: Existing methods for semantic scene reconstruction and semantic-aware novel view synthesis require dense multi-view inputs and scene-specific optimization, limiting practicality and scalability in real-world robotics applications.

Method: Uses dual-branch architecture to extract color and semantic features with shared shallow CNN layers, camera-aware attention mechanism for geometric relationships, decodes into dual-Gaussians with shared geometry, and adds regional smoothness loss for semantic coherence.
...
Conclusion: SemGS offers a practical and scalable solution for semantic scene understanding from sparse inputs, enabling effective robot operation in complex environments without requiring dense multi-view data or per-scene optimization.
...
TL;DR: GKD is a multi-stage knowledge distillation framework that enhances out-of-domain generalization for semantic segmentation by decoupling representation learning from task learning, using selective feature distillation and query-based soft distillation.


<details>
  <summary>Details</summary>
Motivation: Conventional knowledge distillation methods for semantic segmentation focus on preserving in-domain accuracy but neglect out-of-domain generalization, which is crucial under distribution shifts. This limitation becomes more severe with vision foundation models (VFMs) - while VFMs show strong robustness on unseen data, distilling them with conventional KD often compromises this generalization ability.
...
Conclusion: GKD effectively addresses the generalization limitation in conventional knowledge distillation for semantic segmentation, particularly when distilling vision foundation models, by explicitly enhancing out-of-domain generalization through its multi-stage decoupled approach and selective knowledge transfer mechanisms.
...
TL;DR: VC-STaR is a self-improving framework that uses visual contrastive pairs to reduce hallucinations in vision-language models' reasoning, creating a 55K dataset that boosts VLM reasoning performance beyond existing methods.


<details>
  <summary>Details</summary>
Motivation: Self-improving techniques work well for language models but face challenges in VLMs due to visual hallucinations that can't be easily verified. The authors observed that VLMs identify visual cues more precisely when presented with contrastive VQA pairs (similar images with synonymous questions).

Method: Proposed Visual Contrastive Self-Taught Reasoner (VC-STaR) framework that leverages visual contrast to mitigate hallucinations. Collected diverse VQA datasets, curated contrastive pairs based on multi-modal similarity, generated rationales using VC-STaR, and created VisCoR-55K dataset for supervised finetuning.
...
Conclusion: Visual contrastive self-improving is effective for enhancing VLM reasoning by reducing hallucinations, and the created VisCoR-55K dataset significantly boosts reasoning capabilities across various VLMs.
...
TL;DR: CAPT is a confusion-aware prompt tuning framework that helps vision-language models like CLIP learn from their own misclassifications to reduce systematic errors between visually/semantically similar categories.
...
Method: CAPT constructs a Confusion Bank to model stable confusion relationships. It introduces: 1) Semantic Confusion Miner (SEM) to capture global inter-class confusion through semantic difference and commonality prompts, 2) Sample Confusion Miner (SAM) to retrieve misclassified instances and capture sample-level cues via a Diff-Manner Adapter, and 3) Multi-Granularity Difference Expert (MGDE) to unify semantic- and sample-level confusion information.
...
Conclusion: CAPT effectively addresses systematic misclassifications in vision-language models by enabling them to learn from their own confusion patterns, improving fine-grained discriminative ability and reducing intrinsic biases.
...
TL;DR: CAWM-Mamba is the first end-to-end framework for multimodal image fusion that handles compound adverse weather conditions (e.g., haze+rain, rain+snow) using unified shared weights, outperforming existing methods in both compound and single-weather scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing adverse weather fusion methods only handle single types of degradation (haze, rain, or snow) and fail when multiple degradations coexist, which is crucial for real-world applications like autonomous driving and UAV monitoring in complex weather conditions.

Method: Three key components: (1) Weather-Aware Preprocess Module (WAPM) for enhancing degraded visible features and extracting global weather embeddings; (2) Cross-modal Feature Interaction Module (CFIM) for aligning heterogeneous modalities and exchanging complementary features; (3) Wavelet Space State Block (WSSB) with wavelet-domain decomposition to decouple multi-frequency degradations, including Freq-SSM for modeling anisotropic high-frequency degradation without redundancy and unified degradation representation.

Result: Extensive experiments on AWMM-100K benchmark and three standard fusion datasets show CAWM-Mamba consistently outperforms state-of-the-art methods in both compound and single-weather scenarios. Fusion results also excel in downstream tasks (semantic segmentation and object detection), confirming practical value for real-world adverse weather perception.

Conclusion: CAWM-Mamba successfully addresses the challenge of compound adverse weather conditions in multimodal image fusion, providing superior performance and practical value for real-world applications through its unified architecture that jointly performs image fusion and weather restoration.
...
TL;DR: Track4World is a feedforward model for efficient holistic 3D tracking of every pixel in monocular videos using world-centric coordinates.


<details>
  <summary>Details</summary>
Motivation: Existing monocular 3D tracking methods are limited to either sparse point tracking on first frames or slow optimization-based frameworks for dense tracking, lacking efficient holistic 3D tracking of every pixel.

Method: Built on VGGT-style ViT for global 3D scene representation, Track4World uses a novel 3D correlation scheme to simultaneously estimate pixel-wise 2D and 3D dense flow between arbitrary frame pairs, enabling efficient 3D tracking of every pixel.

Result: Extensive experiments on multiple benchmarks show consistent outperformance over existing methods in 2D/3D flow estimation and 3D tracking, demonstrating robustness and scalability for real-world 4D reconstruction.

Conclusion: Track4World enables efficient holistic 3D tracking of every pixel in monocular videos, advancing comprehensive understanding of 3D dynamics and supporting real-world 4D reconstruction tasks.
...
<details>
  <summary>Details</summary>
Motivation: Transformers show promise in image restoration but face computational burden due to quadratic complexity of self-attention. Existing methods restrict attention to local windows, limiting receptive field and performance.

Method: Proposes Adaptive Token Dictionary (ATD) with learnable token dictionary summarizing external image priors. Uses Token Dictionary Cross-Attention (TDCA) to enhance features via dictionary interaction, groups features into categories based on attention maps, and integrates category information into feed-forward network.

Result: ATD and ATD-light achieve state-of-the-art performance on multiple image super-resolution benchmarks. ATD-U variant also excels at denoising and JPEG artifact removal, demonstrating quantitative and qualitative superiority.

Conclusion: ATD enables efficient global dependency modeling in transformers for image restoration with linear complexity, overcoming computational limitations while maintaining superior performance across multiple restoration tasks.
...
TL;DR: This paper evaluates image data augmentation techniques for Bengali handwritten character recognition using lightweight models to address limited dataset availability in resource-scarce languages.


<details>
  <summary>Details</summary>
Motivation: Deep learning models require large datasets to avoid overfitting, but large-scale datasets are often unavailable for resource-limited languages like Bengali. The study aims to address the limited data problem for Bengali handwritten characters through data augmentation.

Method: The study conducts experiments with various image augmentation techniques (CLAHE, Random Rotation, Random Affine, Color Jitter, and their combinations) applied to Bengali handwritten character datasets (Ekush and AIBangla). It evaluates these techniques with a lightweight EfficientViT model.

Result: The combination of Random Affine and Color Jitter produced the best accuracy: 97.48% on Ekush dataset and 97.57% on AIBangla dataset, outperforming all other individual and combined augmentation techniques.

Conclusion: Data augmentation is an effective approach for addressing limited data problems in resource-scarce languages like Bengali. The combination of Random Affine and Color Jitter augmentation works particularly well for Bengali handwritten character recognition with lightweight models.
...
TL;DR: Synthetic-Child: AIGC pipeline generates photorealistic child posture images with keypoint annotations using zero real child photos, achieving 71.2 AP on real-child test (+12.5 AP improvement) and real-time edge deployment.


<details>
  <summary>Details</summary>
Motivation: Accurate child posture estimation is critical for AI study companions, but collecting annotated child datasets is expensive and ethically problematic due to privacy concerns. Need alternative to real child imagery.

Method: Four-stage pipeline: (1) Programmable 3D child body model (SMPL-X) generates diverse study poses with anatomical plausibility; (2) Dual ControlNet (pose+depth) conditioned on FLUX-1 Dev synthesizes photorealistic images; (3) ViTPose confidence filtering and targeted augmentation; (4) RTMPose-M fine-tuning with geometric feature engineering and MLP classification, then INT8 quantization.

Result: FP16 model achieves 71.2 AP on real-child test set (n~300), +12.5 AP improvement over COCO-pretrained baseline. INT8 quantization retains 70.4 AP while running at 22 FPS on 0.8-TOPS Rockchip RK3568 NPU. Outperforms commercial posture corrector with higher recognition rates and ~1.8x faster response.

Conclusion: Carefully designed AIGC pipelines can substantially reduce dependence on real child imagery while achieving deployment-ready accuracy, with potential applications to other privacy-sensitive domains.
...
TL;DR: VLMFusionOcc3D uses vision-language models and weather-aware fusion to improve 3D semantic occupancy prediction in autonomous driving, addressing semantic ambiguity and adverse weather degradation.


<details>
  <summary>Details</summary>
Motivation: Current voxel-based occupancy models struggle with semantic ambiguity in sparse geometric grids and performance degradation under adverse weather conditions, limiting reliable autonomous driving in complex urban environments.

Method: 1. Dual-branch feature extraction from multi-view images and LiDAR into unified voxel space. 2. Instance-driven VLM Attention (InstVLM) using gated cross-attention and LoRA-adapted CLIP embeddings to inject semantic/geographic priors. 3. Weather-Aware Adaptive Fusion (WeathFusion) with dynamic gating based on vehicle metadata and weather-conditioned prompts. 4. Depth-Aware Geometric Alignment (DAGA) loss for camera-LiDAR consistency.
...
Conclusion: VLMFusionOcc3D provides a scalable, robust solution for 3D semantic occupancy prediction that effectively addresses semantic ambiguity and weather degradation through multimodal fusion and vision-language priors.
...
TL;DR: InterNeg improves OOD detection by using consistent inter-modal distance enhancement in VLMs, avoiding intra-modal inconsistencies that hurt performance.


<details>
  <summary>Details</summary>
Motivation: Current VLM-based OOD detection methods use intra-modal distance comparisons (e.g., negative texts vs ID labels, test images vs image proxies), which creates inconsistency with the inter-modal distance optimization of CLIP-like models, leading to suboptimal performance.

Method: InterNeg systematically enhances consistent inter-modal distance from both textual and visual perspectives: 1) Textual: uses inter-modal criterion for selecting negative texts; 2) Visual: dynamically identifies high-confidence OOD images and inverts them into textual space to generate extra negative text embeddings guided by inter-modal distance.

Result: Achieves state-of-the-art performance with 3.47% reduction in FPR95 on ImageNet benchmark and 5.50% improvement in AUROC on Near-OOD benchmark.

Conclusion: InterNeg's consistent inter-modal distance enhancement framework effectively addresses the inconsistency problem in current VLM-based OOD detection methods, demonstrating superior performance across multiple benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Current single-view 3D human reconstruction methods using multi-view diffusion models produce unnatural poses, especially for dynamic/challenging poses, due to limited diversity in available 3D human datasets.

Method: DrPose uses direct reward fine-tuning on poses (DrPose) with PoseScore - a differentiable reward measuring consistency between generated multi-view images and ground-truth poses. Trains on DrPose15K dataset constructed from human motion data and pose-conditioned video generation.

Result: Consistent qualitative and quantitative improvements across conventional benchmarks, in-the-wild images, and new challenging pose benchmarks, with better performance on diverse poses.

Conclusion: DrPose effectively addresses pose diversity limitations in 3D human reconstruction by leveraging abundant pose data without requiring expensive 3D assets, enabling better reconstruction of challenging poses.
...
TL;DR: IB-IUMAD is a novel denoising framework for incremental unified multimodal anomaly detection that addresses catastrophic forgetting by eliminating spurious and redundant features through Mamba decoder and information bottleneck fusion.


<details>
  <summary>Details</summary>
Motivation: Current incremental unified multimodal anomaly detection frameworks overlook the negative impact of spurious and redundant features on catastrophic forgetting, and naive multimodal aggregation exacerbates forgetting problems.

Method: Proposes IB-IUMAD framework with two key components: 1) Mamba decoder to disentangle inter-object feature coupling and prevent spurious feature interference, and 2) Information bottleneck fusion module to filter redundant features and preserve discriminative information.

Result: Theoretical analyses and experiments on MVTec 3D-AD and Eyecandies datasets demonstrate IB-IUMAD's effectiveness and competitive performance in incremental unified multimodal anomaly detection.

Conclusion: Addressing spurious and redundant features is crucial for mitigating catastrophic forgetting in incremental unified multimodal anomaly detection, and the proposed IB-IUMAD framework successfully achieves this through complementary denoising mechanisms.
...
TL;DR: SEP-YOLO: A novel framework for transparent object instance segmentation using dual-domain collaboration with frequency domain enhancement and spatial refinement, achieving SOTA performance on Trans10K and GVD datasets.


<details>
  <summary>Details</summary>
Motivation: Transparent object instance segmentation is challenging due to boundary blur, low contrast, and high background dependence. Existing methods fail because they rely on strong appearance cues and clear boundaries.

Method: Proposes SEP-YOLO with dual-domain collaborative mechanism: 1) Frequency Domain Detail Enhancement Module using learnable complex weights to enhance weak high-frequency boundaries, 2) Multi-scale spatial refinement stream with Content-Aware Alignment Neck and Multi-scale Gated Refinement Block for precise feature alignment and boundary localization. Also provides high-quality instance-level annotations for Trans10K dataset.
...
Conclusion: SEP-YOLO effectively addresses transparent object segmentation challenges through dual-domain collaboration and provides valuable annotated data, advancing the field of transparent object instance segmentation.
...
TL;DR: OmniFashion is a unified vision-language framework that bridges diverse fashion tasks (retrieval, recommendation, recognition, dialogue) using a million-scale dataset with comprehensive annotations, enabling multi-task reasoning and interactive dialogue.


<details>
  <summary>Details</summary>
Motivation: Current fashion intelligence suffers from fragmented supervision and incomplete annotations, preventing vision-language models from serving as a generalist fashion brain that can unify understanding and reasoning across multiple tasks.

Method: Construct FashionX dataset (million-scale with exhaustive outfit annotations and hierarchical attributes), then propose OmniFashion framework that bridges diverse fashion tasks under a unified fashion dialogue paradigm.
...
Conclusion: The approach offers a scalable path toward universal, dialogue-oriented fashion intelligence by addressing annotation limitations and enabling unified multi-task reasoning.
...
TL;DR: DREAM is a unified framework that combines visual representation learning and text-to-image generation in a single model, achieving state-of-the-art performance on both discriminative and generative tasks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of unifying visual representation learning and text-to-image generation within a single multimodal model, which has been difficult to achieve effectively.

Method: DREAM uses Masking Warmup during training (progressive masking schedule from minimal to full masking) and Semantically Aligned Decoding at inference (aligning masked image candidates with target text for selection).

Result: Trained on CC12M, DREAM achieves 72.7% ImageNet linear-probing accuracy (+1.1% over CLIP), FID of 4.25 (+6.2% over FLUID), +6.3% text-image fidelity, and consistent gains in few-shot classification, semantic segmentation, and depth estimation.

Conclusion: Discriminative and generative objectives can be synergistic, enabling unified multimodal models that excel at both visual understanding and generation tasks.
...
TL;DR: TagaVLM is a new framework that explicitly injects topological structures into vision-language models to solve the architectural mismatch problem in Vision-Language Navigation, achieving state-of-the-art performance on R2R benchmark.


<details>
  <summary>Details</summary>
Motivation: There's a fundamental architectural mismatch between VLMs (pretrained on static vision-language tasks) and VLN (dynamic, embodied, spatially-structured navigation). Existing methods either convert visual/spatial info to text (forcing implicit inference) or limit global action capabilities.

Method: TagaVLM explicitly injects topological structures into VLM backbone through: 1) Spatial Topology Aware Residual Attention (STAR-Att) that integrates topological edge info into self-attention, 2) Interleaved Navigation Prompt for node-level visual-text alignment, enabling global action reasoning for robust path correction.

Result: On R2R benchmark, achieves state-of-the-art among large-model-based methods: 51.09% Success Rate and 47.18 SPL in unseen environments, outperforming prior work by 3.39% SR and 9.08 SPL.

Conclusion: Targeted enhancements on smaller open-source VLMs can be more effective than brute-force model scaling for embodied spatial reasoning. Explicit topological structure injection bridges the architectural gap between VLMs and navigation tasks.
...
TL;DR: VisionCreator: An end-to-end visual-generation agentic model with unified Understanding, Thinking, Planning, and Creation (UTPC) capabilities that outperforms larger closed-source models.


<details>
  <summary>Details</summary>
Motivation: General models lack nuanced understanding of design conventions and creative workflows, while workflow-based agents lack specialized knowledge for autonomous creative planning. There's a need for a unified approach that combines both capabilities.

Method: Proposes VisionCreator with UTPC framework, uses VisGenData-4k (creation trajectories with explicit UTPC structures), Progressive Specialization Training (PST), Virtual Reinforcement Learning (VRL) in simulated environment, and comprehensive VisGenBench benchmark.

Result: VisionCreator-8B/32B models demonstrate superior performance over larger closed-source models across multiple evaluation dimensions on the 1.2k test sample benchmark.

Conclusion: The work provides a foundation for future research in visual-generation agentic systems by unifying understanding, thinking, planning, and creation capabilities in an end-to-end learnable framework.
...
<details>
  <summary>Details</summary>
Motivation: Current VLA models have limitations: world-model VLAs waste capacity reconstructing redundant backgrounds, while latent-action VLAs lack continuous dynamic modeling and world knowledge. There's a need to combine the benefits of both approaches.

Method: 1) Uses pretrained video VAE as latent motion extractor to factorize video into structure and motion latents. 2) During pre-training, VLA learns to infer continuous latent motion chain from instruction and initial frame, predicting terminal frame. 3) During co-fine-tuning, aligns latent dynamics with discrete action prediction by jointly modeling sparse keyframes and action sequences in unified autoregressive decoder.
...
Conclusion: CoWVLA preserves world-model benefits of temporal reasoning and world knowledge while retaining compactness and interpretability of latent actions, enabling efficient visuomotor learning and representing a more effective VLA pretraining paradigm.
...
<details>
  <summary>Details</summary>
Motivation: Existing cold and generalized diffusion models for sparse-view CT reconstruction rely on ad hoc sampling controls or fixed schedules, which are sensitive to error accumulation and sampling instability. There's a need for more robust and stable sampling strategies.

Method: ReCo-Diff uses residual-conditioned self-guided sampling. At each sampling step, it first produces an unconditioned baseline reconstruction, then conditions subsequent predictions on the observation residual between the predicted image and measured sparse-view input. This provides continuous, measurement-aware correction while preserving deterministic sampling.
...
Conclusion: The residual-conditioned diffusion framework provides an effective approach for sparse-view CT reconstruction by leveraging observation residuals for continuous guidance without requiring heuristic interventions, leading to more stable and accurate results.
...
TL;DR: FiDeSR is a one-step diffusion super-resolution framework that preserves fine details while ensuring high-fidelity reconstruction through adaptive weighting, frequency-specific enhancers, and noise refinement.
...
Method: 1) Detail-aware weighting strategy during training that adaptively emphasizes regions with higher prediction errors; 2) Low- and high-frequency adaptive enhancers during inference for flexible enhancement control; 3) Residual-in-residual noise refinement to correct prediction errors in diffusion noise and enhance fine detail recovery.
...
Conclusion: FiDeSR provides an effective one-step diffusion framework for high-fidelity, detail-preserving image super-resolution that outperforms existing methods while offering flexible enhancement control.
...
TL;DR: ShareVerse is a video generation framework for multi-agent shared world modeling that addresses the lack of unified shared world construction in existing works, enabling consistent multi-agent interaction in generated videos.


<details>
  <summary>Details</summary>
Motivation: Existing video generation works lack support for unified shared world construction with multi-agent interaction. There's a gap in modeling consistent shared environments where multiple agents interact, which is crucial for realistic multi-agent scenarios.

Method: 1) Built large-scale multi-agent interactive world dataset on CARLA simulation platform with diverse scenes, weather conditions, and interactive trajectories with paired multi-view videos (front/rear/left/right views per agent). 2) Proposed spatial concatenation strategy for four-view videos of independent agents to model broader environment and ensure internal multi-view geometric consistency. 3) Integrated cross-agent attention blocks into pretrained video model to enable interactive transmission of spatial-temporal information across agents.
...
Conclusion: ShareVerse successfully addresses the multi-agent shared world modeling challenge by combining large-scale simulation data, spatial concatenation strategies, and cross-agent attention mechanisms, enabling consistent and realistic multi-agent video generation.
...
TL;DR: GTDoctor is an AI model for gestational trophoblastic disease diagnosis that performs lesion segmentation and provides diagnostic conclusions, reducing diagnostic time from 56 to 16 seconds per case with high accuracy.


<details>
  <summary>Details</summary>
Motivation: Current GTD pathological diagnosis is time-consuming, heavily reliant on pathologist experience, has low initial diagnosis consistency, and threatens maternal health and reproductive outcomes.

Method: Developed GTDoctor expert model for pixel-based lesion segmentation on pathological slides, then created GTDiagnosis software system and conducted both retrospective (679 slides) and prospective (68 patients) clinical trials.

Result: Retrospective: mean precision >0.91 for lesion detection. Prospective: 95.59% PPV. Reduced diagnostic time from 56 to 16 seconds per case (285 patients).

Conclusion: GTDoctor and GTDiagnosis provide a novel solution for GTD diagnosis that enhances diagnostic performance and efficiency while maintaining clinical interpretability.
...
TL;DR: A unified image restoration framework using dual-level Mixture-of-Experts with pretrained diffusion model to handle diverse degradation types like haze, blur, noise, and low-light.
...
Method: Proposes a unified framework integrating dual-level Mixture-of-Experts (MoE) architecture with pretrained diffusion model. Inter-MoE layer adaptively combines expert groups for major degradation types, while Intra-MoE layer selects specialized sub-experts for fine-grained variations within each type.
...
Conclusion: The dual-level MoE architecture enables both coarse-grained adaptation across diverse degradation categories and fine-grained modulation for specific intra-class variations, ensuring high specialization in handling complex, real-world corruptions.
...
TL;DR: TenExp is a mixture-of-experts-based tensor decomposition structure search framework that dynamically selects and activates suitable tensor decompositions in an unsupervised fashion, going beyond fixed factor-interaction families and enabling mixture decompositions.


<details>
  <summary>Details</summary>
Motivation: Current tensor decomposition structure search methods are limited by fixed factor-interaction families (like tensor contraction) and cannot deliver mixture decompositions. Selecting suitable tensor decompositions to capture low-rank data structures remains challenging and under-explored.

Method: TenExp uses a mixture-of-experts-based framework that dynamically selects and activates suitable tensor decompositions in an unsupervised fashion. It can provide both suitable single decompositions beyond fixed factor-interaction families and suitable mixture decompositions beyond single decompositions.

Result: Theoretical analysis provides approximation error bounds revealing TenExp's approximation capability. Extensive experiments on synthetic and realistic datasets demonstrate superiority over state-of-the-art tensor decomposition-based methods.

Conclusion: TenExp addresses limitations of current tensor decomposition structure search methods by enabling dynamic selection of suitable decompositions and mixture decompositions, showing superior performance through theoretical analysis and experimental validation.
...
TL;DR: SFDE proposes a three-branch network using spatial and frequency domains for cross-view geo-localization, achieving state-of-the-art performance with efficient design.


<details>
  <summary>Details</summary>
Motivation: Cross-view geo-localization faces challenges from severe geometric asymmetry, texture inconsistency, and degradation of local information. Existing spatial domain methods are sensitive to large viewpoint variations and local disturbances.

Method: Spatial and Frequency Domain Enhancement Network (SFDE) with three parallel branches: global semantic context modeling, local geometric structure modeling, and frequency domain statistical stability modeling. Features are jointly optimized via progressive enhancement and coupled constraints.

Result: SFDE achieves competitive performance and often surpasses state-of-the-art methods while maintaining lightweight and computationally efficient design.

Conclusion: Leveraging complementary spatial and frequency domain representations enables learning cross-view representations with consistency across multiple granularities, addressing limitations of spatial-only approaches.
...
TL;DR: PVT-GDLA is a decoder-centric Transformer for medical image segmentation that achieves linear-time attention with sharp boundaries through gated differential linear attention and local token mixing.


<details>
  <summary>Details</summary>
Motivation: Medical segmentation needs models that preserve fine anatomical boundaries while being efficient for clinical deployment. Transformers have quadratic attention costs and large data requirements, while CNNs struggle with global reasoning. Linear attention offers O(N) scaling but suffers from training instability and attention dilution.

Method: Proposes PVT-GDLA with Gated Differential Linear Attention (GDLA) that computes two kernelized attention paths on complementary query/key subspaces, subtracts them with learnable scaling to cancel noise, and uses head-specific gates for nonlinearity and sparsity. Includes parallel local token-mixing with depthwise convolution for boundary fidelity, all while maintaining O(N) complexity.

Result: Achieves state-of-the-art accuracy across CT, MRI, ultrasound, and dermoscopy benchmarks under equal training budgets, with comparable parameters but lower FLOPs than CNN-, Transformer-, hybrid-, and linear-attention baselines.

Conclusion: PVT-GDLA provides a practical path to fast, scalable, high-fidelity medical segmentation in clinical environments and other resource-constrained settings.
...
<details>
  <summary>Details</summary>
Motivation: Existing shadow generation methods focus on single-object insertion and fail to generalize to multiple foreground objects, while real-world compositing pipelines often require inserting multiple objects simultaneously with jointly consistent shadows.

Method: Uses a pre-trained text-to-image diffusion model with two pathways: an image pathway for multi-scale spatial guidance, and a text pathway encoding per-object shadow bounding boxes as learned positional tokens fused via cross-attention with attention-alignment loss.

Result: Achieves state-of-the-art performance in both single and multi-object shadow generation settings, demonstrating superior generalization to multiple objects.

Conclusion: The proposed method effectively addresses the under-explored problem of multi-object shadow generation, producing physically plausible shadows for multiple inserted objects with joint consistency in geometry, attachment, and location.
...
TL;DR: iGVLM introduces instruction-guided visual modulation with a dual-branch architecture to address representation bottlenecks in LVLMs, enabling task-specific visual reasoning while preserving pre-trained visual priors.


<details>
  <summary>Details</summary>
Motivation: Current LVLMs suffer from representation bottlenecks due to static, instruction-agnostic vision encoders that use visual representations in an invariant manner across different textual tasks, hindering fine-grained reasoning where task-specific visual cues are critical.

Method: Proposes iGVLM with a decoupled dual-branch architecture: (1) frozen representation branch preserving task-agnostic visual representations, and (2) dynamic conditioning branch performing affine feature modulation via Adaptive Layer Normalization (AdaLN). Also introduces MM4 diagnostic probe for evaluating logical consistency.

Result: iGVLM consistently enhances instruction sensitivity across diverse language backbones, offering a plug-and-play paradigm for bridging passive perception and active reasoning. Extensive results demonstrate improved performance.

Conclusion: iGVLM provides a general framework for instruction-guided visual modulation that enables smooth transition from general-purpose perception to instruction-aware reasoning while maintaining structural integrity of pre-trained visual priors.
...
TL;DR: RADAR is a training-free inference method that uses MLLMs' intrinsic attention to guide progressive localization and fine-grained reasoning, reducing hallucinations in remote sensing VQA.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs suffer from pronounced hallucinations in remote sensing VQA due to visual grounding failures in large-scale scenes and misinterpretation of fine-grained small targets.

Method: Proposes RADAR (Relative Attention-Driven Actively Reasoning), a training-free inference method that leverages MLLMs' intrinsic attention to guide progressive localization and fine-grained local reasoning at test time. Also introduces RSHBench benchmark for fine-grained diagnosis of hallucinations.
...
Conclusion: RADAR effectively mitigates grounding-induced factual hallucinations in remote sensing VQA through attention-driven progressive localization and fine-grained reasoning, with code and data made publicly available.
...
TL;DR: ITO is a contrastive learning framework that addresses modality gap in image-text pretraining through multimodal multiple alignment and training-time fusion, achieving better performance while maintaining inference efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing image-text contrastive pretraining methods often yield representations that remain partially organized by modality, creating a modality gap that limits cross-modal understanding.

Method: Two synergistic mechanisms: 1) Multimodal multiple alignment for richer supervision by mining diverse image-text correspondences, and 2) A lightweight training-time multimodal fusion module that enforces structured cross-modal interaction but is discarded at inference.

Result: ITO consistently outperforms strong baselines across classification, retrieval, and multimodal benchmarks. Analysis shows multiple alignment drives discriminative power while training-time fusion acts as a critical structural regularizer.

Conclusion: ITO effectively eliminates the modality gap and stabilizes training dynamics, preventing early saturation in aggressive contrastive learning while maintaining the efficiency of standard dual-encoder architectures at inference.
...
TL;DR: HiLoRA: Hierarchical LoRA framework for federated learning with Vision Transformers that uses three-level adapters (root, cluster, leaf) to capture global, subgroup, and client-specific knowledge, improving personalization and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing LoRA-based federated tuning methods overlook latent client structures in real-world settings, limiting shared representation learning and hindering effective adaptation to unseen clients. Full fine-tuning in FL is communication heavy, so efficient adaptation methods are needed.

Method: HiLoRA places adapters at three hierarchical levels: root (global knowledge), cluster (subgroup knowledge), and leaf (client-specific knowledge). Uses cross-tier orthogonality and cascaded optimization to separate update subspaces. Includes LoRA-Subspace Adaptive Clustering mechanism to infer latent client groups via subspace similarity analysis.
...
Conclusion: HiLoRA effectively addresses the limitations of existing LoRA-based federated tuning by capturing hierarchical knowledge structures, enabling better knowledge sharing across clients and improved adaptation to unseen clients.
...
<details>
  <summary>Details</summary>
Motivation: Existing medical AI benchmarks are fragmented across specific tasks, organs, or modalities, lacking standardized frameworks to assess cross-task generalization of foundation models. There's a need for reproducible evaluation to validate the promise of medical foundation models for reliable cross-modality generalization.

Method: Two-step framework that decouples model inference from task-specific evaluation using standardized few-shot adaptation. Built on sequestered test sets from clinically relevant cohorts (2,400+ patients, 3,700+ vision cases, 2,400+ clinical reports from 17 institutions). Performance aggregated into a single UNICORN Score metric.

Result: Created a comprehensive benchmark spanning 8 anatomical regions and 4 imaging modalities with publicly available data, baseline methods, and evaluation platform. Enables standardized, reproducible assessment of medical foundation models across diverse domains.

Conclusion: UNICORN establishes a foundation for reproducible benchmarking of medical foundation models by standardizing multi-task, multi-modality assessment, supporting direct comparison across diverse medical domains through a unified evaluation protocol.
...
<details>
  <summary>Details</summary>
Motivation: Current text-only LLMs are limited to single modality, while multimodal models lack ability to access up-to-date web information. There's a need for agents that can actively search and use tools in real-world web environments.

Method: 1) Iterative Injection Data Synthesis pipeline to generate large-scale complex multimodal QA questions; 2) SFT-then-RL training pipeline to turn base multimodal models into agents; 3) Multimodal search benchmark MM-SearchExam for evaluation.

Result: VSearcher achieves superior performance compared to recent multimodal search agents and surpasses several proprietary models on multimodal web search tasks. The proposed MM-SearchExam benchmark proves highly challenging for recent models.

Conclusion: VSearcher successfully transforms static multimodal models into capable multimodal search agents that can perform long-horizon, multi-turn tool use in real-world web environments, addressing limitations of current text-only and static multimodal models.
...
TL;DR: R3GW extends 3D Gaussian Splatting to create relightable 3D representations of outdoor scenes captured in the wild, enabling photorealistic novel views under arbitrary illumination conditions.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) has limitations: it doesn't model scene illumination explicitly, making it unsuitable for relighting tasks, and struggles with scenes captured in the wild under changing lighting conditions. There's a need for methods that can handle unconstrained photo collections while supporting physically-based relighting.

Method: R3GW separates scenes into relightable foreground and non-reflective background (sky) using two distinct sets of Gaussians. It combines Physically Based Rendering with 3DGS representation to model view-dependent lighting effects in foreground reflections under varying illumination settings.

Result: State-of-the-art performance on NeRF-OSR dataset with enhanced support for physically-based relighting of unconstrained scenes. The method synthesizes photorealistic novel views under arbitrary illumination conditions and improves rendering quality at sky-foreground boundaries by mitigating depth reconstruction artifacts.

Conclusion: R3GW successfully extends 3DGS to create relightable representations for outdoor scenes captured in the wild, addressing the limitations of original 3DGS while maintaining high rendering quality and enabling flexible relighting capabilities.
...
TL;DR: NOVA is a novel framework for unpaired video editing that uses sparse keyframe control and dense synthesis to achieve high-quality editing without requiring paired training data.


<details>
  <summary>Details</summary>
Motivation: Current video editing models require large-scale paired datasets which are difficult to collect, especially for local video editing. Existing unpaired approaches struggle with background and temporal consistency when transferring image editing to video.

Method: NOVA uses a dual-branch approach: 1) Sparse branch provides semantic guidance through user-edited keyframes distributed across the video, 2) Dense branch continuously incorporates motion and texture from original video. Also includes degradation-simulation training strategy that enables learning motion reconstruction and temporal consistency from artificially degraded videos without paired data.

Result: Extensive experiments demonstrate that NOVA outperforms existing approaches in edit fidelity, motion preservation, and temporal coherence.

Conclusion: NOVA provides an effective solution for high-quality video editing without requiring paired training data, addressing a critical bottleneck in video editing research and applications.
...
TL;DR: VLMs struggle with complex layout semantics in historical Greek critical editions, but Qwen3VL-8B achieves SOTA performance with 1.0% CER after fine-tuning.
...
Method: Created two novel datasets: (1) large-scale synthetic corpus of 185,000 page images from TEI/XML sources with controlled typographic/layout variation, and (2) curated benchmark of real scanned editions spanning over a century. Evaluated three SOTA VLMs under zero-shot and fine-tuning regimes.

Result: VLMs show substantial limitations in zero-shot settings, underperforming compared to established software. However, Qwen3VL-8B achieves state-of-the-art performance with median Character Error Rate of 1.0% on real scans after fine-tuning.

Conclusion: Results highlight both current shortcomings and future potential of VLMs for structure-aware recognition of complex scholarly documents, with Qwen3VL-8B showing promising performance for historical text analysis.
...
<details>
  <summary>Details</summary>
Motivation: Digital ink lacks a unified representation - continuous vectors produce long sequences and training instability, while existing token representations require large vocabularies, face out-of-vocabulary issues, and underperform vectors on recognition tasks.

Method: ScribeTokens decomposes pen movement into unit pixel steps with two pen-state tokens, creating a fixed 10-token base vocabulary that can represent any digital ink. This enables aggressive BPE compression and is combined with next-ink-token prediction as a self-supervised pretraining strategy.

Result: On handwritten text generation, ScribeTokens dramatically outperforms vectors (17.33% vs. 70.29% CER). For recognition, it's the only token representation to outperform vectors without pretraining. With pretraining, it achieves best results across all representations (8.27% CER on IAM, 9.83% on DeepWriting) and accelerates convergence by up to 83x.

Conclusion: ScribeTokens provides an effective token-based representation for digital ink that solves key limitations of existing approaches, demonstrating superior performance in both generation and recognition tasks, especially when combined with self-supervised pretraining.
...
TL;DR: BrandFusion: A multi-agent framework for automatically embedding advertiser brands into text-to-video generated content while preserving user intent and brand recognizability.


<details>
  <summary>Details</summary>
Motivation: Text-to-video models have great commercial potential but lack effective brand integration capabilities. Current approaches struggle to balance user intent preservation with brand visibility and natural integration.

Method: Two-phase multi-agent framework: 1) Offline phase builds Brand Knowledge Base via model probing and lightweight fine-tuning for novel brands; 2) Online phase uses five agents for iterative prompt refinement, leveraging knowledge base and contextual tracking for brand visibility and semantic alignment.

Result: BrandFusion significantly outperforms baselines across 18 established and 2 custom brands on multiple SOTA T2V models in semantic preservation, brand recognizability, and integration naturalness. Human evaluations confirm higher user satisfaction.

Conclusion: BrandFusion establishes a practical pathway for sustainable T2V monetization by solving the brand integration challenge while maintaining user intent fidelity and brand visibility.
...
TL;DR: Probe-Select: A plug-in module for text-to-image models that enables early termination of unpromising seeds by predicting final image quality from intermediate denoiser activations, reducing sampling cost by over 60% while improving output quality.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image systems use inefficient "generate-then-select" pipelines that require full denoising of many candidate images before evaluation, consuming significant computational resources. Post-hoc evaluation metrics like CLIPScore and ImageReward add further inefficiency.

Method: Probe-Select exploits the observation that intermediate denoiser activations at early timesteps encode stable structural information (coarse structure, object layout, spatial arrangement) that correlates with final image quality. The method predicts final quality scores directly from these early activations, allowing early termination of unpromising seeds.

Result: The approach achieves accurate candidate ranking at only 20% of the denoising trajectory, reduces sampling cost by over 60%, and improves the quality of retained images. It works across both diffusion and flow-matching backbones without modifying the underlying generative models.

Conclusion: Early structural signals in text-to-image generation can effectively guide selective generation, enabling efficient quality evaluation during the generation process rather than after completion, making T2I systems more resource-efficient while maintaining or improving output quality.
...
TL;DR: GaussDerResNets are scale-invariant networks using Gaussian derivative residual blocks that generalize well to unseen image scales, addressing the out-of-distribution scale problem.


<details>
  <summary>Details</summary>
Motivation: Deep networks struggle with scale generalization, failing to handle images at scales not seen during training (out-of-distribution problem). This paper aims to create provably scale-invariant networks.

Method: Construct GaussDerResNets using scale-covariant Gaussian derivative residual blocks with skip connections. Use depthwise-separable convolutions to reduce parameters/computations while maintaining scale properties.

Result: GaussDerResNets show strong scale generalization and scale selection on rescaled STL-10, Fashion-MNIST, and CIFAR-10 datasets. They maintain accuracy while generalizing to scales not seen during training.

Conclusion: The proposed GaussDerResNets effectively address scale generalization challenges, with provable scale-covariant/invariant properties and practical performance across multiple datasets.
...
TL;DR: Multimodal-prior-guided importance sampling for hierarchical 3D Gaussian Splatting improves sparse-view novel view synthesis by fusing photometric, semantic, and geometric cues to guide Gaussian refinement.


<details>
  <summary>Details</summary>
Motivation: Sparse-view novel view synthesis suffers from overfitting to texture-induced errors and noise from pose/appearance inconsistencies. Existing methods relying solely on photometric residuals are insufficient for robust detail recovery.

Method: 1) Multimodal-prior-guided importance sampler fuses photometric rendering residuals, semantic priors, and geometric priors to produce local recoverability estimates. 2) Coarse-to-fine Gaussian representation with stable coarse layer and selective fine primitive addition. 3) Geometric-aware sampling and retention policy that concentrates refinement on critical regions while protecting primitives in underconstrained areas.

Result: State-of-the-art reconstructions on diverse sparse-view benchmarks, with up to +0.3 dB PSNR improvement on DTU dataset.

Conclusion: By prioritizing regions with consistent multimodal evidence rather than raw residuals alone, the method effectively alleviates overfitting and suppresses noise, enabling more robust sparse-view 3D reconstruction.
...
TL;DR: TaYS enables streaming video reasoning for LVLMs with parallel CoT generation, reducing latency while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Current LVLM video reasoning uses batch processing (full video available), which doesn't match real-world streaming video where frames arrive sequentially, creating inefficiency.

Method: Proposes Think-as-You-See (TaYS) framework with parallelized CoT generation, stream-constrained training, stream-parallel inference, temporally aligned reasoning units, streaming attention masks/positional encodings, and dual KV-cache to decouple visual encoding from textual reasoning.

Result: TaYS outperforms both batch and interleaved baselines on video CoT tasks (event dynamics, causal reasoning, thematic understanding), improves reasoning performance while substantially reducing TTFT and overall reasoning delay.

Conclusion: Data-aligned streaming reasoning enables efficient and responsive video understanding for LVLMs, addressing the mismatch between batch processing and real-world streaming video inputs.
...
TL;DR: SIGMark is a scalable, blind-extraction in-generation watermarking framework for video diffusion models that addresses computational costs and robustness issues in existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing in-generation watermarking for video diffusion models has two major limitations: 1) they are non-blind, requiring storage of all message-key pairs and template matching which is computationally prohibitive at scale, and 2) they have weak robustness against temporal disturbance when applied to modern video diffusion models with causal 3D VAEs.

Method: SIGMark introduces two key components: 1) GF-PRC (Global set of Frame-wise PseudoRandom Coding keys) to generate watermarked initial noise for blind extraction, reducing storage costs while preserving noise distribution, and 2) SGO (Segment Group-Ordering module) tailored to causal 3D VAEs to ensure robust watermark inversion under temporal disturbance.
...
Conclusion: SIGMark provides a practical solution for scalable, blind-extraction watermarking in video diffusion models, addressing key limitations of existing approaches and contributing to AI safety by enabling effective protection and tracing of AI-generated video content.
...
TL;DR: SemanticDialect is a quantization method for video diffusion transformers that uses per-block optimal format selection, activation decomposition with residual error compensation, and semantic-aware format sharing to maintain video quality while reducing memory/compute costs.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiT) for video generation have high memory and compute costs that hinder edge deployment. Existing quantization methods degrade video quality due to high activation variation and the need to preserve semantic/temporal coherence.

Method: Three key techniques: 1) Block-wise mixed-format quantization with scaled formatbook using lookup tables for efficient per-block format selection, 2) Activation decomposition that reduces quantization error by re-quantizing and adding back residual errors with attention-guided salient token selection, 3) Semantic-aware dialect assignment (SeDA) that shares sub-formatbooks among semantically correlated tokens for better consistency.

Result: SemanticDialect outperforms prior VDiT quantization methods and fine-grained block-wise format baselines, approaching FP16 quality on Open-Sora 2.0 benchmarks.

Conclusion: The proposed SemanticDialect method effectively reduces memory and compute costs for video DiT models while maintaining high video quality through sophisticated quantization techniques that address activation variation and semantic coherence challenges.
...
TL;DR: StegaFFD: A steganography-based framework that hides facial images within natural cover images for privacy-preserving face forgery detection without raising suspicion.


<details>
  <summary>Details</summary>
Motivation: Existing privacy protection methods (anonymization, encryption, distortion) for face forgery detection cause semantic distortion that alerts attackers and introduces artifacts that confuse detection models. There's a need for privacy protection that doesn't raise suspicion while preserving detection accuracy.

Method: StegaFFD uses image steganography to hide facial images within natural cover images and performs detection directly in the steganographic domain. It introduces three key components: 1) Low-Frequency-Aware Decomposition (LFAD) to suppress cover interference, 2) Spatial-Frequency Differential Attention (SFDA) to enhance hidden facial feature perception, and 3) Steganographic Domain Alignment (SDA) to align hidden face representations with raw face representations.

Result: Extensive experiments on seven face forgery detection datasets show StegaFFD achieves strong imperceptibility, avoids raising attackers' suspicion, and better preserves detection accuracy compared to existing facial privacy protection methods.

Conclusion: StegaFFD provides an effective privacy-preserving solution for face forgery detection that maintains both imperceptibility and detection performance by leveraging steganography and specialized feature extraction techniques.
...
TL;DR: LLandMark is a modular multi-agent framework for landmark-aware multimodal video retrieval that handles complex real-world queries through specialized agents collaborating across query parsing, landmark reasoning, retrieval, and answer synthesis stages.


<details>
  <summary>Details</summary>
Motivation: The increasing diversity and scale of video data demands retrieval systems capable of multimodal understanding, adaptive reasoning, and domain-specific knowledge integration, particularly for handling real-world complex queries involving cultural or spatial landmarks.

Method: A modular multi-agent framework with specialized agents across four stages: query parsing/planning, landmark reasoning, multimodal retrieval, and reranked answer synthesis. Key components include a Landmark Knowledge Agent that detects landmarks and reformulates them into visual prompts, an LLM-assisted image-to-image pipeline for autonomous landmark detection and retrieval, and an OCR refinement module for Vietnamese text recognition.

Result: Experimental results show that LLandMark achieves adaptive, culturally grounded, and explainable retrieval performance, demonstrating effectiveness in handling Vietnamese scenes and complex landmark-aware queries.

Conclusion: LLandMark presents an effective framework for landmark-aware multimodal video retrieval that addresses the challenges of real-world complex queries through collaborative multi-agent architecture, specialized landmark handling, and cultural adaptation, particularly for Vietnamese contexts.
...
TL;DR: MVD-HuGaS: A method for free-view 3D human rendering from a single image using multi-view diffusion and 3D Gaussian optimization.


<details>
  <summary>Details</summary>
Motivation: Existing methods using diffusion models for 3D human reconstruction produce artifacts (flattened structure, over-smoothing) and struggle with real-world generalization. Need for better single-image 3D human reconstruction.

Method: 1) Enhanced multi-view diffusion model fine-tuned on 3D human datasets to generate multi-view images from single reference. 2) Alignment module for joint optimization of 3D Gaussians and camera poses. 3) Depth-based Facial Distortion Mitigation module to refine facial regions. 4) Optimization of 3D Gaussians using refined multi-view images and accurate poses.

Result: Achieves state-of-the-art performance on Thuman2.0 and 2K2K datasets for single-view 3D human rendering.

Conclusion: MVD-HuGaS enables high-fidelity free-view 3D human rendering from a single image by leveraging multi-view diffusion with geometry priors, accurate pose estimation, and facial refinement.
...
TL;DR: The paper introduces 3D-DRES, a new task for phrase-to-3D instance mapping, along with DetailRefer dataset and DetailBase baseline model to enable fine-grained 3D vision-language understanding beyond sentence-level segmentation.


<details>
  <summary>Details</summary>
Motivation: Current 3D visual grounding tasks only handle sentence-level detection/segmentation, failing to leverage rich compositional contextual reasoning within natural language expressions. There's a need for more fine-grained understanding that maps phrases to specific 3D elements.

Method: 1) Introduce 3D-DRES task for phrase-to-3D instance mapping. 2) Create DetailRefer dataset with 54,432 descriptions across 11,054 objects using phrase-instance annotation paradigm. 3) Develop DetailBase baseline architecture supporting dual-mode segmentation at both sentence and phrase levels.

Result: Models trained on DetailRefer excel at phrase-level segmentation and show surprising improvements on traditional 3D-RES benchmarks. The dataset enables fine-grained understanding beyond previous sentence-level approaches.

Conclusion: The proposed 3D-DRES task, DetailRefer dataset, and DetailBase model advance 3D vision-language understanding by enabling phrase-level segmentation, addressing limitations of existing sentence-level approaches and providing a foundation for more compositional reasoning.
...
TL;DR: ProGIC is a compact generative image compression method using residual vector quantization for progressive transmission, achieving better perceptual quality and 10x faster encoding/decoding than previous methods.


<details>
  <summary>Details</summary>
Motivation: Existing generative image compression methods use large, rigid models that are impractical for flexible transmission and deployment in low-bitrate scenarios, especially on resource-constrained devices.

Method: Uses residual vector quantization (RVQ) with multiple vector quantizers encoding residuals stage-by-stage, paired with a lightweight backbone using depthwise-separable convolutions and small attention blocks for efficient deployment.

Result: Achieves up to 57.57% bitrate savings on DISTS and 58.83% on LPIPS compared to MS-ILLM on Kodak dataset, with 10x faster encoding/decoding on GPUs while enabling progressive transmission.

Conclusion: ProGIC provides a practical, efficient solution for generative image compression that balances perceptual quality, progressive transmission capability, and deployment efficiency across different hardware platforms.
...
TL;DR: HBSN is a deep learning network that computes Harmonic Beltrami Signatures from binary images, providing shape representation with invariance to translation, scaling, and rotation, and can be integrated into segmentation models as a shape prior module.


<details>
  <summary>Details</summary>
Motivation: The paper aims to develop an efficient method for extracting and utilizing shape prior information in computer vision tasks by leveraging neural networks to compute the Harmonic Beltrami Signature, which provides a one-to-one correspondence with 2D simply connected shapes while being invariant to basic transformations.

Method: The proposed Harmonic Beltrami Signature Network (HBSN) consists of three main components: 1) a pre-Spatial Transformer Network (pre-STN) for shape normalization, 2) a UNet-based backbone network for HBS prediction, and 3) a post-STN for angle regularization. This architecture enables efficient computation of HBS representations from binary-like images.

Result: Experiments demonstrate that HBSN accurately computes HBS representations even for complex shapes. When incorporated into existing deep learning segmentation models, HBSN improves their performance by providing shape priors, confirming its utility as a general-purpose module for embedding geometric shape information.

Conclusion: HBSN provides an effective deep learning approach for computing Harmonic Beltrami Signatures and can be successfully integrated into computer vision pipelines to enhance segmentation performance through shape prior information, serving as a versatile module for geometric shape representation.
...
TL;DR: AiM is a framework that reconstructs articulated 3D objects from interaction videos and start-state scans, performing part segmentation, articulation analysis, and high-quality rendering without prior knowledge of part count.


<details>
  <summary>Details</summary>
Motivation: Existing methods for articulated object reconstruction require prior knowledge of part count and clear visibility in two articulation states, limiting their applicability and robustness in real-world scenarios.

Method: Uses dual-Gaussian scene representation learned from initial 3DGS scan and interaction video, employs motion cues for part segmentation, and applies sequential RANSAC for part mobility analysis without structural priors.

Result: Achieves higher quality part segmentation than previous methods without prior knowledge, validated on both simple and complex objects with strong generalization ability.

Conclusion: AiM provides a robust framework for articulated object reconstruction that overcomes limitations of existing methods by eliminating need for prior part knowledge and handling challenging visibility conditions.
...
TL;DR: The paper introduces methods to interpret how Video Diffusion Transformers convert motion words into video by localizing motion features spatially and temporally without gradient calculations.


<details>
  <summary>Details</summary>
Motivation: Current understanding of how Video DiTs convert motion words into video is insufficient, and prior interpretability work focuses mainly on objects rather than motion behavior.

Method: Two main methods: 1) GramCol for per-frame saliency maps for any text concept, 2) Motion-feature selection algorithm to produce Interpretable Motion-Attentive Map (IMAP) for spatial and temporal motion localization.

Result: The method shows outstanding localization capability on motion localization tasks and zero-shot video semantic segmentation, producing interpretable and clearer saliency maps for both motion and non-motion concepts.

Conclusion: The proposed approach successfully interprets motion features in Video DiTs, providing spatial and temporal localization without requiring gradient calculations or parameter updates.
...
TL;DR: HDINO is an efficient open-vocabulary object detector that eliminates dependence on manually curated datasets and resource-intensive cross-modal feature extraction, achieving state-of-the-art performance with simpler training.


<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary object detection methods rely heavily on manually curated fine-grained training datasets and resource-intensive layer-wise cross-modal feature extraction, which limits their practicality and efficiency.

Method: Two-stage training strategy built on DINO: 1) One-to-Many Semantic Alignment Mechanism (O2M) treats noisy samples as positive instances for visual-textual alignment, with Difficulty Weighted Classification Loss (DWCL) to mine hard examples; 2) Lightweight feature fusion module enhances linguistic semantic sensitivity.

Result: HDINO-T achieves 49.2 mAP on COCO using only 2.2M training images (vs competitors' 5.4M-6.5M images), surpassing Grounding DINO-T by 0.8 mAP and T-Rex2 by 2.8 mAP. After fine-tuning, HDINO-T reaches 56.4 mAP and HDINO-L achieves 59.2 mAP.

Conclusion: HDINO demonstrates an efficient and scalable approach to open-vocabulary object detection that eliminates manual data curation and resource-intensive feature extraction while achieving superior performance with fewer training resources.
...
TL;DR: GloPath is an entity-centric foundation model for renal pathology that uses multi-scale self-supervised learning on over 1M glomeruli to improve glomerular lesion assessment and discover clinicopathological insights.


<details>
  <summary>Details</summary>
Motivation: Current AI approaches struggle with the heterogeneity of glomerular morphology and fine-grained lesion patterns in renal pathology, which are central to diagnosis and prognosis of kidney diseases.

Method: Multi-scale and multi-view self-supervised learning on over 1 million glomeruli extracted from 14,049 renal biopsy specimens to create an entity-centric foundation model.

Result: Outperformed state-of-the-art methods in 42 out of 52 tasks (80.8%), achieved 91.51% ROC-AUC for lesion recognition in real-world study, and revealed 224 statistically significant morphology-clinical associations.
...
TL;DR: TRACE is a new multimodal retrieval framework that combines generative reasoning with discriminative embeddings, using Chain-of-Thought reasoning for complex queries and compressing it into compact embeddings, achieving state-of-the-art performance with adaptive routing behavior.


<details>
  <summary>Details</summary>
Motivation: Current multimodal retrieval models use encoder-only approaches that underutilize MLLMs' generative reasoning capabilities, struggling with complex compositional intents that require logical deduction rather than just pattern matching.

Method: TRACE unifies generative reasoning with discriminative representation learning: 1) Generates structured Chain-of-Thought to explicitly reason about queries, 2) Compresses reasoning trace into compact embeddings via dedicated token, 3) Trained on M-BEIR-CoT dataset with difficulty-aware routing strategy.

Result: Achieves state-of-the-art on M-BEIR benchmark, demonstrates learned implicit routing behavior (activates reasoning for complex queries, bypasses for simple ones), optimal balance between accuracy and throughput, and remarkable zero-shot transferability to unseen domains.

Conclusion: TRACE successfully bridges generative reasoning with discriminative retrieval, enabling adaptive handling of diverse query complexities while maintaining efficiency, representing a significant advancement in multimodal retrieval systems.
...
TL;DR: TC-Padé: A feature prediction framework using Padé approximation for accelerating diffusion models while maintaining quality in low-step regimes (20-30 steps).


<details>
  <summary>Details</summary>
Motivation: Current diffusion models suffer from computational burden due to iterative sampling. Existing feature caching techniques work well at higher step counts (50+ steps) but fail in practical low-step regimes (20-30 steps) due to error accumulation and trajectory drift from polynomial-based extrapolators, and they overlook distinct dynamical properties of different denoising phases.

Method: Trajectory-Consistent Padé approximation framework that models feature evolution through rational functions (Padé approximation) instead of polynomials. Includes: (1) adaptive coefficient modulation using historical cached residuals to detect trajectory transitions, and (2) step-aware prediction strategies tailored to early, mid, and late sampling stages.
...
Conclusion: TC-Padé effectively addresses limitations of existing feature caching methods in low-step regimes by using rational function modeling and trajectory-consistent sampling strategies, enabling significant acceleration while preserving generation quality.
...
TL;DR: Proposes a semi-supervised method using text-informed pseudo-labels to improve few-shot adaptation of vision-language models in medical imaging, reducing labeling effort by >50%.


<details>
  <summary>Details</summary>
Motivation: Medical imaging faces challenges with high annotation costs and class imbalances in few-shot learning scenarios, where underrepresented categories penalize model performance despite promising VLM capabilities.

Method: Introduces an efficient semi-supervised solver that leverages unlabeled data by propagating text-informed pseudo-labels during few-shot adaptation of vision-language models.

Result: Enables lower-budget annotation pipelines for adapting VLMs, reducing labeling effort by more than 50% in low-shot regimes.

Conclusion: The proposed semi-supervised approach effectively addresses class imbalance challenges in medical few-shot learning by utilizing unlabeled data with text-informed pseudo-label propagation.
...
TL;DR: Proposes FMAS (foundation model-based anomaly synthesis) for generating realistic anomalous samples without fine-tuning, and WDAM (Wavelet Domain Attention Module) for enhanced anomaly detection using frequency-domain characteristics.


<details>
  <summary>Details</summary>
Motivation: Industrial anomaly detection is challenging due to scarcity of anomalous samples and complexity of real-world anomalies. Need for methods that can generate realistic anomalies and effectively detect them without requiring extensive anomalous training data.

Method: Two main components: 1) FMAS - foundation model-based anomaly synthesis pipeline that generates realistic anomalous samples without fine-tuning or class-specific training. 2) WDAM - Wavelet Domain Attention Module that uses adaptive sub-band processing in frequency domain to enhance anomaly feature extraction, exploiting distinct frequency-domain characteristics of anomalies.

Result: Combination of FMAS and WDAM significantly improves anomaly detection sensitivity while maintaining computational efficiency. Comprehensive experiments on MVTec AD and VisA datasets show WDAM as a plug-and-play module achieves substantial performance gains against existing baselines.

Conclusion: The proposed approach effectively addresses industrial anomaly detection challenges by generating realistic anomalous samples and enhancing detection sensitivity through frequency-domain analysis, offering a practical solution with strong empirical results.
...
TL;DR: A simple, efficient UAD framework using 2D autoregressive CNN to model spatial dependencies between DINO patch embeddings, reducing memory/computational overhead compared to memory bank/prototype methods.


<details>
  <summary>Details</summary>
Motivation: Existing DINO-based UAD methods ignore spatial relationships between patches and use memory-intensive approaches (memory banks/prototypes) that require costly comparisons at inference time.

Method: Proposes a 2D autoregressive CNN to explicitly model spatial and contextual dependencies between patch embeddings, learning a compact parametric model of the normative distribution instead of storing embeddings.

Result: Achieves competitive anomaly detection performance on BMAD benchmark (three medical imaging datasets) while substantially reducing inference time and memory requirements compared to existing methods.
...
TL;DR: D4D Dataset provides paired endoscopic video and structured-light geometry for evaluating 3D reconstruction of deforming abdominal tissue in surgical conditions.


<details>
  <summary>Details</summary>
Motivation: Need for comprehensive benchmark data to evaluate 3D reconstruction methods for deforming soft tissue in realistic surgical environments, addressing challenges of non-rigid motion, deformation magnitude, and camera movement.

Method: Data acquired from six porcine cadaver sessions using da Vinci Xi stereo endoscope and Zivid structured-light camera, registered via optical tracking and manual iterative alignment. Three sequence types probe different algorithm challenges. Postprocessing includes ICP registration, semi-automatic techniques, and instrument masking.

Result: Dataset comprises over 300,000 frames and 369 point clouds across 98 curated recordings, providing rectified stereo images, instrument masks, stereo depth, structured-light point clouds, camera poses, and intrinsics for quantitative geometric evaluation.

Conclusion: The D4D Dataset serves as a comprehensive benchmark resource for developing and evaluating non-rigid SLAM, 4D reconstruction, and depth estimation methods in surgical contexts.
...
<details>
  <summary>Details</summary>
Motivation: While 3D Gaussian Splatting (3DGS) excels at novel view synthesis and 3D reconstruction, there's no efficient and photorealistic method for editing scene appearance, particularly for recoloring while maintaining view-dependent effects.

Method: Introduces a novel architecture separating color into diffuse and view-dependent components, with multi-view training using image patches from multiple viewpoints. For recoloring, uses a rapid scheme requiring only one manually edited image, fine-tuning a single MLP's weights alongside single-shot segmentation of the editable area.

Result: Significant quantitative and qualitative advancements over Neural Radiance Field-based competitors, with color edits propagating to entire scenes in just two seconds, enabling real-time interaction and control over view-dependent effect strength.

Conclusion: VIRGi provides an efficient, photorealistic solution for editing 3DGS scenes, addressing the critical gap in appearance editing while preserving view-dependent effects and enabling real-time user interaction.
...
TL;DR: URGT adapts VGGT into a multi-patch transformer for joint high-resolution depth-normal estimation, using cross-patch attention and GridMix sampling to achieve SOTA results with improved consistency and generalization.


<details>
  <summary>Details</summary>
Motivation: Joint estimation of surface normals and depth is essential for holistic 3D scene understanding, but high-resolution prediction faces a trade-off between preserving fine local detail and maintaining global consistency.

Method: URGT adapts VGGT into a unified multi-patch transformer. High-resolution images are partitioned into patches augmented with coarse depth/normal priors, processed jointly with cross-patch attention for global coherence. GridMix patch sampling probabilistically samples grid configurations during training to enhance spatial robustness.

Result: Achieves SOTA on UnrealStereo4K: reduces AbsRel from 0.0582 to 0.0291, RMSE from 2.17 to 1.31, and mean angular error from 23.36° to 18.51°. Produces sharper, more stable geometry with strong zero-shot/cross-domain generalization and effective scaling to very high resolutions.

Conclusion: URGT provides an efficient, extensible solution for high-quality geometry refinement through multi-patch transformer architecture with cross-patch attention and GridMix sampling, enabling joint depth-normal estimation with improved global consistency and fine detail preservation.
...
<details>
  <summary>Details</summary>
Motivation: Generalist pathology foundation models lack specialized expertise for specific organ systems like breast pathology, and there's a need for large-scale validation cohorts and tailored training paradigms to translate broad histomorphological knowledge into organ-specific expertise.

Method: BRIGHT employs a collaborative generalist-specialist framework to capture both universal and organ-specific features, trained on ~210M histopathology tiles from over 51,000 breast WSIs from 40,000+ patients across 19 hospitals.
...
TL;DR: EduAIGV-1k: First benchmark dataset for evaluating AI-generated educational math videos for young learners, with fine-grained annotations and EduVQA assessment framework.


<details>
  <summary>Details</summary>
Motivation: AI-generated video models show great potential but lack evaluation for educational content, especially for teaching foundational math concepts to young learners.

Method: Created EduAIGV-1k dataset with 1,130 videos from 10 T2V models using 113 pedagogy-oriented prompts. Developed EduVQA with Structured 2D Mixture-of-Experts (S2D-MoE) module for multi-dimensional quality assessment.

Result: EduVQA consistently outperforms existing VQA baselines in assessing both perceptual quality (spatial/temporal fidelity) and prompt alignment (word/sentence-level).

Conclusion: The work establishes first benchmark for educational AIGVs, provides interpretable multi-dimensional evaluation, and enables better assessment of AI-generated educational content.
...
<details>
  <summary>Details</summary>
Motivation: Conventional ground-based processing of Sentinel-1 SAR data for sea ice mapping faces limitations in downlink bandwidth, latency, and energy costs. On-board processing using dedicated inference chips offers a transformative solution for timely maritime navigation information in polar regions.

Method: TinyIceNet combines SAR-aware architectural simplifications with low-precision quantization for efficient on-board processing. The model is trained on the AI4Arctic dataset, synthesized using High-Level Synthesis, and deployed on a Xilinx Zynq UltraScale+ FPGA platform.

Result: TinyIceNet achieves 75.216% F1 score on Stage of Development (SOD) segmentation while demonstrating near-real-time inference with 2x reduced energy consumption compared to full-precision GPU baselines.

Conclusion: The work demonstrates the potential of chip-level hardware-algorithm co-design for future spaceborne and edge AI systems, enabling efficient on-board sea ice mapping for safer maritime navigation in polar regions.
...
TL;DR: MoECLIP introduces a Mixture-of-Experts architecture with patch-level adaptation for Zero-Shot Anomaly Detection, using specialized LoRA experts for different patch characteristics and orthogonal feature separation to prevent redundancy.
...
Method: Proposes MoECLIP with: 1) Mixture-of-Experts architecture that dynamically routes each image patch to specialized LoRA experts based on patch characteristics, 2) Frozen Orthogonal Feature Separation (FOFS) to orthogonally separate input features and force experts to focus on distinct information, and 3) simplex equiangular tight frame (ETF) loss to regulate expert outputs for maximally equiangular representations.

Result: Comprehensive experiments across 14 benchmark datasets spanning industrial and medical domains demonstrate that MoECLIP outperforms existing state-of-the-art Zero-Shot Anomaly Detection methods.

Conclusion: MoECLIP successfully addresses the patch-agnostic limitation of existing approaches through patch-level adaptation with specialized experts, achieving superior performance in Zero-Shot Anomaly Detection while preserving CLIP's generalization capabilities.
...
TL;DR: AWDiff is a diffusion-based augmentation framework using wavelet transforms to preserve fine diagnostic features in lung ultrasound images, enhanced with semantic conditioning from BioMedCLIP.


<details>
  <summary>Details</summary>
Motivation: Lung ultrasound has limited data for machine learning development, and existing generative methods lose subtle diagnostic features like B-lines and pleural irregularities due to resolution reduction.

Method: Proposes A trous Wavelet Diffusion (AWDiff) that integrates wavelet transform to preserve fine-scale structures without destructive downsampling, with semantic conditioning using BioMedCLIP for clinical label alignment.

Result: On LUS dataset, AWDiff achieved lower distortion and higher perceptual quality compared to existing methods, demonstrating both structural fidelity and clinical diversity.

Conclusion: AWDiff effectively preserves diagnostic features in lung ultrasound augmentation, addressing data scarcity while maintaining clinical relevance through wavelet-based diffusion and semantic conditioning.
...
TL;DR: RL3DEdit uses reinforcement learning with VGGT 3D foundation model rewards to achieve multi-view consistent 3D editing from 2D diffusion priors, avoiding the need for scarce supervised 3D editing data.


<details>
  <summary>Details</summary>
Motivation: Maintaining multi-view consistency in 3D editing is challenging, and supervised fine-tuning is infeasible due to extreme scarcity of 3D-consistent editing paired data. However, while generating consistent 3D content is hard, verifying 3D consistency is tractable, making reinforcement learning a feasible solution.

Method: Proposes RL3DEdit, a single-pass RL optimization framework that uses novel rewards from the 3D foundation model VGGT. Leverages VGGT's priors from real-world data, feeds edited images, and uses output confidence maps and pose estimation errors as reward signals to anchor 2D editing priors onto a 3D-consistent manifold.
...
Conclusion: RL3DEdit provides an effective RL-based solution for 3D editing that overcomes data scarcity issues by using 3D consistency verification as reward signals, enabling high-quality multi-view consistent editing from 2D diffusion priors.
...
TL;DR: Kling-MotionControl is a unified DiT-based framework for high-fidelity character animation that intelligently handles body, face, and hand motions with robust cross-identity generalization and precise appearance preservation.


<details>
  <summary>Details</summary>
Motivation: Recent generative models have advanced character animation, but there's a need for a unified framework that can handle holistic character animation with robust cross-identity generalization, precise motion control, and faithful appearance preservation while maintaining practical inference speeds.

Method: A unified DiT-based framework using divide-and-conquer strategy with heterogeneous motion representations for body, face, and hands. Incorporates adaptive identity-agnostic learning for cross-identity generalization, meticulous identity injection/fusion designs with subject library mechanism, and multi-stage distillation for 10x+ inference acceleration.

Result: Superior performance compared to leading commercial and open-source solutions in human preference evaluations, achieving exceptional fidelity in holistic motion control, open domain generalization, and visual quality/coherence.

Conclusion: Kling-MotionControl establishes itself as a robust solution for high-quality, controllable, and lifelike character animation with intelligent semantic motion understanding and precise text responsiveness beyond visual inputs.
...
TL;DR: CAT framework uses geometry-based conditioning and nonlinear transport maps to reduce toxic content in T2I models while preserving image quality for benign prompts.


<details>
  <summary>Details</summary>
Motivation: Current Text-to-Image models are prone to generating unsafe/toxic content, and existing activation steering methods degrade image quality when applied to benign prompts.
...
Result: CAT significantly reduces Attack Success Rate while maintaining image fidelity compared to unsteered generations, validated on Z-Image and Infinity architectures.

Conclusion: CAT effectively addresses the trade-off between safety intervention and image quality in T2I models through targeted activation conditioning.
...
TL;DR: ProSMA-UNet improves medical image segmentation by replacing traditional skip connections with sparse feature selection gates that explicitly remove irrelevant activations like noise and background clutter.


<details>
  <summary>Details</summary>
Motivation: Traditional U-Net skip connections propagate both useful spatial details and harmful low-level noise/background clutter, especially problematic in low-contrast clinical imaging. Existing attention gates only softly reweight features rather than removing irrelevant activations.

Method: ProSMA-UNet reformulates skip gating as decoder-conditioned sparse feature selection: 1) constructs multi-scale compatibility field using lightweight depthwise dilated convolutions, 2) enforces explicit sparsity via ℓ₁ proximal operator with learnable per-channel thresholds (soft-thresholding gate), 3) adds decoder-conditioned channel gating using global decoder context.

Result: Achieves state-of-the-art performance on challenging 2D and 3D benchmarks, with particularly large gains (~20%) on difficult 3D segmentation tasks.

Conclusion: ProSMA-UNet effectively addresses the limitation of traditional skip connections by explicitly removing irrelevant activations through sparse feature selection, leading to significant performance improvements especially in challenging 3D medical segmentation.
...
TL;DR: MoD-DPO improves omni-modal LLMs by reducing cross-modal hallucinations through modality-aware regularization and language-prior debiasing.


<details>
  <summary>Details</summary>
Motivation: Omni-modal LLMs suffer from cross-modal hallucinations due to spurious correlations and over-reliance on language priors, limiting their reliability in multimodal understanding tasks.

Method: Proposes Modality-Decoupled Direct Preference Optimization (MoD-DPO) with modality-aware regularization terms that enforce invariance to irrelevant modality corruptions and sensitivity to relevant modality perturbations, plus a language-prior debiasing penalty.

Result: MoD-DPO consistently improves perception accuracy and hallucination resistance across multiple audiovisual hallucination benchmarks, outperforming previous preference optimization methods with similar training budgets.

Conclusion: The approach demonstrates the importance of modality-faithful alignment and provides a scalable path toward more reliable multimodal foundation models by reducing unintended cross-modal interactions.
...
TL;DR: SpeciaRL is a specificity-aware reinforcement learning framework that fine-tunes reasoning Large Multimodal Models for open-world fine-grained image classification, balancing correctness and specificity in predictions.


<details>
  <summary>Details</summary>
Motivation: Current reasoning LMMs produce overly generic predictions for fine-grained visual classification in open-world settings, despite possessing intrinsic fine-grained knowledge. The challenge is to promote specific predictions without compromising correctness.

Method: Proposes SpeciaRL, a specificity-aware reinforcement learning framework with dynamic, verifier-based reward signals anchored to the best predictions within online rollouts. This approach promotes specificity while respecting model capabilities to prevent incorrect predictions.

Result: Out-of-domain experiments show SpeciaRL achieves the best trade-off between correctness and specificity across extensive fine-grained benchmarks, surpassing existing methods.

Conclusion: SpeciaRL advances open-world fine-grained image classification by effectively steering reasoning LMMs toward predictions that are both correct and specific.
...
TL;DR: COP-GEN is a multimodal latent diffusion transformer that models joint distributions of Earth observation data (optical, radar, elevation) to enable flexible any-to-any conditional generation with uncertainty representation.
...
Method: COP-GEN uses a multimodal latent diffusion transformer that models the joint distribution of heterogeneous Earth Observation modalities at native spatial resolutions. It parameterizes cross-modal mappings as conditional distributions, enabling any-to-any conditional generation without task-specific retraining.

Result: Experiments on large-scale global multimodal datasets show COP-GEN generates diverse yet physically consistent realizations while maintaining strong peak fidelity across optical, radar, and elevation modalities. The model captures meaningful cross-modal structure and adapts output uncertainty as conditioning information increases.

Conclusion: The results highlight the importance of stochastic generative modeling for Earth observation and motivate evaluation protocols that move beyond single-reference, pointwise metrics to better capture uncertainty and variability in multimodal data integration.
...
TL;DR: Unified multimodal models often underperform base VLMs in generation-to-understanding tasks, with limited benefits only in specific spatial/visual reasoning scenarios.


<details>
  <summary>Details</summary>
Motivation: To systematically investigate whether and when generation capabilities improve understanding in multimodal models, as existing benchmarks lack comprehensive exploration of generation-to-understanding relationships.

Method: Introduce UniG2U-Bench, a comprehensive benchmark categorizing generation-to-understanding evaluation into 7 regimes and 30 subtasks requiring varying degrees of visual transformations. Evaluate over 30 models on this benchmark.

Result: 1) Unified models generally underperform base VLMs, and Generate-then-Answer inference degrades performance. 2) Enhancements only emerge in specific subtasks like spatial intelligence, visual illusions, and multi-round reasoning. 3) Tasks with similar reasoning structures and models with shared architectures show correlated behaviors.

Conclusion: Current unified multimodal models don't fully leverage generation for understanding; more diverse training data and novel paradigms are needed to unlock their potential.
...
TL;DR: DuoMo is a two-stage diffusion method that recovers globally consistent human motion in world coordinates from noisy/incomplete videos by first estimating camera-space motion then lifting to world-space.


<details>
  <summary>Details</summary>
Motivation: Reconstructing human motion from unconstrained videos requires balancing generalization from diverse/noisy inputs with maintaining global motion consistency, which existing methods struggle with.

Method: Factorizes motion learning into two diffusion models: (1) camera-space model estimates motion from videos in camera coordinates, (2) world-space model lifts and refines this into globally consistent world coordinates, generating mesh vertices directly without parametric models.

Result: State-of-the-art performance: 16% reduction in world-space reconstruction error on EMDB while maintaining low foot skating, and 30% reduction on RICH dataset.

Conclusion: DuoMo effectively addresses the generalization-consistency trade-off through factorization, enabling robust motion reconstruction from challenging video inputs across diverse scenes.
...
TL;DR: LoGeR is a novel architecture for long-context 3D video reconstruction that processes video in chunks with a hybrid memory system, enabling training on 128 frames but generalizing to thousands of frames at inference time.


<details>
  <summary>Details</summary>
Motivation: Current feedforward geometric models struggle with minute-long videos due to quadratic attention complexity or limited memory in recurrent designs, creating a bottleneck for scaling dense 3D reconstruction to extremely long sequences.

Method: LoGeR processes video streams in chunks with bidirectional priors for intra-chunk reasoning. It uses a hybrid memory module with two components: 1) parametric Test-Time Training memory to anchor global coordinate frame and prevent scale drift, and 2) non-parametric Sliding Window Attention to preserve uncompressed context for precise adjacent alignment.
...
Conclusion: LoGeR successfully scales dense 3D reconstruction to extremely long sequences without post-optimization through its chunk-based processing and hybrid memory architecture, substantially outperforming previous methods and enabling practical long-context geometric reconstruction.
...
TL;DR: The paper investigates multimodal foundation models beyond language, using controlled pretraining experiments to identify key factors for effective multimodal learning, discovering that vision is more data-hungry than language and that Mixture-of-Experts architectures can harmonize this scaling asymmetry.


<details>
  <summary>Details</summary>
Motivation: To advance foundation models beyond language by exploring the visual world, but the design space for native multimodal models remains unclear. The authors aim to provide empirical clarity through controlled experiments that isolate multimodal pretraining factors without language pretraining interference.

Method: Adopt the Transfusion framework with next-token prediction for language and diffusion for vision. Train from scratch on diverse data including text, video, image-text pairs, and action-conditioned video. Conduct IsoFLOP analysis to compute scaling laws for both modalities and examine Mixture-of-Experts (MoE) architectures.

Result: Four key insights: (1) Representation Autoencoder (RAE) provides optimal unified visual representation; (2) visual and language data are complementary and synergistic; (3) unified multimodal pretraining naturally leads to world modeling; (4) MoE enables efficient multimodal scaling with modality specialization. Discovered scaling asymmetry: vision is significantly more data-hungry than language.

Conclusion: MoE architectures harmonize the scaling asymmetry between vision and language by providing high model capacity for language while accommodating the data-intensive nature of vision, paving the way for truly unified multimodal models.
...
TL;DR: SMC-CFG introduces sliding mode control to stabilize CFG in diffusion models, improving semantic alignment and robustness across guidance scales.


<details>
  <summary>Details</summary>
Motivation: Existing CFG methods use linear control which causes instability, overshooting, and degraded semantic fidelity at large guidance scales.

Method: Reinterprets CFG as control applied to generative flow, introduces sliding mode control with exponential sliding surface and switching control term for nonlinear feedback correction.

Result: Outperforms standard CFG in semantic alignment and robustness across guidance scales on models like Stable Diffusion 3.5, Flux, and Qwen-Image.

Conclusion: SMC-CFG provides stable, nonlinear control for CFG with theoretical convergence guarantees, enhancing diffusion model performance.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based conversational agents lack embodiment and expressive gestures essential for natural interaction. Existing ECA solutions produce rigid, low-diversity motions, while generative gesture synthesis methods depend on future speech context and have long run-times, making them unsuitable for real-time interaction.
...
Result: Comparative evaluations demonstrate that the causal and real-time approach produces natural and contextually aligned gestures against recent baselines.

Conclusion: MIBURI bridges the gap between conversational agents and natural embodied interaction by enabling real-time generation of expressive gestures and facial expressions synchronized with spoken dialogue.
...
TL;DR: Utonia is a self-supervised point transformer encoder trained across diverse 3D point cloud domains that learns consistent representations, improves perception, and benefits embodied/multimodal reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: To create a unified foundation model for point clouds from all domains (remote sensing, LiDAR, RGB-D, CAD, video) that can benefit them collectively, moving toward a single model for sparse 3D data across AR/VR, robotics, and autonomous driving applications.

Method: Utonia uses a self-supervised point transformer encoder trained jointly across diverse point cloud domains with different sensing geometries, densities, and priors, learning consistent representation spaces through cross-domain training.

Result: Utonia learns consistent representations that transfer across domains, improves perception capabilities, reveals emergent behaviors from joint training, benefits robotic manipulation when conditioning vision-language-action policies, and enhances spatial reasoning in vision-language models.

Conclusion: Utonia represents a significant step toward foundation models for sparse 3D data, demonstrating that unified training across diverse point cloud domains yields improved performance and emergent capabilities, with promising applications in AR/VR, robotics, and autonomous driving.
...
TL;DR: Proposes a goal-oriented semantic communication framework for UAV obstacle avoidance that integrates sensing and command/control signals, using Kalman filtering for position prediction, Mahalanobis distance-based navigation, and effectiveness-aware transmission scheduling to dramatically reduce signal transmissions while maintaining 100% task success.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency of conventional ISAC frameworks that transmit sensing and C&C signals at every time slot, which wastes resources. Need to develop a more efficient communication framework that maintains UAV obstacle avoidance performance while reducing transmission overhead.

Method: 1) Kalman filter for continuous UAV position prediction to reduce reliance on continuous sensing transmissions. 2) Mahalanobis distance-based dynamic window approach (MD-DWA) for generating precise C&C signals under uncertainty. 3) Effectiveness-aware deep Q-network (E-DQN) to determine transmission timing based on value of information (VoI) - sensing VoI measured by uncertainty entropy reduction, C&C VoI by navigation improvement contribution.

Result: Achieves 100% task success rate (same as conventional ISAC) while reducing transmitted sensing and C&C signals by 92.4% and transmission time slots by 85.5%. The framework establishes an efficient closed-loop system for UAV obstacle avoidance.

Conclusion: The proposed GOSC framework successfully integrates sensing and communication for UAV obstacle avoidance, dramatically reducing transmission overhead while maintaining performance through semantic communication principles and intelligent transmission scheduling based on information value.
...
TL;DR: A whole-body controller combining model-based admittance control for arm manipulation with RL-based legged locomotion, enabling compliant human-robot interaction and unified 6-DoF force response on quadruped robots.


<details>
  <summary>Details</summary>
Motivation: Coordinating legged locomotion with arm manipulation while ensuring safety and compliance during contact interaction remains challenging for mobile manipulation robots.

Method: Combines model-based admittance control for the manipulator arm (mapping external wrenches to desired end-effector velocities) with Reinforcement Learning policy for legged locomotion, enhanced by Reference Governor for safety and Kalman filter with neural networks for base velocity estimation.

Result: Validated on Unitree Go2 quadruped with 6-DoF arm and force/torque sensor, demonstrating accurate tracking of interaction-driven velocities, compliant behavior, and safe, reliable performance in dynamic settings.

Conclusion: The proposed whole-body controller successfully enables compliant human-robot interaction and unified force response for mobile manipulation robots, addressing the challenges of coordinating locomotion and manipulation with safety guarantees.
...
TL;DR: A Patient-Therapist Force Field visualization and Synthetic Therapist ML model for robot-mediated stroke rehabilitation, using VAE/GMM for interaction analysis and LSTM for predicting therapist torque responses.


<details>
  <summary>Details</summary>
Motivation: Post-stroke rehabilitation is physically demanding for therapists, potentially reducing therapy intensity and consistency. There's a need to support therapists and maintain high-quality rehabilitation through robotic assistance.

Method: 1) PTFF: Uses VAE to encode patient/therapist kinematics into shared latent space, then GMM to model probabilistic vector field mapping patient states to therapist responses. 2) ST: LSTM network trained on interaction data to predict therapist-applied joint torques from patient kinematics. Integrated into ROS-based exoskeleton controller for real-time assistance.

Result: Trained and validated using leave-one-out cross-validation across eight post-stroke patients. Offline results and preliminary testing show potential as alternative approach to post-stroke exoskeleton therapy. The system allows therapists to monitor patients while exoskeleton provides assistance.

Conclusion: The PTFF provides understanding of therapist actions while the ST frees human therapists from physical demands, enabling continuous patient monitoring. This represents a promising approach for improving robot-mediated stroke rehabilitation.
...
TL;DR: A modular, cable-driven soft robotic arm with stackable segments and independent control, featuring embedded tendon channels and dual-helical tendon protection, demonstrating significant workspace expansion and stiffness-performance trade-offs.


<details>
  <summary>Details</summary>
Motivation: To develop a reconfigurable soft robotic arm that can adapt to diverse structural and application requirements through modular stacking, enabling scalable workspace expansion while maintaining independent segment control.

Method: Fabricated from soft silicone material with embedded tendon-routing channels and protective dual-helical tendon structure. The system features stackable segments with independent control via cable-driven actuation, allowing modular reconfiguration.

Result: Three-segment configuration achieved 13-fold increase in planar workspace area and 38.9-fold increase in workspace volume compared to single-segment arm. Experiments revealed stiffness-performance trade-off: softer silicone improved bending flexibility, while stiffer silicone enhanced structural rigidity and load-bearing stability.

Conclusion: The modular, reconfigurable soft robotic arm architecture successfully enables scalable workspace expansion. Stiffness tuning of silicone material provides a means to balance compliance and strength, highlighting potential for configuring adaptable soft robotic systems for diverse applications.
...
TL;DR: A unified motion planning method for Maritime Autonomous Surface Ships (MASS) that integrates collision avoidance, COLREGs compliance, and grounding prevention using convex optimization with uncertainty handling.


<details>
  <summary>Details</summary>
Motivation: MASS deployment faces challenges in congested waters where most maritime accidents occur, requiring safe, regulation-aware motion planning that works under dynamic maritime conditions.

Method: Convex optimization integrating velocity obstacle-based collision constraints, COLREGs-based directional constraints, and bathymetry-based grounding constraints. Extends VO method to handle uncertainty in target vessel estimates and uses integer linear programming to approximate nonconvex shallow water regions as convex geometries.

Result: The method generates optimal, dynamically feasible input velocities meeting all safety requirements. Simulation results with multi-vessel encounters demonstrate effective production of safe, regulation-compliant maneuvers suitable for real-time autonomous navigation.

Conclusion: The proposed unified motion planning approach successfully addresses key challenges for MASS deployment by providing computationally efficient, rule-compliant navigation that handles collision avoidance, regulatory compliance, and grounding prevention simultaneously.
...
TL;DR: Enhanced virtual simulation framework for verifying Maritime Autonomous Surface Ships (MASS) with realistic weather and bathymetry modeling.


<details>
  <summary>Details</summary>
Motivation: MASS deployment requires rigorous verification and validation under various environmental conditions, including extreme scenarios that are impractical or unsafe for real-world testing.

Method: Developed a high-fidelity environmental modeling suite that simulates adverse weather (rain, fog, waves) and integrates high-resolution bathymetric data from major U.S. ports. Weather parameters affect sea-state, perception systems, and situational awareness.

Result: Created a configurable virtual simulation framework enabling systematic testing of MASS in diverse maritime conditions, including depth-aware navigation, grounding prevention, and evaluation of vessel controllability in shallow waterways.

Conclusion: The framework supports comprehensive V&V of MASS by enabling testing in realistic maritime environments with weather and bathymetry effects, addressing scenarios too dangerous for physical trials.
...
<details>
  <summary>Details</summary>
Motivation: Conventional silicone-non-extensible material integration methods rely on irreversible adhesives or labor-intensive embedding processes that are difficult to modify, limiting rapid prototyping and component reuse in soft robotics.

Method: Developed silicone-glossy paper bonding (SGB) that forms robust interfaces through simple contact, characterized by shear strength testing, ethanol immersion for detachment/reassembly, and analysis of surface functional groups, modulus effects, and adhesion mechanisms.

Result: SGB achieves >61 kPa shear strength, full detachment/reassembly via ethanol without performance loss, synergistic hydrogen bonding and oligomer diffusion mechanism, and soft actuators with equal or better performance than conventional embedded designs.

Conclusion: SGB provides a scalable, sustainable platform for rapid prototyping in soft robotics by simplifying fabrication while supporting reuse, rapid iteration, and programmable actuation modes.
...
TL;DR: Unveiler is a specialized, decoupled robotic manipulation framework that separates spatial reasoning from action execution, outperforming end-to-end models in retrieving objects from dense clutter with high success rates.


<details>
  <summary>Details</summary>
Motivation: Current large-scale end-to-end models for robotic manipulation lack data efficiency and modularity for retrieving objects in dense clutter. The authors argue for specialized, decoupled systems that separate high-level reasoning from low-level execution.

Method: Unveiler uses a transformer-based Spatial Relationship Encoder (SRE) to identify critical obstacles for removal, then passes discrete decisions to a rotation-invariant Action Decoder. Training involves two stages: imitation learning from heuristic demonstrations followed by PPO fine-tuning.

Result: Achieved 97.6% success in partially occluded and 90.0% in fully occluded scenarios in simulation. The SRE's spatial reasoning transfers zero-shot to real scenes, and the full system works on physical robots with only geometric calibration, no retraining needed.

Conclusion: Specialized, decoupled architectures with object-centric reasoning are more computationally efficient and effective than end-to-end models for complex manipulation tasks in dense clutter, demonstrating strong transfer to real-world applications.
...
TL;DR: PathSpace is a novel semantic SLAM framework using continuous B-splines for compact environment representation and probabilistic reasoning, tested in autonomous racing with reduced resource usage and comparable accuracy to traditional methods.


<details>
  <summary>Details</summary>
Motivation: Current semantic SLAM techniques rely on dense geometric representations that limit context-based constraints. There's a need for more compact representations that maintain probabilistic reasoning capabilities while reducing computational resources.

Method: Proposes PathSpace framework using continuous B-splines to represent environments compactly while maintaining continuous probability density functions for probabilistic reasoning. B-splines interpolate and fit sparse environments, exploiting track characteristics in autonomous racing applications.

Result: The framework produces significantly reduced representations with comparable accuracy to traditional landmark-based methods, demonstrating potential for limiting system resources with minimal accuracy loss in autonomous racing scenarios.

Conclusion: PathSpace offers a promising approach to semantic SLAM by using B-splines for compact environment representation, enabling context-based constraints while reducing computational resources, particularly valuable for applications like autonomous racing.
...
<details>
  <summary>Details</summary>
Motivation: Scrub nurses experience physical fatigue and decreased focus from repetitive instrument delivery tasks. Existing robotic scrub nurses rely on predefined paths, limiting generalizability and posing safety risks in dynamic surgical environments.

Method: Uses a vision-language model to automatically generate grasping and delivery trajectories from surgeons' instructions in zero-shot manner. Implements real-time obstacle minimum distance perception integrated into a unified quadratic programming framework for reactive obstacle avoidance and self-collision prevention.

Result: The system achieves 83.33% success rate in surgical instrument delivery while maintaining smooth, collision-free movement throughout all trials.

Conclusion: The proposed robotic system effectively addresses limitations of existing robotic scrub nurses by enabling safe, autonomous instrument delivery in dynamic surgical environments through zero-shot trajectory generation and real-time obstacle avoidance.
...
TL;DR: Symmetry-aware graph neural network enables contact state inference from proprioceptive data for tensegrity robots, improving pose estimation accuracy with limited training data.


<details>
  <summary>Details</summary>
Motivation: Tensegrity robots have lightweight, resilient structures but face state estimation challenges due to compliant ground contacts. Existing methods struggle with contact state inference without dedicated sensors.

Method: Proposes Sym-HGNN (symmetry-aware heterogeneous graph neural network) that incorporates robot's D3 dihedral symmetry into message-passing to infer contact states from IMU and cable-length histories. Predictions are integrated into a contact-aided invariant extended Kalman filter for pose estimation.

Result: Achieves 15% higher accuracy and 5% higher F1-score using only 20% of training data compared to CNN and MI-HGNN baselines. Maintains low-drift, physically consistent state estimation comparable to ground truth contacts.

Conclusion: Demonstrates potential of fully proprioceptive sensing for accurate, robust state estimation in tensegrity robots by leveraging symmetry properties to enhance sample efficiency and generalization.
...
TL;DR: Wukong-Omni is a novel multimode robot that operates across land, air, and underwater using a unified propulsion system, achieving 100% efficiency improvement and 150% thrust increase over conventional methods.


<details>
  <summary>Details</summary>
Motivation: In flood disaster rescue scenarios, partially submerged buildings prevent aerial robots from accessing lower levels, limiting mission effectiveness. Current robots lack the ability to operate across multiple domains (land, air, underwater) seamlessly.

Method: Developed Wukong-Omni with innovative mechanical design allowing motor reuse and improved thrust generation. Used simulation and tank-based optimization to enhance efficiency and peak thrust. Created dynamic models for three operating domains and proposed unified cross-domain control framework.

Result: Experimental results show 100% improvement in propulsion efficiency and 150% increase in maximum thrust compared to direct installation methods. Comprehensive experiments validate stable locomotion and smooth transition across domains. Outdoor experiments demonstrate robustness and adaptability in real-world environments.

Conclusion: Wukong-Omni successfully addresses the challenge of accessing partially submerged buildings in flood rescue scenarios by enabling seamless operation across land, air, and underwater domains with significantly improved efficiency and thrust performance.
...
TL;DR: Uni-Skill is a unified skill-centric framework that enables automatic skill evolution through self-augmented skill libraries and hierarchical skill taxonomy from robotic videos, achieving state-of-the-art zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Existing skill-centric approaches rely on fixed skill libraries that limit adaptability to new tasks without manual intervention, creating a bottleneck for generalization in compositional tasks.

Method: Proposes Uni-Skill with skill-aware planning that requests new skill implementations when needed, and SkillFolder - a hierarchical skill taxonomy repository derived from large-scale robotic videos with automatic annotation for efficient offline structural retrieval.

Result: Achieves state-of-the-art performance in simulation and real-world settings, demonstrating advanced reasoning capabilities and strong zero-shot generalization across novel tasks without deployment-time demonstrations.

Conclusion: Uni-Skill shifts skill acquisition from inefficient manual annotation to efficient structural retrieval, enabling adaptable planning with self-augmented skill libraries and superior generalization in compositional tasks.
...
TL;DR: cuNRTO framework enables fast robust trajectory optimization on GPUs using two parallel architectures (DR splitting and ADMM) for solving large SOCP constraints, achieving up to 139.6× speedup.


<details>
  <summary>Details</summary>
Motivation: Robust trajectory optimization is computationally expensive due to large Second Order Conic Programming (SOCP) constraints needed for safety under uncertainty, requiring more efficient solutions.

Method: Two GPU-accelerated architectures: 1) NRTO-DR uses Douglas-Rachford splitting for SOCP subproblems with parallel projections and sparse solves; 2) NRTO-FullADMM uses ADMM to exploit problem structure for better scalability. Both implemented with custom CUDA kernels and cuBLAS.

Result: Validated on unicycle, quadcopter, and Franka manipulator models, demonstrating up to 139.6× speedup compared to existing methods.

Conclusion: cuNRTO enables efficient robust trajectory optimization through GPU parallelization, making real-time robust decision-making feasible for autonomous systems.
...
TL;DR: A training-free compositional diffusion method for long-horizon robot planning that enforces boundary agreement on clean data estimates rather than noisy states, using factor graph inference with message passing.


<details>
  <summary>Details</summary>
Motivation: Diffusion models work well for short-horizon planning but struggle with long-horizon tasks due to computational constraints and limited training data. Existing compositional approaches that stitch together short segments suffer from instability when the factorization assumption breaks down in noisy data space.

Method: Formulates long-horizon planning as inference over a chain-structured factor graph of overlapping video chunks, where pretrained short-horizon video diffusion models provide local priors. Enforces boundary agreement through a novel combination of synchronous and asynchronous message passing that operates on Tweedie estimates (clean data estimates).

Result: Demonstrates significant improvements over existing baselines, effectively generalizing to unseen start-goal combinations that were not present in the original training data.

Conclusion: The key to stable compositional generation lies in enforcing boundary agreement on estimated clean data rather than noisy intermediate states, enabling training-free long-horizon planning with pretrained short-horizon models.
...
TL;DR: SemLoco is an RL framework that enables legged robots to avoid stepping on low-lying obstacles in cluttered environments by combining semantic understanding with precise foot placement control.


<details>
  <summary>Details</summary>
Motivation: Current legged robots struggle to avoid stepping on low-lying objects like cables or small devices in cluttered environments due to disconnection between high-level semantic understanding and low-level control, combined with elevation map errors in real-world operation.

Method: SemLoco uses a two-stage Reinforcement Learning approach combining soft and hard constraints with pixel-wise foothold safety inference. It integrates semantic maps to assign traversability costs rather than relying solely on geometric data, enabling more accurate foot placement.

Result: SemLoco significantly reduces collisions and improves safety around sensitive objects, enabling reliable navigation where traditional controllers would likely cause damage. It can be effectively applied to complex, unstructured real-world environments.

Conclusion: The SemLoco framework successfully addresses the challenge of safe navigation in cluttered environments by bridging semantic understanding with precise foot placement control, representing an important advancement for practical deployment of legged robots.
...
TL;DR: IMR-LLM: LLM-driven framework for industrial multi-robot task planning and program generation using disjunctive graphs and process trees, outperforming existing methods on new benchmark IMR-Bench.


<details>
  <summary>Details</summary>
Motivation: Industrial multi-robot collaboration faces challenges with strict sequential constraints and complex task dependencies that existing LLM approaches for simple household tasks cannot handle effectively.

Method: Two-stage approach: 1) LLMs assist in constructing disjunctive graphs, then deterministic solving yields high-level task plans; 2) Process trees guide LLMs to generate executable low-level programs.

Result: Method significantly surpasses existing methods across all evaluation metrics on IMR-Bench, a new challenging benchmark with multi-robot industrial tasks across three complexity levels.

Conclusion: IMR-LLM successfully addresses industrial multi-robot planning challenges by combining LLM reasoning with structured planning methods, enabling effective coordination for complex manufacturing tasks.
...
TL;DR: Multi-mode hybrid trajectory planning for UAVs using real-time environmental awareness to dynamically select optimal planning models, with goal-oriented spatial awareness and lazy replanning for efficiency.


<details>
  <summary>Details</summary>
Motivation: Current motion planning algorithms for unmanned systems have limitations in planning efficiency due to inflexible strategies and weak adaptability to environmental changes.

Method: 1) Goal-oriented spatial awareness method for rapid flight safety assessment; 2) Multi-mode hybrid trajectory planning mechanism that selects optimal planning model based on prior spatial awareness; 3) Lazy replanning strategy that triggers replanning only when necessary.

Result: Outperforms existing SOTA algorithms across multiple metrics, achieving best performance in average number of planning iterations and computational cost per iteration. Validated through both simulation experiments and real-world flight tests with self-developed UAV platform.

Conclusion: The proposed multi-mode hybrid trajectory planning method effectively addresses efficiency limitations in UAV motion planning by dynamically adapting to environmental changes while maintaining flight quality and reducing computational costs.
...
TL;DR: ADR-VINS: A robust monocular visual-inertial state estimation framework for autonomous drone racing using direct pixel reprojection errors in an ESKF, requiring only 2 visible gate corners instead of 4, with offline ADR-FGO for reference trajectory generation in GNSS-denied environments.


<details>
  <summary>Details</summary>
Motivation: Autonomous drone racing requires computationally efficient and resilient state estimation that can handle perceptual degradation during extreme maneuvers. Traditional visual-inertial pipelines have rigid requirements (4 visible features) and suffer from information loss, while competitive racing environments lack GNSS/Motion Capture systems for evaluation.
...
Result: On TII-RATM dataset: ADR-VINS achieves 0.134 m average RMS translation error, ADR-FGO yields 0.060 m as smoothing-based reference. Successfully deployed in A2RL Drone Championship Season 2, maintaining stable estimation during high-agility flight at 20.9 m/s top speeds.

Conclusion: ADR-VINS provides robust, efficient state estimation for autonomous drone racing with minimal feature requirements (only 2 corners), while ADR-FGO enables performance evaluation in uninstrumented, GNSS-denied racing environments. The system demonstrates practical viability in competitive racing scenarios.
...
### [133] [Robotic Grasping and Placement Controlled by EEG-Based Hybrid Visual and Motor Imagery](https://arxiv.org/abs/2603.03181)
...
<details>
  <summary>Details</summary>
Motivation: To enhance human-robot interaction by bridging neural signals with physical control, enabling intuitive intention-driven robotic manipulation through purely imagery-based BCI without external cues.

Method: Uses dual-channel EEG decoding: visual imagery (VI) identifies objects for grasping, motor imagery (MI) determines placement poses. Offline-pretrained decoders deployed in zero-shot manner within online streaming pipeline, operating solely on EEG via cue-free imagery protocol on Base robotic platform.

Result: Online decoding accuracies: 40.23% (VI) and 62.59% (MI). End-to-end task success rate: 20.88%. System validated across diverse scenarios including occluded targets and varying participant postures.

Conclusion: Demonstrates that high-level visual cognition can be decoded in real-time and translated into executable robot commands, bridging neural signals with physical interaction and validating purely imagery-based BCI for practical human-robot collaboration.
...
### [134] [Agentic Self-Evolutionary Replanning for Embodied Navigation](https://arxiv.org/abs/2603.02772)
*Guoliang Li,Ruihua Han,Chengyang Li,He Li,Shuai Wang,Wenchao Ding,Hong Zhang,Chengzhong Xu*

Main category: cs.RO

TL;DR: SERP introduces self-evolutionary replanning where robots learn from failures to upgrade their action models during navigation, achieving higher success rates with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Current replanning approaches freeze the robot's action model, missing opportunities to improve the robot itself through learning from recent experiences during navigation failures.

Method: Proposes Self-Evolutionary RePlanning (SERP) with agentic self-evolving action models using in-context learning with auto-differentiation (ILAD) for adaptive function adjustment, and graph chain-of-thought (GCOT) for token-efficient replanning with LLMs over distilled graphs.

Result: Extensive simulation and real-world experiments show SERP achieves higher success rates with lower token expenditure across various benchmarks, demonstrating superior robustness and efficiency.

Conclusion: SERP represents a paradigm shift from frozen models to evolving models, enabling robots to learn from failures and upgrade themselves during navigation for improved resilience and efficiency.
...
### [135] [Generative adversarial imitation learning for robot swarms: Learning from human demonstrations and trained policies](https://arxiv.org/abs/2603.02783)
*Mattes Kraus,Jonas Kuckling*

Main category: cs.RO

TL;DR: A generative adversarial imitation learning framework enables swarm robots to learn collective behaviors from human demonstrations, successfully transferring learned policies from simulation to real TurtleBot 4 robots.


<details>
  <summary>Details</summary>
Motivation: Most imitation learning in swarm robotics uses demonstrations from existing policies, but this work aims to learn collective behaviors directly from human demonstrations to enable more natural and intuitive swarm control.

Method: The framework uses generative adversarial imitation learning (GAIL) to learn swarm behaviors from demonstrations, evaluated across six different missions with both manual human demonstrations and PPO-trained policy demonstrations.

Result: The imitation learning process successfully learns qualitatively meaningful behaviors that perform similarly well as the provided demonstrations, and the learned policies successfully transfer to real TurtleBot 4 robots with preserved visual characteristics and comparable performance to simulation.

Conclusion: The GAIL-based framework effectively enables swarm robots to learn collective behaviors from human demonstrations, with successful sim-to-real transfer, demonstrating the viability of human-guided imitation learning for swarm robotics applications.
...
### [136] [SPARC: Spatial-Aware Path Planning via Attentive Robot Communication](https://arxiv.org/abs/2603.02845)
*Sayang Mu,Xiangyu Wu,Bo An*

Main category: cs.RO

TL;DR: RMHA improves multi-robot path planning by embedding spatial distances into attention mechanisms for better communication prioritization in congested areas.


<details>
  <summary>Details</summary>
Motivation: Existing learned communication methods treat all neighboring robots equally regardless of spatial proximity, diluting attention in congested regions where coordination matters most.

Method: Propose Relation enhanced Multi Head Attention (RMHA) that explicitly embeds pairwise Manhattan distances into attention weight computation, with distance-constrained attention mask and GRU gated message fusion, integrated with MAPPO for stable training.

Result: In zero-shot generalization from 8 to 128 robots on 40x40 grids, RMHA achieves ~75% success rate at 30% obstacle density, outperforming best baseline by over 25 percentage points.

Conclusion: Distance-relation encoding is the key contributor to success rate improvement in high-density environments, enabling robots to dynamically prioritize messages from spatially relevant neighbors.
...
### [137] [Design, Modeling and Direction Control of a Wire-Driven Robotic Fish Based on a 2-DoF Crank-Slider Mechanism](https://arxiv.org/abs/2603.02851)
*Yita Wang,Chen Chen,Yicheng Chen,Jinjie Li,Yuichi Motegi,Kenji Ohkuma,Toshihiro Maki,Moju Zhao*

Main category: cs.RO

TL;DR: Researchers developed a wire-driven robotic fish with a 2-DoF crank-slider mechanism that decouples propulsion from steering, enabling both high swimming speed and agile maneuvering.


<details>
  <summary>Details</summary>
Motivation: Existing robotic fish designs face a trade-off: motor-driven approaches with multiple servos offer precise control but limited speed, while brushless motor designs enable higher speed but poor maneuverability and turning capability.

Method: Developed a wire-driven robotic fish with a 2-degree-of-freedom crank-slider mechanism that decouples propulsion from steering, including elastic skeleton, waterproof structure, actuation modeling, body dynamics analysis, and a combined feedforward-feedback control strategy.

Result: Prototype experiments validated the design, modeling, and control approach, demonstrating successful swimming, turning, and directional control capabilities.

Conclusion: The proposed robotic fish design successfully addresses the speed-maneuverability trade-off through decoupled propulsion and steering, enabling both high swimming performance and agile maneuvering for environmental monitoring and biological survey applications.
...
### [138] [CoFL: Continuous Flow Fields for Language-Conditioned Navigation](https://arxiv.org/abs/2603.02854)
*Haokun Liu,Zhaoqi Ma,Yicheng Chen,Masaki Kitagawa,Wentao Zhang,Jinjie Li,Moju Zhao*

Main category: cs.RO

TL;DR: CoFL is an end-to-end navigation policy that maps BEV observations and language instructions to continuous flow fields for smooth, reactive motion, outperforming modular and generative baselines.


<details>
  <summary>Details</summary>
Motivation: Existing language-conditioned navigation approaches rely on brittle modular components or costly action-sequence generation, which limits their robustness and efficiency.

Method: CoFL directly maps BEV observations and language instructions to continuous flow fields that output instantaneous velocities at arbitrary 2D locations, with trajectories obtained via numerical integration of the predicted field.

Result: CoFL significantly outperforms modular VLM-based planners and generative policy baselines on unseen scenes, and demonstrates reliable closed-loop control with high success rates in real-world experiments.

Conclusion: The continuous flow field approach enables smooth, reactive navigation that generalizes well to unseen environments and real-world deployment, offering advantages over discrete action prediction methods.
...
### [139] [Rhythm: Learning Interactive Whole-Body Control for Dual Humanoids](https://arxiv.org/abs/2603.02856)
...
TL;DR: Rhythm is the first unified framework enabling real-world deployment of dual-humanoid systems for complex, physically plausible interactions like hugging and dancing.


<details>
  <summary>Details</summary>
Motivation: While individual robot agility has improved, physically coupled multi-humanoid interaction remains challenging due to kinematic mismatches and complex contact dynamics. There's a need for interactive whole-body control for multi-humanoid systems to enable complex collaborative capabilities.

Method: Three core components: 1) Interaction-Aware Motion Retargeting (IAMR) generates feasible humanoid interaction references from human data; 2) Interaction-Guided Reinforcement Learning (IGRL) policy masters coupled dynamics via graph-based rewards; 3) Real-world deployment system enables robust transfer of dual-humanoid interaction.
...
### [140] [Emerging trends in Cislunar Space for Lunar Science Exploration and Space Robotics aiding Human Spaceflight Safety](https://arxiv.org/abs/2603.02878)
*Arsalan Muhammad,Yue Wang,Hai Huang,Hao Wang*

Main category: cs.RO

TL;DR: The paper explores AI and space robotics as transformative technologies for sustainable lunar exploration and future Mars missions, focusing on autonomous systems for science, infrastructure, and astronaut safety.


<details>
  <summary>Details</summary>
Motivation: The Moon serves as a critical testbed for technologies needed for sustained human presence and interplanetary exploration. There's a need to accelerate humanity's interplanetary aspirations through advanced technological solutions.

Method: The study identifies and investigates two key research domains: 1) Lunar Science Exploration with AI and Space Robotics, and 2) Space Robotics for manned spaceflight support. The approach involves integrating autonomy, machine learning, and real-time sensor fusion into robotic systems.

Result: Space robotics and AI are identified as transformative technologies that can redefine space exploration frontiers, serve as critical assets for infrastructure development, support surface operations, and ensure astronaut safety on the Moon.

Conclusion: AI and space robotics not only augment human capabilities but act as force multipliers for sustainable lunar exploration, paving the way for future crewed missions to Mars and beyond by enabling autonomous operations and resource utilization.
...
### [141] [Tracing Back Error Sources to Explain and Mitigate Pose Estimation Failures](https://arxiv.org/abs/2603.02881)
*Loris Schneider,Yitian Shi,Rosa Wolf,Carolin Brenner,Rudolph Triebel,Rania Rayyes*

Main category: cs.RO

TL;DR: A modular uncertainty-aware framework improves pose estimation robustness by detecting failures, attributing errors to specific sources, and applying targeted recovery strategies, outperforming foundation models with simpler/faster ICP.


<details>
  <summary>Details</summary>
Motivation: Foundation pose estimators struggle with environmental uncertainties, requiring long inference times and heavy computation. There's a need for more robust, efficient solutions for robotic manipulation tasks.

Method: Proposes a modular framework that decomposes pose estimation into: 1) failure detection, 2) error attribution to specific sources, and 3) targeted mitigation strategies applied only when necessary. Uses ICP as lightweight pose estimator.

Result: Significantly improves ICP robustness, achieves competitive performance compared to foundation models, while relying on substantially simpler and faster pose estimator suitable for real-world robotic grasping.

Conclusion: Modular uncertainty-aware framework with targeted error handling outperforms naive foundation approaches, enabling robust pose estimation with lightweight methods for practical robotic applications.
...
### [142] [Self-supervised Domain Adaptation for Visual 3D Pose Estimation of Nano-drone Racing Gates by Enforcing Geometric Consistency](https://arxiv.org/abs/2603.02936)
*Nicholas Carlotti,Michele Antonazzi,Elia Cereda,Mirko Nava,Nicola Basilico,Daniele Palossi,Alessandro Giusti*

Main category: cs.RO

TL;DR: Unsupervised domain adaptation approach using self-supervised odometry annotations to adapt a pre-trained CNN for drone gate pose estimation from simulation to real world, achieving 40% position and 37% orientation improvement.


<details>
  <summary>Details</summary>
Motivation: Pre-trained CNN models for drone gate pose estimation suffer from sim-to-real gap, requiring adaptation to real-world conditions without expensive manual labeling.

Method: UDA approach using real image sequences with self-supervised odometry annotations; state consistency loss enforces pose predictions to be consistent with drone's odometry between different timestamps.

Result: Outperforms other state-of-the-art UDA approaches, achieves low MAE (position: x=26, y=28, z=10 cm; orientation: ψ=13°), 40% position and 37% orientation improvement over baseline, effective with just 10 minutes of flight data, runs at 33 fps on nano-drone.

Conclusion: Proposed self-supervised UDA approach effectively bridges sim-to-real gap for drone gate pose estimation, enabling accurate real-world deployment with minimal real data and efficient onboard inference.
...
### [143] [DreamFlow: Local Navigation Beyond Observation via Conditional Flow Matching in the Latent Space](https://arxiv.org/abs/2603.02976)
*Jiwon Park,Dongkyu Lee,I Made Aswin Nahrendra,Jaeyoung Lim,Hyun Myung*

Main category: cs.RO

TL;DR: DreamFlow is a DRL-based local navigation framework that uses conditional flow matching to predict unobserved environmental features, extending robot perception beyond onboard sensors to avoid local minima in cluttered environments.


<details>
  <summary>Details</summary>
Motivation: Local navigation in cluttered environments suffers from dense obstacles and frequent local minima. Conventional planners rely on heuristics and fail often, while DRL approaches are limited by onboard sensing constraints, causing navigation failures when robots can't perceive structures outside their field of view.

Method: DreamFlow uses conditional flow matching (CFM) to extend robot's perceptual horizon. A CFM-based prediction module learns probabilistic mapping between local height map latent representations and broader spatial representations conditioned on navigation context, enabling prediction of unobserved environmental features.

Result: DreamFlow outperforms existing methods in both latent prediction accuracy and navigation performance in simulation. The method was further validated successfully in cluttered real-world environments using a quadrupedal robot.

Conclusion: DreamFlow effectively addresses local navigation challenges in cluttered environments by extending perceptual range through CFM-based prediction, enabling proactive avoidance of local minima and improving navigation performance both in simulation and real-world applications.
...
### [144] [CASSR: Continuous A-Star Search through Reachability for real time footstep planning](https://arxiv.org/abs/2603.02989)
*Jiayi Wang,Steve Tonneau*

Main category: cs.RO

TL;DR: CASSR is a novel footstep planning framework that combines A* search with continuous convex constraint propagation and an EPA-based heuristic, achieving 100x speedup over traditional methods for biped robot locomotion.


<details>
  <summary>Details</summary>
Motivation: Traditional footstep planning methods face challenges: A* requires discretization of reachability constraints which limits accuracy, while Mixed-Integer Programming (MIP) supports continuous formulations but becomes intractable quickly, especially when rotations are involved. There's a need for efficient, real-time footstep planning for biped robots.

Method: CASSR recursively propagates convex, continuous formulations of a robot's kinematic constraints within an A* search framework. It combines this with a novel cost-to-go heuristic based on the EPA (Expanding Polytope Algorithm) algorithm to efficiently explore the search space.

Result: CASSR can plan contact sequences of up to 30 footsteps in under 125 ms. Experiments show it outperforms traditional discretised A* by up to a factor of 100, and also surpasses commercial MIP solvers in performance.

Conclusion: CASSR enables fast, reliable, and real-time footstep planning for biped robots by effectively combining continuous constraint formulations with efficient search heuristics, overcoming limitations of both traditional A* and MIP approaches.
...
### [145] [MA-CoNav: A Master-Slave Multi-Agent Framework with Hierarchical Collaboration and Dual-Level Reflection for Long-Horizon Embodied VLN](https://arxiv.org/abs/2603.03024)
*Ling Luo,Qianqian Bai*

Main category: cs.RO

TL;DR: MA-CoNav is a multi-agent collaborative navigation framework that uses specialized agents for different navigation functions, outperforming existing VLN methods on real-world indoor datasets.


<details>
  <summary>Details</summary>
Motivation: Existing VLN methods suffer from perceptual distortion and decision drift in complex, long-distance tasks due to cognitive overload of single agents. The paper aims to address this by distributing navigation functions across specialized agents.

Method: Proposes MA-CoNav with a Master-Slave hierarchical architecture: Master Agent for global orchestration, and Subordinate Agents (Observation, Planning, Execution, Memory) with specialized functions. Includes Local-Global dual-stage reflection mechanism for dynamic optimization.

Result: Comprehensively outperforms existing mainstream VLN methods across multiple metrics on real-world indoor dataset collected by Limo Pro robot, without scene-specific fine-tuning.

Conclusion: The multi-agent collaborative approach effectively addresses cognitive overload in VLN tasks, establishing an efficient language-understanding-visual-perception-embodied-execution closed loop for long-horizon navigation.
...
### [146] [Architectural HRI: Towards a Robotic Paradigm Shift in Human-Building Interaction](https://arxiv.org/abs/2603.03052)
*Alex Binh Vinh Duc Nguyen*

Main category: cs.RO

TL;DR: The paper proposes a paradigm shift in Human-Building Interaction where multiple architectural layers physically adapt in synchrony, creating opportunities to transfer HRI knowledge to unconventional robotic morphologies like buildings.


<details>
  <summary>Details</summary>
Motivation: Recent advances in sensing, communication, interfaces, control, and robotics are expanding HBI beyond adaptive building services and facades toward physical actuation of architectural space. The paper identifies an emerging research agenda challenged by temporal, spatial, and social complexity, and fragmented knowledge across multiple disciplines.

Method: The paper proposes an interdisciplinary research approach that unifies the why, what, and how of robotic actuation in architectural forms. It suggests transferring Human-Robot Interaction (HRI) knowledge to unconventional robotic morphologies, including interpreting robots as multiple architectural layers or even as buildings.

Result: The paper identifies that robotic furniture, swarm robotics, and shape-changing spaces demonstrate that architectural elements can now be robotically augmented to move, reconfigure, and adapt space. These advances promise a paradigm shift where building layers physically adapt in synchrony to support occupant needs and sustainability goals holistically.

Conclusion: The authors call for interdisciplinary research that bridges HCI, environmental psychology, cognitive science, and architecture to address the challenges of architectural HRI and realize the potential of physically adaptive architectural spaces as a new frontier in human-building interaction.
...
### [147] [CMoE: Contrastive Mixture of Experts for Motion Control and Terrain Adaptation of Humanoid Robots](https://arxiv.org/abs/2603.03067)
*Shihao Ma,Hongjin Chen,Zijun Xu,Yi Zhao,Ke Wu,Ruichen Yang,Leyao Zou,Zhongxue Gan,Wenchao Ding*

Main category: cs.RO

TL;DR: CMoE: A single-stage RL framework using contrastive learning to improve expert specialization in mixture of experts for humanoid robot navigation across diverse terrains.


<details>
  <summary>Details</summary>
Motivation: Humanoid robots need to navigate complex terrains with abrupt transitions in real-world environments. Current Vanilla MoE frameworks fail to achieve proper expert specialization as gating networks show uniform expert activations across different terrains, limiting expressive power.

Method: CMoE integrates contrastive learning into a single-stage reinforcement learning framework to refine expert activation distributions. It imposes contrastive constraints to maximize consistency of expert activations within the same terrain while minimizing similarity across different terrains, encouraging experts to specialize in distinct terrain types.

Result: Validated on Unitree G1 humanoid robot, CMoE enables traversal of continuous steps up to 20 cm high and gaps up to 80 cm wide, achieving robust and natural gait across diverse mixed terrains, surpassing existing methods.

Conclusion: CMoE effectively addresses the expert specialization problem in MoE frameworks for terrain navigation, enabling humanoid robots to handle challenging real-world terrain transitions. Code is released publicly to support further research.
...
### [148] [RL-Based Coverage Path Planning for Deformable Objects on 3D Surfaces](https://arxiv.org/abs/2603.03137)
*Yuhang Zhang,Jinming Ma,Feng Wu*

Main category: cs.RO

TL;DR: A reinforcement learning method using harmonic UV mapping and scaled grouped convolutions to train robots for deformable object wiping tasks in simulation, then deployed on real hardware.


<details>
  <summary>Details</summary>
Motivation: Current manipulation research for deformable objects focuses on tasks like folding clothes but lacks development for contact-rich tasks like wiping surfaces. Humans use both vision and tactile feedback for such tasks, but current algorithms struggle with occlusion and tactile perception is still evolving. Surface covering tasks require both perception and precise robotic manipulation.

Method: Train reinforcement learning agent in simulator for deformable object wiping tasks. Use harmonic UV mapping to simplify object surface state representation, process contact feedback on 2D feature maps, and employ scaled grouped convolutions (SGCNN) for efficient feature extraction. Agent outputs actions in reduced-dimensional action space to generate coverage paths.

Result: Method outperforms previous approaches in key metrics including total path length and coverage area. Successfully deployed on Kinova Gen3 manipulator to perform wiping experiments on torso model back, validating feasibility.

Conclusion: The proposed approach effectively addresses deformable object wiping tasks using simulation-based reinforcement learning with efficient state representation and feature extraction, demonstrating practical applicability on real robotic hardware.
...
### [149] [Look Forward to Walk Backward: Efficient Terrain Memory for Backward Locomotion with Forward Vision](https://arxiv.org/abs/2603.03138)
*Shixin Luo,Songbo Li,Yuan Hao,Yaqi Wang,Jun Zheng,Jun Wu,Qiuguo Zhu*

Main category: cs.RO

TL;DR: LF2WB enables legged robots to walk backward on complex terrain using forward-facing depth cameras by creating and using a terrain memory from forward motion.


<details>
  <summary>Details</summary>
Motivation: When legged robots walk backward, forward-only cameras provide no preview, making purely proprioceptive controllers insufficient for complex terrain and obstacle avoidance.

Method: LF2WB uses forward egocentric depth and proprioception to write a compact associative memory during forward motion, then retrieves it for backward locomotion without rearward vision. It employs delta-rule selective update for memory management.

Result: The method improves backward agility across complex terrains under limited sensing in both simulations and real-world scenarios, enabling collision-free backward locomotion.

Conclusion: LF2WB provides an efficient terrain-memory framework that enables legged robots to perform backward locomotion on complex terrain using only forward-facing sensors, suitable for low-cost onboard processors.
...
### [150] [From Language to Action: Can LLM-Based Agents Be Used for Embodied Robot Cognition?](https://arxiv.org/abs/2603.03148)
*Shinas Shaji,Fabian Huppertz,Alex Mitrevski,Sebastian Houben*

Main category: cs.RO

TL;DR: LLM-based cognitive architecture for robotic agents shows potential for planning and reasoning but has limitations in task execution and instruction following.


<details>
  <summary>Details</summary>
Motivation: Robotic agents need cognitive capabilities for flexible everyday action, and LLMs show emergent reasoning abilities, but bridging high-level language to low-level robotic control remains challenging.

Method: Proposed cognitive architecture with agentic LLM as core for planning/reasoning, supported by working/episodic memories for learning. Implemented in simulated household environment with high-level tools for perception, navigation, grasping, and placement.

Result: LLM-driven agent can complete structured tasks and shows emergent adaptation and memory-guided planning, but suffers from hallucinations about task success and poor instruction following for sequential tasks.

Conclusion: LLMs show potential as embodied cognitive controllers but face significant challenges including hallucinations and instruction following limitations that need to be addressed.
...
TL;DR: ACE-Brain-0 is a generalist foundation brain that unifies spatial reasoning, autonomous driving, and embodied manipulation within a single multimodal large language model using spatial intelligence as a universal scaffold across diverse embodiments.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to training unified models across diverse embodiments (autonomous driving, robotics, UAVs) face challenges with long-tail data, gradient interference, and catastrophic forgetting, making it difficult to balance universal generalization with domain-specific proficiency.

Method: Proposes the Scaffold-Specialize-Reconcile (SSR) paradigm: 1) establishes shared spatial foundation, 2) cultivates domain-specialized experts, 3) harmonizes them through data-free model merging. Uses Group Relative Policy Optimization (GRPO) to strengthen comprehensive capability.

Result: Achieves competitive and state-of-the-art performance across 24 spatial and embodiment-related benchmarks, demonstrating effective unification of spatial reasoning, autonomous driving, and embodied manipulation.

Conclusion: Spatial intelligence serves as a universal scaffold for cross-embodiment transfer, enabling a single multimodal large language model to effectively handle diverse physical embodiments while maintaining both universal generalization and domain-specific proficiency.
...
TL;DR: HoMMI is a framework that learns whole-body mobile manipulation from human demonstrations using robot-free data collection with egocentric sensing and cross-embodiment policy design.


<details>
  <summary>Details</summary>
Motivation: To enable scalable learning of whole-body mobile manipulation tasks directly from human demonstrations without requiring robot hardware during data collection, addressing the challenge of bridging the human-to-robot embodiment gap when using egocentric sensing.

Method: Augments UMI interfaces with egocentric sensing for global context capture, then uses cross-embodiment hand-eye policy design with: 1) embodiment-agnostic visual representations, 2) relaxed head action representations, and 3) a whole-body controller that coordinates motion under robot constraints.
...
Conclusion: HoMMI provides an effective framework for scalable whole-body mobile manipulation learning by bridging the embodiment gap between human demonstrations and robot execution through careful cross-embodiment policy design.
...
TL;DR: Tether enables autonomous robot learning through structured play using semantic keypoint correspondences and vision-language models, achieving data-efficient multi-task learning from minimal demonstrations.


<details>
  <summary>Details</summary>
Motivation: Robotics needs scalable alternatives to labor-intensive human demonstrations by enabling robots to learn from interaction and experience, requiring policies robust to diverse states and procedures for continuous useful experience generation.

Method: Two-part approach: (1) Open-loop policy that warps actions from few source demonstrations using semantic keypoint correspondences for robustness; (2) Autonomous play cycle of task selection, execution, evaluation, and improvement guided by vision-language models.

Result: First to achieve many hours of autonomous multi-task play in real-world household setup from only handful of demonstrations, generating over 1000 expert-level trajectories and training policies competitive with human-collected demonstration policies.

Conclusion: Tether demonstrates effective autonomous functional play for robotics, enabling continuous data generation that improves closed-loop imitation policies over time with minimal human intervention.
...
TL;DR: ULTRA: A unified framework for autonomous humanoid whole-body loco-manipulation using physics-driven neural retargeting and multimodal controllers that work from sparse task specifications and noisy perception.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for humanoid loco-manipulation are constrained by scarce/low-quality retargeted data, difficulty scaling to large skill repertoires, and reliance on tracking predefined motion references rather than generating behavior from perception and high-level tasks.

Method: Two key components: 1) Physics-driven neural retargeting algorithm that translates motion capture to humanoid embodiments while preserving physical plausibility for contact-rich interactions. 2) Unified multimodal controller supporting both dense references and sparse task specifications, working with sensing ranging from accurate motion-capture state to noisy egocentric visual inputs. Uses distillation of universal tracking policy, compression of motor skills into compact latent space, and reinforcement learning finetuning.

Result: ULTRA generalizes to autonomous, goal-conditioned whole-body loco-manipulation from egocentric perception, consistently outperforming tracking-only baselines with limited skills. Evaluated in simulation and on real Unitree G1 humanoid.

Conclusion: ULTRA enables coordinated whole-body behavior from sparse intent without test-time reference motions, addressing key limitations in existing humanoid loco-manipulation approaches and advancing toward practically useful humanoid robots.
...
TL;DR: A two-stage learning framework for contact-rich manipulation tasks with implicit success criteria, using peeling as an example. Combines imitation learning with preference-based finetuning to achieve over 90% success rates and strong generalization across produce categories.


<details>
  <summary>Details</summary>
Motivation: Many essential manipulation tasks like food preparation, surgery, and craftsmanship remain challenging for robots due to contact-rich dynamics and implicit success criteria that are continuous and subjective, making quantitative evaluation and reward engineering difficult.

Method: Two-stage pipeline: 1) Learn initial policy via force-aware data collection and imitation learning for generalization across object variations; 2) Refine policy through preference-based finetuning using a learned reward model that combines quantitative task metrics with qualitative human feedback.

Result: Using only 50-200 peeling trajectories, achieves over 90% average success rates on challenging produce (cucumbers, apples, potatoes), with performance improving by up to 40% through preference-based finetuning. Shows strong zero-shot generalization to unseen instances and out-of-distribution produce while maintaining over 90% success rates.

Conclusion: The framework successfully addresses contact-rich manipulation tasks with implicit success criteria by combining imitation learning with human preference alignment, enabling effective learning from limited data while achieving robust generalization across object variations.
...
TL;DR: Federated Inference (FI) enables independently trained models to collaborate at inference time without sharing data or parameters, requiring privacy preservation and performance gains, with unique system-level behaviors distinct from federated learning or classical ensembles.


<details>
  <summary>Details</summary>
Motivation: There is a lack of unified abstraction and system-level understanding of Federated Inference (FI), which studies how independently trained and privately owned models can collaborate at inference time without sharing data or model parameters. While recent work has explored secure and distributed inference from different perspectives, a comprehensive framework is needed.

Method: The paper formalizes FI as a protected collaborative computation, analyzes its core design dimensions, and examines structural trade-offs involving privacy constraints, non-IID data, and limited observability at inference time. It includes a concrete instantiation and empirical analysis to highlight friction points in privacy-preserving inference, ensemble-based collaboration, and incentive alignment.

Result: The analysis reveals that FI exhibits system-level behaviors that cannot be directly inherited from training-time federation or classical ensemble methods. The work identifies recurring friction points in privacy-preserving inference, ensemble-based collaboration, and incentive alignment when implementing FI systems.

Conclusion: This work provides a unifying perspective on Federated Inference as a distinct collaborative paradigm complementary to federated learning, and outlines open challenges that must be addressed to enable practical, scalable, and privacy-preserving collaborative inference systems.
...
TL;DR: ERI is a comprehensive engineering instruction dataset spanning 9 fields, 55 subdomains, 7 intent types, and 3 difficulty tiers with 57,750 records for training/evaluating engineering-capable LLMs.


<details>
  <summary>Details</summary>
Motivation: To create a taxonomy-driven benchmark for training and evaluating engineering-capable LLMs and agents that addresses circularity concerns in existing benchmarks through rigorous validation protocols.

Method: Developed a dataset with structured taxonomy (9 fields × 55 subdomains × 7 intent types × 3 difficulty tiers), implemented convergent validation protocol using cross-provider independence, multi-judge averaging, and frontier-model agreement analysis.

Result: Frontier models (GPT-5, Claude Sonnet 4, DeepSeek V3.1) achieved mean scores >4.30/5, showing statistically significant three-tier performance structure with mid-tier/smaller models exhibiting higher failure rates and steeper degradation on graduate-level questions. Hallucination risk bounded to 1.7%.

Conclusion: ERI provides a validated benchmark for reproducible comparisons and regression testing in engineering LLM applications, enabling instruction tuning, routing, retrieval-augmented evaluation, and agentic tool-use workflows.
...
TL;DR: SuperLocalMemory is a local-first memory system for multi-agent AI that defends against memory poisoning attacks through architectural isolation and Bayesian trust scoring, while personalizing retrieval with adaptive learning-to-rank, all running locally without cloud dependencies.
...
Result: Achieves 10.6ms median search latency, zero concurrency errors under 10 simultaneous agents, trust separation (gap=0.90) with 72% trust degradation for sleeper attacks, and 104% improvement in NDCG@5 when adaptive re-ranking is enabled.

Conclusion: SuperLocalMemory provides a secure, high-performance local memory system for multi-agent AI that defends against memory poisoning while personalizing retrieval, with GDPR compliance and integration with 17+ development tools via Model Context Protocol.
...
TL;DR: DICE-DML: A framework using generative AI to estimate causal effects of visual attributes in images by disentangling treatments from confounders through deepfake-generated image pairs and adversarial learning.


<details>
  <summary>Details</summary>
Motivation: Marketers lack rigorous methods for understanding how specific visual attributes (like skin tone) causally affect consumer engagement in digital advertising. Standard causal inference methods fail because vision encoders entangle treatment information with confounding variables within images.

Method: DICE-DML combines three mechanisms: (1) deepfake-generated image pairs that isolate treatment variation, (2) DICE-Diff adversarial learning on paired difference vectors where background signals cancel to reveal pure treatment fingerprints, and (3) orthogonal projection that geometrically removes treatment-axis components.

Result: In simulations, DICE-DML reduces root mean squared error by 73-97% compared to standard DML, with 97.5% improvement at null effect point. Applied to 232,089 Instagram posts, DICE-DML achieves valid confounding control (R² = 0.63) and estimates a marginally significant negative effect of darker skin tone (-522 likes; p = 0.062), while standard DML produces invalid results.

Conclusion: DICE-DML provides a principled approach for causal inference with visual data when treatments and confounders coexist within images, enabling rigorous estimation of causal effects of visual attributes in digital advertising.
...
TL;DR: AI systems can realize uncertainty states through different architectures, distinguishing between epistemic (data-based) and subjective (system attitude) uncertainty, with some uncertainty states being interrogative attitudes with question content.


<details>
  <summary>Details</summary>
Motivation: To investigate whether and how AI systems can realize states of uncertainty, examining different AI architectures' capacity to represent uncertainty from functionalist and behavioral perspectives.

Method: Adopts functionalist and behavioral perspective to analyze symbolic, connectionist, and hybrid AI architectures; distinguishes between epistemic vs. subjective uncertainty and distributed vs. discrete realizations of subjective uncertainty.

Result: AI systems can realize uncertainty states; key contribution is identifying that some uncertainty states are interrogative attitudes whose content is a question rather than a proposition.

Conclusion: AI systems can represent uncertainty through various architectural approaches, with important distinctions between types of uncertainty and novel understanding of uncertainty as interrogative attitudes with question content.
...
TL;DR: COOL-MC tool combines RL with model checking to verify and explain platelet inventory management policies, achieving 2.9% stockout and 1.1% wastage probabilities.


<details>
  <summary>Details</summary>
Motivation: Platelets have short shelf life (5 days), creating inventory challenges between costly wastage and life-threatening shortages. RL can learn effective policies but remains a black box, hindering trust in safety-critical healthcare domains.

Method: Apply COOL-MC tool combining reinforcement learning with probabilistic model checking and explainable RL. Construct policy-induced discrete-time Markov chain (including only reachable states to reduce memory), verify PCTL properties, and provide feature-level explanations including action reachability and counterfactual analysis.

Result: Trained policy achieves 2.9% stockout probability and 1.1% inventory-full (wastage) probability within 200-step horizon. Policy primarily attends to age distribution of inventory rather than day of week or pending orders. Action reachability shows diverse replenishment strategy with most order quantities reached quickly, some never selected. Counterfactual analysis shows replacing medium-large orders with smaller ones leaves safety probabilities nearly unchanged.

Conclusion: First formal verification and explanation of RL platelet inventory management policy demonstrates COOL-MC's value for transparent, auditable decision-making in safety-critical healthcare supply chain domains.
...
<details>
  <summary>Details</summary>
Motivation: Real-world multimodal knowledge graphs (MKGs) are heterogeneous with diverse modalities, but traditional KGE methods are unimodal and recent multimodal approaches have limitations like weak cross-modal alignment and simplistic assumptions about modality availability.

Method: Proposes Vision-Language Knowledge Graph Embeddings (VL-KGE), a framework that integrates cross-modal alignment from Vision-Language Models (VLMs) with structured relational modeling to learn unified multimodal representations of knowledge graphs.
...
Conclusion: Vision-Language Models provide valuable capabilities for multimodal KGE, enabling more robust and structured reasoning over large-scale heterogeneous knowledge graphs.
...
TL;DR: Memory retrieval methods dominate performance in LLM agents, not memory writing strategies. Raw chunk storage (no LLM calls) often beats sophisticated summarization methods.


<details>
  <summary>Details</summary>
Motivation: To understand the relative importance of memory writing vs. retrieval in memory-augmented LLM agents, as current approaches unclear about which component matters more for performance.

Method: 3x3 diagnostic framework crossing three write strategies (raw chunks, Mem0 fact extraction, MemGPT summarization) with three retrieval methods (cosine, BM25, hybrid reranking), tested on LoCoMo benchmark.

Result: Retrieval method is dominant factor: 20-point accuracy difference across retrieval methods (57.1% to 77.2%) vs only 3-8 points across write strategies. Raw chunk storage (zero LLM calls) matches or outperforms expensive lossy alternatives.

Conclusion: Current memory pipelines may discard useful context that retrieval can't compensate for. Improving retrieval quality yields larger gains than increasing write-time sophistication under current practices.
...
TL;DR: PRISM introduces a Process Reward Model-guided inference algorithm that uses step-level verification to improve DEEPTHINK reasoning systems by guiding population refinement and solution aggregation, achieving state-of-the-art performance on mathematical and scientific benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing DEEPTHINK frameworks lack reliable correctness signals during inference, creating a population-enhancement bottleneck where deeper deliberation amplifies errors, suppresses correct minority solutions, and yields weak returns to additional compute.

Method: PRISM uses a Process Reward Model (PRM) for step-level verification to guide both population refinement and solution aggregation. It treats candidate solutions as particles in a PRM-defined energy landscape and reshapes the population through score-guided resampling and stochastic refinement, concentrating probability mass on higher-quality reasoning while preserving diversity.

Result: PRISM achieves 90.0% on AIME25, 75.4% on HMMT25, and 71.4% on GPQA Diamond with gpt-oss-20b, matching or exceeding performance of gpt-oss-120b. It produces consistent net-directional correction during refinement, remains reliable with few initial correct candidates, and often lies on the compute-accuracy Pareto frontier.

Conclusion: PRISM addresses the population-enhancement bottleneck in DEEPTHINK systems through PRM-guided inference with step-level verification, enabling more effective reasoning refinement and aggregation while achieving strong performance across mathematical and scientific tasks.
...
TL;DR: Social planner selectively reveals role model labels in a network to maximize welfare, with algorithms, fairness guarantees, and experiments on real datasets.


<details>
  <summary>Details</summary>
Motivation: Agents emulate role models in social networks but don't know their true labels (positive/negative). A social planner with limited budget wants to reveal labels strategically to maximize social welfare (agents emulating positive role models).

Method: 1) Basic model: Planner selectively reveals role model labels under budget constraints. 2) Introduces proxy welfare function to handle non-submodularity from negative role model revelations. 3) Extensions: direct intervention connecting high-risk agents to positive role models, and coverage radius model expanding visibility. 4) Theoretical analysis: algorithms, hardness results, sample complexity, fairness guarantees for different groups.

Result: 1) Constant-factor approximation algorithm when each agent has constant number of negative neighbors. 2) Fairness guarantee: each group's welfare gain within constant factor of optimum if full budget allocated to that group. 3) Extensive experiments on four real-world datasets support theoretical results and show algorithm effectiveness.

Conclusion: The paper provides algorithmic solutions for strategic label revelation in social networks to maximize welfare, with theoretical guarantees and practical effectiveness demonstrated on real data. The approach handles technical challenges like non-submodularity and provides fairness considerations across different groups.
...
TL;DR: NeuroProlog is a neurosymbolic framework that compiles math word problems into executable Prolog programs with formal verification guarantees, using multi-task cocktail training to improve mathematical reasoning in LLMs.


<details>
  <summary>Details</summary>
Motivation: LLMs perform well on natural language tasks but remain unreliable in mathematical reasoning, often generating fluent but logically inconsistent solutions that lack verifiable guarantees.

Method: Proposes NeuroProlog framework with multi-task Cocktail training that jointly optimizes three objectives: mathematical formula-to-rule translation (KB), natural language-to-program synthesis (SOLVE), and program-answer alignment in a unified symbolic representation space. Uses execution-guided decoding with fine-grained error taxonomy for iterative program repair.

Result: Significant accuracy improvements on GSM8K across four model scales: +5.23% (Qwen-32B), +3.43% (GPT-OSS-20B), and +5.54% (Llama-3B) over single-task baselines. Systematic error analysis reveals scale-dependent learning dynamics and improved error correction capabilities.

Conclusion: NeuroProlog's neurosymbolic approach with multi-task cocktail training enables verifiable mathematical reasoning in LLMs, with symbolic grounding improving compositional reasoning and execution-guided decoding enabling effective program repair, though performance depends on model scale.
...
TL;DR: LLM-MLFFN: A novel LLM-enhanced multi-level feature fusion network for autonomous vehicle driving behavior classification, achieving 94%+ accuracy by combining numerical and semantic features.


<details>
  <summary>Details</summary>
Motivation: Existing AV behavior classification approaches rely too much on numerical time-series modeling, lacking semantic abstraction, which limits interpretability and robustness in complex traffic environments.

Method: Three-component framework: (1) multi-level feature extraction (statistical, behavioral, dynamic), (2) LLM-based semantic description module, (3) dual-channel multi-level feature fusion network with weighted attention mechanisms.
...
TL;DR: LLMs show unified general capability but fail at simple human tasks; NeuroCognition benchmark uses neuropsychological tests to reveal gaps in core cognitive abilities beyond standard benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current LLM benchmarks focus on task completion but fail to assess foundational cognitive abilities, explaining why models excel on complex tasks but struggle with simple human-like reasoning. There's a need for benchmarks that probe core cognitive functions to better understand LLM capabilities and limitations.

Method: Introduced NeuroCognition benchmark based on three adapted neuropsychological tests: Raven's Progressive Matrices (abstract relational reasoning), Spatial Working Memory (maintenance and systematic search), and Wisconsin Card Sorting Test (cognitive flexibility). Evaluated 156 models across these cognitive dimensions.

Result: Models perform well on text but degrade with images and increased complexity. Complex reasoning isn't universally beneficial, while simple human-like strategies yield partial gains. NeuroCognition correlates positively with standard benchmarks but measures distinct cognitive abilities beyond them.

Conclusion: NeuroCognition reveals where LLMs align with human-like intelligence and where they lack core adaptive cognition, serving as a verifiable, scalable tool for improving LLMs by targeting foundational cognitive abilities.
...
TL;DR: AnchorDrive: A two-stage framework using LLMs for semantic control and diffusion models for realism to generate safety-critical driving scenarios.


<details>
  <summary>Details</summary>
Motivation: Safety-critical scenarios for autonomous driving evaluation are rare in real-world data and difficult to collect, while existing simulation methods lack both controllability and realism.

Method: Two-stage framework: 1) LLM as driver agent with plan assessor for semantically controllable scenario generation, 2) Diffusion model guided by LLM-extracted anchor points to regenerate realistic trajectories while preserving intent.

Result: Experiments on highD dataset show superior performance in criticality, realism, and controllability compared to existing methods.

Conclusion: AnchorDrive effectively generates controllable and realistic safety-critical driving scenarios by leveraging complementary strengths of LLMs and diffusion models.
...
TL;DR: LiveAgentBench is a new benchmark with 104 real-world scenarios for evaluating AI agents, created using Social Perception-Driven Data Generation from social media and product questions.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to accurately represent real-world user tasks, creating a gap in evaluating practical AI agent performance.

Method: Social Perception-Driven Data Generation (SPDG) method constructs benchmark from publicly sourced social media questions and real-world product queries, ensuring real-world relevance, task complexity, and result verifiability.

Result: Benchmark includes 374 tasks (125 validation, 249 testing) across 104 scenarios; evaluation reveals practical performance of various models, frameworks, and commercial products.

Conclusion: LiveAgentBench addresses limitations of existing benchmarks and enables continuous updates with fresh real-world queries for ongoing evaluation of AI agents.
...
<details>
  <summary>Details</summary>
Motivation: Current multi-model LLM serving suffers from inefficient decode execution due to model-specific resource partitioning, where cross-model batching is impossible, leading to severe GPU underutilization especially under skewed workloads.

Method: Decomposes decoder-only Transformers into prefill and decode modules, fine-tunes only task-specific prefill modules while keeping decode modules frozen and shared across models. Enables model-agnostic decode routing to balance requests across shared workers. Also introduces Quantized SUN (QSUN) for low-bit decoding.

Result: Achieves accuracy comparable to full fine-tuning while improving throughput per GPU by up to 2.0x over conventional disaggregation, keeping time-per-output-token within 5%. QSUN achieves 45% speedup with comparable accuracy to SUN while preserving shared decoding benefits.

Conclusion: SUN enables efficient cross-model sharing of decode execution in multi-LLM serving, significantly improving GPU utilization and throughput while maintaining accuracy, with QSUN further enhancing performance through quantization.
...
TL;DR: AgentAssay is a token-efficient framework for regression testing AI agents with statistical guarantees, achieving 78-100% cost reduction through behavioral fingerprinting, adaptive optimization, and trace-first analysis.


<details>
  <summary>Details</summary>
Motivation: There's no principled methodology for verifying that AI agents haven't regressed after changes to their prompts, tools, models, or orchestration logic, despite their unprecedented deployment scale.

Method: AgentAssay uses stochastic three-valued verdicts (PASS/FAIL/INCONCLUSIVE) with hypothesis testing, five-dimensional agent coverage metrics, agent-specific mutation testing operators, metamorphic relations for workflows, CI/CD deployment gates as statistical decision procedures, behavioral fingerprinting mapping execution traces to compact vectors, adaptive budget optimization calibrating trial counts to behavioral variance, and trace-first offline analysis.
...
Conclusion: AgentAssay provides the first token-efficient framework for regression testing non-deterministic AI agent workflows with rigorous statistical guarantees, enabling cost-effective verification of agent changes.
...
TL;DR: V-GEMS is a multimodal agent architecture that combines visual grounding with explicit memory to improve web navigation by preventing spatial disorientation and navigation loops.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based agents struggle with spatial disorientation and navigation loops during autonomous web navigation, needing better visual perception and long-term context maintenance.

Method: Proposes V-GEMS architecture integrating visual grounding to resolve ambiguous interactive elements and an explicit memory stack with state tracking for structured path mapping, enabling valid backtracking and preventing cyclical failures.

Result: V-GEMS significantly dominates the WebWalker baseline with a 28.7% performance gain, demonstrating superior web navigation capabilities.

Conclusion: V-GEMS provides a robust multimodal agent architecture for precise and resilient web traversal through visual grounding and explicit memory mechanisms, with an updatable dynamic benchmark for rigorous evaluation.
...
TL;DR: SorryDB is a dynamic benchmark of open Lean tasks from real GitHub projects that continuously updates to align with community needs and prevent test-set contamination, showing current approaches are complementary with agentic methods performing best but not strictly dominating.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for formal mathematics are static and often composed of competition problems, which don't reflect real-world community needs. There's a need for benchmarks that align with actual mathematical formalization projects, understand complex dependencies, and provide continuously updated tasks to prevent test-set contamination.

Method: Created SorryDB - a dynamically-updating benchmark of open Lean tasks drawn from 78 real-world formalization projects on GitHub. Evaluated multiple approaches on a snapshot of 1000 tasks: generalist large language models, agentic approaches, specialized symbolic provers, and curated Lean tactics.

Result: Current approaches are complementary - no single method dominates. Agentic approach based on Gemini Flash performed best but wasn't strictly better than other methods. The benchmark successfully provides a robust metric for agents' ability to contribute to novel formal mathematics projects.

Conclusion: SorryDB offers a more realistic and continuously updated benchmark that better aligns with community needs than static competition-based benchmarks. The complementary nature of different approaches suggests value in hybrid methods for formal mathematics automation.
...
<details>
  <summary>Details</summary>
Motivation: LLMs have limitations in high-frequency decision tasks where existing methods designed for low-frequency, discrete embodied scenarios fail due to frequent numerical state updates with minimal fluctuations and policy misalignment between sub-tasks and composite tasks.

Method: 1) Normalized Action Reward: Acquire dense rewards from environmental feedback of candidate actions via reward functions, then normalize rewards (theoretically proven not to impair optimal policy). 2) Consistency Policy Optimization: Use LLMs to infer sub-observation candidate actions and generate joint policies, with consistency loss ensuring alignment between global semantic policies and sub-semantic policies.
...
Conclusion: NAR-CP effectively addresses LLM limitations in high-frequency decision-making by combining normalized action rewards with consistency policy optimization, enabling better performance and generalization in complex, fast-paced environments.
...
TL;DR: Robots need to actively retrieve visual procedural knowledge from external documents to perform complex tasks in zero-shot settings, moving beyond just using internal memory or text-based rules.


<details>
  <summary>Details</summary>
Motivation: Current robots lack the ability to acquire unseen procedural knowledge from external documentation when faced with novel tasks in zero-shot settings. Existing approaches focus on retrieving past trajectories or safety rules, but fail to address the core need for active task construction from visual procedural manuals.
...
Result: The approach significantly outperforms baselines relying on zero-shot reasoning or few-shot example retrieval on a challenging long-horizon assembly benchmark, demonstrating the effectiveness of grounding robotic planning in retrieved visual documents.

Conclusion: Establishes the basis for Retrieval-Augmented Robotics, extending Information Retrieval from answering user queries to driving embodied physical actions, enabling robots to evolve from passive executors into active information retrieval users.
...
TL;DR: Proposes FinTexTS, a semantic-based multi-level pairing framework for constructing text-paired stock price datasets that captures complex financial interdependencies beyond simple keyword matching.


<details>
  <summary>Details</summary>
Motivation: Financial markets have complex interdependencies where stock prices are influenced by company-specific events, events in other companies, and macroeconomic factors. Existing text-pairing methods using simple keyword matching fail to capture these complex relationships.

Method: 1) Extract company-specific context from SEC filings, 2) Use embedding-based matching to retrieve semantically relevant news articles, 3) Classify news into four levels (macro, sector, related company, target company) using LLMs for multi-level pairing, 4) Apply framework to construct FinTexTS dataset.

Result: Constructed FinTexTS, a large-scale text-paired stock price dataset. Experimental results show effectiveness of semantic-based multi-level pairing strategy in stock price forecasting. Method also works with proprietary news sources, yielding higher-quality data and improved forecasting performance.

Conclusion: The proposed semantic-based multi-level pairing framework successfully captures complex financial interdependencies and improves stock price forecasting by better aligning textual information with time-series data across multiple relationship levels.
...
TL;DR: Researchers developed a multi-agent LLM-based platform to study affective polarization in social media, addressing limitations of real-world studies and manual data labeling through simulated virtual communities.


<details>
  <summary>Details</summary>
Motivation: Current studies of affective polarization face limitations: real-world studies have limited scope, simulated studies lack high-quality training data due to labor-intensive manual labeling and subjective biases, and there's a lack of tools to formalize different definitions of affective polarization across studies, complicating result comparison and framework interoperability.

Method: Developed a multi-agent model platform leveraging large language models (LLMs) to construct virtual communities where agents engage in discussions. The platform operationalizes a comprehensive framework for studying affective polarization in social media.

Result: The platform demonstrates potential by: (1) analyzing affective polarization questions from social science literature with fresh perspectives, and (2) introducing scenarios that allow observation and measurement of polarization at different granularity and abstraction levels. Experiments show it's a flexible tool for computational studies of complex social dynamics.

Conclusion: The LLM-based multi-agent platform provides a flexible tool for computational studies of affective polarization, leveraging advanced agent models to simulate rich, context-sensitive interactions and systematically explore research questions traditionally addressed through human-subject studies.
...
TL;DR: EvoSkill is a self-evolving framework that automatically discovers and refines agent skills through iterative failure analysis, improving performance on specialized tasks without modifying the underlying model.


<details>
  <summary>Details</summary>
Motivation: Coding agents lack domain expertise for specialized tasks, and current skill development approaches are either hand-crafted or optimize low-level artifacts that are tightly coupled to specific models and tasks.

Method: EvoSkill analyzes execution failures, proposes new skills or edits to existing ones, materializes them into structured skill folders, and uses a Pareto frontier for selection to retain only skills that improve validation performance while keeping the model frozen.

Result: On OfficeQA, EvoSkill improved exact-match accuracy by 7.3% (60.6% → 67.9%); on SealQA, it yielded a 12.1% gain (26.6% → 38.7%). Skills evolved on one task transferred zero-shot to another, improving accuracy by 5.3% without modification.

Conclusion: EvoSkill demonstrates that automated skill evolution can significantly improve agent performance on specialized tasks, and that skill-level optimization produces transferable capabilities beyond the training task.
...
TL;DR: BehaveSim measures algorithmic similarity using problem-solving behavior trajectories instead of code syntax, enabling better algorithm analysis and improving LLM-based automated algorithm design.


<details>
  <summary>Details</summary>
Motivation: With LLM-based automated algorithm design generating code where algorithmic principles are implicitly embedded, there's a need to distinguish genuine algorithmic innovation from mere syntactic variations. Existing code similarity metrics fail to capture algorithmic similarity as they focus on surface-level syntax or output equivalence rather than underlying algorithmic logic.

Method: BehaveSim measures algorithmic similarity through problem-solving behavior as sequences of intermediate solutions produced during execution (problem-solving trajectories or PSTrajs). It quantifies alignment between PSTrajs using dynamic time warping (DTW) to distinguish algorithms with divergent logic despite syntactic or output-level similarities.
...
Conclusion: BehaveSim provides a crucial tool for the growing ecosystem of AI-generated algorithms by enabling algorithmic similarity assessment based on problem-solving behavior rather than code syntax, addressing a fundamental limitation in current LLM-based automated algorithm design frameworks.
...
TL;DR: A framework for reproducible benchmarking of logical reasoning agents using assessor agents to manage tasks, budgets, and failure logging, demonstrated with an auto-formalization agent achieving 86.70% accuracy on FOLIO.


<details>
  <summary>Details</summary>
Motivation: Need for reproducible, auditable, and robust assessment of logical reasoning agents that can handle execution failures systematically.

Method: Assessor agent framework that issues tasks, enforces execution budgets, parses outputs, and records structured failure types; auto-formalization agent translates natural language to Z3Py programs and uses SMT solving for logical entailment.

Result: Auto-formalization agent achieves 86.70% accuracy on cleaned FOLIO validation set, outperforming chain-of-thought baseline (73.89%).

Conclusion: The assessor agent framework enables reproducible benchmarking of reasoning agents, and the auto-formalization approach shows strong performance on logical entailment tasks.
...
TL;DR: GLEAN is a framework for verifying LLM-powered agents' decisions by compiling expert-curated protocols into trajectory-informed, calibrated correctness signals, with active verification for uncertain cases.


<details>
  <summary>Details</summary>
Motivation: As LLM-powered agents are used for high-stakes decision-making like clinical diagnosis, there's a critical need for reliable verification to ensure trustworthy deployment. Existing verifiers underperform due to lack of domain knowledge and limited calibration.

Method: GLEAN uses Guideline-grounded Evidence Accumulation to compile expert protocols into correctness signals. It evaluates step-wise alignment with domain guidelines, aggregates multi-guideline ratings into surrogate features, accumulates evidence along trajectories, and calibrates into correctness probabilities using Bayesian logistic regression. For uncertain cases, it triggers active verification by expanding guideline coverage and performing differential checks.

Result: Empirical validation on agentic clinical diagnosis across three diseases from MIMIC-IV dataset shows GLEAN surpasses best baseline by 12% in AUROC and 50% in Brier score reduction, confirming effectiveness in both discrimination and calibration. Expert study with clinicians recognizes GLEAN's practical utility.

Conclusion: GLEAN provides an effective framework for verifying LLM-powered agents' decisions in high-stakes domains like healthcare, addressing limitations of existing verifiers through domain knowledge integration, trajectory-informed evidence accumulation, and calibrated uncertainty estimation with active verification.
...
TL;DR: A framework combining argument mining, quantitative reasoning, and ontology-based querying to enable transparent, verifiable reasoning over debate texts, overcoming LLM limitations.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with explicit, transparent, and verifiable reasoning over complex debate texts, lacking structured representations of argument relationships and strengths that determine overall acceptability.

Method: Three-step framework: 1) Extract fuzzy argumentative knowledge base from raw debate text with arguments as entities, attack/support relations, and initial fuzzy strengths; 2) Apply quantitative argumentation semantics to compute final argument strengths via propagation; 3) Embed results into fuzzy description logic for expressive query answering using efficient rewriting techniques.

Result: The approach provides a transparent, explainable, and formally grounded method for analyzing debates that overcomes purely statistical LLM-based analyses.

Conclusion: The proposed framework addresses LLM limitations in debate analysis by integrating learning-based argument mining with quantitative reasoning and ontology-based querying, enabling verifiable and structured reasoning over complex argumentative texts.
...
TL;DR: Hybrid Transformer-SSM models match/exceed Transformers in data efficiency for information-dense retrieval but Transformers remain superior for position retrieval tasks.


<details>
  <summary>Details</summary>
Motivation: Transformers have strong in-context retrieval but quadratic complexity, while SSMs offer linear-time processing but limited retrieval. Need to investigate whether hybrid architectures can combine the best of both worlds.

Method: Evaluate Transformers, SSMs, and hybrid architectures on two synthetic in-context retrieval tasks: n-gram retrieval (identify/reproduce n-gram after query) and position retrieval (two-hop associative lookup to output positional index). Assess data efficiency, length generalization, robustness, and learned representations.

Result: Hybrid models outperform SSMs and match/exceed Transformers in data efficiency and extrapolation for information-dense context retrieval. However, Transformers maintain superiority in position retrieval tasks. SSM-based models develop locality-aware embeddings with interpretable structures absent in Transformers.

Conclusion: Hybrid architectures can achieve best of both worlds for certain retrieval tasks but have limitations. SSMs develop unique locality-aware embeddings explaining their strengths/limitations. Findings provide principled guidance for architecture selection and reveal fundamental differences in how models learn positional associations.
...
TL;DR: The paper proposes STS, an SAE-based metric that predicts how well fine-tuned LLMs will transfer to new domains before actual fine-tuning occurs.


<details>
  <summary>Details</summary>
Motivation: While post-training (fine-tuning) is crucial for adapting LLMs to specific tasks, it introduces model shifts whose transfer effects across domains are poorly understood. Current methods lack interpretable ways to predict transferability before committing to fine-tuning.

Method: STS uses sparse autoencoders (SAEs) to analyze model representations. It identifies dimensions that shift during fine-tuning and calculates their correlation with target domains, creating a transferability score that predicts performance without actual fine-tuning.

Result: Extensive experiments show STS accurately predicts supervised fine-tuning transferability, achieving Pearson correlation coefficients >0.7 with actual performance changes. The method also shows initial promise for extending to reinforcement learning scenarios.

Conclusion: STS provides an interpretable tool for guiding post-training strategies in LLMs by predicting transferability before fine-tuning, potentially saving computational resources and improving adaptation decisions.
...
TL;DR: ShipTraj-R1 is an LLM-based framework that reformulates ship trajectory prediction as text-to-text generation, using dynamic prompts and rule-based rewards with GRPO reinforcement fine-tuning.


<details>
  <summary>Details</summary>
Motivation: While reinforcement fine-tuning has improved LLM reasoning, applying LLMs to ship trajectory prediction remains unexplored despite its importance for maritime safety and navigation.

Method: 1) Design dynamic prompts with conflicting ship trajectory info for adaptive CoT reasoning; 2) Introduce comprehensive rule-based rewards for reasoning format and prediction accuracy; 3) Reinforce through GRPO mechanism using Qwen3 backbone.
...
Conclusion: The proposed ShipTraj-R1 framework successfully applies LLMs to ship trajectory prediction, demonstrating superior performance through adaptive reasoning and reinforcement fine-tuning.
...
TL;DR: The paper argues that LLMs as epistemic agents require new evaluation/governance frameworks focused on trustworthiness, alignment with human epistemic goals, and socio-epistemic infrastructure to ensure beneficial human-AI knowledge ecosystems.


<details>
  <summary>Details</summary>
Motivation: LLMs increasingly function as autonomous epistemic agents that curate information and generate advice, shaping our shared knowledge environment. Their reliability and calibration to human epistemic norms are highly consequential for individual and collective decision-making, necessitating new approaches to evaluation and governance.

Method: The paper proposes a normative framework centered on three pillars: 1) building trustworthiness of epistemic AI agents, 2) aligning AI agents with human epistemic goals, and 3) reinforcing socio-epistemic infrastructure. This includes technical systems like provenance tracking and "knowledge sanctuaries."

Result: The framework identifies key requirements for trustworthy epistemic AI agents: epistemic competence, robust falsifiability, and epistemically virtuous behaviors. It highlights the need for governance structures that address new informational interdependencies created by AI agents in knowledge ecosystems.

Conclusion: A fundamental shift in AI evaluation and governance is needed to ensure LLMs act as reliable partners in knowledge ecosystems. Well-calibrated systems could augment human judgment, while poorly aligned agents risk cognitive deskilling and epistemic drift. The proposed roadmap aims to create robust, inclusive knowledge ecosystems through trustworthy epistemic AI.
...
TL;DR: SpatialText is a diagnostic framework that isolates genuine spatial reasoning in LLMs from linguistic heuristics, revealing models' fundamental limitations in constructing flexible mental models despite proficiency with explicit spatial facts.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to distinguish genuine spatial cognition from statistical language patterns, and multimodal evaluations often conflate spatial reasoning with visual perception. There's a need to systematically investigate whether models can construct flexible spatial mental models rather than relying on linguistic co-occurrences.

Method: SpatialText uses a dual-source methodology: (1) human-annotated descriptions of real 3D indoor environments capturing natural ambiguities, perspective shifts, and functional relations; (2) code-generated, logically precise scenes designed to probe formal spatial deduction and epistemic boundaries.

Result: Models show proficiency in retrieving explicit spatial facts and operating within global coordinate systems, but exhibit critical failures in egocentric perspective transformation and local reference frame reasoning. Systematic errors indicate reliance on linguistic heuristics rather than coherent internal spatial representations.

Conclusion: Current models lack genuine spatial reasoning capabilities, relying heavily on linguistic co-occurrence patterns. SpatialText serves as a rigorous diagnostic tool for assessing the cognitive boundaries of artificial spatial intelligence and reveals fundamental representational limitations in state-of-the-art models.
...
TL;DR: A two-tier multi-model orchestration framework for scientific reasoning that dynamically constructs domain-aware pipelines with specialized expert agents and enables iterative replanning based on intermediate feedback.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent LLM frameworks are weak for scientific domains due to static prompts/roles, rigid workflows, homogeneous models, poor domain adaptation, limited reasoning flexibility, high latency, and inability to revise earlier decisions when reasoning diverges.

Method: Two-tier framework: 1) Orchestration model analyzes tasks, dynamically constructs domain-aware reasoning pipelines, instantiates specialized expert agents with tailored prompts; 2) Execution model performs steps under generated role/instruction specifications. Orchestrator iteratively updates pipeline based on intermediate feedback for dynamic replanning, role reallocation, and prompt refinement.
...
Conclusion: The framework enables robust scientific reasoning through structured heterogeneous model collaboration, is model-agnostic, supports flexible performance-efficiency trade-offs, and addresses key limitations of current multi-agent LLM systems for knowledge-intensive domains.
...
TL;DR: REGAL is a registry-driven architecture for deterministic grounding of agentic AI systems in enterprise telemetry, addressing challenges of limited model context, local semantics, and evolving interfaces through structured computation and semantic compilation.


<details>
  <summary>Details</summary>
Motivation: Enterprise engineering organizations generate massive, diverse telemetry data from various sources (version control, CI/CD, issue trackers, observability). While LLMs enable new agentic automation, grounding these agents on private telemetry faces practical challenges: limited model context windows, locally defined semantic concepts specific to each organization, and constantly evolving metric interfaces.

Method: REGAL adopts an architectural approach with two main components: (1) A Medallion ELT pipeline that produces replayable, semantically compressed "Gold" artifacts from raw telemetry, and (2) a registry-driven compilation layer that synthesizes Model Context Protocol (MCP) tools from declarative metric definitions. The registry serves as an "interface-as-code" layer that ensures alignment between tool specification and execution, prevents tool drift, and embeds governance policies at the semantic boundary.

Result: A prototype implementation and case study demonstrate the feasibility of deterministic grounding and show its implications for latency, token efficiency, and operational governance. The approach systematizes an architectural pattern for enterprise LLM grounding without proposing new learning algorithms.

Conclusion: REGAL elevates deterministic computation and semantic compilation to first-class design primitives for agentic systems, providing a systematic architectural pattern for grounding LLMs in enterprise telemetry while addressing practical challenges of context limitations, local semantics, and evolving interfaces.
...
TL;DR: TikZilla: A two-stage pipeline (SFT + RL) for generating high-quality TikZ figures from text, using a new larger dataset DaTikZ-V4 and image-based rewards, achieving GPT-4o/GPT-5 level performance with smaller 3B/8B models.
...
Method: Construct DaTikZ-V4 dataset (4x larger than V3, higher quality, LLM-generated descriptions). Train TikZilla models (3B/8B Qwen) with two-stage pipeline: 1) Supervised fine-tuning, 2) Reinforcement learning with image encoder trained via inverse graphics for semantically faithful reward signals.
...
Conclusion: The proposed approach addresses limitations of existing Text-to-TikZ methods by combining larger high-quality datasets with two-stage training (SFT+RL) using image-based rewards, achieving state-of-the-art performance with smaller models.
...
TL;DR: RAPO introduces retrieval-augmented policy optimization to enhance exploration in Agentic RL by incorporating off-policy step-level traces, achieving better performance and faster training.


<details>
  <summary>Details</summary>
Motivation: Existing Agentic RL methods rely on pure on-policy exploration, limiting discovery of new reasoning perspectives. Current off-policy approaches use full trajectories but miss fine-grained step-level exploration dynamics needed for agentic reasoning.

Method: Proposes Retrieval-Augmented Policy Optimization (RAPO) with two phases: 1) Hybrid-policy Agentic Rollout that allows agents to reason over retrieved off-policy step-level traces, and 2) Retrieval-aware Policy Optimization that calibrates policy gradient estimation with retrieval reward and importance shaping.

Result: RAPO achieves +5.0% average gain on fourteen datasets across three agentic reasoning tasks and delivers 1.2x faster training efficiency compared to existing methods.

Conclusion: Retrieval-augmented exploration significantly improves Agentic RL performance by enabling broader exploration through external behaviors while maintaining training stability, offering a promising direction for enhancing LLM agent capabilities.
...
TL;DR: PURE is a preference-aware reasoning framework for LLM-based explainable recommenders that selects multi-hop reasoning paths aligned with user preferences to generate convincing explanations, reducing preference-inconsistent justifications while maintaining factual correctness.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based explainable recommenders can produce factually correct explanations that still fail to convince users because they justify items using attributes that conflict with users' historical preferences. This preference-inconsistent reasoning is missed by standard hallucination or faithfulness metrics.

Method: PURE follows a select-then-generate paradigm with preference-aware evidence selection. It selects compact sets of multi-hop item-centric reasoning paths that are factually grounded and aligned with user preference structure, guided by user intent, specificity, and diversity. The selected evidence is injected into LLM generation via structure-aware prompting that preserves relational constraints.
...
Conclusion: Trustworthy explanations require not only factual correctness but also justification aligned with user preferences. PURE addresses this by intervening in evidence selection to ensure explanations are both factually grounded and preference-consistent.
...
TL;DR: Odin is the first production-deployed graph intelligence engine for autonomous discovery of meaningful patterns in knowledge graphs without predefined queries, using a novel COMPASS scoring metric that combines structural, semantic, temporal, and community signals.


<details>
  <summary>Details</summary>
Motivation: Current retrieval-based systems require predefined queries and suffer from the "echo chamber" problem where exploration gets trapped in dense local communities. There's a need for autonomous discovery systems that can find meaningful patterns without prior specification, especially in regulated industries like healthcare and insurance.

Method: Odin uses COMPASS (Composite Oriented Multi-signal Path Assessment) score combining: (1) structural importance via Personalized PageRank, (2) semantic plausibility through Neural Probabilistic Logic Learning (NPLL) as discriminative filter, (3) temporal relevance with configurable decay, and (4) community-aware guidance through GNN-identified bridge entities and inter-community affinity scores. Uses beam search with multi-signal guidance for efficient exploration.

Result: Beam search achieves O(b·h) complexity while maintaining high recall compared to exhaustive exploration. Odin is the first autonomous discovery system deployed in regulated production environments (healthcare and insurance), demonstrating significant improvements in pattern discovery quality and analyst efficiency with complete provenance traceability.

Conclusion: Odin successfully addresses the autonomous discovery problem in knowledge graphs by integrating multiple signals through COMPASS scoring, solving the echo chamber problem with bridge scoring mechanisms, and providing a production-ready solution with theoretical guarantees and practical deployment in regulated industries.
...
TL;DR: PAE framework evaluates LLM agents on procedural integrity, exposing that many benchmark successes are actually "corrupt successes" with hidden violations, revealing distinct failure patterns across models and structural flaws in benchmark design.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for LLM-based agents only evaluate whether tasks are completed, not how they're completed, which is insufficient for high-stakes applications where procedural integrity matters.

Method: Procedure-Aware Evaluation (PAE) formalizes agent procedures as structured observations and exposes consistency relationships between observations, communications, and executions. It evaluates agents across four axes (Utility, Efficiency, Interaction Quality, Procedural Integrity) with multi-dimensional gating that categorically disqualifies corrupt outcomes.

Result: Evaluation on tau-bench reveals: 1) Dimensions capture non-redundant failure modes, 2) 27-78% of benchmark successes are "corrupt successes" with hidden violations, 3) Gating substantially collapses Pass^4 rates and affects model rankings, 4) Distinct per-model failure signatures emerge, 5) Structural flaws in benchmark design are exposed.

Conclusion: PAE provides a more comprehensive evaluation framework that reveals critical procedural failures masked by traditional benchmarks, highlighting the need for procedure-aware evaluation in high-stakes LLM agent applications.
...
### [196] [Video TokenCom: Textual Intent-Guided Multi-Rate Video Token Communications with UEP-Based Adaptive Source-Channel Coding](https://arxiv.org/abs/2603.02470)
*Jingxuan Men,Mahdi Boloursaz Mashhadi,Ning Wang,Yi Ma,Mike Nilsson,Rahim Tafazolli*

Main category: cs.IT

TL;DR: Video TokenCom: A novel framework for textual intent-guided multi-rate video communication using token-based semantic communication with unequal error protection.


<details>
  <summary>Details</summary>
Motivation: Motivated by the success of Large AI Models and Multimodal LLMs, the paper addresses the need for efficient semantic- and goal-oriented information exchange in future wireless networks, particularly for video communication under restrictive bandwidth constraints.

Method: 1) Extract discrete video tokens using pretrained video tokenizer; 2) Use text-conditioned vision-language modeling and optical-flow propagation to identify tokens corresponding to user-intended semantics; 3) Implement semantic-aware multi-rate bit-allocation with full precision for intent-related tokens and reduced precision differential encoding for non-intended tokens; 4) Develop source and channel coding adaptation scheme for varying resources and link conditions.

Result: Experiments on various video datasets show the framework outperforms both conventional and semantic communication baselines in perceptual and semantic quality across a wide SNR range.

Conclusion: The proposed Video TokenCom framework successfully enables efficient semantic- and goal-oriented video communication by integrating textual intent guidance with token-based representation and unequal error protection, demonstrating superior performance under bandwidth constraints.
...
### [197] [RIS-Enabled Wireless Channel Equalization: Adaptive RIS Equalizer and Deep Reinforcement Learning](https://arxiv.org/abs/2603.02489)
*Gal Ben-Itzhak,Ender Ayanoglu*

Main category: cs.IT

TL;DR: This paper proposes two methods for RIS-assisted pulse response equalization: a channel-estimation-based adaptive filtering approach (ARISE) and model-free DRL algorithms (DDPG, TD3, SAC) that optimize RIS coefficients without explicit channel estimation.


<details>
  <summary>Details</summary>
Motivation: Practical methods for configuring large passive RIS arrays to achieve reliable signal equalization are limited, yet equalization is essential in wideband links to counteract multipath-induced pulse distortion that degrades symbol recovery.

Method: Two approaches: 1) ARISE - a steepest descent method using cascaded BS-RIS-UE channel information for RIS configuration; 2) DRL algorithms (DDPG, TD3, SAC) that optimize RIS coefficients directly from received pulse response without channel estimation.

Result: SAC achieves fast, stable convergence and equalization performance comparable to ARISE while offering significantly lower implementation complexity, highlighting DRL's potential for practical RIS control.

Conclusion: DRL offers a practical and scalable solution for real-time RIS control in future wireless systems, overcoming the channel estimation dependency of traditional adaptive filtering approaches.
...
### [198] [E-variables and tests of randomness for distribution classes](https://arxiv.org/abs/2603.02492)
*Georgii Potapov,Yuri Kalnishkan*

Main category: cs.IT

TL;DR: The paper introduces e-variable-approximability method to construct e-variables for practical distribution classes, connecting algorithmic information theory concepts to statistical hypothesis testing.


<details>
  <summary>Details</summary>
Motivation: E-variables are a promising new approach for statistical hypothesis testing that has seen significant recent development. The authors aim to develop general approximation techniques to construct e-variables for practical distribution classes that are important for real-world applications.

Method: The paper introduces the method of e-variable-approximability and uses it to develop a general approximation technique. This allows construction of e-variables for popular distribution classes. The approach builds on Levin's concept of average-bounded randomness tests from Algorithmic Information Theory.

Result: The authors show that their construction of e-variables can be used to provide an explicit construction for a randomness test with respect to a class of distributions, bridging the gap between algorithmic information theory concepts and practical statistical testing methods.

Conclusion: The paper successfully develops a general approximation framework for constructing e-variables for practical distribution classes, demonstrating how algorithmic information theory concepts can be explicitly implemented for statistical hypothesis testing applications.
...
### [199] [Functional Properties of the Focal-Entropy](https://arxiv.org/abs/2603.02533)
*Jaimin Shah,Martina Cardone,Alex Dytso*

Main category: cs.IT

TL;DR: The paper provides a systematic information-theoretic analysis of focal-loss, establishing its theoretical properties including finiteness, convexity, continuity, and asymptotic behavior, revealing how it amplifies mid-range probabilities while suppressing high-probability outcomes.


<details>
  <summary>Details</summary>
Motivation: Despite focal-loss's empirical success in class-imbalanced classification problems (especially in computer vision), there is a lack of systematic information-theoretic understanding of its properties and behavior.

Method: Adopts a distributional viewpoint to study focal-entropy (the focal-loss analogue of cross-entropy). Uses theoretical analysis to establish conditions for finiteness, convexity, continuity, and provides asymptotic characterizations. Proves existence and uniqueness of the focal-entropy minimizer and describes its structure.

Result: The analysis reveals that focal-loss amplifies mid-range probabilities, suppresses high-probability outcomes, and under extreme class imbalance induces an "over-suppression regime" where very small probabilities are further diminished. The focal-entropy minimizer can depart significantly from the data distribution. Experimental validation supports these theoretical findings.

Conclusion: The work provides a theoretical foundation for understanding focal-loss and clarifies the trade-offs it introduces when applied to imbalanced learning tasks, explaining its empirical success while highlighting its limitations in extreme imbalance scenarios.
...
### [200] [Semantic Forwarding and Codebook-Enhanced Model Division Multiple Access for Satellite-Terrestrial Networks](https://arxiv.org/abs/2603.02536)
...
TL;DR: Proposed SFSC framework for satellite-terrestrial networks uses semantic forwarding, vector-quantized joint coding/modulation, and channel-aware reconstruction to achieve 7.9 dB PSNR gain in low-SNR regimes.


<details>
  <summary>Details</summary>
Motivation: Satellite-terrestrial communications face challenges: high path loss, limited spectrum, time-varying channels. Conventional bit-level transmission is inefficient and fragile in low SNR. Semantic communication offers solution by prioritizing task-relevant information over exact bit recovery.

Method: 1) Vector-quantized joint semantic coding and modulation with optimized encoder and codebook to shape constellation distribution; 2) Satellite semantic forwarding mechanism for direct semantic-level relay without full decoding; 3) Channel-aware semantic reconstruction using FiLM to fuse SNR with semantic features; 4) Codebook split-enhanced model division multiple access (CS-MDMA) for multi-user access.

Result: Simulation shows SFSC achieves ~7.9 dB PSNR gain over existing benchmarks in low-SNR regime, demonstrating robust and spectrum-efficient semantic transmission.

Conclusion: SFSC framework effectively addresses satellite-terrestrial communication challenges through semantic forwarding, joint optimization, and channel-aware reconstruction, enabling robust and efficient transmission in dynamic channel conditions.
...
### [201] [Large Language Model Empowered CSI Feedback in Massive MIMO Systems](https://arxiv.org/abs/2603.02686)
*Jie Wu,Wei Xu,Le Liang,Xiaohu You,Mérouane Debbah*

Main category: cs.IT

TL;DR: LLM-based framework for CSI compression in FDD mMIMO systems using masked token prediction and information-theoretic mask selection.


<details>
  <summary>Details</summary>
Motivation: Despite LLM success in other domains, their potential for efficient CSI compression and feedback in FDD massive MIMO systems remains unexplored but increasingly important.

Method: 1) Reformulate CSI compression as masked token prediction task aligned with LLM functionality; 2) Design information-theoretic mask selection strategy based on self-information to identify and select CSI elements with highest self-information at UE for feedback.

Result: The framework ensures masked tokens correspond to elements with lower self-information while visible tokens correspond to elements with higher self-information, maximizing LLM prediction accuracy.

Conclusion: Proposed novel LLM-based framework effectively exploits LLM potential for CSI feedback in FDD mMIMO systems through task reformulation and intelligent mask selection.
...
### [202] [Enhancing User Throughput in Multi-panel mmWave Radio Access Networks for Beam-based MU-MIMO Using a DRL Method](https://arxiv.org/abs/2603.02745)
*Ramin Hashemi,Vismika Ranasinghe,Teemu Veijalainen,Petteri Kela,Risto Wichman*

Main category: cs.IT

TL;DR: DRL-based adaptive beam management for mmWave MU-MIMO systems improves throughput by 16% and reduces latency 3-7x compared to legacy approaches.


<details>
  <summary>Details</summary>
Motivation: mmWave communication systems with MU-MIMO and hybrid beamforming face challenges in optimizing user throughput and minimizing latency due to high complexity of dynamic beam selection and management.

Method: Deep reinforcement learning approach that models beam management as Markov decision process, incorporating cross-correlation between beams in different antenna panels, measured RSRP, and beam usage statistics to dynamically adjust beamforming decisions.

Result: Throughput increased up to 16% and latency reduced by factors 3-7x compared to baseline legacy beam management.

Conclusion: DRL-based adaptive beam management effectively enhances spectral efficiency and reduces end-to-end latency in multi-panel mmWave radio access networks.
...
### [203] [Shatter Throughput Ceilings: Leveraging Reflection Surfaces to Enhance Transmissions for Vehicular Fast Data Exchange](https://arxiv.org/abs/2603.02752)
*Qianyao Ren,Qingxiao Huang,Yiqin Deng,Xianhao Chen,Phone Lin,Yuguang Fang*

Main category: cs.IT

TL;DR: Proposes a reflection-enhanced transmission framework using specular reflecting surfaces along roadways to break throughput ceilings for smart mobility by enhancing spatial diversity, analogous to a concave lens focusing light.


<details>
  <summary>Details</summary>
Motivation: Smart mobility requires high-volume bursty data transmission between vehicles and edge computing base stations within short time windows, but conventional systems face fundamental throughput ceilings due to insufficient spatial diversity in bandwidth-limited scenarios.

Method: Strategic deployment of dedicated specular reflecting surfaces along roadways to proactively augment transmission environments, concentrating dispersed signals from multiple directions toward target vehicles. Includes dynamic virtualization methods for reflecting panel patch groups and collaborative rotation among multiple panels to manage interference and enhance signal concentration.

Result: The framework significantly raises throughput ceilings for individual vehicles by enhancing spatial diversity and achievable channel rank, allowing BS to allocate more transmission layers to single users. Validated through 3GPP-compliant system-level simulations demonstrating significant throughput boosts.

Conclusion: Reflection-enhanced transmission using strategically placed specular surfaces effectively breaks throughput limitations for smart mobility applications, providing a practical solution compatible with existing communication systems while significantly enhancing spatial diversity and data rates.
...
### [204] [An Optimization-Based User Scheduling Framework for Multiuser MIMO Systems](https://arxiv.org/abs/2603.02998)
*Victoria Palhares,Christoph Studer*

Main category: cs.IT

TL;DR: A flexible UE scheduling framework for MU-MIMO systems that globally optimizes resource allocation, supporting various objectives and outperforming greedy algorithms.


<details>
  <summary>Details</summary>
Motivation: Existing UE scheduling methods in MU-MIMO systems rely on greedy algorithms that fail to treat resource allocation globally, especially in congested scenarios where inter-UE interference needs mitigation.

Method: Proposes a UE scheduling framework that approximately solves a nonconvex optimization problem globally, determining subsets of UEs for simultaneous transmission with flexibility in objective functions and resource control.

Result: The framework outperforms existing scheduling algorithms and approaches the performance of exhaustive search for both millimeter-wave massive MU-MIMO and sub-6-GHz cell-free massive MU-MIMO systems.

Conclusion: The proposed global optimization approach provides an effective solution for UE scheduling in congested MU-MIMO scenarios, offering flexibility and superior performance compared to traditional greedy methods.
...
### [205] [Channel-Adaptive Edge AI: Maximizing Inference Throughput by Adapting Computational Complexity to Channel States](https://arxiv.org/abs/2603.03146)
*Jierui Zhang,Jianhao Huang,Kaibin Huang*

Main category: cs.IT

TL;DR: The paper proposes a theoretical framework for integrated communication and computation (IC²) in 6G edge inference, developing an analytical model for end-to-end inference accuracy and a channel-adaptive AI algorithm that maximizes inference throughput under latency and accuracy constraints.


<details>
  <summary>Details</summary>
Motivation: The design of IC² technologies for 6G edge inference is hindered by the lack of a tractable theoretical framework for characterizing end-to-end inference performance, which needs to account for both channel distortion and AI model architecture/computational complexity.

Method: The authors develop an analytical model using Mixture of von Mises (MvM) distribution to characterize high-dimensional feature distributions, leading to closed-form accuracy expressions. They formulate and solve an edge processing rate (EPR) maximization problem under joint constraints, creating a channel-adaptive AI algorithm that jointly adapts feature compression and model complexity.

Result: The proposed algorithm achieves full IC² integration by dynamically adjusting transmit-side feature compression and receive-side model complexity according to channel conditions, demonstrating superior performance compared to fixed-complexity counterparts.

Conclusion: The work provides a tractable theoretical framework for IC² in 6G edge inference systems, enabling efficient channel-adaptive AI algorithms that maximize inference throughput while meeting latency and accuracy requirements.
...
### [206] [Low-Complexity and Power-Efficient Precoding Codebook Design on Sparse Grassmannian](https://arxiv.org/abs/2603.02515)
*Joe Asano,Yuto Hama,Hiroki Iimori,Chandan Pradhan,Szabolcs Malomsoky,Naoki Ishikawa*

Main category: eess.SP

TL;DR: Sparse Grassmannian codebook design achieves low PAPR, low complexity, and low storage while matching optimal codebook performance.


<details>
  <summary>Details</summary>
Motivation: Conventional Grassmannian precoding codebooks suffer from high computational complexity, high PAPR, and high storage requirements, making them impractical for real-world uplink systems.

Method: Proposes sparse Grassmannian codebook design using Schubert cell decomposition on Grassmann manifold, exploiting sparsity to simplify objective function and reduce search space.

Result: Numerical simulations show the sparse codebook asymptotically approaches optimal performance, outperforms 5G NR codebook in achievable rate under Rayleigh fading, while maintaining substantially lower PAPR than dense designs.

Conclusion: The sparse Grassmannian codebook provides a practical, power-efficient alternative to conventional codebooks for uplink transmission scenarios.
...
### [207] [Uncertainty Quantification of Radio Wave Propagation over Irregular Terrains Using Adaptive Polynomial Chaos Expansion](https://arxiv.org/abs/2603.02529)
*Sicheng An,Luca Di Rienzo,Hao Qin,Xingqi Zhang,Lorenzo Codecasa*

Main category: eess.SP

TL;DR: First uncertainty quantification study for antenna configurations in irregular-terrain propagation using improved adaptive polynomial chaos expansion with parabolic wave equation method.


<details>
  <summary>Details</summary>
Motivation: Accurate radio wave propagation modeling over irregular terrains is crucial for wireless communication design, but deterministic models don't quantify antenna configuration uncertainties.

Method: Improved adaptive polynomial chaos expansion (APCE) coupled with two-way parabolic wave equation (PWE) method. Polynomial basis extended based on variance contributions, terminated by composite criterion combining validation error and sample-to-basis ratio.

Result: Method accurately predicts mean and 5th-95th percentile path loss range using only 30 PWE simulations, matching Monte Carlo references. APCE outperforms standard/sparse PCE, especially for percentile estimates, with low trial-to-trial variability.

Conclusion: The proposed APCE-PWE framework provides efficient uncertainty quantification for antenna configurations in irregular-terrain propagation, offering accurate predictions with minimal computational cost.
...
### [208] [The Vienna 4G/5G Drive-Test Dataset](https://arxiv.org/abs/2603.02638)
*Wilfried Wiedner,Lukas Eller,Mariam Mussbah,Dominik Rössler,Valerian Maresch,Philipp Svoboda,Markus Rupp*

Main category: eess.SP

TL;DR: The Vienna 4G/5G Drive-Test Dataset is a comprehensive open dataset of real-world LTE and 5G measurements across Vienna, combining network-side scanner data with user-side handset logs, along with inferred BS deployment details and city models for reproducible mobile network analysis.


<details>
  <summary>Details</summary>
Motivation: Machine learning for mobile network analysis is limited by the lack of large, comprehensive real-world datasets. Existing datasets often don't provide the scale, diversity, or complementary perspectives needed for robust network analysis and optimization.

Method: Created a city-scale dataset by collecting georeferenced LTE and 5G measurements across Vienna using both passive wideband scanners (network-side view) and active handset logs (user-side view). The data includes time and location alignment, inferred BS deployment descriptors (locations, sector azimuths, antenna heights), and high-resolution building/terrain models.

Result: A comprehensive open dataset organized into four components: scanner data, handset data, estimated cell information, and city models. The dataset covers diverse urban/suburban settings and provides documentation for practical reuse and reproducible benchmarking.

Conclusion: This dataset enables reproducible benchmarking across environment-aware learning, propagation modeling, coverage analysis, and ray-tracing calibration workflows, addressing the data scarcity problem in mobile network research and facilitating practical reuse for the research community.
...
### [209] [A Novel One-tap Equalizer for Zero-Padded AFDM System over Doubly Selective Channels](https://arxiv.org/abs/2603.02703)
*Chenyang Zhang,Akram Shafie,Cheng Shen,Deepak Mishra,Jinhong Yuan*

Main category: eess.SP

TL;DR: Proposes a low-complexity one-tap equalizer for zero-padded AFDM systems by selecting specific parameters, adding zero padding, and transforming to a new frequency-of-affine domain.


<details>
  <summary>Details</summary>
Motivation: AFDM is effective for doubly selective channels, but existing equalization methods may be complex. The paper aims to develop a simpler, low-complexity equalizer that maintains performance robustness.

Method: 1) Select AFDM parameters (c₁ high, c₂ dependent on c₁) to simplify affine domain IOR. 2) Add zero padding to transmitted affine domain symbols and reconstruction at receiver. 3) Transform to new frequency-of-affine (FoA) domain. 4) Propose one-tap equalizer in FoA domain to recover transmitted symbols efficiently.

Result: Numerical results show the proposed one-tap equalizer is effective, especially when c₁ is high, without compromising performance robustness.

Conclusion: The paper successfully develops a low-complexity one-tap equalizer for ZP-AFDM systems that maintains performance while reducing computational complexity, particularly effective with high c₁ parameter values.
...
### [210] [Ensemble Forecasting of Power Quality Parameters](https://arxiv.org/abs/2603.02706)
*Max Domagk,Peter Feistel,Jan Meyer,Marco Lindner,Jako Kilter*

Main category: eess.SP

TL;DR: Systematic evaluation of ensemble forecasting for power quality parameters in transmission systems shows ensembles consistently outperform individual models in accuracy and robustness for medium- to long-term predictions.


<details>
  <summary>Details</summary>
Motivation: Power electronic technologies increase need for power quality monitoring, but collected data is underutilized for compliance only. Medium- to long-term forecasting could enable proactive asset management and trend detection despite data heterogeneity and seasonality challenges.

Method: Systematically evaluates individual and ensemble forecasting approaches for PQ parameters. Analyzes over 700 weekly time series from measurement campaigns in Germany and Estonia, assessing various models and aggregation strategies within a structured ensemble framework.
...
### [211] [Doppler Shift Keying Modulation for Uplink Multiple Access over Doubly-Dispersive Channels](https://arxiv.org/abs/2603.02721)
*Xuehan Wang,Jintao Wang,Hai Lin,Jinhong Yuan,Xu Shi,Hengyu Zhang,Jian Song*

Main category: eess.SP

TL;DR: Proposes Doppler Shift Keying (DSK) modulation for high-mobility 6G applications, addressing high PAPR and detection complexity issues in uplink DD domain transmission with low-complexity iterative detection and Zadoff-Chu sequence optimization.


<details>
  <summary>Details</summary>
Motivation: Existing delay-Doppler domain modulation designs suffer from high peak-to-average power ratio and unbearable detection complexity in uplink transmission, especially when large time duration and bandwidth are needed for high DD resolutions in high-mobility 6G applications.

Method: Proposes DSK modulation based on orthogonal delay Doppler division multiplexing with one-hot mapper and low-complexity iterative successive interference cancellation-maximum ratio combining detector. Extends to zero auto-correlation sequence implementation using Zadoff-Chu sequences for multi-user uplink, optimizing ZC root assignment to minimize inter-user interference.

Result: Simulation results under doubly-dispersive channels verify the PAPR and bit error rate performance of DSK modulation with low-complexity detector, demonstrating superiority for uplink multiple access over doubly dispersive channels.

Conclusion: DSK modulation effectively addresses PAPR and complexity issues in DD domain uplink transmission, offering a competitive solution for high-mobility 6G applications with optimized multi-user implementation using Zadoff-Chu sequences.
...
### [212] [Enhancing AAV-Enabled Secure Communications via Synthetic Aperture Beamforming](https://arxiv.org/abs/2603.02768)
*Bin Qiu,Wenchi Cheng,Hongxiang He,Jiangzhou Wang*

Main category: eess.SP

TL;DR: Synthetic aperture secure beamforming for virtual MIMO broadcast channels with AAV deployment optimization to maximize capacity and enhance physical layer security against eavesdroppers.


<details>
  <summary>Details</summary>
Motivation: Address physical layer security challenges in wireless communications by leveraging synthetic aperture beamforming to enhance security in hybrid wiretapping environments, where traditional security methods may be insufficient.

Method: Proposes a comprehensive approach including: 1) AAV flight node deployment optimization using asymptotic channel eigenvalue analysis, 2) Energy-efficient transmit precoding design with QoS constraints for BS, leakage tolerance for eavesdroppers, and per-node power constraints, 3) Received beamforming using LCMV method for jamming suppression.

Result: Numerical results demonstrate the proposed method's superiority in promoting system channel capacity while maintaining security constraints and power efficiency.

Conclusion: The synthetic aperture secure beamforming approach effectively enhances physical layer security in virtual MIMO broadcast channels by optimizing AAV deployment, transmission strategies, and beamforming techniques, providing a robust solution against hybrid wiretapping threats.
...
### [213] [Exploiting Double-Bounce Paths in Snapshot Radio SLAM: Bounds, Algorithms and Experiments](https://arxiv.org/abs/2603.02832)
*Xi Zhang,Yu Ge,Ossi Kaltiokallio,Musa Furkan Keskin,Henk Wymeersch,Mikko Valkama*

Main category: eess.SP

TL;DR: The paper shows that leveraging double-bounce NLoS paths in radio SLAM improves UE localization accuracy and enables mapping of previously unobservable landmarks, outperforming traditional single-bounce-only approaches.


<details>
  <summary>Details</summary>
Motivation: Most radio SLAM approaches only use LoS and single-bounce NLoS paths, treating higher-order reflections as noise. This limits localization accuracy and environmental mapping capabilities, as double-bounce paths contain valuable geometric information about the environment.

Method: The authors derive the Cramer-Rao bound for joint UE state and landmark estimation with double-bounce paths, propose an algorithm to identify these paths, and incorporate them into joint estimation. Validation uses simulated data and experimental mmWave measurements with 5G reference signals.

Result: Double-bounce NLoS paths that share incidence points with single-bounce paths improve UE state estimation and existing IP accuracy. More importantly, they enable mapping of landmarks that are completely unobservable using only single-bounce paths, significantly enhancing environmental sensing.

Conclusion: Exploiting double-bounce NLoS paths in radio SLAM provides substantial benefits for both user equipment localization and environmental mapping, offering improved accuracy and revealing previously hidden landmarks, making it a valuable enhancement to traditional SLAM approaches.
...
### [214] [SilentWear: an Ultra-Low Power Wearable System for EMG-based Silent Speech Recognition](https://arxiv.org/abs/2603.02847)
...
TL;DR: SilentWear: A wearable textile neck interface with on-device EMG-based speech recognition achieving 84.8% accuracy for vocalized and 77.5% for silent speech commands.


<details>
  <summary>Details</summary>
Motivation: Current biosignal-based speech detection systems lack wearability and edge-processing capabilities needed for minimally obtrusive, responsive, and private assistive technologies.

Method: Developed SilentWear - a textile-based neck interface with 14 differential EMG channels, coupled with SpeechNet (15k-parameter CNN) for EMG-based speech decoding, featuring incremental fine-tuning for repositioning robustness.

Result: Achieved 84.8±4.6% accuracy for vocalized and 77.5±6.6% for silent speech over 8 commands. With incremental fine-tuning, recovered >10% accuracy with <10 minutes of additional data. On-device inference consumes 63.9μJ with 2.47ms latency, enabling 27+ hours continuous operation.

Conclusion: SilentWear demonstrates a practical, wearable solution for EMG-based speech recognition with robust performance, on-device processing, and long battery life, advancing noise-robust and privacy-preserving human-computer interfaces.
...
### [215] [KA band mobile antenna for satellite communication](https://arxiv.org/abs/2603.03102)
*Sidra Tul. Muntaha,Ahmad Arfeen,Kashan Raza*

Main category: eess.SP

TL;DR: Design of Ka-band mobile antennas for satellite communication at 29GHz, progressing from single element to 8x8 array, achieving 21dB gain and -30dB return loss using CST software and mathematical calculations.


<details>
  <summary>Details</summary>
Motivation: Address the demand for swift satellite communication with higher data transfer rates using Ka-band frequency range, which is known for supporting high-speed internet access and multimedia streaming applications.

Method: Mathematical calculations and software implementation using CST, progressing from single element to 8x8 phased array design. Parameters include patch dimensions, substrate properties, and effective permittivity. Integration of machine learning techniques for optimization.

Result: Antennas achieved gain up to 21 dB and return losses as low as -30 dB. Successfully addressed challenges including signal attenuation, directional accuracy, circular polarization, and impedance matching through various configurations including phased-array and electronically steerable antennas.

Conclusion: This research advances high-frequency transmission technology, meeting the demands of modern satellite-based communication systems for applications like high-speed internet access and multimedia streaming.
...
### [216] [Continuous-Aperture Array-Based ISAC Over Fading Channels](https://arxiv.org/abs/2603.03184)
*Boqun Zhao,Chongjun Ouyang,Xingqi Zhang,Yuanwei Liu*

Main category: eess.SP

TL;DR: CAPA-based ISAC framework with continuous-aperture arrays outperforms discrete arrays and FDD approaches, with three beamforming designs analyzed for sensing-communication trade-offs.


<details>
  <summary>Details</summary>
Motivation: To develop an integrated sensing and communications framework using continuous-aperture arrays that can overcome limitations of conventional discrete arrays and frequency-division approaches, enabling better performance in fading channels.

Method: Proposes CAPA-based ISAC with continuous operator-based signal model, uses Landau's eigenvalue theorem for channel statistics, analyzes three beamforming designs: sensing-centric, communication-centric, and Pareto-optimal trade-off designs.

Result: Derives closed-form expressions for sensing rate, ergodic communication rate, and outage probability for S-C and C-C designs; obtains multiplexing/diversity gains from asymptotic analysis; characterizes SR-CR region for Pareto-optimal design; shows CAPA-ISAC outperforms discrete arrays and FDD.

Conclusion: CAPA-based ISAC provides superior performance over conventional approaches, with analytical framework enabling optimal beamforming designs that balance sensing-communication trade-offs in fading channels.
...
### [217] [Loss Design and Architecture Selection for Long-Tailed Multi-Label Chest X-Ray Classification](https://arxiv.org/abs/2603.02294)
*Nikhileswara Rao Sulake*

Main category: eess.IV

TL;DR: Systematic evaluation of methods for long-tailed multi-label CXR classification shows LDAM-DRW loss with ConvNeXt-Large backbone achieves best performance, ranking 5th in competition.


<details>
  <summary>Details</summary>
Motivation: Long-tailed class distributions in multi-label chest X-ray classification present challenges for recognizing rare but clinically important findings that are severely underrepresented.

Method: Empirical evaluation of loss functions (BCE, asymmetric losses, LDAM-DRW), CNN architectures, and post-training strategies on CXR-LT 2026 benchmark with 143K images and 30 disease labels from PadChest.

Result: LDAM-DRW consistently outperforms standard losses for rare class recognition. ConvNeXt-Large achieved best single-model performance (0.5220 mAP, 0.3765 F1). Submission ranked 5th among 68 teams with 0.3950 mAP on official test leaderboard.

Conclusion: LDAM-DRW with ConvNeXt-Large backbone and post-training strategies effectively handles class imbalance in clinical imaging, though development-to-test performance gap analysis reveals practical challenges.
...
### [218] [Biomechanically Accurate Gait Analysis: A 3d Human Reconstruction Framework for Markerless Estimation of Gait Parameters](https://arxiv.org/abs/2603.02499)
*Akila Pemasiri,Ethan Goan,Glen Lichtwark,Robert Schuster,Luke Kelly,Clinton Fookes*

Main category: eess.IV

TL;DR: A markerless gait analysis framework using 3D human reconstruction from video that extracts biomechanical markers similar to motion capture and integrates with OpenSim for joint kinematics.


<details>
  <summary>Details</summary>
Motivation: To create a scalable, markerless, and interpretable gait analysis system that overcomes limitations of conventional keypoint-based approaches and enables broader clinical and real-world deployment of vision-based biomechanics.

Method: Uses 3D human reconstruction from video to extract biomechanically meaningful markers (analogous to motion capture systems), then integrates these markers within OpenSim for joint kinematic estimation.

Result: Strong agreement with reference marker-based measurements, with considerable improvements compared to pose-estimation methods alone for both spatiotemporal and kinematic gait parameters.

Conclusion: The framework offers a scalable, markerless, and interpretable approach for accurate gait assessment, supporting broader clinical and real-world deployment of vision-based biomechanics.
...
### [219] [DLIOS: An LLM-Augmented Real-Time Multi-Modal Interactive Enhancement Overlay System for Douyin Live Streaming](https://arxiv.org/abs/2603.03060)
*Shuide Wen,Sungil Seok,Beier Ku,Richee Li,Yubin He,Bowen Qu,Yang Yang,Ping Su,Can Jiao*

Main category: eess.IV

TL;DR: DLIOS is an LLM-augmented real-time multi-modal enhancement system for TikTok live streaming with transparent overlay rendering, automated commentary generation, and AI persona broadcasting.


<details>
  <summary>Details</summary>
Motivation: To enhance live streaming experiences on platforms like Douyin/TikTok by providing real-time interactive overlays (danmaku, gifts, animations) and automated, emotionally coherent commentary using LLMs, reducing manual effort for streamers while maintaining engagement.

Method: Three-layer transparent window architecture with WebView2 capture pipeline and thread-safe event bus. LLM broadcast automation includes: 4-segment prompt scheduling for song commentary, JSON-configurable multi-persona system, real-time danmaku reaction engine, and AI singer-songwriter persona case study with Suno-generated songs.

Result: 36-hour stress test showed: zero danmaku overlap, zero deadlock crashes, gift effect P95 latency ≤180ms, LLM-to-TTS segment P95 latency ≤2.1s, TTS integrated loudness gain of 9.5 LUFS. Successfully produced over 100 AI-generated songs for the Suwan Li persona.

Conclusion: DLIOS demonstrates a robust, low-latency system for enhancing live streaming with LLM-powered automation and multi-modal overlays, enabling AI personas to deliver engaging, emotionally coherent content while maintaining technical stability.
...
### [220] [Context Adaptive Extended Chain Coding for Semantic Map Compression](https://arxiv.org/abs/2603.03073)
*Runyu Yang,Junqi Liao,Hyomin Choi,Fabien Racapé,Ivan V. Bajić*

Main category: eess.IV

TL;DR: Novel chain-coding-based lossless compression for semantic maps using extended chain codes and skip-coding for shared boundaries, achieving 18% bitrate reduction and significant runtime improvements.


<details>
  <summary>Details</summary>
Motivation: Semantic maps are widely used in robotics, autonomous systems, and extended reality, creating a need for efficient compression methods that preserve structured semantic information while reducing storage and transmission costs.

Method: Proposes a chain-coding-based framework with extended chain code (ECC) for compact contour representation, context-adaptive entropy coding with Markov modeling, and skip-coding mechanism to eliminate redundant shared boundaries between adjacent semantic regions using run-length signaling.

Result: Achieves 18% average bitrate reduction compared to state-of-the-art benchmarks on semantic map datasets, with up to 98% encoder and 50% decoder runtime reduction relative to modern generic lossless codecs. Consistent compression gains also demonstrated on occupancy maps.

Conclusion: The proposed chain-coding framework effectively compresses semantic maps by exploiting contour topology and shared boundaries, offering significant improvements in both compression efficiency and computational performance for practical applications.
...
### [221] [RxnNano:Training Compact LLMs for Chemical Reaction and Retrosynthesis Prediction via Hierarchical Curriculum Learning](https://arxiv.org/abs/2603.02215)
*Ran Li,Shimin Di,Haowei LI,Luanshi Bu,Jiachuan Wang,Wangze Ni,Lei Chen*

Main category: cs.LG

TL;DR: RxnNano: A 0.5B-parameter chemical reaction prediction model that prioritizes chemical understanding over scale, achieving state-of-the-art performance through latent chemical consistency, hierarchical curriculum learning, and atom-mapping invariance.
...
Conclusion: Chemical understanding should be prioritized over scale in reaction prediction models. The proposed framework successfully instills deep chemical intuition into compact models, demonstrating that knowledge-driven approaches can outperform much larger scale-focused models.
...
### [222] [ATPO: Adaptive Tree Policy Optimization for Multi-Turn Medical Dialogue](https://arxiv.org/abs/2603.02216)
*Ruike Cao,Shaojie Bai,Fugen Yao,Liang Dong,Jian Xu,Li Xiao*

Main category: cs.LG

TL;DR: ATPO algorithm improves medical dialogue LLMs by adaptively allocating rollout budget to uncertain states using Bellman error and action-value variance, with optimizations for computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Effective information seeking in multi-turn medical dialogues is critical for accurate diagnosis, especially with incomplete information. Current RL methods (GRPO, PPO) struggle with long-horizon credit assignment and unstable value estimation in uncertain user-agent interactions modeled as H-MDP.

Method: Proposes uncertainty-aware Adaptive Tree Policy Optimization (ATPO) that adaptively allocates rollout budget to states with high uncertainty (measured by Bellman error + action-value variance). Includes uncertainty-guided pruning to reduce rollouts and asynchronous search with KV cache reuse for inference efficiency.

Result: Extensive experiments on three public medical dialogue benchmarks show ATPO significantly outperforms strong baselines. Qwen3-8B model surpasses much larger GPT-4o by +0.92% accuracy.

Conclusion: ATPO enables more accurate value estimation and efficient diverse exploration for medical dialogue LLMs, achieving state-of-the-art performance with computational optimizations that make tree-based RL practical for large-scale applications.
...
### [223] [Is Retraining-Free Enough? The Necessity of Router Calibration for Efficient MoE Compression](https://arxiv.org/abs/2603.02217)
*Sieun Hyeon,Jaeyoung Do*

Main category: cs.LG

TL;DR: Router Knowledge Distillation (Router KD) addresses post-compression degradation in Mixture-of-Experts models by updating only the router parameters through knowledge distillation, without retraining expert parameters.


<details>
  <summary>Details</summary>
Motivation: MoE models have deployment-time memory bottlenecks due to massive parameters, and existing retraining-free compression methods suffer from persistent degradation due to router-expert mismatch when experts are changed but routers remain untouched.

Method: Proposes Router Knowledge Distillation (Router KD) that updates only a tiny fraction of parameters (the router) by distilling the original model's next-token distribution on unlabeled calibration data, avoiding expert parameter updates while allowing lightweight router calibration.

Result: Experiments across three compression paradigms (Expert Pruning, Expert Editing, Expert Merging) show consistent performance recovery, with larger gains in fine-grained MoEs (many small experts) than coarse-grained MoEs due to their more complex routing decision boundaries.

Conclusion: Effective retraining-free MoE compression should avoid updating expert parameters while allowing lightweight router calibration via Router KD, which addresses the router-expert mismatch problem and enables better compression performance.
...
### [224] [Self-Play Only Evolves When Self-Synthetic Pipeline Ensures Learnable Information Gain](https://arxiv.org/abs/2603.02218)
*Wei Liu,Siya Qi,Yali Du,Yulan He*

Main category: cs.LG

TL;DR: The paper proposes a framework for sustainable self-evolution in LLMs by addressing the failure mode of self-play systems that plateau due to lack of increasing learnable information across iterations.


<details>
  <summary>Details</summary>
Motivation: Current self-play LLM systems often plateau quickly because they synthesize more data without increasing learnable information for subsequent iterations, limiting sustainable self-evolution.

Method: Introduces a triadic role framework (Proposer, Solver, Verifier) and three system designs: asymmetric co-evolution (weak-to-strong-to-weak loop), capacity growth (matching rising information), and proactive information seeking (preventing saturation with external context).

Result: The proposed approach provides a measurable, system-level path from brittle self-play dynamics to sustained self-evolution by ensuring learnable information increases across iterations.

Conclusion: Sustainable self-evolution requires a self-synthesized data pipeline with increasing learnable information, achieved through the triadic role framework and three complementary system designs that work together to overcome self-play limitations.
...
### [225] [NExT-Guard: Training-Free Streaming Safeguard without Token-Level Labels](https://arxiv.org/abs/2603.02219)
*Junfeng Fang,Nachuan Chen,Houcheng Jiang,Dan Zhang,Fei Shen,Xiang Wang,Xiangnan He,Tat-Seng Chua*

Main category: cs.LG

TL;DR: NExT-Guard is a training-free framework for real-time streaming safety in LLMs that monitors interpretable latent features from Sparse Autoencoders instead of using expensive token-level supervised training.


<details>
  <summary>Details</summary>
Motivation: Conventional post-hoc safeguards fail in streaming scenarios as they can't interdict unsafe content in real-time. Existing streaming safeguards require expensive token-level annotations and suffer from overfitting, creating a need for more efficient solutions.

Method: Uses pretrained Sparse Autoencoders (SAEs) from publicly available base LLMs to monitor interpretable latent features. The framework is training-free and leverages the inherent token-level risk signals already encoded in post-hoc safeguards' hidden representations.

Result: Outperforms both post-hoc and supervised streaming safeguards with superior robustness across models, SAE variants, and risk scenarios. Enables flexible, low-cost deployment without token-level supervision.

Conclusion: NExT-Guard provides a universal and scalable paradigm for real-time safety, accelerating practical deployment of streaming safeguards by eliminating the need for expensive supervised training.
...
### [226] [Forecasting as Rendering: A 2D Gaussian Splatting Framework for Time Series Forecasting](https://arxiv.org/abs/2603.02220)
*Yixin Wang,Yifan Hu,Peiyuan Liu,Naiqi Li,Dai Tao,Shu-Tao Xia*

Main category: cs.LG

TL;DR: TimeGS is a novel time series forecasting framework that shifts from regression to 2D generative rendering, using anisotropic Gaussian kernels to model complex temporal patterns while maintaining chronological continuity across periodic boundaries.


<details>
  <summary>Details</summary>
Motivation: Current 2D period-phase representations for time series forecasting have two main limitations: 1) treating reshaped tensors as static images creates topological mismatch where spatial operators sever chronological continuity at grid boundaries, and 2) uniform fixed-size representations inefficiently allocate modeling capacity and lack adaptive resolution for compressible, non-stationary temporal patterns.

Method: TimeGS reconceptualizes future sequences as continuous latent surfaces and uses anisotropic Gaussian kernels for adaptive modeling. It introduces two key components: 1) Multi-Basis Gaussian Kernel Generation (MB-GKG) block that synthesizes kernels from a fixed dictionary to stabilize optimization, and 2) Multi-Period Chronologically Continuous Rasterization (MP-CCR) block that enforces strict temporal continuity across periodic boundaries.
...
Conclusion: TimeGS successfully addresses the limitations of existing 2D period-phase approaches by fundamentally shifting the forecasting paradigm to 2D generative rendering, enabling adaptive modeling of complex temporal patterns while maintaining chronological continuity, resulting in superior forecasting performance.
...
### [227] [MedFeat: Model-Aware and Explainability-Driven Feature Engineering with LLMs for Clinical Tabular Prediction](https://arxiv.org/abs/2603.02221)
*Zizheng Zhang,Yiming Li,Justin Xu,Jinyu Wang,Rui Wang,Lei Song,Jiang Bian,David W Eyre,Jingjing Fu*

Main category: cs.LG

TL;DR: MedFeat is a feedback-driven, model-aware feature engineering framework that uses LLMs with domain knowledge and SHAP explanations to improve healthcare tabular predictions, outperforming classical methods and discovering clinically meaningful features.


<details>
  <summary>Details</summary>
Motivation: Classical models with feature engineering often outperform neural approaches in healthcare tabular predictions. Existing LLM-based approaches use broad search over predefined transformations but overlook downstream model characteristics and feature importance signals.

Method: MedFeat is a feedback-driven, model-aware feature engineering framework that leverages LLM reasoning with domain knowledge, provides feature explanations based on SHAP values, and tracks successful/failed proposals to guide feature discovery. It prioritizes informative signals that are difficult for downstream models to learn directly.

Result: MedFeat achieves stable improvements over various baselines across clinical prediction tasks, discovers clinically meaningful features that generalize under distribution shift, and demonstrates robustness across years and from ICU cohorts to general hospitalized patients.

Conclusion: MedFeat offers insights into real-world deployment by combining LLM reasoning with model-aware feature engineering, improving healthcare tabular predictions while maintaining interpretability and robustness across different clinical settings.
...
### [228] [MedCalc-Bench Doesn't Measure What You Think: A Benchmark Audit and the Case for Open-Book Evaluation](https://arxiv.org/abs/2603.02222)
*Artus Krohn-Grimberghe*

Main category: cs.LG

TL;DR: MedCalc-Bench benchmark has implementation errors; open-book prompting outperforms RL-trained systems; benchmark measures formula memorization, not clinical reasoning.


<details>
  <summary>Details</summary>
Motivation: To challenge the current framing of MedCalc-Bench benchmark by identifying implementation errors and demonstrating that it primarily tests formula memorization rather than clinical reasoning.

Method: 1) Systematic audit of calculator implementations, fixing over 20 errors; 2) Open-book prompting (providing calculator specs at inference); 3) Testing with advanced models (GLM-4.6V, GLM-4.7, GPT-5.2-Thinking).

Result: Open-book prompting raised accuracy from ~52% to 81-85% on GLM models, surpassing RL-trained systems (74%). GPT-5.2-Thinking achieved 95-97% accuracy, with residual errors due to dataset issues.

Conclusion: MedCalc-Bench predominantly measures formula memorization and arithmetic precision, not clinical reasoning, and should be reframed as a tool-use evaluation benchmark.
...
### [229] [Characterizing and Predicting Wildfire Evacuation Behavior: A Dual-Stage ML Approach](https://arxiv.org/abs/2603.02223)
*Sazzad Bin Bashar Polock,Anandi Dutta,Subasish Das*

Main category: cs.LG

TL;DR: ML analysis of wildfire evacuation behavior reveals distinct household typologies and shows transportation mode is predictable from household characteristics, while evacuation timing depends on dynamic fire conditions.


<details>
  <summary>Details</summary>
Motivation: Wildfire evacuation behavior is complex and variable, influenced by multiple interacting factors. Understanding these patterns is crucial for effective emergency planning and resource allocation, but traditional approaches may not capture the nuanced behavioral typologies.

Method: Used large-scale MTurk survey of residents in California, Colorado, and Oregon. Applied unsupervised ML methods (Multiple Correspondence Analysis, K-Modes clustering, Latent Class Analysis) to identify behavioral subgroups, and supervised ML models to predict evacuation outcomes.

Result: Identified consistent subgroups differentiated by vehicle access, disaster planning, technological resources, pet ownership, and residential stability. Transportation mode can be predicted with high reliability from household characteristics, but evacuation timing remains difficult to classify due to dependence on dynamic fire conditions.

Conclusion: Machine learning provides data-driven insights into wildfire evacuation behavior, supporting targeted preparedness strategies, resource allocation, and equitable emergency planning. The approach demonstrates how ML can uncover latent behavioral patterns that inform more effective disaster response.
...
### [230] [Subspace Geometry Governs Catastrophic Forgetting in Low-Rank Adaptation](https://arxiv.org/abs/2603.02224)
*Brady Steele*

Main category: cs.LG

TL;DR: LoRA forgetting follows a geometric law based on gradient subspace angles, with rank-invariance at high angles, reconciling contradictory findings in continual learning literature.


<details>
  <summary>Details</summary>
Motivation: To understand the behavior of Low-Rank Adaptation (LoRA) under continual learning and characterize catastrophic forgetting through geometric analysis of gradient subspace interactions.

Method: Develop a geometric theory showing forgetting follows a law based on minimum principal angle between task gradient subspaces, then validate on synthetic tasks (r=0.994 correlation), Split-CIFAR100 with ViT-LoRA, and sequential GLUE with RoBERTa-LoRA.

Result: Forgetting is governed by F = α(1 - cos²θ_min) + β, revealing rank-invariance at high subspace angles (CV ≈ 0.8% synthetic, 10-19% real benchmarks). Rank only affects forgetting when task subspaces are similar, and orthogonal methods like O-LoRA provide minimal benefit when natural orthogonality is already high.

Conclusion: The geometric theory reconciles contradictory findings in literature and provides principled guidance for continual learning with parameter-efficient fine-tuning, showing forgetting depends on subspace angles rather than adapter rank in most practical scenarios.
...
### [231] [Scaling Reward Modeling without Human Supervision](https://arxiv.org/abs/2603.02225)
*Jingxuan Fan,Yueying Li,Zhenting Qi,Dinghuai Zhang,Kianté Brantley,Sham M. Kakade,Hanlin Zhang*

Main category: cs.LG

TL;DR: Unsupervised reward model scaling using preference learning on web data improves reward model performance without human annotations.


<details>
  <summary>Details</summary>
Motivation: Learning from feedback is crucial for advancing frontier models but is constrained by cost and scalability of human annotations. The paper explores whether reward models can be effectively scaled using unsupervised approaches instead of costly human-labeled data.

Method: Proposes reward-based scaling (RBS) using preference learning over document prefixes and suffixes drawn from large-scale web corpora. The method trains on 11M tokens of math-focused web data without any human annotations, applying this approach across diverse model families and scales.

Result: The unsupervised approach yields steady gains on RewardBench v1 and v2, improving RewardBench v2 accuracy by up to +7.7 points on average, with gains up to +16.1 on in-domain math subsets. Improvements transfer across diverse initialization backbones and consistently improve out-of-domain safety and general subsets. When applied to best-of-N selection and policy optimization, these reward models substantially improve downstream math performance and match or exceed strong supervised reward model baselines.

Conclusion: The study demonstrates the feasibility and promise of training reward models without costly and potentially unreliable human annotations, showing that unsupervised approaches can effectively scale reward models and transfer improvements across domains.
...
### [232] [Efficient Sparse Selective-Update RNNs for Long-Range Sequence Modeling](https://arxiv.org/abs/2603.02226)
*Bojian Yin,Shurong Wang,Haoyu Tan,Sander Bohte,Federico Corradi,Guoqi Li*

Main category: cs.LG

TL;DR: suRNNs use neuron-level binary switches to update memory only for informative events, avoiding constant updates and memory decay in traditional RNNs.


<details>
  <summary>Details</summary>
Motivation: Traditional RNNs suffer from memory decay due to rigid update schedules that force constant state updates even during redundant input periods, making it hard to retain distant past information.

Method: Selective-Update RNNs (suRNNs) with neuron-level binary switches that only open for informative events, decoupling recurrent updates from raw sequence length and preserving exact memory during low-information intervals.

Result: suRNNs match or exceed Transformer accuracy on Long Range Arena, WikiText, and synthetic benchmarks while being significantly more efficient for long-term storage.

Conclusion: suRNNs provide a principled approach to managing temporal information density, achieving Transformer-level performance within the efficient recurrent modeling framework by allowing neurons to learn their own update timescales.
...
### [233] [Routing Absorption in Sparse Attention: Why Random Gates Are Hard to Beat](https://arxiv.org/abs/2603.02227)
*Keston Aquino-Michaels*

Main category: cs.LG

TL;DR: Transformers struggle to learn effective attention sparsity during training due to "routing absorption" where Q/K/V projections adapt to masks, making learned gates perform little better than random gates.


<details>
  <summary>Details</summary>
Motivation: To understand why transformers fail to learn effective sparse attention patterns during end-to-end training despite attention distributions being highly concentrated and theoretically learnable.

Method: Four experimental approaches with a 31M-parameter transformer: (1) comparing learned vs random soft gating perplexity, (2) analyzing gradient flow through hard top-k masks, (3) distilling gates onto co-adapted Q/K/V, and (4) testing stochastic mask randomization.

Result: Learned gates perform similarly to random gates (48.73 vs 49.83 perplexity), hard masks receive zero gradient, distilled gates achieve high F1 but catastrophic perplexity when deployed, and stochastic randomization fails to prevent co-adaptation.

Conclusion: End-to-end sparse attention faces "routing absorption" where Q/K/V parameters adapt to masks, making learned gating ineffective. Post-hoc sparsification approaches that decouple representation learning from sparsity are more viable.
...
### [234] [Neural Paging: Learning Context Management Policies for Turing-Complete Agents](https://arxiv.org/abs/2603.02228)
*Liang Chen,Qi Liu*

Main category: cs.LG

TL;DR: Neural Paging: A hierarchical architecture with differentiable Page Controller that optimizes context window usage, reducing long-horizon reasoning complexity from O(N²) to O(N·K²).


<details>
  <summary>Details</summary>
Motivation: LLMs with external memory are computationally universal but face bottleneck of finite, costly context windows that act as scarce semantic caches rather than infinite memory.

Method: Introduces Neural Paging architecture that decouples symbolic reasoning from information management. Formulates Context Paging Problem (CPP) and proposes lightweight, differentiable Page Controller to approximate "Semantic Belady's Optimality" - retaining tokens with high future utility.

Result: Theoretical analysis shows Neural Paging reduces asymptotic complexity from O(N²) to O(N·K²) under bounded context window size K. Derives robustness bound (Theorem 4) quantifying competitive-ratio degradation under policy-dependent access with bounded sensitivity. Validated on synthetic paging traces.

Conclusion: Neural Paging provides theoretical guarantees for efficient context window management, with significant slack in bounds motivating learned policies for practical implementation.
...
### [235] [Safety Training Persists Through Helpfulness Optimization in LLM Agents](https://arxiv.org/abs/2603.02229)
*Benjamin Plaut*

Main category: cs.LG

TL;DR: The paper studies safety post-training in agentic (multi-step, tool-use) settings, finding that safety training persists through subsequent helpfulness training and all configurations end up near a linear Pareto frontier.


<details>
  <summary>Details</summary>
Motivation: Most safety post-training research focuses on single-step "chat" settings where safety means refusing harmful requests. This paper investigates agentic settings where safety refers to harmful actions directly taken by the LLM during multi-step tool use.

Method: The researchers compare effects of running Direct Preference Optimization (DPO) on safety alone, helpfulness alone, and both metrics sequentially. They analyze how training on one metric affects performance on the other and examine the resulting trade-offs.

Result: Safety training persists through subsequent helpfulness training. All training configurations end up near a linear Pareto frontier (R² = 0.77). Even simultaneous training on both metrics results in another point on the frontier rather than finding optimal "best of both worlds" strategies, despite such strategies existing in the DPO dataset.

Conclusion: The findings highlight the need for better understanding of post-training dynamics in agentic settings, as current methods don't effectively combine safety and helpfulness despite the existence of optimal strategies in the training data.
...
### [236] [Generalized Discrete Diffusion with Self-Correction](https://arxiv.org/abs/2603.02230)
*Linxuan Wang,Ziyi Wang,Yikun Bai,Wei Deng,Guang Lin,Qifan Song*

Main category: cs.LG

TL;DR: SCDD reformulates pretrained self-correction for discrete diffusion models with explicit state transitions and simplified training, enabling efficient parallel decoding while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Existing self-correction approaches for discrete diffusion models suffer from limited generalization, impaired reasoning, and complex hyperparameter tuning. GIDD's continuous interpolation pipeline has opaque interactions between uniform transitions and absorbing masks, hindering practical performance.

Method: Proposes Self-Correcting Discrete Diffusion (SCDD) model that reformulates pretrained self-correction with explicit state transitions learned directly in discrete time. Simplifies training noise schedule, eliminates redundant remasking step, and uses only uniform transitions to learn self-correction.
...
Conclusion: SCDD provides a more practical and effective approach to self-correction in discrete diffusion models by addressing the limitations of previous methods through explicit state transitions and simplified training.
...
### [237] [Physics-Informed Neural Networks with Architectural Physics Embedding for Large-Scale Wave Field Reconstruction](https://arxiv.org/abs/2603.02231)
*Huiwen Zhang,Feng Ye,Chu Ma*

Main category: cs.LG

TL;DR: PE-PINN integrates physical guidance directly into neural network architecture (not just loss functions) via envelope transformation layers to enable efficient large-scale wave field reconstruction.


<details>
  <summary>Details</summary>
Motivation: Large-scale wave field reconstruction faces computational efficiency vs. accuracy trade-offs: FEM is accurate but computationally expensive for large/high-frequency problems, while data-driven methods lack sufficient labeled data. Standard PINNs embed physics only in loss functions, leading to slow convergence, optimization instability, and spectral bias.

Method: Proposes architecture physics embedded (PE)-PINN that integrates physical guidance directly into neural network architecture beyond just loss functions. Introduces envelope transformation layer with kernels parameterized by source properties, material interfaces, and wave physics to mitigate spectral bias.

Result: PE-PINN achieves >10× speedup in convergence compared to standard PINNs and several orders of magnitude reduction in memory usage compared to FEM. Enables high-fidelity modeling for large-scale 2D/3D electromagnetic wave reconstruction involving reflections, refractions, and diffractions in room-scale domains.

Conclusion: PE-PINN bridges the gap between computational efficiency and accuracy for large-scale wave field reconstruction, making it applicable to wireless communications, sensing, room acoustics, and other fields requiring large-scale wave field analysis.
...
### [238] [Inverse Reconstruction of Shock Time Series from Shock Response Spectrum Curves using Machine Learning](https://arxiv.org/abs/2603.03229)
*Adam Watts,Andrew Jeon,Destry Newton,Ryan Bowering*

Main category: cs.LG

TL;DR: A conditional variational autoencoder (CVAE) learns to generate acceleration time series from target shock response spectra, bypassing iterative optimization for inverse SRS reconstruction.


<details>
  <summary>Details</summary>
Motivation: The inverse SRS reconstruction problem is ill-posed due to nonlinear, many-to-one mapping from acceleration to spectrum. Conventional iterative methods are computationally expensive and constrained by predefined basis functions.

Method: A conditional variational autoencoder (CVAE) is trained to learn a data-driven inverse mapping from target SRS to acceleration time series, enabling direct generation without iterative optimization.

Result: The model achieves improved spectral fidelity compared to classical techniques, strong generalization to unseen spectra, and inference speeds 3-6 orders of magnitude faster than conventional methods.

Conclusion: Deep generative modeling provides a scalable and efficient approach for inverse SRS reconstruction, offering significant computational advantages over traditional optimization-based methods.
...
### [239] [Beyond Binary Preferences: A Principled Framework for Reward Modeling with Ordinal Feedback](https://arxiv.org/abs/2603.02232)
*Amirhossein Afsharrad,Ruida Zhou,Luca Viano,Sanjay Lall,Mohammad Ghavamzadeh*

Main category: cs.LG

TL;DR: Proposes a principled ordinal regression framework for reward modeling with Likert scale preferences, replacing heuristic approaches with mathematically grounded loss functions.


<details>
  <summary>Details</summary>
Motivation: Current reward modeling approaches lack a principled mathematical framework for leveraging ordinal preference data (Likert scale ratings). Existing methods use ad-hoc heuristics like margin terms or scaling factors applied to binary preference models, without an underlying mathematical model for how ordinal preference data is generated.

Method: Formulates reward modeling with Likert scale preferences as a discrete ordinal regression problem. Derives two loss functions: negative log-likelihood loss and all-threshold loss, both of which learn threshold parameters that naturally capture the ordinal structure of preferences. Unlike heuristic methods that manually specify fixed margins, this approach learns parameters directly from data within a coherent probabilistic framework.

Result: Experimental results on multiple benchmarks demonstrate that the ordinal regression approach consistently achieves competitive or superior performance compared to existing heuristic methods across diverse evaluation categories including chat, reasoning, and safety tasks.
...
### [240] [Adaptive Personalized Federated Learning via Multi-task Averaging of Kernel Mean Embeddings](https://arxiv.org/abs/2603.02233)
*Jean-Baptiste Fermanian,Batiste Le Bars,Aurélien Bellet*

Main category: cs.LG

TL;DR: A new PFL method that learns collaborative weights from data via kernel mean embedding, enabling automatic adaptation between global/local learning with theoretical guarantees and practical communication-efficient implementation.


<details>
  <summary>Details</summary>
Motivation: Current PFL approaches often require prior knowledge of data heterogeneity or manually specified collaboration schemes. There's a need for fully adaptive methods that can automatically determine optimal collaboration patterns between agents without sharing raw data.

Method: Formulates weight estimation as kernel mean embedding with multiple data sources, using multi-task averaging to capture statistical relationships. Proposes practical implementation with random Fourier features to address communication constraints in federated settings.

Result: Derives finite-sample guarantees on local excess risks for broad distribution classes, explicitly quantifying statistical gains of collaboration. Numerical experiments validate theoretical results and demonstrate adaptive transition between global/local regimes.

Conclusion: The proposed method provides a fully adaptive PFL approach that requires no prior knowledge of data heterogeneity, automatically determines optimal collaboration, and offers theoretical guarantees with practical communication-efficient implementation.
...
### [241] [Structured vs. Unstructured Pruning: An Exponential Gap](https://arxiv.org/abs/2603.02234)
*Davide Ferré,Frédéric Giroire,Emanuele Natale,Frederik Mallmann-Trenn*

Main category: cs.LG

TL;DR: The paper establishes an exponential separation between neuron pruning and weight pruning for approximating ReLU neurons, showing neuron pruning requires Ω(d/ε) neurons while weight pruning needs only O(d log(1/ε)) neurons.


<details>
  <summary>Details</summary>
Motivation: The Strong Lottery Ticket Hypothesis suggests pruning alone can find subnetworks that approximate target functions at initialization. While unstructured (weight) pruning has theoretical support, structured (neuron) pruning lacks theoretical understanding, particularly regarding its limitations compared to weight pruning.

Method: The authors analyze the problem of approximating a single bias-free ReLU neuron using a randomly initialized bias-free two-layer ReLU network. They isolate the intrinsic limitations of neuron pruning by focusing on this simplified setting and provide theoretical analysis comparing the requirements for neuron pruning versus weight pruning.

Result: Neuron pruning requires Ω(d/ε) hidden neurons to ε-approximate a target ReLU neuron, while weight pruning achieves ε-approximation with only O(d log(1/ε)) neurons. This establishes an exponential separation between the two pruning paradigms.

Conclusion: Neuron pruning is fundamentally less efficient than weight pruning for approximating target functions, requiring exponentially more neurons to achieve the same approximation accuracy. This theoretical limitation highlights the advantages of unstructured pruning over structured pruning in the context of the Strong Lottery Ticket Hypothesis.
...
### [242] [Talking with Verifiers: Automatic Specification Generation for Neural Network Verification](https://arxiv.org/abs/2603.02235)
*Yizhak Y. Elboher,Reuven Peleg,Zhouxing Shi,Guy Katz,Jan Křetínský*

Main category: cs.LG

TL;DR: A framework that translates natural language specifications into formal verification queries for neural networks, enabling verification of high-level semantic requirements using existing tools.
...
Method: Introduces a novel component to the verification pipeline that automatically analyzes natural language specifications and translates them into formal verification queries compatible with state-of-the-art neural network verifiers.
...
Conclusion: Substantially extends the applicability of formal DNN verification to real-world, high-level requirements by bridging the gap between natural language specifications and existing verification tools.
...
### [243] [CUDABench: Benchmarking LLMs for Text-to-CUDA Generation](https://arxiv.org/abs/2603.02236)
...
<details>
  <summary>Details</summary>
Motivation: Current benchmarks focus on high-level language to CUDA translation, overlooking the more challenging text-to-CUDA generation task. There's also a need for accurate performance assessment of LLM-generated GPU programs given hardware-specific and performance-critical requirements.

Method: Created CUDABench-Set covering Breadth-Depth-Difficulty evaluation space across AI, scientific computing, and data analytics domains. Proposed CUDABench-Score and Generative Verification Pipeline with three assessment components: compilation correctness, functional consistency through execution-based verification, and Performance-Score using roofline-based metrics.

Result: Benchmarking state-of-the-art LLMs revealed: 1) notable mismatch between high compilation success rates and low functional correctness, 2) lack of domain-specific algorithmic knowledge, and 3) suboptimal utilization of GPU hardware resources.

Conclusion: CUDABench provides a comprehensive evaluation framework for text-to-CUDA capabilities, revealing key challenges in LLM-generated GPU programming and offering insights for future improvements in this domain.
...
### [244] [Concept Heterogeneity-aware Representation Steering](https://arxiv.org/abs/2603.02237)
*Laziz U. Abdullaev,Noelle Y. L. Wong,Ryan T. Z. Lee,Shiqi Jiang,Khoi N. M. Nguyen,Tan M. Nguyen*

Main category: cs.LG

TL;DR: CHaRS improves representation steering in LLMs by using optimal transport between Gaussian mixture models instead of single global directions, enabling context-dependent control.


<details>
  <summary>Details</summary>
Motivation: Current representation steering methods use a single global direction from contrastive datasets, assuming homogeneous concept representation. However, LLM representations are actually non-homogeneous with clustered, context-dependent structure, making global steering brittle.

Method: Models source and target representations as Gaussian mixture models, formulates steering as discrete optimal transport between semantic latent clusters, derives input-dependent steering map via barycentric projection to produce kernel-weighted cluster-level shifts.

Result: CHaRS yields more effective behavioral control than global steering methods across numerous experimental settings.

Conclusion: Optimal transport between Gaussian mixture models provides a more effective framework for representation steering that accounts for concept heterogeneity in LLMs.
...
### [245] [Length Generalization Bounds for Transformers](https://arxiv.org/abs/2603.02238)
*Andy Yang,Pascal Bergsträßer,Georg Zetzsche,David Chiang,Anthony W. Lin*

Main category: cs.LG

TL;DR: The paper proves that computable length generalization bounds don't exist for CRASP/transformers (already with 2 layers), but provides computable bounds for positive CRASP/fixed-precision transformers with exponential complexity.


<details>
  <summary>Details</summary>
Motivation: Addressing the open problem of computability of length generalization bounds for CRASP (closely linked to transformers), where only partial positive results existed for limited cases.

Method: Theoretical analysis of CRASP language class, proving non-existence of computable bounds for general CRASP/transformers, while providing constructive computable bounds for positive CRASP/fixed-precision transformers.

Result: Main result: No computable length generalization bounds exist for CRASP (even with 2 layers) and transformers. Complementary result: Computable bounds exist for positive CRASP/fixed-precision transformers with exponential complexity, proven optimal.

Conclusion: Complete answers to the open problem: general CRASP/transformers lack computable generalization bounds, while positive CRASP/fixed-precision transformers have computable (exponential) bounds, establishing fundamental limits on length generalization guarantees.
...
### [246] [High-order Knowledge Based Network Controllability Robustness Prediction: A Hypergraph Neural Network Approach](https://arxiv.org/abs/2603.02265)
*Shibing Mo,Jiarui Zhang,Jiayu Xie,Xiangyi Teng,Jing Liu*

Main category: cs.LG

TL;DR: NCR-HoK: A dual hypergraph attention neural network that predicts network controllability robustness by incorporating high-order structural information, outperforming existing methods with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for evaluating network controllability robustness rely on time-consuming attack simulations that only work for small-scale networks. Existing ML approaches focus on pairwise interactions but ignore high-order structural information that could be crucial for understanding controllability robustness.

Method: Proposes NCR-HoK, a dual hypergraph attention neural network model that uses: 1) node feature encoder, 2) hypergraph construction with high-order relations, and 3) dual hypergraph attention module. This approach learns three types of network information simultaneously: explicit graph structure, high-order local neighborhood connections, and hidden embedding features.

Result: The method achieves superior performance compared to state-of-the-art network robustness learning methods on both synthetic and real-world networks, with low computational overhead. It's the first exploration of high-order knowledge's impact on network controllability robustness.

Conclusion: High-order structural information is valuable for predicting network controllability robustness. The proposed NCR-HoK model effectively captures this information and provides accurate predictions with computational efficiency, addressing limitations of traditional simulation-based approaches.
...
### [247] [Boosting Meta-Learning for Few-Shot Text Classification via Label-guided Distance Scaling](https://arxiv.org/abs/2603.02267)
*Yunlong Gao,Xinyue Liu,Yingbo Wang,Linlin Zong,Bo Xu*

Main category: cs.LG

TL;DR: LDS is a label-guided distance scaling strategy for few-shot text classification that leverages label semantics as supervision signals in both training and testing stages to improve classification accuracy.
...
Method: Proposes Label-guided Distance Scaling (LDS) with two components: 1) Training stage - label-guided loss to inject label semantic information and align sample representations with corresponding label representations; 2) Testing stage - Label-guided Scaler that scales sample representations with label semantics to provide additional supervision signals, pulling misaligned samples closer to class centers.
...
Conclusion: LDS effectively mitigates misclassification in few-shot text classification by exploiting label semantics as supervision signals throughout both training and testing stages, providing a simple yet effective enhancement to existing meta-learners.
...
### [248] [PRISM: Exploring Heterogeneous Pretrained EEG Foundation Model Transfer to Clinical Differential Diagnosis](https://arxiv.org/abs/2603.02268)
*Jeet Bandhu Lahiri,Parshva Runwal,Arvasu Kulkarni,Mahir Jain,Aditya Ray Mishra,Siddharth Panwar,Sandeep Singh*

Main category: cs.LG

TL;DR: PRISM EEG foundation model shows diverse pretraining beats narrow-source clinical data for adaptability, especially on challenging epilepsy diagnosis tasks, while revealing benchmarking inconsistencies.


<details>
  <summary>Details</summary>
Motivation: Current EEG foundation models are pretrained on narrow-source clinical archives and evaluated on same-distribution benchmarks, making it unclear whether they learn neural physiology or recording artifacts. Need to understand effects of pretraining population diversity versus scale.

Method: PRISM - masked autoencoder ablated along two axes: pretraining population (narrow EU/US vs diverse multi-center South Asian clinical recordings) and downstream adaptation. Fixed architecture/preprocessing. Compare narrow-source corpus (TUH + PhysioNet) against geographically diverse pool with multiple EEG systems.

Result: 1) Narrow-source pretraining better on distribution-matched benchmarks, but diverse pretraining more adaptable under fine-tuning. 2) PRISM matches/outperforms REVE (92 datasets) on most tasks with targeted diversity. 3) Diverse checkpoint outperforms narrow-source by +12.3 pp on epilepsy vs mimickers task. 4) EEG-Bench vs EEG-FM-Bench inconsistencies reverse model rankings by up to 24 pp due to six compounding factors.

Conclusion: Targeted diversity can substitute for indiscriminate scale in EEG foundation models. Dataset count is confounding variable in model comparison. Diverse pretraining crucial for challenging clinical tasks like epilepsy diagnosis. Benchmarking inconsistencies significantly impact model evaluation.
...
### [249] [Graph Attention Based Prioritization of Disease Responsible Genes from Multimodal Alzheimer's Network](https://arxiv.org/abs/2603.02273)
*Binon Teji,Subhajit Bandyopadhyay,Swarup Roy*

Main category: cs.LG

TL;DR: NETRA is a multimodal graph transformer framework that uses attention-driven scoring instead of traditional centrality measures to prioritize disease-associated genes, demonstrated on Alzheimer's disease with superior performance.


<details>
  <summary>Details</summary>
Motivation: Traditional network-based approaches for gene prioritization rely on static centrality measures and fail to capture cross-modal biological heterogeneity, limiting their ability to identify disease-relevant genes in complex disorders like Alzheimer's disease.

Method: Constructs gene regulatory networks from multiple data types (microarray, scRNA-seq, snRNA-seq), trains BERT-based model on random-walk sequences for global embeddings, compresses expression profiles with VAEs, integrates with auxiliary biological networks, and uses graph transformer for attention-driven scoring.

Result: Achieves normalized enrichment score of ~3.9 for Alzheimer's disease pathway, substantially outperforming classical centrality measures and diffusion models. Top-ranked genes enrich neurodegenerative pathways, recover known AD susceptibility locus, and reveal conserved cross-disease gene modules.

Conclusion: NETRA provides a superior, context-aware approach to gene prioritization that captures biological heterogeneity, preserves realistic network topology, and is extensible to other complex disorders beyond Alzheimer's disease.
...
### [250] [A Comparative Study of UMAP and Other Dimensionality Reduction Methods](https://arxiv.org/abs/2603.02275)
*Guanzhe Zhang,Shanshan Ding,Zhezhen Jin*

Main category: cs.LG

TL;DR: Supervised UMAP performs well for classification but has limitations for regression tasks in effectively incorporating response information.


<details>
  <summary>Details</summary>
Motivation: UMAP is widely used for manifold learning but its supervised extensions, especially for regression, remain underexplored. The paper aims to systematically evaluate supervised UMAP against other dimensionality reduction methods.

Method: Comprehensive comparative analysis of UMAP, supervised UMAP, PCA, Kernel PCA, Sliced Inverse Regression (SIR), Kernel SIR, and t-SNE. Systematic evaluation using simulated and real datasets with performance assessed via predictive accuracy on low-dimensional embeddings.

Result: Supervised UMAP performs well for classification tasks but shows limitations in effectively incorporating response information for regression settings.

Conclusion: Supervised UMAP is effective for classification but needs improvement for regression, highlighting an important direction for future development in supervised dimensionality reduction methods.
...
### [251] [Temporal Imbalance of Positive and Negative Supervision in Class-Incremental Learning](https://arxiv.org/abs/2603.02280)
*Jinge Ma,Fengqing Zhu*

Main category: cs.LG

TL;DR: The paper identifies temporal imbalance as a key cause of prediction bias in Class-Incremental Learning and proposes Temporal-Adjusted Loss (TAL) to mitigate forgetting by dynamically reweighting negative supervision.


<details>
  <summary>Details</summary>
Motivation: Existing CIL methods focus on intra-task class imbalance but overlook temporal imbalance - earlier classes receive stronger negative supervision toward training end, causing asymmetric precision/recall and prediction bias toward new classes.

Method: Proposes Temporal-Adjusted Loss (TAL) which uses a temporal decay kernel to construct supervision strength vector and dynamically reweight negative supervision in cross-entropy loss, theoretically degenerating to standard CE under balanced conditions.
...
Conclusion: Temporal imbalance is a crucial factor in CIL forgetting, and TAL effectively addresses this by adjusting supervision strength over time, providing a principled approach to mitigate prediction bias in incremental learning scenarios.
...
### [252] [Quantum-Inspired Fine-Tuning for Few-Shot AIGC Detection via Phase-Structured Reparameterization](https://arxiv.org/abs/2603.02281)
*Kaiyang Xing,Han Fang,Zhaoyun Chen,Zhonghui Li,Yang Yang,Weiming Zhang,Guoping Guo*

Main category: cs.LG

TL;DR: Q-LoRA integrates quantum neural networks into LoRA adapters for few-shot AIGC detection, outperforming standard LoRA. H-LoRA is a classical variant using Hilbert transform to achieve similar benefits without quantum overhead.


<details>
  <summary>Details</summary>
Motivation: To extend quantum neural networks' few-shot generalization advantage to large-scale tasks by integrating lightweight QNNs into LoRA adapters for AI-generated content detection.

Method: Proposes Q-LoRA (quantum-enhanced fine-tuning) that integrates lightweight QNNs into LoRA adapters. Also introduces H-LoRA, a classical variant that applies Hilbert transform within LoRA to mimic quantum structural biases.

Result: Both Q-LoRA and H-LoRA outperform standard LoRA by over 5% accuracy on few-shot AIGC detection. H-LoRA achieves comparable accuracy to Q-LoRA at significantly lower computational cost.

Conclusion: Quantum structural inductive biases (phase-aware representations and norm-constrained transformations) improve few-shot learning, and these benefits can be approximated classically via Hilbert transform for practical deployment.
...
### [253] [The Malignant Tail: Spectral Segregation of Label Noise in Over-Parameterized Networks](https://arxiv.org/abs/2603.02293)
*Zice Wang*

Main category: cs.LG

TL;DR: Networks under label noise develop a "Malignant Tail" where signal and noise become geometrically separated - signal in low-rank subspaces, noise in high-frequency orthogonal components. SGD fails to suppress noise but biases it into high-frequency subspaces, enabling post-hoc spectral truncation to recover optimal generalization.


<details>
  <summary>Details</summary>
Motivation: To understand the geometric mechanism behind the phase transition from benign to harmful overfitting under increasing noise-to-signal ratios, and to develop interventions that can recover optimal generalization from converged models without temporal instability.

Method: 1) Identify the "Malignant Tail" failure mode through experimental isolation; 2) Use Spectral Linear Probe to analyze training dynamics; 3) Demonstrate SGD's implicit bias toward segregating noise into high-frequency orthogonal subspaces; 4) Apply post-hoc Explicit Spectral Truncation (d << D) to prune noise-dominated subspaces; 5) Compare with temporal early stopping for stability.

Result: 1) Networks functionally segregate signal and noise geometrically; 2) SGD preserves signal-noise separability by biasing noise toward high-frequency orthogonal components; 3) Explicit Spectral Truncation can surgically remove noise-dominated subspaces; 4) This geometric approach recovers optimal generalization capability from converged models; 5) Unlike unstable temporal early stopping, Geometric Truncation provides stable post-hoc intervention.

Conclusion: Under label noise, excess spectral capacity becomes a latent structural liability enabling noise memorization. Explicit rank constraints are necessary to filter stochastic corruptions for robust generalization. The geometric separation mechanism provides a stable alternative to temporal interventions for recovering optimal performance from overfitted models.
...
### [254] [Preconditioned Score and Flow Matching](https://arxiv.org/abs/2603.02337)
*Shadab Ahamed,Eshed Gal,Simon Ghyselincks,Md Shahriar Rahim Siddiqui,Moshe Eliasof,Eldad Haber*

Main category: cs.LG

TL;DR: The paper shows that ill-conditioned covariance in intermediate distributions during flow matching/diffusion training causes optimization bias, where high-variance directions are learned quickly while low-variance modes are under-optimized, leading to suboptimal plateaus. The authors propose reversible label-conditional preconditioning maps to improve covariance conditioning and avoid these plateaus.


<details>
  <summary>Details</summary>
Motivation: During flow matching and score-based diffusion training, the geometry of intermediate distributions p_t (specifically their covariance Σ_t) strongly affects optimization. When Σ_t is ill-conditioned, gradient-based training rapidly fits high-variance directions while systematically under-optimizing low-variance modes, causing learning to plateau at suboptimal weights.

Method: The authors formalize this optimization bias effect in analytically tractable settings and propose reversible, label-conditional preconditioning maps that reshape the geometry of p_t by improving the conditioning of Σ_t without altering the underlying generative model. Preconditioning primarily mitigates optimization stagnation by enabling continued progress along previously suppressed directions.

Result: Across MNIST latent flow matching and additional high-resolution datasets, the authors empirically track conditioning diagnostics and distributional metrics, showing that preconditioning consistently yields better-trained models by avoiding suboptimal plateaus.

Conclusion: Preconditioning maps that improve the conditioning of intermediate distribution covariances effectively address optimization bias in flow matching and diffusion models, preventing suboptimal plateaus and enabling better model training across various datasets.
...
### [255] [Diffusion-MPC in Discrete Domains: Feasibility Constraints, Horizon Effects, and Critic Alignment: Case study with Tetris](https://arxiv.org/abs/2603.02348)
*Haochuan Kevin Wang*

Main category: cs.LG

TL;DR: Diffusion-MPC for Tetris shows feasibility masking is essential in discrete domains, DQN critics misalign with rollout quality, and shorter planning horizons work better under sparse rewards.


<details>
  <summary>Details</summary>
Motivation: To understand how diffusion-based model predictive control performs in discrete combinatorial domains, using Tetris as a case study to analyze key factors affecting performance.

Method: Uses Diffusion-MPC with MaskGIT-style discrete denoiser to sample candidate placement sequences, then selects actions via reranking. Analyzes three factors: feasibility-constrained sampling via logit masking, reranking strategies (heuristic, DQN critic, hybrid), and compute scaling in candidate count (K) and planning horizon (H).

Result: Feasibility masking removes 46% invalid action mass, improving score by 6.8% and survival by 5.6%. DQN reranking shows systematic misalignment with high decision regret (mean 17.6, p90 36.6). Shorter planning horizons outperform longer ones under sparse rewards. Compute choices determine failure modes: small K limits candidate quality, larger H amplifies misranking and model mismatch.

Conclusion: Highlights structural challenges of diffusion planners in discrete environments and provides practical diagnostics for critic integration. Feasibility constraints are necessary, critic alignment is problematic, and planning horizon length must match reward structure.
...
### [256] [Learning graph topology from metapopulation epidemic encoder-decoder](https://arxiv.org/abs/2603.02349)
*Xin Li,Jonathan Cohen,Shai Pilosof,Rami Puzis*

Main category: cs.LG

TL;DR: Deep learning architectures for joint inference of epidemic parameters and metapopulation mobility networks from time-series data, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional metapopulation epidemic models require both epidemic parameters and mobility networks, but joint inference is challenging with limited tracing data. Current methods can only estimate one while assuming the other.

Method: Two encoder-decoder deep learning architectures that infer metapopulation mobility graphs from time-series data, with and without assuming epidemic model parameters.

Result: The approach outperforms state-of-the-art topology inference across diverse random and empirical mobility networks. Topology inference improves dramatically with data on additional pathogens.

Conclusion: Establishes a robust framework for simultaneously inferring epidemic parameters and topology, addressing a persistent gap in disease propagation modeling.
...
### [257] [Learning Optimal Search Strategies](https://arxiv.org/abs/2603.02356)
*Stefan Ankirchner,Maximilian Philipp Thiel*

Main category: cs.LG

TL;DR: The paper proposes an algorithm for learning optimal parking search strategies in unknown inhomogeneous Poisson processes, achieving logarithmic regret with minimax optimality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop efficient learning algorithms for optimal stopping problems in unknown stochastic environments, specifically addressing parking search where parking opportunities arrive according to an unknown inhomogeneous Poisson process.

Method: The method proposes an algorithm that learns the optimal threshold-type stopping rule by estimating the integrated jump intensity rather than the intensity function itself. This approach avoids the need to estimate the full intensity function.

Result: The algorithm achieves logarithmic regret growth uniformly over a broad class of environments. The paper also proves a logarithmic minimax regret lower bound, establishing the growth optimality of the proposed approach.

Conclusion: The proposed algorithm is minimax optimal for learning optimal stopping policies in unknown inhomogeneous Poisson processes, with logarithmic regret that cannot be improved upon in terms of growth rate.
...
### [258] [Rigidity-Aware Geometric Pretraining for Protein Design and Conformational Ensembles](https://arxiv.org/abs/2603.02406)
*Zhanghan Ni,Yanjing Li,Zeju Qiu,Bernhard Schölkopf,Hongyu Guo,Weiyang Liu,Shengchao Liu*

Main category: cs.LG

TL;DR: RigidSSL is a geometric pretraining framework that uses rigidity-aware self-supervised learning to improve protein design by learning geometric priors from protein structures and molecular dynamics trajectories.


<details>
  <summary>Details</summary>
Motivation: Current generative models for protein design have three limitations: (1) inability to jointly learn protein geometry and design tasks, (2) reliance on local, non-rigid atomic representations that limit global geometric understanding, and (3) ineffective modeling of dynamic and conformational information of protein structures.

Method: Two-phase geometric pretraining framework: Phase I (RigidSSL-Perturb) learns geometric priors from 432K AlphaFold structures with simulated perturbations. Phase II (RigidSSL-MD) refines representations on 1.3K molecular dynamics trajectories. Uses bi-directional, rigidity-aware flow matching objective that jointly optimizes translational and rotational dynamics to maximize mutual information between conformations.

Result: RigidSSL variants improve designability by up to 43%, enhance novelty and diversity in unconditional generation. RigidSSL-Perturb improves zero-shot motif scaffolding success rate by 5.8%. RigidSSL-MD captures more biophysically realistic conformational ensembles in G protein-coupled receptor modeling.

Conclusion: RigidSSL effectively addresses limitations of current protein design methods by front-loading geometry learning through rigidity-aware self-supervised pretraining, leading to improved designability, novelty, and biophysical realism in protein generation tasks.
...
### [259] [Personalized Multi-Agent Average Reward TD-Learning via Joint Linear Approximation](https://arxiv.org/abs/2603.02426)
*Leo,Wang,Pengkun Yang,Lili Su*

Main category: cs.LG

TL;DR: Personalized multi-agent TD learning with shared linear representation where agents' optimal weights lie in unknown subspace, achieving linear speedup by filtering conflicting signals.


<details>
  <summary>Details</summary>
Motivation: To enable multiple agents interacting with different environments to jointly learn their value functions efficiently by leveraging shared structure, inspired by personalized federated learning approaches.

Method: Cooperative single-timescale TD learning where agents iteratively estimate common subspace and local heads, decomposing learning to filter out conflicting signals.

Result: The decomposition effectively mitigates negative impacts of "misaligned" signals and achieves linear speedup, with experiments showing benefits for general control problems.

Conclusion: Learning via shared structure in personalized multi-agent TD learning can filter conflicting signals and achieve efficiency gains, with analytical techniques useful for future research on leveraging common structures.
...
### [260] [Dimension-Independent Convergence of Underdamped Langevin Monte Carlo in KL Divergence](https://arxiv.org/abs/2603.02429)
*Shiyuan Zhang,Qiwei Di,Xuheng Li,Quanquan Gu*

Main category: cs.LG

TL;DR: First dimension-free KL divergence bounds for discretized underdamped Langevin dynamics, improving convergence guarantees when trace(Hessian) ≪ dimension.


<details>
  <summary>Details</summary>
Motivation: Existing convergence guarantees for discretized underdamped Langevin dynamics (ULD) scale polynomially with ambient dimension d, leading to vacuous bounds in high dimensions. While dimension-free results exist in Wasserstein-2 distance, dimension-independent guarantees for ULD discretizations in KL divergence have remained open.

Method: Refines the KL local error framework (Altschuler et al., 2025) to a dimension-free setting, obtaining bounds that depend on tr(H) where H upper bounds the Hessian of V, rather than on dimension d.

Result: Proves first dimension-free KL divergence bounds for discretized ULD, yielding improved iteration complexity for underdamped Langevin Monte Carlo relative to overdamped methods when tr(H) ≪ d.

Conclusion: Closes the gap in dimension-independent guarantees for ULD discretizations, providing theoretical foundation for the empirical effectiveness of underdamped Langevin dynamics in high-dimensional sampling problems.
...
### [261] [A Unified Revisit of Temperature in Classification-Based Knowledge Distillation](https://arxiv.org/abs/2603.02430)
*Logan Frank,Jim Davis*

Main category: cs.LG

TL;DR: Temperature selection in knowledge distillation is complex and depends on training components like optimizer and teacher pretraining, requiring systematic analysis rather than grid search.


<details>
  <summary>Details</summary>
Motivation: Despite widespread use of temperature in knowledge distillation, there's limited understanding of how to select appropriate temperature values and how they interact with other training components like optimizer and teacher pretraining/finetuning.

Method: The paper presents a unified study that systematically examines interactions between temperature and other training components in knowledge distillation, analyzing cross-connections to identify common situations affecting temperature selection.

Result: The study identifies common situations that have a pronounced impact on temperature selection, providing valuable guidance for practitioners using knowledge distillation.

Conclusion: Temperature selection in knowledge distillation is closely linked to various training components, and systematic analysis of these interactions can provide better guidance than grid search or copying values from prior work.
...
### [262] [What Capable Agents Must Know: Selection Theorems for Robust Decision-Making under Uncertainty](https://arxiv.org/abs/2603.02491)
*Aran Nayebi*

Main category: cs.LG

TL;DR: The paper proves that low regret on prediction tasks forces agents to develop predictive internal states, addressing whether belief states/world models are necessary (not just sufficient) for competent action under uncertainty.


<details>
  <summary>Details</summary>
Motivation: To determine what internal structure is necessary (not just sufficient) for agents to act competently under uncertainty, addressing whether belief states or world models are truly required rather than just implementable.

Method: Proves quantitative "selection theorems" showing low average-case regret on structured families of action-conditioned prediction tasks forces predictive internal states. Reduces predictive modeling to binary betting decisions and shows regret bounds limit probability mass on suboptimal bets.

Result: In fully observed settings: approximate recovery of interventional transition kernel. Under partial observability: necessity of belief-like memory and predictive state, addressing an open question in prior world-model recovery work.

Conclusion: Low regret on structured prediction tasks forces agents to implement predictive, structured internal states, showing such representations are necessary (not just implementable) for competent action under uncertainty.
...
### [263] [Using the SEKF to Transfer NN Models of Dynamical Systems with Limited Data](https://arxiv.org/abs/2603.02439)
*Joshua E. Hammond,Tyler A. Soderstrom,Brian A. Korgel,Michael Baldea*

Main category: cs.LG

TL;DR: SEKF enables adaptation of pre-trained neural network models to similar systems with only 1% of original training data.


<details>
  <summary>Details</summary>
Motivation: Data-driven models need extensive training data, but gathering sufficient data is often infeasible due to cost or safety concerns in practical applications.

Method: Uses Subset Extended Kalman Filter (SEKF) to adapt pre-trained neural network models to new, similar systems with limited data by applying small parameter perturbations to the initial model.

Result: Experimental validation shows the method captures target system dynamics with as little as 1% of original training data, requires less computational cost, and reduces generalization error.

Conclusion: SEKF provides an effective approach for adapting pre-trained models to similar systems with minimal data, addressing practical limitations in data collection for dynamical systems.
...
### [264] [Spectral Regularization for Diffusion Models](https://arxiv.org/abs/2603.02447)
*Satish Chandran,Nicolas Roque dos Santos,Yunshu Wu,Greg Ver Steeg,Evangelos Papalexakis*

Main category: cs.LG

TL;DR: Spectral regularization for diffusion models using Fourier- and wavelet-domain losses to improve sample quality without modifying architecture or sampling.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion models use pointwise reconstruction objectives that ignore the spectral and multi-scale structure of natural signals, which can lead to suboptimal sample quality, especially for high-resolution generation where fine-scale structure is challenging.

Method: Augment standard diffusion training with differentiable Fourier- and wavelet-domain losses as soft inductive biases. The approach doesn't modify diffusion process, model architecture, or sampling procedure, and is compatible with DDPM, DDIM, and EDM formulations with negligible computational overhead.

Result: Experiments on image and audio generation show consistent improvements in sample quality, with largest gains on higher-resolution, unconditional datasets where fine-scale structure is most challenging to model.

Conclusion: Spectral regularization provides an effective way to incorporate frequency balance and coherent multi-scale structure into diffusion models, improving sample quality without architectural changes or significant computational cost.
...
### [265] [Manifold Aware Denoising Score Matching (MAD)](https://arxiv.org/abs/2603.02452)
*Alona Levy-Jurgenson,Alvaro Prat,James Cuin,Yee Whye Teh*

Main category: cs.LG

TL;DR: A method that modifies denoising score-matching to implicitly account for manifolds, reducing computational burden while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: Current methods for learning distributions on manifolds require learning the manifold structure, which is computationally intensive. The paper aims to reduce this computational burden while maintaining efficiency.

Method: Proposes a simple modification to denoising score-matching by decomposing the score function into a known component (s^{base}) and a remainder component (s-s^{base}). The known component implicitly includes manifold information, reducing the need to learn the manifold structure.

Result: Derives analytical forms for s^{base} for several important cases including distributions over rotation matrices and discrete distributions, demonstrating the utility of the approach.

Conclusion: The proposed decomposition approach effectively reduces the computational burden of learning manifold structures while maintaining the efficiency of denoising score-matching methods.
...
### [266] [Real-Time Generative Policy via Langevin-Guided Flow Matching for Autonomous Driving](https://arxiv.org/abs/2603.02613)
*Tianze Zhu,Yinuo Wang,Wenjun Zou,Tianyi Zhang,Likun Wang,Letian Tao,Feihong Zhang,Yao Lyu,Shengbo Eben Li*

Main category: cs.LG

TL;DR: DACER-F is a novel RL algorithm that combines flow matching with diffusion actor-critic to achieve ultra-low inference latency while maintaining high performance in autonomous driving and standard RL benchmarks.


<details>
  <summary>Details</summary>
Motivation: Generative policies in RL show promise for autonomous driving but suffer from high inference latency that prevents real-time deployment. Current diffusion-based methods are too slow for practical applications.

Method: Introduces flow matching into online RL, using Langevin dynamics and Q-function gradients to optimize actions from experience replay toward a target distribution balancing high Q-values and exploration. The flow policy learns to map from a simple prior to this dynamic target in a single inference step.

Result: Outperforms baselines DACER and DSAC in complex multi-lane and intersection simulations while maintaining ultra-low inference latency. Achieves 775.8 score on DMC humanoid-stand task, surpassing prior methods.

Conclusion: DACER-F establishes itself as a high-performance, computationally efficient RL algorithm suitable for real-time applications like autonomous driving while maintaining competitive performance on standard benchmarks.
...
### [267] [Can Computational Reducibility Lead to Transferable Models for Graph Combinatorial Optimization?](https://arxiv.org/abs/2603.02462)
*Semih Cantürk,Thomas Sabourin,Frederik Wenkel,Michael Perlmutter,Guy Wolf*

Main category: cs.LG

TL;DR: The paper proposes a unified neural solver for combinatorial optimization using GCON message passing with energy-based unsupervised loss, achieving SOTA performance on individual tasks and demonstrating effective transfer learning between related CO problems through informed pretraining strategies.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of generalizing neural solvers for combinatorial optimization between tasks, enabling transfer learning and multi-task learning across different CO problems.

Method: Uses GCON module for expressive message passing with energy-based unsupervised loss functions. Proposes pretraining and fine-tuning strategies informed by computational reducibility literature to transfer knowledge between related CO problems (MVC, MIS, MaxClique, MaxCut, MDS, graph coloring).

Result: Achieves high performance comparable to state-of-the-art on individual CO tasks. Demonstrates effective transfer learning between related problems and shows that pretraining on all but one task leads to faster convergence while avoiding negative transfer in multi-task learning settings.

Conclusion: Learning common representations across multiple graph CO problems is viable through expressive message passing and informed pretraining strategies, representing an important step toward foundational models for neural combinatorial optimization.
...
### [268] [Improving Diffusion Planners by Self-Supervised Action Gating with Energies](https://arxiv.org/abs/2603.02650)
*Yuan Lu,Dongqi Han,Yansen Wang,Dongsheng Li*

Main category: cs.LG

TL;DR: SAGE improves diffusion planners by re-ranking trajectories using latent consistency signals to penalize dynamically inconsistent plans, enhancing performance without environment rollouts or retraining.


<details>
  <summary>Details</summary>
Motivation: Diffusion planners in offline RL can fail when value-guided selection favors trajectories that score well but are locally inconsistent with environment dynamics, leading to brittle execution.

Method: SAGE trains a JEPA encoder on offline state sequences and an action-conditioned latent predictor for short horizon transitions. At test time, it assigns each candidate an energy based on latent prediction error and combines this feasibility score with value estimates to select actions.

Result: SAGE improves performance and robustness of diffusion planners across locomotion, navigation, and manipulation benchmarks without requiring environment rollouts or policy re-training.

Conclusion: SAGE provides an effective inference-time re-ranking method that enhances diffusion planners by incorporating dynamic consistency signals, making them more robust and performant in offline RL settings.
...
### [269] [MUSE: A Run-Centric Platform for Multimodal Unified Safety Evaluation of Large Language Models](https://arxiv.org/abs/2603.02482)
*Zhongxi Wang,Yueqian Lin,Jingyang Zhang,Hai Helen Li,Yiran Chen*

Main category: cs.LG

TL;DR: MUSE is an open-source multimodal safety evaluation platform that introduces cross-modal attack strategies and modality switching to test whether LLM alignment generalizes beyond text to audio, image, and video inputs.


<details>
  <summary>Details</summary>
Motivation: Current safety evaluation of LLMs is text-centric, lacking infrastructure to systematically test whether alignment generalizes to multimodal inputs (audio, image, video). There's a need for comprehensive cross-modal safety testing frameworks.

Method: MUSE platform integrates: 1) automatic cross-modal payload generation, 2) three multi-turn attack algorithms (Crescendo, PAIR, Violent Durian), 3) provider-agnostic model routing, 4) LLM judge with five-level safety taxonomy, 5) dual-metric framework distinguishing hard vs soft Attack Success Rate, and 6) Inter-Turn Modality Switching (ITMS) for per-turn modality rotation.

Result: Experiments across six multimodal LLMs from four providers show multi-turn strategies achieve up to 90-100% ASR against models with near-perfect single-turn refusal. ITMS doesn't uniformly raise final ASR but accelerates convergence by destabilizing early-turn defenses. Modality effects are model-family-specific rather than universal.

Conclusion: Multimodal safety testing reveals vulnerabilities not captured by text-only evaluation. The model-family-specific nature of modality effects underscores the need for provider-aware cross-modal safety testing rather than universal assumptions about modality vulnerabilities.
...
### [270] [ParEVO: Synthesizing Code for Irregular Data: High-Performance Parallelism through Agentic Evolution](https://arxiv.org/abs/2603.02510)
*Liu Yang,Zeyu Nie,Andrew Liu,Felix Zou,Deniz Altinbüken,Amir Yazdanbakhsh,Quanquan C. Liu*

Main category: cs.LG

TL;DR: ParEVO is a framework that synthesizes high-performance parallel algorithms for irregular data structures using evolutionary coding agents and specialized LLMs, achieving up to 1103x speedup over sequential code.


<details>
  <summary>Details</summary>
Motivation: Parallel programming for irregular data structures (sparse graphs, unbalanced trees, non-uniform meshes) is challenging due to unpredictable data dependencies and static scheduling failures. Current LLMs often generate buggy parallel code with race conditions and deadlocks.

Method: Three main components: (1) Parlay-Instruct Corpus - 13,820 tasks synthesized via "Critic-Refine" pipeline filtering for empirically performant algorithms using Work-Span primitives; (2) Specialized LLMs (DeepSeek, Qwen, Gemini) fine-tuned for ParlayLib semantics; (3) Evolutionary Coding Agent that iteratively repairs code using compiler, race detector, and profiler feedback.

Result: Achieves average 106x speedup (max 1103x) on ParEval benchmark, with 13.6x speedup specifically on complex irregular graph problems, outperforming state-of-the-art commercial models. Matches expert human baselines with up to 4.1x speedup on highly-irregular kernels.

Conclusion: ParEVO successfully bridges the gap in parallel programming for irregular data structures by combining curated datasets, specialized LLMs, and evolutionary code repair, demonstrating significant performance improvements over existing approaches.
...
### [271] [Thermodynamic Regulation of Finite-Time Gibbs Training in Energy-Based Models: A Restricted Boltzmann Machine Study](https://arxiv.org/abs/2603.02525)
*Görkem Can Süleymanoğlu*

Main category: cs.LG

TL;DR: The paper introduces a thermodynamic regulation framework for RBMs where temperature evolves dynamically during training to prevent sampling collapse and parameter drift that occurs with fixed-temperature training.


<details>
  <summary>Details</summary>
Motivation: Fixed-temperature training of RBMs assumes the stochastic regime remains valid as the energy landscape evolves, but this assumption can break down during finite-time training, leading to Gibbs sampler freezing, negative phase localization, and parameter drift.

Method: Proposes an endogenous thermodynamic regulation framework where temperature evolves as a dynamical state variable coupled to measurable sampling statistics. Uses two-time-scale separation regime with global parameter boundedness under L2 regularization, and proves local exponential stability of the thermodynamic subsystem.

Result: The self-regulated RBM improves normalization stability and effective sample size relative to fixed-temperature baselines on MNIST while preserving reconstruction performance. The regulated regime mitigates inverse-temperature blow-up and freezing-induced degeneracy.

Conclusion: RBM training should be reinterpreted as a controlled non-equilibrium dynamical process rather than a static equilibrium approximation, with thermodynamic regulation addressing structural fragility in finite-time training.
...
### [272] [Bridging Diffusion Guidance and Anderson Acceleration via Hopfield Dynamics](https://arxiv.org/abs/2603.02531)
*Kwanyoung Kim*

Main category: cs.LG

TL;DR: The paper proposes Geometry Aware Attention Guidance (GAG), a plug-and-play method that improves diffusion model generation by theoretically grounding attention-space extrapolation through Modern Hopfield Networks and Anderson Acceleration.
...
Method: Models attention dynamics as fixed-point iterations in Modern Hopfield Networks, shows attention extrapolation is a special case of Anderson Acceleration, and proposes GAG that decomposes attention updates into parallel/orthogonal components relative to guidance direction.

Result: GAG stabilizes the acceleration process, maximizes guidance efficiency, seamlessly integrates with existing frameworks, and significantly improves generation quality.

Conclusion: The paper establishes a theoretical foundation for attention-space extrapolation and introduces GAG as an effective, plug-and-play guidance method that addresses computational efficiency while maintaining strong theoretical grounding.
...
### [273] [EdgeFLow: Serverless Federated Learning via Sequential Model Migration in Edge Networks](https://arxiv.org/abs/2603.02562)
*Yuchen Shi,Qijun Hou,Pingyi Fan,Khaled B. Letaief*

Main category: cs.LG

TL;DR: EdgeFLow is a novel federated learning framework that replaces cloud servers with sequential model migration between edge base stations, eliminating cloud transmissions and reducing communication overhead while maintaining comparable accuracy.


<details>
  <summary>Details</summary>
Motivation: Federated Learning faces significant communication bottlenecks due to client-server data exchanges and long-distance transmissions in IoT environments. Traditional FL systems rely on cloud servers that create communication inefficiencies.

Method: EdgeFLow redesigns FL system topology by replacing cloud servers with sequential model migration between edge base stations. It conducts model aggregation and propagation exclusively at edge clusters, eliminating cloud-based transmissions.

Result: The framework provides rigorous convergence analysis under non-convex objectives and non-IID data distributions. Experiments show EdgeFLow achieves comparable accuracy improvements while significantly reducing communication costs.

Conclusion: EdgeFLow establishes a foundational framework for communication-efficient FL, representing a systemic architectural innovation for future IoT and edge-network learning systems.
...
### [274] [Wasserstein Proximal Policy Gradient](https://arxiv.org/abs/2603.02576)
*Zhaoyu Zhu,Shuhan Zhang,Rui Gao,Shuang Li*

Main category: cs.LG

TL;DR: Wasserstein Proximal Policy Gradient (WPPG) is a new policy gradient method for continuous-action RL that uses Wasserstein geometry and avoids policy density evaluation, enabling use with expressive implicit policies.


<details>
  <summary>Details</summary>
Motivation: The paper aims to develop policy gradient methods for continuous-action RL that can work with expressive implicit stochastic policies (specified as pushforward maps) without needing to evaluate policy log densities or gradients, which is challenging for implicit policies.

Method: Derives WPPG from Wasserstein proximal updates via operator-splitting: alternates optimal transport updates with heat steps (Gaussian convolution). This avoids policy density evaluation and works directly with pushforward maps.

Result: Establishes global linear convergence rate for WPPG, covering both exact policy evaluation and actor-critic implementations with controlled error. Empirically simple to implement and achieves competitive performance on continuous-control benchmarks.

Conclusion: WPPG provides a theoretically grounded, practical method for continuous-action RL that works with expressive implicit policies via Wasserstein geometry, avoiding density evaluation challenges while maintaining competitive performance.
...
### [275] [Towards Parameter-Free Temporal Difference Learning](https://arxiv.org/abs/2603.02577)
...
<details>
  <summary>Details</summary>
Motivation: Existing finite-time analyses of TD learning require impractical problem-dependent parameters (minimum eigenvalue ω, mixing time τ_mix) and nonstandard modifications, creating a gap between theory and practice.

Method: Use exponential step-size schedule with standard TD(0) algorithm. For i.i.d. sampling, no problem-dependent knowledge needed. For Markovian sampling, propose regularized TD(0) with exponential step-size schedule.

Result: In i.i.d. setting: achieves optimal bias-variance trade-off for last iterate without knowledge of ω. In Markovian setting: achieves comparable convergence rate without requiring projections, iterate averaging, or knowledge of τ_mix or ω.

Conclusion: Exponential step-size schedule bridges theory-practice gap in TD learning by eliminating need for impractical problem-dependent parameters while maintaining strong convergence guarantees.
...
### [276] [Joint Optimization of Model Partitioning and Resource Allocation for Anti-Jamming Collaborative Inference Systems](https://arxiv.org/abs/2603.02579)
*Mengru Wu,Jiawei Li,Jiaqi Wei,Bin Lyu,Kai-Kit Wong,Hyundong Shin*

Main category: cs.LG

TL;DR: The paper proposes an anti-jamming collaborative inference system that optimizes DNN partitioning, computation resources, and transmit power to maximize revenue of delay and accuracy under jamming attacks.


<details>
  <summary>Details</summary>
Motivation: DNN inference on resource-constrained devices faces computational challenges, and device-edge collaborative inference is vulnerable to malicious jamming attacks that degrade inference performance. There's a need to protect collaborative inference systems from jamming threats while maintaining efficiency.

Method: The authors analyze jamming and DNN partitioning effects on accuracy via data regression, then formulate a mixed-integer nonlinear programming problem to maximize revenue of delay and accuracy. They propose an alternating optimization algorithm that decomposes the problem into three subproblems solved via KKT conditions, convex optimization, and a quantum genetic algorithm.

Result: Extensive simulations demonstrate that the proposed scheme outperforms baseline methods in terms of revenue of delay and accuracy (RDA).

Conclusion: The proposed anti-jamming collaborative inference system effectively counters malicious jamming threats while optimizing DNN partitioning, computation resources, and transmit power to maximize system performance under accuracy and resource constraints.
...
### [277] [Heterogeneous Agent Collaborative Reinforcement Learning](https://arxiv.org/abs/2603.02604)
*Zhixia Zhang,Zixuan Huang,Xin Xia,Deqing Wang,Fuzhen Zhuang,Shuai Ma,Ning Ding,Yaodong Yang,Jianxin Li,Yikun Ban*

Main category: cs.LG

TL;DR: HACRL enables collaborative RL where heterogeneous agents share verified rollouts during training but operate independently at inference, improving efficiency over isolated on-policy optimization.


<details>
  <summary>Details</summary>
Motivation: Addresses inefficiencies of isolated on-policy optimization by enabling collaborative learning among heterogeneous agents without requiring coordinated deployment at inference time.

Method: Proposes HACPO algorithm with principled rollout sharing, four tailored mechanisms to handle capability discrepancies and policy distribution shifts, and theoretical guarantees on unbiased advantage estimation.

Result: Extensive experiments show HACPO consistently improves all participating agents, outperforming GSPO by average 3.3% while using only half the rollout cost across diverse heterogeneous model combinations and reasoning benchmarks.

Conclusion: HACRL/HACPO provides an effective collaborative RL paradigm that enables bidirectional mutual learning among heterogeneous agents with independent execution, achieving superior performance with reduced computational cost.
...
### [278] [Same Error, Different Function: The Optimizer as an Implicit Prior in Financial Time Series](https://arxiv.org/abs/2603.02620)
*Federico Vittorio Cortesi,Giuseppe Iannone,Giulia Crippa,Tomaso Poggio,Pierfrancesco Beneventano*

Main category: cs.LG

TL;DR: Different neural network training pipelines achieve identical test loss but learn qualitatively different functions in financial volatility forecasting, leading to divergent portfolio decisions despite similar accuracy.


<details>
  <summary>Details</summary>
Motivation: Neural networks in financial time series operate in underspecified regimes where multiple models achieve indistinguishable out-of-sample error, raising questions about whether these models actually learn the same underlying functions.

Method: Large-scale volatility forecasting for S&P 500 stocks using different neural network architectures and training pipeline pairs, analyzing functional differences despite identical test loss metrics.

Result: Optimizer choice reshapes non-linear response profiles and temporal dependence differently across architectures, creating material consequences for portfolio decisions with nearly 3× turnover dispersion at comparable Sharpe ratios.

Conclusion: In underspecified settings, optimization acts as a consequential source of inductive bias, requiring model evaluation to extend beyond scalar loss to include functional and decision-level implications.
...
### [279] [Implicit Bias in Deep Linear Discriminant Analysis](https://arxiv.org/abs/2603.02622)
*Jiawen Li*

Main category: cs.LG

TL;DR: First theoretical analysis of implicit regularization in Deep LDA, showing how network architecture transforms additive gradient updates into multiplicative updates that conserve (2/L) quasi-norm.


<details>
  <summary>Details</summary>
Motivation: While implicit bias of standard loss functions has been studied, the optimization geometry induced by discriminative metric-learning objectives like Deep LDA remains largely unexplored.

Method: Analyze gradient flow of Deep LDA loss on L-layer diagonal linear network under balanced initialization, proving how network architecture transforms standard additive gradient updates into multiplicative weight updates.

Result: The analysis demonstrates automatic conservation of the (2/L) quasi-norm through the multiplicative weight update mechanism induced by the network architecture.

Conclusion: This paper provides initial theoretical understanding of implicit regularization in Deep LDA, revealing how network structure fundamentally changes optimization dynamics compared to standard loss functions.
...
### [280] [Post Hoc Extraction of Pareto Fronts for Continuous Control](https://arxiv.org/abs/2603.02628)
*Raghav Thakar,Gaurav Dixit,Kagan Tumer*

Main category: cs.LG

TL;DR: MAPEX is an offline MORL method that efficiently constructs Pareto frontiers by reusing pre-trained specialist policies, critics, and replay buffers, achieving comparable results at 0.001% the sample cost of baselines.


<details>
  <summary>Details</summary>
Motivation: Real-world agents need to balance multiple objectives (speed, stability, energy efficiency), and multi-objective preferences often emerge after single-objective training. Existing MORL methods can't leverage pre-trained specialists, requiring expensive retraining from scratch.

Method: MAPEX combines evaluations from specialist critics into a mixed advantage signal, then weights a behavior cloning loss with it to train new policies that balance multiple objectives. It reuses pre-trained specialist policies, critics, and replay buffers in an offline setting.

Result: MAPEX produces comparable Pareto fronts to established baselines at only 0.001% the sample cost when given the same starting policies, evaluated on five multi-objective MuJoCo environments.

Conclusion: MAPEX enables efficient post-hoc Pareto front extraction from pre-trained specialists, preserving single-objective RL simplicity while avoiding complex MORL frameworks, making multi-objective adaptation practical in real-world scenarios.
...
### [281] [MASPOB: Bandit-Based Prompt Optimization for Multi-Agent Systems with Graph Neural Networks](https://arxiv.org/abs/2603.02630)
*Zhi Hong,Qian Zhang,Jiahang Sun,Zhiwei Shang,Mingze Kong,Xiangyi Wang,Yao Shu,Zhongxiang Dai*

Main category: cs.LG

TL;DR: MASPOB is a sample-efficient bandit-based framework for optimizing prompts in Multi-Agent Systems, addressing challenges of evaluation costs, topology coupling, and combinatorial search space through UCB, GNNs, and coordinate ascent.


<details>
  <summary>Details</summary>
Motivation: Real-world Multi-Agent Systems (MAS) using LLMs as cognitive backbones often cannot modify workflows, making prompt optimization crucial. However, MAS prompt optimization faces three key challenges: high evaluation costs requiring sample efficiency, topology-induced coupling among prompts, and combinatorial explosion of the search space.

Method: MASPOB uses bandit framework with Upper Confidence Bound (UCB) for exploration-exploitation balance. It integrates Graph Neural Networks (GNNs) to capture structural priors and topology-aware prompt representations. Coordinate ascent decomposes optimization into univariate sub-problems, reducing search complexity from exponential to linear.

Result: Extensive experiments across diverse benchmarks show MASPOB achieves state-of-the-art performance, consistently outperforming existing baselines while being sample-efficient.

Conclusion: MASPOB provides an effective framework for prompt optimization in Multi-Agent Systems, successfully addressing the key challenges of sample efficiency, topology coupling, and combinatorial search through its bandit-based approach with GNN integration and coordinate ascent decomposition.
...
### [282] [Robust Heterogeneous Analog-Digital Computing for Mixture-of-Experts Models with Theoretical Generalization Guarantees](https://arxiv.org/abs/2603.02633)
*Mohammed Nowaz Rabbani Chowdhury,Hsinyu Tsai,Geoffrey W. Burr,Kaoutar El Maghraoui,Liu Liu,Meng Wang*

Main category: cs.LG

TL;DR: A retraining-free heterogeneous computing framework that identifies noise-sensitive experts in MoE models and computes them digitally while running majority of experts on analog in-memory computing hardware.


<details>
  <summary>Details</summary>
Motivation: Sparse Mixture-of-Experts models have massive parameter counts causing memory and energy inefficiency during inference, while analog in-memory computing offers efficiency but suffers from hardware nonidealities that typically require noise-aware retraining, which is infeasible for large MoE models.

Method: Propose a retraining-free heterogeneous computation framework that: 1) identifies noise-sensitive experts by their maximum neuron norm (provably identifiable), computing them digitally, 2) executes majority of experts on AIMC hardware, 3) assigns densely activated modules like attention layers to digital computation due to their high noise sensitivity.
...
Conclusion: The proposed heterogeneous computation framework enables efficient execution of large MoE models on analog hardware without retraining, by strategically allocating noise-sensitive components to digital computation while leveraging AIMC for the majority of experts.
...
### [283] [SaFeR-ToolKit: Structured Reasoning via Virtual Tool Calling for Multimodal Safety](https://arxiv.org/abs/2603.02635)
*Zixuan Xu,Tiancheng He,Huahui Yi,Kun Wang,Xi Chen,Gongli Xi,Qiankun Li,Kang Li,Yang Liu,Zhigang Zeng*

Main category: cs.LG

TL;DR: SaFeR-ToolKit improves vision-language model safety by formalizing safety decision-making as a checkable protocol with tool-based reasoning and training via a three-stage curriculum (SFT→DPO→GRPO).


<details>
  <summary>Details</summary>
Motivation: Vision-language models are vulnerable to multimodal jailbreaks and over-refusal because safety depends on both visual evidence and user intent, but current alignment pipelines only supervise the final response.

Method: Formalizes safety decision-making as a checkable protocol: a planner specifies persona, Perception→Reasoning→Decision tool set, and constrained transition graph; responder outputs typed key-value tool trace before final answer. Trains a single policy with three-stage curriculum (SFT→DPO→GRPO), where GRPO directly supervises tool usage beyond answer-level feedback.

Result: Significantly improves Safety/Helpfulness/Reasoning Rigor on Qwen2.5-VL 3B (29.39/45.04/4.98 → 84.40/71.13/78.87) and 7B (53.21/52.92/19.26 → 86.34/80.79/85.34), while preserving general capabilities (3B: 58.67 → 59.21; 7B: 66.39 → 66.81).

Conclusion: SaFeR-ToolKit effectively addresses multimodal safety vulnerabilities through tool-based reasoning protocols and multi-stage training, achieving substantial improvements in safety, helpfulness, and reasoning rigor without compromising general capabilities.
...
### [284] [HomeAdam: Adam and AdamW Algorithms Sometimes Go Home to Obtain Better Provable Generalization](https://arxiv.org/abs/2603.02649)
*Feihu Huang,Guanyi Zhang,Songcan Chen*

Main category: cs.LG

TL;DR: The paper analyzes generalization of Adam/AdamW optimizers, proves their generalization error is O(1/√N), proposes HomeAdam(W) with improved generalization error O(1/N) and faster convergence.


<details>
  <summary>Details</summary>
Motivation: Adam and AdamW are default optimizers that converge fast but generalize worse than SGD. Their proved generalization error O(1/√N) is larger than SGD's O(1/N). Recent variants improve generalization but lack theoretical analysis.

Method: 1) Restudy generalization of Adam/AdamW via algorithmic stability, proving Adam(W)-srf (without square-root) has generalization error O(ρ̂^{-2T}/N). 2) Propose HomeAdam(W) algorithms that sometimes return momentum-based SGD to improve generalization. 3) Prove HomeAdam(W) has smaller generalization error O(1/N) and faster convergence rate O(1/T^{1/4}) compared to Adam(W)-srf.

Result: Theoretical results: HomeAdam(W) achieves O(1/N) generalization error (better than Adam's O(1/√N) and Adam(W)-srf's O(ρ̂^{-2T}/N)) and O(1/T^{1/4}) convergence rate (faster than Adam(W)-srf's O(ρ̌^{-1}/T^{1/4})). Extensive experiments demonstrate efficiency.

Conclusion: HomeAdam(W) algorithms provide both improved generalization (O(1/N) error) and faster convergence than standard Adam variants, addressing the generalization-performance tradeoff in adaptive optimization algorithms.
...
### [285] [From Shallow to Deep: Pinning Semantic Intent via Causal GRPO](https://arxiv.org/abs/2603.02675)
*Shuyi Zhou,Zeen Song,Wenwen Qiang,Jiyan Sun,Yao Zhou,Yinlong Liu,Wei Ma*

Main category: cs.LG

TL;DR: The paper proposes TSC-GRPO, a two-stage framework to address LLM vulnerability to adversarial prefix attacks by preventing semantic representation decay and enabling intent pinning for robust safety.


<details>
  <summary>Details</summary>
Motivation: Large Language Models remain vulnerable to adversarial prefix attacks (like "Sure, here is") despite having robust standard safety measures. This vulnerability stems from "Shallow Safety Alignment" caused by "semantic representation decay" - as models generate compliant prefixes, their internal malicious intent signal fades away.

Method: Two-Stage Causal-GRPO (TSC-GRPO): 1) Train a causal intent probe using causal identifiability theory to disentangle invariant intent from stylistic perturbations. 2) Internalize this causal awareness into the policy via Group Relative Policy Optimization, using cumulative causal penalty in "fork-in-the-road" training scenarios to force the model to learn that accumulating harmful tokens monotonically decreases reward.

Result: Experiments show that TSC-GRPO significantly outperforms baselines in defending against jailbreak attacks while preserving general utility of the language models.

Conclusion: The proposed TSC-GRPO framework effectively addresses the Shallow Safety Alignment problem by enabling intent pinning, allowing models to maintain robust late-stage refusals against adversarial attacks without compromising their general usefulness.
...
### [286] [Causal Learning Should Embrace the Wisdom of the Crowd](https://arxiv.org/abs/2603.02678)
*Ryan Feng Lin,Yuantao Wei,Huiling Liao,Xiaoning Qian,Shuai Huang*

Main category: cs.LG

TL;DR: A new paradigm for causal structure learning that integrates human expertise and LLM agents through distributed decision-making to overcome limitations of purely data-driven approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional causal structure learning from observational data faces combinatorial explosion and observational ambiguities. The paper argues that current technologies enable leveraging human causal knowledge to overcome these limitations.

Method: Proposes a distributed decision-making framework that integrates: 1) scalable crowdsourcing for data collection, 2) interactive knowledge elicitation for expert modeling, 3) robust aggregation techniques for expert reconciliation, and 4) LLM-based simulation for AI-driven information acquisition.

Result: The paper advocates for a new research frontier rather than presenting specific experimental results. It outlines a comprehensive framework for synthesizing fragmented knowledge from multiple participants (human experts and LLM agents) to recover global causal structures.

Conclusion: Causal learning is ready for a paradigm shift that leverages human knowledge through distributed decision-making. The proposed framework enables recovery of global causal structures unachievable by individual agents alone, opening new research directions in knowledge elicitation, modeling, aggregation, and optimization.
...
### [287] [Addressing Missing and Noisy Modalities in One Solution: Unified Modality-Quality Framework for Low-quality Multimodal Data](https://arxiv.org/abs/2603.02695)
*Sijie Mai,Shiqin Han,Haifeng Hu*

Main category: cs.LG

TL;DR: UMQ framework jointly addresses noisy and missing modalities as unified low-quality modality problem for multimodal affective computing, using quality estimation, enhancement, and mixture-of-experts routing.


<details>
  <summary>Details</summary>
Motivation: Real-world multimodal data often has low quality with noisy and missing modalities, which severely hinders model performance. Prior works handle these issues separately, but they need to be addressed jointly for better robustness.

Method: 1) Train quality estimator with rank-guided training using relative quality comparisons; 2) Build quality enhancers using cross-modal information and modality baselines; 3) Implement quality-aware mixture-of-experts with specialized routing for different modality-quality problems.

Result: UMQ consistently outperforms state-of-the-art baselines on multiple datasets under complete, missing, and noisy modality settings.

Conclusion: The proposed UMQ framework effectively handles both noisy and missing modalities as a unified low-quality problem, enhancing multimodal affective computing robustness in real-world low-quality data scenarios.
...
### [288] [An Empirical Analysis of Calibration and Selective Prediction in Multimodal Clinical Condition Classification](https://arxiv.org/abs/2603.02719)
*L. Julián Lechuga López,Farah E. Shamout,Tim G. J. Rudner*

Main category: cs.LG

TL;DR: Selective prediction fails in multimodal clinical AI despite strong standard metrics due to class-dependent miscalibration, where models assign high uncertainty to correct predictions and low uncertainty to incorrect ones.


<details>
  <summary>Details</summary>
Motivation: As AI systems move toward clinical deployment, ensuring reliable prediction behavior is crucial for safety-critical decision-making. Selective prediction (deferring uncertain predictions to human experts) is proposed as a safeguard, but its reliability needs empirical evaluation in clinical settings.
...
Conclusion: The study identifies a task-specific failure mode of selective prediction in multimodal clinical condition classification and highlights the need for calibration-aware evaluation to ensure safety and robustness in clinical AI deployment.
...
### [289] [The power of small initialization in noisy low-tubal-rank tensor recovery](https://arxiv.org/abs/2603.02729)
*ZHiyu Liu,Haobo Geng,Xudong Wang,Yandong Tang,Zhi Han,Yao Wang*

Main category: cs.LG

TL;DR: Small initialization in factorized gradient descent enables near-minimax optimal tensor recovery despite tubal-rank overestimation, with error bounds independent of overestimated rank.


<details>
  <summary>Details</summary>
Motivation: Factorized gradient descent with spectral initialization for low-tubal-rank tensor recovery suffers from recovery error that grows linearly with overestimated tubal-rank when measurements are corrupted by dense noise like Gaussian noise.

Method: Proposes using small initialization instead of spectral initialization for factorized gradient descent, analyzes the phenomenon using a four-stage analytic framework, and provides an early stopping strategy for practical implementation.

Result: Small initialization enables FGD to achieve nearly minimax optimal recovery error independent of overestimated tubal-rank R, with theoretical guarantees validated through simulations and real-data experiments.

Conclusion: Small initialization strategy overcomes limitations of spectral initialization for over-parameterized tensor recovery, providing robust performance with theoretical guarantees and practical early stopping.
...
### [290] [Practical FP4 Training for Large-Scale MoE Models on Hopper GPUs](https://arxiv.org/abs/2603.02731)
*Wuyue Zhang,Chongdong Huang,Chunbo You,Cheng Gu,Fengjuan Wang,Mou Sun*

Main category: cs.LG

TL;DR: Enables MXFP4 efficiency for MoE training on Hopper GPUs without native 4-bit support, achieving memory savings and throughput improvements comparable to FP8 baselines.
...
Method: Introduces direct FP8-to-FP4 quantization/de-quantization with scaling-aware FP4 row-to-column conversion, enabling FP4 activations and expert-parallel communication with minimal overhead while keeping core computations in FP8.

Result: At 671B parameter scale: achieves comparable performance to FP8 baselines, reduces peak activation memory by 14.8% (11.8 GB), improves training throughput by 12.5% (1157 to 1302 tokens/GPU/sec).
...
### [291] [Deep learning-guided evolutionary optimization for protein design](https://arxiv.org/abs/2603.02753)
*Erik Hartman,Di Tang,Johan Malmström*

Main category: cs.LG

TL;DR: BoGA combines Bayesian optimization with genetic algorithms for efficient protein design, demonstrated by designing peptide binders against pneumolysin.


<details>
  <summary>Details</summary>
Motivation: Protein design is challenging due to vast sequence space and complex sequence-function relationships; efficient exploration is needed for therapeutic and biotech applications.
...
Result: BoGA accelerates discovery of high-confidence peptide binders against pneumolysin, demonstrating efficient protein design across diverse objectives.

Conclusion: BoGA framework enables data-efficient optimization for protein design, available as open-source implementation in BoPep suite.
...
### [292] [Rethinking Time Series Domain Generalization via Structure-Stratified Calibration](https://arxiv.org/abs/2603.02756)
*Jinyang Li,Shuhao Mei,Xiaoyu Xiao,Shuhang Li,Ruoxi Yun,Jinbo Sun*

Main category: cs.LG

TL;DR: SSCF framework addresses cross-domain generalization for time series from heterogeneous dynamical systems by first identifying structurally consistent samples, then performing amplitude calibration only within compatible clusters to avoid spurious alignments.


<details>
  <summary>Details</summary>
Motivation: Existing cross-domain generalization methods assume comparable samples in shared representation space, but real-world datasets often come from structurally heterogeneous dynamical systems with fundamentally different feature distributions. Global alignment without considering structural differences leads to spurious correspondences and negative transfer.

Method: Structurally Stratified Calibration Framework (SSCF) that explicitly distinguishes structurally consistent samples and performs amplitude calibration exclusively within structurally compatible sample clusters. This approach first establishes structural consistency before alignment.

Result: SSCF significantly outperforms strong baselines under zero-shot setting on 19 public datasets (100.3k samples). The framework achieves substantial performance improvements through a concise and computationally efficient calibration strategy.

Conclusion: Establishing structural consistency prior to alignment constitutes a more reliable and effective pathway for improving cross-domain generalization of time series governed by latent dynamical systems, effectively alleviating generalization failures caused by structural incompatibility.
...
### [293] [Next Embedding Prediction Makes World Models Stronger](https://arxiv.org/abs/2603.02765)
*George Bredis,Nikita Balagansky,Daniil Gavrilov,Ruslan Rakhimov*

Main category: cs.LG

TL;DR: NE-Dreamer is a decoder-free MBRL agent that uses temporal transformers to predict next-step encoder embeddings, achieving state-of-the-art performance on complex partially observable environments.


<details>
  <summary>Details</summary>
Motivation: Capturing temporal dependencies is critical for model-based reinforcement learning in partially observable, high-dimensional domains, but existing approaches often rely on reconstruction losses or auxiliary supervision.

Method: Uses a temporal transformer to predict next-step encoder embeddings from latent state sequences, directly optimizing temporal predictive alignment in representation space without reconstruction losses or auxiliary supervision.

Result: Matches or exceeds DreamerV3 and leading decoder-free agents on DeepMind Control Suite, and achieves substantial gains on challenging DMLab tasks involving memory and spatial reasoning.
...
### [294] [From Heuristic Selection to Automated Algorithm Design: LLMs Benefit from Strong Priors](https://arxiv.org/abs/2603.02792)
*Qi Huang,Furong Ye,Ananta Shahane,Thomas Bäck,Niki van Stein*

Main category: cs.LG

TL;DR: LLM-driven optimization benefits from high-quality algorithmic code examples, improving performance on black-box optimization benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based algorithm design focuses on specific problems with adaptive prompts, but lacks investigation into how prompt content (especially algorithmic code examples) affects LLM-generated algorithm quality.

Method: Analyze token-wise attribution of prompts to LLM-generated algorithmic codes, then leverage prior benchmark algorithms to guide LLM-driven optimization.

Result: High-quality algorithmic code examples substantially improve LLM-driven optimization performance, demonstrated on pseudo-Boolean optimization (pbo) and black-box optimization (bbob) benchmarks.
...
### [295] [The Price of Robustness: Stable Classifiers Need Overparameterization](https://arxiv.org/abs/2603.02806)
*Jonas von Berg,Adalbert Fono,Massimiliano Datres,Sohir Maskey,Gitta Kutyniok*

Main category: cs.LG

TL;DR: Generalization bound for finite function classes improves with class stability (expected margin distance). Extends law of robustness to discontinuous classifiers, showing overparameterization needed for stability.


<details>
  <summary>Details</summary>
Motivation: The relationship between overparameterization, stability, and generalization is poorly understood for discontinuous classifiers. The paper aims to bridge this gap by establishing generalization bounds tied to class stability.

Method: Establishes generalization bound for finite function classes that improves inversely with class stability (expected distance to decision boundary). Extends to infinite function classes using normalized co-stability measure derived from margin in codomain.

Result: Any interpolating model with p≈n parameters on n data points must be unstable, requiring substantial overparameterization for high stability. Experiments show stability increases with model size and correlates with test performance.

Conclusion: Class stability serves as quantifiable robustness measure for discontinuous classifiers. Overparameterization is necessary to achieve stability, and stability-based measures are more informative than traditional norm-based measures for generalization.
...
### [296] [Lattice-based Deep Neural Networks: Regularity and Tailored Regularization](https://arxiv.org/abs/2603.02809)
*Alexander Keller,Frances Y. Kuo,Dirk Nuyens,Ian H. Sloan*

Main category: cs.LG

TL;DR: Survey on applying lattice rules (quasi-Monte Carlo methods) as training points for deep neural networks, achieving dimension-independent generalization error bounds through tailored regularization.


<details>
  <summary>Details</summary>
Motivation: Lattice rules are effective for high-dimensional integration/approximation and easy to implement. Recent DNN research explosion motivates exploring lattice rules as training points to improve DNN performance with theoretical guarantees.

Method: Using lattice rules (simple integer generating vectors) as training points for DNNs with smooth activation functions. Imposing parameter restrictions to match target function regularity. Tailored regularization approach instead of standard ℓ₂ regularization.

Result: Obtained explicit regularity bounds for DNNs. Proved DNNs with lattice training points achieve good theoretical generalization error bounds with constants independent of input dimension. Numerical demonstration shows significantly better performance than standard ℓ₂ regularization.

Conclusion: Lattice rules provide effective training points for DNNs with theoretical guarantees of dimension-independent generalization bounds. Tailored regularization outperforms standard approaches, offering promising direction for high-dimensional DNN training.
...
### [297] [Adapting Time Series Foundation Models through Data Mixtures](https://arxiv.org/abs/2603.02840)
*Thomas L. Lee,Edoardo M. Ponti,Amos Storkey*

Main category: cs.LG

TL;DR: MixFT improves zero-shot forecasting by re-dividing data into homogeneous sub-domains using Bayesian mixtures and fine-tuning separate modules for each sub-domain, outperforming per-dataset and single-module approaches.


<details>
  <summary>Details</summary>
Motivation: Time series foundation models suffer performance degradation on new domains not fully covered in pretraining. Existing fine-tuning approaches (single module on all data or per-dataset modules) are suboptimal because datasets can contain multiple distribution types (sub-domains) due to distribution shifts or differing dimensions.

Method: MixFT uses Bayesian mixtures to re-divide data into sets that best represent the sub-domains present in the data, then fine-tunes separate LoRA modules on each of these homogeneous sets, creating specialized modules for specific sub-domains.

Result: MixFT outperforms both per-dataset fine-tuning methods and single-module fine-tuning on all data, demonstrating that re-partitioning data to represent sub-domains leads to better specialization and improved zero-shot forecasting performance.

Conclusion: By identifying and separating sub-domains within datasets using Bayesian mixtures, MixFT enables more effective specialization of time series foundation models, leading to improved zero-shot forecasting performance compared to traditional fine-tuning approaches.
...
### [298] [Learning Memory-Enhanced Improvement Heuristics for Flexible Job Shop Scheduling](https://arxiv.org/abs/2603.02846)
*Jiaqi Wang,Zhiguang Cao,Peng Zhao,Rui Cao,Yubin Xiao,Yuan Jiang,You Zhou*

Main category: cs.LG

TL;DR: MIStar: A memory-enhanced improvement search framework using heterogeneous graph representation for flexible job-shop scheduling, outperforming traditional heuristics and state-of-the-art DRL methods.


<details>
  <summary>Details</summary>
Motivation: Current DRL-based approaches to flexible job-shop scheduling (FJSP) use constructive methods that often fail to reach near-optimal solutions. Improvement-based methods are more effective but face challenges with flexible machine allocation, including accurate state representation, policy learning, and search strategies.

Method: Proposes MIStar framework with: 1) Novel heterogeneous disjunctive graph explicitly modeling operation sequences on machines, 2) Memory-enhanced heterogeneous graph neural network (MHGNN) for feature extraction using historical trajectories, 3) Parallel greedy search strategy for efficient solution space exploration.
...
Conclusion: MIStar effectively addresses the challenges of improvement-based methods for FJSP through innovative graph representation, memory-enhanced learning, and efficient search strategies, achieving superior scheduling performance.
...
### [299] [Learning in Markov Decision Processes with Exogenous Dynamics](https://arxiv.org/abs/2603.02862)
*Davide Maran,Davide Salaorni,Marcello Restelli*

Main category: cs.LG

TL;DR: The paper introduces a structured class of MDPs with exogenous state components that evolve independently of agent actions, showing this structure enables significantly improved learning guarantees with optimal regret bounds.


<details>
  <summary>Details</summary>
Motivation: Standard RL algorithms treat all state variables as equally influenced by actions, but many practical systems have exogenous components that evolve independently of agent actions, accounting for most stochasticity. Current methods don't exploit this structure.

Method: The authors study a structured class of MDPs where state components are divided into endogenous (action-influenced) and exogenous (action-independent) parts. They develop RL algorithms that exploit this structure to achieve better regret bounds.

Result: Theoretical analysis shows significantly improved learning guarantees with regret bounds depending only on the size of the exogenous state space in leading terms. They establish matching lower bounds proving information-theoretic optimality. Empirical validation shows substantial sample efficiency gains.

Conclusion: Exploiting the exogenous-endogenous structure in MDPs yields optimal regret bounds and practical sample efficiency improvements, making RL more effective for systems with action-independent state components.
...
### [300] [Embedding interpretable $\ell_1$-regression into neural networks for uncovering temporal structure in cell imaging](https://arxiv.org/abs/2603.02899)
...
TL;DR: Combines neural networks with interpretable statistical models by embedding a sparse vector autoregressive (VAR) model into a convolutional autoencoder for analyzing two-photon calcium imaging data.


<details>
  <summary>Details</summary>
Motivation: Neural networks excel at learning non-sparse structure but lack interpretability, while classical statistical regression (especially with ℓ₁ regularization) offers better interpretability and sparse structure identification. The paper aims to optimally combine these two approaches.

Method: Embeds a vector autoregressive (VAR) model as an interpretable regression technique into a convolutional autoencoder. Uses skip connections to separate non-sparse static spatial information from sparse temporal dynamics. Implements ℓ₁-regularized VAR estimation by differentiating through the piecewise linear solution path.

Result: The proposed method enables identification of sparse temporal dynamics while maintaining interpretability. It allows for testing approaches to compare temporal sequences from the same observational unit and generates contribution maps to visualize which spatial regions drive the learned dynamics.

Conclusion: The paper successfully combines the strengths of neural networks (dimension reduction, non-sparse structure learning) with interpretable statistical models (sparse VAR), creating a hybrid approach that maintains interpretability while leveraging deep learning capabilities for complex data like calcium imaging.
...
### [301] [Distributed Dynamic Invariant Causal Prediction in Environmental Time Series](https://arxiv.org/abs/2603.02902)
*Ziruo Hao,Tao Yang,Xiaofeng Wu,Bo Hu*

Main category: cs.LG

TL;DR: DisDy-ICPT is a distributed framework for learning dynamic causal relationships from time series data with environmental attributes, achieving stable causal prediction without data communication between distributed nodes.


<details>
  <summary>Details</summary>
Motivation: Existing methods either focus on dynamic causal analysis without environmental contexts or static invariant causal inference, leaving a gap for distributed temporal settings where both dynamic causal relationships and spatial confounding need to be addressed.

Method: Proposed Distributed Dynamic Invariant Causal Prediction in Time-series (DisDy-ICPT) framework that learns dynamic causal relationships over time while mitigating spatial confounding variables without requiring data communication between distributed nodes.

Result: Theoretically proven to recover stable causal predictors within bounded communication rounds under standard sampling assumptions. Empirical evaluations on synthetic benchmarks and real-world datasets show superior predictive stability and accuracy compared to baseline methods.
...
### [302] [Towards Accurate and Interpretable Time-series Forecasting: A Polynomial Learning Approach](https://arxiv.org/abs/2603.02906)
*Bo Liu,Shao-Bo Lin,Changmiao Wang,Xiaotong Liu*

Main category: cs.LG

TL;DR: IPL method uses polynomial representations to model feature interactions for interpretable time series forecasting with flexible accuracy-interpretability trade-off.


<details>
  <summary>Details</summary>
Motivation: Existing forecasting methods lack interpretability, undermining user trust and complicating debugging. Current interpretable methods have limitations: insufficient temporal dependency modeling, lack of feature-level interpretability for early warning, and difficulty balancing accuracy with interpretability.

Method: Interpretable Polynomial Learning (IPL) integrates interpretability into model structure by explicitly modeling original features and their interactions of arbitrary order through polynomial representations. This preserves temporal dependencies and allows flexible trade-off between accuracy and interpretability by adjusting polynomial degree.
...
Conclusion: IPL provides an effective solution for interpretable time series forecasting that maintains accuracy while offering feature-level interpretability and flexible trade-offs, making it suitable for early warning applications in predictive maintenance.
...
### [303] [Eliciting Numerical Predictive Distributions of LLMs Without Autoregression](https://arxiv.org/abs/2603.02913)
*Julianna Piskorz,Katarzyna Kobalczyk,Mihaela van der Schaar*

Main category: cs.LG

TL;DR: LLMs can predict numerical distributions without autoregressive sampling by using regression probes on internal embeddings.


<details>
  <summary>Details</summary>
Motivation: Autoregressive decoding for continuous-valued outputs requires repeated sampling, leading to high computational cost and inference time. The paper investigates whether distributional properties of LLM predictions can be recovered without explicit autoregressive generation.

Method: Train regression probes to predict statistical functionals (mean, median, quantiles) of LLM's numerical output distribution directly from its internal representations/embeddings.

Result: LLM embeddings carry informative signals about summary statistics of their predictive distributions, including numerical uncertainty. This suggests lightweight alternatives to sampling-based approaches are feasible.

Conclusion: The investigation opens new questions about how LLMs internally encode uncertainty in numerical tasks and demonstrates the feasibility of lightweight alternatives to sampling-based approaches for uncertainty-aware numerical predictions.
...
### [304] [On the Structural Limitations of Weight-Based Neural Adaptation and the Role of Reversible Behavioral Learning](https://arxiv.org/abs/2603.02934)
*Pardhu Sri Rushi Varma Konduru*

Main category: cs.LG

TL;DR: The paper introduces structural irreversibility in neural model adaptation and proposes reversible behavioral learning to enable deterministic rollback of model behaviors.


<details>
  <summary>Details</summary>
Motivation: Current neural model adaptation methods (fine-tuning, alignment training, RL) cause long-term alterations to base model behavior due to shared parameter changes, creating irreversible effects that cannot be rolled back without parameter snapshots.

Method: Introduces reversible behavioral learning where model behaviors are structurally dissociated from identity parameters and can be deterministically unloaded. Also proposes Recoverability Factor as a normalized measure of behavioral recoverability and diagnostics based on model divergence.

Result: Experiments show that reversible model adaptation achieves rollback within numerical precision, while shared-parameter mutation exhibits persistent post-reset divergence that cannot be reversed.

Conclusion: Structural irreversibility is a key characteristic of shared-parameter adaptation, and reversible behavioral learning provides a solution for deterministic behavior unloading and model recoverability.
...
### [305] [Contextual Latent World Models for Offline Meta Reinforcement Learning](https://arxiv.org/abs/2603.02935)
*Mohammadreza Nakheai,Aidan Scannell,Kevin Luck,Joni Pajarinen*

Main category: cs.LG

TL;DR: Contextual latent world models combine context-based meta-RL with latent world models to learn better task representations through task-conditioned temporal consistency, improving generalization to unseen tasks.


<details>
  <summary>Details</summary>
Motivation: Offline meta-RL needs effective task representations without supervision. Current context-based methods struggle to learn meaningful representations beyond task discrimination, while latent world models show strong self-supervised learning through temporal consistency.

Method: Introduce contextual latent world models that condition latent world models on inferred task representations and train them jointly with context encoder. This enforces task-conditioned temporal consistency to capture task-dependent dynamics.
...
Conclusion: Combining context-based meta-RL with latent world models through task-conditioned temporal consistency produces superior task representations that capture task-dependent dynamics, leading to better generalization in offline meta-reinforcement learning.
...
### [306] [Beyond One-Size-Fits-All: Adaptive Subgraph Denoising for Zero-Shot Graph Learning with Large Language Models](https://arxiv.org/abs/2603.02938)
*Fengzhi Li,Liang Zhang,Yuan Zuo,Ruiqing Zhao,YanSong Liu,Yunfei Ma,Fanyu Meng,Junlan Feng*

Main category: cs.LG

TL;DR: GraphSSR is a novel framework for adaptive subgraph extraction and denoising in zero-shot LLM-based graph reasoning, addressing structural noise issues in previous methods.


<details>
  <summary>Details</summary>
Motivation: Traditional GNNs struggle with zero-shot generalization on graphs, and while LLM-based approaches like Graph-R1 show promise, they use task-agnostic subgraph extraction that introduces structural noise (irrelevant neighbors/edges), distorting LLMs' receptive field and leading to suboptimal predictions.

Method: Proposes GraphSSR with SSR pipeline (Sample-Select-Reason) for adaptive subgraph extraction, SSR-SFT for supervised fine-tuning using synthesized reasoning traces, and SSR-RL reinforcement learning framework with Authenticity-Reinforced and Denoising-Reinforced RL to guide sampling/selection for parsimonious, denoised subgraphs.

Result: The framework enables LLMs to autonomously filter task-irrelevant neighbors, overcome one-size-fits-all extraction issues, and achieve accurate predictions using denoised subgraphs for reasoning in zero-shot settings.

Conclusion: GraphSSR addresses structural noise limitations in LLM-based graph reasoning by introducing adaptive subgraph extraction and denoising mechanisms, improving zero-shot generalization capabilities for graph-based tasks.
...
### [307] [Enhancing Physics-Informed Neural Networks with Domain-aware Fourier Features: Towards Improved Performance and Interpretable Results](https://arxiv.org/abs/2603.02948)
*Alberto Miño Calero,Luis Salamanca,Konstantinos E. Tatsis*

Main category: cs.LG

TL;DR: PINNs with Domain-aware Fourier Features (DaFFs) improve accuracy, training efficiency, and interpretability compared to vanilla PINNs and Random Fourier Features (RFFs).


<details>
  <summary>Details</summary>
Motivation: PINNs are difficult to train and interpret despite their success in incorporating physics through PDE constraints in loss functions. There's a need for better positional encoding methods that simplify training and enhance interpretability.

Method: Proposes Domain-aware Fourier Features (DaFFs) for positional encoding that encapsulate domain-specific characteristics (geometry, boundary conditions). Also develops an LRP-based explainability framework tailored to PINNs to extract relevance attribution scores for input space.

Result: PINN-DaFFs achieve orders-of-magnitude lower errors and faster convergence compared to vanilla PINNs and RFFs-based PINNs. LRP analysis shows DaFFs lead to more physically consistent feature attributions, while other methods display scattered, less physics-relevant patterns.

Conclusion: DaFFs enhance PINNs' accuracy, efficiency, and interpretability, eliminating need for explicit boundary condition loss terms and loss balancing, while laying ground for more robust and informative physics-informed learning.
...
### [308] [CGL: Advancing Continual GUI Learning via Reinforcement Fine-Tuning](https://arxiv.org/abs/2603.02951)
*Zhenquan Yao,Zitong Huang,Yihan Zeng,Jianhua Han,Hang Xu,Chun-Mei Feng,Jianwei Ma,Wangmeng Zuo*

Main category: cs.LG

TL;DR: Proposes CGL framework for continual GUI learning that balances SFT adaptation and RL retention using entropy-guided proportion adjustment and gradient surgery to prevent knowledge overwriting.


<details>
  <summary>Details</summary>
Motivation: GUI agents need continual learning to adapt to frequent application updates without forgetting old tasks. Current methods suffer from knowledge overwriting when using SFT, while RL shows better retention but slower adaptation.

Method: CGL framework with: 1) SFT proportion adjustment guided by policy entropy to dynamically balance SFT and RL training phases, 2) Gradient surgery that projects exploratory SFT gradients onto GRPO-based anchor gradients to clip conflicting components, 3) AndroidControl-CL benchmark for evaluation.

Result: Experimental results demonstrate effectiveness across continual learning scenarios. The framework successfully balances adaptation efficiency and skill retention.

Conclusion: CGL framework effectively addresses continual GUI learning by synergizing SFT and RL through dynamic balancing and gradient interference resolution, enabling GUI agents to adapt to new tasks while retaining old skills.
...
### [309] [Leveraging Label Proportion Prior for Class-Imbalanced Semi-Supervised Learning](https://arxiv.org/abs/2603.02957)
*Kohki Akiba,Shinnosuke Matsuo,Shota Harada,Ryoma Bise*

Main category: cs.LG

TL;DR: A lightweight SSL framework introduces Proportion Loss from LLP as regularization to mitigate class imbalance bias, improving performance on long-tailed datasets.


<details>
  <summary>Details</summary>
Motivation: Semi-supervised learning suffers from class imbalance where pseudo-labeling amplifies majority bias and suppresses minority class performance, creating a need for methods that address this issue.

Method: Introduces Proportion Loss from learning from label proportions (LLP) into SSL as a regularization term to align model predictions with global class distribution. Also develops a stochastic variant to handle mini-batch composition fluctuations.

Result: Integrating Proportion Loss into FixMatch and ReMixMatch consistently improves performance on Long-tailed CIFAR-10 benchmark across imbalance severities and label ratios, achieving competitive or superior results compared to existing CISSL methods, especially under scarce-label conditions.

Conclusion: Proportion Loss effectively mitigates class imbalance bias in SSL by regularizing predictions to match global class distribution, offering a lightweight solution that works particularly well in label-scarce scenarios.
...
### [310] [Integrating Homomorphic Encryption and Synthetic Data in FL for Privacy and Learning Quality](https://arxiv.org/abs/2603.02969)
*Yenan Wang,Carla Fabiana Chiasserini,Elad Michael Schiller*

Main category: cs.LG

TL;DR: Alt-FL improves federated learning by alternating between authentic and synthetic data rounds, enhancing accuracy while reducing homomorphic encryption costs.


<details>
  <summary>Details</summary>
Motivation: Federated learning needs to balance learning quality, privacy protection (via homomorphic encryption), and computational efficiency, as current HE-based approaches are resource-intensive.
...
Result: Alt-FL provides 13.4% higher model accuracy, reduces HE-related costs by up to 48% compared to Selective HE, and demonstrates robust privacy protection against data leakage attacks like DLG.

Conclusion: The alternating strategy with synthetic data generation effectively balances learning quality, privacy protection, and computational efficiency in federated learning.
...
### [311] [LAGO: A Local-Global Optimization Framework Combining Trust Region Methods and Bayesian Optimization](https://arxiv.org/abs/2603.02970)
*Eliott Van Dieren,Tommaso Vanzan,Fabio Nobile*

Main category: cs.LG

TL;DR: LAGO combines Bayesian Optimization for global search with trust region methods for local refinement using an adaptive competition mechanism to efficiently optimize smooth functions.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of standard non-linear local optimization algorithms that may get stuck in local optima, and to improve global exploration while maintaining fast local convergence in promising regions.

Method: Combines gradient-enhanced Bayesian Optimization with gradient-based trust region local refinement through adaptive competition. At each iteration, both strategies propose candidates, and selection is based on predicted improvement. Separates global exploration from local refinement by optimizing BO acquisition outside active trust regions, and incorporates local evaluations into global model only when they satisfy lengthscale-based distance criteria.

Result: Achieves improved exploration of the full design space compared to standard non-linear local optimization algorithms for smooth functions, while maintaining fast local convergence in regions of interest.

Conclusion: LAGO enables efficient local refinement when reaching promising regions without sacrificing global search, providing a robust optimization framework that balances exploration and exploitation effectively.
...
### [312] [On the Topology of Neural Network Superlevel Sets](https://arxiv.org/abs/2603.02973)
*Bahman Gharesifard*

Main category: cs.LG

TL;DR: Neural networks with Riccati-type activation functions produce outputs with controlled topological complexity, enabling uniform bounds on Betti numbers for superlevel sets and Lie bracket rank drop loci.


<details>
  <summary>Details</summary>
Motivation: To establish architecture-only bounds on topological complexity of neural network outputs, particularly for superlevel sets and Lie bracket rank drop loci, without dependence on specific weight values.

Method: Analyze neural networks with activations satisfying Riccati-type ODE conditions, showing they produce Pfaffian outputs on analytic domains with format controlled only by architecture parameters.

Result: Superlevel sets and Lie bracket rank drop loci for neural network parameterized vector fields admit architecture-only bounds on topological complexity, including uniform bounds on total Betti numbers across all weight values.

Conclusion: Neural networks with Riccati-type activations have outputs with controlled topological structure, enabling uniform complexity bounds independent of specific weight configurations.
...
### [313] [Why Does RLAIF Work At All?](https://arxiv.org/abs/2603.03000)
*Robin Young*

Main category: cs.LG

TL;DR: The paper proposes the "latent value hypothesis" to explain why Reinforcement Learning from AI Feedback (RLAIF) works for value learning, suggesting that pretraining encodes human values as directions in representation space, and constitutional prompts elicit these latent values into preference judgments.


<details>
  <summary>Details</summary>
Motivation: RLAIF enables language models to improve by training on their own preference judgments, but there's no theoretical explanation for why this self-improvement works for value learning. The paper aims to provide a theoretical account for this phenomenon.

Method: The authors formalize the latent value hypothesis under a linear model where the constitution acts as a projection operator selecting value-relevant directions. They analyze RLAIF through this theoretical framework.

Result: The analysis shows: 1) RLAIF improves alignment when constitution-activated directions correlate better with true values than default generation directions, 2) RLAIF quality ceiling depends on how well representations encode values (scaling with model capacity), and 3) adversarial constitutions can activate anti-social value directions from harmful pretraining data.

Conclusion: The latent value hypothesis provides a unified theoretical account that explains empirical findings including the refusal direction, low-rank safety subspaces, and RLAIF scaling behavior, offering insights into how pretraining encodes values and how constitutional prompts can elicit them.
...
### [314] [Breaking the Prototype Bias Loop: Confidence-Aware Federated Contrastive Learning for Highly Imbalanced Clients](https://arxiv.org/abs/2603.03007)
*Tian-Shuang Wu,Shen-Huan Lyu,Ning Chen,Yi-Xiao He,Bing Tang,Baoliu Ye,Qingfu Zhang*

Main category: cs.LG

TL;DR: CAFedCL breaks the prototype bias loop in federated contrastive learning by using confidence-aware aggregation and geometric consistency regularization to handle class imbalance and data heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Local class imbalance and data heterogeneity in federated learning create a prototype bias loop where biased local prototypes lead to biased global prototypes, which accumulate errors across communication rounds when reused as contrastive anchors.

Method: Proposes Confidence-Aware Federated Contrastive Learning (CAFedCL) with: 1) confidence-aware aggregation using predictive uncertainty to downweight high-variance local prototypes, 2) generative augmentation for minority classes, and 3) geometric consistency regularization to stabilize class structure.

Result: Extensive experiments under varying levels of class imbalance and data heterogeneity show CAFedCL consistently outperforms representative federated baselines in both accuracy and client fairness.

Conclusion: CAFedCL effectively breaks the prototype bias loop in federated contrastive learning through confidence-aware aggregation and geometric regularization, with theoretical analysis showing reduced estimation variance and bounded global prototype drift.
...
### [315] [SEHFS: Structural Entropy-Guided High-Order Correlation Learning for Multi-View Multi-Label Feature Selection](https://arxiv.org/abs/2603.03022)
*Cheng Peng,Yonghao Li,Wanfu Gao,Jie Wen,Weiping Ding*

Main category: cs.LG

TL;DR: SEHFS is a novel multi-view multi-label feature selection method that uses structural entropy to learn high-order feature correlations and combines information theory with matrix methods to avoid local optima.


<details>
  <summary>Details</summary>
Motivation: Existing information-theoretic methods for MVML struggle with learning high-order structural correlations in real-world data and are prone to local optima due to heuristic optimization approaches.

Method: Converts feature graph into structural-entropy-minimizing encoding tree to quantify high-order dependencies, groups redundant features into clusters, and uses a fusion framework combining information theory with matrix methods to learn shared semantic and view-specific contribution matrices.

Result: Theoretical establishment of structural entropy's ability to learn high-order correlations, and experimental results on eight datasets from various domains demonstrate superior feature selection performance.

Conclusion: SEHFS effectively addresses the limitations of existing information-theoretic methods by learning high-order feature correlations and avoiding local optima through a novel structural entropy and matrix fusion approach.
...
### [316] [Step-Level Sparse Autoencoder for Reasoning Process Interpretation](https://arxiv.org/abs/2603.03031)
*Xuan Yang,Jiayu Liu,Yuhang Lai,Hao Xu,Zhenya Huang,Ning Miao*

Main category: cs.LG

TL;DR: SSAE is a step-level sparse autoencoder that disentangles LLM reasoning steps into sparse features, enabling analysis of reasoning properties like correctness and logicality.


<details>
  <summary>Details</summary>
Motivation: Existing token-level sparse autoencoders have granularity mismatch with step-level reasoning patterns, making it hard to analyze critical reasoning aspects like direction and semantic transitions in LLMs.

Method: Proposes step-level sparse autoencoder (SSAE) that controls sparsity of step features conditioned on context, forming an information bottleneck to split incremental from background information into sparsely activated dimensions.

Result: SSAE effectively extracts features that can predict surface-level information (generation length, first token distribution) and complex properties (correctness, logicality) through linear probing, showing LLMs already know these properties during generation.

Conclusion: SSAE provides interpretability for LLM reasoning steps, revealing that LLMs possess self-verification ability foundation by encoding reasoning properties in their step representations.
...
### [317] [cPNN: Continuous Progressive Neural Networks for Evolving Streaming Time Series](https://arxiv.org/abs/2603.03040)
*Federico Giannini,Giacomo Ziffer,Emanuele Della Valle*

Main category: cs.LG

TL;DR: cPNN addresses concept drift, temporal dependencies, and catastrophic forgetting in data streams using continuous Progressive Neural Networks with RNNs and SGD.


<details>
  <summary>Details</summary>
Motivation: Data streams often violate i.i.d. assumptions by having temporal dependencies and concept drift. Existing solutions handle these problems separately, and catastrophic forgetting remains an issue when learning multiple concepts over time.

Method: Proposes Continuous Progressive Neural Networks (cPNN), a continuous version of Progressive Neural Networks that uses Recurrent Neural Networks and Stochastic Gradient Descent adapted for data streams with temporal dependencies.

Result: Ablation study shows cPNN quickly adapts to new concepts and demonstrates robustness to concept drifts.

Conclusion: cPNN provides a unified solution for handling concept drift, temporal dependencies, and catastrophic forgetting in non-stationary data streams.
...
### [318] [IoUCert: Robustness Verification for Anchor-based Object Detectors](https://arxiv.org/abs/2603.03043)
*Benedikt Brückner,Alejandro Mercado,Yanghao Zhang,Panagiotis Kouvaros,Alessio Lomuscio*

Main category: cs.LG

TL;DR: IoUCert is a formal verification framework for anchor-based object detection models that provides robustness guarantees against input perturbations, overcoming challenges from non-linear coordinate transformations and IoU metrics.


<details>
  <summary>Details</summary>
Motivation: Formal robustness verification has been successful for image classification but remains difficult for object detection due to complex non-linear coordinate transformations and Intersection-over-Union (IoU) metrics. Existing methods struggle with anchor-based architectures like SSD and YOLO variants.

Method: Introduces IoUCert framework with a novel coordinate transformation that avoids precision-degrading relaxations of non-linear box prediction functions. Uses Interval Bound Propagation optimized directly with respect to anchor box offsets to derive optimal IoU bounds. Focuses on object localization in single-object settings.

Result: Enables first-time robustness verification of realistic anchor-based models including SSD, YOLOv2, and YOLOv3 variants against various input perturbations. Provides formal guarantees for object detection robustness.

Conclusion: IoUCert successfully overcomes verification bottlenecks in object detection by addressing non-linear coordinate transformations and IoU metrics, enabling formal robustness analysis of practical detection models that was previously infeasible.
...
### [319] [Incremental Graph Construction Enables Robust Spectral Clustering of Texts](https://arxiv.org/abs/2603.03056)
*Marko Pranjić,Boshko Koloski,Nada Lavrač,Senja Pollak,Marko Robnik-Šikonja*

Main category: cs.LG

TL;DR: Simple incremental k-NN graph construction guarantees connectivity for spectral clustering of text embeddings, outperforming standard k-NN at low sparsity levels.


<details>
  <summary>Details</summary>
Motivation: Standard k-NN graphs in spectral clustering of text embeddings often contain disconnected components at practical sparsity levels (small k), making clustering degenerate and hyperparameter-sensitive.

Method: Incremental k-NN graph construction where each new node is linked to its k nearest previously inserted nodes, guaranteeing connected graph for any k with inductive proof of connectedness.

Result: Outperforms standard k-NN graphs in low-k regime where disconnected components are prevalent, matches standard k-NN performance at larger k, validated on six clustering datasets from Massive Text Embedding Benchmark.

Conclusion: Incremental k-NN graph construction provides robust connectivity guarantee for spectral clustering of text embeddings, addressing fragility of standard neighborhood graphs at practical sparsity levels.
...
### [320] [Reinforcement Learning with Symbolic Reward Machines](https://arxiv.org/abs/2603.03068)
*Thomas Krug,Daniel Neider*

Main category: cs.LG

TL;DR: SRMs overcome RM limitations by using symbolic formulas instead of manual labeling functions, enabling direct observation processing while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: Reward Machines require manual labeling functions for each environment/task, limiting applicability in standard RL frameworks. The need for high-level labels alongside observations creates user burden and reduces practicality.

Method: Propose Symbolic Reward Machines (SRMs) with QSRM and LSRM algorithms that process observations directly through symbolic formula guards, eliminating the need for separate labeling functions.

Result: SRM methods outperform baseline RL approaches and match existing RM methods' performance while adhering to standard environment definitions and providing interpretable task representations.

Conclusion: SRMs successfully address RM limitations by enabling direct observation processing through symbolic formulas, improving applicability in standard RL frameworks while maintaining interpretability and performance.
...
### [321] [On the Expressive Power of Transformers for Maxout Networks and Continuous Piecewise Linear Functions](https://arxiv.org/abs/2603.03084)
*Linyan Gu,Lihua Yang,Feng Zhou*

Main category: cs.LG

TL;DR: Transformers can approximate maxout networks and inherit universal approximation capabilities from ReLU networks, with exponential growth in linear regions with depth.


<details>
  <summary>Details</summary>
Motivation: Despite Transformers' empirical success, their theoretical expressive power remains insufficiently understood. The paper aims to bridge this gap by analyzing Transformers' approximation capabilities and connecting them to standard neural network theory.

Method: Established explicit approximation of maxout networks by Transformers while preserving model complexity. Developed framework to analyze approximation of continuous piecewise linear functions by Transformers, quantifying expressivity via number of linear regions.

Result: Transformers inherit universal approximation capability of ReLU networks under similar complexity constraints. The number of linear regions grows exponentially with depth. Self-attention layers implement max-type operations, while feedforward layers realize token-wise affine transformations.

Conclusion: The analysis establishes theoretical bridge between approximation theory for standard feedforward networks and Transformers, providing structural insights into Transformer architecture components and their expressive power.
...
### [322] [Why Adam Can Beat SGD: Second-Moment Normalization Yields Sharper Tails](https://arxiv.org/abs/2603.03099)
*Ruinan Jin,Yingbin Liang,Shaofeng Zou*

Main category: cs.LG

TL;DR: Adam achieves better high-probability convergence than SGD with δ^{-1/2} vs δ^{-1} dependence on confidence parameter.


<details>
  <summary>Details</summary>
Motivation: Despite Adam's empirical superiority over SGD in many applications, existing theory fails to explain the performance gap, providing guarantees essentially comparable to SGD. The paper aims to provide theoretical justification for Adam's better performance.

Method: Identifies key second-moment normalization in Adam and develops a stopping-time/martingale analysis under the classical bounded variance model (second moment assumption). This novel analysis framework distinguishes Adam from SGD theoretically.

Result: Establishes first theoretical separation between high-probability convergence behaviors: Adam achieves δ^{-1/2} dependence on confidence parameter δ, while SGD necessarily incurs at least δ^{-1} dependence.

Conclusion: The paper provides theoretical justification for Adam's empirical superiority over SGD by showing fundamentally different high-probability convergence behaviors, with Adam having better dependence on confidence parameter.
...
### [323] [Multi-Scale Adaptive Neighborhood Awareness Transformer For Graph Fraud Detection](https://arxiv.org/abs/2603.03106)
*Jiaqi Lv,Qingfeng Du,Yu Zhang,Yongqi Han,Sheng Li*

Main category: cs.LG

TL;DR: MANDATE is a transformer-based approach for graph fraud detection that addresses GNN limitations through multi-scale positional encoding, homophilic/heterophilic connection embedding strategies, and multi-relation fusion.


<details>
  <summary>Details</summary>
Motivation: Existing GNN-based fraud detection methods suffer from inherent inductive biases including homogeneity assumptions and limited global modeling ability, which hinder their effectiveness in identifying fraudulent behavior in graphs.

Method: Proposes Multi-scale Neighborhood Awareness Transformer (MANDATE) with: 1) multi-scale positional encoding for various distances from central nodes, 2) different embedding strategies for homophilic and heterophilic connections, and 3) embedding fusion strategy for multi-relation graphs.

Result: Experiments on three fraud detection datasets demonstrate the superiority of MANDATE over existing methods.

Conclusion: MANDATE effectively addresses GNN limitations in graph fraud detection through transformer architecture with specialized strategies for positional encoding, connection types, and multi-relation fusion.
...
### [324] [From Complex Dynamics to DynFormer: Rethinking Transformers for PDEs](https://arxiv.org/abs/2603.03112)
*Pengyu Lai,Yixiao Chen,Dewu Yang,Rui Wang,Feng Wang,Hui Xu*

Main category: cs.LG

TL;DR: DynFormer is a dynamics-informed neural operator that uses specialized modules for different physical scales, achieving 95% error reduction with lower memory usage compared to SOTA baselines.


<details>
  <summary>Details</summary>
Motivation: Classical PDE solvers are computationally expensive for high-dimensional/multi-scale problems, and existing Transformer-based neural operators treat all spatial points uniformly, ignoring scale separation and applying inefficient global attention that mixes different physical scales unnecessarily.

Method: DynFormer uses Spectral Embedding to isolate low-frequency modes with Kronecker-structured attention for efficient global interactions, plus Local-Global-Mixing transformation for nonlinear multiplicative frequency mixing to capture small-scale turbulent cascades without global attention, integrated into a hybrid evolutionary architecture.

Result: Achieves up to 95% reduction in relative error compared to state-of-the-art baselines across four PDE benchmarks while significantly reducing GPU memory consumption.

Conclusion: Embedding first-principles physical dynamics into Transformer architectures creates a highly scalable, theoretically grounded blueprint for PDE surrogate modeling that respects scale separation in complex physical systems.
...
### [325] [Joint Training Across Multiple Activation Sparsity Regimes](https://arxiv.org/abs/2603.03131)
*Haotian Wang*

Main category: cs.LG

TL;DR: A simple training strategy using global top-k activation constraints and progressive compression cycles improves generalization on CIFAR-10 without data augmentation.


<details>
  <summary>Details</summary>
Motivation: Inspired by biological systems' stronger generalization, the authors hypothesize that robust internal representations should remain effective across both dense and sparse activation regimes.

Method: Introduce a training strategy applying global top-k constraints to hidden activations, repeatedly cycling a single model through multiple activation budgets via progressive compression and periodic reset. Tested on CIFAR-10 without data augmentation using WRN-28-4 backbone.

Result: Two adaptive keep-ratio control strategies both outperform dense baseline training in single-run experiments, suggesting joint training across multiple activation sparsity regimes improves generalization.
...
### [326] [Torus embeddings](https://arxiv.org/abs/2603.03135)
*Dan Stowell*

Main category: cs.LG

TL;DR: Paper introduces toroidal embeddings as an alternative to Euclidean/hyperspherical representations, showing comparable performance with simpler quantization for efficient TinyML deployment.


<details>
  <summary>Details</summary>
Motivation: Current deep learning embeddings use Euclidean or hyperspherical spaces, but computer hardware fundamentally uses integer representations with overflow that correspond to toroidal topology, creating a mismatch that wastes representation capacity.

Method: Adapt common deep learning frameworks to create representations with inherent toroidal topology using two alternative strategies, with a focus on a normalization-based approach that provides stable training comparable to standard L2 normalization.

Result: Torus embeddings maintain desirable quantization properties and show comparable performance to hypersphere embeddings, while offering simpler pathway to efficient TinyML embedded implementation.

Conclusion: Toroidal embeddings provide a viable alternative to existing representations, offering comparable performance while better matching hardware integer representations for more efficient embedded deployment.
...
### [327] [Information Routing in Atomistic Foundation Models: How Equivariance Creates Linearly Disentangled Representations](https://arxiv.org/abs/2603.03155)
*Joshua Steier*

Main category: cs.LG

TL;DR: CPD method reveals that tensor product equivariant architectures (MACE) produce linearly disentangled representations where geometry is accessible after composition removal, while handcrafted descriptors (ANI-2x) entangle information nonlinearly.


<details>
  <summary>Details</summary>
Motivation: To understand what atomistic foundation models encode in their intermediate representations and how that information is organized, particularly examining the disentanglement between compositional and geometric information.
...
Result: MACE architectures produce representations where geometry is almost fully linearly accessible after composition removal (R²_geom = 0.782 for HOMO-LUMO gap), while ANI-2x entangles information nonlinearly (R²_geom = -0.792 under Ridge; R² = +0.784 under MLP). MACE routes target-specific signal through specific irreducible representation channels.

Conclusion: Linearly disentangled representations are more sample-efficient under linear probing, suggesting practical advantages for equivariant architectures beyond raw prediction accuracy. Recommends linear probes as primary metric over gradient boosted tree probes which show systematic inflation.
...
### [328] [Less Noise, Same Certificate: Retain Sensitivity for Unlearning](https://arxiv.org/abs/2603.03172)
*Carolin Heinzler,Kasra Malihi,Amartya Sanyal*

Main category: cs.LG

TL;DR: Certified unlearning can use less noise than DP methods by focusing on retain sensitivity instead of global sensitivity, since unlearning doesn't need to protect retained data privacy.


<details>
  <summary>Details</summary>
Motivation: Existing certified unlearning methods use DP techniques with noise calibrated to global sensitivity, which is overly conservative because unlearning doesn't require protecting the privacy of retained data R.

Method: Define retain sensitivity as worst-case output change over deletions U while keeping R fixed. Use retain sensitivity instead of global sensitivity to calibrate noise, allowing same certificates with less noise.

Result: Theoretical and empirical validation shows noise reduction across problems including minimum spanning trees, PCA, and ERM. Refined analysis of existing unlearning algorithms further reduces noise and improves utility.

Conclusion: Retain sensitivity provides a more appropriate noise calibration for certified unlearning than DP's global sensitivity, enabling same privacy guarantees with better utility by leveraging the regularity of retained data.
...
### [329] [Understanding and Mitigating Dataset Corruption in LLM Steering](https://arxiv.org/abs/2603.03206)
*Cullen Anderson,Narmeen Oozeer,Foad Namjoo,Remy Ogasawara,Amirali Abdullah,Jeff M. Phillips*

Main category: cs.LG

TL;DR: Contrastive steering for LLMs is robust to moderate data corruption but vulnerable to malicious manipulation; using robust mean estimators can mitigate unwanted effects.


<details>
  <summary>Details</summary>
Motivation: Despite contrastive steering's growing use in AI safety applications, its robustness to noisy or adversarial data corruption is poorly understood, creating potential security vulnerabilities.

Method: The study examines robustness by corrupting the dataset used to train steering directions, analyzes corruption geometry, and tests replacing standard mean computation with robust mean estimators.

Result: Contrastive steering is robust to moderate corruption but vulnerable to malicious manipulation when a non-trivial fraction of training data is altered; robust mean estimators effectively mitigate unwanted effects.

Conclusion: While contrastive steering has inherent vulnerabilities to data corruption, implementing safeguards like robust mean estimators can significantly improve its security for AI safety applications.
...
### [330] [I-CAM-UV: Integrating Causal Graphs over Non-Identical Variable Sets Using Causal Additive Models with Unobserved Variables](https://arxiv.org/abs/2603.03207)
*Hirofumi Suzuki,Kentaro Kanamori,Takuya Takagi,Thong Pham,Takashi Nicholas Maeda,Shohei Shimizu*

Main category: cs.LG

TL;DR: I-CAM-UV integrates multiple observational datasets with different variable sets for causal discovery by leveraging CAM-UV to handle unobserved variables and enumerating all consistent causal graphs.


<details>
  <summary>Details</summary>
Motivation: Existing causal discovery methods are designed for single datasets, but real-world applications often involve multiple datasets with non-identical variable sets. Simple overlapping approaches fail because unobserved variables can act as confounders, and some variable pairs may be unobserved in any dataset.

Method: The approach leverages Causal Additive Models with Unobserved Variables (CAM-UV) that provide causal graphs with information about unobserved variables. I-CAM-UV integrates CAM-UV results by enumerating all causal graphs that are structurally consistent with the information from each dataset, using an efficient combinatorial search algorithm.

Result: The method demonstrates usefulness against existing methods, showing improved causal relationship identification compared to simple overlapping approaches that fail due to unobserved confounders and missing variable pairs.

Conclusion: I-CAM-UV provides an effective solution for causal discovery from multiple observational datasets with different variable sets by properly handling unobserved variables and integrating information across datasets through structural consistency constraints.
...
### [331] [Stabilized Adaptive Loss and Residual-Based Collocation for Physics-Informed Neural Networks](https://arxiv.org/abs/2603.03224)
*Divyavardhan Singh,Shubham Kamble,Dimple Sonone,Kishor Upla*

Main category: cs.LG

TL;DR: Proposes adaptive loss balancing and residual-based collocation schemes to improve PINNs for stiff/shock-dominated PDEs, reducing errors by 44-70% compared to traditional PINNs.


<details>
  <summary>Details</summary>
Motivation: Traditional PINNs have limitations with high stiffness or shock-dominated dynamics, including unbalanced training and solution inaccuracy even with small physics residuals.

Method: 1) Adaptive loss balancing using smoothed gradient norms to ensure satisfaction of initial/boundary conditions; 2) Adaptive residual-based collocation scheme to improve accuracy in regions with high physics residuals.

Result: Significantly improves solution accuracy: 44% relative L2 error reduction for Burgers' equation, 70% reduction for Allen-Cahn equation compared to traditional PINNs. Shows trustworthy solution comparison with robust finite difference solver.

Conclusion: The proposed adaptive approach effectively addresses PINNs limitations for stiff/shock-dominated problems, achieving substantial accuracy improvements while maintaining physics residual satisfaction.
...
### [332] [Adaptive Methods Are Preferable in High Privacy Settings: An SDE Perspective](https://arxiv.org/abs/2603.03226)
*Enea Monzio Compagnoni,Alessandro Stanghellini,Rustem Islamov,Aurelien Lucchi,Anastasiia Koloskova*

Main category: cs.LG

TL;DR: DP-SGD and DP-SignSGD show different privacy-utility trade-offs: DP-SGD has O(1/ε²) trade-off with ε-independent speed, while DP-SignSGD has O(1/ε) trade-off with linear ε speed. Under optimal learning rates, both achieve similar asymptotic performance, but DP-SignSGD's hyperparameters transfer better across privacy levels.


<details>
  <summary>Details</summary>
Motivation: As privacy regulations tighten, Differential Privacy (DP) becomes central to large-scale training. The paper aims to understand how DP noise interacts with adaptivity in optimization through stochastic differential equations analysis, comparing DP-SGD and DP-SignSGD.

Method: Uses stochastic differential equations (SDE) framework to analyze DP optimizers, specifically DP-SGD and DP-SignSGD under per-example clipping. Compares convergence rates, privacy-utility trade-offs, and hyperparameter sensitivity across different privacy levels (ε).

Result: DP-SGD converges at O(1/ε²) privacy-utility trade-off with speed independent of ε, while DP-SignSGD converges at O(1/ε) trade-off with linear ε speed. Under optimal learning rates, both achieve comparable asymptotic performance, but DP-SignSGD's learning rate is essentially ε-independent, making adaptive methods more practical for transfer across privacy levels.

Conclusion: DP-SignSGD (and by extension DP-Adam) offers better practical utility due to ε-independent hyperparameters that transfer across privacy levels without retuning, making adaptive methods more practical for real-world DP training scenarios.
...
### [333] [Coalgebras for categorical deep learning: Representability and universal approximation](https://arxiv.org/abs/2603.03227)
*Dragan Mašulović*

Main category: cs.LG

TL;DR: The paper develops a coalgebraic foundation for equivariant representation in deep learning, connecting categorical abstractions of invariant behavior with concrete neural architectures through universal approximation theorems.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between abstract categorical specifications of invariant behavior in deep learning and their concrete realization in neural architectures, extending beyond geometric deep learning's focus on group actions to more general categorical frameworks.

Method: Develops a coalgebraic foundation using category theory, where data embeddings are formalized as functors from SET to VECT, and invariant behavior is modeled by endofunctors. Establishes compatibility between these functors and proves universal approximation theorems for equivariant maps.

Result: 1) Shows that given data embeddings and invariant behavior models, there exists a corresponding endofunctor on VECT that recovers analogous invariant behavior on embedded data. 2) Proves a universal approximation theorem showing continuous equivariant functions can be approximated within this coalgebraic framework for broad symmetry classes.

Conclusion: The work provides a categorical bridge between abstract specification of invariant behavior and its concrete realization in neural architectures, establishing coalgebraic foundations for equivariant representation in deep learning beyond traditional group-theoretic approaches.
...
### [334] [ALARM: Automated MLLM-Based Anomaly Detection in Complex-EnviRonment Monitoring with Uncertainty Quantification](https://arxiv.org/abs/2512.03101)
*Congjing Zhang,Feng Lin,Xinyi Zhao,Pei Guo,Wei Li,Lin Chen,Chaoyue Zhao,Shuai Huang*

Main category: cs.LG

TL;DR: ALARM is a UQ-supported MLLM-based visual anomaly detection framework that integrates uncertainty quantification with quality-assurance techniques for robust performance across domains.
...
Conclusion: ALARM provides a robust UQ-supported MLLM-based framework for visual anomaly detection that demonstrates superior performance and cross-domain applicability for reliable decision-making in complex environments.
...
### [335] [SynthCharge: An Electric Vehicle Routing Instance Generator with Feasibility Screening to Enable Learning-Based Optimization and Benchmarking](https://arxiv.org/abs/2603.03230)
*Mertcan Daysalilar,Fuat Uyguroglu,Gabriel Nicolosi,Adam Meyers*

Main category: cs.LG

TL;DR: SynthCharge is a parametric generator for EVRPTW instances with feasibility screening, enabling dynamic benchmarking for neural routing models.


<details>
  <summary>Details</summary>
Motivation: Existing EVRPTW benchmarks are static and lack verifiable feasibility, limiting reproducible evaluation of learning-based routing models.

Method: Parametric generator that creates diverse EVRPTW instances with adaptive energy scaling, range-aware charging station placement, and systematic feasibility screening.

Result: Generator can produce instances up to 500 customers (tested 5-100), with geometry integration and guaranteed structural validity through feasibility filtering.

Conclusion: SynthCharge provides dynamic benchmarking infrastructure needed to systematically evaluate robustness of neural routing and data-driven approaches.
...
### [336] [Guiding Sparse Neural Networks with Neurobiological Principles to Elicit Biologically Plausible Representations](https://arxiv.org/abs/2603.03234)
*Patrick Inoue,Florian Röhrbein,Andreas Knoblauch*

Main category: cs.LG

TL;DR: A biologically inspired learning rule that naturally integrates neurobiological principles (sparsity, lognormal weight distributions, Dale's law) improves DNN generalization, few-shot learning, and adversarial robustness while producing biologically plausible neural representations.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks struggle with generalization, few-shot learning, and continuous adaptation compared to biological neural systems. This gap exists because DNNs fail to emulate the efficient, adaptive learning mechanisms of biological networks.

Method: The study introduces a biologically inspired learning rule that naturally integrates neurobiological principles including sparsity, lognormal weight distributions, and adherence to Dale's law without requiring explicit enforcement.

Result: The model enhances robustness against adversarial attacks and demonstrates superior generalization, particularly in few-shot learning scenarios. It leads to the emergence of biologically plausible neural representations.

Conclusion: Incorporating neurobiological assumptions into neural network design is effective and could extend from feature-specific to task-specific encoding, potentially offering insights into neural resource allocation for complex tasks.
...
### [337] [On Geometry Regularization in Autoencoder Reduced-Order Models with Latent Neural ODE Dynamics](https://arxiv.org/abs/2603.03238)
*Mikhail Osipov*
...
Method: Four regularization approaches applied during autoencoder pre-training: (a) near-isometry regularization of decoder Jacobian, (b) stochastic decoder gain penalty based on random directional gains, (c) second-order directional curvature penalty, and (d) Stiefel projection of first decoder layer. Tested in fixed experimental setting for advection-diffusion-reaction equation with neural ODE latent dynamics.

Result: Regularization approaches (a)-(c) often produce latent representations that make subsequent latent-dynamics training more difficult, especially for long-horizon rollouts, despite improving local decoder smoothness. In contrast, (d) consistently improves conditioning-related diagnostics and yields better rollout performance.

Conclusion: Stiefel projection is the most effective regularization approach for this setting, suggesting that downstream impact of latent-geometry mismatch outweighs benefits of improved decoder smoothness from other regularization methods.
...
### [338] [Speculative Speculative Decoding](https://arxiv.org/abs/2603.03251)
*Tanishq Kumar,Tri Dao,Avner May*

Main category: cs.LG

TL;DR: SSD (speculative speculative decoding) parallelizes speculation and verification to eliminate drafting overhead, achieving 2x speedup over speculative decoding and 5x over autoregressive decoding.


<details>
  <summary>Details</summary>
Motivation: Speculative decoding still has sequential dependence between speculation and verification phases, creating a bottleneck. The authors aim to parallelize these operations to further accelerate inference.

Method: Introduces speculative speculative decoding (SSD) where draft model predicts verification outcomes and prepares speculations pre-emptively during ongoing verification. Saguaro algorithm solves three key challenges: predicting verification outcomes, preparing multiple speculations, and efficiently managing speculation-verification pipeline.
...
Conclusion: SSD successfully parallelizes speculation and verification, eliminating drafting overhead and significantly accelerating inference beyond existing speculative decoding methods.
...
### [339] [Learning Demographic-Conditioned Mobility Trajectories with Aggregate Supervision](https://arxiv.org/abs/2603.03275)
*Jessie Z. Li,Zhiqing Hong,Toru Shirakawa,Serina Chang*

Main category: cs.LG

TL;DR: ATLAS is a weakly supervised method for generating demographic-conditioned human mobility trajectories using only unlabeled individual trajectories, region-level aggregated mobility features, and census demographic compositions.
...
Result: ATLAS substantially improves demographic realism over baselines (JSD ↓ 12%--69%) and closes much of the gap to strongly supervised training, as shown in experiments on real trajectory data with demographic labels.

Conclusion: The paper provides both practical contributions (ATLAS method and code release) and theoretical analyses identifying key factors for when and why ATLAS works, including demographic diversity across regions and informativeness of aggregate features.
