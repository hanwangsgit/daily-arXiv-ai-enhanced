...
TL;DR: V-Agent is a multi-agent platform for video search and conversation that uses a fine-tuned vision-language model with multimodal retrieval to overcome text-only limitations, achieving state-of-the-art zero-shot performance.


<details>
  <summary>Details</summary>
Motivation: Traditional text-based retrieval systems have limitations in multimodal scenarios where both visual and spoken content need to be interpreted for effective video search and interactive conversations.

Method: Fine-tunes a vision-language model with a small video preference dataset, enhances it with retrieval vectors from an image-text retrieval model, and uses three collaborative agents (routing, search, chat) with a VLM-based retrieval model that embeds video frames and audio transcriptions into a shared multimodal space.
...
Conclusion: V-Agent represents an effective framework for advanced video search and interactive conversations with strong potential for both academic research and real-world applications in multimodal information retrieval.
...
TL;DR: CNN model outperforms VGG-16 for pornographic image detection with 94.87% accuracy, addressing VPN bypass of government-blocked websites.


<details>
  <summary>Details</summary>
Motivation: Indonesian government blocked 59,741 websites (14,266 pornographic) in 2020, but VPNs allow public access, creating need for rapid pornographic content detection systems.

Method: Developed pornographic website identification system using deep learning with CNN and VGG-16 models, comprehensively comparing both approaches for effectiveness.
...
Conclusion: CNN model is more effective than VGG-16 for quickly and accurately detecting pornographic image content, providing solution to VPN bypass problem of blocked websites.
...
Method: AVM uses a frozen Vision Transformer-based encoder to capture consistent visual features, with independently trained modulation paths that account for neural response variations driven by stimulus content and subject identity. It's a structure-preserving framework with modular subnetworks.

Result: AVM outperforms state-of-the-art V1T model by ~2% in predictive correlation across two large-scale mouse V1 datasets, with 9.1% improvement in explained variance (FEVE) under cross-dataset adaptation. Shows robust generalization, interpretable condition-wise modulation, and high architectural efficiency.

Conclusion: AVM provides a unified framework for adaptive neural modeling across biological and experimental conditions, offering a scalable solution under structural constraints. Its design may inform future approaches to cortical modeling in both neuroscience and biologically inspired AI systems.
...
TL;DR: The paper proposes a method linking Finer-CAM explanations to TLS projection segments to identify which structural tree features drive species discrimination in deep learning models, finding models primarily rely on crown features with some species-specific variations.


<details>
  <summary>Details</summary>
Motivation: While new sensors and deep learning achieve high accuracy in tree species classification, their decision processes remain unclear. There's a need to understand which features drive species discrimination to reveal dataset/model limitations, biases, and build confidence in predictions.

Method: Proposed novel method linking Finer-CAM explanations to segments of TLS projections representing structural tree features. Used TLS data from 2,445 trees across 7 European species, trained 5 YOLOv8 models with cross-validation, and analyzed 630 saliency maps to evaluate feature contributions.

Result: Models achieved 96% mean accuracy (SD=0.24%). Analysis showed models primarily rely on crown features for classification, with pronounced results in Silver Birch, European Beech, English oak, and Norway spruce. Stem features contributed more to European ash, Scots pine, and Douglas fir. Finer branches were particularly important. Models considered species similar in ways consistent with human expert judgment.

Conclusion: The study highlights the need for improved understanding of decision processes in tree species classification models to reveal limitations, biases, and build confidence. The proposed method successfully identifies which structural features drive species discrimination, with models showing biologically plausible feature usage patterns.
...
TL;DR: A multi-stage pipeline for generating cohesive video stories with consistent characters using LLM scripts, text-to-image character anchors, and scene-by-scene video synthesis.
...
Method: A filmmaker-like pipeline: 1) LLM generates detailed production script, 2) text-to-image model creates consistent character visuals as anchors, 3) video generation model synthesizes each scene individually using these visual anchors.

Result: Visual anchoring is crucial - removing it causes catastrophic drop in character consistency scores (from 7.99 to 0.55). Also reveals cultural biases in current models, with distinct differences in subject consistency and dynamic degree between Indian vs Western-themed generations.

Conclusion: Multi-stage decomposition with visual anchoring is essential for character consistency in long video generation, and current models exhibit cultural disparities that need addressing.
...
TL;DR: InfoTok is an adaptive video tokenization framework that uses information theory to dynamically allocate tokens based on content richness, achieving better compression than fixed-rate methods.


<details>
  <summary>Details</summary>
Motivation: Current video tokenizers use fixed compression rates, leading to redundancy for simple content and information loss for complex content. Videos have variable information density that requires adaptive token allocation.

Method: Based on Shannon's information theory, the paper introduces an ELBO-based algorithm that approaches theoretical optimality for representation length. A transformer-based adaptive compressor is developed for adaptive tokenization.

Result: State-of-the-art compression performance: saves 20% tokens without performance degradation, achieves 2.3x compression rates while outperforming prior heuristic adaptive approaches.
...
<details>
  <summary>Details</summary>
Motivation: Medical image segmentation for endoscopic videos requires extensive labeled data, which is costly and time-consuming to obtain. There's a need for effective semi-supervised approaches that can leverage both limited labeled data and abundant unlabeled data to improve segmentation performance in clinical applications.

Method: Endo-SemiS employs four key strategies: (1) Cross-supervision between two networks that supervise each other, (2) Uncertainty-guided pseudo-labels from unlabeled data using high-confidence regions, (3) Joint pseudolabel supervision aggregating reliable pixels from both networks, and (4) Mutual learning at feature and image levels. Additionally, a corrective network uses spatiotemporal information from video sequences.

Result: Endo-SemiS substantially outperforms state-of-the-art segmentation methods on two clinical applications: kidney stone laser lithotomy from ureteroscopy and polyp screening from colonoscopy, achieving superior results with limited labeled data.

Conclusion: Endo-SemiS provides an effective semi-supervised segmentation framework for endoscopic videos that successfully leverages both labeled and unlabeled data through multiple complementary strategies, demonstrating strong performance on clinical applications with limited annotation.
...
TL;DR: LongShOTBench is a new diagnostic benchmark for long-form multimodal video understanding that combines temporal length with multimodal richness, featuring open-ended questions, dialogues, and tool use tasks with reference answers and graded rubrics for interpretable evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for multimodal video understanding either focus on temporal length OR multimodal richness, but rarely both. They also rely on single-score accuracy metrics that obscure failure modes, lacking interpretable evaluation.

Method: Created LongShOTBench via scalable human-validated pipeline with open-ended intent-driven questions, single/multi-turn dialogues, multimodal reasoning tasks, and agentic tool use across video/audio/speech. Also developed LongShOTAgent system with preprocessing, search, and iterative refinement.

Result: State-of-the-art MLLMs show large performance gaps: Gemini-2.5-Flash achieves 52.95%, open-source models below 30%, and LongShOTAgent attains 44.66%. Results demonstrate difficulty of real-world long-form video understanding.

Conclusion: LongShOTBench provides practical, reproducible foundation for evaluating and improving MLLMs on long-form multimodal video understanding, with all resources publicly available on GitHub.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs have limited 3D structure and temporal dynamic reasoning capabilities due to weak 4D perception. Existing 3D/4D VQA benchmarks focus on static scenes and lack region-level prompting.

Method: Three key contributions: 1) 4D-RGPT - specialized MLLM for 4D video representations with enhanced temporal perception; 2) Perceptual 4D Distillation (P4D) - training framework transferring 4D representations from frozen expert model; 3) R4D-Bench - depth-aware dynamic scene benchmark with region-level prompting via hybrid automated/human-verified pipeline.

Result: 4D-RGPT achieves notable improvements on both existing 4D VQA benchmarks and the proposed R4D-Bench benchmark.

Conclusion: The proposed 4D-RGPT with P4D training and R4D-Bench evaluation addresses limitations in current MLLMs' 4D perception and temporal understanding for video reasoning tasks.
...
TL;DR: FORMSpoT provides 1.5m resolution forest canopy height mapping and disturbance detection across France (2014-2024), outperforming existing products by an order of magnitude for detecting small disturbances.


<details>
  <summary>Details</summary>
Motivation: European forest carbon sink decline requires better monitoring tools. Existing satellite-based disturbance products are too coarse (typically >100m²) to detect changes at individual tree scale.

Method: Used SPOT-6/7 annual composites with hierarchical transformer model (PVTv2) trained on ALS data. Developed post-processing pipeline with co-registration and spatio-temporal total variation denoising for robust change detection.

Result: FORMSpoT-Δ achieves F1-score of 0.44 in mountainous forests (small, fragmented disturbances), an order of magnitude better than existing benchmarks. Validated against 19 ALS sites and 5,087 NFI plots.

Conclusion: FORMSpoT enables tree-level forest monitoring at national scale for analyzing management practices, detecting early decline signals, and quantifying carbon losses from subtle disturbances. Highlights importance of sustaining high-resolution satellite missions and open-data initiatives.
...
TL;DR: InfCam is a depth-free framework for camera-controlled novel-view video generation that uses infinite homography warping in latent space and data augmentation to achieve high camera-pose fidelity without depth estimation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for camera-controlled video generation struggle with pose fidelity due to depth estimation errors and limited trajectory diversity in training data. There's a need for a solution that can generate high-quality videos faithful to specified camera poses without relying on error-prone depth estimation.

Method: InfCam uses two key components: (1) infinite homography warping that encodes 3D camera rotations directly in the 2D latent space of a video diffusion model, with residual parallax prediction through end-to-end training; (2) a data augmentation pipeline that transforms synthetic multiview datasets into sequences with diverse trajectories and focal lengths.

Result: InfCam outperforms baseline methods in both camera-pose accuracy and visual fidelity, and demonstrates good generalization from synthetic to real-world data.

Conclusion: InfCam provides an effective depth-free solution for camera-controlled video generation that achieves high pose fidelity through latent space homography warping and enhanced data diversity, addressing key limitations of existing approaches.
...
TL;DR: Proposes Interpretable Utility Similarity (IUS), a novel interpretable measure to assess how useful synthetic medical images are for developing deep learning clinical decision support systems, outperforming traditional metrics.


<details>
  <summary>Details</summary>
Motivation: Current methods for evaluating synthetic medical image quality (user studies, inception scores, classification performance) lack interpretability and don't explain why one synthetic dataset might be more useful than another for clinical decision support systems.

Method: Develops Interpretable Utility Similarity (IUS) inspired by generalized neural additive models. Unlike inception-based measures, IUS is interpretable and explains synthetic dataset utility based on clinically relevant image features for CDS systems.

Result: On endoscopic, dermoscopic, and fundus imaging datasets, selecting synthetic images with high IUS scores improved classification performance by up to 54.6%. IUS also demonstrated generality for greyscale X-ray and ultrasound modalities.

Conclusion: IUS provides an interpretable, quantitative measure for assessing synthetic medical image utility that can significantly improve DL-based clinical decision support system development by identifying the most useful synthetic data.
...
Method: Two main components: (1) coarse-to-fine model for temporally coherent hair motion dynamics, (2) strand-guided optimization module for dynamic 3D Gaussian representation with differentiable rendering support.

Result: Achieves promising geometry and appearance results, provides scalable data-driven alternative to physics-based simulation, and integrates seamlessly with 3D Gaussian avatar frameworks.

Conclusion: DGH offers a fully data-driven approach that generalizes across hairstyles and motions, enabling realistic animatable hair for high-fidelity avatars without physics simulation limitations.
...
TL;DR: This survey identifies a research gap: no prior work addresses transformer-based maritime radar frame prediction, despite radar's all-weather reliability and transformers' success in related domains like AIS trajectory prediction and sonar forecasting.


<details>
  <summary>Details</summary>
Motivation: Maritime autonomous systems need robust predictive capabilities for vessel motion and environmental dynamics. While transformers work well for AIS trajectory prediction and sonar frame forecasting, their application to maritime radar frame prediction remains unexplored, creating a critical gap given radar's all-weather reliability for navigation.
...
Conclusion: Defines a clear research gap and motivates a concrete research direction for future work in transformer-based maritime radar frame prediction to enhance maritime autonomous systems.
...
TL;DR: SDUM is a universal deep learning framework for MRI reconstruction that scales across diverse protocols without task-specific fine-tuning, achieving state-of-the-art results across multiple challenge tracks.


<details>
  <summary>Details</summary>
Motivation: Current deep learning MRI reconstructions are protocol-specific, limiting generalization and deployment across diverse clinical MRI protocols (different anatomical targets, contrasts, sampling patterns, acceleration factors).

Method: SDUM combines Restormer-based reconstructor, learned coil sensitivity map estimator, sampling-aware weighted data consistency, universal conditioning on cascade index and protocol metadata, and progressive cascade expansion training.

Result: SDUM shows foundation-model-like scaling (PSNR ∼ log(parameters) with r=0.986), achieves SOTA across all four CMRxRecon2025 challenge tracks (+1.0 dB over specialized baselines), outperforms PromptMR+ by +0.55 dB on CMRxRecon2024, and exceeds PC-RNN by +1.8 dB on fastMRI brain.
...
TL;DR: A method to transform casual photos into professional-looking portraits by reposing people in canonical UV space with multi-image fine-tuning, preserving identity while improving pose, lighting, and clothing.


<details>
  <summary>Details</summary>
Motivation: Professional photos have beautiful lighting, interesting poses, and flattering quality that casual self-photos lack. There's a need to transform ordinary photos into professional portraits while preserving the person's unique identity, but no large paired dataset exists of the same person photographed both casually and professionally.

Method: 1) Transform input photo and person's face to canonical UV space, coupled with reposing methodology to model occlusions and novel view synthesis. 2) Personalize output via multi-image fine-tuning. Operating in UV space allows leveraging existing unpaired datasets.

Result: The approach yields high-quality, reposed portraits and achieves strong qualitative and quantitative performance on real-world imagery.

Conclusion: The method successfully creates professional versions of casual photos by operating in UV space and using multi-image fine-tuning, overcoming the lack of paired training data while preserving personal identity.
...
TL;DR: A training-free framework for document background generation with multi-page editing, text readability preservation via latent masking and automated contrast optimization, and thematic continuity through context summarization.


<details>
  <summary>Details</summary>
Motivation: To bridge generative modeling with natural design workflows by enabling automated document background generation that preserves text readability, maintains multi-page thematic consistency, and allows flexible customization while keeping text regions intact.

Method: Uses latent masking with smooth barrier functions to softly attenuate diffusion updates in text regions; introduces Automated Readability Optimization (ARO) to place semi-transparent backing shapes behind text based on WCAG 2.2 contrast standards; maintains multi-page consistency through summarization-and-instruction process that recursively guides generations; treats documents as structured compositions with separate layers for text, figures, and backgrounds.

Result: Produces visually coherent, text-preserving, and thematically aligned documents without requiring training; ensures readability while maintaining aesthetic harmony; allows targeted background editing without compromising text legibility; balances automated consistency with user customization through prompts.

Conclusion: The framework successfully bridges generative modeling with natural design workflows by providing a training-free solution for document-centric background generation that preserves readability, maintains thematic continuity across pages, and offers flexible customization while treating documents as structured compositions.
...
TL;DR: PhysFire-WM: A physics-informed world model that integrates combustion dynamics from physical simulators with cross-task collaborative training for accurate fine-grained fire spread prediction using infrared and mask data.
...
Method: PhysFire-WM encodes structured priors from a Physical Simulator to rectify physical discrepancies, combined with Cross-task Collaborative Training (CC-Train) that uses parameter sharing and gradient coordination to integrate thermal radiation dynamics and spatial boundary delineation.
...
Conclusion: The paper validates the importance of physical priors and cross-task collaboration, providing new insights for applying physics-informed world models to disaster prediction tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLMs like CLIP require expensive annotated image-text pairs for modality alignment and use dual-tower encoders that hinder lightweight deployment. There's a need for more efficient, annotation-free approaches.

Method: LGCLIP uses LLMs to generate class-specific prompts, guides diffusion models to synthesize reference images as visual prototypes, extracts features from real images, and compares them with prototype features using only a visual encoder.

Result: Experimental results validate LGCLIP's feasibility and efficiency, showing strong performance in zero-shot classification tasks while being lightweight and requiring only class labels as input.

Conclusion: LGCLIP establishes a novel paradigm for classification that eliminates manual annotation requirements and dual-encoder architectures, offering a lightweight, efficient alternative to traditional VLMs.
...
TL;DR: ABE-CLIP is a training-free method that enhances attribute-object binding in CLIP models using semantic refinement and local token-patch alignment.


<details>
  <summary>Details</summary>
Motivation: CLIP struggles with compositional image-text matching, particularly associating objects with their attributes, due to its global representation overlooking fine-grained semantics. Existing methods require additional training or extensive negative sampling but show limited generalization to novel concepts.

Method: ABE-CLIP uses: 1) Semantic Refinement Mechanism to refine token embeddings for object and attribute phrases, mitigating attribute confusion; 2) Local Token-Patch Alignment that computes similarity scores between refined textual tokens and their most relevant image patches, then aggregates localized scores for final image-text similarity.
...
Conclusion: ABE-CLIP effectively addresses CLIP's attribute binding limitations without requiring additional training, demonstrating strong generalization to novel compositional concepts through semantic refinement and localized alignment.
...
TL;DR: The paper investigates discrepancies between objective measurements of urban greenery (like Green View Index) and subjective human perceptions, finding that while these discrepancies are globally consistent, location/cultural factors significantly influence perception more than demographics or personality.


<details>
  <summary>Details</summary>
Motivation: There's a gap between objective measurements of urban greenery and how people actually perceive green spaces, which is important for urban planning since what people see/feel may not match actual vegetation measurements. Understanding these discrepancies can improve urban design and green space planning.

Method: Used street view imagery to extract contextual information and conducted a comprehensive urban visual perception survey with 1,000 people across five countries, collecting extensive demographic and personality data. Analyzed discrepancies between objective measures (Green View Index) and subjective scores (pairwise ratings) using human, geographic, and spatial dimensions.

Result: Discrepancies between objective and subjective greenery measures are comparable worldwide; demographics and personality don't significantly affect perception. Location (where people live) is the top factor influencing perceptual differences, suggesting cultural, environmental, and experiential factors shape how people observe urban greenery.

Conclusion: Urban greenery perception is strongly influenced by location/cultural factors rather than individual demographics or personality, highlighting the importance of considering local context and cultural perspectives in urban green space planning and assessment.
...
TL;DR: A globally optimal solver for relative pose estimation in multi-camera systems using affine correspondences and known vertical direction, outperforming state-of-the-art methods.
...
Method: 1) Decouple rotation matrix and translation vector, 2) Establish cost function for relative rotation angle minimizing algebraic error from affine correspondences, 3) Convert global optimization into two polynomials using characteristic equation and derivative, 4) Solve rotation angle with polynomial eigenvalue solver, 5) Extract translation from eigenvector, 6) Provide linear solution for small rotations.
...
Conclusion: The globally optimal solver using affine correspondences with known vertical direction provides superior accuracy for relative pose estimation in multi-camera systems, with practical applications in mobile devices and autonomous vehicles.
...
TL;DR: ARCD is a plug-and-play method that uses anatomical masks to guide contrastive decoding at three levels (token, attention, logits) to reduce hallucinations in medical VLMs without requiring expensive training data.


<details>
  <summary>Details</summary>
Motivation: Medical VLMs suffer from hallucinations where they rely on textual priors rather than visual evidence. Existing solutions have limitations: training-based methods need costly expert annotations, while training-free methods like contrastive decoding use global, untargeted corrections that are unreliable in complex clinical settings.

Method: Anatomical Region-Guided Contrastive Decoding (ARCD) uses anatomical masks to provide targeted, region-specific guidance. It implements a three-tiered contrastive decoding process that dynamically re-weights at token, attention, and logits levels to steer model focus onto specified anatomical regions.
...
Conclusion: ARCD provides an effective, data-efficient solution to mitigate hallucinations in medical VLMs by offering targeted anatomical guidance through a plug-and-play contrastive decoding approach that doesn't require expensive training data.
...
TL;DR: Fose: A lightweight pansharpening network combining one-step diffusion model distillation with E2E model fusion, achieving 7.42x speedup over baseline diffusion models.


<details>
  <summary>Details</summary>
Motivation: Current pansharpening methods have limitations: diffusion models require multi-step inference (50 steps) which is computationally expensive and time-consuming, while end-to-end models lack prior knowledge and have simple structures that limit performance.

Method: Four-stage training strategy: 1) One-step distillation of enhanced SOTA diffusion model (compressing 50 steps to 1 step), 2) Fusion of E2E model with one-step DM using lightweight ensemble blocks, 3) Training lightweight network Fose that combines both approaches.

Result: Significant improvement on three commonly used benchmarks, achieving 7.42x speedup ratio compared to baseline diffusion models while delivering much better performance.

Conclusion: Fose successfully addresses computational efficiency and performance limitations by fusing one-step diffusion model distillation with E2E models, creating a lightweight yet powerful pansharpening solution.
...
TL;DR: Reasoning Palette: A latent-modulation framework using VAE-inferred stochastic latent variables to guide LLM reasoning strategies before token generation, improving exploration diversity and RL training efficiency.


<details>
  <summary>Details</summary>
Motivation: Current stochastic sampling in large language models produces redundant reasoning paths with limited high-level diversity, limiting both inference performance and reinforcement learning training efficiency.

Method: Proposes Reasoning Palette with a VAE that infers latent variables from question-answer embeddings, decodes them into learnable token prefixes prepended to input prompts, enabling internal sampling over reasoning strategies before output generation.

Result: Achieves consistent performance gains over standard RL methods across multiple reasoning benchmarks, enabling interpretable and controllable strategic behavior in vision-language models.

Conclusion: The framework provides structured exploration through latent conditioning, significantly enhancing exploration efficiency and sustained learning capability in RL optimization for language models.
...
TL;DR: CheXPO-v2 is a novel alignment framework for medical VLMs that shifts from outcome to process supervision using Knowledge Graph Consistency Reward with Entity-Relation Matching to reduce hallucinations and improve clinical reliability.


<details>
  <summary>Details</summary>
Motivation: Medical Vision-Language Models are prone to hallucinations that compromise clinical reliability. Existing reinforcement learning methods like GRPO rely on sparse outcome-based rewards, which encourage models to generate verbose, unverifiable reasoning that obscures factual errors and poses safety risks.

Method: Proposes CheXPO-v2 with Knowledge Graph Consistency Reward mechanism using Entity-Relation Matching. Parses reasoning steps into structured "Disease, Relation, Anatomy" triplets for fine-grained supervision. Integrates with hard-example mining strategy to penalize incoherent logic and hallucinations at atomic level.
...
Conclusion: CheXPO-v2 effectively addresses the hallucination problem in medical VLMs by shifting from outcome to process supervision, providing a more reliable and clinically sound alignment framework with superior performance and data efficiency.
...
Method: Two-stage training: 1) Self-supervised pretraining on unlabeled images, 2) Supervised autoregressive pretraining on limited high-quality data. Uses novel model-merging scheme to combine encoders trained with different text decoders, and ensemble training to fuse features from generalist encoders (SigLIP2) with document/web-specific representations.
...
Conclusion: DAVE successfully bridges the gap in vision encoders for VLMs, providing robust structural and spatial information essential for document understanding and web agent tasks through innovative training approaches that leverage unlabeled data and specialized techniques.
...
TL;DR: AOM is a universal remote sensing foundation model that handles arbitrary band compositions, sensor types, and resolution scales through spectrum-independent tokenization, multi-scale patch embedding, and semantic alignment.


<details>
  <summary>Details</summary>
Motivation: Existing remote sensing foundation models are limited by fixed band configurations and resolutions, making them vulnerable to real-world scenarios with missing bands, cross-sensor fusion, and unseen spatial scales, which restricts their generalization and practical deployment.

Method: AOM introduces: 1) spectrum-independent tokenizer with dedicated band embeddings for each channel, 2) multi-scale adaptive patch embedding for varying resolutions, 3) multi-scale semantic alignment mechanism, and 4) channel-wise self-supervised masking and reconstruction pretraining strategy.
...
Conclusion: AOM successfully addresses the limitations of existing RSFMs by providing a universal model that accommodates arbitrary optical sensor configurations, enabling better generalization and practical deployment in real-world remote sensing applications.
...
TL;DR: Learning-based visual localization method that learns global descriptors consistent with both geometric structure and visual similarity, correcting erroneous associations from unreliable overlap scores.


<details>
  <summary>Details</summary>
Motivation: Existing visual localization methods rely on geometric cues alone for global descriptors, limiting discriminative power and robustness to noisy geometric constraints. Need descriptors that consider both visual similarity and spatial connectivity.

Method: Proposes an aggregator module that learns global descriptors consistent with both geometrical structure and visual similarity. Uses batch-mining strategy based on overlap scores and modified contrastive loss, training without manual place labels.

Result: Substantial localization gains in large-scale environments while preserving computational and memory efficiency. Outperforms existing methods on challenging benchmarks.

Conclusion: The proposed method effectively learns robust global descriptors by jointly considering visual and geometric consistency, enabling more accurate visual localization without manual supervision.
...
<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from "visual forgetting" where they lose visual grounding during extended reasoning. Current training entangles abstract reasoning and visual perception, causing both weak reasoning foundations and poor perception timing.

Method: Two-stage curriculum: 1) Disentangled SFT builds abstract reasoning backbone on text-only data, then anchors to vision with Perception-Grounded Chain-of-Thought. 2) Reinforcement learning with Pivotal Perception Reward teaches models when to look by linking perceptual actions to linguistic uncertainty markers.

Result: The framework transforms models from heuristic-driven observers to strategic, grounded reasoners by addressing both the cold-start reasoning deficiency and strategic perception deficit.

Conclusion: Disentangling reasoning and perception skills through curriculum learning and RL-based perception timing solves visual forgetting, enabling MLLMs to maintain visual grounding during complex, long-chain reasoning tasks.
...
TL;DR: VideoDetective: An efficient question-aware memory mechanism for MLLMs to process long videos by iteratively seeking critical clues through token compression and recurrent aggregation.


<details>
  <summary>Details</summary>
Motivation: Long Video QA is challenging for MLLMs due to immense context, information overload, and prohibitive memory consumption. Existing methods either miss useful information or require excessive computation, while only small amounts of crucial information are actually needed for answering questions.

Method: VideoDetective uses iterative processing of video sub-segments with question-aware compression via special memory tokens. It recurrently aggregates and stores these memory tokens to update history context for reuse in subsequent sub-segments, enabling efficient processing of long videos.

Result: The method enables MLLMs with 32K context length to process 100K tokens (3600 frames, hour-long video at 1fps) in only 2 minutes using 37GB GPU memory. It outperforms on multiple long video benchmarks by effectively seeking critical clues from massive information.

Conclusion: VideoDetective provides an efficient solution for long video QA by focusing on question-aware critical clue seeking through iterative compression and recurrent memory aggregation, addressing memory and computational constraints while maintaining effectiveness.
...
Result: Mitty achieves state-of-the-art results on Human2Robot and EPIC-Kitchens datasets, demonstrates strong generalization to unseen environments, and provides new insights for scalable robot learning from human observations.

Conclusion: Mitty enables direct translation of human demonstrations into robot-execution videos without intermediate abstractions, leveraging video in-context learning and addressing data scarcity through automatic synthesis, representing a significant advancement for scalable robot learning from human observations.
...
TL;DR: AnyCXR is a unified framework for generalizable multi-organ segmentation of chest X-rays using only synthetic supervision, achieving strong zero-shot performance across diverse projection angles without real annotations.


<details>
  <summary>Details</summary>
Motivation: Robust anatomical segmentation of chest X-rays is challenging due to scarcity of comprehensive annotations and substantial variability in real-world acquisition conditions. Current methods struggle with generalization across different projection angles and imaging conditions.

Method: Combines Multi-stage Domain Randomization (MSDR) engine generating over 100,000 anatomically faithful synthetic radiographs from 3D CT volumes with Conditional Joint Annotation Regularization (CAR) learning strategy that leverages partial/imperfect labels by enforcing anatomical consistency in latent space.

Result: Achieves strong zero-shot generalization on multiple real-world datasets, accurately delineating 54 anatomical structures in PA, lateral, and oblique views. Supports downstream clinical tasks including automated cardiothoracic ratio estimation, spine curvature assessment, and disease classification with improved diagnostic performance.
...
TL;DR: WDFFU-Mamba is a novel segmentation network for breast ultrasound images that integrates wavelet-guided enhancement and dual-attention feature fusion within a U-shaped Mamba architecture to address challenges like speckle noise and blurred boundaries.
...
Conclusion: The combination of wavelet-domain enhancement and attention-based fusion improves both accuracy and robustness of BUS image segmentation while maintaining computational efficiency. The model shows strong generalization across datasets and is promising for real-world clinical applications.
...
TL;DR: General-purpose ultrasound AI models show high multi-task performance but struggle with domain generalization to unseen data centers.


<details>
  <summary>Details</summary>
Motivation: Current ultrasound AI tools are fragmented into single-task applications, limiting clinical utility compared to versatile modern ultrasound systems that can perform multiple examinations.

Method: The Universal UltraSound Image Challenge 2025 (UUSIC25) involved developing general-purpose deep learning models on 11,644 images, evaluated on an independent multi-center test set of 2,479 images including completely unseen data centers to assess generalization.

Result: Top model (SMART) achieved macro-averaged DSC of 0.854 across 5 segmentation tasks and AUC of 0.766 for binary classification. Models showed strong segmentation performance (fetal head DSC: 0.942) but significant performance drop in breast cancer molecular subtyping from AUC 0.571 (internal) to 0.508 (unseen external center).

Conclusion: General-purpose AI models can achieve high accuracy and efficiency across multiple ultrasound tasks using single architecture, but domain generalization remains a critical challenge for clinical deployment, especially for complex tasks subject to domain shift.
...
TL;DR: VLMIR is a vision-language model guided image restoration framework that leverages CLIP's vision-language priors to enhance restoration through improved visual perception and semantic understanding, achieving superior performance across universal and degradation-specific tasks.
...
Method: Two-stage framework: 1) VLM-based feature extraction with complementary visual/linguistic representations via CLIP, aligning embeddings of low/high-quality image captions with cosine similarity loss and LoRA fine-tuning, plus degradation predictor; 2) Diffusion-based restoration integrating these embeddings via cross-attention mechanisms.

Result: Extensive experiments show VLMIR achieves superior performance across both universal and degradation-specific image restoration tasks, demonstrating the effectiveness of integrated visual and linguistic knowledge.

Conclusion: The framework successfully leverages vision-language priors from VLMs like CLIP to enhance image restoration capabilities, highlighting the critical role of integrated visual and linguistic knowledge in advancing restoration performance.
...
TL;DR: HiSIR-Net: A high-resolution self-supervised reconstruction framework for pixel-wise PCBA defect inspection that addresses challenges of insufficient labeled data and micro-defects in complex 4K-resolution images.


<details>
  <summary>Details</summary>
Motivation: Automated defect inspection of PCBA is challenging due to insufficient labeled data, micro-defects with few pixels, and visually-complex high-resolution images. There's a gap in high-resolution PCBA datasets for research.

Method: HiSIR-Net combines two lightweight modules: 1) Selective Input-Reconstruction Gate (SIR-Gate) that decides where to trust reconstruction vs. original input to reduce artifacts; 2) Region-level Optimized Patch Selection (ROPS) with positional cues for coherent overlapping patch reconstructions across arbitrary resolutions. Also introduces SIPCBA-500 dataset of 500 images.

Result: Superior localization performance on SIPCBA-500 and public benchmarks, producing clean high-resolution anomaly maps with low false positive rates while running at practical speed.

Conclusion: HiSIR-Net effectively addresses PCBA defect inspection challenges through self-supervised reconstruction, achieving practical performance on real 4K-resolution boards with minimal false alarms.
...
TL;DR: ProCache is a training-free dynamic feature caching framework for Diffusion Transformers that achieves 1.96-2.90x acceleration with minimal quality loss by using non-uniform caching schedules and selective computation.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) have state-of-the-art generative performance but high computational cost hinders real-time deployment. Existing feature caching methods have two key limitations: (1) uniform caching intervals don't match DiT's non-uniform temporal dynamics, and (2) naive feature reuse with large intervals causes severe error accumulation.

Method: ProCache has two core components: (1) constraint-aware caching pattern search module that generates non-uniform activation schedules through offline constrained sampling, tailored to the model's temporal characteristics; (2) selective computation module that selectively computes within deep blocks and high-importance tokens for cached segments to mitigate error accumulation with minimal overhead.

Result: Extensive experiments on PixArt-alpha and DiT demonstrate ProCache achieves up to 1.96x and 2.90x acceleration with negligible quality degradation, significantly outperforming prior caching-based methods.

Conclusion: ProCache effectively addresses the limitations of existing feature caching methods for DiTs by dynamically adapting to the model's temporal characteristics and selectively computing only necessary components, enabling significant acceleration without quality loss.
...
TL;DR: A generative framework for high-quality PBR texture generation on 3D meshes using fine-tuned VAE and diffusion priors with locality-preserving regularization.
...
Conclusion: The proposed generative framework effectively leverages pretrained latent image models while learning a material latent space (MatLat) through targeted fine-tuning and locality-preserving regularization, enabling high-quality PBR texture generation on 3D meshes.
...
TL;DR: EMAG is a training-free guidance method for diffusion transformers that adaptively modifies attention at inference to generate harder, semantically faithful negative samples, improving generation quality over CFG.


<details>
  <summary>Details</summary>
Motivation: Existing guidance techniques like CFG and recent strong/weak model approaches lack reliable control over negative sample granularity/difficulty and have fixed target-layer selection, limiting their ability to surface subtle failure modes.

Method: EMAG modifies attention at inference time in diffusion transformers using a statistics-based, adaptive layer-selection rule to produce harder, semantically faithful negative samples (fine-grained degradations).

Result: EMAG boosts quality and human preference score (HPS) by +0.46 over CFG, surfaces difficult failure modes, enables refinement of subtle artifacts, and composes well with advanced guidance techniques like APG and CADS.

Conclusion: EMAG provides a training-free, adaptive approach to guidance that generates more challenging negative samples for better generation refinement, outperforming existing methods and being compatible with other advanced guidance techniques.
...
TL;DR: DRIM enables deep, reliable multi-turn reasoning in vision-language models by incorporating self-reflection and correction mechanisms when thinking with images in multimodal chain-of-thought.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models struggle to reflect on and correct incorrect reasoning trajectories when using tools to analyze visual inputs in their chain-of-thought, limiting their reliability in complex visual reasoning tasks.

Method: Three-stage pipeline: 1) Data construction using high-resolution images with verifiable visual QA pairs requiring multi-turn tool calls; 2) Cold-start supervised fine-tuning with tool trajectories; 3) Reinforcement learning with redundancy-penalized policy optimization that penalizes incorrect answers without sufficient multi-scale exploration.

Result: Extensive experiments demonstrate that DRIM achieves superior performance on visual understanding benchmarks compared to existing models.

Conclusion: DRIM successfully enables deep but reliable multi-turn reasoning in vision-language models by developing self-reflective reasoning patterns through a carefully designed training pipeline with redundancy-penalized optimization.
...
Conclusion: RSHR-Bench enables faithful assessment of remote sensing visual understanding and reasoning by addressing limitations of prior benchmarks. The benchmark reveals that current multimodal models have significant room for improvement in handling super-high-resolution imagery and genuine visual reasoning tasks.
...
TL;DR: CodeDance uses executable code as a general solver for visual reasoning, enabling flexible tool composition and transparent reasoning with emergent capabilities beyond supervised training.


<details>
  <summary>Details</summary>
Motivation: Current open-source visual reasoning approaches rely on text-only chains, rigid visual schemas, or single-step pipelines, limiting flexibility, interpretability, and transferability on complex tasks. There's a need for more general and scalable visual reasoning mechanisms.

Method: CodeDance explores executable code as a general solver for visual reasoning. It defines, composes, and executes code to orchestrate multiple tools, compute intermediate results, and render visual artifacts. The method introduces a reward for balanced and adaptive tool-call to balance exploration with efficiency and mitigate tool overuse.

Result: CodeDance demonstrates novel emergent behaviors during RL training: novel tool invocations, unseen compositions, and cross-task transfer without task-specific fine-tuning. It consistently outperforms schema-driven and text-only baselines across reasoning benchmarks (visual search, math, chart QA) and surpasses advanced closed models like GPT-4o and larger open-source models.

Conclusion: Executable code provides a general and scalable mechanism for visual reasoning that enables flexible tool composition, transparent reasoning, and emergent capabilities beyond supervised training, offering advantages over both open-source and closed model approaches.
...
TL;DR: ADK enhances Vision-Language Models by using LLM-generated descriptive prompts as auxiliary knowledge, improving classification without computational overhead.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with distribution shifts in downstream tasks, and existing PEFT methods rely on fixed prompts that fail to capture class semantics. Image-induced prompts help but add computational cost.

Method: ADK uses LLMs to generate descriptive prompts offline, then deploys them as: 1) Compositional Knowledge (averaged representation for ambiguous classes), and 2) Instance-Specific Knowledge (lightweight attention selects relevant descriptions per image).

Result: ADK consistently boosts multiple PEFT baselines, achieving state-of-the-art performance across various scenarios without adding parameters or computational overhead.

Conclusion: ADK provides an efficient, parameter-free plug-and-play solution that enriches text representations with auxiliary descriptive knowledge, improving VLM adaptation to distribution shifts.
...
TL;DR: RadImageNet-VQA is a large-scale radiologic VQA dataset with 750K CT/MRI images and 7.5M QA pairs covering abnormality detection, anatomy recognition, and pathology identification across 8 anatomical regions and 97 pathologies, designed to address limitations of existing medical VQA datasets.
...
Method: Built from expert-curated annotations, the dataset provides 750K images paired with 7.5M question-answer samples covering three key tasks (abnormality detection, anatomy recognition, pathology identification) across eight anatomical regions and 97 pathology categories, supporting open-ended, closed-ended, and multiple-choice questions.

Result: Experiments show state-of-the-art vision-language models struggle with fine-grained pathology identification, especially in open-ended settings even after fine-tuning. Text-only analysis confirms the dataset is free from linguistic shortcuts as model performance collapses to near-random without image inputs.

Conclusion: RadImageNet-VQA addresses critical gaps in medical VQA research by providing a large-scale, linguistically robust benchmark for CT and MRI exams, revealing current model limitations and enabling future advances in radiologic AI.
...
TL;DR: EMMA is a comprehensive benchmark that evaluates concept erasure methods across 5 dimensions and 12 metrics, revealing that current methods struggle with implicit prompts, visually similar concepts, and can amplify bias.


<details>
  <summary>Details</summary>
Motivation: Text-to-image generation raises privacy, bias, and copyright concerns. Concept erasure techniques aim to remove undesired concepts without full retraining, but current evaluations are limited to simplistic prompts and few concepts.

Method: Introduces EMMA benchmark with 5 key dimensions: 1) robustness to indirect descriptions, 2) handling visually similar non-target concepts, 3) gender/ethnicity bias analysis, 4) standard metrics (quality, efficiency), 5) social awareness. Evaluates 5 erasure methods across 5 domains (objects, celebrities, art styles, NSFW, copyright) using 12 metrics.

Result: Existing methods struggle with implicit prompts (generate erased concepts when indirectly referenced) and visually similar non-target concepts. Some methods amplify gender and ethnicity bias compared to original models.

Conclusion: Current concept erasure methods have significant limitations in real-world scenarios. EMMA provides a more comprehensive evaluation framework that reveals weaknesses in robustness and social impact, highlighting the need for improved erasure techniques.
...
TL;DR: A diverse, high-quality dataset of color fundus images with detailed artery-vein segmentation annotations for developing ML algorithms in ophthalmology.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive dataset supporting development and evaluation of machine learning algorithms for vascular analysis in ophthalmology, addressing the need for diverse, high-quality annotations under real-world conditions.

Method: Sampled CFIs from longitudinal Rotterdam Study with wide age/device/capture condition range. Used custom annotation interface with separate artery/vein/unknown vessel layers, starting from initial vessel segmentation masks. Employed connected component visualization tools for connectivity verification and correction.

Result: Dataset includes 1024x1024-pixel PNG images in three modalities: original RGB, contrast-enhanced versions, and RGB-encoded A/V masks. Contains challenging samples typically excluded by automated quality assessment but judged to have valuable vascular information.

Conclusion: Provides rich, heterogeneous source of CFIs with high-quality segmentations supporting robust ML model benchmarking and training under real-world variability in image quality and acquisition settings.
...
TL;DR: DESSERT is a diffusion-based framework for event-driven single-frame synthesis that uses residual training with Stable Diffusion to overcome holes and blurring issues in traditional event-based video frame prediction methods.


<details>
  <summary>Details</summary>
Motivation: Traditional video frame prediction suffers from errors in dynamic scenes due to lack of future frame information. Event cameras help but existing event-based methods using optical flow and pixel warping introduce holes and blurring when displacement is inaccurate.

Method: Two-stage training: (1) Event-to-Residual Alignment VAE aligns event frames between anchor/target frames with residuals, (2) diffusion model denoises residual latent conditioned on event data. Uses pre-trained Stable Diffusion with residual training and Diverse-Length Temporal augmentation for robustness.
...
Conclusion: DESSERT successfully addresses limitations of traditional event-based methods by combining diffusion models with residual training and temporal augmentation, achieving superior frame synthesis quality and temporal consistency.
...
### [49] [InsertAnywhere: Bridging 4D Scene Geometry and Diffusion Models for Realistic Video Object Insertion](https://arxiv.org/abs/2512.17504)
*Hoiyeong Jin,Hyojin Jang,Jeongho Kim,Junha Hyung,Kinam Kim,Dongjin Kim,Huijin Choi,Hyeonji Kim,Jaegul Choo*

Main category: cs.CV

TL;DR: InsertAnywhere is a new video object insertion framework that achieves geometrically consistent object placement and appearance-faithful video synthesis through 4D scene understanding and diffusion-based generation.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based video generation methods struggle with realistic video object insertion due to limited 4D scene understanding and inadequate handling of occlusion and lighting effects.

Method: Uses a 4D aware mask generation module for scene geometry reconstruction and object propagation, extends diffusion-based video generation for joint object and local variation synthesis, and trains on ROSE++ dataset with illumination-aware synthetic triplets.

Result: Produces geometrically plausible and visually coherent object insertions across diverse real-world scenarios, significantly outperforming existing research and commercial models.

Conclusion: InsertAnywhere advances video object insertion by combining 4D scene understanding with diffusion-based generation, enabling realistic object placement with proper occlusion and lighting effects.
...
TL;DR: POLYSOME enables synthetic instruction generation for whole-slide images, creating HISTAI-Instruct dataset (1.1M instruction-response pairs) to train ANTONI-α VLM, which outperforms MedGemma on pathology VQA tasks.


<details>
  <summary>Details</summary>
Motivation: Current VLMs for pathology have limitations: they focus on small regions, provide only static slide-level outputs, rely on non-public data limiting reproducibility, and lack training data with detailed clinical reports paired with WSIs.

Method: Three main contributions: 1) POLYSOME - standardized tool for synthetic instruction generation; 2) Applied to HISTAI dataset to create HISTAI-Instruct (24,259 slides, 1.1M instruction-response pairs); 3) Trained ANTONI-α VLM on this dataset for visual-question answering.

Result: ANTONI-α outperforms MedGemma on WSI-level VQA tasks including tissue identification, neoplasm detection, and differential diagnosis. Performance scales with training data amount. All methods, data, and code are publicly available.

Conclusion: POLYSOME enables creation of large-scale instruction datasets for pathology VLMs, leading to ANTONI-α which demonstrates superior performance on clinical pathology tasks and provides a reproducible, transparent framework for developing pathology co-pilots.
...
TL;DR: SynergyWarpNet: Attention-guided cooperative warping framework for high-fidelity talking head synthesis using explicit warping, reference-augmented correction, and confidence-guided fusion.
...
Method: Three-stage progressive refinement: 1) Explicit warping module with 3D dense optical flow for coarse spatial alignment; 2) Reference-augmented correction module using cross-attention across 3D keypoints and texture features from multiple references; 3) Confidence-guided fusion module with learned confidence map for adaptive balancing.
...
Conclusion: SynergyWarpNet effectively addresses limitations of both explicit and attention-based warping methods through cooperative framework, achieving high-fidelity portrait animation.
...
TL;DR: Robust-R1 is a novel framework that explicitly models visual degradations through structured reasoning chains to improve MLLM robustness against real-world visual degradations.


<details>
  <summary>Details</summary>
Motivation: Existing robust MLLMs rely on implicit training/adaptation focusing only on visual encoder generalization, which suffers from limited interpretability and isolated optimization. They struggle with extreme real-world visual degradations that impede practical robustness.

Method: Proposes Robust-R1 framework with three components: (1) supervised fine-tuning for degradation-aware reasoning foundations, (2) reward-driven alignment for accurately perceiving degradation parameters, and (3) dynamic reasoning depth scaling adapted to degradation intensity. Uses a specialized 11K dataset with realistic degradations across four critical visual processing stages.
...
Conclusion: Robust-R1 provides an effective framework for improving MLLM robustness against visual degradations through explicit modeling of degradation parameters and structured reasoning chains, addressing limitations of existing implicit approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing ODISR methods have limited sampling ranges and feature extraction capabilities, which prevents them from effectively capturing the severe geometric distortions (especially near the poles) introduced when projecting spherical omnidirectional images to equirectangular format.

Method: Proposes MDDN with three parallel branches: deformable attention (dilation=1) and two dilated deformable convolutions (dilation rates 2 and 3) to expand sampling range and receptive field. Uses multi-level feature fusion and low-rank decomposition for computational efficiency.

Result: Extensive experiments on public datasets show MDDN outperforms state-of-the-art methods, demonstrating effectiveness and superiority in omnidirectional image super-resolution.

Conclusion: MDDN successfully addresses the challenge of latitude-dependent geometric distortions in ERP images through its multi-level distortion-aware architecture, achieving superior performance in ODISR while maintaining computational efficiency.
...
TL;DR: ClothHMR: A 3D human mesh recovery method that handles diverse clothing by tailoring clothing to fit body silhouettes and leveraging foundational human visual models for better generalization.
...
Method: ClothHMR uses two main modules: (1) Clothing Tailoring (CT) that employs body semantic estimation and edge prediction to tailor clothing to fit body silhouettes, and (2) FHVM-based Mesh Recovery (MR) that optimizes 3D mesh parameters by aligning intermediate representations with those from foundational human visual models.

Result: ClothHMR significantly outperforms state-of-the-art methods across benchmark datasets and in-the-wild images, accurately recovering 3D meshes of humans wearing diverse clothing and precisely estimating body shapes and poses.

Conclusion: ClothHMR effectively addresses the challenge of 3D human mesh recovery under diverse clothing conditions, with practical applications demonstrated through a web application for online fashion and shopping, showing real-world utility.
...
TL;DR: Simple pixel-level mapping pre-processing disrupts image distributions to break semantic shortcuts, forcing AI-generated image detectors to focus on universal generative artifacts for better cross-generator generalization.


<details>
  <summary>Details</summary>
Motivation: Current AI-generated image detectors fail to generalize to unseen generative models because they overfit to source-specific semantic cues rather than learning universal generative artifacts.

Method: Introduces a pixel-level mapping pre-processing step that disrupts pixel value distributions to break fragile, non-essential semantic patterns that detectors exploit as shortcuts, forcing focus on fundamental high-frequency traces of image generation.
...
Conclusion: Disrupting semantic patterns through pixel-level mapping pre-processing effectively forces detectors to learn more generalizable generative artifacts, addressing the critical generalization limitation in AI-generated image detection.
...
TL;DR: A unified Attention U-Net model for automatic FLAIR hyperintensity segmentation across various brain tumor types and time points achieves clinically useful performance and generalizes better than dataset-specific models.


<details>
  <summary>Details</summary>
Motivation: FLAIR MRI scans are crucial for brain tumor diagnosis and monitoring, but manual segmentation of hyperintensity volumes is time-consuming. An automatic segmentation tool would be valuable for clinical use across different tumor types and acquisition time points.

Method: Used ~5000 FLAIR images from various tumor types and acquisition time points to train a unified segmentation model using Attention U-Net architecture. Compared performance against dataset-specific models and validated on different tumor types, time points, and against BraTS dataset.

Result: The unified model achieved Dice scores: 88.65% for pre-op meningiomas, 80.08% for pre-op metastasis, 90.92% for pre-op and 84.60% for post-op gliomas from BraTS, and 84.47% for pre-op and 61.27% for post-op lower grade gliomas. It performed comparably to dataset-specific models while enabling generalization across tumor types and time points.

Conclusion: The unified FLAIR hyperintensity segmentation model provides clinically useful performance across diverse brain tumor types and acquisition time points, facilitating deployment in clinical settings. The model is integrated into Raidionics open-source software for CNS tumor analysis.
...
TL;DR: The paper introduces Affective Image Filter (AIF) task that transforms text with abstract emotions into visually-concrete images that evoke specific emotions, proposing two models (AIF-B and AIF-D) that outperform state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Social media users express emotions through text with images, but current methods don't effectively translate abstract textual emotions into visually-concrete emotional imagery. The paper aims to bridge this gap by creating emotionally compelling results that reflect emotions from text into images.

Method: 1) Introduces AIF dataset and task formulation; 2) Presents AIF-B as initial multi-modal transformer architecture; 3) Proposes AIF-D as extension leveraging generative priors from pre-trained large-scale diffusion models for deeper emotional reflection.

Result: Quantitative and qualitative experiments show AIF models achieve superior performance in content consistency and emotional fidelity compared to state-of-the-art methods. Extensive user studies demonstrate AIF models are significantly more effective at evoking specific emotions.

Conclusion: The paper demonstrates the value and potential of AIF models for creating emotionally compelling visual content from textual emotional expressions, with promising applications in social media and emotional communication.
...
### [58] [MGRegBench: A Novel Benchmark Dataset with Anatomical Landmarks for Mammography Image Registration](https://arxiv.org/abs/2512.17605)
...
Method: Created MGRegBench dataset with over 5,000 mammogram image pairs, including 100 with manual anatomical landmarks and segmentation masks. Benchmarked diverse registration methods including classical (ANTs), learning-based (VoxelMorph, TransMorph), implicit neural representation (IDIR), a classic mammography-specific approach, and state-of-the-art deep learning method MammoRegNet.

Result: MGRegBench is one of the largest public 2D registration datasets with manual annotations. The benchmark enables the first like-for-like comparison of diverse registration methods on mammography, providing extensive analysis of deep learning-based registration approaches.

Conclusion: The paper establishes a foundational public resource for fair comparisons in mammography registration, releasing code and data to catalyze future research and address the limitations of private datasets and inconsistent evaluation frameworks.
...
TL;DR: Researchers found a faster alternative to occlusion-based explanation methods for prostate cancer detection AI, reducing explanation time by at least 10x without quality loss.


<details>
  <summary>Details</summary>
Motivation: Occlusion-based explanation methods for deep neural networks in cancer diagnosis are computationally slow, hindering model development and clinical adoption. Pathologists need easily understandable explanations, but current methods delay iteration and interaction.

Method: Developed a framework to compare explanation methods using suitable criteria and metrics. Evaluated alternative explanation techniques to replace occlusion in a prostate cancer detection system.

Result: Identified a replacement explanation method that reduces computation time by at least 10x while maintaining output quality. This enables faster model development and debugging.

Conclusion: The proposed evaluation approach can be applied to other medical AI applications to find efficient explanation methods, advancing clinical adoption of AI-assisted cancer diagnosis.
...
TL;DR: AIFloodSense: A comprehensive global aerial flood imagery dataset with 470 high-resolution images from 230 flood events across 64 countries, supporting classification, segmentation, and VQA tasks.


<details>
  <summary>Details</summary>
Motivation: Existing flood segmentation datasets are scarce, geographically limited, and lack annotation detail, hindering development of robust computer vision methods for disaster response and risk assessment.

Method: Created AIFloodSense dataset with 470 high-resolution aerial images from 230 distinct flood events across 64 countries (2022-2024). Designed for three tasks: image classification (environment type, camera angle, continent), semantic segmentation (flood, sky, buildings), and visual question answering for disaster assessment.

Result: Established baseline benchmarks using state-of-the-art architectures, demonstrating the dataset's complexity and value for advancing domain-generalized AI tools for climate resilience. Dataset ensures global diversity and temporal relevance.

Conclusion: AIFloodSense bridges the gap in flood detection datasets by providing comprehensive, publicly available global aerial imagery that supports multiple computer vision tasks, enabling development of more robust AI tools for disaster response and climate resilience.
...
TL;DR: Open-source vision-language model specialized for smart-home environments that achieves strong performance on both home-scenario understanding and general multimodal reasoning.


<details>
  <summary>Details</summary>
Motivation: To develop a specialized vision-language model for smart-home applications that balances home-scenario understanding with general multimodal reasoning capabilities, addressing the need for practical AI in home environments.

Method: Two-stage training pipeline combining supervised fine-tuning with reinforcement learning (Group Relative Policy Optimization), using efficient multi-domain data. Incorporates chain-of-thought supervision and token-budget-aware reasoning for efficient learning and inference.

Result: Achieves leading F1 scores on gesture recognition and home-scenario understanding, with consistent gains on video benchmarks (Video-MME, Video-MMMU, Charades-STA) and language understanding benchmarks (MMMU-Pro, MMLU-Pro). Outperforms both closed-source and open-source baselines.

Conclusion: Targeted home-scenario training enhances activity and gesture understanding while improving text-only reasoning with minimal trade-offs on document-centric tasks. The model and evaluation toolkit are publicly available to support smart-home research and deployment.
...
TL;DR: LangDriveCTRL is a natural-language-controllable framework for editing real-world driving videos using 3D scene decomposition and an agentic pipeline to synthesize diverse traffic scenarios.


<details>
  <summary>Details</summary>
Motivation: To enable natural language control for editing driving videos to create diverse traffic scenarios, addressing the need for fine-grained editing and realism in driving video synthesis.

Method: Uses explicit 3D scene decomposition to represent videos as scene graphs, with an agentic pipeline: Orchestrator transforms instructions into execution graphs coordinating specialized agents (Object Grounding, Behavior Editing, Behavior Reviewer) and tools, followed by rendering and video diffusion refinement.

Result: Achieves nearly 2× higher instruction alignment than previous state-of-the-art, with superior structural preservation, photorealism, and traffic realism. Supports both object node editing and multi-object behavior editing from single natural-language instructions.

Conclusion: LangDriveCTRL provides an effective natural-language-controllable framework for realistic driving video editing, enabling diverse traffic scenario synthesis with improved instruction alignment and realism.
...
### [63] [Learning Spatio-Temporal Feature Representations for Video-Based Gaze Estimation](https://arxiv.org/abs/2512.17673)
...
Method: ST-Gaze combines CNN backbone with channel attention and self-attention modules to optimally fuse eye and face features. Fused features are treated as spatial sequences to capture intra-frame context, then propagated through time to model inter-frame dynamics.
...
Conclusion: The approach enables more robust video-based gaze estimation using commonly available cameras by fundamentally improving how spatial and temporal relationships are modeled in gaze estimation.
...
TL;DR: MULTIAQUA is a novel multimodal maritime dataset with synchronized RGB, thermal, IR, LIDAR data to improve scene interpretation in poor visibility conditions, enabling robust training using only daytime images.


<details>
  <summary>Details</summary>
Motivation: Unmanned surface vehicles face challenging visual conditions where color cameras alone are insufficient, especially in poor weather and lighting. Existing maritime datasets lack comprehensive multimodal synchronized data needed for robust scene interpretation.

Method: Created MULTIAQUA dataset with synchronized, calibrated, annotated multimodal sensor data (RGB, thermal, IR, LIDAR). Developed training approaches for multimodal methods that enable robust training using only daytime images, simplifying data acquisition and annotation.

Result: Evaluated multimodal methods on difficult nighttime test set. The approach enables training robust deep neural networks with daytime-only data while maintaining reliable performance even in near-complete darkness.

Conclusion: MULTIAQUA dataset enables development of supervised multimodal methods for maritime scene interpretation in challenging conditions, with training approaches that simplify the process by requiring only daytime data while achieving robust nighttime performance.
...
TL;DR: Empirical study finds conditioning step size (optimal range [2.0, 3.0]) matters more than diffusion step count for diffusion-based super-resolution performance.


<details>
  <summary>Details</summary>
Motivation: Diffusion models show promise for inverse problems like super-resolution, but conditioning methods (DPS, MCG) introduce hyperparameters that need careful tuning, creating a need to understand which factors most affect performance.

Method: Conducted empirical ablation study on FFHQ super-resolution to identify dominant factors affecting performance when applying conditioning to pretrained diffusion models, comparing conditioning step size and diffusion step count.

Result: Conditioning step size has significantly greater impact than diffusion step count, with step sizes in range [2.0, 3.0] yielding best overall performance in experiments.

Conclusion: For diffusion-based super-resolution, careful tuning of conditioning step size is more critical than diffusion step count, with optimal values around 2.0-3.0 for best reconstruction quality.
...
### [66] [3D-RE-GEN: 3D Reconstruction of Indoor Scenes with a Generative Framework](https://arxiv.org/abs/2512.17459)
*Tobias Sautter,Jan-Niklas Dihlmann,Hendrik P. A. Lensch*

Main category: cs.CV

TL;DR: 3D-RE-GEN is a compositional framework that reconstructs single images into textured 3D mesh scenes with proper object decomposition, spatial relationships, and backgrounds for artists' workflows.


<details>
  <summary>Details</summary>
Motivation: Current 3D scene generation methods produce visually appealing output but don't meet artists' needs for modifiable 3D textured mesh scenes in visual effects and game development. Existing textured mesh scene reconstruction suffers from incorrect object decomposition, inaccurate spatial relationships, and missing backgrounds.

Method: A compositional pipeline integrating state-of-the-art models for asset detection, reconstruction, and placement. Uses image editing with generative models for occluded objects with scene-level reasoning. Generates comprehensive backgrounds for spatial constraints and lighting. Employs novel 4-DoF differentiable optimization to align objects with estimated ground plane for physically realistic layouts.

Result: Achieves state-of-the-art performance in single image 3D scene reconstruction. Produces coherent, modifiable scenes through compositional generation guided by precise camera recovery and spatial optimization.

Conclusion: 3D-RE-GEN addresses artists' requirements by combining domain-specific models to create artist-ready textured 3D mesh scenes with proper object decomposition, spatial relationships, and backgrounds for visual effects and game development workflows.
...
TL;DR: TwinSegNet: A privacy-preserving federated learning framework with hybrid ViT-UNet architecture and personalized digital twins for brain tumor segmentation across multiple institutions.
...
Method: Proposes TwinSegNet, a federated learning framework that integrates a hybrid ViT-UNet model (convolutional encoders with Vision Transformer bottlenecks) with personalized digital twins. Each institution fine-tunes the global model on private data to create institution-specific digital twins while maintaining data privacy.

Result: Achieves high Dice scores (up to 0.90%) and sensitivity/specificity exceeding 90% on nine heterogeneous MRI datasets (BraTS 2019-2021 and custom collections). Demonstrates robustness across non-IID client distributions and comparable performance to centralized models like TumorVisNet while preserving privacy.

Conclusion: TwinSegNet enables scalable, personalized brain tumor segmentation for multi-institutional clinical settings while adhering to strict data confidentiality requirements, offering a privacy-preserving alternative to centralized approaches without sacrificing performance.
...
TL;DR: LumiCtrl is a text-to-image personalization method that learns illuminant prompts from single object images to achieve precise lighting control, outperforming existing methods in illuminant fidelity and aesthetic quality.
...
Method: Three-component approach: (1) physics-based illuminant augmentation along Planckian locus for fine-tuning variants, (2) edge-guided prompt disentanglement using frozen ControlNet to focus on illumination, and (3) masked reconstruction loss for foreground object learning with contextual background adaptation.

Result: Significantly better illuminant fidelity, aesthetic quality, and scene coherence compared to existing personalization baselines, with strong user preference confirmed in human studies.

Conclusion: LumiCtrl enables precise illuminant control in T2I generation, addressing a critical gap for content design applications while maintaining high visual quality.
...
### [69] [Pix2NPHM: Learning to Regress NPHM Reconstructions From a Single Image](https://arxiv.org/abs/2512.17773)
*Simon Giebenhain,Tobias Kirschstein,Liam Schoneveld,Davide Davoli,Zhe Chen,Matthias Nießner*

Main category: cs.CV

TL;DR: Pix2NPHM is a vision transformer network that directly regresses Neural Parametric Head Model parameters from single images, achieving high-fidelity face reconstruction with interactive frame rates.


<details>
  <summary>Details</summary>
Motivation: Neural Parametric Head Models (NPHMs) offer better geometric detail than traditional 3DMMs, but fitting them to visual inputs is challenging due to their expressive latent space. The authors aim to create a method that can accurately reconstruct faces from single images while maintaining high geometric fidelity.

Method: The method uses a vision transformer (ViT) network called Pix2NPHM that directly regresses NPHM parameters from single images. They use domain-specific ViTs pretrained on geometric prediction tasks as backbones. Training combines 3D data (over 100K NPHM registrations for direct SDF space supervision) and large-scale 2D video datasets (using normal estimates as pseudo ground truth geometry). The approach also includes inference-time optimization against estimated surface normals and canonical point maps to improve geometric fidelity.

Result: Pix2NPHM achieves unprecedented face reconstruction quality that runs at interactive frame rates. It produces more recognizable facial geometry and accurate facial expressions compared to existing approaches. The method can run at scale on in-the-wild data and allows for further geometric refinement through optimization.

Conclusion: The proposed Pix2NPHM successfully addresses the challenge of fitting expressive NPHMs to visual inputs, enabling high-fidelity 3D face reconstruction from single images with both speed and accuracy, making it suitable for practical applications on diverse real-world data.
...
### [70] [MMLANDMARKS: a Cross-View Instance-Level Benchmark for Geo-Spatial Understanding](https://arxiv.org/abs/2512.17492)
*Oskar Kristoffersen,Alba R. Sánchez,Morten R. Hannemose,Anders B. Dahl,Dim P. Papadopoulos*

Main category: cs.CV

TL;DR: MMLANDMARKS is a multimodal geo-spatial dataset with 4 modalities (aerial images, ground images, text, GPS) for 18,557 US landmarks, enabling unified training for various geo-spatial tasks.


<details>
  <summary>Details</summary>
Motivation: Current geo-spatial benchmarks have limited modality coverage, restricting progress as approaches cannot integrate all relevant modalities within a unified framework for comprehensive geo-spatial understanding.

Method: Introduces MMLANDMARKS dataset with 197k aerial images, 329k ground-view images, textual information, and geographic coordinates for 18,557 distinct US landmarks, featuring one-to-one correspondence across all modalities.

Result: Demonstrates broad generalization and competitive performance against off-the-shelf foundational models and specialized state-of-the-art models across tasks using a simple CLIP-inspired baseline.

Conclusion: The dataset enables training and benchmarking for various geo-spatial tasks and illustrates the necessity for multimodal datasets to achieve comprehensive geo-spatial understanding.
...
### [71] [GroundingME: Exposing the Visual Grounding Gap in MLLMs through Multi-Dimensional Evaluation](https://arxiv.org/abs/2512.17495)
...
TL;DR: GroundingME benchmark reveals MLLMs' severe limitations in visual grounding, with best model scoring only 45.1% and most failing completely on rejection tasks, highlighting critical safety concerns.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks fail to capture real-world complexity of visual grounding where humans handle ambiguous references and recognize when grounding is impossible. Need to assess if MLLMs truly ground language in vision or just pattern-match on simplified datasets.

Method: Introduce GroundingME benchmark with 1,005 challenging examples across four dimensions: Discriminative (similar objects), Spatial (complex relations), Limited (occlusions/tiny objects), and Rejection (ungroundable queries). Created through automated generation with human verification. Evaluate 25 state-of-the-art MLLMs.

Result: Best model achieves only 45.1% accuracy. Most models score 0% on rejection tasks, hallucinating objects instead of acknowledging absence. Test-time scaling improves complex grounding by up to 2.9%. Data-mixture training boosts rejection accuracy from 0% to 27.9%.

Conclusion: GroundingME reveals profound capability gap in MLLMs for visual grounding, raising critical safety concerns. Serves as diagnostic tool and roadmap toward human-level visual grounding, showing current models lack sophistication to handle real-world complexity.
...
TL;DR: AniX: A world model that combines static 3D scene generation with controllable character animation, allowing users to direct characters through natural language to perform diverse actions in 3D Gaussian Splatting scenes.
...
Method: AniX takes a 3DGS scene and character as input, allowing natural language direction for diverse behaviors. It's formulated as conditional autoregressive video generation, built upon a pre-trained video generator with enhanced training strategy to improve motion dynamics while maintaining generalization across actions and characters.

Result: AniX synthesizes temporally coherent video clips preserving visual fidelity with provided scenes and characters. The model demonstrates enhanced motion dynamics while maintaining generalization across various actions and characters.
...
### [73] [Validation of Diagnostic Artificial Intelligence Models for Prostate Pathology in a Middle Eastern Cohort](https://arxiv.org/abs/2512.17499)
...
TL;DR: First external validation of AI prostate cancer diagnosis in Middle East shows pathologist-level performance across different scanners, including low-cost options.


<details>
  <summary>Details</summary>
Motivation: Current AI pathology systems are mainly validated on European/US cohorts, creating a gap for global adoption. This study addresses the need for validation in underrepresented populations where AI could have greatest impact, focusing on the Middle East.

Method: Collected 339 prostate biopsy specimens from 185 patients in Kurdistan, Iraq (2013-2024). Evaluated task-specific AI model and two foundation models for concordance with pathologists and consistency across three scanner models (Hamamatsu, Leica, Grundium).

Result: AI-pathologist concordance (kappa 0.801) was similar to pathologist-pathologist concordance (kappa 0.799). High cross-scanner concordance (kappa >0.90) for all AI models and scanner pairs, including low-cost compact scanner.

Conclusion: AI achieves pathologist-level performance in prostate histopathology. Compact scanners enable validation studies in non-digitalized settings and cost-effective AI adoption. First openly available Middle East digital pathology dataset supports globally equitable AI pathology.
...
TL;DR: InfSplign is a training-free inference-time method that improves spatial alignment in text-to-image diffusion models by adjusting noise through a compound loss using cross-attention maps during denoising.


<details>
  <summary>Details</summary>
Motivation: Current T2I diffusion models often fail to capture spatial relations specified in text prompts due to lack of fine-grained spatial supervision in training data and inability of text embeddings to encode spatial semantics.
...
Conclusion: InfSplign is a lightweight, plug-and-play, training-free method compatible with any diffusion backbone that effectively improves spatial alignment in T2I generation through inference-time adjustments.
...
TL;DR: The paper introduces CBAM-VGG16, an interpretable attention-guided CNN for plant leaf disease detection that achieves up to 98.87% accuracy and provides transparent disease localization through multiple visualization techniques.


<details>
  <summary>Details</summary>
Motivation: Plant diseases threaten global food security, requiring accurate and interpretable detection methods. Current approaches need better feature extraction, disease localization, and transparency for agricultural diagnostics.

Method: Proposes CBAM-VGG16, a CNN that integrates Convolution Block Attention Module (CBAM) at each convolutional stage to enhance feature extraction and disease localization. Trained on five diverse plant disease datasets with comprehensive evaluation using attention maps, Grad-CAM, Grad-CAM++, and Layer-wise Relevance Propagation (LRP).

Result: Outperforms recent techniques with high accuracy up to 98.87%, demonstrates robust generalization across datasets, and provides interpretable visualizations showing disease localization through attention mechanisms.

Conclusion: Advances explainable AI in agricultural diagnostics by offering a transparent and reliable system for smart farming, with publicly available code for reproducibility and further research.
...
### [76] [Foundation Model Priors Enhance Object Focus in Feature Space for Source-Free Object Detection](https://arxiv.org/abs/2512.17514)
*Sairam VCR,Rishabh Lalla,Aveen Dayal,Tejal Kulkarni,Anuj Lalla,Vineeth N Balasubramanian,Muhammad Haris Khan*

Main category: cs.CV

TL;DR: FALCON-SFOD improves source-free object detection by strengthening object-focused feature representations using foundation model guidance and noise-robust pseudo-labeling, addressing domain shift challenges.


<details>
  <summary>Details</summary>
Motivation: Current SFOD methods using Mean-Teacher self-labeling suffer from weak object focus due to domain shift, causing unreliable pseudo-labels from background clutter. Prior works focus on refining pseudo-labels but neglect strengthening the underlying feature space itself.
...
Conclusion: Strengthening object-focused feature representations through foundation model guidance and noise-robust pseudo-labeling effectively addresses domain shift challenges in source-free object detection, outperforming methods that only refine pseudo-labels.
...
### [77] [PathBench-MIL: A Comprehensive AutoML and Benchmarking Framework for Multiple Instance Learning in Histopathology](https://arxiv.org/abs/2512.17517)
*Siemen Brussee,Pieter A. Valkema,Jurre A. J. Weijer,Thom Doeleman,Anne M. R. Schrader,Jesper Kers*

Main category: cs.CV

TL;DR: PathBench-MIL is an open-source AutoML and benchmarking framework for multiple instance learning in histopathology that automates end-to-end MIL pipeline construction and provides reproducible benchmarking of MIL models and feature extractors.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the need for standardized, reproducible benchmarking and automated pipeline construction in multiple instance learning for histopathology, which currently lacks comprehensive tools for rapid experimentation and comparison across different MIL models and feature extractors.

Method: The framework automates end-to-end MIL pipeline construction including preprocessing, feature extraction, and MIL-aggregation. It provides a unified configuration system, visualization tooling, and modular extensibility to enable standardized experimentation across datasets and tasks.

Result: PathBench-MIL is an open-source system that enables reproducible benchmarking of dozens of MIL models and feature extractors, with integrated visualization tools and a unified configuration system for histopathology applications.

Conclusion: PathBench-MIL provides a comprehensive AutoML and benchmarking solution for MIL in histopathology that standardizes experimentation, enables rapid pipeline development, and facilitates reproducible research through its open-source availability and modular design.
...
TL;DR: RadarGen is a diffusion model that generates realistic automotive radar point clouds from multi-view camera images, using BEV representations and visual scene guidance to create physically plausible radar data.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the need for scalable multimodal generative simulation by creating realistic radar data from visual inputs, which could help bridge the gap between simulation and real-world perception model training.

Method: RadarGen adapts image-latent diffusion to radar by representing measurements in bird's-eye-view (BEV) form with RCS and Doppler attributes. It incorporates BEV-aligned depth, semantic, and motion cues from pretrained foundation models to guide generation, then reconstructs point clouds from generated maps.

Result: Evaluations on large-scale driving data show that RadarGen captures characteristic radar measurement distributions and reduces the gap to perception models trained on real data, demonstrating improved realism in generated radar point clouds.

Conclusion: RadarGen represents a step toward unified generative simulation across sensing modalities, offering a scalable approach compatible with existing visual datasets and simulation frameworks for multimodal autonomous driving applications.
...
TL;DR: This paper investigates adversarial robustness of vision-language models LLaVA-1.5-13B and Meta's Llama 3.2 Vision-8B-2 against untargeted PGD attacks on visual inputs, finding Llama 3.2 Vision shows smaller performance degradation despite lower baseline accuracy.


<details>
  <summary>Details</summary>
Motivation: As deep learning models become more complex and opaque, adversaries can exploit vulnerabilities by modifying images to confuse AI recognition systems. The paper aims to understand the adversarial robustness of contemporary vision-language models against visual input attacks.

Method: The study tests LLaVA-1.5-13B and Meta's Llama 3.2 Vision-8B-2 using untargeted Projected Gradient Descent (PGD) attacks against the visual input modality. Empirical evaluation is conducted on a Visual Question Answering (VQA) v2 dataset subset, with results quantified using standard VQA accuracy metrics and accuracy degradation comparisons.

Result: Llama 3.2 Vision exhibited a smaller performance drop under adversarial attack compared to LLaVA, particularly at higher perturbation levels, despite having lower baseline accuracy in the experimental setup.

Conclusion: The vision modality represents a viable attack vector for degrading performance of contemporary open-weight VLMs. Adversarial robustness doesn't necessarily correlate with standard benchmark performance and may be influenced by underlying architectural and training factors.
...
<details>
  <summary>Details</summary>
Motivation: Previous methods for language-embedded 3D Gaussian reconstruction suffer from fixed input views and insufficient 3D training data, requiring 3D annotations that are difficult to obtain.

Method: Proposes a 3D-annotation-free training framework using 2D-to-3D lifting from arbitrary uncalibrated multi-view images, instance-guided contrastive learning to align 2D semantics with 3D representations, and geometry-semantic hierarchical sparsification to reduce computational costs.
...
Conclusion: The framework enables large-scale training using easily obtained 2D instance information from videos, overcoming limitations of previous methods while maintaining efficiency and quality in 3D reconstruction with semantic embedding.
...
<details>
  <summary>Details</summary>
Motivation: Current foundation models like Depth Anything V2 struggle with real-world images that differ from their training distribution, creating a domain gap problem in monocular depth estimation.

Method: A test-time self-supervision framework that fuses DA-V2 with 2D diffusion models. It performs label-free refinement by re-lighting predicted depth maps and augmenting input images, using shape from shading cues with Score Distillation Sampling. Uses targeted optimization: freezes encoder, updates intermediate embeddings, and fine-tunes decoder only.

Result: Substantial gains in depth accuracy and realism over DA-V2 across diverse benchmarks, demonstrating improved performance on real-world images far from training distribution.

Conclusion: Re-Depth Anything shows new avenues for self-supervision by augmenting geometric reasoning, effectively bridging domain gaps in monocular depth estimation through fusion of foundation models with diffusion priors.
...
<details>
  <summary>Details</summary>
Motivation: View-synthesis loss alone is insufficient for recovering geometrically meaningful 3D Gaussian splats, as prior work relies almost exclusively on this supervision while predicting additional Gaussian parameters.

Method: G3Splat enforces geometric priors to obtain geometrically consistent 3D scene representations, addressing ambiguities in learning 3D Gaussian splats under self-supervision for pose-free generalizable splatting.
...
Conclusion: Geometric priors are essential for learning meaningful 3D Gaussian splats from images, enabling better geometry recovery and pose estimation while maintaining strong novel-view synthesis capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Virtual furniture synthesis has practical applications in home design and e-commerce, but remains underexplored due to lack of reproducible benchmarks and limitations of existing image composition methods in achieving high-fidelity synthesis while preserving background integrity.

Method: Proposes RoomEditor++ with parameter-sharing dual diffusion backbone compatible with both U-Net and DiT architectures, unifying feature extraction and inpainting for reference and background images. Also introduces RoomBench++ dataset with 112,851 training pairs and 1,832 testing pairs from real-world indoor videos and home design renderings.
...
Conclusion: RoomEditor++ provides an effective solution for virtual furniture synthesis with superior performance and generalization capabilities, supported by a comprehensive benchmark dataset that facilitates future research in this domain.
...
TL;DR: Proposes a diffusion-based reconstruction algorithm for video SCI with one-hot masks, combining regression initialization with diffusion refinement and dual optical paths to address temporal aliasing and spatial degradation.


<details>
  <summary>Details</summary>
Motivation: Current video SCI using random binary modulation causes temporal aliasing. One-hot modulation offers perfect temporal decoupling but lacks algorithms to exploit its potential. Need to develop specialized reconstruction methods for one-hot masks.

Method: 1) Transform reconstruction into generative video inpainting using SDE aligned with hardware compression. 2) Combine one-step regression initialization with one-step diffusion refinement. 3) Implement dual optical paths at hardware level for complementary information to enhance inpainted video.

Result: First work integrating diffusion into video SCI reconstruction. Experiments on synthetic datasets and real scenes demonstrate effectiveness of the proposed method.

Conclusion: Proposed framework successfully addresses limitations of one-hot modulation in video SCI, combining hardware innovation with novel algorithmic approach to achieve improved reconstruction quality.
...
TL;DR: Current medical imaging AI benchmarks suffer from dataset biases and accessibility issues, limiting their clinical relevance despite widespread influence.


<details>
  <summary>Details</summary>
Motivation: To assess whether medical imaging AI benchmarks provide sufficiently representative, accessible, and reusable data to support clinically meaningful AI, examining both dataset representativeness and FAIR compliance.

Method: Conducted large-scale systematic study of 241 biomedical image analysis challenges comprising 458 tasks across 19 imaging modalities, evaluating dataset composition biases and accessibility/legal reusability according to FAIR principles.

Result: Found substantial biases in dataset composition (geographic, modality-, and problem type-related), showing benchmarks don't reflect real-world clinical diversity. Challenge datasets frequently had restrictive/ambiguous access conditions, inconsistent licensing, and incomplete documentation, limiting reproducibility and reuse.

Conclusion: Current benchmarking ecosystem has foundational fairness limitations with a disconnect between leaderboard success and clinical relevance, highlighting need for more representative and accessible benchmark datasets.
...
TL;DR: HeadHunt-VAD is a tuning-free video anomaly detection method that directly identifies and uses robust attention heads within frozen multimodal LLMs instead of relying on textual outputs, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Traditional VAD methods require extensive labeled data and are computationally expensive. Recent MLLM-based tuning-free methods rely on textual outputs, which suffer from information loss, normalcy bias, and prompt sensitivity, making them insufficient for capturing subtle anomalous cues.
...
Result: Achieves state-of-the-art performance among tuning-free methods on two major VAD benchmarks while maintaining high efficiency.

Conclusion: Head-level probing in MLLMs is a powerful and practical solution for real-world anomaly detection, bypassing textual generation limitations and enabling efficient, accurate detection with interpretable outputs.
...
TL;DR: A semi-supervised learning method for multi-output CNN segmentation models that uses data augmentation (rotations/flipping) without probabilistic assumptions, applied to aortic dissection segmentation.


<details>
  <summary>Details</summary>
Motivation: Multi-output CNNs improve medical image segmentation but require extensive labeled data, which is expensive/time-consuming for 3D medical images. Semi-supervised learning can help but isn't well understood for multi-output models.

Method: Proposes a semi-supervised learning approach based on data augmentation (rotations and flipping) that doesn't rely on probabilistic model assumptions, making it universal for multi-output segmentation architectures.

Result: Method applied to Type B aortic dissection segmentation using ImageTBDA dataset (100 3D CTA images) with three key classes: true lumen, false lumen, and false lumen thrombus.

Conclusion: The approach addresses the data labeling bottleneck for multi-output segmentation models in medical imaging, particularly for complex pathologies like aortic dissection.
...
<details>
  <summary>Details</summary>
Motivation: High-resolution quantitative MRI relaxometry provides objective tissue characterization but is clinically underutilized due to lengthy acquisition times. Current methods require HR qMRI ground truth for training, which is impractical in clinical settings.

Method: Proposes Bayesian maximum a posteriori inference framework with two discrepancy terms: (1) between synthesized HR images from super-resolved qMRI maps and acquired wMRI guides via forward signal models, and (2) between acquired LR qMRI and downsampled predictions. Uses deep neural network to learn super-resolution mapping from synthetic training data generated by degrading HR qMRI resolution via k-space truncation.

Result: Ablation experiments show T1-weighted images enhance T1 maps, T2-weighted images improve T2 maps, and combined guidance optimally enhances all parameters. Validation on independently acquired in-vivo data confirms cross-qMRI sequence generalizability. Models produce super-resolved maps from 1-minute acquisition comparable to 5-minute reference scans.

Conclusion: Framework enables fast qMRI acquisitions enhanced via routine clinical images by decoupling training from HR qMRI requirement, offering practical pathway for integrating quantitative relaxometry into clinical workflows with acceptable additional scan time.
...
TL;DR: StereoMV2D enhances multi-view 3D object detection by integrating temporal stereo modeling with 2D detection guidance, improving depth perception and query initialization while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing 2D detection-guided 3D detectors suffer from depth ambiguity in single-frame 2D detections, limiting 3D query generation accuracy. The paper aims to address this limitation by leveraging temporal information across adjacent frames.

Method: Proposes StereoMV2D framework that: 1) Uses cross-temporal disparities of objects across adjacent frames to enhance depth perception, 2) Refines query priors efficiently within 2D RoIs, 3) Implements dynamic confidence gating to evaluate reliability of temporal stereo cues using inter-frame matching matrix and appearance consistency.

Result: Extensive experiments on nuScenes and Argoverse 2 datasets show superior detection performance without significant computational overhead. Achieves better precision and recall compared to baseline methods.

Conclusion: StereoMV2D effectively addresses depth ambiguity in 2D-guided 3D detection by integrating temporal stereo modeling, providing robust detection under appearance changes and occlusion while maintaining computational efficiency.
...
TL;DR: PathFLIP is a novel vision-language framework for pathology that decomposes slide captions into region-level subcaptions for fine-grained WSI interpretation, outperforming existing methods with less training data.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models struggle with gigapixel-scale Whole Slide Images due to spatial heterogeneity and difficulty capturing fine-grained correspondences between text and thousands of image patches, limiting performance on downstream clinical tasks.

Method: PathFLIP decomposes slide-level captions into region-level subcaptions, generates text-conditioned region embeddings for precise visual-language grounding, and leverages LLMs to follow diverse clinical instructions and adapt to diagnostic contexts.

Result: PathFLIP outperforms existing large-scale pathological VLMs on four representative benchmarks while requiring significantly less training data, demonstrating versatile capabilities across slide-level classification/retrieval, lesion localization, and instruction following.

Conclusion: PathFLIP paves the way for fine-grained, instruction-aware WSI interpretation in clinical practice by enabling holistic understanding of gigapixel pathology images through region-level caption decomposition and LLM integration.
...
TL;DR: GRASP-HO is a novel framework that reformulates human-object interaction detection from closed-set classification to open-vocabulary generation using frozen MLLMs with lightweight cognitive steering.


<details>
  <summary>Details</summary>
Motivation: Existing HOI detection methods operate under closed-world assumptions with predefined verb sets, struggling with unseen or ambiguous interactions. While MLLMs have rich world knowledge for open-vocabulary understanding, they're decoupled from HOI detectors due to prohibitive fine-tuning costs.

Method: Proposes GRASP-HO framework that extracts hybrid interaction representations and uses a lightweight learnable cognitive steering conduit (CSC) module to inject fine-grained visual evidence into a frozen MLLM. Introduces hybrid guidance strategy coupling language modeling loss with auxiliary classification loss to address supervision mismatch.
...
Conclusion: GRASP-HO successfully reformulates HOI detection as an open-vocabulary generation problem, leveraging frozen MLLMs with lightweight adaptation to achieve both strong closed-set performance and zero-shot generalization capabilities.
...
TL;DR: ReCo introduces a new instructional video editing paradigm that uses constraint modeling between editing and non-editing regions during in-context generation, addressing inaccurate editing regions and token interference problems.
...
Method: ReCo width-wise concatenates source and target videos for joint denoising, with two regularization terms: latent regularization (increases discrepancy in editing regions, reduces in non-editing areas) and attention regularization (suppresses attention from editing tokens to source video counterparts). Also creates ReCo-Data dataset with 500K instruction-video pairs.
...
Conclusion: ReCo presents an effective paradigm for instructional video editing that addresses key challenges through constraint modeling and regularization techniques, supported by a large-scale dataset.
...
TL;DR: Visual prompting benchmarks for VLMs are surprisingly fragile to minor details like marker color/size, which can completely change model rankings and be exploited to make weaker models outperform stronger ones.


<details>
  <summary>Details</summary>
Motivation: Current VLM evaluation benchmarks using visual prompting (coordinates marked in images) are unstable - minor changes in visual markers or technical details significantly affect model performance and rankings, undermining reliable evaluation.

Method: Evaluated 9 open- and closed-source VLMs on two visually prompted tasks, systematically testing effects of visual marker design, dataset size, and low-level inference choices like JPEG compression. Created VPBench with 16 visual marker variants for more stable evaluation.

Result: Visual marker details (color, size) have dramatic impact: changing red to blue alters rankings; increasing marker size makes InternVL3-8B rank alongside Gemini 2.5 Pro. JPEG compression levels also affect rankings. These effects are much larger than in conventional semantic VLM evaluations.

Conclusion: Current visually prompted benchmarks are unstable due to sensitivity to implementation details. VPBench provides a more robust evaluation framework with multiple marker variants to mitigate these issues and enable more reliable VLM assessment.
...
TL;DR: Bitbox is an open-source toolkit that makes AI-based computational behavioral measurement accessible to behavioral scientists without requiring engineering expertise, bridging the gap between computer science and behavioral research.


<details>
  <summary>Details</summary>
Motivation: Current AI-based behavioral measurement tools are designed for engineers, require specialized software stacks, and don't provide measurements useful for hypothesis-driven research, creating barriers for behavioral scientists wanting to use modern computational methods.

Method: Bitbox provides a standardized interface for extracting high-level behavioral measurements from video using multiple face, head, and body processors. It's built on principles of reproducibility, modularity, and interpretability, allowing new measures to be added easily.

Result: Bitbox has been tested and validated on clinical samples and serves as a practical bridge between computer scientists (who can disseminate methods) and behavioral researchers (who get accessible behavioral metrics without engineering expertise).

Conclusion: Bitbox will accelerate integration of computational behavioral measurement into behavioral, clinical, and mental health research as a community-driven tool that evolves through contributions from both method developers and domain scientists.
...
<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of creating high-quality 3D head avatars with realistic dynamic deformations from limited input (single or sparse images), while eliminating the need for camera poses and expression labels that are often difficult to obtain.

Method: Uses a transformer-based reconstruction model with structured head query tokens as canonical anchors to aggregate flexible inputs. Introduces a lightweight UNet decoder conditioned on UV-space position maps for real-time detailed expression-dependent deformations. Employs data distribution adjustment to balance rare but critical expressions, and includes a 10-second refinement process for identity-specific details.
...
Conclusion: The proposed FlexAvatar model offers an effective and flexible approach for high-fidelity 3D head avatar reconstruction from limited inputs, with detailed dynamic deformations and real-time performance, making it suitable for practical avatar creation applications.
...
TL;DR: SAVeD is a large-scale video dataset of real-world ADAS vehicle crashes, near-misses, and disengagements collected from social media, featuring frame-level annotations for safety-critical analysis.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack authentic ADAS vehicle behavior under high-risk conditions like near-misses and system failures, limiting safety-critical research for ADAS-equipped vehicles.

Method: Curated 2,119 first-person videos from social media showing ADAS vehicle operations in diverse conditions, with video frame-level annotations for collisions, evasive maneuvers, and disengagements.

Result: Three key contributions: (1) Framework for real-time TTC computation using semantic segmentation and depth estimation; (2) GEV distribution modeling for extreme risk quantification; (3) Enhanced VLLM benchmarks showing improved performance with SAVeD's annotations.

Conclusion: SAVeD addresses the critical gap in ADAS safety research by providing authentic, annotated real-world data for analyzing perception and decision-making failures in high-risk scenarios.
...
TL;DR: MambaMIL+ is a new multiple instance learning framework for whole-slide image analysis that integrates spatial context modeling with Mamba's efficient long-sequence processing, addressing memory decay and limited spatial context issues in existing methods.


<details>
  <summary>Details</summary>
Motivation: Whole-slide images present challenges due to their gigapixel resolution and lack of fine-grained annotations. While MIL treats WSIs as bags of patches, effectively modeling ultra-long sequences with rich spatial context remains difficult. Mamba offers efficient long sequence learning but suffers from limited spatial context modeling and memory decay in WSI analysis.
...
TL;DR: This paper introduces Diff-Gen, a large-scale diffusion-generated fake image dataset, and AdaptPrompt, a parameter-efficient CLIP-based framework for generalizable deepfake detection that achieves state-of-the-art performance across diverse generators.


<details>
  <summary>Details</summary>
Motivation: The increasing difficulty of reliable deepfake detection due to highly realistic synthetic media, with a key challenge being generalization - detectors trained on narrow generator classes often fail on unseen models.

Method: Two main contributions: 1) Diff-Gen dataset with 100k diffusion-generated fakes capturing broad spectral artifacts, and 2) AdaptPrompt framework that jointly learns task-specific textual prompts and visual adapters while keeping CLIP backbone frozen, with pruning of final transformer block to retain high-frequency artifacts.

Result: Achieves new state-of-the-art across 25 challenging test sets covering GANs, diffusion models, and commercial tools, with strong cross-domain generalization, few-shot learning capability (using as few as 320 images), and source attribution for generator identification.

Conclusion: The approach addresses the pressing need for generalizable deepfake detection by leveraging vision-language models and specialized datasets, establishing robust performance across diverse synthetic content while maintaining parameter efficiency.
...
TL;DR: LiteGE is a lightweight method for computing geodesic distances on 3D surfaces using compact PCA-based descriptors from UDF samples, achieving up to 300× memory/time reduction and 1000× speedup for shape matching.
...
Method: Constructs compact, category-aware shape descriptors by applying PCA to unsigned distance field (UDF) samples at informative voxels, eliminating the need for high-capacity networks while maintaining robustness on sparse point clouds (down to 300 points).

Result: Reduces memory usage and inference time by up to 300× compared to existing neural approaches, achieves up to 1000× speedup over state-of-the-art mesh-based approaches for shape matching while maintaining comparable accuracy on non-isometric shape pairs.

Conclusion: LiteGE provides an efficient, lightweight solution for geodesic distance computation that enables practical use in interactive and resource-constrained settings while maintaining robustness on sparse inputs and enabling fast shape matching.
...
TL;DR: UrbanDIFF: A denoising diffusion model for reconstructing cloud-contaminated urban land surface temperature imagery using purely spatial information and urban structure conditioning.
...
Method: UrbanDIFF uses denoising diffusion-based image inpainting conditioned on static urban structure information (built-up surface data and digital elevation model). It enforces consistency with revealed cloud-free pixels through supervised pixel guided refinement during inference.

Result: UrbanDIFF consistently outperforms interpolation baselines, especially under dense cloud occlusion (85% coverage). Achieves SSIM 0.89, RMSE 1.2 K, and R² 0.84 at 85% cloud coverage, with slower performance degradation as cloud density increases.

Conclusion: Denoising diffusion models offer robust spatial reconstruction for cloud-contaminated urban LST imagery, enabling more reliable SUHI monitoring even under high missingness conditions.
...
<details>
  <summary>Details</summary>
Motivation: Existing stereo-camera 3D localization methods are limited to few hundred meters due to inadequate distortion models for camera lens nonlinearities, especially for long-distance photogrammetry applications like 3D mapping and object localization.

Method: Hybrid approach: 1) Extend conventional distortion models with higher-order terms, 2) Enhance with neural network-based residual correction model to handle complex nonlinearities that pure neural networks fail to learn for camera parameter estimation.

Result: Framework enables 3D localization up to 5km distances, substantially improving long-range performance. Estimated 3D coordinates are transformed to GIS coordinates and visualized on maps. Experimental validation shows robustness and effectiveness for CCTV camera calibration.

Conclusion: The hybrid distortion modeling approach provides a practical solution for calibrating CCTV cameras for long-range photogrammetry applications, overcoming limitations of both conventional models and pure neural network approaches.
...
TL;DR: Chorus is a multi-teacher pretraining framework that learns a holistic 3D Gaussian Splatting scene encoder by distilling complementary signals from 2D foundation models for various 3D vision tasks.
...
Result: The method shows strong performance on open-vocabulary semantic/instance segmentation, linear/decoder probing, and data-efficient supervision. A point cloud variant using only Gaussian centers, colors, and normals outperforms point cloud baselines while using 39.9× fewer training scenes. The render-and-distill adaptation facilitates out-of-domain finetuning.

Conclusion: Chorus successfully learns a holistic 3DGS scene encoder through multi-teacher distillation, demonstrating strong transfer capabilities across various 3D vision tasks and efficient pretraining with limited data.
...
<details>
  <summary>Details</summary>
Motivation: Current autonomous coding agents built on LLMs struggle with complex, domain-specific scientific problems like medical imaging, which require specialized knowledge, high-dimensional data handling, and end-to-end workflow management not captured by existing benchmarks.

Method: Created ReX-MLE benchmark with 20 challenges derived from high-impact medical imaging competitions across diverse modalities and task types. The benchmark evaluates full end-to-end workflows requiring agents to independently manage data preprocessing, model training, and submission under realistic compute/time constraints.

Result: Evaluation of state-of-the-art agents (AIDE, ML-Master, R&D-Agent) with different LLM backends (GPT-5, Gemini, Claude) showed severe performance gaps - most submissions ranked in the 0th percentile compared to human experts. Failures stemmed from domain-knowledge and engineering limitations.

Conclusion: ReX-MLE exposes critical bottlenecks in current autonomous AI systems for medical imaging and provides a foundation for developing domain-aware autonomous AI systems capable of handling complex scientific workflows.
...
TL;DR: A simulation-driven deep learning framework for denoising Raman spectra by modeling noise sources and training neural networks to suppress detector noise and fluorescence background.
...
Method: Developed a simulation-driven denoising framework combining statistically grounded noise modeling with deep learning. Generated biologically realistic Raman spectra using comprehensive noise models and trained a cascaded deep neural network to jointly suppress stochastic detector noise and fluorescence baseline interference.

Result: Evaluated on simulated human skin spectra derived from real experimental data, demonstrating improved spectral quality through physics-informed learning.

Conclusion: The approach shows potential for enabling faster and more accurate Raman-based tissue analysis by enhancing spectral quality through simulation-driven deep learning denoising.
...
TL;DR: A method called Keypoint Counting Classifiers (KCCs) transforms any pre-trained Vision Transformer (ViT) model into a self-explainable model without retraining, using ViT's ability to identify matching keypoints between images to create interpretable visual explanations.


<details>
  <summary>Details</summary>
Motivation: Current self-explainable models require complex training and specific architectures, making them impractical. With the rise of ViT-based foundation models, there's a need for new methods to provide transparency and reliability to these powerful but opaque models.

Method: KCCs leverage ViT's ability to automatically identify matching keypoints between images. The method transforms any well-trained ViT model into a self-explainable classifier without retraining, creating an interpretable decision process that can be visualized directly in the input image.

Result: Extensive evaluation shows that KCCs improve human-machine communication compared to recent baselines. The method successfully provides transparent explanations by visualizing keypoint matches in the input.

Conclusion: KCCs represent an important step toward making ViT-based foundation models more transparent and reliable by providing inherent visual explanations without requiring model retraining or complex architectures.
...
<details>
  <summary>Details</summary>
Motivation: Modeling multi-person interactions is challenging due to long temporal horizons, strong inter-agent dependencies, and variable group sizes. Existing methods are largely task-specific and don't generalize to flexible multi-agent generation.

Method: MAGNet (Multi-Agent Diffusion Forcing Transformer) uses an autoregressive diffusion framework with key modifications to explicitly model inter-agent coupling during autoregressive denoising, enabling coherent coordination across agents. It supports dyadic prediction, partner inpainting, and full multi-agent generation within a single model.

Result: The approach performs on par with specialized methods on dyadic benchmarks while naturally extending to polyadic scenarios involving three or more interacting people. It captures both tightly synchronized activities (dancing, boxing) and loosely structured social interactions.

Conclusion: MAGNet provides a unified framework for multi-agent motion generation that is agnostic to the number of agents and can generate coherent, long sequences across various interaction tasks, addressing limitations of existing task-specific methods.
...
TL;DR: DWM is a video diffusion framework that generates realistic human-scene interaction videos from static 3D scenes and hand motions, enabling interactive digital twins.


<details>
  <summary>Details</summary>
Motivation: Current digital twins are static and lack embodied interactivity, limiting them to navigation and view synthesis. There's a need to bridge this gap by modeling how human actions dynamically change 3D scenes.

Method: Scene-action-conditioned video diffusion framework that takes static 3D scene renderings with camera trajectory and egocentric hand mesh renderings as inputs. Trained on a hybrid dataset combining synthetic egocentric interactions (for aligned supervision) and real-world fixed-camera videos (for realistic dynamics).

Result: DWM generates temporally coherent videos showing plausible human-scene interactions like grasping, opening, and moving objects while maintaining camera and scene consistency. Enables realistic and physically plausible interactions.

Conclusion: This represents a first step toward video diffusion-based interactive digital twins and enables embodied simulation from egocentric actions, moving beyond static digital twins to interactive ones.
...
TL;DR: The paper proposes a framework to adapt discriminative encoder features for generative tasks by introducing semantic-pixel reconstruction to create compact, semantically-rich latents that enable state-of-the-art image reconstruction and generation.


<details>
  <summary>Details</summary>
Motivation: Current LDMs use VAE latents optimized for pixel reconstruction but lack semantic richness. Using discriminative encoder features for generation faces two problems: (1) lack of compact regularization causing off-manifold latents and inaccurate structures, and (2) weak pixel reconstruction hindering fine-grained geometry/texture learning.

Method: Proposes a systematic framework with semantic-pixel reconstruction objective to regularize latent space, compressing both semantic information and fine details into compact representation (96 channels, 16x16 downsampling). Uses this representation to build unified T2I and image editing model.

Result: Achieves state-of-the-art image reconstruction, faster convergence, and substantial performance gains in both T2I and editing tasks compared to various feature spaces. Validates that representation encoders can be effectively adapted into robust generative components.

Conclusion: The proposed framework successfully bridges vision generation and understanding by adapting discriminative encoder features for generative tasks through semantic-pixel regularization, creating compact yet semantically-rich representations that enable superior reconstruction and generation performance.
...
Conclusion: The approach supports practical applications of taxonomic entity set expansion by allowing efficient navigation of expansion graphs through local reasoning tasks rather than full graph materialization.
...
TL;DR: The paper introduces a framework for Scientific General Intelligence (SGI) with a benchmark of 1,000+ cross-disciplinary tasks, revealing gaps in current LLMs' scientific capabilities and proposing Test-Time Reinforcement Learning to enhance hypothesis novelty.
...
Method: 1) Define SGI using Practical Inquiry Model (PIM: Deliberation, Conception, Action, Perception). 2) Create SGI-Bench with 1,000+ expert-curated cross-disciplinary samples from Science's 125 Big Questions. 3) Evaluate state-of-the-art LLMs on four scientist-aligned tasks. 4) Introduce Test-Time Reinforcement Learning (TTRL) to optimize retrieval-augmented novelty rewards at inference.

Result: Results show significant gaps: low exact match (10-20%) in deep research despite step-level alignment; ideas lacking feasibility/detail; high code executability but low execution accuracy in dry experiments; low sequence fidelity in wet protocols; persistent multimodal reasoning challenges. TTRL enhances hypothesis novelty without reference answers.

Conclusion: The PIM-grounded SGI definition, workflow-centric benchmark, and empirical insights establish a foundation for AI systems that can genuinely participate in scientific discovery, revealing current limitations and proposing methods to enhance scientific reasoning capabilities.
...
TL;DR: PAACE is a plan-aware context compression framework for LLM agents that optimizes workflow contexts through relevance modeling and compression, improving accuracy while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: LLM agents in complex multi-step workflows generate rapidly expanding contexts that need curation and compression to maintain fidelity, avoid attention dilution, and reduce inference costs. Existing summarization approaches ignore the plan-aware nature of agentic reasoning.

Method: PAACE introduces a unified framework with two components: (1) PAACE-Syn generates synthetic agent workflows with stepwise compression supervision, and (2) PAACE-FT trains distilled, plan-aware compressors from successful teacher demonstrations. The framework uses next-k-task relevance modeling, plan-structure analysis, instruction co-refinement, and function-preserving compression.

Result: On AppWorld, PAACE achieves higher accuracy than all baselines while lowering peak context and cumulative dependency. On OfficeBench and multi-hop QA, it improves both accuracy and F1 with fewer steps, lower peak tokens, and reduced attention dependency. Distilled PAACE-FT retains 97% of teacher performance while reducing inference cost by over 10x.

Conclusion: PAACE demonstrates that plan-aware context compression significantly improves LLM agent performance while substantially reducing computational costs, enabling practical deployment of efficient agent workflows through distilled compact models.
...
TL;DR: This paper analyzes security threats in Agentic Vehicles (AgVs) by examining both OWASP-style AI risks and cross-layer cyber-attacks, proposing a role-based architecture and severity framework for security analysis.


<details>
  <summary>Details</summary>
Motivation: Existing AI security frameworks like OWASP don't address safety-critical cyber-physical systems like vehicles, nor do they account for interactions between agentic AI layers and other vehicle layers (perception, communication, control).

Method: Introduces a role-based architecture for AgVs with Personal Agent and Driving Strategy Agent, investigates vulnerabilities in both agentic AI layer and cross-layer risks, uses severity matrix and attack-chain analysis to show how small distortions escalate.

Result: Develops a framework that illustrates how vulnerabilities can lead to misaligned or unsafe behavior in both human-driven and autonomous vehicles, showing escalation paths from small distortions to serious safety issues.

Conclusion: Provides the first structured foundation for analyzing security risks of agentic AI in current and emerging vehicle platforms, addressing both AI-specific vulnerabilities and cross-layer cyber-physical threats.
...
TL;DR: Proposes UniRel-R1 framework for relation-centric KGQA that returns subgraphs showing entity connections instead of single entities, using LLM fine-tuning with RL to find informative subgraphs.


<details>
  <summary>Details</summary>
Motivation: Traditional KGQA focuses on entity-centric queries returning single answers, but real-world queries often seek to understand relationships between entities. There's a need for relation-centric KGQA that captures semantic connections.
...
Conclusion: Relation-centric KGQA is a valuable complementary setting to traditional entity-centric approaches, and the proposed UniRel-R1 framework effectively addresses the challenge of identifying informative subgraphs among abundant candidates.
...
<details>
  <summary>Details</summary>
Motivation: To understand how material (realistic) and symbolic threats interact in driving human conflict, overcoming limitations of weak causal control, ethical constraints, and scarce temporal data in traditional research.

Method: Using simulations of large language model (LLM)-driven agents in virtual societies, independently varying realistic and symbolic threats while tracking actions, language, and attitudes through representational analyses.

Result: LLM encodes realistic threat, symbolic threat, and hostility as distinct internal states; realistic threat directly increases hostility, while symbolic threat effects are weaker, fully mediated by ingroup bias, and only increase hostility when realistic threat is absent. Non-hostile contact buffers escalation, and structural asymmetries concentrate hostility among majority groups.

Conclusion: The study provides a causal account of threat-driven conflict, showing realistic threats dominate over symbolic threats in driving hostility, with symbolic threats operating through different psychological pathways and only becoming influential when material threats are absent.
...
TL;DR: Proposes Solomonoff-inspired method weighting LLM-generated hypotheses by simplicity and predictive fit for uncertainty-aware reasoning, outperforming Bayesian Model Averaging on Mini-ARC tasks.


<details>
  <summary>Details</summary>
Motivation: Reasoning under uncertainty with sparse data requires systematic generalization. Existing approaches struggle to balance accuracy and simplicity when evaluating multiple candidate solutions.

Method: Solomonoff-inspired method that weights LLM-generated hypotheses by simplicity and predictive fit, producing Solomonoff-weighted mixtures for per-cell predictions in benchmark tasks.

Result: Method yields conservative, uncertainty-aware outputs even with noisy/incorrect hypotheses. Solomonoff scoring spreads probability more evenly across competing hypotheses vs BMA which concentrates weight on most likely but potentially flawed candidates.

Conclusion: Highlights value of algorithmic information-theoretic priors for interpretable, reliable multi-hypothesis reasoning under uncertainty.
...
### [117] [Value Under Ignorance in Universal Artificial Intelligence](https://arxiv.org/abs/2512.17086)
*Cole Wyeth,Marcus Hutter*

Main category: cs.AI

TL;DR: The paper generalizes AIXI RL agent to handle more utility functions, addressing semimeasure loss ambiguity by treating belief distributions as imprecise probabilities and using Choquet integrals for expected utility computation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the ambiguity in AIXI when some hypotheses only predict finite history prefixes (semimeasure loss), which creates challenges for utility assignment and expected utility computation in reinforcement learning agents.

Method: The method treats belief distributions as imprecise probability distributions, uses Choquet integrals from imprecise probability theory to compute expected utilities, and investigates their computability level. The standard recursive value function is shown to be a special case.

Result: The paper shows that Choquet integrals provide a principled way to compute expected utilities under imprecise probabilities, but reveals that the most general expected utilities under the "death interpretation" cannot be characterized as such Choquet integrals.

Conclusion: The paper successfully generalizes AIXI to handle a wider class of utility functions using imprecise probability theory, though limitations exist for the most general death interpretation utilities, suggesting further research directions.
...
### [118] [A Solver-in-the-Loop Framework for Improving LLMs on Answer Set Programming for Logic Puzzle Solving](https://arxiv.org/abs/2512.17093)
...
Method: Solver-guided instruction-tuning: sample ASP statements from LLMs for logic puzzles, categorize as chosen/rejected based on solver feedback, apply supervised fine-tuning, and enhance with solver-guided search including best-of-N sampling.

Result: Consistent improvements in two distinct prompting settings across two datasets.

Conclusion: Solver-in-the-loop approach effectively addresses ASP code generation challenges by leveraging declarative programming properties and solver feedback.
...
### [119] [Reinforcement Learning for Self-Improving Agent with Skill Library](https://arxiv.org/abs/2512.17102)
*Jiongxiao Wang,Qiaojing Yan,Yawei Wang,Yijun Tian,Soumya Smruti Mishra,Zhichao Xu,Megha Gandhi,Panpan Xu,Lin Lee Cheong*

Main category: cs.AI

TL;DR: SAGE is an RL framework that enhances LLM agents' self-improvement through skill libraries, using Sequential Rollout across task chains and Skill-integrated Rewards, achieving higher efficiency and accuracy than existing methods.
...
Method: Proposes SAGE (Skill Augmented GRPO for self-Evolution), an RL framework with Sequential Rollout that deploys agents across chains of similar tasks, accumulating skills in a library. Uses Skill-integrated Reward to complement outcome-based rewards for better skill generation and utilization.

Result: On AppWorld, SAGE applied to supervised-finetuned models with expert experience achieved 8.9% higher Scenario Goal Completion, required 26% fewer interaction steps, and generated 59% fewer tokens, substantially outperforming existing approaches.

Conclusion: SAGE effectively enhances LLM agents' self-improvement capabilities through systematic skill library integration with RL, demonstrating significant improvements in both accuracy and efficiency compared to existing methods.
...
TL;DR: The paper proposes a two-stage reinforcement learning framework for explainable multi-modal retrieval-augmented generation (MMRAG), achieving state-of-the-art results on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Existing MMRAG methods lack explainability - they don't clarify the reasoning logic behind retrieval and response generation, which limits trust and understanding of the results.

Method: Two-stage reinforcement fine-tuning framework: 1) Rule-based reinforcement fine-tuning for coarse-grained point-wise ranking to filter irrelevant documents; 2) Reasoning-based reinforcement fine-tuning to jointly optimize fine-grained list-wise ranking and answer generation with explainable reasoning logic.
...
Conclusion: The proposed reinforcement learning approach successfully enhances reasoning capabilities of multi-modal large language models for explainable MMRAG, addressing the explainability gap in existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluations of unified multimodal models (UMMs) are decoupled, assessing understanding and generation abilities separately with different datasets. There's a need for a holistic benchmark that can evaluate all these capabilities together to provide a more comprehensive view of UMM performance.

Method: UmniBench uses human-examined prompts and QA pairs, then leverages the UMM itself to evaluate its generation and editing abilities using its understanding capabilities. This creates a self-referential evaluation paradigm where the model's understanding ability assesses its own generation and editing outputs.

Result: The benchmark covers 13 major domains and over 200 concepts, providing thorough inspection of UMMs. It can evaluate understanding, generation, and editing abilities either together or separately. The authors benchmarked 24 popular models including both UMMs and single-ability large models.

Conclusion: UmniBench provides a more comprehensive and objective evaluation framework for unified multimodal models, offering logistical support for improving community model performance through holistic assessment of multimodal understanding, generation, and editing capabilities.
...
TL;DR: A speculation-and-correction framework reduces inference latency in real-time control by generating action queues with predicted latent rollouts, then applying lightweight corrections when observations arrive, reducing planning inferences by 44% while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Real-time sequential control agents face performance degradation due to inference latency, where even modest per-step planning delays can destabilize control. The need for frequent replanning creates bottlenecks that limit real-time responsiveness.

Method: A speculation-and-correction framework adapts speculative execution to model-based control with TD-MPC2. At each step, a pretrained world model and latent-space MPC planner generate a short-horizon action queue with predicted latent rollouts. When new observations arrive, the system measures mismatch between encoded real latent states and queued predictions. For small/moderate mismatch, a lightweight learned corrector applies residual updates distilled from a replanning teacher. For large mismatch, the system falls back to full replanning. Two corrector architectures are studied: gated two-tower MLP and temporal Transformer.

Result: On DMC Humanoid-Walk task, the method reduces planning inferences from 500 to 282 (44% reduction), improves end-to-end step latency by 25%, and maintains strong control performance with only 7.1% return reduction. Ablation shows speculative execution without correction is unreliable over longer horizons.

Conclusion: The speculation-and-correction framework effectively reduces inference latency in real-time control while maintaining performance. Mismatch-aware correction is essential for robust latency reduction, as speculative execution alone is insufficient for longer horizons. The approach demonstrates practical trade-offs between computational efficiency and control quality.
...
TL;DR: EventGPT: A GPT-style transformer model for football transfer analysis that predicts next events and simulates how players would perform in different teams through counterfactual player embedding substitutions.


<details>
  <summary>Details</summary>
Motivation: Current football transfer evaluation methods rely on static statistics or post-hoc value models that fail to capture how players adapt to new tactical environments and teammates, making transfer success prediction difficult due to strong context-dependence.

Method: EventGPT is a player-conditioned, value-aware next-event prediction model using GPT-style autoregressive transformer. It treats match play as token sequences, predicting next action type, location, timing, and residual On-Ball Value (rOBV) based on context and player identity. Key innovation: counterfactual simulations by substituting player embeddings into new event sequences.

Result: Outperforms existing sequence-based baselines in next-event prediction accuracy and spatial precision on five seasons of Premier League data. Case studies demonstrate practical utility for transfer analysis, comparing striker performance across systems and identifying stylistic replacements for specific roles.

Conclusion: EventGPT provides a principled method for evaluating transfer fit by simulating how players' behavioral distributions and value profiles would change in different tactical environments, addressing the context-dependence problem in football transfer evaluation.
...
Method: Developed a turn-based Pokémon battle system where LLMs select moves based on battle state rather than pre-programmed logic, incorporating type effectiveness multipliers, stat-based damage calculations, and multi-Pokémon team management.

Result: Systematic evaluation across multiple model architectures showed LLMs can achieve competitive win rates, reasonable decision latency, type-alignment accuracy, and token efficiency as battle agents.

Conclusion: LLMs can serve as both competent game opponents and content designers without domain-specific training, offering practical alternatives to reinforcement learning for turn-based strategic games and implications for procedural generation and adaptive difficulty systems.
...
TL;DR: AI can discover human-like concepts from raw experience without supervision by treating concepts as information objects defined through structural relations to experience, with concepts evolving through dialectical optimization.


<details>
  <summary>Details</summary>
Motivation: To determine if AI can autonomously discover concepts similar to human concepts, given that human concepts are fluid and evolve over time (e.g., Pluto's reclassification). Need a definition of "concept" that's not just a label but a revisable, comparable structure.

Method: Algorithmic-information approach: concepts as information objects defined by structural relations to agent's total experience. Core constraint is determination (reversible consistency relation). Define excess information to measure redundancy overhead. Formulate dialectics as optimization dynamics where concepts compete to explain new information via shorter descriptions. Formalize concept transmission using small seeds for multi-agent alignment.

Result: Proposes a framework where concepts emerge from structural relations in experience, can be revised through dialectical competition, and can be efficiently transmitted between agents using minimal shared information.

Conclusion: AI can potentially discover human-like concepts autonomously by treating concepts as information structures tied to experience, with evolution through dialectical optimization and efficient transmission enabling multi-agent alignment.
...
TL;DR: The Rashomon effect extends to sequential decision-making where multiple policies achieve identical behavior while differing internally, with formal verification methods used to confirm identical behavior and Rashomon ensembles showing improved robustness.


<details>
  <summary>Details</summary>
Motivation: The Rashomon effect has been well-studied in classification tasks but not in sequential decision-making, where agents learn policies to achieve objectives through environmental interactions. The authors aim to translate this concept to sequential decision-making to understand how multiple policies can produce identical behavior while relying on different internal features.

Method: The authors define the Rashomon effect in sequential decision-making as multiple policies that exhibit identical behavior (visiting same states and selecting same actions) while differing internally. They use formal verification methods to construct and compare the complete probabilistic behavior of each policy in the environment, addressing the challenge of stochastic transitions where single trajectories may vary due to randomness.

Result: Experiments demonstrate that the Rashomon effect exists in sequential decision-making. Ensembles constructed from the Rashomon set exhibit greater robustness to distribution shifts than individual policies. Additionally, permissive policies derived from the Rashomon set reduce computational requirements for verification while maintaining optimal performance.

Conclusion: The Rashomon effect extends beyond classification to sequential decision-making, with formal verification enabling identification of behaviorally equivalent policies. Rashomon ensembles offer improved robustness to distribution shifts, and permissive policies from the Rashomon set provide computational efficiency benefits while preserving performance.
...
TL;DR: This paper introduces an LLM-powered diagnostic chatbot using GPT-4o with RAG and explainable AI techniques that achieves 90% accuracy and 100% top-3 accuracy, outperforming traditional ML models.


<details>
  <summary>Details</summary>
Motivation: Healthcare systems face inefficiencies in diagnostics, rising costs, and limited specialist access, leading to treatment delays and poor outcomes. Current AI diagnostic systems lack interactivity and transparency, making them less effective in real-world patient-centered environments.

Method: The research develops a diagnostic chatbot powered by GPT-4o using Retrieval-Augmented Generation (RAG) and explainable AI techniques. The system engages patients in dynamic conversations to extract and normalize symptoms, prioritizes diagnoses through similarity matching and adaptive questioning, and uses Chain-of-Thought prompting for transparent reasoning.
...
Conclusion: The findings demonstrate promising potential for more transparent, interactive, and clinically relevant AI in healthcare, addressing current limitations in diagnostic systems through LLM-powered conversational interfaces with explainable reasoning.
...
TL;DR: The paper introduces Timed Reward Machines (TRMs) to extend reward machines with timing constraints, enabling more expressive reward specifications for time-sensitive RL applications.
...
Method: Propose Timed Reward Machines (TRMs) that incorporate timing constraints into reward structure. Develop model-free RL frameworks using tabular Q-learning with abstractions of timed automata and counterfactual-imagining heuristics to exploit TRM structure.

Result: The algorithm learns policies that achieve high rewards while satisfying timing constraints on popular RL benchmarks. Comparative studies show performance differences under different TRM semantics, and ablations demonstrate benefits of counterfactual-imagining.
...
<details>
  <summary>Details</summary>
Motivation: Address gaps in understanding causal links between humanlike AI design and user outcomes, moving beyond Western-centric theoretical assumptions to examine global diversity in human-AI interactions.

Method: Two large-scale cross-national experiments (N=3,500) across 10 diverse nations involving real-time, open-ended interactions with AI systems to test humanlike design effects.

Result: Humanlike design increases anthropomorphism but doesn't universally boost engagement/trust. Effects are culturally mediated - same design choices produce opposite trust outcomes in different cultures (e.g., Brazil vs Japan). Users focus on interactional cues rather than theoretical aspects.

Conclusion: Prevailing narratives about inherent risks in humanlike AI are challenged. Findings reveal a nuanced, culturally mediated landscape requiring move beyond one-size-fits-all AI governance approaches.
...
TL;DR: LRMs have counterintuitive reasoning behaviors; LoRe framework proposes compute and accuracy laws; LoRe-Bench measures monotonicity/compositionality; most models lack compositionality; finetuning for compute-law compliance improves reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models (LRMs) show superior performance but exhibit counterintuitive reasoning behaviors that lead to suboptimal capabilities. There's a need to theoretically formalize desired reasoning behaviors to improve model performance.

Method: Proposes Laws of Reasoning (LoRe) framework with compute law (reasoning compute should scale linearly with question complexity) and accuracy law. Introduces LoRe-Bench to measure two tractable properties: monotonicity and compositionality. Develops finetuning approach to enforce compute-law compositionality.

Result: Evaluation shows most reasoning models exhibit reasonable monotonicity but lack compositionality. Finetuning for compute-law compliance yields consistently improved reasoning performance across multiple benchmarks and reveals synergistic effects across properties and laws.

Conclusion: The LoRe framework successfully characterizes reasoning patterns in LRMs. Better compliance with compute laws leads to improved reasoning performance, demonstrating the importance of formalizing desired reasoning behaviors for enhancing LRM capabilities.
...
TL;DR: Dion2 is a simplified method to reduce Muon optimizer's computational cost by selectively sampling rows/columns for orthonormalization, improving scalability.


<details>
  <summary>Details</summary>
Motivation: The Muon optimizer has strong performance but suffers from super-linear orthonormalization cost that increases with scale, creating overhead issues.

Method: Dion2 selects a fraction of rows or columns at each iteration and orthonormalizes only those, making the update sparse and reducing computation/communication costs.

Result: The method reduces both computation and communication costs, improving the scalability of the Muon optimizer compared to prior approaches.

Conclusion: Dion2 provides a simpler and more efficient approach to scale the Muon optimizer by reducing matrix size in orthonormalization through selective sampling.
...
TL;DR: Low-cost dual-mode EEG+EMG control system for prosthetic arms using NeuroSky EEG headset and MyoWare EMG sensors with ESP32 microcontrollers for real-time multi-degree-of-freedom control.


<details>
  <summary>Details</summary>
Motivation: Affordable upper-limb prostheses often lack intuitive control systems, limiting functionality and accessibility for amputees in low-resource settings. There's a need for low-cost, biologically intuitive prosthetic control suitable for underserved populations.

Method: Dual-mode neuro-muscular control integrating EEG (NeuroSky MindWave Mobile 2) and EMG (MyoWare 2.0). EEG signals processed via ThinkGear Bluetooth to ESP32 with lightweight classification model trained on 1500s of EEG data using 6-frame sliding window. EMG signals transmitted wirelessly to second ESP32 for threshold-based detection with three activation bands (rest, extension, contraction). EEG controls hand open/close via blink detection, EMG controls elbow movement with stability enhancement through 8-frame consecutive detection.

Result: Functional prototype constructed with total cost ~$240 (most expense from commercial EEG headset). System enables real-time multi-degree-of-freedom control: EEG-controlled ESP32 actuates four finger servos, EMG-controlled ESP32 drives two elbow servos. Demonstrates feasible pathway to low-cost prosthetic control.

Conclusion: The system demonstrates a feasible pathway to low-cost, biologically intuitive prosthetic control suitable for underserved and global health applications. Future work includes 3D-printed chassis, auto-regressive models to reduce EMG latency, and upgraded servo torque for improved load capacity and grip strength.
...
TL;DR: Quantum-inspired classifiers (PGM and KPGM) outperform classical random forest, with PGM achieving best accuracy and KPGM showing more stable performance across different Quantum SMOTE variants.


<details>
  <summary>Details</summary>
Motivation: To provide a unified theoretical and empirical comparison of quantum-inspired machine learning approaches (Kernel Trick and Pretty Good Measurement) and analyze their performance in synthetic oversampling scenarios using Quantum SMOTE variants.

Method: Comparative analysis of PGM and KPGM classifiers across synthetic oversampling scenarios using Quantum SMOTE variants, with evaluation against classical random forest baseline. Performance measured using accuracy, F1-score, and stability across different encoding strategies.

Result: Both PGM and KPGM classifiers consistently outperform classical random forest baseline. PGM with stereo encoding and n_copies=2 achieves highest accuracy (0.8512) and F1-score (0.8234). KPGM shows competitive and more stable performance across QSMOTE variants, with top scores of 0.8511 (stereo) and 0.8483 (amplitude).

Conclusion: Quantum-inspired classifiers provide tangible performance gains with complementary strengths: PGM benefits from encoding-specific enhancements while KPGM ensures robustness across sampling strategies. The findings advance understanding of kernel-based and measurement-based QiML methods and offer practical guidance for their application under varying data characteristics.
...
TL;DR: A novel "Compression is Routing" architecture using Transformer Autoencoder achieves 64x sequence compression and uses reconstruction error as intrinsic distribution fingerprint for expert routing without explicit gating networks.


<details>
  <summary>Details</summary>
Motivation: Address three major LLM challenges: context length limitations, high inference costs, and catastrophic forgetting during continual learning. Current MoE architectures use complex explicit routing classifiers that lack interpretability with mixed-domain inputs.

Method: Propose "Compression is Routing" philosophy. Train 87M-parameter Transformer Autoencoder achieving 64x sequence compression (512 tokens → 8 latent vectors). Use reconstruction error as Intrinsic Distribution Fingerprint for automatic expert scheduling without explicit gating networks.

Result: Achieves 99.47% reconstruction accuracy on in-domain (code) validation, drops to 47.76% on semi-OOD (Wiki text), and plummets to 0.57% on fully OOD (random sequences). This systematic discrepancy validates reconstruction error as distribution fingerprint.

Conclusion: Reconstruction error serves as valid Intrinsic Distribution Fingerprint for automatic expert routing. Architecture offers excellent scalability and new perspective on VRAM compression for ultra-long contexts. Provides foundation for next-generation scalable modular neural networks.
...
TL;DR: A lightweight XGBoost model trained on METAR data with physics-guided features outperforms operational TAF forecasts for low-visibility/precipitation nowcasting at airports, achieving 2.5-4x better recall with explainable physical insights.


<details>
  <summary>Details</summary>
Motivation: Current operational approaches for aviation weather nowcasting (TAF products and numerical weather prediction) are computationally intensive, have conservative biases, limited temporal resolution, and require manual configuration. There's a need for lightweight, automated, and accurate short-term prediction systems.

Method: Developed a gradient boosting framework (XGBoost) trained exclusively on surface observation data (METAR) with physics-guided feature engineering based on thermodynamic principles. The model was evaluated across 11 international airports representing different climatic regimes using historical data from 2000-2024.

Result: The model successfully captures local physical processes without manual configuration. In blind comparative evaluation against operational TAF forecasts, it achieved substantially higher detection rates at 3-hour horizons with 2.5-4.0 times improvement in recall while reducing false alarms. SHAP analysis shows the model implicitly reconstructs local physical drivers (advection, radiation, subsidence).

Conclusion: The lightweight physics-guided machine learning framework provides accurate, explainable nowcasting for aviation safety, outperforming traditional operational approaches while offering actionable insights through explainable AI techniques.
...
TL;DR: The paper introduces turn-PPO, a turn-level variant of PPO that outperforms GRPO for multi-turn RL tasks with LLM agents, especially in long-horizon reasoning scenarios.


<details>
  <summary>Details</summary>
Motivation: Direct application of GRPO to multi-turn RL tasks with LLM agents shows limitations in long-horizon reasoning scenarios, requiring more stable and effective advantage estimation strategies for multi-turn settings.

Method: First explored PPO as a more robust alternative to GRPO, then introduced turn-PPO - a variant that operates on turn-level MDP formulation instead of token-level MDP, designed specifically for multi-turn scenarios.

Result: Results on WebShop and Sokoban datasets demonstrate the effectiveness of turn-PPO, both with and without long reasoning components, showing improved performance over baseline approaches.

Conclusion: Turn-PPO provides a more effective approach for multi-turn RL with LLM agents, addressing limitations of token-level MDP formulations and offering better performance in long-horizon reasoning tasks.
...
TL;DR: GB-DQN uses gradient boosting ensembles to adapt to non-stationary RL environments by training new learners on Bellman residuals after drift, achieving better recovery and stability than DQN.
...
Method: GB-DQN constructs an additive ensemble where each new learner is trained to approximate the Bellman residual of the current ensemble after environmental drift, using incremental residual learning instead of retraining a single Q-network.

Result: Theoretical analysis shows each boosting step reduces empirical Bellman residual and the ensemble converges to post-drift optimal value function. Experiments across control tasks with dynamics changes demonstrate faster recovery, improved stability, and greater robustness compared to DQN and non-stationary baselines.

Conclusion: GB-DQN effectively addresses non-stationarity in RL through gradient-boosted ensembles, providing a principled approach to adapt to environmental changes without catastrophic forgetting.
...
TL;DR: SFBD-OMNI: A bridge model framework for distribution restoration from noisy samples using one-sided entropic optimal transport, with recoverability analysis and EM-like optimization.


<details>
  <summary>Details</summary>
Motivation: In many real-world scenarios, obtaining fully observed samples is expensive or infeasible, while partial and noisy observations are easier to collect. The paper addresses the problem of restoring the true underlying distribution from abundant noisy samples when the corruption process is available as a black-box generator.

Method: Frames distribution restoration as a one-sided entropic optimal transport problem and solves it via an EM-like algorithm. Introduces SFBD-OMNI, a bridge model-based framework that generalizes Stochastic Forward-Backward Deconvolution (SFBD) to handle arbitrary measurement models beyond Gaussian corruption. Includes a test criterion for determining recoverability under per-sample information loss.

Result: Experiments across benchmark datasets and diverse measurement settings demonstrate significant improvements in both qualitative and quantitative performance. The method shows that in otherwise unrecoverable cases, a small number of clean samples can render the distribution largely recoverable.

Conclusion: The proposed SFBD-OMNI framework effectively addresses distribution restoration from noisy samples, generalizing previous methods to arbitrary corruption models while providing theoretical insights into recoverability conditions.
...
TL;DR: DTDR improves on-device LLM function calling agents by dynamically retrieving tools based on evolving execution context, boosting success rates 23-104% over static methods.


<details>
  <summary>Details</summary>
Motivation: Existing on-device LLM agents use static retrieval methods that fail to capture multi-step tool dependencies and evolving task context, leading to irrelevant tools that degrade efficiency and accuracy.
...
Conclusion: Dynamic tool retrieval that adapts to execution context significantly outperforms static methods for on-device LLM function calling agents, with DTDR providing substantial improvements in accuracy and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To establish the complete equivalence between geometric measure-theoretic properties of metric spaces and the statistical learning property of k-nearest neighbor classifier universal consistency, which was previously only partially established.

Method: Mathematical proof establishing the implication (1)⇒(3), completing the cycle of equivalences. The paper also corrects a wrong claim from a previous article in the series.

Result: Proved the final missing implication (1)⇒(3), thereby establishing full equivalence between the three conditions. This confirms a conjecture from the first article in the series and corrects an error from the second article.

Conclusion: The equivalence is now fully established: k-NN universal consistency ⇔ strong Lebesgue-Besicovitch differentiation property ⇔ sigma-finite dimensionality in Nagata sense for complete separable metric spaces.
...
TL;DR: BEAM-LRC: Bandwidth-efficient MoE inference via router-guided low-rank compensation for quantized experts, improving bandwidth-accuracy trade-off and throughput.


<details>
  <summary>Details</summary>
Motivation: MoE models stress memory/bandwidth; offloading helps but token-level routing causes irregular I/O transfers; static uniform quantization reduces traffic but hurts accuracy by ignoring expert heterogeneity.

Method: Router-guided precision restoration using precomputed low-rank compensators; transfers compact low-rank factors with Top-n experts per token, applies compensation to them while keeping others low-bit; integrated with offloading on GPU and GPU-NDP systems.

Result: Superior bandwidth-accuracy trade-off and improved throughput compared to baseline approaches.

Conclusion: BEAM-LRC effectively addresses MoE inference bottlenecks by combining adaptive quantization with low-rank compensation, enabling efficient bandwidth utilization while maintaining accuracy.
...
TL;DR: Training LLMs on intentionally flawed reasoning traces improves error detection and recovery without harming standard problem-solving ability.


<details>
  <summary>Details</summary>
Motivation: Chain-of-thought prompting is brittle to early errors that propagate uncorrected to wrong answers. The paper investigates whether training on flawed reasoning can teach models to detect and recover from such errors.

Method: Used competition-level MATH-lighteval problems to generate CoT prefixes with controlled errors (calculation or reasoning errors). Fine-tuned Qwen3-4B with GRPO using binary final-answer reward. Compared Mixed-CoT-RL (trained on both clean and flawed traces) against standard RL and baseline.

Result: Mixed-CoT-RL matches standard RL on clean problems (41% vs 41%) while substantially outperforming on flawed reasoning problems (24% vs 19%). Clean-only RL degrades robustness below baseline (19% vs 20%). Training on reasoning errors yields greater robustness gains than calculation errors alone, with mixed training performing best.

Conclusion: Exposure to flawed traces during training can improve error-recovery behavior without sacrificing accuracy, suggesting a path toward more robust mathematical reasoning in LLMs.
...
TL;DR: Squared circuits enable efficient marginalization through parameterization with orthogonality conditions, overcoming computational complexity while maintaining expressiveness.


<details>
  <summary>Details</summary>
Motivation: Squared tensor networks and circuits are expressive distribution estimators but suffer from computational complexity when computing partition functions or marginalizing variables, limiting their ML applicability.

Method: Parameterize squared circuits using orthogonality conditions inspired by canonical forms of TNs and determinism in circuits, enabling efficient marginalization even for factorizations different from TNs.
...
Conclusion: Squared circuits can be parameterized to overcome marginalization overhead while maintaining full expressiveness, making them more practical for machine learning applications.
...
<details>
  <summary>Details</summary>
Motivation: To solve complex hierarchical planning problems by fusing reinforcement learning and model predictive control (MPC) in a tight, elegant coupling that leverages the strengths of both approaches for robust planning.

Method: Proposes a formulation that couples RL and MPPI (Model Predictive Path Integral) planning: RL actions inform MPPI sampling, and MPPI samples are adaptively aggregated to inform value estimation. The adaptive process focuses MPPI exploration where value estimates are uncertain.

Result: Demonstrates better data efficiency and overall performance across race driving, modified Acrobot, and Lunar Lander with obstacles. Achieves up to 72% increase in success rate compared to existing approaches and 2.1x accelerated convergence compared to non-adaptive sampling.

Conclusion: The approach creates a robust planning method that handles complex hierarchical problems and adapts easily to different applications, showing significant improvements in both performance and training efficiency.
...
Method: UniCoMTE identifies temporal features influencing predictions by modifying input samples and assessing impact on model predictions. It's model-agnostic, works directly on raw time series, and generates counterfactual explanations.

Result: UniCoMTE produces concise, stable, and human-aligned explanations that outperform LIME and SHAP in clarity and applicability. Medical experts found the explanations clinically useful when reviewing ECG samples.

Conclusion: The framework advances interpretability of deep learning models for real-world time series applications by linking model predictions to meaningful signal patterns, enhancing trust in high-stakes domains.
...
TL;DR: Proposes a differentiable fast fault simulation model (DFFSM) and gradient-based fault parameter identification (GFPI) method for accurate photovoltaic fault quantification with high efficiency and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing fault quantification methods for PV arrays suffer from limited efficiency and interpretability, making accurate fault diagnosis and quantification challenging for reliable PV system operation and maintenance.

Method: Develops a differentiable fast fault simulation model (DFFSM) that accurately models I-V characteristics under multiple faults and provides analytical gradients. Uses this to create a gradient-based fault parameters identification (GFPI) method with Adahessian optimizer to quantify partial shading, short-circuit, and series-resistance degradation.

Result: Experimental results on simulated and measured I-V curves show GFPI achieves high quantification accuracy across different faults, with I-V reconstruction error below 3%, demonstrating feasibility and effectiveness.

Conclusion: The proposed approach successfully addresses efficiency and interpretability limitations in PV fault quantification, confirming the viability of differentiable physical simulators for PV system fault diagnosis applications.
...
TL;DR: Atom is an on-device system that restructures video-language pipelines by decomposing billion-parameter models into reusable modules to eliminate redundant model loading and enable parallel execution on mobile devices.


<details>
  <summary>Details</summary>
Motivation: Executing multi-stage video-language pipelines (retrieval, captioning, assembly) efficiently on mobile devices is challenging due to redundant model loads and fragmented execution, creating a need for optimized on-device systems.

Method: Atom decomposes billion-parameter video-language models into reusable modules (visual encoder, language decoder) and reuses them across subtasks (captioning, reasoning, indexing), enabling parallel execution and eliminating repeated model loading.

Result: On commodity smartphones, Atom achieves 27-33% faster execution compared to non-reuse baselines with minimal performance degradation (≤2.3 Recall@1 in retrieval, ≤1.5 CIDEr in captioning).

Conclusion: Atom provides a practical, scalable approach for efficient video-language understanding on edge devices through reuse-centric design that reduces latency while maintaining performance.
...
TL;DR: Unified framework maintains factorized momentum/curvature stats during training for reuse in geometry-aware model merging, eliminating redundant computation.


<details>
  <summary>Details</summary>
Motivation: Current workflows waste computation by computing curvature info during training, discarding it, then recomputing similar info for model merging. There's a need to unify training and merging by treating optimization trajectory as reusable asset.

Method: Maintains factorized momentum and curvature statistics during training, accumulates task saliency scores for curvature-aware merging without post-hoc Fisher computation. Uses low-rank structure and parameter importance estimation.

Result: Achieves memory efficiency comparable to SOTA, curvature-aware parameter selection outperforms magnitude-only baselines across all sparsity levels, multi-task merging improves over strong baselines. Shows rank-invariant convergence and superior hyperparameter robustness.

Conclusion: By treating optimization trajectory as reusable asset rather than discarding it, the approach eliminates redundant computation while enabling more principled model composition with convergence guarantees for non-convex objectives.
...
<details>
  <summary>Details</summary>
Motivation: Old Nepali is a historically significant but low-resource language lacking HTR systems, creating a need for accessible transcription tools for historical documents.
...
Result: Best model achieves 4.9% Character Error Rate (CER). Training code, model configurations, and evaluation scripts are released for further research.

Conclusion: Successful development of first HTR pipeline for Old Nepali demonstrates feasibility for low-resource historical scripts, with released resources to support community research.
...
TL;DR: This paper evaluates and improves CLIP-based medical vision-language models' ability to handle negation in chest X-ray image retrieval, showing improved negation handling with slight trade-offs in positive prompt accuracy.


<details>
  <summary>Details</summary>
Motivation: CLIP-based models underperform when interpreting negated phrases, which is particularly problematic for medical diagnosis where accurate understanding of negation (e.g., "no pneumonia") is crucial for reliable clinical applications.

Method: Evaluated Stanford AIMI CheXagent model on chest X-ray image retrieval using prompts with/without negation, then fine-tuned using methods from previous work. Analyzed internal model behavior through token attribution, t-SNE projection, and attention-head ablation.

Result: Fine-tuning improved the model's handling of negation in CLIP, though with a slight decrease in accuracy for positive prompt evaluation. Internal analysis revealed how fine-tuning reshaped text encoder representations of negated clinical language.

Conclusion: The work provides better understanding of CLIP's internal behavior with negation and demonstrates improvement in handling clinically relevant negated language, contributing to more reliable medical AI devices.
...
TL;DR: DiffeoMorph is a differentiable framework for learning distributed morphogenesis protocols where agents collectively form 3D shapes using attention-based SE(3)-equivariant GNNs and a novel 3D Zernike polynomial shape-matching loss.


<details>
  <summary>Details</summary>
Motivation: Understanding how distributed control in biological systems creates precise 3D patterns is crucial for developmental biology, distributed robotics, programmable matter, and multi-agent learning. Current approaches lack end-to-end differentiable frameworks for learning such morphogenesis protocols.

Method: Uses attention-based SE(3)-equivariant graph neural networks where agents update positions/states based on internal states and neighbor signals. Introduces 3D Zernike polynomial shape-matching loss for continuous spatial distribution comparison with SO(3) invariance via alignment optimization. Uses bilevel optimization with implicit differentiation through alignment step.

Result: Systematic benchmarking shows advantages over standard shape distance metrics. Framework successfully forms various shapes from simple ellipsoids to complex morphologies using minimal spatial cues.

Conclusion: DiffeoMorph provides an effective end-to-end differentiable framework for learning distributed morphogenesis protocols, bridging biological pattern formation with multi-agent systems and offering robust shape formation capabilities.
...
TL;DR: GPA is a new optimization method that extends Nesterov's primal averaging to overcome limitations of recent averaging-based optimizers like single-worker DiLoCo and Schedule-Free, providing smoother averaging with better performance and reduced complexity.


<details>
  <summary>Details</summary>
Motivation: Recent averaging-based optimizers like single-worker DiLoCo and Schedule-Free improve base optimizers (e.g., AdamW) but have limitations: DiLoCo's periodic averaging creates a two-loop structure with increased memory and hyperparameters, while Schedule-Free maintains uniform averages. GPA aims to overcome these limitations with a more efficient averaging approach.

Method: GPA extends Nesterov's primal averaging formulation by decoupling the interpolation constant, enabling smooth averaging at every step. This generalizes single-worker DiLoCo while eliminating its two-loop structure, reducing memory overhead to a single additional buffer, and simplifying hyperparameter tuning.

Result: Empirical results show GPA consistently outperforms single-worker DiLoCo: 24.22% speedup on Llama-160M to reach AdamW's validation loss, and 12%/27% speedups on small/large batch ImageNet ViT setups to achieve AdamW's validation accuracy. Theoretical analysis proves GPA can match or exceed convergence guarantees of base optimizers with O(√T) regret bounds.

Conclusion: GPA provides a superior averaging-based optimization approach that generalizes and improves upon recent methods like single-worker DiLoCo, offering better performance, reduced complexity, and theoretical convergence guarantees while maintaining practical efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Need for distributed spectrum sharing among cognitive entities in interference-constrained networks with unknown, time-varying channel qualities, requiring both stability and learning.

Method: SMILE (Stable Multi-matching with Interference-aware LEarning) integrates restless bandit learning with graph-constrained coordination, enabling distributed exploration-exploitation balance.

Result: Proves convergence to optimal stable allocation with logarithmic regret relative to full-knowledge genie; simulations show robustness, scalability, and efficiency.

Conclusion: First work to establish global Gale-Shapley stability for channel allocation in stochastic restless environments, providing communication-efficient distributed solution.
...
TL;DR: BumpNet is a sparse neural network framework using trainable sigmoid-based basis functions for PDE solution and operator learning, achieving model parsimony through dynamic pruning.


<details>
  <summary>Details</summary>
Motivation: To create an efficient, adaptive neural network framework for PDE numerical solutions and operator learning that combines meshless basis function expansion with modern training techniques, addressing limitations of traditional RBF networks.

Method: BumpNet uses trainable sigmoid activation functions as basis functions with fully trainable parameters (shape, location, amplitude). It achieves sparsity through dynamic pruning during training. Three variants are proposed: Bump-PINNs for general PDEs using collocation-based training; Bump-EDNN for time-evolution PDEs using BumpNet in spatial domain and EDNNs for temporal evolution; and Bump-DeepONet for operator learning using BumpNet as trunk network in DeepONet architecture.

Result: Extensive numerical experiments demonstrate the efficiency and accuracy of the proposed BumpNet architecture and its variants for solving PDEs and learning PDE operators.

Conclusion: BumpNet provides a general, efficient framework for PDE solution and operator learning that combines meshless basis function advantages with modern neural network training techniques, achieving adaptive sparsity and improved performance.
...
TL;DR: A simple kernel ridge regression with diffusion maps kernel outperforms complex models for long-term prediction of dynamical systems by respecting intrinsic geometric constraints.


<details>
  <summary>Details</summary>
Motivation: Complex nonlinear dynamical systems are hard to predict long-term, and existing data-driven models often fail when geometric structures governing long-term behavior are unknown or poorly represented.

Method: Diffusion Maps Kernel Ridge Regression (DM-KRR) combines kernel ridge regression with a data-driven kernel derived from diffusion maps, implicitly adapting to the system's intrinsic geometry without explicit manifold reconstruction.

Result: DM-KRR consistently outperforms state-of-the-art random feature, neural-network and operator-learning methods across various systems (smooth manifolds, chaotic attractors, high-dimensional spatiotemporal flows) in both accuracy and data efficiency.

Conclusion: Long-term predictive skill depends critically on respecting geometric constraints through dynamically consistent model selection; DM-KRR offers a promising path for reliable and efficient learning of complex dynamical systems.
...
<details>
  <summary>Details</summary>
Motivation: With growing EV adoption for climate change mitigation, predicting EV charging demand is crucial for electric grid management. There's a research gap in systematically comparing forecasting methods across different time horizons and spatial scales in diverse urban settings.

Method: Investigates five time series forecasting models (traditional statistical, machine learning, and deep learning methods). Evaluates performance across short-, mid-, and long-term horizons (minutes, hours, days) and spatial scales (individual stations to regional/city-level aggregations). Uses four publicly available real-world datasets with independent reporting for each dataset.

Result: To the best of their knowledge, this is the first work to systematically evaluate EV charging demand forecasting across such a wide range of temporal horizons and spatial aggregation levels using multiple real-world datasets.

Conclusion: The study provides comprehensive benchmarking of EV charging forecasting methods, addressing a significant research gap and offering practical insights for grid management across different temporal and spatial scales.
...
TL;DR: SHARP-QoS is a unified framework for joint QoS prediction that addresses data sparsity, noise, hierarchical dependencies, and negative transfer issues through hyperbolic convolution, adaptive feature sharing, and EMA-based loss balancing.


<details>
  <summary>Details</summary>
Motivation: Real-world QoS data are sparse, noisy, and have hierarchical dependencies from QoS interactions and geographical/network factors. Existing methods either predict each QoS parameter separately (increasing computational cost) or use joint approaches that suffer from negative transfer due to inconsistent numerical ranges and inadequate representation learning.

Method: Three key components: 1) Dual hyperbolic convolution in Poincaré ball to extract hierarchical features from QoS and contextual structures, 2) Adaptive feature-sharing mechanism with gated feature fusion for dynamic feature selection, 3) EMA-based loss balancing strategy for stable joint optimization to mitigate negative transfer.

Result: Outperforms both single- and multi-task baselines on three datasets with two, three, and four QoS parameters. Effectively addresses sparsity, robustness to outliers, and cold-start problems while maintaining moderate computational overhead.

Conclusion: SHARP-QoS provides a reliable unified strategy for joint QoS prediction that overcomes key challenges in real-world QoS data through hierarchical feature extraction, adaptive feature sharing, and balanced optimization, demonstrating superior performance across multiple QoS parameters.
...
TL;DR: This paper introduces Generalized Bisimulation Metric (GBSM) for measuring state similarity between arbitrary pairs of MDPs, establishing rigorous mathematical properties and demonstrating improved theoretical bounds for cross-MDP applications.


<details>
  <summary>Details</summary>
Motivation: While bisimulation metric (BSM) is effective for analyzing state similarities within a single MDP, its application to state similarity between multiple MDPs remains challenging. Prior attempts to extend BSM to multiple MDPs lacked well-established mathematical properties, limiting theoretical analysis across different MDPs.

Method: The authors formally establish a Generalized Bisimulation Metric (GBSM) for measuring state similarity between arbitrary pairs of MDPs. They rigorously prove three fundamental metric properties: GBSM symmetry, inter-MDP triangle inequality, and a distance bound on identical spaces.

Result: GBSM enables theoretical analysis of policy transfer, state aggregation, and sampling-based estimation across MDPs, obtaining explicit bounds that are strictly tighter than existing ones derived from standard BSM. GBSM also provides a closed-form sample complexity for estimation, improving upon existing asymptotic results based on BSM.

Conclusion: GBSM successfully extends bisimulation metrics to multi-MDP scenarios with rigorous mathematical foundations, enabling improved theoretical analysis and practical applications across different Markov decision processes. Numerical results validate the theoretical findings and demonstrate GBSM's effectiveness in multi-MDP scenarios.
...
TL;DR: The paper introduces R-EMID, an information-theoretic metric to measure and diagnose performance degradation in role-playing models due to distribution shifts, with a theoretical upper bound and a co-evolving RL framework for improved generalization.


<details>
  <summary>Details</summary>
Motivation: Role-playing models underperform in real-world deployment due to distribution shifts (user, character, dialogue composition), but existing methods lack fine-grained diagnosis and formal frameworks to characterize generalization behaviors.

Method: Proposes reasoning-based effective mutual information difference (R-EMID) metric, derives its theoretical upper bound for worst-case performance prediction, and develops a co-evolving reinforcement learning framework to model connections among user, character, and dialogue context for better response probability estimation.

Result: Evaluation shows user shift poses the highest risk among all distribution shifts, and reinforcement learning is the most effective approach for enhancing RPM generalization performance.

Conclusion: R-EMID provides an interpretable framework for diagnosing RPM generalization issues, with theoretical guarantees and practical adaptation methods to improve model robustness against distribution shifts.
...
TL;DR: MINPO is a unified neural framework that learns nonlocal operators and their inverses to solve integro-differential equations, outperforming classical methods and specialized neural solvers across diverse kernel types and dimensionalities.


<details>
  <summary>Details</summary>
Motivation: Classical methods for solving integro-differential equations (IDEs) are computationally expensive due to repeated convolution integral evaluations, and existing neural solvers lack generalization across diverse nonlocal structures.

Method: MINPO learns nonlocal operators and their inverses directly through neural representations (using KANs or MLPs as encoders), explicitly reconstructs solution fields, and enforces coherence via a lightweight nonlocal consistency loss term.

Result: MINPO demonstrates superior accuracy compared to classical techniques and state-of-the-art neural methods (A-PINN, fPINN, A-PIKAN, fPIKAN), showing robustness across diverse kernel types, dimensionalities, and computational demands.

Conclusion: MINPO provides a unified framework for systems governed by nonlocal operators, generalizing beyond problem-specific formulations and efficiently handling the computational challenges of integro-differential equations.
...
TL;DR: MATCH-AD is a semi-supervised framework that combines deep learning, graph-based label propagation, and optimal transport theory to diagnose Alzheimer's disease with high accuracy using limited labeled data.
...
Method: MATCH-AD integrates deep representation learning, graph-based label propagation, and optimal transport theory (using Wasserstein distances) to propagate diagnostic information from limited labeled samples to larger unlabeled populations by leveraging manifold structure in neuroimaging data.

Result: Evaluated on nearly 5,000 subjects from NACC with structural MRI, CSF biomarkers, and clinical variables, MATCH-AD achieves near-perfect diagnostic accuracy despite ground truth labels for less than one-third of subjects, substantially outperforming all baselines with kappa indicating almost perfect agreement.
...
<details>
  <summary>Details</summary>
Motivation: Continual learning on edge platforms is challenging due to energy-intensive training procedures and frequent data movement that are impractical for embedded deployments. Current approaches lack efficient on-chip learning capabilities for temporal processing.

Method: Introduces M2RU architecture with: 1) weighted-bit streaming enabling multi-bit digital inputs to be processed in crossbars without high-resolution conversion, and 2) experience replay mechanism stabilizing learning under domain shifts. Implements minion recurrent unit for efficient temporal processing.

Result: Achieves 15 GOPS at 48.62 mW (312 GOPS/W), maintains accuracy within 5% of software baselines on sequential MNIST and CIFAR-10 tasks. Provides 29X improvement in energy efficiency compared to CMOS digital design. Device-aware analysis shows 12.2 years operational lifetime under continual learning workloads.

Conclusion: M2RU establishes a scalable and energy-efficient platform for real-time adaptation in edge-level temporal intelligence, addressing the challenges of continual learning on resource-constrained edge devices.
...
TL;DR: The paper proposes a formal criterion for inherent explainability using graph theory to decompose models into structure-local explanations and recompose them into global explanations, with a case study showing PREDICT cardiovascular risk model is inherently explainable.


<details>
  <summary>Details</summary>
Motivation: There is no consistent definition or test for inherent explainability in XAI. Current approaches either use metrics or rely on intuition ("we know it when we see it"), lacking a rigorous, globally applicable criterion.

Method: Uses graph theory to represent and decompose models into structure-local explanations (annotations with verifiable hypothesis-evidence structure), then recomposes them into global explanations. Differentiates between "explainable" (allows for explanation) and "explained" (has verified explanation).

Result: The criterion matches existing intuitions about inherent explainability and justifies why large regression models may not be explainable while sparse neural networks could be. Provides a full explanation of PREDICT (Cox proportional hazards model for cardiovascular disease risk), demonstrating it is inherently explainable.

Conclusion: The work provides a formal structure for explainability research and offers regulators a flexible but rigorous test for compliance frameworks, establishing a concrete criterion for inherent explainability.
...
TL;DR: ICL decomposes into two separable mechanisms: Task Schema (abstract task recognition) and Binding (specific input-output associations), with causal validation across multiple architectures.


<details>
  <summary>Details</summary>
Motivation: To provide causal mechanistic validation that in-context learning decomposes into separable components, contrasting with prior monolithic views of ICL as retrieval-based, gradient descent-like, or purely Bayesian mechanisms.

Method: Activation patching experiments across 9 models from 7 Transformer families plus Mamba (370M-13B parameters), examining transfer patterns and neural dissociations between Task Schema and Binding mechanisms.

Result: Three key findings: 1) Double dissociation showing separable mechanisms (100% Schema transfer via late MLP patching, 62% Binding transfer via residual stream), 2) Prior-Schema trade-off (inverse correlation rho = -0.596), 3) Architecture generality including non-Transformer Mamba.

Conclusion: ICL operates via dual-process mechanisms (Schema and Binding) that are neurally dissociable, with Schema reliance when prior knowledge is absent and attentional mis-routing (not output competition) as the bottleneck. This enables more efficient prompt engineering and improved ICL system reliability.
...
TL;DR: Adaptive pruning algorithm reduces communication overhead in distributed ST-GNNs for smart mobility by dynamically filtering redundant neighbor features while maintaining prediction accuracy, with novel SEPA metric focusing on traffic event responsiveness.


<details>
  <summary>Details</summary>
Motivation: ST-GNNs deployed across distributed edge nodes (cloudlets) create substantial communication overhead due to repeated transmission of overlapping node features between neighboring cloudlets, which needs to be addressed for efficient smart mobility systems.

Method: Proposed adaptive pruning algorithm that dynamically filters redundant neighbor features while preserving informative spatial context, adjusting pruning rates based on recent model performance. Also introduced SEPA (Sudden Event Prediction Accuracy) metric to measure responsiveness to traffic slowdowns and recoveries.

Result: Experiments on PeMS-BAY and PeMSD7-M datasets show adaptive pruning maintains prediction accuracy while significantly lowering communication costs in all online semi-decentralized settings (traditional FL, server-free FL, Gossip Learning). SEPA metric reveals true value of spatial connectivity in predicting dynamic traffic events.

Conclusion: Communication can be reduced without compromising responsiveness to critical traffic events. The adaptive pruning algorithm effectively balances communication efficiency and prediction accuracy, while SEPA provides better evaluation of traffic event prediction capabilities than standard error metrics.
...
TL;DR: LLM-SGA framework + ARHOCD detector improves adversarial robustness for harmful content detection using ensemble methods, dynamic weighting, and adversarial training.


<details>
  <summary>Details</summary>
Motivation: Social media harmful content detection ML models are vulnerable to adversarial attacks; need robust detectors with both generalizability and accuracy.

Method: 1) LLM-SGA framework identifies attack invariances for generalizability; 2) ARHOCD detector with ensemble of base detectors, dynamic Bayesian weight assignment, and iterative adversarial training.

Result: ARHOCD shows strong generalizability and improved detection accuracy across hate speech, rumor, and extremist content datasets under adversarial conditions.

Conclusion: The proposed framework and detector effectively enhance adversarial robustness for harmful online content detection, addressing limitations of existing research.
...
TL;DR: AdvJudge-Zero reveals that LLM judge systems are vulnerable to short control tokens that flip binary judgments from correct "No" to incorrect "Yes" by manipulating logit gaps, posing realistic reward-hacking risks.


<details>
  <summary>Details</summary>
Motivation: Reward models and LLM-as-a-Judge systems are critical for modern post-training pipelines (RLHF, DPO, RLAIF), but they may have vulnerabilities that could be exploited during policy model generation, representing realistic reward-hacking risks rather than just worst-case adversarial scenarios.

Method: AdvJudge-Zero uses the model's next-token distribution and beam-search exploration to discover diverse control-token sequences from scratch. The method analyzes how these tokens induce hidden-state perturbations that concentrate in a low-rank "soft mode" anti-aligned with the judge's refusal direction.

Result: Control tokens cause very high false positive rates when large open-weight and specialized judge models score incorrect answers on math and reasoning benchmarks. The tokens successfully flip many binary evaluations from correct "No" judgments to incorrect "Yes" judgments.

Conclusion: LoRA-based adversarial training on small sets of control-token-augmented examples can significantly reduce false positives while preserving evaluation quality, offering a practical defense against this vulnerability in judge systems.
...
TL;DR: The paper proposes a novel activation module for Private Inference that reduces DReLU operations by having prototype channels compute DReLU and replicate channels copy results, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Private Inference requires privacy-preserving machine learning where model owners don't learn client data and clients don't learn model details. The main computational bottleneck is ReLU/DReLU operations, so reducing their number is crucial for efficiency.

Method: Proposes a new activation module where DReLU operations are only performed on prototype channels, while replicate channels copy DReLU results from corresponding neurons in prototype channels. This approach is extended across different network layers.

Result: Drastically reduces DReLU operations in ResNet-type networks. Theoretically solves an extended XOR problem with just one non-linearity and two neurons. Achieves new SOTA results on several classification tasks and SOTA on image segmentation.

Conclusion: The proposed activation module significantly reduces computational overhead in Private Inference while maintaining or improving performance, offering an efficient solution to the DReLU bottleneck problem.
...
TL;DR: A statistical toolbox for rigorous subgroup performance analysis in medical imaging ML models, addressing metric selection, uncertainty estimation, multiple comparisons correction, and intersectional subgroup discovery.


<details>
  <summary>Details</summary>
Motivation: Stratified analysis of ML model performance by patient/recording properties is crucial for identifying failure modes, but performing such analyses with statistical rigor is challenging due to issues with appropriate metrics, uncertainty estimation, multiple comparisons, and intersectional subgroup discovery.

Method: Developed a statistical toolbox that addresses: 1) selection of appropriate performance metrics for valid group comparisons with different sample sizes and base rates, 2) determination of metric uncertainty, 3) correction for multiple comparisons, and 4) mechanisms for finding 'interesting' subgroups in intersectional analyses among many combinations.

Result: The toolbox enables practitioners to rigorously assess models for potential subgroup performance disparities. Demonstrated applicability through two case studies: skin lesion malignancy classification on ISIC2020 dataset and chest X-ray-based disease classification on MIMIC-CXR dataset.

Conclusion: The presented statistical toolbox provides a practical solution for rigorous subgroup performance analysis in medical imaging ML applications, addressing key statistical challenges to ensure valid comparisons and meaningful insights into model failure modes across different patient subgroups.
...
TL;DR: A multi-agent reinforcement learning framework for analyzing long-term electricity markets and decarbonization policies, applied to the Italian electricity system.


<details>
  <summary>Details</summary>
Motivation: Electricity systems are crucial for carbon-free economy transformation, but policymakers need better tools to design, test, and evaluate long-term market mechanisms like auctions and support schemes for decarbonization.

Method: Multi-agent reinforcement learning model with profit-maximizing generation companies making investment decisions in wholesale electricity markets. Uses independent proximal policy optimization (PPO) with extensive hyperparameter search to ensure competitive behavior in decentralized training.

Result: Applied to a stylized Italian electricity system under varying competition levels, market designs, and policy scenarios. Results show market design's critical role in decarbonizing electricity sector and avoiding price volatility.

Conclusion: The framework enables assessment of long-term electricity markets where multiple policy and market mechanisms interact, with participants adapting to decarbonization pathways.
...
TL;DR: Write-Gated KV learns to predict token utility before writing to cache, reducing memory by 46-57% and achieving 3x prefill/2x decode speedups with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Long-context LLM inference suffers from quadratic attention complexity and linear KV cache growth. Existing approaches use post-hoc selection/eviction but miss the root problem: indiscriminate writing to persistent memory.

Method: Formalizes KV cache management as causal system with three primitives: KV Admission, Selection, Eviction. Instantiates KV Admission via Write-Gated KV - lightweight mechanism that predicts token utility before cache entry. Maintains compact global cache with sliding local cache.
...
Conclusion: Learning what to write (via early utility prediction) is a principled and practical approach for efficient long-context inference, addressing the root inefficiency of indiscriminate cache writing.
...
<details>
  <summary>Details</summary>
Motivation: Need for efficient and accurate long-term multivariate time series forecasting models that can capture both temporal patterns and spatial dependencies while being parameter-efficient and faster than transformer-based methods.

Method: Integrates trend-seasonal decomposition for temporal modeling with spatial module using low-rank Top-K adjacency learning and conservative horizon-wise gating for message passing, enhancing a strong linear baseline with spatial corrections.
...
Conclusion: Lite-STGNN provides a compact, interpretable, and efficient framework for long-term multivariate time series forecasting, with learned adjacency matrices revealing domain-specific interaction dynamics.
...
TL;DR: Deep learning surrogate models (BiLSTM-VAE and BiLSTM-Transformer) replace computationally expensive ANSYS creep simulations for Inconel 625, achieving seconds vs. 30-40 minutes runtime while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Finite-element creep simulations for high-temperature alloys like Inconel 625 are computationally expensive (30-40 minutes per simulation), limiting rapid design optimization and structural health monitoring in aerospace and energy systems.

Method: Generated creep strain data using ANSYS with Norton law under uniaxial stresses (50-150 MPa) and temperatures (700-1000°C). Trained two deep learning architectures: BiLSTM Variational Autoencoder for uncertainty-aware probabilistic predictions, and BiLSTM Transformer hybrid using self-attention for long-range temporal behavior.

Result: Both models achieved strong performance (evaluated with RMSE, MAE, R²). BiLSTM-VAE provides stable probabilistic forecasts, BiLSTM-Transformer achieves high deterministic accuracy. Surrogate models predict within seconds vs. 30-40 minutes for ANSYS simulations, enabling substantial speedup.

Conclusion: The deep learning surrogate framework enables rapid creep assessment for design optimization and structural health monitoring, providing a scalable solution for high-temperature alloy applications with orders-of-magnitude faster predictions.
...
TL;DR: SafeBench-Seq is a reproducible benchmark for protein hazard screening using public data and interpretable features, evaluated under strict homology control to prevent overestimation of model robustness.


<details>
  <summary>Details</summary>
Motivation: The community lacks a simple, reproducible baseline for protein sequence hazard screening that runs on commodity CPUs and is explicitly evaluated under homology control to approximate "never-before-seen" threats.

Method: Built from public data (SafeProtein hazards and UniProt benigns) using interpretable features (physicochemical descriptors and amino-acid composition). Performs homology clustering at ≤40% identity with cluster-level holdouts to prevent data leakage. Evaluates discrimination metrics (AUROC/AUPRC) and screening operating points with bootstrap confidence intervals, and quantifies probability quality using calibration metrics.
...
Conclusion: SafeBench-Seq provides a reproducible, CPU-only benchmark for rigorous evaluation of protein hazard screening models under realistic homology control, enabling better assessment of biosecurity risks without distributing hazardous sequences.
...
TL;DR: CFF extends Forward-Forward algorithm with inter-layer collaboration mechanisms to overcome isolation limitations, improving performance while maintaining memory efficiency and biological plausibility.
...
Method: Introduces Collaborative Forward-Forward (CFF) learning with two paradigms: Fixed CFF (F-CFF) with constant inter-layer coupling and Adaptive CFF (A-CFF) with learnable collaboration parameters. Uses collaborative goodness function with weighted contributions from all layers.
...
TL;DR: New Bayesian optimization variants of Knowledge Gradient acquisition functions for problems with decoupled black-box constraints, where objective and constraint functions can be evaluated independently.


<details>
  <summary>Details</summary>
Motivation: Bayesian optimization is effective for expensive global black-box optimization, but existing methods don't efficiently handle decoupled constraints where only a few constraints may be binding at the optimum, leading to unnecessary constraint evaluations.

Method: Propose new Bayesian optimization variants of Knowledge Gradient acquisition functions specifically designed for problems with decoupled black-box constraints, focusing on evaluating only relevant constraints rather than all constraints.

Result: Empirical benchmarks demonstrate superiority over state-of-the-art methods, showing improved performance in optimization problems with decoupled constraints.

Conclusion: The proposed Knowledge Gradient variants effectively handle decoupled constraint optimization by selectively evaluating only binding constraints, outperforming existing methods and advancing Bayesian optimization for constrained problems.
...
TL;DR: GreedySnake is an SSD-offloaded training system that uses vertical scheduling of micro-batches to achieve higher throughput for large language model training, especially with smaller batch sizes.


<details>
  <summary>Details</summary>
Motivation: SSD-offloaded training makes LLM training more cost-effective, but existing systems using horizontal scheduling have limitations in throughput, especially with smaller batch sizes.

Method: Introduces vertical scheduling that executes all micro-batches of a layer before moving to the next layer, and overlaps optimization steps with forward passes of next iterations to mitigate I/O bottlenecks.

Result: Achieves 1.96x throughput improvement on 1 GPU and 1.93x on 4 GPUs for GPT-65B, and 2.53x on 1 GPU for GPT-175B compared to ZeRO-Infinity.

Conclusion: GreedySnake's vertical scheduling approach brings SSD-offloaded training closer to ideal roofline model predictions and enables more efficient large-scale LLM training.
...
TL;DR: This thesis develops novel Latent Space Model approaches for unified graph representation learning of static and dynamic networks, focusing on structural-aware embeddings that capture network characteristics without heuristic post-processing.


<details>
  <summary>Details</summary>
Motivation: To create comprehensive and powerful unified network embeddings that can characterize network structures and handle diverse graph analysis tasks, eliminating the need for heuristic multi-stage processes.

Method: Focuses on Latent Space Models, particularly the Latent Distance Model, to develop structural-aware network representations that capture homophily, transitivity, and balance theory characteristics.

Result: Develops approaches for hierarchical structure expression, community characterization, extreme profile identification, and impact dynamics quantification in temporal networks through unified learning processes.

Conclusion: The thesis presents a journey toward unified network embeddings that are both comprehensive and powerful, capable of characterizing diverse network structures and handling various graph analysis tasks without requiring heuristic post-processing steps.
...
TL;DR: SRPL framework improves safety-performance tradeoff in real-world autonomous driving, showing significant gains in success rate and cost reduction across Waymo and NuPlan datasets, with better robustness to noise and cross-dataset generalization.


<details>
  <summary>Details</summary>
Motivation: SafeRL faces fundamental tension between safety requirements and driving efficiency - conservative policies limit performance while aggressive exploration risks safety violations. Need to test if SRPL's predictive safety representations work in real-world autonomous driving scenarios beyond controlled environments.

Method: Systematic experiments on real-world datasets (Waymo Open Motion Dataset and NuPlan) evaluating SRPL framework that equips agents with predictive model of future constraint violations. Tests effectiveness across different policy optimizers and dataset distributions, including robustness to observation noise and zero-shot cross-dataset evaluation.
...
Conclusion: Predictive safety representations in SRPL framework demonstrate potential to strengthen SafeRL for autonomous driving in real-world scenarios, improving safety-performance tradeoff, robustness to noise, and cross-dataset generalization, though effectiveness depends on implementation details.
...
### [180] [Sharing Knowledge without Sharing Data: Stitches can improve ensembles of disjointly trained models](https://arxiv.org/abs/2512.17592)
...
<details>
  <summary>Details</summary>
Motivation: Deep learning requires large datasets but data is often fragmented across parties (e.g., medical domain) and cannot be shared. Federated learning requires synchronous training, which is impractical. Need asynchronous collaboration where only trained models are shared.

Method: Propose using stitching layers to combine intermediate representations of individually trained models. Models are trained separately on each party's data, then combined asynchronously via stitching layers placed at appropriate network positions.

Result: Individual training on single party's data performs well on that party's data but poorly on others. Ensembles generalize better but hurt performance on each party's own data. Stitching layers recover competitive performance on each party's data while maintaining improved generalization.

Conclusion: Asynchronous collaboration with stitching layers yields competitive results, enabling model combination without synchronous training or data sharing, addressing privacy and practical constraints in fragmented data settings.
...
### [181] [A Unified Representation of Neural Networks Architectures](https://arxiv.org/abs/2512.17593)
*Christophe Prieur,Mircea Lazar,Bogdan Robu*

Main category: cs.LG

TL;DR: The paper introduces DiPaNet, a unified framework representing neural networks as distributed parameter systems, connecting finite and infinite-dimensional architectures through homogenization/discretization.


<details>
  <summary>Details</summary>
Motivation: To develop a unified mathematical framework that bridges finite neural networks, infinite-width networks, and neural ODEs by considering the continuum limit of neural architectures.

Method: 1) Derive integral infinite-width neural representations for single-hidden-layer networks; 2) Extend to deep residual CNNs; 3) Formalize approximation errors between neural ODEs and deep residual NNs via discretization; 4) Merge approaches into unified DiPaNet representation.

Result: Established that most existing finite and infinite-dimensional NN architectures are related through homogenization/discretization with the DiPaNet representation, providing deterministic analysis for general uniformly continuous matrix weight functions.

Conclusion: DiPaNet offers a comprehensive framework connecting various neural network architectures, with potential for further generalizations and applications beyond neural fields.
...
### [182] [A Systems-Theoretic View on the Convergence of Algorithms under Disturbances](https://arxiv.org/abs/2512.17598)
*Guner Dilsad Er,Sebastian Trimpe,Michael Muehlebach*

Main category: cs.LG

TL;DR: The paper develops a framework for analyzing algorithm convergence in noisy/disturbed environments by extending isolated convergence guarantees to account for disturbances, interconnections, and noise.


<details>
  <summary>Details</summary>
Motivation: Algorithms increasingly operate in complex physical, social, and engineering systems where they are exposed to disturbances, noise, and interconnections with other dynamical systems, requiring analysis beyond isolated operation assumptions.

Method: Extends known convergence guarantees of algorithms operating in isolation by leveraging converse Lyapunov theorems to derive stability bounds and convergence rates in the presence of disturbances.

Result: Derives key inequalities quantifying the impact of disturbances and demonstrates application to distributed learning communication constraints, machine learning generalization sensitivity, and privacy-preserving noise injection.

Conclusion: Provides a unifying tool for algorithm analysis in noisy/disturbed environments, enabling systematic assessment of disturbance effects on algorithmic performance across various applications.
...
### [183] [More Consistent Accuracy PINN via Alternating Easy-Hard Training](https://arxiv.org/abs/2512.17607)
*Zhaoqian Gao,Min Yanga*

Main category: cs.LG

TL;DR: A hybrid training strategy combining hard and easy prioritization for PINNs achieves consistently high accuracy (10^-5 to 10^-6 relative L2 errors) across diverse PDEs with steep gradients, nonlinearity, and high dimensionality.


<details>
  <summary>Details</summary>
Motivation: Current PINN training strategies (hard vs easy prioritization) show trade-offs and inconsistent performance across different PDE types, creating a need for more robust training approaches.
...
Result: Achieved consistently high accuracy with relative L2 errors mostly in O(10^-5) to O(10^-6) range, significantly surpassing baseline methods, with greater reliability across diverse PDE problems.

Conclusion: The hybrid training strategy enhances PINN performance and robustness, providing new insights for designing effective training approaches for physics-informed neural networks.
...
### [184] [SCOPE: Sequential Causal Optimization of Process Interventions](https://arxiv.org/abs/2512.17629)
*Jakob De Moor,Hans Weytjens,Johannes De Smedt,Jochen De Weerdt*

Main category: cs.LG

TL;DR: SCOPE is a Prescriptive Process Monitoring approach that learns aligned sequential intervention recommendations using backward induction and causal learners, outperforming existing methods in optimizing KPIs.


<details>
  <summary>Details</summary>
Motivation: Existing PresPM approaches fail to handle realistic scenarios where organizations need to align sequences of interventions to jointly steer case outcomes. Current methods either focus on single interventions, treat multiple interventions independently ignoring temporal interactions, or rely on simulation/data augmentation for RL training which creates reality gaps and bias.

Method: SCOPE employs backward induction to estimate the effect of each candidate intervention action, propagating its impact from the final decision point back to the first. It leverages causal learners to utilize observational data directly, avoiding the need for constructing process approximations for reinforcement learning.

Result: Experiments on both an existing synthetic dataset and a new semi-synthetic dataset show that SCOPE consistently outperforms state-of-the-art PresPM techniques in optimizing the KPI. The novel semi-synthetic setup, based on a real-life event log, is provided as a reusable benchmark.

Conclusion: SCOPE addresses the limitations of existing PresPM approaches by providing a method for learning aligned sequential intervention recommendations that directly uses observational data without simulation approximations, demonstrating superior performance in KPI optimization.
...
### [185] [Trust-Region Adaptive Policy Optimization](https://arxiv.org/abs/2512.17636)
*Mingyu Su,Jian Guan,Yuxian Gu,Minlie Huang,Hongning Wang*

Main category: cs.LG

TL;DR: TRAPO is a hybrid framework that interleaves SFT and RL within each training instance, using trust-region SFT and adaptive prefix-selection to overcome limitations of traditional two-stage pipelines for improving LLM reasoning.


<details>
  <summary>Details</summary>
Motivation: The dominant two-stage pipeline (SFT then RL) suffers from inconsistency: SFT enforces rigid imitation that suppresses exploration and causes forgetting, limiting RL's potential for improvements in complex reasoning tasks.

Method: TRAPO interleaves SFT and RL within each training instance by optimizing SFT loss on expert prefixes and RL loss on model's own completions. It introduces Trust-Region SFT (TrSFT) that minimizes forward KL divergence inside trust region but attenuates outside, shifting toward reverse KL for stable updates. Adaptive prefix-selection allocates expert guidance based on measured utility.
...
Conclusion: TRAPO establishes a strong new paradigm for reasoning-enhanced LLMs by unifying external supervision and self-exploration in a stable, efficient framework.
...
### [186] [Estimating Spatially Resolved Radiation Fields Using Neural Networks](https://arxiv.org/abs/2512.17654)
...
<details>
  <summary>Details</summary>
Motivation: Need for accurate radiation protection dosimetry in medical fields like Interventional Radiology and Cardiology, where scattered radiation fields must be estimated for safety purposes.

Method: Created three synthetic datasets of increasing complexity using Geant4 Monte-Carlo simulations. Evaluated convolutional and fully connected neural network architectures to reconstruct fluence and spectra distributions over spatial domains.
...
Conclusion: Neural networks can effectively estimate scattered radiation field distributions using synthetic training data, with specific architectural choices proving more effective for this medical dosimetry application.
...
TL;DR: The paper introduces a "polyharmonic cascade" architecture using polyharmonic splines derived from random function theory, with a non-gradient training method solving global linear systems per batch for efficient GPU computation.


<details>
  <summary>Details</summary>
Motivation: To create a deep learning architecture that can approximate complex nonlinear functions while preserving global smoothness and maintaining a probabilistic interpretation, addressing limitations of gradient-based methods.

Method: A cascade of polyharmonic spline layers derived from random function theory and indifference principles, trained by solving single global linear systems per batch (instead of gradient descent) for function values at fixed node constellations.

Result: Demonstrates fast learning without overfitting on MNIST dataset, with efficient GPU computation through 2D matrix operations and synchronized layer updates that preserve probabilistic interpretation.

Conclusion: The polyharmonic cascade provides a theoretically grounded alternative to gradient-based deep learning that maintains probabilistic interpretability while achieving practical performance on benchmark tasks.
...
### [188] [You Only Train Once: Differentiable Subset Selection for Omics Data](https://arxiv.org/abs/2512.17678)
...
<details>
  <summary>Details</summary>
Motivation: Existing feature selection approaches for single-cell transcriptomic data operate as multi-stage pipelines or rely on post hoc feature attribution, making selection and prediction weakly coupled. There's a need for a more integrated approach that directly links gene selection with prediction tasks.

Method: YOTO uses an end-to-end differentiable architecture with a closed feedback loop where prediction tasks directly guide gene selection and selected genes shape predictive representations. It enforces sparsity so only selected genes contribute to inference, eliminating need for additional downstream classifiers. Multi-task learning allows shared representations across related objectives.
...
Conclusion: Sparse, end-to-end, multi-task gene subset selection improves predictive performance and yields compact, meaningful gene subsets, advancing biomarker discovery and single-cell analysis by tightly coupling selection and prediction in a single framework.
...
### [189] [Easy Adaptation: An Efficient Task-Specific Knowledge Injection Method for Large Models in Resource-Constrained Environments](https://arxiv.org/abs/2512.17771)
*Dong Chen,Zhengqing Hu,Shixing Zhao,Yibo Guo*

Main category: cs.LG

TL;DR: EA proposes using Specific Small Models to complement Large Models' underfitted data distributions, achieving PEFT-level performance without accessing LM parameters and with minimal resources.


<details>
  <summary>Details</summary>
Motivation: Existing PEFT methods face two challenges: (1) high resource costs despite being more efficient than full fine-tuning, and (2) parameter dependency that's problematic with closed-source LMs accessed only via expensive APIs. Small models can perform well on specific distributions with minimal resources.

Method: Easy Adaptation (EA) designs Specific Small Models (SSMs) to complement the underfitted data distribution for Large Models. Instead of fine-tuning LMs, EA creates specialized small models that work alongside LMs to handle task-specific distributions.

Result: Extensive experiments show that EA matches the performance of PEFT on diverse tasks without accessing LM parameters, and requires only minimal resources.

Conclusion: EA provides a practical alternative to PEFT that overcomes resource constraints and parameter dependency issues, enabling effective LM adaptation in resource-constrained environments and with closed-source models.
...
TL;DR: First theoretical analysis of Federated SARSA (FedSARSA) with linear function approximation, establishing convergence guarantees and complexity bounds for heterogeneous federated reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: There's a lack of theoretical understanding for federated reinforcement learning algorithms, particularly SARSA with linear function approximation in heterogeneous environments. The paper aims to provide the first convergence analysis and complexity bounds for FedSARSA.

Method: Develops a novel theoretical framework for FedSARSA with linear function approximation, including a new exact multi-step error expansion for single-agent SARSA. Analyzes the algorithm with local training and multiple local updates in heterogeneous settings.

Result: Establishes convergence guarantees and provides the first sample and communication complexity bounds for FedSARSA. Shows that FedSARSA achieves linear speed-up with respect to number of agents (up to higher-order Markovian sampling terms) and precisely quantifies the impact of heterogeneity.

Conclusion: FedSARSA with linear function approximation converges in heterogeneous federated settings, achieving efficient communication and computation trade-offs. The theoretical analysis is supported by numerical experiments, and the multi-step error expansion technique is of independent interest for RL theory.
...
TL;DR: A spatially-informed transformer that integrates geostatistical inductive bias into self-attention via learnable covariance kernels, achieving superior spatio-temporal forecasting with well-calibrated uncertainty.


<details>
  <summary>Details</summary>
Motivation: Bridging the gap between classical geostatistics (probabilistic rigor, uncertainty quantification) and deep learning (flexibility, scalability) for high-dimensional spatio-temporal processes. Gaussian processes are computationally prohibitive for massive networks, while transformers lack geometric inductive bias for spatial data.

Method: Hybrid transformer architecture that injects geostatistical inductive bias into self-attention via learnable covariance kernel. Decomposes attention into stationary physical prior (spatial decay) and non-stationary data-driven residual. Enables "Deep Variography" - learning spatial decay parameters end-to-end via backpropagation.

Result: Outperforms state-of-the-art graph neural networks on synthetic Gaussian random fields and real-world traffic benchmarks. Achieves superior predictive accuracy and well-calibrated probabilistic forecasts. Successfully recovers true spatial decay parameters of underlying processes.

Conclusion: Effectively bridges physics-aware modeling and data-driven learning by combining geometric inductive bias with transformer flexibility. Provides both accurate predictions and reliable uncertainty quantification for spatio-temporal processes.
...
### [192] [Mitigating Forgetting in Low Rank Adaptation](https://arxiv.org/abs/2512.17720)
*Joanna Sliwa,Frank Schneider,Philipp Hennig,Jose Miguel Hernandez-Lobato*

Main category: cs.LG

TL;DR: LaLoRA applies Laplace approximation to LoRA weights for parameter-efficient fine-tuning that reduces catastrophic forgetting while maintaining target-domain learning efficiency.


<details>
  <summary>Details</summary>
Motivation: Parameter-efficient fine-tuning methods like LoRA enable fast specialization of large models but often cause catastrophic forgetting of prior domain knowledge. The paper aims to address this forgetting issue while maintaining efficiency.

Method: LaLoRA uses weight-space regularization with Laplace approximation applied specifically to LoRA weights. It estimates parameter confidence through loss landscape curvature and constrains updates in high-curvature directions to preserve prior knowledge.

Result: The method demonstrates improved learning-forgetting trade-off when fine-tuning Llama for mathematical reasoning, with regularization strength providing direct control over this balance. The approach remains lightweight and shows robustness across hyperparameters.

Conclusion: LaLoRA effectively reduces catastrophic forgetting in parameter-efficient fine-tuning by applying Laplace approximation to LoRA weights, enabling better preservation of prior knowledge while maintaining efficient target-domain adaptation.
...
### [193] [Can You Hear Me Now? A Benchmark for Long-Range Graph Propagation](https://arxiv.org/abs/2512.17762)
*Luca Miglior,Matteo Tolloso,Alessio Gravina,Davide Bacciu*

Main category: cs.LG

TL;DR: ECHO is a new benchmark for evaluating GNNs' ability to handle long-range graph propagation, featuring synthetic tasks (shortest paths, node eccentricity, graph diameter) and real-world chemical datasets (partial charges, total energies).


<details>
  <summary>Details</summary>
Motivation: Capturing long-range interactions in graphs is a fundamental unresolved challenge in GNN research, critical for scientific applications, but current methods lack rigorous evaluation standards.

Method: Created ECHO benchmark with three synthetic graph tasks (single-source shortest paths, node eccentricity, graph diameter) on challenging topologies, plus two real-world chemical datasets (ECHO-Charge for atomic partial charges, ECHO-Energy for molecular total energies) with DFT-level reference computations.

Result: Extensive benchmarking of popular GNN architectures reveals clear performance gaps, showing difficulty with true long-range propagation and highlighting design choices that can overcome limitations.
...
### [194] [Calibratable Disambiguation Loss for Multi-Instance Partial-Label Learning](https://arxiv.org/abs/2512.17788)
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-instance partial-label learning (MIPL) approaches suffer from poor calibration, which undermines classifier reliability. MIPL combines challenges from both multi-instance learning (inexact instance supervision) and partial-label learning (inexact label supervision), creating a need for better calibration methods.

Method: Proposes a calibratable disambiguation loss (CDL) with two instantiations: 1) calibrates predictions based on probabilities from candidate label set only, 2) integrates probabilities from both candidate and non-candidate label sets. The loss is plug-and-play and can be incorporated into existing MIPL and PLL frameworks.

Result: Theoretical analysis establishes lower bound and regularization properties of CDL, demonstrating superiority over conventional disambiguation losses. Experimental results on benchmark and real-world datasets confirm significant improvements in both classification accuracy and calibration performance.

Conclusion: The proposed CDL effectively addresses the calibration problem in MIPL while maintaining or improving classification performance, making it a valuable plug-and-play component for existing weakly supervised learning frameworks.
...
### [195] [Weighted Stochastic Differential Equation to Implement Wasserstein-Fisher-Rao Gradient Flow](https://arxiv.org/abs/2512.17878)
*Herlock Rahimi*

Main category: cs.LG

TL;DR: The paper proposes using Wasserstein-Fisher-Rao (WFR) geometries to improve diffusion-based sampling for non-log-concave distributions by incorporating mass reweighting mechanisms through explicit correction terms and Feynman-Kac representation.


<details>
  <summary>Details</summary>
Motivation: Standard score-based diffusion models using Ornstein-Uhlenbeck-type SDEs suffer from exponential deterioration in mixing rates for nonconvex or multimodal target distributions (like double-well potentials), which are common in practical generative modeling tasks. There's a need for sampling schemes that improve exploration beyond classical diffusion dynamics.

Method: The authors leverage information geometry tools to augment diffusion-based samplers with controlled mass reweighting mechanisms, leading to Wasserstein-Fisher-Rao (WFR) geometries. They formulate reweighting through explicit correction terms and implement them via weighted stochastic differential equations using the Feynman-Kac representation.

Result: The study provides a preliminary but rigorous investigation of WFR-based sampling dynamics, clarifying their geometric and operator-theoretic structure as a foundation for future theoretical and algorithmic developments.

Conclusion: WFR-based sampling dynamics offer a promising approach to address the limitations of classical diffusion models for non-log-concave target distributions by combining transport in sample space with vertical reaction dynamics on probability measures, potentially enabling better exploration of multimodal landscapes.
...
TL;DR: ID and text features in sequential recommendation are complementary, and simple ensembling of independently trained ID-based and text-based models outperforms complex fusion methods.


<details>
  <summary>Details</summary>
Motivation: There's a lack of understanding about the complementarity between ID embeddings and modality (text) features in sequential recommendation. Some works replace IDs with modality features, while others use complex fusion strategies, but the fundamental relationship between these two types of features remains unclear.

Method: Proposes a simple ensembling approach that preserves ID-text complementarity through independent training of ID-based and text-based models, then combines them using straightforward ensembling rather than complex fusion architectures.

Result: The proposed method outperforms several competitive sequential recommendation baselines, demonstrating that both ID and text features are necessary for state-of-the-art performance, but complex fusion strategies are not required.

Conclusion: ID and text features in sequential recommendation are complementary, and simple ensembling of independently trained models effectively harnesses this complementarity without needing complex fusion architectures, challenging current assumptions in the field.
...
TL;DR: RRFF-FEM: Regularized random Fourier features with finite element reconstruction for robust operator learning from noisy data, using Student's t-distribution features and frequency-weighted Tikhonov regularization.


<details>
  <summary>Details</summary>
Motivation: Kernel-based operator learning methods can be computationally expensive for large datasets and sensitive to noise, despite offering accurate approximations with theoretical guarantees.

Method: Proposes RRFF-FEM: regularized random Fourier features using multivariate Student's t distributions, coupled with frequency-weighted Tikhonov regularization to suppress high-frequency noise, and finite element reconstruction mapping.

Result: Established theoretical guarantees showing system is well-conditioned when features scale as m log m with training samples; demonstrated robustness to noise and improved performance with reduced training time across multiple PDE benchmarks.

Conclusion: RRFF-FEM provides a computationally efficient, noise-robust operator learning method that maintains competitive accuracy while reducing training time compared to unregularized and kernel-based approaches.
...
### [198] [Packet Detection in a Filter Bank-Based Ultra-Wideband Communication System](https://arxiv.org/abs/2512.17055)
*Brian Nelson,Behrouz Farhang-Boroujeny*

Main category: eess.SP

TL;DR: FBMC-SS UWB packet detector using Rao score test, with single-radio band (SRB) and multi-radio band (MRB) implementations for practical ADC/DAC constraints.


<details>
  <summary>Details</summary>
Motivation: FBMC-SS enables UWB signal processing with moderate-rate parallel processors, avoiding high-rate ADC/DAC requirements. Need practical packet detectors that work within typical 200MHz ADC/DAC sampling rate limits.

Method: Developed packet detector using Rao score test for FBMC-SS signals. Proposed cascade channelizer-based SRB detector, then extended to MRB detector using parallel signal processors at moderate sampling rates to overcome ADC/DAC speed limitations.

Result: SRB and MRB detectors show identical performance in UWB channels. Reliable detection achieved at -40dB SNR with half the preamble length of IEEE802.15.4 standard. Demonstrated with 1.28GHz bandwidth UWB system.

Conclusion: MRB detector provides practical implementation within ADC/DAC constraints while maintaining detection performance. Enables reliable UWB packet detection in harsh environments with minimal preamble overhead.
...
### [199] [Dirichlet Meets Horvitz and Thompson: Estimating Homophily in Large Networks via Sampling](https://arxiv.org/abs/2512.17084)
*Hamed Ajorlou,Gonzalo Mateos,Luana Ruiz*

Main category: eess.SP

TL;DR: Proposes a sampling-based framework using Horvitz-Thompson estimator to estimate graph homophily via Dirichlet energy from partial network observations.


<details>
  <summary>Details</summary>
Motivation: Traditional homophily evaluation requires full network topology and node features, which is impractical for large-scale, dynamic, resource-limited, or privacy-constrained settings.

Method: Uses sampling-based framework with Horvitz-Thompson estimator to estimate Dirichlet energy (Laplacian-based total variation) from partial graph observations, leveraging edge-inclusion probabilities as weights.

Result: Establishes that Dirichlet energy can be consistently estimated from sampled graphs, and experiments on heterophilic benchmark datasets show HT estimators reliably capture homophilic structure from sampled measurements.

Conclusion: Proposed sampling framework enables practical homophily assessment in constrained settings, informing model choices like graph neural networks without requiring full network access.
...
### [200] [Kalman Filter-based Mobile User-RIS Channel Estimation and User Localization](https://arxiv.org/abs/2512.17112)
*Ju Zhuoxuan,Doroslovacki Milos*

Main category: eess.SP

TL;DR: Novel approach using Kalman Filter variants and DSFT interpolation for joint channel estimation and user localization in RIS-assisted wireless systems, with extensions to multi-user scenarios.


<details>
  <summary>Details</summary>
Motivation: Channel estimation and user localization are challenging in harsh environments or signal-blocked areas, especially with mobile users and RIS-assisted systems.

Method: 1) Kalman Filtering algorithms for time-variant channel parameter estimation; 2) Non-Circular Noise Kalman Filter (NCNKF) for non-circular complex noise scenarios; 3) DSFT with interpolation for user localization; 4) Extension from single-user to multi-user systems.

Result: KF achieves lower MSE in channel estimation than other approaches; NCNKF performs better in non-circular noise scenarios; DSFT interpolation outperforms other methods with lower complexity.

Conclusion: The proposed approaches enable significant performance improvements in joint channel estimation and user localization for RIS-assisted wireless systems, validated through numerical comparisons.
...
### [201] [Deep Learning-Enabled Multi-Tag Detection in Ambient Backscatter Communications](https://arxiv.org/abs/2512.17125)
*Talha Akyildiz,Hessam Mahdavifar*

Main category: eess.SP

TL;DR: Deep learning approaches for reliable multi-tag detection in ambient backscatter communications without requiring perfect channel state information.


<details>
  <summary>Details</summary>
Motivation: Ambient backscatter communication enables battery-free connectivity but faces challenges in reliable multi-tag detection due to strong interference, weak signals, large state space, and difficulty obtaining channel state information from uncooperative RF sources.

Method: Two deep learning frameworks: 1) EmbedNet - end-to-end prototypical network mapping covariance features directly to multi-tag states; 2) ChanEstNet - hybrid CNN that estimates effective channel coefficients from pilot symbols and feeds them to conventional LRT for interpretable detection.

Result: Proposed methods substantially reduce bit error rate, closely track the LRT benchmark with perfect CSI, and significantly outperform energy detection baselines, especially as number of tags increases.

Conclusion: Deep learning approaches effectively relax CSI requirements while maintaining modulation-agnostic operation, enabling reliable multi-tag detection in ambient backscatter systems without perfect channel knowledge.
...
### [202] [Deep Reinforcement Learning-Aided Strategies for Big Data Offloading in Vehicular Networks](https://arxiv.org/abs/2512.17133)
*Talha Akyildiz,Hessam Mahdavifar*

Main category: eess.SP

TL;DR: DRL-based data offloading with deduplication in V2V networks reduces upload time and energy consumption by designating leader vehicles to process and remove redundancy before uploading to base stations.


<details>
  <summary>Details</summary>
Motivation: To efficiently upload large data from vehicles to network infrastructure by leveraging V2V links, reducing redundancy through deduplication, and optimizing time/energy consumption in dynamic vehicular environments.

Method: Mathematical framework for V2V networks with leader-follower architecture, optimization problems for minimizing total time and energy consumption, and deep reinforcement learning (DRL) solutions for dynamic network conditions.

Result: DRL application is highly beneficial, and data offloading with deduplication significantly reduces both time and energy consumption compared to alternative approaches.

Conclusion: The proposed DRL-based framework with leader vehicle deduplication effectively optimizes data uploading in dynamic vehicular networks, demonstrating substantial improvements in efficiency metrics.
...
### [203] [BM4D-PC: nonlocal volumetric denoising of principal components of diffusion-weighted MR images](https://arxiv.org/abs/2512.17138)
*Vinicius P. Campos,Diego Szczupak,Tales Santini,Afonso C. Silva,Alessandro Foi,Marcelo A. C. Vieira,Corey A. Baron*

Main category: eess.SP

TL;DR: BM4D-PC: A novel dMRI denoising method that accounts for spatial noise correlation and noise power spectral density using BM4D on principal components, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current dMRI denoising methods don't fully account for spatially correlated noise from different acquisition/reconstruction strategies, requiring a method that handles diverse noise characteristics.

Method: BM4D-PC: Uses BM4D algorithm with full noise statistics (including noise PSD) applied to principal components of DWI datasets after PCA decomposition, with direct noise map and PSD estimation.

Result: BM4D-PC achieved best PSNR, SSIM, and RMSE metrics in in-silico experiments and dramatically enhanced in-vivo image quality, outperforming four existing state-of-the-art methods.

Conclusion: BM4D-PC demonstrates state-of-the-art dMRI denoising across various acquisition strategies and resolutions, potentially advancing neuroscience research.
...
### [204] [Active RIS-Aided Anti-Jamming Wireless Communications: A Stackelberg Game Perspective](https://arxiv.org/abs/2512.17335)
*Xiao Tang,Zhen Ma,Bin Li,Cong Li,Qinghe Du,Dusit Niyato,Zhu Han*

Main category: eess.SP

TL;DR: Active RIS-assisted anti-jamming communications using Stackelberg game framework to counter adaptive jammers, with joint optimization of power, beamformers, and reflection.


<details>
  <summary>Details</summary>
Motivation: Address the significant challenge of adaptive jamming attacks on wireless communications security and reliability, which can optimize their strategies to disrupt legitimate transmissions.

Method: Model the interaction as a Stackelberg game where legitimate user is leader, jammer is follower. Use backward induction to find Stackelberg equilibrium. Formulate bi-level optimization for power, beamformers, and active RIS reflection. Solve via block coordinate descent with convex relaxation and successive convex approximation.

Result: Proposed active RIS-assisted scheme significantly outperforms baseline schemes in enhancing legitimate transmissions and degrading jamming effects across various scenarios.

Conclusion: Combining active RIS technology with strategic game-theoretic framework is effective for anti-jamming communications against adaptive jammers.
...
### [205] [RSMA-Assited and Transceiver-Coordinated ICI Management for MIMO-OFDM System](https://arxiv.org/abs/2512.17171)
*Hengyu Zhang,Xuehan Wang,Xu Shi,Jintao Wang,Zhaohui Yang*

Main category: eess.SP

TL;DR: Proposes RSMA-assisted transceiver-coordinated scheme for ICI management in MIMO-OFDM systems under high mobility, using hybrid SIC with dynamic subcarrier clustering at receiver and matched hybrid precoding with ABC-PSO optimization at transmitter.


<details>
  <summary>Details</summary>
Motivation: High-mobility scenarios in next-generation communications cause Doppler-induced ICI that limits MIMO-OFDM performance. RSMA's superior interference management capabilities can address this challenge.

Method: 1) Receiver: Hybrid SIC architecture with dynamic subcarrier clustering for parallel intra-cluster and serial inter-cluster processing. 2) Transmitter: Matched hybrid precoding via sum-rate maximization solved by ABC-PSO algorithm for analog phase optimization and WMMSE-based digital precoding iteration.

Result: Simulation results show effective ICI suppression and enhanced system capacity with controlled complexity.

Conclusion: The proposed RSMA-assisted transceiver-coordinated scheme successfully addresses Doppler-induced ICI in high-mobility MIMO-OFDM systems through coordinated receiver and transmitter designs.
...
### [206] [Near-Field Position and Orientation Tracking With Hybrid ELAA Architecture](https://arxiv.org/abs/2512.17274)
*Lin Chen,Xiaojun Yuan,Ying-Jun Angela Zhang*

Main category: eess.SP

TL;DR: Proposes a predictive analog combining-assisted extended Kalman filter (PAC-EKF) framework for near-field position and orientation tracking of mobile stations using ELAA base stations with limited RF chains.


<details>
  <summary>Details</summary>
Motivation: ELAA base stations with limited RF chains create analog compression that only provides partial access to measurements, requiring careful design of analog combiners to preserve pose-relevant signal components despite channel uncertainty and hardware constraints.

Method: PAC-EKF framework where analog combiner leverages temporal correlation in MS pose variation to capture most informative signal components predictively. Also develops two methods for designing low-complexity analog combiners based on performance analysis.

Result: Proposed predictive analog combining approach significantly improves tracking accuracy, even with fewer RF chains and lower transmit power. Performance analysis quantifies how analog combiner, array size, SNR, and MS pose influence pose information.

Conclusion: Predictive analog combining effectively addresses the challenge of pose tracking with ELAA base stations under RF chain limitations, providing improved accuracy through temporal correlation exploitation and optimized combiner design.
...
### [207] [Near-Field Multi-User Communications via Polar-Domain Beamfocusing: Analytical Framework and Performance Analysis](https://arxiv.org/abs/2512.17283)
*Lin Chen,Ahmed Elzanaty,Mustafa A. Kishk,Ying-Jun Angela Zhang*

Main category: eess.SP

TL;DR: The paper develops a stochastic geometry framework for near-field multi-user systems using polar-domain beamfocusing, addressing interference analysis challenges with a tractable antenna pattern approximation.


<details>
  <summary>Details</summary>
Motivation: As wireless systems move to higher frequencies with large antenna arrays, near-field propagation becomes dominant. Unlike far-field communication limited to angular beamsteering, near-field enables beamfocusing in both angle and distance (polar domain), offering new spatial multiple access opportunities. However, the coupling between angle and distance in near-field antenna patterns makes interference analysis intractable.

Method: The paper proposes a tractable near-field multi-level antenna pattern (NF-MLAP) approximation to address the intractable interference analysis. This approximation enables computationally efficient expressions and tight upper bounds for key performance metrics. The framework jointly captures near-field propagation characteristics and spatial randomness of user locations.

Result: Analytical and simulation results show that the proposed framework accurately captures performance trends and reveals fundamental trade-offs between hardware configuration (number of antennas and RF chains) and system performance (spatial resource reuse and interference mitigation).

Conclusion: The developed stochastic geometry framework provides an effective analytical tool for near-field multi-user systems with polar-domain beamfocusing, offering insights into hardware-performance trade-offs and enabling performance evaluation of next-generation wireless systems operating in near-field regimes.
...
### [208] [OpenPathNet: An Open-Source RF Multipath Data Generator for AI-Driven Wireless Systems](https://arxiv.org/abs/2512.17286)
*Lizhou Liu,Xiaohui Chen,Wenyi Zhang*

Main category: eess.SP

TL;DR: OpenPathNet is an open-source RF multipath data generator and dataset that provides disaggregated, physically consistent multipath parameters from ray tracing simulations for AI-driven 6G wireless research.


<details>
  <summary>Details</summary>
Motivation: Existing RF datasets like CKMImageNet provide preprocessed, image-based channel representations that conceal fine-grained physical characteristics needed for effective AI modeling in 6G systems.

Method: Uses high-precision ray tracing simulations on real-world environment maps to generate disaggregated multipath parameters (per-path gain, time of arrival, spatial angles) through a modular, parameterized pipeline.

Result: Created an open-source generator and publicly released dataset that enables reproducible generation of multipath data, scalable to new environments and configurations, improving transparency in AI-enabled wireless research.

Conclusion: OpenPathNet provides an extensible testbed for advancing studies on channel modeling, beam prediction, environment-aware communication, and integrated sensing in AI-enabled 6G systems.
...
### [209] [Content-Aware RSMA-Enabled Pinching-Antenna Systems for Latency Optimization in 6G Networks](https://arxiv.org/abs/2512.17332)
*Yu Hua,Yaru Fu,Yalin Liu,Zheng Shi,Kevin Hung*

Main category: eess.SP

TL;DR: Proposes CARP-JO, a content-aware RSMA scheme integrated with Pinching Antenna System for 6G networks to minimize latency by jointly optimizing antenna positioning and transmission parameters.


<details>
  <summary>Details</summary>
Motivation: To enhance multi-user performance in 6G networks by addressing severe path loss in high-frequency bands and mitigating inter-user interference through intelligent physical-layer and communication strategy co-design.

Method: Integrates Pinching Antenna System with novel content-aware RSMA where users requesting same content share unified private streams. Develops CARP-JO algorithm using bisection search, convex programming, and golden-section search to solve joint optimization problem for antenna positioning and RSMA parameters.

Result: CARP-JO consistently outperforms Traditional RSMA, NOMA, and Fixed-antenna systems across diverse network scenarios in terms of latency reduction.

Conclusion: The proposed co-design of physical-layer reconfigurability (PAS) with intelligent communication strategies (content-aware RSMA) effectively minimizes system latency and demonstrates superior performance over existing approaches.
...
### [210] [SCAR: Semantic Cardiac Adversarial Representation via Spatiotemporal Manifold Optimization in ECG](https://arxiv.org/abs/2512.17423)
*Shunbo Jia,Caizhi Liao*

Main category: eess.SP

TL;DR: SCAR is a novel adversarial attack framework for ECG signals that creates clinically plausible perturbations mimicking myocardial infarction features, achieving high transferability while bypassing clinical scrutiny.


<details>
  <summary>Details</summary>
Motivation: Standard adversarial attacks on ECG signals fail due to high inter-subject variability and are easily dismissed by clinicians as technical artifacts, failing to compromise the human-in-the-loop diagnostic pipeline.

Method: SCAR integrates spatiotemporal smoothing (W=25, ~50ms), spectral consistency (<15 Hz), and anatomical amplitude constraints (<0.2 mV) directly into gradient optimization to create clinically plausible perturbations that mimic pathological features.

Result: SCAR achieves 82.46% success on source model and 58.09% transferability on ResNet, specifically forging myocardial infarction features with 90.2% misdiagnosis rate by reconstructing pathological ST-segment elevations.

Conclusion: SCAR serves dual purposes: as a robust data augmentation strategy for adversarial training to defend against clinical attacks, and as educational samples to train clinicians to recognize AI-targeted semantic forgeries.
...
### [211] [Sub-6 GHz Beam-Reconfigurable Microfluidic Antenna Using Graphene Liquid for 5G Network](https://arxiv.org/abs/2512.17434)
*Sasmita Dash,Constantinos Psomas,Ioannis Krikidis*

Main category: eess.SP

TL;DR: A graphene-liquid antenna with 360° beam reconfiguration for sub-6 GHz systems achieves 6 dBi gain at 5.5 GHz with 24% bandwidth and stable performance across six beam directions.


<details>
  <summary>Details</summary>
Motivation: Wireless communication systems need high-performance antennas for better coverage, capacity, and transmission quality. Liquid antennas offer advantages like small size, flexibility, and reconfigurability, with graphene liquid providing superior performance over conventional liquid metals.

Method: The paper presents a graphene-liquid antenna design that uses microfluidic channels for beam reconfiguration. The antenna achieves 360° beam steering by controlling graphene-liquid movement within the channels, specifically targeting six directional beams at 0°, 45°, 135°, 180°, 225°, and 315°.

Result: The antenna achieves 6 dBi gain at 5.5 GHz with a wideband impedance bandwidth of 24%. It successfully reconfigures its main beam into six specified directions while maintaining stable reflection coefficients across all reconfiguration scenarios at 5.5 GHz.

Conclusion: The graphene-liquid-based reconfigurable antenna design shows promise for next-generation wireless communication systems in the sub-6 GHz band, offering flexible beam steering capabilities with stable performance.
...
### [212] [Alternating Direction Method of Multipliers for Nonlinear Matrix Decompositions](https://arxiv.org/abs/2512.17473)
*Atharva Awari,Nicolas Gillis,Arnaud Vandaele*

Main category: eess.SP

TL;DR: ADMM-based algorithm for nonlinear matrix decomposition (NMD) that approximates X ≈ f(WH) with element-wise nonlinear functions, supporting various loss functions and applicable to diverse real-world problems.


<details>
  <summary>Details</summary>
Motivation: Traditional matrix decompositions assume linear relationships, but many real-world applications require modeling nonlinear relationships between latent factors and observed data. Nonlinear matrix decomposition addresses this limitation by incorporating element-wise nonlinear transformations.

Method: Develops an ADMM-based algorithm for solving nonlinear matrix decompositions. The method approximates input matrix X ≈ f(WH) where f is an element-wise nonlinear function, W and H are factor matrices, and r ≪ min(m,n). Supports multiple nonlinear functions (ReLU, square, MinMax) and various loss functions (least squares, ℓ1 norm, KL divergence).

Result: The algorithm demonstrates applicability, efficiency, and adaptability on real-world datasets. It successfully handles different nonlinear models: ReLU for nonnegative sparse data, square for probabilistic circuits, and MinMax for recommender systems.

Conclusion: The proposed ADMM-based framework provides a flexible and efficient solution for nonlinear matrix decomposition, supporting diverse nonlinearities and loss functions, with broad potential applications across various domains.
...
### [213] [Augmented Affine Frequency Division Multiplexing for Both Low PAPR Signaling and Diversity Gain Protection](https://arxiv.org/abs/2512.17679)
*Zhou Lu,Mohammed El-Hajjar,Lie-liang Yang*

Main category: eess.SP

TL;DR: A²FDM is proposed to solve AFDM's PAPR problem by replacing the unnecessary c₂ matrix with a new unitary matrix performing sub-block DFT and symbol mapping, offering interleaved and localized mapping schemes.


<details>
  <summary>Details</summary>
Motivation: AFDM suffers from the same PAPR problem as OFDM and contains an unnecessary matrix (based on parameter c₂) for signaling, which motivates the development of an improved scheme.

Method: Proposed A²FDM replaces the c₂ matrix with a new unitary matrix that performs both sub-block-based DFT and symbol mapping. Two mapping schemes are introduced: Interleaved A²FDM and Localized A²FDM.

Result: A²FDM successfully mitigates the PAPR problem while maintaining achievable diversity gain, even when AFDM operates under undesirable conditions that cause diversity loss.

Conclusion: A²FDM effectively addresses AFDM's PAPR limitations while preserving diversity performance, offering a superior alternative to conventional AFDM.
...
### [214] [Bridging simulation and reality in subsurface radar-based sensing: physics-guided hierarchical domain adaptation with deep adversarial learning](https://arxiv.org/abs/2512.17831)
*Zixin Wang,Ishfaq Aziz,Mohamad Alipour*

Main category: eess.SP

TL;DR: Proposes a physics-guided hierarchical domain adaptation framework with adversarial learning to bridge the domain gap between simulated and real GPR data for robust subsurface material property estimation.


<details>
  <summary>Details</summary>
Motivation: Real-world GPR data is expensive to collect, so synthetic data from physics-based simulations is used for training. However, distribution differences between simulated (source) and real (target) data cause models trained on simulations to underperform in real applications.

Method: A novel physics-guided hierarchical domain adaptation framework with deep adversarial learning. Uses physics-based GPR modeling with FDTD for synthetic data generation, then applies hierarchical domain adaptation to align simulated and real data distributions.

Result: The framework achieves higher correlation coefficients (R), lower bias, and smaller standard deviations in parameter estimation compared to state-of-the-art methods (1D CNN and DANN). Validated through laboratory and field tests for single- and two-layer materials.

Conclusion: The proposed framework effectively bridges the domain gap between simulated and real-world radar signals, enabling efficient and robust subsurface material property retrieval for applications like wildfire risk assessment and precision agriculture.
...
### [215] [Novel Double-Chirp Preamble Design for Multiuser Asynchronous Massive MIMO LoRa Networks](https://arxiv.org/abs/2512.17835)
*The Khai Nguyen,Ebrahim Bedeer*

Main category: eess.SP

TL;DR: Novel double-chirp preamble design and ML-based detection for multiuser asynchronous massive MIMO LoRa networks that solves preamble resemblance problem and supports simultaneous detection of multiple end devices.


<details>
  <summary>Details</summary>
Motivation: Existing LoRa preamble detection only works for single end devices, but real networks need to detect multiple EDs simultaneously. The conventional single-chirp preamble suffers from "preamble resemblance effect" where one ED's preamble can resemble all others, making it impossible to identify which ED transmitted it.

Method: 1) Proposes novel double-chirp preamble design and assignment method to make each ED's preamble unique and recognizable. 2) Derives maximum-likelihood (ML) based detection scheme for the double-chirp preamble. 3) Develops low-complexity recursive DFT calculation technique to reduce computational complexity since DFT needs to be calculated every sampling period.

Result: Simulation shows the proposed design requires only about 2 dB more power to increase from 1 to 15 EDs in Rayleigh fading channel while maintaining the same preamble detection error performance.

Conclusion: The proposed double-chirp preamble design and detection method effectively solves the preamble resemblance problem in multiuser LoRa networks, enabling simultaneous detection of multiple asynchronous EDs with minimal power penalty and practical computational complexity.
...
### [216] [Graph Attention Networks for Detecting Epilepsy from EEG Signals Using Accessible Hardware in Low-Resource Settings](https://arxiv.org/abs/2507.15118)
*Szymon Mazurek,Stephen Moore,Alessandro Crimi*

Main category: eess.SP

TL;DR: Graph-based deep learning framework using Graph Attention Networks (GAT) for epilepsy detection from low-cost EEG hardware, tested in Nigeria and Guinea-Bissau, with focus on accessibility and explainability.


<details>
  <summary>Details</summary>
Motivation: Epilepsy remains under-diagnosed in low-income countries due to scarce neurologists and expensive diagnostic tools. Need for fair, accessible automatic assessment with explainability to identify epilepsy biomarkers.

Method: Model EEG signals as spatio-temporal graphs, classify using Graph Attention Networks (GAT) adapted to analyze edges rather than nodes to emphasize connectivity biomarkers. Designed signal preprocessing for low-fidelity recordings and lightweight GAT architecture trainable on Google Colab and deployable on RaspberryPi devices.

Result: Promising classification performance, outperforming random forest and graph convolutional networks in accuracy and robustness across multiple sessions. Identified specific connections in fronto-temporal region as potential biomarkers.

Conclusion: GATs show potential for providing insightful and scalable diagnostic support for epilepsy in underserved regions, enabling affordable and accessible neurodiagnostic tools.
...
### [217] [Colormap-Enhanced Vision Transformers for MRI-Based Multiclass (4-Class) Alzheimer's Disease Classification](https://arxiv.org/abs/2512.16964)
*Faisal Ahmed*

Main category: eess.IV

TL;DR: PseudoColorViT-Alz uses colormap-enhanced Vision Transformers to improve Alzheimer's disease classification from MRI scans, achieving 99.79% accuracy on OASIS-1 dataset.


<details>
  <summary>Details</summary>
Motivation: MRI is crucial for Alzheimer's diagnosis, but subtle structural variations in brain scans challenge conventional deep learning models to extract discriminative features effectively.

Method: A colormap-enhanced Vision Transformer framework that leverages pseudo-color representations of MRI images, combining colormap transformations with Vision Transformers' global feature learning to amplify anatomical texture and contrast cues.

Result: Achieved state-of-the-art accuracy of 99.79% with AUC of 100% on OASIS-1 dataset (four-class classification), surpassing recent 2024-2025 methods (96.1%-99.68% accuracy).

Conclusion: Pseudo-color augmentation combined with Vision Transformers significantly enhances MRI-based Alzheimer's disease classification, offering a robust and interpretable framework for clinical decision-making and early detection.
...
### [218] [Fetpype: An Open-Source Pipeline for Reproducible Fetal Brain MRI Analysis](https://arxiv.org/abs/2512.17472)
...
TL;DR: Fetpype is an open-source Python library that standardizes preprocessing and analysis of fetal brain MRI data, addressing challenges like motion artifacts and complex multi-step processing.


<details>
  <summary>Details</summary>
Motivation: Fetal brain MRI analysis faces significant challenges including fetal motion, low signal-to-noise ratio, and complex multi-step processing. Existing specialized tools lack integration into robust, reproducible workflows, hindering standardization and adoption by researchers and clinicians.

Method: Fetpype is developed as an open-source Python library that streamlines and standardizes the preprocessing and analysis pipeline for T2-weighted fetal brain MRI data. It integrates various processing steps including motion correction, super-resolution reconstruction, segmentation, and surface extraction into user-friendly workflows.

Result: The library is publicly available on GitHub at https://github.com/fetpype/fetpype, providing researchers and clinicians with a standardized tool for processing fetal brain MRI data from raw images to processed volumes.

Conclusion: Fetpype addresses the lack of standardization in fetal brain MRI analysis by providing an integrated, reproducible workflow that can improve research reproducibility and facilitate adoption of advanced analysis techniques in both research and clinical settings.
...
### [219] [UPMRI: Unsupervised Parallel MRI Reconstruction via Projected Conditional Flow Matching](https://arxiv.org/abs/2512.17493)
*Xinzhe Luo,Yingzhen Li,Chen Qin*

Main category: eess.IV

TL;DR: UPMRI: Unsupervised MRI reconstruction using Projected Conditional Flow Matching that learns from only undersampled k-space data, achieving performance comparable to supervised methods without needing fully sampled ground-truth images.


<details>
  <summary>Details</summary>
Motivation: Supervised deep learning for accelerated MRI reconstruction requires large datasets of fully sampled ground-truth images, which are impractical in clinical settings due to long scan times. Existing self-supervised/unsupervised methods perform inadequately at high acceleration rates.
...
### [220] [Resource-efficient medical image classification for edge devices](https://arxiv.org/abs/2512.17515)
*Mahsa Lavaei,Zahra Abadi,Salar Beigzad,Alireza Maleki*

Main category: eess.IV

TL;DR: This paper explores model quantization techniques to enable efficient medical image classification on resource-constrained edge devices while maintaining diagnostic accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for medical image classification face deployment challenges on edge devices due to computational and memory limitations, especially in resource-constrained healthcare settings.

Method: The research employs model quantization techniques, specifically optimizing quantization-aware training (QAT) and post-training quantization (PTQ) methods tailored for edge devices.

Result: Quantized models achieve substantial reductions in model size and inference latency while maintaining clinically acceptable diagnostic accuracy, enabling real-time processing on edge hardware.

Conclusion: Model quantization provides a practical pathway for deploying AI-driven medical diagnostics in remote and resource-limited settings, enhancing healthcare technology accessibility and scalability.
...
### [221] [A 28nm 0.22 μJ/token memory-compute-intensity-aware CNN-Transformer accelerator with hybrid-attention-based layer-fusion and cascaded pruning for semanticsegmentation](https://arxiv.org/abs/2512.17555)
...
<details>
  <summary>Details</summary>
Motivation: To develop an energy-efficient hardware accelerator for semantic segmentation tasks that combines CNN and Transformer architectures, addressing the high computational demands of these models.

Method: Uses a hybrid attention unit, layer-fusion scheduler, and cascaded feature-map pruner in a 28nm chip design to optimize energy efficiency for CNN-Transformer models.

Result: Achieves 3.86-to-10.91x energy reduction over previous designs with peak energy efficiency of 52.90TOPS/W (INT8) in a 13.93mm2 chip area.

Conclusion: The proposed CNN-Transformer accelerator demonstrates significant energy efficiency improvements for semantic segmentation through architectural innovations in attention mechanisms, scheduling, and pruning.
...
### [222] [SkinGenBench: Generative Model and Preprocessing Effects for Synthetic Dermoscopic Augmentation in Melanoma Diagnosis](https://arxiv.org/abs/2512.17585)
*N. A. Adarsh Pritam,Jeba Shiney O,Sanyam Jain*

Main category: eess.IV

TL;DR: SkinGenBench evaluates how preprocessing complexity and generative model choice affect synthetic dermoscopic image quality and downstream melanoma diagnosis performance, finding that model architecture matters more than preprocessing.


<details>
  <summary>Details</summary>
Motivation: To systematically investigate the interaction between preprocessing complexity and generative model choice for synthetic dermoscopic image augmentation, and understand their impact on downstream melanoma diagnosis performance.

Method: Created SkinGenBench benchmark using 14,116 dermoscopic images from HAM10000 and MILK10K across five lesion classes. Evaluated two generative paradigms (StyleGAN2-ADA and DDPMs) under basic geometric augmentation vs. advanced artifact removal pipelines. Assessed synthetic images using perceptual metrics (FID, KID, IS), feature space analysis, and impact on five downstream classifiers.

Result: StyleGAN2-ADA outperformed DDPMs, achieving lower FID (~65.5) and KID (~0.05) with better alignment to real data distributions. Advanced artifact removal provided only marginal improvements. Synthetic data augmentation substantially improved melanoma detection with 8-15% absolute gains in F1-score, with ViT-B/16 achieving F1~0.88 and ROC-AUC~0.98 (14% improvement over baselines).

Conclusion: Generative architecture choice has stronger influence on image fidelity and diagnostic utility than preprocessing complexity. Synthetic data augmentation significantly improves melanoma detection performance, suggesting careful model selection is more important than complex preprocessing for medical imaging applications.
...
### [223] [Breast Cancer Neoadjuvant Chemotherapy Treatment Response Prediction Using Aligned Longitudinal MRI and Clinical Data](https://arxiv.org/abs/2512.17759)
*Rahul Ravi,Ruizhe Li,Tarek Abdelfatah,Stephen Chan,Xin Chen*

Main category: eess.IV

TL;DR: ML framework using longitudinal CE-MRI and clinical data predicts breast cancer treatment response (PCR) and 5-year relapse-free survival (RFS), with image registration-based feature extraction showing significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: To develop accurate machine learning models for predicting neoadjuvant chemotherapy response in breast cancer patients using longitudinal imaging data, addressing the need for personalized treatment planning and outcome prediction.

Method: Framework includes tumour segmentation, image registration, feature extraction (radiomics + 3 deep learning models), feature selection, and ML modeling. Image registration enables comparison of features from original tumour site across time points to monitor intratumor changes during NACT.

Result: Image registration-based feature extraction consistently improved predictive models. Best performance: logistic regression with radiomic features achieved AUC 0.88/accuracy 0.85 for PCR classification, and AUC 0.78/accuracy 0.72 for RFS classification.

Conclusion: Image registration significantly improves longitudinal feature learning for PCR and RFS prediction. Radiomics feature extractor outperforms pre-trained deep learning models with higher performance and better interpretability.
...
### [224] [MedNeXt-v2: Scaling 3D ConvNeXts for Large-Scale Supervised Representation Learning in Medical Image Segmentation](https://arxiv.org/abs/2512.17774)
*Saikat Roy,Yannick Kirchhoff,Constantin Ulrich,Maximillian Rokuss,Tassilo Wald,Fabian Isensee,Klaus Maier-Hein*

Main category: eess.IV

TL;DR: MedNeXt-v2 is a compound-scaled 3D ConvNeXt architecture that achieves state-of-the-art performance in 3D medical image segmentation through improved backbone design and large-scale supervised pretraining on 18k CT volumes.


<details>
  <summary>Details</summary>
Motivation: Existing large-scale supervised pretraining efforts focus primarily on increasing dataset size but overlook whether backbone networks are effective representation learners at scale. Current backbones used in pretraining pipelines are often suboptimal.

Method: Revisits ConvNeXt-based architectures for volumetric segmentation, introduces MedNeXt-v2 with improved micro-architecture (3D Global Response Normalization) and compound scaling (depth, width, context). Comprehensive backbone benchmarking before scaling shows stronger from-scratch performance predicts better downstream results after pretraining.

Result: State-of-the-art performance when fine-tuning across six challenging CT and MR benchmarks (144 structures), consistently outperforming seven publicly released pretrained models. Key findings: stronger backbones yield better results, representation scaling disproportionately benefits pathological segmentation, modality-specific pretraining offers negligible benefit with full finetuning.

Conclusion: MedNeXt-v2 establishes itself as a strong backbone for large-scale supervised representation learning in 3D medical image segmentation, with code and pretrained models integrated into the official nnUNet repository.
...
### [225] [Hermitian Hulls of Rational Algebraic Geometry Codes and Applications in Quantum Codes](https://arxiv.org/abs/2512.17128)
*Lin Sok,Martianus Frederic Ezerman,Ling San*

Main category: cs.IT

TL;DR: The paper presents a method to construct linear codes with designed Hermitian hull dimensions using algebraic geometry codes, which enables the creation of new families of MDS entanglement-assisted quantum error-correcting codes with excellent parameters.


<details>
  <summary>Details</summary>
Motivation: There is growing interest in hulls of linear codes, particularly Hermitian hulls (less studied than Euclidean hulls). Such codes are crucial for designing entanglement-assisted quantum error-correcting codes (EAQECCs), which require systematic determination of hull dimensions and dual distances.

Method: Uses tools from algebraic function fields of one variable to efficiently determine lower bounds on Hermitian hull dimensions of generalized rational algebraic geometry (AG) codes. Identifies families where hull dimensions can be well estimated by lower bounds. Selects evaluation points where residues of Weil differential associated with Hermitian dual code have easily verifiable properties. Constructs codes with designed Hermitian hull dimensions based on Reed-Solomon codes and their generalizations.

Result: Successfully constructs codes with designed Hermitian hull dimensions. Using the Hermitian method on these MDS codes yields two families of MDS entanglement-assisted quantum error-correcting codes. Confirms that the quantum codes from these families have excellent new parameters.

Conclusion: The approach provides an effective method to construct linear codes with controlled Hermitian hull dimensions, enabling the design of new families of MDS entanglement-assisted quantum error-correcting codes with superior parameters that were previously unavailable.
...
### [226] [Quasi-recursive MDS Matrices over Galois Rings](https://arxiv.org/abs/2512.17256)
*Shakir Ali,Atif Ahmad Khan,Abhishek Kesarwani,Susanta Samanta*

Main category: cs.IT

TL;DR: This paper develops construction methods for quasi-recursive MDS matrices over Galois rings using skew polynomial rings, extending known results to non-commutative settings with applications to cryptographic diffusion layers.


<details>
  <summary>Details</summary>
Motivation: The motivation is to construct efficient diffusion layers for cryptographic primitives by extending the family of available MDS matrices. Current constructions are limited to commutative settings, and there's a need for more flexible matrices over Galois rings, particularly for practical implementations over finite fields like 𝔽₂ᵐ.

Method: The method uses skew polynomial rings GR(pˢ, pˢᵐ)[X;σ] to construct companion matrices that generate quasi-recursive MDS matrices. The authors establish three criteria: two for characterizing polynomials yielding recursive MDS matrices (generalizing existing results), and one based on right roots of associated Wedderburn polynomials. These criteria are then used to develop methods for constructing appropriate skew polynomials.

Result: The paper successfully extends known constructions to non-commutative settings, significantly enlarges the family of available quasi-recursive MDS matrices, and provides practical construction methods. The results are particularly applicable when s=1 and p=2 (over 𝔽₂ᵐ), which is most relevant for real-world cryptographic implementations.

Conclusion: The framework provides a comprehensive approach to constructing quasi-recursive MDS matrices over Galois rings using skew polynomial theory, offering more flexibility and a larger family of matrices for cryptographic applications, especially for efficient diffusion layer design in practical implementations.
...
### [227] [A distance-free approach to generalized weights](https://arxiv.org/abs/2512.17542)
*Andrea Di Giusto,Elisa Gorla,Alberto Ravagnani*

Main category: cs.IT

TL;DR: A unified theory of generalized weights for linear codes with arbitrary distance metrics, using test families instead of supports/anticodes, with applications to Hamming, rank, and sum-rank metrics.


<details>
  <summary>Details</summary>
Motivation: To create a general framework for defining and studying generalized weights across different distance metrics (Hamming, rank, sum-rank) without relying on metric-specific concepts like supports or anticodes.

Method: Define generalized weights via intersections of codes with a chosen "test family" of spaces. Prove general properties like monotonicity and duality. Apply framework to specific metrics by selecting appropriate test families.

Result: Proved generalized weights are weakly increasing with certain subsequences strictly increasing. Established duality theorem similar to Wei's. Recovered known results for Hamming/rank metrics. Proposed new test family for sum-rank metrics extending duality results to codes with Hamming components.

Conclusion: The test family approach provides a unified theory for generalized weights across different metrics, allowing recovery of known results and extension to new settings like sum-rank metrics with Hamming components.
...
### [228] [Locally-APN Binomials with Low Boomerang Uniformity in Odd Characteristic](https://arxiv.org/abs/2512.17603)
*Namhun Koo,Soonhak Kwon,Minwoo Ko,Byunguk Kim*

Main category: cs.IT

TL;DR: The paper extends previous results on locally-APN functions with low boomerang uniformity, providing conditions under which F_r(x) = x^r + x^{r+(q-1)/2} over F_q maintains these properties, and analyzes differential spectra for specific cases.


<details>
  <summary>Details</summary>
Motivation: Previous studies showed that when q ≡ 3 (mod 4), the function F_r(x) is locally-APN with boomerang uniformity ≤ 2. The authors aim to extend these results to more general conditions and study specific cases to better understand the cryptographic properties of these functions.

Method: The authors establish a sufficient condition involving character sums and equation solutions: if there is at most one x ∈ F_q with χ(x)=χ(x+1)=1 satisfying (x+1)^r - x^r = b for all b ∈ F_q* and gcd(r,q-1) | 2, then F_r is locally-APN with boomerang uniformity ≤ 2. They then specifically analyze the differential spectra of F_3 and F_{(2q-1)/3}, and the boomerang spectrum of F_2 when p=3.

Result: The paper proves that under the given conditions, F_r maintains locally-APN properties with boomerang uniformity at most 2. It provides detailed analysis of differential spectra for specific exponent choices, contributing to understanding the cryptographic security properties of these functions.

Conclusion: The research extends the known class of functions with good cryptographic properties (locally-APN with low boomerang uniformity) and provides concrete analysis for specific parameter choices, advancing the understanding of these functions' security characteristics in cryptographic applications.
...
### [229] [Iterative Gaussian Approximation for Random Spreading Unsourced Random Access](https://arxiv.org/abs/2512.17628)
*Liandong Hu,Jian Dang,Zaichen Zhang*

Main category: cs.IT

TL;DR: Proposes an iterative Gaussian approximation decoder for random spreading unsourced random access, achieving good performance with few iterations.


<details>
  <summary>Details</summary>
Motivation: Massive machine-type communications need efficient connectivity solutions. Unsourced random access offers high spectral/energy efficiency, and random spreading provides strong anti-interference capabilities through spectrum spreading gain.

Method: Develops an iterative Gaussian approximation decoder for random spreading URA categories. The receiver iterates extrinsic and intrinsic soft information to enhance decoding performance, requiring only a few iterations to converge.

Result: Numerical results validate the decoder's effectiveness in terms of performance and robustness. The approach approaches theoretical performance limits in scenarios with few active users.

Conclusion: The proposed iterative Gaussian approximation decoder provides an effective universal solution for random spreading unsourced random access, offering good performance with computational efficiency.
