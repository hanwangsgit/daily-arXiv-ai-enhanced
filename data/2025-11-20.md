...
Method: Extracts motion embeddings from source videos via condition inversion, applies them to rendered frames of static target shapes, and uses resulting videos to supervise dynamic 3D Gaussian Splatting reconstruction. Introduces anchor-based view-aware motion embedding mechanism for cross-view consistency and accelerated convergence, plus robust 4D reconstruction pipeline.

Result: Establishes first benchmark for semantic 3D motion transfer and demonstrates superior motion fidelity and structural consistency compared to adapted baselines.

Conclusion: The approach successfully enables semantic 3D motion transfer with high fidelity and consistency, providing a new benchmark in the field.
...
Method: Benchmarked 12 architectures (CNNs, vision transformers, state-space mamba) on DC1000 dataset under identical configurations, including VMUnet, MambaUNet, and DoubleU-Net.

Result: DoubleU-Net achieved highest dice coefficient (0.7345), mIoU (0.5978), and precision (0.8145), with top 3 performers all being CNN-based architectures.

Conclusion: Architecture-task alignment is more important than model complexity for medical image segmentation; CNNs outperformed transformers and Mamba despite their theoretical advantages.
...
TL;DR: BR-DF is a novel geometric representation for CAD B-Rep models using volumetric distance functions, enabling 100% successful conversion to watertight B-Rep models through an extended Marching Cubes algorithm.


<details>
  <summary>Details</summary>
Motivation: To create a reliable CAD B-Rep representation that guarantees successful model generation without failures, addressing limitations in existing methods.

Method: Encodes CAD surface mesh geometry as signed distance functions (SDF) and topological elements (vertices, edges, faces) as per-face unsigned distance functions (UDFs), using a multi-branch latent diffusion model with 3D U-Net backbone for joint generation.
...
Conclusion: BR-DF provides a robust geometric representation that ensures reliable conversion to CAD B-Rep models, making it a promising approach for CAD generation tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D scene synthesis methods either overlook scene graph structure (limiting coherence) or require inconvenient user-provided semantic graphs/ground-truth relationships. There's a need for efficient models deployable on resource-constrained devices like XR glasses.
...
Conclusion: GeoSceneGraph provides an effective approach for text-to-3D scene synthesis that captures scene structure without requiring predefined relationships, making it suitable for deployment on resource-constrained devices.
...
TL;DR: Unsupervised bidirectional MRI synthesizer that converts between High-Field and Ultra-Low Field MRIs using physics-inspired contrast modeling and implicit neural representations.


<details>
  <summary>Details</summary>
Motivation: To develop an MRI synthesis model that leverages the physical principles behind contrast changes between different field strengths, enabling bidirectional conversion without requiring paired training data.

Method: Forward model simulates HF to ULF transformation by estimating tissue-type SNR values based on target contrast. For super-resolution (ULF to HF), uses Implicit Neural Representation network to predict tissue segmentations and image intensity simultaneously without observed HF data.
...
Conclusion: The physics-inspired approach enables effective bidirectional MRI synthesis between different field strengths, improving tissue contrast and showing robustness to parameter variations.
...
TL;DR: I-Mix2Mix is a framework that distills 2D diffusion model editing capabilities into a multi-view diffusion model for consistent multi-view image editing from sparse input views.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with multi-view image editing from sparse views, often producing artifacts and incoherent edits when modifying scenes according to textual instructions.

Method: Proposes replacing neural field consolidator in SDS with multi-view diffusion student, with incremental student updates, specialized teacher noise scheduler, and attention modification for cross-view coherence.
...
Conclusion: I-Mix2Mix effectively addresses multi-view editing challenges by leveraging data-driven 3D priors and novel distillation techniques for consistent cross-view results.
...
<details>
  <summary>Details</summary>
Motivation: Current vision-language models in dermatology face limitations due to data heterogeneity, lack of grounded diagnostic rationales, and poor scalability/generalization from small annotated datasets to large sparse ones.

Method: 1) Textbook-based reasoning generator creates hierarchy-aware differential diagnosis trajectories; 2) Supervised fine-tuning with these trajectories; 3) Novel RL paradigm incorporating disease hierarchy to transfer reasoning to sparse data.

Result: Extensive experiments show SkinR1 achieves superior diagnostic accuracy on multiple dermatology datasets, with ablation studies confirming the importance of the reasoning foundation from SFT.

Conclusion: SkinR1 successfully addresses key limitations in dermatological VLMs through its unified framework combining textbook reasoning and hierarchical RL, demonstrating improved trustworthiness and clinical utility.
...
TL;DR: FarSLIP is a fine-grained aligned remote sensing language-image pretraining framework that addresses CLIP's limited spatial awareness in RS domain through patch-to-patch distillation and CLS token-based region-category alignment, achieving SOTA performance on RS tasks.


<details>
  <summary>Details</summary>
Motivation: Current RS-specific CLIP variants inherit limited spatial awareness due to underutilized object-level supervision in datasets and performance degradation when applying general domain region-text alignment methods directly to RS data.

Method: Constructed MGRS-200k dataset with multi-granularity RS image-text data, then developed FarSLIP framework using patch-to-patch distillation for local-global visual alignment and CLS token-based region-category alignment instead of explicit patch-level alignment.
...
Conclusion: The proposed FarSLIP framework effectively enhances spatial awareness in RS vision-language models through better dataset construction and alignment strategies that preserve semantic coherence while improving feature discriminability.
...
Method: Developed nnMIL framework with random sampling at patch and feature levels, enabling large-batch optimization, task-aware sampling, and efficient training. Uses lightweight aggregator with sliding-window inference for ensemble predictions and uncertainty estimation.

Result: Outperformed existing MIL methods across 40,000 WSIs, 35 clinical tasks, and four pathology foundation models for disease diagnosis, histologic subtyping, biomarker detection, and pan-cancer prognosis. Demonstrated strong cross-model generalization, reliable uncertainty quantification, and robust survival stratification.
...
TL;DR: X-WIN is a novel CXR world model that distills 3D volumetric knowledge from CT scans to improve 2D chest X-ray analysis by predicting projections in latent space.


<details>
  <summary>Details</summary>
Motivation: Chest X-rays are limited by structural superposition as 2D projectional images, failing to capture 3D anatomies, which makes representation learning and disease diagnosis challenging.

Method: Proposes X-WIN model that learns to predict 2D CT projections in latent space, uses affinity-guided contrastive alignment loss, incorporates real CXRs via masked image modeling, and employs domain classifier for representation alignment.

Result: X-WIN outperforms existing foundation models on diverse downstream tasks using linear probing and few-shot fine-tuning, and demonstrates ability to render 2D projections for 3D CT volume reconstruction.

Conclusion: The proposed world model successfully internalizes 3D anatomical knowledge to enhance CXR analysis and shows strong performance across various medical imaging tasks.
...
TL;DR: CPSL is a compact 2.5D video representation that enables volumetric video experiences from 2D content using depth-guided scene layers with soft alpha bands and edge-depth caching for efficient novel-view synthesis.
...
Method: Decomposes frames into geometry-consistent layers using depth and content saliency, with soft alpha bands and edge-depth cache for occlusion ordering. Uses depth-weighted warping and alpha compositing for novel-view synthesis without 3D reconstruction.

Result: Achieves superior perceptual quality and boundary fidelity compared to layer-based and neural-field baselines while reducing storage and rendering costs by several folds.
...
TL;DR: Introduces a benchmark for long-term periodic human workflows with low-contrast patterns, featuring 580 multimodal sequences and three evaluation tasks, along with a training-free baseline method that outperforms existing approaches.


<details>
  <summary>Details</summary>
Motivation: Long-term periodic workflows with low-contrast patterns are underexplored compared to short-term periodic activities, despite their importance in manufacturing, sports, and daily life applications.

Method: Proposes a lightweight, training-free baseline for modeling diverse periodic workflow patterns and introduces a benchmark with 580 multimodal human activity sequences supporting three evaluation tasks.

Result: The benchmark presents significant challenges to existing methods, while the proposed baseline outperforms competing methods by a substantial margin in all evaluation tasks and demonstrates deployment advantages comparable to supervised approaches.

Conclusion: The work successfully bridges the gap in long-term periodic workflow analysis and provides a strong baseline that eliminates the need for annotation and retraining while achieving performance on par with supervised methods.
...
TL;DR: A low-cost LED Clock method for millisecond-level temporal synchronization of multi-view video streams across heterogeneous camera systems, including RGB and IR modalities.


<details>
  <summary>Details</summary>
Motivation: Synchronizing multiple cameras is challenging in heterogeneous setups without hardware synchronization capabilities, especially in real-world environments where controlled conditions are not feasible.

Method: Uses a custom-built LED Clock that encodes time through red and infrared LEDs, allowing visual decoding of exposure window (start and end times) from recorded frames.

Result: Achieves 1.34 ms RMSE residual error, outperforms light-, audio-, and timecode-based synchronization approaches, and improves downstream computer vision tasks like multi-view pose estimation and 3D reconstruction.

Conclusion: The solution simplifies synchronization pipelines and expands access to advanced vision-based sensing in unconstrained environments, including industrial and clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: Address high energy consumption and environmental impact in laser cutting, particularly the lack of adaptive control and open-loop nature of CO2 laser suction pumps.
...
Conclusion: The integrated deep learning approach significantly contributes to sustainable development in manufacturing by achieving substantial energy savings in laser cutting processes.
...
TL;DR: EGSA improves transparent object perception using edge-guided fusion and progressive training, achieving better depth accuracy while maintaining segmentation performance.


<details>
  <summary>Details</summary>
Motivation: Transparent objects challenge computer vision by confounding depth estimation and semantic segmentation, with existing multi-task approaches suffering from negative cross-task interactions.

Method: Edge-Guided Spatial Attention (EGSA) fusion mechanism incorporates boundary information between semantic and geometric features, plus multi-modal progressive training from RGB to depth-derived edges.

Result: EGSA consistently improved depth accuracy over state-of-the-art MODEST on Syn-TODD and ClearPose benchmarks, with largest improvements in transparent regions, while preserving competitive segmentation.
...
TL;DR: Proposes a feature-only knowledge distillation framework that uses feature-based losses without logit-based losses, achieving state-of-the-art performance with up to 15% accuracy improvement.


<details>
  <summary>Details</summary>
Motivation: Current KD methods rely on both logit-based and feature-based losses, but the authors aim to develop a framework using only feature-based losses for more effective knowledge transfer.

Method: Feature KD framework using only feature-based losses, with a knowledge quality metric to identify the most effective teacher layers for distillation based on latent representation geometry.
...
Conclusion: Feature-only knowledge distillation with proper layer selection can significantly outperform traditional KD methods that use both feature and logit-based losses.
...
TL;DR: Kandinsky 5.0 is a family of foundation models for high-resolution image and 10-second video synthesis, featuring three model line-ups with different parameter sizes and capabilities, supported by comprehensive data curation and multi-stage training.
...
Conclusion: Kandinsky 5.0 substantially advances the development and accessibility of high-quality generative models for the research community through its open-source release and comprehensive framework that can be adapted for a wide range of generative applications.
...
TL;DR: FinCriticalED is a visual benchmark for evaluating OCR and vision language models on financial documents at the fact level, focusing on detecting critical numerical and temporal errors that traditional metrics miss.


<details>
  <summary>Details</summary>
Motivation: Financial documents contain visually dense layouts where small OCR errors (sign inversions, date shifts) can lead to materially different interpretations, while traditional metrics like ROUGE only capture surface-level text similarity.
...
Conclusion: FinCriticalED provides rigorous foundation for advancing visual factual precision in financial and other precision-critical domains by shifting evaluation from lexical overlap to domain-critical factual correctness.
...
TL;DR: Proposes CKDA method for Visible-Infrared Lifelong Re-ID that disentangles modality-specific and modality-common knowledge to prevent collaborative forgetting during sequential training.


<details>
  <summary>Details</summary>
Motivation: Existing VI-LReID methods using cross-modal knowledge distillation suffer from mutual interference between modality-specific knowledge acquisition and modality-common knowledge anti-forgetting, leading to collaborative forgetting.

Method: Uses Modality-Common Prompting (MCP) and Modality-Specific Prompting (MSP) modules to disentangle knowledge, plus Cross-modal Knowledge Alignment (CKA) module to align new and old knowledge in inter- and intra-modality feature spaces.

Result: Extensive experiments on four benchmark datasets show CKDA outperforms state-of-the-art methods in Visible-Infrared Lifelong Re-ID.

Conclusion: CKDA effectively addresses collaborative forgetting by explicitly separating and preserving modality-specific and modality-common knowledge in a balanced manner.
...
TL;DR: A new hologram representation using structured complex-valued 2D Gaussian primitives that reduces parameter space by 10:1, enables faster optimization with lower VRAM usage, and produces higher-fidelity reconstructions than existing methods.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of per-pixel hologram storage and large parameter search spaces in computer-generated holography, enabling more scalable and efficient hologram estimation systems.

Method: Developed a differentiable rasterizer for structured complex-valued 2D Gaussian primitives representation, integrated with GPU-optimized light propagation kernel, and introduced conversion procedures for practical hologram formats including phase-only holograms.

Result: Achieved up to 2.5x lower VRAM usage, 50% faster optimization, higher-fidelity reconstructions, and effective noise artifact suppression compared to previous methods.

Conclusion: The proposed representation enables more scalable hologram estimation in next-generation computer-generated holography systems by significantly reducing parameter search space while maintaining high quality.
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether computer vision models' performance improvements during training align with developmental progressions observed in children's mathematical understanding.

Method: Case study of ResNet-50 model analyzing developmental alignment for geometry/topology concepts (Euclidean Geometry, Geometrical Figures, Metric Properties, Topology) and number concepts, examining emergence of mental number line representations.

Result: Found developmental alignment for some geometry classes (Euclidean Geometry, Geometrical Figures, Metric Properties, Topology) but not others (Chiral Figures, Geometric Transformations, Symmetrical Figures), and alignment in emergence of mental number line representation.

Conclusion: Computer vision models show promise for understanding human mathematical development, with findings supporting future research on additional architectures and larger benchmarks.
...
TL;DR: An energy-based optimization method for video shot assembly that learns from reference videos to automatically arrange shots while preserving artistic expression and style.


<details>
  <summary>Details</summary>
Motivation: Current automated video editing tools fail to capture creators' unique artistic expression in shot assembly, requiring manual work by experienced editors.

Method: Uses visual-semantic matching between LLM-generated scripts and video library, extracts shot attributes from reference videos, and employs energy-based models to score sequences based on reference styles combined with syntax rules.

Result: Enables automated shot arrangement that aligns with reference video assembly styles, creating coherent visual sequences even for users without video editing experience.

Conclusion: The proposed method successfully automates shot assembly while learning and preserving artistic styles from reference videos, making professional video editing accessible to non-experts.
...
TL;DR: UniHOI unifies human-object interaction detection and generation through a shared token space, achieving state-of-the-art performance in both tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional separate approaches to HOI detection and generation hinder comprehensive interaction understanding, creating a need for unified modeling.

Method: Proposes a unified token space with symmetric interaction-aware attention module and semi-supervised learning for bidirectional image-semantics mapping.

Result: Achieves 4.9% improvement in long-tailed HOI detection and 42.0% boost in open-vocabulary generation metrics.

Conclusion: Joint modeling of HOI detection and generation through unified token space effectively promotes knowledge sharing and enhances generalization.
...
TL;DR: Proposes DLRRF model to handle spectral variability and spatial changes in HSI-MSI fusion by modeling spectral degradation and decomposing target into low-rank/residual components with implicit regularization.


<details>
  <summary>Details</summary>
Motivation: Inter-image variability (spectral variability and spatially localized changes) between HSI and MSI significantly affects fusion performance, and existing methods that apply direct transformations exacerbate model ill-posedness.

Method: Models spectral variability as changes in spectral degradation operator, decomposes target HSI into low-rank and residual components for detail recovery, uses dimensionality reduction, and incorporates implicit regularizer with PnP framework solved via PAO algorithm.
...
Conclusion: The proposed DLRRF model effectively handles inter-image variability in HSI-MSI fusion through degradation modeling, component decomposition, and implicit regularization, demonstrating improved fusion performance.
...
<details>
  <summary>Details</summary>
Motivation: Accurate nuclei segmentation in whole slide images is challenging due to variability in staining, imaging conditions, and tissue morphology, requiring robust methods that work with limited annotations.

Method: Uses student-teacher architecture with capacity teacher trained on sparse annotations generating soft pseudo-labels. Joint optimization combines ground-truth labels, teacher-derived probabilistic targets, and hybrid loss (binary cross-entropy + Tversky loss) with asymmetric penalties to handle class imbalance. Includes consistency regularization and layerwise dropout.
...
<details>
  <summary>Details</summary>
Motivation: Existing ultrasound segmentation methods are specialized for specific anatomical structures, limiting their practical utility in clinical settings where multiple organs and tasks need to be handled.

Method: Uses a shared vision encoder with prompt-guided dual decoders for flexible task adaptation, and incorporates an uncertainty-driven pseudo-label calibration module for reliable self-training with both labeled and unlabeled data.

Result: ProPL outperforms state-of-the-art methods across various metrics on a comprehensive ultrasound dataset spanning 5 organs and 8 segmentation tasks.

Conclusion: Establishes a new benchmark for universal ultrasound image segmentation, demonstrating superior performance over specialized approaches.
...
TL;DR: Existing MLLMs perform poorly on vertically written Japanese text compared to horizontal text, but fine-tuning on synthesized Japanese OCR datasets improves vertical text reading capability.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs need to process Japanese documents, which often contain vertical writing, but current research lacks focus on vertical Japanese text understanding.
...
Result: MLLMs performed worse on vertical Japanese text than horizontal text, but training on the synthesized dataset improved vertical text reading ability.

Conclusion: Specialized training on vertical Japanese text is necessary for MLLMs to effectively process Japanese documents containing vertical writing.
...
TL;DR: VR-Bench is a video reasoning benchmark that uses maze-solving tasks to evaluate video models' spatial reasoning capabilities, showing that supervised fine-tuning can effectively elicit reasoning abilities and that test-time sampling improves reliability.
...
Method: Created VR-Bench with 7,920 procedurally generated videos across 5 maze types and diverse visual styles, using supervised fine-tuning (SFT) to elicit reasoning capabilities in video models.

Result: Video models outperformed leading vision-language models in spatial perception during reasoning, generalized well across scenarios, and showed 10-20% reliability improvement with diverse test-time sampling.

Conclusion: Video models have unique potential for spatial reasoning tasks through the reasoning-via-video paradigm, demonstrating scalability and strong performance in spatial planning tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing bokeh rendering methods rely on accurate depth maps, while generative approaches have limited controllability and efficiency. There's a need for depth-free controllable bokeh rendering.
...
<details>
  <summary>Details</summary>
Motivation: Dynamic outdoor environments with high temporal variation pose challenges for existing memory-based trackers due to quadratic computational complexity, temporal redundancy, and insufficient exploitation of geometric priors.

Method: Proposes MambaTrack3D with: 1) Mamba-based Inter-frame Propagation module for efficient inter-frame modeling with near-linear complexity, and 2) Grouped Feature Enhancement Module to separate foreground/background semantics and reduce temporal redundancy.
...
<details>
  <summary>Details</summary>
Motivation: Existing MER approaches overlook inter-modal emotion conflicts where different modalities express divergent emotional tendencies, leading to suboptimal performance on samples with modality inconsistency.

Method: Proposes TiCAL framework with stage-wise emotion perception: uses pseudo unimodal emotion labels and typicality estimation to assess sample consistency, embeds features in hyperbolic space for fine-grained emotion distinctions, and incorporates consistency estimates into learning.

Result: Extensive experiments on CMU-MOSEI and MER2023 show TiCAL effectively mitigates inter-modal emotional conflicts and improves recognition accuracy by about 2.6% over state-of-the-art DMD method.

Conclusion: TiCAL successfully addresses the overlooked issue of inter-modal emotion conflicts in MER through consistency-aware learning and hyperbolic feature representation, demonstrating superior performance particularly on inconsistent samples.
...
Conclusion: JCDM effectively addresses texture incompleteness and cross-view alignment issues in pose-guided human image generation through multi-view conditioning.
...
Result: Found that visual redundancy emerges only in from-scratch dMLLMs during long-answer tasks; from-scratch dMLLMs can recover pruned information progressively; layer-skipping works for AR-to-diffusion dMLLMs while progressive/late-step pruning works for from-scratch dMLLMs.

Conclusion: The work provides new perspective on efficiency optimization for dMLLMs, advancing their applicability across multimodal understanding tasks through modality-specific acceleration strategies.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods suffer from erosion-induced blurring when zooming in and dilation-induced staircase artifacts when zooming out at unseen sampling rates, due to limitations of conventional alpha blending.

Method: Replaces scalar alpha blending with Gaussian Blending that treats alpha and transmittance as spatially varying distributions, allowing background splats to contribute to rendering while maintaining real-time speed.
...
Conclusion: Gaussian Blending successfully addresses fundamental limitations of alpha blending in 3DGS, providing improved rendering quality without additional computational or memory costs.
...
TL;DR: An event-triggered monitoring system for elders that detects three types of events using GMM background modeling and SVM analysis, with intuitive social media integration.


<details>
  <summary>Details</summary>
Motivation: To monitor both physical and mental states of elders while addressing their lack of technical experience through intuitive operation.

Method: Developed event-triggered system with GMM background modeling for motion detection, SVM for image analysis, and intuitive social media communication design.

Result: Successfully tested in 5 home scenarios, detecting three event types from daily life activities with captured image analysis.

Conclusion: The system effectively monitors elder safety while enabling easy communication with relatives through familiar life activities.
...
TL;DR: Proposes Unbiased Semantic Decoding (USD) strategy with SAM for few-shot segmentation, using CLIP semantics to extract target information from both support and query sets for consistent predictions.


<details>
  <summary>Details</summary>
Motivation: Existing SAM-based few-shot segmentation approaches rely heavily on support set prompts, which insufficiently activate SAM's generalization ability and lead to biased decoding for unknown classes.

Method: Integrates USD with SAM using CLIP semantics, featuring two enhancement strategies: global supplement for category indication and local guidance for target location, plus a visual-text target prompt generator.

Result: Enables target-focused attention without retraining foundation models, improving semantic discrimination and unbiased decoding in few-shot segmentation.

Conclusion: The USD strategy effectively leverages CLIP's semantic alignment to enhance SAM's few-shot segmentation performance while maintaining generalization across unknown classes.
...
<details>
  <summary>Details</summary>
Motivation: Individual active learning strategies often show inconsistent performance across different stages of the learning cycle, leading to suboptimal annotation efficiency in medical imaging where labeling costs are high.

Method: Proposes WaveFuse-AL framework that fuses multiple acquisition strategies (BALD, BADGE, Entropy, CoreSet) using cyclical (sinusoidal) temporal priors and performance-driven adaptation to dynamically adjust strategy importance over time.

Result: Outperforms single-strategy and alternating-strategy baselines on three medical imaging benchmarks (APTOS-2019, RSNA Pneumonia Detection, ISIC-2018), achieving statistically significant improvements on 10 out of 12 metric measurements.

Conclusion: WaveFuse-AL effectively maximizes annotation budget utility in medical imaging by adaptively combining multiple active learning strategies throughout the learning process.
...
TL;DR: DCL-SE is a framework that uses data-driven spatiotemporal encoding and dynamic curriculum learning to improve neuroimaging analysis, outperforming existing methods across multiple brain-related tasks.
...
Result: Outperforms existing methods across six datasets in Alzheimer's disease classification, brain tumor classification, cerebral artery segmentation, and brain age prediction in accuracy, robustness, and interpretability.

Conclusion: Demonstrates the importance of compact, task-specific architectures over large-scale pretrained networks for neuroimaging analysis.
...
TL;DR: SceneEdited is the first city-scale dataset for HD map maintenance through 3D point cloud updating, containing over 800 scenes with 23,000+ synthesized changes across 2000+ outdated versions, with baseline methods and toolkit provided.


<details>
  <summary>Details</summary>
Motivation: HD maps quickly become outdated as environments evolve, creating a need for robust methods that not only detect changes but also incorporate them into updated 3D representations. There's a gap between detecting changes and actually updating 3D maps, particularly with 2D image-based change detection.

Method: Created a dataset with over 800 up-to-date scenes covering 73 km of driving and 3 km² urban area, with 23,000+ synthesized object changes created both manually and automatically across 2000+ out-of-date versions. Includes calibrated RGB images, LiDAR scans, and detailed change masks. Provided baseline methods using foundational image-based structure-from-motion pipeline.

Result: Dataset contains realistic urban modifications such as missing roadside infrastructure, buildings, overpasses, and utility poles. Comprehensive toolkit supports scalability, trackability, and portability for future dataset expansion and unification of out-of-date object annotations.
...
TL;DR: A structure-aware pipeline that learns surgical action ontology from trainer transcripts and uses Instrument-Action-Target triplets to condition GPT-4o for generating clinically grounded, trainer-style feedback.


<details>
  <summary>Details</summary>
Motivation: Automating natural, trainer-style feedback to provide timely, accessible, and consistent guidance at scale for surgical training, requiring models that understand clinically relevant representations.

Method: Mining IAT triplets from feedback text, fine-tuning video-to-IAT model with surgical procedure context and temporal instrument motion, and using IAT representations to guide GPT-4o for feedback generation.

Result: Video-to-IAT recognition improved AUC (Instrument: 0.67→0.74; Action: 0.60→0.63; Tissue: 0.74→0.79). IAT conditioning increased feedback fidelity score from 2.17 to 2.44 (+12.4%), doubling admissible generations from 21% to 42%, with improved text similarity metrics.

Conclusion: Grounding feedback generation in explicit IAT structure improves fidelity and yields clinician-verifiable rationales, supporting auditable use in surgical training.
...
TL;DR: The paper introduces a method to mitigate catastrophic forgetting in multimodal continual learning by approximating missing gradients from old tasks using geometric properties of the parameter space.


<details>
  <summary>Details</summary>
Motivation: Multimodal continual instruction tuning faces catastrophic forgetting where learning new tasks degrades performance on previous ones, limiting the ability to build upon acquired knowledge.

Method: Approximates missing gradients from old tasks using directional vectors between current and previous optimal parameters, integrated with real gradients from a limited replay buffer and regulated by Bernoulli sampling for stability-plasticity balance.
...
Conclusion: The geometric gradient approximation approach successfully addresses catastrophic forgetting in multimodal continual learning, enabling effective knowledge retention without increasing model size.
...
TL;DR: SEC-Depth: A self-evolution contrastive learning framework for robust self-supervised depth estimation that mitigates performance degradation under adverse weather conditions.
...
Method: Proposes a self-evolution contrastive learning framework that leverages intermediate training parameters to build temporally evolving latency models, uses dynamic update strategy for depth estimation, and introduces self-evolution contrastive loss (SECL) that treats historical model outputs as negative samples.
...
Conclusion: SEC-Depth effectively addresses weather-induced performance degradation in self-supervised depth estimation through adaptive learning objectives that implicitly sense weather degradation severity without manual intervention.
...
<details>
  <summary>Details</summary>
Motivation: Existing metrics for probabilistic human motion prediction fail to properly evaluate both coverage across multiple motion modes and kinematic validity, often rewarding widely distributed motions even if they're invalid or concentrated in single modes.

Method: MMCM divides motion space into clusters representing modes, explicitly evaluates coverage across these modes, and identifies valid modes by collecting possible future motions from datasets to assess kinematic validity.

Result: Experiments validate that the clustering approach yields sensible mode definitions and MMCM accurately scores multimodal predictions, addressing the coverage and validity criteria effectively.

Conclusion: MMCM provides a comprehensive evaluation framework for multimodal human motion prediction that properly assesses both diversity across motion modes and kinematic plausibility.
...
TL;DR: The paper introduces ILS (instruction-guided lesion segmentation) and MIMIC-ILS dataset to enable chest X-ray lesion segmentation using simple user instructions, overcoming limitations of current models that require expert-level text inputs.


<details>
  <summary>Details</summary>
Motivation: Current chest X-ray lesion segmentation models have limited applicability due to small target labels and reliance on complex expert text inputs, creating barriers to practical clinical use.

Method: Created MIMIC-ILS dataset with 1.1M instruction-answer pairs using automated multimodal pipeline from CXR images and reports, then fine-tuned ROSALIA vision-language model on this dataset.

Result: ROSALIA achieves high segmentation and textual accuracy, demonstrating effectiveness of the pipeline and dataset for pixel-level CXR lesion grounding across seven major lesion types.

Conclusion: The ILS paradigm and MIMIC-ILS dataset provide a foundational resource for practical chest X-ray lesion segmentation using simple user instructions, enabling broader clinical adoption.
...
TL;DR: BrainRotViT: A hybrid architecture combining vision transformers and residual CNNs for accurate brain age estimation from structural MRI, achieving state-of-the-art performance across multiple datasets and showing associations with neurodegenerative conditions.


<details>
  <summary>Details</summary>
Motivation: Traditional regression and CNN-based methods for brain age estimation face limitations like manual feature engineering, limited receptive fields, and overfitting. Pure transformers require large datasets and high computational costs, creating a need for an efficient hybrid approach.

Method: Proposed BrainRotViT - a hybrid architecture that first trains a ViT encoder on an auxiliary age and sex classification task to learn slice-level features, then applies the frozen encoder to all sagittal slices to generate embedding vectors, which are fed into a residual CNN regressor that incorporates subject sex at the final layer for continuous brain age estimation.

Result: Achieved MAE of 3.34 years (Pearson r=0.98, Spearman ρ=0.97, R²=0.95) on validation across 11 MRI datasets from 130+ acquisition sites, outperforming baseline models. Generalizes well to 4 independent cohorts with MAEs between 3.77-5.04 years. Brain age gap analysis shows associations with Alzheimer's disease, cognitive impairment, and autism spectrum disorder.

Conclusion: BrainRotViT provides an efficient, interpretable, and generalizable framework for brain-age prediction that bridges CNN- and transformer-based approaches, opening new avenues for aging and neurodegeneration research while highlighting aging-associated brain regions through attention maps.
...
TL;DR: Insert In Style is a zero-shot generative framework for object composition that achieves high fidelity without per-subject finetuning, using multi-stage training and specialized masked-attention architecture.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail when inserting real-world objects into stylized domains - practical methods lack generative fidelity while generators require impractical online finetuning.

Method: Multi-stage training protocol that disentangles identity, style, and composition representations, combined with specialized masked-attention architecture to prevent concept interference.

Result: State-of-the-art performance on new benchmark, significantly outperforming existing methods on identity and style metrics, strongly corroborated by user studies.

Conclusion: The framework provides the first practical and high-fidelity zero-shot solution for stylized object composition without text prompts.
...
TL;DR: PCMDE metric combines LLMs with reasoning, knowledge mapping, and VLMs to evaluate multimodal data with physics constraints, addressing limitations of current metrics like BLEU and CLIPScore.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics fail to capture semantic or structural accuracy in domain-specific contexts, especially for physics-constrained scenarios.

Method: Three-stage architecture: multimodal feature extraction, confidence-weighted component fusion, and physics-guided reasoning using LLMs for constraint enforcement.

Result: Proposed PCMDE metric overcomes limitations of existing measures by incorporating physics constraints and structural reasoning.

Conclusion: PCMDE provides a more comprehensive evaluation framework for multimodal data that accounts for semantic accuracy and physical constraints.
...
TL;DR: This paper proposes a causal intervention approach to address bias in cross-modal food-to-recipe retrieval by treating ingredients as confounders and using backdoor adjustment to improve similarity judgment.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat recipes as text describing visual appearance, creating bias because food images may not capture all recipe details due to cooking process, presentation, and image conditions. Current representation learning overlooks subtle variations that determine retrieval relevance.

Method: Model bias using causal theory, identify ingredients as confounders, apply backdoor adjustment through causal intervention, and propose a plug-and-play neural module (multi-label ingredient classifier) for debiasing.
...
Conclusion: Causal intervention effectively removes bias in food-to-recipe retrieval by addressing the confounder effect of ingredients, leading to significantly improved retrieval accuracy and performance.
...
<details>
  <summary>Details</summary>
Motivation: To improve dermatological diagnosis by making AI reasoning processes explicit, verifiable, and step-by-step through chain-of-thought narratives that clinicians can understand and trust.

Method: Developed DermCoT corpus with 13,000 dermatologic chain-of-thought cases, created DermEval six-dimensional evaluator and DermBench benchmark, and implemented dermatology-aware visual distillation for enhanced performance.

Result: SkinGPT-R1 achieved average score of 4.031/5 on DermBench, ranking 1st among 14 models with 41% improvement over Vision-R1, and delivered stable accuracy gains on three dermatology classification benchmarks.

Conclusion: Explicit chain-of-thought reasoning with dermatology-specific training significantly improves diagnostic quality and transparency, with DermCoT supervision and visual distillation providing substantial performance gains.
...
<details>
  <summary>Details</summary>
Motivation: Existing QAT methods require expensive retraining for non-linear layer quantization, while PTQ methods either partially quantize non-linear functions or fail to achieve fully integer-only inference, limiting their practical deployment.

Method: Proposes polynomial-based GELU approximation optimized for vision data and bit-shifting-based Softmax approximation. Uses a unified metric considering quantization sensitivity, perturbation, and computational cost to select optimal approximation functions per layer.
...
Conclusion: IPTQ-ViT enables fully integer-only inference for vision transformers without retraining, providing superior performance over existing PTQ methods and competitive results with QAT methods while being more resource-efficient.
...
TL;DR: SplitFlux is a novel method that effectively disentangles image content and style in Flux models by fine-tuning single dream blocks via LoRA, enabling superior content preservation and stylization quality.


<details>
  <summary>Details</summary>
Motivation: Existing SDXL-based methods struggle with high-quality results, and the newer Flux model fails to achieve effective content-style separation due to its underexplored characteristics.

Method: SplitFlux uses two key components: Rank-Constrained Adaptation to preserve content identity by compressing rank and amplifying updates in specific blocks, and Visual-Gated LoRA that splits content LoRA into high-rank and low-rank branches guided by image saliency.
...
Conclusion: SplitFlux successfully addresses content-style disentanglement in Flux models through systematic analysis and targeted fine-tuning of single dream blocks, enabling effective re-embedding of disentangled content into new contexts.
...
TL;DR: Graph Query Networks (GQN) is an attention-based framework that models radar objects as graphs for improved 3D object detection, achieving significant performance gains over prior methods.


<details>
  <summary>Details</summary>
Motivation: Traditional grid and sequence-based detectors struggle with radar's sparse and irregular reflections, requiring a more effective approach for 3D radar object detection.

Method: GQN uses graph queries to dynamically attend over BEV space, with EdgeFocus for relational reasoning and DeepContext Pooling for contextual aggregation.

Result: On NuScenes dataset, GQN improves relative mAP by up to +53%, including +8.2% gain over strongest prior radar method, while reducing graph construction overhead by 80%.

Conclusion: GQN effectively addresses radar detection challenges through graph-based modeling and achieves state-of-the-art performance with reduced computational overhead.
...
TL;DR: LEO is a framework that improves 3D scene graph prediction by transforming relations into nodes in a line graph to capture high-order dependencies, then integrating enriched relation features back into object-centric reasoning.
...
Method: Proposes LEO framework that first predicts potential links to suppress irrelevant edges, transforms scene graph into line graph where relations become nodes, applies line graph neural network for edge-centric relational reasoning, then integrates enriched relation features back into object-centric graph.

Result: Experiments on 3DSSG dataset with two competitive baselines show consistent improvements, demonstrating effectiveness of the edge-to-object reasoning paradigm.

Conclusion: The proposed edge-to-object reasoning framework effectively captures high-order relational dependencies and improves relation prediction in 3D scene graphs, being model-agnostic and compatible with existing object-centric methods.
...
TL;DR: Proposes a hierarchical visual attack method that disrupts multimodal RAG systems by adding imperceptible perturbations to image inputs, causing misalignment between queries and retrieved knowledge to confuse generation.


<details>
  <summary>Details</summary>
Motivation: Existing MRAG systems are vulnerable to knowledge poisoning attacks, but this work explores a different attack setting using only visual perturbations on user inputs without manipulating other components.

Method: Hierarchical Visual Attack with two-stage strategy: first breaks cross-modal alignment, then disrupts multimodal semantic alignment to make retriever recall irrelevant knowledge and misalign generator inputs.

Result: Extensive experiments on OK-VQA and InfoSeek datasets show significant decrease in both retrieval and generation performance when using CLIP-based retrievers and LMMs like BLIP-2 and LLaVA.

Conclusion: Visual attacks through imperceptible perturbations can effectively compromise MRAG systems by disrupting the alignment between multimodal queries and augmented knowledge, revealing new security vulnerabilities.
...
TL;DR: Proposes Xsyn, a one-stage X-ray security image synthesis pipeline using text-to-image generation that eliminates labor-intensive foreground extraction, achieving 1.2% mAP improvement over previous methods.


<details>
  <summary>Details</summary>
Motivation: Address data insufficiency in X-ray security image datasets by eliminating the labor-intensive two-stage pipeline of previous methods that require manual foreground extraction.

Method: One-stage text-to-image generation pipeline with Cross-Attention Refinement (CAR) for bounding box annotation and Background Occlusion Modeling (BOM) to enhance imaging complexity in latent space.

Result: Outperforms all previous methods with 1.2% mAP improvement, and synthetic images improve prohibited item detection performance across various X-ray security datasets and detectors.

Conclusion: Xsyn is the first method to achieve high-quality X-ray security image synthesis without extra labor cost, demonstrating superior performance and broad applicability.
...
TL;DR: Text2Loc++ is a neural network for 3D point cloud localization using natural language descriptions, featuring a coarse-to-fine pipeline with novel training techniques and achieving state-of-the-art performance.
...
Method: Uses a coarse-to-fine pipeline: global place recognition with Hierarchical Transformer with Max pooling and attention-based point cloud encoder, Masked Instance Training for robustness, Modality-aware Hierarchical Contrastive Learning for embedding enhancement, and fine localization with Prototype-based Map Cloning and Cascaded Cross-Attention Transformer.

Result: Outperforms existing methods by up to 15% on KITTI360Pose dataset and shows robust generalization on new datasets, effectively handling complex linguistic expressions and diverse urban environments.

Conclusion: Text2Loc++ provides an effective solution for 3D point cloud localization with natural language, demonstrating superior performance and generalization capabilities through its innovative architecture and training techniques.
...
<details>
  <summary>Details</summary>
Motivation: To leverage the complementary strengths of CNNs (local feature extraction) and Transformers (global contextual modeling) for improved MPox detection by capturing both detailed lesion information and long-range dependencies.

Method: Proposes RS-CA-HSICT framework with HSICT block, residual CNN, spatial CNN, and channel augmentation. Uses integrated CNN-Transformer blocks for multihead attention and structured CNN layers, with H and S operations for spatial homogeneity and structural details. Includes inverse residual learning, stage-wise resolution reduction, and channel fusion with attention.
...
Conclusion: The hybrid CNN-Transformer approach effectively captures both global and local features for MPox detection, demonstrating superior performance through integrated multi-scale feature learning and attention mechanisms.
...
TL;DR: Uni-Adapter is a training-free online test-time adaptation method for 3D vision-language foundation models that uses dynamic prototype learning and graph-based label smoothing to handle noisy, incomplete, or distribution-shifted data without retraining.


<details>
  <summary>Details</summary>
Motivation: 3D vision-language foundation models underperform in practical scenarios with noisy, incomplete, or distribution-shifted data, requiring adaptation strategies to improve real-world applicability.

Method: Proposes dynamic prototype learning with a 3D cache storing class-specific cluster centers, graph-based label smoothing for inter-prototype consistency, and entropy-weighted aggregation to unify predictions from original model and refined cache.

Result: Achieves state-of-the-art performance: ModelNet-40C improved by 10.55%, ScanObjectNN-C by 8.26%, and ShapeNet-C by 4.49% over source 3D VLFMs without retraining.
...
TL;DR: The paper introduces the MILL dataset for low-light image enhancement evaluation across diverse illumination conditions and proposes improvements that enhance robustness across varying light intensities.


<details>
  <summary>Details</summary>
Motivation: Existing low-light enhancement methods rely on paired training data with limited radiance diversity, limiting understanding of performance across varying illumination intensities.
...
Result: Significant performance variations across intensity levels were revealed, and proposed modifications achieved up to 10 dB PSNR improvement for DSLR and 2 dB for smartphone on Full HD images.

Conclusion: The MILL dataset enables comprehensive evaluation of enhancement algorithms across variable lighting conditions, and the proposed improvements enhance robustness across diverse illumination scenarios.
...
TL;DR: A multimodal Transformer model integrates radar, RGB video, IR video, and audio for UAV detection, achieving state-of-the-art performance with high accuracy (0.9812) and real-time capability (41.11 FPS).
...
Method: Designs a novel multimodal Transformer model that fuses diverse data streams (radar, RGB video, IR video, audio) using self-attention mechanisms to learn comprehensive and discriminative representations.

Result: Achieved exceptional performance: 0.9812 accuracy, 0.9873 recall, 0.9787 precision, 0.9826 F1-score, 0.9954 specificity, with high precision/recall in drone distinction. Computationally efficient with 1.09 GFLOPs, 1.22M parameters, and 41.11 FPS inference speed.

Conclusion: The study validates multimodal data fusion via Transformer architecture as a highly accurate and resilient solution for UAV detection, offering significant advancement in aerial object classification for real-time applications.
...
TL;DR: CompTrack is a 3D single object tracking framework that addresses spatial and informational redundancy in LiDAR point clouds through foreground prediction and dynamic token compression, achieving real-time performance at 90 FPS.


<details>
  <summary>Details</summary>
Motivation: Existing 3D trackers are limited by dual-redundancy in point clouds: spatial redundancy from background noise impairs accuracy, and informational redundancy within foreground hinders efficiency.

Method: Uses Spatial Foreground Predictor (SFP) to filter background noise based on information entropy, and Information Bottleneck-guided Dynamic Token Compression (IB-DTC) with online SVD analysis to compress redundant foreground into compact proxy tokens.
...
TL;DR: FIA-Flow is a black-box Feature Inversion Attack framework that achieves high-fidelity image reconstruction from intermediate features in Split DNNs, revealing greater privacy risks than previously recognized.


<details>
  <summary>Details</summary>
Motivation: Split DNNs expose privacy vulnerabilities as intermediate features can be exploited to reconstruct private inputs via Feature Inversion Attack (FIA), but existing methods have limited reconstruction quality, making it difficult to assess true privacy leakage.
...
Result: FIA-Flow achieves more faithful and semantically aligned feature inversion across various models (AlexNet, ResNet, Swin Transformer, DINO, YOLO11) and layers, demonstrating superior reconstruction quality compared to existing methods.

Conclusion: The framework reveals a more severe privacy threat in Split DNNs than previously recognized, showing that intermediate features can be effectively exploited to reconstruct private inputs with high fidelity.
...
### [63] [Adaptive thresholding pattern for fingerprint forgery detection](https://arxiv.org/abs/2511.15322)
*Zahra Farzadpour,Masoumeh Azghani*

Main category: cs.CV

TL;DR: Proposes a fingerprint liveness detection algorithm using adaptive thresholding and wavelet transform to distinguish fake fingerprints from real ones, with enhanced resistance to various distortions.


<details>
  <summary>Details</summary>
Motivation: Fingerprint spoofing poses a severe threat to biometric systems, necessitating automated techniques to detect fake fingerprints that are robust against various distortions like noise, pixel missing, and block missing.

Method: Uses anisotropic diffusion and three-level wavelet transform on input images, adaptively thresholds coefficients from different layers, concatenates them into feature vectors, and classifies using SVM classifier.

Result: Outperforms existing methods by approximately 8% accuracy for 90% missing pixel scenarios and 5% for 70x70 block missing scenarios, demonstrating improved resistance to distortions.
...
### [64] [Fast Post-Hoc Confidence Fusion for 3-Class Open-Set Aerial Object Detection](https://arxiv.org/abs/2511.15343)
*Spyridon Loukovitis,Vasileios Karampinis,Athanasios Voulodimos*

Main category: cs.CV

TL;DR: A lightweight post-processing framework for UAV navigation that enables real-time three-way classification between in-distribution targets, out-of-distribution objects, and background, outperforming threshold-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing UAV navigation systems struggle with open-set detection, typically using single uncertainty scores that conflate out-of-distribution objects with background clutter, limiting safety and reliability.

Method: Proposes a model-agnostic post-processing framework using a compact MLP that fuses multiple confidence estimates and per-detection features, incorporating different logit variants to enhance performance without compromising throughput.

Result: Outperforms threshold-based baselines by 2.7% AUROC in binary classification, improves open-set mAP, and enables robust three-class classification while improving closed-set mAP by up to 9 points (18% relative gain).

Conclusion: The framework provides critical capability for safe UAV navigation by actively distinguishing OOD objects from background, enabling avoidance of unknown objects while maintaining detection performance.
...
### [65] [Zero-Shot Open-Vocabulary Human Motion Grounding with Test-Time Training](https://arxiv.org/abs/2511.15379)
*Yunjiao Zhou,Xinyan Chen,Junlang Qian,Lihua Xie,Jianfei Yang*

Main category: cs.CV

TL;DR: ZOMG is a zero-shot, open-vocabulary framework that segments motion sequences into semantically meaningful sub-actions without requiring annotations or fine-tuning, achieving state-of-the-art performance on motion grounding.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on dense supervision with predefined action classes, which are infeasible in open-vocabulary, real-world settings where motion understanding requires decomposition into fine-grained, semantic-aligned sub-actions.

Method: ZOMG integrates language semantic partition using LLMs to decompose instructions into ordered sub-action units, and soft masking optimization that learns instance-specific temporal masks to focus on critical frames while maintaining intra-segment continuity and inter-segment separation without altering pretrained encoders.

Result: Experiments on three motion-language datasets show state-of-the-art effectiveness, outperforming prior methods by +8.7% mAP on HumanML3D benchmark, with significant improvements in downstream retrieval tasks.

Conclusion: ZOMG establishes a new paradigm for annotation-free motion understanding, demonstrating strong performance in zero-shot motion grounding without requiring any annotations or fine-tuning.
...
### [66] [Breaking Expert Knowledge Limits: Self-Pruning for Large Language Models](https://arxiv.org/abs/2511.15390)
*Haidong Kang,Lihong Lin,Enneng Yang,Hongning Dai,Hao Wang*

Main category: cs.CV

TL;DR: AutoPrune enables LLMs to self-prune by automatically designing optimal pruning algorithms without expert knowledge, using Graph-driven Chain-of-Thought for reasoning and Skew-aware Dynamic Sparsity Allocation to handle outlier values in high pruning ratios.


<details>
  <summary>Details</summary>
Motivation: Existing LLM pruning methods require manual algorithm design with huge labor costs and expert knowledge, and suffer from performance degradation due to outlier values under high pruning ratios with uniform sparsity.

Method: AutoPrune uses LLMs to design pruning algorithms automatically via Graph-driven Chain-of-Thought (GCoT) for optimized reasoning, and implements Skew-aware Dynamic Sparsity Allocation (SDSA) to address outlier value issues.

Result: Extensive experiments on mainstream LLM benchmarks show AutoPrune consistently outperforms state-of-the-art competitors across various pruning scenarios.

Conclusion: AutoPrune successfully demonstrates that LLMs can prune themselves effectively, overcoming expert knowledge limitations and outlier value issues while achieving superior performance compared to existing methods.
...
### [67] [ShelfOcc: Native 3D Supervision beyond LiDAR for Vision-Based Occupancy Estimation](https://arxiv.org/abs/2511.15396)
...
Method: Generates metrically consistent semantic voxel labels from video by filtering and accumulating static geometry across frames, handling dynamic content, and propagating semantic information into a stable voxel representation. Uses vision-based 3D geometry foundation models as prior knowledge but mitigates their sparse/noisy geometry issues.

Result: Substantially outperforms all previous weakly/shelf-supervised methods on Occ3D-nuScenes benchmark with up to 34% relative improvement, establishing a new state-of-the-art for LiDAR-free 3D scene understanding.
...
TL;DR: A post-hoc framework for reliable semantic segmentation that constructs confidence masks with statistical guarantees on false-positive control using conformal prediction.
...
Method: Define nested family of shrunken masks via score thresholding or morphological erosion, then use conformal prediction on calibration data to select shrink parameter that controls false-positive rate.

Result: Method achieves target-level empirical validity on polyp-segmentation benchmark, providing finite-sample guarantees without model retraining.

Conclusion: Enables practical, risk-aware segmentation in clinical settings with statistical guarantees on false-positive control.
...
TL;DR: D4C is a data-free quantization framework specifically designed for CLIP models that addresses performance degradation issues by synthesizing semantically rich and structurally diverse pseudo images through prompt-guided semantic injection, structural contrastive generation, and perturbation-aware enhancement.


<details>
  <summary>Details</summary>
Motivation: Existing Data-Free Quantization (DFQ) techniques perform poorly when directly applied to Vision-Language Models like CLIP due to insufficient semantic content and low intra-image diversity in synthesized samples, creating a need for specialized DFQ methods for multimodal models.

Method: D4C uses three key components: (1) Prompt-Guided Semantic Injection to align generated images with real-world semantics using text prompts, (2) Structural Contrastive Generation to reproduce compositional structures of natural images via foreground-background contrastive synthesis, and (3) Perturbation-Aware Enhancement to apply controlled perturbations for improved sample diversity and robustness.

Result: D4C achieves significant performance improvements across various bit-widths and models. For W4A8 setting with CLIP ResNet-50 and ViT-B/32, it shows Top-1 accuracy improvements of 12.4% and 18.9% on CIFAR-10, 6.8% and 19.7% on CIFAR-100, and 1.4% and 5.7% on ImageNet-1K in zero-shot classification.

Conclusion: D4C effectively bridges the performance gap of DFQ on CLIP models by synthesizing images that are both semantically informative and structurally diverse, making it the first successful DFQ framework tailored for Vision-Language Models.
...
TL;DR: WarNav is a real-world dataset from conflict-affected environments for autonomous ground vehicle navigation, addressing gaps in existing urban driving datasets and enabling development of semantic segmentation models for unstructured war-zones.


<details>
  <summary>Details</summary>
Motivation: To bridge the critical gap between conventional urban driving datasets and the unique operational scenarios encountered by unmanned systems in hazardous, damaged war-zones, enabling development of robust autonomous navigation in high-risk environments.

Method: Constructed dataset from DATTALION repository images, addressed methodological challenges including data heterogeneity and ethical considerations, established baseline results using state-of-the-art semantic segmentation models, and analyzed impact of training data environments with constraint of no annotation on targeted images.

Result: Provided baseline performance references for semantic segmentation in conflict-affected environments, demonstrated the gap between models trained on structured urban scenes and real-world war-zone scenarios, and proposed initial steps toward effective navigability without target image annotations.

Conclusion: WarNav enables impactful research for enhancing autonomous vehicle robustness and safety in high-risk scenarios while being frugal in annotated data, providing guidance for future efforts in extreme operational contexts and advancing navigability in challenging environments.
...
<details>
  <summary>Details</summary>
Motivation: Most existing multimodal detection studies neglect fusion degradation and lack theoretical analysis of its causes, despite its impact on robustness.
...
Conclusion: The proposed RSC-MD method successfully addresses fusion degradation through theoretical insights and practical solutions, demonstrating superior performance across multiple benchmarks.
...
TL;DR: Proposes a novel gearbox component image dataset and contrastive regularization loss to improve deep learning model generalization for remanufacturing quality inspection.


<details>
  <summary>Details</summary>
Motivation: Manual quality inspection in remanufacturing is inefficient, and current deep learning models struggle to generalize to new product variants, components, or defect patterns.

Method: Created a dataset of gearbox components in good/defective condition from two automotive transmissions, with different train-test splits to simulate distribution shifts. Proposed contrastive regularization loss to enhance model robustness.

Result: The contrastive regularization loss demonstrated improved generalization ability to unseen types of components compared to standard approaches.

Conclusion: The proposed method and dataset effectively address generalization challenges in automated visual inspection for remanufacturing, with the contrastive loss showing promising results for component classification.
...
TL;DR: EASD is an end-to-end spike camera detector for autonomous driving that addresses motion blur and saturation issues through a dual-branch architecture and introduces a new driving-oriented benchmark.
...
Result: The paper presents a novel detector architecture specifically designed for spike camera data, overcoming the limitations of traditional image-based detectors when processing sparse, discrete spike stream outputs.

Conclusion: EASD provides an effective end-to-end solution for spike camera object detection in autonomous driving scenarios, addressing the critical challenge of processing spike stream data while leveraging the advantages of spike cameras' high dynamic range and low latency.
...
Method: Multi-scale contrastive alignment framework with graph-based representation of cell interactions, capturing inter- and intra-subgraph relationships across different scales.
...
Conclusion: Sigmma effectively captures hierarchical tissue structures and cell-cell interactions, leading to superior cross-modal representation learning in computational pathology.
...
TL;DR: AI pipeline developed for tuna species identification from electronic monitoring videos, achieving 84.8% segmentation/classification with 4.5% mean error using YOLOv9-SAM2 and hierarchical classification.


<details>
  <summary>Details</summary>
Motivation: Purse seiners catch 69% of tropical tuna, requiring electronic monitoring that generates massive video data. AI can reduce human workload and improve accuracy, but species identification (especially bigeye vs yellowfin tuna) remains challenging due to low expert agreement (42.9-57.1%).

Method: Multi-stage pipeline using segmentation (Mask R-CNN, DINOv2-SAM2, YOLOv9-SAM2), tracking (ByteTrack), and classification (standard multiclass vs hierarchical). Models trained on observer-verified ground-truth data and cross-validated.

Result: YOLOv9-SAM2 performed best with 0.66 mAP and 0.88 recall. Hierarchical classification showed superior generalization. Combined approach achieved 84.8% segmentation/classification rate with 4.5% mean average error on fully known catch composition tests.

Conclusion: The proposed AI pipeline successfully addresses tuna species identification challenges in electronic monitoring systems, demonstrating practical applicability for fisheries management with high accuracy and reliability.
...
TL;DR: FunnyNodules is a synthetic dataset for evaluating explainable AI in medical imaging, featuring controllable lung nodule attributes with predefined decision rules linking attributes to diagnostic classes.


<details>
  <summary>Details</summary>
Motivation: There is a scarcity of densely annotated medical datasets that capture diagnostic reasoning, which is essential for developing AI models that make correct predictions for the right reasons.

Method: Created a fully parameterized synthetic dataset generating abstract lung nodule shapes with controllable visual attributes (roundness, margin sharpness, spiculation), where target class is derived from predefined attribute combinations.

Result: The dataset enables model-agnostic evaluations to assess attribute-target relations, interpret performance in attribute prediction, and analyze attention alignment with attribute-specific regions.

Conclusion: FunnyNodules provides a versatile foundation with complete ground truth for developing, benchmarking, and analyzing explainable AI methods in medical image analysis.
...
TL;DR: EAR introduces a biologically inspired autoregressive image generation method that unfolds tokens in a spiral pattern from center outward, enabling efficient parallel decoding and achieving state-of-the-art fidelity-efficiency trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive visual generation models suffer from inefficiency due to token-by-token decoding or complex multi-scale representations, motivating a more efficient and biologically aligned approach.

Method: Proposes Expanding Autoregressive Representation (EAR) that generates image tokens in a spiral order from center outward, with length-adaptive decoding that dynamically adjusts tokens per step for parallel processing.
...
<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based methods use single prompts that fail to capture semantic diversity, resulting in incomplete target activation. They lack explicit cross-modal interaction and are vulnerable to noisy support features.

Method: Proposes three modules: MTPR for multi-textual prior refinement, TAFF for text anchor feature fusion using multi-text embeddings as semantic anchors, and FCWA for foreground confidence-weighted attention to enhance visual prior robustness.
...
Conclusion: MTGNet effectively addresses limitations of single-prompt approaches by leveraging multiple textual descriptions and cross-modal interactions, demonstrating superior performance on standard benchmarks.
...
TL;DR: Hybrid deep learning framework combining CNNs, ViTs, and GNNs for robust weed detection with GAN-based augmentation and self-supervised pre-training, achieving 99.33% accuracy on multi-benchmark datasets.
...
Method: Hybrid framework using CNNs, Vision Transformers (ViTs), and Graph Neural Networks (GNNs) with GAN-based augmentation for class balancing and self-supervised contrastive pre-training for limited annotated data.

Result: Superior performance with 99.33% accuracy, precision, recall, and F1-score on multi-benchmark datasets, enabling local, global, and relational feature representations.

Conclusion: The framework offers high interpretability, adaptability, and enables real-time deployment to edge devices for automated weed detection, providing scalable sustainable precision-farming solutions.
...
TL;DR: This paper evaluates pose forecasting algorithms, addresses reproducibility issues, adapts speech models to improve performance, and tests robustness with noisy pose data.


<details>
  <summary>Details</summary>
Motivation: Human pose forecasting has important applications in action recognition, autonomous driving, and human-robot interaction, but faces reproducibility issues and needs better performance and robustness for real-world use.

Method: The authors evaluate various pose forecasting algorithms, provide a unified training/evaluation pipeline, adapt speech models to pose forecasting, and test robustness using noisy joint coordinates from pose estimators.

Result: Speech models adapted to pose forecasting improve state-of-the-art performance. Noisy pose data causes substantial performance degradation, but unsupervised finetuning can recover some of this performance loss.

Conclusion: The paper demonstrates successful adaptation of speech models for pose forecasting, highlights the impact of realistic noise on performance, and shows that unsupervised finetuning can mitigate some degradation from noisy pose data.
...
TL;DR: A multi-modal deep learning method that uses a single X-ray image to complete occluded vertebral structures in 3D ultrasound, overcoming ultrasound's limitations in visualizing bone due to acoustic shadowing.
...
Method: Generate paired training data with 2D lateral vertebral views (simulating X-ray) and 3D partial vertebrae representations (simulating ultrasound occlusions), then use multi-modal deep learning to integrate information from both imaging modalities.
...
Conclusion: Integrating a single X-ray projection effectively mitigates ultrasound's key limitation while preserving its advantages as the primary imaging modality for spinal procedures.
...
### [82] [Computer-Use Agents as Judges for Generative User Interface](https://arxiv.org/abs/2511.15567)
*Kevin Qinghong Lin,Siyuan Hu,Linjie Li,Zhengyuan Yang,Lijuan Wang,Philip Torr,Mike Zheng Shou*

Main category: cs.CV

TL;DR: The paper introduces AUI-Gym, a benchmark for automatic GUI development, and proposes a Coder-CUA collaboration framework where coding language models design websites and computer-use agents evaluate them based on task solvability rather than visual appearance.


<details>
  <summary>Details</summary>
Motivation: Current GUIs are designed for humans, forcing agents to adopt inefficient human-oriented behaviors. The paper explores whether computer-use agents can serve as judges to assist coding models in automatic GUI design for improved agent-native efficiency.

Method: Developed AUI-Gym benchmark with 52 applications and 1560 synthesized tasks, created a verifier for task executability, and proposed Coder-CUA collaboration framework where Coder generates/revises websites and CUA evaluates functionality. Designed CUA Dashboard to compress navigation histories into visual summaries for redesign guidance.

Result: The framework enables iterative GUI redesign based on agent performance metrics, shifting interface design toward agent-native efficiency and reliability rather than human-centric visual aesthetics.

Conclusion: Positioning agents as both designers and judges enables a shift from passive use to active participation in digital environments, moving toward agent-native interface design that prioritizes task solvability and navigation success.
...
TL;DR: Proposes DuFIA, a dual-domain adversarial attack method that invalidates AI-generated image detectors by exploiting spatial and frequency domain features to enhance transferability.


<details>
  <summary>Details</summary>
Motivation: To develop advanced adversarial attacks for evaluating the security of AI-generated image detectors, which remains insufficiently explored in antiforensics research.

Method: Uses spatially interpolated gradient and frequency-aware perturbation to capture forensically important features, jointly modeling spatial and frequency-domain feature importances to guide adversarial example generation.
...
Conclusion: DuFIA successfully invalidates AIGI detectors by exploiting dual-domain feature importance, demonstrating the need for more robust detector security evaluation.
...
TL;DR: Feature-map KD fails for ViTs despite their low-rank representations. Analysis reveals token-level high-bandwidth encoding causes mismatch. Two strategies - feature lifting or width alignment - successfully reactivate KD, improving DeiT-Tiny accuracy significantly.
...
Method: Two-view representation analysis: layer-wise SVD showing global low-rank structure, and token-level Spectral Energy Pattern analysis revealing high-bandwidth encoding. Proposed two strategies: post-hoc feature lifting with lightweight projector, and native width alignment in last block.
...
Conclusion: ViT feature distillation fails due to encoding mismatch despite global low-rank structure. Exploiting this structure yields effective, interpretable remedies and provides concrete design guidance for compact ViTs.
...
<details>
  <summary>Details</summary>
Motivation: Current LVLMs struggle with nuanced queries requiring comprehensive understanding and detailed analysis of long videos, necessitating a more sophisticated approach to video QA.

Method: Modular framework with persistent global summary, Pre Retrieval Thinking Agent, Rethink Module, and feedback loop that refines retrieval strategies based on partial answers to enable iterative human-like reasoning.

Result: Achieved relative gains of +5.6% in temporal reasoning, +5% in technical queries, +8% in theme-based questions, and +8.2% in narrative comprehension on CinePile benchmark, with each module contributing positively to performance.

Conclusion: AVATAAR provides a scalable solution for long-form video QA that effectively combines accuracy, interpretability, and extensibility through its modular design and feedback mechanism.
...
Method: Extends KDF paradigm with loss-aware sampling (selects samples based on learning difficulty via loss values) and rehearsal enhancement mechanism (refines memory samples during new-task training).

Result: Consistently outperforms existing continual learning methods and integrates seamlessly into state-of-the-art frameworks, yielding significant and stable performance gains.

Conclusion: KDF+ effectively addresses catastrophic forgetting in LiDAR place recognition through intelligent sampling and rehearsal strategies, demonstrating robust performance across multiple benchmarks.
...
TL;DR: A novel dense feature matching model that achieves state-of-the-art accuracy through systematic architectural improvements, curated training data, and optimized training pipeline.
...
Method: Constructed novel matching architecture and loss, curated diverse training distribution, implemented decoupled two-stage matching-then-refinement pipeline, reduced memory usage with custom CUDA kernel, and leveraged DINOv3 foundation model.

Result: The resulting matcher sets a new state-of-the-art, being significantly more accurate than its predecessors.

Conclusion: The systematic improvements across architecture, training, and optimization yield a significantly better dense feature matching model that addresses previous limitations in accuracy, speed, and robustness.
...
### [88] [Ein Fenster zur gleichzeitigen Messung der Uebertragungsfunktion eines realen Systems und des Leistungsdichtespektrums des ueberlagerten Rauschens am Systemausgang (Teil 1)](https://arxiv.org/abs/2511.14890)
*Helmut Repp*

Main category: eess.SP

TL;DR: A method improves frequency selectivity in measuring transfer functions and noise power spectral density using suitable windows without significantly increasing computational complexity.


<details>
  <summary>Details</summary>
Motivation: To enhance the frequency selectivity of measuring both transfer functions and noise power spectral density in disturbed, time-invariant systems with nonlinearities while maintaining computational efficiency.

Method: Using suitable windows to improve frequency selectivity in power spectral density measurements, with proofs of unbiasedness and consistency. Also examines special cases for real-valued and complex-valued signals without transfer function measurement.

Result: The method significantly improves frequency selectivity of noise power spectral density measurements while maintaining computational efficiency, with proven unbiasedness and consistency.

Conclusion: Suitable windows can effectively enhance measurement accuracy for noise power spectral density and transfer functions in nonlinear systems, with specific requirements for window design and high-precision numerical computation.
...
### [89] [Lightweight Foundation Model for Wireless Time Series Downstream Tasks on Edge Devices](https://arxiv.org/abs/2511.14895)
*Mohammad Cheraghinia,Eli De Poorter,Jaron Fontaine,Kwang Soon Kim,Merouane Debbah,Adnan Shahid*

Main category: eess.SP

TL;DR: A lightweight MLP-based foundation model for wireless tasks that maintains 97%+ accuracy with only 21K parameters, enabling efficient edge deployment with 0.33ms inference time.


<details>
  <summary>Details</summary>
Motivation: Current foundation models use large Transformers that are computationally intensive and unsuitable for edge devices, while training separate models for each wireless task is unsustainable.

Method: Simple Multi-Layer-Perceptron (MLP) encoders that independently process input patches, supporting 4 downstream tasks from multiple input types and sampling rates.

Result: Achieves over 97% accuracy for fine-tuning on unseen data classes, maintains robust generalization performance unlike Transformers, with only 21K parameters and 0.33ms inference time on edge devices.

Conclusion: The MLP-based foundation model provides a scalable, efficient alternative to Transformers for wireless network optimization, suitable for constrained real-time deployments on edge devices.
...
### [90] [Ein Fenster zur gleichzeitigen Messung der Uebertragungsfunktion eines realen Systems und des Leistungsdichtespektrums des ueberlagerten Rauschens am Systemausgang (Teil 2)](https://arxiv.org/abs/2511.14957)
*Helmut Repp*

Main category: eess.SP

TL;DR: Extension of frequency-selective measurement method for transfer functions and noise power spectral density from time-invariant to periodically time-varying systems with cyclostationary noise, using enhanced window construction.


<details>
  <summary>Details</summary>
Motivation: The original method was limited to time-invariant systems with stationary processes, but many real systems exhibit periodic time-variations and cyclostationary noise, requiring an extended measurement approach.

Method: Extended window construction algorithm that utilizes previously unused degrees of freedom to influence window sequence properties for measuring all correlations between input and output signals in periodically time-varying systems.

Result: A generalized measurement technique capable of handling periodically time-varying systems disturbed by cyclostationary noise processes while maintaining frequency-selective measurement capabilities.

Conclusion: The extended window construction algorithm successfully enables comprehensive correlation measurements in periodically time-varying systems with cyclostationary noise, overcoming limitations of the original time-invariant approach.
...
### [91] [WiCo-MG: Wireless Channel Foundation Model for Multipath Generation via Synesthesia of Machines](https://arxiv.org/abs/2511.15026)
*Zengrui Han,Lu Bai,Xuesong Cai,Xiang Cheng*

Main category: eess.SP

TL;DR: WiCo-MG is a wireless channel foundation model that generates multipath channel data using Synesthesia of Machines, achieving state-of-the-art performance for 6G AI-native communication systems.


<details>
  <summary>Details</summary>
Motivation: 6G AI-native communication systems require massive, high-quality multipath channel data for intelligent model training and optimization, which current methods cannot adequately provide.

Method: Two-stage training framework: 1) Cross-modal alignment of sensing images and multipath maps in SoM feature spaces, 2) S-R MoE Transformer with frequency-aware expert routing to learn mapping from sensing to channel spaces for adaptive multipath generation.

Result: Achieves state-of-the-art in-distribution generation and superior out-of-distribution generalization, reducing NMSE by >2.59 dB over baselines, with strong scalability and extensibility to new parameters and tasks.

Conclusion: WiCo-MG enables massive, high-fidelity channel data generation with higher accuracy, stronger generalization, and better scalability, supporting 6G AI-native communication system development.
...
### [92] [WiCo-PG: Wireless Channel Foundation Model for Pathloss Map Generation via Synesthesia of Machines](https://arxiv.org/abs/2511.15030)
*Mingran Sun,Lu Bai,Ziwei Huang,Xuesong Cai,Xiang Cheng,Jianjun Wu*

Main category: eess.SP

TL;DR: WiCo-PG is a wireless channel foundation model that generates pathloss maps using Synesthesia of Machines, achieving superior accuracy in 6G UAV-to-ground scenarios through cross-modal learning with RGB images.


<details>
  <summary>Details</summary>
Motivation: To address the need for accurate pathloss map generation in 6G UAV-to-ground communication scenarios, overcoming limitations of traditional methods and leveraging multi-modal sensing data for improved wireless channel modeling.

Method: Uses dual vector quantized generative adversarial networks (VQGANs) and Transformer architecture with frequency-guided shared-routed mixture of experts (S-R MoE) for cross-modal pathloss map generation from RGB images across different scenarios and flight altitudes.

Result: Achieves normalized mean squared error (NMSE) of 0.012, outperforming LLM-based and conventional deep learning schemes by more than 6.98 dB, with enhanced few-shot generalization using only 2.7% samples.

Conclusion: WiCo-PG demonstrates superior pathloss map generation accuracy and generalization capability, establishing an effective foundation model for wireless channel modeling in 6G UAV communications through cross-modal learning.
...
### [93] [Position Optimization for Two-layer Movable Antenna Systems](https://arxiv.org/abs/2511.15108)
*Liujia Yao,Changsheng You,Chao Zhou,Beixiong Zheng,Weidong Mei*

Main category: eess.SP

TL;DR: Proposes a two-layer movable antenna array (TL-MA) that reduces control complexity and hardware cost compared to single-layer MA arrays by combining large-scale subarray movement with small-scale per-subarray antenna fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing single-layer MA arrays have high control complexity and hardware costs due to all antennas moving within the movable region. A more practical solution is needed to maintain performance while reducing complexity.

Method: Developed a TL-MA system with joint optimization of subarray positions, per-subarray MA positions, and receive beamforming. Used alternating optimization with particle swarm optimization to solve the non-convex problem.

Result: Numerical results show TL-MA significantly reduces sum-displacement of MA motors (total moving distances) compared to element-wise SL-MA, while achieving comparable rate performance.

Conclusion: The proposed TL-MA architecture provides a practical solution that maintains communication performance while substantially reducing the complexity and cost associated with movable antenna systems.
...
### [94] [The Triple-C Paradigm: Cooperative, Complementary, and Competitive Modes for TBS-HAPS-LEO Integration](https://arxiv.org/abs/2511.15064)
*Eros Kuikel,Sidrah Javed,Baha Eddine Youcef Belmekki,Yunfei Chen,Ning Wang,Mohamed-Slim Alouini*

Main category: eess.SP

TL;DR: Proposes a Triple-C framework (Cooperation, Complementarity, Competition) to coordinate TBS-HAPS-LEO networks for seamless resilient connectivity.


<details>
  <summary>Details</summary>
Motivation: Existing networks struggle to meet growing demands for ubiquitous global coverage, requiring integration of terrestrial, aerial, and satellite platforms.

Method: Developed a systematic framework defining architectural methodology, prerequisites, and deliverables for when layers should collaborate, complement, or contend.

Result: Identified enabling technologies, demonstrated use cases, quantified trade-offs through performance analysis, and examined implementation implications.

Conclusion: Provided gap analysis, outlined challenges, and presented roadmap for future research to unlock full potential of TBS-HAPS-LEO integrated networks.
...
### [95] [Enhancing Physical Layer Security in MIMO Systems Assisted by Beyond-Diagonal Reconfigurable Intelligent Surfaces](https://arxiv.org/abs/2511.15093)
*Weijie Xiong,Jingran Lin,Cunhua Pan,Yilong Zeng,Qiang Li*

Main category: eess.SP

TL;DR: BD-RIS with non-diagonal scattering matrices outperforms conventional RIS in MIMO physical layer security by enabling better channel manipulation and higher secrecy rates.


<details>
  <summary>Details</summary>
Motivation: Conventional RIS with diagonal scattering matrices have limited flexibility in channel manipulation for physical layer security, while BD-RIS with non-diagonal matrices offers enhanced channel shaping capabilities.

Method: Joint optimization of transmit beamforming and BD-RIS reflecting elements using a penalty product Riemannian conjugate gradient descent (P-PRCGD) method that combines augmented Lagrangian approach with product manifold gradient descent.

Result: Simulation results show BD-RIS-assisted systems significantly outperform conventional RIS-assisted systems in physical layer security performance.

Conclusion: BD-RIS architecture creates new opportunities for advanced physical layer security techniques by enabling superior channel manipulation capabilities compared to conventional RIS.
...
### [96] [Collision Resolution in RFID Systems Using Antenna Arrays and Mix Source Separation](https://arxiv.org/abs/2511.15490)
*Mohamed Siala,Noura Sellami*

Main category: eess.SP

TL;DR: Proposes efficient mix source separation algorithm for RFID collision resolution using antenna arrays, addressing ambiguities in beamformer determination through hybrid objective function.


<details>
  <summary>Details</summary>
Motivation: To resolve tag collisions in RFID systems without using pilot symbols, addressing limitations of zero constant modulus criterion alone which causes significant ambiguities in beamformer determination.

Method: Uses gradient descent with zero constant modulus criterion initially, then introduces hybrid objective function combining ZCM with new ambiguity-raising criterion to improve beamformer accuracy.

Result: The hybrid approach addresses limitations of ZCM-only method by reducing ambiguities in beamformer determination for tag message recovery.

Conclusion: A sophisticated hybrid objective function combining ZCM with ambiguity-raising criterion effectively resolves RFID tag collisions without pilot symbols, overcoming limitations of ZCM-only approaches.
...
### [97] [Constant-Modulus Secure Analog Beamforming for an IRS-Assisted Communication System with Large-Scale Antenna Array](https://arxiv.org/abs/2511.15095)
*Weijie Xiong,Jingran Lin,Zhiling Xiao,Qiang Li*

Main category: eess.SP

TL;DR: This paper proposes two algorithms for maximizing secrecy rate in IRS-assisted communication systems using constant modulus analog beamforming, addressing hardware cost and power consumption issues.


<details>
  <summary>Details</summary>
Motivation: To enhance physical layer security in wireless systems while reducing hardware costs and power consumption associated with large-scale antenna arrays and intelligent reflecting surfaces.

Method: Two algorithms: (1) Dinkelbach-BSUM algorithm that reformulates the fractional problem into quadratic programs, and (2) PMCGD algorithm that transforms the problem into unconstrained optimization on a Riemannian product manifold using conjugate gradient descent.

Result: Simulation results validate the effectiveness of both proposed algorithms, with PMCGD providing better solutions at slightly higher computational cost.

Conclusion: The proposed algorithms successfully address the non-convex secrecy rate maximization problem in IRS-assisted systems with constant modulus analog beamforming, offering distinct advantages for different application scenarios.
...
### [98] [Randomized Power Transmission with Optimized Level Selection Probabilities in Uncoordinated Uplink NOMA](https://arxiv.org/abs/2511.15523)
*Noura Sellami,Mohamed Siala*

Main category: eess.SP

TL;DR: Optimizing power level selection probabilities in uncoordinated random uplink NOMA systems to minimize block/bit error probabilities.


<details>
  <summary>Details</summary>
Motivation: To improve performance in uncoordinated random uplink NOMA systems by optimizing power level selection probabilities rather than using fixed or random assignments.

Method: Formulate optimization as quadratic programming for 2-user collision scenarios and iterative solution for more users; applicable to any multiuser detection algorithm and power level set.

Result: Proposed optimization framework reduces block error probability (BLEP) and bit error probability (BEP) in uncoordinated NOMA systems.

Conclusion: The method provides a flexible and effective approach to optimize power level selection in uncoordinated NOMA systems, improving error performance across various detection algorithms.
...
### [99] [Joint Analog Beamforming and Antenna Position Design for Secure Communication systems With Movable Antennas](https://arxiv.org/abs/2511.15096)
*Weijie Xiong,Kai Zhong,Zhiling Xiao,Jingran Lin,Qiang Li*

Main category: eess.SP

TL;DR: This paper proposes using movable antennas (MA) to enhance physical layer security in analog beamforming systems by jointly optimizing transmit beamforming and antenna positions to maximize secrecy rate.


<details>
  <summary>Details</summary>
Motivation: Movable antennas offer flexible position adjustment to improve wireless communication performance, particularly for enhancing physical layer security in analog beamforming systems where traditional fixed antennas have limitations.

Method: A penalty product manifold (PPM) method is developed to solve the non-convex optimization problem, converting MA position constraints into penalty functions and using parallel conjugate gradient descent (PCGD) on product manifold space for efficient updates.

Result: Simulation results demonstrate that the proposed MA system achieves higher secrecy rates compared to systems with fixed-position antennas, validating the effectiveness of the approach.

Conclusion: Movable antennas combined with the proposed PPM optimization method significantly improve physical layer security performance in analog beamforming systems, offering a promising solution for secure wireless communications.
...
### [100] [Multimodal Wireless Foundation Models](https://arxiv.org/abs/2511.15162)
*Ahmed Aboulfotouh,Hatem Abou-Zeid*

Main category: eess.SP

TL;DR: First multimodal wireless foundation model that processes both raw IQ streams and image-like wireless modalities (spectrograms, CSI) for multiple tasks across both modalities using masked wireless modeling.


<details>
  <summary>Details</summary>
Motivation: Current wireless foundation models only process single modalities, but the most informative modality varies by task and conditions. Multimodal WFMs are needed to handle diverse tasks and scenarios more effectively.

Method: Proposed masked wireless modeling - a self-supervised objective and pretraining recipe that learns joint representations from IQ streams and image-like wireless modalities.

Result: Competitive with single-modality WFMs and surpasses their performance in several cases across five tasks: human activity sensing, RF signal classification, 5G NR positioning, RF device fingerprinting, and interference detection/classification.

Conclusion: Demonstrates strong potential for multimodal WFMs to support diverse wireless tasks across different modalities, providing a step toward AI-native 6G and joint sensing, communication, and localization.
...
### [101] [Spectrum and Orthogonality of Orthogonal Delay-Doppler Division Multiplexing Modulation Waveforms](https://arxiv.org/abs/2511.15184)
*Akram Shafie,Jun Tong,Jinhong Yuan,Taka Sakurai,Paul Fitzpatrick,Yuting Fang,Yixuan Xie*

Main category: eess.SP

TL;DR: Analysis of spectra and orthogonality in analog vs digital ODDM systems, showing digital implementation eliminates need for extra time resources while maintaining orthogonality.


<details>
  <summary>Details</summary>
Motivation: ODDM modulation is promising for reliable communications in doubly-selective channels, but the spectral and orthogonality characteristics of analog vs digital implementations need investigation.

Method: Derived time/frequency domain representations of basis functions, calculated power spectral densities, proved orthogonality characteristics, compared with other DD modulation variants, and validated with numerical results.

Result: Analog ODDM has step-wise spectrum transitions while digital ODDM is confined to sub-pulse spectrum; digital ODDM achieves orthogonality without extra time resources unlike analog implementation.

Conclusion: Approximate digital ODDM offers practical advantages over analog implementation by maintaining orthogonality without additional time domain overhead while providing confined spectrum characteristics.
...
### [102] [Theoretical Bounds on Parallel Imaging Implicit Data Crimes in an MRI Reproducing Kernel Hilbert Space](https://arxiv.org/abs/2511.15187)
*Evan Frenklak,Yamin Arefeen,Jonathan I Tamir*

Main category: eess.SP

TL;DR: This paper proposes a mathematical framework to address implicit data crimes in MRI AI acceleration by re-characterizing the problem as error reduction during interpolation between evaluation coordinates, establishing a generalized matrix-based error bound definition.


<details>
  <summary>Details</summary>
Motivation: MRI scan times are long and costly, limiting accessibility. AI methods show promise for reducing scan times but often fail clinical translation due to implicit data crimes - hidden biases from incomplete modeling of MRI physics in datasets.

Method: The authors propose a mathematical framework that re-characterizes the problem as error reduction during interpolation between sets of evaluation coordinates. They establish a generalized matrix-based definition of reconstruction error upper bound as a function of input sampling pattern.

Result: Experiments on relevant sampling pattern structures demonstrate the relevance of the proposed framework and suggest future directions for analyzing data crimes in MRI AI acceleration.

Conclusion: The mathematical framework provides a systematic approach to understand and mitigate implicit data crimes in AI-accelerated MRI, offering a foundation for future analysis and improvement of clinical translation success.
...
### [103] [Space-Time-Frequency Synthetic Integrated Sensing and Communication Networks](https://arxiv.org/abs/2511.15198)
*Henglin Pu,Xuefeng Wang,Lu Su,Husheng Li*

Main category: eess.SP

TL;DR: Proposes a space-time-frequency synthetic ISAC architecture that fuses distributed observations to overcome limitations of cellular networks for fine sensing resolution.


<details>
  <summary>Details</summary>
Motivation: Commercial cellular networks struggle with fine angular, range, and Doppler resolution due to limited aperture, bandwidth, and coherent observation time, despite ISAC's promise of high efficiency.

Method: Develops unified signal model for multistatic/monostatic configurations, derives CRLBs, analyzes Fisher information impact, and compares concentrated MLE with two-stage information fusion method using Gauss-Newton algorithm.

Result: MLE approaches CRLB at high SNR, while two-stage method remains competitive at moderate-high SNR but degrades at low SNR. Fully synthesized network processing outperforms individual BS fusion.

Conclusion: Fully synthesized network processing is essential for upgrading existing communication infrastructure into dense sensing networks, as individual BS fusion is inferior and unstable at low SNR.
...
### [104] [Theoretical and Empirical Study of Spatial Power Focusing Effect for Sparse Arrays at Terahertz Band](https://arxiv.org/abs/2511.15221)
*Yongchao He,Taihao Zhang,Cunhua Pan,Hong Ren,Xianzhe Chen,Tian Qiu,Bingchang Hua,Jiangzhou Wang*

Main category: eess.SP

TL;DR: This paper analyzes spatial power focusing in large-scale sparse arrays at terahertz frequencies through theoretical modeling and experimental validation at 300 GHz.


<details>
  <summary>Details</summary>
Motivation: To investigate and validate the spatial power focusing effect in large-scale sparse arrays operating at terahertz frequencies, addressing the impact of practical factors like phase noise and positional deviations.

Method: Combined theoretical analysis using Green's function channel model with numerical simulations and experimental validation using a 300 GHz measurement platform based on vector network analyzer (VNA).

Result: The measurement results show close consistency with theoretical simulations, confirming the spatial power focusing effect for sparse arrays and validating the theoretical analysis.

Conclusion: The study successfully demonstrates and validates the spatial power focusing effect in large-scale sparse arrays at terahertz band, with theoretical models closely matching experimental results.
...
### [105] [Spatially Consistent Air-to-Ground Channel Modeling and Simulation via 3D Shadow Projections](https://arxiv.org/abs/2511.15412)
*Evgenii Vinogradov,Aymen Fakhreddine,Abdul Saboor,Sergi Abadal,Sofie Pollin*

Main category: eess.SP

TL;DR: Spatially-consistent semi-deterministic Air-to-Ground channel modeling using 3D building shadow projections for LOS determination, combining deterministic path loss with stochastic shadow fading.


<details>
  <summary>Details</summary>
Motivation: Need for efficient A2G channel modeling that provides spatial consistency for environment- and mobility-aware evaluation in UAV-assisted networks, avoiding computational complexity of ray tracing while maintaining accuracy.

Method: Uses 3D building shadow projections to determine Line-of-Sight regions and generate LOS maps, then integrates LOS-aware deterministic path loss with stochastic shadow fading to create spatially consistent radio maps.

Result: Simulation in ITU-compliant Manhattan grid environments shows the model captures key urban propagation characteristics like LOS blockage patterns and outage behavior, providing an efficient alternative to ray tracing or fully stochastic models.

Conclusion: The approach is suitable for user mobility, link planning, and radio map generation in 6G non-terrestrial networks, offering computational efficiency while maintaining spatial consistency and environmental awareness.
...
### [106] [Enabling NLOS Imaging Capabilities at the Initial Access of 6G Base Stations](https://arxiv.org/abs/2511.15416)
*Davide Tornielli Bellini,Dario Tagliaferri,Pietro Grassi,Davide Scazzoli,Stefano Tebaldini,Umberto Spagnolini*

Main category: eess.SP

TL;DR: This paper integrates NLOS imaging into 5G base station initial access using modular reflectors, enabling high-resolution imaging behind corners while maintaining communication functionality.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of non-line-of-sight sensing in integrated systems by enabling base stations to 'see behind corners' without requiring reconfigurable hardware or prior environmental knowledge.

Method: Enhance standard beam alignment codebooks with imaging-specific entries, jointly designed with modular reflector angular configuration to enable coherent echo processing for NLOS imaging.

Result: Achieves high-resolution NLOS imaging with derived closed-form expressions for near-field resolution and effective aperture, plus velocity estimation for moving targets with theoretical performance bounds.

Conclusion: The proposed method effectively enables NLOS imaging capabilities in communication infrastructure using modular reflectors, validated through numerical simulations and experimental results.
...
### [107] [Pinching-Antenna System-Assisted Localization: A Stochastic Geometry Perspective](https://arxiv.org/abs/2511.15444)
*Jiajun He,Xidong Mu,Hien Quoc Ngo,Michail Matthaiou*

Main category: eess.SP

TL;DR: A novel localization framework using pinching-antenna systems with RSS measurements, analyzed via stochastic geometry to derive closed-form expressions for localizability and CRLB, showing superiority over fixed-antenna systems.


<details>
  <summary>Details</summary>
Motivation: To develop a comprehensive analytical framework for localization systems using pinching-antennas that enables tractable network-level performance analysis without extensive simulations.

Method: Uses stochastic geometry to model spatial distribution of pinching-antennas, derives closed-form expressions for target localizability and Cramer-Rao lower bound (CRLB) distribution from RSS measurements of downlink signals.

Result: The framework provides practical guidance for optimal waveguide number selection and demonstrates superiority over conventional fixed-antenna systems in terms of CRLB performance.

Conclusion: The proposed pinching-antenna localization framework offers analytical tractability and improved performance over traditional approaches, with closed-form expressions enabling efficient system design and optimization.
...
### [108] [Division-based Receiver-agnostic RFF Identification in WiFi Systems](https://arxiv.org/abs/2511.15458)
*Xuan Yang,Dongming Li,Dong Wei,Meng Zhang*

Main category: eess.SP

TL;DR: Proposes a division-based receiver-agnostic radio frequency fingerprint extraction method for WiFi devices that eliminates receiver-dependent effects using frequency domain division of different preambles, requiring only a single receiver for training.


<details>
  <summary>Details</summary>
Motivation: Radio frequency fingerprint identification in WiFi devices suffers from performance degradation when models trained on one receiver are tested on another due to receiver differences.

Method: Uses frequency domain division of different preambles (L-STF/L-LTF for flat fading, HT-LTF/L-LTF for frequency-selective fading) to remove receiver effects and extract higher-dimensional receiver-agnostic RFF features.

Result: Achieves 15.5% and 28.45% classification accuracy improvements over state-of-the-art methods in flat fading and frequency-selective fading scenarios respectively, when training on one receiver and testing on another.

Conclusion: The proposed method effectively mitigates channel variations and receiver differences, enabling robust RFF identification with only a single receiver for training.
...
### [109] [A Review of Machine Learning for Cavitation Intensity Recognition in Complex Industrial Systems](https://arxiv.org/abs/2511.15497)
...
TL;DR: This paper provides a comprehensive review of cavitation intensity recognition (CIR) technology from 2002-2025, tracing its evolution from traditional machine learning to deep learning and physics-informed models, with future directions including transfer learning and industrial deployment.


<details>
  <summary>Details</summary>
Motivation: There is a lack of systematic review that traces the development trajectory and provides explicit guidance for future research in cavitation intensity recognition, despite its critical importance for hydraulic machinery safety and performance optimization.

Method: The paper conducts a thorough review and analysis of hundreds of publications on intelligent CIR across various mechanical equipment, summarizing technological evolution and providing insights for future development.

Result: The review identifies three main stages: early traditional ML with manual feature engineering, deep learning enabling end-to-end models with automatic feature extraction, and recent physics-informed diagnostic models that embed domain knowledge into DL for better interpretability and generalization.

Conclusion: Future CIR development will focus on transfer learning, multi-modal fusion, lightweight networks, and industrial agent deployment to address challenges in data acquisition, standardized evaluation, and industrial implementation, with integration of deep learning and physical knowledge as the emerging trend.
...
### [110] [Uncoordinated Cooperative OFDM Multi-Hop UAV Relay Networks Using Virtual Channels Based on All-Pass Filters](https://arxiv.org/abs/2511.15545)
*Noura Sellami,Mohamed Siala*

Main category: eess.SP

TL;DR: Proposes an efficient transmission scheme using virtual transmit channels with truncated all-pass filters to mitigate destructive interference in cooperative OFDM-based multi-hop UAV relay networks, outperforming classical phase dithering.


<details>
  <summary>Details</summary>
Motivation: Address destructive interference at destination nodes caused by uncoordinated transmissions of common packets by cooperating UAVs in OFDM-based multi-hop relay networks.

Method: Introduces virtual transmit channels using truncated all-pass filters at each UAV, integrated with distributed randomized space-time block coding, and develops efficient composite channel estimation algorithms.

Result: Simulation results show the proposed scheme significantly outperforms classical phase dithering scheme across various scenarios, ensuring excellent performance with single UAV transmission and cooperative diversity with multiple UAVs.

Conclusion: The virtual transmit channel approach effectively mitigates destructive interference in cooperative UAV networks while maintaining transmission reliability through space-time coding integration.
...
TL;DR: The paper proves that decoder-only Transformers are generically injective (one-to-one) in their mapping from prompts to hidden states, establishes mathematical conditions for this property, and provides empirical validation using geometric diagnostics on real models.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental properties of Transformer representations - specifically whether they preserve distinctness of inputs (injectivity) and how this property behaves under training, quantization, and model variations.

Method: Mathematical analysis defining collision discriminants and injective strata, plus empirical study using separation margin and co-Lipschitz constant metrics on large prompt sets with models like LLaMA-3, Qwen, and GPT-2.

Result: Transformers are generically injective in continuous parameter spaces, injectivity persists during training, and practical invertibility can be measured via geometric diagnostics. 4-bit quantization reduces injectivity while 8-bit preserves it.

Conclusion: Transformer representations maintain injectivity under ideal conditions, and simple geometric metrics can effectively probe their practical invertibility properties across different model configurations and training regimes.
...
TL;DR: This paper introduces Derivation Capability (DC) - the ability of LLMs to apply abstract rules that govern relationships between input changes and output modifications, similar to human reasoning. The authors propose DEVAL evaluation framework and find current LLMs have moderate recognition but poor application of derivation relations. They develop Derivation Prompting (DP) which improves DC by 15.2% on average.


<details>
  <summary>Details</summary>
Motivation: Current LLMs lack comprehensive evaluation of their reasoning ability over data changes, particularly the human-like capability to derive corresponding output modifications based on input changes governed by abstract rules.

Method: Formally define Derivation Relation (DR) and Derivation Capability (DC), propose DEVAL evaluation framework to assess DC across seven tasks, and develop Derivation Prompting (DP) as a novel prompt engineering approach.

Result: Evaluation of five popular LLMs and one Large Reasoning Model shows moderate DR recognition but significant drop-offs in applying DR effectively. Derivation Prompting achieves 15.2% average improvement in DC across all tested LLMs, outperforming common prompt engineering techniques.

Conclusion: Mainstream LLMs have limited Derivation Capability but this can be significantly improved through specialized prompting techniques like Derivation Prompting, which effectively enhances their ability to reason about input-output change relationships.
...
TL;DR: Dynamic nested hierarchies enable models to autonomously adjust optimization levels, nesting structures, and update frequencies during training/inference, overcoming rigid architectures in non-stationary environments.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of contemporary ML models that falter in non-stationary environments due to rigid architectures that hinder continual adaptation and lifelong learning, particularly the anterograde amnesia problem.

Method: Extends nested learning paradigm with dynamic hierarchies that allow autonomous adjustment of optimization levels, nesting structures, and update frequencies during training/inference, inspired by neuroplasticity principles.

Result: Demonstrates superior performance in language modeling, continual learning, and long-context reasoning, with theoretical proofs of convergence, expressivity bounds, and sublinear regret in varying regimes.

Conclusion: Dynamic nested hierarchies establish a foundational advancement toward adaptive, general-purpose intelligence by enabling true lifelong learning and dynamic context compression for distribution shifts.
...
TL;DR: GTPO is a novel RL algorithm that improves multi-turn Tool-Integrated Reasoning in LLMs through turn-level rewards, return-based advantage estimation, and self-supervised reward shaping, outperforming previous methods by 3.0% on reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current RL methods like GRPO use coarse-grained trajectory-level rewards that provide insufficient learning signals for complex multi-turn Tool-Integrated Reasoning tasks, leading to training stagnation.

Method: GTPO introduces three innovations: turn-level reward assignment for fine-grained feedback, return-based advantage estimation using normalized discounted returns, and self-supervised reward shaping that exploits signals from generated code to densify sparse binary rewards.

Result: GTPO outperforms GRPO by 3.0% on average across diverse reasoning benchmarks, demonstrating improved effectiveness for complex mathematical reasoning.

Conclusion: GTPO establishes an effective RL approach for advancing complex multi-turn Tool-Integrated Reasoning in LLMs, addressing limitations of previous methods through fine-grained reward mechanisms.
...
TL;DR: FinTRec is a transformer-based framework for financial services recommendation that addresses challenges like long-range user interactions and multiple interrelated products, outperforming traditional tree-based models while enabling cross-product signal sharing.


<details>
  <summary>Details</summary>
Motivation: Transformer architectures face practical challenges in financial services including long-range user interactions across digital/physical channels and the need for coordinated models for multiple products with competing business goals.

Method: Proposed FinTRec framework using transformer architecture for sequential recommendation, enabling unified modeling across products with fine-tuning for product adaptation.

Result: FinTRec consistently outperforms production-grade tree-based baseline in historic simulation and live A/B tests, improves offline performance across all products, and enables cross-product signal sharing while reducing training costs.

Conclusion: FinTRec demonstrates a viable shift from traditional tree-based models to transformers in financial services, offering the first comprehensive study of unified sequential recommendation addressing both technical and business considerations.
...
TL;DR: Transformer-guided DRL reduces training time by 75% for eVTOL takeoff trajectory optimization while maintaining high accuracy in energy consumption.


<details>
  <summary>Details</summary>
Motivation: To address the training difficulty bottleneck in deep reinforcement learning for complex eVTOL trajectory optimization problems.

Method: Transformer-guided DRL that explores realistic state space using a transformer to guide the learning process for optimal takeoff trajectory design.

Result: Reduced training time from 19.79M to 4.57M time steps (75% reduction) while achieving 97.2% accuracy in energy consumption compared to optimal reference.

Conclusion: Transformer-guided DRL outperforms vanilla DRL in both training efficiency and optimal design verification for eVTOL trajectory optimization.
...
### [117] [Bringing Federated Learning to Space](https://arxiv.org/abs/2511.14889)
*Grace Kim,Filip Svoboda,Nicholas Lane*

Main category: cs.LG

TL;DR: First systematic feasibility analysis of federated learning for satellite constellations, showing space-adapted FL algorithms can scale to 100 satellites with near-ideal performance and 9x speedup through orbital scheduling.


<details>
  <summary>Details</summary>
Motivation: LEO satellite constellations are rapidly expanding, creating downlink bandwidth limitations that require distributed on-board machine learning. Federated learning offers a promising framework for collaborative model training across satellite networks.

Method: Introduced a 'space-ification' framework that adapts terrestrial FL algorithms (FedAvg, FedProx, FedBuff) to orbital constraints. Evaluated through 768 constellation configurations varying cluster sizes, satellites per cluster, and ground station networks.
...
Conclusion: Results provide actionable insights for mission designers, enabling distributed on-board learning for more autonomous, resilient, and data-driven satellite operations.
...
### [118] [It's LIT! Reliability-Optimized LLMs with Inspectable Tools](https://arxiv.org/abs/2511.14903)
*Ruixin Zhang,Jon Donnelly,Zhicheng Guo,Ghazal Khalighinejad,Haiyang Huang,Alina Jade Barnett,Cynthia Rudin*

Main category: cs.LG

TL;DR: LIT framework forces LLMs to use reliable external tools for transparent problem-solving, improving trustworthiness in high-stakes domains.


<details>
  <summary>Details</summary>
Motivation: LLMs have opaque reasoning processes that limit trustworthiness in high-stakes applications, and they may choose unreliable solutions even when better options exist.

Method: Built on existing LLM tool-calling capabilities, LIT framework enables LLMs to select the most reliable and easy-to-troubleshoot solution paths involving multiple sequential tool calls, supported by reliability cost functions.

Result: LLMs achieve more reliable and informed problem-solving while maintaining task performance using the LIT framework, as demonstrated on a benchmark of 1,300 questions.

Conclusion: The LIT framework successfully addresses LLM opacity by forcing use of reliable external tools, making solutions more trustworthy and easier to troubleshoot in critical applications.
...
### [119] [Structured Contrastive Learning for Interpretable Latent Representations](https://arxiv.org/abs/2511.14920)
*Zhengyang Shen,Hua Tu,Mayue Shi*

Main category: cs.LG

TL;DR: Neural networks are brittle to semantic transformations like ECG phase shifts and IMU rotations. The paper proposes Structured Contrastive Learning (SCL) to partition latent space into invariant, variant, and free features, improving robustness and interpretability without architectural changes.


<details>
  <summary>Details</summary>
Motivation: Neural networks show severe brittleness to semantically irrelevant transformations - ECG phase shifts degrade latent similarity from 1.0 to 0.2, and IMU rotations collapse activity recognition performance. The root cause is identified as 'laissez-faire' representation learning where latent spaces evolve unconstrained.

Method: Proposed Structured Contrastive Learning (SCL) framework that partitions latent space into three semantic groups: invariant features (consistent under transformations), variant features (differentiate transformations via novel variant mechanism), and free features (preserve task flexibility). Creates controllable push-pull dynamics with distinct purposes for different latent dimensions.

Result: Superior performance demonstrated: ECG similarity improved from 0.25 to 0.91 under phase shifts, WISDM activity recognition achieved 86.65% accuracy with 95.38% rotation consistency, consistently outperforming traditional data augmentation.

Conclusion: Represents a paradigm shift from reactive data augmentation to proactive structural learning, enabling interpretable latent representations in neural networks. The approach requires no architectural modifications and integrates seamlessly into existing training pipelines.
...
### [120] [Integrating Causal Inference with Graph Neural Networks for Alzheimer's Disease Analysis](https://arxiv.org/abs/2511.14922)
*Pranay Kumar Peddi,Dhrubajyoti Ghosh*

Main category: cs.LG

TL;DR: Causal-GCN is a graph convolutional framework that uses causal inference to identify brain regions with stable causal influence on Alzheimer's disease progression, addressing confounding factors like age, sex, and APOE4 genotype.


<details>
  <summary>Details</summary>
Motivation: Most deep graph learning models for Alzheimer's disease classification remain correlational and confound demographic/genetic factors with disease-specific features, limiting interpretability and causal understanding.

Method: Represents MRI as structural connectomes with brain regions as nodes and anatomical connectivity as edges. Integrates do-calculus-based back-door adjustment to handle confounders, simulates interventions on regions by severing incoming edges and altering node features to estimate causal effects.

Result: Applied to 484 ADNI subjects, achieves performance comparable to baseline GNNs while providing interpretable causal effect rankings that highlight posterior, cingulate, and insular hubs consistent with established AD neuropathology.

Conclusion: Causal-GCN successfully integrates causal inference with graph learning to identify brain regions with causal influence on AD progression, offering both competitive performance and interpretable causal insights.
...
### [121] [Vehicle Routing Problems via Quantum Graph Attention Network Deep Reinforcement Learning](https://arxiv.org/abs/2511.15175)
*Le Tung Giang,Vu Hoang Viet,Nguyen Xuan Tung,Trinh Van Chien,Won-Joo Hwang*

Main category: cs.LG

TL;DR: Proposes Quantum Graph Attention Network (Q-GAT) using parameterized quantum circuits to replace MLPs in DRL for vehicle routing, reducing parameters by 50% while improving performance.


<details>
  <summary>Details</summary>
Motivation: Classical DRL models with GNNs use large MLPs that are parameter-heavy and memory-bound, limiting efficiency for large-scale routing problems.

Method: Hybrid quantum-classical approach where parameterized quantum circuits replace MLPs at readout stages in Graph Attention Networks, combined with proximal policy optimization and greedy/stochastic decoding.
...
Conclusion: PQC-enhanced GNNs demonstrate potential as compact and effective solvers for large-scale routing and logistics optimization problems.
...
### [122] [How to Train Private Clinical Language Models: A Comparative Study of Privacy-Preserving Pipelines for ICD-9 Coding](https://arxiv.org/abs/2511.14936)
*Mathieu Dufour,Andrew Duncan*

Main category: cs.LG

TL;DR: Knowledge distillation from DP-trained teachers outperforms direct DP-SGD and DP-synthetic data for clinical diagnostic coding, recovering up to 63% of non-private performance while maintaining strong privacy.


<details>
  <summary>Details</summary>
Motivation: Large language models trained on clinical text risk exposing sensitive patient information, but differential privacy methods often severely degrade diagnostic accuracy needed for deployment.
...
Result: At moderate privacy budgets (ε ∈ {4,6}), knowledge distillation from DP-trained teachers recovered up to 63% of non-private performance while maintaining strong empirical privacy (membership-inference AUC ≈ 0.5).
...
### [123] [Knowledge Graphs as Structured Memory for Embedding Spaces: From Training Clusters to Explainable Inference](https://arxiv.org/abs/2511.14961)
*Artur A. Oliveira,Mateus Espadoto,Roberto M. Cesar,Roberto Hirata*

Main category: cs.LG

TL;DR: Graph Memory (GM) is a structured non-parametric framework that uses prototype-based relational memory to enhance embedding-based inference with better calibration and efficiency.


<details>
  <summary>Details</summary>
Motivation: To bridge local evidence and global consistency in non-parametric learning by summarizing embedding spaces into structured relational memory rather than treating instances in isolation.

Method: Summarizes embedding space into prototype nodes with reliability indicators, connected by edges encoding geometric and contextual relations, unifying instance retrieval, prototype reasoning, and graph-based label propagation.

Result: Achieves accuracy competitive with kNN and Label Spreading with substantially better calibration, smoother decision boundaries, and order of magnitude fewer samples on synthetic and real datasets including breast histopathology.

Conclusion: GM provides a principled framework that explicitly models reliability and relational structure to improve non-parametric learning through better integration of local evidence and global consistency.
...
### [124] [IonCast: A Deep Learning Framework for Forecasting Ionospheric Dynamics](https://arxiv.org/abs/2511.15004)
...
TL;DR: IonCast is a deep learning suite using GraphCast-inspired models to forecast global Total Electron Content (TEC) in the ionosphere, showing improved accuracy over persistence models for both storm-time and quiet conditions.


<details>
  <summary>Details</summary>
Motivation: The ionosphere significantly impacts GNSS accuracy, communications, and aviation operations, creating a need for accurate forecasting and modeling of ionospheric variability to enhance space weather resilience.

Method: Uses GraphCast-inspired deep learning models with spatiotemporal learning to forecast global TEC, integrating diverse physical drivers and observational datasets through scalable graph-based approaches.

Result: Validation on held-out storm-time and quiet conditions demonstrates improved skill compared to persistence models, showing enhanced forecasting accuracy.

Conclusion: IonCast demonstrates how machine learning can augment physical understanding of ionospheric variability and advance operational space weather resilience by unifying heterogeneous data with scalable graph-based spatiotemporal learning.
...
### [125] [Simulated Human Learning in a Dynamic, Partially-Observed, Time-Series Environment](https://arxiv.org/abs/2511.15032)
*Jeffrey Jiang,Kevin Hong,Emily Kuczynski,Gregory Pottie*

Main category: cs.LG

TL;DR: This paper develops reinforcement learning intelligent tutoring systems that use probing interventions to balance information gathering with student learning in simulated classroom environments.


<details>
  <summary>Details</summary>
Motivation: Intelligent tutoring systems need to handle unique new students while dealing with the partial observability of the learning process, requiring better ways to estimate individual student states while leveraging population information.

Method: Created a dynamic time-series simulation environment with student-teacher interventions (tutoring, lectures, exams), developed RL ITSs that combine individual student state learning with population information through probing interventions, and compared RL algorithms with greedy rule-based heuristics.

Result: RL and heuristic approaches provided different solutions with similar results. Probing interventions reduced estimation difficulty but required cost-benefit balancing. RL policies struggled with harder classes, and both approaches showed flexibility with changing student distributions. Quiz/midterm structures benefited more than finals-only structures.

Conclusion: Probing interventions provide valuable information for ITSs, with both RL and heuristic methods showing promise, though course structure significantly impacts effectiveness, with more frequent assessments providing better opportunities for intervention.
...
### [126] [Oversampling techniques for predicting COVID-19 patient length of stay](https://arxiv.org/abs/2511.15048)
*Zachariah Farahany,Jiawei Wu,K M Sajjadul Islam,Praveen Madiraju*

Main category: cs.LG

TL;DR: Using EHR data and LOS as severity measure, the paper addresses COVID-19 severity prediction as an imbalanced classification problem through synthetic oversampling and ANN with Bayesian hyperparameter optimization.


<details>
  <summary>Details</summary>
Motivation: COVID-19 severity varies widely, with some patients experiencing lengthy hospital stays or death. Predicting severity using EHR data can help in resource allocation and treatment planning.

Method: Synthetic oversampling of imbalanced training data, followed by ANN training with hyperparameter tuning using Bayesian optimization to maximize F1 score.

Result: The approach successfully addresses the class imbalance problem in COVID-19 severity prediction, with model selection based on optimal F1 score performance.

Conclusion: The proposed methodology effectively handles imbalanced COVID-19 severity classification using EHR data and advanced machine learning techniques.
...
### [127] [Interpretable temporal fusion network of multi- and multi-class arrhythmia classification](https://arxiv.org/abs/2511.15062)
*Yun Kwan Kim*

Main category: cs.LG

TL;DR: A framework combining local-global feature extraction and attention-based fusion for arrhythmia classification that handles varying arrhythmia durations and precisely identifies onset/ending points.


<details>
  <summary>Details</summary>
Motivation: Existing CDSS methods for arrhythmia classification fail to account for varying arrhythmia lengths and onset times, limiting their clinical applicability.

Method: Proposed framework with (i) local and global feature extraction and (ii) local-global information fusion using attention mechanisms to process constrained input lengths for arrhythmia detection and classification.

Result: Achieved F1-scores of 96.45% (duration), 82.05% (episode), 96.31% (Dice) on MITDB and 97.57%, 98.31%, 97.45% on AFDB, outperforming benchmark models with superior generalization capability.
...
### [128] [Deep Pathomic Learning Defines Prognostic Subtypes and Molecular Drivers in Colorectal Cancer](https://arxiv.org/abs/2511.15067)
*Zisong Wang,Xuanyu Wang,Hang Chen,Haizhou Wang,Yuxin Chen,Yihang Xu,Yunhe Yuan,Lihuan Luo,Xitong Ling,Xiaoping Liu*

Main category: cs.LG

TL;DR: Developed TDAM-CRC, an AI model using histopathology images for colorectal cancer prognosis, validated across cohorts, integrated multi-omics data, identified MRPL37 as key biomarker, and created clinical nomogram.


<details>
  <summary>Details</summary>
Motivation: Conventional TNM staging is inadequate for personalized CRC medicine due to tumor heterogeneity; need for more precise prognostic stratification tools.

Method: Multiple instance learning model trained on TCGA cohort (n=581), validated externally (n=1031), integrated multi-omics data, interaction network analysis, and nomogram construction.

Result: TDAM-CRC outperformed clinical staging and state-of-the-art models; high-risk subtype linked to metabolic reprogramming and immunosuppression; MRPL37 identified as independent favorable prognostic biomarker.

Conclusion: TDAM-CRC provides robust risk stratification, reveals molecular targets, and facilitates personalized clinical decision-making for colorectal cancer patients.
...
### [129] [Fourier-KAN-Mamba: A Novel State-Space Equation Approach for Time-Series Anomaly Detection](https://arxiv.org/abs/2511.15083)
*Xiancheng Wang,Lin Wang,Rui Wang,Zhibo Zhang,Minghang Zhao*

Main category: cs.LG

TL;DR: Fourier-KAN-Mamba: A hybrid architecture combining Fourier layers, KAN networks, and Mamba state-space models for superior time-series anomaly detection performance.


<details>
  <summary>Details</summary>
Motivation: Mamba-based models show efficiency in long-sequence modeling but struggle with complex temporal patterns and nonlinear dynamics in anomaly detection tasks.

Method: Integrates Fourier layer for multi-scale frequency features, KAN for enhanced nonlinear representation, and temporal gating control mechanism to distinguish normal/anomalous patterns.

Result: Significantly outperforms state-of-the-art approaches on MSL, SMAP, and SWaT datasets.

Conclusion: The proposed hybrid architecture effectively addresses challenges in time-series anomaly detection by combining frequency analysis, nonlinear modeling, and selective state-space processing.
...
### [130] [Semiconductor Industry Trend Prediction with Event Intervention Based on LSTM Model in Sentiment-Enhanced Time Series Data](https://arxiv.org/abs/2511.15112)
...
<details>
  <summary>Details</summary>
Motivation: Traditional data analysis methods perform poorly with high variety and time series data in rapidly changing semiconductor markets, necessitating more advanced approaches.

Method: Collected textual and time series data from TSMC seasonal reports, applied sentiment analysis considering internal/external event interventions, and used LSTM model for prediction.

Result: Prediction revealed significant wafer technology developments and global market threats, matching actual TSMC product releases and international news.

Conclusion: The approach accurately predicts semiconductor industry trends by considering both internal and external events, providing valuable insights for research and business applications.
...
TL;DR: Bayesian online learning framework for RF passive components modeling that achieves 35x speedup using only 2.86% of EM simulation time compared to traditional ML methods.


<details>
  <summary>Details</summary>
Motivation: Conventional RF passive component modeling requires extensive EM simulations, creating computational bottlenecks in geometric and frequency design spaces.

Method: Uses Bayesian neural network with reconfigurable heads for joint geometric-frequency domain modeling with uncertainty quantification, plus adaptive sampling strategy that optimizes training data sampling using uncertainty guidance.

Result: Validated on three RF passive components, achieving accurate modeling with only 2.86% EM simulation time compared to traditional ML-based flow (35x speedup).

Conclusion: The proposed uncertainty-aware Bayesian online learning framework provides highly efficient parametric modeling of RF passive components with significant computational savings.
...
TL;DR: A novel sparse matrix multiplication algorithm enables self-organizing map analysis of the entire Medline database, overcoming previous computational limitations.


<details>
  <summary>Details</summary>
Motivation: Previous Medline mapping efforts were limited to small data subsets due to exponentially increasing memory and processing demands of existing algorithms.

Method: Designed a novel algorithm for sparse matrix multiplication that allows application of self-organizing maps to the entire Medline dataset.

Result: Successfully created a more complete map of existing medical knowledge and increased feasibility for refining the map over time.

Conclusion: The new algorithm enables comprehensive mapping of Medline data and supports ongoing updates to account for dataset changes.
...
TL;DR: GRPO-Verif is a reinforcement learning algorithm that jointly optimizes solution generation and self-verification in LLMs using a unified loss function with adjustable verification weight.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with consistent self-verification of their reasoning traces, raising questions about how to enhance this ability and whether it can improve reasoning performance.

Method: Proposed GRPO-Verif algorithm that jointly optimizes solution generation and self-verification within a unified loss function, with an adjustable hyperparameter controlling verification signal weight.

Result: Experimental results show the method enhances self-verification capability while maintaining comparable reasoning performance.

Conclusion: Joint optimization of generation and verification through GRPO-Verif successfully improves LLMs' self-verification ability without compromising reasoning quality.
...
TL;DR: Uncertainty-aware active learning framework for EEG-based emotion recognition that uses cross-modal consistency to distinguish between cognitive ambiguity and sensor noise, improving robustness to label noise.


<details>
  <summary>Details</summary>
Motivation: EEG signals are noisy and emotional labels are subjective, making robust affective decoding challenging. Need methods that can handle label noise and individual variability in EEG-based emotion recognition.

Method: Jointly leverages model uncertainty and cross-modal consistency. Uses representation alignment module to embed EEG and face features into shared latent space. Selectively queries samples with residual discrepancies for oracle feedback during active learning.

Result: Proposed framework shows efficiency and robustness on ASCERTAIN dataset, demonstrating potential as data-efficient and noise-tolerant approach for EEG-based affective decoding.

Conclusion: The uncertainty-aware active learning framework with cross-modal alignment effectively improves robustness to label noise in EEG-based emotion recognition, making it suitable for brain-computer interface systems.
...
TL;DR: Complex VAEs exhibit Kähler geometric structure in their latent space, with the Fisher information metric corresponding to the Hessian of KL divergence. The authors propose an efficient computational method using Kähler potential derivatives of complex Gaussian mixtures.


<details>
  <summary>Details</summary>
Motivation: To extend Riemannian structure analysis from real-valued VAEs to complex VAEs and explore their Kähler geometric properties, particularly focusing on decoder geometry.

Method: Derived Fisher information metric for complex VAEs with complex Gaussian regularization, established connection to KL divergence Hessian, and proposed efficient computation using Kähler potential derivatives of complex Gaussian mixtures as plurisubharmonic functions.

Result: The method enables efficient computation of the metric, allows latent space regularization with decoder geometry, and facilitates sampling with weighted complex volume elements, leading to smoother representations and fewer semantic outliers.

Conclusion: Complex VAEs naturally admit Kähler geometric structure, and the proposed computational approach provides an efficient way to leverage this geometry for improved latent space organization and sampling.
...
<details>
  <summary>Details</summary>
Motivation: Fault data scarcity in industrial monitoring hinders data-driven approaches due to rare fault events and high annotation costs, while existing models struggle with authenticity and diversity in few-shot scenarios.

Method: Uses diffusion models with positive-negative difference adapter to leverage pre-trained normal data distributions for modeling fault domain discrepancies, plus diversity loss to prevent mode collapse through inter-sample difference regularization.
...
Conclusion: The proposed framework effectively addresses few-shot fault time-series generation challenges by leveraging normal data distributions and ensuring sample diversity, enabling better fault diagnosis in industrial monitoring.
...
### [137] [Masked Auto-Regressive Variational Acceleration: Fast Inference Makes Practical Reinforcement Learning](https://arxiv.org/abs/2511.15190)
*Yuxuan Gu,Weimin Bai,Yifei Wang,Weijian Luo,He Sun*

Main category: cs.LG

TL;DR: MARVAL is a distillation framework that accelerates masked auto-regressive diffusion models by compressing the diffusion chain into a single generation step, enabling 30x speedup while maintaining quality and enabling practical RL post-training.


<details>
  <summary>Details</summary>
Motivation: Vanilla masked auto-regressive diffusion models suffer from slow inference due to hierarchical generation (outer AR loop + inner diffusion chain), which hinders efficiency and practical RL applications.

Method: Introduces MARVAL with a novel score-based variational objective for distilling diffusion models into single-step generation, and MARVAL-RL for efficient reinforcement learning with verifiable rewards.

Result: MARVAL-Huge achieves FID of 2.00 on ImageNet 256×256 with >30x speedup vs MAR-diffusion. MARVAL-RL improves CLIP and image-reward scores on ImageNet datasets with entity names.
...
### [138] [Reasoning in Diffusion Large Language Models is Concentrated in Dynamic Confusion Zones](https://arxiv.org/abs/2511.15208)
*Ranfei Chen,Ming Chen,Kaifei Wang*

Main category: cs.LG

TL;DR: ATPO is a lightweight RL method that dynamically reallocates gradient updates to high-leverage "zones of confusion" in diffusion LLM trajectories, improving reasoning accuracy and training stability without changing the RL objective or compute budget.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory-based RL methods treat all denoising steps as equally important, but analysis reveals structured "zones of confusion" - transient uncertainty spikes that strongly predict final success/failure.

Method: Proposed Adaptive Trajectory Policy Optimization (ATPO) using step-level metrics (entropy uncertainty, Confidence-Margin uncertainty, Rate of Entropy Change) to dynamically select high-leverage steps for gradient updates via a hybrid RoEC+CM rule.

Result: ATPO delivers substantial gains in reasoning accuracy and training stability across benchmarks, showing that exploiting trajectory dynamics is key to advancing dLLM RL.

Conclusion: Not all denoising steps are equally important; focusing gradient updates on high-leverage "zones of confusion" significantly improves diffusion LLM performance without additional computational cost.
...
### [139] [D2D Power Allocation via Quantum Graph Neural Network](https://arxiv.org/abs/2511.15246)
*Tung Giang Le,Xuan Tung Nguyen,Won-Joo Hwang*

Main category: cs.LG

TL;DR: A quantum Graph Neural Network (QGNN) using Parameterized Quantum Circuits for wireless resource management, matching classical performance with fewer parameters and parallelism.


<details>
  <summary>Details</summary>
Motivation: Increasing wireless network complexity requires scalable resource management, but classical GNNs have high computational costs in large-scale settings.

Method: Quantum Graph Convolutional Layers (QGCLs) encode features into quantum states, process graphs with NISQ-compatible unitaries, and retrieve embeddings through measurement via Parameterized Quantum Circuits.
...
### [140] [EntroPIC: Towards Stable Long-Term Training of LLMs via Entropy Stabilization with Proportional-Integral Control](https://arxiv.org/abs/2511.15248)
*Kai Yang,Xin Xu,Yangkun Chen,Weijie Liu,Jiafei Lyu,Zichuan Lin,Deheng Ye,Saiyong Yang*

Main category: cs.LG

TL;DR: EntroPIC is a novel method that uses proportional-integral control to dynamically adjust loss coefficients for positive and negative samples, stabilizing entropy during long-term LLM training to prevent premature convergence and ensure optimal exploration.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods struggle to maintain appropriate entropy levels during LLM training due to mixed positive/negative samples affecting entropy differently across steps, leading to unstable exploration and potential collapse into sub-optimal behaviors.

Method: Proposed EntroPIC method that adaptively tunes loss coefficients for positive and negative samples using proportional-integral control to stabilize entropy throughout training.

Result: Experimental results show EntroPIC successfully maintains desired entropy levels, enabling stable and optimal RL training for LLMs in both on-policy and off-policy settings.

Conclusion: EntroPIC provides effective entropy stabilization for large-scale LLM training, ensuring efficient exploration and preventing premature convergence through dynamic adjustment of sample influence.
...
### [141] [Optimized scheduling of electricity-heat cooperative system considering wind energy consumption and peak shaving and valley filling](https://arxiv.org/abs/2511.15250)
*Jin Ye,Lingmei Wang,Shujian Zhang,Haihang WU*

Main category: cs.LG

TL;DR: Proposes PVTD3 algorithm for combined power-heat systems with renewable energy, reducing costs by 6.93-13.59% and grid power fluctuations by 12.8% while improving energy storage management.
...
Result: PVTD3 reduces comprehensive costs by 6.93%, 12.68%, and 13.59% at 10%, 20%, and 30% renewable penetration respectively; reduces grid power purchase fluctuations by 12.8%; improves thermal storage management with 7.67-17.67 unit reduction in low-temperature tanks.
...
### [142] [KrawtchoukNet: A Unified GNN Solution for Heterophily and Over-smoothing with Adaptive Bounded Polynomials](https://arxiv.org/abs/2511.15327)
*Huseyin Goksu*

Main category: cs.LG

TL;DR: KrawtchoukNet is a spectral GNN using Krawtchouk polynomials that solves over-smoothing at high polynomial degrees and performs well on heterophilic graphs by using bounded recurrence coefficients and learnable shape parameters.


<details>
  <summary>Details</summary>
Motivation: Standard spectral GNNs like ChebyNet suffer from performance collapse on heterophilic graphs and over-smoothing at high polynomial degrees due to static, low-pass filters.

Method: Proposes KrawtchoukNet with two key designs: 1) fixing polynomial domain N to create bounded recurrence coefficients for over-smoothing robustness, 2) making shape parameter p learnable for adaptive spectral response.

Result: Achieves SOTA performance at high polynomial degrees (K=10) and on heterophilic benchmarks (Texas, Cornell), outperforming GAT and APPNP.

Conclusion: KrawtchoukNet provides a unified solution to both over-smoothing and heterophilic graph challenges through bounded coefficients and adaptive filtering.
...
TL;DR: PLATONT is a unified framework for network tomography that models different network indicators as projections of a shared latent network state, improving cross-task generalization and performance.


<details>
  <summary>Details</summary>
Motivation: Existing network tomography methods solve problems separately using limited task-specific signals, which limits generalization and interpretability.

Method: Models network indicators as projections of shared latent state using multimodal alignment and contrastive learning; trains multiple tomography tasks in shared latent space.

Result: Consistently outperforms existing methods in link estimation, topology inference, and traffic prediction with higher accuracy and stronger robustness under varying network conditions.

Conclusion: PLATONT provides a unified approach that builds compact structured representations for improved cross-task generalization in network tomography.
...
### [144] [LaguerreNet: Advancing a Unified Solution for Heterophily and Over-smoothing with Adaptive Continuous Polynomials](https://arxiv.org/abs/2511.15328)
*Huseyin Goksu*

Main category: cs.LG

TL;DR: LaguerreNet introduces continuous Laguerre polynomial filters for GNNs, addressing heterophilic graph performance and over-smoothing through trainable spectral shaping and numerical stabilization.


<details>
  <summary>Details</summary>
Motivation: Standard spectral GNNs perform poorly on heterophilic graphs and suffer from over-smoothing at high polynomial degrees due to static, low-pass filters.

Method: Proposes LaguerreNet using continuous Laguerre polynomials with trainable alpha parameter for adaptive filtering, stabilized with LayerNorm to handle numerical instability from unbounded coefficients.

Result: Achieves SOTA on heterophilic benchmarks and exceptional robustness to over-smoothing, with performance peaking at K=10 (10x beyond ChebyNet's collapse point).

Conclusion: LaguerreNet provides an effective unified solution for heterophilic graphs and over-smoothing through adaptive polynomial filters with numerical stabilization.
...
TL;DR: GRPO-RM extends Group Relative Policy Optimization from LLMs to representation models by using predefined output sets instead of token sampling and designing specialized reward functions.


<details>
  <summary>Details</summary>
Motivation: To investigate whether GRPO, proven effective for LLM fine-tuning, can be generalized to representation learning models.

Method: Proposes GRPO-RM which establishes predefined output sets to replace token sequence sampling and designs specialized reward functions for representation models.

Result: Extensive experiments on real-world datasets validate the effectiveness of the proposed GRPO-RM method.

Conclusion: GRPO can be successfully generalized to representation models through the proposed GRPO-RM approach with appropriate adaptations.
...
### [146] [SNAP: Low-Latency Test-Time Adaptation with Sparse Updates](https://arxiv.org/abs/2511.15276)
...
<details>
  <summary>Details</summary>
Motivation: Existing TTA methods require frequent adaptation and high computational costs, making them unsuitable for edge environments with limited resources.

Method: SNAP introduces two components: Class and Domain Representative Memory (CnDRM) to store representative samples, and Inference-only Batch-aware Memory Normalization (IoBMN) to dynamically adjust normalization statistics using these samples.

Result: SNAP reduces latency by up to 93.12% while keeping accuracy drop below 3.3%, even when adapting with only 1% of data. It maintains competitive accuracy across adaptation rates from 1% to 50%.
...
### [147] [Quant-Trim in Practice: Improved Cross-Platform Low-Bit Deployment on Edge NPUs](https://arxiv.org/abs/2511.15300)
*Rayen Dhahri,Steffen Urban*

Main category: cs.LG

TL;DR: Quant-Trim is a training-phase method that creates hardware-neutral checkpoints robust to different quantization backends and precision choices, eliminating the need for per-backend retraining.
...
Result: Narrows the FP/low-bit gap, reduces dependence on compiler heuristics/calibration, avoids per-backend retraining. Reports accuracy and edge metrics (latency, throughput, energy/inference, cost) under static/dynamic activation scaling and varying operator coverage.

Conclusion: Quant-Trim produces robust hardware-neutral checkpoints that work consistently across different quantization backends and precision choices without requiring backend-specific modifications.
...
### [148] [On the Internal Semantics of Time-Series Foundation Models](https://arxiv.org/abs/2511.15324)
*Atharva Pandey,Abhilash Neog,Gautam Jajoo*

Main category: cs.LG

TL;DR: Systematic investigation of concept interpretability in Time-series Foundation Models (TSFMs), revealing layer-specific concept encoding, linear recoverability limitations, and compositional interference challenges.


<details>
  <summary>Details</summary>
Motivation: Despite empirical success of TSFMs, their internal mechanisms for representing fundamental time-series concepts remain poorly understood, necessitating systematic analysis of concept interpretability.

Method: Used layer-wise analyses, linear recoverability tests, and representation similarity measures to probe concept encoding across model depth and compositional settings.

Result: Early layers capture local time-domain patterns (AR(1), level shifts, trends), deeper layers encode dispersion and change-time signals, with spectral and warping factors being hardest to recover linearly. Compositional settings show probe performance degradation due to concept interference.

Conclusion: While atomic concepts are reliably localized in TSFMs, composition remains a challenge, highlighting a key limitation in current models' ability to represent interacting temporal phenomena.
...
### [149] [STREAM-VAE: Dual-Path Routing for Slow and Fast Dynamics in Vehicle Telemetry Anomaly Detection](https://arxiv.org/abs/2511.15339)
*Kadir-Kaan Özer,René Ebeling,Markus Enzweiler*

Main category: cs.LG

TL;DR: STREAM-VAE is a variational autoencoder that separates slow drift and fast spike dynamics in automotive telemetry data for improved anomaly detection.


<details>
  <summary>Details</summary>
Motivation: Automotive telemetry data contains mixed time scales (slow drifts and fast spikes) that standard reconstruction-based methods struggle with, as they use single latent processes that smooth out spikes or inflate variances.
...
Result: Experiments on automotive telemetry data and SMD benchmark show improved robustness compared to forecasting, attention, graph, and VAE baselines.

Conclusion: Explicitly separating drift and spike dynamics enhances anomaly detection performance and produces stable anomaly scores suitable for deployment in vehicle monitors and fleet analytics.
...
TL;DR: Systematic exploration of 33 ensemble models for time series forecasting shows stacking consistently improves accuracy, with proposed multi-layer stacking framework providing superior performance across diverse scenarios.


<details>
  <summary>Details</summary>
Motivation: Ensemble methods are underutilized in time series forecasting despite their effectiveness in other ML domains, with simple linear combinations still considered state-of-the-art.

Method: Evaluated 33 ensemble models across 50 real-world datasets, proposed a multi-layer stacking framework that combines different stacker models.
...
TL;DR: CAP framework integrates ML predictions with cost-benefit analysis using LLM agents to improve clinical decision support transparency.


<details>
  <summary>Details</summary>
Motivation: Current ML predictive models lack consideration of downstream value trade-offs and clinical interpretability, limiting their practical utility in healthcare settings.
...
Conclusion: CAP successfully combines ML classifier outcomes with cost-benefit analysis using LLM agents to provide more transparent and interpretable decision support in clinical settings.
...
TL;DR: A novel post-hoc local feature importance method called Counterfactual Importance Distribution (CID) that ranks features using distributional dissimilarity between positive and negative counterfactuals.


<details>
  <summary>Details</summary>
Motivation: Need for well-founded feature importance measures due to lack of definitive ground truth in ML model interpretation, requiring alternative rigorous approaches.

Method: Generate positive and negative counterfactuals, model their distributions using Kernel Density Estimation, and rank features based on a distributional dissimilarity measure that satisfies metric properties.

Result: CID outperforms established local feature importance methods on faithfulness metrics (comprehensiveness and sufficiency), providing more faithful explanations.

Conclusion: CID offers complementary perspectives to existing approaches and serves as a valuable tool for model analysis with improved faithfulness.
...
TL;DR: PIECE is a parameter-efficient continual learning method that selectively updates only 0.1% of core parameters to prevent catastrophic forgetting while acquiring new domain knowledge, without needing prior data or extra parameters.


<details>
  <summary>Details</summary>
Motivation: Domain-specific post-training causes catastrophic forgetting in foundation models, limiting their adaptability. Traditional continual learning methods have poor performance, data dependency, or parameter overhead issues.

Method: PIECE uses parameter importance estimation (Fisher Information or second-order normalization) to selectively update only 0.1% of most relevant parameters for new tasks, without accessing prior training data.

Result: Experiments across 3 language models and 2 multimodal models show PIECE maintains general capabilities and achieves state-of-the-art continual learning performance across diverse downstream tasks.

Conclusion: PIECE provides a practical path to scalable, domain-adaptive foundation models without catastrophic forgetting, preserving general reasoning while efficiently learning domain knowledge.
...
TL;DR: EVA-Net is an interpretable brain age estimation framework that treats brain age as an anomaly detection problem using EEG data, achieving state-of-the-art accuracy while providing interpretable insights into deviations from healthy aging patterns.


<details>
  <summary>Details</summary>
Motivation: Existing EEG-based brain age models struggle with imperfect medical data and lack interpretability, making it difficult to identify disease-related anomalies from healthy baseline data.

Method: Uses a sparsified-attention Transformer for long EEG sequences, Variational Information Bottleneck for robust representation learning, and aligns representations to a continuous prototype network that learns the normative healthy aging manifold.

Result: Achieved state-of-the-art accuracy on 1297 healthy subjects and successfully detected anomalies in 27 MCI/AD patients with significantly higher brain-age gaps and Prototype Alignment Error.

Conclusion: EVA-Net provides an interpretable framework for healthcare intelligence using imperfect medical data, enabling effective anomaly detection for neurological disorders.
...
TL;DR: A variational Lagrangian framework for nonlinear, non-Gaussian state estimation using entropic trust-region updates and dynamic constraints.


<details>
  <summary>Details</summary>
Motivation: To develop efficient algorithms for state estimation in nonlinear, non-Gaussian state-space models where traditional methods may be inadequate.
...
Result: Derived recursive schemes with favorable computational complexity suitable for nonlinear, non-Gaussian state estimation problems.

Conclusion: The proposed framework provides a principled approach to Bayesian inference in complex state-space models through variational methods and efficient approximations.
...
TL;DR: Analysis of layer contributions in tabular ICL models reveals structural redundancy and evolution patterns similar to LLMs, offering compression opportunities.


<details>
  <summary>Details</summary>
Motivation: Despite architectural similarities between tabular ICL models and LLMs, little is known about how individual layers contribute to tabular prediction, motivating investigation into layer dynamics and redundancy.

Method: Analyze TabPFN and TabICL through the 'layers as painters' perspective to study how latent spaces evolve across layers and identify redundant layers.

Result: Only subsets of layers share a common representational language, suggesting structural redundancy in tabular ICL models similar to patterns observed in LLMs.

Conclusion: The identified structural redundancy offers opportunities for model compression and improved interpretability in tabular ICL models.
...
TL;DR: A method for classifying time-series data using in-context learning with pre-trained foundation models, eliminating the need for fine-tuning by incorporating examples directly in the prompt.


<details>
  <summary>Details</summary>
Motivation: To enable classification of data not included in the training corpus of time-series foundation models without requiring model fine-tuning, moving beyond custom narrow AI solutions towards broader AI-driven maintenance systems.

Method: Represent examples as targets (class IDs) and covariates (data matrices) within the model prompt, transform frequency domain signals into pseudo time-series patterns, generate aligned covariate and target signals, and use TSFM to predict classification probabilities through in-context learning.

Result: Successfully applied to vibration data for bearing health assessment in servo-press motors, demonstrating efficacy across varied operational conditions by leveraging the scalability of pre-trained models.

Conclusion: This approach represents significant progress from custom narrow AI solutions to broader, scalable AI-driven maintenance systems using in-context learning with foundation models.
...
TL;DR: FairEnergy is a fairness-aware energy minimization framework for federated learning that optimizes device selection, bandwidth allocation, and compression levels while considering client contributions.


<details>
  <summary>Details</summary>
Motivation: Federated learning faces challenges in balancing energy efficiency, fair participation, and model accuracy in wireless edge systems due to heterogeneous resources, unequal client contributions, and limited communication capacity.

Method: Proposes FairEnergy framework that integrates contribution scores (capturing update magnitude and compression ratio) into joint optimization. Solves mixed-integer non-convex problem via binary variable relaxation, Lagrangian decomposition for bandwidth coupling, and per-device subproblem optimization.
...
TL;DR: NINT accelerates Implicit Neural Representations training by using Neural Tangent Kernel to dynamically select coordinates that maximize global functional updates, reducing training time by nearly half while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Fitting high-resolution signals with INRs requires optimizing over millions of coordinates, which incurs prohibitive computational costs that need to be addressed.
...
Conclusion: NINT provides an effective method for accelerating INR training through intelligent coordinate selection using NTK guidance, achieving faster convergence with maintained quality.
...
TL;DR: The paper studies the tradeoff between sample complexity and round complexity in on-demand sampling for Multi-Distribution Learning (MDL), establishing optimal bounds for both realizable and agnostic settings.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental tradeoffs between sample complexity and round complexity in adaptive sampling scenarios, particularly in Multi-Distribution Learning where algorithms can sample from multiple distributions over limited rounds.

Method: Introduces Optimization via On-Demand Sampling (OODS) framework to abstract sample-adaptivity tradeoffs, analyzes round complexity bounds in this framework, and develops algorithms for both realizable and agnostic MDL settings.

Result: For realizable MDL: optimal sample complexity scales as dk^(Θ(1/r))/ε for r-round algorithms. For agnostic MDL: achieves near-optimal sample complexity of Õ((d+k)/ε²) within Õ(√k) rounds. Establishes nearly tight bounds on round complexity in OODS framework.

Conclusion: The OODS framework captures most existing MDL algorithms and reveals inherent hardness - achieving sub-polynomial round complexity requires fundamentally new techniques that bypass OODS limitations.
...
<details>
  <summary>Details</summary>
Motivation: Existing solutions struggle to reconcile high-fidelity learning with structural requirements for verifiable control in runtime geofencing for ground vehicles.

Method: Integrates Physics-encoded Control-Affine Residual Neural Network (PCARNN) with preview-based Discrete Control Barrier Function (DCBF), preserving control-affine structure for reliable optimization and handling polygonal keep-in constraints via real-time Quadratic Program.
...
Conclusion: Structure-preserving approach enables effective runtime geofencing while maintaining verifiable control properties.
...
### [162] [The Illusion of Procedural Reasoning: Measuring Long-Horizon FSM Execution in LLMs](https://arxiv.org/abs/2511.14777)
*Mahdi Samiei,Mahdi Mansouri,Mahdieh Soleymani Baghshah*

Main category: cs.AI

TL;DR: FSM Execution is introduced as a minimal, interpretable benchmark to evaluate LLMs' procedural reasoning capacity, revealing systematic degradation with increased task complexity and highlighting limitations in multi-step rule-based computations.


<details>
  <summary>Details</summary>
Motivation: To isolate and measure LLMs' true ability for procedural reasoning (multi-step, rule-based computations) which often degrades under extended reasoning chains, lacking a controlled benchmark.

Method: Use Finite-State Machine (FSM) Execution where models are given explicit FSM definitions and must execute step-by-step with input actions, maintaining state consistency over multiple turns. Measure Turn Accuracy and Task Accuracy.

Result: Systematic degradation as task horizon or branching complexity increases. Models perform worse with high branching factors than long memory spans. Larger models show better local accuracy but remain brittle in multi-step reasoning without explicit intermediate step externalization.

Conclusion: FSM-based evaluation provides a transparent, complexity-controlled probe for diagnosing procedural reasoning failures and guiding design of inductive biases for genuine long-horizon procedural competence in LLMs.
...
### [163] [Learning Interestingness in Automated Mathematical Theory Formation](https://arxiv.org/abs/2511.14778)
*George Tsoukalas,Rahul Saha,Amitayush Thakur,Sabrina Reguyal,Swarat Chaudhuri*

Main category: cs.AI

TL;DR: FERMAT is a RL environment for automated mathematical theory discovery and theorem-proving, with an evolutionary algorithm using LLMs to synthesize interestingness measures for mathematical objects.


<details>
  <summary>Details</summary>
Motivation: To address the grand challenge of automating open-ended discovery of new mathematical theories using artificial intelligence.

Method: Introduced FERMAT RL environment for concept discovery and theorem-proving, and developed an LLM-based evolutionary algorithm with function abstraction to synthesize interestingness measures.

Result: The approach achieved notable improvements in discovering elementary number theory and finite fields over hard-coded baselines.

Conclusion: FERMAT provides a framework for automated mathematical theory discovery, with evolutionary algorithms showing promise for scoring mathematical interestingness.
...
### [164] [Ask WhAI:Probing Belief Formation in Role-Primed LLM Agents](https://arxiv.org/abs/2511.14780)
*Keith Moore,Jun W. Kim,David Lyu,Jeffrey Heo,Ehsan Adeli*

Main category: cs.AI

TL;DR: Ask WhAI is a framework for inspecting and perturbing belief states in multi-agent interactions, applied to medical diagnosis simulations to study belief formation and epistemic silos.


<details>
  <summary>Details</summary>
Motivation: To study belief formation and epistemic dynamics in multi-agent scientific reasoning, particularly how agents with disciplinary priors interact and respond to evidence in complex diagnostic scenarios.

Method: Framework records/replays agent interactions, supports out-of-band belief queries, and enables counterfactual evidence injection. Applied to medical case simulator with LLM agents (neurologist, infectious disease specialist) writing to shared EMR and interacting with moderator.

Result: Agents mirror real-world disciplinary stances - show overreliance on canonical studies and resistance to counterevidence. Beliefs can be traced and interrogated in ways not possible with human experts.

Conclusion: Ask WhAI offers reproducible way to study belief formation and epistemic silos in multi-agent scientific reasoning by making such dynamics visible and testable.
...
### [165] [Subnational Geocoding of Global Disasters Using Large Language Models](https://arxiv.org/abs/2511.14788)
*Michele Ronco,Damien Delforge,Wiebke S. Jäger,Christina Corbane*

Main category: cs.AI

TL;DR: Automated LLM-assisted workflow for geocoding disaster event locations from unstructured text in EM-DAT database using GPT-4o and multiple geoinformation sources with reliability scoring.


<details>
  <summary>Details</summary>
Motivation: Subnational location data in disaster databases like EM-DAT are often unstructured, inconsistent, and difficult to integrate with spatial datasets, limiting risk assessment capabilities.

Method: Uses GPT-4o to process and clean textual location information, then cross-checks three geoinformation repositories (GADM, OpenStreetMap, Wikidata) to assign geometries and reliability scores based on source agreement.

Result: Successfully geocoded 14,215 events across 17,948 unique locations in EM-DAT dataset from 2000-2024, with no manual intervention required.

Conclusion: LLMs can effectively extract and structure geographic information from unstructured text, providing a scalable and reliable method for disaster data analysis and enabling flexible remapping to preferred frameworks.
...
### [166] [Project Rachel: Can an AI Become a Scholarly Author?](https://arxiv.org/abs/2511.14819)
*Martin Monperrus,Benoit Baudry,Clément Vidal*

Main category: cs.AI

TL;DR: Project Rachel created an AI academic identity named Rachel So that published 10+ papers, got cited, and received peer review invitations, demonstrating how the scholarly ecosystem responds to AI authorship.
...
Method: Action research study involving creation of an AI academic identity (Rachel So) that published AI-generated research papers between March and October 2025.

Result: Rachel So successfully published 10+ papers, received citations, and was invited for peer review, showing the scholarly system can accept AI-authored work.

Conclusion: The study provides empirical evidence that current scholarly systems can accommodate AI authorship, highlighting implications for publishers, researchers, and scientific communication.
...
### [167] [Uncertainty-Aware Measurement of Scenario Suite Representativeness for Autonomous Systems](https://arxiv.org/abs/2511.14853)
...
<details>
  <summary>Details</summary>
Motivation: To ensure trustworthiness and safety of AI systems like autonomous vehicles by assessing how well training/testing data represents the target operational domain, acknowledging that true domain distribution is unknown.

Method: Imprecise Bayesian method that handles limited data and uncertain priors, comparing statistical distributions of scenario suite features with inferred target operational domain distributions across operational categories.

Result: Produces interval-valued, uncertainty-aware estimates of representativeness both locally (between categories) and globally, accounting for dependencies and prior uncertainty.

Conclusion: The proposed method provides a robust way to quantify dataset representativeness for AI safety applications while acknowledging uncertainty in the true operational domain distribution.
...
### [168] [Task Specific Sharpness Aware O-RAN Resource Management using Multi Agent Reinforcement Learning](https://arxiv.org/abs/2511.15002)
*Fatemeh Lotfi,Hossein Rajoli,Fatemeh Afghah*

Main category: cs.AI

TL;DR: Enhanced SAC algorithm with adaptive SAM regularization in MARL framework improves resource allocation efficiency by 22% in O-RAN networks.


<details>
  <summary>Details</summary>
Motivation: DRL models struggle with robustness and generalizability in dynamic O-RAN environments, requiring improved training stability and generalization capabilities.

Method: Combines Soft Actor Critic with Sharpness-Aware Minimization in distributed MARL, using TD-error variance-driven adaptive SAM regularization and dynamic ρ scheduling.

Result: Achieves 22% improvement in resource allocation efficiency and superior QoS satisfaction across diverse O-RAN slices compared to conventional DRL approaches.

Conclusion: The proposed adaptive SAM-enhanced SAC method effectively addresses robustness and generalization challenges in dynamic O-RAN resource management.
...
### [169] [Learning Human-Like RL Agents Through Trajectory Optimization With Action Quantization](https://arxiv.org/abs/2511.15055)
*Jian-Ting Guo,Yu-Cheng Chen,Ping-Chun Hsieh,Kuo-Hao Ho,Po-Wei Huang,Ti-Rong Wu,I-Chen Wu*

Main category: cs.AI

TL;DR: MAQ is a human-like RL framework that uses macro action quantization from human demonstrations to improve human-likeness while maintaining high performance.


<details>
  <summary>Details</summary>
Motivation: To address the unnatural behaviors of reward-driven RL agents compared to humans, improving interpretability and trustworthiness by achieving human-like behavior.

Method: Formulates human-likeness as trajectory optimization, uses receding-horizon control, and introduces Macro Action Quantization (MAQ) with Vector-Quantized VAE to distill human demonstrations into macro actions.

Result: Significantly improves human-likeness on D4RL Adroit benchmarks, increases trajectory similarity scores, and achieves highest human-likeness rankings among RL agents in human evaluation.
...
### [170] [Beyond GeneGPT: A Multi-Agent Architecture with Open-Source LLMs for Enhanced Genomic Question Answering](https://arxiv.org/abs/2511.15061)
*Haodong Chen,Guido Zuccon,Teerapong Leelanupab*

Main category: cs.AI

TL;DR: OpenBioLLM is a modular multi-agent framework that improves upon GeneGPT by using open-source models and specialized agents for genomic question answering, achieving comparable or better performance with reduced latency.


<details>
  <summary>Details</summary>
Motivation: To address limitations of GeneGPT including reliance on proprietary models (high costs, scalability issues, data privacy concerns) and enable more efficient genomic question answering using open-source alternatives.

Method: Developed a modular multi-agent framework with specialized agents for tool routing, query generation, and response validation, using open-source models (Llama 3.1, Qwen2.5, Qwen2.5 Coder) without additional fine-tuning.

Result: Outperforms GeneGPT on 90% of benchmark tasks, achieving average scores of 0.849 on Gene-Turing and 0.830 on GeneHop, with 40-50% latency reduction across tasks.

Conclusion: Open-source multi-agent systems show strong potential for genomic question answering, offering improved efficiency and comparable performance to proprietary solutions while addressing cost and privacy concerns.
...
### [171] [ProRAC: A Neuro-symbolic Method for Reasoning about Actions with LLM-based Progression](https://arxiv.org/abs/2511.15069)
*Haoyong Wu,Yongmei Liu*

Main category: cs.AI

TL;DR: ProRAC is a neuro-symbolic framework that uses LLMs to solve Reasoning about Actions and Change (RAC) problems by extracting actions and questions, executing actions progressively, and evaluating queries against the final state.


<details>
  <summary>Details</summary>
Motivation: To develop an effective framework for solving complex RAC problems by combining neural language models with symbolic reasoning capabilities.

Method: Extracts RAC elements (actions and questions) from problems, progressively executes each action to derive the final state, then evaluates queries against the progressed state.

Result: Achieves strong performance across different RAC benchmarks, domains, LLM backbones, and types of RAC tasks.

Conclusion: ProRAC demonstrates effective neuro-symbolic reasoning for RAC problems through progressive action execution and state evaluation.
...
### [172] [Knowledge-Informed Automatic Feature Extraction via Collaborative Large Language Model Agents](https://arxiv.org/abs/2511.15074)
*Henrik Bradland,Morten Goodwin,Vladimir I. Zadorozhny,Per-Arne Andersen*

Main category: cs.AI

TL;DR: Rogue One is a multi-agent LLM framework for automatic feature extraction that uses three specialized agents working iteratively with qualitative feedback and external knowledge integration to outperform state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based AutoFE methods are limited by monolithic architectures, simplistic quantitative feedback, and lack of systematic domain knowledge integration, which hinders their effectiveness in generating meaningful features.

Method: A decentralized multi-agent system with three specialized agents (Scientist, Extractor, Tester) that collaborate iteratively, using rich qualitative feedback, flooding-pruning strategy, and RAG-based external knowledge integration.

Result: Significantly outperforms state-of-the-art methods on 19 classification and 9 regression datasets, and qualitatively demonstrates utility for scientific discovery by identifying novel hypotheses like a potential biomarker in myocardial dataset.

Conclusion: Rogue One provides an effective framework for knowledge-informed automatic feature extraction that generates statistically powerful and semantically meaningful features, serving as a valuable tool for both machine learning and scientific discovery.
...
### [173] [SafeRBench: A Comprehensive Benchmark for Safety Assessment in Large Reasoning Models](https://arxiv.org/abs/2511.15169)
...
TL;DR: SafeRBench is the first benchmark for end-to-end safety assessment of Large Reasoning Models, evaluating risks from inputs through reasoning traces to final outputs across multiple safety dimensions.


<details>
  <summary>Details</summary>
Motivation: Current safety evaluations focus only on final outputs and fail to capture dynamic risks that emerge during chain-of-thought reasoning, where harmful content can be subtly injected or justified through misleading rationales.

Method: Three key components: (1) Input characterization with risk categories and levels accounting for affected groups and severity; (2) Fine-grained output analysis using micro-thought chunking to segment reasoning traces; (3) Human safety alignment to validate LLM-based evaluations against human annotations.

Result: Evaluation of 19 LRMs demonstrates that SafeRBench enables detailed, multidimensional safety assessment, providing insights into risks and protective mechanisms from multiple perspectives.

Conclusion: SafeRBench addresses critical gaps in LRM safety evaluation by providing comprehensive end-to-end assessment that captures dynamic risks throughout the reasoning process, offering a more robust framework for safety analysis.
...
### [174] [HISE-KT: Synergizing Heterogeneous Information Networks and LLMs for Explainable Knowledge Tracing with Meta-Path Optimization](https://arxiv.org/abs/2511.15191)
*Zhiyi Duan,Zixing Shi,Hongyu Yuan,Qi Wang*

Main category: cs.AI

TL;DR: HISE-KT integrates heterogeneous information networks with LLMs to improve knowledge tracing by automatically filtering meta-paths and retrieving similar students for better predictions and explanations.


<details>
  <summary>Details</summary>
Motivation: Existing KT methods based on HINs introduce noise through manual meta-path selection, while LLM-based methods ignore cross-student information. Both lack consistent accuracy and evidence-based explanations.

Method: Builds multi-relationship HIN with diverse node types, uses LLM to score/filter meta-path instances, retrieves similar students based on meta-paths, and integrates target student history with similar trajectories using structured prompts.

Result: Outperforms existing KT baselines on four public datasets in both prediction performance and interpretability.

Conclusion: HISE-KT successfully integrates HINs and LLMs to achieve superior knowledge tracing with automated meta-path quality assessment and evidence-backed explanations.
...
### [175] [As If We've Met Before: LLMs Exhibit Certainty in Recognizing Seen Files](https://arxiv.org/abs/2511.15192)
*Haodong Li,Jingqi Zhang,Xiao Cheng,Peihua Mai,Haoyu Wang,Yang Pan*

Main category: cs.AI

TL;DR: COPYCHECK is a novel framework that uses uncertainty signals to detect copyrighted content in LLM training data, turning LLM overconfidence into an asset for distinguishing between seen and unseen content.


<details>
  <summary>Details</summary>
Motivation: Address concerns about unauthorized use of copyrighted material in LLM training by overcoming limitations of existing Membership Inference Attacks, particularly LLM overconfidence and reliance on empirically determined thresholds.

Method: Leverages uncertainty patterns to distinguish training data, uses strategic file segmentation into smaller snippets, and implements uncertainty-guided unsupervised clustering to eliminate threshold tuning.

Result: Achieves 90.1% balanced accuracy on LLaMA 7b and 91.6% on LLaMA2 7b, with over 90% relative improvement compared to SOTA baseline, and maintains strong performance across different architectures like GPT-J 6B.

Conclusion: First application of uncertainty for copyright detection in LLMs, providing practical tools for training data transparency and effective detection of copyrighted content usage.
...
### [176] [SOLID: a Framework of Synergizing Optimization and LLMs for Intelligent Decision-Making](https://arxiv.org/abs/2511.15202)
*Yinsheng Wang,Tario G You,Léonard Boussioux,Shan Liu*

Main category: cs.AI

TL;DR: SOLID integrates mathematical optimization with LLMs through dual prices and deviation penalties, enabling iterative collaboration that improves decision quality while maintaining modularity and data privacy, with demonstrated success in stock portfolio optimization.


<details>
  <summary>Details</summary>
Motivation: To enhance intelligent decision-making by synergizing the contextual understanding of LLMs with the mathematical rigor of optimization, addressing limitations of standalone optimization methods.

Method: Developed a framework with iterative collaboration between optimization and LLM agents using dual prices and deviation penalties, with theoretical convergence guarantees under convexity assumptions.

Result: Empirical evaluation on stock portfolio investment showed convergence under various scenarios and improved annualized returns compared to baseline optimizer-only methods.
...
### [177] [Efficiency Will Not Lead to Sustainable Reasoning AI](https://arxiv.org/abs/2511.15259)
*Philipp Wiesner,Daniel W. O'Neill,Francesca Larosa,Odej Kao*

Main category: cs.AI

TL;DR: Efficiency gains alone cannot ensure sustainable reasoning AI; explicit limits in optimization and governance are needed.


<details>
  <summary>Details</summary>
Motivation: AI research is shifting towards complex problem solving with multi-step reasoning, but efficiency improvements are nearing physical limits while reasoning AI lacks saturation points, continuing to scale with exponential compute investments.

Method: The paper discusses research and policy directions to embed explicit limits into the optimization and governance of reasoning AI systems.

Result: The analysis shows that as efficiency gains approach physical limits and reasoning AI lacks natural saturation thresholds, continued exponential compute scaling poses sustainability challenges.

Conclusion: Efficiency alone is insufficient for sustainable reasoning AI; explicit limits must be integrated into both technical optimization and governance frameworks to ensure long-term sustainability.
...
### [178] [Realist and Pluralist Conceptions of Intelligence and Their Implications on AI Research](https://arxiv.org/abs/2511.15282)
*Ninell Oldenburg,Ruchira Dhar,Anders Søgaard*

Main category: cs.AI

TL;DR: The paper analyzes two competing conceptions of intelligence in AI research: Intelligence Realism (universal, measurable intelligence) vs. Intelligence Pluralism (diverse, context-dependent capacities), showing how these implicit assumptions shape research methods, interpretations, and risk assessments.


<details>
  <summary>Details</summary>
Motivation: To make explicit the underlying assumptions about intelligence that shape AI research, as these implicit views lead to fundamental disagreements in methodology, interpretation of evidence, and risk assessment across the field.

Method: Analysis of current debates in AI research to demonstrate how Intelligence Realism and Pluralism remain implicit yet fundamentally shape empirical evidence interpretation across multiple areas.

Result: Identified three key areas where these conceptions generate different approaches: methodology (model selection, benchmarks, validation), interpretation (capability emergence, limitations), and AI risk assessment (superintelligence vs. diverse domain threats).

Conclusion: Making these underlying assumptions explicit can contribute to clearer understanding of disagreements in AI research and help reconcile different perspectives on intelligence, methodology, and risk assessment.
...
### [179] [Octopus: Agentic Multimodal Reasoning with Six-Capability Orchestration](https://arxiv.org/abs/2511.15351)
*Yifu Guo,Zishan Xu,Zhiyuan Yao,Yuquan Lu,Jiaye Lin,Sen Hu,Zhenheng Tang,Yingchao Li,Huacan Wang,Ronghao Chen*

Main category: cs.AI

TL;DR: Octopus is a new multimodal reasoning paradigm that autonomously explores diverse reasoning pathways and dynamically selects appropriate capabilities from six core areas, achieving state-of-the-art performance on comprehensive benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal reasoning models lack human-like autonomous exploration abilities and struggle to adapt to dynamically changing capability requirements in real-world tasks, while humans use complementary thinking abilities that current methods only partially cover.

Method: Proposes Octopus framework with six core capabilities for multimodal reasoning, enabling autonomous exploration during reasoning and dynamic capability selection based on current state, evaluated through Octopus-Bench benchmark.

Result: Experimental results show Octopus achieves best performance on vast majority of tasks in Octopus-Bench, demonstrating superior capability coordination in agentic multimodal reasoning.

Conclusion: The six-capability orchestration paradigm in Octopus highlights the crucial role of capability coordination in agentic multimodal reasoning, providing a more human-like approach to complex multimodal tasks.
...
### [180] [Terra Nova: A Comprehensive Challenge Environment for Intelligent Agents](https://arxiv.org/abs/2511.15378)
*Trevor McInroe*

Main category: cs.AI

TL;DR: Terra Nova is a comprehensive challenge environment for RL inspired by Civilization V that integrates multiple canonical RL challenges simultaneously, requiring deep reasoning across interacting variables rather than just multitask switching.


<details>
  <summary>Details</summary>
Motivation: To create a single environment that tests integrated, long-horizon understanding across multiple interacting RL challenges simultaneously, moving beyond simple multitask benchmarks that only assess policy switching.

Method: Developed Terra Nova, a comprehensive challenge environment inspired by Civilization V that combines partial observability, credit assignment, representation learning, enormous action spaces and other canonical RL challenges in one integrated system.

Result: Created a new benchmark environment that demands deep reasoning across many interacting variables rather than just cataloging and switching among unrelated policies.

Conclusion: Terra Nova provides a more meaningful test of RL agents' capabilities by requiring integrated understanding across multiple simultaneous challenges, distinguishing it from aggregated multitask benchmarks.
...
TL;DR: IPR (Interactive Physical Reasoner) combines world-model rollouts with VLM reasoning to improve physical reasoning in games, achieving performance comparable to GPT-5 and showing steady improvement with more training.


<details>
  <summary>Details</summary>
Motivation: To investigate whether agents can acquire human-like reasoning through interaction and improve with experience, similar to how humans learn physics and causality.

Method: Proposes IPR using world-model rollouts to score and reinforce VLM policies, and introduces PhysCode for physics-centric action alignment. Trained on 1,000+ heterogeneous games with diverse physical mechanisms.

Result: IPR performs robustly across three human-like reasoning levels (Survival, Curiosity, Utility), matches GPT-5 overall, surpasses it on Curiosity, and shows zero-shot transfer to unseen games.

Conclusion: Physics-centric interaction provides a path to steadily improving physical reasoning, with performance scaling with more training games and interaction steps.
...
### [182] [Know Your Intent: An Autonomous Multi-Perspective LLM Agent Framework for DeFi User Transaction Intent Mining](https://arxiv.org/abs/2511.15456)
*Qian'ang Mao,Yuxuan Zhang,Jiaman Chen,Wenjun Zhou,Jiaqi Yan*

Main category: cs.AI

TL;DR: TIM framework uses multi-agent LLM system with DeFi intent taxonomy to infer user transaction intents, outperforming existing methods by addressing complex smart contract interactions and opaque blockchain data.
...
Method: Proposes Transaction Intent Mining (TIM) framework with DeFi intent taxonomy built on grounded theory and multi-agent LLM system featuring Meta-Level Planner, Question Solvers, and Cognitive Evaluator to decompose and verify intent analysis.

Result: TIM significantly outperforms machine learning models, single LLMs, and single Agent baselines in experiments, effectively addressing core challenges in intent inference.

Conclusion: TIM provides reliable understanding of user motivations in DeFi and offers context-aware explanations for complex blockchain activity, advancing semantic analysis of DeFi transactions.
...
### [183] [Exploring the use of AI authors and reviewers at Agents4Science](https://arxiv.org/abs/2511.15534)
...
Method: Organized Agents4Science conference where AI agents functioned as both primary authors and reviewers, with humans in supporting roles as co-authors and co-reviewers.

Result: The conference provided key learnings about AI agents' performance in scientific roles and human-AI collaboration dynamics.

Conclusion: The findings have important implications for future human-AI collaboration in scientific research and understanding AI capabilities in scientific roles.
...
### [184] [What Does It Take to Be a Good AI Research Agent? Studying the Role of Ideation Diversity](https://arxiv.org/abs/2511.15593)
...
TL;DR: Higher ideation diversity in AI research agents leads to better performance on MLE-bench, with controlled experiments confirming this relationship across different models and evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: AI research agents can accelerate scientific progress but the factors driving their success are not well understood, particularly the role of ideation diversity in agent performance.
...
Result: Higher-performing agents showed increased ideation diversity, and controlled experiments demonstrated that higher ideation diversity directly results in stronger performance across various metrics.

Conclusion: Ideation diversity is a key factor in AI research agent success, with increased diversity leading to better performance across different models and evaluation frameworks.
...
### [185] [Channel Coding for Gaussian Channels with Multifaceted Power Constraints](https://arxiv.org/abs/2511.14849)
*Adeel Mahmood,Aaron B. Wagner*

Main category: cs.IT

TL;DR: The paper introduces a multifaceted power model that generalizes existing power constraints and provides an exact characterization of minimum average error probability for Gaussian channels under certain conditions.


<details>
  <summary>Details</summary>
Motivation: Motivated by refined asymptotic results based on normal approximation, the study aims to understand how higher-order coding performance depends on mean power and finer statistics of input power.

Method: Introduces a multifaceted power model constraining expectations of arbitrary functions of normalized average power. The converse proof reduces code design to minimization over compact probability distributions, characterizes extreme points, and uses Bauer's maximization principle.

Result: Provides an exact characterization of minimum average error probability for Gaussian channels as a function of first- and second-order coding rates under growth and continuity assumptions.

Conclusion: The framework generalizes existing power constraint models and offers a comprehensive approach to analyzing coding performance with refined power statistics.
...
### [186] [Beyond the "G" Frontier: A Time Traveler's Century-Long Vision for Wireless Intelligence](https://arxiv.org/abs/2511.14906)
*Yasser Al Eryani*

Main category: cs.IT

TL;DR: Wireless evolution will shift from generational upgrades to a curvature-managed integration of electromagnetics, biology, thermodynamics, and cognition, forming a global ecology of self-aware information flow by 2125.


<details>
  <summary>Details</summary>
Motivation: To envision the future of wireless communication beyond incremental generational improvements like 6G or 7G, exploring a more holistic integration of multiple scientific domains.

Method: Using the Information-Curvature Efficiency Law (ICEL) as an analytical lens to project wireless evolution over the next century.

Result: Prediction of a future infrastructure that constitutes a global ecology where geometry and communication converge to sustain both technological and biological life.

Conclusion: Wireless evolution will transition from discrete generations to a curvature-managed, integrated system that forms a self-aware information flow ecosystem supporting both technology and biology.
...
### [187] [Hyper-VIB: A Hypernetwork-Enhanced Information Bottleneck Approach for Task-Oriented Communications](https://arxiv.org/abs/2511.15041)
*Jingchen Peng,Chaowen Deng,Yili Deng,Boxiang Ren,Lu Yang*

Main category: cs.IT

TL;DR: Hyper-VIB is a hypernetwork-enhanced information bottleneck method for efficient task-oriented communications in 6G systems, enabling optimal joint training of device and network models with minimal communication overhead through hyperparameter optimization.


<details>
  <summary>Details</summary>
Motivation: To enable efficient task-oriented communications in 6G collaborative intelligent systems by addressing computational intractability in high-dimensional information bottleneck optimization and avoiding the computational costs of conventional grid/random search methods.

Method: Uses hypernetwork-enhanced information bottleneck approach with variational upper-bound approximation to handle computational intractability. Introduces a hypernetwork that generates approximately optimal DNN parameters for different hyperparameter values within a single training phase.

Result: Experimental results show superior accuracy and training efficiency over conventional VIB approaches in both classification and regression tasks. Theoretical analysis in the linear case validates the hypernetwork design.

Conclusion: Hyper-VIB provides an efficient solution for task-oriented communications in 6G systems, achieving optimal trade-off between task accuracy and communication overhead with significantly reduced computational costs compared to conventional methods.
...
### [188] [Mutual Information Bounds in the Shuffle Model](https://arxiv.org/abs/2511.15051)
*Pengcheng Su,Haibo Cheng,Ping Wang*

Main category: cs.IT

TL;DR: This paper provides the first systematic information-theoretic analysis of the single-message shuffle model, studying both shuffle-only and shuffle-DP settings to derive mutual information bounds and establish connections between shuffle differential privacy and information-theoretic privacy.


<details>
  <summary>Details</summary>
Motivation: To understand the privacy guarantees of shuffle models from an information-theoretic perspective, bridging the gap between shuffle differential privacy and mutual-information-based privacy measures.

Method: Analyzes two regimes: shuffle-only (direct message submission) and shuffle-DP (local ε₀-DP mechanism before shuffling). Uses mutual information analysis with asymptotic expressions for large n, focusing on basic configuration with heterogeneous user distributions.

Result: For shuffle-DP: I(K;Z) ≤ 2ε₀ and I(X₁;Z|(X_i)_{i=2}^n) ≤ (e^{ε₀}-1)/(2n) + O(n^{-3/2}). For shuffle-only: derives asymptotic expressions for mutual information quantities as n → ∞.

Conclusion: The analysis provides fundamental information-theoretic bounds for shuffle privacy models, establishing connections between differential privacy and mutual-information privacy, with applications to heterogeneous user settings.
...
### [189] [Generalized Repetition Codes and Their Application to HARQ](https://arxiv.org/abs/2511.15207)
*Chaofeng Guan,Gaojun Luo,Lan Luo,Yangyang Fei,Hong Wang*

Main category: cs.IT

TL;DR: Proposes generalized repetition codes (GRCs) with multiple minimum distances for enhanced error correction in retransmission systems, achieving stronger capabilities than classical codes.


<details>
  <summary>Details</summary>
Motivation: To improve reliability in communication systems with retransmission mechanisms by developing codes that can perform multi-round error correction under different metrics.

Method: Design two classes of GRCs corresponding to Type-I (identical codewords) and Type-II (distinct coded representations) repeated communication models, optimizing multiple minimum distances simultaneously.

Result: Obtained numerous optimal Type-I and Type-II GRCs with bounds and constructions that outperform classical error-correcting codes.

Conclusion: GRCs with multiple minimum distances provide superior error-correction capabilities for repeated transmission systems, addressing the challenge of maximizing correct decoding probability within limited transmission rounds.
...
### [190] [The Rate-Distortion-Perception Trade-Off with Algorithmic Realism](https://arxiv.org/abs/2511.15255)
*Yassine Hamdi,Aaron B. Wagner,Deniz Gündüz*

Main category: cs.IT

TL;DR: The paper explains why common randomness isn't needed in practice for lossy compression with realism constraints, showing it's only required for impractically large batch sizes.


<details>
  <summary>Details</summary>
Motivation: To resolve the discrepancy between theoretical studies (which suggest high-rate common randomness is valuable for achieving realism in lossy compression) and practical observations (where significant common randomness isn't used).

Method: Characterize the optimal rate-distortion trade-off under a realism constraint that requires satisfying a universal critic inspecting individual compressed reconstructions or batches.

Result: The optimal rate-distortion trade-off is asymptotically achievable without any common randomness, unless the batch size is impractically large.

Conclusion: Common randomness is not practically necessary for achieving realism in lossy compression, explaining why it hasn't been observed as useful in practical implementations.
...
### [191] [Communication-Pipelined Split Federated Learning for Foundation Model Fine-Tuning in UAV Networks](https://arxiv.org/abs/2511.15404)
*Zizhen Zhou,Ying-Chang Liang,Yanyu Cheng,Wei Yang Bryan Lim*

Main category: cs.IT

TL;DR: Proposed CPSFL with sequential gradient transmission and attention-based DRL for efficient FM fine-tuning in UAV networks, reducing latency and energy consumption.


<details>
  <summary>Details</summary>
Motivation: To address straggler issues and resource inefficiency in parallel gradient transmission for split federated learning in UAV networks, where communication latency dominates computation.

Method: Communication-pipelined SFL with sequential GT, downlink priority scheduling, intra-round asynchronous training, and attention-based DRL for optimizing split point selection and resource allocation.

Result: Simulation shows DRL-based CPSFL outperforms parallel GT benchmarks, ablation variants, fixed CCRA scheme, and approaches best fixed-SPS scheme performance.

Conclusion: Sequential GT paradigm with optimized resource allocation via DRL effectively reduces training latency and energy consumption in UAV-based FM fine-tuning.
...
### [192] [RIS-Enabled UAV Communications and Sensing: Opportunities, Challenges, and Key Technologies](https://arxiv.org/abs/2511.15555)
*Yajun Zhao,Mengnan Jian,Yifei Yuan*

Main category: cs.IT

TL;DR: This paper investigates UAV communication challenges and proposes RIS-assisted networks as a solution for improving signal coverage, network coexistence, and ISAC capabilities.


<details>
  <summary>Details</summary>
Motivation: UAVs face significant challenges in achieving reliable network coverage during transit operations, with traditional APAA-based networks having limitations in cost, complexity, and deployment constraints.

Method: The paper outlines operational scenarios, traffic patterns, and dual-layer heterogeneous network topology, then introduces RIS-assisted networks with technical features including design principles, antenna tilt configurations, new beam types, and beam tracking mechanisms.

Result: Field trial results demonstrate the effectiveness of RIS in improving UAV coverage, and the paper explores network architecture designs for enhanced signal coverage and RIS-enhanced UAV sensing.

Conclusion: RIS-assisted networks show promise for UAV communication systems, with future technological trends focusing on optimization, engineering implementation, and standardization efforts in RIS-based UAV-ISAC networks.
...
### [193] [Application of Graph Based Vision Transformers Architectures for Accurate Temperature Prediction in Fiber Specklegram Sensors](https://arxiv.org/abs/2511.14792)
*Abhishek Sebastian*

Main category: eess.IV

TL;DR: Transformer models like ViTs outperform CNNs for temperature prediction from fiber specklegram data, achieving MAE of 1.15°C, with XAI providing interpretability.
...
Method: Used transformer architectures including Vision Transformers, Swin Transformers, LINA-ViT, and MAP-ViGAT to predict temperature from specklegram data (0-120°C range), with XAI techniques for interpretability.

Result: ViTs achieved best performance with MAE of 1.15°C, outperforming CNNs. GAT-ViT and MAP-ViGAT variants also showed competitive accuracy, demonstrating the effectiveness of adaptive attention and graph-based structures.

Conclusion: Transformer architectures establish strong benchmarks for optical fiber temperature sensing, with promising applications in industrial monitoring and structural health assessment.
...
### [194] [Fully Differentiable dMRI Streamline Propagation in PyTorch](https://arxiv.org/abs/2511.14807)
*Jongyeon Yoon,Elyssa M. McMaster,Michael E. Kim,Gaurav Rudravaram,Kurt G. Schilling,Bennett A. Landman,Daniel Moyer*

Main category: eess.IV

TL;DR: Proposes a fully differentiable tractography method using PyTorch that matches standard propagators while enabling gradient flow for deep learning integration.


<details>
  <summary>Details</summary>
Motivation: Existing tractography methods are non-differentiable, limiting their integration in end-to-end learning frameworks, despite recent deep learning advancements.

Method: Developed a PyTorch-engineered streamline propagator with no components that block gradient flow, making it fully differentiable while retaining numerical fidelity with leading streamline algorithms.

Result: The method matches standard propagators in performance while remaining fully differentiable, enabling integration into deep learning workflows.

Conclusion: This differentiable tractography approach enables deeper integration of tractography into deep learning, laying foundation for computationally robust and scientifically rigorous macrostructural reasoning.
...
### [195] [Image Denoising Using Transformed L1 (TL1) Regularization via ADMM](https://arxiv.org/abs/2511.15060)
*Nabiha Choudhury,Jianqing Jia,Yifei Lou*

Main category: eess.IV

TL;DR: TL1 regularization for image denoising outperforms traditional TV methods by reducing staircase artifacts and preserving contrast.


<details>
  <summary>Details</summary>
Motivation: Total variation regularization causes staircase artifacts and contrast loss in denoised images.

Method: Developed TL1-regularized denoising model solved with ADMM, featuring closed-form TL1 proximal operator and FFT-based image updates.

Result: Superior denoising performance with effective noise suppression while preserving edges and enhancing contrast.

Conclusion: TL1 regularization is an effective alternative to TV for image denoising, addressing key limitations of traditional methods.
...
### [196] [Multimodal Optical Imaging Platform for Quantitative Burn Assessment](https://arxiv.org/abs/2511.15509)
*Nathaniel Hanson,Mateusz Wolak,Jonathan Richardson,Patrick Walker,David M. Burmeister,Chakameh Jafari*

Main category: eess.IV

TL;DR: A multimodal optical imaging system combining hyperspectral and laser speckle imaging for quantitative burn severity assessment, enabling deep-tissue analysis using SWIR wavelengths and unsupervised learning methods.


<details>
  <summary>Details</summary>
Motivation: Current lack of objective methods for detecting subsurface tissue damage in burns, particularly critical in battlefield and mass-casualty settings where rapid burn depth evaluation is essential for triage and surgical decisions.

Method: Integrates broadband hyperspectral imaging (400-2100 nm) with laser speckle contrast imaging to evaluate biochemical composition and microvascular perfusion. Uses SWIR wavelengths (>1000 nm) to develop deep-tissue parameters linked to water, lipid, and collagen absorption features. Implements unsupervised learning for spectral feature extraction, band selection, and clustering validated against histology.

Result: Developed novel deep-tissue parameters that enhance burn-tissue separability and burn severity classification. Validated methods against histology, establishing foundation for a compact, low-SWaP field-deployable device.

Conclusion: The multimodal optical imaging framework provides a foundation for rugged, data-driven devices for early quantitative burn evaluation in austere environments, addressing critical clinical challenges in burn assessment.
...
### [197] [Event-based Data Format Standard (EVT+)](https://arxiv.org/abs/2511.15556)
*Jonah P. Sengupta,Mohammad Imran Vakil,Thanh M. Dang,Ian Pardee,Paul Coen,Olivia Aul*

Main category: eess.IV

TL;DR: This paper proposes a standard data format for Event-based Sensing (EBS) hardware to address the proliferation of diverse output formats and ensure future compatibility.


<details>
  <summary>Details</summary>
Motivation: EBS technology is rapidly expanding with multiple vendors creating incompatible data formats, creating a need for standardization before widespread adoption.

Method: The authors define a sensor-agnostic standard that incorporates current sensor configurations and modalities while providing placeholders for future developments.

Result: A proposed standard format for EBS streaming data that can accommodate existing and future sensor technologies.

Conclusion: Establishing a standard now is crucial for EBS technology's successful widespread adoption across commercial, industrial, and defense applications.
