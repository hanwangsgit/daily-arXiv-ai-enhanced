...
TL;DR: AREA3D is an active 3D reconstruction agent that uses feed-forward 3D reconstruction models with vision-language guidance for efficient viewpoint selection, achieving state-of-the-art accuracy especially in sparse-view scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing active 3D reconstruction methods rely on hand-crafted geometric heuristics that often lead to redundant observations without significantly improving reconstruction quality. There's a need for more intelligent viewpoint selection that goes beyond purely geometric cues.
...
Conclusion: AREA3D successfully addresses limitations of traditional active reconstruction methods by combining feed-forward 3D reconstruction with vision-language guidance, enabling more efficient and accurate scene geometry acquisition through intelligent viewpoint selection.
...
TL;DR: The paper identifies "Scale Anchoring" as a fundamental problem in zero-shot super-resolution spatiotemporal forecasting, where models trained on low-resolution data fail to reduce error when deployed on higher resolutions. The authors propose Frequency Representation Learning to address this issue.


<details>
  <summary>Details</summary>
Motivation: Existing approaches incorrectly interpret similar error across different resolutions as successful generalization, but models serving as alternatives to numerical solvers should actually reduce error as resolution increases. The fundamental limitation is that low-resolution data has constrained Nyquist frequencies, preventing models from processing unseen high-frequency components during high-resolution inference.

Method: Proposes Frequency Representation Learning (FRL), an architecture-agnostic approach that uses resolution-aligned frequency representations and spectral consistency training. FRL ensures that on grids with higher Nyquist frequencies, the frequency response in high-frequency bands remains stable.

Result: FRL-enhanced variants allow errors to decrease with increasing resolution and significantly outperform baselines within the task and resolution range studied. The approach achieves these improvements with only modest computational overhead.

Conclusion: The paper defines "Scale Anchoring" as a new fundamental problem distinct from existing issues in zero-shot super-resolution forecasting. The proposed Frequency Representation Learning effectively addresses this problem by enabling models to properly generalize across resolutions and reduce error as resolution increases.
...
TL;DR: InvarDiff: Training-free acceleration method for diffusion models that exploits feature invariance across timesteps and layers to achieve 2-3× speed-ups with minimal quality degradation.


<details>
  <summary>Details</summary>
Motivation: Diffusion models produce high-quality synthesis but are slow due to iterative sampling. The authors observed feature invariance in deterministic sampling that can be exploited to reduce redundant computations.

Method: InvarDiff computes a binary cache plan matrix from a few deterministic runs, using quantile-based change metrics to identify which modules at which steps can be reused. It applies step-first and layer-wise caching guided by this matrix, with re-sampling correction to avoid drift when consecutive caches occur.

Result: When applied to DiT and FLUX models, InvarDiff achieves 2-3× end-to-end speed-ups with minimal impact on standard quality metrics. Visual quality shows almost no degradation compared to full computations.

Conclusion: InvarDiff provides an effective training-free acceleration method for diffusion models by exploiting temporal and layer-scale invariance, significantly reducing inference time while maintaining output quality.
...
TL;DR: AI-ECG model predicts severe coronary artery stenosis with good performance on internal/external validation, showing robustness across subgroups and providing interpretable waveform insights.


<details>
  <summary>Details</summary>
Motivation: Coronary CT angiography (CCTA) has limitations for large-scale CAD screening due to equipment requirements, radiation exposure, and patient cooperation needs. AI-ECG offers a promising alternative for non-invasive CAD assessment.
...
Result: Model achieved AUCs of 0.794-0.818 on internal validation and 0.667-0.971 on external validation for different arteries. Performance remained stable in normal-ECG subsets and across demographic/acquisition-time subgroups. Interpretability analysis revealed distinct waveform differences between risk groups.

Conclusion: AI-ECG provides a viable non-invasive screening tool for coronary artery stenosis with good predictive performance, robustness across clinical scenarios, and interpretable insights into ECG correlates of coronary disease.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models struggle with figure-ground segregation in cluttered backgrounds, especially when targets are camouflaged. Current benchmarks lack controlled, reproducible evaluation for this specific challenge.

Method: Created ChromouVQA benchmark using Ishihara-style chromatic camouflaged images with varied parameters (chromatic separation, density, size, occlusion, rotation). Also proposed a model-agnostic contrastive recipe aligning silhouettes with camouflaged renderings.

Result: Large performance gaps between humans and VLMs, especially under subtle chromatic contrast or disruptive geometric fills. The proposed contrastive learning approach improves global shape recovery.

Conclusion: ChromouVQA provides a compact, controlled benchmark for reproducible evaluation of VLMs on figure-ground segregation tasks, highlighting current limitations and offering a solution through contrastive learning.
...
TL;DR: A transfer-learning generative downscaling framework using U-Net encoder + diffusion model to reconstruct high-resolution satellite images from coarse inputs, validated on NASA MERRA-2 (50km) to GEOS-5 Nature Run (7km) data.


<details>
  <summary>Details</summary>
Motivation: To develop a robust method for downscaling long time series of coarse resolution satellite images with limited training data, enabling improved environmental exposure assessment and long-term monitoring.

Method: Combines lightweight U-Net transfer encoder (pretrained on coarse data) with diffusion-based generative model. U-Net encoder learns spatiotemporal representations, then frozen and transferred as physically meaningful latent features. Applied to NASA MERRA-2 (50km) → GEOS-5 Nature Run (7km) data for Asia region, split into subregions and seasons.

Result: Excellent performance (R2 = 0.65 to 0.94) across seasonal regional splits, outperforming deterministic U-Nets, variational autoencoders, and prior transfer learning baselines. Predicted images preserved physically consistent spatial variability and temporal autocorrelation, enabling stable autoregressive reconstruction beyond training data.

Conclusion: Transfer-enhanced diffusion models provide robust, physically coherent solution for downscaling long time series of coarse resolution images with limited training, with significant implications for environmental exposure assessment and monitoring.
...
TL;DR: FlowEO: A flow-matching based unsupervised domain adaptation framework for Earth observation images that outperforms existing image translation methods while maintaining good perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Earth observation data is heterogeneous due to different sensors, regions, times, and conditions, causing distribution shifts that limit model generalization. Unsupervised domain adaptation is crucial for real-world remote sensing applications.

Method: Uses flow matching to learn a semantically preserving mapping that transports from source to target image distribution, enabling adaptation for classification and semantic segmentation tasks.

Result: Outperforms existing image translation approaches for domain adaptation across four datasets covering SAR to optical translation and temporal/semantic shifts from natural disasters, while achieving on-par or better perceptual image quality.

Conclusion: FlowEO demonstrates the potential of flow-matching-based UDA for remote sensing, effectively handling challenging domain adaptation scenarios in Earth observation.
...
TL;DR: Self-training framework for VLM judges without human annotations, using iterative synthetic data generation and reasoning traces, achieving competitive performance against larger models.


<details>
  <summary>Details</summary>
Motivation: Current VLM judge training relies on costly human preference annotations that quickly become obsolete as models improve. Need for scalable, annotation-free methods that can evolve with rapidly advancing VLM capabilities.
...
Result: Improved Llama-3.2-11B multimodal judge from 0.38 to 0.51 overall accuracy on VL-RewardBench, often outperforming larger models (Llama-3.2-90B, GPT-4o, Claude 3.5 Sonnet) with strong gains in general, hallucination, and reasoning dimensions.

Conclusion: Human-annotation-free self-training shows promise for developing VLM judges that can evolve alongside rapidly improving model capabilities, offering a scalable alternative to costly human preference annotations.
...
TL;DR: TwinFlow is a simple framework for training 1-step generative models that bypasses pretrained teacher models and adversarial networks, achieving efficient multi-modal generation with 100x computational reduction.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal generative models use multi-step frameworks (diffusion/flow matching) requiring 40-100 NFEs, making them inefficient. Existing few-step acceleration methods have limitations: distillation methods need iterative procedures or degrade at very few steps (<4-NFE), while adversarial training methods introduce instability, complexity, and high memory overhead.

Method: TwinFlow is a simple framework that trains 1-step generative models without fixed pretrained teacher models and avoids standard adversarial networks. It demonstrates scalability through full-parameter training on large models like Qwen-Image-20B, transforming them into efficient few-step generators.

Result: On text-to-image tasks, TwinFlow achieves GenEval score of 0.83 in 1-NFE, outperforming baselines like SANA-Sprint and RCGM. With just 1-NFE, it matches the performance of original 100-NFE models on GenEval and DPG-Bench benchmarks, reducing computational cost by 100x with minor quality degradation.

Conclusion: TwinFlow provides an effective solution for efficient multi-modal generation by enabling 1-step inference without the limitations of existing distillation and adversarial training methods, making it ideal for building large-scale, efficient models with significant computational savings.
...
TL;DR: The paper proposes a tiered embedder and super-resolution approach for fine-grained image generation using diffusion models, addressing semantic entanglement and insufficient detail issues.


<details>
  <summary>Details</summary>
Motivation: Current class-conditional diffusion models struggle with fine-grained image generation due to semantic information entanglement between super and child classes, and insufficient detail in generated images, especially for less common categories.

Method: Introduces a tiered embedder that integrates semantic information from both super and child classes to reduce semantic entanglement. Uses super-resolution during perceptual information generation with enhancement and degradation models to improve detail. Also proposes an efficient ProAttention mechanism for diffusion models.

Result: The method outperforms other state-of-the-art fine-tuning methods on public benchmarks, demonstrating improved performance in fine-grained image generation.

Conclusion: The proposed tiered embedder and super-resolution approach effectively addresses semantic entanglement and detail insufficiency in fine-grained image generation using diffusion models, with the ProAttention mechanism providing efficient implementation.
...
<details>
  <summary>Details</summary>
Motivation: Existing LLM-based RL methods focus on policy guidance but suffer from limited backbone network representations. The paper aims to enhance visual RL by leveraging VLM's common-sense knowledge to extract richer semantic and motion representations.

Method: Uses a dual-path backbone to extract semantic and motion representations from RGB flows. Leverages VLM for semantic understanding and CLIP for text-image alignment. Employs separate supervision to guide representation extraction while allowing spontaneous interaction between semantics and motion.

Result: Extensive experiments show the method exhibits efficient and adaptive ability compared to state-of-the-art methods, demonstrating the effectiveness of VLM guidance at the feature level.

Conclusion: Semore successfully addresses the representation limitation problem in LLM-based RL by integrating VLM knowledge at the feature level, enabling more effective visual reinforcement learning through enhanced semantic and motion representations.
...
TL;DR: PELC enables pixel-equivalent latent compositing in diffusion models, fixing artifacts from linear VAE latent interpolation with DecFormer transformer for mask-consistent fusion.


<details>
  <summary>Details</summary>
Motivation: Current latent inpainting relies on linearly interpolating VAE latents under downsampled masks, causing artifacts at seams, global degradation, and color shifts. Modern VAEs capture global context beyond local structure, making linear blending non-pixel-equivalent.

Method: Propose Pixel-Equivalent Latent Compositing (PELC) principle: latent compositing should match pixel-space compositing. Introduce DecFormer, a 7.7M-parameter transformer that predicts per-channel blend weights and residual correction for mask-consistent latent fusion. Trained to match pixel-space alpha compositing after decoding.

Result: DecFormer restores global color consistency, soft-mask support, sharp boundaries, and high-fidelity masking, reducing edge errors by up to 53% over standard interpolation. As inpainting prior with lightweight LoRA, achieves fidelity comparable to fully finetuned FLUX.1-Fill model.

Conclusion: PELC enables full-resolution mask control and true soft-edge alpha compositing in diffusion models. DecFormer provides plug-compatible solution with minimal overhead (0.07% parameters, 3.5% FLOPs). PELC is general recipe for pixel-equivalent latent editing beyond inpainting.
...
TL;DR: DEAR is the first dataset for evaluating image rendering aesthetics based on human preferences, using pairwise comparisons from crowdsourcing to move beyond traditional distortion-based quality assessment.


<details>
  <summary>Details</summary>
Motivation: Traditional IQA focuses on technical degradations (noise, blur, compression) but fails to capture aesthetic preferences in rendering styles, which is crucial for photographic editing, content creation, and AI-generated imagery. There's a lack of datasets reflecting subjective style preferences.

Method: Built on MIT-Adobe FiveK dataset, collected pairwise human preference scores via large-scale crowdsourcing (25 evaluators per image pair, total 13,648 participants). Created DEAR dataset with nuanced, context-sensitive aesthetic preferences for Evaluation of Aesthetics of Rendering (EAR).

Result: DEAR is the first systematic dataset for image rendering aesthetics assessment grounded in subjective human preferences. Published subset of 100 images with markup on HuggingFace. Enables style preference prediction, aesthetic benchmarking, and personalized aesthetic modeling.

Conclusion: DEAR addresses the underexplored domain of rendering aesthetics evaluation, providing a benchmark for models that go beyond traditional distortion-based IQA to capture subjective human aesthetic judgments of image rendering styles.
...
TL;DR: Hybrid capture system combines sparse RGB keyframes with continuous event streams to reconstruct full RGB video offline, reducing power consumption while maintaining video quality.


<details>
  <summary>Details</summary>
Motivation: Continuous video monitoring faces power constraints with conventional RGB cameras, while event cameras offer low-power sensing but produce non-standard outputs. Need a solution that reduces capture power while maintaining standard RGB video for downstream applications.

Method: Propose IE2Video task: reconstruct RGB video from initial frame + event data. Two approaches: 1) HyperE2VID (autoregressive model adaptation), 2) LTX (inject event representations into pretrained text-to-video diffusion model via learned encoders and low-rank adaptation).

Result: Diffusion-based approach achieves 33% better perceptual quality than autoregressive baseline (0.283 vs 0.422 LPIPS). Validated across three event camera datasets (BS-ERGB, HS-ERGB far/close) with varying sequence lengths (32-128 frames), showing robust cross-dataset generalization.

Conclusion: Hybrid capture paradigm successfully reduces power consumption while reconstructing high-quality RGB video from sparse keyframes and event streams, with diffusion models outperforming autoregressive approaches for video reconstruction.
...
TL;DR: AionHMR is a framework for age-inclusive 3D human shape and pose estimation that works for adults, children, and infants using SMPL-A body model and transformer-based architecture.


<details>
  <summary>Details</summary>
Motivation: Existing 3D shape and pose estimation methods perform well for adults but fail to generalize to children and infants, creating a significant domain gap in age-inclusive human modeling.

Method: 1) Optimization-based extension of top-performing model with SMPL-A body model, 2) Generation of pseudo-ground-truth annotations for child/infant databases, 3) Development of transformer-based deep learning model for real-time 3D reconstruction.
...
Conclusion: AionHMR bridges the domain gap for age-inclusive 3D human modeling, enabling privacy-preserving anonymized datasets and establishing foundation for inclusive, privacy-aware, age-diverse human reconstruction.
...
TL;DR: CARD is a training-free diffusion method that handles correlated sensor noise by whitening observations and using noise-whitened updates, outperforming existing methods on real-world correlated noise tasks.


<details>
  <summary>Details</summary>
Motivation: Most diffusion models assume i.i.d. Gaussian noise, but real-world sensors exhibit spatially correlated noise due to readout mechanisms, limiting practical effectiveness in image restoration tasks.

Method: CARD whitens noisy observations to convert correlated noise to i.i.d. form, then replaces diffusion restoration steps with noise-whitened updates while maintaining DDRM's closed-form sampling efficiency. Also introduces CIN-D dataset for real correlated noise evaluation.

Result: CARD consistently outperforms existing methods across denoising, deblurring, and super-resolution tasks on both synthetic correlated noise benchmarks and the new CIN-D real-world dataset.

Conclusion: Addressing correlated noise is crucial for practical image restoration, and CARD provides an effective training-free solution that handles real-world sensor noise while maintaining computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for 4D scene reconstruction often focus on single objects using category-specific parametric models, leading to inconsistent scene configurations and limited to modeled object categories. There's a need for a method that can jointly predict structure and spatio-temporal configuration of multiple objects in dynamic scenes.

Method: COM4D uses a carefully designed training of spatial and temporal attentions on 2D video input. Training is disentangled into learning from object compositions (static multi-object) and single object dynamics, avoiding reliance on 4D compositional training data. At inference, an attention mixing mechanism combines these independently learned attentions.
...
Conclusion: COM4D provides a novel approach for 4D scene reconstruction that overcomes limitations of existing methods by using compositional learning without requiring 4D compositional training data, enabling reconstruction of complex dynamic scenes with multiple objects from monocular videos.
...
TL;DR: Introduces TAD benchmark for evaluating temporal understanding in autonomous driving, shows current models perform poorly, proposes two training-free solutions that improve accuracy by up to 17.72%.


<details>
  <summary>Details</summary>
Motivation: Existing temporal reasoning benchmarks focus on sports, cooking, movies but lack ego-centric autonomous driving footage. Need specialized benchmark for AD's unique temporal challenges.
...
Conclusion: TAD benchmark addresses gap in temporal understanding evaluation for autonomous driving. Proposed training-free solutions effectively enhance motion understanding. Work aims to catalyze future research in AD temporal reasoning.
...
TL;DR: SpaceControl is a training-free method for explicit spatial control of 3D generation that accepts various geometric inputs and integrates with pre-trained models, allowing users to balance geometric fidelity with output realism.


<details>
  <summary>Details</summary>
Motivation: Existing 3D generative methods rely on text or image prompts which lack geometric specificity - language is ambiguous and images are cumbersome to edit. There's a need for more intuitive and precise control over object geometry.

Method: Training-free test-time method that accepts diverse geometric inputs (from coarse primitives to detailed meshes) and integrates with modern pre-trained generative models without additional training. Includes a controllable parameter for trading off geometric fidelity vs. realism.

Result: Outperforms both training-based and optimization-based baselines in geometric faithfulness while preserving high visual quality. Includes an interactive UI for editing superquadrics and converting them into textured 3D assets.

Conclusion: SpaceControl provides effective explicit spatial control for 3D generation, bridging the gap between geometric precision and generative quality, with practical applications in creative workflows through its interactive interface.
...
TL;DR: A feedforward model for interactive editing of 3D Gaussian Splatting assets using 2D views, enabling real-time refinement, painting, and recoloring while preserving original asset identity.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods lack interactive refinement and editing capabilities. Existing diffusion or optimization approaches are too slow, destructive to original asset identity, or lack precision for fine-grained control.

Method: Introduces a state-aware feedforward model that directly predicts updates to compact, feature-rich Gaussian representation attributes. Uses Test-Time Training for state-aware iterative workflow from user-provided 2D views.

Result: Enables diverse editing tasks at interactive speeds: high-fidelity local detail refinement, local paint-over, and consistent global recoloring. Single architecture handles multiple editing operations.

Conclusion: Paves the way for fluid and intuitive 3D content authoring by providing an interactive editing solution for 3D Gaussian assets that overcomes limitations of existing approaches.
...
TL;DR: GOLA is a parameter-efficient fine-tuning framework for RGB-T tracking that reduces rank space redundancy by grouping and orthogonalizing low-rank parameters to learn complementary features for diverse tracking challenges.


<details>
  <summary>Details</summary>
Motivation: Current low-rank adaptation methods in RGB-T tracking suffer from significant redundancy in the rank space, where many ranks contribute little practical information. This limits the model's ability to learn diverse knowledge needed to address various tracking challenges.

Method: Proposes Group Orthogonal Low-Rank Adaptation (GOLA) framework: 1) Uses rank decomposition via SVD to quantify rank importance, 2) Freezes crucial ranks to preserve pretrained priors, 3) Clusters redundant ranks into groups, and 4) Applies inter-group orthogonal constraints to enforce complementary feature learning.
...
Conclusion: The GOLA framework successfully addresses rank space redundancy in parameter-efficient fine-tuning for RGB-T tracking, enabling more diverse knowledge learning through structured parameter learning and orthogonal constraints, leading to superior tracking performance.
...
TL;DR: PoolNet is a deep learning framework that quickly validates whether image data is suitable for Structure-from-Motion processing, significantly reducing computation time compared to traditional SfM algorithms.


<details>
  <summary>Details</summary>
Motivation: Traditional SfM processing is time-consuming and computationally expensive, and most publicly available image data is unsuitable due to issues like inadequate camera pose variation, obscuring scene elements, and noisy data.

Method: Introduces PoolNet, a versatile deep learning framework for frame-level and scene-level validation of in-the-wild image data to determine SfM readiness.

Result: The model successfully differentiates SfM-ready scenes from unfit ones while significantly undercutting the time state-of-the-art algorithms take to obtain SfM data.

Conclusion: PoolNet provides an efficient solution for validating image data suitability for SfM processing, addressing the computational burden and data quality issues in traditional approaches.
...
<details>
  <summary>Details</summary>
Motivation: VLLMs face high computational load during pre-filling due to processing enormous visual tokens. Existing attention-based pruning methods suffer from significant performance degradation when applied at early decoder layers, especially under high compression rates.

Method: ShaRP integrates three key components: segment-aware causal masking, positional debiasing, and token deduplication for enhanced token selection. This framework enables effective pruning at shallow layers without retraining.

Result: Extensive experiments show ShaRP achieves competitive performance across multiple video understanding benchmarks while accelerating VLLM inference.

Conclusion: ShaRP establishes a new paradigm for accelerating VLLM inference by enabling effective pruning at shallow decoder layers while maintaining stable performance under high compression rates.
...
TL;DR: LoC-Path is an efficient multimodal LLM for pathology that reduces computational costs by identifying and focusing on only task-relevant tiles in whole slide images, instead of processing all patches.
...
Method: LoC-Path replaces expensive slide-level encoders with redundancy-reducing modules: 1) Sparse Token Merger (STM) and MAE-pretrained resampler to remove local redundancy and compress globally redundant tokens, 2) Cross-Attention Routing Adapter (CARA) and Token Importance Scorer (TIS) to integrate compressed visual representations with language models efficiently.

Result: Extensive experiments show that LoC-Path achieves performance comparable to state-of-the-art whole-slide MLLMs while requiring significantly lower computation and memory.

Conclusion: The proposed LoC-Path framework demonstrates that efficient WSI-language modeling is achievable by focusing on task-relevant regions and reducing redundancy, offering a more practical solution for computational pathology.
...
TL;DR: SSVAE introduces spectral-structured video VAE with two regularizers to optimize latent space for diffusion training, achieving 3× faster convergence and 10% better video quality.


<details>
  <summary>Details</summary>
Motivation: Existing video VAEs focus on reconstruction fidelity but ignore latent structure, which strongly influences diffusion training difficulty. The paper identifies that optimal latent spaces for diffusion need specific spectral properties.
...
Result: SSVAE achieves 3× speedup in text-to-video generation convergence and 10% gain in video reward compared to strong open-source VAEs, demonstrating improved diffusion training efficiency and quality.

Conclusion: Optimizing VAE latent structure for spectral properties is crucial for efficient diffusion training, and SSVAE's regularizers provide an effective backbone-agnostic solution for video generation.
...
<details>
  <summary>Details</summary>
Motivation: Classical structure from motion pipelines struggle with dynamic objects in videos, and learning-based methods are limited by the need for large motion segmentation datasets, leading to inaccurate segmentation and poor 3D understanding.

Method: Dynamic Prior leverages Vision-Language Models for reasoning capabilities and SAM2 for fine-grained spatial segmentation to identify dynamic objects without task-specific training, then integrates this into state-of-the-art pipelines for camera pose optimization, depth reconstruction, and 4D trajectory estimation.

Result: Extensive experiments on synthetic and real-world videos show state-of-the-art performance on motion segmentation and significant improvements in accuracy and robustness for structural 3D understanding.

Conclusion: Dynamic Prior provides an effective way to handle dynamic objects in videos without specialized training, enhancing various 3D reconstruction tasks through better motion segmentation.
...
<details>
  <summary>Details</summary>
Motivation: Traditional stereo matching algorithms (SGBM+WLS) are fast for UAV applications but require meticulous manual parameter tuning, which is inefficient and limits performance in real-world forestry applications.

Method: Proposes a Genetic Algorithm (GA) based parameter optimization framework that automatically searches for optimal parameter configurations for SGBM and WLS filtering, eliminating manual tuning while maintaining processing efficiency.

Result: GA-optimized approach reduces Mean Squared Error by 42.86%, increases Peak Signal-to-Noise Ratio by 8.47%, and improves Structural Similarity by 28.52% compared to baseline configurations. Shows superior generalization across varied imaging conditions.

Conclusion: The GA-based optimization framework provides a practical solution for resource-constrained UAV systems, enabling enhanced precision in tree branch distance measurement while maintaining the speed advantages of traditional stereo matching algorithms.
...
TL;DR: Computer vision framework using YOLO + stereo vision enables autonomous drone pruning, eliminating expensive LiDAR while achieving accurate branch detection and localization for safer forestry operations.


<details>
  <summary>Details</summary>
Motivation: Manual pruning of radiata pine trees is dangerous due to extreme heights and challenging terrain, creating significant safety risks for workers in commercial forestry operations.

Method: Integrates YOLO object detection with Semi-Global Block Matching (SGBM) stereo vision for autonomous drone-based pruning, using only stereo camera input instead of expensive LiDAR sensors.

Result: YOLO achieves 82.0% mAPmask50-95 for branch segmentation, outperforming Mask R-CNN. System accurately localizes branches within 2m range with processing under one second per frame.

Conclusion: Demonstrates feasibility of cost-effective autonomous pruning systems that enhance worker safety and operational efficiency in commercial forestry through computer vision-based automation.
...
TL;DR: A multi-input CNN with attention module achieves 99% accuracy for solar system moving object detection, reducing human workload by 99%.


<details>
  <summary>Details</summary>
Motivation: Current moving object detection in solar system surveys relies heavily on manual verification by human eyes, which is labor-intensive and costly. There's a need to reduce this human dependency and automate the verification process.

Method: Proposes a multi-input convolutional neural network integrated with a convolutional block attention module. The multi-input architecture processes multiple stacked images simultaneously, while the attention module helps the model focus on essential features in both spatial and channel dimensions.
...
Conclusion: The proposed model demonstrates excellent classification performance for moving object detection in solar system surveys, significantly reducing reliance on manual intervention while maintaining high accuracy.
...
TL;DR: This paper evaluates deep learning methods for UAV-based tree branch segmentation across three resolutions (256x256, 512x512, 1024x1024) using the Urban Street Tree Dataset, identifying optimal architectures for accuracy-efficiency trade-offs in embedded forestry systems.


<details>
  <summary>Details</summary>
Motivation: UAV-based autonomous forestry operations require rapid and precise tree branch segmentation for safe navigation and automated pruning, but need to handle varying pixel resolutions and operational conditions efficiently.

Method: Evaluated 22 deep learning configurations across three resolutions (256x256, 512x512, 1024x1024) using Urban Street Tree Dataset. Used standard metrics (IoU, Dice) plus specialized measures: Thin Structure IoU (TS-IoU) and Connectivity Preservation Rate (CPR).

Result: U-Net with MiT-B4 backbone performed best at 256x256. At 512x512, MiT-B4 led in IoU, Dice, TS-IoU, and Boundary-F1. At 1024x1024, U-Net+MiT-B3 showed best validation performance for IoU/Dice and precision, while U-Net++ excelled in boundary quality. PSPNet was most efficient (2.36/9.43/37.74 GFLOPs) with 25.7/19.6/11.8 percentage point IoU reductions.

Conclusion: The study establishes multi-resolution benchmarks for accuracy-efficiency trade-offs in embedded forestry systems, providing guidance for selecting appropriate segmentation architectures based on resolution requirements and computational constraints.
...
TL;DR: ParaUni is a unified multimodal model that extracts parallel features from different VLM layers to enhance visual generation, using layer integration and dynamic adjustment mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing unified multimodal models struggle to balance sufficient interaction and flexible implementation due to representation differences between vision-language models and diffusion models. VLM layers contain hierarchical information from low-level details to high-level semantics that isn't fully utilized.

Method: Proposes ParaUni with two key components: 1) Parallel feature extraction from all VLM layers fed into a Layer Integration Module (LIM) to fuse fine-grained details and semantic abstractions as diffusion model conditions, and 2) Layer-wise Dynamic Adjustment Mechanism (LDAM) that uses RL to align hierarchical layer properties with multiple rewards.
...
Conclusion: ParaUni successfully addresses the interaction-implementation trade-off in unified multimodal models by parallel feature extraction and layer-wise dynamic adjustment, achieving better visual generation through comprehensive hierarchical information utilization.
...
TL;DR: TED-4DGS: A rate-distortion-optimized compression framework for dynamic 3D Gaussian Splatting that uses temporal activation and embedding-based deformation with hyperprior compression.
...
Method: TED-4DGS uses sparse anchor-based 3DGS representation with learnable temporal-activation parameters for appearance/disappearance transitions. Each anchor queries a shared deformation bank via lightweight temporal embeddings. For compression, employs INR-based hyperprior to model anchor attribute distributions and channel-wise autoregressive model for intra-anchor correlations.

Result: Achieves state-of-the-art rate-distortion performance on several real-world datasets. Represents one of the first rate-distortion-optimized compression frameworks for dynamic 3DGS representations.

Conclusion: TED-4DGS successfully unifies the strengths of both 4DGS and canonical 3DGS approaches, providing an efficient, compact deformation scheme with optimized compression for dynamic 3D scene representation.
...
<details>
  <summary>Details</summary>
Motivation: As edge device training becomes more feasible, there's a need for mission-specific datasets for Wireless Ad Hoc Federated Learning (WAFL) systems, particularly for building recognition tasks in specific environments like university campuses.

Method: Developed the Chulalongkorn University Building Recognition Dataset (CUBR) as a case study, and evaluated WAFL-ViT (Vision Transformer in Wireless Ad Hoc Federated Learning) scenarios compared to self-training approaches.

Result: Training in WAFL scenarios achieves better accuracy than self-training scenarios, demonstrating the effectiveness of collaborative learning with device-to-device communication among edge devices.

Conclusion: The CUBR dataset enables mission-oriented WAFL-ViT applications for building recognition, and WAFL collaborative training provides superior performance over individual self-training approaches for edge devices.
...
<details>
  <summary>Details</summary>
Motivation: Existing image stylization methods focus on visual appearance but overlook emotional impact. The authors aim to bridge this gap by creating AI-generated art that can evoke specific emotions through stylization.

Method: 1) Construct EmoStyleSet dataset from ArtEmis with content-emotion-stylized image triplets; 2) Develop Emotion-Content Reasoner to integrate emotional cues with content for style queries; 3) Create Style Quantizer to convert continuous style features into emotion-related codebook entries.

Result: Extensive evaluations show EmoStyle enhances emotional expressiveness while maintaining content consistency. The learned emotion-aware style dictionary is adaptable to other generative tasks, demonstrating broader applicability.

Conclusion: The work establishes a foundation for emotion-driven image stylization, expanding the creative potential of AI-generated art by enabling emotional expression through style application.
...
TL;DR: UniFS is a unified frequency-spatial fusion model for multi-contrast MR reconstruction that handles multiple k-space undersampling patterns without retraining, achieving SOTA performance across various scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing MCMR methods struggle with generalization across different k-space undersampling patterns (requiring separate models for each pattern) and fail to fully leverage complementary cross-modal frequency information, limiting practical applicability.
...
Result: Experimental results on BraTS and HCP datasets with various k-space undersampling patterns and acceleration factors (including unseen patterns) demonstrate state-of-the-art performance and superior generalization capability.

Conclusion: UniFS successfully addresses the generalization limitations of existing MCMR methods by handling multiple k-space undersampling patterns without retraining and effectively leveraging both spatial and frequency information, making it more practical for real-world applications.
...
TL;DR: A cross-modal framework that uses 2D Vision-Language Models to identify and mine rare objects from driving scenes to improve 3D object detection performance with minimal annotation burden.


<details>
  <summary>Details</summary>
Motivation: Rare-object detection in autonomous driving is challenging with point cloud data alone. Vision-Language Models have strong image understanding capabilities but haven't been fully explored for enhancing 3D object detection through intelligent data mining.

Method: A cross-modal framework combining object detection, semantic feature extraction, dimensionality reduction, and multi-faceted outlier detection. Uses Isolation Forest and t-SNE-based outlier detection with concept-based filtering to identify semantically meaningful rare objects. Extracts and annotates targeted rare object concepts like construction vehicles, motorcycles, and barriers.
...
Conclusion: The approach substantially reduces annotation burden and focuses on valuable training samples, with significant implications for efficient dataset curation in safety-critical autonomous systems.
...
TL;DR: WaterWave is an implicit representation method for underwater video enhancement that ensures temporal consistency using wavelet-based temporal frequency analysis and flow correction, outperforming frame-by-frame single-image enhancement approaches.


<details>
  <summary>Details</summary>
Motivation: Underwater video pairs are difficult to obtain, and existing methods that apply single-image enhancement frame by frame lack temporal consistency, resulting in unnatural video flow.

Method: Proposes WaterWave, an implicit representation in wavelet-based temporal consistency field that filters inconsistent components while preserving motion details. Includes an underwater flow correction module to rectify estimated flows considering underwater transmission effects.

Result: Significantly enhances video quality compared to single-image enhancement methods. Shows high potential in downstream tracking tasks (UOSTrack and MAT), outperforming original videos by 19.7% and 9.7% on precision respectively.

Conclusion: WaterWave effectively addresses temporal consistency in underwater video enhancement through wavelet-based temporal frequency analysis and flow correction, demonstrating superior performance in both enhancement quality and downstream applications.
...
TL;DR: Proposes a novel decoder framework with three modules (ACFA, TFFA, SMMM) to address Transformer decoder limitations in medical image segmentation by enhancing edge details, local textures, and spatial continuity modeling.


<details>
  <summary>Details</summary>
Motivation: Address limitations of Transformer decoders in capturing edge details, recognizing local textures, and modeling spatial continuity for medical image segmentation tasks.

Method: Three core modules: 1) Adaptive Cross-Fusion Attention (ACFA) with learnable guidance in three directions; 2) Triple Feature Fusion Attention (TFFA) fusing spatial, Fourier and wavelet domains; 3) Structural-aware Multi-scale Masking Module (SMMM) optimizing skip connections with multi-scale context and structural saliency filtering.

Result: Significantly enhances performance in high-precision tasks like tumor segmentation and organ boundary extraction, improving both segmentation accuracy and model generalization. Provides efficient and practical solution for medical image segmentation.

Conclusion: The proposed decoder framework effectively addresses traditional decoder shortcomings and demonstrates superior performance in medical image segmentation through synergistic integration of three specialized modules.
...
TL;DR: A framework that adapts visual foundation models to infrared small target detection without increasing inference overhead, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Large visual foundation models have strong generalization but haven't been explored for single-frame infrared small target (SIRST) detection. The authors aim to leverage these models' capabilities while avoiding the inference burden they typically introduce.

Method: Proposes Foundation-Driven Efficient Paradigm (FDEP) with: 1) Semantic Alignment Modulation Fusion module for dynamic alignment of VFM features with task-specific features, 2) Collaborative Optimization-based Implicit Self-Distillation strategy for implicit semantic transfer without inference overhead, and 3) Holistic SIRST Evaluation metric for unified evaluation.

Result: SIRST detection networks equipped with FDEP achieve state-of-the-art performance on multiple public datasets. The framework improves accuracy without additional inference overhead.

Conclusion: The proposed FDEP framework successfully adapts visual foundation models to SIRST detection, providing an efficient paradigm that enhances accuracy while maintaining inference efficiency, with a comprehensive evaluation metric for fair comparison.
...
<details>
  <summary>Details</summary>
Motivation: Current Video-Language Models have impressive multimodal understanding capabilities but lack proper grounding in space and time - they can't effectively "show what they know" or ground their reasoning in visual and temporal evidence.

Method: Created Know-Show benchmark with 2.5K human-authored questions from Charades, Action Genome, and Ego4D datasets, covering five spatial and temporal scenarios. Proposed GRAM - a training-free plug-in using attention-based video token selection and explicit timestamp encoding.

Result: Benchmark reveals significant gaps between current Video-LMs (Qwen, VideoLLaVA, GPT-4o, Gemini) and human reasoning, especially in fine-grained hand-object interactions. GRAM improves grounding capabilities without additional training.
...
TL;DR: DashFusion: A novel multimodal sentiment analysis framework using dual-stream alignment (temporal & semantic) with hierarchical bottleneck fusion to address alignment and fusion challenges, achieving SOTA performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Multimodal sentiment analysis faces challenges with alignment (temporal and semantic synchronization across modalities) and fusion (integrating aligned features). Existing methods often address these issues in isolation, leading to performance and efficiency limitations.

Method: Proposes DashFusion with: 1) Dual-stream alignment module (temporal alignment via cross-modal attention for frame-level correspondences, semantic alignment via contrastive learning for feature consistency), 2) Supervised contrastive learning to refine modality features using label information, 3) Hierarchical bottleneck fusion that progressively integrates multimodal information through compressed bottleneck tokens.

Result: Achieves state-of-the-art performance on CMU-MOSI, CMU-MOSEI, and CH-SIMS datasets across various metrics. Ablation studies confirm effectiveness of alignment and fusion techniques.

Conclusion: DashFusion effectively addresses alignment and fusion challenges in multimodal sentiment analysis through its dual-stream alignment and hierarchical bottleneck fusion approach, achieving superior performance while balancing computational efficiency.
...
TL;DR: VOST-SGG: A VLM-aided one-stage spatio-temporal scene graph generation framework that integrates vision-language models to address limitations in existing DETR-style approaches, achieving state-of-the-art performance on Action Genome dataset.


<details>
  <summary>Details</summary>
Motivation: Current DETR-style single-stage ST-SGG models have two key limitations: 1) learnable queries are semantically uninformed and instance-agnostically initialized, and 2) they rely exclusively on unimodal visual features for predicate classification, lacking semantic grounding and multi-modal reasoning capabilities.
...
Conclusion: Integrating vision-language models' common sense reasoning capabilities into ST-SGG pipelines through semantically grounded query initialization and multi-modal feature fusion significantly improves performance and addresses key limitations of existing approaches.
...
Method: Uses pretrained monocular depth estimation to get depth maps, creates depth-guided point prompts, SAM2 converts these to class-agnostic masks, then classifies masks via template matching using pooled visual features against a template bank from annotated frames.

Result: Achieves 35.9% mIoU on CholecSeg8k dataset, significantly improving over SAM2 auto segmentation baseline (14.7% mIoU). Maintains competitive performance with only 10-20% of object templates.

Conclusion: Depth-guided prompting and template-based classification offer an annotation-efficient segmentation approach for surgical scenes, reducing reliance on dense annotations while maintaining performance.
...
TL;DR: The paper presents a Bayesian ideal observer framework for image segmentation using dead leaves models, providing a principled upper-bound for comparing human and algorithmic performance.


<details>
  <summary>Details</summary>
Motivation: To develop a theoretical approach for studying visual segmentation decisions by creating a principled benchmark that accounts for natural scene occlusions and overlapping objects.

Method: Extends previous work on dead leaves models (generative models with object position, shape, color, and texture distributions) to derive a Bayesian ideal observer for pixel partitioning, with detailed explanations of posterior probability computation and practical feasibility factors.

Result: A theoretical framework for computing posterior probabilities in dead leaves image segmentation, establishing a principled upper-bound on segmentation performance for limited pixel sets.

Conclusion: The dead leaves model and associated ideal observer provide a valuable benchmark for studying segmentation decisions, enabling comparison between human visual performance and computational vision algorithms.
...
<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models suffer from text inertia where attention drifts from visual evidence to linguistic priors, causing object hallucinations. Existing methods either intervene too late (at output logits) or use unprincipled internal controls.

Method: Two components: 1) Cognitive Demand Sensor using Harsanyi interactions to estimate vision-text synergy and detect when visual grounding is needed; 2) Focused Consensus Induction module that selectively reorients mid-layer attention toward visual tokens before collapse into text priors.
...
Conclusion: Token-level sensing enables precise, context-aware intervention without compromising foundational knowledge, demonstrating that principled game-theoretic interpretability can be converted into actionable decoding control.
...
TL;DR: A new dataset (2K-Characters-10K-Stories) and pipeline for sequential identity consistency in visual storytelling with decoupled control of stable identities vs. transient attributes like pose and expression.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack sufficient fidelity and fail to disentangle stable identities from transient attributes, limiting structured control over pose, expression, and scene composition for reliable sequential synthesis in visual storytelling.

Method: 1) Created 2K-Characters-10K-Stories dataset with 2,000 unique stylized characters across 10,000 illustration stories; 2) Human-in-the-Loop pipeline with expert-verified character templates and LLM-guided narrative planning; 3) Decoupled control scheme separating persistent identity from transient attributes; 4) Quality-Gated loop with MMLM evaluation, Auto-Prompt Tuning, and Local Image Editing for pixel-level consistency.

Result: Models fine-tuned on the dataset achieve performance comparable to closed-source models in generating visual narratives, demonstrating effective sequential identity consistency.

Conclusion: The proposed dataset and pipeline successfully address the challenge of sequential identity consistency in controllable visual storytelling by providing large-scale unique identities with explicit, decoupled control signals.
...
TL;DR: ProPhy is a Progressive Physical Alignment Framework for physics-aware video generation that uses a Mixture-of-Physics-Experts mechanism to produce more realistic and physically coherent videos.
...
Method: Two-stage Mixture-of-Physics-Experts (MoPE) mechanism: Semantic Experts infer semantic-level physical principles from text, and Refinement Experts capture token-level physical dynamics. Physical alignment strategy transfers physical reasoning from vision-language models to Refinement Experts.
...
Conclusion: ProPhy addresses physical consistency limitations in video generation through explicit physics-aware conditioning and anisotropic generation, enabling better simulation of dynamic physical phenomena.
...
TL;DR: MedDIFT is a training-free 3D medical image registration framework that uses multi-scale features from pretrained latent medical diffusion models as voxel descriptors, achieving state-of-the-art performance without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Traditional medical image registration methods rely on local intensity-based similarity measures that fail to capture global semantic structure and perform poorly in low-contrast or anatomically variable regions. There's a need for methods that can leverage richer semantic information for more accurate spatial correspondence.

Method: MedDIFT extracts multi-scale features from a pretrained latent medical diffusion model and fuses them into rich voxel-wise descriptors. Correspondence is established via cosine similarity matching with an optional local-search prior. The approach is training-free and leverages diffusion model activations as descriptors.

Result: On a publicly available lung CT dataset, MedDIFT achieves correspondence accuracy comparable to state-of-the-art learning-based UniGradICON model and surpasses conventional B-spline-based registration. Ablation studies show that multi-level feature fusion and modest diffusion noise improve performance.

Conclusion: Pretrained diffusion models encode rich geometric and semantic information that can be effectively leveraged for medical image registration without additional training. MedDIFT demonstrates that training-free approaches using diffusion features can achieve state-of-the-art performance in 3D medical image correspondence tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing skinning-based methods produce misaligned shapes that corrupt high-frequency wrinkles, limiting animation quality for virtual try-on and extended reality applications.

Method: Proposes independent estimation of vertex positions (low-frequency shape) and vertex normals (high-frequency wrinkles), encodes them as texture images for 2D transfer using pretrained image models, and uses multimodal fusion to recover 3D garments.

Result: Significantly improves animation quality on various garment types and recovers finer wrinkles than state-of-the-art methods.

Conclusion: The skinning-free approach with 2D image transfer effectively decouples frequency modalities, enabling high-fidelity garment animation with fine wrinkle details without manual UV partitioning.
...
<details>
  <summary>Details</summary>
Motivation: Current perception-generalist approaches using language models achieve SOTA results but rely on slow autoregressive next-token prediction. Need faster inference for 3D scene layout estimation while maintaining accuracy.

Method: Uses multi-token prediction (MTP) to reduce autoregressive iterations, adapts self-speculative decoding (SSD) for structured language models, introduces confidence-guided decoding (CGD) with improved scoring for token reliability, and designs parameter-efficient mechanism to reduce MTP overhead.

Result: Generates up to 9 tokens per decoder inference step without compromising accuracy, adds only ~7.5% additional parameters, and significantly accelerates inference on ASE and Structured3D benchmarks.

Conclusion: Fast SceneScript provides an efficient solution for 3D scene layout estimation by combining multi-token prediction with reliability filtering, achieving substantial speed improvements while maintaining accuracy with minimal parameter overhead.
...
<details>
  <summary>Details</summary>
Motivation: Mobile laser scanning (MLS) shows promise for accurate tree-level inventory, but there's a need for effective tree species classification methods that work across different sensor types and can leverage both geometric and radiometric information.

Method: NormalView converts point cloud data into 2D projections by embedding local geometric information as normal vector estimates, then uses these projections as inputs to YOLOv11 image classification network. Also examines multispectral radiometric intensity effects.

Result: Achieves 95.5% overall accuracy (94.8% macro-average) on high-density MLS data (7 species) and 91.8% (79.1% macro-average) on high-density ALS data (9 species). Multispectral intensity from all three channels provides best performance on ALS data.

Conclusion: Projection-based methods enhanced with geometric information and state-of-the-art image classification backbones can achieve exceptional results, are sensor-agnostic, and benefit from multispectral intensity information. MLS dataset is publicly released.
...
TL;DR: DistillFSS is a distillation-based framework for Cross-Domain Few-Shot Semantic Segmentation that embeds support-set knowledge into model parameters, eliminating the need for support images at test time for fast inference.


<details>
  <summary>Details</summary>
Motivation: Standard episodic methods for CD-FSS are unreliable and computationally demanding due to substantial domain shifts, disjoint label spaces, and scarce support images. There's a need for more efficient and reliable approaches.

Method: Teacher-student distillation framework that internalizes few-shot reasoning into a dedicated layer within the student network, enabling fast inference without support images and allowing efficient extension to novel classes through teacher-driven specialization.

Result: DistillFSS matches or surpasses state-of-the-art baselines, particularly in multi-class and multi-shot scenarios, while offering substantial efficiency gains. A new CD-FSS benchmark spanning medical imaging, industrial inspection, and remote sensing was introduced.

Conclusion: The proposed distillation framework effectively addresses CD-FSS challenges by embedding support knowledge into model parameters, enabling fast inference and efficient scaling to large support sets with reduced computational overhead.
...
TL;DR: A novel unsupervised training framework using Unbalanced Optimal Transport (UOT) to train ISP pipelines without requiring large-scale paired raw-to-sRGB datasets, achieving performance comparable to or better than paired training methods.


<details>
  <summary>Details</summary>
Motivation: Current learned ISP pipelines depend on costly paired raw-to-sRGB datasets, creating a significant bottleneck. The authors aim to overcome this limitation by developing an unsupervised training approach that doesn't require paired data.

Method: Proposes an unsupervised training framework based on Unbalanced Optimal Transport (UOT) for cross-domain translation. Introduces a "committee of expert discriminators" as a hybrid adversarial regularizer to guide the optimal transport mapping by correcting specific ISP failure modes like color fidelity, structural artifacts, and frequency-domain realism.

Result: The framework successfully trains existing state-of-the-art ISP architectures in both paired and unpaired modes. In paired mode, it exceeds original paired methods across all metrics. In unpaired mode, it achieves quantitative and qualitative performance that rivals or surpasses original paired-trained counterparts.

Conclusion: The UOT-based framework effectively addresses the paired data bottleneck in ISP training, offering robust performance even with unpaired data while providing outlier robustness through unbalanced optimal transport.
...
TL;DR: Self-supervised AI image detection using EXIF metadata features, achieving state-of-the-art cross-model generalization and robustness to perturbations.


<details>
  <summary>Details</summary>
Motivation: Existing AI-generated image detectors rely on assumptions about specific generative models' internals, limiting cross-model applicability. The proliferation of AI-generated imagery creates escalating challenges for multimedia forensics that require more generalizable solutions.

Method: Self-supervised approach using EXIF metadata tags from camera-captured photos. Pretext task trains feature extractor by classifying categorical EXIF tags (camera model, scene type) and pairwise-ranking ordinal/continuous tags (focal length, aperture). Uses EXIF-induced features for one-class detection via Gaussian mixture modeling and binary detection with high-frequency residuals from spatially scrambled patches.
...
Conclusion: EXIF metadata provides effective features for detecting AI-generated images without relying on generative model internals, enabling cross-model generalization and practical forensic applications.
...
TL;DR: LeAD-M3D is a real-time monocular 3D object detector that achieves state-of-the-art accuracy without LiDAR or geometric priors, using asymmetric augmentation denoising distillation, 3D-aware consistent matching, and confidence-gated inference.


<details>
  <summary>Details</summary>
Motivation: Real-time monocular 3D object detection faces challenges including severe depth ambiguity, viewpoint shifts, and high computational cost. Existing methods either rely on LiDAR/geometric priors or sacrifice efficiency for accuracy.

Method: Three key components: 1) Asymmetric Augmentation Denoising Distillation (A2D2) transfers geometric knowledge from clean-image teacher to mixup-noised student via weighted depth-feature loss; 2) 3D-aware Consistent Matching (CM3D) integrates 3D MGIoU into matching score for better supervision; 3) Confidence-Gated 3D Inference (CGI3D) accelerates detection by restricting expensive 3D regression to top-confidence regions.
...
TL;DR: Real-time facial expression recognition using MediaPipe FaceMesh for landmark detection, geometric features (distances/angles), temporal dynamics analysis, and ConvLSTM1D+MLP classification achieving 93% accuracy on CK+ dataset with 165 FPS performance.


<details>
  <summary>Details</summary>
Motivation: Enhance human-computer interaction and develop emotion-aware systems for applications like user experience personalization and intelligent surveillance through real-time facial expression recognition.

Method: MediaPipe FaceMesh for facial landmark detection, extraction of geometric features (Euclidean distances and angles), temporal analysis of feature differences between frames to detect expression phases, and classification using ConvLSTM1D network with multilayer perceptron blocks.

Result: Achieved accuracies of 93% (CK+), 79% (Oulu-CASIA VIS), 77% (Oulu-CASIA NIR), and 68% (MMI). Real-time performance of ~165 FPS on consumer hardware. Demonstrated generalization capabilities through composite dataset experiments.

Conclusion: Provides a fast, accurate, and adaptable solution for facial expression analysis with potential for advancing emotion-aware technologies and personalized user experiences. Source code publicly available on GitHub.
...
<details>
  <summary>Details</summary>
Motivation: Current approaches for controllable 4D video generation require expensive fine-tuning of pre-trained Video Diffusion Models, which is computationally heavy, needs large datasets, causes architectural modifications, and suffers from catastrophic forgetting of original generative priors.

Method: Proposes InverseCrafter, an inpainting inverse solver that encodes pixel space degradation operators into continuous, multi-channel latent masks. This bypasses repeated VAE operations and backpropagation by solving the 4D generation task as an inpainting problem in latent space.
...
TL;DR: 3D-CSCNet: An autoencoder-based network using 3D convolutional sparse coding for hyperspectral unmixing, combining deep algorithm unrolling with endmember initialization via PSVM.


<details>
  <summary>Details</summary>
Motivation: Existing unrolling-based networks for hyperspectral unmixing don't leverage the powerful autoencoder framework, and need better integration of spectral and spatial relationships in 3D HSI data cubes.

Method: Propose 3D-CSCNet built on 3D convolutional sparse coding model within autoencoder framework. Use 3D-CSC block derived through deep algorithm unrolling to estimate abundance matrix, then reconstruct HSI via decoder to extract endmembers. Initialize decoder weights with endmembers from proposed PSVM algorithm.
...
Conclusion: 3D-CSCNet effectively combines autoencoder framework with 3D convolutional sparse coding for hyperspectral unmixing, achieving superior performance by jointly learning spectral-spatial relationships and using PSVM initialization.
...
TL;DR: ZRNet is a physics-informed framework for microscopy image restoration that jointly predicts Zernike coefficients and restores images using explicit modeling of optical physics relationships.


<details>
  <summary>Details</summary>
Motivation: Optical aberrations degrade microscopy image quality, especially in deep imaging. Existing methods treat the problem as black-box mapping without leveraging optical physics principles, and only handle mild aberrations on limited sample types.

Method: ZRNet jointly performs Zernike coefficient prediction and optical image restoration. It includes a Zernike Graph module that models physical relationships between Zernike polynomials based on azimuthal degrees, and a Frequency-Aware Alignment (FAA) loss that aligns Zernike predictions with image features in Fourier domain.
...
Conclusion: ZRNet successfully integrates optical physics principles into deep learning for microscopy aberration correction, outperforming existing methods by explicitly modeling wavefront distortion relationships and enforcing physical consistency between image restoration and Zernike prediction.
...
TL;DR: OWL proposes unsupervised 3D object detection using occupancy-guided warm-up and large-model priors to address poor pseudo-label quality in self-training.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised 3D detection methods suffer from incorrect pseudo-labels at training start, which mislead optimization and are hard to filter/refine effectively.

Method: Three components: 1) Occupancy Guided Warm-up (OGW) to initialize backbone with spatial perception; 2) Instance-Cued Reasoning (ICR) using large-model priors to assess pseudo-label quality; 3) Weight-adapted Self-training (WAS) to dynamically re-weight pseudo-labels.

Result: Outperforms state-of-the-art unsupervised methods by over 15.0% mAP on Waymo Open Dataset and KITTI benchmarks.

Conclusion: OWL effectively addresses pseudo-label quality issues in unsupervised 3D detection through warm-up initialization, large-model reasoning, and adaptive self-training.
...
<details>
  <summary>Details</summary>
Motivation: Current video understanding methods waste computation on irrelevant content and lose fine-grained details because they use query-agnostic captioners. Inspired by active perception theory, the authors argue that agents should actively decide what, when, and where to observe in videos.

Method: AVP uses an iterative plan-observe-reflect process with MLLM agents: 1) Planner proposes targeted video interactions, 2) Observer executes them to extract time-stamped evidence, 3) Reflector evaluates evidence sufficiency, either halting with an answer or triggering further observation.
...
Conclusion: The evidence-seeking approach of AVP enables efficient long video understanding by focusing computational resources on query-relevant content, demonstrating significant improvements in both accuracy and efficiency compared to existing methods.
...
Conclusion: By explicitly incorporating nonlinear geometry information through geodesic-aware relational attention, the method achieves improved semantic coherence and structural fidelity in point cloud completion.
...
TL;DR: A privacy-preserving framework to distill knowledge from large LLMs into local VLMs for surgical scene understanding, using expert-supervised datasets generated without sensitive images.


<details>
  <summary>Details</summary>
Motivation: Current VLMs lack domain-specific surgical scene understanding capabilities and pose privacy risks when patient data is sent to external servers. There's a need for locally deployable models that can understand surgical contexts while protecting patient privacy.

Method: Generate expert-supervised dataset by prompting teacher LLM with only textual context and binary segmentation masks (no sensitive images). Use this dataset for Supervised Fine-Tuning (SFT) and Direct Preference Optimization (DPO) of a local VLM.

Result: Finetuning VLMs with the generated datasets significantly increases surgical domain knowledge compared to base VLMs. The approach provides data-efficient and privacy-conforming training.

Conclusion: Validates a privacy-preserving method to create locally deployable VLMs optimized for surgical scene understanding, addressing both domain expertise gaps and data privacy concerns.
...
TL;DR: The paper analyzes test-time verifiers for spatial reasoning in Vision-Language Models, finds MindJourney's verifier unreliable, proposes ViSA framework that improves performance on some benchmarks but not others, revealing limitations in world models.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models have limitations in spatial reasoning requiring multi-view understanding. Recent approaches like MindJourney use test-time scaling with world models and heuristic verifiers, but their effectiveness needs systematic examination.

Method: Systematically analyze test-time verifiers across benchmarks, conduct uncertainty-based analyses, and introduce Verification through Spatial Assertions (ViSA) framework that grounds test-time rewards in verifiable, frame-anchored micro-claims.

Result: MindJourney's verifier provides little meaningful calibration; random scoring often reduces answer entropy equally well. ViSA improves spatial reasoning on SAT-Real benchmark and corrects trajectory-selection biases, but fails to achieve consistent scaling on challenging MMSI-Bench.

Conclusion: Current world models form an information bottleneck where imagined views fail to enrich fine-grained reasoning. The findings chart the bad (unreliable verifiers), good (ViSA improvements), and ugly (fundamental limitations) aspects of test-time verification for world-model-based reasoning.
...
TL;DR: HQ-DM is a quantization-aware training framework using Single Hadamard Transformation to reduce activation outliers in diffusion models, enabling efficient low-bit quantization with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Diffusion models have high computational/memory costs for deployment. Existing quantization methods struggle with activation outliers during inference, causing significant performance degradation in low-bit quantization scenarios.

Method: Propose HQ-DM, a quantization-aware training framework that applies Single Hadamard Transformation to activation matrices to reduce outliers while preserving model performance under quantization. This approach supports INT convolution operations and prevents weight outlier amplification.

Result: For conditional generation on ImageNet 256x256 using LDM-4 model, W4A4 and W4A3 quantization schemes improve Inception Score by 12.8% and 467.73% respectively over state-of-the-art methods.

Conclusion: HQ-DM effectively addresses activation outlier issues in diffusion model quantization, enabling efficient low-bit deployment with substantial performance improvements over existing methods.
...
TL;DR: USV is a unified sparsification framework that jointly optimizes attention pruning, token merging, and denoising step reduction for efficient video diffusion models.


<details>
  <summary>Details</summary>
Motivation: Video diffusion models suffer from quadratic complexity of global spatio-temporal attention and computational overhead of long iterative denoising trajectories. Existing accelerators target single dimensions in isolation and face diminishing returns as remaining bottlenecks become dominant.
...
TL;DR: This paper introduces an image-based approach for classifying six optical fiber events using Phase-OTDR data by transforming 1D signals into grayscale images and combining them into RGB representations for transfer learning, achieving over 98% accuracy with EfficientNetB0 and DenseNet121 models.


<details>
  <summary>Details</summary>
Motivation: Traditional Phase-OTDR data analysis faces challenges in handling complex fiber optic sensing data. The motivation is to develop a more robust and adaptable approach by transforming 1D data into image representations that can leverage powerful transfer learning models for improved event detection accuracy.

Method: Transform 1D Phase-OTDR data into grayscale images using three techniques: Gramian Angular Difference Field (GADF), Gramian Angular Summation Field (GASF), and Recurrence Plot. Combine these grayscale images into a multi-channel RGB representation. Apply transfer learning models (EfficientNetB0 and DenseNet121) for classification, validated with 5-fold cross-validation.

Result: Achieved high classification accuracies of 98.84% with EfficientNetB0 and 98.24% with DenseNet121. 5-fold cross-validation confirmed reliability with test accuracies of 99.07% and 98.68% respectively. The approach reduces dataset size while improving analysis efficiency.

Conclusion: Image-based analysis transforms complex fiber optic sensing data interpretation, offering significant advancements in accuracy and reliability of monitoring systems. The publicly available code and dataset support further research in this transformative approach to optical fiber event detection.
...
TL;DR: A novel active learning method for 3D point cloud semantic segmentation that uses a simple 2D grid subdivision approach and network ensemble uncertainty estimation, achieving state-of-the-art performance with reduced annotation costs.


<details>
  <summary>Details</summary>
Motivation: Semantic segmentation of 3D point clouds requires expensive manual annotation. Active learning can reduce annotation costs, but existing methods use complex heuristics for both splitting point clouds into annotatable regions and selecting data for annotation. The authors aim to develop a simpler, more effective approach.

Method: Two key innovations: 1) A simple 2D grid-based approach to subdivide point clouds into columns for annotation, avoiding complex region splitting heuristics. 2) Using a network ensemble to estimate uncertainty in predictions for selecting the most beneficial data to annotate next.

Result: The method achieves performance on par with or better than complex state-of-the-art methods on three datasets: S3DIS, Toronto-3D, and a large-scale urban point cloud of Freiburg. The authors also demonstrate that annotated area (rather than number of points) is a more meaningful measure for active learning in point clouds.

Conclusion: The proposed simple 2D grid subdivision with network ensemble uncertainty estimation provides an effective and easy-to-implement active learning strategy for 3D point cloud segmentation, reducing annotation costs while maintaining or improving performance compared to more complex methods.
...
TL;DR: FNOpt is a self-supervised cloth simulation framework that uses Fourier neural operators as neural optimizers for resolution-agnostic, stable cloth dynamics simulation without requiring ground truth data.


<details>
  <summary>Details</summary>
Motivation: Prior neural cloth simulators have limitations: they either require extensive ground truth data, sacrifice fine-scale details, or generalize poorly across different mesh resolutions and motion patterns. There's a need for a more robust, data-efficient approach that can handle diverse simulation scenarios.

Method: FNOpt formulates time integration as an optimization problem and trains a resolution-agnostic neural optimizer parameterized by a Fourier neural operator (FNO). It's trained only on coarse grids using physics-based losses, enabling it to generalize to finer resolutions without retraining.

Result: FNOpt outperforms prior learning-based approaches in out-of-distribution settings, achieving better accuracy and robustness. It captures fine-scale wrinkles, preserves rollout stability across diverse mesh resolutions and motion patterns, and reduces the need for curated data.

Conclusion: FNO-based meta-optimization presents a compelling alternative to previous neural simulators for cloth, offering improved cross-resolution reliability and reducing dependency on extensive ground truth data while maintaining physical plausibility.
...
TL;DR: A novel curvature regularization method using discrete Laplacian operator achieves 18.1% better 3D reconstruction accuracy than standard VAEs when depth sensors provide only 5% of needed measurements.


<details>
  <summary>Details</summary>
Motivation: Sparse depth measurements (only 5% of needed data) make complete 3D scene reconstruction challenging for autonomous vehicles and robots, as sparse reconstruction introduces unacceptable geometric errors.

Method: Proposes curvature regularization through a discrete Laplacian operator that provides stable gradients and noise suppression. The method challenges the assumption that multiple geometric constraints are needed, showing a single well-designed regularization term can outperform complex multi-term formulations.

Result: Achieves 18.1% better reconstruction accuracy than standard variational autoencoders with only 15% training overhead and zero inference cost. The discrete Laplacian offers stable gradients and effective noise suppression.

Conclusion: A single carefully designed regularization term (curvature regularization via discrete Laplacian) can exceed the performance of complex multi-term geometric constraints in 3D reconstruction, challenging an implicit assumption in geometric deep learning. The method is efficient with minimal training overhead and no inference cost.
...
TL;DR: C3 is a novel uncertainty quantification method for controllable video models that provides dense, calibrated confidence estimates at subpatch level to localize hallucination in generated videos.


<details>
  <summary>Details</summary>
Motivation: Controllable video models often hallucinate (generate physically unrealistic frames), which is problematic for critical applications like robot policy evaluation and planning. Current models lack confidence assessment capabilities, making hallucination mitigation difficult.
...
Conclusion: C3 successfully addresses the hallucination problem in controllable video models by providing high-resolution uncertainty heatmaps that precisely localize untrustworthy regions in generated video frames, enabling better trust and reliability in critical applications.
...
<details>
  <summary>Details</summary>
Motivation: Current CTVG methods assume static concepts and struggle with forgetting/neglect when continuously learning new subjects and motions over time.

Method: Uses concept-specific attribute retention module and task-aware concept aggregation to prevent forgetting, plus controllable conditional synthesis with region attention for concept neglect.

Result: Outperforms existing CTVG models in extensive experimental comparisons.

Conclusion: CCVD enables effective continual learning of new concepts in text-to-video generation while maintaining old concept knowledge.
...
TL;DR: The paper proposes using synthetic datasets to systematically study how background correlations affect traffic sign classification and feature importance explanations, addressing limitations of real-world data for testing XAI interpretations.


<details>
  <summary>Details</summary>
Motivation: Current XAI methods for deep learning (like SHAP and GradCAM) rely on intuitive interpretations about whether classifiers focus on object vs. background pixels, but these interpretations are difficult to test quantitatively due to unavoidable correlations in real-world data and lack of ground truth about what constitutes "spurious" correlations.

Method: Systematically generate six synthetic datasets for traffic sign recognition that differ only in their degree of camera variation and background correlation, enabling controlled isolation of these factors. The datasets allow quantification of background correlation influence, camera variation levels, and traffic sign shapes on both classification performance and background feature importance.

Result: The approach enables quantification of when and how much background features gain importance to support classification based on changes in the training domain. The synthetic datasets provide a controlled environment to test XAI interpretations that is not possible with real-world data.

Conclusion: Synthetic datasets offer a valuable approach to systematically study and quantify the effects of background correlations on classifier behavior and XAI interpretations, addressing the fundamental challenge of testing intuitive explanations in real-world scenarios where correlations are unavoidable and their legitimacy is debatable.
...
TL;DR: ZoomClick: A training-free method that leverages zoom as a prior for GUI grounding, improving performance of vision-language models on GUI element localization tasks without additional training.


<details>
  <summary>Details</summary>
Motivation: Existing GUI grounding approaches face challenges with cross-platform generalization, complex layout analysis, and fine-grained element localization despite using large-scale bounding box supervision. The authors identify zoom as an underexplored but powerful prior for GUI grounding.

Method: Proposes ZoomClick, a training-free method that characterizes four key properties of zoom (pre-zoom, depth, shrink size, minimal crop size) to enable dynamic spatial focusing and adaptive context switching for GUI grounding.

Result: Significantly boosts performance of both general vision-language and specialized GUI grounding models, achieving state-of-the-art results on mainstream benchmarks. UI-Venus-72B achieves 73.1% success rate on ScreenSpot-Pro. Also introduces GUIZoom-Bench benchmark for evaluating model adaptability to zoom.

Conclusion: Zoom is a strong prior for GUI grounding that enables training-free performance improvements. The proposed ZoomClick method and GUIZoom-Bench benchmark provide foundations for future research on improving zoom capabilities for GUI grounding tasks.
...
TL;DR: UG-FedDA: A federated domain adaptation framework with uncertainty quantification for Alzheimer's disease classification across multiple MRI datasets while preserving privacy.
...
Result: Achieved consistent cross-domain improvements across three classification tasks on ADNI, AIBL, and OASIS datasets. Best accuracy of 90.54% for AD vs. NC on ADNI, with strong performance across all tasks and datasets.
...
TL;DR: VRSA is a novel jailbreak attack method that decomposes harmful text queries into sequential visual reasoning sub-images to exploit vulnerabilities in multimodal LLMs, achieving higher attack success rates than existing methods.


<details>
  <summary>Details</summary>
Motivation: While previous jailbreak attacks have focused on text reasoning vulnerabilities in MLLMs, similar threats in the visual modal have been largely overlooked. The authors aim to fully evaluate potential safety risks in visual reasoning tasks by exploring how multimodal models can be manipulated through sequential visual inputs.

Method: VRSA decomposes harmful text queries into several sequentially related sub-images that gradually externalize and aggregate harmful intent. It uses three key techniques: 1) Adaptive Scene Refinement to optimize scenes relevant to the harmful query, 2) Semantic Coherent Completion to iteratively rewrite sub-texts with contextual information, and 3) Text-Image Consistency Alignment to maintain semantic consistency between text and images.

Result: Experiments show VRSA achieves higher attack success rates compared to state-of-the-art jailbreak attack methods on both open-source and closed-source MLLMs, including GPT-4o and Claude-4.5-Sonnet.

Conclusion: The paper demonstrates significant safety vulnerabilities in MLLMs' visual reasoning capabilities, showing that sequential visual attacks can effectively bypass safety mechanisms. This highlights the need for improved multimodal safety evaluations and defenses against such visual reasoning attacks.
...
TL;DR: AQUA-Net is a lightweight underwater image enhancement model that combines frequency fusion and illumination awareness to restore color, contrast, and realism while reducing computational complexity for real-time applications.


<details>
  <summary>Details</summary>
Motivation: Underwater images suffer from color distortion, low contrast, and haze due to light absorption/scattering, while existing deep learning models are computationally heavy, limiting real-time deployment.

Method: AQUA-Net integrates residual encoder-decoder with dual auxiliary branches: frequency fusion encoder (enriches spatial representations with Fourier domain cues) and illumination-aware decoder (performs adaptive exposure correction using learned illumination map inspired by Retinex theory).

Result: Extensive experiments show AQUA-Net performs on par with SOTA methods in qualitative/quantitative evaluations while using fewer parameters. Ablation studies confirm complementary contributions of frequency and illumination branches for improved visibility and color representation.

Conclusion: AQUA-Net provides an effective, lightweight solution for real-world underwater imaging with strong generalization capability and robustness, enabled by joint spatial-frequency-illumination design and validated on a new Mediterranean Sea dataset.
...
TL;DR: A plug-and-play edit-aware loss function that improves RAW reconstruction from sRGB images by making recovered RAWs more robust to different rendering styles and edits through a differentiable ISP simulation.


<details>
  <summary>Details</summary>
Motivation: Users often edit camera images post-capture, but while RAW editing offers more flexibility, most editing happens on sRGB JPEGs since RAW files are rarely stored. Existing RAW reconstruction methods focus on pixel-wise fidelity and degrade under diverse rendering styles and editing operations.
...
TL;DR: A Swin Transformer-based GAN framework achieves state-of-the-art underwater image reconstruction by effectively addressing color distortion, low contrast, and haze effects through global dependency modeling.


<details>
  <summary>Details</summary>
Motivation: Underwater imaging suffers from severe degradation due to wavelength-dependent absorption and scattering, causing color distortion, low contrast, and haze. Traditional methods and CNNs fail to adequately address these challenges due to limited receptive fields and inability to model global dependencies.
...
Result: State-of-the-art performance on EUVP dataset with PSNR of 24.76 dB and SSIM of 0.89, significantly outperforming existing methods. Visual results show effective color balance restoration, contrast improvement, and haze reduction. Ablation study confirms superiority of Swin Transformer over convolutional alternatives.

Conclusion: The proposed Swin Transformer-based GAN offers robust underwater image reconstruction suitable for various marine applications, effectively addressing the limitations of traditional methods through global dependency modeling.
...
<details>
  <summary>Details</summary>
Motivation: Existing motion transfer approaches often fail to preserve structural fidelity and temporal consistency in wild scenarios with complex motion and cross-identity animations, making it challenging to achieve studio-grade production standards.

Method: Two key innovations: 1) Novel 3D pose representation for robust motion signals, 2) Full-context pose injection mechanism within a diffusion-transformer architecture for effective spatio-temporal reasoning over full motion sequences. Also includes curated data pipeline and comprehensive benchmark.

Result: SCAIL achieves state-of-the-art performance and advances character animation toward studio-grade reliability and realism.

Conclusion: SCAIL successfully addresses challenges in character animation by combining 3D pose representation with full-context pose injection, enabling high-quality motion transfer that meets studio production standards.
...
<details>
  <summary>Details</summary>
Motivation: Accurate prediction of postoperative facial appearance in orthognathic surgery is challenging due to complex nonlinear interactions between skeletal movements and facial soft tissue. Existing methods lack computational efficiency or fail to capture these intricate interactions.

Method: Proposes Neural Implicit Craniofacial Model (NICE) with two modules: 1) Shape module using region-specific implicit Signed Distance Function (SDF) decoders to reconstruct facial surface, maxilla, and mandible; 2) Surgery module using region-specific deformation decoders driven by shared surgical latent code to model nonlinear biomechanical responses, outputting point-wise displacement fields.

Result: NICE outperforms current state-of-the-art methods, notably improving prediction accuracy in critical facial regions (lips and chin) while robustly preserving anatomical integrity.
...
TL;DR: Proposes a cluster-free, one-stage learnable-prototype framework with diversity regularization for weakly supervised semantic segmentation in histopathology, addressing limitations of existing two-stage clustering approaches.


<details>
  <summary>Details</summary>
Motivation: Existing weakly supervised semantic segmentation methods in histopathology face challenges: inter-class homogeneity, intra-class heterogeneity, and CAM-induced region shrinkage. Current two-stage clustering approaches are costly, hyperparameter-sensitive, and decouple prototype discovery from segmentation learning, limiting effectiveness and efficiency.

Method: A cluster-free, one-stage learnable-prototype framework with diversity regularization to enhance morphological intra-class heterogeneity coverage. Unlike previous two-stage clustering methods, this approach integrates prototype learning directly with segmentation in a single stage.

Result: Achieves state-of-the-art performance on BCSS-WSSS benchmark, outperforming prior methods in mIoU and mDice metrics. Qualitative results show sharper boundaries, fewer mislabels, and activation heatmaps reveal that learnable prototypes cover more diverse and complementary regions within each class compared to clustering-based prototypes.

Conclusion: The proposed one-stage learnable-prototype framework with diversity regularization effectively addresses limitations of existing WSSS methods in histopathology, providing superior performance, better coverage of intra-class heterogeneity, and more efficient training without the need for separate clustering stages.
...
TL;DR: This paper compares synthetic facial datasets generated using different techniques (diffusion models, GANs, 3D models) for facial recognition tasks, evaluating their effectiveness across eight datasets to address privacy, bias, and data scarcity issues.


<details>
  <summary>Details</summary>
Motivation: Facial recognition faces challenges in explainability, demographic bias, privacy, and robustness to variations. Privacy regulations have degraded datasets, raising legal/ethical concerns. Synthetic data generation offers solutions by mitigating privacy issues, enabling controlled attribute experimentation, alleviating bias, and supplementing real data.

Method: Comparative evaluation of synthetic facial datasets generated using different techniques (diffusion models, GANs, 3D models) on facial recognition tasks. Performance measured using accuracy, rank-1, rank-5, and true positive rate at false positive rate of 0.01% across eight leading datasets.

Result: Synthetic data demonstrates ability to capture realistic variations, but performance gap with real data persists. Techniques like diffusion models, GANs, and 3D models show substantial progress, though challenges remain in achieving parity with real data performance.

Conclusion: Synthetic facial data is a promising solution for privacy, bias, and data scarcity issues in facial recognition, but further research is needed to close the performance gap with real data and address remaining challenges in synthetic data generation techniques.
...
TL;DR: A synthetic traffic sign recognition dataset (Synset Signset Germany) combining GAN-based texture generation for realistic dirt/wear with analytical scene modulation for physically correct lighting, enabling XAI applications and robustness testing.


<details>
  <summary>Details</summary>
Motivation: To create a synthetic traffic sign dataset that combines data-driven realism with analytical parameterization for explainable AI and robustness testing, particularly for rare/new German traffic signs.

Method: Combines GAN-based texture generation for realistic dirt/wear artifacts with analytical scene modulation for physically correct lighting and detailed parameterization. Creates 105,500 images of 211 German traffic sign classes with masks, segmentation, and extensive metadata.

Result: Created Synset Signset Germany dataset with 105,500 images covering 211 German traffic sign classes (including rare 2020 signs). Provides comprehensive metadata for XAI applications and demonstrates parameter sensitivity evaluation.

Conclusion: The hybrid approach successfully creates realistic synthetic traffic sign data suitable for XAI and robustness testing, addressing limitations of purely data-driven or analytical methods while covering rare traffic signs.
...
TL;DR: EditThinker: A deliberative image editing framework using MLLM-based reasoning to iteratively critique and refine instructions, improving instruction-following through reinforcement learning.
...
Method: Proposes a deliberative editing framework with a Think-while-Edit cycle: Critiquing results and Refining instructions, then Repeating generation. Trains a single MLLM (EditThinker) to produce critique scores, reasoning processes, and refined instructions. Uses reinforcement learning to align thinking with editing for targeted instruction improvements.

Result: Extensive experiments on four benchmarks show the approach significantly improves instruction-following capability of any image editing model by a large margin.

Conclusion: The deliberative framework with EditThinker effectively addresses instruction adherence limitations in image editing, with plans to release data construction framework, datasets, and models to benefit the community.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in machine learning including computational constraints, limitations of single-view learning, and complexity of processing large datasets from multiple domains/sources/views. MVC emerges as a powerful approach to overcome these challenges by providing richer data representation and effective solutions for unsupervised learning tasks.

Method: Conducted an extensive survey reviewing over 140 foundational and recent publications. Systematically categorized MVC methods into groups: co-training, co-regularization, subspace, deep learning, kernel-based, anchor-based, and graph-based strategies. Developed comparative insights on integration strategies (early fusion, late fusion, joint learning) and investigated practical use cases.

Result: Provides a comprehensive threefold contribution: (1) systematic categorization of MVC methods, (2) in-depth analysis of strengths, weaknesses, and practical challenges (scalability, incomplete data), (3) forward-looking discussion of emerging trends, interdisciplinary applications, and future directions. The survey fills existing gaps in MVC research and offers actionable insights.

Conclusion: Multi-view clustering is a powerful approach that compensates for single-view method shortcomings and provides richer data representation. Despite inherent complexity, the semantically rich nature of multi-view data increases its practical utility. This comprehensive survey aims to advance the field by providing structured insights and identifying future research directions.
...
TL;DR: The paper proposes CV-Masking, a volatility-aware pretraining strategy for masked autoencoders on EHR data that adapts masking probabilities based on feature variability, outperforming uniform random masking.


<details>
  <summary>Details</summary>
Motivation: Existing MAE approaches for EHR use uniform random masking, assuming all features are equally predictable. However, laboratory tests show substantial heterogeneity in volatility - some biomarkers are stable while others fluctuate considerably. Volatile biomarkers often signal acute pathophysiology and require more sophisticated modeling to capture complex temporal patterns.

Method: Proposes Coefficient of Variation Masking (CV-Masking) that adaptively adjusts masking probabilities according to the intrinsic variability of each feature. Combined with a value-only masking objective aligned with clinical workflows.

Result: CV-Masking yields systematic improvements over random and variance-based strategies. Experiments on large panel of laboratory tests show enhanced reconstruction, improved downstream predictive performance, accelerated convergence, and more robust clinically meaningful EHR representations.

Conclusion: Volatility-aware pretraining with CV-Masking produces superior EHR representations compared to uniform masking strategies by accounting for the heterogeneous predictability of clinical biomarkers.
...
TL;DR: Systematic evaluation of tokenization strategies for clinical time series modeling shows explicit time encodings provide no consistent benefit, value features have task-dependent importance, and frozen pretrained code encoders outperform trainable ones with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Tokenization strategies significantly impact how models process electronic health records, but there's a lack of fair comparisons of their effectiveness. The paper aims to provide a systematic evaluation of different tokenization approaches for clinical time series modeling.

Method: Conducted controlled ablations across four clinical prediction tasks on MIMIC-IV using transformer-based architectures. Evaluated different tokenization strategies including explicit time encodings, value features, and code sequences. Compared frozen vs trainable pretrained code encoders and examined parameter efficiency.

Result: Explicit time encodings provided no consistent statistically significant benefit. Value features showed task-dependent importance (important for mortality prediction but not readmission). Frozen pretrained code encoders dramatically outperformed trainable counterparts while requiring fewer parameters. Larger clinical encoders with frozen embeddings provided consistent improvements across tasks.

Conclusion: Simpler, parameter-efficient approaches can achieve strong performance in many cases, though optimal tokenization strategy remains task-dependent. The controlled evaluation enables fairer tokenization comparisons and demonstrates that code sequences alone can carry sufficient predictive signal for certain tasks.
...
TL;DR: Combining Protein Language Models with Semi-Supervised Learning improves antigenicity prediction for influenza viruses when labeled data is scarce, enabling better use of genomic surveillance data for vaccine strain selection.


<details>
  <summary>Details</summary>
Motivation: Influenza A viruses evolve rapidly requiring frequent vaccine updates, but current HI assays for measuring antigenicity are labor-intensive and don't scale. This creates a bottleneck where genomic data vastly outpaces available phenotypic labels, limiting traditional supervised models.

Method: Evaluated two SSL strategies (Self-training and Label Spreading) against supervised baselines using four PLM embeddings (ESM-2, ProtVec, ProtT5, ProtBert) on HA sequences. Used nested cross-validation to simulate low-label regimes (25%, 50%, 75%, 100% label availability) across four IAV subtypes (H1N1, H3N2, H5N1, H9N2).

Result: SSL consistently improved performance under label scarcity. Self-training with ProtVec showed largest relative gains. ESM-2 remained highly robust, achieving F1 scores above 0.82 with only 25% labeled data. H1N1 and H9N2 predicted with high accuracy, while hypervariable H3N2 remained challenging but SSL mitigated performance decline.

Conclusion: Integrating PLMs with SSL can address the antigenicity labeling bottleneck, enabling more effective use of unlabeled surveillance sequences for rapid variant prioritization and timely vaccine strain selection.
...
TL;DR: VaRDASS introduces a variance reduction technique using stratified sampling for unsupervised domain adaptation, improving discrepancy estimation and target domain performance.


<details>
  <summary>Details</summary>
Motivation: Domain shift is a major challenge for real-world ML deployment. While UDA aims to minimize domain discrepancy, current methods suffer from high variance in stochastic settings, limiting their theoretical benefits.

Method: Proposes VaRDASS - a specialized stochastic variance reduction technique for UDA using stratified sampling. Derives ad hoc stratification objectives for correlation alignment and MMD discrepancy measures, and introduces a practical k-means style optimization algorithm.

Result: Provides expected and worst-case error bounds, proves theoretical optimality (variance minimization) for MMD under certain assumptions. Experiments on three domain shift datasets show improved discrepancy estimation accuracy and target domain performance.

Conclusion: VaRDASS effectively addresses variance issues in UDA through stratified sampling, offering theoretical guarantees and practical improvements for domain adaptation tasks.
...
TL;DR: MAR-FL is a peer-to-peer federated learning system that reduces communication overhead from O(N²) to O(N log N) using iterative group-based aggregation while maintaining robustness to network churn.


<details>
  <summary>Details</summary>
Motivation: Next-generation wireless systems need efficient and robust federated learning methods that can handle wireless-connected peers and network churn. Current P2P FL approaches suffer from excessive communication complexity, limiting their practical scalability.

Method: MAR-FL uses iterative group-based aggregation to reduce communication overhead. The system operates without a central coordinator and is designed to be robust to unreliable FL clients while supporting private computing integration.

Result: MAR-FL achieves communication costs that scale as O(N log N) compared to O(N²) in existing baselines, making it more scalable as the number of peers grows. The system maintains effectiveness and robustness to churn.

Conclusion: MAR-FL provides a scalable P2P FL solution that significantly reduces communication overhead while maintaining robustness to network churn and supporting private computing, addressing key limitations of existing approaches.
...
TL;DR: The paper proposes E-WL, an extension of the Weisfeiler-Lehman algorithm that incorporates edge features, and EGIN, a corresponding Graph Isomorphism Network model that leverages edge features for improved graph classification performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of the classical 1-WL algorithm and many GNNs that do not utilize edge features in graph learning, despite edge features being important in many real-world applications.

Method: The authors propose two main contributions: 1) E-WL algorithm that extends 1-WL to incorporate edge features during color refinement, and 2) EGIN model (Edged Graph Isomorphism Network) that builds upon E-WL to exploit edge features in graph neural networks.

Result: Experimental results on 12 edge-featured benchmark graph datasets show that the proposed EGIN models generally demonstrate superior performance in graph classification tasks compared to state-of-the-art baseline models.

Conclusion: The paper successfully addresses the limitation of edge feature utilization in graph learning by proposing E-WL and EGIN, which effectively incorporate edge features and achieve better performance on graph classification tasks.
...
<details>
  <summary>Details</summary>
Motivation: Quantum PDE solvers face severe near-term hardware constraints: limited qubit counts restrict spatial resolution to coarse grids, and circuit depth limitations prevent accurate long-time integration. These bottlenecks confine quantum solvers to low-fidelity regimes despite theoretical speedup potential.

Method: Introduces a multifidelity learning framework that trains a low-fidelity surrogate on abundant quantum solver outputs, then learns correction mappings through a multifidelity neural architecture balancing linear and nonlinear transformations. Demonstrated on benchmark nonlinear PDEs using quantum lattice Boltzmann methods.

Result: Successfully corrects coarse quantum predictions and achieves temporal extrapolation well beyond the classical training window. Produces predictions competitive with classical accuracy while reducing expensive high-fidelity simulation requirements.

Conclusion: Bridges the gap between hardware-limited quantum simulations and application requirements, establishing a pathway for extracting computational value from current quantum devices in real-world scientific applications, advancing both algorithm development and practical deployment of near-term quantum computing for computational physics.
...
<details>
  <summary>Details</summary>
Motivation: Current unlearning methods treat all data points equally, but some points have negligible impact on model learning. As data privacy concerns grow, efficient unlearning methods are needed.

Method: Uses influence functions to identify subsets of training data with negligible impact on model outputs, then reduces dataset size before applying unlearning techniques.

Result: Achieves significant computational savings (up to approximately 50%) on real-world empirical examples across language and vision tasks.

Conclusion: Not all data points need equal unlearning attention; identifying negligible-impact points enables more efficient unlearning frameworks with substantial computational benefits.
...
TL;DR: DMAGT is a transformer-based graph neural network model that predicts drug-miRNA associations with high accuracy (AUC up to 95.24%), validated through extensive experiments and practical drug case studies.


<details>
  <summary>Details</summary>
Motivation: Traditional wet lab experiments for exploring drug-miRNA associations are inefficient and costly. miRNAs play crucial roles in gene regulation and drug development, creating a need for computational methods to efficiently predict these associations.

Method: DMAGT transforms drug-miRNA associations into graphs, uses Word2Vec for embedding drug molecular structures and miRNA base sequences, and employs a multi-layer transformer-based graph neural network to learn from embedded features and relational structures for prediction.

Result: Achieved AUC up to 95.24±0.05 on three datasets (ncDR, RNAInter, SM2miR). In practical validation with 5-Fluorouracil and Oxaliplatin, 14 out of 20 top-predicted associations were successfully confirmed.

Conclusion: DMAGT demonstrates excellent performance and stability in predicting drug-miRNA associations, providing an efficient computational shortcut for miRNA-targeted drug development compared to traditional experimental approaches.
...
TL;DR: RSA2C is an attribution-aware actor-critic RL method that uses RKHS-SHAP state attributions to weight gradients and targets, improving interpretability and performance.


<details>
  <summary>Details</summary>
Motivation: Current actor-critic methods lack interpretability, and existing explainable RL approaches don't properly account for heterogeneous impacts of different state dimensions on rewards, treating all features equally.

Method: Proposes RSA2C with three RKHS-based components: Actor in vector-valued RKHS with Mahalanobis-weighted kernel, Value Critic and Advantage Critic in scalar RKHSs. Uses sparsified dictionaries and computes state attributions via RKHS-SHAP, converting them into Mahalanobis-gated weights that modulate Actor gradients and Advantage Critic targets.

Result: Theoretical convergence bound under state perturbations shows stability and efficiency. Empirical results on three continuous-control environments demonstrate improved efficiency, stability, and interpretability compared to standard approaches.

Conclusion: RSA2C successfully integrates attribution awareness into actor-critic methods, providing both theoretical guarantees and practical performance improvements while enhancing interpretability through state attribution analysis.
...
TL;DR: CFO is a continuous-time neural operator framework for PDEs that uses flow matching to learn PDE dynamics without autoregressive prediction, enabling time-resolution invariant training and inference with superior long-horizon stability.


<details>
  <summary>Details</summary>
Motivation: Autoregressive neural operators for time-dependent PDEs accumulate error over long rollouts and require uniform temporal discretization. There's a need for methods that avoid error accumulation while being computationally efficient and flexible with temporal resolution.

Method: CFO repurposes flow matching to directly learn PDE right-hand sides without ODE solver backpropagation. It fits temporal splines to trajectory data, uses finite-difference estimates of time derivatives at knots to construct probability paths, and trains a neural operator via flow matching to predict analytic velocity fields.

Result: CFO demonstrates superior long-horizon stability and remarkable data efficiency across four benchmarks (Lorenz, 1D Burgers, 2D diffusion-reaction, 2D shallow water). Trained on only 25% of irregularly subsampled time points, it outperforms autoregressive baselines trained on complete data with relative error reductions up to 87%.

Conclusion: CFO provides a time-resolution invariant framework for continuous-time PDE learning that avoids autoregressive error accumulation, enables flexible temporal querying, and achieves competitive efficiency while uniquely supporting reverse-time inference and arbitrary temporal resolution.
...
TL;DR: SVGP KANs combine sparse variational Gaussian processes with Kolmogorov-Arnold Networks for scalable Bayesian uncertainty quantification in scientific ML.


<details>
  <summary>Details</summary>
Motivation: Standard Kolmogorov-Arnold Networks lack principled uncertainty quantification needed for scientific applications, requiring a framework that can distinguish different types of uncertainty while maintaining interpretability.

Method: Integrate sparse variational Gaussian process inference with Kolmogorov-Arnold topology using analytic moment matching to propagate uncertainty through deep additive structures with quasi-linear computational complexity.

Result: Demonstrated framework's ability to distinguish aleatoric from epistemic uncertainty in three applications: fluid flow reconstruction noise calibration, advection-diffusion forecasting confidence degradation, and convolutional autoencoder out-of-distribution detection.

Conclusion: SVGP KANs are a promising architecture for uncertainty-aware learning in scientific machine learning, combining interpretability with principled uncertainty quantification.
...
TL;DR: SOTA ML models struggle to distinguish human vs LLM-generated scientific ideas, especially after paraphrasing, with detection performance dropping 25.4% after 5 paraphrasing stages. Context helps slightly, but simplified non-expert style paraphrasing erodes LLM signatures most.


<details>
  <summary>Details</summary>
Motivation: As LLMs become research agents, distinguishing between LLM and human-generated ideas is crucial for understanding LLMs' cognitive capabilities. While LLM text detection is well-studied, distinguishing scientific ideas remains unexplored.

Method: Systematically evaluate SOTA ML models' ability to differentiate human vs LLM-generated ideas, particularly after successive paraphrasing stages. Test detection performance with and without research problem context.

Result: Detection performance declines by average 25.4% after 5 consecutive paraphrasing stages. Incorporating research problem context improves detection by up to 2.97%. Detection struggles most when ideas are paraphrased into simplified, non-expert style.

Conclusion: Distinguishing human vs LLM-generated scientific ideas is challenging, especially after paraphrasing. Simplified non-expert style paraphrasing most effectively erodes LLM signatures, highlighting limitations in current detection methods.
...
TL;DR: DPER (Decoupled Prioritized Experience Replay) improves deep deterministic policy gradient algorithms by using separate, tailored transition batches for Actor and Critic networks instead of identical batches, enhancing training dynamics and performance.


<details>
  <summary>Details</summary>
Motivation: Actor-Critic architectures in deep reinforcement learning typically use identical batches for both networks, but their different learning objectives and update dynamics suggest this uniform approach may not be optimal. The paper aims to improve performance by providing appropriate learning signals to each component through decoupled experience replay.

Method: Introduces Decoupled Prioritized Experience Replay (DPER), which allows independent sampling of transition batches for Actor and Critic networks. DPER can be integrated into any off-policy deep RL algorithm for continuous control domains. The method is combined with Twin Delayed DDPG and evaluated on standard MuJoCo benchmarks.

Result: DPER outperforms conventional experience replay strategies including vanilla experience replay and prioritized experience replay across multiple MuJoCo tasks from the OpenAI Gym suite.
...
### [101] [Wasserstein distance based semi-supervised manifold learning and application to GNSS multi-path detection](https://arxiv.org/abs/2512.05567)
*Antoine Blais,Nicolas Couëllan*

Main category: cs.LG

TL;DR: Proposes optimal transport-based semi-supervised learning using Wasserstein distance for similarity metric in graph-based label propagation with deep CNNs, applied to GNSS multi-path interference detection.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of learning from scarce labeled image data by leveraging optimal transport theory for semi-supervised learning, particularly in real-world applications like GNSS where labeled data is limited.

Method: Uses implicit graph-based transductive semi-supervised learning with Wasserstein distance as similarity metric between image samples, integrated into label propagation mechanism during deep convolutional network training.

Result: For specific hyperparameter choices controlling semi-supervision amount and metric sensitivity, classification accuracy significantly improves over fully supervised training methods across various signal conditions.

Conclusion: Optimal transport-based semi-supervised approach effectively leverages limited labeled data for improved classification performance in GNSS multi-path interference detection, demonstrating practical value for real-world applications with scarce labeled data.
...
### [102] [Robustness Test for AI Forecasting of Hurricane Florence Using FourCastNetv2 and Random Perturbations of the Initial Condition](https://arxiv.org/abs/2512.05323)
...
<details>
  <summary>Details</summary>
Motivation: Understanding AI weather forecasting model robustness is crucial for assessing output reliability, especially for extreme weather events like hurricanes where accurate predictions are critical for safety and preparedness.
...
Result: FCNv2 preserves hurricane features under low-moderate noise; maintains general storm structure even with high noise (though positional accuracy degrades); consistently underestimates storm intensity/persistence; generates smooth forecasts from random inputs after few timesteps.

Conclusion: FCNv2 shows robust behavior with tendency toward stable, smoothed outputs, but has systematic intensity underestimation; approach is simple and portable for testing other AI weather models.
...
### [103] [Non-Convex Federated Optimization under Cost-Aware Client Selection](https://arxiv.org/abs/2512.05327)
*Xiaowen Jiang,Anton Rodomanov,Sebastian U. Stich*

Main category: cs.LG

TL;DR: The paper introduces a unified model for comparing federated optimization algorithms with different client-selection strategies, proposes a new algorithm with best-known communication and local computation complexities, and develops improved gradient estimators using SAGA and Recursive-Gradient techniques.


<details>
  <summary>Details</summary>
Motivation: Existing federated optimization comparison metrics don't account for different client-selection strategies (random subset, all clients, or hybrid) which have different communication costs in practice. There's a need for a unified model that quantifies these costs to fairly compare algorithms.

Method: 1) Introduces a new federated optimization model quantifying communication and local computation costs; 2) Proposes a new algorithm based on inexact composite gradient method with carefully constructed gradient estimator; 3) Uses SAGA variance-reduced gradient estimator with new variance bound analysis; 4) Introduces Recursive-Gradient technique to improve gradient estimators; 5) Creates RG-SAGA estimator with improved error bounds.

Result: The proposed algorithm achieves best-known communication and local computation complexities among existing federated optimization methods for non-convex optimization. The Recursive-Gradient technique improves error bounds for conditionally unbiased gradient estimators like SAGA and SVRG, with RG-SAGA showing better performance than original SAGA.

Conclusion: The paper provides a unified framework for comparing federated optimization algorithms with different client-selection strategies, introduces an algorithm with state-of-the-art complexity bounds, and develops improved gradient estimation techniques that can benefit various federated learning scenarios.
...
### [104] [PathFinder: MCTS and LLM Feedback-based Path Selection for Multi-Hop Question Answering](https://arxiv.org/abs/2512.05336)
*Durga Prasad Maram,Kalpa Gunaratna,Vijay Srinivasan,Haris Jeelani,Srinivas Chappidi*

Main category: cs.LG

TL;DR: PATHFINDER uses Monte Carlo Tree Search to generate training paths, filters them with quality checks, and reformulates sub-queries to improve multi-hop QA performance.


<details>
  <summary>Details</summary>
Motivation: Existing training-based approaches for multi-hop QA suffer from LLM hallucinations and incorrect reasoning paths that hinder performance, despite LLMs' reasoning capabilities.

Method: PATHFINDER uses: (i) Monte Carlo Tree Search to generate training path traces, (ii) quality filtering using sub-answer recall and LLM-as-a-judge verification to remove erroneous/lengthy traces, and (iii) sub-query reformulation for failed retrieval cases.

Result: PATHFINDER improves the performance of multi-hop QA over public benchmark datasets.
...
### [105] [Interaction Tensor Shap](https://arxiv.org/abs/2512.05338)
*Hiroki Hasegawa,Yukihiko Okada*

Main category: cs.LG

TL;DR: IT SHAP enables tractable computation of higher-order Shapley interactions via tensor network contractions, reducing exponential complexity to polynomial time.


<details>
  <summary>Details</summary>
Motivation: Existing Shapley-based methods cannot tractably evaluate higher-order feature interactions in deep, high-dimensional models - STII requires exponential enumeration and current tensor approaches are limited to first-order effects.

Method: Introduces Interaction Tensor SHAP (IT SHAP) that reformulates Shapley Taylor Interaction Index as contraction of Value and Weight Tensors, using Tensor Train structure with polynomial TT ranks for efficient computation.

Result: IT SHAP reduces exponential complexity Θ(4^n) of STII to NC2 parallel time, enabling polynomial time and polylog depth computation of higher-order interactions under TT structure.

Conclusion: IT SHAP provides a unified, axiomatic, and computationally tractable framework for scalable interaction-aware explainable AI, making interaction analysis feasible for large black-box models.
...
### [106] [Taxonomy-Adaptive Moderation Model with Robust Guardrails for Large Language Models](https://arxiv.org/abs/2512.05339)
*Mahesh Kumar Nandwana,Youngwan Lim,Joseph Liu,Alex Yang,Varun Notibala,Nishchaie Khanna*

Main category: cs.LG

TL;DR: Roblox Guard 1.0 is a safety-focused LLM built on Llama-3.1-8B-Instruct that provides comprehensive input-output moderation through instruction fine-tuning, using synthetic and open-source datasets with CoT rationales and input inversion techniques.


<details>
  <summary>Details</summary>
Motivation: Despite post-training alignment, LLMs can still generate inappropriate outputs that pose risks to users, highlighting the need for robust safeguards operating across both model inputs and outputs.

Method: Built on Llama-3.1-8B-Instruct, instruction fine-tuned using a mix of synthetic and open-source safety datasets, augmented with chain-of-thought rationales and input inversion to enhance contextual understanding and decision making.

Result: The model demonstrates strong performance on out-of-domain safety benchmarks and generalizes across previously unseen safety taxonomies. The authors also release RobloxGuard-Eval benchmark for systematic evaluation.

Conclusion: Roblox Guard 1.0 provides an effective safety moderation framework for LLM systems through comprehensive input-output moderation, with extensible evaluation capabilities via the released benchmark.
...
### [107] [When Forgetting Builds Reliability: LLM Unlearning for Reliable Hardware Code Generation](https://arxiv.org/abs/2512.05341)
*Yiwen Liang,Qiufeng Li,Shikai Wang,Weidong Cao*

Main category: cs.LG

TL;DR: A novel unlearning framework for LLMs in hardware code generation that removes problematic knowledge while preserving code quality.


<details>
  <summary>Details</summary>
Motivation: LLMs in hardware design face reliability issues due to memorization of proprietary IP, contaminated benchmarks, and unsafe coding patterns, requiring mitigation of these risks.

Method: Combines syntax-preserving unlearning strategy to maintain hardware code structural integrity with fine-grained floor-aware selective loss for precise removal of problematic knowledge.

Result: Supports forget sets up to 3x larger, typically requires only single training epoch, while preserving syntactic correctness and functional integrity of RTL codes.

Conclusion: The framework paves an avenue towards reliable LLM-assisted hardware design by enabling effective unlearning without degrading code generation capabilities.
...
### [108] [Enhancing Dimensionality Prediction in Hybrid Metal Halides via Feature Engineering and Class-Imbalance Mitigation](https://arxiv.org/abs/2512.05367)
...
<details>
  <summary>Details</summary>
Motivation: Predicting structural dimensionality of hybrid metal halides is challenging due to highly imbalanced datasets across dimensionality classes (0D, 1D, 2D, 3D), requiring specialized approaches to handle class imbalance.

Method: Combines chemically-informed feature engineering with advanced class-imbalance handling (SMOTE augmentation), interaction-based descriptors, multi-stage workflow with feature selection, model stacking, and performance optimization.
...
Conclusion: The framework successfully addresses class imbalance challenges in predicting HMH structural dimensionality through integrated feature engineering and imbalance handling techniques.
...
### [109] [Text Rationalization for Robust Causal Effect Estimation](https://arxiv.org/abs/2512.05373)
...
<details>
  <summary>Details</summary>
Motivation: High-dimensional text data in causal inference causes positivity assumption violations, leading to extreme propensity scores, unstable weights, and inflated variance in effect estimates.

Method: Confounding-Aware Token Rationalization (CATR) selects sparse necessary token subsets using a residual-independence diagnostic to preserve confounding information for unconfoundedness while discarding irrelevant text.
...
Conclusion: CATR effectively addresses positivity violations in text-based causal inference by selecting relevant tokens, improving estimation performance and interpretability.
...
### [110] [China Regional 3km Downscaling Based on Residual Corrective Diffusion Model](https://arxiv.org/abs/2512.05377)
*Honglu Sun,Hao Jing,Zhixiang Dai,Sa Xiao,Wei Xue,Jian Sun,Qifeng Lu*

Main category: cs.LG

TL;DR: This paper presents a diffusion-based statistical downscaling method (CorrDiff) that generates high-resolution 3km weather forecasts for China from 25km global model outputs, outperforming operational regional models.


<details>
  <summary>Details</summary>
Motivation: The need for efficient high-resolution numerical weather prediction, addressing the computational challenges of dynamical downscaling by using statistical downscaling with deep learning approaches.

Method: Uses CorrDiff diffusion model framework with modifications: expanded region coverage (20x larger), inclusion of high-level variables at six pressure levels, addition of global residual connection for improved accuracy. Applied to 25km forecasts from CMA-GFS and SFF models.

Result: The downscaled 3km forecasts outperform CMA-MESO regional model in MAE for target variables. As a generative model, CorrDiff produces realistic fine-scale details in radar composite reflectivity predictions compared to deterministic regression models.

Conclusion: Diffusion-based statistical downscaling (CorrDiff) is effective for generating high-resolution weather forecasts, offering computational efficiency while maintaining or exceeding the accuracy of operational regional models, with particular strength in generating realistic fine-scale details.
...
### [111] [Generalization Beyond Benchmarks: Evaluating Learnable Protein-Ligand Scoring Functions on Unseen Targets](https://arxiv.org/abs/2512.05386)
*Jakub Kopko,David Graber,Saltuk Mustafa Eyrilmez,Stanislav Mazurenko,David Bednar,Jiri Sedlar,Josef Sivic*

Main category: cs.LG

TL;DR: Current protein-ligand scoring functions perform well on standard benchmarks but struggle to generalize to novel protein targets, revealing a need for better evaluation protocols and methods to improve generalization.


<details>
  <summary>Details</summary>
Motivation: As machine learning becomes central to molecular design, ensuring the reliability of learnable protein-ligand scoring functions on novel protein targets is crucial. While many functions perform well on standard benchmarks, their generalization capability remains a significant challenge.

Method: The authors evaluate generalization capability using dataset splits that simulate evaluation on targets with limited known structures and experimental affinity measurements. They investigate whether large-scale self-supervised pretraining can bridge the generalization gap and probe simple methods that leverage limited test-target data to improve performance.

Result: The analysis reveals that commonly used benchmarks do not reflect the true challenge of generalizing to novel targets. Preliminary evidence suggests large-scale self-supervised pretraining has potential to bridge the generalization gap, and simple methods leveraging limited test-target data can improve scoring function performance.

Conclusion: The findings underscore the need for more rigorous evaluation protocols and offer practical guidance for designing scoring functions with predictive power extending to novel protein targets.
...
### [112] [Smart Timing for Mining: A Deep Learning Framework for Bitcoin Hardware ROI Prediction](https://arxiv.org/abs/2512.05402)
*Sithumi Wickramasinghe,Bikramjit Das,Dorien Herremans*

Main category: cs.LG

TL;DR: MineROI-Net: A Transformer-based model for predicting ASIC mining hardware profitability timing, achieving 83.7% accuracy in classifying returns as profitable, marginal, or unprofitable within one year.


<details>
  <summary>Details</summary>
Motivation: Bitcoin mining hardware acquisition lacks guidance despite being capital-intensive, with no computational frameworks addressing when to purchase ASICs given market volatility, technological obsolescence, and protocol-driven revenue cycles.

Method: Formulates hardware acquisition as time series classification task, predicting ROI categories. Proposes MineROI-Net, an open-source Transformer-based architecture capturing multi-scale temporal patterns in mining profitability.

Result: Outperforms LSTM and TSLANet baselines with 83.7% accuracy and 83.1% macro F1-score. Achieves 93.6% precision for unprofitable periods and 98.5% for profitable ones, with no misclassification between profitable/unprofitable scenarios.

Conclusion: MineROI-Net provides practical, data-driven tool for timing mining hardware acquisitions, potentially reducing financial risk in capital-intensive mining operations. Model is publicly available on GitHub.
...
### [113] [RevoNAD: Reflective Evolutionary Exploration for Neural Architecture Design](https://arxiv.org/abs/2512.05403)
*Gyusam Chang,Jeongyoon Yoon,Shin han yi,JaeHyeok Lee,Sujin Jang,Sangpil Kim*

Main category: cs.LG

TL;DR: RevoNAD is a reflective evolutionary orchestrator that bridges LLM-based reasoning with feedback-aligned neural architecture search, achieving SOTA performance across multiple datasets by combining multi-expert consensus, adaptive exploration, and Pareto-guided selection.


<details>
  <summary>Details</summary>
Motivation: Current LLM-driven neural architecture design faces challenges with discrete token-level design loops that are non-differentiable, preventing smooth feedback guidance. These methods often suffer from mode collapse into redundant structures or drift toward infeasible designs when constructive reasoning isn't well grounded.

Method: RevoNAD introduces three key components: 1) Multi-round Multi-expert Consensus to transfer isolated design rules into meaningful architectural clues, 2) Adaptive Reflective Exploration that adjusts exploration degree based on reward variance (explores when feedback is uncertain, refines when stable), and 3) Pareto-guided Evolutionary Selection that promotes architectures optimizing accuracy, efficiency, latency, confidence, and structural diversity.

Result: RevoNAD achieves state-of-the-art performance across CIFAR10, CIFAR100, ImageNet16-120, COCO-5K, and Cityscape datasets. Ablation and transfer studies further validate its effectiveness in enabling practically reliable and deployable neural architecture design.

Conclusion: RevoNAD successfully bridges LLM-based reasoning with feedback-aligned architectural search, overcoming limitations of previous LLM-driven NAD systems by providing a more stable, reliable, and deployable approach to neural architecture design.
...
### [114] [Sepsis Prediction Using Graph Convolutional Networks over Patient-Feature-Value Triplets](https://arxiv.org/abs/2512.05416)
*Bozhi Dan,Di Wu,Ji Xu,Xiang Liu,Yiziting Zhu,Xin Shu,Yujie Li,Bin Yi*

Main category: cs.LG

TL;DR: Triplet-GCN: A graph convolutional model using patient-feature-value triplets for early sepsis detection from EHR data, outperforming traditional tabular ML methods.


<details>
  <summary>Details</summary>
Motivation: Sepsis detection in ICU is challenging due to complex, sparse, heterogeneous EHR data. Traditional methods struggle with these characteristics, requiring better representation learning approaches.

Method: Triplet-GCN represents each encounter as patient-feature-value triplets, constructs bipartite EHR graph, learns patient embeddings via GCN + MLP. Uses type-specific preprocessing (median imputation, effect coding, mode imputation with embeddings), initializes patient nodes with summary statistics while keeping measurement values on edges.

Result: Outperforms strong tabular baselines (KNN, SVM, XGBoost, Random Forest) across discrimination and balanced error metrics in multi-center Chinese cohort (N=648). Achieves better sensitivity-specificity trade-off and improved utility for early warning.

Conclusion: Encoding EHR as triplets and propagating information over patient-feature graphs produces more informative patient representations than feature-independent models, offering simple end-to-end blueprint for deployable sepsis risk stratification.
...
TL;DR: TS-Hint: A time series foundation model with chain-of-thought reasoning that uses attention hints to improve MRR prediction in semiconductor manufacturing with limited data.


<details>
  <summary>Details</summary>
Motivation: Existing methods for predicting material removal rate in semiconductor manufacturing lose temporal dynamics by using static features and require large datasets for training, which is inefficient and limited in practical applications.
...
Result: Experimental results show the model is effective in limited data settings via few-shot learning and can learn directly from multivariate time series features.

Conclusion: TS-Hint addresses limitations of existing methods by preserving temporal dynamics and enabling effective learning with limited data through attention-based chain-of-thought reasoning.
...
TL;DR: IdealTSF framework leverages non-ideal negative samples to enhance time series forecasting by pretraining on negative data, transforming sequences to ideal positives, and applying adversarial optimization.


<details>
  <summary>Details</summary>
Motivation: Missing values and anomalies in time series data hinder deep learning forecasting performance. Previous approaches either extract features or treat suboptimal data as positive samples, but leveraging negative samples could be more effective for enhancing event prediction.

Method: Proposes IdealTSF framework with three progressive steps: 1) Pretraining by extracting knowledge from negative sample data, 2) Training by transforming sequence data into ideal positive samples, and 3) Optimization using negative optimization mechanism with adversarial disturbances.

Result: Extensive experiments show that negative sample data unlocks significant potential within basic attention architecture for time series forecasting, demonstrating improved performance.

Conclusion: IdealTSF is particularly well-suited for applications with noisy samples or low-quality data, as it effectively leverages both ideal positive and negative samples to enhance forecasting capabilities.
...
TL;DR: Ensembles can achieve high accuracy with small generalization gaps by reducing variance through averaging or controlled boosting, but their effectiveness depends on dataset characteristics like linearity, noise, and class imbalance.


<details>
  <summary>Details</summary>
Motivation: To understand how ensemble models balance accuracy and overfitting across different tabular classification tasks, and to determine when ensembles can maintain high accuracy while keeping generalization gaps small.

Method: Used repeated stratified cross-validation with statistical significance testing to compare linear models, single decision trees, and nine ensemble methods across four tabular datasets (Breast Cancer, Heart Disease, Pima Diabetes, Credit Card Fraud). Also computed dataset complexity indicators like linearity score, Fisher ratio, and noise estimates.

Result: Ensembles can reach high accuracy without large generalization gaps by reducing variance. On nearly linear/clean data, linear models generalize well and ensembles offer little benefit. On datasets with meaningful nonlinear structure, tree-based ensembles increase test accuracy by 5-7 points while keeping gaps below 3%. On noisy/imbalanced datasets, ensembles remain competitive but require regularization to avoid overfitting.

Conclusion: Ensembles effectively control variance through averaging or controlled boosting, but their success depends on dataset characteristics. Dataset complexity indicators can predict when ensembles will work well. The study provides practical guidance for model selection in real-world tabular applications based on data properties.
...
TL;DR: The paper compares Geometric Quantum Machine Learning models on molecular datasets (LiH and NH3), finding that permutational symmetric embedding is the most generalizable model for geometric learning.


<details>
  <summary>Details</summary>
Motivation: To understand how different types of symmetry equivariance in Quantum Machine Learning models affect performance and generalizability on molecular geometry datasets, and to establish criteria for model selection based on molecular structure.

Method: Comparative study of QML models with different symmetry properties: no symmetry equivariance, rotational and permutational equivariance, and graph embedded permutational equivariance. Uses classical equivariant model as baseline. Evaluates on two molecular datasets (linear LiH and trigonal pyramidal NH3) using accuracy and generalizability metrics.

Result: Graph embedding of features improves trainability for geometric datasets. Permutational symmetric embedding emerges as the most generalizable quantum ML model for geometric learning. Performance differentials reveal criteria for model selection based on molecular geometry.

Conclusion: The choice of symmetry equivariance in QML models significantly impacts performance on molecular geometry tasks. Graph embedding enhances trainability, and permutational symmetric embedding offers the best generalizability for geometric learning applications.
...
### [119] [Turbulence Regression](https://arxiv.org/abs/2512.05483)
*Yingang Fan,Binjie Ding,Baiyi Chen*

Main category: cs.LG

TL;DR: The paper proposes a NeuTucker decomposition model using discretized data to predict low-altitude turbulence from sparse 3D wind field data, outperforming traditional regression methods.


<details>
  <summary>Details</summary>
Motivation: Traditional methods struggle to accurately predict low-altitude turbulence states using only wind profile radar data due to complex factors and sparse observations. Current observational conditions make turbulence prediction challenging.

Method: Introduces a NeuTucker decomposition model that: 1) discretizes continuous input data to adapt to models requiring discrete inputs, and 2) constructs a four-dimensional Tucker interaction tensor to represent all possible spatio-temporal interactions among different elevations and three-dimensional wind speeds. Uses a low-rank Tucker decomposition model based on Tucker neural network.

Result: The discretized NeuTucF model demonstrates superior performance compared to various common regression models in estimating missing observations in real datasets.

Conclusion: The proposed NeuTucker decomposition approach effectively captures latent interactions in sparse 3D wind field data and provides better turbulence prediction than traditional methods.
...
### [120] [GRASP: Graph Reasoning Agents for Systems Pharmacology with Human-in-the-Loop](https://arxiv.org/abs/2512.05502)
*Omid Bazgir,Vineeth Manthapuri,Ilia Rattsev,Mohammad Jafarnejad*

Main category: cs.LG

TL;DR: GRASP is a multi-agent, graph-reasoning framework that converts natural language QSP model specifications into executable MATLAB/SimBiology code while preserving biological constraints, outperforming existing LLM approaches.


<details>
  <summary>Details</summary>
Motivation: Quantitative Systems Pharmacology (QSP) modeling is crucial for drug development but requires significant expert time, limiting throughput. Current approaches lack automated tools that can translate natural language specifications into executable models while maintaining biological fidelity.

Method: GRASP uses a multi-agent framework with typed biological knowledge graphs and a human-in-the-loop conversational interface. It features a two-phase workflow: Understanding (graph reconstruction of legacy code) and Action (constraint-checked, language-driven modification), orchestrated by a state machine with iterative validation. It performs breadth-first parameter alignment around new entities to identify dependencies and propose biologically plausible defaults.

Result: GRASP outperforms SME-guided Chain-of-Thought and Tree-of-Thought baselines across biological plausibility, mathematical correctness, structural fidelity, and code quality (≈9-10/10 vs. 5-7/10). BFS alignment achieves F1 = 0.95 for dependency discovery, units, and range validation.

Conclusion: Graph-structured, agentic workflows can make QSP model development both accessible and rigorous, enabling domain experts to specify mechanisms in natural language without sacrificing biomedical fidelity, potentially accelerating drug development.
...
### [121] [Credal and Interval Deep Evidential Classifications](https://arxiv.org/abs/2512.05526)
*Michele Caprio,Shireen K. Manchingal,Fabio Cuzzolin*

Main category: cs.LG

TL;DR: Novel credal and interval deep evidential classification methods (CDEC/IDEC) for uncertainty quantification that can abstain from predictions when uncertainty is too high, and provide robust probabilistic guarantees when uncertainty is acceptable.


<details>
  <summary>Details</summary>
Motivation: Addressing the critical challenge of uncertainty quantification in AI for better decision-making, risk assessment, and model reliability, particularly overcoming limitations of previous evidential deep learning approaches.

Method: CDEC uses credal sets (closed convex probability sets) while IDEC uses intervals of evidential predictive distributions. Both are trained with standard backpropagation using evidence theory-based loss functions to capture epistemic and aleatoric uncertainties.

Result: Achieved competitive predictive accuracy on MNIST, CIFAR-10/100, state-of-the-art OoD detection under epistemic/total uncertainty, well-calibrated prediction regions that expand under distribution shift, and stable uncertainty estimates with small ensembles.

Conclusion: CDEC and IDEC effectively address uncertainty quantification challenges by providing systematic assessment of both epistemic and aleatoric uncertainties, enabling abstention when uncertainty is excessive and robust probabilistic guarantees when acceptable.
...
### [122] [IDK-S: Incremental Distributional Kernel for Streaming Anomaly Detection](https://arxiv.org/abs/2512.05531)
...
<details>
  <summary>Details</summary>
Motivation: Streaming anomaly detection faces challenges in maintaining accuracy with evolving data distributions while ensuring real-time efficiency. Existing methods struggle to balance these requirements effectively.

Method: IDK-S builds on the Isolation Distributional Kernel framework with two key innovations: 1) leverages data-dependent kernel advantages from offline detection, and 2) implements lightweight incremental updates instead of full retraining, maintaining statistical equivalence to full models.

Result: Extensive experiments on 13 benchmarks show IDK-S achieves superior detection accuracy while operating substantially faster (often by an order of magnitude) than existing state-of-the-art methods.

Conclusion: IDK-S effectively addresses streaming anomaly detection challenges by combining the power of distributional kernels with efficient incremental updates, offering both high accuracy and computational efficiency.
...
### [123] [On the Theoretical Foundation of Sparse Dictionary Learning in Mechanistic Interpretability](https://arxiv.org/abs/2512.05534)
*Yiming Tang,Harshvardhan Saini,Yizhen Liao,Dianbo Liu*

Main category: cs.LG

TL;DR: First unified theoretical framework for sparse dictionary learning methods in mechanistic interpretability, explaining empirical phenomena like feature absorption and dead neurons.


<details>
  <summary>Details</summary>
Motivation: AI models learn meaningful concepts as directions in representation spaces, often encoded in superposition. While sparse dictionary learning methods (autoencoders, transcoders, crosscoders) show empirical success in disentangling concepts, they lack theoretical understanding beyond tied-weight autoencoders.

Method: Develop unified theoretical framework treating sparse dictionary learning as one optimization problem. Analyze how diverse methods instantiate this framework and study the optimization landscape rigorously.

Result: Provides first theoretical explanations for empirically observed phenomena: feature absorption, dead neurons, and neuron resampling technique. Validates theoretical results through controlled experiments.

Conclusion: Establishes formal grounding for the broader family of sparse dictionary learning methods in mechanistic interpretability, bridging empirical success with theoretical understanding.
...
### [124] [SCoNE: Spherical Consistent Neighborhoods Ensemble for Effective and Efficient Multi-View Anomaly Detection](https://arxiv.org/abs/2512.05540)
*Yang Xu,Hang Zhang,Yixiao Ma,Ye Zhu,Kai Ming Ting*

Main category: cs.LG

TL;DR: SCoNE is a novel multi-view anomaly detection method that represents consistent neighborhoods directly across views with data-dependent properties, achieving O(N) time complexity and superior accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view anomaly detection methods have two key issues: (1) they struggle to capture consistent neighbors well when neighbors are in regions of varied densities across views, leading to inferior accuracy; (2) they have high O(N²) computational cost, making them inapplicable for large datasets.

Method: SCoNE (Spherical Consistent Neighborhoods Ensemble) represents consistent neighborhoods directly with multi-view instances without intermediate representations. It uses data-dependent properties where neighborhoods are large in sparse regions and small in dense regions, enabling consistent neighborhood representation across views without learning.
...
Conclusion: SCoNE effectively addresses the limitations of existing multi-view anomaly detection methods by providing a more accurate and computationally efficient approach that scales well to large datasets.
...
### [125] [RoBoN: Routed Online Best-of-n for Test-Time Scaling with Multiple LLMs](https://arxiv.org/abs/2512.05542)
*Jonathan Geuter,Gregor Kornhardt*

Main category: cs.LG

TL;DR: RoBoN is a sequential multi-LLM routing approach that outperforms single-model best-of-n by leveraging model diversity at inference time without additional training.


<details>
  <summary>Details</summary>
Motivation: Traditional best-of-n uses a single model despite evidence that LLMs have complementary strengths across tasks. The authors aim to exploit model diversity at inference time to improve performance over any single model alone.

Method: RoBoN sequentially routes generations across multiple models using a reward model and agreement signal on predicted responses. It requires no additional training, maintains compute parity, and works with any plug-in reward model.

Result: Across reasoning benchmarks (MATH500, OlympiadBench, MinervaMath, GSM8K, MMLU), RoBoN consistently outperforms standard best-of-n applied to individual models for larger n, with gains up to 3.4% absolute accuracy, and beats uniform multi-model portfolio baselines.

Conclusion: Model diversity can be exploited at inference to improve best-of-n performance over any constituent model alone, providing a simple, training-free path to test-time scaling with multiple LLMs.
...
### [126] [Improving Local Fidelity Through Sampling and Modeling Nonlinearity](https://arxiv.org/abs/2512.05556)
*Sanjeev Shrestha,Rahul Dubey,Hui Liu*

Main category: cs.LG

TL;DR: A novel explanation method using MARS for non-linear local boundaries and N-ball sampling improves faithfulness over LIME by 37% RMSE reduction.


<details>
  <summary>Details</summary>
Motivation: LIME assumes linear local decision boundaries and fails to capture non-linear relationships, leading to incorrect explanations for complex black-box ML models used in high-stakes applications.

Method: Proposes using Multivariate Adaptive Regression Splines (MARS) to model non-linear local boundaries and N-ball sampling technique that samples directly from desired distribution instead of reweighting samples like LIME.

Result: Method yields more faithful explanations compared to baselines, achieving average 37% reduction in root mean square error and significantly improving local fidelity across three UCI datasets with different classifiers and kernel widths.

Conclusion: The proposed approach enhances local fidelity of explanations by better capturing non-linear relationships and improving sampling methodology, making it more suitable for explaining complex black-box models in critical applications.
...
TL;DR: Proposes Entropy Ratio Clipping (ERC) to stabilize RL training for LLMs by constraining global distribution shifts using entropy ratio between current and previous policies.


<details>
  <summary>Details</summary>
Motivation: Off-policy RL training for LLMs causes distribution shift that pushes policies beyond trust regions, leading to training instability (fluctuating entropy, unstable gradients). PPO-Clip only handles local importance clipping but ignores global distributional shift of actions.

Method: Introduces entropy ratio between current and previous policies as global metric for policy exploration changes. Proposes Entropy Ratio Clipping (ERC) mechanism with bidirectional constraints on entropy ratio to stabilize policy updates at global distribution level and compensate for PPO-clip's inability to regulate probability shifts of unsampled actions. Integrates ERC into DAPO and GPPO algorithms.

Result: Experiments across multiple benchmarks show ERC consistently improves performance.

Conclusion: ERC effectively addresses global distribution shift in LLM RL training, providing stable policy updates and improved performance over standard PPO-clip approaches.
...
TL;DR: Scaling preconditioned optimizers (Shampoo, SOAP, Muon) via proper hyperparameter transfer rules enables consistent speedups over AdamW for large language models, but incorrect scaling causes benefits to vanish at scale.


<details>
  <summary>Details</summary>
Motivation: Recent preconditioned optimizers show promise but have mixed replication results at scale; need to understand how to properly scale their hyperparameters to validate effectiveness for large models.

Method: Study hyperparameter scaling rules for learning rate and weight decay across optimizers, accounting for width/depth scaling, blocking, grafting, and spectral normalization; apply μP principles and test on Llama models from 190M to 1.4B parameters.

Result: Proper scaling enables Muon (1.4×) and Shampoo (1.3×) consistent speedups over AdamW across model sizes; incorrect scaling causes benefits to vanish; compute-optimal weight decay scales as 1/width; blocking and spectral normalization mitigate finite-width deviations.

Conclusion: Studying optimal hyperparameter transfer is essential for reliable optimizer comparison at scale; proper scaling rules enable preconditioned optimizers to maintain advantages over AdamW for large language models.
...
TL;DR: Proposes a GNN framework for community detection that allows flexible control over number of clusters, supporting both range specification and exact number requirements.


<details>
  <summary>Details</summary>
Motivation: Standard GNN-based community detection methods often fail to return the exact number of clusters even when specified, and many methods require pre-specification of cluster count which is computationally infeasible to search exhaustively.

Method: Introduces a flexible framework that allows users to specify either a plausible range for number of clusters or an exact number, with mechanisms to enforce these bounds during GNN training.

Result: The proposed method reliably returns the specified number of clusters when an exact count is required, and operates within user-specified bounds when a range is provided.

Conclusion: Provides a principled solution to the cluster count control problem in GNN-based community detection, offering flexibility while maintaining reliability.
...
TL;DR: The paper introduces Modular Jets to analyze whether a model's internal decomposition is uniquely determined by data, distinguishing between "mirage" regimes (multiple decompositions produce same jets) and "identifiable" regimes (jets uniquely determine decomposition).


<details>
  <summary>Details</summary>
Motivation: Traditional supervised learning evaluates models via predictive risk, which quantifies overall performance but doesn't address whether the internal modular decomposition of a model is uniquely determined by the data and evaluation design. There's a need to understand when different internal structures can produce identical input-output behavior.

Method: Introduces Modular Jets - empirical jets that are local linear response maps describing how each module reacts to small structured perturbations. Proposes MoJet algorithm for empirical jet estimation and mirage diagnostics. Proves a jet-identifiability theorem for two-module linear regression pipelines under mild rank assumptions.

Result: Shows that risk-only evaluation admits large families of "mirage decompositions" that implement the same input-to-output map, while access to module-level jets uniquely determines the internal factorization under certain conditions. Demonstrates the framework on linear/deep regression and pipeline classification.

Conclusion: Modular Jets provide a framework to analyze model decompositions beyond predictive risk, revealing when internal structures are uniquely identifiable versus when multiple decompositions remain observationally equivalent (mirage regimes). This offers deeper insights into model interpretability and structure determination.
...
TL;DR: SGTM (Selective GradienT Masking) improves upon Gradient Routing for safer pretraining by localizing knowledge to removable parameters, showing better robustness to label noise and adversarial fine-tuning compared to data filtering and other unlearning methods.


<details>
  <summary>Details</summary>
Motivation: Addressing dual-use risks in LLMs where data filtering is expensive and prone to mislabeling, and even small amounts of mislabeled harmful content could create dangerous capabilities in increasingly sample-efficient large models.

Method: Selective GradienT Masking (SGTM) - an improved variant of Gradient Routing that zero-masks selected gradients so target domain examples only update dedicated parameters, allowing later removal of those localized parameters.

Result: SGTM provides better retain/forget trade-off with labeling errors than data filtering and previous Gradient Routing; shows strong robustness to adversarial fine-tuning (7x more steps needed compared to RMU); tested on removing language knowledge from bilingual dataset and biology knowledge from Wikipedia.

Conclusion: SGTM offers a promising pretraining-time safety complement, especially where label noise is unavoidable, by localizing potentially harmful knowledge to removable parameters while maintaining robustness against adversarial attempts to restore removed knowledge.
...
<details>
  <summary>Details</summary>
Motivation: To explore whether AI-assisted end-user coding (using LLM-based assistants) can complement or replace traditional low-code/no-code platforms for end-user development, offering greater flexibility, faster development, and reduced vendor lock-in.

Method: Conducted a case study where non-programmers were asked to develop a basic web app through interaction with AI assistants, analyzing their success rates, completion times, and feedback.

Result: Majority of participants successfully completed the web app development task in reasonable time and expressed support for AI-assisted end-user coding as a viable approach for end-user development.

Conclusion: AI-assisted end-user coding appears to be a feasible paradigm that could complement or even replace traditional LCNC platforms, with implications for practice, research, and academic teaching.
...
TL;DR: The paper proposes a POMDP-based online search approach for beam selection in 5G/6G networks that outperforms supervised learning methods by orders of magnitude and handles new trajectories and environmental changes.


<details>
  <summary>Details</summary>
Motivation: Current analog beamforming uses codebooks with pre-configured beams, but optimal beam selection is challenging due to large codebooks, reflections, blockages, and moving UEs. Supervised learning approaches fail to handle new trajectories and environmental changes.
...
Result: The method outperforms previous supervised learning approaches by orders of magnitude and successfully handles new or unforeseen UE trajectories and changes in the physical environment.

Conclusion: The POMDP-based online search approach provides a robust solution for beam selection in dynamic 5G/6G environments, overcoming limitations of supervised learning methods and significantly improving performance.
...
<details>
  <summary>Details</summary>
Motivation: Cellular networks need intelligent solutions for traffic prediction and energy optimization to reduce power consumption while maintaining service quality, requiring flexible systems that can balance competing objectives according to operator preferences.

Method: BERTO uses BERT-based transformer architecture with a Balancing Loss Function and prompt-based customization. Natural language prompts guide the model to manage underprediction/overprediction trade-offs according to operator intent.

Result: BERTO achieves 4.13% reduction in MSE compared to existing models, operates over flexible ranges (1.4 kW power variation, up to 9× service quality variation), and enables balancing power saving vs. performance through simple natural language inputs.

Conclusion: BERTO provides an effective, flexible solution for intelligent RAN deployments that allows operators to customize the trade-off between energy savings and network performance using natural language prompts.
...
TL;DR: Language models trained on MechSMILES format achieve high accuracy in predicting chemical reaction mechanisms, enabling applications in synthesis planning validation, atom mapping, and catalyst-aware template extraction.


<details>
  <summary>Details</summary>
Motivation: Current Computer-Assisted Synthesis Planning (CASP) systems lack mechanistic grounding, which is crucial for understanding synthesizability and chemical validity.
...
Conclusion: Mechanistic understanding enables three key applications: post-hoc validation for CASP systems, holistic atom-to-atom mapping, and catalyst-aware reaction template extraction, providing a pathway toward more explainable and chemically valid computational synthesis planning.
...
TL;DR: ABM-NNs: A framework for designing neural networks that preserve agent-based model principles through restricted graph neural networks and hierarchical decomposition, validated on ecological, epidemiological, and macroeconomic systems.


<details>
  <summary>Details</summary>
Motivation: Standard neural differential equations fail to enforce important constraints in complex systems (mass conservation, network locality, bounded rationality) that are crucial for agent-based models, necessitating structure-preserving neural architectures.

Method: Agent-Based-Model informed Neural Networks (ABM-NNs) using restricted graph neural networks and hierarchical decomposition to learn interpretable, structure-preserving dynamics while enforcing domain-specific constraints.

Result: Successful validation across three case studies: (1) recovered ground-truth parameters in Lotka-Volterra systems with interventions, (2) outperformed GCN, GraphSAGE, and Graph Transformer in SIR contagion forecasting and noise robustness, (3) learned coupled GDP dynamics from real macroeconomic data enabling gradient-based counterfactual policy analysis.

Conclusion: ABM-NNs provide an effective framework for learning interpretable, structure-preserving dynamics in complex systems while outperforming standard neural approaches and enabling practical applications like counterfactual policy analysis.
...
TL;DR: The paper develops a theoretical framework showing that learnability windows in recurrent neural networks are governed by effective learning rates from gate-induced Jacobian products, not just numerical stability, with sample complexity scaling inversely with these rates under heavy-tailed gradient noise.


<details>
  <summary>Details</summary>
Motivation: Classical analyses focus on numerical stability of Jacobian products in RNNs, but this alone is insufficient to explain learnability windows - the temporal horizons over which gradient information remains statistically recoverable. The paper aims to develop a more comprehensive theoretical framework that links gate mechanisms, gradient noise, and sample complexity.

Method: The authors develop a theoretical framework analyzing gating mechanisms through first-order expansions of gate-induced Jacobian products in Backpropagation Through Time. They derive effective learning rates μ_{t,ℓ} as per-lag, per-neuron quantities that act as multiplicative filters controlling gradient transport. They analyze learnability under heavy-tailed (α-stable) gradient noise and derive scaling laws.

Result: The minimal sample size to detect dependencies at lag ℓ scales as N(ℓ) ∝ f(ℓ)^{-α}, where f(ℓ)=∥μ_{t,ℓ}∥₁ is the effective learning rate envelope. This leads to explicit formulas for learnability windows H_N and closed-form scaling laws for logarithmic, polynomial, and exponential decay patterns. Broader/more heterogeneous gate spectra produce slower decay and larger learnability windows, while heavier-tailed noise compresses learnability windows.

Conclusion: Effective learning rates from gate-induced Jacobian products are the fundamental quantities governing when and for how long gated recurrent networks can learn long-range temporal dependencies, linking gate time-scale structure, gradient noise, and sample complexity in a unified framework.
...
TL;DR: TopK SAEs reveal biological concepts but lack causal control; Ordered SAEs enable reliable steering but with less interpretable patterns.


<details>
  <summary>Details</summary>
Motivation: To investigate and steer an autoregressive antibody language model (p-IgGen) using sparse autoencoders for mechanistic interpretability.

Method: Employed TopK and Ordered SAEs on p-IgGen to analyze latent features and test generative steering capabilities.

Result: TopK SAEs reveal biologically meaningful features but high concept correlation doesn't guarantee causal control. Ordered SAEs reliably identify steerable features but produce more complex, less interpretable activation patterns.

Conclusion: TopK SAEs are sufficient for concept mapping, while Ordered SAEs are preferable for precise generative steering in protein language models.
...
TL;DR: The paper proposes a unified theoretical framework for dataset distillation that explains performance saturation and configuration diversity scaling, showing different matching methods are interchangeable surrogates for the same generalization error.


<details>
  <summary>Details</summary>
Motivation: Existing dataset distillation methods lack theoretical foundations - they use heterogeneous surrogate objectives and optimization assumptions, making it hard to analyze common principles or provide guarantees. It's unclear when distilled data retains effectiveness across different training configurations.
...
Result: Two main theoretical results: (1) scaling law showing error decreases with distilled sample size, explaining performance saturation; (2) coverage law showing required distilled sample size scales linearly with configuration diversity. Shows various matching methods are interchangeable surrogates reducing the same generalization error.

Conclusion: The unified framework advances theoretical foundation for dataset distillation, enables theory-driven design of compact, configuration-robust distilled datasets, and explains why different methods work and how surrogate choices affect efficiency and robustness.
...
<details>
  <summary>Details</summary>
Motivation: The computational cost of optimizing acquisition functions in hypervolume-based Bayesian optimization is a significant bottleneck, primarily due to expensive hypervolume improvement calculations. While HV box-decomposition offers efficiency, it suffers from super-polynomial memory complexity O(MN^⌊(M+1)/2⌋) in worst cases. An approximation algorithm was proposed by Couckuyt et al. (2012) but lacks rigorous algorithmic description in literature.

Method: The paper provides comprehensive mathematical and algorithmic details for the approximation algorithm proposed by Couckuyt et al. (2012) to address the memory complexity issues in hypervolume box-decomposition for Bayesian optimization.

Result: The paper bridges the gap in literature by providing the missing rigorous algorithmic description of the approximation algorithm for hypervolume calculations in Bayesian optimization.

Conclusion: This work completes the documentation of an important approximation technique for hypervolume-based Bayesian optimization, making the algorithm more accessible and implementable for researchers and practitioners working on multi-objective decision-making problems.
...
TL;DR: NEAT is an autoregressive model for 3D molecular generation that treats molecules as sets of atoms, achieving permutation invariance without relying on canonical orders or focus atoms.


<details>
  <summary>Details</summary>
Motivation: Autoregressive models for 3D molecular generation face a fundamental mismatch: while text has natural sequential order, molecular graphs should be invariant to atom permutations. Previous approaches used canonical orders or focus atoms to work around this, but this is unnecessary and limits model flexibility.

Method: NEAT (Neighborhood-guided, Efficient, Autoregressive, Set Transformer) treats molecular graphs as sets of atoms and learns order-agnostic distributions over admissible tokens at the graph boundary using an autoregressive flow model. It achieves permutation invariance by modeling molecules as sets rather than sequences.

Result: NEAT approaches state-of-the-art performance in 3D molecular generation while maintaining high computational efficiency and achieving atom-level permutation invariance.

Conclusion: NEAT establishes a practical foundation for scalable molecular design by providing an efficient, permutation-invariant autoregressive approach that doesn't rely on artificial ordering schemes, making it more suitable for molecular structure generation.
...
<details>
  <summary>Details</summary>
Motivation: To demonstrate that transformer attention contains significant redundancy and can be made orders of magnitude sparser while preserving performance, potentially leading to more structured and interpretable models.

Method: Post-training method using flexible sparsity regularization under a constrained-loss objective, applied to models up to 1B parameters to reduce attention connectivity while retaining original pretraining loss.

Result: Achieved ≈0.3% attention edge retention (orders of magnitude sparser) without sacrificing performance, with cascading effects: task-specific circuits involve far fewer components (attention heads and MLPs) with up to 100x fewer connecting edges.

Conclusion: Transformer attention contains substantial computational redundancy, and sparsity can serve as a guiding principle for creating more structured, interpretable models while preserving capability.
...
TL;DR: SNNs with Bayesian Optimization and novel Penalized Spike Accuracy objective outperform conventional models for high-frequency price spike forecasting, achieving 76.8% cumulative return.


<details>
  <summary>Details</summary>
Motivation: Conventional financial models fail to capture fine temporal structure needed for high-frequency trading, especially sudden price spikes. SNNs offer biologically inspired framework well-suited for processing discrete events and millisecond-scale timing.

Method: Convert high-frequency stock data into spike trains and evaluate three architectures: unsupervised STDP-trained SNN, novel SNN with explicit inhibitory competition, and supervised backpropagation network. Use Bayesian Optimization with novel Penalized Spike Accuracy (PSA) objective to align predicted spike rates with empirical rates.

Result: Models optimized with PSA consistently outperformed Spike Accuracy-tuned counterparts and baselines. Extended SNN model with PSA achieved highest cumulative return (76.8%) in simple backtesting, significantly surpassing supervised alternative (42.54% return).

Conclusion: SNNs, when robustly tuned with task-specific objectives like PSA, show strong potential for effective price spike forecasting in high-frequency trading environments.
...
TL;DR: ML models predict vapor pressure to discover new space-suitable liquid lubricants for moving mechanical assemblies in vacuum conditions.


<details>
  <summary>Details</summary>
Motivation: Current space-suitable liquid lubricants are limited - few have low enough vapor pressure for vacuum conditions, and existing options impose design constraints on moving mechanical assemblies. There's a need to discover new lubricants that can function in space environments.

Method: Data-driven machine learning approach using models trained on both high-throughput molecular dynamics simulations and experimental databases. Models prioritize interpretability to identify relationships between chemical structure and vapor pressure.

Result: ML models successfully predict vapor pressure, enabling virtual screening. Several candidate molecules with promising properties for space lubricant applications are proposed based on the structural insights gained.

Conclusion: The ML approach provides an effective method for discovering new space-suitable liquid lubricants by predicting vapor pressure and understanding structure-property relationships, potentially overcoming limitations of current lubricants for moving mechanical assemblies in space.
...
TL;DR: Neural Coherence: A novel model selection method using activation statistics to select optimal pre-trained checkpoints for few-shot, unlabeled, out-of-distribution target tasks.


<details>
  <summary>Details</summary>
Motivation: Current model selection methods fail when target task data is scarce, unlabeled, and out-of-distribution, making it unreliable to choose the best pre-trained checkpoint for fine-tuning.

Method: Proposes Neural Coherence - characterizing model activation statistics for source and target domains to create data-efficient model selection methods that work with just a few unlabeled examples.

Result: Significantly improves generalization across diverse target domains (Food-101, PlantNet-300K, iNaturalist) and meta-learning settings compared to established baselines. Also effective for training data selection.

Conclusion: Neural Coherence provides a powerful, versatile principle for model selection that works reliably with minimal unlabeled data, addressing a critical challenge in transfer learning for out-of-distribution tasks.
...
TL;DR: DAE-HardNet is a physics-constrained neural network that strictly enforces differential-algebraic equations (DAEs) through a differentiable projection layer, achieving orders of magnitude better physics constraint satisfaction than traditional PINNs while maintaining prediction accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional PINNs only minimize physics constraints in a soft manner and don't strictly satisfy differential-algebraic equations, especially when constraints involve differential operators. There's a need for neural networks that can strictly embed domain knowledge and first-principles while learning both functions and their derivatives simultaneously.

Method: DAE-HardNet uses a differentiable projection layer that projects model predictions onto the constraint manifold, enforcing both algebraic and differential constraints. It learns both functions and their derivatives simultaneously, unlike traditional approaches that treat functions as black-boxes whose derivatives are obtained only after function evaluation.

Result: Compared to MLPs and PINNs, DAE-HardNet achieves orders of magnitude reduction in physics loss while maintaining prediction accuracy. It successfully learns derivatives, improves constrained learning of the backbone network, and can estimate unknown parameters. For some problems, the projection layer can be bypassed for faster inference.

Conclusion: DAE-HardNet provides a physics-constrained approach that strictly satisfies DAEs through differentiable projection, offering significant improvements over soft-constraint methods like PINNs while enabling derivative learning and parameter estimation capabilities.
...
<details>
  <summary>Details</summary>
Motivation: The fractional Poisson process models event arrivals with memory and long-range dependence, but traditional parameter estimation methods like method of moments may not effectively capture temporal dependencies in inter-arrival time sequences.

Method: Recurrent neural network (RNN) framework using Long Short-Term Memory (LSTM) networks to estimate FPP parameters μ>0 and β∈(0,1) from sequences of inter-arrival times, leveraging LSTM's ability to model temporal dependencies.

Result: On synthetic data: 55.3% reduction in mean squared error compared to traditional method of moments, reliable performance across different training conditions. On real-world data: Effective tracking of daily patterns and parameter changes in emergency call records and stock trading data, demonstrating effectiveness on complex time-dependent real-world data.

Conclusion: LSTM-based approach provides superior parameter estimation for fractional Poisson processes compared to traditional methods, effectively capturing temporal dependencies in both synthetic and real-world high-frequency event data with memory and long-range dependence characteristics.
...
TL;DR: A method for constructing Lipschitz-constrained deep residual networks using Linear Matrix Inequality framework with LDL decomposition, achieving better accuracy than SLL Layers on UCI datasets.


<details>
  <summary>Details</summary>
Motivation: ResNets are successful but controlling Lipschitz constant is important for adversarial robustness and network certifiability. Need rigorous methods to design Lipschitz-constrained networks.

Method: Reformulate ResNet architecture as cyclic tridiagonal LMI, derive closed-form constraints for Lipschitz continuity. Use new LDL decomposition approach to extend construction to any nonlinear architecture. Employ Cholesky decomposition for efficient parameterization.

Result: LDL formulation is tight relaxation of SDP-based network, maintains full expressiveness, achieves 3%-13% accuracy gains over SLL Layers on 121 UCI data sets.

Conclusion: Provides provable parameterization methodology for Lipschitz-constrained residual networks and hierarchical architectures, enabling robust designs for adversarial robustness, certified training, and control systems.
...
TL;DR: KQ-SVD: A method that directly performs optimal low-rank decomposition of attention matrices via closed-form solution, preserving attention outputs better than previous KV cache compression methods.


<details>
  <summary>Details</summary>
Motivation: KV cache becomes a major memory bottleneck as sequence length and batch size grow in LLMs. Prior compression methods apply suboptimal strategies that neglect that attention fundamentally depends on inner products between queries and keys.

Method: KQ-SVD directly performs optimal low-rank decomposition of the attention matrix via a closed-form solution, targeting the true source of redundancy by considering both queries and keys together rather than compressing them separately.

Result: KQ-SVD preserves attention outputs with higher fidelity under compression and consistently delivers superior projection quality in extensive evaluations on LLaMA and Mistral models.

Conclusion: Directly decomposing the attention matrix via KQ-SVD is more effective than previous KV cache compression methods that separately compress keys or jointly embed queries and keys, as it better preserves the attention mechanism's fundamental operations.
...
TL;DR: The paper addresses limitations in existing surrogate losses for disagreement discrepancy, proves they lack Bayes consistency, introduces new theoretical bounds, and proposes a novel provably consistent surrogate loss that outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks fail under distribution shifts, and while disagreement discrepancy is a promising approach for addressing this, existing surrogate losses for optimizing it are fundamentally flawed and not Bayes consistent.

Method: The authors prove existing surrogates lack Bayes consistency, provide new theoretical bounds on optimality gaps, and propose a novel disagreement loss paired with cross-entropy that yields a provably consistent surrogate for disagreement discrepancy.

Result: Empirical evaluations across diverse benchmarks show the proposed method provides more accurate and robust estimates of disagreement discrepancy than existing approaches, especially under challenging adversarial conditions.

Conclusion: The paper successfully addresses a fundamental flaw in existing approaches to disagreement discrepancy optimization by introducing a theoretically-grounded, provably consistent surrogate loss that improves robustness to distribution shifts.
...
TL;DR: Synthetic data generation for business surveys to protect confidentiality while preserving statistical properties, demonstrated with 2007 Survey of Business Owners data.


<details>
  <summary>Details</summary>
Motivation: Increased computing power and big data availability have heightened re-identification risks for anonymized census data, threatening confidentiality pledges to survey respondents. Business data presents unique challenges due to lack of anonymity and easy industry identification in geographic areas.

Method: Uses machine learning model to construct synthetic public-use microdata samples (PUMS) that preserve critical moments of empirical data without containing actual respondent records. Applied to Annual Business Survey (ABS) and demonstrated with 2007 Survey of Business Owners data.

Result: Econometric replication of a high-impact analysis published in Small Business Economics demonstrates the verisimilitude of synthetic data to true data. Quality metrics show synthetic data preserves statistical properties while protecting confidentiality.

Conclusion: Synthetic data generation is a viable solution for creating public-use business survey data that maintains confidentiality while enabling meaningful research. The approach shows promise for ABS applications and addresses unique challenges of business data anonymity.
...
TL;DR: Impugan is a conditional GAN for imputing missing values and integrating heterogeneous datasets, outperforming traditional methods by capturing nonlinear relationships.


<details>
  <summary>Details</summary>
Motivation: Real-world data often has missing values due to sensor failures, inconsistent records, and heterogeneous data sources. Traditional imputation methods rely on strong linearity and independence assumptions that don't hold for complex data, leading to biased estimates.

Method: Impugan uses a conditional Generative Adversarial Network (cGAN) trained on complete samples. The generator reconstructs missing entries from observed features, while the discriminator enforces realism by distinguishing true from imputed data through adversarial training.
...
TL;DR: MaxShapley is an efficient algorithm for fair attribution in generative search engines using RAG, reducing computational cost from exponential to linear while maintaining comparable attribution quality to exact Shapley values.


<details>
  <summary>Details</summary>
Motivation: Generative search engines based on LLMs are replacing traditional search, changing how information providers are compensated. There's a need for fair mechanisms to attribute and compensate content providers based on their contributions to generated answers in this new ecosystem.

Method: MaxShapley is introduced as a special case of Shapley value that leverages a decomposable max-sum utility function. This allows computing attributions with linear computation in the number of documents, rather than the exponential cost of traditional Shapley values.

Result: Evaluation on three multi-hop QA datasets (HotPotQA, MuSiQUE, MS MARCO) shows MaxShapley achieves comparable attribution quality to exact Shapley computation while consuming significantly fewer tokens. It gives up to 8x reduction in resource consumption over prior state-of-the-art methods at the same attribution accuracy.

Conclusion: MaxShapley provides an efficient solution for fair attribution in generative search pipelines, enabling sustainable compensation for content providers in the LLM-based search ecosystem with dramatically reduced computational overhead.
...
TL;DR: RL for LLMs causes diversity loss due to mode-seeking behavior; new method uses α-divergence with filtered target distribution to balance precision and diversity, achieving SOTA on Lean theorem-proving benchmark.


<details>
  <summary>Details</summary>
Motivation: RL training for LLMs leads to significant loss in diversity because RL implicitly optimizes the mode-seeking Reverse KL divergence, causing models to concentrate on high-probability regions while neglecting others. This reduces the model's ability to explore diverse solutions.

Method: Start from an explicit target distribution obtained by filtering out incorrect answers while preserving relative probabilities of correct ones. Approximate this target distribution using the α-divergence family, which unifies prior approaches and enables direct control of precision-diversity trade-off by interpolating between mode-seeking and mass-covering divergences.

Result: On a Lean theorem-proving benchmark, the method achieves state-of-the-art performance along the coverage-precision Pareto frontier, outperforming all prior methods on the coverage axis.

Conclusion: Using α-divergence with explicit target distributions provides better control over the precision-diversity trade-off in LLM fine-tuning, addressing the diversity loss problem in RL-based approaches and achieving superior performance on reasoning tasks.
...
<details>
  <summary>Details</summary>
Motivation: SMEs rely heavily on tacit, undocumented knowledge that gets lost, creating barriers to process improvement and institutional knowledge preservation. Traditional process modeling requires specialized skills and is too expensive for SMEs.

Method: Developed an LLM-driven conversational assistant using Gemini 2.5 Pro with Gradio front-end and bpmn-js visualization. The system conducts interview-style dialogues to elicit process details, supports clarifying conversations, and renders live BPMN diagrams that users can refine in real-time.

Result: Proof-of-concept evaluation in equipment-maintenance scenario showed the chatbot produced accurate "AS-IS" models, flagged issues via annotations, and generated improved "TO-BE" variants within ~12 minutes at SME-friendly API costs. System successfully enforced BPMN 2.0 XML schemas.

Conclusion: Conversational LLMs can lower skill and cost barriers for rigorous process documentation in SMEs, helping preserve institutional knowledge, enhance operational transparency, and accelerate continuous improvement. Future work includes agentic and multimodal deployments.
...
<details>
  <summary>Details</summary>
Motivation: Evaluating faithfulness of LLMs to a given task is complex, and there's a need for unsupervised metrics that can assess how well LLMs stay faithful to the source context when generating answers.
...
Result: Demonstrates the framework on LLM summarization of corporate SEC 10-K filings. Shows that high faithfulness generally implies low entropy production, and both metrics can be used jointly or separately for LLM evaluation and hallucination control.

Conclusion: Proposes novel unsupervised metrics for LLM faithfulness evaluation based on information theory and thermodynamics, providing a principled approach to assess and control hallucinations in LLM outputs.
...
<details>
  <summary>Details</summary>
Motivation: To address the gap between traditional machine learning education and the rapid emergence of Large Language Models, creating a comprehensive curriculum that prepares students for the evolving AI landscape.

Method: A two-part sequential course structure: foundational machine learning concepts followed by contemporary LLM applications, implemented over two seven-week terms with detailed architecture, implementation strategies, and assessment methods.

Result: The integrated approach successfully enhances student comprehension of the AI landscape and better prepares them for industry demands in the rapidly evolving field of artificial intelligence.

Conclusion: Systematically bridging traditional ML with modern LLMs through structured pedagogical design effectively develops comprehensive AI understanding and practical skills for students.
...
TL;DR: This paper proves that no algorithm can demonstrate truly new functional capabilities beyond what was already present in its initial design, thus establishing fundamental limits on algorithmic creativity and AGI.


<details>
  <summary>Details</summary>
Motivation: The motivation is to establish fundamental limits on artificial general intelligence (AGI) by examining the upper bounds of any machine-computable process, addressing the question of how close humanity is to developing human-level intelligence in machines.

Method: The authors adopt a specific definition of AGI from prior work that emphasizes creativity and innovation in unlocking new functional capabilities. Using this definition, they conduct formal mathematical proof to establish bounds on computational limits.

Result: The paper formally proves that no algorithm can demonstrate new functional capabilities that weren't already present in the initial algorithm itself, meaning algorithms cannot be truly creative in any field (science, engineering, art, etc.).

Conclusion: The conclusion discusses implications for AI development and human intelligence origins, suggesting that while AI can demonstrate existing capabilities and their combinations, true creativity and innovation beyond initial algorithmic design is fundamentally impossible for any computational process.
...
<details>
  <summary>Details</summary>
Motivation: To resolve the crisis in uncertainty reasoning caused by Dempster-Shafer Theory paradoxes, and demonstrate that possibility theory offers a mathematically rigorous alternative that aligns with natural intelligence logic.

Method: Axiomatic approach from Bychkov's article building a logically consistent foundation using dualistic possibility and necessity measures, with comparative analysis of probabilistic, evidential, and possibilistic paradigms using a classic medical diagnostic dilemma.
...
Conclusion: Possibility theory is not merely an alternative to DST but provides a fundamental resolution to its paradoxes through mathematically rigorous axiomatic foundations that better handle uncertainty and contradictory data.
...
TL;DR: The paper advocates for "co-improvement" (human-AI collaboration in AI research) as a safer and more achievable alternative to pure AI self-improvement, aiming for co-superintelligence through symbiosis.


<details>
  <summary>Details</summary>
Motivation: Current focus on AI self-improvement is dangerous and may take too long to achieve. The authors argue that human-AI collaborative improvement is more achievable and safer for humanity.

Method: Proposes focusing on developing AI systems that can work with human researchers throughout the entire AI research process - from ideation to experimentation - specifically targeting improved collaboration capabilities.

Result: Not specified in the abstract (this appears to be a position paper advocating for a research direction rather than presenting completed research results).

Conclusion: Co-improvement through human-AI symbiosis will accelerate AI research more safely and lead to co-superintelligence, benefiting both humans and AIs while reducing risks associated with pure AI self-improvement.
...
TL;DR: MCP-AI is a novel healthcare AI architecture combining Model Context Protocol with clinical applications, enabling autonomous, context-aware reasoning with physician oversight, validated through two clinical use cases.


<details>
  <summary>Details</summary>
Motivation: Traditional healthcare AI systems struggle with integrating contextual reasoning, long-term state management, and human-verifiable workflows. There's an urgent need for autonomous, context-aware clinical reasoning frameworks as healthcare systems become more complex.

Method: Introduces MCP-AI architecture built on Model Context Protocol (MCP) - a modular, executable specification for orchestrating generative and descriptive AI agents in real-time workflows. Each MCP file captures clinical objectives, patient context, reasoning state, and task logic as reusable, auditable memory objects.
...
Conclusion: MCP-AI provides a scalable basis for interpretable, composable, and safety-oriented AI in clinical environments, representing a significant shift from traditional CDSS and prompt-based LLMs by supporting adaptive, longitudinal, and collaborative reasoning across care settings.
...
TL;DR: ChipMind is a knowledge graph-augmented reasoning framework that addresses LLMs' context window limitations for IC development by transforming circuit specs into domain-specific knowledge graphs and using adaptive retrieval with semantic filtering.


<details>
  <summary>Details</summary>
Motivation: LLMs have great potential for automating integrated circuit development, but their practical deployment is limited by restricted context windows. Existing context-extension methods fail to effectively model semantics and perform multi-hop reasoning over complex circuit specifications.
...
Conclusion: The framework bridges a critical gap between academic research and practical industrial deployment of LLM-aided Hardware Design (LAD) by effectively handling lengthy, complex circuit specifications.
...
TL;DR: BEAVER is the first practical framework for computing deterministic, sound probability bounds on LLM constraint satisfaction, offering provable guarantees instead of sampling-based estimates.


<details>
  <summary>Details</summary>
Motivation: As LLMs move to production, practitioners need reliable methods to verify that model outputs satisfy required constraints. Sampling-based estimates provide intuition but no sound guarantees, creating a gap for deterministic verification methods.
...
TL;DR: The paper introduces "akrasia" (weakness of will) as a framework for analyzing inconsistency in AI agents, proposing a benchmark to measure when models know the right answer but fail to act on it.


<details>
  <summary>Details</summary>
Motivation: LLMs show inconsistency between knowing correct answers and acting on them, similar to human akrasia. The paper aims to provide a conceptual framework and empirical tools to analyze this tension in agentic AI systems.

Method: Introduces the Akrasia Benchmark with structured prompting conditions (Baseline, Synonym, Temporal, and Temptation) to measure when models' local responses contradict prior commitments. Enables quantitative comparison across models, decoding strategies, and temptation types.

Result: Proposes a preliminary benchmark for measuring "self-control" in AI systems and outlines how micro-level akrasia may compound into macro-level instability in multi-agent systems, potentially manifesting as "scheming" or deliberate misalignment.

Conclusion: Reframing inconsistency as weakness of will connects agentic AI behavior to classical theories of agency, creating an empirical bridge between philosophy, psychology, and the science of agentic AI.
...
TL;DR: MIND framework enhances MLLMs with human-like cognitive reasoning through rationale augmentation, progressive correction learning, and contrastive alignment, achieving SOTA performance across multiple reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs have limitations in multi-rationale semantic modeling, logical robustness, and are vulnerable to misleading interpretations in complex scenarios, requiring a paradigm shift from passive imitation to active discriminative reasoning.

Method: Proposes MIND framework with three components: 1) RAD paradigm for automatic dataset expansion with diverse rationales, 2) P2CL strategy with two-phase learning (multi-rationale positive learning followed by active logic discrimination/correction), and 3) MCA optimization for semantic aggregation of correct reasoning and boundary separation of incorrect reasoning.

Result: Achieves state-of-the-art performance on multiple public datasets covering scientific, commonsense, and mathematical reasoning scenarios, demonstrating superior reasoning capabilities.

Conclusion: MIND provides a new perspective for advancing MLLMs toward higher cognitive intelligence by enabling human-like "Understand -> Rethink -> Correct" reasoning abilities through active discriminative reasoning rather than passive imitation.
...
TL;DR: Proposes Executor-Analyst Framework to solve Context Utilization Failure in clinical AI agents, using modular architecture with specialized TxAgents and long-context models, achieving SOTA without expensive finetuning.


<details>
  <summary>Details</summary>
Motivation: Current clinical agents built on small LLMs suffer from Context Utilization Failure - they successfully retrieve biomedical evidence but fail to ground their diagnosis in that information, leading to reasoning deficits in monolithic models.

Method: Executor-Analyst Framework: modular architecture decoupling syntactic precision of tool execution (specialized TxAgents as Executors) from semantic robustness of clinical reasoning (long-context foundation models as Analysts). Uses Stratified Ensemble strategy instead of global pooling to preserve evidentiary diversity and address information bottleneck.

Result: Achieves state-of-the-art performance on CURE-Bench without expensive end-to-end finetuning. Stress tests reveal: (1) Context-Performance Paradox - extending reasoning contexts beyond 12k tokens introduces noise degrading accuracy; (2) Curse of Dimensionality in action spaces - expanding toolsets requires hierarchical retrieval strategies.

Conclusion: Demonstrates potential of training-free architectural engineering for scalable, agile foundation for next-generation trustworthy AI-driven therapeutics. Modular approach effectively addresses reasoning deficits in clinical agents while avoiding expensive finetuning.
...
TL;DR: LLMs show promise for ontology axiom identification but performance varies by axiom type and ontology domain, with Axiom-by-Axiom prompting outperforming direct approaches.


<details>
  <summary>Details</summary>
Motivation: Ontology development is complex and requires significant expertise. While NLP and LLMs have advanced ontology learning, there's a need to systematically evaluate LLMs' ability to identify fundamental ontology axioms (logical relations between classes and properties).

Method: Created OntoAxiom benchmark with 9 ontologies (17,118 triples, 2,771 axioms). Tested 12 LLMs on subclass, disjoint, subproperty, domain, and range axioms. Compared three shot settings and two prompting strategies: Direct (all axioms at once) vs Axiom-by-Axiom (one axiom per prompt).

Result: Axiom-by-Axiom prompting achieved higher F1 scores than Direct approach. Performance varied significantly across axiom types and ontologies (FOAF: 0.642 F1 for subclass, music ontology: 0.218). Larger LLMs outperformed smaller ones, but smaller models remained viable for resource-constrained settings.

Conclusion: LLMs cannot fully automate axiom identification yet, but they provide valuable candidate axioms to support ontology engineers in development and refinement tasks.
...
TL;DR: DeepDist introduces a novel SLS solver with distinct clause weighting schemes for PMS and WPMS, plus decimation method, outperforming state-of-the-art solvers.


<details>
  <summary>Details</summary>
Motivation: Existing SLS algorithms for PMS/WPMS use uniform update strategies that fail to distinguish between PMS and WPMS problems, overlooking their structural differences and limiting performance.

Method: 1) Novel clause weighting scheme with distinct update conditions for PMS vs WPMS; 2) New initialization method tailored to each problem type; 3) Decimation method prioritizing unit and hard clauses; 4) Integration into DeepDist SLS solver.

Result: DeepDist outperforms state-of-the-art SLS solvers on MaxSAT Evaluation benchmarks. Hybrid solver (DeepDist + TT-Open-WBO-Inc) surpasses MaxSAT Evaluation 2024 winners SPB-MaxSAT-c-Band and SPB-MaxSAT-c-FPS.

Conclusion: The proposed distinct treatment of PMS and WPMS with specialized clause weighting schemes and complementary decimation method significantly improves SLS solver performance for (W)PMS problems.
...
TL;DR: KANFormer: A novel deep learning model combining dilated causal CNN, Transformer encoder, and Kolmogorov-Arnold Networks to predict limit order time-to-fill using both market and agent-level information, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing models for predicting limit order time-to-fill rely only on limit order book snapshots, missing important agent-level information and order queue position that could improve prediction accuracy and capture execution likelihood patterns.

Method: KANFormer combines a Dilated Causal Convolutional network with a Transformer encoder, enhanced by Kolmogorov-Arnold Networks (KANs) for better nonlinear approximation. It integrates both market-level LOB data and agent-level information including order queue position.

Result: KANFormer outperforms existing models on CAC 40 index futures data in both calibration (Right-Censored Log-Likelihood, Integrated Brier Score) and discrimination (C-index, time-dependent AUC) metrics. SHAP analysis provides interpretable feature importance over time.

Conclusion: Combining rich market signals with expressive neural architectures (CNN+Transformer+KAN) enables accurate and interpretable predictions of fill probabilities, demonstrating the value of integrating agent-level information beyond traditional LOB snapshots.
...
TL;DR: FARDA is a deep reinforcement learning framework for fast radar deployment that achieves comparable coverage to evolutionary algorithms while being ~7,000x faster.
...
Result: Achieves coverage comparable to evolutionary algorithms while deploying radars approximately 7,000 times faster; ablation studies confirm necessity of all components.

Conclusion: FARDA provides an efficient neural network-based solution for fast anti-jamming radar deployment that overcomes limitations of traditional evolutionary approaches.
...
TL;DR: ERO framework uses evolutionary optimization to enhance LLMs' reasoning abilities, showing that weak models can develop strong reasoning through evolution while even advanced LLMs still have limited system 2 reasoning.


<details>
  <summary>Details</summary>
Motivation: Current LLMs show substantial specific skills but fall short in general intelligence and system 2 reasoning (slow thinking). The paper aims to explore whether machine intelligence can evolve reasoning abilities similar to human beings.

Method: Proposes Evolutionary Reasoning Optimization (ERO) framework that initializes multiple LLMs as a population and uses evolutionary strategies to maximize reasoning scores through survival of the fittest selection.

Result: Two key findings: 1) Even latest LLMs like GPT-5 show limited system 2 reasoning ability; 2) Simple evolution-loop with ERO can enhance relatively weak models (Qwen-7B) to emerge powerful reasoning abilities.

Conclusion: Evolutionary optimization can effectively enhance reasoning abilities in LLMs, demonstrating a promising path toward developing more human-like general intelligence in machines.
...
TL;DR: The paper argues LLMs are not dead ends for AGI - they provide necessary pattern repositories (System-1), and the missing piece is a System-2 coordination layer for reasoning and planning.


<details>
  <summary>Details</summary>
Motivation: To counter influential critiques that claim LLMs are fundamentally incapable of reasoning or planning, and to show that the real bottleneck is not the pattern-matching capability itself but the lack of a coordination mechanism to guide and constrain these patterns.

Method: Proposes UCCT (theory of semantic anchoring) that models reasoning as a phase transition governed by effective support, representational mismatch, and adaptive anchoring budget. Implements this with MACI architecture featuring baiting (behavior-modulated debate), filtering (Socratic judging), and persistence (transactional memory).

Result: Theoretical framework shows how "reasoning" emerges when anchors shift posterior distributions toward goal-directed constraints, distinguishing it from ungrounded generation which is just maximum likelihood retrieval. Provides architecture to implement this coordination layer.

Conclusion: LLMs are essential for AGI as the System-1 substrate, and the path forward requires adding System-2 coordination mechanisms rather than abandoning LLMs. Common objections can be reframed as testable coordination failures within this framework.
...
TL;DR: MOA (Multimodal Oncology Agent) combines histology analysis via TITAN foundation model with clinical/genomic reasoning to predict IDH1 mutations in low-grade gliomas, achieving superior performance over baselines.


<details>
  <summary>Details</summary>
Motivation: IDH1 mutations in low-grade gliomas define clinically distinct subgroups with important prognostic and therapeutic implications, necessitating accurate prediction methods.

Method: Developed a Multimodal Oncology Agent integrating: 1) TITAN foundation model for histology-based IDH1 prediction, 2) Reasoning over structured clinical/genomic data using PubMed, Google Search, and OncoKB, 3) Fusion of histology features with clinical data.

Result: MOA without histology outperformed clinical baseline (F1: 0.826 vs 0.798). MOA with histology fusion achieved best performance (F1: 0.912), exceeding histology baseline (0.894) and fused histology-clinical baseline (0.897). Evaluated on 488 TCGA-LGG patients.

Conclusion: MOA captures complementary mutation-relevant information enriched through external biomedical sources, enabling accurate IDH1 mutation prediction in low-grade gliomas.
...
TL;DR: LLM-based pipeline generates personalized psychological networks from therapy transcripts for treatment planning, outperforming direct prompting with 90% expert preference.


<details>
  <summary>Details</summary>
Motivation: Network-based treatment personalization requires intensive longitudinal data, which is not always feasible. LLMs offer a scalable solution to generate personalized networks from therapy transcripts for case conceptualization.

Method: End-to-end pipeline using in-context learning to identify psychological processes from 77 therapy transcripts, then grouping processes into clinically meaningful clusters and generating explanation-augmented relationships between clusters.

Result: High performance with few training examples; experts preferred multi-step approach (90%) over direct prompting; networks rated 72-75% for clinical relevance, novelty, and usefulness.

Conclusion: Proof of concept for using LLMs to create clinically relevant networks from therapy transcripts, enabling bottom-up case conceptualization and latent theme identification. Future research should test treatment outcome improvements.
...
TL;DR: GPT-5-based Paper Correctness Checker finds increasing objective mistakes in AI papers over time, with 83.2% precision in error detection and 75.8% success in proposing fixes.


<details>
  <summary>Details</summary>
Motivation: Published AI papers form the foundation of research, but errors can propagate unnoticed, creating confusion and complicating reproducibility. The accelerating pace of research and demands on peer-review make mistakes harder to detect.

Method: Developed a Paper Correctness Checker based on GPT-5 to systematically identify objective mistakes (formulas, derivations, calculations, figures, tables) in papers from top AI conferences/journals. Human experts reviewed AI-identified mistakes to validate findings.

Result: Published papers contain non-negligible objective mistakes, with average mistakes per paper increasing over time: NeurIPS (3.8 to 5.9, 55.3% increase), ICLR (4.1 to 5.2), TMLR (5.0 to 5.5). Human experts confirmed 263 of 316 AI-identified mistakes (83.2% precision). AI Checker can propose correct fixes for 75.8% of identified mistakes.

Conclusion: Frontier LLMs like GPT-5 can effectively detect and correct objective mistakes in published papers, helping establish a firmer knowledge foundation and improving reproducibility in AI research.
...
TL;DR: PRiSM is a synthetic, dynamic multimodal benchmark for evaluating scientific reasoning in VLMs using Python code, featuring 24,750+ physics/math problems with automated ground truth verification.


<details>
  <summary>Details</summary>
Motivation: Current VLM benchmarks fail to address scientific reasoning requirements like conceptual understanding, symbolic reasoning, and formal law adherence. Existing datasets are static, lack intermediate reasoning steps, robustness to variations, and mechanisms for verifying scientific correctness.

Method: Introduces PRiSM benchmark with scalable agent-based pipeline (PrismAgent) to generate well-structured problem instances. Each problem includes dynamic textual/visual input, generated figures, executable Python code for ground truth generation/verification, and detailed step-by-step reasoning.

Result: Created benchmark with over 24,750 university-level physics and math problems. Enables fine-grained experimental auditing of multimodal VLMs, revealing failure modes, uncertainty behaviors, and scientific reasoning limitations through five targeted evaluation tasks.

Conclusion: PRiSM provides deeper insights into VLM scientific reasoning capabilities, highlighting their limitations and enabling comprehensive evaluation through dynamic, Python-powered verification mechanisms.
...
TL;DR: TRACE is a framework for evaluating reasoning trajectories in vision-language models using Auxiliary Reasoning Sets to diagnose intermediate step failures that standard final-answer evaluation misses.


<details>
  <summary>Details</summary>
Motivation: Current evaluation of large vision-language models for mathematical and scientific reasoning only looks at final answers, masking reasoning errors and allowing silent failures to persist. There's a need for more transparent evaluation of the reasoning process itself.

Method: TRACE uses Auxiliary Reasoning Sets - compact sub question-answer pairs that decompose complex problems. It evaluates intermediate steps through consistency-based metrics to expose failures overlooked by standard evaluation.

Result: Experiments show that consistency across ARS correlates with final-answer correctness and helps pinpoint specific reasoning steps where failures arise. TRACE defines confidence regions that distinguish reliable from unreliable reasoning paths.

Conclusion: TRACE provides actionable signals for model improvement by enabling transparent reasoning evaluation, supporting effective filtering, debugging, and model refinement through consistency-based analysis of reasoning trajectories.
...
TL;DR: VQR-DQN combines quantum circuits with Rainbow DQN to solve human resource allocation problems, achieving significant performance improvements over classical methods.


<details>
  <summary>Details</summary>
Motivation: Resource allocation problems are NP-hard and classical DRL methods like Rainbow DQN have limited representational power due to classical function approximators. Quantum computing offers potential advantages through superposition and entanglement.

Method: Introduces Variational Quantum Rainbow DQN (VQR-DQN) that integrates ring-topology variational quantum circuits with Rainbow DQN. Frames human resource allocation as a Markov decision process with combinatorial action spaces based on officer capabilities, event schedules, and transition times.

Result: On four HRAP benchmarks, VQR-DQN achieves 26.8% normalized makespan reduction versus random baselines and outperforms Double DQN by 13.4% and classical Rainbow DQN by 4.9%.

Conclusion: The performance gains demonstrate the potential of quantum-enhanced DRL for large-scale resource allocation, with theoretical connections between circuit expressibility, entanglement, and policy quality. Implementation is publicly available.
...
TL;DR: SymPyBench: A large-scale synthetic physics benchmark with 15,045 parameterized problems, featuring multiple question formats and novel evaluation metrics for testing scientific reasoning in LLMs.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive benchmark for evaluating scientific reasoning capabilities of language models, particularly in physics, with fully parameterized problems that enable testing on effectively infinite variations and provide structured reasoning traces.

Method: Developed a synthetic benchmark of 15,045 university-level physics problems (90/10 train/test split) with three question types: MC-Symbolic, MC-Numerical, and free-form. Each problem is fully parameterized with executable Python code generating ground-truth solutions. Introduced novel evaluation metrics: Consistency Score, Failure Rate, and Confusion Rate.

Result: Experiments with state-of-the-art instruction-tuned language models revealed both strengths and limitations in scientific reasoning, demonstrating the benchmark's utility for assessing model capabilities.

Conclusion: SymPyBench serves as a foundation for developing more robust and interpretable reasoning systems by providing a dynamic, code-driven benchmark with novel evaluation metrics that quantify variability and uncertainty across problem variants.
...
### [180] [Two-Stage Camera Calibration Method for Multi-Camera Systems Using Scene Geometry](https://arxiv.org/abs/2512.05171)
*Aleksandr Abramov*

Main category: eess.IV

TL;DR: A novel two-stage calibration method for multi-camera systems that works without floor plans, physical access, or synchronized video by using operator annotations of natural geometric primitives and interactive EFOV polygon manipulation.


<details>
  <summary>Details</summary>
Motivation: Traditional multi-camera calibration methods fail in real-world conditions where accurate floor plans are unavailable, physical access for placing calibration patterns is restricted, or synchronized video streams are not available, limiting deployment of precise tracking systems.

Method: Two-stage approach: 1) Partial calibration using operator annotations of natural geometric primitives (parallel/perpendicular/vertical lines, equal-length segments) to estimate roll, pitch, focal length and project EFOV polygons; 2) Interactive manipulation of EFOV polygons to align with floor plan or virtual calibration elements to determine camera position and yaw.

Result: Method enables calibration using only static images from each camera, implemented as practical web service, with comparative analysis and demonstration videos confirming applicability, accuracy, and flexibility for previously infeasible scenarios.

Conclusion: The proposed method overcomes traditional limitations and enables deployment of precise multi-camera tracking systems in challenging real-world conditions without requiring floor plans, physical access, or synchronized video.
...
### [181] [CATNUS: Coordinate-Aware Thalamic Nuclei Segmentation Using T1-Weighted MRI](https://arxiv.org/abs/2512.05329)
...
TL;DR: CATNUS is a deep learning method for accurate thalamic nuclei segmentation using coordinate-aware 3D U-Net, showing improved performance over existing methods and strong generalization across diverse MRI acquisitions.


<details>
  <summary>Details</summary>
Motivation: Thalamic nuclei segmentation is crucial for understanding brain function and neurological disorders, but remains challenging due to small nucleus sizes, limited contrast/resolution, and anatomical variability between subjects.

Method: CATNUS uses a 3D U-Net architecture enhanced with coordinate convolution layers for better localization, with pre-trained models for T1 maps, MPRAGE, and FGATIR sequences to support broad clinical use.

Result: CATNUS outperforms FreeSurfer, THOMAS, and HIPS-THOMAS in segmentation accuracy and test-retest reliability, and demonstrates strong generalization across multiple scanners, field strengths, and vendors.

Conclusion: CATNUS provides an accurate, generalizable solution for thalamic nuclei segmentation with potential to facilitate large-scale neuroimaging studies and support clinical assessment.
...
### [182] [Image Semantic Communication with Quadtree Partition-based Coding](https://arxiv.org/abs/2512.05395)
*Yinhuan Huang,Zhijin Qin*

Main category: eess.IV

TL;DR: Quad-DeepSC: A novel image semantic communication system using quadtree partition-based joint semantic-channel coding that outperforms traditional communication systems while maintaining low complexity.


<details>
  <summary>Details</summary>
Motivation: Existing image DeepSC methods struggle to balance rate-distortion performance and computational complexity, often performing worse than traditional schemes, especially on high-resolution datasets.

Method: Proposes Quad-DeepSC with quadtree partition-based entropy estimation and feature coding modules combined with lightweight feature extraction/reconstruction networks. Uses a two-stage training: first trains Quad-LIC (learned image codec) for source compression, then fine-tunes end-to-end over wireless channels.

Result: Quad-DeepSC is the first DeepSC system to surpass conventional systems (VTM source coding + 3GPP optimal MCS) across datasets of varying resolutions. Both Quad-DeepSC and Quad-LIC exhibit minimal latency suitable for real-time deployment.

Conclusion: The proposed Quad-DeepSC successfully addresses limitations of existing image DeepSC methods by achieving state-of-the-art performance with low complexity, making it practical for real-time wireless communication systems.
...
### [183] [General and Domain-Specific Zero-shot Detection of Generated Images via Conditional Likelihood](https://arxiv.org/abs/2512.05590)
*Roy Betser,Omer Hofman,Roman Vainshtein,Guy Gilboa*

Main category: eess.IV

TL;DR: CLIDE is a zero-shot AI-generated image detection method using conditional likelihood approximation that adapts to diverse image domains without requiring domain-specific training data.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot detection methods struggle with domain adaptation (e.g., artistic images), limiting real-world applicability. Maintaining updated datasets for supervised/few-shot methods is time-consuming.

Method: CLIDE uses conditional likelihood approximation, computing likelihoods conditioned on real images to enable adaptation across diverse image domains without domain-specific training.

Result: State-of-the-art performance on large-scale general datasets and significantly outperforms existing methods in domain-specific cases like artistic images.

Conclusion: CLIDE demonstrates robustness and highlights the need for broad, domain-aware generalization in AI-generated image detection tasks.
...
### [184] [Low-Complexity OFDM Deep Neural Receivers](https://arxiv.org/abs/2512.05249)
*Ankit Gupta,Onur Dizdar,Yun Chen,Fehmi Emre Kadan,Ata Sattarzadeh,Stephen Wang*

Main category: cs.IT

TL;DR: Proposed a new ResNet block design for OFDM neural receivers that reduces computational complexity (FLOPs) and improves training convergence while maintaining/improving decoding accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing neural receivers for OFDM signals ignore training convergence time and computational complexity (FLOPs), which increase significantly with performance improvements.

Method: Designed new ResNet block with small kernel sizes and dilation rates to reduce FLOPs, uniform channel sizes to reduce memory access cost, novel channel split and shuffle blocks, removed element-wise additions, and used GELU activations.

Result: Proposed NeuralRx reduces number of FLOPs (NFLOPs), improves training convergence, and improves decoding accuracy based on extensive simulations.

Conclusion: The proposed ResNet block design successfully addresses computational complexity and training efficiency challenges in neural OFDM receivers while enhancing decoding performance.
...
### [185] [Uncertainty-Aware Data-Efficient AI: An Information-Theoretic Perspective](https://arxiv.org/abs/2512.05267)
*Osvaldo Simeone,Yaniv Romano*

Main category: cs.IT

TL;DR: Review paper examines formal methods for AI in data-limited regimes, focusing on epistemic uncertainty quantification and synthetic data augmentation through information-theoretic perspectives.


<details>
  <summary>Details</summary>
Motivation: AI systems in robotics, telecommunications, and healthcare often face limited training data, leading to epistemic uncertainty that fundamentally limits predictive performance.

Method: Examines two complementary approaches: 1) quantifying epistemic uncertainty via generalized Bayesian learning, information-theoretic bounds, and finite-sample methods like conformal prediction; 2) mitigating data scarcity through synthetic data augmentation and combining limited labeled data with model predictions.

Result: Provides comprehensive review of formal methodologies for data-limited regimes, highlighting information-theoretic frameworks that quantify the relationship between training data quantity and predictive uncertainty.

Conclusion: Information-theoretic perspectives are crucial for addressing data scarcity in AI applications, with generalized Bayesian learning and synthetic data augmentation offering complementary solutions to epistemic uncertainty challenges.
...
### [186] [Foundations of information theory for coding theory](https://arxiv.org/abs/2512.05316)
*El Mahdi Mouloua,Essaid Mohamed*

Main category: cs.IT

TL;DR: Lecture note introducing information theory with focus on algebraic coding theory, covering Shannon's foundations, entropy, channel capacity, and noisy channel coding theorem.


<details>
  <summary>Details</summary>
Motivation: To bridge probabilistic information theory with structural/algebraic techniques in modern coding theory, providing mathematical foundations for quantifying uncertainty and information transmission.

Method: Develops mathematical foundations based on Shannon's formulation, uses examples like binary symmetric channel to illustrate entropy, conditional entropy, mutual information, and noisy channel models. Describes maximum likelihood decoding principles and Shannon's coding theorem.

Result: Provides comprehensive introduction to information theory concepts including entropy, channel capacity, noisy channel models, and theoretical limits of reliable communication over noisy channels.

Conclusion: The lecture note serves as a helpful resource for students and researchers seeking to understand the connection between probabilistic information theory frameworks and algebraic coding techniques.
...
### [187] [GNSS Jammer Direction Finding in Dynamic Scenarios Using an Inertial-based Multi-Antenna System](https://arxiv.org/abs/2512.05128)
*Lucas Heublein,Thorsten Nowak,Tobias Feigl,Jaspar Pahl,Felix Ott*

Main category: eess.SP

TL;DR: The paper proposes a synthetic aperture system using a 2x2 patch antenna with IMU to enhance GNSS jammer localization by fusing IQ data, spectrograms, and AoA features, overcoming multipath limitations.


<details>
  <summary>Details</summary>
Motivation: GNSS jamming devices disrupt satellite signals and threaten positioning reliability, requiring effective detection and localization for situational awareness and countermeasures.

Method: Uses a 2x2 patch antenna system (USRP X440) with IMU for motion prediction, creates synthetic aperture for better resolution, and fuses IQ samples, FFT spectrograms, 22 AoA features, and relative movement data for enhanced direction finding.

Result: The proposed approach enables prediction of jammer angle, elevation, and distance, with superior angular resolution compared to classical AoA methods that suffer in multipath environments.

Conclusion: The synthetic aperture system with fused data sources provides enhanced GNSS jammer direction finding capabilities, overcoming multipath limitations for more reliable localization.
...
### [188] [Elevation- and Tilt-Aware Shadow Fading Correlation Modeling for UAV Communications](https://arxiv.org/abs/2512.05332)
*Mushfiqur Rahman,Ismail Guvenc,Mihail Sichitiu,Jason A. Abrahamson,Bryton J. Petersen,Amitabh Mishra,Arupjyoti Bhuyan*

Main category: eess.SP

TL;DR: Proposes a new shadow fading correlation model for UAV communications that accounts for 3D orientation and elevation angles, showing significant improvements over traditional distance-only models.


<details>
  <summary>Details</summary>
Motivation: Future wireless networks need better channel understanding for efficient communication with reduced interference. UAVs are becoming integral to these networks but accurately characterizing shadow fading in UAV communications is challenging because traditional models only consider spatial distance and ignore 3D orientation and elevation angles, even though small pitch variations (5-10°) significantly affect signal strength.

Method: Investigates impact of UAV pitch and elevation geometry on shadow fading and proposes an elevation- and tilt-aware spatial correlation model. Uses real-world fixed-altitude UAV measurement dataset collected in rural environment at 3.32 GHz with 125 kHz bandwidth. Integrates proposed correlation model into ordinary Kriging framework for signal strength prediction.

Result: 10-degree tilt-angle separation reduces shadow fading correlation by up to 15%, and 20-degree elevation-angle separation reduces it by up to 40%. Integrating the proposed model into ordinary Kriging yields approximately 1.5 dB improvement in median RMSE compared to traditional correlation models that ignore UAV orientation and elevation.

Conclusion: UAV orientation and elevation significantly impact shadow fading correlation in wireless communications. The proposed elevation- and tilt-aware spatial correlation model provides more accurate signal strength prediction than traditional distance-only models, offering better performance for future UAV-integrated wireless networks.
...
### [189] [Noise Suppression for Time Difference of Arrival: Performance Evaluation of a Generalized Cross-Correlation Method Using Mean Signal and Inverse Filter](https://arxiv.org/abs/2512.05355)
*Hirotaka Obo,Yuki Fujita,Masahisa Ishii,Hideki Moriyama,Ryota Tsuchiya,Yuta Ohashi,Kotaro Seki*

Main category: eess.SP

TL;DR: Proposes GCC-MSIF, a novel generalized cross-correlation method that improves TDOA estimation accuracy in noisy environments by using mean signal estimation and inverse filtering to suppress out-of-band noise.


<details>
  <summary>Details</summary>
Motivation: Conventional GCC methods suffer performance degradation under low SNR conditions, especially when signal bandwidth is unknown, limiting robust passive localization in practical blind environments.

Method: GCC-MSIF introduces a "mean signal" estimated from multi-channel inputs and an "inverse filter" to virtually reconstruct the source signal, enabling adaptive suppression of out-of-band noise.

Result: GCC-MSIF significantly outperforms conventional methods (GCC-PHAT, GCC-SCOT) in low SNR regions, achieves robustness comparable to or exceeding GCC-ML, and shows scalable accuracy improvement with more array elements.

Conclusion: GCC-MSIF is a promising solution for robust passive localization in practical blind environments due to its superior performance in noisy conditions and scalability with array size.
...
### [190] [Hardware-Impaired Over-the-Air Computation with Fluid Antenna Array](https://arxiv.org/abs/2512.05368)
*Zilong Li,Jianxin Dai,Zhaohui Yang,Zhaoyang Zhang,Kai-Kit Wong,Zhiyang Li,Shunkuan Cheng*

Main category: eess.SP

TL;DR: FA array-enhanced AirComp system with HWIs jointly optimizes transmit power, receive beamforming, and antenna positions to minimize MSE, outperforming fixed antenna arrays.


<details>
  <summary>Details</summary>
Motivation: To exploit the new degrees of freedom offered by reconfigurable antenna positioning in fluid antenna arrays to enhance AirComp performance in the presence of hardware impairments, overcoming limitations of conventional fixed-position antenna arrays.

Method: Joint optimization of transmit power control, receive beamforming, and antenna position vector using block coordinate descent framework, decomposing the non-convex problem into three manageable subproblems with closed-form solutions or efficient numerical algorithms.

Result: Simulation results show the proposed joint design significantly reduces MSE compared to conventional fixed-position antenna arrays and exhibits enhanced robustness against hardware impairments, with algorithm effectiveness and convergence validated under various configurations.

Conclusion: Fluid antenna arrays with joint transceiver and antenna position optimization provide superior AirComp performance and hardware impairment robustness compared to traditional fixed antenna systems.
...
### [191] [Exploiting Spatial Multiplexing Based on Pixel Antennas: An Antenna Coding Approach](https://arxiv.org/abs/2512.05706)
*Zixiang Han,Shanpu Shen,Ross Murch*

Main category: eess.SP

TL;DR: Antenna coding technique using pixel antennas to enhance MIMO spectral efficiency by exploiting beamspace multiplexing, achieving up to 12 bits/s/Hz improvement or 90% power reduction.


<details>
  <summary>Details</summary>
Motivation: To exploit spatial multiplexing capability of pixel antennas by leveraging additional degrees of freedom in beamspace domain to transmit more information streams, addressing spectral efficiency limitations in conventional MIMO systems.

Method: Proposes antenna coding approach using pixel antennas with reconfigurable radiation patterns via RF switches. Introduces antenna coder and pattern coder, provides MIMO system model with antenna coding in beamspace domain, derives spectral efficiency, analyzes radiation patterns, and develops efficient optimization algorithms for antenna coding design.

Result: Numerical simulations show the technique enhances spectral efficiency of 4×4 MIMO by up to 12 bits/s/Hz or reduces required transmit power by up to 90% compared to conventional MIMO, demonstrating significant performance improvements.

Conclusion: Antenna coding technique using pixel antennas effectively enhances spectral efficiency and shows promise for future 6G wireless communication systems by exploiting beamspace spatial multiplexing capabilities.
...
### [192] [Individual Channel Estimation for Beyond Diagonal Reconfigurable Intelligent Surfaces](https://arxiv.org/abs/2512.05404)
*Kangchun Zhao,Yijie Mao*

Main category: eess.SP

TL;DR: Proposed individual channel estimation framework for BD-RIS that separately estimates static BS-RIS channel and dynamic RIS-user channels using FD approach and LS estimation, achieving higher accuracy with reduced pilot overhead.


<details>
  <summary>Details</summary>
Motivation: BD-RIS offers greater wave manipulation flexibility than traditional diagonal RIS, but interconnections between RIS elements create new channel estimation challenges that make existing diagonal RIS approaches ineffective and increase pilot overhead significantly.

Method: Individual channel estimation framework that separates BS-RIS channel (static) and RIS-user channels (dynamic). Uses full-duplex approach to estimate BS-RIS channel by leveraging its inherent sparsity, followed by least squares approach for RIS-user channel estimation.

Result: Numerical results show significantly higher channel estimation accuracy, especially with large number of RIS elements, while substantially reducing pilot overhead compared to conventional cascaded channel estimation methods.

Conclusion: The proposed individual channel estimation framework effectively addresses BD-RIS channel estimation challenges, offering improved accuracy and reduced pilot overhead by separately handling static and dynamic channel components.
...
### [193] [A Non-Invasive Path to Animal Welfare: Contactless Vital Signs and Activity Monitoring of In-Vivo Rodents Using a mm-Wave FMCW Radar](https://arxiv.org/abs/2512.05595)
*Tommaso Polonelli,Manuel Glahn,Stefano Kron,Stefan Selbert,Marco Garzola,Michele Magno*

Main category: eess.SP

TL;DR: Contactless 60 GHz FMCW radar system for non-invasive monitoring of rodent activity and vital signs in home-cage environments, achieving 3 cm motion sensitivity and 2 bpm respiration accuracy.


<details>
  <summary>Details</summary>
Motivation: Conventional rodent monitoring techniques are invasive, cause stress, and compromise data fidelity. There's a need for contactless, non-invasive methods that improve animal welfare and measurement reliability.

Method: Uses low-power 60 GHz FMCW radar (Infineon BGT60 series) with optimized sensor placement and signal processing pipeline including range bin selection, phase extraction, and frequency-domain estimation tailored to rodent vital signs.

Result: Achieves 3 cm motion sensitivity, 0.1 m/s activity detection, 2 um distance resolution for cardiopulmonary micro-movements, and 2 bpm accuracy for respiration rate extraction in realistic in-vivo cage experiments.

Conclusion: The system provides a refined alternative to traditional monitoring, improving animal welfare and data reliability. It's the first demonstration of continuous radar-based vital sign monitoring in freely moving, group-housed rodents, laying foundation for scalable automated ethical monitoring.
...
### [194] [Over-the-Air Semantic Alignment with Stacked Intelligent Metasurfaces](https://arxiv.org/abs/2512.05657)
*Mario Edoardo Pandolfo,Kyriakos Stylianopoulos,George C. Alexandropoulos,Paolo Di Lorenzo*

Main category: eess.SP

TL;DR: SIM-based over-the-air semantic alignment framework enables latent-space alignment directly in wave domain, reducing device complexity while maintaining task accuracy.


<details>
  <summary>Details</summary>
Motivation: Semantic communication systems suffer performance degradation when heterogeneous transmitter-receiver models produce misaligned latent representations. Existing alignment methods increase device complexity through additional digital processing.

Method: Introduces stacked intelligent metasurfaces (SIM) as trainable linear operators that emulate supervised linear aligners and zero-shot Parseval-frame-based equalizers. Develops gradient-based optimization to tailor metasurface transfer function to desired semantic mapping.

Result: SIMs accurately reproduce both supervised and zero-shot semantic equalizers, achieving up to 90% task accuracy in high SNR regimes while maintaining strong robustness at low SNR values with heterogeneous ViT encoders.

Conclusion: SIM-based over-the-air semantic alignment enables efficient latent-space alignment directly in wave domain, substantially reducing computational burden at device level while maintaining performance across different SNR conditions.
...
### [195] [Codebook-based Port Selection and Combining for CSI-Free Uplink Fluid Antenna Multiple Access](https://arxiv.org/abs/2512.05748)
*Chenguang Rao,Kai-Kit Wong,Sai Xu,Xusheng Zhu,Yangyang Zhang,Chan-Byoung Chae*

Main category: eess.SP

TL;DR: Proposes codebook-based port selection and combining (CPSC) FAMA for uplink without CSI at BS, achieving higher rates than fixed-antenna systems with low complexity.


<details>
  <summary>Details</summary>
Motivation: Most existing FAMA studies focus on downlink with perfect CSI, while uplink counterpart remains unexplored. Need for scalable multiuser connectivity with low implementation complexity for future 6G networks.

Method: BS designs and broadcasts predefined codebook. Each UE uses fluid antenna, acquires local CSI, selects best codeword, activates corresponding ports, and determines combining weights. BS separates signals via codebook-guided projection without global CSI. Three lightweight scheduling strategies handle codeword collisions.

Result: CPSC-FAMA achieves substantially higher rates than fixed-antenna systems while maintaining low complexity. Amortizing optimization cost over UEs reduces BS processing burden and enhances scalability.

Conclusion: Proposed CPSC-FAMA scheme is a strong candidate for future 6G networks, offering scalable uplink multiuser connectivity without requiring global CSI at BS.
...
### [196] [Radar Network Waveform Design for Target Tracking](https://arxiv.org/abs/2512.05757)
*Tao Fan,Augusto Aubry,Antonio De Maio,Luca Pallotta,Xianxiang Yu,Guolong Cui*

Main category: eess.SP

TL;DR: This paper proposes a method for designing slow-time coded waveforms in radar networks to improve single target tracking accuracy under colored Gaussian interference, using PCRLB optimization with similarity constraints and solving via block-MM algorithm.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve target tracking accuracy in radar networks operating under colored Gaussian interference by optimizing waveform design, which is crucial for radar performance but challenging due to transmitter limitations and complex optimization requirements.

Method: The method formulates waveform design as minimizing the trace of Posterior Cramér Rao Lower Bound (PCRLB) with power budget and similarity constraints, then approximates the problem using Taylor-series expansion and solves it with a customized block Majorization-Minimization (block-MM) algorithm.

Result: Numerical results show improved target state estimation accuracy and robust tracking performance under uncertain target state conditions compared to reference codes, with thorough convergence analysis of the proposed algorithm.

Conclusion: The proposed technique successfully addresses the challenging waveform optimization problem for radar target tracking, providing better tracking accuracy than existing methods while maintaining practical constraints and demonstrating robust performance.
...
### [197] [A Residual Variance Matching Recursive Least Squares Filter for Real-time UAV Terrain Following](https://arxiv.org/abs/2512.05918)
*Xiaobo Wu,Youmin Zhang*

Main category: eess.SP

TL;DR: RVM-RLS filter improves UAV terrain following waypoint estimation by 88% over benchmarks for wildfire patrol missions.


<details>
  <summary>Details</summary>
Motivation: Existing real-time filtering algorithms struggle with accurate waypoint estimation under measurement noise in nonlinear, time-varying UAV terrain following systems, risking flight instability and missed wildfire detections during patrol missions.

Method: Proposed Residual Variance Matching Recursive Least Squares (RVM-RLS) filter guided by Residual Variance Matching Estimation (RVME) criterion to adaptively estimate real-time waypoints for nonlinear, time-varying UAV terrain following systems.

Result: RVM-RLS filter improves waypoints estimation accuracy by approximately 88% compared with benchmark algorithms across multiple evaluation metrics in simulated terrain environment experiments.

Conclusion: The RVM-RLS filter demonstrates both methodological advances in real-time filtering and practical potential for UAV-based online wildfire patrol missions by significantly improving terrain following accuracy.
