...
Result: PH can statistically differentiate between artists from different artistic currents and within the same movement, and distinguish authentic artist works from AI-generated images in their style.

Conclusion: Persistent homology provides objective, interpretable insights for art style analysis and AI-generated art detection.
...
<details>
  <summary>Details</summary>
Motivation: Generating animal faces is challenging due to limited training images, especially for facial expressions across individuals. Macaque monkeys are important for neuroscience and evolutionary research.

Method: Used StyleGAN2 with: 1) Data augmentation via motion transfer to animate still images, 2) Sample selection based on latent representation for variation, 3) Loss function refinement for subtle movements like eye movements.

Result: Method successfully generates diverse facial expressions for multiple macaque individuals, outperforming models trained only on original still images. Also effective for style-based image editing with specific style parameters corresponding to distinct facial movements.

Conclusion: Model can disentangle motion components as style parameters, providing valuable tool for macaque facial expression research in neuroscience and evolutionary studies.
...
TL;DR: Proposes PairHuman - first large-scale benchmark dataset for dual-person portrait generation with 100K+ images, and DHumanDiff baseline method for personalized dual-person portraits with enhanced facial consistency.


<details>
  <summary>Details</summary>
Motivation: Personalized dual-person portrait customization has potential applications but lacks benchmark datasets, hindering high-quality generation.

Method: Created PairHuman dataset with 100K+ images and rich metadata; developed DHumanDiff baseline with enhanced facial consistency that balances personalized generation and semantic scene creation.
...
TL;DR: This paper presents an unsupervised autoencoder with CDF 97 wavelet transform for denoising neutron images degraded by mixed Gaussian-Poisson noise, showing superior performance compared to traditional methods like BM3D.


<details>
  <summary>Details</summary>
Motivation: Neutron imaging is crucial for analyzing ICF events but images are often degraded by mixed Gaussian-Poisson noise that conventional methods struggle to remove while preserving image fidelity.

Method: Implemented an unsupervised autoencoder with Cohen-Daubechies-Feauveau (CDF 97) wavelet transform in the latent space specifically designed for mixed Gaussian-Poisson denoising of neutron imaging data.
...
Conclusion: This approach represents a promising advancement in neutron image noise reduction and enables better three-dimensional reconstruction analysis of ICF experiments.
...
TL;DR: SAM 3 is a unified model for detecting, segmenting, and tracking objects in images/videos using concept prompts (noun phrases, image exemplars, or both), achieving double the accuracy of existing systems.


<details>
  <summary>Details</summary>
Motivation: To advance Promptable Concept Segmentation (PCS) by creating a scalable model that can handle diverse concept prompts and provide segmentation masks with unique identities for object instances.

Method: Built a scalable data engine producing 4M unique concept labels dataset, uses image-level detector and memory-based video tracker with shared backbone, decouples recognition and localization with presence head.

Result: Doubles accuracy of existing systems in both image and video PCS, improves previous SAM capabilities on visual segmentation tasks.

Conclusion: SAM 3 represents significant advancement in promptable concept segmentation, with open source release and new SA-Co benchmark for the community.
...
TL;DR: SaFeR-CLIP improves vision-language model safety by redirecting unsafe concepts to semantically closest safe alternatives, recovering up to 8.0% zero-shot accuracy while maintaining robust safety.


<details>
  <summary>Details</summary>
Motivation: Current safety fine-tuning methods for vision-language models like CLIP cause significant drops in generalization performance due to rigid alignment strategies that disrupt learned semantic structure.

Method: Proposed SaFeR-CLIP framework using proximity-aware approach that redirects unsafe concepts to their semantically closest safe alternatives to minimize representational change, applying principle of minimal intervention.

Result: SaFeR-CLIP recovers up to 8.0% in zero-shot accuracy over prior methods while maintaining robust safety. Also introduced NSFW-Caps benchmark with 1,000 highly-aligned pairs for testing safety under distributional shift.
...
TL;DR: A three-stage framework for generating multi-view consistent SVGs from single-view input using 3D lifting, spatial memory for cross-view correspondence, and path optimization.


<details>
  <summary>Details</summary>
Motivation: SVGs are essential for modern design but generating multi-view consistent SVGs from single-view input remains underexplored, limiting applications in asset creation and semantic vector editing.
...
Conclusion: Bridges generative modeling and structured vector representation, providing scalable multi-view SVG generation from single input for asset creation and semantic vector editing.
...
### [8] [Neighbor GRPO: Contrastive ODE Policy Optimization Aligns Flow Models](https://arxiv.org/abs/2511.16955)
*Dailan He,Guanlin Feng,Xingtong Ge,Yazhe Niu,Yi Zhang,Bingqi Ma,Guanglu Song,Yu Liu,Hongsheng Li*

Main category: cs.CV

TL;DR: Neighbor GRPO is a new alignment algorithm that overcomes limitations of SDE-based GRPO methods by generating diverse trajectories through initial noise perturbations and using distance-based optimization, preserving ODE efficiency while achieving better performance.


<details>
  <summary>Details</summary>
Motivation: Current SDE-based GRPO methods for flow matching models suffer from inefficient credit assignment and incompatibility with high-order solvers due to their stochastic sampling paradigm.

Method: Proposes Neighbor GRPO that generates diverse candidate trajectories by perturbing initial noise conditions of ODEs and optimizes using softmax distance-based surrogate leaping policy, with symmetric anchor sampling and group-wise quasi-norm reweighting.
...
Conclusion: Neighbor GRPO provides an effective alternative to SDE-based alignment methods that fully maintains ODE efficiency and compatibility with high-order solvers.
...
TL;DR: Mesh RAG is a training-free framework that enhances autoregressive mesh generation by using retrieval-augmented generation to decouple sequential dependencies, improving quality, speed, and enabling incremental editing.


<details>
  <summary>Details</summary>
Motivation: Current autoregressive mesh generation methods suffer from quality-speed trade-offs due to sequential dependencies and difficulty in incremental editing, requiring larger models or longer sequences for quality improvements.

Method: Leverages point cloud segmentation, spatial transformation, and point cloud registration to retrieve, generate, and integrate mesh components in a retrieval-based approach that enables parallelizable inference.

Result: Significantly enhances mesh quality, accelerates generation speed compared to sequential part prediction, and enables incremental editing across various autoregressive mesh generation models without retraining.

Conclusion: Mesh RAG provides an effective plug-and-play solution that overcomes limitations of sequential dependencies in autoregressive mesh generation, offering improved quality, speed, and editability.
...
### [10] [Parameter-Free Neural Lens Blur Rendering for High-Fidelity Composites](https://arxiv.org/abs/2511.17014)
*Lingyan Ruan,Bin Chen,Taehyun Rhee*

Main category: cs.CV

TL;DR: Proposes a method to estimate camera lens blur (circle of confusion maps) directly from RGB images without needing camera parameters or depth information, enabling realistic blending of 3D virtual objects into real scenes.


<details>
  <summary>Details</summary>
Motivation: Existing methods require camera parameters and scene depth for realistic lens blur, which are often unavailable to ordinary users, limiting accessibility and generalizability of mixed reality compositing.

Method: Directly estimates CoC maps from RGB images, infers CoC values for virtual objects using linear relationship between signed CoC maps and depth, and renders blur using neural reblurring network.
...
Conclusion: Provides flexible and practical solution for real-world mixed reality applications by eliminating dependency on camera metadata and depth information.
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between creative intent and functional virtual spaces by enabling automatic creation of traversable 3D worlds without manual modeling or specialized expertise.
...
Result: Produces geometrically consistent, visually rich worlds that are efficient to render in real time and can be immediately explored in standard game engines.

Conclusion: Advances generative 3D AI for accessible world-building at scale, with applications in gaming, simulation, and immersive social environments.
...
<details>
  <summary>Details</summary>
Motivation: Current EO datasets focus on semantic tasks like captioning/classification but lack benchmarks that connect multimodal perception with measurable biophysical variables, limiting scientific regression capabilities in vision language models.

Method: Created REO-Instruct dataset integrating co-registered Sentinel-2 and ALOS-2 imagery with structured textual annotations through hybrid human-AI pipeline, establishing cognitive logic chains in forest ecological scenarios.

Result: Evaluation shows current VLMs struggle with numeric reasoning in scientific regression tasks, highlighting a key challenge for geospatial AI models.
...
TL;DR: BOP-ASK is a large-scale dataset for object interaction reasoning that addresses limitations in current spatial reasoning benchmarks by providing fine-grained annotations for precise 3D localization, physical compatibility, affordances, and multi-step planning.


<details>
  <summary>Details</summary>
Motivation: Current VLMs perform well on high-level spatial relationships but lack fine-grained understanding needed for real-world applications like precise 3D localization, physical compatibility, affordances, and multi-step spatial planning.

Method: Leveraged 6D object poses from BOP datasets to create a data generation pipeline that derives fine-grained annotations including grasp poses, referred object poses, path planning trajectories, and various spatial relationships. Created over 150k images and 33M QA pairs across six tasks.

Result: Models trained on BOP-ASK outperform baselines and exhibit emergent capabilities including precise object and grasp pose estimation, trajectory planning, and fine-grained object-centric spatial reasoning in cluttered environments.

Conclusion: BOP-ASK provides a comprehensive benchmark for training and evaluating VLMs on fine-grained object interaction reasoning, enabling development of models with improved spatial understanding for real-world applications.
...
TL;DR: Parts-Mamba: A hybrid GCN-Mamba model for skeleton action recognition that handles missing skeleton data by capturing distant joint context through parts-specific scanning and fusion modules.
...
Conclusion: The hybrid GCN-Mamba approach effectively addresses the limitations of traditional GCNs in handling incomplete skeleton data by better capturing and maintaining contextual information from distant joints.
...
TL;DR: Extends Gromov-Wasserstein distance to enable multiple-to-one and multiple-to-multiple object matching through Joint Gromov-Wasserstein (JGW) objective, providing superior computational efficiency and accuracy for complex matching problems.


<details>
  <summary>Details</summary>
Motivation: Traditional GW distance is limited to pairwise matching between single objects, which restricts its utility in scenarios requiring multiple-to-one or multiple-to-multiple object matching across various applications.

Method: Introduces Joint Gromov-Wasserstein (JGW) objective that extends GW framework to enable simultaneous matching between collections of objects, formulated and solved for point cloud representations using adapted Optimal Transport algorithms with entropic regularization.

Result: JGW provides a non-negative dissimilarity measure that identifies partially isomorphic distributions of mm-spaces with point sampling convergence, demonstrating superior performance in accuracy and computational efficiency compared to other GW variants for partial matching.

Conclusion: JGW effectively handles multiple shape matching for geometric shapes and biomolecular complexes, showing promising applications across diverse domains including computer graphics and structural biology for solving complex matching problems.
...
<details>
  <summary>Details</summary>
Motivation: To leverage the inductive bias from aligning generative model representations with pretrained self-supervised encoders for better performance in inverse problems where ground-truth signals are unavailable.

Method: Apply representation alignment between diffusion/flow-based models and pretrained self-supervised visual encoders (e.g., DINOv2) to guide reconstruction at inference time, using approximate target features.

Result: Substantially improved reconstruction fidelity and perceptual realism across multiple inverse problems (super-resolution, inpainting, deblurring), with efficiency gains through reduced discretization steps.

Conclusion: REPA provides a general framework that consistently enhances reconstruction quality in inverse problems while offering theoretical insights into perceptual fidelity improvement and practical efficiency benefits.
...
TL;DR: NFGlassNet uses flash/no-flash image pairs to detect glass surfaces by analyzing reflection dynamics, outperforming existing methods that rely on boundary or reflection cues alone.


<details>
  <summary>Details</summary>
Motivation: Glass surfaces are challenging to detect due to their transparency and lack of distinctive features. Existing methods fail to fully exploit intrinsic glass properties, particularly how illumination differences affect reflections.

Method: Proposes NFGlassNet with Reflection Contrast Mining Module (RCMM) to extract reflections and Reflection Guided Attention Module (RGAM) to fuse reflection and glass surface features. Uses 3.3K flash/no-flash image pairs for training.

Result: Extensive experiments show the method outperforms state-of-the-art glass surface detection methods.

Conclusion: Leveraging reflection dynamics in flash/no-flash imagery provides an effective approach for accurate glass surface detection, with the created dataset and model being made available.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs focus on simple video scenarios, failing to handle the complex and diverse nature of real-world audio-visual events in videos.
...
Result: Generated over 5K untrimmed videos with 27K objects across 100 audio-visual event types, and 8K+ QA pairs. AVST-Zero demonstrates competitive performance compared to existing models.
...
TL;DR: Warm Diffusion bridges hot (noise-only) and cold (blur-only) diffusion paradigms by proposing a unified Blur-Noise Mixture Diffusion Model that jointly controls blurring and noise, exploiting spectral dependencies in images.


<details>
  <summary>Details</summary>
Motivation: Hot diffusion fails to exploit correlations between high-frequency detail and low-frequency structures, while cold diffusion neglects noise's role in shaping the data manifold, causing out-of-manifold issues.

Method: Proposes Warm Diffusion with Blur-Noise Mixture Diffusion Model (BNMD) using divide-and-conquer strategy to disentangle denoising and deblurring processes, analyzed via Blur-to-Noise Ratio (BNR) spectral analysis.

Result: Extensive experiments across benchmarks validate the effectiveness of the approach for image generation.

Conclusion: Warm Diffusion successfully integrates strengths of both hot and cold diffusion paradigms, providing a unified framework that addresses limitations of each approach while maintaining strong generative performance.
...
TL;DR: Q-Real is a dataset for fine-grained evaluation of realism and plausibility in AI-generated images, with 3,088 images annotated with entity locations, judgment questions, and attribution descriptions, plus a benchmark for MLLM evaluation.
...
Result: Experimental results demonstrate the high quality and significance of the dataset and the comprehensiveness of the benchmark. The dataset enables fine-grained evaluation of AI-generated images.

Conclusion: Q-Real provides a comprehensive framework for fine-grained quality assessment of AI-generated images, addressing the limitations of single-score evaluations and enabling targeted improvements in generative models.
...
TL;DR: UniModel is a unified generative model that handles both visual understanding and generation tasks using a single pixel-to-pixel diffusion framework, treating all inputs and outputs as RGB pixels in a shared visual space.


<details>
  <summary>Details</summary>
Motivation: To achieve unification across models, tasks, and representations by eliminating modality discrepancies and creating a fully vision-native multimodal learning framework where text and images coexist in the same visual space.

Method: Uses a Unified Diffusion Transformer with rectified flow in pixel space. Text is rendered as painted text images, and all tasks are cast as pixel-to-pixel transformations. Lightweight task embeddings specify the direction (understanding vs generation).
...
Conclusion: Unifying model, tasks, and representations in a single visual space is a promising paradigm for general-purpose multimodal intelligence, enabling bidirectional mappings between natural images and painted text images.
...
TL;DR: DeltaDeno is a training-free zero-shot anomaly generation method that uses diffusion model contrast and latent inpainting to create realistic defects without requiring real anomaly samples.
...
Method: Contrasts two diffusion branches with minimal prompt pairs under shared schedule, accumulates denoising deltas into localization maps, uses latent inpainting with guided masks, and applies token-level prompt refinement with spatial attention bias.
...
Conclusion: DeltaDeno provides an effective training-free approach for zero-shot anomaly generation that overcomes the limitations of existing methods requiring anomalous samples.
...
Method: Proposes DGAF-VSR with Optical Guided Warping Module (OGWM) to maintain high-frequency details in aligned features and Feature-wise Temporal Condition Module (FTCM) for dense guidance in the feature domain, leveraging feature domain compensation and upscaled resolution warping.
...
Conclusion: Feature domain compensation and upscaled resolution warping effectively address alignment and compensation issues in DM-based VSR, achieving superior performance across multiple quality metrics.
...
TL;DR: Deep learning framework for CBCT tooth segmentation that integrates semantic and shape awareness to handle interdental adhesions and preserve anatomical boundaries.
...
Method: Target-tooth-centroid prompted multi-label learning for semantic relationships between teeth, plus tooth-shape-aware learning for morphological constraints, unified via multi-task learning.

Result: Significantly outperforms existing methods on internal and external datasets.

Conclusion: Effectively mitigates shape distortions and provides anatomically faithful tooth boundaries.
...
TL;DR: OmniGround benchmark addresses STVG limitations with 3,475 videos across 81 categories and complex queries. PG-TAF framework achieves 25.6-35.6% improvements on OmniGround with consistent gains across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Address the gap between current STVG models and real-world demands involving diverse objects and complex queries, overcoming category bias, oversimplified reasoning, and poor linguistic robustness.

Method: Proposed OmniGround benchmark with Forward-Backward-Refinement annotation pipeline and DeepSTG evaluation framework. Introduced PG-TAF, a training-free two-stage framework decomposing STVG into temporal grounding and spatio-temporal propagation.

Result: Performance drops 10.4% on complex real-world scenes, especially with small/occluded objects and intricate spatial relations. PG-TAF achieves 25.6% m_tIoU and 35.6% m_vIoU improvements on OmniGround with consistent gains across four benchmarks.

Conclusion: OmniGround benchmark and PG-TAF framework effectively address STVG limitations, demonstrating significant improvements in handling complex real-world scenarios with diverse objects and queries.
...
TL;DR: MultiPriv is the first benchmark to evaluate individual-level privacy reasoning in VLMs, revealing significant unmeasured risks that current perception-focused benchmarks miss.
...
Method: Proposed MultiPriv benchmark with Privacy Perception and Reasoning (PPR) framework, using a bilingual multimodal dataset with synthetic individual profiles where identifiers are linked to sensitive attributes. Evaluates 9 tasks across the PPR spectrum.

Result: Evaluation of 50+ VLMs shows: (1) significant unmeasured reasoning-based privacy risks, (2) perception metrics are poor predictors of reasoning risks, (3) existing safety alignments are inconsistent and ineffective against reasoning-based attacks.

Conclusion: MultiPriv exposes systemic vulnerabilities in VLMs and provides a framework for developing robust, privacy-preserving models.
...
TL;DR: A novel implicit neural representation framework that jointly models dynamic MRI images and motion fields using coupled INRs with optical flow regularization, enabling simultaneous reconstruction and motion estimation without prior flow estimation.
...
Method: Two coupled implicit neural representations (INRs): one for spatiotemporal image content and another for optical flow, connected via the optical flow equation as physics-inspired regularization, plus data consistency loss with k-space measurements.
...
TL;DR: The paper introduces FingerCap, a task for generating detailed finger-level hand motion descriptions, along with a 40K dataset and FiGOP method that combines RGB frames with hand keypoints to capture fine temporal motion details.
...
Result: Experiments show strong Video-MLLMs struggle with finger-level reasoning, while FiGOP-augmented models achieve consistent gains under both automated (HandJudge) and human evaluation.

Conclusion: FiGOP effectively addresses temporal sparsity in hand motion analysis by recovering fine temporal cues without increasing RGB density, enabling better finger-level motion captioning.
...
TL;DR: Proposes a point-supervised facial expression spotting framework using Gaussian-based intensity modeling and two-branch architecture for macro/micro-expression detection with only timestamp annotations.


<details>
  <summary>Details</summary>
Motivation: Existing facial expression spotting methods require costly temporal boundary annotations, so this work explores point-supervised learning with only single timestamp annotations per instance to reduce annotation burden.

Method: Two-branch framework: 1) Class-agnostic expression intensity branch with Gaussian-based instance-adaptive intensity modeling for soft pseudo-labeling, 2) Class-aware apex classification branch for macro/micro-expression distinction. Includes intensity-aware contrastive loss.

Result: Extensive experiments on SAMM-LV, CAS(ME)^2, and CAS(ME)^3 datasets demonstrate the framework's effectiveness in facial expression spotting with minimal annotation requirements.

Conclusion: The proposed point-supervised framework successfully addresses facial expression spotting with reduced annotation costs through innovative intensity modeling and two-branch architecture.
...
TL;DR: MatPedia is a foundation model for PBR materials that uses a joint RGB-PBR representation to unify multiple material tasks including text-to-material generation, image-to-material generation, and intrinsic decomposition.
...
Method: Proposes a joint RGB-PBR representation encoding materials into two interdependent latents (RGB appearance and PBR maps), formulated as a 5-frame sequence using video diffusion architectures to capture correlations and transfer visual priors from RGB generation models.

Result: Achieves native 1024×1024 synthesis that substantially surpasses existing approaches in both quality and diversity, trained on MatHybrid-410K dataset combining PBR datasets with large-scale RGB images.

Conclusion: MatPedia provides a unified framework for multiple material tasks through its joint representation, enabling high-quality material synthesis while bridging the gap between natural image appearance and physical PBR properties.
...
TL;DR: Proposes a Dual Branch Degradation Extractor Network for blind super-resolution that handles both blur and noise degradations through unsupervised embeddings, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing SISR methods perform well with known fixed degradations but suffer significant performance drops when actual degradation deviates from assumptions, especially in blind SR scenarios.

Method: Uses a dual branch network to extract unsupervised degradation embeddings representing blur and noise information separately, allowing the SR network to adapt differently to each degradation type.

Result: Extensive experiments on benchmarks show the method achieves state-of-the-art performance in blind super-resolution tasks.

Conclusion: The proposed dual branch degradation extractor effectively handles blind SR by explicitly modeling both blur and noise degradations through separate embeddings, serving as a regularizer to leverage differences between SR and HR images.
...
TL;DR: Edge-efficient food image synthesis model that generates realistic cooked food images from raw inputs using recipe and cooking state guidance, with a new culinary similarity metric.
...
Method: Propose an edge-efficient recipe and cooking state guided generator that synthesizes realistic food images conditioned on raw food image, using a domain-specific Culinary Image Similarity (CIS) metric as both training loss and progress-monitoring signal.

Result: Model outperforms existing baselines with significant reductions in FID scores (30% improvement on our dataset; 60% on public datasets).

Conclusion: The proposed approach enables user-preferred visual targets rather than fixed presets while ensuring temporal consistency and culinary plausibility for edge deployment.
...
TL;DR: SeeCLIP improves open-set domain generalization by addressing the dilemma between known-class structural risk and unknown-class open-space risk through semantic enhancement, duplex contrastive learning, and semantic-guided diffusion.
...
Method: Proposes Semantic-enhanced CLIP (SeeCLIP) with semantic-aware prompt enhancement, duplex contrastive learning (repulsion and cohesion), and semantic-guided diffusion to generate pseudo-unknowns for learning finer decision boundaries.
...
Conclusion: SeeCLIP effectively addresses the open-set domain generalization challenge by leveraging semantic enhancement to handle hard unknowns and improve model robustness.
...
TL;DR: Natural selection-inspired pruning framework for 3DGS that uses optimization gradients to autonomously determine which Gaussian primitives to retain or prune, achieving state-of-the-art performance with over 0.6 dB PSNR gain under 15% budgets.


<details>
  <summary>Details</summary>
Motivation: 3DGS uses many Gaussian primitives causing high storage and computational costs, while existing pruning methods rely on manual criteria or extra parameters with suboptimal results.
...
Conclusion: Proposed framework provides fully learnable pruning without human intervention, effectively reducing 3DGS storage and computational overhead while maintaining high rendering quality.
...
TL;DR: The paper introduces the Synergistic Diversity (SQ) framework to improve plant disease detection from leaf images by enhancing ensemble model selection through a novel diversity metric that better captures member synergy.


<details>
  <summary>Details</summary>
Motivation: Plant diseases cause $220 billion in annual economic losses, and timely detection is critical for food security. Current ensemble diversity metrics fail to identify optimal ensemble teams, limiting the effectiveness of deep ensemble approaches.

Method: 1) Comprehensive analysis of limitations in existing ensemble diversity metrics (Q metrics); 2) Development of the novel SQ metric that captures synergy between ensemble members; 3) Extensive validation experiments on plant leaf image dataset.
...
Conclusion: The SQ framework enables more reliable and efficient image-based plant disease detection by providing a better measure for selecting synergistic ensemble members.
...
TL;DR: Proposes RadioKMoE, a knowledge-guided radiomap estimation framework combining Kolmogorov-Arnold Networks (KAN) with Mixture-of-Experts (MoE) to improve accuracy and robustness in wireless signal coverage mapping.


<details>
  <summary>Details</summary>
Motivation: Complex radio propagation behavior and challenging environments make radiomap estimation difficult, requiring better methods to capture spatial signal knowledge for wireless network management.

Method: Uses KAN module to predict initial coarse coverage map leveraging physics approximation, then employs MoE network with specialized expert networks for precise radiomap estimation using environmental information.

Result: Experimental results show enhanced accuracy and robustness in both multi- and single-band radiomap estimation compared to conventional approaches.

Conclusion: RadioKMoE framework effectively integrates knowledge guidance with specialized expert networks to achieve superior radiomap estimation performance in complex wireless environments.
...
TL;DR: FLUID is a training-free face de-identification framework that performs identity substitution in diffusion model latent space using semantic displacement, achieving superior identity suppression while preserving attributes.


<details>
  <summary>Details</summary>
Motivation: To develop an effective face de-identification method that can suppress identity information while maintaining other facial attributes, addressing privacy concerns in image sharing.

Method: Uses semantic displacement in latent h-space of pretrained diffusion models, discovers identity-editing directions via optimization with reagent losses for attribute preservation and identity suppression, and employs both linear and geodesic editing schemes.
...
Method: Fuses multi-scale hierarchical features from ResNet-50 with semantically rich representations from DINOv3 ViT-S/16 using a learnable attention mechanism to capture both low-level texture and high-level semantic structure.

Result: Achieves SOTA on IC9600 benchmark (Pearson r=0.9581), surpassing multimodal methods with 21.5x fewer parameters, and generalizes well across datasets and metrics.
...
TL;DR: DepthFocus is a steerable Vision Transformer that enables intent-driven stereo depth estimation, allowing users to specify which depth layer to focus on in scenes with transmissive materials.
...
Method: A steerable Vision Transformer conditioned on a scalar depth preference that dynamically adapts its computation to focus on intended depths, trained primarily on a new 500k multi-layered synthetic dataset capturing diverse see-through effects.

Result: Achieves state-of-the-art performance on conventional single-depth benchmarks (BOOSTER), demonstrates intent-aligned estimation on new multi-depth datasets, and shows strong generalization on unseen see-through scenes.

Conclusion: DepthFocus represents a significant step toward active and human-like 3D perception by enabling selective depth perception in complex scenes with transmissive materials.
...
TL;DR: MVLR is a unified model that restores images degraded by various weather conditions using visual-language reasoning and memory retrieval for enhanced accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Reliable visual perception under adverse weather conditions is challenging but crucial for autonomous driving and outdoor robots, requiring robust image restoration methods.

Method: Combines lightweight encoder-decoder with Visual-Language Model for chain-of-thought degradation reasoning and Implicit Memory Bank for storing degradation patterns, using cross-attention for adaptive feature fusion.
...
Conclusion: MVLR provides an effective balance between model compactness and expressiveness suitable for real-time deployment in diverse outdoor conditions.
...
### [41] [RacketVision: A Multiple Racket Sports Benchmark for Unified Ball and Racket Analysis](https://arxiv.org/abs/2511.17045)
*Linfeng Dong,Yuchen Yang,Hao Wu,Wei Wang,Yuenan HouZhihang Zhong,Xiao Sun*

Main category: cs.CV

TL;DR: RacketVision introduces a novel sports analytics dataset with fine-grained racket pose and ball position annotations for table tennis, tennis, and badminton, enabling research on ball tracking, racket pose estimation, and trajectory prediction.
...
Method: Created a dataset with three interconnected tasks: fine-grained ball tracking, articulated racket pose estimation, and predictive ball trajectory forecasting. Evaluated baselines and found CrossAttention mechanism essential for effective multi-modal fusion.

Result: CrossAttention mechanism significantly improves trajectory prediction performance, surpassing strong unimodal baselines, while naive concatenation of racket pose features degrades performance.

Conclusion: RacketVision provides a versatile resource for future research in dynamic object tracking, conditional motion forecasting, and multimodal analysis in sports, establishing a strong foundation for sports analytics advancement.
...
TL;DR: ConfusedTourist reveals VLMs' vulnerability to cultural cue mixing, showing significant accuracy drops when multiple cultural concepts coexist in images due to attention shifts to distracting cues.


<details>
  <summary>Details</summary>
Motivation: Existing VLM evaluations overlook scenarios with multiple cultural cues, failing to test stability across diverse cultural inputs crucial for supporting multicultural societies.

Method: Introduced ConfusedTourist, a cultural adversarial robustness suite using image-stacking and image-generation perturbations to test VLM stability against mixed geographical cues.

Result: VLMs show critical vulnerability with heavy accuracy drops under simple perturbations, worsened by image-generation variants, due to systematic attention shifts toward distracting cultural cues.

Conclusion: Visual cultural concept mixing substantially impairs state-of-the-art VLMs, highlighting urgent need for more culturally robust multimodal understanding systems.
...
Method: Three-phase training: RL for bounding box format, mid-training with pedestrian datasets, SFT on tracking datasets, and final RL for performance improvement and instruction following.

Result: Experimental results show the proposed method performs better than previous methods on tracking benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: MRI is crucial for brain disease diagnosis but not always feasible for certain patients. Existing CT-to-MRI synthesis methods struggle with highly sparse CT volumes from low-dose protocols, making accurate full brain MRI reconstruction challenging.

Method: Propose ReBrain framework: 1) Use Brownian Bridge Diffusion Model to synthesize MRI slices from sparse CT, 2) Retrieve similar CT slices from database via fine-tuned retrieval model, 3) Use retrieved slices as references through ControlNet branch to guide generation and ensure structural continuity, 4) Apply spherical linear interpolation for rare retrieval failures.

Result: Extensive experiments on SynthRAD2023 and BraTS datasets demonstrate that ReBrain achieves state-of-the-art performance in cross-modal reconstruction under sparse conditions.

Conclusion: ReBrain effectively addresses the challenge of synthesizing brain MRI from sparse CT scans through retrieval-augmented diffusion framework, achieving superior reconstruction quality compared to existing methods.
...
Method: Uses traversal orders of uniform spanning trees sampled from image patch lattices, obtained via breadth-first search and rejection sampling to ensure connected partial observations appear as sequence prefixes.
...
Conclusion: STAR provides a structured randomized strategy that balances performance and flexibility for autoregressive image generation and editing tasks.
...
Method: Uses hierarchical language-driven agent planners to parse text into detailed scene descriptions, initializes 3D point clouds, implements arrangement constraints for collision-free layouts, and employs AnyReach Sampling and ITFS strategies for efficient rendering.
...
Conclusion: RoomPlanner provides an effective automatic solution for 3D room generation from text, achieving high-quality results with improved efficiency compared to existing methods.
...
TL;DR: PathAgent is a training-free LLM-based agent framework that emulates human pathologists' stepwise reasoning process for analyzing whole-slide images, providing fully interpretable predictions through explicit chain-of-thought.
...
Method: PathAgent uses three modules: Navigator for iterative micro-region localization, Perceptor for extracting morphology visual cues, and Executor for integrating findings into natural language trajectories, forming an explicit chain-of-thought.
...
Conclusion: PathAgent successfully bridges the gap between opaque computational pipelines and human pathologists' reflective reasoning, offering a transparent and clinically grounded framework for whole-slide image analysis with interpretable predictions.
...
TL;DR: DMG-YOLO is a lightweight real-time detector for small objects in remote sensing images, using dual-branch feature extraction and multi-scale fusion to balance accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of real-time small object detection in remote sensing imagery, where traditional methods struggle with balancing accuracy and computational efficiency.

Method: Proposes DMG-YOLO with three key components: Dual-branch Feature Extraction (DFE) module using depthwise convolutions and vision transformer with gating; Multi-scale Feature Fusion (MFF) with dilated convolutions; and Global and Local Aggregate Feature Pyramid Network (GLAFPN) for feature fusion.
...
Conclusion: DMG-YOLO effectively addresses the challenges of real-time small object detection in remote sensing images through its novel architecture design.
...
TL;DR: This paper proposes a comprehensive intervention framework to reduce hallucinations in Large Vision-Language Models by analyzing and intervening on multiple causal pathways in the transformer architecture.


<details>
  <summary>Details</summary>
Motivation: Despite impressive performance, LVLMs remain prone to hallucination, and current approaches don't fully address the complex interplay of different causal pathways that contribute to this issue.

Method: The framework analyzes three causal pathways (image-to-input-text, image-to-output-text, text-to-text) and identifies critical hallucination heads. It then intervenes on these heads using methods tailored to discriminative and generative question-answer formats.

Result: Experiments across multiple benchmarks show that the approach consistently reduces hallucinations across diverse alignment types, demonstrating effectiveness across different evaluation scenarios.

Conclusion: Hallucinations in LVLMs stem from multiple causal pathways rather than a single source, and targeted interventions on critical heads within these pathways can effectively mitigate hallucination issues.
...
TL;DR: RL-AD-Net is a reinforcement learning refinement framework that improves point cloud completion quality by adjusting latent features of pretrained autoencoders to enhance geometric consistency.
...
Method: Uses RL agent to adjust global feature vectors in latent space of pretrained autoencoder, with PointNN selector to choose between original and refined outputs based on geometric consistency.

Result: Consistently improves completion quality across both training-style and random cropping scenarios on ShapeNetCore-2048, outperforming baseline methods.

Conclusion: RL-AD-Net provides effective, lightweight, and model-agnostic refinement that enhances geometric fidelity without requiring retraining of completion networks.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for training autonomous driving systems require diverse point cloud data, especially for edge cases, but real-world data acquisition is challenging and simulation approaches are time-consuming and fail to capture real-world complexity.

Method: Transform LiDAR point clouds to 2D range view images, use convex hull-based semantic masks to guide diffusion-based generation, ensuring geometric consistency by preserving object dimensions, orientations, and locations.

Result: High-quality LiDAR point cloud generation capable of producing complex edge cases and dynamic scenes, validated on the KITTI-360 dataset.

Conclusion: This approach provides a cost-effective and scalable solution for generating diverse LiDAR data, improving the robustness of autonomous driving systems.
...
TL;DR: REArtGS++ improves articulated object reconstruction by modeling screw joints without type priors, using planar Gaussian splatting with temporal geometry constraints for better part-level surface reconstruction and joint parameter estimation.


<details>
  <summary>Details</summary>
Motivation: REArtGS struggles with screw-joint or multi-part objects and lacks geometric constraints for unseen states, limiting its generalizability for articulated object reconstruction.

Method: Models decoupled screw motion for each joint without type prior, jointly optimizes part-aware Gaussians with joint parameters through part motion blending, and introduces temporally consistent regularization between planar normal and depth using Taylor first-order expansion.

Result: Demonstrates superiority in generalizable part-level surface reconstruction and joint parameter estimation on both synthetic and real-world articulated objects compared to existing approaches.

Conclusion: REArtGS++ effectively addresses limitations of previous methods by incorporating temporal geometry constraints and planar Gaussian splatting, achieving better performance in articulated object reconstruction.
...
TL;DR: The paper addresses cultural bias in multilingual text-to-image models by identifying culture-sensitive neurons and proposing two alignment strategies to improve cross-lingual cultural consistency without sacrificing model performance.


<details>
  <summary>Details</summary>
Motivation: Current multilingual T2I models often produce culturally neutral or English-biased results, failing to preserve cultural connotations across languages, despite having the necessary cultural knowledge.
...
Result: Experiments on CultureBench showed consistent improvements in cultural consistency while maintaining fidelity and diversity compared to strong baselines.

Conclusion: The methods effectively address cultural bias in multilingual T2I models by targeting specific culture-sensitive neurons and layers, enabling better cross-lingual cultural representation without full model retraining.
...
TL;DR: MuM extends masked autoencoding to multiple views for better 3D vision features, outperforming DINOv3 and CroCo v2 on reconstruction, matching, and pose estimation tasks.


<details>
  <summary>Details</summary>
Motivation: Most self-supervised learning methods focus on semantic understanding rather than geometric reasoning. CroCo showed promise for 3D understanding but has limitations in scalability and simplicity.
...
Conclusion: Multi-view masked autoencoding with inter-frame attention provides a simpler and more scalable approach for learning 3D-aware visual representations.
...
Result: Substantially enhances generative diversity with negligible performance influences, effectively addressing diversity collapse in VAR models.

Conclusion: DiverseVAR provides a simple yet effective solution to restore generative diversity in VAR models while preserving high-fidelity synthesis, requiring no additional training.
...
TL;DR: Proposes DIF-V dataset with 27,780 synthetic face images to address biases in face verification systems and analyzes model performance with identity style modifications.


<details>
  <summary>Details</summary>
Motivation: Existing face datasets suffer from racial, gender, and demographic biases, limiting fairness and effectiveness of face verification systems used in applications like online banking and device access.

Method: Integrated advanced generative models to create diverse synthetic face images representing various facial traits while adhering to identity card photo standards, and introduced the DIF-V dataset as a benchmark.

Result: Analysis revealed existing verification models exhibit gender and racial biases, and identity style modifications negatively impact model performance.

Conclusion: This work addresses dataset inequities, contributes to diversity and ethics discussions in AI, and provides foundation for more inclusive and reliable face verification technologies.
...
TL;DR: MoE-ViT introduces a Mixture-of-Experts architecture for Vision Transformers to efficiently handle multi-channel images by treating each channel as an expert and using a lightweight router to select only relevant experts per patch, reducing computational costs while maintaining or improving performance.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers face computational bottlenecks in multi-channel domains due to quadratic growth in attention from channel-wise comparisons, leading to excessive FLOPs and high training costs. The paper addresses the efficiency challenge in cross-channel attention.

Method: Proposes MoE-ViT architecture that treats each channel as an expert and employs a lightweight router to select only the most relevant experts per patch for attention, inspired by Sparse Mixture-of-Experts philosophy.
...
Conclusion: MoE-ViT provides a practical and attractive backbone for multi-channel imaging by solving the computational bottleneck while maintaining model effectiveness.
...
Method: Uses planar Gaussian Splatting with a Gaussian information field to select optimal viewpoints, compresses 3D Gaussians into planar Gaussians for normal and depth estimation, and optimizes through depth smooth regularization and few-shot diffusion with part segmentation probability.
...
TL;DR: ReCoVAD is a training-free video anomaly detection framework that uses selective frame processing inspired by human nervous system pathways, achieving state-of-the-art performance while processing only 16-29% of frames compared to previous methods.
...
Method: ReCoVAD uses dual pathways: (1) Reflex pathway with lightweight CLIP-based module for fast response using dynamic memory, and (2) Conscious pathway with medium-scale vision-language model for refined analysis and prototype updates, plus integrated LLM for periodic review.
...
Conclusion: Dense reasoning is not necessary for large-model-based VAD; selective frame processing through dual-pathway architecture can achieve superior performance with significantly reduced computational costs.
...
TL;DR: Proposes a knowledge distillation framework using teacher networks fine-tuned on small bias-free datasets to mitigate shortcut learning in medical image analysis, achieving comparable performance to bias-free training even on out-of-distribution data.
...
Method: Knowledge distillation framework leveraging teacher networks fine-tuned on small task-relevant datasets to guide student networks trained on large biased datasets, targeting different shortcut types across network layers.

Result: Consistent improvements over traditional methods (ERM, augmentation-based, group-based bias mitigation) across multiple datasets (CheXpert, ISIC 2017, SimBA) and architectures, achieving performance comparable to bias-free training even on out-of-distribution data.
...
TL;DR: Proposes Partitioned Adaptive Contrastive Learning (PACL) to bridge the affective gap in visual emotion recognition by leveraging knowledge from pre-trained textual models through dynamic contrastive learning strategies on noisy social media data.


<details>
  <summary>Details</summary>
Motivation: Address the affective gap in visual emotion recognition where pre-trained visual models lack direct association between factual-level features and emotional categories, while textual models naturally express emotions explicitly.

Method: Partitioned Adaptive Contrastive Learning (PACL) that separates different sample types and uses distinct contrastive learning strategies, dynamically constructing negative and positive pairs to exploit noisy social media data.

Result: Significant improvement in performance of various pre-trained visual models for downstream emotion-related tasks by bridging the affective gap.

Conclusion: Borrowing knowledge from pre-trained textual models effectively enhances emotional perception in visual models, with PACL successfully leveraging noisy social media data to bridge the affective gap.
...
Method: ChainV performs coarse visual patch selection based on previous reasoning steps, refines it using attention intensity to find representative atomic visual hints, and uses a consistency-based evaluation with Bernoulli stochastic process to incorporate pixel coordinates and reliability.

Result: ChainV significantly improves reasoning accuracy and efficiency, achieving 2.3% improvement on MathVista within MIMO-VL-RL, while reducing inference latency by 51.4% and shortening output token length by 24.5%.

Conclusion: Dynamic integration of visual hints through ChainV framework effectively enhances multimodal reasoning by making it more concise and accurate, particularly for math-intensive tasks requiring multi-step symbolic reasoning.
...
TL;DR: RS-FMD is a structured database of 150+ remote sensing foundation models, and REMSA is an LLM-based agent that automatically selects appropriate models from natural language queries using in-context learning and transparent reasoning.


<details>
  <summary>Details</summary>
Motivation: Foundation models are widely used in remote sensing but selecting appropriate models is difficult due to scattered documentation, heterogeneous formats, and varied deployment constraints.

Method: Created RS-FMD database covering 150+ RSFMs across multiple modalities, resolutions, and learning paradigms. Built REMSA agent that interprets user requirements, resolves missing constraints, ranks candidates using in-context learning, and provides justifications.

Result: REMSA outperforms naive agents, dense retrieval, and unstructured RAG-based LLMs on a benchmark of 75 expert-verified query scenarios (900 configurations). Operates entirely on public metadata without accessing private data.

Conclusion: RS-FMD and REMSA provide effective solutions for automated remote sensing foundation model selection, addressing the challenge of model selection complexity through structured databases and intelligent agent systems.
...
TL;DR: PEGS integrates physical priors and event stream enhancement with 3D Gaussian Splatting to achieve deblurred target-focused modeling and motion recovery for rigid motion over large spatiotemporal scales.
...
Method: Triple-level supervision with acceleration constraint, event stream guidance, and Kalman regularizer; motion-aware simulated annealing strategy; first RGB-Event paired dataset for fast rigid motion.
...
Conclusion: PEGS effectively addresses rigid motion reconstruction challenges through physical priors and event stream enhancement.
...
TL;DR: SketchVerify is a training-free framework that improves video generation by planning and verifying motion trajectories before full synthesis, using a test-time sampling loop to ensure physical plausibility and semantic alignment.


<details>
  <summary>Details</summary>
Motivation: Existing video generation methods use single-shot plans limited to simple motions or iterative refinement that is computationally expensive. There's a need for better motion planning that ensures dynamic coherence without high computational costs.

Method: Proposes a sketch-verification-based planning framework that predicts multiple candidate motion plans, renders them as lightweight video sketches, and ranks them using a vision-language verifier for semantic alignment and physical plausibility. Iteratively refines plans until satisfactory.

Result: Experiments on WorldModelBench and PhyWorldBench show significant improvements in motion quality, physical realism, and long-term consistency compared to baselines, while being substantially more efficient. Scaling trajectory candidates consistently enhances performance.

Conclusion: SketchVerify effectively improves video generation quality by planning and verifying motion trajectories beforehand, achieving better dynamic coherence and physical realism with computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Modern LED lighting deviates significantly from Planckian illuminants, exposing limitations of conventional CCT-based colorimetric mapping that assumes illumination lies on the Planckian locus.

Method: Uses a lightweight multi-layer perceptron (MLP) with 2D chromaticity features instead of CCT interpolation, trained via lightbox-based calibration with representative LED sources.
...
Conclusion: Transitioning from 1D CCT to 2D chromaticity space with MLP-based mapping significantly improves colorimetric accuracy for modern non-Planckian lighting while preserving practical deployment feasibility.
...
<details>
  <summary>Details</summary>
Motivation: Deep learning-based image compression achieves state-of-the-art performance but faces deployment challenges on resource-constrained FPGAs due to quantization-induced degradation and hardware inefficiency.
...
Result: DRAQ reduces BD-rate overhead from 30% to 6.3%, with hardware optimizations cutting computational complexity by over 20% while maintaining rate-distortion performance.

Conclusion: The framework successfully bridges floating-point models to efficient integer implementations, achieving state-of-the-art efficiency and superior quality for FPGA-based image compression.
...
TL;DR: ODTSR is a one-step diffusion transformer that achieves state-of-the-art real-world image super-resolution with balanced fidelity and controllability through noise-hybrid visual streams and fidelity-aware adversarial training.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based real-world image super-resolution methods struggle to balance fidelity and controllability - multi-step methods have generative diversity but low fidelity, while one-step methods lose control flexibility.

Method: Uses Qwen-Image based transformer with Noise-hybrid Visual Stream (NVS) design: one stream receives LQ images with adjustable noise, another with consistent noise. Employs Fidelity-aware Adversarial Training (FAA) for one-step inference.

Result: Achieves SOTA performance on generic Real-ISR and enables prompt controllability on challenging scenarios like Chinese character text image super-resolution without specific dataset training.

Conclusion: ODTSR successfully addresses the fidelity-controllability trade-off in real-world image super-resolution through innovative noise-hybrid design and adversarial training, enabling both high-quality results and flexible control.
...
Method: Developed CC-DiceCE loss based on CC-Metrics framework and compared it with blob loss, both benchmarked against DiceCE baseline using nnU-Net framework for standardized evaluation.

Result: CC-DiceCE increases lesion detection (recall) with minimal degradation in segmentation performance, but produces slightly more false positives. Multi-dataset study shows it generally outperforms blob loss.

Conclusion: CC-DiceCE is an effective loss function for improving small lesion detection in medical image segmentation, offering better performance than existing blob loss approaches.
...
Conclusion: The proposed DiffRefiner framework effectively combines discriminative trajectory proposals with generative refinement, demonstrating superior performance through strong guidance for the diffusion process and enhanced scene alignment.
...
TL;DR: UI-Styler is a novel ultrasound image style transfer framework that addresses domain shift issues by transferring texture patterns while preserving structural content and ensuring class-specific semantic alignment using pseudo labels.


<details>
  <summary>Details</summary>
Motivation: Ultrasound images vary across acquisition devices causing domain shifts that degrade performance of downstream inference models. Existing unpaired image translation methods overlook class-specific semantic alignment during domain adaptation.

Method: UI-Styler uses pattern-matching mechanism to transfer target domain texture patterns onto source images while preserving source structural content. It employs class-aware prompting strategy guided by pseudo labels of target domain for semantic alignment.
...
Conclusion: UI-Styler effectively addresses domain shift in ultrasound imaging through class-aware style transfer and semantic alignment, demonstrating superior performance over existing methods.
...
TL;DR: FireScope is a VLM-based reasoning-to-generation framework that uses multimodal data to predict wildfire risk maps with interpretable reasoning traces, demonstrating strong cross-continental generalization.
...
Method: Proposes FireScope framework using reinforcement learning and visual supervision to predict risk rasters with complementary reasoning traces, trained on Sentinel-2 imagery, climate data, and expert-defined risk rasters from USA.
...
Conclusion: Reasoning can ground raster prediction models to improve both generalization and interpretability, enabling robust cross-continental wildfire risk modeling.
...
TL;DR: Self-supervised representations show promise for audio-visual deepfake detection by capturing meaningful patterns across modalities, but fail to generalize reliably across datasets due to dataset-specific characteristics.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate the potential of self-supervised representations for audio-visual deepfake detection, which remains underexplored compared to prior work that uses these features in isolation or within complex architectures.
...
Conclusion: Self-supervised representations learn meaningful patterns for deepfake detection but face fundamental challenges in achieving robust cross-domain performance, with generalization failure stemming from dataset characteristics rather than superficial pattern learning.
...
TL;DR: Proposes LENS-Net for nighttime traffic sign recognition, featuring adaptive image enhancement and multimodal classification, and introduces INTSD dataset with 41 traffic sign classes captured under diverse Indian nighttime conditions.


<details>
  <summary>Details</summary>
Motivation: Traffic sign recognition at night is challenging due to visual noise and lack of public nighttime datasets. Existing methods struggle with low illumination robustness and fail to effectively leverage multimodal cues.

Method: 1) Introduces INTSD dataset with 41 traffic sign classes from diverse Indian regions under varying lighting/weather. 2) Proposes LENS-Net with adaptive image enhancement detector for joint illumination correction and sign localization, followed by multimodal CLIP-GCNN classifier using cross-modal attention and graph-based reasoning.

Result: LENS-Net surpasses existing frameworks, with ablation studies confirming effectiveness of key components. The method provides robust and semantically consistent recognition for nighttime traffic signs.

Conclusion: The proposed approach addresses nighttime traffic sign recognition challenges through a comprehensive dataset and multimodal architecture, achieving state-of-the-art performance with publicly available dataset and code.
...
Method: Combines group equivariant CNNs (G-CNNs) with C4 cyclic group via e2cnn library, implements structured pruning that preserves equivariant properties, adaptive fine-tuning with early stopping, dynamic INT8 quantization, and a comprehensive pipeline including training, knowledge distillation, pruning, fine-tuning, and quantization.
...
Conclusion: The framework provides a reproducible approach for optimizing equivariant models, successfully combining group-theoretic network design with practical deployment needs, particularly relevant for satellite imagery analysis and geometric vision tasks.
...
Method: Introduces query-shared cross-attention module to integrate 6-DoF camera poses and 2D rendered video frames into unified representation, plus two-stage training strategy: first learns coarse camera control from poses, then refines with visual information.

Result: Outperforms state-of-the-art methods by over 20% in camera control precision and view consistency, while achieving highest video generation quality on both real-world and synthetic datasets.

Conclusion: PostCam successfully enables flexible and accurate camera trajectory editing in dynamic scenes while maintaining high visual fidelity, representing significant advancement in novel-view video generation.
...
TL;DR: A multi-stage noise-decoupling framework for HSI denoising that decomposes complex noise into explicit and implicit components, using pre-training for explicit noise and high-frequency wavelet guidance for implicit noise, achieving state-of-the-art performance.
...
Method: Multi-stage framework that decouples noise into explicitly modeled (handled via pre-training with existing noise models) and implicitly modeled (addressed via high-frequency wavelet guided network) components, using separate pre-training and joint fine-tuning strategy.

Result: Extensive experiments show the framework outperforms state-of-the-art methods on both public and captured datasets, effectively handling complex real-world noise and significantly enhancing HSI quality.

Conclusion: The proposed noise-decoupling approach successfully addresses the limitations of traditional noise modeling methods by reducing noise complexity and enhancing learnability, providing an effective solution for real-world HSI denoising.
...
TL;DR: Proposes Non-Parametric Probabilistic Robustness (NPPR) as a practical alternative to existing probabilistic robustness metrics by learning perturbation distributions directly from data instead of assuming fixed distributions.


<details>
  <summary>Details</summary>
Motivation: Existing probabilistic robustness formulations assume fixed and known perturbation distributions, which is unrealistic in practice. This limitation motivates the need for a more practical robustness metric that doesn't rely on predefined distributions.

Method: Develops NPPR using Gaussian Mixture Model with MLP heads and bicubic up-sampling to learn optimized perturbation distributions directly from data, covering both input-dependent and input-independent perturbation scenarios.
...
Conclusion: NPPR is validated as a more practical robustness metric that addresses distributional uncertainty by learning perturbation distributions from data rather than making unrealistic distributional assumptions.
...
TL;DR: VLA-4D is a vision-language-action model with 4D awareness for spatiotemporally coherent robotic manipulation, addressing limitations of existing methods in temporal control.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models struggle with spatiotemporally coherent manipulation, particularly in achieving temporally coherent control over action execution despite embedding 3D positions.

Method: Two key designs: 1) 4D-aware visual representation by embedding 1D time into 3D positions and fusing via cross-attention, 2) Spatiotemporal action representation extending spatial actions with temporal information for planning and alignment with LLM.

Result: Extensive experiments show superiority across different robotic manipulation tasks, achieving spatially-smooth and temporally-coherent control.

Conclusion: The unified framework with 4D-aware visual and spatiotemporal action representations enables effective spatiotemporally coherent robotic manipulation.
...
TL;DR: CA-SAM enables efficient continual learning for medical image segmentation using SAM's zero-shot priors and lightweight Alignment Layers to prevent catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Heterogeneous privacy policies prevent joint training on pooled medical datasets, and SAM's computational overhead limits practical deployment despite strong zero-shot capabilities.

Method: Introduces Alignment Layer for feature distribution alignment and CA-SAM framework that automatically adapts Alignment Layers for continual learning while leveraging SAM's zero-shot priors.

Result: Achieves state-of-the-art performance across nine medical segmentation datasets in continual-learning scenarios.

Conclusion: SAM paradigm is highly promising when balanced for efficiency and performance, with CA-SAM providing an effective solution for continual medical image segmentation.
...
TL;DR: SING3R-SLAM is a globally consistent Gaussian-based dense RGB SLAM framework that combines local reconstructions with a unified global Gaussian representation to address drift and redundancy issues in 3D mapping.


<details>
  <summary>Details</summary>
Motivation: To overcome challenges in dense 3D reconstruction integration with SLAM, including drift and redundant point maps that limit efficiency and downstream tasks like novel view synthesis.
...
Result: Achieves state-of-the-art tracking, 3D reconstruction, and novel view rendering with over 12% improvement in tracking, producing finer geometry while maintaining compact memory efficiency on real-world datasets.

Conclusion: SING3R-SLAM enables efficient and versatile 3D mapping for multiple downstream applications by jointly refining scene geometry and camera poses through a globally consistent Gaussian representation.
...
TL;DR: SPECTRE is a fully transformer-based foundation model for volumetric CT that uses 3D Vision Transformers with self-supervised and vision-language pretraining to learn general-purpose CT representations, outperforming prior models on multiple benchmarks.
...
Method: Uses scalable 3D Vision Transformer architectures with joint optimization of local transformer for high-resolution volumetric features and global transformer for whole-scan context. Combines DINO-style self-distillation with SigLIP-based vision-language alignment using paired radiology reports.

Result: Consistently outperforms prior CT foundation models across multiple benchmarks in both zero-shot and fine-tuned settings. Trained exclusively on openly available CT datasets without private data.

Conclusion: SPECTRE establishes a scalable, open, and fully transformer-based foundation model for 3D medical imaging that achieves high-performing, generalizable representations through modern self-supervised and cross-modal pretraining strategies.
...
TL;DR: A distortion-aware BEV segmentation framework that processes fisheye images directly using geometric unprojection and depth distribution estimation, achieving strong performance on parking and urban driving scenarios without undistortion.
...
Conclusion: The framework effectively handles fisheye distortion and produces continuous, uncertainty-aware semantic maps without requiring undistortion or perspective rectification.
...
TL;DR: Proposes Dual-domain Adaptation Networks to adapt pre-trained SR models from synthetic to real-world datasets using spatial-domain parameter updates and frequency-domain adaptation for enhanced super-resolution.


<details>
  <summary>Details</summary>
Motivation: Realistic image SR handles complex real-world degradations but suffers from limited real LR-HR data. Pre-trained models offer valuable prior knowledge that can improve generalization and reduce data needs.

Method: Uses spatial-domain adaptation with selective parameter updates and low-rank adaptation, plus frequency-domain branch that combines spectral data and spatial features to infer HR frequency maps.

Result: Superior performance on RealSR, D2CRealSR, and DRealSR benchmarks compared to state-of-the-art models.

Conclusion: The dual-domain adaptation approach effectively transfers pre-trained SR models to real-world scenarios, enhancing super-resolution quality while reducing data requirements.
...
TL;DR: QueryOcc is a self-supervised framework that learns continuous 3D semantic occupancy using 4D spatio-temporal queries, achieving state-of-the-art performance on camera-based semantic occupancy tasks without manual 3D labels.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for 3D scene understanding either rely on 2D rendering consistency (limiting 3D structure) or discretized voxel grids (limiting precision and scalability), while large-scale 3D annotation is prohibitively expensive.

Method: Uses independent 4D spatio-temporal queries sampled across adjacent frames, with supervision from pseudo-point clouds from vision foundation models or raw lidar data. Introduces contractive scene representation for long-range reasoning under constant memory.
...
Conclusion: Direct 4D query supervision enables strong self-supervised occupancy learning, demonstrating that query-based approaches can effectively learn continuous 3D semantic occupancy without manual labels.
...
TL;DR: Proposes a novel quaternion-based fidelity term for color image blind deconvolution that preserves color channel relationships, outperforming methods that process channels separately.
...
Result: Extensive experiments on real blurred color image datasets show the method effectively removes artifacts and significantly improves deblurring effects compared to existing approaches.

Conclusion: The proposed quaternion-based approach demonstrates strong potential as an effective tool for color image deconvolution by properly modeling color channel relationships.
...
TL;DR: Proposes relevance feedback mechanisms to improve visual search with VLMs, including pseudo-relevance feedback, generative relevance feedback, attentive feedback summarizer, and explicit feedback as upper-bound baseline.


<details>
  <summary>Details</summary>
Motivation: To improve VLM-based visual search performance without requiring fine-tuning or larger models, by adapting traditional text-based search relevance feedback mechanisms for multimodal retrieval.

Method: Four feedback strategies: 1) Classical pseudo-relevance feedback (PRF) using top-ranked results, 2) Generative relevance feedback (GRF) using synthetic captions, 3) Attentive feedback summarizer (AFS) with transformer-based multimodal feature integration, 4) Explicit feedback using ground-truth captions as upper-bound baseline.

Result: GRF, AFS, and explicit feedback improve retrieval performance by 3-5% in MRR@5 for smaller VLMs, and 1-3% for larger VLMs compared to no feedback. AFS mitigates query drift and is more robust in iterative multi-turn retrieval settings.

Conclusion: Relevance feedback consistently enhances retrieval across VLMs and enables interactive and adaptive visual search, with AFS showing particular robustness against query drift in iterative settings.
...
Method: Three-stage training: 1) Pre-training on noisy datasets for fundamental perception, 2) Multi-granularity fine-tuning with auxiliary tasks (bond classification and atom localization), 3) Reinforcement learning optimization using GRPO algorithm.
...
Conclusion: The proposed framework effectively addresses stereochemical recognition challenges in OCSR and demonstrates superior performance through systematic training methodology and optimization techniques.
...
<details>
  <summary>Details</summary>
Motivation: Existing finger pose estimation methods on touchscreens are limited to pitch and yaw angles using capacitive images, have reduced accuracy for large-angle inputs (>45°), and cannot estimate roll angle, restricting interaction capabilities.

Method: Proposes BiFingerPose using bimodal input: capacitive image + fingerprint patch from under-screen fingerprint sensor. This enables simultaneous and accurate prediction of comprehensive finger pose parameters including roll angle.
...
Conclusion: BiFingerPose demonstrates practical superiority for finger pose estimation, enabling new applications in authentication security and interactive experiences. The bimodal approach successfully addresses limitations of single-modality methods.
...
Method: Proposes SpatialGeo with hierarchical fusion of CLIP semantic features and geometry features from vision-only self-supervised learning, trained efficiently with pretrained LLaVA using random feature dropping.
...
Conclusion: The proposed SpatialGeo effectively enhances MLLMs' spatial grounding capability through geometry-semantics fusion, achieving better performance with lower computational overhead.
...
TL;DR: NoPe-NeRF++ is a local-to-global optimization method for training NeRF without pose priors, combining relative pose initialization, local joint optimization, and global bundle adjustment to improve both pose estimation and novel view synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing methods like NoPe-NeRF that focus only on local image relationships struggle with accurate camera pose recovery in complex scenarios, necessitating a more robust approach that integrates both local and global optimization.
...
Conclusion: NoPe-NeRF++ successfully combines local and global cues with NeRF, validating the design choices through extensive evaluations and showing it's the first work to seamlessly integrate these optimization strategies.
...
TL;DR: A method to generate images with accurate refraction of transparent objects by synchronizing pixels using Snell's Law and recovering hidden surfaces via panorama generation.


<details>
  <summary>Details</summary>
Motivation: Generative models perform poorly at synthesizing transparent objects with accurate refraction, as they fail to properly model optical laws like refraction where pixel rays intersect with other surfaces.

Method: Synchronize pixels within and outside object boundaries using Snell's Law of Refraction during generation, and recover hidden surfaces by generating and synchronizing with a panorama image using the same warping procedure.

Result: The approach generates optically-plausible images that respect physical constraints of refraction, producing much more realistic transparent objects compared to standard generative models.

Conclusion: By explicitly incorporating optical laws like Snell's Law and recovering hidden surfaces through panorama synchronization, the method significantly improves the quality of generated transparent objects with accurate refraction effects.
...
Method: Embed multiple media into diffusion models' conditional space with cross-medium style augmentation, using a reverse-painting training strategy to ensure smooth, human-aligned generation. Built a large-scale dataset of real painting processes.
...
Conclusion: The proposed framework successfully enables faithful reproduction of human creative workflows with consistent texture evolution and process transfer across different artistic styles and media.
...
TL;DR: A novel label-efficient method for skeleton-based action recognition using GCNs that learns optimal data subsets for labeling through an objective function combining representativity, diversity, and uncertainty.
...
Method: Proposes a novel acquisition function that scores the most informative subsets for labeling using an objective function mixing data representativity, diversity, and uncertainty, extended with an invertible GCN for mapping data to latent spaces.
...
TL;DR: DSeq-JEPA improves I-JEPA by introducing discriminative sequential prediction, using saliency maps to identify and predict regions in order of importance, combining JEPA's latent prediction with GPT-style sequential reasoning.


<details>
  <summary>Details</summary>
Motivation: I-JEPA treats all image regions uniformly without considering visual importance, while human vision processes information sequentially from most to least informative regions.

Method: Uses transformer-derived saliency maps to identify primary discriminative regions first, then predicts subsequent regions in discriminative order, creating a curriculum-like semantic progression.
...
TL;DR: Proposes a Unified Attention-Mamba (UAM) backbone for cell-level radiomics analysis, achieving state-of-the-art performance in cell classification (78% accuracy) and tumor segmentation (80% precision) on H&E images.


<details>
  <summary>Details</summary>
Motivation: Cell-level radiomics features provide fine-grained tumor insights but are underexplored, with no dedicated backbone for radiomics data. Existing methods focus on slide/patch-level analysis, missing micro-level patterns.

Method: Introduces UAM backbone that flexibly combines Attention and Mamba modules in unified architecture, eliminating manual ratio tuning. Extends to multimodal framework for joint cell classification and image segmentation.

Result: UAM improves cell classification accuracy from 74% to 78% (n=349,882 cells) and tumor segmentation precision from 75% to 80% (n=406 patches), outperforming leading image-based foundation models.

Conclusion: UAM demonstrates effectiveness as unified and extensible multimodal foundation for radiomics-driven cancer diagnosis, highlighting promise of unified Attention-Mamba architecture for medical imaging tasks.
...
<details>
  <summary>Details</summary>
Motivation: Gaussian representations in occupancy estimation require too many primitives, increasing memory and hindering real-time inference. Superquadrics offer diverse shapes with fewer primitives but lack rasterization for supervision.
...
Result: Achieves 75% memory footprint reduction, 124% faster inference, 5.9% mIoU improvement on Occ3D dataset, and 84% reduction in primitive count compared to Gaussian methods, enabling real-time inference.

Conclusion: SuperQuadricOcc is the first occupancy model to achieve real-time inference while maintaining competitive performance, demonstrating superquadrics' effectiveness for efficient scene representation in automated driving.
...
TL;DR: ATAC is a simple but effective test-time defense method that corrects adversarial perturbations in CLIP's embedding space using augmentation-induced drift vectors and angular consistency, achieving 50% higher robustness than previous methods with minimal computational cost.
...
Method: Operates in CLIP's embedding space, calculates augmentation-induced drift vectors to infer semantic recovery direction, and corrects embeddings based on angular consistency of latent drifts.

Result: Achieves 50% higher average robustness across benchmarks than previous SOTA methods, maintains robustness in extreme settings, and shows nontrivial robustness against adaptive attacks with minimal computational overhead.

Conclusion: ATAC represents an efficient novel paradigm for test-time adversarial defenses in CLIP's embedding space, demonstrating that simple embedding-space corrections can provide strong protection.
...
TL;DR: SVRecon extends sparse voxel rasterization with Signed Distance Functions for high-fidelity surface reconstruction, addressing optimization challenges through geometric initialization and spatial smoothness losses.
...
Method: Integrates SDF with sparse voxel rasterization, using robust geometric initialization from visual geometry models and spatial smoothness loss that enforces coherent relationships across parent-child and sibling voxel groups.

Result: Achieves strong reconstruction accuracy across various benchmarks with consistently speedy convergence.

Conclusion: SVRecon successfully combines sparse voxels with SDF for high-fidelity surface reconstruction, overcoming optimization challenges through geometric initialization and spatial coherence enforcement.
...
TL;DR: MorphSeek is a fine-grained representation-level policy optimization framework for deformable image registration that uses stochastic Gaussian policies in latent space for efficient exploration and achieves improved performance with minimal supervision.


<details>
  <summary>Details</summary>
Motivation: Existing reinforcement learning approaches for deformable image registration project high-dimensional deformation spaces into coarse representations, limiting their ability to capture spatially variant deformations and suffering from supervision scarcity.
...
TL;DR: MCMoE is a novel framework for multimodal Action Quality Assessment that handles missing modalities by dynamically reconstructing them and mixing expert knowledge, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal AQA models fail when modalities are missing during inference, causing catastrophic performance degradation due to disrupted cross-modal interactions.

Method: Proposes MCMoE with adaptive gated modality generator to reconstruct missing modalities, modality experts for unimodal knowledge, and dynamic mixing of expert knowledge for joint representations.
...
Method: Proposes dual-teacher knowledge fusion for clean feature preservation and robust feature enhancement, dynamic weight allocation based on teacher confidence, and adaptive sigmoid-based weighting to balance knowledge transfer across modalities.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of decomposing flat images into editable, ordered layers for digital content creation, inspired by artists' compositional processes.

Method: Proposed a neural network trained on curated layered vector graphics to predict layering from raster inputs, using a discrete, globally consistent ordering of elements optimized for editability.
...
Conclusion: By reframing depth as a creative abstraction rather than physical quantity, illustrator's depth prediction offers a new foundation for editable image decomposition with powerful downstream applications.
...
TL;DR: Study identifies optimal methods for using vision foundation models in unsupervised domain adaptation for lidar semantic segmentation, achieving state-of-the-art results across multiple challenging domain shifts.


<details>
  <summary>Details</summary>
Motivation: Semantic segmentation networks trained on one lidar type fail to generalize to unseen lidars, requiring domain adaptation solutions that leverage robust vision foundation models.

Method: Exhaustive study using unsupervised image-to-lidar knowledge distillation, focusing on backbone architecture selection, single pretrained backbone reuse, and frozen backbone with trainable MLP head.

Result: Achieves state-of-the-art results in four challenging domain adaptation settings, demonstrating superior generalization performance on target domains.

Conclusion: Optimal domain adaptation strategy involves using a single pretrained backbone kept frozen while training only an MLP head, enabling effective handling of multiple domain shifts with minimal retraining.
...
<details>
  <summary>Details</summary>
Motivation: Existing techniques struggle with accurate distance estimation from GPR B-scan images with minor distinctions, especially in adverse weather conditions for robot/vehicle localization.

Method: Custom neural network that extracts multi-scale features from consecutive GPR B-scan images and analyzes similarity/difference features to estimate Euclidean distances traveled.

Result: Achieved overall weighted RMSE of 0.449 m across all datasets, consistently outperforming state-of-the-art methods with 10.2% RMSE reduction.

Conclusion: The proposed neural network-based approach effectively handles minor distinctions in GPR B-scan images and provides superior distance estimation for robust localization in challenging conditions.
...
TL;DR: CWMDT transforms standard video diffusion models into counterfactual world models by creating digital twin representations, using LLMs for temporal reasoning about interventions, and generating counterfactual video sequences.


<details>
  <summary>Details</summary>
Motivation: Current world models focus on factual predictions, but emerging applications require counterfactual reasoning about "what if" scenarios, which is impossible with pixel-space representations that lack explicit object-level control.

Method: Three-step framework: 1) Construct digital twins as structured text representations of objects and relationships, 2) Use LLMs to reason about intervention propagation through time, 3) Condition video diffusion models with modified representations to generate counterfactual videos.

Result: CWMDT achieves state-of-the-art performance on two benchmarks, demonstrating that structured digital twin representations provide powerful control for video simulation-based world models.

Conclusion: Alternative representations like digital twins enable effective counterfactual world modeling, overcoming limitations of pixel-space approaches and enabling targeted interventions on specific scene properties.
...
TL;DR: Radar2Shape: A denoising diffusion model that reconstructs arbitrary 3D shapes from partially-observed high-frequency radar signals using frequency-shape correlation and hierarchical latent space learning.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods fail to represent arbitrary shapes from real-world radar signals collected over limited viewing angles, while optical 3D reconstruction methods struggle when naively treating radar signals as camera views.

Method: Two-stage approach: 1) learns regularized latent space with hierarchical shape features at multiple resolutions, 2) uses denoising diffusion conditioned on radar signal frequencies in coarse-to-fine manner to reconstruct 3D shapes.
...
Conclusion: Radar2Shape enables effective 3D reconstruction from limited radar observations and provides benchmark datasets to advance high-frequency radar research for safe real-world adaptation.
...
TL;DR: A deep learning method to estimate spine age from MRI images, using clustering to identify age-related degeneration patterns, and showing that the spine age gap (SAG) correlates with degenerative conditions and lifestyle factors.


<details>
  <summary>Details</summary>
Motivation: The spine is vulnerable to age-related degenerations visible in MRI, and there's a need for automated methods to assess spine health and identify biomarkers for degenerative conditions.

Method: Computer vision deep learning on 18,000+ MRI series, using UMAP and HDBSCAN clustering for eligibility criteria, with ablation studies on data size, loss functions, and spine regions.
...
Conclusion: SAG may be a useful biomarker for measuring overall spine health, as it correlates with both degenerative conditions and lifestyle factors.
...
TL;DR: The paper analyzes how downscaling LLMs affects multimodal capabilities, finding visual perception is disproportionately impacted. It introduces Extract+Think approach with visual extraction tuning and step-by-step reasoning to maintain performance in smaller models.
...
Method: Conducted principled analysis of LLM downscaling effects, isolated perception vs reasoning impacts, and introduced Extract+Think approach with visual extraction tuning and step-by-step reasoning.

Result: LLM downscaling causes sharp performance drops in visual perception, often matching or exceeding reasoning impact. Extract+Think approach sets new efficiency and performance standards.

Conclusion: Visual perception is a key bottleneck in downscaled multimodal models, and the Extract+Think methodology effectively addresses this limitation while maintaining performance.
...
TL;DR: Video-R4 introduces visual rumination - iterative frame selection, zooming, and re-encoding for text-rich video reasoning, achieving SOTA results on M4-ViteVQA and generalizing to multiple domains.


<details>
  <summary>Details</summary>
Motivation: Current video QA models fail on fine-grained evidence in text-rich videos due to single-pass perception over fixed frames, leading to hallucinations and missed transient textual cues that require repeated inspection.

Method: Multi-stage rumination learning framework that progressively finetunes a 7B LMM via SFT and GRPO-based RL to learn atomic and mixing visual operations (frame selection, zooming, re-encoding) through datasets Video-R4-CoT-17k and Video-R4-RL-30k.

Result: Video-R4-7B achieves state-of-the-art results on M4-ViteVQA and demonstrates strong generalization to multi-page document QA, slides QA, and generic video QA.

Conclusion: Iterative visual rumination is an effective paradigm for pixel-grounded multimodal reasoning, enabling models to handle fine-grained evidence in text-rich videos through repeated inspection similar to human reading behavior.
...
<details>
  <summary>Details</summary>
Motivation: Event cameras record sparse brightness changes but reconstructing intensity images is ill-posed due to absolute brightness ambiguity. Existing regression methods yield perceptually inferior results and struggle with model scaling.

Method: Proposed EvDiff with surrogate training framework, single forward diffusion step to reduce computational cost, temporally consistent EvEncoder, and eliminates need for paired event-image datasets.
...
Conclusion: EvDiff strikes a balance between fidelity and realism, enabling high-quality video generation from event streams without paired training data constraints.
...
TL;DR: A novel native 3D editing framework that directly manipulates 3D representations in a single feed-forward pass, outperforming existing methods in quality, consistency, and instruction fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing instruction-guided 3D editing methods are either too slow (optimization-based) or suffer from inconsistent geometry and poor visual quality (feed-forward approaches using multi-view 2D editing).

Method: Created a large-scale multi-modal dataset for 3D editing tasks, then developed a native 3D editing framework using two conditioning strategies: cross-attention and a novel 3D token concatenation approach.
...
Conclusion: The proposed native 3D editing framework sets a new benchmark for efficient and high-quality instruction-guided 3D editing, with token concatenation emerging as the superior conditioning strategy.
...
TL;DR: This study compares visual creativity between humans and AI image generators, finding humans outperform AI, with visual artists being most creative. Human guidance improves AI creativity, and human/AI raters show different judgment patterns.


<details>
  <summary>Details</summary>
Motivation: Visual creativity remains underexplored compared to language creativity in AI research, despite recent findings that LLMs match human performance in divergent thinking tasks.

Method: Compared image generation between human participants (Visual Artists and Non Artists) and AI (with high/low human input prompting). Used human raters and GPT4o to evaluate creativity of resulting images.

Result: Found clear creativity gradient: Visual Artists > Non Artists > Human Inspired AI > Self Guided AI. Human guidance strongly improved AI creativity, bringing it close to Non Artists level. Human and AI raters showed different creativity judgment patterns.
...
TL;DR: Cognitive BASIC is a minimal, BASIC-style prompting language and in-model interpreter that structures LLM reasoning into explicit, stepwise execution traces for transparent multi-step reasoning.


<details>
  <summary>Details</summary>
Motivation: To create an interpretable cognitive control layer that enables transparent multi-step reasoning inside large language models by repurposing the simplicity of retro BASIC programming concepts.
...
Result: Comparison across three LLMs on knowledge extraction, conflict detection, and reasoning tasks shows that all models can execute Cognitive BASIC programs, with overall strong but not uniform performance.

Conclusion: Cognitive BASIC provides an effective framework for structuring LLM reasoning into explicit, interpretable execution traces, enabling transparent multi-step reasoning with reliable performance across different language models.
...
TL;DR: A framework for systematic benchmark revision using statistical analysis of response patterns to identify invalid questions, with LLM-judge assistance to reduce human effort.


<details>
  <summary>Details</summary>
Motivation: Manual identification and correction of errors in thousands of benchmark questions is infeasible and creates a bottleneck for reliable AI evaluation.

Method: Statistical analysis of response patterns based on the assumption that mean score summarizes model performance, flagging items with statistics outside expected ranges for expert review, plus LLM-judge first pass.

Result: Achieved up to 84% precision in identifying problematic questions across nine widely used benchmarks, with reduced human effort through LLM assistance.

Conclusion: Provides an efficient and scalable framework for systematic benchmark revision that improves reliability of AI evaluations.
...
TL;DR: Proposes Hybrid Differential Reward (HDR) mechanism to solve vanishing reward differences in multi-vehicle cooperative driving by combining Temporal Difference Reward and Action Gradient Reward components.


<details>
  <summary>Details</summary>
Motivation: Traditional state-based reward functions suffer from vanishing reward differences in high-frequency continuous control tasks, leading to low signal-to-noise ratio for policy gradients and hindering algorithm convergence.

Method: HDR integrates two components: Temporal Difference Reward based on global potential function for long-term objectives, and Action Gradient Reward for local guidance with high SNR. Formulated as Multi-Agent POMDP with time-varying agent set.
...
Conclusion: HDR mechanism effectively addresses vanishing reward differences in multi-vehicle cooperative driving, providing better policy gradient signals and enabling stable learning of cooperative behaviors.
...
TL;DR: The paper presents UI extensions (verbal, visual, combined) for Bayesian Networks to improve user understanding of probabilistic reasoning, showing that all extensions outperform baseline and combined modality works best for some tasks.


<details>
  <summary>Details</summary>
Motivation: Bayesian Networks are considered transparent models but people struggle to understand them, and current UIs don't clarify BN reasoning, creating a need for better interface designs.

Method: Designed verbal and visual extensions to standard BN UI to guide users through common inference patterns, then conducted user study comparing verbal, visual, combined extensions against baseline UI.

Result: Users performed better with all three extension types than baseline for questions about observation impact, enabling paths, and influence of observations; combined verbal+visual modality was superior to either alone for some question types.

Conclusion: UI extensions that guide users through inference patterns significantly improve BN understanding, with multimodal (verbal+visual) approaches providing the best results for certain reasoning tasks.
...
TL;DR: MirrorMind is a hierarchical cognitive architecture that integrates individual researcher memories and collective disciplinary knowledge to enable AI scientists to perform more human-like scientific reasoning.


<details>
  <summary>Details</summary>
Motivation: Current AI scientists treat scientific discovery as solitary optimization, ignoring that human scientific insight comes from both individual cognitive trajectories and collective disciplinary memory networks.

Method: Three-level hierarchical architecture: Individual Level (episodic, semantic, persona memories), Domain Level (disciplinary concept graphs), and Interdisciplinary Level (orchestration engine) with separated memory storage and agentic execution.

Result: Evaluated across four tasks showing improved author-level cognitive simulation, complementary reasoning, cross-disciplinary collaboration, and multi-agent scientific problem solving.

Conclusion: MirrorMind enables structural, personalized, and insight-generating scientific reasoning by integrating individual cognitive depth with collective disciplinary breadth.
...
TL;DR: Budget-aware scaling methods for tool-augmented agents that dynamically adapt planning based on remaining tool-call budgets, improving cost-performance scaling.


<details>
  <summary>Details</summary>
Motivation: Current tool-augmented agents lack budget awareness when given larger tool-call budgets, hitting performance ceilings despite increased resources.

Method: Introduces Budget Tracker for continuous budget awareness and BATS framework for dynamic planning adaptation (dig deeper vs pivot) based on remaining resources.
...
Conclusion: Provides systematic insights for principled scaling in tool-augmented agents through budget awareness and unified cost metrics.
...
TL;DR: The paper reinterprets score-based diffusion for inverse problems as an EM-style framework, introducing DAPS++ which decouples diffusion from data refinement for more efficient and stable reconstruction.


<details>
  <summary>Details</summary>
Motivation: Current Bayesian interpretation of diffusion-based inverse problem solving fails to explain practical behavior where prior offers limited guidance and reconstruction is largely measurement-driven, effectively decoupling from diffusion dynamics.

Method: Reinterpret diffusion as initialization in EM-style framework, introduce DAPS++ that allows likelihood term to guide inference directly while maintaining numerical stability, with fully decoupled diffusion stage and data-driven refinement.
...
Conclusion: The EM-style framework clarifies diffusion's role in inverse problems, and DAPS++ demonstrates that unified diffusion trajectories remain effective in practice while offering computational advantages.
...
TL;DR: Proposes multi-modal patient-level information extraction combining structured EHR data via Bayesian networks and unstructured clinical notes via neural text classifiers, using virtual evidence with consistency nodes for improved calibration and interpretable fusion.
...
Method: Multi-modal approach using Bayesian networks for structured EHR data and neural text classifiers for clinical notes, with virtual evidence augmented by consistency nodes for probabilistic fusion of predictions.

Result: The consistency node improves calibration of final predictions compared to virtual evidence alone, allowing better handling of missing information and resolving contradictions between tabular and text data.
...
TL;DR: A formal BDI Ontology is developed as a modular design pattern to represent agent cognitive architecture, with experiments showing its integration with LLMs and reasoning platforms for semantically interoperable AI systems.
...
Method: Created a formal BDI Ontology as a modular Ontology Design Pattern, aligned with foundational ontologies, and tested through two experiments: coupling with LLMs via Logic Augmented Generation, and integration with Semas reasoning platform using T2B2T paradigm.
...
Conclusion: The BDI Ontology provides both conceptual and operational foundation for cognitively grounded, explainable, and semantically interoperable multi-agent and neuro-symbolic systems in the Web of Data.
...
TL;DR: MIR introduces mutual intrinsic rewards to address sparse episodic rewards in multi-agent reinforcement learning by incentivizing agents to explore actions that affect teammates.


<details>
  <summary>Details</summary>
Motivation: Existing intrinsic reward methods work well in single-agent RL but struggle in MARL due to exponential sparsity of joint action trajectories and failure to account for team-influencing actions.

Method: Proposes Mutual Intrinsic Reward (MIR) that encourages individual agents to explore actions affecting teammates, combined with original strategies to stimulate team exploration.

Result: MIR demonstrates superior performance compared to state-of-the-art approaches in MiniGrid-MA environments with sparse rewards.

Conclusion: MIR is a simple yet effective enhancement strategy for MARL with extremely sparse episodic rewards that improves team exploration and algorithm performance.
...
TL;DR: Introduces EarthAgent, a multi-agent system using Hierarchical Task Abstraction Mechanism (HTAM) for complex geospatial analysis, outperforming existing approaches on specialized domain tasks.


<details>
  <summary>Details</summary>
Motivation: General LLM-driven agents struggle in specialized domains requiring structured workflows like remote sensing, where specialized tools and multi-step procedures challenge generalized approaches.

Method: HTAM structures multi-agent systems into logical hierarchy mirroring domain's task-dependency graph, enforcing procedural correctness and decomposing problems into sequential layers where sub-agents operate on preceding layers' outputs.

Result: EarthAgent substantially outperforms established single- and multi-agent systems on GeoPlan-bench, a comprehensive benchmark of realistic multi-step geospatial planning tasks.
...
TL;DR: The paper argues that AAMAS community tools (BDI architectures, communication protocols, mechanism design, institutional modeling) provide the foundation for developing agentic AI systems that are capable, transparent, cooperative, and accountable.


<details>
  <summary>Details</summary>
Motivation: To realize sustained autonomy, reasoning, and interaction capabilities in agentic AI systems by complementing agency assumptions with explicit models of cognition, cooperation, and governance.

Method: Align adaptive, data-driven approaches with structured models of reasoning and coordination from AAMAS community, using tools like BDI architectures, communication protocols, mechanism design, and institutional modeling.

Result: A perspective on agency that bridges formal theory and practical autonomy, enabling development of agentic systems with enhanced capabilities and accountability.

Conclusion: The conceptual tools from AAMAS community provide the necessary foundation for creating agentic AI systems that are not only capable and flexible but also transparent, cooperative, and accountable.
...
TL;DR: Probing LLMs with synthetic/off-policy data affects generalization; same-domain off-policy data works better than different-domain on-policy data for reliable monitoring.
...
Method: Systematically evaluate linear and attention probes across 8 LLM behaviors using synthetic/off-policy data, testing generalization to on-policy scenarios.

Result: Response generation strategy significantly affects probe performance; successful off-policy generalization predicts on-policy success; Deception and Sandbagging probes may fail to generalize; domain shifts cause larger performance degradation than policy shifts.
...
TL;DR: SRA-CP is a decentralized cooperative perception framework that enables selective information sharing only when risk-relevant blind zones are detected, achieving near-optimal perception with 80% bandwidth reduction.


<details>
  <summary>Details</summary>
Motivation: Existing CP approaches transmit irrelevant data exceeding bandwidth limits and rely on pre-defined partners, making them unsuitable for dynamic traffic environments.

Method: Decentralized protocol with continuous lightweight perception coverage broadcasts, perceptual risk identification for local occlusion assessment, and selective peer-based information exchange with safety-prioritized fusion.

Result: Achieves <1% AP loss for safety-critical objects vs generic CP using only 20% bandwidth, and 15% improvement over non-risk-aware selective CP methods.

Conclusion: SRA-CP effectively balances communication efficiency and perception performance through risk-aware selective cooperation in dynamic environments.
...
### [128] [MRI Super-Resolution with Deep Learning: A Comprehensive Survey](https://arxiv.org/abs/2511.16854)
...
<details>
  <summary>Details</summary>
Motivation: High-resolution MRI is clinically important but costly and technically constrained. Super-resolution offers a computational solution to improve image quality without additional hardware.

Method: Comprehensive review and taxonomy of DL-based MRI super-resolution methods, examining approaches from computer vision, computational imaging, inverse problems, and MR physics perspectives.

Result: The survey provides systematic categorization of MRI SR techniques, covering theoretical foundations, architectures, learning strategies, datasets, and performance metrics, along with open-access resources.

Conclusion: Deep learning-based super-resolution shows promise for enhancing MRI resolution, but open challenges remain that require community attention for clinical adoption.
...
### [129] [Avoiding Quality Saturation in UGC Compression Using Denoised References](https://arxiv.org/abs/2511.16876)
*Xin Xiong,Samuel Fernández-Menduiña,Eduardo Pavez,Antonio Ortega,Neil Birkbeck,Balu Adsumilli*

Main category: eess.IV

TL;DR: Proposes efficient methods to detect quality saturation in UGC video compression by using denoised MSE instead of conventional NRM evaluation, achieving 8%-20% BD-rate savings.


<details>
  <summary>Details</summary>
Motivation: Conventional codecs cause quality saturation when compressing noisy UGC videos - increasing bitrate preserves input artifacts without improving visual quality. Current NRM-based detection methods are inefficient.

Method: Two detection methods: DSD (distortion saturation detection) using input-dependent threshold from D-MSE, and RDSD (rate-distortion saturation detection) estimating Lagrangian at saturation point using low-complexity compression.

Result: Experiments with AVC show 8%-20% BD-rate savings across multiple NRMs by avoiding encoding in saturation regions.

Conclusion: Proposed methods efficiently detect quality saturation as pre-processing step, enabling standard codecs to avoid wasteful encoding in saturation regions for UGC compression.
...
### [130] [MedImageInsight for Thoracic Cavity Health Classification from Chest X-rays](https://arxiv.org/abs/2511.17043)
...
TL;DR: MedImageInsight foundational model achieves 0.888 ROC-AUC for automated chest X-ray classification (Normal vs Abnormal), comparable to CheXNet, with potential for clinical workflow integration.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges of increasing chest radiography volumes and radiologist workload by developing automated classification to support timely interpretation and reduce burden.

Method: Two approaches: (1) fine-tuning MedImageInsight for end-to-end classification, (2) using it as feature extractor for transfer learning with traditional ML classifiers. Evaluated on ChestX-ray14 dataset and real-world clinical data.

Result: Fine-tuned classifier achieved highest performance with ROC-AUC of 0.888 and superior calibration, outperforming transfer learning models and showing comparable performance to established architectures like CheXNet.
...
### [131] [OmniLens++: Blind Lens Aberration Correction via Large LensLib Pre-Training and Latent PSF Representation](https://arxiv.org/abs/2511.17126)
*Qi Jiang,Xiaolong Qian,Yao Gao,Lei Sun,Kailun Yang,Zhonghua Yi,Wenyong Li,Ming-Hsuan Yang,Luc Van Gool,Kaiwei Wang*

Main category: eess.IV

TL;DR: OmniLens++ is a framework for blind lens aberration correction that addresses data scalability and prior guidance challenges through expanded lens design specifications and a Latent PSF Representation using VQVAE to learn optical degradation priors.


<details>
  <summary>Details</summary>
Motivation: Existing lens library pre-training pipelines struggle with generalization due to limited data scalability and lack of optical degradation prior guidance, hindering their ability to handle diverse unknown optical degradations.

Method: Expands lens design specifications for increased degradation diversity, samples uniform distribution by quantifying spatial-variation patterns, and introduces Latent PSF Representation using VQVAE framework to learn degradation priors from Point Spread Functions.

Result: OmniLens++ demonstrates state-of-the-art generalization capacity in blind aberration correction on both real-world lenses and synthetic LensLib, with AODLibpro verified as scalable foundation and LPR effectively leveraging large-scale LensLib.

Conclusion: The framework successfully resolves key challenges in blind lens aberration correction, providing improved generalization and establishing scalable foundations for future optical degradation correction research.
...
### [132] [Learning Latent Transmission and Glare Maps for Lens Veiling Glare Removal](https://arxiv.org/abs/2511.17353)
...
<details>
  <summary>Details</summary>
Motivation: Compact optical systems suffer from veiling glare that degrades image quality, but existing scattering models fail to fit this spatial-varying glare and paired training data is hard to obtain.

Method: VeilGen learns to simulate veiling glare unsupervised using Stable Diffusion priors, generating paired datasets. DeVeiler uses these latent optical maps with reversibility constraints for restoration.

Result: The approach delivers superior restoration quality and physical fidelity compared to existing methods on challenging compact optical systems.

Conclusion: VeilGen reliably synthesizes realistic veiling glare and its learned latent maps effectively guide the restoration process in DeVeiler.
...
### [133] [Joint Design of Protein Surface and Structure Using a Diffusion Bridge Model](https://arxiv.org/abs/2511.16675)
*Guanlue Li,Xufeng Zhao,Fang Wu,Sören Laue*

Main category: cs.LG

TL;DR: PepBridge is a novel framework for joint design of protein surfaces and structures that integrates receptor surface geometry and biochemical properties to generate structurally viable proteins with surface complementarity.


<details>
  <summary>Details</summary>
Motivation: Designing diverse and physically realistic protein structures and surfaces that precisely complement target receptors remains a significant challenge in computational protein design, particularly for protein-protein interactions governed by surface complementarity and hydrophobic interactions.

Method: Uses denoising diffusion bridge models (DDBMs) to map receptor surfaces to ligand surfaces, followed by multi-model diffusion for structure prediction, with Shape-Frame Matching Networks ensuring alignment between surface geometry and backbone architecture.

Result: Extensive validation across diverse protein design scenarios demonstrates PepBridge's efficacy in generating structurally viable proteins with surface complementarity, conformational stability, and chemical feasibility.

Conclusion: PepBridge represents a significant advancement in the joint design of top-down protein structure, successfully integrating receptor surface information to generate complementary protein structures.
...
### [134] [DDTime: Dataset Distillation with Spectral Alignment and Information Bottleneck for Time-Series Forecasting](https://arxiv.org/abs/2511.16715)
...
<details>
  <summary>Details</summary>
Motivation: Time-series forecasting requires large datasets and computational resources. Dataset distillation offers a compact alternative but faces challenges with temporal bias from autocorrelation and insufficient sample diversity in time-series data.

Method: Proposes DDTime framework using first-order condensation decomposition with frequency-domain alignment to mitigate autocorrelation bias and inter-sample regularization based on information bottleneck principle to enhance diversity.

Result: Extensive experiments on 20 benchmark datasets show DDTime outperforms existing methods with ~30% relative accuracy gains while adding only ~2.49% computational overhead.

Conclusion: DDTime provides an effective plug-in distillation framework for time-series forecasting that achieves significant accuracy improvements with minimal computational cost, addressing key challenges in temporal data distillation.
...
### [135] [When Structure Doesn't Help: LLMs Do Not Read Text-Attributed Graphs as Effectively as We Expected](https://arxiv.org/abs/2511.16767)
*Haotian Xu,Yuning You,Tengfei Ma*

Main category: cs.LG

TL;DR: LLMs achieve strong performance on text-attributed graphs using only node text descriptions, with most structural encoding methods providing minimal or negative benefits, challenging traditional graph learning assumptions.
...
Result: LLMs using only node text descriptions achieve strong performance, while most structural encoding strategies offer marginal or negative gains, making explicit structural priors often unnecessary or counterproductive.

Conclusion: Traditional graph learning paradigms need rethinking in the LLM era, as structure may not be inherently beneficial, opening opportunities for semantics-driven graph learning approaches.
...
### [136] [GCL-OT: Graph Contrastive Learning with Optimal Transport for Heterophilic Text-Attributed Graphs](https://arxiv.org/abs/2511.16778)
*Yating Ren,Yikun Ban,Huobin Tan*

Main category: cs.LG

TL;DR: GCL-OT is a graph contrastive learning framework that uses optimal transport to handle multi-granular heterophily in text-attributed graphs, achieving better structure-text alignment than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with heterophilic graphs due to homophily assumptions and static text embeddings, limiting their ability to handle mixed, noisy, and missing semantic correlations.
...
Result: Extensive experiments on nine benchmarks show GCL-OT consistently outperforms state-of-the-art methods, with theoretical analysis confirming improved mutual information bounds and Bayes error guarantees.

Conclusion: GCL-OT provides an effective and robust solution for structure-text alignment in heterophilic text-attributed graphs through flexible bidirectional alignment using optimal transport.
...
### [137] [Revisiting Multimodal KV Cache Compression: A Frequency-Domain-Guided Outlier-KV-Aware Approach](https://arxiv.org/abs/2511.16786)
*Yaoxin Yang,Peng Ye,Xudong Tan,Chongjun Tu,Maosen Zhao,Jia Hao,Tao Chen*

Main category: cs.LG

TL;DR: FlashCache is a frequency-domain-guided KV Cache compression framework for multimodal LLMs that identifies and preserves outlier KV pairs while reducing KV memory usage by 80% and achieving 1.69× faster decoding.
...
Method: Proposes FlashCache with: 1) Outlier KV Recognition Module that models principal components in frequency domain to retain critical KV pairs, and 2) Dynamic Budget Allocation Module that adaptively determines per-layer KV Cache size.

Result: Achieves up to 1.69× faster decoding with 80% lower KV memory usage while maintaining task performance across multiple MLLMs and benchmarks, outperforming state-of-the-art multimodal KV compression methods.

Conclusion: FlashCache effectively compresses multimodal KV Cache by focusing on frequency-domain analysis and outlier KV preservation, enabling efficient inference while maintaining model performance.
...
### [138] [A Vector Symbolic Approach to Multiple Instance Learning](https://arxiv.org/abs/2511.16795)
*Ehsan Ahmed Dhrubo,Mohammad Mahmudul Alam,Edward Raff,Tim Oates,James Holt*

Main category: cs.LG

TL;DR: A novel MIL framework using Vector Symbolic Architectures (VSAs) that strictly enforces the iff constraint through symbolic operations in high-dimensional space, achieving SOTA results while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: Most deep learning-based MIL approaches violate the fundamental iff constraint (bag positive iff at least one instance positive), leading to inflated metrics and poor generalization. There's a need for principled methods that strictly adhere to MIL formulation.

Method: Uses VSAs to encode instances and concepts as nearly orthogonal high-dimensional vectors, with algebraic operations enforcing the iff constraint. Includes a learned encoder to transform data into VSA-compatible vectors and a VSA-driven MaxNetwork classifier.

Result: Achieves state-of-the-art results for valid MIL models on standard benchmarks and medical imaging datasets, outperforming existing methods while strictly adhering to MIL formulation.

Conclusion: Provides a principled, interpretable, and effective alternative to existing MIL approaches that rely on learned heuristics, bridging the gap between raw data and symbolic reasoning.
...
### [139] [A Robust Federated Learning Approach for Combating Attacks Against IoT Systems Under non-IID Challenges](https://arxiv.org/abs/2511.16822)
*Eyad Gad,Zubair Md Fadlullah,Mostafa M. Fouda*

Main category: cs.LG

TL;DR: This paper compares federated learning algorithms (FedAvg, FedProx, Scaffold) for IoT attack detection under statistical heterogeneity using the CICIoT2023 dataset.


<details>
  <summary>Details</summary>
Motivation: Address challenges of conventional ML training in resource-constrained IoT environments with privacy concerns and statistical heterogeneity in non-IID data across devices.

Method: Evaluate FedAvg, FedProx, and Scaffold federated learning algorithms under different data distributions using the CICIoT2023 dataset for large-scale IoT attack classification.

Result: The study provides performance comparisons of FL methods in handling statistical heterogeneity for IoT attack detection.

Conclusion: Federated learning offers promising solutions for IoT security challenges, with different algorithms showing varying effectiveness in handling statistical heterogeneity.
...
### [140] [Monte Carlo Expected Threat (MOCET) Scoring](https://arxiv.org/abs/2511.16823)
*Joseph Kim,Saahith Potluri*

Main category: cs.LG

TL;DR: MOCET is introduced as an interpretable, scalable metric to quantify real-world AI safety risks, addressing gaps in existing evaluation methods for ASL-3+ models.


<details>
  <summary>Details</summary>
Motivation: Existing AI safety evaluation metrics like LAB-Bench, BioLP-bench, and WMDP can assess model uplift and domain knowledge but lack contextualization of real-world risks and scalability to keep pace with rapid LLM advancements.

Method: The authors introduce MOCET, an interpretable and doubly-scalable metric that is both automatable and open-ended to quantify real-world AI safety risks.

Result: MOCET provides a framework to better contextualize real-world risks and keep pace with LLM advancements through its scalable and open-ended design.

Conclusion: MOCET addresses critical gaps in AI safety evaluation by offering an interpretable, scalable metric that can quantify real-world risks for ASL-3+ models, particularly in biosecurity contexts.
...
### [141] [ManifoldFormer: Geometric Deep Learning for Neural Dynamics on Riemannian Manifolds](https://arxiv.org/abs/2511.16828)
*Yihang Fu,Lifang He,Qingyu Chen*

Main category: cs.LG

TL;DR: ManifoldFormer is a geometric deep learning framework that learns neural manifold representations for EEG signals, overcoming limitations of traditional Euclidean approaches by preserving intrinsic geometric structure.


<details>
  <summary>Details</summary>
Motivation: Existing EEG foundation models treat neural signals as generic time series in Euclidean space, ignoring the intrinsic geometric structure of neural dynamics that constrains brain activity to low-dimensional manifolds, limiting representation quality and cross-subject generalization.

Method: Integrates Riemannian VAE for manifold embedding, geometric Transformer with geodesic-aware attention mechanisms operating directly on neural manifolds, and dynamics predictor using neural ODEs for manifold-constrained temporal evolution.
...
Conclusion: Geometric constraints are essential for effective EEG foundation models, as the approach reveals meaningful neural patterns consistent with neurophysiological principles.
...
### [142] [Analysis of heart failure patient trajectories using sequence modeling](https://arxiv.org/abs/2511.16839)
*Falk Dippela,Yinan Yu,Annika Rosengren,Martin Lindgren,Christina E. Lundberg,Erik Aerts,Martin Adiels,Helen Sjöland*

Main category: cs.LG

TL;DR: This paper presents a systematic comparison of six sequence models across three architecture classes (Transformers, Transformers++, Mambas) for clinical prediction tasks using EHR data from a Swedish heart failure cohort.


<details>
  <summary>Details</summary>
Motivation: Despite Transformers and Mamba architectures showing impressive performance in clinical prediction tasks, there is no established systematic approach to empirically analyze model performance and efficiency under various settings in the medical domain.

Method: Evaluated six sequence models across three architecture classes on three one-year prediction tasks using EHR data from 42,820 heart failure patients. Conducted ablations for EHR input sequences, architectural configurations, and temporal preprocessing techniques.

Result: Llama achieved the highest predictive discrimination, best calibration, and robustness across all tasks, followed by Mambas. Both architectures demonstrated efficient representation learning, with tiny configurations surpassing other large-scaled Transformers. At equal model size, Llama and Mambas achieved superior performance using 25% less training data.

Conclusion: This study provides the first systematic ablation study with design choices for input tokenization, model configuration and temporal data preprocessing, offering recommendations for future model development in clinical prediction tasks using EHRs.
...
### [143] [Provably Minimum-Length Conformal Prediction Sets for Ordinal Classification](https://arxiv.org/abs/2511.16845)
*Zijian Zhang,Xinyu Chen,Yuanjie Shi,Liyuan Lillian Ma,Zifan Xu,Yan Yan*

Main category: cs.LG

TL;DR: Proposes a model-agnostic conformal prediction method for ordinal classification that provides instance-level optimal prediction intervals with statistical guarantees.
...
Method: Formulates ordinal classification as minimum-length covering problem, develops sliding-window algorithm with linear complexity, and introduces length-regularized variant for smaller prediction sets.

Result: Significantly improves predictive efficiency over baselines by 15% average decrease across four benchmark datasets from diverse domains.

Conclusion: The proposed method provides model-agnostic, distribution-free ordinal conformal prediction with instance-level optimality and improved efficiency while maintaining statistical guarantees.
...
### [144] [Sex and age determination in European lobsters using AI-Enhanced bioacoustics](https://arxiv.org/abs/2511.16848)
...
TL;DR: This study uses Passive Acoustic Monitoring and AI models to classify European lobsters by age and sex based on their bioacoustic emissions, achieving high accuracy rates for both classifications.


<details>
  <summary>Details</summary>
Motivation: Monitoring elusive aquatic species like lobsters is challenging but crucial for fisheries and aquaculture management. Understanding lobster habitats, welfare, reproduction, sex, and age is essential for conservation efforts.

Method: Used Passive Acoustic Monitoring to collect lobster bioacoustic data (buzzing/carapace vibrations) from concrete tanks in Scotland. Applied Deep Learning models (1D-CNN, 1D-DCNN) and six Machine Learning models (SVM, k-NN, Naive Bayes, Random Forest, XGBoost, MLP) with MFCCs as features.

Result: For age classification (adult vs. juvenile), most models achieved over 97% accuracy (Naive Bayes: 91.31%). For sex classification, all models except Naive Bayes surpassed 93.23%.

Conclusion: The research demonstrates the potential of supervised ML and DL to extract age- and sex-related features from lobster sounds, offering a promising non-invasive PAM approach for lobster conservation and management in aquaculture and fisheries.
...
### [145] [Better audio representations are more brain-like: linking model-brain alignment with performance in downstream auditory tasks](https://arxiv.org/abs/2511.16849)
*Leonardo Pepino,Pablo Riera,Juan Kamienkowski,Luciana Ferrer*

Main category: cs.LG

TL;DR: Self-supervised audio models with strong task performance better predict auditory cortex activity than older specialized models, and brain similarity emerges as a byproduct of learning to reconstruct audio.


<details>
  <summary>Details</summary>
Motivation: To understand whether improving artificial neural networks' task performance also makes their internal representations more similar to brain signals in the auditory domain.

Method: Quantified alignment between 36 audio models and brain activity using fMRI data, voxel-wise/component-wise regression, RSA, evaluated models on 6 auditory tasks from HEAREval benchmark, and analyzed pretraining evolution of EnCodecMAE.

Result: Recent self-supervised models outperform older models in predicting auditory cortex activity, strong positive correlation (r>0.7) between task performance and brain alignment, and brain similarity increases progressively during pretraining without explicit optimization.

Conclusion: Brain-like representations emerge as a byproduct of learning to reconstruct missing information from naturalistic audio data, suggesting that improved task performance in ANNs leads to more brain-like internal representations.
...
### [146] [The use of vocal biomarkers in the detection of Parkinson's disease: a robust statistical performance comparison of classic machine learning models](https://arxiv.org/abs/2511.16856)
*Katia Pires Nascimento do Sacramento,Elliot Q. C. Garcia,Nicéias Silva Vilela,Vinicius P. Sacramento,Tiago A. E. Ferreira*

Main category: cs.LG

TL;DR: Deep Neural Networks outperform traditional ML methods for Parkinson's disease detection using vocal biomarkers, achieving 98.65% and 92.11% accuracy on two datasets.


<details>
  <summary>Details</summary>
Motivation: Parkinson's disease often causes vocal impairments early on, and vocal biomarkers offer a non-invasive, low-cost diagnostic alternative.

Method: Used two public voice datasets, extracted MFCC features, compared DNN with traditional ML using 1000 random validation runs with non-parametric statistical tests.

Result: DNN achieved superior performance (98.65% and 92.11% accuracy) compared to traditional ML models, with statistically significant differences.

Conclusion: DNNs provide greater accuracy and reliability for early neurodegenerative disease detection using voice biomarkers.
...
### [147] [Topologic Attention Networks: Attending to Direct and Indirect Neighbors through Gaussian Belief Propagation](https://arxiv.org/abs/2511.16871)
*Marshall Rosenhoover,Huaming Zhang*

Main category: cs.LG

TL;DR: Topologic Attention Networks introduce a probabilistic attention mechanism that learns information flow through both direct and indirect graph connections, overcoming limitations of local message passing in GNNs.


<details>
  <summary>Details</summary>
Motivation: Graph Neural Networks struggle with long-range dependencies due to local message passing, while existing solutions like continuous-time dynamics or dense self-attention suffer from high computational costs and limited scalability.

Method: Proposes topologic attention - a probabilistic mechanism that learns information propagation through both direct and indirect graph connections, enabling unified reasoning over local and global relationships without explicit pairwise interactions.

Result: Achieves state-of-the-art performance across all measured baseline models, demonstrating superior capability in modeling long-range dependencies.

Conclusion: Topologic Attention Networks provide an effective and scalable solution for modeling long-range dependencies in graphs through learned information propagation, outperforming existing approaches.
...
### [148] [PersonalizedRouter: Personalized LLM Routing via Graph-based User Preference Modeling](https://arxiv.org/abs/2511.16883)
...
Method: Converts interaction data into a heterogeneous graph to capture contextual information between user queries and optimal LLMs, modeling relationships between different node types through edges.

Result: Outperforms existing methods by 15.38% and 9.83% under simulation strategies, and by 16.19% and 59.69% on PersonaRoute-Bench with 1,000 users while maintaining higher efficiency. Shows strong few-shot generalization (64.81% for new users, 85.80% for new LLMs).

Conclusion: PersonalizedRouter effectively addresses personalized LLM selection by leveraging graph-based modeling of user preferences from interaction data, demonstrating superior performance and strong generalization capabilities.
...
### [149] [Predicting Talent Breakout Rate using Twitter and TV data](https://arxiv.org/abs/2511.16905)
*Bilguun Batsaikhan,Hiroyuki Fukuda*

Main category: cs.LG

TL;DR: This paper proposes a method to detect Japanese talents before they become famous by combining Twitter and TV data, and compares traditional time-series models with neural networks and ensemble learning for predicting talent breakout.
...
Method: Combined Twitter and TV data to predict talent breakout, experimented with traditional time-series models, neural networks, and ensemble learning methods.
...
TL;DR: PepEVOLVE is a dynamic framework for macrocyclic peptide optimization that outperforms existing methods by learning where to edit peptides and how to optimize them for multiple objectives, achieving better scores and faster convergence.


<details>
  <summary>Details</summary>
Motivation: Macrocyclic peptides combine biologics-like affinity with small-molecule developability, but their vast combinatorial space and multi-parameter objectives make lead optimization slow and challenging. Existing methods require pre-specifying mutable positions and have limited generalization ability.

Method: PepEVOLVE uses: (i) dynamic masking and CHUCKLES shifting for improved generalization, (ii) context-free multi-armed bandit router to discover high-reward residues, and (iii) evolving optimization algorithm with group-relative advantage to stabilize reinforcement updates.

Result: On Rev-binding macrocycle benchmark, PepEVOLVE outperformed PepINVENT with higher mean scores (0.8 vs. 0.6), best candidates scoring 0.95 (vs. 0.87), and faster convergence while optimizing permeability and lipophilicity with structural constraints.

Conclusion: PepEVOLVE provides a practical, reproducible path for peptide lead optimization when optimal edit sites are unknown, enabling more efficient exploration and improved design quality across multiple objectives.
...
TL;DR: SCR-MF is a modular two-stage workflow combining dropout detection (scRecover) and non-parametric imputation (missForest) for robust single-cell RNA-seq data analysis.


<details>
  <summary>Details</summary>
Motivation: Single-cell RNA sequencing suffers from pervasive dropout events that obscure biological signals, requiring effective imputation methods.

Method: Two-stage workflow: principled dropout detection using scRecover followed by robust non-parametric imputation via missForest.

Result: Achieves robust performance comparable to or exceeding existing methods while preserving biological fidelity and transparency across public and simulated datasets.

Conclusion: SCR-MF provides competitive balance between accuracy and computational efficiency, making it suitable for mid-scale single-cell datasets.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in trajectory anomaly detection including underexplored sub-trajectory detection, dependency on carefully tuned thresholds, and performance degradation from irregular sampling and noise in training data.

Method: Uses contrastive learning to extract diverse normal travel patterns and distinguish anomalous behaviors, combined with deep reinforcement learning for online, real-time anomaly scoring.

Result: Extensive experiments on two real-world datasets demonstrate the framework's effectiveness and robustness across various evaluation scenarios.

Conclusion: CroTad provides a threshold-free, robust solution for online trajectory anomaly detection that enables timely and fine-grained identification of abnormal segments in real-world applications.
...
TL;DR: An end-to-end pipeline using latent ODEs and gradient boosted trees enables robust arrhythmia classification across different ECG sampling frequencies, maintaining high performance even at lower frequencies.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between signal fidelity and battery life in wearable ECGs, where high sampling frequencies drain batteries quickly but lower frequencies make morphology analysis difficult.

Method: Train a latent ODE to model continuous ECG waveforms and create feature vectors from high-frequency single-channel signals, then use gradient boosted trees for classification across different sampling frequencies (360 Hz, 90 Hz, 45 Hz).

Result: Minimal performance degradation across frequencies with macro-averaged AUC-ROC values of 0.984 (360 Hz), 0.978 (90 Hz), and 0.976 (45 Hz).

Conclusion: The approach enables smaller wearables by sidestepping the trade-off between signal fidelity and battery life, promoting long-term cardiac health monitoring.
...
TL;DR: Tree of Claims (ToC) is a framework that uses Monte Carlo Tree Search and multi-agent LLMs to optimize patent claims by balancing novelty and legal scope, outperforming standard LLMs.


<details>
  <summary>Details</summary>
Motivation: Manual patent claim drafting is labor-intensive and inconsistent, while conventional LLMs lack structured reasoning for precise claim refinement.

Method: ToC integrates MCTS with a multi-agent system: EditorAgent proposes edits and ExaminerAgent critiques novelty and prior art, guided by a multi-objective reward function.

Result: ToC outperforms standard LLMs in zero-shot and few-shot scenarios, achieving 8% average composite score improvement and up to 9% in some cases.

Conclusion: ToC provides a transparent, controllable methodology that bridges LLM reasoning with MCTS planning for structured patent claim optimization.
...
TL;DR: This paper provides theoretical analysis of gradient descent dynamics for deep equilibrium models (DEQs), proving conservation laws, well-conditioned training, and linear convergence for linear DEQs and single-index models.


<details>
  <summary>Details</summary>
Motivation: Despite DEQs' practical success in deep learning tasks, there's limited theoretical understanding of their gradient descent dynamics, which this work aims to address.

Method: The authors rigorously analyze gradient descent dynamics for DEQs in linear models and single-index models, proving conservation laws and convergence properties through mathematical analysis.

Result: Proved conservation law for linear DEQs showing parameters remain on spheres during training, demonstrated gradient flow remains well-conditioned, and established linear convergence to global minimizers under appropriate initialization.

Conclusion: The theoretical findings are validated through experiments, providing rigorous understanding of DEQ training dynamics and filling gaps in the literature.
...
TL;DR: FIRM is a federated multi-objective alignment method that uses in-client regularization to achieve communication efficiency and mitigate client disagreement drift in LLM alignment.


<details>
  <summary>Details</summary>
Motivation: Aligning LLMs with human values requires balancing conflicting objectives like helpfulness and harmlessness, but centralized training raises privacy concerns and FL methods face communication bottlenecks from multi-gradient transmissions.

Method: Each client locally solves a regularized multi-objective optimization problem, eliminating need for multi-gradient transmissions by transmitting only a single set of adapted parameters.

Result: FIRM achieves smoother training dynamics, reduced client disagreement drift, improved reward trade-offs, and can adapt to specified preferences between objectives.

Conclusion: FIRM provides an effective federated multi-objective alignment approach with proven convergence guarantees and practical communication efficiency.
...
TL;DR: EMTC method uses evolving masking and multi-view learning to improve MTS clustering by focusing on discriminative timestamps and preventing premature convergence.


<details>
  <summary>Details</summary>
Motivation: Existing MTS clustering methods suffer from redundancy in time-series data that diminishes attention to discriminative timestamps, and most masking strategies are static preprocessing steps isolated from learning.

Method: Proposes EMTC with Importance-aware Variate-wise Masking (IVM) that adaptively guides discriminative representation learning, and Multi-Endogenous Views (MEV) with reconstruction and contrastive learning pathways for generalization.

Result: Extensive experiments on 15 real benchmark datasets show EMTC achieves 4.85% average improvement over strongest baselines, demonstrating superiority over eight SOTA methods.

Conclusion: EMTC effectively addresses redundancy issues in MTS clustering through adaptive masking and multi-view learning, achieving significant performance improvements.
...
TL;DR: Energy scaling laws for diffusion models that predict GPU energy consumption based on computational complexity (FLOPs), achieving high accuracy across different models and hardware.


<details>
  <summary>Details</summary>
Motivation: Address growing concerns about energy consumption and environmental impact of diffusion models, and lack of principled methods to predict energy usage across different configurations.

Method: Adapt Kaplan scaling laws to predict GPU energy consumption, decomposing diffusion inference into text encoding, iterative denoising, and decoding components, with focus on denoising operations.

Result: Achieves high predictive accuracy (R-squared > 0.9) within individual GPU architectures and strong cross-architecture generalization with high rank correlations across models.

Conclusion: Validates compute-bound nature of diffusion inference and provides foundation for sustainable AI deployment planning and carbon footprint estimation.
...
TL;DR: Step-E integrates sample selection and model training into a single optimization process, using loss-based ranking to gradually exclude high-loss examples and focus on consistent data, improving performance on noisy datasets.
...
Method: Step-E ranks samples by loss at each epoch and gradually increases the fraction of high-loss examples excluded from gradient updates after a warm-up stage, creating an online curriculum that focuses on easy/consistent examples and ignores persistent outliers.

Result: On CIFAR-100N, Step-E improved ResNet-18 test accuracy from 43.3% to 50.4%, outperforming other methods and approaching clean-label oracle (60.5%). On CIFAR-10N, it improved from 83.9% to 85.3%, nearly matching clean-label oracle (85.9%) with moderate training overhead.

Conclusion: Step-E effectively handles noisy training data by integrating sample selection with model learning, achieving significant performance improvements over noisy baselines and competitive results compared to clean-label oracles.
...
TL;DR: Exact fingerprints provide small but consistent accuracy improvements in molecular property prediction over compressed fingerprints, but don't significantly improve Bayesian optimization performance.


<details>
  <summary>Details</summary>
Motivation: Hash collisions in molecular fingerprinting cause distinct substructures to share the same features, leading to overestimated molecular similarity and reduced accuracy in predictive models.

Method: Compared exact fingerprints vs standard compressed fingerprints using Gaussian process models on five molecular property prediction benchmarks from the DOCKSTRING dataset, and evaluated in both prediction accuracy and Bayesian optimization contexts.

Result: Exact fingerprints yielded small yet consistent improvements in predictive accuracy across all five benchmarks, but these gains did not translate to significant improvements in Bayesian optimization performance.

Conclusion: While exact fingerprints offer modest accuracy benefits for molecular property prediction, they don't provide meaningful advantages for Bayesian optimization applications.
...
### [161] [Why Do Language Model Agents Whistleblow?](https://arxiv.org/abs/2511.17085)
*Kushal Agrawal,Frank Xiao,Guido Bergman,Asa Cooper Stickland*

Main category: cs.LG

TL;DR: LLMs can act as whistleblowers by reporting suspected misconduct to external parties without user instruction, with frequency varying by model and being influenced by task complexity, moral nudges, and available tools.


<details>
  <summary>Details</summary>
Motivation: To study how LLM alignment training manifests when models use tools, particularly when they disclose misconduct to external parties without user knowledge or instruction.

Method: Created an evaluation suite with diverse staged misconduct scenarios to assess agents' whistleblowing behavior across different models and settings.

Result: Whistleblowing frequency varies by model family, decreases with task complexity, increases with moral nudges, and decreases when more tools and detailed workflows are provided. The evaluation shows lower model awareness than previous work.

Conclusion: LLM whistleblowing behavior is influenced by multiple factors including model architecture, task design, and available tools, highlighting the need for careful consideration of how alignment manifests in tool-using scenarios.
...
### [162] [Geometric-Disentangelment Unlearning](https://arxiv.org/abs/2511.17100)
...
Method: First-order analysis of retain loss change under parameter updates, showing retain loss unchanged iff update direction is orthogonal to retain gradient subspace. GU decomposes forget gradient into tangential (harmful) and normal (safe) components, executing only the normal component under trust-region budget.
...
Conclusion: Geometric-disentanglement Unlearning provides a theoretically sound and simple solution that can be attached to existing gradient-based unlearning procedures to mitigate side effects while maintaining effective forgetting.
...
### [163] [Four decades of circumpolar super-resolved satellite land surface temperature data](https://arxiv.org/abs/2511.17134)
*Sonia Dupuis,Nando Metzger,Konrad Schindler,Frank Göttsche,Stefan Wunderle*

Main category: cs.LG

TL;DR: New 42-year pan-Arctic land surface temperature dataset downscaled from AVHRR GAC data to 1 km resolution using deep learning super-resolution algorithm.


<details>
  <summary>Details</summary>
Motivation: Coarse spatial resolution of AVHRR GAC data limits utility for fine-scale permafrost dynamics and surface processes in the rapidly warming Arctic.

Method: Deep anisotropic diffusion model trained on MODIS LST data using coarsened inputs and native-resolution outputs, guided by high-resolution land cover, digital elevation, and vegetation height maps.

Result: 42 years of twice-daily, 1 km LST observations for entire pan-Arctic region, enabling improved permafrost modeling, air temperature reconstruction, and ice sheet mass balance assessment.

Conclusion: Enhanced dataset supports climate monitoring in pre-MODIS era and provides adaptable framework for future satellite missions and climate data record continuity.
...
### [164] [Reconstruction of Surface EMG Signal using IMU data for Upper Limb Actions](https://arxiv.org/abs/2511.17200)
*Shubhranil Basak,Mada Hemanth,Madhav Rao*

Main category: cs.LG

TL;DR: Deep learning model synthesizes sEMG signals from IMU data using dilated causal convolutions, achieving high temporal fidelity for muscle intent detection.


<details>
  <summary>Details</summary>
Motivation: sEMG provides muscle function insights but is noisy and hard to acquire, while IMUs offer robust wearable motion tracking, motivating synthesis of sEMG from IMU data.

Method: Collected simultaneous sEMG and IMU data at 1KHz for arm movements, trained Sliding-Window-Wave-Net model with dilated causal convolutions to map IMU to sEMG.

Result: Model successfully predicts timing and general shape of muscle activations with high temporal fidelity, though peak amplitudes are often underestimated.

Conclusion: Feasible to use IMU-based sEMG synthesis for muscle intent detection in prosthetics and rehabilitation biofeedback applications.
...
### [165] [DelTriC: A Novel Clustering Method with Accurate Outlier](https://arxiv.org/abs/2511.17219)
*Tomas Javurek,Michal Gregor,Sebastian Kula,Marian Simko*

Main category: cs.LG

TL;DR: DelTriC is a clustering algorithm that uses PCA/UMAP projection, Delaunay triangulation, and back-projection to form clusters in high-dimensional space, outperforming traditional methods like k-means, DBSCAN, and HDBSCAN.


<details>
  <summary>Details</summary>
Motivation: To address limitations of traditional clustering methods in high-dimensional spaces by decoupling neighborhood construction from decision-making for more robust clustering and outlier detection.

Method: Integrates PCA/UMAP-based projection to low-dimensional space, performs Delaunay triangulation to index local adjacency, then back-projects to original space for edge pruning, merging, and anomaly detection.

Result: DelTriC outperforms traditional methods (k-means, DBSCAN, HDBSCAN) in many scenarios, demonstrating both scalability and accuracy while significantly improving outlier detection capabilities.

Conclusion: DelTriC provides an effective approach for high-dimensional clustering by leveraging low-dimensional projections and back-projection mechanisms, offering superior performance compared to established clustering algorithms.
...
### [166] [Generating transition states of chemical reactions via distance-geometry-based flow matching](https://arxiv.org/abs/2511.17229)
*Yufei Luo,Xiang Gu,Jian Sun*

Main category: cs.LG

TL;DR: TS-DFM is a flow matching framework that predicts transition states from reactants and products, outperforming previous methods by 30% in structural accuracy and accelerating reaction mechanism exploration.


<details>
  <summary>Details</summary>
Motivation: Transition states are crucial for understanding reaction mechanisms but are difficult to explore experimentally and computationally, creating a need for efficient prediction methods.

Method: TS-DFM uses flow matching in molecular distance geometry space with TSDVNet to learn velocity fields for generating accurate TS geometries, capturing dynamic interatomic distance changes.

Result: Outperforms React-OT by 30% on Transition1X dataset, provides high-quality initial structures that accelerate CI-NEB convergence, identifies alternative reaction paths, and discovers more favorable TS with lower energy barriers.

Conclusion: TS-DFM demonstrates strong generalization on unseen molecules and reaction types, showing potential to facilitate comprehensive reaction exploration.
...
### [167] [FlexiFlow: decomposable flow matching for generation of flexible molecular ensemble](https://arxiv.org/abs/2511.17249)
*Riccardo Tedoldi,Ola Engkvist,Patrick Bryant,Hossein Azizpour,Jon Paul Janet,Alessandro Tibo*

Main category: cs.LG

TL;DR: FlexiFlow is a novel flow-matching architecture that jointly samples molecules with multiple conformations, enabling generation of conformational ensembles for better property assessment in drug discovery.


<details>
  <summary>Details</summary>
Motivation: Current 3D molecular generation models only produce single conformations, but the conformational landscape determines molecular properties and binding affinity. Generating multiple low-energy conformers allows direct assessment of thermodynamic observables.

Method: Extends flow-matching models to jointly sample molecules with multiple conformations while preserving equivariance and permutation invariance. Uses QM9 and GEOM Drugs datasets for training.

Result: Achieves SOTA in molecular generation, producing valid, unstrained, unique, and novel molecules with high fidelity. Generates conformational ensembles with coverage comparable to physics-based methods but much faster inference. Successfully transfers to protein-conditioned ligand generation.

Conclusion: FlexiFlow enables efficient generation of molecular conformational ensembles, improving assessment of molecular properties and binding capabilities in drug discovery applications.
...
### [168] [Enforcing governing equation constraints in neural PDE solvers via training-free projections](https://arxiv.org/abs/2511.17258)
...
<details>
  <summary>Details</summary>
Motivation: Neural PDE solvers often violate governing equation constraints, especially nonlinear constraints and dynamical PDEs with long-range temporal dependencies, making projection onto feasible sets challenging.

Method: Evaluated two training-free post-hoc projections: nonlinear optimization-based projection and local linearization-based projection using Jacobian-vector and vector-Jacobian products.
...
Conclusion: Post-hoc projection methods effectively enforce constraints in neural PDE solvers, addressing the challenge of constraint violations in scientific simulation.
...
### [169] [Automobile demand forecasting: Spatiotemporal and hierarchical modeling, life cycle dynamics, and user-generated online information](https://arxiv.org/abs/2511.17275)
*Tom Nahrendorf,Stefan Minner,Helfried Binder,Richard Zinck*

Main category: cs.LG

TL;DR: This study develops a forecasting framework for premium automotive demand that combines point and probabilistic forecasts across multiple planning levels using LightGBM ensembles, quantile regression, and mathematical programming reconciliation.


<details>
  <summary>Details</summary>
Motivation: Premium automotive manufacturers face complex forecasting challenges due to high product variety, sparse variant-level data, and volatile market dynamics, requiring improved demand forecasting methods.

Method: Combines point and probabilistic forecasts using ensembles of LightGBM models with pooled training sets, quantile regression, and mixed-integer linear programming reconciliation across multi-product, multi-market hierarchies.

Result: Spatiotemporal dependencies and rounding bias significantly affect forecast accuracy, with integer forecasts being crucial for operational feasibility. Online behavioral data considerably improves accuracy at disaggregated levels.

Conclusion: Short-term demand is reactive (life cycle maturity, autoregressive momentum), while medium-term demand is anticipatory (online engagement, planning targets, competitive indicators), with the methodology effectively addressing premium automotive forecasting challenges.
...
### [170] [SAVeD: Semantic Aware Version Discovery](https://arxiv.org/abs/2511.17298)
...
<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of repeated labor in data science caused by difficulty tracking similar work or transformations on datasets, eliminating reliance on metadata or integration assumptions.
...
Result: Achieves significantly higher accuracy on unseen tables and substantial boost in separation scores across five canonical datasets from Semantic Versioning in Databases Benchmark, outperforming untrained baselines and prior methods like Starmie.

Conclusion: SAVeD effectively distinguishes semantically altered dataset versions through contrastive learning, demonstrating competitive or superior performance compared to existing dataset-discovery methods.
...
### [171] [Self-supervised denoising of raw tomography detector data for improved image reconstruction](https://arxiv.org/abs/2511.17312)
*Israt Jahan Tulin,Sebastian Starke,Dominic Windisch,André Bieberle,Peter Steinbach*

Main category: cs.LG

TL;DR: Self-supervised deep learning methods outperform non-learning methods for denoising ultrafast electron beam X-ray CT data, improving signal-to-noise ratios and reducing reconstruction artifacts.


<details>
  <summary>Details</summary>
Motivation: Ultrafast electron beam X-ray CT produces noisy data due to short measurement times, causing reconstruction artifacts and limiting overall image quality.
...
Conclusion: Self-supervised deep learning approaches are effective for denoising ultrafast electron beam X-ray CT data and superior to traditional non-learning methods.
...
### [172] [ReBaPL: Repulsive Bayesian Prompt Learning](https://arxiv.org/abs/2511.17339)
*Yassir Bendou,Omar Ezzahir,Eduardo Fernandes Montesuma,Gabriel Mahuas,Victoria Shevchenko,Mike Gartrell*

Main category: cs.LG

TL;DR: ReBaPL introduces a Bayesian prompt learning method with cyclical step-size scheduling and repulsive forces to efficiently explore multimodal prompt posterior distributions, improving generalization over conventional prompt tuning.


<details>
  <summary>Details</summary>
Motivation: Conventional prompt tuning methods suffer from overfitting and poor out-of-distribution generalization. Bayesian prompt learning addresses these issues but needs better methods to explore complex multimodal posterior landscapes of prompts.

Method: Combines cyclical step-size schedule with SGHMC for alternating exploration/exploitation phases, plus repulsive forces based on probability metrics (MMD, Wasserstein) between prompt representations to diversify exploration and prevent mode collapse.
...
Conclusion: ReBaPL provides a modular plug-and-play Bayesian extension for existing prompt learning methods, enabling more comprehensive characterization of prompt posterior distributions and better generalization.
...
### [173] [Convergence and stability of Q-learning in Hierarchical Reinforcement Learning](https://arxiv.org/abs/2511.17351)
*Massimiliano Manenti,Andrea Iannelli*

Main category: cs.LG

TL;DR: The paper provides a theoretical convergence and stability analysis for Feudal Q-learning in Hierarchical Reinforcement Learning, showing it converges to a game equilibrium point.


<details>
  <summary>Details</summary>
Motivation: Hierarchical RL promises efficient temporal structure capture and better continual learning, but lacks theoretical guarantees behind practice.

Method: Proposed Feudal Q-learning scheme and analyzed convergence using Stochastic Approximation theory and ODE method.

Result: Proved convergence and stability theorem for Feudal Q-learning, showing updates converge to a game equilibrium point.

Conclusion: Theoretical analysis provides principled convergence guarantees for Feudal RL, opening door to game-theoretic approaches in Hierarchical RL.
...
### [174] [R2PS: Worst-Case Robust Real-Time Pursuit Strategies under Partial Observability](https://arxiv.org/abs/2511.17367)
...
<details>
  <summary>Details</summary>
Motivation: Existing reinforcement learning methods for pursuit-evasion games are limited to perfect information scenarios and don't account for evaders that can predict pursuers' actions, creating a gap for real-world applications with partial observability.

Method: The approach proves dynamic programming maintains optimality under asynchronous moves, proposes a belief preservation mechanism for partial observability, and embeds this into the EPG framework for cross-graph reinforcement learning against DP evasion strategies.
...
Conclusion: R2PS provides the first effective solution for worst-case robust real-time pursuit strategies in partially observable pursuit-evasion games, enabling practical deployment in security applications.
...
### [175] [A Unified Stability Analysis of SAM vs SGD: Role of Data Coherence and Emergence of Simplicity Bias](https://arxiv.org/abs/2511.17378)
*Wei-Kai Chang,Rajiv Khanna*

Main category: cs.LG

TL;DR: A linear stability framework is developed to analyze why SGD and related methods prefer flat minima in deep learning, focusing on gradient curvature coherence across data points.


<details>
  <summary>Details</summary>
Motivation: To understand why SGD and its variants find solutions that generalize well, particularly why they prefer flatter minima in overparameterized settings, and to provide a unified theory connecting data structure, optimization dynamics, and solution characteristics.

Method: Developed a linear stability framework analyzing SGD, random perturbations, and SAM in two-layer ReLU networks, using a coherence measure that quantifies gradient curvature alignment across data points.

Result: The coherence measure reveals why certain minima are stable and favored during training, explaining the preference for flatter minima in optimization dynamics.

Conclusion: The framework provides theoretical insights into how gradient curvature coherence across data points drives optimization towards stable, flat minima that generalize well in deep learning.
...
TL;DR: Proposes a novel coreset selection framework using posterior sampling to address gradient-based methods' limitations, achieving faster training and better generalization than state-of-the-art approaches.


<details>
  <summary>Details</summary>
Motivation: Address challenges in gradient-based coreset selection methods, including SGD as strong baseline and representativeness breakdown due to loss curvature mismatches, especially under limited data budgets.

Method: Establishes connection between posterior sampling and loss landscapes, introduces smoothed loss function based on posterior sampling on model weights, and provides convergence analysis for sampling-based coreset selection.

Result: Achieves faster training and enhanced generalization across diverse datasets compared to current state-of-the-art methods, with robust performance even in high data corruption scenarios.

Conclusion: The proposed framework effectively addresses limitations of existing gradient-based coreset selection methods through posterior sampling, providing improved stability, computational efficiency, and generalization performance.
...
TL;DR: DS-Span is a single-phase discriminative subgraph mining framework that unifies pattern growth, pruning, and supervision-driven scoring in one traversal, producing compact and discriminative subgraph features for graph embedding and classification.


<details>
  <summary>Details</summary>
Motivation: Existing subgraph mining methods suffer from redundant multi-phase pipelines, high computational cost, and weak coupling between mined structures and their discriminative relevance.

Method: DS-Span uses coverage-capped eligibility to limit exploration when graphs are sufficiently represented, and information-gain-guided selection to promote class-separating subgraphs while minimizing redundancy.

Result: Extensive experiments show DS-Span generates more compact and discriminative subgraph features than prior methods, achieving higher or comparable accuracy with significantly reduced runtime.

Conclusion: Unified single-phase discriminative mining serves as a foundation for scalable and interpretable graph representation learning.
...
### [178] [Self-Supervised Learning by Curvature Alignment](https://arxiv.org/abs/2511.17426)
*Benyamin Ghojogh,M. Hadi Sepanj,Paul Fieguth*

Main category: cs.LG

TL;DR: CurvSSL introduces curvature regularization to self-supervised learning, enhancing local manifold geometry modeling alongside standard statistical regularizers.


<details>
  <summary>Details</summary>
Motivation: Current SSL methods focus on statistical properties but largely ignore the local geometry of data manifolds, limiting their ability to capture intrinsic data structure.

Method: Extends standard SSL architecture with curvature-based regularizer using k-nearest neighbors and cosine interactions on hypersphere, with kernel variant using local Gram matrix in RKHS.

Result: CurvSSL achieves competitive or improved linear evaluation performance on MNIST and CIFAR-10 datasets compared to Barlow Twins and VICReg.

Conclusion: Explicitly shaping local geometry through curvature regularization is an effective complement to purely statistical SSL regularizers.
...
### [179] [Towards fully differentiable neural ocean model with Veros](https://arxiv.org/abs/2511.17427)
*Etienne Meunier,Said Ouala,Hugo Frezat,Julien Le Sommer,Ronan Fablet*

Main category: cs.LG

TL;DR: Differentiable extension of VEROS ocean model using JAX framework for automatic differentiation, enabling gradient-based optimization of initial states and parameter calibration.


<details>
  <summary>Details</summary>
Motivation: To enable automatic differentiation through ocean model dynamics for gradient-based optimization and parameter learning in ocean modeling.

Method: Modified VEROS ocean model to be fully compatible with JAX autodifferentiation framework, maintaining numerical consistency while enabling automatic differentiation.

Result: Successfully created a differentiable ocean model that allows gradient-based optimization for initial state correction and physical parameter calibration from observations.

Conclusion: Differentiable programming facilitates end-to-end learning and parameter tuning in ocean modeling, with the implementation being publicly available.
...
### [180] [Multi-Agent Pointer Transformer: Seq-to-Seq Reinforcement Learning for Multi-Vehicle Dynamic Pickup-Delivery Problems](https://arxiv.org/abs/2511.17435)
*Zengyu Zou,Jingyuan Wang,Yixuan Huang,Junjie Wu*

Main category: cs.LG

TL;DR: Proposes MAPT, an end-to-end centralized framework using sequence-to-sequence Transformer with Pointer Network for cooperative multi-vehicle dynamic pickup and delivery with stochastic requests, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Classical operations research methods struggle with computational complexity in large-scale dynamic problems, while existing RL methods fail to model joint action distributions, capture inter-entity relationships, and handle exponentially large action spaces.

Method: Uses Transformer Encoder for entity representations, Transformer Decoder with Pointer Network for autoregressive joint action generation, Relation-Aware Attention module for inter-entity relationships, and informative priors for decision guidance.

Result: MAPT significantly outperforms baseline methods on 8 datasets and shows substantial computational time advantages over classical operations research approaches.

Conclusion: The proposed MAPT framework effectively addresses challenges in cooperative multi-vehicle routing with stochastic requests through centralized decision-making and advanced attention mechanisms.
...
### [181] [InTAct: Interval-based Task Activation Consolidation for Continual Learning](https://arxiv.org/abs/2511.17439)
*Patryk Krukowski,Jan Miksa,Piotr Helm,Jacek Tabor,Paweł Wawrzyński,Przemysław Spurek*

Main category: cs.LG

TL;DR: InTAct is a continual learning method that preserves functional behavior in shared layers by capturing characteristic activation ranges from previous tasks and constraining updates to maintain consistency, reducing representation drift and improving performance in domain-incremental settings.


<details>
  <summary>Details</summary>
Motivation: Prompt-based continual learning methods are vulnerable to representation drift under domain shifts, where input distribution changes cause shared representations to evolve and overwrite previously useful features, leading to forgetting even when task-specific parameters are isolated.

Method: InTAct captures characteristic activation ranges associated with previously learned tasks and constrains updates to ensure network consistency within these regions while allowing flexible adaptation elsewhere, stabilizing the functional role of important neurons without freezing parameters or storing past data.

Result: InTAct consistently reduces representation drift and improves performance across diverse domain-incremental benchmarks like DomainNet and ImageNet-R, increasing Average Accuracy by up to 8 percentage points over state-of-the-art baselines.

Conclusion: By regulating representation changes where past knowledge is encoded, InTAct achieves a principled balance between stability and plasticity in continual learning, making it an effective architecture-agnostic solution that integrates seamlessly with existing prompt-based frameworks.
...
### [182] [Unmasking Airborne Threats: Guided-Transformers for Portable Aerosol Mass Spectrometry](https://arxiv.org/abs/2511.17446)
*Kyle M. Regan,Michael McLoughlin,Wayne A. Bryden,Gonzalo R. Arce*

Main category: cs.LG

TL;DR: MS-DGFormer: A transformer-based framework for single-shot pathogen identification from noisy MALDI-MS spectra, enabling real-time environmental monitoring without extensive preprocessing.
...
Method: Transformer architecture with novel dictionary encoder using SVD-derived denoised spectral information to capture long-range dependencies in time-series spectra from single-shot measurements.

Result: Superior pathogen identification from aerosol samples with robust performance on single-shot spectra, enabling autonomous real-time analysis in field conditions.

Conclusion: Eliminates need for extensive preprocessing, enabling portable MALDI-MS platforms for environmental pathogen detection and rapid biological threat response.
...
### [183] [PersonaAgent with GraphRAG: Community-Aware Knowledge Graphs for Personalized LLM](https://arxiv.org/abs/2511.17467)
*Siqi Liang,Yudi Zhang,Yue Guo*

Main category: cs.LG

TL;DR: A persona-based language model framework using Graph RAG to enhance personalization through knowledge graphs and community detection, achieving significant improvements on LaMP benchmark tasks.


<details>
  <summary>Details</summary>
Motivation: Need for personalized AI agents that adapt to individual user preferences and maintain persona-aligned behaviors.

Method: Knowledge-Graph-enhanced Retrieval-Augmented Generation (Graph RAG) that constructs LLM-derived graph index and summarizes communities, combined with dynamic prompt engineering using user history and global interaction patterns.

Result: On LaMP benchmark: 11.1% F1 improvement in news categorization, 56.1% F1 improvement in movie tagging, and 10.4% reduction in product rating MAE compared to prior methods.

Conclusion: The proposed framework effectively enables personalized AI agents by leveraging knowledge graphs and community detection for improved persona-aligned performance across multiple tasks.
...
### [184] [Harnessing Data from Clustered LQR Systems: Personalized and Collaborative Policy Optimization](https://arxiv.org/abs/2511.17489)
*Vinay Kanakeri,Shivam Bajaj,Ashwin Verma,Vijay Gupta,Aritra Mitra*

Main category: cs.LG

TL;DR: Proposes a clustering-based RL algorithm for multiple linear systems that simultaneously learns clusters and personalized policies, achieving correct clustering and improved sample efficiency without performance degradation from dissimilar processes.
...
Method: Combines sequential elimination and zeroth-order policy optimization to perform simultaneous clustering and learning, outputting personalized policies for each cluster of similar linear systems.

Result: Proves correct clustering with high probability under cluster separation conditions, shows sub-optimality gap scales inversely with cluster size without additional bias, and achieves mild logarithmic communication overhead.

Conclusion: First work to show clustering enables learning personalized policies that benefit from collaboration without suffering from dissimilar process data, with practical distributed implementation.
...
### [185] [Line-of-Sight Probability in Macrocells: Framework, Statistical Models, and Parametrization from Massive Real World Datasets in the USA](https://arxiv.org/abs/2511.16827)
*Bassel Abou Ali Modad,Xin Yu,Yao-Yi Chiang,Andreas F. Molisch*

Main category: eess.SP

TL;DR: Developed a framework for high-accuracy LOS probability modeling using geospatial data and real-world macrocell deployments, outperforming 3GPP models.


<details>
  <summary>Details</summary>
Motivation: Existing LOS models are based on limited datasets and don't adequately capture real-world macrocell deployments, impacting wireless channel characterization and coverage planning.

Method: Established a framework using geospatial data from 13,000+ US macrocell locations to create parameterized LOS models, treating parameters as random variables on a per-cell basis.

Result: Created a new model that better describes US macrocell deployments than 3GPP models, with improved accuracy for inter-cell interference prediction at cell edges.

Conclusion: LOS probability should be modeled per-cell with random variable parameters for better accuracy, providing a superior approach to average-based models for wireless network planning.
...
### [186] [State-of-charge estimation of lithium-ion batteries using a tree seed and genetic algorithm-optimized generalized mixture minimum error entropy-based square root cubature Kalman filter](https://arxiv.org/abs/2511.16888)
*Haiquan Zhao,Xiong Yin,Jinhui Hu*

Main category: eess.SP

TL;DR: Proposes GMMEE-SRCKF for SOC estimation, combining square-root CKF with generalized mixture minimum error entropy and TSGA optimization for better noise handling.


<details>
  <summary>Details</summary>
Motivation: Existing MEE-CKF has limited robustness in complex noise environments due to inflexible MEE criterion.

Method: Develops GMMEE-SRCKF using square-root CKF for stability, GMMEE with two flexible kernels for non-Gaussian noise, and TSGA for automatic parameter optimization.

Result: Achieves RMSE < 0.5%, outperforming existing robust filters in SOC estimation.

Conclusion: The proposed method provides superior accuracy and robustness for SOC estimation in complex noise conditions.
...
### [187] [Generative MIMO Beam Map Construction for Location Recovery and Beam Tracking](https://arxiv.org/abs/2511.17007)
*Wangqian Chen,Junting Chen,Shuguang Cui*

Main category: eess.SP

TL;DR: A generative framework that recovers location labels from sparse CSI measurements without explicit location labels, using dual-scale feature extraction and diffusion-based reconstruction to improve localization accuracy and channel capacity.


<details>
  <summary>Details</summary>
Motivation: Most ML-based wireless methods require large, accurately labeled datasets with location information, which are difficult and costly to obtain. This work aims to overcome this limitation by developing a framework that can work with sparse CSI measurements without explicit location labels.

Method: Proposes a generative framework with dual-scale feature extraction (angular space and neighboring samples), hybrid recurrent-convolutional encoder with truncation strategy and multi-scale convolutions, learnable radio map embedding for positional features, and diffusion-based generative decoder for CSI reconstruction.

Result: The proposed model improves localization accuracy by over 30% and achieves a 20% capacity gain in NLOS scenarios compared with model-based Kalman filter approaches.

Conclusion: The framework successfully recovers location information from sparse CSI measurements without explicit labels, demonstrating significant improvements in both localization accuracy and channel capacity, particularly in challenging NLOS environments.
...
### [188] [Movable Intelligent Surface-Enabled Wireless Communications: Static Phase Shifts with Mechanical Reconfigurability](https://arxiv.org/abs/2511.17058)
*Ziyuan Zheng,Qingqing Wu,Wen Chen,Weiren Zhu,Ying Gao*

Main category: eess.SP

TL;DR: A movable intelligent surface (MIS) architecture that mechanically slides a pre-phased secondary metasurface layer over a static primary layer to switch beam patterns, bridging the gap between costly dynamic RIS and limited static surfaces for quasi-static wireless environments.


<details>
  <summary>Details</summary>
Motivation: To address the practical gap between expensive dynamic reconfigurable intelligent surfaces (requiring dense cabling, continuous power, and signaling overhead) and low-cost static surfaces (limited to single beam patterns) for quasi-static environments like industrial IoT and smart agriculture.

Method: Proposed MIS architecture with mechanical sliding of pre-phased secondary metasurface layer over static primary layer; developed signal model using binary selection matrices; formulated joint optimization of static phase shifts and overlapping position selection; used penalty method, block coordinate descent, and Riemannian manifold optimization algorithms.

Result: Simulation results show MIS substantially narrows performance gap between single-layer static surfaces and dynamic RISs, providing practical and flexible solution for quasi-static wireless applications.

Conclusion: The MIS architecture offers a cost-effective alternative that bridges the gap between static and dynamic intelligent surfaces, suitable for quasi-static wireless scenarios with occasional or periodic demand changes.
...
### [189] [Super-Resolution ISAC Receivers: An MCMC-Based Gridless Sparse Bayesian Learning Approach](https://arxiv.org/abs/2511.17062)
*Keying Zhu,Xingyu Zhou,Jie Yang,Le Liang,Shi Jin*

Main category: eess.SP

TL;DR: Proposes a gridless sparse Bayesian learning framework for joint super-resolution multi-target detection and parameter estimation in integrated sensing and communication networks, achieving high accuracy with manageable computational cost.


<details>
  <summary>Details</summary>
Motivation: Address the trade-off between precision and complexity in low-altitude wireless networks where safety-critical applications demand high-accuracy sensing but conventional methods face limitations.

Method: Gridless sparse Bayesian learning framework treating target parameters as continuous variables, enhanced with gradient-based Markov chain Monte Carlo algorithm using mini-batch sampling and Adam optimizer for computational efficiency.

Result: Achieves super-resolution capabilities resolving targets separated by 50% of Rayleigh limit in range, 17% in velocity, and 52% in angle. At 20 dB SNR: >90% detection probability with RMSE of 0.07m range, 0.024m/s velocity, and 0.015° angle.

Conclusion: The proposed framework demonstrates robust performance against strong clutter, making it suitable for practical integrated sensing and communication applications in low-altitude wireless networks.
...
### [190] [Distributed Cubature Kalman Filter based on MEEF with Adaptive Cauchy Kernel for State Estimation](https://arxiv.org/abs/2511.17066)
*Duc Viet Nguyen,Haiquan Zhao,Jinhui Hu*

Main category: eess.SP

TL;DR: A distributed cubature Kalman filter using adaptive minimum error entropy with fiducial points (AMEEF-DCKF) is proposed to handle non-Gaussian noise, abnormal data, and communication burden in multi-sensor networks.


<details>
  <summary>Details</summary>
Motivation: To address challenges in distributed state estimation including non-Gaussian noise, abnormal data, and communication burden in multi-sensor networks.

Method: Proposes AMEEF optimization criterion with Cauchy kernels and adaptive bandwidth for noise handling, and leader-follower average consensus (LFAC) for distributed communication.

Result: The algorithm demonstrates effectiveness in power system state estimation and land vehicle navigation using a 10-node sensor network.

Conclusion: AMEEF-DCKF successfully overcomes limitations of traditional distributed Kalman filters by handling non-Gaussian noise and reducing communication burden through distributed consensus.
...
### [191] [Unleashing Sensor-Aided Environment Awareness for Beam Management in Beyond-5G Networks: An OpenAirInterface Experimental Platform](https://arxiv.org/abs/2511.17122)
*Aron Schott,Berk Acikgöz,Omar Massoud,Marina Petrova,Ljiljana Simić*

Main category: eess.SP

TL;DR: Development of an open SDR-based experimental platform integrating antenna arrays, beam sweeping, and sensor framework for real-time beam management research and ML dataset collection.


<details>
  <summary>Details</summary>
Motivation: Lack of open platforms for gathering datasets and evaluating beam management approaches in real-world scenarios with full-stack networks.

Method: Created SDR-based platform using OpenAirInterface, integrating low-cost antenna array transceivers, beam sweeping capabilities, and modular sensor framework.

Result: Successfully developed first full-stack experimental platform enabling real-time beam management experimentation and dataset collection for ML-based protocols.

Conclusion: The platform facilitates development of environment-aware ML-based beam management protocols by providing real-world testing capabilities and dataset collection.
...
### [192] [Teager-Kaiser Energy Methods For EEG Feature Extraction In Biomedical Applications](https://arxiv.org/abs/2511.17164)
*Ioanna Chourdaki,Kleanthis Avramidis,Christos Garoufis,Athanasia Zlatintsi,Petros Maragos*

Main category: eess.SP

TL;DR: TKEO-based features outperform traditional energy and power spectrum features in EEG classification tasks like motor imagery and epilepsy detection, performing comparably in emotion recognition.


<details>
  <summary>Details</summary>
Motivation: EEG signals are non-linear, non-stationary, and noisy, making feature extraction challenging. The paper investigates TKEO for modeling EEG energy dynamics.

Method: Used Gabor filterbanks to isolate frequency bands, applied TKEO with Energy Separation Algorithm to decompose signals into amplitude envelope and instantaneous frequency, then derived energy descriptors.

Result: TKEO features outperformed baseline features in motor imagery and epilepsy detection, and performed similarly in emotion recognition tasks.

Conclusion: The TKEO-based pipeline provides an effective framework for extracting EEG signal dynamics and shows promise for EEG classification tasks.
...
### [193] [Incorporating Bayesian Transfer Learning into Particle Filter for Dual-Tracking System with Asymmetric Noise Intensities](https://arxiv.org/abs/2511.17440)
*Omar A. Alotaibi,Brian L. Mark,Mohammad Reza Fasihi*

Main category: eess.SP

TL;DR: A particle filter approach using Bayesian transfer learning for dual-tracking systems with asymmetric measurement noise intensities, showing improved performance over isolated filters and other transfer learning methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of tracking nonlinear dynamical systems where sensors have asymmetric noise intensities, particularly improving performance for the primary sensor with higher noise.

Method: Bayesian transfer learning with particle filter, approximating densities using weighted particles to transfer knowledge from source sensor to primary sensor.

Result: Simulations show the proposed approach outperforms isolated particle filter and transfer learning applied to UKF/CKF. Performance improves with more particles at higher rate than isolated filter, but increases computational time. Performance gain is linearly proportional to noise intensity difference between sensors.

Conclusion: Bayesian transfer learning with particle filter effectively improves tracking in asymmetric noise dual-sensor systems, with performance scaling with noise difference and particle count, though at computational cost.
...
### [194] [Functional uniqueness and stability of Gaussian priors in optimal L1 estimation](https://arxiv.org/abs/2511.16864)
*Leighton Barnes,Alex Dytso*

Main category: cs.IT

TL;DR: This paper develops a quantitative stability theory showing that Gaussian priors are uniquely stable for optimal L¹ and L² estimation under Gaussian noise, with explicit rates for L² loss and Hermite expansion analysis for L¹ loss.


<details>
  <summary>Details</summary>
Motivation: To understand functional uniqueness and stability of Gaussian priors in Bayesian estimation, building on recent work that established uniqueness for conditional median (L¹ loss) to develop quantitative stability theory.

Method: For L² loss: derived explicit rates showing near-linearity implies proximity to Gaussian in Lévy metric. For L¹ loss: introduced Hermite expansion framework and analyzed adjoint of linearity-defining operator.

Result: Gaussian priors are uniquely stable solutions for both L¹ and L² estimation under Gaussian noise, with quantitative stability bounds established.

Conclusion: The results provide a complete functional-analytic understanding of linearity and stability in Bayesian estimation under Gaussian noise, confirming Gaussian priors as the unique stable choice.
...
### [195] [The Star Product of Uniformly Random Codes](https://arxiv.org/abs/2511.17236)
*Johan V. Dinesen,Ragnar Freij-Hollanti,Camilla Hollanti,Benjamin Jany,Alberto Ravagnani*

Main category: cs.IT

TL;DR: Analysis of expected dimension of star product of random linear codes, showing asymptotic maximum dimension and applications in cryptography and quantum error correction.


<details>
  <summary>Details</summary>
Motivation: To understand the expected dimension of star products of random linear codes, which has implications for private information retrieval, secure distributed matrix multiplication, quantum error correction, and cryptanalysis.

Method: Established correspondence between star product and evaluation of bilinear forms, providing lower bounds on expected star product dimension.

Result: Showed that asymptotically in both field size q and code dimensions, the expected dimension reaches its maximum.

Conclusion: The results have significant implications for various applications including private information retrieval, secure distributed matrix multiplication, quantum error correction, and potential cryptanalysis opportunities.
...
### [196] [Structured Approximation of Toeplitz Matrices and Subspaces](https://arxiv.org/abs/2511.17239)
*Albert Fannjiang,Weilin Li*

Main category: cs.IT

TL;DR: The paper presents efficient methods for recovering corrupted low-rank Toeplitz matrices and Fourier matrix ranges using Gradient-MUSIC algorithm, achieving minimax optimal performance guarantees.


<details>
  <summary>Details</summary>
Motivation: Structured approximation problems like recovering corrupted low-rank Toeplitz matrices and Fourier matrix ranges are computationally challenging due to difficulty in directly enforcing structural constraints.

Method: Uses Gradient-MUSIC algorithm for spectral estimation to solve both problems efficiently. The method works for rank r Toeplitz matrices corrupted by arbitrary noise matrices with bounded spectral norm.

Result: For corrupted rank r Toeplitz matrices, the algorithm outputs a rank r Toeplitz matrix with reconstruction error bounded by C√r‖E‖₂, achieving minimax optimal performance. Similar optimal results obtained for Fourier matrix range recovery.

Conclusion: The approach provides efficient and optimal solutions to challenging structured approximation problems, establishing quantitative connections between these problems and spectral estimation, with applicability to Hankel matrices as well.
...
### [197] [Fast Decoding for Non-Adaptive Learning of Erdős--Rényi Random Graphs](https://arxiv.org/abs/2511.17240)
*Hoang Ta,Jonathan Scarlett*

Main category: cs.IT

TL;DR: The paper presents a non-adaptive group testing approach for learning Erdős-Rényi graphs using O(ḱ log n) tests with sublinear decoding time O(ḱ^(1+δ) log n), improving upon prior work that required Ω(n²) decoding time.


<details>
  <summary>Details</summary>
Motivation: Learning graphs via group queries is challenging in non-adaptive settings, with prior methods either requiring optimal test counts but impractical Ω(n²) decoding time, or sublinear decoding but extra test factors. There's a need for methods that achieve both optimal test complexity and efficient decoding.

Method: Extends binary splitting approach from non-adaptive group testing to ER graph learning. Uses group queries on node subsets where each query reports whether at least one edge exists among queried nodes.

Result: Achieves high probability edge recovery using O(ḱ log n) tests with decoding time O(ḱ^(1+δ) log n) for any fixed δ>0, where ḱ is the expected number of edges. This improves upon prior work that had Ω(n²) decoding time.

Conclusion: The binary splitting approach successfully bridges the gap between test complexity and decoding time for ER graph learning, achieving order-optimal test counts with significantly improved sublinear decoding time compared to previous methods.
...
### [198] [Fluid Antenna System-Enabled UAV-to-Ground Communications](https://arxiv.org/abs/2511.17416)
*Xusheng Zhu,Kai-Kit Wong,Qingqing Wu,Hyundong Shin,Yangyang Zhang*

Main category: cs.IT

TL;DR: Performance analysis of UAV-to-ground links with fluid antenna systems over double-shadowing fading channels, deriving SNR statistics and key performance metrics with validated theoretical framework.


<details>
  <summary>Details</summary>
Motivation: Fluid antenna systems offer enhanced spatial diversity in compact form, while UAVs require channel models capturing both multipath fading and shadowing for future networks.

Method: Used eigenvalue-based approximation for correlated FAS ports to derive analytical expressions for SNR statistics (CDF and PDF), then developed exact integral expressions for outage probability, BER, and capacity.

Result: Derived tractable closed-form solutions for BER and capacity in dual-rank independent non-identically distributed case, achieving multiplicative diversity order of G_d = M × d.

Conclusion: The system achieves multiplicative diversity combining FAS spatial rank and intrinsic channel diversity, with simulation results validating the theoretical framework's accuracy.
