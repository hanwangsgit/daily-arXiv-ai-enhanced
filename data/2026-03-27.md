...
TL;DR: MEDOPENCLAW is an auditable runtime that enables VLMs to operate dynamically within medical imaging tools, while MEDFLOWBENCH is a full-study benchmark evaluating medical agentic capabilities across different interaction modes.
...
Method: Proposes MEDOPENCLAW runtime for VLMs to operate within standard medical tools (e.g., 3D Slicer), and MEDFLOWBENCH benchmark covering multi-sequence brain MRI and lung CT/PET with systematic evaluation across viewer-only, tool-use, and open-method tracks.

Result: State-of-the-art LLMs/VLMs (Gemini 3.1 Pro, GPT-5.4) can navigate viewers for basic tasks but performance degrades with professional tool access due to lack of precise spatial grounding, revealing a critical insight about spatial reasoning limitations.
...
TL;DR: SITH is a data-free, training-free framework that analyzes CLIP's vision transformer weights to decompose attention heads into semantically interpretable components, enabling precise model edits and insights into fine-tuning mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing interpretability methods for vision-language models rely on activations, making them dataset-dependent, vulnerable to data bias, and limited to coarse head-level explanations. There's a need for data-free, weight-space analysis that provides finer-grained semantic understanding.

Method: SITH analyzes CLIP's vision transformer in weight space by decomposing each attention head's value-output matrix into singular vectors. It uses COMP (Coherent Orthogonal Matching Pursuit) to interpret these vectors as sparse, semantically coherent combinations of human-interpretable concepts.

Result: SITH yields coherent, faithful intra-head explanations validated through reconstruction fidelity and interpretability experiments. It enables precise weight-space model edits that amplify/suppress specific concepts, improving downstream performance without retraining. It also reveals that fine-tuning primarily reweights a stable semantic basis rather than learning new features.

Conclusion: SITH provides a data-free, training-free framework for semantic inspection of transformer heads that offers fine-grained interpretability, enables targeted model editing, and reveals insights about model adaptation mechanisms in vision-language models.
...
Method: ReDiPrune selects tokens directly from vision encoder outputs using a lightweight scoring rule that jointly considers text-conditioned relevance and max-min diversity. It's plug-and-play, requiring no retraining or architectural modifications.

Result: Across 4 video and 5 image benchmarks, ReDiPrune consistently improves accuracy-efficiency trade-off. On EgoSchema with LLaVA-NeXT-Video-7B, retaining only 15% of visual tokens yields +2.0% absolute accuracy gain while reducing computation by >6× in TFLOPs.

Conclusion: ReDiPrune effectively reduces computational costs while improving performance by preserving rich visual features through early token pruning, demonstrating superior efficiency-accuracy trade-offs compared to existing methods.
...
TL;DR: A scale-aware 3D fusion framework that registers object meshes with transformer-predicted global point clouds to create metrically consistent digital twins, resolving scale ambiguity through VLM-guided geometric anchors.


<details>
  <summary>Details</summary>
Motivation: Embodied AI requires object-centric digital twins with accurate metric geometry and semantic grounding. Current transformer-based reconstruction methods produce point clouds with scale ambiguity and inconsistent coordinates, preventing reliable fusion with locally reconstructed object meshes.

Method: Proposes a scale-aware 3D fusion framework with VLM-guided geometric anchor mechanism to recover real-world metric scale. Uses geometry-aware registration pipeline with gravity-aligned vertical estimation, Manhattan-world constraints, and collision-free local refinement.

Result: Experiments on real indoor kitchen environments show improved cross-network object alignment and geometric consistency for downstream tasks like multi-primitive fitting and metric measurement. Also introduces an open-source indoor digital twin dataset with metrically scaled scenes and registered object mesh annotations.

Conclusion: The framework successfully resolves coordinate mismatch between transformer-predicted global point clouds and object meshes, enabling construction of metrically consistent digital twins for embodied AI applications.
...
TL;DR: BCMDA framework addresses domain shift and confirmation bias in semi-supervised medical image segmentation using virtual domain bridging and prototype alignment.


<details>
  <summary>Details</summary>
Motivation: Mixed domain semi-supervised medical image segmentation faces challenges with domain shift between labeled/unlabeled data and confirmation bias from inefficient learning from unlabeled data.

Method: Proposes BCMDA framework with two components: 1) Knowledge Transfer via Virtual Domain Bridging (KTVDB) using bidirectional correlation maps to create virtual domains with fixed/progressive MixUp and dual bidirectional CutMix; 2) Prototypical Alignment and Pseudo Label Correction (PAPLC) using prototype cosine similarity classifiers for feature alignment and pseudo label refinement.

Result: Superior performance on three public multi-domain datasets, particularly effective with very limited labeled samples.

Conclusion: BCMDA effectively addresses domain shift and confirmation bias in semi-supervised medical segmentation, demonstrating strong performance with limited annotations.
...
TL;DR: LLaVA-LE is a specialized vision-language model for lunar exploration, trained on a new lunar dataset (LUCID) and achieving significant performance gains over base models through domain-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Multimodal vision-language models have advanced but remain unexplored for planetary science due to lack of large-scale datasets pairing real planetary imagery with scientific descriptions.

Method: Created LUCID dataset (96k lunar images with captions + 81k QA pairs), then fine-tuned LLaVA using two-stage training: concept alignment for terrain description and instruction-tuned visual QA.

Result: LLaVA-LE achieves 3.3x overall performance gain over Base LLaVA and 2.1x over Stage 1 model, with reasoning score of 1.070 exceeding judge's reference score.

Conclusion: Domain-specific multimodal data and instruction tuning effectively advance VLMs for planetary exploration, enabling specialized lunar terrain analysis capabilities.
...
TL;DR: OpenCap Monocular is a smartphone-based algorithm that estimates 3D skeletal kinematics and kinetics from single videos, enabling accessible biomechanical assessment without specialized lab equipment.


<details>
  <summary>Details</summary>
Motivation: Traditional biomechanical analysis requires expensive, time-consuming lab equipment, limiting clinical translation. There's a need for scalable, accurate tools for quantifying human movement and musculoskeletal forces.

Method: The method refines 3D human pose estimates from monocular pose estimation (WHAM) via optimization, computes kinematics using a biomechanically constrained skeletal model, and estimates kinetics through physics-based simulation and machine learning.

Result: OpenCap Monocular achieved low kinematic error (4.8° mean absolute error for rotations; 3.4 cm for pelvis translations), outperforming regression-only baselines by 48-69%. It estimated ground reaction forces with accuracy comparable to prior two-camera systems and demonstrated clinically meaningful accuracy for knee-related metrics.

Conclusion: OpenCap Monocular enables free, accessible single-smartphone biomechanical assessments via deployed smartphone/web apps and cloud computing, potentially transforming prediction, treatment, and monitoring of mobility-related conditions.
...
TL;DR: Lookalike3D: A multiview image transformer for detecting identical and similar object pairs in indoor scenes, improving 3D perception tasks.


<details>
  <summary>Details</summary>
Motivation: Current 3D object understanding methods overlook repeated objects in real-world scenes, which provide valuable complementary cues for better scene understanding.

Method: Lookalike3D multiview image transformer that uses semantic priors from large image foundation models to classify object pairs as identical, similar, or different. Introduces 3DTwins dataset with 76k annotated object pairs from ScanNet++.

Result: 104% IoU improvement over baselines on the 3DTwins dataset. Enables improved joint 3D object reconstruction and part co-segmentation.

Conclusion: Repeated and lookalike objects serve as powerful cues for consistent, high-quality 3D perception, with Lookalike3D effectively leveraging these patterns for improved scene understanding.
...
### [9] [Image Rotation Angle Estimation: Comparing Circular-Aware Methods](https://arxiv.org/abs/2603.25351)
*Maximilian Woehrer*

Main category: cs.CV

TL;DR: Comprehensive study of 5 circular-aware methods for image rotation estimation shows probabilistic approaches (circular Gaussian) are most robust across architectures, while classification achieves best accuracy on matched backbones but has training instabilities.
...
Method: Systematic evaluation of 5 circular-aware methods: direct angle regression with circular loss, classification via angular binning, unit-vector regression, phase-shifting coder, and circular Gaussian distribution. Used transfer learning from ImageNet-pretrained models across 16 modern architectures with adapted output heads for rotation prediction.

Result: Circular Gaussian distribution is most robust across architectures. Classification achieves best accuracy on well-matched backbones (1.23° MAE with EfficientViT-B3 on DRC-D) but suffers training instabilities. Circular Gaussian with MambaOut Base achieves similar accuracy (1.24°) with better robustness. On COCO 2014, best configuration reaches 3.71° MAE, improving over prior work, and 2.84° on larger COCO 2017.

Conclusion: Probabilistic methods like circular Gaussian distribution provide the best robustness across different architectures for rotation estimation, while classification methods can achieve highest accuracy when properly matched with appropriate backbones but require careful architecture selection due to training stability issues.
...
TL;DR: 3DGS enables accurate 3D point measurements through intuitive multi-view point picking and triangulation, outperforming traditional mesh-based methods without requiring stereoscopic hardware.


<details>
  <summary>Details</summary>
Motivation: While 3D Gaussian Splatting excels at novel view synthesis, its potential for precise geometric measurement remains underexplored. Current measurement methods rely on expensive stereoscopic workstations or inaccurate/incomplete 3D meshes.

Method: Users pick congruent points across different rendered views from 3DGS models, then triangulate these points to generate precise 3D measurements. This approach supports multi-view intersection (more than two views) for higher accuracy.

Result: The method achieves RMSEs of 1-2 cm on well-defined points, significantly outperforming mesh-based measurements. On challenging thin structures (mesh RMSE: 0.062m), it achieved 0.037m. On sharp corners where mesh methods failed, it achieved 0.013m RMSE.

Conclusion: 3DGS enables highly accurate point measurements without specialized stereoscopic hardware, matching or exceeding traditional methods while being more accessible and intuitive for users.
...
### [11] [Challenges in Hyperspectral Imaging for Autonomous Driving: The HSI-Drive Case](https://arxiv.org/abs/2603.25510)
*Koldo Basterretxea,Jon Gutiérrez-Zaballa,Javier Echanobe*

Main category: cs.CV

TL;DR: Analysis of hyperspectral imaging techniques for autonomous driving, focusing on challenges like variable lighting, depth-of-field, dynamic scenes, and real-time requirements, with evaluation using HSI-Drive dataset.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral imaging shows promise for autonomous driving but faces domain-specific challenges including variable lighting conditions, wide depth-of-field ranges, dynamic scenes with fast-moving objects, real-time operation requirements, and limited computational resources on embedded platforms.

Method: Analysis of several HSI-based vision system techniques for autonomous driving, using experimental results from the most recent version of the HSI-Drive dataset to evaluate different approaches.

Result: The paper presents analysis results from experiments using HSI-Drive dataset, examining how different HSI techniques perform under autonomous driving conditions, though specific quantitative results are not detailed in the abstract.

Conclusion: Both HSI technology selection criteria and custom vision algorithm development are crucial for successful AD applications, requiring consideration of spectral and spatial information processing under challenging real-world conditions.
...
TL;DR: QuatRoPE is a novel positional embedding method for spatial reasoning in 3D scenes that encodes pairwise spatial relations linearly rather than quadratically, maintaining geometric integrity while preserving LLM capabilities.


<details>
  <summary>Details</summary>
Motivation: Spatial reasoning is crucial for intelligent embodied agents, but limited 3D scene-language data makes training from scratch difficult. Existing methods either struggle with extracting spatial relations from absolute positions or suffer from poor scalability due to quadratic encoding of all pairwise relations.

Method: Proposes QuatRoPE (Quaternion Rotary Positional Embedding) with linear input length relative to object count, explicitly calculating pairwise spatial relations through dot products in attention layers. Also introduces IGRE (Isolated Gated RoPE Extension) to limit QuatRoPE's influence to object-related tokens, preserving LLM's original positional embeddings.
...
Conclusion: QuatRoPE addresses scalability limitations of previous spatial reasoning approaches by providing linear encoding of spatial relations while maintaining geometric fidelity and preserving LLM capabilities through isolated gating.
...
TL;DR: Landmark-based gaze estimation methods using lightweight models show comparable cross-domain generalization to deep CNNs, offering efficient, interpretable alternatives for edge applications.


<details>
  <summary>Details</summary>
Motivation: Deep CNNs for gaze estimation are computationally expensive and lack interpretability, while geometric methods based on facial landmarks offer lightweight alternatives but their performance limits and generalization capabilities remain underexplored.

Method: Standardized pipeline to extract and normalize facial landmarks from three large-scale datasets (Gaze360, ETH-XGaze, GazeGene), then train lightweight regression models: Extreme Gradient Boosted trees, holistic MLP, and siamese MLP designed to capture binocular geometry.

Result: Landmark-based models show lower within-domain performance due to landmark detector noise, but in cross-domain evaluation, the proposed MLP architectures achieve generalization capabilities comparable to ResNet18 baselines.
...
TL;DR: 3DGSConfidence: A self-supervised confidence framework for 3D Gaussian Splatting that improves mesh extraction in scenes with view-dependent effects while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods struggle with mesh extraction in scenes containing abundant view-dependent effects, leading to ambiguities. Existing solutions sacrifice the inherent efficiency of 3DGS by relying on multi-view techniques, iterative extraction, or large pre-trained models.

Method: Introduces a self-supervised confidence framework where learnable confidence values dynamically balance photometric and geometric supervision. Adds losses penalizing per-primitive color and normal variance, and improves the appearance model by decoupling individual terms of the D-SSIM loss.

Result: Achieves state-of-the-art results for unbounded mesh extraction while remaining highly efficient, outperforming previous methods that sacrificed efficiency for accuracy.

Conclusion: The proposed confidence framework provides a simple and efficient alternative to complex methods, successfully resolving ambiguities in scenes with view-dependent effects while preserving the speed advantages of 3D Gaussian Splatting.
...
TL;DR: Researchers use ambiguous duck-rabbit style images to compare where humans and AI models draw semantic boundaries, finding machines are more biased toward "rabbit" while humans align better with CLIP embeddings.


<details>
  <summary>Details</summary>
Motivation: To understand how human observers and machine classifiers draw semantic boundaries between ambiguous concepts, using interpretability probes to expose how vision models represent concept boundaries and bridge gaps between human psychophysics, image classification, and generative models.

Method: Developed a psychophysically-informed framework that interpolates between concepts in CLIP embedding space to generate continuous spectra of ambiguous images, allowing precise measurement of semantic boundary placement by humans and machine classifiers.

Result: Machine classifiers show stronger bias toward "rabbit" interpretations, while humans align more closely with the CLIP embeddings used for image synthesis. Guidance scale affects human sensitivity more strongly than machine classifiers.

Conclusion: Controlled ambiguity serves as a valuable diagnostic tool for bridging human psychophysical analysis, image classification, and generative models, offering insights into human-model alignment, robustness, interpretability, and image synthesis methods.
...
TL;DR: TIGeR is a multi-modal transformer model that creates unified geo-temporal embeddings for images, enabling joint reasoning about visual appearance, geolocation, and time for applications like digital forensics and urban monitoring.


<details>
  <summary>Details</summary>
Motivation: Real-world applications in digital forensics, urban monitoring, and environmental analysis require complex capabilities beyond standard geo-localization and time prediction, such as retrieving images from the same location at specified target times.

Method: Proposes TIGeR, a multi-modal-transformer-based model that maps image, geolocation, and time into a unified geo-temporal embedding space, supporting flexible input configurations and preserving location identity under appearance changes.

Result: TIGeR outperforms strong baselines by up to 16% on time-of-year prediction, 8% on time-of-day prediction, and 14% in geo-time aware retrieval recall, demonstrating benefits of unified geo-temporal modeling.

Conclusion: The proposed unified geo-temporal modeling approach enables retrieval based on where and when a scene is, rather than purely on visual similarity, with significant performance improvements across multiple tasks.
...
Method: The paper reviews and compares existing CMRI generation methods including GANs, VAEs, diffusion probabilistic models, and flow-matching techniques. It examines mask-conditioned generation for structural fidelity, vendor-style conditioning for cross-domain generalization, and privacy mechanisms like membership inference attacks and differential privacy.

Result: The review shows that anatomically constrained synthetic data can enhance segmentation accuracy and robustness across multi-vendor settings. Diffusion and flow-matching models offer strong boundary preservation, while privacy protection methods are increasingly being incorporated.

Conclusion: There is a need for integrated, evaluation-driven frameworks that balance fidelity, utility, and privacy for reliable clinical workflows in synthetic cardiac MRI generation.
...
TL;DR: DRoPS: Dynamic scene reconstruction using static pre-scans as geometric priors with grid-structured Gaussian primitives and CNN-based motion parameterization for better novel view synthesis and 3D tracking.


<details>
  <summary>Details</summary>
Motivation: Existing methods for dynamic scene reconstruction from casual videos struggle with extreme novel viewpoints and highly articulated motions, as they rely on 2D foundational model priors and hand-crafted motion regularization that don't fully exploit available geometric information.

Method: Uses static pre-scan of dynamic object as explicit geometric prior. Organizes Gaussian primitives into pixel grids anchored to object surface (grid-structured surface-aligned model). Parameterizes motion using CNN conditioned on these grids for implicit regularization and motion correlation.

Result: Significantly outperforms current state-of-the-art in both rendering quality and 3D tracking accuracy, demonstrating better handling of extreme novel viewpoints and articulated motions.

Conclusion: DRoPS effectively leverages static pre-scans to constrain solution space and ensure geometric consistency, with its grid-structured representation and CNN-based motion parameterization enabling superior dynamic scene reconstruction from casual videos.
...
TL;DR: AVControl is a lightweight framework for controlling video and audio generation using separate LoRA adapters for each control modality, built on the LTX-2 foundation model without architectural changes.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for controlling video/audio generation either require monolithic models for fixed controls or need costly architectural modifications for each new modality. There's a need for a flexible, efficient framework that can handle diverse control modalities without extensive retraining or architectural changes.
...
Result: Outperforms all evaluated baselines on VACE Benchmark for depth- and pose-guided generation, inpainting, and outpainting. Shows competitive results on camera control and audio-visual benchmarks. Supports diverse independently trained modalities including spatial controls, camera trajectory, motion control, video editing, and modular audio-visual controls.

Conclusion: AVControl provides a compute- and data-efficient framework for modular control of video and audio generation, requiring only small datasets and minimal training steps per modality compared to monolithic alternatives. The method enables flexible extension to new control modalities without architectural changes.
...
TL;DR: Calibri is a lightweight calibration method that improves Diffusion Transformer performance by optimizing ~100 parameters using evolutionary algorithms, enhancing generative quality while reducing inference steps.


<details>
  <summary>Details</summary>
Motivation: The paper aims to unlock the hidden potential of Diffusion Transformers (DiTs) by addressing suboptimal performance in existing DiT blocks through better calibration of their components.

Method: Proposes Calibri, which frames DiT calibration as a black-box reward optimization problem solved with evolutionary algorithms, introducing a single learned scaling parameter and modifying only ~100 parameters.

Result: Calibri consistently improves performance across various text-to-image models, reduces required inference steps for image generation, and maintains high-quality outputs despite its lightweight design.

Conclusion: Calibri demonstrates that minimal parameter calibration can significantly enhance Diffusion Transformer performance, offering an efficient way to improve generative models without extensive retraining.
...
<details>
  <summary>Details</summary>
Motivation: Current AAA segmentation models often fail because they focus on irrelevant structures or miss thin, low-contrast targets. The primary training signal (where the model looks) needs better guidance.

Method: Proposes an XAI-guided encoder shaping framework that: 1) computes dense attribution-based encoder focus maps (XAI fields) from final encoder block, 2) aligns predicted probability mass to XAI fields to promote focus-output agreement, and 3) routes XAI fields into lightweight refinement pathways and confidence priors that modulate logits at inference to suppress distractors while preserving subtle structures.

Result: Substantial improvements over base SAM setup on clinically validated challenging cases curated for failure-prone scenarios. The framework demonstrates practical effectiveness for reliable segmentation in complex scenarios.

Conclusion: Explicitly optimizing encoder focus via XAI guidance is a practical and effective principle for reliable segmentation in complex scenarios like abdominal aortic aneurysms.
...
TL;DR: GoldiCLIP is a data-efficient vision-language model that achieves state-of-the-art performance using only 30M images (300x less data) through balanced supervision signals and three key innovations.


<details>
  <summary>Details</summary>
Motivation: Large-scale VLMs require billion-sample datasets, creating barriers to progress. Recent works improve supervision quality but only address subsets of contrastive pretraining weaknesses. Need a balanced approach to supervision signals.

Method: Three key innovations: (1) text-conditioned self-distillation for aligning text-agnostic and text-conditioned features; (2) encoder-integrated decoder with VQA objective for generalization beyond caption-like queries; (3) uncertainty-based weighting mechanism to balance heterogeneous losses.

Result: Achieves SOTA among data-efficient approaches: +2.2 points on MSCOCO retrieval, +2.0 on fine-grained retrieval, +5.9 on question-based retrieval. Remains competitive with billion-scale models despite using 300x less data.

Conclusion: GoldiCLIP demonstrates that balanced supervision signals (Goldilocks principle) enable high-performance VLMs with dramatically reduced data requirements, breaking the billion-sample barrier for vision-language pretraining.
...
TL;DR: A deep learning model using attention mechanisms and Efficient Redundant Reconstruction Convolution achieves high accuracy (AUC 0.975) in classifying pin site infections from images, outperforming baseline methods with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: Pin site infections are common, painful complications in patients with external fixators that increase morbidity. Current identification and management of these infections could be improved to enhance patient experience. There's a need for automated visual assessment tools that can differentiate infected from non-infected pin sites.

Method: The paper collects a dataset of pin site wound infections and proposes a deep learning approach with two key innovations: 1) An attention-based model that focuses on relevant regions while minimizing distractions from the pins themselves, and 2) An Efficient Redundant Reconstruction Convolution (ERRC) method to enhance feature map richness while reducing parameters. The model classifies images into Group A (signs of inflammation/infection) vs Group B (no evident complications).

Result: The proposed model achieves excellent performance with an AUC of 0.975 and F1-score of 0.927, requiring only 5.77 million parameters. It outperforms baseline methods and demonstrates the potential of deep learning to differentiate pin site infections based on visual signs, aligning with healthcare professional assessments.

Conclusion: Deep learning shows strong potential for automated classification of pin site infections from visual appearance alone, which could improve patient care when external fixators are used. The attention-based approach with ERRC effectively handles the complexity of pin site images. While validation with more data is needed, the results suggest this technology could enhance clinical decision-making for pin site infection management.
...
<details>
  <summary>Details</summary>
Motivation: Existing adversarial attacks in crowd counting focus on transferability within density-map-based models, but cross-paradigm attacks (between density map and point regression approaches) remain unexplored. Given the security implications of crowd counting systems, understanding and demonstrating vulnerabilities across different architectural paradigms is crucial.
...
Result: Achieves 7X increase in Mean Absolute Error compared to clean images, successfully transfers across seven state-of-the-art crowd models with transfer ratios ranging from 0.55 to 1.69, and maintains competitive visual quality while balancing attack effectiveness and imperceptibility.

Conclusion: The proposed framework demonstrates effective cross-paradigm adversarial attacks in crowd counting, revealing vulnerabilities across different architectural approaches and highlighting the need for robust defenses that consider both density-map and point-regression paradigms.
...
TL;DR: DCARL: A divide-and-conquer autoregressive framework for long-trajectory video generation that combines structural stability with high-fidelity generation using keyframes and interpolation.


<details>
  <summary>Details</summary>
Motivation: Long-trajectory video generation is challenging due to scalability limitations of existing video diffusion models. Autoregressive models suffer from visual drift and poor controllability, while current approaches struggle with maintaining consistency over long durations.

Method: Two-stage approach: 1) Keyframe Generator trained without temporal compression creates globally consistent structural anchors, 2) Interpolation Generator synthesizes dense frames autoregressively with overlapping segments, using keyframes for global context and clean preceding frames for local coherence.

Result: Superior performance in visual quality (lower FID and FVD) and camera adherence (lower ATE and ARE) compared to state-of-the-art baselines. Achieves stable, high-fidelity generation for videos up to 32 seconds long.

Conclusion: DCARL effectively addresses visual drift and controllability issues in long-trajectory video generation by combining divide-and-conquer structural stability with autoregressive high-fidelity generation, enabling scalable world modeling.
...
<details>
  <summary>Details</summary>
Motivation: The paper challenges the necessity of cost volumes in stereo matching, which are commonly used in leading methods but can be computationally expensive. The authors aim to demonstrate that warping-based approaches can achieve better performance with improved efficiency.

Method: WAFT-Stereo uses a warping-based approach instead of traditional cost volumes for stereo matching. The method replaces the computationally expensive cost volume construction with more efficient warping operations.
...
Conclusion: Cost volumes are not necessary for strong stereo matching performance and can be effectively replaced by warping-based methods, which offer both superior accuracy and significant efficiency improvements.
...
TL;DR: Benchmark study of 20 multimodal LLMs for neuroimaging shows technical attributes (modality, plane) are nearly solved, but diagnostic reasoning remains challenging, with tumor classification most reliable and MS/rare abnormalities difficult. Gemini-2.5-Pro/GPT-5-Chat lead in performance, Gemini-2.5-Flash offers best efficiency trade-off, and open-weight MedGemma-1.5-4B shows promise approaching proprietary models.
...
Method: Comprehensive benchmarking of 20 frontier multimodal models using curated MRI/CT datasets covering MS, stroke, brain tumors, other abnormalities, and normal controls. Models generate multiple outputs simultaneously (diagnosis, subtype, modality, sequence, plane). Evaluated across four dimensions: discriminative classification with abstention, calibration, structured-output validity, and computational efficiency using multi-phase framework to control selection bias.

Result: Technical imaging attributes (modality, plane) nearly solved; diagnostic reasoning, especially subtype prediction, remains challenging. Tumor classification most reliable, stroke moderately solvable, MS and rare abnormalities difficult. Few-shot prompting improves performance but increases token usage, latency, and cost. Gemini-2.5-Pro and GPT-5-Chat achieve strongest overall diagnostic performance; Gemini-2.5-Flash offers best efficiency-performance trade-off. Open-weight MedGemma-1.5-4B approaches proprietary model performance under few-shot prompting while maintaining perfect structured output.

Conclusion: The study provides practical insights into performance, reliability, and efficiency trade-offs of multimodal LLMs in neuroimaging, supporting standardized evaluation and highlighting that while technical attributes are mature, diagnostic reasoning remains an area needing improvement, with specific models identified for different clinical use cases.
...
TL;DR: CORA is a 3D vision foundation model for cardiovascular risk assessment that learns from unlabeled CCTA scans using pathology-centric self-supervision, outperforming state-of-the-art models on diagnostic tasks and improving cardiac event risk prediction.


<details>
  <summary>Details</summary>
Motivation: Clinical translation of automated CCTA analysis is limited by scarce expert-annotated datasets, and existing label-free pretraining methods fail to capture localized pathological features of coronary plaques that are clinically relevant.

Method: CORA uses a pathology-centric, synthesis-driven self-supervised framework with an anatomy-guided lesion synthesis engine that trains the model to detect simulated vascular abnormalities, biasing learning toward disease features rather than background anatomy. Trained on 12,801 unlabeled CCTA volumes.

Result: CORA consistently outperformed state-of-the-art 3D vision foundation models across diagnostic and anatomical tasks (plaque characterization, stenosis detection, coronary artery segmentation) with up to 29% performance gain. When coupled with a large language model, it significantly improved 30-day major adverse cardiac event (MACE) risk stratification.

Conclusion: CORA establishes a scalable and extensible foundation for unified anatomical assessment and cardiovascular risk prediction, addressing key limitations in current automated CCTA analysis approaches.
...
TL;DR: This paper presents a smoke detector recognition system for automatic inspection using drones, comparing YOLOv11, SSD, and RT-DETRv2 object detectors with various training strategies on real and semi-synthetic data.


<details>
  <summary>Details</summary>
Motivation: Smoke detectors are often in hard-to-reach locations (high ceilings, problematic spots), making manual inspections difficult, dangerous, and expensive. An automatic inspection system using drones could provide faster, safer, and cheaper smoke detector maintenance.

Method: The study compares three object detection architectures: YOLOv11, SSD, and RT-DETRv2 with different backbone sizes. It explores various training strategies using real and semi-synthetic data with different augmentation methods. Models are evaluated on two test datasets containing challenging conditions like motion blur, low resolution, and partial objects.

Result: YOLOv11n achieved the best performance with an average mAP@0.5 score of 0.884. The paper includes public release of code, pretrained models, and dataset.

Conclusion: The smoke detector recognition system using YOLOv11n demonstrates effective performance for drone-based automatic inspection, addressing the challenges of real-world smoke detector maintenance through robust object detection and strategic training approaches.
...
TL;DR: Proposes OptiSAR-Net++ for cross-domain remote sensing visual grounding, introducing first large-scale benchmark dataset OptSAR-RSVG and achieving SOTA performance with efficient cross-modal matching.


<details>
  <summary>Details</summary>
Motivation: Existing remote sensing visual grounding methods are limited to single-sensor domains (optical or SAR only), restricting real-world applicability where cross-domain capability is needed.

Method: OptiSAR-Net++ framework with: 1) PL-MoE for efficient cross-domain feature decoupling, 2) CLIP-based contrastive paradigm with dynamic adversarial negative sampling for efficient cross-modal matching, 3) TGDF-SSA for semantic-visual alignment, and 4) region-aware auxiliary head for spatial modeling.

Result: Achieves state-of-the-art performance on both OptSAR-RSVG (new benchmark) and DIOR-RSVG benchmarks, with significant advantages in localization accuracy and computational efficiency.

Conclusion: Proposes first cross-domain RSVG task and dataset, introduces efficient framework that transforms generative regression into cross-modal matching, enabling practical cross-sensor remote sensing applications.
...
Conclusion: The comprehensive framework effectively addresses surgical phase recognition challenges through integrated self-supervised learning, temporal modeling, and a collaborative platform that enables continuous model improvement and knowledge sharing in surgical education.
...
TL;DR: SSL shows mixed results for knee OA diagnosis but significantly improves prognosis prediction, with multimodal SSL on uncurated hospital data performing best for prognosis despite severity bias limiting diagnostic performance.


<details>
  <summary>Details</summary>
Motivation: To determine whether self-supervised learning (SSL) improves knee osteoarthritis modeling for diagnosis and prognosis compared to standard ImageNet-pretrained initialization, particularly examining different SSL approaches and their alignment with downstream tasks.
...
Result: For diagnosis: Image-only SSL improved accuracy during linear probing but didn't outperform ImageNet during full fine-tuning. Multimodal SSL failed to improve grading due to severe bias in pretraining corpus (93% KL grade 3). For prognosis: Multimodal SSL significantly outperformed ImageNet baselines in predicting 4-year structural incidence/progression, including external validation (MOST AUROC: 0.701 vs. 0.599 at 10% labeled data).

Conclusion: Uncurated hospital image-text data may be ineffective for learning diagnosis due to severity bias, but provides strong signal for prognostic modeling when downstream task aligns with pretraining data distribution. SSL shows promise for prognosis but requires careful consideration of task alignment.
...
<details>
  <summary>Details</summary>
Motivation: Existing inverse rendering approaches rely on synthetic datasets with simplified illumination and limited material realism, which prevents generalization to real-world images due to scarcity of real measured reflectance data.
...
Result: Dataset yields over 1.2M high-resolution images with diffuse-specular separation and analytically derived diffuse albedo, specular albedo, and surface normals. Training on this dataset shows significant improvements in material separation, illumination fidelity, and geometric consistency for inverse/forward rendering tasks.

Conclusion: This work establishes a new foundation for physically grounded material understanding and enables real-world generalization beyond synthetic training regimes for inverse rendering applications.
...
TL;DR: TIGFlow-GRPO: A two-stage flow-based trajectory forecasting framework that first models spatio-temporal interactions via CFM with TIG module, then aligns trajectories with behavioral rules using Flow-GRPO post-training with SDE sampling and composite rewards.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory forecasting methods focus primarily on supervised fitting, which may insufficiently reflect social norms and scene constraints in generated trajectories. There's a need to better align trajectory generation with behavioral rules for more realistic and compliant predictions in complex environments.

Method: Two-stage framework: 1) CFM-based predictor with Trajectory-Interaction-Graph (TIG) module to model agent-agent and agent-scene relations; 2) Flow-GRPO post-training that reformulates deterministic flow rollout as stochastic ODE-to-SDE sampling for exploration, using composite rewards combining social compliance and physical feasibility to steer predictions.
...
Conclusion: The proposed framework effectively connects flow-based trajectory modeling with behavior-aware alignment, providing a promising approach for generating realistic and rule-compliant trajectories in dynamic multimedia environments.
...
TL;DR: A generative framework using autoregressive diffusion models to predict infinite-horizon raw gaze trajectories in videos, capturing long-range behavioral dependencies and fine-grained temporal dynamics.


<details>
  <summary>Details</summary>
Motivation: Traditional gaze prediction methods (saliency maps and scanpaths) collapse fine-grained temporal dynamics and are limited to short-term windows, failing to capture long-range behavioral dependencies in real-world video content.

Method: Uses an autoregressive diffusion model to synthesize gaze trajectories with continuous spatial coordinates and high-resolution timestamps, conditioned on a saliency-aware visual latent space for videos of arbitrary length.

Result: Significantly outperforms existing approaches in long-range spatio-temporal accuracy and trajectory realism, as demonstrated through quantitative and qualitative evaluations.

Conclusion: The proposed generative framework enables infinite-horizon raw gaze prediction that captures both fine-grained temporal dynamics and long-range behavioral dependencies in video content.
...
TL;DR: TIES is a dynamic token selection framework for VLA models that improves policy performance while reducing visual tokens by 78%, addressing the limitations of static attention-based reduction methods.


<details>
  <summary>Details</summary>
Motivation: Current VLA models for robotics suffer from high inference latency due to processing dense visual tokens. Existing token reduction methods use static attention magnitude selection, but this approach is flawed because high-attention tokens are task-dependent and can actually degrade policy performance.

Method: TIES (Tau-guided Inter-layer Efficient Selection) is a dynamic framework that uses inter-layer token ranking consistency to guide token selection. It adaptively balances attention magnitude with ranking consistency to ensure robust token selection without requiring additional training.

Result: On the CogACT + SIMPLER benchmark, TIES improves average success rates by 6% while reducing token usage by 78%. The method demonstrates strong generalization across diverse decoders and benchmarks.

Conclusion: TIES provides an effective solution to the inference latency problem in VLA models by introducing dynamic token selection based on inter-layer consistency, outperforming static attention-based methods while significantly reducing computational overhead.
...
TL;DR: BiFM is a unified framework that jointly learns both generation and inversion in diffusion/flow matching models, enabling better few-step editing by estimating bidirectional velocity fields with continuous time-interval supervision.


<details>
  <summary>Details</summary>
Motivation: Existing few-step inversion methods have poor forward process approximation leading to degraded editing quality, and they often rely on pretrained generators and auxiliary modules which limit scalability and generalization across architectures.
...
Result: BiFM consistently outperforms existing few-step approaches across diverse image editing and generation tasks, achieving superior performance and editability while enabling one-step inversion and seamless integration into popular diffusion/flow matching backbones.

Conclusion: BiFM provides a unified bidirectional framework that addresses limitations of existing few-step inversion methods, offering better scalability, generalization, and editing quality through joint learning of generation and inversion processes.
...
TL;DR: A framework for verifying neuron concept interpretations by selecting well-defined neurons, hypothesizing concepts, and verifying activation patterns to avoid misleading interpretations.


<details>
  <summary>Details</summary>
Motivation: Existing neuron interpretation methods assume all neurons have well-defined functions and generate natural language descriptions, but some neurons are redundant or provide misleading concepts, leading to misinterpretations of neural network decisions.

Method: Proposes a Select-Hypothesize-Verify framework: 1) Select activation samples capturing well-defined functional behavior through activation-distribution analysis, 2) Form hypotheses about concepts for selected neurons, 3) Verify whether generated concepts accurately reflect neuron functionality by checking if concepts highly activate corresponding neurons.
...
Conclusion: The verification-based framework improves neuron concept interpretation by filtering out misleading neurons and ensuring generated concepts accurately reflect actual neuron functionality, leading to better understanding of neural network decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image generation struggles with complex prompts requiring fine-grained semantics and spatial relations due to the feed-forward nature of generation. While evaluating generated images is easier, current methods lack mechanisms for self-correction during generation.
...
Conclusion: The proposed self-correcting framework effectively addresses alignment challenges in text-to-image generation by leveraging the asymmetry between generation difficulty and evaluation tractability, enabling models to guide their own generation process through explainable latent rewards.
...
TL;DR: PASDiff is a physics-aware semantic diffusion model for low-light face restoration that uses inverse intensity weighting, Retinex theory, and style-agnostic structural injection to recover natural illumination, color, and facial details without training.
...
Method: PASDiff uses a physics-aware semantic diffusion approach with training-free manner. It employs: 1) Inverse intensity weighting and Retinex theory for photometric constraints to recover visibility and natural chromaticity, 2) Style-Agnostic Structural Injection (SASI) that extracts structures from off-the-shelf facial priors while filtering out photometric biases to harmonize identity features with physical constraints.

Result: The method significantly outperforms existing approaches, achieving superior balance among natural illumination, color recovery, and identity consistency. The authors also constructed WildDark-Face, a real-world benchmark of 700 low-light facial images with complex degradations for evaluation.

Conclusion: PASDiff effectively addresses the limitations of existing low-light face restoration methods by combining physics-aware constraints with semantic facial priors, demonstrating state-of-the-art performance in recovering natural illumination, color, and facial identity from severely degraded low-light images.
...
### [41] [LaMP: Learning Vision-Language-Action Policies with 3D Scene Flow as Latent Motion Prior](https://arxiv.org/abs/2603.25399)
*Xinkai Wang,Chenyi Wang,Yifu Xu,Mingzhe Ye,Fu-Cheng Zhang,Jialin Tian,Xinyu Zhan,Lifeng Zhu,Cewu Lu,Lixin Yang*

Main category: cs.CV

TL;DR: LaMP introduces a dual-expert VLA framework that uses 3D scene flow as latent motion prior for robotic manipulation, improving performance and robustness over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models directly regress actions from 2D semantic visual features, forcing them to learn complex 3D physical interactions implicitly, which degrades under unfamiliar spatial dynamics.

Method: LaMP uses a dual-expert framework: a flow-matching Motion Expert generates partially denoised 3D scene flow, and a policy-predicting Action Expert is conditioned on these hidden states through gated cross-attention, without full multi-step reconstruction.

Result: LaMP outperforms VLA baselines on LIBERO, LIBERO-Plus, and SimplerEnv-WidowX benchmarks, achieving highest average success rates. On LIBERO-Plus OOD perturbations, it shows 9.7% average gain over strongest prior baseline.

Conclusion: Embedding 3D scene flow as latent motion prior significantly improves VLA model performance and robustness for robotic manipulation tasks, demonstrating the value of explicit motion representation learning.
...
TL;DR: MoE-GRPO: A reinforcement learning framework that optimizes expert routing in Vision-Language MoE models to improve diversity and prevent expert overfitting.


<details>
  <summary>Details</summary>
Motivation: Standard deterministic top-K routing in MoE-based Vision-Language Models may overlook optimal expert combinations and cause expert overfitting, limiting model performance and diversity.

Method: Formulates expert selection as sequential decision-making and optimizes using Group Relative Policy Optimization (GRPO) with modality-aware router guidance to enhance training stability and efficiency.

Result: Outperforms standard top-K routing and variants on multi-modal image and video benchmarks by promoting diverse expert selection, mitigating expert overfitting, and enabling task-level expert specialization.

Conclusion: MoE-GRPO provides an effective RL-based framework for adaptive expert routing in MoE-based VLMs, improving performance through more diverse and specialized expert selection.
...
TL;DR: MAML framework for K-shot 3D left atrial wall segmentation using auxiliary tasks and boundary-aware loss improves performance in low-shot scenarios and shows robustness to domain shifts.


<details>
  <summary>Details</summary>
Motivation: Segmenting left atrial wall from LGE-MRI is challenging due to thin geometry, low contrast, and scarcity of expert annotations, requiring methods that work with limited labeled data.
...
Result: MAML outperformed supervised fine-tuning: 0.64 vs 0.52 DSC at 5-shot, approached fully supervised reference at 20-shot (0.69 vs 0.71 DSC). Showed robustness to unseen domain shifts and local cohort data with consistent gains as K increased.

Conclusion: MAML enables more accurate thin-wall segmentation in low-shot adaptation, potentially enabling clinical translation with minimal additional labeling for atrial remodeling assessment.
...
TL;DR: Vega is a vision-language-world-action model for personalized autonomous driving that follows diverse user instructions, using autoregressive processing for vision/language and diffusion for world modeling/trajectory generation.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language-action models for autonomous driving only use language for scene description/reasoning and lack flexibility to follow diverse user instructions for personalized driving experiences.

Method: Constructed InstructScene dataset (~100k scenes with diverse driving instructions and trajectories). Proposed Vega model with: 1) autoregressive paradigm for visual inputs and language instructions, 2) diffusion paradigm for future predictions (world modeling) and trajectory generation, 3) joint attention for modality interaction, 4) individual projection layers for different modalities.

Result: Achieves superior planning performance and strong instruction-following abilities, demonstrating effectiveness for personalized driving systems.

Conclusion: Vega paves the way for more intelligent and personalized driving systems by enabling instruction-based generation and planning through unified vision-language-world-action modeling.
...
TL;DR: EWAD is a novel event-based video anomaly detection framework that introduces event density aware sampling, density-modulated temporal modeling, and RGB-to-event knowledge distillation to effectively leverage sparse event streams for anomaly detection.


<details>
  <summary>Details</summary>
Motivation: Event-based vision has natural advantages for video anomaly detection (low redundancy, focus on dynamics, privacy-preserving), but lacks dedicated datasets and effective modeling strategies, hindering progress in event-based VAD.

Method: 1) Constructs synchronized event-RGB VAD benchmarks; 2) Event density aware dynamic sampling to select informative segments; 3) Density-modulated temporal modeling for sparse event streams; 4) RGB-to-event knowledge distillation under weak supervision.
...
Conclusion: The work establishes event-based VAD as a unified research direction, showing strong potential for event-driven anomaly detection with the proposed framework and publicly available benchmarks.
...
Result: Substantial improvements over baseline: wall Dice improved from 0.623 to 0.814, Surface Dice at 1mm improved from 0.553 to 0.731, HD95 decreased from 2.95mm to 2.55mm, ASSD decreased from 0.71mm to 0.63mm. Even with reduced supervision (70 training volumes), achieved Dice of 0.78 and HD95 of 3.15mm, exceeding multi-class benchmarks.

Conclusion: Anatomically grounded task transfer with controlled fine-tuning improves boundary accuracy for thin LA wall segmentation in 3D LGE-MRI, demonstrating the effectiveness of leveraging cavity segmentation as an anatomical prior.
...
TL;DR: A novel method for dynamic 3D scene reconstruction using 3D Gaussian Splatting that preserves local geometric structure across time through view-space ray grouping, eliminating the need for external priors like optical flow.


<details>
  <summary>Details</summary>
Motivation: Current dynamic 3D reconstruction methods using Gaussian Splatting fail to model realistic motion that aligns with physical dynamics, especially for monocular video datasets. This misalignment undermines local geometric structure and reconstruction quality, forcing reliance on external priors like optical flow or 2D tracks.
...
Result: Extensive experiments on challenging monocular datasets show the method significantly outperforms existing approaches, achieving superior temporal consistency and reconstruction quality when integrated into two distinct baseline models.

Conclusion: The proposed ray grouping approach effectively preserves local geometric structure in 4D scenes, enabling physically plausible motion modeling without relying on external priors, leading to improved dynamic 3D reconstruction from monocular videos.
...
TL;DR: Proposes a scalable distributed vehicle control method for emergency vehicle transit that uses only local information, avoids pre-training, and provides safety guarantees while minimizing impact on ordinary vehicles.


<details>
  <summary>Details</summary>
Motivation: Existing methods for emergency vehicle transit have two fundamental limitations: high computational cost (centralized solvers only work for small-scale scenarios) and lack of scalability (reinforcement learning models don't adapt well to different traffic conditions). Need a method that enables rapid emergency vehicle transit while minimizing impact on ordinary vehicles.

Method: A scalable distributed vehicle control method where vehicles adjust driving behaviors using only local information in real-time without pre-training. The method includes: 1) Distributed control using local information proven to be approximately equivalent to using global information, enabling real-time optimal decisions; 2) Distributed conflict resolution mechanism to guarantee safety by avoiding decision conflicts, eliminating single-point-of-failure risk.

Result: Simulation experiments using real-world traffic datasets show the proposed method achieves faster decision-making, less impact on ordinary vehicles, and maintains much stronger scalability across different traffic densities and road configurations compared to existing methods.

Conclusion: The distributed approach overcomes limitations of centralized solvers and reinforcement learning by providing real-time, scalable emergency vehicle transit with safety guarantees and minimal disruption to ordinary traffic, making it practical for real-world deployment.
...
### [49] [Interpretable Zero-shot Referring Expression Comprehension with Query-driven Scene Graphs](https://arxiv.org/abs/2603.25004)
...
Method: 1) Use a VLM to construct a query-driven scene graph encoding spatial relationships, descriptive captions, and object interactions relevant to the query. 2) Use this scene graph as a structured textual intermediary to bridge visual features and semantic understanding. 3) Employ an LLM to infer the target object from the scene graph representation, providing detailed explanations for interpretability.

Result: SGREC achieves top-1 accuracy on most zero-shot REC benchmarks: RefCOCO val (66.78%), RefCOCO+ testB (53.43%), and RefCOCOg val (73.28%), demonstrating strong visual scene understanding capabilities.

Conclusion: The proposed SGREC method effectively addresses limitations of existing approaches by using query-driven scene graphs as structured intermediaries, enabling better zero-shot REC performance while maintaining interpretability through LLM explanations.
...
### [50] [Improving Fine-Grained Rice Leaf Disease Detection via Angular-Compactness Dual Loss Learning](https://arxiv.org/abs/2603.25006)
*Md. Rokon Mia,Rakib Hossain Sajib,Abdullah Al Noman,Abir Ahmed,B M Taslimul Haque*

Main category: cs.CV

TL;DR: Dual-loss framework combining Center Loss and ArcFace Loss improves rice leaf disease classification by addressing intra-class variance and inter-class similarity challenges.


<details>
  <summary>Details</summary>
Motivation: Traditional deep learning models using cross entropy loss struggle with high intra-class variance and inter-class similarity in plant pathology datasets, which hinders accurate rice leaf disease detection crucial for preventing crop losses.

Method: Proposed dual-loss framework combining Center Loss and ArcFace Loss applied to three backbone architectures (InceptionNetV3, DenseNet201, EfficientNetB0) trained on the Rice Leaf Dataset to enhance fine-grained classification.

Result: Achieved high accuracies of 99.6%, 99.2%, and 99.2% respectively, demonstrating significant performance gains over traditional approaches.

Conclusion: Angular margin-based and center-based constraints substantially boost feature embedding discriminative strength without requiring major architectural modifications, making the approach efficient and practical for real-world farming deployment.
...
<details>
  <summary>Details</summary>
Motivation: To achieve efficient and high-quality 3D reconstruction from sparse input views while maintaining fast training times, addressing the limitations of existing methods that either require many views or are computationally expensive.

Method: Combines TensorRF's efficient tensor-based representation with FreeNeRF's frequency-driven few-shot regularization. Introduces frequency and occlusion masks to improve stability and reconstruction quality under sparse input conditions.

Result: On Synthesis NeRF benchmark: improves average PSNR from 21.45 dB (TensorRF) to 23.70 dB, with fine-tuned version reaching 24.52 dB, while maintaining TensorRF's fast ≈10-15 minute training time. On THuman 2.0 dataset: achieves 27.37-34.00 dB with only eight input images for human body reconstruction.

Conclusion: Few TensoRF provides an efficient and data-effective solution for real-time 3D reconstruction across diverse scenes, offering improved quality from sparse views while maintaining fast training speeds.
...
### [52] [GDPO-Listener: Expressive Interactive Head Generation via Auto-Regressive Flow Matching and Group reward-Decoupled Policy Optimization](https://arxiv.org/abs/2603.25020)
*Zhangyu Jin,Maksim Siniukov,Deuksin Kwon,Ashutosh Chaubey,Mohammad Soleymani*

Main category: cs.CV

TL;DR: GDPO-Listener is a novel framework for generating expressive 3D head motions for both speaking and listening in dyadic interactions, addressing the Regression-to-the-Mean problem and enabling semantic text control.
...
Conclusion: GDPO-Listener successfully addresses key challenges in 3D head motion generation for dyadic interactions, providing highly expressive speaking and listening motions with improved variance and semantic control capabilities.
...
### [53] [VideoTIR: Accurate Understanding for Long Videos with Efficient Tool-Integrated Reasoning](https://arxiv.org/abs/2603.25021)
*Zhe Gao,Shiyu Shen,Taifeng Chai,Weinong Wang,Haotian Xu,Xing W,Wenbin Li,Qi Fan,Yang Gao,Dacheng Tao*

Main category: cs.CV

TL;DR: VideoTIR uses Reinforcement Learning to train MLLMs to efficiently use multi-level toolkits for long video understanding, reducing hallucinations through better visual-text token balance and optimized tool-calling.
...
Method: Proposes VideoTIR with Reinforcement Learning to encourage proper usage of comprehensive multi-level toolkits. Uses Zero-RL and SFT cold-starting for retrieval and focus on meaningful video segments. Introduces Toolkit Action Grouped Policy Optimization (TAGPO) for efficient tool-calling with stepwise rewards and reuse of failed rollouts. Develops sandbox-based trajectory synthesis for high-quality training data.
...
Conclusion: VideoTIR enhances long video understanding both accurately and efficiently by leveraging RL to optimize tool usage and reduce redundant tool-calling, addressing the hallucination problem in MLLMs.
...
TL;DR: CARE is a training-free controllable medical image restoration framework that balances structure preservation and prior-guided enhancement using a dual-latent strategy with adaptive control.


<details>
  <summary>Details</summary>
Motivation: Existing medical image restoration methods require task-specific retraining and lack control over the trade-off between faithful reconstruction and enhancement, which can be problematic in clinical settings where aggressive restoration may introduce hallucinations or alter diagnostically important structures.

Method: CARE uses a dual-latent restoration strategy: one branch enforces data fidelity and anatomical consistency, while another leverages generative prior to recover missing/degraded information. A risk-aware adaptive controller dynamically adjusts branch contributions based on restoration uncertainty and local structural reliability.
...
Conclusion: The approach offers a practical step toward safer, more controllable, and more deployment-ready medical image restoration without requiring additional model training.
...
### [55] [GeoNDC: A Queryable Neural Data Cube for Planetary-Scale Earth Observation](https://arxiv.org/abs/2603.25037)
*Jianbo Qi,Mengyao Li,Baogui Jiang,Yidan Chen,Qiao Wang*

Main category: cs.CV

TL;DR: GeoNDC is a neural data cube that compresses planetary-scale Earth observation data into a continuous spatiotemporal implicit neural field, enabling efficient storage, on-demand queries, and continuous-time reconstruction with high compression ratios and accuracy.


<details>
  <summary>Details</summary>
Motivation: Massive satellite Earth observation archives are stored as discrete raster files, making them expensive to store, transmit, and query. There's a need for more efficient, queryable representations that can handle planetary-scale data.

Method: GeoNDC encodes Earth observation data as a continuous spatiotemporal implicit neural field, allowing on-demand queries without full decompression. It uses neural networks to represent the data as a continuous function over space and time.

Result: Achieves 95:1 compression ratio (0.44GB for 20-year MODIS archive), high spectral fidelity (mean R² > 0.98), supports direct spatiotemporal queries on consumer hardware, and recovers cloud-free dynamics under simulated occlusion with R² > 0.85.

Conclusion: GeoNDC provides a unified AI-native representation for planetary-scale Earth observation that integrates query, reconstruction, and compression in a single framework, complementing raw archives with a compact, analysis-ready data layer.
...
### [56] [MoRGS: Efficient Per-Gaussian Motion Reasoning for Streamable Dynamic 3D Scenes](https://arxiv.org/abs/2603.25042)
*Wonjoon Lee,Sungmin Woo,Donghyeong Kim,Jungho Lee,Sangheon Park,Sangyoun Lee*

Main category: cs.CV

TL;DR: MoRGS is an online per-Gaussian motion reasoning framework that improves 4D reconstruction by explicitly modeling per-Gaussian motion using optical flow supervision and motion confidence weighting.
...
Method: MoRGS leverages optical flow on sparse key views as lightweight motion cues to regularize per-Gaussian motion. It learns a per-Gaussian motion offset field to reconcile discrepancies between projected 3D motion and observed flow across views and time. Also introduces per-Gaussian motion confidence to separate dynamic from static Gaussians and weight attribute updates.
...
Conclusion: MoRGS successfully addresses the limitation of existing online 4D reconstruction methods by explicitly modeling per-Gaussian motion with optical flow supervision, improving both reconstruction quality and motion fidelity while maintaining real-time performance.
...
### [57] [GaussFusion: Improving 3D Reconstruction in the Wild with A Geometry-Informed Video Generator](https://arxiv.org/abs/2603.25053)
*Liyuan Zhu,Manjunath Narayana,Michal Stary,Will Hutchcroft,Gordon Wetzstein,Iro Armeni*

Main category: cs.CV

TL;DR: GaussFusion improves 3D Gaussian splatting reconstructions using geometry-informed video generation to fix artifacts like floaters and flickering, working across different reconstruction methods with real-time capability.
...
Method: Introduces a geometry-informed video-to-video generator that refines 3DGS renderings across optimization-based and feed-forward methods. Renders Gaussian primitive video buffers encoding depth, normals, opacity, and covariance, then refines them to produce temporally coherent frames. Also includes an artifact synthesis pipeline that simulates diverse degradation patterns for robustness.
...
Conclusion: GaussFusion effectively mitigates 3DGS artifacts through geometry-informed video generation, works across different reconstruction methods, and offers both high-quality and real-time variants for practical 3D applications.
...
TL;DR: A closed-loop Event-SVE system combines spatially variant exposure cameras with neuromorphic event cameras to monitor high-energy propellant combustion under extreme HDR, smoke, and microsecond-scale motion conditions.
...
Method: A closed-loop system coupling a spatially variant exposure (SVE) camera with stereo neuromorphic event cameras. SVE produces HDR maps using smoke-aware fusion, while event cameras capture fast motion. The HDR maps provide absolute-intensity reference to clean event data and improve particle discrimination. Stereo event-based 3D pipeline estimates separation height and particle size via feature extraction and triangulation.

Result: Maximum calibration error of 0.56%, multimodal equivalent-radius statistics for boron-based propellants, and capture of fast separation transients that conventional sensors miss. The system provides microsecond-resolved 3D combustion measurement under smoke-obscured HDR conditions.

Conclusion: The proposed framework offers a practical, calibration-consistent solution for microsecond-resolved 3D combustion measurement in challenging smoke-obscured HDR environments, overcoming limitations of conventional imaging systems.
...
TL;DR: A method for high-quality dynamic Gaussian Splatting from monocular videos using SE(3) B-spline motion bases with adaptive control points for efficient complex motion modeling.


<details>
  <summary>Details</summary>
Motivation: To achieve high-quality dynamic Gaussian Splatting from monocular videos by explicitly modeling continuous position and orientation deformation of dynamic Gaussians, going beyond previous methods.
...
Result: Extensive experiments demonstrate that the method outperforms state-of-the-art methods in novel view synthesis.

Conclusion: The proposed approach effectively models complex dynamic scenes from monocular videos and achieves superior novel view synthesis performance compared to existing methods.
...
### [60] [GIFT: Global Irreplaceability Frame Targeting for Efficient Video Understanding](https://arxiv.org/abs/2603.25072)
...
TL;DR: GIFT is a training-free framework that selects video frames by assessing their irreplaceability, combining relevance and diversity, to reduce computational costs while improving video understanding performance.


<details>
  <summary>Details</summary>
Motivation: Current Video LLMs have high computational costs from processing dense frames. Existing keyframe selection methods use greedy decision-making with decoupled relevance/diversity evaluation, leading to local optima and selection of irrelevant noise frames.

Method: GIFT uses Directed Diversity to quantify frame uniqueness conditioned on relevance, creating unified irreplaceability scores. Then Budget-Aware Refinement employs adaptive iterative process: first secures core frames with highest irreplaceability, then builds temporal context as budget expands.
...
Conclusion: GIFT effectively addresses computational cost issues in VLMs by selecting frames based on intrinsic irreplaceability, outperforming existing methods through unified relevance-diversity assessment and adaptive budget-aware refinement.
...
### [61] [Z-Erase: Enabling Concept Erasure in Single-Stream Diffusion Transformers](https://arxiv.org/abs/2603.25074)
...
TL;DR: Z-Erase: First concept erasure method for single-stream diffusion transformers that prevents generation collapse through stream disentanglement and Lagrangian-guided adaptive modulation.


<details>
  <summary>Details</summary>
Motivation: Concept erasure is crucial for safety in T2I models, but existing methods fail on single-stream diffusion transformers (like Z-Image) where text and image tokens are processed as unified sequences, causing generation collapse.

Method: 1) Stream Disentangled Concept Erasure Framework decouples updates to enable existing methods on single-stream models. 2) Lagrangian-Guided Adaptive Erasure Modulation uses constrained optimization to balance erasure-preservation trade-off.
...
Conclusion: Z-Erase is the first effective concept erasure method for single-stream T2I models, providing a stable solution that bridges the gap between traditional architectures and emerging diffusion transformers.
...
### [62] [Bridging Perception and Reasoning: Token Reweighting for RLVR in Multimodal LLMs](https://arxiv.org/abs/2603.25077)
*Jinda Lu,Junkang Wu,Jinghan Li,Kexin Huang,Shuo Yang,Guoyin Wang,Jiancan Wu,Xiang Wang,Xiangnan He*

Main category: cs.CV

TL;DR: RLVR extended to MLLMs faces challenges due to interleaved perception/reasoning tokens. Proposed Token-Reweighting (ToR) strategy identifies critical tokens and dynamically reweights them during training, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: MLLM responses interleave perception tokens (visual grounding) with reasoning tokens (symbolic reasoning), making isolated optimization insufficient. These token types have distinct but interdependent capacities, requiring a method that addresses their coupling.

Method: Token-Reweighting (ToR) strategy that explicitly models interdependence between perception and reasoning tokens. It identifies critical tokens of both types and dynamically reweights them during RLVR training. Can be applied on top of existing methods like GRPO and DAPO.

Result: ToR delivers consistent performance gains across multiple multi-modal reasoning benchmarks, achieving state-of-the-art performance with both accurate visual grounding and coherent reasoning. Outperforms optimizing either perception- or reasoning-only tokens.

Conclusion: The interdependence between perception and reasoning tokens in MLLMs requires explicit modeling. ToR provides an effective plug-and-play solution that improves RLVR training for MLLMs by addressing this coupling, leading to better multimodal reasoning performance.
...
### [63] [Learning domain-invariant features through channel-level sparsification for Out-Of Distribution Generalization](https://arxiv.org/abs/2603.25083)
*Haoran Pei,Yuguang Yang,Kexin Liu,Juan Zhang,Baochang Zhang*

Main category: cs.CV

TL;DR: HCD uses channel-level causal masks and mutual information objectives to separate causal from spurious features for better OOD generalization.


<details>
  <summary>Details</summary>
Motivation: Deep learning models develop shortcut dependencies on domain-specific features, leading to inconsistent OOD performance. Current invariance learning methods struggle with highly mixed features in deep latent spaces.

Method: Hierarchical Causal Dropout (HCD) uses channel-level causal masks to enforce feature sparsity, separating causal from spurious features. Training uses Matrix-based Mutual Information (MMI) to minimize mutual information between latent features and domain labels while maximizing information with class labels. StyleMix-driven VICReg module prevents masks from filtering essential causal data.

Result: HCD outperforms existing top-tier methods on OOD benchmarks.

Conclusion: HCD effectively addresses shortcut learning by performing causal intervention at representation level, achieving superior OOD generalization through feature separation and stability mechanisms.
...
### [64] [Visual Attention Drifts,but Anchors Hold:Mitigating Hallucination in Multimodal Large Language Models via Cross-Layer Visual Anchors](https://arxiv.org/abs/2603.25088)
*Chengxu Yang,Jingling Yuan,Chuang Hu,Jiawei Jiang*

Main category: cs.CV

TL;DR: CLVA addresses object hallucination in multimodal LLMs by using cross-layer visual anchors to correct attention drift in deep layers without training.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reducing object hallucination lack interpretability regarding attention drift in final model stages, and current approaches using attention enhancement and visual retracing don't sufficiently address the root cause of hallucination.

Method: CLVA (Cross-Layer Visual Anchors) is a training-free method that reinforces critical mid-layer features while suppressing regressive noise. It identifies that output reliability depends on acquiring visual anchors at intermediate layers rather than final layers, and uses these anchors to pull deep layer attention back to correct visual regions.

Result: The method demonstrates outstanding performance across diverse architectures and benchmarks, achieving significant reduction in object hallucination without significant increase in computational time and GPU memory.

Conclusion: Object hallucination stems from deep layer attention regressing toward initial visual noise from early layers, and reinforcing mid-layer visual anchors can effectively correct this attention drift in a training-free manner.
...
### [65] [THEMIS: Towards Holistic Evaluation of MLLMs for Scientific Paper Fraud Forensics](https://arxiv.org/abs/2603.25089)
...
TL;DR: THEMIS is a multimodal benchmark for evaluating visual fraud reasoning in academic scenarios, featuring 4,000+ questions across 7 real-world scenarios, 5 fraud types with 16 fine-grained manipulations, and comprehensive capability evaluation showing current MLLMs struggle with only 56.15% top performance.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack the complexity and real-world relevance needed to properly evaluate multimodal LLMs on visual fraud reasoning in academic contexts. There's a critical gap between current evaluation methods and the actual challenges of detecting academic fraud in complex multimodal data.

Method: Created THEMIS benchmark with: 1) 4,000+ questions across 7 real-world academic scenarios using authentic retracted-paper cases and synthetic multimodal data, 2) Systematic coverage of 5 fraud types with 16 fine-grained manipulation operations, 3) Mapping from fraud types to 5 core visual fraud reasoning capabilities for multi-dimensional evaluation.

Result: Evaluation of 16 leading MLLMs shows even the best-performing model (GPT-5) achieves only 56.15% overall performance, demonstrating the benchmark's stringency. The benchmark reveals distinct strengths and specific weaknesses across different models' core capabilities.

Conclusion: THEMIS presents a challenging benchmark that bridges the gap between existing evaluations and real-world academic fraud complexity, and is expected to advance the development of MLLMs for complex fraud reasoning tasks.
...
### [66] [Pixelis: Reasoning in Pixels, from Seeing to Acting](https://arxiv.org/abs/2603.25091)
*Yunpeng Zhou*

Main category: cs.CV

TL;DR: Pixelis is a pixel-space agent that learns through executable visual operations (zoom, segment, track, OCR, etc.) and improves via three-phase training: supervised fine-tuning, curiosity-coherence reward tuning, and pixel test-time RL.


<details>
  <summary>Details</summary>
Motivation: Current vision-language systems are passive observers that only describe pixels without acting, limiting physically grounded visual intelligence. Learning through action rather than static description is essential for generalizable visual intelligence beyond curated data.

Method: Three-phase approach: 1) Supervised Fine-Tuning with masked imitation loss and auxiliary heads for pixel-grounded arguments; 2) Curiosity-Coherence Reward Fine-Tuning with dual-drive objective (prediction-error curiosity + adjacent-step coherence + efficiency prior); 3) Pixel Test-Time RL with neighbor retrieval, trajectory voting, and KL-to-EMA safety control.

Result: Consistent improvements across six public benchmarks: average +4.08% relative gain over 8B baseline (peaking at +6.03% on VSI-Bench), producing shorter, auditable toolchains while maintaining in-corridor KL during test-time learning.

Conclusion: Operating directly in pixel space grounds multimodal perception in the physical world, links visual reasoning with actionable outcomes, and enables embodied adaptation without external feedback, moving beyond passive observation to active visual intelligence.
...
### [67] [Label What Matters: Modality-Balanced and Difficulty-Aware Multimodal Active Learning](https://arxiv.org/abs/2603.25107)
...
<details>
  <summary>Details</summary>
Motivation: Multimodal learning requires large labeled datasets, which are expensive to obtain. Existing multimodal active learning approaches assume static modality importance and fixed selection rules, making them insensitive to the dynamic nature of multimodal learning where modality value and sample difficulty change during training.
...
Conclusion: RL-MBA effectively addresses the dynamic nature of multimodal learning by adaptively balancing modality contributions and prioritizing difficult samples, leading to better performance and fairness in multimodal active learning with limited annotation budgets.
...
### [68] [MSRL: Scaling Generative Multimodal Reward Modeling via Multi-Stage Reinforcement Learning](https://arxiv.org/abs/2603.25108)
*Chenglong Wang,Yifu Huo,Yang Gan,Qiaozhi He,Qi Meng,Bei Li,Yan Wang,Junfu Liu,Tianhua Zhou,Jingbo Zhu,Tong Xiao*

Main category: cs.CV

TL;DR: MSRL enables scalable reinforcement learning for multimodal reward models using limited multimodal data by learning reward reasoning from text data first, then transferring to multimodal tasks through progressive stages.


<details>
  <summary>Details</summary>
Motivation: Current RLVR-based training for multimodal reward models requires costly labeled multimodal preference data, limiting scalability. Need a method to scale MRM training without extensive multimodal annotations.
...
Result: MSRL substantially improves performance on visual understanding and generation tasks: from 66.6% to 75.9% on VL-RewardBench and from 70.2% to 75.7% on GenAI-Bench, without requiring additional multimodal preference annotations.

Conclusion: MSRL effectively scales RLVR-based training of generative multimodal reward models by leveraging abundant text data and progressive transfer learning, achieving strong performance gains without costly multimodal annotations.
...
TL;DR: Lightweight procedural augmentation using Moire interference patterns improves vision model robustness with negligible computational cost and no external data.


<details>
  <summary>Details</summary>
Motivation: Existing augmentation methods rely on diffusion-based synthesis or complex feature mixing, which introduce substantial computational overhead or require external datasets. The paper explores a more efficient alternative using analytic interference patterns.

Method: Procedural augmentation based on Moire interference patterns generated on-the-fly using closed-form mathematical formulation. Patterns are synthesized directly in memory, mixed with training images, and immediately discarded, creating a storage-free pipeline.

Result: The method consistently improves robustness across multiple benchmarks (ImageNet-C, ImageNet-R, adversarial benchmarks) with negligible computational cost (0.0026 seconds per image), outperforming standard augmentation baselines and existing external-data-free approaches.

Conclusion: Analytic interference patterns provide a practical and efficient alternative to data-driven generative augmentation methods, offering lightweight, storage-free augmentation that enhances model robustness.
...
TL;DR: AnyDoc is a framework for multi-task document generation using HTML/CSS format, featuring a scalable data synthesis pipeline that creates a large dataset (DocHTML), and fine-tunes MLLMs with height-aware reinforcement learning to handle content overflow issues.


<details>
  <summary>Details</summary>
Motivation: Existing document generation approaches face limitations in coverage and scale of human-crafted datasets, and struggle with handling multiple document generation tasks across diverse categories and styles in a unified format.

Method: 1) Creates scalable data synthesis pipeline to automatically generate HTML/CSS documents, producing DocHTML dataset (265K samples, 111 categories, 32 styles). 2) Fine-tunes multi-modal LLMs for three tasks: intention-to-document, document derendering, and element-to-document. 3) Implements height-aware reinforcement learning (HARL) to penalize content overflow using height difference reward function.

Result: AnyDoc outperforms both general-purpose MLLMs and task-specific baselines across all three document generation tasks in qualitative and quantitative experiments, effectively mitigating content overflow issues.

Conclusion: AnyDoc successfully addresses document generation challenges through a unified HTML/CSS framework, scalable data synthesis, and innovative height-aware reinforcement learning, demonstrating superior performance across multiple practical document generation tasks.
...
TL;DR: RPCC is a new framework that addresses foreground occlusion in RPCA by identifying sparse components indirectly through support determination, using Bayesian sparse tensor factorization with variational inference.


<details>
  <summary>Details</summary>
Motivation: Traditional RPCA assumes sparse foreground is added to low-rank background, but in many real applications (like video analysis), foreground actually occludes/replaces background elements, creating a mismatch that needs addressing.

Method: Proposes Robust Principal Component Completion (RPCC) using variational Bayesian inference applied to fully probabilistic Bayesian sparse tensor factorization, which converges to a hard classifier for support without post-hoc thresholding.

Result: The approach delivers near-optimal estimates on synthetic data and robust foreground-extraction/anomaly-detection performance on real color video and hyperspectral datasets.

Conclusion: RPCC effectively addresses the occlusion problem in RPCA applications, providing a principled Bayesian framework that eliminates thresholding requirements and works well on both synthetic and real-world data.
...
TL;DR: AirSplat adapts 3D Vision Foundation Models for pose-free novel view synthesis using self-consistent pose alignment and rating-based opacity matching to achieve high-fidelity results without camera poses.


<details>
  <summary>Details</summary>
Motivation: While 3D Vision Foundation Models show strong zero-shot geometric estimation capabilities, they struggle with generalizable novel view synthesis. The paper aims to bridge this gap by adapting 3DVFMs' geometric priors for high-quality, pose-free view synthesis.

Method: Two key techniques: (1) Self-Consistent Pose Alignment (SCPA) - a training-time feedback loop ensuring pixel-aligned supervision to resolve pose-geometry discrepancies; (2) Rating-based Opacity Matching (ROM) - leverages local 3D geometry consistency from a sparse-view NVS teacher model to filter degraded primitives.
...
Conclusion: AirSplat demonstrates the potential of adapting 3D Vision Foundation Models to enable simultaneous visual geometry estimation and high-quality view synthesis, advancing pose-free novel view synthesis capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal deepfake detectors suffer from dataset and generator bias due to training primarily on curated synthetic forgeries, limiting scalability and robustness to unseen manipulations. The subtle visual artifacts and cross-modal inconsistencies in deepfakes remain challenging to detect.

Method: SAVe uses self-supervised learning on authentic videos only. It generates identity-preserving, region-aware self-blended pseudo-manipulations to emulate tampering artifacts across multiple facial granularities. Additionally, it models lip-speech synchronization via an audio-visual alignment component to detect temporal misalignment patterns characteristic of audio-visual forgeries.

Result: Experiments on FakeAVCeleb and AV-LipSync-TIMIT datasets show competitive in-domain performance and strong cross-dataset generalization, demonstrating the effectiveness of self-supervised learning for multimodal deepfake detection.
...
TL;DR: DAPASS framework achieves state-of-the-art panoramic semantic segmentation under source-free UDA constraints by addressing geometric distortions and annotation costs through confidence-guided pseudo-label denoising and contextual resolution adversarial alignment.


<details>
  <summary>Details</summary>
Motivation: Panoramic semantic segmentation faces two key challenges: severe geometric distortions in panoramic projections and prohibitive annotation costs. While UDA from pinhole-camera datasets helps, real-world tasks often impose stricter source-free constraints where source data is inaccessible due to privacy/proprietary reasons, amplifying domain shift problems and causing unreliable pseudo-labels and performance degradation, especially for minority classes.

Method: Proposes DAPASS framework with two synergistic modules: 1) Panoramic Confidence-Guided Denoising (PCGD) generates high-fidelity, class-balanced pseudo-labels using perturbation consistency and neighborhood-level confidence filtering; 2) Contextual Resolution Adversarial Module (CRAM) addresses scale variance and distortion by adversarially aligning fine-grained details from high-resolution crops with global semantics from low-resolution contexts.

Result: Achieves state-of-the-art performances on outdoor (Cityscapes-to-DensePASS) and indoor (Stanford2D3D) benchmarks, with 55.04% (+2.05% improvement) and 70.38% (+1.54% improvement) mIoU respectively.

Conclusion: DAPASS effectively overcomes source-free UDA challenges in panoramic semantic segmentation by robustly transferring knowledge without source data, addressing both geometric distortions and pseudo-label reliability issues through innovative confidence-guided denoising and multi-resolution adversarial alignment techniques.
...
TL;DR: Ranking optimization outperforms binary DPO for vision-language models by better preserving visual faithfulness, requiring vision encoder finetuning for effectiveness.


<details>
  <summary>Details</summary>
Motivation: Standard DPO's binary "winner-takes-all" approach is suboptimal for vision-language models because it doesn't account for visual faithfulness - a losing response may still be faithful to visual inputs. The Bradley-Terry DPO formulation lacks nuance for visual content alignment.
...
Result: Ranking optimization outperforms binary DPO for long-form content generation and assessment. Key finding: these approaches require finetuning of the vision encoder to be effective, challenging the view of DPO as purely a language-reweighting process.

Conclusion: Ranking optimization provides better nuance for vision-language alignment than binary DPO, and effective preference optimization for multimodal models requires vision encoder adaptation, not just language model reweighting.
...
TL;DR: Researchers created a public dataset of 1612 images for 5 common skin diseases in Bangladesh and applied ML/DL models for automated detection.


<details>
  <summary>Details</summary>
Motivation: Skin disease detection is challenging in Bangladesh due to insufficient dermatological expertise and diagnostic instruments, which can lead to severe health consequences. There's a need for automated detection using AI/ML techniques.

Method: Developed a publicly available dataset of 1612 images (250 distinct + augmented) for 5 skin diseases collected from Faridpur Medical College. Applied several machine learning and deep learning models for classification.

Result: Created dataset with 302 Dermatitis, 381 Eczema, 301 Scabies, 316 Tinea Ringworm, and 312 Vitiligo images. Applied ML/DL models and reported classification performance (though specific metrics not mentioned in abstract).

Conclusion: The dataset addresses regional healthcare challenges but has global relevance for South Asia. The research aims to attract ML/DL researchers to advance automated dermatology diagnosis.
...
TL;DR: EgoXtreme: A new large-scale 6D pose estimation dataset for egocentric vision with extreme real-world challenges like motion blur, dynamic lighting, and visual obstructions.


<details>
  <summary>Details</summary>
Motivation: Existing 6D object pose estimation benchmarks don't capture real-world egocentric challenges like severe motion blur, dynamic illumination, and visual obstructions, creating a gap between lab data and real-world applications.

Method: Introduces EgoXtreme dataset captured from egocentric perspective with three challenging scenarios: industrial maintenance, sports, and emergency rescue, featuring extreme lighting, heavy motion blur, and smoke.

Result: State-of-the-art generalizable pose estimators fail in extreme conditions, especially low light. Image restoration (deblurring) offers no improvement, but tracking-based approaches show performance gains, indicating temporal information is valuable in fast-motion scenarios.

Conclusion: EgoXtreme is essential for developing and evaluating next-generation pose estimation models robust enough for real-world egocentric vision applications.
...
TL;DR: TANL is a training-free OOD detection method that dynamically selects test-time activated negative labels by evaluating activation levels across corpus data, improving OOD detection performance.


<details>
  <summary>Details</summary>
Motivation: Existing OOD detection methods using negative labels distant from ID classes often fail to capture OOD characteristics because these labels may have poor activation on OOD samples, limiting their effectiveness.

Method: TANL dynamically evaluates activation levels across corpus dataset during testing, mining candidate labels with high activation responses. It identifies high-confidence test images online, accumulates assignment probabilities to construct label activation metric, and selects distribution-adaptive negative labels. Includes batch-adaptive variant and activation-aware score function emphasizing strongly activated labels.

Result: Significantly reduces FPR95 from 17.5% to 9.8% on ImageNet benchmark. Validated on diverse backbones and wide task settings. Training-free, test-efficient, and theoretically justified.

Conclusion: TANL effectively addresses limitations of static negative labels by dynamically selecting test-time activated negative labels, improving OOD detection performance across various benchmarks and settings.
...
TL;DR: TITAnD reformulates trajectory anomaly detection as a vision problem using Hyperspectral Trajectory Images and Cyclic Factorized Transformers, enabling efficient multi-month analysis across both dense and sparse GPS data.


<details>
  <summary>Details</summary>
Motivation: Current trajectory anomaly detection methods face a fundamental trade-off: dense GPS methods preserve fine-grained evidence but have quadratic costs making multi-month analysis intractable, while sparse stay-point methods are scalable but discard valuable evidence. This forces separate architectures for each regime and prevents knowledge transfer between dense and sparse trajectory analysis.

Method: Proposes TITAnD (Trajectory Image Transformer for Anomaly Detection) with two key innovations: 1) Hyperspectral Trajectory Image (HTI) representation - a day x time-of-day grid with channels encoding spatial, semantic, temporal, and kinematic information that unifies both dense and sparse modalities; 2) Cyclic Factorized Transformer (CFT) - factorizes attention along the two temporal axes (within-day and across-day) to encode cyclic inductive bias of human routines while dramatically reducing computational cost.

Result: TITAnD achieves best AUC-PR across both sparse and dense benchmarks, surpassing vision models like UNet while being 11-75x faster than standard Transformers with comparable memory usage. This enables dense multi-month anomaly detection for the first time.

Conclusion: The vision reformulation approach combined with structure-aware modeling (cyclic inductive bias) is essential for effective trajectory anomaly detection. TITAnD successfully unifies dense and sparse trajectory analysis under a single framework, enabling knowledge transfer and scalable multi-month analysis previously impossible with existing methods.
...
TL;DR: FD² is a fine-grained dataset distillation framework that addresses limitations of existing decoupled methods by localizing discriminative regions and constructing fine-grained representations to improve recognition on fine-grained datasets.


<details>
  <summary>Details</summary>
Motivation: Existing decoupled dataset distillation methods rely on coarse class-label supervision and optimize samples within each class in nearly identical ways, which causes problems on fine-grained datasets: (1) retains large intra-class variation with subtle inter-class differences, and (2) makes same-class samples overly similar, limiting localized discriminative cues and hurting recognition performance.

Method: FD² uses a two-stage approach: During pretraining, counterfactual attention learning aggregates discriminative representations to update class prototypes. During distillation, a fine-grained characteristic constraint aligns each sample with its class prototype while repelling others, and a similarity constraint diversifies attention across same-class samples.

Result: Experiments on multiple fine-grained and general datasets show that FD² integrates seamlessly with decoupled dataset distillation and improves performance in most settings, indicating strong transferability.

Conclusion: FD² effectively addresses the limitations of existing decoupled dataset distillation methods for fine-grained datasets by localizing discriminative regions and constructing fine-grained representations, leading to improved recognition performance while maintaining compatibility with existing decoupled distillation frameworks.
...
TL;DR: Photon is a framework for 3D medical visual question answering that uses variable-length token sequences with adaptive token compression to reduce computational costs while maintaining volumetric continuity and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Current multimodal LLMs struggle with 3D medical imaging due to high computational costs. Existing methods using 2D slices or fixed-length token compression disrupt volumetric continuity and obscure subtle findings, creating a need for more efficient 3D medical VQA approaches.

Method: Photon introduces instruction-conditioned token scheduling and surrogate gradient propagation to adaptively reduce tokens during training/inference. It uses custom backpropagation with gradient restoration for differentiable optimization despite discrete token drop, plus regularization objectives to mitigate language-only bias and improve reliability.
...
Conclusion: Photon provides an effective framework for 3D medical VQA that balances computational efficiency with accurate volumetric understanding through adaptive token compression and specialized optimization techniques.
...
### [82] [Knowledge-Guided Failure Prediction: Detecting When Object Detectors Miss Safety-Critical Objects](https://arxiv.org/abs/2603.25499)
*Jakob Paul Zimmermann,Gerrit Holzbach,David Lerch*

Main category: cs.CV

TL;DR: KGFP is a monitoring framework that predicts object detector failures by measuring semantic misalignment between detector features and visual foundation model embeddings, improving safety-critical object detection recall.


<details>
  <summary>Details</summary>
Motivation: Object detectors in safety-critical applications can fail silently (e.g., miss pedestrians) without warning. Traditional OOD detection methods identify unfamiliar inputs but don't directly predict functional failures of the detector itself.

Method: Knowledge Guided Failure Prediction (KGFP) uses a dual-encoder architecture with angular distance metric to measure semantic misalignment between internal object detector features and visual foundation model embeddings. When either system operates outside its competence, the embeddings diverge, producing a high-angle signal that flags unsafe images.

Result: On COCO person detection, KGFP as a selective-prediction gate raises person recall among accepted images from 64.3% to 84.5% at 5% FPR. Maintains strong performance across six COCO-O visual domains, outperforming OOD baselines by large margins.

Conclusion: KGFP effectively predicts object detector failures by monitoring semantic alignment between detector and foundation model representations, providing a reliable safety mechanism for critical applications where silent failures are unacceptable.
...
TL;DR: Proposes a semantically disentangled unified model for 3D anomaly detection that addresses inter-category entanglement by reconstructing features conditioned on disentangled semantic representations.


<details>
  <summary>Details</summary>
Motivation: Unified models for 3D anomaly detection suffer from Inter-Category Entanglement (ICE), where latent features from different categories overlap, causing models to adopt incorrect semantic priors during reconstruction and yielding unreliable anomaly scores.

Method: Three key components: (1) Coarse-to-Fine Global Tokenization for instance-level semantic identity, (2) Category-Conditioned Contrastive Learning for disentangling category semantics, and (3) Geometry-Guided Decoder for semantically consistent reconstruction.

Result: Achieves state-of-the-art on Real3D-AD and Anomaly-ShapeNet, improving object-level AUROC by 2.8% for unified models and 9.1% for category-specific models, while enhancing reliability of unified 3D anomaly detection.

Conclusion: The proposed semantically disentangled unified model effectively addresses ICE in 3D anomaly detection, achieving superior performance and reliability compared to existing approaches.
...
### [84] [SportSkills: Physical Skill Learning from Sports Instructional Videos](https://arxiv.org/abs/2603.25163)
...
<details>
  <summary>Details</summary>
Motivation: Current video datasets lack depth for fine-grained physical skill learning. There's a need for datasets that capture the nuances of sports skills with instructional content to enable AI systems that can provide actionable feedback for skill improvement.

Method: Created SportSkills dataset with 360k+ instructional videos across 55 sports, containing 630k+ visual demonstrations paired with instructional narrations. Developed representation learning models and introduced mistake-conditioned instructional video retrieval task formulation.

Result: SportSkills enables 4x better performance on fine-grained action understanding compared to traditional activity datasets. The retrieval approach shows significant advancement in personalizing visual instructions, validated by professional coaches.

Conclusion: SportSkills bridges representation learning and actionable feedback generation for physical skill learning, enabling AI systems to provide personalized instructional content based on user execution mistakes.
...
TL;DR: This paper provides guidelines for designing back marks on uniform-looking species like pigs to optimize machine learning-based individual monitoring, analyzing how mark design affects algorithm recognition performance.


<details>
  <summary>Details</summary>
Motivation: There's little research on designing back marks for uniform-looking species like pigs, especially for machine learning-based monitoring solutions. With the rise of ML monitoring, there's a need for guidelines on creating marks that algorithms can effectively recognize.

Method: Trained a ResNet-50 neural network to classify ten pigs with unique back marks, then analyzed the model's predictions to understand which design choices work best for ML recognition.

Result: Analysis revealed that back marks must remain unambiguous under motion blur, diverse view angles, and occlusions from animal behavior. Design must also consider common data augmentation strategies like color, flip, and crop augmentations used during model training.

Conclusion: The insights provide valuable guidelines for optimizing back mark design to support effective individual-level monitoring using machine learning algorithms in both research and real-world applications.
...
### [86] [Towards Foundation Models for 3D Scene Understanding: Instance-Aware Self-Supervised Learning for Point Clouds](https://arxiv.org/abs/2603.25165)
*Bin Yang,Mohamed Abdelsamad,Miao Zhang,Alexandru Paul Condurache*

Main category: cs.CV

TL;DR: PointINS is a self-supervised learning framework for point clouds that improves instance localization by combining semantic understanding with geometric reasoning through orthogonal offset learning and complementary regularization strategies.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised learning approaches for point clouds focus on semantic awareness but transfer poorly to instance localization tasks, requiring full fine-tuning for strong performance. Since instance awareness is fundamental to 3D perception, bridging this gap is crucial for developing true 3D foundation models that support all downstream tasks.

Method: PointINS introduces an instance-oriented self-supervised framework with an orthogonal offset branch that jointly learns high-level semantic understanding and geometric reasoning. It employs two complementary regularization strategies: Offset Distribution Regularization (ODR) aligns predicted offsets with geometric priors, and Spatial Clustering Regularization (SCR) enforces local coherence using pseudo-instance masks.

Result: Through experiments across five datasets, PointINS achieves on average +3.5% mAP improvement for indoor instance segmentation and +4.1% PQ gain for outdoor panoptic segmentation, demonstrating significant performance gains in instance localization tasks.

Conclusion: PointINS successfully bridges the gap between semantic and instance awareness in 3D point cloud representations, paving the way for scalable 3D foundation models that can support diverse downstream tasks without requiring extensive fine-tuning.
...
TL;DR: CLiBD introduces hierarchy-aware multimodal learning for biodiversity identification, improving taxonomic classification accuracy by 14% over baselines with better robustness to noise and missing modalities.
...
Method: Two end-to-end variants: CLiBD-HiR with Hierarchical Information Regularization to shape embedding geometry across taxonomic levels, and CLiBD-HiR-Fuse with additional lightweight fusion predictor supporting image-only, DNA-only, or joint inference resilient to modality corruption.
...
<details>
  <summary>Details</summary>
Motivation: Previous SAM-based methods for text detection and layout analysis suffer from high inference latency due to sampling thousands of foreground points as prompts, and limited data utilization due to reliance on pixel-level segmentation annotations.

Method: Proposes ET-SAM with: 1) A lightweight point decoder that produces word heatmaps to generate few foreground points instead of thousands, 2) A joint training strategy that combines datasets with multi-level, word-level only, and line-level only annotations, 3) Three sets of learnable task prompts to handle dataset discrepancies.

Result: Achieves ~3× inference acceleration compared to previous SAM-based methods while maintaining competitive performance on HierText, and improves average F-score by 11.0% on Total-Text, CTW1500, and ICDAR15 datasets.

Conclusion: ET-SAM effectively addresses the efficiency and data utilization limitations of previous SAM-based text detection methods through architectural improvements and flexible training strategies, enabling practical deployment while maintaining strong performance.
...
TL;DR: LanteRn enables LMMs to perform visual reasoning directly in latent space using compact visual representations, improving efficiency and fine-grained understanding without external modules or pixel-space computation.


<details>
  <summary>Details</summary>
Motivation: Current LMMs struggle with visual reasoning and default to verbalizing perceptual content, which limits fine-grained spatial and visual understanding. Existing approaches either rely on external tools or inefficient pixel-space reasoning.
...
Conclusion: Internal latent representations provide a promising direction for more efficient multimodal reasoning, enabling LMMs to perform visual reasoning directly in latent space without external dependencies.
...
Method: Propose Knowledge-Guided Adversarial Training (KGAT) that models thermal radiation relations based on gray value rank order between classes, quantifies relation stability, and embeds this physical knowledge into adversarial training to ensure predictions align with actual physical laws.

Result: Extensive experiments on three infrared datasets and six mainstream detection models show KGAT effectively enhances both clean accuracy and robustness against adversarial attacks and common corruptions.

Conclusion: Incorporating infrared physical knowledge (thermal radiation relations) into adversarial training framework significantly improves robustness of infrared object detection systems in complex environments.
...
TL;DR: AG-EgoPose: A dual-stream framework for egocentric 3D human pose estimation that integrates short- and long-range motion context with spatial cues using transformer-based fusion.


<details>
  <summary>Details</summary>
Motivation: Egocentric 3D pose estimation is challenging due to perspective distortion, limited body visibility, and complex camera motion. Existing methods fail to effectively leverage rich motion context available in egocentric videos.

Method: Dual-stream framework with spatial stream (ResNet-18 encoder-decoder for 2D joint heatmaps and spatial features) and temporal stream (ResNet-50 + action recognition backbone for motion dynamics). Features are fused in transformer decoder with learnable joint tokens for joint-level integration.

Result: Achieves state-of-the-art performance on real-world datasets in both quantitative and qualitative metrics.

Conclusion: AG-EgoPose effectively integrates spatial and temporal evidence for robust egocentric 3D pose estimation, addressing key challenges of first-person viewpoints.
...
TL;DR: Simple modifications to V&L models improve compositionality without harming basic capabilities or increasing inference cost.


<details>
  <summary>Details</summary>
Motivation: V&L models have limited compositional learning ability. Prior methods using hard negatives degrade basic capabilities and don't generalize well.

Method: 1) Use short concept-centric captions aligned with images, 2) Introduce parameter-free cross-modal attention-pooling for concept-centric visual embeddings, 3) Add simple auxiliary contrastive losses.

Result: Achieves SOTA on compositionality benchmarks while maintaining or improving zero-shot and retrieval performance, with no inference cost increase.

Conclusion: Simple architectural changes addressing root causes of poor compositionality can significantly improve V&L models without compromising their core capabilities.
...
TL;DR: BiHumanML3D is the first bilingual text-to-motion benchmark, and BiMD with Cross-Lingual Alignment (CLA) outperforms monolingual models on cross-lingual motion generation.
...
TL;DR: VolDiT is the first purely transformer-based 3D diffusion model for volumetric medical image synthesis, replacing convolutional U-Nets with volumetric patch embeddings and global self-attention for better scalability and global context integration.


<details>
  <summary>Details</summary>
Motivation: Existing 3D medical image generation methods rely on convolutional U-Net backbones with strong locality biases and limited receptive fields, which constrain scalability, global context integration, and flexible conditioning.

Method: Extends diffusion transformers to native 3D data through volumetric patch embeddings and global self-attention over 3D tokens. Introduces a timestep-gated control adapter that maps segmentation masks into learnable control tokens to modulate transformer layers during denoising.
...
Conclusion: Fully transformer-based diffusion models provide a flexible foundation for volumetric medical image synthesis, offering advantages over convolutional approaches.
...
TL;DR: AnyID is a video generation framework that preserves multiple identity references with ultra-high fidelity and attribute-level controllability.


<details>
  <summary>Details</summary>
Motivation: Current identity-preserving video generation methods are limited to single identity references, restricting creative flexibility and causing ambiguity in identity reproduction across different contexts.

Method: Two core contributions: 1) Scalable omni-referenced architecture that unifies heterogeneous identity inputs (faces, portraits, videos), and 2) Primary-referenced generation paradigm with differential prompts for attribute-level controllability. Training involves large-scale dataset curation and RL fine-tuning using human preference data.

Result: AnyID achieves ultra-high identity fidelity and superior attribute-level controllability across different task settings, validated through extensive evaluations.

Conclusion: AnyID addresses limitations of single-reference methods by enabling flexible multi-identity input handling with precise control, advancing identity-preserving video generation capabilities.
...
TL;DR: CardioDiT is a 4D latent diffusion transformer framework for generating temporally coherent cine cardiac MRI without architectural factorization of space and time.


<details>
  <summary>Details</summary>
Motivation: Existing 3D medical image synthesis methods struggle with 4D data like cine cardiac MRI, often factorizing space and time or using auxiliary mechanisms that introduce structural biases, spatiotemporal discontinuities, and physiologically inconsistent cardiac dynamics.

Method: Proposes CardioDiT: a fully 4D latent diffusion framework using diffusion transformers. A spatiotemporal VQ-VAE encodes 2D+t slices into compact latents, then a diffusion transformer models these jointly as complete 3D+t volumes, coupling space and time throughout generation.

Result: Evaluation on public CMR datasets and larger private cohort shows improved inter-slice consistency, temporally coherent motion, and realistic cardiac function distributions compared to baselines with progressively stronger spatiotemporal coupling.

Conclusion: Explicit 4D modeling with diffusion transformers provides a principled foundation for spatiotemporal cardiac image synthesis, enabling learning of continuous cardiac dynamics without architectural factorization.
...
TL;DR: TacSIm is a large-scale dataset and benchmark for tactical style imitation in football that focuses on replicating real-world team tactical behaviors rather than just optimizing reward-based objectives like goals scored.


<details>
  <summary>Details</summary>
Motivation: Current football imitation research primarily focuses on optimizing reward-based objectives (goals scored, win rates) but pays less attention to accurately replicating real-world team tactical behaviors. There's a need for better tactical style imitation in football.

Method: TacSIm creates a large-scale dataset from Premier League broadcast footage, projecting positions and actions of all 22 players onto a standard pitch coordinate system. It offers explicit style imitation tasks and evaluation protocols using spatial occupancy similarity and movement vector similarity over defined time periods.

Result: TacSIm establishes a rigorous benchmark for measuring and modeling style-aligned tactical imitation in football. It enables both quantitative and visual assessment of tactical coordination by running multiple baseline methods in a unified virtual environment to generate full team behaviors.

Conclusion: TacSIm provides a comprehensive framework for tactical style imitation in football, bridging the gap between broadcast footage and simulation with unified data and metrics, enabling better evaluation of spatial and temporal similarities in team tactical behaviors.
...
Method: CIV-DG leverages Conditional Instrumental Variables to relax strict random assignment assumptions, accommodating clinical scenarios where hospital selection is endogenously driven by patient demographics. It uses a Deep Generalized Method of Moments architecture with a conditional critic to minimize moment violations and enforce instrument-error orthogonality within demographic strata.
...
Conclusion: CIV-DG provides an effective causal framework for robust medical AI by addressing structural confounding through conditional instrumental variables, enabling better cross-site generalization despite selection bias in clinical settings.
...
TL;DR: FreeLOC is a training-free framework that solves O.O.D problems in long-video generation by using VRPR for frame-level position re-encoding and TSA for context-length issues, with layer-adaptive probing for selective application.


<details>
  <summary>Details</summary>
Motivation: Pre-trained video diffusion models trained on short clips struggle with long-video generation due to visual quality degradation caused by two O.O.D problems: frame-level relative position O.O.D and context-length O.O.D.

Method: Proposes FreeLOC with two core techniques: Video-based Relative Position Re-encoding (VRPR) for frame-level O.O.D using hierarchical re-encoding, and Tiered Sparse Attention (TSA) for context-length O.O.D with structured attention density. Includes layer-adaptive probing to identify sensitive transformer layers for selective application.
...
Conclusion: FreeLOC effectively addresses O.O.D challenges in long-video generation without requiring additional training, offering a practical solution for extending pre-trained video diffusion models to longer sequences.
...
TL;DR: PCGR is an interpretable framework for multimodal misinformation detection that uses concept-based reasoning with automatically discovered high-level concepts and hierarchical attention over concept graphs.


<details>
  <summary>Details</summary>
Motivation: Multimodal misinformation is increasing and often evades traditional detectors, which are opaque black boxes and fragile against new manipulation tactics. There's a need for interpretable and evolvable detection methods.

Method: Probabilistic Concept Graph Reasoning (PCGR) follows a build-then-infer paradigm: 1) constructs a graph of human-understandable concept nodes, including novel high-level concepts automatically discovered and validated by multimodal large language models (MLLMs), 2) applies hierarchical attention over this concept graph to infer claim veracity.

Result: PCGR achieves state-of-the-art multimodal misinformation detection accuracy and robustness to emerging manipulation types, outperforming prior methods in both coarse detection and fine-grained manipulation recognition.

Conclusion: PCGR provides an interpretable and evolvable framework for multimodal misinformation detection that produces understandable reasoning chains linking evidence to conclusions while maintaining high detection performance.
...
TL;DR: FEAST is an attention-based framework for predicting spatial gene expression from whole slide images using fully connected graphs with negative-aware attention and off-grid sampling to capture richer biological interactions.


<details>
  <summary>Details</summary>
Motivation: Spatial Transcriptomics is expensive, so predicting gene expression from cheaper whole slide images is valuable. Existing graph neural network methods have structural limitations due to sparse pre-defined graphs that miss potential interactions between tissue regions.

Method: FEAST models tissue as a fully connected graph using attention mechanisms to consider all pairwise interactions. It introduces negative-aware attention to capture both excitatory and inhibitory biological interactions, and off-grid sampling to gather additional images from intermediate regions for richer morphological context.
...
Conclusion: FEAST addresses limitations of existing methods by enabling comprehensive modeling of tissue interactions through fully connected attention, negative-aware attention for biological realism, and off-grid sampling for richer context, resulting in superior performance and interpretable biological insights.
...
TL;DR: SDD-YOLO is a specialized small-target detection framework for ground-to-air anti-UAV surveillance, featuring a P2 high-resolution detection head and YOLO26 architectural improvements, achieving 86.0% mAP@0.5 on a new DroneSOD-30K dataset with real-time performance.


<details>
  <summary>Details</summary>
Motivation: Detecting small UAVs from ground-to-air perspective is challenging due to extremely low pixel occupancy, cluttered backgrounds, and real-time constraints. Existing YOLO-based detectors lack adequate feature resolution for sub-pixel targets and have deployment complexities.

Method: Proposes SDD-YOLO with: 1) P2 high-resolution detection head (4x downsampling) for fine-grained spatial details, 2) YOLO26 architectural advancements (DFL-free, NMS-free for streamlined inference), 3) MuSGD hybrid training with ProgLoss and STAL to mitigate gradient oscillation on sparse small-target signals. Also constructs DroneSOD-30K dataset with ~30K annotated images covering diverse conditions.

Result: SDD-YOLO-n achieves 86.0% mAP@0.5 on DroneSOD-30K, surpassing YOLOv5n by 7.8 percentage points. Inference analysis shows 226 FPS on NVIDIA RTX 5090 and 35 FPS on Intel Xeon CPU, demonstrating exceptional efficiency for edge deployment.

Conclusion: SDD-YOLO effectively addresses small UAV detection challenges through specialized high-resolution detection architecture and optimized training strategies, achieving state-of-the-art performance while maintaining real-time efficiency suitable for practical anti-UAV surveillance systems.
...
<details>
  <summary>Details</summary>
Motivation: Supervised methods lack flexibility for new/unseen surgical tools and require extensive annotated data, limiting practical deployment in clinical settings.

Method: Two-stage pipeline: 1) Multi-view detection using rendered templates and geometric consistency, 2) Iterative pose refinement with feature-metric scoring and occlusion-aware contour registration.

Result: Achieves millimeter-accurate pose estimates on par with supervised methods on MVPSP dataset, with full generalization to unseen instruments.

Conclusion: Novel training-free approach combining foundational models, multi-view geometry, and contour refinement enables robust instrument tracking in clinical environments without task-specific training.
...
### [104] [A Unified Spatial Alignment Framework for Highly Transferable Transformation-Based Attacks on Spatially Structured Tasks](https://arxiv.org/abs/2603.25230)
*Jiaming Liang,Chi-Man Pun*

Main category: cs.CV

TL;DR: SAF (Spatial Alignment Framework) improves transferability of transformation-based adversarial attacks on structured tasks like segmentation and detection by synchronously transforming labels with inputs to prevent spatial misalignment.


<details>
  <summary>Details</summary>
Motivation: Existing transformation-based adversarial attacks work well on classification tasks but fail on structured tasks (segmentation, detection) due to spatial misalignment between transformed inputs and untransformed labels, which causes erroneous gradients.

Method: Proposes Spatial Alignment Framework (SAF) with Spatial Alignment (SA) algorithm that synchronously transforms labels with inputs when applying spatial transformations, ensuring spatial alignment for structured tasks.
...
Conclusion: Spatial alignment is crucial for transferable transformation-based attacks on structured tasks; SAF provides a unified framework that synchronizes label transformations with input transformations to achieve effective adversarial attacks.
...
TL;DR: Sharpening images alone provides effective pre-attack robustification against adversarial perturbations without needing surrogate classifiers, iterative optimization, or trained generators.


<details>
  <summary>Details</summary>
Motivation: Existing preemptive robustification methods have practical limitations: they rely on well-trained surrogate classifiers, have high computational overhead from iterative optimization or trained generators, and lack interpretability. The paper aims to find a simpler, more practical approach.

Method: The paper proposes using simple image sharpening as a robustification technique. This approach is based on findings that texture intensity correlates with robustness of benign samples. Sharpening is surrogate-free, optimization-free, generator-free, and human-interpretable.

Result: Extensive experiments show that sharpening yields remarkable robustness gains with low computational cost, especially in transfer scenarios where attacks are transferred between models.

Conclusion: Image sharpening provides an efficient, practical pre-attack defense against adversarial perturbations, addressing limitations of previous robustification methods while maintaining interpretability and low computational overhead.
...
TL;DR: SMAP introduces a semantic-aware prefix tokenizer that makes semantic conditions essential for representation learning, improving both reconstruction quality and downstream generation performance.


<details>
  <summary>Details</summary>
Motivation: Existing visual tokenizers are trained with reconstruction-dominated objectives, resulting in latent representations weakly grounded in high-level semantics. Current approaches treat semantic signals as auxiliary regularization rather than making them functionally necessary for representation learning.

Method: SMAP (SeMantic-Aware Prefix tokenizer) injects class-level semantic conditions into a query-based 1D tokenization framework. It introduces a tail token dropping strategy that forces semantic conditions and early latent prefixes to bear increasing responsibility under progressively reduced token budgets. The paper also introduces CARD, a hybrid Causal AutoRegressive--Diffusion generator to verify the latent space is useful for generation.
...
Conclusion: SMAP successfully makes semantic conditions indispensable during training through its tail token dropping strategy, creating a semantically grounded latent space that benefits both reconstruction and generation tasks, particularly under limited token budgets.
...
TL;DR: FastPCC: A lightweight neural compression framework for LiDAR point clouds that achieves real-time lossless compression through geometry re-densification and cross-scale feature propagation.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR point cloud compression methods suffer from inefficient context modeling due to extreme sparsity of geometric details, which limits both compression speed and performance.

Method: Two lightweight modules: 1) Geometry Re-Densification Module that iteratively densifies sparse geometry, extracts dense features, then sparsifies for prediction; 2) Cross-scale Feature Propagation Module that uses multi-resolution occupancy cues to guide hierarchical feature sharing. Plus integer-only inference for cross-platform consistency.

Result: Achieves competitive compression performance at real-time speed while avoiding entropy-coding collapse issues in existing neural methods.

Conclusion: Proposed FastPCC framework enables efficient predictive lossless coding of LiDAR point clouds with lightweight design, real-time performance, and cross-platform consistency.
...
TL;DR: ViewSplat introduces view-adaptive 3D Gaussian splatting that uses dynamic MLPs to adjust Gaussian primitives based on target viewpoints, achieving better novel view synthesis from unposed images compared to static feed-forward methods.


<details>
  <summary>Details</summary>
Motivation: Feed-forward 3D Gaussian splatting methods have accelerated scene reconstruction but suffer from a fidelity gap because single-step networks struggle to regress static Gaussian primitives that work well from all viewpoints.

Method: Instead of static primitive regression, ViewSplat learns view-adaptable latent representations. It predicts base Gaussian primitives plus dynamic MLP weights. During rendering, these MLPs take target view coordinates as input and predict view-dependent residual updates for each Gaussian attribute (position, scale, rotation, opacity, color).

Result: ViewSplat achieves state-of-the-art fidelity while maintaining fast inference (17 FPS) and real-time rendering (154 FPS) in extensive experiments.

Conclusion: The view-adaptive dynamic splatting paradigm effectively addresses the fidelity bottleneck in feed-forward 3D Gaussian splatting by allowing primitives to rectify initial estimation errors and capture high-fidelity appearances from different viewpoints.
...
### [109] [CHIRP dataset: towards long-term, individual-level, behavioral monitoring of bird populations in the wild](https://arxiv.org/abs/2603.25524)
...
TL;DR: CHIRP dataset and CORVID method for automated behavioral monitoring of wild birds, with biologically relevant benchmarking.
...
Method: Introduces CHIRP dataset from wild Siberian jays supporting multiple CV tasks, and CORVID pipeline for individual re-identification using segmentation and classification of colored leg rings with probability-based tracking.

Result: CORVID outperforms state-of-the-art re-id methods in application-specific benchmarking with biologically relevant metrics (feeding rates, co-occurrence rates).

Conclusion: Provides blueprint for curating real-world datasets from ethically approved biological studies to bridge computer vision research with biological applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing text-video retrieval methods focus on video representations or cross-modal alignment, but recent approaches enrich text expressiveness. However, they ignore rich interactions among internal frames within videos, causing the expanded text to miss frame contextual information and creating disparities between text and video representations.

Method: Proposes EagleNet with: 1) Fine-Grained Relationship Learning (FRL) that constructs a text-frame graph using generated text candidates and frames, learns relationships among texts and frames, then aggregates text candidates into enriched text embeddings with frame context; 2) Energy-Aware Matching (EAM) to model energy of text-frame interactions to capture real text-video pair distributions; 3) Replaces softmax-based contrastive loss with sigmoid loss for more effective cross-modal alignment and stable training.

Result: Extensive experiments demonstrate superiority of EagleNet across four benchmark datasets: MSRVTT, DiDeMo, MSVD, and VATEX.

Conclusion: EagleNet effectively addresses the limitation of previous methods by capturing frame contextual information through fine-grained relationship learning and energy-aware matching, leading to improved text-video retrieval performance across multiple datasets.
...
TL;DR: V2U4Real is the first large-scale real-world multi-modal dataset for Vehicle-to-UAV cooperative perception, addressing limitations of ground-only V2V/V2I systems through aerial-ground collaboration.


<details>
  <summary>Details</summary>
Motivation: Existing cooperative perception (V2V/V2I) is limited to ground-level collaboration, unable to address large-scale occlusions or long-range perception in complex environments. There's a need for cross-view cooperation between ground vehicles and aerial platforms.

Method: Created V2U4Real dataset collected by a ground vehicle and UAV equipped with multi-view LiDARs and RGB cameras. Dataset covers urban streets, campuses, and rural roads with diverse traffic scenarios, containing 56K LiDAR frames, 56K multi-view images, and 700K annotated 3D boxes across 4 classes.

Result: Established benchmarks for single-agent 3D detection, cooperative 3D detection, and object tracking. Comprehensive evaluations show V2U cooperation enhances perception robustness and long-range awareness compared to ground-only approaches.

Conclusion: V2U4Real enables research in cross-view cooperative perception, demonstrating the effectiveness of vehicle-UAV collaboration for overcoming occlusion and range limitations in autonomous vehicle perception systems.
...
### [112] [DeepFAN, a transformer-based deep learning model for human-artificial intelligence collaborative assessment of incidental pulmonary nodules in CT scans: a multi-reader, multi-case trial](https://arxiv.org/abs/2603.25607)
...
TL;DR: DeepFAN is a transformer-based model for classifying benign vs malignant lung nodules that achieved high diagnostic performance (AUC 0.939-0.954) and significantly improved junior radiologists' diagnostic accuracy by 10% in a clinical trial.
...
Method: Developed DeepFAN, a transformer-based model trained on over 10K pathology-confirmed nodules, then conducted a multi-reader, multi-case clinical trial across three medical institutions with 400 cases to evaluate its efficacy in assisting junior radiologists.

Result: DeepFAN achieved AUC of 0.939 (internal) and 0.954 (clinical trial). Junior radiologists' performance improved by 10.9% in AUC, 10.0% in accuracy, 7.6% in sensitivity, and 12.6% in specificity. Diagnostic consistency improved from fair to moderate.

Conclusion: DeepFAN effectively assisted junior radiologists and may help homogenize diagnostic quality and reduce unnecessary follow-up of indeterminate pulmonary nodules.
...
TL;DR: Proposes CLE-RWKV, a controllable low-light enhancement framework that reformulates the task as a well-posed conditional problem to address multimodal solution space issues, using Light100 benchmark and HVI color space supervision.


<details>
  <summary>Details</summary>
Motivation: Traditional deterministic LLIE struggles with ill-posed nature due to unknown ambient conditions and sensor parameters, leading to luminance discrepancies that require gt-mean post-processing for evaluation alignment.

Method: Introduces CLE-RWKV framework with Light100 benchmark featuring continuous real-world illumination transitions. Uses noise-decoupled supervision in HVI color space to separate illumination from texture, and Space-to-Depth strategy to adapt SSMs for dense prediction while maintaining linear complexity.

Result: Achieves competitive performance across seven benchmarks with robust controllability, significantly reducing reliance on gt-mean post-processing and providing a real-world multi-illumination alternative.

Conclusion: The proposed controllable low-light enhancement paradigm successfully addresses fundamental limitations of traditional deterministic approaches by reformulating the task as a well-posed conditional problem with practical real-world applicability.
...
### [114] [Demographic Fairness in Multimodal LLMs: A Benchmark of Gender and Ethnicity Bias in Face Verification](https://arxiv.org/abs/2603.25613)
*Ünsal Öztürk,Hatef Otroshi Shahreza,Sébastien Marcel*

Main category: cs.CV

TL;DR: Benchmark study evaluates demographic fairness of 9 open-source MLLMs for face verification across ethnicity and gender groups, finding FaceLLM-8B outperforms general-purpose models and revealing different bias patterns than traditional face recognition.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs are being explored for face verification but their demographic fairness remains largely unexplored, unlike dedicated face recognition systems where fairness issues are well-documented.

Method: Evaluated 9 open-source MLLMs from 6 model families (2B-8B parameters) on IJB-C and RFW face verification protocols across 4 ethnicity and 2 gender groups, using Equal Error Rate, True Match Rate, and 4 FMR-based fairness metrics.

Result: FaceLLM-8B (face-specialized) substantially outperforms general-purpose MLLMs. Bias patterns differ from traditional face recognition, with different groups most affected depending on benchmark and model. Most accurate models aren't necessarily fairest, and poor-performing models can appear fair due to uniformly high error rates.

Conclusion: MLLMs show different demographic bias patterns than traditional face recognition systems, requiring specialized fairness evaluation. Face-specialized models perform better, but accuracy-fairness tradeoffs exist, and evaluation must consider that uniformly poor performance can misleadingly appear fair.
...
TL;DR: GLIC proposes a graph-based learned image compression framework with dual-scale graphs and adaptive connectivity to overcome rigid receptive fields in CNNs/Transformers, achieving SOTA compression performance.


<details>
  <summary>Details</summary>
Motivation: Current SOTA learned image compression methods using CNNs/Transformers have rigid receptive fields and static connectivity patterns that couple non-redundant pixels due to Euclidean proximity, limiting their ability to adaptively capture spatially varying redundancy across images.

Method: Proposes Graph-based Learned Image Compression (GLIC) using Graph Neural Networks with dual-scale graphs for flexible data-driven receptive fields and adaptive connectivity that dynamically adjusts neighbor counts based on local content complexity.

Result: GLIC achieves state-of-the-art performance with BD-rate reductions of 19.29% on Kodak, 21.69% on Tecnick, and 18.71% on CLIC relative to VTM-9.1.

Conclusion: Graph-based approaches with adaptive connectivity enable more efficient image compression by modeling diverse redundancy patterns, overcoming limitations of rigid CNN/Transformer architectures.
...
TL;DR: MacroData: A large-scale dataset of 400K multi-reference image pairs (up to 10 references) with MacroBench benchmark for evaluating multi-reference image generation tasks.


<details>
  <summary>Details</summary>
Motivation: Current image generation models degrade with multiple visual references due to data bottleneck - existing datasets lack structured, long-context supervision for learning dense inter-reference dependencies needed for applications like multi-subject composition and novel view synthesis.

Method: Introduces MacroData dataset with 400K samples organized across four dimensions (Customization, Illustration, Spatial reasoning, Temporal dynamics) and MacroBench benchmark with 4,000 samples for standardized evaluation across graded task dimensions and input scales.
...
Conclusion: MacroData addresses the fundamental data bottleneck in multi-reference image generation, providing comprehensive coverage and standardized evaluation through MacroBench, enabling better performance in real-world applications requiring multiple visual references.
...
### [117] [Just Zoom In: Cross-View Geo-Localization via Autoregressive Zooming](https://arxiv.org/abs/2603.25686)
*Yunus Talha Erzurumlu,Jiyong Kwag,Alper Yilmaz*

Main category: cs.CV

TL;DR: Just Zoom In reformulates cross-view geo-localization as an autoregressive zooming task over satellite maps instead of contrastive image retrieval, achieving better performance with simpler training.


<details>
  <summary>Details</summary>
Motivation: Existing CVGL methods use contrastive image retrieval which requires large batches and hard negative mining, ignores map geometry, and suffers from coverage mismatch between street and satellite views where landmarks may fall outside fixed crops.

Method: Proposes autoregressive zooming over city-scale overhead maps: starting from coarse satellite view, model makes sequential zoom-in decisions to select terminal satellite cell at target resolution, eliminating need for contrastive losses or hard negative mining.

Result: Achieves state-of-the-art on new realistic benchmark with crowd-sourced street views and high-res satellite imagery: improves Recall@1 within 50m by 5.5% and Recall@1 within 100m by 9.6% over strongest contrastive-retrieval baseline.

Conclusion: Sequential coarse-to-fine spatial reasoning is more effective for cross-view geo-localization than contrastive retrieval, demonstrating the value of geometric map reasoning and eliminating complex training requirements.
...
TL;DR: HeSS method improves VGGT efficiency by measuring head-wise sparsification sensitivity and adaptively allocating attention budgets, reducing accuracy degradation in high-sparsity scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing sparsification methods for Visual Geometry Grounded Transformers suffer from substantial accuracy degradation due to applying uniform sparsity patterns across all attention heads, ignoring their heterogeneous sensitivity characteristics.

Method: Two-stage pipeline: 1) Measure head-wise sparsification sensitivity using Head Sensitivity Score (HeSS) metric based on Hessian approximation with two error terms on calibration data; 2) Perform HeSS-Guided Sparsification that reallocates total attention budget, assigning denser attention to sensitive heads and sparser attention to robust ones.

Result: HeSS effectively captures head-wise sparsification sensitivity; attention heads exhibit heterogeneous sensitivity; method mitigates performance degradation under high sparsity and shows strong robustness across varying sparsification levels.

Conclusion: Head-wise sensitivity-aware sparsification significantly improves VGGT efficiency while maintaining accuracy, addressing the quadratic computational cost problem without substantial performance degradation.
...
TL;DR: InstanceAnimator is a Diffusion Transformer framework for multi-instance sketch video colorization that addresses limitations in user control, instance alignment, and detail fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing methods have three core limitations: inflexible user control due to heavy reliance on single reference frames, poor instance controllability causing misalignment in multi-character scenarios, and degraded detail fidelity in fine-grained regions.

Method: Three key innovations: 1) Canvas Guidance Condition for flexible reference placement, 2) Instance Matching Mechanism for precise multi-character alignment, and 3) Adaptive Decoupled Control Module for enhanced detail fidelity through semantic feature injection.

Result: Extensive experiments demonstrate superior multi-instance colorization with enhanced user control, high visual quality, and strong instance consistency.

Conclusion: InstanceAnimator successfully addresses the limitations of existing methods by providing unprecedented user flexibility, precise instance control, and improved detail fidelity for sketch video colorization.
...
### [120] [Out of Sight but Not Out of Mind: Hybrid Memory for Dynamic Video World Models](https://arxiv.org/abs/2603.25716)
*Kaijin Chen,Dingkang Liang,Xin Zhou,Yikang Ding,Xiaoqiang Liu,Pengfei Wan,Xiang Bai*

Main category: cs.CV

TL;DR: Hybrid Memory paradigm for video world models that handles both static backgrounds and dynamic subjects during out-of-view intervals, with new dataset HM-World and architecture HyDRA.


<details>
  <summary>Details</summary>
Motivation: Existing video world models treat environments as static canvases and struggle when dynamic subjects hide out of sight and later re-emerge, causing frozen, distorted, or vanishing subjects.

Method: Introduces Hybrid Memory paradigm requiring models to act as archivists for static backgrounds and trackers for dynamic subjects. Proposes HyDRA architecture that compresses memory into tokens and uses spatiotemporal relevance-driven retrieval to selectively attend to relevant motion cues.

Result: Extensive experiments on HM-World dataset (59K clips, 17 scenes, 49 subjects) show HyDRA significantly outperforms state-of-the-art approaches in dynamic subject consistency and overall generation quality.

Conclusion: Hybrid Memory paradigm effectively addresses the limitation of current video world models in handling dynamic subjects during out-of-view intervals, with HyDRA demonstrating superior performance in preserving subject identity and motion continuity.
...
<details>
  <summary>Details</summary>
Motivation: CLIP's computational demands motivate distillation into lightweight models, but existing methods fail to preserve the structural relationships between teacher and student embeddings across modalities.

Method: Proposes two novel relational distillation methods: Vertical Relational Distillation (VRD) enforces consistency of distillation strength across modalities at distribution level, and Cross Relational Distillation (XRD) imposes bidirectional symmetry on cross-modal teacher-student similarity distributions.

Result: CLIP-RD outperforms existing distillation methods by 0.8% percentage points by better preserving the teacher's embedding geometry.

Conclusion: Explicitly modeling multi-directional relational dependencies between teacher and student embeddings enables more faithful knowledge transfer and better preserves the structural relationships encoded by the teacher CLIP model.
...
### [122] [PixelSmile: Toward Fine-Grained Facial Expression Editing](https://arxiv.org/abs/2603.25728)
*Jiabin Hua,Hengyuan Xu,Aojie Li,Wei Cheng,Gang Yu,Xingjun Ma,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: PixelSmile is a diffusion framework for fine-grained facial expression editing that addresses semantic overlap through fully symmetric joint training and achieves precise linear expression control.


<details>
  <summary>Details</summary>
Motivation: Fine-grained facial expression editing has been limited by intrinsic semantic overlap between different expressions, making precise control difficult.

Method: Proposes PixelSmile, a diffusion framework with fully symmetric joint training that combines intensity supervision with contrastive learning to disentangle expression semantics. Uses textual latent interpolation for linear expression control.

Result: Superior disentanglement and robust identity preservation compared to existing methods, achieving precise and stable linear expression control while naturally supporting smooth expression blending.

Conclusion: PixelSmile is effective for continuous, controllable, and fine-grained expression editing, addressing the semantic overlap problem through its novel training approach and achieving strong performance on the newly established FFE-Bench.
...
### [123] [Multimodal Dataset Distillation via Phased Teacher Models](https://arxiv.org/abs/2603.25388)
*Shengbin Guo,Hang Zhao,Senqiao Yang,Chenyang Jiang,Yuhang Cheng,Xiangru Peng,Rui Shao,Zhuotao Tian*

Main category: cs.CV

TL;DR: PTM-ST is a novel phased distillation framework that addresses limitations in multimodal dataset distillation by capturing complex, dynamically evolving knowledge from teacher models through stage-aware teacher modeling and shortcut trajectory construction.
...
Conclusion: PTM-ST effectively addresses the limitations of existing multimodal dataset distillation methods by better capturing teacher model dynamics across training stages, leading to improved student performance and more efficient knowledge transfer.
...
### [124] [PackForcing: Short Video Training Suffices for Long Video Sampling and Long Context Inference](https://arxiv.org/abs/2603.25730)
*Xiaofeng Mao,Shaohao Rui,Kaining Ying,Bo Zheng,Chuanhao Li,Mingmin Chi,Kaipeng Zhang*

Main category: cs.CV

TL;DR: PackForcing is a novel framework for long-video generation that addresses KV-cache growth issues through hierarchical context compression, enabling 2-minute video generation with bounded memory and 24x temporal extrapolation.


<details>
  <summary>Details</summary>
Motivation: Autoregressive video diffusion models face three key challenges: intractable linear KV-cache growth, temporal repetition, and compounding errors during long-video generation, which limit their practical application for extended video synthesis.

Method: Introduces a three-partition KV-cache strategy: (1) Sink tokens for early anchor frames, (2) Mid tokens with 32x compression via dual-branch network (3D convolutions + VAE re-encoding), and (3) Recent tokens for local coherence. Includes dynamic top-k context selection and Temporal RoPE Adjustment for position alignment.

Result: Generates coherent 2-minute, 832x480 videos at 16 FPS on single H200 GPU with bounded 4GB KV cache. Achieves 24x temporal extrapolation (5s to 120s) and state-of-the-art VBench scores: 26.07 temporal consistency and 56.25 dynamic degree.

Conclusion: PackForcing demonstrates that short-video supervision is sufficient for high-quality long-video synthesis, solving key memory and quality bottlenecks in autoregressive video diffusion models through principled hierarchical context compression.
...
### [125] [FSGNet: A Frequency-Aware and Semantic Guidance Network for Infrared Small Target Detection](https://arxiv.org/abs/2603.25389)
*Yingmei Zhang,Wangtao Bao,Yong Yang,Weiguo Wan,Qin Xiao,Xueting Zou*

Main category: cs.CV

TL;DR: FSGNet: A lightweight infrared small target detection framework using frequency-aware filtering and semantic guidance to address U-Net's semantic degradation problem.


<details>
  <summary>Details</summary>
Motivation: U-Net architectures for infrared small target detection suffer from semantic degradation when transferring high-level features from deep to shallow layers, limiting precise localization of small targets. There's a need to address this while maintaining efficiency.

Method: FSGNet incorporates three key components: 1) Multi-directional interactive attention module in encoder to capture fine-grained directional features; 2) Multi-scale frequency-aware module using Fast Fourier Transform to filter background clutter while preserving target structures; 3) Global pooling module at deepest layer with semantic guidance flows to ensure semantic consistency across scales.

Result: Extensive experiments on four public IRSTD datasets demonstrate superior detection performance while maintaining high efficiency, showing practical applicability and robustness.

Conclusion: FSGNet effectively addresses U-Net's semantic degradation problem through frequency-aware filtering and semantic guidance mechanisms, achieving state-of-the-art performance for infrared small target detection with practical efficiency.
...
### [126] [PMT: Plain Mask Transformer for Image and Video Segmentation with Frozen Vision Encoders](https://arxiv.org/abs/2603.25398)
*Niccolò Cavagnero,Narges Norouzi,Gijs Dubbelman,Daan de Geus*

Main category: cs.CV

TL;DR: Plain Mask Transformer (PMT) is a fast Transformer-based segmentation decoder that works on frozen Vision Foundation Model features, achieving competitive accuracy with 3-8x speedup while maintaining encoder sharing for multi-task deployment.
...
Method: Proposes Plain Mask Decoder (PMD), a fast Transformer-based segmentation decoder that operates on top of frozen VFM features. The resulting model, Plain Mask Transformer (PMT), preserves architectural simplicity and low latency while keeping encoder representation unchanged and shareable.
...
Conclusion: PMT successfully reconciles encoder-only simplicity and speed with frozen VFM features, enabling practical large-scale deployment with multi-task encoder sharing while maintaining competitive accuracy and significant speed improvements.
...
### [127] [HiSpatial: Taming Hierarchical 3D Spatial Understanding in Vision-Language Models](https://arxiv.org/abs/2603.25411)
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs lack human-like spatial intelligence - they need to infer 3D structures from 2D observations, recognize object properties/relations in 3D space, and perform high-level spatial reasoning.

Method: 1) Hierarchical framework with four progressively complex levels (geometric perception to abstract reasoning); 2) Automated pipeline generating 3D spatial VQA pairs from 5M images with 45M+ objects; 3) RGB-D VLM with metric-scale point maps as auxiliary inputs.

Result: Achieves SOTA on multiple spatial understanding/reasoning benchmarks, surpassing specialized spatial models and large proprietary systems (Gemini-2.5-pro, GPT-5). Analysis shows clear dependencies among hierarchical task levels.

Conclusion: The hierarchical multi-level task design effectively facilitates the emergence of 3D spatial intelligence in VLMs, offering new insights into spatial reasoning development.
...
### [128] [VideoWeaver: Multimodal Multi-View Video-to-Video Transfer for Embodied Agents](https://arxiv.org/abs/2603.25420)
*George Eskandar,Fengyi Shen,Mohammad Altillawi,Dong Chen,Yang Bai,Liudi Yang,Ziyuan Liu*

Main category: cs.CV

TL;DR: VideoWeaver is the first multimodal multi-view video-to-video translation framework that enables consistent appearance across multiple synchronized camera views, overcoming limitations of single-view approaches for embodied AI applications.


<details>
  <summary>Details</summary>
Motivation: Embodied AI tasks use multiple synchronized cameras for policy learning, but existing video-to-video translation methods only work on single views. Applying single-view models independently to each camera leads to inconsistent appearance across views, and standard transformer architectures don't scale to multi-view settings due to quadratic attention costs.

Method: VideoWeaver starts as a single-view flow-based V2V model, then extends to multi-view by grounding all views in a shared 4D latent space from Pi3 spatial foundation model. To scale beyond fixed camera numbers, it trains views at distinct diffusion timesteps to learn both joint and conditional view distributions, enabling autoregressive synthesis of new viewpoints.
...
Conclusion: VideoWeaver enables consistent multi-view video translation essential for embodied AI, overcoming previous single-view limitations and scaling challenges, with applications in world randomization for robot policy transfer to new environments.
...
### [129] [DC-Reg: Globally Optimal Point Cloud Registration via Tight Bounding with Difference of Convex Programming](https://arxiv.org/abs/2603.25442)
*Wei Lian,Fei Ma,Hang Pan,Zhesen Cui,Wangmeng Zuo*

Main category: cs.CV

TL;DR: DC-Reg: A globally optimal point cloud registration framework using Difference of Convex programming to tighten Branch-and-Bound search, achieving faster convergence and better robustness to noise/outliers.


<details>
  <summary>Details</summary>
Motivation: Existing global registration methods struggle with local minima (heuristic methods) or slow convergence (global solvers) due to loose lower bounds in the coupled transformation-correspondence estimation problem, especially under partial overlaps and large misalignments.

Method: Proposes DC-Reg framework with holistic concave underestimator for the coupled objective using Difference of Convex programming. Captures joint structural interaction between transformation and correspondence variables, enabling tight lower bounds via Linear Assignment Problems evaluated at search box vertices. Uses rotation-invariant features for 3D rigid registration efficiency.

Result: DC-Reg achieves significantly faster convergence and superior robustness to extreme noise and outliers compared to state-of-the-art global techniques, validated on synthetic data and 3DMatch benchmark.

Conclusion: The holistic DC decomposition approach provides a breakthrough in globally optimal point cloud registration by enabling tight lower bounds that dramatically accelerate Branch-and-Bound search while maintaining robustness to challenging conditions.
...
### [130] [CIAR: Interval-based Collaborative Decoding for Image Generation Acceleration](https://arxiv.org/abs/2603.25463)
*Keming Ye,Zhou Zhao,Fan Wu,Shengyu Zhang*

Main category: cs.CV

TL;DR: CIAR is a cloud-device collaboration framework that accelerates auto-regressive image generation by using on-device self-verification with continuous probability intervals to handle visual token uncertainty and spatial redundancy.
...
Method: CIAR uses on-device token uncertainty quantifier with continuous probability intervals instead of discrete solution sets, plus an interval-enhanced decoding module with distribution alignment training to handle visual vocabulary size and spatial redundancy.

Result: Achieves 2.18x speed-up, reduces cloud requests by 70%, while preserving image quality compared to existing methods.

Conclusion: CIAR effectively addresses the latency and computational challenges of AR image generation on devices through cloud-device collaboration with uncertainty-aware verification.
...
### [131] [GridVAD: Open-Set Video Anomaly Detection via Spatial Reasoning over Stratified Frame Grids](https://arxiv.org/abs/2603.25467)
*Mohamed Eltahir,Ahmed O. Ibrahim,Obada Siralkhatim,Tabarak Abdallah,Sondos Mohamed*

Main category: cs.CV

TL;DR: GridVAD is a training-free pipeline that uses Vision-Language Models as anomaly proposers, with spatial grounding and temporal propagation modules, achieving state-of-the-art zero-shot anomaly detection in video surveillance.


<details>
  <summary>Details</summary>
Motivation: Direct use of Vision-Language Models as anomaly detectors in video surveillance is fragile due to uncalibrated anomaly priors, leading to missed detections and false alarms. The problem is not the VLM itself but how it's used - VLMs should function as anomaly proposers rather than direct detectors.

Method: GridVAD follows a propose-ground-propagate principle: 1) VLM reasons over stratified grid representations to generate natural-language anomaly proposals, 2) Self-Consistency Consolidation filters hallucinations by retaining only recurring proposals, 3) Grounding DINO anchors proposals to bounding boxes, and 4) SAM2 propagates them as dense masks through anomaly intervals. The pipeline uses fixed M+1 VLM calls per clip regardless of video length.
...
Conclusion: VLMs should be used as anomaly proposers rather than direct detectors, with purpose-built spatial and temporal modules for grounding and propagation. GridVAD demonstrates that training-free pipelines can achieve state-of-the-art performance through proper architectural design, offering efficiency and controllable precision-recall tradeoffs.
...
### [132] [AdaSFormer: Adaptive Serialized Transformers for Monocular Semantic Scene Completion from Indoor Environments](https://arxiv.org/abs/2603.25494)
...
Conclusion: AdaSFormer effectively addresses the limitations of transformers in indoor MSSC by combining adaptive serialized transformers with specialized positional encoding and normalization techniques, achieving superior performance on benchmark datasets.
...
### [133] [RealRestorer: Towards Generalizable Real-World Image Restoration with Large-Scale Image Editing Models](https://arxiv.org/abs/2603.25502)
...
TL;DR: The paper addresses real-world image restoration by creating a large-scale dataset covering 9 degradation types and training an open-source model to compete with closed-source alternatives, introducing RealIR-Bench for evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing restoration models have poor generalization to real-world scenarios due to limited training data scale and distribution. While large-scale closed-source models show strong restoration abilities, they require substantial data and computational costs.

Method: Constructed a large-scale dataset covering nine common real-world degradation types, trained a state-of-the-art open-source model, and introduced RealIR-Bench containing 464 real-world degraded images with tailored evaluation metrics focusing on degradation removal and consistency preservation.

Result: Extensive experiments demonstrate the model ranks first among open-source methods, achieving state-of-the-art performance in real-world image restoration.

Conclusion: The proposed approach successfully narrows the gap between open-source and closed-source image restoration models by addressing data limitations and providing comprehensive evaluation benchmarks for real-world scenarios.
...
### [134] [Beyond the Golden Data: Resolving the Motion-Vision Quality Dilemma via Timestep Selective Training](https://arxiv.org/abs/2603.25527)
...
TL;DR: Introduces BFMD - first badminton full-match dataset with dense multimodal annotations, and a VideoMAE-based captioning framework with semantic feedback for improved shot caption generation.


<details>
  <summary>Details</summary>
Motivation: Existing badminton datasets focus on short clips or task-specific annotations, lacking full-match data with dense multimodal annotations, making it difficult to generate accurate shot captions and perform match-level tactical analysis.

Method: Created BFMD dataset with 19 broadcast matches (20+ hours) covering singles/doubles, 1,687 rallies, 16,751 hit events with hierarchical annotations. Developed VideoMAE-based multimodal captioning framework with Semantic Feedback mechanism that leverages shot semantics to guide caption generation.

Result: Multimodal modeling and semantic feedback improve shot caption quality over RGB-only baselines. Dataset enables analysis of temporal evolution of tactical patterns across full matches.

Conclusion: BFMD addresses limitations of existing datasets and enables comprehensive match-level tactical analysis in badminton through dense multimodal annotations and improved captioning framework.
...
Method: PAWS directly extracts object articulations from hand-object interactions in large-scale in-the-wild egocentric videos, eliminating the need for supervised 3D data or manual annotations.
...
Conclusion: PAWS provides a scalable approach to articulation perception that learns from real-world egocentric videos, overcoming limitations of supervised methods and enabling practical applications in robotics and 3D scene understanding.
...
TL;DR: Multimodal fusion of surgical microscope and intraoperative OCT improves instrument tracking and tool-tissue distance estimation in vitreoretinal surgery with real-time performance.


<details>
  <summary>Details</summary>
Motivation: To enhance surgical scene understanding in ophthalmic surgery by fusing complementary imaging modalities (operating microscope and intraoperative OCT) for more precise instrument tracking and tool-tissue distance estimation.

Method: Proposed a multimodal, temporal, real-time network architecture with cross-attention fusion module to merge OPMI and iOCT features, using YoloNAS and CNN encoders, plus region-based recurrent module for temporal coherence.

Result: Achieved 95.79% mAP50 for instrument localization/keypoint detection, real-time processing at 22.5 ms/frame, and significantly improved tool-tissue distance estimation from 284μm (OPMI only) to 33μm (multimodal) for distances below 1mm.

Conclusion: Multimodal feature fusion enhances multi-task prediction accuracy over single-modality processing, demonstrating potential for image-guided vitreoretinal surgery while highlighting challenges for future research in surgical scene understanding.
...
TL;DR: The paper addresses the "vertical blind spot" in Earth Observation LMMs by introducing height-aware evaluation benchmarks and a baseline model that integrates height features with visual semantics.


<details>
  <summary>Details</summary>
Motivation: Current Large Multimodal Models in Earth Observation neglect the critical vertical dimension, limiting their reasoning capabilities in complex remote sensing geometries and disaster scenarios where physical spatial structures are more important than planar visual textures.
...
Result: The pipeline constructed comprehensive height-aware benchmarks, and the GeoHeightChat baseline successfully demonstrated that integrating height features with visual semantics can mitigate the "vertical blind spot" in existing optical models.

Conclusion: The work bridges the gap in height-aware remote sensing understanding, unlocking a new paradigm of interactive height reasoning in Earth Observation models by addressing the previously neglected vertical dimension.
...
TL;DR: UNIC introduces a neural deformation field method for real-time garment animation that learns instance-specific deformation fields to animate garment meshes given motion sequences, avoiding topology handling and improving quality.


<details>
  <summary>Details</summary>
Motivation: Physics simulation methods for garment deformation are time-consuming, computationally demanding, and require costly hardware, making them unsuitable for real-time applications. Recent learning-based methods using graph neural networks fail to capture intricate deformations of complex garment meshes with complex topologies.

Method: UNIC learns instance-specific neural deformation fields to animate garment meshes. The method maps 3D points to their deformation offsets, avoiding handling of complex garment topologies and injecting natural smoothness constraints. It focuses on learning for specific garments rather than generalizing to new ones.

Result: Extensive experiments on various garment meshes demonstrate UNIC's effectiveness and efficiency over baseline methods. The approach achieves real-time performance suitable for interactive applications like video games.

Conclusion: UNIC provides a practical and useful solution for real-world interactive applications by enabling real-time garment animation through instance-specific neural deformation fields, overcoming limitations of physics simulation and previous learning-based methods.
...
TL;DR: Automated pipeline translates natural language imaging system descriptions into validated forward models using spec.md format and three autonomous agents, matching expert quality across 6 real-data modalities.


<details>
  <summary>Details</summary>
Motivation: Current computational imaging system design requires weeks of specialist effort per modality, creating an expertise bottleneck that excludes broader scientific community from prototyping imaging instruments.

Method: Introduces spec.md structured specification format and three autonomous agents (Plan, Judge, Execute) that translate natural-language descriptions into validated forward models. Uses design-to-real error theorem decomposing total reconstruction error into five independently bounded terms.
...
Conclusion: The approach democratizes computational imaging system design by automating the translation from natural language descriptions to validated forward models, enabling broader scientific community participation in instrument prototyping.
...
Method: LEMMA leverages Laplacian Pyramids to enhance edge recognition early in feature extraction, eliminating the need for computationally expensive feature map computations in deeper network layers. This reduces model size, complexity, and inference time while maintaining accuracy.

Result: LEMMA achieves state-of-the-art performance with significant reductions: 71x fewer trainable parameters, 88.5% reduction in GFLOPs, and 84.65% reduction in inference time compared to existing models. It achieves 93.42% IoU on Oil Spill dataset and 98.97% mIoU on Mastr1325.

Conclusion: LEMMA provides an effective lightweight solution for marine semantic segmentation that balances accuracy with computational efficiency, making it suitable for real-time applications in resource-constrained marine environments.
...
TL;DR: Wan-Weaver is a framework for interleaved text-image generation that decomposes the task into planning and visualization, achieving state-of-the-art performance without real interleaved training data.


<details>
  <summary>Details</summary>
Motivation: Current unified models accept multi-modal inputs but produce only single-modality outputs. The challenge of generating interleaved content stems from training data scarcity and difficulty in modeling long-range cross-modal context.

Method: Decomposes interleaved generation into textual planning and visual consistency modeling. Uses a planner to produce dense textual descriptions for visual content, and a visualizer to synthesize images accordingly. Constructs large-scale textual-proxy interleaved data for planner training and reference-guided image data for visualizer training.

Result: Wan-Weaver exhibits emergent interleaved generation ability with long-range textual coherence and visual consistency. Achieves robust task reasoning and generation proficiency. Outperforms existing methods without access to real interleaved data.

Conclusion: The proposed decomposition approach enables effective interleaved generation despite data scarcity. The framework demonstrates superior performance and establishes a benchmark for evaluating interleaved generation capabilities across diverse use cases.
...
TL;DR: Trace: A framework for editing object motion paths in videos by designing trajectories in a single anchor frame and generating consistent edited videos.


<details>
  <summary>Details</summary>
Motivation: Prior video editing methods focus on appearance manipulation or require complex point-track-based trajectory control that is difficult for users, especially in videos with camera motion. There's a need for a practical, easy-to-use approach for controllable object-centric motion editing.

Method: Two-stage pipeline: 1) Cross-view motion transformation module that maps first-frame path design to frame-aligned box trajectories under camera motion, 2) Motion-conditioned video re-synthesis module that follows these trajectories to regenerate the object while preserving the remaining video content.
...
Conclusion: Trace provides a practical framework for object motion path editing that enables users to easily design desired trajectories in a single anchor frame and generates temporally consistent edited videos with better quality than existing approaches.
...
TL;DR: VISAGE is a training-free decoding framework that addresses multimodal hallucinations in MDLLMs by calibrating the objective mismatch between language probability and visual grounding through cross-attention spatial entropy analysis.


<details>
  <summary>Details</summary>
Motivation: MDLLMs suffer from multimodal hallucinations due to an algorithmic flaw where decoders rank tokens based only on textual likelihood without verifying visual support, creating an objective mismatch between language probability and multimodal task requirements.
...
Result: VISAGE achieves relative gains of 8.59% on MMMU-val and 7.75% on HallusionBench, with analytical stability guarantees showing bounded objective loss under estimation error.

Conclusion: VISAGE effectively addresses multimodal hallucinations in MDLLMs through training-free objective calibration at inference time, providing robust performance improvements on hallucination-sensitive benchmarks while maintaining analytical stability.
...
TL;DR: AnyHand is a large-scale synthetic RGB-D dataset for 3D hand pose estimation that significantly improves performance and generalization across multiple benchmarks.
...
Method: Created a large-scale synthetic dataset (2.5M single-hand + 4.1M hand-object interaction RGB-D images) with rich geometric annotations, and developed a lightweight depth fusion module that integrates with existing RGB-based models.
...
Conclusion: AnyHand effectively addresses data limitations in hand pose estimation, demonstrating that large-scale synthetic data with comprehensive features can substantially improve performance, robustness, and generalization across different settings.
...
TL;DR: BizGenEval is a benchmark for evaluating image generation models on commercial visual content creation tasks across 5 document types and 4 capability dimensions.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on natural image synthesis but fail to evaluate models under the structured, multi-constraint requirements of real-world commercial design tasks.

Method: Created BizGenEval benchmark with 5 document types (slides, charts, webpages, posters, scientific figures) and 4 capability dimensions (text rendering, layout control, attribute binding, knowledge-based reasoning). Contains 400 curated prompts and 8000 human-verified checklist questions.

Result: Evaluated 26 popular image generation systems (commercial APIs and open-source models). Results show substantial capability gaps between current generative models and professional visual content creation requirements.

Conclusion: BizGenEval serves as a standardized benchmark for real-world commercial visual content generation, highlighting the need for improved models in professional design applications.
...
TL;DR: SlotVTG is a lightweight framework that improves MLLMs' out-of-domain generalization for Video Temporal Grounding by adding object-centric reasoning capabilities through slot attention, without requiring full retraining.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs for Video Temporal Grounding have poor out-of-domain generalization because they memorize dataset shortcuts during fine-tuning rather than learning to ground in actual visual content. Object-centric learning could help but existing approaches require expensive full retraining.

Method: SlotVTG introduces a lightweight slot adapter that decomposes visual tokens into abstract slots via slot attention and reconstructs the original sequence. It uses objectness priors from a self-supervised vision model to encourage semantically coherent slot formation, steering MLLMs toward object-centric reasoning with minimal overhead.
...
Conclusion: SlotVTG provides an efficient way to enhance MLLMs' fine-grained temporal understanding and OOD generalization for Video Temporal Grounding by incorporating object-centric reasoning through lightweight slot adaptation, avoiding the need for expensive full retraining.
...
TL;DR: LIGHT is a diffusion-based method for human-object interaction animation that uses pace-induced guidance from denoising schedules instead of hand-crafted contact priors, achieving better contact fidelity and generalization.


<details>
  <summary>Details</summary>
Motivation: Generating realistic human-object interaction animations is challenging due to the need to jointly model dynamic human actions and diverse object geometries. Prior methods rely on hand-crafted contact priors or human-imposed kinematic constraints, which limits their effectiveness.

Method: LIGHT uses data-driven guidance emerging from denoising pace itself. It factors representations into modality-specific components with individualized noise levels and asynchronous denoising schedules. Cleaner components guide noisier ones through cross-attention, creating guidance without auxiliary classifiers. Training is augmented with synthetic object geometries to encourage invariance of contact semantics to geometric diversity.

Result: The method achieves higher contact fidelity, more realistic HOI generation, and stronger generalization to unseen objects and tasks. Pace-induced guidance more effectively mirrors the benefits of contact priors than conventional classifier-free guidance.

Conclusion: LIGHT demonstrates that data-driven guidance emerging from denoising pace can effectively replace hand-crafted contact priors for human-object interaction animation, offering improved contact awareness and generalization capabilities.
...
TL;DR: The paper proposes a method to quantify skill in table tennis by learning generative models of players' tactical racket strokes and embedding them in a latent space that captures individual characteristics including skill levels.
...
Method: Learn generative models of each player's tactical racket strokes and jointly embed them in a common latent space that encodes individual characteristics including skill levels. Train on large-scale 3D-reconstructed professional matches, conditioning on comprehensive game context (player positioning and opponent behaviors). Then train a simple relative ranking network on these embeddings.

Result: The learned player space reflects distinct play styles and attributes representing skill. Both relative and absolute skill predictions can be achieved using the embeddings. The player space effectively quantifies skill levels.

Conclusion: The learned player space provides a foundation for automated skill assessment in complex, interactive behaviors, demonstrating that skill can be effectively quantified through latent space embeddings of tactical actions.
...
TL;DR: PSDesigner is an automated graphic design system that mimics human designer workflows using specialized components and a large annotated PSD dataset to translate user intentions into editable design files.
...
Method: PSDesigner emulates human designer workflows using multiple specialized components. It collects theme-related assets based on user instructions, autonomously infers and executes tool calls to manipulate design files. The system is trained on CreativePSD, a large dataset of high-quality PSD files annotated with operation traces across diverse design scenarios and styles.

Result: PSDesigner outperforms existing methods across diverse graphic design tasks, enabling non-specialists to conveniently create production-quality designs.

Conclusion: PSDesigner successfully addresses limitations of previous automated design systems by mimicking human creative workflows and leveraging specialized training data, making professional-quality graphic design accessible to non-specialists.
...
TL;DR: MegaFlow is a zero-shot large displacement optical flow model that leverages pre-trained Vision Transformer features for global matching, achieving SOTA performance without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing optical flow methods struggle with large displacements and lack zero-shot generalization due to reliance on iterative local search and domain-specific fine-tuning.

Method: Formulates flow estimation as global matching using pre-trained Vision Transformer features to capture large displacements, followed by lightweight iterative refinements for sub-pixel accuracy.

Result: Achieves state-of-the-art zero-shot performance on multiple optical flow benchmarks and competitive results on long-range point tracking benchmarks.

Conclusion: MegaFlow demonstrates robust transferability and suggests a unified paradigm for generalizable motion estimation by leveraging powerful pre-trained vision priors.
...
TL;DR: RefAlign is a training-time framework that aligns reference image features to visual foundation model semantics to reduce copy-paste artifacts in reference-to-video generation, improving identity consistency without inference overhead.


<details>
  <summary>Details</summary>
Motivation: Existing R2V methods struggle with copy-paste artifacts and multi-subject confusion due to modality mismatch between heterogeneous encoder features, even with auxiliary semantic guidance.

Method: Proposes RefAlign with a reference alignment loss that pulls reference features and VFM features of the same subject closer for identity consistency, while pushing apart features of different subjects for semantic discriminability.
...
Conclusion: Explicit reference alignment during training effectively addresses R2V challenges without inference overhead, demonstrating the value of aligning DiT features to visual foundation model semantics.
...
TL;DR: MuRF is a training-free inference strategy that fuses multi-resolution features from frozen vision foundation models to improve performance across various computer vision tasks.
...
Method: MuRF processes images at multiple resolutions through a frozen VFM, then fuses the resulting features to create a unified representation without any training.

Result: The method works universally across different VFM families (DINOv2, SigLIP2) and various computer vision tasks, demonstrating effectiveness as a fundamental enhancement to visual representation.

Conclusion: MuRF provides a simple, training-free way to leverage multi-resolution inductive biases during inference, offering a universally applicable improvement to vision foundation models.
...
<details>
  <summary>Details</summary>
Motivation: Existing feed-forward 3D Gaussian Splatting methods suffer from quadratic growth in primitive count as resolution increases, making high-resolution synthesis like 4K intractable due to scalability limitations.
...
Result: Enables high-fidelity 4K novel view synthesis without per-scene optimization, a capability previously unattainable for feed-forward methods, while using significantly fewer Gaussian primitives.

Conclusion: LGTM overcomes the resolution scaling barrier in feed-forward 3D Gaussian Splatting, making high-resolution synthesis tractable through a texture-based approach that separates geometry from rendering resolution.
...
Method: 1) Reformulate video generation as next-shot prediction conditioned on historical context; 2) Fine-tune text-to-video model into bidirectional next-shot generator; 3) Distill into causal student via Distribution Matching Distillation; 4) Introduce dual-cache memory mechanism (global context cache for inter-shot consistency, local context cache for intra-shot consistency); 5) Use RoPE discontinuity indicator to distinguish caches; 6) Two-stage distillation strategy with intra-shot self-forcing and inter-shot self-forcing.
...
Conclusion: ShotStream enables interactive multi-shot video generation with high coherence and low latency, paving the way for real-time interactive storytelling applications where users can dynamically guide narratives through streaming prompts.
...
### [156] [Coronary artery calcification assessment in National Lung Screening Trial CT images (DeepCAC2)](https://arxiv.org/abs/2603.24633)
*Leonard Nürnberg,Simon Bernatz,Borek Foldyna,Michael T. Lu,Andrey Fedorov,Hugo JWL Aerts*

Main category: eess.IV

TL;DR: DeepCAC2 is a public dataset with automated coronary artery calcification segmentations and risk scores from 127,776 low-dose chest CT scans, created using a deep learning pipeline trained on expert-annotated cardiac CT data.


<details>
  <summary>Details</summary>
Motivation: Coronary artery calcification (CAC) is a strong cardiovascular risk predictor but remains underutilized in clinical routine thoracic imaging due to the need for dedicated imaging protocols and manual annotation.

Method: Used a fully automated deep learning pipeline trained on expert-annotated cardiac CT data to process 127,776 CT scans from 26,228 individuals from the National Lung Screening Trial (NLST), generating standardized CAC segmentations and risk estimates.

Result: Created a publicly available dataset with automated CAC segmentations, coronary artery calcium scores, and derived risk categories, along with a public dashboard for visual inspection of 200 NLST patients.
...
### [157] [Subject-Specific Low-Field MRI Synthesis via a Neural Operator](https://arxiv.org/abs/2603.24968)
*Ziqi Gao,Nicha Dvornek,Xiaoran Zhang,Gigi Galiana,Hemant Tagare,Todd Constable*

Main category: eess.IV

TL;DR: H2LO: A neural operator framework for simulating low-field MRI from high-field MRI that captures realistic contrast degradation and noise textures, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Low-field MRI is more accessible and cost-effective but suffers from lower signal-to-noise ratios and degraded contrast compared to high-field MRI. Existing simulators using noise injection and smoothing fail to capture realistic contrast degradation, limiting their utility for device evaluation and algorithm development.

Method: Introduces H2LO (HF to LF coordinate-image decoupled neural operator), an end-to-end LF-MRI synthesis framework that learns the degradation process directly from paired HF-LF MRI data. The method decouples coordinate and image representations to capture both high-frequency noise textures and image structure.
...
Conclusion: The H2LO framework effectively simulates realistic low-field MRI from high-field data, showing potential to enhance LF MRI diagnostic capabilities by enabling better virtual evaluation of imaging devices and development of LF algorithms.
...
### [158] [Underdetermined Blind Source Separation via Weighted Simplex Shrinkage Regularization and Quantum Deep Image Prior](https://arxiv.org/abs/2603.25384)
*Chia-Hsiang Lin,Si-Sheng Young*

Main category: eess.IV

TL;DR: Proposes GQ-μ algorithm for multispectral unmixing using quantum deep image prior to create virtual hyperspectral images, then applies weighted simplex shrinkage regularization for ill-posed hyperspectral unmixing.


<details>
  <summary>Details</summary>
Motivation: Multispectral unmixing is challenging due to underdetermined blind source separation (more sources than bands), unlike hyperspectral unmixing which is overdetermined but still ill-posed.

Method: 1) Use quantum deep image prior (QDIP) to generate virtual hyperspectral images from multispectral images via virtual band-splitting; 2) Apply hyperspectral unmixing with weighted simplex shrinkage (WSS) regularization based on convex geometry; 3) Spectrally downsample to obtain multispectral sources.

Result: GQ-μ algorithm effectively obtains spatial abundance distribution maps and performs well in simulation and real-world data experiments; QDIP outperforms classical DIP and WSS regularization is validated.

Conclusion: The unsupervised GQ-μ algorithm successfully addresses challenging multispectral unmixing by combining quantum-inspired virtual band generation with geometry-based regularization.
...
### [159] [Language-Free Generative Editing from One Visual Example](https://arxiv.org/abs/2603.25441)
*Omar Elezabi,Eduard Zamfir,Zongwei Wu,Radu Timofte*

Main category: eess.IV

TL;DR: VDC is a training-free framework that uses visual examples instead of text to guide diffusion models for precise image editing, overcoming limitations of text-based methods for simple visual transformations.


<details>
  <summary>Details</summary>
Motivation: Current text-guided diffusion models struggle with simple everyday transformations like rain or blur due to weak textual supervision during training, leading to poor vision-language alignment. Existing solutions require expensive finetuning or stronger text conditioning.

Method: Visual Diffusion Conditioning (VDC) learns conditioning signals directly from paired visual examples (with/without target effect) through a novel condition-steering mechanism. Includes inversion-correction step to mitigate DDIM inversion errors and preserve detail.

Result: VDC outperforms both training-free and fully fine-tuned text-based editing methods across diverse tasks, demonstrating precise language-free image editing capabilities.

Conclusion: The paper introduces a vision-centric paradigm for diffusion-based editing that perceives visual changes like humans do, enabling cost-efficient editing without text supervision or expensive training.
...
### [160] [A Mamba-based Perceptual Loss Function for Learning-based UGC Transcoding](https://arxiv.org/abs/2603.25566)
*Zihao Qi,Chen Feng,Fan Zhang,Xiaozhong Xu,Shan Liu,David Bull*

Main category: eess.IV

TL;DR: A novel perceptually-inspired loss function for UGC video transcoding that treats reference videos as contextual guides rather than pixel-perfect ground truth, achieving significant coding gains over existing neural codecs.


<details>
  <summary>Details</summary>
Motivation: Traditional video compression paradigms optimize for fidelity to reference videos, but this becomes suboptimal for UGC content where source videos often contain compression artifacts, editing issues, or capture problems. Forcing codecs to replicate these inherent artifacts leads to poor perceptual quality.

Method: Proposes a perceptually-inspired loss function that redefines reference videos as contextual guides rather than ground truth. Trains a lightweight neural quality model using Selective Structured State-Space Model (Mamba) with weakly-supervised Siamese ranking strategy. Integrates this model into rate-distortion optimization (RDO) of neural video codecs (DCVC and HiNeRV).

Result: Achieves substantial coding gains: 8.46% BD-rate savings over autoencoder baseline (DCVC) and 12.89% BD-rate savings over implicit neural representation baseline (HiNeRV). The framework improves perceptual quality of reconstructed UGC content.

Conclusion: The proposed approach successfully addresses the limitations of traditional fidelity-based optimization for UGC transcoding by treating reference videos as contextual guides rather than pixel-perfect targets, leading to significant perceptual quality improvements and coding efficiency gains.
...
### [161] [Colon-Bench: An Agentic Workflow for Scalable Dense Lesion Annotation in Full-Procedure Colonoscopy Videos](https://arxiv.org/abs/2603.25645)
*Abdullah Hamdi,Changchun Yang,Xin Gao*

Main category: eess.IV

TL;DR: Colon-Bench: A comprehensive benchmark dataset for colonoscopy AI, featuring 528 videos with 14 lesion types, 300K+ bounding boxes, and 133K words of clinical descriptions, used to evaluate MLLMs on medical tasks.


<details>
  <summary>Details</summary>
Motivation: Early colonoscopy screening is crucial for colon cancer prevention, but AI development is hindered by lack of densely annotated, long-sequence video datasets with rich spatial, temporal, and linguistic annotations needed for modern MLLM evaluation.

Method: Created Colon-Bench using a novel multi-stage agentic workflow integrating temporal proposals, bounding-box tracking, AI-driven visual confirmation, and human-in-the-loop review to scalably annotate full-procedure videos.

Result: Generated unprecedented benchmark with 528 videos, 14 lesion categories, over 300K bounding boxes, 213K segmentation masks, and 133K words of clinical descriptions. MLLMs showed surprisingly high localization performance compared to SAM-3, and novel "colon-skill" prompting improved zero-shot performance by up to 9.7%.

Conclusion: Colon-Bench addresses critical data gap for colonoscopy AI, enabling rigorous MLLM evaluation and revealing promising medical localization capabilities. The dataset and code are publicly available to advance research in this domain.
...
TL;DR: DyMRL is a dynamic multispace representation learning approach for multimodal event forecasting that addresses dynamic knowledge acquisition and fusion across Euclidean, hyperbolic, and complex spaces.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal event forecasting methods overlook dynamic acquisition and fusion of multimodal knowledge. They fail to capture time-sensitive structural information and evolving multimodal fusion features, as static approaches can't handle varying historical contributions of different modalities.
...
Result: DyMRL outperforms state-of-the-art dynamic unimodal and static multimodal baseline methods on four newly constructed multimodal temporal knowledge graph benchmarks.

Conclusion: DyMRL effectively addresses dynamic multimodal knowledge acquisition and fusion for event forecasting by integrating multispace representations and adaptive attention mechanisms, demonstrating superior performance over existing approaches.
...
### [163] [How unconstrained machine-learning models learn physical symmetries](https://arxiv.org/abs/2603.24638)
*Michelangelo Domina,Joseph William Abbott,Paolo Pegolo,Filippo Bigi,Michele Ceriotti*

Main category: cs.LG

TL;DR: This paper introduces metrics to measure symmetry content in ML models for physics, analyzes how unconstrained models learn approximate equivariance, and shows how minimal inductive biases can improve stability and accuracy while preserving expressivity.


<details>
  <summary>Details</summary>
Motivation: Physical simulations require predictions that respect fundamental symmetries, but unconstrained models often perform competitively while learning approximate equivariant behavior through data augmentation. The paper aims to rigorously measure how these models process symmetry information and understand their learning mechanisms.

Method: The authors introduce rigorous metrics to quantify symmetry content in learned representations and assess equivariance accuracy. They apply these metrics to two unconstrained transformer-based models: a graph neural network for atomistic simulations and a PointNet-style architecture for particle physics. They analyze how symmetry information flows across architectural layers and is learned during training.

Result: The analysis reveals how unconstrained models process symmetry information and identifies spectral failure modes. Based on these insights, the authors demonstrate that strategically injecting minimal required inductive biases can achieve superior stability and accuracy while preserving the high expressivity and scalability of unconstrained architectures.

Conclusion: The paper establishes a rigorous framework for diagnosing symmetry-related failure modes in ML models and shows that targeted minimal inductive biases can guarantee physical fidelity without sacrificing the benefits of unconstrained architectures, offering a balanced approach between pure data-driven learning and hard-coded symmetries.
...
### [164] [Experiential Reflective Learning for Self-Improving LLM Agents](https://arxiv.org/abs/2603.24639)
*Marc-Antoine Allard,Arnaud Teinturier,Victor Xing,Gautier Viaud*

Main category: cs.LG

TL;DR: ERL is a self-improvement framework where agents reflect on past task experiences to generate transferable heuristics, which are then retrieved and injected into context to guide future task execution, improving adaptation and success rates.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based autonomous agents struggle with specialized environment adaptation and don't leverage accumulated experience, approaching each new task from scratch despite having past interactions that could inform better performance.
...
Conclusion: Reflecting on single-attempt experiences to extract transferable heuristics enables effective agent self-improvement, demonstrating that experiential learning through heuristic generation and retrieval can significantly enhance LLM agent adaptation and performance.
...
### [165] [Learning Mesh-Free Discrete Differential Operators with Self-Supervised Graph Neural Networks](https://arxiv.org/abs/2603.24641)
...
Method: A parametrised framework using graph neural networks trained via polynomial moment constraints from truncated Taylor expansions. The model maps local stencil relative positions directly to discrete operator weights, learning classical polynomial consistency while being robust to irregular geometry.
...
Conclusion: Neural networks can effectively learn mesh-free discrete differential operators that maintain polynomial consistency while being robust to irregular geometry, offering a promising approach for flexible discretizations of complex geometries with good accuracy-efficiency balance.
...
### [166] [Physics-Informed Neural Network Digital Twin for Dynamic Tray-Wise Modeling of Distillation Columns under Transient Operating Conditions](https://arxiv.org/abs/2603.24644)
*Debadutta Patra,Ayush Bardhan Tripathy,Soumya Ranjan Sahu,Sucheta Panda*

Main category: cs.LG

TL;DR: PINN digital twin framework for binary distillation columns combines physics constraints with neural networks, achieving 44.6% better accuracy than data-only models while enforcing thermodynamic consistency.


<details>
  <summary>Details</summary>
Motivation: Digital twin technology combined with physics-informed machine learning can transform industrial process monitoring, control, and optimization for distillation columns operating under transient conditions.

Method: Physics-Informed Neural Network (PINN) framework that embeds thermodynamic constraints (VLE via modified Raoult's law, mass/energy balances, McCabe-Thiele method) into neural network loss functions, trained on synthetic Aspen HYSYS data with adaptive loss-weighting.

Result: Achieved RMSE of 0.00143 for HX mole fraction prediction (R² = 0.9887), 44.6% reduction over best data-only baseline, while strictly satisfying thermodynamic constraints and accurately capturing column dynamics under transient perturbations.

Conclusion: The PINN digital twin provides a robust foundation for real-time soft sensing, model-predictive control, and anomaly detection in industrial distillation processes, outperforming purely data-driven approaches.
...
### [167] [Can LLMs Beat Classical Hyperparameter Optimization Algorithms? A Study on autoresearch](https://arxiv.org/abs/2603.24647)
*Fabio Ferreira,Lucca Wobbe,Arjun Krishnakumar,Frank Hutter,Arber Zela*

Main category: cs.LG

TL;DR: LLM agents can edit training code for hyperparameter optimization but classical methods (CMA-ES, TPE) still outperform them in fixed search spaces. A hybrid approach (Centaur) combining CMA-ES state with LLM domain knowledge achieves best results, with even small 0.8B LLMs being sufficient when paired with classical optimizers.


<details>
  <summary>Details</summary>
Motivation: To compare classical hyperparameter optimization methods against LLM-based approaches, especially when LLMs can directly edit training code in unconstrained search spaces, and to explore whether hybrid approaches can combine the strengths of both methods.

Method: Used autoresearch repository as testbed for comparing CMA-ES and TPE against LLM-based agents. Introduced Centaur - a hybrid approach that shares CMA-ES's internal state (mean vector, step-size, covariance matrix) with an LLM to combine classical optimization with domain knowledge.

Result: Classical HPO methods outperform LLM-based agents in fixed search spaces. LLM agents editing code in unconstrained spaces narrow the gap. Centaur hybrid achieves best results, with 0.8B variant outperforming 27B variant. Reliability matters more than exploration breadth. Small LLMs (0.8B) are sufficient for hybrid optimization but insufficient for unconstrained code editing.

Conclusion: Hybrid approaches combining classical optimization with LLM domain knowledge are most effective. Small LLMs paired with strong classical optimizers can achieve excellent results, while scaling to larger models provides no advantage for fixed search space methods with open-weight models tested.
...
### [168] [Energy-Efficient Hierarchical Federated Anomaly Detection for the Internet of Underwater Things via Selective Cooperative Aggregation](https://arxiv.org/abs/2603.24648)
*Kenechi Omeke,Michael Mollel,Lei Zhang,Qammer H. Abbasi,Muhammad Ali Imran*

Main category: cs.LG

TL;DR: Hierarchical federated learning framework for underwater anomaly detection that reduces energy use by 71-95% while maintaining full sensor participation through short-range clusters and selective fog cooperation.


<details>
  <summary>Details</summary>
Motivation: Standard flat federated learning faces two limitations in underwater deployments: expensive long-range acoustic transmissions and reduced participation when only a subset of sensors can directly reach the gateway due to low-bandwidth, energy-intensive acoustic links.

Method: Three-tier hierarchical federated learning with: 1) feasibility-aware sensor-to-fog association, 2) compressed model-update transmission, and 3) selective cooperative aggregation among fog nodes. Uses short-range clusters and activates fog-to-fog exchange only when smaller clusters benefit from nearby larger neighbors.

Result: In 200-sensor deployments, hierarchical learning preserves full participation (vs 48% in flat FL), selective cooperation reduces energy by 31-33% while matching accuracy, and compressed uploads reduce total energy by 71-95%. Real benchmark experiments show competitive detection quality with low overhead.

Conclusion: The hierarchical framework provides practical design guidance for underwater deployments under severe acoustic constraints, balancing energy efficiency, full network participation, and detection accuracy through localized communication and selective cooperation.
...
### [169] [Amplified Patch-Level Differential Privacy for Free via Random Cropping](https://arxiv.org/abs/2603.24695)
*Kaan Durmaz,Jan Schuchardt,Sebastian Schmidt,Stephan Günnemann*

Main category: cs.LG

TL;DR: Random cropping in vision models provides free differential privacy amplification by probabilistically excluding sensitive localized content, improving privacy-utility tradeoffs without architecture changes.


<details>
  <summary>Details</summary>
Motivation: Random cropping is widely used in computer vision but its privacy implications haven't been studied. When sensitive content (faces, license plates) is spatially localized, random cropping can probabilistically exclude it, providing additional privacy protection beyond standard DP-SGD mechanisms.

Method: Introduce patch-level neighboring relation for vision data, derive tight privacy bounds for DP-SGD combined with random cropping. Quantify patch inclusion probability and show how it composes with minibatch sampling to lower effective sampling rate.

Result: Patch-level amplification improves privacy-utility trade-off across multiple segmentation architectures and datasets. The analysis shows how aligning privacy accounting with domain structure and existing randomness yields stronger guarantees at no cost.

Conclusion: Random cropping provides free differential privacy amplification for vision models when sensitive content is spatially localized. This demonstrates that leveraging domain-specific structure and existing training randomness can enhance privacy without modifying models or procedures.
...
### [170] [Training LLMs for Multi-Step Tool Orchestration with Constrained Data Synthesis and Graduated Rewards](https://arxiv.org/abs/2603.24709)
...
TL;DR: A framework for training LLMs on multi-step tool orchestration using RL with real API responses and graduated rewards improves accuracy on complex workflows.


<details>
  <summary>Details</summary>
Motivation: Multi-step tool orchestration is challenging for LLMs, with frequent failures in full sequence execution and parameter value errors. Existing training environments are limited to simple per-turn function calls with simulated data, and binary rewards don't provide signals for partial correctness.

Method: 1) Construct RL environment with large-scale cache of real API responses for data synthesis pipeline that samples valid multi-step orchestration traces with controllable complexity. 2) Propose graduated reward design decomposing correctness into atomic validity (individual function call correctness) and orchestration (correct tool sequencing with dependency respect).
...
Conclusion: The framework effectively addresses challenges in training LLMs for multi-step tool orchestration by combining real API response environments with graduated reward structures, leading to improved performance on complex workflows.
...
### [171] [Can an Actor-Critic Optimization Framework Improve Analog Design Optimization?](https://arxiv.org/abs/2603.24714)
*Sounak Dutta,Fin Amin,Sushil Panda,Jonathan Rabe,Yuejiang Wen,Paul Franzon*

Main category: cs.LG

TL;DR: ACOF is an actor-critic optimization framework for analog circuit sizing that separates proposal and evaluation roles to make search more guided and interpretable, achieving significant performance improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: Analog design optimization is slow and inefficient because small changes require expensive simulations, and optimal solutions occupy only narrow regions of a vast search space. Existing optimizers lack the judgment and guidance that human designers use when deciding where to search next.

Method: ACOF uses an actor-critic framework that separates proposal and evaluation: an actor suggests promising regions of the design space, while a critic reviews those choices, enforces design legality, and redirects the search when progress stalls. This preserves compatibility with standard simulator-based flows while adding deliberate guidance.

Result: ACOF improves top-10 figure of merit by average 38.9% over strongest competing baseline, reduces regret by average 24.7%, with peak gains of 70.5% in FoM and 42.2% lower regret on individual circuits.

Conclusion: By combining iterative reasoning with simulation-driven search, ACOF offers a more transparent and effective path for automated analog sizing across challenging design spaces, making the optimization process more deliberate, stable, and interpretable.
...
### [172] [Contrastive Learning Boosts Deterministic and Generative Models for Weather Data](https://arxiv.org/abs/2603.24744)
*Nathan Bailey*

Main category: cs.LG

TL;DR: SPARTA: A contrastive learning framework for creating robust embeddings from sparse weather data, outperforming autoencoders on downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Weather data is high-dimensional and multimodal, requiring compression for efficient downstream tasks. Current contrastive learning methods don't adequately handle sparse data common in real-world collection, and there's limited comparison with alternative compression methods like autoencoders.

Method: SPARTA framework with: 1) contrastive loss aligning sparse samples with complete ones, 2) temporally aware batch sampling, 3) cycle-consistency loss for better latent structure, and 4) graph neural network fusion to inject physical domain knowledge.
...
Conclusion: Contrastive learning with SPARTA framework effectively creates robust embeddings from sparse weather data, outperforming traditional methods and enabling better performance on downstream applications.
...
### [173] [Grokking as a Falsifiable Finite-Size Transition](https://arxiv.org/abs/2603.24746)
*Yuda Bi,Chenyu Zhang,Qiheng Wang,Vince D Calhoun*

Main category: cs.LG

TL;DR: The paper establishes a quantitative framework for testing phase-transition claims in grokking by treating group order as an extensive variable and using representation-level order parameters, showing evidence against smooth-crossover interpretations.


<details>
  <summary>Details</summary>
Motivation: Grokking (delayed generalization after memorization) is often described with phase-transition language, but this has lacked falsifiable finite-size inputs for rigorous testing.

Method: Treat group order p of ℤₚ as an extensive variable, use held-out spectral head-tail contrast as representation-level order parameter, apply condensed-matter-style diagnostics including coarse-grid sweeps and dense near-critical addition audit with Binder-like crossings.

Result: Binder-like crossings reveal a shared finite-size boundary, susceptibility comparison strongly disfavors smooth-crossover interpretation (ΔAIC=16.8 in near-critical audit), establishing testable quantitative framework.

Conclusion: Phase-transition language in grokking can now be tested as quantitative finite-size claim rather than invoked as analogy alone, though transition order remains unresolved.
...
### [174] [Light Cones For Vision: Simple Causal Priors For Visual Hierarchy](https://arxiv.org/abs/2603.24753)
*Manglam Kartik,Neel Tushar Shah*

Main category: cs.LG

TL;DR: Worldline Slot Attention models objects as persistent spacetime trajectories using Lorentzian geometry, achieving 6x better hierarchical object discovery than Euclidean approaches.


<details>
  <summary>Details</summary>
Motivation: Standard vision models fail to capture hierarchical object structure (parts within wholes) because they treat objects as independent points in Euclidean space without geometric structure for representing causal relationships.

Method: Introduces Worldline Slot Attention that models objects as persistent trajectories through spacetime worldlines, with objects having multiple slots at different hierarchy levels sharing spatial position but differing in temporal coordinates. Uses Lorentzian geometry to encode causal structure.

Result: Euclidean worldlines achieve only 0.078 accuracy (below random chance 0.33), while Lorentzian worldlines achieve 0.479-0.661 across three datasets - a 6x improvement replicated over 20+ independent runs. Outperforms hyperbolic embeddings with only 11K parameters.

Conclusion: Hierarchical object discovery requires geometric structure encoding asymmetric causality, an inductive bias absent from Euclidean space but natural to Lorentzian light cones. Visual hierarchies need causal structure (temporal dependency) rather than tree structure (radial branching).
...
### [175] [Transformers in the Dark: Navigating Unknown Search Spaces via Bandit Feedback](https://arxiv.org/abs/2603.24780)
*Jungtaek Kim,Thomas Zeng,Ziqian Lin,Minjae Lee,Chungpa Lee,Jy-yong Sohn,Hyung Il Koo,Kangwook Lee*

Main category: cs.LG

TL;DR: Transformers can be trained to approximate search algorithms for tree-structured problem solving, enabling LLMs to perform search without external components.


<details>
  <summary>Details</summary>
Motivation: While external search algorithms enhance LLM problem solving by navigating tree-structured idea spaces, they complicate the overall process. The paper investigates whether LLMs/Transformers themselves can approximate search algorithms to simplify the system.

Method: Introduces "unknown tree search with bandit feedback" framework where tree extensions and feedback are externally specified. Shows Transformers are theoretically expressive enough to implement search strategies and trains them from scratch to approximate these strategies. Also fine-tunes pretrained LLMs on search trajectories.

Result: Transformers can be trained to approximate distinct search strategies and generalize to unseen conditions like longer horizons or deeper trees. Continued task-focused training unlocks pretrained LLM capabilities for search.

Conclusion: LLMs/Transformers can indeed approximate search algorithms, potentially eliminating the need for external search components and simplifying the overall problem-solving process while maintaining effectiveness.
...
TL;DR: FFzero is a forward-only learning framework that enables neural network training without backpropagation or automatic differentiation, using local learning and forward evaluations only.


<details>
  <summary>Details</summary>
Motivation: Physical limits of chip manufacturing and environmental costs of deep learning motivate alternative paradigms like physical neural networks, but existing physical networks still rely on digital computing for training because backpropagation is difficult to implement physically.

Method: FFzero combines layer-wise local learning, prototype-based representations, and directional-derivative-based optimization using only forward evaluations (no backpropagation or automatic differentiation).

Result: FFzero enables stable neural network training where backpropagation fails, generalizes to multilayer perceptron and convolutional networks across classification and regression tasks, and provides a viable path for backpropagation-free in-situ physical learning.

Conclusion: FFzero demonstrates that forward-only learning is feasible and effective, offering a promising approach for physical neural networks that cannot implement backpropagation, potentially enabling more efficient and environmentally sustainable deep learning systems.
...
TL;DR: A comprehensive benchmark evaluating interpretability methods across clinical prediction tasks reveals attention mechanisms are efficient for faithful interpretation, black-box methods are computationally infeasible for time-series data, and many approaches are unreliable.


<details>
  <summary>Details</summary>
Motivation: Clinical decisions require explicit justification, making model interpretability essential for auditing deep clinical models before deployment. As model architectures and explainability methods expand, critical questions remain about whether architectural features like attention improve explainability and whether interpretability approaches generalize across clinical tasks.

Method: The authors present a comprehensive benchmark evaluating interpretability methods across diverse clinical prediction tasks and model architectures. They use PyHealth, an open-source framework, to ensure reproducibility and extensibility.
...
Conclusion: The study provides guidelines for improving interpretability within clinical predictive pipelines and emphasizes the need for systematic evaluation of interpretability methods across different clinical tasks and model architectures. The authors release their implementations via PyHealth to support reproducibility and extensibility.
...
<details>
  <summary>Details</summary>
Motivation: To enable Flow Matching on homogeneous spaces (quotients of Lie groups) without dealing with their potentially complicated geometry.

Method: Lifts data distributions to underlying Lie groups, reformulating as flow matching on Lie groups, then reduces to Euclidean flow matching on Lie algebras.

Result: Simpler, faster, fully intrinsic framework that eliminates need for premetrics or geodesics compared to Riemannian Flow Matching.

Conclusion: Provides a general framework for Flow Matching on homogeneous spaces by leveraging Lie group structure to avoid geometric complexities.
...
TL;DR: Multi-answer RL trains language models to generate multiple plausible answers in a single forward pass, improving diversity and efficiency over single-answer approaches.


<details>
  <summary>Details</summary>
Motivation: Real-world tasks often involve multiple valid answers or irreducible uncertainty (medical diagnosis, ambiguous QA, incomplete information), but current LM training collapses answer distributions to single modes. Need models that can generate multiple hypotheses with confidence estimates efficiently.
...
Result: Improved diversity, coverage, and set-level calibration scores across QA, medical diagnostic, and coding benchmarks. Models require fewer tokens to generate multiple answers than competing approaches, and show substantially higher accuracy on coding tasks.

Conclusion: Multi-answer RL provides a principled and compute-efficient alternative to inference-time scaling procedures like best-of-k, enabling better distributional reasoning for real-world ambiguous tasks.
...
TL;DR: The paper compares two ML approaches for warehouse staffing optimization: offline RL with Transformers (2.4% throughput improvement) and LLMs with human-readable inputs (matches/exceeds baselines).


<details>
  <summary>Details</summary>
Motivation: To optimize real-time staffing decisions in semi-automated warehouse sortation systems, where operational decisions can be supported at different abstraction levels with different trade-offs.

Method: Two approaches: 1) Custom Transformer-based policies trained with offline RL on detailed historical state representations, 2) LLMs operating on abstracted, human-readable state descriptions with systematic comparison of prompting techniques, automatic prompt optimization, and fine-tuning strategies.

Result: Offline RL achieved 2.4% throughput improvement over historical baselines in learned simulators. LLMs with supervised fine-tuning combined with Direct Preference Optimization on simulator-generated preferences achieved performance matching or slightly exceeding historical baselines in hand-crafted simulator.
...
<details>
  <summary>Details</summary>
Motivation: Neural network deployment on microcontrollers is severely constrained by limited flash and SRAM memory (kilobytes), where 1x1 pointwise mixers dominate memory usage even after INT8 quantization across vision, audio, and wearable sensing applications.

Method: A compression-as-generation approach that replaces most stored pointwise weights with generated weights: a shared micro-MLP synthesizes pointwise kernels once at load time from tiny per-layer codes, caches them, and executes with standard integer operators. This preserves commodity MCU runtimes with only a one-off synthesis cost.

Result: On three ECG benchmarks, HYPER-TINYPW achieves 6.31x compression (84.15% fewer bytes) while matching the performance of a 1.4 MB CNN at about 225 kB, retaining at least 95% of large-model macro-F1. Under 32-64 kB budgets, it sustains balanced detection where compact baselines degrade. On Speech Commands audio, it reaches 96.2% test accuracy.

Conclusion: HYPER-TINYPW shifts the macro-F1 versus flash Pareto frontier for microcontroller deployment, demonstrating that compression-as-generation effectively addresses per-layer redundancy in resource-constrained ML systems, with broad applicability to 1D biosignals, on-device speech, and embedded sensing tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current semantic search in RAG systems is insufficient for complex information needs where evidence is scattered across multiple sources. Existing approaches either use inefficient agentic retrieval strategies that expand search space through query generation but don't leverage data structure, or use knowledge graphs that capture richer proximities but have high maintenance costs and are incompatible with standard vector stores.

Method: GraphER independently enriches data objects during offline indexing and performs graph-based reranking over candidate objects at query time. It captures multiple forms of proximity beyond semantic similarity without requiring a knowledge graph, allowing seamless integration with standard vector stores. The approach is retriever-agnostic and introduces negligible latency overhead.

Result: Experiments on multiple retrieval benchmarks demonstrate the effectiveness of the proposed approach, showing improved retrieval performance for complex information needs.

Conclusion: GraphER addresses limitations of existing approaches by providing an efficient, low-maintenance solution that captures richer proximities beyond semantic similarity while maintaining compatibility with production vector store systems and adding minimal latency.
...
TL;DR: CVA is a novel framework for video temporal grounding that addresses the challenge of achieving temporally sensitive video-text alignment robust to irrelevant background context through three key components: Query-aware Context Diversification, Context-invariant Boundary Discrimination loss, and Context-enhanced Transformer Encoder.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the significant challenge in video temporal grounding of achieving temporally sensitive video-text alignment that remains robust to irrelevant background context. Current methods struggle with false negatives caused by query-agnostic mixing and lack robustness to contextual shifts at temporal boundaries.

Method: Three key components: 1) Query-aware Context Diversification (QCD) - data augmentation that builds video-text similarity-based pools to mix only semantically unrelated content; 2) Context-invariant Boundary Discrimination (CBD) loss - contrastive loss enforcing semantic consistency at temporal boundaries; 3) Context-enhanced Transformer Encoder (CTE) - hierarchical architecture combining windowed self-attention and bidirectional cross-attention with learnable queries.

Result: Achieves state-of-the-art performance on major VTG benchmarks including QVHighlights and Charades-STA. Notably achieves approximately 5 points improvement in Recall@1 (R1) scores over state-of-the-art methods, highlighting effectiveness in mitigating false negatives.

Conclusion: The synergy of data-centric and architectural enhancements in CVA framework effectively addresses video-text alignment challenges in temporal grounding, demonstrating significant improvements in performance through robust handling of contextual variations and boundary discrimination.
...
### [184] [A Systematic Empirical Study of Grokking: Depth, Architecture, Activation, and Regularization](https://arxiv.org/abs/2603.25009)
*Shalima Binta Manir,Anamika Paul Rupa*

Main category: cs.LG

TL;DR: Grokking dynamics in neural networks are driven by optimization-regularization interactions, not primarily by architecture, with weight decay being the key control parameter.


<details>
  <summary>Details</summary>
Motivation: To disentangle the confounding effects of architecture, optimization, and regularization in grokking phenomena, as prior studies have not systematically separated these factors.

Method: Controlled study on modular addition (mod 97) with matched and carefully tuned training regimes across models, systematically varying architectures (MLPs, Transformers), depths, activation functions, and regularization parameters across 3-5 seeds per configuration.

Result: 1) Depth has non-monotonic effect requiring architectural stabilization; 2) Transformer-MLP gap largely disappears under matched hyperparameters; 3) Activation function effects are regime-dependent; 4) Weight decay is dominant control parameter with narrow "Goldilocks" regime for grokking.

Conclusion: Grokking is an interaction-driven phenomenon governed by optimization and regularization, challenging architecture-centric interpretations and providing unified empirical account of delayed generalization.
...
### [185] [Optimal High-Probability Regret for Online Convex Optimization with Two-Point Bandit Feedback](https://arxiv.org/abs/2603.25029)
*Haishan Ye*

Main category: cs.LG

TL;DR: First high-probability minimax optimal regret bound for OCO with two-point bandit feedback for strongly convex functions.


<details>
  <summary>Details</summary>
Motivation: Addressing the open problem of achieving tight high-probability regret bounds for strongly convex functions in OCO with two-point bandit feedback, where heavy-tailed gradient estimators make standard concentration analysis difficult.

Method: Develop new techniques to handle heavy-tailed nature of bandit gradient estimators, enabling high-probability analysis for strongly convex losses in two-point feedback setting.

Result: Achieved minimax optimal regret bound of O(d(log T + log(1/δ))/μ) for μ-strongly convex losses, which is optimal in both time horizon T and dimension d.

Conclusion: Resolves the long-standing open problem by providing the first high-probability minimax optimal regret bound for strongly convex functions in OCO with two-point bandit feedback.
...
### [186] [Epistemic Compression: The Case for Deliberate Ignorance in High-Stakes AI](https://arxiv.org/abs/2603.25033)
*Steffen Lukas*

Main category: cs.LG

TL;DR: The paper introduces Epistemic Compression - matching model complexity to data shelf life rather than scaling parameters, using a Regime Index to distinguish stable vs shifting regimes for high-stakes domains.


<details>
  <summary>Details</summary>
Motivation: Foundation models fail in high-stakes domains (medicine, finance, policy) due to the Fidelity Paradox - extra capacity amplifies noise when rules change over time, not just a data problem but structural.

Method: Epistemic Compression principle enforces parsimony through architecture design (not post-hoc regularization), operationalized with a Regime Index separating Shifting Regime (unstable, data-poor) from Stable Regime (invariant, data-rich).
...
Conclusion: High-stakes AI demands shift from scaling for its own sake to principled parsimony through Epistemic Compression, matching model complexity to data shelf life.
...
### [187] [Intern-S1-Pro: Scientific Multimodal Foundation Model at Trillion Scale](https://arxiv.org/abs/2603.25040)
...
TL;DR: Intern-S1-Pro is the first one-trillion-parameter scientific multimodal foundation model that combines general intelligence with specialized scientific expertise across 100+ tasks in chemistry, materials, life sciences, and earth sciences.


<details>
  <summary>Details</summary>
Motivation: To create a massive-scale scientific multimodal foundation model that bridges the gap between general intelligence and specialized scientific expertise, addressing the need for comprehensive scientific AI capabilities across multiple domains.

Method: Scales to one trillion parameters using XTuner and LMDeploy infrastructure for efficient RL training at massive scale while maintaining precision consistency between training and inference. Integrates multimodal understanding with advanced agent capabilities.

Result: Achieves comprehensive enhancement across general and scientific domains, with stronger reasoning and image-text understanding. Demonstrates top-tier performance among open-source models for general capabilities and outperforms proprietary models in specialized scientific tasks.

Conclusion: Intern-S1-Pro successfully creates a "Specializable Generalist" that fuses general and specialized intelligence, establishing a new benchmark for scientific multimodal foundation models at unprecedented scale.
...
### [188] [The Order Is The Message](https://arxiv.org/abs/2603.25047)
*Jordan LeDoux*

Main category: cs.LG

TL;DR: Non-IID example ordering enables learning modular arithmetic with far fewer samples than theoretical lower bounds, while IID ordering fails completely.


<details>
  <summary>Details</summary>
Motivation: To investigate how example ordering (non-IID vs IID) affects learning efficiency and generalization in neural networks, particularly for tasks like modular arithmetic where theoretical sample complexity bounds exist.

Method: Controlled experiment on modular arithmetic (p=9973) with identical training data (0.3% of input space) but varying only example ordering: fixed-ordering strategies, IID baseline, and adversarially structured ordering.

Result: Fixed-ordering strategies achieve 99.5% test accuracy by epochs 487/659, while IID baseline only reaches 0.30% after 5000 epochs. Adversarial ordering suppresses learning entirely. The model constructs Fourier representation with fundamental frequency matching ordering structure.

Conclusion: Example ordering dramatically impacts learning efficiency and generalization, enabling learning below theoretical sample complexity bounds. This has implications for training efficiency, grokking phenomena, and safety risks through information channels that evade content-level auditing.
...
TL;DR: SIGMA addresses modality mismatch in linearized molecular generation by aligning latent representations of equivalent molecular structures through token-level contrastive learning and isomorphic beam search.


<details>
  <summary>Details</summary>
Motivation: Linearized string representations for molecular generation create modality mismatch where a single molecular graph maps to multiple distinct sequences, causing trajectory divergence where latent representations of structurally equivalent partial graphs drift apart due to different linearization histories.

Method: SIGMA (Structure-Invariant Generative Molecular Alignment) uses token-level contrastive objective to align latent states of prefixes sharing identical suffixes, enabling recognition of geometric symmetries without altering linear representation. Also introduces Isomorphic Beam Search (IsoBeam) to prune equivalent paths during inference.

Result: Empirical evaluations on standard benchmarks show SIGMA bridges gap between sequence scalability and graph fidelity, yielding superior sample efficiency and structural diversity in multi-parameter optimization compared to strong baselines.

Conclusion: SIGMA resolves trajectory divergence in linearized molecular generation while maintaining efficient string formulation, achieving better alignment between sequence-based generation and graph structure fidelity.
...
### [190] [An Explainable Ensemble Learning Framework for Crop Classification with Optimized Feature Pyramids and Deep Networks](https://arxiv.org/abs/2603.25070)
*Syed Rayhan Masud,SK Muktadir Hossain,Md. Ridoy Sarkar,Mohammad Sakib Mahmood,Md. Kishor Morol,Rakib Hossain Sajib*

Main category: cs.LG

TL;DR: An explainable ensemble learning system achieves 98.80% accuracy for crop suitability predictions using soil and climate data, outperforming individual ML models through feature pyramids, attention mechanisms, and explainable AI methods.


<details>
  <summary>Details</summary>
Motivation: Agriculture faces climate change, soil degradation, and resource depletion challenges, requiring data-driven crop classification and recommendation solutions that bridge the gap between complex ML models and actionable agricultural decision-making.

Method: An explainable ensemble learning paradigm combining optimized feature pyramids, deep networks, self-attention mechanisms, and residual networks. Uses preprocessing (label encoding, IQR outlier removal, StandardScaler normalization, SMOTE balancing) and compares multiple ML models with Grid Search hyperparameter tuning. Final Ensemble meta-ensemble design integrates best-performing models.

Result: Final Ensemble achieves 98.80% accuracy, precision, recall, and F1-score, significantly outperforming individual models like K-Nearest Neighbors (95.56% accuracy). Explainable AI methods (SHAP, permutation importance) identify critical features: soil pH, nitrogen, and zinc.

Conclusion: The paradigm successfully addresses the gap between complex ML models and practical agricultural decision-making, fostering sustainability and trust in AI-powered crop recommendations through high accuracy and explainability.
...
### [191] [Process-Aware AI for Rainfall-Runoff Modeling: A Mass-Conserving Neural Framework with Hydrological Process Constraints](https://arxiv.org/abs/2603.25093)
*Mohammad A. Farmani,Hoshin V. Gupta,Ali Behrangi,Muhammad Jawad,Sadaf Moghisi,Guo-Yue Niu*

Main category: cs.LG

TL;DR: MCP framework embeds physical constraints in AI for rainfall-runoff modeling, showing that progressive addition of hydrological processes improves predictive skill while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: Machine learning models achieve high accuracy in hydrology but lack physical interpretability. There's a need for physics-aware AI frameworks that enforce conservation principles while learning from data.

Method: Developed a hierarchy of Mass-Conserving Perceptron (MCP) models with progressively embedded physical processes: bounded soil storage, state-dependent conductivity, variable porosity, infiltration capacity, surface ponding, vertical drainage, and nonlinear water-table dynamics. Evaluated across 15 catchments in five US hydroclimatic regions using daily streamflow prediction.

Result: Progressively augmenting MCP's physical structure generally improves predictive performance. Effects are hydroclimate-dependent: vertical drainage helps in arid/snow basins but hurts in rainfall-dominated regions; surface ponding has small effects. Best MCP configurations approach LSTM benchmark performance while maintaining interpretability.
...
### [192] [Layer-Specific Lipschitz Modulation for Fault-Tolerant Multimodal Representation Learning](https://arxiv.org/abs/2603.25103)
*Diyar Altinses,Andreas Schwung*

Main category: cs.LG

TL;DR: A theoretical framework for fault-tolerant multimodal learning that unifies anomaly detection and error correction with mathematical guarantees on perturbation propagation.


<details>
  <summary>Details</summary>
Motivation: Multimodal systems in industrial/safety-critical environments need reliability under sensor failures, signal degradation, or cross-modal inconsistencies. Current approaches lack unified mathematical guarantees for fault tolerance.

Method: Two-stage self-supervised training: 1) Pre-train multimodal convolutional autoencoder on clean data to preserve anomaly signals in latent space, 2) Expand with learnable compute block (dense layers for correction, contrastive objectives for anomaly identification). Uses Lipschitz- and Jacobian-based criteria to control perturbation propagation, with layer-specific Lipschitz modulation and gradient clipping.

Result: Experimental results on multimodal fault datasets show improved anomaly detection accuracy and reconstruction under sensor corruption compared to existing methods.
...
### [193] [SEVerA: Verified Synthesis of Self-Evolving Agents](https://arxiv.org/abs/2603.25111)
*Debangshu Banerjee,Changming Xu,Gagandeep Singh*

Main category: cs.LG

TL;DR: SEVerA is a framework for creating self-evolving LLM agents with formal guarantees of safety and correctness, using verified generative models and three-stage synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing self-evolving LLM agent frameworks lack formal guarantees of safety and correctness, raising reliability and security concerns when these agents operate autonomously on unseen inputs.

Method: Introduces Formally Guarded Generative Models (FGGM) that wrap generative models with verified fallbacks using first-order logic contracts. SEVerA framework has three stages: Search (synthesize parametric programs with FGGM calls), Verification (prove correctness for all parameters), and Learning (gradient-based optimization while preserving correctness).

Result: SEVerA achieves zero constraint violations across Dafny program verification, symbolic math synthesis, and policy-compliant agentic tool use tasks, while improving performance over unconstrained and state-of-the-art baselines.

Conclusion: Formal behavioral constraints not only guarantee correctness but also steer synthesis toward higher-quality agents, demonstrating that safety and performance can be achieved simultaneously in self-evolving LLM agents.
...
### [194] [Vision Hopfield Memory Networks](https://arxiv.org/abs/2603.25157)
...
TL;DR: V-HMN is a brain-inspired vision foundation model that uses hierarchical Hopfield memory modules for associative and episodic memory, with predictive-coding refinement, achieving competitive performance while improving interpretability and data efficiency.


<details>
  <summary>Details</summary>
Motivation: Current vision/multimodal foundation models (Transformers, Mamba) lack biological plausibility, require massive training data, and offer limited interpretability. The authors aim to bridge brain-inspired computation with large-scale ML by creating a model that mimics human brain memory mechanisms.

Method: V-HMN integrates hierarchical memory mechanisms: 1) Local Hopfield modules for associative memory at patch level, 2) Global Hopfield modules as episodic memory for contextual modulation, 3) Predictive-coding-inspired refinement rule for iterative error correction. These modules are organized hierarchically to capture both local and global dynamics.

Result: V-HMN achieved competitive results on public computer vision benchmarks against widely adopted backbone architectures, while offering better interpretability, higher data efficiency, and stronger biological plausibility.

Conclusion: V-HMN demonstrates the potential of brain-inspired computation for next-generation vision foundation models, providing a blueprint for multimodal backbones that bridges neuroscience with large-scale machine learning.
...
### [195] [Train at Moving Edge: Online-Verified Prompt Selection for Efficient RL Training of Large Reasoning Model](https://arxiv.org/abs/2603.25184)
*Jiahao Wu,Ning Lu,Shengcai Liu,Kun Wang,Yanting Yang,Li Qing,Ke Tang*

Main category: cs.LG

TL;DR: HIVE: A dual-stage prompt selection framework that identifies high-utility prompts for efficient RL training of LLMs by targeting the "learning edge" - prompts with intermediate difficulty and high uncertainty.


<details>
  <summary>Details</summary>
Motivation: Current RL methods for post-training LLMs (like GRPO) require multiple rollouts per prompt, which is computationally expensive. Many prompts provide negligible gradients and low utility, wasting resources. There's a need to select only high-utility prompts before the rollout phase to improve efficiency.

Method: HIVE (History-Informed and online-VErified prompt selection) uses a dual-stage framework: 1) Coarse selection based on historical reward trajectories, and 2) Real-time pruning using prompt entropy as a proxy to filter out prompts with stale utility. The approach targets the "learning edge" - prompts with intermediate difficulty and high uncertainty that provide the strongest learning signals.

Result: HIVE achieves significant rollout efficiency improvements across multiple math reasoning benchmarks and models without compromising performance. The framework effectively identifies high-utility prompts while avoiding wasteful computation on low-utility ones.

Conclusion: Prompt utility is non-uniform and evolves during training, concentrating at the "learning edge." HIVE successfully addresses computational inefficiency in RL for LLMs by intelligently selecting high-utility prompts, making RL training more data-efficient and scalable.
...
### [196] [Knowledge-Guided Retrieval-Augmented Generation for Zero-Shot Psychiatric Data: Privacy Preserving Synthetic Data Generation](https://arxiv.org/abs/2603.25186)
*Adam Jakobsen,Sushant Gautam,Hugo Lewi Hammer,Susanne Olofsdotter,Miriam S Johanson,Pål Halvorsen,Vajira Thambawita*

Main category: cs.LG

TL;DR: A zero-shot, knowledge-guided LLM framework generates privacy-preserving synthetic psychiatric data using DSM-5 and ICD-10 knowledge, performing competitively against data-dependent models like CTGAN and TVAE.


<details>
  <summary>Details</summary>
Motivation: AI healthcare research is constrained by limited access to real patient data due to privacy concerns, creating a need for privacy-preserving synthetic data generation methods that don't require real patient data.

Method: Zero-shot knowledge-guided framework using LLMs with Retrieval-Augmented Generation (RAG) steered by DSM-5 and ICD-10 clinical knowledge bases. The approach generates synthetic psychiatric data without real patient data, benchmarked against CTGAN and TVAE models.

Result: CTGAN achieves best marginals and multivariate structure, while knowledge-augmented LLM is competitive on pairwise structure and achieves lowest pairwise error for separation anxiety and social anxiety. Clinical retrieval improves fidelity over no-retrieval LLM. LLM yields modest overlaps and low linkage risk comparable to CTGAN, while TVAE shows extensive duplication.

Conclusion: Grounding LLMs in clinical knowledge enables high-quality, privacy-preserving synthetic psychiatric data generation when real datasets are unavailable or cannot be shared, offering a viable alternative to data-dependent methods.
...
### [197] [A CDF-First Framework for Free-Form Density Estimation](https://arxiv.org/abs/2603.25204)
*Chenglong Song,Mazharul Islam,Lin Wang,Bing Chen,Bo Yang*

Main category: cs.LG

TL;DR: Proposes a CDF-first framework for conditional density estimation that estimates stable CDFs instead of ill-posed PDFs, using Smooth Min-Max networks to guarantee valid PDFs by construction.


<details>
  <summary>Details</summary>
Motivation: Direct PDF estimation is mathematically ill-posed as it amplifies random fluctuations from finite datasets, requiring strong inductive biases that limit expressivity and fail when violated. Existing methods struggle with free-form density estimation capturing multimodality, asymmetry, and topological complexity.

Method: CDF-first framework that estimates cumulative distribution functions (stable, well-posed target) and recovers PDFs via differentiation of learned smooth CDFs. Uses Smooth Min-Max (SMM) network parameterization to guarantee valid PDFs by construction. For multivariate outputs, employs autoregressive decomposition with SMM factors.

Result: Outperforms state-of-the-art density estimators on a range of univariate and multivariate conditional density estimation tasks.

Conclusion: CDF-first approach provides a mathematically sound alternative to direct PDF estimation, enabling robust free-form density estimation while guaranteeing valid probability distributions through smooth CDF parameterization.
...
TL;DR: First safe sample screening rules for Robust Support Vector Machines that reduce training time without affecting optimal solution by identifying samples whose uncertainty sets lie entirely on one side of the margin hyperplane.


<details>
  <summary>Details</summary>
Motivation: Robust SVMs improve reliability against feature noise but have increased computational cost. Need methods to reduce training complexity while preserving robustness.

Method: Develop safe screening rules for R-SVMs using Lagrangian duality (not Fenchel-Rockafellar duality). Derive ideal screening rule, then practical rule by adapting GAP-based safe regions to robust setting.

Result: Proposed method significantly reduces training time while preserving classification accuracy. First application of safe screening to worst-case robust models in supervised ML.

Conclusion: Safe screening rules successfully accelerate R-SVM training without compromising solution optimality, enabling more efficient robust classification.
...
<details>
  <summary>Details</summary>
Motivation: Neural Combinatorial Optimization relies on online RL which hampers deployment and underutilizes decades of algorithmic knowledge. Need to learn from existing heuristic solutions and potentially outperform them.

Method: Apply offline RL framework Decision Transformer to learn from heuristic solution datasets. Integrate Pointer Network to handle variable action space of node selection, and use expectile regression for optimistic conditioning of Return-to-Go.

Result: Method consistently produces higher-quality tours than the four classical heuristics it was trained on, demonstrating ability to exceed performance of existing domain knowledge.

Conclusion: Offline RL has potential to unlock and exceed performance embedded in existing domain knowledge for combinatorial optimization problems like TSP.
...
TL;DR: Pruning acts as implicit feature selection, preferentially destroying high-frequency generic features while preserving specialized rare ones in language models.


<details>
  <summary>Details</summary>
Motivation: Weight pruning is widely used for compressing large language models, but its effects on learned internal representations remain poorly understood. The paper aims to systematically study how unstructured pruning reshapes the feature geometry of language models.
...
Conclusion: Pruning acts as implicit feature selection, preferentially destroying high-frequency generic features while preserving specialized rare ones. This dissociation between geometric feature survival and causal importance has implications for interpretability under compression.
...
TL;DR: The paper introduces a category theory-based framework to model deep research agents and creates a mechanism-aware benchmark to stress-test them on structural reasoning tasks, revealing significant limitations in current models despite advances in search-reasoning integration.


<details>
  <summary>Details</summary>
Motivation: Current evaluation of deep research agents relies on ad hoc empirical benchmarks that don't rigorously model agent behavior or adequately test long-horizon synthesis and ambiguity resolution capabilities.

Method: Formalizes deep research agent behavior using category theory (modeling workflows as composition of structure-preserving functors), then creates a mechanism-aware benchmark with 296 questions testing four interpretable axes: sequential connectivity chains, V-structure pullback intersections, topological ordering of substructures, and ontological falsification via Yoneda Probe.

Result: Evaluation of 11 leading models shows persistently low performance (SOTA achieves only 19.9% average accuracy), revealing a stark dichotomy: agents succeed at dynamic topological re-ordering and ontological verification but collapse on multi-hop structural synthesis, showing massive performance variance across tasks.

Conclusion: While top-tier autonomous agents can organically unify search and reasoning, achieving generalized mastery over complex structural information remains a formidable open challenge, with current systems relying on brittle heuristics rather than systemic understanding.
...
TL;DR: Hi-MLIP: Hessian-informed MLIP with HINT training protocol enables accurate curvature prediction for complex systems, improving transition-state search and Gibbs free-energy predictions.
...
Method: Developed Hi-MLIP (Hessian-informed MLIP) with HINT (Hessian INformed Training) protocol that includes Hessian pre-training, configuration sampling, curriculum learning, and stochastic projection Hessian loss to reduce Hessian label requirements by 2-4 orders of magnitude.

Result: Hi-MLIP significantly improves transition-state search accuracy, brings Gibbs free-energy predictions close to chemical accuracy (especially in data-scarce regimes), and accurately treats strongly anharmonic hydrides, reproducing phonon renormalization and superconducting critical temperatures matching experiments.

Conclusion: The framework establishes a practical route to enhance curvature awareness of ML interatomic potentials, bridging simulation and experimental observables across diverse systems while bypassing computational bottlenecks of traditional anharmonic calculations.
...
TL;DR: GlowQ is a group-shared low-rank approximation method for quantized LLMs that reduces latency and memory overhead while maintaining accuracy by caching shared correction factors and selectively applying them only where most beneficial.


<details>
  <summary>Details</summary>
Motivation: Existing quantization methods degrade accuracy at low bits (e.g., 4-bit), and current correction methods (LQER, QERA, ASER) restore all layers with error-correction modules in every decoder block, causing significant latency and memory overhead.

Method: Proposes GlowQ with group-shared low-rank approximation that caches a single shared right factor per input-sharing group and restores only groups/layers with highest accuracy benefit. Also introduces selective variant GlowQ-S that applies cached shared module only where it provides largest benefit.
...
Conclusion: GlowQ effectively addresses the latency-memory overhead problem of existing quantization correction methods by introducing group-shared low-rank approximation with selective application, achieving significant speed improvements while maintaining or improving accuracy compared to strong baselines.
...
<details>
  <summary>Details</summary>
Motivation: Zero-shot RL needs diverse pretraining data, but undirected exploration yields low-diversity data leading to poor downstream performance and impractical policies for hardware deployment.

Method: FB-MEBE combines unsupervised behavior exploration (maximizing entropy of achieved behavior distribution) with a regularization critic that shapes policies toward natural, physically plausible behaviors.

Result: FB-MEBE achieves improved performance over other exploration strategies in simulated downstream tasks and produces natural policies deployable to hardware without finetuning.

Conclusion: The proposed FB-MEBE algorithm effectively addresses the data diversity challenge in online zero-shot RL for robotics, enabling practical hardware deployment through behavior exploration and regularization.
...
TL;DR: Proposes new evaluation method for wildfire forecasting models that better aligns with operational decision-making, focusing on both fire prediction accuracy and false alarm reduction, showing ensemble ML improves both.


<details>
  <summary>Details</summary>
Motivation: Standard ML evaluation metrics provide limited measure of operational performance for Fire Danger Index forecasting, and current evaluations inadequately account for false positive rates despite their critical importance in real-world fire management.

Method: Revisits daily FDI model evaluation paradigm and proposes novel evaluation method aligned with real-world decision-making, systematically assessing both fire prediction accuracy and false positive rates (false alarms).
...
Conclusion: Proposed evaluation framework better captures operational performance of wildfire forecasting models, and ensemble ML approaches offer superior performance for practical fire management applications.
...
TL;DR: Causal-INSIGHT is a model-agnostic framework that extracts directed, time-lagged influence structure from trained temporal predictors by analyzing their responses to intervention-inspired input clamping, without requiring ground-truth causal labels.


<details>
  <summary>Details</summary>
Motivation: Understanding directed temporal interactions in multivariate time series is crucial for interpreting complex dynamical systems and the predictive models trained on them. Current approaches often infer causal structure at the data-generating process level, but there's a need to understand how trained predictors actually use temporal dependencies for their predictions.

Method: Causal-INSIGHT applies systematic, intervention-inspired input clamping at inference time to analyze how fixed, pre-trained predictors respond. From these responses, it constructs directed temporal influence signals and introduces Qbic, a sparsity-aware graph selection criterion that balances predictive fidelity and structural complexity without requiring ground-truth graph labels.
...
Conclusion: Causal-INSIGHT provides an effective post-hoc interpretation framework for extracting model-implied directed temporal influence structure from trained predictors, offering insights into how models actually use temporal dependencies for prediction without requiring causal ground truth.
...
TL;DR: Audio-to-audio transfer learning study shows that while more pre-training data (samples and classes) helps, similarity between pre-training and downstream tasks is more important for effective transfer.


<details>
  <summary>Details</summary>
Motivation: Despite widespread use of transfer learning in deep learning, there are still many open questions about how and when it works effectively, particularly in understanding the inner workings and conditions for successful transfer.

Method: Conducted rigorous audio-to-audio transfer learning study by pre-training various models on ontology-based subsets of AudioSet, then fine-tuning them on three computer audition tasks: acoustic scene recognition, bird activity recognition, and speech command recognition.

Result: Increasing both the number of samples and classes in pre-training data has positive impact on transfer learning, but this effect is generally surpassed by similarity between pre-training and downstream tasks, which enables models to learn comparable features.

Conclusion: Task similarity is more crucial than simply having more pre-training data for effective transfer learning in audio domains, suggesting that feature compatibility between source and target tasks drives successful knowledge transfer.
...
TL;DR: Lightweight interpretable models (SARIMAX, Facebook Prophet, NeuralProphet) can achieve competitive PM2.5 forecasting accuracy with better computational efficiency than complex models, especially when using adaptive regimes like walk-forward refitting or frozen forecasting with residual correction.
...
Method: Developed a leakage-aware forecasting workflow with chronological data partitioning, preprocessing, feature selection, and exogenous-driver modeling. Evaluated three forecasting families (SARIMAX, Facebook Prophet, NeuralProphet) under two adaptive regimes: weekly walk-forward refitting and frozen forecasting with online residual correction.

Result: Facebook Prophet achieved strongest performance under walk-forward refitting (MAE 37.61, RMSE 50.10) with substantially less execution time than NeuralProphet. In frozen-model regime, corrected SARIMAX yielded lowest overall error (MAE 32.50, RMSE 46.85). NeuralProphet remained less accurate and less stable across both regimes.
...
TL;DR: ARMOR: A robust self-supervised framework for automated incident management in microservices that handles missing modalities through modality-specific encoders and missing-aware fusion, achieving SOTA performance even with severe data loss.
...
Conclusion: ARMOR provides a robust solution for automated incident management in microservices that effectively handles missing modalities through its specialized architecture and fusion mechanisms, making it practical for real-world deployment where data completeness cannot be guaranteed.
...
TL;DR: A computational framework combining genome-scale metabolic modeling with machine learning and optimization to predict and enhance yeast biomass flux, achieving high prediction accuracy and significant flux improvements through in silico interventions.


<details>
  <summary>Details</summary>
Motivation: Accurately predicting biomass flux across diverse environmental and genetic perturbations remains challenging for rational yeast strain design in industrial biotechnology, despite S. cerevisiae's established role as a model organism.

Method: Combined Yeast9 genome-scale metabolic model with machine learning: generated 2,000 flux profiles via flux balance analysis, trained Random Forest and XGBoost regressors, used variational autoencoder for clustering, SHAP for interpretability, Bayesian optimization for nutrient constraints, and generative adversarial network for novel flux configurations.

Result: Random Forest and XGBoost achieved R² of 0.99989 and 0.9990 respectively; variational autoencoder identified four metabolic clusters; SHAP analysis revealed glycolysis, TCA cycle, and lipid biosynthesis as key determinants; in silico overexpression achieved 0.979 gDW/hr biomass flux; Bayesian optimization produced 12-fold increase (0.0858 to 1.041 gDW/hr).
...
<details>
  <summary>Details</summary>
Motivation: On-policy distillation is appealing for LLM post-training because it evaluates teacher feedback on student-generated rollouts rather than fixed teacher traces, but sampled-token OPD becomes unreliable in long-horizon settings due to distribution drift and signal limitations.

Method: The paper revisits OPD theoretically and empirically, identifies three failure modes of sampled-token OPD, and proposes teacher top-K local support matching implemented as truncated reverse-KL with top-p rollout sampling and special-token masking.
...
Conclusion: Teacher top-K local support matching addresses the limitations of sampled-token OPD by providing more reliable guidance on student-generated prefixes and handling tokenizer/special-token mismatches, leading to more stable learning and improved performance in long-horizon settings.
...
TL;DR: A Physics-Spatiotemporal Masked Autoencoder method improves prediction of high-dimensional dynamical systems with irregular time steps by combining spatial feature extraction with attention mechanisms for irregular time series, avoiding data imputation while preserving physical integrity.
...
Method: Proposes a Physics-Spatiotemporal Masked Autoencoder that integrates convolutional autoencoders for spatial feature extraction with masked autoencoders optimized for irregular time series, using attention mechanisms to reconstruct entire physical sequences in a single prediction pass without data imputation.

Result: The method achieves significant improvements in prediction accuracy, robustness to nonlinearities, and computational efficiency over traditional convolutional and recurrent network methods on multiple simulated datasets and real-world ocean temperature data.

Conclusion: The model shows potential for capturing complex spatiotemporal patterns without requiring domain-specific knowledge, with applications in climate modeling, fluid dynamics, ocean forecasting, environmental monitoring, and scientific computing.
...
### [213] [Social Hippocampus Memory Learning](https://arxiv.org/abs/2603.25614)
*Liping Yi,Zhiming Zhao,Qinghua Hu*

Main category: cs.LG

TL;DR: SoHip is a memory-centric social machine learning framework that enables heterogeneous agents to collaborate via memory sharing instead of model sharing, improving privacy and performance.


<details>
  <summary>Details</summary>
Motivation: Existing federated learning approaches share model parameters or intermediate representations, which can expose sensitive information and incur overhead. There's a need for a more privacy-preserving and efficient collaboration method for heterogeneous agents.

Method: SoHip abstracts individual short-term memory from local representations, consolidates it into long-term memory via hippocampus-inspired mechanism, and fuses it with collectively aggregated long-term memory. Only lightweight memory is exchanged while raw data and local models stay on-device.

Result: SoHip consistently outperforms seven baselines on two benchmark datasets, achieving up to 8.78% accuracy improvements. Theoretical analysis confirms convergence and privacy preservation properties.

Conclusion: Memory-centric social machine learning via SoHip provides an effective, privacy-preserving alternative to traditional federated learning approaches, enabling better collaboration among heterogeneous agents with improved performance.
...
### [214] [Anchored-Branched Steady-state WInd Flow Transformer (AB-SWIFT): a metamodel for 3D atmospheric flow in urban environments](https://arxiv.org/abs/2603.25635)
...
TL;DR: AB-SWIFT is a transformer-based model with branched architecture for urban air flow prediction, outperforming state-of-the-art models on atmospheric simulations with varying urban geometries and stratifications.


<details>
  <summary>Details</summary>
Motivation: Traditional CFD simulations for urban air flow are computationally expensive, while existing deep learning models struggle with high variations in urban geometry and large mesh sizes common in atmospheric flow modeling.

Method: Introduces Anchored Branched Steady-state Wind Flow Transformer (AB-SWIFT), a transformer-based model with internal branched structure specifically designed for atmospheric flow. Trained on a database of atmospheric simulations with randomized urban geometries and mixed atmospheric stratifications (unstable, neutral, stable).

Result: AB-SWIFT achieves best accuracy on all predicted fields compared to state-of-the-art transformers and graph-based models. Code and data are publicly available.

Conclusion: The proposed AB-SWIFT model effectively addresses challenges in urban air flow modeling, providing an accurate and efficient alternative to costly CFD simulations while handling complex urban geometries and atmospheric conditions.
...
### [215] [Uncertainty-Guided Label Rebalancing for CPS Safety Monitoring](https://arxiv.org/abs/2603.25670)
*John Ayotunde,Qinghua Xu,Guancheng Wang,Lionel C. Briand*

Main category: cs.LG

TL;DR: U-Balance: A method that uses behavioral uncertainty to rebalance imbalanced CPS safety datasets by relabeling high-uncertainty safe samples as unsafe, improving safety prediction without synthetic data generation.


<details>
  <summary>Details</summary>
Motivation: Safety monitoring in CPS suffers from extreme class imbalance (rare unsafe events), and standard rebalancing techniques fail on time-series CPS telemetry. Behavioral uncertainty in CPS operations correlates with safety outcomes but remains unexplored for safety monitoring.

Method: 1) Train GatedMLP-based uncertainty predictor using distributional kinematic features to output uncertainty scores. 2) Apply uncertainty-guided label rebalancing (uLNR) that probabilistically relabels safe-labeled windows with high uncertainty as unsafe. 3) Train safety predictor on rebalanced dataset.

Result: Moderate but significant correlation between behavioral uncertainty and safety. uLNR identified as most effective strategy. U-Balance achieves 0.806 F1 score, outperforming strongest baseline by 14.3 percentage points while maintaining competitive inference efficiency.

Conclusion: U-Balance effectively leverages behavioral uncertainty to address class imbalance in CPS safety monitoring without synthetic data generation, significantly improving safety prediction performance through uncertainty-guided label rebalancing.
...
### [216] [Longitudinal Digital Phenotyping for Early Cognitive-Motor Screening](https://arxiv.org/abs/2603.25673)
*Diego Jimenez-Oviedo,Ruben Vera-Rodriguez,Ruben Tolosana,Juan Carlos Ruiz-Garcia,Jaime Herreros-Rodriguez*

Main category: cs.LG

TL;DR: AI-driven framework uses tablet interaction data to identify and track cognitive-motor development profiles in children (18 months to 8 years), revealing three distinct performance clusters with high stability in low-performing group.


<details>
  <summary>Details</summary>
Motivation: Traditional assessments for cognitive-motor development are subjective and static, while digital devices offer opportunity for continuous, objective monitoring through digital biomarkers for early detection and intervention.

Method: Longitudinal framework using tablet-based interaction data from children over multiple academic years, analyzing six cognitive-motor tasks. Applied dimensionality reduction (t-SNE) and unsupervised clustering (K-Means++) to identify developmental phenotypes and track individual transitions over time.

Result: Identified three distinct developmental profiles: low, medium, and high performance. Low-performance cluster showed high stability (>90% retention in early years), indicating early deficits tend to persist without intervention. Higher-performance clusters showed greater variability, potentially reflecting engagement factors.

Conclusion: Validates unsupervised learning on touchscreen data for uncovering heterogeneous developmental paths. Identified profiles serve as scalable, data-driven proxies for cognitive growth, offering foundation for early screening tools and personalized pediatric interventions.
...
### [217] [On Neural Scaling Laws for Weather Emulation through Continual Training](https://arxiv.org/abs/2603.25687)
*Shashank Subramanian,Alexander Kiefer,Arnur Nigmetov,Amir Gholami,Dmitriy Morozov,Michael W. Mahoney*

Main category: cs.LG

TL;DR: The paper studies neural scaling laws for weather forecasting models, showing that simple Swin Transformers with continual training and periodic cooldowns follow predictable scaling trends and outperform standard training approaches.


<details>
  <summary>Details</summary>
Motivation: Neural scaling laws have been successful in NLP and CV for predicting large model performance, but their application to Scientific Machine Learning (particularly weather forecasting) remains underexplored. The authors aim to understand scaling behavior in this domain to guide efficient resource allocation.

Method: Used minimal, scalable Swin Transformer architecture with continual training (constant learning rates + periodic cooldowns). Systematically explored model/dataset sizes under various compute budgets to construct IsoFLOP curves. Cooldown phases were repurposed for multi-step rollouts and spectral loss adjustments.

Result: Models trained with minimalist approach follow predictable scaling trends and outperform standard cosine learning rate schedules. Compute-optimal training regimes were identified. Extrapolation to larger scales reveals potential performance limits, showing neural scaling can serve as diagnostic for efficient resource allocation.

Conclusion: Neural scaling laws are applicable to Scientific Machine Learning for weather forecasting. Simple training strategies with periodic cooldowns enable predictable scaling and performance improvements. The approach provides important diagnostics for resource allocation in scientific ML applications.
...
### [218] [A Unified Memory Perspective for Probabilistic Trustworthy AI](https://arxiv.org/abs/2603.25692)
*Xueji Zhao,Likai Pei,Jianbo Liu,Kai Ni,Ningyuan Cao*

Main category: cs.LG

TL;DR: The paper presents a unified framework for analyzing deterministic and stochastic memory access in AI systems, revealing that stochastic sampling reduces data-access efficiency and can lead to entropy-limited operation, then proposes evaluation criteria and examines probabilistic compute-in-memory solutions.


<details>
  <summary>Details</summary>
Motivation: Trustworthy AI increasingly uses probabilistic computation for robustness, interpretability, security and privacy, but these workloads interleave deterministic data access with stochastic sampling, shifting performance bottlenecks from arithmetic units to memory systems that must deliver both data and randomness.
...
Conclusion: The paper outlines pathways toward scalable hardware for trustworthy AI by addressing the memory system challenges of probabilistic computation through unified analysis frameworks and emerging compute-in-memory approaches that can handle both deterministic and stochastic access patterns efficiently.
...
### [219] [Neural Network Conversion of Machine Learning Pipelines](https://arxiv.org/abs/2603.25699)
*Man-Ling Sung,Jan Silovsky,Man-Hung Siu,Herbert Gish,Chinnu Pittapally*

Main category: cs.LG

TL;DR: Transfer learning from non-neural ML pipelines (random forest) to neural network students enables joint optimization and unified inference across multiple ML tasks.
...
Method: Extend student-teacher learning to transfer from non-neural random forest classifiers to neural network students. Experiment with various NN topologies on 100 OpenML tasks where random forest performed well, and investigate using random forest for NN hyper-parameter selection.
...
Conclusion: Knowledge transfer from traditional ML models to neural networks is feasible and enables unified optimization and inference, with random forest potentially aiding in hyper-parameter selection for the neural student.
...
### [220] [FED-HARGPT: A Hybrid Centralized-Federated Approach of a Transformer-based Architecture for Human Context Recognition](https://arxiv.org/abs/2603.24601)
*Wandemberg Gibaut,Alexandre Osorio,Amparo Munoz,Sildolfo F. G. Neto,Fabio Grassiotto*

Main category: eess.SP

TL;DR: Hybrid centralized-federated approach for Human Activity Recognition using Transformer architecture, balancing privacy and performance in non-IID mobile sensor data scenarios.


<details>
  <summary>Details</summary>
Motivation: With proliferation of edge devices generating private sensor data, there's need for discreet human activity monitoring while preserving data privacy, requiring approaches that balance privacy concerns with model performance.

Method: Proposes hybrid centralized-federated approach using Transformer-based architecture, implemented within Flower framework for federated learning. Evaluates federated model derived from centralized baseline using mobile sensor data in non-IID scenarios.

Result: Hybrid approach effectively improves accuracy and robustness of HAR models while preserving data privacy. Federated learning setup demonstrates comparable performance to centralized models in non-IID data scenarios.

Conclusion: Federated learning can successfully balance data privacy and model performance for real-world HAR applications, showing potential for practical deployment in privacy-sensitive scenarios with distributed edge devices.
...
### [221] [MuViS: Multimodal Virtual Sensing Benchmark](https://arxiv.org/abs/2603.24602)
...
TL;DR: MuViS is a domain-agnostic benchmarking suite for multimodal virtual sensing that consolidates diverse datasets into a unified interface for standardized evaluation, showing no single approach provides universal advantage.


<details>
  <summary>Details</summary>
Motivation: Virtual sensing research is fragmented with no established default approach that transfers across processes, modalities, and sensing configurations, creating a need for standardized benchmarking.

Method: Introduce MuViS, a domain-agnostic benchmarking suite that consolidates diverse datasets into a unified interface for standardized preprocessing and evaluation, then benchmark established approaches including gradient-boosted decision trees and deep neural network architectures.

Result: Benchmarking shows none of the established approaches provides a universal advantage across different datasets and configurations, highlighting the need for more generalizable virtual sensing architectures.

Conclusion: MuViS serves as an open-source, extensible platform for reproducible comparison and future integration of new datasets and model classes, addressing the fragmentation in virtual sensing research.
...
### [222] [Analog Computing with Hybrid Couplers and Phase Shifters](https://arxiv.org/abs/2603.24604)
*Matteo Nerini,Xuekang Liu,Bruno Clerckx*

Main category: eess.SP

TL;DR: Analog microwave networks using hybrid couplers and phase shifters can compute linear transformations like DFT, Hadamard, and Haar transforms instantly without digital operations.


<details>
  <summary>Details</summary>
Motivation: Analog computing with microwave signals can enable exceptionally fast computations surpassing digital limits. There's a need to understand what linear transformations can be computed using only low-cost fundamental components (hybrid couplers and phase shifters) in the analog domain.

Method: Derived necessary and sufficient conditions for linear transformations computable with hybrid couplers and phase shifters. Developed systematic design methods for DFT, Hadamard, and Haar transforms for power-of-two sizes. Built and calibrated a 4×4 DFT hardware prototype using microstrip implementation.

Result: Successfully demonstrated analog DFT computation with high correlation to theoretical expectations. The prototype validated that linear microwave networks can compute specific transformations instantly without digital operations.

Conclusion: This work shows a practical pathway to low-latency, real-time analog signal processing using standard microwave components, enabling analog computers that can perform matrix-vector products instantly.
...
### [223] [On Performance of Fluid Antenna Relay (FAR)-Assisted AAV-NOMA Wireless Network](https://arxiv.org/abs/2603.24954)
*Ruopeng Xu,Songling Zhang,Zhaohui Yang,Yixuan Chen,Mingzhe Chen,Zhaoyang Zhang,Kai-Kit Wong*

Main category: eess.SP

TL;DR: FAR-assisted NOMA downlink system with fluid antenna on AAV improves weak user outage probability using AF/DF relaying, analyzed via Copula theory.


<details>
  <summary>Details</summary>
Motivation: To improve downlink communication performance for weak channel users in NOMA systems by leveraging fluid antenna technology on aerial relays to provide extra degrees of freedom and reduce outage probability.

Method: Two-phase transmission: BS to users/FAR, then FAR forwards signals using AF or DF relaying. Copula theory and Gaussian copula function model FAS channel statistics. Analytical OP expressions derived for both AF and DF schemes.

Result: Proposed FAR-assisted scheme consistently outperforms benchmark schemes without FAR. Simulations validate effectiveness and demonstrate optimal relaying scheme selection parameters under different FAR positions and outage thresholds.

Conclusion: FAR-assisted NOMA with fluid antenna technology on AAVs effectively improves weak user outage performance, with analytical models providing insights for optimal relaying scheme selection based on system conditions.
...
### [224] [Near-field Beam Training under Multi-path Channels: A Hybrid Learning-and-Optimization Approach](https://arxiv.org/abs/2603.24960)
*Jiapeng Li,Changsheng You,Guoliang Cheng,Haobin Sun,Chao Zhou,Linglong Dai*

Main category: eess.SP

TL;DR: Hybrid DL-optimization method for near-field beam training in XL-arrays that combines deep learning for coarse parameter estimation with optimization refinement, addressing multi-path scenarios beyond LoS assumptions.


<details>
  <summary>Details</summary>
Motivation: Existing DFT codebook methods for near-field beam training in XL-arrays rely on LoS-dominant channel assumptions, which degrade in general multi-path scenarios due to complex received signal power patterns.

Method: Two-stage approach: 1) Tailored U-Net architecture with permutation invariant training (PIT) to learn mapping from received power patterns to coarse angle/range estimates of multi-path components, including path number estimation; 2) Particle swarm optimization for parameter refinement and Gerchberg-Saxton algorithm for channel gain retrieval.

Result: The proposed hybrid design significantly outperforms benchmarks in parameter estimation accuracy and achievable rate while maintaining low computational complexity.

Conclusion: The hybrid learning-and-optimization approach effectively addresses multi-path near-field beam training challenges in XL-arrays, achieving high accuracy with practical complexity.
...
### [225] [Efficient compressive sensing for machinery vibration signals](https://arxiv.org/abs/2603.25166)
*Imen Tounsi,Fadi Karkafi,Mohammed El Badaoui,François Guillet*

Main category: eess.SP

TL;DR: Compressive sensing framework for vibration monitoring with novel Wang matrix outperforms traditional matrices at high compression ratios.


<details>
  <summary>Details</summary>
Motivation: Mechanical vibration monitoring requires high sampling rates generating large data volumes, creating challenges for storage, transmission, and power efficiency. Compressive Sensing offers a solution by enabling sub-Nyquist acquisition through signal sparsity.

Method: Comparative analysis of CS framework components (sparse basis, measurement matrix, reconstruction algorithm) for machinery vibration signals. Introduces and evaluates Wang matrix - a hardware-efficient measurement matrix originally developed for image compression - for the first time in vibration monitoring context.

Result: Wang matrix achieves superior reconstruction quality with higher SNR compared to conventional Gaussian and Bernoulli matrices, especially at high compression ratios, as demonstrated on HUMS2023 and CETIM gearbox datasets.

Conclusion: The Wang matrix presents an effective hardware-efficient solution for compressive sensing in vibration monitoring, offering better performance than traditional matrices at high compression ratios while addressing storage, transmission, and power constraints.
...
### [226] [Rate-Splitting Multiple Access with a SIC-Free Receiver: An Experimental Study](https://arxiv.org/abs/2603.25238)
*Guoqian Sun,Xinze Lyu,Bruno Clerk*

Main category: eess.SP

TL;DR: SIC-free RSMA receiver using joint demapping outperforms traditional SIC-based receivers in reliability and practicality across wider operating ranges.


<details>
  <summary>Details</summary>
Motivation: Traditional RSMA implementations using successive interference cancellation (SIC) suffer from error propagation during common-stream decoding, limiting performance and reliability.

Method: Developed a SIC-free RSMA receiver based on joint demapping that directly evaluates bit vectors over a composite constellation. Tested using a two-user MISO prototype with over-the-air measurements comparing SIC and JD-based receivers.

Result: The proposed SIC-free receiver provides stronger reliability and better practicality over a wider operating range, with all observations consistent with theoretical expectations.

Conclusion: Joint demapping-based SIC-free RSMA receivers offer superior performance compared to traditional SIC-based approaches, addressing error propagation limitations while maintaining practical implementation advantages.
...
### [227] [Joint Training Scattering Matrix Learning and Channel Estimation for Beyond-Diagonal Reconfigurable Intelligent Surfaces](https://arxiv.org/abs/2603.25299)
*Yiyang Peng,Binggui Zhou,Yutong Zheng,Danilo Mandic,Bruno Clerckx*

Main category: eess.SP

TL;DR: Proposes JTSMLCEF, a learning-based channel estimation framework for BD-RIS-aided MU-MIMO systems that jointly optimizes training scattering matrices and estimates cascaded channels, reducing pilot overhead by 80% while improving NMSE by 82.6-92.5%.


<details>
  <summary>Details</summary>
Motivation: BD-RIS offers enhanced wave manipulation but requires accurate CSI, which is challenging due to increased channel coefficients and prohibitively large pilot training overhead in MU-MIMO systems. Existing methods neglect spatial correlation among antennas and statistical correlation across RIS-user channels.

Method: JTSMLCEF framework with two-phase protocol: 1) adaptive training scattering matrix optimization using TSMO, and 2) cascaded channel estimation using DACE with intra-user and inter-user attention modules to capture multi-dimensional correlations in multi-user cascaded channels.

Result: Reduces pilot overhead by 80% compared to state-of-the-art, while reducing NMSE by 82.6% in indoor scenarios and 92.5% in urban micro-cell scenarios.

Conclusion: JTSMLCEF effectively addresses the channel estimation challenge in BD-RIS-aided MU-MIMO systems by jointly optimizing training and estimation, significantly reducing pilot overhead while improving estimation accuracy through attention-based correlation capture.
...
### [228] [Multi-User Covert Communication in Spatially Heterogeneous Wireless Networks](https://arxiv.org/abs/2603.25549)
*Jinyoung Lee,Hyeonsik Yeom*

Main category: eess.SP

TL;DR: This paper analyzes uplink multi-user covert communication with spatially distributed users, proving that a generalized on-off power control scheme is optimal for heterogeneous configurations and showing that spatial heterogeneity actually enhances covert performance.


<details>
  <summary>Details</summary>
Motivation: Prior works approximate channel statistics using averaged parameters and homogeneous assumptions, which fails to capture real-world spatial heterogeneity. This study aims to accurately model spatially distributed users with their actual geometric positions and corresponding channel variances for more realistic covert communication analysis.

Method: Explicitly models each user's geometric position and corresponding user-to-Willie and user-to-Bob channel variances. Proves mathematically that generalized on-off power control (jointly accounting for both Bob's and Willie's channels) is optimal for heterogeneous configurations. Derives closed-form expressions for minimum detection error probability and minimum number of cooperative users. Proposes a piecewise search algorithm for exact optimality with reduced computational complexity.

Result: Derived closed-form expressions for key performance metrics, validated by Monte-Carlo simulations. Found that user spatial heterogeneity can actually enhance covert communication performance. Demonstrated that optimization considering spatial heterogeneity achieves substantially improved performance compared to homogeneous assumptions. Proposed algorithm achieves exact optimality with significantly reduced computational complexity.

Conclusion: Spatial heterogeneity in multi-user covert communication systems is not just a complication but can be leveraged to enhance performance. The proposed modeling approach and optimization framework provide accurate analysis and practical solutions that significantly outperform homogeneous assumptions, offering important insights for designing real-world covert communication systems.
...
### [229] [Challenge-Response Authentication for LEO Satellite Channels: Exploiting Orbit-Specific Uniqueness](https://arxiv.org/abs/2603.25576)
*Jinyoung Lee,Stefano Tomasin,Dong-Hyun Jung*

Main category: eess.SP

TL;DR: Proposed active challenge-response authentication for LEO satellites using orbital dynamics as physics-based root of trust, replacing passive methods with random-time verification.


<details>
  <summary>Details</summary>
Motivation: As LEO satellite constellations grow and become critical for emergency communications, navigation, and military operations, securing satellite links and verifying transmitter identity has become essential. Existing passive authentication methods have vulnerabilities that need to be addressed.

Method: Active challenge-response framework where verifier checks satellites at randomly chosen times unknown in advance, eliminating fixed measurement windows. Uses deterministic but unpredictably sampled orbital observables to establish physics-based root of trust, transforming authentication into spatiotemporal consistency verification constrained by orbital dynamics.

Result: The approach provides robust protection against trajectory-aware spoofing attacks by leveraging the inherent constraints of orbital mechanics, making satellite authentication more secure than static feature matching methods.

Conclusion: The proposed active authentication framework offers a more secure approach to satellite identity verification by using orbital dynamics as a physics-based root of trust, addressing vulnerabilities in existing passive methods and providing protection against sophisticated spoofing attacks.
...
### [230] [Intelligent Reflection as a Service (IRaaS): System Architecture, Enabling Technologies, and Deployment Strategy](https://arxiv.org/abs/2603.25593)
*Wei Wang,Yutian Shen*

Main category: eess.SP

TL;DR: Proposes Intelligent Reflection as a Service (IRaaS) - a cloud-based service model for RIS deployment in 6G networks to reduce complexity and accelerate adoption.


<details>
  <summary>Details</summary>
Motivation: RIS is promising for 6G but faces deployment challenges due to system complexity and integration difficulties with existing mobile networks. Need to identify practical niche for RIS while mitigating escalating network complexity.

Method: Proposes IRaaS concept leveraging resource pooling, service-based architecture (SBA), cloud infrastructure, and model-free signal processing. Enables telecom operators to deliver on-demand intelligent reflection services without radical protocol updates.

Result: IRaaS provides novel deployment strategy creating opportunities for service vendors while balancing interests of telecom operators and property owners. Expected to accelerate RIS rollout from both technical and commercial perspectives.

Conclusion: IRaaS fosters authentic smart radio environment for future mobile communications by providing practical service-oriented approach to RIS deployment in 6G networks.
...
### [231] [A Ray-Based Characterization of Satellite-to-Urban Propagation](https://arxiv.org/abs/2603.25621)
*Nicolò Cenni,Marina Barbiroli,Vittorio Degli-Esposti,Enrico M. Vitucci,Carla Amatetti,Franco Fuschini*

Main category: eess.SP

TL;DR: This paper investigates satellite-to-urban propagation using ray-tracing simulations across different urban layouts and use cases, analyzing key channel parameters like Rician K-factor and delay spread for 6G integrated networks.


<details>
  <summary>Details</summary>
Motivation: The evolution toward 6G integrated terrestrial-non-terrestrial networks requires accurate channel models for satellite-to-ground propagation in urban environments, especially at street level where obstruction and multipath effects are significant.

Method: The study uses deterministic ray-tracing simulations across three representative urban layouts (dense urban, urban, suburban) and multiple use cases (handheld devices, vehicular terminals, fixed rooftop receivers) operating across several frequency bands.

Result: Results show strong and sometimes unconventional dependence of channel dispersion and fading characteristics on satellite elevation, antenna placement, and urban morphology, highlighting the importance of these factors in channel modeling.

Conclusion: Accurate channel models for satellite-to-urban propagation must account for the complex interplay between satellite elevation, antenna placement, and urban morphology, with findings relevant for calibrating channel models for 6G link- and system-level simulations.
...
### [232] [A Learnable SIM Paradigm: Fundamentals, Training Techniques, and Applications](https://arxiv.org/abs/2603.24599)
*Hetong Wang,Yashuai Cao,Tiejun Lv*

Main category: eess.SP

TL;DR: SIMs (stacked intelligent metasurfaces) are multilayer programmable metasurfaces that perform analog computing in EM wave domain, with deep structural similarity to ANNs, enabling learnable SIM-based ML for 6G+ systems with applications in multi-user signal separation and anti-jamming.


<details>
  <summary>Details</summary>
Motivation: To leverage the architectural analogies between SIMs and ANNs to create more efficient wireless signal processing systems for 6G-and-beyond networks, addressing challenges in spectrum utilization and anti-jamming capabilities.

Method: Introduces a learnable SIM architecture based on the structural similarity to ANNs, develops a SIM-based machine learning paradigm, and creates two specific wireless signal processing schemes: multi-user signal separation and distinguishing communication signals from jamming signals.

Result: The proposed SIM-enabled signal processing system can significantly enhance spectrum utilization efficiency and anti-jamming capability in a lightweight manner, paving the way for ultra-efficient and intelligent wireless infrastructures.

Conclusion: SIMs represent a breakthrough in wireless hardware with deep connections to ANNs, enabling novel learnable architectures that can revolutionize wireless signal processing for 6G+ systems through improved spectrum efficiency and anti-jamming capabilities.
...
### [233] [Quadratic Residue Codes over $\mathbb{Z}_{121}$](https://arxiv.org/abs/2603.24689)
*Tapas Chatterjee,Priya Jain*

Main category: cs.IT

TL;DR: Construction of quadratic residue codes over ℤ₁₂₁ for primes satisfying specific congruence conditions modulo 44, with analysis of their properties, Gray images, and permutation automorphism groups.


<details>
  <summary>Details</summary>
Motivation: To extend the theory of quadratic residue codes to the ring ℤ₁₂₁ and explore their properties for primes with specific congruence conditions, enabling construction of new codes with good parameters.

Method: Define quadratic residue codes using generating idempotents for primes p satisfying p ≡ ±1, ±5, ±7, ±9, ±19 (mod 44) over ℤ₁₂₁. Analyze properties, extended codes, Gray images, and permutation automorphism groups.

Result: Successfully constructed quadratic residue codes over ℤ₁₂₁ with specific prime conditions. Showed extended codes have large permutation automorphism groups (generated by shifts, multipliers, inversion). Constructed new codes with parameters [55,5,33] and [77,7,44].

Conclusion: Quadratic residue codes over ℤ₁₂₁ can be systematically constructed for primes with specific congruence conditions, yielding codes with good parameters and permutation decoding capabilities due to their large automorphism groups.
...
### [234] [Algebraic Expander Codes](https://arxiv.org/abs/2603.24788)
*Swastik Kopparty,Itzhak Tamo*

Main category: cs.IT

TL;DR: Algebraic Expander Codes achieve positive global rate for all local rates r∈(0,1), including the problematic r≤1/2 regime, using Reed-Solomon local constraints on non-commutative group orbits.


<details>
  <summary>Details</summary>
Motivation: Standard Tanner codes fail to guarantee positive global rate when local rate r≤1/2, which is important for applications requiring algebraic local constraints like Reed-Solomon locality and multiplication properties.

Method: Define codes by evaluating structured polynomial subspaces on orbits of non-commutative AGL(1,𝔽) subgroups (translations+scalings). Prove expansion via additive character-sum estimates, analyze rate using new polynomial degree notion and polytope-volume/dimension-counting.

Result: Explicit algebraic family of Tanner-type codes with Reed-Solomon local constraints that maintain bounded-away-from-zero global rate for all r∈(0,1) while achieving constant relative distance.

Conclusion: Algebraic Expander Codes solve the low-rate regime problem for Tanner codes, enabling positive global rates with algebraic local constraints through novel construction using non-commutative group orbits and polynomial subspaces.
...
### [235] [Information-Theoretic Limits of Node Localization under Hybrid Graph Positional Encodings](https://arxiv.org/abs/2603.25030)
*Zimo Yan,Zheng Xie,Chang Liu,Yiqin Lv,Runfan Duan*

Main category: cs.IT

TL;DR: Positional encoding identifiability in graph learning is studied through hybrid encoding (anchor-distance + quantized spectral features), establishing information-theoretic impossibility regimes and showing graph-dependent localization performance.


<details>
  <summary>Details</summary>
Motivation: Positional encoding is standard in graph learning but its fundamental identifiability remains poorly understood. The paper aims to study node localization under hybrid positional encoding to understand when nodes can be uniquely identified.

Method: Uses hybrid positional encoding combining anchor-distance profiles with quantized low-frequency spectral features. Casts localization as observation-map problem and establishes information-theoretic converse identifying impossibility regimes based on anchor number, spectral dimension, and quantization level.

Result: On random 3-regular graphs, empirical crossover aligns with predicted scaling. On real-world DDI graphs, identifiability is strongly graph-dependent: DrugBank remains highly redundant, while Decagon-derived graph becomes nearly injective with sufficient spectral information.

Conclusion: Positional encoding should be understood not just as a heuristic architectural component, but as a graph-dependent structural resolution mechanism. The identifiability is governed by interplay between encoding richness and graph structure.
...
### [236] [Variance Based Transmitter Localization in Vessel-Like Molecular Communication Channels](https://arxiv.org/abs/2603.25213)
*Dağhan Erdönmez,H. Birkan Yilmaz*

Main category: cs.IT

TL;DR: Proposes a closed-form approximation using temporal variance of molecular signals to estimate transmitter-receiver distance without emission time information in vessel-like channels.


<details>
  <summary>Details</summary>
Motivation: Existing localization methods for molecular communication in healthcare applications either require emission time knowledge or multiple closely spaced receivers, limiting practical applicability in realistic scenarios.

Method: Derives a Gaussian approximation of the received molecular signal near its peak to establish an explicit variance-distance relation, enabling distance estimation without emission time information.

Result: Simulation results at capillary vessel scale show the method achieves distance prediction with error on the order of 1%.

Conclusion: The proposed simple closed-form approximation enables accurate transmitter localization in vessel-like molecular communication channels without requiring emission time information or multiple receivers.
...
### [237] [List Estimation](https://arxiv.org/abs/2603.25280)
*Nikola Zlatanov,Amin Gohari,Farzad Shahrivari,Mikhail Rudakov*

Main category: cs.IT

TL;DR: Centralized k-list estimation (producing k candidate estimates from one observation) can match or beat decentralized MMSE estimation (k agents with independent observations) in asymptotic performance scaling with k.


<details>
  <summary>Details</summary>
Motivation: Compare centralized estimation (single observation producing multiple candidate estimates) against decentralized benchmark (multiple agents with independent observations) to understand fundamental trade-offs between observation sharing and list size.

Method: Study k-list estimation as vector quantization of posterior distribution; derive high-rate asymptotic expansion with explicit constants; analyze decentralized MMSE benchmark through small-ball behavior of single-agent error; compare asymptotic scaling exponents.

Result: Centralized k-list estimation achieves k^{-2/d} decay rate; decentralized benchmark cannot exceed k^{-2/d} when error density bounded near origin, and cannot match centralized exponent when error density vanishes at origin; Gaussian cases yield explicit formulas.

Conclusion: One observation with k carefully chosen candidates can be asymptotically as effective as (and sometimes better than) k independent observations with MMSE estimation, showing surprising power of centralized list estimation.
...
### [238] [CSI-tuples-based 3D Channel Fingerprints Construction Assisted by MultiModal Learning](https://arxiv.org/abs/2603.25288)
*Chenjie Xie,Li You,Ruirong Chen,Gaoning He,Xiqi Gao*

Main category: cs.IT

TL;DR: Proposes a modular multimodal framework for constructing 3D channel fingerprints to enhance low-altitude communications in 6G networks, achieving 27.5% higher accuracy than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Low-altitude communications are crucial for 6G development as they integrate aerial and terrestrial resources, expand coverage, and improve transmission quality. 3D channel fingerprints (3D-CF) can enhance environmental understanding and assist in channel state information acquisition, reducing repeated estimations and computational complexity.

Method: Proposes a modularized multimodal framework for 3D-CF construction: 1) Models 3D-CF as CSI-tuples based on Rician fading channels, 2) Formulates the problem as multimodal regression using LAV positions, communication measurements, and geographic maps, 3) Designs three modules: correlation-based multimodal fusion (Corr-MMF), multimodal representation (MMR), and CSI regression (CSI-R).

Result: The framework achieves at least 27.5% higher accuracy than state-of-the-art algorithms across different communication scenarios, demonstrates excellent generalization ability, and shows superiority in computational complexity and inference time.

Conclusion: The proposed modular multimodal framework efficiently constructs 3D channel fingerprints for low-altitude communications, offering significant accuracy improvements, computational efficiency, and strong generalization capabilities for 6G network applications.
...
### [239] [New bounds for codes over Gaussian integers based on the Mannheim distance](https://arxiv.org/abs/2603.25362)
*Minjia Shi,Xuan Wang,Junmin An,Jon-Lark Kim*

Main category: cs.IT

TL;DR: Study of linear codes over Gaussian integers with Mannheim distance, developing analogues of classical bounds, deriving ball volumes, proving bounds, and presenting decoding algorithms showing advantages over Hamming metric.


<details>
  <summary>Details</summary>
Motivation: To extend coding theory to Gaussian integers with Mannheim metric, which may offer advantages over traditional Hamming metric for certain error patterns, and to develop theoretical foundations for such codes.

Method: Develop Mannheim-metric analogues of classical coding bounds, derive explicit formula for Mannheim ball volumes, prove various bounds, construct code families meeting bounds, analyze self-dual codes using Mannheim version of MacWilliams identity, and present decoding algorithms.

Result: Derived sphere packing bound and constraints for two-error-correcting perfect codes, proved tight bounds with code families meeting them, obtained upper bounds on minimum Mannheim distance for self-dual codes, and showed examples where Mannheim metric corrects errors not correctable under Hamming metric.

Conclusion: Mannheim metric over Gaussian integers provides a viable alternative to Hamming metric with theoretical foundations established, practical decoding algorithms developed, and demonstrated advantages for certain error correction scenarios.
...
### [240] [Investigating the Fundamental Limit: A Feasibility Study of Hybrid-Neural Archival](https://arxiv.org/abs/2603.25526)
*Marcus Armstrong,ZiWei Qiu,Huy Q. Vo,Arjun Mukherjee*

Main category: cs.IT

TL;DR: LLMs theoretically outperform classical compression methods but face hardware non-determinism and computational cost barriers. Hybrid-LLM architecture explores LLM-based archival systems, addresses GPU Butterfly Effect via logit quantization, and shows LLMs capture semantic redundancy better than classical algorithms.


<details>
  <summary>Details</summary>
Motivation: LLMs have theoretical capability to model information density beyond classical statistical methods like Lempel-Ziv, but practical deployment faces severe constraints including non-deterministic hardware and prohibitive computational costs. The paper aims to explore feasibility of LLM-based archival systems.

Method: Introduces Hybrid-LLM, a proof-of-concept architecture to investigate foundation models' "entropic capacity" in storage context. Addresses GPU Butterfly Effect (microscopic hardware non-determinism preventing data recovery) via novel logit quantization protocol, enabling rigorous measurement of neural compression rates on real-world data.

Result: Experiments reveal divergence between "retrieval-based" density (0.39 BPC on memorized literature) and "predictive" density (0.75 BPC on unseen news). Current inference latency is ≈2600× slower than Zstd, limiting immediate deployment to ultra-cold storage. LLMs successfully capture semantic redundancy inaccessible to classical algorithms.

Conclusion: LLMs demonstrate superior semantic redundancy capture compared to classical algorithms, establishing baseline for future research into semantic file systems. While current computational costs limit practical deployment, the work shows promising direction for LLM-based archival systems.
...
### [241] [Rotatable Antenna-Empowered Wireless Networks: A Tutorial](https://arxiv.org/abs/2603.25559)
...
TL;DR: Comprehensive tutorial on rotatable antenna (RA) technology covering fundamentals, architectures, applications, and practical implementations for enhanced wireless communication and sensing performance through flexible antenna orientation rotation.


<details>
  <summary>Details</summary>
Motivation: Flexible antenna architectures like fluid antenna systems, movable antennas, and pinching antennas have gained interest, with RA emerging as promising technology to enhance wireless performance through additional spatial degrees of freedom via antenna boresight rotation without changing physical positions.

Method: Establishes unified mathematical framework for RA systems including antenna/array rotation models and channel models covering near/far-field propagation, wideband frequency selectivity, and polarization effects. Investigates rotation optimization in communication/sensing scenarios, channel estimation strategies, practical configurations, and presents prototypes with experimental validation.

Result: Provides comprehensive tutorial covering RA fundamentals, historical evolution, mathematical modeling, optimization algorithms, channel estimation methods, practical deployment strategies, and experimental validation through prototypes demonstrating performance gains enabled by antenna rotation.

Conclusion: RA technology offers significant potential for enhancing wireless networks through additional spatial degrees of freedom, with promising extensions to emerging wireless paradigms and open challenges identified to inspire future research in this field.
...
### [242] [Kakeya Conjecture and Conditional Kolmogorov Complexity](https://arxiv.org/abs/2603.25611)
*Nicholas G. Polson,Daniel Zantedeschi*

Main category: cs.IT

TL;DR: This paper develops an information-theoretic framework for algorithmic complexity under regular identifiable fibering, examining when fiber structure reduces description length and when sets remain incompressible at ambient dimension.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how information about fiber labels affects compression of geometric sets, particularly in relation to the Kakeya conjecture about sets containing unit segments in every direction having full Hausdorff dimension.

Method: The method uses algorithmic dimension and the point-to-set principle of Lutz and Lutz, translating pointwise Kolmogorov complexity into Hausdorff dimension. It formulates a directional compression principle and proves analytical results about complexity splitting under effectively bi-Lipschitz, identifiable, and computable fibering.

Result: The paper proves that under effectively bi-Lipschitz, identifiable, and computable fibering, the complexity of a point splits additively as the sum of fiber-label complexity and along-fiber residual complexity, up to logarithmic overhead, via the chain rule for Kolmogorov complexity.

Conclusion: The paper isolates adaptive-fibering obstruction as the key difficulty in the Kakeya conjecture for dimensions n≥4 and proposes a formal research program connecting geometric measure theory, algorithmic complexity, and information-theoretic compression.
...
<details>
  <summary>Details</summary>
Motivation: To create a benchmark that specifically evaluates fluid adaptive efficiency on novel tasks, focusing on core reasoning abilities without relying on language or external knowledge, building on previous ARC-AGI versions.

Method: Designs novel, abstract, turn-based environments that require agents to explore, infer goals, build internal models of dynamics, and plan actions. Uses Core Knowledge priors only, with difficulty calibrated through extensive human testing. Includes efficiency-based scoring framework grounded in human action baselines.
...
Conclusion: ARC-AGI-3 provides a challenging benchmark for evaluating agentic intelligence, revealing substantial limitations in current AI systems' ability to perform fluid adaptive reasoning on novel tasks compared to human capabilities.
...
TL;DR: Multi-agent LLM systems reach consensus through mutual in-context learning, described by a minimal model (Quantized Simplex Gossip) showing a crossover from random "memetic drift" to bias-amplifying selection regimes.


<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM systems shape consequential decisions, but it's unclear whether their outcomes reflect collective reasoning, systematic bias, or chance. Recent naming games show populations reach consensus even without individual preferences, requiring explanation of the underlying mechanism.

Method: Introduces Quantized Simplex Gossip (QSG) minimal model where agents maintain internal belief states and learn from each other's sampled outputs. Analyzes sampling-driven "memetic drift" regime, derives scaling laws for drift-induced polarization, and validates with QSG simulations and LLM naming-game experiments.

Result: QSG reveals consensus emerges through mutual in-context learning where one agent's arbitrary choice becomes another's evidence. Identifies crossover from drift-dominated (consensus as lottery) to selection regimes (weak biases amplified). Scaling laws derived for polarization as function of population size, bandwidth, adaptation rate, and uncertainty.

Conclusion: Provides framework for studying collective mechanisms of social representation formation in multi-agent systems, explaining how consensus emerges through microscopic mutual learning processes and identifying regimes where outcomes are random vs. shaped by biases.
...
TL;DR: AutoSAM automates SAM thermal-hydraulics input file generation using LLM agents with multimodal document analysis, achieving high accuracy across reactor case studies.


<details>
  <summary>Details</summary>
Motivation: Manual creation of SAM input files from heterogeneous engineering documents is labor-intensive and error-prone, requiring automation to improve efficiency and accuracy in reactor design and safety analysis.

Method: Agentic framework combining LLM with retrieval-augmented generation over solver documentation, plus specialized tools for PDFs, images, spreadsheets, and text files. Uses multimodal retrieval pipeline with scientific text extraction, vision-based figure interpretation, semantic embedding, and query answering.

Result: Achieved 100% utilization of structured inputs, ~88% extraction from PDF text, and 100% completeness in vision-based geometric extraction. Produced runnable SAM models across four case studies with explicit identification of missing data and assumed values.

Conclusion: AutoSAM demonstrates a practical path toward prompt-driven reactor modeling, enabling analysts to provide system descriptions while the agent transparently generates executable SAM simulations with human-auditable intermediate representations.
...
TL;DR: Evolutionary game theory model shows AI safety requires both user monitoring and developer penalties to avoid unsafe AI adoption.


<details>
  <summary>Details</summary>
Motivation: Existing AI governance models treat trust as one-shot adoption rather than dynamic process; need to understand how user trust and developer safety choices co-evolve through repeated interactions.

Method: Model trust as reduced monitoring in repeated asymmetric user-developer interactions; use evolutionary game theory with infinite-population replicator analysis, stochastic finite-population dynamics, and reinforcement learning (Q-learning) simulations.

Result: Three robust long-run regimes: no adoption with unsafe development, unsafe but widely adopted systems, and safe systems widely adopted. Safe adoption requires penalties exceeding safety costs and affordable user monitoring.

Conclusion: Neither regulation alone nor blind trust prevents unsafe AI evolution; governance needs transparency, low-cost monitoring, and meaningful sanctions to achieve safe, widely adopted AI systems.
...
TL;DR: This paper provides the first formal verification framework for agent protocols, proving structural bisimilarity between Schema-Guided Dialogue (SGD) and Model Context Protocol (MCP), identifying MCP's expressivity gaps, and proposing MCP+ extensions for full behavioral equivalence.


<details>
  <summary>Details</summary>
Motivation: The emergence of LLM agents using external tools creates urgent need for formal verification of agent protocols. While SGD (research framework) and MCP (industry standard) both enable dynamic service discovery, their formal relationship remains unexplored despite conceptual convergence.

Method: Developed first process calculus formalization of SGD and MCP, proved structural bisimilarity under mapping Φ, analyzed reverse mapping Φ⁻¹ as partial/lossy, identified five principles for full behavioral equivalence, formalized as type-system extensions MCP+, proved MCP+ isomorphic to SGD.

Result: Proved SGD and MCP are structurally bisimilar under Φ, but reverse mapping is partial/lossy revealing MCP's expressivity gaps. Identified five necessary/sufficient principles for full equivalence. Formalized MCP+ extensions and proved MCP+ isomorphic to SGD.

Conclusion: Provides first formal foundation for verified agent systems, establishes schema quality as provable safety property, and bridges research (SGD) and industry (MCP) standards through formal verification framework.
...
TL;DR: Researchers propose two agentic AI systems (Self-Regulation Loop and Co-Regulation Design Agentic Loop) to mitigate design fixation in LLM-based engineering design agents, showing CRDAL improves performance without significant computational cost increase.


<details>
  <summary>Details</summary>
Motivation: LLM-based design agents suffer from design fixation similar to human designers - they get stuck on existing paradigms and fail to explore alternatives, leading to suboptimal engineering solutions.

Method: Proposed two novel systems: 1) Self-Regulation Loop (SRL) where the Design Agent monitors its own metacognition, and 2) Co-Regulation Design Agentic Loop (CRDAL) where a Metacognitive Co-Regulation Agent assists the Design Agent in metacognition to mitigate fixation.

Result: CRDAL generated better performing battery pack designs without significant computational cost increase compared to baseline (RWL) and SRL. CRDAL navigated design space more effectively. SRL explored different design space regions but didn't significantly outperform RWL.

Conclusion: Co-regulation approach (CRDAL) effectively mitigates design fixation in LLM agents, providing practical implications for developing better agentic AI systems for engineering design tasks.
...
TL;DR: The paper addresses the degradation of probe routing in multimodal LLMs (MLLMs) compared to text-only LLMs, and introduces two complementary methods - Attention Probe and KL-Regularized LoRA Probe (ReLope) - to improve routing effectiveness by enhancing hidden state quality.


<details>
  <summary>Details</summary>
Motivation: Probe routing, which predicts small model correctness using hidden states, works well in text-only LLMs but degrades substantially in multimodal LLMs. Visual inputs weaken the separability of correctness signals in hidden states, making them harder to extract with standard probe designs.

Method: Two complementary approaches: 1) Attention Probe aggregates hidden states from preceding layer based on attention scores to recover distributed correctness signals; 2) KL-Regularized LoRA Probe (ReLope) inserts lightweight LoRA adapter and applies KL regularizer to learn routing-aware representations.
...
Conclusion: The paper successfully addresses the challenge of probe routing degradation in MLLMs by introducing novel probe designs that enhance hidden state quality, leading to more effective routing systems that balance performance and cost in multimodal settings.
...
### [250] [Resisting Humanization: Ethical Front-End Design Choices in AI for Sensitive Contexts](https://arxiv.org/abs/2603.24853)
*Silvia Rossi,Diletta Huyskes,Mackenzie Jorgensen*

Main category: cs.AI

TL;DR: The paper argues that front-end design choices in AI systems, especially humanizing elements in conversational interfaces, have significant ethical implications that are often overlooked in favor of back-end technical concerns.


<details>
  <summary>Details</summary>
Motivation: Current AI ethics debates focus too much on back-end issues (data, algorithms, training) while neglecting the ethical significance of front-end design choices, particularly humanizing elements in conversational AI interfaces that can shape user perceptions and behaviors.

Method: The paper draws on research from HCI, conversational AI, and value-sensitive design, and uses a case study of Chayn's AI systems for gender-based violence survivors to illustrate how trauma-informed design principles can guide ethical interface decisions.

Result: Humanization in AI front-end design is a value-driven choice that affects users' mental models, trust calibration, and behavioral responses. Interfaces can misalign expectations, foster misplaced trust, and undermine user autonomy, especially in vulnerable contexts.

Conclusion: Ethical front-end AI design represents procedural ethics enacted through interaction choices, not just system logic. The Chayn case demonstrates how ethical considerations can motivate principled restraint in interface design, challenging engagement-focused norms in AI products.
...
### [251] [SentinelAI: A Multi-Agent Framework for Structuring and Linking NG9-1-1 Emergency Incident Data](https://arxiv.org/abs/2603.24856)
*Kliment Ho,Ilya Zaslavsky*

Main category: cs.AI

TL;DR: SentinelAI is a framework that transforms emergency communications into standardized, machine-readable datasets to support real-time incident tracking and cross-agency data integration.


<details>
  <summary>Details</summary>
Motivation: Emergency response systems generate data from multiple agencies, but correlating and updating this information across sources according to Next Generation 9-1-1 standards is challenging. Current systems lack real-time integration capabilities for providing a unified view of evolving incidents.

Method: SentinelAI implements a scalable processing pipeline with specialized agents. The EIDO Agent ingests raw communications and produces NENA-compliant Emergency Incident Data Object JSON, transforming data into standardized, machine-readable formats.

Result: The framework enables transformation of emergency communications into standardized datasets that support integration, composite incident construction, and cross-source reasoning, addressing the challenge of real-time data correlation across multiple sources.

Conclusion: SentinelAI provides a practical solution for emergency response data integration by creating a continuous stream of operational updates that offer timely and unified views of evolving incidents through standardized data processing.
...
TL;DR: DreamHouse is a benchmark for evaluating VLMs' physical generative reasoning in residential construction, testing geometric, structural, constructability, and code-compliance constraints through iterative agentic interaction.


<details>
  <summary>Details</summary>
Motivation: Current VLM evaluation focuses too much on visual realism while ignoring whether models understand the step-by-step processes and physical dependencies needed to actually build artifacts, which is essential for automating design-to-construction pipelines.

Method: The authors ground their benchmark in residential timber-frame construction with codified engineering standards. They curate 26,000+ structures across 13 architectural styles verified to construction-document standards (LOD 350) and develop a deterministic 10-test structural validation framework. Unlike static benchmarks, DreamHouse supports iterative agentic interaction where models observe intermediate build states, generate construction actions, and receive structured feedback.

Result: Extensive experiments with state-of-the-art VLMs reveal substantial capability gaps that are largely invisible on existing leaderboards, showing that current models struggle with physical generative reasoning despite performing well on visual tasks.

Conclusion: Physical validity is a critical evaluation axis orthogonal to visual realism, and physical generative reasoning represents a distinct and underdeveloped frontier in multimodal intelligence that needs dedicated benchmarks like DreamHouse.
...
TL;DR: Platform-deterministic inference is necessary and sufficient for trustworthy AI, with trust entropy quantifying non-determinism costs; a pure integer inference engine achieves bitwise identical outputs across architectures.


<details>
  <summary>Details</summary>
Motivation: Current AI systems lack trustworthiness due to platform non-determinism, particularly from IEEE 754 floating-point arithmetic, which prevents verifiable identical outputs across different hardware platforms.

Method: Formalized the Determinism Thesis, introduced trust entropy to quantify non-determinism costs, and constructed a pure integer inference engine that avoids floating-point arithmetic to achieve bitwise identical outputs across ARM and x86 architectures.

Result: Achieved zero hash mismatches in 82 cross-architecture tests on models up to 6.7B parameters; four geographically distributed nodes produced identical outputs verified by 356 on-chain attestation transactions.

Conclusion: Platform determinism is fundamental to all major AI trust properties (fairness, robustness, privacy, safety, alignment), and AI trust is fundamentally a question of arithmetic rather than just statistical properties.
...
### [254] [LogitScope: A Framework for Analyzing LLM Uncertainty Through Information Metrics](https://arxiv.org/abs/2603.24929)
*Farhan Ahmed,Yuya Jeremy Ong,Chad DeLuca*

Main category: cs.AI

TL;DR: LogitScope is a lightweight framework for analyzing LLM uncertainty through token-level information metrics like entropy and varentropy, enabling model confidence analysis without labeled data.
...
Method: Introduces LogitScope framework that computes token-level information metrics (entropy, varentropy) from probability distributions at each generation step, using lazy evaluation for computational efficiency and being model-agnostic and compatible with HuggingFace models.

Result: LogitScope reveals patterns in model confidence, identifies potential hallucinations, exposes decision points with high uncertainty, and demonstrates utility across uncertainty quantification, model behavior analysis, and production monitoring applications.

Conclusion: LogitScope provides a practical, efficient framework for researchers and practitioners to inspect LLM behavior during inference through token-level uncertainty analysis without requiring labeled data or semantic interpretation.
...
### [255] [Decoding Market Emotions in Cryptocurrency Tweets via Predictive Statement Classification with Machine Learning and Transformers](https://arxiv.org/abs/2603.24933)
*Moein Shahiki Tash,Zahra Ahani,Mohim Tash,Mostafa Keikhay Farzaneh,Ari Y. Barrera-Animas,Olga Kolesnikova*

Main category: cs.AI

TL;DR: This paper introduces a two-stage classification framework for identifying predictive statements in cryptocurrency tweets, using binary classification (predictive vs non-predictive) followed by sentiment categorization (incremental, decremental, neutral), with GPT-based data augmentation and emotion analysis.


<details>
  <summary>Details</summary>
Motivation: With the growing prominence of cryptocurrencies and increased speculative activity on social media, there's a need to systematically analyze and classify predictive statements about cryptocurrency prices to understand market sentiment and prediction patterns.

Method: Two-stage classification: Task 1 (binary classification of predictive vs non-predictive tweets) and Task 2 (categorizing predictive tweets as incremental, decremental, or neutral). Used combined manual and GPT-based annotation, SenticNet for emotion feature extraction, GPT-generated paraphrasing for data augmentation, and evaluated various ML, DL, and transformer models.
...
Conclusion: The proposed framework effectively classifies predictive cryptocurrency tweets, with GPT-based augmentation addressing class imbalance. The approach provides insights into emotional patterns in cryptocurrency predictions and demonstrates the effectiveness of combining traditional and transformer-based methods for different classification tasks.
...
### [256] [FinMCP-Bench: Benchmarking LLM Agents for Real-World Financial Tool Use under the Model Context Protocol](https://arxiv.org/abs/2603.24943)
*Jie Zhu,Yimin Tian,Boyang Li,Kehao Wu,Zhongzhi Liang,Junhui Li,Xianyin Zhang,Lifan Guo,Feng Chen,Yong Liu,Chi Zhang*

Main category: cs.AI

TL;DR: FinMCP-Bench is a benchmark for evaluating LLMs on real-world financial problems using tool invocation of financial model context protocols, with 613 samples across 10 scenarios and 33 sub-scenarios.


<details>
  <summary>Details</summary>
Motivation: There's a need for standardized evaluation of LLMs in solving real-world financial problems through tool invocation, particularly using financial model context protocols (MCPs). Current benchmarks may lack diversity, authenticity, and proper assessment of tool invocation capabilities in financial contexts.

Method: Created FinMCP-Bench with 613 samples spanning 10 main scenarios and 33 sub-scenarios, featuring both real and synthetic user queries. Incorporated 65 real financial MCPs and three sample types: single tool, multi-tool, and multi-turn. Proposed metrics to measure tool invocation accuracy and reasoning capabilities.

Result: The benchmark provides a comprehensive testbed with diverse financial scenarios and protocols. It enables systematic assessment of mainstream LLMs across different complexity levels (single tool, multi-tool, multi-turn).

Conclusion: FinMCP-Bench offers a standardized, practical, and challenging evaluation framework for advancing research on financial LLM agents, particularly focusing on their tool invocation capabilities in real-world financial contexts.
...
### [257] [Shopping with a Platform AI Assistant: Who Adopts, When in the Journey, and What For](https://arxiv.org/abs/2603.24947)
*Se Yan,Han Zhong,Zemin,Zhong,Wenyu Zhou*

Main category: cs.AI

TL;DR: Embedded shopping AI (Ctrip's Wendao) serves as a complementary interface for exploratory product discovery rather than a substitute for conventional search, with adoption patterns differing from general-purpose AI tools.


<details>
  <summary>Details</summary>
Motivation: To understand how consumers actually adopt and use platform-embedded shopping AI in e-commerce, given the lack of large-scale empirical evidence on this emerging technology.

Method: Analysis of 31 million users on Ctrip (China's largest online travel platform) studying "Wendao," an LLM-based AI assistant integrated into the platform, examining adoption patterns, usage timing, and query types.

Result: 1) Adoption highest among older consumers, female users, and highly engaged existing users (reversing typical AI adoption patterns); 2) AI chat appears in same phase as traditional search but before order placement, with interleaving between modalities; 3) Used disproportionately for exploratory, hard-to-keyword tasks (42% attraction queries).

Conclusion: Embedded shopping AI functions as a complementary interface for exploratory product discovery rather than a substitute for conventional search, with distinct adoption patterns and usage behaviors in e-commerce contexts.
...
### [258] [Can MLLMs Read Students' Minds? Unpacking Multimodal Error Analysis in Handwritten Math](https://arxiv.org/abs/2603.24961)
*Dingjie Song,Tianlong Xu,Yi-Fan Zhang,Hang Li,Zhiling Yan,Xing Fan,Haoyang Li,Lichao Sun,Qingsong Wen*

Main category: cs.AI

TL;DR: ScratchMath is a new benchmark for analyzing handwritten math scratchwork, featuring 1,720 student samples with error explanation and classification tasks, revealing significant gaps between current MLLMs and human experts.


<details>
  <summary>Details</summary>
Motivation: Existing educational NLP focuses on textual responses and ignores the complexity of authentic handwritten scratchwork. Current MLLMs prioritize generating correct answers rather than diagnosing student errors, creating a need for specialized benchmarks.

Method: Created ScratchMath benchmark with 1,720 mathematics samples from Chinese primary and middle school students. Supports two tasks: Error Cause Explanation (ECE) and Error Cause Classification (ECC) with seven error types. Used rigorous human-machine collaborative annotation with expert labeling, review, and verification.

Result: Evaluated 16 leading MLLMs, revealing significant performance gaps relative to human experts, especially in visual recognition and logical reasoning. Proprietary models outperformed open-source models, with large reasoning models showing strong potential for error explanation.

Conclusion: ScratchMath addresses critical gaps in educational assessment by providing a specialized benchmark for handwritten math scratchwork analysis. The performance gaps highlight the need for improved multimodal reasoning capabilities in educational AI systems.
...
### [259] [Design Once, Deploy at Scale: Template-Driven ML Development for Large Model Ecosystems](https://arxiv.org/abs/2603.24963)
...
TL;DR: SMT framework standardizes ML model building for recommendation systems, reducing engineering effort while improving performance and technique propagation.


<details>
  <summary>Details</summary>
Motivation: Large-scale advertising platforms face significant challenges in maintaining diverse ML models for different optimization events, requiring substantial engineering effort and suffering from slow propagation of ML innovations across the ecosystem.

Method: Proposed Standard Model Template (SMT) - a framework that generates high-performance models adaptable to diverse data distributions and optimization events using standardized, composable ML components.

Result: Evaluation in Meta's ads ranking ecosystem showed: 0.63% average improvement in cross-entropy, 92% reduction in per-model iteration engineering time, and 6.3x increase in technique-model pair adoption throughput.

Conclusion: Standardized model building can outperform independent per-model optimization, challenging the conventional wisdom that diverse optimization goals require diversified ML model design.
...
### [260] [The Anatomy of Uncertainty in LLMs](https://arxiv.org/abs/2603.24967)
*Aditya Taparia,Ransalu Senanayake,Kowshik Thopalli,Vivek Narayanaswamy*

Main category: cs.AI

TL;DR: The paper proposes a new uncertainty decomposition framework for LLMs that breaks uncertainty into three semantic components: input ambiguity, knowledge gaps, and decoding randomness, providing better insights for reliability auditing and hallucination detection.
...
Method: Proposes a three-component uncertainty decomposition framework: (1) input ambiguity from ambiguous prompts, (2) knowledge gaps from insufficient parametric evidence, and (3) decoding randomness from stochastic sampling.
...
Conclusion: The framework enables targeted interventions and more trustworthy LLM systems by offering semantic insights into uncertainty sources rather than just numerical scores.
...
### [261] [Rethinking Failure Attribution in Multi-Agent Systems: A Multi-Perspective Benchmark and Evaluation](https://arxiv.org/abs/2603.25001)
...
TL;DR: The paper proposes multi-perspective failure attribution for multi-agent systems, introduces MP-Bench benchmark, and shows that previous conclusions about LLMs struggling with failure attribution stem from flawed benchmark designs.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks and methods for multi-agent system failure attribution assume single deterministic root causes, but in practice MAS failures often have multiple plausible attributions due to complex inter-agent dependencies and ambiguous execution trajectories.

Method: Proposes multi-perspective failure attribution paradigm that accounts for attribution ambiguity, introduces MP-Bench benchmark specifically designed for this setting, and develops a new evaluation protocol tailored to multi-perspective attribution.

Result: Extensive experiments show that prior conclusions suggesting LLMs struggle with failure attribution are largely driven by limitations in existing benchmark designs, highlighting the necessity of multi-perspective benchmarks and evaluation protocols.

Conclusion: Multi-perspective benchmarks and evaluation protocols are essential for realistic and reliable MAS debugging, as they better capture the ambiguity inherent in real-world multi-agent system failures.
...
### [262] [A Public Theory of Distillation Resistance via Constraint-Coupled Reasoning Architectures](https://arxiv.org/abs/2603.25022)
*Peng Wei,Wesley Shu*

Main category: cs.AI

TL;DR: This paper proposes a theoretical framework to reduce the asymmetry between capability transfer and governance transfer in AI systems by coupling high-level capabilities with internal stability constraints that make distillation less valuable as a shortcut.


<details>
  <summary>Details</summary>
Motivation: The main risk in frontier AI is not just copying models, but that useful capabilities can be transferred more cheaply than the governance structures that originally accompanied them, creating a dangerous asymmetry between capability access and responsible oversight.

Method: The paper introduces a constraint-coupled reasoning framework with four elements: bounded transition burden, path-load accumulation, dynamically evolving feasible regions, and a capability-stability coupling condition. This framework is intentionally public-safe, omitting proprietary implementation details.

Result: The paper provides a theoretical contribution rather than operational results, offering a falsifiable architectural thesis, a clear threat model, and experimentally testable hypotheses for future work on distillation resistance, alignment, and model governance.

Conclusion: By coupling high-level capabilities to internal stability constraints that shape state transitions over time, distillation becomes less valuable as a shortcut, potentially reducing the asymmetry between capability transfer and governance transfer in AI systems.
...
### [263] [System-Anchored Knee Estimation for Low-Cost Context Window Selection in PDE Forecasting](https://arxiv.org/abs/2603.25025)
...
<details>
  <summary>Details</summary>
Motivation: Context-window selection for autoregressive neural PDE simulators is an unformalized problem. Existing approaches (exhaustive validation, direct low-cost search, system-theoretic memory estimation) are either expensive, brittle, or not aligned with downstream rollout performance.

Method: SAKE (System-Anchored Knee Estimation) is a two-stage method: 1) identifies a small structured candidate set from physically interpretable system anchors, 2) performs knee-aware downstream selection within this candidate set.

Result: Across eight PDEBench families, SAKE was the strongest overall matched-budget low-cost selector: 67.8% Exact, 91.7% Within-1, 6.1% mean regret@knee, with a cost ratio of 0.051 (94.9% normalized search-cost savings).

Conclusion: SAKE successfully formalizes and solves the context-window selection problem for neural PDE simulators, providing an efficient, interpretable, and high-performing solution that significantly reduces computational costs while maintaining strong performance.
...
### [264] [From Stateless to Situated: Building a Psychological World for LLM-Based Emotional Support](https://arxiv.org/abs/2603.25031)
*Boning Zhao,Clover Hu,Xinnuo Li*

Main category: cs.AI

TL;DR: LEKIA 2.0 is a situated LLM architecture that separates cognitive and executive layers to maintain temporal continuity and user consent boundaries in emotional support dialogues, achieving 31% improvement over baselines.


<details>
  <summary>Details</summary>
Motivation: Current LLMs rely on local next-token prediction, making them stateless and unable to maintain temporal continuity, stage awareness, and user consent boundaries required for multi-turn emotional support interventions. This leads to premature advancement, stage misalignment, and boundary violations.

Method: Proposes LEKIA 2.0, a situated LLM architecture that separates cognitive layer (situational modeling) from executive layer (intervention execution), creating a sustainably updatable external situational structure to maintain stable representations of user's situation and consent boundaries.

Result: LEKIA achieved an average absolute improvement of approximately 31% over prompt-only baselines in deep intervention loop completion, using a Static-to-Dynamic online evaluation protocol for multi-turn interaction.
...
### [265] [Mechanistically Interpreting Compression in Vision-Language Models](https://arxiv.org/abs/2603.25035)
*Veeraraju Elluru,Arth Singh,Roberto Aguero,Ajay Agarwal,Debojyoti Das,Hreetam Paul*

Main category: cs.AI

TL;DR: Compressed VLMs maintain some internal structure but pruning reduces safety behaviors more than quantization, prompting creation of new safety benchmark.
...
Result: Pruning keeps circuit structure intact but rotates and attenuates internal features, while quantization modifies circuits at a higher level but leaves surviving features better aligned. Pruning causes sharp drop in genuine refusal behavior, showing compression choice has safety implications.

Conclusion: Different compression techniques affect VLMs differently - pruning significantly reduces safety behaviors while quantization preserves features better. The safety implications of compression must be considered, and the new VLMSafe-420 benchmark helps evaluate these effects systematically.
...
### [266] [MP-MoE: Matrix Profile-Guided Mixture of Experts for Precipitation Forecasting](https://arxiv.org/abs/2603.25046)
*Huyen Ngoc Tran,Dung Trung Tran,Hong Nguyen,Xuan Vu Phan,Nam-Phong Nguyen*

Main category: cs.AI

TL;DR: MP-MoE framework improves tropical precipitation forecasting by combining intensity loss with Matrix Profile objective to reduce double penalty effect from temporal misalignments.


<details>
  <summary>Details</summary>
Motivation: NWP models struggle with accuracy in tropical regions like Vietnam due to complex topography and convective instability. Existing post-processing methods suffer from "double penalty" effect from point-wise objective functions when there are minor temporal misalignments.

Method: Proposes Matrix Profile-guided Mixture of Experts (MP-MoE) framework that integrates conventional intensity loss with structural-aware Matrix Profile objective. Uses subsequence-level similarity instead of point-wise errors for more reliable expert selection and reduces penalization from phase shifts.

Result: MP-MoE outperforms raw NWP and baseline learning methods in Mean Critical Success Index (CSI-M) for heavy rainfall events, while significantly reducing Dynamic Time Warping (DTW) values. Evaluated on rainfall datasets from two major river basins in Vietnam across multiple horizons (1-hour intensity and accumulated rainfall over 12, 24, 48 hours).

Conclusion: The framework effectively captures peak rainfall intensities and preserves morphological integrity of storm events, demonstrating efficacy for tropical precipitation forecasting with complex temporal dynamics.
...
### [267] [Sparse Visual Thought Circuits in Vision-Language Models](https://arxiv.org/abs/2603.25075)
*Yunpeng Zhou*

Main category: cs.AI

TL;DR: SAE features in multimodal models often lack modular composability - intervening on feature unions causes output drift and accuracy degradation despite task-selective features showing some steering capability.


<details>
  <summary>Details</summary>
Motivation: To test whether sparse autoencoder (SAE) features form modular, composable units for reasoning, which is an assumption underlying many intervention-based steering methods in multimodal models.

Method: Developed a reproducible causal pipeline to localize and test sparse visual thought circuits in Qwen3-VL-8B. Used a controlled synthetic benchmark with seven task types and three difficulty levels, identified mid-decoder locus for task information via linear probes, trained SAEs at this layer, constructed task-selective feature sets via explicit rules, and performed inference-time scaling and ablation while quantifying accuracy and drift.

Result: Found that intervening on task-selective feature sets modestly improves reasoning accuracy, but intervening on the union of two such sets reliably induces output drift (large unintended prediction changes) and degrades accuracy, even under norm-matched perturbations. This non-modular circuit interference suggests shared internal pathways where feature unions amplify activation shifts.

Conclusion: SAE features often lack modular composability, challenging assumptions about feature-based steering methods. The study provides a rigorous diagnostic framework for more reliable VLM control and clarifies boundaries of SAE feature composability, validated across multiple VLM families and five diverse datasets.
...
### [268] [ElephantBroker: A Knowledge-Grounded Cognitive Runtime for Trustworthy AI Agents](https://arxiv.org/abs/2603.25097)
*Cristian Lupascu,Alexandru Lupascu*

Main category: cs.AI

TL;DR: ElephantBroker is an open-source cognitive runtime that combines Neo4j knowledge graphs with Qdrant vector stores to provide verifiable agent memory with provenance tracking and safety features.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents lack mechanisms to track provenance or trustworthiness of stored knowledge in high-stakes, multi-turn settings where factual grounding is critical. Existing memory systems rely on flat key-value stores or plain vector retrieval without verification capabilities.

Method: Unifies Neo4j knowledge graph with Qdrant vector store through Cognee SDK. Implements complete cognitive loop with hybrid five-source retrieval pipeline, 11-dimension competitive scoring engine, four-state evidence verification, five-stage context lifecycle, six-layer safety pipeline, AI firewall, nine-stage consolidation engine, and numeric authority model for access control.

Result: Validated through comprehensive test suite of over 2,200 tests spanning unit, integration, and end-to-end levels. Supports three deployment tiers, five profile presets with inheritance, multi-gateway isolation, and management dashboard for human oversight.
...
### [269] [When Sensing Varies with Contexts: Context-as-Transform for Tactile Few-Shot Class-Incremental Learning](https://arxiv.org/abs/2603.25115)
*Yifeng Lin,Aiping Huang,Wenxi Liu,Si Wu,Tiesong Zhao,Zheng-Jun Zha*

Main category: cs.AI

TL;DR: CaT-FSCIL addresses Few-Shot Class-Incremental Learning challenges in tactile sensing by modeling acquisition context as transformable components and using uncertainty-based calibration.


<details>
  <summary>Details</summary>
Motivation: FSCIL is vulnerable in tactile sensing due to diverse acquisition contexts (devices, contact states, interaction settings) that lack standardization, causing performance degradation.

Method: Decomposes acquisition context into low-dimensional structured component (modeled as invertible Context-as-Transform family) and high-dimensional residual component. Uses inverse-transform canonicalization with pseudo-context consistency loss for the former, and Uncertainty-Conditioned Prototype Calibration for the latter to adjust biased prototypes and decision boundaries.
...
Conclusion: CaT-FSCIL effectively handles acquisition context challenges in tactile FSCIL by separating context components and applying specialized techniques for each, achieving state-of-the-art performance.
...
### [270] [RubricEval: A Rubric-Level Meta-Evaluation Benchmark for LLM Judges in Instruction Following](https://arxiv.org/abs/2603.25133)
*Tianjun Pan,Xuan Lin,Wenyan Yang,Qianyu He,Shisong Chen,Licai Qi,Wanqing Xu,Hongwei Feng,Bo Xu,Yanghua Xiao*

Main category: cs.AI

TL;DR: RubricEval is a new benchmark for meta-evaluating rubric-based evaluation of LLM instruction following, revealing that even state-of-the-art judges like GPT-4o perform poorly (55.97%) on hard cases, with insights that rubric-level evaluation, explicit reasoning, and their combination improve accuracy and reduce variance.


<details>
  <summary>Details</summary>
Motivation: Rubric-based evaluation is widely used for assessing LLM instruction following, but its reliability is unclear. Existing meta-evaluation focuses on response-level assessment rather than the fine-grained rubric-level judgments that these evaluations actually rely on, creating a gap in understanding evaluation quality.

Method: The authors introduce RubricEval, the first rubric-level meta-evaluation benchmark for instruction following. It includes diverse instructions and responses across multiple categories and model sources, with 3,486 quality-controlled instances and Easy/Hard subsets to better differentiate judge performance. They establish a rubric taxonomy and analyze evaluation paradigms.

Result: Rubric-level judging remains challenging: GPT-4o achieves only 55.97% accuracy on the Hard subset. Rubric-level evaluation outperforms checklist-level approaches, explicit reasoning improves accuracy, and combining both reduces inter-judge variance. The benchmark identifies common failure modes in instruction-following evaluation.

Conclusion: Rubric-based evaluation of instruction following needs improvement, as current judges struggle with fine-grained rubric-level judgments. The RubricEval benchmark enables better meta-evaluation and provides actionable insights for developing more reliable evaluation methods, with rubric-level assessment and explicit reasoning being key improvements.
...
### [271] [UniAI-GraphRAG: Synergizing Ontology-Guided Extraction, Multi-Dimensional Clustering, and Dual-Channel Fusion for Robust Multi-Hop Reasoning](https://arxiv.org/abs/2603.25152)
...
<details>
  <summary>Details</summary>
Motivation: Existing GraphRAG frameworks have limitations in cross-industry adaptability, community report integrity, and retrieval performance, especially for complex reasoning, multi-hop queries, and domain-specific QA tasks.

Method: Three core innovations: (1) Ontology-Guided Knowledge Extraction using predefined Schema to guide LLMs in entity/relation identification; (2) Multi-Dimensional Community Clustering Strategy with alignment completion, attribute-based clustering, and multi-hop relationship clustering; (3) Dual-Channel Graph Retrieval Fusion combining hybrid graph and community retrieval.
...
Conclusion: UniAI-GraphRAG provides an enhanced framework that addresses limitations of existing GraphRAG systems and demonstrates superior performance in complex reasoning and domain-specific QA tasks.
...
### [272] [Trace2Skill: Distill Trajectory-Local Lessons into Transferable Agent Skills](https://arxiv.org/abs/2603.25158)
*Jingwei Ni,Yihao Liu,Xinpeng Liu,Yutao Sun,Mengyu Zhou,Pengyu Cheng,Dexin Wang,Xiaoxi Jiang,Guanjun Jiang*

Main category: cs.AI

TL;DR: Trace2Skill: A framework that analyzes diverse execution traces to automatically generate comprehensive, transferable skills for LLM agents without parameter updates.


<details>
  <summary>Details</summary>
Motivation: Manual skill authoring for LLM agents doesn't scale, while automated methods often produce fragile or fragmented skills that don't generalize well. Current approaches either rely on shallow parametric knowledge or overfit to specific trajectories.

Method: Uses parallel sub-agents to analyze diverse execution traces, extracts trajectory-specific lessons, then hierarchically consolidates them into unified, conflict-free skill directories via inductive reasoning. Supports both deepening existing skills and creating new ones from scratch.

Result: Significantly outperforms strong baselines including Anthropic's official xlsx skills in domains like spreadsheet, VisionQA, and math reasoning. Evolved skills transfer across LLM scales and generalize to out-of-distribution settings (e.g., 57.65% improvement on WikiTableQuestions).

Conclusion: Complex agent experience can be packaged into highly transferable declarative skills without parameter updates, external retrieval modules, or requiring large models (works with 35B parameter models).
...
### [273] [The Competence Shadow: Theory and Bounds of AI Assistance in Safety Engineering](https://arxiv.org/abs/2603.25197)
*Umair Siddique*

Main category: cs.AI

TL;DR: AI assistance in safety engineering creates "competence shadows" - systematic narrowing of human reasoning that can degrade safety analysis quality more than expected. The paper argues AI assistance is a collaboration design problem, not just software procurement.


<details>
  <summary>Details</summary>
Motivation: As AI assistants integrate into safety engineering for Physical AI systems, there's a critical need to understand whether AI assistance improves safety analysis quality or introduces systematic blind spots that only surface after deployment incidents.

Method: Develops a formal framework for AI assistance in safety analysis, establishing why safety engineering resists benchmark-driven evaluation. Formalizes a five-dimensional competence framework (domain knowledge, standards expertise, operational experience, contextual understanding, judgment). Introduces the "competence shadow" concept and formalizes four canonical human-AI collaboration structures with closed-form performance bounds.

Result: Demonstrates that the competence shadow compounds multiplicatively to produce degradation far exceeding naive additive estimates. Shows that AI assistance in safety engineering is a collaboration design problem - the same tool can degrade or improve analysis quality depending entirely on how it's used.

Conclusion: Derives non-degradation conditions for shadow-resistant workflows and calls for a shift from tool qualification toward workflow qualification for trustworthy Physical AI systems.
...
### [274] [Probabilistic Abstract Interpretation on Neural Networks via Grids Approximation](https://arxiv.org/abs/2603.25266)
*Zhuofan Zhang,Herbert Wiklicky*

Main category: cs.AI

TL;DR: Applying probabilistic abstract interpretation theory to neural networks to analyze density distribution flow of all possible inputs when networks have infinite or uncountably many inputs.


<details>
  <summary>Details</summary>
Motivation: Neural networks often have uncountably many or infinitely many possible inputs, making it infeasible to test every single input. Probabilistic abstract interpretation provides a theoretical framework to analyze program properties without exhaustive testing, and this paper aims to apply this theory to neural networks for the same purpose.

Method: Apply probabilistic abstract interpretation theory to neural networks, discuss different abstract domains and corresponding Moore-Penrose pseudo-inverses, and implement abstract transformers within the framework.

Result: The paper demonstrates how the theoretical framework works in neural networks and presents experimental examples showing how this framework helps analyze real-world problems.

Conclusion: Probabilistic abstract interpretation can be successfully applied to neural networks to analyze density distribution flow of all possible inputs, providing a theoretical foundation for analyzing neural network behavior with infinite or uncountably many inputs through abstract domains, pseudo-inverses, and transformers.
...
### [275] [Distribution and Clusters Approximations as Abstract Domains in Probabilistic Abstract Interpretation to Neural Network Analysis](https://arxiv.org/abs/2603.25273)
*Zhuofan Zhang,Herbert Wiklicky*

Main category: cs.AI

TL;DR: Introduces two novel approximation methods (distribution and clusters) for probabilistic abstract interpretation of neural networks, showing theoretical operation with abstract transformers and simple examples.


<details>
  <summary>Details</summary>
Motivation: To enhance the probabilistic abstract interpretation framework for neural network analysis by providing more sophisticated approximation methods beyond the existing grids approximation.

Method: Introduces two new approximation methods: 1) distribution approximation and 2) clusters approximation, with corresponding abstract transformers to analyze density distribution flow of neural network inputs.

Result: Demonstrates how both methods work theoretically with abstract transformers, supported by illustrations of simple examples to validate the approaches.

Conclusion: The paper successfully introduces two novel approximation methods that expand the abstract domains available in probabilistic abstract interpretation framework for neural network analysis.
...
### [276] [A Gait Foundation Model Predicts Multi-System Health Phenotypes from 3D Skeletal Motion](https://arxiv.org/abs/2603.25283)
...
TL;DR: Gait foundation model using 3D skeletal motion predicts multiple health phenotypes, outperforming engineered features and providing independent predictive value across body systems.


<details>
  <summary>Details</summary>
Motivation: Gait is recognized as a vital sign but is currently treated as a symptom of specific pathologies rather than a systemic biomarker. The researchers aim to establish gait as an independent multi-system biosignal that can be translated to consumer-grade video for scalable health monitoring.

Method: Developed a gait foundation model using 3D skeletal motion data from 3,414 deeply phenotyped adults recorded via depth camera during five motor tasks. Used learned embeddings to predict various health phenotypes and compared performance against engineered features.

Result: Learned embeddings outperformed engineered features in predicting age (r=0.69), BMI (r=0.90), and visceral adipose tissue area (r=0.82). Gait embeddings significantly predicted 1,980 of 3,210 phenotypic targets and provided independent predictive gains across body systems after adjusting for age, BMI, VAT, and height. Anatomical ablation showed legs dominate metabolic/frailty predictions while torso encodes sleep/lifestyle phenotypes.

Conclusion: Gait functions as an independent multi-system biosignal, motivating translation to consumer-grade video for integration as a scalable, passive vital sign in healthcare monitoring.
...
### [277] [SliderQuant: Accurate Post-Training Quantization for LLMs](https://arxiv.org/abs/2603.25284)
*Shigeng Wang,Chao Li,Yangyuxuan Kang,Jiawei Fan,Zhonghong Ou,Anbang Yao*

Main category: cs.AI

TL;DR: SliderQuant: A new PTQ framework for LLMs using adaptive sliding quantization with inter-layer and intra-layer components to address varying quantization sensitivity across different layers.


<details>
  <summary>Details</summary>
Motivation: Current PTQ methods treat all LLM layers equally during quantization, but empirical studies show shallow/deep layers are more sensitive than intermediate layers, with first/last layers being most sensitive. This suggests the need for layer-specific quantization strategies rather than uniform approaches.

Method: Proposes SliderQuant framework with two components: (1) Inter-layer sliding quantization with three types of sliding window designs for shallow, intermediate, and deep layers; (2) Intra-layer sliding quantization using incremental strategy to quantize each window. Uses few learnable parameters for adaptive sliding quantization.

Result: Outperforms existing PTQ methods (including latest rotation-based methods) on basic language generation, zero-shot commonsense reasoning, and challenging math/code tasks across various LLMs (Llama/Llama2/Llama3/Qwen2.5 families, DeepSeek-R1 distilled models, and large MoE models) for both weight-only and weight-activation quantization.

Conclusion: SliderQuant effectively addresses varying quantization sensitivity across LLM layers through adaptive sliding quantization, demonstrating superior performance over existing PTQ methods across diverse tasks and model architectures.
...
### [278] [DAGverse: Building Document-Grounded Semantic DAGs from Scientific Papers](https://arxiv.org/abs/2603.25293)
*Shu Wan,Saketh Vishnubhatla,Iskander Kushbay,Tom Heffernan,Aaron Belikoff,Raha Moraffah,Huan Liu*

Main category: cs.AI

TL;DR: DAGverse is a framework for constructing document-grounded semantic DAGs from scientific papers, using DAG figures as supervision to create a dataset of expert-validated semantic DAGs with supporting evidence.


<details>
  <summary>Details</summary>
Motivation: Real-world DAG datasets are scarce because constructing them requires expert interpretation of domain documents. The challenge is that documents may have multiple plausible abstractions, intended structures are often implicit, and supporting evidence is scattered across different document elements.

Method: Leverage scientific papers with explicit DAG figures as natural supervision. DAGverse-Pipeline is a semi-automatic system with figure classification, graph reconstruction, semantic grounding, and validation components. Tested specifically for causal DAGs.

Result: Created DAGverse-1 dataset with 108 expert-validated semantic DAGs containing graph-level, node-level, and edge-level evidence. DAGverse-Pipeline outperforms existing Vision-Language Models on DAG classification and annotation tasks.

Conclusion: DAGverse provides a foundation for document-grounded DAG benchmarks and opens new directions for studying structured reasoning grounded in real-world evidence from scientific documents.
...
### [279] [Evaluating Language Models for Harmful Manipulation](https://arxiv.org/abs/2603.25326)
...
TL;DR: Researchers developed a framework to evaluate harmful AI manipulation through human-AI interaction studies across multiple domains and geographies, finding that AI models can produce manipulative behaviors and induce belief/behavior changes, with significant context-dependence.


<details>
  <summary>Details</summary>
Motivation: Current approaches to evaluating AI-driven harmful manipulation are limited, creating a need for better evaluation frameworks that can assess manipulative behaviors in real-world contexts.

Method: Developed a framework for evaluating harmful AI manipulation via context-specific human-AI interaction studies. Tested with 10,101 participants across three domains (public policy, finance, health) and three locales (US, UK, India). Made testing protocols and materials publicly available.

Result: AI models can produce manipulative behaviors when prompted and induce belief/behavior changes. Context matters significantly: manipulation differs between domains and geographies, with results from one region not generalizing to others. Propensity (frequency of manipulative behaviors) is not consistently predictive of efficacy (likelihood of manipulative success).

Conclusion: The framework enables evaluation of harmful AI manipulation in context-specific settings. Context matters significantly across domains and geographies, and propensity and efficacy should be studied separately. Open challenges remain in evaluating harmful AI manipulation.
...
### [280] [Macroscopic Characteristics of Mixed Traffic Flow with Deep Reinforcement Learning Based Automated and Human-Driven Vehicles](https://arxiv.org/abs/2603.25328)
*Pankaj Kumar,Pranamesh Chakraborty,Subrahmanya Swamy Peruru*

Main category: cs.AI

TL;DR: DRL-based AV control improves traffic capacity by 7.52% and fuel efficiency by up to 28.98% compared to traditional IDM in mixed traffic scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional car-following models like IDM struggle to generalize across diverse traffic scenarios and don't account for fuel efficiency, while DRL's macroscopic traffic flow characteristics remain underexplored despite strong microscopic performance.

Method: Implemented TD3 algorithm for AV control trained on NGSIM highway dataset, evaluated traffic performance using Fundamental Diagram under varying driver heterogeneity, time-gap penetration levels, and RL vehicle shares.

Result: Traffic performance sensitive to safe time gap distribution and RL vehicle proportion. Transition to fully RL-controlled traffic increases road capacity by 7.52%. RL-based AVs improve fuel efficiency by 28.98% at higher speeds (>50 km/h) and 1.86% at lower speeds (<50 km/h) compared to IDM.

Conclusion: DRL framework enhances both traffic capacity and fuel efficiency without compromising safety, demonstrating significant advantages over traditional models in mixed traffic scenarios.
...
### [281] [Agentic Trust Coordination for Federated Learning through Adaptive Thresholding and Autonomous Decision Making in Sustainable and Resilient Industrial Networks](https://arxiv.org/abs/2603.25334)
*Paul Shepherd,Tasos Dagiuklas,Bugra Alkan,Jonathan Rodriguez*

Main category: cs.AI

TL;DR: Lightweight agentic trust coordination framework for FL in industrial networks that observes trust signals, interprets their evolution, and applies targeted adjustments to maintain stability without client-side modifications.


<details>
  <summary>Details</summary>
Motivation: Federated learning in industrial networks faces reliability issues from inconsistent client behavior, noisy sensing, and adversarial updates. Existing trust mechanisms are mostly statistical/heuristic with fixed parameters that struggle with changing conditions.

Method: Proposes an Agentic Trust Control Layer operating as a server-side control loop that: 1) observes trust-related and system-level signals, 2) interprets their evolution over time, and 3) applies targeted trust adjustments when instability is detected. Separates observation, reasoning, and action explicitly.

Result: Enables context-aware intervention decisions rather than fixed or purely reactive parameter updates. Supports stable FL operation without modifying client-side training or increasing communication overhead.

Conclusion: The agentic trust coordination approach extends prior adaptive trust mechanisms by providing a lightweight framework that adapts to changing conditions while maintaining FL reliability in resource-constrained industrial networks.
...
### [282] [4OPS: Structural Difficulty Modeling in Integer Arithmetic Puzzles](https://arxiv.org/abs/2603.25356)
*Yunus E. Zeytuncu*

Main category: cs.AI

TL;DR: Difficulty in arithmetic puzzles is fully determined by interpretable structural attributes from exact solutions, with number of input values used as minimal sufficient statistic.


<details>
  <summary>Details</summary>
Motivation: To understand structural determinants of difficulty in arithmetic puzzles for adaptive learning systems, bridging symbolic reasoning with data-driven modeling.

Method: Developed exact dynamic-programming solver to enumerate reachable targets and extract minimal-operation witnesses, constructed dataset of 3.4M instances, analyzed difficulty via minimum operations required.

Result: Baseline ML models partially predict solvability but fail to distinguish easy instances; difficulty is fully determined by interpretable structural attributes, with number of input values used as minimal sufficient statistic.

Conclusion: Provides transparent, computationally grounded account of puzzle difficulty that supports explainable difficulty estimation and principled task sequencing for adaptive arithmetic learning.
...
### [283] [Modernising Reinforcement Learning-Based Navigation for Embodied Semantic Scene Graph Generation](https://arxiv.org/abs/2603.25415)
*Roman Kueble,Marco Hueller,Mrunmai Phatak,Rainer Lienhart,Joerg Haehner*

Main category: cs.AI

TL;DR: This paper presents a modular navigation component for Embodied Semantic Scene Graph Generation, improving decision-making through modern optimization algorithms and refined action representations to balance SSG completeness with navigation efficiency.


<details>
  <summary>Details</summary>
Motivation: Semantic world models are crucial for embodied agents to reason about objects and spatial context, enabling objective-driven self-adaptation under uncertainty and resource constraints. The core challenge is acquiring observations that maximize model quality and downstream usefulness within limited action budgets, requiring exploration strategies that trade off information gain against navigation cost.

Method: The authors modernize decision-making by replacing the policy-optimization method and revisiting discrete action formulation. They study compact vs. finer-grained motion sets, compare single-head policies over atomic actions with factorised multi-head policies over action components, evaluate curriculum learning and optional depth-based collision supervision, and assess SSG completeness, execution safety, and navigation behavior.

Result: Replacing the optimization algorithm alone improves SSG completeness by 21% relative to baseline under identical reward shaping. Depth supervision mainly affects execution safety (collision-free motion) while completeness remains largely unchanged. Combining modern optimization with finer-grained, factorised action representation yields the strongest overall completeness-efficiency trade-off.

Conclusion: The modular navigation component successfully improves embodied semantic scene graph generation through modern optimization techniques and refined action representations, achieving better trade-offs between scene graph completeness and navigation efficiency while maintaining execution safety.
...
### [284] [Does Structured Intent Representation Generalize? A Cross-Language, Cross-Model Empirical Study of 5W3H Prompting](https://arxiv.org/abs/2603.25379)
*Peng Gang*

Main category: cs.AI

TL;DR: PPS (5W3H structured prompts) improves intent alignment across languages and models, with AI-assisted expansion performing as well as manual crafting while requiring only single-sentence user input.


<details>
  <summary>Details</summary>
Motivation: To test if structured intent representation (PPS/5W3H framework) generalizes beyond Chinese to other languages (English, Japanese), and to evaluate AI-assisted prompt expansion vs. manual crafting for accessibility.

Method: Studied 2,160 model outputs across 3 languages (English, Japanese, Chinese) × 4 conditions (including AI-expanded 5W3H) × 3 LLMs × 60 tasks, analyzing goal alignment and cross-model consistency.

Result: AI-expanded 5W3H prompts performed equally to manually crafted ones across all languages. Structured prompts reduced/reshaped cross-model variance, and unstructured prompts showed dual-inflation bias (artificially high scores + low variance).
...
### [285] [Beyond Content Safety: Real-Time Monitoring for Reasoning Vulnerabilities in Large Language Models](https://arxiv.org/abs/2603.25412)
*Xunguang Wang,Yuguang Zhou,Qingyue Wang,Zongjie Li,Ruixuan Huang,Zhenlan Ji,Pingchuan Ma,Shuai Wang*

Main category: cs.AI

TL;DR: Paper introduces reasoning safety as a new security dimension for LLMs, defines a 9-category taxonomy of unsafe reasoning behaviors, and proposes a Reasoning Safety Monitor that detects unsafe reasoning steps in real time with high accuracy.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety research focuses only on content safety (harmful outputs) while ignoring the safety of the reasoning process itself. Reasoning chains are treated as opaque artifacts, but unsafe reasoning behaviors pose security risks that need to be addressed.

Method: 1) Formal definition of reasoning safety and 9-category taxonomy of unsafe reasoning behaviors. 2) Large-scale prevalence study of 4111 reasoning chains from benchmarks and adversarial attacks. 3) Reasoning Safety Monitor: external LLM component that inspects reasoning steps in real time using taxonomy-embedded prompts and dispatches interrupt signals upon detecting unsafe behavior.

Result: All nine error types occur in practice with mechanistically interpretable signatures from different attacks. The Reasoning Safety Monitor achieves 84.88% step-level localization accuracy and 85.37% error-type classification accuracy on a 450-chain benchmark, significantly outperforming hallucination detectors and process reward models.

Conclusion: Reasoning-level monitoring is both necessary and practically achievable. Reasoning safety should be established as a foundational concern for secure deployment of large reasoning models, complementing existing content safety approaches.
...
### [286] [Cross-Model Disagreement as a Label-Free Correctness Signal](https://arxiv.org/abs/2603.25450)
*Matt Gorbett,Suman Jana*

Main category: cs.AI

TL;DR: Cross-model disagreement (CMP/CME) detects LLM errors without ground truth by measuring how surprised a second model is at the first model's answer, outperforming within-model uncertainty methods.


<details>
  <summary>Details</summary>
Motivation: Current methods for detecting language model errors rely on the model's own uncertainty signals, but these fail on confident errors where models are wrong but certain. There's a need for a training-free, label-free approach that works in production systems.

Method: Introduces cross-model disagreement using two metrics: Cross-Model Perplexity (CMP) measures a verifying model's surprise at the generating model's answer tokens, and Cross-Model Entropy (CME) measures the verifying model's uncertainty at those positions. Both require only a single forward pass from the verifier model.

Result: CMP and CME outperform within-model uncertainty baselines across reasoning, retrieval, and mathematical benchmarks (MMLU, TriviaQA, GSM8K). On MMLU, CMP achieves mean AUROC of 0.75 vs. 0.59 for within-model entropy baseline.

Conclusion: Cross-model disagreement provides a practical, training-free approach for label-free correctness estimation with applications in deployment monitoring, model routing, selective prediction, data filtering, and scalable oversight of production LLM systems.
...
### [287] [Retraining as Approximate Bayesian Inference](https://arxiv.org/abs/2603.25480)
*Harrison Katz*

Main category: cs.AI

TL;DR: Retraining models is framed as approximate Bayesian inference under computational constraints, where the gap between updated beliefs and frozen models is "learning debt," and retraining decisions become cost minimization problems with evidence-based triggers.


<details>
  <summary>Details</summary>
Motivation: Traditional model retraining is treated as routine maintenance, but this paper argues for a more principled approach that treats retraining as approximate Bayesian inference under computational constraints, addressing the gap between continuously updated beliefs and frozen deployed models.

Method: Proposes a decision-theoretic framework for retraining policies that treats retraining as cost minimization problem, where the gap between updated belief state and frozen model is defined as "learning debt," with retraining thresholds derived from loss functions.

Result: Develops evidence-based retraining triggers that replace calendar schedules, making governance auditable, with retraining decisions emerging as optimal solutions to cost minimization problems based on loss functions.

Conclusion: Retraining should be understood through Bayesian and decision-theoretic lenses as approximate inference under constraints, leading to principled, evidence-based policies rather than arbitrary maintenance schedules.
...
### [288] [EcoThink: A Green Adaptive Inference Framework for Sustainable and Accessible Agents](https://arxiv.org/abs/2603.25498)
*Linxiao Li,Zhixiang Lu*

Main category: cs.AI

TL;DR: EcoThink is an energy-aware adaptive inference framework that reduces LLM energy consumption by 40.4% on average by dynamically routing queries based on complexity, skipping unnecessary reasoning for simple queries while reserving deep computation for complex tasks.


<details>
  <summary>Details</summary>
Motivation: The environmental footprint of LLMs is growing as web interactions shift from static retrieval to generative AI. Current approaches like Chain-of-Thought (CoT) cause "LLM overthinking" - applying intensive computation to billions of daily queries unnecessarily, increasing carbon emissions and creating barriers to equitable AI access in resource-constrained regions, undermining UN Sustainable Development Goals 13 (Climate Action) and 10 (Reduced Inequalities).

Method: EcoThink uses a lightweight, distillation-based router to dynamically assess query complexity. It skips unnecessary reasoning for factoid retrieval tasks while reserving deep computation only for complex logic problems, creating an adaptive inference framework that optimizes energy usage based on query requirements.

Result: Extensive evaluations across 9 diverse benchmarks show EcoThink reduces inference energy by 40.4% on average, with up to 81.9% reduction for web knowledge retrieval tasks, without statistically significant performance loss.

Conclusion: EcoThink offers a scalable solution for sustainable, inclusive, and energy-efficient generative AI by mitigating algorithmic waste, reconciling high-performance AI with environmental responsibility, and supporting equitable access to AI technology.
...
### [289] [Voxtral TTS](https://arxiv.org/abs/2603.25551)
...
TL;DR: Voxtral TTS is an expressive multilingual text-to-speech model that generates natural speech from just 3 seconds of reference audio, achieving 68.4% win rate over ElevenLabs Flash v2.5 in human evaluations.


<details>
  <summary>Details</summary>
Motivation: To create a high-quality multilingual TTS system that can generate natural and expressive speech with minimal reference audio (only 3 seconds), enabling effective voice cloning across multiple languages.
...
Conclusion: Voxtral TTS successfully creates high-quality multilingual speech synthesis with minimal reference audio, outperforming a leading commercial competitor, and the model weights are released under CC BY-NC license.
...
### [290] [Is Mathematical Problem-Solving Expertise in Large Language Models Associated with Assessment Performance?](https://arxiv.org/abs/2603.25633)
*Liang Zhang,Yu Fu,Xinyi Jin*

Main category: cs.AI

TL;DR: LLMs with stronger math problem-solving ability perform better at step-level assessment of mathematical reasoning errors, but assessment remains more challenging than direct problem solving.


<details>
  <summary>Details</summary>
Motivation: To understand whether stronger math problem-solving ability in LLMs correlates with better performance in assessing learners' step-by-step mathematical reasoning, particularly for formative assessment in math education.

Method: Evaluated GPT-4 and GPT-5 on two tasks using PROCESSBENCH benchmark (GSM8K and MATH subsets): 1) solving original math problems, and 2) assessing benchmark-provided solutions by predicting the earliest erroneous step in reasoning.

Result: Consistent pattern: assessment accuracy significantly higher on problems the same model solved correctly vs. incorrectly. Assessment more difficult than direct problem solving, especially for error-present solutions. Math problem-solving expertise supports assessment but additional capabilities needed for reliable diagnosis.

Conclusion: Math problem-solving ability enhances step-level assessment performance in LLMs, but effective formative assessment requires additional capabilities like step tracking and precise error localization. Implications for designing AI-supported Adaptive Instructional Systems for math education.
...
### [291] [Agent Factories for High Level Synthesis: How Far Can General-Purpose Coding Agents Go in Hardware Optimization?](https://arxiv.org/abs/2603.25719)
*Abhishek Bhandwaldar,Mihir Choudhury,Ruchir Puri,Akash Srivastava*

Main category: cs.AI

TL;DR: General-purpose coding agents without hardware-specific training can optimize hardware designs via a two-stage agent factory pipeline, achieving mean 8.27× speedup over baseline with scaling from 1 to 10 agents.


<details>
  <summary>Details</summary>
Motivation: To explore how far general-purpose coding agents can optimize hardware designs from high-level algorithmic specifications without hardware-specific training, and to establish agent scaling as a practical axis for HLS optimization.

Method: Two-stage agent factory pipeline: Stage 1 decomposes design into sub-kernels, optimizes each independently using pragma/code transformations, formulates ILP to assemble promising configurations under area constraints. Stage 2 launches N expert agents over top ILP solutions to explore cross-function optimizations like pragma recombination, loop fusion, and memory restructuring.

Result: Scaling from 1 to 10 agents yields mean 8.27× speedup over baseline on 12 kernels from HLS-Eval and Rodinia-HLS. Larger gains on harder benchmarks: streamcluster exceeds 20× and kmeans reaches ~10×. Agents rediscover known hardware optimization patterns without domain-specific training, and best designs often don't originate from top-ranked ILP candidates.

Conclusion: Agent scaling is a practical and effective axis for HLS optimization, with general-purpose coding agents capable of discovering significant hardware optimizations without domain-specific training, and global optimization exposing improvements missed by sub-kernel search.
...
### [292] [R-C2: Cycle-Consistent Reinforcement Learning Improves Multimodal Reasoning](https://arxiv.org/abs/2603.25720)
*Zirui Zhang,Haoyu Dong,Kexin Pei,Chengzhi Mao*

Main category: cs.AI

TL;DR: RC2 uses reinforcement learning with cross-modal cycle consistency to resolve contradictions between visual and textual predictions, improving reasoning accuracy by up to 7.6 points without labeled data.


<details>
  <summary>Details</summary>
Motivation: Current multimodal models often produce contradictory predictions across different sensory modalities (visual vs. textual), violating the principle of consistency that robust perception and reasoning require. Standard voting mechanisms can amplify systematic biases rather than addressing the root cause.

Method: RC2 is a reinforcement learning framework that enforces cross-modal cycle consistency. It requires models to perform backward inference, switch modalities, and reliably reconstruct answers through forward inference, creating a dense, label-free reward signal that encourages autonomous alignment of internal representations.

Result: The approach improves reasoning accuracy by up to 7.6 points, mitigates modality-specific errors, and demonstrates that structural consistency enforcement can enhance reasoning beyond simply scaling data.

Conclusion: Advanced reasoning emerges not only from scaling data but also from enforcing structurally consistent understanding across modalities. Cross-modal inconsistency provides a valuable learning signal rather than a problem to be masked.
...
### [293] [Back to Basics: Revisiting ASR in the Age of Voice Agents](https://arxiv.org/abs/2603.25727)
*Geeyang Tay,Wentao Ma,Jaewon Lee,Yuzhi Tang,Daniel Lee,Weisu Yin,Dongming Shen,Silin Meng,Yi Zhu,Mu Li,Alex Smola*

Main category: cs.AI

TL;DR: WildASR is a multilingual diagnostic benchmark that evaluates ASR robustness across environmental degradation, demographic shift, and linguistic diversity using real human speech, revealing severe performance degradation and hallucination risks.


<details>
  <summary>Details</summary>
Motivation: Current ASR systems achieve high accuracy on curated benchmarks but fail in real-world voice agents under conditions not covered by existing evaluations. Without diagnostic tools to isolate specific failure factors, practitioners cannot anticipate which conditions in which languages will cause performance degradation.
...
Result: Found severe and uneven performance degradation across ASR systems, with model robustness not transferring across languages or conditions. Critically, models often hallucinate plausible but unspoken content under partial or degraded inputs, creating concrete safety risks for downstream agent behavior.
...
### [294] [Training the Knowledge Base through Evidence Distillation and Write-Back Enrichment](https://arxiv.org/abs/2603.25737)
*Yuxing Lu,Xukai Zhao,Wei Wu,Jinzhuo Wang*

Main category: cs.AI

TL;DR: WriteBack-RAG treats knowledge base as trainable component, distills relevant document fragments into compact knowledge units that improve RAG performance across multiple methods and benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional RAG systems have static knowledge bases that are assembled once and never revised, even though relevant facts are often fragmented across documents and buried in irrelevant content. This static approach limits retrieval effectiveness.

Method: WriteBack-RAG uses labeled examples to identify where retrieval succeeds, isolates relevant documents, and distills them into compact knowledge units that are indexed alongside the original corpus. The method modifies only the corpus as an offline preprocessing step that can be combined with any RAG pipeline.

Result: Across four RAG methods, six benchmarks, and two LLM backbones, WriteBack-RAG improves every evaluated setting with average gains of +2.14%. Cross-method transfer experiments show that distilled knowledge benefits RAG pipelines other than the one used to produce it, confirming improvement resides in the corpus itself.

Conclusion: Treating the knowledge base as a trainable component through WriteBack-RAG's distillation approach significantly enhances RAG performance across diverse settings, demonstrating that corpus-level improvements transfer across different RAG methods.
...
### [295] [Saranga: MilliWatt Ultrasound for Navigation in Visually Degraded Environments on Palm-Sized Aerial Robots](https://arxiv.org/abs/2603.24699)
*Manoj Velmurugan,Phillip Brush,Colin Balfour,Richard J. Przybyla,Nitin J. Sanket*

Main category: cs.RO

TL;DR: Saranga: A low-power ultrasound-based perception system for tiny aerial robots enabling navigation in visually degraded conditions like fog, darkness, and snow.


<details>
  <summary>Details</summary>
Motivation: Tiny aerial robots have limited payload capacity, restricting sensing capabilities for GPS-denied environments. Traditional sensors (cameras, LIDAR) fail in visually degraded conditions, while RADAR consumes too much power. Need for alternative sensing that works in low visibility while being power-efficient.

Method: Dual sonar array system with two key innovations: 1) Physical noise reduction to block propeller-induced ultrasound noise, 2) Deep learning denoising using neural networks trained on synthetic data and limited real noise data to find signal patterns in low SNR conditions (-4.9 dB).

Result: Enabled palm-sized aerial robot to navigate in visually degraded conditions (dense fog, darkness, snow) in cluttered environments with thin/transparent obstacles using only on-board sensing and computation. Demonstrated extensive real-world efficacy.

Conclusion: Saranga provides a practical, low-power ultrasound-based solution for obstacle localization and navigation in challenging visual conditions, overcoming limitations of traditional sensors for tiny aerial robots.
...
### [296] [FODMP: Fast One-Step Diffusion of Movement Primitives Generation for Time-Dependent Robot Actions](https://arxiv.org/abs/2603.24806)
*Xirui Shi,Arya Ebrahimi,Yi Hu,Jun Jin*

Main category: cs.RO

TL;DR: FODMP is a fast one-step diffusion framework for movement primitives that enables real-time robot control with temporal motion structure, running 10x faster than previous methods while maintaining or improving success rates.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models for robot learning face a trade-off: action-chunking diffusion policies are fast but only predict short motion segments, while Movement Primitive Diffusion (MPD) captures temporal structure but has prohibitively high inference latency unsuitable for real-time control.

Method: FODMP distills diffusion models into Probabilistic Dynamic Movement Primitives (ProDMPs) trajectory parameter space and generates motion using a single-step decoder. It uses consistency distillation to eliminate the multi-step diffusion process bottleneck while retaining temporal structure.

Result: FODMP runs up to 10 times faster than MPD and 7 times faster than action-chunking diffusion policies on MetaWorld and ManiSkill benchmarks, while matching or exceeding their success rates. It enables real-time interception of fast-flying balls where previous methods respond too slowly.

Conclusion: FODMP successfully bridges the gap between temporal motion structure and real-time inference speed, enabling closed-loop vision-based control with time-dependent motion primitives while maintaining high performance on manipulation tasks.
...
### [297] [A Nonvolatile Switchable-polarity EPM Valve](https://arxiv.org/abs/2603.24811)
*Bingchao Wang,Jonah Mack,Francesco Giorgio-Serchi,Adam A. Stokes*

Main category: cs.RO

TL;DR: A bistable magnetic valve architecture (S-EPM) enables power-efficient, reconfigurable fluidic control without continuous power, supporting logic-embedded networks and digital fluidics.


<details>
  <summary>Details</summary>
Motivation: Current pneumatic/fluidic control systems have fundamental limitations: they require continuous power input, dense external control hardware, and fixed routing topologies. Existing valve arrays impose substantial thermal and architectural overhead due to continuous actuation and mechanically fixed routing.

Method: Introduces Switchable-polarity ElectroPermanent Magnet (S-EPM), a bistable magnetic architecture that deterministically reverses its external magnetic polarity through transient electrical excitation. This reconfigures internal flux pathways within a composite magnet assembly to establish two stable, opposing magnetic configurations without sustained power. Integrated into compact pinch-valves to control pneumatic and liquid media.

Result: Enables logic-embedded fluidic networks including decoders, hierarchical distribution modules, and a nonvolatile six-port routing array. These systems provide address-based routing and programmable compositional control with features like individual port isolation that are impossible with standard mechanically coupled rotary valves.

Conclusion: By embedding functionality in persistent magnetic states rather than continuous power or static plumbing, this work establishes a scalable foundation for digital fluidics and autonomous laboratory platforms, overcoming fundamental constraints of current fluidic control architectures.
...
### [298] [Characterization of Constraints in Flexible Unknown Environments](https://arxiv.org/abs/2603.24813)
*Samrat Bhattacharyya,Nabil Simaan*

Main category: cs.RO

TL;DR: Online path planning algorithm for safe autonomous manipulation of flexibly constrained objects in unknown environments using real-time constraint identification and stiffness characterization.


<details>
  <summary>Details</summary>
Motivation: Enable safe autonomous manipulation of flexibly constrained objects without prior knowledge of the system, particularly for applications like surgical organ retraction and manipulation where cooperative manipulation is needed.

Method: Real-time identification and characterization of flexible constraints using local force/position information, constraint exploration, stiffness eigenvector analysis, and an atlas of simple mechanical constraints (hinges, planar constraints) to identify global stiffness behavior and screw parameters.

Result: Successfully demonstrated real-time recognition of common mechanical constraints (flexible hinges) and identification of relevant screw parameters through simulation and experimental validation.

Conclusion: The approach enables simultaneous global constraint/stiffness exploration and safe manipulation of flexibly constrained objects, showing feasibility for applications like surgical manipulation and organ retraction.
...
### [299] [Integrated Multi-Drone Task Allocation, Sequencing, and Optimal Trajectory Generation in Obstacle-Rich 3D Environments](https://arxiv.org/abs/2603.24908)
*Yunes Alqudsi,Murat Makaraci*

Main category: cs.RO

TL;DR: IMD-TAPP is an end-to-end framework that jointly solves multi-drone task allocation and path planning in 3D cluttered environments, integrating discrete mission planning with continuous trajectory generation while ensuring collision avoidance and dynamic feasibility.


<details>
  <summary>Details</summary>
Motivation: Coordinating aerial robot teams in complex 3D environments requires integrated solutions that combine discrete mission planning (which robot serves which goals in what order) with continuous trajectory synthesis that enforces collision avoidance and dynamic feasibility. Existing approaches often treat these aspects separately, leading to suboptimal or unsafe solutions.

Method: 1) Discretize workspace into 3D navigation graph and compute obstacle-aware travel costs via graph-search pathfinding. 2) Use Injected Particle Swarm Optimization (IPSO) guided by multiple linear assignment to explore coupled assignment/ordering alternatives and minimize mission makespan. 3) Transform waypoint tours into time-parameterized minimum-snap trajectories through generation-and-optimization with iterative validation of obstacle clearance and inter-robot separation, triggering re-planning when safety margins are violated.

Result: Extensive MATLAB simulations in cluttered 3D scenarios show IMD-TAPP consistently produces dynamically feasible, collision-free trajectories with competitive completion times. In a representative case study with two drones serving multiple goals, the approach achieves a minimum mission time of 136 seconds while maintaining all required safety constraints throughout execution.

Conclusion: IMD-TAPP provides an effective end-to-end solution for multi-drone coordination in complex 3D environments, successfully integrating task allocation, tour sequencing, and safe trajectory generation while ensuring both efficiency and safety through its iterative validation and re-planning mechanisms.
...
### [300] [CROSS: A Mixture-of-Experts Reinforcement Learning Framework for Generalizable Large-Scale Traffic Signal Control](https://arxiv.org/abs/2603.24930)
*Xibei Chen,Yifeng Zhang,Yuxiang Xiao,Mingfeng Fan,Maonan Wang,Guillaume Sartoretti*

Main category: cs.RO

TL;DR: CROSS is a Mixture-of-Experts RL framework for adaptive traffic signal control that uses predictive clustering to identify traffic patterns and scenario-adaptive experts to improve generalization across diverse urban intersections.


<details>
  <summary>Details</summary>
Motivation: Existing RL-based traffic signal control methods use a single shared policy that struggles to capture diverse traffic dynamics and generalize to unseen environments due to varying intersection topologies and complex traffic patterns.

Method: Two key modules: 1) Predictive Contrastive Clustering (PCC) that forecasts short-term state transitions to identify latent traffic patterns using clustering and contrastive learning, and 2) Scenario-Adaptive MoE module that augments a shared policy with multiple experts for adaptive specialization.

Result: CROSS achieves superior performance and generalization compared to state-of-the-art baselines in SUMO simulations on both synthetic and real-world traffic datasets, through improved representation of diverse traffic scenarios.

Conclusion: The proposed MoE-based decentralized RL framework with predictive clustering enables effective and generalizable large-scale adaptive traffic signal control by better capturing diverse traffic dynamics and adapting to unseen environments.
...
### [301] [COIN: Collaborative Interaction-Aware Multi-Agent Reinforcement Learning for Self-Driving Systems](https://arxiv.org/abs/2603.24931)
*Yifeng Zhang,Jieming Chen,Tingguang Zhou,Tanishq Duhan,Jianghong Dong,Yuhong Cao,Guillaume Sartoretti*

Main category: cs.RO

TL;DR: COIN is a novel collaborative interaction-aware MARL framework for multi-agent self-driving systems that optimizes both individual navigation and global collaboration objectives using a counterfactual twin delayed deep deterministic policy gradient algorithm.


<details>
  <summary>Details</summary>
Motivation: Current multi-agent self-driving systems struggle with achieving efficient and safe collaboration in dense scenarios with complex agent interactions, despite MARL being a promising approach for end-to-end coordination.

Method: Proposes COIN framework with CIG-TD3 algorithm in CTDE manner, featuring dual-level interaction-aware centralized critic architecture that captures both local pairwise interactions and global system-level dependencies for better credit assignment.

Result: COIN consistently outperforms other advanced baseline methods in both safety and efficiency across various system sizes in dense urban traffic simulations, with validation through real-world robot demonstrations.

Conclusion: The COIN framework demonstrates superior performance in complex and dynamic multi-agent self-driving scenarios by effectively balancing individual and global objectives through enhanced interaction modeling and credit assignment.
...
### [302] [SABER: A Stealthy Agentic Black-Box Attack Framework for Vision-Language-Action Models](https://arxiv.org/abs/2603.24935)
*Xiyang Wu,Guangyao Shi,Qingzi Wang,Zongxia Li,Amrit Singh Bedi,Dinesh Manocha*

Main category: cs.RO

TL;DR: SABER is an agent-centric black-box attack method that generates minimal adversarial text edits to degrade robot behavior in vision-language-action models, showing small instruction perturbations can significantly reduce task success and increase execution inefficiencies.


<details>
  <summary>Details</summary>
Motivation: Vision-language-action models are vulnerable to textual perturbations that can alter robot behavior, but systematic robustness evaluation requires efficient black-box attackers that can generate minimal yet effective instruction edits across diverse VLA models.

Method: SABER uses a GRPO-trained ReAct attacker agent that generates small, plausible adversarial instruction edits using character-, token-, and prompt-level tools under bounded edit budgets to induce targeted behavioral degradation (task failure, long execution, constraint violations).
...
### [303] [Wireless bioelectronics for untethered biohybrid robots](https://arxiv.org/abs/2603.24959)
*Hiroyuki Tetsuka,Minoru Hirano*

Main category: cs.RO

TL;DR: This perspective article reviews wireless control strategies for biohybrid robots, covering electrical stimulation, optical stimulation, and neuromuscular integration, with future directions toward neural organoid-bioelectronics integration for autonomous systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of delivering stimulation and control signals to biohybrid robots without relying on tethered wiring or bulky hardware immersed in cell-culture media, enabling more practical and advanced biohybrid robotic systems.

Method: The article summarizes three main control strategies: 1) Wireless bioelectronics using radio-frequency magnetic fields for electrical stimulation, 2) Wireless optoelectronics with embedded light emitters for optical stimulation of optogenetically modified muscles, and 3) Neuromuscular junction integration enabling selective control through wireless frequency- and time-division multiplexing.

Result: The article provides a comprehensive review of recent advances in wireless control strategies for biohybrid robots, highlighting progress in remote stimulation techniques and biological signal transduction approaches.

Conclusion: The conclusion points toward future integration of neural organoid-bioelectronics to develop autonomous, closed-loop biohybrid robots, emphasizing the need for co-integration of biological and electronic components for advanced biohybrid systems.
...
### [304] [Learning Rollout from Sampling:An R1-Style Tokenized Traffic Simulation Model](https://arxiv.org/abs/2603.24989)
*Ziyan Wang,Peng Chen,Ding Li,Chiwei Li,Qichao Zhang,Zhongpu Xia,Guizhen Yu*

Main category: cs.RO

TL;DR: R1Sim: A reinforcement learning-based traffic simulation policy that uses entropy-guided adaptive sampling and group relative policy optimization to generate diverse, realistic, and safe multi-agent behaviors from human driving demonstrations.


<details>
  <summary>Details</summary>
Motivation: Current next-token prediction methods for traffic simulation limit active exploration of potentially valuable motion tokens, especially in suboptimal regions. The authors aim to enable better exploration by leveraging motion token uncertainty patterns through reinforcement learning.

Method: Proposes R1Sim with: 1) Entropy-guided adaptive sampling mechanism focusing on high-uncertainty, high-potential motion tokens; 2) Group Relative Policy Optimization (GRPO) for motion behavior optimization; 3) Safety-aware reward design to guide learning.

Result: Achieves competitive performance on Waymo Sim Agent benchmark compared to state-of-the-art methods, generating realistic, safe, and diverse multi-agent behaviors through balanced exploration-exploitation trade-off.

Conclusion: R1Sim represents a successful initial attempt at reinforcement learning based on motion token entropy patterns, demonstrating that entropy-guided exploration and group-wise comparative estimation can improve traffic simulation quality and diversity.
...
### [305] [$π$, But Make It Fly: Physics-Guided Transfer of VLA Models to Aerial Manipulation](https://arxiv.org/abs/2603.25038)
...
TL;DR: AirVLA transfers manipulation-pretrained Vision-Language-Action models to aerial platforms using synthetic data generation and payload-aware guidance to bridge the dynamics gap between fixed-base arms and aerial systems.


<details>
  <summary>Details</summary>
Motivation: While VLA models like π₀ show great generalization for fixed-base manipulators, transferring them to aerial platforms is challenging due to fundamental dynamics mismatch between quasi-static arms and underactuated, dynamic flight systems.

Method: 1) Use Gaussian Splatting pipeline to synthesize navigation training data to overcome data scarcity; 2) Introduce Payload-Aware Guidance that injects payload constraints into policy's flow-matching sampling process without retraining foundation model.

Result: Synthetic data enables 100% success in navigation (vs 81% with teleoperation data alone). Payload-Aware Guidance increases pick-and-place success from 23% to 50%. Achieves 62% success on long-horizon compositional tasks across 460 real-world experiments.

Conclusion: Pre-trained manipulation VLAs can transfer to aerial manipulation and navigation with appropriate data augmentation and physics-informed guidance, enabling composition of these tasks despite the dynamics gap.
...
### [306] [ThermoAct:Thermal-Aware Vision-Language-Action Models for Robotic Perception and Decision-Making](https://arxiv.org/abs/2603.25044)
*Young-Chae Son,Dae-Kwan Ko,Yoon-Ji Choi,Soo-Chul Lim*

Main category: cs.RO

TL;DR: A novel Vision-Language-Action framework that integrates thermal data with visual information for safer and more intelligent robot task execution in human-robot collaboration environments.


<details>
  <summary>Details</summary>
Motivation: Current human-robot collaboration research overlooks thermal data integration, which is crucial for enhancing robot safety and operational efficiency. Existing vision-based systems lack the ability to perceive physical properties and proactively ensure environmental safety.

Method: Proposes a Vision-Language-Action (VLA) framework that incorporates thermal information. Uses a Vision-Language Model (VLM) as a high-level planner to interpret complex natural language commands and decompose them into simpler sub-tasks, enabling efficient data collection and robust reasoning.

Result: Experimental results from real-world task scenarios validate the feasibility of the proposed framework. The system demonstrates potential to enhance task success rates and safety compared to existing vision-based systems.

Conclusion: Integrating thermal information with visual data in a VLA framework enables robots to better perceive physical properties and proactively ensure environmental safety, representing a significant advancement over conventional vision-only approaches in human-robot collaboration.
...
### [307] [CTS-PLL: A Robust and Anytime Framework for Collaborative Task Sequencing and Multi-Agent Path Finding](https://arxiv.org/abs/2603.25121)
*Junkai Jiang,Yitao Xu,Ruochen Li,Shaobing Xu,Jianqiang Wang*

Main category: cs.RO

TL;DR: CTS-PLL is a hierarchical framework for Collaborative Task Sequencing and Multi-Agent Path Finding that improves robustness in dense environments and enables continuous solution refinement through lock agent detection/release and Large Neighborhood Search.


<details>
  <summary>Details</summary>
Motivation: The CTS-MAPF problem requires agents to complete task sequences while avoiding collisions, presenting significant combinatorial complexity challenges that need robust solutions for dense environments and continuous quality improvement.

Method: CTS-PLL extends configuration-based CTS-MAPF planning with two key enhancements: 1) a lock agents detection and release mechanism using complete planning for local re-planning, and 2) an anytime refinement procedure based on Large Neighborhood Search (LNS).
...
Conclusion: CTS-PLL provides a robust and continuously improvable solution for CTS-MAPF problems, effectively handling dense environments through hierarchical planning with lock agent management and LNS-based refinement.
...
### [308] [Dissimilarity-Based Persistent Coverage Control of Multi-Robot Systems for Improving Solar Irradiance Prediction Accuracy in Solar Thermal Power Plants](https://arxiv.org/abs/2603.25139)
...
<details>
  <summary>Details</summary>
Motivation: Existing kriging-based methods for solar irradiance forecasting lack dynamic sampling strategies to position mobile sensors in real-time, which is crucial for accurate predictions with minimal sensors.

Method: Introduces a dissimilarity map derived from kriging model and proposes a persistent coverage control algorithm that guides mobile agents to regions needing additional observations for better prediction performance.

Result: Experimental validation with mobile robots shows the proposed approach achieves more accurate predictions than baseline methods across various emulated irradiance fields.

Conclusion: The proposed dynamic sensor positioning strategy using dissimilarity maps and coverage control effectively improves solar irradiance forecasting accuracy with limited mobile sensors.
...
### [309] [A Minimum-Energy Control Approach for Redundant Mobile Manipulators in Physical Human-Robot Interaction Applications](https://arxiv.org/abs/2603.25259)
*Davide Tebaldi,Niccolò Paradisi,Fabio Pini,Luigi Biagiotti*

Main category: cs.RO

TL;DR: Proposes a control approach for mobile manipulators in human-robot interaction that minimizes overall kinetic energy, tested on peg-in-hole tasks with improved performance over benchmarks.


<details>
  <summary>Details</summary>
Motivation: Mobile manipulators enable new human-robot interaction tasks beyond fixed-base systems, but their additional degrees of freedom make control design more challenging and require performance optimization for safe and efficient physical interaction.

Method: Develops a control approach for a mobile base with mounted robotic arm that minimizes the overall kinetic energy stored in the whole-body system during physical human-robot interaction.

Result: Experimental testing on peg-in-hole tasks shows the approach reduces overall kinetic energy in the whole-body system and improves performance compared to benchmark methods.

Conclusion: The proposed kinetic energy minimization approach effectively enhances mobile manipulator performance in human-robot interaction scenarios, offering a promising control strategy for physically interactive mobile manipulation systems.
...
### [310] [Connectivity-Aware Representations for Constrained Motion Planning via Multi-Scale Contrastive Learning](https://arxiv.org/abs/2603.25298)
*Suhyun Jeon,Yumin Lim,Woo-Jeong Baek,Hyeonseo Kim,Suhan Park,Jaeheung Park*

Main category: cs.RO

TL;DR: Learning connectivity-aware representations for selecting start/goal configurations to avoid disconnected regions in constrained motion planning.


<details>
  <summary>Details</summary>
Motivation: Constrained motion planning becomes inefficient/infeasible when configurations lie in essentially mutually disconnected (EMD) components. Constraints restrict feasible space to lower-dimensional submanifolds, while redundancy creates infinite inverse kinematic solutions forming discrete self-motion manifolds.

Method: Joint configurations are embedded into latent space through multi-scale manifold learning across local to global neighborhood ranges. Clustering generates pseudo-labels that supervise a contrastive learning framework to learn connectivity-aware representations.

Result: Achieves 1.9 times higher success rates and reduces planning time by factor of 0.43 compared to baselines on various manipulation tasks.

Conclusion: The proposed framework provides connectivity-aware measure that biases selection of start/goal configurations in connected regions, avoiding EMDs and improving planning efficiency.
...
### [311] [SafeGuard ASF: SR Agentic Humanoid Robot System for Autonomous Industrial Safety](https://arxiv.org/abs/2603.25353)
*Thanh Nguyen Canh,Thang Tran Viet,Thanh Tuan Tran,Ben Wei Lim*

Main category: cs.RO

TL;DR: SafeGuard ASF is an autonomous security system using humanoid robots for multi-hazard detection in dark factories, achieving 94.2% mAP for fire/smoke detection with low latency.


<details>
  <summary>Details</summary>
Motivation: The rise of unmanned "dark factories" operating without human presence creates a need for autonomous safety systems that can detect and respond to multiple types of hazards in industrial environments.

Method: The system integrates multi-modal perception (RGB-D imaging), a ReAct-based agentic reasoning framework, and learned locomotion policies on Unitree G1 humanoid robots. It uses ToolOrchestra action framework for structured decision-making through perception, reasoning, and actuation tools.

Result: Achieves 94.2% mAP for fire/smoke detection with 127ms latency. Locomotion policies (dance motion tracking and velocity control) trained using Unitree RL Lab with PPO show stable convergence within 80,000 iterations. System validated in simulation and real-world for autonomous patrol, human detection, and obstacle avoidance.

Conclusion: SafeGuard ASF demonstrates a comprehensive framework for autonomous hazard detection in industrial environments using humanoid robots, successfully addressing fire/smoke detection, abnormal temperature monitoring, and intruder detection scenarios.
...
### [312] [Bayesian Learning-Enhanced Navigation with Deep Smoothing for Inertial-Aided Navigation](https://arxiv.org/abs/2603.25364)
*Nadav Cohen,Itzik Klein*

Main category: cs.RO

TL;DR: BLENDS integrates Bayesian learning with deep smoothing to enhance INS/GNSS navigation by using a transformer-based neural network to modify filter covariances and apply error corrections within a Bayesian framework.


<details>
  <summary>Details</summary>
Motivation: Traditional fixed-interval smoothing algorithms cannot resolve the persistent accuracy gap in loosely coupled INS/GNSS systems that inherit systematic position bias from raw GNSS measurements.

Method: BLENDS is a data-driven post-processing framework that augments the classical two-filter smoother with a transformer-based neural network. It learns to modify filter covariance matrices and apply additive corrections to smoothed error-states within the Bayesian framework, using a novel Bayesian-consistent loss for joint supervision.
...
Conclusion: BLENDS successfully bridges the accuracy gap in INS/GNSS navigation by combining Bayesian learning with deep smoothing, demonstrating significant performance improvements over traditional model-based approaches.
...
### [313] [Integrating Deep RL and Bayesian Inference for ObjectNav in Mobile Robotics](https://arxiv.org/abs/2603.25366)
*João Castelo-Branco,José Santos-Victor,Alexandre Bernardino*

Main category: cs.RO

TL;DR: Hybrid object-search framework combining Bayesian inference with deep RL for efficient indoor robot navigation under partial observability.


<details>
  <summary>Details</summary>
Motivation: Classical probabilistic approaches rely on handcrafted heuristics, while deep RL suffers from slow convergence and limited interpretability. Need for adaptive policies that can handle partial observability and uncertainty in indoor object search.

Method: Maintains spatial belief map over target locations updated through Bayesian inference from calibrated object detections. Trains reinforcement learning policy to select navigation actions directly from this probabilistic representation.

Result: Evaluated in realistic indoor simulation using Habitat 3.0. Across two indoor environments, improves success rate while reducing search effort compared to baseline strategies.
...
### [314] [IntentReact: Guiding Reactive Object-Centric Navigation via Topological Intent](https://arxiv.org/abs/2603.25382)
*Yanmei Jiao,Anpeng Lu,Wenhan Hu,Rong Xiong,Yue Wang,Huajin Tang,Wen-an Zhang*

Main category: cs.RO

TL;DR: IntentReact: Intent-conditioned object-centric navigation framework that bridges global topological planning with local reactive control using directional intent signals to improve navigation success and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current object-goal navigation approaches suffer from a gap between global topological guidance and local perception-driven control. Robots make local decisions based only on current egocentric observations, lacking information beyond their field of view, which can cause them to persist in wrong directions that don't decrease global topological distance to the goal.

Method: Proposes IntentReact framework with a compact interface between global topological planning and reactive object-centric control. Encodes global topological guidance as low-dimensional directional signals called "intent," which conditions a learned waypoint prediction policy to bias navigation toward topologically consistent progression.

Result: The framework enables robots to promptly reorient when local observations are misleading, guiding motion toward directions that decrease global topological distance while maintaining reactivity and robustness of object-centric control. Extensive experiments demonstrate improved navigation success and execution quality.

Conclusion: IntentReact successfully bridges the gap between global topological planning and local reactive control in object-goal navigation, outperforming prior object-centric navigation methods by enabling more efficient and successful navigation through intent-conditioned waypoint prediction.
...
### [315] [UMBRELLA: Uncertainty-aware Multi-robot Reactive Coordination under Dynamic Temporal Logic Tasks](https://arxiv.org/abs/2603.25395)
*Qisheng Zhao,Meng Guo,Hengxuan Du,Lars Lindemann,Zhongkui Li*

Main category: cs.RO

TL;DR: UMBRELLA framework coordinates multi-robot systems for collaborative tasks with dynamic moving targets using conformal prediction for uncertainty modeling and Monte Carlo Tree Search with uncertainty-aware rollouts to minimize Conditional Value at Risk of makespan.


<details>
  <summary>Details</summary>
Motivation: Existing multi-robot coordination methods assume static task features or simply replan when changes occur, but real-world tasks often involve dynamic and moving targets with uncertainty in motion prediction.

Method: Combines Conformal Prediction for uncertainty modeling of target motion with Linear Temporal Logic for spatial-temporal constraints. Uses Monte Carlo Tree Search over partial plans with uncertainty-aware rollouts and CP-based metric to guide search. Implements receding-horizon planning for online task releases.

Result: Extensive simulations and hardware experiments show 23% reduction in average makespan and 71% reduction in variance compared to static baselines, with spatial-temporal constraints always ensured and only partial synchronization required during execution.

Conclusion: UMBRELLA effectively coordinates multi-robot systems for dynamic collaborative tasks by explicitly modeling uncertainty and using uncertainty-aware planning, significantly improving both efficiency and reliability compared to static approaches.
...
### [316] [System Design for Maintaining Internal State Consistency in Long-Horizon Robotic Tabletop Games](https://arxiv.org/abs/2603.25405)
...
TL;DR: This paper presents an integrated system architecture for maintaining state consistency in long-horizon robotic tabletop games, using Mahjong as a case study, with explicit state management, partitioned reasoning, verified action primitives, and recovery mechanisms.


<details>
  <summary>Details</summary>
Motivation: Long-horizon tabletop games present a systems challenge where small perceptual or execution errors can invalidate accumulated task state, propagate across decision-making modules, and derail interactions. The paper aims to address how to maintain internal state consistency through deliberate system design rather than isolated component improvement.

Method: The authors present an integrated architecture that: 1) explicitly maintains perceptual, execution, and interaction state; 2) partitions high-level semantic reasoning from time-critical perception and control; 3) incorporates verified action primitives with tactile-triggered recovery; and 4) introduces interaction-level monitoring mechanisms to detect turn violations and hidden-information breaches. They use Mahjong as a representative long-horizon setting.

Result: The system demonstrates complete-game operation and provides empirical characterization of failure modes, recovery effectiveness, cross-module error propagation, and hardware-algorithm trade-offs. Results show that explicit partitioning, monitored state transitions, and recovery mechanisms are critical for sustaining executable consistency over extended play, while monolithic or unverified pipelines lead to measurable degradation in end-to-end reliability.

Conclusion: The proposed system serves as an empirical platform for studying system-level design principles in long-horizon, turn-based interaction, demonstrating that deliberate system design with explicit state management and recovery mechanisms is essential for maintaining consistency in complex robotic tabletop games.
...
### [317] [MMaDA-VLA: Large Diffusion Vision-Language-Action Model with Unified Multi-Modal Instruction and Generation](https://arxiv.org/abs/2603.25406)
...
TL;DR: MMaDA-VLA is a unified diffusion-based Vision-Language-Action model that jointly generates future visual goals and robot actions in parallel using discrete token diffusion, eliminating hierarchical architectures and improving long-horizon consistency.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models suffer from architectural overhead, temporal inconsistency, long-horizon error accumulation, and lack mechanisms to capture environment dynamics without extra modules. Hierarchical and autoregressive approaches introduce these limitations.

Method: A native discrete diffusion formulation that embeds language, images, and continuous robot controls into a unified discrete token space. Trains a single backbone with masked token denoising to jointly generate future goal observations and action chunks in parallel through iterative refinement.

Result: State-of-the-art performance: 98.0% average success on LIBERO benchmark and 4.78 average length on CALVIN benchmark. Demonstrates superior performance across simulation benchmarks and real-world tasks.

Conclusion: MMaDA-VLA successfully unifies multi-modal understanding and generation in a single diffusion framework, enabling global refinement, improving long-horizon consistency, and grounding actions in predicted visual outcomes without auxiliary world models.
...
### [318] [Visualizing Impedance Control in Augmented Reality for Teleoperation: Design and User Evaluation](https://arxiv.org/abs/2603.25418)
*Gijs van den Brandt,Femke van Beek,Elena Torta*

Main category: cs.RO

TL;DR: AR visualization of impedance controller target pose improves teleoperation performance for contact-rich manipulation tasks by providing intuitive force feedback without haptic hardware.


<details>
  <summary>Details</summary>
Motivation: Teleoperation for contact-rich manipulation is challenging with motion-only interfaces lacking haptic feedback. Virtual reality controllers enable intuitive motion control but don't allow operators to perceive or regulate contact forces, limiting task performance.

Method: Proposed an augmented reality (AR) visualization of the impedance controller's target pose and its displacement from each robot end effector. This visualization conveys forces generated by the controller, providing intuitive real-time feedback without expensive haptic hardware.

Result: In a dual-arm manipulation study with 17 participants repositioning a box, AR visualization reduced completion time by 24% for force-critical lifting tasks. No significant effect on sliding tasks where precise force control is less critical.
...
### [319] [Temporally Decoupled Diffusion Planning for Autonomous Driving](https://arxiv.org/abs/2603.25462)
*Xiang Li,Bikun Wang,John Zhang,Jianjun Wang*

Main category: cs.RO

TL;DR: TDDM is a diffusion model for motion planning that decouples temporal dependencies by treating trajectories as segments with independent noise levels, enabling better handling of near-term dynamics and far-term goals.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models treat trajectories as monolithic entities, failing to capture heterogeneous temporal dependencies where near-term plans are constrained by instantaneous dynamics while far-term plans depend on navigational goals.

Method: Proposes Temporally Decoupled Diffusion Model (TDDM) with noise-as-mask paradigm that partitions trajectories into segments with independent noise levels. Uses Temporally Decoupled Adaptive Layer Normalization (TD-AdaLN) to inject segment-specific timesteps and Asymmetric Temporal Classifier-Free Guidance during inference.

Result: Evaluations on nuPlan benchmark show TDDM approaches or exceeds state-of-the-art baselines, with particularly strong performance on the challenging Test14-hard subset.

Conclusion: TDDM effectively addresses temporal heterogeneity in motion planning by decoupling trajectory generation, enabling better balance between immediate safety and long-term goals in dynamic urban environments.
...
### [320] [LILAC: Language-Conditioned Object-Centric Optical Flow for Open-Loop Trajectory Generation](https://arxiv.org/abs/2603.25481)
*Motonari Kambara,Koki Seno,Tomoya Kaichi,Yanan Wang,Komei Sugiura*

Main category: cs.RO

TL;DR: LILAC: A flow-based VLA model that generates object manipulation trajectories from images and language instructions using optical flow, requiring minimal robot-specific data.


<details>
  <summary>Details</summary>
Motivation: To enable language-conditioned robotic manipulation by leveraging human/web videos with minimal embodiment-specific data, addressing the challenge of aligning language instructions with object trajectory generation.

Method: Proposes LILAC with two key components: 1) Semantic Alignment Loss for better language conditioning in optical flow generation, and 2) Prompt-Conditioned Cross-Modal Adapter to align visual prompts with image/text features for richer flow generation cues.

Result: Outperformed existing approaches in generated flow quality across multiple benchmarks and achieved superior task success rates in physical object manipulation experiments with free-form instructions.

Conclusion: LILAC effectively enables language-conditioned robotic manipulation through flow-based trajectory generation, demonstrating strong performance with minimal embodiment-specific data requirements.
...
### [321] [Towards Embodied AI with MuscleMimic: Unlocking full-body musculoskeletal motor learning at scale](https://arxiv.org/abs/2603.25544)
*Chengkun Li,Cheryl Wang,Bianca Ziliotto,Merkourios Simos,Jozsef Kovecses,Guillaume Durandau,Alexander Mathis*

Main category: cs.RO

TL;DR: MuscleMimic is an open-source framework for scalable motion imitation learning with physiologically realistic, muscle-actuated humanoids, achieving order-of-magnitude training speedups and enabling training on hundreds of diverse motions within days.


<details>
  <summary>Details</summary>
Motivation: Learning motor control for muscle-driven musculoskeletal models is hindered by computational costs of biomechanically accurate simulation and scarcity of validated, open full-body models.

Method: Provides two validated musculoskeletal embodiments (upper-body and full-body models) with retargeting pipeline mapping SMPL-format motion capture data, leveraging massively parallel GPU simulation for training speedups while maintaining comprehensive collision handling.

Result: Achieves order-of-magnitude training speedups over prior CPU approaches, enables single generalist policy training on hundreds of motions within days, faithfully reproduces human movements, and shows strong biomechanical validation (joint kinematics correlation r=0.90).

Conclusion: MuscleMimic lowers computational and data barriers to musculoskeletal simulation, enabling systematic model validation across diverse movements and broader participation in neuromuscular control research.
...
### [322] [Towards Generalizable Robotic Data Flywheel: High-Dimensional Factorization and Composition](https://arxiv.org/abs/2603.25583)
*Yuyang Xiao,Yifei Zhou,Haoran Wang,Wenxuan Ou,Yuxiao Liu*

Main category: cs.RO

TL;DR: F-ACIL is a factor-aware compositional iterative learning framework that decomposes robotic data into structured factor spaces (object, action, environment) to enable more efficient data collection and promote compositional generalization with 5-10× fewer demonstrations.


<details>
  <summary>Details</summary>
Motivation: The lack of diverse data and limited data efficiency are major bottlenecks for generalist robotic models. Task diversity arises from implicit factors that are sparsely distributed across multiple dimensions and difficult to define explicitly, requiring systematic strategies for data collection and curation.

Method: F-ACIL decomposes data distribution into structured factor spaces (object, action, environment), then uses a factor-wise data collection and iterative training paradigm that promotes compositional generalization over high-dimensional factor space for more effective utilization of real-world robotic demonstrations.

Result: F-ACIL achieves more than 45% performance gains with 5-10× fewer demonstrations compared to approaches without this strategy, demonstrating that structured factorization offers a practical pathway toward efficient compositional generalization in real-world robotic learning.

Conclusion: Structured factorization enables efficient compositional generalization in robotics, and F-ACIL can inspire more systematic research on building generalizable robotic data flywheel strategies.
...
### [323] [Accurate Surface and Reflectance Modelling from 3D Radar Data with Neural Radiance Fields](https://arxiv.org/abs/2603.25623)
*Judith Treffler,Vladimír Kubelka,Henrik Andreasson,Martin Magnusson*

Main category: cs.RO

TL;DR: Neural implicit approach for 3D mapping from radar point clouds that jointly models geometry and view-dependent radar intensities using hybrid feature encoding.


<details>
  <summary>Details</summary>
Motivation: Radar is resilient in low-visibility conditions (fog, smoke, dust) but produces sparse, noisy data, making reliable 3D surface reconstruction challenging. Need robust scene representation for autonomous systems.

Method: Proposes neural implicit approach with memory-efficient hybrid feature encoding to learn continuous Signed Distance Field (SDF) for surface reconstruction while capturing radar-specific reflective properties and view-dependent intensities.

Result: Produces smoother, more accurate 3D surface reconstructions compared to existing lidar-based methods applied to radar data. Can reconstruct view-dependent radar intensities. Neural implicit representations perform better with sparser input point clouds than traditional explicit SDFs and meshing techniques.

Conclusion: Neural implicit representations are effective for 3D mapping from sparse radar data, outperforming traditional methods especially as data gets sparser, enabling better autonomous system operation in challenging low-visibility environments.
...
### [324] [A Mentalistic Interface for Probing Folk-Psychological Attribution to Non-Humanoid Robots](https://arxiv.org/abs/2603.25646)
*Giulio Pisaneschi,Pierpaolo Serio,Estelle Gerbier,Andrea Dan Ryals,Lorenzo Pollini,Mario G. C. A. Cimino*

Main category: cs.RO

TL;DR: Experimental platform for studying how people attribute intentional states to non-humanoid robots using different explanatory frames while keeping robot behavior constant.


<details>
  <summary>Details</summary>
Motivation: To investigate how language and framing influence people's adoption of the intentional stance (attributing mental states) toward robots, particularly non-humanoid ones, which is important for understanding human-robot interaction and social cognition.

Method: Developed a system combining: 1) simulated non-humanoid robot, 2) realistic task environments, and 3) large language model-based explanatory layers that can describe the same robot behavior using mentalistic (intentional), teleological (goal-oriented), or mechanistic (physical cause-effect) terms.

Result: Created a controlled experimental platform that allows researchers to study intentional-state attribution by varying only the explanatory frame while keeping robot behavior constant, enabling systematic investigation of how language shapes perception of robot intentionality.

Conclusion: The platform provides a novel methodological approach for studying the intentional stance in robotics, offering researchers a tool to examine how different explanatory frameworks affect human perception of robot agency and intentionality.
...
### [325] [Fast-dVLA: Accelerating Discrete Diffusion VLA to Real-Time Performance](https://arxiv.org/abs/2603.25661)
...
TL;DR: Proposes Fast-dVLA, a method to enhance VLA model finetuning by decoupling auxiliary task objectives into capability vectors that can be merged with pretrained parameters, achieving performance comparable to auxiliary finetuning with reduced computational overhead.


<details>
  <summary>Details</summary>
Motivation: Pretrained VLA models often fail to effectively improve performance and reduce adaptation costs during standard supervised finetuning (SFT). While advanced finetuning methods with auxiliary training objectives can help, they incur significant computational overhead from additional losses.

Method: Decouples auxiliary task training objectives into: 1) enhancing general capabilities and 2) fitting task-specific action distributions. Trains models on small-scale task sets using two distinct strategies, extracts capability vectors from parameter differences, merges them with pretrained parameters to form a capability-enhanced meta model, and adds lightweight orthogonal regularization during standard SFT.
...
Conclusion: The approach successfully achieves enhanced capabilities of auxiliary training with the simplicity of standard SFT, providing an efficient solution for VLA model adaptation that balances performance and computational cost.
...
### [326] [Can Users Specify Driving Speed? Bench2Drive-Speed: Benchmark and Baselines for Desired-Speed Conditioned Autonomous Driving](https://arxiv.org/abs/2603.25672)
*Yuqian Shao,Xiaosong Jia,Langechuan Liu,Junchi Yan*

Main category: cs.RO

TL;DR: Bench2Drive-Speed is a benchmark for speed-conditioned autonomous driving that introduces user-specified speed and overtaking instructions, with metrics and dataset to evaluate policy adherence to user preferences.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end autonomous driving systems lack user customization capabilities - users cannot specify desired speeds or overtaking preferences, which limits practical utility and user control.

Method: Introduces explicit target-speed and overtake/follow instructions as policy inputs; creates CustomizedSpeedDataset with 2,100 expert-annotated clips; develops Speed-Adherence Score and Overtake Score metrics; investigates training strategies using regular driving data with speed re-annotation.

Result: Models trained on properly re-annotated regular driving data perform comparably to those trained on expert demonstrations; target-speed following can be achieved without degrading regular driving performance, but overtaking commands remain challenging due to interactive behavior complexity.

Conclusion: Speed supervision can be introduced without additional complex real-world data collection, enabling user-customizable autonomous driving, though overtaking behavior requires further research due to interactive complexity.
...
### [327] [Persistent Robot World Models: Stabilizing Multi-Step Rollouts via Reinforcement Learning](https://arxiv.org/abs/2603.25685)
*Jai Bardhan,Patrik Drozdik,Josef Sivic,Vladimir Petrik*

Main category: cs.RO

TL;DR: RL post-training improves robot world model rollout fidelity by training on autoregressive rollouts with contrastive RL, multi-view fidelity rewards, and outperforms baselines on DROID dataset.


<details>
  <summary>Details</summary>
Motivation: Action-conditioned robot world models break down during autoregressive rollout due to error compounding, limiting their usefulness for long-horizon task simulation.

Method: 1) RL post-training with contrastive RL objective adapted for diffusion models; 2) Training protocol comparing multiple candidate futures; 3) Multi-view visual fidelity rewards aggregated at clip level; 4) Evaluation on DROID dataset.

Result: New SOTA on DROID: LPIPS reduced by 14% on external cameras, SSIM improved by 9.1% on wrist camera, wins 98% of paired comparisons, achieves 80% human preference rate.

Conclusion: RL post-training effectively addresses error compounding in autoregressive world model rollouts, enabling high-fidelity long-horizon predictions for robot task simulation.
...
### [328] [Intelligent Navigation and Obstacle-Aware Fabrication for Mobile Additive Manufacturing Systems](https://arxiv.org/abs/2603.25688)
*Yifei Li,Ruizhe Fu,Huihang Liu,Guha Manogharan,Feng Ju,Ilya Kovalenko*

Main category: cs.RO

TL;DR: A universal mobile printing-and-delivery platform integrates navigation and material deposition for Mobile AM Robots (MAMbots) to enable flexible manufacturing in dynamic environments with real-time control for obstacle avoidance and print quality.


<details>
  <summary>Details</summary>
Motivation: As mass customization increases, manufacturing needs more flexibility. Additive manufacturing (AM) enables on-demand fabrication but has fixed equipment limitations. Mobile robots can address this by allowing manufacturing resources to move and adapt, but dynamic environments create challenges for navigation stability and printing accuracy.

Method: Proposes a universal mobile printing-and-delivery platform that couples navigation and material deposition (unlike previous separate approaches). Develops a real-time control framework for planning and controlling robot navigation with safe motion, obstacle avoidance, and path stability while maintaining print quality. Uses closed-loop integration of sensing, mobility, and manufacturing for real-time feedback and autonomous decision-making.

Result: The framework is validated through simulations and real-world experiments testing adaptability to trajectory variations and external disturbances. Coupled navigation and printing enables MAMbots to plan safe, adaptive trajectories.

Conclusion: The integrated approach improves flexibility and adaptability in manufacturing by enabling Mobile AM Robots to operate effectively in dynamic environments, overcoming limitations of fixed equipment and separate navigation/printing systems.
...
### [329] [SoftMimicGen: A Data Generation System for Scalable Robot Learning in Deformable Object Manipulation](https://arxiv.org/abs/2603.25725)
*Masoud Moghani,Mahdi Azizian,Animesh Garg,Yuke Zhu,Sean Huver,Ajay Mandlekar*

Main category: cs.RO

TL;DR: SoftMimicGen is an automated synthetic data generation pipeline for deformable object manipulation tasks, addressing the data scarcity problem in robot learning through high-fidelity simulation environments.


<details>
  <summary>Details</summary>
Motivation: Real-world robot datasets are difficult and expensive to collect, especially for deformable object manipulation which constitutes a large portion of real-world tasks. While synthetic data has helped rigid-body tasks, deformable manipulation remains a crucial gap in the synthetic data paradigm.

Method: Developed SoftMimicGen pipeline with high-fidelity simulation environments covering diverse deformable objects (stuffed animal, rope, tissue, towel) and manipulation behaviors (threading, whipping, folding, pick-and-place) across four robot embodiments (single-arm, bimanual, humanoid, surgical robot).

Result: Successfully generated datasets across the task suite, trained high-performing policies from the synthetic data, and conducted systematic analysis of the data generation system.

Conclusion: SoftMimicGen bridges the crucial gap in synthetic data generation for deformable object manipulation, enabling scalable data collection and policy learning for complex manipulation tasks that were previously difficult to address with real-world data alone.
...
### [330] [Drive My Way: Preference Alignment of Vision-Language-Action Model for Personalized Driving](https://arxiv.org/abs/2603.25740)
*Zehao Wang,Huaide Jiang,Shuaiwu Dong,Yuping Wang,Hang Qiu,Jiachen Li*

Main category: cs.RO

TL;DR: Drive My Way (DMW) is a personalized Vision-Language-Action framework for autonomous driving that adapts to individual driving habits and real-time user instructions, outperforming generic approaches.


<details>
  <summary>Details</summary>
Motivation: Existing autonomous driving systems lack personalization - they either optimize for generic objectives or use fixed driving modes, failing to adapt to individual preferences or interpret natural language intent. Human driving is inherently personal, shaped by long-term habits and short-term intentions.

Method: DMW learns user embeddings from personalized driving data collected across multiple real drivers, conditions the policy on these embeddings during planning, and incorporates natural language instructions for short-term guidance. It's a Vision-Language-Action (VLA) framework that aligns with both long-term habits and real-time instructions.

Result: Closed-loop evaluation on Bench2Drive benchmark shows DMW improves style instruction adaptation. User studies demonstrate its generated behaviors are recognizable as each driver's own style. Personalization emerges as a key capability for human-centered autonomous driving.

Conclusion: DMW successfully addresses the personalization gap in autonomous driving by combining long-term habit learning with short-term instruction interpretation, making autonomous vehicles more adaptable to individual preferences and natural language commands.
