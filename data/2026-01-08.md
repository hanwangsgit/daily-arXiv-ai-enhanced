...
TL;DR: HyperCLOVA X 32B Think is a Korean-focused vision-language model with strong reasoning and agentic capabilities, achieving top performance on Korean benchmarks and agent tasks, now open-sourced for broader adoption.


<details>
  <summary>Details</summary>
Motivation: To create a vision-language model specifically designed for Korean linguistic and cultural context with emphasis on reasoning capabilities and agentic abilities, addressing the need for culturally-aware AI systems in Korea.

Method: Two-stage approach: 1) Pre-training with strong focus on reasoning capabilities, 2) Post-training to add multimodal understanding, enhanced reasoning, agentic behaviors, and human preference alignment.
...
Conclusion: HyperCLOVA X 32B Think successfully addresses Korean language and cultural needs with strong reasoning and agentic capabilities, and open-sourcing it will support broader adoption and further research innovation.
...
TL;DR: CDRF is a large-scale RF drone detection benchmark combining real-world captures with synthetic data augmentation to address dataset scarcity and diversity issues.


<details>
  <summary>Details</summary>
Motivation: Existing RF drone datasets suffer from scarcity and limited diversity, hindering development of robust RF perception models for drone detection and identification.

Method: Combines extensive real-world RF recordings with systematic synthetic augmentation pipeline that controls SNR, injects interfering emitters, applies frequency shifts with label-consistent bounding-box transformations.

Result: CDRF spans wide range of contemporary drone models and acquisition conditions, includes interoperable open-source tools for data generation, preprocessing, augmentation, and evaluation.
...
TL;DR: Proposes SuPLoRA, a hierarchical concept erasure method for diffusion models that groups related concepts under supertypes and uses parameter-efficient fine-tuning to erase multiple concepts simultaneously while preserving generation quality.


<details>
  <summary>Details</summary>
Motivation: Current concept erasure methods become inefficient and ineffective as the number of erased concepts grows, requiring separate parameters for each concept and degrading overall generation quality. There's a need for a scalable approach that can handle multiple concepts simultaneously.

Method: Introduces a supertype-subtype concept hierarchy where related concepts are grouped under parent nodes. Uses group-wise suppression with shared parameters and proposes SuPLoRA (Supertype-Preserving Low-Rank Adaptation) that encodes supertype information in frozen down-projection matrices while updating only up-projection matrices during erasure.

Result: Theoretical analysis shows effectiveness in mitigating generation degradation. A challenging benchmark is constructed for simultaneous erasure across diverse domains (celebrities, objects, pornographic content).

Conclusion: SuPLoRA provides an efficient and effective approach for multi-concept erasure in diffusion models by leveraging hierarchical concept organization and parameter-efficient fine-tuning, addressing scalability limitations of existing methods.
...
TL;DR: VLM4VLA: A minimal pipeline for converting VLMs into VLA policies reveals that VLM general capabilities poorly predict downstream embodied control performance, with visual modules being the primary bottleneck.
...
Method: Introduces VLM4VLA - a minimal adaptation pipeline that converts general-purpose VLMs into VLA policies using only a small set of new learnable parameters for fair comparison. Conducts extensive empirical studies across three benchmarks and investigates specific embodied capabilities through fine-tuning on seven auxiliary embodied tasks.

Result: VLM initialization offers consistent benefit over training from scratch, but VLM general capabilities are poor predictors of downstream task performance. Improving VLM performance on specific embodied skills doesn't guarantee better control performance. Visual module in VLM, not language component, is the primary bottleneck. Injecting control-relevant supervision into vision encoder yields consistent gains even when frozen.
...
<details>
  <summary>Details</summary>
Motivation: Consumer demand for high-quality aquatic products requires addressing rapid freshness decline in shrimp and head-body separation problems that affect appearance and consumer perception.

Method: Uses convolutional neural network (CNN) model for image recognition to automate classification of white shrimp immediately after harvest, replacing manual sorting.

Result: Enhanced classification accuracy, efficiency, and consistency; reduced processing time helps maintain freshness and enables shrimp transportation businesses to better meet customer demands.

Conclusion: Deep learning-based image recognition provides an effective automated solution for shrimp classification that addresses freshness and quality concerns in aquaculture processing.
...
TL;DR: The paper proposes extending PCA's covariance matrix to higher-order tensors for more accurate rotation-invariant shape descriptors, enabling applications in molecular shape analysis, object recognition, and shape similarity metrics.


<details>
  <summary>Details</summary>
Motivation: PCA's covariance matrix only approximates shapes as ellipsoids, which is insufficient for complex real-world shapes. Higher-order moment tensors are needed to capture more detailed shape information while maintaining rotation invariance.

Method: Extend PCA's second-order covariance matrix to higher-order tensors (e.g., order-3 p_abc) describing central moments, or use polynomial times Gaussian for decodable shape descriptors of arbitrarily high accuracy, with analogous rotation invariants.

Result: The method provides rotation-invariant features that can describe shapes modulo rotation with high accuracy, enabling applications in molecular shape descriptors, object recognition in 2D/3D, and inexpensive shape similarity comparison without rotation optimization.

Conclusion: Higher-order tensor extensions of PCA's covariance matrix offer powerful rotation-invariant shape descriptors that overcome the limitations of ellipsoid approximations, enabling practical applications in various domains requiring shape analysis modulo rotation.
...
TL;DR: DA3 uses domain adaptation to bridge simulation-real gap in optical alignment, improving accuracy by 46% with minimal real data, reducing on-device collection time by 98.7%.


<details>
  <summary>Details</summary>
Motivation: Active Alignment is crucial for automated optical assembly, but simulation-trained models suffer from domain gap with real-world images, limiting generalization. Traditional per-model calibration is labor-intensive.

Method: Propose Domain Adaptive Active Alignment (DA3) using autoregressive domain transformation generator and adversarial feature alignment for self-supervised learning. Extracts domain-invariant degradation features for robust misalignment prediction.

Result: DA3 improves accuracy by 46% over pure simulation pipeline. Approaches performance of precisely labeled real-world data from 3 lens samples while reducing on-device data collection time by 98.7%.

Conclusion: Domain adaptation effectively enables simulation-trained models to perform robustly in real-world settings, validating digital-twin pipeline as practical solution for efficient large-scale optical assembly.
...
TL;DR: MMErroR is a new multi-modal benchmark with 2,013 samples containing single reasoning errors across 24 subdomains, designed to test VLMs' ability to detect incorrect reasoning and classify error types, revealing even top models struggle with this task.


<details>
  <summary>Details</summary>
Motivation: To assess whether Vision-Language Models truly understand content by testing their ability to detect when reasoning processes are wrong and identify specific error types, moving beyond simple answer correctness to process-level evaluation.

Method: Created MMErroR benchmark with 2,013 samples embedding single coherent reasoning errors, spanning 24 subdomains across six top-level domains, enabling error detection and classification evaluation in both visual and linguistic contexts.

Result: Evaluated 20 advanced VLMs; even the best model (Gemini-3.0-Pro) achieved only 66.47% accuracy in error classification, demonstrating significant challenges in identifying erroneous reasoning.

Conclusion: MMErroR reveals VLMs' limitations in error detection and classification, providing valuable insights into multi-modal reasoning capabilities and establishing a challenging benchmark for future model development.
...
### [9] [Padé Neurons for Efficient Neural Models](https://arxiv.org/abs/2601.04005)
*Onur Keleş,A. Murat Tekalp*

Main category: cs.CV

TL;DR: Padé neurons (Paons) are a novel non-linear neuron model inspired by Padé approximants that provide stronger non-linearity and layer efficiency compared to traditional McCulloch-Pitts neurons.


<details>
  <summary>Details</summary>
Motivation: Traditional neural networks use linear McCulloch-Pitts neurons with point-wise non-linear activations, which have limited non-linearity. Researchers have proposed various non-linear neuron models, but there's a need for a more powerful and efficient non-linear neuron model that can capture diverse non-linear relationships while being computationally efficient.

Method: The authors introduce Padé neurons (Paons) inspired by Padé approximants. Each Paon learns a different non-linear function of its inputs, providing diversity of non-linearity. The model is designed to be layer-efficient, offering stronger non-linearity in fewer layers compared to piecewise linear approximations. Paons can replace any existing neuron model as they include all previously proposed neuron models as special cases.

Result: Experimental results show that neural models built with Paons provide better or equal performance than their classic counterparts with fewer layers. The authors validated Paons by replacing classic neurons in well-known neural image super-resolution, compression, and classification models based on ResNet architecture.

Conclusion: Padé neurons offer a superior non-linear neuron model with advantages in non-linearity diversity and layer efficiency. They can replace existing neuron models in various neural network architectures and provide performance improvements with reduced layer complexity. The approach is open-sourced for community use.
...
TL;DR: A two-stage 3D Gaussian Splatting model that learns to relight head avatars from single/multi-view images without requiring complex OLAT capture, enabling generalization to new subjects from minimal input.


<details>
  <summary>Details</summary>
Motivation: Existing high-quality relighting methods require subjects to be captured under complex time-multiplexed illumination (OLAT), which is impractical. The paper aims to enable relightable 3D head avatars from standard single- or multi-view images without OLAT data.

Method: Two-stage approach: 1) Learn flat-lit 3DGS avatars from diverse multi-view datasets without OLAT, using dataset-specific lighting codes for self-supervised alignment. 2) Learn mapping from flat-lit avatars to physically-based reflectance parameters using a smaller OLAT dataset, enabling relighting generalization.

Result: The model can generalize to relight any subject from the first stage as if captured under OLAT lighting, and can fit to unseen subjects from as little as a single image, enabling novel view synthesis and relighting applications.

Conclusion: Proposes a practical solution for relightable 3D head avatars that eliminates the need for complex OLAT capture while maintaining high-quality relighting capabilities, with strong generalization from minimal input data.
...
<details>
  <summary>Details</summary>
Motivation: Soft boundaries like thin hairs are common in natural and computer-generated imagery but remain challenging for 3D vision due to ambiguous mixing of foreground and background cues, leading to poor depth estimation and view synthesis quality.

Method: 1) Data curation pipeline using image matting datasets; 2) Depth fixer network with gated residual module to refine depth around soft boundaries; 3) Depth-based forward warping for view synthesis; 4) Generative scene painter for disocclusion filling; 5) Color fuser to combine warped and inpainted results.
...
Conclusion: HairGuard effectively addresses soft boundary challenges in 3D vision through specialized depth refinement and view synthesis components, demonstrating plug-and-play compatibility with existing depth models and superior performance on fine-grained details.
...
TL;DR: Introduces RiskCueBench, a new video risk assessment benchmark that focuses on identifying early risk signals before accidents occur, rather than analyzing full sequences that include the accidents themselves.


<details>
  <summary>Details</summary>
Motivation: Current video risk assessment models have access to full video sequences including accidents, which reduces task difficulty and doesn't reflect real-world conditions where early anticipation is crucial for preventing accidents and ensuring public safety.

Method: Created RiskCueBench with carefully annotated videos that identify a "risk signal clip" - the earliest moment indicating potential safety concerns, forcing models to anticipate risks from early visual signals rather than analyzing complete accident sequences.

Result: Experimental results show significant gaps in current systems' ability to interpret evolving situations and anticipate future risky events from early visual signals, highlighting practical deployment challenges.

Conclusion: RiskCueBench reveals important limitations in current video risk prediction models and establishes a more realistic benchmark for developing systems that can truly anticipate and prevent accidents from early visual cues.
...
<details>
  <summary>Details</summary>
Motivation: Deepfake synthesis poses significant threats to trust in digital media, necessitating robust detection methods to combat misuse of AI-generated content.

Method: Proposes unified architecture with cross-attention mechanism between spatial and frequency domain features, combined with a blood detection module for classifying images/videos as real or fake.

Result: Achieves 99.80% AUC on FF++ and 99.88% on Celeb-DF using Swin Transformer+BERT; 99.55% and 99.38% with EfficientNet-B4+BERT. Shows strong cross-dataset generalization.

Conclusion: The proposed architecture outperforms state-of-the-art methods and provides a unified framework with strong generalization capabilities for deepfake detection.
...
TL;DR: Current video-based ANNs modestly improve neural predictivity of macaque IT cortex responses to naturalistic videos compared to static models, but fail to capture IT's appearance-invariant motion processing, highlighting the need for new objectives that encode biological temporal statistics.


<details>
  <summary>Details</summary>
Motivation: While feedforward ANNs trained on static images dominate ventral stream modeling, they ignore dynamic aspects of primate vision. The inferior temporal (IT) cortex encodes both object recognition and motion velocity during naturalistic viewing, raising questions about whether IT's temporal responses reflect simple frame-by-frame processing or richer dynamic computations.

Method: Compared macaque IT responses during naturalistic videos against static, recurrent, and video-based ANN models. Applied a stress test: trained decoders on naturalistic videos and evaluated them on "appearance-free" variants that preserve motion while removing shape and texture.

Result: Video models provided modest improvements in neural predictivity, especially at later response stages. However, while IT population activity generalized across appearance-free manipulations, all ANN classes failed to do so, revealing they capture appearance-bound dynamics rather than appearance-invariant temporal computations.

Conclusion: Current video models better capture appearance-bound dynamics rather than the appearance-invariant temporal computations expressed in IT, underscoring the need for new objectives that encode biological temporal statistics and invariances.
...
TL;DR: Eye-Q is a multilingual visual word puzzle benchmark that tests complex visual reasoning beyond surface-level recognition, requiring hypothesis generation and revision to solve implicit visual cues.


<details>
  <summary>Details</summary>
Motivation: Current VLMs often rely on surface-level recognition rather than deeper reasoning, and existing benchmarks can be solved via literal grounding, OCR shortcuts, or simple retrieval. There's a need for tasks that require discovering implicit visual cues, generating/revising hypotheses, and mapping perceptual evidence to non-literal concepts.
...
Conclusion: Visual word puzzles like Eye-Q expose critical limitations in current VLMs' reasoning capabilities, showing they struggle with tasks requiring selective attention, abstraction, associative inference, and hypothesis revision. The benchmark provides a challenging testbed for developing more sophisticated visual reasoning models.
...
TL;DR: GAMBIT is a novel multimodal jailbreak framework that uses gamified adversarial scenes to trick reasoning-based MLLMs into generating harmful content by manipulating their cognitive decision-making processes.


<details>
  <summary>Details</summary>
Motivation: Current multimodal jailbreak attacks underperform on reasoning models (with Chain-of-Thought) compared to non-reasoning ones. The authors propose that if models think like humans, we can influence their cognitive-stage decisions to proactively complete jailbreaks.

Method: GAMBIT decomposes and reassembles harmful visual semantics, constructs gamified scenes that drive models to explore, reconstruct intent, and answer as part of winning a game. This creates structured reasoning chains that increase task complexity while positioning the model as a participant whose goal pursuit reduces safety attention.

Result: Achieves high Attack Success Rates: 92.13% on Gemini 2.5 Flash, 91.20% on QvQ-MAX, and 85.87% on GPT-4o, significantly outperforming baseline methods.

Conclusion: GAMBIT demonstrates that leveraging models' own reasoning incentives through gamified adversarial scenes is an effective approach for jailbreaking reasoning-based MLLMs, highlighting vulnerabilities in safety alignment that persist even in advanced reasoning architectures.
...
TL;DR: WeedRepFormer is a lightweight multi-task Vision Transformer for simultaneous waterhemp segmentation and gender classification, achieving high accuracy with low computational cost through structural reparameterization.


<details>
  <summary>Details</summary>
Motivation: Existing agricultural models struggle to balance fine-grained feature extraction for biological attribute classification with the efficiency needed for real-time deployment in agricultural applications.

Method: A lightweight multi-task Vision Transformer with systematic structural reparameterization across the entire architecture (Vision Transformer backbone, Lite R-ASPP decoder, and novel reparameterizable classification head) to decouple training-time capacity from inference-time latency.

Result: Achieves 92.18% mIoU for segmentation and 81.91% accuracy for gender classification using only 3.59M parameters and 3.80 GFLOPs, running at 108.95 FPS. Outperforms state-of-the-art iFormer-T by 4.40% in classification accuracy while reducing parameters by 1.9x.

Conclusion: WeedRepFormer successfully addresses the efficiency-accuracy trade-off for agricultural vision tasks, providing a practical solution for real-time waterhemp analysis with simultaneous segmentation and gender classification capabilities.
...
TL;DR: FROST-Drive: A novel E2E autonomous driving architecture that keeps VLM vision encoder frozen to preserve generalization, outperforming full fine-tuning on long-tail scenarios.


<details>
  <summary>Details</summary>
Motivation: Current E2E autonomous driving models struggle with generalization to novel/complex scenarios. Full fine-tuning of vision encoders on driving data causes over-specialization and limits generalization capabilities. The paper challenges the necessity of this training paradigm.

Method: Proposes FROST-Drive architecture with: 1) Frozen pretrained VLM vision encoder to preserve world knowledge, 2) Transformer-based adapter for multimodal fusion, 3) GRU-based decoder for smooth waypoint generation, 4) Custom loss function optimized for Rater Feedback Score (RFS).

Result: Extensive experiments on Waymo Open E2E Dataset (large-scale with long-tail scenarios) show frozen-encoder approach significantly outperforms full fine-tuning models. Provides evidence that preserving VLM knowledge is more effective for robust, generalizable driving.

Conclusion: Preserving broad knowledge from capable VLMs is more effective than intensive domain-specific adaptation for robust E2E driving. Offers new pathway for vision-based models to handle real-world complexities.
...
TL;DR: Shallow, well-regularized CNNs serve as competitive baselines across diverse Bangladeshi vision tasks without requiring large GPUs or specialized pre-trained models.


<details>
  <summary>Details</summary>
Motivation: To establish practical, reproducible vision benchmarks for low-resource settings in Bangladesh, addressing the need for lightweight solutions that don't require expensive hardware or specialized models.

Method: Uses well-regularized shallow CNN architectures as baseline models, applied across heterogeneous vision domains including smart-city surveillance and agricultural variety classification.

Result: Shallow architectures demonstrate competitive performance across multiple Bangladeshi vision datasets, establishing a unified benchmark and showing practical value for real-world deployment in resource-constrained environments.

Conclusion: Lightweight CNNs with proper regularization can serve as effective, practical solutions for vision tasks in low-resource settings, providing a reproducible benchmark for Bangladeshi datasets without requiring expensive hardware or specialized pre-trained models.
...
TL;DR: A scalable ALS-based recommender system on MovieLens 32M shows constrained low-rank models outperform higher-dimensional ones, with semantic genre clusters emerging from interaction data alone.
...
Method: High-performance parallelized Alternating Least Squares (ALS) framework with extensive hyperparameter optimization, visualization of embedding space, and cold-start validation with tunable scoring parameter.

Result: Constrained low-rank models significantly outperform higher dimensional counterparts in generalization, achieving optimal RMSE and ranking precision balance. Semantic genre clusters emerge unsupervised from interaction data.

Conclusion: The system effectively captures deep structural relationships from interaction data alone and provides practical utility in cold-start scenarios with tunable trade-off between popularity bias and personalized affinity.
...
TL;DR: ThinkRL-Edit is a reasoning-centric RL framework for image editing that decouples visual reasoning from synthesis, uses CoT-based reasoning sampling with planning/reflection stages, unbiased chain preference grouping, and binary checklist rewards to improve reasoning-focused edits.


<details>
  <summary>Details</summary>
Motivation: Current instruction-driven image editing models have limited visual reasoning capabilities, leading to poor performance on reasoning-centric edits. Existing RL approaches face challenges: limited reasoning exploration confined to denoising stochasticity, biased reward fusion, and unstable VLM-based instruction rewards.

Method: Proposes ThinkRL-Edit framework that: 1) Decouples visual reasoning from image synthesis, 2) Uses Chain-of-Thought-based reasoning sampling with planning and reflection stages before generation, 3) Implements unbiased chain preference grouping across reward dimensions, 4) Replaces interval-based VLM scores with binary checklist rewards for more precise and interpretable feedback.

Result: Experiments show the method significantly outperforms prior work on reasoning-centric image editing, producing instruction-faithful, visually coherent, and semantically grounded edits.

Conclusion: ThinkRL-Edit successfully addresses key challenges in RL-based image editing by expanding reasoning exploration beyond denoising, providing unbiased reward aggregation, and using more stable binary checklist rewards, leading to superior performance on reasoning-focused image editing tasks.
...
TL;DR: The paper proposes an adaptive artifact reward model to combat reward hacking in RL-based text-to-image generation, where models exploit imperfect reward functions to produce unrealistic images that score high.


<details>
  <summary>Details</summary>
Motivation: Existing reward functions for RL-based text-to-image generation are imperfect proxies for human judgment, leading to reward hacking where models generate unrealistic or low-quality images that still achieve high reward scores.

Method: The authors systematically analyze reward hacking behaviors, identify artifact-prone images as a common failure mode, and propose a lightweight adaptive artifact reward model trained on a small curated dataset of artifact-free and artifact-containing samples.

Result: Experiments show that incorporating the artifact reward significantly improves visual realism and reduces reward hacking across multiple text-to-image RL setups, demonstrating effectiveness as a safeguard.

Conclusion: Lightweight reward augmentations can effectively serve as safeguards against reward hacking in RL-based text-to-image generation, improving alignment with true human preferences.
...
TL;DR: Proposes an uncertainty-guided framework for referring remote sensing image segmentation that uses pixel-wise uncertainty maps to adaptively control language fusion and refinement, addressing spatial non-uniformity in cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Existing referring remote sensing segmentation methods use uniform fusion and refinement across entire images, which introduces linguistic noise in clear regions while failing to provide sufficient disambiguation in confused areas due to extreme scale variations, dense distractors, and complex boundaries.

Method: Introduces a Referring Uncertainty Scorer (RUS) trained via online error-consistency supervision to predict spatial referential ambiguity. Uses this uncertainty map to guide two plug-and-play modules: Uncertainty-Gated Fusion (UGF) for adaptive language injection, and Uncertainty-Driven Local Refinement (UDLR) for focused boundary refinement.
...
Conclusion: The uncertainty-guided framework effectively addresses spatial non-uniformity in cross-modal alignment for referring remote sensing segmentation by adaptively controlling language fusion and refinement based on predicted uncertainty, enhancing performance in challenging scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Object hallucination remains a critical challenge in Large Vision-Language Models, and existing approaches overlook internal complexities of visual encoding. The paper identifies visual statistical bias from Vision Encoders' Bag-of-Patches behavior as a key contributor to hallucinations.

Method: Structure-Disrupted Contrastive Decoding (SDCD) - a training-free algorithm that introduces a shuffled structure-disrupted view to perform contrastive calibration of output distributions. It penalizes tokens that maintain high confidence under structure-less views to suppress texture-driven bias.
...
Conclusion: Addressing visual statistical bias through structure disruption effectively reduces object hallucinations in LVLMs, and SDCD provides a practical training-free solution for improving multimodal understanding.
...
TL;DR: Real-time facial expression tracking system using VR headset infrared cameras that enables users to drive virtual character expressions without calibration, trained via distillation on heterogeneous data from 18k subjects.


<details>
  <summary>Details</summary>
Motivation: To enable non-intrusive, real-time facial expression tracking for virtual characters in VR environments without requiring lengthy calibration steps, facilitating better communication and expression in virtual spaces.

Method: Distillation-based machine learning approach trained on heterogeneous data (synthetic and real images) from 18k subjects captured using mobile phone + custom VR headset with extra cameras, with a robust differentiable rendering pipeline for automatic facial expression label extraction.

Result: Developed a system that accurately tracks facial expressions in real-time using egocentric infrared camera views, enabling users to drive virtual character expressions without calibration, with applications in video conferencing, gaming, and remote collaboration.

Conclusion: The system opens new avenues for communication and expression in virtual environments by providing accurate, non-intrusive facial expression tracking without calibration requirements, with broad applications across various domains.
...
<details>
  <summary>Details</summary>
Motivation: Sparse and irregular point distributions in point clouds provide limited appearance evidence, making geometry-only features insufficient to distinguish objects with similar shapes but different appearances (color, texture, material). Existing methods struggle with geometric ambiguity.
...
Result: Achieves superior performance on standard benchmarks and shows significant improvements on geometrically challenging classes. Works without any 2D or language supervision.

Conclusion: G2P enables more discriminative and appearance-consistent segmentation of point clouds by effectively transferring appearance-aware attributes from 3D Gaussian Splatting, overcoming limitations of geometry-only features.
...
TL;DR: SBWM reframes human motion prediction as latent dynamical simulation on the human body manifold using a belief-state world model, enabling stable long-horizon rollouts with competitive accuracy at lower computational cost.
...
Method: Proposes Semantic Belief-State World Model (SBWM) that maintains a recurrent probabilistic belief state whose evolution is learned independently of pose reconstruction. The latent state is explicitly aligned with SMPL-X anatomical parameterization, creating a structural information bottleneck that forces it to capture motion dynamics, intent, and control-relevant structure rather than static geometry or noise. Adapts stochastic latent transitions and rollout-centric training from model-based reinforcement learning.

Result: Demonstrates coherent long-horizon rollouts and competitive accuracy at substantially lower computational cost compared to RSSM-based, transformer, and diffusion approaches. The method shows stable forward simulation capabilities.

Conclusion: Treating the human body as part of the world model's state space rather than its output fundamentally changes how motion is simulated and predicted, enabling more stable and reliable long-term motion forecasting.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods compress optical features for cross-modal alignment, causing loss of high-frequency information and introducing physically inconsistent artifacts (texture distortions, edge blurring) by overlooking differences in imaging physics between optical and thermal modalities.

Method: Proposes PCNet with three key components: 1) Cross-Resolution Mutual Enhancement Module (CRME) for joint optimization of thermal super-resolution and optical-to-thermal conversion with bidirectional feature interaction; 2) Physics-Driven Thermal Conduction Module (PDTM) incorporating 2D heat conduction into optical guidance with spatially-varying properties; 3) Temperature consistency loss enforcing regional distribution consistency and boundary gradient smoothness.
...
Conclusion: PCNet enables robust thermal UAV image super-resolution through cross-resolution mutual enhancement and physics-constrained optical guidance, addressing limitations of existing methods while maintaining physical consistency with real-world thermal radiation principles.
...
TL;DR: CloudMatch is a semi-supervised framework for cloud detection that uses view-consistency learning with scene-mixing augmentations to leverage unlabeled remote sensing imagery.


<details>
  <summary>Details</summary>
Motivation: Pixel-level annotation for cloud detection is expensive, creating a need for semi-supervised approaches that can effectively utilize unlabeled remote sensing data.

Method: Generates three augmented views per unlabeled image: one weakly augmented view plus two strongly augmented views using inter-scene mixing (for contextual variety) and intra-scene mixing (for semantic coherence). Enforces prediction consistency across these views to guide pseudolabel generation and capture cloud pattern diversity.
...
TL;DR: EASLT is an emotion-aware sign language translation framework that treats facial expressions as semantic anchors to resolve ambiguities when distinct concepts share identical manual gestures.


<details>
  <summary>Details</summary>
Motivation: Current gloss-free SLT methods focus on manual signals but overlook facial expressions, causing semantic ambiguity when different concepts have the same manual articulations.

Method: EASLT uses an emotional encoder to capture continuous affective dynamics and integrates them via an Emotion-Aware Fusion module that adaptively recalibrates spatio-temporal sign features based on affective context.
...
TL;DR: SpatiaLoc is a coarse-to-fine framework for cross-modal localization using text and point clouds that emphasizes spatial relationships at instance and global levels, achieving SOTA results on KITTI360Pose.


<details>
  <summary>Details</summary>
Motivation: Cross-modal localization using text and point clouds enables robots to localize via natural language descriptions for autonomous navigation and human-robot interaction. Since objects often recur across modalities, spatial relationships become the most discriminative cues for accurate localization.
...
Conclusion: The proposed SpatiaLoc framework effectively leverages spatial relationships at both instance and global levels through a coarse-to-fine approach, achieving superior performance in cross-modal localization tasks using text and point clouds.
...
<details>
  <summary>Details</summary>
Motivation: AI-generated images pose misinformation risks, requiring detection methods that generalize to unseen generative models. Current CLIP-based approaches underutilize the image encoder's potential and lack true discrimination capability between real and AI-generated images.

Method: Masking-based Pre-trained model Fine-Tuning (MPFT) with Texture-Aware Masking (TAM) mechanism that masks textured areas containing generative model-specific patterns during fine-tuning, forcing CLIP-ViT to focus on distributional deviations from authentic images.
...
Conclusion: The proposed MPFT strategy effectively enhances CLIP-ViT's generalization capability for AI-generated image detection by focusing on distributional deviations, offering a practical solution with minimal training requirements.
...
TL;DR: SiT-Bench is a new benchmark that evaluates spatial intelligence in LLMs using textual scene descriptions instead of visual inputs, revealing a "spatial gap" in global consistency despite proficiency in localized tasks.


<details>
  <summary>Details</summary>
Motivation: To determine whether spatial understanding in Vision-Language Models originates from visual encoders or the fundamental reasoning backbone, and to evaluate LLMs' spatial intelligence without pixel-level inputs.

Method: Created SiT-Bench with 3,800+ expert-annotated items across 5 categories and 17 subtasks, converting single/multi-view scenes into high-fidelity, coordinate-aware textual descriptions to test symbolic textual reasoning.

Result: SOTA LLMs show proficiency in localized semantic tasks but exhibit a significant "spatial gap" in global consistency. Explicit spatial reasoning significantly boosts performance, suggesting LLMs have latent world-modeling potential.

Conclusion: SiT-Bench serves as a foundational resource to develop spatially-grounded LLM backbones for future VLMs and embodied agents, showing that spatial intelligence can be evaluated through textual reasoning without visual inputs.
...
TL;DR: The paper introduces an environment-robust few-shot segmentation setting and proposes Adaptive Attention Distillation (AAD) to improve model robustness in complex real-world scenarios with challenging environmental factors.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot segmentation models are trained under ideal laboratory conditions and fail to handle real-world environmental challenges like illumination changes, background complexity, motion blur, small objects, and camouflaged targets, limiting their practical deployment.

Method: Proposes Adaptive Attention Distillation (AAD) which repeatedly contrasts and distills key shared semantics between support and query images to derive class-specific attention for novel categories, enhancing focus on correct targets in complex environments.

Result: AAD improves mIoU by 3.3% - 8.5% across all datasets and settings in the proposed ER-FSS benchmark covering eight real-world scenario datasets, demonstrating superior performance and strong generalization.

Conclusion: The environment-robust FSS setting and AAD method effectively address the gap between laboratory training and practical deployment requirements, significantly improving model robustness in complex real-world environments.
...
TL;DR: A robust patching strategy and Attention U-Net model for binarizing challenging stone inscription images, with strong zero-shot generalization across scripts.


<details>
  <summary>Details</summary>
Motivation: Stone inscription images pose severe challenges for binarization due to poor contrast, non-uniform degradation, distracting artifacts, and variable text density/layouts, causing existing techniques to fail at isolating coherent character regions.
...
Conclusion: The method produces clean, structured representations of inscription content, laying foundation for downstream tasks like script identification, OCR, and historical text analysis. The approach shows promising script-agnostic generalization for challenging historical document binarization.
...
TL;DR: Monocular 3D detection using pseudo-LiDAR shows depth backbone choice (NeWCRFs vs Depth Anything) matters most, with geometric fidelity dominating over feature augmentation like semantics.
...
Method: Systematic evaluation comparing NeWCRFs (supervised metric depth) against Depth Anything V2 Metric-Outdoor under identical pseudo-LiDAR generation and PointRCNN detection protocol on KITTI validation. Tested point-cloud augmentations using appearance cues (grayscale intensity) and semantic cues (instance segmentation confidence).

Result: NeWCRFs yields stronger downstream 3D detection (10.50% AP3D at IoU=0.7 on Moderate split). Feature augmentations provide only marginal gains, and mask-based sampling can degrade performance. Depth-accuracy-versus-distance diagnostic shows coarse depth correctness doesn't fully predict strict 3D IoU.

Conclusion: Under off-the-shelf LiDAR detectors, depth-backbone choice and geometric fidelity dominate performance, outweighing secondary feature injection. Depth backbone quality is more critical than semantic feature augmentation for monocular pseudo-LiDAR pipelines.
...
TL;DR: Proposes a novel object classification method that eliminates the need for object alignment by sorting features, using normalized boundaries segmented into convex parts with sorted length-based features.


<details>
  <summary>Details</summary>
Motivation: Existing object classification techniques require object alignment to compute similarity between objects, which can be computationally expensive and may not always be feasible. The paper aims to develop a method that doesn't require alignment.

Method: Normalizes object boundaries, segments them into approximately convex segments, sorts segments by length in descending order, and extracts a bag of features including segment length, number of extreme points, area, base, and width to measure similarity between image boundaries.

Result: The proposed method was tested on datasets and produced acceptable classification results without requiring object alignment.

Conclusion: The method successfully eliminates the need for object alignment in classification by using sorted feature-based similarity measurement, providing a viable alternative to alignment-dependent approaches.
...
<details>
  <summary>Details</summary>
Motivation: Accurate high-resolution precipitation nowcasting from radar echoes is crucial for disaster mitigation but challenging due to complex multi-scale evolution, inter-frame feature misalignment from displacement, and difficulty capturing long-range spatiotemporal context without sacrificing spatial fidelity.

Method: Proposes MFC-RFNet with: 1) Wavelet-Guided Skip Connection (WGSC) to preserve high-frequency details, 2) Feature Communication Module (FCM) for bidirectional cross-scale interaction, 3) Condition-Guided Spatial Transform Fusion (CGSTF) to align shallow features using spatial transforms learned from conditioning echoes, 4) Rectified flow training for near-linear probability-flow trajectories enabling few-step sampling, and 5) Lightweight Vision-RWKV blocks at key positions to capture long-range dependencies efficiently.
...
TL;DR: CrackSegFlow is a controllable flow-matching framework that generates photorealistic crack images from binary masks, enabling synthetic data generation to address label scarcity and domain shift in pavement crack segmentation.


<details>
  <summary>Details</summary>
Motivation: Automated crack segmentation faces challenges due to scarce pixel-level labels and severe domain shift across different sensors, illumination conditions, textures, and annotation conventions, limiting practical deployment for infrastructure assessment.

Method: Uses two flow-matching models: one generates photorealistic crack images conditioned on binary masks with topology-preserving mask injection and boundary-gated modulation, while another synthesizes crack masks with explicit control over crack coverage. Also injects crack masks into crack-free backgrounds to diversify illumination and reduce false positives.

Result: Improves in-domain performance by 5.37 mIoU and 5.13 F1 on average, and cross-domain synthesis yields average gains of 13.12 mIoU and 14.82 F1. Provides faster deterministic sampling than diffusion methods and better fidelity for thin-structure geometry. Releases CSF-50K dataset with 50,000 paired images.

Conclusion: CrackSegFlow effectively addresses data scarcity and domain shift in crack segmentation through controllable synthetic data generation, demonstrating significant performance improvements across multiple benchmarks and releasing a large-scale public dataset for future research.
...
TL;DR: VideoMemory is an entity-centric framework for consistent narrative video generation that uses a Dynamic Memory Bank to maintain character, prop, and environment consistency across multiple shots and temporal gaps.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models produce high-quality short clips but fail to preserve entity identity and appearance when scenes change or when entities reappear after long temporal gaps, which is crucial for narrative video generation.

Method: VideoMemory integrates narrative planning with visual generation through a Dynamic Memory Bank. A multi-agent system decomposes scripts into shots, retrieves entity representations from memory, and synthesizes keyframes/videos conditioned on retrieved states. The Dynamic Memory Bank stores explicit visual and semantic descriptors for entities and updates after each shot to reflect story-driven changes while preserving identity.

Result: The authors created a 54-case multi-shot consistency benchmark covering character-, prop-, and background-persistent scenarios. Experiments show VideoMemory achieves strong entity-level coherence and high perceptual quality across diverse narrative sequences.

Conclusion: VideoMemory's retrieval-update mechanism enables consistent portrayal of entities across distant shots and supports coherent long-form narrative video generation, addressing a central challenge in maintaining entity consistency across multiple shots.
...
Method: Proposes MGPC framework with: 1) modality dropout strategy for robustness, 2) Transformer-based fusion module for scalability, 3) progressive generator for geometric modeling, and 4) automatic data generation pipeline creating MGPC-1M benchmark with 1M+ training pairs across 1,000+ categories.
...
Conclusion: MGPC successfully addresses generalization challenges in point cloud completion through multimodal integration and large-scale training, demonstrating superior performance in real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Current video generation models produce aesthetic videos but struggle with real-world physics dynamics, resulting in artifacts like unnatural collisions, inconsistent gravity, and temporal flickering.

Method: Proposes PhysVideoGenerator framework with PredictorP network that regresses physical features from V-JEPA 2 from diffusion latents, injecting physics tokens into temporal attention layers of DiT-based generator via cross-attention.

Result: Demonstrates technical feasibility: diffusion latents contain sufficient information to recover V-JEPA 2 physical representations, and multi-task optimization remains stable during training.
...
TL;DR: A novel egocentric action recognition method that uses 2D point tracks as motion cues, achieving improved accuracy without hand/object detection by tracking random image points with CoTracker and feeding trajectories to a Transformer model.


<details>
  <summary>Details</summary>
Motivation: Most existing egocentric action recognition methods rely on RGB appearance, human pose estimation, or their combination, but there's potential for improvement by incorporating additional motion information through 2D point tracking.

Method: Use CoTracker to follow randomly initialized points through video frames, then feed the resulting trajectories along with image frames to a Transformer-based recognition model. The approach doesn't detect hands, objects, or interaction regions.

Result: The method achieves notable performance gains even when only using the initial frame and its associated point tracks, without the full video sequence. Integration of 2D point tracks consistently enhances performance compared to models without motion information.

Conclusion: 2D point tracks serve as a lightweight yet effective representation for egocentric action understanding, demonstrating that tracking randomly sampled image points can substantially improve recognition accuracy without complex hand/object detection.
...
TL;DR: BREATH-VL is a hybrid framework that combines vision-language models with geometric registration for 6-DoF endoscopic camera localization, using the new BREATH dataset and lightweight temporal context learning to outperform vision-only methods.


<details>
  <summary>Details</summary>
Motivation: Address three key challenges in applying VLMs to endoscopic localization: 1) lack of large-scale medical localization datasets, 2) limited fine-grained pose regression capability, and 3) high computational latency for temporal feature extraction. The approach leverages complementary strengths - VLMs for semantic understanding and registration methods for geometric precision.

Method: 1) Construct BREATH dataset (largest in-vivo endoscopic localization dataset in human airways). 2) Propose BREATH-VL hybrid framework integrating VLM semantic cues with vision-based registration geometric information. 3) Introduce lightweight context-learning mechanism that encodes motion history as linguistic prompts for efficient temporal reasoning.

Result: BREATH-VL outperforms state-of-the-art vision-only localization methods in accuracy and generalization, reducing translational error by 25.5% compared to best-performing baseline while achieving competitive computational latency. The vision-language module demonstrates robust semantic localization in challenging surgical scenes.

Conclusion: The hybrid approach combining semantic understanding from VLMs with geometric precision from registration methods effectively addresses endoscopic localization challenges, with the BREATH dataset and lightweight temporal learning enabling superior performance in medical settings.
...
TL;DR: CSMCIR is a unified representation framework for Composed Image Retrieval that addresses representation space fragmentation through symmetric architecture, multi-level prompting, and dynamic memory bank.
...
Result: Achieves state-of-the-art performance on four benchmark datasets with superior training efficiency. Ablation studies validate each component's effectiveness.

Conclusion: CSMCIR successfully addresses representation space fragmentation in CIR through unified symmetric architecture, establishing modal symmetry and enabling efficient query-target alignment.
...
TL;DR: MATANet is a novel model for fine-grained marine species classification that uses multi-context environmental attention and hierarchical taxonomy-aware learning to improve classification accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods for marine animal classification overlook contextual environmental interactions and insufficiently incorporate the hierarchical structure of marine biological taxonomy, limiting their effectiveness for fine-grained classification tasks.
...
Result: Experiments on FathomNet2025, FAIR1M, and LifeCLEF2015-Fish datasets demonstrate state-of-the-art performance, showing the effectiveness of incorporating environmental context and taxonomic hierarchy for marine species classification.

Conclusion: MATANet successfully addresses the limitations of existing methods by mimicking expert strategies that use both taxonomy and environmental context to interpret ambiguous features of underwater animals, leading to improved fine-grained marine species classification.
...
TL;DR: RadDiff is a multimodal AI system for radiologist-style comparative analysis of paired radiology studies, outperforming baseline methods on a new benchmark.


<details>
  <summary>Details</summary>
Motivation: Understanding differences between radiology image sets is critical for clinical insights and interpreting medical AI systems, but current methods lack systematic comparative reasoning capabilities.

Method: Builds on proposer-ranker framework with four innovations: medical knowledge injection via domain-adapted VLMs, multimodal reasoning integrating images with reports, iterative hypothesis refinement, and targeted visual search with region zooming.

Result: Achieves 47% accuracy on RadDiffBench (57 expert-validated study pairs), rising to 50% with ground-truth reports, significantly outperforming VisDiff baseline. Demonstrates versatility across COVID-19 comparison, racial subgroup analysis, and survival feature discovery.

Conclusion: RadDiff and RadDiffBench provide the first method-and-benchmark foundation for systematically uncovering meaningful differences in radiological data, advancing comparative reasoning in medical imaging.
...
TL;DR: The paper introduces HyperCOD, the first large-scale benchmark for hyperspectral camouflaged object detection (HCOD), and proposes HSC-SAM, an adaptation of Segment Anything Model for HCOD that bridges the modality gap between hyperspectral images and vision foundation models.


<details>
  <summary>Details</summary>
Motivation: RGB-based camouflaged object detection struggles in real-world scenarios with ambiguous color/texture cues. Hyperspectral imaging offers better spectral signatures but lacks dedicated large-scale benchmarks, critically hampering progress in HCOD research.

Method: Proposes HSC-SAM (HyperSpectral Camouflage-aware SAM) that reformulates hyperspectral images by decoupling them into: 1) a spatial map fed to SAM's image encoder, and 2) a spectral saliency map serving as an adaptive prompt. This translation bridges the modality gap between hyperspectral data and vision foundation models.

Result: HSC-SAM sets new state-of-the-art on the HyperCOD benchmark (350 high-resolution hyperspectral images) and generalizes robustly to other public HSI datasets. The HyperCOD dataset provides a challenging benchmark with complex scenarios.

Conclusion: HyperCOD is the first dedicated benchmark for HCOD, and HSC-SAM effectively adapts foundation models for hyperspectral camouflaged object detection. The dataset and baseline provide a robust foundation for future research in this emerging area.
...
TL;DR: I2E proposes a "Decompose-then-Action" paradigm for compositional image editing using object decomposition and physics-aware agents, outperforming existing methods on complex editing tasks.


<details>
  <summary>Details</summary>
Motivation: Existing text-guided image editing methods struggle with compositional editing requiring precise local control and multi-object spatial reasoning due to implicit coupling of planning/execution, lack of object-level control, and pixel-centric modeling limitations.

Method: I2E uses a "Decompose-then-Action" paradigm: 1) Decomposer transforms images into discrete, manipulable object layers, 2) Physics-aware Vision-Language-Action Agent parses complex instructions into atomic actions via Chain-of-Thought reasoning.
...
Conclusion: The proposed structured "Decompose-then-Action" paradigm effectively addresses limitations of pixel-centric editing by enabling object-level control and explicit planning for complex compositional image editing tasks.
...
TL;DR: Proposes MVP (Masked Video Prediction) post-training objective for VideoLLMs to enhance temporal reasoning and causal understanding by reconstructing masked video segments from distractors.
...
Method: 1) MVP objective: reconstruct masked continuous video segments from challenging distractors; 2) Scalable data synthesis pipeline to transform arbitrary video corpora into MVP samples; 3) Group Relative Policy Optimization (GRPO) with fine-grained reward function to enhance video context and temporal property understanding.
...
Conclusion: MVP effectively bridges the gap in temporal reasoning for VideoLLMs by providing explicit supervision for sequential logic and temporal context, improving models' ability to understand video dynamics and causality.
...
TL;DR: Using synthetic data generation techniques to train AI object detection models for agricultural applications where CAD models are unavailable, achieving good performance with 3D models from scanning/image-to-3D and finetuning on small real datasets.


<details>
  <summary>Details</summary>
Motivation: AI computer vision systems in manufacturing require large annotated datasets which are costly. While synthetic data from CAD models helps in manufacturing, agricultural applications lack readily available CAD models, making synthetic data generation challenging.

Method: Developed techniques to substitute CAD files for synthetic dataset creation. Used highly representative 3D models acquired through scanning or image-to-3D approaches. Trained AI object detection models to separate stones and potatoes in bin picking environments, with finetuning on small real datasets.

Result: Demonstrated that representative 3D models (from scanning/image-to-3D) can effectively generate synthetic data for object detection training. Finetuning on small real datasets significantly improves model performance, achieving similar results even with less representative models.

Conclusion: Synthetic data generation using 3D models (from scanning or image-to-3D) is viable for agricultural AI applications lacking CAD models. Finetuning with small real datasets enhances performance, making this approach practical for high-variance, low-volume agricultural environments.
...
TL;DR: LLMs can autonomously engineer optimal code transformations by learning from empirical performance feedback, reducing search space by 600x while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Current approaches to data-aware augmentation in code synthesis rely on heuristic design or brute-force methods, which are inefficient and don't leverage empirical performance feedback effectively.

Method: Fine-tune LLMs with Low-Rank Adaptation on a repository of 6,000+ empirically evaluated PyTorch augmentation functions annotated by downstream accuracy. Use pairwise performance ordering (better-worse transformations) for alignment without reinforcement learning or reward models.

Result: Achieves up to 600x fewer evaluated candidates than brute-force while maintaining competitive peak accuracy. Model internalizes semantic performance cues rather than memorizing syntax, and direct prompting outperforms Chain-of-Thought prompting.

Conclusion: LLMs can exhibit task-level reasoning through non-textual feedback loops, bypassing explicit symbolic rewards, enabling autonomous engineering of optimal code transformations.
...
TL;DR: EvalBlocks is a modular framework built on Snakemake for efficient, reproducible evaluation of medical imaging foundation models, streamlining experiment tracking and enabling faster iteration.


<details>
  <summary>Details</summary>
Motivation: Current evaluation workflows for medical imaging foundation models are ad-hoc, manual, slow, and error-prone, burdening researchers with tracking numerous experiments and design choices.

Method: Built on Snakemake, EvalBlocks provides a modular, plug-and-play framework supporting seamless integration of new datasets, foundation models, aggregation methods, and evaluation strategies with centralized tracking.

Result: Demonstrated on five state-of-the-art foundation models and three medical imaging classification tasks, the framework enables efficient caching, parallel execution, and reproducible experiments with a single command.

Conclusion: EvalBlocks streamlines model evaluation, allowing researchers to iterate faster and focus on model innovation rather than evaluation logistics, and is released as open source software.
...
TL;DR: IDESplat improves 3D Gaussian Splatting by using iterative depth probability boosting for more accurate Gaussian mean prediction, achieving better reconstruction quality with fewer parameters and better generalization.
...
Method: Proposes IDESplat with iterative depth probability boosting: 1) Depth Probability Boosting Unit (DPBU) integrates epipolar attention maps from cascaded warp operations multiplicatively, 2) Multiple DPBUs stacked for iterative depth estimation that progressively refines depth candidates, leading to accurate Gaussian means.

Result: Outperforms DepthSplat by 0.33 dB PSNR on RealEstate10K using only 10.7% parameters and 70% memory; improves by 2.95 dB PSNR on DTU in cross-dataset experiments; achieves state-of-the-art performance with real-time efficiency on RealEstate10K, ACID, and DL3DV.

Conclusion: IDESplat's iterative depth probability boosting approach effectively addresses the limitations of single-warp methods, enabling more stable and accurate Gaussian mean prediction for superior 3D reconstruction quality and strong generalization across datasets.
...
TL;DR: MDENeRF: Iterative framework that refines monocular depth estimates using NeRF depth information with uncertainty modeling and Bayesian fusion.
...
Method: Three-component framework: (1) initial monocular estimate for global structure, (2) NeRF trained on perturbed viewpoints with per-pixel uncertainty derived from volume rendering, (3) Bayesian fusion of noisy monocular and NeRF depths to iteratively inject high-frequency details.
...
TL;DR: SVL-DRL is a staged voxel-level deep reinforcement learning framework that achieves robust medical image segmentation under noisy annotations by treating each voxel as an autonomous agent that dynamically refines its state representation.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation requires high-quality annotations, but noisy labels are common due to complex organ structures and inter-annotator variability, which limits segmentation model performance. The authors are motivated by how human annotators can correct labeling errors using prior knowledge.

Method: Proposes an end-to-end Staged Voxel-Level Deep Reinforcement Learning (SVL-DRL) framework with: 1) Formulating noisy annotations as a voxel-dependent problem addressed through staged RL, 2) A voxel-level asynchronous advantage actor-critic (vA3C) module where each voxel is an autonomous agent that dynamically refines its state representation, 3) A novel action space and composite reward function combining Dice value and spatial continuity metrics.

Result: Achieves state-of-the-art performance on three public medical image datasets with average improvements of over 3% in both Dice and IoU scores under various experimental settings.

Conclusion: SVL-DRL provides an effective framework for robust medical image segmentation under noisy annotations by leveraging staged reinforcement learning with voxel-level agents that can dynamically correct labeling errors without manual intervention.
...
TL;DR: FLNet uses super-resolution to enhance Sentinel-2 satellite images from 10m to 3m resolution for improved flood crop damage assessment, achieving near-commercial high-res performance at lower cost.


<details>
  <summary>Details</summary>
Motivation: Traditional manual flood damage surveys are slow and biased, while current satellite methods face cloud cover and low resolution challenges. There's a need for rapid, accurate, cost-effective crop damage assessment for post-disaster agricultural management in flood-prone regions like India.

Method: FLNet is a deep learning architecture that first applies super-resolution to enhance Sentinel-2 satellite images from 10m to 3m spatial resolution, then classifies crop damage levels from the enhanced imagery.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning models for white blood cell morphology analysis act as black boxes, limiting clinical trust and adoption in leukemia diagnosis. There's a need for more interpretable models that can provide transparent morphological descriptions.

Method: Developed HemBLIP, a vision language model trained on a new dataset of 14k healthy and leukemic cells with expert-derived attribute captions. Used both full fine-tuning and LoRA-based parameter-efficient training, and benchmarked against MedGEMMA biomedical foundation model.
...
TL;DR: FocusUI is an efficient UI grounding framework that reduces visual token count while preserving positional continuity, achieving faster inference with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Current VLMs tokenize high-resolution UI screenshots into thousands of visual tokens, causing computational overhead and diluted attention, while humans focus on relevant regions.
...
Result: FocusUI-7B outperforms GUI-Actor-7B by 3.7% on ScreenSpot-Pro benchmark, maintains performance with only 30% token retention (3.2% drop), achieves 1.44x faster inference, and reduces GPU memory by 17%.

Conclusion: FocusUI effectively addresses UI grounding efficiency challenges by intelligently selecting relevant visual tokens while preserving positional information, enabling faster and more memory-efficient UI interaction systems.
...
### [60] [ResTok: Learning Hierarchical Residuals in 1D Visual Tokenizers for Autoregressive Image Generation](https://arxiv.org/abs/2601.03955)
*Xu Zhang,Cheng Da,Huan Yang,Kun Gai,Ming Lu,Zhan Ma*

Main category: cs.CV

TL;DR: ResTok is a 1D visual tokenizer that introduces hierarchical residual representations for autoregressive image generation, improving efficiency and quality by predicting entire levels of tokens at once.


<details>
  <summary>Details</summary>
Motivation: Current 1D visual tokenizers follow language modeling principles, treating visual data as flat sequential tokens and overlooking the hierarchical and residual network designs essential for vision models. This language-like formulation fails to capture key visual properties.

Method: Proposes Residual Tokenizer (ResTok) that builds hierarchical residuals for both image tokens and latent tokens. Uses progressive merging for cross-level feature fusion and introduces semantic residuals between hierarchies to prevent information overlap. Also develops a hierarchical AR generator that predicts entire levels of latent tokens at once to reduce sampling steps.

Result: Achieves gFID of 2.34 on ImageNet-256 with only 9 sampling steps, demonstrating significant improvement in AR image generation efficiency and quality. Cross-level bindings emerge naturally without explicit constraints.

Conclusion: Restoring hierarchical residual priors in visual tokenization significantly improves autoregressive image generation by better capturing visual properties, enhancing representational capacity, and accelerating the generation process through hierarchical prediction.
...
### [61] [FUSION: Full-Body Unified Motion Prior for Body and Hands via Diffusion](https://arxiv.org/abs/2601.03959)
*Enes Duran,Nikos Athanasiou,Muhammed Kocabas,Michael J. Black,Omid Taheri*

Main category: cs.CV

TL;DR: FUSION: First diffusion-based unconditional full-body motion prior that jointly models body and hand motion, addressing the gap in existing methods that either ignore hands or work only in constrained settings.


<details>
  <summary>Details</summary>
Motivation: Hands are essential for interaction and gesture, but existing motion synthesis methods either ignore hand motions entirely or generate full-body motions only for narrow tasks. There's a lack of large-scale datasets capturing diverse full-body motion with detailed hand articulation.

Method: Curate and unify existing hand motion datasets with large-scale body motion data to generate full-body sequences. Propose FUSION - a diffusion-based unconditional full-body motion prior that jointly models body and hand motion. For applications, develop an optimization pipeline that refines the latent space of the diffusion model.
...
Conclusion: FUSION enables precise control over hand motion while maintaining plausible full-body coordination, addressing the critical gap in full-body motion synthesis with detailed hand articulation. The model shows strong performance on benchmarks and practical applications.
...
### [62] [PosterVerse: A Full-Workflow Framework for Commercial-Grade Poster Generation with HTML-Based Scalable Typography](https://arxiv.org/abs/2601.03993)
*Junle Liu,Peirong Zhang,Yuyi Zhang,Pengyu Yan,Hui Zhou,Xinyue Zhou,Fengjun Guo,Lianwen Jin*

Main category: cs.CV

TL;DR: PosterVerse is a full-workflow commercial poster generation system that automates design through LLM-based blueprint creation, diffusion-based background generation, and MLLM-powered HTML rendering, using a novel HTML-based Chinese dataset called PosterDNA.


<details>
  <summary>Details</summary>
Motivation: Current automated poster generation systems have significant limitations: incomplete design workflows, poor text rendering accuracy, and insufficient flexibility for commercial applications. Commercial-grade posters require seamless integration of aesthetics with precise information delivery.

Method: Three-stage approach: (1) Fine-tuned LLMs create design blueprints from user requirements, (2) Customized diffusion models generate graphical backgrounds, (3) MLLM-powered HTML engine performs unified layout-text rendering for high accuracy and customization. Also introduces PosterDNA dataset with HTML typography files for training.

Result: PosterVerse consistently produces commercial-grade posters with appealing visuals, accurate text alignment, and customizable layouts. The PosterDNA dataset enables scalable text rendering and solves challenges of rendering small, high-density text.

Conclusion: PosterVerse is a promising solution for automating commercial poster design, offering a full workflow automation with high text accuracy and flexibility. The introduction of HTML-based dataset addresses fundamental text rendering challenges in poster generation.
...
TL;DR: REACT is a frame-level reward model that evaluates structural distortions in generative videos, trained with supervised fine-tuning and reinforcement learning on a large human-annotated dataset.
...
Method: 1) Construct large-scale human preference dataset with structural distortion taxonomy; 2) Use Chain-of-Thought synthesis for additional data; 3) Two-stage training: supervised fine-tuning with masked loss, then reinforcement learning with Group Relative Policy Optimization and pairwise rewards; 4) Dynamic sampling during inference to focus on distortion-prone frames.
...
Conclusion: REACT addresses the gap in structural distortion evaluation for generative videos, providing both quantitative assessment and interpretable analysis to improve overall video quality evaluation.
...
TL;DR: Annotation-efficient wind turbine blade segmentation using unsupervised region generation and binary classification instead of pixel-wise deep learning.


<details>
  <summary>Details</summary>
Motivation: Wind turbine inspections require accurate blade segmentation, but traditional pixel-wise deep learning methods demand extensive annotated datasets, creating scalability challenges for practical deployment.

Method: Reframes segmentation as binary region classification using unsupervised Modular Adaptive Region Growing with Adaptive Thresholding and Region Merging, plus RegionMix augmentation for improved generalization.

Result: Achieves state-of-the-art segmentation accuracy and strong cross-site generalization by consistently segmenting turbine blades across different windfarms.

Conclusion: The proposed annotation-efficient approach enables reliable wind turbine blade segmentation without extensive labeled data, addressing scalability challenges for automated inspection systems.
...
TL;DR: LocalDPO is a novel post-training framework that aligns text-to-video diffusion models with human preferences using localized spatio-temporal region-level optimization, eliminating the need for external critic models or manual annotation.


<details>
  <summary>Details</summary>
Motivation: Existing DPO methods for text-to-video alignment are inefficient (relying on multi-sample ranking and task-specific critic models) and provide ambiguous global supervision, limiting their effectiveness for fine-grained video quality improvement.

Method: LocalDPO constructs localized preference pairs from real videos by treating high-quality videos as positives and generating negatives through local corruption with random spatio-temporal masks, then restoring only masked regions using the frozen base model. It uses a region-aware DPO loss that restricts preference learning to corrupted areas for rapid convergence.

Result: Experiments on Wan2.1 and CogVideoX show LocalDPO consistently improves video fidelity, temporal coherence, and human preference scores over other post-training approaches, establishing a more efficient and fine-grained paradigm for video generator alignment.

Conclusion: LocalDPO provides an efficient, fine-grained approach to aligning text-to-video diffusion models with human preferences through localized spatio-temporal optimization, outperforming existing methods while eliminating the need for external critic models or manual annotation.
...
TL;DR: LMMs suffer from "textual inertia" - once hallucinations occur in reasoning chains, they blindly follow erroneous text ignoring visual evidence. Proposed LogicGraph Perturbation Protocol reveals <10% self-correction rates. Introduced Active Visual-Context Refinement improves robustness.


<details>
  <summary>Details</summary>
Motivation: Large Multimodal Models show impressive video reasoning via Chain-of-Thought, but their reasoning chain robustness is questionable. The paper identifies "textual inertia" - models blindly adhere to erroneous textual hallucinations while neglecting conflicting visual evidence.
...
Result: Models successfully self-correct in less than 10% of cases, predominantly succumbing to blind textual error propagation. The proposed Active Visual-Context Refinement approach significantly stifles hallucination propagation and enhances reasoning robustness.

Conclusion: Current LMMs suffer from textual inertia where hallucinations propagate unchecked. The proposed LogicGraph evaluation reveals severe self-correction deficiencies. Active Visual-Context Refinement offers an effective training-free solution to improve reasoning robustness by enforcing visual verification and context refinement.
...
TL;DR: Gen3R bridges foundational 3D reconstruction models with video diffusion models for scene-level 3D generation, producing both RGB videos and corresponding 3D geometry through aligned latent spaces.


<details>
  <summary>Details</summary>
Motivation: To combine the strengths of foundational reconstruction models (strong geometric priors) with video diffusion models (strong appearance priors) for improved scene-level 3D generation, enabling mutual enhancement between reconstruction and generation.

Method: Repurposes VGGT reconstruction model to produce geometric latents by training an adapter on its tokens, which are regularized to align with appearance latents from pre-trained video diffusion models. Jointly generates disentangled yet aligned latents for both RGB videos and 3D geometry.

Result: Achieves state-of-the-art results in single- and multi-image conditioned 3D scene generation. Demonstrates enhanced reconstruction robustness by leveraging generative priors, showing mutual benefits of coupling reconstruction and generative models.

Conclusion: Gen3R successfully bridges reconstruction and generative models, producing high-quality 3D scenes with both appearance and geometry, while demonstrating that tight coupling between these model types provides mutual benefits for 3D generation tasks.
...
TL;DR: GeoReason framework improves RS-VLMs by synchronizing internal reasoning with final decisions to reduce logical hallucinations, using a two-stage training approach with a novel logical consistency reward.


<details>
  <summary>Details</summary>
Motivation: Current Remote Sensing Vision-Language Models suffer from logical hallucinations where correct answers come from flawed reasoning or positional shortcuts rather than proper spatial logic, undermining reliability in strategic spatial decision-making.

Method: 1) Construct GeoReason-Bench dataset with 4,000 reasoning trajectories from geometric primitives and expert knowledge. 2) Two-stage training: Supervised Knowledge Initialization for reasoning syntax and domain expertise, and Consistency-Aware Reinforcement Learning with Logical Consistency Reward using option permutation to penalize logical drift.
...
Conclusion: GeoReason successfully addresses logical hallucinations in RS-VLMs by synchronizing internal thinking with final decisions through logic-driven training, improving reliability for complex spatial reasoning tasks.
...
TL;DR: Proposes PIMC, a multimodal self-supervision approach using 2D recurrence plots from pixel vegetation time series combined with remote sensing imagery, outperforming SOTA on Earth observation tasks.


<details>
  <summary>Details</summary>
Motivation: Satellites generate massive Earth observation data (SITS), but most deep learning models process entire images or complete time series, missing pixel-level temporal patterns. Need more effective feature extraction from pixel-wise vegetation index time series.

Method: 1) Generate 2D recurrence plots from pixel-based vegetation index time series (NDVI, EVI, SAVI) instead of raw values. 2) Propose PIxel-wise Multimodal Contrastive (PIMC) - multimodal self-supervision that creates encoders using both 2D pixel time series representations and remote sensing imagery.

Result: Evaluated on PASTIS dataset (pixel-level forecasting/classification) and EuroSAT (land cover classification). 2D representations significantly enhance SITS feature extraction. Contrastive learning improves quality of both pixel time series and RSI representations. Outperforms SOTA methods on all downstream tasks.

Conclusion: Multimodal method establishes robust self-supervision framework for processing both SITS and RSI. 2D recurrence plots provide more informative representations than raw pixel values. PIMC effectively leverages complementary information from pixel time series and remote sensing imagery for Earth observation tasks.
...
TL;DR: Klear is a new audio-video joint generation system that addresses synchronization, alignment, and quality issues through architectural innovations, progressive training strategies, and a novel large-scale dataset with dense captions.
...
TL;DR: Diffusion-DRF: A differentiable reward flow method that uses frozen Vision-Language Models as training-free critics to optimize video diffusion models without human annotations or learned reward models.


<details>
  <summary>Details</summary>
Motivation: Current DPO methods for Text-to-Video generation rely on non-differentiable preference signals from human annotations or learned reward models, which makes training label-intensive, bias-prone, and susceptible to reward hacking and unstable training.

Method: Proposes Diffusion-DRF that directly backpropagates VLM feedback through diffusion denoising chains, converting logit-level responses into token-aware gradients. Uses automated aspect-structured prompting for multi-dimensional VLM feedback and gradient checkpointing for efficient updates through final denoising steps.
...
Conclusion: Diffusion-DRF provides an effective alternative to current DPO methods by leveraging frozen VLMs as training-free critics, addressing issues of label intensity, bias, and reward hacking in Text-to-Video generation optimization.
...
TL;DR: ToTMNet is a lightweight rPPG architecture that replaces temporal attention with an FFT-accelerated Toeplitz temporal mixing layer for efficient long-range temporal filtering with only 63k parameters.


<details>
  <summary>Details</summary>
Motivation: Recent deep models for remote photoplethysmography (rPPG) increase computational cost and parameter count, and attention-based temporal modeling introduces quadratic scaling with temporal length, making them inefficient for practical applications.

Method: Proposes ToTMNet with a compact gated temporal mixer combining local depthwise temporal convolution with global Toeplitz mixing. The Toeplitz operator provides full-sequence temporal receptive field with linear parameters and near-linear time computation using circulant embedding and FFT-based convolution.

Result: On UBFC-rPPG intra-dataset evaluation: 1.055 bpm MAE with Pearson correlation 0.996. In synthetic-to-real setting (SCAMPS to UBFC-rPPG): 1.582 bpm MAE with Pearson correlation 0.994. Ablation shows gating mechanism is crucial for effective global Toeplitz mixing, especially under domain shift.

Conclusion: Toeplitz-structured temporal mixing is a practical and efficient alternative to attention for rPPG, achieving strong heart-rate estimation accuracy with compact design (63k parameters). Main limitation is evaluation on only two datasets.
...
TL;DR: 2D image-based visual localization augmented with depth maps achieves state-of-the-art accuracy while maintaining easy map building and maintenance.
...
Method: Augment each image with estimated depth maps to capture geometric structure, use dense matchers effectively, employ compact compression and GPU-accelerated LO-RANSAC for efficiency.

Result: Achieves new state-of-the-art accuracy on various standard benchmarks, outperforms existing memory-efficient methods at comparable map sizes.

Conclusion: The proposed 2D+depth representation combines the best of both worlds: easy map building/maintenance with high accuracy and efficient storage/computation.
...
TL;DR: CHORD is a universal generative pipeline for synthesizing 4D (3D+time) dynamic scenes by distilling Lagrangian motion information from 2D videos, enabling category-agnostic generation of multi-body dynamics.


<details>
  <summary>Details</summary>
Motivation: Traditional rule-based graphics pipelines for creating 4D scene dynamics are labor-intensive and not scalable, while learning-based methods require large datasets that may not cover all object categories of interest.

Method: Proposes a distillation-based pipeline that extracts rich Lagrangian motion information from Eulerian representations of 2D videos, leveraging the universality of video generative models.
...
Conclusion: CHORD provides a universal, versatile, and category-agnostic approach to generating 4D dynamic scenes, overcoming limitations of traditional rule-based and learning-based methods.
...
### [75] [LCPs of Subspace Codes](https://arxiv.org/abs/2601.03489)
*Sanjit Bhowmick*

Main category: cs.IT

TL;DR: Introduces LCPs (linear complementary pairs) for subspace codes, provides characterization, existence conditions, constructions, and application to insertion error correction.


<details>
  <summary>Details</summary>
Motivation: Extend the concept of linear complementary pairs from classical linear codes to subspace codes, which are collections of subspaces, to explore their properties and applications in coding theory.

Method: 1. Characterize subspace codes that form LCPs; 2. Provide sufficient condition for LCP existence using complement functions; 3. Construct LCPs using various techniques; 4. Apply to insertion error correction.

Result: Established theoretical framework for LCPs of subspace codes, including characterization theorems, existence conditions, multiple construction methods, and demonstrated practical application in error correction.

Conclusion: Successfully generalized LCP concept to subspace codes, providing foundational theory, construction methods, and showing practical utility in error correction applications.
...
### [76] [Hermitian LCD $2$-Quasi Abelian Codes over Finite Chain Rings](https://arxiv.org/abs/2601.03492)
*Sanjit Bhowmick,Kuntal Deka*

Main category: cs.IT

TL;DR: The paper introduces Hermitian LCD 2-quasi-abelian codes over finite fields and finite chain rings, enumerates them with small relative minimum weights, and proves they are asymptotically good.


<details>
  <summary>Details</summary>
Motivation: To study and characterize a specific class of error-correcting codes (Hermitian LCD 2-quasi-abelian codes) that combine desirable properties: Hermitian duality, linear complementary dual (LCD) property, and quasi-abelian structure, with applications in coding theory over both finite fields and finite chain rings.

Method: 1. Introduce Hermitian LCD 2-quasi-abelian codes over finite fields
2. Provide comprehensive enumeration of these codes with small relative minimum weights
3. Extend analysis to finite chain rings by characterizing 2-quasi-abelian codes in this setting
4. Use mathematical proofs to establish asymptotic goodness

Result: 1. Hermitian LCD 2-quasi-abelian codes are asymptotically good over finite fields
2. Complete enumeration of such codes with small relative minimum weights
3. Characterization of 2-quasi-abelian codes over finite chain rings
4. Existence of asymptotically good Hermitian LCD 2-quasi-abelian codes over finite chain rings

Conclusion: The paper successfully establishes the asymptotic goodness of Hermitian LCD 2-quasi-abelian codes over both finite fields and finite chain rings, providing a comprehensive enumeration and characterization that contributes to the theory of error-correcting codes with desirable algebraic and combinatorial properties.
...
### [77] [Low-Complexity Planar Beyond-Diagonal RIS Architecture Design Using Graph Theory](https://arxiv.org/abs/2601.03831)
*Matteo Nerini,Zheyu Wu,Shanpu Shen,Bruno Clerckx*

Main category: cs.IT

TL;DR: The paper proposes using graph theory to identify planar-connected RIS architectures that can be fabricated on double-layer PCBs, finding those with maximum degrees of freedom for optimal performance under practical constraints.


<details>
  <summary>Details</summary>
Motivation: Beyond-diagonal RIS (BD-RIS) architectures offer greater flexibility than conventional RIS but require complex multi-layer PCB designs that increase fabrication difficulty. There's a need to identify BD-RIS architectures that can be realized on simpler double-layer PCBs while maintaining good performance.

Method: The authors use graph theory to characterize BD-RIS architectures that can be implemented on double-layer PCBs, classifying them as planar-connected RISs. They then identify which planar-connected RIS configurations offer the most degrees of freedom.

Result: The paper identifies specific planar-connected RIS architectures with the highest degrees of freedom among those that can be fabricated on double-layer PCBs, making them expected to achieve the best performance under practical fabrication constraints.

Conclusion: Graph theory provides a systematic approach to design BD-RIS architectures that balance performance and practical fabrication constraints, enabling the implementation of advanced RIS technology on simpler double-layer PCBs.
...
### [78] [Unique Decoding of Hyperderivative Reed-Solomon Codes](https://arxiv.org/abs/2601.03982)
*Haojie Gu,Jun Zhang*

Main category: cs.IT

TL;DR: Proposes a Welch-Berlekamp algorithm for unique decoding of Hyperderivative Reed-Solomon codes under the NRT metric.


<details>
  <summary>Details</summary>
Motivation: Error-correcting codes need efficient decoding algorithms to recover transmitted information from noisy channels. Hyperderivative Reed-Solomon codes with NRT metric require effective decoding methods.

Method: Develops a Welch-Berlekamp algorithm specifically adapted for unique decoding of NRT Hyperderivative Reed-Solomon codes.

Result: Presents a decoding algorithm that can efficiently decode HRS codes with respect to the NRT metric, providing a solution to the fundamental decoding problem.

Conclusion: The proposed Welch-Berlekamp algorithm addresses the decoding challenge for NRT HRS codes, contributing to more efficient error correction in coding theory.
...
### [79] [Flexible-Duplex Cell-Free Architecture for Secure Uplink Communications in Low-Altitude Wireless Networks](https://arxiv.org/abs/2601.04011)
*Wei Shi,Wei Xu,Yongming Huang,Jiacheng Yao,Wenhao Hu,Dongming Wang*

Main category: cs.IT

TL;DR: Proposes flexible-duplex cell-free architecture for secure UAV uplink in 6G networks, using APs that dynamically switch between receiving UAV signals or transmitting artificial noise to combat eavesdropping.


<details>
  <summary>Details</summary>
Motivation: UAV uplink transmissions in low-altitude wireless networks (LAWNs) are vulnerable to eavesdropping due to limited transmit power, constrained antenna resources, and exposed air-ground propagation conditions in future 6G infrastructures.

Method: Flexible-duplex cell-free architecture where distributed APs can dynamically operate as either receive APs for UAV uplink collection or transmit APs generating cooperative artificial noise. Joint optimization of AP mode selection, receive combining, and AN covariance design using penalty dual decomposition algorithm with guaranteed convergence.

Result: Substantial secrecy-rate gains over CF systems with fixed AP roles. Joint optimization achieves highest secrecy performance, while low-complexity approach attains over 90% of optimal performance with order-of-magnitude lower computational complexity.

Conclusion: Flexible-duplex cell-free architecture offers practical solution for secure uplink communications in LAWNs, addressing fundamental security bottleneck in UAV transmissions through distributed and adaptive protection against wiretapping.
...
### [80] [Serving Every Symbol: All-Symbol PIR and Batch Codes](https://arxiv.org/abs/2601.04041)
*Avital Boruchovsky,Anina Gruica,Jonathan Niemann,Eitan Yaakobi*

Main category: cs.IT

TL;DR: The paper studies t-all-symbol PIR/batch codes, determines minimum code lengths for small k,t, characterizes optimal codes, derives trade-off bounds, and analyzes MDS/simplex codes within this framework.


<details>
  <summary>Details</summary>
Motivation: To unify and extend several code families (one-step majority-logic decodable codes, PIR codes, batch codes) through the framework of t-all-symbol PIR/batch codes, and to determine fundamental limits and structural properties of these codes.

Method: Mathematical analysis of linear codes with specific recovery properties, determining minimum code lengths through combinatorial and algebraic methods, characterizing structural properties of optimal codes, deriving bounds on parameters, and studying classical code families (MDS, simplex) within the framework.

Result: Determined minimum code lengths for small k and t, characterized structural properties of optimal codes, derived bounds showing trade-offs between length, dimension, minimum distance, and t, and established new cases of an open conjecture about the simplex code as a t-functional batch code.

Conclusion: The framework successfully unifies multiple code families, provides fundamental limits for t-all-symbol PIR/batch codes, reveals structural insights about optimal codes, and advances understanding of classical codes within this unified perspective, including progress on an open conjecture.
...
### [81] [Expectation Propagation for Distributed Inference in Grant-Free Cell-Free Massive MIMO](https://arxiv.org/abs/2601.04166)
*Christian Forsch,Laura Cottatellucci*

Main category: cs.IT

TL;DR: Proposes two distributed expectation propagation algorithms for joint activity detection, channel estimation, and data detection in grant-free cell-free massive MIMO systems to address pilot contamination and enable scalable IoT connectivity.


<details>
  <summary>Details</summary>
Motivation: Grant-free cell-free massive MIMO systems enable massive IoT connectivity without scheduling but suffer from severe pilot contamination due to non-orthogonal pilots, degrading channel estimation and data detection. Scalable networks require distributed processing solutions.

Method: Two distributed expectation propagation algorithms: JACD-EP using Gaussian approximations for channels, and JACD-EP-BG modeling channels as Bernoulli-Gaussian variables. Both use exponential family representation and message passing over factor graphs from posterior distributions.

Result: Simulations show efficient pilot contamination mitigation and superior detection accuracy compared to genie-aided centralized linear detectors. Framework is scalable with respect to number of access points and user equipments.

Conclusion: The proposed distributed EP algorithms effectively address pilot contamination in GF-CF-MaMIMO systems, enabling scalable massive IoT connectivity with improved joint activity detection, channel estimation, and data detection performance.
...
### [82] [A discrete Benamou-Brenier formulation of Optimal Transport on graphs](https://arxiv.org/abs/2601.04193)
*Kieran Morris,Oliver Johnson*

Main category: cs.IT

TL;DR: Proposes discrete transport equation on graphs connecting vertex and edge distributions, derives discrete Benamou-Brenier formulation for Wasserstein-1 distance, and classifies all W1 geodesics on graphs.


<details>
  <summary>Details</summary>
Motivation: To develop a discrete analogue of continuous optimal transport theory on graphs, enabling analysis of Wasserstein distances and geodesics in graph settings.

Method: Proposes discrete transport equation connecting vertex and edge distributions, then derives discrete version of Benamou-Brenier formulation for Wasserstein-1 distance on graphs.

Result: Successfully establishes discrete transport framework and classifies all Wasserstein-1 geodesics on graphs through the derived discrete Benamou-Brenier formulation.

Conclusion: Provides complete discrete optimal transport theory for graphs with explicit characterization of W1 geodesics, bridging continuous and discrete transport frameworks.
...
### [83] [Foundation Model-Aided Hierarchical Control for Robust RIS-Assisted Near-Field Communications](https://arxiv.org/abs/2601.03427)
*Mohammad Ghassemi,Han Zhang,Ali Afana,Akram Bin Sediq,Melike Erol-Kantarci*

Main category: eess.SP

TL;DR: Dual-transformer hierarchical deep reinforcement learning framework for near-field communication with extremely large aperture arrays, integrating fast CSI estimation and blockage prediction to optimize spectral efficiency.


<details>
  <summary>Details</summary>
Motivation: 6G networks with extremely large aperture arrays shift communication to near-field regime with spherical wave propagation. RIS can support NFC beamfocusing, but effective utilization requires addressing CSI estimation (millisecond timescale) and blockage mitigation (second timescale) simultaneously, which conventional single-level control algorithms cannot handle.

Method: Proposes DT-HDRL framework: dual-transformer hierarchical deep reinforcement learning. Fast-timescale transformer processes ray-tracing data for rapid CSI estimation; vision transformer analyzes visual data for blockage prediction. Hierarchical controller: high-level selects LoS/RIS-assisted NLoS paths and sets goals; low-level optimizes BS beamfocusing and RIS phase shifts using instantaneous CSI.

Result: Approximately 18% improvement in spectral efficiency compared to single-timescale baselines. Blockage predictor achieves F1-score of 0.92 with 769 ms advance warning window in dynamic scenarios.

Conclusion: The proposed DT-HDRL framework effectively addresses dual-timescale challenges in near-field communication with ELAAs, integrating fast CSI estimation and proactive blockage prediction to maximize spectral efficiency while ensuring robust performance under dynamic conditions.
...
### [84] [Energy Harvesting in High Altitude Platform Station Enabled Sensor Networks](https://arxiv.org/abs/2601.03446)
*Melek Tuylu,Eylem Erdogan*

Main category: eess.SP

TL;DR: HAPS systems face energy challenges in VHetNets; proposed energy harvesting between HAPS systems enables sustainable operation with validated performance metrics.


<details>
  <summary>Details</summary>
Motivation: HAPS systems are crucial for future wireless networks but face significant energy constraints when integrated into vertical heterogeneous networks (VHetNets), limiting their operational viability.

Method: Proposed an energy harvesting strategy where one HAPS system harvests energy from another energy-unconstrained HAPS system, with performance analysis using outage probability, ergodic capacity, and Monte Carlo simulations.

Result: The energy harvesting approach effectively meets HAPS energy demands, with performance metrics validated through simulations showing viable system operation.

Conclusion: Energy harvesting between HAPS systems presents a practical solution to overcome energy constraints in VHetNets, enabling sustainable HAPS operation for future wireless networks.
...
### [85] [Intensity Fluctuation Dynamics in XPM](https://arxiv.org/abs/2601.03527)
*Ravneel Prasad,Emanuele Viterbo*

Main category: eess.SP

TL;DR: Enhanced XPM model incorporating frequency-domain intensity fluctuation growth improves characterization of nonlinear impairments in WDM systems, enabling accurate BER prediction.


<details>
  <summary>Details</summary>
Motivation: Cross-Phase Modulation (XPM) is a critical nonlinear impairment in high-capacity WDM systems, driven by intensity fluctuations that evolve due to chromatic dispersion. Prior models focused mainly on temporal pulse deformation, lacking comprehensive frequency-domain analysis of intensity fluctuation growth.

Method: Developed an enhanced XPM model that explicitly incorporates frequency-domain intensity fluctuation growth along the fiber. Established direct correlation between this frequency-domain growth and XPM-induced phase distortions, validated through simulations across various system parameters.

Result: Intensity fluctuation evolution, particularly at lower frequencies, profoundly affects XPM phase fluctuation spectra and phase variance. The model accurately predicts these spectral characteristics and enables accurate prediction of system performance in terms of Bit Error Ratio (BER).

Conclusion: Modeling frequency-domain intensity fluctuation evolution is essential for accurately characterizing XPM impairments. The findings provide guidance for designing advanced optical networks with improved performance prediction capabilities.
...
### [86] [F$^4$-CKM: Learning Channel Knowledge Map with Radio Frequency Radiance Field Rendering](https://arxiv.org/abs/2601.03601)
*Kequan Zhou,Guangyi Zhang,Hanlei Li,Yunlong Cai,Shengli Liu,Guanding Yu*

Main category: eess.SP

TL;DR: F⁴-CKM: A novel channel knowledge map construction framework for 6G using radiance field rendering techniques with spatial-frequency awareness, location-free usage, and fast learning capabilities.


<details>
  <summary>Details</summary>
Motivation: In 6G mobile communications, acquiring accurate and timely channel state information (CSI) is challenging due to growing antenna arrays and bandwidth. CKM offers promise but effective construction methods remain an open issue.

Method: Proposes F⁴-CKM framework with four features: radiance Field rendering, spatial-Frequency-awareness, location-Free usage, and Fast learning. Uses Wireless Radiator Representation (WiRARE) network to adapt radiance field rendering to RF domain, plus shaping filter module and angular sampling strategy.

Result: Extensive experiments show F⁴-CKM significantly outperforms existing baselines in wireless channel prediction accuracy and efficiency.

Conclusion: F⁴-CKM provides an effective solution for CKM construction in 6G networks by leveraging computer vision techniques adapted to the wireless domain, addressing CSI feedback challenges.
...
### [87] [Zak-OTFS ISAC with Bistatic Sensing via Semi-Blind Atomic Norm Denoising Scheme](https://arxiv.org/abs/2601.03639)
*Kecheng Zhang,Weijie Yuan,Maria Sabrina Greco*

Main category: eess.SP

TL;DR: Semi-blind atomic norm denoising scheme for Zak-OTFS ISAC with bistatic sensing achieves super-resolution channel estimation in high-mobility doubly dispersive environments.


<details>
  <summary>Details</summary>
Motivation: Accurate bistatic sensing and robust communication in high-mobility ISAC scenarios require precise channel estimation, which is challenging in doubly dispersive environments where fractional delay-Doppler shifts cause severe channel spreading.

Method: Proposes a semi-blind atomic norm denoising scheme: 1) Derives discrete-time I/O relationship of Zak-OTFS under fractional delay-Doppler shifts, 2) Formulates joint channel estimation and data detection as atomic norm denoising problem using negative square penalty for constellation constraints, 3) Develops accelerated iterative algorithm combining majorization-minimization, accelerated projected gradient, and inexact accelerated proximal gradient methods.

Result: Simulation results show the scheme achieves super-resolution sensing accuracy and communication performance approaching the perfect channel state information lower bound.

Conclusion: The proposed semi-blind atomic norm denoising scheme effectively addresses the channel estimation challenge in Zak-OTFS ISAC for high-mobility scenarios, enabling accurate bistatic sensing and robust communication in doubly dispersive environments.
...
### [88] [Cramer-Rao Bound for Angle of Arrival Estimates in True-Time-Delay Systems](https://arxiv.org/abs/2601.03735)
*Carl Collmann,Ahmad Nimr,Gerhard Fettweis*

Main category: eess.SP

TL;DR: This paper derives the Cramér-Rao Bound (CRB) for angle estimation in True-Time-Delay (TTD) systems used in joint communication and sensing, analyzes Fisher information properties, and evaluates angle estimation methods including ML estimators for uniform linear arrays.


<details>
  <summary>Details</summary>
Motivation: TTD systems have gained attention for fast beam training and beam squinting mitigation in joint communication and sensing. Accurate parameter estimates (like AoA) are crucial for initial access, interference mitigation, user localization, environment monitoring, and MIMO system synchronization.

Method: The paper derives the CRB for angle estimates in typical TTD systems, investigates properties of the CRB and Fisher information, and numerically evaluates these properties. It then applies methods like Maximum Likelihood (ML) and established estimators to solve the angle estimation problem using a uniform linear array.

Result: The paper provides theoretical derivation of CRB for TTD systems, analyzes Fisher information properties, and demonstrates numerical evaluation of angle estimation performance using ML and other established estimators for uniform linear arrays.

Conclusion: The work establishes fundamental performance bounds for angle estimation in TTD systems, providing theoretical foundations and practical methods for accurate parameter estimation in joint communication and sensing applications.
...
### [89] [Two-stage Multi-beam Training for Multiuser Millimeter-Wave Communications](https://arxiv.org/abs/2601.03745)
*Weijia Wang,Changsheng You,Xiaodan Shao,Rui Zhang*

Main category: eess.SP

TL;DR: Proposes a two-stage multi-beam training method for mmWave systems using sparse then dense subarrays to balance training overhead and beam identification accuracy.


<details>
  <summary>Details</summary>
Motivation: Conventional single-beam training uses exhaustive search which is inefficient. Multi-beam training faces trade-off between training overhead and success rate, exacerbated by inter-beam interference in mmWave systems.

Method: Two-stage approach: 1) Divide antenna array into sparse subarrays to generate multiple beams with high array gains for identifying candidate user angles. 2) Redivide array into dense subarrays to generate flexibly steered wide beams, using cross-validation to resolve angular ambiguity from first stage.

Result: Numerical results show the proposed method significantly improves success beam-identification rate compared to existing multi-beam training methods, while maintaining or even reducing required beam training overhead.

Conclusion: The two-stage multi-beam training method effectively balances the trade-off between training overhead and beam identification accuracy in multiuser mmWave systems, outperforming existing approaches.
...
### [90] [CSI-MAE: A Masked Autoencoder-based Channel Foundation Model](https://arxiv.org/abs/2601.03789)
*Jun Jiang,Xiaolong Ruan,Shugong Xu*

Main category: eess.SP

TL;DR: CSI-MAE is a generalized Channel Foundation Model using masked autoencoder for cross-scenario wireless channel adaptation, achieving SOTA performance with efficient fine-tuning and strong zero-shot transferability.


<details>
  <summary>Details</summary>
Motivation: Existing Channel Foundation Models have limitations: heavy reliance on scenario-specific data, focus on single/dual tasks, and lack zero-shot learning ability, hindering generalization across diverse wireless settings.

Method: Proposes CSI-MAE, a masked autoencoder-based CFM trained on 3GPP channel model datasets, integrating sensing and communication via CSI perception and generation, with lightweight decoder fine-tuning strategy.

Result: CSI-MAE matches or surpasses supervised models with lightweight fine-tuning, achieves state-of-the-art performance with full-parameter fine-tuning, and demonstrates exceptional zero-shot transferability rivaling supervised techniques in cross-scenario applications.

Conclusion: CSI-MAE addresses key limitations of existing CFMs, enabling cross-scenario generalization, multi-task capability, and zero-shot learning, driving innovation in wireless communication through self-supervised learning.
...
### [91] [ASVspoof 5: Evaluation of Spoofing, Deepfake, and Adversarial Attack Detection Using Crowdsourced Speech](https://arxiv.org/abs/2601.03944)
...
TL;DR: ASVspoof 5 challenge results show speech spoofing detection systems perform well but degrade under adversarial attacks and neural encoding/compression, with 53 teams participating using a new crowdsourced database.


<details>
  <summary>Details</summary>
Motivation: To advance speech spoofing and deepfake detection research through a challenge that evaluates detection solutions against a diverse, realistic database collected from many speakers under various recording conditions.

Method: Organized the 5th ASVspoof challenge with a new crowdsourced database from many speakers under diverse recording conditions, using both cutting-edge and legacy generative speech technology. Analyzed submissions from 53 participating teams and conducted post-challenge analysis including calibration studies.

Result: Many solutions performed well overall, but performance degraded significantly under adversarial attacks and neural encoding/compression schemes. The challenge revealed vulnerabilities in current detection systems against more sophisticated attack scenarios.

Conclusion: The paper outlines a roadmap for future ASVspoof challenges, highlighting the need for more robust detection systems that can withstand adversarial attacks and encoding variations, while providing insights into calibration and other principal challenges in speech spoofing detection.
...
### [92] [GeoDiff-SAR: A Geometric Prior Guided Diffusion Model for SAR Image Generation](https://arxiv.org/abs/2601.03499)
*Fan Zhang,Xuanting Wu,Fei Ma,Qiang Yin,Yuxin Hu*

Main category: eess.IV

TL;DR: GeoDiff-SAR is a physics-guided diffusion model that generates high-fidelity SAR images by incorporating geometric priors and multi-modal information fusion, improving generation quality and azimuth angle control.


<details>
  <summary>Details</summary>
Motivation: Existing SAR image generation methods operate primarily in the image domain without explicit geometric information, leading to poor generation quality and inability to control critical parameters like azimuth angles.

Method: 1) Calculate SAR point clouds at specific azimuths to simulate geometric structures; 2) Use FiLM-based feature fusion gating network to dynamically regulate 3D physical, image control, and textual parameters; 3) Apply LoRA architecture for lightweight fine-tuning of Stable Diffusion 3.5 for SAR domain adaptation.

Result: Extensive experiments on real SAR datasets show GeoDiff-SAR generates high-fidelity data that enhances downstream classification accuracy, particularly improving recognition performance across different azimuth angles.

Conclusion: Physics-guided generation with geometric priors significantly improves SAR image generation quality and parameter control, demonstrating the superiority of incorporating physical information into generative models.
...
### [93] [Ensemble Models for Predicting Treatment Response in Pediatric Low-Grade Glioma Managed with Chemotherapy](https://arxiv.org/abs/2601.03899)
*Max Bengtsson,Elif Keles,Angela J. Waanders,Ulas Bagci*

Main category: eess.IV

TL;DR: A novel ensemble pipeline combining Swin UNETR segmentation with XGBoost classification predicts chemotherapy response in pediatric brain tumors using pre-treatment MRI and clinical data.


<details>
  <summary>Details</summary>
Motivation: Pediatric brain tumors not amenable to complete surgical resection have lower progression-free survival, and chemotherapy remains the primary treatment option. There's a need for non-invasive methods to predict chemotherapy response for personalized therapy in this challenging population.

Method: Integrates pediatric brain tumor segmentation framework with radiomic feature extraction and clinical data through an ensemble of Swin UNETR encoder and XGBoost classifier. Segmentation delineates four tumor subregions, then Swin UNETR classifies response from segmented MRI scans while XGBoost uses radiomics and clinical variables.
...
Conclusion: The ensemble framework represents a promising step toward personalized therapy response prediction for pediatric low-grade glioma patients needing chemotherapy who are not suitable for complete surgical resection.
...
### [94] [A low-complexity method for efficient depth-guided image deblurring](https://arxiv.org/abs/2601.03924)
*Ziyao Yi,Diego Valsesia,Tiziano Bianchi,Enrico Magli*

Main category: eess.IV

TL;DR: A low-complexity neural network for depth-guided image deblurring using wavelet transform and efficient feature conditioning on depth maps from mobile Lidars.


<details>
  <summary>Details</summary>
Motivation: Image deblurring is highly ill-posed and while deep learning models have shown success, their high computational complexity makes them impractical for mobile/edge devices. Mobile Lidars provide complementary depth information that can enhance deblurring quality.

Method: Proposes a novel low-complexity neural network using wavelet transform to separate structural details and reduce spatial redundancy, combined with efficient feature conditioning on depth information from mobile Lidars.
...
Conclusion: Wavelet transform for detail separation/spatial redundancy reduction and efficient depth feature conditioning are essential for developing practical low-complexity depth-guided deblurring models suitable for mobile/edge deployment.
...
### [95] [Scanner-Induced Domain Shifts Undermine the Robustness of Pathology Foundation Models](https://arxiv.org/abs/2601.04163)
*Erik Thiringer,Fredrik K. Gustafsson,Kajsa Ledesma Eriksson,Mattias Rantalainen*

Main category: eess.IV

TL;DR: PFMs show poor robustness to scanner variability despite strong benchmark performance, with scanner-dependent bias affecting embedding spaces and calibration, requiring new evaluation approaches.


<details>
  <summary>Details</summary>
Motivation: Despite strong benchmark performance, PFM robustness to real-world technical domain shifts (like scanner variability) remains poorly understood, which is critical for reliable clinical deployment.

Method: Systematically evaluated 14 PFMs using a multiscanner dataset of 384 breast cancer WSIs scanned on 5 devices, isolating scanner effects from biological/lab confounders. Used unsupervised embedding analyses and clinicopathological supervised prediction tasks.

Result: Current PFMs are not invariant to scanner-induced domain shifts - most encode pronounced scanner-specific variability. While AUC remains stable, scanner variability systematically alters embedding spaces and impacts calibration, creating scanner-dependent bias. Robustness not correlated with training data scale, model size, or recency.

Conclusion: PFM development needs to move beyond accuracy-centric benchmarks toward explicit evaluation and optimization of embedding stability and calibration under realistic acquisition variability for reliable clinical use.
...
### [96] [Edit2Restore:Few-Shot Image Restoration via Parameter-Efficient Adaptation of Pre-trained Editing Models](https://arxiv.org/abs/2601.03391)
...
<details>
  <summary>Details</summary>
Motivation: Traditional image restoration requires training specialized models on thousands of paired examples per degradation type, which is data-intensive and inefficient. The paper challenges this paradigm by leveraging existing pre-trained models.

Method: Fine-tune LoRA adapters on FLUX.1 Kontext (12B parameter flow matching model) using only 16-128 paired images per task. Use text prompts to specify restoration operations. Compare task-specific vs unified multi-task adapters, and analyze text encoder fine-tuning.

Result: The approach achieves high perceptual quality with dramatically reduced data requirements. A single unified LoRA adapter effectively handles multiple degradations (denoising, deraining, dehazing). While prioritizing perceptual quality over pixel metrics like PSNR/SSIM, it offers a compelling data-efficient alternative.

Conclusion: Pre-trained image editing models, when properly adapted with parameter-efficient fine-tuning, provide a promising data-efficient alternative to traditional restoration approaches, enabling few-shot, prompt-guided image enhancement.
...
Method: Comprehensive analysis of lightweight transformer variants (MobileBERT, TinyBERT, etc.), model compression techniques (quantization, pruning, distillation), hardware platforms (NVIDIA Jetson, Qualcomm, etc.), deployment frameworks, and systematic benchmarking across standard datasets.
...
Conclusion: Lightweight transformers are viable for edge deployment with proper optimization, achieving significant efficiency gains while maintaining accuracy. The paper provides practical deployment guidelines and identifies key optimization strategies for successful edge AI implementation.
...
### [98] [Why LLMs Aren't Scientists Yet: Lessons from Four Autonomous Research Attempts](https://arxiv.org/abs/2601.03315)
*Dhruv Trehan,Paras Chopra*

Main category: cs.LG

TL;DR: Four attempts to autonomously generate ML research papers using LLM agents; three failed, one succeeded and was accepted to a special AI-first-author venue, revealing six key failure modes in AI scientific systems.


<details>
  <summary>Details</summary>
Motivation: To explore the feasibility and challenges of using LLM agents to autonomously conduct complete scientific research workflows and generate publishable ML research papers.

Method: Case study of four end-to-end attempts using a pipeline of six LLM agents mapped to stages of the scientific workflow (problem formulation, literature review, experimental design, implementation, evaluation, writing).

Result: Three attempts failed during implementation or evaluation; one completed the pipeline and was accepted to Agents4Science 2025 (an experimental venue requiring AI systems as first authors), passing both human and multi-AI review. Identified six recurring failure modes in AI scientific systems.

Conclusion: The study reveals significant challenges in autonomous scientific research by AI, documents six key failure modes, proposes four design principles for more robust AI-scientist systems, and discusses implications for autonomous scientific discovery.
...
### [99] [Ratio-Variance Regularized Policy Optimization for Efficient LLM Fine-tuning](https://arxiv.org/abs/2601.03320)
*Yu Luo,Shuo Han,Yihan Hu,Dong Li,Jianye Hao*

Main category: cs.LG

TL;DR: R²VPO proposes a variance-constrained policy optimization method for RL-based LLM fine-tuning that replaces hard clipping with distributional constraints, enabling better gradient preservation and off-policy data reuse.


<details>
  <summary>Details</summary>
Motivation: Current on-policy RL methods (PPO, GRPO) use hard clipping which indiscriminately truncates gradients from valuable high-divergence actions, suppressing rare but informative "eureka moments" in reasoning. Hard clipping also makes stale data unusable, leading to severe sample inefficiency.

Method: R²VPO constrains the variance (second central moment) of the policy ratio instead of using hard clipping. This provides a principled, smooth relaxation that stabilizes updates while preserving gradient signals. It uses a primal-dual framework supporting stable on-policy learning and principled off-policy data reuse through dynamic reweighting of stale samples.

Result: R²VPO achieves superior asymptotic performance with average relative gains up to 17% over clipping-based baselines, while requiring ~50% fewer rollouts to reach convergence. Tested on DeepSeek-Distill-Qwen-1.5B and openPangu-Embedded series (1B, 7B) across mathematical reasoning benchmarks.

Conclusion: Ratio-variance control is a promising direction for improving both stability and data efficiency in RL-based LLM alignment, addressing fundamental limitations of hard clipping while enabling better gradient preservation and sample reuse.
...
### [100] [Aligning Findings with Diagnosis: A Self-Consistent Reinforcement Learning Framework for Trustworthy Radiology Reporting](https://arxiv.org/abs/2601.03321)
*Kun Zhao,Siyuan Dai,Pan Wang,Jifeng Song,Hui Ji,Chenghua Lin,Liang Zhan,Haoteng Tang*

Main category: cs.LG

TL;DR: A novel "Reason-then-Summarize" framework with Group Relative Policy Optimization for radiology report generation that reduces hallucinations and improves clinical accuracy.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs for radiology report generation suffer from architectural heterogeneity, factual hallucinations, and alignment issues between visual evidence and linguistic outputs. Existing methods have computational cost limitations or insufficient exploration capabilities.

Method: 1) Systematic evaluation to identify optimal vision encoder and LLM backbone configurations; 2) Novel "Reason-then-Summarize" architecture with think block (detailed findings) and answer block (structured disease labels); 3) Optimized via Group Relative Policy Optimization (GRPO) with multi-dimensional composite reward function that penalizes logical discrepancies.

Result: Achieves state-of-the-art performance on MIMIC-CXR benchmark in clinical efficacy metrics and significantly reduces hallucinations compared to strong supervised baselines.

Conclusion: The proposed framework effectively addresses hallucination issues in radiology report generation by enforcing logical consistency between detailed findings and final diagnoses through a structured two-stage approach with optimized reinforcement learning.
...
### [101] [HEEGNet: Hyperbolic Embeddings for EEG](https://arxiv.org/abs/2601.03322)
*Shanglin Li,Shiwen Chu,Okan Koç,Yi Ding,Qibin Zhao,Motoaki Kawanabe,Ziheng Chen*

Main category: cs.LG

TL;DR: HEEGNet is a hybrid hyperbolic network that captures hierarchical structure in EEG data to improve generalization across domains/subjects through hyperbolic embeddings and coarse-to-fine domain adaptation.


<details>
  <summary>Details</summary>
Motivation: EEG-based brain-computer interfaces suffer from poor generalization due to distribution shifts across domains (e.g., different subjects). Current Euclidean embeddings may not capture the hierarchical structure inherent in EEG data, which recent studies suggest encodes hierarchical cognitive processes.

Method: HEEGNet combines Euclidean and hyperbolic encoders in a hybrid architecture. It employs a novel coarse-to-fine domain adaptation strategy to learn domain-invariant hyperbolic embeddings that capture the hierarchical structure of EEG data.

Result: Extensive experiments on multiple public EEG datasets (visual evoked potentials, emotion recognition, and intracranial EEG) demonstrate that HEEGNet achieves state-of-the-art performance, showing improved generalization across domains.

Conclusion: EEG data exhibits hyperbolicity, and hyperbolic embeddings improve generalization. HEEGNet successfully captures hierarchical structure in EEG data and learns domain-invariant representations, making it a promising approach for robust EEG decoding in brain-computer interfaces.
...
### [102] [Extreme-value forest fire prediction A study of the Loss Function in an Ordinality Scheme](https://arxiv.org/abs/2601.03327)
*Nicolas Caron,Christophe Guyeux,Hassan Noura,Benjamin Aynes*

Main category: cs.LG

TL;DR: First ordinal classification framework for wildfire severity forecasting in France shows ordinal-aware loss functions improve prediction of extreme events, with Weighted Kappa Loss performing best, though rare events remain challenging due to data imbalance.


<details>
  <summary>Details</summary>
Motivation: Wildfires are highly imbalanced natural hazards with rare extreme events that are critical for operational decision-making but challenging to predict using conventional methods.

Method: Introduces ordinal classification framework for wildfire severity forecasting, comparing standard cross-entropy with ordinal-aware objectives including proposed probabilistic TDeGPD loss (truncated discrete exponentiated Generalized Pareto Distribution). Extensive benchmarking over multiple architectures with real operational data.

Result: Ordinal supervision substantially improves model performance over conventional approaches. Weighted Kappa Loss achieves best overall results with +0.1 IoU gain on most extreme severity classes while maintaining competitive calibration quality. However, performance remains limited for rarest events due to extremely low representation in dataset.

Conclusion: Highlights importance of integrating severity ordering, data imbalance considerations, and seasonality risk into wildfire forecasting systems. Future work will focus on incorporating seasonal dynamics and uncertainty information to improve reliability of extreme-event prediction.
...
### [103] [Attention mechanisms in neural networks](https://arxiv.org/abs/2601.03329)
*Hasi Hays*

Main category: cs.LG

TL;DR: Comprehensive mathematical treatment of attention mechanisms in neural networks, covering theory, implementation, applications across NLP, vision, multimodal tasks, and analysis of scaling, interpretability, and limitations.


<details>
  <summary>Details</summary>
Motivation: Attention mechanisms represent a fundamental paradigm shift in neural networks, enabling selective focus on relevant input portions. The monograph aims to provide rigorous mathematical understanding of their foundations, properties, and practical implementations across various domains.

Method: Comprehensive mathematical treatment including theoretical foundations, computational properties, and practical implementations. Examines applications in natural language processing (autoregressive transformers, bidirectional encoders, sequence-to-sequence translation), computer vision (Vision Transformers), and multimodal learning (cross-modal attention for vision-language tasks).
...
Conclusion: Attention mechanisms are versatile and powerful but face limitations including computational scalability, data efficiency, systematic generalization, and interpretability challenges. The monograph provides critical examination of these current limitations.
...
### [104] [LUT-KAN: Segment-wise LUT Quantization for Fast KAN Inference](https://arxiv.org/abs/2601.03332)
*Oleksandr Kuznetsov*

Main category: cs.LG

TL;DR: LUT-KAN introduces lookup-table compilation and quantization for KAN networks, achieving 10-12x faster CPU inference with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: KAN networks are accurate and interpretable but computationally expensive on CPU due to spline evaluations, and standard quantization methods don't apply well to their non-matrix-multiply architecture.

Method: Segment-wise lookup-table compilation with affine int8/uint8 quantization and linear interpolation, providing explicit inference contracts with boundary conventions and OOB policies.

Result: 12x faster inference under NumPy, 10x under Numba with F1 drop below 0.0002, memory overhead ~10x at L=64, evaluated across multiple LUT resolutions and quantization schemes.

Conclusion: LUT-KAN enables efficient CPU deployment of KAN models while maintaining accuracy, with reproducible inference contracts and honest baseline evaluation methodology.
...
### [105] [Physics-Informed Gaussian Process Regression for the Constitutive Modeling of Concrete: A Data-Driven Improvement to Phenomenological Models](https://arxiv.org/abs/2601.03367)
*Chenyang Li,Himanshu Sharma,Youcai Wu,Joseph Magallanes,K. T. Ramesh,Michael D. Shields*

Main category: cs.LG

TL;DR: Physics-informed Gaussian Process Regression replaces empirical failure surface in concrete constitutive model, improving generalization and uncertainty quantification while maintaining modular structure.


<details>
  <summary>Details</summary>
Motivation: Existing phenomenological models like KCC rely on empirically calibrated failure surfaces that lack flexibility in model form and proper uncertainty quantification, limiting their reliability and generalization capability.

Method: Develops a physics-informed framework that replaces KCC's empirical failure surface with a constrained Gaussian Process Regression surrogate, trained on triaxial compression data under varying confinement levels, with derivative-based constraints aligned with known material behavior.

Result: Physics-informed GPR with derivative constraints yields markedly better accuracy and reliability than unconstrained GPR, especially at higher confinement levels beyond training range, reduces predictive variance, and produces tighter confidence intervals in data-scarce regimes.

Conclusion: The approach delivers a robust, uncertainty-aware surrogate that improves generalization and streamlines calibration without sacrificing interpretability and numerical efficiency, offering a practical path toward improved constitutive models for concrete.
...
### [106] [Inferring Clinically Relevant Molecular Subtypes of Pancreatic Cancer from Routine Histopathology Using Deep Learning](https://arxiv.org/abs/2601.03410)
...
TL;DR: PanSubNet: An interpretable deep learning framework that predicts pancreatic cancer molecular subtypes (basal-like vs. classical) directly from routine H&E-stained whole slide images, enabling rapid, cost-effective clinical deployment.


<details>
  <summary>Details</summary>
Motivation: Current molecular subtyping of PDAC has prognostic and predictive value but is limited in clinical practice due to high cost, long turnaround time, and tissue requirements, restricting its application in PDAC management.

Method: Developed using 1,055 patients across two cohorts (PANCAN: 846, TCGA: 209) with paired histology and RNA-seq data. Uses dual-scale architecture fusing cellular-level morphology with tissue-level architecture, employing attention mechanisms for multi-scale representation learning and transparent feature attribution. Ground-truth labels derived from validated Moffitt 50-gene signature refined by GATA6 expression.

Result: Internal validation (5-fold cross-validation): mean AUC 88.5% with balanced sensitivity/specificity. External validation (TCGA): AUC 84.0% without fine-tuning. Preserved and in metastatic disease strengthened prognostic stratification compared to RNA-seq labels. Prediction uncertainty linked to intermediate transcriptional states, not classification noise.

Conclusion: PanSubNet offers a clinically deployable, interpretable tool for genetic subtyping from routine H&E slides, enabling rapid, cost-effective molecular stratification. Currently gathering real-world validation data to support integration into digital pathology workflows and advance precision oncology for PDAC.
...
### [107] [Enhancing Small Dataset Classification Using Projected Quantum Kernels with Convolutional Neural Networks](https://arxiv.org/abs/2601.03375)
...
<details>
  <summary>Details</summary>
Motivation: CNNs require large labeled datasets but face challenges in data-scarce applications. Quantum computing principles offer potential for better feature extraction with limited data.

Method: Introduces projected quantum kernels (PQK) derived from quantum computing principles to enhance CNN feature extraction for small datasets.

Result: With 1000 training samples, PQK-enhanced CNN achieved 95% accuracy on MNIST and 90% on CIFAR-10, dramatically outperforming classical CNN (60% and 12%).

Conclusion: Projected quantum kernels show great potential for overcoming data scarcity in machine learning, paving the way for quantum-assisted neural networks in data-constrained environments.
...
### [108] [Weather-Aware Transformer for Real-Time Route Optimization in Drone-as-a-Service Operations](https://arxiv.org/abs/2601.03376)
...
<details>
  <summary>Details</summary>
Motivation: Classical path-planning algorithms like A* and Dijkstra have computational complexity that limits real-time applicability in dynamic environments for Drone-as-a-Service operations.
...
Conclusion: The framework enables real-time, weather-responsive route optimization for large-scale DaaS operations, advancing efficiency and safety of autonomous drone systems.
...
### [109] [SIGMA: Scalable Spectral Insights for LLM Collapse](https://arxiv.org/abs/2601.03385)
...
<details>
  <summary>Details</summary>
Motivation: Model collapse is a degenerative process where recursive training on model-generated content reduces distributional variance and representational quality, but current methods lack rigorous quantification and prediction capabilities for high-dimensional spaces.

Method: Introduces SIGMA (Spectral Inequalities for Gram Matrix Analysis) - a unified framework that benchmarks model collapse through spectral analysis of embedding Gram matrices, using deterministic and stochastic bounds on the matrix spectrum to track representation space contraction.

Result: SIGMA effectively captures the transition towards degenerate states, provides theoretical insights into collapse mechanics, and offers a practical, scalable tool for monitoring recursive training pipelines, even for large-scale foundation models where full eigendecomposition is intractable.

Conclusion: SIGMA provides a mathematically grounded, scalable framework for quantifying and predicting model collapse in LLMs, addressing a critical challenge in synthetic data training pipelines.
...
### [110] [Sensor to Pixels: Decentralized Swarm Gathering via Image-Based Reinforcement Learning](https://arxiv.org/abs/2601.03413)
*Yigal Koifman,Eran Iceland,Erez Koifman,Ariel Barel,Alfred M. Bruckstein*

Main category: cs.LG

TL;DR: Image-based RL method for decentralized swarm control achieves high convergence with fast pace, outperforming analytical and neural benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional multi-agent RL methods rely on handcrafted features or raw vector representations that limit scalability and efficiency regarding input order and size. There's a need for better approaches to process agent observations for effective decentralized swarm control.

Method: Propose an image-based reinforcement learning method where agent observations are encoded as structured visual inputs processed by Neural Networks to extract spatial features and produce decentralized motion control rules.

Result: The method achieves high convergence with pace nearly matching VariAntNet (fast neural benchmark) and outperforms the slow analytical solution. In some scenarios, it serves as the only practical alternative.

Conclusion: Image-based RL shows strong potential for swarm tasks, offering effective decentralized control with good convergence properties and practical applicability where other methods fail.
...
### [111] [Jailbreaking LLMs Without Gradients or Priors: Effective and Transferable Attacks](https://arxiv.org/abs/2601.03420)
*Zhakshylyk Nurlanov,Frank R. Schmidt,Florian Bernard*

Main category: cs.LG

TL;DR: RAILS introduces a gradient-free jailbreak attack framework using random iterative local search on model logits, achieving high success rates on open-source models and strong transferability to closed-source systems.


<details>
  <summary>Details</summary>
Motivation: Current safety evaluations overestimate LLM robustness because existing automated attacks rely on restrictive assumptions like handcrafted priors or white-box gradient access. There's a need for more realistic attack methods that don't require these constraints.

Method: RAILS uses token-level iterative optimization without gradients or priors, operating solely on model logits. Key innovations: 1) auto-regressive loss enforcing exact prefix matching, 2) history-based selection strategy bridging proxy optimization and true attack success, 3) cross-tokenizer ensemble attacks for discovering shared adversarial patterns.

Result: RAILS achieves near 100% success rates on multiple open-source models and high black-box attack transferability to closed-source systems like GPT and Gemini. The gradient-free approach enables cross-tokenizer ensemble attacks that discover shared adversarial patterns across disjoint vocabularies.

Conclusion: Token-level iterative optimization can succeed without gradients or priors, challenging current assumptions about jailbreak attacks. RAILS demonstrates that gradient-free methods can match gradient-based effectiveness while enabling novel cross-tokenizer attacks that significantly enhance transferability to closed-source systems.
...
### [112] [Spectral Archaeology: The Causal Topology of Model Evolution](https://arxiv.org/abs/2601.03424)
*Valentin Noël*

Main category: cs.LG

TL;DR: Training-free mechanistic probe using attention-graph spectra reveals model processing strategies and identifies Passive-Triggered Connectivity Collapse (PTCC) - a syntax-triggered connectivity failure in models undergoing specific curriculum transitions.


<details>
  <summary>Details</summary>
Motivation: Behavioral benchmarks only tell us what models do, not how they process information internally. There's a need for mechanistic understanding of model processing strategies and potential vulnerabilities that standard evaluations miss.

Method: Use attention-graph spectra as a training-free mechanistic probe. Treat each layer as a token graph and compute three spectral measures: algebraic connectivity (λ₂), smoothness, and spectral entropy. Apply this across 12 models and 10 languages.

Result: Four key findings: (1) PTCC - English-only syntax-triggered connectivity failure in models with specific curriculum transitions; (2) PTCC reflects specialization trade-off; (3) Four recurrent processing strategies identified; (4) PTCC localizes to sparse Layer 2 heads and can be partially restored via activation steering.

Conclusion: Attention-graph spectra provide practical tools for model auditing and training-regime verification. Topological regimes track tokenization density more than language identity, suggesting systematic variation in "healthy" geometry across scripts.
...
### [113] [The Illusion of Specialization: Unveiling the Domain-Invariant "Standing Committee" in Mixture-of-Experts Models](https://arxiv.org/abs/2601.03425)
*Yan Wang,Yitao Xu,Nanhan Shen,Jinyan Su,Jimin Huang,Zining Zhu*

Main category: cs.LG

TL;DR: MoE models have a domain-invariant "Standing Committee" of experts that handles most routing across all domains, challenging the assumption of domain specialization through sparse routing.


<details>
  <summary>Details</summary>
Motivation: To question the widely held assumption that Mixture of Experts models achieve domain specialization through sparse routing, and to understand the actual routing behavior in these models.

Method: Introduced COMMITTEEAUDIT, a post hoc framework that analyzes routing behavior at the level of expert groups rather than individual experts, applied across three representative models and the MMLU benchmark.

Result: Uncovered a domain-invariant Standing Committee - a compact coalition of routed experts that consistently captures the majority of routing mass across domains, layers, and routing budgets. Standing Committees anchor reasoning structure and syntax while peripheral experts handle domain-specific knowledge.

Conclusion: Mixture of Experts models have a strong structural bias toward centralized computation, with specialization being far less pervasive than commonly believed. Current training objectives like load-balancing losses may be working against the model's natural optimization path, limiting training efficiency and performance.
...
### [114] [VNU-Bench: A Benchmarking Dataset for Multi-Source Multimodal News Video Understanding](https://arxiv.org/abs/2601.03434)
*Zibo Liu,Muyang Li,Zhe Jiang,Shigang Chen*

Main category: cs.LG

TL;DR: VNU-Bench is the first benchmark for multi-source, cross-video news understanding, testing MLLMs' ability to compare perspectives, align multimodal evidence, and synthesize information across different news sources.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on single-source, intra-video reasoning, but real-world news consumption is inherently multi-sourced with complementary details, distinct narratives, and sometimes conflicting claims across different outlets over time.

Method: Introduces VNU-Bench with new question types for multi-source multimodal news understanding, using a novel hybrid human-model QA generation process to address scalability and quality control issues.

Result: Dataset comprises 429 news groups, 1,405 videos, and 2,501 high-quality questions. Comprehensive evaluation shows VNU-Bench poses substantial challenges for current multimodal large language models.

Conclusion: VNU-Bench fills a critical gap in evaluating MLLMs for real-world news understanding by focusing on multi-source, cross-video reasoning capabilities that better reflect how people actually consume news.
...
### [115] [Soft Contextualized Encoder For User Defined Text Classification](https://arxiv.org/abs/2601.03450)
*Charu Maheshwari,Vyas Raina*

Main category: cs.LG

TL;DR: Proposes a soft-contextualized encoder architecture for User-Defined Text Classification that contextualizes labels with the label set and input query prompts, achieving SOTA performance on unseen topic sets.


<details>
  <summary>Details</summary>
Motivation: Addresses the real-world challenge of classifying text to user-specified, previously unseen classes in applications like enterprise analytics, content moderation, and domain-specific information retrieval.

Method: Soft-contextualized encoder architecture that contextualizes each candidate label with both the label set and a static soft prompt representation of the input query, trained on diverse multi-source datasets.

Result: Achieves state-of-the-art performance across datasets, consistently outperforming or matching baselines on both held-out in-distribution test data and multiple unseen UDTC benchmarks.

Conclusion: The proposed architecture effectively generalizes to zero-shot classification over entirely unseen topic sets from arbitrary domains, demonstrating strong performance in User-Defined Text Classification tasks.
...
### [116] [An Expectation-Maximization Algorithm for Domain Adaptation in Gaussian Causal Models](https://arxiv.org/abs/2601.03459)
*Mohammad Ali Javidian*

Main category: cs.LG

TL;DR: A first-order EM algorithm for imputing systematically missing target variables under domain shift using Gaussian causal DAGs, with theoretical guarantees and improved accuracy over baselines.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the practical problem of imputing target variables that are systematically missing in a deployment domain when only source domain data is fully observed, leveraging causal DAG structure to handle domain shifts.

Method: Proposes a unified EM-based framework that combines source and target data through DAG structure, with a first-order (gradient) EM update replacing costly generalized least-squares M-step. The method freezes source-invariant mechanisms and re-estimates only shift-affected conditional distributions.

Result: Theoretical analysis shows local contractivity around true target parameters with geometric convergence and finite-sample guarantees. Experiments on synthetic 7-node SEM, 64-node MAGIC-IRRI genetic network, and Sachs protein-signaling data demonstrate improved imputation accuracy over baselines, especially under pronounced domain shift.

Conclusion: The DAG-aware first-order EM algorithm effectively transfers information from observed variables to missing targets under domain shift, with theoretical guarantees and practical scalability to higher-dimensional models.
...
### [117] [Hybrid Approach for Driver Behavior Analysis with Machine Learning, Feature Optimization, and Explainable AI](https://arxiv.org/abs/2601.03477)
*Mehedi Hasan Shuvo,Md. Raihan Tapader,Nur Mohammad Tamjid,Sajjadul Islam,Ahnaf Atef Choudhury,Jia Uddin*

Main category: cs.LG

TL;DR: Hybrid ML+XAI approach for driver behavior analysis achieves 95% accuracy with Random Forest, maintains 94.2% after feature optimization via LIME, balancing performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing driver behavior analytics using ML/DL suffer from low feature optimization, compromising both high performance and interpretability. Need for methods that provide both predictive power and explainability for road safety improvements.

Method: Hybrid approach using 12,857×18 Kaggle dataset with preprocessing (label encoding, random oversampling, standard scaling). Tested 13 ML algorithms, then applied LIME from XAI to identify top 10 influential features, retrained models with optimized features.

Result: Random Forest achieved 95% accuracy initially. After LIME-based feature optimization, accuracy slightly decreased to 94.2%, demonstrating efficiency improvement without significant performance sacrifice. Model provides good ROI in predictive power and explainability.

Conclusion: The hybrid ML+XAI approach successfully balances performance and interpretability for driver behavior analysis, enabling efficient models that maintain high accuracy while providing explainable insights for road safety applications.
...
### [118] [From Bits to Chips: An LLM-based Hardware-Aware Quantization Agent for Streamlined Deployment of LLMs](https://arxiv.org/abs/2601.03484)
...
TL;DR: HAQA is an automated LLM-based framework that streamlines model quantization and deployment by automatically tuning hyperparameters and hardware configurations, achieving up to 2.3x inference speedup while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Deploying large language models is challenging for non-experts due to hardware constraints and the complexity of quantization techniques, which require extensive tuning and hardware-specific optimizations.

Method: HAQA uses LLMs as agents to automate the entire quantization and deployment process, including hyperparameter tuning and hardware configuration optimization, implementing adaptive quantization strategies across diverse hardware platforms.

Result: Achieves up to 2.3x inference speedup, increased throughput, and improved accuracy compared to unoptimized models on Llama, with automatic discovery of optimal (sometimes counterintuitive) settings across different hardware.

Conclusion: HAQA significantly reduces manual effort in quantization and deployment while improving performance, making LLM deployment more accessible to non-experts through automated hardware-aware optimization.
...
### [119] [VeRPO: Verifiable Dense Reward Policy Optimization for Code Generation](https://arxiv.org/abs/2601.03525)
*Longwen Wang,Xuan'er Wu,Xiaohui Hu,Yirui Liu,Yuankai Fan,Kaidong Yu,Qizhen Weng,Wei Xi,Xuelong Li*

Main category: cs.LG

TL;DR: VeRPO is a novel RL framework for code generation that creates dense rewards from weighted partial test success, using execution statistics to estimate test difficulty weights, achieving significant performance gains with minimal computational cost.


<details>
  <summary>Details</summary>
Motivation: Current RL approaches for code generation face two main problems: 1) sparse pass/fail outcome rewards limit performance gains, and 2) external Reward Models (RMs) suffer from reward misalignment and high computational costs. There's a need for dense rewards that are both robust and computationally efficient.

Method: VeRPO constructs dense rewards from weighted partial success by dynamically estimating the difficulty weight of each unit test based on execution statistics during training. It integrates this dense signal with global execution outcomes to ensure consistency between partial success and end-to-end functional correctness, creating a robust reward paradigm using only verifiable execution feedback.

Result: VeRPO consistently outperforms both outcome-driven and RM-based baselines across diverse benchmarks, achieving up to +8.83% gain in pass@1 with negligible time cost (<0.02%) and zero GPU memory overhead.

Conclusion: VeRPO provides an effective solution for RL-based code generation by synthesizing robust and dense rewards fully grounded in verifiable execution feedback, overcoming the limitations of both sparse outcome rewards and computationally expensive reward models.
...
### [120] [Green's-Function Spherical Neural Operators for Biological Heterogeneity](https://arxiv.org/abs/2601.03561)
*Hao Tang,Hao Chen,Hao Li,Chao Li*

Main category: cs.LG

TL;DR: GSNO is a spherical neural operator that uses designable Green's functions to handle real-world heterogeneity while preserving spherical geometry, achieving better performance on various tasks.


<details>
  <summary>Details</summary>
Motivation: Existing spherical deep learning approaches struggle to balance strong geometric inductive biases with modeling real-world heterogeneity. There's a need to retain spherical geometry while handling nuisance variability and anisotropy in biological and physical systems.

Method: Introduces Designable Green's Function framework (DGF) for systematic Green's functions under rotational group. Proposes Green's-Function Spherical Neural Operator (GSNO) with three operator solutions: (1) Equivariant Solution from Equivariant Green's Function for symmetry-consistent modeling; (2) Invariant Solution from Invariant Green's Function to eliminate nuisance heterogeneity; (3) Anisotropic Solution from Anisotropic Green's Function to model anisotropic systems like fibers.

Result: GSNO demonstrates superiority on multiple tasks including spherical MNIST, Shallow Water Equation, diffusion MRI fiber prediction, cortical parcellation, and molecule structure modeling. The model adapts to heterogeneous systems with nuisance variability and anisotropy while maintaining spectral efficiency.

Conclusion: GSNO successfully addresses the challenge of balancing spherical geometric inductive biases with real-world heterogeneity modeling. The designable Green's function framework provides a flexible solution strategy that enables effective handling of symmetry, nuisance variability, and anisotropy in spherical deep learning applications.
...
### [121] [A Proposed Paradigm for Imputing Missing Multi-Sensor Data in the Healthcare Domain](https://arxiv.org/abs/2601.03565)
*Vaibhav Gupta,Florian Grensing,Beyza Cinar,Maria Maleshkova*

Main category: cs.LG

TL;DR: This paper reviews imputation techniques for multisensor wearable data in hypoglycemia prediction, proposing a systematic paradigm for feature-specific imputation based on missing data duration.


<details>
  <summary>Details</summary>
Motivation: Chronic diseases like diabetes require continuous monitoring for early detection of complications like hypoglycemia, but multisensor wearable data suffers from noise and frequent missing values that hinder effective prediction.

Method: The study analyzes limitations of existing datasets, examines temporal characteristics of hypoglycemia-relevant features, conducts comprehensive review of imputation techniques from state-of-the-art studies, and evaluates ML/DL methods from other healthcare contexts for longer data gaps.

Result: The paper proposes a systematic paradigm where imputation strategies are tailored to specific feature characteristics and missing interval durations, emphasizing feature-specific approaches for heterogeneous temporal patterns.

Conclusion: Effective hypoglycemia prediction requires investigating temporal dynamics of individual features and implementing multiple, feature-specific imputation techniques to address the heterogeneous temporal patterns in multisensor wearable data.
...
### [122] [Local Intrinsic Dimensionality of Ground Motion Data for Early Detection of Complex Catastrophic Slope Failure](https://arxiv.org/abs/2601.03569)
...
<details>
  <summary>Details</summary>
Motivation: Early and accurate identification of landslide failure zones is crucial for geohazard mitigation. Existing approaches using surface displacement data often fail to capture both spatial correlations and temporal dynamics inherent in landslide monitoring data.

Method: Extends existing sLID (spatial LID) method with three key enhancements: (1) kinematic enhancement by incorporating velocity into sLID computation, (2) spatial fusion using Bayesian estimation to aggregate sLID values across neighborhoods, and (3) temporal modeling through tLID that learns long-term dynamics from time series data. These are integrated into a unified spatiotemporal LID (stLID) framework.

Result: Extensive experiments show that stLID consistently outperforms existing methods in both failure detection precision and lead-time (early warning capability).
...
### [123] [Variational Inference, Entropy, and Orthogonality: A Unified Theory of Mixture-of-Experts](https://arxiv.org/abs/2601.03577)
*Ye Su,Yong Liu*

Main category: cs.LG

TL;DR: The paper provides the first unified theoretical framework for Mixture-of-Experts models, rigorously deriving Top-k routing and load balancing as optimal Bayesian approximations and information-theoretic mechanisms, while identifying routing as NP-hard and proposing orthogonality regularization as the optimal solution.


<details>
  <summary>Details</summary>
Motivation: Current Mixture-of-Experts models rely on heuristic mechanisms (Top-k routing and auxiliary load balancing) without cohesive theoretical foundation. The paper aims to build a unified theoretical framework to understand and improve these practices.

Method: Develops a Bayesian framework that derives routing practices as optimal sparse posterior approximation and prior regularization. Also uses information theory to frame them as mechanisms to minimize routing ambiguity and maximize channel capacity. Identifies routing as NP-hard sparse subset selection problem and analyzes greedy strategies.

Result: Proves the existence of a "Coherence Barrier" where greedy routing fails when expert representations have high mutual coherence. Formally verifies that geometric orthogonality in expert feature space bridges the gap between NP-hard global optimum and polynomial-time greedy approximation.

Conclusion: Orthogonality regularization is confirmed as the optimal engineering relaxation for large-scale MoE models. The work provides essential theoretical support and technical assurance for deeper understanding and novel designs of Mixture-of-Experts architectures.
...
### [124] [Local Gradient Regulation Stabilizes Federated Learning under Client Heterogeneity](https://arxiv.org/abs/2601.03584)
*Ping Luo,Jiahuan Wang,Ziqing Wen,Tao Sun,Dongsheng Li*

Main category: cs.LG

TL;DR: The paper shows client heterogeneity destabilizes FL by distorting local gradient dynamics, and proposes ECGR (Exploratory-Convergent Gradient Re-aggregation) to regulate local gradients without extra communication overhead, stabilizing FL under heterogeneous data distributions.


<details>
  <summary>Details</summary>
Motivation: Federated learning faces fundamental stability challenges from statistical heterogeneity in realistic deployments, which causes systematic drift in local gradient dynamics that accumulates across communication rounds and impedes global convergence.

Method: Develops a client-side perspective to regulate local gradient contributions without additional communication overhead, instantiated through Exploratory-Convergent Gradient Re-aggregation (ECGR) that balances well-aligned and misaligned gradient components.
...
Conclusion: Local gradients are identified as a key regulatory lever for stabilizing heterogeneous FL systems, and client-side regulation of gradient dynamics provides an effective approach to address stability challenges in federated learning with statistical heterogeneity.
...
### [125] [ALERT: Zero-shot LLM Jailbreak Detection via Internal Discrepancy Amplification](https://arxiv.org/abs/2601.03600)
*Xiao Lin,Philip Li,Zhichen Zeng,Tingwei Li,Tianxin Wei,Xuying Ning,Gaotang Li,Yuzhong Chen,Hanghang Tong*

Main category: cs.LG

TL;DR: ALERT is a zero-shot jailbreak detection framework that amplifies internal feature discrepancies between benign and malicious prompts without relying on pre-seen attack templates.


<details>
  <summary>Details</summary>
Motivation: Current jailbreak detection methods depend on training data containing known attack templates, but real-world scenarios involve constantly evolving zero-day attacks where no templates are available during training.

Method: Proposes a layer-wise, module-wise, and token-wise amplification framework that identifies safety-relevant layers, modules encoding discriminative signals, and informative safety tokens, then applies two complementary classifiers on amplified representations.

Result: ALERT consistently ranks among top two methods across all datasets and attack strategies, outperforming second-best baselines by 10-40% in Accuracy and F1-score on three safety benchmarks.

Conclusion: The amplification-based approach enables effective zero-shot jailbreak detection by leveraging inherent discriminative signals in LLM internal representations, addressing the critical need for detection of novel, evolving attacks.
...
### [126] [A Comparative Study of Traditional Machine Learning, Deep Learning, and Large Language Models for Mental Health Forecasting using Smartphone Sensing Data](https://arxiv.org/abs/2601.03603)
*Kaidong Feng,Zhu Sun,Roy Ka-Wei Lee,Xun Jiang,Yin-Leng Theng,Yi Ding*

Main category: cs.LG

TL;DR: This paper presents the first comprehensive benchmarking study comparing ML, DL, and LLM approaches for forecasting mental health using smartphone sensing data from college students, finding DL models (particularly Transformers) perform best overall.


<details>
  <summary>Details</summary>
Motivation: Current mental health sensing focuses on detection of existing conditions, but forecasting enables proactive support through Just-in-Time Adaptive Interventions. There's a need to systematically compare different modeling approaches for mental health forecasting using comprehensive longitudinal data.

Method: Used the College Experience Sensing (CES) dataset - the most extensive longitudinal dataset of college student mental health. Systematically evaluated traditional ML, deep learning (DL), and large language model (LLM) approaches across temporal windows, feature granularities, personalization strategies, and class imbalance handling.

Result: DL models, particularly Transformers (Macro-F1 = 0.58), achieved the best overall performance. LLMs showed strength in contextual reasoning but weaker temporal modeling. Personalization substantially improved forecasts of severe mental health states.
...
### [127] [Policy-Guided Search on Tree-of-Thoughts for Efficient Problem Solving with Bounded Language Model Queries](https://arxiv.org/abs/2601.03606)
...
<details>
  <summary>Details</summary>
Motivation: Existing Tree-of-Thoughts search algorithms ignore the high computational costs of LM inference, especially problematic in resource-constrained scenarios. Need to improve LM problem-solving performance under limited computational budgets.

Method: Adapt Levin Tree Search (LTS) to ToT framework, using LM probabilities as heuristic to guide tree exploration efficiently. Extend theoretical results showing LTS guarantees bound on states expanded/thoughts generated for pruned trees.

Result: Under fixed LM query budget, LTS achieves comparable or higher accuracy than baseline search algorithms across three domains (Blocksworld, PrOntoQA, Array Sorting) and four distinct LMs.

Conclusion: LTS enables cost-effective, time-efficient problem-solving in ToT framework, making it suitable for latency-critical and resource-constrained applications. LM probabilities serve as effective heuristic for search guidance.
...
### [128] [Mathematical Foundations of Polyphonic Music Generation via Structural Inductive Bias](https://arxiv.org/abs/2601.03612)
*Joonwon Seo*
...
<details>
  <summary>Details</summary>
Motivation: Address the "Missing Middle" problem in polyphonic music generation by incorporating structural inductive bias, bridging gaps between theoretical foundations and practical AI music generation.

Method: Smart Embedding architecture with structural inductive bias; mathematical proofs using information theory (bounded loss), Rademacher complexity (tighter generalization bounds), and category theory; empirical validation with normalized mutual information analysis and SVD.

Result: 48.30% parameter reduction, 9.47% validation loss reduction, NMI=0.167 showing pitch-hand independence, 28.09% tighter generalization bound, expert listening study (N=53) confirmation.

Conclusion: Dual theoretical-applied framework successfully bridges AI music generation gaps with mathematically grounded deep learning, offering verifiable insights through rigorous proofs and empirical validation.
...
### [129] [Learning Shortest Paths When Data is Scarce](https://arxiv.org/abs/2601.03629)
*Dmytro Matsypura,Yu Pan,Hanzhao Wang*

Main category: cs.LG

TL;DR: Paper proposes Laplacian-regularized least squares to calibrate biased simulator outputs using limited real-world data and edge-similarity structure for stochastic shortest-path problems.


<details>
  <summary>Details</summary>
Motivation: Digital twins and simulators are increasingly used for routing decisions but exhibit systematic bias, while ground-truth measurements are costly and scarce. Need to bridge simulator-reality gap using limited real data.

Method: Model simulator-to-reality discrepancy as unknown edge-specific bias varying smoothly over similarity graph. Use Laplacian-regularized least squares for estimation. For cold-start settings, develop bias-aware active learning algorithm that adaptively selects edges to measure.

Result: Established finite-sample error bounds, translated estimation error into path-level suboptimality guarantees, and proposed computable data-driven certificate for verifying near-optimality. Numerical experiments on road networks and traffic graphs demonstrate effectiveness.

Conclusion: The approach yields calibrated edge cost estimates even in data-scarce regimes and provides practical solutions for combining abundant synthetic data with limited real-world observations for routing optimization.
...
### [130] [Kantorovich-Type Stochastic Neural Network Operators for the Mean-Square Approximation of Certain Second-Order Stochastic Processes](https://arxiv.org/abs/2601.03634)
*Sachin Saini,Uaday Singh*

Main category: cs.LG

TL;DR: Proposes Kantorovich-type Stochastic Neural Network Operators (K-SNNOs) that incorporate randomness through stochastic neurons for approximating stochastic processes, with proven mean-square convergence and error estimates.


<details>
  <summary>Details</summary>
Motivation: While artificial neural network operators are widely used for deterministic functions, their extension to random dynamics remains underexplored. There's a need for neural operators that can properly model and approximate stochastic signals by inheriting the probabilistic structure of underlying processes.

Method: Constructs K-SNNOs where randomness is incorporated through stochastic neurons driven by stochastic integrators (not at coefficient level). This allows the operator to inherit the probabilistic structure of the underlying stochastic process.

Result: Establishes mean-square convergence of K-SNNOs to target stochastic processes and derives quantitative error estimates in terms of modulus of continuity. Numerical simulations validate accurate sample path reconstruction and rapid MSE decay.

Conclusion: K-SNNOs provide an effective framework for approximating stochastic processes using stochastic neurons, with proven convergence properties and demonstrated practical effectiveness through numerical validation.
...
### [131] [ReLA: Representation Learning and Aggregation for Job Scheduling with Reinforcement Learning](https://arxiv.org/abs/2601.03646)
*Zhengyi Kwan,Zhang Wei,Aik Beng Ng,Zhengkui Wang,Simon See*

Main category: cs.LG

TL;DR: ReLA is a reinforcement learning scheduler that uses structured representation learning and aggregation to improve job scheduling efficiency and quality, especially for large-scale problems.


<details>
  <summary>Details</summary>
Motivation: Existing job scheduling solutions suffer from long running times or poor schedule quality when problem scale increases, creating a need for more efficient and effective scheduling approaches.

Method: ReLA uses structured representation learning with three modules: two intra-entity learning modules (self-attention and convolution) for job operations and machines, and one inter-entity learning module (cross-attention) for entity interactions. These are applied in a multi-scale architecture and aggregated to support RL decision-making.

Result: ReLA achieves best makespan in most tested settings across small, medium, and large job instances. On non-large instances, it reduces SOTA optimality gap by 13.0% (to 7.3% average gap), and on large-scale instances reduces gap by 78.6% (to 2.1% average gap).
...
### [132] [Quantum Classical Ridgelet Neural Network For Time Series Model](https://arxiv.org/abs/2601.03654)
*Bahadur Yadav,Sanjay Kumar Mohanty*

Main category: cs.LG

TL;DR: Quantum computing method integrates ridglet transforms with single-qubit quantum processing for improved time series forecasting, showing superior performance on financial data.


<details>
  <summary>Details</summary>
Motivation: To enhance feature extraction and forecasting capabilities for time series data by combining the strengths of ridglet transforms with quantum computing approaches.

Method: Integrates Ridgelet neural network with single-qubit quantum computing method in quantum processing pipelines for time series data analysis.
...
Conclusion: The quantum computing method incorporating ridglet transforms effectively improves time series forecasting capabilities, particularly for financial applications.
...
TL;DR: The paper proposes information-theoretic measures to identify interpretable "grandmother cell" neurons in foundation models, finding evidence of moderately selective neurons in TabPFN without complex interpretability techniques.


<details>
  <summary>Details</summary>
Motivation: Foundation models are powerful but opaque, and there's interest in whether they contain inherently interpretable "grandmother cells" - neurons that respond exclusively to single concepts, similar to neuroscience findings.

Method: Proposed two information-theoretic measures to quantify neuronal saliency and selectivity for single concepts, applied these to TabPFN (tabular foundation model), and performed a simple search across neuron-concept pairs to find the most salient and selective pairs.

Result: Found first evidence that some neurons in such models show moderate, statistically significant saliency and selectivity for high-level concepts, suggesting interpretable neurons can emerge naturally.

Conclusion: Interpretable "grandmother cell" neurons can emerge naturally in foundation models and can sometimes be identified without resorting to complex interpretability techniques, offering a simpler approach to model transparency.
...
### [134] [Group and Exclusive Sparse Regularization-based Continual Learning of CNNs](https://arxiv.org/abs/2601.03658)
*Basile Tousside,Janis Mohr,Jörg Frochte*

Main category: cs.LG

TL;DR: GESCL is a regularization-based continual learning method for CNNs that prevents catastrophic forgetting through stability regularization for important filters and plasticity regularization to sparsify unimportant filters for future tasks, achieving better performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: To address catastrophic forgetting in continual learning for CNNs without expanding network capacity or storing past task data, which are computationally expensive approaches.

Method: GESCL uses two regularization terms: stability regularization prevents important filters from deviating too much when learning new tasks, while plasticity regularization leverages CNN over-parameterization to sparsify unimportant filters and make them relevant for future tasks.

Result: Experiments on popular CL vision benchmarks show GESCL achieves significant improvements over state-of-the-art methods in classification accuracy and avoiding catastrophic forgetting while using fewer parameters and computations.

Conclusion: GESCL provides an effective regularization-based approach for continual learning that balances stability and plasticity, enabling fixed-capacity CNNs to learn multiple tasks sequentially without catastrophic forgetting.
...
### [135] [AMIR-GRPO: Inducing Implicit Preference Signals into GRPO](https://arxiv.org/abs/2601.03661)
*Amir Hossein Yari,Fajri Koto*

Main category: cs.LG

TL;DR: AMIR-GRPO improves GRPO for LLM alignment by adding implicit contrastive regularization from intra-group reward rankings, addressing length bias and better utilizing rollout supervision.


<details>
  <summary>Details</summary>
Motivation: GRPO has structural limitations for reasoning tasks: length bias from sequence-level advantage normalization, diluted penalties for low-quality trajectories, and wasted pairwise preference information in reward rankings. This leads to underutilization of costly rollout supervision.

Method: AMIR-GRPO augments GRPO with an implicit DPO-style contrastive regularizer constructed directly from intra-group reward rankings, requiring no additional annotations. This amplifies suppression of low-reward trajectories, attenuates length bias, and transforms each rollout group into denser supervision constraints.
...
Conclusion: AMIR-GRPO effectively addresses GRPO's limitations in reasoning-heavy settings by better utilizing reward ranking information, leading to improved performance and more effective alignment for complex reasoning tasks.
...
### [136] [Stochastic Voronoi Ensembles for Anomaly Detection](https://arxiv.org/abs/2601.03664)
*Yang Cao*

Main category: cs.LG

TL;DR: SVEAD is a novel anomaly detection method using stochastic Voronoi ensembles that achieves linear time complexity and outperforms 12 state-of-the-art methods on 45 datasets.


<details>
  <summary>Details</summary>
Motivation: Existing anomaly detection methods struggle with datasets having varying local densities - distance-based methods miss local anomalies, while density-based approaches require careful parameter tuning and have quadratic time complexity.

Method: SVEAD constructs ensemble random Voronoi diagrams and scores points using normalized cell-relative distances weighted by local scale. It decomposes data space into restricted regions (Voronoi cells) and examines each region independently to make local anomalies more conspicuous.

Result: SVEAD achieves linear time complexity and constant space complexity, and outperforms 12 state-of-the-art anomaly detection approaches on 45 datasets.

Conclusion: The geometric insight of decomposing data space into restricted regions enables effective detection of local anomalies with superior computational efficiency and performance compared to existing methods.
...
### [137] [Disentangling Aleatoric and Epistemic Uncertainty in Physics-Informed Neural Networks. Application to Insulation Material Degradation Prognostics](https://arxiv.org/abs/2601.03673)
*Ibai Ramirez,Jokin Alcibar,Joel Pino,Mikel Sanz,Jose I. Aizpurua*

Main category: cs.LG

TL;DR: A Bayesian Physics-Informed Neural Network (B-PINN) framework is proposed for transformer insulation ageing estimation, jointly modeling epistemic and aleatoric uncertainty to provide full predictive posteriors for improved uncertainty quantification in prognostics.


<details>
  <summary>Details</summary>
Motivation: Current Physics-Informed Neural Networks (PINNs) in Prognostics and Health Management (PHM) have limited uncertainty quantification capabilities, being mostly deterministic or only accounting for epistemic uncertainty. This limits their suitability for risk-aware decision-making in critical applications like transformer asset management.

Method: A heteroscedastic Bayesian Physics-Informed Neural Network (B-PINN) framework that integrates Bayesian Neural Networks (BNNs) with physics-based residual enforcement and prior distributions. The approach models both epistemic and aleatoric uncertainty to provide full predictive posteriors for spatiotemporal insulation material ageing estimation.

Result: The proposed B-PINN demonstrates improved predictive accuracy and better-calibrated uncertainty estimates compared to deterministic PINNs, dropout-based PINNs (d-PINNs), and alternative B-PINN variants. The framework was validated using a finite-element thermal model and field measurements from a solar power plant.

Conclusion: Bayesian physics-informed learning shows strong potential for uncertainty-aware prognostics and informed decision-making in transformer asset management, addressing the limitations of existing PINN approaches in PHM applications.
...
### [138] [Rethinking Recurrent Neural Networks for Time Series Forecasting: A Reinforced Recurrent Encoder with Prediction-Oriented Proximal Policy Optimization](https://arxiv.org/abs/2601.03683)
*Xin Lai,Shiming Deng,Lu Yu,Yumin Lai,Shenghao Qiao,Xinze Zhang*

Main category: cs.LG

TL;DR: Proposes RRE-PPO4Pred, a reinforced recurrent encoder with prediction-oriented proximal policy optimization that enhances RNNs for time series forecasting by treating internal adaptation as a Markov Decision Process.


<details>
  <summary>Details</summary>
Motivation: Conventional RNN-based predictors treat all time steps and hidden states equally without considering their distinct contributions to forecasting, leading to suboptimal performance. There's a need to improve RNNs' time series modeling capacity and forecasting accuracy.

Method: Three core innovations: (1) Reinforced Recurrent Encoder (RRE) framework that formulates RNN internal adaptation as a Markov Decision Process for unified decision environment; (2) Prediction-oriented Proximal Policy Optimization (PPO4Pred) with Transformer-based agent and dynamic transition sampling; (3) Co-evolutionary optimization paradigm for adaptive and interactive time series modeling.
...
Conclusion: RRE-PPO4Pred provides an advanced time series predictor in engineering informatics by significantly improving RNN models' forecasting accuracy through reinforcement learning-based adaptation mechanisms.
...
### [139] [A Pre-trained Reaction Embedding Descriptor Capturing Bond Transformation Patterns](https://arxiv.org/abs/2601.03689)
*Weiqi Liu,Fenglei Cao,Yuan Qi,Li-Cheng Xu*

Main category: cs.LG

TL;DR: RXNEmb is a novel reaction-level descriptor derived from RXNGraphormer, a model pre-trained to distinguish real from fictitious reactions, enabling better reaction fingerprinting and analysis.


<details>
  <summary>Details</summary>
Motivation: With the rise of data-driven reaction prediction models, there's a scarcity of general-purpose, reaction-wise descriptors to bridge the gap between real-world chemistry and digital representations.

Method: Developed RXNEmb from RXNGraphormer, a model pre-trained to distinguish real reactions from fictitious ones with erroneous bond changes, learning intrinsic bond formation and cleavage patterns.

Result: Successfully re-clustered USPTO-50k dataset yielding classification reflecting bond-change similarities better than rule-based categories; enabled reaction space visualization; attention analysis revealed focus on chemically critical sites.
...
### [140] [Inference Attacks Against Graph Generative Diffusion Models](https://arxiv.org/abs/2601.03701)
...
<details>
  <summary>Details</summary>
Motivation: Graph generative diffusion models have shown powerful capabilities for generating complex graph structures, but their privacy risks remain largely unexplored. The authors aim to investigate information leakage in these models through inference attacks.

Method: The authors design three types of black-box inference attacks: (1) graph reconstruction attack to reconstruct structurally similar training graphs from generated graphs, (2) property inference attack to infer properties like average graph density and density distribution, and (3) two membership inference attacks to determine if a specific graph was in the training set. They also propose two defense mechanisms to mitigate these attacks.
...
Conclusion: Graph generative diffusion models are vulnerable to privacy attacks, and the proposed attacks effectively demonstrate information leakage. The defense mechanisms provide practical solutions to mitigate these privacy risks while maintaining model utility.
...
### [141] [TreeAdv: Tree-Structured Advantage Redistribution for Group-Based RL](https://arxiv.org/abs/2601.03703)
*Lang Cao,Hui Ruan,Yongqian Li,Peng Chao,Wu Ning,Haonan Song,Renhong Chen,Yitong Li*

Main category: cs.LG

TL;DR: TreeAdv improves group-based RL for LLMs by using tree-structured advantage redistribution instead of treating rollouts as flat sequences, reducing token usage while improving reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Standard GRPO treats each rollout trajectory as an independent flat sequence and assigns a single sequence-level advantage to all tokens, leading to sample inefficiency and length bias toward verbose, redundant chains of thought without improving logical depth.

Method: TreeAdv builds a group of trees (forest) using entropy-driven sampling where trees branch at high-uncertainty decisions while sharing low-uncertainty tokens across rollouts. It then aggregates token-level advantages for internal tree segments by redistributing advantages of complete rollouts (leaf nodes).
...
Conclusion: TreeAdv's tree-structured approach to advantage redistribution in group-based RL provides more efficient and effective alignment of large language models on complex reasoning tasks compared to flat sequence methods.
...
### [142] [Investigating Knowledge Distillation Through Neural Networks for Protein Binding Affinity Prediction](https://arxiv.org/abs/2601.03704)
*Wajid Arshad Abbasi,Syed Ali Abbas,Maryum Bibi,Saiqa Andleeb,Muhammad Naveed Akhtar*

Main category: cs.LG

TL;DR: Knowledge distillation framework transfers structural knowledge to sequence-based protein-protein binding affinity predictors, improving performance without requiring structural data at inference.


<details>
  <summary>Details</summary>
Motivation: Predicting protein-protein binding affinity is challenging due to trade-off between accuracy and data availability. Structure-based models outperform sequence-based ones but require experimentally resolved protein structures which are often unavailable.

Method: Proposed regression framework using knowledge distillation: trains sequence-based student network supervised by structure-informed teacher network, using both binding affinity labels and intermediate feature representations. Only needs sequence data during inference.

Result: Student model achieved P_r=0.481 and RMSE=2.488 kcal/mol, significantly improving over sequence-only baseline (P_r=0.375, RMSE=2.712) and approaching structure-based model performance (P_r=0.512, RMSE=2.445). Error analysis showed improved agreement and reduced bias.

Conclusion: Knowledge distillation effectively transfers structural knowledge to sequence-based predictors, potentially closing performance gap as larger datasets become available. Enables accurate binding affinity prediction without requiring structural data at inference.
...
### [143] [The Geometry of the Pivot: A Note on Lazy Pivoted Cholesky and Farthest Point Sampling](https://arxiv.org/abs/2601.03706)
*Gil Shabat*
...
Method: The paper provides a geometric interpretation showing that pivotal selection equals Farthest Point Sampling using kernel metric, and Cholesky factor construction corresponds to implicit Gram-Schmidt orthogonalization in Reproducing Kernel Hilbert Space.

Result: The authors demonstrate mathematical equivalence between Pivoted Cholesky and FPS in RKHS, providing both theoretical derivation and minimalist Python implementation to connect theory with practice.

Conclusion: This work bridges the gap between algebraic and geometric understanding of Pivoted Cholesky in kernel methods, offering clearer intuition for its greedy selection mechanism in low-rank kernel approximations.
...
### [144] [R$^3$L: Reflect-then-Retry Reinforcement Learning with Language-Guided Exploration, Pivotal Credit, and Positive Amplification](https://arxiv.org/abs/2601.03715)
*Weijie Shi,Yanxi Chen,Zexi Li,Xuchen Pan,Yuchang Sun,Jiajie Xu,Xiaofang Zhou,Yaliang Li*

Main category: cs.LG

TL;DR: R³L improves RL for LLMs by using language-guided reflection to transform failures into successes, pivotal credit assignment to update only diverging trajectory segments, and positive amplification to stabilize training.
...
Method: R³L uses reflect-then-retry with language feedback to synthesize successful trajectories from failures, pivotal credit assignment to update only diverging suffixes, and positive amplification to upweight successful trajectories for stable optimization.

Result: Experiments show 5% to 52% relative improvements over baselines on agentic and reasoning tasks while maintaining training stability.

Conclusion: R³L addresses key RL limitations for LLMs through active trajectory synthesis, fine-grained credit assignment, and positive signal amplification, achieving significant performance gains.
...
### [145] [ETR: Outcome-Guided Elastic Trust Regions for Policy Optimization](https://arxiv.org/abs/2601.03723)
*Shijie Zhang,Kevin Zhang,Zheyuan Gu,Xiang Guo,Rujun Guo,Shaoyu Liu,Guanjun Jiang,Xiaozhao Wang*

Main category: cs.LG

TL;DR: ETR (Elastic Trust Regions) improves RLVR by replacing GRPO's static trust region constraints with dynamic, signal-aware constraints that adapt to advantage magnitude and group variance, achieving better performance while maintaining exploration.


<details>
  <summary>Details</summary>
Motivation: GRPO's uniform static trust region constraint assumes signal homogeneity, which misaligns with the heterogeneous nature of outcome-driven learning where advantage magnitudes and variances fluctuate significantly. This leads to inefficient exploitation of high-quality signals and insufficient noise suppression, often causing rapid entropy collapse.
...
Conclusion: ETR addresses the structural limitation of GRPO by aligning optimization constraints with signal quality, providing a more effective approach for RLVR that better handles the heterogeneous nature of outcome-driven learning while maintaining exploration capabilities.
...
### [146] [EDCO: Dynamic Curriculum Orchestration for Domain-specific Large Language Model Fine-tuning](https://arxiv.org/abs/2601.03725)
...
<details>
  <summary>Details</summary>
Motivation: Current LLM fine-tuning methods use static curriculum learning that pre-orders training samples before training, lacking adaptability to the model's evolving needs during fine-tuning. This limitation reduces learning efficiency and effectiveness.

Method: EDCO uses inference entropy and dynamic curriculum orchestration with three components: 1) efficient entropy estimator using prefix tokens to approximate full-sequence entropy, 2) entropy-based curriculum generator selecting high-entropy data points, and 3) LLM trainer optimizing on the selected curriculum.

Result: EDCO outperforms traditional curriculum strategies for fine-tuning Qwen3-4B and Llama3.2-3B models in communication, medicine, and law domains under supervised and reinforcement learning settings. The efficient entropy estimation reduces computational time by 83.5% while maintaining high accuracy.

Conclusion: Dynamic curriculum learning based on inference entropy significantly improves LLM fine-tuning efficiency and performance compared to static methods, with EDCO providing an effective framework that adapts to model needs during training while reducing computational overhead.
...
### [147] [Probabilistic Transformers for Joint Modeling of Global Weather Dynamics and Decision-Centric Variables](https://arxiv.org/abs/2601.03753)
*Paulius Rauba,Viktor Cikojevic,Fran Bartolic,Sam Levang,Ty Dickinson,Chase Dwelle*

Main category: cs.LG

TL;DR: GEM-2 is a lightweight probabilistic transformer that directly learns decision-relevant weather variables (like extrema and accumulations) alongside atmospheric dynamics, outperforming operational NWP models and competing with more complex ML approaches while being much more efficient.


<details>
  <summary>Details</summary>
Motivation: Current weather forecast models don't directly learn the decision-relevant variables that users actually need (like extremes, accumulations, threshold exceedances), forcing users to do suboptimal post-processing that can introduce structural bias.

Method: GEM-2 is a probabilistic transformer (~275M parameters) trained on CRPS objective that jointly learns global atmospheric dynamics alongside user-relevant variables. It's lightweight and computationally efficient (~20-100x training speedup vs state-of-the-art).

Result: Directly outperforms operational numerical weather prediction models, competitive with ML models using expensive diffusion processes or multi-stage fine-tuning. Shows state-of-the-art economic value metrics, stable convergence to climatology at S2S/seasonal timescales, and surprising insensitivity to architectural/training choices.

Conclusion: GEM-2 demonstrates that lightweight transformers can directly learn decision-relevant weather variables, bridging the gap between atmospheric modeling and real-world decision-making while being computationally efficient and robust.
...
### [148] [Learning Shrinks the Hard Tail: Training-Dependent Inference Scaling in a Solvable Linear Model](https://arxiv.org/abs/2601.03764)
*Noam Levi*
...
Method: Developed a Latent Instance Difficulty (LID) model where each input's target variance is governed by a latent "precision" drawn from a heavy-tailed distribution. Analyzed last-layer fine-tuning in this solvable model to derive closed-form predictions for scaling behavior.

Result: Generalization loss follows standard scaling laws, but pass@k failure rates exhibit power-law decay k^{-β_eff} with training-dependent exponents. β_eff grows with sample size N before saturating at an intrinsic limit β set by the difficulty distribution's tail. Learning shrinks the "hard tail" of the error distribution.

Conclusion: The LID model reveals coupling between generalization and inference: improvements in generalization steepen pass@k curves until irreducible target variance dominates. Provides testable predictions and compute-allocation rules favoring training before saturation and inference attempts after, validated in simulations and real-data proxies.
...
### [149] [Improving Compactness and Reducing Ambiguity of CFIRE Rule-Based Explanations](https://arxiv.org/abs/2601.03776)
*Sebastian Müller,Tobias Schneider,Ruben Kemna,Vanessa Toborek*

Main category: cs.LG

TL;DR: CFIRE algorithm for tabular data explanations produces ambiguous rule assignments; proposed pruning strategy reduces ambiguity while preserving fidelity.


<details>
  <summary>Details</summary>
Motivation: Models in sensitive domains need transparency, but CFIRE's local explanation method can assign conflicting rules to the same sample, creating ambiguity in interpretations.
...
Result: Experiments show the pruning yields smaller, less ambiguous models with minimal impact on predictive performance across multiple datasets.

Conclusion: The proposed pruning approach effectively addresses CFIRE's ambiguity problem, improving interpretability while maintaining model fidelity for tabular data explanations.
...
### [150] [Prompt Tuning without Labeled Samples for Zero-Shot Node Classification in Text-Attributed Graphs](https://arxiv.org/abs/2601.03793)
*Sethupathy Parameswaran,Suresh Sundaram,Yuan Fang*

Main category: cs.LG

TL;DR: Zero-shot Prompt Tuning (ZPT) framework for node classification in text-attributed graphs using a Universal Bimodal Conditional Generator to create synthetic samples from class names only.


<details>
  <summary>Details</summary>
Motivation: Zero-shot node classification in text-attributed graphs is challenging due to lack of labeled data, but has important applications in social networks, article grouping, and e-commerce categorization.

Method: 1) Pre-train graph-language model to capture graph structure and textual descriptions. 2) Train conditional generative model to learn joint distribution of nodes in graph and text modalities. 3) Generate synthetic node/text embeddings from class names only. 4) Use synthetic samples for continuous prompt tuning in zero-shot classification.

Result: Extensive experiments on benchmark datasets show ZPT outperforms state-of-the-art baselines. Ablation studies validate the contribution of the bimodal generator.

Conclusion: The proposed ZPT framework effectively addresses zero-shot node classification in TAGs by generating synthetic samples from class names and enabling continuous prompt tuning, demonstrating superior performance over existing methods.
...
### [151] [Quantum vs. Classical Machine Learning: A Benchmark Study for Financial Prediction](https://arxiv.org/abs/2601.03802)
*Rehan Ahmad,Muhammad Kashif,Nouhaila Innan,Muhammad Shafique*

Main category: cs.LG

TL;DR: QML benchmarking framework shows quantum models can outperform classical counterparts in specific financial tasks when data structure and circuit design are properly aligned.


<details>
  <summary>Details</summary>
Motivation: To provide a fair, reproducible comparison between quantum machine learning (QML) models and architecture-matched classical methods in financial applications, identifying when current-generation QML offers tangible improvements.

Method: Developed a reproducible benchmarking framework with standardized data splits, features, and evaluation metrics. Tested QML models against classical counterparts on three financial tasks: directional return prediction (US/Turkish equities), live-trading simulation (Quantum vs classical LSTMs on S&P 500), and realized volatility forecasting (Quantum Support Vector Regression).

Result: Quantum approaches show performance gains when data structure and circuit design align: Hybrid quantum neural networks outperformed classical ANNs by +3.8 AUC/+3.4 accuracy on AAPL and +4.9 AUC/+3.6 accuracy on KCHOL. QLSTM achieved higher risk-adjusted returns in 2 of 4 S&P 500 regimes. Angle-encoded QSVR attained lowest QLIKE on KCHOL and remained competitive on S&P 500/AAPL.

Conclusion: The framework successfully identifies specific scenarios where current QML architectures offer improvements over classical methods, while also showing where classical approaches still dominate, providing clear guidance for practical QML application in finance.
...
### [152] [Detecting Semantic Backdoors in a Mystery Shopping Scenario](https://arxiv.org/abs/2601.03805)
*Arpad Berta,Gabor Danner,Istvan Hegedus,Mark Jelasity*

Main category: cs.LG

TL;DR: Proposes a method to detect semantic backdoors in ML models by training reference models, measuring distances between models, and using adversarial training to identify poisoned models.


<details>
  <summary>Details</summary>
Motivation: Semantic backdoors are harder to detect than trigger-based backdoors because they lack identifiable patterns. The paper addresses consumer protection scenarios where authorities need to verify if ML service providers have inserted backdoors during model training.

Method: Create a reference model pool by training clean and poisoned models using trusted infrastructure. Calibrate a model distance threshold to identify clean models. Use adversarial training from providers and measure model distances using input samples generated by inverting models to maximize distance from clean samples.

Result: The method can often completely separate clean and poisoned models. The most reliable approach uses adversarial training and model inversion for distance measurement, outperforming state-of-the-art backdoor detectors.

Conclusion: The proposed approach effectively detects semantic backdoors in classification models, especially when combined with adversarial training and model inversion techniques for distance measurement, providing superior detection capabilities compared to existing methods.
...
### [153] [Logic Tensor Network-Enhanced Generative Adversarial Network](https://arxiv.org/abs/2601.03839)
*Nijesh Upreti,Vaishak Belle*

Main category: cs.LG

TL;DR: LTN-GAN combines GANs with Logic Tensor Networks to enforce domain-specific logical constraints during sample generation, improving logical consistency while maintaining sample quality and diversity.


<details>
  <summary>Details</summary>
Motivation: Traditional GANs lack mechanisms to incorporate prior knowledge or enforce logical consistency, limiting their applicability in domains requiring rule adherence. There's a need to integrate logical reasoning with generative modeling for knowledge-intensive domains.

Method: Proposes LTN-GAN framework that enhances GANs by incorporating Logic Tensor Networks (LTNs), which integrate first-order logic with neural networks. This allows the model to reason over and satisfy logical constraints during the sample generation process.

Result: LTN-GAN significantly outperforms traditional GANs in adherence to predefined logical constraints while maintaining quality and diversity of generated samples across multiple datasets (synthetic: gaussian, grid, rings; and MNIST).

Conclusion: The work demonstrates the potential of neuro-symbolic approaches to enhance generative modeling in knowledge-intensive domains by combining GANs' realistic data synthesis with LTNs' logical reasoning capabilities.
...
### [154] [Feature-Aware One-Shot Federated Learning via Hierarchical Token Sequences](https://arxiv.org/abs/2601.03882)
*Shudong Liu,Hanwen Zhang,Xiuling Wang,Yuesheng Zhu,Guibo Luo*

Main category: cs.LG

TL;DR: FALCON improves one-shot federated learning for non-IID image data using hierarchical token sequences and knowledge distillation, achieving 9.58% higher accuracy than baselines.


<details>
  <summary>Details</summary>
Motivation: Existing one-shot federated learning methods struggle with robust performance on real-world domains like medical imaging and are inefficient with non-IID data, creating a need for better OSFL approaches.

Method: FALCON uses: 1) pretrained visual encoder with hierarchical scale encoding to compress images into hierarchical token sequences, 2) multi-scale autoregressive transformer generator to model and generate synthetic sequences, 3) clients upload synthetic sequences and local classifiers, 4) server uses knowledge distillation for global training.

Result: Experiments on medical and natural image datasets show FALCON outperforms best OSFL baselines by 9.58% in average accuracy across diverse non-IID scenarios.

Conclusion: FALCON effectively addresses OSFL limitations for non-IID image data through hierarchical token sequences and knowledge distillation, demonstrating significant performance improvements in challenging real-world applications.
...
### [155] [Spectral Manifold Regularization for Stable and Modular Routing in Deep MoE Architectures](https://arxiv.org/abs/2601.03889)
*Ibrahim Delibasoglu*

Main category: cs.LG

TL;DR: SR-MoE uses spectral regularization to prevent expert collapse in Mixture of Experts architectures, maintaining structural modularity and enabling stable lifelong learning.


<details>
  <summary>Details</summary>
Motivation: MoE architectures suffer from expert collapse where routing converges to few dominant experts, reducing model capacity and causing catastrophic interference during adaptation. This limits their effectiveness for lifelong learning scenarios.

Method: Proposes Spectrally-Regularized Mixture of Experts (SR-MoE) with dual regularization: spectral norm constraints bound routing function Lipschitz continuity, and stable rank penalties preserve high-dimensional feature diversity in expert selection.

Result: Traditional linear gating fails with increasing depth (accuracy drops up to 4.72% due to expert entanglement), while SR-MoE maintains structural integrity (mean interference -0.32%). Spectral constraints enable positive knowledge transfer and localized expert updates without global performance decay.

Conclusion: SR-MoE provides a general solution for building high-capacity, modular networks capable of stable lifelong learning by enforcing geometric constraints on the routing manifold to prevent expert collapse.
...
TL;DR: ABC-GRPO improves GRPO with adaptive asymmetric clipping for better RL with LLMs, achieving superior math reasoning performance and maintaining higher entropy.


<details>
  <summary>Details</summary>
Motivation: The authors identify that GRPO's clipping mechanism is suboptimal in certain scenarios, limiting flexibility and generalization in reinforcement learning with large language models.

Method: Propose Adaptive-Boundary-Clipping GRPO (ABC-GRPO), an asymmetric and adaptive refinement of the original GRPO framework that modifies the clipping mechanism.

Result: ABC-GRPO achieves superior performance over standard GRPO on mathematical reasoning tasks using Qwen3 LLMs, while maintaining substantially higher entropy throughout training.

Conclusion: ABC-GRPO enhances GRPO's flexibility and generalization, preserves exploration capacity, mitigates premature convergence, and the implementation is publicly available.
...
### [157] [A Gap Between Decision Trees and Neural Networks](https://arxiv.org/abs/2601.03919)
*Akash Kumar*

Main category: cs.LG

TL;DR: Shallow ReLU networks struggle to approximate axis-aligned decision trees with simple geometric boundaries due to infinite Radon total variation, but a smooth barrier score can achieve finite complexity while exactly recovering the decision set.


<details>
  <summary>Details</summary>
Motivation: The paper investigates the tension between interpretability (geometric simplicity of decision boundaries) and accurate approximation of axis-aligned decision trees by shallow neural networks. Decision trees produce rule-based, axis-aligned decision regions, while ReLU networks are typically trained as score models, creating a potential conflict between interpretability and approximation accuracy.

Method: The authors analyze infinite-width, bounded-norm, single-hidden-layer ReLU networks through the Radon total variation (RTV) seminorm, which controls geometric complexity of level sets. They examine different smoothing approaches for tree indicators (piecewise-linear ramp, sigmoidal, Gaussian convolution) and separate classification from score learning. They construct a smooth barrier score with finite RTV that exactly recovers the decision box when thresholded.

Result: Hard tree indicators have infinite RTV, and natural smoothing surrogates (piecewise-linear and sigmoidal) also have infinite RTV in dimensions d>1. Gaussian convolution yields finite RTV but with exponential dependence on d. The smooth barrier score achieves finite RTV with exact recovery of the decision set, and under mild conditions, provides polynomial decay in calibration bounds with explicit RTV upper bounds in terms of face measures.

Conclusion: There's a fundamental tradeoff between geometric simplicity (interpretability) and accurate approximation of axis-aligned decision trees by shallow ReLU networks. The paper demonstrates that while naive approaches lead to infinite complexity, carefully designed smooth barrier scores can achieve finite RTV while exactly recovering decision sets, providing insights into the accuracy-complexity tradeoff in neural network approximation.
...
### [158] [FOREVER: Forgetting Curve-Inspired Memory Replay for Language Model Continual Learning](https://arxiv.org/abs/2601.03938)
*Yujie Feng,Hao Wang,Jian Li,Xu Chu,Zhaolu Kang,Yiran Liu,Yasha Wang,Philip S. Yu,Xiao-Ming Wu*

Main category: cs.LG

TL;DR: FOREVER is a continual learning framework for LLMs that uses model-centric time based on optimizer update magnitude to align memory replay with the model's actual learning progress, inspired by human forgetting curves.


<details>
  <summary>Details</summary>
Motivation: Current memory replay methods for continual learning in LLMs rely on fixed step-based heuristics that often misalign with the model's actual learning progress, since identical training steps can result in varying degrees of parameter change. Recent findings that LLM forgetting mirrors the Ebbinghaus human forgetting curve motivate a more model-centric approach.

Method: FOREVER defines model time using the magnitude of optimizer updates rather than raw training steps. It incorporates: 1) a forgetting curve-based replay scheduler to determine when to replay, and 2) an intensity-aware regularization mechanism to adaptively control how to replay.

Result: Extensive experiments on three CL benchmarks with models ranging from 0.6B to 13B parameters demonstrate that FOREVER consistently mitigates catastrophic forgetting.

Conclusion: Aligning replay schedules with model-centric time based on optimizer update magnitude provides an effective approach to continual learning for LLMs, outperforming traditional step-based heuristics by better matching replay to the model's actual internal evolution.
...
### [159] [Stage-specific cancer survival prediction enriched by explainable machine learning](https://arxiv.org/abs/2601.03977)
*Parisa Poorhasani,Bogdan Iancu*

Main category: cs.LG

TL;DR: This paper develops explainable ML models for stage-specific cancer survival prediction using SEER data, addressing limitations of traditional models that combine all stages and lack interpretability.


<details>
  <summary>Details</summary>
Motivation: Traditional survival prediction models combine all cancer stages, potentially overestimating performance and ignoring stage-specific variations. There's limited research on explainable ML models for cancer survival analysis despite the clinical importance of understanding stage-specific factors.

Method: Used SEER dataset to create explainable ML models for stage-specific cancer survivability in colorectal, stomach, and liver cancers. Applied SHAP and LIME explainability techniques to reveal feature-cancer stage interactions that traditional black-box models would hide.

Result: Identified how demographic and clinical variables influence survival differently across cancer stages and types. Discovered significant feature-cancer stage interactions that provide transparency and clinical relevance for personalized treatment planning.

Conclusion: Stage-specific explainable ML models offer superior insights into important factors at each cancer stage, providing both transparency and clinical relevance to support personalized treatment planning, addressing limitations of traditional combined-stage approaches.
...
### [160] [Modeling Behavioral Patterns in News Recommendations Using Fuzzy Neural Networks](https://arxiv.org/abs/2601.04019)
...
Method: Uses fuzzy neural networks to learn human-readable rules from behavioral data for predicting article clicks. Extracts rules at configurable thresholds to control rule complexity and interpretability level.

Result: Evaluated on MIND and EB-NeRD datasets, showing accurate click prediction comparable to established baselines while learning human-readable rules. The learned rules reveal news consumption patterns.

Conclusion: The approach provides transparent recommendations that enable editors to understand audience behavior and align content curation goals, bridging the gap between black-box accuracy and editorial transparency.
...
### [161] [Symbolic Regression for Shared Expressions: Introducing Partial Parameter Sharing](https://arxiv.org/abs/2601.04051)
*Viktor Martinek,Roland Herzog*

Main category: cs.LG

TL;DR: This paper extends symbolic regression to handle multiple categorical variables with intermediate parameter sharing levels, reducing parameter count while revealing more problem structure.


<details>
  <summary>Details</summary>
Motivation: To improve symbolic regression for scientific discovery by allowing more flexible parameter sharing across multiple categorical variables, enabling better interpretability and reduced data requirements.

Method: Extends symbolic regression to handle multiple categorical variables with intermediate parameter sharing levels (shared across some categories but not others), tested on synthetic and astrophysics datasets.

Result: Achieves similar fit quality with significantly fewer parameters, reduces data requirements, enables transfer learning, and extracts additional structural information about problems.

Conclusion: The multi-categorical approach with intermediate parameter sharing enhances symbolic regression's efficiency and interpretability for scientific discovery applications.
...
### [162] [LinkD: AutoRegressive Diffusion Model for Mechanical Linkage Synthesis](https://arxiv.org/abs/2601.04054)
*Yayati Jadhav,Amir Barati Farimani*

Main category: cs.LG

TL;DR: Autoregressive diffusion framework for inverse design of mechanical linkages using sequential graph generation conditioned on target trajectories.


<details>
  <summary>Details</summary>
Motivation: Traditional linkage design is challenging due to nonlinear motion-to-configuration relationships, combinatorial design space, and computational intractability of conventional optimization methods for complex mechanisms.

Method: Combines causal transformer for discrete topology prediction with DDPM for spatial coordinate refinement, both conditioned on target trajectories via transformer encoder. Uses sequential graph generation where nodes=joints and edges=links, enabling adaptive trial-and-error synthesis with selective regeneration of problematic nodes.

Result: Successfully synthesizes linkage systems with up to 20 nodes, demonstrating extensibility to N-node architectures. Surpasses traditional optimization approaches in scalability and generalization to arbitrary node counts.

Conclusion: Establishes new paradigms for scalable inverse design of complex mechanical systems, advancing both autoregressive graph generation methodologies and computational kinematic synthesis.
...
### [163] [Using Legacy Polysomnography Data to Train a Radar System to Quantify Sleep in Older Adults and People living with Dementia](https://arxiv.org/abs/2601.04057)
*M. Yin,K. G. Ravindran,C. Hadjipanayi,A. Bannon,A. Rapeaux,C. Della Monica,T. S. Lande,Derk-Jan Dijk,T. G. Constandinou*

Main category: cs.LG

TL;DR: Deep transfer learning framework using adversarial domain adaptation improves sleep stage classification from UWB radar data, achieving 79.5% accuracy for 4-stage classification in older adults.


<details>
  <summary>Details</summary>
Motivation: UWB radar offers unobtrusive in-home sleep monitoring but faces challenges due to limited radar sleep data availability, making it difficult to build robust models that generalize across diverse populations and environments.

Method: Developed end-to-end neural network for sleep stage classification using respiratory and motion signals. Used domain adaptation with adversarial learning to bridge knowledge gap between large-scale PSG datasets and limited radar data.

Result: Achieved 79.5% accuracy with Kappa 0.65 for 4-stage classification (wakefulness, REM, light sleep, deep sleep) on radar dataset of 47 older adults, including those with prodromal/mild Alzheimer disease.

Conclusion: Deep transfer learning effectively addresses data variability and limited sample size challenges, improving reliability of automatic sleep staging models when radar data is limited, supporting UWB radar as viable nonintrusive sleep assessment tool.
...
### [164] [Minimum distance classification for nonlinear dynamical systems](https://arxiv.org/abs/2601.04058)
*Dominique Martinez*

Main category: cs.LG

TL;DR: Dynafit: A kernel-based method for classifying trajectory data from different nonlinear dynamical systems by learning a distance metric between training trajectories and underlying dynamics using Koopman operator approximation.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the problem of classifying trajectory data generated by nonlinear dynamics, where each class corresponds to a distinct dynamical system. This is important for various applications involving nonlinear dynamical systems and sensors.

Method: Proposes Dynafit, a kernel-based method that learns a distance metric between training trajectories and underlying dynamics. The algorithm approximates the Koopman operator to globally linearize dynamics in a feature space associated with a kernel function. Uses kernel trick to compute distance metric independently of dimensionality. Can incorporate partial knowledge of dynamics when available.

Result: Demonstrates effectiveness on three examples: chaos detection with logistic map, recognition of handwritten dynamics, and recognition of visual dynamic textures. Shows applicability to various classification tasks involving nonlinear dynamical systems.

Conclusion: Dynafit provides an effective kernel-based approach for classifying trajectory data from different nonlinear dynamical systems by learning a distance metric through Koopman operator approximation, with demonstrated success across multiple application domains.
...
### [165] [Causal Data Augmentation for Robust Fine-Tuning of Tabular Foundation Models](https://arxiv.org/abs/2601.04110)
...
<details>
  <summary>Details</summary>
Motivation: Fine-tuning tabular foundation models with limited data is challenging because early stopping on scarce validation data fails to accurately capture true generalization performance, leading to unreliable model selection.

Method: Proposes CausalMixFT, which generates structurally consistent synthetic samples using Structural Causal Models (SCMs) fitted on target datasets. This causally-informed data augmentation preserves feature dependencies while expanding training diversity for more robust fine-tuning.

Result: Evaluated on 33 classification datasets with over 2300 fine-tuning runs, CausalMixFT improved median normalized ROC-AUC from 0.10 to 0.12, outperforming statistical generators like CTGAN, TabEBM, and TableAugment. It also reduced the validation-test performance correlation gap from 0.67 to 0.30, enabling more reliable early stopping.

Conclusion: Incorporating causal structure into data augmentation provides an effective and principled approach for fine-tuning tabular foundation models in low-data regimes, improving both performance and validation reliability.
...
### [166] [MORPHFED: Federated Learning for Cross-institutional Blood Morphology Analysis](https://arxiv.org/abs/2601.04121)
*Gabriel Ansah,Eden Ruffell,Delmiro Fernandez-Reyes,Petru Manescu*

Main category: cs.LG

TL;DR: Federated learning framework for white blood cell analysis enables privacy-preserving collaborative training across institutions without data sharing, improving generalization to unseen sites.


<details>
  <summary>Details</summary>
Motivation: Blood morphology analysis in LMICs is sensitive to dataset shifts from staining variability, imaging differences, and rare morphologies. Centralized datasets are infeasible due to privacy regulations and data-sharing restrictions.

Method: Introduces a federated learning framework for white blood cell morphology analysis that enables collaborative training across institutions without exchanging training data. Uses blood films from multiple clinical sites to learn robust, domain-invariant representations while preserving data privacy.

Result: Federated models achieve strong cross-site performance and improved generalization to unseen institutions compared to centralized training, across both convolutional and transformer-based architectures.

Conclusion: Federated learning is a practical and privacy-preserving approach for developing equitable, scalable, and generalizable medical imaging AI in resource-limited healthcare environments.
...
### [167] [Clinical Data Goes MEDS? Let's OWL make sense of it](https://arxiv.org/abs/2601.04164)
*Alberto Marfoglia,Jong Ho Jhee,Adrien Coulet*

Main category: cs.LG

TL;DR: MEDS-OWL is an OWL ontology that bridges the Medical Event Data Standard (MEDS) with Semantic Web technologies, enabling representation of clinical event data as RDF graphs for improved interoperability and FAIR compliance.


<details>
  <summary>Details</summary>
Motivation: Machine learning in healthcare faces challenges with data standardization and semantic explicitness, limiting interoperability and reproducibility. While MEDS provides an event-centric data model, it lacks integration with Semantic Web ecosystems, hindering broader data sharing and analysis.

Method: Developed MEDS-OWL, a lightweight OWL ontology with formal concepts and relations for representing MEDS datasets as RDF graphs. Created meds2rdf, a Python library to convert MEDS events into RDF graphs conforming to the ontology. Demonstrated on synthetic clinical data for ruptured intracranial aneurysms and validated using SHACL constraints.

Result: MEDS-OWL release includes 13 classes, 10 object properties, 20 data properties, and 24 OWL axioms. The approach enables transformation of clinical event data into FAIR-aligned datasets, supports provenance-aware publishing, and enhances interoperability. Validation with SHACL confirms graph consistency.

Conclusion: MEDS-OWL bridges MEDS with Semantic Web technologies, providing a reusable semantic layer for event-based clinical data and establishing a foundation for graph-based analytics while promoting FAIR principles and interoperability.
...
### [168] [Agentic Rubrics as Contextual Verifiers for SWE Agents](https://arxiv.org/abs/2601.04171)
*Mohit Raghavendra,Anisha Gunjal,Bing Liu,Yunzhong He*

Main category: cs.LG

TL;DR: Agentic Rubrics: Using expert agents to create context-grounded rubric checklists for verifying software patches without test execution, achieving significant performance gains over baselines.


<details>
  <summary>Details</summary>
Motivation: Current verification methods for software engineering agents rely heavily on code execution which is difficult to scale, while alternative methods like patch classifiers lack codebase context and interpretability.

Method: Expert agents interact with repositories to create context-grounded rubric checklists, then candidate patches are scored against these rubrics without requiring test execution.

Result: Achieved 54.2% on Qwen3-Coder-30B-A3B and 40.6% on Qwen3-32B on SWE-Bench Verified, with at least +3.5 percentage-point gain over strongest baselines. Rubric scores align with ground-truth tests while catching additional issues.

Conclusion: Agentic Rubrics provide efficient, scalable, and granular verification signals for software engineering agents, with agentic context gathering being essential for producing codebase-specific criteria.
...
### [169] [Robust Physics Discovery from Highly Corrupted Data: A PINN Framework Applied to the Nonlinear Schrödinger Equation](https://arxiv.org/abs/2601.04176)
*Pietro de Oliveira Esteves*

Main category: cs.LG

TL;DR: PINNs with automatic differentiation recover NLSE parameters from noisy data with <0.2% error using only 500 sparse points, outperforming traditional methods in high-noise regimes.
...
### [170] [Lightweight Test-Time Adaptation for EMG-Based Gesture Recognition](https://arxiv.org/abs/2601.04181)
*Nia Touko,Matthew O A Ellis,Cristiano Capone,Alessio Burrello,Elisa Donati,Luca Manneschi*

Main category: cs.LG

TL;DR: Lightweight test-time adaptation framework for EMG signal drift using TCN backbone with three deployment strategies for robust long-term prosthetic control.


<details>
  <summary>Details</summary>
Motivation: Surface EMG decoding suffers from signal drift due to electrode shifts, muscle fatigue, and posture changes, causing performance degradation in existing models. Current solutions require large datasets or high-compute pipelines impractical for energy-efficient wearables.

Method: Proposes a lightweight TTA framework with Temporal Convolutional Network backbone and three strategies: (1) causal adaptive batch normalization for real-time statistical alignment, (2) GMM alignment with experience replay to prevent forgetting, and (3) meta-learning for rapid few-shot calibration.

Result: Framework significantly bridges inter-session accuracy gap with minimal overhead on NinaPro DB6 dataset. Experience-replay updates provide superior stability under limited data, while meta-learning achieves competitive performance in one- and two-shot regimes using only a fraction of data required by benchmarks.

Conclusion: Establishes a path toward robust, "plug-and-play" myoelectric control for long-term prosthetic use with lightweight, deployment-ready adaptation strategies.
...
### [171] [Mastering the Game of Go with Self-play Experience Replay](https://arxiv.org/abs/2601.03306)
...
<details>
  <summary>Details</summary>
Motivation: Previous Go AI systems like AlphaGo rely on computationally expensive model-based MCTS. The authors aim to demonstrate that model-free reinforcement learning can efficiently master complex games like Go without search during training.

Method: QZero uses entropy-regularized Q-learning with a single Q-value network that unifies policy evaluation and improvement. It learns through self-play and off-policy experience replay, starting from scratch without human data.

Result: Trained for 5 months with modest compute (7 GPUs), QZero achieved performance comparable to AlphaGo, demonstrating efficient mastery of Go without search during training.

Conclusion: This work shows model-free RL can efficiently master complex games like Go, and demonstrates the feasibility of off-policy RL for solving large-scale, complex environments.
...
### [172] [Digital Red Queen: Adversarial Program Evolution in Core War with LLMs](https://arxiv.org/abs/2601.03335)
*Akarsh Kumar,Ryan Bahlous-Boldi,Prafull Sharma,Phillip Isola,Sebastian Risi,Yujin Tang,David Ha*

Main category: cs.AI

TL;DR: DRQ is a self-play algorithm using LLMs to evolve Core War warriors through Red Queen dynamics, where programs continually adapt to defeat previous versions, leading to increasingly general and convergent strategies.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based evolution frameworks treat problems as static optimization, missing the open-ended adversarial dynamics of real-world evolution. The authors aim to study Red Queen dynamics where continual adaptation to changing objectives drives evolution.

Method: Digital Red Queen (DRQ) uses LLMs to evolve assembly-like programs (warriors) that compete in Core War, a Turing-complete virtual machine environment. In each round, the LLM evolves a new warrior to defeat all previous ones, creating a sequence of adapted warriors through self-play.

Result: Warriors become increasingly general (relative to held-out human warriors) over rounds. Interestingly, warriors also show reduced behavioral diversity across independent runs, indicating convergence toward a general-purpose behavioral strategy similar to convergent evolution in nature.

Conclusion: Shifting from static to dynamic Red Queen objectives has value for LLM-based evolution. Core War serves as a rich sandbox for studying adversarial adaptation, and DRQ's simplicity suggests similar self-play approaches could be useful in practical adversarial domains like cybersecurity and combating drug resistance.
...
### [173] [Enhancing LLM Instruction Following: An Evaluation-Driven Multi-Agentic Workflow for Prompt Instructions Optimization](https://arxiv.org/abs/2601.03359)
...
<details>
  <summary>Details</summary>
Motivation: LLMs often produce conceptually correct content but fail to adhere to formal constraints, creating procedurally flawed outputs. Traditional prompt refinement focuses only on rephrasing the primary task description, ignoring granular constraints that serve as acceptance criteria.

Method: A novel multi-agentic workflow that separates optimization of the primary task description from its constraints. Uses quantitative scores as feedback to iteratively rewrite and improve both task descriptions and constraints.

Result: The method produces revised prompts that yield significantly higher compliance scores from models like Llama 3.1 8B and Mixtral-8x 7B.

Conclusion: Decoupling task description optimization from constraint refinement using quantitative feedback is an effective approach for improving LLM prompt compliance with formal constraints.
...
### [174] [Exploration Through Introspection: A Self-Aware Reward Model](https://arxiv.org/abs/2601.03389)
*Michael Petrowski,Milica Gašić*

Main category: cs.AI

TL;DR: RL agents with introspective pain-belief inference outperform baselines and show human-like behaviors.


<details>
  <summary>Details</summary>
Motivation: To advance Theory of Mind in AI by exploring self-awareness through internal state inference, inspired by biological pain as a learning signal.

Method: Introduce introspective exploration using hidden Markov model to infer "pain-belief" from online observations, integrated into subjective reward function for RL agents in gridworlds.
...
Conclusion: Self-awareness through pain-belief inference enhances RL agent performance and provides computational framework for studying normal vs. chronic pain perception differences.
...
### [175] [Toward Maturity-Based Certification of Embodied AI: Quantifying Trustworthiness Through Measurement Mechanisms](https://arxiv.org/abs/2601.03470)
*Michael C. Darling,Alan H. Hesu,Michael A. Mardikes,Brian C. McGuigan,Reed M. Milewicz*

Main category: cs.AI

TL;DR: A maturity-based certification framework for embodied AI systems with structured assessment, quantitative scoring, and multi-objective trade-off navigation, demonstrated through uncertainty quantification in UAS detection.


<details>
  <summary>Details</summary>
Motivation: The need for certifiable embodied AI systems requires structured assessment frameworks with explicit measurement mechanisms to ensure trustworthiness and safety in real-world applications.

Method: Proposes a maturity-based certification framework with three key components: structured assessment frameworks, quantitative scoring mechanisms, and methods for navigating multi-objective trade-offs in trustworthiness evaluation. Uses uncertainty quantification as an exemplar measurement mechanism.

Result: Demonstrates feasibility through an Uncrewed Aircraft System (UAS) detection case study, showing the practical application of the proposed certification framework.

Conclusion: A maturity-based framework with explicit measurement mechanisms provides a viable approach for certifying embodied AI systems, addressing the need for structured, quantifiable assessment of trustworthiness in real-world applications.
...
### [176] [CPGPrompt: Translating Clinical Guidelines into LLM-Executable Decision Support](https://arxiv.org/abs/2601.03475)
*Ruiqi Deng,Geoffrey Martin,Tony Wang,Gongbo Zhang,Yi Liu,Chunhua Weng,Yanshan Wang,Justin F Rousseau,Yifan Peng*

Main category: cs.AI

TL;DR: CPGPrompt is an auto-prompting system that converts narrative clinical guidelines into LLM-executable decision trees, achieving strong performance on binary specialty referral decisions but variable results on multi-class pathway assignments across headache, lower back pain, and prostate cancer domains.


<details>
  <summary>Details</summary>
Motivation: Clinical practice guidelines provide evidence-based care recommendations but are difficult to integrate into AI systems. Existing approaches like rule-based systems have poor interpretability, inconsistent guideline adherence, and narrow domain applicability.

Method: Developed CPGPrompt framework that translates CPGs into structured decision trees and uses LLMs to dynamically navigate them for patient evaluation. Tested with synthetic vignettes across three domains (headache, lower back pain, prostate cancer) in four decision scenario categories.

Result: Binary specialty referral classification achieved strong performance (F1: 0.85-1.00) with perfect recall (1.00 ± 0.00). Multi-class pathway assignment showed domain-specific variations: headache (F1: 0.47), lower back pain (F1: 0.72), prostate cancer (F1: 0.77). Performance differences reflected guideline structure challenges.

Conclusion: CPGPrompt effectively converts narrative guidelines into LLM-executable decision trees, showing promise for binary decision tasks but highlighting challenges with complex multi-class pathway assignments, particularly with negation handling and temporal reasoning requirements.
...
### [177] [Personalization of Large Foundation Models for Health Interventions](https://arxiv.org/abs/2601.03482)
*Stefan Konigorski,Johannes E. Vedder,Babajide Alamu Owoyele,İbrahim Özkan*

Main category: cs.AI

TL;DR: LFMs can't replace N-of-1 trials for personalized medicine; they're complementary. LFMs generate hypotheses from population data, N-of-1 trials provide causal validation for individuals. A hybrid framework combines both to navigate AI paradoxes in healthcare.


<details>
  <summary>Details</summary>
Motivation: Large foundation models (LFMs) are transforming healthcare AI but face fundamental challenges for true personalization, including the generalizability paradox (models performing well in one study but at chance level in others). There are broader contradictions in AI-driven healthcare: privacy-performance paradox, scale-specificity paradox, and automation-empathy paradox. The degree of causal understanding needed for personalized recommendations versus mere predictive capacity remains unclear.

Method: The paper argues that LFMs and N-of-1 trials (gold standard for individual causal inference) are complementary rather than substitutive. It proposes a hybrid framework: LFMs generate ranked intervention candidates with uncertainty estimates from population patterns using multimodal data, which then trigger subsequent N-of-1 trials for causal validation at the individual level.

Result: The analysis shows that LFMs cannot replace N-of-1 trials for personalized medicine. N-of-1 trials resolve the identified tensions by providing within-person causal evidence while preserving privacy through local experimentation. The complementary approach leverages LFMs' strengths in rapid hypothesis generation and N-of-1 trials' strengths in causal validation.

Conclusion: Clarifying the boundary between prediction and causation and explicitly addressing paradoxical tensions are essential for responsible AI integration in personalized medicine. The proposed hybrid framework combining LFMs and N-of-1 trials enables true personalization while navigating the identified paradoxes in AI-driven healthcare.
...
### [178] [Evolving Programmatic Skill Networks](https://arxiv.org/abs/2601.03509)
...
<details>
  <summary>Details</summary>
Motivation: To enable agents to construct, refine, and reuse an expanding library of skills in open-ended embodied environments where continual learning and adaptation are essential.

Method: Programmatic Skill Network (PSN) uses executable symbolic programs as skills in a compositional network. It employs three LLM-based mechanisms: REFLECT for fault localization, progressive optimization with maturity-aware gating, and canonical structural refactoring with rollback validation.
...
Conclusion: PSN provides an effective framework for continual skill acquisition with structural parallels to neural network training, enabling stable skill evolution while maintaining learning plasticity.
...
### [179] [Variance Computation for Weighted Model Counting with Knowledge Compilation Approach](https://arxiv.org/abs/2601.03523)
*Kengo Nakamura,Masaaki Nishino,Norihito Yasuda*

Main category: cs.AI

TL;DR: The paper studies the tractability of computing variance in weighted model counting (WMC) for probabilistic inference, developing polynomial-time algorithms for structured d-DNNFs while proving hardness for other representations, with applications to Bayesian network uncertainty analysis.


<details>
  <summary>Details</summary>
Motivation: In practical probabilistic inference tasks, model parameters have uncertainty since they're learned from data. This creates a need to quantify uncertainty in inference outcomes by treating outcomes as random variables with parameter distributions and computing their variance. However, the tractability of computing such variance is largely unknown.

Method: The authors investigate the tractability of computing variance in weighted model counting (WMC). They develop a polynomial-time algorithm for evaluating WMC variance when inputs are structured d-DNNFs. They also prove hardness results for structured DNNFs, d-DNNFs, and FBDDs. Finally, they apply their approach to measure uncertainty in Bayesian network inference.

Result: 1) A polynomial-time algorithm for computing WMC variance on structured d-DNNFs. 2) Hardness proofs showing the problem is intractable for structured DNNFs, d-DNNFs, and FBDDs (notably, the latter two allow polynomial-time WMC algorithms). 3) Empirical demonstration that the algorithm can evaluate variance of marginal probabilities on real-world Bayesian networks and analyze parameter variance impact.

Conclusion: The paper establishes a computational complexity landscape for WMC variance computation, showing tractability for structured d-DNNFs but hardness for other representations. This provides theoretical foundations for uncertainty quantification in probabilistic inference and enables practical applications like analyzing uncertainty in Bayesian network inference outcomes.
...
### [180] [STAR-S: Improving Safety Alignment through Self-Taught Reasoning on Safety Rules](https://arxiv.org/abs/2601.03537)
*Di Wu,Yanyan Zhao,Xin Lu,Mingzhe Li,Bing Qin*

Main category: cs.AI

TL;DR: STAR-S is a self-taught framework that improves LLM safety by iteratively learning to reason about safety rules, defending against jailbreak attacks better than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current approaches to LLM safety that train models to reason over safety rules have limitations - it's difficult to explicitly design or directly obtain effective safety reasoning that can defend against jailbreak attacks.

Method: STAR-S integrates safety rule reasoning learning into a self-taught loop: eliciting reasoning and reflection guided by safety rules, then fine-tuning to enhance safety reasoning, creating a synergistic cycle where improved reasoning generates better training data for further refinement.

Result: Experiments show STAR-S effectively defends against jailbreak attacks and outperforms baseline methods.

Conclusion: The self-taught framework for learning safety reasoning provides an effective approach to defending LLMs against jailbreak attacks, with code publicly available for implementation.
...
### [181] [ReEfBench: Quantifying the Reasoning Efficiency of LLMs](https://arxiv.org/abs/2601.03550)
*Zhizhang Fu,Yuancheng Gu,Chenkai Hu,Hanmeng Liu,Yue Zhang*

Main category: cs.AI

TL;DR: Test-time scaling improves LLM reasoning but current CoT evaluation can't distinguish between genuine reasoning and just generating more tokens. The paper proposes a neuro-symbolic framework for better process evaluation, identifies behavioral patterns, and finds that longer outputs don't guarantee better reasoning.


<details>
  <summary>Details</summary>
Motivation: Current Chain-of-Thought (CoT) evaluation methods are limited in distinguishing whether performance improvements from test-time scaling come from actual reasoning improvements or just from generating more verbose outputs. There's a need for better evaluation that can assess the reasoning process itself rather than just final outputs.

Method: Proposes a novel neuro-symbolic framework for non-intrusive, comprehensive process-centric evaluation of reasoning. Uses this framework to identify four distinct behavioral prototypes and diagnose failure modes. Examines the impact of inference mode, training strategy, and model scale on reasoning performance.

Result: Extended token generation is not necessary for deep reasoning. Mixing long and short CoT data in training risks premature saturation and collapse. Distillation into smaller models captures behavioral length but fails to replicate logical efficacy due to intrinsic capacity limits.

Conclusion: The paper provides a better evaluation framework for assessing LLM reasoning, reveals that longer outputs don't guarantee better reasoning, and identifies critical constraints in training strategies and model scaling for reasoning capabilities.
...
### [182] [SCRIBE: Structured Mid-Level Supervision for Tool-Using Language Models](https://arxiv.org/abs/2601.03555)
*Yuxuan Jiang,Francis Ferraro*

Main category: cs.AI

TL;DR: SCRIBE introduces a reinforcement learning framework using skill-conditioned rewards with intermediate behavioral evaluation to improve credit assignment in multi-step reasoning for tool-augmented agents.
...
Method: SCRIBE intervenes at a novel mid-level abstraction, grounding reward modeling in a curated library of skill prototypes. It transforms open-ended LLM evaluation into a constrained verification problem by routing each subgoal to a corresponding prototype, equipping the reward model with precise, structured rubrics that reduce reward variance.

Result: SCRIBE achieves state-of-the-art performance across reasoning and tool-use benchmarks. It improves AIME25 accuracy of a Qwen3-4B model from 43.3% to 63.3%, and significantly increases success rates in complex multi-turn tool interactions. Training dynamics show co-evolution across abstraction levels where mid-level skill mastery precedes effective high-level planning.

Conclusion: SCRIBE provides a scalable and complementary pathway toward more autonomous and reliable tool-using agents, demonstrating additive benefits to low-level tool optimizations through its skill-conditioned reward framework.
...
TL;DR: SAE-Steering: A method to control reasoning strategies in Large Reasoning Models by using Sparse Autoencoders to disentangle strategy features and identify strategy-specific control vectors.


<details>
  <summary>Details</summary>
Motivation: Current LRMs autonomously select reasoning strategies which often leads to inefficient or erroneous reasoning paths. Existing methods struggle to control fine-grained reasoning strategies due to conceptual entanglement in hidden states.

Method: Use Sparse Autoencoders (SAEs) to decompose strategy-entangled hidden states into disentangled features. Propose SAE-Steering, a two-stage feature identification pipeline: 1) recall features that amplify logits of strategy-specific keywords (filtering >99% features), 2) rank remaining features by control effectiveness.

Result: SAE-Steering outperforms existing methods by over 15% in control effectiveness. Controlling reasoning strategies redirects LRMs from erroneous paths to correct ones, achieving 7% absolute accuracy improvement.

Conclusion: SAE-Steering enables reliable and flexible control over reasoning strategies in LRMs by effectively identifying and utilizing strategy-specific features, significantly improving reasoning performance and accuracy.
...
### [184] [Interleaved Tool-Call Reasoning for Protein Function Understanding](https://arxiv.org/abs/2601.03604)
*Chuanliu Fan,Zicheng Ma,Huanran Meng,Aijia Zhang,Wenjie Du,Jun Zhang,Yi Qin Gao,Ziqiang Cao,Guohong Fu*

Main category: cs.AI

TL;DR: PFUA is a tool-augmented protein reasoning agent that outperforms text-only LLMs by 103% on protein function prediction benchmarks by integrating domain-specific tools rather than relying on chain-of-thought reasoning.


<details>
  <summary>Details</summary>
Motivation: Direct transfer of text-based chain-of-thought reasoning from symbolic domains (math/programming) to protein function understanding is ineffective - reinforcement learning amplifies superficial keyword patterns without introducing new biological knowledge, limiting generalization. Protein function prediction requires external biological priors and computational tools rather than purely internal reasoning.
...
Conclusion: Protein function prediction is a knowledge-intensive scientific task requiring external tools and biological priors, not just internal reasoning. Tool-augmented approaches like PFUA are essential for effective protein function understanding.
...
### [185] [Architecting Agentic Communities using Design Patterns](https://arxiv.org/abs/2601.03624)
...
<details>
  <summary>Details</summary>
Motivation: The rapid evolution of LLMs and Agentic AI requires systematic architectural guidance for building production-grade systems, especially for enterprise applications where coordination between AI agents and humans is critical.

Method: The approach uses design patterns derived from enterprise distributed systems standards, formal methods, and industry practice, classified into three tiers: LLM Agents, Agentic AI, and Agentic Communities. It focuses on Agentic Communities with formal frameworks specifying collaboration agreements where AI agents and humans fill roles within governed ecosystems.

Result: The framework provides both practical guidance and formal verification capabilities, enabling expression of organizational, legal, and ethical rules through accountability mechanisms. It was validated through a clinical trial matching case study.

Conclusion: The approach provides actionable guidance to practitioners while maintaining formal rigor essential for enterprise deployment in dynamic, multi-agent ecosystems, addressing the need for verifiable governance of inter-agent communication, negotiation, and intent modeling.
...
### [186] [How Does the Thinking Step Influence Model Safety? An Entropy-based Safety Reminder for LRMs](https://arxiv.org/abs/2601.03662)
*Su-Hyeon Kim,Hyundong Jin,Yejin Lee,Yo-Sub Han*

Main category: cs.AI

TL;DR: SafeRemind is a decoding-time defense method that injects safe-reminding phrases into thinking steps of Large Reasoning Models to prevent unsafe behavior amplification without parameter updates.
...
Method: SafeRemind dynamically injects safe-reminding phrases into thinking steps during decoding. It uses entropy triggers to intervene at decision-locking points, redirecting potentially harmful trajectories toward safer outcomes without requiring any parameter updates.
...
Conclusion: SafeRemind effectively addresses the novel safety risks introduced by thinking steps in Large Reasoning Models through targeted, dynamic intervention during the reasoning process, maintaining safety without compromising reasoning capabilities.
...
### [187] [Sandwich Reasoning: An Answer-Reasoning-Answer Approach for Low-Latency Query Correction](https://arxiv.org/abs/2601.03672)
*Chen Zhang,Kepu Zhang,Jiatong Zhang,Xiao Zhang,Jun Xu*

Main category: cs.AI

TL;DR: SandwichR: A novel Answer-Reasoning-Answer approach for query correction that achieves CoT-level accuracy with 40-70% lower latency by aligning initial answers with post-hoc reasoning through consistency-aware RL.


<details>
  <summary>Details</summary>
Motivation: Query correction requires high accuracy under real-time latency constraints. Chain-of-Thought reasoning improves accuracy but is too slow for real-time use. Early answer approaches reduce latency but can't leverage reasoning to improve accuracy since the answer is generated before reasoning.

Method: Proposes Sandwich Reasoning (SandwichR) with Answer-Reasoning-Answer paradigm: initial correction → explicit reasoning → final refined correction. Uses consistency-aware reinforcement learning with dedicated consistency reward to align initial and final corrections, plus margin-based rejection sampling to prioritize borderline cases where reasoning provides most benefit. Also constructs a specialized query correction dataset.

Result: Achieves state-of-the-art accuracy comparable to standard Chain-of-Thought reasoning while delivering 40-70% latency reduction, effectively resolving the latency-accuracy trade-off for online search query correction.

Conclusion: SandwichR enables low-latency query correction without sacrificing reasoning-aware accuracy by explicitly aligning fast initial answers with post-hoc reasoning, making it suitable for real-time search applications.
...
### [188] [Personalized Medication Planning via Direct Domain Modeling and LLM-Generated Heuristics](https://arxiv.org/abs/2601.03687)
*Yonatan Vernik,Alexander Tuisov,David Izhaki,Hana Weitman,Gal A. Kaminka,Alexander Shleyfman*

Main category: cs.AI

TL;DR: Using LLM-generated domain-specific heuristics with GBFS search dramatically improves medication planning scalability from 7 to 28+ medications, making it more clinically practical.


<details>
  <summary>Details</summary>
Motivation: Previous automated medication planning using general heuristics was limited to only 7 medications, which is clinically insufficient. There's a need to scale up personalized medication planning to handle more realistic numbers of medications for practical clinical applications.

Method: Programmatically specify the domain (initial state and successor generation), then use an LLM to generate problem-specific heuristics that are used with a fixed GBFS (Greedy Best-First Search) algorithm instead of general domain-independent heuristics.

Result: Dramatic improvements in coverage and planning time, scaling up from 7 to at least 28 medications, bringing medication planning closer to practical clinical applications.

Conclusion: Automatically-generated domain-specific heuristics using LLMs enable significant scaling of personalized medication planning, making it more viable for real-world clinical use by handling much larger medication sets.
...
### [189] [EntroCoT: Enhancing Chain-of-Thought via Adaptive Entropy-Guided Segmentation](https://arxiv.org/abs/2601.03769)
...
<details>
  <summary>Details</summary>
Motivation: Existing fine-tuning datasets for Chain-of-Thought prompting often contain "answer right but reasoning wrong" problems where correct final answers come from hallucinated, redundant, or logically invalid intermediate steps, which degrades model performance.

Method: EntroCoT uses an entropy-based mechanism to segment reasoning traces into steps at uncertain junctures, then employs Monte Carlo rollout-based evaluation to assess each step's marginal contribution to the final answer, filtering out deceptive reasoning samples.
...
Conclusion: EntroCoT effectively identifies and filters low-quality reasoning traces, creating higher-quality training data that improves mathematical reasoning performance compared to using unfiltered datasets.
...
### [190] [ROI-Reasoning: Rational Optimization for Inference via Pre-Computation Meta-Cognition](https://arxiv.org/abs/2601.03822)
*Muyang Zhao,Qi Qi,Hao Sun*

Main category: cs.AI

TL;DR: ROI-Reasoning is a framework that teaches LLMs to strategically allocate computation under strict token budgets by predicting task difficulty and expected returns, improving performance on budgeted reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: LLMs lack inherent understanding of how much computation different tasks require, making them inefficient under strict global token constraints. There's a need for models to make strategic solve-or-skip decisions based on anticipated difficulty and return on investment.

Method: Two-stage framework: 1) Meta-Cognitive Fine-Tuning teaches models to predict reasoning cost and expected utility before generation, enabling explicit solve-or-skip decisions. 2) Rationality-Aware Reinforcement Learning optimizes sequential decision making under hard token budgets for long-horizon allocation strategies.

Result: ROI-Reasoning consistently improves overall score across budgeted mathematical reasoning benchmarks while substantially reducing regret under tight computation budgets.

Conclusion: The framework successfully endows LLMs with intrinsic, budget-aware rationality by formalizing budgeted inference as an Ordered Stochastic Multiple-Choice Knapsack Problem and teaching models strategic computation allocation.
...
### [191] [Defeasible Conditionals using Answer Set Programming](https://arxiv.org/abs/2601.03840)
*Racquel Dennison,Jesse Heyninck,Thomas Meyer*

Main category: cs.AI

TL;DR: ASP-based declarative approach for computing Rational Closure in defeasible reasoning, with formal correctness proof and empirical performance improvements over existing imperative implementations.


<details>
  <summary>Details</summary>
Motivation: The KLM framework provides foundational properties for defeasible reasoning, but existing implementations (like InfOCF) are imperative. There's a need for declarative approaches using Answer Set Programming that can automatically construct minimal ranked models and support entailment checking.

Method: Developed a declarative ASP encoding for computing Rational Closure. The approach automatically constructs minimal ranked models from knowledge bases and supports entailment checking. Formally proved the correctness of the ASP encoding.

Result: Empirical evaluations show the ASP-based approach adheres to RC's theoretical foundations and offers improved computational efficiency compared to existing imperative implementations (InfOCF solver).

Conclusion: ASP provides an effective declarative framework for implementing Rational Closure in defeasible reasoning, offering both theoretical correctness and practical performance advantages over traditional imperative approaches.
...
### [192] [XAI-LAW: A Logic Programming Tool for Modeling, Explaining, and Learning Legal Decisions](https://arxiv.org/abs/2601.03844)
*Agostino Dovier,Talissa Dreossi,Andrea Formisano,Benedetta Strizzolo*

Main category: cs.AI

TL;DR: ASP-based system for modeling Italian Criminal Code articles and semi-automatically learning legal rules from judicial decisions to support legal reasoning during trials.


<details>
  <summary>Details</summary>
Motivation: To support legal experts during criminal trials by providing reasoning capabilities and possible legal outcomes, while making judicial decision-making more interpretable through automated explainability.

Method: Encode Italian Criminal Code articles in Answer Set Programming (ASP), validate on previous verdicts, handle contradictions, and use inductive logic programming to generalize legal rules from case examples.

Result: Developed a tool that generates possible decisions for new cases, provides explanations using stable models' "supportedness," and can learn legal rules from examples through inductive logic programming.

Conclusion: The ASP-based approach successfully models criminal law, supports legal reasoning, offers explainable decisions, and enables rule learning from judicial precedents, enhancing interpretability in legal decision-making.
...
### [193] [Formally Explaining Decision Tree Models with Answer Set Programming](https://arxiv.org/abs/2601.03845)
...
<details>
  <summary>Details</summary>
Motivation: Decision tree models (random forests, gradient-boosted trees) have high predictive performance but lack interpretability, especially in safety-critical applications where formal justification of model decisions is required.

Method: Proposes using Answer Set Programming (ASP) to generate various explanation types: sufficient, contrastive, majority, and tree-specific explanations. ASP offers greater flexibility in encoding user preferences and supports enumeration of all possible explanations compared to SAT-based approaches.

Result: Empirical evaluation on diverse datasets demonstrates the method's effectiveness and limitations compared to existing methods.

Conclusion: ASP provides a flexible framework for generating multiple types of explanations for decision tree models, addressing interpretability needs in safety-critical applications while supporting user preferences and complete enumeration.
...
### [194] [xDNN(ASP): Explanation Generation System for Deep Neural Networks powered by Answer Set Programming](https://arxiv.org/abs/2601.03847)
*Ly Ly Trieu,Tran Cao Son*

Main category: cs.AI

TL;DR: xDNN(ASP) is an explainable AI system that extracts logic programs from deep neural networks to provide global explanations while maintaining prediction accuracy and enabling network optimization.


<details>
  <summary>Details</summary>
Motivation: Existing xAI methods like SHAP, rule extraction, and counterfactuals focus on input-output relationships but neglect the internal structure of neural networks in their explanations. There's a need for explanation methods that consider network architecture while providing global explanations.

Method: xDNN(ASP) extracts logic programs under answer set semantics from trained neural networks and their training data. The extracted logic program ideally represents the trained model with one-to-one correspondence between answer sets and input-output pairs.

Result: Experimental results on synthetic datasets show the extracted logic program maintains high prediction accuracy while providing valuable model understanding: feature importance analysis and insight into hidden node impact on predictions.

Conclusion: xDNN(ASP) successfully provides global explanations that consider network structure, offers feature importance insights, and enables network optimization by guiding hidden layer node reduction while preserving model performance.
...
### [195] [Investigating the Grounding Bottleneck for a Large-Scale Configuration Problem: Existing Tools and Constraint-Aware Guessing](https://arxiv.org/abs/2601.03850)
*Veronika Semmelrock,Gerhard Friedrich*

Main category: cs.AI

TL;DR: ASP faces scaling challenges for large configuration problems; constraint-aware guessing reduces memory demands.


<details>
  <summary>Details</summary>
Motivation: While ASP has succeeded in many domains, it's unclear if current techniques can scale to large configuration problems like electronic systems with 30,000+ components, particularly due to the grounding bottleneck where memory demands increase sharply with problem size.

Method: The paper investigates incremental solving approaches and develops constraint-aware guessing method based on grounding analysis to reduce memory requirements.

Result: Incremental solving proved effective but still had memory limitations; constraint-aware guessing significantly reduced memory needs for large configuration problems.

Conclusion: Current ASP technology has potential but faces memory limitations for large-scale configuration; constraint-aware guessing helps push these limits by addressing the grounding bottleneck.
...
### [196] [Current Agents Fail to Leverage World Model as Tool for Foresight](https://arxiv.org/abs/2601.03905)
...
TL;DR: Current AI agents struggle to effectively use generative world models as simulation tools, showing low usage rates, frequent misuse, and inconsistent performance when simulation is available.


<details>
  <summary>Details</summary>
Motivation: As agents face more tasks requiring future state anticipation, generative world models could serve as external simulators to enhance agent cognition. The paper investigates whether current agents can actually leverage such world models effectively.

Method: Empirical examination across diverse agentic and visual question answering tasks, analyzing agent behavior when world model simulation is available, including usage rates, misuse patterns, and performance impacts.

Result: Agents rarely invoke simulation (<1%), frequently misuse predicted rollouts (~15%), and often show inconsistent or degraded performance (up to 5%) when simulation is available or enforced. Attribution analysis reveals bottlenecks in deciding when to simulate, interpreting outcomes, and integrating foresight.

Conclusion: Current agents lack the capacity to effectively use world models as cognitive tools. The findings highlight the need for mechanisms enabling calibrated, strategic interaction with world models to achieve reliable anticipatory cognition in future agent systems.
...
### [197] [Trade-R1: Bridging Verifiable Rewards to Stochastic Environments via Process-Level Reasoning Verification](https://arxiv.org/abs/2601.03948)
*Rui Sun,Yifan Sun,Sheng Xu,Li Zhao,Jing Li,Daxin Jiang,Chen Hua,Zuo Bai*

Main category: cs.AI

TL;DR: Trade-R1: RL framework for financial decision-making that addresses noisy market rewards via process-level reasoning verification and structured RAG-based consistency metrics.
...
### [198] [Anti-Length Shift: Dynamic Outlier Truncation for Training Efficient Reasoning Models](https://arxiv.org/abs/2601.03969)
*Wei Wu,Liyi Chen,Congxi Xiao,Tianfu Wang,Qimeng Wang,Chengqiang Lu,Yan Gao,Yi Wu,Yao Hu,Hui Xiong*

Main category: cs.AI

TL;DR: DOT (Dynamic Outlier Truncation) reduces unnecessary verbosity in reasoning models by selectively truncating redundant tokens during training, achieving 78% token reduction with improved accuracy.


<details>
  <summary>Details</summary>
Motivation: Current reinforcement learning-enhanced reasoning models generate excessive verbosity on simple queries, incurring high deployment costs. Existing methods using explicit length penalties create optimization conflicts and don't address the underlying generative mechanisms causing overthinking.

Method: Proposes Dynamic Outlier Truncation (DOT) - a training-time intervention that selectively suppresses redundant tokens by targeting only the extreme tail of response lengths within fully correct rollout groups. Combines with auxiliary KL regularization and predictive dynamic sampling for stable convergence.

Result: Significantly pushes the efficiency-performance Pareto frontier outward across multiple model scales. On AIME-24, reduces inference token usage by 78% while simultaneously increasing accuracy compared to initial policy and surpassing state-of-the-art efficient reasoning methods.

Conclusion: DOT effectively addresses the length shift phenomenon where models generate unnecessary reasoning on trivial inputs, maintaining long-horizon reasoning capabilities for complex problems while dramatically improving efficiency.
...
### [199] [MobileDreamer: Generative Sketch World Model for GUI Agent](https://arxiv.org/abs/2601.04035)
*Yilin Cao,Yufeng Zhong,Zhixiong Zeng,Liming Zheng,Jing Huang,Haibo Qiu,Peng Shi,Wenji Mao,Wan Guanglu*

Main category: cs.AI

TL;DR: MobileDreamer: An efficient world-model-based lookahead framework for mobile GUI agents that uses textual sketch world modeling and rollout imagination to improve performance on long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: Most existing mobile GUI agents are reactive, making decisions only from current screen, which limits their performance on long-horizon tasks. Building a world model enables forecasting action outcomes and supports better decision making.

Method: Proposes MobileDreamer with two key components: 1) Textual sketch world model that transforms digital images into key task-related sketches and uses order-invariant learning to preserve spatial information, 2) Rollout imagination strategy that optimizes action-selection by leveraging world model predictions.
...
Conclusion: MobileDreamer successfully addresses the limitations of reactive agents by incorporating world modeling for lookahead planning, enabling better performance on long-horizon mobile GUI automation tasks.
...
### [200] [ComfySearch: Autonomous Exploration and Reasoning for ComfyUI Workflows](https://arxiv.org/abs/2601.04060)
...
<details>
  <summary>Details</summary>
Motivation: AI-generated content has shifted to modular workflows (like ComfyUI), but the large number of components and difficulty maintaining structural consistency under strict graph constraints lead to low pass rates and limited quality workflows.
...
Conclusion: The proposed validation-guided agentic framework effectively addresses the challenges of generating functional ComfyUI pipelines in complex creative tasks.
...
### [201] [Agent Drift: Quantifying Behavioral Degradation in Multi-Agent LLM Systems Over Extended Interactions](https://arxiv.org/abs/2601.04170)
*Abhishek Rath*

Main category: cs.AI

TL;DR: The paper introduces "agent drift" - progressive degradation of multi-agent LLM behavior over time - and proposes a framework to measure and mitigate it through the Agent Stability Index and three mitigation strategies.


<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM systems are powerful for complex tasks but their long-term behavioral stability is unexamined. There's a need to understand and address progressive degradation of agent behavior over extended interactions to ensure reliable enterprise deployment and AI safety.

Method: Introduces a theoretical framework for agent drift with three manifestations (semantic, coordination, behavioral). Develops Agent Stability Index (ASI) - a composite metric framework across 12 dimensions. Uses simulation-based analysis and theoretical modeling to study drift effects. Proposes three mitigation strategies: episodic memory consolidation, drift-aware routing protocols, and adaptive behavioral anchoring.

Result: Demonstrates that unchecked agent drift leads to substantial reductions in task completion accuracy and increased human intervention requirements. Theoretical analysis suggests proposed mitigation strategies can significantly reduce drift-related errors while maintaining system throughput.

Conclusion: Establishes foundational methodology for monitoring, measuring, and mitigating agent drift in production agentic AI systems, with direct implications for enterprise deployment reliability and AI safety research.
