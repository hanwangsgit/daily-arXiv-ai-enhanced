...
TL;DR: VQQA is a multi-agent framework that uses vision-language models to generate semantic critiques as gradients for efficient black-box prompt optimization in video generation, improving quality by +11.57% on T2V-CompBench.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models struggle to align outputs with complex user intent, and current test-time optimization methods are either computationally expensive or require white-box access to model internals.

Method: VQQA uses a multi-agent framework that dynamically generates visual questions and uses VLM critiques as semantic gradients to provide human-interpretable feedback, enabling efficient closed-loop prompt optimization through a black-box natural language interface.

Result: VQQA effectively isolates and resolves visual artifacts, improving generation quality in few refinement steps, achieving +11.57% improvement on T2V-CompBench and +8.43% on VBench2 over vanilla generation, outperforming state-of-the-art methods.

Conclusion: VQQA provides a unified, efficient framework for video generation optimization that works across diverse input modalities and tasks, replacing traditional metrics with actionable feedback through a black-box interface.
...
TL;DR: Proposes a decoupled kinetic paradigm using Alternating Gradient Flow (AGF) for structural pruning of vision networks, addressing magnitude bias in existing metrics and achieving Pareto-optimal efficiency with hybrid routing.


<details>
  <summary>Details</summary>
Motivation: Existing pruning metrics (Wanda, RIA) suffer from magnitude bias when applied to structural pruning of deep vision networks, failing to preserve critical functional pathways and leading to performance collapse at high sparsity.
...
Conclusion: The decoupled kinetic paradigm effectively addresses magnitude bias in structural pruning, preserves functionality at extreme sparsity, and enables Pareto-optimal efficiency through hybrid routing that separates offline structural search from online execution.
...
TL;DR: GenEval uses Vision Language Models with LoRA adaptation to improve cross-domain generalization for medical imaging tasks like diabetic retinopathy grading and seizure detection, achieving state-of-the-art performance.
...
Method: Introduces domain conformal bounds (DCB) to evaluate domain divergence, then proposes GenEval - a multimodal VLM approach combining foundational models (MedGemma-4B) with human knowledge via Low-Rank Adaptation (LoRA) to bridge causal gaps for single-source domain generalization.

Result: GenEval achieves superior SDG performance across eight DR datasets (69.2% average accuracy, 9.4% improvement over baselines) and two SOZ datasets (81% average accuracy, 1.8% improvement over baselines).

Conclusion: The proposed GenEval framework effectively addresses cross-domain generalization challenges in medical imaging by combining theoretical domain assessment (DCB) with practical VLM adaptation, demonstrating significant performance improvements in critical healthcare applications.
...
TL;DR: SPARROW is a pixel-grounded video MLLM that improves spatial accuracy and temporal stability for video object grounding through target-specific tracked features and dual-prompt design.


<details>
  <summary>Details</summary>
Motivation: Existing video MLLMs struggle with spatial precision and temporal consistency when tracking objects across frames, suffering from spatial drift, identity switches, and unstable initialization due to reliance on static segmentation tokens without temporal context.

Method: SPARROW introduces: (1) Target-Specific Tracked Features (TSF) that inject temporally aligned referent cues during training, and (2) a dual-prompt design that decodes box and segmentation tokens to fuse geometric priors with semantic grounding. It operates end-to-end without external detectors using a class-agnostic SAM2-based proposer.

Result: Integrated into three open-source video MLLMs (UniPixel, GLUS, and VideoGLaMM), SPARROW achieves consistent gains across six benchmarks: +8.9 J&F on RVOS, +5 mIoU on visual grounding, and +5.4 CLAIR on GCG.

Conclusion: SPARROW substantially improves referential stability, spatial precision, and temporal coherence in pixel-grounded video understanding, demonstrating effective unification of spatial accuracy and temporal stability.
...
TL;DR: EMC-Gaze is a lightweight, calibration-friendly gaze tracking method using facial landmarks with session-wise adaptation, achieving competitive accuracy with minimal calibration points and small runtime footprint suitable for browser deployment.


<details>
  <summary>Details</summary>
Motivation: Practical webcam gaze tracking needs to balance accuracy with deployment constraints: calibration burden, robustness to head motion, session drift, runtime footprint, and browser compatibility. Current methods often focus on large backbone architectures, but there's a need for lightweight, calibration-friendly solutions.

Method: Uses session-wise adaptation with a shared geometric encoder producing embeddings aligned to new sessions from small calibration sets. Combines E(3)-equivariant landmark-graph encoder, local eye geometry, binocular emphasis, auxiliary 3D gaze-direction supervision, and closed-form ridge calibrator differentiated through episodic meta-training. Includes two-view canonicalization consistency loss to reduce pose leakage.

Result: Achieves 5.79 +/- 1.81 deg RMSE after 9-point calibration vs 6.68 +/- 2.34 deg for Elastic Net. Gains larger on still-head queries (2.92 +/- 0.75 deg vs 4.45 +/- 0.30 deg). Maintains advantage across subject holdouts. On MPIIFaceGaze, reaches 8.82 +/- 1.21 deg at 16-shot calibration, outperforming Elastic Net from 3-shot onward. Encoder has 944,423 parameters, 4.76 MB in ONNX, with 12.58 ms inference time in browser.

Conclusion: EMC-Gaze provides a practical, calibration-friendly operating point for webcam gaze tracking with lightweight deployment, competitive accuracy from minimal calibration, and efficient browser compatibility, positioning it as a viable alternative to heavier appearance-based systems for real-world applications.
...
TL;DR: ABRA transfers class-specific detection knowledge from source to target domains without target domain training data by formulating adaptation as geometric transport in weight space.
...
Method: ABRA formulates adaptation as geometric transport problem in weight space of pretrained detector, aligning source and target domain experts to transport class-specific knowledge.

Result: Extensive experiments across challenging domain shifts demonstrate successful teleportation of class-level specialization under multiple adverse conditions.

Conclusion: ABRA enables transfer of detection knowledge to domains without training data, addressing the domain shift problem in Open-Vocabulary Object Detection.
...
TL;DR: Neuro-symbolic framework combining LLM-based rule extraction with ASP logical arbitration for safe, interpretable autonomous driving trajectory planning.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end autonomous driving models lack interpretability and safety guarantees in complex scenarios due to their purely data-driven "black-box" nature.

Method: Uses LLM to extract scene rules, ASP solver for logical arbitration, decision-conditioned decoding to transform discrete decisions into embeddings, and combines KBM-generated trajectories with neural residual corrections.

Result: Outperforms state-of-the-art MomAD on nuScenes: L2 mean error 0.57m, collision rate 0.075%, trajectory prediction consistency 0.47m.

Conclusion: The neuro-symbolic approach provides interpretable, safe trajectory planning with kinematic feasibility guarantees while maintaining competitive performance.
...
TL;DR: Surg-R1 is a surgical vision-language model that introduces hierarchical reasoning with a three-level framework, trained on the largest surgical chain-of-thought dataset, achieving state-of-the-art performance on surgical benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing surgical vision-language models lack interpretable reasoning chains that surgeons can verify, while general-purpose reasoning models fail on compositional surgical tasks due to lack of domain-specific knowledge.

Method: Three-level reasoning hierarchy (perceptual grounding, relational understanding, contextual reasoning), largest surgical chain-of-thought dataset (320,000 reasoning pairs), and four-stage training pipeline from supervised fine-tuning to group relative policy optimization and iterative self-improvement.
...
Conclusion: Surg-R1 addresses the interpretability gap in surgical AI through hierarchical reasoning, demonstrating superior performance across diverse surgical tasks while providing verifiable reasoning chains aligned with clinical expertise.
...
### [9] [Prompt-Driven Lightweight Foundation Model for Instance Segmentation-Based Fault Detection in Freight Trains](https://arxiv.org/abs/2603.12624)
*Guodong Sun,Qihang Liang,Xingyu Pan,Moyun Liu,Yang Zhang*

Main category: cs.CV

TL;DR: A lightweight self-prompted instance segmentation framework for freight train fault detection that leverages SAM with automatic prompt generation and TinyViT backbone for real-time edge deployment.


<details>
  <summary>Details</summary>
Motivation: Accurate visual fault detection in freight trains is challenging due to complex environments, repetitive components, and occlusions/contaminations. Conventional instance segmentation methods suffer from poor generalization and limited boundary accuracy in these conditions.

Method: Proposes a lightweight self-prompted instance segmentation framework that leverages Segment Anything Model (SAM) with a self-prompt generation module for automatic task-specific prompts, and uses Tiny Vision Transformer backbone to reduce computational cost for real-time edge deployment.

Result: Achieves 74.6 AP^box and 74.2 AP^mask on a domain-specific freight train inspection dataset, outperforming state-of-the-art methods in accuracy and robustness while maintaining low computational overhead.
...
TL;DR: Heterogeneous Vision Foundation Models (VFMs) can be effectively stitched together using a simple feature-matching loss at the target model's penultimate layer, enabling integration of complementary strengths with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: To investigate whether Vision Foundation Models (VFMs) with different objectives, data, and modality mixes (like CLIP, DINOv2, SigLIP) can be stitched together to combine their complementary strengths, moving stitching from a diagnostic probe to a practical integration method.

Method: Systematic protocol exploring stitch points, stitch layer families, training losses, and downstream tasks. Key innovation: using a simple feature-matching loss at the target model's penultimate layer rather than conventional intermediate feature matching or end-to-end task loss optimization.

Result: 1) Conventional stitching approaches struggle with accuracy retention, especially at shallow stitch points. 2) With the proposed feature-matching loss at penultimate layer, heterogeneous VFMs become reliably stitchable across vision tasks. 3) Deep stitch points can surpass constituent models with minimal inference overhead. 4) Introduced VFM Stitch Tree (VST) for sharing early layers across VFMs while retaining later layers.

Conclusion: Stitching can be elevated from a diagnostic probe to a practical recipe for integrating complementary VFM strengths, enabling controllable accuracy-latency trade-offs for multimodal LLMs and pinpointing where VFM representations align or diverge.
...
### [11] [Marker-Based 3D Reconstruction of Aggregates with a Comparative Analysis of 2D and 3D Morphologies](https://arxiv.org/abs/2603.12667)
...
<details>
  <summary>Details</summary>
Motivation: Current methods for 3D characterization of aggregate morphology are difficult during production/construction and often require expensive equipment like 3D laser scanners or CT equipment, limiting practical QA/QC applications.

Method: Marker-based photogrammetry approach that enables background suppression, point cloud stitching, and scale referencing to obtain high-quality 3D aggregate models from images.
...
Conclusion: The photogrammetry method provides a flexible, cost-effective way to obtain 3D shape information of aggregates, enabling convenient inspection, data collection, and morphological analysis for construction applications.
...
### [12] [Adaptation of Weakly Supervised Localization in Histopathology by Debiasing Predictions](https://arxiv.org/abs/2603.12468)
...
<details>
  <summary>Details</summary>
Motivation: WSOL models suffer performance degradation under domain shifts in histopathology (new organs/institutions). SFDA methods using self-training reinforce initial prediction biases toward dominant classes, degrading both classification and localization.
...
Conclusion: SFDA-DeP effectively addresses prediction bias amplification in WSOL domain adaptation for histopathology, enabling better adaptation to new domains without source data access.
...
### [13] [UNIStainNet: Foundation-Model-Guided Virtual Staining of H&E to IHC](https://arxiv.org/abs/2603.12716)
*Jillur Rahman Saurav,Thuong Le Hoai Pham,Pritam Mukherjee,Paul Yi,Brent A. Orr,Jacob M. Luber*

Main category: cs.CV

TL;DR: UNIStainNet uses a frozen pathology foundation model (UNI) to provide semantic guidance for virtual IHC staining from H&E images, achieving SOTA results with a single unified model for multiple IHC markers.


<details>
  <summary>Details</summary>
Motivation: Virtual IHC staining from H&E images can accelerate diagnostics by providing molecular insight directly from routine sections, reducing the need for repeat sectioning when tissue is limited. Existing methods lack direct guidance from pathology foundation models.

Method: UNIStainNet uses a SPADE-UNet conditioned on dense spatial tokens from a frozen pathology foundation model (UNI) to provide tissue-level semantic guidance. Includes misalignment-aware loss suite to preserve stain quantification accuracy, and learned stain embeddings enable single model to serve multiple IHC markers simultaneously.

Result: Achieves state-of-the-art distributional metrics on all four stains (HER2, Ki67, ER, PR) from a single unified model on MIST dataset, where prior methods typically train separate per-stain models. Also achieves best distributional metrics on BCI dataset. Tissue-type stratified failure analysis shows remaining errors are systematic and concentrate in non-tumor tissue.

Conclusion: UNIStainNet demonstrates that conditioning on pathology foundation model features enables superior virtual IHC staining performance with a single unified model for multiple markers, though systematic errors remain in non-tumor tissue.
...
TL;DR: AbSteering is a framework that adapts general Video Language Models for precise HRCT report generation by using abnormality-centric reasoning and preference optimization with clinical confusable cases.
...
Result: AbSteering outperforms state-of-the-art domain-specific CT foundation models (pretrained with large-scale CTs), achieving superior detection sensitivity while simultaneously mitigating hallucinations. General-purpose VideoLMs show strong transferability to medical imaging when guided by this paradigm.

Conclusion: The AbSteering framework successfully adapts general VideoLMs for precise HRCT report generation, demonstrating that domain-specific medical imaging tasks can benefit from general-purpose video-language models when properly guided with clinical reasoning mechanisms.
...
### [15] [Empowering Semantic-Sensitive Underwater Image Enhancement with VLM](https://arxiv.org/abs/2603.12773)
*Guodong Fan,Shengning Zhou,Genji Yuan,Huiyu Li,Jingchun Zhou,Jinjiang Li*

Main category: cs.CV

TL;DR: The paper proposes a semantic-guided underwater image enhancement method using Vision-Language Models to generate spatial semantic guidance maps that steer enhancement networks to focus on key object regions rather than uniform global improvement.
...
Method: 1) Use VLMs to generate textual descriptions of key objects from degraded images. 2) Employ text-image alignment model to remap descriptions onto images, creating spatial semantic guidance maps. 3) Guide UIE network through dual-guidance mechanism combining cross-attention and explicit alignment loss to focus on semantic-sensitive regions.
...
Conclusion: The proposed semantic-guided learning mechanism effectively improves UIE models by ensuring faithful restoration of key object features, enhancing both perceptual quality and downstream task performance.
...
### [16] [Less Data, Faster Convergence: Goal-Driven Data Optimization for Multimodal Instruction Tuning](https://arxiv.org/abs/2603.12478)
*Rujie Wu,Haozhe Zhao,Hai Ci,Yizhou Wang*

Main category: cs.CV

TL;DR: GDO is a goal-driven data optimization framework that selects optimal training subsets from mixed image-video pools, achieving faster convergence and higher accuracy with far fewer samples than baseline methods.


<details>
  <summary>Details</summary>
Motivation: Multimodal instruction tuning is compute-inefficient due to training budgets being spread across large mixed image-video pools with highly uneven utility, necessitating smarter data selection.

Method: GDO computes six sample descriptors for each candidate and constructs optimized 1× training subsets for different goals, using far fewer training samples than baseline methods under fixed training protocols.

Result: GDO achieves the Uni-10x baseline performance with only 35.4k, 26.6k, 27.3k, and 34.7k samples on MVBench, VideoMME, MLVU, and LVBench respectively, while improving accuracy by +1.38 to +3.08 percentage points.
...
TL;DR: SAW is a surgical action world model using video diffusion conditioned on lightweight signals (language, reference scene, affordance mask, tool trajectories) to generate realistic surgical videos for AI training and simulation.
...
Method: Conditional video diffusion reformulated as trajectory-conditioned surgical action synthesis, fine-tuned on 12,044 laparoscopic clips with lightweight spatiotemporal conditioning signals and depth consistency loss for geometric plausibility.

Result: Achieves state-of-the-art temporal consistency (CD-FVD: 199.19 vs. 546.82) and strong visual quality; improves action recognition (clipping F1: 20.93% to 43.14%; cutting: 0% to 8.33%) and enables realistic simulation rendering.

Conclusion: SAW demonstrates effective surgical world modeling through lightweight-conditioned video diffusion, addressing data scarcity and sim-to-real gaps for surgical AI and simulation applications.
...
### [18] [CalliMaster: Mastering Page-level Chinese Calligraphy via Layout-guided Spatial Planning](https://arxiv.org/abs/2603.12482)
*Tianshuo Xu,Tiantian Hong,Zhifei Chen,Fei Chao,Ying-cong Chen*

Main category: cs.CV

TL;DR: CalliMaster is a unified framework for page-level calligraphy synthesis that decouples spatial planning from content synthesis, enabling controllable generation and editing while maintaining both layout composition and brushwork detail.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for page-level calligraphy synthesis face a fundamental conflict: character-level models lack spatial context for proper layout composition, while page-level methods often sacrifice brushwork detail. There's a need for a solution that balances glyph precision with layout composition at the page scale.

Method: A coarse-to-fine pipeline (Text → Layout → Image) inspired by human "planning before writing" cognitive process. Uses a single Multimodal Diffusion Transformer where: 1) Spatial planning stage predicts character bounding boxes for global arrangement, 2) Content synthesis stage uses the layout as geometric prompt with flow-matching to render high-fidelity brushwork.

Result: Achieves state-of-the-art generation quality. The disentanglement enables versatile downstream capabilities including controllable semantic re-planning (resizing/repositioning characters with automatic void space and brush momentum harmonization), artifact restoration, and forensic analysis for digital cultural heritage.

Conclusion: CalliMaster resolves the conflict between glyph precision and layout composition through spatial-content disentanglement, providing a comprehensive framework for controllable calligraphy generation and editing with applications in digital cultural heritage preservation and analysis.
...
### [19] [InterEdit: Navigating Text-Guided Multi-Human 3D Motion Editing](https://arxiv.org/abs/2603.13082)
...
TL;DR: InterEdit3D: A new dataset and diffusion model for text-guided multi-person 3D motion editing, addressing the challenge of editing complex two-person interactions with text instructions.


<details>
  <summary>Details</summary>
Motivation: Text-guided 3D motion editing has been successful for single-person scenarios but is less explored for multi-person settings due to limited paired data and the complexity of inter-person interactions. There's a need to extend motion editing capabilities to handle realistic multi-person scenarios.

Method: Proposes InterEdit3D dataset with manual two-person motion change annotations and a TMME benchmark. Introduces InterEdit, a synchronized classifier-free conditional diffusion model with Semantic-Aware Plan Token Alignment (learnable tokens for interaction cues) and Interaction-Aware Frequency Token Alignment (using DCT and energy pooling for periodic motion dynamics).

Result: InterEdit improves text-to-motion consistency and edit fidelity, achieving state-of-the-art performance on the TMME benchmark. The model effectively handles complex multi-person motion editing tasks.

Conclusion: The work successfully extends text-guided motion editing to multi-person scenarios, providing both a new dataset and a novel diffusion-based model that captures interaction dynamics. The approach demonstrates promising results for editing complex two-person interactions with text instructions.
...
### [20] [RAW-Domain Degradation Models for Realistic Smartphone Super-Resolution](https://arxiv.org/abs/2603.12493)
...
<details>
  <summary>Details</summary>
Motivation: Digital zoom on smartphones uses SR models on RAW images, but getting sensor-specific training data is hard due to lack of ground-truth. Synthetic data via unprocessing pipelines can help but often creates domain gaps from unrealistic degradation modeling.

Method: Instead of generic priors for camera blur/noise, model device-specific degradations through calibration, unprocess publicly available rendered images into RAW domain of different smartphones, then train single-image RAW-to-RGB SR model.

Result: Experiments show accurate degradation modeling leads to noticeable improvements, with SR model outperforming baselines trained on large pools of arbitrarily chosen degradations when evaluated on real data from held-out device.

Conclusion: Principled and carefully designed degradation modeling enhances SR performance in real-world conditions, demonstrating the value of device-specific calibration over generic degradation approaches.
...
### [21] [Naïve PAINE: Lightweight Text-to-Image Generation Improvement with Prompt Evaluation](https://arxiv.org/abs/2603.12506)
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models rely on random Gaussian noise, causing inconsistent results that require multiple generation cycles (gambler's burden) to get satisfactory outputs. The quality distribution depends heavily on both the prompt and the model's generative ability for that prompt.

Method: Naïve PAINE predicts numerical image quality directly from initial noise and given prompt using T2I preference benchmarks. It selects a handful of high-quality noise seeds and forwards them to the diffusion model for generation. The approach is lightweight and integrates seamlessly into existing pipelines.
...
Conclusion: Naïve PAINE effectively addresses the gambler's burden problem in diffusion models by predicting and selecting high-quality noise seeds before generation, leading to better and more consistent results with fewer generation cycles.
...
### [22] [MemRoPE: Training-Free Infinite Video Generation via Evolving Memory Tokens](https://arxiv.org/abs/2603.12513)
*Youngrae Kim,Qixin Hu,C. -C. Jay Kuo,Peter A. Beerel*

Main category: cs.CV

TL;DR: MemRoPE is a training-free framework for autoregressive video diffusion that maintains long-term coherence by compressing past context into dual memory streams and dynamically applying positional embeddings.


<details>
  <summary>Details</summary>
Motivation: Existing sliding-window caches in autoregressive video diffusion discard past context, causing fidelity degradation, identity drift, and motion stagnation over long horizons. Current approaches use static early tokens as anchors but cannot adapt to evolving video content.

Method: Two co-designed components: 1) Memory Tokens that continuously compress all past keys into dual long-term and short-term streams via exponential moving averages, maintaining both global identity and recent dynamics within fixed-size cache. 2) Online RoPE Indexing that caches unrotated keys and applies positional embeddings dynamically at attention time, ensuring aggregation free of conflicting positional phases.
...
Conclusion: MemRoPE enables training-free, long-horizon video generation with improved temporal coherence by combining dynamic memory compression with positional decoupling, making fixed-size caching viable for unbounded generation.
...
### [23] [Addressing Data Scarcity in 3D Trauma Detection through Self-Supervised and Semi-Supervised Learning with Vertex Relative Position Encoding](https://arxiv.org/abs/2603.12514)
*Shivam Chaudhary,Sheethal Bhat,Andreas Maier*

Main category: cs.CV

TL;DR: Self-supervised pre-training with Masked Image Modeling on unlabeled CT scans combined with semi-supervised learning enables accurate 3D injury detection and classification with minimal labeled data.
...
Method: Patch-based Masked Image Modeling (MIM) pre-training on 1,206 unlabeled CT volumes, followed by semi-supervised detection with VDETR and Vertex Relative Position Encoding using 2,000 unlabeled volumes and consistency regularization, plus multi-label injury classification with frozen encoder.

Result: 56.57% validation mAP@0.50 and 45.30% test mAP@0.50 for detection with only 144 labeled samples (115% improvement over supervised-only), and 94.07% test accuracy for classification across seven injury categories with 2,244 labeled samples using frozen encoder.
...
### [24] [Do You See What I Am Pointing At? Gesture-Based Egocentric Video Question Answering](https://arxiv.org/abs/2603.12533)
*Yura Choi,Roy Miles,Rolandos Alexandros Potamias,Ismail Elezi,Jiankang Deng,Stefanos Zafeiriou*

Main category: cs.CV

TL;DR: EgoPointVQA dataset and HINT method for gesture-grounded egocentric question answering, achieving SOTA performance by encoding 3D hand keypoints as tokens to interpret pointing intent.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with gesture-based question answering due to lack of gesture-rich data and limited ability to infer fine-grained pointing intent from egocentric video, which is essential for next-generation AI assistants.

Method: Introduces EgoPointVQA dataset (4000 synthetic + 400 real videos) and Hand Intent Tokens (HINT) - encoding 3D hand keypoints from reconstruction model as tokens interleaved with model input to provide explicit spatial/temporal context.

Result: HINT-14B achieves 68.1% average accuracy across 6 tasks, surpassing state-of-the-art InternVL3-14B by 6.6%, outperforming others in different backbones and model sizes.

Conclusion: The proposed approach effectively addresses gesture understanding in egocentric QA, with code, model, and dataset to be released to facilitate open research in this area.
...
### [25] [Spatio-Semantic Expert Routing Architecture with Mixture-of-Experts for Referring Image Segmentation](https://arxiv.org/abs/2603.12538)
*Alaa Dalaq,Muzammil Behzad*

Main category: cs.CV

TL;DR: SERA introduces a Spatio-Semantic Expert Routing Architecture for referring image segmentation that uses expression-aware expert refinement at two stages to address fragmented regions and inaccurate boundaries in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing referring image segmentation methods often use uniform refinement strategies that don't match diverse reasoning requirements, leading to fragmented regions, inaccurate boundaries, or wrong object selection, especially when pretrained backbones are frozen for efficiency.

Method: SERA introduces two complementary stages: 1) SERA-Adapter inserts expression-conditioned adapters into selected backbone blocks for spatial coherence and boundary precision, and 2) SERA-Fusion reshapes token features into spatial grids and applies geometry-preserving expert transformations before multimodal interaction. A lightweight routing mechanism adaptively weights expert contributions while updating less than 1% of backbone parameters.
...
Conclusion: SERA effectively addresses limitations of existing methods by introducing expression-aware expert refinement that improves spatial coherence and boundary precision while maintaining computational efficiency through parameter-efficient tuning.
...
### [26] [Spatial Reasoning is Not a Free Lunch: A Controlled Study on LLaVA](https://arxiv.org/abs/2603.12545)
...
<details>
  <summary>Details</summary>
Motivation: Vision-language models have advanced rapidly but still fail at basic 2D spatial reasoning tasks like relative position, layout, and counting. The authors argue this is not just a data problem but is tied to fundamental design choices in current VLM architectures.
...
### [27] [Decoding Matters: Efficient Mamba-Based Decoder with Distribution-Aware Deep Supervision for Medical Image Segmentation](https://arxiv.org/abs/2603.12547)
*Fares Bougourzi,Fadi Dornaika,Abdenour Hadid*

Main category: cs.CV

TL;DR: Proposes Deco-Mamba, a decoder-centric U-Net architecture with Transformer-CNN-Mamba design for generalized 2D medical image segmentation, achieving SOTA performance with strong generalization across diverse modalities.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for medical image segmentation are often task-specific with limited generalization across imaging modalities, and rely heavily on encoder-focused designs with large pretrained backbones that increase computational complexity.

Method: Decoder-centric U-Net-like architecture with Transformer-CNN-Mamba design. Encoder combines CNN block and Transformer backbone, while decoder integrates novel Co-Attention Gate (CAG), Vision State Space Module (VSSM), and deformable convolutional refinement block. Uses windowed distribution-aware KL-divergence loss for deep supervision across multiple decoding stages.
...
Conclusion: The proposed decoder-centric approach with Deco-Mamba architecture effectively addresses generalization limitations in medical image segmentation, offering a balanced solution between performance and computational efficiency.
...
### [28] [CVGL: Causal Learning and Geometric Topology](https://arxiv.org/abs/2603.12551)
...
<details>
  <summary>Details</summary>
Motivation: Cross-view geo-localization is critical for autonomous navigation but remains challenging due to significant viewpoint differences and confounding factors that affect model performance.

Method: Proposes CLGT framework with: 1) Causal Feature Extractor using causal intervention to focus on stable semantics, 2) Geometric Topology Fusion module injecting BEV road topology to alleviate cross-view inconsistencies, and 3) Data-Adaptive Pooling to enhance semantically rich regions.

Result: Achieves state-of-the-art performance on CVUSA, CVACT, and their robustness-enhanced variants (CVUSA-C-ALL and CVACT-C-ALL), particularly under challenging real-world corruptions.

Conclusion: CLGT effectively addresses cross-view geo-localization challenges by combining causal learning and geometric topology, demonstrating superior robustness and performance in complex real-world scenarios.
...
### [29] [AccelAes: Accelerating Diffusion Transformers for Training-Free Aesthetic-Enhanced Image Generation](https://arxiv.org/abs/2603.12575)
*Xuanhua Yin,Chuanzhi Xu,Haoxian Zhou,Boyu Wei,Weidong Cai*

Main category: cs.CV

TL;DR: AccelAes accelerates Diffusion Transformers by reducing redundant computation in low-aesthetic regions using aesthetic focus masks and step-level caching.


<details>
  <summary>Details</summary>
Motivation: DiTs suffer from high inference latency due to quadratic self-attention over dense spatial tokens, and denoising is spatially non-uniform with respect to aesthetic descriptors in prompts.

Method: Proposes AccelAes with: 1) AesMask - one-shot aesthetic focus mask from prompt semantics and cross-attention; 2) SkipSparse - reallocates computation to masked regions; 3) Step-level prediction cache to reduce temporal redundancy.

Result: Achieves 2.11× speedup on Lumina-Next and improves ImageReward by +11.9% over dense baseline, with consistent acceleration across DiT families.

Conclusion: AccelAes enables training-free acceleration of DiTs through aesthetics-aware spatio-temporal reduction while improving perceptual aesthetics.
...
### [30] [DINOLight: Robust Ambient Light Normalization with Self-supervised Visual Prior Integration](https://arxiv.org/abs/2603.12579)
*Youngjin Oh,Junhyeong Kwon,Nam Ik Cho*

Main category: cs.CV

TL;DR: DINOLight is a new ambient light normalization framework that integrates DINOv2's self-supervised visual features as a prior to restore images degraded by non-uniform shadows and lighting from multiple light sources and complex geometries.


<details>
  <summary>Details</summary>
Motivation: Ambient light normalization aims to restore images degraded by non-uniform shadows and lighting caused by multiple light sources and complex scene geometries. Existing methods need better visual priors to handle these complex degradation patterns.

Method: The framework integrates DINOv2 features through: 1) an adaptive feature fusion module combining features from different DINOv2 layers using point-wise softmax masks, and 2) integration into the restoration network in both spatial and frequency domains via auxiliary cross-attention mechanisms.

Result: DINOLight achieves superior performance on the Ambient6K dataset and demonstrates that DINOv2 features effectively enhance ambient light normalization. The method also achieves competitive results on shadow-removal benchmark datasets compared to methods using mask priors.

Conclusion: DINOv2's self-supervised features provide effective visual priors for ambient light normalization, and the proposed DINOLight framework successfully integrates these features through adaptive fusion and cross-attention mechanisms to achieve state-of-the-art performance.
...
### [31] [MRGeo: Robust Cross-View Geo-Localization of Corrupted Images via Spatial and Channel Feature Enhancement](https://arxiv.org/abs/2603.12587)
*Le Wu,Lv Bo,Songsong Ouyang,Yingying Zhu*

Main category: cs.CV

TL;DR: MRGeo is the first systematic method for robust cross-view geo-localization under image corruption, using hierarchical defense with spatial-channel enhancement and geometric alignment to maintain performance in real-world degraded conditions.


<details>
  <summary>Details</summary>
Motivation: Existing cross-view geo-localization methods achieve near-perfect performance on clean datasets but fail in real-world corrupted environments (blur, weather effects), causing severe performance degradation and limiting practical deployment.

Method: MRGeo employs hierarchical defense: 1) Spatial-Channel Enhancement Block with Spatial Adaptive Representation Module (parallel global/local features with dynamic gating) and Channel Calibration Module (multi-granularity channel dependencies); 2) Region-level Geometric Alignment Module for coarse-grained geometric consistency.

Result: Achieves average R@1 improvement of 2.92% across three robustness benchmarks (CVUSA-C-ALL, CVACT_val-C-ALL, CVACT_test-C-ALL) and superior performance in cross-area evaluation, demonstrating both robustness and generalization capability.

Conclusion: MRGeo addresses the critical gap in robust cross-view geo-localization under corruption through systematic hierarchical defense, enabling practical deployment in real-world degraded conditions while maintaining strong performance.
...
### [32] [SDF-Net: Structure-Aware Disentangled Feature Learning for Opticall-SAR Ship Re-identification](https://arxiv.org/abs/2603.12588)
*Furui Chen,Han Wang,Yuhan Sun,Jianing You,Yixuan Lv,Zhuang Zhou,Hong Tan,Shengyang Li*

Main category: cs.CV

TL;DR: SDF-Net is a structure-aware disentangled feature learning network for cross-modal ship re-identification between optical and SAR imagery, leveraging geometric consistency to address radiometric discrepancies.


<details>
  <summary>Details</summary>
Motivation: Existing cross-modal ship ReID methods overlook the critical physical prior that ships are rigid objects with stable geometric structures across modalities, while texture appearance is highly modality-dependent due to different sensing principles.

Method: Built on ViT backbone, SDF-Net introduces structure consistency constraint using scale-invariant gradient energy statistics from intermediate layers, then disentangles representations into modality-invariant identity features and modality-specific characteristics, integrated via parameter-free additive residual fusion.

Result: Extensive experiments on HOSS-ReID dataset show SDF-Net consistently outperforms existing state-of-the-art methods for optical-SAR ship re-identification.

Conclusion: Incorporating geometric structure consistency and disentangling modality-invariant identity features from modality-specific characteristics effectively addresses cross-modal ship ReID challenges, with publicly available code and models.
...
### [33] [Neural Gate: Mitigating Privacy Risks in LVLMs via Neuron-Level Gradient Gating](https://arxiv.org/abs/2603.12598)
*Xiangkui Cao,Jie Zhang,Meina Kan,Shiguang Shan,Xilin Chen*

Main category: cs.CV

TL;DR: Neural Gate is a neuron-level model editing method that enhances privacy protection in Large Vision-Language Models by increasing refusal rates for sensitive queries while preserving model utility.
...
Method: Neural Gate uses neuron-level model editing to improve privacy safeguards. It learns a feature vector to identify neurons associated with privacy-related concepts within the model's representation of a subject, then precisely guides parameter updates based on this localization.

Result: The method significantly boosts privacy protection in MiniGPT and LLaVA models by increasing refusal rates for privacy-related questions, including novel sensitive queries not seen during editing, while preserving original model utility.

Conclusion: Neural Gate provides an effective approach to mitigate privacy risks in LVLMs through targeted neuron-level editing, addressing generalization and non-destructiveness limitations of previous methods.
...
TL;DR: PAP framework integrates prediction-perception architecture into 3D object perception, improving tracking accuracy by 10% and inference speed by 15% on UniAD model with nuScenes dataset.


<details>
  <summary>Details</summary>
Motivation: Inspired by human visual perception where we predict future positions to track moving objects, the paper aims to enhance 3D object perception accuracy by integrating prediction capabilities into perception models.

Method: Proposes Prediction-As-Perception (PAP) framework with two modules: prediction module forecasts future positions of ego vehicles and surrounding traffic participants, and perception module uses these predictions as queries for subsequent frames in an iterative feedback loop.

Result: Evaluated on UniAD model with nuScenes dataset, PAP improves target tracking accuracy by 10% and increases inference speed by 15%, demonstrating enhanced efficiency and accuracy while reducing computational resources.

Conclusion: The biomimetic PAP framework successfully enhances 3D object perception by integrating prediction capabilities, showing significant improvements in both accuracy and efficiency for autonomous driving applications.
...
### [35] [A2Z-10M+: Geometric Deep Learning with A-to-Z BRep Annotations for AI-Assisted CAD Modeling and Reverse Engineering](https://arxiv.org/abs/2603.12605)
*Pritham Kumar Jena,Bhavika Baburaj,Tushar Anand,Vedant Dutta,Vineeth Ulavala,Sk Aziz Ali*

Main category: cs.CV

TL;DR: A2Z dataset: 10M multi-modal annotations for 1M CAD models with meshes, sketches, BRep data, and text captions to advance parametric CAD feature understanding and reverse engineering.
...
Method: Created largest compilation of 10M multi-modal annotations for 1M ABC CAD models (A2Z dataset) with meshes, 3D sketches, BRep geometric/topological info, and text captions. Added 25K professional electronic enclosure models. Trained foundation model on 150K subset for BRep co-edge and corner detection from 3D scans.

Result: Produced comprehensive 5TB dataset with novel metrics validated by GPT-5, Gemini, and human feedback. Demonstrated foundation model for CAD reverse engineering tasks. Dataset, metrics, and checkpoints will be publicly released.

Conclusion: A2Z dataset enables unprecedented BRep learning for CAD reverse engineering and multi-modal understanding, supporting numerous research directions in industrial product design.
...
### [36] [Mastering Negation: Boosting Grounding Models via Grouped Opposition-Based Learning](https://arxiv.org/abs/2603.12606)
*Zesheng Yang,Xi Jiang,Bingzhang Hu,Weili Guan,Runmin Cong,Guo-Jun Qi,Feng Zheng*

Main category: cs.CV

TL;DR: D-Negation dataset and learning framework improve vision-language grounding models' ability to handle negative semantics, achieving up to 5.7 mAP improvement on negative semantic evaluations.


<details>
  <summary>Details</summary>
Motivation: Current vision-language detection models struggle with negative semantics due to lack of high-quality training data with explicit negative samples and negation-aware descriptions.

Method: Introduces D-Negation dataset with positive/negative semantic annotations, and a grouped opposition-based learning framework with structured semantic groups and complementary loss functions for negation reasoning.

Result: Fine-tuning <10% of model parameters achieves up to 4.4 mAP improvement on positive semantics and 5.7 mAP on negative semantics evaluations.
...
TL;DR: RoboStereo: A symmetric dual-tower 4D world model with unified policy optimization framework for scalable embodied AI, achieving >97% improvement on manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Scalable Embodied AI faces prohibitive costs and safety risks in real-world interaction. Existing Embodied World Models suffer from geometric hallucinations and lack unified optimization frameworks for practical policy improvement.

Method: Introduces RoboStereo, a symmetric dual-tower 4D world model with bidirectional cross-modal enhancement for spatiotemporal geometric consistency. Presents unified framework: (1) Test-Time Policy Augmentation for pre-execution verification, (2) Imitative-Evolutionary Policy Learning with visual perceptual rewards, (3) Open-Exploration Policy Learning for autonomous skill discovery.

Result: Achieves state-of-the-art generation quality with >97% average relative improvement on fine-grained manipulation tasks compared to existing approaches.

Conclusion: RoboStereo provides a high-fidelity 4D simulator and unified optimization framework that addresses fundamental constraints of scalable embodied AI, significantly improving manipulation performance while reducing real-world interaction costs and risks.
...
### [38] [LR-SGS: Robust LiDAR-Reflectance-Guided Salient Gaussian Splatting for Self-Driving Scene Reconstruction](https://arxiv.org/abs/2603.12647)
*Ziyu Chen,Fan Zhu,Hui Zhu,Deyi Kong,Xinkai Kuang,Yujia Zhang,Chunmao Jiang*

Main category: cs.CV

TL;DR: LR-SGS: LiDAR-reflectance-guided Salient Gaussian Splatting for self-driving scenes that leverages LiDAR reflectance and RGB complementarity to improve reconstruction in challenging conditions like high ego-motion and complex lighting.


<details>
  <summary>Details</summary>
Motivation: Existing 3D Gaussian Splatting methods for self-driving scenes don't fully exploit LiDAR point cloud information (especially reflectance) and the complementarity between LiDAR and RGB, leading to degradation in challenging scenarios with high ego-motion and complex lighting.

Method: Proposes structure-aware Salient Gaussian representation initialized from LiDAR geometric and reflectance feature points, refined through salient transform and improved density control. Calibrates LiDAR intensity into reflectance and attaches it to each Gaussian as lighting-invariant material channel, jointly aligned with RGB for boundary consistency.

Result: Achieves superior reconstruction performance with fewer Gaussians and shorter training time on Waymo Open Dataset. Particularly on Complex Lighting scenes, surpasses OmniRe by 1.18 dB PSNR.

Conclusion: LR-SGS effectively leverages LiDAR reflectance and RGB complementarity to create robust and efficient reconstruction for challenging self-driving scenes, demonstrating significant improvements over existing methods.
...
### [39] [From Sparse to Dense: Multi-View GRPO for Flow Models via Augmented Condition Space](https://arxiv.org/abs/2603.12648)
...
TL;DR: MV-GRPO enhances preference alignment in text-to-image models by using multi-view reward mapping instead of single-condition evaluation, improving exploration of inter-sample relationships.


<details>
  <summary>Details</summary>
Motivation: Standard GRPO suffers from insufficient exploration of inter-sample relationships due to sparse single-view evaluation (evaluating a group of samples against a single condition), which constrains alignment efficacy and performance ceilings.

Method: Proposes Multi-View GRPO (MV-GRPO) that augments the condition space using a flexible Condition Enhancer to generate semantically adjacent yet diverse captions. This enables multi-view advantage re-estimation by deriving probability distributions of original samples conditioned on new captions without costly sample regeneration.

Result: Extensive experiments demonstrate that MV-GRPO achieves superior alignment performance over state-of-the-art methods.

Conclusion: MV-GRPO effectively addresses the limitations of single-view evaluation in GRPO by creating dense multi-view reward mapping, leading to better exploration of inter-sample relationships and improved preference alignment in text-to-image models.
...
### [40] [VGGT-World: Transforming VGGT into an Autoregressive Geometry World Model](https://arxiv.org/abs/2603.12655)
*Xiangyu Sun,Shijie Wang,Fengyi Zhang,Lin Liu,Caiyan Jia,Ziying Song,Zi Huang,Yadan Luo*

Main category: cs.CV

TL;DR: VGGT-World is a geometry world model that forecasts frozen geometry-foundation-model features instead of generating video frames, achieving better geometric consistency and faster inference.


<details>
  <summary>Details</summary>
Motivation: Traditional world models focus on photometric video frame generation but often produce geometrically inconsistent predictions. The authors propose to instead forecast the evolution of geometry features from frozen foundation models to maintain geometric consistency.

Method: Repurposes latent tokens from frozen VGGT as world state, trains lightweight temporal flow transformer to autoregressively predict future feature trajectories. Uses clean-target parameterization for higher signal-to-noise ratio and two-stage latent flow-forcing curriculum to address exposure bias.
...
### [41] [VFM-Recon: Unlocking Cross-Domain Scene-Level Neural Reconstruction with Scale-Aligned Foundation Priors](https://arxiv.org/abs/2603.12657)
*Yuhang Ming,Tingkang Xi,Xingrui Yang,Lixin Yang,Yong Peng,Cewu Lu,Wanzeng Kong*

Main category: cs.CV

TL;DR: VFMRecon bridges vision foundation model priors with scale-consistent neural volumetric reconstruction from monocular videos, achieving SOTA performance across in-distribution and out-of-distribution datasets.


<details>
  <summary>Details</summary>
Motivation: Scene-level neural volumetric reconstruction from monocular videos faces challenges with domain shifts. Vision foundation models provide transferable priors but their scale-ambiguous predictions conflict with the scale consistency needed for volumetric fusion.

Method: Introduces lightweight scale alignment to restore multiview scale coherence, then integrates pretrained VFM features via task-specific adapters that preserve cross-domain robustness while training for reconstruction.

Result: Achieves state-of-the-art performance on ScanNet test split and out-of-distribution TUM RGB-D and Tanks and Temples datasets. On Tanks and Temples, achieves F1 score of 70.1 vs closest competitor's 51.8.

Conclusion: VFMRecon successfully bridges transferable VFM priors with scale-consistent requirements for scene-level neural reconstruction, demonstrating robust performance across diverse domains.
...
TL;DR: AVION is a knowledge distillation framework that adapts vision-language models to remote sensing imagery by using a teacher module to create semantically rich textual prototypes and a student module with learnable prompts guided by the teacher.
...
### [43] [Learning Geometric and Photometric Features from Panoramic LiDAR Scans for Outdoor Place Categorization](https://arxiv.org/abs/2603.12663)
*Kazuto Nakashima,Hojung Jung,Yuki Oto,Yumi Iwashita,Ryo Kurazume,Oscar Martinez Mozos*

Main category: cs.CV

TL;DR: This paper presents a CNN-based method for outdoor place categorization using omnidirectional depth/reflectance images from 3D LiDARs, with a new MPO dataset and improved performance over traditional approaches.


<details>
  <summary>Details</summary>
Motivation: Outdoor place categorization is essential for autonomous robots/vehicles but more challenging than indoor due to perceptual variations like dynamic illumination and occlusions. Existing methods need improvement for robust outdoor scene understanding.

Method: 1) Created MPO dataset with two types of LiDAR point clouds labeled with six outdoor categories. 2) Developed CNN models that take omnidirectional depth/reflectance images as inputs for place categorization. 3) Used both depth and reflectance modalities for improved performance.

Result: The CNN approach outperforms traditional methods on the MPO dataset, demonstrating effectiveness of using both depth and reflectance modalities. Feature visualizations provide insights into what the networks learn.

Conclusion: The proposed CNN-based method with multimodal LiDAR data (depth+reflectance) effectively categorizes outdoor places, addressing challenges of outdoor scene understanding for autonomous systems.
...
TL;DR: V3Fusion introduces a novel vision-language model ensemble method using focal error diversity and CKA-based metrics to select complementary models, achieving state-of-the-art performance on multiple VLM benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing ensemble methods for Vision-Language Models (VLMs) primarily use language-based techniques, but there's a need to leverage both vision and language modalities for better model selection and fusion to improve multi-model reasoning.

Method: Proposes focal error diversity to capture complementary reasoning across VLMs and CKA-focal metric to measure visual embedding disagreements. Uses Genetic Algorithm to prune ineffective models from ensemble surface, then fuses outputs of selected VLMs to produce dual focal-diversity predictions.

Result: Outperforms best-performing VLM on MMMU by 8.09% and MMMU-Pro by 4.87% accuracy. For generative tasks, beats top-2 VLMs (Intern-VL2-8b and Qwen2.5-VL-7b) on both A-OKVQA and OCR-VQA benchmarks.

Conclusion: V3Fusion effectively combines heterogeneous VLMs using vision-language modalities, dynamically captures epistemic uncertainty, mitigates hallucinations, and achieves superior performance even when majority of models are wrong or lack consensus.
...
### [45] [Bin~Wan,G2HFNet: GeoGran-Aware Hierarchical Feature Fusion Network for Salient Object Detection in Optical Remote Sensing Images](https://arxiv.org/abs/2603.12680)
*Bin Wan,Runmin Cong,Xiaofei Zhou,Hao Fang,Chengtao Lv,Sam Kwong*

Main category: cs.CV

TL;DR: G2HFNet is a novel network for remote sensing salient object detection that leverages geometric and granular cues through hierarchical feature fusion to handle scale variations and complex backgrounds.


<details>
  <summary>Details</summary>
Motivation: Remote sensing images have significant scale variations and complex backgrounds, but existing SOD methods use uniform attention mechanisms at single scales, resulting in suboptimal representations and incomplete detection.

Method: Proposes G2HFNet with Swin Transformer backbone and four key modules: MDE for scale handling and detail enrichment, DGC for fine-grained details and positional information, DSP for high-level positional refinement via self-attention, and LGF for multi-level feature integration.
...
<details>
  <summary>Details</summary>
Motivation: Addresses the inconsistency in salient regions between RGB and thermal images, which poses challenges for cross-modal salient object detection.

Method: Two-stage approach: 1) Region guidance stage with three parallel branches using CI and SF modules to generate guidance maps and similarity scores; 2) Saliency generation stage with SO module for selective fusion, DDE module for detail enhancement, and MIS module for location cues.

Result: Achieves competitive performance against 27 state-of-the-art SOD methods on RGB-T datasets.

Conclusion: The proposed RSONet effectively handles modality inconsistency in RGB-T salient object detection through region-guided selective optimization and achieves superior performance.
...
### [47] [STRAP-ViT: Segregated Tokens with Randomized -- Transformations for Defense against Adversarial Patches in ViTs](https://arxiv.org/abs/2603.12688)
*Nandish Chattopadhyay,Anadi Goyal,Chandan Karfa,Anupam Chattopadhyay*

Main category: cs.CV

TL;DR: STRAP-ViT is a plug-and-play defense mechanism that detects adversarial patches in Vision Transformers by identifying anomalous tokens using Jensen-Shannon Divergence, then applies randomized transformations to mitigate the attack.


<details>
  <summary>Details</summary>
Motivation: Adversarial patches can hijack ViT self-attention by focusing attention on small high-contrast regions, corrupting class tokens to force misclassifications. The authors observe that tokens overlapping with adversarial noise have different statistical properties than clean tokens.

Method: Two-phase approach: 1) Detection Phase uses Jensen-Shannon Divergence to identify anomalous tokens overlapping with adversarial patches; 2) Mitigation Phase applies randomized composite transformations to those tokens (minimum transformation covers ≥50% of patch). Non-trainable plug-and-play block for inference only.

Result: Tested on ViT-base-16 and DinoV2 architectures with ImageNet and CalTech-101 datasets against multiple attacks (Adversarial Patch, LAVAN, GDPA, RP2). Achieves robust accuracies within 2-3% of clean baselines and outperforms state-of-the-art defenses.

Conclusion: STRAP-ViT provides effective, computationally efficient defense against adversarial patches in Vision Transformers without requiring retraining, demonstrating strong robustness across architectures, datasets, and attack methods.
...
TL;DR: A self-supervised system that trains bespoke object detectors from human demonstrations without language descriptions or prompt engineering, enabling robots to quickly recognize novel objects.


<details>
  <summary>Details</summary>
Motivation: Existing closed-set object detectors fail with out-of-distribution objects, while open-set detectors require expensive human-in-the-loop prompt engineering. Need a faster, more efficient way for robots to recognize novel objects shown during demonstrations.

Method: "Show, Don't Tell" paradigm: automatically creates training datasets from human demonstrations, then trains bespoke object detectors on these datasets without using language descriptions. Bypasses language entirely by showing detectors the specific objects of interest during demonstrations.

Result: Significantly outperforms state-of-the-art detection and recognition methods for manipulated objects, leading to improved task completion for real-world robots.

Conclusion: The self-supervised "Show, Don't Tell" approach eliminates need for language descriptions and prompt engineering, enabling rapid training of customized object detectors from demonstrations for improved robot performance with novel objects.
...
TL;DR: CM-Bench is a comprehensive benchmark for cross-modal feature matching that evaluates 30 algorithms across diverse datasets, introduces adaptive preprocessing, and provides a new infrared-satellite dataset for geo-localization.


<details>
  <summary>Details</summary>
Motivation: Cross-modal feature matching (especially IR-VIS) is crucial for visual localization and perception, but lacks standardized benchmarks and metrics for evaluation despite significant appearance differences between modalities.

Method: 1) Created CM-Bench benchmark evaluating 30 feature matching algorithms across diverse cross-modal datasets; 2) Categorized methods into sparse, semidense, and dense approaches; 3) Introduced classification-network-based adaptive preprocessing front-end; 4) Created novel infrared-satellite cross-modal dataset with manual ground-truth annotations.

Result: Provides comprehensive evaluation framework for cross-modal feature matching across three tasks: homography estimation, relative pose estimation, and feature-matching-based geo-localization. The benchmark and new dataset enable standardized comparison of algorithms.

Conclusion: CM-Bench addresses the critical gap in cross-modal feature matching research by providing standardized benchmarks, metrics, and a practical evaluation dataset, facilitating future development and comparison of algorithms in this challenging domain.
...
### [50] [PVI: Plug-in Visual Injection for Vision-Language-Action Models](https://arxiv.org/abs/2603.12772)
...
TL;DR: PVI is a lightweight module that injects temporal video features into VLA manipulation policies via zero-initialized residuals, improving performance on multi-phase tasks without major architectural changes.


<details>
  <summary>Details</summary>
Motivation: VLMs in manipulation policies lose fine-grained geometric cues and lack temporal evidence, while existing feature injection methods either use static representations or require heavy architectural modifications.

Method: Plug-in Visual Injection (PVI) attaches to pretrained action experts via zero-initialized residual pathways, injecting auxiliary visual representations (like V-JEPA2 video features) with only single-stage fine-tuning.

Result: PVI consistently outperforms base policies and alternative injection strategies, with temporal video features showing largest gains on multi-phase tasks requiring state tracking and coordination.

Conclusion: PVI provides a practical, encoder-agnostic approach to enhance VLA manipulation policies with temporal visual information, demonstrated effective in both simulation and real-robot cloth folding tasks.
...
### [51] [HSEmotion Team at ABAW-10 Competition: Facial Expression Recognition, Valence-Arousal Estimation, Action Unit Detection and Fine-Grained Violence Classification](https://arxiv.org/abs/2603.12693)
*Andrey V. Savchenko,Kseniia Tsypliakova*

Main category: cs.CV

TL;DR: Proposed fast facial emotion analysis approach using EfficientNet-based embeddings with confidence thresholds and MLP refinement, plus violence detection with frame embedding aggregation, achieving significant improvements in ABAW competition tasks.


<details>
  <summary>Details</summary>
Motivation: To develop efficient and accurate methods for frame-wise facial emotion understanding (expression recognition, valence-arousal estimation, action unit detection) and video-level violence detection for the ABAW competition, improving over existing baselines.

Method: 1) For facial emotion tasks: Use pre-trained EfficientNet-based emotion models to extract facial embeddings; apply confidence threshold - if high confidence, use model prediction directly; otherwise feed embeddings to MLP trained on AffWild2. 2) Apply sliding window smoothing to frame-wise predictions. 3) For violence detection: Test various pre-trained architectures for frame embeddings and aggregate them for video classification.

Result: The approach significantly improves validation metrics over existing baselines across all four tasks in the ABAW challenge, demonstrating effectiveness of the proposed confidence-based hybrid approach and embedding aggregation methods.

Conclusion: The proposed fast approach combining pre-trained emotion models with confidence thresholds and MLP refinement, along with frame embedding aggregation for video tasks, provides effective solutions for facial emotion analysis and violence detection in the ABAW competition.
...
### [52] [VCBench: A Streaming Counting Benchmark for Spatial-Temporal State Maintenance in Long Videos](https://arxiv.org/abs/2603.12703)
*Pengyiang Liu,Zhongyue Shi,Hongye Hao,Qi Fu,Xueting Bi,Siwei Zhang,Xiaoyang Hu,Zitian Wang,Linjiang Huang,Si Liu*

Main category: cs.CV

TL;DR: VCBench is a streaming counting benchmark that uses counting as a minimal probe to diagnose world state maintenance capability in video understanding models, with fine-grained subcategories and comprehensive metrics.


<details>
  <summary>Details</summary>
Motivation: Existing video understanding benchmarks insufficiently observe how models maintain world state during playback. There's a need for better diagnostic tools to evaluate models' ability to continuously track and update world state.

Method: Propose VCBench with 406 videos containing frame-by-frame annotations of 10,071 event/object state moments, generating 1,000 streaming QA pairs. Decompose world state maintenance into object counting (visible vs. cumulative) and event counting (instantaneous vs. complete cycles), forming 8 subcategories. Design three complementary metrics for numerical precision, trajectory consistency, and temporal awareness.

Result: Evaluation on mainstream video-language models shows significant deficiencies in spatial-temporal state maintenance, particularly with periodic event counting tasks. Models struggle with maintaining consistent world state tracking.

Conclusion: VCBench provides a diagnostic framework for measuring and improving state maintenance in video understanding systems, revealing current models' limitations in world state tracking and offering a structured approach for future improvements.
...
### [53] [ESPIRE: A Diagnostic Benchmark for Embodied Spatial Reasoning of Vision-Language Models](https://arxiv.org/abs/2603.13033)
*Yanpeng Zhao,Wentao Ding,Hongtao Li,Baoxiong Jia,Zilong Zheng*

Main category: cs.CV

TL;DR: ESPIRE is a diagnostic benchmark for evaluating embodied spatial reasoning in vision-language models, using simulated environments and generative task decomposition to bridge evaluation with real-world robotic applications.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations for vision-language models in embodied domains are limited in both paradigm and coverage, hindering rapid model development and failing to properly assess spatial reasoning capabilities needed for real-world robotic deployment.

Method: ESPIRE creates a simulated world to physically ground VLMs and evaluates them on spatial-reasoning-centric robotic tasks. It decomposes tasks into localization and execution phases, framing both as generative problems rather than discriminative evaluations. The benchmark is systematically designed at both instruction and environment levels for broad coverage.

Result: The authors use ESPIRE to diagnose a range of frontier VLMs and provide in-depth analysis of their spatial reasoning behaviors, though specific results are not detailed in the abstract.

Conclusion: ESPIRE addresses limitations in current VLM evaluations by providing a comprehensive diagnostic benchmark for embodied spatial reasoning that bridges the gap between evaluation and real-world robotic deployment through generative task decomposition and simulated environments.
...
TL;DR: HFP-SAM is a hierarchical frequency-prompted SAM framework for marine animal segmentation that addresses SAM's limitations in fine-grained detail perception and frequency information by incorporating frequency domain priors, frequency-aware point selection, and full-view mamba modules.


<details>
  <summary>Details</summary>
Motivation: Current deep learning-based marine animal segmentation methods struggle with long-distance modeling, and while SAM is popular for general segmentation, it lacks fine-grained detail perception and frequency information handling needed for complex marine environments.

Method: Three main components: 1) Frequency Guided Adapter (FGA) injects marine scene info into frozen SAM backbone using frequency domain prior masks; 2) Frequency-aware Point Selection (FPS) generates highlighted regions via frequency analysis to create point prompts; 3) Full-View Mamba (FVM) extracts spatial and channel contextual info with linear computational complexity for comprehensive segmentation masks.

Result: Extensive experiments on four public datasets demonstrate superior performance of HFP-SAM approach compared to existing methods.

Conclusion: The proposed HFP-SAM framework effectively addresses SAM's limitations for marine animal segmentation by incorporating frequency domain information and efficient contextual modeling, achieving state-of-the-art performance with publicly available source code.
...
TL;DR: TPSNet uses CLIP-generated text prompts and domain-invariant phase features as dual priors to improve unsupervised cross-domain image retrieval by providing better semantic guidance and preserving semantic integrity during domain alignment.


<details>
  <summary>Details</summary>
Motivation: Existing UCDIR methods rely on discrete pseudo-labels from clustering, which provide inaccurate semantic guidance. They also overlook domain-semantic entanglement, causing semantic degradation in learned representations and poor retrieval performance.

Method: TPSNet introduces dual priors: 1) Text prior - CLIP-generated domain-specific prompts for precise semantic supervision, 2) Phase prior - domain-invariant phase features integrated into image representations to bridge domain gaps while preserving semantics.

Result: TPSNet significantly outperforms state-of-the-art methods on unsupervised cross-domain image retrieval benchmarks.

Conclusion: The synergy of text and phase priors effectively addresses limitations of existing UCDIR methods by providing accurate semantic guidance and preserving semantic integrity during cross-domain alignment, leading to superior retrieval performance.
...
TL;DR: The paper introduces a new semantic labeling approach (SSU) and decomposable metric (COTe) for document layout analysis that addresses limitations of traditional object detection metrics like IoU/F1/mAP, showing COTe is more informative and reduces interpretation-performance gaps by up to 76%.


<details>
  <summary>Details</summary>
Motivation: Traditional object detection metrics (IoU, F1, mAP) are designed for 2D projections of 3D space, not for natively 2D printed media, leading to misleading or uninformative interpretation of document layout analysis model performance.

Method: Proposes two innovations: 1) Structural Semantic Unit (SSU) - a relational labeling approach focusing on semantic structure rather than physical structure; 2) Coverage, Overlap, Trespass, and Excess (COTe) score - a decomposable metric for measuring page parsing quality. Evaluated 5 common DLA models on 3 datasets.

Result: COTe score is more informative than traditional metrics, reveals distinct failure modes (e.g., breaching semantic boundaries, repeated parsing), reduces interpretation-performance gap by up to 76% relative to F1, and maintains robustness even without explicit SSU labeling. Released SSU-labeled dataset and Python library.

Conclusion: The proposed SSU and COTe methods enable more robust, comparable, and nuanced document layout analysis by shifting focus from physical to semantic structure and providing a decomposable metric that better captures parsing quality, with practical tools released for community adoption.
...
TL;DR: CMHANet is a cross-modal hybrid attention network for robust point cloud registration that fuses 2D image context with 3D point cloud geometry and uses contrastive learning for improved robustness to noise and partial data.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based point cloud registration methods degrade in real-world scenarios with incomplete data, sensor noise, and low overlap regions, limiting their practical application in 3D reconstruction, AR, and scene understanding.

Method: Proposes CMHANet, a cross-modal hybrid attention network that integrates 2D image contextual information with 3D point cloud geometric details. Introduces a contrastive learning-based optimization function to enforce geometric consistency and improve robustness to noise and partial observations.

Result: Achieves substantial improvements in registration accuracy and robustness on 3DMatch and challenging 3DLoMatch datasets. Zero-shot evaluations on TUM RGB-D SLAM dataset demonstrate strong generalization to unseen domains, outperforming current techniques.

Conclusion: CMHANet effectively addresses robustness challenges in point cloud registration by leveraging cross-modal fusion and contrastive learning, making it suitable for real-world applications with incomplete and noisy data.
...
<details>
  <summary>Details</summary>
Motivation: Addresses challenges in EEG-based visual reconstruction including fidelity loss and representation shift by leveraging complementary multi-modal information.

Method: Proposes CognitionCapturerPro with uncertainty-weighted similarity scoring, fusion encoder for shared representations, simplified alignment module, and pre-trained diffusion model.

Result: Outperforms original CognitionCapturer on THINGS-EEG dataset with 25.9% Top-1 and 10.6% Top-5 retrieval accuracy improvements.

Conclusion: Integration of EEG with multi-modal priors via collaborative training and uncertainty-aware fusion effectively enhances visual stimulus reconstruction from brain signals.
...
### [60] [MoKus: Leveraging Cross-Modal Knowledge Transfer for Knowledge-Aware Concept Customization](https://arxiv.org/abs/2603.12743)
*Chenyang Zhu,Hongxiang Li,Xiu Li,Long Chen*

Main category: cs.CV

TL;DR: MoKus is a novel framework for knowledge-aware concept customization that binds diverse textual knowledge to target visual concepts through cross-modal knowledge transfer, outperforming existing methods on the new KnowCusBench benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing concept customization methods that use rare tokens suffer from unstable performance and fail to convey inherent knowledge of target concepts, creating a need for a more robust approach that can bind diverse textual knowledge to visual concepts.

Method: MoKus uses a two-stage approach: (1) visual concept learning to create anchor representations storing visual information, and (2) textual knowledge updating that modifies knowledge queries to align with anchor representations, leveraging cross-modal knowledge transfer where textual modifications naturally transfer to visual generation.

Result: MoKus outperforms state-of-the-art methods on the new KnowCusBench benchmark, demonstrates extensibility to other knowledge-aware applications (virtual concept creation, concept erasure), and shows improvements on world knowledge benchmarks.

Conclusion: The proposed knowledge-aware concept customization task and MoKus framework effectively address limitations of existing methods by enabling high-fidelity customized generation through cross-modal knowledge transfer, with strong performance and broad applicability.
...
### [61] [Thinking in Dynamics: How Multimodal Large Language Models Perceive, Track, and Reason Dynamics in Physical 4D World](https://arxiv.org/abs/2603.12746)
...
TL;DR: Dyn-Bench is a new benchmark for evaluating MLLMs' ability to understand dynamic 4D scenes (spatial + temporal), testing both spatio-temporal reasoning and dynamic object grounding. Current models struggle with these tasks, but structured integration approaches show promise.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs excel at static visual understanding but lack capabilities for "thinking in dynamics" - perceiving, tracking, and reasoning about spatio-temporal dynamics in evolving scenes. There's a need to systematically assess their ability to understand the physical 4D world where geometric structure and semantic content evolve over time.

Method: Created Dyn-Bench, a large-scale benchmark from diverse real-world and synthetic video datasets through multi-stage filtering. Contains 1k videos, 7k VQA pairs, and 3k dynamic object grounding pairs. Evaluated MLLMs using conventional prompting strategies and structured integration approaches including Mask-Guided Fusion and Spatio-Temporal Textual Cognitive Map (ST-TCM).
...
Conclusion: The paper introduces Dyn-Bench to systematically evaluate MLLMs' dynamic scene understanding capabilities. Current models have significant limitations in 4D reasoning, but structured approaches like Mask-Guided Fusion and ST-TCM offer promising directions for improving spatio-temporal understanding in physical 4D worlds.
...
### [62] [SLICE: Semantic Latent Injection via Compartmentalized Embedding for Image Watermarking](https://arxiv.org/abs/2603.12749)
*Zheng Gao,Yifan Yang,Xiaoyu Li,Xiaoyan Feng,Haoran Fan,Yang Song,Jiaojiao Jiang*

Main category: cs.CV

TL;DR: SLICE proposes a semantic-aware watermarking method that decouples image semantics into four factors and anchors them to distinct noise regions, enabling robust detection and localization of semantic tampering.
...
Method: SLICE decouples image semantics into four semantic factors (subject, environment, action, detail) and precisely anchors them to distinct regions in the initial Gaussian noise of diffusion models. This fine-grained semantic binding enables advanced verification where semantic tampering is detectable and localizable.

Result: SLICE significantly outperforms existing baselines against advanced semantic-guided regeneration attacks, substantially reducing attack success while preserving image quality and semantic fidelity. It provides statistical guarantees on false-accept rates.

Conclusion: SLICE offers a practical, training-free provenance solution that is both fine-grained in diagnosis and robust to realistic adversarial manipulations, addressing limitations of previous semantic-aware watermarking approaches.
...
### [63] [FC-Track: Overlap-Aware Post-Association Correction for Online Multi-Object Tracking](https://arxiv.org/abs/2603.12758)
...
<details>
  <summary>Details</summary>
Motivation: Online MOT methods are vulnerable to identity switches from frequent occlusions and object overlap, where incorrect associations propagate over time and degrade tracking reliability, especially for robotic systems in dynamic environments.
...
Conclusion: FC-Track effectively mitigates long-term identity switches without global optimization or re-identification, making it suitable for real-time robotic applications while maintaining state-of-the-art performance on MOT20 benchmark.
...
### [64] [SAP: Segment Any 4K Panorama](https://arxiv.org/abs/2603.12759)
...
TL;DR: Segment Any 4K Panorama (SAP) is a foundation model for 4K panoramic instance segmentation that treats panoramas as fixed-trajectory perspective videos, achieving significant zero-shot performance gains over SAM2 on real-world 4K panoramas.
...
Method: Reformulates panoramic segmentation as fixed-trajectory perspective video segmentation by decomposing panoramas into overlapping perspective patches sampled along continuous spherical traversal. Uses large-scale synthetic supervision with 183,440 4K panoramic images generated by InfiniGen engine.
...
Conclusion: SAP successfully addresses the performance degradation of perspective-trained models on panoramas through trajectory-aligned reformulation and large-scale synthetic training, enabling high-quality 4K panoramic instance segmentation.
...
### [65] [HIFICL: High-Fidelity In-Context Learning for Multimodal Tasks](https://arxiv.org/abs/2603.12760)
*Xiaoyu Li,Yuhang Liu,Zheng Luo,Xuanshuo Kang,Fangqi Lou,Xiaohua Wu,Zihan Xiong*

Main category: cs.CV

TL;DR: HiFICL improves In-Context Learning for Large Multimodal Models by introducing virtual key-value pairs and low-rank factorization to better approximate the attention mechanism, outperforming existing methods on multimodal benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current In-Context Learning (ICL) for Large Multimodal Models is sensitive to demonstration configurations and computationally expensive. Existing approximation methods that learn a "shift vector" oversimplify the attention mechanism decomposition.

Method: HiFICL introduces three components: 1) virtual key-value pairs as learnable context, 2) low-rank factorization for stable training, and 3) end-to-end training objective. This approach more faithfully models the ICL mechanism as context-aware Parameter-Efficient Fine-Tuning.
...
Conclusion: HiFICL provides a more accurate and effective approach to In-Context Learning approximation for Large Multimodal Models, with code publicly available for further research and application.
...
TL;DR: Cheers is a unified multimodal model that decouples patch-level details from semantic representations to enable both visual comprehension and generation in a single model with 4x token compression.


<details>
  <summary>Details</summary>
Motivation: Current multimodal models struggle to unify visual comprehension and generation due to mismatched decoding regimes and visual representations, making joint optimization in a shared feature space challenging.
...
Result: Cheers matches or surpasses advanced UMMs in both visual understanding and generation, achieves 4x token compression for efficient high-resolution image encoding/generation, outperforms Tar-1.5B on GenEval and MMBench with only 20% training cost.

Conclusion: Cheers demonstrates effective and efficient unified multimodal modeling through semantic-detail decoupling, achieving strong performance in both comprehension and generation tasks with significant token compression and reduced training costs.
...
### [67] [TerraFlow: Multimodal, Multitemporal Representation Learning for Earth Observation](https://arxiv.org/abs/2603.12762)
*Nazar Puriy,Johannes Jakubik,Benedikt Blumenstiel,Konrad Schindler*

Main category: cs.CV

TL;DR: TerraFlow is a novel multimodal, multitemporal learning approach for Earth observation that outperforms state-of-the-art foundation models on temporal tasks and shows promise for natural disaster risk map prediction.


<details>
  <summary>Details</summary>
Motivation: Earth observation data presents challenges with variable-length multimodal and multitemporal inputs, and existing foundation models struggle with temporal tasks and natural disaster risk prediction where they often collapse.

Method: TerraFlow uses temporal training objectives that enable sequence-aware learning across space, time, and modality while remaining robust to variable-length inputs commonly found in real-world Earth observation data.

Result: TerraFlow demonstrates superiority over state-of-the-art foundation models across all temporal tasks in the GEO-Bench-2 benchmark, and shows initial success in deep-learning based risk map prediction for natural disasters where other models frequently fail.

Conclusion: TerraFlow outperforms state-of-the-art foundation models by up to 50% in F1 score and 24% in Brier score, making it a promising approach for multimodal, multitemporal Earth observation tasks including challenging applications like natural disaster risk prediction.
...
### [68] [SAVA-X: Ego-to-Exo Imitation Error Detection via Scene-Adaptive View Alignment and Bidirectional Cross View Fusion](https://arxiv.org/abs/2603.12764)
*Xiang Li,Heqian Qiu,Lanxiao Wang,Benliu Qiu,Fanman Meng,Linfeng Xu,Hongliang Li*

Main category: cs.CV

TL;DR: SAVA-X is a new framework for detecting imitation errors when comparing first-person (ego) actions to third-person (exo) demonstrations, addressing cross-view domain shift and temporal misalignment challenges.


<details>
  <summary>Details</summary>
Motivation: Current error detection methods assume single-view settings and fail to handle practical scenarios where third-person demonstrations are used to assess first-person imitations, which involves cross-view domain shift, temporal misalignment, and heavy redundancy.

Method: SAVA-X uses an Align-Fuse-Detect framework with three key components: (1) view-conditioned adaptive sampling, (2) scene-adaptive view embeddings, and (3) bidirectional cross-attention fusion to handle the ego→exo imitation error detection problem.
...
Conclusion: The paper introduces a novel cross-view imitation error detection problem and proposes SAVA-X as an effective solution that outperforms existing methods, with code made publicly available for further research.
...
### [69] [Hierarchical Dual-Change Collaborative Learning for UAV Scene Change Captioning](https://arxiv.org/abs/2603.12832)
...
<details>
  <summary>Details</summary>
Motivation: Traditional change captioning focuses on fixed-camera temporal differences, but UAV imagery introduces both temporal and spatial variations from moving viewpoints. Need to understand viewpoint-induced scene changes with partially overlapping content and exploit relative orientation between images.
...
Result: Proposed method achieves state-of-the-art performance on the new UAV-SCC task. Created a new benchmark dataset for UAV scene change captioning that will be publicly released.

Conclusion: UAV-SCC is a novel and challenging task requiring understanding of viewpoint-induced changes. The proposed HDC-CL method effectively addresses these challenges through adaptive layout modeling and orientation calibration, setting new benchmarks for the field.
...
### [70] [Catalyst4D: High-Fidelity 3D-to-4D Scene Editing via Dynamic Propagation](https://arxiv.org/abs/2603.12766)
*Shifeng Chen,Yihui Li,Jun Liao,Hongyu Yang,Di Huang*

Main category: cs.CV

TL;DR: Catalyst4D is a framework for editing dynamic 4D Gaussian scenes by transferring high-quality 3D edits while maintaining spatial and temporal coherence, addressing motion artifacts and flickering issues in existing methods.


<details>
  <summary>Details</summary>
Motivation: Dynamic scene editing remains challenging as existing methods that extend 2D diffusion models to 4D often produce motion artifacts, temporal flickering, and inconsistent style propagation, despite advances in static 3D scene editing.

Method: Two core components: 1) Anchor-based Motion Guidance (AMG) builds structurally stable anchors from original and edited Gaussians, using optimal transport for consistent deformation propagation without cross-region interference. 2) Color Uncertainty-guided Appearance Refinement (CUAR) preserves temporal appearance consistency by estimating per-Gaussian color uncertainty and selectively refining occlusion-prone regions.
...
Conclusion: Catalyst4D successfully addresses key challenges in dynamic scene editing by maintaining spatial and temporal coherence through anchor-based motion guidance and uncertainty-aware appearance refinement, enabling high-quality 4D scene editing.
...
TL;DR: Multimodal fusion approach for video-level ambivalence/hesitancy recognition achieves 83.25% MF1, outperforming unimodal baselines (70.02%) through integration of scene, face, audio, and text modalities with prototype-augmented fusion models.


<details>
  <summary>Details</summary>
Motivation: Ambivalence/hesitancy recognition in unconstrained videos is challenging due to the subtle, multimodal, and context-dependent nature of this behavioral state, requiring comprehensive multimodal analysis.

Method: Integrates four modalities: scene dynamics (VideoMAE-based model), facial information (emotional frame-level embeddings with statistical pooling), audio (EmotionWav2Vec2.0 with Mamba-based temporal encoder), and text (fine-tuned transformer models). Uses multimodal fusion models including prototype-augmented variants.

Result: Multimodal fusion achieved 83.25% average MF1 vs 70.02% for best unimodal configuration. Ensemble of five prototype-augmented fusion models achieved 71.43% final test performance on BAH corpus.

Conclusion: Results highlight the importance of complementary multimodal cues and robust fusion strategies for effective ambivalence/hesitancy recognition in videos.
...
### [72] [Finite Difference Flow Optimization for RL Post-Training of Text-to-Image Models](https://arxiv.org/abs/2603.12893)
*David McAllister,Miika Aittala,Tero Karras,Janne Hellsten,Angjoo Kanazawa,Timo Aila,Samuli Laine*

Main category: cs.CV

TL;DR: Online RL variant for diffusion models reduces variance by sampling paired trajectories and treating entire sampling process as single action, achieving faster convergence and better quality than previous methods.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods for post-training diffusion models treat each sampling step as separate policy actions, which can lead to high variance in model updates. The authors aim to develop a more stable and efficient RL approach for improving diffusion-based image synthesis.

Method: Proposes an online RL variant that samples paired trajectories and pulls the flow velocity toward more favorable images. Key innovation: treats the entire sampling process as a single action rather than separate step-wise actions. Uses both high-quality vision language models and off-the-shelf quality metrics for reward signals.

Result: Method converges faster than previous approaches and yields higher output quality and better prompt alignment. Evaluated using a broad set of metrics showing superior performance.

Conclusion: Treating the entire sampling process as a single action in RL for diffusion models reduces variance in updates and leads to more efficient training with improved results in both quality and prompt alignment.
...
### [73] [Generalized Recognition of Basic Surgical Actions Enables Skill Assessment and Vision-Language-Model-based Surgical Planning](https://arxiv.org/abs/2603.12787)
...
TL;DR: Researchers created the largest basic surgical actions (BSA) dataset with 10 actions across 6 specialties (11,000+ clips) and developed a foundation model for robust cross-specialty BSA recognition, enabling downstream applications like skill assessment and action planning.


<details>
  <summary>Details</summary>
Motivation: Basic surgical actions (BSA) are fundamental units of surgery, and understanding/modeling them is crucial for advancing AI in surgical practice, training, and automation. Current limitations include lack of comprehensive datasets and models that work across different surgical specialties.

Method: 1) Created largest BSA dataset with 10 basic actions across 6 surgical specialties (11,000+ video clips). 2) Developed a foundation model for general-purpose BSA recognition. 3) Tested cross-specialty performance on different procedural types and body parts. 4) Applied model to downstream tasks: surgical skill assessment in prostatectomy and action planning in cholecystectomy/nephrectomy using large vision-language models.

Result: 1) Foundation model demonstrated robust cross-specialty performance across different procedures and body parts. 2) Enabled surgical skill assessment with domain-specific knowledge. 3) Generated clinically relevant action planning explanations via large vision-language models (validated by multinational surgeons). 4) Showed BSA can be robustly recognized across diverse surgical scenarios.

Conclusion: Accurate BSA understanding models can facilitate complex surgical applications and accelerate the development of surgical superintelligence. The foundation model enables robust cross-specialty recognition and supports downstream tasks like skill assessment and action planning with clinical relevance.
...
### [74] [FedBPrompt: Federated Domain Generalization Person Re-Identification via Body Distribution Aware Visual Prompts](https://arxiv.org/abs/2603.12912)
*Xin Xu,Weilong Li,Wei Liu,Wenke Huang,Zhixi Yu,Bin Yang,Xiaoying Liao,Kui Jiang*

Main category: cs.CV

TL;DR: FedBPrompt introduces visual prompts for federated domain generalization in person re-identification, using body-aware prompts to guide ViT attention and reduce communication costs through prompt-only fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers in federated person re-identification struggle with distinguishing pedestrians from similar backgrounds and diverse viewpoints, especially with cross-client distribution shifts. Existing methods lack efficient attention guidance and suffer from high communication costs in federated learning.

Method: FedBPrompt uses Body Distribution Aware Visual Prompts Mechanism (BAPM) with two components: Holistic Full Body Prompts to suppress background noise, and Body Part Alignment Prompts to capture fine-grained details. It also employs Prompt-based Fine-Tuning Strategy (PFTS) that freezes the ViT backbone and updates only lightweight prompts to reduce communication overhead.

Result: The method effectively enhances feature discrimination and cross-domain generalization, achieves notable performance gains within few aggregation rounds, and significantly reduces communication costs while maintaining adaptability.

Conclusion: FedBPrompt provides a flexible and effective solution for federated person re-identification that can be easily integrated into existing ViT-based frameworks, addressing both attention guidance and communication efficiency challenges.
...
### [75] [Think and Answer ME: Benchmarking and Exploring Multi-Entity Reasoning Grounding in Remote Sensing](https://arxiv.org/abs/2603.12788)
*Shuchang Lyu,Haiquan Wen,Guangliang Cheng,Meng Li,Zheng Zhou,You Zhou,Dingding Yao,Zhenwei Shi*

Main category: cs.CV

TL;DR: A new benchmark ME-RSRG and EAR framework for multi-entity reasoning grounding in remote sensing, using structured reasoning traces and entity-aware reinforcement learning optimization.


<details>
  <summary>Details</summary>
Motivation: Existing remote sensing grounding methods are limited to perception-level matching and single-entity formulations, lacking explicit reasoning and inter-entity modeling capabilities despite advances in reasoning language models.

Method: Proposes Entity-Aware Reasoning (EAR) framework built on visual-linguistic foundation models that generates structured reasoning traces and subject-object grounding outputs, using supervised fine-tuning for initialization and entity-aware reward-driven Group Relative Policy Optimization (GRPO) for further optimization.
...
Conclusion: The work successfully extends reasoning paradigms to remote sensing visual grounding through a new multi-entity reasoning benchmark and an effective entity-aware reasoning framework with reinforcement learning optimization.
...
TL;DR: Proposes a structure-faithful framework for machine unlearning using semantic anchors to maintain knowledge structure while removing designated data, achieving significant performance gains across tasks.
...
Method: Introduces stakes (semantic anchors) derived from language-driven attribute descriptions encoded by semantic encoders like CLIP. Uses structure-aware alignment to maintain knowledge organization before/after unlearning around anchors, and structure-aware regularization to regulate updates to structure-critical parameters.

Result: Achieves average performance gains of 32.9% in image classification, 22.5% in retrieval, and 19.3% in face recognition, effectively balancing the deletion-retention trade-off and enhancing generalization.

Conclusion: The proposed structure-faithful framework with semantic anchors successfully addresses structural collapse in machine unlearning, preserving knowledge organization while removing designated data, leading to improved performance across multiple vision tasks.
...
<details>
  <summary>Details</summary>
Motivation: Most existing 3D human reconstruction approaches focus on monocular inputs, and extending to multi-view settings requires additional overhead modules or preprocessed data. There's a need for a unified solution that handles multi-person multi-view reconstruction efficiently.

Method: Integrates geometric and human priors from Pi3X and Multi-HMR into a single trainable neural network. Introduces scale adjustment module to solve scale discrepancy between humans and scene, multi-view fusion strategy to aggregate per-view estimates, and geometry-based multi-person association method.
...
Conclusion: CHROMM provides an efficient, unified framework for multi-person multi-view reconstruction without external dependencies, demonstrating strong performance and significant speed improvements over existing methods.
...
### [78] [Thinking in Streaming Video](https://arxiv.org/abs/2603.12938)
...
<details>
  <summary>Details</summary>
Motivation: Existing video reasoning approaches use batch processing which causes high latency and growing computational costs, making them unsuitable for real-time streaming scenarios like interactive assistants.

Method: Watch-Think-Speak paradigm with Reasoning-Compressed Streaming Memory (RCSM) that compresses reasoning traces into semantic memory, and Streaming Reinforcement Learning with Verifiable Rewards for training.

Result: Outperforms existing online video models on multiple streaming benchmarks while maintaining low latency and memory usage.

Conclusion: ThinkStream provides an effective framework for streaming video reasoning that enables real-time understanding with incremental updates and efficient memory management.
...
### [79] [Spectral Defense Against Resource-Targeting Attack in 3D Gaussian Splatting](https://arxiv.org/abs/2603.12796)
*Yang Chen,Yi Yu,Jiaming He,Yueqi Duan,Zheng Zhu,Yap-Peng Tan*

Main category: cs.CV

TL;DR: Proposes Spectral Defense against resource-targeting attacks on 3D Gaussian Splatting by filtering abnormal high-frequency Gaussians and regularizing renderings to distinguish natural vs. attack-induced frequencies.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting is vulnerable to resource-targeting attacks where poisoned training images cause excessive Gaussian growth leading to resource exhaustion. Existing spatial-domain defenses overlook how stealthy perturbations distort spectral behaviors of training data, causing abnormal high-frequency amplifications that mislead 3DGS into interpreting noise as detailed structures.

Method: Two-part spectral defense: 1) 3D frequency filter to selectively prune Gaussians exhibiting abnormally high frequencies, and 2) 2D spectral regularization on renderings that distinguishes naturally isotropic frequencies while penalizing anisotropic angular energy to constrain noisy patterns.

Result: Defense suppresses Gaussian overgrowth by up to 5.92×, reduces memory consumption by up to 3.66×, and improves rendering speed by up to 4.34× under attacks while maintaining robust, accurate, and secure 3DGS.

Conclusion: Spectral defense effectively addresses resource-targeting attacks on 3DGS by operating in the frequency domain to distinguish between legitimate scene details and attack-induced noise, providing comprehensive protection against Gaussian overgrowth and resource exhaustion.
...
### [80] [What Makes VLMs Robust? Towards Reconciling Robustness and Accuracy in Vision-Language Models](https://arxiv.org/abs/2603.12799)
*Sen Nie,Jie Zhang,Zhongqi Wang,Zhaoyang Wei,Shiguang Shan,Xilin Chen*

Main category: cs.CV

TL;DR: R-Adapt is a framework that freezes pre-trained VLM weights and adds minimal adaptations only in shallow layers to achieve exceptional balance between adversarial robustness and clean accuracy, based on findings that robustness is localized in shallow layers.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the long-standing trade-off between adversarial robustness and clean accuracy in Vision-Language Models (VLMs), where improving robustness typically degrades performance on clean data. The authors investigate what makes VLMs robust internally to understand this trade-off better.

Method: Through analysis of adversarially fine-tuned models, the authors discover that adversarial robustness is localized in shallow layers, driven by low-frequency spectral bias and input-insensitive attention patterns. They propose R-Adapt, which freezes all pre-trained weights and introduces minimal adaptations only in initial layers, supporting training-free, model-guided, and data-driven paradigms.

Result: Extensive evaluations on 18 datasets and diverse tasks demonstrate state-of-the-art performance under various attacks. R-Adapt generalizes efficiently to large VLMs (e.g., LLaVA and Qwen-VL) to enhance their robustness while maintaining clean accuracy.

Conclusion: The paper shows that adversarial robustness in VLMs is primarily localized in shallow layers, and by freezing pre-trained weights and adding minimal adaptations only in these layers, R-Adapt achieves exceptional balance between robustness and accuracy, offering flexible pathways to equip standard models with robustness.
...
### [81] [Fair Lung Disease Diagnosis from Chest CT via Gender-Adversarial Attention Multiple Instance Learning](https://arxiv.org/abs/2603.12988)
*Aditya Parikh,Aasa Feragen*

Main category: cs.CV

TL;DR: A fairness-aware MIL framework for multi-class lung disease diagnosis from chest CT scans, using attention-based ConvNeXt with GRL to suppress gender bias, achieving competitive performance while addressing demographic imbalances.


<details>
  <summary>Details</summary>
Motivation: Address two core challenges in fair lung disease diagnosis: sparse pathological signals across hundreds of CT slices, and severe demographic imbalances across disease class and gender, while meeting the Fair Disease Diagnosis Challenge requirements for gender-equitable predictions.

Method: Attention-based Multiple Instance Learning (MIL) on ConvNeXt backbone to identify relevant slices without supervision, augmented with Gradient Reversal Layer (GRL) to adversarially suppress gender-predictive features. Training uses focal loss with label smoothing, stratified cross-validation, and targeted oversampling of underrepresented subgroups.

Result: Achieves mean validation competition score of 0.685 (std 0.030), with best single fold reaching 0.759. Five-fold ensemble with test-time augmentation and threshold optimization provides robust performance.

Conclusion: The proposed fairness-aware MIL framework effectively addresses both technical (sparse signals) and fairness (demographic imbalance) challenges in lung disease diagnosis, providing a competitive solution for the CVPR 2026 challenge with publicly available implementation.
...
### [82] [OARS: Process-Aware Online Alignment for Generative Real-World Image Super-Resolution](https://arxiv.org/abs/2603.12811)
*Shijie Zhao,Xuanyu Zhang,Bin Chen,Weiqi Li,Qunliang Xing,Kexin Zhang,Yan Wang,Junlin Li,Li Zhang,Jian Zhang,Tianfan Xue*

Main category: cs.CV

TL;DR: OARS: Online Alignment framework for Real-world Image Super-Resolution using COMPASS, a MLLM-based reward model that balances fidelity and perception with adaptive trade-off.


<details>
  <summary>Details</summary>
Motivation: Aligning generative real-world image SR models with human visual preference is challenging due to perception-fidelity trade-off and diverse unknown degradations. Prior approaches rely on offline preference optimization and static metric aggregation, which are non-interpretable and prone to pseudo-diversity under strong conditioning.

Method: Proposes OARS (process-aware online alignment framework) built on COMPASS, a MLLM-based reward that evaluates LR-to-SR transition by jointly modeling fidelity preservation and perceptual gain with input-quality-adaptive trade-off. Uses three-stage perceptual annotation pipeline to create COMPASS-20K dataset. OARS performs progressive online alignment from cold-start flow matching to full-reference and finally reference-free RL via shallow LoRA optimization for on-policy exploration.
...
Conclusion: OARS framework successfully addresses the perception-fidelity trade-off in real-world image SR through online alignment guided by a sophisticated MLLM-based reward model, outperforming previous methods on real-world benchmarks.
...
### [83] [coDrawAgents: A Multi-Agent Dialogue Framework for Compositional Image Generation](https://arxiv.org/abs/2603.12829)
*Chunhan Li,Qifeng Wu,Jia-Hui Pan,Ka-Hei Hui,Jingyu Hu,Yuming Jiang,Bin Sheng,Xihui Liu,Wenjuan Gong,Zhengzhe Liu*

Main category: cs.CV

TL;DR: coDrawAgents is a multi-agent dialogue framework with four specialized agents (Interpreter, Planner, Checker, Painter) that collaborate to improve compositional text-to-image generation by addressing layout complexity, visual context grounding, and explicit error correction.
...
Method: A four-agent interactive framework: Interpreter adaptively chooses generation pathway and parses prompts; Planner uses divide-and-conquer strategy for incremental layout planning; Checker validates spatial consistency and attribute alignment; Painter synthesizes images step-by-step with visual context.
...
Conclusion: The multi-agent collaborative approach effectively addresses key challenges in compositional text-to-image generation through specialized agents working together in an interactive dialogue framework.
...
### [84] [SortScrews: A Dataset and Baseline for Real-time Screw Classification](https://arxiv.org/abs/2603.13027)
*Tianhao Fu,Bingxuan Yang,Juncheng Guo,Shrena Sribalan,Yucheng Chen*

Main category: cs.CV

TL;DR: SortScrews dataset for screw type classification with 560 RGB images, 6 screw types + background class, captured under controlled conditions with baseline models achieving strong accuracy despite small dataset size.
...
Method: Created SortScrews dataset with 560 RGB images at 512×512 resolution covering 6 screw types and background class, captured using standardized acquisition setup with mild lighting and perspective variations. Provided reusable data collection script for custom hardware components. Established baseline results using transfer learning with EfficientNet-B0 and ResNet-18 classifiers pretrained on ImageNet, plus failure analysis.

Result: Lightweight models achieved strong classification accuracy despite limited dataset size, demonstrating that controlled acquisition conditions enable effective learning even with relatively small datasets.

Conclusion: The SortScrews dataset, collection pipeline, and baseline training code are publicly available, facilitating reproducible research and dataset expansion for screw classification tasks in industrial automation applications.
...
### [85] [Are General-Purpose Vision Models All We Need for 2D Medical Image Segmentation? A Cross-Dataset Empirical Study](https://arxiv.org/abs/2603.13044)
*Vanessa Borst,Samuel Kounev*

Main category: cs.CV

TL;DR: GP-VMs outperform specialized medical segmentation architectures across multiple datasets, suggesting general-purpose vision models can be viable alternatives for medical image segmentation.


<details>
  <summary>Details</summary>
Motivation: There's insufficient understanding of whether specialized medical segmentation architectures (SMAs) provide systematic advantages over modern general-purpose vision models (GP-VMs) for 2D medical image segmentation, despite GP-VMs' strong performance on natural image benchmarks.

Method: Controlled empirical study comparing 11 SMAs and GP-VMs using unified training/evaluation protocol across three heterogeneous datasets covering different modalities, class structures, and data characteristics. Includes qualitative Grad-CAM visualizations for explainability analysis.

Result: GP-VMs outperform majority of specialized MIS models across analyzed datasets. XAI analyses show GP-VMs can capture clinically relevant structures without explicit domain-specific architectural design.

Conclusion: GP-VMs represent viable alternatives to domain-specific methods for medical image segmentation, highlighting importance of informed model selection for end-to-end MIS systems.
...
### [86] [Multimodal Protein Language Models for Enzyme Kinetic Parameters: From Substrate Recognition to Conformational Adaptation](https://arxiv.org/abs/2603.12845)
*Fei Wang,Xinye Zheng,Kun Li,Yanyan Wei,Yuxin Liu,Ganpeng Hu,Tong Bao,Jingwen Yang*

Main category: cs.CV

TL;DR: ERBA is a two-stage multimodal adapter that injects substrate information into protein language models for more accurate enzyme kinetic prediction by modeling both substrate recognition and conformational adaptation.


<details>
  <summary>Details</summary>
Motivation: Current learning approaches for enzyme kinetic prediction treat it as a static compatibility problem between enzyme and substrate, overlooking the staged nature of catalysis involving both substrate recognition and conformational adaptation.

Method: ERBA uses two-stage conditioning: Molecular Recognition Cross-Attention (MRCA) injects substrate info into enzyme representation for specificity, and Geometry-aware Mixture-of-Experts (G-MoE) integrates active-site structure with pocket-specialized experts. Enzyme-Substrate Distribution Alignment (ESDA) maintains semantic fidelity in PLM manifold.
...
Conclusion: ERBA offers a biologically grounded approach for scalable kinetic prediction and provides a foundation for incorporating additional factors like cofactors, mutations, and time-resolved structural cues.
...
### [87] [Team RAS in 10th ABAW Competition: Multimodal Valence and Arousal Estimation Approach](https://arxiv.org/abs/2603.13056)
...
<details>
  <summary>Details</summary>
Motivation: Continuous emotion recognition under in-the-wild conditions is challenging due to variations in appearance, pose, illumination, occlusions, and subject-specific expression patterns. Existing methods struggle with these real-world complexities.

Method: Three-modality approach: 1) Face modality uses GRADA-based frame embeddings with Transformer temporal regression; 2) Behavior modality uses Qwen3-VL-4B-Instruct for video segment analysis with Mamba for temporal modeling; 3) Audio modality uses WavLM-Large with attention-statistics pooling and cross-modal filtering. Two fusion strategies: Directed Cross-Modal Mixture-of-Experts and Reliability-Aware Audio-Visual Fusion.
...
Conclusion: The proposed multimodal fusion strategy effectively combines complementary information from face, behavior, and audio modalities, addressing challenges of in-the-wild emotion recognition and achieving competitive performance on benchmark datasets.
...
TL;DR: Vision-based hierarchical classification framework for automated wear condition monitoring of abrasive flap wheels using EfficientNetV2 and Grad-CAM, achieving 93.8-99.3% accuracy across three logical wear assessment levels.


<details>
  <summary>Details</summary>
Motivation: Abrasive flap wheels are flexible tools for finishing complex surfaces, but their flexibility leads to complex wear patterns (concave/convex profiles, tears) that affect grinding quality. Manual monitoring is inefficient, so automated wear condition monitoring is needed for better process control.

Method: Proposes a hierarchical classification framework with three levels: (1) state detection (new vs. worn), (2) wear type identification (rectangular, concave, convex) and flap tear detection, (3) severity assessment (partial vs. complete deformation). Uses transfer learning with EfficientNetV2 architecture on a custom-built dataset of real flap wheel images, with Grad-CAM for model interpretability.
...
Conclusion: The hierarchical method provides a basis for adaptive process control and wear consideration in automated flap wheel grinding, enabling automated monitoring of complex wear patterns in flexible abrasive tools.
...
TL;DR: CompoSIA is a compositional driving video simulator that disentangles traffic factors (scene structure, object identity, ego actions) for fine-grained control over adversarial driving scenarios, enabling synthesis of dangerous combinations that entangled generators cannot produce.


<details>
  <summary>Details</summary>
Motivation: The "long tail" of safety-critical edge cases in autonomous driving emerges from unusual combinations of common traffic elements. Current controllable generative models provide incomplete or entangled guidance, preventing independent manipulation of scene structure, object identity, and ego actions needed for effective adversarial scenario synthesis.

Method: CompoSIA introduces noise-level identity injection for pose-agnostic identity generation from single reference images, and a hierarchical dual-branch action control mechanism to improve action controllability. This enables disentangled control over traffic factors for adversarial scenario synthesis.

Result: Superior controllable generation quality over state-of-the-art baselines: 17% improvement in FVD for identity editing, 30% reduction in rotation errors, 47% reduction in translation errors for action control. Downstream stress-testing shows substantial planner failures with 173% average increase in collision rate across editing modalities.

Conclusion: CompoSIA's compositional approach enables systematic synthesis of dangerous driving scenarios by combining safe elements into unsafe configurations, revealing critical planner vulnerabilities that entangled generators cannot expose, advancing safety testing for autonomous driving systems.
...
TL;DR: TRACE is a structure-aware diffusion model framework for localized character encoding that embeds data by exploiting stable character structures, achieving superior performance in document security applications.


<details>
  <summary>Details</summary>
Motivation: Existing methods for character encoding rely on edge features or pre-defined codebooks, which may not be robust against noise interference. Character structures offer inherent stability and unified representation across diverse characters, making them more resistant to noise.

Method: TRACE uses three key components: (1) adaptive diffusion initialization with specialized algorithms (MPE, TPE, MDM) to identify handle points, target points, and editing regions, (2) guided diffusion encoding for precise point movement, and (3) masked region replacement with a specialized loss function to minimize feature alterations.

Result: TRACE achieves more than 5 dB improvement in PSNR and 5% higher extraction accuracy following cross-media transmission compared to state-of-the-art methods, with broad generalizability across multiple languages and fonts.

Conclusion: TRACE demonstrates superior performance for localized character encoding, making it particularly suitable for practical document security applications due to its robustness and cross-language/font generalizability.
...
Method: Three-step protocol: 1) Select reference staining conditions, 2) Characterize test set staining properties, 3) Apply CPath models under simulated reference staining conditions. Created a reference staining library from PLISM dataset and tested 306 MSI classification models on SurGen colorectal cancer dataset.

Result: Classification performance ranged from AUC 0.769-0.911 across models and staining conditions. Robustness (min-max AUC range) ranged from 0.007-0.079, showing weak inverse correlation with classification performance (Pearson r=-0.22).
...
TL;DR: Video-based epileptic seizure forecasting using cross-species transfer learning from rodent videos to predict human seizures from short pre-ictal video segments.


<details>
  <summary>Details</summary>
Motivation: Existing seizure forecasting relies on invasive neural signals (EEG) requiring specialized equipment, while video-based approaches only focus on post-onset detection. There's a need for non-invasive, accessible seizure forecasting methods.

Method: Formulates video-based seizure forecasting task using 3-10 second pre-ictal video segments to predict seizures within 5 seconds. Uses cross-species transfer learning framework leveraging large-scale rodent video data for auxiliary pretraining to capture seizure-related behavioral dynamics that generalize across species.

Result: Achieves over 70% prediction accuracy in strictly video-only setting and outperforms existing baselines.

Conclusion: Cross-species learning shows potential for building non-invasive, scalable early-warning systems for epilepsy using accessible video data.
...
TL;DR: A framework for benchmarking, diagnosing, and injecting camera motion awareness into VideoLLMs using synthetic datasets, geometric cue extraction from 3D foundation models, and structured prompting.
...
Method: Three-part framework: 1) Created CameraMotionDataset with explicit camera control and CameraMotionVQA benchmark; 2) Diagnosed weak camera motion representation in VideoLLMs through probing experiments; 3) Proposed lightweight pipeline that extracts geometric camera cues from 3D foundation models, predicts motion primitives with temporal classifier, and injects them via structured prompting.

Result: Substantial errors in camera motion recognition across off-the-shelf VideoLLMs; probing revealed weak camera motion representation especially in deeper ViT blocks; injection method improved motion recognition and produced more camera-aware model responses.

Conclusion: Geometry-driven cue extraction and structured prompting provide practical steps toward camera-aware VideoLLMs, bridging the gap without costly training or fine-tuning.
...
### [94] [Spectral-Geometric Neural Fields for Pose-Free LiDAR View Synthesis](https://arxiv.org/abs/2603.12903)
*Yinuo Jiang,Jun Cheng,Yiran Wang,Cheng Cheng*

Main category: cs.CV

TL;DR: SG-NLF is a pose-free LiDAR NeRF framework that uses spectral priors and geometric consistency to address sparsity and textureless challenges in LiDAR novel view synthesis.


<details>
  <summary>Details</summary>
Motivation: Most LiDAR NeRF methods rely on accurate camera poses, and the sparsity/textureless nature of LiDAR data causes geometric holes and discontinuous surfaces, requiring a pose-free solution that handles these challenges.

Method: Hybrid representation based on spectral priors for smooth geometry; confidence-aware graph for global pose alignment; adversarial learning for cross-frame consistency enhancement.

Result: Improves reconstruction quality by over 35.8% and pose accuracy by 68.8% compared to previous state-of-the-art methods, especially effective in low-frequency scenarios.
...
### [95] [Visual-ERM: Reward Modeling for Visual Equivalence](https://arxiv.org/abs/2603.13224)
*Ziyu Liu,Shengyuan Ding,Xinyu Fang,Xuanlang Dai,Penghui Yang,Jianze Liang,Jiaqi Wang,Kai Chen,Dahua Lin,Yuhang Zang*

Main category: cs.CV

TL;DR: Visual-ERM is a multimodal generative reward model that provides fine-grained visual feedback for vision-to-code tasks, improving RL performance by addressing reward hacking and visual discrepancy issues.


<details>
  <summary>Details</summary>
Motivation: Existing rewards for vision-to-code RL are inadequate - textual rules and coarse visual embeddings fail to capture fine-grained visual discrepancies and are vulnerable to reward hacking, limiting RL effectiveness.

Method: Proposes Visual-ERM, a multimodal generative reward model that evaluates vision-to-code quality directly in rendered visual space, providing fine-grained, interpretable, task-agnostic feedback for RL training.

Result: Visual-ERM improves Qwen3-VL-8B-Instruct by +8.4 on chart-to-code and yields consistent gains on table and SVG parsing (+2.7, +4.1 average). It outperforms Qwen3-VL-235B-Instruct on VC-RewardBench and approaches closed-source models.

Conclusion: Fine-grained visual reward supervision is both necessary and sufficient for vision-to-code RL, regardless of task specificity, and Visual-ERM provides an effective solution for this challenge.
...
TL;DR: VIRD: A cross-view pose estimation method using dual-axis transformation to bridge ground-satellite viewpoint gap for accurate camera localization without orientation priors.


<details>
  <summary>Details</summary>
Motivation: GNSS-based localization degrades due to occlusion and multipath effects. Cross-view pose estimation offers an alternative but existing methods struggle with significant viewpoint gaps between ground and satellite views due to limited spatial correspondences.

Method: Proposes VIRD (View-Invariant Representations through Dual-axis transformation): 1) Applies polar transformation to satellite view to establish horizontal correspondence, 2) Uses context-enhanced positional attention on ground and polar-transformed satellite features to resolve vertical misalignment, 3) Introduces view-reconstruction loss to strengthen view invariance by encouraging representations to reconstruct original and cross-view images.
...
Conclusion: VIRD effectively bridges the ground-satellite viewpoint gap through dual-axis transformation and view-invariant representation learning, achieving significant improvements in cross-view pose estimation accuracy for autonomous driving and robotics applications.
...
TL;DR: A new IFDL-VLM pipeline leverages vision-language models for image forgery detection and localization by addressing VLM biases and using location masks as priors, achieving SOTA performance across 9 benchmarks.


<details>
  <summary>Details</summary>
Motivation: With the rise of AIGC making image manipulation more accessible, there's a growing need for effective image forgery detection and localization. Vision-language models offer potential but have inherent biases toward semantic plausibility rather than authenticity, limiting their effectiveness for IFDL tasks.

Method: Proposes IFDL-VLM pipeline that addresses VLM biases by using location masks as explicit priors. These masks encode forgery concepts to ease VLM training optimization and enhance interpretability of detection/localization results.

Result: Achieves state-of-the-art performance on 9 popular benchmarks in both in-domain and cross-dataset generalization settings. Consistently outperforms existing methods in detection, localization, and interpretability.

Conclusion: The IFDL-VLM pipeline successfully leverages vision-language models for image forgery detection by overcoming their inherent biases through location mask priors, demonstrating superior performance and interpretability across diverse benchmarks.
...
TL;DR: SGMatch: A learning-based framework for semantic-guided non-rigid 3D shape matching that integrates semantic features from vision foundation models with geometric descriptors and uses conditional flow matching for spatial regularization.
...
Conclusion: SGMatch effectively addresses limitations of existing functional map approaches by incorporating semantic guidance and spatial regularization, demonstrating improved robustness for non-rigid shape matching under challenging conditions.
...
TL;DR: CleanSight is a training-free defense against backdoor attacks in large vision-language models that detects poisoned inputs via attention stealing patterns and purifies them by pruning suspicious visual tokens.


<details>
  <summary>Details</summary>
Motivation: Existing defenses against backdoor attacks in LVLMs require computationally expensive retraining with clean data, which often degrades model performance. The authors discovered that backdoor triggers work through attention stealing rather than low-level visual patterns, suggesting a more efficient defense approach.

Method: CleanSight operates at test time without training: (1) detects poisoned inputs by analyzing relative visual-text attention ratios in cross-modal fusion layers, and (2) purifies inputs by selectively pruning suspicious high-attention visual tokens to neutralize backdoor activation.
...
Conclusion: The attention stealing mechanism provides a new understanding of backdoor behaviors in LVLMs, enabling effective training-free defense through attention-based detection and token pruning, offering computational efficiency and performance preservation.
...
TL;DR: A training-free debiasing method for Vision-Language Models that provides closed-form solutions with bounded utility losses, achieving Pareto-optimal fairness across both visual and textual modalities without requiring annotated data.


<details>
  <summary>Details</summary>
Motivation: VLMs inherit social biases from training data and propagate them to downstream applications. Existing debiasing methods lack theoretical guarantees for preserving model utility while improving fairness.

Method: Proposes a training-free debiasing approach with closed-form solutions in cross-modal space. The method jointly debiases both visual and textual modalities without requiring annotated data, achieving Pareto-optimal fairness with bounded utility losses.

Result: Outperforms existing methods across diverse fairness metrics and datasets for both group and intersectional fairness in downstream tasks (zero-shot image classification, text-to-image retrieval, text-to-image generation) while preserving task performance.

Conclusion: The method provides an effective, theoretically-grounded approach to debias VLMs with guaranteed utility preservation, addressing both group and intersectional fairness across multiple modalities and downstream applications.
...
TL;DR: Multimodal OCR (MOCR) jointly parses text and graphics into unified textual representations, treating visual elements as first-class parsing targets for better document reconstruction and multimodal supervision.


<details>
  <summary>Details</summary>
Motivation: Conventional OCR systems focus only on text recognition and leave graphical regions as cropped pixels, losing semantic relationships between textual and visual components in documents.

Method: Developed dots.mocr method that treats charts, diagrams, tables, and icons as first-class parsing targets. Built comprehensive data engine from PDFs, webpages, and SVG assets, and trained a compact 3B-parameter model through staged pretraining and supervised fine-tuning.

Result: On document parsing benchmarks: ranks second only to Gemini 3 Pro on OCR Arena Elo, surpasses existing open-source systems, sets new SOTA of 83.9 on olmOCR Bench. On structured graphics parsing: achieves higher reconstruction quality than Gemini 3 Pro across image-to-SVG benchmarks for charts, UI layouts, scientific figures, and chemical diagrams.

Conclusion: MOCR paradigm enables scalable building of large-scale image-to-code corpora for multimodal pretraining, demonstrating practical joint parsing of text and graphics while preserving semantic relationships across document elements.
...
<details>
  <summary>Details</summary>
Motivation: Topological correctness is critical for tubular structures but existing methods require domain-specific ground truth which is costly and doesn't transfer across domains. Need to detect topological anomalies without supervision in new domains.

Method: Develop automated data-curation pipeline to synthesize topological anomalies with verifiable annotations. Introduce Topo-R1 framework with two-stage training: supervised fine-tuning followed by reinforcement learning with Group Relative Policy Optimization (GRPO) and topology-aware composite reward.
...
Conclusion: The approach enables topology-aware perception in VLMs for detecting topological anomalies without ground-truth supervision, addressing a critical gap in quality assessment for tubular structures.
...
TL;DR: Proposes VTON-IQA, a reference-free image quality assessment framework for virtual try-on systems, along with VTON-QBench, the largest human-annotated benchmark for VTON evaluation.


<details>
  <summary>Details</summary>
Motivation: Current VTON evaluation faces two major challenges: 1) ground-truth images are typically unavailable in real-world scenarios, making reference-based evaluation impractical, and 2) distribution-level metrics like FID and KID fail to reflect perceptual quality of individual generated images.

Method: 1) Constructs VTON-QBench - a large-scale human-annotated benchmark with 62,688 try-on images from 14 VTON models and 431,800 quality annotations. 2) Proposes VTON-IQA framework with Interleaved Cross-Attention module that extends transformer blocks by inserting cross-attention between self-attention and MLP to model garment-person interactions.

Result: VTON-IQA achieves reliable human-aligned image-level quality prediction. The paper also provides comprehensive benchmark evaluation of 14 representative VTON models using the proposed framework.

Conclusion: VTON-IQA addresses critical evaluation challenges in virtual try-on systems by providing a reference-free, human-aligned quality assessment framework that can evaluate both garment fidelity and preservation of person-specific details without requiring ground-truth images.
...
TL;DR: Two methods to address memorization in text-to-image diffusion models: RAPTA reduces overfitting through region-aware prompt augmentation during training, and ADMCD detects copying using multimodal fusion without large datasets.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models can memorize and reproduce training images, creating copyright and privacy risks. Existing inference-time prompt perturbations reduce copying but often harm image quality and prompt alignment.

Method: 1. Region-Aware Prompt Augmentation (RAPTA): Uses object detector to find salient regions, creates semantically grounded prompt variants, randomly samples them during training to increase diversity while maintaining alignment. 2. Attention-Driven Multimodal Copy Detection (ADMCD): Aggregates local patch, global semantic, and texture cues with lightweight transformer, applies thresholded decision rules to detect copying without needing large annotated datasets.
...
Conclusion: The proposed complementary methods effectively address memorization issues in text-to-image diffusion models - RAPTA prevents overfitting during training, while ADMCD detects copying at inference time, together mitigating copyright and privacy risks.
...
Method: Developed a learning-from-observation framework using wind-tunnel PIV data, comparing Kriging interpolation with three deep learning models (UNet, ViTAE, CWGAN), evaluating single vs. mixed wind-direction training, testing sensor densities (5-30), assessing robustness to sensor perturbations (±1 grid), and optimizing sensor placement via POD with QR decomposition.

Result: Deep learning models improved SSIM by up to 32.7%, FAC2 by 24.2%, and NMSE by 27.8% compared to Kriging. Mixed wind-direction training further improved performance with gains up to 173.7% in SSIM, 16.7% in FAC2, and 98.3% in MG. QR-based optimization improved robustness by up to 27.8% under sensor perturbations.

Conclusion: Deep learning effectively reconstructs rooftop wind fields from sparse sensors, with mixed wind-direction training and optimized sensor placement providing substantial benefits. Sensor configuration, optimization, and training strategy should be considered jointly for reliable deployment, and experimental data provides practical guidance for real-world applications.
...
TL;DR: Video generative models can be adapted for few-shot image restoration with minimal training data, achieving competitive performance across multiple tasks with a single model.


<details>
  <summary>Details</summary>
Motivation: Large-scale video generative models have rich visual priors but their potential for general-purpose visual learning remains untapped. The authors aim to bridge this latent capacity to few-shot image restoration tasks.

Method: V-Bridge framework reinterprets image restoration as a progressive generative process rather than static regression. It leverages pretrained video models to simulate gradual refinement from degraded inputs to high-fidelity outputs, requiring only 1,000 multi-task training samples.

Result: With less than 2% of training data compared to existing methods, pretrained video models achieve competitive image restoration performance across multiple tasks with a single model, rivaling specialized architectures.
...
TL;DR: VAEX-BENCH is a new benchmark for evaluating abstractive spatiotemporal reasoning in multimodal LLMs, focusing on integrating observations over time and inferring implicit structure rather than just extracting explicit information from videos.


<details>
  <summary>Details</summary>
Motivation: Existing video understanding benchmarks focus on extractive reasoning where answers are explicitly present in videos, but there's a need to evaluate abstractive spatiotemporal reasoning that requires integrating dispersed cues over time and inferring implicit spatial and contextual structure.
...
Result: Extensive experiments with state-of-the-art MLLMs reveal their limitations on abstractive tasks, exposing underlying bottlenecks in spatiotemporal reasoning capabilities when compared to extractive performance.

Conclusion: The paper introduces a comprehensive benchmark for abstractive spatiotemporal reasoning, highlighting current MLLM limitations and providing a framework for future research in embodied agent video understanding.
...
TL;DR: The paper addresses manufacturability prediction challenges in CAD designs, proposes a taxonomy for manufacturability metrics, and introduces BenDFM - a synthetic dataset for sheet metal bending with 20,000 parts and multiple manufacturability labels.


<details>
  <summary>Details</summary>
Motivation: Current learning-based approaches for predicting manufacturability within specific processes are limited due to inconsistent definitions of manufacturability across prior work and scarcity of suitable datasets. Existing datasets either contain only manufacturable parts or focus on simple geometries, lacking comprehensive coverage for systematic study.

Method: Proposes a taxonomy of manufacturability metrics along axes of configuration dependence and measurement type. Introduces BenDFM dataset with 20,000 parts (manufacturable and unmanufacturable) generated using process-aware bending simulations, providing both folded/unfolded geometries and multiple manufacturability labels. Benchmarks two state-of-the-art 3D learning architectures on the dataset.

Result: Graph-based representations that capture relationships between part surfaces achieve better accuracy for manufacturability prediction. Predicting metrics that depend on specific manufacturing setups remains more challenging than more general manufacturability assessments.

Conclusion: The proposed taxonomy provides clearer scoping for manufacturability learning objectives, while BenDFM enables systematic study of learning-based DFM challenges. The work advances manufacturability prediction by addressing dataset scarcity and definition inconsistencies in the field.
...
TL;DR: NOIR reframes medical imaging tasks as operator learning between continuous function spaces using Implicit Neural Representations, enabling resolution-independent transformations instead of discrete grid-based processing.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of discrete grid-based deep learning in medical imaging, which struggles with resolution dependencies and fixed pixel/voxel representations, by establishing a continuous function space framework.

Method: Embeds discrete medical signals into shared Implicit Neural Representations, learns a Neural Operator that maps between latent modulations of these representations, enabling resolution-independent function-to-function transformations.

Result: Achieves competitive performance at native resolution across 2D/3D tasks (segmentation, shape completion, image translation, synthesis) on multiple datasets, demonstrates strong robustness to unseen discretizations, and satisfies key theoretical properties of neural operators.

Conclusion: NOIR successfully establishes a continuous function space paradigm for medical imaging that outperforms discrete grid-based approaches in resolution independence and robustness while maintaining competitive performance.
...
Conclusion: FDeID-Toolbox provides a comprehensive solution for reproducible face de-identification research, standardizing implementations and evaluations to advance the field through consistent benchmarking.
...
Method: Built on a vision-language backbone, f-CBM uses two complementary strategies: 1) a differentiable leakage loss to mitigate leakage, and 2) a Kolmogorov-Arnold Network prediction head to improve concept detection expressiveness.
...
Conclusion: f-CBM provides a versatile multimodal CBM framework that jointly addresses faithfulness requirements through complementary strategies for leakage mitigation and concept detection.
...
TL;DR: Proposes a lightweight perception scheduling framework for HRC that reduces computational latency by 27.52% while improving MMPose activation recall by 72.73% through efficient module scheduling based on scene context.


<details>
  <summary>Details</summary>
Motivation: Multiple perception modules in HRC create latency in streaming scenarios due to information redundancy and suboptimal computational resource allocation, despite recent advances in scene understanding efficiency.

Method: A novel lightweight perception scheduling framework that leverages previous frame outputs to estimate and schedule necessary perception modules in real-time based on scene context, inspired by Relevance concept and HRC information sparsity.

Result: Reduces computational latency by up to 27.52% compared to conventional parallel pipelines, achieves 72.73% improvement in MMPose activation recall, and maintains high keyframe accuracy up to 98%.

Conclusion: The framework enhances real-time perception efficiency without significantly compromising accuracy, showing potential as a scalable, systematic solution for multimodal streaming perception systems in HRC.
...
TL;DR: A robust brain tumor classification framework combining Non-Negative Matrix Factorization, lightweight CNNs, and diffusion-based feature purification to enhance adversarial robustness while maintaining competitive accuracy.
...
Result: The framework achieves competitive classification performance while significantly enhancing robustness against adversarial attacks (evaluated using AutoAttack), balancing clean accuracy and robust accuracy.

Conclusion: Combining interpretable NNMF-based representations with lightweight deep learning and diffusion-based defense provides an effective and reliable solution for medical image classification under adversarial conditions.
...
TL;DR: The paper introduces ActionGenome4D dataset for 4D scene understanding and formalizes World Scene Graph Generation (WSGG) task to model both observed and unobserved objects, proposing three methods with different inductive biases for reasoning about occluded objects.


<details>
  <summary>Details</summary>
Motivation: Existing spatio-temporal scene graph methods are frame-centric, only reasoning about visible objects and discarding entities upon occlusion, operating in 2D. The paper aims to address these limitations by creating world-centric, temporally persistent scene understanding.

Method: 1) Introduces ActionGenome4D dataset with 4D scenes via 3D reconstruction, world-frame bounding boxes, and dense relationship annotations including unobserved objects. 2) Formalizes World Scene Graph Generation (WSGG) task. 3) Proposes three methods: PWG (zero-order feature buffer for object permanence), MWAE (masked completion with cross-view associative retrieval), and 4DST (temporal attention with 3D motion and camera-pose features). 4) Evaluates Vision-Language Models via Graph RAG-based approaches.

Result: The paper establishes ActionGenome4D dataset and formalizes WSGG task. It proposes three complementary methods for reasoning about unobserved objects and establishes baselines for unlocalized relationship prediction using Vision-Language Models with Graph RAG approaches.

Conclusion: WSGG advances video scene understanding toward world-centric, temporally persistent, and interpretable scene reasoning by addressing limitations of frame-centric approaches and enabling reasoning about both observed and unobserved objects in 4D scenes.
...
TL;DR: STEVO-Bench is a benchmark for evaluating whether video world models can decouple state evolution from observation by testing them with occluders, darkness, and camera lookaway trajectories.


<details>
  <summary>Details</summary>
Motivation: Real-world evolutions happen regardless of observation, but video world models generate worlds via 2D frame observations. The paper aims to test if these models can properly decouple state evolution from observation.

Method: The benchmark applies observation control to evolving processes through instructions like occluder insertion, turning off lights, or specifying camera "lookaway" trajectories. It evaluates video models with and without camera control across diverse natural evolutions.

Result: The benchmark exposes limitations of video world models in decoupling state evolution from observation and provides an automatic evaluation protocol to detect and disentangle failure modes across key aspects of natural state evolution.

Conclusion: Analysis of STEVO-Bench results provides new insights into potential data and architecture biases of present-day video world models, revealing their inability to properly separate state evolution from observation.
...
### [116] [End-to-End Deep Learning in Wireless Communication Systems: A Tutorial Review](https://arxiv.org/abs/2603.12289)
*Abdelrahman Elfiky,Zouheir Rezki,Jorge Cortez,Youssef Boumhaout,Anne Xia,Abdulkadir Celik,Georges Kaddoum*

Main category: cs.IT

TL;DR: Survey paper reviewing deep learning applications for end-to-end physical layer optimization in wireless systems using autoencoder frameworks.


<details>
  <summary>Details</summary>
Motivation: Traditional model-based PHY methods face limitations with real-world nonlinearities, hardware imperfections, and modern network complexity, requiring more adaptive approaches.

Method: Uses autoencoder (AE) model as end-to-end deep learning framework for joint transmitter/receiver optimization, surveying various DL models and their applications.

Result: Comprehensive review of DL applications in PHY tasks (modulation, error correction, channel estimation) across various scenarios (point-to-point, multiple access, interference channels).

Conclusion: Learning-based approaches offer advantages over traditional methods, with future directions needed to bridge theory-practice gaps for next-generation wireless systems.
...
TL;DR: This paper introduces a framework for complex-valued probability measures with phase modulation, defining three new information-theoretic quantities (complex entropy, divergence, and metric) and demonstrating applications in statistical analysis and hypothesis testing.


<details>
  <summary>Details</summary>
Motivation: To extend classical probability theory by incorporating complex-valued measures with phase modulation, enabling richer analysis of probability distributions through complex analysis and interference phenomena, with potential applications across information theory, statistics, and machine learning.

Method: Defines complex probability measures as phase-modulated extensions of classical measures. Proposes three core information-theoretic quantities: complex entropy (measuring distribution uniformity via phase coherence), complex divergence (asymmetric dissimilarity measure), and complex metric (symmetric distance satisfying triangle inequality). Establishes rigorous mathematical foundations for both continuous and discrete distributions, proving properties like boundedness, continuity, and extremal behaviors.

Result: Develops a comprehensive framework for complex-valued probability measures with proven mathematical properties. Shows unique geometric and interpretive advantages over classical measures (Shannon entropy, KL divergence), particularly through sensitivity to distributional shape via tunable phase parameter. Identifies formal analogy between complex entropy integral and Feynman's path integral. Demonstrates practical utility in nonparametric two-sample hypothesis testing with conceptual simulation.

Conclusion: The work opens new avenues for analyzing probability distributions through complex analysis and interference phenomena, establishing a conceptual bridge between information theory and quantum mechanics, with significant potential impacts across multiple fields including statistical inference and machine learning.
...
TL;DR: PE repair scheme reduces sub-packetization for MDS codes achieving cut-set bound by allowing flexible exclusion sets, outperforming conventional repair schemes.


<details>
  <summary>Details</summary>
Motivation: Conventional MDS code repair schemes require super-exponential sub-packetization levels, which severely limits practical deployment. Need to address this challenge.

Method: Introduce partial-exclusion (PE) repair scheme with flexible exclusion sets for each node. Propose two generic constructions of Reed-Solomon codes that achieve cut-set bound under PE framework.

Result: For sufficiently large flexibility, PE repair achieves lower sub-packetization than known lower bounds for conventional schemes. Implemented as executable Magma programs showing practical efficiency.

Conclusion: PE repair framework enables MDS codes with reduced sub-packetization while maintaining optimal repair bandwidth, outperforming all existing conventional repair schemes.
...
### [119] [Boosting Spectral Efficiency via Spatial Path Index Modulation in RIS-Aided mMIMO](https://arxiv.org/abs/2603.12619)
*Ahmet M. Elbir,Abdulkadir Celik,Asmaa Abdallah,Ahmed M. Eltawil*

Main category: cs.IT

TL;DR: SPIM (Spatial Path Index Modulation) for RIS-aided mMIMO systems improves spectral efficiency by encoding extra bits in spatial path indices, using hybrid beamforming with few RF chains.


<details>
  <summary>Details</summary>
Motivation: Next-gen wireless needs better spectral efficiency with lower power/cost. RIS helps, and index modulation (IM) boosts SE by encoding bits in transmission media. Spatial path IM (SPIM) specifically exploits spatial diversity in RIS-mMIMO systems.

Method: Propose SPIM for RIS-aided mMIMO: assign index bits to spatial patterns of mmWave channel via RIS. Design low-complexity hybrid beamformers using steering vectors of selected spatial paths. Theoretically analyze SE and derive relationship between SPIM-hybrid and fully digital beamforming.

Result: SPIM improves SE performance, even surpassing fully digital beamformers while using fewer RF chains. Theoretical analysis validated via numerical simulations.

Conclusion: SPIM effectively enhances spectral efficiency in RIS-aided mMIMO systems by exploiting spatial path diversity with low-complexity hybrid beamforming, achieving better performance than fully digital approaches with reduced hardware complexity.
...
### [120] [Research on Linear Codes Holding $q$-Ary $t$-Designs](https://arxiv.org/abs/2603.12761)
*Xinghao Wu,Junling Zhou*

Main category: cs.IT

TL;DR: The paper develops criteria for determining when codewords of fixed weight in linear codes form q-ary t-designs, and applies these to construct q-ary 2-designs from various code families including one/two-weight codes, self-dual codes, and Reed-Solomon codes.


<details>
  <summary>Details</summary>
Motivation: While linear codes supporting ordinary combinatorial designs have been well-studied, the case where codes hold q-ary designs remains largely unexplored. This motivates systematic investigation into whether codewords of fixed weight in linear codes can form q-ary t-designs.

Method: The authors develop two new criteria for determining when codewords of fixed weight form q-ary t-designs. For codes not satisfying these criteria, they provide an alternative approach based on the automorphism group of the code, enabling construction from doubly-extended Reed-Solomon codes.

Result: Several families of linear codes are shown to hold q-ary 2-designs, including one- and two-weight codes, extremal self-dual codes, and certain dual codes, shortened codes, and punctured codes derived from them. For a class previously known to support 4-designs, codewords of certain weights give rise to q-ary 2-designs with precisely determined parameters.

Conclusion: The paper successfully develops systematic methods for constructing q-ary t-designs from linear codes, expanding the known families of codes that support such designs and providing tools for further exploration in this understudied area.
...
### [121] [Upward Spatial Coverage Recovery via Movable Antenna in Low-Altitude Communications](https://arxiv.org/abs/2603.12792)
*Kan Yu,Kaixuan Li,Yujia Zhao,Dingyou Ma,Qixun Zhang,Zhiyong Feng*

Main category: cs.IT

TL;DR: MA technology enables dynamic antenna positioning to optimize 3D airspace coverage for UAV communications, outperforming conventional fixed antennas.


<details>
  <summary>Details</summary>
Motivation: Conventional cellular systems with fixed-position antennas cannot adapt to the highly dynamic 3D propagation environments needed for reliable UAV communications in low-altitude airspace.

Method: Formulates spatial coverage maximization over discretized 3D voxel space, optimizing MA's 3D positions and beamforming matrices for each voxel, using hybrid particle swarm optimization and simulated annealing framework.

Result: MA design achieves 26.8% coverage below 300m and 29.65% below 600m, substantially outperforming FPA-based schemes, with further enhancement possible through mechanical tilt adjustment.

Conclusion: MA technology shows strong potential for reliable low-altitude communication coverage by enabling flexible spatial adaptation through controllable antenna positioning.
...
### [122] [Rethinking Mutual Coupling in Movable Antenna MIMO Systems](https://arxiv.org/abs/2603.12817)
*Tianyi Liao,Wei Guo,Jun Qian,Shenghui Song,Jun Zhang,Khaled B. Letaief*

Main category: cs.IT

TL;DR: MA systems exploit mutual coupling effects to enhance MIMO capacity through antenna movement optimization.


<details>
  <summary>Details</summary>
Motivation: Mutual coupling effects in movable antenna systems have been previously ignored but can be exploited to enhance MIMO system capacity.

Method: Model MA-enabled MIMO system with mutual coupling using circuit-theoretic framework, formulate capacity maximization as non-concave optimization, solve via BCA algorithm with TRM-based subproblem for MA position optimization using Sylvester equations.

Result: Simulation results show significant capacity gains from leveraging mutual coupling effects, primarily due to customizable MC matrices and superdirectivity.

Conclusion: Mutual coupling effects in movable antenna systems can be effectively exploited to significantly enhance MIMO capacity through proper optimization of antenna positions.
...
### [123] [Reweighted information inequalities](https://arxiv.org/abs/2603.13135)
*Jonathan Niles-Weed*

Main category: cs.IT

TL;DR: Fisher information bounds for mixture distributions provide control over relative entropy and transport distance to reweighted mixtures.


<details>
  <summary>Details</summary>
Motivation: To understand Fisher information bounds for non-log-concave measures and explain phenomena observed in Langevin Monte Carlo for multimodal distributions.

Method: Establish variants of log-Sobolev and transport-information inequalities for mixture distributions by decomposing probability measures into components that individually satisfy such inequalities.

Result: If a measure μ is close to π in relative Fisher information, then μ is close in relative entropy or transport distance to a reweighted version of π with the same mixture components but possibly different weights.

Conclusion: Provides user-friendly interpretation of Fisher information bounds for non-log-concave measures and explains Langevin Monte Carlo behavior for multimodal distributions.
...
### [124] [Distribution-Aware GMD Transceiver Design for Probabilistic Shaping in MIMO](https://arxiv.org/abs/2603.12447)
*Tzu-Hsuan Chou,Chih-Hao Liu,Jing Jiang*

Main category: eess.SP

TL;DR: Proposes a distribution-aware MIMO transceiver with Bayesian GMD precoder and MAP-VBLAST detector for probabilistic shaping, plus layer-contained MIMO mapping for error propagation mitigation.


<details>
  <summary>Details</summary>
Motivation: To improve spectral efficiency in 6G networks by combining MIMO transceiver design with probabilistic shaping, addressing the need for distribution-aware processing that incorporates PS priors into both precoding and detection.

Method: 1) Bayesian geometric-mean decomposition (BGMD) precoder that incorporates PS priors and equalizes layer gains; 2) Maximum a posteriori-VBLAST (MAP-VBLAST) detector using PS priors within SIC framework; 3) Layer-contained MIMO (LC-MIMO) mapping that contains each codeblock within a single layer to enable SIC at CB level.

Result: Numerical results show the BGMD transceiver with LC-MIMO achieves notable performance gains over state-of-the-art methods, enabling single modulation and coding scheme for low-complexity transmissions while preserving channel capacity.

Conclusion: The proposed distribution-aware MIMO transceiver with BGMD precoder, MAP-VBLAST detector, and LC-MIMO mapping effectively combines MIMO and probabilistic shaping for high spectral efficiency in 6G networks, with significant performance improvements over existing approaches.
...
### [125] [Load Balancing in Non-Terrestrial Networks Using Free Space Optical Inter-satellite Links](https://arxiv.org/abs/2603.12546)
*Abid Afridi,Alexis A. Dowhuszko,Jevgenij Krivochiza,Risto Wichman,Jyri Hämäläinen*

Main category: eess.SP

TL;DR: A fairness-driven load balancing algorithm for NGSO satellite constellations that uses FSO inter-satellite links to redistribute traffic and improve worst-case downlink throughput by dynamically selecting ground stations and offloading data to adjacent satellites.


<details>
  <summary>Details</summary>
Motivation: Non-terrestrial networks with NGSO constellations face uneven satellite-gateway visibility, traffic congestion, and rain-induced feeder link attenuation, leading to non-uniform load distribution where some satellites are underutilized while others become bottlenecks.

Method: Treats the satellite constellation as an anycast multi-commodity flow problem and solves an equivalent linear programming optimization problem to dynamically select optimal ground stations for each satellite and offload traffic through FSO inter-satellite links when needed.

Result: The algorithm enhances minimum downlink data rate by more than 25% in rain conditions and over 10% under no-rain conditions, stabilizing reverse link service while maintaining average data rate and improving worst-case throughput.

Conclusion: ISL-assisted load-balancing mitigates feeder link bottlenecks and enhances fairness across satellite constellations, providing a scalable basis for resource allocation in future non-terrestrial network systems.
...
### [126] [Dual-Chirp AFDM for Joint Delay-Doppler Estimation with Rydberg Atomic Quantum Receivers](https://arxiv.org/abs/2603.12728)
*Hanvit Kim,Hyeonseok Rou,Kihong Min,Giuseppe Thadeu Freitas de Abreu,Sunwoo Kim*

Main category: eess.SP

TL;DR: Proposes dual-chirp AFDM framework for joint delay-Doppler estimation in Rydberg atomic quantum receivers, enabling hyper-ISAC for 6G+ networks.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of joint delay-Doppler parameter estimation in Rydberg atomic quantum receivers (RAQRs) for hyper integrated sensing and communication (ISAC) in 6G and beyond. Current approaches suffer from inherent time-frequency parameter coupling in optical readout, making joint estimation ambiguous.

Method: Proposes a dual-chirp affine frequency division multiplexing (AFDM) framework that uses distinct chirp parameters to convert the ambiguous estimation problem into a full-rank system, enabling unique delay-Doppler parameter extraction from RAQRs.

Result: Numerical simulations show superior delay-Doppler estimation performance compared to classical single-chirp AFDM over RAQRs, while preserving the extreme sensitivity of RAQRs.

Conclusion: The dual-chirp AFDM framework provides a pioneering solution for joint delay-Doppler estimation in RAQRs, enabling hyper-ISAC capabilities for future 6G+ networks by overcoming inherent time-frequency parameter coupling limitations.
...
### [127] [Unified framework for outage-constrained rate maximization in secure ISAC under various sensing metrics](https://arxiv.org/abs/2603.12798)
*Hancheng Zhu,Zongze Li,Yik-Chung Wu*

Main category: eess.SP

TL;DR: A unified optimization framework for secure ISAC systems that jointly addresses worst-case and sum secrecy rates while controlling outage probabilities under diverse sensing constraints.


<details>
  <summary>Details</summary>
Motivation: ISAC systems face security vulnerabilities to eavesdropping, especially with uncertain channel state information, while diverse sensing requirements complicate resource optimization, necessitating a flexible framework.

Method: Proposes a unified optimization framework integrating sensing constraints into objective via auxiliary variable, using alternating optimization with guaranteed convergence to stationary points.

Result: Framework achieves higher optimized secrecy rates under various sensing constraints compared to existing methods, demonstrating superiority and versatility.

Conclusion: The proposed unified framework effectively addresses security and sensing challenges in ISAC systems, offering flexible optimization with theoretical convergence guarantees.
...
### [128] [BER Analysis and Optimization of Pinching-Antenna-Based NOMA Communications](https://arxiv.org/abs/2603.12836)
*Mahmoud AlaaEldin,Amy S. Inwood,Xidong Mu,Michail Matthaiou*

Main category: eess.SP

TL;DR: First BER analysis of pinching-antenna NOMA systems with closed-form expressions and optimization of antenna placement for both uplink and downlink scenarios.


<details>
  <summary>Details</summary>
Motivation: To analyze and optimize the performance of pinching-antenna based NOMA systems, which haven't been studied before in terms of BER analysis, especially considering practical imperfect SIC conditions.

Method: Derived exact closed-form BER expressions for each user under imperfect SIC, then optimized PA location for uplink (using smoothing technique to handle non-convex BER function) and jointly optimized PA location with power allocation for downlink.

Result: Simulation results verified analytical derivations and optimization effectiveness. Optimally positioned PA can create required power difference between equally powered UEs for reliable NOMA decoding under imperfect SIC.

Conclusion: The paper provides first comprehensive BER analysis of PA-NOMA systems with practical optimization methods that enable reliable NOMA operation even with imperfect SIC through optimal antenna placement.
...
### [129] [AoI-FusionNet: Age-Aware Tightly Coupled Fusion of UWB-IMU under Sparse Ranging Conditions](https://arxiv.org/abs/2603.12849)
*Tehmina Bibi,Anselm Köhler,Jan-Thomas Fischer,Falko Dressler*

Main category: eess.SP

TL;DR: AoI-FusionNet is a deep learning framework that fuses raw UWB ToF measurements with IMU data for 3D trajectory estimation in GNSS-denied outdoor environments, outperforming traditional methods under intermittent sensing conditions.


<details>
  <summary>Details</summary>
Motivation: Accurate motion tracking in avalanche events requires robust localization in GNSS-denied outdoor environments where traditional methods fail due to insufficient ranging availability.

Method: Tightly coupled deep learning framework that directly combines raw UWB ToF measurements with IMU data, featuring AoI-aware decay module to reduce stale measurement influence and learned attention gating to balance sensor modalities based on availability and freshness.

Result: AoI-FusionNet substantially reduces mean and tail localization errors under intermittent and degraded sensing conditions compared to UWB multilateration and loose-coupled fusion baselines.

Conclusion: The proposed tightly coupled fusion approach with AoI-awareness and adaptive gating provides superior 3D trajectory estimation in challenging GNSS-denied outdoor environments with intermittent sensor availability.
...
### [130] [Explainable AI Using Inherently Interpretable Components for Wearable-based Health Monitoring](https://arxiv.org/abs/2603.12880)
*Maurice Kuschel,Solveig Vieluf,Claus Reinsberger,Tobias Loddenkemper,Tanuj Hasija*

Main category: eess.SP

TL;DR: Proposes a novel XAI method combining explanation spaces and concept-based explanations for wearable time-series data, using Inherently Interpretable Components to preserve model performance while achieving interpretability.


<details>
  <summary>Details</summary>
Motivation: Wearables with AI offer real-time health monitoring but require explainable AI (XAI) to build trust among stakeholders. Explaining time-series data from wearables is challenging due to complex temporal dependencies, and current interpretable feature methods often sacrifice performance.

Method: Uses Inherently Interpretable Components (IICs) that encapsulate domain-specific, interpretable concepts within a custom explanation space. Combines explanation spaces with concept-based explanations to explain AI predictions on time-series data while preserving model performance.

Result: Demonstrates usability in real applications including state assessment and epileptic seizure detection. The approach maintains model performance while achieving the interpretability of concept-based explanations based on extracted features.

Conclusion: Proposed method successfully addresses the trade-off between performance and interpretability in XAI for wearable time-series data, enabling trustworthy AI applications in healthcare monitoring with domain-specific interpretable components.
...
### [131] [Exploiting Near-Field Dynamics with Movable Antennas to Enhance Discrete Transmissive RIS](https://arxiv.org/abs/2603.12891)
*Marjan Boloori,Chu Li,Aydin Sezgin*

Main category: eess.SP

TL;DR: Proposes BS architecture integrating movable antenna with transmissive RIS in near-field, jointly optimizing MA position and quantized RIS phases via alternating optimization.


<details>
  <summary>Details</summary>
Motivation: Low complexity transceiver design is crucial for next-gen wireless systems. Movable antennas and transmissive RIS are promising technologies for enhancing wireless performance, but their joint optimization in near-field scenarios presents challenges.

Method: Proposes BS architecture with single MA integrated with TRIS operating in near-field. Develops alternating optimization framework: MA position updated via gradient ascent, TRIS phases optimized through quantized phase alignment to handle non-convex coupling and discrete phase constraints.

Result: Proposed architecture significantly outperforms conventional BS with fixed fully-active antenna array under same channel model and transmit power. MA repositioning effectively mitigates performance degradation from discrete TRIS phase quantization in near-field environments.

Conclusion: Integration of movable antennas with transmissive RIS in near-field operation provides significant performance gains over conventional architectures, with MA repositioning compensating for quantization effects in near-field propagation.
...
### [132] [Environment-aware Near-field UE Tracking under Partial Blockage and Reflection](https://arxiv.org/abs/2603.12896)
*Hyunwoo Park,Hyeon Seok Rou,Giuseppe Thadeu Freitas de Abreu,Sunwoo Kim*

Main category: eess.SP

TL;DR: Environment-aware near-field UE tracking method for large arrays that integrates surface geometry and LOS/NLOS indicators to handle NF-specific blockages and reflections, outperforming far-field baselines.


<details>
  <summary>Details</summary>
Motivation: Conventional far-field assumptions fail to capture near-field spherical-wavefront effects like partial blockages and reflections, limiting tracking accuracy in extremely large aperture array systems.

Method: Integrates known surface geometries and tracks LOS/NLOS indicators per antenna element to model NF-specific effects, then tracks UE positions by maximizing cosine similarity between predicted and received channels.

Result: Increasing environment-awareness improves accuracy; NF consistently outperforms FF baselines, achieving 0.22m root-mean-square error with full environment-awareness.

Conclusion: Environment-aware near-field tracking is essential for accurate UE positioning in large array systems, enabling robust tracking even under complete LOS obstruction.
...
### [133] [Joint and Streamwise Distributed MIMO Satellite Communications with Multi-Antenna Ground Users](https://arxiv.org/abs/2603.12914)
*Parisa Ramezani,Emil Björnson*

Main category: eess.SP

TL;DR: The paper proposes two distributed transmission modes for multi-satellite downlink systems: joint transmission (all satellites transmit all streams) and streamwise transmission (each stream sent by single satellite) with reduced fronthaul, using statistical CSI and novel transceiver designs.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of multi-satellite downlink communications in LEO systems where multiple satellites jointly serve multi-antenna ground users, with focus on balancing spectral efficiency and fronthaul requirements.

Method: 1) Joint transmission: formulate sum SE maximization under convex power constraints, use tractable SE approximation, derive iterative transceiver design via weighted sum MSE minimization equivalence. 2) Streamwise transmission: propose eigenmode-based stream-satellite association using participation factors and maximum-weight bipartite matching solved by Hungarian algorithm.

Result: Numerical simulations validate SE approximation accuracy, show streamwise transmission achieves near-optimal performance in certain conditions or trades SE for lower overhead, demonstrate impact of stream/user loading, and show substantial gains over conventional benchmarks.

Conclusion: The paper provides two practical transmission strategies for multi-satellite systems: joint transmission for maximum SE and streamwise transmission for reduced fronthaul, with streamwise offering good performance-overhead tradeoffs and significant improvements over existing approaches.
...
### [134] [Identification and Visualization of Correlation Structures in Large-Scale Power Quality Data](https://arxiv.org/abs/2603.12948)
*Max Domagk,Jan Meyer,Marco Lindner*

Main category: eess.SP

TL;DR: Automated methodology for analyzing and visualizing correlation structures in large power quality datasets using aggregation and distance-based visualization techniques.


<details>
  <summary>Details</summary>
Motivation: Large-scale power quality measurement campaigns generate vast multivariate data where systematic dependencies are difficult to identify using conventional analysis techniques.

Method: Adapts existing framework for shorter observation periods, enhances with aggregation and distance-based visualization. Daily Spearman correlation coefficients are averaged via Fisher's z-transformation and aggregated across phases, parameters, and sites. Correlation structures are visualized using hierarchical clustering and multidimensional scaling.

Result: Methodology successfully demonstrated using data from 85 measurement sites within the German transmission system.

Conclusion: The approach enables automated analysis and visualization of correlation structures in large PQ datasets, revealing consistent and recurring relationships that are difficult to identify with conventional techniques.
...
### [135] [Association-Aware GNN for Precoder Learning in Cell-Free Systems](https://arxiv.org/abs/2603.13035)
*Mingyu Deng,Shengqian Han*

Main category: eess.SP

TL;DR: AAGNN: Association-Aware Graph Neural Network for cell-free MIMO precoding that incorporates UE-AP association status, leveraging permutation equivariance and attention for improved performance and generalization.


<details>
  <summary>Details</summary>
Motivation: Cell-free systems differ from cellular systems in flexible UE-AP associations, making optimal precoding dependent on both channel state information and dynamic association status, requiring new learning approaches.

Method: Propose Association-Aware Graph Neural Network (AAGNN) that explicitly incorporates UE-AP association status into precoding design, leveraging permutation equivariance properties to reduce training complexity and using attention mechanism for better generalization.

Result: Simulation results show AAGNN outperforms baseline learning methods in both learning performance and generalization capabilities while maintaining low training and inference complexity.

Conclusion: The proposed AAGNN effectively addresses the unique challenges of cell-free precoding by incorporating association awareness, demonstrating superior performance and generalization with manageable complexity.
...
### [136] [AirGuard: UAV and Bird Recognition Scheme for Integrated Sensing and Communications System](https://arxiv.org/abs/2603.13112)
*Hongliang Luo,Zhonghua Chu,Tengyu Zhang,Chuanbin Zhao,Bo Lin,Feifei Gao*

Main category: eess.SP

TL;DR: AirGuard: A UAV-bird recognition scheme using dual feature fusion of micro-Doppler spectrum and HRRP images with CNN for ISAC systems.


<details>
  <summary>Details</summary>
Motivation: Need for effective low-altitude target monitoring to distinguish between UAVs and birds in integrated sensing and communications systems, as both share similar flight characteristics but require different responses.

Method: Extract centralized micro-Doppler (cmD) spectrum and high resolution range profile (HRRP) from echo signals, then design a CNN-based dual feature fusion network that takes both cmD and HRRP images as inputs for joint classification.

Result: Generated 237,600 cmD and HRRP image samples for training/validation/evaluation. Simulation results demonstrate the effectiveness of the proposed AirGuard scheme.

Conclusion: AirGuard successfully integrates signal processing and deep learning for UAV-bird recognition in ISAC systems, providing a practical solution for low-altitude target monitoring with dual feature fusion approach.
...
### [137] [No More DeLuLu: Physics-Inspired Kernel Networks for Geometrically-Grounded Neural Computation](https://arxiv.org/abs/2603.12276)
*Taha Bouhsine*

Main category: cs.LG

TL;DR: The paper introduces yat-product, a novel kernel operator combining quadratic alignment with inverse-square proximity, and uses it as the sole non-linearity in Neural Matter Networks (NMNs), simplifying neural architectures while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: To create a principled alternative to conventional neural architectures by unifying kernel learning, gradient stability, and information geometry through a geometrically-grounded kernel operation that can replace complex linear-activation-normalization blocks.

Method: Introduce yat-product kernel operator with proven Mercer kernel properties; build Neural Matter Networks (NMNs) using yat-product as the only non-linearity, replacing traditional blocks with this single operation; apply to both vision (MNs) and language (Aether-GPT2) tasks.

Result: NMN classifiers match linear baselines on MNIST with bounded prototype evolution and superposition robustness; Aether-GPT2 achieves lower validation loss than GPT-2 with comparable parameters using yat-based attention and MLP blocks.

Conclusion: The yat-product framework successfully unifies kernel learning, gradient stability, and information geometry, establishing NMNs as a viable, principled alternative to conventional neural architectures with architectural simplification and competitive performance.
...
TL;DR: The paper explains why modern tabular ML models succeed with messy, high-dimensional data by showing that data architecture synergy with model capacity overcomes noise better than traditional data cleaning approaches.


<details>
  <summary>Details</summary>
Motivation: To resolve the paradox that modern tabular ML achieves state-of-the-art performance using high-dimensional, collinear, error-prone data despite the "Garbage In, Garbage Out" principle, by developing a theoretical framework explaining this phenomenon.

Method: Synthesizes principles from Information Theory, Latent Factor Models, and Psychometrics; partitions predictor-space noise into "Predictor Error" and "Structural Uncertainty"; proves asymptotic properties of high-dimensional error-prone predictors; analyzes "Informative Collinearity" effects; proposes "Proactive Data-Centric AI" for efficient predictor selection.

Result: Shows that leveraging high-dimensional sets of error-prone predictors asymptotically overcomes both types of noise, while cleaning low-dimensional data is fundamentally bounded by Structural Uncertainty; demonstrates how informative collinearity enhances reliability and convergence efficiency; explains why increased dimensionality reduces latent inference burden.

Conclusion: Redefines data quality from item-level perfection to portfolio-level architecture, providing theoretical rationale for "Local Factories" that learn from uncurated enterprise data, supporting a paradigm shift from "Model Transfer" to "Methodology Transfer" to overcome static generalizability limitations.
...
### [139] [Multi-objective Genetic Programming with Multi-view Multi-level Feature for Enhanced Protein Secondary Structure Prediction](https://arxiv.org/abs/2603.12293)
*Yining Qian,Lijie Su,Meiling Xu,Xianpeng Wang*

Main category: cs.LG

TL;DR: MOGP-MMF is a multi-objective genetic programming framework for protein secondary structure prediction that uses multi-view multi-level feature representation and automated feature selection/fusion optimization.


<details>
  <summary>Details</summary>
Motivation: Protein secondary structure prediction is crucial for understanding protein function and drug discovery, but the complex sequence-structure relationship makes accurate modeling challenging. Existing methods struggle with feature selection and fusion optimization.

Method: Proposes MOGP-MMF framework with: 1) Multi-view multi-level representation integrating evolutionary, semantic, and structural views; 2) Enriched operator set for evolving linear/nonlinear fusion functions; 3) Improved multi-objective GP algorithm with knowledge transfer mechanism using prior evolutionary experience.

Result: Extensive experiments on seven benchmark datasets show MOGP-MMF outperforms state-of-the-art methods, especially in Q8 accuracy and structural integrity. The framework generates diverse non-dominated solutions for flexible model selection.

Conclusion: MOGP-MMF effectively addresses the protein secondary structure prediction problem through automated feature optimization, achieving superior performance while providing practical flexibility for different application scenarios.
...
### [140] [Synthetic Data Generation for Brain-Computer Interfaces: Overview, Benchmarking, and Future Directions](https://arxiv.org/abs/2603.12296)
*Ziwei Wang,Zhentao He,Xingyi He,Hongbin Wang,Tianwang Jia,Jingwei Luo,Siyang Li,Xiaoqing Chen,Dongrui Wu*

Main category: cs.LG

TL;DR: Comprehensive survey of brain signal generation methods for BCIs, categorizing approaches, benchmarking performance, and discussing future directions for privacy-aware BCI systems.


<details>
  <summary>Details</summary>
Motivation: BCI development is constrained by limited, heterogeneous, and privacy-sensitive neural data, creating a need for synthetic yet physiologically plausible brain signals to overcome data scarcity and enhance model capacity.

Method: Systematically categorizes generative algorithms into four types: knowledge-based, feature-based, model-based, and translation-based approaches. Benchmarks these approaches across four representative BCI paradigms and provides objective performance comparisons.

Result: Provides comprehensive methodological taxonomy, benchmark experiments, evaluation metrics, and key applications. The benchmark codebase is publicly available at https://github.com/wzwvv/DG4BCI.

Conclusion: Brain signal generation is crucial for addressing BCI data limitations. The survey discusses potentials and challenges of current approaches and prospects future research on accurate, data-efficient, and privacy-aware BCI systems.
...
### [141] [Global Evolutionary Steering: Refining Activation Steering Control via Cross-Layer Consistency](https://arxiv.org/abs/2603.12298)
*Xinyan Jiang,Wenjing Yu,Di Wang,Lijie Hu*

Main category: cs.LG

TL;DR: GER-steer is a training-free framework that uses global evolutionary signals to refine steering vectors for LLM control, outperforming existing methods by decoupling semantic intent from noise and artifacts.


<details>
  <summary>Details</summary>
Motivation: Existing activation engineering methods suffer from high-dimensional noise and layer-wise semantic drift, capturing spurious correlations rather than target intent. They rely on static activation differences that are susceptible to these issues.

Method: GER-steer exploits the geometric stability of the network's representation evolution to rectify raw steering vectors. It uses global evolutionary signals to decouple robust semantic intent from orthogonal artifacts without requiring training or fine-tuning.

Result: Extensive evaluations show GER-steer consistently outperforms baselines, delivering superior efficacy and generalization without layer-specific tuning.

Conclusion: GER-steer establishes a universal solution for reliable model alignment through training-free activation engineering that leverages global evolutionary signals to produce robust steering vectors.
...
### [142] [A Geometrically-Grounded Drive for MDL-Based Optimization in Deep Learning](https://arxiv.org/abs/2603.12304)
*Ming Lei,Shufan Wu,Christophe Baehr*

Main category: cs.LG

TL;DR: A novel optimization framework that integrates Minimum Description Length (MDL) principle into neural network training dynamics, using geometric manifolds and coupled Ricci flow with MDL Drive to balance data fidelity and model compression.


<details>
  <summary>Details</summary>
Motivation: To move beyond MDL's conventional role as a model selection criterion and instead use it as an active driving force within optimization, creating more autonomous, generalizable, and interpretable AI systems by unifying geometric deep learning with information-theoretic principles.

Method: Reformulates MDL as an active optimization force using a geometrically-grounded cognitive manifold governed by coupled Ricci flow with a novel MDL Drive term. The method includes theoretical foundations, geometric surgery protocols, and a practical algorithm with O(N log N) complexity.

Result: Established theoretical properties including monotonic decrease of description length, finite topological phase transitions, universal critical behavior, and provided a computationally efficient algorithm with stability guarantees. Empirical validation on synthetic tasks shows robust generalization and autonomous model simplification.

Conclusion: This work provides a principled path toward more autonomous, generalizable, and interpretable AI systems by fundamentally integrating MDL into training dynamics, unifying geometric deep learning with information-theoretic principles for improved model compression and generalization.
...
### [143] [HCP-DCNet: A Hierarchical Causal Primitive Dynamic Composition Network for Self-Improving Causal Understanding](https://arxiv.org/abs/2603.12305)
*Ming Lei,Shufan Wu,Christophe Baehr*

Main category: cs.LG

TL;DR: HCP-DCNet is a hierarchical causal framework that bridges continuous physical dynamics with discrete symbolic causal inference through reusable causal primitives and dynamic composition, enabling autonomous self-improvement and outperforming SOTA in causal reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Deep learning lacks causal modeling, making AI systems brittle under distribution shifts and unable to answer "what-if" questions. There's a need for robust AI that can understand and reason about cause and effect, including interventions, counterfactuals, and underlying mechanisms.

Method: HCP-DCNet decomposes causal scenes into reusable typed causal primitives organized into four abstraction layers (physical, functional, event, rule). A dual-channel routing network dynamically composes these primitives into task-specific, fully differentiable Causal Execution Graphs (CEGs). The system uses causal-intervention-driven meta-evolution strategy for autonomous self-improvement through a constrained Markov decision process.

Result: Extensive experiments across simulated physical and social environments show HCP-DCNet significantly outperforms state-of-the-art baselines in causal discovery, counterfactual reasoning, and compositional generalization. The framework provides theoretical guarantees including type-safe composition, routing convergence, and universal approximation of causal dynamics.

Conclusion: HCP-DCNet provides a principled, scalable, and interpretable architecture for building AI systems with human-like causal abstraction and continual self-refinement capabilities, bridging the gap between continuous physical dynamics and discrete symbolic causal inference.
...
### [144] [Thermodynamics of Reinforcement Learning Curricula](https://arxiv.org/abs/2603.12324)
*Jacob Adamczyk,Juan Sebastian Rojas,Rahul V. Kulkarni*

Main category: cs.LG

TL;DR: The paper proposes a geometric framework for RL by interpreting reward parameters as coordinates on a task manifold, showing that optimal curricula correspond to geodesics when minimizing excess thermodynamic work.


<details>
  <summary>Details</summary>
Motivation: To leverage connections between statistical mechanics and machine learning to formalize curriculum learning in reinforcement learning using non-equilibrium thermodynamics principles.

Method: Proposes a geometric framework where reward parameters are coordinates on a task manifold, and shows that minimizing excess thermodynamic work leads to optimal curricula as geodesics in this space. Provides the MEW algorithm for principled temperature annealing in maximum-entropy RL.

Result: Develops a theoretical framework connecting thermodynamics to curriculum learning in RL, showing that geodesics in task space correspond to optimal curricula, and provides a practical algorithm for temperature scheduling.

Conclusion: The work establishes a principled connection between non-equilibrium thermodynamics and curriculum learning in RL, providing both theoretical insights and practical algorithms for improved training through optimal task sequencing.
...
### [145] [Fractals made Practical: Denoising Diffusion as Partitioned Iterated Function Systems](https://arxiv.org/abs/2603.13069)
*Ann Dooms*

Main category: cs.LG

TL;DR: The paper shows that DDIM diffusion models operate as Partitioned Iterated Function Systems (PIFS), providing a geometric framework to analyze and optimize diffusion model design through computable contraction/expansion metrics.


<details>
  <summary>Details</summary>
Motivation: To understand what diffusion models actually do when transforming noise into images, and to develop a unified geometric framework for analyzing diffusion model dynamics without requiring model evaluation.

Method: The authors demonstrate that deterministic DDIM reverse chains function as Partitioned Iterated Function Systems (PIFS). They derive three computable geometric quantities from this structure: per-step contraction threshold L*_t, diagonal expansion function f_t(λ), and global expansion threshold λ**. These metrics characterize denoising dynamics and explain the two-regime behavior of diffusion models.

Result: The PIFS framework explains diffusion model behavior: global context assembly at high noise via diffuse cross-patch attention, and fine-detail synthesis at low noise via patch-by-patch suppression release. Self-attention emerges as the natural primitive for PIFS contraction. The Kaplan-Yorke dimension of the PIFS attractor is determined analytically. The framework also explains four empirical design choices as approximate solutions to geometric optimization problems.

Conclusion: Diffusion models operate as PIFS, providing a geometric foundation for understanding and optimizing diffusion model design. The framework explains empirical design choices and offers computable metrics for analyzing denoising dynamics without model evaluation, bridging theory and practice.
...
### [146] [Maximum Entropy Exploration Without the Rollouts](https://arxiv.org/abs/2603.12325)
*Jacob Adamczyk,Adam Kamoski,Rahul V. Kulkarni*

Main category: cs.LG

TL;DR: EVE: EigenVector-based Exploration algorithm for maximum entropy exploration without rollouts, using spectral methods to compute optimal visitation distributions.


<details>
  <summary>Details</summary>
Motivation: Efficient exploration is crucial for RL pretraining when rewards are unavailable. Existing methods require expensive on-policy rollouts for visitation estimation, creating computational bottlenecks.

Method: Formulates exploration as maximizing steady-state visitation entropy. Uses spectral characterization of entropy-regularized objective via dominant eigenvectors of transition matrix. EVE algorithm computes solutions through iterative updates without rollouts. Employs posterior-policy iteration for unregularized objective.

Result: Proves convergence under standard assumptions. Demonstrates competitive exploration performance in deterministic grid-worlds compared to rollout-based baselines.

Conclusion: EVE provides efficient, rollout-free approach to maximum entropy exploration with theoretical guarantees and empirical effectiveness.
...
### [147] [Generalist Large Language Models for Molecular Property Prediction: Distilling Knowledge from Specialist Models](https://arxiv.org/abs/2603.12344)
*Khiem Le,Sreejata Dey,Marcos Martínez Galindo,Vanessa Lopez,Ting Hua,Nitesh V. Chawla,Hoang Thanh Lam*

Main category: cs.LG

TL;DR: TreeKD improves LLM performance for molecular property prediction by distilling knowledge from tree-based specialist models through verbalized rules and rule-consistency ensembling.


<details>
  <summary>Details</summary>
Motivation: Current LLM performance in molecular property prediction remains below practical adoption thresholds, despite their promise as generalist models for drug discovery tasks.

Method: TreeKD trains specialist decision trees on functional group features, verbalizes their learned predictive rules as natural language for rule-augmented context learning, and introduces rule-consistency ensembling at test time.

Result: Experiments on 22 ADMET properties from TDC benchmark show TreeKD substantially improves LLM performance, narrowing the gap with SOTA specialist models.

Conclusion: TreeKD advances toward practical generalist models for molecular property prediction by enabling LLMs to leverage structural insights difficult to extract from SMILES strings alone.
...
### [148] [Budget-Sensitive Discovery Scoring: A Formally Verified Framework for Evaluating AI-Guided Scientific Selection](https://arxiv.org/abs/2603.12349)
*Abhinaba Basu,Pavan Chakraborty*

Main category: cs.LG

TL;DR: Introduces Budget-Sensitive Discovery Score (BSDS) and Discovery Quality Score (DQS) to evaluate AI candidate selection strategies under budget constraints, showing LLMs don't add value over existing ML pipelines in drug discovery.


<details>
  <summary>Details</summary>
Motivation: Scientific discovery relies on AI to select candidates for expensive experiments, but lacks principled budget-aware evaluation frameworks, especially with LLMs generating plausible proposals without reliable evaluation.

Method: Developed BSDS (formally verified with 20 theorems in Lean 4) that penalizes false discoveries and excessive abstention at each budget level, with DQS as budget-averaged summary. Applied to 39 proposers (ML variants and LLM configurations) on MoleculeNet HIV dataset with bootstrap replicates.

Result: Simple RF-based Greedy-ML achieved best DQS (-0.046), outperforming all MLP variants and LLM configurations. No LLM surpassed Greedy-ML baseline, showing LLMs provide no marginal value over existing trained classifiers. Results generalized across multiple benchmarks and parameters.

Conclusion: BSDS/DQS provides principled evaluation framework for budget-constrained candidate selection. LLMs don't add value over existing ML pipelines in drug discovery, challenging assumptions about LLM utility in scientific discovery pipelines.
...
### [149] [Spatial PDE-aware Selective State-space with Nested Memory for Mobile Traffic Grid Forecasting](https://arxiv.org/abs/2603.12353)
*Zineddine Bettouche,Khalid Ali,Andreas Fischer,Andreas Kassler*

Main category: cs.LG

TL;DR: NeST-S6 is a convolutional selective state-space model with spatial PDE-aware core and nested learning memory for efficient spatiotemporal grid forecasting of cellular network traffic.


<details>
  <summary>Details</summary>
Motivation: Traditional cell-specific models are expensive to train/maintain, while global models fail to capture spatial heterogeneity. Recent spatiotemporal architectures have high computational overhead, limiting scalability for large networks.

Method: Proposes NeST-S6: convolutional selective state-space model with spatial PDE-aware core in nested learning paradigm. Uses convolutional local spatial mixing feeding into spatial PDE-aware SSM core, with nested-learning long-term memory updated by learned optimizer when prediction errors indicate unmodeled dynamics.

Result: Achieves lower errors than Mamba-family baseline on Milan dataset at three resolutions (202, 502, 1002) in both single-step and 6-step autoregressive rollouts. Under drift stress tests, nested memory lowers MAE by 48-65% over no-memory ablation. Speeds full-grid reconstruction by 32x, reduces MACs by 4.3x, and achieves 61% lower per-pixel RMSE compared to per-pixel scanning models.

Conclusion: NeST-S6 provides an efficient, scalable solution for spatiotemporal grid forecasting that balances accuracy with computational efficiency, addressing limitations of existing approaches for large-scale cellular network traffic prediction.
...
TL;DR: The paper establishes a theoretical connection between drifting generative dynamics and Sinkhorn divergence gradient flows, showing that Sinkhorn drifting improves stability and performance over prior drifting methods.


<details>
  <summary>Details</summary>
Motivation: Prior drifting formulations have an identifiability gap where zero drift doesn't guarantee model-target measure matching. The authors aim to connect drifting dynamics to Sinkhorn divergence gradient flows to resolve this issue and improve generative quality.

Method: Theoretical analysis shows Sinkhorn divergence yields a cross-minus-self structure similar to drifting but with terms defined by entropic optimal-transport couplings via two-sided Sinkhorn scaling. This provides a precise connection between drifting and Sinkhorn-divergence gradient flows.

Result: Sinkhorn drifting reduces sensitivity to kernel temperature and improves one-step generative quality. On FFHQ-ALAE, it reduces mean FID from 187.7 to 37.1 and mean latent EMD from 453.3 to 144.4 at lowest temperature. On MNIST, it preserves full class coverage across temperature sweep.

Conclusion: Sinkhorn drifting acts as a surrogate for Sinkhorn-divergence gradient flow, resolving identifiability issues in prior drifting formulations. It provides more stable optimization without changing inference procedures, translating to significant practical improvements in low-temperature settings.
...
### [151] [NeuroLoRA: Context-Aware Neuromodulation for Parameter-Efficient Multi-Task Adaptation](https://arxiv.org/abs/2603.12378)
*Yuxin Yang,Haoran Zhang,Mingxuan Li,Jiachen Xu,Ruoxi Shen,Zhenyu Wang,Tianhao Liu,Siqi Chen,Weilin Huang*

Main category: cs.LG

TL;DR: NeuroLoRA introduces a context-aware neuromodulation gate to LoRA, improving performance over static routing methods like FlyLoRA while maintaining parameter efficiency.


<details>
  <summary>Details</summary>
Motivation: Current LoRA methods like FlyLoRA use static, magnitude-based routing that ignores input context, limiting their ability to adapt dynamically to different tasks and inputs.

Method: NeuroLoRA combines Mixture-of-Experts with a learnable neuromodulation gate that contextually rescales projection space before expert selection, plus a Contrastive Orthogonality Loss to separate expert subspaces.

Result: Outperforms FlyLoRA and other baselines on MMLU, GSM8K, and ScienceQA across single-task adaptation, multi-task merging, and continual learning while maintaining comparable parameter efficiency.

Conclusion: NeuroLoRA successfully introduces biological neuromodulation principles to create a more context-aware and effective parameter-efficient fine-tuning framework.
...
### [152] [SpectralGuard: Detecting Memory Collapse Attacks in State Space Models](https://arxiv.org/abs/2603.12414)
*Davi Bonetto*

Main category: cs.LG

TL;DR: SSMs like Mamba have a vulnerability where adversarial inputs can collapse memory by driving spectral radius to zero, destroying reasoning capacity without detection. The paper introduces SpectralGuard to monitor spectral stability in real-time.


<details>
  <summary>Details</summary>
Motivation: State Space Models achieve efficient linear-time processing through input-dependent recurrence, but this creates a critical safety vulnerability where adversaries can poison hidden states to collapse memory, silently destroying reasoning without triggering output-level alarms.

Method: The paper proves an Evasion Existence Theorem showing vulnerabilities exist for output-only defenses, then introduces SpectralGuard - a real-time monitor that tracks spectral stability across all model layers to detect adversarial attacks that collapse memory.

Result: SpectralGuard achieves F1=0.961 against non-adaptive attackers and retains F1=0.842 under strongest adaptive settings, with sub-15ms per-token latency. Causal interventions and cross-architecture transfer to hybrid SSM-Attention systems confirm effectiveness.

Conclusion: Spectral monitoring provides a principled, deployable safety layer for recurrent foundation models, addressing critical vulnerabilities in SSMs that can be exploited through gradient-based hidden state poisoning attacks.
...
### [153] [Overcoming the Modality Gap in Context-Aided Forecasting](https://arxiv.org/abs/2603.12451)
...
TL;DR: Semi-synthetic data augmentation creates CAF-7M dataset to address context quality issues in multimodal forecasting, showing dataset quality (not architecture) is the bottleneck.


<details>
  <summary>Details</summary>
Motivation: Multimodal models often fail to outperform unimodal ones in context-aided forecasting due to poor context quality in existing datasets, which is hard to verify.

Method: Introduce semi-synthetic data augmentation method that generates contexts both descriptive of temporal dynamics and verifiably complementary to numerical histories, creating CAF-7M (7M context-augmented time series windows).

Result: Semi-synthetic pre-training transfers effectively to real-world evaluation, shows clear evidence of context utilization, and suggests dataset quality is the primary bottleneck.

Conclusion: Dataset quality, rather than architectural limitations, has been the primary bottleneck in context-aided forecasting, and semi-synthetic data generation addresses this issue.
...
### [154] [Bases of Steerable Kernels for Equivariant CNNs: From 2D Rotations to the Lorentz Group](https://arxiv.org/abs/2603.12459)
*Alan Garbarz*

Main category: cs.LG

TL;DR: Presents a new method for solving steerable kernel constraints in equivariant CNNs using explicit bases that bypass Clebsch-Gordan coefficient computations.


<details>
  <summary>Details</summary>
Motivation: Current methods for designing steerable equivariant CNNs require computing Clebsch-Gordan coefficients, which can be numerically or analytically challenging. The paper aims to provide a simpler, more accessible approach.

Method: Find explicit real and complex bases for kernels by first solving a simpler invariance condition at a reference point x₀, then using the steerability equation to extend to arbitrary points x = g·x₀. This avoids direct Clebsch-Gordan computations.

Result: Provides ready-to-use bases for different symmetry groups and arbitrary tensor types, making the method directly applicable without complex coefficient calculations.

Conclusion: Offers a more accessible and practical approach to designing steerable equivariant CNNs by simplifying the kernel constraint solution process, making the technique available to a broader audience.
...
### [155] [Modal Logical Neural Networks for Financial AI](https://arxiv.org/abs/2603.12487)
*Antonin Sulc*

Main category: cs.LG

TL;DR: Modal Logical Neural Networks (MLNNs) bridge deep learning and symbolic logic for finance, enabling differentiable reasoning about necessity, possibility, time, and knowledge through Kripke semantics integrated into neural architectures.


<details>
  <summary>Details</summary>
Motivation: The financial industry faces a dichotomy: deep learning offers strong empirical performance but lacks interpretability, while symbolic logic provides interpretability and rule adherence needed for regulated settings. There's a need to bridge these approaches to combine performance with compliance.

Method: MLNNs integrate Kripke semantics into neural architectures, creating a differentiable "Logic Layer" with Necessity Neurons (□) and Learnable Accessibility (Aθ) that enable reasoning about modal concepts. This allows mapping logical constraints to financial applications like regulatory guardrails and market analysis.
...
Conclusion: MLNNs successfully bridge deep learning and symbolic logic for finance, offering a framework that combines empirical performance with interpretability and rule adherence needed in regulated financial environments, addressing key challenges in AI adoption for the industry.
...
### [156] [Probing Length Generalization in Mamba via Image Reconstruction](https://arxiv.org/abs/2603.12499)
*Jan Rathjens,Robin Schiewer,Laurenz Wiskott,Anand Subramoney*

Main category: cs.LG

TL;DR: Mamba's performance degrades when inference sequences exceed training lengths; study reveals it adapts to training length distribution and fails to generalize; length-adaptive variant improves performance.


<details>
  <summary>Details</summary>
Motivation: Mamba shows promise as a general-purpose sequence model with low computational complexity, but its performance degrades when inference sequence lengths exceed those seen during training, which is a critical limitation for practical applications.

Method: Used controlled vision task where Mamba reconstructs images from sequences of image patches. Analyzed reconstructions at different stages of sequence processing. Introduced a length-adaptive variant of Mamba to test improvements.

Result: Mamba qualitatively adapts its behavior to the distribution of sequence lengths encountered during training, resulting in strategies that fail to generalize beyond this range. The length-adaptive variant improves performance across training sequence lengths.

Conclusion: The study provides intuitive perspective on length generalization in Mamba and suggests directions for improving the architecture, particularly through length-adaptive mechanisms.
...
### [157] [Adaptive Conditional Forest Sampling for Spectral Risk Optimisation under Decision-Dependent Uncertainty](https://arxiv.org/abs/2603.12507)
*Marcell T. Kurbucz*

Main category: cs.LG

TL;DR: ACFS is a simulation-optimization framework for minimizing spectral risk with decision-dependent uncertainty, combining random forests for distribution approximation, global exploration, focused augmentation, and two-stage reranking.


<details>
  <summary>Details</summary>
Motivation: Minimizing spectral risk (CVaR + expected cost) is challenging with decision-dependent uncertainty distributions, as both surrogate modeling and simulation-based ranking are sensitive to tail estimation errors.

Method: Adaptive Conditional Forest Sampling (ACFS) - a four-phase framework integrating: 1) Generalised Random Forests for decision-conditional distribution approximation, 2) CEM-guided global exploration, 3) rank-weighted focused augmentation, and 4) surrogate-to-oracle two-stage reranking with multi-start gradient-based refinement.

Result: ACFS achieves lowest median oracle spectral risk on Gaussian copula benchmark across all configurations (6.0-20.0% improvement over GP-BO). On Student-t copula, statistically similar median performance but reduces cross-replication dispersion by 1.8-1.9x (first benchmark) and 1.7-2.0x (second benchmark), improving reliability. Outperforms CEM-SO, SGD-CVaR, and KDE-SO in nearly all settings.

Conclusion: ACFS effectively addresses spectral risk minimization with decision-dependent uncertainty, demonstrating superior performance, improved reliability, and robustness through ablation and sensitivity analyses.
...
### [158] [Byzantine-Robust Optimization under $(L_0, L_1)$-Smoothness](https://arxiv.org/abs/2603.12512)
*Arman Bolatov,Samuel Horváth,Martin Takáč,Eduard Gorbunov*

Main category: cs.LG

TL;DR: Byz-NSGDM: A Byzantine-robust normalized stochastic gradient descent method with momentum for distributed optimization under $(L_0,L_1)$-smoothness, achieving $O(K^{-1/4})$ convergence rate with robustness against Byzantine attacks.


<details>
  <summary>Details</summary>
Motivation: Distributed optimization faces challenges from Byzantine attacks where malicious workers can send arbitrary information. Additionally, many real-world optimization problems exhibit $(L_0,L_1)$-smoothness (state-dependent gradient Lipschitz constants) rather than standard $L$-smoothness, requiring specialized algorithms that can handle both Byzantine robustness and this generalized smoothness condition.

Method: Byz-NSGDM combines momentum normalization with Byzantine-robust aggregation enhanced by Nearest Neighbor Mixing (NNM). The algorithm uses normalized stochastic gradient descent with momentum to handle $(L_0,L_1)$-smoothness, while NNM-enhanced aggregation provides robustness against Byzantine workers who may send arbitrary or malicious gradient information.

Result: Theoretical analysis proves Byz-NSGDM achieves $O(K^{-1/4})$ convergence rate up to a Byzantine bias floor proportional to the robustness coefficient and gradient heterogeneity. Experimental validation on heterogeneous MNIST classification, synthetic $(L_0,L_1)$-smooth optimization, and character-level language modeling with a small GPT model demonstrates effectiveness against various Byzantine attack strategies. Ablation studies show robustness across wide ranges of momentum and learning rate choices.

Conclusion: Byz-NSGDM provides an effective solution for distributed optimization under both Byzantine attacks and $(L_0,L_1)$-smoothness conditions, combining momentum normalization with robust aggregation to achieve theoretical convergence guarantees while maintaining practical effectiveness across diverse applications and attack scenarios.
...
### [159] [Learning Pore-scale Multiphase Flow from 4D Velocimetry](https://arxiv.org/abs/2603.12516)
...
TL;DR: Multimodal learning framework predicts multiphase pore-scale flow from 4D micro-velocimetry data, combining graph networks for particle motion with 3D U-Nets for interface evolution.


<details>
  <summary>Details</summary>
Motivation: Multiphase flow in porous media is crucial for subsurface energy technologies (CO₂ and hydrogen storage), but pore-scale dynamics in 3D materials are hard to characterize and predict experimentally.

Method: Multimodal learning framework that couples graph network simulator for Lagrangian tracer-particle motion with 3D U-Net for voxelized interface evolution. Uses imaged pore geometry as boundary constraint, with iterative coupling of flow velocity and interface predictions at each time step. Trained autoregressively on experimental sequences in capillary-dominated conditions.

Result: Learned surrogate captures transient flow perturbations and abrupt interface rearrangements (Haines jumps) over seconds of physical time, reducing hour-to-day-scale direct numerical simulations to seconds of inference.

Conclusion: Framework enables "digital experiments" to replicate pore-scale physics from multiphase flow experiments, offering efficient tool for exploring injection conditions and pore-geometry effects relevant to subsurface carbon and hydrogen storage.
...
TL;DR: Curriculum Sampling improves Flow Matching models by using middle-biased sampling early for fast structure learning, then switching to Uniform sampling for boundary refinement, achieving better FID in fewer training steps.


<details>
  <summary>Details</summary>
Motivation: Current Flow Matching models use static middle-biased timestep sampling (like Logit-Normal), which creates a speed-quality trade-off: fast early convergence but worse asymptotic fidelity than Uniform sampling. The authors identify that under-sampling endpoints leaves fine details unresolved.

Method: Analyzed per-timestep training losses revealing a U-shaped difficulty profile with persistent errors near boundaries. Proposed Curriculum Sampling: a two-phase schedule starting with middle-biased sampling for rapid structure learning, then switching to Uniform sampling for boundary refinement.

Result: On CIFAR-10, Curriculum Sampling improved best FID from 3.85 (Uniform) to 3.22, reaching peak performance at 100k training steps instead of 150k. This demonstrates both quality improvement and training efficiency.

Conclusion: Timestep sampling should be treated as an evolving curriculum rather than a fixed hyperparameter. Curriculum Sampling effectively addresses the speed-quality trade-off by adapting sampling strategy during training.
...
TL;DR: LLM judges with moderate global correlation perform poorly for best-of-n selection tasks due to weak within-prompt ranking signal and high tie rates.


<details>
  <summary>Details</summary>
Motivation: Current validation of LLM judges using single global metrics like correlation is misleading for real-world deployment tasks involving best-of-n selection within prompts.

Method: Analyzed 5,000-prompt best-of-4 benchmark from Chatbot Arena, comparing global vs within-prompt correlation, tie rates, and using explicit pairwise judging in matched-pair best-of-2 audit.

Result: Judge with moderate global correlation (r=0.47) captures only 21.0% of potential improvement; within-prompt correlation is much lower (r_within=0.27) with 67% tie rate. Pairwise judging improves recovery from 21.1% to 61.2%.

Conclusion: For judge-based selection tasks, audits should report within-prompt signal, tie rates, and recovery/top-1 accuracy rather than relying solely on global agreement metrics.
...
TL;DR: TERMINATOR is an early-exit strategy that reduces overthinking in Large Reasoning Models by predicting optimal reasoning lengths, achieving 14%-55% CoT length reduction across challenging datasets.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models suffer from significant overthinking, spending excessive compute time even after generating correct answers early. While optimal reasoning lengths exist, determining them is highly non-trivial and task/model-dependent.

Method: TERMINATOR leverages the predictability of first answer arrivals to create a dataset of optimal reasoning lengths. It trains an early-exit strategy that truncates reasoning at optimal points to reduce overthinking.

Result: Achieves 14%-55% reduction in Chain-of-Thought lengths on average across four challenging datasets (MATH-500, AIME 2025, HumanEval, GPQA) while outperforming current state-of-the-art methods.

Conclusion: TERMINATOR effectively mitigates overthinking in Large Reasoning Models by predicting optimal reasoning lengths, significantly reducing computational overhead without compromising performance.
...
TL;DR: Extends "contexts are cheap" perspective to Markovian contextual linear bandits with exogenous Markov chain action sets, providing reduction to standard linear bandits with minimal mixing time dependence.


<details>
  <summary>Details</summary>
Motivation: Motivated by applications with temporally correlated availability, where action sets evolve via exogenous Markov chains (e.g., resource availability, user preferences). Extends the advantageous "contexts are cheap" perspective from i.i.d. contexts to Markovian settings.

Method: 1) Reduction under uniform geometric ergodicity using stationary surrogate action set with delayed-update scheme to control bias from nonstationary conditional context distributions. 2) Phased algorithm for unknown transition distributions that learns surrogate mapping online.

Result: Obtains high-probability worst-case regret bound matching underlying linear bandit oracle, with only lower-order dependence on mixing time (not dominating the regret).

Conclusion: Successfully extends the "contexts are cheap" framework to Markovian contextual linear bandits, enabling application of mature linear bandit techniques to problems with temporally correlated action availability while maintaining strong theoretical guarantees.
...
TL;DR: EQML brings quantum ML to edge devices via hybrid workflows (offloading quantum subroutines) or embedded QPU concepts, with quantum-inspired classical approaches as a practical bridge. The paper analyzes feasibility, identifies key barriers, and outlines engineering directions.


<details>
  <summary>Details</summary>
Motivation: To enable quantum machine learning capabilities on resource-constrained edge platforms (IoT nodes, wearables, drones, controllers) by analyzing current feasibility and identifying pathways for practical implementation.

Method: Analysis from a circuits-and-systems perspective, formalizing two implementation pathways: (1) hybrid workflows with remote QPU offloading, and (2) embedded QPU concepts with integrated quantum co-processors. Also considers quantum-inspired classical approaches on embedded processors/FPGAs.

Result: Identifies dominant barriers: latency, data encoding overhead, NISQ noise, tooling mismatch, and energy constraints. Maps these to concrete engineering directions in interface design, control electronics, power management, verification, and security.

Conclusion: EQML is currently feasible only in limited experimental forms. Practical implementation requires addressing technical barriers through specific engineering solutions, and responsible deployment needs adversarial evaluation and governance practices similar to edge AI systems.
...
TL;DR: Transformer depth dynamics can be accurately approximated by low-order linear surrogates within context, with agreement improving as model size scales.


<details>
  <summary>Details</summary>
Motivation: Large language models are typically treated as black-box nonlinear systems, but understanding their internal dynamics could provide better analysis and control methods.

Method: Developed low-order linear surrogates (32-dimensional) to approximate transformer depth dynamics, tested across tasks (toxicity, irony, hate speech, sentiment) on GPT-2 models, and studied scaling properties across the GPT-2 family.

Result: Linear surrogates reproduce GPT-2-large's layerwise sensitivity with near-perfect agreement, show monotonic improvement in agreement with model size scaling, and enable principled multi-layer interventions requiring less energy than heuristic schedules.

Conclusion: As language models scale, low-order linear depth dynamics emerge within contexts, providing a systems-theoretic foundation for analyzing and controlling transformer-based models.
...
TL;DR: CALF framework trains RL policies under realistic network conditions to improve deployment performance in distributed edge-cloud systems.


<details>
  <summary>Details</summary>
Motivation: Standard RL training assumes zero-latency interaction, causing severe performance degradation when deployed across edge devices and cloud servers facing real network delays, jitter, and packet loss.

Method: Introduces CALF (Communication-Aware Learning Framework) which trains policies under realistic network models during simulation, explicitly modeling communication constraints.

Result: Network-aware training substantially reduces deployment performance gaps compared to network-agnostic baselines. Distributed policy deployments across heterogeneous hardware validate robust real-world execution.
...
TL;DR: DDMM improves unsupervised multivariate time series similarity retrieval by learning minute differences within states using weighted anchor-positive pairs based on Euclidean distance.


<details>
  <summary>Details</summary>
Motivation: Industrial plants need to recognize minute differences between states in time series data for accurate similarity retrieval, but existing methods struggle with learning these subtle variations.

Method: DDMM uses a learning algorithm that assigns weights to anchor-positive sample pairs based on Euclidean distance, then learns the differences within these weighted pairs, enabling learning of minute differences while sampling from entire time series.

Result: DDMM significantly outperforms state-of-the-art time series representation learning methods on Pulp-and-paper mill dataset, and combining with existing feature extraction methods further improves accuracy.

Conclusion: DDMM effectively improves retrieval accuracy in industrial plant time series data by learning minute differences within states, and can be enhanced through integration with existing feature extraction techniques.
...
### [168] [Asymptotic and Finite-Time Guarantees for Langevin-Based Temperature Annealing in InfoNCE](https://arxiv.org/abs/2603.12552)
*Faris Chaudhry*

Main category: cs.LG

TL;DR: Theoretical analysis shows that slow logarithmic temperature annealing in contrastive learning ensures convergence to optimal representations, while faster schedules risk suboptimal minima.


<details>
  <summary>Details</summary>
Motivation: The InfoNCE loss in contrastive learning critically depends on temperature parameters, but the dynamics of fixed versus annealed temperature schedules remain poorly understood theoretically.

Method: Model embedding evolution under Langevin dynamics on a compact Riemannian manifold, analyzing temperature schedules using classical simulated annealing theory under mild smoothness and energy-barrier assumptions.

Result: Slow logarithmic inverse-temperature schedules guarantee convergence in probability to globally optimal representations, while faster schedules risk becoming trapped in suboptimal minima.

Conclusion: Establishes a theoretical link between contrastive learning and simulated annealing, providing principled basis for understanding and tuning temperature schedules in practice.
...
### [169] [Reinforcement Learning for Diffusion LLMs with Entropy-Guided Step Selection and Stepwise Advantages](https://arxiv.org/abs/2603.12554)
*Vishnu Teja Kunde,Fatemeh Doudi,Mahdi Farahbakhsh,Dileep Kalathil,Krishna Narayanan,Jean-Francois Chamberland*

Main category: cs.LG

TL;DR: This paper introduces an exact, unbiased policy gradient method for reinforcement learning with diffusion language models, overcoming challenges with intractable sequence-level likelihoods through a novel MDP formulation over denoising trajectories.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods for autoregressive language models don't extend well to diffusion language models due to intractable sequence-level likelihoods. Current approaches use surrogate likelihoods or heuristic approximations that introduce bias and obscure the sequential structure of denoising.

Method: Formulates diffusion-based sequence generation as a finite-horizon Markov decision process over denoising trajectory. Derives an exact, unbiased policy gradient that decomposes over denoising steps using intermediate advantages. Uses entropy-guided approximation bound to select denoising steps for policy updates and estimates intermediate advantages using one-step denoising reward from the diffusion model.

Result: Achieves state-of-the-art results on coding and logical reasoning benchmarks, with strong competitive performance on mathematical reasoning. Outperforms existing RL post-training approaches for diffusion language models.

Conclusion: The proposed method provides an effective RL framework for diffusion language models that avoids biased approximations and costly rollouts, enabling better post-training performance on reasoning tasks.
...
### [170] [Scaling Laws and Pathologies of Single-Layer PINNs: Network Width and PDE Nonlinearity](https://arxiv.org/abs/2603.12556)
*Faris Chaudhry*

Main category: cs.LG

TL;DR: Empirical scaling laws for Single-Layer Physics-Informed Neural Networks reveal dual optimization failures: baseline pathology (solution error doesn't improve with width) and compounding pathology (worsened by nonlinearity), showing complex non-separable scaling beyond simple power laws.


<details>
  <summary>Details</summary>
Motivation: To understand why Physics-Informed Neural Networks (PINNs) fail to achieve expected performance improvements with increased network width, particularly for nonlinear PDEs, despite theoretical approximation bounds suggesting they should.

Method: Established empirical scaling laws for Single-Layer PINNs on canonical nonlinear PDEs, identified dual optimization failures, provided quantitative evidence against simple separable power laws, and proposed methodology to measure complex scaling effects.

Result: Found two pathologies: (1) baseline pathology where solution error doesn't decrease with network width even at fixed nonlinearity, and (2) compounding pathology where nonlinearity exacerbates this failure. Scaling behavior follows complex non-separable relationships, consistent with spectral bias where networks struggle with high-frequency components intensified by nonlinearity.

Conclusion: Optimization, not approximation capacity, is the primary bottleneck in PINNs. The failures are consistent with spectral bias, and the scaling behavior requires more complex modeling than simple power laws. The paper provides methodology to empirically measure these complex scaling effects.
...
### [171] [Lyapunov Stable Graph Neural Flow](https://arxiv.org/abs/2603.12557)
*Haoyu Chu,Xiaotong Chen,Wei Zhou,Wenjun Cui,Kai Zhao,Shikui Wei,Qiyu Kang*

Main category: cs.LG

TL;DR: The paper proposes a novel defense framework for Graph Neural Networks (GNNs) against adversarial attacks by integrating control theory principles, specifically integer- and fractional-order Lyapunov stability, to constrain GNN feature-update dynamics and provide theoretical stability guarantees.


<details>
  <summary>Details</summary>
Motivation: GNNs are highly vulnerable to adversarial perturbations in both topology and features, creating a critical need for robust representation learning. Existing defense strategies like adversarial training or data purification are resource-heavy, motivating a more fundamental approach to constrain GNN dynamics.

Method: The authors bridge GNNs with control theory, introducing a defense framework based on Lyapunov stability. They propose an adaptive, learnable Lyapunov function paired with a novel projection mechanism that maps the network's state into a stable space. This approach fundamentally constrains the underlying feature-update dynamics of GNNs.

Result: Extensive experiments show that the proposed Lyapunov-stable graph neural flows substantially outperform base neural flows and state-of-the-art baselines across standard benchmarks and various adversarial attack scenarios. The method is orthogonal to existing defenses and can be integrated with techniques like adversarial training for cumulative robustness.

Conclusion: The paper successfully introduces a novel control theory-based defense framework for GNNs that provides theoretically provable stability guarantees against adversarial attacks. The approach offers a fundamental solution to GNN vulnerability by constraining feature-update dynamics rather than relying on resource-heavy conventional methods.
...
### [172] [A Spectral Revisit of the Distributional Bellman Operator under the Cramér Metric](https://arxiv.org/abs/2603.12576)
*Keru Wang,Yixin Deng,Yao Lyu,Stephen Redmond,Shengbo Eben Li*

Main category: cs.LG

TL;DR: The paper analyzes distributional Bellman dynamics at the CDF level, showing Bellman updates act affinely on CDFs and linearly on CDF differences, and constructs regularized spectral Hilbert representations that realize Cramér geometry through exact conjugation.


<details>
  <summary>Details</summary>
Motivation: Existing distributional RL analyses focus on metric contraction properties without elucidating the structural action of Bellman updates on distributions. The authors aim to analyze distributional Bellman dynamics directly at the CDF level to clarify the operator structure.

Method: Analyze distributional Bellman dynamics at the CDF level, treating Cramér geometry as intrinsic setting. Show Bellman update acts affinely on CDFs and linearly on CDF differences. Construct family of regularized spectral Hilbert representations that realize CDF-level geometry by exact conjugation without modifying underlying Bellman dynamics.

Result: Bellman update contraction yields uniform bound on linear action at CDF level. Regularization affects only geometry and vanishes in zero-regularization limit, recovering native Cramér metric. Framework clarifies operator structure underlying distributional Bellman updates.

Conclusion: The intrinsic CDF-level analysis provides foundation for functional and operator-theoretic analyses in distributional RL, moving beyond purely metric approaches to understand structural properties of distributional Bellman dynamics.
...
### [173] [CA-HFP: Curvature-Aware Heterogeneous Federated Pruning with Model Reconstruction](https://arxiv.org/abs/2603.12591)
*Gang Hu,Yinglei Teng,Pengfei Wu,Shijun Ma*

Main category: cs.LG

TL;DR: CA-HFP enables personalized pruning for heterogeneous edge devices in federated learning while maintaining aggregation compatibility and stable convergence through curvature-aware pruning and lightweight reconstruction.


<details>
  <summary>Details</summary>
Motivation: Federated learning on heterogeneous edge devices requires personalized compression to handle device diversity while preserving aggregation compatibility and stable convergence across the system.

Method: Curvature-Aware Heterogeneous Federated Pruning (CA-HFP) framework where each client performs device-specific structured pruning guided by curvature-informed significance scores, then maps compact submodels back to global parameter space via lightweight reconstruction.

Result: Extensive experiments on FMNIST, CIFAR-10, and CIFAR-100 using VGG and ResNet architectures under varying data heterogeneity show CA-HFP preserves model accuracy while significantly reducing per-client computation and communication costs, outperforming standard federated training and existing pruning baselines.

Conclusion: CA-HFP provides a practical solution for efficient federated learning on heterogeneous edge devices by enabling personalized compression with theoretical convergence guarantees and empirical performance improvements.
...
TL;DR: SPL addresses posterior collapse in variational preference learning by using swap annotators and mirroring properties to guide latent variable learning, improving personalization in RLHF.


<details>
  <summary>Details</summary>
Motivation: RLHF assumes a single universal reward, ignoring diverse user preferences. VPL introduces user-specific latents but suffers from posterior collapse under sparse data, reverting to single-reward models.

Method: Swap-guided Preference Learning (SPL) constructs fictitious swap annotators and uses mirroring properties to guide the encoder. Includes swap-guided base regularization, Preferential Inverse Autoregressive Flow (P-IAF), and adaptive latent conditioning.

Result: SPL mitigates posterior collapse, enriches user-specific latent representations, and improves preference prediction performance compared to baseline methods.

Conclusion: SPL successfully addresses posterior collapse in variational preference learning, enabling better personalization in RLHF systems by maintaining meaningful user-specific latent variables.
...
### [175] [Maximizing Incremental Information Entropy for Contrastive Learning](https://arxiv.org/abs/2603.12594)
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses limitations of static augmentations and rigid invariance constraints in existing contrastive learning methods, seeking to bridge theoretical principles with practical applications.

Method: Proposes IE-CL framework with two components: learnable transformation for entropy generation and encoder regularizer for entropy preservation, reframing the encoder as an information bottleneck.

Result: IE-CL consistently improves performance on CIFAR-10/100, STL-10, and ImageNet datasets, particularly under small-batch settings, and can be integrated into existing frameworks.

Conclusion: The work offers a new perspective in contrastive learning by bridging theoretical principles with practice through explicit entropy optimization while maintaining semantic consistency.
...
### [176] [Optimize Wider, Not Deeper: Consensus Aggregation for Policy Optimization](https://arxiv.org/abs/2603.12596)
*Zelal Su,Mustafaoglu,Sungyoung Lee,Eshan Balachandar,Risto Miikkulainen,Keshav Pingali*

Main category: cs.LG

TL;DR: CAPO improves PPO by optimizing wider instead of deeper: multiple PPO replicas with different minibatch orders are aggregated into a consensus, achieving better performance without extra environment interactions.


<details>
  <summary>Details</summary>
Motivation: PPO's multiple epochs of clipped SGD create path-dependent noise where signal saturates but waste grows with additional epochs, creating an optimization-depth dilemma. The paper aims to understand this drift and improve policy optimization efficiency.

Method: Proposes Consensus Aggregation for Policy Optimization (CAPO): K PPO replicates are optimized on the same batch with different minibatch shuffling orders, then aggregated into a consensus. Studies aggregation in Euclidean parameter space and natural parameter space via logarithmic opinion pool.

Result: In natural parameter space, consensus provably achieves higher KL-penalized surrogate and tighter trust region compliance than mean expert. On continuous control tasks, CAPO outperforms PPO and compute-matched deeper baselines by up to 8.6x under fixed sample budgets.

Conclusion: Policy optimization can be improved by optimizing wider rather than deeper without additional environment interactions. CAPO demonstrates the effectiveness of consensus aggregation for more efficient policy optimization.
...
### [177] [Feynman: Knowledge-Infused Diagramming Agent for Scalable Visual Designs](https://arxiv.org/abs/2603.12597)
*Zixin Wen,Yifu Cai,Kyle Lee,Sam Estep,Josh Sunshine,Aarti Singh,Yuejie Chi,Wode Ni*

Main category: cs.LG

TL;DR: Feynman is an AI agent pipeline that automatically generates diverse, knowledge-rich diagrams with grounded captions at scale, creating a 100k+ dataset for visual reasoning evaluation.


<details>
  <summary>Details</summary>
Motivation: High-quality vision-language data is scarce despite abundant internet content, limiting multi-modal AI system development. There's a need for scalable generation of well-aligned image-text pairs, particularly for visual design applications.

Method: Feynman agent pipeline: 1) enumerates domain-specific knowledge components ("ideas"), 2) performs code planning, 3) translates ideas into declarative programs, 4) iterates with feedback for visual refinement, 5) renders using Penrose diagramming system with optimization-based layout that preserves visual semantics while injecting randomness.

Result: Generated over 100k well-aligned diagram-caption pairs with visual consistency and diversity. Created Diagramma benchmark for evaluating visual reasoning capabilities of vision-language models. The pipeline enables low-cost, time-efficient diagram authoring.

Conclusion: Feynman provides a scalable solution for generating high-quality vision-language data, addressing the scarcity of knowledge-rich image-text pairs. The open-source release of dataset, benchmark, and pipeline will advance multi-modal AI research and visual reasoning evaluation.
...
### [178] [FastDSAC: Unlocking the Potential of Maximum Entropy RL in High-Dimensional Humanoid Control](https://arxiv.org/abs/2603.12612)
*Jun Xue,Junze Wang,Xinming Zhang,Shanze Wang,Yanjun Chen,Wei Zhang*

Main category: cs.LG

TL;DR: FastDSAC enables effective maximum entropy RL for high-dimensional humanoid control by addressing exploration inefficiency and training instability through dimension-wise entropy modulation and distributional critics.


<details>
  <summary>Details</summary>
Motivation: Scaling maximum entropy RL to high-dimensional humanoid control is challenging due to the "curse of dimensionality" causing exploration inefficiency and training instability. Current approaches rely on deterministic policy gradients with massive parallel simulation, but the authors believe stochastic policies have untapped potential.

Method: FastDSAC framework with two key components: 1) Dimension-wise Entropy Modulation (DEM) to dynamically redistribute exploration budget and enforce diversity across action dimensions, and 2) a continuous distributional critic designed to ensure value fidelity and mitigate high-dimensional value overestimation.

Result: Extensive evaluations on HumanoidBench and other continuous control tasks show that FastDSAC consistently matches or outperforms deterministic baselines, achieving significant gains of 180% on Basketball and 400% on Balance Hard tasks.

Conclusion: Rigorously designed stochastic policies can effectively compete with or surpass deterministic approaches in complex continuous control, demonstrating that maximum entropy RL can be successfully scaled to high-dimensional humanoid control tasks.
...
### [179] [When Drafts Evolve: Speculative Decoding Meets Online Learning](https://arxiv.org/abs/2603.12617)
...
<details>
  <summary>Details</summary>
Motivation: Current speculative decoding suffers from limited draft model capacity leading to poor approximation of target distributions, resulting in short acceptance lengths and diminished speedup. The verification feedback inherent in speculative decoding provides free information about draft-target deviation that can be leveraged for continuous improvement.

Method: Proposes OnlineSpec framework that treats speculative decoding as an online learning problem. Uses dynamic regret minimization to link online learning performance with acceleration rate. Implements two key techniques: 1) Optimistic online learning that reuses historical gradients as predictive update hints, and 2) Online ensemble learning that dynamically maintains multiple draft models.

Result: Achieves up to 24% speedup over seven benchmarks and three foundation models. The framework provides theoretical justifications and improved acceleration rates compared to traditional speculative decoding approaches.

Conclusion: OnlineSpec successfully leverages the inherent verification feedback in speculative decoding to continuously evolve draft models through online learning, significantly improving inference acceleration while maintaining theoretical guarantees.
...
### [180] [Human-AI Collaborative Autonomous Experimentation With Proxy Modeling for Comparative Observation](https://arxiv.org/abs/2603.12618)
*Arpan Biswas,Hiroshi Funakubo,Yongtao Liu*

Main category: cs.LG

TL;DR: Proposes proxy-modelled Bayesian optimization (px-BO) that combines human expertise with AI through a voting system to guide material exploration, avoiding errors from purely data-driven objective functions.


<details>
  <summary>Details</summary>
Motivation: Traditional Bayesian optimization for material discovery uses purely data-driven objective functions that can be erroneous due to noisy high-dimensional data and may miss subtle physical features, potentially failing to discover unknown material phenomena.

Method: Introduces px-BO with human-AI teaming: human agents vote on preferred experimental outcomes, votes are transformed into proxy objective functions using Bradley-Terry model, and the proxy model acts as AI agent for surrogate votes with periodic human validation and correction.

Result: Demonstrated on simulated and BEPS data from PTO samples, showing better expert control and improved search compared to traditional data-driven exploration.

Conclusion: Human-AI teaming is essential for accelerated and meaningful material space exploration, with px-BO providing superior guidance over purely data-driven approaches.
...
### [181] [Spend Less, Reason Better: Budget-Aware Value Tree Search for LLM Agents](https://arxiv.org/abs/2603.12634)
*Yushu Li,Wenlong Deng,Jiajin Li,Xiaoxiao Li*

Main category: cs.LG

TL;DR: BAVT is a training-free inference-time framework that models multi-hop reasoning as a dynamic search tree with budget-aware node selection and residual value prediction to optimize LLM agent performance under strict resource constraints.


<details>
  <summary>Details</summary>
Motivation: Current test-time scaling approaches treat compute as abundant, allowing agents to waste resources on redundant steps or dead-end trajectories. Existing budget-aware methods require expensive fine-tuning or use coarse trajectory-level heuristics that cannot intervene mid-execution.

Method: BAVT models multi-hop reasoning as a dynamic search tree guided by step-level value estimation within a single LLM backbone. It uses budget-conditioned node selection with remaining resource ratio as a scaling exponent, and employs a residual value predictor that scores relative progress rather than absolute state quality to combat LLM overconfidence.

Result: BAVT consistently outperforms parallel sampling baselines on four multi-hop QA benchmarks across two model families. Under strict low-budget constraints, BAVT surpasses baseline performance at 4× the resource allocation, showing intelligent budget management fundamentally outperforms brute-force compute scaling.

Conclusion: BAVT provides a principled, training-free approach to budget-aware reasoning that enables efficient resource allocation through dynamic search tree optimization, with theoretical convergence guarantees and practical performance improvements over existing methods.
...
### [182] [LightMoE: Reducing Mixture-of-Experts Redundancy through Expert Replacing](https://arxiv.org/abs/2603.12645)
*Jiawei Hao,Zhiwei Hao,Jianyuan Guo,Li Shen,Yong Luo,Han Hu,Dan Zeng*

Main category: cs.LG

TL;DR: LightMoE is a novel expert compression framework that replaces redundant experts with parameter-efficient modules, achieving high compression ratios (30-50%) while maintaining or improving performance compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Mixture-of-Experts LLMs have memory constraints due to loading many expert modules. Existing compression methods (pruning/merging) cause irreversible knowledge loss or high training overhead.

Method: Expert replacing paradigm: replace redundant experts with parameter-efficient modules and recover capabilities with low training costs. LightMoE enhances this with adaptive expert selection, hierarchical expert construction, and annealed recovery strategy.

Result: Matches LoRA fine-tuning performance at 30% compression ratio. At 50% compression, outperforms existing methods with 5.6% average improvement across five diverse tasks.

Conclusion: LightMoE achieves superior balance among memory efficiency, training efficiency, and model performance for MoE-based LLMs.
...
### [183] [Adaptive Diffusion Posterior Sampling for Data and Model Fusion of Complex Nonlinear Dynamical Systems](https://arxiv.org/abs/2603.12635)
*Dibyajyoti Chakraborty,Hojin Kim,Romit Maulik*

Main category: cs.LG

TL;DR: A generative diffusion model framework for probabilistic forecasting of turbulent flows with adaptive sensor placement and data assimilation capabilities.


<details>
  <summary>Details</summary>
Motivation: High-fidelity simulations of chaotic, high-dimensional nonlinear systems are computationally expensive, and existing deterministic surrogate models fail to capture the intrinsic distributional uncertainty of chaotic systems.

Method: Multi-step autoregressive diffusion objective for long-rollout stability, multi-scale graph transformer architecture with diffusion preconditioning and voxel-grid pooling for complex geometries, and diffusion posterior sampling for data assimilation without retraining.
...
Conclusion: The framework provides a unified platform for probabilistic forecasting, uncertainty-aware sensor placement, and data assimilation in chaotic turbulent flows using generative diffusion models.
...
### [184] [RetroReasoner: A Reasoning LLM for Strategic Retrosynthesis Prediction](https://arxiv.org/abs/2603.12666)
*Hanbum Ko,Chanhui Lee,Ye Rin Kim,Rodrigo Hormazabal,Sehui Han,Sungbin Lim,Sungwoong Kim*

Main category: cs.LG

TL;DR: RetroReasoner is a retrosynthesis prediction model that mimics chemists' strategic thinking by explicitly reasoning about bond-disconnection strategies, trained with supervised fine-tuning and reinforcement learning, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Traditional retrosynthesis requires expert knowledge and is time-consuming. Existing molecular LLMs either predict reactants without strategic reasoning or conduct only generic product analysis, lacking explicit reasoning about bond-disconnection strategies that logically lead to specific reactant choices.

Method: Proposes RetroReasoner with two-stage training: 1) Supervised fine-tuning using SyntheticRetro framework that generates structured disconnection rationales alongside reactant predictions; 2) Reinforcement learning using round-trip accuracy as reward, where predicted reactants are passed through a forward synthesis model and rewarded when the forward-predicted product matches the original input.

Result: RetroReasoner outperforms prior baselines and generates a broader range of feasible reactant proposals, particularly excelling at handling more challenging reaction instances.

Conclusion: The proposed RetroReasoner successfully incorporates chemists' strategic thinking into retrosynthesis prediction through explicit bond-disconnection reasoning and dual training approach, demonstrating superior performance and broader applicability.
...
### [185] [Sobolev--Ricci Curvature](https://arxiv.org/abs/2603.12652)
*Kyoichi Iwasaki,Tam Le,Hideitsu Hino*

Main category: cs.LG

TL;DR: Sobolev-Ricci Curvature (SRC) is a new graph Ricci curvature based on Sobolev transport geometry that enables efficient computation and serves as a reusable primitive for curvature-driven graph transformations.


<details>
  <summary>Details</summary>
Motivation: Existing graph Ricci curvatures like Ollivier-Ricci Curvature (ORC) have computational limitations. The authors aim to develop a more practical, scalable curvature measure that maintains consistency with classical transport curvature while enabling efficient evaluation.

Method: Propose Sobolev-Ricci Curvature (SRC) induced by Sobolev transport geometry, which leverages a tree-metric Sobolev structure on neighborhood measures for efficient evaluation. Validate SRC through consistency proofs with classical curvature and demonstrate applications in two pipelines: Sobolev-Ricci Flow (reweighting) and curvature-guided edge pruning.

Result: SRC shows two key consistency properties: (1) recovers ORC on trees with length measure in W1 setting, (2) vanishes in Dirac limit matching flat case of measure-theoretic Ricci curvature. SRC enables scalable curvature-driven graph transformation and manifold-preserving pruning.

Conclusion: SRC provides a transport-based foundation for practical curvature analysis on graphs, offering computational efficiency while maintaining theoretical consistency with classical curvature concepts, making it suitable for scalable network geometry applications.
...
### [186] [PhysMoDPO: Physically-Plausible Humanoid Motion with Preference Optimization](https://arxiv.org/abs/2603.13228)
*Yangsong Zhang,Anujith Muraleedharan,Rikhat Akizhanov,Abdul Ahad Butt,Gül Varol,Pascal Fua,Fabio Pizzati,Ivan Laptev*

Main category: cs.LG

TL;DR: PhysMoDPO: Direct Preference Optimization framework that integrates Whole-Body Controller into training to optimize diffusion models for physics-compliant text-to-motion generation and robot control.


<details>
  <summary>Details</summary>
Motivation: Current methods using Whole-Body Controllers to convert diffusion-generated motions into executable robot trajectories often produce substantial deviations from original motion, compromising both physics compliance and task fidelity.

Method: Proposes PhysMoDPO, a Direct Preference Optimization framework that integrates WBC into training pipeline, uses physics-based and task-specific rewards to assign preferences to synthesized trajectories, and optimizes diffusion models for WBC-compatible outputs.

Result: Demonstrates consistent improvements in physical realism and task-related metrics on simulated robots, shows significant improvements for zero-shot motion transfer in simulation, and enables successful real-world deployment on a G1 humanoid robot.

Conclusion: PhysMoDPO effectively bridges the gap between diffusion-based motion generation and physics-compliant robot control by directly optimizing for WBC-compatible outputs, outperforming heuristic-based approaches and enabling practical real-world applications.
...
### [187] [Federated Hierarchical Clustering with Automatic Selection of Optimal Cluster Numbers](https://arxiv.org/abs/2603.12684)
*Yue Zhang,Chuanlong Qiu,Xinfa Liao,Yiqun Zhang*

Main category: cs.LG

TL;DR: Fed-$k^*$-HC is a federated clustering framework that automatically determines optimal cluster count $k^*$ using hierarchical clustering with density-based merging, handling unknown cluster numbers and imbalanced sizes.


<details>
  <summary>Details</summary>
Motivation: Existing federated clustering methods assume known number of uniformly sized clusters, but real scenarios have unknown cluster counts and naturally imbalanced sizes. Privacy constraints in federated learning reduce usable information, making robust FC challenging.

Method: Clients generate micro-subclusters and upload prototypes to server. Server performs hierarchical merging using density-based approach that explores varying cluster shapes/sizes. Progressive merging self-terminates based on prototype neighboring relationships to determine optimal $k^*$.

Result: Extensive experiments on diverse datasets demonstrate Fed-$k^*$-HC's capability to accurately explore proper number of clusters in federated settings.

Conclusion: Fed-$k^*$-HC provides a novel solution for federated clustering that automatically determines optimal cluster count while handling real-world challenges of unknown cluster numbers and imbalanced sizes.
...
### [188] [Disentangled Latent Dynamics Manifold Fusion for Solving Parameterized PDEs](https://arxiv.org/abs/2603.12676)
*Zhangyong Liang,Ji Zhang*

Main category: cs.LG

TL;DR: DLDMF is a physics-informed framework that disentangles space, time, and parameters for neural PDE surrogate modeling, enabling simultaneous parameter generalization and temporal extrapolation through latent dynamics evolution.


<details>
  <summary>Details</summary>
Motivation: Existing neural surrogate models struggle with both parameter generalization and temporal extrapolation simultaneously. Parameterized models treat time as just another input and fail to capture intrinsic dynamics, while continuous-time latent methods require expensive test-time auto-decoding for each instance, disrupting continuity across parameter space.

Method: DLDMF explicitly separates space, time, and parameters using a feed-forward network to map PDE parameters to continuous latent embeddings. These embeddings initialize and condition a latent state evolved by a parameter-conditioned Neural ODE. A dynamic manifold fusion mechanism combines spatial coordinates, parameter embeddings, and time-evolving latent states through a shared decoder to reconstruct spatiotemporal solutions.

Result: DLDMF consistently outperforms state-of-the-art baselines in accuracy, parameter generalization, and extrapolation robustness across several benchmark problems. It reduces interference between parameter variation and temporal evolution while preserving smooth solution manifolds.

Conclusion: By modeling prediction as latent dynamic evolution rather than static coordinate fitting, DLDMF successfully handles both unseen parameter settings and long-term temporal extrapolation, addressing key limitations in current neural PDE surrogate methods.
...
### [189] [Cost-Efficient Multimodal LLM Inference via Cross-Tier GPU Heterogeneity](https://arxiv.org/abs/2603.12707)
*Donglin Yu*

Main category: cs.LG

TL;DR: MLLM inference has compute-bound vision encoding and memory-bandwidth-bound language generation phases. Partitioning at the modality boundary minimizes cross-device transfer compared to stage-level disaggregation, enabling cost-effective heterogeneous deployment over commodity PCIe.


<details>
  <summary>Details</summary>
Motivation: Current MLLM serving systems face hardware mismatch: vision encoding is compute-intensive while language generation is memory-bandwidth-intensive. Stage-level disaggregation requires expensive high-bandwidth interconnects (NVLink) and incurs large KV cache transfers, making heterogeneous deployment impractical.

Method: Propose modality-level partitioning at the boundary between vision encoder and language model, reducing transfer from O(L*s_ctx) to O(N_v*d) bytes. Develop a closed-form cost model for phase-separable workloads, and build HeteroServe runtime with modality-level partitioning and cross-tier scheduling.

Result: Modality-level partitioning reduces cross-device transfer by O(L) factor (L=transformer depth). Heterogeneous deployment achieves 31.4% predicted cost savings (40.6% observed). HeteroServe improves throughput by up to 54% on identical hardware, and achieves 37% better Tokens/$ with 40% lower cluster cost.

Conclusion: Modality-level disaggregation enables cost-effective heterogeneous MLLM serving over commodity PCIe by aligning hardware with computational phases. This approach outperforms stage-level disaggregation and enables significant cost savings without latency degradation.
...
### [190] [RXNRECer Enables Fine-grained Enzymatic Function Annotation through Active Learning and Protein Language Models](https://arxiv.org/abs/2603.12694)
*Zhenkun Shi,Jun Zhu,Dehang Wang,BoYu Chen,Qianqian Yuan,Zhitao Mao,Fan Wei,Weining Wu,Xiaoping Liao,Hongwu Ma*

Main category: cs.LG

TL;DR: RXNRECer is a transformer-based ensemble framework that directly predicts enzyme-catalyzed reactions without using EC numbers as intermediaries, outperforming EC-based methods by 16.54% in F1 score and 15.43% in accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing enzyme annotation methods rely on EC numbers as intermediaries, which introduces ambiguity due to complex many-to-many mappings among proteins, EC numbers, and reactions, plus issues with EC number updates and database inconsistencies.

Method: Transformer-based ensemble framework integrating protein language modeling and active learning to capture both high-level sequence semantics and fine-grained transformation patterns, directly predicting reactions without EC numbers.

Result: Consistent improvements over six EC-based baselines on curated cross-validation and temporal test sets, with 16.54% F1 score gain and 15.43% accuracy gain. Enables scalable proteome-wide annotation, refined reaction specificity, annotation of uncurated proteins, and identification of enzyme promiscuity.

Conclusion: RXNRECer provides a robust, EC-free solution for fine-grained enzyme function prediction with interpretable rationales using large language models, offering advantages for enzyme research and industrial applications.
...
### [191] [SciDesignBench: Benchmarking and Improving Language Models for Scientific Inverse Design](https://arxiv.org/abs/2603.12724)
*David van Dijk,Ivan Vrkic*

Main category: cs.LG

TL;DR: SciDesignBench is a new benchmark with 520 simulator-grounded inverse design tasks across 14 scientific domains, showing current models struggle (29% success) and that different models excel in different settings (de novo vs refinement vs seed-based design).


<details>
  <summary>Details</summary>
Motivation: Inverse problems are fundamental in science and engineering - finding designs that achieve desired outcomes. While evaluating candidates is often routine, searching combinatorial design spaces is much harder. There's a need for benchmarks to measure progress in scientific reasoning and practical methods for amortizing expensive simulator computations.

Method: Created SciDesignBench with 520 tasks across 14 domains and 5 settings (single-shot design, short/long-horizon feedback, seed-design optimization). Evaluated zero-shot models, analyzed performance with simulator feedback, and introduced RLSF (simulator-feedback training recipe) to improve models through training.

Result: Best zero-shot model achieves only 29% success on 10-domain subset. Different models excel in different settings: Sonnet 4.5 best for one-turn de novo design, Opus 4.6 best after 20 turns of refinement. Seed designs reshuffle leaderboard, showing constrained modification requires different capabilities. RLSF-tuned 8B model improves single-turn success by 8-17 percentage points across three domains.

Conclusion: Simulator-grounded inverse design serves as both a benchmark for scientific reasoning and a practical approach for amortizing expensive test-time compute into model weights. Different capabilities are needed for different design settings, and targeted training (RLSF) can significantly improve performance.
...
TL;DR: In-context operator learning outperforms classical operator learning on complex spatiotemporal tasks, generalizing across domains and scaling from few to many examples.


<details>
  <summary>Details</summary>
Motivation: Prior work lacks systematic comparison between in-context operator learning (learning from contextual examples) and classical single-operator learning using identical training data, creating a research gap.

Method: Proposed GICON (Graph In-Context Operator Network) combining graph message passing for geometric generalization with example-aware positional encoding for cardinality generalization. Conducted controlled experiments comparing both approaches under same training steps and dataset.

Result: In-context operator learning outperforms classical operator learning on complex tasks (air quality prediction across two Chinese regions), generalizing across spatial domains and scaling robustly from few training examples to 100 at inference.

Conclusion: In-context operator learning is more effective than classical operator learning for complex spatiotemporal systems, demonstrating better generalization and scalability with the proposed GICON architecture.
...
TL;DR: Current ATP systems show 26% performance drop when moving from standard MathLib definitions to bespoke mathematical constructions, revealing limited generalization across definitional frameworks.


<details>
  <summary>Details</summary>
Motivation: ATP benchmarks are heavily biased toward MathLib's definitional framework, but frontier mathematics often uses bespoke constructions that deviate from standard libraries. The paper aims to evaluate ATP robustness when applied to novel definitional frameworks.

Method: Introduces TaoBench, an undergraduate-level benchmark derived from Terence Tao's Analysis I that formalizes analysis from scratch without MathLib definitions. Creates paired TaoBench-Mathlib statements for direct comparison, and builds an agentic pipeline for compilable, self-contained local environments.

Result: State-of-the-art ATP models perform capably within MathLib framework but show average 26% performance drop on definitionally equivalent Tao formulation, indicating the bottleneck is limited generalization across definitional frameworks rather than task difficulty.

Conclusion: TaoBench highlights a gap between benchmark performance and real-world applicability, providing a foundation for developing provers better aligned with research mathematics that often uses bespoke constructions.
...
### [194] [Upper Bounds for Local Learning Coefficients of Three-Layer Neural Networks](https://arxiv.org/abs/2603.12785)
*Yuki Kurumadani*

Main category: cs.LG

TL;DR: Derives an upper-bound formula for the local learning coefficient at singular points in three-layer neural networks, applicable to general analytic activation functions including swish and polynomial functions.


<details>
  <summary>Details</summary>
Motivation: Existing methods for evaluating learning coefficients in neural networks are limited, and current formulas for semiregular models only apply to nonsingular points, leading to substantial discrepancies from known values in some cases.

Method: Derives an upper-bound formula for the local learning coefficient at singular points in three-layer neural networks that can be interpreted as a counting rule under budget constraints and demand-supply constraints.

Result: The formula applies to general analytic activation functions (including swish and polynomial functions). When input dimension is one, the upper bound coincides with known learning coefficient values, partially resolving previous discrepancies.

Conclusion: Provides a systematic perspective on how weight parameters affect learning coefficients in three-layer neural networks and extends previous results to a wider class of activation functions.
...
### [195] [A Fractional Fox H-Function Kernel for Support Vector Machines: Robust Classification via Weighted Transmutation Operators](https://arxiv.org/abs/2603.12794)
*Gustavo Dorrego*

Main category: cs.LG

TL;DR: Proposes Fox-Dorrego kernel, a novel non-stationary kernel derived from fractional diffusion-wave equations, outperforming Gaussian RBF by reducing classification error by ~50% with better outlier robustness.


<details>
  <summary>Details</summary>
Motivation: Standard Gaussian RBF kernels are highly susceptible to structural noise and outliers due to exponential decay, leading to severe overfitting in complex datasets. There's a need for more robust kernel functions that can handle outliers better.

Method: Derives a novel class of non-stationary kernels from the fundamental solution of generalized time-space fractional diffusion-wave equations. Uses structure-preserving transmutation method over Weighted Sobolev Spaces to create the Fox-Dorrego Kernel, an exact analytical Mercer kernel governed by Fox H-function. Incorporates aging weight function ("Amnesia Effect") to penalize distant outliers and fractional asymptotic power-law decay for robust, heavy-tailed feature mapping.
...
Conclusion: The proposed Fox-Dorrego kernel provides a superior alternative to Gaussian RBF for SVM applications, offering better outlier robustness and significantly improved classification performance through its non-stationary, heavy-tailed characteristics derived from fractional diffusion theory.
...
### [196] [A Multi-task Large Reasoning Model for Molecular Science](https://arxiv.org/abs/2603.12808)
*Pengfei Liu,Shuang Ge,Jun Tao,Zhixiang Ren*

Main category: cs.LG

TL;DR: A multi-task large reasoning model for molecular science that integrates scientific logic with deep learning, achieving 50.3% average improvement over base architecture while using fewer resources than large foundation models.


<details>
  <summary>Details</summary>
Motivation: Current AI for molecular science needs to shift from purely data-driven predictions to knowledge-guided computational reasoning. Existing models are proprietary, lack general molecular intelligence, and don't integrate scientific logic with deep learning effectively.

Method: Multi-task large reasoning model with multi-specialist modules for molecular expertise, and a chain-of-thought framework enhanced by reinforcement learning infused with molecular knowledge for structured reflective reasoning.

Result: 50.3% average improvement over base architecture across 10 molecular tasks and 47 metrics, outperforming 20+ state-of-the-art baselines including ultra-large foundation models, despite using significantly fewer training data and computational resources.
...
### [197] [Residual SODAP: Residual Self-Organizing Domain-Adaptive Prompting with Structural Knowledge Preservation for Continual Learning](https://arxiv.org/abs/2603.12816)
*Gyutae Oh,Jungwoo Bae,Jitae Shin*

Main category: cs.LG

TL;DR: Residual SODAP improves continual learning by combining prompt-based representation adaptation with classifier-level knowledge preservation, achieving SOTA results on domain-incremental learning benchmarks without task IDs or extra data storage.


<details>
  <summary>Details</summary>
Motivation: Continual learning suffers from catastrophic forgetting, especially in domain-incremental learning where task identifiers are unavailable and storing past data is infeasible. Prompt-based CL alone is insufficient due to suboptimal prompt selection and classifier-level instability under domain shifts.

Method: Proposes Residual SODAP framework that jointly performs prompt-based representation adaptation and classifier-level knowledge preservation. Combines α-entmax sparse prompt selection with residual aggregation, data-free distillation with pseudo-feature replay, prompt-usage-based drift detection, and uncertainty-aware multi-loss balancing.

Result: Achieves state-of-the-art AvgACC/AvgF of 0.850/0.047 (DR), 0.760/0.031 (Skin Cancer), and 0.995/0.003 (CORe50) across three DIL benchmarks without task IDs or extra data storage.

Conclusion: Residual SODAP effectively addresses catastrophic forgetting in domain-incremental learning by combining prompt-based representation adaptation with classifier-level knowledge preservation, outperforming existing methods on multiple benchmarks.
...
### [198] [Hierarchical Reference Sets for Robust Unsupervised Detection of Scattered and Clustered Outliers](https://arxiv.org/abs/2603.12847)
*Yiqun Zhang,Zexi Tan,Xiaopeng Luo,Yunlin Liu*

Main category: cs.LG

TL;DR: A novel graph-based outlier detection method for IoT data that handles both scattered and clustered outliers by leveraging natural neighboring relationships for multi-perspective anomaly evaluation.


<details>
  <summary>Details</summary>
Motivation: IoT data analysis tasks are mostly unsupervised and vulnerable to outliers. Beyond scattered outliers from faulty sensors, IoT systems often have clustered outliers (micro-clusters) from localized interference, security threats, or regional false alarms. These clustered outliers can be mistaken for normal behavior due to their high local density, obscuring detection of both scattered and contextual anomalies.

Method: Proposes a graph-based outlier detection paradigm that leverages natural neighboring relationships using graph structures. The approach enables multi-perspective anomaly evaluation by incorporating reference sets at both local and global scales derived from the graph. This helps recognize scattered outliers without interference from clustered anomalies while simultaneously reflecting and isolating clustered outlier groups.

Result: Extensive experiments demonstrate the method's efficacy, including comparative performance analysis, ablation studies, validation on downstream clustering tasks, and evaluation of hyperparameter sensitivity. The source code is publicly available.

Conclusion: The proposed graph-based approach effectively addresses the challenge of detecting both scattered and clustered outliers in IoT data by leveraging natural neighboring relationships for multi-perspective evaluation, outperforming existing methods and showing practical utility in real-world IoT applications.
...
### [199] [On Linear Separability of the MNIST Handwritten Digits Dataset](https://arxiv.org/abs/2603.12850)
*Ákos Hajnal*

Main category: cs.LG

TL;DR: The paper investigates whether the MNIST dataset is linearly separable, addressing conflicting claims by systematically examining pairwise and one-vs-rest separability across training, test, and combined sets.


<details>
  <summary>Details</summary>
Motivation: Despite MNIST being a fundamental benchmark dataset with long history and relative simplicity, there are conflicting claims about whether it's linearly separable, which has never been fully answered. Linear separability is a key concept in statistical and machine learning techniques.

Method: Comprehensive empirical investigation distinguishing pairwise and one-vs-rest separation of training, test, and combined sets. Reviews theoretical approaches to assessing linear separability alongside state-of-the-art methods and tools, then systematically examines all relevant assemblies.

Result: The paper reports findings from systematic examination of all relevant assemblies (though specific results are not provided in the abstract).

Conclusion: The paper aims to provide a definitive answer to the long-standing question of MNIST's linear separability through comprehensive empirical investigation.
...
### [200] [Test-time RL alignment exposes task familiarity artifacts in LLM benchmarks](https://arxiv.org/abs/2603.12875)
*Kun Wang,Reinhard Heckel*

Main category: cs.LG

TL;DR: Test-time RL alignment method for LLM evaluation that controls for task familiarity without needing task-specific training data, revealing that performance gaps often reflect task familiarity rather than true capability differences.


<details>
  <summary>Details</summary>
Motivation: Direct benchmark evaluation of LLMs can be misleading because strong performance may reflect task familiarity rather than true capability. Existing train-before-test methods require task-specific training data which is often unavailable, and results vary with data choice.

Method: Two-stage test-time reinforcement learning alignment: 1) RL with single sample aligns model to task format, 2) Test-time RL with majority-voting reward aligns model to benchmark distribution. This replaces SFT-based train-before-test without needing task-specific training data.

Result: Test-time RL alignment performs similarly to SFT-based train-before-test but without training data. On domain-specific benchmarks without training data, direct evaluation underestimates base models which perform substantially better after alignment. For reasoning tasks, performance gaps between fine-tuned and base models largely disappear after alignment.

Conclusion: Many reported gains from RLVR/SFT in literature are artifacts of task familiarity rather than differences in reasoning capability. The proposed test-time RL alignment provides more faithful evaluation of LLM capabilities by controlling for task familiarity without requiring task-specific training data.
...
### [201] [Enhanced Drug-drug Interaction Prediction Using Adaptive Knowledge Integration](https://arxiv.org/abs/2603.12885)
*Pengfei Liu,Jun Tao,Zhixiang Ren*

Main category: cs.LG

TL;DR: Proposed knowledge augmentation framework using reinforcement learning to adaptively infuse prior drug knowledge into LLMs for improved drug-drug interaction event prediction with few-shot learning.


<details>
  <summary>Details</summary>
Motivation: Existing DDIE prediction methods struggle with imbalanced datasets, complex interaction mechanisms, and poor generalization to unknown drug combinations, necessitating better approaches.

Method: Knowledge augmentation framework that uses reinforcement learning to adaptively extract and synthesize prior drug knowledge, optimizing strategy space to enhance LLM accuracy for DDIE predictions.

Result: Achieved notable improvement over baseline through few-shot learning, establishing effective framework for scientific knowledge learning in DDIE predictions.

Conclusion: The proposed reinforcement learning-based knowledge augmentation framework successfully enhances LLM performance for DDIE prediction, addressing key challenges in the field.
...
### [202] [DirPA: Addressing Prior Shift in Imbalanced Few-shot Crop-type Classification](https://arxiv.org/abs/2603.12905)
*Joana Reuss,Ekaterina Gikalo,Marco Körner*

Main category: cs.LG

TL;DR: Dirichlet Prior Augmentation (DirPA) is extended to multiple EU countries, showing effectiveness in improving robustness and stabilizing training for agricultural monitoring under long-tailed distributions.


<details>
  <summary>Details</summary>
Motivation: Real-world agricultural monitoring faces severe class imbalance and high labeling costs, creating a disconnect between artificially balanced few-shot learning training and natural long-tailed distributions, leading to poor generalization.

Method: Extending Dirichlet Prior Augmentation (DirPA) approach across multiple European Union countries to test resilience across diverse agricultural environments, moving beyond localized experiments.

Result: DirPA demonstrates effectiveness across different geographical regions, improving system robustness, stabilizing training under extreme long-tailed distributions, and substantially improving individual class-specific performance.

Conclusion: DirPA successfully addresses the distribution shift problem in agricultural monitoring by proactively simulating priors, making it a robust solution for real-world few-shot learning applications across diverse geographical regions.
...
TL;DR: AxonAD detects multivariate time series anomalies by predicting attention query evolution and combining reconstruction error with query mismatch scores, outperforming baselines on autonomous driving telemetry.


<details>
  <summary>Details</summary>
Motivation: Multivariate time series anomalies often involve shifts in cross-channel dependencies rather than amplitude changes. Traditional residual-based detectors can miss these when sequence models still reconstruct signals plausibly despite altered coordination between channels.

Method: AxonAD uses multi-head attention query evolution as a short-horizon predictable process. It combines a gradient-updated reconstruction pathway with a history-only predictor that forecasts future query vectors from past context, trained via masked predictor-target objective against an EMA target encoder. At inference, reconstruction error is combined with tail-aggregated query mismatch score measuring cosine deviation between predicted and target queries.

Result: On proprietary in-vehicle telemetry with interval annotations and TSB-AD multi-variate suite (17 datasets, 180 series), AxonAD improves ranking quality and temporal localization over strong baselines using threshold-free and range-aware metrics.

Conclusion: AxonAD effectively detects structural dependency shifts in multivariate time series while retaining amplitude-level detection. Ablations confirm that query prediction and combined scoring are the primary drivers of performance gains.
...
### [204] [SCOPE: Semantic Coreset with Orthogonal Projection Embeddings for Federated learning](https://arxiv.org/abs/2603.12976)
*Md Anwar Hossen,Nathan R. Tallent,Luanzheng Guo,Ali Jannesary*

Main category: cs.LG

TL;DR: SCOPE is a federated learning coreset framework that uses semantic analysis to filter anomalies and prune redundant data, addressing class imbalance and communication efficiency with 128-512x bandwidth reduction.


<details>
  <summary>Details</summary>
Motivation: Scientific discovery requires federated learning on imbalanced datasets from high-resolution instruments, but current ML approaches either need impractical data aggregation or fail due to class imbalance, while existing coreset methods use local heuristics that are unaware of global data distribution.

Method: SCOPE analyzes latent space distribution to score each data point using: 1) representation score for core class feature reliability, 2) diversity score for novelty of orthogonal residuals, and 3) boundary proximity score for similarity to competing classes. It shares only scalar metrics with a federated server to build global consensus, then dynamically filters local noise and discards redundant samples.

Result: SCOPE achieves competitive global accuracy and robust convergence while delivering exceptional efficiency: 128x to 512x reduction in uplink bandwidth, 7.72x wall-clock acceleration, and reduced FLOP and VRAM footprints for local coreset selection.

Conclusion: SCOPE provides an effective solution for federated learning on imbalanced datasets by enabling communication-efficient coreset selection through semantic analysis and global consensus, overcoming limitations of existing methods while maintaining accuracy.
...
### [205] [Exact Federated Continual Unlearning for Ridge Heads on Frozen Foundation Models](https://arxiv.org/abs/2603.12977)
*Yijun Quan,Wentai Wu,Giovanni Montana*

Main category: cs.LG

TL;DR: Federated unlearning for frozen foundation models with ridge-regression heads using additive sufficient statistics and fixed-size messages to guarantee exact retrain-equivalence.


<details>
  <summary>Details</summary>
Motivation: Foundation models are deployed as frozen feature extractors in federated settings, but the "right to be forgotten" requires removing specific samples/users. Existing federated unlearning methods are approximate and costly for general deep models.

Method: Use frozen foundation model with ridge-regression head where exact optimum depends on two additive sufficient statistics. Create communication protocol supporting add/delete requests via fixed-size messages, maintaining head pointwise identical to centralized retraining.

Result: Method matches centralized ridge retraining to within 10^-9 relative Frobenius error, completes requests at orders-of-magnitude faster speed than retraining, with deterministic retrain-equivalence guarantees and zero KL divergence certificate.

Conclusion: Provides exact, efficient federated unlearning for frozen foundation models with ridge heads, addressing right to be forgotten with mathematical guarantees and practical efficiency.
...
### [206] [Retrieval-Enhanced Real Estate Appraisal](https://arxiv.org/abs/2603.12986)
*Simon Popelier,Matthieu X. B. Sarazin,Maximilien Bohm,Mathieu Gierski,Hanna Mergui,Matthieu Ospici,Adrien Bernhardt*

Main category: cs.LG

TL;DR: The paper proposes learning a selection policy for comparables in real estate appraisal instead of using fixed selection rules, using a hybrid vector-geographical retrieval module that improves comparable selection and enables more efficient models.


<details>
  <summary>Details</summary>
Motivation: The Sales Comparison Approach (SCA) is widely used in real estate appraisal but relies on selecting similar past transactions (comparables). Current methods impose fixed selection rules rather than learning optimal selection policies, which limits performance.

Method: A hybrid vector-geographical retrieval module that learns to select comparables by adapting to different datasets, optimized jointly with an estimation module. The approach learns selection policies instead of imposing fixed rules.

Result: The learned selection policy significantly improves comparable selection compared to state-of-the-art algorithms. Models using carefully selected comparables require fewer comparables and parameters while achieving performance close to state-of-the-art models. Evaluated on five datasets from the US, Brazil, and France.

Conclusion: Learning selection policies for comparables in real estate appraisal outperforms traditional fixed selection rules, enabling more efficient models with comparable performance using fewer comparables and parameters across diverse geographic datasets.
...
### [207] [Dependency-Aware Parallel Decoding via Attention for Diffusion LLMs](https://arxiv.org/abs/2603.12996)
*Bumjun Kim,Dongjae Jeon,Moongyu Jeon,Albert No*

Main category: cs.LG

TL;DR: DAPD enables parallel decoding for diffusion LLMs by using self-attention to create a dependency graph over masked tokens, then unmasking independent sets in parallel without retraining.


<details>
  <summary>Details</summary>
Motivation: Parallel decoding in diffusion LLMs is challenging because each denoising step only provides token-wise marginal distributions, while unmasking multiple tokens simultaneously requires handling inter-token dependencies.

Method: DAPD uses self-attention to induce a conditional dependency graph over masked tokens, where edges capture strong interactions and non-edges indicate weak dependence. It selects an independent set on this graph and unmask those tokens in parallel.

Result: Experiments on LLaDA and Dream show DAPD improves the accuracy-steps trade-off over existing methods and enables more globally distributed parallel updates that better exploit dLLMs' any-order generation capability.

Conclusion: DAPD provides a simple, training-free approach for parallel decoding in diffusion LLMs that effectively handles token dependencies through attention-based dependency graphs.
...
### [208] [Deconstructing the Failure of Ideal Noise Correction: A Three-Pillar Diagnosis](https://arxiv.org/abs/2603.12997)
*Chen Feng,Zhuo Zhi,Zhao Huang,Jiawei Ge,Ling Xiao,Nicu Sebe,Georgios Tzimiropoulos,Ioannis Patras*

Main category: cs.LG

TL;DR: Even with perfect noise transition matrix, correction methods still fail due to deeper flaws, not just estimation issues.


<details>
  <summary>Details</summary>
Motivation: To test the longstanding hypothesis that noise-correction methods would work perfectly if given an accurate noise transition matrix, and to understand why these theoretically grounded methods underperform compared to empirical approaches.

Method: Conducted experiments under idealized conditions with oracle transition matrix, analyzed macroscopic convergence states, microscopic optimization dynamics, and information-theoretic limits.

Result: Even with perfect transition matrix, correction methods still suffer from performance collapse during training, demonstrating the failure is not fundamentally a T-estimation problem but stems from deeper flaws.

Conclusion: The paper provides formal explanation for why ideal noise correction fails and offers guidance for designing more reliable methods for learning with noisy labels.
...
### [209] [PISmith: Reinforcement Learning-based Red Teaming for Prompt Injection Defenses](https://arxiv.org/abs/2603.13026)
*Chenlong Yin,Runpeng Geng,Yanting Wang,Jinyuan Jia*

Main category: cs.LG

TL;DR: PISmith is an RL-based red-teaming framework that systematically evaluates prompt injection defenses by training an attack LLM to optimize injected prompts in black-box settings, revealing vulnerabilities in state-of-the-art defenses.


<details>
  <summary>Details</summary>
Motivation: Prompt injection poses serious security risks to LLM applications, especially autonomous agents. Existing defenses lack robust evaluation against adaptive attacks, potentially creating false security.

Method: Uses reinforcement learning with adaptive entropy regularization and dynamic advantage weighting to sustain exploration and amplify learning from scarce successes when standard GRPO fails due to reward sparsity.

Result: Extensive evaluation on 13 benchmarks shows state-of-the-art prompt injection defenses remain vulnerable. PISmith outperforms 7 baselines across static, search-based, and RL-based attack categories with highest attack success rates.

Conclusion: PISmith demonstrates that current prompt injection defenses are insufficient against adaptive attacks, highlighting the need for more robust security measures in LLM applications.
...
### [210] [OpenACMv2: An Accuracy-Constrained Co-Optimization Framework for Approximate DCiM](https://arxiv.org/abs/2603.13042)
*Yiqi Zhou,Yue Yuan,Yikai Wang,Bohao Liu,Qinxin Mei,Zhuohua Liu,Shan Shen,Wei Xing,Daying Sun,Li Li,Guozhu Liu*

Main category: cs.LG

TL;DR: OpenACMv2 is an open framework for accuracy-constrained co-optimization of approximate digital compute-in-memory architectures, using two-level optimization to balance PPA and accuracy tradeoffs.


<details>
  <summary>Details</summary>
Motivation: Approximate digital compute-in-memory can improve power-performance-area (PPA) but requires careful co-optimization across architecture and transistor levels while maintaining accuracy constraints.

Method: Two-level optimization: (1) accuracy-constrained architecture search using GNN-based surrogate models for PPA and error, and (2) variation-aware transistor sizing with Monte Carlo for standard cells and SRAM bitcells.

Result: Demonstrates significant PPA improvements under controlled accuracy budgets, enabling rapid "what-if" exploration for approximate DCiM with robust convergence.

Conclusion: OpenACMv2 provides a reproducible framework for accuracy-constrained co-optimization of approximate DCiM, compatible with FreePDK45 and OpenROAD for easy adoption.
...
TL;DR: 3DTCR is a physics-based generative AI framework that reconstructs 3D tropical cyclone inner-core structures with high resolution and improved intensity forecasting, outperforming ECMWF-HRES while being computationally efficient.


<details>
  <summary>Details</summary>
Motivation: Current TC intensity forecasting methods have limitations: numerical/AI models fail to represent extreme TC structure adequately, time-series forecasting only outputs intensity sequences (not 3D structure), and high-resolution simulations are computationally expensive for operational use.

Method: 3DTCR combines physical constraints with generative AI efficiency using conditional Flow Matching (CFM). It's trained on a 6-year, 3-km-resolution WRF dataset, employs latent domain adaptation and two-stage transfer learning for region-adaptive vortex-following reconstruction.

Result: 3DTCR outperforms ECMWF-HRES in TC intensity prediction at nearly all lead times up to 5 days, reduces RMSE of maximum WS10M by 36.5% relative to FuXi inputs, and improves representation of TC inner-core structure while maintaining track stability.

Conclusion: 3DTCR offers a promising physics-based generative framework that efficiently resolves fine-scale TC structures at lower computational cost, potentially improving operational TC intensity forecasting.
...
TL;DR: A lightweight framework for predicting perturbation effects using bulk molecular data and structured biological knowledge, achieving competitive accuracy with simpler models than large foundation models.


<details>
  <summary>Details</summary>
Motivation: Current perturbation prediction methods rely on large-scale single-cell data and massive foundation models, which are often inaccessible in academic or clinical settings due to computational resource constraints.

Method: Leverages structured nature of biological interventions and specific inductive biases/invariances, using available perturbation effect information to generalize to novel contexts with widely-available bulk molecular data.

Result: Extensive testing shows accurate prediction of context-specific perturbation effects compared to real large-scale interventional experiments, competitive with state-of-the-art foundation models.

Conclusion: Accurate perturbation effect prediction is possible without proprietary hardware or very large models, enabling broader adoption of causal learning approaches in biomedicine with simpler data and smaller models.
...
TL;DR: The paper develops competition-aware CPC forecasting using semantic, behavioral, and geographic signals to approximate latent competition in paid search auctions, improving forecast stability and accuracy at business-relevant horizons.


<details>
  <summary>Details</summary>
Motivation: CPC in paid search is volatile and driven by partially observable competitive dynamics. Traditional forecasting methods lack visibility into competitive landscape shifts that affect CPC outcomes.

Method: Three complementary signals: (1) semantic neighborhoods from transformer-based keyword representations, (2) behavioral neighborhoods via DTW alignment of CPC trajectories, (3) geographic-intent covariates for localized demand. These are used as stand-alone covariates and relational priors in spatiotemporal graph forecasters.

Result: Competition-aware augmentation improves forecast stability and error profiles at medium/long horizons where competitive regimes shift. Semantic and geographic priors provide scalable approximation of latent competition.

Conclusion: Broad market-outcome coverage combined with keyword-derived semantic and geographic priors offers scalable approach to approximate latent competition and improve CPC forecasting in auction-driven markets.
...
### [214] [L2GTX: From Local to Global Time Series Explanations](https://arxiv.org/abs/2603.13065)
*Ephrem Tibebe Mekonnen,Luca Longo,Lucas Rizzo,Pierpaolo Dondio*

Main category: cs.LG

TL;DR: L2GTX is a model-agnostic framework that generates class-wise global explanations for time series classification by aggregating local explanations from representative instances, using temporal event primitives and importance clustering.


<details>
  <summary>Details</summary>
Motivation: Current XAI methods for time series have three key limitations: 1) model-agnostic methods from images/tabular data don't extend well to time series, 2) global explanation synthesis for time series is underexplored, and 3) most existing global approaches are model-specific. Understanding class-level decision behavior in time series classification remains challenging.

Method: L2GTX extracts clusters of parameterized temporal event primitives (trends, extrema) with importance scores from instance-level explanations using LOMATCE. It merges clusters across instances to reduce redundancy, uses an instance-cluster importance matrix to estimate global relevance, selects representative instances under a budget to maximize coverage of influential clusters, and aggregates events into class-wise global explanations.
...
Conclusion: L2GTX successfully addresses the limitations of existing approaches by providing a model-agnostic framework for generating class-wise global explanations for time series classification that respects temporal dependencies and identifies recurring patterns.
...
### [215] [GeoChemAD: Benchmarking Unsupervised Geochemical Anomaly Detection for Mineral Exploration](https://arxiv.org/abs/2603.13068)
*Yihao Ding,Yiran Zhang,Chris Gonzalez,Eun-Jung Holden,Wei Liu*

Main category: cs.LG

TL;DR: GeoChemAD is an open-source benchmark dataset for geochemical anomaly detection compiled from government surveys, with GeoChemFormer, a transformer-based framework using self-supervised pretraining, achieving state-of-the-art performance across diverse regions and sampling conditions.


<details>
  <summary>Details</summary>
Motivation: Current geochemical anomaly detection research suffers from limited generalizability due to single-region studies and lack of reproducibility due to proprietary datasets, hindering progress in mineral exploration.

Method: Created GeoChemAD dataset from government geological surveys covering multiple regions, sampling sources, and target elements. Developed GeoChemFormer, a transformer-based framework with self-supervised pretraining to learn target-element-aware geochemical representations for spatial samples.

Result: GeoChemFormer consistently achieves superior and robust performance across all eight dataset subsets, outperforming existing unsupervised methods in both anomaly detection accuracy and generalization capability.

Conclusion: The GeoChemAD dataset and GeoChemFormer framework provide a foundation for reproducible research and future development in geochemical anomaly detection, addressing key limitations of previous studies.
...
TL;DR: Linearized attention doesn't converge to its infinite-width NTK limit even at large widths due to spectral amplification, requiring impractical widths for convergence. This creates influence malleability that enables task alignment but also increases vulnerability to adversarial manipulation.


<details>
  <summary>Details</summary>
Motivation: To understand the theoretical foundations of attention mechanisms, which remain challenging due to their complex, non-linear dynamics, and to reveal fundamental trade-offs in their learning dynamics.

Method: Using a linearized attention mechanism with exact correspondence to a data-dependent Gram-induced kernel, with both empirical and theoretical analysis through the Neural Tangent Kernel (NTK) framework. Formal spectral amplification analysis shows the attention transformation cubes the Gram matrix's condition number.

Result: Linearized attention does not converge to its infinite-width NTK limit even at large widths, requiring width m = Ω(κ^6) for convergence (impractical for natural image datasets). Attention exhibits 6-9× higher influence malleability than ReLU networks, enabling task alignment but increasing vulnerability to adversarial manipulation.

Conclusion: Attention's power and vulnerability share a common origin in its departure from the kernel regime - its data-dependent kernel can reduce approximation error by aligning with task structure, but this same sensitivity increases susceptibility to adversarial manipulation of training data.
...
### [217] [Breaking the Tuning Barrier: Zero-Hyperparameters Yield Multi-Corner Analysis Via Learned Priors](https://arxiv.org/abs/2603.13092)
*Wei W. Xing,Kaiqi Huang,Jiazhan Liu,Hong Qiu,Shan Shen*

Main category: cs.LG

TL;DR: Foundation model with in-context learning replaces hyperparameter tuning for multi-corner circuit analysis, achieving state-of-the-art accuracy with zero tuning and 10x cost reduction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-corner circuit analysis face a fundamental trade-off: simple models are automated but fail on nonlinear circuits, while advanced AI models require extensive hyperparameter tuning per design iteration (the "Tuning Barrier"). Traditional yield analysis across 25+ Process-Voltage-Temperature corners has combinatorial simulation costs of O(K×N) where N exceeds 10^4 samples per corner.

Method: Replace engineered priors with learned priors from a foundation model pre-trained on millions of regression tasks. The model performs in-context learning, instantly adapting to each circuit without tuning or retraining. Uses attention mechanism to automatically transfer knowledge across corners by identifying shared circuit physics between operating conditions. Includes automated feature selector that reduces dimensions from 1152D to 48D.

Result: Matches state-of-the-art accuracy with mean MREs as low as 0.11% while requiring zero tuning. Reduces total validation cost by over 10× compared to existing methods.

Conclusion: The foundation model approach breaks the "Tuning Barrier" in multi-corner circuit analysis by eliminating hyperparameter tuning while maintaining high accuracy, enabling efficient validation across numerous process corners without manual intervention.
...
### [218] [BoSS: A Best-of-Strategies Selector as an Oracle for Deep Active Learning](https://arxiv.org/abs/2603.13109)
...
<details>
  <summary>Details</summary>
Motivation: Existing active learning strategies lack robustness across different models, budgets, and datasets, and current oracle strategies don't scale to large datasets and complex neural networks. There's a need for a reliable reference point to evaluate AL methods.

Method: BoSS constructs candidate batches through an ensemble of selection strategies, then selects the batch yielding the highest performance gain. It's designed to be extensible with new state-of-the-art strategies as they emerge.

Result: BoSS outperforms existing oracle strategies; state-of-the-art AL strategies still fall short of oracle performance especially in large-scale datasets with many classes; ensemble-based approaches may help address inconsistent AL performance.

Conclusion: BoSS provides a scalable, extensible oracle strategy for evaluating AL methods, revealing significant gaps in current approaches and suggesting ensemble methods as a promising direction for improving AL robustness.
...
### [219] [ZO-SAM: Zero-Order Sharpness-Aware Minimization for Efficient Sparse Training](https://arxiv.org/abs/2603.13115)
*Jie Ji,Gen Li,Kaiyuan Deng,Fatemeh Afghah,Xiaolong Ma*

Main category: cs.LG

TL;DR: ZO-SAM combines zero-order optimization with Sharpness-Aware Minimization to reduce computational costs by half while stabilizing sparse neural network training and improving generalization.


<details>
  <summary>Details</summary>
Motivation: Deep learning models have high computational costs and memory requirements, limiting deployment in resource-constrained environments. Sparse neural networks help but existing sparse training methods suffer from chaotic/noisy gradients that hinder convergence, especially at high sparsity levels.

Method: Proposes Zero-Order Sharpness-Aware Minimization (ZO-SAM) that integrates zero-order optimization within SAM framework. Uses only single backpropagation step during perturbation with selective zero-order gradient estimations, reducing backpropagation cost by half compared to traditional SAM.

Result: Reduces gradient variance, eliminates computational overhead, stabilizes training, accelerates convergence. Particularly effective for sparse training where computational cost is bottleneck. Models show improved robustness under distribution shift.

Conclusion: ZO-SAM provides efficient optimization framework for sparse neural networks, reducing computational costs while maintaining SAM's benefits of finding flat minima and improving generalization/robustness, making it practical for real-world deployments.
...
### [220] [MXNorm: Reusing MXFP block scales for efficient tensor normalisation](https://arxiv.org/abs/2603.13180)
*Callum McLean,Luke Y. Prince,Alexandre Payot,Paul Balança,Carlo Luschi*

Main category: cs.LG

TL;DR: MXNorm is a drop-in replacement for RMSNorm that uses MXFP8 block scales to estimate RMS, reducing normalization computation by 32x while maintaining accuracy in Llama 3 training.


<details>
  <summary>Details</summary>
Motivation: Matrix multiplication performance improvements have far outpaced improvements in reduction and elementwise computations, which still use higher precision. This creates a bottleneck where normalization operations (like RMSNorm) become relatively more expensive compared to matmuls.

Method: MXNorm estimates the root mean square (RMS) using only the block scales calculated as part of the MXFP8 cast, enabling a 32x reduction in the size of reduction needed for normalization. It's designed as a drop-in replacement for RMSNorm in transformer architectures.

Result: Validation on Llama 3 models (125M, 1B, 8B parameters) shows minimal training accuracy loss compared to RMSNorm baseline with MXFP8 matmuls. Practical kernel speedups of up to 2.4x using torch.compile, corresponding to 1.3% speedup in Llama 3 8B transformer layers in MXFP8 and 2.6% speedup in NVFP4.

Conclusion: MXNorm effectively addresses the performance bottleneck in normalization computations by leveraging existing MXFP8 block scales, providing significant speedups while maintaining model accuracy, making it a practical optimization for low-precision deep learning workloads.
...
### [221] [Learnability and Privacy Vulnerability are Entangled in a Few Critical Weights](https://arxiv.org/abs/2603.13186)
*Xingli Fang,Jung-Eun Kim*

Main category: cs.LG

TL;DR: Proposes a targeted weight rewinding method for membership privacy that only modifies critical vulnerable weights instead of retraining entire networks, achieving better privacy-utility trade-off.


<details>
  <summary>Details</summary>
Motivation: Traditional membership privacy methods require full network retraining which is costly, causes unnecessary utility loss, and can create prediction misalignment between training and non-training data.

Method: Identifies privacy-vulnerable weights (small fraction of total), scores critical weights, and rewinds only those specific weights for fine-tuning rather than discarding entire neurons or retraining all weights.

Result: The method shows superior resilience against Membership Inference Attacks in most cases while maintaining model utility, demonstrating better privacy-utility trade-off than full retraining approaches.

Conclusion: Targeted weight rewinding based on weight importance (location rather than value) provides an effective and efficient approach to membership privacy preservation with minimal utility impact.
...
### [222] [Representation Learning for Spatiotemporal Physical Systems](https://arxiv.org/abs/2603.13227)
*Helen Qu,Rudy Morel,Michael McCabe,Alberto Bietti,François Lanusse,Shirley Ho,Yann LeCun*

Main category: cs.LG

TL;DR: Self-supervised learning methods for physics systems are evaluated not on next-frame prediction but on downstream scientific tasks like parameter estimation, revealing that latent-space methods outperform pixel-level prediction approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional ML approaches to physical systems focus on next-frame prediction, which is computationally expensive and suffers from compounding errors. The authors want to evaluate models on more meaningful downstream scientific tasks that better reflect physical relevance.

Method: Evaluate general-purpose self-supervised methods on their ability to learn physics-grounded representations useful for downstream tasks like physical parameter estimation. Compare different approaches including latent-space methods (JEPAs) vs pixel-level prediction methods.

Result: Not all physical modeling methods outperform generic self-supervised methods on downstream tasks. Methods that learn in latent space (e.g., JEPAs) outperform those optimizing pixel-level prediction objectives.

Conclusion: Downstream scientific tasks provide better evaluation of physical relevance than next-frame prediction. Latent-space learning approaches show superior performance for physics-grounded representation learning.
...
### [223] [Unmasking Biases and Reliability Concerns in Convolutional Neural Networks Analysis of Cancer Pathology Images](https://arxiv.org/abs/2603.12445)
*Michael Okonoda,Eder Martinez,Abhilekha Dalal,Lior Shamir*

Main category: eess.IV

TL;DR: CNNs achieve high accuracy on cancer pathology datasets even when trained on non-clinical background segments, revealing evaluation biases in standard ML practices.


<details>
  <summary>Details</summary>
Motivation: To investigate the soundness of standard CNN evaluation practices in cancer pathology, given the opaque nature of CNNs and the need for reliable assessment beyond empirical evaluation.

Method: Analyzed 13 cancer benchmark datasets using 4 common CNN architectures across different cancer types. Compared model accuracy on original datasets versus datasets made of cropped background segments containing no clinical information (null hypothesis: chance-level accuracy).

Result: CNN models achieved high accuracy (up to 93%) on cropped background segments lacking biomedical information, showing sensitivity to biases. Some architectures were more prone to bias than others.

Conclusion: Standard ML evaluation practices can lead to unreliable results in cancer pathology. Biases in benchmark datasets are difficult to identify and may mislead researchers about CNN efficacy.
...
### [224] [Multiscale Structure-Guided Latent Diffusion for Multimodal MRI Translation](https://arxiv.org/abs/2603.12581)
...
<details>
  <summary>Details</summary>
Motivation: Existing diffusion models for multi-modal MRI translation suffer from anatomical inconsistencies or degraded texture details when handling arbitrary missing-modality scenarios, limiting their practical clinical application.

Method: Proposes a latent diffusion framework with style-structure disentanglement in latent space, separating modality-specific style features from shared structural representations. Uses multi-scale feature modeling for low-frequency anatomical layouts and high-frequency boundary details, with explicit high-frequency structural incorporation during structure disentanglement. Includes style consistency loss and structure-aware loss for improved stability.

Result: Extensive experiments on BraTS2020 and WMH datasets demonstrate superior performance over existing MRI synthesis approaches, particularly in reconstructing complete structures with reliable boundary details.

Conclusion: MSG-LDM effectively addresses anatomical inconsistency and texture degradation in multi-modal MRI translation through latent space disentanglement and multi-scale feature modeling, outperforming state-of-the-art methods.
...
### [225] [Deep Learning Based Estimation of Blood Glucose Levels from Multidirectional Scleral Blood Vessel Imaging](https://arxiv.org/abs/2603.12715)
*Muhammad Ahmed Khan,Manqiang Peng,Ding Lin,Saif Ur Rehman Khan*

Main category: eess.IV

TL;DR: ScleraGluNet: A multiview deep learning framework using scleral vessel images from multiple gaze directions to classify metabolic status (normal/controlled diabetes/high-glucose) and estimate fasting plasma glucose noninvasively.


<details>
  <summary>Details</summary>
Motivation: Regular blood-based glycemic monitoring is burdensome for frequent assessment. The sclera contains superficial microvasculature that may show diabetes-related changes and is readily visible on the ocular surface, offering a potential noninvasive alternative.

Method: Multiview deep learning framework using 5 gaze directions per participant. After vascular enhancement, features are extracted via parallel convolutional branches, refined with Manta Ray Foraging Optimization (MRFO), and fused using transformer-based cross-view attention. Evaluated with subject-wise 5-fold cross-validation on 445 participants with 2,225 images.

Result: Achieved 93.8% overall accuracy for 3-class classification with AUCs of 0.971, 0.956, and 0.982 for normal, controlled diabetes, and high-glucose diabetes respectively. For FPG estimation: MAE = 6.42 mg/dL, RMSE = 7.91 mg/dL, strong correlation (r = 0.983; R² = 0.966), and Bland-Altman bias of +1.45 mg/dL.

Conclusion: Multidirectional scleral vessel imaging with multiview learning is a promising noninvasive approach for glycemic assessment, warranting multicenter validation before clinical deployment.
...
### [226] [GLEAM: A Multimodal Imaging Dataset and HAMM for Glaucoma Classification](https://arxiv.org/abs/2603.12800)
*Jiao Wang,Chi Liu,Yiying Zhang,Hongchen Luo,Zhifen Guo,Ying Hu,Ke Xu,Jing Zhou,Hongyan Xu,Ruiting Zhou,Man Tang*

Main category: eess.IV

TL;DR: GLEAM is the first public tri-modal glaucoma dataset with fundus images, OCT scans, and visual field maps, annotated with 4 disease stages. The paper proposes HAMM, a hierarchical attentive masked modeling approach for multimodal glaucoma classification.


<details>
  <summary>Details</summary>
Motivation: There is a lack of publicly available multimodal glaucoma datasets that combine different imaging modalities (fundus, OCT, visual field) with disease stage annotations. This limits the development of comprehensive AI systems that can leverage complementary information across modalities for accurate glaucoma diagnosis and staging.

Method: The paper introduces GLEAM dataset and proposes HAMM (hierarchical attentive masked modeling) framework. HAMM uses hierarchical attentive encoders and light decoders to focus cross-modal representation learning on the encoder side, effectively integrating information from the three different imaging modalities.

Result: The authors created the first publicly available tri-modal glaucoma dataset (GLEAM) with comprehensive annotations across 4 disease stages. They developed HAMM framework for multimodal glaucoma classification that can effectively exploit complementary information across different imaging modalities.

Conclusion: GLEAM dataset enables effective exploitation of multimodal complementary information for glaucoma diagnosis and treatment across disease stages. The proposed HAMM framework successfully integrates cross-modal information through hierarchical attentive masked modeling, facilitating accurate glaucoma classification.
...
### [227] [Editing Away the Evidence: Diffusion-Based Image Manipulation and the Failure Modes of Robust Watermarking](https://arxiv.org/abs/2603.12949)
*Qian Qi,Jiangyun Tang,Jim Lee,Emily Davis,Finn Carter*

Main category: eess.IV

TL;DR: Diffusion-based image editing can unintentionally remove robust watermarks by contracting low-amplitude signals through stochastic transformations, making watermark recovery information-theoretically impossible under certain conditions.


<details>
  <summary>Details</summary>
Motivation: To understand why diffusion-based image editing, which introduces fundamentally different transformations than traditional post-processing, can degrade or remove watermarks that are designed to survive common operations, and to analyze the implications for copyright protection and content provenance.

Method: Model diffusion editing as a stochastic transformation that contracts off-manifold perturbations, derive theoretical bounds on watermark signal-to-noise ratio and mutual information along diffusion trajectories, and empirically evaluate representative watermarking systems under various diffusion-based editing scenarios.

Result: Even routine semantic edits through diffusion editing can significantly reduce watermark recoverability, with conditions where reliable recovery becomes information-theoretically impossible due to signal decay.

Conclusion: Diffusion editing poses a new challenge for watermark robustness, requiring new design principles for watermarking approaches that remain effective under generative image editing while maintaining content provenance capabilities.
...
### [228] [Reinforcing the Weakest Links: Modernizing SIENA with Targeted Deep Learning Integration](https://arxiv.org/abs/2603.12951)
*Riccardo Raciti,Lemuel Puglisi,Francesco Guarnera,Daniele Ravì,Sebastiano Battiato*

Main category: eess.IV

TL;DR: Deep learning substitutions for skull stripping and segmentation in SIENA improve brain atrophy measurement accuracy, robustness, and speed while preserving interpretability.


<details>
  <summary>Details</summary>
Motivation: SIENA is a widely used method for measuring brain atrophy via PBVC, but it relies on classical image processing steps (skull stripping and tissue segmentation) whose failures can propagate and bias atrophy estimates. The authors want to see if targeted deep learning substitutions can improve SIENA while preserving its established and interpretable framework.

Method: Integrate deep learning models SynthStrip (for skull stripping) and SynthSeg (for tissue segmentation) into the SIENA pipeline. Evaluate three pipeline variants on ADNI and PPMI longitudinal cohorts using three criteria: correlation with longitudinal clinical/structural decline, scan-order consistency, and end-to-end runtime.

Result: Replacing skull-stripping module yields most consistent gains: strengthens associations between PBVC and disease progression measures in ADNI, improves robustness under scan reversal across both datasets. Fully integrated pipeline achieves strongest scan-order consistency (error reduction up to 99.1%). GPU-enabled variants reduce execution time by up to 46% while maintaining comparable CPU runtimes to standard SIENA.
...
### [229] [Accelerating Stroke MRI with Diffusion Probabilistic Models through Large-Scale Pre-training and Target-Specific Fine-Tuning](https://arxiv.org/abs/2603.13007)
*Yamin Arefeen,Sidharth Kumar,Steven Warach,Hamidreza Saber,Jonathan Tamir*

Main category: eess.IV

TL;DR: Data-efficient diffusion model strategy for accelerated MRI reconstruction using foundation model paradigm: pre-train on large public brain MRI data, then fine-tune on small target dataset (20 subjects). Achieves comparable performance to models trained with much more target data.


<details>
  <summary>Details</summary>
Motivation: To enable faster MRI scan times in clinical stroke applications when only limited fully-sampled data is available, addressing the data efficiency challenge for diffusion probabilistic models in accelerated MRI reconstruction.

Method: Foundation model approach: 1) Pre-train Diffusion Probabilistic Model on large diverse brain MRI dataset (~4000 subjects, non-FLAIR contrasts), 2) Fine-tune on small target dataset (20 subjects, FLAIR data) with carefully selected learning rates and durations, 3) Evaluate on fastMRI experiments and clinical stroke MRI with blinded reader study.

Result: Fine-tuned DPMs achieve reconstruction performance comparable to models trained with substantially more target-domain FLAIR data across multiple acceleration factors. Moderate fine-tuning with reduced learning rate works best. Blinded reader study shows 2× accelerated reconstructions are non-inferior to standard-of-care in image quality and structural delineation.

Conclusion: Large-scale pre-training with targeted fine-tuning enables DPM-based MRI reconstruction in data-constrained clinical stroke MRI, reducing need for large application-specific datasets while maintaining clinically acceptable image quality, supporting foundation-inspired diffusion models for accelerated MRI.
...
### [230] [DiT-IC: Aligned Diffusion Transformer for Efficient Image Compression](https://arxiv.org/abs/2603.13162)
*Junqi Shi,Ming Lu,Xingchen Li,Anle Ke,Ruiqi Zhang,Zhan Ma*

Main category: eess.IV

TL;DR: DiT-IC is a diffusion transformer for image compression that operates in deep latent space (32x downscaled), achieving SOTA perceptual quality with 30x faster decoding and lower memory usage than existing diffusion codecs.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based image compression methods suffer from prohibitive sampling overhead and high memory usage due to U-Net architectures operating in shallow latent spaces (8x downscaled). The paper aims to enable diffusion to work in deeper latent spaces (like conventional VAE codecs) without compromising quality.

Method: DiT-IC replaces U-Net with Diffusion Transformer operating at 32x downscaled resolution. It adapts pretrained multi-step DiT to single-step reconstruction via three alignment mechanisms: 1) variance-guided reconstruction flow adapting denoising strength to latent uncertainty, 2) self-distillation alignment enforcing consistency with encoder-defined latent geometry, and 3) latent-conditioned guidance replacing text prompts with semantically aligned latent conditions.

Result: DiT-IC achieves state-of-the-art perceptual quality while offering up to 30x faster decoding and drastically lower memory usage than existing diffusion-based codecs. It can reconstruct 2048x2048 images on a 16 GB laptop GPU.

Conclusion: The paper demonstrates that diffusion can effectively operate in compact latent spaces (32x downscaled) without compromising reconstruction quality, making diffusion-based image compression practical with significantly improved efficiency and accessibility.
...
### [231] [A Learning-Based Approach for Contact Detection, Localization, and Force Estimation of Continuum Manipulators With Integrated OFDR Optical Fiber](https://arxiv.org/abs/2603.12347)
*Mobina Tavangarifard,Jonathan S. Kacines,Qiyu Li,Farshid Alambeigi*

Main category: cs.RO

TL;DR: A cascade learning framework using distributed OFDR fiber sensing enables contact detection, localization, and force estimation in continuum manipulators with a single sensor.


<details>
  <summary>Details</summary>
Motivation: Continuum manipulators are useful for minimally invasive procedures but their distributed deformation makes force sensing, contact detection, and localization challenging, especially when interactions occur at unknown locations along the robot.

Method: Proposes a cascade learning-based framework (CLF) using a single distributed Optical Frequency Domain Reflectometry (OFDR) fiber embedded along one side of the robot. The framework first detects contact using a Gradient Boosting classifier, then estimates contact location and interaction force magnitude using a CNN-FiLM model that predicts spatial force distribution.
...
Conclusion: The proposed cascade learning framework successfully addresses the challenge of force sensing and contact localization in continuum manipulators using minimal sensing infrastructure (single OFDR fiber), enabling better interaction awareness in minimally invasive procedures.
...
### [232] [GNN-DIP: Neural Corridor Selection for Decomposition-Based Motion Planning](https://arxiv.org/abs/2603.12361)
*Peng Xie,Yanlinag Huang,Wenyuan Wu,Amr Alanwar*

Main category: cs.RO

TL;DR: GNN-DIP integrates Graph Neural Networks with decomposition-based planning to efficiently solve narrow passage problems, achieving 99-100% success rates with 2-280x speedup over sampling-based methods.


<details>
  <summary>Details</summary>
Motivation: Motion planning through narrow passages is challenging: sampling-based planners rarely sample inside narrow regions, and decomposition-based planners face combinatorial explosion in corridor selection through cell graphs.

Method: GNN-DIP combines Graph Neural Networks with Decomposition-Informed Planner (DIP). GNN predicts portal scores on cell adjacency graph to bias corridor search toward near-optimal regions while preserving completeness. Uses Constrained Delaunay Triangulation with Funnel algorithm in 2D, and Slab convex decomposition with portal-face sampling in 3D.
...
Conclusion: GNN-DIP effectively addresses narrow passage planning challenges by combining neural network guidance with decomposition-based methods, significantly outperforming traditional sampling-based approaches while maintaining completeness guarantees.
...
### [233] [Push, Press, Slide: Mode-Aware Planar Contact Manipulation via Reduced-Order Models](https://arxiv.org/abs/2603.12399)
*Melih Özcan,Ozgur S. Oguz,Umut Orguner*

Main category: cs.RO

TL;DR: A mode-aware framework for planar non-prehensile manipulation using contact topology selection and reduced-order kinematic models to simplify complex contact mechanics without optimization.


<details>
  <summary>Details</summary>
Motivation: Non-prehensile planar manipulation (pushing, press-and-slide) is challenging due to hybrid contact mechanics, under-actuation, and asymmetric friction limits that typically require computationally expensive iterative control methods.

Method: Proposes a mode-aware framework that abstracts complex wrench-twist limit surface mechanics into a discrete library of physically intuitive reduced-order models. Maps various single-arm and bimanual contact topologies to simple non-holonomic formulations (e.g., unicycle for press-and-slide). Uses trajectory generation anchored to these models to compute required object wrench and distributes feasible, friction-bounded contact forces via direct algebraic allocation. Incorporates manipulator kinematics for long-horizon feasibility.

Result: Demonstrates a fast, optimization-free approach in simulation across diverse single-arm and bimanual manipulation tasks, showing computational efficiency compared to traditional iterative control methods.

Conclusion: The framework successfully simplifies complex planar manipulation problems by using contact topology selection and reduced-order modeling, enabling efficient non-prehensile manipulation without expensive optimization while maintaining physical feasibility.
...
### [234] [Beyond Motion Imitation: Is Human Motion Data Alone Sufficient to Explain Gait Control and Biomechanics?](https://arxiv.org/abs/2603.12408)
*Xinyi Liu,Jangwhan Ahn,Edgar Lobaton,Jennie Si,He Huang*

Main category: cs.RO

TL;DR: Adding foot-ground interaction measures (contacts and forces) as reward terms in reinforcement learning-based motion imitation learning improves biomechanical plausibility of joint kinetics in gait simulation, addressing limitations of kinematics-only approaches.


<details>
  <summary>Details</summary>
Motivation: Motion imitation learning for human biomechanics and wearable robotics often focuses on kinematics matching but may produce physically inconsistent joint kinetics. The study investigates whether incorporating foot-ground interaction measures can improve biomechanical plausibility.

Method: Reinforcement learning-based imitation learning framework with additional reward terms for foot-ground contacts and contact forces, comparing against motion-only approaches that only match forward kinematics.

Result: Adding foot-ground interaction measures enables prediction of joint moments significantly closer to inverse dynamics calculations. Kinematics-only approaches fail to produce biomechanically plausible joint kinetics despite accurate kinematics reproduction.

Conclusion: Kinetics-based reward shaping is necessary for physically consistent gait representations in human biomechanics and wearable robot research. Ground reaction forces and center of pressure information significantly enhance realism of internal and external kinetics.
...
### [235] [Predictive and adaptive maps for long-term visual navigation in changing environments](https://arxiv.org/abs/2603.12460)
*Lucie Halodova,Eliska Dvorakova,Filip Majer,Tomas Vintr,Oscar Martinez Mozos,Feras Dayoub,Tomas Krajnik*

Main category: cs.RO

TL;DR: Paper compares map management techniques for long-term visual navigation in changing environments, finding that strategies modeling cyclic appearance changes outperform those without temporal modeling.


<details>
  <summary>Details</summary>
Motivation: Enable reliable long-term visual navigation in changing environments where the robot needs to continuously update its feature map to adapt to appearance changes over time.

Method: Propose and evaluate several map management strategies for teach-and-repeat navigation, focusing on feature selection, removal of obsolete features, and addition of new features from current camera views.

Result: Strategies that model cyclic changes of environment appearance and predict feature visibility at specific times/locations outperform strategies without explicit temporal modeling, based on three-month experiments measuring robot localization accuracy.

Conclusion: Explicitly modeling temporal evolution of environment changes is crucial for effective long-term visual navigation, with cyclic change modeling strategies providing superior localization performance.
...
### [236] [One-Step Flow Policy: Self-Distillation for Fast Visuomotor Policies](https://arxiv.org/abs/2603.12480)
*Shaolong Li,Lichao Sun,Yongchao Chen*

Main category: cs.RO

TL;DR: One-Step Flow Policy (OFP) enables single-step action generation for robotics, eliminating iterative sampling latency while maintaining high precision, achieving 100× speedup over diffusion/flow models.


<details>
  <summary>Details</summary>
Motivation: Generative flow and diffusion models provide good multimodal action distributions but suffer from severe inference latency due to iterative sampling, degrading control frequency and performance in time-sensitive robotic manipulation tasks.

Method: OFP uses a from-scratch self-distillation framework without pre-trained teacher, combining self-consistency loss for coherent transport across time intervals, self-guided regularization to sharpen predictions toward expert modes, and warm-start mechanism leveraging temporal action correlations to minimize generative transport distance.

Result: Evaluated across 56 diverse simulated manipulation tasks, one-step OFP achieves state-of-the-art results, outperforming 100-step diffusion and flow policies while accelerating action generation by over 100×. Integrated into π₀.₅ model on RoboTwin 2.0, one-step OFP surpasses original 10-step policy.
...
### [237] [COAD: Constant-Time Planning for Continuous Goal Manipulation with Compressed Library and Online Adaptation](https://arxiv.org/abs/2603.12488)
*Adil Shiyas,Zhuoyun Zhong,Constantinos Chamzas*

Main category: cs.RO

TL;DR: COAD enables constant-time motion planning for repetitive robotic manipulation tasks by compressing solution libraries and adapting root motions to new goals.


<details>
  <summary>Details</summary>
Motivation: Robotic manipulation often involves solving similar motion-planning problems repeatedly with only goal/obstacle location changes. Existing methods lack continuous task space coverage guarantees and require storing large solution libraries.

Method: COAD discretizes continuous task space into Task Coverage Regions, solves only representative root problems offline, and uses lightweight adaptation modules (linear interpolation, Dynamic Movement Primitives, trajectory optimization) to adapt root motions to new goals at query time.

Result: COAD achieves substantial motion library compression while maintaining high success rates with sub-millisecond query times, outperforming baselines in both efficiency and path quality across various manipulators and environments in simulation and real-world tests.

Conclusion: COAD provides an efficient framework for constant-time planning over continuous goal-parameterized task spaces, balancing compression, success rates, and query speed for repetitive robotic manipulation tasks.
...
### [238] [Robots that redesign themselves through kinematic self-destruction](https://arxiv.org/abs/2603.12505)
*Chen Yu,Sam Kriegman*

Main category: cs.RO

TL;DR: A robot that actively redesigns itself during its lifetime by identifying and breaking off redundant links to improve locomotion, using a single learned controller that transfers from simulation to reality.


<details>
  <summary>Details</summary>
Motivation: Current robots are predesigned before deployment and cannot adapt their physical structure during operation. The paper explores whether robots could actively participate in their own design through self-modification to potentially outperform static designs.
...
Result: The optimized policy successfully transfers to reality and generalizes to previously unseen kinematic trees. The self-designing robot generates forward locomotion more effectively than equivalent policies that randomly remove links or cannot remove any links at all.

Conclusion: Self-designing robots may outperform predesigned robots in some cases, and kinematic self-destruction (though reductive and irreversible) could provide a general adaptive strategy for a wide range of robots.
...
### [239] [Red-Teaming Vision-Language-Action Models via Quality Diversity Prompt Generation for Robust Robot Policies](https://arxiv.org/abs/2603.12510)
...
TL;DR: Q-DIG uses Quality Diversity techniques with Vision-Language Models to generate diverse adversarial instructions that expose vulnerabilities in Vision-Language-Action models, improving their robustness through fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models are sensitive to precise wording of instructions, making it difficult to predict failures. Current systems lack robustness to different natural language phrasings of the same task.

Method: Q-DIG integrates Quality Diversity techniques with Vision-Language Models to perform red-teaming, scalably identifying diverse natural language task descriptions that cause failures while remaining task-relevant. It generates a broad spectrum of adversarial instructions to expose vulnerabilities.

Result: Q-DIG finds more diverse and meaningful failure modes than baselines across simulation benchmarks. Fine-tuning VLAs on generated instructions improves task success rates. User studies show Q-DIG generates more natural, human-like prompts. Real-world evaluations confirm simulation results.

Conclusion: Q-DIG is a promising approach for identifying vulnerabilities and improving robustness of VLA-based robots through adversarial instruction generation and fine-tuning.
...
### [240] [Beyond Dense Futures: World Models as Structured Planners for Robotic Manipulation](https://arxiv.org/abs/2603.12553)
*Minghao Jin,Mozheng Liao,Mingfei Han,Zhihui Li,Xiaojun Chang*

Main category: cs.RO

TL;DR: StructVLA reformulates world models into structured planners using sparse kinematic frames instead of dense visual predictions, improving long-horizon robotic manipulation.


<details>
  <summary>Details</summary>
Motivation: Existing world-model-based VLA approaches suffer from visual redundancy and error accumulation in dense predictions, while sparse methods lack explicit kinematic grounding, causing misalignment between planning and low-level execution.

Method: Two-stage training with unified discrete tokens: 1) world model predicts sparse structured frames derived from kinematic cues (gripper transitions, turning points), 2) model maps structured foresight to low-level actions.

Result: Achieves 75.0% success on SimplerEnv-WidowX and 94.8% on LIBERO. Real-world deployments show reliable task completion and robust generalization across basic and complex long-horizon tasks.

Conclusion: StructVLA's explicit structured planning with kinematic-grounded frames effectively bridges visual planning and motion control, addressing limitations of both dense and sparse world model approaches.
...
### [241] [From Woofs to Words: Towards Intelligent Robotic Guide Dogs with Verbal Communication](https://arxiv.org/abs/2603.12574)
...
TL;DR: A novel dialog system for robotic guide dogs using LLMs to verbalize navigational plans and scenes, enabling collaborative decision-making between visually impaired handlers and robots.


<details>
  <summary>Details</summary>
Motivation: Robotic guide dogs need language capabilities beyond simple dialog systems - they must ground language in dynamic environments and improve spatial awareness for visually impaired handlers, enabling better collaborative decision-making.

Method: Developed a novel dialog system using Large Language Models (LLMs) to verbalize both navigational plans and environmental scenes, addressing the challenges of grounding language in changing environments and enhancing spatial awareness.

Result: Conducted human studies to evaluate different verbalization strategies and simulation studies to assess navigation efficiency and accuracy, demonstrating the system's effectiveness in assistive robotics applications.

Conclusion: The LLM-based dialog system successfully enables verbal communication for collaborative decision-making in handler-robot teams, advancing assistive robotics for visually impaired individuals through improved language grounding and spatial awareness.
...
### [242] [Skill-informed Data-driven Haptic Nudges for High-dimensional Human Motor Learning](https://arxiv.org/abs/2603.12583)
...
<details>
  <summary>Details</summary>
Motivation: To improve motor skill learning in high-dimensional tasks by designing optimal haptic feedback that accelerates learning and helps learners discover efficient motor representations.

Method: 1) Model human motor learning dynamics using Input-Output Hidden Markov Model (IOHMM) to decouple latent skill evolution from observable kinematics. 2) Formulate haptic nudge design as Partially Observable Markov Decision Process (POMDP). 3) Derive optimal nudging policy that minimizes long-term performance cost and guides learners toward robust skill regions.

Result: Human-subject study (N=30) with hand-exoskeleton task showed: 1) POMDP-derived policy group significantly accelerated task performance compared to heuristic-based feedback or no feedback groups. 2) Synergy analysis revealed POMDP group discovered efficient low-dimensional motor representations more rapidly.

Conclusion: The proposed data-driven skill-informed framework successfully designs optimal haptic nudge feedback that accelerates motor learning and promotes discovery of efficient motor synergies in high-dimensional tasks.
...
### [243] [CarPLAN: Context-Adaptive and Robust Planning with Dynamic Scene Awareness for Autonomous Driving](https://arxiv.org/abs/2603.12607)
*Junyong Yun,Jungho Kim,ByungHyun Lee,Dongyoung Lee,Sehwan Choi,Seunghyeop Nam,Kichun Jo,Jun Won Choi*

Main category: cs.RO

TL;DR: CarPLAN: An IL-based motion planning framework for autonomous driving that enhances context understanding through displacement-aware encoding and adaptive multi-expert decoding, achieving SOTA performance on nuPlan benchmark.


<details>
  <summary>Details</summary>
Motivation: Imitation learning for autonomous driving needs to capture complex driving contexts and enable adaptive decision-making, not just imitate expert trajectories, for safe and robust real-world driving.

Method: Two key components: 1) Displacement-Aware Predictive Encoding (DPE) that predicts future displacement vectors between AV and surroundings, with augmented loss for displacement prediction; 2) Context-Adaptive Multi-Expert Decoder (CMD) using Mixture of Experts framework to dynamically select expert decoders based on scene structure at each Transformer layer.

Result: State-of-the-art performance across all closed-loop simulation metrics on nuPlan benchmark, with robust performance on challenging Test14-Hard scenarios. Additional validation on Waymax benchmark demonstrates generalization capability.

Conclusion: CarPLAN effectively enhances driving context understanding and enables adaptive planning in diverse traffic scenarios, providing a robust IL-based motion planning framework for autonomous driving.
...
### [244] [Autonomous Integration and Improvement of Robotic Assembly using Skill Graph Representations](https://arxiv.org/abs/2603.12649)
...
<details>
  <summary>Details</summary>
Motivation: Traditional robotic assembly systems require substantial manual engineering effort for integration, adaptation, and performance improvement. There's a need for more autonomous, adaptive, and reusable systems that can handle new tasks and environments with less human intervention.

Method: Proposes Skill Graph representations that organize robot capabilities as verb-based skills, linking semantic descriptions (verbs and nouns) with executable policies, pre/post-conditions, and evaluators. This enables semantic-level planning while grounding execution through interfaces to robot controllers and perception modules.

Result: The framework enables rapid system integration, systematic data collection, and closed-loop performance improvement. It unifies system configuration, execution, evaluation, and learning within a single representation, providing a scalable pathway toward adaptive robotic assembly systems.

Conclusion: Skill Graphs provide a unified representation that supports autonomous integration and continuous improvement of robotic assembly systems, making them more adaptive and reusable while reducing manual engineering effort.
...
### [245] [Panoramic Multimodal Semantic Occupancy Prediction for Quadruped Robots](https://arxiv.org/abs/2603.13108)
...
Method: (1) Created PanoMMOcc dataset with four sensing modalities across diverse scenes; (2) Proposed VoxelHound framework with Vertical Jitter Compensation (VJC) module to mitigate viewpoint perturbations from body pitch/roll, and Multimodal Information Prompt Fusion (MIPF) module to leverage panoramic visual cues and auxiliary modalities.

Result: VoxelHound achieves state-of-the-art performance on PanoMMOcc (+4.16% in mIoU). Dataset and code will be publicly released with calibration tools.

Conclusion: The work bridges the gap in panoramic multimodal occupancy perception for quadruped robots, providing a comprehensive dataset and effective framework for embodied robotic systems in challenging environments.
...
### [246] [TacVLA: Contact-Aware Tactile Fusion for Robust Vision-Language-Action Manipulation](https://arxiv.org/abs/2603.12665)
...
<details>
  <summary>Details</summary>
Motivation: Standard VLA models rely on vision and language but perform poorly in tasks with visual occlusion, fine-grained manipulation, and physical contact. There's a need to incorporate tactile sensing to address these limitations.

Method: Introduces TacVLA, a fine-tuned VLA model that integrates tactile modalities into transformer-based policy. Uses a contact-aware gating mechanism to selectively activate tactile tokens only when contact is detected, enabling adaptive multimodal fusion without irrelevant tactile interference.
...
Conclusion: Incorporating tactile sensing with adaptive fusion significantly enhances VLA models for contact-rich manipulation tasks, addressing limitations of vision-only approaches in occlusion and fine-grained scenarios.
...
### [247] [Learning Athletic Humanoid Tennis Skills from Imperfect Human Motion Data](https://arxiv.org/abs/2603.12686)
...
TL;DR: LATENT learns athletic humanoid tennis skills from imperfect human motion data fragments, enabling robots to consistently strike and return tennis balls with natural motion styles.


<details>
  <summary>Details</summary>
Motivation: Humanoid robots struggle to reproduce versatile tennis skills due to lack of perfect human motion data from real tennis matches. Collecting complete, precise tennis motion sequences is difficult.

Method: Uses imperfect human motion fragments capturing primitive tennis skills as priors. Learns humanoid policy through correction and composition of these fragments. Includes designs for robust sim-to-real transfer to deploy on Unitree G1 robot.

Result: Achieves consistent ball striking under wide conditions with target return accuracy while preserving natural motion. Successfully deployed on real robot, sustaining multi-shot rallies with human players.

Conclusion: LATENT demonstrates that imperfect human motion fragments can be effectively used to learn complex athletic skills for humanoid robots, enabling realistic tennis play with robust sim-to-real transfer.
...
### [248] [HaltNav: Reactive Visual Halting over Lightweight Topological Priors for Robust Vision-Language Navigation](https://arxiv.org/abs/2603.12696)
*Pingcong Li,Zihui Yu,Bichi Zhang,Sören Schwertfeger*

Main category: cs.RO

TL;DR: HaltNav: Hierarchical navigation framework combining global topological planning (osmAG) with local VLN execution and reactive visual halting for robust open-vocabulary navigation under environmental changes.


<details>
  <summary>Details</summary>
Motivation: VLN is moving toward open-vocabulary, goal-oriented autonomy without exhaustive routing prompts, but existing approaches using heavy 2D/3D metric maps are brittle in real-world deployments where local connectivity changes (closed doors, crowded passages).

Method: Hierarchical framework with MLLM-based brain module for task grounding and obstruction awareness. Uses lightweight osmAG (OpenStreetMap Area Graph) for global planning, converts routes to localized execution snippets for VLN executor, and implements Reactive Visual Halting (RVH) to detect anomalies, update topology, and trigger replanning. Includes data synthesis pipeline using generative models to create realistic obstacle scenarios for training.
...
Conclusion: HaltNav effectively bridges global topological planning with local VLN execution through reactive halting, enabling robust open-vocabulary navigation in dynamic environments without requiring exhaustive routing prompts.
...
### [249] [Altered Thoughts, Altered Actions: Probing Chain-of-Thought Vulnerabilities in VLA Robotic Manipulation](https://arxiv.org/abs/2603.12717)
*Tuan Duong Trinh,Naveed Akhtar,Basim Azam*

Main category: cs.RO

TL;DR: VLA models using chain-of-thought reasoning are vulnerable to targeted corruption of intermediate text plans, especially object name substitution, which degrades physical task performance while other corruptions have minimal impact.


<details>
  <summary>Details</summary>
Motivation: To investigate the security vulnerabilities in Vision-Language-Action models that use chain-of-thought reasoning, specifically examining which properties of intermediate reasoning traces the action decoder actually relies on, and whether targeted corruption of reasoning traces alone can degrade robot task performance.

Method: Designed a taxonomy of seven text corruptions organized into three attacker tiers (blind noise, mechanical-semantic, and LLM-adaptive), applied to a state-of-the-art reasoning VLA across 40 LIBERO tabletop manipulation tasks, with cross-architecture control using a non-reasoning VLA.

Result: Object name substitution in reasoning traces reduces overall success rate by 8.3 percentage points (reaching -19.3pp on goal-conditioned tasks and -45pp on individual tasks), while other corruptions like sentence reordering, spatial-direction reversal, token noise, and even LLM-crafted wrong plans have negligible impact (±4pp). The vulnerability is exclusive to reasoning-augmented models.

Conclusion: Action decoders in reasoning VLAs depend primarily on entity-reference integrity rather than reasoning quality or sequential structure, making internal reasoning traces a distinct and stealthy threat vector invisible to input-validation defenses, with sophisticated LLM attackers underperforming simple object-name substitution.
...
### [250] [AnchorVLA4D: an Anchor-Based Spatial-Temporal Vision-Language-Action Model for Robotic Manipulation](https://arxiv.org/abs/2603.12730)
...
Method: Proposes AnchorVLA4D that augments visual input with an anchor image to preserve initial scene context, adds a lightweight spatial encoder to process anchor and current frames jointly, and uses a Qwen2.5-VL backbone with diffusion-based action head.
...
Conclusion: Visual anchors combined with spatial encoding effectively enhance spatial-temporal reasoning in VLA policies, addressing key limitations of current systems while maintaining efficiency.
...
### [251] [Conflict Mitigation in Shared Environments using Flow-Aware Multi-Agent Path Finding](https://arxiv.org/abs/2603.12736)
*Lukas Heuer,Yufei Zhu,Luigi Palmieri,Andrey Rudenko,Anna Mannucci,Sven Koenig,Martin Magnusson*

Main category: cs.RO

TL;DR: FA-MAPF integrates learned motion patterns of uncontrollable agents into centralized MAPF to reduce conflicts by up to 55% while maintaining task efficiency.


<details>
  <summary>Details</summary>
Motivation: Multi-robot systems face challenges in environments with dynamic, uncontrollable agents where individual robots experience delays. Existing research focuses on preserving solution completeness but lacks emphasis on using environmental information to improve solution quality.

Method: Proposes Flow-Aware Multi-Agent Path Finding (FA-MAPF) framework that integrates learned motion patterns of uncontrollable agents into centralized MAPF algorithms.

Result: Evaluation on benchmark maps with simulated uncontrollable agents and real-world maps with recorded human trajectories shows FA-MAPF consistently reduces conflicts with uncontrollable agents by up to 55% without compromising task efficiency.

Conclusion: FA-MAPF effectively enhances multi-robot system performance in shared environments by leveraging learned motion patterns of uncontrollable agents to reduce conflicts while maintaining operational efficiency.
...
### [252] [Easy-IIL: Reducing Human Operational Burden in Interactive Imitation Learning via Assistant Experts](https://arxiv.org/abs/2603.12769)
...
<details>
  <summary>Details</summary>
Motivation: Current Interactive Imitation Learning requires extensive human involvement for both offline demonstrations and online interactions. While prior work focuses on reducing passive monitoring, active human operation remains burdensome. Model-based imitation methods perform well with few demonstrations but are surpassed by end-to-end methods with more data.

Method: Easy-IIL uses off-the-shelf model-based imitation methods as an assistant expert to replace active human operation for most data collection. Human expert provides only one demonstration to initialize the assistant and intervenes only in critical states where tasks approach failure. The framework maintains data quality for both offline and online phases.

Result: Extensive simulation and real-world experiments show Easy-IIL significantly reduces human operational burden while maintaining performance comparable to mainstream IIL baselines. User studies confirm reduced subjective workload for human experts.

Conclusion: Easy-IIL successfully reduces human effort in Interactive Imitation Learning by leveraging model-based imitation as an assistant expert, requiring minimal human intervention while maintaining performance quality.
...
### [253] [Motion-Specific Battery Health Assessment for Quadrotors Using High-Fidelity Battery Models](https://arxiv.org/abs/2603.12791)
*Joonhee Kim,Sanghyun Park,Donghyeong Kim,Eunseon Choi,Soohee Han*

Main category: cs.RO

TL;DR: This paper presents a motion-aware battery health assessment framework for quadrotors that captures how flight motions induce dynamic current loads accelerating battery degradation, rather than treating batteries as simple energy reservoirs.


<details>
  <summary>Details</summary>
Motivation: Current quadrotor energy planning overlooks how flight motions create dynamic current loads that accelerate battery degradation, treating batteries as simple energy reservoirs instead of accounting for motion-specific degradation effects.

Method: 1) Design wide-range current sensing module to capture motion-specific current profiles during real flights; 2) Calibrate high-fidelity battery model using reference performance tests and metaheuristic degradation-coupled electrochemical model; 3) Simulate measured flight loads in calibrated model to resolve how different motions translate into degradation modes.

Result: Results show that flight profiles with the same average energy consumption can drive different degradation pathways (loss of lithium inventory, loss of active material, internal side reactions) based on their transient load structures.

Conclusion: Motion-aware battery management is needed to balance efficiency with battery degradation, as transient load structures significantly impact degradation pathways even when total energy consumption is identical.
...
### [254] [FLUX: Accelerating Cross-Embodiment Generative Navigation Policies via Rectified Flow and Static-to-Dynamic Learning](https://arxiv.org/abs/2603.12806)
*Zeying Gong,Yangyi Zhong,Yiyi Ding,Tianshuai Hu,Guoyang Zhao,Lingdong Kong,Rong Li,Jiadi You,Junwei Liang*

Main category: cs.RO

TL;DR: FLUX is a flow-based unified navigation policy that achieves SOTA across six navigation tasks and demonstrates zero-shot sim-to-real transfer on multiple robotic platforms without fine-tuning.
...
Method: FLUX uses linearized probability flow to replace iterative denoising with straight-line trajectories, improving inference efficiency. It follows a static-to-dynamic curriculum: first establishing geometric priors, then refined through RL in dynamic social environments.

Result: FLUX improves per-step inference efficiency by 47% over prior flow-based methods and 29% over diffusion-based ones. It achieves state-of-the-art performance across all six navigation tasks and demonstrates zero-shot sim-to-real transfer on wheeled, quadrupedal, and humanoid platforms.

Conclusion: FLUX provides an efficient, unified navigation policy that excels across diverse navigation tasks and enables practical deployment on various robotic platforms without additional fine-tuning.
...
### [255] [Reinforcement Learning for Elliptical Cylinder Motion Control Tasks](https://arxiv.org/abs/2603.12807)
*Pawel Marczewski,Paulina Superczynska,Jakub Bernat,Szymon Szczesny*

Main category: cs.RO

TL;DR: Reinforcement Learning applied to control an elliptic cylinder with limited torque, comparing RL policy against classical two-stage controller (energy-shaping swing-up + LQR stabilization).


<details>
  <summary>Details</summary>
Motivation: Control of devices with limited input torque is challenging but important, inspired by untethered magnetic devices that operate with distance-limited torque. The elliptic cylinder control problem serves as a benchmark for underactuated systems.

Method: Uses Reinforcement Learning to solve elliptic cylinder control with limited torque. Compares against classical baseline: two-stage controller with energy-shaping swing-up law to drive system toward equilibrium, followed by LQR stabilization around target orientation.

Result: Learning is possible for elliptic cylinder control, but stabilization in upward position or half-turn rotation becomes very difficult for increasing mass or ellipses with strongly unequal perimeter ratios.

Conclusion: RL can solve limited-torque elliptic cylinder control, though challenging cases remain difficult. The classical swing-up + LQR policy provides strong, interpretable baseline for underactuated systems.
...
### [256] [SmoothTurn: Learning to Turn Smoothly for Agile Navigation with Quadrupedal Robots](https://arxiv.org/abs/2603.12842)
*Zunzhi You,Haolan Guo,Yunke Wang,Chang Xu*

Main category: cs.RO

TL;DR: SmoothTurn: A learning-based control framework for quadrupedal robots to perform smooth turning while running at high speeds for agile sequential local navigation.


<details>
  <summary>Details</summary>
Motivation: Real-world applications like fire rescue and industrial inspection require quadrupedal robots to navigate agilely at high speeds with smooth direction changes. Existing single-goal reaching policies fail to anticipate upcoming maneuvers or maintain momentum when switching between sequential goals, limiting the robot's agility potential.

Method: Formulates the task as sequential local navigation and introduces SmoothTurn framework with: 1) novel sequential goal-reaching reward, 2) expanded observation space with lookahead window for future goals, and 3) automatic goal curriculum that progressively increases difficulty based on performance.

Result: The trained policy enables smooth turning across goals with emergent behaviors like momentum control during goal switching, anticipatory facing toward future goals, and efficient path planning. Successfully deployed on real quadrupedal robots with onboard sensors and computation.

Conclusion: SmoothTurn effectively addresses the limitations of single-goal policies for agile sequential navigation, enabling quadrupedal robots to fully exploit their agility potential through smooth turning and momentum management across goal sequences.
...
### [257] [Beyond Imitation: Reinforcement Learning Fine-Tuning for Adaptive Diffusion Navigation Policies](https://arxiv.org/abs/2603.12868)
*Junhe Sheng,Ruofei Bai,Kuan Xu,Ruimeng Liu,Jie Chen,Shenghai Yuan,Wei-Yun Yau,Lihua Xie*

Main category: cs.RO

TL;DR: Reinforcement learning fine-tuning framework for diffusion-based navigation policies that improves zero-shot generalization and safety in unseen environments without requiring a separate value network.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based navigation policies trained on imitation learning datasets have strong zero-shot generalization but suffer from distribution shift in unseen settings, leading to safety-critical failures. Traditional RL adaptation is challenging due to diffusion models' iterative denoising structure and computational expense of training value networks.

Method: Proposes RL fine-tuning framework using Group Relative Policy Optimization (GRPO) that estimates relative advantages across sampled trajectories without a separate value network. Freezes visual encoder while selectively updating higher decoder layers and action head to preserve pretrained representations while enabling adaptation through online environmental feedback.

Result: On PointGoal task in Isaac Sim: Success Rate improved from 52.0% to 58.7%, SPL from 0.49 to 0.54 on unseen scenes while reducing collision frequency. Fine-tuned policy transfers zero-shot to real quadruped platform and maintains stable performance in geometrically out-of-distribution environments.

Conclusion: The proposed RL fine-tuning framework enables diffusion-based navigation policies to adapt to new domains while preserving pretrained knowledge, achieving improved safety-aware behaviors and better generalization to unseen environments without the computational burden of training separate value networks.
...
### [258] [Consistent and Efficient MSCKF-based LiDAR-Inertial Odometry with Inferred Cluster-to-Plane Constraints for UAVs](https://arxiv.org/abs/2603.12904)
*Jinwen Zhu,Xudong Zhao,Fangcheng Zhu,Jun Hu,Shi Jin,Yinian Mao,Guoquan Huang*

Main category: cs.RO

TL;DR: A consistent and efficient LiDAR-Inertial Odometry framework for UAVs using MSCKF with coplanar constraints and parallel voxel-based data association for real-time performance on resource-constrained platforms.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-Inertial Odometry systems suffer from estimation inconsistency and computational bottlenecks when deployed on UAVs with strict SWaP constraints, making robust and accurate navigation challenging for these platforms.

Method: Proposes a tightly-coupled LIO framework using Multi-State Constraint Kalman Filter with coplanar constraints from planar features across a sliding window. Uses null-space projection to eliminate feature parameter dependency, parallel voxel-based data association, and a novel compact cluster-to-plane measurement model for dimensionality reduction.

Result: Outperforms most SOTA approaches with superior consistency-efficiency balance, improved robustness in degenerate scenarios, lowest memory usage (map-free), and real-time operation on embedded platforms like NVIDIA Jetson TX2.

Conclusion: The proposed LIO framework successfully addresses consistency and efficiency challenges for UAV navigation, providing a practical solution for resource-constrained platforms while maintaining high performance and robustness.
...
### [259] [GoalSwarm: Multi-UAV Semantic Coordination for Open-Vocabulary Object Navigation](https://arxiv.org/abs/2603.12908)
*MoniJesu Wonders James,Amir Atef Habel,Aleksey Fedoseev,Dzmitry Tsetserokou*

Main category: cs.RO

TL;DR: GoalSwarm: A decentralized multi-UAV framework for zero-shot semantic object-goal navigation using lightweight 2D semantic occupancy maps and foundation models.


<details>
  <summary>Details</summary>
Motivation: Achieving robust open-vocabulary object-goal navigation for aerial robot teams is challenging due to computational constraints of heavy perception models onboard and complexity of decentralized multi-agent coordination.

Method: 1) Uses SAM3 foundation model for zero-shot open-vocabulary detection and segmentation; 2) Creates Bayesian Value Map fusing multi-viewpoint detection confidences into goal-relevance distribution; 3) Implements decentralized coordination with semantic frontier extraction, cost-utility bidding with geodesic paths, and spatial separation penalties.

Result: A fully decentralized multi-UAV framework that eliminates computational burden of full 3D representations while preserving essential geometric and semantic structure for cooperative visual semantic navigation.

Conclusion: GoalSwarm enables robust open-vocabulary object-goal navigation for aerial robot teams through lightweight 2D semantic mapping, zero-shot foundation models, and efficient decentralized coordination strategies.
...
### [260] [MotionAnymesh: Physics-Grounded Articulation for Simulation-Ready Digital Twins](https://arxiv.org/abs/2603.12936)
*WenBo Xu,Liu Liu,Li Zhang,Dan Guo,RuoNan Liu*

Main category: cs.RO

TL;DR: MotionAnymesh: A zero-shot framework that converts static 3D meshes into simulation-ready articulated assets by combining kinematic-aware segmentation with physics-constrained joint estimation to eliminate kinematic hallucinations and guarantee collision-free articulation.
...
Method: 1) Kinematic-aware part segmentation module that grounds VLM reasoning with explicit SP4D physical priors to eliminate kinematic hallucinations. 2) Geometry-physics joint estimation pipeline combining type-aware initialization with physics-constrained trajectory optimization to guarantee collision-free articulation.

Result: MotionAnymesh significantly outperforms state-of-the-art baselines in both geometric precision and dynamic physical executability, providing highly reliable simulation-ready digital twin assets for downstream applications.

Conclusion: The proposed framework successfully bridges the gap between static mesh conversion and physically-grounded articulation, enabling automated creation of reliable simulation-ready assets without manual intervention.
...
### [261] [RoboStream: Weaving Spatio-Temporal Reasoning with Memory in Vision-Language Models for Robotics](https://arxiv.org/abs/2603.12939)
...
TL;DR: RoboStream introduces a training-free framework for long-horizon robotic manipulation that maintains persistent geometric anchoring and causal memory through Spatio-Temporal Fusion Tokens and Causal Spatio-Temporal Graphs, addressing limitations of current VLM-based planners.


<details>
  <summary>Details</summary>
Motivation: Current VLM-based planners treat each manipulation step as isolated, forcing them to reinfer scene geometry from raw pixels at every decision point without tracking how prior actions have changed the environment. This leads to perceptual error accumulation, catastrophic forgetting of occluded objects, and cascading precondition violations in long-horizon tasks.
...
Result: RoboStream achieves 90.5% success on long-horizon RLBench tasks and 44.4% on challenging real-world block-building tasks, significantly outperforming SoFar and VoxPoser (both 11.1%), demonstrating the critical importance of spatio-temporal reasoning and causal memory.

Conclusion: Spatio-temporal reasoning and causal memory are essential missing components for reliable long-horizon robotic manipulation, and RoboStream's training-free framework successfully addresses these limitations by maintaining persistent geometric anchoring and tracking action consequences across interactions.
...
### [262] [Coordinated Manipulation of Hybrid Deformable-Rigid Objects in Constrained Environments](https://arxiv.org/abs/2603.12940)
*Anees Peringal,Anup Teejo Mathew,Panagiotis liatsis,Federico Renda*

Main category: cs.RO

TL;DR: A quasi-static optimization-based planner for manipulating hybrid deformable linear objects (hDLOs) using a strain-based Cosserat rod model achieves 33x speedup via analytical gradients and enables manipulation in constrained environments.


<details>
  <summary>Details</summary>
Motivation: While coordinated manipulation of purely deformable linear objects (DLOs) has been studied, handling hybrid assemblies with both deformable and rigid elements in constrained environments remains challenging. There's a need for planners that can exploit compliance of deformable links to achieve task-space objectives unreachable with rigid tools.

Method: The method uses a quasi-static optimization-based manipulation planner with a strain-based Cosserat rod model that extends rigid-body formulations to hybrid deformable linear objects. It employs a differentiable model with analytically derived gradients for inverse kinetostatic (IKS) problems, followed by trajectory optimization warm-started from IKS solutions.

Result: The method achieves up to 33x speedup over finite-difference baselines for IKS problems. Experimental validation shows average deformation error of ~3 cm (5% of deformable link length). The planner outperforms sampling-based feasibility planners adapted to the strain-based formulation.

Conclusion: The proposed optimal planner is effective for robotic manipulation of hybrid assemblies in constrained environments, demonstrating accuracy and computational efficiency through analytical derivatives and the strain-based Cosserat rod model.
...
### [263] [ReMem-VLA: Empowering Vision-Language-Action Model with Memory via Dual-Level Recurrent Queries](https://arxiv.org/abs/2603.12942)
*Hang Li,Fengyi Shen,Dong Chen,Liudi Yang,Xudong Wang,Jinkui Shi,Zhenshan Bing,Ziyuan Liu,Alois Knoll*

Main category: cs.RO

TL;DR: ReMem-VLA introduces recurrent memory queries for vision-language-action models to handle tasks requiring historical context, outperforming existing memory-free and memory-based VLAs on memory-dependent tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models for robot control rely on Markov assumptions and struggle with tasks requiring historical context. Current memory approaches either use memory banks (prone to distractors) or fixed frame windows (limited retention), creating a need for better memory mechanisms.

Method: Introduces ReMem-VLA with two sets of learnable queries: frame-level recurrent memory queries for short-term memory across consecutive frames, and chunk-level recurrent memory queries for long-term memory across temporal chunks. Also adds Past Observation Prediction as an auxiliary training objective to enhance visual memory.

Result: ReMem-VLA demonstrates strong memory capabilities across spatial, sequential, episodic, temporal, and visual memory dimensions. It significantly outperforms memory-free baselines (π0.5 and OpenVLA-OFT) and surpasses MemoryVLA on memory-dependent tasks by a large margin.

Conclusion: The recurrent memory query approach effectively incorporates historical context into VLA models for robot control without additional training or inference cost, enabling better performance on tasks requiring memory across multiple dimensions.
...
### [264] [Efficient Real-World Autonomous Racing via Attenuated Residual Policy Optimization](https://arxiv.org/abs/2603.12960)
...
<details>
  <summary>Details</summary>
Motivation: Residual policy learning (RPL) shows strong performance in robotic applications like autonomous racing but introduces system complexity and increased inference latency when deployed.

Method: α-RPO attenuates the base policy over time to create a standalone neural policy, integrates with PPO with dynamic compensation, and enables privileged learning using additional sensor modalities during training.

Result: α-RPO reduces system complexity, improves driving performance in simulation, and enables zero-shot real-world transfer to Roboracer cars compared to baselines.

Conclusion: α-RPO provides a practical solution for robotic deployment by creating standalone policies that maintain performance while reducing complexity and latency.
...
### [265] [Language-Grounded Decoupled Action Representation for Robotic Manipulation](https://arxiv.org/abs/2603.12967)
*Wuding Weng,Tongshu Wu,Liucheng Chen,Siyu Xie,Zheng Wang,Xing Xu,Jingkuan Song,Heng Tao Shen*

Main category: cs.RO

TL;DR: LaDA framework uses language to bridge vision-language understanding with robotic action control through three interpretable action primitives, improving generalization to novel tasks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the fundamental challenge of bridging high-level vision-language understanding with low-level action control in robotic manipulation. Current methods struggle with generating robust actions for novel or semantically related tasks.

Method: LaDA introduces a fine-grained intermediate layer with three interpretable action primitives (translation, rotation, gripper control) and uses semantic-guided soft-label contrastive learning to align similar action primitives across tasks. It also employs an adaptive weighting strategy inspired by curriculum learning to balance contrastive and imitation objectives.

Result: Extensive experiments on simulated benchmarks (LIBERO and MimicGen) and real-world demonstrations show that LaDA achieves strong performance and effectively generalizes to unseen or related tasks.

Conclusion: The LaDA framework successfully bridges the gap between vision-language understanding and robotic action control through language-grounded decoupled action representation, enabling better generalization to novel manipulation tasks.
...
### [266] [Route Fragmentation Based on Resource-centric Prioritisation for Efficient Multi-Robot Path Planning in Agricultural Environments](https://arxiv.org/abs/2603.12994)
...
<details>
  <summary>Details</summary>
Motivation: Agricultural environments have dense navigation bottlenecks that limit existing agent-centric MRPP approaches. Current methods focus on agent conflicts rather than resource contention, and spatial interleaving capabilities are limited by contention density in corridor-dominant agricultural settings.

Method: Two variants of priority-based Fragment Planner (FP) are presented as resource-centric MRPP algorithms that use route fragmentation to enable partial route progression and limit binary-based waiting. Evaluated in lifelong simulation over 3.6km topological map of commercial polytunnel environment.
...
Conclusion: For long-term deployment of agricultural robots in corridor-dominant environments, resource-centric MRPP approaches are necessary for high-efficacy operational planning, with route fragmentation enabling better performance in dense bottleneck scenarios.
...
### [267] [From Passive Monitoring to Active Defence: Resilient Control of Manipulators Under Cyberattacks](https://arxiv.org/abs/2603.13003)
*Gabriele Gualandi,Alessandro V. Papadopoulos*

Main category: cs.RO

TL;DR: This paper proposes an active defense mechanism for robotic manipulators against stealthy false data injection attacks, moving beyond passive anomaly detection to provide control-level protection with stability guarantees.


<details>
  <summary>Details</summary>
Motivation: Cyber-physical robotic systems are vulnerable to stealthy false data injection attacks that can evade traditional passive anomaly detectors like chi-squared tests, allowing attackers to induce significant end-effector deviations without triggering alarms.

Method: The authors develop a closed-loop model with feedback-linearized manipulator, Kalman filter, and chi-squared detector, then add an active control-level defense that attenuates control inputs based on anomaly scores from a novel actuation-projected, measurement-free state predictor.

Result: The proposed defense provides probabilistic guarantees on nominal actuation loss and preserves closed-loop stability. Simulations on a 6-DOF planar manipulator show significant reduction in attack-induced end-effector deviation while maintaining nominal task performance when no attacks are present.

Conclusion: The paper successfully advances robotic system security from passive monitoring to active defense, demonstrating effective protection against stealthy false data injection attacks while maintaining system stability and performance.
...
### [268] [SldprtNet: A Large-Scale Multimodal Dataset for CAD Generation in Language-Driven 3D Design](https://arxiv.org/abs/2603.13098)
*Ruogu Li,Sikai Li,Yao Mu,Mingyu Ding*

Main category: cs.RO

TL;DR: SldprtNet is a large-scale multimodal dataset of 242,000+ industrial CAD parts with aligned 3D models, parameterized scripts, rendered images, and natural language descriptions for semantic-driven CAD modeling and cross-modal learning.


<details>
  <summary>Details</summary>
Motivation: To address the need for comprehensive multimodal datasets for CAD modeling, geometric deep learning, and training multimodal models for 3D design, particularly for semantic-driven CAD modeling where understanding part appearance and functionality is crucial.

Method: Created dataset with 3D models in .step/.sldprt formats, developed encoder/decoder tools for parametric modeling supporting 13 CAD commands, generated composite images from 7 rendered views, used Qwen2.5-VL-7B to generate natural language descriptions, and manually verified alignment between all modalities.

Result: Successfully constructed SldprtNet with 242,000+ industrial parts featuring real-world diversity, supporting tools for scalability, and fully aligned multimodal data. Fine-tuning experiments showed multimodal (image+text) inputs outperform text-only inputs for CAD generation tasks.

Conclusion: SldprtNet demonstrates the necessity and value of multimodal datasets for CAD generation, providing a comprehensive resource for semantic-driven CAD modeling, geometric deep learning, and cross-modal learning with real-world industrial applications.
...
### [269] [Evaluating VLMs' Spatial Reasoning Over Robot Motion: A Step Towards Robot Planning with Motion Preferences](https://arxiv.org/abs/2603.13100)
*Wenxi Wu,Jingjing Zhang,Martim Brandão*

Main category: cs.RO

TL;DR: VLMs show moderate spatial reasoning capability for robot motion preferences, with Qwen2.5-VL achieving 71.4% accuracy zero-shot, but overall performance remains limited.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether Vision-Language Models (VLMs) have sufficient spatial reasoning capabilities to understand and enforce user preferences or constraints on robot motion, such as desired distances from objects and motion style preferences.

Method: Evaluated four state-of-the-art VLMs using four different querying methods on spatial reasoning tasks related to robot motion. Tested two types of motion preferences: object-proximity and path-style. Analyzed accuracy vs. computation cost trade-off in token usage.

Result: Qwen2.5-VL achieved 71.4% accuracy zero-shot and 75% after fine-tuning on a smaller model. GPT-4o showed lower performance. The study revealed moderate spatial reasoning capabilities but highlighted limitations in current VLM performance for robot motion planning.

Conclusion: VLMs show some promise for integration with robot motion planning pipelines, but current spatial reasoning capabilities are limited. The work provides a benchmark for evaluating VLM spatial reasoning and highlights the need for improved models and querying methods.
...
### [270] [A Feasibility-Enhanced Control Barrier Function Method for Multi-UAV Collision Avoidance](https://arxiv.org/abs/2603.13103)
*Qishen Zhong,Junlong Wu,Jian Yang,Guanwei Xiao,Junqi Wu,Zimeng Jiang,Pingan Fang*

Main category: cs.RO

TL;DR: FECBF framework enhances feasibility of CBF-QP for multi-UAV collision avoidance by addressing internal incompatibility among CBF constraints through sign-consistency constraints.


<details>
  <summary>Details</summary>
Motivation: In dense multi-UAV scenarios, the feasibility of CBF-QP can be compromised due to internal incompatibility among multiple CBF constraints, which hinders effective collision avoidance.

Method: Analyze internal compatibility of CBF constraints, derive sufficient condition for compatibility, introduce sign-consistency constraint, incorporate into decentralized CBF-QP using worst-case estimates and slack variables.

Result: Significantly reduces infeasibility and improves collision avoidance performance compared to baselines in dense scenarios, robust under varying time delays, validated through real-world experiments.

Conclusion: The FECBF framework effectively addresses internal incompatibility in CBF constraints, enhancing feasibility and collision avoidance performance for multi-UAV systems in dense environments.
...
### [271] [DecoVLN: Decoupling Observation, Reasoning, and Correction for Vision-and-Language Navigation](https://arxiv.org/abs/2603.13133)
...
<details>
  <summary>Details</summary>
Motivation: Existing VLN approaches struggle with constructing effective long-term memory banks and overcoming compounding errors during long-horizon navigation in complex 3D environments.

Method: 1) Formulates long-term memory construction as an optimization problem with adaptive refinement mechanism that selects frames from historical pool using unified scoring function balancing semantic relevance, visual diversity, and temporal coverage. 2) Introduces state-action pair-level corrective finetuning using geodesic distance to quantify deviation from expert trajectory, collecting high-quality data while filtering polluted data.

Result: Extensive experiments demonstrate effectiveness of DecoVLN, and it has been deployed in real-world environments.
...
### [272] [Context-Enriched Natural Language Descriptions of Vessel Trajectories](https://arxiv.org/abs/2603.12287)
...
TL;DR: Framework transforms raw AIS vessel trajectories into structured, semantically enriched representations with contextual information, enabling LLM-generated natural language descriptions for maritime analytics.


<details>
  <summary>Details</summary>
Motivation: Raw AIS trajectory data is noisy and lacks semantic structure, making it difficult for humans to interpret and for machine reasoning systems to use effectively. There's a need to transform this data into enriched representations that can support higher-level maritime analytics and integration with modern AI systems like LLMs.

Method: Proposes a context-aware trajectory abstraction framework that: 1) segments noisy AIS sequences into distinct trips, 2) further divides trips into clean, mobility-annotated episodes, 3) enriches each episode with multi-source contextual information (geographic entities, offshore navigation features, weather conditions), and 4) enables generation of controlled natural language descriptions using LLMs.

Result: The framework produces structured representations that increase semantic density and reduce spatiotemporal complexity. Empirical examination shows the quality of natural language descriptions generated by several LLMs using the enriched AIS data with open contextual features.

Conclusion: The abstraction framework facilitates downstream analytics and enables integration with LLMs for higher-level maritime reasoning tasks by transforming raw trajectory data into interpretable, semantically rich representations that bridge the gap between raw sensor data and human/machine understanding.
...
### [273] [Efficient Reasoning with Balanced Thinking](https://arxiv.org/abs/2603.12372)
*Yulin Li,Tengyao Tu,Li Ding,Junjie Wang,Huiling Zhen,Yixin Chen,Yong Li,Zhuotao Tian*

Main category: cs.AI

TL;DR: ReBalance is a training-free framework that uses confidence monitoring to dynamically balance reasoning in Large Reasoning Models, reducing overthinking and underthinking to improve efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: LRMs suffer from overthinking (wasting computation on simple problems) and underthinking (failing to explore sufficient reasoning paths), leading to inefficiencies and inaccuracies. Existing methods to fix overthinking often cause underthinking, creating a trade-off problem.

Method: Uses confidence as a continuous indicator to detect overthinking (high confidence variance) and underthinking (consistent overconfidence). Aggregates hidden states from a small dataset into reasoning mode prototypes, computes a steering vector to guide reasoning trajectories, and uses a dynamic control function to modulate vector strength/direction based on real-time confidence.

Result: Extensive experiments on 4 models (0.5B to 32B) across 9 benchmarks in math reasoning, QA, and coding show ReBalance effectively reduces output redundancy while improving accuracy.

Conclusion: ReBalance offers a general, training-free, plug-and-play strategy for efficient and robust LRM deployment by dynamically balancing reasoning processes to prevent both overthinking and underthinking.
...
### [274] [Generating Expressive and Customizable Evals for Timeseries Data Analysis Agents with AgentFuel](https://arxiv.org/abs/2603.12483)
*Aadyaa Maddi,Prakhar Naval,Deepti Mande,Shane Duan,Muckai Girish,Vyas Sekar*

Main category: cs.AI

TL;DR: AgentFuel is a framework for creating customized evaluations for conversational data analysis agents working with timeseries data, addressing expressivity gaps in existing evals.


<details>
  <summary>Details</summary>
Motivation: Existing conversational data analysis agents fail on stateful and incident-specific queries for timeseries data in domains like IoT, observability, and cybersecurity. Current evaluations lack domain-customized datasets and domain-specific query types.

Method: AgentFuel helps domain experts create customized evals for end-to-end functional testing of timeseries data agents. It addresses expressivity gaps by enabling generation of domain-specific datasets and query types.

Result: AgentFuel benchmarks expose key improvement directions for existing data agent frameworks and show anecdotal evidence of performance improvement (e.g., with GEPA). Benchmarks are publicly available on HuggingFace.

Conclusion: AgentFuel provides a practical solution for domain experts to create expressive evaluations for conversational data analysis agents, helping identify and address limitations in current agent frameworks for timeseries data analysis.
...
### [275] [AI Planning Framework for LLM-Based Web Agents](https://arxiv.org/abs/2603.12710)
*Orit Shahnovsky,Rotem Dror*

Main category: cs.AI

TL;DR: This paper introduces a framework that maps modern LLM web agents to traditional planning paradigms (BFS, DFS, Best-First Search) and proposes new evaluation metrics beyond success rates to better diagnose agent failures and select appropriate architectures.


<details>
  <summary>Details</summary>
Motivation: LLM agents for web tasks operate as black boxes, making it difficult to diagnose failures or understand their planning processes. There's a need for principled frameworks to analyze agent behaviors and evaluate them beyond simple success rates.

Method: The authors develop a taxonomy mapping agent architectures to planning paradigms: Step-by-Step agents to BFS, Tree Search agents to Best-First Tree Search, and Full-Plan-in-Advance agents to DFS. They propose five novel evaluation metrics and create a dataset of 794 human-labeled trajectories from WebArena. They validate the framework by comparing baseline Step-by-Step agents against novel Full-Plan-in-Advance implementations.

Result: Step-by-Step agents align more closely with human gold trajectories (38% overall success), while Full-Plan-in-Advance agents excel in technical measures like element accuracy (89%). The proposed metrics reveal trade-offs between different agent architectures that simple success rates would miss.

Conclusion: The planning paradigm framework provides principled diagnosis of agent failures, and the proposed evaluation metrics are necessary for selecting appropriate agent architectures based on specific application constraints, moving beyond simplistic success rate comparisons.
...
### [276] [On Using Machine Learning to Early Detect Catastrophic Failures in Marine Diesel Engines](https://arxiv.org/abs/2603.12733)
*Francesco Maione,Paolo Lino,Giuseppe Giannino,Guido Maione*

Main category: cs.AI

TL;DR: A novel method for early detection of catastrophic marine engine failures using derivatives of sensor deviation signals and Random Forest classification, enabling advance warnings before critical thresholds are reached.


<details>
  <summary>Details</summary>
Motivation: Catastrophic marine engine failures are sudden, unpredictable events that pose severe threats to navigation, crew, and passengers. Current research focuses on gradual degradation modeling, leaving a gap in detecting sudden anomalous phenomena that require early warning systems.

Method: The approach analyzes derivatives of deviations between actual sensor readings and expected engine variable values. A Random Forest algorithm (selected as most suitable among tested ML methods) predicts failures. Deep Learning-based data augmentation addresses training data scarcity.

Result: The method successfully detects anomalies before measurements reach critical alarm thresholds, providing earlier warnings than traditional deviation-based approaches. Simulation results confirm effectiveness, and real-world validation demonstrates robustness and practical applicability.

Conclusion: The proposed derivative-based approach enables early detection of catastrophic engine failures, allowing operators to shut down engines, prevent damage, and safely reroute ships. This represents a significant advancement over traditional threshold-based alarm systems for marine safety.
...
### [277] [ToolTree: Efficient LLM Agent Tool Planning via Dual-Feedback Monte Carlo Tree Search and Bidirectional Pruning](https://arxiv.org/abs/2603.12740)
*Shuo Yang,Soyeon Caren Han,Yihao Ding,Shuhe Wang,Eduard Hoy*

Main category: cs.AI

TL;DR: ToolTree: A Monte Carlo tree search-inspired planning method for LLM agents that improves tool selection by exploring trajectories with dual-stage evaluation and pruning, achieving ~10% performance gains over SOTA.


<details>
  <summary>Details</summary>
Motivation: Current LLM agent tool planning methods use greedy, reactive strategies that lack foresight and fail to account for inter-tool dependencies, limiting their effectiveness in complex multi-step tasks.

Method: ToolTree uses a Monte Carlo tree search-inspired planning paradigm with dual-stage LLM evaluation (before and after tool execution) and bidirectional pruning to explore tool usage trajectories while efficiently pruning less promising branches.

Result: Empirical evaluations across 4 benchmarks show ToolTree consistently improves performance while maintaining high efficiency, achieving average gains of around 10% compared to state-of-the-art planning methods.

Conclusion: ToolTree provides an effective planning framework that enables LLM agents to make informed, adaptive decisions over extended tool-use sequences, addressing limitations of current reactive tool selection approaches.
...
### [278] [AI Model Modulation with Logits Redistribution](https://arxiv.org/abs/2603.12755)
*Zihan Wang,Zhongkui Ma,Xinguo Feng,Zhiyang Mei,Ethan Ma,Derui Wang,Minhui Xue,Guangdong Bai*

Main category: cs.AI

TL;DR: AIM is a model modulation paradigm that enables a single model to exhibit diverse behaviors through utility and focus modulations without retraining, using a logits redistribution strategy.


<details>
  <summary>Details</summary>
Motivation: Maintaining multiple specialized versions of large-scale models for different requirements is inefficient. There's a need for a single model that can adapt to diverse end requirements dynamically.
...
Result: Evaluation confirms AIM's practicality and versatility across tasks including image classification, semantic segmentation, and text generation, and across prevalent architectures including ResNet, SegFormer, and Llama.

Conclusion: AIM provides an efficient solution for model modulation that allows a single model to meet diverse requirements through dynamic control mechanisms without the need for maintaining multiple specialized versions or retraining.
...
### [279] [Context is all you need: Towards autonomous model-based process design using agentic AI in flowsheet simulations](https://arxiv.org/abs/2603.12813)
*Pascal Schäfer,Lukas J. Krinke,Martin Wlotzka,Norbert Asprion*

Main category: cs.AI

TL;DR: Agentic AI framework using LLMs to assist with chemical process flowsheet modeling in industrial simulation environments, demonstrated through multi-agent system for engineering problem-solving and code generation.
...
Method: Developed a multi-agent system where one agent solves abstract problems using engineering knowledge and another implements solutions as Chemasim code, leveraging GitHub Copilot with state-of-the-art LLMs (Claude Opus 4.6) trained on technical documentation and examples.

Result: Demonstrated effectiveness for typical flowsheet modeling examples including reaction/separation process, pressure-swing distillation, and heteroazeotropic distillation with entrainer selection, showing capability to generate valid syntax for in-house tool Chemasim.
...
### [280] [ODRL Policy Comparison Through Normalisation](https://arxiv.org/abs/2603.12926)
*Jaime Osvaldo Salas,Paolo Pareti,George Konstantinidis*

Main category: cs.AI

TL;DR: ODRL policy normalization approach that transforms complex policies into minimal components, converting permissions/prohibitions to permissions-only and simplifying constraints, enabling interoperability and easier comparison.
...
Method: Parametrized normalization approach that transforms ODRL policies into minimal components: reformulates permissions/prohibitions into permissions-only policies, simplifies complex logic constraints into simple ones, with algorithms for computing normal forms and simplifying constraints.

Result: Algorithms preserve policy semantics; size complexity is exponential on number of attributes and linear on number of unique attribute values; enables representation of complex policies in basic ODRL fragments and reduces policy comparison to checking rule identity.

Conclusion: The normalization approach addresses ODRL complexity barriers by enabling interoperability, simplifying policy comparison, and making complex policies representable in basic ODRL fragments.
...
### [281] [Efficient and Interpretable Multi-Agent LLM Routing via Ant Colony Optimization](https://arxiv.org/abs/2603.12933)
...
TL;DR: AMRO-S is an efficient, interpretable routing framework for LLM-driven Multi-Agent Systems that models routing as semantic-conditioned path selection using intent inference, task-specific pheromone specialists, and quality-gated asynchronous updates.


<details>
  <summary>Details</summary>
Motivation: Real-world deployment of LLM-driven Multi-Agent Systems faces challenges with high inference costs, latency, limited transparency, and inefficient routing. Existing routing strategies use expensive LLM-based selectors or static policies with limited controllability for semantic-aware routing under dynamic loads, leading to unstable performance and poor resource utilization.

Method: AMRO-S models MAS routing as semantic-conditioned path selection with three key mechanisms: 1) Uses supervised fine-tuned small language model for low-overhead intent inference, 2) Decomposes routing memory into task-specific pheromone specialists to reduce cross-task interference, 3) Employs quality-gated asynchronous update mechanism to decouple inference from learning.
...
Conclusion: AMRO-S addresses key limitations in MAS routing by providing an efficient, interpretable framework that enhances routing performance through semantic-aware mechanisms while maintaining low latency and offering transparency through structured pheromone patterns.
...
### [282] [Structured Distillation for Personalized Agent Memory: 11x Token Reduction with Retrieval Preservation](https://arxiv.org/abs/2603.13017)
*Sydney Lewis*

Main category: cs.AI

TL;DR: Personalized agent memory system compresses conversation history 11x (371→38 tokens/exchange) while maintaining 96% retrieval effectiveness, enabling thousands of exchanges to fit in a single prompt.


<details>
  <summary>Details</summary>
Motivation: Long AI conversations are useful but expensive to carry verbatim; need compact retrieval layer that preserves personalized memory while reducing context costs.

Method: Distills each exchange into structured object with four fields (exchange_core, specific_context, thematic room_assignments, regex-extracted files_touched). Evaluated with 201 queries, 107 search configurations across 5 pure and 5 cross-layer modes, using 5 LLM graders on 4,182 conversations.

Result: Achieves 11x compression (371→38 tokens/exchange). Best distilled configuration reaches 96% of verbatim MRR (0.717 vs 0.745). Vector search unaffected, BM25 degrades significantly. Best cross-layer setup slightly exceeds pure verbatim baseline (MRR 0.759).

Conclusion: Structured distillation compresses single-user agent memory without uniformly sacrificing retrieval quality, enabling thousands of exchanges in a single prompt while keeping verbatim source available for drill-down.
...
### [283] [Beyond Final Answers: CRYSTAL Benchmark for Transparent Multimodal Reasoning Evaluation](https://arxiv.org/abs/2603.13099)
*Wayner Barrios,SouYoung Jin*

Main category: cs.AI

TL;DR: CRYSTAL is a diagnostic benchmark with 6,372 instances for evaluating multimodal reasoning through verifiable intermediate steps, featuring two metrics (Match F1 and Ordered Match F1) and revealing systematic failures in MLLMs like cherry-picking and disordered reasoning.


<details>
  <summary>Details</summary>
Motivation: Current multimodal reasoning evaluation focuses on final answer accuracy, missing systematic failures in reasoning processes like cherry-picking, disordered chains, and non-monotonic scaling trade-offs that are invisible to traditional metrics.

Method: Created CRYSTAL benchmark via Delphi-inspired pipeline: four independent MLLMs generate reasoning trajectories, aggregated through semantic clustering and human validation. Proposed Causal Process Reward (CPR) - multiplicative reward coupling answer correctness with step alignment, and CPR-Curriculum that progressively increases reasoning difficulty during training.

Result: Evaluation of 20 MLLMs reveals universal cherry-picking (precision far exceeds recall), non-monotonic scaling trade-offs, and disordered reasoning where no competitive model preserves more than 60% of matched steps in correct order. CPR-Curriculum achieves +32% Match F1 via GRPO where additive reward strategies fail.

Conclusion: CRYSTAL exposes critical reasoning deficiencies in MLLMs invisible to accuracy metrics. The proposed CPR and CPR-Curriculum offer effective training strategies to improve reasoning alignment without manual step annotation, addressing systematic failures in multimodal reasoning.
...
### [284] [Steve-Evolving: Open-World Embodied Self-Evolution via Fine-Grained Diagnosis and Dual-Track Knowledge Distillation](https://arxiv.org/abs/2603.13131)
*Zhengwei Xie,Zhisheng Chen,Ziyan Weng,Tingyu Wu,Chenglong Li,Vireo Zhang,Kun Wang*

Main category: cs.AI

TL;DR: Steve-Evolving: A non-parametric self-evolving framework for embodied agents that organizes interaction experience through fine-grained execution diagnosis and dual-track knowledge distillation in a closed loop.


<details>
  <summary>Details</summary>
Motivation: Open-world embodied agents need to solve long-horizon tasks where the main bottleneck is not single-step planning quality but how interaction experience is organized and evolved over time.

Method: Three-phase framework: 1) Experience Anchoring - solidifies subgoal attempts into structured experience tuples with multi-dimensional indexing; 2) Experience Distillation - generalizes successes into reusable skills and failures into executable guardrails; 3) Knowledge-Driven Closed-Loop Control - injects retrieved skills/guardrails into LLM planner with diagnosis-triggered replanning.
...
Conclusion: Steve-Evolving enables continual evolution of embodied agents without model parameter updates through systematic experience organization and closed-loop knowledge distillation.
...
### [285] [When Right Meets Wrong: Bilateral Context Conditioning with Reward-Confidence Correction for GRPO](https://arxiv.org/abs/2603.13134)
*Yu Li,Tian Lan,Zhengling Qi*

Main category: cs.AI

TL;DR: The paper introduces Bilateral Context Conditioning (BICC) and Reward-Confidence Correction (RCC) to enhance GRPO by leveraging contrastive information between correct and incorrect reasoning traces within groups.


<details>
  <summary>Details</summary>
Motivation: GRPO treats outputs as independent samples and misses the valuable structural signal of contrasting correct vs incorrect solutions within the same group. The authors aim to leverage this comparative data to improve reasoning model training.

Method: 1) Contrastive reformulation of GRPO showing it implicitly maximizes margin between policy ratios of correct/incorrect samples. 2) Bilateral Context Conditioning (BICC) enables cross-referencing successful and failed reasoning traces during optimization. 3) Reward-Confidence Correction (RCC) stabilizes training by dynamically adjusting advantage baseline using reward-confidence covariance.

Result: Experiments on mathematical reasoning benchmarks show consistent improvements across comprehensive models and algorithms. The methods require no additional sampling or auxiliary models and can be adapted to all GRPO variants.

Conclusion: The proposed BICC and RCC mechanisms effectively leverage contrastive information between correct and incorrect reasoning traces to enhance GRPO training, demonstrating improved performance on reasoning tasks while maintaining computational efficiency.
...
### [286] [Developing and evaluating a chatbot to support maternal health care](https://arxiv.org/abs/2603.13168)
...
TL;DR: A maternal health chatbot for India combining stage-aware triage, hybrid retrieval, and evidence-conditioned generation, with a novel evaluation workflow for high-stakes deployment under limited expert supervision.


<details>
  <summary>Details</summary>
Motivation: Maternal health chatbots can significantly impact low-resource settings with low health literacy and limited care access, but deployment is challenging due to short, underspecified, code-mixed queries, regional context requirements, and incomplete symptom information.
...
Result: Triage benchmark (N=150) achieved 86.7% emergency recall with explicit trade-off reporting; synthetic retrieval benchmark (N=100) with chunk-level evidence labels; LLM-as-judge comparison on real queries (N=781) using clinician-codesigned criteria; expert validation shows system effectiveness.
...
### [287] [Semantic Invariance in Agentic AI](https://arxiv.org/abs/2603.13173)
*I. de Zarzà,J. de Curtò,Jordi Cabot,Pietro Manzoni,Carlos T. Calafate*

Main category: cs.AI

TL;DR: LLM reasoning agents lack semantic invariance - their outputs change under semantically equivalent input variations. A metamorphic testing framework with 8 semantic transformations reveals that larger models are not more robust, with Qwen3-30B-A3B showing highest stability.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used as autonomous reasoning agents in critical applications, but standard benchmarks fail to assess whether their reasoning remains stable under semantically equivalent input variations (semantic invariance). This reliability gap needs systematic evaluation.

Method: Developed a metamorphic testing framework applying eight semantic-preserving transformations (identity, paraphrase, fact reordering, expansion, contraction, academic context, business context, contrastive formulation) across 7 foundation models from 4 architectural families, evaluated on 19 multi-step reasoning problems across 8 scientific domains.

Result: Model scale does not predict robustness. Qwen3-30B-A3B (smaller model) achieved highest stability (79.6% invariant responses, semantic similarity 0.91), while larger models exhibited greater fragility. Semantic invariance varies significantly across models and transformations.

Conclusion: Current LLM reasoning agents lack semantic invariance, posing reliability risks for consequential applications. Standard accuracy benchmarks are insufficient; systematic robustness testing is needed. Model size doesn't guarantee stability, highlighting the need for specialized evaluation frameworks.
