...
TL;DR: Text-guided fairness enhancement for facial gender classification using semantic information from image captions to reduce demographic bias without requiring demographic labels.


<details>
  <summary>Details</summary>
Motivation: Address demographic bias and fairness issues in facial image-based gender classification algorithms, particularly across gender and racial groups, by leveraging semantic information to create more equitable AI systems.

Method: Two text-guided approaches: 1) Image Text Matching (ITM) guidance that trains models to discern fine-grained alignments between images and texts for enhanced multimodal representations, and 2) Image Text fusion that combines both modalities into comprehensive representations. Both methods operate without demographic labels.

Result: Extensive experiments on benchmark datasets show these approaches effectively mitigate bias and improve accuracy across gender and racial groups compared to existing methods, while providing interpretable and intuitive training.

Conclusion: The research demonstrates that semantic information from text captions can significantly reduce disparities in facial analysis algorithms, offering valuable insights for cultivating more equitable gender classification systems without requiring demographic labels.
...
TL;DR: SoccerMaster is the first soccer-specific vision foundation model that unifies diverse visual understanding tasks (from detection to event classification) via supervised multi-task pretraining, outperforming task-specific expert models.


<details>
  <summary>Details</summary>
Motivation: Soccer understanding has unique domain-specific complexities, but prior works rely on isolated, task-specific expert models. There's a need for a unified approach to handle diverse soccer visual understanding tasks.

Method: Developed SoccerMaster, a soccer-specific vision foundation model using supervised multi-task pretraining. Created an automated data curation pipeline to generate scalable spatial annotations and integrated them with existing datasets to build SoccerFactory, a comprehensive pretraining resource.
...
Conclusion: The proposed unified approach effectively handles diverse soccer visual understanding tasks, offering a comprehensive solution that surpasses traditional task-specific models. The data, code, and model will be publicly available.
...
TL;DR: This paper proposes a knowledge distillation method using ResNet50 to reduce spurious correlations in TB detection from chest X-rays, achieving 0.2428 mIOU without requiring bounding-box annotations.


<details>
  <summary>Details</summary>
Motivation: TB remains a major global health issue, especially in resource-limited settings. While CXR is accessible for diagnosis, expert interpretation is scarce. Existing ML models for TB classification often rely on spurious correlations and fail to generalize well. Additionally, creating large, high-quality annotated medical datasets is expensive and logistically challenging, requiring multiple expert annotators to reach consensus.

Method: The study repurposes knowledge distillation technique to train CNN models that reduce spurious correlations and localize TB-related abnormalities without needing bounding-box annotations. The method uses a teacher-student framework with ResNet50 architecture, trained on the TBX11k dataset.

Result: The proposed method achieves an impressive 0.2428 mIOU score. Experimental results show that the student model consistently outperforms the teacher model, demonstrating improved robustness and potential for broader clinical deployment in diverse settings.

Conclusion: The knowledge distillation approach successfully reduces spurious correlations in TB detection from chest X-rays while eliminating the need for expensive bounding-box annotations. The student model's superior performance over the teacher model indicates enhanced robustness, making this method promising for clinical deployment in resource-limited settings where expert interpretation is scarce.
...
TL;DR: SVR framework synthesizes realistic retinal vasculature images with DR pathologies and corresponding reasoning texts to train VLMs for medical diagnosis, achieving 89.67% zero-shot accuracy on real OCTA images.


<details>
  <summary>Details</summary>
Motivation: VLMs need large-scale image-text datasets for medical reasoning, but specialized domains like OCTA imaging lack precise text descriptions of pathologies. There's a need for synthetic data generation to overcome this bottleneck.

Method: Introduces Synthetic Vasculature Reasoning (SVR) framework that controllably synthesizes realistic retinal vasculature images with DR features (capillary dropout, microaneurysms, neovascularization, tortuosity) and automatically generates granular reasoning texts. Creates OCTA-100K-SVR dataset with 100,000 image-text pairs.

Result: General-purpose VLM (Qwen3-VL-8b) trained on OCTA-100K-SVR achieves 89.67% zero-shot balanced classification accuracy on real OCTA images, outperforming supervised baselines. Human expert evaluation shows significant improvement in explanation quality and pathology localization on clinical data.

Conclusion: SVR framework effectively addresses the data scarcity problem in specialized medical domains by generating synthetic image-text pairs, enabling VLMs to achieve state-of-the-art performance in medical diagnosis with interpretable reasoning capabilities.
...
TL;DR: This paper surveys the evolution of urban dynamics monitoring, focusing on event-based cameras as a promising technology for studying city movements while addressing privacy concerns.


<details>
  <summary>Details</summary>
Motivation: Traditional urban monitoring methods have limitations, and there's a need for better approaches to understand city dynamics while addressing privacy concerns and technical challenges.

Method: The paper conducts a comprehensive survey analyzing event-based cameras, their applications, advantages, challenges, and machine learning applications in urban dynamics research.

Result: Event-based cameras offer unique advantages for urban monitoring including low-light operation and privacy preservation, and multi-sensor fusion with infrared, LiDAR, or vibration sensors can enhance their capabilities.

Conclusion: Event-based cameras represent a promising medium for studying urban dynamics, with multi-sensor fusion approaches offering solutions to overcome their current limitations and enhance urban monitoring capabilities.
...
TL;DR: VDAWorld: A new paradigm for world modeling that distills image caption pairs into abstract representations optimized for simulation, using VLMs to orchestrate scene construction and physics simulation.


<details>
  <summary>Details</summary>
Motivation: Current generative video models violate physical/logical rules, lack interactivity, and are opaque black boxes unsuitable for building structured, queryable worlds.

Method: VDAWorld framework uses Vision-Language Models as intelligent agents to distill image caption pairs into tractable abstract representations. The VLM autonomously constructs grounded (2D/3D) scene representations using vision tools and selects compatible physics simulators (rigid body, fluid) to act upon them, then infers latent dynamics to predict future states.

Result: The combination of intelligent abstraction and adaptive simulation produces a versatile world model capable of high-quality simulations across diverse dynamic scenarios.

Conclusion: VDAWorld overcomes limitations of generative video models by providing a structured, queryable world modeling approach that respects physical/logical rules through intelligent abstraction and adaptive simulation.
...
TL;DR: LEGO is a three-stage framework for adapting pre-trained image restoration models to unseen real-world degradations without paired data, using a generative oracle to create pseudo-ground-truths for fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Pre-trained image restoration models fail on real-world out-of-distribution degradations due to domain gaps, and existing adaptation methods require complex architectural changes or paired data which is unavailable for unseen domains.

Method: Three-stage framework: 1) Get initial restorations from pre-trained model, 2) Use frozen large-scale generative oracle to refine these into high-quality pseudo-ground-truths, 3) Fine-tune original model using mixed-supervision combining in-distribution data with pseudo-pairs.
...
<details>
  <summary>Details</summary>
Motivation: Manufacturing environments often have low-light conditions where conventional vision systems fail. Infrared cameras work well in these settings, but supervised AI requires large labeled datasets. While vision-language models offer zero-shot capabilities, they can't process infrared data since they're trained on RGB images.

Method: VLM-IRIS preprocesses infrared images from FLIR Boson sensors into RGB-compatible inputs using magma color representation. It applies centroid prompt ensembling with CLIP ViT-B/32 encoder for zero-shot predictions without model retraining.
...
TL;DR: This paper introduces Embodied Image Compression (EIC) as a new research problem for compressing visual data for embodied AI agents operating in real-world environments, establishes a benchmark called EmbodiedComp for evaluation under ultra-low bitrate conditions, and shows current VLAs fail at simple tasks below embodied bitrate thresholds.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the shift in compression targets from task-specific virtual models to embodied agents in real-world environments, addressing communication constraints in multi-agent systems and ensuring real-time task execution for embodied AI.

Method: The paper introduces the scientific problem of Embodied Image Compression, establishes a standardized benchmark called EmbodiedComp for systematic evaluation under ultra-low bitrate conditions in closed-loop settings, and conducts extensive empirical studies in both simulated and real-world settings.

Result: The results show that existing Vision-Language-Action models (VLAs) fail to reliably perform even simple manipulation tasks when compressed below the Embodied bitrate threshold, demonstrating the need for specialized compression methods for embodied agents.

Conclusion: The paper concludes that EmbodiedComp will catalyze the development of domain-specific compression tailored for embodied agents, thereby accelerating the deployment of Embodied AI in real-world applications.
...
TL;DR: VGent is a modular visual grounding system that separates reasoning (frozen MLLM encoder) from box prediction (detector-based decoder), achieving SOTA performance with fast inference.


<details>
  <summary>Details</summary>
Motivation: Current visual grounding models either use slow auto-regressive MLLMs prone to hallucinations, or realign LLMs with vision features which compromises pretrained reasoning abilities. There's a need for a system that preserves reasoning power while enabling efficient, accurate grounding.

Method: Modular encoder-decoder architecture: frozen MLLM encoder for reasoning, decoder with detector-proposed boxes as queries that select targets via cross-attention. Includes QuadThinker (RL training for multi-target reasoning), mask-aware labels for detection-segmentation ambiguity, and global target recognition.

Result: Achieves new SOTA with +20.6% F1 improvement over prior methods on multi-target benchmarks, +8.2% gIoU and +5.8% cIoU gains under visual reference challenges, while maintaining constant fast inference latency.

Conclusion: VGent successfully disentangles reasoning from box prediction, leveraging advances in both object detection and MLLMs without compromising either. The modular design enables independent upgrades and avoids auto-regressive decoding pitfalls.
...
TL;DR: EditMGT: First MGT-based image editing framework using Masked Generative Transformers instead of diffusion models, achieving localized editing with region-hold sampling and attention-based localization, 6x faster with fewer than 1B parameters.


<details>
  <summary>Details</summary>
Motivation: Diffusion models conflate local editing targets with full-image context, causing unintended modifications in non-target regions. MGTs offer localized decoding paradigm that can explicitly preserve non-relevant regions during editing.

Method: 1) Uses MGT's cross-attention maps for localization with multi-layer attention consolidation; 2) Region-hold sampling restricts token flipping to low-attention areas; 3) Adapts pre-trained text-to-image MGT via attention injection without new parameters; 4) Trained on CrispEdit-2M dataset (7 editing categories).

Result: Achieves similarity performance with 6x faster editing using <1B parameters. Improves style change by 3.6% and style transfer by 17.6% compared to baselines. Preserves non-target regions better than diffusion models.

Conclusion: MGTs are promising alternative to diffusion models for image editing, offering inherent localized decoding that better preserves non-target regions while being faster and more parameter-efficient.
...
TL;DR: Intelligent correlation-guided fusion of multiple pathology foundation models improves cancer grading/staging performance across kidney, prostate, and rectal cancers compared to single models or naive fusion.


<details>
  <summary>Details</summary>
Motivation: While foundation models show strong performance in pathology tasks, there's limited understanding of their complementarity, redundancy in embedding spaces, and biological feature interpretation. The study aims to develop an information-driven fusion strategy to integrate multiple pathology FMs for better cancer analysis.

Method: Used H&E whole-slide images from kidney (519), prostate (490), and rectal (200) cancers dichotomized into low vs high grade/stage. Evaluated tile-level FMs (Conch v1.5, MUSK, Virchow2, H-Optimus1, Prov-Gigapath) and slide-level FMs (TITAN, CHIEF, MADELEINE). Compared three fusion schemes: majority-vote ensembling, naive feature concatenation, and intelligent fusion based on correlation-guided pruning of redundant features using patient-stratified cross-validation.

Result: Intelligent fusion of tile-level embeddings consistently outperformed best single FMs and naive fusion across all three cancers. Global similarity metrics showed substantial embedding space alignment but lower local neighborhood agreement, indicating complementary fine-grained information. Attention maps revealed concentrated focus on tumor regions with reduced spurious attention to benign areas.

Conclusion: Correlation-guided intelligent fusion of pathology foundation models produces compact, task-tailored representations that enhance both predictive performance and interpretability in computational pathology tasks.
...
TL;DR: Fast-FoundationStereo achieves real-time stereo matching with strong zero-shot generalization using knowledge distillation, neural architecture search, and structured pruning.


<details>
  <summary>Details</summary>
Motivation: Stereo foundation models have strong zero-shot generalization but are too slow for real-time applications, while efficient stereo architectures sacrifice robustness and require per-domain fine-tuning. There's a need to bridge this gap between accuracy and speed.

Method: Three-part divide-and-conquer acceleration strategy: (1) knowledge distillation to compress hybrid backbone into efficient student, (2) blockwise neural architecture search for optimal cost filtering designs with exponential search complexity reduction, (3) structured pruning for redundancy elimination in iterative refinement module. Plus automatic pseudo-labeling pipeline curating 1.4M in-the-wild stereo pairs to supplement synthetic training data.
...
Conclusion: Fast-FoundationStereo successfully bridges the gap between accuracy and speed in stereo vision, achieving strong zero-shot generalization at real-time frame rates for the first time.
...
TL;DR: ItemizedCLIP is a framework that learns visual representations from itemized text annotations (multiple independent text items per image) using cross-attention and tailored objectives for independence and completeness.


<details>
  <summary>Details</summary>
Motivation: Many visual domains like medical imaging and remote sensing have itemized text annotations where multiple independent text items describe different findings within a single image, which differs from standard multi-caption supervision where captions are redundant.

Method: Uses cross-attention module to produce text item-conditioned visual embeddings, with tailored objectives that enforce item independence (distinct regions for distinct items) and representation completeness (coverage of all items).

Result: Achieves substantial improvements in zero-shot performance and fine-grained interpretability over baselines across four domains with naturally itemized text supervision (brain MRI, head CT, chest CT, remote sensing) and one synthetic dataset.

Conclusion: ItemizedCLIP produces semantically grounded, item-differentiable, complete, and visually interpretable representations that effectively handle itemized text supervision in specialized visual domains.
...
TL;DR: The paper presents a reproduction study of the Monkey VLM, confirming its tiling approach for high-resolution image understanding while investigating global context effects and reporting task-dependent performance variations.


<details>
  <summary>Details</summary>
Motivation: To address reproducibility challenges in complex multimodal models by providing transparent implementation details and accessible training infrastructure through a detailed reproduction of the Monkey VLM.

Method: Replicated the Monkey VLM's image tiling strategy using open checkpoints and reimplemented the training pipeline, then extended the work by investigating the effect of including global context information.

Result: Confirmed the original finding that tiling effectively recovers local details, but reported deviations in results where performance effects depend heavily on task type and tile granularity. Provided practical insights for future high-resolution multimodal modeling.

Conclusion: The reproduction study validates the core tiling approach of Monkey VLM while revealing important task-dependent performance variations and highlighting the importance of considering global context in high-resolution multimodal modeling.
...
TL;DR: LGSCV: Lightweight 3D Gaussian Splatting compression using video codecs with two-stage Morton scan and MiniPLAS for efficient 2D map generation.


<details>
  <summary>Details</summary>
Motivation: Current GS compression methods using PLAS are computationally expensive and time-consuming, limiting deployment on lightweight devices. Need faster, more efficient compression.

Method: Two-stage Morton scan (3D then 2D) creates blockwise 2D maps for video codecs. PCA reduces SH dimensionality, and MiniPLAS permutes primitives within blocks for better RD performance.

Result: Achieves over 20% RD gain vs state-of-the-art, reduces 2D map generation to ~1 second, cuts encoding time by 50% on MPEG dataset.

Conclusion: LGSCV enables efficient GS compression for lightweight devices with significant speed and quality improvements over existing methods.
...
TL;DR: The paper presents a winning solution for the BinEgo-360 Challenge at ICCV 2025, focusing on temporal action localization in multi-perspective, multi-modal videos using an extended Temporal Shift Module with multi-task learning and ensemble strategies.


<details>
  <summary>Details</summary>
Motivation: The challenge addresses temporal action localization in complex multi-perspective (panoramic, third-person, egocentric) and multi-modal video settings, requiring robust methods to handle diverse viewpoints and fine-grained action annotations.

Method: Extends Temporal Shift Module (TSM) for TAL by adding background class and classifying fixed-length non-overlapping intervals. Uses multi-task learning framework combining scene classification and TAL. Integrates multiple models through weighted ensemble strategy.

Result: Ranked first in both initial and extended rounds of the BinEgo-360 Challenge at ICCV 2025, demonstrating superior performance in temporal action localization for multi-perspective video settings.

Conclusion: The combination of multi-task learning, efficient TSM backbone, and ensemble learning proves effective for temporal action localization in complex multi-perspective and multi-modal video scenarios, achieving state-of-the-art results in the competition.
...
Method: Proposes CADKnitter framework with geometry-guided diffusion sampling strategy. Also creates KnitCAD dataset with 310,000+ CAD models, text prompts, and assembly metadata providing semantic/geometric constraints.
...
Conclusion: CADKnitter enables more practical CAD generation by addressing the compositional nature of real-world CAD design, where multiple parts must be assembled under constraints, moving beyond single-part generation limitations.
...
TL;DR: AutoRefiner is a noise refinement method for autoregressive video diffusion models that improves sample fidelity without retraining, using pathwise noise refinement and reflective KV-cache.


<details>
  <summary>Details</summary>
Motivation: AR-VDMs show promise for real-time video generation but have room for improvement in sample fidelity. Existing inference-time alignment methods are computationally impractical for AR-VDMs, and naive extensions of T2I noise refiners fail.

Method: AutoRefiner introduces two key designs: 1) Pathwise noise refinement that refines noise along stochastic denoising paths, and 2) Reflective KV-cache mechanism tailored for AR-VDMs.

Result: Experiments show AutoRefiner effectively enhances sample fidelity in AR-VDMs by refining noise along denoising paths, serving as an efficient plug-in without updating model parameters.

Conclusion: AutoRefiner successfully extends noise refinement to AR-VDMs, addressing the limitations of T2I methods and providing a practical solution for improving video generation quality in real-time applications.
...
<details>
  <summary>Details</summary>
Motivation: Wildfire smoke is visually similar to clouds and challenging to detect early, creating a need to evaluate multimodal LLMs' capabilities for safety-critical wildfire monitoring applications.
...
Result: Models can classify smoke when covering large areas but struggle with accurate localization, especially in early stages. Smoke volume strongly correlates with performance, while contrast has minor effect.

Conclusion: Current MLLMs have critical limitations for wildfire monitoring, highlighting need for improved methods for early-stage smoke localization.
...
TL;DR: A generative forecasting method using autoregressive flow matching in vision foundation model feature space, enabling stochastic prediction of multiple future modalities while preserving uncertainty.


<details>
  <summary>Details</summary>
Motivation: Existing approaches have limitations: pixel-based video generation is computationally intensive and not directly useful for decision-making, while deterministic regression on VFM features averages over plausible futures and fails to capture uncertainty.

Method: Introduces a generative forecaster that performs autoregressive flow matching in VFM feature space, encoding features into a compact latent space suitable for diffusion that preserves information better than PCA-based alternatives.

Result: With matched architecture and compute, the method produces sharper and more accurate predictions than regression across all modalities (semantic segmentation, depth, surface normals, RGB). The latent space preserves information effectively for forecasting and other applications like image generation.

Conclusion: Stochastic conditional generation of VFM features offers a promising and scalable foundation for future world models, enabling uncertainty-aware forecasting with interpretable multi-modal outputs.
...
TL;DR: FutureX enhances autonomous driving planners by using Chain of Thought reasoning to predict future scene evolution and refine trajectories, switching between Thinking mode for complex scenes and Instant mode for simple ones.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end planners rely only on current scene data, which can lead to suboptimal responses in dynamic traffic environments where ego actions affect future scenes. There's a need to model scene evolution for better motion planning.

Method: Proposes FutureX pipeline with Auto-think Switch that decides when to use Thinking mode. In Thinking mode, Latent World Model performs CoT-guided rollout to predict future scene representations, then Summarizer Module refines motion plans. For simple scenes, uses Instant mode for forward-pass planning.

Result: FutureX enhances existing methods by producing more rational motion plans with fewer collisions without compromising efficiency. Achieves 6.2 PDMS improvement for TransFuser on NAVSIM benchmark.

Conclusion: FutureX successfully integrates Chain of Thought reasoning with world modeling to improve autonomous driving planners by enabling future scene prediction and trajectory refinement, balancing computational efficiency with planning quality.
...
TL;DR: REST is the first diffusion-based real-time streaming talking head generation framework that achieves fast inference through compact video latent space, ID-Context Cache mechanism, and Asynchronous Streaming Distillation training.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based talking head generation models suffer from slow inference speeds and non-autoregressive paradigms, which severely limit their practical applications in real-time scenarios.

Method: 1) Learn compact video latent space via high spatiotemporal VAE compression; 2) Introduce ID-Context Cache mechanism for autoregressive streaming with identity coherence; 3) Propose Asynchronous Streaming Distillation to reduce error accumulation and enhance temporal consistency.

Result: REST outperforms state-of-the-art methods in both generation speed and overall performance, enabling real-time talking head generation while maintaining quality.

Conclusion: REST bridges the gap between autoregressive and diffusion-based approaches, demonstrating substantial value for real-time talking head generation applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for indoor scene generation are either limited in input modalities or rely on stochastic processes that reduce controllability, hindering applications in game development, architectural visualization, and embodied AI training.

Method: Introduces RoomPilot framework that parses multi-modal inputs (text or CAD floor plans) into an Indoor Domain-Specific Language (IDSL), which serves as a shared semantic representation. Uses a curated dataset of interaction-annotated assets to synthesize environments with realistic object behaviors.

Result: Extensive experiments show strong multi-modal understanding, fine-grained controllability in scene generation, superior physical consistency and visual fidelity compared to conventional procedural methods.

Conclusion: RoomPilot represents a significant advancement toward general-purpose controllable 3D indoor scene generation by enabling coherent, high-quality scene synthesis from diverse input modalities while maintaining interaction semantics.
...
TL;DR: WildCap enables high-quality facial appearance capture from smartphone videos in uncontrolled lighting using a hybrid inverse rendering approach with texel grid lighting and diffusion priors.


<details>
  <summary>Details</summary>
Motivation: Existing facial appearance capture methods require controlled lighting, which increases cost and limits usability. There's a need for high-quality capture from casual smartphone videos in uncontrolled environments.

Method: Proposes WildCap: 1) Uses SwitchLight (data-driven) to convert wild images to constrained conditions, 2) Model-based inverse rendering with texel grid lighting model to handle non-physical artifacts, 3) Joint optimization with diffusion priors for reflectance maps to resolve scale ambiguity between local lights and albedo.
...
Conclusion: WildCap enables high-quality facial appearance capture from casual smartphone videos without controlled lighting, making facial capture more accessible and practical for real-world applications.
...
TL;DR: ComP: Cross-modal Prompting method for incomplete multi-modal emotion recognition that enhances modality-specific features and improves accuracy through progressive prompt generation and cross-modal knowledge propagation.


<details>
  <summary>Details</summary>
Motivation: IMER faces challenges with performance gaps, modality under-optimization, and missing data issues that hinder effective multi-modal learning despite the potential of multi-source data to provide abundant information.

Method: Progressive prompt generation with dynamic gradient modulator produces consistent modality semantic cues; cross-modal knowledge propagation amplifies consistent information; coordinator dynamically re-weights modality outputs as complement to balance strategy.
...
Conclusion: The ComP method successfully addresses IMER challenges by enhancing modality-specific features, improving overall recognition accuracy, and handling missing data through coherent information emphasis and cross-modal prompting.
...
TL;DR: PersonaLive is a diffusion-based framework for real-time portrait animation with multi-stage training that achieves 7-22x speedup over previous methods.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based portrait animation models focus on visual quality but overlook generation latency and real-time performance, limiting their application in live streaming scenarios.

Method: 1) Uses hybrid implicit signals (implicit facial representations and 3D implicit keypoints) for expressive motion control. 2) Fewer-step appearance distillation to eliminate redundancy and improve efficiency. 3) Autoregressive micro-chunk streaming generation with sliding training and historical keyframe mechanism for low-latency, stable long-term video generation.

Result: Achieves state-of-the-art performance with 7-22x speedup over prior diffusion-based portrait animation models.

Conclusion: PersonaLive enables streaming real-time portrait animation with significantly improved inference efficiency while maintaining high visual quality, making it suitable for live streaming applications.
...
TL;DR: Reformer-based vision models offer theoretical O(n log n) complexity vs ViT's O(n²), but practical gains require much longer sequences than typical high-resolution images provide.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers (ViTs) have computational limitations due to quadratic self-attention scaling, making them impractical for high-resolution inputs and resource-constrained settings. The paper investigates whether Reformer architecture with locality-sensitive hashing attention can provide better efficiency while maintaining performance.

Method: Proposes a Reformer-based vision model combining patch-based tokenization with locality-sensitive hashing (LSH) attention to approximate global self-attention with reduced theoretical complexity from O(n²) to O(n log n). Evaluated on CIFAR-10 (small-scale), ImageNet-100 (accuracy-efficiency trade-off), and high-resolution medical imaging dataset (long token sequences).

Result: Reformer achieves higher accuracy on CIFAR-10 compared to ViT baseline, but ViT consistently outperforms Reformer in practical efficiency and end-to-end computation time across larger and higher-resolution settings. Theoretical advantages of LSH-based attention don't translate to meaningful computation gains with typical high-resolution image sequence lengths.

Conclusion: Despite theoretical complexity advantages, Reformer-based vision models don't provide practical efficiency benefits over standard ViTs for typical high-resolution image tasks. The sequence lengths required for meaningful computation gains are substantially longer than those produced by typical high-resolution images.
...
TL;DR: Multi-temporal Sentinel-2 satellite imagery with feature enhancement can effectively monitor invasive serrated tussock grass at landscape scale, achieving comparable accuracy to aerial imagery but with better cost-effectiveness and scalability.


<details>
  <summary>Details</summary>
Motivation: Serrated tussock is a highly invasive grass causing ecological and agricultural damage in Victoria, Australia. Current ground surveys and aerial imagery are either not scalable or too expensive for landscape-scale monitoring, creating a need for cost-effective alternatives.

Method: Developed 11 models using multi-temporal Sentinel-2 imagery with various combinations of spectral bands, texture features, vegetation indices, and seasonal data. Used random forest classifier to evaluate performance against aerial imagery models on the same dataset.

Result: Best Sentinel-2 model (M76*) achieved 68% Overall Accuracy and 0.55 Overall Kappa, slightly outperforming the best aerial imaging model (67% OA, 0.52 OK). This demonstrates comparable performance despite lower spatial resolution.

Conclusion: Multi-seasonal feature-enhanced satellite-based models provide a viable, cost-effective alternative for landscape-scale invasive species monitoring, offering scalability advantages over traditional aerial imagery approaches.
...
Method: Uses autoregressive diffusion paradigm for arbitrary-length generation. Key innovation: dual-level cache mechanism - shot memory caches keyframes from preceding shots for character/scene consistency, and temporal memory retains current shot frames for smooth motion. Supports flexible user interaction and downstream tasks like multi-concept injection and video extension.

Result: Extensive experiments show the method surpasses existing approaches on both consistency and aesthetic quality metrics. Also developed a pipeline to construct high-quality multi-shot video dataset for training.

Conclusion: FilmWeaver opens new possibilities for creating more consistent, controllable, and narrative-driven video content, addressing critical limitations in current video generation models for multi-shot synthesis.
...
TL;DR: RcAE: Recursive autoencoder with iterative reconstruction for anomaly detection, using cross-recursion tracking and detail preservation to outperform existing methods with 10x fewer parameters than diffusion models.


<details>
  <summary>Details</summary>
Motivation: Traditional autoencoder-based anomaly detection methods struggle with incomplete anomaly suppression and loss of fine details due to single-pass decoding, failing to handle anomalies with varying severity and scale effectively.

Method: Proposes Recursive Autoencoder (RcAE) with iterative reconstruction to progressively suppress anomalies while refining normal structures. Introduces Cross Recursion Detection (CRD) module to track inconsistencies across recursion steps, and Detail Preservation Network (DPN) to recover high-frequency textures lost during reconstruction.

Result: Significantly outperforms existing non-diffusion methods, achieves performance on par with recent diffusion models with only 10% of their parameters and substantially faster inference.

Conclusion: The recursive architecture with cross-recursion tracking and detail preservation offers a practical and efficient approach for real-world industrial anomaly detection applications.
...
TL;DR: ARVAE is an autoregressive video autoencoder that decouples temporal and spatial information using flow fields and spatial compensation, enabling efficient compression and reconstruction of arbitrary-length videos with superior quality using lightweight models.


<details>
  <summary>Details</summary>
Motivation: Existing video autoencoders often entangle spatial and temporal information, limiting their ability to capture temporal consistency and leading to suboptimal performance in video reconstruction and generation tasks.

Method: ARVAE compresses and reconstructs each frame conditioned on its predecessor in an autoregressive manner. It uses temporal-spatial decoupled representation combining downsampled flow fields for temporal coherence with spatial relative compensation for new content. The encoder compresses current and previous frames into temporal motion and spatial supplement, while the decoder reconstructs the original frame from these latent representations given the preceding frame. A multi-stage training strategy is employed for progressive optimization.
...
Conclusion: ARVAE effectively addresses the spatial-temporal entanglement problem in video autoencoders through autoregressive processing and decoupled representations, enabling efficient video compression and reconstruction with high quality and flexibility for arbitrary-length videos.
...
TL;DR: A few-shot VLM-based approach for verifying manually generated G-code by simultaneously evaluating both G-code text and HMI display screenshots, outperforming zero-shot methods in detecting errors and safety issues.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based G-code verification only examines programming errors but misses HMI-related issues since LLMs lack vision capabilities. CNC machining requires understanding both G-code and the Human-Machine Interface display for comprehensive error detection and safety verification.

Method: Proposes a few-shot VLM approach using paired G-code text and HMI screenshots from a 15-slant-PRO lathe. Uses structured JSON schema based on heuristic knowledge for few-shot learning, with correct and error-prone examples as few-shot prompts to guide the VLM.

Result: Few-shot VLM outperformed zero-shot VLM in detecting HMI errors and G-code discrepancies across multiple error scenarios. Showed enhanced comprehensive debugging capabilities for both programming and interface-related issues.

Conclusion: The few-shot VLM framework is suitable for verifying manually generated G-code in CNC training, providing more comprehensive error detection by simultaneously analyzing both G-code and HMI displays.
...
TL;DR: MultiEgo: First multi-view egocentric dataset for 4D dynamic scene reconstruction, featuring 5 social interaction scenes with synchronized AR glasses videos and accurate pose annotations.


<details>
  <summary>Details</summary>
Motivation: Existing reconstruction datasets focus on static multi-view or single-egocentric setups, lacking multi-view egocentric datasets for dynamic scene reconstruction needed for holographic documentation of social interactions.

Method: Created MultiEgo dataset with 5 social interaction scenes (meetings, performances, presentation) captured by 5 participants wearing AR glasses. Developed hardware-based data acquisition system and processing pipeline achieving sub-millisecond temporal synchronization and accurate pose annotations.

Result: Dataset provides synchronized multi-view egocentric videos with precise temporal alignment and pose data. Experimental validation demonstrates practical utility for free-viewpoint video (FVV) applications.

Conclusion: MultiEgo establishes a foundational resource for advancing multi-view egocentric dynamic scene reconstruction research, addressing the gap in existing datasets and enabling new applications in holographic social interaction documentation.
...
TL;DR: Proposes a visual knowledge distillation approach for MLLM unlearning that selectively erases target visual knowledge while preserving textual knowledge, outperforming existing methods in effectiveness and efficiency.


<details>
  <summary>Details</summary>
Motivation: Most existing machine unlearning methods are tailored for LLMs, while MLLM-oriented unlearning remains at its early stage. There's a need for approaches that can selectively remove sensitive visual information from multimodal large language models while preserving their textual capabilities.

Method: Disentangles visual and textual knowledge in MLLMs and introduces Visual Knowledge Distillation (VKD) scheme that leverages intermediate visual representations as supervision signals. Only fine-tunes the visual components of the MLLM for efficiency.
...
Conclusion: The proposed VKD approach provides an effective and efficient solution for MLLM unlearning by selectively erasing target visual knowledge while preserving textual capabilities, with demonstrated robustness against relearning attacks.
...
TL;DR: SATMapTR is a novel online HD map construction model that effectively fuses satellite images with BEV features using gated feature refinement and geometry-aware fusion, achieving state-of-the-art performance on nuScenes dataset.


<details>
  <summary>Details</summary>
Motivation: HD maps need real-time construction but suffer from low-quality onboard sensor data due to limitations and occlusions. Satellite images offer wide-area views but are degraded by shadows and vegetation. Existing fusion methods are ineffective.

Method: Proposes SATMapTR with two key components: (1) gated feature refinement module that adaptively filters satellite features using high-level semantics and low-level structural cues, and (2) geometry-aware fusion module that consistently fuses satellite and BEV features at grid-to-grid level.

Result: Achieves highest mAP of 73.8 on nuScenes, outperforming state-of-the-art satellite-enhanced models by up to 14.2 mAP. Shows lower degradation under adverse weather/sensor failures and nearly 3× higher mAP at extended perception ranges.

Conclusion: SATMapTR effectively addresses satellite image degradation and fusion challenges, demonstrating superior performance and robustness for real-time HD map construction in autonomous driving scenarios.
...
### [37] [Surveillance Video-Based Traffic Accident Detection Using Transformer Architecture](https://arxiv.org/abs/2512.11350)
*Tanu Singh,Pranamesh Chakraborty,Long T. Truong*

Main category: cs.CV

TL;DR: Transformer-based model for traffic accident detection achieves 88.3% accuracy by combining RGB and optical flow features, outperforming traditional methods and VLMs.


<details>
  <summary>Details</summary>
Motivation: Traffic accidents are a major global cause of mortality with rising rates. Traditional computer vision methods have limitations in spatiotemporal understanding and cross-domain generalization. Transformer architectures show promise but are hindered by small, non-diverse datasets.

Method: Created a comprehensive traffic accident dataset, then developed a transformer-based model using pre-extracted spatial video features. Used convolutional layers for local correlations and transformers for sequential-temporal dependencies. Evaluated multiple motion cue integration strategies, finding RGB+optical flow concatenation most effective.

Result: The proposed model achieved 88.3% accuracy using concatenated RGB and optical flow features. This outperformed comparison with vision language models (GPT, Gemini, LLaVA-NeXT-Video).

Conclusion: Transformer-based models with proper motion cue integration (RGB+optical flow) offer effective solutions for automated traffic accident detection, addressing limitations of traditional methods and existing datasets.
...
TL;DR: KeyframeFace introduces a large-scale multimodal dataset and LLM-based framework for text-to-facial-animation with keyframe-level supervision, enabling interpretable and expressive 3D facial animation generation from natural language.


<details>
  <summary>Details</summary>
Motivation: Existing datasets and methods for facial animation lack semantic grounding and temporal structures needed for expressive human performance generation, focusing mainly on speech-driven animation or unstructured expression sequences.

Method: Creates KeyframeFace dataset with 2,100 expressive scripts paired with videos, ARKit coefficients, contextual backgrounds, emotions, and manually defined keyframes. Proposes an LLM-based framework that leverages LLM priors to align semantic understanding with interpretable ARKit coefficient structure for facial motion synthesis.

Result: Establishes a new foundation for interpretable, keyframe-guided, and context-aware text-to-animation with high-fidelity expressive animation generation.

Conclusion: KeyframeFace dataset and LLM-based framework together provide a comprehensive solution for text-to-facial-animation research, addressing the limitations of existing approaches through semantic grounding and temporal structure.
...
TL;DR: Flowception is a non-autoregressive video generation framework that combines discrete frame insertions with continuous denoising, offering better efficiency and quality than autoregressive or full-sequence methods.


<details>
  <summary>Details</summary>
Motivation: To address limitations of existing video generation approaches: autoregressive methods suffer from error accumulation/drift, while full-sequence flows are computationally expensive and less flexible for variable-length video generation.

Method: Learns a probability path that interleaves discrete frame insertions with continuous frame denoising, using frame insertion as an efficient compression mechanism for long-term context, enabling joint learning of video length and content.

Result: Achieves improved FVD and VBench metrics over autoregressive and full-sequence baselines, reduces training FLOPs three-fold compared to full-sequence flows, and enables seamless integration of tasks like image-to-video generation and video interpolation.

Conclusion: Flowception provides an efficient, flexible framework for variable-length video generation that outperforms existing approaches while enabling multi-task capabilities through its unique frame insertion/denoising mechanism.
...
### [40] [Boosting Skeleton-based Zero-Shot Action Recognition with Training-Free Test-Time Adaptation](https://arxiv.org/abs/2512.11458)
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of improving model generalization to unseen actions in skeleton-based zero-shot action recognition without requiring additional training or access to training data during inference.

Method: Skeleton-Cache reformulates inference as a lightweight retrieval process using a non-parametric cache that stores structured skeleton representations with both global and local descriptors. It leverages LLMs to assign class-specific importance weights for fusing descriptor-wise predictions.

Result: Extensive experiments on NTU RGB+D 60/120 and PKU-MMD II datasets show that Skeleton-Cache consistently boosts performance of various SZAR backbones under both zero-shot and generalized zero-shot settings.

Conclusion: Skeleton-Cache provides an effective training-free test-time adaptation framework that dynamically adapts to unseen actions by integrating structured skeleton representations with LLM-guided semantic priors, demonstrating strong generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Video flare removal is more challenging than image flare removal due to complex, independent motion of flares, light sources, and scene content, causing flicker and artifacts in existing methods.
...
Result: Created first video flare dataset with synthetic paired videos and real-world videos. Method outperforms existing video restoration and image flare removal methods on both real and synthetic videos, effectively removing dynamic flares while preserving light sources and spatiotemporal consistency.

Conclusion: Proposed approach successfully addresses motion-independent spatiotemporal flare characteristics in videos, eliminating need for multi-frame alignment and improving flare removal performance while maintaining scene integrity.
...
TL;DR: MetaCanvas is a lightweight framework that enables multimodal LLMs to reason and plan directly in spatial/spatiotemporal latent spaces for precise control over diffusion-based image/video generation, bridging the gap between multimodal understanding and generation.


<details>
  <summary>Details</summary>
Motivation: Current multimodal LLMs have strong reasoning capabilities for understanding complex scenes but are reduced to simple text encoders in generation tasks, creating a gap between their understanding power and generation precision. Most of their reasoning and planning abilities remain unused in visual generation.

Method: MetaCanvas is a lightweight framework that allows MLLMs to reason and plan directly in spatial and spatiotemporal latent spaces, tightly interfacing with diffusion generators. It treats MLLMs as latent-space planners rather than just global text encoders.

Result: MetaCanvas was implemented on three different diffusion backbones and evaluated across six tasks (text-to-image, text/image-to-video, image/video editing, in-context video generation). It consistently outperformed global-conditioning baselines, demonstrating better layout precision, attribute binding, and reasoning-intensive control.

Conclusion: Treating MLLMs as latent-space planners is a promising direction for narrowing the gap between multimodal understanding and generation, enabling more precise and structured control over visual generation tasks.
...
TL;DR: FreqDINO: A frequency-guided segmentation framework that enhances boundary perception in ultrasound images by leveraging multi-scale frequency analysis and boundary refinement.


<details>
  <summary>Details</summary>
Motivation: Ultrasound image segmentation is challenging due to speckle noise and imaging artifacts. While DINOv3 shows promise for medical image segmentation, it lacks sensitivity to ultrasound-specific boundary degradation because it was pre-trained on natural images.
...
Conclusion: FreqDINO effectively addresses the boundary perception limitations of DINOv3 for ultrasound segmentation through frequency-guided analysis, leading to improved segmentation performance and generalization.
...
TL;DR: UFVideo is the first Video LLM with unified multi-grained cooperative understanding capabilities, handling global, pixel, and temporal scales within a single model for comprehensive video perception.


<details>
  <summary>Details</summary>
Motivation: Existing Video LLMs are limited to specialized video understanding tasks and fail to achieve comprehensive, multi-grained video perception. There's a gap in models that can handle video understanding across different scales (global, pixel, temporal) in a unified manner.
...
Result: The model demonstrates flexibility and advantages over GPT-4o on the UFVideo-Bench (three collaborative tasks across scales). It also shows effectiveness across 9 public benchmarks covering various common video understanding tasks.

Conclusion: UFVideo represents a significant advancement in Video LLMs by providing unified multi-grained cooperative understanding capabilities, offering valuable insights for future Video LLM development and addressing the limitations of specialized-only approaches.
...
TL;DR: TS-FSAR improves few-shot action recognition via Ladder Side Network for efficient CLIP fine-tuning, Task-Specific Distance Correlation Matching for nonlinear dependencies, and GLAC module for regularization.


<details>
  <summary>Details</summary>
Motivation: Existing FSAR methods have two key limitations: 1) set matching metrics rely on cosine similarity and instance-level information only, failing to capture nonlinear relationships and task-specific cues; 2) side layers for CLIP adaptation are difficult to optimize under limited data conditions.

Method: Proposes TS-FSAR with three components: 1) visual Ladder Side Network (LSN) for efficient CLIP fine-tuning; 2) Task-Specific Distance Correlation Matching (TS-DCM) using α-distance correlation to model linear/nonlinear inter-frame dependencies with task prototypes; 3) Guiding LSN with Adapted CLIP (GLAC) module to regularize LSN using frozen CLIP for better α-distance correlation estimation.
...
Conclusion: TS-FSAR effectively addresses limitations in existing FSAR methods by combining efficient CLIP adaptation, nonlinear dependency modeling, and regularization techniques for improved few-shot action recognition.
...
### [46] [DentalGPT: Incentivizing Multimodal Complex Reasoning in Dentistry](https://arxiv.org/abs/2512.11558)
...
TL;DR: DentalGPT is a specialized 7B-parameter multimodal LLM for dentistry that achieves state-of-the-art performance through high-quality domain knowledge injection and reinforcement learning, using the largest annotated dental image dataset to date.
...
Result: DentalGPT achieves superior performance on intraoral/panoramic benchmarks and dental subsets of medical VQA benchmarks, outperforming many state-of-the-art MLLMs in disease classification and dental VQA tasks despite having only 7B parameters.

Conclusion: High-quality dental data combined with staged adaptation provides an effective pathway for building capable and domain-specialized dental MLLMs, demonstrating the value of targeted domain expertise in medical AI systems.
...
TL;DR: Developed a multi-mode underwater structured light 3D imaging system for pipeline corrosion detection using multi-source information fusion and adaptive algorithms.


<details>
  <summary>Details</summary>
Motivation: Underwater pipelines are highly susceptible to corrosion, posing safety risks and shortening service life. Manual inspection is unreliable, so an intelligent real-time imaging system is needed for precise defect characterization.

Method: 1) Rapid distortion correction (FDC) for underwater image rectification; 2) Factor graph-based parameter optimization for sensor calibration; 3) Multi-mode 3D imaging strategy; 4) Multi-source information fusion with adaptive extended Kalman filter (AEKF); 5) Edge detection-based ICP (ED-ICP) algorithm combining pipeline edge detection network with enhanced point cloud registration.

Result: Extensive experiments under different operation modes, velocities, and depths demonstrate superior accuracy, adaptability, and robustness of the developed UW-SLD system.

Conclusion: The system provides a solid foundation for autonomous underwater pipeline detection, achieving robust and high-fidelity reconstruction of defect structures even under variable motion conditions.
...
### [48] [Multi-temporal Calving Front Segmentation](https://arxiv.org/abs/2512.11560)
*Marcel Dreier,Nora Gourmelon,Dakota Pyles,Fei Wu,Matthias Braun,Thorsten Seehaus,Andreas Maier,Vincent Christlein*

Main category: cs.CV

TL;DR: A new deep learning approach improves calving front delineation in SAR imagery by processing multiple temporal frames in parallel and exchanging temporal information between them, achieving state-of-the-art performance on the CaFFe benchmark.


<details>
  <summary>Details</summary>
Motivation: Current deep learning models for automatically delineating glacier calving fronts in SAR imagery struggle with seasonal conditions like ice mélange or snow-covered surfaces, which affect classification accuracy and require continuous monitoring of glacier dynamics.

Method: Proposes processing multiple frames from satellite image time series in parallel and exchanging temporal information between corresponding feature maps to stabilize predictions. The approach is integrated into the state-of-the-art Tyrion architecture.

Result: Achieves new state-of-the-art performance on the CaFFe benchmark dataset with Mean Distance Error of 184.4 m and mean Intersection over Union of 83.6.

Conclusion: The temporal information exchange approach successfully addresses seasonal condition challenges in calving front delineation, improving accuracy and stability of predictions for better glacier monitoring.
...
TL;DR: A fully automatic pipeline for dynamic scene reconstruction from monocular RGB videos using enhanced priors with Dynamic Gaussian Splatting, achieving superior results over previous methods.


<details>
  <summary>Details</summary>
Motivation: To create a fully automatic system for reconstructing dynamic scenes from casually captured monocular RGB videos without requiring specialized scene representations, improving upon existing monocular dynamic scene reconstruction methods.

Method: Enhances Dynamic Gaussian Splatting priors using video segmentation with epipolar-error maps for object-level masks. These masks guide object-depth loss for sharper video depth and support skeleton-based sampling with mask-guided re-identification for comprehensive 2D tracks. Includes virtual-view depth loss to remove floaters and scaffold-projection loss to tie motion nodes to tracks.

Result: The system surpasses previous monocular dynamic scene reconstruction methods and delivers visibly superior renderings, with improved handling of thin structures, reduced floaters, and better preservation of fine geometry and coherent motion.

Conclusion: The proposed fully automatic pipeline successfully enhances Dynamic Gaussian Splatting priors through improved segmentation and tracking, resulting in state-of-the-art dynamic scene reconstruction from monocular RGB videos with superior visual quality.
...
TL;DR: Transfer learning with Faster R-CNN enables efficient rice seedling detection in UAV imagery, showing robust performance across temporal variations.


<details>
  <summary>Details</summary>
Motivation: Crop detection via UAVs is crucial for precision agriculture but challenging due to small target sizes and environmental variability. Rice seedling detection in paddy fields specifically needs efficient solutions.

Method: Uses Faster R-CNN architecture initialized via transfer learning. Curates a significant UAV dataset for training and evaluates generalization across three distinct test sets acquired at different temporal intervals to assess robustness against varying imaging conditions.

Result: Transfer learning facilitates rapid convergence of object detection models in agricultural contexts and yields consistent performance despite domain shifts in image acquisition.

Conclusion: Transfer learning with Faster R-CNN is effective for rice seedling detection in UAV imagery, demonstrating robustness across temporal variations and supporting scalable precision agriculture applications.
...
### [51] [Smudged Fingerprints: A Systematic Evaluation of the Robustness of AI Image Fingerprints](https://arxiv.org/abs/2512.11771)
*Kai Yao,Marc Juarez*

Main category: cs.CV

TL;DR: First systematic security evaluation of model fingerprint detection for AI-generated images shows these techniques are vulnerable to adversarial attacks, with removal attacks being highly effective and forgery attacks varying in success.


<details>
  <summary>Details</summary>
Motivation: While model fingerprint detection techniques show promise for attributing AI-generated images to source models, their robustness under adversarial conditions remains unexplored, creating a critical security gap that needs systematic evaluation.
...
Result: Significant gap between clean and adversarial performance: removal attacks achieve >80% success in white-box and >50% in black-box settings; forgery success varies across models; utility-robustness trade-off exists (high accuracy methods are often vulnerable); no method achieves high robustness and accuracy across all threat models.

Conclusion: Current model fingerprint detection techniques are vulnerable to adversarial attacks, highlighting the need for methods that balance robustness and accuracy, with specific approaches identified as most promising for advancing this goal.
...
TL;DR: Proposes ARNet-v2 with Channel Information Interaction Module and collaborative decoding architecture for better camouflaged object detection by addressing cross-channel interaction and boundary-region co-modeling issues.


<details>
  <summary>Details</summary>
Motivation: Current COD methods have two critical issues: insufficient cross-channel information interaction within same-layer features, and inability to effectively co-model boundary and region information, making it hard to accurately reconstruct complete regions and sharp boundaries.
...
Result: Extensive experiments on four COD benchmark datasets validate state-of-the-art performance. Model successfully transferred to SOD task and demonstrated adaptability across downstream tasks including polyp segmentation, transparent object detection, and industrial/road defect detection.

Conclusion: The proposed ARNet-v2 effectively addresses cross-channel interaction and boundary-region co-modeling issues in COD, achieving superior performance and demonstrating strong adaptability across various downstream segmentation tasks.
...
### [53] [Particulate: Feed-Forward 3D Object Articulation](https://arxiv.org/abs/2512.11798)
*Ruining Li,Yuxin Yao,Chuanxia Zheng,Christian Rupprecht,Joan Lasenby,Shangzhe Wu,Andrea Vedaldi*

Main category: cs.CV

TL;DR: Particulate is a feed-forward transformer network that directly infers articulated structure from a single 3D mesh, predicting parts, kinematics, and motion constraints in seconds without per-object optimization.


<details>
  <summary>Details</summary>
Motivation: Prior approaches for 3D articulation estimation require time-consuming per-object optimization. There's a need for fast, feed-forward methods that can infer complete articulated structure from static 3D meshes, especially for everyday objects and AI-generated assets.

Method: Uses a transformer network (Part Articulation Transformer) that processes input mesh point clouds with a flexible architecture to predict 3D parts, kinematic structure, and motion constraints with native multi-joint support. Trained end-to-end on diverse articulated 3D datasets.

Result: Produces fully articulated 3D models in seconds, significantly faster than optimization-based methods. Works with both real and AI-generated 3D assets, and outperforms state-of-the-art approaches in quantitative and qualitative evaluation on a new challenging benchmark.

Conclusion: Particulate enables fast, accurate articulation estimation from single 3D meshes, bridging the gap between static 3D models and fully articulated assets, with applications in 3D content creation and AI-generated 3D object extraction.
...
TL;DR: A novel evidence segmentation framework using Wasserstein loss improves out-of-distribution object segmentation for safety-critical applications like autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks for semantic segmentation fail when encountering unknown objects in open-world scenarios, which is critical for safety applications like automated driving where recognizing out-of-distribution objects is essential.
...
Conclusion: The evidence segmentation framework with Wasserstein loss effectively addresses the limitation of predefined class sets in semantic segmentation, enabling better handling of unknown objects in open-world scenarios for safety-critical applications.
...
TL;DR: The paper introduces the N-Body Problem: given one egocentric video, determine how N individuals could hypothetically perform the same tasks in parallel to maximize speed-up while respecting real-world constraints.


<details>
  <summary>Details</summary>
Motivation: Humans can intuitively parallelize complex activities, but current models struggle to learn this from observing a single person. The challenge is to assign video segments to multiple individuals while avoiding physically impossible scenarios like object conflicts or spatial collisions.

Method: Formalizes the N-Body Problem and proposes evaluation metrics for performance (speed-up, task coverage) and feasibility (spatial collisions, object conflicts, causal constraints). Introduces a structured prompting strategy that guides a Vision-Language Model (VLM) to reason about 3D environment, object usage, and temporal dependencies to produce viable parallel execution plans.
...
Conclusion: The proposed structured prompting approach enables VLMs to effectively reason about parallel task execution while respecting real-world constraints, significantly improving both performance and feasibility metrics compared to naive approaches.
...
TL;DR: FlowDC is a text-based image editing method that handles complex editing with multiple targets by decoupling them into parallel sub-editing effects and decomposing velocity to preserve source structure.


<details>
  <summary>Details</summary>
Motivation: Current complex image editing solutions (single-round and multi-round) struggle with long text following and cumulative inconsistency, failing to balance semantic alignment and source consistency when editing multiple targets.

Method: FlowDC decouples complex editing into multiple sub-editing effects and superposes them in parallel. It also decomposes velocity into components, decaying the orthogonal part to better preserve source structure.

Result: FlowDC shows superior results on two benchmarks (including the newly constructed Complex-PIE-Bench) compared to existing methods, with detailed ablation studies validating module designs.

Conclusion: FlowDC effectively addresses complex image editing challenges by parallel sub-editing decomposition and velocity orthogonal component decay, achieving better balance between semantic alignment and source consistency.
...
TL;DR: CRR framework transforms reconstruction to repairation for multi-class anomaly detection, addressing identity mapping issues by repairing synthesized anomalies and using feature-level masking.


<details>
  <summary>Details</summary>
Motivation: Industrial anomaly detection faces challenges with separate models per class (high memory, limited generalizability). Conventional reconstruction networks suffer from identity mapping where they replicate inputs regardless of normality, causing detection failures.

Method: Proposes Collaborative Reconstruction and Repair (CRR): 1) Optimizes decoder to reconstruct normal samples while repairing synthesized anomalies, 2) Uses feature-level random masking for local information, 3) Trains segmentation network supervised by synthetic anomaly masks to minimize detection errors from encoder-decoder discrepancies.
...
Conclusion: CRR framework successfully addresses the identity mapping problem in reconstruction-based anomaly detection and provides an effective unified solution for multi-class industrial anomaly detection.
...
TL;DR: JoyAvatar: Real-time audio-driven avatar generation with infinite-length video capability using autoregressive diffusion with progressive step bootstrapping, motion condition injection, and cache-resetting RoPE.


<details>
  <summary>Details</summary>
Motivation: Existing DiT-based audio-driven avatar methods have high computational overhead and can't synthesize long videos. Autoregressive methods exist but suffer from error accumulation and quality degradation.
...
Result: 1.3B-parameter causal model achieves 16 FPS on single GPU with competitive visual quality, temporal consistency, and lip synchronization.

Conclusion: JoyAvatar enables real-time inference and infinite-length video generation for audio-driven avatars while addressing error accumulation and quality degradation issues of previous methods.
...
TL;DR: Enhanced yawning detection for driver fatigue monitoring using improved dataset labeling and efficient edge AI models
...
Result: Improved frame accuracy by up to 6% and mAP by 5% over video-level supervision, achieving 99.34% classification accuracy and 95.69% detection mAP. Runs at 59.8 FPS on NVIDIA Jetson Nano edge hardware.

Conclusion: Enhanced data quality alone enables accurate on-device yawning monitoring without server computation, supporting real-time driver fatigue detection.
...
TL;DR: DOS is a self-supervised learning framework for 3D point clouds that distills semantic relevance softmaps only at observable points, preventing information leakage and providing richer supervision than discrete token assignments.


<details>
  <summary>Details</summary>
Motivation: SSL for 3D point clouds faces challenges due to irregular geometry, shortcut-prone reconstruction, and unbalanced semantics distribution. Existing methods suffer from information leakage from masked regions and limited supervision from discrete token assignments.

Method: DOS self-distills semantic relevance softmaps only at observable (unmasked) points. It introduces Zipfian prototypes with a modified Sinkhorn-Knopp algorithm (Zipf-Sinkhorn) that enforces a power-law prior over prototype usage and modulates target softmap sharpness during training.

Result: DOS outperforms current state-of-the-art methods on semantic segmentation and 3D object detection across multiple benchmarks (nuScenes, Waymo, SemanticKITTI, ScanNet, ScanNet200) without relying on extra data or annotations.

Conclusion: Observable-point softmaps distillation offers a scalable and effective paradigm for learning robust 3D representations, addressing key challenges in SSL for point clouds.
...
TL;DR: CADMorph is a framework for geometry-driven parametric CAD editing that uses pretrained foundation models to synchronize geometric shape changes with parametric construction sequences while preserving structure, ensuring semantic validity, and maintaining shape fidelity.


<details>
  <summary>Details</summary>
Motivation: During iterative CAD design, geometric adjustments require synchronized edits to underlying parametric sequences, but this is challenging due to the need to preserve sequence structure, ensure semantic validity, maintain shape fidelity, and operate with scarce editing data.

Method: CADMorph uses an iterative plan-generate-verify framework with pretrained domain-specific foundation models: a parameter-to-shape (P2S) latent diffusion model and a masked-parameter-prediction (MPP) model. Planning uses P2S cross-attention maps to identify segments needing modification, generation uses MPP to infill masks with valid edits, and verification uses P2S to measure shape similarity and select best candidates.
...
Conclusion: CADMorph effectively addresses geometry-driven parametric CAD editing by leveraging pretrained geometric priors through a three-stage framework that handles structure preservation, semantic validity, and shape fidelity while bypassing data scarcity limitations.
...
TL;DR: VLM2GeoVec is a unified vision-language model for remote sensing that embeds interleaved images, text, bounding boxes, and geographic coordinates into a single vector space, enabling both scalable retrieval and region-level spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Satellite imagery has unique characteristics (aerial view, high resolution, scale variations, small objects) requiring both region-level spatial reasoning and holistic scene understanding. Current approaches are fragmented between dual-encoder retrieval models (good for search but can't interleave modalities) and generative assistants (support region interpretation but lack scalable retrieval).
...
Result: Achieves 26.6% P@1 on region-caption retrieval (+25 pp vs. dual-encoder baselines), 32.5% P@1 on referring-expression retrieval (+19 pp), and 17.8% P@1 on semantic geo-localization retrieval (over 3× prior best). Matches or exceeds specialized baselines on conventional tasks like scene classification and cross-modal retrieval.

Conclusion: VLM2GeoVec unifies scalable retrieval with region-level spatial reasoning, enabling cohesive multimodal analysis in remote sensing. The model addresses the fragmentation in current approaches by providing a single solution for both retrieval and spatial reasoning tasks.
...
TL;DR: TSkel-Mamba: A hybrid Transformer-Mamba framework for skeleton-based action recognition that combines Spatial Transformer for spatial features and enhanced Mamba with Temporal Dynamic Modeling block for temporal modeling, achieving state-of-the-art performance with low inference time.


<details>
  <summary>Details</summary>
Motivation: Skeleton-based action recognition needs better modeling of both spatial and temporal dynamics. While Mamba shows success in 1D temporal sequences, it has limitations in modeling inter-channel dependencies needed for skeleton data.
...
Conclusion: TSkel-Mamba effectively captures both spatial and temporal dynamics for skeleton-based action recognition, being both efficient and highly effective through its hybrid Transformer-Mamba architecture with enhanced temporal modeling capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Manual abutment design is tedious and time-consuming, while AI automation faces data scarcity issues. Traditional self-supervised learning requires separate pre-training and fine-tuning, leading to high computational costs and long training times.

Method: Proposes SS$A^3$D with dual-branch architecture: reconstruction branch learns structural information from masked intraoral scans, regression branch predicts abutment parameters under supervision. Includes Text-Conditioned Prompt (TCP) module to incorporate clinical information (implant location, system, series) to guide network focus.

Result: SS$A^3$D saves half the training time compared to traditional SSL methods, achieves higher accuracy, and demonstrates state-of-the-art performance on collected dataset, improving both accuracy and efficiency of automated abutment design.

Conclusion: The proposed SS$A^3$D framework effectively addresses data scarcity in dental abutment design automation by eliminating separate pre-training/fine-tuning, reducing computational costs, and incorporating clinical guidance through TCP module, resulting in significant improvements in both training efficiency and prediction accuracy.
...
TL;DR: VGGT is a 3D foundation model that implicitly learns geometric concepts like correspondence matching and epipolar geometry through global attention layers, despite being trained without explicit geometric constraints.


<details>
  <summary>Details</summary>
Motivation: To understand whether VGGT builds upon geometric concepts like traditional multi-view methods or relies primarily on learned appearance-based data-driven priors, and to analyze its internal mechanisms for geometric understanding.

Method: Systematic analysis through probing intermediate features, analyzing attention patterns, performing interventions, spatial input masking, and perturbation experiments to assess robustness to occlusions, appearance variations, and camera configurations.

Result: VGGT implicitly performs correspondence matching within its global attention layers and encodes epipolar geometry despite no explicit geometric constraints. It shows robustness comparable to classical multi-stage pipelines while using learned data-driven priors.

Conclusion: VGGT internalizes geometric structure while leveraging learned data-driven priors, demonstrating that geometric understanding emerges within its representations through attention mechanisms.
...
TL;DR: The paper proposes methods to integrate event cameras with Multimodal LLMs using reconstruction techniques, introduces the first real-world benchmark (EvQA), and achieves SOTA performance.


<details>
  <summary>Details</summary>
Motivation: To enable general scene understanding in challenging visual conditions by integrating event cameras with MLLMs, while navigating the trade-off between preserving event data advantages and ensuring compatibility with frame-based models.

Method: Two approaches: 1) Frame-based Reconstruction and Tokenization (FRT) - straightforward method using reconstruction as a bridge; 2) Adaptive Reconstruction and Tokenization (ART) - efficient method leveraging event sparsity.
...
Conclusion: The proposed methods successfully bridge event cameras with MLLMs, with ART leveraging event sparsity for efficiency, and the introduced EvQA benchmark enables robust evaluation of event-based MLLMs.
...
<details>
  <summary>Details</summary>
Motivation: Fine-scale forest monitoring is crucial for understanding canopy structure, carbon stocks, biodiversity, and forest health. Current methods often require expensive very high resolution imagery or pretrained models, limiting scalability and cost-effectiveness.

Method: THREASURE-Net is a novel end-to-end framework that integrates spectral, temporal, and spatial signals from Sentinel-2 time series. It uses LiDAR-derived height metrics as reference data at multiple spatial resolutions (2.5m, 5m, 10m) over Metropolitan France. The model learns super-resolution directly from LiDAR data without pretrained models or very high resolution optical imagery.

Result: The model outperforms existing state-of-the-art Sentinel-based methods and is competitive with methods using very high resolution imagery. It achieves mean absolute errors of 2.62m, 2.72m, and 2.88m at 2.5m, 5m, and 10m resolutions respectively. The framework can generate high-precision annual canopy-height maps.

Conclusion: THREASURE-Net demonstrates the potential for scalable, cost-effective structural monitoring of temperate forests using only freely available satellite data, enabling annual canopy-height mapping without expensive optical imagery or pretrained models.
...
TL;DR: Proposes an end-to-end trainable framework for task-adaptive key frame selection in video understanding, addressing limitations of independent scoring and static pseudo labels through dynamic query generation, set-level optimization, and student-teacher mutual learning.


<details>
  <summary>Details</summary>
Motivation: Traditional top-K frame selection methods score frames independently, leading to temporally clustered and visually redundant selections. Additionally, training lightweight selectors with static MLLM-generated pseudo labels prevents dynamic adaptation to task objectives.

Method: 1) Chain-of-Thought guided SLM generates task-specific implicit query vectors combined with multimodal features for dynamic frame scoring. 2) Continuous set-level objective function (relevance, coverage, redundancy) optimized via Gumbel-Softmax for optimal frame combinations. 3) Student-teacher mutual learning aligns SLM selector and MLLM reasoner frame importance distributions via KL divergence and cross-entropy loss.
...
Conclusion: The proposed end-to-end trainable framework enables task-adaptive frame selection by addressing independent scoring limitations and eliminating reliance on static pseudo labels through dynamic query generation, set-level optimization, and mutual learning between selector and reasoner.
...
TL;DR: Infinity-8B VAR model achieves best compositional alignment in text-to-image generation, outperforming diffusion models like SDXL and PixArt-α across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Compositional alignment (objects, attributes, spatial relationships) remains a core challenge for text-to-image models, with VAR models' compositional behavior largely unexamined compared to diffusion models.

Method: Benchmarked six T2I systems (SDXL, PixArt-α, Flux-Dev, Flux-Schnell, Infinity-2B, Infinity-8B) across T2I-CompBench++ and GenEval suites, evaluating color/attribute binding, spatial relations, numeracy, and complex multi-object prompts.

Result: Infinity-8B achieved strongest overall compositional alignment, Infinity-2B matched/exceeded larger diffusion models in several categories, while SDXL and PixArt-α showed persistent weaknesses in attribute-sensitive and spatial tasks.

Conclusion: First systematic comparison of VAR vs diffusion approaches to compositional alignment, establishing unified baselines for future T2I model development, with VAR models showing promising efficiency-performance trade-offs.
...
TL;DR: SSL-MedSAM2: A novel semi-supervised learning framework for medical image segmentation that combines training-free few-shot learning with SAM2 for pseudo label generation and nnUNet for iterative refinement, achieving SOTA results on liver segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: Medical image annotation is highly time-consuming, hindering clinical applications. While deep learning models excel at segmentation, most require large annotated datasets. Semi-supervised learning offers a solution by reducing labeling costs while maintaining performance.

Method: Two-branch framework: 1) TFFS-MedSAM2 - training-free few-shot learning branch using pretrained SAM2 foundation model for initial pseudo label generation; 2) FSL-nnUNet - iterative fully-supervised learning branch using nnUNet for pseudo label refinement through iterative training cycles.

Result: Outstanding performance on MICCAI2025 CARE-LiSeg challenge: Dice scores of 0.9710 (GED4) and 0.9648 (T1 MRI); Hausdorff distances of 20.07 and 21.97 respectively, demonstrating superior liver segmentation with limited annotations.

Conclusion: SSL-MedSAM2 effectively reduces annotation burden while achieving state-of-the-art segmentation performance by leveraging foundation models for pseudo label generation and iterative refinement, making it highly suitable for clinical applications with limited labeled data.
...
TL;DR: 3DTeethSAM adapts SAM2 for 3D teeth segmentation using multi-view rendering, 2D segmentation, and 3D reconstruction with learnable modules for prompt generation, mask refinement, and classification.


<details>
  <summary>Details</summary>
Motivation: 3D teeth segmentation is critical for digital dentistry but challenging due to complex real-world dentition. Existing methods need improvement for accurate tooth localization and semantic categorization in 3D dental models.

Method: Adapts SAM2 for 3D teeth segmentation by: 1) Rendering 3D teeth models from multiple views, 2) Using SAM2 for 2D segmentation, 3) Reconstructing 3D results via 2D-3D projections, 4) Adding three learnable modules (prompt embedding generator, mask refiner, mask classifier), and 5) Incorporating Deformable Global Attention Plugins (DGAP) into SAM2's encoder.

Result: Achieves 91.90% IoU on the 3DTeethSeg benchmark with high-resolution 3D teeth meshes, establishing new state-of-the-art performance.

Conclusion: 3DTeethSAM successfully adapts SAM2 for 3D teeth segmentation through multi-view rendering and specialized learnable modules, achieving superior accuracy and setting a new benchmark in the field.
...
TL;DR: A novel benchmark for evaluating in-context 3D spatial understanding of foundation models without finetuning, using MVImgNet dataset to test segmentation of novel views based on key-query view contrast.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations rely on downstream finetuning, making it difficult to isolate the intrinsic 3D reasoning ability of pretrained encoders. There's a need for benchmarks that directly probe dense visual features without task-specific adjustments.

Method: Extends the Hummingbird framework for 2D scene understanding to 3D using the Multi-View ImageNet (MVImgNet) dataset. Given images from objects at specific angles (keys), the method benchmarks segmentation of novel views (queries), categorizing difficulty into easy, medium, hard, and extreme based on key-query view contrast.

Result: Benchmarked 8 state-of-the-art foundation models. DINO-based encoders remain competitive across large viewpoint shifts, while 3D-aware models like VGGT require dedicated multi-view adjustments.

Conclusion: The proposed benchmark effectively isolates and evaluates intrinsic 3D reasoning capabilities of foundation models without finetuning, revealing that DINO-based models maintain strong performance across viewpoint changes while specialized 3D models need additional multi-view optimization.
...
TL;DR: ContextSeisNet: An in-context learning model for seismic demultiple processing that uses spatially related example pairs to achieve consistent results with user control and data efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional seismic processing methods face challenges with noisy data and manual parameter tuning. Existing deep learning approaches suffer from spatially inconsistent results across neighboring seismic gathers and lack user control.

Method: ContextSeisNet uses in-context learning, conditioning predictions on a support set of spatially related example pairs (neighboring common-depth point gathers and their labels). The model learns task-specific processing at inference time by observing how similar gathers should be processed, without retraining.

Result: On synthetic data: Outperforms U-Net baseline quantitatively with enhanced spatial coherence. On field data: Achieves superior lateral consistency compared to traditional Radon demultiple and U-Net baseline, with improved near-offset performance and more complete multiple removal. Achieves comparable performance despite being trained on 90% less data.
...
<details>
  <summary>Details</summary>
Motivation: Existing GUI agents are evaluated primarily on commodity software (Word, Excel) but perform poorly on high-value CAD/EDA domains, which offer order-of-magnitude higher economic return but remain unsolved challenges.

Method: Created GUI-EDA dataset with 2,000+ screenshot-answer-action pairs from 5 CAD tools across 5 physical domains; benchmarked 30+ GUI agents; developed EDAgent with reflection mechanism specialized for EDA workflows.

Result: EDA tasks constitute a major unsolved challenge for existing agents; EDAgent achieves reliable performance on industrial CAD software and for the first time outperforms Electrical Engineering Ph.D. students.

Conclusion: Extends GUI agents from generic office automation to specialized high-value engineering domains, offering new avenue for advancing EDA productivity and demonstrating practical value in professional CAD environments.
...
TL;DR: Gaussian-based explicit representation replaces neural implicit representations for 3D medical image reconstruction, providing 5-10× speed-up with closed-form analytical solution instead of stochastic sampling.


<details>
  <summary>Details</summary>
Motivation: High-resolution 3D reconstruction from motion-corrupted 2D fetal MRI is crucial for neurodevelopmental diagnosis, but current implicit neural representations suffer from computational bottlenecks due to expensive stochastic Monte Carlo sampling for PSF approximation.

Method: Shift from neural network implicit representations to Gaussian explicit representations. Parameterize HR 3D volume as anisotropic Gaussian primitives, leveraging closure under convolution property to derive closed-form analytical solution for forward model (Σ_obs = Σ_HR + Σ_PSF).

Result: Matches reconstruction quality of state-of-the-art SVR frameworks while achieving 5-10× speed-up on neonatal and fetal data, with convergence often under 30 seconds.

Conclusion: Gaussian-based explicit representation enables fast, high-quality 3D reconstruction, paving way for clinical translation of real-time fetal 3D MRI by eliminating computational bottlenecks of stochastic sampling.
...
TL;DR: FactorPortrait is a video diffusion method for portrait animation that uses disentangled control signals for facial expressions, head movement, and camera viewpoints to create lifelike animations from a single portrait image.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of controllable portrait animation with disentangled control over facial expressions, head movements, and camera viewpoints, enabling more realistic and expressive portrait animation from minimal input.

Method: Uses a pre-trained image encoder to extract facial expression latents from driving videos, which are injected into a video diffusion transformer via an expression controller. For camera and head pose control, employs Plücker ray maps and normal maps from 3D body mesh tracking. Trained on a large-scale synthetic dataset with diverse combinations of camera viewpoints, head poses, and facial expressions.

Result: The method outperforms existing approaches in realism, expressiveness, control accuracy, and view consistency, demonstrating superior performance in portrait animation tasks.

Conclusion: FactorPortrait provides an effective solution for controllable portrait animation with disentangled control over facial expressions, head movements, and camera viewpoints, enabling high-quality animation from minimal input data.
...
TL;DR: KineMIC adapts Text-to-Motion diffusion models for few-shot human activity recognition by using CLIP text embeddings to bridge the domain gap between artistic motion generation and precise HAR requirements.


<details>
  <summary>Details</summary>
Motivation: Large annotated motion datasets are expensive to acquire for skeletal-based Human Activity Recognition (HAR). While Text-to-Motion (T2M) models offer scalable synthetic data, they focus on artistic motion rather than kinematically precise, class-discriminative actions needed for HAR, creating a significant domain gap.

Method: KineMIC is a transfer learning framework that adapts T2M diffusion models to HAR domains using kinetic mining. It leverages CLIP text embeddings to establish semantic correspondences between sparse HAR labels and T2M source data, providing soft supervision for kinematic distillation to transform generalist T2M models into specialized few-shot Action-to-Motion generators.

Result: Validated using HumanML3D as source T2M dataset and NTU RGB+D 120 subset as target HAR domain with only 10 samples per action class. The approach generates significantly more coherent motions and provides robust data augmentation, delivering +23.1% accuracy points improvement.

Conclusion: KineMIC successfully bridges the domain gap between generalist T2M models and HAR requirements, enabling effective few-shot action synthesis for data augmentation in human activity recognition tasks.
...
TL;DR: CLV-Net is a visual prompt-guided multimodal image understanding method that uses bounding box cues to generate correlated segmentation masks and captions, with context-aware modeling of inter-object relationships to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with steering models to user-relevant regions using only simple text prompts, especially in aerial imagery where objects have similar appearances and rich inter-object relationships, making accurate recognition challenging.

Method: Proposes CLV-Net with Context-Aware Mask Decoder to model inter-object relationships, and Semantic and Relationship Alignment module with Cross-modal Semantic Consistency Loss for fine-grained discrimination and Relationship Consistency Loss for aligning textual relations with visual interactions.
...
Conclusion: CLV-Net successfully addresses the challenges of visual prompt-guided multimodal image understanding in remote sensing by leveraging visual cues and modeling inter-object relationships, enabling accurate recognition and faithful reflection of user intent.
...
TL;DR: Depth Copy Paste is a multimodal, depth-aware augmentation framework that generates realistic face detection training samples by intelligently pasting full-body person instances into semantically compatible scenes using semantic coherence assessment, precise segmentation, and depth-guided placement.


<details>
  <summary>Details</summary>
Motivation: Traditional copy-paste augmentation produces unrealistic composites due to inaccurate foreground extraction, inconsistent scene geometry, and mismatched background semantics, limiting the effectiveness of face detection training under challenging conditions like occlusion and illumination variation.

Method: Uses BLIP and CLIP for semantic/visual coherence assessment to retrieve suitable backgrounds, integrates SAM3 for precise segmentation and Depth-Anything to extract non-occluded visible person regions, and employs depth-guided sliding window placement on background depth maps for optimal depth continuity and scale alignment.

Result: Extensive experiments show that Depth Copy Paste generates more diverse and realistic training data, leading to significant performance improvements in downstream face detection tasks compared to traditional copy-paste and depth-free augmentation methods.

Conclusion: Depth Copy Paste effectively addresses limitations of traditional augmentation by ensuring physical consistency through multimodal semantic assessment and depth-aware placement, resulting in more robust face detection systems.
...
TL;DR: A text image classifier device that identifies and categorizes textual content in images into Invoice, Form, Letter, or Report categories using DBNet++ for text detection and BART for classification, achieving 94.62% recognition rate.


<details>
  <summary>Details</summary>
Motivation: To address practical challenges in real-world document processing including changing lighting conditions, random orientation, curvature/partial text coverage, low resolution, and barely visible text in uncontrolled imaging environments.

Method: Four-step pipeline: 1) Image acquisition/preprocessing from gallery mode (flash disks, HDDs, microSD) or live mode (camera feeds), 2) Text detection using DBNet++ (Differentiable Binarization Network Plus), 3) Text classification using BART (Bidirectional Auto-Regressive Transformers), 4) Results presentation via Python/PyQt5 GUI.

Result: Achieved 94.62% text recognition rate when tested on the Total-Text dataset over 10 hours, demonstrating effectiveness in handling mixed-source text categorization under challenging imaging conditions.

Conclusion: The proposed system effectively addresses practical document processing challenges through a smooth workflow combining DBNet++ and BART models, proving robust for real-world text image classification in uncontrolled environments.
...
TL;DR: RCD framework enables targeted change detection in remote sensing using natural language prompts, overcoming limitations of traditional methods through a two-stage approach with RCDNet for cross-modal fusion and RCDGen for synthetic data generation.


<details>
  <summary>Details</summary>
Motivation: Traditional change detection identifies all changes without distinguishing types, while semantic methods use rigid class definitions that limit dataset mixing and model reuse. There's a need for flexible, user-specified change detection that can handle limited annotated data and class imbalance.

Method: Two-stage framework: (I) RCDNet - cross-modal fusion network for referring change detection using natural language prompts; (II) RCDGen - diffusion-based synthetic data generation pipeline that creates realistic post-change images and change maps from pre-change images without needing semantic segmentation masks.

Result: Experiments across multiple datasets show the framework enables scalable and targeted change detection. The approach successfully integrates language understanding with visual analysis for user-specified change detection.

Conclusion: Referring Change Detection (RCD) with natural language prompts provides a flexible solution for targeted change detection in remote sensing, overcoming limitations of traditional methods through synthetic data generation and cross-modal fusion, enabling scalable applications.
...
TL;DR: The paper proposes a novel music-to-dance generation approach by reframing it as music-token-conditioned multi-channel image synthesis, using pose sequences encoded as one-hot images and modeled with a DiT-style backbone.


<details>
  <summary>Details</summary>
Motivation: The key challenge in music-to-dance generation is creating temporally coherent, rhythm-aligned 2D poses from music, especially for complex, high-variance in-the-wild distributions. Existing pose-to-video models can translate poses to videos, but generating high-quality pose sequences from music remains difficult.

Method: 1) Encode 2D pose sequences as one-hot images and compress them using a pretrained image VAE. 2) Use a DiT-style backbone for modeling, inheriting advances from modern text-to-image models. 3) Introduce time-shared temporal indexing to synchronize music tokens and pose latents. 4) Implement reference-pose conditioning to preserve body proportions and enable segment-and-stitch generation.

Result: Experiments on large in-the-wild 2D dance corpus and AIST++2D benchmark show consistent improvements over existing methods in pose- and video-space metrics and human preference. Ablations validate the contributions of the representation, temporal indexing, and reference conditioning.

Conclusion: The proposed approach effectively addresses music-to-dance generation by leveraging image synthesis techniques, achieving better performance in capturing high-variance pose distributions and producing rhythm-aligned dance sequences.
...
<details>
  <summary>Details</summary>
Motivation: Need for reliable segmentation of overlapping cell nuclei in multiplex cyclic immunofluorescent whole-slide images without requiring human annotations, especially for new instruments/protocols, and to enable automated quality assessment in production environments where manual proofreading is impractical.

Method: Weak to strong generalization methodology using multi-head extension of Mask-RCNN with efficient channel attention, featuring pseudo-label correction and coverage expansion mechanisms for automated training without human annotations.

Result: Benchmarked against five current widely used methods and showed significant improvement. Provides automated self-diagnosis metrics for segmentation quality in production environments.

Conclusion: The method enables de novo segmentation of new image classes from new instruments/protocols without human annotations, with open-source code, sample images, and results provided for community adoption.
...
TL;DR: SVG-T2I scales self-supervised VFM representations for text-to-image generation, achieving competitive performance without training in pixel space.


<details>
  <summary>Details</summary>
Motivation: Visual Foundation Model (VFM) representations offer a unified pathway for visual understanding, perception, and generation, but training large-scale text-to-image diffusion models entirely within VFM representation space remains unexplored.

Method: Scale the SVG (Self-supervised representations for Visual Generation) framework to support high-quality text-to-image synthesis directly in VFM feature domain using a standard text-to-image diffusion pipeline.

Result: Achieves competitive performance: 0.75 on GenEval and 85.78 on DPG-Bench, validating VFMs' representational power for generative tasks.

Conclusion: The work demonstrates the feasibility and effectiveness of representation-driven visual generation, with full open-source release to facilitate further research in this direction.
...
TL;DR: InST-based style transfer improves synthetic microscopy images for cell counting, reducing domain gap and achieving better performance than hard-coded synthetic data and even real data alone.


<details>
  <summary>Details</summary>
Motivation: Generating realistic synthetic microscopy images is crucial for training deep learning models in label-scarce environments, but traditional domain adaptation methods struggle when synthetic images lack complex textures and visual patterns of real samples.

Method: Adapt Inversion-Based Style Transfer (InST) framework to biomedical microscopy by combining latent-space Adaptive Instance Normalization with stochastic inversion in a diffusion model to transfer style from real fluorescence microscopy images to synthetic ones while weakly preserving content structure.

Result: Models trained with InST-synthesized images achieve up to 37% lower MAE than hard-coded synthetic data, 52% reduction compared to Cell200-s (53.70 to 25.95 MAE), and outperform real data alone (25.95 vs. 27.74 MAE). Further improvements with DACS and CutMix.
...
TL;DR: The paper introduces a Matting Quality Evaluator (MQE) for assessing alpha mattes without ground truth, uses it to create a large-scale real-world video matting dataset (VMReal), and proposes MatAnyone 2 model with reference-frame training strategy for state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Video matting is limited by small datasets and lack of realistic data. Existing approaches using segmentation data often produce segmentation-like mattes without fine details due to insufficient boundary supervision.

Method: 1) Learned Matting Quality Evaluator (MQE) that assesses semantic and boundary quality without ground truth, producing pixel-wise evaluation maps. 2) MQE used for online training feedback to suppress errors and offline data curation. 3) Created VMReal dataset with 28K clips and 2.4M frames. 4) Reference-frame training strategy incorporating long-range frames beyond local windows. 5) MatAnyone 2 model.

Result: Built VMReal dataset (28K clips, 2.4M frames). MatAnyone 2 achieves state-of-the-art performance on both synthetic and real-world benchmarks, surpassing prior methods across all metrics.

Conclusion: The MQE enables large-scale video matting dataset creation and quality assessment without ground truth. The proposed methods and dataset significantly advance video matting performance, with MatAnyone 2 setting new state-of-the-art results.
...
TL;DR: A trustworthy, frequency-aware segmentation framework for quantifying vitiligo extent in clinical photos using domain-adaptive pre-training, high-frequency spectral gating, and uncertainty estimation to improve reliability and accuracy.


<details>
  <summary>Details</summary>
Motivation: Accurate quantification of vitiligo extent in clinical photographs is crucial for longitudinal monitoring of treatment response, requiring reliable automated segmentation methods.

Method: Three synergistic pillars: 1) Data-efficient training with domain-adaptive pre-training on ISIC 2019 and ROI-constrained dual-task loss; 2) ConvNeXt V2-based encoder with High-Frequency Spectral Gating module and stem-skip connections; 3) Clinical trust mechanism using K-fold ensemble and Test-Time Augmentation for uncertainty maps.

Result: Achieved Dice score of 85.05%, significantly reduced boundary error (95% Hausdorff Distance improved from 44.79 px to 29.95 px), outperformed CNN and Transformer baselines, with zero catastrophic failures and interpretable entropy maps.

Conclusion: The proposed framework establishes a robust and reliable standard for automated vitiligo assessment, providing trustworthy segmentation with uncertainty estimation for clinical review.
...
TL;DR: SAM2VideoX distills structure-preserving motion priors from autoregressive video tracking (SAM2) into bidirectional video diffusion (CogVideoX) for realistic motion generation of articulated objects.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models struggle with generating physically plausible motion for articulated/deformable objects (humans, animals). Scaling training data hasn't solved implausible transitions, and existing approaches rely on noisy motion representations like optical flow or imperfect skeleton extraction.

Method: Two innovations: (1) bidirectional feature fusion module extracts global structure-preserving motion priors from recurrent model SAM2; (2) Local Gram Flow loss aligns how local features move together. Distills motion priors from autoregressive video tracking into bidirectional video diffusion.

Result: Achieves 95.51% on VBench (+2.60% over REPA), reduces FVD to 360.57 (21-22% improvement), and gets 71.4% human preference. Outperforms prior baselines in structure-preserving motion generation.

Conclusion: Distilling motion priors from tracking models into diffusion models effectively addresses structure-preserving motion generation challenges, enabling more realistic video generation for articulated objects.
...
TL;DR: V-RGBX is the first end-to-end framework for intrinsic-aware video editing that unifies video inverse rendering, synthesis from intrinsic representations, and keyframe-based editing with physically grounded manipulations.


<details>
  <summary>Details</summary>
Motivation: Current large-scale video generation models lack a closed-loop framework that jointly understands intrinsic scene properties (albedo, normal, material, irradiance), uses them for video synthesis, and supports editable intrinsic representations. There's an unexplored need for systems that can both extract and manipulate these physically meaningful properties for video editing.

Method: V-RGBX uses an interleaved conditioning mechanism that enables intuitive, physically grounded video editing through user-selected keyframes. The framework supports flexible manipulation of any intrinsic modality and propagates keyframe edits across sequences in a physically plausible manner.

Result: Extensive qualitative and quantitative results show V-RGBX produces temporally consistent, photorealistic videos while effectively propagating keyframe edits. The system demonstrates effectiveness in diverse applications including object appearance editing and scene-level relighting, surpassing prior methods.

Conclusion: V-RGBX represents a significant advancement in intrinsic-aware video editing, providing the first end-to-end framework that unifies inverse rendering, synthesis, and editing capabilities while maintaining physical plausibility and temporal consistency.
...
TL;DR: 3D Gaussian Splatting extended with moment-based transmittance computation for accurate rendering of translucent objects without ray tracing or sorting.


<details>
  <summary>Details</summary>
Motivation: 3DGS has limitations in rendering complex overlapping semi-transparent objects due to simplified alpha blending and coarse density approximations in rasterization.

Method: Extends rasterization-based 3D Gaussian rendering with moment-based order-independent transparency. Computes per-pixel statistical moments from contributing Gaussians to reconstruct continuous transmittance functions, then samples within each Gaussian.

Result: Bridges gap between rasterization and physical accuracy by modeling light attenuation in translucent media, improving reconstruction and rendering quality without ray tracing or per-pixel sorting.

Conclusion: Novel method enables high-fidelity transmittance computation for 3D Gaussian representations, addressing limitations of current 3DGS for complex translucent object rendering.
...
### [91] [RMSup: Physics-Informed Radio Map Super-Resolution for Compute-Enhanced Integrated Sensing and Communications](https://arxiv.org/abs/2512.10965)
*Qiming Zhang,Xiucheng Wang,Nan Cheng,Zhisheng Yin,Xiang Li*

Main category: eess.SP

TL;DR: RMSup: A physics-informed super-resolution framework for constructing high-fidelity radio maps from sparse measurements and imperfect environment priors, jointly recovering environmental contours for ISAC applications.


<details>
  <summary>Details</summary>
Motivation: Constructing high-fidelity radio maps at operational scales is challenging because physics-based solvers are time-consuming and require precise scene models, while learning methods degrade under incomplete priors and sparse measurements and often smooth away critical discontinuities.

Method: RMSup extracts Helmholtz equation-informed boundary and singularity prompts from measurements, fuses them with base-station side information and coarse scene descriptors as conditional inputs, and uses a boundary-aware dual-head network to reconstruct high-fidelity radio maps and recover environmental contours jointly.

Result: Experimental results show RMSup achieves state-of-the-art performance both in radio map construction and ISAC-related environment sensing.

Conclusion: RMSup provides an effective physics-informed super-resolution framework that works with uniform sparse sampling and imperfect environment priors, enabling high-fidelity radio map construction and environmental contour recovery for integrated sensing and communications.
...
### [92] [Uplink Rate Maximization for Pinching Antenna- Assisted Covert Backscatter Communication](https://arxiv.org/abs/2512.10970)
*Yulei Wang,Yalin Liu,Yaru Fu,Yuanwei Liu*

Main category: eess.SP

TL;DR: PA-assisted backscatter communication optimizes transmit power and antenna positioning to maximize covert transmission rate while addressing the double near-far problem in ambient signal-based systems.


<details>
  <summary>Details</summary>
Motivation: Pinching antenna technology enables flexible positioning for LoS links, offering potential for ambient signal-based backscatter communication. There's a need to enhance both communication performance and covertness in the presence of randomly distributed eavesdroppers.

Method: Formulates optimization problem to maximize uplink covert transmission rate by jointly optimizing transmit power and antenna positions. Proposes an alternative optimization (AO)-based framework to solve the problem while satisfying communication reliability and covertness constraints.

Result: Numerical results show PA-BSC effectively mitigates the double near-far problem (where energy harvesting and backscatter transmission degrade simultaneously due to distance disparities), improving downlink energy harvesting and uplink data transmission while maintaining covertness performance.

Conclusion: PA-assisted BSC provides an effective solution for enhancing both communication performance and covertness in practical deployment scenarios by leveraging flexible antenna positioning to optimize system parameters.
...
### [93] [A Unified Theory of Dynamic Programming Algorithms in Small Target Detection](https://arxiv.org/abs/2512.11170)
*Nicholas Bampton,Tian J. Ma,Minh N. Do*

Main category: eess.SP

TL;DR: A framework for small target detection using dynamic programming with rigorous convergence analysis, introducing NPI algorithm for targets with unknown/time-varying features.


<details>
  <summary>Details</summary>
Motivation: Small target detection is challenging due to minimal size, lack of distinctive features, complex backgrounds, heavy noise, and computational intensity of multi-frame integration. Existing dynamic programming approaches lack analytical tools for rigorous study.

Method: Proposes a robust framework for dynamic programming-based detection algorithms with rigorous convergence analysis. Models error probabilities as function of distance from target, establishing relationship between location uncertainty and existence uncertainty. Introduces Normalized Path Integration (NPI) algorithm that leverages similarity between sequential observations.

Result: Establishes rigorous convergence results for error rates under mild assumptions. NPI algorithm enables target detection with unknown or time-varying features by utilizing sequential observation similarity.

Conclusion: The framework provides analytical foundation for dynamic programming-based small target detection, with NPI offering practical solution for detecting targets with unknown or time-varying characteristics in noisy environments.
...
### [94] [Robust Detection of Underwater Target Against Non-Uniform Noise With Optical Fiber DAS Array](https://arxiv.org/abs/2512.11231)
*Siyuan Cang,Cong Liu,Xueli Sheng,Xiaoming Cui,Chao Li,Changxin Fa,Jiantong Chen,Chaoran Yang,Huayong Yang*

Main category: eess.SP

TL;DR: Novel underwater target detection using optical fiber DAS with spiral-sensitized cable and sparse covariance-fitting algorithm achieves high accuracy and noise robustness.


<details>
  <summary>Details</summary>
Motivation: Underwater target detection faces challenges from non-uniform marine environmental noise and complex acoustic sources (shipping, marine life, geological activity), requiring advanced sensors and signal processing techniques.

Method: Combines optical fiber distributed acoustic sensing (DAS) system with broadband generalized sparse covariance-fitting framework. Uses newly developed spiral-sensitized optical cable for improved sensitivity (-145.69 dB re: 1 rad/(uPa*m)).

Result: Simulations show higher accuracy and reduced background noise compared to conventional beamforming and other sparse techniques. Pool experiment demonstrates high fidelity with 0.973 correlation coefficient between DAS and standard hydrophone.

Conclusion: The proposed DAS system with spiral-sensitized cable and sparse covariance-fitting algorithm provides robust underwater target direction sensing against non-uniform noise, outperforming conventional methods.
...
### [95] [Source Localization and Power Estimation through RISs: Performance Analysis and Prototype Validations](https://arxiv.org/abs/2512.11420)
*Fuhai Wang,Tiebin Mi,Chun Wang,Rujing Xiong,Zhengyu Wang,Robert Caiming Qiu*

Main category: eess.SP

TL;DR: Single and multiple RISs can perform backward localization by using configurational diversity to estimate direction of arrival, with multi-RIS setups improving accuracy. Theoretical analysis provides performance bounds and system design guidance.


<details>
  <summary>Details</summary>
Motivation: To investigate the capabilities of reconfigurable intelligent surfaces (RISs) for backward localization, where spatial information is encoded into measurements through diverse radiation patterns for region reconstruction.

Method: Using configurational diversity with RISs to create diverse radiation patterns that encode spatial information. Decomposing the sensing operator into structured matrices for single-RIS localization, and analyzing the conditioning of sensing operators to establish performance indicators. Deriving governing laws via rank analysis for multi-RIS settings.

Result: Single RIS can estimate direction of arrival, while multiple RISs improve source localization and power estimation accuracy. Theoretical upper bounds on reconstruction error are derived, showing how key performance indicators affect estimation error. Numerical experiments confirm theoretical error trends.

Conclusion: RIS-based backward localization is effective, with configurational diversity enabling spatial sensing. The theoretical framework provides valuable guidance for system optimization and clarifies the relationship between system parameters and performance.
...
### [96] [Point Target Near-Field Bistatic Imaging: Chirp-Based Aliasing Analysis](https://arxiv.org/abs/2512.11444)
*Baptiste Sambon,Gilles Monnoyer,Claude Oestges,Luc Vandendorpe*

Main category: eess.SP

TL;DR: A chirp-based framework for analyzing aliasing in bistatic near-field imaging systems with multidimensional arrays, providing closed-form expressions for aliasing-free conditions and geometric interpretations.


<details>
  <summary>Details</summary>
Motivation: To develop a comprehensive framework for characterizing and understanding aliasing effects in bistatic near-field imaging systems, which is crucial for designing effective imaging systems that avoid reconstruction artifacts.

Method: Extends monostatic formulations to bistatic systems using chirp-based analysis, derives closed-form expressions for maximum spatial frequency, establishes analytical conditions for aliasing-free reconstruction, and provides geometric interpretation based on array geometry, target position, and element spacing.

Result: Numerical results confirm theoretical findings: aliasing-free region expands with smaller antenna spacing, greater target range, lower array dimensionality, and smaller arrays. The framework enables analytical determination of aliasing conditions.

Conclusion: The developed framework provides tools for more effective design of bistatic near-field imaging systems by enabling systematic analysis and avoidance of aliasing artifacts through proper system parameter selection.
...
### [97] [STAR-RIS-Aided Secure Communications:Analytical Insights and Performance Comparison](https://arxiv.org/abs/2512.11461)
*Taissir Y. Elganimi,Mahmoud Aldababsa,Ali A. Nasir,Khaled M. Rabie*

Main category: eess.SP

TL;DR: STAR-RIS technology enables full-space signal manipulation and outperforms conventional RIS and DF relaying in both achievable and secrecy rates for wireless communications.


<details>
  <summary>Details</summary>
Motivation: STAR-RISs have emerged as promising technology for enhancing wireless network coverage and capacity through full-space signal manipulation, but comprehensive analytical comparisons with existing technologies like conventional RISs and DF relaying schemes are needed.

Method: Derived closed-form expressions for achievable secrecy rates of STAR-RIS-aided communications, incorporated direct source-destination links in all schemes, investigated optimal transmit power allocation for HD and FD-DF relaying, and analyzed impacts of key system parameters including transmit power, number of elements, power ratios, and deployment positions.

Result: STAR-RIS systems achieve superior rates and secrecy rates compared to SISO, conventional RISs, and DF relaying (both HD and FD modes). The paper provides conditions under which STAR-RIS outperforms benchmark schemes and quantifies minimum number of elements required for superior performance.

Conclusion: STAR-RIS technology demonstrates significant advantages over existing wireless communication enhancement techniques, offering better performance in both achievable rates and secrecy rates, with practical guidelines for system design parameters.
...
### [98] [RadarFuseNet: Complex-Valued Attention-Based Fusion of IQ Time- and Frequency-Domain Radar Features for Classification Tasks](https://arxiv.org/abs/2512.11537)
*Stefan Hägele,Adam Misik,Eckehard Steinbach*

Main category: eess.SP

TL;DR: Proposes a bidirectional cross-attention fusion network combining IQ-signal and FFT-transformed radar features using complex-valued CNNs for improved mmWave radar-based occluded-object and material classification.


<details>
  <summary>Details</summary>
Motivation: mmWave radar is effective for perception tasks where vision sensors fail (occluded objects, material distinction), but current IQ signal-based methods have room for improvement in classification accuracy and robustness.

Method: Bidirectional cross-attention fusion network that combines IQ-signal and FFT-transformed radar features extracted by distinct complex-valued convolutional neural networks.

Result: Achieves 99.92% material classification accuracy at trained distances, 67.38% at unseen distances (improved generalization), and 94.20% occluded object classification (up from 91.99% with standalone CNNs).

Conclusion: The proposed bidirectional cross-attention fusion approach outperforms standalone complex-valued CNNs, offering improved performance, robustness, and generalization for mmWave radar-based classification tasks.
...
### [99] [ACCOR: Attention-Enhanced Complex-Valued Contrastive Learning for Occluded Object Classification Using mmWave Radar IQ Signals](https://arxiv.org/abs/2512.11556)
*Stefan Hägele,Adam Misik,Constantin Patsch,Eckehard Steinbach*

Main category: eess.SP

TL;DR: ACCOR: Attention-enhanced complex-valued contrastive learning for mmWave radar-based occluded object classification, achieving 96.60% accuracy at 64GHz and 93.59% at 67GHz for ten objects.


<details>
  <summary>Details</summary>
Motivation: mmWave radar offers robust sensing in adverse conditions and can penetrate materials like packaging, but existing models for occluded object classification need improvement and better evaluation across different frequencies.

Method: Proposes ACCOR: processes complex-valued IQ radar signals using complex-valued CNN backbone, multi-head attention layer, and hybrid loss combining weighted cross-entropy with supervised contrastive term. Extends existing 64GHz dataset with 67GHz subset.

Result: Outperforms prior radar-specific models and adapted image classification models, achieving 96.60% accuracy at 64GHz and 93.59% at 67GHz for ten different objects.

Conclusion: Demonstrates benefits of complex-valued deep learning with attention and contrastive learning for mmWave radar-based occluded object classification in industrial and automated environments.
...
### [100] [PaddleSat Optical Charging Station in Space](https://arxiv.org/abs/2512.11629)
*Anish Nair,Austin O' Connell,Shalomi Arulpragasam,Noah Kim*

Main category: eess.SP

TL;DR: PaddleSat is a companion spacecraft that wirelessly charges a host spacecraft using directional laser power transmission to supplement power or extend mission lifetimes.


<details>
  <summary>Details</summary>
Motivation: To reduce onboard power system requirements on host spacecraft and extend mission lifetimes by providing supplemental power through wireless charging.

Method: Investigates feasibility through system performance estimates, link budget calculations, optical transmission hardware analysis, and design tradeoffs between beam divergence, optical efficiency, and orbital control requirements.

Result: The paper examines the technical feasibility and design considerations for implementing a laser-based wireless power transmission system between spacecraft.

Conclusion: Wireless power transmission via directional lasers from a companion spacecraft (PaddleSat) is a promising approach to supplement host spacecraft power, though requires careful optimization of optical and orbital parameters.
...
### [101] [Sphere Decoding Revisited](https://arxiv.org/abs/2512.11195)
*Zheng Wang,Cong Ling,Shi Jin,Yongming Huang,Feifei Gao*

Main category: cs.IT

TL;DR: The paper revisits sphere decoding for integer least squares problems, proposing Equivalent Sphere Decoding (ESD) with explicit complexity bounds controlled by parameter K, and enhancement mechanisms for better performance.


<details>
  <summary>Details</summary>
Motivation: To address the lack of explicit complexity specification in traditional sphere decoding algorithms and provide a tractable, flexible decoding trade-off that can bridge suboptimal and optimal performance.

Method: Proposes Equivalent Sphere Decoding (ESD) that characterizes sphere radius D with two parameters (initial searching size K and deviation factor σ), establishes complexity bound |S|<nK, and introduces enhancement mechanisms: normalized weighting and candidate protection.

Result: ESD provides explicit complexity specification with tractable trade-off controlled solely by K, achieves larger sphere radius with same parameters through enhancements, and bridges suboptimal/optimal decoding performance flexibly.

Conclusion: The paper presents a novel sphere decoding approach with explicit complexity bounds and flexible performance-complexity trade-off, verified through large-scale MIMO detection applications.
...
### [102] [Information-Theoretic Equivalences Across Rate-Distortion, Quantization, and Decoding](https://arxiv.org/abs/2512.11279)
*Bruno Macchiavello*

Main category: cs.IT

TL;DR: A unified mathematical framework connecting rate-distortion theory, lattice quantization, and error-correcting codes through variational and convex-analytic principles.


<details>
  <summary>Details</summary>
Motivation: To establish a common mathematical foundation that reveals the deep structural connections between seemingly disparate areas of information theory - compression, quantization, and error correction - by emphasizing their shared variational and convex-analytic properties.

Method: 1) Gibbs-type variational formulation of rate-distortion function showing optimal test channels form exponential families; 2) Extension of reverse water-filling to distributed lattice quantization; 3) Formalization of inference as decoding by interpreting belief propagation and polarization as recursive variational inference.

Result: The paper demonstrates that optimal test channels form exponential families with KL divergence as Bregman divergence, establishes a generalized Pythagorean theorem for projections, extends reverse water-filling to lattice quantization, and shows belief propagation/polar codes can be viewed as recursive variational inference.

Conclusion: Compression, quantization, and decoding are unified as convex projections of continuous information onto discrete manifolds, with extensions to neural compression and quantum information illustrating the framework's universality across information processing domains.
...
### [103] [Refinements and Generalizations of the Shannon Lower Bound via Extensions of the Kraft Inequality](https://arxiv.org/abs/2512.11322)
*Neri Merhav*

Main category: cs.IT

TL;DR: The paper extends the Kraft inequality for lossy compression, leading to refined Shannon lower bounds for various rate-distortion coding scenarios.


<details>
  <summary>Details</summary>
Motivation: To develop more precise bounds in rate-distortion theory by extending fundamental inequalities beyond classical Shannon lower bounds.

Method: Derive extended versions of the Kraft inequality for lossy compression, then use these to refine and extend the Shannon lower bound.

Result: Several new bounds including: sharper bounds for one-to-one and D-semifaithful codes, Shannon lower bound for sliding-window distortion measures, and individual-sequence counterpart of Shannon lower bound.

Conclusion: Extended Kraft inequalities provide powerful tools for deriving refined rate-distortion bounds across diverse coding scenarios, advancing theoretical foundations of lossy compression.
...
### [104] [AMBER: An Adaptive Multimodal Mask Transformer for Beam Prediction with Missing Modalities](https://arxiv.org/abs/2512.11331)
*Chenyiming Wen,Binpu Shi,Min Li,Ming-Min Zhao,Min-Jian Zhao,Jiangzhou Wang*

Main category: cs.IT

TL;DR: AMBER is a novel multimodal transformer framework for robust beam prediction in mmWave vehicular networks that handles arbitrary missing sensor data through adaptive masking and modality alignment.


<details>
  <summary>Details</summary>
Motivation: Traditional beam prediction relies on in-band training, while recent multimodal approaches degrade in real-world settings due to sensor failures (blockage, poor lighting, GPS dropouts). There's a need for robust multimodal fusion that handles missing data.

Method: AMBER processes temporal sequences of image, LiDAR, radar, and GPS data using learnable modality tokens with missing-modality-aware masks to prevent noise propagation. It employs learnable fusion tokens with multihead attention for robust feature distillation, plus class-former-aided modality alignment (CMA) and temporal-aware positional embedding for semantic alignment and temporal coherence.

Result: Extensive experiments on DeepSense6G dataset show AMBER significantly outperforms existing multimodal baselines, maintaining high beam prediction accuracy and robustness even under severe missing-modality scenarios.

Conclusion: AMBER provides an effective and practical solution for robust beam prediction in real-world vehicular networks by adaptively handling missing sensor data while preserving temporal coherence and semantic alignment across modalities.
...
### [105] [Capacity-Achieving Codes with Inverse-Ackermann-Depth Encoders](https://arxiv.org/abs/2512.11443)
*Yuan Li*

Main category: cs.IT

TL;DR: The paper proves that for symmetric discrete memoryless channels with prime power alphabet size q, there exist capacity-approaching error-correcting codes that can be encoded by arithmetic circuits over 𝔽_q with linear size and inverse-Ackermann depth.


<details>
  <summary>Details</summary>
Motivation: The motivation is to find capacity-achieving error-correcting codes that admit highly efficient encoding circuits, specifically seeking codes with both linear size and very shallow depth (inverse-Ackermann function depth) for practical implementation.

Method: The construction composes: (1) a linear code with constant rate and relative distance based on prior work by Gál et al. (2013) and Drucker & Li (2023), with (2) an additional layer formed by a disperser graph whose edge weights are chosen uniformly at random.

Result: The paper proves the existence of capacity-approaching codes encodable by arithmetic circuits over 𝔽_q of size O(n) and depth α(n), where α(n) is a version of the inverse Ackermann function.

Conclusion: The results suggest that certain capacity-achieving codes can have highly efficient encoding circuits with both linear size and inverse-Ackermann depth, making them practically implementable for symmetric discrete memoryless channels with prime power alphabet sizes.
...
### [106] [Stable low-rank matrix recovery from 3-designs](https://arxiv.org/abs/2512.11642)
*Timm Gilles*

Main category: cs.IT

TL;DR: This paper establishes recovery guarantees for low-rank Hermitian matrices from rank-one measurements using 3-designs, closing the gap between known results for 2-designs (insufficient) and 4-designs (optimal).


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the theoretical gap between 2-designs (which don't allow subquadratic recovery) and 4-designs (which have near-optimal guarantees), while providing practical advantages since 3-designs are easier to construct than 4-designs.

Method: The method uses nuclear-norm minimization to recover low-rank Hermitian matrices from rank-one measurements obtained by uniform sampling from complex projective 3-designs. This framework includes PhaseLift for phase retrieval as a special case.

Result: The paper establishes recovery guarantees for 3-designs that parallel the best-known results for 4-designs, deriving bounds on the number of measurements sufficient for stable and robust low-rank recovery via nuclear-norm minimization.

Conclusion: This work closes the theoretical gap by showing that 3-designs provide recovery guarantees comparable to 4-designs, with practical advantages since explicit constructions of 3-designs are significantly easier than those of 4-designs.
...
<details>
  <summary>Details</summary>
Motivation: Traditional MILP models for combinatorial sequential decision making often fail in real-world applications because they prioritize accurate modeling over decision quality. Existing ML approaches rely on supervised learning with optimal decision labels and use surrogate gradients, limiting their effectiveness.

Method: CORL framework casts MILP solved by branch-and-bound as a differentiable stochastic policy compatible with RL, enabling end-to-end fine-tuning using reinforcement learning on real-world data to maximize operational performance.

Result: The method is validated in a simple illustrative combinatorial sequential decision making example, demonstrating proof of concept for the CORL framework.

Conclusion: CORL provides a novel approach to improve MILP-based decision making by directly optimizing for real-world performance using reinforcement learning, overcoming limitations of traditional modeling and supervised learning methods.
...
### [108] [Deep Learning--Accelerated Multi-Start Large Neighborhood Search for Real-time Freight Bundling](https://arxiv.org/abs/2512.11187)
*Haohui Zhang,Wouter van Heeswijk,Xinyu Hu,Neil Yorke-Smith,Martijn Mes*

Main category: cs.AI

TL;DR: Learning-accelerated hybrid search pipeline combining Transformer-based constructive policy with Multi-Start Large Neighborhood Search for combinatorial freight bundling in online freight exchanges.
...
Method: Model the problem as multi-commodity one-to-one pickup-and-delivery selective traveling salesperson problem (m1-PDSTSP). Propose a hybrid pipeline: Transformer Neural Network-based constructive policy paired with Multi-Start Large Neighborhood Search metaheuristic within rolling-horizon scheme.

Result: Outperforms state-of-the-art neural combinatorial optimization and metaheuristic baselines, achieving optimality gap less than 2% in total revenue relative to best exact baseline method with comparable time.

Conclusion: First work showing Deep Neural Network-based constructors can reliably provide high-quality seeds for multi-start improvement heuristics, with applicability beyond m1-PDSTSP to broader class of selective traveling salesperson and pickup-delivery problems.
...
### [109] [FutureWeaver: Planning Test-Time Compute for Multi-Agent Systems with Modularized Collaboration](https://arxiv.org/abs/2512.11213)
*Dongwon Jung,Peng Shi,Yi Zhang*

Main category: cs.AI

TL;DR: FutureWeaver is a framework that optimizes test-time compute allocation in multi-agent systems under fixed budgets, using modularized collaboration and dual-level planning to improve performance without additional training.


<details>
  <summary>Details</summary>
Motivation: While test-time computation scaling improves single-agent LLM performance, there's no principled way to allocate compute for collaboration in multi-agent systems, nor mechanisms to distribute compute across agents under budget constraints.

Method: FutureWeaver introduces modularized collaboration (callable functions encapsulating reusable multi-agent workflows derived through self-play reflection) and a dual-level planning architecture that optimizes compute allocation by reasoning over current task state while speculating on future steps.
...
Conclusion: FutureWeaver successfully addresses the gap in principled test-time compute allocation for multi-agent systems, enabling effective collaboration optimization under budget constraints through modularization and dual-level planning.
...
### [110] [A-LAMP: Agentic LLM-Based Framework for Automated MDP Modeling and Policy Generation](https://arxiv.org/abs/2512.11270)
*Hong Je-Gal,Chan-Bin Yi,Hyun-Suk Lee*

Main category: cs.AI

TL;DR: A-LAMP is an LLM-based framework that automatically converts natural language task descriptions into formal MDP formulations and trained RL policies through verifiable stages, outperforming single LLMs and preserving task optimality.


<details>
  <summary>Details</summary>
Motivation: Applying RL to real-world tasks requires manual conversion of informal descriptions to formal MDPs, environment implementation, and policy training, which is challenging due to modeling errors, fragile code, and misaligned objectives that impede policy training.

Method: A-LAMP uses an agentic LLM-based framework that decomposes the process into verifiable stages: MDP modeling, coding, and training. It translates free-form natural language task descriptions into MDP formulations and trained policies, ensuring semantic alignment throughout the pipeline.

Result: A-LAMP consistently achieves higher policy generation capability than single state-of-the-art LLMs across classic control and custom RL domains. Even its lightweight variant with smaller models approaches larger model performance. Case studies show it generates environments and policies that preserve task optimality.

Conclusion: A-LAMP provides a reliable framework for automated RL task formulation and policy generation that overcomes traditional challenges, demonstrates correctness through optimality preservation, and offers efficiency through lightweight variants that match larger model performance.
...
### [111] [TriFlow: A Progressive Multi-Agent Framework for Intelligent Trip Planning](https://arxiv.org/abs/2512.11271)
*Yuxing Chen,Basem Suleiman,Qifan Chen*

Main category: cs.AI

TL;DR: TriFlow is a multi-agent framework for trip planning that combines structured reasoning with LLM flexibility to create feasible itineraries under constraints, achieving SOTA performance with 10x efficiency gains.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based trip planning agents struggle with constraint satisfaction, tool coordination, and efficiency, often producing infeasible or costly plans that don't meet real-world spatial, temporal, and budgetary requirements.

Method: TriFlow uses a progressive three-stage pipeline: retrieval, planning, and governance. It combines rule-based reasoning with LLM flexibility, progressively narrowing search space, assembling constraint-consistent itineraries via rule-LLM collaboration, and performing bounded iterative refinement.

Result: Achieved state-of-the-art results with 91.1% final pass rate on TravelPlanner and 97% on TripTailor benchmarks, with over 10x runtime efficiency improvement compared to current SOTA methods.

Conclusion: TriFlow successfully addresses limitations of existing LLM-based trip planning agents by unifying structured reasoning with language-based flexibility, enabling efficient generation of feasible, personalized itineraries under real-world constraints.
...
### [112] [CAPTURE: A Benchmark and Evaluation for LVLMs in CAPTCHA Resolving](https://arxiv.org/abs/2512.11323)
*Jianyi Zhang,Ziyin Zhou,Xu Ji,Shizhao Liu,Zhangchi Zhao*

Main category: cs.AI

TL;DR: CAPTURE is the first comprehensive CAPTCHA benchmark specifically designed for Large Visual Language Models (LVLMs), covering 4 main types and 25 sub-types from 31 vendors to thoroughly evaluate LVLM performance on CAPTCHA solving.


<details>
  <summary>Details</summary>
Motivation: Existing CAPTCHA benchmarks for LVLMs are limited - they were customized for specific research objectives and don't comprehensively cover all CAPTCHA types. There's a lack of dedicated benchmarks specifically designed for LVLMs to evaluate their CAPTCHA-solving capabilities.

Method: Created CAPTURE (CAPTCHA for Testing Under Real-world Experiments) benchmark with extensive coverage: 4 main CAPTCHA types, 25 sub-types from 31 different vendors. Features large-scale data, diverse classes, and unique LVLM-tailored labels designed specifically for evaluating LVLMs.

Result: When evaluated using the CAPTURE benchmark, current LVLMs demonstrate poor performance in solving CAPTCHAs, highlighting their limitations despite strong multi-modal alignment capabilities.

Conclusion: CAPTURE fills the gap in comprehensive CAPTCHA evaluation for LVLMs with its diverse coverage and tailored design, revealing that current LVLMs still struggle with CAPTCHA solving despite their advanced visual-language capabilities.
...
### [113] [Towards Trustworthy Multi-Turn LLM Agents via Behavioral Guidance](https://arxiv.org/abs/2512.11421)
*Gonca Gürsun*

Main category: cs.AI

TL;DR: A framework for LLM-based agents that ensures reliable, verifiable behavior in multi-turn tasks by integrating task profiling, verifiable reasoning, and constraint-compliant generation within RL environments.


<details>
  <summary>Details</summary>
Motivation: LLMs show strong reasoning and generation capabilities but often lack reliability and verifiability in multi-turn tasks, making their behavior unpredictable and hard to trust in complex environments.

Method: A three-component framework: 1) lightweight task profiler selects reasoning/generation strategies, 2) reasoning module learns verifiable observation-action mappings, 3) generation module enforces constraint-compliant outputs through validation or deterministic synthesis.

Result: The components co-evolve as the agent interacts with the environment, yielding trustworthy behavior that is both reliable and verifiable in multi-turn tasks.

Conclusion: The framework enables LLM-based agents to operate under explicit behavioral guidance in RL environments, addressing the reliability and verifiability gaps in current LLM applications for multi-turn tasks.
...
### [114] [AgentBalance: Backbone-then-Topology Design for Cost-Effective Multi-Agent Systems under Budget Constraints](https://arxiv.org/abs/2512.11426)
*Shuowei Cai,Yansong Ning,Hao Liu*

Main category: cs.AI

TL;DR: AgentBalance is a framework for building cost-effective multi-agent systems under explicit token-cost and latency budgets through backbone-then-topology design.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based multi-agent systems focus on communication topologies and agent backbones but don't optimize under explicit token-cost and latency budgets, leading to suboptimal cost-effectiveness when budgets are binding constraints.

Method: Two-stage approach: 1) Backbone-oriented agent generation (LLM pool construction, pool selection, role-backbone matching), 2) Adaptive MAS topology generation (agent representation learning, gating, latency-aware topology synthesis).

Result: Achieves up to 10% and 22% performance gains under matched token-cost and latency budgets respectively, strong AUC on performance-vs-budget curves, and works as a plug-in for existing MAS.

Conclusion: AgentBalance enables practical, budget-aware deployment of LLM-based multi-agent systems by optimizing both agent backbones and communication topologies under explicit cost constraints.
...
### [115] [Back to the Baseline: Examining Baseline Effects on Explainability Metrics](https://arxiv.org/abs/2512.11433)
*Agustin Martin Picard,Thibaut Boissin,Varshini Subhash,Rémi Cadène,Thomas Fel*

Main category: cs.AI

TL;DR: The paper reveals a critical flaw in popular XAI evaluation metrics (Insertion/Deletion) where baseline choice biases results, proposes criteria for better baselines, and introduces a novel model-dependent baseline that improves the trade-off between information removal and avoiding out-of-distribution images.


<details>
  <summary>Details</summary>
Motivation: Current XAI evaluation metrics (Insertion/Deletion) rely on baselines that arbitrarily favor certain attribution methods, creating unreliable comparisons where even simple linear models contradict themselves depending on baseline choice.

Method: The paper analyzes baseline problems through two criteria: information removal and avoiding out-of-distribution images. It tests existing baselines against these criteria and introduces a novel model-dependent baseline using feature visualization techniques to create baselines that remove information without producing overly OOD images.

Result: Existing baselines fail to satisfy both criteria simultaneously, showing a trade-off: they either remove information effectively or avoid OOD images. The proposed model-dependent baseline improves this trade-off compared to existing baselines.

Conclusion: Baseline choice fundamentally biases XAI evaluation metrics, requiring careful consideration. The proposed model-dependent baseline offers a better solution by balancing information removal with distributional properties, though the trade-off between these criteria remains an open challenge.
...
### [116] [Motif-2-12.7B-Reasoning: A Practitioner's Guide to RL Training Recipes](https://arxiv.org/abs/2512.11463)
...
TL;DR: Motif-2-12.7B-Reasoning is a 12.7B parameter language model that achieves frontier-level reasoning performance through innovative training techniques addressing model collapse and stability issues.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between open-weight systems and proprietary frontier models in complex reasoning and long-context understanding, while addressing common challenges of model collapse and training instability in reasoning adaptation.

Method: Comprehensive training recipe with: 1) Memory-efficient infrastructure for 64K-token contexts using hybrid parallelism and kernel-level optimizations, 2) Two-stage SFT curriculum with verified synthetic data to mitigate distribution mismatch, 3) Robust RLFT pipeline with difficulty-aware data filtering and mixed-policy trajectory reuse for training stability.
...
Conclusion: Provides the community with both a competitive open reasoning model and a practical blueprint for scaling reasoning capabilities under realistic compute constraints.
...
### [117] [Three methods, one problem: Classical and AI approaches to no-three-in-line](https://arxiv.org/abs/2512.11469)
...
TL;DR: First systematic comparison of classical optimization (ILP) vs AI methods (PatternBoost transformer & PPO) for No-Three-In-Line problem on n×n grids, showing ILP optimal up to 19×19, PatternBoost matches optimal up to 14×14, PPO works only up to 10×10.


<details>
  <summary>Details</summary>
Motivation: The No-Three-In-Line problem is a famous combinatorial geometry challenge where classical methods like ILP guarantee optimal solutions but scale exponentially with grid size. Recent advances in machine learning offer promising alternatives for pattern-based approximation, motivating the first systematic comparison of classical optimization and AI approaches.

Method: Applied three approaches: 1) Integer Linear Programming (ILP) as classical optimization baseline, 2) PatternBoost transformer learning (first application to this problem), and 3) Reinforcement learning with PPO (first application to this problem). Compared performance against traditional algorithms on n×n grids.

Result: ILP achieved provably optimal solutions up to 19×19 grids. PatternBoost matched optimal performance up to 14×14 grids with 96% test loss reduction. PPO achieved perfect solutions on 10×10 grids but failed at 11×11 grids due to constraint violations preventing valid configurations.
...
### [118] [General-purpose AI models can generate actionable knowledge on agroecological crop protection](https://arxiv.org/abs/2512.11474)
*Kris A. G. Wyckhuys*

Main category: cs.AI

TL;DR: LLMs like DeepSeek and ChatGPT show promise for agroecological knowledge generation but have accuracy limitations including hallucinations and data omissions.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of generative AI in democratizing scientific knowledge for agri-food science, specifically for agroecological crop protection, which remains an unexplored application area.

Method: Compared web-grounded (DeepSeek) vs non-grounded (ChatGPT free-tier) LLMs on factual accuracy, data consistency, and knowledge breadth for nine globally limiting pests, weeds, and plant diseases. Assessed literature corpus size, biological control agents reported, and efficacy estimates.

Result: DeepSeek screened 4.8-49.7x larger literature corpus, reported 1.6-2.4x more biological control agents/solutions, showed 21.6% higher efficacy estimates, better lab-to-field consistency, and more realistic effects. Both models hallucinated (fabricated references/agents), reported implausible ecological interactions, confused nomenclatures, and omitted key data.

Conclusion: LLMs can report correct low-resolution efficacy trends and, when paired with rigorous human oversight, may become powerful tools for farm-level decision-making and scientific creativity in agroecological crop protection.
...
<details>
  <summary>Details</summary>
Motivation: AI-generated text detectors are increasingly used in education and professional settings, but prior research only identified isolated cases of bias (particularly against English Language Learners). There's a lack of systematic evaluation across broader sociolinguistic factors, creating a need for comprehensive bias assessment before widespread deployment.

Method: Proposed BAID framework with 200k+ samples spanning 7 bias categories: demographics, age, educational grade level, dialect, formality, political leaning, and topic. Created synthetic versions of each sample using carefully crafted prompts to preserve content while reflecting subgroup-specific writing styles. Evaluated four open-source state-of-the-art AI text detectors using this dataset.

Result: Found consistent disparities in detection performance, particularly low recall rates for texts from underrepresented groups. The framework reveals systematic bias patterns that isolated evaluations would miss.

Conclusion: BAID provides a scalable, transparent approach for auditing AI detectors and emphasizes the critical need for bias-aware evaluation before deploying these tools for public use. The systematic framework helps identify and address performance disparities across diverse sociolinguistic groups.
...
### [120] [EmeraldMind: A Knowledge Graph-Augmented Framework for Greenwashing Detection](https://arxiv.org/abs/2512.11506)
...
TL;DR: EmeraldMind is a fact-centric framework that uses a domain-specific knowledge graph and retrieval-augmented generation to detect greenwashing in corporate ESG claims with transparent, evidence-backed verdicts.


<details>
  <summary>Details</summary>
Motivation: As AI systems become more involved in decision-making, there's a critical need to design intelligent systems that support sustainability efforts while guarding against misinformation like greenwashing (misleading corporate sustainability claims), which poses a major challenge to environmental progress.

Method: EmeraldMind integrates a domain-specific knowledge graph (EmeraldGraph) built from diverse corporate ESG reports with retrieval-augmented generation. The framework surfaces verifiable evidence often missing in generic knowledge bases and supports large language models in claim assessment, delivering justification-centric classifications with transparent, evidence-backed verdicts while responsibly abstaining when claims cannot be verified.

Result: Experiments on a new greenwashing claims dataset show that EmeraldMind achieves competitive accuracy, greater coverage, and superior explanation quality compared to generic LLMs, without requiring fine-tuning or retraining.

Conclusion: EmeraldMind provides an effective framework for automated greenwashing detection that combines domain-specific knowledge with LLM capabilities to deliver transparent, evidence-based assessments of corporate sustainability claims.
...
### [121] [AI-MASLD Metabolic Dysfunction and Information Steatosis of Large Language Models in Unstructured Clinical Narratives](https://arxiv.org/abs/2512.11544)
*Yuan Shen,Xiaojun Wu,Linghua Yu*

Main category: cs.AI

TL;DR: LLMs show functional defects in extracting medical info from noisy patient complaints, with performance decline resembling metabolic dysfunction (AI-MASLD concept).


<details>
  <summary>Details</summary>
Motivation: To evaluate LLMs' ability to extract core medical information from noisy patient complaints in clinical scenarios and check if they show functional decline similar to metabolic dysfunction.

Method: Cross-sectional analysis using standardized medical probes on four LLMs (GPT-4o, Gemini 2.5, DeepSeek 3.1, Qwen3-Max). Used 20 medical probes across 5 dimensions with gold-standard answers, assessed via double-blind inverse rating by two clinicians.

Result: All models showed functional defects, with Qwen3-Max best and Gemini 2.5 worst. Most models collapsed under extreme noise. GPT-4o made severe misjudgment in PE risk assessment secondary to DVT.

Conclusion: First empirical confirmation that LLMs exhibit metabolic dysfunction-like features when processing clinical info (AI-MASLD concept). LLMs need human supervision as auxiliary tools due to significant gap between theoretical knowledge and practical application.
...
### [122] [AI Benchmark Democratization and Carpentry](https://arxiv.org/abs/2512.11588)
...
TL;DR: AI benchmarks need to evolve from static to dynamic, adaptive frameworks to keep pace with AI's rapid development and address real-world deployment challenges, requiring new educational initiatives in "AI Benchmark Carpentry."


<details>
  <summary>Details</summary>
Motivation: Current AI benchmarks are becoming obsolete due to rapid model evolution, memorization by LLMs, and lack of real-world relevance. Static benchmarks create a gap between reported results and actual deployment performance, necessitating more adaptive evaluation approaches.

Method: Proposes dynamic benchmarking frameworks that incorporate evolving models, updated data, and heterogeneous platforms while maintaining transparency and reproducibility. Advocates for "AI Benchmark Carpentry" education to build expertise in benchmark design and use across different skill levels.

Result: Identifies key barriers: high resource demands, limited hardware access, lack of design expertise, and uncertainty in relating results to applications. Current benchmarks focus too much on peak performance on top-tier hardware, offering limited guidance for diverse real-world scenarios.

Conclusion: Benchmarking must become dynamic and inclusive to keep pace with AI evolution. Community efforts should support application-relevant comparisons and build sustained expertise through systematic education, ensuring responsible, reproducible, and accessible AI deployment.
...
### [123] [Causal Inference in Energy Demand Prediction](https://arxiv.org/abs/2512.11653)
*Chutian Ma,Grigorii Pomazkin,Giacinto Paolo Saggese,Paul Smith*

Main category: cs.AI

TL;DR: Proposes a structural causal model for energy demand prediction that captures causal relationships between weather, calendar factors, and demand, then builds a Bayesian model using these causal insights as priors, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Energy demand prediction is critical for grid operations but complex due to interdependent causal factors like weather conditions and calendar information that affect daily schedules. Simple correlation-based approaches are insufficient for capturing these causal relationships.

Method: 1) Develop a structural causal model to explain causal relationships between variables (weather, calendar, demand). 2) Perform full causal analysis to validate causal beliefs. 3) Build a Bayesian model that incorporates learned causal insights as prior knowledge. 4) Train and test on unseen data with cross-validation.

Result: Achieved state-of-the-art performance with 3.84% MAPE on test set. Cross-validation across two years yielded average MAPE of 3.88%, demonstrating strong robustness. Causal analysis revealed important insights: energy demand responds to temperature fluctuations with season-dependent sensitivity, and exhibits lower variance in winter due to decoupling between temperature changes and daily activity patterns.

Conclusion: The proposed causal modeling approach successfully captures complex interdependencies in energy demand prediction, outperforming correlation-based methods. Incorporating causal insights as priors in Bayesian models leads to accurate and robust predictions with practical applications for grid operators and energy consumers.
...
TL;DR: TxAgent is an AI system for therapeutic decision-making that uses iterative RAG with a fine-tuned Llama-3.1-8B model to dynamically call biomedical tools, achieving Excellence Award in Open Science at CURE-Bench NeurIPS 2025 Challenge.


<details>
  <summary>Details</summary>
Motivation: Therapeutic decision-making requires robust multi-step reasoning with reliable biomedical knowledge, but medical AI faces stringent safety constraints where both reasoning traces and tool invocations must be accurate.

Method: TxAgent uses iterative retrieval-augmented generation with a fine-tuned Llama-3.1-8B model that dynamically generates and executes function calls to a unified biomedical tool suite (ToolUniverse) integrating FDA Drug API, OpenTargets, and Monarch resources.

Result: The system demonstrated performance gains through improved tool-retrieval strategies and won the Excellence Award in Open Science at the CURE-Bench NeurIPS 2025 Challenge, which benchmarks therapeutic-reasoning systems on correctness, tool utilization, and reasoning quality.

Conclusion: Retrieval quality for function calls significantly influences overall model performance in therapeutic reasoning, and treating token-level reasoning and tool-usage behaviors as explicit supervision signals is crucial for medical AI safety and accuracy.
...
### [125] [An Open Source Realtime GPU Beamformer for Row-Column and Top Orthogonal to Bottom Electrode (TOBE) Arrays](https://arxiv.org/abs/2512.11086)
*Randy Palamar,Darren Dahunsi,Tyler Henry,Mohammad Rahim Sobhani,Roger Zemp*

Main category: eess.IV

TL;DR: Open-source GPU-accelerated software enables real-time navigation for 2D ultrasound arrays like row-column arrays, integrated with novel TOBE arrays for improved imaging.


<details>
  <summary>Details</summary>
Motivation: Research ultrasound platforms lack real-time navigation capabilities for emerging 2D arrays like row-column arrays, limiting their practical clinical application and advanced imaging potential.

Method: Developed open-source GPU-accelerated reconstruction and rendering software integrated with programmable ultrasound platform and novel electrostrictive TOBE arrays, using OpenGL compute shaders for beamforming and rendering with maximum data throughput.

Result: System supports advanced real-time modes including cross-plane aperture-encoded synthetic-aperture imaging and aperture-encoded volumetric scanning, with TOBE-enabled methods showing improved image quality and expanded field of view compared to conventional RCA techniques.

Conclusion: The software suite enables real-time navigation for 2D ultrasound arrays, provides sample datasets for external testing, and demonstrates superior performance with novel TOBE arrays compared to conventional approaches.
...
### [126] [Feature Compression for Machines with Range-Based Channel Truncation and Frame Packing](https://arxiv.org/abs/2512.11134)
*Juan Merlos,Fabien Racapé,Hyomin Choi,Mateen Ulhaq,Hari Kalva*

Main category: eess.IV

TL;DR: Proposes channel truncation and packing method for MPEG-FCM standard to enhance compression of intermediate neural network features in split computing, achieving 10.59% average rate reduction while preserving task accuracy.


<details>
  <summary>Details</summary>
Motivation: The MPEG-FCM standard aims to provide interoperable compressed bitstreams of features for split computing scenarios where neural network inference is split between devices. Current methods convert 3D feature tensors to 2D video frames for compression, but there's a need to better preserve relevant channels and reduce bandwidth while maintaining task performance.

Method: Introduces an additional channel truncation and packing method that preserves relevant channels based on feature statistics at inference time. The method operates within the MPEG-FCM test model framework, where intermediate feature tensors are reduced using neural layers before being converted to 2D video frames for compression with existing video standards.

Result: The proposed method yields an average reduction in rate by 10.59% for a given accuracy across multiple computer vision tasks and datasets, demonstrating improved compression performance while preserving computer vision task performance at the receiver.

Conclusion: The channel truncation and packing method effectively enhances the compression performance of the MPEG-FCM standard for feature coding in split computing scenarios, achieving significant bandwidth reduction without compromising task accuracy.
...
TL;DR: mViSE is a query-driven visual search engine for multiplex brain imaging that learns tissue architecture without programming, enabling retrieval of cells, niches, and brain regions through self-supervised learning.


<details>
  <summary>Details</summary>
Motivation: Whole-slide multiplex brain imaging produces massive, information-dense images that are challenging to analyze and typically require custom software and programming skills, creating barriers for researchers.

Method: Divide-and-conquer strategy organizing data into panels of related molecular markers, using self-supervised learning to train multiplex encoders for each panel with visual confirmation. Multiple panels can be combined for visual queries using information-theoretic methods to retrieve similar cells or multicellular niches.

Result: Successfully validated mViSE's ability to retrieve single cells, proximal cell pairs, tissue patches, delineate cortical layers, and identify brain regions and sub-regions without programming.

Conclusion: mViSE provides a programming-free, query-driven alternative for analyzing multiplex brain imaging data, enabling tissue exploration and region profiling through visual search, available as an open-source QuPath plug-in.
...
### [128] [Scalable Data Synthesis for Computer Use Agents with Step-Level Filtering](https://arxiv.org/abs/2512.10962)
*Yifei He,Pranit Chawla,Yaser Souri,Subhojit Som,Xia Song*

Main category: cs.LG

TL;DR: WebSTAR: A scalable data synthesis pipeline that filters noisy CUA rollouts into reliable training data via step-level filtering and reasoning augmentation, achieving SOTA results on WebVoyager benchmark.


<details>
  <summary>Details</summary>
Motivation: Computer use agents (CUAs) are hard to train due to expensive GUI interaction costs and limited high-quality trajectory data from human demonstrations. Noisy rollouts from strong CUAs contain many incorrect/suboptimal actions, making naive imitation ineffective.

Method: Step-level filtering pipeline evaluates individual actions to retain only correct steps from noisy CUA rollouts, plus reasoning augmentation for better planning. Created WebSTAR dataset (13.3K trajectories, 100K graded steps) from OpenAI's computer-use-preview model. Also built WebSCORE dataset of graded step-level actions and trained StepRM reward model distilled from o4-mini.

Result: Qwen-2.5-VL-Instruct models trained on WebSTAR: 7B model surpasses UI-TARS-1.5-7B by >15% on WebVoyager with only supervised finetuning. StepRM (7B) matches o4-mini grading quality while being much more efficient for deployment.

Conclusion: Step-level filtering is key for scalable CUA training. WebSTAR, WebSCORE datasets and StepRM reward model provide practical tools for advancing robust, efficient computer use agents.
...
### [129] [Multimodal Fusion of Regional Brain Experts for Interpretable Alzheimer's Disease Diagnosis](https://arxiv.org/abs/2512.10966)
*Farica Zhuang,Dinara Aliyeva,Shu Yang,Zixuan Wen,Duy Duong-Tran,Christos Davatzikos,Tianlong Chen,Song Wang,Li Shen*

Main category: cs.LG

TL;DR: MREF-AD: A Multimodal Regional Expert Fusion model for Alzheimer's disease diagnosis using Mixture-of-Experts framework with two-level gating networks for adaptive fusion of amyloid PET and MRI biomarkers across brain regions.


<details>
  <summary>Details</summary>
Motivation: Conventional multimodal fusion approaches for Alzheimer's disease diagnosis rely on simple feature concatenation, which cannot adaptively balance contributions of different biomarkers (amyloid PET and MRI) across brain regions, unlike clinical practice that integrates complementary information from multiple modalities.

Method: MREF-AD uses a Mixture-of-Experts framework that models meso-scale brain regions in each modality as independent experts, with two-level gating networks to learn subject-specific fusion weights, enabling adaptive multimodal fusion.

Result: Using ADNI data, MREF-AD achieves state-of-the-art performance over baselines while providing enhanced interpretability of brain region-specific biomarker relevance.

Conclusion: MREF-AD serves as a general framework for adaptive and interpretable multimodal fusion in neuroimaging, offering both improved diagnostic performance and insights into how structural and molecular imaging jointly contribute to disease diagnosis.
...
TL;DR: MoB replaces learned gating in MoE with VCG auctions where experts bid true costs (execution + forgetting), achieving stateless routing immune to catastrophic forgetting and emergent specialization without task boundaries.


<details>
  <summary>Details</summary>
Motivation: Mixture of Experts architectures suffer from catastrophic forgetting in continual learning because the learned gating network itself forgets previous tasks, limiting their application to continual learning scenarios.

Method: Replace learned gating networks with Vickrey-Clarke-Groves (VCG) auctions where experts bid their true cost (execution cost = predicted loss + forgetting cost = Elastic Weight Consolidation penalty). Extend with autonomous self-monitoring experts that detect knowledge consolidation boundaries.

Result: On Split-MNIST benchmarks, MoB achieves 88.77% average accuracy vs 19.54% for Gated MoE and 27.96% for Monolithic EWC (4.5× improvement). Provides stateless routing immune to forgetting, truthful bidding guarantee, and emergent specialization without task boundaries.

Conclusion: MoB successfully addresses catastrophic forgetting in MoE architectures through game-theoretic routing mechanisms, enabling effective continual learning with emergent specialization and eliminating need for explicit task demarcation.
...
TL;DR: RL framework optimizes heparin therapy for surgical sepsis using continuous cxSOFA scoring and TECM evaluation, reducing mortality and hospital stays.


<details>
  <summary>Details</summary>
Motivation: Sepsis is life-threatening and requires optimized heparin therapy. Current discrete SOFA scoring lacks nuance for RL-based treatment optimization.

Method: Converted discrete SOFA to continuous cxSOFA for better state/reward functions; defined stepwise treatment strategies; created Treatment Effect Comparison Matrix (TECM) for evaluation; applied Q-Learning, DQN, DDQN, BCQ, CQL algorithms.
...
Conclusion: RL framework enables interpretable heparin therapy optimization. Continuous cxSOFA and TECM provide nuanced assessment, improving clinical outcomes and decision-support reliability.
...
TL;DR: A multi-agent framework for training multimodal emotion recognition systems that improves flexibility, scalability, and training efficiency compared to traditional deep learning approaches.


<details>
  <summary>Details</summary>
Motivation: Current multimodal emotion recognition systems using deep learning are computationally intensive and inflexible to modality changes, making them difficult to maintain and adapt for human-agent interaction scenarios.
...
Result: Demonstrated feasibility through a proof-of-concept implementation supporting vision, audio, and text modalities, with improved training efficiency and flexibility for modality integration.
...
TL;DR: MolSculpt is a novel framework that generates 3D molecular geometries by "sculpting" them from chemical syntax, bridging the gap between 1D molecular representations and 3D geometric realization.


<details>
  <summary>Details</summary>
Motivation: Prior methods using 1D representations like SELFIES fail to fully exploit rich chemical knowledge from 1D models, creating a disconnect between 1D syntactic generation and 3D geometric realization needed for drug discovery and material science.

Method: MolSculpt combines a frozen 1D molecular foundation model with a 3D molecular diffusion model. It uses learnable queries to extract chemical knowledge from the foundation model, then injects this cross-modal information into the diffusion model's conditioning space through a trainable projector for guided 3D geometry generation.

Result: Achieves state-of-the-art performance in both de novo and conditional 3D molecule generation, showing superior 3D fidelity and stability on GEOM-DRUGS and QM9 datasets.

Conclusion: MolSculpt successfully bridges the gap between 1D chemical syntax and 3D geometry by deeply integrating latent chemical knowledge into the 3D generation process through end-to-end optimization.
...
### [134] [Memoryless Policy Iteration for Episodic POMDPs](https://arxiv.org/abs/2512.11082)
...
<details>
  <summary>Details</summary>
Motivation: Memoryless and finite-memory policies are practical for POMDPs as they operate in output space rather than high-dimensional belief space, but extending classical methods like policy iteration is difficult due to non-Markovian output processes.

Method: Introduces a family of monotonically improving policy-iteration algorithms that alternate between single-stage output-based policy improvements and policy evaluations according to prescribed periodic patterns. Identifies optimal patterns maximizing computational efficiency and simplest minimal-period pattern. Develops model-free variant estimating values from data to learn memoryless policies directly.

Result: Achieves significant computational speedups over policy-gradient baselines and recent specialized algorithms across several POMDP examples in both model-based and model-free settings.

Conclusion: The proposed periodic policy-iteration framework provides an efficient approach for solving POMDPs with memoryless/finite-memory policies, overcoming challenges of non-Markovian output processes while enabling both model-based and model-free learning.
...
### [135] [Clip-and-Verify: Linear Constraint-Driven Domain Clipping for Accelerating Neural Network Verification](https://arxiv.org/abs/2512.11087)
*Duo Zhou,Jorge Chavez,Hesun Chen,Grani A. Hanasusanto,Huan Zhang*

Main category: cs.LG

TL;DR: Clip-and-Verify introduces a linear constraint-driven clipping framework to enhance neural network verification efficiency by reducing input space and improving bounds during branch-and-bound procedures.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art NN verifiers rely on branch-and-bound with fast bounding techniques, but there's room for improvement in handling challenging verification properties more efficiently.

Method: Developed linear constraint-driven clipping framework with two novel algorithms: 1) reduces verified/irrelevant input space portions during BaB, 2) directly improves intermediate network bounds. Uses specialized GPU procedure to handle linear constraints without expensive external solvers.

Result: Achieved up to 96% reduction in subproblems during branch-and-bound, state-of-the-art verified accuracy across multiple benchmarks, and integration with α,β-CROWN (VNN-COMP 2025 winner).

Conclusion: Clip-and-Verify provides scalable, efficient verification that consistently tightens bounds and significantly reduces computational burden, making it a valuable addition to modern NN verification toolkits.
...
### [136] [Investigating ECG Diagnosis with Ambiguous Labels using Partial Label Learning](https://arxiv.org/abs/2512.11095)
...
<details>
  <summary>Details</summary>
Motivation: Label ambiguity is inherent in real-world ECG diagnosis due to overlapping conditions and diagnostic disagreement, but current ECG models assume clean annotations, limiting development and evaluation under real-world conditions. PLL frameworks for medical time-series domains like ECG remain largely unexplored.
...
Result: PLL methods vary substantially in their robustness to different types and degrees of ambiguity. The study identified key limitations of current PLL approaches in clinical settings.

Conclusion: Outlined future directions for developing robust and clinically aligned ambiguity-aware learning frameworks for ECG diagnosis, highlighting the need for improved PLL methods that can handle real-world label ambiguity in medical applications.
...
### [137] [Limits and Gains of Test-Time Scaling in Vision-Language Reasoning](https://arxiv.org/abs/2512.11109)
*Mohammadjavad Ahmadpour,Amirmahdi Meighani,Payam Taebi,Omid Ghahroodi,Amirmohammad Izadi,Mahdieh Soleymani Baghshah*

Main category: cs.LG

TL;DR: TTS improves LLM reasoning but its effectiveness for VLMs varies: closed-source models benefit from structured reasoning and self-refinement, while open-source models show inconsistent results with external verification working best. Effectiveness depends on dataset type - good for multi-step reasoning but limited for perception tasks.


<details>
  <summary>Details</summary>
Motivation: Test-time scaling (TTS) has shown promise for improving LLM reasoning by adding computation at inference, but its application to multimodal Vision-Language Models (VLMs) remains underexplored. The paper aims to systematically study how TTS methods perform across different VLMs and benchmarks.

Method: Conducted systematic empirical study of inference time reasoning methods across both open-source and closed-source VLMs on different benchmarks. Evaluated structured reasoning, iterative self-refinement, and external verification approaches.

Result: Closed-source models consistently benefit from structured reasoning and iterative self-refinement. Open-source VLMs show inconsistent behavior: external verification provides most reliable gains, while iterative refinement often degrades performance. TTS effectiveness is dataset-dependent - clear improvements on multi-step reasoning tasks but limited gains on perception-focused benchmarks.

Conclusion: TTS is not a universal solution and must be tailored to both model capabilities and task characteristics. Findings motivate future work on adaptive TTS strategies and multimodal reward models for VLMs.
...
### [138] [In-Context Multi-Objective Optimization](https://arxiv.org/abs/2512.11114)
*Xinyu Zhang,Conor Hassan,Julien Martinelli,Daolang Huang,Samuel Kaski*

Main category: cs.LG

TL;DR: TAMO is a transformer-based amortized policy for multi-objective Bayesian optimization that eliminates per-task surrogate fitting and acquisition engineering, providing fast proposals via pretrained models.


<details>
  <summary>Details</summary>
Motivation: Current multi-objective Bayesian optimization methods require problem-specific tuning of surrogates and acquisition functions, are myopic (single-step planning), and have computational overhead that limits parallel/time-sensitive applications.

Method: TAMO uses a transformer architecture that operates across varying input/objective dimensions, pretrained with reinforcement learning to maximize cumulative hypervolume improvement over full trajectories, conditioning on entire query history.

Result: TAMO reduces proposal time by 50-1000x versus alternatives while matching or improving Pareto quality under tight evaluation budgets across synthetic benchmarks and real tasks.

Conclusion: Transformers can perform multi-objective optimization entirely in-context, eliminating per-task surrogate fitting and acquisition engineering, enabling foundation-style plug-and-play optimizers for scientific workflows.
...
### [139] [Refining Graphical Neural Network Predictions Using Flow Matching for Optimal Power Flow with Constraint-Satisfaction Guarantee](https://arxiv.org/abs/2512.11127)
*Kshitiz Khanal*

Main category: cs.LG

TL;DR: A two-stage learning framework combining physics-informed GNNs and Continuous Flow Matching achieves near-optimal DC-OPF solutions with high feasibility and minimal cost gaps.


<details>
  <summary>Details</summary>
Motivation: Traditional DC-OPF solvers are computationally expensive for large-scale systems requiring frequent recalculations, while existing ML approaches struggle with constraint satisfaction and optimality. There's a need for fast, accurate solutions for modern power systems with high renewable penetration.

Method: Two-stage approach: 1) Physics-informed GNN trained with losses encoding power system constraints (economic dispatch optimality, Kirchhoff's laws, KKT conditions) to produce feasible initial solutions. 2) Continuous Flow Matching refines solutions toward optimality through learned vector field regression.

Result: On IEEE 30-bus system across load scenarios (70-130% nominal), achieves near-optimal solutions with cost gaps below 0.1% for nominal loads and below 3% for extreme conditions, while maintaining 100% feasibility.

Conclusion: The framework bridges fast but approximate neural predictions with optimal but slow numerical solvers, offering practical solution for modern power systems requiring frequent dispatch updates.
...
### [140] [Fairness-Regularized Online Optimization with Switching Costs](https://arxiv.org/abs/2512.11131)
*Pengfei Li,Yuelin Han,Adam Wierman,Shaolei Ren*

Main category: cs.LG

TL;DR: The paper introduces FairOBD, an algorithm for fairness-regularized smoothed online convex optimization with switching costs, addressing the tension between hitting costs, switching costs, and fairness constraints.


<details>
  <summary>Details</summary>
Motivation: Fairness and action smoothness are crucial in online optimization but haven't been addressed simultaneously. The paper aims to solve fairness-regularized smoothed online convex optimization with switching costs, which is challenging due to long-term fairness constraints evaluated over entire action sequences.

Method: Proposes FairOBD (Fairness-regularized Online Balanced Descent), which decomposes long-term fairness costs into online costs using an auxiliary variable to regularize actions. The method handles switching costs through a novel accounting approach and competes against an optimal offline algorithm with parameterized constraints.

Result: Theoretical analysis shows FairOBD achieves worst-case asymptotic competitive ratio against the optimal offline algorithm as T→∞. Empirical evaluation on dynamic computing resource provisioning for AI inference demonstrates FairOBD reduces total fairness-regularized cost and promotes fair outcomes better than baselines.

Conclusion: FairOBD effectively reconciles the tension between hitting costs, switching costs, and fairness costs in online optimization, providing both theoretical guarantees and practical benefits for socially responsible applications like AI inference resource provisioning.
...
### [141] [The Vekua Layer: Exact Physical Priors for Implicit Neural Representations via Generalized Analytic Functions](https://arxiv.org/abs/2512.11138)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: Vekua Layer (VL) is a differentiable spectral method using Generalized Analytic Functions that transforms learning from gradient descent to convex least-squares, achieving machine precision on PDEs and acting as a physics-informed spectral filter.


<details>
  <summary>Details</summary>
Motivation: Implicit Neural Representations (INRs) suffer from spectral bias and computational expense of non-convex optimization, motivating a more efficient and accurate approach for parameterizing physical fields.

Method: VL restricts hypothesis space to kernel of governing differential operator using Harmonic and Fourier-Bessel bases, transforming learning to strictly convex least-squares solved via linear projection rather than iterative gradient descent.

Result: VL achieves machine precision (MSE ≈ 10⁻³³) on exact reconstruction tasks, superior stability with incoherent sensor noise (MSE ≈ 0.03), and enables "holographic" extrapolation from partial boundary data via analytic continuation.

Conclusion: VL provides a physics-informed spectral method that outperforms SIRENs on homogeneous elliptic PDEs, offering exact reconstruction, noise robustness, and analytic continuation capabilities absent in standard coordinate-based approximations.
...
### [142] [Autoencoder-based Semi-Supervised Dimensionality Reduction and Clustering for Scientific Ensembles](https://arxiv.org/abs/2512.11145)
...
<details>
  <summary>Details</summary>
Motivation: Scientific ensemble datasets are high-dimensional and complex, making analysis and visualization challenging. Traditional dimensionality reduction and autoencoders struggle with such data.

Method: Uses EfficientNetV2 to generate pseudo-labels for unlabeled data. Develops enhanced autoencoder with three losses: reconstruction, clustering (soft silhouette score), and contrastive loss. Applies UMAP to latent space for 2D visualization.

Result: Models with clustering or contrastive loss marginally outperform baseline approaches on two scientific ensemble datasets (soil channel structures and droplet-on-film impact dynamics).

Conclusion: The proposed framework with clustering and contrastive losses enhances feature extraction and visualization of complex scientific ensemble data.
...
### [143] [Harnessing Rich Multi-Modal Data for Spatial-Temporal Homophily-Embedded Graph Learning Across Domains and Localities](https://arxiv.org/abs/2512.11178)
*Takuya Kurihana,Xiaojian Zhang,Wing Yee Au,Hon Yung Wong*

Main category: cs.LG

TL;DR: Proposes a heterogeneous data pipeline for cross-domain urban data fusion that integrates spatial-varying datasets into graph learning, enabling scalable smart city analytics across multiple domains and localities.


<details>
  <summary>Details</summary>
Motivation: Modern cities need data-driven insights but face challenges with heterogeneous, multi-modal data collected independently by different agencies with diverse standards. National datasets are consumable but exhibit significant heterogeneity, making cross-domain urban problem-solving difficult.

Method: A heterogeneous data pipeline that performs cross-domain data fusion over time-varying, spatial-varying, and spatial-varying time-series datasets. The data-learning module integrates homophily from spatial-varying datasets into graph learning, embedding information from various localities into models. Uses over 50 data sources.

Result: Demonstrated through five real-world observations using publicly accessible datasets (ride-share, traffic crash, crime reports) from multiple cities. The framework shows strong predictive performance while requiring minimal reconfiguration when transferred to new localities or domains.

Conclusion: The research advances scalable data-informed urban systems, addressing pressing challenges in smart city analytics by providing a flexible, generalizable framework for heterogeneous urban data fusion across multiple domains and localities.
...
### [144] [Bandwidth-constrained Variational Message Encoding for Cooperative Multi-agent Reinforcement Learning](https://arxiv.org/abs/2512.11179)
...
Result: BVME achieves comparable or superior performance across SMACv1, SMACv2, and MPE benchmarks while using 67-83% fewer message dimensions. Gains are most pronounced on sparse graphs where message quality critically impacts coordination. Ablations show U-shaped sensitivity to bandwidth, with BVME excelling at extreme compression ratios.

Conclusion: BVME provides an effective solution for bandwidth-limited MARL by introducing variational message encoding that maintains coordination performance under severe bandwidth constraints while adding minimal computational overhead.
...
TL;DR: The paper proposes progress-oriented benchmarks where solving the benchmark itself advances the field, demonstrated through a NanoGPT speedrun environment that achieved new state-of-the-art training time.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks test LLMs on static, already-solved problems, which constrains the kinds of advances we can measure and incentivize. The authors argue for benchmarks whose objectives are themselves core targets of scientific progress.

Method: The authors instantiate a progress-oriented benchmark environment based on NanoGPT speedrun, standardizing dataset slice, reference model and training harness, rich telemetry, with run-time verification and anti-gaming checks. Evaluation focuses on scientific delta: best-attained loss and efficiency frontier.

Result: Using this environment, they achieved a new state-of-the-art training time, improving upon the previous record by 3 seconds, and qualitatively observed the emergence of novel algorithmic ideas.

Conclusion: The benchmark's purpose is to catalyze reusable improvements to the language modeling stack and seed a community shift from static problem leaderboards to test-time research on open-ended yet measurable scientific problems, reframing "benchmarking" as a vehicle for scientific advancement.
...
### [146] [On the failure of ReLU activation for physics-informed machine learning](https://arxiv.org/abs/2512.11184)
*Conor Rowan*

Main category: cs.LG

TL;DR: ReLU performs poorly in physics-informed ML due to automatic differentiation issues with discontinuous fields, not just its inability to handle second-order derivatives.


<details>
  <summary>Details</summary>
Motivation: To diagnose why ReLU consistently underperforms compared to other activation functions (sigmoid, tanh, swish) in physics-informed machine learning, despite being widely used in other ML domains.

Method: Analyzed ReLU's failure in physics-informed ML by examining automatic differentiation behavior in PyTorch, specifically how it handles derivatives of discontinuous fields during training.

Result: Found that ReLU fails even on first-order variational problems, not just second-order differential equations. The root cause is that automatic differentiation fails to properly characterize derivatives of discontinuous fields, leading to mis-specified gradients of the physics-informed loss.

Conclusion: ReLU's poor performance in physics-informed ML stems from fundamental issues with how automatic differentiation handles its discontinuous nature during training, explaining why smooth activation functions consistently outperform it in this domain.
...
### [147] [Beyond Memorization: Gradient Projection Enables Selective Learning in Diffusion Models](https://arxiv.org/abs/2512.11194)
*Divya Kothandaraman,Jaclyn Pytlarz*

Main category: cs.LG

TL;DR: A Gradient Projection Framework for concept-level dememorization in diffusion models that removes sensitive feature influences during training while preserving generation quality.


<details>
  <summary>Details</summary>
Motivation: Memorization in text-to-image diffusion models creates security/IP risks, enabling unauthorized reproduction of sensitive features. Current dememorization techniques fail at concept-level feature exclusion, and discarding all relevant images wastes valuable training data.

Method: Gradient Projection Framework that operates during backpropagation by identifying and removing training signals aligned with prohibited attribute embeddings. Projects gradient updates onto orthogonal complement of sensitive feature's embedding space to zero out its influence on model weights.

Result: Framework drastically reduces memorization while rigorously preserving generation quality and semantic fidelity. Integrates seamlessly into standard training pipelines and complements existing defenses against feature extraction attacks.
...
### [148] [Fast EXP3 Algorithms](https://arxiv.org/abs/2512.11201)
...
<details>
  <summary>Details</summary>
Motivation: To address the computational efficiency of EXP3 algorithm in adversarial multi-armed bandit problems, making it more practical for real-world applications by reducing time complexity while maintaining regret guarantees.

Method: Proposes constant-time implementations of EXP3 and introduces new practical algorithms that balance computational efficiency with theoretical regret bounds.

Result: Shows that EXP3 can achieve constant time per round while maintaining sublinear regret, and presents algorithms with varying trade-offs between time complexity and regret performance.

Conclusion: The paper demonstrates that efficient constant-time algorithms for adversarial bandits are possible, providing practical solutions with clear trade-offs between computational complexity and regret guarantees.
...
### [149] [Latent Variable Causal Discovery under Selection Bias](https://arxiv.org/abs/2512.11219)
*Haoyue Dai,Yiwen Qiu,Ignavier Ng,Xinshuai Dong,Peter Spirtes,Kun Zhang*

Main category: cs.LG

TL;DR: Rank constraints in covariance matrices can identify latent variable causal structures under selection bias, enabling identification of one-factor models despite biased data.


<details>
  <summary>Details</summary>
Motivation: Selection bias in latent variable causal discovery is important but underexplored due to lack of statistical tools. Existing methods for handling latent variables haven't been adapted for selection bias scenarios.

Method: Uses rank constraints as a generalization of conditional independence constraints, exploiting ranks of covariance submatrices in linear Gaussian models. Provides graph-theoretic characterization of these rank constraints under selection bias.

Result: Rank constraints in biased covariance matrices preserve meaningful information about causal structures and selection mechanisms. Demonstrates that one-factor models can be identified under selection bias using this approach.

Conclusion: Rank constraints provide an effective tool for latent variable causal discovery under selection bias, with simulations and real-world experiments confirming their effectiveness.
...
TL;DR: ASR-KF-EGR is a training-free inference framework that reduces KV cache memory by 55-67% through reversible soft-freezing of low-importance tokens, preserving them in off-GPU storage for on-demand recovery.


<details>
  <summary>Details</summary>
Motivation: To enable efficient large language model generation in memory-constrained environments without requiring fine-tuning, addressing the memory bottleneck of KV cache during long-context inference.

Method: Uses reversible soft-freeze mechanism to temporarily suspend KV updates for low-importance tokens within sliding attention window, with sublinear freeze scheduling that increases freeze duration gradually based on repeated detections, and preserves all tokens in off-GPU storage for recovery.

Result: Demonstrates 55-67% reduction in active KV cache size on LLaMA-3 8B while maintaining generation quality and passing needle-in-haystack retrieval tests.

Conclusion: Provides a practical, architecture-agnostic solution for memory-efficient LLM deployment that requires no fine-tuning and preserves context integrity through reversible freezing.
...
### [151] [Task-Aware Multi-Expert Architecture For Lifelong Deep Learning](https://arxiv.org/abs/2512.11243)
*Jianyu Wang,Jacob Nean-Hua Sheikh,Cat P. Le,Hoda Bidkhori*

Main category: cs.LG

TL;DR: TAME is a continual learning algorithm that uses task similarity to select relevant pretrained experts, integrates them via shared dense layers, and employs replay buffers with attention mechanisms to prevent catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Lifelong deep learning aims to train neural networks sequentially across tasks while preserving prior knowledge, but faces challenges with catastrophic forgetting and efficient knowledge transfer between tasks.

Method: TAME maintains a pool of pretrained neural networks and activates the most relevant expert for each new task based on task similarity. It uses a shared dense layer to integrate features from chosen experts, a replay buffer storing representative samples/embeddings from previous tasks, and an attention mechanism to prioritize relevant stored information.

Result: Experiments on binary classification tasks derived from CIFAR-100 show TAME improves accuracy on new tasks while sustaining performance on earlier ones, effectively balancing adaptation and retention in lifelong learning.

Conclusion: TAME demonstrates effectiveness in lifelong learning by leveraging task similarity for expert selection, enabling flexible adaptation while retaining important knowledge across evolving task sequences through its replay buffer and attention mechanisms.
...
### [152] [Insight Miner: A Time Series Analysis Dataset for Cross-Domain Alignment with Natural Language](https://arxiv.org/abs/2512.11251)
...
TL;DR: Insight Miner is a multimodal model that generates comprehensive time-series descriptions using domain knowledge, trained on TS-Insights dataset created via agentic workflow.


<details>
  <summary>Details</summary>
Motivation: Time-series analysis requires deep domain expertise and is labor-intensive; there's a need for automated systems that can generate high-quality insights from time-series data without extensive human intervention.

Method: Created TS-Insights dataset (100k time-series windows from 20 forecasting datasets) using agentic workflow: statistical tools extract features from raw time series, then GPT-4 synthesizes them into coherent descriptions. Used this dataset for instruction tuning of Insight Miner model.

Result: Insight Miner outperforms state-of-the-art multimodal models (LLaVA and GPT-4) in generating time-series descriptions and insights after instruction tuning on TS-Insights dataset.

Conclusion: The approach shows promise for leveraging multimodal models in time series analysis and represents a foundational step toward enabling LLMs to interpret time series as a native input modality.
...
### [153] [A Simple Generalisation of the Implicit Dynamics of In-Context Learning](https://arxiv.org/abs/2512.11255)
*Francesco Innocenti,El Mehdi Achour*

Main category: cs.LG

TL;DR: The paper extends Dherin et al.'s theory of implicit weight updates in transformers during in-context learning to more realistic settings: all sequence positions (not just last), any transformer block (not just first), and with layer normalization.


<details>
  <summary>Details</summary>
Motivation: Previous theories of in-context learning relied on toy models and simplified data settings. While Dherin et al. showed transformers can implicitly update weights during ICL, their analysis was limited to specific conditions (last position, first block, without layer norm). This paper aims to generalize these findings to more realistic transformer architectures.

Method: The authors provide mathematical generalizations of Dherin et al.'s implicit update theory for: (1) all sequence positions beyond just the last token, (2) any transformer block beyond just the first, and (3) residual blocks including layer normalization. They empirically validate their theory on simple in-context linear regression tasks and analyze relationships between implicit updates across different tokens and blocks.

Result: The paper successfully generalizes the implicit weight update theory to more realistic transformer settings. Empirical verification on linear regression tasks confirms the theoretical extensions, and the analysis reveals relationships between implicit updates across different tokens and transformer blocks.

Conclusion: This work brings Dherin et al.'s theory of implicit weight updates in transformers closer to practical applications by extending it to realistic architectural components and broader conditions. The findings provide a foundation for validating these mechanisms in large-scale models and understanding how transformers perform in-context learning across their full architecture.
...
### [154] [Features Emerge as Discrete States: The First Application of SAEs to 3D Representations](https://arxiv.org/abs/2512.11263)
*Albert Miao,Chenliang Zhou,Jiawei Zhou,Cengiz Oztireli*

Main category: cs.LG

TL;DR: First application of Sparse Autoencoders (SAEs) to 3D domain reveals neural networks encode discrete rather than continuous features, approximating discrete state spaces with phase-like transitions.


<details>
  <summary>Details</summary>
Motivation: SAEs are powerful for decomposing neural activations but have rarely been applied outside textual domain, limiting theoretical explorations of feature decomposition in other modalities like 3D.

Method: Applied SAEs to analyze features in a state-of-the-art 3D reconstruction VAE using 53k 3D models from Objaverse dataset, examining feature decomposition in 3D domain.

Result: Found networks encode discrete features rather than continuous ones, approximating discrete state spaces with phase-like transitions; observed three unintuitive behaviors explained through state transition framework.

Conclusion: Provides framework explaining model's feature learning dynamics, reveals how models redistribute interference from superposition to prioritize feature saliency, advancing understanding of feature decomposition beyond text.
...
### [155] [SRLR: Symbolic Regression based Logic Recovery to Counter Programmable Logic Controller Attacks](https://arxiv.org/abs/2512.11298)
*Hao Zhou,Suman Sourav,Binbin Chen,Ke Yu*

Main category: cs.LG

TL;DR: SRLR is a symbolic regression-based method for recovering PLC logic from inputs/outputs to detect controller logic attacks with explainable rules, outperforming existing methods by up to 39%.


<details>
  <summary>Details</summary>
Motivation: PLC controllers in industrial control systems are vulnerable to cyber-attacks. Existing detection methods have limitations: specification-based approaches require expert knowledge or source code access, while machine learning models lack explainability for their decisions.

Method: SRLR uses symbolic regression to recover PLC logic from only inputs and outputs. It enhances deep symbolic regression with ICS-specific properties: 1) frequency domain representation for some control logic, 2) handling multiple operational modes with infrequent switches, 3) filtering outlier inputs for noisy sensor data, and 4) reducing formula complexity for effective search.

Result: SRLR consistently outperforms all existing methods across various ICS settings, achieving up to 39% higher recovery accuracy in challenging environments. It also demonstrates stability when applied to large-scale systems like distribution grids with hundreds of voltage regulators.

Conclusion: SRLR provides an effective, explainable solution for PLC logic recovery and attack detection that overcomes limitations of existing approaches by leveraging ICS-specific properties through enhanced symbolic regression techniques.
...
### [156] [QGEC : Quantum Golay Code Error Correction](https://arxiv.org/abs/2512.11307)
*Hideo Mukai,Hoshitaro Ohnishi*

Main category: cs.LG

TL;DR: Proposed Quantum Golay code Error Correction (QGEC) using Golay code with Transformer decoder, showing better decoding accuracy than toric code despite requiring fewer qubits.


<details>
  <summary>Details</summary>
Motivation: Quantum error correction is essential for fault-tolerant quantum computation, but current methods need improvement. Golay code from classical information theory offers efficient coding that could enhance QEC performance with fewer qubits.

Method: Developed QGEC method using Golay code with Transformer-based decoder. Evaluated decoder accuracy with three different generative polynomial weight sets and three noise models with varying bit-flip/phase-flip error correlations. Compared performance with toric code under discrete uniform noise distribution.

Result: Noise models with smaller error correlation gave better accuracy; polynomial weights had little effect. Golay code (23 data qubits, distance 7) achieved higher decoding accuracy than toric code (50 data qubits, distance 5).

Conclusion: Transformer-based decoding enables Golay code to potentially realize more efficient fault-tolerant quantum computation than toric code, offering better performance with fewer qubits.
...
### [157] [Benchmarking the Generality of Vision-Language-Action Models](https://arxiv.org/abs/2512.11315)
...
<details>
  <summary>Details</summary>
Motivation: Current evaluation of multimodal agents is fragmented across isolated benchmarks, making it difficult to assess whether foundation models truly generalize beyond their training distributions. There's a need for unified evaluation to measure cross-domain generality.

Method: Introduces MultiNet v1.0 benchmark covering six foundational capability regimes: visual grounding, spatial reasoning, tool use, physical commonsense, multi-agent coordination, and continuous robot control. Evaluates models (GPT-5, Pi0, Magma) across these domains to measure generalization.

Result: No model demonstrates consistent generality - all show substantial degradation on unseen domains, unfamiliar modalities, or cross-domain task shifts despite strong in-distribution performance. Failures include modality misalignment, output format instability, and catastrophic knowledge degradation under domain transfer.

Conclusion: There's a persistent gap between the aspiration of generalist intelligence and actual capabilities of current foundation models. MultiNet v1.0 provides standardized evaluation for diagnosing these gaps and guiding future generalist agent development.
...
### [158] [Condensation-Concatenation Framework for Dynamic Graph Continual Learning](https://arxiv.org/abs/2512.11317)
*Tingxu Yan,Ye Yuan*

Main category: cs.LG

TL;DR: CCC framework for continual learning on dynamic graphs addresses catastrophic forgetting by condensing historical snapshots into semantic representations and concatenating them with current representations, with refined forgetting measure for dynamic graphs.


<details>
  <summary>Details</summary>
Motivation: Dynamic graphs experience continuous structural changes that cause catastrophic forgetting in GNNs. Existing continual learning methods for dynamic graphs overlook how topological changes affect existing nodes.

Method: CCC framework: 1) Condenses historical graph snapshots into compact semantic representations preserving original label distribution and topological properties. 2) Concatenates these historical embeddings with current graph representations selectively. 3) Refines forgetting measure (FM) to quantify predictive performance degradation of existing nodes caused by structural updates.
...
Conclusion: The proposed CCC framework effectively addresses catastrophic forgetting in dynamic graphs by preserving historical information through condensation and selective concatenation, with a refined forgetting measure tailored for dynamic graph scenarios.
...
### [159] [Pace: Physics-Aware Attentive Temporal Convolutional Network for Battery Health Estimation](https://arxiv.org/abs/2512.11332)
*Sara Sameer,Wei Zhang,Kannan Dhivya Dharshini,Xin Lou,Yulin Gao,Terence Goh,Qingyu Yan*

Main category: cs.LG

TL;DR: Pace is a physics-aware attentive temporal convolutional network for battery health estimation that combines sensor data with battery physics features, achieving significant performance improvements over existing models.


<details>
  <summary>Details</summary>
Motivation: Effective battery health management is essential for battery system safety, cost-efficiency, and sustainability in modern energy systems like electric vehicles and power grid storage.

Method: Pace integrates raw sensor measurements with battery physics features from equivalent circuit models. It uses three specialized modules: dilated temporal blocks for temporal encoding, chunked attention blocks for context modeling, and a dual-head output block for fusing short- and long-term degradation patterns.

Result: Pace outperforms existing models by 6.5% average improvement and 2.0x better than two best-performing baselines. It was successfully deployed in real-time on a Raspberry Pi, demonstrating practical viability.

Conclusion: Pace establishes itself as a practical and high-performance solution for battery health analytics, combining physics awareness with efficient neural network architecture for accurate battery health estimation across various usage conditions.
...
### [160] [Spectral entropy prior-guided deep feature fusion architecture for magnetic core loss](https://arxiv.org/abs/2512.11334)
*Cong Yao,Chunye Gong,Jin Zhang*

Main category: cs.LG

TL;DR: SEPI-TFPNet is a hybrid model combining empirical models with deep learning for accurate core loss prediction in magnetic components, achieving improved accuracy and robustness over purely data-driven approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional core loss modeling methods have limited prediction accuracy, and while data-driven models show good fitting performance, they lack interpretability and cross-distribution generalization capabilities. The MagNet Challenge highlighted the need for better approaches to uncover complex loss patterns in magnetic components.

Method: Proposes SEPI-TFPNet, a hybrid model with two submodules: 1) Physical-prior submodule using spectral entropy discrimination to select optimal empirical models for different excitation waveforms, and 2) Data-driven submodule combining CNNs, multi-head attention, and BiLSTM networks to extract flux-density time-series features. Includes adaptive feature fusion for multimodal feature integration.

Result: Evaluated on MagNet dataset with various magnetic materials, compared with 21 representative models from 2023 challenge and three advanced methods from 2024-2025. The proposed method achieves improved modeling accuracy and robustness.

Conclusion: SEPI-TFPNet successfully addresses limitations of purely data-driven models by integrating empirical knowledge with deep learning, providing better interpretability, generalization, and prediction accuracy for core loss modeling in power electronics design.
...
### [161] [DAPO: Design Structure-Aware Pass Ordering in High-Level Synthesis with Graph Contrastive and Reinforcement Learning](https://arxiv.org/abs/2512.11342)
...
TL;DR: DAPO: A design structure-aware pass ordering framework for HLS that uses program semantics, contrastive learning, and reinforcement learning to discover design-specific optimization strategies, achieving 2.36× speedup over Vitis HLS.


<details>
  <summary>Details</summary>
Motivation: Existing HLS tools use fixed optimization strategies inherited from software compilers, which limits their effectiveness. Tailoring optimizations requires deep semantic understanding, accurate hardware metric estimation, and advanced search algorithms that current approaches lack.

Method: Extracts program semantics from control and data flow graphs, uses contrastive learning to generate rich embeddings, employs an analytical model for accurate hardware metric estimation, and guides a reinforcement learning agent to discover design-specific optimization strategies.

Result: The end-to-end flow delivers a 2.36× speedup over Vitis HLS on average when evaluated on classic HLS designs.

Conclusion: DAPO demonstrates that design structure-aware optimization strategies guided by program semantics and reinforcement learning can significantly outperform traditional fixed optimization approaches in HLS.
...
### [162] [Symmetry-Aware Steering of Equivariant Diffusion Policies: Benefits and Limits](https://arxiv.org/abs/2512.11345)
*Minwoo Park,Junwoo Chang,Jongeun Choi,Roberto Horowitz*

Main category: cs.LG

TL;DR: Equivariant diffusion policies (EDPs) combine diffusion models with geometric symmetries for efficient learning. The paper introduces a symmetry-aware RL framework for steering EDPs, showing it improves sample efficiency and prevents value divergence compared to standard RL approaches.


<details>
  <summary>Details</summary>
Motivation: Standard RL methods for fine-tuning equivariant diffusion policies are sample-inefficient and unstable because they ignore the geometric symmetries that EDPs are designed to exploit. There's a need for a principled symmetry-aware steering framework that can leverage these symmetries during reinforcement learning.

Method: The paper theoretically establishes that the diffusion process of an EDP is equivariant, which induces a group-invariant latent-noise MDP. Building on this theory, the authors introduce a symmetry-aware steering framework and compare standard, equivariant, and approximately equivariant RL strategies across tasks with varying symmetry degrees.

Result: Experiments show that exploiting symmetry during steering yields substantial benefits: enhanced sample efficiency, prevention of value divergence, and strong policy improvements even when EDPs are trained from extremely limited demonstrations. The paper also identifies practical boundaries of strict equivariance under symmetry breaking.

Conclusion: Symmetry-aware RL steering of equivariant diffusion policies is crucial for efficient and stable fine-tuning. The proposed framework successfully leverages geometric symmetries to overcome limitations of standard RL approaches, enabling effective policy improvement even with minimal demonstration data.
...
### [163] [CAT: Can Trust be Predicted with Context-Awareness in Dynamic Heterogeneous Networks?](https://arxiv.org/abs/2512.11352)
*Jie Wang,Zheng Yan,Jiahe Lan,Xuyan Li,Elisa Bertino*

Main category: cs.LG

TL;DR: CAT is a context-aware GNN-based trust prediction model that addresses limitations of existing approaches by handling trust dynamicity, network heterogeneity, and context-awareness through continuous-time representations, dual attention mechanisms, and meta-path-based context extraction.


<details>
  <summary>Details</summary>
Motivation: Current GNN-based trust prediction models have three key limitations: they fail to capture trust dynamicity, ignore network heterogeneity (losing rich semantics), and lack context-awareness (a basic property of trust), resulting in questionable inferences and coarse-grained predictions.

Method: CAT uses a four-layer architecture: graph construction, embedding, heterogeneous attention, and prediction layers. It handles dynamic graphs with continuous-time representations and time encoding, models heterogeneity with dual attention mechanisms (node types and nodes within types), and achieves context-awareness through meta-path-based context feature extraction and context-aware aggregation.

Result: Extensive experiments on three real-world datasets show CAT outperforms five groups of baselines in trust prediction, while demonstrating strong scalability to large-scale graphs and robustness against both trust-oriented and GNN-oriented attacks.

Conclusion: CAT is the first context-aware GNN-based trust prediction model that effectively addresses trust dynamicity, network heterogeneity, and context-awareness, providing more accurate and robust trust predictions for real-world applications.
...
### [164] [Attacking and Securing Community Detection: A Game-Theoretic Framework](https://arxiv.org/abs/2512.11359)
*Yifan Niu,Aochuan Chen,Tingyang Xu,Jia Li*

Main category: cs.LG

TL;DR: This paper extends adversarial attacks from graph classification to community detection, proposing novel attack/defense techniques and a game-theoretic framework (CD-GAME) where attacker and defender dynamically interact until reaching Nash equilibrium.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address adversarial attacks in community detection, which is more challenging than graph classification. This has practical applications in protecting personal privacy in social networks and understanding camouflage patterns in transaction networks.

Method: The paper proposes: 1) Novel attack techniques to hide targeted individuals from detection models, 2) Defense techniques to enhance robustness of community detection models, and 3) A game-theoretic framework called CD-GAME where a graph attacker and Rayleigh Quotient defender dynamically update strategies until reaching Nash equilibrium.

Result: Extensive experiments show the proposed attack and defense methods outperform existing baselines by significant margins. The CD-GAME framework reveals that at Nash equilibrium, attackers adopt more imperceptible strategies that maintain satisfactory attack effectiveness even after defense.

Conclusion: The work successfully extends adversarial graph concepts to community detection, providing effective attack/defense techniques and a valuable game-theoretic framework for understanding interactive scenarios in community detection problems.
...
<details>
  <summary>Details</summary>
Motivation: Address the long-standing trade-off between computational efficiency and physical fidelity in high-precision scientific simulation, particularly for ocean modeling.

Method: Fuses differentiable programming with deep learning: 1) A fully differentiable dynamical solver with physics knowledge as inductive bias, 2) Learnable physical parameters (e.g., diffusion coefficients) optimized via end-to-end training, 3) A deep neural network to correct subgrid-scale processes and discretization errors, 4) Both components integrated by a unified ODE solver.

Result: NeuralOGCM maintains long-term stability and physical consistency, significantly outperforming traditional numerical models in speed and pure AI baselines in accuracy.

Conclusion: The work paves a new path for building fast, stable, and physically-plausible models for scientific computing by synergistically combining differentiable physics with deep learning.
...
### [166] [Mitigating the Safety Alignment Tax with Null-Space Constrained Policy Optimization](https://arxiv.org/abs/2512.11391)
*Yifan Niu,Han Xiao,Dongyi Liu,Nuo Chen,Jia Li*

Main category: cs.LG

TL;DR: NSPO is a novel RL framework for LLM safety alignment that projects safety gradients into the null space of general tasks to prevent forgetting of core abilities while ensuring effective safety training.


<details>
  <summary>Details</summary>
Motivation: Current RL-based safety alignment methods for LLMs suffer from "alignment tax" - forgetting learned general abilities (math, coding, instruction-following) while optimizing for safety. This trade-off between safety and capability preservation needs to be addressed.

Method: Null-Space constrained Policy Optimization (NSPO) projects safety policy gradients into the null space of general tasks. This geometric projection ensures safety updates don't interfere with core capabilities while maintaining effective safety alignment direction.

Result: NSPO achieves state-of-the-art safety performance without sacrificing accuracy on general tasks (math, code, instruction-following). It's data-efficient, requiring only 40% of PKU-SafeRLHF safety data and no large mixed general tasks dataset.

Conclusion: NSPO effectively addresses the alignment tax problem by preserving LLMs' core abilities during safety alignment through null-space projection, offering a practical and theoretically-grounded solution for real-world LLM deployment.
...
### [167] [Contrastive Time Series Forecasting with Anomalies](https://arxiv.org/abs/2512.11526)
...
<details>
  <summary>Details</summary>
Motivation: Standard forecasting models fail to distinguish between anomalies with lasting effects (which should influence forecasts) and short-lived anomalies (which should be ignored), leading to either overreaction to noise or missing persistent distributional shifts.

Method: Proposes Co-TSFA framework with input-only and input-output augmentations to model different anomaly types, and introduces a latent-output alignment loss that ties representation changes to forecast changes, encouraging invariance to irrelevant perturbations while preserving sensitivity to meaningful shifts.

Result: Experiments on Traffic, Electricity benchmarks, and a real-world cash-demand dataset show Co-TSFA improves performance under anomalous conditions while maintaining accuracy on normal data.

Conclusion: Co-TSFA provides an effective regularization framework for time series forecasting that learns to appropriately respond to different types of anomalies, enhancing model robustness in real-world scenarios with anomalous events.
...
### [168] [Bhargava Cube--Inspired Quadratic Regularization for Structured Neural Embeddings](https://arxiv.org/abs/2512.11392)
*S Sairam,Prateek P Kulkarni*

Main category: cs.LG

TL;DR: Novel neural representation learning approach using Bhargava cubes from number theory to impose algebraic constraints on 3D latent spaces, achieving high accuracy on MNIST with interpretable embeddings.


<details>
  <summary>Details</summary>
Motivation: Traditional deep learning methods create unstructured latent spaces lacking interpretability and mathematical consistency. The paper aims to incorporate structured mathematical priors inspired by number theory to improve representation learning.
...
Result: Achieved 99.46% accuracy on MNIST while producing interpretable 3D embeddings that naturally cluster by digit class and satisfy learned quadratic constraints. Unlike existing manifold learning approaches, this method imposes weak algebraic priors through differentiable constraints.

Conclusion: First application of number-theoretic constructs to neural representation learning, establishing a foundation for incorporating structured mathematical priors in neural networks. The approach ensures compatibility with standard optimization while providing interpretable, mathematically consistent representations.
...
### [169] [Optimizing the Training Diet: Data Mixture Search for Robust Time Series Forecasting](https://arxiv.org/abs/2512.11546)
*Federico Pennino,Maurizio Gabbrielli*

Main category: cs.LG

TL;DR: A data-centric approach that optimizes training data composition rather than model hyperparameters, showing "less is more" by discovering optimal data mixtures that improve model performance by 19.41% on PMSM dataset.


<details>
  <summary>Details</summary>
Motivation: Raw sensor data is often imbalanced and redundant, with not all data contributing equally to model generalization. The standard assumption that more data is always better may not hold, suggesting that optimizing data composition could be more effective than just collecting more data.

Method: 1) Partition dataset into behaviorally consistent clusters using large-scale encoder and k-means clustering to identify fundamental 'ingredients'. 2) Use Optuna optimization framework to search high-dimensional space of possible data mixtures, with each trial proposing specific sampling ratios for each cluster. 3) Construct training sets based on these recipes and train/evaluate smaller target models.

Result: The data-centric search consistently discovers data mixtures that yield significantly higher performance compared to baselines trained on entire datasets. On PMSM dataset, improved MSE from 1.70 to 1.37 (19.41% improvement).

Conclusion: Optimizing training data composition can be more effective than simply using all available data, demonstrating that "less is more" in certain cases. The framework provides a systematic way to discover optimal data diets from large, unlabeled time series corpora.
...
TL;DR: Sliced ReLU attention is a new attention mechanism with O(n log n) complexity using sorting on 1D projections of key-query differences, offering computational efficiency for long contexts while maintaining theoretical expressivity.


<details>
  <summary>Details</summary>
Motivation: To develop an attention mechanism that combines computational efficiency (quasi-linear complexity) for long contexts with strong theoretical expressive power, addressing limitations of both softmax and existing ReLU-based attention methods.

Method: Instead of applying nonlinearities to pairwise dot products, the method operates on one-dimensional projections of key-query differences and leverages sorting to achieve O(n log n) complexity. It creates a differentiable, non-symmetric kernel through a sorting procedure.

Result: The model achieves quasi-linear O(n log n) complexity suitable for very long contexts. Theoretically, it preserves two key expressive properties: ability to perform nontrivial sequence-to-sequence disentangling tasks and contextual universal approximation property, both previously known only for softmax attention. Small-scale experiments demonstrate practical potential.

Conclusion: Sliced ReLU attention offers a promising alternative to existing attention mechanisms by combining computational efficiency for long contexts with strong theoretical expressive guarantees, potentially bridging the gap between practical efficiency and theoretical expressivity in attention-based models.
...
### [171] [Hyperbolic Gaussian Blurring Mean Shift: A Statistical Mode-Seeking Framework for Clustering in Curved Spaces](https://arxiv.org/abs/2512.11448)
*Arghya Pratihar,Arnab Seal,Swagatam Das,Inesh Chattopadhyay*

Main category: cs.LG

TL;DR: HypeGBMS extends Gaussian Blurring Mean Shift to hyperbolic space using hyperbolic distances and Möbius-weighted means to handle hierarchical/tree-like data structures.


<details>
  <summary>Details</summary>
Motivation: Standard GBMS works well in Euclidean space but fails with hierarchical/tree-like datasets. There's a need for clustering methods that can capture latent hierarchies while maintaining density-seeking behavior.

Method: Extends GBMS to hyperbolic space by replacing Euclidean computations with hyperbolic distances and using Möbius-weighted means to ensure geometric consistency. Provides theoretical analysis of convergence and computational complexity.

Result: HypeGBMS significantly outperforms conventional mean-shift methods on 11 real-world datasets in non-Euclidean settings, demonstrating improved clustering quality for hierarchical data.

Conclusion: Bridges classical mean-shift clustering with hyperbolic representation learning, offering a principled density-based clustering approach for curved spaces that effectively captures latent hierarchies.
...
TL;DR: The paper proposes a Plasticity-Ceiling Framework to optimize post-training using expert trajectories, establishing Sequential SFT-then-RL as superior to synchronized approaches with specific scaling guidelines for maximizing performance.


<details>
  <summary>Details</summary>
Motivation: Current post-training methods combining SFT and RL lack optimal mechanisms for utilizing expert trajectories, creating uncertainty about how to best leverage these valuable resources for maximum performance gains.

Method: Proposes the Plasticity-Ceiling Framework that decomposes performance into foundational SFT performance and RL plasticity. Uses extensive benchmarking to evaluate different training pipelines and derive scaling guidelines based on transition timing, data characteristics, and loss metrics.

Result: Establishes Sequential SFT-then-RL as the superior standard over synchronized approaches. Provides three key scaling guidelines: optimal transition timing at SFT Stable/Mild Overfitting phase, data scale as primary determinant of potential with trajectory difficulty as multiplier, and minimum SFT validation loss as indicator for selecting best expert trajectories.

Conclusion: The Plasticity-Ceiling Framework provides actionable guidelines for maximizing value from expert trajectories in post-training, with Sequential SFT-then-RL pipeline and specific scaling principles offering optimal performance outcomes.
...
### [173] [Atomic Action Slicing: Planner-Aligned Options for Generalist VLA Agents](https://arxiv.org/abs/2512.11584)
*Stefan Tabakov,Asen Popov,Dimitar Dimitrov,S. Ensiye Kiyamousavi,Vladimir Hristov,Boris Kraychev*

Main category: cs.LG

TL;DR: AAS decomposes long-horizon VLA demonstrations into typed atomic actions, improving generalization and task success rates on LIBERO benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current VLA models generalize poorly when tasks require new compositions of skills or objects, needing better decomposition methods for long-horizon tasks.

Method: Atomic Action Slicing (AAS) decomposes demonstrations into short, typed atomic actions aligned with planner needs, validated on LIBERO to create labeled dataset with action types and temporal spans.

Result: Stronger segmenter (Gemini 2.5 Pro) matches planner-defined plans robustly; fine-tuning CLIP-RT+ on atomic dataset improves task success from 94.2% to 95.3% on LIBERO-Goal and 83.8% to 88.8% on LIBERO-Long.

Conclusion: AAS effectively decomposes VLA tasks into atomic actions, improving model generalization and performance, with publicly released GATE-VLAP dataset.
...
### [174] [xGR: Efficient Generative Recommendation Serving at Scale](https://arxiv.org/abs/2512.11529)
...
TL;DR: xGR is a serving system for generative recommendation that optimizes LLM-based recommendation workloads by addressing computational inefficiencies in beam search and KV cache management, achieving 3.49x throughput improvement under strict latency constraints.


<details>
  <summary>Details</summary>
Motivation: Generative recommendation (GR) using LLMs has unique workload characteristics: long prompts with short fixed-length outputs, high computational cost in decode phases due to large beam width, and time-consuming sorting overhead from vast item spaces. Current systems are inefficient for these GR-specific requirements.

Method: xGR proposes three key optimizations: 1) Unified prefill/decode processing with staged computation and separated KV cache, 2) Early sorting termination and mask-based item filtering with data structure reuse, and 3) Pipeline reconstruction to exploit multilevel overlap and multi-stream parallelism.
...
Conclusion: xGR effectively addresses the unique computational challenges of generative recommendation systems, providing a specialized serving system that meets low-latency requirements in high-concurrency scenarios through targeted optimizations for GR workloads.
...
<details>
  <summary>Details</summary>
Motivation: Need more efficient and accurate methods for solving parametric integrals across various problem classes, as traditional machine learning approaches may not fully leverage available derivative information.
...
Conclusion: Incorporating derivative information via differential learning provides significant advantages for parametric integral problems, making it a superior approach compared to standard machine learning methods.
...
TL;DR: Automated MLOps pipeline for neural network classifier retraining that detects data distribution drift using statistical techniques and triggers updates only when needed, improving efficiency and model performance.


<details>
  <summary>Details</summary>
Motivation: ML models degrade when data distributions change over time (distribution drift), and current MLOps processes are often manual, requiring human intervention for retraining and redeployment, which is inefficient and reactive.

Method: Developed an automated MLOps pipeline that uses multi-criteria statistical techniques to detect significant data distribution shifts, then automatically triggers neural network classifier retraining and redeployment only when necessary.

Result: Experiments on benchmark anomaly detection datasets show significant improvements in model accuracy and robustness compared to traditional retraining strategies, while optimizing computational resources.

Conclusion: Provides a foundation for deploying more reliable and adaptive ML systems in dynamic real-world settings where data distribution changes are common, enabling automated, efficient model maintenance.
...
TL;DR: ENMKL introduces an elastic-net regularized multiple kernel learning approach with analytical kernel weight updates, outperforming l1-MKL and providing sparser, more interpretable models for neuroimaging data.


<details>
  <summary>Details</summary>
Motivation: Existing elastic-net MKL methods use complex two-stage procedures, while neuroimaging applications need interpretable models that handle correlated kernels effectively.

Method: Proposed ENMKL formulation with analytical kernel weight updates, implemented for SVM and KRR in PRoNTo toolbox, using elastic-net regularization combining l1- and l2-norms.

Result: ENMKL matches or outperforms l1-MKL in all tasks, only underperforms standard SVM in one scenario, and produces sparser, more interpretable models by selectively weighting correlated kernels.

Conclusion: ENMKL provides an effective alternative to existing MKL methods with simpler implementation, better performance, and improved interpretability for neuroimaging applications.
...
TL;DR: Graph View Transformation (GVT) enables fully inductive graph learning across datasets by using a novel view space representation that maintains equivariance to node/feature permutations.


<details>
  <summary>Details</summary>
Motivation: Current graph models struggle with cross-dataset generalization because feature spaces vary widely across different graph datasets, making it difficult to apply pretrained models to unseen datasets without retraining.

Method: Introduces the view space as a unified representation for arbitrary graphs, and proposes Graph View Transformation (GVT) - a node- and feature-permutation-equivariant mapping. Uses Recurrent GVT as a fully inductive model for node representation learning.

Result: Recurrent GVT pretrained on OGBN-Arxiv outperforms prior fully inductive model GraphAny by +8.93% and surpasses 12 individually tuned GNNs by at least +3.30% across 27 node-classification benchmarks.

Conclusion: The view space provides a principled and effective foundation for fully inductive node representation learning, enabling cross-dataset generalization without retraining.
...
TL;DR: Brain-Semantoks is a self-supervised fMRI foundation model that learns abstract brain dynamics representations using semantic tokenization and self-distillation, achieving strong downstream performance with minimal fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current fMRI foundation models focus on low-level regional information through mask-and-reconstruct objectives, making them noise-sensitive and requiring extensive fine-tuning for downstream tasks. There's a need for models that learn more abstract, robust representations of brain dynamics.

Method: Brain-Semantoks uses two core innovations: 1) a semantic tokenizer that aggregates noisy regional signals into robust tokens representing functional networks, and 2) a self-distillation objective that enforces representational stability across time, stabilized through a novel training curriculum.

Result: The learned representations enable strong performance on various downstream tasks using only linear probes (minimal fine-tuning). Scaling analyses show that more unlabeled data reliably improves out-of-distribution performance without domain adaptation.

Conclusion: Brain-Semantoks provides a more effective self-supervised framework for fMRI foundation models by learning abstract, robust representations of brain dynamics that transfer well to downstream tasks with minimal adaptation.
...
TL;DR: GD for neural networks reduces to generalized perceptron algorithms, revealing implicit acceleration: nonlinear models achieve $\tilde{O}(\sqrt{d})$ iteration complexity vs $Ω(d)$ for linear models.


<details>
  <summary>Details</summary>
Motivation: Understanding optimization dynamics of gradient descent in neural network training is challenging, especially regarding convergence rates, iterate trajectories, function value oscillations, and implicit acceleration phenomena.

Method: Analyze nonlinear models with logistic loss, showing GD steps reduce to generalized perceptron algorithms. Use classical linear algebra tools to analyze simplified algorithmic steps on minimalistic two-layer model examples.

Result: Nonlinearity in two-layer models provably yields faster iteration complexity $\tilde{O}(\sqrt{d})$ compared to $Ω(d)$ for linear models, explaining implicit acceleration. Theoretical results supported by extensive numerical experiments.

Conclusion: The reduction of GD to generalized perceptron algorithms provides a new perspective on neural network optimization dynamics and helps explain implicit acceleration, advancing research in this area.
...
TL;DR: Proposes fuzzy greedy tree algorithm for interpretable fuzzy rule mining that combines computational efficiency of greedy algorithms with interpretability of fuzzy logic, achieving competitive accuracy with lower computational cost than evolutionary approaches.


<details>
  <summary>Details</summary>
Motivation: Existing fuzzy rule-mining algorithms don't guarantee both sensible linguistic partitions and small rule-base sizes needed for interpretability. Evolutionary approaches are computationally expensive, while neural methods like ANFIS lose linguistic interpretability.
...
Result: Achieves interpretable linguistic partitions, substantially improves running time compared to evolutionary approaches, maintains competitive predictive performance, and produces more interpretable rule bases with constrained complexity.

Conclusion: The proposed fuzzy greedy tree approach provides an effective balance between interpretability, computational efficiency, and predictive performance for fuzzy rule-based systems in interpretable decision-making.
...
TL;DR: The paper proposes using large in-context tabular models (LTMs) as a bridge between Stream Learning (SL) and Continual Learning (CL), creating Streaming Continual Learning (SCL) with two core principles: distribution matching for plasticity-stability balance and distribution compression for memory control.


<details>
  <summary>Details</summary>
Motivation: Existing research communities address streaming adaptation and long-term retention in isolation. SL focuses on rapid adaptation to high-frequency streams but neglects forgetting, while CL focuses on mitigating catastrophic forgetting but lacks real-time constraints. There's a need to bridge these paradigms for practical streaming continual learning scenarios.

Method: Proposes using large in-context tabular models (LTMs) as a natural bridge for SCL. Unbounded streams are summarized into compact sketches consumed by LTMs. Structures SCL around two core principles: (1) distribution matching to balance plasticity and stability, and (2) distribution compression to control memory size through diversification and retrieval mechanisms.

Result: The paper presents a conceptual framework that shows how SL and CL communities implicitly use divide-to-conquer strategies to manage plasticity-stability tension while imposing minimal complexity constraints. It demonstrates how LTMs can naturally bridge the two fields by recovering SL's stream compression motivation while aligning with CL's experience-replay requirements.

Conclusion: Large in-context tabular models provide a promising bridge for Streaming Continual Learning by enabling on-the-fly stream summarization into compact sketches. The proposed framework with distribution matching and distribution compression principles offers a structured approach to combine the strengths of both Stream Learning and Continual Learning paradigms.
...
### [183] [High-Dimensional Surrogate Modeling for Closed-Loop Learning of Neural-Network-Parameterized Model Predictive Control](https://arxiv.org/abs/2512.11705)
*Sebastian Hirt,Valentinus Suwanto,Hendrik Alsmeier,Maik Pfefferkorn,Rolf Findeisen*

Main category: cs.LG

TL;DR: Bayesian neural networks outperform Gaussian processes as surrogate models for Bayesian optimization when tuning high-dimensional controller parameters, enabling optimization of hundreds to thousands of parameters.


<details>
  <summary>Details</summary>
Motivation: Bayesian optimization struggles with dense high-dimensional controller parameterizations (like in model predictive control) because standard surrogate models (Gaussian processes) fail to capture the structure of such high-dimensional spaces.

Method: Proposes using Bayesian neural networks as surrogate models instead of Gaussian processes. Compares three approaches: Gaussian processes with Matern kernels, finite-width Bayesian neural networks, and infinite-width Bayesian neural networks on a cart-pole control task.

Result: Bayesian neural networks achieve faster and more reliable convergence of closed-loop cost and enable successful optimization of parameterizations with hundreds of dimensions. Infinite-width Bayesian neural networks maintain performance with over 1000 parameters, while Gaussian processes rapidly lose effectiveness.
...
### [184] [SpectralKrum: A Spectral-Geometric Defense Against Byzantine Attacks in Federated Learning](https://arxiv.org/abs/2512.11760)
*Aditya Tripathi,Karan Sharma,Rahul Mishra,Tapas Kumar Maiti*

Main category: cs.LG

TL;DR: SpectralKrum: A federated learning defense combining spectral subspace estimation with Krum selection to filter Byzantine attacks, effective against directional/subspace-aware attacks but limited against spectrally indistinguishable attacks.


<details>
  <summary>Details</summary>
Motivation: Current robust aggregation methods in federated learning (like Krum, Bulyan) have theoretical guarantees but degrade significantly when client data is heterogeneous (non-IID) and adversaries can observe/approximate the defense mechanism.

Method: SpectralKrum fuses spectral subspace estimation with geometric neighbor-based selection: 1) learns low-dimensional manifold from historical aggregates, 2) projects incoming updates into this subspace, 3) applies Krum selection in compressed coordinates, 4) filters candidates whose orthogonal residual energy exceeds data-driven threshold. Requires no auxiliary data, operates only on model updates.

Result: Evaluated against 8 baselines across 7 attack scenarios on CIFAR-10 with non-IID partitions. Competitive against directional and subspace-aware attacks (adaptive-steer, buffer-drift), but offers limited advantage under label-flip and min-max attacks where malicious updates remain spectrally indistinguishable from benign ones.

Conclusion: SpectralKrum provides effective defense against certain Byzantine attacks in heterogeneous FL settings by leveraging spectral subspace estimation, but has limitations against attacks that remain spectrally similar to benign updates.
...
### [185] [The Adaptive Vekua Cascade: A Differentiable Spectral-Analytic Solver for Physics-Informed Representation](https://arxiv.org/abs/2512.11776)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: AVC is a hybrid neural architecture that combines deep learning with classical approximation theory to overcome spectral bias and curse of dimensionality in coordinate-based networks, achieving state-of-the-art accuracy with dramatically fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Coordinate-based neural networks suffer from two fundamental limitations: spectral bias (difficulty learning high-frequency dynamics) and curse of dimensionality (parameter explosion in discrete feature grids), which hinder their effectiveness for representing complex physical fields.

Method: AVC decouples manifold learning from function approximation using a deep network to learn a diffeomorphic warping of the physical domain, projecting dynamics onto a latent manifold where solutions are represented by generalized analytic functions. It replaces gradient-descent output layers with a differentiable linear solver for optimal spectral coefficient resolution during forward pass.

Result: AVC achieves state-of-the-art accuracy on five physics benchmarks (Helmholtz wave propagation, medical reconstruction, 3D Navier-Stokes turbulence) while reducing parameters by orders of magnitude (840 vs 4.2M for 3D grids) and converging 2-3x faster than implicit neural representations.

Conclusion: AVC establishes a new paradigm for memory-efficient, spectrally accurate scientific machine learning by bridging deep learning with classical approximation theory, overcoming fundamental pathologies of coordinate-based neural networks.
...
### [186] [Softmax as Linear Attention in the Large-Prompt Regime: a Measure-based Perspective](https://arxiv.org/abs/2512.11784)
*Etienne Boursier,Claire Boyer*

Main category: cs.LG

TL;DR: Softmax attention converges to linear attention in infinite-prompt limit, enabling transfer of linear attention analysis to softmax attention for long prompts.


<details>
  <summary>Details</summary>
Motivation: Softmax attention's nonlinear structure makes theoretical analysis challenging, but understanding its behavior is crucial for transformer analysis.

Method: Develop measure-based framework for softmax attention; show softmax converges to linear operator in infinite-prompt limit; establish concentration bounds for outputs/gradients; analyze training dynamics in in-context learning settings.

Result: Proved softmax attention approaches linear attention as prompt length increases; established non-asymptotic concentration bounds; showed stability along training trajectory; enabled transfer of linear attention analysis to softmax attention for long prompts.

Conclusion: Large-prompt softmax attention inherits analytical structure of linear attention, providing principled toolkit for studying training dynamics and statistical behavior in large prompt regimes.
...
### [187] [A General Algorithm for Detecting Higher-Order Interactions via Random Sequential Additions](https://arxiv.org/abs/2512.11793)
*Ahmad Shamail,Claire McWhite*

Main category: cs.LG

TL;DR: A geometric method using random sequential addition of elements reveals interaction patterns through L-shaped distributions, quantified by an L-score from -1 (synergy) to +1 (redundancy).


<details>
  <summary>Details</summary>
Motivation: Many systems have complex component interactions (amplification, redundancy, independence) that need systematic discovery and quantification methods.

Method: Add elements in random sequential orders, plot contributions over trials to observe L-shaped patterns. Formalize with L-score (-1 to +1) and analyze pairwise contributions as 2D point clouds.

Result: L-shaped patterns distinguish interaction types: redundancy shows only first-added contributes, synergy shows only together contribution, independence shows order-invariant distributions. Higher-order interactions emerge from cross-pair relationships.

Conclusion: The geometric approach provides a unified, metric-agnostic method for discovering and quantifying interaction structure in any domain with incremental performance evaluation.
