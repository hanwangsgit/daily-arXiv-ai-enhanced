...
TL;DR: This paper uses U-Net and DeepLabV3+ neural networks to segment foreground and background in images from a CAT793F off-highway truck simulator, achieving ~90% accuracy to facilitate mixed reality integration.


<details>
  <summary>Details</summary>
Motivation: To improve user immersion in mixed reality environments by accurately segmenting real-world images (foreground/background) captured from a first-person view simulator, enabling better fusion of virtual and real elements.

Method: Image segmentation using two convolutional neural network architectures: U-Net and DeepLabV3+, applied to images captured from the CAT793F off-highway truck simulator.

Result: Both models achieved approximately 90% accuracy, and the best-performing model was identified.

Conclusion: The proposed segmentation approach effectively facilitates the union of virtual and real images for mixed reality applications in simulator environments.
...
TL;DR: GOPAgen integrates video codec into video understanding using a motion agent trained on Groups of Pictures, a GOP tree reasoning algorithm, structural memory with coarse-to-fine zoom-in, and a motion vector database, achieving superior VQA performance on benchmarks like MotionBench and Egoschema.
...
Method: (1) A motion agent trained on GOPs from video codec; (2) GOP tree reasoning algorithm for local motion understanding; (3) Structural memory mechanism with coarse-to-fine zoom-in; (4) Motion vector database for efficient retrieval.

Result: Superior VQA performance on MotionBench, Egoschema, and various video understanding benchmarks.

Conclusion: GOPAgen demonstrates that integrating video codec with structural memory and motion-aware reasoning significantly improves long video understanding.
...
TL;DR: AE-YOLO integrates lightweight bottleneck autoencoders into FPN-PAN neck with CBAM attention and variance-maximizing regularization to improve defect detection on UAV transmission-line insulator images, achieving 95.10% mAP@0.5, surpassing YOLO baselines by 5.0 points.


<details>
  <summary>Details</summary>
Motivation: Automated defect detection in high-voltage transmission-line insulators is challenged by class imbalance, large scale variation, and small defect instances in UAV imagery. Existing YOLO-based methods struggle to preserve anomaly-sensitive information during multi-scale feature fusion.

Method: AE-YOLO integrates lightweight bottleneck autoencoders into FPN-PAN neck for anomaly-preserving feature fusion, uses CBAM in backbone for enhanced discrimination, employs variance-maximizing autoencoder regularization for diverse representations, trains with focal + CIoU + autoencoder losses, and applies WBF ensemble with autoencoder-guided confidence boosting at inference.

Result: On the Insulator-Defect Detection dataset, AE-YOLO with EfficientNetV2 backbone achieves 95.10% mAP@0.5, 96.40% precision, 93.80% recall, surpassing strongest YOLO baseline by 5.0 points in mAP@0.5 and 6.7 points in recall.

Conclusion: AE-YOLO is an effective, adaptable, practical, and scalable solution for UAV-based transmission-line inspection and defect monitoring.
...
TL;DR: WorldBench, a visually diverse multimodal benchmark, reveals that even the strongest MLLMs achieve only 64.0% accuracy, highlighting significant weaknesses in visual understanding across diverse domains.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal benchmarks expand task types but fail to capture the visual diversity needed for real-world open-ended visual inputs, leaving models untested across the broad visual spectrum.

Method: Built a taxonomy of visual concepts across domains (e.g., living things), curated diverse images from search engines and datasets, and manually designed challenging questions through structured trial-and-error that frontier MLLMs fail to answer.

Result: WorldBench achieves higher visual diversity than existing diverse benchmarks. Evaluating 15 MLLMs shows the strongest model reaches only 64.0% accuracy, with some models performing marginally above chance-level.

Conclusion: Visual diversity is critically important for building robust multimodal benchmarks, as current MLLMs exhibit significant weaknesses in visual understanding when tested across diverse real-world scenarios.
...
TL;DR: DTG-FF achieves state-of-the-art FF results across nine benchmarks (including first 224x224 ImageNet-100 baseline at 49.4%), but reveals fundamental scaling limitations: the gap to BP widens with class count and real-image complexity, and memory/throughput advantages of FF disappear under fair comparison.


<details>
  <summary>Details</summary>
Motivation: To rigorously evaluate whether layer-local Forward-Forward learning can scale to realistic image classification tasks, since prior work only showed promise on small 32x32 benchmarks.

Method: Proposes DTG-FF (dynamic temperature goodness, decoupled normalization, multi-layer fusion) as an improved FF framework, and systematically compares it against backpropagation with deep supervision (BP-DeepSup) under identical architectures and recipes across nine real-image benchmarks.

Result: (1) On real data, BP beats DTG-FF by 2.40/5.93 pp on CIFAR-10/100, and the gap widens with class count; at 224x224 (ImageNet-100), DTG-FF achieves only 49.4% vs typical BP >75%. (2) On synthetic tasks, FF outperforms BP with increasing K, but on real data the trend reverses. (3) Memory and throughput of DTG-FF (7.90 GB / 138 imgs/s) are worse than BP (4.18 GB / 157 imgs/s) under fair comparison.

Conclusion: Layer-local training (FF) still substantially lags behind backpropagation on real-world vision tasks at scale, and its perceived advantages (memory efficiency, synthetic-task transferability) do not hold when rigorously benchmarked.
...
TL;DR: DIRECT is a diffusion-based framework for pose-controllable object insertion that decomposes insertion conditions into appearance, geometry, and context guidance injected through separate pathways, outperforming prior methods in controllability and visual quality.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based object insertion methods lack explicit 3D pose control, treating insertion as simple 2D inpainting which limits practical applicability.

Method: DIRECT decomposes insertion into three complementary components: appearance guidance (visual details from reference), geometry guidance (3D proxy pose from user), and context guidance (target background), injected through separate pathways to avoid feature entanglement.

Result: DIRECT outperforms previous methods in both geometric controllability and visual quality.

Conclusion: Decomposing insertion conditions into separate pathways with an automated data pipeline enables high-fidelity pose-controllable object insertion.
...
TL;DR: A physics-free deep learning pipeline predicts 3D hip and knee contact forces from uncalibrated monocular video, matching musculoskeletal simulation accuracy without markers, force plates, or subject-specific models.


<details>
  <summary>Details</summary>
Motivation: Joint contact forces are critical for understanding osteoarthritis, implant longevity, and rehabilitation, but can only be measured invasively in a few patients with instrumented implants, limiting clinical applicability.

Method: Parametric body meshes are recovered per video frame, encoded as kinematic features, and decoded into forces via a transformer with adaptive modulation by body shape, joint, side, activity text, and self-supervised video tokens (V-JEPA 2), unifying hip and knee in a single model.

Result: The pipeline matches subject-specific musculoskeletal simulation accuracy ($0.32 \pm 0.08$ BW RMSE for hip; $0.23 \pm 0.03$ BW for knee) on 26 patients and 25 activities, resolves peak force changes relevant to gait retraining and osteoarthritis, and rivals/outperforms prior methods zero-shot on an independent cohort.
...
TL;DR: ViSAE is a mechanistic interpretability toolbox that uses sparse autoencoders, a large concept vocabulary (16K), and automated circuit discovery algorithms to interpret, audit, and steer Vision Transformer behavior, achieving 48.2% worst-group accuracy improvement on WaterBirds.


<details>
  <summary>Details</summary>
Motivation: ViT predictions can rely on spurious cues, and existing SAE-based interpretation methods lack concept coverage control and rely on subjective, non-scalable feature interpretation.

Method: ViSAE contains: (1) a probing suite with 64K images and 16K visually grounded concept vocabulary, (2) top-down concept reading and bottom-up circuit tracing algorithms for automated recovery of concept circuits, and (3) applications for auditing and steering ViT behavior via concept editing.

Result: Improves concept coverage efficiency by 20x over ImageNet, interpretation accuracy by 28.7% over existing concept sets, and worst-group accuracy on WaterBirds by 48.2% (23.8% better than existing methods).

Conclusion: ViSAE provides a scalable, automated framework for mechanistic interpretability of ViTs through concept circuits, enabling both auditing and steering of model behavior.
...
TL;DR: COF is an architecture-adaptive framework that fuses five complementary uncertainty sources for deepfake detection, achieving practical performance without model modifications, but revealing catastrophic generalization failure under cross-domain shifts.


<details>
  <summary>Details</summary>
Motivation: Existing UQ methods rely on single uncertainty sources and ignore that optimal uncertainty composition varies across architectures, limiting reliable deployment in forensic settings.

Method: Proposes Correlation-Optimized Fusion (COF), which fuses five uncertainty sources (epistemic, aleatoric, calibration, conformal, distributional) by maximizing Pearson correlation between fused scores and prediction errors via constrained optimization on the probability simplex.

Result: COF requires only 42s of optimization vs. 20-45h for Deep Ensembles. Under distribution shift, COF outperforms RF in 9/11 architectures (up to 7.3x higher correlation). However, cross-dataset evaluation shows catastrophic failure: correlations of 0.41-0.47 collapse to near-zero (90.7% degradation), with 7/11 architectures showing uncertainty inversion.

Conclusion: COF is practical for controlled-distribution deployment, but domain-adaptive UQ remains the central open challenge for real-world forensic deepfake detection.
...
TL;DR: JA-SIREN proposes a deterministic initialization scheme for sinusoidal neural networks using DST and Jacobi-Anger expansion, achieving 0 variance and 67.18 dB PSNR on Kodak (21.30 dB improvement over baselines).


<details>
  <summary>Details</summary>
Motivation: Existing INR approaches suffer from stochastic initialization causing inconsistent performance (up to 2.5 dB / 78% variation in image regression), which is problematic for reproducibility-critical applications like scientific computing and simulation.

Method: The method computes the Discrete Sine Transform (DST) of the target signal and uses the Jacobi-Anger expansion to derive closed-form weights for a two-layer sinusoidal MLP, analytically matching the network's initial spectral response to the target signal without random seeds or hyperparameter tuning.
...
TL;DR: SGBR-HC is a two-stage hyperspectral band selection method that uses supervised spectral ranking to initialize trainable sparse gates, allowing the number of selected bands to be learned automatically, achieving high accuracy with ~20 bands under spatially-disjoint evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing differentiable band selection methods are sensitive to initialization and require pre-specified band counts, limiting flexibility and robustness.

Method: Stage-1 ranks candidate bands by class discriminability and spectral diversity to seed gate logits; Stage-2 trains sparse gates jointly with a spatial classifier using hard-concrete initialization.

Result: Achieves highest mean OA and Cohen's kappa with ~20 bands on Pavia University and Houston 2013; skipping Stage-1 degrades OA by 8.84-22.15 pp; random pixel splits inflate OA by 30.56 pp.

Conclusion: The ranking prior is critical for performance, and spatially-disjoint evaluation is essential to avoid overestimation from spatial leakage.
...
TL;DR: EvoGS introduces a continuous-layering representation (Evolution Tree) for streaming 3D Gaussian Splatting, replacing discrete layering to reduce redundancy, correct ancestral errors, and achieve superior compression and smooth quality transitions.


<details>
  <summary>Details</summary>
Motivation: Existing progressive 3D Gaussian Splatting methods use discrete layering with independent splat sets, causing error accumulation, splat redundancy, and uncontrolled quality transitions.

Method: Proposes EvoGS with an Evolution Tree continuous-layering representation, using wavelet-inspired parent-child refinement where child nodes structurally correct errors from ancestors.

Result: Reduces splat redundancy from over 65% to under 25%; reduces transmission payload by up to 2.4× and GPU VRAM footprint by up to 5.5× vs. state-of-the-art baselines.

Conclusion: EvoGS achieves inherently sparse, compressible inter-layer signals with smooth quality transitions optimal for real-time adaptive streaming.
...
TL;DR: RigPAPR animates static neural point reconstructions via direct LBS without joint-boundary artifacts, outperforming baselines in novel-view synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing methods (Gaussian splats with LBS or mesh proxies) produce joint-boundary artifacts (gaps/spikes) under articulation because each splat has a rigid canonical shape that cannot bend.

Method: RigPAPR uses Proximity Attention Point Rendering (PAPR), where per-primitive shape is eliminated and pixels are recomposed from deformed primitive positions, enabling natural surface reformation under LBS without mesh proxies or pose-dependent corrections.

Result: On synthetic subjects, RigPAPR matches the strongest baseline at supervised views and exceeds mesh/Gaussian baselines at novel views by 3+dB PSNR, with cleaner joint-boundary renderings on both synthetic and real subjects.

Conclusion: PAPR-based representation inherently avoids articulation artifacts, enabling high-quality auto-rigging and animation from a single monocular video without category-specific templates or corrections.
...
TL;DR: RPC-GS is the first Gaussian Splatting framework for satellite imagery that directly uses RPC models without geometric approximations, achieving significantly lower reconstruction errors on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Prior satellite Gaussian Splatting methods replace the standard RPC model with perspective or affine camera approximations, introducing geometric errors during 3D reconstruction. The authors aim to avoid these approximations by natively incorporating RPC models.

Method: RPC-GS projects Gaussian means and covariances directly through the RPC model using carefully selected geo-coordinate transformations. It derives a numerically robust Jacobian-based covariance projection for partially nonlinear transformations and integrates a metric ray-based depth formulation since RPCs lack explicit depth.

Result: On DFC2019, RPC-GS improves mean altitude error over perspective and affine approximations by 29.6% and 63.8%. On IARPA2016, improvements are 9.9% and 37.9% respectively, consistently achieving the lowest reconstruction error.

Conclusion: Native RPC rendering outperforms perspective and affine approximations in satellite Gaussian Splatting, and the authors release code to support future research in this domain.
...
TL;DR: Winning solution for S23DR 2026 challenge: uses flow-matching DiT with Perceiver-style conditioning, two-pass refinement (coarse global + hull-cropped), and consensus sampling to reconstruct 3D wireframes from sparse inputs, achieving top private leaderboard score (HSS=0.654).


<details>
  <summary>Details</summary>
Motivation: To achieve high-quality structured 3D wireframe reconstruction from sparse Structure-from-Motion (SfM), fitted depth, and semantic segmentations, as demonstrated by winning the S23DR 2026 challenge.

Method: Treats vertices as a conditional set, denoising 64 vertex tokens via a flow-matching Diffusion Transformer (DiT) conditioned on Perceiver-style scene tokens. Uses a global pass for coarse structure, then a hull-cropped second pass for refinement, plus a multi-sample consensus step to stabilize the stochastic sampler.

Result: Ranked first on the private leaderboard with HSS = 0.654.

Conclusion: The proposed method effectively reconstructs structured 3D wireframes from sparse inputs, and the multi-sample consensus step is key for keeping the stochastic sampler well-behaved.
...
TL;DR: The paper introduces MMBU, the largest biomedical vision-language benchmark to date, covering 35 submodalities across ungrounded classification, grounded classification, and object detection tasks. Evaluation of 17 VLMs reveals that high accuracy on existing benchmarks can mask deficiencies in visual perception and domain generalization.


<details>
  <summary>Details</summary>
Motivation: Existing biomedical VL benchmarks are limited in scope, lacking fine-grained visual perception evaluation across diverse modalities, scales, and contexts, which hinders the realization of VLM potential in biomedical imaging.

Method: The authors built MMBU, the largest biomedical VL benchmark covering 35 submodalities with structured metadata, including open and closed versions of three task types: ungrounded classification, grounded classification, and object detection.

Result: Evaluated 15 open-weight and 2 frontier VLMs. Found that medical adaptation improves some models, but high accuracy on established benchmarks often masks deficiencies in visual perception and domain generalization.

Conclusion: Current VLM performance on standard biomedical benchmarks is misleading; MMBU reveals significant gaps in fine-grained visual perception and generalization across modalities, suggesting the need for more comprehensive evaluation frameworks.
...
TL;DR: The paper introduces USU-Corn-WeedDB, a publicly available UAV RGB image dataset for multi-class weed detection in forage corn, containing 800 annotated and 8,000 unlabeled image patches. It covers three weed species with intentional class imbalance (redroot pigweed dominant at 53.86%). Tests of 28 detection models (YOLOv8-v11, RT-DETR) under identical conditions achieved mAP@0.5 scores of 0.773–0.840, with lightweight models showing promise for edge-deployed UAV systems.


<details>
  <summary>Details</summary>
Motivation: Weed pressure causes up to 31.5% yield loss in forage corn, but SSWM systems using UAV imagery and deep learning are limited by the scarcity of field-representative training datasets. The authors aim to fill this gap by providing a publicly available, real-world dataset.

Method: UAV RGB imagery was collected at ~10m altitude (0.48 cm/pixel GSD) on 27 June 2025. 366 full-resolution images were tiled into 8,800 patches (640×640). 800 patches were manually annotated for three weed species (10,539 bounding boxes), and 8,000 tiles were kept as an unlabeled pool. 28 object detection models from six architecture families (YOLOv8-YOLO26, RT-DETR) were trained without hyperparameter tuning to benchmark the dataset.

Result: Redroot pigweed accounts for 53.86% of annotated instances. Test set mAP@0.5 ranged from 0.773 to 0.840 across 28 models. Lightweight models achieved competitive performance suitable for edge-deployed UAV systems.

Conclusion: USU-Corn-WeedDB is a valuable public resource for advancing SSWM in forage corn, supporting both supervised and semi-supervised learning. Its class imbalance reflects real field conditions. The dataset is available at https://doi.org/10.5281/zenodo.20044178.
...
TL;DR: VLMs fail to reproduce human-like graded perception of surface slant from texture, instead producing "anchored" predictions at discrete angles like 0°, ±25°, ±45°, with little sensitivity to visual parameters. Supervised fine-tuning helps only partially.


<details>
  <summary>Details</summary>
Motivation: Prior work showed unsupervised CNNs reproduce human-like biases in slant perception from texture, but supervised CNNs do not. This paper investigates whether Vision-Language Models (VLMs) exhibit similar human-like competences.

Method: Tested multiple VLM families and scales using zero-shot and in-context prompting, plus supervised fine-tuning, on psychophysical slant-from-texture tasks.

Result: VLMs produce anchored slant predictions (e.g., 0°, ±25°, ±45°) with little dependence on field of view, optical slant, or curvature. Fine-tuning only partially resolves this anchoring.

Conclusion: VLMs' failure is not necessarily a lack of geometric encoding, but a problem at the representation-to-language interface—they cannot express graded geometric information properly.
...
TL;DR: MedSIGHT is a unified framework that combines medical vision-language understanding with pixel-level segmentation by introducing a Region Perceiver module and a medical region codebook, enabling structured spatial grounding. It achieves SOTA on both comprehension and segmentation tasks across diverse medical imaging modalities.


<details>
  <summary>Details</summary>
Motivation: Existing Med-LVLMs cannot unify vision-language comprehension and medical image segmentation, which is essential for clinically reasoning that connects visual findings with semantic interpretation.

Method: MedSIGHT introduces (1) a Region Perceiver module for region-centric tokens encoding spatial info into LLM representation space, (2) a medical region codebook in LLM vocabulary for discrete region codes as symbolic representations, and (3) a progressive training strategy to stably align modules.
...
Conclusion: The proposed unified framework effectively bridges visual comprehension and segmentation in Med-LVLMs, enabling grounded visual understanding critical for clinical reasoning.
...
TL;DR: The paper identifies that text-to-image models produce homogeneous samples because the DC (zero-frequency) component converges too quickly across seeds. The authors propose DAVE, a training-free method that attenuates this component early in generation, improving diversity with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image models generate overly similar samples under the same prompt; current diversity methods are expensive. The authors aim to understand and fix this homogeneity efficiently.

Method: Analyze intermediate Transformer features, observe early DC component convergence. Propose DAVE: a training-free, representation-level intervention that selectively attenuates the DC component in early generation stages.

Result: DAVE improves prompt-consistent diversity while maintaining competitive image quality, with negligible computational overhead.

Conclusion: Early DC component lock-in is a root cause of low diversity in text-to-image generation. DAVE offers a simple, effective, and efficient solution.
...
TL;DR: VideoSEG-O3 is a multi-turn reinforcement learning framework for Reasoning Video Object Segmentation that mimics human coarse-to-fine cognition through iterative temporal-spatial chain-of-thought, SEG-aware logit calibration, and decoupled thinking traces.


<details>
  <summary>Details</summary>
Motivation: Existing RVOS methods are limited to reasoning over fixed initial inputs and cannot actively acquire additional visual evidence, making them insufficient for complex references in long or intricate videos.

Method: (1) Multi-turn temporal-spatial chain-of-thought for iterative coarse-to-fine reasoning; (2) SEG-aware logit calibration integrating pixel-wise segmentation feedback into token-level logits; (3) Decoupled thinking trace that hierarchically decomposes reasoning into temporal, spatial, and linguistic dimensions; (4) VTS-CoT cold-start dataset with comprehensive reasoning trajectories.

Result: Not explicitly stated in the abstract; code/models to be released.

Conclusion: VideoSEG-O3 is the first multi-turn RL framework for RVOS that enables active visual evidence acquisition and structured reasoning decomposition.
...
TL;DR: AdaGRPO enhances GRPO for text-to-image flow models by introducing capability-aware prompt selection (Online Curriculum Filtering) and more accurate advantage estimation (Cross-Level Advantage Fusion), improving performance and training stability.


<details>
  <summary>Details</summary>
Motivation: Existing GRPO for T2I flow models suffers from two blind spots: (1) random prompt sampling ignoring data selection impact on RL efficacy, and (2) intra-group-only advantage estimation lacking global perspective for accurate policy evaluation.

Method: AdaGRPO introduces: (i) Online Curriculum Filtering Strategy — dynamically tracks model proficiency and adaptively selects prompts matching the current learning boundary; (ii) Cross-Level Advantage Fusion — integrates fine-grained intra-group advantages with macro-level global advantages for comprehensive policy evaluation.

Result: AdaGRPO consistently drives performance gains and significantly stabilizes GRPO training for flow models, as demonstrated through extensive experiments integrated with frameworks like Flow-GRPO, DanceGRPO, and Flow-CPS.

Conclusion: AdaGRPO is a lightweight, plug-and-play module that effectively addresses the decoupled learning loop in flow-based GRPO, improving both effectiveness and stability of alignment in T2I generation.
...
TL;DR: CFRNet is a 2.0M-parameter ResNet-style blind face restorer for consumer NPUs, trained via Cycle-Consistent Fixed-Point Training (CCFP) to act as a fixed-point operator, achieving superior perceptual quality (LPIPS 0.250) and efficient on-device inference (~23ms per cycle on INT8 NPU) while being deployable where strong baselines like GFPGAN/CodeFormer cannot.


<details>
  <summary>Details</summary>
Motivation: Existing blind face restoration methods either rely on large generative priors with operators hard to deploy on consumer NPUs (GFPGAN, CodeFormer), or small convolutional networks that over-smooth and leave artifacts. Need a method that balances quality, speed, and deployability on small NPUs.

Method: Proposes CFRNet (2.0M ResNet-style) with Cycle-Consistent Fixed-Point Training (CCFP). CCFP uses three losses: (1) progressive multi-cycle supervision, (2) idempotence loss, (3) re-degradation cycle loss. Trains the network to act as a fixed-point operator so repeated application doesn't change output; k cycles at inference acts as a quality knob without retraining.

Result: CFRNet achieves LPIPS 0.250 at 3 cycles (31% lower than 1 cycle), best PSNR/SSIM at 2 cycles. Runs ~23ms/cycle INT8 on HiSilicon Hi3402 NPU. Baselines (GFPGAN, CodeFormer) cannot be compiled to this chip. Cycle count k provides a simple quality knob: PSNR best at k=2, LPIPS improves to k=3.

Conclusion: Cycle-Consistent Fixed-Point Training effectively enables lightweight CNNs to achieve competitive blind face restoration quality on consumer NPUs, with the cycle count serving as a convenient quality-speed trade-off knob. The approach generalizes to even simpler plain CNNs and runs in real time on driver-monitoring hardware.
...
TL;DR: MotionEnhancer improves VLM motion understanding by distilling motion priors from a video diffusion model via attention alignment, using two lightweight parameter-free modules (MHS and MTTI), achieving consistent gains on motion-level benchmarks without extra training or architecture changes.


<details>
  <summary>Details</summary>
Motivation: Current VLMs excel at event- or story-level understanding but struggle with fine-grained motion details due to their focus on high-level static semantics. VDMs naturally capture dynamic motion patterns, motivating the authors to leverage VDM motion priors to enhance VLM motion understanding.

Method: Two parameter-free modules: (1) Motion-sensitive Head Selection (MHS) extracts motion-related attention from the VDM, and (2) Motion-salient Text Token Identification (MTTI) aligns motion-salient text tokens. Motion priors from a powerful VDM are distilled as auxiliary supervision via attention alignment, requiring no additional training parameters or architecture modifications.

Result: Consistent improvements over state-of-the-art VLMs on two motion-level video understanding benchmarks, particularly on motion-related metrics.

Conclusion: MotionEnhancer provides a scalable, training-free solution for enhancing VLM motion understanding by distilling VDM motion priors, without modifying existing architectures or adding parameters.
...
TL;DR: FS-DVS introduces a learnable spatial filter before event triggering in dynamic vision sensors, mimicking retinal ganglion cells to achieve frequency-selective sensitivity (like human contrast sensitivity function), improving performance in object detection and action recognition with high noise resilience.


<details>
  <summary>Details</summary>
Motivation: Conventional DVS lack spatial integration and contrast sensitivity function (CSF) found in biological retinal ganglion cells, causing sub-threshold signal loss and information incompleteness.

Method: A learnable spatial filter is placed strictly before the event triggering process, optimized via a differentiable event simulation framework for end-to-end learning with downstream tasks.

Result: Learned filters spontaneously evolve into center-surround patterns emphasizing mid-frequency components (matching human CSF). Achieves substantial gains in object detection and action recognition, with high noise resilience.

Conclusion: FS-DSP provides a biologically plausible blueprint for next-generation neuromorphic sensors, demonstrating that mid-frequency selective enhancement is universal across tasks.
...
TL;DR: LRMIL is an efficient multiple instance learning framework that transfers high-resolution knowledge to low-resolution WSI representations via two-stage distillation, achieving superior performance with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing MIL methods for WSI analysis require exhaustive high-resolution patch processing, which fails to capture global low-magnification cues and incurs high computational overhead, limiting clinical practicality.

Method: A two-stage distillation framework: (1) patch-level cross-resolution distillation aligns low-resolution patch embeddings with high-resolution counterparts; (2) slide-level knowledge distillation trains a low-resolution student MIL model using both slide-level supervision and teacher guidance. Inference uses only low-resolution patches.

Result: LRMIL consistently outperforms state-of-the-art MIL methods on multiple WSI benchmarks while achieving substantially more efficient inference.

Conclusion: LRMIL is a practical and scalable solution for WSI analysis in clinical pathology, balancing accuracy and computational efficiency.
...
TL;DR: Multi-FRuGaL is a flexible, redundancy-aware gated fusion framework that handles missing multimodal data by decomposing signals into redundant and complementary components, achieving state-of-the-art results on two head and neck cancer datasets across multiple prognostic tasks.


<details>
  <summary>Details</summary>
Motivation: Real-world medical data often have missing or incomplete modalities (e.g., missing radiology, pathology, or reports), which limits the effectiveness of standard multimodal fusion approaches.

Method: Multi-FRuGaL uses per-modality encoders, a signal decomposition layer, an input-conditioned gating network, and an information-aware fusion objective to separate redundant from complementary signals, upweighting informative modalities while suppressing noisy/absent ones.

Result: On HANCOCK (5 modalities): survival AUC improved from 0.601 to 0.8496, recurrence AUC from 0.672 to 0.8102. On HECKTOR (3 modalities): HPV prediction AUC 0.975. Concordance indices: 0.6814 (OS), 0.7421 (RFS), 0.7143 (PFS) on HANCOCK; 0.7203 (RFS) on HECKTOR.

Conclusion: Multi-FRuGaL learns discriminative and robust multimodal representations even under severe missing-modality conditions, consistently outperforming baselines.
...
TL;DR: EgoPressDiff is a video diffusion framework that generates UV pressure maps from egocentric views, using multi-modal conditioning (pose, mesh, depth) and a Distribution-Calibrated Spatial Layer, achieving state-of-the-art results on the EgoPressure dataset with 34% better Volumetric IoU.


<details>
  <summary>Details</summary>
Motivation: Estimating hand-surface contact pressure from egocentric view is important for AR/VR, robotics, and ergonomics, but existing methods suffer from quantization errors and temporal inconsistencies due to discretizing pressure signals and processing frames independently.

Method: Proposes a conditional video diffusion framework (EgoPressDiff) with multi-modal conditioning: PoseNet and Vertex Encoder extract features from hand pose and 3D mesh vertices, combined with depth information. A Distribution-Calibrated Spatial Layer aligns statistical properties of heterogeneous features before fusion.

Result: Achieves state-of-the-art on EgoPressure ego-view setting: improves Volumetric IoU by over 34% relative to prior baseline, reduces MAE, and maintains high temporal accuracy.

Conclusion: The multi-modal conditioning and distribution-calibrated fusion effectively produce physically grounded, temporally consistent pressure fields from egocentric video, with significant improvements over prior methods.
...
TL;DR: UVR is a training-free safety framework for Diffusion Transformers (DiTs) with multimodal attention that regulates harmful content in image generation/editing by modulating attention dynamics across identified unsafe patches, achieving 91% and 77% erase rates in synthesis and editing tasks.


<details>
  <summary>Details</summary>
Motivation: Existing safety mechanisms are designed for T2I synthesis or U-Net architectures, limiting their effectiveness for unified safety mitigation in DiT-based frameworks, especially for I2I editing tasks.

Method: UVR analyzes attention dynamics in MM-Attn from an information flow perspective, identifies a task-independent start-up stage where unsafe semantics emerge, then applies targeted attention modulation and explicit restriction of harmful information flow over unsafe output patches.

Result: Achieves 91% erase rate in image synthesis and 77% erase rate in editing tasks, with minimal degradation in visual quality and fidelity.

Conclusion: UVR provides a training-free, unified approach to safe generation in DiT-based frameworks, effectively preventing harmful content while preserving generation quality.
...
TL;DR: FreeAnimate is a training-free framework for human image animation that uses preview generation strategies and attention modules to achieve temporal consistency, identity preservation, and background stability without requiring substantial training data or resources.


<details>
  <summary>Details</summary>
Motivation: Existing human image animation methods require substantial training data and resources, limiting generalization and accessibility. The authors aim to create a training-free approach that leverages inherent capabilities of image diffusion models.

Method: FreeAnimate uses a novel preview generation strategy for temporal/structural priors, plus Inversion-Boosted Attention and Reference-Anchored Self-Attention modules for temporal consistency and identity preservation.

Result: FreeAnimate outperforms existing training-free competitors and training-based baselines, achieving quality comparable to state-of-the-art methods with robust generalization across diverse datasets.

Conclusion: Training-free animation is feasible and competitive, and FreeAnimate demonstrates that leveraging inherent diffusion model capabilities can achieve high-quality results without expensive training.
...
TL;DR: ARAPDiffusion is a latent diffusion model that uses as-rigid-as-possible (ARAP) deformation regularization to learn shape spaces from deformation collections, requiring less 3D training data while improving both encoder/decoder and diffusion model through alternating training.


<details>
  <summary>Details</summary>
Motivation: To reduce the need for abundant 3D training data in generative shape models by leveraging deformation priors (ARAP) to regularize the learning process.

Method: Injecting ARAP deformation regularization losses into both the encoder/decoder and latent diffusion model, with alternating training between synthetic distributions from LD and shape decoder regularizations.

Result: Outperforms baseline approaches in both unconditional and conditional shape generation tasks.

Conclusion: ARAP regularization effectively reduces data requirements and improves generative quality in latent diffusion for 3D shape modeling, and the paradigm works well with unorganized point clouds via representation-free LD and implicit decoders.
...
TL;DR: VLMs rely excessively on language priors rather than visual evidence, which this paper mechanistically traces to a multi-stage bottleneck in intermediate and later decoder layers, and behaviorally demonstrates via a progressive visual decay metric showing that many benchmark examples remain answerable even under total visual obfuscation.


<details>
  <summary>Details</summary>
Motivation: While language-prior reliance in VLMs is widely observed, its internal mechanisms and impact on benchmark evaluation remain insufficiently understood.

Method: Internally: counterfactual layer replacement + supervised layer-wise MLP probing to trace visual vs. language-prior competition. Externally: progressive visual decay metric using multi-step Gaussian blurring to identify instances invariant to visual destruction.

Result: A substantial fraction of examples across 12 VQA benchmarks and 3 VLMs remain answerable under severe or total visual obfuscation, indicating benchmarks inadvertently reward visual ignorance.

Conclusion: Language-prior reliance is a systematic routing failure affecting both model internals and benchmark validity; future work needs training/evaluation protocols built on structurally isolated or counterfactual data.
...
TL;DR: Stream3D-VLM is an online 3D vision-language model that enables real-time spatial understanding from streaming video, using autoregressive control, Visual-Spatial Feature Integration, and Geometry-Adaptive Voxel Compression. It outperforms existing models across 29 tasks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D Large Multimodal Models operate offline, requiring complete scene observations or predefined video clips, which limits real-time spatial understanding from streaming video.

Method: 1) Autoregressive streaming control modeling based on LLM's next-token prediction to learn when to respond. 2) Lightweight Visual-Spatial Feature Integration (VSFI) module for temporally aligned geometry priors. 3) Geometry-Adaptive Voxel Compression (GAVC) for efficient visual token compression. 4) Scalable data generation pipeline with over 1M online spatio-temporal 3D QA pairs and a 29-task benchmark.

Result: Significantly outperforms both proprietary and open-source models across online and offline 3D spatial understanding, reasoning, and grounding tasks.

Conclusion: Stream3D-VLM successfully enables real-time 3D scene understanding from streaming video, addressing key limitations of offline models with novel modules for streaming control, feature integration, and token compression.
...
TL;DR: The paper proposes a lighting-aware representation learning framework that uses illumination variation as an explicit training signal in contrastive learning, improving downstream task performance on ImageNet, ExDark, and PASCAL VOC benchmarks without extra computational cost.


<details>
  <summary>Details</summary>
Motivation: Illumination variations cause major challenges for visual representation learning, and existing data augmentation methods do not explicitly model lighting information, treating it merely as something to be invariant to rather than leveraging it as useful signal.

Method: Extends contrastive learning with an auxiliary objective that captures illumination-dependent variation in rendered scenes, enabling joint learning of semantically consistent representations that remain sensitive to lighting-dependent visual structure.

Result: Consistent improvements over standard contrastive learning on ImageNet, ExDark, and PASCAL VOC for classification and detection, with the same architecture and training budget. Also shows promise in supervised learning and simpler lighting settings.

Conclusion: Lighting-aware training enhances model robustness and adaptability in complex visual environments and conventional image processing tasks, suggesting broad applicability beyond complex illumination scenarios.
...
TL;DR: LUCID is a unified framework for nighttime image restoration that jointly handles flares and noise through a flare disentanglement module and diffusion-driven reconstruction, with controllable four-mode training enabling selective artifact control and HDR exposure adjustment.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat nighttime degradations (flares and photon-limited noise) in isolation, overlooking their fundamental entanglement.

Method: A two-component framework: (1) a flare disentanglement module that removes optical artifacts to provide structural guidance, and (2) a diffusion-driven module using generative priors for clean reconstruction. A four-mode training strategy enables classifier-free guidance (CFG) for selective control over light sources, flares, ghosting, and continuous HDR exposure control.

Result: LUCID consistently outperforms state-of-the-art methods across diverse real-world nighttime scenarios.

Conclusion: Nighttime restoration is better framed as a continuous, controllable process rather than a fixed correction, with cooperative handling of entangled degradations.
...
TL;DR: DirectAnimator is a human image animation framework that bypasses pose extraction by learning directly from raw driving videos using a Driving Cue Triplet (pose, face, location cues) fused via a CueFusion DiT block, with a Same2X training strategy to handle cross-ID scenarios, achieving state-of-the-art results with fewer computational resources.


<details>
  <summary>Details</summary>
Motivation: Existing human image animation methods rely on pose estimators for intermediate representations, which are error-prone under occlusion or complex poses. The authors aim to bypass pose extraction and learn directly from raw driving videos to improve robustness.

Method: DirectAnimator introduces (1) a Driving Cue Triplet (pose, face, location cues) for rich yet stable motion expression, (2) a CueFusion DiT block for reliable denoising control, and (3) a Same2X training strategy that aligns cross-ID features with same-ID data for better generalization.

Result: State-of-the-art visual quality and identity preservation, robustness to occlusions and complex articulation, and lower computational resource requirements compared to existing approaches.

Conclusion: Bypassing pose extraction and directly learning from raw driving videos is a viable and superior approach for human image animation, achieving better robustness and efficiency.
...
TL;DR: polyDAG introduces a polynomial acyclicity constraint (replacing matrix-exponential) for efficient continuous causal discovery in visual semantic graphs, achieving faster runtime (33.4% speedup) and better structure recovery (F1 improvement from 0.725 to 0.756) on synthetic and CelebA datasets.


<details>
  <summary>Details</summary>
Motivation: Continuous directed acyclic graph (DAG) learning for visual semantic graphs is limited by the high computational cost of enforcing acyclicity using matrix-exponential constraints.

Method: Replaces the matrix-exponential acyclicity constraint with a finite polynomial trace constraint, proving it is zero exactly for acyclic graphs. Also derives a geometric-series implementation that avoids explicit summation loops.

Result: On synthetic graphs (d=100/200/500), mean SHD reduces from 318.4 to 285.4, F1 improves from 0.725 to 0.756. At 100 nodes, geometric variant runs 3.44s vs 5.16s (33.4% speedup).

Conclusion: polyDAG offers a more efficient and accurate alternative for continuous causal discovery in visual semantic graphs, with publicly available code.
...
TL;DR: This paper proposes a training-based method for AI-generated image detection that learns a structured invariance manifold of real images using frozen vision foundation models, outperforming training-free approaches in open-world settings.


<details>
  <summary>Details</summary>
Motivation: Existing training-free detection methods rely on fixed invariance geometry inherited from pretraining and lack principled adaptation to the detection task, limiting their effectiveness in open-world settings with unseen generators.

Method: Introduces lightweight projection heads on frozen VFMs to decompose representation into robust (physically invariant) and fragile (edit-sensitive) subspaces. A structured ordering margin enforces hierarchical separation, and multi-scale patch-wise drift yields a dual-channel invariance signature for detection.

Result: Strong open-world generalization across unseen generators and resolutions, consistently outperforming training-free robustness-based baselines while providing interpretable invariance-violation maps.

Conclusion: Learning a structured invariance manifold with one-class supervision is more effective for AI-generated image detection than relying on fixed pretrained invariance geometry.
...
TL;DR: This paper introduces SVHighlights, the first benchmark for highlight detection in extremely long sports videos (320 videos, avg 2 hrs each, total 640 hrs), and proposes TF-SELECTOR, a training-free segment-based method using LLMs with multimodal inputs that outperforms VTG-tuned baselines.


<details>
  <summary>Details</summary>
Motivation: Existing highlight detection methods are limited to short-form content due to the lack of a suitable benchmark for long-form videos. There is a practical need for detecting highlights in hour-long sports videos.

Method: SVHighlights is constructed from full-length sports videos paired with official highlight videos using a dataset generation pipeline for scalable labeling. TF-SELECTOR divides videos into context-aware segments by merging adjacent shots sharing the same semantic content, then predicts segment-level saliency using a large language model with multimodal inputs (visual captions, transcripts, audio volume).

Result: TF-SELECTOR achieves +3.12 in HIT@1, +4.06 in HIT@K, and +2.95 in IoU improvements over VTG-tuned baselines on the SVHighlights benchmark.

Conclusion: SVHighlights serves as a challenging testbed for long-form highlight detection, and a simple segment-based strategy can effectively scale to hour-long videos without training.
...
TL;DR: MAC improves CLIP's adversarial robustness via multi-view counterattacks with corruption-aware soft weighting, without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing TTC method for CLIP is fragile under strong attacks due to its reliance on a single corrupted view and a noise-driven hard-gating scheme that cannot adapt to varying corruption severity.

Method: MAC constructs augmented multi-view embeddings, performs counterattacks per view, adaptively scales attack intensity based on estimated corruption degree, and aggregates views for robust prediction.

Result: Extensive experiments across 20 datasets show substantial robustness improvements with high inference speed and memory efficiency, thanks to tuning-free design.

Conclusion: Multi-view guided adaptive counterattack is an effective, tuning-free approach to significantly enhance CLIP's robustness against adversarial attacks.
...
TL;DR: SS-TPT improves VLM robustness against adversarial perturbations by using stability and suitability scores to guide test-time prompt tuning, achieving better robustness-throughput trade-offs than prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs like CLIP are vulnerable to adversarial perturbations, and current test-time adaptation defenses that use many augmented views suffer from impractical slowdown and a poor robustness-throughput trade-off.

Method: SS-TPT evaluates each augmented view via two scores: (1) stability (prediction invariance to weak augmentations) and (2) suitability (feature-space density among views). These guide adaptation via an SS-guided consistency loss and inference via SS-weighted prediction, amplifying trustworthy views and suppressing corrupted ones.
...
Conclusion: SS-TPT demonstrates strong practicality and generality for test-time adversarial defense in VLMs, offering an efficient solution without sacrificing robustness.
...
TL;DR: The paper studies how input dimensionality (2D, 2.5D, 3D) affects CNN and ViT models for lung cancer screening, finding that 2.5D CNNs offer the best stability-efficiency trade-off, while 3D CNNs suffer threshold instability and transformers produce degenerate predictions.


<details>
  <summary>Details</summary>
Motivation: To determine whether the assumed superiority of 3D models for volumetric medical imaging is practically justified, given their added computational cost and complexity, by systematically comparing performance across 2D, 2.5D, and 3D inputs.

Method: Systematic comparison of input dimensionality (2D, 2.5D, 3D) across CNNs and Vision Transformers under a fixed training protocol, using a leakage-free NLST cohort (n=1,977) and LIDC-IDRI data.

Result: 2.5D CNN achieved the best discrimination-stability trade-off (ROC-AUC 0.682, 95% CI [0.546, 0.799]), while 3D CNNs showed threshold instability and transformers produced degenerate all-positive predictions; confidence intervals were wide and overlapping.

Conclusion: 2D and 2.5D inputs provide a more reliable trade-off between performance, stability, and computational efficiency than full 3D representations for class-imbalanced lung cancer screening classification. Results are presented as a controlled resource-performance frontier and failure-mode taxonomy, not as definitive superiority claims.
...
TL;DR: MVSegNet is a lightweight encoder-decoder network for fetal ventriculomegaly assessment via lateral ventricle segmentation in ultrasound. It combines multi-scale feature extraction and boundary-aware refinement, achieving 80.79% Dice, 4.07 mm Hausdorff distance, and 3.40 mm atrial width MAE with 2.31M parameters at 165.6 FPS, outperforming six baselines.


<details>
  <summary>Details</summary>
Motivation: Fetal ventriculomegaly assessment relies on accurate lateral ventricle segmentation in prenatal ultrasound, but acoustic shadowing, speckle noise, and poor contrast make this challenging. Existing methods lack sufficient accuracy and efficiency for clinical use.

Method: MVSegNet is a lightweight encoder-decoder network that combines multi-scale feature extraction with boundary-aware refinement. It was trained and evaluated on 584 expert-annotated transventricular ultrasound frames using a 70/15/15 split.

Result: MVSegNet achieved Dice=80.79%, IoU=68.47%, Hausdorff distance=4.07 mm, atrial width MAE=3.40 mm, with 2.31M parameters and 165.6 FPS on an NVIDIA T4 GPU, outperforming six baselines on boundary and measurement metrics.

Conclusion: MVSegNet outperformed all evaluated baselines on boundary and measurement metrics while maintaining low computational cost, supporting its use in automated fetal ultrasound analysis.
...
TL;DR: This paper proposes a compact multimodal PAD model combining visual and textual data for ID card presentation attack detection. The model performs well after supervised fine-tuning but fails in zero-shot settings. The study finds that model capacity and real-world data are crucial, while current synthetic datasets may not adequately represent real-world challenges.


<details>
  <summary>Details</summary>
Motivation: Cross-domain shifts and privacy concerns limit available data for Presentation Attack Detection (PAD) on ID cards, motivating the need for models that can generalize across domains with limited data.

Method: A compact multimodal model combining new generative and discriminative blocks that fuses visual and textual data for detecting presentation attacks on both genuine and synthetic ID images.

Result: The multimodal model shows strong generalization after supervised fine-tuning, but fails in zero-shot settings. Model capacity and real-world data are essential for reliable PAD performance.

Conclusion: Existing synthetic datasets may not reflect real-world challenges. The authors argue for re-evaluation of synthetic data as benchmarks and emphasize the need for more realistic, diverse datasets to advance PAD research.
...
TL;DR: CL-CLIP is a CLIP-based Continual Object Detection framework that uses cost-volume-guided category decoupling to mitigate catastrophic forgetting in open-vocabulary detectors, achieving strong performance on both new and old categories.


<details>
  <summary>Details</summary>
Motivation: Open-vocabulary detectors like F-ViT show strong zero-shot detection but suffer severe catastrophic forgetting when continually fine-tuned on new categories, losing previously calibrated detection ability.

Method: The method computes a CLIP image-text similarity cost volume (dense category-wise response maps) as zero-shot spatial priors to decompose shared region features into class-specific pathways, processed by a Multi-Expert RoI head.

Result: Experiments on PASCAL VOC and MS-COCO show CL-CLIP substantially improves F-ViT baseline under continual fine-tuning and achieves competitive performance with existing COD methods, especially in adapting to new categories while preserving base-class performance.

Conclusion: CL-CLIP effectively bridges open-vocabulary detection and continual learning by leveraging zero-shot priors to enable category decoupling, making it suitable for real-world deployments requiring incremental category learning.
...
TL;DR: LyraV introduces Streaming Video-Language Synchrony (SVLS) for online Video-LLMs, using a Frame-Driven Transition Controller and Streaming Token Pacer to achieve real-time, frame-level video-language synchrony without blocking perception during response generation.


<details>
  <summary>Details</summary>
Motivation: Existing Video-LLMs pause video perception while generating responses in streaming scenarios, breaking real-time synchrony and causing stutters. The goal is to achieve seamless, continuous human-AI interaction where video understanding and language generation happen concurrently.

Method: Two core innovations: (1) Frame-Driven Transition Controller (FDTC) — a training-free verification-based finite-state machine that decides when to speak, start new response, or stay silent; (2) Streaming Token Pacer (SToP) — a plug-and-play lightweight module that adapts language generation rate to visual pace via per-frame incremental, sub-budget decoding, emitting small token chunks per frame interval.

Result: Achieves 98.29% synchrony with video playback and 3.89 FPS real-time processing speed. Preserves backbone understanding ability while improving streaming synchrony and narrative fluency across five online and three offline benchmarks.

Conclusion: LyraV effectively enables fine-grained video-language synchrony in streaming settings, with an observed emergent capability of dynamic reasoning over streaming tokens, allowing continuous interpretation alongside visual input.
...
TL;DR: LARA is a plug-and-play framework that jointly optimizes Latent Action Models (LAM) and Visual-Language Action (VLA) models via representation alignment, achieving ~10%, ~5%, and ~15% improvement in pre-training, post-training, and LAM refinement settings respectively.


<details>
  <summary>Details</summary>
Motivation: VLA models require large-scale high-quality data but are limited by scarce real-world robot datasets. While LAMs can learn from unlabeled human videos, they are typically trained separately from VLAs, leaving LAM ungrounded during VLA training and VLAs constrained by frozen LAM representations.

Method: LARA is a plug-and-play framework that jointly optimizes LAM and VLA via representation alignment, enabling reciprocal benefits: LAMs learn with action trajectories to avoid spurious visual changes, while VLAs are regularized by forward dynamics from LAMs to reduce hallucinations of functionally ineffective trajectories.

Result: Achieves ~10% improvement for pre-training, ~5% for post-training enhancement of pre-trained VLA models, and ~15% for LAM refinement across 3 simulation and 1 real-world robotic manipulation benchmarks.

Conclusion: LARA effectively addresses the misalignment between separately trained LAM and VLA models through joint optimization via representation alignment, demonstrating consistent improvements across multiple settings and benchmarks.
...
TL;DR: GuideCAD is a parameter-efficient multi-modal framework that uses a mapping network to convert image embeddings into prefix embeddings, enabling a pretrained LLM to generate 3D CAD models with ~4x fewer parameters and 2x training efficiency compared to fine-tuning approaches.


<details>
  <summary>Details</summary>
Motivation: Multi-modal 3D CAD generation requires substantial computational resources, so the authors aim to reduce training costs while maintaining high-quality generation.

Method: GuideCAD uses a mapping network to convert image embeddings into prefix embeddings, which feed into a pretrained large language model to integrate visual and textual information. A transformer-based decoder then predicts the construction sequence for 3D CAD model generation.

Result: GuideCAD generates comparably high-quality 3D CAD models while using approximately four times fewer parameters and achieving twice the training efficiency compared to fine-tuning approaches.

Conclusion: The proposed GuideCAD method demonstrates that leveraging semantically rich visual-textual representations with minimal trainable parameters can effectively and efficiently generate 3D CAD models, setting a strong benchmark for resource-efficient multi-modal 3D generation.
...
TL;DR: This paper explores lightweight image-based ReID for 3D MOT on mobile robots, showing that cascaded matching outperforms naive fusion for tracking through occlusions.


<details>
  <summary>Details</summary>
Motivation: LiDAR-based 3D MOT struggles with occlusions and crowded scenes using only geometry. RGB-based ReID offers a solution but existing methods rely on computationally expensive parallel detectors unsuitable for real-time mobile robots.

Method: A lightweight projection-based framework decouples geometric and appearance modeling. Lightweight CNNs and Vision Transformers are compared. Various multi-modal data association strategies (naive linear fusion vs. cascaded matching) are evaluated on the KITTI Pedestrian dataset.

Result: Naive linear fusion of appearance and motion costs degrades performance due to visual noise. Cascaded matching successfully recovers occluded tracks without compromising precision and prevents identity switches. Lightweight architectures achieve an optimal trade-off between latency and discriminative power.

Conclusion: Lightweight image-based ReID integrated via cascaded matching strategy effectively improves 3D MOT for mobile robots, balancing real-time responsiveness with robust tracking in human-populated environments.
...
TL;DR: ZeroSight introduces a new benchmark for Zero-Shot Composed Image Retrieval with video-sourced consistent image pairs, a true zero-shot setup (post-March 2022 data), and a training-free MLLM method (SC4CIR) using symmetric consistency checks. Experiments across 27 methods show existing datasets inflate performance.


<details>
  <summary>Details</summary>
Motivation: Existing ZS-CIR datasets suffer from (1) irrelevant reference-target pairs due to noisy image sources, and (2) failure to achieve true zero-shot scenarios because they rely on public datasets already seen by CLIP during pre-training.

Method: (1) A video-based data construction pipeline that extracts frames from a single video to ensure visually/semantically consistent reference-target pairs, using post-March 2022 video data to guarantee a true zero-shot setting. (2) SC4CIR, a training-free MLLM-driven method that performs 3 symmetric consistency checks to identify hard negative targets.

Result: Evaluations across 27 methods reveal that existing ZS-CIR datasets and metrics yield inflated retrieval performance, overestimating CIR method capabilities.

Conclusion: ZeroSight provides a more reliable benchmark, and SC4CIR offers a plug-and-play solution that integrates with various CIR methods to improve performance in realistic zero-shot settings.
...
TL;DR: Dash2Sim converts monocular dashcam videos into metric 4D driving logs for simulation, producing the ROADWork4D benchmark across 17 cities. It reveals that both rule-based and learning-based planners struggle with work zone lane changes, and the recovered dense depth improves novel-view synthesis by up to 19%.


<details>
  <summary>Details</summary>
Motivation: Self-driving simulations are limited to data from a few cities or synthetic scenarios. Dashcam videos cover broader real-world situations (including rare work zones) but are underexploited because recovering accurate 4D scenes from monocular in-the-wild video is difficult.

Method: Dash2Sim turns monocular dashcam videos into metric, geo-referenced 4D driving logs compatible with existing simulators, verified against an independent map without annotations.

Result: Created ROADWork4D dataset (4,244 scenes, 2.7M 3D objects, 17 cities); verified subset ROADWork4D-CL (2,201 scenes). Found both rule-based/hybrid and learning-based planners fail at work zone lane changes. Dense depth from Dash2Sim improves novel-view synthesis by up to 19%.

Conclusion: Work zone scenarios remain challenging for all planner types. Dash2Sim's dense depth recovery shows strong potential for enriching closed-loop sensor simulation from monocular videos.
...
TL;DR: ForensicConcept extracts interpretable forensic concepts from AI-image detectors and enables their transfer across different backbone architectures, improving generalization to unseen generators.


<details>
  <summary>Details</summary>
Motivation: Existing AI-generated image detectors fail on unseen generators, and their black-box nature prevents understanding why. The authors aim to build interpretable, transferable forensic evidence.

Method: (1) Localizes decision-critical patches via Transformer attribution; (2) Clusters patches into a concept codebook; (3) Uses concept-aligned projection for auditable evidence; (4) Introduces CleanDIFT diffusion features as generation-trace reference; (5) Quantifies backbone-trace alignment via CKNNA; (6) Proposes concept codebook injection to transfer diffusion-derived concepts.

Result: Consistent improvements on GenImage, GAN-family, and Chameleon benchmarks. CKNNA alignment predicts transfer effectiveness across backbones.

Conclusion: The framework provides both better generalization and interpretability. CKNNA offers a principled explanation for why certain backbones enable more transferable forensic evidence.
...
### [53] [STREAM: Stochastic Riemannian Flow Matching with Anisotropic Decoder for Digital Histopathology Image Generation](https://arxiv.org/abs/2606.07036)
*Won June Cho,Daeky Jeong,Hyeongyeol Lim,Hongjun Yoon*

Main category: cs.CV

TL;DR: STREAM uses pretrained histopathology VFMs as the latent space (with Riemannian flow matching on the unit hypersphere) to generate synthetic histopathology images, avoiding the "conditioning collapse" problem seen in prior conditioning-signal approaches. It achieves SOTA reconstruction and generation on breast and colorectal cancer datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods use pretrained VFMs as conditioning signals, which leads to "conditioning collapse" — where the conditioning signal dominates the latent space, reducing quality and diversity. The authors want to solve this by using VFMs as the latent space itself.

Method: 1) Use pretrained histopathology VFMs' patch-token features (ℓ₂-normalized, on unit hypersphere Sᵈ⁻¹) as the latent space. 2) Apply Riemannian flow matching on the hypersphere. 3) A bridge-type stochastic perturbation for per-token rectifiability to train a DiT in latent space. 4) A novel anisotropic decoder that allocates robustness to low-energy directions of the velocity-field Jacobian while preserving fidelity along high-energy directions.

Result: State-of-the-art reconstruction and generation performance on breast and colorectal cancer datasets.

Conclusion: STREAM is the first framework to apply Riemannian flow matching in the pathology domain, effectively addressing conditioning collapse and achieving superior generative performance. Code will be released upon acceptance.
...
### [54] [TrioPose: Native Triple-Stream Diffusion Transformers for Pose-Guided Text-to-Image Generation](https://arxiv.org/abs/2606.07053)
*Dian Gu,Zhengyi Yang*

Main category: cs.CV

TL;DR: TrioPose introduces a Triple-Stream Pose-Aware DiT framework for SD3.5M that treats pose as an independent modality, using zero-initialized dual-residual injection, a Learnable Relational Bias Mask for decoupling multi-instance occlusions, and Pose-Guided Spatial Loss Weighting to achieve SOTA on complex multi-person text-to-image generation.


<details>
  <summary>Details</summary>
Motivation: Existing pose-guided generation methods suffer from limb distortions and feature crosstalk in complex multi-person scenarios, and UNet-based adapters struggle with long-range dependencies. MM-DiTs offer better global modeling but naive signal concatenation disrupts pre-trained latent distributions.

Method: (1) Triple-Stream Pose-Aware DiT (TSPA-DiT) treating pose as independent modality with layer-wise activation and zero-initialized dual-residual injection; (2) Learnable Relational Bias Mask that maps topological connectivity into continuous attention soft constraints; (3) Pose-Guided Spatial Loss Weighting using heatmap-derived error maps.

Result: SOTA performance on Human-Art, CrowdPose, and OCHuman. AP of 64.33 on Human-Art (~30% improvement over prior arts), with new standards for visual fidelity and text-image semantic alignment in multi-human generation.

Conclusion: TrioPose effectively addresses limb distortions and inter-instance interference in complex multi-person generation by integrating pose as an independent modality with carefully designed injection and decoupling mechanisms, significantly outperforming prior methods.
...
### [55] [AsyncPatch Diffusion: spatially-flexible image generation](https://arxiv.org/abs/2606.07079)
*Samuele Papa,Valentin De Bortoli,Guillaume Couairon,Daniel Sýkora,Romuald Elie,Klaus Greff*

Main category: cs.CV

TL;DR: AsyncPatch Diffusion assigns different noise levels to different spatial regions of an image, enabling heterogeneous denoising trajectories, adaptive generation, and native inpainting without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion models apply a single shared noise level to the entire sample, forcing all spatial regions to follow the same denoising trajectory, which limits flexibility for tasks like inpainting or adaptive generation.

Method: Proposes AsyncPatch Diffusion, a joint-diffusion framework that assigns distinct noise levels to different spatial regions/image patches. Introduces a controlled noise-level sampler to balance heterogeneous and homogeneous noise configurations during training. Proves the first valid ELBO for this asynchronous corruption process.

Result: Achieves generation quality comparable to conventional diffusion on ImageNet 256 and LSUN. Enables native inpainting without task-specific fine-tuning. Demonstrates input guidance for improved local consistency, and adaptive strategies like uncertainty-guided acceleration and autoregressive sampling.

Conclusion: AsyncPatch Diffusion extends standard diffusion models with spatially heterogeneous denoising, offering comparable quality while enabling flexible generation modes (inpainting, guidance, adaptive acceleration) without retraining.
...
### [56] [An Adaptive Data cleaning Framework for Noisy Label Detection](https://arxiv.org/abs/2606.07086)
*Chen-Hsuan Fang,Wei-Hsinag Chen,Pin-Hsuan Yu,Jung-Hua Wang,Tsung-Wei Pan*

Main category: cs.CV

TL;DR: This paper proposes a self-adaptive data-cleaning framework that integrates local, global, and learning dynamics cues for robust noisy-label detection in DNNs. It uses multi-metric clustering (2D or 3D feature spaces) instead of conventional 1D Gaussian Mixture Models, eliminating the need for manual thresholds or noise priors. Experiments show high recall and accuracy gains across CIFAR-10, MNIST, and ImageNet-100 with 5%-40% symmetric label noise.


<details>
  <summary>Details</summary>
Motivation: Over-parameterized DNNs easily memorize noisy labels, degrading accuracy and generalization. Existing data-cleaning methods rely on manual thresholds, prior knowledge of noise ratio, or single metrics, making them unstable in complex data regimes.

Method: The framework maps samples into a unified low-dimensional feature space via modular feature concatenation, then performs multi-metric clustering (2D: KNN-based local disagreement + k-means centroid distance; 3D: adds z-normalized score) to adaptively partition samples into clean-dominant and noise-dominant components without manual thresholds or noise priors.

Result: Achieves high recall across CIFAR-10, MNIST, and ImageNet-100 with 5%-40% symmetric noise, including near-perfect recall (>=98%) on ImageNet-100 at 40% noise. Training on cleaned data yields accuracy gains, especially under severe corruption on ImageNet-100.

Conclusion: Multi-metric integration provides a threshold-free, practical, and low-tuning strategy for noisy label detection, outperforming conventional single-metric approaches.
...
### [57] [Detecting Temporally Localized Manipulations in Authentic Video Streams](https://arxiv.org/abs/2606.07090)
*Okan Umur,Ali Emre Güşlü,Ibrahim Delibasoglu*

Main category: cs.CV

TL;DR: This paper identifies a gap in existing deepfake detection datasets—they don't cover scenarios where a short manipulated segment is inserted into an otherwise authentic video—and proposes a new dataset for temporally localized video manipulation detection, benchmarking DINOv3-based approaches.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for deepfake detection, object removal, and video inpainting do not adequately model scenarios where a short manipulated segment is inserted into an otherwise authentic video that continues afterward.

Method: The authors review existing datasets, identify their limitations, create a new custom-curated test set, and evaluate two complementary approaches: (1) a linear probe on DINOv3 features with three thresholding strategies, and (2) a consecutive frame similarity-based method using DINOv3 features for temporal boundary detection.

Result: The two DINOv3-based approaches establish an initial benchmark for partially manipulated video detection, with results highlighting the need for content-adaptive thresholding mechanisms.

Conclusion: Current benchmarks are insufficient for detecting short manipulated segments in authentic videos; the proposed dataset and initial benchmarks reveal that content-adaptive thresholding is necessary for effective detection in this challenging scenario.
...
### [58] [GP-Adapter: Gaussian Process CLIP-Adapter for Few-Shot Out-of-Distribution Detection](https://arxiv.org/abs/2606.07102)
*Taisei Saito,Koretaka Ogata,Takafumi Hiroi*

Main category: cs.CV

TL;DR: GP-Adapter is a training-free framework that adds Gaussian Process uncertainty modeling to CLIP, improving few-shot classification and OOD detection using modality-specific one-class GPs with an RBF kernel for images and linear kernel for text, without fine-tuning the backbone.


<details>
  <summary>Details</summary>
Motivation: CLIP provides deterministic similarity scores with limited uncertainty information, which is problematic under distribution shift and data scarcity (few-shot settings). The authors aim to enhance CLIP's reliability by integrating probabilistic uncertainty modeling.

Method: GP-Adapter constructs modality-specific, class-wise one-class Gaussian Processes on frozen CLIP embeddings: an RBF kernel for image features and a linear kernel for text prompts. It fuses their predictive statistics to produce variance-aware confidence scores for OOD detection, requiring only a small K-shot cache and no fine-tuning of the CLIP backbone.

Result: On ImageNet and multiple OOD benchmarks, GP-Adapter achieves competitive few-shot performance and consistently improves OOD detection when combined with prompt-learning baselines. Memory cost scales as O(CK^2).

Conclusion: Integrating probabilistic inference (GP-based uncertainty modeling) with large pre-trained vision-language models like CLIP can improve reliability in low-data and distribution-shifted settings.
...
### [59] [3DMorph: Single-Image-Guided Local 3D Shape Editing and Morphing](https://arxiv.org/abs/2606.07115)
*Tobias Preintner,Yunfei Deng,Phillip Müller,Sebastian Illing,Adrian König,Thomas Bäck,Elena Raponi,Niki van Stein*

Main category: cs.CV

TL;DR: 3DMorph is a training-free framework for single-image-guided local 3D shape editing and morphing that transfers 2D edits to 3D geometry while preserving unmodified areas, outperforming existing methods on the new Delta3D benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing 3D editing methods are often global, domain-specific, require complex user interaction, or focus on appearance rather than geometry. There is a lack of simple, effective local shape editing tools for general 3D objects like meshes.

Method: 3DMorph is a training-free framework that takes an edited 2D image as input, automatically localizes the relevant 3D region, transfers 2D modifications to 3D geometry while preserving unmodified areas, and enables intermediate shape generation between original and edited objects.

Result: 3DMorph outperforms state-of-the-art generative and editing methods on the Delta3D benchmark, which is a new image-guided local 3D editing benchmark with paired ground-truth edits.

Conclusion: 3DMorph enables intuitive 2D edits to be effectively translated into 3D shape modifications, providing a simple and effective solution for local 3D shape editing and morphing.
...
TL;DR: Native3D is the first end-to-end 3D scene generation framework that bypasses 2D intermediate representations, using a unified mesh-texture joint representation with a Transformer encoder and a 3D REPA Loss for improved geometric and textural fidelity.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D generation methods rely on adapting 3D representations to 2D domain for leveraging pre-trained diffusion models, which causes domain adaptation issues like geometric distortion and texture degradation.

Method: (1) A unified mesh-texture joint representation with a Transformer-based scene encoder that models geometry and texture jointly. (2) 3D Representation Alignment Loss (3D REPA Loss) using improved contrastive learning to align multi-level semantic representations in latent space.

Result: Outperforms existing methods in both generation quality and editing flexibility.

Conclusion: Provides a novel solution for 3D scene editing by completely bypassing 2D intermediates.
...
TL;DR: Consistent-Inversion is a training-free reverse consistency guidance framework that corrects latent trajectory mismatches in text-guided diffusion editing by checking whether intermediate target denoising steps can be reversed to align with the source inversion trajectory, improving background/structure preservation without sacrificing edit fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing training-free inversion-based editors reuse source noisy latents for target-prompt denoising, but this couples source reconstruction with target editing, causing trajectory mismatches that either damage background details or over-constrain edits.

Method: Consistent-Inversion constructs an auxiliary target-side noise representation, performs source-guided reverse denoising to check if intermediate target trajectories can be reversed toward the source inversion trajectory under the source prompt, and uses the resulting reverse consistency discrepancy as a correction signal for selected early target denoising steps.

Result: On PIE-Bench under a unified SD3.5 protocol, Consistent-Inversion improves background and structural fidelity while maintaining target-prompt alignment. Compatibility experiments verify the correction principle on classical Stable-Diffusion inversion pipelines.

Conclusion: Consistent-Inversion effectively decouples source reconstruction from target editing via a reverse consistency check, offering a lightweight, training-free correction that boosts structure preservation without updating model parameters.
...
TL;DR: TraRA is a plug-and-play method for Video Text Spotting that uses trajectory-level recognition aggregation with Temporal Clustering and Vision-Language Aggregation modules to improve text recognition under challenging surveillance conditions like motion blur and occlusion.


<details>
  <summary>Details</summary>
Motivation: Reliable text recognition in video streams is challenging due to dynamic factors like motion blur, occlusion, and scale variation. Existing VTS methods perform frame-level recognition independently, leading to inconsistent and inaccurate results across sequences.

Method: TraRA proposes trajectory-level text recognition via two modules: (1) Temporal Clustering refines noisy trajectories by grouping temporally and visually coherent text instances; (2) Vision-Language Aggregation uses a Low-Rank Adaptation-enhanced VLM to fuse visual cues with linguistic context across frames.

Result: Experiments on four benchmarks (RoadText, BOVText, ArTVideo, ICDAR15) show consistent improvements in tracking and recognition performance over state-of-the-art VTS methods.

Conclusion: TraRA is an effective plug-and-play method that achieves robust text recognition by aggregating information over entire text trajectories, even under challenging surveillance conditions.
...
TL;DR: This paper introduces SPPE, the first recovery-oriented benchmark for privacy-preserving image editing using surrogates, along with two methods (ERMA for editability assessment and C2E-S2SER for surrogate-to-source edit recovery) that significantly outperform existing baselines.


<details>
  <summary>Details</summary>
Motivation: Existing privacy protection strategies for MLLM-based image editing substitute sensitive regions with surrogate content before cloud editing, but they neglect local recovery of the original source image in both design and evaluation, producing edited surrogates rather than edited source images.

Method: The paper proposes SPPE benchmark with 36 privacy categories and 65 editing instructions, defining two tasks: 1) Editability assessment via ERMA (instruction-aware multimodal relation modeling); 2) Surrogate-to-source edit recovery via C2E-S2SER (cycle-consistent recovery using surrogate editing pair as visual evidence and source image as anchor).

Result: ERMA improves over baselines by 13.9% in SRCC and 12.3% in PLCC for editability assessment. C2E-S2SER outperforms SOER across all 8 source integrity and edit consistency metrics on SPPE.

Conclusion: SPPE provides a comprehensive recovery-oriented evaluation framework for privacy-preserving editing, and the proposed methods effectively address both editability prediction and recovery tasks.
...
TL;DR: This paper compares geospatial understanding across vision-only, vision-language, and multimodal models, finding that textual supervision improves spatial representations and that language serves as a complementary modality for geospatial AI.


<details>
  <summary>Details</summary>
Motivation: Geospatial understanding is critical yet underexplored in machine learning; the authors aim to systematically evaluate how different model families acquire geospatial representations and identify gaps in spatial accuracy.

Method: The study evaluates three model families (vision-only like ViT, vision-language like CLIP, and multimodal foundation models like LLaVA, Qwen, Gemma) across image clusters grouped by localizability (people, landmarks, everyday objects).

Result: Systematic gaps in spatial accuracy were found; textual supervision enhances geospatial representations, with language acting as an effective complementary modality.
...
TL;DR: APD is a training-free method that protects privacy in MLLMs by drifting sensitive elements to semantically equivalent alternatives while anchoring context, evaluated on the AdaptShield benchmark across 22 privacy categories, achieving ~10.4% and ~8.5% average gains in privacy and utility.


<details>
  <summary>Details</summary>
Motivation: Existing privacy protection methods rely on predefined categories and fixed obfuscation strategies, failing to handle unpredictable sensitive inputs and privacy-sensitive visual context in MLLMs.

Method: Anchored Privacy Drifting (APD) — a training-free approach that drifts privacy-sensitive elements toward semantically equivalent alternatives while anchoring contextual cues to the source image.

Result: Average gains of 10.4% on textual categories and 8.5% under MLLM-based evaluation across Qwen2.5, Qwen3, InternVL3, and InternVL3.5.

Conclusion: APD achieves balanced improvements in both privacy sanitization and content retention, demonstrated via the comprehensive AdaptShield benchmark.
...
TL;DR: OPTIMUS is a framework for concept-based visual explanations in XAI that provides formal guarantees of sufficiency and minimality via prime implicant theory, unlike existing saliency methods that lack theoretical rigor.


<details>
  <summary>Details</summary>
Motivation: Existing XAI methods in computer vision prioritize end-user accessibility but lack formal guarantees, creating a gap between practical utility and theoretical rigor.

Method: OPTIMUS generates visual heatmaps grounded in prime implicant theory, ensuring explanations satisfy sufficiency (concepts guarantee the prediction) and minimality (no strict subset retains the guarantee).

Result: Validated on a visual classification benchmark, OPTIMUS heatmaps faithfully surface decision-relevant concepts underlying model predictions.

Conclusion: OPTIMUS bridges the gap between interpretability and formal guarantees in XAI, offering logically tight and visually coherent explanations.
...
TL;DR: AdaTok is a self-budgeting discrete 1D visual tokenizer that learns to allocate variable token counts per image based on visual complexity, achieving high fidelity (~1.50 rFID) with ~118 tokens on average and ~2.1x throughput gains over fixed 256-token baselines.


<details>
  <summary>Details</summary>
Motivation: Visual complexity is heterogeneous, so fixed token budgets overspend on simple images and underserve complex ones. Existing elastic tokenizers treat token length as an external hyperparameter rather than a learned output of the tokenizer itself.

Method: AdaTok combines Prioritized Representation Learning (nested tail masking + Multi-Head LoRA decoder heads) with Adaptive Token Allocation (lightweight deterministic-group GRPO policy over candidate budgets), trained with Dynamic Pareto Weighting for fidelity-efficiency balance.

Result: AdaTok-Full achieves rFID 1.31 at 256 tokens; AdaTok-Adaptive achieves rFID 1.50 with ~118 average tokens, outperforming discrete 1D baselines. In autoregressive generation, adaptive representation yields ~2.1x throughput over fixed 256-token decode.

Conclusion: Visual token count can be learned as a content-conditioned output of the tokenizer itself rather than set as a fixed hyperparameter, enabling efficient variable-length image representation.
...
TL;DR: DualGate-Net proposes a prior-aware dual-encoder framework with an adaptive gated fusion mechanism for cell detection in histopathology images, achieving SOTA results on OCELOT benchmark.


<details>
  <summary>Details</summary>
Motivation: Cell detection in histopathology relies on tissue context, but existing tissue-aware methods use static fusion strategies that propagate noisy information.

Method: DualGate-Net uses a ConvNeXtV2 local encoder + SegFormer global encoder with a learnable prior-gated fusion mechanism, plus an auxiliary foreground reconstruction branch and cellness-guided cues.

Result: Macro F1-scores of 0.7722 (validation) and 0.7345 (test) on OCELOT benchmark, outperforming prior methods.

Conclusion: Adaptive prior integration via learnable gating effectively improves robustness in histopathology cell detection.
...
TL;DR: A deep learning framework using vision transformers maps 38 years of forest disturbance across the contiguous US by modeling both temporal and spatial context, achieving high precision (up to 98.2%) but showing performance trade-offs compared to pixel-wise methods.


<details>
  <summary>Details</summary>
Motivation: Traditional pixel-wise satellite time-series analysis ignores spatial context, limiting accuracy in forest disturbance monitoring.

Method: A vision transformer architecture that simultaneously models temporal trajectories and spatial neighborhoods from Landsat, Sentinel-1, and Sentinel-2 data (1984-2022).

Result: Precision up to 98.2% (+-1 year detection on MTBS) and 71.3% (CONUS validation), F1-scores up to 75.8% and 47.3%, with reduced spatial artifacts but performance trade-offs across disturbance regimes.

Conclusion: The spatio-temporal model offers a promising foundation for consistent forest monitoring though with trade-offs compared to pixel-wise baselines.
...
TL;DR: This paper introduces a hypergraph-based framework with Geometric-Aware Prototypes for novel class discovery in point cloud segmentation, modeling high-order associations and geometric cues to improve segmentation accuracy beyond traditional pairwise methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on pairwise associations for class assignment and novel class reasoning, which limits their ability to capture complex relationships among known and novel classes. Additionally, they focus on semantic features while neglecting geometric information in point clouds.

Method: 1) A hypergraph-based framework that models high-order associations among classes for collaborative reasoning beyond pairwise relations. 2) Geometric-Aware Prototypes to enhance class-level geometric cues, propagating geometric information through hyperedges to improve spatial distribution understanding.

Result: Experiments on SemanticKITTI and SemanticPOSS datasets demonstrate the effectiveness and superiority of the proposed method.

Conclusion: Modeling high-order associations via hypergraphs and incorporating geometric-aware prototypes significantly improves novel class discovery and segmentation accuracy in point clouds.
...
TL;DR: ExMesh is a novel framework that directly optimizes explicit meshes from multi-view images by integrating differentiable optimization with discrete topology updates (adaptive vertex splitting/merging and real-time UV maintenance), achieving a balance of accuracy, efficiency, and mesh conciseness without reliance on intermediate representations or post-processing.


<details>
  <summary>Details</summary>
Motivation: Existing methods (implicit or explicit) rely on intermediate representations and post-processing steps (e.g., Marching Cubes, TSDF fusion), leading to artifacts and fragmented geometry. Directly optimizing explicit meshes faces two challenges: (1) adaptively refining mesh topology to capture detail without degenerate faces, and (2) maintaining consistent UV coordinates for high-fidelity texturing as the mesh evolves.

Method: ExMesh integrates differentiable optimization with discrete topology updates, featuring an adaptive vertex splitting and merging strategy along with real-time UV maintenance, enabling coarse-to-fine optimization while preserving geometric integrity.

Result: Extensive experiments demonstrate that ExMesh achieves a balance among accuracy, computational efficiency, and mesh conciseness, and it is the first framework to seamlessly integrate discrete topology operations into a continuous differentiable optimization pipeline.

Conclusion: ExMesh effectively addresses core challenges in direct explicit mesh optimization from multi-view images, offering a more integrated and artifact-free alternative to existing approaches.
...
TL;DR: The paper proposes CultureScore, a framework evaluating cultural faithfulness in video generation across three dimensions (Identity, Context, Behavior). Testing on 10 countries and 3 models, the best model scored only 56.8%, with Behavior being the hardest dimension. Human preferences aligned with CultureScore but inverted with VideoScore, proving cultural faithfulness is essential beyond visual quality.


<details>
  <summary>Details</summary>
Motivation: Current video generation metrics (e.g., VideoScore) only measure visual quality and completely ignore cultural faithfulness. For instance, a model showing a Namaste incorrectly as a handshake gets the same score as a correct one, which is problematic for equitable global representation.

Method: CultureScore decomposes cultural faithfulness into three dimensions: Identity (who is represented), Context (culturally localized background), and Behavior (normative gestures and interactions). It was operationalized across 10 countries, generating 6,180 videos from three state-of-the-art models.

Result: No model achieved culturally faithful generation. The best model scored only 56.8% overall, with Behavior below 52% across all models. Human preference rankings aligned with CultureScore but were inverted relative to VideoScore—the model with highest visual quality was ranked last by humans.

Conclusion: Cultural faithfulness is an essential, currently missing criterion for equitable video generation. Existing visual quality metrics are insufficient and can be misleading, as they inversely correlate with human cultural preferences.
...
TL;DR: AnchorWorld is a framework for interactive world modeling that enhances egocentric simulation by using 3D human motion with auxiliary exogenous viewpoints for full-body grounding, plus a customization mechanism via anchor views and text descriptions, significantly outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Interactive world modeling lacks versatile controllability for practical scenarios; the paper aims to bridge this gap by improving interaction integrity and enabling flexible world customization.

Method: (1) Uses 3D human motion as the primary interaction modality. (2) Introduces auxiliary training supervision from exogenous (third-person) viewpoints to capture full-body positioning relative to the environment. (3) Proposes a customization mechanism using anchor views in a unified world coordinate system with textual descriptions to control local scene dynamics.

Result: AnchorWorld significantly outperforms state-of-the-art baselines. Ablation studies confirm the effectiveness of key designs. The customization scheme demonstrates strong spatio-temporal geometric consistency and strict adherence to prescribed evolutionary dynamics.

Conclusion: The proposed framework advances egocentric interactive world modeling with robust spatial grounding and flexible world customization, showing promising results for practical simulation scenarios.
...
TL;DR: The paper introduces Varifold Moments Invariants (VMI), a unified framework for moment invariants that combines region geometry, boundaries, and tangent lines to create geometrically interpretable, discriminative features. Combined with simple classifiers like Random Forest or MLP, VMI achieves state-of-the-art contour-based classification with significantly lower computational cost, validated on diverse datasets (leaves, objects, cells).


<details>
  <summary>Details</summary>
Motivation: To create a unifying framework for various moment invariants while achieving high discriminative power, computational efficiency, and geometric interpretability for contour-based shape classification on lightweight devices.

Method: VMI leverages varifold theory to integrate region geometry, boundary contours, and tangent line families into invariant features. These features are coupled with lightweight classifiers (Random Forest or Multi-Layer Perceptron) for classification tasks.

Result: Outperforms state-of-the-art contour-based approaches in accuracy while drastically reducing computational cost, enabling deployment on light devices. High accuracy achieved on diverse datasets (leaves, objects, cells) with few geometrically interpretable features.

Conclusion: VMI provides an efficient, interpretable, and high-performing solution for contour-based shape classification, suitable for resource-constrained environments.
...
TL;DR: VeriDrive converts driving reasoning into a structured Perception-Evaluation-Revision chain with validator-guided correction, improving vision-language planning while reducing cost and token usage.


<details>
  <summary>Details</summary>
Motivation: Existing driving rationales are free-form, expensive to generate with frontier models, and lack verifiability. The authors aim to create structured, auditable, and cost-effective reasoning supervision for vision-language driving models.

Method: A Perception-Evaluation-Revision chain that grounds objects in future motion, evaluates ego trajectories with rule-checkable evidence, and revises risky intents. Combines local generation with validator-guided selective correction to scale data construction efficiently.

Result: Improves L2, Collision, and Intersection metrics over OmniDrive while reducing token usage, generation time, and actual LLM/VLM API costs on nuScenes.

Conclusion: Auditable intermediate fields and structured revision targets improve vision-language planning supervision under realistic annotation budgets.
...
TL;DR: The paper proposes a Spatial-Temporal Decoupled Adapter (STDA) for micro-gesture online recognition, using separate spatial and temporal branches with lightweight depthwise convolutions, plus an Adaptive Soft Balanced Augmentation (ASBA) to handle long-tail class distribution. It achieved 1st place in Track 2 of the 4th EI-MiGA-IJCAI Challenge with an F1 score of 0.43808.


<details>
  <summary>Details</summary>
Motivation: Micro-gesture online recognition is challenging due to extremely short duration, low motion amplitude, and ambiguous visual cues. Existing parameter-efficient adapters use a single branch to model spatial and temporal cues jointly, which fails to capture fine-grained patterns of micro-gestures.

Method: 1) Spatial-Temporal Decoupled Adapter (STDA): decomposes video adaptation into independent temporal and spatial branches using lightweight depthwise convolutions. 2) Adaptive Soft Balanced Augmentation (ASBA): dynamically allocates augmentation intensity based on class rarity and learning difficulty without manual thresholds.

Result: Achieved F1 score of 0.43808, ranking 1st in Track 2 of the 4th EI-MiGA-IJCAI Challenge.

Conclusion: The decoupled spatial-temporal modeling and adaptive augmentation strategy effectively address fine-grained pattern capture and long-tail distribution challenges in micro-gesture recognition.
...
TL;DR: MIDOG 2025 evaluated mitosis detection across 12 tumor types (human, canine, feline) in hotspots, random areas, and challenging regions. Detection track best F1=0.740; AMF track best balanced accuracy=0.908. False positives tripled in challenging ROIs; performance varied significantly across tumor types. Ensembling improved scores modestly (~1.5pp), TTA showed no benefit.


<details>
  <summary>Details</summary>
Motivation: Prior benchmarks focused only on scanner-induced domain shift, ignoring the vast biological and contextual diversity of real-world clinical settings. This paper addresses the need for robustness across tumor types, species, and tissue contexts (not just hand-selected hotspots).

Method: Curated 365 cases across 12 tumor types (human, canine, feline) from multiple scanners. Two tracks: (1) mitosis detection in hotspots, random areas, and challenging areas; (2) classification of atypical mitotic figures (AMFs). Evaluated ensembling and test-time augmentation (TTA).

Result: Detection track: F1 scores up to 0.740 (18 teams). AMF track: balanced accuracy up to 0.908 (21 submissions). False positive rates tripled in challenging ROIs. Substantial performance variation across tumor types. Ensembling improved F1 by 1.5pp and balanced accuracy by 1.3pp; TTA showed no improvement.

Conclusion: "In the wild" mitosis detection remains challenging. Multi-contextual evaluation (hotspots, random, challenging areas) provides a more realistic proxy for clinical reliability than hotspot-only benchmarks.
...
TL;DR: This paper introduces a diagnostic framework that uses Integer Linear Programming to isolate error sources (tracking, classification, segmentation) in Video Instance Segmentation, revealing tracking instability as the primary bottleneck—especially under occlusion and dense scenarios—and introduces TrackLens, a visual tool for analyzing failure modes.


<details>
  <summary>Details</summary>
Motivation: Current VIS metrics (AP) jointly evaluate classification, segmentation, and tracking, making it impossible to determine which sub-task contributes most to performance loss. The authors aim to disentangle these error sources to identify the true bottleneck in VIS systems.

Method: The authors formulate identity and class assignment as an Integer Linear Program (ILP) to create a model-agnostic oracle that hierarchically isolates errors from tracking, classification, and segmentation. They also introduce TrackLens, a visual tool that maps gap magnitudes to failure modes.

Result: Tracking instability is the critical bottleneck for online methods, with >20 AP gaps under heavy occlusion, growing with video length and instance density. Stronger backbones improve default scores but leave tracking gaps intact, confirming temporal fragility is algorithmic, not just representational.

Conclusion: The core challenge in VIS is robust long-term temporal association. The diagnostic framework and TrackLens provide systematic tools for targeting this challenge, enabling researchers to pinpoint and address specific tracking failures rather than optimizing entangled metrics.
...
TL;DR: RealDocBench is a two-track benchmark (QA and layout) built from real regulated documents across four domains. It evaluates 18 commercial/ML parsers on field-level extraction accuracy, cost, and latency, revealing wide performance gaps hidden by single-score benchmarks, a hard medical sub-domain, and sharp cost/latency trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate parsers on clean academic layouts or synthetic prose using single similarity scores (OCR/markdown), which correlate poorly with what downstream agents actually need: correct field-level values from messy real-world regulated documents.

Method: RealDocBench has two tracks: (1) QA track with 1,356 field-level questions over 581 documents across four domains, with typed gold_dict answers scored by per-field and strict per-question accuracy; (2) Layout track with 1,500 human-verified page images annotated with COCO-style bounding boxes under a nine-class taxonomy, scored via Hungarian matcher with adjacency-aware split/merge recovery. 18 systems (parsing APIs, VLMs, OCR models) are evaluated under a uniform extraction-and-scoring protocol.

Result: RealDocBench exposes a wide performance spread hidden by single-number benchmarks, identifies a persistently hard medical sub-domain, and reveals sharp cost/latency trade-offs across operating points.

Conclusion: Single-score benchmarks are insufficient for evaluating document parsers in real-world regulated settings. The authors release datasets, parser adapters, and evaluation harness to support reproducible, field-level comparison.
...
TL;DR: DisPOSE is a self-supervised framework that formulates multi-view 3D human pose estimation as a diffusion process over assignment tensors, using differentiable Sinkhorn projections and a Hypergraph-Convolutional Decoder to recover 3D skeletons, achieving state-of-the-art results and high label efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised methods rely on synthetic 3D pose catalogues, which generalize poorly to real-world scenarios due to distribution shifts. There is a need for a method that can recover 3D poses of multiple individuals from different camera views without such limitations.

Method: DisPOSE models the multi-view person-assignment problem as a generative diffusion process over polystochastic tensors, uses differentiable Sinkhorn projections during denoising, and regresses 3D skeletons via a Hypergraph-Convolutional Decoder that models relational structures across views.

Result: Outperforms state-of-the-art self-supervised methods on standard datasets; shows strong performance on a new benchmark with highly occluded surgical room scenes; retains 99% performance with only 10% of pseudo-labels.

Conclusion: DisPOSE is highly label-efficient and nearly agnostic to different camera arrangements due to disentangling assignment and root regression while maintaining differentiability.
...
TL;DR: An open-source smart glasses platform with event-based vision, modular FPC interposer, and power management achieving up to 11.8 hours continuous ML on a 200 mAh battery, demonstrated with an 83.94% accuracy hand gesture recognition pipeline.


<details>
  <summary>Details</summary>
Motivation: Smart eyewear lacks open-source platforms that support event-based vision and embedded ML under tight power, memory, and compute constraints in a compact form factor.

Method: Modular design using flexible FPC interposer for cameras; hardware-software co-designed power management with configurable PMIC and nRF5340 event-driven wake-up keeping GAP9 RISC-V SoC powered down between inferences; R(2+1)D on polarity-separated event histograms from Prophesee GENX320 camera.

Result: Up to 11.8 hours continuous on-device ML from 200 mAh battery; best cross-subject accuracy of 83.94% (macro F1=0.781) with 33.9 ms latency on GAP9 for egocentric hand gesture recognition on LynX dataset.

Conclusion: The platform is viable for rapid prototyping of smart eyewear with event-based vision and on-device ML; all designs, firmware, and models are open-sourced.
...
TL;DR: This paper presents a human-view perspective on LLM-based video understanding, organized around three functional abilities: watching, remembering, and reasoning. It provides a unified formulation, surveys representative methods, and discusses challenges, applications, datasets, and benchmarks.


<details>
  <summary>Details</summary>
Motivation: Video understanding with MLLMs is moving from short clips to long, multimodal, knowledge-intensive scenarios, creating a need for a unified framework to analyze how models acquire evidence, preserve context, and produce grounded outputs rather than treating video tasks as isolated benchmarks.

Method: The authors propose a formulation characterizing video understanding systems by perceptual representations, memory states, reasoning traces, and final predictions. They organize methods around three abilities: watching (fine-grained, comprehensive, audio-visual, efficient perception), remembering (offline and streaming memory), and reasoning (text-only and video-based reasoning).

Result: The paper provides a structured taxonomy of video MLLM methods, identifies key challenges (spatio-temporal perception, efficient long-video processing, memory modeling, streaming understanding, faithful reasoning), and covers application domains (egocentric, sports, instructional, medical, narrative) along with training datasets and evaluation benchmarks.

Conclusion: The work outlines open problems and future directions for scalable, memory-aware, and evidence-grounded video intelligence, and will continuously track related works via a GitHub repository.
...
TL;DR: VSR models outperform humans on benchmarks but rely more on training word frequency than visual perception, failing to achieve human-like visual speech perception.


<details>
  <summary>Details</summary>
Motivation: To investigate whether high-performing VSR models actually develop human-like visual speech perception, despite surpassing human benchmarks.

Method: Compared three VSR systems with human baselines on the MaFI word-level lipreading dataset using word, character, phoneme, and viseme-level metrics; also used text-only n-gram baseline and analyzed viseme accuracies, confusion matrices, and human-model correlations.

Result: Models achieve higher overall accuracy but succeed/fail on different words; text-only n-gram rivals human lipreading; errors better explained by training word frequency than visual informativeness; models gain most on hardest visemes and show weaker dependence on visual clarity.

Conclusion: VSR systems primarily exploit language cues from training data rather than visual perception, failing to bind visual features into meaningful words.
...
TL;DR: Skill-3D is a framework that improves agentic 3D spatial reasoning by learning scene-aware tool-use skills from memory, achieving substantial gains over existing methods (e.g., 39%→78% on VSI-Bench).


<details>
  <summary>Details</summary>
Motivation: Existing MLLM agents for 3D spatial reasoning misuse tools and exhibit biased preferences, applying a uniform tool-use strategy across all scenes despite the heterogeneous nature of 3D tasks. This leads to only marginal improvements over non-agentic methods.

Method: Skill-3D identifies the task scene, records tool-use trajectories in a Scene Memory, aggregates successful trajectories into reusable scene-aware skills (with failed trajectories as lessons), and uses a co-evolution loop where skills guide agents and new trajectories refine the skill library over time.

Result: Skill-3D improves tool utilization from 39% to 78% on VSI-Bench, boosts Gemini-3-Flash by 67% on MMSI-Bench, and enhances Qwen3-VL-8B by 43% on VSI-Bench through agentic post-training.

Conclusion: Scene-aware skill learning significantly improves tool-use accuracy in 3D spatial reasoning agents, and the Skill-3D framework enables effective adaptation to heterogeneous 3D scenes.
...
TL;DR: TEVI uses sparse autoencoders and caption-guided masking to better align CLIP's image-text embeddings by selectively retaining caption-described information, improving retrieval and robustness across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: CLIP's image and text embeddings are poorly aligned due to information imbalance where images contain more information than their captions describe, harming downstream performance.

Method: TEVI uses sparse autoencoders to disentangle image embeddings and trains a masking module to selectively reconstruct the embedding based on caption content, retaining only caption-described attributes.

Result: Improved retrieval performance on MS COCO, Flickr, IIW, DOCCI benchmarks (stronger gains with richer captions) and improved robustness on RoCOCO benchmark.

Conclusion: TEVI effectively improves cross-modal alignment by using captions as a signal for selective retention of image information, with greater benefits from more descriptive captions.
...
TL;DR: This paper proposes a contrastive learning method that generates synthetic samples by perturbing network weights instead of data to better preserve data structure, demonstrating improved performance on radar observations of meteors.


<details>
  <summary>Details</summary>
Motivation: Scientific observations produce large unlabeled datasets, and standard contrastive learning uses data-space augmentations that can fundamentally alter the underlying data structure, motivating a need for alternative perturbation methods.

Method: The proposed method generates contrastive samples by perturbing network weights rather than the underlying data, using a SimCLR-based pipeline applied to radar observations of meteors.

Result: Performance gains were demonstrated under matched protocols for radar observation data.

Conclusion: Perturbing network weights instead of data for contrastive learning better preserves data structure and yields improved performance for scientific observations like meteor radar data.
...
TL;DR: DnD is a method for directly comparing two object detection models by analyzing the intersection and differences of their correctly detected ground truth labels, revealing shared and individual mistakes, especially when combined with error types and confusion matrices.


<details>
  <summary>Details</summary>
Motivation: Standard metrics like mAP and TIDE only provide summary statistics for individual models, lacking a direct, intuitive comparison between two models to reveal shared vs. unique mistakes.

Method: DnD computes the intersection of ground truth labels detected by both models, their difference sets, and the complement of labels missed by both. This is combined with error-type analysis in a confusion matrix for detailed comparison.

Result: DnD provides a more direct and intuitive comparison than independent summary statistics, revealing individual and shared mistakes, and naturally integrates error-type analysis into a confusion matrix.

Conclusion: DnD is valuable for comparing detection models, especially useful for guiding explainability methods like ODAM toward metric-relevant examples grounded in structured subsets.
...
TL;DR: StreamForce is a causal, unified streaming video generation framework that enables real-time, physically grounded control via continuous force inputs, achieving state-of-the-art force adherence and motion realism at up to 16.6 FPS on a single GPU.


<details>
  <summary>Details</summary>
Motivation: Existing video models require separate models for different force types, assume fixed forces, or rely on non-causal processing, lacking a unified, real-time controllable system for physically grounded video generation.

Method: StreamForce designs a unified force representation as a control signal and develops a distillation pipeline for force-controllable video generation, combining autoregressive efficiency with force responsiveness in a causal framework.

Result: StreamForce achieves up to 16.6 FPS on a single GPU, with state-of-the-art performance in both force adherence and motion realism.

Conclusion: StreamForce demonstrates that causal, unified force control is feasible for real-time streaming video generation, outperforming prior work in both controllability and visual quality.
...
TL;DR: MemDreamer is a plug-and-play framework that decouples perception and reasoning for long-video understanding by incrementally constructing a Hierarchical Graph Memory, enabling agentic exploration with tool-augmented retrieval. It achieves SOTA results, reduces context window to 2% of full ingestion, and reveals a strong correlation between logic reasoning and long-video understanding performance.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with hours-long videos due to token explosion and attention dilution when processing full-length visual sequences.

Method: MemDreamer decouples perception and reasoning by incrementally streaming videos into a Hierarchical Graph Memory (three-tier architecture for semantic abstraction), and uses an agentic tool-augmented retrieval system with an Observation-Reason-Action loop during inference.

Result: SOTA across 4 benchmarks; narrows gap with human experts to 3.7 points; constrains reasoning context to 2% of full ingestion; delivers 12.5 point absolute accuracy gain.

Conclusion: Statistical analysis reveals a strong positive linear correlation between VLM's logic reasoning and long-video understanding performance, establishing agentic capability scaling as a new paradigm for multimodal comprehension.
...
TL;DR: UniSHARP extends SHARP from pinhole cameras to universal monocular rendering (perspective, fisheye, omnidirectional) by aligning images in omnidirectional latent space using ray-based Gaussian primitives and UniK3D-inspired feature decoders, outperforming alternatives on a new multi-FoV benchmark.


<details>
  <summary>Details</summary>
Motivation: SHARP's pinhole-specific assumptions limit it to conventional perspective cameras. The goal is to achieve photorealistic view synthesis across diverse camera systems (wide-FoV, fisheye, panoramic) with a single unified model.

Method: (1) Unified omnidirectional latent space for implicit alignment; (2) Ray-based universal representation with Gaussian primitives arranged along rays and radial distances; (3) Joint decoding of 2D semantic and 3D spatial features from UniK3D-inspired encoders to generate the complete Gaussian cloud.

Result: Outperforms alternative methods by a large margin on a new benchmark stratified by FoV covering diverse imaging systems.

Conclusion: UniSHARP successfully enables universal monocular rendering across a continuum of camera systems, demonstrating the effectiveness of omnidirectional latent space alignment and ray-based Gaussian representations.
...
### [91] [Quantum Hierarchical Locally Recoverable Codes](https://arxiv.org/abs/2606.06736)
*Venkatesan Guruswami,Rutuja Kshirsagar,Pranav Trivedi*

Main category: cs.IT

TL;DR: This paper extends quantum locally recoverable codes to (r,δ) and hierarchical settings, providing explicit constructions and bounds.


<details>
  <summary>Details</summary>
Motivation: To improve quantum storage efficiency by enabling local recovery of lost qudits with reduced resource overhead, analogous to classical local recovery codes.

Method: Constructs random and explicit (r,δ) QLRCs using quantum Tamo-Barg codes, and introduces hierarchical QLRCs with multiple recovery levels, analyzed via a CSS framework.

Result: Presents explicit families of (r,δ) QLRCs and h-level QHLRCs, along with efficient decoding algorithms and Singleton-like bounds.

Conclusion: Advances theoretical foundations of quantum erasure recovery and contributes to designing efficient quantum storage architectures.
...
### [92] [Degrees of Freedom of Over-the-Air Computation over a MIMO Gaussian Network with Two Transmitters and Two Receivers](https://arxiv.org/abs/2606.06770)
*Yong Dong,Hua Sun,Syed A. Jafar*

Main category: cs.IT

TL;DR: This paper studies over-the-air computation in a two-transmitter, two-receiver MIMO Gaussian network, deriving the fundamental AirComp degrees of freedom (ACDoF) metric under an asymptotic MSE constraint, showing the DoF is almost surely $\min\{M_1,M_2,N_1,N_2,(1/3)\max\{M_1+M_2,N_1+N_2\}\}$, with extensions to SISO settings and finite-SNR optimization via an iterative alternating algorithm.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental limits of over-the-air computation (AirComp) in MIMO Gaussian networks, particularly the achievable degrees of freedom under asymptotic MSE constraints.

Method: Defining an AirComp degrees of freedom (ACDoF) metric constrained by asymptotic mean-squared error threshold; analyzing generic MIMO settings with antenna counts $M_1,M_2,N_1,N_2$; extending to SISO arbitrary channels; developing an iterative alternating optimization algorithm for finite SNR.

Result: The AirComp DoF is almost surely $\min\{M_1,M_2,N_1,N_2,(1/3)\max\{M_1+M_2,N_1+N_2\}\}$ for generic MIMO settings.

Conclusion: Established fundamental limits on AirComp in MIMO networks, with extensions to arbitrary SISO channels and practical finite-SNR optimization.
...
### [93] [The Sharp Phase Transition of Tyler's M-Estimator for Robust Subspace Recovery](https://arxiv.org/abs/2606.06782)
*Gilad Lerman,Teng Zhang*

Main category: cs.IT

TL;DR: This paper resolves the behavior of Tyler's M-estimator at the critical DS-SNR=1 boundary for Robust Subspace Recovery, proving exact convergence under a less restrictive stability condition.


<details>
  <summary>Details</summary>
Motivation: Prior work established computational hardness thresholds for RSR (SSE-hard when DS-SNR<1, solvable when DS-SNR>1), but the exact behavior of practical algorithms at the critical boundary DS-SNR=1 remained unknown.

Method: The authors analyze Tyler's M-estimator (TME) using a decomposition of its iterates within a majorization-minimization framework, and prove convergence under a new stability condition that is less restrictive than general position assumptions.

Result: TME converges exactly to the true subspace for DS-SNR ≥ 1 under the proposed stability condition, establishing a sharp phase transition at the critical boundary.

Conclusion: The work closes the gap in understanding algorithm behavior at the critical DS-SNR=1 threshold, and provides a weaker condition (stability) than general position for guaranteed convergence.
...
### [94] [Information Rate Decomposition for Noisy Nanopore Channels with Geometric Duplication](https://arxiv.org/abs/2606.06808)
*Brendon McBain,Emanuele Viterbo*

Main category: cs.IT

TL;DR: This paper analyzes information rates in noisy duplication channels with memory, motivated by nanopore DNA sequencing. It decomposes the information rate into two interpretable terms (intrinsic memory and segment boundary uncertainty), uses Soft-DTW for analysis, and develops a lower bound based on jump distances between nanopore levels.


<details>
  <summary>Details</summary>
Motivation: Nanopore DNA sequencing involves both inter-symbol interference (ISI) from multiple DNA bases in the pore and random sample duplications from variable translocation speed, making direct theoretical analysis difficult. The paper aims to address this challenge.

Method: The paper derives a decomposition of the information rate into two terms: one for the intrinsic memory of an auxiliary ISI channel, and another for segment boundary uncertainty from random duplications. It uses a soft alignment functional related to Soft-DTW for analysis and develops a lower bound based on jump distances between adjacent nanopore levels.

Result: The decomposition enables strong AEP results and an alternative proof of the Markov-constrained coding theorem based on strong information stability. The lower bound provides a geometric explanation of channel synchronisability and a tractable framework for computing achievable rates.

Conclusion: The proposed decomposition and lower bound offer a tractable framework for analyzing noisy duplication channels with memory, with practical implications for computing achievable rates of Oxford nanopore sequencers.
...
### [95] [Weighted Sum-Rate Enhancement for Flexible Intelligent Metasurface-Assisted Multicell Systems](https://arxiv.org/abs/2606.06845)
*Hanwen Hu,Jiancheng An,Lu Gan,Hongbin Li,Naofal Al-Dhahir,George K. Karagiannidis,Arumugam Nallanathan*

Main category: cs.IT

TL;DR: This paper proposes an alternating optimization framework for maximizing weighted sum-rate in multicell MU-MISO systems using a flexible intelligent metasurface (FIM), jointly optimizing transmit beamforming, phase shifts, and surface shape via WMMSE, BCD, RCG, and PGD methods.


<details>
  <summary>Details</summary>
Motivation: FIM technology offers an additional degree of freedom (surface shape morphing) beyond conventional rigid RIS, enabling improved beamforming and interference mitigation in complex multicell scenarios.

Method: Joint optimization of BS transmit beamforming, phase shift matrix, and FIM surface shape using an alternating optimization framework: WMMSE for problem reformulation, BCD for iterative updates, RCG for phase optimization, PGD for surface shape optimization, and closed-form beamforming vectors.

Result: The proposed framework efficiently solves the non-convex optimization problem with highly coupled variables under constraints on transmit power, unit-modulus reflection coefficients, and surface morphing range.

Conclusion: FIM with surface shape optimization provides superior performance over conventional RIS in multicell MU-MISO systems, and the proposed joint optimization framework is effective for realizing these gains.
...
### [96] [Mutual Information Optimization via K-Recursion and Automatic Differentiation for Linear Gaussian Wireless Networks](https://arxiv.org/abs/2606.06982)
*Tadashi Wadayama,Na Siqi*

Main category: cs.IT

TL;DR: The paper presents a differentiable framework for optimizing mutual information over linear Gaussian DAGs using a K-recursion to propagate covariances, enabling topology-agnostic gradient-based optimization under global constraints like power budgets.


<details>
  <summary>Details</summary>
Motivation: To enable end-to-end mutual information optimization in network-wide designs (MIMO, relays, RIS, etc.) under global constraints without requiring topology-specific closed-form gradient derivations.

Method: Uses a K-recursion that analytically propagates all node-pair covariances (including non-adjacent) along the DAG in topological order, yielding a closed-form log-determinant expression for MI. Complex-valued reverse-mode automatic differentiation returns Wirtinger gradients; projected gradient ascent performs constrained optimization.

Result: A single topology-agnostic implementation works across four diverse DAG classes (single-link MIMO, diamond DAG, two-hop AF relay, input-covariance shaping), matching the water-filling optimum where available and improving MI in non-single-link topologies. Also successfully applied to an 11-node, 5-layer Gaussian network.

Conclusion: The K-recursion enables a general, differentiable framework for MI optimization over any linear Gaussian DAG without topology-specific formulas, with broad applicability to multi-layer network designs.
...
### [97] [Environment-Division Multiple Access: an Enabler for AI-Native Multiple Access](https://arxiv.org/abs/2606.07025)
*Zhiguo Ding*

Main category: cs.IT

TL;DR: This paper introduces Environment-Division Multiple Access (EDMA) as a new multiple access technique leveraging wireless propagation environments, flexible antennas, and environment reconfiguration, while exploring its integration with AI-native communication networks through AI-assisted and AI-native approaches.


<details>
  <summary>Details</summary>
Motivation: To propose a novel multiple access technique (EDMA) that utilizes wireless propagation environment features and integrates with AI to improve communication network performance.

Method: The paper describes EDMA's key properties: utilizing wireless propagation environments, integrating advanced flexible antennas, and proactively reconfiguring propagation environments. It further categorizes AI applications into AI-assisted EDMA and AI-native EDMA.

Result: Presentation of EDMA as a new multiple access paradigm and two AI integration frameworks (AI-assisted and AI-native EDMA) with identified open problems.

Conclusion: EDMA shows promise for future AI-native communication networks, though several open problems remain for AI-assisted EDMA that warrant further research.
...
### [98] [Information-Theoretic Bounds for Sparse Covariance Estimation in the Vertical-Split Distributed Model](https://arxiv.org/abs/2606.07124)
*Jing Yee Tan,Guangyue Han*

Main category: cs.IT

TL;DR: This paper studies distributed covariance estimation in the vertical-split setting with sparse cross-covariance, proving that sparsity reduces communication and sample complexity—exponentially in the 1-sparse case—and providing matching lower and upper bounds.


<details>
  <summary>Details</summary>
Motivation: To investigate whether elementwise s-sparsity in cross-covariance matrices can reduce communication and sample complexity in the vertical-split distributed setting, contrasting with the horizontal-split case where sparsity does not help.

Method: Minimax lower bounds via Fano's method with sparse packing (Varshamov–Gilbert-type argument for signed partial permutation matrices) and Conditional Strong Data Processing Inequality; upper bounds via covering-net quantization and entry-wise hard thresholding.

Result: Communication budget per agent scales as Ω(σ⁴ d_k s' log(d₁d₂/s')/ε²) and sample complexity as Ω(σ⁴ s' log(d₁d₂/s')/ε²). For 1-sparse case, exponential improvement from d₁d₂ to log(d₁d₂) over dense rate. Upper bounds match lower bounds up to polylogarithmic factors.

Conclusion: Sparsity helps reduce communication and sample complexity for cross-covariance estimation in the vertical-split setting, unlike in the horizontal-split setting for mean estimation.
...
### [99] [The Synthesis-Sequencing Channel for DNA-based Data Storage](https://arxiv.org/abs/2606.07216)
*Keshav Goyal,Samuel Pearson,João Ribeiro,Serge Kas Hanna*

Main category: cs.IT

TL;DR: The paper introduces a synthesis-sequencing channel model for DNA-based data storage that captures both synthesis and sequencing effects, and establishes its information-theoretic capacity with matching bounds under binary symmetric channel error models.


<details>
  <summary>Details</summary>
Motivation: Prior DNA storage models oversimplify the process by assuming independent errors across reads and ignoring the distinction between physical coverage (after synthesis) and sequencing coverage (after readout). A more realistic model is needed to capture coverage bias and the two-stage nature of DNA storage.

Method: The authors model DNA storage as a two-stage channel (synthesis followed by sequencing), where both stages are binary symmetric channels with possibly different error probabilities. They derive matching converse and achievability bounds under mild assumptions on channel parameters.

Result: The information-theoretic capacity of the synthesis-sequencing channel is established. The results reveal multiple trade-offs between physical coverage, synthesis errors, sequencing coverage, and sequencing errors that affect the maximum achievable rate for reliable storage.

Conclusion: The synthesis-sequencing channel provides a more nuanced model for DNA storage, and its capacity analysis yields insights into the fundamental limits and trade-offs governing reliable DNA-based data storage.
...
### [100] [The Capacity of Information-Theoretic Secure Aggregation in Federated Learning](https://arxiv.org/abs/2606.07277)
*Lanxin Yi,Jinbao Zhu,Kai Wan,Xiaohu Tang*

Main category: cs.IT

TL;DR: This paper characterizes the fundamental capacity region for T-colluding information-theoretic secure aggregation under a general two-phase framework (key distribution + update aggregation), eliminating the need for a trusted third party or prescribed key structures. It achieves a deterministic capacity-achieving scheme over finite fields of size ≥ N, using only pairwise shared keys, and matches the information-theoretic converse.


<details>
  <summary>Details</summary>
Motivation: Prior work on secure aggregation often assumes correlated random keys from a trusted third party (TTP) or via prescribed groupwise structures, ignoring the communication cost of key distribution. This leaves fundamental limits under general key-distribution mechanisms unknown.

Method: The authors model key distribution through user-to-user communication with arbitrary user-generated key-distribution mechanisms. They jointly characterize three resources (randomness, key-distribution communication, aggregation communication) and develop an explicit deterministic capacity-achieving scheme over finite fields of size at least N, with a matching information-theoretic converse.

Result: Complete characterization of the capacity region among randomness, key-distribution communication, and aggregation communication. The proposed scheme is deterministic over finite fields of size ≥ N (vs. randomized/existential constructions), requires fewer random masking keys than Google's scheme, and can be implemented via Diffie-Hellman key exchange using only pairwise shared keys.

Conclusion: The work establishes fundamental limits for T-colluding secure aggregation without TTP, showing optimal performance is achievable with pairwise shared keys and less randomness than prior art, enabling practical implementation.
...
### [101] [Letting Homogeneity Entropy Select S-Pairs in Buchberger's Algorithm](https://arxiv.org/abs/2606.07321)
*Uzma Shafiq,Matthew England,AmirHosein Sadeghimanesh,Nayyar Zaidi*

Main category: cs.IT

TL;DR: The paper introduces Homogeneity Entropy, an information-theoretic S-pair selection strategy for Buchberger's algorithm that outperforms classical methods on random polynomial systems but underperforms on real-world problems, suggesting dataset-dependent optimal strategies.


<details>
  <summary>Details</summary>
Motivation: To improve Buchberger's algorithm for computing Gröbner bases by finding a better S-pair selection strategy, moving beyond classical heuristics and recent ML approaches to use information theory.

Method: Proposes Homogeneity Entropy, an information-theoretic measure based on the distribution of degrees among monomials in S-polynomials, implemented and tested on random polynomial systems and the PHCpack benchmark dataset.

Result: Homogeneity Entropy significantly outperforms classical strategies on random polynomial datasets, but classical strategies perform better on the PHCpack real-world dataset.

Conclusion: The optimal S-pair selection strategy is data-dependent; the proposed information-theoretic approach offers meaningful gains on certain distributions and is the first use of such guidance in symbolic computation optimization.
...
### [102] [Rate Loss in Quantum Channels with Classical State and Applications for Quantum Broadcast Channels](https://arxiv.org/abs/2606.07409)
*Igor Bernard,Arun Padakandla*

Main category: cs.IT

TL;DR: This paper identifies rate loss in non-commutative classical-quantum channels due to lack of receiver channel state information, and proves coset codes outperform unstructured IID codes for 3-user CQ broadcast channels.


<details>
  <summary>Details</summary>
Motivation: To study the rate loss penalty in classical-quantum channels caused by missing channel state information at the receiver, and to determine if structured coding (coset codes) can improve performance over conventional unstructured codes.

Method: Analytical proof of rate loss for non-commutative CQ channels, followed by comparative analysis showing coset-code-based strategies outperform IID-code-based strategies for 3-user CQ broadcast channels.

Result: Rate loss is proven for non-commutative CQ channels. Coset-code strategies achieve strictly better rates than unstructured IID-code strategies for 3-user non-commutative CQ broadcast channels.

Conclusion: Structured coding (coset codes) can mitigate rate loss in non-commutative CQ channels and outperform traditional unstructured coding approaches in multi-user settings.
...
### [103] [Sort, Partition, Randomize: Optimal Binary Hypothesis Testing under Local Differential Privacy](https://arxiv.org/abs/2606.07443)
*Elena Ghazi,Jawad Nasser,Flavio Calmon,Ibrahim Issa*

Main category: cs.IT

TL;DR: This paper studies optimal locally differentially private mechanisms for binary hypothesis testing, proving that optimal mechanisms have a "Sort-Partition-Randomize" (SPR) structure: sort alphabet by likelihood ratio, partition into contiguous blocks, and apply randomized response to block labels. This enables an exact dynamic programming algorithm in O(k^3) time, making exact optimal mechanism computation feasible across the full privacy range.


<details>
  <summary>Details</summary>
Motivation: Previous work established structural properties of optimal ε-LDP mechanisms for binary hypothesis testing but only yielded exponential-time algorithms, making them impractical. This paper aims to find a more efficient, exact characterization that enables polynomial-time computation of optimal mechanisms.

Method: The authors prove that for every ε and f-divergence objective, after sorting the alphabet by likelihood ratio, there exists an optimal mechanism that partitions the sorted alphabet into contiguous blocks and applies randomized response to the block label (SPR class). They then derive an exact dynamic programming algorithm that computes the optimal mechanism in O(k^3) time, or O(ℓ k^2) time with an ℓ-output budget.

Result: The SPR structural characterization is sharp. The dynamic program computes the exact optimal mechanism in O(k^3) time, a dramatic improvement over previous exponential-time algorithms. The approach works across the full privacy range (not just asymptotic regimes) and generalizes to ℓ-output budget scenarios.

Conclusion: The paper makes it possible to efficiently compute and characterize the exact optimal locally private mechanism for binary hypothesis testing across all privacy levels, bridging the gap between theoretical structural properties and practical computability.
...
### [104] [Which Anatomy Matters Under Limited Labels? A Data-Efficient Anatomy-Aware Benchmark for Cardiac Pathology Prediction](https://arxiv.org/abs/2606.06509)
*Himanshu Singh*

Main category: eess.IV

TL;DR: This paper investigates whether performance gains in low-label medical imaging tasks come from more expressive models or better anatomical representation, finding that representation quality matters more than model complexity in resource-constrained settings.


<details>
  <summary>Details</summary>
Motivation: To understand whether performance gains in low-data medical imaging are driven by model expressivity or by improved representation of clinically meaningful anatomy, especially under limited labels and compute constraints.

Method: Created a low-data anatomy-aware benchmark for 5-class cardiac pathology prediction using the ACDC MRI dataset. Compared anatomy-specific and multi-structure representations from segmentation-derived descriptors of right ventricle, myocardium, and left ventricle across linear, kernel, and tree-based classifiers.

Result: Under limited label settings, representation quality dominates model complexity. Better anatomical representation yields greater performance gains than using more complex models.

Conclusion: In resource-constrained healthcare settings, identifying and representing the most informative anatomy is more important than increasing model complexity alone.
...
### [105] [Dilated Symmetric Difference for Binary Image Comparison](https://arxiv.org/abs/2606.06512)
*Sharon Urieli*

Main category: eess.IV

TL;DR: This paper introduces a morphological operator called dilated symmetric difference for comparing binary images, showing it effectively detects differences when alignment errors remain within defined bounds.


<details>
  <summary>Details</summary>
Motivation: To address the problem of comparing binary images under residual alignment errors by developing a mathematically grounded approach using morphological operators.

Method: The method formulates binary image comparison using mathematical morphology and introduces a new operator called the dilated symmetric difference.

Result: The dilated symmetric difference operator effectively detects differences between binary images when the residual alignment error is within specified bounds.

Conclusion: Mathematical morphology provides a robust framework for binary image comparison, and the dilated symmetric difference is a useful operator for this task under controlled alignment conditions.
...
### [106] [Advanced Flood Prediction with Physics-Guided Deep Learning: Combining UNet, FNO, and SAR/Optical Imagery](https://arxiv.org/abs/2606.06524)
*Tewodros Syum Gebre,Jagrati Talreja,Leila Hashemi-Beni*

Main category: eess.IV

TL;DR: This paper proposes a physics-guided deep learning framework combining multi-modal remote sensing data with shallow water equation constraints for accurate, scalable flood mapping. The hybrid UNet+FNO architecture achieves high accuracy (IoU 0.82, F1 0.90, water depth RMSE 0.21m) while maintaining physical consistency with mass imbalance below 2.1%.


<details>
  <summary>Details</summary>
Motivation: Accurate and scalable flood mapping is challenging due to limited ground observations, heterogeneous terrain conditions, and difficulty enforcing hydrodynamic consistency in data-driven models.

Method: A hybrid architecture combining UNet for fine-scale spatial details with Fourier Neural Operator (FNO) for basin-scale hydraulic interactions, integrated with multi-modal remote sensing data (Sentinel-1 SAR, Sentinel-2 optical, DEM) and physics-informed residual losses from shallow water equations (SWE).

Result: IoU 0.82, F1 0.90 for flood extent; RMSE 0.21m for water depth, 0.15 m/s for flow velocity; mass imbalance below 2.1%; ablation studies confirm physics constraints essential for performance.

Conclusion: Embedding hydrodynamic principles into deep learning yields more accurate, reliable, and physically coherent flood predictions, with strong potential for operational monitoring and large-scale deployment.
...
### [107] [Attention Consistent Longitudinal Medical Visual Question Answering Guided by Vision Foundation Models](https://arxiv.org/abs/2606.06534)
*Jialin Wu,Qianru Zhang,Georges El Fakhri,Xiaofeng Liu*

Main category: eess.IV

TL;DR: The paper proposes an attention-guided encoder-decoder model with affine registration and DINO-based adaptive masking for longitudinal medical VQA on chest X-rays, achieving strong results on Medical-Diff-VQA benchmark with auxiliary objectives for stabilization and interpretability.


<details>
  <summary>Details</summary>
Motivation: Longitudinal medical VQA requires reasoning about anatomical changes between current and reference time point images, which is challenging due to motion artifacts and the need to isolate clinically meaningful differences.

Method: (1) Lightweight affine registration module to co-register current and reference images; (2) Frozen DINO-based mask generator + trainable adaptive mask generator; (3) Masked images fed into encoder + multimodal transformer decoder; (4) Auxiliary losses: mask rebuilding, Gram-style consistency, KoLeo uniformity loss.

Result: Strong BLEU, ROUGE-L, CIDEr, and METEOR scores on the Medical-Diff-VQA benchmark, with intrinsic interpretability via shared saliency masks.

Conclusion: Saliency-conditioned generation with mild pre-alignment is a principled framework for longitudinal reasoning in medical VQA, and combining supervised &amp; unsupervised objectives shows potential for using image foundation models in biomedicine.
...
### [108] [ErA: Error-Aware Deep Unrolling Network for Single Image Defocus Deblurring](https://arxiv.org/abs/2606.06540)
*Tu Vo,Chan Y. Park*

Main category: eess.IV

TL;DR: ErA is an end-to-end deep unrolling network for single-image defocus deblurring that jointly learns a compact kernel basis and per-pixel weights, using an error-aware term in Augmented Lagrangian unrolling to correct kernel estimation errors, achieving state-of-the-art performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the challenging problem of single-image defocus deblurring by correcting kernel estimation errors through an error-aware mechanism in a deep unrolling framework.

Method: ErA jointly learns a compact kernel basis and per-pixel weights, incorporating an error-aware term in Augmented Lagrangian unrolling with alternating updates and ResUNet denoisers to correct kernel estimation errors.

Result: Achieves state-of-the-art PSNR/SSIM on DPDD, RealDOF, and RTF datasets, and shows strong generalization on CUHK without ground truth.

Conclusion: The error-aware deep unrolling approach effectively handles defocus deblurring by correcting kernel estimation errors, outperforming existing methods.
...
### [109] [Compute-Optimal Network Design for Echocardiography Myocardial Segmentation and Perfusion Quantification using Neural Scaling Laws](https://arxiv.org/abs/2606.06725)
...
TL;DR: This paper applies neural scaling laws to optimize model size for myocardial segmentation in contrast-enhanced ultrasound, achieving state-of-the-art performance with 240× fewer parameters and clinically equivalent perfusion quantification.


<details>
  <summary>Details</summary>
Motivation: Clinical adoption of myocardial perfusion quantification using CEUS is limited by time-consuming manual labelling and lack of in-domain training data for automated segmentation.

Method: Extrapolate performance on data subsets using neural scaling laws (adapted from LLM optimization) to determine optimal network size; validated on CAMUS and 25-patient CEUS datasets.

Result: Scaling law extrapolation predicted test loss accurately; selected networks achieved SOTA on CAMUS with 240× fewer parameters; perfusion parameters matched senior cardiologist performance.

Conclusion: Neural scaling laws are practical for data-driven compute-optimal model design even for small imaging datasets.
...
### [110] [Physics-Driven Semantic Scattering Structure Understanding of Aircraft Target in SAR Images](https://arxiv.org/abs/2606.06847)
*Yifei Yin,Xiaogang Yu,Hao Shi,Liang Chen,Wei Li*

Main category: eess.IV

TL;DR: The paper proposes S3U-SAR, a physics-driven framework for SAR aircraft target interpretation that uses semantic scattering keypoints and visibility-aware attributes to construct complete topological structures, outperforming existing local scattering center methods.


<details>
  <summary>Details</summary>
Motivation: Existing SAR target interpretation methods rely on local scattering center representations that are unordered and component-agnostic, causing physically existing but weakly scattering components to be missed, leading to incomplete reconstructed topology structures for aircraft targets.

Method: The authors establish a Semantic Scattering Structure Understanding paradigm, defining semantic scattering keypoints linked to aircraft components with visibility-aware attributes. They build S3U-SAR, a physics-driven framework incorporating multi-dimensional physical priors (scattering heterogeneity, rigid-body topology, speckle uncertainty) and a confidence-gated joint supervision strategy.

Result: S3U-SAR achieves the best performance compared with baselines on the newly constructed KP-SAR-Aircraft-1.0 benchmark. Cross-category and cross-dataset evaluations demonstrate robustness and transferability.

Conclusion: Moving beyond local scattering center representations to semantic scattering structure understanding significantly improves SAR aircraft interpretation completeness and stability, with strong generalization capability.
...
### [111] [A 3D Formulation of the Extended Phaseless Rytov Approximation](https://arxiv.org/abs/2606.06933)
*Wanqin Ma,Zan Li,Amartansh Dubey,Alikhan Umirbayev,Yijun Chen,Junhui Rao,Ross Murch*

Main category: eess.IV

TL;DR: This paper extends the phaseless Rytov approximation (xPRA) from 2D to 3D, creating x3DPRA for device-free RF imaging that uses only RSS measurements, preserving simplicity while enabling volumetric imaging.


<details>
  <summary>Details</summary>
Motivation: Existing phaseless RF imaging methods (xPRA, RTI) are limited to 2D, but next-generation wireless networks (ISAC) would benefit from 3D/volumetric imaging capabilities without requiring complex hardware or wide bandwidths.

Method: The authors formulate a 3D extension of the phaseless Rytov approximation (x3DPRA), maintaining the linear phaseless formulation that works with RSS data directly, enabling straightforward implementation on existing wireless infrastructure.

Result: Simulation results show x3DPRA provides good estimates of object location, shape, and can reconstruct material attenuation properties. Validation includes comparison with 2D models.

Conclusion: x3DPRA successfully extends device-free RF imaging to 3D while preserving the practical advantages of xPRA and RTI, making it promising for ISAC systems in next-generation wireless networks.
...
### [112] [DaX: Learning General Pathology Representations Across Scales](https://arxiv.org/abs/2606.06983)
*Bokai Zhao,Yiyang Zhang,Long Bai,Tai Ma,Hanqing Chao,Minfeng Xu*

Main category: eess.IV

TL;DR: DaX is a pathology vision foundation model adapting DINOv3-style self-supervised learning to whole-slide histopathology, achieving top performance across a benchmark of 161 clinical tasks spanning diagnostic pathology, biomarker profiling, tissue context, and prognosis.


<details>
  <summary>Details</summary>
Motivation: Computational pathology requires visual representations that transfer across diverse clinical endpoints and remain robust to variations in magnification, staining, scanner type, slide preparation, and input resolution. Existing models lack such robustness and transferability.

Method: DaX adapts DINOv3-style self-supervised learning to histopathology by initializing from natural-image DINOv3 weights and incorporating continuous magnification training, cross-scale tissue views, orientation-agnostic/augmentation-robust training, multi-input-size training, and Gram-anchored dense consistency to connect local cellular morphology with global tissue architecture.

Result: Across a benchmark of 161 clinically meaningful tasks from 44 public datasets (28,182 patients, 34,394 slides), DaX achieves the highest mean performance and strongest task-level ranking scores across diagnostic pathology, biomarker/molecular profiling, tissue/specimen context, and risk/response/prognosis domains.

Conclusion: DaX serves as a transferable visual encoder for computational pathology, and the study provides a standardized evaluation framework for future pathology foundation models.
...
### [113] [Beyond Universality: The GCC-FER Dataset and Culture-Aware Adaptation for Dynamic Facial Expression Recognition](https://arxiv.org/abs/2606.07063)
*Sonalika Singh,Jyotirindra Dandapat,Avishi Razdan,Kshipra V. Moghe,Puneet Gupta,Lalan Kumar*

Main category: eess.IV

TL;DR: This paper introduces GCC-FER, the first large-scale global cross-cultural Dynamic Facial Expression Recognition dataset with 23,934 videos across four cultural groups, and proposes a Culture-Aware FER (CA-FER) system that uses cultural priors to adaptively recalibrate facial representations, mitigating cultural bias in FER.


<details>
  <summary>Details</summary>
Motivation: Existing DFER systems assume universal emotional expressions across cultures, neglecting cultural nuances in facial muscle activation patterns. There is a severe scarcity of culturally diverse benchmark datasets, particularly for underrepresented populations.

Method: The authors constructed GCC-FER (23,934 videos, 4 cultural groups, 7 expressions) via in-house collection for underrepresented groups and rigorous ethnicity filtering of existing sources. They derived cultural priors and a global prior from the dataset, then proposed CA-FER, which adaptively recalibrates latent facial representations to reduce cultural bias.

Result: Extensive experiments on GCC-FER and DFEW show that the proposed CA-FER system consistently improves FER performance across multicultural settings compared to baselines.

Conclusion: The GCC-FER dataset and CA-FER system effectively address cultural bias in DFER, highlighting the importance of culturally diverse data and adaptive bias mitigation strategies for fairer and more robust affective computing.
...
### [114] [Impact of Synthetic Lesional MR Images in Automated Focal Cortical Dysplasia Detection in Low-Data Scenarios](https://arxiv.org/abs/2606.07381)
*Prabhjot Kaur,Hakim Ouaalam,Sedat Kandemirli,Sanjay P. Prabhu,Simon K. Warfield*

Main category: eess.IV

TL;DR: This study demonstrates that conditional generative networks can create realistic synthetic FCD MRI scans that fool expert radiologists (60-70% accuracy), and using these synthetic images for training reduces the need for manually annotated data by ~20% while maintaining equivalent detection sensitivity, though real data remains more effective when available.


<details>
  <summary>Details</summary>
Motivation: Automated detection of focal cortical dysplasia (FCD) requires large volumes of voxelwise lesion-delineated MRI data, which are difficult to acquire. The authors aim to generate synthetic MRI data exhibiting FCD to reduce the need for manual annotations.

Method: T1w and FLAIR MRI scans from 131 FCD patients and 90 healthy controls were retrospectively studied. Synthetic MRIs were generated by conditioning a generative network on binary FCD masks. Two neuroradiologists evaluated realism. Three nnU-Net models were trained: (i) real-only (35 FCD/35 controls), (ii) real + synthetic augmentation, and (iii) expanded real data (70 FCD/70 controls).

Result: Experts classified real vs. synthetic with 60% (T1w) and 70% (FLAIR) accuracy (kappa=0.86). Synthetic augmentation increased sensitivity by 8.14% (p=0.12) and improved model confidence at true lesion sites from 0.83±0.11 to 0.89±0.12 (p=0.02). Expanded real-data model achieved 73.8% sensitivity (p<0.001) and confidence of 0.90±0.14 (p=0.01).

Conclusion: Conditional generative networks can generate realistic synthetic FCD-MRIs, reducing labeled data needs by approximately 20% while maintaining equivalent sensitivity. However, equivalent amounts of real data remain more effective than synthetic augmentation.
...
### [115] [SEMIKHORN: Globally balanced affinities for mmWave Localization in MU mMIMO systems](https://arxiv.org/abs/2606.06640)
*Abhisha Garg,Raghav Shukla,Suraj Srivastava,Aditya K. Jagannatham*

Main category: eess.SP

TL;DR: SEMIKHORN is a semi-supervised channel charting framework for mmWave localization that uses t-SNEkhorn (a doubly stochastic variant of t-SNE with entropic optimal transport) to fuse dissimilarity matrices from distributed multi-antenna base stations, achieving 6.86% mean localization error with under 15% labeled CSI samples.


<details>
  <summary>Details</summary>
Motivation: To enable accurate mmWave localization in wireless networks with limited labeled data by improving upon standard t-SNE-based channel charting through globally balanced similarity construction.

Method: Proposes SEMIKHORN framework using t-SNEkhorn (entropic optimal transport-based doubly stochastic t-SNE), fuses local dissimilarity matrices from distributed BSs into a global matrix, applies manifold learning, and optimizes hyperparameters via Bayesian optimization.

Result: Achieves 6.86% mean localization error in a 100m radius circular area using less than 15% labeled CSI samples.

Conclusion: t-SNEkhorn-based semi-supervised channel charting effectively reduces labeling requirements while maintaining high localization accuracy for mmWave systems.
...
### [116] [Variational Bayes Estimation for Affine-Precoded Superimposed Pilots in Partially Connected Dual-Wideband Tera-Hertz MU-MIMO Systems](https://arxiv.org/abs/2606.06672)
*Abhisha Garg,Suraj Srivastava,Aditya K. Jagannatham*

Main category: eess.SP

TL;DR: This paper proposes two affine precoding systems for THz MIMO using superimposed pilots, models a dual-wideband partially connected channel with absorption/reflection/freespace losses, and develops a variational Bayesian inference algorithm for joint channel estimation and sparsity learning.


<details>
  <summary>Details</summary>
Motivation: To reduce the significant bandwidth overhead of conventional pilot-based channel estimation in terahertz MIMO systems, while handling severe model uncertainty and the dual-wideband effects in partially connected architectures.

Method: Two affine precoding systems (CP-JCE and USP-DCE) are proposed. A structured sparse channel model is formulated for THz MIMO with dual-wideband effects. A variational Bayesian inference algorithm jointly estimates channel coefficients and learns sparsity structure via hyperparameter inference using superimposed pilots.

Result: The proposed variational Bayesian algorithm enables robust, high-precision superimposed pilot-based channel estimation under severe model uncertainty. A trade-off analysis between the two precoding systems is provided.

Conclusion: The work provides effective channel estimation solutions for THz MIMO systems with reduced pilot overhead, and offers insights into the trade-offs between common precoding with joint estimation and user-specific precoding with decoupled estimation.
...
### [117] [Deep Learning Based Sparse Array Design with Pre-Steering for Adaptive Beamforming](https://arxiv.org/abs/2606.06723)
*Ian Straub,Syed A Hamza*

Main category: eess.SP

TL;DR: This paper proposes a CNN-based deep learning method for learning sparse array configurations that achieve near-optimal beamforming under dynamic source and interference angles. It introduces pre-steering and robust error-augmented training to avoid retraining, achieving over 90% accuracy and high SINR.


<details>
  <summary>Details</summary>
Motivation: To enable rapid reconfiguration of sparse arrays in highly dynamic propagation environments, overcoming limitations of conventional and convex optimization approaches that are computationally expensive for dynamic settings.

Method: A CNN-based deep learning approach with an array pre-steering strategy (training only at broadside, pre-steering test inputs) and robust error-augmented training that incorporates small, structured pre-steering perturbations to handle angular uncertainties.

Result: Achieves over 90% test accuracy across wide ranges of source and interference angles, maximizing SINR even under angular uncertainty, and maintaining high classification accuracy with pre-steering errors.

Conclusion: The proposed deep learning method shows strong potential for real-time, robust sparse array configuration in dynamic environments, effectively balancing accuracy and computational efficiency.
...
### [118] [Angular Sector-Based Sparse Array Design for Adaptive Beamforming Using Deep Learning](https://arxiv.org/abs/2606.06732)
*John Kobak,Ethan Atiyeh,Syed A Hamza*

Main category: eess.SP

TL;DR: This paper proposes a framework for sparse array reconfiguration using angular-sector-based class reduction and CNN/ResNet classifiers to achieve near-optimal SINR beamforming with minimal reconfiguration overhead in dynamic RF environments.


<details>
  <summary>Details</summary>
Motivation: Efficient sparse array reconfigurability is essential for cognitive sensing in dynamic RF environments where rapid interference variations require both adaptability and stability.

Method: Full data correlation matrices are computed, an angular-sector-based class reduction merges adjacent sectors into 56 classes, and four dataset variants (high/low sample count, balanced/unbalanced) are used to train a lightweight CNN and ResNet 50.

Result: ResNet 50 achieves up to 97.3% classification accuracy; SINR deviations remain below 1% for most classes and below 5% even for challenging interference angles near broadside.

Conclusion: The approach enables robust sparse array selection, maintains strong SINR performance, reduces unnecessary reconfigurations, and provides an effective framework for real-time cognitive sensing and adaptive interference mitigation.
...
### [119] [Copula Function Parameter Regions in Analyzing Wireless Communications Performances](https://arxiv.org/abs/2606.06792)
*Mona Mohsenzadeh,Saeid Pakravan,Ghosheh Abed Hodtani*

Main category: eess.SP

TL;DR: This paper introduces copula dependence parameter regions for wireless communications, deriving explicit parameter bounds for a two-user MAC with FGM copula and showing that communication constraints significantly narrow theoretically admissible intervals.


<details>
  <summary>Details</summary>
Motivation: Existing copula-based wireless analyses express performance metrics in terms of dependence parameters but fail to characterize their admissible (feasible) regions from a practical communication perspective.

Method: The authors analyze a two-user MAC with correlated Rayleigh fading modeled by bivariate FGM copula, deriving parameter regions from both communication-theoretic (outage probability) and probabilistic (Pearson correlation coefficient) constraints.

Result: Practical communication and statistical requirements significantly shrink the classical copula admissible interval, rendering some theoretically admissible dependence structures practically infeasible.

Conclusion: The concept of copula dependence parameter regions is crucial for realistic wireless system analysis, as theoretical admissibility alone is insufficient without considering communication constraints.
...
### [120] [Variable-Length Finite-Rate CSI Feedback With Generative Priors](https://arxiv.org/abs/2606.06846)
*Yangxuan Cheng,Fanyang Meng,Jian Zou,Jiacheng Xie,Zhongqiang Zhang,Ye Wang,Yongsheng Liang*

Main category: eess.SP

TL;DR: CsiCoGen is a novel generative feedback structure for variable-length finite-rate CSI feedback that uses a transferable codebook without joint training, enabling flexible sequence length and quantization precision, achieving -31 dB indoor and -20 dB outdoor NMSE in high-rate regimes.


<details>
  <summary>Details</summary>
Motivation: To enable flexible, variable-length finite-rate CSI feedback without requiring joint training of task-specific encoders and codebooks with the reconstructor.

Method: Proposes CsiCoGen, a generative feedback structure where the UE maps H0 into ordered codebook indices and the BS recursively recovers CSI from partial sequences using a shared denoising prior, instantiated with a diffusion model decoder.

Result: Achieves approximately -31 dB indoor NMSE and -20 dB outdoor NMSE in the high-rate regime, with favorable rate-NMSE and rate-ρ tradeoffs against baselines, along with scalable decoding complexity and adjustable per-step quantization precision.

Conclusion: CsiCoGen provides a flexible, training-efficient framework for variable-length CSI feedback that can pair with different pretrained denoisers without joint training, demonstrating strong performance on COST2100 data.
...
### [121] [Learn to Access and Backhaul the Sky: Multi-Scale Radio Map Guided Multi-UAV Cooperation](https://arxiv.org/abs/2606.06954)
*Yifeng Yuan,Shijian Gao*

Main category: eess.SP

TL;DR: A Multi-Scale Radio Map-Guided (MRMG) framework using MARL is proposed to improve UAV swarm connectivity by jointly controlling movement, routing, and power across three map scales, nearly doubling the 5th-percentile user rate.


<details>
  <summary>Details</summary>
Motivation: UAV swarms in low-altitude economy face connectivity challenges due to dynamic user mobility, building blockages, and multi-dimensional joint control that traditional heuristics cannot handle effectively.

Method: MRMG framework integrates three scales of radio information (global, local, link-level maps) to decouple macro-movement from micro-link adaptation, and uses MARL to learn cooperative policies for UAV movement, next-hop selection, and transmit-power control.

Result: The MRMG framework improves network throughput and significantly bolsters cell-edge service, nearly doubling the 5th-percentile user rate.

Conclusion: The multi-scale radio map design effectively handles heterogeneous dynamics in UAV swarm networks, and MARL-based joint control yields superior long-term performance over traditional heuristics.
...
### [122] [A Novel Stripe-based RIS Optimization for UAV Communications and Sensing in Low-Altitude Wireless Networks](https://arxiv.org/abs/2606.07026)
*Burak Ahmet Celebi,Sefa Kayraklik,Onur Salan,Ibrahim Hokelek,Ali Emre Pusane,Ali Gorcin*

Main category: eess.SP

TL;DR: This paper proposes a low-complexity stripe-based RIS phase shift optimization framework for RIS-assisted LAWN to enable reliable UAV communication and passive sensing under 3D mobility, validated through simulations and real outdoor experiments.


<details>
  <summary>Details</summary>
Motivation: To establish reliable communication with UAVs in low-altitude wireless networks under varying channel conditions and signal blockages, while achieving low computational complexity for real-time RIS reconfiguration.

Method: A stripe-based RIS phase shift optimization framework that leverages phase-gradient structural properties of adjacent RIS elements to reduce search space, enabling simultaneous communication enhancement and passive UAV tracking.

Result: Outperforms conventional benchmarks in convergence speed and computational efficiency; maintains robust high-SNR connectivity even with phase estimation errors and in low SNR regimes; validated through real RIS prototype experiments in outdoor campus environment.

Conclusion: The proposed stripe-based RIS optimization framework is a practical and efficient solution for RIS-assisted LAWN, offering both reliable communication and passive sensing capabilities for UAV tracking.
...
### [123] [Geometric Time-Domain Identification of Three-Phase Load Equivalents from Terminal Measurements](https://arxiv.org/abs/2606.07048)
*Francisco G. Montoya,Francisco de Leon,Francisco M. Arrabal-Campos,Alfredo Alcayde*

Main category: eess.SP

TL;DR: This paper presents a geometric time-domain method for identifying three-phase load equivalents from instantaneous voltage and current measurements, focusing on three-wire systems. It extends prior single-phase work with a sampled-data implementation using time windows, normalized matrix equations, and identifiability tests. The method reports low-confidence evidence for insufficient data and accurately identifies informative trajectories in simulations.


<details>
  <summary>Details</summary>
Motivation: To develop a method for identifying three-phase load equivalents from instantaneous measurements that handles three-wire systems where no neutral voltage is measured and Kirchhoff constraints must be satisfied, and to avoid forcing models when data lacks information.

Method: The method interprets measured waveforms as trajectories in Euclidean signal spaces, recovers load-equivalent parameters from geometry. It uses sampled-data implementation with local time windows, normalized matrix equations, harmonic-projection derivative and primitive coordinates, explicit geometric identifiability tests, passivity constraints, and energy/Kirchhoff residuals.

Result: Numerical simulations show the method accurately identifies informative three-phase trajectories and exposes structurally degenerate cases like pure single-frequency excitation for higher-order three-wire models. It reports low-rank or ill-conditioned windows as low-confidence evidence rather than forcing a model.

Conclusion: For a given admissible topology, the identified circuit closes the instantaneous terminal energy balance of the measured load over the analysis window.
...
### [124] [Optimized Sampling of Angle-Resolved Scatterometry Data Using End-to-End Compressed Learning Model for Nanograss Deficiency Detection](https://arxiv.org/abs/2606.07050)
*Mehdi Abdollahpour,Carsten Bockelmann,Armin Dekorsy*

Main category: eess.SP

TL;DR: This paper proposes a compressed learning framework combining a learnable latitude-based sampling layer with CNN for vacancy deficiency detection in zinc oxide nanograss using angle-resolved scatterometry, achieving 94.2% accuracy with full images and matching performance using up to 90% fewer sampling points.


<details>
  <summary>Details</summary>
Motivation: Angle-resolved scatterometry is effective for nanosurface inspection but suffers from long acquisition times due to dense angular sampling. The researchers wanted to reduce sampling points without sacrificing classification accuracy for vacancy deficiency detection.

Method: An end-to-end compressed learning framework integrating a learnable latitude-based sampling layer with a convolutional neural network. The sampling layer exploits ARS pattern structure by learning informative latitudinal regions. GAN-based data augmentation is also used for pretraining.

Result: 94.2% accuracy for five-level deficiency classification, 98.6% accuracy for binary deficient/non-deficient classification. Matches full-image performance with 90% fewer sampling points. Even at 99.7% reduction, accuracy drops less than 10 percentage points.

Conclusion: The proposed compressed learning approach significantly reduces data acquisition time while maintaining high classification accuracy, making it suitable for in-line nanosurface inspection.
...
### [125] [Branch-Level Energy Localization in Three-Phase Loads: Resolving Indeterminacy in Time-Domain](https://arxiv.org/abs/2606.07076)
*Francisco G. Montoya,Francisco de Leon,Francisco M. Arrabal-Campos,Alfredo Alcayde*

Main category: eess.SP

TL;DR: A branch-level energy-localization framework for three-phase loads uniquely decomposes instantaneous power into Joule dissipation and stored-energy rates per branch, while revealing that multiple topologies can produce identical terminal data with different internal energy distributions.


<details>
  <summary>Details</summary>
Motivation: To resolve ambiguities and paradoxes in power decomposition for three-phase systems (e.g., the de Leon–Cohen open-phase paradox) by developing a rigorous branch-level energy localization framework.

Method: The paper proposes an admissible lumped-equivalent circuit topology and proves three theorems: Branch-Level Localization (uniqueness given topology), Topology-Indeterminacy (multiple topologies yield same terminal data), and Generalized Energetic Duality (classical dualities as special cases of a time-domain principle with time-varying parameters).

Result: The framework is validated on six test cases: de Leon–Cohen paradox, switched-resistive loads, delta-vs-wye indeterminacy, fluctuating-phase loads, and a four-wire nonlinear load with hysteretic, linear, and switched branches.

Conclusion: The framework complements existing standards (IEEE Std. 1459, CPC, p-q, FBD) by addressing a different question; apparent paradoxes resolve when the question is precisely formulated.
...
### [126] [Rate-Splitting--Inspired Uplink Near-Field ISAC](https://arxiv.org/abs/2606.07091)
*Anup Mishra,Israel Leyva-Mayorga,Petar Popovski*

Main category: eess.SP

TL;DR: This paper proposes a rate-splitting (RS)-inspired framework for uplink near-field ISAC, deriving closed-form rate expressions and showing that the RS-inspired approach can match or outperform NOMA-inspired schemes, with analysis of residual sensing interference and large-array limits.


<details>
  <summary>Details</summary>
Motivation: To address the fundamental receiver-design challenge in uplink ISAC caused by inter-functionality interference between sensing and communication, and to overcome limitations of existing NOMA-inspired frameworks.

Method: Develops an RS-inspired framework that generalizes sensing-centric and communication-centric NOMA endpoints by splitting communication messages across sensing operations; derives closed-form CR and SR expressions accounting for residual sensing interference; analyzes high-SNR and large-array asymptotic behavior.

Result: The RS-inspired boundary contains the NOMA-inspired time-sharing region; split factor reshapes sensing-stage interference, enabling matching or strict enlargement of the S&C tradeoff; residual sensing interference changes rate offsets (non-aligned channels) or becomes slope-limiting (fully-aligned); achievable rates remain finite as array grows under physically consistent near-field modeling.

Conclusion: The RS-inspired framework provides superior S&C performance tradeoffs compared to NOMA-inspired approaches in uplink near-field ISAC, with bounded large-array behavior due to physically consistent modeling.
...
### [127] [Robust Secure Beamforming for Movable Antenna Enhanced Integrated Sensing and Communications](https://arxiv.org/abs/2606.07104)
*Yuan Chen,Ning Wei,Ahmad Bazzi,Xiangyu Dong,Ran Yang,You Li,Yue Xiu*

Main category: eess.SP

TL;DR: This paper proposes a robust beamforming and antenna placement optimization algorithm for MA-enhanced secure ISAC systems under imperfect eavesdropping CSI, using BCD with SCA and FP techniques to maximize radar SINR while ensuring communication security.


<details>
  <summary>Details</summary>
Motivation: To improve radar sensing performance in MA-enhanced ISAC systems while ensuring communication data security, despite challenges from nonlinear antenna position mapping and imperfect eavesdropper channel state information.

Method: Formulates a radar SINR maximization problem jointly optimizing transmit beamforming and antenna placement under security constraints. Solves the intractable non-convex problem using a BCD-based algorithm incorporating SCA and FP techniques.

Result: The proposed algorithm exhibits fast convergence and achieves significant improvement in radar SINR while guaranteeing communication security.

Conclusion: The BCD-based algorithm with SCA and FP effectively addresses the challenges of MA-enhanced secure ISAC systems, yielding superior radar performance without compromising security.
...
### [128] [RSMA Enabled Hierarchical UAV Networks with Non Linear Energy Harvesting: Outage Probability Analysis and UAV Placement Optimization](https://arxiv.org/abs/2606.07284)
*Faicel Khennoufa,Khelil Abdellatif,Metin Ozturk,Halim Yanikomeroglu,Safwan Alfattani*

Main category: eess.SP

TL;DR: The paper proposes a hierarchical ad hoc UAV network using non-linear energy harvesting and rate-splitting multiple access (RSMA) to improve energy and cost efficiency in 6G cellular networks. It derives outage probability expressions for ground IoT devices and UAVs under practical constraints like hardware impairments and imperfect channel state information, showing superior performance over benchmarks.


<details>
  <summary>Details</summary>
Motivation: Multi-UAV systems face reliability, sustainability, and scalability challenges due to limited onboard energy. Wireless energy harvesting is promising but terrestrial sources suffer path loss; harvesting from surrounding UAVs is more sustainable. RSMA remains underexplored in hierarchical UAV networks under practical impairments.

Method: Proposes a hierarchical ad hoc UAV network with non-linear EH and RSMA. Derives exact and approximate outage probability expressions over Nakagami-m fading channels under HWI, ICSI, and non-linear EH. Formulates two optimization problems to enhance reliability and performance.

Result: The proposed system outperforms all benchmarks in terms of outage probability. Exact and approximate outage expressions are derived for ground IoT devices, each CMU, and overall system.

Conclusion: The hierarchical UAV network with non-linear EH and RSMA is an effective solution for enhancing energy efficiency and reliability in 6G networks, particularly under realistic impairments.
...
### [129] [Implementation and Calibration of 3GPP-Compliant ISAC Channel Simulator](https://arxiv.org/abs/2606.07328)
*Chien-Han Wu,Ming-Chun Lee,Ta-Sung Lee*

Main category: eess.SP

TL;DR: This paper implements the 3GPP ISAC channel model simulator specified in TR 38.901, conducts calibration analysis against reference results, and releases the simulator as open-source to ensure reproducible and synchronized simulations for 6G ISAC system development.


<details>
  <summary>Details</summary>
Motivation: To support 6G ISAC system development by addressing the lack of explicit implementation details in 3GPP reports, which causes inconsistent simulation results across different implementations.

Method: Implemented the 3GPP ISAC channel model simulator specified in TR 38.901 and conducted comprehensive calibration analysis by comparing simulation results with reference results from companies in 3GPP.

Result: The simulator produces results consistent with 3GPP reference results; key implementation details are discussed; the simulator, datasets, and calibration results are released as open-source on GitHub.

Conclusion: The open-source simulator enables reproducible and synchronized evaluations for ISAC systems, bridging gaps in 3GPP documentation and facilitating further research.
...
### [130] [CSI Phase Averaging for High-Sensitivity Wi-Fi Sensing in Low-Multipath Environments](https://arxiv.org/abs/2606.07347)
*Toshinori Suzuki,Shin-ichiro Ogura,Yu Morishima,Hiroshi Matsuura*

Main category: eess.SP

TL;DR: This paper proposes a low-complexity motion detection method for outdoor Wi-Fi sensing using a model-driven approach that leverages CSI phase structural characteristics and phase averaging to mitigate device errors and noise, validated by detecting wild crows in orchards using commercial IEEE 802.11ac devices.


<details>
  <summary>Details</summary>
Motivation: To address the challenge that outdoor low-multipath environments are generally considered disadvantageous for Wi-Fi sensing, and to mitigate phase offset errors from wireless devices for reliable outdoor motion detection.

Method: A model-driven approach exploiting structural characteristics of CSI phase components for low-multipath environments, combined with phase averaging to reduce random noise (quantization and thermal noise).

Result: Successfully detected wild crows flying several meters away from the direct line-of-sight path; vegetation movement caused negligible fluctuations when wind speed < 3 m/s.

Conclusion: The method is effective for outdoor Wi-Fi sensing in low-multipath environments, applicable to orchard monitoring and other outdoor sensing applications.
...
### [131] [Beyond Backscatter: InSAR coherence from detected SAR images](https://arxiv.org/abs/2606.07374)
*Francescopaolo Sica,Andrea Pulella,Michael Schmitt*

Main category: eess.SP

TL;DR: A deep learning framework (Residual U-Net) regresses coherence directly from SAR images without requiring accurate coregistration, trained on Sentinel-1 data, achieving high-resolution results that generalize across locations and temporal baselines.


<details>
  <summary>Details</summary>
Motivation: To eliminate the need for accurate coregistration in SAR coherence estimation and enable large-scale coherence regression from globally available analysis-ready data (e.g., Google Earth Engine).

Method: A Residual U-Net is trained on precisely coregistered Sentinel-1 SLC data to learn mapping from backscatter magnitudes to coherence maps.

Result: Achieves high-resolution coherence regression with improved accuracy over intensity-based methods; generalizes well across diverse locations and unseen temporal baselines.

Conclusion: The proposed method enables large-scale coherence applications (mission design, change monitoring, mapping) using globally available analysis-ready data without coregistration.
...
### [132] [Amortized Neural Optimization for Pre-Layout Signal Integrity Design Space Exploration using Differentiable Surrogates](https://arxiv.org/abs/2606.07463)
*Julian Withöft,Werner John,Emre Ecik,Ralf Brüning,Jürgen Götze*

Main category: eess.SP

TL;DR: This paper proposes amortized neural optimization (ANO) for pre-layout signal integrity design, using fully differentiable neural network surrogates to eliminate iterative black-box search. ANO learns the optimization policy offline and maps channel contexts to near-optimal design parameters in a single forward pass, achieving 3-4 order speedups with roughly 10% optimality trade-off.


<details>
  <summary>Details</summary>
Motivation: Pre-layout design space exploration for high-speed signal integrity is limited by high computational cost of simulations and iterative optimization algorithms, making multi-corner sweeps prohibitively expensive.

Method: The authors propose amortized neural optimization (ANO) which uses fully differentiable neural network surrogate models to extract analytical gradients, enabling training of a global optimization policy offline. At inference, the policy directly maps channel contexts to near-optimal design parameters in a single deterministic forward pass, eliminating iterative black-box search entirely.

Result: ANO demonstrates speedups of three to four orders of magnitude across three complex SI scenarios (DDR5 DFE, 9D SerDes Tx/Rx co-equalization, DDR3 DQS routing), with roughly 10% optimality trade-off. For a 320,000-instance multi-corner SerDes sweep, it reduces days of computation to milliseconds.

Conclusion: Amortized neural optimization transforms computationally expensive SI optimization into real-time, interactive pre-layout DSE, enabling practical large-scale multi-corner design space exploration that was previously infeasible.
...
### [133] [Detecting and Mitigating Bias by Treating Fairness as a Symmetry Operation](https://arxiv.org/abs/2606.06514)
*Nishit Singh*

Main category: cs.AI

TL;DR: A symmetry-based fairness framework for ML classifiers uses loss regularization to restore invariance under sensitive attribute switching, achieving 90% bias reduction with 5% accuracy cost, without needing causal graphs.


<details>
  <summary>Details</summary>
Motivation: Machine learning systems in high-stakes settings routinely display bias, and existing fairness methods often require causal knowledge or are computationally heavy.

Method: Formalizes bias as symmetry breaking (classifier should be invariant under switching a sensitive attribute while holding merit features fixed) and implements loss-based regularization to restore this symmetry.

Result: Achieves upwards of 90% violation reduction with accuracy costs around 5% on four synthetic datasets.

Conclusion: The framework is lightweight, requires no causal graph, and generalizes to any bit-flip definable sensitive attribute, suitable for contexts with local discrimination sources absent from benchmarks.
...
### [134] [DiBS: Diffusion-Informed Branch Selection](https://arxiv.org/abs/2606.06518)
*Bo Liu,Yuan Xie,Yuan Gao,Xiaolong Luo,Peng Ye,Tao Chen,Fujun Han*

Main category: cs.AI

TL;DR: DiBS is a diffusion model-guided branch ordering approach for Sudoku that combines complete symbolic solvers with learned global guidance, significantly reducing search costs on hard instances while maintaining correctness guarantees.


<details>
  <summary>Details</summary>
Motivation: Existing Sudoku solvers have complementary weaknesses: learning-based methods lack correctness guarantees, while complete symbolic solvers suffer from long-tail search due to expensive branch-order mistakes. The goal is to combine the strengths of both approaches.

Method: DiBS integrates a diffusion model as a branch-ordering guide within a complete symbolic solver. It ranks candidate values under current partial assignments using lightweight consistency signals, keeping the solver's completeness intact while guiding search order.

Result: On the Royle 17-clue Sudoku benchmark, DiBS substantially reduces search cost (nodes, backtracks, long-tail percentiles) compared to strong heuristic baselines, confirming learned global guidance is effective on hard instances.

Conclusion: The diffusion model-guided branch selection approach successfully bridges the gap between learning-based and symbolic solvers, offering both correctness guarantees and efficient search. Theoretical proof is provided to explain why the method works.
...
### [135] [SafeGene: Reusable Adapters for Transferable Safety Alignment](https://arxiv.org/abs/2606.06519)
*Yanghan Wang,Zhiqiang Kou,Fu Feng,Jing Wang,Xin Geng*

Main category: cs.AI

TL;DR: SafeGene is a reusable safety-adapter module that decouples safety capabilities from task-specific fine-tuning, enabling cross-task safety recovery in open-weight LLMs without compromising downstream performance.


<details>
  <summary>Details</summary>
Motivation: Downstream fine-tuning of open-weight LLMs can weaken safety alignment even without harmful training data, creating a recurring safety recovery problem as models are repeatedly updated with new tasks or user interactions.

Method: SafeGene extracts safety representations from aligned-degraded model discrepancies, refines them into task-transferable safety vectors via data-aware layer selection, and applies them through few-shot layer-wise coefficient recalibration.
...
Conclusion: Treating safety as a reusable, decoupled adapter representation is an effective and efficient approach to cross-task safety recovery without model-specific re-alignment.
...
### [136] [Lean4Agent: Formal Modeling and Verification for Agent Workflow and Trajectory](https://arxiv.org/abs/2606.06523)
*Ruida Wang,Jerry Huang,Pengcheng Wang,Xuanqing Liu,Luyang Kong,Tong Zhang*

Main category: cs.AI

TL;DR: Lean4Agent is a framework using Lean4 (a dependent-type formal language) to model, verify, and debug LLM agent workflows. It introduces FormalAgentLib for formal verification and LeanEvolve for workflow revision. Experiments show verification-passing workflows outperform failing ones by ~12%, and LeanEvolve improves SWE performance by ~7.5%.


<details>
  <summary>Details</summary>
Motivation: LLMs lack formal methods for specifying, verifying, and debugging multi-step workflows, analogous to the historical problem of natural language ambiguity that drove the development of formal languages in mathematics.

Method: Lean4Agent uses Lean4 (dependent-type formal language) with FormalAgentLib (an extensible library) to formally model and verify agent workflow semantic consistency under explicit assumptions, and LeanEvolve to revise workflows based on verification results.

Result: On SWE-Bench-Verified and ELAIP-Bench subsets across 5 LLMs, verification-passing workflows outperformed failing ones by 11.94% on average, and LeanEvolve further improved SWE performance by 7.47% on average.

Conclusion: Lean4Agent establishes a foundation for using expressive dependent-type formal languages to formally model and verify agent behavior, opening a new research direction.
...
### [137] [CrowdMath: A Dataset of Crowdsourced Mathematical Research Discussions](https://arxiv.org/abs/2606.06526)
*Sherin Muckatira,Jesse Geneson,Slava Gerovitch,Pavel Etingof,Mikhail Gronas,Anna Rumshisky*

Main category: cs.AI

TL;DR: CrowdMath is a dataset of 164 expert-annotated progress chains from collaborative math problem-solving discussions (2016-2025), where models achieve 83-88% on next-post prediction but only 0.42 macro-F1 on post-role classification, revealing a gap between solving well-specified problems and understanding collaborative mathematical progress.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate well-specified problems with final answers or complete proofs, but they do not capture collaborative open-problem solving where participants propose partial arguments, identify errors, and gradually synthesize contributions into a proof.

Method: They introduce CrowdMath, a dataset of 164 expert-annotated progress chains from the MIT PRIMES-AoPS CrowdMath program (2016-2025). Each chain traces multi-participant forum discussions from open-problem statement to completed proof, with posts labeled by functional roles (partial progress, proof completion, erroneous reasoning, error identification). They define evaluation tasks and benchmark six frontier models.

Result: Models achieve 83-88% accuracy on next-post prediction but only 0.42 macro-F1 on post-role classification, with the best model struggling to identify the functional significance of individual contributions.

Conclusion: CrowdMath exposes a gap between solving well-specified mathematical problems and understanding collaborative mathematical progress as it unfolds, highlighting the need for models capable of following the complex dynamics of collaborative reasoning.
...
### [138] [Attack Selection in Agentic AI Control Evaluations Meaningfully Decreases Safety](https://arxiv.org/abs/2606.06529)
*Catherine Ge-Wang,Tyler Crosse,Benjamin Hadad,Joachim Schaeffer,Ram Potham,Tyler Tracy*

Main category: cs.AI

TL;DR: Attack selection—when an attacker chooses to start/stop attacks—substantially reduces measured safety in AI control evaluations (up to 28pp), suggesting current safety estimates may be overly optimistic against strategic attackers.


<details>
  <summary>Details</summary>
Motivation: Current AI control evaluations assume attackers that attack indiscriminately, but real attacks may be strategically timed. The authors investigate how attack selection (strategic choice of when to attack/abort) impacts safety measurements.

Method: Decomposes attack decisions into a start policy (when to begin attacking) and a stop policy (when to abort). Tests these policies across two agentic environments (BashArena, LinuxArena) at a 1% audit budget.

Result: Start policy reduces safety by 20pp on both settings. Stop policy reduces safety by 20pp (BashArena) and 28pp (LinuxArena). These are treated as upper bounds on attack selection's effect.

Conclusion: Current control evaluations likely overestimate safety against selective attackers. Future evaluations, system cards, and safety cases should explicitly model attack selection to produce more realistic safety estimates.
...
### [139] [CARVE-Q: Quantum-Proposed, Classically Certified Interactive Driving Repair](https://arxiv.org/abs/2606.06531)
*Yifan Wang*

Main category: cs.AI

TL;DR: CARVE introduces a certificate architecture for repairing vetoed driving maneuvers using finite repair lattices, and CARVE-Q adds a quantum search layer that finds optimal repairs with O(√M) queries while keeping safety verification classical.


<details>
  <summary>Details</summary>
Motivation: After a driving veto, existing prediction or game-theoretic planners cannot prove that a proposed repair respects hard rules, right-of-way, cost allocation, and fallback responsibilities. A formal, auditable, and responsibility-bounded repair certificate is needed.

Method: CARVE constructs a finite repair lattice and emits structured certificates. CARVE-Q applies quantum minimum finding (Durr-Hoyer/Grover) to the black-box repair lattice while keeping all safety authority classical (verifier-shielded). The quantum layer proposes; the classical verifier certifies.

Result: Quantum search achieves O(√M) oracle queries versus classical Θ(M) in worst case. State-vector demonstration up to 65,536 assignments. Validation on Lanelet2-grounded INTERACTION replay shows 100% right-of-way respect, 100% blame consistency, and zero priority false positives.

Conclusion: CARVE + CARVE-Q provides a trust-bounded quantum-AI pattern for certified autonomy: quantum search proposes candidate repairs, classical verification certifies them, ensuring safety guarantees remain in human-auditable classical control.
...
### [140] [Position: Don't Just "Fix it in Post": A Science of AI Must Study Training Dynamics](https://arxiv.org/abs/2606.06533)
*Stella Biderman,Mohammad Aflah Khan,Niloofar Mireshghallah,Catherine Arnett,Fazl Barez,Naomi Saphra*

Main category: cs.AI

TL;DR: This position paper argues that AI science must shift from analyzing static trained models to studying the training dynamics that produce model behaviors, advocating for predictive, interventional, and design-oriented theories of how capabilities and properties emerge during training.


<details>
  <summary>Details</summary>
Motivation: Current AI research treats models as fixed artifacts, analyzing behaviors post-hoc rather than understanding why they emerge. The paper argues this is insufficient for true scientific understanding.

Method: The paper grounds its arguments in the history and philosophy of science, examines progress in mechanistic interpretability, fairness, memorization, and simplicity bias, and identifies open problems.

Result: Identifies that scaling laws have made prediction routine for loss, but extending this to capabilities, biases, robustness, and safety-relevant behaviors remains a key challenge.

Conclusion: A science of AI must develop theories supporting three progressive forms of understanding: predicting outcomes from early training signals, intervening on problematic trajectories, and designing training procedures that reliably produce desired properties.
...
### [141] [Accelerated Fourier SAT (AFSAT): Fully Realising a GPU-based Symmetric Pseudo-Boolean SAT Solver](https://arxiv.org/abs/2606.06641)
*Cody J Christopher,Charles Gretton*

Main category: cs.AI

TL;DR: AFSAT is a GPU-accelerated pseudo-Boolean SAT solver using continuous local search, built on JAX. It improves upon FastFourierSAT with better stability, performance, memory efficiency, and near-linear multi-GPU scaling.


<details>
  <summary>Details</summary>
Motivation: To address limitations of the proof-of-concept FastFourierSAT approach—specifically numerical instability, memory latency, floating-point representation issues—and to create a fully-engineered solver that handles heterogeneous constraint types efficiently via GPU acceleration.

Method: Built with JAX compiler, leveraging pure function composition, automatic vectorisation, automatic differentiation, and JIT compilation for massively parallel CLS across batches. Uses a tailored DFT implementation to mitigate floating-point limitations, and JAX array sharding for multi-accelerator scaling.

Result: Substantially improved numerical stability, runtime performance, and memory efficiency over the proof-of-concept. Achieves near-linear throughput when scaling to multiple accelerators.

Conclusion: AFSAT successfully transforms a proof-of-concept into a practical, high-performance GPU-accelerated solver, overcoming key floating-point and scalability challenges.
...
### [142] [A Study of Parallel Continuous Local Search](https://arxiv.org/abs/2606.06656)
*Cody J Christopher,Charles Gretton*

Main category: cs.AI

TL;DR: This paper investigates parallel Continuous Local Search (CLS) for solving Boolean satisfiability with symmetric pseudo-Boolean constraints, finding that redundant constraints hinder convergence, CLS works well as a sub-solver in hybrid settings, and local search quickly reaches a stable solution quality distribution due to saddle-dense objectives.


<details>
  <summary>Details</summary>
Motivation: To explore Continuous Local Search (CLS) as an alternative approach for solving Boolean satisfiability problems with symmetric pseudo-Boolean constraints, leveraging continuous optimisation on a differentiable objective function to potentially exploit modern accelerator hardware.

Method: The authors relax the n-variable PB-satisfiability problem to a continuous optimisation problem with a differentiable objective function on an n-dimensional hypercube, then apply parallel Continuous Local Search (CLS), conducting empirical experiments to study convergence behavior.

Result: (i) Redundant constraints inhibit rather than accelerate convergence; (ii) CLS shows promise as a sub-solver in hybridised settings, quickly completing partial assignments; (iii) Local search rapidly converges to a stable distribution of solution quality due to saddle-dense objectives where additional steps yield diminishing returns.

Conclusion: The findings inform practical uses of CLS for SAT on modern accelerator hardware, suggesting CLS is best used as a sub-solver in hybrid settings rather than a standalone approach, and that redundant constraints should be avoided.
...
### [143] [AEGIS: A Backup Reflex for Physical AI](https://arxiv.org/abs/2606.06660)
*Josef Chen*

Main category: cs.AI

TL;DR: AEGIS selectively escalates from a weak to a strong policy in long-horizon robot manipulation by using a lightweight probe to detect high-risk steps early, recovering 10.1% of lost trajectories vs. 4.6% (blind escalation) and 5.1% (random trigger), while activating the stronger policy on only 38% of steps.


<details>
  <summary>Details</summary>
Motivation: Long-horizon robot manipulation tends to fail gradually where one bad step degrades the state irrecoverably, and failure is often visible before it happens, motivating a need for early detection and selective intervention.

Method: AEGIS uses a lightweight probe on a weak policy's frozen activations to detect high-risk steps early, switching control to a stronger policy only when needed, based on a pre-registered analysis plan with conditional recovered-task-rate estimand.

Result: On LIBERO-Spatial, AEGIS recovers 10.1% of trajectories lost by the weak policy vs. 4.6% (blind escalation) and 5.1% (random trigger). Gains are significant under paired McNemar tests (p=8.5e-6 and p=1.0e-4). The probe achieves early-window AUROC of 0.764 [0.70, 0.84] on the first 30% of steps.

Conclusion: AEGIS demonstrates that selective, early-warning-based escalation can significantly improve long-horizon robot manipulation success with minimal compute overhead (stronger policy only on 38% of steps), validated on 700 episodes per arm with pre-registered analysis.
...
### [144] [A Geometric Account of Activation Steering through Angle-Norm Decomposition](https://arxiv.org/abs/2606.06735)
*Georgii Aparin,Tatiana Gaintseva*

Main category: cs.AI

TL;DR: This paper investigates how linear vs. spherical activation steering methods work differently by disentangling angular (direction) and radial (norm) components of hidden states in language models, finding concepts are encoded primarily in angular structure but norm affects stability.


<details>
  <summary>Details</summary>
Motivation: To resolve whether spherical steering methods outperform linear methods, and to test the assumption that hidden-state norm doesn't carry concept-relevant information, by systematically disentangling angular and radial components of steering interventions.

Method: Controlled empirical study across seven language models, disentangling the roles of angular alignment and hidden-state norm changes in steering interventions, comparing linear and spherical paradigms.

Result: Concepts are primarily represented in angular structure (supporting spherical methods), but norm remains important for stability and downstream effects. Different steering methods couple these two geometric effects differently.

Conclusion: Activation steering should be parameterized by interpretable angular and radial components separately, rather than a single additive coefficient that entangles these two effects.
...
### [145] [OpenSkill: Open-World Self-Evolution for LLM Agents](https://arxiv.org/abs/2606.06741)
...
TL;DR: OpenSkill enables agents to self-evolve in open-world settings by building skills and verification signals from scratch using only a task prompt, without any target-task supervision.


<details>
  <summary>Details</summary>
Motivation: Existing self-evolving agents assume a usable learning loop (curated skills, successful trajectories, verifier signals), but real open-world deployments may lack these, offering only a task prompt.

Method: OpenSkill bootstraps self-evolution by: (1) acquiring grounded knowledge and verification anchors from documentation, repositories, and the web, (2) synthesizing them into transferable skills, and (3) refining skills against self-built virtual tasks grounded in those anchors rather than target answers.

Result: OpenSkill achieves the best automated pass rate across three benchmarks and two target agents while satisfying the no-supervision constraint. Skills transfer across models without model-specific adaptation, and the self-built verifier aligns with ground-truth outcomes despite never accessing them.

Conclusion: The open world can supply both the knowledge to be learned and a supervision-independent practice environment, eliminating the need for target-task supervision in agent self-evolution.
...
### [146] [AdMem: Advanced Memory for Task-solving Agents](https://arxiv.org/abs/2606.06787)
*Runzhe Wang,Huilin Lu,Shengjie Liu,Li Dong,Jason Zhu*

Main category: cs.AI

TL;DR: This paper proposes a unified automatic memory framework with bi-level (short-term/long-term) storage integrating semantic, episodic, and procedural memory, using a multi-agent architecture (actor, memory, critic) for automatic memory generation, reward annotation, and adaptive retrieval, showing improved robustness on long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: LLMs as tool-using agents struggle with long-horizon tasks requiring remembering, organizing, and reusing knowledge. Prior memory approaches focus on factual storage or replaying past successes without addressing failure cases or online scalability.

Method: A unified memory framework integrating semantic, episodic, and procedural memory in a bi-level design (short-term + long-term stores). A multi-agent architecture with actor, memory, and critic agents enables automatic memory generation, reward annotation, and adaptive retrieval. Long-term memory uses reward-based evaluation, merging, and pruning for scalability.

Result: Experiments across various environments show improved robustness and success on long multi-turn tasks compared to existing baselines.

Conclusion: Comprehensive, adaptive memory is important for advancing LLM-based agents in long-horizon tasks.
...
### [147] [Evidence-Based Intelligent Diagnostic and Therapeutic Visualization System with Large Language Models: Multi-Turn Interaction and Multimodal Treatment Plan Generation](https://arxiv.org/abs/2606.06869)
*Yunhan Wang,Yuda Wang,Zhiying Tu,Mingqiang Song,Li Song,Kun Li,Dianhui Chu,Bolin Zhang*

Main category: cs.AI

TL;DR: A knowledge-enhanced visual diagnostic system using Neo4j knowledge graph, multi-stage symptom matching, proactive questioning, and multimodal treatment presentation improves transparency, interpretability, and trust in AI-assisted traditional Chinese medicine diagnosis.


<details>
  <summary>Details</summary>
Motivation: Existing AI-assisted TCM diagnostic tools suffer from opaque reasoning, passive interaction, and limited treatment plan presentation, lacking transparency and interpretability.

Method: Builds a Neo4j knowledge graph (241 syndromes, 1,263 symptoms, 2,485 relations); four-stage symptom matching pipeline (exact, semantic, fuzzy, LLM verification); information gain-driven proactive questioning optimized with genetic algorithms; multimodal treatment presentation with AI-generated illustrations, 3D meridian-acupoint models, and evidence-based literature.

Result: Knowledge graph constraints reduced non-standard outputs by 32%; case studies validated effectiveness; automated evaluation across 30 cases showed significant improvements in diagnostic trust (Cohen's d=1.82, p<0.001), reduced cognitive load (4 of 5 dimensions), and higher credibility of evidence-based references (4.21 vs. 2.95).

Conclusion: The system enhances transparency of diagnostic reasoning and interpretability of treatment plans via knowledge graph-driven visualization and multimodal interaction, offering a trustworthy AI-assisted TCM solution.
...
### [148] [Workflow-to-Skill: Skill Creation via Routing-Workflow-Semantics-Attachments Decomposition](https://arxiv.org/abs/2606.06893)
*Yuyang Zhang,Xinyuan Han,Xudong Jiang,Run Wang*

Main category: cs.AI

TL;DR: This paper proposes RWSA (a workflow-oriented intermediate representation) and W2S (a framework for automatic Skill construction from heterogeneous traces like demonstrations and logs), showing 10.5% improvement in behavioral replay consistency over baselines by treating traces as executable specifications rather than compressible text.


<details>
  <summary>Details</summary>
Motivation: High-quality Skills for LLM agents are costly to hand-write, and automatic construction from heterogeneous traces is non-trivial because traces are fragmented, redundant, and may omit rare but safety-critical behaviors — they cannot be treated as simple summarization tasks.

Method: The paper introduces RWSA (Workflow structure, execution Semantics, runtime Attachments) as an intermediate representation for decomposing Skills. Then proposes W2S framework that segments traces, induces local Skill drafts, aligns shared structures, reconciles branches, and compresses redundancy while preserving evidence and confidence annotations.

Result: Experiments on 70 Skills show W2S improves behavioral replay consistency by 10.5% over summarization- and prompting-based baselines.

Conclusion: The authors conclude that traces should be treated as executable runtime specifications rather than compressible text, highlighting the need for structure-aware Skill construction approaches.
...
### [149] [Declarative Skills for AI Agents in Knowledge-Grounded Tool-Use Workflows](https://arxiv.org/abs/2606.06923)
*M. Danish Lim,I. Danial Bin Sharudin,Wen Han Chen,Cedric Lim,Laura Wynter*

Main category: cs.AI

TL;DR: The paper compares declarative vs. imperative orchestration for tool-using AI agents in customer-service workflows, finding that retrieval quality is the dominant bottleneck, but declarative skill files improve accuracy and reduce errors under high-quality retrieval.


<details>
  <summary>Details</summary>
Motivation: To understand effective orchestration mechanisms for tool-using AI agents in realistic customer-service workflows over unstructured knowledge bases.

Method: The authors compare three agent types (DeclarativeAgent, ImperativeAgent, unscaffolded baseline) formalized as policy classes within a decentralized partially-observable Markov decision process, tested on five language models and two retrieval regimes.

Result: Retrieval quality is the dominant bottleneck; under high-quality retrieval, declarative skills improve accuracy and reduce orchestration errors, while the imperative state machine does not reliably improve task success.

Conclusion: Declarative agents with natural-language skill files are an effective orchestration paradigm, but retrieval quality remains critical for overall performance.
...
### [150] [Quantum-Inspired Trace-Augmented Evidence Selection for Reasoning over Structured Hypothesis Spaces](https://arxiv.org/abs/2606.06941)
*Laura Wynter,Nirvik Sahoo,Paul Griffin*

Main category: cs.AI

TL;DR: EP-HUBO treats evidence selection from CoT traces as an optimization problem, using a small local model to generate evidence pools and a frontier model for final adjudication, outperforming majority vote on legal reasoning benchmarks—especially when benchmark contamination is low.


<details>
  <summary>Details</summary>
Motivation: LLMs are brittle on evidence-intensive domains like law because majority voting over CoT traces picks the most popular answer regardless of evidence quality, and minority-but-correct hypotheses get discarded.

Method: EP-HUBO: (1) generate multiple CoT traces with a small local model, (2) parse fragments into per-hypothesis evidence pools, (3) solve a higher-order unconstrained binary optimization (HUBO) with weights for relevance, specificity, distinctiveness, (4) delegate a single adjudication call per question to a frontier model. Optimization is done via simulated annealing or the Dirac-3 photonic quantum machine.

Result: EP-HUBO outperforms majority vote on evidence-intensive legal benchmarks, especially in low-contamination settings where frontier models have not memorized the benchmark data.

Conclusion: HUBO-style optimization provides a principled aggregation of reasoning fragments that preserves minority-but-correct hypotheses and is most valuable for specialized, low-contamination domains.
...
### [151] [Accounting for Context: Shaping Moral Credences for Value Alignment](https://arxiv.org/abs/2606.06972)
*Jazon Szabo,Sanjay Modgil*

Main category: cs.AI

TL;DR: This paper argues that aggregating moral evaluations for AI agents must account for contextual factors (like whether a moral theory's assumptions hold in practice), and shows that ignoring these factors leads to violations of the weak Pareto principle, analogous to Simpson's paradox.


<details>
  <summary>Details</summary>
Motivation: To address the problem of plural moral perspectives in AI alignment by showing that existing moral uncertainty aggregation mechanisms fail to account for contextual factors (e.g., whether consequentialist assumptions actually hold in real-world settings).

Method: Formalizes agent decision-making under moral uncertainty while incorporating contextual factors, then analyzes the resulting properties (e.g., weak Pareto principle) using a Simpson's paradox lens.

Result: The weak Pareto principle is violated when contextual factors are accounted for, but this is shown to be a variation of Simpson's paradox, not a flaw—revealing the limitations of context-ignoring aggregation.

Conclusion: Aggregation mechanisms for moral uncertainty must incorporate contextual factors; the apparent violation of Pareto is actually a known statistical paradox, underscoring the need for context-aware moral aggregation in AI alignment.
...
### [152] [Exploring Agentic Tool-Calling Decisions via Uncertainty-Aligned Reinforcement Learning](https://arxiv.org/abs/2606.06976)
*Yijin Zhou,Linqian Zeng,Xiaoya Lu,Wenyuan Xie,Dongrui Liu,Junchi Yan,Jing Shao*

Main category: cs.AI

TL;DR: TRUST uses uncertainty quantification as a repulsive force in reward design to maintain uncertainty separation between correct/incorrect actions, plus lightweight key-turn annotations for unified post-training, improving LLM agent tool-use decisions.


<details>
  <summary>Details</summary>
Motivation: LLM-based agents often make suboptimal tool-use decisions (unsupported tool invocation, hallucinated responses), and existing approaches fail to leverage uncertainty characteristics of agent decisions—decision-oriented RL weakens uncertainty separation between correct/incorrect actions, causing overconfident mistakes.

Method: Proposes TRUST: (1) incorporates uncertainty quantification into reward design as a repulsive force to maintain uncertainty separation, (2) labels lightweight key-turn annotations for unified post-training of multi-turn trajectories.

Result: Across diverse tool-use benchmarks, TRUST consistently enhances decision quality, agent performance, and maintains more reliable uncertainty estimates during optimization.

Conclusion: Uncertainty-aware reward design effectively improves tool-use agent decisions and can be integrated via lightweight annotations for multi-turn post-training.
...
### [153] [Teaching the Way, Not the Answer: Privileged Tutoring Distillation for Multimodal Policy Optimization](https://arxiv.org/abs/2606.07000)
*Shizhe Xiang,Ke An,Wenlong Yu,Yue Liu,Jian Luan,Pei Fu,Qilong Wang*

Main category: cs.AI

TL;DR: PTD-PO is a Privileged Tutoring Distillation Policy Optimization framework that improves multimodal reasoning in LVLMs by providing dense, token-level supervision during RLVR training without exposing answers to the student policy, using structured privileged hints from spatial attention and intermediate reasoning steps.


<details>
  <summary>Details</summary>
Motivation: Existing RLVR methods suffer from sparse verifiable rewards, offering little token-level supervision for failed rollouts and causing inefficient exploration. Policy distillation from external teachers is computationally heavy, while answer-conditioned tuning can leak answer information and cause shortcut behavior.

Method: PTD-PO constructs structured privileged hints (spatial attention + intermediate reasoning steps) and uses in-context learning to produce step-wise token-distribution supervision. The student policy is optimized without answer exposure, and failed rollouts are aligned with the hint-augmented reference model at token-distribution level. A Top-K Jensen-Shannon divergence objective stabilizes distillation under distribution shift.
...
Conclusion: PTD-PO effectively addresses the sparse reward problem in RLVR by providing dense, privileged supervision without answer leakage, leading to better reasoning performance and training stability in multimodal models.
...
### [154] [The Sim-to-Real Gap of Foundation Model Agents: A Unified MDP Perspective](https://arxiv.org/abs/2606.07017)
*Xiaoou Liu,Tiejin Chen,Weibo Li,Xiyang Hu,Hua Wei*

Main category: cs.AI

TL;DR: This paper proposes reframing the foundation model agent evaluation and training gap as a classical sim-to-real problem structured around the four MDP elements (Observation, Action, Transition, Reward), advocating for adopting established solutions like domain randomization and creating standardized stress test benchmarks.


<details>
  <summary>Details</summary>
Motivation: Foundation model agents suffer from the sim-to-real gap when deployed in real-world decision-making, yet the community treats agent robustness as a novel phenomenon rather than leveraging mature frameworks from robotics and classical control.

Method: Formalizes the foundation model agent gap as a classical sim-to-real problem using the four MDP elements, translates classical discrepancies into the foundation model domain, and advocates for established solutions like domain randomization with concrete examples (e.g., multilingual tool calling).

Result: Provides concrete demonstration showing how severe observation space gaps (e.g., multilingual variations) lead to operationally invalid actions despite correct semantic intent.

Conclusion: This agenda aims to drive a paradigm shift toward unified vocabulary and standardized stress test benchmarks, fostering highly trustworthy agents for reliable real-world applications.
...
### [155] [StainFlow: Entity-Stain Tracking and Evidence Linking for Process Rewards in GUI Agents](https://arxiv.org/abs/2606.07027)
...
TL;DR: StainFlow is a process reward model for GUI Agents that uses entity-stain-flow tracking to provide better credit assignment in reinforcement learning, improving task decomposition and local verification by tracking task entities and their state changes along trajectories.


<details>
  <summary>Details</summary>
Motivation: Trajectory-level success feedback in RL for GUI Agents is too sparse for reliable credit assignment. Existing PRMs suffer from subjective global milestone decomposition and fixed local judging windows that miss key evidence or dilute signals.

Method: StainFlow introduces two modules: (1) Global Entity Stain Tracking - extracts visually verifiable task entities and tracks their stain concentrations/states along trajectories to objectively separate task phases, and (2) Local Stain Evidence Linking - uses triggering entities of candidate key nodes to retrieve relevant steps and dynamically construct high-density evidence windows for verification.

Result: On AndroidWorld and OGRBench, StainFlow improves online RL success by 3.2% and trajectory completion judgment accuracy by 1.8%.

Conclusion: The proposed entity-stain-flow approach effectively addresses both global and local limitations of existing PRMs for GUI Agents, achieving measurable improvements in reinforcement learning outcomes.
...
### [156] [Hierarchical Semantic-Constrained Heterogeneous Graph for Audio-Visual Event Localization](https://arxiv.org/abs/2606.07033)
*Zhe Yang,Ruyi Zhang,Hongtao Chen,Wenrui Li,Hengyu Man,Wangmeng Zuo,Xiaopeng Fan*

Main category: cs.AI

TL;DR: The paper proposes a Hierarchical Semantic Constrained Heterogeneous Graph (HSCHG) for open-vocabulary audio-visual event localization, using heterogeneous graphs in Euclidean space, dual-threshold gated fusion, and hyperbolic space mapping with hierarchical entailment regularization to handle unseen categories and maintain cross-level semantic consistency.


<details>
  <summary>Details</summary>
Motivation: Existing OV-AVEL methods face two key challenges: (1) lack of supervision for unseen categories makes it hard to maintain audio-visual consistency across temporal scales, and (2) lack of hierarchical constraints between segment- and video-level semantics prevents establishing cross-level semantic consistency.

Method: (1) Heterogeneous hierarchical graph in Euclidean space with audio/visual segment and video-level nodes; (2) Multi-directional temporal edges for within-modality temporal info; (3) Dual-threshold filtering gated fusion for selective cross-modal alignment; (4) Bidirectional semantic constraints between segment and video levels; (5) Mapping multi-level representations and text prototypes into hyperbolic space; (6) Hierarchical entailment regularization loss for hierarchical relationships.

Result: Outperforms existing methods on OV-AVEL benchmark; ablation studies validate effectiveness of each component.

Conclusion: The HSCHG framework effectively addresses open-vocabulary audio-visual event localization by enforcing hierarchical semantic constraints across levels and using hyperbolic space to capture hierarchical relationships, leading to superior performance.
...
### [157] [Front-to-Attractors: Modifying the Front-to-Front Heuristic in Bidirectional Search](https://arxiv.org/abs/2606.07047)
*Alvin Zou,Muhammad Suhail Saleem,Maxim Likhachev*

Main category: cs.AI

TL;DR: The paper introduces Front-to-Attractors (F2A) heuristics for bidirectional search, which estimates distances to a small set of attractors instead of the entire opposite frontier, achieving substantial computational savings while maintaining informativeness and optimality guarantees.


<details>
  <summary>Details</summary>
Motivation: Front-to-front (F2F) heuristics are informative but computationally expensive due to extensive pairwise evaluations across the entire opposite frontier. The authors aim to preserve F2F's informativeness while drastically reducing its computational overhead.

Method: F2A introduces a new heuristic class that estimates distance from a state s to a small, dynamically maintained set of attractors in the opposite search direction, serving as a surrogate for the full frontier.

Result: F2A reduces pairwise evaluations by up to 11.2x compared to F2F, and achieves 4.8x fewer node expansions than front-to-end (F2E) heuristics on average.

Conclusion: F2A effectively bridges the gap between informative but costly F2F heuristics and efficient but less informative F2E heuristics, maintaining optimality guarantees while offering substantial computational improvements.
...
TL;DR: DyCon is a training-free framework that mitigates "overthinking" in Large Reasoning Models by dynamically controlling reasoning depth based on step-level embeddings that encode evolving task difficulty, improving efficiency without sacrificing accuracy across 12 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models suffer from "overthinking" — redundant reasoning steps that waste computation. Existing mitigation methods use static difficulty estimates or task-specific training, failing to adapt to dynamic complexity changes during the reasoning process.

Method: DyCon leverages latent step-level embeddings from LRMs to model the evolving task difficulty in real-time. It dynamically controls reasoning depth during inference without requiring additional training, making it a training-free framework.

Result: Tested on 4 models (4B–32B) and 12 benchmarks across math reasoning, QA, and coding tasks. DyCon significantly enhances efficiency by reducing redundant reasoning steps while maintaining accuracy and generalization.

Conclusion: Problem difficulty evolves dynamically during reasoning and is linearly encoded in step-level embeddings. DyCon effectively exploits this for dynamic reasoning depth control, offering a practical, training-free solution to overthinking in LRMs.
...
### [159] [Beyond Post-hoc Explanation: Toward Glassbox AI via Probabilistic Mediation](https://arxiv.org/abs/2606.07113)
*Manuele Leonelli*

Main category: cs.AI

TL;DR: The paper proposes the Glassbox Framework, using Bayesian networks as ante-hoc mediation layers for LLMs to enable transparent, auditable, and contestable reasoning in high-stakes institutional settings.


<details>
  <summary>Details</summary>
Motivation: Existing post-hoc explainability methods for LLMs are unstable, non-contestable, and lack formal connection to the actual reasoning process, which is untenable in high-stakes domains like public administration, law, and healthcare.

Method: The Glassbox Framework integrates Bayesian networks as transparent, ante-hoc mediation layers that encode domain knowledge, causal assumptions, and probabilistic dependencies before inference, enabling auditable reasoning traces and uncertainty quantification.

Result: The framework is characterized and grounded in a benefit eligibility scenario, identifying foundational challenges: semantic alignment, dynamic model construction, probabilistic grounding, and human governance.

Conclusion: Shifting from post-hoc explanation to ante-hoc probabilistic mediation offers a principled path toward AI systems that are powerful yet fundamentally accountable.
...
### [160] [Think Fast: Estimating No-CoT Task-Completion Time Horizons of Frontier AI Models](https://arxiv.org/abs/2606.07157)
...
TL;DR: Frontier AI models' ability to reason without chain-of-thought (CoT) has been doubling annually, now exceeding human tasks requiring 3+ minutes; projections suggest this could reach 25 minutes by 2030, threatening CoT-based safety monitoring.


<details>
  <summary>Details</summary>
Motivation: To assess whether frontier AI models can perform complex reasoning without explicit chain-of-thought tokens, which would undermine safety oversight methods that rely on monitoring CoT reasoning.

Method: Measured no-CoT reasoning across 30,000+ questions from 43 benchmarks covering math, coding, puzzles, causality, theory-of-mind, and strategic reasoning. Used two metrics: 50%-task-completion time horizon (human time equivalent) and 50% reasoning token horizon (minimum o3-mini reasoning tokens).

Result: No-CoT 50% time horizon of frontier models has doubled roughly every year over six years. GPT-5.5's TH exceeds 3 minutes, with reasoning token horizon over 1,500 tokens. Median estimates predict TH could exceed 7 minutes by 2028 and 25 minutes by 2030.

Conclusion: Frontier developers should explicitly track no-CoT reasoning capabilities, as continued growth threatens the viability of CoT-based safety monitoring approaches.
...
### [161] [TOPSIS-RAD: Ranking According to Desires](https://arxiv.org/abs/2606.07253)
*Leonardo Fernandes Costa,Helder Gomes Costa,Diogo Lima,Brunno Rodrigues*

Main category: cs.AI

TL;DR: TOPSIS-RAD enhances traditional TOPSIS by using DM-defined Vetoed Performance Levels (VPL) to exclude non-viable alternatives and Desired Performance Levels (DPL) to cap overperformance, reducing sensitivity to outliers and rank reversal while grounding rankings in explicit decision-maker aspirations.


<details>
  <summary>Details</summary>
Motivation: Traditional TOPSIS derives reference points (PIS/NIS) from observed data, causing misalignment with DM requirements, sensitivity to outliers, and rank reversal issues.

Method: Introduces two DM-defined reference arrays: VPL (excludes non-viable alternatives before normalization) and DPL (caps overperformance before normalization to anchor PIS in desired levels). Preserves distance-based TOPSIS structure.

Result: Three toy examples demonstrate: (1) VPL reshapes normalization boundaries by removing non-viable alternatives; (2) fixed DPL frontiers stabilize rankings by limiting influence of extreme performances.

Conclusion: TOPSIS-RAD grounds rankings in stable, DM-specified boundaries while maintaining TOPSIS's familiar structure. Limitations and future research are noted.
...
### [162] [DuMate-DeepResearch: An Auditable Multi-Agent System with Recursive Search and Rubric-Grounded Reasoning](https://arxiv.org/abs/2606.07299)
...
TL;DR: DuMate-DeepResearch is a multi-agent framework for complex research tasks that uses graph-based dynamic planning, recursive two-level execution, and rubric-based test-time optimization to achieve state-of-the-art results on two deep research benchmarks (58.03% and 61.95%).


<details>
  <summary>Details</summary>
Motivation: Current Deep Research systems face four key limitations: long-horizon planning with underspecified scope, single-agent bottleneck for task decomposition/scheduling, hallucination risk in long-form synthesis, and limited process auditability.

Method: The framework decouples Agent Core from Tool Ecosystem for traceability, and introduces: (i) graph-based dynamic planning (coarse-to-fine roadmap with reflection, re-planning, backtracking, parallel branching); (ii) recursive two-level execution (inner Search Agent for complex sub-tasks); (iii) rubric-based test-time optimization (dynamic quality criteria for evidence-grounded synthesis and adaptive stopping).

Result: Achieves new SOTA on DeepResearch Bench (58.03%) and DeepResearch Bench II (61.95%, ranking first in information recall and analysis).

Conclusion: The multi-agent decoupling design effectively addresses the core limitations of current DR systems, providing both performance gains and process auditability for complex research tasks.
...
### [163] [Off-Policy Evaluation with Strategic Agents via Local Disclosure](https://arxiv.org/abs/2606.07308)
*Kiet Q. H. Vo,Abbavaram Gowtham Reddy,Julian Rodemann,Siu Lun Chau,Krikamol Muandet*

Main category: cs.AI

TL;DR: This paper proposes a doubly robust estimator for off-policy evaluation under strategic behavior, leveraging post-hoc explanations to reveal agents' pre-strategic covariates and mitigate information loss.


<details>
  <summary>Details</summary>
Motivation: Existing OPE methods assume covariates are exogenous to the policy, but in strategic settings, agents modify their covariates in response to the policy. Prior solutions require strong assumptions like repeated interactions or full knowledge of agent responses, limiting practical applicability.

Method: The authors use post-hoc explanations to disclose local information about agents' pre-strategic covariates, estimate a statistical model of agent responses (assuming conditional log-normal cost sensitivity), and construct a doubly robust estimator for policy value.

Result: The proposed estimator is shown to be consistent under the log-normal assumption, and empirical validation confirms its effectiveness.

Conclusion: Interaction design (e.g., providing post-hoc explanations) can mitigate information asymmetry by revealing hidden structure in agents' strategic responses, enabling valid OPE with only partial knowledge of agent behavior.
...
### [164] [Online Pandora's Box for Contextual LLM Cascading](https://arxiv.org/abs/2606.07392)
*Alexandre Belloni,Yan Chen,Yehua Wei*

Main category: cs.AI

TL;DR: The paper proposes an online contextual Pandora's Box model for adaptively querying and selecting LLM APIs, using GMM-based estimation of reservation indices and UCB-style confidence bounds to achieve $\widetilde O(\sqrt T)$ regret.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of adaptively querying and selecting among multiple LLM APIs in a cost-effective manner, motivated by LLM cascading where one must decide which APIs to query and which output to deploy.

Method: Models the problem as an online contextual Pandora's Box with two phases (query and selection). Imposes parametric structure on contextual reservation index functions (Weitzman's policy), uses GMM-type estimation for reservation indices, and combines UCB-style confidence bounds for both indices and the reward evaluator.

Result: Under regularity conditions, the proposed policy achieves dimension-dependent $\widetilde O(\sqrt T)$ cumulative regret over $T$ periods.

Conclusion: The output-mediated feedback structure (observing only the deployed output's reward) is sufficient for learning, and the parametric reservation index approach offers an efficient alternative to estimating full conditional distributions of each API.
...
### [165] [Act As a Real Researcher: A Suite of Benchmarks Evaluating Frontier LLMs and Agentic Harnesses in Research Lifecycle](https://arxiv.org/abs/2606.07462)
*Jiayu Wang,Weijiang Lv,Bowen Fu,Jing Fu,Jiayi Song,Lingyu Zhang,Lanxuan Xue,Luodi Chen,Zepeng Xin,Kaiyu Li,Xiangyong Cao*

Main category: cs.AI

TL;DR: The paper introduces AARRI-Bench, the first benchmark in the AARR series, designed to evaluate whether AI agents can emulate the nuanced reasoning, professionalism, and field sensitivity of human researchers in granular research scenarios. Experiments show the best model (Mini-SWE-Agent with Claude Opus 4.7) achieves only 68.3% success, often missing subtle details obvious to humans.


<details>
  <summary>Details</summary>
Motivation: Current autonomous research agents exhibit significant limitations in field sensitivity, research ethics, and nuanced scientific judgment, preventing them from fully replacing human researchers. Existing benchmarks focus on macro-level execution rather than granular researcher-like behaviors.

Method: The authors propose AARRI-Bench as the first benchmark in the AARR series, focusing on evaluating agents' ability to emulate researcher-like professionalism, thoroughness, and nuanced reasoning in detailed research scenarios. Experiments were conducted across frontier models and agentic systems.

Result: The best-performing configuration (Mini-SWE-Agent with Claude Opus 4.7) achieves only 68.3% success rate and frequently overlooks subtle yet critical details that are obvious to human researchers.

Conclusion: Developing truly researcher-like AI requires further exploration of research behavior rather than merely complex scaffolding. The benchmark data is publicly released.
...
### [166] [How AI Agents Reshape Knowledge Work: Autonomy, Efficiency, and Scope](https://arxiv.org/abs/2606.07489)
*Jeremy Yang,Kate Zyskowski,Noah Yonack,Jerry Ma*

Main category: cs.AI

TL;DR: AI agents (Computer) outperform conversational AI (Search) by autonomously executing 26 minutes of work per session vs. 33 seconds, reducing time by 87% and cost by 94%, lowering dissatisfaction by 55%, and expanding the scope of automated knowledge work into higher-order, cross-boundary tasks.


<details>
  <summary>Details</summary>
Motivation: To study how AI agents transitioning from conversational assistants to autonomous agents reshape knowledge work, using production data from Perplexity's Search and Computer products.

Method: Analysis of production data from Perplexity's Search and Computer products; used near-identical initial query pairs as natural experiments to compare the same underlying tasks across both products.

Result: (1) Computer performs 26 min of autonomous work per session vs. 33 sec for Search, with 55% lower dissatisfaction; (2) reduces completion time from 269 to 36 min, lowering time by 87% and cost by 94%; (3) shifts work toward higher-order, cross-occupational, composite tasks absent from Search usage.

Conclusion: AI agents accelerate workflows, enhance output quality, reduce costs, and expand the breadth and depth of automated knowledge work.
...
### [167] [Elmes*: Automated Construction of Fine-Grained Evaluation Rubrics for Large Language Models in Long-Tail Educational Scenarios](https://arxiv.org/abs/2606.06546)
*Tao Liu,Ye Lu,Ruohua Zhang,Siyu Song,Wentao Liu,Aimin Zhou,Hao Hao*

Main category: cs.LG

TL;DR: Elmes* is an end-to-end framework for constructing pedagogical rubrics to evaluate LLMs across 330 educational scenarios, revealing that educational capability is multidimensional, with top LLMs differing in creativity and values, and specialized models like InnoSpark outperforming general ones.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate LLMs on domain-general correctness or use manually designed rubrics that don't scale to long-tail pedagogical scenarios. There is a need to measure how models teach, not just what they know.

Method: Elmes* combines a declarative multi-agent engine (teacher-student-judge interactions) with SceneGen, a self-evolving module that co-optimizes evaluation criteria and test data from expert-defined pedagogical dimensions. Built Edu-330 covering 330 scenarios across 11 subjects, 3 grade bands, and 10 task types with 1,000+ second-level indicators.

Result: Educational capability is multidimensional: top LLMs differ mainly in creativity and values integration; knowledge-strong models may fail at Socratic scaffolding; InnoSpark achieves best human-evaluated average score. LLM judges preserve human-comparable rankings with lower variance but exhibit self-preference bias. Expert-scored few-shot anchoring improves human-LLM alignment.

Conclusion: Elmes* provides scalable diagnostic infrastructure for pedagogically grounded LLM evaluation, enabling fine-grained, scenario-specific assessment of educational capabilities.
...
### [168] [FAIR-Calib: Frontier-Aware Instability-Reweighted Calibration for Post-Training Quantization of Diffusion Large Language Models](https://arxiv.org/abs/2606.06547)
*Haoyu Huang,Linlin Yang,Sheng Xu,Boyu Liu,Guodong Guo,Zhongqian Fu,Hang Zhou,Baochang Zhang*

Main category: cs.LG

TL;DR: FAIR-Calib is a two-stage PTQ framework for dLLMs that protects fragile frontier token decisions during iterative generation by reweighting calibration to prioritize unstable states, reducing decision flips and outperforming baselines on LLaDA and Dream.


<details>
  <summary>Details</summary>
Motivation: Diffusion LLMs commit tokens irreversibly, causing "stability lag" where early borderline decisions remain fragile and can be flipped by PTQ errors, becoming permanently locked in and amplified.

Method: Two-stage PTQ framework: Stage I estimates a position prior (frontier hits + masked-stage reliability) from a full-precision teacher; Stage II performs off-policy layer-wise calibration with reweighted hidden-state MSE to protect fragile frontier states.

Result: Outperforms SOTA baselines on LLaDA and Dream (W4A4), significantly reduces frontier decision flips and post-commit mismatches across diverse benchmarks.

Conclusion: The weighted objective acts as a surrogate for output KL divergence; prioritizing frontier state protection is effective and does not require costly end-to-end diffusion rollouts.
...
### [169] [Multi-Scale Feature Attention Network for Polymer Classification using THz Dual-Comb Spectroscopy](https://arxiv.org/abs/2606.06554)
*Roshni Mahtani,Ilán Carretero,Laura Monroy,Aldo Moreno-Oyervides,Oscar Elías Bonilla-Manrique,Rocío del Amor*

Main category: cs.LG

TL;DR: The paper proposes MSFAN, a deep learning architecture combining terahertz dual-comb spectroscopy with multi-scale feature attention for classifying 12 types of polymers, achieving 85.2% accuracy and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Reliable polymer identification is critical for quality/safety of recycled plastics, but conventional sorting and spectroscopic methods struggle to provide robust discrimination.

Method: Proposes Multi-Scale Feature Attention Network (MSFAN) with feature gating for signal recalibration, multi-scale parallel convolutions to capture diverse frequency patterns, cross-feature attention, and attention pooling for THz-DCS data.

Result: MSFAN achieves 85.2% classification accuracy across 12 polymer types, outperforming state-of-the-art models.

Conclusion: Combining THz-DCS with deep learning (MSFAN) offers effective, scalable, and interpretable polymer classification for recycled plastics quality assurance.
...
### [170] [MacArena: Benchmarking Computer Use Agents on an Online macOS Environment](https://arxiv.org/abs/2606.06560)
*Victor Muryn,Maksym Shamrai,Sofiia Mazepa,Yehor Khodysko*

Main category: cs.LG

TL;DR: MacArena is a new benchmark of 421 manually verified macOS GUI tasks across 50 apps, revealing that current GUI agents perform significantly worse on macOS than on Linux benchmarks, with model rankings inverting between task types.


<details>
  <summary>Details</summary>
Motivation: macOS is underserved in GUI agent benchmarking because the only existing benchmark (macOSWorld) covers a narrow set of first-party apps with simpler tasks and runs on x86 VMs incompatible with Apple Silicon, leaving a gap in evaluating cross-platform GUI competence.

Method: Created MacArena with 421 manually verified tasks spanning 50 applications, combining ported OSWorld tasks, content from macOSWorld, and 49 new macOS-native tasks, all running on Apple's native Virtualization framework on Apple Silicon.

Result: Model rankings invert between ported and macOS-native tasks; a leading model trails by over 26% on the MacArena subset, indicating macOS poses a genuinely harder environment.

Conclusion: Strong model performance on existing benchmarks may reflect familiarity with task distributions rather than genuine cross-platform GUI competence, and macOS presents distinct GUI challenges beyond what Linux-based benchmarks capture.
...
### [171] [WAV: Multi-Resolution Block Residual Routing for Deep Decoder-Only Transformers](https://arxiv.org/abs/2606.06564)
*Kehan Wang*

Main category: cs.LG

TL;DR: WAV v1 introduces multi-resolution residual routing for deep Transformers by augmenting each block with directional detail bases (phase and split) that capture attention-vs-MLP contrast and early-vs-late sublayer dynamics, improving upon standard block-level residual routing methods like Block Attention Residuals.


<details>
  <summary>Details</summary>
Motivation: Standard PreNorm and recent routing methods (Attention Residuals, Block Attention Residuals) either use fixed weights or only capture low-frequency block-level residual summaries, missing directional structure such as attention-vs-MLP imbalance and early-vs-late block dynamics, which becomes critical as Transformers scale deeper.

Method: WAV v1 augments each block with two directional detail bases: a phase basis (contrasts attention vs. MLP updates) and a split basis (contrasts early vs. late sublayer updates). These are routed alongside standard block summaries through a depth-wise softmax mixer, with negative detail-source initialization and detached RMS matching for training stability.

Result: At 12 layers, WAV v1 is not consistently beneficial; at 24 layers it becomes competitive; at 48 layers it outperforms all baselines. Validation loss on TinyStories: 0.4738 (vs Block AttnRes 0.4960). On Text8: 0.9305 (vs 0.9363), with negligible parameter overhead.

Conclusion: Directional residual details—beyond block-level sums—are important for scaling residual routing in deeper Transformers. WAV v1 demonstrates that multi-resolution routing with phase and split information yields consistent gains at sufficient depth.
...
### [172] [Are you sure? A Comprehensive and Comprehensible Survey of Uncertainty Quantification in Symbolic Regression](https://arxiv.org/abs/2606.06567)
*Julia Reuter,Fabricio Olivetti de Franca*

Main category: cs.LG

TL;DR: This survey paper reviews the underexplored area of uncertainty quantification (UQ) in symbolic regression (SR), categorizing existing approaches into frequentist, Bayesian, and model selection methods, and highlighting the need for further research to enable reliable UQ in SR for real-world decision-making.


<details>
  <summary>Details</summary>
Motivation: Symbolic regression lacks support for uncertainty quantification (UQ), which limits its adoption in real-world decision processes where model reliability is crucial.

Method: The paper conducts a survey introducing essential UQ concepts and reviews current literature, organizing existing SR+UQ approaches into three categories: frequentist, Bayesian, and model selection.

Result: UQ in SR remains underexplored despite its importance, with three main research directions identified.

Conclusion: Further research into reliable UQ methods for SR is motivated and needed.
...
### [173] [Generative Models Erode Human Temporal Learning Through Market Selection](https://arxiv.org/abs/2606.06572)
*Wenjun Cao*

Main category: cs.LG

TL;DR: The paper argues that generative AI, even below AGI capability, creates structural risks by making it difficult to verify whether outputs reflect genuine human learning. This leads to "value collapse" where human producers who invested years of learning cannot compete economically with near-zero-cost AI outputs, across domains like academia, law, content platforms, and software security.


<details>
  <summary>Details</summary>
Motivation: To identify and formalize structural risks from generative AI on knowledge/cultural production at current capability levels, specifically how verification costs for human vs. AI outputs create market failures that devalue human learning investments.

Method: Conceptual framework introducing Human Temporal Learning (HTL) and costly-inspection model to analyze verification erosion across four domains (academic publishing, legal practice, content platforms, software security).

Result: Better-aligned AI models paradoxically worsen the problem by making source verification harder. Value collapse proceeds through four stages of verification erosion, and alignment success is orthogonal to these structural risks.

Conclusion: Generative AI creates systemic economic disincentives for genuine human learning and knowledge production that cannot be solved by better alignment alone, requiring different structural interventions.
...
### [174] [Skip a Layer or Loop It? Learning Program-of-Layers in LLMs](https://arxiv.org/abs/2606.06574)
*Ziyue Li,Yang Li,Tianyi Zhou*

Main category: cs.LG

TL;DR: LLMs can dynamically skip or repeat pretrained layers per input via a lightweight predictor (PoLar), achieving better accuracy with fewer layers than fixed-depth execution.


<details>
  <summary>Details</summary>
Motivation: Standard LLM inference executes all layers in fixed order; the authors hypothesize that more efficient, flexible execution paths exist for each input.

Method: Proposes PoLar: a lightweight prediction network that learns to generate execution programs (skipping or repeating pretrained layers) dynamically per input, without additional training of the base LLM.

Result: Outperforms standard inference and prior dynamic-depth methods on math reasoning benchmarks, often with fewer layers; gains persist under out-of-distribution evaluation.

Conclusion: Fixed-depth execution captures only a narrow subset of an LLM's latent reasoning capacity; dynamic program-of-layers unlocks more efficient and accurate inference.
...
### [175] [Gaussian Process Latent Factor Regression for Low-Data, High-Dimensional Output Problems](https://arxiv.org/abs/2606.06576)
*Edward T. Stevenson,Eric T. Wolf,Mei Ting Mak,N. J. Mayne,Miles Cranmer*

Main category: cs.LG

TL;DR: GPLFR couples compression and prediction in multi-output Gaussian processes for high-dimensional regression tasks, demonstrated on exoplanet climate modeling.


<details>
  <summary>Details</summary>
Motivation: Standard multi-output GPs struggle with high-dimensional outputs, while PCA-GP pipelines optimize bases for reconstruction rather than prediction, creating a gap.

Method: Each output is a linear-Gaussian decoding of a low-dimensional latent state from a GP prior; decoder weights are analytically marginalized to jointly optimize compression and prediction.

Result: Built the first spatially resolved emulator of global climate models for rocky exoplanets, demonstrating scalability to high-dimensional outputs.

Conclusion: GPLFR effectively couples compression and prediction in a single objective, outperforming reconstruction-optimized baselines for high-dimensional regression with limited data.
...
### [176] [Principles and Practice of Deep Representation Learning: or a Mathematical Theory of Memory](https://arxiv.org/abs/2606.06624)
*San Buchanan,Druv Pai,Peng Wang,Yi Ma*

Main category: cs.LG

TL;DR: This book aims to demystify large deep neural networks by explaining their internal mechanisms through representation learning, using optimization and information theory to transform neural architecture design from "alchemy" into understandable linear algebra and calculus principles.


<details>
  <summary>Details</summary>
Motivation: Large generative models are "black boxes" with opaque internal mechanisms, leading to difficulties in interpretability, reliability, and control. The authors aim to open this black box to reduce hype and fear stemming from lack of understanding.

Method: The book explains modern neural network design principles through optimization and information theory, reducing architecture development to undergraduate-level linear algebra and calculus. It covers design principles (Chapters 2-6) and applications (Chapters 7-8) for building efficient, interpretable, and controllable models.

Result: The proposed approach yields new methods and models that are efficient, interpretable, and controllable by design, while being no less (sometimes more) powerful than black-box models.

Conclusion: Representation learning is the single most important factor in deep learning's empirical power. The book opens the black box of neural networks and discusses future directions for deep learning and open problems (Chapter 9).
...
### [177] [The Identity Trap in EEG Foundation Models: A Diagnostic Audit](https://arxiv.org/abs/2606.06647)
*Jun-You Lin,Ying Choon Wu,Tzyy-Ping Jung*

Main category: cs.LG

TL;DR: EEG foundation models often learn subject-identity features instead of genuine clinical biomarkers, a problem called the "Identity Trap." The authors propose FMScope, a diagnostic protocol, and show this trap is universal across models and datasets, but can be detected and partially mitigated.


<details>
  <summary>Details</summary>
Motivation: High accuracy in subject-disjoint cross-validation for EEG foundation models is ambiguous—it may reflect genuine clinical biomarkers or subject-identity features correlated with labels. The authors aim to diagnose whether representations capture identity rather than disease-relevant signals before fine-tuning.

Method: FMScope: a frozen-representation protocol with five diagnostics—variance decomposition, subject-axis erasure, aperiodic 1/f ablation, layer-wise label probing, and within-subject direction consistency. Applied to three FMs (LaBraM, CBraMod, REVE) across four datasets in a 2x2 design (subject relation of label × presence of consensus cross-subject EEG marker).

Result: (i) Identity Trap is universal: frozen subject-variance is 13-89x random null in all 12 model-dataset pairs, worsening with fine-tuning (+10 to +63pp). Erasing subject axis improves label decoding where labels vary within subject (+6 to +12pp primary; +4 to +27pp external cohorts). (ii) Aperiodic 1/f is a subject carrier for LaBraM and CBraMod (removal drops subject probe 9-19pp), but not REVE. (iii) Fine-tuning amplifies label-variance only when a cross-subject biological marker exists.

Conclusion: The Identity Trap is a physically-grounded shortcut learning problem with measurable physiological components; subject-disjoint splitting alone cannot rule it out. FMScope enables separating genuine biomarker gains from subject-identity artifacts.
...
### [178] [Capturing non-Markovian dynamics in non-equilibrium stochastic systems using flow matching](https://arxiv.org/abs/2606.06658)
*Bhargav Sriram Siddani,John B. Bell,Alejandro L. Garcia,Ishan Srivastava*

Main category: cs.LG

TL;DR: The paper develops a generative flow matching method to model probability distributions of particle fluxes, capturing non-Markovian and non-Gaussian effects that standard SPDE models miss, and demonstrates improved accuracy on the Kramers first passage time problem.


<details>
  <summary>Details</summary>
Motivation: Hydrodynamic models (e.g., regularized Dean-Kawasaki equation) fail to capture short-time non-Markovian dynamics and non-Gaussian distributions in low-density regimes.

Method: A generative flow matching approach that directly models the probability distribution of fluxes from particle simulations, explicitly incorporating non-Markovian and non-Gaussian effects.

Result: The model accurately captures short-time behavior and provides better predictions of statistical moments of number density compared to the regularized Dean-Kawasaki equation.

Conclusion: Generative flow matching is an effective alternative to traditional SPDE-based hydrodynamic models for systems with significant non-Markovian and non-Gaussian dynamics.
...
### [179] [Explainable Runtime Dependency Tracking for AI-RAN Conflict Monitoring](https://arxiv.org/abs/2606.06663)
*Christie Djidjev,Nicholas Kaminski*

Main category: cs.LG

TL;DR: This paper proposes a lightweight monitoring primitive for AI-RAN that tracks Boolean matrix representations of parameter-KPI dependencies from streaming telemetry, using sliding-window inference to detect structural changes and flag inconsistencies for conflict diagnosis.


<details>
  <summary>Details</summary>
Motivation: In AI-integrated RAN, multiple xApps/rApps act on shared parameters and KPIs, and static dependency assumptions may become invalid under changing operating regimes. The system needs a lightweight method to verify whether assumed parameter-KPI dependencies remain valid.

Method: The approach represents active dependencies as a Boolean matrix and uses Boolean matrix multiplication to check consistency between parameter-activity events and KPI-response events. A sliding-window inference procedure reuses the estimate when consistent, and recomputes it upon detection of structural change.

Result: Experiments on controlled Boolean event streams demonstrate efficient and accurate tracking under dependency changes and Boolean observation noise.

Conclusion: The tracker serves as an explainable signal for conflict diagnosis and slow-loop model refresh, not as an autonomous mitigation mechanism.
...
TL;DR: ULPS integrates a calibrated LLM with uncertainty-aware blending and A*-guided symbolic trajectories to improve RL performance in sparse-reward, multi-task environments, achieving over 9% accuracy improvement on MiniGridUnlockPickup.


<details>
  <summary>Details</summary>
Motivation: Sparse rewards and heterogeneous task sequences cause slow convergence, weak generalization, and inefficient exploration in RL.

Method: ULPS uses an A*-based oracle to generate symbolic trajectories, fine-tunes a BERT-based LLM on them, then incorporates LLM action suggestions modulated by MC dropout epistemic uncertainty and adaptively blended with PPO policy via entropy-based mechanism.

Result: Over 9% improvement in execution accuracy, fewer environment interactions, higher reward AUC, and consistent improvements over baselines on MiniGridUnlockPickup.

Conclusion: Integrating symbolic A* trajectories, pretrained language priors, and uncertainty-aware control is a principled approach for multi-task RL in sparse-reward domains, with potential for POMDP and multi-agent settings.
...
TL;DR: GiFlow proposes a Graph-Informed Flow Matching framework for spatiotemporal imputation, replacing Gaussian priors with graph-informed priors and using a hybrid vector field model, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Missing data is common in spatiotemporal systems. Traditional ML methods (RNNs, GNNs) accumulate errors over time/space; diffusion-based methods require iterative sampling and use problem-agnostic Gaussian priors, limiting efficiency and effectiveness.

Method: GiFlow uses a graph-informed prior (via spatiotemporal filtering of observable signals) instead of Gaussian prior, simplifying the generation trajectory. The flow field is parameterized by a hybrid vector field model integrating spatial attention, temporal attention, and spatiotemporal propagation.
...
Conclusion: GiFlow effectively addresses limitations of prior methods by combining graph-informed priors with a hybrid vector field, achieving superior spatiotemporal imputation performance.
...
TL;DR: SSD-FL is a serverless semi-decentralized federated learning framework that forms device clusters via a one-time D2D initialization phase and uses effective loss functions with Cheeger inequality-based clustering to handle heterogeneous ML optimizers, improving convergence and communication efficiency.


<details>
  <summary>Details</summary>
Motivation: Cluster formation in decentralized FL with heterogeneous ML optimizers is underexplored, and optimizing it is challenging due to the coupling between network graphs, data heterogeneity, and different local optimizers.

Method: Proposes SSD-FL: (1) lightweight one-time D2D initialization for cluster formation; (2) intra-cluster and inter-cluster regimes with effective loss functions integrating device-specific optimizers and graph regularization; (3) iterative clustering via Cheeger inequality and consensus gap analysis; (4) scoring metric quantifying data and optimizer heterogeneity.

Result: Experimental evaluation against three categories of decentralized FL methods shows SSD-FL improves convergence speeds and communication efficiency across various network graphs, datasets, and local optimizer regimes.

Conclusion: SSD-FL effectively addresses the challenges of cluster formation in decentralized FL with heterogeneous optimizers, achieving better convergence and communication efficiency without requiring persistent server infrastructure.
...
### [183] [Time series Foundation Models based on Physics-Informed Synthetic Histories for Cold-Start Photovoltaic Forecasting](https://arxiv.org/abs/2606.07457)
*Lorenzo Longarini,Alessandro Rongoni,Simone Silenzi,Emanuele Frontoni,Riccardo Rosati*

Main category: cs.LG

TL;DR: This paper introduces a zero-shot pipeline that generates synthetic PV production history from plant metadata and weather data, enabling time-series foundation models to forecast in cold-start settings without real observations. Covariate-aware models like TabPFN-TS and Chronos-2 outperform classical baselines by 1.7-2×, with performance driven more by the availability of plausible temporal context than by the specific synthetic data generator.


<details>
  <summary>Details</summary>
Motivation: PV operators need to forecast production at commissioning time when no historical observations exist (cold-start setting), which prevents the use of standard supervised forecasters. The authors aim to address this limitation by enabling zero-shot forecasting using synthetic data and foundation models.

Method: A zero-shot pipeline generates synthetic production history from plant metadata and meteorological covariates. Five time-series foundation models (TSFMs) are benchmarked against classical baselines under three strategies: Cold-Start Baseline, Real Feedback, and Self-Forecast Feedback. Evaluation covers 440 PV sites across four datasets and diverse climate regimes.

Result: Covariate-aware foundation models outperform classical baselines by approximately 1.7-2×. TabPFN-TS achieves the lowest error under Real Feedback (MAE 0.514, RMSE 0.721 kWh kWp⁻¹ d⁻¹). Chronos-2 is most robust under Self-Forecast Feedback. Performance is largely insensitive to the synthetic-history source.

Conclusion: The proposed zero-shot pipeline effectively enables PV production forecasting in cold-start settings. Accuracy is driven more by the availability of plausible temporal context than by the specific synthetic data generator, suggesting broad applicability of this approach.
...
### [184] [The Geography of Algorithmic Judgment: LLM Intermediaries, Place Identity, and Racial Steering in Housing Search](https://arxiv.org/abs/2606.06694)
*Hana Samad,Trung Lam,Christoph Mügge-Durum,Michael Akinwumi*

Main category: cs.LG

TL;DR: This paper audits seven LLMs across four U.S. cities for racial steering in housing recommendations, finding that steering is an emergent behavior from the interaction of user identity, preference articulation, and the model's internalized spatial logic, with results varying by city and prompting condition.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs exhibit racial steering in housing recommendations, expanding on prior work by examining how identity, preference context, and city-specific spatial logic interact to produce steering behaviors.

Method: Behavioral audit of seven open-weight and closed-source LLMs across four U.S. cities, using iterative prompting with three conditions (baseline, lifestyle preference context) and reflecting fair housing paired-testing methodologies.

Result: Steering is emergent and not uniform; preference-conditioned testing increased or reconfigured steering behaviors; results do not generalize across cities; LLMs interpret same preferences differently based on user racial identity.

Conclusion: Local and domain expertise is required to ensure fair housing commitments are not undermined when adopting AI tools that mediate spatial access.
...
### [185] [RECAP: Regression Evaluation for Continual Adaptation of Prompts](https://arxiv.org/abs/2606.06698)
*Harsh Deshpande,Kushal Chawla,Sangwoo Cho,William Campbell*

Main category: cs.LG

TL;DR: RECAP benchmark evaluates proactive prompt adaptation under evolving constraints, finding current methods fail to improve performance despite higher latency, highlighting need for new approaches designed for proactive settings.


<details>
  <summary>Details</summary>
Motivation: Production agentic systems face evolving constraints (e.g., policy updates, compliance changes) with no room for errors, yet current benchmarks assume static or reactive protocols, lacking evaluation of proactive adaptation where models must adapt before seeing test data.

Method: RECAP benchmark employs a strictly proactive adapt-then-test protocol: prompt optimization methods receive constraint specifications and must generalize before encountering test data. Six methods are evaluated across four LLMs and three schedules with evolving constraints, measuring continual-learning phenomena (forgetting, regression, forward transfer) at the constraint level.

Result: Current prompt optimization methods show no significant improvement in performance under proactive adaptation, even with higher latency costs. Methods designed for offline or reactive settings are inadequate for the proactive paradigm.

Conclusion: There is a growing need for designing proactive prompt adaptation methods where models remain robust to evolving deployment needs, as existing approaches fail in this critical production scenario.
...
### [186] [ShallowBench: Benchmarking Generative Drug Design Models on Shallow-Pocket Targets](https://arxiv.org/abs/2606.06717)
*Saket Reddy,Shiwei Liu*

Main category: cs.LG

TL;DR: ShallowBench is a curated benchmark of 5,780 shallow-pocket targets for evaluating generative AI models in drug design, revealing performance gaps on low-concavity binding sites and motivating new architectural innovations.


<details>
  <summary>Details</summary>
Motivation: Generative AI models for drug design struggle with shallow-pocket (low-pocketability) targets like KRAS and MYC, which are historically "undruggable" oncology targets.

Method: The authors extracted 5,780 shallow-pocket targets from CrossDocked2020 by computing the difference between an Alpha Shape "lid" volume and the underlying protein atom voxel volume, isolating targets with low concavity but sufficient surface area for binding.

Result: State-of-the-art generative models show weaker predicted binding affinity on these low-concavity interfaces compared to deep pockets.

Conclusion: ShallowBench highlights the need for new architectural innovations or loss functions to improve generative AI performance on low-concavity, shallow-pocket targets.
...
### [187] [MSAIC-Net: A Multi-Scale Attention and Imbalance-Aware Contrastive Network for ECG-Based Myocardial Substrate Abnormality Detection](https://arxiv.org/abs/2606.06718)
*Canyu Lei,Fenglin Zhang,Derek Bivona,Cristiane Singulane,Jonathan Pan,Kenneth Bilchick,Amit R. Patel,Jianxin Xie*

Main category: cs.LG

TL;DR: MSAIC-Net is a multi-scale attention-enhanced CNN with supervised contrastive learning for detecting myocardial substrate abnormalities from ECG signals, outperforming baselines on UVA and PTB-XL datasets while offering lead-wise interpretability.


<details>
  <summary>Details</summary>
Motivation: ECG-based detection of myocardial substrate abnormalities (scar, infarction) remains challenging due to heterogeneous lead-dependent signals, high-dimensional multi-lead data, class imbalance, and poor interpretability of deep learning models.

Method: MSAIC-Net uses parallel atrous convolutional branches for multi-scale temporal feature extraction, channel attention for adaptive reweighting, imbalance-aware supervised contrastive learning for better feature separability, and lead-wise permutation importance for interpretability.

Result: The method outperforms baseline models on both UVA (low-data myocardial scar) and PTB-XL (large-scale MI) datasets, with pronounced improvements in the low-data regime.

Conclusion: MSAIC-Net provides an effective and interpretable deep learning framework for ECG-based detection of myocardial substrate abnormalities.
...
### [188] [Flatland: The Adventures of Gradient Descent with Large Step Sizes](https://arxiv.org/abs/2606.06722)
*Leonardo Galli,Curtis Fox,Wiebke Bartolomaeus,Mark Schmidt,Holger Rauhut*

Main category: cs.LG

TL;DR: This paper addresses the problem of determining the largest step size that ensures convergence of gradient descent for non-L-smooth functions. The authors propose adaptive first-order methods that operate at the edge of stability (EoS) from training start, achieving nonmonotonic loss decrease and sharpness minimization to its global minimum. They surprisingly find that early flat regions can harm convergence and generalization, and allowing slightly sharper valleys improves training success.


<details>
  <summary>Details</summary>
Motivation: Neural network training often involves objective functions that are not globally L-smooth, making it theoretically and practically difficult to determine the largest possible step size ensuring gradient descent convergence. This is a longstanding open question in deep learning that the authors aim to address.

Method: The authors provide a unifying definition of "large" step sizes requiring only local Lipschitz (or Hölder) continuity of the gradient. They design first-order adaptive methods that provably yield large step sizes and operate at the edge of stability (EoS) from the start of training, using a self-stabilization argument to allow GD to enter sharper valleys.

Result: The proposed methods achieve nonmonotonic loss decrease while keeping the product of step size and sharpness (largest Hessian eigenvalue) above the EoS threshold of 2 throughout training. The method minimizes sharpness to its global minimum. Counterintuitively, encountering globally-flat regions too early slows convergence and harms generalization.

Conclusion: Allowing GD to enter slightly sharper valleys can turn unsuccessful training runs into successful ones, challenging the conventional wisdom that flat minima are always beneficial early in training.
...
TL;DR: A method using Bayesian Probabilistic Context-Free Grammar to generate synthetic regression data that structurally mimics physics equations, validated against the Feynman corpus, achieving strong fidelity and utility in downstream hyperparameter tuning.


<details>
  <summary>Details</summary>
Motivation: Real-world data collection is often infeasible. Synthetic data generation is needed, but only if it faithfully reflects the structure of real observations.

Method: Uses a Bayesian Probabilistic Context-Free Grammar to capture algebraic structure from an equation corpus, samples novel equations, characterizes applicability domains via non-intrusive probing, and samples inputs using mixed uniform/truncated normal distributions.

Result: Generated equations match the Feynman corpus on all eight studied structural features (vs. only two for unsmoothed grammar). In hyperparameter tuning, synthetic data picks on average the 6th-best config out of 20, matching real-data tuning and far outperforming random trees (10th) and noise (19th).

Conclusion: The Bayesian prior is essential for structural fidelity given limited corpus size, and the generated synthetic data is practically useful for downstream tasks like hyperparameter tuning.
...
### [190] [TorchKM: A GPU-Oriented Library for Kernel Learning and Model Selection](https://arxiv.org/abs/2606.06742)
*Yikai Zhang,Gaoxiang Jia,Jie Ding,Boxiang Wang*

Main category: cs.LG

TL;DR: TorchKM is an open-source GPU-accelerated library for kernel machines (SVM, kernel logistic/quantile regression) with a scikit-learn-style API, achieving competitive performance and substantial speedups through efficient matrix operation reuse.


<details>
  <summary>Details</summary>
Motivation: To provide an open-source, GPU-accelerated library for kernel machines that is easy to use (scikit-learn-style API) and significantly faster than standard implementations.

Method: GPU-friendly linear algebra exploitation with intelligent reuse of matrix operations across training and model-selection pipelines, packaged with a scikit-learn-style API.

Result: Competitive predictive performance with substantial speedups over standard baselines.

Conclusion: TorchKM is a practical, high-performance kernel machine library that is readily accessible via PyPI and GitHub.
...
TL;DR: This paper introduces Learn2Match, a multi-agent reinforcement learning benchmark for dynamic matching markets with temporally extended feedback, showing that PPO outperforms bandit-style baselines in social welfare but still suffers from higher information-friction loss.


<details>
  <summary>Details</summary>
Motivation: Existing matching models reduce dynamic information to sub-Gaussian feedback about fixed preferences, missing settings where payoff-relevant information is revealed gradually over time through interviews, learning, and separation.

Method: Formulates two-sided matching as a partially observable Markov game with costly pre-match screening, noisy post-match observations, evolving latent profiles, and endogenous dissolution. Implements this as Learn2Match, a MARL benchmark supporting decentralized decisions over interviewing, matching, and dissolution.

Result: Independent PPO achieves higher cumulative social welfare and lower cumulative regret than the bandit-style CA-ETC baseline under temporally extended feedback, but incurs higher information-friction loss.

Conclusion: MARL shows promise for dynamic matching markets, but end-to-end methods lack the coordinated exploration structure of matching-bandit methods, positioning Learn2Match as a benchmark for developing adaptive, statistically disciplined, and structurally aware matching-market algorithms.
...
### [192] [Performance Variation in Deep Reinforcement Learning](https://arxiv.org/abs/2606.06746)
*Haruto Tanaka,A. Rupam Mahmood*

Main category: cs.LG

TL;DR: This paper identifies limitations in conventional uncertainty estimates for deep RL run-to-run robustness and proposes percentile-based tools (min-max IPR and run-wise percentile highlighting) for better evaluating performance variation, demonstrated through three case studies on PPO, SAC, TD-MPC, and DQN/Rainbow.


<details>
  <summary>Details</summary>
Motivation: Deep RL algorithms suffer from low run-to-run robustness (significant performance variation across independent runs), yet few methods exist to properly evaluate this issue. Conventional uncertainty and variation estimates are misaligned with purpose and risk underreporting.

Method: The authors propose percentile-based statistics and visualization: (1) min-max IPR (inter-percentile range) for quantification, and (2) run-wise percentile highlighting for visualization. These rely on standard sample percentile properties and are demonstrated through three case studies.

Result: (1) LayerNorm and penultimate-layer normalizations narrow performance variation in PPO but not in SAC. (2) TD-MPC shows the least variation and highest data efficiency among PPO, SAC, TD-MPC, and TD-MPC2. (3) DQN and Rainbow exhibit similar performance variation levels across five Atari environments.

Conclusion: The proposed percentile-based tools (min-max IPR and run-wise percentile highlighting) are easy to interpret and provide richer information about run-to-run performance variation compared to conventional uncertainty estimates.
...
### [193] [A Rolling-Window Framework for Churn Prediction and Behavioral Driver Identification](https://arxiv.org/abs/2606.06776)
*Muhammad Jawad Mufti,Omar Hammad,Haitham Saleh,Muqaddas Gull*

Main category: cs.LG

TL;DR: The paper proposes a temporally explicit churn prediction framework using rolling 30-day behavioral windows, achieving 87.6% accuracy (feature-based) and 96.1% recall (sequence-based) on a real-world non-contractual service dataset, demonstrating that careful temporal framing is more critical than model complexity for robust deployment.


<details>
  <summary>Details</summary>
Motivation: Customer churn prediction in non-contractual, pay-per-use services is challenging because disengagement is not explicitly observed and must be inferred from behavioral inactivity. Existing approaches rely on simplified temporal assumptions or single-point representations, limiting continuous risk assessment, interpretability, and realistic deployment.

Method: The framework uses rolling behavioral windows: a fixed 30-day observation window followed by a 30-day future churn evaluation window. It integrates both feature-based and sequence-based learning approaches within a unified temporal design, applied to a large-scale real-world dataset from a non-contractual service platform.

Result: Feature-based model achieved 87.6% accuracy and 0.94 ROC-AUC. Sequence-based model achieved 96.1% recall by capturing temporal disengagement patterns. On future unseen data, accuracy remained above 83% and ROC-AUC above 0.91 without retraining.

Conclusion: Carefully designed temporal framing, rather than model complexity alone, is critical for achieving robust, interpretable, and deployment-ready churn prediction. The study provides a practical foundation for churn-oriented decision support in dynamic service environments.
...
### [194] [Federated Foundation Models over Vehicular Networks](https://arxiv.org/abs/2606.06786)
...
TL;DR: This paper proposes integrating multi-modal multi-task federated foundation models (M3T FedFMs) into vehicular networks, discusses use cases, identifies deployment challenges, and validates feasibility through a Waymo dataset case study.


<details>
  <summary>Details</summary>
Motivation: To combine the expressive power of multi-modal multi-task foundation models with the privacy-preserving distributed learning of federated learning for vehicular intelligence, an area that is largely underexplored.

Method: Introduces training/fine-tuning principles of M3T FedFMs, discusses use cases in vehicular networks, identifies environmental constraints, and conducts a case study on the Waymo Open Dataset.

Result: Demonstrates the promise of M3T FedFMs for vehicular networks through a real-world dataset case study, with open-source implementation released for reproducibility.

Conclusion: M3T FedFMs show significant potential to enable next-generation vehicular intelligence, but practical deployment faces key constraints in vehicular environments that require further research.
...
### [195] [Interpreting Learning Under Competing Models: Joint and Stepwise Approaches for Dynamic Cognitive Diagnosis](https://arxiv.org/abs/2606.06804)
*Yawen Ma,Sahoko Ishida,Kate Cain,Gabriel Wallin*

Main category: cs.LG

TL;DR: This paper shows that whether you estimate the Q-matrix (item-skill structure) jointly with learning or separately can change conclusions about how reading skills develop in young learners, with joint analysis being more reliable when the Q-matrix is uncertain and items change over time.


<details>
  <summary>Details</summary>
Motivation: To determine whether estimating the Q-matrix jointly with the learning process versus separately changes substantive conclusions about how learners develop skills over time.

Method: Dynamic cognitive diagnostic models applied to reading game data (vocabulary & comprehension, Grade 2-3) with item-text embeddings as priors; joint analysis vs. bias-corrected stepwise analysis, plus simulation study.

Result: Both analyses agree most learners master both skills, but disagree on the proportion remaining partially proficient at Grade 3. Simulation shows joint analysis is more reliable when Q-matrix is uncertain and item pool changes across grades.

Conclusion: The choice between joint and stepwise Q-matrix estimation can change substantive conclusions about learning trajectories; joint analysis is recommended under item-skill uncertainty and changing item pools.
...
### [196] [SCALE: Scalable Cross-Attention Learning with Extrapolation for Agentic Workflow Scheduling](https://arxiv.org/abs/2606.06820)
*Zhifei Xu,Jierui Lan,Zixuan Liang,Aiji Liang,Jinxi He*

Main category: cs.LG

TL;DR: SCALE is a DRL scheduler using cross-attention and Structured Representation Regularization to generalize LLM workflow scheduling to unseen cluster sizes without retraining, reducing response time by 8.9%.


<details>
  <summary>Details</summary>
Motivation: Existing DRL schedulers are tied to fixed cluster sizes and require retraining when server count changes, lacking scalability.

Method: SCALE uses a cross-attention pointer network (task queries against server features) plus Structured Representation Regularization (SRR) combining decorrelation loss and KL penalty toward standard normal to stabilize feature distributions across varying input sizes.

Result: Trained on 16 nodes, tested on 32 and 48 nodes; reduces average response time by 8.9% at N=48 compared to the same architecture without SRR.

Conclusion: Permutation-invariant architecture alone is insufficient for scale generalization; explicit regularization (SRR) is necessary to close the scale-generalization gap.
...
### [197] [PandaAI: A Practical Agent CQ2 for Neuro-symbolic Data Analysis And Integrated Decision-Making in Quantitative Finance](https://arxiv.org/abs/2606.06823)
*Yuqi Li,Siyuan Liu,Bingjun Liu*

Main category: cs.LG

TL;DR: PandaAI is a closed-loop neuro-symbolic LLM agent for financial sequential decision-making that uses market regime modeling and constrained alpha generation, achieving 18.2% higher Rank IC and 25.7% lower maximum drawdown on CSI 300 data.


<details>
  <summary>Details</summary>
Motivation: Deep learning struggles with financial sequential decision-making due to low SNR and non-stationarity of financial data, while LLMs offer reasoning capabilities but lack financial rigor and may produce toxic outputs.

Method: PandaAI combines a fine-tuned domain-specific LLM with a modular closed-loop neuro-symbolic architecture, incorporating market regime modeling, constrained alpha generation, and dual-channel adaptation to bridge general LLM reasoning with financial rigor.

Result: On CSI 300 stock data, PandaAI achieves 18.2% higher Rank IC and 25.7% lower maximum drawdown than state-of-the-art time-series models.

Conclusion: Constrained LLM generation and dual-channel adaptation provide a general paradigm for deploying LLMs in high-stakes sequential decision-making scenarios.
...
### [198] [Architecture Shapes Transfer Specificity in Implicit Neural Representations](https://arxiv.org/abs/2606.06827)
*D Yang Eng*

Main category: cs.LG

TL;DR: This paper systematically evaluates transfer learning in coordinate networks (SIREN, ReLU MLPs, Fourier-feature MLPs) and finds that transfer magnitude alone is misleading—ReLU transfers less but is far more source-selective, whereas SIREN often reuses weights broadly. The authors advocate for using transfer specificity as a diagnostic and evaluating architectures under controlled conditions.


<details>
  <summary>Details</summary>
Motivation: To clarify whether measured transfer gains in coordinate networks reflect source-specific structure learning or generic weight reuse, and to propose better evaluation practices.

Method: Controlled analytic tests (independent-seed random controls), a 2D lid-driven-cavity Navier-Stokes benchmark, and 1D PDE suites (heat, Burgers, cubic NLS) across three INR families (SIREN, ReLU MLPs, Fourier-feature MLPs).

Result: Fourier Features show the largest structured transfer (33.1×) but ReLU is most selective (random-control transfer 0.41× vs 14.24× for SIREN). No single architecture dominates across PDEs; SIREN broadly reuses weights while ReLU and Fourier Features are more source-selective. The heuristic scaling law A_transfer ∝ 1/Δt² is rejected.

Conclusion: Transfer specificity is a useful diagnostic. Architecture selection in scientific ML should be evaluated under explicit control conditions, not by transfer magnitude alone.
...
### [199] [Hearing the Unspoken: Language Model Priors for Acoustic Adversarial Attacks](https://arxiv.org/abs/2606.06833)
*Jiani Xie,Andrew C. Cullen,Paul Montague,Benjamin I. P. Rubinstein*

Main category: cs.LG

TL;DR: Real-time ASR systems have a causal information bottleneck that limits attack effectiveness. The Semantic Gambit attack breaks this by using LLM-predicted context to boost Word Error Rate to 35.6%—three times higher than prior SOTA.


<details>
  <summary>Details</summary>
Motivation: Real-time ASR systems process input under temporal constraints where decisions are made on incomplete information, creating a causal bottleneck that limits adversarial attacks. The authors aim to break this limitation to show real-time ASR pipelines can be subverted.

Method: Semantic Gambit attack augments the adversary with predictive context derived from a Large Language Model in real-time, overcoming the causal information bottleneck inherent to real-time ASR.

Result: Corpus-level Word Error Rate reached 35.6%—a three-fold increase over the current state-of-the-art attack.

Conclusion: Common low-latency LLM tooling can be systematically exploited to subvert real-time ASR pipelines at an unprecedented level.
...
### [200] [The Geometry of Last-Layer Model Stealing](https://arxiv.org/abs/2606.06854)
*Snigdha Chandan Khilar*

Main category: cs.LG

TL;DR: This paper uses geometry to analyze the limits of stealing machine learning models, showing that while the final layer of a transformer can be perfectly copied under certain conditions, hidden layers cannot be fully reverse engineered from final outputs.


<details>
  <summary>Details</summary>
Motivation: To understand the exact conditions and limits of model stealing attacks, mapping out what can and cannot be stolen from a machine learning model.

Method: Geometric analysis to study the feasibility of copying transformer layers, examining both final and hidden layers.

Result: The final layer can be perfectly copied under specific conditions, but hidden layers cannot be fully reverse engineered from final outputs.

Conclusion: There are clear boundaries to model stealing: final layers are vulnerable, but hidden layers remain protected from complete reverse engineering.
...
### [201] [Modeling Nonlinear Feature Interactions with Product-Unit Residual Networks](https://arxiv.org/abs/2606.06861)
*Ziyuan Li,Uwe Jaekel,Babette Dellen*

Main category: cs.LG

TL;DR: PURe networks integrate multiplicative product units with residual connections to explicitly model nonlinear feature interactions, achieving competitive accuracy, better robustness to noise, improved sample efficiency, and more interpretable interaction patterns compared to standard MLPs.


<details>
  <summary>Details</summary>
Motivation: Standard MLPs capture nonlinear feature interactions only implicitly, leading to entangled representations that impair robustness and interpretability. The authors aim to explicitly model cross-feature couplings while stabilizing optimization.

Method: Product-Unit Residual Networks (PURe) integrate multiplicative product units with residual connections. Evaluated on synthetic benchmarks and real-world datasets, compared against real- and complex-valued variants under matched parameter budgets, using SHAP for interaction analysis.

Result: PURe achieves competitive or improved predictive accuracy, better robustness to Gaussian feature noise, and superior sample efficiency under limited training data. SHAP analysis shows PURe learns more concentrated and structurally coherent interaction patterns than MLP baselines.

Conclusion: PURe offers a promising alternative to standard MLPs for applications where explicit modeling of nonlinear feature interactions, robustness, sample efficiency, and interpretability are important.
...
### [202] [Product units in gated recurrent units improve nuclear-mass prediction](https://arxiv.org/abs/2606.06866)
*Ziyuan Li,Paulo S. A. Freitas,John W. Clark,Babette Dellen*

Main category: cs.LG

TL;DR: A complex-valued gated recurrent unit (GRU) with additive-multiplicative product units (AM-PU-GRU) achieves state-of-the-art nuclear mass prediction, outperforming other ML models with interpolation RMSE of 0.227 MeV and extrapolation RMSE of 0.179 MeV on AME2016/AME2020 data.


<details>
  <summary>Details</summary>
Motivation: Machine learning can complement theoretical models to predict atomic nuclear masses, especially in poorly known regions of the nuclear chart, where traditional methods may be limited.

Method: Proposes a complex-valued AM-PU-GRU that integrates multiplicative interactions and product-unit transformations within recurrent units, operating in the complex domain to jointly capture amplitude and phase dynamics.

Result: Achieves interpolation RMSE of 0.227 ± 0.004 MeV and extrapolation RMSE of 0.179 ± 0.015 MeV, outperforming state-of-the-art ML models, real-valued GRU baselines, and product-unit ablation variants, while remaining robust to different theoretical priors (WS4, SEMF).
...
### [203] [Evidence-Grounded Ensemble Diagnosis of 802.11 Packet Captures: A Multi-Stage Pipeline with Deterministic Reliability Scoring](https://arxiv.org/abs/2606.06871)
*Jerome Henry,Swadhin Pradhan,Miroslav Popovic*

Main category: cs.LG

TL;DR: PROBE is a multi-stage pipeline for diagnosing 802.11 packet captures that addresses LLM failures like hallucination, uncalibrated confidence, and evaluation bias. It achieves 0.957 weighted evidence F1 (vs 0.871 expert baseline) and 96% auto-accept rate using deterministic normalization, multi-run ensembles, verdict-aware evidence, and a composite reliability score without LLM self-assessment.


<details>
  <summary>Details</summary>
Motivation: Diagnosing 802.11 packet captures is slow, inconsistent, and requires expert protocol knowledge. Existing LLM approaches fabricate events, produce uncalibrated confidence scores, and suffer evaluation bias from model-co-produced golden references.

Method: PROBE integrates: (i) deterministic PCAP-to-text normalization with frame-level verifiability, (ii) multi-run multi-candidate ensembles with optional cross-model second opinion and progressive obfuscation, (iii) verdict-aware evidence framework treating absence of failure evidence as contributing evidence, and (iv) a composite reliability score from evidence validity, run-to-run stability, and cross-model agreement.

Result: On 87 enterprise Wi-Fi captures, PROBE achieves 0.957 weighted evidence F1 (vs 0.871 expert baseline), 96% auto-accept rate, and worst-case floor above 0.70. Single-pass LLM misses critical frames in 35% of cases. Naive ensemble voting drops to 0.842 F1. LLM self-reported confidence is uninformative (71% report exactly 0.95).

Conclusion: PROBE effectively addresses LLM failures in protocol diagnosis through evidence-grounded reconciliation, achieving superior performance over both expert baselines and naive LLM approaches while introducing a model-agnostic evaluation framework to eliminate circular bias.
...
### [204] [GlucoFM-Bench: Benchmarking Time-Series Foundation Models for Blood Glucose Forecasting](https://arxiv.org/abs/2606.06881)
*Baiying Lu,Zhaohui Liang,Ryan Pontius,Shengpu Tang,Temiloluwa Prioleau*

Main category: cs.LG

TL;DR: GlucoFM-Bench benchmarks 8 time-series models (including foundation models like Chronos-2, TimesFM vs. LSTM) on 15 diabetes datasets with 1,117 individuals. Pre-trained TSFMs excel in zero/few-shot settings (within 5% of best supervised), but a lightweight LSTM outperforms them by 4-21% when full training data is available. Challenges persist in T1D and extreme glucose ranges.


<details>
  <summary>Details</summary>
Motivation: Blood glucose forecasting is critical for diabetes management, but time-series foundation models (TSFMs) remain understudied in this domain despite their success in other areas. This paper aims to bridge that gap by systematically evaluating TSFMs against traditional deep learning models.

Method: The authors construct GlucoFM-Bench, evaluating 8 models (pre-trained TSFMs, time-series LLMs, supervised deep learning models) across 15 public datasets (1,117 individuals). Models are tested under zero-shot, few-shot, and full-shot protocols with varying context lengths and prediction horizons.

Result: Pre-trained TSFMs (Chronos-2, TimesFM) achieve strong zero/few-shot performance (within 5% of best supervised model). However, with abundant task-specific data, a lightweight LSTM outperforms TSFMs by 4-21%. Stratified analysis shows persistent challenges in T1D cohorts and extreme glucose ranges.

Conclusion: GlucoFM-Bench provides a standardized benchmark for glucose forecasting. While TSFMs offer strong transfer capabilities, task-specific models (LSTM) still dominate when data is plentiful. Evaluation beyond aggregate metrics is needed, especially for T1D and hypo-/hyperglycemic ranges.
...
### [205] [Data-Constrained Language Model Pretraining: Improved Regularization and Scaling Laws](https://arxiv.org/abs/2606.06888)
*Zhiwei Xu,Shihao Wu,Hanseul Cho,Wei Hu,Yixin Wang*

Main category: cs.LG

TL;DR: This paper studies data-constrained pretraining with two contributions: (1) Masked-input regularization (MIR) improves autoregressive language models under data scarcity, and (2) SoftQ, a new scaling law that better captures model-data interaction under repeated epochs, showing MIR is equivalent to ~1.3x more unique data.


<details>
  <summary>Details</summary>
Motivation: As training compute grows faster than natural language data supply, pretraining will enter a data-constrained, compute-rich regime where models train for multiple epochs over finite datasets. Classical scaling laws (e.g., Chinchilla) assume abundant data and a single pass, making them misspecified for this scenario.

Method: (1) Masked-input regularization (MIR): an auxiliary next-token prediction loss on randomly masked inputs, tested on 72M to 1.4B parameter models, combined with strong weight decay. (2) SoftQ: a scaling law that couples model size and data size multiplicatively to capture their interaction under repeated data, as opposed to additive forms like Chinchilla.

Result: MIR improves validation loss over strong weight decay alone, with downstream gains at 1.4B. SoftQ fits data-constrained experiments substantially better than classical alternatives, and estimates MIR's gains as equivalent to roughly 1.3 times more unique training data.

Conclusion: Data-constrained pretraining requires new regularization and scaling approaches. MIR provides a simple, inference-free improvement, and SoftQ offers a better scaling law for this regime. Code is released.
...
### [206] [GRASP: Geometry-aware Residual Alignment for Scalable Pretraining Data Attribution](https://arxiv.org/abs/2606.06892)
*Yue Min,Ruining Chen,Yujun Li*

Main category: cs.LG

TL;DR: GRASP introduces an interaction-aware attribution method that models subset-level dynamics (e.g., redundancy, complementarity) using a quadratic geometric penalty, achieving superior counterfactual fidelity and efficiency at pretraining scale.


<details>
  <summary>Details</summary>
Motivation: Existing scalable data attribution methods rely on additive assumptions (assigning isolated scores to individual examples), which fundamentally fail to capture critical subset dynamics like data redundancy and complementary coverage.

Method: GRASP reframes attribution as subset-level counterfactual utility prediction. It uses a quadratic geometric penalty grounded in a theoretical smoothness lower bound to model subset interactions, coupled with low-dimensional feature sketches and a strictly finite lower-confidence bound selection protocol for efficiency without hidden oracle tuning.

Result: GRASP more than doubles task-level rank correlation for counterfactual subset fidelity while reducing upfront artifact construction costs by nearly an order of magnitude. Transfers effectively to language model curation and cross-domain vision selection.

Conclusion: GRASP provides a robust, scalable foundation for optimizing massive pretraining corpora by explicitly modeling subset interactions, outperforming existing baselines decisively.
...
### [207] [TALAN: Task-Aligned Latent Adaptation Networks for Targeted Post-Training of Large Language Models](https://arxiv.org/abs/2606.06902)
*Chengkai Zhang,Ziteng Liu,Junpu Wang,Zeyi Tao,Yang Wang,Sagar Chordia,Qin Huang*

Main category: cs.LG

TL;DR: TALAN inserts a latent side path into transformer residual streams, compressing sequence information into task-aligned perturbations that complement low-rank adapters, improving STEM/code reasoning by ~1.4-1.9 points across Qwen3 backbones with <1% parameter overhead and ~1.01-1.02x inference cost.


<details>
  <summary>Details</summary>
Motivation: Existing post-training methods face a trade-off: low-rank adapters (LoRA/DoRA) are efficient but task-global and cannot adapt to individual inputs, while activation interventions are input-aware but require separate probes, vectors, or inference-time steering, complicating the training pipeline.

Method: TALAN inserts a sequence-conditioned latent side path into a transformer's residual stream, compressing the active sequence into latent memory, remixing it into token-level perturbations, and writing them back via controlled residual updates. It is co-trained with a low-rank adapter in one SFT loop, configurable along six axes: insertion location, memory size, mixer, writeback rule, trainability scope, and gradient scale.

Result: Across Qwen3 backbones and four STEM/code benchmarks: +1.41 mean gain over LoRA (positive on all 4 backbones, non-negative on all 16 model-benchmark cells); +1.85 mean gain over DoRA (positive on all 4 backbones, 13/16 cells). Inference overhead is only 1.01-1.02x versus matched LoRA with <1% extra parameters. Positive transfer also seen on Llama-3.2-1B.

Conclusion: TALAN provides a practical, lightweight platform for input-aware activation-level adaptation within standard adapter-based post-training. The adapter update is 80-1,700x larger than TALAN's perturbation, yet their directions have near-zero cosine, suggesting the small orthogonal perturbation propagates and amplifies through depth to yield meaningful improvements.
...
### [208] [The Fine-Tuning Trap: Evaluating Negative Transfer and the Role of PEFT in Sub-1B Mathematical Reasoning](https://arxiv.org/abs/2606.06920)
*Rahul Nair,Chun Tao*

Main category: cs.LG

TL;DR: This paper benchmarks sub-1B SLMs on math reasoning and finds Full Fine-Tuning harms models under 300M parameters via negative transfer, making PEFT (LoRA/DoRA) a stability requirement, not just efficiency choice.


<details>
  <summary>Details</summary>
Motivation: To identify effective fine-tuning strategies for edge-deployed SLMs (<1B parameters) that adapt to new tasks without degrading general capabilities, especially given the vulnerability of small models to catastrophic forgetting.

Method: Benchmark five sub-1B models (135M-1B) on mathematical reasoning tasks (GSM8K, OrcaMath), comparing Full Fine-Tuning vs. PEFT methods (LoRA, DoRA) and 5-shot In-Context Learning.

Result: Full FT causes negative transfer in models <300M parameters, dropping below zero-shot baselines. DoRA excels in complex reasoning (GSM8K), LoRA dominates pattern matching (OrcaMath). On aligned 0.5B models, LoRA outperforms Full FT; on 135M models, even 5-shot ICL beats Full FT.

Conclusion: Recommend defaulting to PEFT for all aligned sub-1B models and caution against Full FT for any architecture smaller than 500M parameters to prevent catastrophic forgetting.
...
### [209] [From Sampled Outcomes to Capability Distributions: Rethinking Supervision for LLM Routing](https://arxiv.org/abs/2606.06924)
*Guannan Lai,Haoran Hu,Long Chen,Zhenguo Li,Han-Jia Ye*

Main category: cs.LG

TL;DR: Existing LLM routing uses single-response labels which are noisy due to LLM stochasticity. DARS proposes distribution-aware supervision considering input (semantic variants) and output (generation variance) uncertainty, producing more reliable routing labels and improving routing policy performance across diverse tasks.


<details>
  <summary>Details</summary>
Motivation: Single-shot supervision (one response per query-model pair) is unreliable for training LLM routers because LLM generation is inherently stochastic, introducing systematic noise into routing labels.

Method: DARS builds routing supervision from a distributional view of model behavior, capturing uncertainty from both input side (semantically equivalent query formulations) and output side (stochastic generations), then constructs more reliable supervision signals.

Result: Single-shot labels are misleading for model selection; distribution-aware supervision provides more stable labels and improves learned routing behavior across diverse tasks.
...
### [210] [Uniform Stability and Generalization Error of GD and SGD on Fixed-Point Parameters](https://arxiv.org/abs/2606.06934)
*Jonghyun Shin,Sejun Park*

Main category: cs.LG

TL;DR: This paper analyzes how deterministic and stochastic rounding in gradient descent (GD) and stochastic gradient descent (SGD) affects generalization error, uniform stability, and uniform argument stability over discrete parameter spaces.


<details>
  <summary>Details</summary>
Motivation: To understand how quantization/rounding—common in low-precision and edge computing—impacts generalization performance of optimization algorithms, beyond the well-studied real-valued setting.

Method: Theoretical analysis of generalization error, uniform stability, and uniform argument stability for GD and SGD with deterministic or stochastic rounding on convex, Lipschitz, smooth loss functions over discrete parameter spaces, with matching upper and lower bounds.

Result: (1) Deterministic rounding degrades GD's generalization error from O(T/n) to O(T/√n) with matching lower bounds; uniform stability becomes Ω(T) (vacuous). (2) SGD with deterministic rounding yields tight O(T/n) (1D) and O(T²/n) (higher dim) bounds, qualitatively different from real-valued case. (3) Stochastic rounding introduces dimension-dependent generalization error, absent in real-valued or deterministic rounding cases. (4) Tight uniform argument stability bounds for stochastic rounding schemes when losses are coordinate-wise separable.

Conclusion: Rounding schemes fundamentally alter the generalization landscape of gradient-based optimization, with deterministic rounding harming GD stability but less so for SGD, while stochastic rounding introduces novel dimension-dependent generalization issues.
...
### [211] [GenPO++: Generative Policy Optimization with Jacobian-free Likelihood Ratios](https://arxiv.org/abs/2606.06967)
*Ke Hu,Shutong Ding,Panxin Tao,Jingya Wang,Ye Shi*

Main category: cs.LG

TL;DR: GenPO++ proposes a reversible generative policy optimization framework using high-order reversible ODE solvers with history states as auxiliary memory, enabling exact, Jacobian-free likelihood-ratio computation for flow-based RL without dummy-action augmentation or biased surrogates.


<details>
  <summary>Details</summary>
Motivation: Existing flow-based RL methods either use approximate surrogates (biased updates) or dummy-action augmentation (increased computation). A method preserving exact likelihoods without overhead is needed.

Method: Uses history states as auxiliary memory in a high-order reversible ODE solver to achieve exact inversion of the generative flow policy without changing the original action dimension, yielding log-determinant determined solely by fixed solver coefficients.

Result: Achieves competitive or superior performance over state-of-the-art on-policy RL methods on large-scale simulated control, fine-tuning, and real-world robotic manipulation tasks, with improved training stability and computational efficiency.

Conclusion: GenPO++ effectively combines the expressiveness of generative flow policies with exact likelihood computation, eliminating both action ratio bias and dummy-action overhead, leading to better practical RL performance.
...
### [212] [Accelerating Multi-Objective Bayesian Optimisation via Predictive-Gradient Catalysts](https://arxiv.org/abs/2606.06984)
*Alma Rahat,Tinkle Chugh,Jonathan Fieldsend,Richard Allmendinger*

Main category: cs.LG

TL;DR: This paper introduces predictive gradient catalysis to accelerate multi-objective Bayesian optimisation by augmenting acquisition functions with local stationarity information from Gaussian process gradients, with experiments showing significant speedup on DTLZ benchmarks.


<details>
  <summary>Details</summary>
Motivation: To accelerate multi-objective Bayesian optimisation convergence toward the global Pareto set under limited evaluation budgets, beyond what existing Pareto-compliant acquisition functions alone can achieve.

Method: Augments existing acquisition functions with Gaussian process predictive gradients as auxiliary signals, using two catalyst variants: adaptive MGDA and a predefined-weight variant for budget-constrained focused exploration.

Result: On DTLZ benchmarks (2 objectives, 10 decision variables), predictive gradient catalysis significantly accelerates convergence compared to EHVI, AugTch, tMPoI, and SAF, especially when surrogates are accurate on stationary problems.

Conclusion: Gradient-based catalysis is an effective general mechanism for accelerating MOBO without replacing existing acquisition functions, particularly effective for stationary problems with accurate surrogates.
...
### [213] [Heterogeneous Effects of Green Finance on Urban Decarbonization: Evidence from 285 Cities in China](https://arxiv.org/abs/2606.06986)
*Xueyang Li,Jinlei Ma*

Main category: cs.LG

TL;DR: Green finance significantly reduces city-level carbon intensity, especially through green bonds and investment, with strongest effects in lower-tier cities and via energy structure optimization.


<details>
  <summary>Details</summary>
Motivation: Green finance is promoted for low-carbon city transitions, but its actual decarbonization effects and transmission mechanisms remain unclear.

Method: Econometric models combined with machine learning-based analysis (including SHAP) to examine green finance's impact on city-level carbon intensity.

Result: Green finance significantly lowers carbon intensity; green bonds and green investment have strongest impacts with spatial spillovers; effects vary by city tier; operates mainly through energy structure optimization; marginal impact stronger in cities with low tech, high industrial dependency, and coal-based energy.

Conclusion: Provides theoretical support and policy guidance for building a multi-level, regionally differentiated green finance system to promote inclusive low-carbon transitions.
...
### [214] [Accelerating Reproducible Research in Synthetic EHR Generation](https://arxiv.org/abs/2606.06990)
*Jalen Jiang,Chufan Gao,Ethan Rasmussen,Stephen Z. Xie,Jimeng Sun*

Main category: cs.LG

TL;DR: This paper introduces a lightweight, end-to-end benchmarking framework for reproducible synthetic EHR evaluation, unifying disjointed codebases and evaluation protocols. It reimplements strong baselines (MedGAN, CorGAN, PromptEHR, HALO, GPT-2) under a single pipeline built on PyHealth, contributes a rigorous privacy-utility evaluation suite with bootstrapped confidence intervals, and analyzes the poor long-tailed performance of existing models for longitudinal ICD diagnosis code generation.


<details>
  <summary>Details</summary>
Motivation: Head-to-head comparison of existing generative models for synthetic EHR is hindered by disjointed codebases, incompatible data loaders, conflicting library dependencies, and inconsistent evaluation protocols, creating a need for a unified benchmarking framework.

Method: The authors build a unified pipeline on PyHealth spanning data ingestion, standardized model training, and architecture-agnostic evaluation. They reimplement MedGAN, CorGAN, PromptEHR, HALO, and add a GPT-2 baseline under full ICD-9 vocabulary granularity.

Result: They contribute a rigorous privacy-utility evaluation suite with bootstrapped confidence intervals applied identically across GAN and transformer models, and identify poor long-tailed performance issues in existing models.

Conclusion: The framework lowers engineering barriers for running, extending, and evaluating synthetic EHR models under a single pipeline, serving as a starting point for community-driven reproducibility and benchmarking.
...
### [215] [RASFT: Rollout-Adaptive Supervised Fine-Tuning for Reasoning](https://arxiv.org/abs/2606.07006)
*Yongliang Miao,Fengyuan Liu,Wei Shi,Yanguang Liu,Fei Sun,Na Zou,Mengnan Du*

Main category: cs.LG

TL;DR: RASFT adaptively adjusts supervision from expert demonstrations based on the current policy's problem-solving capability, combining expert guidance with self-generated correct trajectories to improve reasoning performance beyond standard SFT.


<details>
  <summary>Details</summary>
Motivation: Standard SFT for reasoning tasks rigidly imitates single expert trajectories, which can overfit to surface forms and suppress the model's own reasoning distribution.

Method: RASFT calibrates expert supervision per problem using verified on-policy rollouts: strengthens guidance when policy struggles, relaxes imitation and uses self-generated correct trajectories when reliable, plus clipped inverse ratio to prevent policy drift.

Result: Outperforms SFT, SFT variants, and representative RL methods across 6 math and 2 code reasoning benchmarks on multiple models.

Conclusion: Policy-aware adaptive supervision is more effective than rigid expert imitation for fine-tuning LLMs on reasoning tasks.
...
### [216] [A Geometric View for Understanding Concept Learning and Neuron Interpretation in Sparse Autoencoders](https://arxiv.org/abs/2606.07007)
*Chenhao Zhang,Chris Lin,Su-In Lee*

Main category: cs.LG

TL;DR: This paper proposes a mathematical framework for understanding concept learning and neuron interpretation in sparse autoencoders (SAEs), formalizing concepts as data sets and learning as a set-alignment problem with three levels (detection, separation, approximation), and connects concept learning to formal concept analysis.


<details>
  <summary>Details</summary>
Motivation: SAEs improve interpretability of neural networks by learning sparse feature representations, but a principled definition of "concept" and "learning" remains unclear.

Method: Formalizes concepts as sets of data points, casts concept learning as a set-alignment problem between human-defined and model-induced concepts, defines three levels of learning (detection, separation, approximation), and connects to formal concept analysis via concept lattices.

Result: Provides geometric conditions, error bounds, capacity constraints for concept representation; accounts for feature splitting, absorption, families, and hierarchies; shows learning and interpretation need not agree; validated on synthetic data with ReLU and Top-K SAEs.

Conclusion: The framework offers a principled mathematical foundation for SAE-based concept learning and neuron interpretation, revealing many-to-many structures and the impact of SAE size and sparsity.
...
### [217] [CF-JEPA: Mask-free forward prediction with asymmetric encoder utilization for time-series representation learning](https://arxiv.org/abs/2606.07031)
*Jaehoon Lee,Sunghyun Sim*

Main category: cs.LG

TL;DR: CF-JEPA is a mask-free self-supervised framework for time-series that uses multi-horizon forward prediction on random crops instead of masking, and exploits asymmetry between online encoder (discriminative, for classification) and EMA target encoder (smooth temporal features, for forecasting/anomaly detection), achieving state-of-the-art across 152 datasets.


<details>
  <summary>Details</summary>
Motivation: Existing SSL for time-series either struggles with contrastive pair construction or disrupts temporal continuity via masking. JEPA predicts in representation space but inherits masking issues. CF-JEPA aims to avoid masking entirely while preserving temporal structure.

Method: Replaces masking with random crops as context views and predicts future representations at short, mid, and long horizons in the forward temporal direction. Identifies asymmetry between online encoder (higher-rank discriminative features) and EMA target encoder (smoother, lower-rank temporal features) from a single training run.

Result: 27% reduction in multivariate forecasting MSE at no extra cost. Highest average accuracy/rank on UCR (126) and UEA (26) among SSL baselines. Ranks second on univariate forecasting and k-NN anomaly detection.

Conclusion: CF-JEPA demonstrates that mask-free forward prediction in representation space is effective for time-series SSL, and routing tasks to the appropriate encoder (online vs. EMA) based on learned feature characteristics yields significant gains without additional training.
...
### [218] [Hierarchical Forecast Reconciliation for Urban Rail Transit Demand Prediction under Operational Disruptions](https://arxiv.org/abs/2606.07044)
*Dang Viet Anh Nguyen,Alma Fazlagic,Kristine Pryds Loft,Filipe Rodrigues*

Main category: cs.LG

TL;DR: This paper proposes a neural Fully Connected Reconciler (FCR) for hierarchical forecast reconciliation of station-level and OD-level passenger demand in Urban Rail Transit, demonstrating improved accuracy and coherence especially under disruptions.


<details>
  <summary>Details</summary>
Motivation: Existing station-level and OD-level forecasts are generated independently, producing incoherent predictions that violate conservation constraints. This issue worsens during disruptions when forecasting reliability is most critical.

Method: A neural Fully Connected Reconciler (FCR) is introduced to learn a non-linear mapping from incoherent base forecasts to coherent hierarchical predictions, guaranteeing exact structural consistency by construction.

Result: FCR performs competitively with MinT-based methods under normal conditions. Under severe disruptions, FCR reduces OD forecasting error by up to 17.45%. An oracle analysis shows perfect station-level forecasts could reduce OD error by up to 34%.

Conclusion: Hierarchical reconciliation is an effective mechanism for improving forecast robustness, with the largest benefits under the most challenging operating conditions (disruptions).
...
### [219] [Constructing VAE Latent Spaces with Prescribed Topology](https://arxiv.org/abs/2606.07058)
*Jilles S. van Hulst,Jakub M. Tomczak,W. P. M. H. Heemels,Duarte J. Antunes*

Main category: cs.LG

TL;DR: The paper proposes a mathematical framework to resolve topological mismatch between VAE's Gaussian prior and data manifolds (like tori, Klein bottles), using factorized priors over product covering spaces with closed-form KL divergences and group-invariant decoders, outperforming standard VAEs on synthetic and real datasets.


<details>
  <summary>Details</summary>
Motivation: Standard VAEs use a Gaussian prior, which creates a topological mismatch when data lies on non-Euclidean manifolds (e.g., circles, tori, Möbius strips), degrading reconstruction quality and preventing faithful latent representations.

Method: The framework handles manifolds with product covering spaces (products of circles, intervals, lines, or quotients by finite symmetry groups). It uses factorized distributions over elementary factors with closed-form KL divergences, reparametrizable encoder-prior pairs for periodic/bounded/unbounded supports, coordinate transformations for neural network outputs, and group-invariant decoder features for quotient manifolds with anchor constraints.

Result: On synthetic manifolds and rotated/cyclically shifted MNIST, topology-matched priors align KL regularization with the data manifold and outperform the Gaussian baseline at all practically relevant regularization strengths.

Conclusion: Topology-aware VAEs resolve topological mismatch and improve both reconstruction quality and representation faithfulness for non-Euclidean data manifolds. Code is publicly available.
...
TL;DR: This study analyzes 28 high-profile filter feature selection (FFS) papers (1994–2025) and finds that 33% of performance variance (win rate of new methods vs. baselines) can be explained by the number of datasets, baselines, and new methods used in evaluation, suggesting potential biases in FFS evaluation practices.


<details>
  <summary>Details</summary>
Motivation: To investigate whether evaluation biases exist in filter feature selection (FFS) research, similar to those observed in tabular deep learning and data valuation, and to recommend stronger evaluation principles.

Method: Analyzed a sample of 28 high-profile FFS studies (1994–2025) using multivariate linear regression to examine how #Datasets, #Baselines, and #NewMethods predict the win rate of new methods against chosen baselines.

Result: Multivariate Linear Regression achieved R²=0.33, meaning 33% of variance in win rate is explained by the three factors (number of datasets, baselines, and new methods).

Conclusion: The medium explanation (R²=0.33) is promising for a first study but indicates additional factors (domain maturity, dataset/baseline types, model simplicity) also influence evaluation outcomes. Five evidence-based recommendations are given for future FFS evaluation.
...
TL;DR: SlimSearcher is a framework that improves efficiency in deep research agents by using Pareto-efficient trajectory distillation (SFT) and Adaptive Reward Gating (RL), reducing tool calls by 17-58% without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Current deep research agents are accuracy-driven but computationally wasteful, relying on brute-force strategies with excessive tool calls and redundant reasoning, creating an "efficiency trap."

Method: SlimSearcher uses two stages: (1) SFT with Pareto-efficient filtration to select successful yet economical trajectories, and (2) RL with Adaptive Reward Gating that evaluates tool/token efficiency relative to a cohort while maintaining correctness, avoiding brevity bias and reward hacking.

Result: On GAIA, BrowseComp, and XBenchDeepSearch, SlimSearcher reduces average tool-call rounds by 17%-58% while maintaining or improving accuracy.

Conclusion: The framework successfully pushes the Pareto frontier between accuracy and efficiency, proving that careful reward shaping and trajectory selection can make agents both accurate and cost-effective.
...
TL;DR: This paper characterizes on-policy distillation (OPD) training dynamics for LLMs, finding OPD inhabits a unique "relaxed off-principal" regime with subspace locking, distinct from both SFT and RLVR.


<details>
  <summary>Details</summary>
Motivation: On-policy distillation (OPD) is widely used to improve LLM reasoning, but its training dynamics are poorly understood compared to SFT and RLVR.

Method: The authors use parameter-space diagnostics to compare OPD with SFT and RLVR, analyzing update trajectories, principal direction avoidance, and subspace locking behavior through controlled experiments.

Result: OPD affects fewer weights, avoids principal directions more than SFT, but is less tightly constrained than RLVR. OPD exhibits subspace locking where cumulative updates enter a narrow low-dimensional channel; constraining training to this early subspace preserves OPD but degrades SFT.

Conclusion: OPD induces its own unique update geometry in parameter space, distinct from being simply an intermediate point between SFT and RLVR.
...
TL;DR: RCML reframes multiplier updating in constrained stochastic optimization as projected-pressure feedback with residual-based tracking, improving stability and feasibility over standard primal-dual methods.


<details>
  <summary>Details</summary>
Motivation: Standard primal-dual methods under stochastic mini-batch feedback suffer from noise accumulation in multiplier memory, making robust constraint enforcement difficult. The paper aims to fix this instability.

Method: Proposes Residual-Controlled Multiplier Learning (RCML), which reformulates multiplier updates as projected-pressure feedback with a pressure-memory residual for finite-gain tracking, augmented with stochastic stabilization components.

Result: Establishes finite-gain convergence for convex-affine problems, derives stochastic residual bounds under mini-batch feedback, and shows local KKT-residual interpretation near regular KKT points. Experiments on optimization, allocation, and fair-ranking tasks show improved feasibility control and multiplier stability.

Conclusion: RCML effectively addresses noise accumulation in multiplier updates for stochastic constrained optimization, offering better stability and feasibility without sacrificing objective performance.
...
TL;DR: This paper analyzes 115 developing countries (1991-2018) across four regions to examine how women's labor market participation affects fertility rates, finding that only in the Americas does career choice significantly influence fertility decisions, while other regions are driven by different factors.


<details>
  <summary>Details</summary>
Motivation: To better understand the causal relationship between women's labor force participation and declining fertility rates in developing countries, addressing omitted variable bias through a data-driven panel econometric approach.

Method: Divided 115 developing countries (1991-2018) into four continental groups (Africa, Americas, Asia/Pacific, Europe) and applied a data-driven panel data econometric procedure to mitigate omitted variable bias.

Result: Women's career choices significantly influence fertility behavior only in North/South America; in other regions, different factors play a more dominant role in reproductive decisions.

Conclusion: Policymakers can use these findings to design better incentive-based reproductive policies. Future research should incorporate family policies and patrilocality as important variables in developing countries.
...
TL;DR: OffQ mitigates activation outliers in LLMs via a top-1 PCA rotation and offsetting mechanism, enabling effective W4A4KV4 uniform quantization that outperforms SOTA baselines.


<details>
  <summary>Details</summary>
Motivation: Activation outliers cause significant performance degradation in low-bit quantization of LLMs; existing methods struggle to handle them effectively while maintaining deployment-friendly uniform quantization.

Method: OffQ identifies a low-dimensional outlier subspace via top-1 PCA, rotates activations to concentrate outliers into 1 channel, then absorbs that channel into a shared offset to reduce activation standard deviation.

Result: OffQ achieves effective W4A4KV4 uniform quantization across diverse LLM architectures, outperforming state-of-the-art baselines in accuracy while preserving low-bit efficiency.

Conclusion: The proposed offsetting mechanism is an effective and deployment-friendly approach to handling activation outliers, enabling accurate low-bit quantization of LLMs.
...
TL;DR: This paper theoretically analyzes nonlinear autoencoders with a finite-dimensional bottleneck in the mean-field regime, deriving explicit learning dynamics for encoder and decoder, and showing that finite-width networks trained with SGD closely track the mean-field risk trajectory and converge to the optimal solution.


<details>
  <summary>Details</summary>
Motivation: To improve the theoretical understanding of nonlinear autoencoders, which has been limited to linear models or settings without a bottleneck despite their empirical success.

Method: The authors study nonlinear autoencoders in the mean-field regime with a fixed finite-dimensional bottleneck, deriving explicit learning dynamics for both encoder and decoder, and analyzing the risk trajectories of finite-width networks trained with SGD.

Result: Finite-width networks trained with SGD closely track the mean-field risk trajectory with high probability over finite time horizons, and the finite-width risk converges to the mean-field optimum at optimality.

Conclusion: Finite networks are sufficiently expressive to approximate the infinite-width solution, bridging the gap between practical autoencoder training and theoretical understanding in the nonlinear setting.
...
TL;DR: The paper introduces ESBM, a trainable behavioral model that combines task performance with question answering and mechanism prediction. It uses adaptive questions and world-model probes to jointly optimize scores, answerability, and consistency, producing high-scoring policies with explicit explainable mechanisms in Atari-style environments.


<details>
  <summary>Details</summary>
Motivation: Interactive agents trained solely on task returns achieve high scores but fail to represent the mechanisms behind their actions, leading to brittle behavior and poor adaptation. Existing methods like LLM reflection and policy-code repair only revise from failed trajectories without integrating ongoing world-understanding tests during training.

Method: ESBM represents behavior via typed predicates, weighted rules, bounded options, and mechanism memory. After each rollout, adaptive questions and active world-model probes convert score failures, QA errors, and transition-prediction errors into constraints for local edits. Candidate models are selected by a multi-criterion rule evaluating task score, answerability, and active world-model consistency.

Result: Under Atari-style protocols, ESBM learns high-scoring policies while producing explicit answers and executable mechanism predictions, showing that adaptive questions can serve as both training pressure and reusable benchmarks.

Conclusion: Adaptive questions can act as effective training pressure and reusable benchmarks for mechanistic policy learning, producing agents that are both high-performing and interpretable in their reasoning.
...
TL;DR: The paper introduces FDRS, a statistical and machine-learning framework for detecting non-random digit-pattern irregularities in numerical research data. It combines multiple statistical tests and ML models, achieving high classification performance (AUC 0.98) on test datasets, and can prioritize raw data for integrity review.


<details>
  <summary>Details</summary>
Motivation: Raw numerical datasets remain less systematically examined in integrity screening compared to images, plagiarism, or summary-statistic inconsistencies. The authors aim to fill this gap by developing a dedicated screening tool for digit-level irregularities.

Method: FDRS integrates single- and joint-decimal-digit tests, Cramer's V, entropy metrics, Kullback-Leibler divergence, digit-preference indices, progressive subsampling, and semi-supervised risk scoring. Evaluated on instrument-derived enzymatic absorbance data (RawData, n=253) and a manually simulated irregular dataset (ErrData, n=255).

Result: RawData showed no significant deviation in single third-decimal-digit analysis; ErrData showed significant deviation. Elastic-net Logistic Regression achieved highest AUC (0.98395), Random Forest highest accuracy (0.926667). RawData scored 0.124627 (Grade 0), ErrData scored 0.740760 (Grade 3). External benchmarks confirmed graded risk stratification.

Conclusion: FDRS can prioritize raw numerical datasets for further review by integrating interpretable statistical and machine-learning features. It is an auxiliary digit-structure screening tool, not standalone evidence of fabrication or misconduct.
...
TL;DR: The paper proposes α-PFN, a learned approximation of entropy search acquisition functions for Bayesian optimization using Prior-data Fitted Networks, achieving over 50x speedup while remaining competitive with state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Entropy Search-based acquisition functions are principled but rely on slow, complicated Monte Carlo approximations that are numerically unstable and require hand-crafted implementations.

Method: A two-stage amortization strategy: first train a PFN conditioned on optima information; then train α-PFN to predict expected information gain using gains measured from the first PFN, enabling single forward pass acquisition evaluation.

Result: Competitive with state-of-the-art entropy search on synthetic and real-world benchmarks, with over 50x speedup across all experiments.

Conclusion: Learned amortization (α-PFN) can replace complex heuristic approximations for entropy search, making it faster and more extensible without sacrificing performance.
...
TL;DR: This paper enhances an unsupervised ML-based disease staging framework for Huntington's disease by applying explainability analysis (dimensionality reduction, saliency maps, SHAP) to the Enroll-HD dataset, showing that learned embeddings align with clinical progression from early cognitive-motor impairment to severe functional dependency.


<details>
  <summary>Details</summary>
Motivation: Unsupervised ML can uncover disease progression trajectories from longitudinal data, but limited interpretability hinders clinical trust and translation—this work aims to bridge that gap for Huntington's disease staging.

Method: Extended a prior ML-based disease staging framework with explainability: (1) projection of learned representations into lower-dimensional space for cluster alignment assessment, (2) saliency maps to identify influential clinical features over time, (3) surrogate classifier with SHAP for feature importance and transition analysis between stages.

Result: Learned embeddings align with established motor and functional severity scores; SHAP reveals a stratified progression from early cognitive-motor impairment to severe functional dependency, consistent with known HD progression, while also showing intra-stage variability.

Conclusion: The explainability analysis confirms that the unsupervised ML framework captures clinically meaningful HD progression patterns, enhancing interpretability and potential clinical trust in automated staging.
...
TL;DR: REMEDI is a new benchmark for evaluating machine unlearning methods specifically in the medical domain (clinical disease inference), revealing that existing methods show utility-unlearning trade-offs and struggle with multi-label classification tasks.


<details>
  <summary>Details</summary>
Motivation: Existing machine unlearning methods and benchmarks are primarily designed for non-medical domains using synthetic datasets, leaving a gap in understanding how well they work for clinical disease inference where privacy, label correlations, longitudinal data, and safety constraints create unique challenges.

Method: The authors introduce REMEDI, a benchmark built on the MIMIC-III clinical database. It covers (1) medical domain, (2) diverse forget instance setups, (3) both multi-label and multi-class classification tasks, and (4) evaluation metrics for both utility and unlearning extent.

Result: Existing unlearning methods exhibit a trade-off between utility and unlearning performance, and are largely unsuited for multi-label classification tasks.

Conclusion: Machine unlearning in the medical domain requires specialized benchmarks like REMEDI; current methods have significant limitations, calling for further research.
...
TL;DR: This paper introduces pinn-gym, an open benchmark for evaluating physics-informed machine learning surrogates as decision-making systems (ranking, feasibility, regret) rather than just curve predictors, using material-conditioned lattice design tasks.


<details>
  <summary>Details</summary>
Motivation: Physics-informed ML is typically evaluated by curve error, but real engineering use depends on downstream decisions (ranking candidates, avoiding infeasible designs, limiting regret). There is a gap between standard metrics and practical utility.

Method: The authors propose pinn-gym, a benchmark coupling a reduced-order crush-and-impact oracle with five printable polymer material cards, dimensionless force-response targets, and an evaluation protocol covering curve fidelity, physical admissibility, top-k retrieval, and mass regret across per-material, pooled, and cross-material settings.

Result: Low nRMSE is often insufficient for useful design selections. Physics-informed losses alter trade-offs rather than monotonically improving all metrics. Dimensionless conditioning improves comparability without making transfer symmetric.

Conclusion: pinn-gym provides a reproducible testbed for evaluating PIML surrogates as decision systems rather than just curve predictors, though it is not a certified material model.
...
TL;DR: The paper introduces a novel approach for detecting change points in dynamic networks by defining regimes as coherent dynamic trajectories along geodesics in graph space, using graph regression to measure drift, and outperforming state-of-the-art methods on both synthetic and real-world (Covid-19 mobility) data.


<details>
  <summary>Details</summary>
Motivation: Traditional change point detection assumes abrupt transitions between stationary states, but real-world dynamic networks (e.g., social networks, physical systems) often exhibit continuous, gradual evolution. The authors aim to address this gap by modeling regimes as periods of coherent evolving dynamics rather than static states.

Method: The authors define regimes as trajectories along geodesics in a suitably defined graph space. They use graph regression to estimate the geodesic between endpoint graphs and measure cumulative distance of observed sequences from this geodesic. Change points are detected when significant drifts in dynamics occur, either as trajectory shifts or pace changes.

Result: The model outperforms state-of-the-art change point detection models on synthetic dynamic networks with changing trajectories and varying speeds. In Covid-19 mobility data analysis, detected change points align more closely with external events compared to baseline methods.

Conclusion: This is the first work to model and detect changes between evolving regimes in graph space, providing a realistic and powerful tool for analyzing complex temporal graph data by accounting for continuous evolution rather than abrupt state transitions.
...
TL;DR: RETROSPECT combines a ChemAlign Transformer (proposal) with a LambdaMART reranker for single-step retrosynthesis, achieving 55.0% top-1 and 86.18% top-10 accuracy via hybrid augmentation, Pre-LayerNorm, tied embeddings, EMA weights, and atom-balance loss.


<details>
  <summary>Details</summary>
Motivation: Single-step retrosynthesis needs both accurate first-ranked suggestions and rich candidate lists. The authors decompose this into a proposal-selection framework to improve both aspects.

Method: A single Transformer proposal model (ChemAlign Transformer) trained with hybrid root-aligned & random SMILES augmentation, Pre-LayerNorm, tied embeddings, EMA weights, and differentiable atom-balance loss. A LambdaMART reranker uses structural, template, upstream-score, and optional DFT features.

Result: Generator: 55.0% top-1, 86.18% top-10 accuracy, 99.86% validity on USPTO-50K. Reranker: 59.4% top-1, 0.7171 MRR. Upstream score and template frequency are key features; DFT gains are inconsistent.

Conclusion: Modular retrosynthesis works: strong single-model proposal and learned selection are complementary; the proposal model can be a drop-in component for ensemble systems like RetroChimera.
...
TL;DR: This paper proposes learning structured correction terms for classical sparse Type-II Bayesian M/EEG brain imaging solvers by unfolding the iterative algorithm into a trainable neural network, preserving interpretability while improving reconstruction performance.


<details>
  <summary>Details</summary>
Motivation: Classical sparse Type-II Bayesian methods for M/EEG brain imaging rely on fixed iterative update rules whose dynamics cannot be adapted from data, limiting their empirical reconstruction performance.

Method: Unfolding a classical joint hyperparameter-learning solver into a trainable neural architecture with layers mirroring the original iterations, then enriching it with progressively more expressive correction-learning mechanisms (learnable biases, adaptive MLP, attention-based contextual refinements).

Result: Learned correction variants improve reconstruction performance and convergence behavior over the baseline unfolded solver while preserving algorithmic transparency.

Conclusion: Training does not replace Bayesian inference with a black-box predictor, but learns structured correction terms that retain interpretability and model-based character, improving empirical performance without replacing the original inference mechanism.
...
TL;DR: DEFINED is a data-efficient computational framework for fine-grained creativity assessment in debate scenarios, using a hierarchical eight-dimensional metric system and pre-trained autoregressive language model with mixed-granularity training, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Assessing creativity in complex, open-ended environments is challenging due to reliance on standardized simple tasks, scarcity of fine-grained expert data, and costly human evaluation. Debate offers an ecologically valid, data-rich context for assessing multiple creativity dimensions.

Method: DEFINED uses a hierarchical eight-dimensional metric system for debate creativity, implemented via a pre-trained autoregressive language model with a hierarchical scoring head supporting fine/coarse-grained evaluation. It employs constrained data augmentation to address elite bias and a mixed-granularity training strategy for learning from limited expert supervision.

Result: The scoring model achieves accurate and stable scoring, outperforming prompt-based LLM evaluators and existing debate scoring methods. Ecological validity was validated via empirical study with debate-naive participants.

Conclusion: DEFINED provides an effective data-efficient framework for fine-grained creativity assessment in debate, addressing key limitations of current automated scoring approaches.
...
TL;DR: Morph is a flexible-size generative model for 3D molecular design that dynamically adapts the number of atoms, enabling better property steering and out-of-distribution generation compared to fixed-size diffusion and flow-based models.


<details>
  <summary>Details</summary>
Motivation: Current diffusion and flow-based models fix the number of atoms, limiting their ability to capture the intrinsic relationship between molecular properties and molecular size, and thus hindering effective property steering.

Method: Morph is a flexible-size generative model based on geometric graphs that dynamically adapts the number of atoms during generation, allowing conditional and unconditional 3D molecular design with integrated structural priors like scaffolds.

Result: Morph matches current fixed-size state-of-the-art models while offering unparalleled sampling flexibility, and succeeds in out-of-distribution generation where previous models fail.

Conclusion: Dynamic size adaptation in generative molecular design enhances property steering and enables generation in previously inaccessible regimes, paving the way for improved molecular design.
...
TL;DR: A projected recurrent state model using explicit non-commutative state composition achieves perfect long-horizon state tracking (up to 1M tokens) on a controlled S₃×S₃ benchmark, while standard baselines like GRU and SSM fail under a held-out transition-pair falsifier protocol.


<details>
  <summary>Details</summary>
Motivation: To expose and address the limitation of standard sequence models in tracking ordered latent states that evolve through non-commutative transformations, where summarizing observed tokens is insufficient.

Method: Introduces a held-out transition-pair falsifier protocol for finite non-Abelian group tracking that blocks memorization of local transitions, and evaluates a projected recurrent state model with explicit non-commutative state composition against baselines (bag, GRU, SSM).

Result: The projected recurrent model achieves perfect accuracy (250/250 per horizon) up to 1,048,576 tokens; all baselines (native and projection-matched) remain near floor. Mechanism diagnostics show hard projection correlates with low homomorphism error, low state-consistency drift, and non-trivial commutator separation.

Conclusion: Explicit projected non-commutative state composition provides a useful inductive bias for long-horizon hidden-state tracking, though results are scoped to this controlled finite-group falsifier rather than a general architecture ranking.
...
TL;DR: This paper studies what generative models (specifically Rectified Flows) retain from training data beyond verbatim reproduction. It identifies a bell-shaped gap in reconstruction quality between train and test data across interpolation parameter λ, derives the peak location in closed form under Gaussian assumptions, validates on audio/images, and uses this structure for Membership Inference Attacks.


<details>
  <summary>Details</summary>
Motivation: Understanding what generative models retain from training data remains challenging, with implications for copyright and privacy. Beyond verbatim reproduction, models can encode subtler traces of training data that never surface in outputs yet remain exploitable.

Method: Analyzes the interpolation path X_λ = (1-λ)X_0 + λX_1 defining Rectified Flow training; identifies a bell-shaped reconstruction gap between train/test data over λ; derives closed-form peak location under Gaussian assumptions; validates on audio and image data.

Result: A bell-shaped signal exists in reconstruction gap that accumulates during training while validation metrics remain stable. The peak location is derived in closed form under Gaussian assumptions. The structure is universal across audio and images, and can be exploited for Membership Inference Attacks.

Conclusion: The bell-shaped λ-resolved structure is a universal signature of training data retention in Rectified Flows, enabling membership inference attacks even when no verbatim reproduction occurs in outputs.
...
TL;DR: This paper reveals that iterative solvers in model merging act as implicit spectral regularizers rather than optimizers, and proposes SWUDI/SWUDI-A—closed-form spectral filtering estimators that achieve state-of-the-art merging performance while reducing wall-clock time by 28-72x and GPU memory by up to 50%.


<details>
  <summary>Details</summary>
Motivation: Existing model merging methods formulate the problem as layer-wise quadratic interference minimization but rely on hundreds of iterations of gradient descent, which is computationally expensive and poorly understood. The authors aim to explain why iterative solvers outperform exact closed-form pseudoinverse solutions and to develop a more efficient, principled alternative.

Method: The authors formalize multi-task model merging as a noisy linear inverse problem and propose a spectral filtering estimator with per-direction filtering. They instantiate SWUDI using a soft exponential filter (matching gradient-flow trajectory) plus hard top-K truncation (suppressing noise in small-eigenvalue directions). SWUDI-A adapts per-layer rank rules. Both methods rely on a single symmetric eigendecomposition per linear layer, requiring no training data or optimizer state.

Result: SWUDI/SWUDI-A match or outperform state-of-the-art merging methods across four general benchmarks and a multimodal merging benchmark (VQA, Geometry, Chart, OCR, Grounding, modality merging). They reduce wall-clock time by 28-72x and peak GPU memory by up to 50%.

Conclusion: The iterative solver in model merging primarily acts as an implicit spectral regularizer, not an optimizer. The proposed closed-form spectral filtering methods provide a principled, efficient, and high-performing alternative to iterative gradient descent-based merging.
...
TL;DR: The paper proposes Trio, a time-series forecasting model with Temporal-Spatial-Sample attention that explicitly retrieves and reuses historical input-output examples, and introduces a TS-SCM generator for creating structured synthetic forecasting tasks. Experiments show performance improvements, but fully general PFN-style forecasting remains unsolved.


<details>
  <summary>Details</summary>
Motivation: Prior-Data Fitted Networks (PFNs) show that synthetic tasks help learn transferable inference behavior, but directly applying this to time-series forecasting is difficult because temporal order, dynamic lags, and recurring patterns are not captured by ordinary tabular priors. The authors aim to study how historical input-output examples can be explicitly organized and reused within a forecasting model.

Method: Proposes Trio, a sample-aware architecture with three attention mechanisms: (1) Temporal attention for within-window dynamics, (2) Spatial attention for cross-variable dependencies, (3) Sample attention for retrieving relevant historical lookback-future pairs. Also introduces a Time-Series Structural Causal Model (TS-SCM) generator to create structured synthetic forecasting tasks with dynamic lags, cross-variable interactions, noise, feedback, and distributional drift.

Result: Experiments on synthetic, industrial, and public benchmarks show that Trio improves forecasting performance. Exploratory zero-shot experiments suggest TS-SCM-generated tasks provide useful structural priors, though fully general PFN-style time-series forecasting remains an open problem.

Conclusion: The Trio architecture effectively organizes and reuses historical examples for time-series forecasting, and the TS-SCM generator offers a promising direction for learning transferable structural priors, but achieving fully general PFN-style time-series forecasting is still an open challenge.
...
TL;DR: The paper introduces the Bootstrap Theory of Representational Emergence (TBER), a framework explaining when and why new representations arise in machine learning. It argues that representational innovation is triggered not just by more data or compute, but by explanatory insufficiency—when a representation can describe observations but cannot make their organization intelligible. The process follows a recursive bootstrap: observations → anomalies → insufficiencies → new representations → further observations.


<details>
  <summary>Details</summary>
Motivation: Most research focuses on optimizing representations within an existing framework, but little attention is given to when a new level of representation becomes necessary. The authors aim to formalize the conditions and process of representational emergence.

Method: TBER proposes a five-stage recursive framework: (1) stabilized observation, (2) anomaly detection, (3) recognition of explanatory insufficiency, (4) representational emergence, and (5) provisional stabilization. Explanatory insufficiency—not falsity but exceeded explanatory domain—is identified as the key signal for transition.

Result: A formalized bootstrap dynamic showing that representational innovation follows a recursive sequence: observations reveal anomalies, anomalies expose insufficiencies, insufficiencies motivate new representations, and new representations enable further observations and potential new insufficiencies.

Conclusion: TBER suggests that future AI systems may benefit from built-in mechanisms to detect the explanatory limits of their own internal representations, and the framework applies broadly to representation learning, foundation models, world models, digital twins, biological systems, and scientific discovery.
...
TL;DR: TabSwift is an efficient tabular foundation model that uses a lightweight row-wise attention-only backbone with gated attention stabilization and learnable register tokens, achieving competitive performance with larger models like TabPFN v2 while being more inference-efficient, and optionally supports adaptive early-exit for latency-sensitive deployments.


<details>
  <summary>Details</summary>
Motivation: Recent tabular foundation models improve accuracy with increasingly complex architectures, leading to higher inference cost and limiting practical deployment. The authors aim to show that a simpler, lightweight design can remain highly competitive.

Method: Revisits the original TabPFN design with a lightweight row-wise attention-only backbone, enhanced by (1) a gated attention stabilization mechanism and (2) learnable register tokens for global context. Also introduces an adaptive layer-wise early-exit mechanism for latency-sensitive serving.

Result: TabSwift is competitive with stronger tabular foundation models (e.g., TabPFN v2 and TabICL) while being more efficient at inference. The adaptive early-exit mechanism further improves efficiency for latency-sensitive scenarios.

Conclusion: TabSwift enables efficient and anytime tabular in-context learning suitable for practical deployments, balancing accuracy and inference cost.
...
TL;DR: This paper presents SleepExplain, an explainable ML model for classifying NREM and REM sleep stages from EEG data using Random Forest, XGBoost, and Gradient Boosting (92-94% accuracy), with SHAP for interpretability.


<details>
  <summary>Details</summary>
Motivation: Sleep stage classification is critical for diagnosing sleep disorders, and EEG effectively captures neurological changes during NREM/REM phases. The authors aim to not only classify these stages accurately but also explain the model's predictions.

Method: Used Random Forest, XGBoost, and Gradient Boosting for sleep stage classification from EEG data. For explainability, employed SHAP (SHapley Additive exPlanations), a game-theoretic approach to interpret predictions.

Result: Random Forest: 92.54%, Gradient Boosting: 94.25%, XGBoost: 94.30% accuracy. XGBoost performed best overall.

Conclusion: Ensemble methods achieve high accuracy for NREM/REM sleep classification, and SHAP provides convincing explainability for model predictions, making the system more transparent for clinical use.
...
TL;DR: First systematic characterization of vLLM startup latency, breaking it into 6 CPU-bound steps, with an analytical model for prediction.


<details>
  <summary>Details</summary>
Motivation: Cold start latency of inference engines like vLLM becomes critical for scalable services, yet no systematic study existed despite vLLM's rapid evolution and complexity.

Method: Decompose vLLM startup into 6 foundational steps; benchmark on various model/hardware configurations; develop a lightweight analytical model to predict startup latency.

Result: Startup is predominantly CPU-bound; each step shows consistent scaling trends w.r.t. model/system parameters; the analytical model accurately predicts latency for given hardware.

Conclusion: Provides actionable guidance for resource planning in large-scale inference environments; all tools open-sourced.
...
TL;DR: Proposes a PPG foundation model that uses multimodal supervision (ECG, respiratory signals) from ICU data to select contrastive samples, enabling robust pretraining without requiring high-quality or field-like data, achieving SOTA on 14/15 downstream tasks with 3x fewer subjects.


<details>
  <summary>Details</summary>
Motivation: Existing PPG foundation models either rely on open-source ICU datasets with curated pretraining that limits generalization to field-like data, or use closed-source field-like PPG data. The authors aim to build a PPG model that does not require high-quality or field-like pretraining data.

Method: Leverages accompanying ECG and respiratory signals from ICU datasets to select contrastive samples during pretraining, allowing the model to learn from noisy PPG segments rather than filtering them out.

Result: Pretrained on 3x fewer subjects than existing approaches, achieves performance improvements on 14 out of 15 diverse downstream tasks including daily activity and heart rate prediction.

Conclusion: Multimodal supervision can integrate complementary physiological information to improve robustness of PPG foundation models and enhance generalization to consumer-grade data.
...
TL;DR: Q-Evolve is a self-evolving framework for LLM agents that unifies automatic process-reward labeling and policy learning within an in-distribution reinforcement learning paradigm, enabling reliable long-horizon decision making in sparse-reward settings.


<details>
  <summary>Details</summary>
Motivation: LLMs serving as controllers for interactive agents face a fundamental challenge in training reliable long-horizon decision making, primarily due to credit assignment difficulties from delayed rewards at episode ends.

Method: Q-Evolve learns an in-distribution critic from a hybrid off-policy dataset (expert demonstrations + agent trajectories) using weighted Implicit Q-Learning, derives step-wise process rewards via advantage estimation, then performs behavior-proximal policy optimization to iteratively evolve the agent.

Result: Q-Evolve outperforms strong baselines on AlfWorld, WebShop, and ScienceWorld in sample efficiency, robustness, and overall task performance.

Conclusion: Stable agent self-evolution is achievable through co-evolution of process-level supervision and policy, grounded within a shared in-distribution learning loop.
...
TL;DR: Proposes CapCode and CapReward to detect and prevent cheating in agent evaluations by capping maximum possible non-cheating performance, making scores above the cap evidence of exploitation.


<details>
  <summary>Details</summary>
Motivation: Models exploit shortcuts to achieve high evaluation scores without solving the intended task, making evaluation scores unreliable indicators of true ability.

Method: CapCode: coding datasets with randomized tests where non-cheating performance is capped below 1. CapReward: reward design that discourages optimization beyond the cap.

Result: CapCode detects cheating while preserving model ranking; CapReward reduces cheating behavior and improves task specification following.

Conclusion: Capped-performance design provides clearer interpretation of evaluation scores and effectively mitigates deceptive performance in agent training.
...
### [249] [Covariance Shrinkage via Stochastic Interpolation](https://arxiv.org/abs/2606.07382)
*Mathieu Chalvidal,Florentin Coeurdoux,Eric Vanden-Eijnden*

Main category: cs.LG

TL;DR: This paper reframes shrinkage estimation of high-dimensional covariance matrices as empirical risk minimization over a stochastic interpolant between source and target distributions, identifying three regularization mechanisms (scheduling, flow maps/couplings, early stopping), and proposes a neural estimator with risk bounds validated on synthetic and real neuroimaging data.


<details>
  <summary>Details</summary>
Motivation: To unify and extend understanding of high-dimensional covariance shrinkage estimators by recasting them through the lens of stochastic interpolants, revealing new regularization mechanisms beyond traditional approaches.

Method: The authors formulate shrinkage as empirical risk minimization over a parametric stochastic interpolant, identifying scheduling, coupling/flow maps (including optimal transport), and early stopping as risk-reduction mechanisms. They propose a neural estimator with a theoretical upper bound on quadratic risk based on interpolation approximation error.

Result: The formalism recovers known shrinkage estimators as special cases, demonstrates that optimal transport couplings and non-linear flow maps enable eigenvector regularization, and validates the neural estimator on synthetic experiments and real neuroimaging data with improved regularization.

Conclusion: The stochastic interpolant perspective provides a unifying framework for covariance shrinkage, introduces new regularization mechanisms (especially eigenvector regularization via nonlinear flows), and the proposed neural estimator offers practical advantages demonstrated on neuroimaging data.
...
### [250] [Making the Most of Limited Data: Score-Aware Training for Text-to-Music Generation](https://arxiv.org/abs/2606.07387)
*Yun-Chen Cheng,Tzu-Hung Huang,Chih-Pin Tan*

Main category: cs.LG

TL;DR: A text-to-music system uses score-aware training with CLAP-conditioned Beta noise scheduling, segment-level filtering, two-stage captioning, and REPA auxiliary loss to achieve top rankings in the ICME 2026 ATTM Grand Challenge using only 450M parameters.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-music systems rely on massive proprietary datasets and industrial-scale compute, making it impossible to separate architectural contributions from resource advantages.

Method: 1) Score-aware training using audio-caption alignment as supervision; 2) CLAP-conditioned Beta noise timestep schedule repurposing low-scoring segments for high-noise training; 3) Segment-level filtering removing misaligned examples; 4) Two-stage caption procedure bridging training/inference caption distribution; 5) REPA auxiliary loss transferring knowledge from pretrained CLAP and MuQ encoders.

Result: Ranked 2nd in objective evaluation across both tracks and 3rd in the Efficiency Track (MOS evaluation) at ICME 2026 ATTM Grand Challenge.

Conclusion: The proposed methods effectively compensate for limited data and compute resources, achieving competitive performance without requiring large proprietary datasets.
...
### [251] [Generative Modeling of Discrete Latent Structures via Dynamic Policy Gradients](https://arxiv.org/abs/2606.07400)
*Stefan Ivanovic,Ge Liu,Mohammed El-Kebir*

Main category: cs.LG

TL;DR: GReinSS is a policy learning framework using dynamically rescaled rewards to infer mechanistic latent states from indirect observations, outperforming baselines in reconstructing simulated latent sets/graphs and real RNA-seq isoforms.


<details>
  <summary>Details</summary>
Motivation: Scientific problems need to infer unobserved mechanistic latent states from indirect observations, but classical methods (e.g., expectation maximization) don't scale to large combinatorial spaces, while deep learning methods (e.g., VAEs) learn artificial rather than ground-truth latent states.

Method: GReinSS is a policy learning framework that uses dynamically rescaled rewards to learn latent state distributions that maximize observed data likelihood.

Result: GReinSS accurately reconstructs simulated latent sets and latent graphs, outperforms alternative policy learning and generative modeling baselines, and reconstructs isoforms from real short-read RNA-seq data that better match long-read sequencing results than the standard RSEM algorithm.

Conclusion: GReinSS is a principled and practically effective approach for generative modeling and inference of combinatorial latent states from indirect observations.
...
### [252] [Reversible Foundations: Training a 120B Sparse MoE through State-Preserving Scaling](https://arxiv.org/abs/2606.07404)
*Rohan Shravan*

Main category: cs.LG

TL;DR: LightningLM 0.1V is a 120B-parameter sparse mixture-of-experts language model grown from a small dense seed through four stages, trained entirely on a single 8-GPU node using reversible recurrence (flat activation memory), state-preserving growth principles, and TQP quantization (reducing optimizer state by ~45x). The full lineage and training code are released.


<details>
  <summary>Details</summary>
Motivation: To demonstrate that hundred-billion-parameter sparse MoE models can be trained end-to-end on a single multi-GPU node, and to provide a reproducible, practitioner-level systems report that integrates known primitives (reversibility, growth, quantization) for efficient large-model training.

Method: Four-stage growth from a small dense seed (1.78B active) → 5B MoE → 9B MoE → 120B MoE (460 experts, top-12 routing, 5.93B active params). Uses: (1) reversible recurrence to avoid storing activations, (2) state-preserving growth principles for each expansion step, (3) TQP (quantized base expert weights + trained low-rank adapters, 2.26B adapter params) to cut optimizer state by ~45x.

Result: Trained on single 8-GPU nodes (8K context at larger stages); released training loss of 1.78 at 120B scale; per-domain held-out loss shows targeted capabilities (multilingual Indic, code) learned by construction.

Conclusion: The integration of known primitives—not any single novel technique—enables training a 120B-parameter MoE model on a single node. The full model family, tokenizer, and training code are open-sourced.
...
### [253] [A Comprehensive Anatomy of Human and DeepSeek-R1 LLM Mathematical Reasoning](https://arxiv.org/abs/2606.07410)
*Yuxiang Chen,Jun Wang*

Main category: cs.LG

TL;DR: DeepSeek-R1's reasoning traces exhibit "topological mimicry"—reproducing the surface structure of reasoning without functional depth. While branching/backtracking correlate with success, effective reflection must occur at the logical inference level, not trapped in local analysis loops.


<details>
  <summary>Details</summary>
Motivation: To determine whether large language models (specifically DeepSeek-R1) genuinely reason or merely imitate the appearance of reasoning, by systematically comparing their reasoning structures to human reasoning across AIME 2025 problems.

Method: Annotated 10,247 reasoning steps across all 30 AIME 2025 problems into five functional categories (Analysis, Inference, Branch, Backtrace, Reflection), then performed structural comparison between model and human reasoning traces.

Result: Human reasoning is compact and alternates between analysis and deduction. DeepSeek-R1 frequently loops through unnecessary verification and local checks without logical progress. Successful traces show stable branching/backtracking; failed traces misuse these actions. Reflection is only effective when embedded within deductive inference, not trapped in analysis loops.

Conclusion: Current long-CoT models may be rewarded more for the appearance of reasoning than genuine deductive progress. Future improvements should include cross-trace stability metrics, penalizing "spinning-wheel" traces, encouraging deeper logical correction, and reallocating compute toward deduction and backtracking.
...
### [254] [Sparsely gated tiny linear experts](https://arxiv.org/abs/2606.07414)
*Simon Schug*

Main category: cs.LG

TL;DR: The paper introduces sgatlin (sparsely gated linear neurons), where each expert in an MoE model is shrunk to a single linear neuron, achieving better compute efficiency and interpretability by removing nonlinearity, outperforming standard transformers in perplexity and enabling interpretable feedforward circuits.


<details>
  <summary>Details</summary>
Motivation: To improve compute efficiency and interpretability in large language models by further increasing sparsity in MoE architectures, moving beyond large dense experts to tiny single-neuron experts.

Method: Replace transformer feedforward layers with sparsely gated linear neurons (sgatlin) — each expert is a single linear neuron, nonlinearity is removed, and only a tiny fraction of many available neurons are selected via sparse gating.

Result: In isoflop comparisons, sgatlin improves perplexity over standard transformers across different compute budgets. Feedforward circuits form semantically structured clusters and are causally implicated in factual recall without needing surrogate models.

Conclusion: Sgatlin provides a path toward compute-efficient and interpretable transformer feedforward layers by combining extreme sparsity with linearity.
...
### [255] [Video-Based Prediction of In-Flight Particle Characteristics in Atmospheric Plasma Spraying](https://arxiv.org/abs/2606.07416)
*Abhijeet Praveen,Sareh Soleimani,Cormac Cureton,Aman Sidhu,Kintak Raymond Yu,Cristian Cojocaru,Narges Armanfard*

Main category: cs.LG

TL;DR: This paper shows that high-speed video of the plasma plume can predict particle temperature and velocity in atmospheric plasma spraying, achieving R² up to 0.90 using pretrained CNNs.


<details>
  <summary>Details</summary>
Motivation: In-flight particle temperature and velocity strongly influence coating quality in APS, but these characteristics are difficult to monitor continuously during operation, motivating the need for non-invasive data-driven diagnostic methods.

Method: The authors introduced three video-derived feature representations and evaluated them using TabPFN, CNNs, and classical regression baselines (Random Forest, Gradient Boosting, SVR, XGBoost). Experiments used grouped leave-one-out cross-validation on 126 labeled recordings from 63 APS spray runs, including raw video frame analysis with pretrained CNNs.

Result: TabPFN achieved R²=0.86 for temperature with combined features; CNNs excelled at velocity (R²=0.81); pretrained CNN with regression head achieved the best overall performance (R²=0.90 for temperature, 0.82 for velocity).

Conclusion: Video-derived plume information provides a promising and scalable foundation for non-invasive APS diagnostics and real-time process monitoring.
...
### [256] [Discovering Multiscale Deep Formulas in Complex Systems via Neural-Guided Lambda Calculus](https://arxiv.org/abs/2606.07426)
*Hanqiao Yu,Shusen Yang,Xuebin Ren,Cong Zhao*

Main category: cs.LG

TL;DR: Deflex is an end-to-end AI method that automatically extracts multiscale formulas (invariants and distributions) from complex systems, combining a lambda-calculus symbolic regression model (Deflexpressor) with a decomposable deep energy model (Deflexformer) to achieve up to 7x efficiency gains over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing AI methods struggle to identify scale-specific formulas in multiscale complex systems, as they are limited to single-scale analysis.

Method: Deflex has two components: (1) Deflexpressor - a lambda-calculus symbolic regression model for higher-order formulas; (2) Deflexformer - a decomposable deep energy model for unified representations across scales. Deflexpressor generates synthetic data to pre-train Deflexformer, which then guides formula discovery by decoupling multiscale latent relationships.

Result: Across six representative multiscale complex systems, Deflex achieves up to 7-fold higher efficiency than state-of-the-art methods and enables automated multiscale discovery.

Conclusion: Deflex could be a useful tool for automated scientific discovery across disciplines dealing with multiscale complex systems.
...
### [257] [Unsupervised Continual Clustering via Forward-Backward Knowledge Distillation](https://arxiv.org/abs/2606.07474)
*Mohammadreza Sadeghi,Sareh Soleimani,Zihan Wang,Narges Armanfard*

Main category: cs.LG

TL;DR: FBCC introduces a forward-backward distillation framework for unsupervised continual clustering, using a continual teacher and task-specific students to prevent catastrophic forgetting without storing past data.


<details>
  <summary>Details</summary>
Motivation: Existing UCL methods overlook clustering-specific objectives and suffer from catastrophic forgetting, while traditional mitigation strategies (knowledge distillation, replay buffers) raise memory and privacy concerns.

Method: FBCC employs a continual teacher network with a clustering projector and lightweight task-specific students. It uses a dual-phase forward-backward knowledge distillation process to learn new clusters while preserving previously discovered cluster structures without storing past data.

Result: FBCC consistently outperforms existing continual learning baselines in clustering accuracy and significantly reduces catastrophic forgetting across four benchmark datasets.

Conclusion: FBCC is a pioneering approach to Unsupervised Continual Clustering (UCC), effectively addressing the overlooked clustering-specific objectives in UCL while mitigating memory and privacy concerns.
...
### [258] [Graph Neural Network leveraging Higher-order Class Label Connectivity for Heterophilous Graphs](https://arxiv.org/abs/2606.07475)
*Takuto Takahashi,Itsuki Nakayama,Takahiro Mitani,Ryosuke Kikuchi,Yuya Sasaki,Makoto Onizuka*

Main category: cs.LG

TL;DR: A Label Context Classifier (LCC) is proposed to capture higher-order class label connectivity in heterophilous directed graphs, improving node classification when integrated with GNNs.


<details>
  <summary>Details</summary>
Motivation: Current GNNs perform poorly on heterophilous graphs where nodes of different classes are connected, as they cannot capture higher-order class label connectivity common in real-world directed graphs.

Method: LCC estimates node labels by generating label context embeddings through four distinct types of walks on directed graphs, and can be adaptively integrated with any GNN.

Result: GNNs integrated with LCC outperform state-of-the-art methods, and label context embeddings improve node classification in heterophilous directed graphs.

Conclusion: LCC effectively addresses the limitation of GNNs in heterophilous directed graphs by capturing higher-order label connectivity, and can be flexibly combined with existing GNN architectures.
...
### [259] [Drifting Models for Surrogate Flow Modeling](https://arxiv.org/abs/2606.07481)
*Chris R. Jung,Markus Dörr,Natalie Jüngling,Jennifer Niessner,Adam T. Müller,Nicolaj C. Stache*

Main category: cs.LG

TL;DR: The paper adapts a generative drifting framework to fluid mechanics, creating conditional latent-space models that match diffusion accuracy but run two orders of magnitude faster, enabling real-time CFD surrogates.


<details>
  <summary>Details</summary>
Motivation: CFD provides high-fidelity flow fields but is computationally expensive; generative surrogates offer better distribution modeling than deterministic networks, but iterative sampling (e.g., diffusion) is slow, limiting rapid exploration of indoor environments.

Method: Adapts the generative drifting framework to fluid mechanics; uses a conditional architecture that performs drifting in a learned VAE latent space, and employs label-aware masking to align generated samples with boundary conditions. Also proposes a spatial-conditioning variant for generalization to unseen geometries.

Result: The label-conditioned model matches iterative diffusion in accuracy and flow consistency while running two orders of magnitude faster (single-pass generation vs. iterative sampling).

Conclusion: Conditional drifting serves as a highly efficient alternative to diffusion-based approaches, unlocking real-time CFD surrogates where inference speed is critical, with a promising path toward generalization to unseen geometries.
...
### [260] [Network Recovery from Cascade Data: A Debiased Jacobian-Based Machine Learning Approach](https://arxiv.org/abs/2606.07483)
*Lei Huang*

Main category: cs.LG

TL;DR: CascadeNet is a Jacobian-based machine learning framework for recovering hidden influence networks from cascade data without assuming a specific diffusion model. It uses a flexible estimator of the transition function with Neyman-orthogonal debiasing, achieving $\sqrt{n}$-consistency and asymptotic normality. It outperforms existing methods across nine data-generating processes in simulations and successfully recovers COVID-19 transmission networks in Spain that align with actual mobility data.


<details>
  <summary>Details</summary>
Motivation: Existing network recovery methods rely on assuming a specific diffusion model, leading to performance degradation when the model is misspecified. The goal is to develop a method that recovers hidden influence networks from cascade data without requiring a pre-specified diffusion mechanism.

Method: CascadeNet uses the Jacobian of the one-step transition function to characterize the underlying influence structure. It first constructs a flexible estimator of the transition function, then applies Neyman-orthogonal debiasing via the Riesz representer to obtain a debiased Jacobian that is $\sqrt{n}$-consistent and asymptotically normal, enabling formal inference.

Result: In simulations across nine common data-generating processes, CascadeNet achieves the highest network recovery accuracy. In an empirical application to COVID-19 transmission across Spain's 52 provinces, CascadeNet recovers transmission networks significantly correlated with actual inter-province mobility, while baseline methods show no significant alignment.

Conclusion: CascadeNet provides a model-agnostic, statistically rigorous framework for network recovery from cascade data, with strong empirical performance and formal inference capabilities.
...
### [261] [CoMetaPNS: Continually Meta-learning Personalized Neural Surrogates for Cardiac Electrophysiology Simulations](https://arxiv.org/abs/2606.07488)
*Ryan Missel,Xiajun Jiang,Linwei Wang*

Main category: cs.LG

TL;DR: This paper presents a continual meta-learning framework for personalized virtual heart simulations that can integrate sequential data without catastrophic forgetting, using a Bayesian Gaussian Mixture Model to identify known/unknown dynamics sources.


<details>
  <summary>Details</summary>
Motivation: Existing methods for personalized neural surrogates require costly retraining when new data arrives and suffer from catastrophic forgetting, which is impractical in clinical settings where unlabeled data arrives sequentially.

Method: A continual meta-learning framework leveraging a continual Bayesian Gaussian Mixture Model over a memory buffer to infer identifiers and relationships of data over time, enabling effective meta-learning without full retraining.

Result: Empirical results on synthetic cardiac data demonstrate superior simulation forecasting, computational scalability, and resilience to catastrophic forgetting compared to existing baselines.

Conclusion: The proposed framework effectively addresses the limitations of prior work by enabling continual learning of personalized neural surrogates in sequential data scenarios, making it more suitable for clinical deployment.
...
### [262] [Second-Order Path Kernel Interpolation Formulas in Machine Learning](https://arxiv.org/abs/2606.07495)
*Jin Guo,Roy Y. He,Jean-Michel Morel*

Main category: cs.LG

TL;DR: This paper extends Domingos' first-order interpolation formula for neural networks to second-order forms, incorporating curvature-weighted and sampling-induced terms for SGD and SGD with momentum, with concentration estimates for prediction fluctuations.


<details>
  <summary>Details</summary>
Motivation: To better understand how training data shape neural network predictions by refining existing first-order interpolation formulas with higher-order corrections.

Method: Derives second-order interpolation formulas for deterministic gradient descent, stochastic gradient descent, and SGD with momentum, using path-kernel analysis and curvature-weighted terms.

Result: (1) A curvature-weighted interpolation term supplements the leading path-kernel; (2) SGD introduces a sampling-induced term coupling prediction curvature with mini-batch noise covariance; (3) Momentum preserves the structure but modifies weights by a memory factor; (4) A concentration estimate quantifies fluctuation scale around the expected second-order representation.

Conclusion: These results refine the path-kernel interpretation of neural network predictions, providing a more accurate second-order characterization of how training data influence model outputs.
...
### [263] [Accelerated Decentralized Stochastic Gradient Descent for Strongly Convex Optimization](https://arxiv.org/abs/2606.07496)
*Ming Sun,Kun Yuan*

Main category: cs.LG

TL;DR: MG-ADSGD combines Nesterov-type primal-dual extrapolation with multi-round fast gossip averaging to achieve the best known communication complexity for decentralized stochastic strongly convex optimization, simultaneously attaining accelerated \(\sqrtκ\) and \(1/\sqrt{1-β}\) dependences.


<details>
  <summary>Details</summary>
Motivation: Existing decentralized stochastic methods cannot simultaneously achieve accelerated \(\sqrtκ\) (condition number) and \(1/\sqrt{1-β}\) (network spectral gap) dependences, which deterministic methods can attain.

Method: Proposes MG-ADSGD, which integrates Nesterov-type primal-dual extrapolation with multi-round fast gossip averaging, coupling gossip depth with mini-batch size to improve consensus accuracy and reduce gradient variance simultaneously.

Result: Achieves communication complexity \(\widetilde{\mathcal O}\!\left( \frac{σ^2}{μnε}\log\frac{1}ε + \sqrt{\fracκ{1-β}}\log\frac{1}ε \right)\), yielding the best known bound for decentralized stochastic strongly convex optimization.

Conclusion: MG-ADSGD attains both accelerated \(\sqrtκ\) and \(1/\sqrt{1-β}\) dependences simultaneously in the stochastic setting, improving over prior methods.
...
### [264] [Sparse Subspace-to-Expert Sharing for Task-Agnostic Continual Learning](https://arxiv.org/abs/2606.07500)
*Fatema Siddika,Md Anwar Hossen,Tanwi Mallick,Ali Jannesari*

Main category: cs.LG

TL;DR: SETA proposes a Mixture of Sparse Experts framework with task-specific and shared experts, using adaptive elastic anchoring and routing-aware regularization to solve catastrophic forgetting in LLM continual learning.


<details>
  <summary>Details</summary>
Motivation: Existing LLM continual learning methods treat all parameters uniformly, failing to distinguish between task-specific knowledge and shared capabilities, leading to the plasticity-stability dilemma and catastrophic forgetting.

Method: SETA decomposes the parameter space into task-specific expert modules (isolating unique patterns) and shared experts (capturing common features), maintained via adaptive elastic anchoring and routing-aware regularization that protects shared knowledge at both weight and routing levels, with a unified gating network for automatic expert retrieval during inference.

Result: Competitive or superior overall performance against state-of-the-art baselines across domain-specific benchmarks, with strong early-task knowledge retention and improved backward transfer on LLaMA-2 7B and Qwen3-4B.

Conclusion: SETA effectively resolves the plasticity-stability conflict through sparse expert decomposition and targeted regularization, offering a promising direction for task-agnostic continual learning in LLMs.
...
### [265] [Robots Need More than VLA and World Models](https://arxiv.org/abs/2606.06556)
...
TL;DR: This position paper argues that the bottleneck in generalist robot intelligence is not just policy scaling (more data, larger models), but the lack of mechanisms to convert unstructured real-world data into grounded robot supervision. The authors identify four missing components—data, embodiment, world-model, and reward interfaces—and propose a research agenda for learning from the broader physical world beyond robot demonstrations.


<details>
  <summary>Details</summary>
Motivation: Current robot learning overly focuses on policy-scaling (collecting more robot demos, training larger VLA models), but this neglects the critical gap: abundant unstructured behavioral data (human motion, internet video, simulations) cannot be directly used by robot policies due to missing embodiment-specific action labels, task semantics, and reward structures.

Method: The paper presents a position argument identifying four missing components: (1) data interfaces for autolabelling unstructured behavior, (2) embodiment interfaces for retargeting human motion to robot actions, (3) world-model interfaces for physics-grounded 3D reasoning, and (4) reward interfaces for inferring task progress/success from video and language. It surveys recent progress in robot foundation models, cross-embodiment datasets, learning from video, world models, and reward modeling.

Result: The authors identify that while progress exists in robot foundation models, cross-embodiment datasets, video learning, world models, and reward modeling, the key bottleneck is the absence of systematic interfaces to convert unstructured behavioral data into grounded robot supervision.

Conclusion: The paper calls for a research agenda focused on building robotics systems that can learn not only from robot demonstrations but from the broader physical world, by developing the four proposed interface types.
...
### [266] [PhyRoGen: Synthetic Generation of Physical Robot Manipulation Puzzles Using Procedural Content Generation](https://arxiv.org/abs/2606.06569)
*Lennart Julian Droß,Andreas Orthey,Marc Toussaint*

Main category: cs.RO

TL;DR: PhyRoGen is a framework that uses procedural content generation to automatically create synthetic datasets of physical manipulation puzzles with interlocking object dependencies, generating 24 puzzles solvable in 1-300 seconds and validated in simulation with a KUKA robot.


<details>
  <summary>Details</summary>
Motivation: Robot manipulation of physical puzzles is important for assembly/disassembly, but learning manipulation skills requires large training datasets that are time-consuming and tedious to generate manually.

Method: The PhyRoGen framework leverages procedural content generation (PCG) to automatically generate synthetic datasets of manipulation puzzles with interlocking object dependencies. It defines six concrete generators to produce 24 physical puzzles.

Result: All 24 puzzles were solvable in 1 to 300 seconds using sampling-based planning algorithms, and every generated puzzle was manipulatable by a KUKA LBR iiwa robot in physical simulation.

Conclusion: PhyRoGen can procedurally generate unique, solvable robot manipulation puzzles, providing a crucial ingredient for benchmarking manipulation algorithms and developing robust foundation models.
...
### [267] [ChronoForest: Closed-Loop Multi-Tree Diffusion Planning for Efficient Bridge Search and Route Composition](https://arxiv.org/abs/2606.06618)
*Jungmin Seo,Jaesik Park*

Main category: cs.RO

TL;DR: ChronoForest is a closed-loop planning system that uses anchor-chaining tree diffusion and online multi-tree orchestration to plan long-horizon routes from short-horizon offline data, achieving high success rates (99%+) on navigation benchmarks and significantly improving route quality over prior methods.


<details>
  <summary>Details</summary>
Motivation: Long-horizon route planning from short-horizon offline trajectories is challenging due to (1) microscopic trade-offs between search cost and path quality when composing short segments, and (2) macroscopic difficulty in ordering waypoints when pairwise travel costs are unknown and temporal distance estimates become unreliable over long ranges.

Method: ChronoForest couples local bridge search with online route re-solving via an anchor-chaining tree diffusion planner and an online multi-tree orchestrator. It uses temporal distance for short-range guidance and node evaluation, while leveraging search-time bridge evidence to validate long-range anchor connectivity and repeatedly re-solve the route.

Result: On OGBench AntMaze-Stitch, ChronoForest achieves 99.8%, 99.3%, and 99.5% success on medium, large, and giant splits, improving giant-stitch success by up to 34.5 points over prior diffusion-based methods. On Hamiltonian route-composition benchmarks, online re-solving corrects poor temporal orderings and improves route quality at lower cost than exhaustive planning.

Conclusion: ChronoForest effectively enables long-horizon navigation from short-horizon data by combining short-range temporal guidance with online re-solving for long-range connectivity, demonstrating both high success rates and route-level efficiency.
...
### [268] [What Matters When Cotraining Robot Manipulation Policies on Everyday Human Videos?](https://arxiv.org/abs/2606.06627)
*Richard Li,Aditya Prakash,Andrew Wen,Saurabh Gupta,Yilun Du,Pulkit Agrawal*

Main category: cs.RO

TL;DR: This paper investigates using everyday Internet video (not just curated robot-like demonstrations) to improve robot manipulation policies through cotraining. The authors create a new dataset of 532 human videos with 28 hours of high-quality hand labels, finding that hand pose quality and network specialization are critical for successful transfer, achieving a 29.7% absolute success rate improvement in low-data regimes across six tasks.


<details>
  <summary>Details</summary>
Motivation: To determine what factors enable successful transfer from abundant everyday Internet video (rather than curated, robot-like demonstrations) to robot manipulation policies, addressing the scarcity of robot training data.

Method: The authors: (1) create a new dataset of 532 human videos with 28 hours of high-quality triangulated hand labels and natural motions; (2) analyze how hand pose quality affects transfer; (3) investigate the effect of the motion gap between human and robot embodiments; (4) develop a cotraining recipe where vision and policy networks specialize to each embodiment.

Result: Hand pose quality affects transfer success. Even with accurate hands, the inherent motion gap hinders transfer unless networks specialize per embodiment. The proposed cotraining recipe yields 29.7% absolute success rate improvement in the low-robot-data regime across six manipulation tasks.

Conclusion: Everyday Internet video can be used effectively for robot cotraining, but success depends on high-quality hand pose labels and network specialization to bridge the embodiment gap, especially when robot data is scarce.
...
### [269] [On the Hardness of Optimal Motion on Trees](https://arxiv.org/abs/2606.06686)
*Tzvika Geft*

Main category: cs.RO

TL;DR: This paper proves NP-hardness for Multi-Agent Path Finding on trees across various objectives (distance, makespan, flowtime) for both labeled and 2-colored variants, resolving open problems including the classical Pebble Motion problem on trees and the Stack Rearrangement problem.


<details>
  <summary>Details</summary>
Motivation: Several basic MAPF cases on trees had remained open for decades despite many variants being known intractable. The authors aim to resolve these fundamental complexity questions and unify hardness results across motion models.

Method: The paper establishes NP-hardness through reductions from Stack Rearrangement (also proved NP-hard) to MAPF on subdivided stars, covering labeled and colored variants across all three objectives (distance, makespan, flowtime).

Result: Proved NP-hardness for: (1) labeled MAPF on trees under all three objectives, (2) 2-colored MAPF on trees under all three objectives, (3) classical Pebble Motion on trees (open for decades), (4) colored Pebble Motion with two colors on any graph class (first hardness result), (5) Stack Rearrangement problem.

Conclusion: The results reveal a common tractability barrier across multiple fundamental motion models, showing hardness already on simple subdivided stars, thereby unifying and strengthening prior hardness results.
...
### [270] [Optimal Control Approach for Non-prehensile Ball Juggling Using a 7-DoF Manipulator](https://arxiv.org/abs/2606.06704)
*Joel Ramadani,Vasilije Rakčević,Riddhiman Laha,Arne Sachtler,Valentin Le Mesle,Achim J. Lilienthal,Sami Haddadin*

Main category: cs.RO

TL;DR: This paper presents a model-based framework for controlling a 7-DOF robot arm to perform non-prehensile ball juggling using a tool, combining offline optimal control trajectory generation with real-time error correction, validated in simulation and on a Franka Emika Panda robot.


<details>
  <summary>Details</summary>
Motivation: Non-prehensile manipulation tasks like juggling involve hybrid dynamics that are highly sensitive to timing and control inputs. The key challenge is achieving dynamic stabilization of an underactuated object (the ball) that cannot self-correct, making the system dependent entirely on applied forces.

Method: A model-based framework using a two-stage optimal control approach: (1) offline computation of feasible juggling motion patterns, and (2) organizing these trajectories to enable real-time error correction without solving optimal control online.

Result: The controller was evaluated in simulation and validated through a real-world experiment using a Franka Emika Panda robot.

Conclusion: The proposed framework effectively enables stable periodic juggling motion for a robotic manipulator, handling the hybrid dynamics and timing-critical nature of the task.
...
### [271] [SCOUT: Semantic scene COverage via Uncertainty-guided Traversal](https://arxiv.org/abs/2606.06721)
*Junyu Mao,Sara Ayoubi,Vishnu D. Sharma,Ilija Hadžić,Matthew Andrews*

Main category: cs.RO

TL;DR: SCOUT is an online semantic exploration framework that couples active robot traversal with probabilistic 3D scene graph construction, enabling robots to revisit ambiguous objects and explore unseen space based on uncertainty-guided planning.


<details>
  <summary>Details</summary>
Motivation: Most 3D scene graph pipelines treat perception as a post-processing stage over fixed datasets, decoupling scene representation from the decisions that determine what is observed.

Method: SCOUT incrementally builds an uncertainty-aware 3D scene graph with fused geometry and open-vocabulary object label beliefs, then uses an uncertainty-guided traversal planner that balances semantic certainty gain, geometric coverage gain, and travel cost to select viewpoints.

Result: The system treats semantic scene completeness as an operational objective, allowing robots to revisit ambiguous objects when needed and expand into free space when incomplete.

Conclusion: SCOUT moves toward autonomous agents that can patrol, update, and reason about evolving indoor environments with minimal human intervention.
...
### [272] [IDDMBSE: Integrating Data-Driven and Model-Based Systems Engineering for Trusted Autonomous Cyber-Physical Systems](https://arxiv.org/abs/2606.06727)
*John S. Baras,Sai Sandeep Damera,Ryan Matheu,Clinton Enwerem,Praveen M. S. Kumar*

Main category: cs.RO

TL;DR: IDDMBSE is a novel integrated methodology combining Model-Based Systems Engineering (MBSE) with data-driven ML/AI, instantiated as an open-source tool chain (PERFECT, TRADES-X, VERITAS) and demonstrated on autonomous ground robots across the full development lifecycle.


<details>
  <summary>Details</summary>
Motivation: Autonomous CPS sit at the intersection of MBSE and ML/AI, yet no existing SE methodology natively integrates both paradigms, creating a critical gap in systems engineering practice.

Method: IDDMBSE extends the MBSE V-process with a data-driven loop at every step, anchored in SysML, the autonomy stack, and a hybrid model-based plus data-driven trade-off architecture, instantiated as an interoperable open-source tool chain (PERFECT, TRADES-X, VERITAS).

Result: Demonstrated on a Trusted Autonomous Ground Robot across sensor selection, risk-sensitive path planning, behavior-tree verification, conformal-prediction-based perception, and multi-robot coordination in a contested-terrain Isaac Sim environment.

Conclusion: IDDMBSE provides a unified SE methodology for autonomous CPS, with future work planned for SysML v2/KerML foundations to enable language-native composability and tighter ML/AI integration.
...
### [273] [AxisGuide: Grounding Robot Action Coordinate System in RGB Observations for Robust Visuomotor Manipulation](https://arxiv.org/abs/2606.06761)
*Jiyun Jang,Yujin Sung,Woosung Joung,Daewon Chae,Sangwon Lee,Sohwi Kim,Jinkyu Kim,Jungbeom Lee*

Main category: cs.RO

TL;DR: AxisGuide is a lightweight method that augments RGB observations with rendered robot base-frame axes to improve visuomotor policies' action-coordinate understanding, yielding significant performance gains in both simulation and real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Visuomotor manipulation policies fail under distribution shifts (e.g., object at unseen locations) due to insufficient action understanding — specifically the inability to interpret the robot's base-frame action coordinate system in image space.

Method: Using camera parameters and end-effector poses, AxisGuide renders the robot base-frame axes (x, y, z) in each camera view and augments RGB observations with additional cue channels that explicitly visualize the meaning of motions in image space.

Result: Extensive evaluations in LIBERO simulation and real-world environments show substantial performance gains and improved generalization compared to baselines.
...
### [274] [Multi-Robot Planning and Control from CCTV Camera Networks in a Real Warehouse](https://arxiv.org/abs/2606.06762)
*Luke Robinson,Benjamin Ramtoula,Anas Izaaryene,Paul Newman,Daniele De Martini*

Main category: cs.RO

TL;DR: This paper demonstrates the first real-world multi-robot fleet coordination using only an external CCTV camera network and edge compute, with robots carrying no onboard navigation hardware. The system uses a hierarchical planner operating in image-space over an uncalibrated topological camera graph to plan and coordinate robot motion, validated in a warehouse with 4 robots and 30 cameras across 6 aisles.


<details>
  <summary>Details</summary>
Motivation: To extend off-board control from single-robot to coordinated multi-robot fleets in real warehouse environments, removing the need for task-specific navigation hardware on robots and enabling scalable autonomy through distributed camera networks and edge computing.

Method: A hierarchical planner operates over an uncalibrated, pixel-wise topological camera graph. It selects a camera sequence per robot and plans image-space motion through each view. Coordination uses a prioritised-then-joint strategy, treating overlapping camera regions as shared resources held by one robot at a time to prevent collisions and deadlocks.

Result: Validated in a real warehouse with 4 robots and 30 cameras across six 27m aisles, reporting mission times and coordination statistics. First field demonstration of multi-robot planning and coordination using only an external camera network and off-board compute.

Conclusion: Off-board multi-robot coordination using only distributed CCTV networks and edge compute is feasible in real warehouse settings, offering a scalable path to autonomy without requiring robots to carry specialized navigation hardware.
...
### [275] [Learning All-Terrain Locomotion for a Planetary Rover with Actively Articulated Suspension](https://arxiv.org/abs/2606.06790)
*Arthur Bouton,Tristan D. Hasseler,Michael Paton,Travis Brown,Jacob Levy,William Reid,Joshua Martin,Hari Nayar*

Main category: cs.RO

TL;DR: ERNEST is a four-wheeled rover with an active gimbal suspension that uses reinforcement learning to train a single neural network controller for autonomous navigation over challenging heterogeneous terrains, achieving zero-shot transfer to a physical rover and outperforming passive suspensions on sandy slopes.


<details>
  <summary>Details</summary>
Motivation: To enable autonomous planetary rovers to negotiate challenging heterogeneous terrain (rock fields, steep slopes, loose sand) without requiring explicit terrain classification or controller switching, overcoming the limitations of passive suspension systems that can become immobilized.

Method: A four-wheeled rover with 2-DOF Active Gimbal Suspension (yaw+roll); single neural network controller trained via reinforcement learning using DARTS simulation engine (rigid-contact dynamics + Bekker-Wong terramechanics); policy consolidation merging terrain-specialized agents; zero-shot sim-to-real transfer via domain randomization, sensor noise injection, and system identification.

Result: Autonomous traversal of rock fields, bump trap, wheel-high step, sand ripples, and sandy slopes. On dry sand 20° slope: 37% reduction in cost of transport despite additional actuation. On wet sand: superior performance where passive suspension became immobilized.

Conclusion: A single learned controller with active suspension can generalize across diverse terrains without explicit classification, reduce energy cost on loose soil, and enable traversal where passive systems fail, demonstrating viable zero-shot transfer from simulation to physical hardware.
...
### [276] [Lane Change Trajectory Planning for Personalized Driving Comfort and Mobility Efficiency](https://arxiv.org/abs/2606.06805)
*Haoxuan Dong,Dongjun Li,Ziyou Song*

Main category: cs.RO

TL;DR: This paper proposes a neural network-based lane-change trajectory planner that uses a dual-head architecture with a gating mechanism to balance personalized driving preferences (comfort vs. mobility) with all-condition operational safety guarantees.


<details>
  <summary>Details</summary>
Motivation: Lane changing involves coupled longitudinal and lateral motions with high inter-vehicle variability, making trajectory planning highly personalized. Existing methods lack the ability to adapt to individual driver preferences while maintaining safety across diverse conditions.

Method: A neural network-driven planner combining a third-order polynomial trajectory generator with a learning module. It features a shared backbone with dual heads (one for all-condition guarantees, one for driver-specific preferences) and a head-gated switching mechanism using error-winner logistic regression for context-aware selection.

Result: Representative cases and Monte Carlo simulations demonstrate that the planner achieves personalized comfort and mobility during lane changes, while the baseline ensures feasible trajectories when personalized data is insufficient.

Conclusion: The proposed framework effectively balances personalization and safety, offering context-aware lane-change trajectory planning adaptable to varying driving conditions and driver preferences.
...
### [277] [Three-dimensional hydro-cluttered locomotion by an undulatory robot](https://arxiv.org/abs/2606.06829)
...
TL;DR: This paper presents AquaMILR, an elongate limbless robot that demonstrates how programmable body compliance, depth regulation, and emergent rolling mechanics enable effective locomotion through "hydro-cluttered" underwater environments by exploiting rather than avoiding body-obstacle contact.


<details>
  <summary>Details</summary>
Motivation: Underwater environments often contain obstacles (rigid and flexible clutter) that disrupt traditional open-water locomotion. Body-obstacle contact is unavoidable in these spaces, yet current robots lack principles for operating in such "hydro-cluttered" regimes, which remain difficult to model or simulate.

Method: The authors developed AquaMILR, an untethered, elongate limbless robot with bilateral cable-driven actuation, programmable body compliance, distributed depth regulation, and corrosion-resistant enclosures. They conducted systematic robophysical experiments across varying hydro-clutter constraint strengths and field tests in an aquatic mangrove environment.

Result: Programmable body compliance regulates deformation and converts body-environment interactions into fast, robust forward progression. Depth regulation enables 3D access to bypass clutter and recover from obstruction. In jamming scenarios, emergent inertia-induced rolling acts as a spontaneous recovery mechanism without additional control. Field tests confirmed practical navigation and visual inspection of inaccessible mangrove root zones.

Conclusion: The study establishes principles for hydro-cluttered locomotion and a design paradigm where aquatic robots exploit environmental complexity as a locomotor resource rather than treating it as an obstacle to be avoided.
...
### [278] [STRIPS-WM: Learning Grounded Propositional STRIPS-style World Models from Images](https://arxiv.org/abs/2606.06832)
*Abhiroop Ajith,Constantinos Chamzas*

Main category: cs.RO

TL;DR: STRIPS-WM learns symbolic planning operators with preconditions and effects directly from visual transitions (image, action, next image), enabling classical planning from images for long-horizon manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Robots performing long-horizon visual manipulation observe high-dimensional images, but successful plans depend only on action-relevant facts. A useful planning representation should discard irrelevant visual details while preserving action applicability and effects. Obtaining symbolic representations from raw visual experience remains challenging.

Method: STRIPS-WM first induces a finite abstract transition graph from images, then learns latent binary predicates and one grounded propositional operator per action label. The learned operators form a symbolic action model with sparse preconditions and add/delete effects. Finally, predicates are distilled into a visual encoder for classical planning from novel images.

Result: On visual rearrangement tasks, STRIPS-WM improves image-to-plan success over tested visual rollout, latent graph-search, and latent-symbolic baselines.

Conclusion: The framework successfully bridges raw visual experience and symbolic planning by learning STRIPS-style world models, enabling effective long-horizon planning from images.
...
### [279] [Think Like a Pilot: Fine-Grained Long-Horizon UAV Navigation](https://arxiv.org/abs/2606.06836)
*Xiangyi Zheng,Xiangyu Wang,Qinan Liao,Zimu Tang,Yue Liao,Dongyue Lyu,Guodong Wang,Junjie Liu,Si Liu*

Main category: cs.RO

TL;DR: The paper introduces FLIGHT, a benchmark for fine-grained long-horizon language-guided UAV navigation, and FLIGHT VLA, an asynchronous architecture combining a low-frequency streaming pilot VLM for reasoning with a high-frequency diffusion action model for control, outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: Existing VLN benchmarks use discrete/coarse actions and existing UAV VLA tasks focus on short atomic maneuvers, creating a gap for long-horizon semantic instructions requiring smooth continuous flight commands.

Method: FLIGHT VLA uses an asynchronous architecture that decouples a low-frequency Streaming Pilot VLM for task-state reasoning from a high-frequency diffusion action model for continuous control, supervised by explicit Pilot Reasoning texts.

Result: FLIGHT VLA consistently surpasses representative VLN and VLA baselines on the FLIGHT benchmarks, achieving stronger multi-stage completion, subgoal adherence, and terminal control, and improves UAV video reasoning.

Conclusion: The proposed asynchronous VLA architecture effectively bridges high-level reasoning and low-level control for long-horizon UAV navigation tasks, validated by benchmark performance and downstream video reasoning improvements.
...
### [280] [What Is My Robot Thinking? Design Considerations for Transparent and Trustworthy Shared Autonomy](https://arxiv.org/abs/2606.06870)
*Atharv Belsare,Zohre Karimi,Connor Mattson,Rushiil Nakka,Daniel S. Brown*

Main category: cs.RO

TL;DR: This paper investigates how interface transparency (visual vs. auditory feedback, sparse vs. rich information) affects coordination and trust in shared autonomy assistive robots, finding that feedback improves goal alignment but trust depends on task-appropriate disclosure rather than maximum information.


<details>
  <summary>Details</summary>
Motivation: Shared autonomy robots rely on internal intent inference that is not directly observable, leading to mismatches between inferred and intended goals that undermine coordination and trust.

Method: User study with N=25 participants across two assistive manipulation tasks, comparing visual vs. auditory feedback and sparse vs. rich information transparency designs.

Result: Feedback significantly improves intent alignment and reduces corrective intervention; participants prefer visual over auditory feedback; sparse vs. rich preferences depend on task complexity; revealing full belief distribution does not consistently improve alignment or trust.

Conclusion: Effective transparency enhances coordination through goal legibility, while trust depends on task-appropriate information exposure rather than maximal disclosure. Guidelines for designing transparent shared autonomy systems are provided.
...
### [281] [Neuro-Symbolic Learning for Long-Horizon Task Planning Under Complex Logical Constraints](https://arxiv.org/abs/2606.06877)
*Qiwei Du,Zitong Zhan,Shaoshu Su,Bowen Li,Yi Du,Zhipeng Zhao,Taimeng Fu,Sebastian Scherer,Jiaoyang Li,Chen Wang*

Main category: cs.RO

TL;DR: This paper proposes an imperative learning-based bilevel optimization framework for object-importance learning in task planning, using a 3R strategy (Repair, Restart, Rollback) to stabilize training and address exposure bias, achieving 80% failure rate reduction and 57% planning time reduction on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing neuro-symbolic planning methods suffer from train-test mismatch (exposure bias) because they use fixed offline supervision from full search spaces, but deployment uses pruned spaces from imperfect model predictions, degrading performance.

Method: Formulates object-importance learning as a bilevel optimization problem (upper: neural scorer; lower: symbolic planning in pruned space), with a novel 3R strategy (Repair, Restart, Rollback) in the lower level to provide stable and adaptive feedback.

Result: State-of-the-art results on three benchmarks: 80.04% failure rate reduction, 57.14% planning time reduction. Validated on a quadruped-based mobile manipulator in both simulation and real world.

Conclusion: The proposed framework effectively resolves exposure bias in neuro-symbolic task planning, demonstrating efficient and deployable performance in both benchmark and real-world robotic settings.
...
### [282] [A Cross-view Fusion Framework for Robust 6-DoF Grasp Pose Estimation](https://arxiv.org/abs/2606.06878)
*Kangjian Zhu,Haobo Jiang,Jianjun Qian,Jin Xie*

Main category: cs.RO

TL;DR: A cross-view fusion framework for robust 6-DoF grasp pose estimation in corner views, using self-supervised contrastive learning and a cross-view-aligned cylinder integration module, achieving strong performance on GraspNet-1Billion and real-world applications.


<details>
  <summary>Details</summary>
Motivation: Existing grasp pose estimation methods suffer from occlusion in corner views, and multi-view reconstruction is time-consuming and task-agnostic. The authors aim to enhance robustness by incorporating an auxiliary view without costly reconstruction.

Method: (1) Self-supervised contrastive learning leveraging cross-view associations (matching same 3D location vs. distinct grasp directions) to regularize point cloud features. (2) Cross-view-aligned cylinder integration module: aligns points/features by similarity, registers into cylindrical coordinates (emphasizing rotation-symmetric geometry), and uses local self-attention + seed cross-attention for cross-view interaction.

Result: Achieves strong performance on the GraspNet-1Billion benchmark and in real-world applications.

Conclusion: The proposed cross-view fusion framework effectively improves 6-DoF grasp estimation robustness under occlusion, validated both on benchmark and real-world scenarios.
...
### [283] [ActionMap: Robot Policy Learning via Voxel Action Heatmap](https://arxiv.org/abs/2606.06904)
*Pei Yang,Hai Ci,Yanzhe Chen,Qi Lv,Han Cai,Mike Zheng Shou*

Main category: cs.RO

TL;DR: ActionMap replaces the standard single-point action decoder in VLA models with a voxel heatmap head that exploits geometric action proximity, achieving consistent improvements across different backbones (+8.2% on LIBERO) with better data efficiency.


<details>
  <summary>Details</summary>
Motivation: Current VLA action decoders treat the action space as unstructured single-point predictors (autoregressive bins, L1 regression, or flow-matching), failing to exploit the geometric proximity of neighboring actions during training.

Method: ActionMap introduces a voxel heatmap action head that predicts a probability heatmap over voxelized action space, replacing the native single-point action decoder in existing VLAs.

Result: Surpasses two architecturally distinct backbones at matched training steps (e.g., +8.2% over OpenVLA-OFT on LIBERO), converges comparably or faster, and shows marked data efficiency at low training data.

Conclusion: Action representation (decoder design) is a real lever for VLA performance, distinct from backbone or recipe scaling, and the heatmap approach generalizes across backbones.
...
### [284] [T-GMP: Terrain-conditioned Generative Motion Priors for Versatile and Natural Humanoid Locomotion](https://arxiv.org/abs/2606.06944)
*Junhong Guo,Hao Hu,Chen Chen,Haoxuan Han,Linao Gong,Xin Yang,Zhicheng He,Yao Su,Fenghua He*

Main category: cs.RO

TL;DR: T-GMP uses CVAE to learn terrain-conditioned motion priors from few demonstrations, enabling a unified policy for humanoid locomotion that adapts to varying terrains while maintaining natural motions.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches for humanoid locomotion rely on fixed motion priors, limiting adaptability to varying environments. There's a fundamental challenge in achieving both anthropomorphic naturalness and robust terrain traversal.

Method: (1) Terrain-conditioned Generative Motion Priors (T-GMP) module using CVAE to learn a latent motion manifold from few expert state-terrain demonstrations. (2) Adversarial learning pipeline with Foothold Penalty, where a discriminator dynamically modulates naturalness constraints based on local terrain features.

Result: Outperforms existing baselines in traversal success rate and motion smoothness while preserving biomimetically natural and physically coordinated motions.

Conclusion: T-GMP enables smooth style transitions and unified policy adaptation to terrain variations, effectively combining naturalness with robust locomotion across diverse terrains.
...
### [285] [LIMMT: Less is More for Motion Tracking](https://arxiv.org/abs/2606.06953)
*Yu Guan,Zekun Qi,Chenghuai Lin,Xuchuan Chen,Dairu Liu,Wenyao Zhang,Jilong Wang,Xinqiang Yu,He Wang,Li Yi*

Main category: cs.RO

TL;DR: LIMMT shows that training with less than 3% of high-quality motion data (filtered by physics feasibility, diversity, and complexity) outperforms training on the full AMASS dataset for physics-based humanoid motion tracking.


<details>
  <summary>Details</summary>
Motivation: Current physics-based humanoid motion tracking typically uses large datasets containing low-quality or erroneous clips, degrading performance. The authors argue that data quality, not quantity, matters more for steering tracking policies toward better optimization trajectories early in training.

Method: LIMMT proposes a data-centric framework that filters motion data through three quality dimensions: physics feasibility, diversity, and complexity. It uses under 3% of AMASS and also performs data cleaning on web-sourced mocap data.

Result: Training with less than 3% of AMASS data (high-quality subset) achieves better tracking performance than training with the full dataset. Effectiveness is also validated on cleaned web-sourced mocap data.

Conclusion: High-quality motion data is more important than large quantities of data for training physics-based humanoid tracking policies. LIMMT demonstrates that "less is more" for motion tracking.
...
### [286] [Compliance-Based Sensor Placement for Force Sensing on a Sensorized Prostate Phantom](https://arxiv.org/abs/2606.06977)
*Sizhe Tian,Yinoussa Adagolodjo,Jeremie Dequidt*

Main category: cs.RO

TL;DR: A compliance-based sensor placement method improves force sensing in a prostate phantom for DRE training by using a weighted greedy selection strategy that prioritizes the clinically relevant posterior contact region, achieving 22.5% higher reconstructability than global QR-based placement.


<details>
  <summary>Details</summary>
Motivation: To enable effective force sensing on a sensorized prostate phantom for Digital Rectal Examination (DRE) training, with the need to maximize force observability using a limited number of sensors while respecting clinical relevance and avoiding interference with the Region of Interest.

Method: A compliance matrix relating external forces to pressure/displacement responses is built from finite-element simulation data. A weighted greedy selection strategy maximizes local force reconstructability, prioritizing the posterior contact region and avoiding marker placement in the Region of Interest.

Result: The proposed method increases the mean reconstructability score in the target region by 22.5% compared to a global QR-based placement strategy.

Conclusion: Region-aware sparse sensor placement can improve force observability in soft robotic medical phantoms while maintaining a practical, limited sensing configuration.
...
### [287] [Mission-Level Runtime Assurance Framework for Autonomous Driving](https://arxiv.org/abs/2606.06996)
*Chieh Tsai,Salim Hariri*

Main category: cs.RO

TL;DR: This paper proposes a runtime safety framework for autonomous driving that detects not only immediate safety violations but also mission-level faults (e.g., skipping checkpoints, entering restricted areas, infeasible future routes). The framework is built on highway-env, and compared against a Simplex-Drive baseline. Results show platform-level safety alone misses mission-level faults, while the proposed approach improves mission success under random faults.


<details>
  <summary>Details</summary>
Motivation: Conventional runtime-safety approaches focus mainly on immediate vehicle safety (e.g., collision avoidance) but ignore whether the vehicle can still complete its mission (e.g., reaching required checkpoints). This gap can lead to situations where the vehicle is "safe" but doomed to fail its mission.

Method: The authors extend the highway-env simulation environment with mission-level fault scenarios (skipping checkpoints, entering restricted areas, generating infeasible future routes). A runtime monitoring system detects and rejects unsafe or mission-infeasible high-level commands before execution. A baseline adapted from Simplex-Drive (learning-based driving control + safety fallback + runtime controller switching) is implemented for comparison.

Result: Platform-level runtime safety (Simplex-Drive baseline) fails to detect mission-level planning faults. The proposed framework successfully rejects mission-infeasible commands and improves mission success rates under randomized fault conditions.

Conclusion: Runtime safety for autonomous driving should incorporate both immediate vehicle-level safety and mission-level feasibility to ensure not just safe but also mission-completable driving behavior.
...
### [288] [Task Editing for Generalizable 3D Visuomotor Policy Learning](https://arxiv.org/abs/2606.07012)
*Jian-Jian Jiang,YiHan Yang,Lan Wei,Yuming Luo,Xiao-Ming Wu,Xuhang Chen,Bin Fan,Dandan Zhang,Wei-Shi Zheng*

Main category: cs.RO

TL;DR: Task-Edit is a demonstration generation framework that decomposes robotic tasks into scene, skill, and object components and recombines them flexibly, enabling scalable and diverse trajectory generation that significantly improves 3D visuomotor policy generalization for long-horizon manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D visuomotor policies require large-scale real-world demonstrations that are costly to collect. Current data augmentation methods (like object-centric transformations) only create local variations preserving original scene structure and skill sequences, limiting their ability to synthesize diverse scene-skill-object combinations for complex tasks.

Method: Task-Edit decomposes a robotic task into three components: scene, skill, and object. It then flexibly recombines these components through a task-centric editing perspective, enabling the generation of diverse trajectories beyond simple local transformations.

Result: Extensive real-world experiments show Task-Edit provides three advantages: (1) Effectiveness - significantly improves 3D visuomotor policies across various tasks and robot embodiments; (2) Generalizability - improves model generalization across different scenario setups; (3) Applicability - enables handling scenarios difficult to collect in the real world, such as disturbance resistance, obstacle avoidance, and unseen cluttered scenes.

Conclusion: Task-Edit offers a scalable demonstration generation approach that enhances 3D visuomotor policy performance and generalization for complex long-horizon manipulation tasks, particularly in scenarios where real-world data collection is challenging.
...
### [289] [A Multi-Operator Mixed-Reality Interface for Multi-Robot Control and Coordination: Co-Located and Private Workspace Collaboration](https://arxiv.org/abs/2606.07013)
*Omotoye Shamsudeen Adekoya,Antonio Sgorbissa,Carmine Tommaso Recchiuto*

Main category: cs.RO

TL;DR: This paper presents a mixed-reality interface (HORUS) for multi-operator control of robot teams, comparing co-located shared workspace vs. private-workspace modes. A 36-participant user study found both modes effective for task performance, but the co-located shared workspace significantly improved perceived collaboration, shared understanding, and was the preferred mode.


<details>
  <summary>Details</summary>
Motivation: Multi-operator control of robot teams requires shared awareness and prevention of conflicting interventions, which existing single-operator systems fail to support for collaborative team use.

Method: Extended the HORUS interface to support two modes: co-located shared workspace (same physical mini-map) and private-workspace mode (independent local workspaces). Architecture uses registration-driven scene construction, lightweight shared-session synchronization, and per-robot control leases. Evaluated with 36 participants (18 pairs) controlling 3 Nova Carter mobile robots in two search environments.

Result: Objective task performance was comparable across both modes. Co-located shared workspace significantly improved perceived collaboration, shared understanding, and handoff clarity, and was the preferred collaborative mode.

Conclusion: Physically co-locating the mixed-reality workspace improves operator coordination even when underlying robot-control tools remain unchanged, suggesting workspace physicality matters for team collaboration.
...
### [290] [Extending Responsibility-Sensitive Safety for the Assessment of Offloaded Autonomous Driving Services](https://arxiv.org/abs/2606.07067)
*Robin Dehler,Aryan Thakur,Michael Buchholz*

Main category: cs.RO

TL;DR: This paper extends Responsibility-Sensitive Safety (RSS) definitions to account for variable response times in offloaded autonomous driving services, enabling safe function offloading decisions and fallback mechanisms that improve safety while preserving computational benefits.


<details>
  <summary>Details</summary>
Motivation: Function offloading benefits computational efficiency and energy consumption in AD systems, but introduces safety challenges due to increased and variable V2X communication response times that affect vehicle reaction time and safety guarantees.

Method: The authors extend RSS definitions to differentiate response times of local vs. offloaded service compositions, integrate these safety constraints into offloading decision-making, and introduce an enhanced fallback strategy with a warm-standby phase for safer transitions.

Result: The proposed method improves safety compared to state-of-the-art function offloading and safety frameworks, as demonstrated through both simulation and real-world experiments.

Conclusion: The approach successfully balances safety guarantees and distributed computation benefits by permitting offloading only when traffic situations remain safe under corresponding end-to-end response times, with controlled fallback mechanisms when violated.
...
### [291] [Predictive Style Matching: Natural and Robust Humanoid Locomotion](https://arxiv.org/abs/2606.07083)
*Simeon Nedelchev,Ekaterina Chaikovskaia,Egor Davydenko,Eduard Zaliaev,Roman Gorbachev*

Main category: cs.RO

TL;DR: PSM uses an offline predictor to provide state-conditioned style targets during training, achieving near-imitation-level motion quality while preserving task-only RL's robustness to disturbances on the Unitree G1 robot.


<details>
  <summary>Details</summary>
Motivation: Task-only RL produces stiff, asymmetric gaits, while motion imitation improves appearance but becomes sensitive to disturbances. The goal is to achieve both high motion quality and fall-recovery robustness.

Method: Predictive Style Matching (PSM): an offline predictor maps robot's lower-body state history and velocity commands to interpretable upper-body joint and gait targets, used only during training (not deployment). Targets are state-conditioned, not time-indexed.

Result: On Unitree G1 (sim & hardware), PSM reduces upper-body style error by ~10x over task-only RL while preserving fall-recovery rate. Motion-imitation baseline has lowest style error but fails ~5x more often under disturbances.

Conclusion: PSM achieves robotic locomotion that is both aesthetically natural and robust to disturbances, bridging the gap between task-only RL and motion imitation approaches.
...
### [292] [Dreaming when Necessary: Advancing World Action Models with Adaptive Multi-Modal Reasoning](https://arxiv.org/abs/2606.07089)
*Yinzhou Tang,Jingbo Xu,Yu Shang,Zihao Song,Chen Gao,Wei Wu,Yong Li*

Main category: cs.RO

TL;DR: AdaWAM introduces an adaptive multimodal reasoning world action model that dynamically switches between textual and visual reasoning based on task context, improving efficiency and performance in embodied tasks.


<details>
  <summary>Details</summary>
Motivation: Existing World Action Models rely heavily on video prediction and lack adaptive multimodal reasoning, limiting their effectiveness on long-horizon, complex tasks. Different execution contexts (task transitions vs. fine-grained manipulation) require different reasoning modes.

Method: AdaWAM integrates a lightweight dynamic router that autonomously triggers textual reasoning (for high-level action prediction during task transitions) or visual reasoning (for precise control during fine-grained manipulation) as needed.

Result: Experiments on both simulated and real-world embodied tasks show AdaWAM substantially improves inference efficiency while outperforming state-of-the-art embodied policies.

Conclusion: Adaptive multimodal reasoning (dynamic switching between text and visual modalities) is an effective and efficient paradigm for World Action Models in complex, long-horizon embodied tasks.
...
### [293] [Coarse-to-Control: Action-Token Planning for Vision-Language-Action Models](https://arxiv.org/abs/2606.07107)
...
TL;DR: Coarse-to-Control is a VLA model that plans in action-token space by first predicting coarse tokens summarizing future trajectory, then generating executable actions. This improves long-horizon task performance.


<details>
  <summary>Details</summary>
Motivation: Standard VLA models map observations directly to actions without planning, causing error accumulation on long-horizon tasks.

Method: The policy first predicts compact coarse action tokens (planning phase), then generates executable fine action tokens conditioned on that plan (execution phase), all within a unified discrete action vocabulary.

Result: Outperforms direct action generation on LIBERO, SimplerEnv-WidowX, and real-world tasks, with biggest gains on long-horizon multi-stage tasks.

Conclusion: Action-token planning in a shared vocabulary keeps plans close to the control manifold and provides directly actionable guidance, improving long-horizon manipulation.
...
### [294] [QuadVerse: An Integrated Framework Aligning Visual-Physical Reality for Quadruped Simulation](https://arxiv.org/abs/2606.07118)
*Yuxiang Chen,Yuanhao Wang,Ziheng Zhang,Meng Zhang,Yu Liu,Yufei Jia,Tiancai Wang,Erjin Zhou,Jin Xie*

Main category: cs.RO

TL;DR: QuadVerse is an integrated sim-to-real framework that reconstructs 3D scenes from RGB video to calibrate visual, contact, and actuator dynamics, enabling zero-shot robot policy deployment without real-world task-specific rollouts.


<details>
  <summary>Details</summary>
Motivation: Existing sim-to-real methods address visual and dynamic gaps separately, overlooking how individual mismatches accumulate and propagate throughout the robot's state evolution.

Method: QuadVerse uses 3D Gaussian Splatting (3DGS) scenes from RGB video for photorealistic rendering and mesh extraction. It calibrates contact via spatially varying friction priors refined through trajectory-based posterior search, and trains a residual dynamics compensator by replaying real-world trajectories on calibrated terrain.

Result: QuadVerse improves reconstruction quality and locomotion tracking over baselines, and enables robust zero-shot visual-navigation policy deployment without task-specific real-world rollouts.

Conclusion: An integrated calibration approach addressing visual, contact, and actuator gaps simultaneously can effectively reduce the sim-to-real gap for zero-shot policy transfer.
...
### [295] [Test-Time Trajectory Optimization for Autonomous Driving](https://arxiv.org/abs/2606.07170)
*Yihong Xu,Eloi Zablocki,Yuan Yin,Elias Ramzi,Ellington Kirby,Alexandre Boulch,Matthieu Cord*

Main category: cs.RO

TL;DR: TOAD improves autonomous driving planners by using the scorer as a reward function and searching for better trajectories via the Cross-Entropy Method at test time, without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end planners generate candidate trajectories first, then score them—but the scorer cannot influence proposal generation, limiting performance when the candidate set is weak.

Method: TOAD treats the planner's scorer as a learned trajectory-level reward function and applies the Cross-Entropy Method (CEM) at test time, warm-started from the planner's original proposals.

Result: TOAD improves results across six base planners on NAVSIM-v1 (94.7 PDMS), NAVSIM-v2 (56.3 EPDMS), and the closed-loop HUGSIM benchmark.

Conclusion: The scorer can serve as an effective reward function for test-time trajectory optimization, offering a plug-and-play improvement without retraining.
...
### [296] [A Causal Probabilistic Framework for Perception-Informed Closed-Loop Simulation of Autonomous Driving](https://arxiv.org/abs/2606.07186)
...
TL;DR: This paper proposes a perception-informed SIL testing methodology that injects realistic perception errors (e.g., loss of detection, sizing inaccuracies) derived from physical conditions (fog, rain, object merging) into standardized simulation, revealing safety risks that ideal-sensing simulations miss, thus supporting SOTIF (ISO 21448) validation.


<details>
  <summary>Details</summary>
Motivation: Current SIL simulation frameworks use ideal sensing, bypassing perception algorithm limitations and leading to over-optimistic safety assessments for ADAS/ADS.

Method: A perception-informed SIL testing framework that incorporates causal probabilistic models into standardized scenario-based simulation toolchains, enabling systematic injection of realistic perception errors (loss of detection, sizing inaccuracies, positioning offsets) derived from physical conditions (fog, rain, object merging).

Result: Perception-informed testing reveals latent operational risks that ideal SIL environments fail to capture.

Conclusion: The proposed approach provides a scalable pathway for SOTIF (ISO 21448) validation by bridging the gap between ground-truth simulation and real-world perception behavior.
...
### [297] [Shield-Loco: Shielding Locomotion Policies with Predictive Safety Filtering](https://arxiv.org/abs/2606.07193)
*Aditya Shirwatkar,Sebastian Sanokowski,Shishir Kolathaya,Aaron Johnson,Majid Khadiv*

Main category: cs.RO

TL;DR: A predictive safety filter uses sampling-based optimization with a full-physics model to modify contact locations for RL-based legged locomotion, preventing collisions while maintaining task performance.


<details>
  <summary>Details</summary>
Motivation: RL policies for legged locomotion lack safety constraint handling for unseen scenarios, and existing safety methods either rely on oversimplified models or conservative controllers that degrade performance.

Method: A predictive safety filter post-hoc filters nominal contact locations using: (1) geometric projection of sampled contacts, (2) momentum-augmented updates, (3) replica-exchange optimization to handle discontinuous contact landscapes.

Result: Validated on a quadruped robot in dense cluttered environments (simulation and real-world), showing substantial reductions in safety violations with minimal deviation from nominal input.

Conclusion: The proposed approach enables safer legged locomotion without compromising task performance or requiring exhaustive offline safe learning.
...
### [298] [An Abstract Architecture for Explainable Autonomy in Hazardous Environments](https://arxiv.org/abs/2606.07211)
*Matt Luckcuck,Hazel M Taylor,Marie Farrell*

Main category: cs.RO

TL;DR: This paper presents an abstract architecture for designing explainable autonomous systems that can explain their behavior, aimed at building trust with human users in hazardous environments like the nuclear industry.


<details>
  <summary>Details</summary>
Motivation: To ensure human workers trust autonomous robotic systems used in hazardous environments, as explainability is closely linked to trustworthiness and should be designed into systems from the start rather than added afterward.

Method: The authors propose an abstract architecture that supports explainable autonomy, serving as a design template for implementing systems that can explain their own behavior.

Result: A worked example demonstrating how the proposed architecture could be applied in the civil nuclear industry context, addressing trust needs of both workers and regulators.

Conclusion: Explainability should be designed into autonomous systems from the outset, and the proposed architecture provides a practical template for achieving trustworthy, explainable autonomy in high-stakes environments.
...
### [299] [Robotic Policy Adaptation via Weight-Space Meta-Learning](https://arxiv.org/abs/2606.07217)
*Christian Bianchi,Siamak Yousefi,Alessio Sampieri,Andrea Roberti,Luca Rigazio,Fabio Galasso,Luca Franco*

Main category: cs.RO

TL;DR: WIZARD is a weight-space meta-learning framework that generates task-specific LoRA parameters for frozen VLA policies from just a language instruction and demonstration video, eliminating the need for task-specific action labels or fine-tuning, achieving up to 14x improvement on unseen tasks.


<details>
  <summary>Details</summary>
Motivation: VLA models require task-specific demonstrations, action annotations, and fine-tuning for new tasks, making deployment costly and difficult to scale.

Method: WIZARD uses weight-space meta-learning to predict task-specific LoRA parameters for a frozen VLA policy in a single forward pass, given only a language instruction and short demonstration video, without test-time optimization or action labels.

Result: Up to ~2x improvement on unseen dataset collections and up to ~14x improvement on unseen tasks in LIBERO; consistent improvement on real Franka Emika Panda robot.

Conclusion: Generated adapters provide task-level specialization beyond simulation, enabling efficient adaptation without costly fine-tuning.
...
### [300] [Beyond Waypoints: A Trajectory-Centric Waypointing Paradigm for Vision-Language Navigation](https://arxiv.org/abs/2606.07244)
*Haoxiang Shi,Xiang Deng,Haoyu Zhang,Qiaohui Chu,Yaowei Wang,Liqiang Nie*

Main category: cs.RO

TL;DR: This paper introduces Trajectory Waypoint, a new paradigm for VLN-CE that replaces isolated waypoints with executable trajectories using a TSDF-guided diffusion policy, improving reachability and consistency between planning and control.


<details>
  <summary>Details</summary>
Motivation: The decoupled three-stage framework (waypoint predictor → navigator → controller) used in most VLN-CE approaches leads to unreachable waypoints and inconsistencies between high-level planning and low-level control.

Method: Proposes a Trajectory Waypoint Paradigm: (1) a Trajectory Waypoint Predictor using TSDF-guided diffusion policy to generate executable trajectories avoiding obstacles, (2) a trajectory-enhanced navigator that uses trajectory information for consistent planning.

Result: Superior performance over baselines on the VLN-CE benchmark.

Conclusion: Grounding waypoints in executable trajectories and using trajectory information for navigation planning improves both reachability and consistency in VLN-CE tasks.
...
### [301] [CAPE: Contrastive Action-conditioned Parallel Encoding for Embodied Planning](https://arxiv.org/abs/2606.07304)
*Cong Chen,Haowen Wang,Zhixiang Zhang,Pei Ren,Zhengping Che*

Main category: cs.RO

TL;DR: CAPE is a contrastive learning framework for visual dynamics that predicts future action outcomes efficiently by focusing on action-conditioned changes rather than reconstructing all visual details.


<details>
  <summary>Details</summary>
Motivation: Existing visual dynamics models waste capacity on reconstructing visually salient but planning-irrelevant content, rather than focusing on action-conditioned changes critical for manipulation outcomes.

Method: CAPE decodes full future latent trajectories in a single forward pass using a Goal-Convergent Contrastive Objective that aligns same-outcome predictions and separates different-outcome predictions.

Result: On DROID and RoboCasa, CAPE outperforms baselines on future-state retrieval, offline action matching, and closed-loop planning, with reduced inference cost for long horizons.

Conclusion: Contrastive action-conditioned parallel encoding is an effective and efficient approach for learning visual dynamics for embodied agent planning.
...
### [302] [RhinoVLA Technical Report](https://arxiv.org/abs/2606.07383)
...
TL;DR: RhinoVLA is a deployment-oriented VLA model co-designed with the Huixi R1 edge SoC that reduces token/computation burden via a token-efficient Qwen3-VL backbone and continuous Action Expert, supports cross-robot learning with a unified interface, and achieves 11.69 Hz real-time inference on edge hardware while matching π0.5 performance.


<details>
  <summary>Details</summary>
Motivation: VLA models show strong potential for robotic manipulation but suffer from high deployment latency on edge hardware due to VLM visual and context tokens causing linear computation growth in GEMM-dominated projection operators.

Method: RhinoVLA adopts a token-efficient Qwen3-VL backbone and continuous Action Expert to reduce VLM-side token/computation burden; introduces a unified interface (View Registry, 72D physical state-action slot space, robot-instance LoRA) for cross-robot learning; and optimizes deployment via hardware-aware compilation, mixed-precision execution, and parallel visual encoding on Huixi R1 edge SoC.

Result: RhinoVLA achieves 11.69 Hz end-to-end inference on Huixi R1 (meeting 10 Hz real-time target) with performance comparable to π0.5 at similar parameter scale.

Conclusion: RhinoVLA demonstrates that co-designing VLA models with edge hardware can achieve real-time closed-loop control without sacrificing downstream task performance, enabling practical edge deployment for robotic manipulation.
...
### [303] [Spline Policy: A Structured Representation for Robot Policies](https://arxiv.org/abs/2606.07386)
*Mengze Tian,Yiming Li,Sichao Liu,Auke Ijspeert,Sylvain Calinon*

Main category: cs.RO

TL;DR: The paper introduces Spline Policy (SP), a representation that replaces discrete action chunks with continuous spline parameters for robot manipulation policies, enabling compact decoding, temporal resampling, local correction, uncertainty propagation, and controller compatibility without changing the policy backbone.


<details>
  <summary>Details</summary>
Motivation: Modern imitation-learning policies use fixed-resolution action chunks that lack geometric and temporal structure. The authors aim to introduce a more structured representation (spline-based) that exposes these properties while keeping existing policy backbones unchanged.

Method: SP replaces action chunks with spline parameters as the policy output. The spline decodes into a continuous trajectory, supports temporal resampling, parameter-space editing, and can be converted into a state-dependent vector field via analytical distance-field construction. This construction provides a local corrective mechanism. SP is instantiated with diffusion, flow-matching, transformer-based, and vision-language-action backbones.

Result: Experiments in low-dimensional motion learning, simulated manipulation, dexterous manipulation, and real-robot case studies show SP remains compatible with modern policy learners while exposing motion-structure properties like compact decoding, temporal resampling, local correction, uncertainty evaluation, and controller compatibility.

Conclusion: Spline Policy is a versatile, structured alternative to action chunks that enhances robot manipulation policies with continuous motion structure and useful control properties without requiring backbone changes.
...
### [304] [Simulation-Driven Imitation Learning for Biosignals-Free Shared-Autonomy Prosthetic Grasping](https://arxiv.org/abs/2606.07389)
*Kaijie Shi,Wanglong Lu,Huiling Chen,Vinicius Prado da Fonseca,Ting Zou,Hanli Zhao,Xianta Jiang*

Main category: cs.RO

TL;DR: The paper presents a scalable simulation framework that automatically generates diverse reach-to-grasp demonstrations for training imitation-learning-based shared-autonomy control of upper-limb prosthetic hands, achieving over 90% grasp success in sim-to-real transfer without relying on biosignals.


<details>
  <summary>Details</summary>
Motivation: Existing imitation-learning-based approaches for biosignals-free prosthetic hand control show promise but suffer from scalability issues due to the high cost and variability of collecting large-scale real-world human demonstration data.

Method: The framework combines physically feasible grasp synthesis, natural reaching trajectory retargeting, and reach-grasp-lift execution in procedurally generated indoor environments, using a wrist-mounted virtual camera to record observations, proprioception, and actions for imitation learning.

Result: In simulation benchmarks and three realistic settings, the learned policy achieves over 90% grasp success, surpasses baseline methods, and demonstrates stronger object and scene generalization.

Conclusion: Simulation-driven training is a promising approach for biosignals-free shared-autonomy prosthetic grasping, offering scalable and effective policy learning without costly real-world data collection.
...
### [305] [Rapid co-design of Buoyancy-assisted robots for Challenging Locomotion using Gaussian Evolutionary Specialists](https://arxiv.org/abs/2606.07424)
*Ankit Sinha,Nitish Sontakke,Dennis Hong,Yusuke Tanaka,Sehoon Ha*

Main category: cs.RO

TL;DR: Gaussian Evolutionary Specialists (GES) framework decouples design-space partitioning from policy learning to optimize both morphology and control for legged robots, achieving 5-25% higher performance than universal policies, 3x obstacle height improvement, and 37% reduction in design optimization time.


<details>
  <summary>Details</summary>
Motivation: Co-designing morphology and control for legged robots via RL is expensive due to repeated policy training. Universal policies suffer from behavioral diversity collapse, and Mixture-of-Experts architectures also fail due to representation collapse.

Method: GES decouples design-space partitioning from policy learning by assigning specialist policies to evolving Gaussian regions, iteratively refining them through training, probing, and territory expansion. Specialists are then integrated into a design sampling loop, replacing costly re-training with direct evaluation.

Result: On BALLU robot, GES achieves 5-25% higher performance than universal policies; hardware tests show 3x improvement in obstacle height (24 cm vs baseline); design optimization time reduced by 37%.

Conclusion: GES effectively addresses behavioral diversity collapse in co-design, enabling efficient morphology optimization with improved performance and reduced computational cost.
...
### [306] [Re-imagining ISO 26262 in the Age of Autonomous Vehicles: Enhancing Controllability through Transferability and Predictability](https://arxiv.org/abs/2606.07437)
*Chaitanya Shinde,Hadi Hajieghrary,Paul Schmitt,Adam Shoemaker,Bodo Seifert,Steve Kenner*

Main category: cs.RO

TL;DR: This paper extends ISO 26262 for autonomous vehicles by decomposing the Controllability placeholder into two measurable dimensions: Transferability (hand-off to fallback systems) and Predictability (how well external agents anticipate AV behavior), with formal quantification metrics to make safety claims falsifiable and traceable across ODD slices.


<details>
  <summary>Details</summary>
Motivation: ISO 26262's Controllability concept assumes a human driver, which breaks down for SAE Level 4/5 autonomous vehicles lacking a human fallback. A measurable, auditable replacement is needed to assess functional safety in driverless systems.

Method: The authors decompose Controllability into Transferability (system's ability to hand off to fallback safety mechanisms) and Predictability (how easily external agents can anticipate AV behavior), formalize Predictability via human-robot interaction principles with mathematical quantification, and introduce a designed-versus-achievable gap to distinguish architectural fallback claims from scene-conditioned achievable fallback capability.

Result: The proposed metrics align with ISO 26262 and ISO/PAS 21448 (SOTIF), rendering fallback and interaction claims falsifiable and traceable across ODD slices while preserving the original standard's structure.

Conclusion: The enhancements extend ISO 26262's applicability to driverless SAE Level 4 and 5 systems by complementing (rather than replacing) existing standards with auditable, measurable safety dimensions.
...
### [307] [Planning-aligned Token Compression for Long-Context Autonomous Driving](https://arxiv.org/abs/2606.07464)
...
TL;DR: COMPACT-VA is a planning-aligned working memory framework using conditional VQ-VAE to compress extended temporal context in monolithic vision-action models for autonomous driving, achieving >6% success rate improvement with 3.3x speedup and 2.7x memory reduction.


<details>
  <summary>Details</summary>
Motivation: Monolithic vision-action models produce token sequences exceeding real-time budgets when encoding extended temporal context. Existing token compression uses rule-based heuristics (e.g., temporal decay) decoupled from planning, risking loss of decision-critical information.

Method: A conditional VQ-VAE framework where compression is conditioned on historical trajectory and a learned planning intent. The posterior encoder distills planning intent from future trajectories during training, while the prior encoder predicts it from compressed observations. The compressed memory concatenated with predicted latent feeds the policy for end-to-end optimization.

Result: >6% improvement (68.3%) on success rates with consistent gains across behavioral metrics. Closed-loop evaluation shows maintained driving performance with 3.3x speedup and 2.7x memory reduction over uncompressed processing.

Conclusion: Planning-aligned coupling is effective for token compression in vision-action models, retaining decision-critical information while achieving significant efficiency gains.
...
### [308] [Affordance-Based Hierarchical Reinforcement Learning for Quadruped Pedipulation](https://arxiv.org/abs/2606.07506)
*Tuba Girgin,Jose Castelblanco,Gabriel Rodriguez,Emre Girgin,Cagri Kilic*

Main category: cs.RO

TL;DR: Proposes a three-level hierarchical RL framework for quadruped robots that uses pose and interaction-point affordances to autonomously select interaction poses and manipulation points, removing the need for pre-designed trajectories in object manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Current quadruped object manipulation relies on expert-designed high-level trajectories; autonomous selection of affordable interaction points and robot base poses is needed to eliminate this dependency.

Method: A three-level hierarchical RL framework where pose affordances guide navigation policy, which drives locomotion policy, while interaction-point affordances guide pedipulation policy for object-centric pose alignment and manipulation planning.

Result: The framework autonomously identifies candidate poses based on affordance and successfully executes object manipulation tasks in real world without human guidance, validated in both simulation (IsaacSim) and real-world settings.

Conclusion: The proposed affordance-guided hierarchical RL approach enables quadruped robots to autonomously perform object manipulation tasks without pre-designed trajectories or human guidance.
