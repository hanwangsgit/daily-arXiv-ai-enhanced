...
TL;DR: PuzzlePole is a new cylindrical fiducial marker based on PuzzleBoard pattern that enables 360° recognition and robust pose estimation for autonomous systems.


<details>
  <summary>Details</summary>
Motivation: Need for reliable environment perception in autonomous systems, where current visual markers lack 360° viewing capability and robustness to occlusions.

Method: Developed cylindrical marker using PuzzleBoard's combinatorial pattern structure to enable recognition from all viewing angles while maintaining accuracy.

Result: PuzzlePoles provide reliable recognition and pose estimation from 360° viewing direction with high accuracy and occlusion robustness.

Conclusion: The flexible cylindrical marker design enables diverse applications in robot navigation, SLAM, and tangible interfaces for autonomous systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing text-to-image evaluation methods fail to capture individual user preferences and visual tastes, as they rely on general reward functions or similarity metrics that don't account for personal diversity.

Method: PIGReward uses a self-bootstrapping strategy with Chain-of-Thought reasoning over limited reference data to construct rich user contexts, dynamically generating user-conditioned evaluation dimensions and providing personalized feedback for prompt optimization.

Result: Extensive experiments show PIGReward surpasses existing methods in accuracy and interpretability, and the authors introduce PIGBench, a per-user preference benchmark for diverse visual interpretations.

Conclusion: PIGReward establishes a scalable, reasoning-based foundation for personalized T2I evaluation and optimization, representing a robust step toward individually aligned text-to-image generation.
...
TL;DR: SG-OIF is a stability-guided online influence framework that addresses challenges in computing training point influence on test predictions for deep learning vision models, achieving state-of-the-art performance on noise-label and out-of-distribution detection tasks.


<details>
  <summary>Details</summary>
Motivation: Existing influence function implementations are challenging for deep learning vision models due to expensive inverse-curvature computations, training non-stationarity, and fragile rankings that misidentify critical examples. Offline computation lags behind training dynamics, and missing confidence calibration yields unreliable results.
...
Result: SG-OIF achieves state-of-the-art performance: 91.1% accuracy in top 1% prediction samples on CIFAR-10 (20% asymmetric noise) and 99.8% AUPR score on MNIST. It performs well across multiple datasets with various corruption types for noise-label and out-of-distribution detection tasks.

Conclusion: SG-OIF is a practical controller for online influence estimation that effectively addresses computational challenges and training dynamics in deep learning vision models, providing robust influence scoring for identifying noisy data and critical examples.
...
TL;DR: Pistachio is a new Video Anomaly Detection/Understanding benchmark created using a controlled generation pipeline to address limitations in existing datasets, providing diverse scenes, balanced anomalies, and temporal complexity.


<details>
  <summary>Details</summary>
Motivation: Existing VAD benchmarks lack scene diversity, balanced anomaly coverage, and temporal complexity needed for real-world assessment, while VAU requires deeper semantic reasoning but is difficult to benchmark due to manual annotation demands.

Method: Leverages video generation models in a controlled pipeline with scene-conditioned anomaly assignment, multi-step storyline generation, and temporally consistent long-form synthesis to produce coherent 41-second videos with minimal human intervention.

Result: Pistachio demonstrates scale, diversity, and complexity, revealing new challenges for existing methods and motivating future research on dynamic and multi-event anomaly understanding.

Conclusion: The generation-based approach effectively eliminates biases of Internet-collected datasets and provides a robust benchmark for advancing VAD/VAU research.
...
Method: Proposes Decoupled Mixture-of-Expert (DeMoE) to separate cross-modal shared knowledge from specific information, and Task-aware Multi-object Tracking (TaMOT) pipeline to unify all task outputs as instances with calibrated ID information.
...
TL;DR: This paper introduces the DRaM (determinant ratio matrix) approach for solving 3D-3D (EnP) and 3D-2D orthographic (OnP) pose estimation problems, providing both exact solutions for error-free cases and methods for noisy data.


<details>
  <summary>Details</summary>
Motivation: To address the lack of closed-form solutions for noisy 3D-2D orthographic pose estimation (OnP problem) and to provide a unified framework for pose estimation problems that generalizes across dimensions.

Method: Uses the DRaM (determinant ratio matrix) approach to solve least squares systems for error-free EnP and OnP problems, with a rotation correction scheme for noisy data. Places existing methods (QR decomposition, Moore-Penrose pseudoinverse) in the broader DRaM family context.

Result: Presents new solutions to 3D and 2D orthographic pose estimation problems, shows that DRaM solutions could have been discovered in Gauss's time, and demonstrates that the approach generalizes to all N-dimensional Euclidean pose estimation problems.

Conclusion: The work provides both practical solutions to pose estimation problems and valuable theoretical insights, establishing the DRaM family as a comprehensive framework for Euclidean pose estimation across dimensions.
...
TL;DR: Introduces Eyelid Angle (ELA), a novel 3D metric for eye openness that's robust to camera angle changes, enabling accurate blink detection and synthetic dataset generation for driver drowsiness monitoring.


<details>
  <summary>Details</summary>
Motivation: Reliable driver drowsiness detection is crucial for road safety and ADAS systems, but existing methods like EAR are sensitive to camera angles and natural drowsiness data is scarce and risky to collect.

Method: Developed ELA metric from 3D facial landmarks to measure eye openness geometrically, designed blink detection framework using temporal characteristics, and created synthetic datasets by animating avatars in Blender 3D with controllable parameters.

Result: ELA shows lower variance under viewpoint changes compared to EAR, achieves accurate blink detection, and synthetic augmentation expands training data diversity for drowsiness recognition.

Conclusion: ELA serves as both a reliable biometric measure and powerful tool for generating scalable datasets in driver state monitoring, offering robustness and synthetic data generation capabilities.
...
TL;DR: LatentDreamer is a fast 3D generation framework that uses a pre-trained VAE to map 3D geometries to latent features, enabling sequential generation of coarse geometries, refined geometries, and textures in ~70 seconds.


<details>
  <summary>Details</summary>
Motivation: Current automatic 3D generation methods struggle to achieve fast, detailed, and high-fidelity generation simultaneously from single images.

Method: Uses a pre-trained variational autoencoder to map 3D geometries to latent features, then sequentially generates coarse geometries, refined geometries, and realistic textures.

Result: Generates 3D objects with high fidelity to input images in about 70 seconds, showing competitive performance with minimal training compared to contemporary approaches.

Conclusion: LatentDreamer provides an efficient framework for fast, high-fidelity 3D object generation from single images by leveraging latent feature representations.
...
### [9] [Cross-Domain Generalization of Multimodal LLMs for Global Photovoltaic Assessment](https://arxiv.org/abs/2511.19537)
*Muhao Guo,Yang Weng*

Main category: cs.CV

TL;DR: A multimodal LLM approach for global photovoltaic assessment that outperforms traditional CV models in cross-domain generalization, achieving robust performance across unseen regions with minimal degradation.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of distributed PV systems creates grid management challenges due to undocumented installations, and traditional computer vision models require extensive labeled data and fail to generalize across different regions.

Method: Uses a multimodal large language model with structured prompts and fine-tuning to integrate detection, localization, and quantification of PV systems within a unified schema.

Result: Cross-regional evaluation using ΔF1 metric shows the proposed model achieves the smallest performance degradation across unseen regions, outperforming conventional CV and transformer baselines.
...
TL;DR: This paper explores the relationship between model capacity and the minimum number of visual tokens needed to preserve image semantics, revealing that larger models require fewer tokens.


<details>
  <summary>Details</summary>
Motivation: To investigate the fundamental relationship between model capacity and the minimal number of visual tokens required to preserve image semantics, inspired by the Minimum Description Length principle.

Method: Proposes Orthogonal Filtering, a lightweight module that adaptively clusters redundant tokens into a compact set of orthogonal bases, and conducts extensive experiments across ViT models.
...
Conclusion: The study demonstrates that model capacity inversely correlates with the number of tokens needed to represent visual semantics, with larger models achieving semantic preservation with fewer tokens.
...
<details>
  <summary>Details</summary>
Motivation: Existing visual grounding methods require extensive task-specific annotations and fine-tuning, limiting their generalization to novel scenarios. The goal is to create a framework that can perform visual grounding without fine-tuning.

Method: Uses iterative reasoning with pretrained open-vocabulary object detectors, multimodal LLMs, and LLMs to progressively refine candidate regions through joint semantic and spatial analyses.

Result: Achieves 65.1% average zero-shot grounding accuracy on RefCOCO benchmarks without fine-tuning. With MLLM-generated captions replaced by original queries, selection accuracy reaches ~90%, matching supervised performance.

Conclusion: GroundingAgent demonstrates strong zero-shot visual grounding capabilities, highlighting the importance of LLM reasoning, and provides transparent interpretability through step-by-step reasoning.
...
TL;DR: InfoPrune is an information-theoretic framework for compressing vision-language models using Information Bottleneck principle, achieving 3.2x FLOP reduction with minimal performance loss.
...
Method: Uses Information Bottleneck principle with entropy-based effective rank and KS distance to measure attention head importance, plus two schemes: training-based head pruning and training-free FFN compression via low-rank approximation.
...
Conclusion: InfoPrune provides a theoretically grounded and effective approach for efficient multimodal large models.
...
TL;DR: VideoChat-M1 is a multi-agent system for video understanding that uses collaborative policy planning with multiple policy agents that dynamically refine their tool invocation strategies through communication and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent frameworks for video understanding use static, non-learnable tool invocation mechanisms that limit discovery of diverse clues needed for robust perception in complex videos.

Method: Collaborative Policy Planning (CPP) paradigm with three processes: policy generation (each agent creates unique tool invocation policy), policy execution (sequential tool invocation), and policy communication (agents interact to update policies). Enhanced with Multi-Agent Reinforcement Learning (MARL) for joint optimization.
...
Conclusion: The collaborative multi-agent framework with dynamic policy refinement and reinforcement learning enables more effective video understanding, particularly for temporally or spatially complex videos.
...
<details>
  <summary>Details</summary>
Motivation: Current vision-language benchmarks focus on surface-level recognition or image-text alignment, lacking comprehensive evaluation of the fundamental human cognitive process of structured scene understanding that involves recognizing objects, spatial relationships, and inferring task-relevant properties.

Method: Created a benchmark by annotating 3173 objects with 84 fine-grained attributes across four property families, then constructed 28033 template-based multiple-choice questions across four types (object description, spatial reasoning, property matching, taxonomy reasoning) plus 50 expert-crafted questions.

Result: Leading vision-language models perform well on recognition tasks but degrade by 10-20% on property-driven questions, especially those requiring multi-step reasoning over structured attributes. In-context reasoning examples from simulated scenes improve performance on real-world and expert-curated questions.

Conclusion: There is a persistent gap in structured visual understanding, with current models relying heavily on pattern matching rather than true reasoning. Perceptual-taxonomy-guided prompting demonstrates effectiveness in improving model performance on complex reasoning tasks.
...
TL;DR: MapRF is a weakly supervised framework for online HD map construction using only 2D image labels, achieving ~75% of fully supervised performance through NeRF-based pseudo label generation and self-training.


<details>
  <summary>Details</summary>
Motivation: Existing HD map construction methods rely on costly 3D annotations, limiting generalization and scalability across diverse driving environments.

Method: Uses Neural Radiance Fields conditioned on map predictions to generate view-consistent 3D pseudo labels, then iteratively refines map network via self-training with Map-to-Ray Matching to mitigate error accumulation.

Result: Achieves comparable performance to fully supervised methods (~75% of baseline) on Argoverse 2 and nuScenes datasets, surpassing other 2D-label-only approaches.

Conclusion: MapRF enables scalable and cost-effective online HD map construction for autonomous driving without requiring expensive 3D annotations.
...
TL;DR: Vidi2 is a next-generation video understanding model that advances spatio-temporal grounding (STG) and video question answering capabilities, outperforming leading proprietary systems on new benchmarks VUE-STG and VUE-TR-V2.


<details>
  <summary>Details</summary>
Motivation: Video has become the primary medium for internet communication and creativity, creating strong demand for scalable, high-quality video production tools that can understand and reason about video content comprehensively.

Method: Vidi2 extends video understanding with fine-grained spatio-temporal grounding, enabling identification of both timestamps and bounding boxes of target objects given text queries. The model also supports video question answering for comprehensive multimodal reasoning.

Result: Vidi2 substantially outperforms leading proprietary systems like Gemini 3 Pro and GPT-5 on both VUE-TR-V2 and VUE-STG benchmarks, while achieving competitive results with similar-scale open-source models on video QA benchmarks.

Conclusion: Vidi2 represents a significant advancement in video understanding with its end-to-end spatio-temporal grounding capabilities, enabling practical applications in complex editing scenarios and setting new state-of-the-art performance standards.
...
TL;DR: This thesis develops methods to analyze cartographic heritage at scale, using 771,561 map records and 99,715 digitized images to study maps as cultural objects and semantic systems across six centuries.


<details>
  <summary>Details</summary>
Motivation: Current automated map analysis methods lack engagement with cartographic history and the cultural perspective that maps are semantic-symbolic systems reflecting political and epistemic expectations.

Method: Leveraged a large corpus from 38 digital catalogs, applied normalization, semantic segmentation, object detection models trained on annotated and synthetic data, and analyzed spatial focus, collaboration patterns, and figurative shifts.

Result: Revealed links between Atlantic maritime charting, triangular trade, and colonial expansion; documented progression of national focus and military conflict impacts; identified figurative shifts (e.g., hachures to contours); and showed maps emphasize features through centering and semantic symmetries.

Conclusion: Maps are designed cultural objects with locally consistent sign systems, where legitimacy, major actors, and cities play key roles in spreading figurative norms and semiotic cultures across cartographic history.
...
TL;DR: SpLap is a proxy-free deformation method for Gaussian splats that uses a novel surface-aware splat graph to compute Laplacian operators for more plausible deformations without requiring deformation proxies.


<details>
  <summary>Details</summary>
Motivation: Existing Gaussian splat deformation methods rely on deformation proxies like cages or meshes, which depend on proxy quality and add computational overhead. Direct Laplacian-based approaches treating splats as point clouds fail to capture surface information properly due to lack of explicit structure.

Method: Constructs a surface-aware splat graph where neighboring splats are defined by their intersections rather than just distance between centers. Uses Laplacian operator derived from this graph and introduces Gaussian kernel adaptation to preserve surface structure during deformation.
...
Conclusion: The proposed method enables more plausible deformations that preserve details and topology without requiring deformation proxies, improving rendering quality after deformation.
...
<details>
  <summary>Details</summary>
Motivation: Traditional AI approaches for disaster damage assessment are limited by costly data annotation, fixed answer spaces, and the inability to provide new information without retraining. Generative models with ICL offer flexibility but suffer from hallucinations and generic responses.

Method: Two-stage framework: (1) Generate structured reasoning traces using CoT prompting and ICL for interpretable reasoning, (2) Answer selection module evaluates responses and assigns the most coherent and contextually accurate answer. Integrates custom information retrieval, domain-specific prompting, and reasoning-guided selection.

Result: Experiments on FloodNet and RescueNet-VQA datasets show ThiFAN-VQA achieves superior accuracy, interpretability, and adaptability for real-world post-disaster damage assessment compared to existing methods.
...
TL;DR: HunyuanOCR is a 1B-parameter open-source VLM for OCR that outperforms commercial APIs and larger models, achieving SOTA results in both perception and semantic tasks while unifying versatility and efficiency in a streamlined end-to-end architecture.


<details>
  <summary>Details</summary>
Motivation: To address limitations of narrow OCR expert models and inefficient general VLMs by creating a lightweight model that unifies core OCR capabilities while eliminating dependencies on pre-processing modules to prevent error propagation.

Method: Uses a Native Vision Transformer and lightweight LLM connected via MLP adapter, with data-driven approaches and pioneering use of Reinforcement Learning strategies for OCR tasks in an end-to-end architecture.

Result: Outperforms commercial APIs, traditional pipelines, and larger models; achieves first place in ICDAR 2025 DIMT Challenge (Small Model Track) and SOTA on OCRBench among VLMs with <3B parameters.

Conclusion: HunyuanOCR successfully demonstrates that a lightweight model can achieve superior OCR performance through unified architecture, end-to-end design, and innovative RL strategies, providing a solid foundation for both research and industrial applications.
...
Method: Adversarial framework with GANs using Dice loss, cross-entropy loss, feature matching loss and self-training loss to learn from limited labeled and larger unlabeled NCCT scans.
...
TL;DR: A novel VAE-based method called MSVQ-VAE generates synthetic WCE images with abnormalities to address medical data scarcity, achieving comparable classification performance to real data training.


<details>
  <summary>Details</summary>
Motivation: Address the scarcity of large, diverse medical datasets for WCE image analysis due to privacy constraints and annotation costs, which hinders Clinical Decision Support system development.

Method: Proposes Multiscale Vector Quantized Variational Autoencoder (MSVQ-VAE) that can seamlessly introduce abnormalities into normal WCE images and enable conditional generation of different abnormality types including polyps, vascular and inflammatory conditions.

Result: Training a CDS classifier with synthetic abnormal images generated by MSVQ-VAE yields comparable results to training with only real data, demonstrating the utility of generated images for clinical applications.

Conclusion: The proposed MSVQ-VAE methodology effectively addresses medical data scarcity by generating high-quality synthetic WCE images with abnormalities, showing promise for various medical multimedia domains.
...
TL;DR: SkillSight enables power-efficient skill assessment using gaze data instead of continuous video processing, achieving high accuracy with 73x less power consumption.


<details>
  <summary>Details</summary>
Motivation: Automatic skill assessment from egocentric perception on smart glasses is challenging, and current methods require continuous video processing which consumes significant power.

Method: Two-stage framework: first learns joint modeling of gaze and video for skill prediction, then distills a gaze-only student model that eliminates video processing.

Result: Achieves state-of-the-art performance on three datasets (cooking, music, sports) and maintains high accuracy with 73x less power than competing methods.

Conclusion: Gaze data is valuable for skill understanding across diverse real-world settings, enabling practical AI-supported skill learning applications.
...
Result: Experiments on knee and brain datasets showed semantic priors from images preserve anatomical structures with superior perceptual quality (lower LPIPS, higher Tenengrad scores, better reader study scores) compared to conventional methods. Image-language information further enables high-level control over reconstruction attributes.

Conclusion: Vision-language foundation models can effectively improve undersampled MRI reconstruction through semantic-space optimization, maintaining data fidelity while enhancing perceptual quality and structural preservation.
...
<details>
  <summary>Details</summary>
Motivation: General-purpose large multimodal models have shown poor performance in medical image interpretation, particularly for gigapixel pathology images, likely due to using low-resolution thumbnails or random patches rather than proper navigation.
...
Result: GIANT substantially outperforms conventional patch- and thumbnail-based baselines, approaching or surpassing specialized models. On pathologist-authored questions, GPT-5 with GIANT achieved 62.5% accuracy vs 43.8% for TITAN and 37.5% for SlideChat.

Conclusion: The study reveals strengths and limitations of current foundation models and provides groundwork for developing LMMs for expert reasoning in pathology through agentic navigation of whole-slide images.
...
TL;DR: CodeV is a code-based visual agent trained with Tool-Aware Policy Optimization (TAPO) to address unfaithful visual reasoning in vision-language models, achieving high accuracy while substantially improving faithful tool-use rates.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models often achieve high final-answer accuracy but exhibit unfaithful visual reasoning - using tools on irrelevant regions or ignoring tool outputs while still guessing correct answers, highlighting the need for more trustworthy visual reasoning systems.

Method: Proposes CodeV, a code-based visual agent trained with Tool-Aware Policy Optimization (TAPO), a process-level RL framework that provides dense rewards based on visual tool inputs and outputs rather than chain-of-thought tokens. CodeV represents visual tools as executable Python code.

Result: CodeV achieves competitive or superior accuracy while substantially increasing faithful tool-use rates on visual search benchmarks. It also attains strong performance on multimodal reasoning and math benchmarks.

Conclusion: Explicitly supervising intermediate tool behavior through process-level RL is crucial for building trustworthy, agentic visual reasoning systems that use tools faithfully rather than just achieving high final accuracy.
...
TL;DR: OncoVision is a multimodal AI system that combines mammography images and clinical data to improve breast cancer diagnosis through accurate segmentation and clinical feature prediction, with late-fusion strategies for enhanced precision.


<details>
  <summary>Details</summary>
Motivation: To address the need for better breast cancer diagnosis by leveraging both imaging and clinical data, reducing inter-observer variability, and making screening more accessible in underprivileged areas.

Method: Uses an attention-based encoder-decoder backbone for joint segmentation of four ROIs (masses, calcifications, axillary findings, breast tissues) and predicts ten clinical features. Implements two late-fusion strategies to combine imaging and clinical insights.

Result: Achieves state-of-the-art accuracy in segmentation and robust prediction of clinical features. Produces structured reports with dual-confidence scoring and attention-weighted visualizations for real-time diagnostic support.

Conclusion: OncoVision provides a scalable and equitable solution for early breast cancer detection, improving clinician trust and facilitating medical teaching through its secure web application platform.
...
<details>
  <summary>Details</summary>
Motivation: Existing layer pruning methods lead to significant performance drop when applied to VLMs, creating a need for more effective pruning approaches that preserve model capabilities.

Method: Analyzes triplets of consecutive layers to identify local redundancy, removes the most redundant of the first two layers, finetunes the remaining layer to compensate for lost capacity, and freezes the third layer as a stable anchor during finetuning using an interleaved finetune-freeze design.

Result: Achieves 88.9% average performance retention after dropping 25% of the network, with rapid convergence using minimal data (just 1% of FineVision dataset for one epoch), achieving state-of-the-art performance.

Conclusion: The interleaved finetune-freeze design enables efficient layer pruning in VLMs with minimal performance degradation and data requirements, setting a new standard for VLM compression.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of comprehensive multimodal datasets for industrial tasks, particularly focusing on collaborative work in cognitively and physically intensive industrial environments.

Method: Collected 197 hours of egocentric and 97 hours of exocentric recordings across various industrial tasks, with rich multimodal data including eye gaze, narration, sound, motion, and detailed annotations.

Result: Created a comprehensive dataset with detailed annotations, processed outputs, and benchmarks showing that state-of-the-art multimodal models struggle with the dataset's challenges in mistake detection and reasoning tasks.

Conclusion: IndEgo presents a challenging benchmark for multimodal AI in industrial settings and is publicly available for research on procedural task understanding, mistake detection, and collaborative work analysis.
...
TL;DR: A prototype-based method for interpretable cell counting via density map estimation that learns representative visual patterns for cells and background artifacts, validated by biologists and achieving interpretability without compromising counting performance.


<details>
  <summary>Details</summary>
Motivation: The interpretability of deep learning models in cell counting remains a significant challenge for clinical applications, limiting trust and adoption in critical biomedical settings.

Method: Integration of a prototype layer into the density estimation network to learn representative visual patterns for both cells and background artifacts, with interpretations highlighting regions in input images most similar to each prototype.

Result: Extensive experiments on two public datasets demonstrate the method achieves interpretability without compromising counting effectiveness, with biologists confirming the relevance of learned visual patterns through survey validation.

Conclusion: Provides researchers and clinicians with a transparent and reliable tool for cell counting that can increase trust and accelerate adoption of deep learning in critical biomedical applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing OVSS approaches either have limited generalization due to scarce training data or combine multiple models at high computational cost. RADIO is an overlooked foundation model that can address these limitations.

Method: Enhanced RADIO with self-correlating recursive attention, self-correlating global aggregation, and efficient mask refinement to improve zero-shot OVSS performance.

Result: RADSeg achieves 6-30% mIoU improvement over base ViT, is 3.95x faster, uses 2.5x fewer parameters, and outperforms previous huge model combinations (850-1350M) with only 105M parameters.

Conclusion: RADSeg demonstrates that leveraging RADIO foundation model enables state-of-the-art OVSS performance with substantially lower computational and memory costs compared to previous approaches.
...
TL;DR: Branch-based structural reparameterization reduces Vision Transformer layers while maintaining accuracy, achieving up to 37% inference speedup by consolidating parallel branches during training.


<details>
  <summary>Details</summary>
Motivation: Existing ViT acceleration focuses on algorithmic optimizations, leaving layer depth reduction underexplored. The paper questions whether we can reduce stacked transformer layers while maintaining representational capacity.

Method: Proposes branch-based structural reparameterization that uses parallel branches during training, then systematically consolidates them into single-path models for inference by merging branches at nonlinear component entry points.

Result: Successfully reduces ViT-Tiny from 12 layers to 6, 4, or 3 layers while maintaining ImageNet-1K accuracy. Achieves up to 37% inference speedup on mobile CPU platforms.

Conclusion: Conventional wisdom favoring extremely deep transformer stacks may be unnecessarily restrictive, pointing toward new opportunities for efficient vision transformer construction.
...
TL;DR: An altitude-aware dynamic tiling method for UAV-based search and rescue that improves small object detection by 38% mAP and doubles inference speed compared to static tiling.


<details>
  <summary>Details</summary>
Motivation: Small objects are difficult to detect from high UAV altitudes due to low object-to-background pixel ratios in maritime search and rescue missions.

Method: Proposes altitude-aware dynamic tiling that scales and adaptively subdivides images into tiles, integrating altitude-dependent scaling with adaptive tiling factors to reduce computation while maintaining detection performance.

Result: Tested on SeaDronesSee dataset with YOLOv5 and SAHI framework, achieved 38% improvement in mAP for small objects compared to baseline and more than double the inference speed compared to static tiling.

Conclusion: The approach enables more efficient and accurate UAV-based search and rescue operations under diverse conditions by optimizing detection performance and computational efficiency.
...
TL;DR: The paper studies the transferability of optimized slicers in min-Sliced Transport Plan (min-STP) framework, showing that slicers trained on one distribution pair can effectively transfer to new pairs under distributional shift, with theoretical guarantees and empirical validation.


<details>
  <summary>Details</summary>
Motivation: To address the computational cost limitations of Optimal Transport (OT) while enabling transferability of learned slicers across related distribution pairs, which is crucial for evolving data and repeated OT computations.

Method: Introduces min-Sliced Transport Plan (min-STP) framework with a minibatch formulation for scalability, theoretically analyzes slicer transferability under distribution perturbations, and provides statistical guarantees.

Result: Theoretically shows optimized slicers remain close under slight data perturbations; empirically demonstrates strong one-shot matching performance and facilitates amortized training for point cloud alignment and flow-based generative modeling.

Conclusion: Transferable min-STP enables efficient OT computations across related tasks by leveraging transferable slicers, with both theoretical foundations and practical effectiveness demonstrated.
...
TL;DR: PathFMTools is a lightweight Python package for efficient execution, analysis, and visualization of pathology foundation models, tested on CONCH and MUSK models for cSCC histological grading.


<details>
  <summary>Details</summary>
Motivation: Adapting computational pathology foundation models to clinical tasks is challenging due to WSI complexity, feature opacity, and diverse adaptation strategies.

Method: Developed PathFMTools package to interface with CONCH and MUSK models, benchmarked multiple adaptation strategies on 440 cSCC H&E WSIs for histological grading.

Result: Demonstrated trade-offs across prediction approaches and validated using foundation model embeddings to train specialist models.

Conclusion: Pathology foundation models show promise for clinical applications, with PathFMTools enabling efficient analysis and validation.
...
TL;DR: Extends prototype networks to multimodal settings for species detection, enabling interpretable decisions and cost-aware allocation of expensive genetic data while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Address two major drawbacks in multimodal neural networks for species detection: lack of interpretability in decision-making and high cost/invasiveness of genetic data collection.

Method: Extends prototype networks (ProtoPNets) to multimodal setting by ensembling prototypes from each modality with associated weights, and introduces methods to identify cases where expensive genetic data is not needed for confident predictions.

Result: Approach can intelligently allocate expensive genetic data for fine-grained distinctions while using abundant image data for clearer visual classifications, achieving comparable accuracy to models that consistently use both modalities.

Conclusion: The proposed multimodal prototype network framework successfully addresses interpretability and cost concerns in species detection while maintaining performance comparable to traditional multimodal approaches.
...
TL;DR: VESSA integrates vision-language models into semi-supervised medical image segmentation, using template-guided learning and dynamic student interaction to improve accuracy with limited annotations.
...
Method: Two-stage approach: Stage 1 trains VESSA as reference-guided segmentation assistant using template bank; Stage 2 integrates VESSA into SSL framework for dynamic interaction with student model, using refined predictions as prompts for better pseudo-labels.
...
Conclusion: VESSA successfully incorporates foundation-level visual-semantic understanding into SSL frameworks, demonstrating the value of vision-language models for improving semi-supervised medical image segmentation.
...
TL;DR: Proposes a new 'relative angle' feature for point cloud damage reconstruction that reduces data volume while maintaining performance comparable to normal vectors.


<details>
  <summary>Details</summary>
Motivation: Point cloud reconstruction is effective for damage assessment but constrained by high 3D data volume; existing methods are vulnerable to background noise.

Method: Introduces relative angle feature - angle between point's normal vector and parent point cloud's average normal vector. Uses entropy-based feature evaluation and trains models with PointNet++.

Result: Models using relative angles achieved similar performance to normal vector models while providing 27.6% storage reduction and 83% input channel compression.

Conclusion: Relative angle feature enables larger-batch execution on resource-constrained hardware without requiring architectural modifications to models.
...
TL;DR: CrispFormer enhances SegFormer decoder with boundary supervision, uncertainty-guided refinement, and dynamic multi-scale fusion to improve weakly supervised semantic segmentation without altering the backbone.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of learning dense masks from noisy, under-specified cues in weakly supervised semantic segmentation by improving boundary preservation, scale selection, and noise resistance.

Method: Three synergistic decoder modifications: (1) boundary branch with edge head and boundary-aware loss, (2) uncertainty-guided refiner using aleatoric uncertainty for loss weighting and residual correction, (3) dynamic multi-scale fusion with spatial softmax gating.

Result: Consistent improvements in boundary F-score, small-object recall, and mIoU over SegFormer baselines while adding minimal computational overhead.

Conclusion: CrispFormer provides a simple, broadly compatible decoder-centric approach that offers reproducible path to higher-fidelity masks from image-level supervision in WSSS.
...
TL;DR: Prune-Then-Plan framework stabilizes VLM-based embodied question answering by pruning implausible frontier choices and using coverage-based planning to prevent frontier oscillations and improve navigation efficiency.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models exhibit frontier oscillations (unstable back-and-forth movements) when used directly for step-level exploration in embodied question answering, leading to inefficient navigation and degraded answer quality due to overconfidence and miscalibration.

Method: Proposes Prune-Then-Plan framework that uses Holm-Bonferroni inspired pruning to remove implausible frontier choices, then delegates final decisions to a coverage-based planner, converting overconfident VLM predictions into conservative, interpretable actions.
...
Conclusion: The separation of pruning and planning effectively calibrates step-level VLM behavior, stabilizing exploration and improving embodied question answering performance through conservative, interpretable actions.
...
TL;DR: Identifies structural failure in linear RoPE interpolation for mixed-resolution diffusion transformers, proposes CRPA as training-free fix to align phases across resolutions.


<details>
  <summary>Details</summary>
Motivation: Linear interpolation of rotary positional embeddings causes attention collapse when mixing tokens from different spatial grids due to phase aliasing from incompatible sampling rates.

Method: Cross-Resolution Phase-Aligned Attention (CRPA) - modifies RoPE index map to express all Q/K positions on query's stride, ensuring equal physical distances produce identical phase increments.

Result: CRPA stabilizes all attention heads and layers, enables high-fidelity mixed-resolution generation, outperforms previous state-of-the-art methods on image and video generation.

Conclusion: CRPA provides training-free solution to structural RoPE phase alignment problem, making pretrained DiTs robust for mixed-resolution tasks without retraining.
...
TL;DR: VLMs struggle with abstention in safety-critical STVQA tasks due to unreliable output probabilities. The paper proposes Latent Representation Probing (LRP) to detect uncertainty from internal representations instead, achieving 7.6% improvement in abstention accuracy.


<details>
  <summary>Details</summary>
Motivation: VLMs deployed in safety-critical applications need reliable abstention capabilities, especially when OCR errors could cause severe consequences like traffic accidents. Existing methods fail due to miscalibrated probabilities or semantic agreement requirements unsuitable for OCR tasks.

Method: Proposed Latent Representation Probing (LRP): training lightweight probes on hidden states or attention patterns. Three probe designs: concatenating representations across all layers, aggregating attention over visual tokens, and ensembling single layer probes by majority vote.

Result: Experiments on four benchmarks across image and video modalities show LRP improves abstention accuracy by 7.6% over best baselines. Probes generalize across uncertainty sources and datasets, with optimal signals emerging from intermediate rather than final layers.

Conclusion: LRP establishes a principled framework for deployment-ready AI systems by detecting confidence signals from internal states rather than unreliable outputs, enabling more reliable abstention in safety-critical applications.
...
TL;DR: TPSO is a training-free module that enhances text-to-image diversity by optimizing token embedding space to explore underrepresented regions while maintaining image quality through global semantic constraints.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models often generate repetitive outputs due to collapsing toward strong modes in the learned distribution, hindering creative exploration and downstream applications.

Method: Token-Prompt embedding Space Optimization (TPSO) introduces learnable parameters to explore underrepresented regions of token embedding space, with prompt-level constraints preventing quality degradation.

Result: TPSO significantly improves generative diversity from 1.10 to 4.18 points on MS-COCO across three diffusion backbones without sacrificing image quality.

Conclusion: TPSO effectively addresses diversity collapse in text-to-image generation while maintaining high fidelity, offering a model-agnostic solution that doesn't require retraining.
...
TL;DR: CropVLM is a reinforcement learning-based method that enables VLMs to dynamically zoom in on relevant image regions to improve fine-grained image understanding without requiring human-labeled bounding boxes or modifying the target VLM.
...
Method: CropVLM uses reinforcement learning to train a model that can dynamically crop and zoom into relevant image regions, without human-labeled bounding boxes or synthetic evaluations. It's trained once and can be paired with various VLMs.

Result: Significant improvements on tasks requiring high-resolution image understanding, especially for out-of-domain benchmarks, without catastrophic forgetting since the VLM itself isn't modified.

Conclusion: CropVLM provides an effective external solution for enhancing VLM performance on fine-grained visual tasks through dynamic region zooming, while maintaining model compatibility and avoiding catastrophic forgetting.
...
TL;DR: ReDirector is a camera-controlled video retake generation method that fixes RoPE misuse and introduces Rotary Camera Encoding (RoCE) for better multi-view integration, enabling generalization to diverse camera trajectories and video lengths.


<details>
  <summary>Details</summary>
Motivation: To address the common misuse of RoPE in previous video retake generation methods and improve camera controllability, geometric consistency, and video quality across different trajectories and lengths.

Method: Rectifies RoPE misuse by aligning spatiotemporal positions, and introduces Rotary Camera Encoding (RoCE) - a camera-conditioned RoPE phase shift that captures multi-view relationships within and across input and target videos.

Result: Significant improvements in camera controllability, geometric consistency, and video quality across various trajectories and lengths, with better dynamic object localization and static background preservation.

Conclusion: ReDirector effectively generalizes to out-of-distribution camera trajectories and video lengths by integrating camera conditions into RoPE, demonstrating superior performance in video retake generation.
...
TL;DR: BHD-RAG is a multimodal retrieval-augmented generation framework that integrates domain-specific knowledge and clinical precedents with MLLMs to improve diagnostic accuracy for Birt-Hogg-Dube syndrome from CT images, addressing challenges of limited samples and low inter-class differentiation in Diffuse Cystic Lung Diseases.


<details>
  <summary>Details</summary>
Motivation: Deep learning methods face challenges with limited clinical samples and low inter-class differentiation among DCLDs for BHD diagnosis via CT imaging. MLLMs show diagnostic potential but suffer from domain knowledge gaps and hallucination risks without referable radiological features.

Method: BHD-RAG framework with three components: (1) specialized agent generating CT imaging manifestation descriptions to build multimodal DCLDs corpus, (2) cosine similarity-based retriever finding relevant image-description pairs for query images, and (3) MLLM synthesizing retrieved evidence with imaging data for diagnosis.

Result: Validated on dataset with four types of DCLDs, achieving superior accuracy and generating evidence-based descriptions closely aligned with expert insights.

Conclusion: BHD-RAG effectively addresses the diagnostic challenges for rare diseases like BHD by integrating domain-specific knowledge and clinical precedents with multimodal AI, reducing hallucination risks and improving diagnostic reliability.
...
TL;DR: Rectified SpaAttn addresses attention sparsity biases in diffusion transformers by rectifying attention allocation with implicit full attention reference, achieving significant speedups while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing attention sparsity methods suffer from severe performance degradation due to systematic biases: excessive focus on critical tokens amplifies their weights, and complete neglect of non-critical tokens causes loss of relevant attention weights.

Method: Proposes Rectified SpaAttn with two key components: (1) Isolated-Pooling Attention Reallocation for critical tokens to calculate accurate rectification factors, and (2) Gain-Aware Pooling Rectification for non-critical tokens to ensure rectified gain surpasses induced error.

Result: Achieves up to 3.33x and 2.08x speedups on HunyuanVideo and Wan 2.1 respectively while maintaining high generation quality. Custom Triton kernel implementation.

Conclusion: Rectified SpaAttn effectively addresses attention sparsity biases, enabling efficient video generation with preserved quality, and is released as open-source.
...
TL;DR: 4DWorldBench is a comprehensive benchmark for evaluating World Generation Models across four key dimensions: perceptual quality, condition-4D alignment, physical realism, and 4D consistency, using adaptive multimodal conditioning and unified evaluation methods.
...
Method: Introduces 4DWorldBench with adaptive conditioning across multiple modalities, mapping all modality conditions into unified textual space, and integrating LLM-as-judge, MLLM-as-judge, and traditional network-based evaluation methods.
...
Conclusion: The benchmark serves as a foundation for objective comparisons and improvements, accelerating the transition from visual generation to world generation by systematically evaluating World Models across multiple dimensions.
...
TL;DR: The paper introduces IMIC, a gradient-coupled interleaving training method that enables vision foundation models to perform four tasks (object recognition, face recognition from high/low-quality images, and person recognition) in a single embedding space without catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Vision foundation models suffer from catastrophic forgetting when fine-tuned for specific tasks, limiting their ability to handle multiple tasks simultaneously in a unified embedding space.
...
Result: EVA-02 and CLIP with IMIC performed comparably with domain experts on all four tasks concurrently, were more accurate than humans at multi-tasking, and maintained out-of-distribution generalization. Representations were linearly separable but with substantial feature sharing across tasks.

Conclusion: IMIC enables foundation models to handle multiple vision tasks in a unified embedding space without catastrophic forgetting, while maintaining generalization capabilities and achieving human-level or better performance across diverse domains.
...
TL;DR: DOGE introduces a Bézier Graph representation for road network extraction, learning directly from segmentation masks without vector ground-truth through differentiable optimization.


<details>
  <summary>Details</summary>
Motivation: Traditional polyline-based methods struggle with curvilinear road geometry, and obtaining vector ground-truth for curve-based representations is difficult.

Method: DOGE uses Bézier Graph representation with two alternating modules: DiffAlign for differentiable geometry optimization and TopoAdapt for discrete topology refinement, learning directly from segmentation masks.
...
Conclusion: Presents a new paradigm for road network extraction that eliminates the need for curve ground-truth while achieving superior performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods based on 3D Gaussian Splatting have limitations including rigid motion from Linear Blend Skinning, limited expressiveness, and inability to handle frequently occluded regions like mouth interiors and eyelids.

Method: Proposes two key components: (1) UV-Adaptive Soft Binding framework that uses image-based and geometric priors to learn per-Gaussian feature offsets in UV space, supporting dynamic resampling and compatibility with Adaptive Density Control; (2) Temporal ADC strategy that clusters similar frames and uses fused perceptual error as clone criterion to capture geometric and textural discrepancies.
...
Conclusion: STAvatar effectively addresses the limitations of existing methods and achieves superior performance in 3D head avatar reconstruction from monocular videos.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail to establish semantic-level alignment between time series and images, and the potential of using continuous sequential data for high-fidelity image generation remains unexplored.

Method: Uses a 'warmup-align' paradigm: self-supervised training of dual-autoencoder and shared quantizer on large datasets, then freezing encoders and introducing projection to align temporal and visual representations.
...
TL;DR: GigaWorld-0 is a unified world model framework that generates synthetic embodied interaction data for training Vision-Language-Action models, enabling strong real-world robot performance without real-world training.


<details>
  <summary>Details</summary>
Motivation: World models are emerging as a foundational paradigm for scalable, data-efficient embodied AI, addressing the need for diverse, high-quality training data without real-world interaction.

Method: Combines GigaWorld-0-Video (large-scale video generation) and GigaWorld-0-3D (3D generative modeling with Gaussian Splatting, system identification, and motion planning) with joint optimization, plus efficient GigaTrain framework using FP8-precision and sparse attention.

Result: Generates high-quality, diverse, and controllable embodied interaction data that is visually compelling, spatially coherent, physically plausible, and instruction-aligned.

Conclusion: VLA models trained on GigaWorld-0-generated data achieve strong real-world performance on physical robots, significantly improving generalization and task success without any real-world interaction during training.
...
TL;DR: MAPS is a fine-tuning framework for Vision-Language-Action models that schedules proximity constraints to balance stability and flexibility, improving generalization without extra parameters.


<details>
  <summary>Details</summary>
Motivation: Naive fine-tuning of VLAs disrupts pretrained VLM representations and harms generalization, while existing methods either overconstrain adaptation or ignore component roles.

Method: MAPS linearly schedules proximity constraint relaxation, keeping visual encoders close to pretrained priors while allowing action-oriented language layers to adapt more freely.

Result: MAPS consistently boosts in-distribution and out-of-distribution performance by up to 30% across multiple benchmarks and real-world evaluations.

Conclusion: Empirically guided proximity to pretrained VLMs is a simple yet powerful principle for preserving broad generalization in VLM-to-VLA transfer.
...
TL;DR: ChessMamba is a structure-aware framework for change detection in multitemporal remote sensing imagery that addresses heterogeneity and spatiotemporal misalignment through interleaved state-space modeling with chessboard serialization and structure-aware fusion.


<details>
  <summary>Details</summary>
Motivation: Existing methods based on vision transformers or state-space models disrupt local structural consistency during temporal serialization, obscuring discriminative cues under misalignment and hindering reliable change localization in multitemporal remote sensing.

Method: Integrates SpatialMamba encoder with cross-source interaction module using: (i) Chessboard interleaving with snake scanning order to serialize multi-temporal features into unified sequence, and (ii) Structure-aware fusion via multi-dilated convolutions to capture center-and-corner neighborhood contexts.
...
Conclusion: ChessMamba provides a robust solution for change detection in multitemporal remote sensing by preserving structural consistency and enabling direct comparison for accurate change localization through its innovative interleaved state-space modeling approach.
...
<details>
  <summary>Details</summary>
Motivation: Knowledge distillation effectiveness decreases in cross-modal scenarios due to representation inconsistencies between modalities like vision and language.

Method: Uses frequency-domain analysis to decouple features - applies strong alignment for consistent low-frequency features and relaxed alignment for dissimilar high-frequency features, plus scale consistency loss and shared classifier.
...
Conclusion: Frequency-domain feature decoupling effectively addresses cross-modal knowledge transfer challenges and improves distillation performance.
...
TL;DR: A multi-task liver dataset (LiMT) was created for liver segmentation, tumor segmentation, lesion classification, and detection using arterial phase CT scans to address limitations of single-task datasets in CAD development.


<details>
  <summary>Details</summary>
Motivation: Existing CAD datasets are limited to single tasks, constraining the development of computer-aided diagnosis technology for liver lesions. There's a need for datasets that can explore task correlations without dataset heterogeneity issues.

Method: Constructed LiMT dataset with 150 CT volumes from arterial phase-enhanced CT, including four liver disease types and normal cases. All volumes were annotated and calibrated by experienced clinicians to support multiple tasks simultaneously.

Result: Created a public multi-task liver dataset that enables joint training for segmentation, classification, and detection tasks. Provided baseline experimental results and reviewed existing liver-related datasets and methods.

Conclusion: The LiMT dataset serves as a valuable resource for medical imaging research, allowing exploration of task correlations and overcoming dataset heterogeneity limitations in liver CAD development.
...
TL;DR: Proposes a Generate-then-Verify framework to create VeriSciQA, a high-quality SVQA dataset that addresses systematic errors in existing synthetic data through cross-modal consistency checks, achieving better performance than existing datasets.


<details>
  <summary>Details</summary>
Motivation: Open-source LVLMs struggle with Scientific Visual Question Answering due to lack of large-scale, high-quality datasets, and existing synthetic data methods have systematic errors from LVLM limitations and information asymmetry.

Method: Generate-then-Verify framework: first generates QA pairs with figure-associated text context, then applies cross-modal consistency checks against figures and auxiliary filters to eliminate erroneous pairs.

Result: Created VeriSciQA with 20,351 QA pairs across 20 scientific domains and 12 figure types. Open-source models achieve 64% accuracy vs proprietary model's 82%. Fine-tuned models show consistent improvements on SVQA benchmarks with scaling benefits.

Conclusion: The scalable framework enables continued data expansion to advance SVQA capability in open-source community, with human evaluation confirming VeriSciQA's superior correctness over existing datasets.
...
<details>
  <summary>Details</summary>
Motivation: Overcome limitations of human-annotated supervision in vision-language agents and address issues with text-based self-evaluation that struggles with complex visual reasoning and suffers from hallucinations.

Method: Integrates tool usage into reasoning, self-evaluation, and self-repair through two synergistic roles: Solver (multi-turn tool-integrated reasoning) and Verifier (structured feedback and fine-grained self-rewards via tool-grounded critique), connected through a Self-Evolving Reasoning Cycle.
...
Conclusion: Agent0-VL enables stable self-improvement by aligning reasoning and verification behaviors without human annotation or external reward models, demonstrating effective continual self-evolution in vision-language tasks.
...
TL;DR: A multimodal approach for continuous ASL sign recognition using 3D skeletal features and handshape classification to detect sign boundaries, followed by recognition of segmented signs.


<details>
  <summary>Details</summary>
Motivation: To improve continuous sign language recognition by accurately detecting sign boundaries in ASL sentences, which is challenging due to the fluid nature of signing and differences between isolated and continuous signs.

Method: Uses 3D skeletal features to capture sign dynamics at boundaries, pretrains a handshape classifier for 87 canonical categories, and employs multimodal fusion to combine video segmentation with handshape information for boundary detection.

Result: Significant improvements over previous work when evaluated on the ASLLRP corpus.

Conclusion: The multimodal approach combining 3D skeletal features and handshape classification effectively improves continuous sign recognition by better detecting sign boundaries.
...
TL;DR: Motion Marionette is a zero-shot framework for rigid motion transfer from source videos to target images using spatial-temporal priors instead of external constraints, achieving temporally consistent results across diverse objects.


<details>
  <summary>Details</summary>
Motivation: Previous methods use external geometric, generative, or simulation priors that create trade-offs between generalizability and temporal consistency. The authors aim to overcome these limitations by using internal spatial-temporal transformations shared between source and target.

Method: Lift source video and target image into unified 3D representation space, extract motion trajectories to create spatial-temporal prior, integrate with target object to synthesize controllable velocity field, refine with Position-Based Dynamics to reduce artifacts.
...
Conclusion: Motion Marionette successfully transfers rigid motion using internal spatial-temporal priors, avoiding external constraints and achieving better generalizability and temporal consistency than previous approaches.
...
TL;DR: Reasoning-VLA is a fast action-generation VLA framework that uses learnable action queries and reasoning-enhanced features to generate continuous action trajectories in parallel, achieving state-of-the-art performance in autonomous driving.
...
Method: Uses learnable action queries initialized via Gaussian sampling from ground-truth trajectories, interacts with reasoning-enhanced vision-language features, consolidates 8 autonomous driving datasets into standardized Chain-of-Thought reasoning format, and employs both supervised learning and reinforcement learning fine-tuning.
...
Conclusion: Reasoning-VLA provides a general and fast framework for autonomous driving decision-making with strong generalization capabilities.
...
TL;DR: A framework for predicting photochemical conversion in 3D printed objects using visual data, introducing a novel computer vision task of predicting volumetric physical properties from 3D visual data.


<details>
  <summary>Details</summary>
Motivation: To enable virtual chemical characterization of 3D printed objects without traditional post-print measurements, addressing the challenge that conventional vision models lack inductive bias for coupled optical and material physics interactions.

Method: Proposed Coupled Physics-Gated Adaptation (C-PGA), a multimodal fusion architecture that uses sparse geometrical and process parameters as Query to dynamically gate and adapt dense visual features via feature-wise linear modulation (FiLM), processing dual 3D visual streams with parallel 3D-CNNs.

Result: The approach enables precise prediction and control over chemical conversion state in 3D printed objects, using the largest-ever optically printed 3D specimen dataset with parametrically designed complex minimal surface structures.

Conclusion: C-PGA offers a breakthrough in virtual chemical characterization by explicitly modeling physical coupling between optical and material physics, eliminating the need for traditional post-print measurements.
...
<details>
  <summary>Details</summary>
Motivation: Existing test-time scaling methods operate at full-image level, wasting computation on satisfactory regions and insufficiently correcting localized defects, ignoring spatial heterogeneity in image quality.

Method: Uses cross- and self-attention contrast under quality-aware prompts to identify defective regions, then refines them into masks and performs local denoising only on defective areas while preserving the rest of the image.

Result: Achieves state-of-the-art performance on SD2.1, SDXL, and FLUX, consistently improving both local quality and global fidelity while reducing GPU cost by 2-4x compared to Best-of-N sampling.

Conclusion: Localized test-time scaling is established as a promising new direction for scaling diffusion models at inference time, offering significant efficiency gains without compromising quality.
...
TL;DR: HybriDLA is a novel generative framework for document layout analysis that combines diffusion and autoregressive decoding to handle modern documents with diverse element counts and complex layouts.


<details>
  <summary>Details</summary>
Motivation: Traditional DLA methods struggle with contemporary documents that have varied element counts and increasingly complex layouts, requiring a more flexible approach than fixed query sets or single-pass processing.

Method: Unifies diffusion and autoregressive decoding in a single layer, with diffusion refining bounding-box hypotheses and autoregressive components adding semantic/contextual awareness. Uses a multi-scale feature-fusion encoder to capture both fine-grained and high-level visual cues.

Result: Achieves 83.5% mean Average Precision (mAP) and sets state-of-the-art performance on DocLayNet and M$^6$Doc benchmarks, outperforming previous approaches.

Conclusion: HybriDLA effectively addresses the limitations of conventional DLA methods for modern documents through its hybrid generative framework and multi-scale feature encoding.
...
TL;DR: DetVLM is a novel image retrieval framework that combines object detection with Visual Large Models to enable state-specific and zero-shot fine-grained image retrieval, achieving 94.82% overall accuracy.


<details>
  <summary>Details</summary>
Motivation: Conventional methods for fine-grained image retrieval have limitations: manual features lack robustness, deep learning detectors can't perform state-specific retrieval or zero-shot search, and VLMs have poor spatial grounding and high computational costs.

Method: A two-stage pipeline: YOLO detector first performs efficient component-level screening, then a VLM acts as recall-enhancement unit for secondary verification. This enables state search (verifying component existence and state judgments) and zero-shot search (recognizing unseen components without training).

Result: Achieved 94.82% overall retrieval accuracy on vehicle component dataset, significantly outperforming detection-only baselines. Also attained 94.95% accuracy in zero-shot search for driver mask-wearing and over 90% average accuracy in state search tasks.

Conclusion: DetVLM successfully bridges the gaps in fine-grained image retrieval by synergistically fusing object detection with VLMs, enabling advanced capabilities like state-specific retrieval and zero-shot search with high accuracy and efficiency.
...
TL;DR: CounterVQA benchmark evaluates VLMs' counterfactual reasoning in videos, revealing performance gaps on complex causal chains. CFGPT method improves reasoning by distilling language modality capabilities.


<details>
  <summary>Details</summary>
Motivation: Counterfactual reasoning capability in VLMs remains underexplored despite recent advances in video understanding, and this capability is essential for robust video understanding that goes beyond pattern recognition.

Method: Introduced CounterVQA benchmark with three progressive difficulty levels, and developed CFGPT post-training method that distills counterfactual reasoning capability from language modality to enhance visual reasoning.

Result: Evaluation revealed substantial performance gap: models achieve reasonable accuracy on simple counterfactual questions but degrade significantly on complex multi-hop causal chains. CFGPT yielded consistent improvements across all difficulty levels.

Conclusion: Current VLMs have limitations in complex counterfactual reasoning, but CFGPT method effectively enhances this capability by leveraging language modality distillation.
...
TL;DR: CPDATrack is a Transformer-based tracking framework that suppresses background and distractor interference through token pruning and selective attention mechanisms while maintaining target context.


<details>
  <summary>Details</summary>
Motivation: Existing one-stream Transformer trackers suffer from excessive background token attention that weakens discriminative capability, and current token pruning methods often remove important contextual information near targets.

Method: Uses a learnable module to estimate target probability for search tokens, prunes less-informative background tokens while preserving target context, employs discriminative selective attention to block search-to-template attention in early layers, and selectively extracts high-probability target tokens from localized regions in later layers.

Result: Achieves state-of-the-art performance across multiple benchmarks, with 75.1% average overlap on GOT-10k.

Conclusion: CPDATrack effectively suppresses background and distractor interference while maintaining computational efficiency, demonstrating superior tracking performance through targeted token pruning and selective attention mechanisms.
...
TL;DR: DRIFT: A zero-shot object tracking framework using diffusion model self-attention maps as semantic propagation kernels, achieving SOTA performance on video object segmentation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Image diffusion models capture rich semantic structures that can be leveraged for recognition and localization tasks beyond image generation, particularly for object tracking in videos.

Method: Reinterpret diffusion model self-attention maps as semantic label propagation kernels, extend to temporal propagation for video tracking, use test-time optimization (DDIM inversion, textual inversion, adaptive head weighting), and integrate SAM-guided mask refinement.

Result: Achieves state-of-the-art zero-shot performance on standard video object segmentation benchmarks.

Conclusion: Diffusion models' self-attention mechanisms provide powerful semantic correspondence capabilities that enable effective zero-shot object tracking when properly adapted and refined.
...
Method: Patch-wise optimization on high-resolution source images, followed by a fine-grained detail transfer module and a novel synchronization strategy to maintain consistency across patches.

Result: Extensive experiments show the method produces high-quality edits, facilitating high-resolution content creation.

Conclusion: The proposed framework enables effective high-resolution image editing, advancing the field toward high-resolution content creation.
...
TL;DR: SPROUT is a training-free framework for nuclear instance segmentation that uses histology-informed prototypes and partial optimal transport to generate prompts for SAM, achieving competitive performance without supervision.


<details>
  <summary>Details</summary>
Motivation: Existing nuclear segmentation methods rely on dense supervision and expensive fine-tuning, creating a need for training-free approaches that can scale effectively in computational pathology.

Method: Uses histology-informed priors to create slide-specific reference prototypes, applies partial optimal transport for feature alignment, and generates positive/negative point prompts for SAM without parameter updates.

Result: Achieves competitive performance across multiple histopathology benchmarks without any supervision or retraining, demonstrating effective nuclear instance segmentation.

Conclusion: SPROUT establishes a novel paradigm for scalable, training-free nuclear instance segmentation in pathology, eliminating the need for annotations and computational fine-tuning.
...
TL;DR: GFT-GCN is a privacy-preserving 3D face recognition framework that combines spectral graph learning with diffusion-based template protection for secure biometric authentication.


<details>
  <summary>Details</summary>
Motivation: 3D face recognition provides robust biometric authentication but requires protection of stored templates against security threats and privacy breaches.

Method: Integrates Graph Fourier Transform (GCN) and Graph Convolutional Networks (GCN) to extract spectral features from 3D face meshes, then applies spectral diffusion for template protection using a client-server architecture.

Result: Achieves high recognition accuracy on BU-3DFE and FaceScape datasets with strong resistance to reconstruction attacks.

Conclusion: GFT-GCN effectively balances privacy and performance, offering a practical solution for secure 3D face authentication systems.
...
TL;DR: MambaEye is a causal sequential visual encoder using Mamba2 backbone with unidirectional processing and relative move embedding, enabling input-size agnostic performance with linear complexity.


<details>
  <summary>Details</summary>
Motivation: To create a truly input-size agnostic visual encoder that mimics human vision's fundamental characteristic of processing visual information regardless of input size.

Method: Uses causal sequential encoder with Mamba2 backbone, strictly unidirectional processing, relative move embedding for spatial shifts, and diffusion-inspired loss function for step-wise supervision.

Result: Demonstrates robust performance across wide range of image resolutions, especially at high resolutions like 1536² on ImageNet-1K classification, while maintaining linear time and memory complexity.

Conclusion: MambaEye successfully achieves input-size agnostic visual encoding with strong performance across resolutions, offering a promising approach for scalable vision models.
...
TL;DR: HiCoGen is a hierarchical compositional generative framework that uses Chain of Synthesis and reinforcement learning to improve text-to-image generation for complex prompts with multiple objects and hierarchical structures.
...
Method: Proposes HiCoGen framework with Chain of Synthesis paradigm: uses LLM to decompose prompts into semantic units, iteratively synthesizes them with visual context, and employs RL with Decaying Stochasticity Schedule and hierarchical rewards.

Result: Significantly outperforms existing methods in both concept coverage and compositional accuracy on the new HiCoPrompt benchmark.

Conclusion: The hierarchical compositional approach with enhanced exploration through decaying stochasticity and multi-level rewards effectively addresses complex prompt generation challenges.
...
TL;DR: VGGT4D extends the 3D foundation model VGGT for robust 4D scene reconstruction without training, using mined dynamic cues from global attention layers to separate static and dynamic elements.


<details>
  <summary>Details</summary>
Motivation: Existing 4D approaches rely on external priors, heavy post-optimization, or fine-tuning on 4D datasets, while VGGT's performance drops when moving objects dominate.

Method: Mines and amplifies VGGT's global attention layers' dynamic cues via gram similarity, aggregates them temporally, refines masks with projection gradient, and integrates masks into early-stage inference.

Result: Achieves superior performance across six datasets in dynamic object segmentation, camera pose estimation, and dense reconstruction, supporting single-pass inference on 500+ frame sequences.

Conclusion: VGGT4D provides a training-free framework for robust 4D scene reconstruction by effectively leveraging VGGT's implicit dynamic cues to mitigate motion interference.
...
TL;DR: RLVR post-training shifts low-entropy activations in LMMs, which is linked to reasoning capability. Activation Replay boosts multimodal reasoning by replaying low-entropy activations from base models to RLVR models without additional training.


<details>
  <summary>Details</summary>
Motivation: To understand the underlying mechanisms of RLVR in LMMs and leverage the observation that RLVR shifts low-entropy activations to improve multimodal reasoning capabilities.
...
Result: Activation Replay improves reasoning across mathematics, visual agents, and video reasoning tasks, boosts Pass@K scores, and mitigates narrower reasoning coverage of RLVR. Outperforms alternative approaches like replaying high-entropy activations.

Conclusion: Modulating low-entropy activations plays a beneficial role in LMM reasoning, and Activation Replay provides an effective training-free method to enhance multimodal reasoning in post-trained LMMs.
...
TL;DR: EmoFeedback2 introduces a generation-understanding-feedback reinforcement paradigm for continuous emotional image generation, using fine-tuned LVLM to provide reward and textual feedback for better emotional control and fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing C-EICG approaches lack emotional feedback from generated images and fail to adaptively adjust emotional prompts, leading to limited emotional continuity and insufficient emotional fidelity.

Method: Proposes EmoFeedback2 with emotion-aware reward feedback (LVLM evaluates emotional values and computes rewards) and self-promotion textual feedback (LVLM analyzes images and produces refinement suggestions for next-round prompts).
...
Conclusion: The proposed reinforcement paradigm successfully enhances emotional continuity and fidelity in continuous emotional image generation through LVLM-based feedback mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Existing guidance-based methods for using generic text-to-image models for inpainting are limited in effectiveness, requiring specialized inpainting models. The authors identify that optimizing the initial seed noise is the missing component for effective training-free inpainting.

Method: Proposes optimizing initial seed noise to match unmasked image regions using few optimization steps. Uses linear approximation to avoid costly unrolling and optimizes in spectral domain for stability. Combines with conventional training-free inpainting methods.

Result: Method outperforms state-of-the-art approaches on various inpainting tasks, demonstrating effectiveness across different scenarios.

Conclusion: The proposed training-free inpainting approach successfully enables off-the-shelf text-to-image models to perform high-quality inpainting by optimizing initial seed noise, eliminating the need for specialized inpainting models.
...
TL;DR: A multimodal VR gaze prediction method that combines gaze history, head movement, and visual scene data using gated fusion with cross-modal attention, achieving 93.1% accuracy across 22 VR scenes.
...
Method: Uses gated fusion mechanism with cross-modal attention to adaptively weight gaze history, head movement, and scene content based on contextual relevance across multiple modalities.

Result: Achieved 93.1% validation accuracy with 5.3M gaze samples across 22 VR scenes, showing improved predictive accuracy when combining modalities compared to individual data streams, with consistent performance across 1-3 future frames.

Conclusion: Integrating past gaze trajectories with head orientation and scene content enhances gaze prediction accuracy, contributing to understanding attention mechanisms in VR and enabling more efficient systems that anticipate user attention patterns.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models struggle to preserve fine-grained visual details when refining images using references due to VAE-based latent compression discarding subtle texture information, and existing post-editing methods often produce inconsistent results in lighting, texture, or shape.

Method: Two consecutive stages: 1) Fine-tune a single-image diffusion editor to jointly process draft and reference images for globally coherent refinement while maintaining structural fidelity, 2) Apply reinforcement learning to strengthen localized editing capability by optimizing for detail accuracy and semantic consistency.
...
TL;DR: This paper introduces CREward, the first type-specific creativity reward model that assesses creativity across three axes (geometry, material, texture) aligned with the image formation pipeline, using human benchmarks and LVLM-generated labels.


<details>
  <summary>Details</summary>
Motivation: Creativity is complex and treating it as a single undifferentiated quantity is naive. The authors aim to develop a more nuanced, type-specific approach to represent and assess creativity through different aspects of image formation.

Method: Conducted human benchmark evaluation to capture human perception of creativity across three types, analyzed correlation between human judgments and LVLM predictions, collected LVLM-generated labels to train the CREward model.

Result: Confirmed strong alignment between LVLMs and human perception of creativity. Successfully developed CREward model applicable to both evaluation and generation of creative images.

Conclusion: CREward enables three key applications: creativity assessment, explainable creativity, and creative sample acquisition for human design inspiration and guiding creative generation through low-rank adaptation.
...
TL;DR: This paper introduces Semantic-Aware Universal Perturbations (SAUPs) that can hijack entire decision chains of neural networks with a single perturbation, causing cascading errors across multiple predefined targets.


<details>
  <summary>Details</summary>
Motivation: Real-world models operate in sequences of decisions where isolated mistakes can be corrected, but cascading errors pose severe risks. Current adversarial attacks focus on single decisions, missing the threat of hijacking entire decision chains.
...
Conclusion: SAUPs demonstrate significant vulnerability in real-world models, showing that single perturbations can effectively hijack entire decision chains and cause cascading errors with severe consequences.
...
TL;DR: A multimodal fusion network using 7 modality features from visual and motion branches with depth-guided attention and temporal attention for pedestrian crossing intention prediction.


<details>
  <summary>Details</summary>
Motivation: Pedestrian crossing intention prediction is crucial for autonomous vehicles to reduce collision risks, but challenging due to diverse pedestrian behavior and contextual dependencies.

Method: Uses Transformer-based extraction modules for motion/visual features, depth-guided attention for spatial feature interactions, and modality/temporal attention to emphasize important modalities and capture temporal dependencies.

Result: Extensive experiments on JAAD dataset show superior performance compared to baseline methods.

Conclusion: The proposed multimodal fusion network effectively integrates complementary cues across modalities for improved pedestrian crossing intention prediction.
...
TL;DR: Proposes MFT - a multi-context fusion Transformer for pedestrian crossing intention prediction that integrates four contextual dimensions using progressive fusion strategy with attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Accurate pedestrian intention prediction in urban environments is challenging due to multiple factors affecting pedestrian behavior, which is essential for autonomous vehicle safety and reducing traffic accidents.

Method: Multi-context fusion Transformer (MFT) with progressive fusion strategy: mutual intra-context attention for feature fusion within each context, mutual cross-context attention for integration across contexts, and guided attention mechanisms for refinement.
...
Conclusion: MFT effectively integrates diverse contextual information through progressive fusion strategy and attention mechanisms, demonstrating superior performance in pedestrian crossing intention prediction.
...
TL;DR: ACIT is a Transformer-based model that uses attention mechanisms to integrate six visual and motion modalities for pedestrian crossing intention prediction, achieving state-of-the-art performance.
...
Method: Uses attention-guided cross-modal interaction Transformer with six modalities grouped into three pairs: global semantic map+optical flow, local RGB+optical flow, and ego-vehicle speed+pedestrian bounding box. Employs dual-path attention, cross-modal attention, and Transformer-based temporal feature aggregation.

Result: Outperforms state-of-the-art methods with 70% accuracy on JAADbeh and 89% on JAADall datasets.

Conclusion: ACIT effectively integrates multiple modalities through attention mechanisms and demonstrates superior performance in pedestrian crossing intention prediction.
...
TL;DR: The paper introduces Safety-Critical Reasoning as a new task for autonomous driving, addressing how avoiding one traffic risk can create another. It presents WaymoQA, a dataset of 35,000 human-annotated QA pairs for complex driving scenarios, and shows that fine-tuning MLLMs with this dataset significantly improves reasoning in safety-critical situations.


<details>
  <summary>Details</summary>
Motivation: High-level reasoning in safety-critical driving scenarios remains challenging for multimodal large language models (MLLMs), as avoiding one risk can create another. Current approaches with single front view inputs are insufficient for comprehensive environmental understanding.

Method: Defined Safety-Critical Reasoning as a two-stage process: resolve immediate risk first, then mitigate decision-induced downstream risks. Created WaymoQA dataset with 35,000 human-annotated question-answer pairs covering complex driving scenarios using multi-view inputs across image and video modalities.

Result: Experiments showed existing MLLMs underperform in safety-critical scenarios compared to normal scenes. However, fine-tuning with WaymoQA significantly improved their reasoning ability in these complex situations.
...
TL;DR: SAM-MI is a mask-injected framework that integrates SAM with open-vocabulary semantic segmentation models to address over-segmentation and hard mask-label combinations, achieving significant performance improvements and speedups.


<details>
  <summary>Details</summary>
Motivation: To overcome challenges in leveraging SAM for OVSS, specifically SAM's tendency to over-segment and the difficulty of combining fixed masks with labels effectively.

Method: Uses Text-guided Sparse Point Prompter for faster mask generation, Shallow Mask Aggregation to merge partial masks and reduce over-segmentation, and Decoupled Mask Injection to incorporate SAM masks at different frequency levels.

Result: Achieves 16.7% relative improvement in mIoU over Grounded-SAM on MESS benchmark with 1.6x speedup.

Conclusion: SAM-MI serves as an effective alternative methodology to equip OVSS models with SAM capabilities, addressing key challenges in the integration process.
...
Method: VGA constructs precise visual grounding from visual token semantics and uses it to guide attention to relevant regions. For image captioning, it dynamically suppresses already-described regions during generation.
...
Conclusion: Explicit visual guidance significantly enhances MLLMs' visual understanding capabilities, and VGA effectively addresses hallucination issues without requiring retraining.
...
TL;DR: COVec is an illumination-aware vectorization framework that uses intrinsic image decomposition to separate images into albedo, shade, and light layers in vector format, achieving superior visual fidelity and editability.


<details>
  <summary>Details</summary>
Motivation: Existing vectorization methods struggle with complex real-world images, often producing fragmented shapes and lacking semantic conciseness.

Method: Introduces intrinsic image decomposition in vector domain, separating images into albedo, shade, and light layers with semantic-guided initialization and two-stage optimization using differentiable rendering.

Result: Experiments show COVec achieves higher visual fidelity and significantly improved editability compared to existing methods across various datasets.

Conclusion: COVec successfully addresses limitations of traditional vectorization by incorporating illumination awareness through intrinsic decomposition, enabling better representation of complex real-world images.
...
Method: Multi-scale Flow Matching with coarse-to-fine generation paradigm using structured downsampling and upsampling to preserve geometric structure across resolutions.

Result: Achieves best-in-class performance among point-based methods and challenges representation-based methods, with strong results in multi-category and high-resolution generation.

Conclusion: MFM-Point substantially improves point-based generation quality and scalability while preserving simplicity and efficiency.
...
TL;DR: HACBSR is an unsupervised blind super-resolution framework for planetary remote sensing that operates without ground-truth images or external kernel priors, using contrastive kernel sampling and history-augmented contrastive learning.
...
Result: HACBSR achieves competitive performance compared to state-of-the-art unsupervised methods across multiple upscaling factors, with a new Ceres-50 dataset introduced for evaluation in planetary applications.

Conclusion: The proposed unsupervised framework effectively addresses blind super-resolution for planetary remote sensing without requiring ground-truth images or external kernel priors, demonstrating practical applicability through the Ceres-50 dataset.
...
TL;DR: DeLight-Mono is a self-supervised monocular depth estimation framework that addresses performance degradation in endoscopic images caused by uneven illumination through illumination decoupling.


<details>
  <summary>Details</summary>
Motivation: Existing depth estimation methods suffer from performance degradation in endoscopic images due to uneven illumination, particularly in low-intensity regions. Current low-light enhancement techniques fail to effectively guide depth networks, and solutions from other fields require well-lit images, making them unsuitable for endoscopic applications.

Method: Proposes an illumination-reflectance-depth model to represent endoscopic images, decomposing them with auxiliary networks. Introduces a self-supervised joint-optimizing framework with novel losses that leverage the decoupled components to mitigate uneven illumination effects on depth estimation.

Result: The effectiveness was rigorously verified through extensive comparisons and ablation studies on two public datasets, showing improved performance in handling uneven illumination conditions.

Conclusion: DeLight-Mono successfully addresses the challenge of uneven illumination in endoscopic depth estimation through illumination decoupling and joint optimization, providing a more robust solution for endoscopic navigation systems.
...
<details>
  <summary>Details</summary>
Motivation: Balancing compression ratio and reconstruction quality is challenging because low-frequency and high-frequency components contribute differently at the same resolution, making it difficult to achieve high compression ratios while maintaining quality.

Method: Proposes frequency-aware mechanism that decouples low/high-frequency components, converts voxelized embeddings to triplane representations to reduce sparsity, uses frequency-disentangling technique across scales, stores components in binary format, and employs frequency-based attention mechanism for 3D correlation recovery.
...
Conclusion: FLaTEC effectively addresses the challenge of balancing compression ratio and reconstruction quality by frequency-aware decomposition and triplane representation, enabling high compression ratios while maintaining reconstruction quality.
...
TL;DR: PRADA is a simple, interpretable method that detects autoregressive-generated images and attributes them to their source models using probability ratio analysis.


<details>
  <summary>Details</summary>
Motivation: There's a lack of work specifically targeting detection of images generated by autoregressive image generators, which are becoming increasingly realistic and need reliable detection methods.

Method: Analyzes the ratio of a model's conditional and unconditional probability for the autoregressive token sequence representing an image, exploiting unique characteristics in probability ratios for detection and attribution.

Result: Highly effective against eight class-to-image and four text-to-image autoregressive models, demonstrating reliable detection and attribution capabilities.

Conclusion: PRADA provides a simple yet effective approach for detecting and attributing autoregressive-generated images through probability ratio analysis.
...
TL;DR: The paper introduces a Task-Step-State (TSS) framework that uses 'states' as visually-grounded semantic layers to better align procedural knowledge with video representations, outperforming existing methods on multiple video understanding tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods align visual content with abstract task and step descriptions, but these high-level abstractions fail to form robust alignment with concrete visual details, limiting procedural reasoning capabilities.

Method: Proposed TSS framework with states as textual snapshots of object configurations, and progressive pre-training strategy that unfolds the TSS hierarchy to ground representations in states while associating them with steps and tasks.

Result: Outperforms baseline models on COIN and CrossTask datasets for task recognition, step recognition, and next step prediction. State supervision is key driver of performance gains across all tasks.

Conclusion: Introducing state supervision and progressive pretraining effectively grounds abstract procedures in observable visual details, enabling better procedural reasoning in video understanding.
...
Method: Develops CONVAD by: (i) creating a Concept Dataset for VAD, (ii) improving CBM architecture for dual concept-based and visual explanations, (iii) introducing artificial anomaly synthesis pipeline.
...
Conclusion: CONVAD successfully bridges semantic and localization interpretability in VAD, offering more insightful anomaly explanations through concept learning.
...
### [97] [Blind Adaptive Local Denoising for CEST Imaging](https://arxiv.org/abs/2511.20081)
*Chu Chen,Aitor Artola,Yang Liu,Se Weon Park,Raymond H. Chan,Jean-Michel Morel,Kannie W. Y. Chan*

Main category: cs.CV

TL;DR: Proposes BALD method for CEST MRI denoising that handles heteroscedastic noise through adaptive variance-stabilizing transform and two-stage denoising using local SVD decomposition.


<details>
  <summary>Details</summary>
Motivation: Clinical translation of CEST MRI is hindered by spatially varying noise from hardware limitations and complex protocols, which traditional denoising methods can't handle without altering critical biomedical information.

Method: BALD uses self-similarity of CEST data to create adaptive variance-stabilizing transform, performs two-stage denoising on linear transformation of data, and employs local SVD decomposition to prevent spatial/spectral artifacts.

Result: BALD consistently outperformed state-of-the-art CEST denoisers in denoising metrics and downstream tasks including molecular concentration maps estimation and cancer detection across phantom and in vivo experiments.

Conclusion: BALD effectively addresses heteroscedastic noise in CEST MRI while preserving critical molecular information, enabling more accurate quantitative contrast mapping for clinical applications.
...
### [98] [LungEvaty: A Scalable, Open-Source Transformer-based Deep Learning Model for Lung Cancer Risk Prediction in LDCT Screening](https://arxiv.org/abs/2511.20116)
*Johannes Brandt,Maulik Chevli,Rickmer Braren,Georgios Kaissis,Philip Müller,Daniel Rueckert*

Main category: cs.CV

TL;DR: LungEvaty is a transformer-based framework for predicting 1-6 year lung cancer risk from single LDCT scans, operating on whole-lung inputs without pixel-level annotations and achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: As lung cancer screening programs using LDCT expand globally, there's a need for scalable methods that can efficiently process entire lung volumes from large screening datasets without relying on labor-intensive pixel-level annotations.

Method: A fully transformer-based framework that learns directly from large-scale screening data (90,000+ CT scans) to capture comprehensive anatomical and pathological cues, with optional Anatomically Informed Attention Guidance (AIAG) loss to encourage anatomically focused attention.

Result: LungEvaty matches state-of-the-art performance using only imaging data without region supervision, and can be further refined by the optional AIAG loss.

Conclusion: The framework provides a simple, data-efficient, and fully open-source solution that serves as an extensible foundation for future research in longitudinal and multimodal lung cancer risk prediction.
...
### [99] [WPT: World-to-Policy Transfer via Online World Model Distillation](https://arxiv.org/abs/2511.20095)
*Guangfeng Jiang,Yueru Luo,Jun Liu,Yi Huang,Yiyao Zhu,Zhan Qu,Dave Zhenyu Chen,Bingbing Liu,Xu Yan*

Main category: cs.CV

TL;DR: WPT introduces a World-to-Policy Transfer training paradigm that uses an end-to-end world model to guide online distillation, enabling lightweight student policies with fast inference while maintaining high performance.
...
Result: Achieved state-of-the-art performance with 0.11 collision rate (open-loop) and 79.23 driving score (closed-loop), surpassing both world-model-based and imitation-learning methods in accuracy and safety.

Conclusion: WPT enables lightweight policies with up to 4.9x faster inference while retaining most performance gains, demonstrating effective knowledge transfer from world models to deployable policies.
...
TL;DR: Proposes a synthetic dataset for early forest fire detection using game simulators, combining it with existing data, and evaluates YOLOv7 and detection transformers for smoke/fire identification.


<details>
  <summary>Details</summary>
Motivation: Addresses the lack of sizeable datasets and models specifically tuned for early forest fire detection, as existing methods suffer from missed detection due to limited training data.

Method: Created a synthetic dataset using game simulators (Red Dead Redemption 2) to capture early fire indicators like smoke plumes, combined with existing published images. Evaluated image classification and localization using YOLOv7 and various detection transformer models.

Result: Developed a comprehensive dataset for early forest fire detection and compared performance of different object detection methods on this dataset.

Conclusion: The proposed synthetic dataset generation approach combined with modern detection models provides an effective solution for early forest fire warning systems.
...
TL;DR: LMMs fail to accurately identify when and where physical interactions begin/end in videos, despite being able to describe objects and actions.
...
Method: Created a large-scale dataset with 20K+ annotated interactions from Something-Something-V2, labeled by 250 human annotators for 'contact' and 'release' events. Tested two LMMs (Qwen-2.5VL and GPT-4o) on locating these events in short videos.

Result: Models can reliably name objects, identify actions, and provide coherent reasoning, but consistently fail to identify the exact frame and location where interactions begin or end.

Conclusion: LMMs lack perceptual grounding required for deeper understanding of dynamic scenes, struggling to pinpoint the moment and location of physical contact that defines interactions.
...
### [102] [Multi Head Attention Enhanced Inception v3 for Cardiomegaly Detection](https://arxiv.org/abs/2511.20101)
...
<details>
  <summary>Details</summary>
Motivation: To revolutionize healthcare imaging by developing an automated system for detecting cardiomegaly using advanced deep learning techniques, addressing the need for efficient diagnosis of cardiovascular structural abnormalities.

Method: Integrated approach using CNN with Inception V3 model combined with multi-layer attention mechanisms, particularly multi-head attention for selective region focusing. Process includes data collection, preprocessing, and strict evaluation.

Result: The model achieved outstanding performance: 95.6% accuracy, 95.2% precision, 96.2% recall, 95.7% sensitivity, 96.1% specificity, and 96.0% AUC, demonstrating high clinical diagnostic capability.

Conclusion: The proposed deep learning system with attention mechanisms successfully enables sensitive and accurate automatic detection of cardiomegaly from X-ray images, showing significant clinical potential for cardiovascular disease diagnosis.
...
### [103] [OmniAlpha: A Sequence-to-Sequence Framework for Unified Multi-Task RGBA Generation](https://arxiv.org/abs/2511.20211)
...
<details>
  <summary>Details</summary>
Motivation: Bridge the gap between specialized single-task RGBA models (limited versatility) and unified RGB frameworks (no RGBA support) for real-world applications requiring layer manipulation.

Method: Proposes OmniAlpha with MSRoPE-BiL, a novel RoPE method for Diffusion Transformer backbone enabling concurrent RGBA layer processing, trained on AlphaLayers dataset of 1,000 multi-layer triplets.

Result: Achieves 84.8% relative SAD reduction for mask-free matting on AIM-500, wins over 90% human preference in layer-conditioned completion, and consistently outperforms specialized baselines across 21 tasks.

Conclusion: Unified multi-task models can learn superior shared representations for RGBA, paving the way for more powerful layer-aware generative systems.
...
TL;DR: UltraViCo addresses video length extrapolation in diffusion transformers by suppressing attention dispersion for tokens beyond training length, achieving 4x extrapolation with significant quality improvements.


<details>
  <summary>Details</summary>
Motivation: Video diffusion transformers struggle to generalize beyond training length due to attention dispersion causing quality degradation and periodic content repetition, which prior methods only partially addressed.

Method: Propose UltraViCo, a training-free plug-and-play method that suppresses attention for tokens beyond training window using a constant decay factor to address attention dispersion.

Result: Outperforms baselines across models and extrapolation ratios, achieving 4x extrapolation with 233% improvement in Dynamic Degree and 40.5% in Imaging Quality over previous best method.

Conclusion: The method effectively addresses both failure modes of video length extrapolation and generalizes to downstream tasks like controllable video synthesis and editing.
...
TL;DR: PETRG-3D is an end-to-end 3D dual-branch framework for automated PET/CT report generation that addresses challenges in functional imaging through volumetric modeling and style-adaptive prompts to handle inter-hospital variability.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of PET/CT scanners has outpaced specialist availability, creating a need for automated report generation to reduce clinical workload, with functional PET posing unique challenges compared to structural imaging.

Method: Proposed PETRG-3D framework with separate PET and CT volume encoding, style-adaptive prompts to handle reporting variability, and constructed PETRG-Lym multi-center dataset (824 reports) with AutoPET-RG-Lym public benchmark.
...
Conclusion: This work establishes a foundation for PET/CT-specific models emphasizing disease-aware reasoning and clinically reliable evaluation, with codes, models, and AutoPET-RG-Lym benchmark to be released.
...
### [106] [Uplifting Table Tennis: A Robust, Real-World Application for 3D Trajectory and Spin Estimation](https://arxiv.org/abs/2511.20250)
*Daniel Kienzle,Katja Ludwig,Julian Lorenz,Shin'ichi Satoh,Rainer Lienhart*

Main category: cs.CV

TL;DR: A two-stage pipeline for 3D table tennis ball motion analysis that separates 2D perception from 3D uplifting, trained on real 2D data and synthetic 3D data to overcome lack of real 3D ground truth.
...
Method: Two-stage pipeline: front-end perception (ball and table detection) trained on TTHQ dataset with 2D supervision, and back-end uplifting network trained on synthetic data, made robust to real-world artifacts like missing detections.

Result: Transforms proof-of-concept uplifting into practical end-to-end application for robust 3D trajectory and spin analysis.

Conclusion: The separation of 2D perception and 3D uplifting enables effective training with available data sources, creating a robust system for real-world table tennis motion analysis.
...
TL;DR: HCFSSNet is a hybrid image compression network combining CNNs for local high-frequency details with Vision Frequency State Space blocks for long-range low-frequency modeling, achieving competitive performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Existing methods like Transformers and SSMs provide good long-range modeling but may cause structural information loss or ignore frequency characteristics crucial for compression. CNNs capture local details well but lack long-range modeling capabilities.

Method: Hybrid architecture using CNNs for local high-frequency extraction and Vision Frequency State Space (VFSS) blocks with Omni-directional Neighborhood State Space (VONSS) and Adaptive Frequency Modulation Module (AFMM). Also includes Frequency Swin Transformer Attention Module (FSTAM) for frequency-aware side information modeling.

Result: Achieves competitive rate-distortion performance with significantly fewer parameters than recent SSM-based codecs. Reduces BD rate over VTM anchor by 18.06% on Kodak, 24.56% on Tecnick, and 22.44% on CLIC datasets.

Conclusion: HCFSSNet provides an efficient and interpretable hybrid architecture that effectively combines local and global frequency modeling for learned image compression, offering a promising direction for future compression systems.
...
TL;DR: A lightweight ResNet18-based pipeline detects camera activation state and position in DaVinci Xi surgical videos with near-perfect accuracy, enabling automated preprocessing for surgical data science.
...
Method: Developed a lightweight pipeline using ResNet18 CNN to identify camera tile position and activation state. Model was fine-tuned on manually annotated SurgToolLoc data and evaluated on three public datasets with over 70,000 frames.
...
Result: Shows superior perceptual quality and faster processing time compared to diffusion and flow matching-based methods on natural and medical datasets.
...
TL;DR: R-TNAG framework for Alzheimer's disease progression modeling from irregularly sampled longitudinal MRI data using Riemannian manifolds, time-aware neural ODEs, and attention-based Riemannian GRUs.


<details>
  <summary>Details</summary>
Motivation: Clinical uncertainty leads to irregular observation intervals in longitudinal imaging data, and Euclidean space models fail to capture the intrinsic continuity and nonlinear geometric structure of irregularly sampled data.

Method: Projects sMRI features into manifold space, uses time-aware neural ODE for continuous evolution modeling, and attention-based Riemannian GRU for adaptive historical integration.

Result: Outperforms state-of-the-art models in disease prediction and cognitive score regression, shows stable performance across varying sequence lengths and missing data, and demonstrates cross-dataset robustness.

Conclusion: The proposed framework effectively handles irregular sampling in longitudinal imaging data and provides robust AD trajectory prediction with strong temporal generalizability across clinical settings.
...
TL;DR: MAP-World is a prior-free multi-modal planning framework that uses masked action planning with path-weighted world models to handle multiple plausible futures without handcrafted anchors or reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Existing motion planning systems discard information about alternative futures by selecting a single best mode, complicating optimization and losing valuable information about plausible alternatives.

Method: Uses Masked Action Planning (MAP) that treats future ego motion as masked sequence completion, expands compact latent states into multiple trajectory queries with noise, and employs a lightweight world model to roll out future BEV semantics for each candidate trajectory.
...
Conclusion: The framework successfully learns from the full distribution of plausible futures without reinforcement learning or anchor libraries, providing diverse and temporally consistent trajectory modes.
...
TL;DR: SKEL-CF is a coarse-to-fine framework that improves SKEL parameter estimation for anatomically accurate 3D human modeling, achieving state-of-the-art performance on the MOYO dataset.


<details>
  <summary>Details</summary>
Motivation: Existing parametric 3D human models like SMPL have simplified kinematics that limit biomechanical realism, while the anatomically accurate SKEL model faces challenges in parameter estimation due to limited training data, perspective ambiguities, and complex human articulation.

Method: SKEL-CF uses a transformer-based encoder-decoder architecture where the encoder predicts coarse camera and SKEL parameters, and the decoder progressively refines them. The framework incorporates explicit camera modeling and uses a newly created SKEL-aligned dataset (4DHuman-SKEL) for training.

Result: On the MOYO dataset, SKEL-CF achieves 85.0 MPJPE / 51.4 PA-MPJPE, significantly outperforming the previous SKEL-based state-of-the-art HSMR (104.5 / 79.6).

Conclusion: SKEL-CF establishes a scalable and anatomically faithful framework for human motion analysis, bridging the gap between computer vision and biomechanics.
...
TL;DR: PromptCT is a storage-efficient deep unfolding framework for sparse-view CT reconstruction that handles multiple view configurations in one model using Lipschitz-constrained networks and explicit prompts.


<details>
  <summary>Details</summary>
Motivation: Address limitations of current deep learning SVCT methods: (i) difficulty proving Lipschitz constraints for prior networks, and (ii) high storage costs from training separate models for each view setting.

Method: Developed LipNet - an explicitly provable Lipschitz-constrained network, integrated with explicit prompt module for discriminative knowledge of different sparse sampling settings. Created PromptCT framework embedding LipNet as prior network.

Result: Outperformed benchmark algorithms in multiple-in-one SVCT reconstruction, achieving higher-quality reconstructions with lower storage costs in both simulated and real data experiments.

Conclusion: Proved LipNet satisfies boundary property and Lipschitz continuity, analyzed convergence of iterative algorithms. Provides efficient solution for multiple view configurations in single model with theoretical guarantees.
...
TL;DR: HPA is a post-training framework that balances safety and task performance in continual visual instruction tuning for safety-aligned multimodal large language models by partitioning parameters, balanced selection, and orthogonal constraints.


<details>
  <summary>Details</summary>
Motivation: Existing continual visual instruction tuning studies focus on models without safety alignment, ignoring that real-world MLLMs require safety mechanisms. Safety-aligned MLLMs suffer from both task forgetting and safety degradation during continual adaptation.

Method: Proposed Harmonious Parameter Adaptation (HPA) framework with three components: focusing-based parameter partition (dividing parameters into safety-focused and task-focused), harmoniously balanced parameter selection, and orthogonal parameter adjustment to alleviate catastrophic forgetting.
...
### [115] [3D Motion Perception of Binocular Vision Target with PID-CNN](https://arxiv.org/abs/2511.20332)
*Shi Jiazhao,Pan Pan,Shi Haotian*

Main category: cs.CV

TL;DR: A 17-layer PID convolutional neural network with 413K parameters was developed for real-time 3D motion perception of binocular vision targets, achieving near-optimal accuracy on simulated moving ball datasets.


<details>
  <summary>Details</summary>
Motivation: To develop a neural network capable of perceiving 3D motion information (coordinates, velocity, acceleration) from binocular vision in real-time, with basic spatiotemporal perception capabilities.

Method: Designed a PID convolutional neural network using second-order difference equations and nonlinearities to fit local problems, with feature reuse through concatenation and pooling. Trained on simulated randomly moving ball datasets.
...
TL;DR: ADNet is a large-scale multi-domain anomaly detection benchmark with 380 categories from 49 datasets, revealing scalability challenges in existing methods and proposing Dinomaly-m as a solution.
...
Method: Created ADNet benchmark with 196,294 RGB images (116,192 normal, 80,102 test with 60,311 anomalies) from 49 datasets across 5 domains, standardized with MVTec-style pixel annotations and text descriptions. Proposed Dinomaly-m, a context-guided Mixture-of-Experts extension that expands decoder capacity without increasing inference cost.

Result: Existing methods drop from 90.6% I-AUROC in one-for-one settings to 78.5% when scaling to 380 categories. Dinomaly-m achieves 83.2% I-AUROC and 93.1% P-AUROC, outperforming existing approaches.

Conclusion: ADNet addresses the scalability gap in anomaly detection, provides a standardized extensible benchmark for diverse domains, and supports future anomaly detection foundation models.
...
TL;DR: First battery-powered wearable eye tracking system using event-based sensors and neuromorphic processing, achieving 100Hz pupil tracking with under 5mW power per eye.


<details>
  <summary>Details</summary>
Motivation: Current eye tracking systems lack robust, high-frequency tracking with ultra-low power consumption needed for wearable platforms, while event-based sensors haven't had fully integrated low-power processing solutions.

Method: Combines event-based sensing and neuromorphic processing on Speck2f SoC with lightweight coordinate decoding on microcontroller, using novel uncertainty-quantifying spiking neural network with gated temporal decoding optimized for memory/bandwidth constraints.

Result: Validated on multi-user dataset, wearable prototype achieves robust binocular pupil tracking at 100Hz with average power consumption below 5mW per eye.
...
### [118] [From Passive Perception to Active Memory: A Weakly Supervised Image Manipulation Localization Framework Driven by Coarse-Grained Annotations](https://arxiv.org/abs/2511.20359)
*Zhiqing Guo,Dongdong Xi,Songlin Li,Gaobo Yang*

Main category: cs.CV

TL;DR: BoxPromptIML is a weakly-supervised image manipulation localization framework that balances annotation cost and localization accuracy using coarse region annotations and knowledge distillation from SAM, achieving performance comparable to fully-supervised methods.


<details>
  <summary>Details</summary>
Motivation: Address the fundamental trade-off between annotation cost and localization accuracy in image manipulation localization, where fully-supervised methods require expensive pixel-level masks while weakly-supervised approaches lack precise spatial localization.

Method: Uses coarse region annotation strategy for cost-effective mask generation, knowledge distillation from SAM teacher to lightweight student model, and dual-guidance feature fusion module inspired by human subconscious memory mechanism for dynamic knowledge recollection.

Result: Outperforms or rivals fully-supervised models across in-distribution and out-of-distribution datasets while maintaining strong generalization, low annotation cost, and efficient deployment characteristics.

Conclusion: BoxPromptIML effectively balances annotation cost and localization performance, demonstrating that weakly-supervised approaches can achieve comparable results to fully-supervised methods with significantly reduced annotation effort.
...
Method: Three-module framework: EgoExo-Align for latent-space view alignment, MultiExoCon for multi-view video conditioning, and PoseInj for camera pose injection to guide geometry-aware synthesis.
...
Conclusion: Exo2EgoSyn successfully extends foundation models for scalable cross-view video generation, paving the way for broader applications in multi-perspective video synthesis.
...
Method: Proposed SFA framework that adaptively scans video frames, selectively focuses on key regions containing relevant text, and amplifies these regions to guide Video-LLM attention without requiring additional training.
...
Conclusion: SFA effectively addresses Video TextVQA challenges by mimicking human cognitive processes and demonstrates strong effectiveness and generalizability.
...
### [121] [StableTrack: Stabilizing Multi-Object Tracking on Low-Frequency Detections](https://arxiv.org/abs/2511.20418)
...
<details>
  <summary>Details</summary>
Motivation: Current MOT approaches require processing every frame, making them computationally expensive and unsuitable for limited computing resources. The paper addresses the challenge of maintaining tracking quality with low-frequency detections.

Method: Proposes a two-stage matching strategy with novel Bbox-Based Distance instead of Mahalanobis distance, and integrates visual tracking into Kalman Filter and tracking pipeline.

Result: Achieves 11.6% HOTA improvement at 1Hz on MOT17-val with low-frequency detections, while maintaining competitive performance on standard MOT17, MOT20, and DanceTrack benchmarks with full-frequency detections.

Conclusion: StableTrack effectively addresses the computational limitations of MOT systems by stabilizing tracking quality on low-frequency detections, making MOT more practical for resource-constrained environments.
...
TL;DR: GHR-VQA is a human-centric Video QA framework that uses scene graphs to model human-object interactions across video frames, enabling cross-frame reasoning through graph neural networks and hierarchical feature integration.


<details>
  <summary>Details</summary>
Motivation: Traditional pixel-based Video QA methods struggle to capture complex human-object interactions and spatiotemporal dynamics. The authors aim to create a more interpretable framework that explicitly models human-centered relationships in videos.
...
Result: Achieved significant performance improvements on the AGQA dataset, including 7.3% improvement in object-relation reasoning over state-of-the-art methods.

Conclusion: The explicit human-rooted graph structure enables better interpretability and more profound understanding of spatiotemporal dynamics in video question answering, demonstrating the effectiveness of graph-based reasoning for human-centric video analysis.
...
TL;DR: A U-Net based deep learning framework won first place in the BraTS-Inpainting 2025 challenge by effectively synthesizing healthy brain tissue in 3D MRI scans, outperforming previous winning solutions.


<details>
  <summary>Details</summary>
Motivation: To mitigate dataset biases that limit deep learning models in quantitative brain tumor MRI analysis by developing a method for synthesizing healthy tissue in corrupted regions.
...
Result: Achieved SSIM of 0.919±0.088, PSNR of 26.932±5.057, and RMSE of 0.052±0.026 on final test set, securing first place in the 2025 challenge and surpassing 2023-2024 winning solutions.

Conclusion: The proposed framework successfully addresses dataset bias issues in brain tumor MRI analysis through effective inpainting of healthy tissue, demonstrating state-of-the-art performance in the BraTS-Inpainting challenge.
...
TL;DR: CT-CIG is a controllable text-guided method for generating realistic camouflage images that blend objects naturally into backgrounds using VLMs for text annotation and Stable Diffusion fine-tuning with location/shape control.


<details>
  <summary>Details</summary>
Motivation: Existing camouflage image generation methods fail to produce natural results because they overlook the logical relationship between camouflaged objects and background environments, leading to unrealistic blending.

Method: Uses Large Visual Language Models to annotate camouflage datasets with text prompts, fine-tunes Stable Diffusion with a lightweight controller for object location/shape guidance, and employs a Frequency Interaction Refinement Module to capture high-frequency texture features.
...
Conclusion: CT-CIG successfully addresses the logical relationship issue in camouflage image generation by combining text guidance, controllable object placement, and frequency-based texture refinement to produce realistic and logically plausible results.
...
TL;DR: Fine-tuned Vision Transformer for glioblastoma histopathological analysis achieved 2nd place in BraTS-Path 2025 Challenge with MCC 0.6509 and F1-score 0.5330 on test set.


<details>
  <summary>Details</summary>
Motivation: Glioblastoma heterogeneity complicates diagnosis and patient stratification, while deep learning offers objective automated analysis of whole slide images.

Method: Fine-tuned pre-trained Vision Transformer encoder with dedicated classification head on official training dataset.

Result: Achieved MCC 0.7064 and F1-score 0.7676 on validation set, and MCC 0.6509 and F1-score 0.5330 on test set (2nd place in challenge).

Conclusion: Establishes solid baseline for ViT-based histopathological analysis; future work will address performance gap on unseen validation data.
...
<details>
  <summary>Details</summary>
Motivation: Existing adversarial attack methods struggle with controllability and fail to precisely manipulate specific concept semantics in images due to semantic entanglement in patch-token representations where global context dominates individual patch features.

Method: V-Attack targets value features (V) in transformer attention blocks, which suppress global-context channels and retain disentangled local semantic information. It includes Self-Value Enhancement to refine semantic richness and Text-Guided Value Manipulation to locate source concepts and optimize toward target concepts using text prompts.

Result: V-Attack achieves highly effective semantic control and improves attack success rate by an average of 36% over state-of-the-art methods across diverse LVLMs including LLaVA, InternVL, DeepseekVL and GPT-4o.

Conclusion: The method exposes critical vulnerabilities in modern visual-language understanding by demonstrating that value features serve as precise handles for semantic manipulation, bypassing the limitations of entangled patch features.
...
TL;DR: HistoSpeckle-Net is a deep learning architecture that reconstructs medical images from multimode fiber speckles using distribution-aware learning and histogram-based mutual information loss, achieving superior performance with limited data and under fiber bending conditions.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for MMF imaging focus on simpler datasets and are data-intensive, limiting their applicability to complex real-world medical imaging tasks. There's a need for models that can handle diverse, complex images with reduced data requirements.

Method: Developed HistoSpeckle-Net with a histogram computation unit for estimating smooth marginal and joint histograms to calculate mutual information loss, and a Three-Scale Feature Refinement Module for multiscale SSIM loss computation. Built a clinically relevant dataset using an optical setup with SLM and MMF to capture speckle patterns corresponding to OrganAMNIST images.

Result: HistoSpeckle-Net achieves higher fidelity than baseline models (U-Net and Pix2Pix) on the complex OrganAMNIST dataset, showing superior performance even with limited training samples and across varying fiber bending conditions.

Conclusion: HistoSpeckle-Net effectively reconstructs complex anatomical features with reduced data and under fiber perturbations, bringing MMF imaging closer to practical deployment in real-world clinical environments.
...
TL;DR: The paper studies the fidelity-diversity dilemma in text-to-image generation with long prompts, introduces a benchmark (LPD-Bench), and proposes PromptMoG, a training-free method that samples from Mixture-of-Gaussians in embedding space to enhance diversity while preserving semantics.


<details>
  <summary>Details</summary>
Motivation: Long prompts in text-to-image generation encode rich information but suppress diversity, leading to repetitive outputs. Current models show clear diversity drop as prompt length increases, creating a fidelity-diversity dilemma that needs systematic study.

Method: Introduces LPD-Bench benchmark for evaluation, develops theoretical framework for sampling entropy through prompt reformulation, and proposes PromptMoG - a training-free method that samples prompt embeddings from Mixture-of-Gaussians in embedding space.

Result: Extensive experiments on four state-of-the-art models (SD3.5-Large, Flux.1-Krea-Dev, CogView4, Qwen-Image) show PromptMoG consistently improves long-prompt generation diversity without semantic drifting.

Conclusion: PromptMoG effectively addresses the fidelity-diversity dilemma in long-prompt text-to-image generation by enhancing diversity while maintaining semantic fidelity, providing a practical solution for more creative and varied outputs.
...
<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary 3D detectors still depend on training scenes, limiting their ability to recognize diverse, previously unseen objects in real-world environments. The goal is to create a truly training-free solution for 3D object detection.

Method: Zoo3D constructs 3D bounding boxes via graph clustering of 2D instance masks, then assigns semantic labels using a novel open-vocabulary module with best-view selection and view-consensus mask generation. It operates in two modes: zero-shot Zoo3D₀ (no training) and self-supervised Zoo3D₁ (refines boxes using pseudo labels).

Result: Across ScanNet200 and ARKitScenes benchmarks, both Zoo3D₀ and Zoo3D₁ achieve state-of-the-art results in open-vocabulary 3D object detection. Remarkably, the zero-shot Zoo3D₀ outperforms all existing self-supervised methods.

Conclusion: Zoo3D demonstrates the power and adaptability of training-free, off-the-shelf approaches for real-world 3D understanding, offering a flexible solution that works with point clouds, posed images, and even unposed images.
...
TL;DR: Adv-GRPO is an RL framework that uses adversarial rewards with reference images and vision foundation models to prevent reward hacking and improve image generation quality.


<details>
  <summary>Details</summary>
Motivation: Current RL approaches rely on scalar rewards from pre-trained models that fail to capture human perception and are vulnerable to reward hacking, where higher scores don't correspond to better images.

Method: Introduces adversarial reward framework that iteratively updates both reward model and generator, using reference images as positive samples and vision foundation models (e.g., DINO) to provide rich visual rewards instead of single scalar values.

Result: Outperforms Flow-GRPO and SD3 with 70.0% win rate in image quality and 72.4% in aesthetics in human evaluation. Achieves consistent gains across quality, aesthetics, and task-specific metrics.

Conclusion: Using reference images with foundation-model rewards enables distribution transfer and flexible style customization while preventing reward hacking, leading to higher-quality image generation.
...
TL;DR: MBCD is a collaborative distillation framework that addresses WA's limitations in multi-modal domain generalization by using adaptive modality dropout, gradient consistency constraints, and WA-based cross-modal distillation to achieve flatter minima and better generalization.


<details>
  <summary>Details</summary>
Motivation: Weight Averaging (WA) struggles in multi-modal domain generalization due to different optimization speeds across modalities, causing early overfitting to faster modalities and suppressing slower complementary ones, leading to poor modality fusion and sharp minima.

Method: Proposes MBCD with three components: adaptive modality dropout to prevent early bias, gradient consistency constraint to align learning signals, and WA-based teacher for cross-modal distillation that transfers fused knowledge to uni-modal branches.
...
Conclusion: MBCD successfully retains WA's flatness-inducing advantages while overcoming its multi-modal limitations, enabling effective modality fusion and convergence to flatter, more generalizable solutions.
...
<details>
  <summary>Details</summary>
Motivation: Conventional classification approaches perform suboptimally under high-uncertainty conditions like corrupted images or limited training data, as they directly predict class labels without modeling uncertainty.

Method: Proposes Discrete Diffusion Classification Modeling (DiDiCM) that uses diffusion-based procedure to model posterior distribution of class labels conditioned on input images, supporting predictions on class probabilities or discrete labels.

Result: DiDiCM achieves higher classification accuracy on ImageNet with just a few diffusion iterations compared to baselines, with accuracy gains increasing as tasks become more challenging.

Conclusion: Diffusion-based classification modeling provides a flexible and effective approach for handling uncertainty in image classification, outperforming standard classifiers especially in challenging scenarios.
...
TL;DR: Semi-supervised deep reinforcement learning framework for industrial anomaly detection that combines neural batch sampling, autoencoder loss profiling, and predictor segmentation to effectively detect subtle defects with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Anomaly detection in industrial visual inspection is challenging due to scarcity of defective samples, and existing unsupervised reconstruction methods often overfit and perform poorly on subtle defects.

Method: Proposes a semi-supervised deep RL framework with neural batch sampler (balancing exploration/exploitation), autoencoder for loss profiles, and predictor for segmentation in loss-profile space.
...
Conclusion: The framework effectively learns both normal and defective patterns with limited labeled data while maintaining low complexity, outperforming state-of-the-art approaches.
...
TL;DR: VKnowU is a benchmark for evaluating visual knowledge in MLLMs, revealing gaps in world-centric understanding. VideoKnow+ model with structured reasoning and reinforcement learning achieves +3.7% improvement.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack intuitive understanding of physical and social principles that bridge perception and reasoning, which is crucial for true visual understanding.
...
TL;DR: ScenarioCLIP addresses the limitation of current CLIP models in handling compositional scenes by explicitly modeling inter-object relations and actions, achieving superior performance on scenario-based tasks.


<details>
  <summary>Details</summary>
Motivation: Current CLIP models focus on single-object classification or retrieval but fail to capture the rich compositional structure of real-world scenes involving multiple objects and actions. Existing improvements remain confined to predefined classes without explicit relational modeling.

Method: Proposed ScenarioCLIP accepts input texts, grounded relations, and images with focused regions highlighting relations. The model is pretrained on curated scenario data generated by extending existing datasets using language models to ground actions, objects, and relations, then finetuned for downstream tasks.

Result: ScenarioCLIP demonstrates robust zero-shot and finetune performance on various domain-specific tasks, establishing comprehensive benchmarks that outperform baseline methods in scenario-based tasks.

Conclusion: Explicit modeling of relational and compositional structure significantly improves scene understanding capabilities in CLIP models, enabling better performance on complex real-world scenario tasks.
...
TL;DR: DAPointMamba is a novel framework for domain adaptive point cloud completion that uses State Space Models to achieve global receptive fields with linear complexity, addressing challenges in spatial topology preservation and domain-agnostic representation learning.


<details>
  <summary>Details</summary>
Motivation: Existing methods for domain adaptive point cloud completion suffer from limited receptive fields or quadratic complexity when using CNNs or Transformers, and directly applying SSMs disrupts spatial topology and local geometric features.

Method: Proposed DAPointMamba with three modules: Cross-Domain Patch-Level Scanning for local alignment, Cross-Domain Spatial SSM Alignment for spatial consistency, and Cross-Domain Channel SSM Alignment for global semantic gaps.
...
Conclusion: DAPointMamba demonstrates strong adaptability across domains with advantages of global receptive fields and efficient linear complexity, making it effective for domain adaptive point cloud completion.
...
TL;DR: SelfMOTR is a novel tracking transformer that leverages self-generated detection priors from MOTR-like models to address detection-association conflicts in end-to-end tracking.


<details>
  <summary>Details</summary>
Motivation: To overcome poor detection performance and the conflict between detection and association in joint transformer architectures for multi-object tracking.

Method: Utilizes self-generated detection priors by uncovering the hidden detection capabilities of MOTR-like models, providing practical tools to leverage these capabilities effectively.

Result: Achieves strong performance on DanceTrack, competing with recent state-of-the-art end-to-end tracking methods.

Conclusion: SelfMOTR demonstrates that leveraging self-generated detection priors from transformer tracking models can effectively address detection-association conflicts and achieve competitive tracking performance.
...
TL;DR: Training-free iterative self-refinement framework using LLMs and VLMs to improve physics consistency in video generation through multimodal chain-of-thought reasoning.
...
Method: Propose iterative self-refinement framework with multimodal chain-of-thought process that refines prompts based on physics inconsistency feedback from LLMs and VLMs.
...
Conclusion: This work provides a preliminary exploration of physics-consistent video generation and offers insights for future research in this direction.
...
TL;DR: BTTF is a novel optimization framework that generates video counterfactual explanations (CFEs) by ensuring temporal coherence and physical plausibility, addressing limitations of existing image-based CFE methods.


<details>
  <summary>Details</summary>
Motivation: Existing counterfactual explanation methods are designed for image classifiers and fail to generate temporally coherent, smooth, and physically plausible video CFEs, leaving video classifier explanations largely unexplored.

Method: BTTF introduces: 1) optimization to retrieve initial latent noise conditioned by the first video frame, 2) two-stage optimization to search for CFEs near the input video, and 3) progressive optimization with increasing denoising steps for faster convergence.
...
Conclusion: BTTF successfully addresses the challenge of generating video counterfactual explanations by ensuring temporal coherence and physical plausibility, providing faithful explanations for video classifiers.
...
TL;DR: CrossEarth-Gate is a PEFT method for remote sensing that uses a comprehensive module toolbox and Fisher-guided adaptive selection to handle multifaceted domain gaps in large-scale Earth observation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing PEFT methods fail in large-scale RS tasks due to inability to handle multifaceted domain gaps (spatial, semantic, frequency shifts) inherent in RS data.

Method: Establishes RS module toolbox with spatial, semantic, and frequency modules, plus Fisher-guided adaptive selection mechanism that quantifies module importance and dynamically activates critical modules at appropriate layers.
...
Conclusion: CrossEarth-Gate effectively addresses multifaceted domain gaps in RS data through adaptive module selection and achieves superior performance in cross-domain semantic segmentation tasks.
...
TL;DR: TaCo is a spatio-temporal semantic consistent network for remote sensing change detection that introduces text-guided transition features and joint constraints to address semantic inconsistencies in bi-temporal satellite images.


<details>
  <summary>Details</summary>
Motivation: Previous methods rely solely on mask supervision which provides limited constraints on temporal semantic transitions, leading to spatially coherent but semantically inconsistent predictions.
...
Result: Extensive experiments on six public datasets for binary and semantic change detection tasks demonstrate that TaCo consistently achieves state-of-the-art performance without additional computational overhead during inference.

Conclusion: TaCo effectively addresses semantic inconsistencies in change detection by modeling change as semantic transitions between bi-temporal states and enforcing spatio-temporal semantic consistency through joint constraints.
...
TL;DR: TReFT enables one-step image translation with pretrained Rectified Flow models by using velocity prediction as output, achieving real-time performance comparable to state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Rectified Flow models require costly multi-step denoising for image translation, hindering real-time applications. Direct adversarial training on RF models causes convergence issues.

Method: TReFT uses velocity predicted by pretrained DiT/UNet as output, with memory-efficient latent cycle-consistency and identity losses, plus lightweight architectural simplifications.

Result: Pretrained RF models finetuned with TReFT achieve performance comparable to state-of-the-art methods across multiple datasets while enabling real-time inference.

Conclusion: TReFT successfully enables one-step image translation with RF models, solving convergence issues and achieving real-time performance without sacrificing quality.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for IRSTD suffer from pattern drift across diverse scenarios due to static pattern learning, limiting robustness in varying conditions like day/night variations and different domains.

Method: Proposes IrisNet with an image-to-decoder transformer that dynamically maps infrared image features to entire decoder parameters, representing decoder as structured 2D tensor and using self-attention for inter-layer dependencies and cross-attention for adaptive decoding. Integrates high-frequency components for enhanced perception.

Result: Achieves state-of-the-art performance on NUDT-SIRST, NUAA-SIRST, and IRSTD-1K datasets, demonstrating superior detection capability.

Conclusion: IrisNet effectively addresses pattern drift in IRSTD through dynamic parameter adaptation and enhanced feature perception, providing robust detection across diverse infrared scenarios.
...
TL;DR: Introduces an Impartial World Model framework that teaches autonomous driving systems to honestly predict danger through counterfactual data synthesis, enabling safer policy refinement via RL.


<details>
  <summary>Details</summary>
Motivation: Address the optimistic bias in world models for autonomous driving RL, which hinders safety and handling of long-tail events by failing to accurately predict dangerous scenarios.

Method: Developed a Counterfactual Synthesis pipeline to generate plausible collisions and off-road events, creating an Impartial World Model that serves as an honest internal critic for RL policy refinement.

Result: Significantly outperforms baselines in failure prediction on Risk Foreseeing Benchmark and enables substantial reduction in safety violations in challenging simulations.

Conclusion: Teaching models to accurately dream of danger through impartial world modeling is crucial for building truly safe and intelligent autonomous agents.
...
TL;DR: A deep learning framework for single-image homography estimation using ConvNeXt backbone and normalized coordinate regression, achieving competitive accuracy and speed for shelf image rectification.


<details>
  <summary>Details</summary>
Motivation: Single-view homography estimation is challenging but valuable for retail applications like shelf monitoring where only one viewpoint is available.

Method: Uses ConvNeXt-based backbone with normalized coordinate regression and introduces novel augmentation strategy by modeling and sampling synthetic homographies.
...
TL;DR: AMB3R is a multi-view feed-forward model for dense 3D reconstruction that uses a sparse volumetric representation and achieves state-of-the-art performance across multiple 3D vision tasks without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To create a unified model that can handle diverse 3D vision tasks including multi-view reconstruction, visual odometry, and structure from motion without requiring task-specific adaptations or optimization.

Method: Leverages a sparse yet compact volumetric scene representation as backend for geometric reasoning, using a multi-view feed-forward architecture trained solely for multi-view reconstruction but extensible to other tasks.
...
Conclusion: AMB3R demonstrates that a single model trained for multi-view reconstruction can effectively generalize to multiple 3D vision tasks, outperforming specialized methods while maintaining spatial compactness.
...
<details>
  <summary>Details</summary>
Motivation: LiDAR-based methods lack semantics and textures, while LiDAR-camera fusion requires complex calibration and struggles with materials like glass. Camera-only approach eliminates hardware complexity and calibration requirements.
...
Result: Provides sensor simulation fidelity comparable to LiDAR-camera fusion while eliminating hardware complexity and calibration requirements. Validated using LiDAR as ground truth for reflectivity validation.

Conclusion: Camera-only approach combines photorealistic reconstruction with physics-based material assignment, offering comparable performance to fusion methods without the hardware complexity.
...
TL;DR: Proposes humanoid visual search using 360° panoramic images to mimic human head-eye coordination, introduces H* Bench benchmark with challenging real-world scenes, and shows significant performance improvements through post-training while highlighting remaining challenges.


<details>
  <summary>Details</summary>
Motivation: Current visual search approaches are limited to static images and neglect embodied interaction with 3D environments. The goal is to develop efficient embodied visual search agents that mimic human head-eye coordination while avoiding real-world hardware constraints.

Method: Proposes humanoid visual search where agents actively rotate their head in 360° panoramic images. Built H* Bench benchmark with challenging real-world scenes requiring advanced visual-spatial reasoning. Used post-training techniques to enhance open-source Qwen2.5-VL model.

Result: Top-tier proprietary models achieved only ~30% success in object and path search. Post-training increased Qwen2.5-VL's success rate by over threefold: object search from 14.83% to 47.38%, path search from 6.44% to 24.94%. Path search proved significantly more challenging due to spatial commonsense requirements.

Conclusion: The approach shows promising progress but reveals immense remaining challenges in building MLLM agents that can be seamlessly integrated into everyday human life, particularly for tasks requiring sophisticated spatial reasoning.
...
TL;DR: GS-Checker is a novel method for locating tampered regions in 3D Gaussian Splatting models without requiring expensive 3D supervision labels.


<details>
  <summary>Details</summary>
Motivation: Recent advances in 3DGS editing technologies raise concerns about malicious manipulation of 3D content, making tampering localization crucial to prevent malicious applications.

Method: Integrates 3D tampering attribute into Gaussian parameters, uses 3D contrastive mechanism to compare attribute similarities for tampering cues, and employs cyclic optimization strategy to refine tampering attributes.

Result: Extensive experiments demonstrate the method's effectiveness in accurately locating tampered 3DGS areas.

Conclusion: The proposed GS-Checker provides an effective solution for tampering localization in 3DGS models without requiring costly 3D supervision.
...
Result: Achieves state-of-the-art results on benchmarks with impressive generalization to in-the-wild data, reconstructing in 10 seconds for 16 views on a single RTX 4090.

Conclusion: VGGTFace successfully addresses limitations of existing methods by leveraging VGGT's generalization and expressive power while adding topology information for consistent mesh reconstruction.
...
TL;DR: FREE is a novel framework that accelerates Diffusion Transformers (DiTs) using feature-level autoregression with parallel verification, achieving up to 2.25× speedup while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: DiTs require long sequential denoising trajectories leading to high inference latency, and existing speculative inference methods have limited acceleration due to insufficient draft accuracy during verification.

Method: Proposes FREE framework with lightweight drafter for feature-level autoregression and parallel verification, plus FREE (relax) with uncertainty-guided relaxation that dynamically adjusts acceptance probability based on prediction variance.
...
Conclusion: FREE provides effective lossless acceleration for DiTs through feature-level speculative sampling and uncertainty-aware verification, significantly reducing inference latency without compromising generation quality.
...
TL;DR: MultiID is a training-free method for multi-ID customization that adapts single-ID models to generate images integrating multiple identities while maintaining text controllability and avoiding copy-paste issues.


<details>
  <summary>Details</summary>
Motivation: Multi-ID customization faces two major challenges: copy-paste issues from training on cropped person regions, and inferior text controllability where generated images simply combine persons without aligning with input text.

Method: Uses ID-decoupled cross-attention to inject distinct ID embeddings into corresponding image regions, plus three strategies: local prompts, depth-guided spatial control, and extended self-attention for enhanced controllability.

Result: Extensive qualitative and quantitative results show MultiID effectively solves the two challenges and performs comparably or better than training-based multi-ID customization methods.

Conclusion: MultiID provides an effective training-free solution for multi-ID customization that achieves high-quality identity integration while maintaining strong text alignment and avoiding copy-paste artifacts.
...
TL;DR: TBCM is a data-free consistency distillation method that extracts latent representations from teacher model's generation trajectory, eliminating external training data dependency while improving efficiency and performance.
...
Method: Proposes Trajectory-Backward Consistency Model (TBCM) that extracts latent representations directly from teacher model's generation trajectory, eliminating need for VAE encoding and large datasets through self-contained distillation paradigm.

Result: Achieves 6.52 FID and 28.08 CLIP scores on MJHQ-30k under one-step generation, reduces training time by ~40% compared to Sana-Sprint, saves substantial GPU memory while maintaining quality.
...
TL;DR: MajutsuCity is a natural language-driven framework for generating 3D urban scenes with structural consistency and stylistic diversity, featuring an interactive editing agent and outperforming existing methods in layout quality.


<details>
  <summary>Details</summary>
Motivation: Existing 3D city generation methods struggle to balance text-based creative flexibility with object-level editability from explicit structural representations.
...
Result: Reduces layout FID by 83.7% vs CityDreamer and 20.1% vs CityCraft; ranks first across all AQS and RDR scores, establishing new state-of-the-art in geometric fidelity and controllability.

Conclusion: MajutsuCity represents a new state-of-the-art in 3D city generation with superior geometric fidelity, stylistic adaptability, and semantic controllability, expected to inspire future research.
...
### [155] [Hidden markov model to predict tourists visited place](https://arxiv.org/abs/2511.19465)
*Theo Demessance,Chongke Bi,Sonia Djebali,Guillaume Guerard*

Main category: cs.LG

TL;DR: Using social network data to predict tourist movements in Paris through a machine learning grammatical inference algorithm adapted for big data.


<details>
  <summary>Details</summary>
Motivation: Social networks provide valuable digital traces of tourist behavior, enabling better understanding of tourist movements for tourism marketing and decision support.

Method: Machine learning grammatical inference algorithm adapted for big data context, producing a flexible and editable hidden Markov model representing tourist movements.

Result: Successfully demonstrated the method's efficiency using Paris as a case study, creating a model that can represent and predict tourist movements.

Conclusion: The proposed method effectively learns and predicts tourist movements from social network data, with the hidden Markov model providing a flexible framework for ongoing analysis.
...
### [156] [Quantifying Modality Contributions via Disentangling Multimodal Representations](https://arxiv.org/abs/2511.19470)
*Padegal Amit,Omkar Mahesh Kashyap,Namitha Rayasam,Nidhi Shekhar,Surabhi Narayan*

Main category: cs.LG

TL;DR: A framework using Partial Information Decomposition to quantify modality contributions in multimodal models, distinguishing between unique, redundant, and synergistic information without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing accuracy-based approaches conflate modality contributions by failing to distinguish between inherently informative modalities and those that only provide value through interactions with other modalities.

Method: Proposed a framework based on Partial Information Decomposition (PID) and developed an algorithm using Iterative Proportional Fitting Procedure (IPFP) for scalable, inference-only analysis of internal embeddings.

Result: The method provides a principled, representation-level view of multimodal behavior that offers clearer and more interpretable insights than outcome-based metrics.

Conclusion: The proposed framework enables better understanding of modality contributions by decomposing predictive information into unique, redundant, and synergistic components, particularly useful for cross-attention architectures.
...
### [157] [PrefixGPT: Prefix Adder Optimization by a Generative Pre-trained Transformer](https://arxiv.org/abs/2511.19472)
...
<details>
  <summary>Details</summary>
Motivation: Designing optimized prefix adders is challenging due to strict design rules and exponentially large design space, requiring automated solutions for efficient hardware design.

Method: Represent adder topology as 2D coordinate sequences, use legality masks for valid designs, employ decoder-only Transformer architecture pre-trained on random valid adders then fine-tuned for optimization.

Result: Found new optimal design with 7.7% improved ADP and superior exploration quality with up to 79.1% lower average ADP compared to existing works.

Conclusion: GPT-style models can master complex hardware design principles and apply them for efficient design optimization, demonstrating significant potential in automated hardware design.
...
### [158] [WavefrontDiffusion: Dynamic Decoding Schedule or Improved Reasoning](https://arxiv.org/abs/2511.19473)
*Haojin Yang,Rui Hu,Zequn Sun,Rui Zhou,Yujun Cai,Yiwei Wang*

Main category: cs.LG

TL;DR: WavefrontDiffusion is a dynamic decoding approach for Diffusion Language Models that expands tokens outward from finalized positions, achieving state-of-the-art performance while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard Diffusion causes premature end-of-sequence predictions by finalizing incomplete context, while BlockDiffusion's rigid structure breaks coherent semantic units and disrupts reasoning. There's a need for an adaptive denoising strategy that follows natural semantic flow.

Method: WavefrontDiffusion uses a wavefront expansion approach that dynamically expands active tokens outward from finalized positions, adapting to semantic structure while keeping computational cost equal to block-based methods.

Result: Across four benchmarks in reasoning and code generation, WavefrontDiffusion achieves state-of-the-art performance with higher semantic fidelity compared to existing denoising strategies.

Conclusion: WavefrontDiffusion demonstrates the value of adaptive scheduling for producing more coherent and efficient text generation in Diffusion Language Models.
...
### [159] [Exploiting the Experts: Unauthorized Compression in MoE-LLMs](https://arxiv.org/abs/2511.19480)
*Pinaki Prasad Guha Neogi,Ahmad Mohammadshirazi,Dheeraj Kulshrestha,Rajiv Ramnath*

Main category: cs.LG

TL;DR: MoE-LLMs are vulnerable to unauthorized compression via expert pruning and fine-tuning, enabling license bypass. The paper analyzes this threat, develops attribution methods, and proposes defense strategies.


<details>
  <summary>Details</summary>
Motivation: MoE architectures' modularity creates security vulnerabilities where adversaries can prune experts and fine-tune models to bypass licensing and security constraints.

Method: Developed expert attribution framework to identify task-critical experts, evaluated pruning effects, and used active learning-driven fine-tuning for re-alignment. Proposed defense strategies including entangled expert training.

Result: Found critical knowledge loss-recovery trade-off: task accuracy can be retained by isolating specific experts, but significant degradation occurs without targeted re-alignment.

Conclusion: MoE modularity has dual-use nature - both a threat vector and defense target. Provides first systematic framework for secure specialization of MoE-LLMs with proposed defense mechanisms.
...
### [160] [Quality analysis and evaluation prediction of RAG retrieval based on machine learning algorithms](https://arxiv.org/abs/2511.19481)
*Ruoxin Zhang,Zhizhao Wen,Chao Wang,Chenchen Tang,Puyang Xu,Yifan Jiang*

Main category: cs.LG

TL;DR: Proposes an XGBoost regression model with feature engineering and PSO optimization to improve RAG system performance by enhancing retrieval quality, showing superior metrics over other models.


<details>
  <summary>Details</summary>
Motivation: RAG systems depend heavily on retrieval quality; poor relevance or noisy information leads to distorted generated content. Existing models struggle with tabular feature processing.

Method: XGBoost machine learning regression model using feature engineering and particle swarm optimization (PSO), compared against decision trees and AdaBoost.

Result: VMD PSO BiLSTM model outperforms others with lower MSE, RMSE, MAE, MAPE and higher R2. Document relevance (0.66 correlation) significantly improves answer quality; semantic similarity and redundancy negatively affect diversity (-0.89, -0.88).

Conclusion: The model effectively optimizes retrieval quality and enhances RAG generation, providing valuable implementation and application pathways for related technologies.
...
### [161] [OmniTFT: Omni Target Forecasting for Vital Signs and Laboratory Result Trajectories in Multi Center ICU Data](https://arxiv.org/abs/2511.19485)
*Wanzhe Xu,Yutong Dai,Yitao Yang,Martin Loza,Weihang Zhang,Yang Cui,Xin Zeng,Sung Joon Park,Kenta Nakai*

Main category: cs.LG

TL;DR: OmniTFT is a deep learning framework that jointly forecasts vital signs and laboratory results in ICUs using Temporal Fusion Transformer with four novel strategies for handling noisy data, missing values, and measurement lags.


<details>
  <summary>Details</summary>
Motivation: Accurate multivariate time-series prediction in ICUs is crucial for early intervention, but vital signs are noisy with rapid fluctuations and lab tests have missing values, measurement lags, and device-specific bias, making integrative forecasting challenging.

Method: Proposes OmniTFT based on Temporal Fusion Transformer with four strategies: sliding window equalized sampling, frequency-aware embedding shrinkage, hierarchical variable selection, and influence-aligned attention calibration to handle heterogeneous clinical data.

Result: OmniTFT achieves substantial performance improvement for both vital signs and laboratory results on MIMIC-III, MIMIC-IV, and eICU datasets, with interpretable attention patterns consistent with known pathophysiology.

Conclusion: OmniTFT enables unified modeling of multiple heterogeneous clinical targets while preserving cross-institutional generalizability, showing potential utility for quantitative decision support in clinical care.
...
### [162] [Efficient Inference Using Large Language Models with Limited Human Data: Fine-Tuning then Rectification](https://arxiv.org/abs/2511.19486)
*Lei Wang,Zikun Ye,Jinglong Zhao*

Main category: cs.LG

TL;DR: A framework combining fine-tuning and rectification of LLMs for market research, with optimal sample allocation between stages to minimize prediction error variance.


<details>
  <summary>Details</summary>
Motivation: To improve LLM performance in market research by combining fine-tuning and rectification approaches, addressing the limitation of using either method alone.

Method: Developed a framework that combines fine-tuning (to align LLM with human responses) and rectification (to correct biases), with optimal sample allocation between stages using variance minimization as the fine-tuning objective.

Result: Empirical analysis shows improved estimation and inference performance compared to using fine-tuning or rectification alone.

Conclusion: The proposed framework effectively combines fine-tuning and rectification with optimal sample allocation, enhancing LLM performance in market research applications.
...
### [163] [The Generalized Proximity Forest](https://arxiv.org/abs/2511.19487)
*Ben Shaw,Adam Rustad,Sofia Pelagalli Maia,Jake S. Rhodes,Kevin R. Moon*

Main category: cs.LG

TL;DR: The paper introduces a generalized Proximity Forest (PF) model that extends Random Forest proximities to all supervised distance-based machine learning contexts, including regression tasks and meta-learning for imputation.


<details>
  <summary>Details</summary>
Motivation: Random Forest proximities are useful for various tasks but depend on RF model success, which isn't ideal in all contexts. The paper aims to extend RF proximities to broader applications through a generalized approach.

Method: Developed a generalized PF model that extends RF proximities to all supervised distance-based machine learning, introduced PF variant for regression, and used PF as meta-learning framework for supervised imputation with pre-trained classifiers.

Result: Experimental results show unique advantages of the generalized PF model compared to both Random Forest and k-nearest neighbors models.

Conclusion: The generalized PF model successfully extends RF proximities to broader contexts, providing a flexible framework for distance-based supervised learning with demonstrated performance benefits.
...
### [164] [Generative Model-Aided Continual Learning for CSI Feedback in FDD mMIMO-OFDM Systems](https://arxiv.org/abs/2511.19490)
*Guijun Liu,Yuwen Cao,Tomoaki Ohtsuki,Jiguang He,Shahid Mumtaz*

Main category: cs.LG

TL;DR: Proposes a GAN-based continual learning approach for CSI feedback in mMIMO-OFDM systems to address catastrophic forgetting and adapt to dynamic environments without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing CSI feedback models struggle with user mobility and dynamic environments, requiring retraining for new distributions and suffering from catastrophic forgetting when returning to previous environments.

Method: Uses a GAN generator as a memory unit to preserve knowledge from past environments, enabling continual learning while maintaining performance across diverse scenarios.

Result: Simulation results show enhanced generalization capability of DAE framework with low memory overhead, and seamless integration with other advanced CSI feedback models.

Conclusion: The proposed GAN-based approach provides robust and adaptable CSI feedback solution that prevents catastrophic forgetting and maintains consistent performance across changing environments.
...
TL;DR: A novel open-world machine learning model that discovers unknown classes and performs class incremental learning, enabling continual learning and outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: Conventional machine learning models follow closed-world assumptions, hindering their ability to retain previously learned knowledge for future tasks, while automated intelligence systems need to learn about novel classes and previously known tasks.

Method: The model consists of two connected tasks: discovering unknown classes in data and creating novel classes, then performing class incremental learning for each new class, enabling continual learning.

Result: The model outperformed existing approaches in open-world learning and demonstrated strong performance in continuous learning, achieving highest average accuracy of 82.54% over four iterations and minimum accuracy of 65.87%.

Conclusion: The proposed model enables continual learning by expanding understanding of data over time through novel class discovery and incremental learning, showing superior performance in open-world learning scenarios.
...
### [166] [RFX: High-Performance Random Forests with GPU Acceleration and QLORA Compression](https://arxiv.org/abs/2511.19493)
*Chris Kuchar*

Main category: cs.LG

TL;DR: RFX v1.0 is a production-ready Python implementation of Random Forests that solves the proximity matrix memory bottleneck using QLORA compression and TriBlock storage, enabling analysis of datasets up to 200,000+ samples.


<details>
  <summary>Details</summary>
Motivation: Traditional Random Forest implementations are limited by the memory requirements of proximity matrices, restricting analysis to about 60,000 samples. RFX addresses this bottleneck to enable proximity-based analysis on much larger datasets.

Method: Four key solutions: (1) QLORA compression for GPU proximity matrices (12,500x memory reduction), (2) CPU TriBlock storage with upper-triangle and block-sparse thresholding, (3) SM-aware GPU batch sizing, and (4) GPU-accelerated 3D MDS visualization from low-rank factors.

Result: QLORA achieves 12,500x compression (80GB to 6.4MB for 100k samples) with 99% geometric structure preservation. CPU TriBlock provides 2.7x memory reduction losslessly. GPU achieves 1.4x speedup over CPU. Scales from 1,000 to 200,000+ samples.

Conclusion: RFX v1.0 eliminates the proximity memory bottleneck, enabling proximity-based Random Forest analysis on datasets orders of magnitude larger than previously possible, while maintaining production-ready classification following Breiman and Cutler's original methodology.
...
### [167] [A Systematic Study of Compression Ordering for Large Language Models](https://arxiv.org/abs/2511.19495)
*Shivansh Chhawri,Rahul Mahadik,Suparna Rooj*

Main category: cs.LG

TL;DR: Systematic analysis of LLM compression techniques (knowledge distillation, pruning, quantization) shows that sequence ordering significantly impacts final model quality, with P-KD-Q (Pruning-Knowledge Distillation-Quantization) achieving optimal 3.68x compression while preserving performance.


<details>
  <summary>Details</summary>
Motivation: LLMs require substantial computational resources, making compression essential for efficient deployment, but the interactions and optimal sequencing of compression techniques remain unclear.

Method: Systematically examined compression techniques on Qwen2.5 3B model, evaluating single and multi-technique sequences using perplexity, G-Eval, clarity, prompt alignment, and compression ratio metrics.

Result: Quantization provides greatest standalone compression, pruning introduces moderate quality degradation. P-KD-Q sequence yields best balance with 3.68x compression ratio while preserving instruction-following and language understanding capabilities.

Conclusion: Ordering of compression techniques significantly affects final model quality, with P-KD-Q being optimal. Early quantization causes severe performance degradation due to irreversible information loss.
...
### [168] [Prompt Fairness: Sub-group Disparities in LLMs](https://arxiv.org/abs/2511.19956)
*Meiyu Zhong,Noel Teku,Ravi Tandon*

Main category: cs.LG

TL;DR: This paper investigates prompt fairness in LLMs, showing that different prompt phrasings from various user subgroups can lead to biased responses. The authors propose information-theoretic metrics to measure bias and practical interventions to mitigate disparities.


<details>
  <summary>Details</summary>
Motivation: LLMs exhibit significant response quality variations based on prompt phrasing from different user subgroups, creating fairness issues where similar questions asked in different styles elicit different responses.

Method: Proposed information-theoretic metrics to capture subgroup sensitivity and cross-group consistency, along with practical interventions including majority voting across multiple generations and prompt neutralization.

Result: Empirical analysis revealed structural inequities with certain demographic subgroups experiencing higher internal variability and greater divergence from others. Cross-group divergence values reached 0.28 before mitigation, typically ranging from 0.14 to 0.22.

Conclusion: The proposed neutralization and multi-generation strategy successfully reduced disparities, with largest gap reduced to 0.22 and many distances falling to 0.17 or below, indicating more stable and consistent outputs across subgroups.
...
### [169] [Xmodel-2.5: 1.3B Data-Efficient Reasoning SLM](https://arxiv.org/abs/2511.19496)
*Yang Liu,Xiaolong Zhong,Ling Jiang*

Main category: cs.LG

TL;DR: Xmodel-2.5 is a 1.3B parameter small language model designed as a drop-in agent core for edge deployments, using μP training and a novel optimizer switching strategy from AdamW to Muon.


<details>
  <summary>Details</summary>
Motivation: Large language models have strong reasoning capabilities but are computationally expensive for edge or cost-sensitive deployments, creating a need for efficient small models.

Method: Used maximal-update parameterization (μP) for hyperparameter transfer, 1.4T-token curriculum training (Warmup-Stable-Decay), and optimizer switching from AdamW to Muon during decay phase with FP8-mixed-precision.

Result: Switching from AdamW to Muon during decay improved 13-task reasoning average by 4.58% while keeping other hyperparameters fixed, demonstrating better downstream performance.

Conclusion: Early AdamW stability paired with late Muon sharpening provides better performance, and the model is released as open source with training recipes and evaluation code.
...
TL;DR: PeriodNet introduces a novel Transformer-based architecture with period attention mechanisms for time series forecasting, achieving significant performance improvements over existing models.


<details>
  <summary>Details</summary>
Motivation: Attention mechanisms have shown great success in NLP but underperform in time series forecasting. There's a need for better network structures specifically designed for TSF tasks.

Method: Uses period attention and sparse period attention to analyze adjacent periods, iterative grouping for cross-variable redundancy reduction, and a period diffuser for multi-period prediction. Redesigns the entire Transformer architecture.

Result: Outperforms six state-of-the-art models on eight datasets in both univariate and multivariate TSF, achieving 22% relative improvement for 720-length time series forecasting compared to conventional encoder-decoder Transformers.

Conclusion: PeriodNet demonstrates that specialized attention mechanisms and architectural redesigns can significantly improve time series forecasting performance, bridging the gap between attention's success in NLP and its underperformance in TSF.
...
### [171] [Hierarchical Dual-Strategy Unlearning for Biomedical and Healthcare Intelligence Using Imperfect and Privacy-Sensitive Medical Data](https://arxiv.org/abs/2511.19498)
*Yi Zhang,Tianxiang Xu,Zijian Li,Chao Zhang,Kunyu Zhang,Zhan Gao,Meinuo Li,Xiaohan Zhang,Qichao Qi,Bing Chen*

Main category: cs.LG

TL;DR: A hierarchical dual-strategy framework for selective knowledge unlearning in medical LLMs that removes specialized knowledge while preserving fundamental medical competencies, achieving high forgetting rates with minimal parameter modifications.


<details>
  <summary>Details</summary>
Motivation: Address privacy risks in healthcare LLMs due to training data memorization of imperfect or privacy-sensitive patient information, while maintaining regulatory compliance and ethical standards.

Method: Hierarchical dual-strategy framework combining geometric-constrained gradient updates and concept-aware token-level interventions using a four-level medical concept hierarchy to distinguish preservation-critical from unlearning-targeted tokens.

Result: Achieved 82.7% forgetting rate and 88.5% knowledge preservation on MedMCQA and MHQA datasets, with robust privacy guarantees while modifying only 0.1% of parameters.

Conclusion: The framework effectively addresses critical needs for regulatory compliance, auditability, and ethical standards in clinical research by enabling precise knowledge removal while preserving essential medical competencies.
...
### [172] [Beyond Binary Classification: A Semi-supervised Approach to Generalized AI-generated Image Detection](https://arxiv.org/abs/2511.19499)
*Hong-Hanh Nguyen-Le,Van-Tuan Tran,Dinh-Thuc Nguyen,Nhien-An Le-Khac*

Main category: cs.LG

TL;DR: TriDetect is a semi-supervised detector that improves cross-generator generalization by learning latent architectural patterns in synthetic images, addressing the failure of current detectors to generalize across different generator architectures like GANs and Diffusion Models.


<details>
  <summary>Details</summary>
Motivation: Current synthetic image detectors fail to generalize across different generator architectures (GANs vs Diffusion Models) due to fundamental differences in artifacts produced by distinct architectures, creating a critical vulnerability in digital media forensics.

Method: Proposed TriDetect - a semi-supervised approach that enhances binary classification by discovering latent architectural patterns within fake images using balanced cluster assignment via Sinkhorn-Knopp algorithm and cross-view consistency mechanism.

Result: TriDetect demonstrates superior generalization capability to unseen generators compared to 13 baselines, evaluated on two standard benchmarks and three in-the-wild datasets.

Conclusion: The proposed TriDetect effectively addresses cross-architecture generalization in synthetic image detection by learning fundamental architectural distinctions between different generator types.
...
### [173] [Position: The Complexity of Perfect AI Alignment -- Formalizing the RLHF Trilemma](https://arxiv.org/abs/2511.19504)
*Subramanyam Sahoo,Aman Chadha,Vinija Jain,Divya Chaudhary*

Main category: cs.LG

TL;DR: The paper formalizes the Alignment Trilemma in RLHF, proving that no system can simultaneously achieve representativeness, tractability, and robustness, with current implementations sacrificing representativeness.


<details>
  <summary>Details</summary>
Motivation: To explain the persistent tension in RLHF where improving safety reduces fairness, scaling becomes intractable, and robustness amplifies majority biases.

Method: Complexity-theoretic analysis integrating statistical learning theory and robust optimization to prove the trilemma mathematically.

Result: Achieving both representativeness and robustness requires super-polynomial operations (Ω(2^{d_context})), while current RLHF uses only 10^3-10^4 samples vs needed 10^7-10^8 for global representation.

Conclusion: The framework explains RLHF pathologies and provides directions for navigating trade-offs through strategic relaxation of alignment requirements.
...
### [174] [Profile Generators: A Link between the Narrative and the Binary Matrix Representation](https://arxiv.org/abs/2511.19506)
*Raoul H. Kutil,Georg Zimmermann,Barbara Strasser-Kirchweger,Christian Borgelt*

Main category: cs.LG

TL;DR: Developed a symptom profile generator format as an alternative to binary matrices for representing complex cognitive disorders, enabling automated generation of valid symptom combinations and calculation of similarity metrics between disorders.


<details>
  <summary>Details</summary>
Motivation: Existing machine-actionable representations of DSM-5 cognitive disorders are insufficient for complex cases, and generating full binary matrices for similarity calculations is computationally infeasible due to the vast number of symptom combinations.

Method: Created a symptom profile generator format using lists, sets, and numbers to represent diagnostic pathways, enabling automated generation of valid symptom combinations and development of profile reduction methods for calculating maximum pairwise cosine similarity between disorders.

Result: Successfully represented several psychotic disorders in generator form, showing that binary matrix representations become unmanageably large for complex disorders, and developed methods to calculate specific similarity metrics through conditional generators.

Conclusion: The symptom profile generator provides a readable, adaptable, and comprehensive alternative to binary matrices for complex cognitive disorders, enabling practical similarity analysis that was previously computationally infeasible.
...
### [175] [TouchFormer: A Robust Transformer-based Framework for Multimodal Material Perception](https://arxiv.org/abs/2511.19509)
*Kailin Lyu,Long Xiao,Jianing Zeng,Junhao Dong,Xuexin Liu,Zhuojun Zou,Haoyue Yang,Lin Shu,Jie Hao*

Main category: cs.LG

TL;DR: TouchFormer is a robust multimodal fusion framework that addresses challenges in non-visual material perception, achieving significant accuracy improvements on benchmark tasks through adaptive feature integration and cross-instance regularization.


<details>
  <summary>Details</summary>
Motivation: Traditional vision-based methods degrade under visually impaired conditions, and existing multimodal approaches perform naive fusion without addressing modality-specific noise, missing modalities, and dynamic modality importance.

Method: Uses Modality-Adaptive Gating (MAG) mechanism with intra- and inter-modality attention for adaptive cross-modal feature integration, plus Cross-Instance Embedding Regularization (CER) for fine-grained material recognition.

Result: Achieves 2.48% and 6.83% accuracy improvements on SSMC and USMC tasks respectively compared to existing non-visual methods, with real-world robotic experiments validating effectiveness.

Conclusion: TouchFormer enables better environmental perception for robots, paving way for deployment in safety-critical applications like emergency response and industrial automation.
...
### [176] [Row-stochastic matrices can provably outperform doubly stochastic matrices in decentralized learning](https://arxiv.org/abs/2511.19513)
*Bing Liu,Boao Kong,Limin Lu,Kun Yuan,Chengcheng Zhao*

Main category: cs.LG

TL;DR: This paper analyzes two decentralized learning strategies with heterogeneous node weights: embedding weights into local losses (doubly stochastic matrix) vs keeping original losses with row-stochastic matrix. Using weighted Hilbert-space analysis, it shows row-stochastic converges faster due to self-adjoint properties and provides topology design guidelines.


<details>
  <summary>Details</summary>
Motivation: To understand fundamental differences between two natural strategies for handling heterogeneous node weights in decentralized learning, as prior work showed both yield same expected descent direction but convergence behaviors remain unclear.

Method: Developed a weighted Hilbert-space framework L²(λ;ℝᵈ) to obtain tighter convergence rates than Euclidean analysis. Used Rayleigh-quotient and Loewner-order eigenvalue comparison to analyze spectral properties and derive topology conditions.

Result: Row-stochastic matrix becomes self-adjoint in this geometry while doubly stochastic does not, creating penalty terms that amplify consensus error and slow convergence. Row-stochastic can converge faster even with smaller spectral gap under certain conditions.

Conclusion: Convergence differences arise from both spectral gaps and penalty terms. Derived sufficient conditions and topology design guidelines where row-stochastic strategy has advantage over doubly stochastic approach.
...
### [177] [Automating Deception: Scalable Multi-Turn LLM Jailbreaks](https://arxiv.org/abs/2511.19517)
*Adarsh Kumarappan,Ananya Mujoo*

Main category: cs.LG

TL;DR: Automated pipeline generates large-scale multi-turn jailbreak datasets using Foot-in-the-Door psychological techniques, revealing GPT models are vulnerable to conversational context while Gemini shows strong resistance.
...
Method: Systematically operationalize FITD techniques into reproducible templates, creating 1,500 scenarios across illegal activities and offensive content. Evaluate 7 models from 3 LLM families under multi-turn and single-turn conditions.
...
Conclusion: Critical divergence in how current safety architectures handle conversational context, highlighting need for defenses that resist narrative-based manipulation.
...
### [178] [Shortcut Invariance: Targeted Jacobian Regularization in Disentangled Latent Space](https://arxiv.org/abs/2511.19525)
*Shivam Pal,Sakshi Varshney,Piyush Rai*

Main category: cs.LG

TL;DR: A method to make classifiers functionally invariant to shortcut features by injecting targeted anisotropic noise in a disentangled latent space, achieving state-of-the-art OOD performance.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks often learn spurious correlations (shortcuts) that fail in out-of-distribution scenarios. Existing robust representation methods are complex and brittle, so the authors propose learning robust functions instead.

Method: Use a disentangled latent space to isolate spurious and core features, identify shortcut features by their strong label correlation, then inject targeted anisotropic noise during training to desensitize the classifier to these features.

Result: Achieves state-of-the-art out-of-distribution performance on established shortcut learning benchmarks.

Conclusion: Learning robust functions through targeted noise injection in disentangled latent spaces is an effective alternative to complex robust representation methods for preventing shortcut learning.
...
### [179] [Learning to Solve Weighted Maximum Satisfiability with a Co-Training Architecture](https://arxiv.org/abs/2511.19544)
*Kaidi Wan,Minghao Liu,Yong Lai*

Main category: cs.LG

TL;DR: SplitGNN is a GNN-based approach for solving weighted MaxSAT problems using a co-training architecture with supervised message passing and unsupervised solution boosting, achieving faster convergence and better performance than other methods.


<details>
  <summary>Details</summary>
Motivation: To develop a more effective approach for solving challenging weighted maximum satisfiability (MaxSAT) problems by leveraging graph neural networks to capture structural information and improve solution quality.

Method: Proposes SplitGNN with co-training architecture combining supervised message passing and unsupervised solution boosting layer. Introduces edge-splitting factor graph representation based on spanning tree generation and edge classification. Implements GPU-accelerated layer with efficient score calculation and relaxation-based optimization.

Result: Achieves 3x faster convergence and better predictions compared to other GNN-based architectures. Outperforms modern heuristic MaxSAT solvers on larger and harder weighted benchmarks. Demonstrates exceptional generalization abilities on diverse structural instances.

Conclusion: SplitGNN successfully bridges the gap between neural approaches and traditional solvers for MaxSAT problems, showing superior performance on challenging weighted instances while maintaining strong generalization capabilities.
...
### [180] [When Should Neural Data Inform Welfare? A Critical Framework for Policy Uses of Neuroeconomics](https://arxiv.org/abs/2511.19548)
*Yiven,Zhu*

Main category: cs.LG

TL;DR: This paper develops a framework for determining when neural data can legitimately inform welfare judgments for policy, rather than just describing behavior, using a neuroeconomic approach.


<details>
  <summary>Details</summary>
Motivation: To address the increasing use of neural data by policy and commercial actors to justify paternalistic regulation and interventions, and to establish conditions under which such data can properly inform welfare analysis.

Method: Develops a non-empirical, model-based framework linking neural signals, computational decision models, and normative welfare criteria using an actor-critic reinforcement-learning model to formalize the inference path from neural activity to welfare claims.

Result: Shows that neural evidence constrains welfare judgments only when: neural-computational mapping is well validated, decision model identifies 'true' interests vs context-dependent mistakes, and welfare criterion is explicitly specified and defended. Creates a Neuroeconomic Welfare Inference Checklist.

Conclusion: Neural reward signals (biological or artificial) are computational quantities that cannot be treated as welfare measures without an explicit normative model, requiring careful validation of the inference path from neural data to welfare claims.
...
### [181] [Online Sparse Feature Selection in Data Streams via Differential Evolution](https://arxiv.org/abs/2511.19555)
*Ruiyang Xu*

Main category: cs.LG

TL;DR: ODESFS is a novel online feature selection method that improves upon existing approaches by combining latent factor analysis for missing data imputation with differential evolution for feature evaluation, achieving superior performance on real-world datasets.


<details>
  <summary>Details</summary>
Motivation: Existing online streaming feature selection methods struggle with data incompleteness and have limitations in feature evaluation, leading to performance deterioration in practical applications.

Method: ODESFS uses two key innovations: (1) missing value imputation via latent factor analysis model, and (2) feature importance evaluation through differential evolution optimization.

Result: Comprehensive experiments on six real-world datasets show ODESFS consistently outperforms state-of-the-art OSFS and OS2FS methods by selecting optimal feature subsets and achieving superior accuracy.

Conclusion: The proposed ODESFS method effectively addresses limitations in existing online sparse streaming feature selection approaches and demonstrates significant performance improvements.
...
### [182] [Merging without Forgetting: Continual Fusion of Task-Specific Models via Optimal Transport](https://arxiv.org/abs/2511.19561)
...
TL;DR: OTMF is a novel model merging framework that uses optimal transport theory to align semantic geometry between task-specific models, avoiding distribution shift issues from naive parameter interpolation.


<details>
  <summary>Details</summary>
Motivation: Existing model merging approaches using parameter interpolation cause significant distribution shift in feature space and undermine task-specific knowledge, limiting multi-task system performance.

Method: OTMF discovers common masks applied to task vectors through optimal transport plans, selectively extracting transferable components while preserving unique task identities. It supports continual fusion for scalable integration of new tasks.
...
Conclusion: OTMF provides a practical and theoretically grounded approach to model merging that effectively addresses distribution shift issues and enables scalable multi-task systems.
...
### [183] [ModHiFi: Identifying High Fidelity predictive components for Model Modification](https://arxiv.org/abs/2511.19566)
*Dhruva Kashyap,Chaitanya Murti,Pranav K Nayak,Tanay Narshana,Chiranjib Bhattacharyya*

Main category: cs.LG

TL;DR: Proposes ModHiFi, a method for model modification (pruning/unlearning) without needing training data, loss functions, gradients, or ground-truth labels, using only distributional access like synthetic data.


<details>
  <summary>Details</summary>
Motivation: Open weight models rarely provide training data or loss functions, making modifications like pruning/unlearning challenging. Existing methods require gradients or labels, which are infeasible in resource-constrained settings.

Method: Uses Subset Fidelity metric based on local reconstruction errors to quantify component importance. ModHiFi algorithm selects components via their Subset Fidelity scores without data/loss access.
...
Conclusion: Demonstrates that model modification is possible without training data or loss functions using only distributional access, with theoretical guarantees for Lipschitz-continuous networks.
...
### [184] [An Invariant Latent Space Perspective on Language Model Inversion](https://arxiv.org/abs/2511.19569)
*Wentao Ye,Jiaqi Hu,Haobo Wang,Xinpeng Ti,Zhiqing Xiao,Hao Chen,Liyao Li,Lei Feng,Sai Wu,Junbo Zhao*

Main category: cs.LG

TL;DR: Inv^2A is a language model inversion method that treats LLMs as invariant decoders and learns lightweight inverse encoders to recover hidden prompts from outputs, outperforming baselines by 4.77% BLEU score while reducing dependence on large inverse corpora.


<details>
  <summary>Details</summary>
Motivation: Language model inversion (LMI) poses a concrete threat to user privacy and system security by enabling recovery of hidden prompts from model outputs.

Method: Proposes Inv^2A based on Invariant Latent Space Hypothesis: treats LLM as invariant decoder, learns lightweight inverse encoder, uses sparse concatenation of multiple outputs, and employs two-stage training with contrastive alignment and supervised reinforcement.

Result: Outperforms baselines by average 4.77% BLEU score across 9 datasets covering user and system prompt scenarios, while reducing dependence on large inverse corpora.

Conclusion: Prevalent defenses provide limited protection against LMI attacks, underscoring the need for stronger security strategies to protect user privacy and system security.
...
### [185] [Neural Tractability via Structure: Learning-Augmented Algorithms for Graph Combinatorial Optimization](https://arxiv.org/abs/2511.19573)
*Jialiang Li,Weitong Chen,Mingyu Guo*

Main category: cs.LG

TL;DR: A framework combining neural models with parameterized algorithms for graph combinatorial optimization, using neural models for advisory signals on hard parts and parameterized search for easy parts, achieving superior solution quality and better generalization.


<details>
  <summary>Details</summary>
Motivation: Neural models offer fast inference but lack optimality guarantees, while classical search algorithms provide quality guarantees but are slower. The goal is to combine the strengths of both approaches.

Method: Use parameterized algorithms to identify structurally easy parts of instances, neural models to generate advisory signals for hard parts, and integrate both through systematic search.

Result: Achieves superior solution quality competitive with commercial solvers and improved out-of-distribution generalization compared to neural-only approaches.

Conclusion: The proposed framework successfully combines neural inference efficiency with search-based optimality guarantees, producing strictly better solutions than neural solvers alone while maintaining good generalization.
...
### [186] [Learning Massively Multitask World Models for Continuous Control](https://arxiv.org/abs/2511.19584)
*Nicklas Hansen,Hao Su,Xiaolong Wang*

Main category: cs.LG

TL;DR: Newt is a language-conditioned multitask world model trained on 200 diverse tasks using pretraining on demonstrations followed by online reinforcement learning, achieving superior multitask performance and rapid adaptation to unseen tasks.


<details>
  <summary>Details</summary>
Motivation: To address the gap in general-purpose control where most RL research focuses on single-task or offline regimes, and to explore whether a single agent can be trained on hundreds of tasks with online interaction.

Method: Pretrain a language-conditioned multitask world model on demonstrations to acquire task-aware representations and action priors, then jointly optimize with online interaction across all 200 tasks.

Result: Newt yields better multitask performance and data-efficiency than strong baselines, exhibits strong open-loop control, and enables rapid adaptation to unseen tasks.

Conclusion: The foundation model recipe (large-scale pretraining followed by light RL) can successfully scale to hundreds of tasks, demonstrating that online RL can indeed scale for general-purpose control.
...
### [187] [Many Ways to be Right: Rashomon Sets for Concept-Based Neural Networks](https://arxiv.org/abs/2511.19636)
*Shihan Feng,Cheng Zhang,Michael Xi,Ethan Hsu,Lesia Semenova,Chudi Zhong*

Main category: cs.LG

TL;DR: Rashomon Concept Bottleneck Models learn multiple accurate neural networks that use distinct human-understandable concepts for reasoning, revealing diverse decision-making patterns in equally performing models.


<details>
  <summary>Details</summary>
Motivation: To uncover the diversity in deep architectures where multiple models can achieve identical performance but rely on different features or reasoning patterns (Rashomon Effect), which is challenging due to countless numerically distinct but behaviorally similar near-optimal solutions.

Method: Combines lightweight adapter modules with a diversity-regularized training objective to construct multiple deep concept-based models efficiently without retraining from scratch.

Result: Produces networks that provide fundamentally different reasoning processes for the same predictions, revealing how concept reliance and decision making vary across equally performing solutions.

Conclusion: Enables systematic exploration of data-driven reasoning diversity in deep models, offering a new mechanism for auditing, comparison, and alignment across equally accurate solutions.
...
### [188] [Lower Complexity Bounds for Nonconvex-Strongly-Convex Bilevel Optimization with First-Order Oracles](https://arxiv.org/abs/2511.19656)
*Kaiyi Ji*

Main category: cs.LG

TL;DR: This paper establishes new lower bounds for bilevel optimization in the smooth nonconvex-strongly-convex setting, showing that existing upper bounds may be suboptimal and revealing significant complexity gaps.


<details>
  <summary>Details</summary>
Motivation: Progress on lower bounds for bilevel optimization has been limited due to the complexity of the bilevel structure, despite extensive study of upper bound guarantees. The authors aim to develop nontrivial lower bounds to better understand the fundamental complexity of bilevel optimization.

Method: The authors develop new hard instances for bilevel optimization and analyze them under deterministic and stochastic first-order oracle models. They focus on the smooth nonconvex-strongly-convex setting and use zero-respecting algorithms as their computational model.

Result: In the deterministic case: Ω(κ^{3/2}ε^{-2}) oracle calls are required. In the stochastic case: Ω(κ^{5/2}ε^{-4}) stochastic oracle calls are necessary. These bounds improve upon known lower bounds for single-level nonconvex optimization and nonconvex-strongly-convex min-max problems.

Conclusion: The results reveal substantial gaps between current upper and lower bounds for bilevel optimization, suggesting that even simplified regimes with quadratic lower-level objectives require further investigation to understand the optimal complexity under standard first-order oracles.
...
TL;DR: A forecast-blur-denoise framework that improves time-series forecasting by modeling structured noise through Gaussian Processes, enabling better capture of multi-scale temporal patterns while maintaining high-resolution detail.


<details>
  <summary>Details</summary>
Motivation: Existing neural models struggle to represent multi-scale temporal patterns (from broad trends to fine-grained fluctuations), leading to unstable predictions and reduced reliability in downstream applications.

Method: A forecast-blur-denoise framework with learnable Gaussian Process module for smooth correlated perturbations, encouraging long-range structure capture, plus a refinement model to restore high-resolution temporal detail. Components are trained jointly to avoid artifacts from isotropic corruption methods.

Result: Experiments across electricity, traffic, and solar datasets show consistent gains in multi-horizon accuracy and stability. The modular design also enables lightweight enhancement of pretrained models in limited-data scenarios.

Conclusion: The framework strengthens reliability and interpretability of fine-scale temporal predictions, contributing to more trustworthy AI systems for forecasting-driven decision support in energy, infrastructure, and other time-critical domains.
...
### [190] [Demystifying Diffusion Objectives: Reweighted Losses are Better Variational Bounds](https://arxiv.org/abs/2511.19664)
*Jiaxin Shi,Michalis K. Titsias*

Main category: cs.LG

TL;DR: The paper provides a new theoretical interpretation of reweighted losses in diffusion models by constructing time-dependent variational lower bounds that improve upon standard ELBO and reduce KL-divergence.


<details>
  <summary>Details</summary>
Motivation: To develop a better theoretical understanding of reweighted losses used in diffusion model training and create improved training objectives that can be applied to both continuous and discrete diffusion models.

Method: Constructing a cascade of time-dependent variational lower bounds on data log-likelihood that provably improves upon standard evidence lower bound, resulting in reduced data-model KL-divergences.

Result: The framework significantly improves training losses in pixel-space image modeling for masked diffusion, approaching sample quality comparable to continuous diffusion models.

Conclusion: The method provides theoretical justification for simple weighting schemes used in masked image models and creates improved reweighted objectives applicable to any generative diffusion model.
...
### [191] [TREASURE: A Transformer-Based Foundation Model for High-Volume Transaction Understanding](https://arxiv.org/abs/2511.19693)
...
Method: Transformer-based model with dedicated input modules for static/dynamic attributes, efficient training paradigm for high-cardinality categorical attributes, and simultaneous capture of consumer behavior and payment network signals.
...
Conclusion: TREASURE demonstrates effectiveness as both standalone model and embedding provider, providing comprehensive transaction data representation for various applications.
...
### [192] [TiCT: A Synthetically Pre-Trained Foundation Model for Time Series Classification](https://arxiv.org/abs/2511.19694)
...
<details>
  <summary>Details</summary>
Motivation: Address the gap in general-purpose time series foundation models for classification, overcoming the high cost of labeled data through in-context learning capabilities.
...
Result: Achieves competitive performance against state-of-the-art supervised methods on UCR Archive using only in-context examples at inference, without weight updates.

Conclusion: TiCT demonstrates that effective time series classification can be achieved through in-context learning with synthetic pre-training, eliminating the need for expensive labeled data and fine-tuning.
...
### [193] [CafeQ: Calibration-free Quantization via Learned Transformations and Adaptive Rounding](https://arxiv.org/abs/2511.19705)
*Ziteng Sun,Adrian Benton,Samuel Kushnir,Asher Trockman,Vikas Singh,Suhas Diggavi,Ananda Theertha Suresh*

Main category: cs.LG

TL;DR: Proposes calibration-free post-training quantization methods using optimized transformations and adaptive rounding to handle weight outliers, achieving performance comparable to calibration-dependent methods with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing post-training quantization methods rely on calibration data, which may be unavailable or subject to privacy constraints in real-world scenarios, limiting their applicability.

Method: Develops algorithms to optimize transformations and adaptive rounding without calibration data by designing proxy functions for quantization loss. Uses structured matrix transformations for single matrices and dual matrix transformations with adaptive rounding for paired weights in computation graphs.

Result: On Gemma 2 models, improves average benchmark score from 61.9 to 62.4 for 4-bit quantization and from 52.0 to 60.6 for 3-bit quantization, with less than 3% computation overhead. Achieves performance comparable to GPTQ method that requires calibration data.

Conclusion: The proposed calibration-free quantization method effectively handles weight outliers and achieves competitive performance with minimal computational cost, making it suitable for scenarios where calibration data is unavailable.
...
### [194] [Training-Free Active Learning Framework in Materials Science with Large Language Models](https://arxiv.org/abs/2511.19730)
*Hongchen Wang,Rafael Espinosa Castañeda,Jay R. Werber,Yao Fehlis,Edward Kim,Jason Hattrick-Simpers*

Main category: cs.LG

TL;DR: LLM-based active learning framework (LLM-AL) outperforms traditional ML models, reducing experiments needed by over 70% across diverse materials science datasets through iterative few-shot learning and two prompting strategies.


<details>
  <summary>Details</summary>
Motivation: Traditional ML models in active learning suffer from cold-start limitations and domain-specific feature engineering, restricting generalizability. LLMs offer a new paradigm with pretrained knowledge and universal token-based representations.

Method: LLM-AL framework operates in iterative few-shot setting using two prompting strategies: concise numerical inputs for compositional datasets and expanded descriptive text for experimental datasets to provide additional context.

Result: LLM-AL reduces experiments needed by over 70% to reach top-performing candidates, consistently outperforms traditional ML models, performs broader exploratory searches while reaching optima with fewer iterations, and shows stable performance across runs.
...
### [195] [DISCO: A Browser-Based Privacy-Preserving Framework for Distributed Collaborative Learning](https://arxiv.org/abs/2511.19750)
...
TL;DR: DISCO is an open-source distributed collaborative learning platform that enables non-technical users to collaboratively build ML models without sharing original data or requiring programming knowledge.


<details>
  <summary>Details</summary>
Motivation: Address data sharing limitations due to privacy, IP, and legal constraints that fragment statistical power and create accessibility bias where accuracy favors those with resources to overcome these barriers.

Method: Web application trains models locally in browser (cross-platform including smartphones), offers modular design with federated/decentralized paradigms, privacy guarantees, and weight aggregation strategies for model personalization and bias resilience.

Result: Created an accessible platform that enables collaborative ML training while preserving data privacy and requiring no technical expertise from users.

Conclusion: DISCO provides a practical solution to democratize collaborative ML by eliminating data sharing requirements and technical barriers, making distributed learning accessible to non-technical users across various devices.
...
### [196] [When +1% Is Not Enough: A Paired Bootstrap Protocol for Evaluating Small Improvements](https://arxiv.org/abs/2511.19794)
*Wenzhang Du*

Main category: cs.LG

TL;DR: Proposes a conservative evaluation protocol using paired multi-seed runs, BCa bootstrap confidence intervals, and sign-flip permutation tests to prevent over-claiming small performance gains in ML benchmarks.


<details>
  <summary>Details</summary>
Motivation: Recent ML papers report 1-2% improvements that are sensitive to random seeds and implementation details, but rarely provide uncertainty estimates or significance tests, making it unclear if gains are real or noise.

Method: Uses paired multi-seed runs, bias-corrected and accelerated (BCa) bootstrap confidence intervals, and sign-flip permutation test on per-seed deltas under realistic compute budgets.

Result: On CIFAR-10, CIFAR-10N, and AG News datasets, single runs and unpaired t-tests often suggest significant gains for 0.6-2.0 point improvements, but with only three seeds, the proposed paired protocol never declares significance in these settings.

Conclusion: Conservative evaluation using the proposed protocol is a safer default for assessing small performance gains under tight compute budgets, preventing over-claiming of insignificant improvements.
...
### [197] [Terminal Velocity Matching](https://arxiv.org/abs/2511.19797)
*Linqi Zhou,Mathias Parger,Ayaan Haque,Jiaming Song*

Main category: cs.LG

TL;DR: Terminal Velocity Matching (TVM) is a flow matching generalization that enables high-fidelity one- and few-step generative modeling by modeling transitions between diffusion timesteps and regularizing terminal behavior.


<details>
  <summary>Details</summary>
Motivation: To achieve state-of-the-art performance for one/few-step generative models from scratch, overcoming limitations of flow matching and enabling efficient training with transformer architectures.

Method: TVM models transitions between diffusion timesteps with terminal time regularization, introduces minimal architectural changes for stable training, and develops fused attention kernels for efficient Jacobian-Vector Product backward passes.

Result: Achieves 3.29 FID with 1 NFE and 1.99 FID with 4 NFEs on ImageNet-256x256, and 4.32 1-NFE FID and 2.94 4-NFE FID on ImageNet-512x512.

Conclusion: TVM represents state-of-the-art performance for one/few-step generative models, enabling high-fidelity generation with minimal function evaluations.
...
### [198] [BlockCert: Certified Blockwise Extraction of Transformer Mechanisms](https://arxiv.org/abs/2511.17645)
*Sandro Andric*

Main category: cs.LG

TL;DR: BlockCert is a framework for certified blockwise extraction of transformer mechanisms with formal guarantees on approximation error, enabling certified model editing and bridging mechanistic interpretability with formal reasoning.


<details>
  <summary>Details</summary>
Motivation: Current mechanistic interpretability and model editing approaches lack formal guarantees about how extracted or edited models deviate from originals, relying on informal evidence and ad-hoc experiments.

Method: Extracts structured surrogate implementations for transformer residual blocks with machine-checkable certificates bounding approximation error, coverage metrics, and artifact hashes. Uses Lipschitz-based composition theorem to lift local guarantees to global deviation bounds.

Result: Applied to GPT-2 small, TinyLlama-1.1B-Chat, and Llama-3.2-3B, achieving high per-block coverage and small residual errors. Fully stitched TinyLlama model matches baseline perplexity within ~6e-5 on stress prompts.
...
### [199] [Scalable Data Attribution via Forward-Only Test-Time Inference](https://arxiv.org/abs/2511.19803)
*Sibo Ma,Julian Nyarko*

Main category: cs.LG

TL;DR: A new data attribution method that traces model behavior to training data without expensive inference-time backward passes, using gradient propagation during training for efficient forward-only attribution.


<details>
  <summary>Details</summary>
Motivation: Classical influence functions are impractical for modern networks due to expensive backpropagation or Hessian inversion at inference time, limiting real-time data attribution for large models.

Method: Simulates training examples' parameter influence through short-horizon gradient propagation during training, then reads out attributions using only forward evaluations at inference.

Result: Matches or surpasses state-of-the-art baselines like TRAK on attribution metrics (LOO and LDS) while offering orders-of-magnitude lower inference cost on MLP benchmarks.

Conclusion: Combines influence-function fidelity with first-order scalability, providing a practical framework for real-time data attribution in large pretrained models.
...
### [200] [Learning to Clean: Reinforcement Learning for Noisy Label Correction](https://arxiv.org/abs/2511.19808)
*Marzi Heidari,Hanping Zhang,Yuhong Guo*

Main category: cs.LG

TL;DR: RLNLC frames noisy label correction as a reinforcement learning problem, using an actor-critic method to learn a policy for iterative label correction that outperforms state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Noisy labels severely degrade machine learning model performance, requiring effective correction methods to maintain prediction accuracy.

Method: Proposes RLNLC framework with state space (data and labels), action space (label corrections), reward mechanism, and deep feature representation policy network using actor-critic reinforcement learning.

Result: Extensive experiments on benchmark datasets show RLNLC consistently outperforms existing state-of-the-art techniques for learning with noisy labels.

Conclusion: Reinforcement learning provides an effective framework for noisy label correction, with RLNLC demonstrating superior performance through iterative policy-based label correction.
...
### [201] [Provably Outlier-resistant Semi-parametric Regression for Transferable Calibration of Low-cost Air-quality Sensors](https://arxiv.org/abs/2511.19810)
...
TL;DR: RESPIRE technique for calibrating low-cost CO sensors in air quality monitoring networks, offering improved cross-site/season/sensor performance with outlier resistance and explainability.


<details>
  <summary>Details</summary>
Motivation: Calibration of low-cost air quality sensors is expensive and laborious, especially for large-scale deployments in diverse geographical areas.

Method: RESPIRE technique with training algorithm resistant to outliers and explainable model that can flag overfitting instances.

Result: Improved prediction performance in cross-site, cross-season, and cross-sensor settings compared to baseline methods.

Conclusion: RESPIRE provides an effective calibration solution for large-scale low-cost air quality sensor networks with practical advantages over existing methods.
...
### [202] [Mosaic Pruning: A Hierarchical Framework for Generalizable Pruning of Mixture-of-Experts Models](https://arxiv.org/abs/2511.19822)
*Wentao Hu,Mingkuan Zhao,Shuangyong Song,Xiaoyan Zhu,Xin Lai,Jiayin Wang*

Main category: cs.LG

TL;DR: Mosaic Pruning (MoP) addresses the generalization gap in sparse Mixture-of-Experts models by selecting functionally complementary experts across domains, outperforming single-corpus pruning methods.


<details>
  <summary>Details</summary>
Motivation: Existing SMoE pruning methods degrade performance when applied to new domains due to single-corpus optimization, requiring costly re-pruning for each domain.

Method: Uses a "cluster-then-select" process with similarity metrics across task domains to group experts functionally, then selects most representative experts using Activation Variability Score.

Result: Achieves 7.24% gain on general tasks and 8.92% on specialized tasks (math reasoning, code generation) compared to prior methods.

Conclusion: MoP enables pruned models to handle diverse downstream tasks by retaining functionally complementary experts, like mosaic tiles forming a complete capability picture.
...
### [203] [GED-Consistent Disentanglement of Aligned and Unaligned Substructures for Graph Similarity Learning](https://arxiv.org/abs/2511.19837)
*Zhentao Zhan,Xiaoliang Xu,Jingjing Wang,Junmei Wang*

Main category: cs.LG

TL;DR: GCGSim is a novel graph similarity learning framework that addresses limitations in existing GNN-based approaches by focusing on graph-level matching and substructure-level edit costs, achieving state-of-the-art performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Existing GNN-based Graph Edit Distance (GED) approaches use node-centric matching which creates a mismatch with GED principles, leading to failure in capturing global structural correspondence and misattribution of edit costs from spurious node-level signals.

Method: Proposed GCGSim framework centers on graph-level matching and substructure-level edit costs, with three core technical contributions to ensure GED-consistent graph similarity learning.

Result: Extensive experiments on four benchmark datasets show GCGSim achieves state-of-the-art performance, with comprehensive analyses validating that the framework effectively learns disentangled and semantically meaningful substructure representations.

Conclusion: GCGSim successfully addresses the limitations of node-centric matching in GED computation by focusing on graph-level alignment and substructure-level cost attribution, providing a more principled approach to graph similarity learning.
...
### [204] [Cisco Time Series Model Technical Report](https://arxiv.org/abs/2511.19841)
...
TL;DR: Cisco Time Series Model is a univariate zero-shot forecasting foundation model with multiresolution input capability, trained on 300B+ data points, achieving superior performance on observability datasets while maintaining strong general forecasting capabilities.


<details>
  <summary>Details</summary>
Motivation: To create a time series foundation model that can handle multiresolution input and excel specifically in observability domains while maintaining general forecasting performance.

Method: Applied architectural innovation for multiresolution input capability to TimesFM (decoder-only time series model), trained on 300B+ unique data points with majority from observability domain.

Result: Achieves superior performance on observability datasets while maintaining similar performance on general-purpose forecasting benchmark (GIFT-Eval); multiresolution structure enables more accurate predictions on long context input.

Conclusion: The multiresolution decoder-only architecture successfully creates a specialized yet generalizable time series foundation model that excels in observability applications while preserving broad forecasting capabilities.
...
### [205] [SX-GeoTree: Self-eXplaining Geospatial Regression Tree Incorporating the Spatial Similarity of Feature Attributions](https://arxiv.org/abs/2511.19845)
*Chaogui Kang,Lijian Luo,Qingfeng Guan,Yu Liu*

Main category: cs.LG

TL;DR: SX-GeoTree is a self-explaining geospatial regression tree that improves spatial dependence capture and explanation robustness by integrating impurity reduction, spatial residual control, and modularity-based explanation stability during recursive splitting.


<details>
  <summary>Details</summary>
Motivation: Address limitations of traditional decision trees in capturing spatial dependence and producing locally stable explanations for geospatial data.

Method: Integrates three objectives during recursive splitting: MSE impurity reduction, spatial residual control via global Moran's I, and explanation robustness via modularity maximization on consensus similarity networks from GWR coefficients and SHAP attributions.

Result: Maintains competitive predictive accuracy (within 0.01 R² of decision trees) while improving residual spatial evenness and doubling attribution consensus (modularity: Fujian 0.19 vs 0.09; Seattle 0.10 vs 0.05).

Conclusion: Demonstrates how spatial similarity extended beyond geometric proximity can be embedded in interpretable models, advancing trustworthy geospatial machine learning and offering transferable domain-aware explainability.
...
### [206] [Accelerating Wireless Distributed Learning via Hybrid Split and Federated Learning Optimization](https://arxiv.org/abs/2511.19851)
*Kun Guo,Xuefei Li,Xijun Wang,Howard H. Yang,Wei Feng,Tony Q. S. Quek*

Main category: cs.LG

TL;DR: HSFL combines split learning and federated learning to accelerate distributed training in wireless networks by optimizing learning mode selection, batch size, and resource allocation to minimize delay while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To leverage the advantages of both federated learning (low latency) and split learning (high accuracy) while overcoming their respective limitations, by developing a hybrid approach that optimizes learning performance in wireless networks.

Method: Proposed a two-stage optimization: 1) block coordinate descent for relaxed problem to find locally optimal solution, 2) rounding algorithm to recover integer batch sizes with near-optimal performance, while analyzing convergence and formulating delay minimization problem.

Result: Experimental results show the approach significantly accelerates convergence to target accuracy compared to existing methods, demonstrating effective delay reduction while maintaining learning performance.

Conclusion: The proposed HSFL framework with joint optimization of learning mode selection, batch size, and resource allocation successfully accelerates distributed learning in wireless networks by balancing latency and accuracy trade-offs.
...
### [207] [Frailty-Aware Transformer for Recurrent Survival Modeling of Driver Retention in Ride-Hailing Platforms](https://arxiv.org/abs/2511.19893)
*Shuoyan Xu,Yu Zhang,Eric J. Miller*

Main category: cs.LG

TL;DR: Proposes FACT, a Transformer-based survival analysis framework for ride-hailing driver idle behavior, outperforming existing models in risk prediction.


<details>
  <summary>Details</summary>
Motivation: Ride-hailing platforms have high-frequency, behavior-driven environments, but survival analysis for recurrent driver events remains unexplored despite its potential applications.

Method: Formulates idle behavior as recurrent survival process using large-scale data; develops FACT with Transformer architecture, causal masking for temporal dependencies, and driver-specific embeddings for heterogeneity.

Result: FACT achieves highest time-dependent C-indices and lowest Brier Scores on Toronto ride-hailing data, outperforming classical and deep learning survival models.

Conclusion: The approach enables accurate risk estimation, supports platform retention strategies, and provides policy-relevant insights for ride-hailing platforms.
...
### [208] [EfficientXpert: Efficient Domain Adaptation for Large Language Models via Propagation-Aware Pruning](https://arxiv.org/abs/2511.19935)
*Songlin Zhao,Michael Pitts,Zhuwei Qin*

Main category: cs.LG

TL;DR: EfficientXpert is a domain-pruning framework that combines propagation-aware pruning with efficient adapter updates to create sparse, domain-adapted LLMs, achieving 98% performance at 40% sparsity.


<details>
  <summary>Details</summary>
Motivation: Large language models need domain specialization but their size hinders deployment in resource-constrained environments, and existing compression methods either generalize poorly or have high overhead.

Method: Combines Foresight Mask (propagation-aware pruning criterion) with Partial Brain Surgeon (efficient adapter-update algorithm) integrated into LoRA fine-tuning for one-step transformation of pretrained models into sparse domain experts.
...
Conclusion: Domain-dependent structural shifts degrade general pruning masks, highlighting the need for adaptive, domain-aware pruning strategies tailored to each specific domain.
...
### [209] [Adaptivity and Universality: Problem-dependent Universal Regret for Online Convex Optimization](https://arxiv.org/abs/2511.19937)
*Peng Zhao,Yu-Hu Yan,Hang Yu,Zhi-Hua Zhou*

Main category: cs.LG

TL;DR: UniGrad is a universal online learning method that achieves both universality (optimal regret for different function types) and adaptivity (regret scaling with gradient variation V_T), with two variants offering different trade-offs between regret bounds and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing universal online learning methods lack problem-dependent adaptivity and cannot provide regret bounds that scale with gradient variation V_T, which is crucial for applications like stochastic optimization and fast convergence in games.

Method: Two main approaches: UniGrad.Correct and UniGrad.Bregman, both using meta algorithms with O(log T) base learners. UniGrad++ reduces computational cost to 1 gradient query per round via surrogate optimization while maintaining regret guarantees.

Result: UniGrad achieves universal regret guarantees that adapt to gradient variation: O(log V_T) for strongly convex functions, O(d log V_T) for exp-concave functions, and O(√V_T log V_T) or O(√V_T) for convex functions depending on the variant.

Conclusion: UniGrad provides the first universal online learning method with problem-dependent adaptivity to gradient variation, offering both theoretical guarantees and computational efficiency improvements through UniGrad++.
...
### [210] [Optimize Flip Angle Schedules In MR Fingerprinting Using Reinforcement Learning](https://arxiv.org/abs/2511.19941)
*Shenjun Zhong,Zhifeng Chen,Zhaolin Chen*

Main category: cs.LG

TL;DR: RL framework optimizes flip-angle schedule in MRF to enhance fingerprint separability and potentially accelerate acquisitions.


<details>
  <summary>Details</summary>
Motivation: MRF sequence design is a complex high-dimensional problem; RL can automate parameter selection to maximize fingerprint distinguishability.

Method: Reinforcement learning framework for optimizing flip-angle schedule in Magnetic Resonance Fingerprinting.

Result: Learned schedule exhibits non-periodic patterns that enhance fingerprint separability and may enable reduction in repetition time.

Conclusion: RL-optimized flip-angle schedules improve MRF performance and could accelerate acquisitions.
...
### [211] [Differential Smoothing Mitigates Sharpening and Improves LLM Reasoning](https://arxiv.org/abs/2511.19942)
*Jingchu Gai,Guanning Zeng,Huaqing Zhang,Aditi Raghunathan*

Main category: cs.LG

TL;DR: RL fine-tuning of LLMs causes diversity collapse. The paper introduces differential smoothing, a principled method that improves both correctness and diversity, outperforming vanilla RL and entropy-based heuristics.


<details>
  <summary>Details</summary>
Motivation: Prior heuristic methods for addressing diversity collapse in RL fine-tuning are ad hoc, trade correctness for diversity, and have inconsistent effectiveness across tasks.

Method: Differential smoothing - a principled reward modification method that only needs to be applied on correct trajectories, based on formal analysis of selection and reinforcement bias.

Result: Differential smoothing consistently improves both Pass@1 and Pass@k metrics across models (1B-7B parameters) and domains including CountDown and mathematical reasoning, with up to 6.7% improvements on AIME24 dataset.

Conclusion: Differential smoothing is universally superior to existing heuristics, provably improving both correctness and diversity while providing theoretical foundation for when existing methods help or fail.
...
### [212] [Hierarchical Spatio-Temporal Attention Network with Adaptive Risk-Aware Decision for Forward Collision Warning in Complex Scenarios](https://arxiv.org/abs/2511.19952)
*Haoran Hu,Junren Shi,Shuo Jiang,Kun Cheng,Xia Yang,Changhao Piao*

Main category: cs.LG

TL;DR: Integrated FCW framework combining Hierarchical Spatio-Temporal Attention Network and Dynamic Risk Threshold Adjustment for real-time collision warning with improved accuracy and reliability.


<details>
  <summary>Details</summary>
Motivation: Current FCW systems struggle with balancing precise multi-agent interaction modeling and real-time decision adaptability, suffering from high computational costs and unreliable simplified interaction models with high false alarm rates.

Method: HSTAN with decoupled architecture (Graph Attention Network for spatial, cascaded GRU with self-attention for temporal) paired with Dynamic Risk Threshold Adjustment using conformalized quantile regression and physics-informed risk potential function.

Result: 12.3 ms inference time (73% faster than Transformer), ADE of 0.73m (42.2% better than Social_LSTM), 91.3% coverage at 90% confidence, F1 score of 0.912, 8.2% false alarm rate, and 2.8s warning lead time.

Conclusion: The framework demonstrates superior performance and practical deployment feasibility in complex environments, effectively addressing computational complexity and modeling insufficiency challenges.
...
### [213] [ParaBlock: Communication-Computation Parallel Block Coordinate Federated Learning for Large Language Models](https://arxiv.org/abs/2511.19959)
*Yujia Wang,Yuanpu Cao,Jinghui Chen*

Main category: cs.LG

TL;DR: ParaBlock: A parallel communication-computation approach for efficient federated fine-tuning of large language models that maintains convergence while reducing communication latency.


<details>
  <summary>Details</summary>
Motivation: Federated block coordinate descent faces high communication costs in LLM fine-tuning, as even single blocks contain many parameters, creating latency issues for resource-constrained clients.

Method: Proposes ParaBlock with two parallel threads for communication and computation to overlap operations and enhance efficiency while maintaining the same convergence properties.

Result: Theoretical proof shows same convergence rate as standard methods. Empirical evaluations on LLM fine-tuning for instruction following and mathematical reasoning confirm strong performance with significantly improved communication efficiency.

Conclusion: ParaBlock effectively addresses communication bottlenecks in federated LLM fine-tuning while maintaining model performance and convergence guarantees.
...
### [214] [Stragglers Can Contribute More: Uncertainty-Aware Distillation for Asynchronous Federated Learning](https://arxiv.org/abs/2511.19966)
*Yujia Wang,Fenglong Ma,Jinghui Chen*

Main category: cs.LG

TL;DR: FedEcho is a novel asynchronous federated learning framework that uses uncertainty-aware distillation to handle outdated updates from straggler clients and mitigate bias from faster clients under heterogeneous data distributions.


<details>
  <summary>Details</summary>
Motivation: Asynchronous FL faces challenges with outdated updates from slow clients degrading model performance and bias from faster clients dominating learning, especially with heterogeneous data. Existing methods only address one issue, creating conflicts.

Method: FedEcho incorporates uncertainty-aware distillation where the server assesses prediction reliability from straggler clients and dynamically adjusts their influence based on estimated uncertainty, prioritizing certain predictions while leveraging diverse client information.

Result: Extensive experiments show FedEcho consistently outperforms existing asynchronous FL baselines, achieving robust performance without requiring access to private client data.

Conclusion: FedEcho effectively mitigates negative impacts of outdated updates and data heterogeneity in asynchronous federated learning through uncertainty-aware distillation.
...
### [215] [Rethinking Semi-Supervised Node Classification with Self-Supervised Graph Clustering](https://arxiv.org/abs/2511.19976)
*Songbo Wang,Renchi Yang,Yurui Lai,Xiaoyang Lin,Tsz Nam Chan*

Main category: cs.LG

TL;DR: NCGC integrates self-supervised graph clustering with semi-supervised classification to leverage community structures in graphs for improved node classification with limited labels.


<details>
  <summary>Details</summary>
Motivation: Real graphs contain tightly-knit communities that provide abundant signals to compensate for label scarcity in semi-supervised node classification, but prior methods haven't explored this potential.

Method: Develops soft orthogonal GNNs (SOGNs) with refined message passing, combines self-supervised clustering with classification using multi-task objective, and employs Sinkhorn-Knopp normalization for balanced soft pseudo-labels.

Result: NCGC consistently and considerably outperforms popular GNN models and recent baselines on seven real graphs across various GNN backbones.

Conclusion: Integrating self-supervised graph clustering with semi-supervised classification creates synergy that enhances model capacity for node classification tasks with limited supervision.
...
### [216] [Operator Learning at Machine Precision](https://arxiv.org/abs/2511.19980)
...
<details>
  <summary>Details</summary>
Motivation: Existing neural operator methods often fail to substantially improve accuracy despite increased complexity, remaining comparable to simpler approaches like kernel methods and reduced-order models.

Method: Regresses Cholesky factors of elliptic operators associated with Tikhonov-regularized Newton-Kantorovich updates, creating an unrolled iteration that forms a neural architecture requiring lower accuracy than end-to-end solution operator approximation.

Result: Achieves machine precision on various nonlinear forward and inverse problems including nonlinear elliptic equations, Burgers' equation, Darcy flow, Calderón problem, inverse wave scattering, and seismic imaging. FONKNORIS variant successfully solves unseen nonlinear PDEs like Klein-Gordon and Sine-Gordon equations.

Conclusion: CHONKNORIS provides theoretical convergence guarantees and enables machine-precision solutions through contractive mapping, while FONKNORIS demonstrates foundation model capabilities for solving diverse, unseen PDE problems.
...
### [217] [Rethinking Message Passing Neural Networks with Diffusion Distance-guided Stress Majorization](https://arxiv.org/abs/2511.19984)
*Haoran Zheng,Renchi Yang,Yubo Zhou,Jianliang Xu*

Main category: cs.LG

TL;DR: DDSM is a new MPNN model that addresses over-smoothing and over-correlation issues in graph neural networks through stress majorization and orthogonal regularization, incorporating diffusion distances for improved message passing.


<details>
  <summary>Details</summary>
Motivation: To overcome severe issues in existing MPNNs such as over-smoothing and over-correlation, which stem from minimizing Dirichlet energy and neighborhood aggregation operations.

Method: Proposes DDSM built on an optimization framework with stress majorization and orthogonal regularization, introduces diffusion distances for nodes to guide message passing, and develops efficient distance approximation algorithms.

Result: DDSM consistently and considerably outperforms 15 strong baselines on both homophilic and heterophilic graphs in comprehensive experiments.

Conclusion: The proposed DDSM model effectively addresses fundamental limitations of traditional MPNNs through its novel optimization framework and diffusion distance integration.
...
### [218] [On-Demand Multi-Task Sparsity for Efficient Large-Model Deployment on Edge Devices](https://arxiv.org/abs/2511.19986)
*Lianming Huang,Haibo Hu,Qiao Li,Nan Guan,Chun Jason Xue*

Main category: cs.LG

TL;DR: On-demand multi-task sparsity framework that maximizes parameter reuse across tasks to minimize switching costs, achieving 6.6X faster task switching than existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional sparsity optimization for individual tasks ignores significant I/O overhead during frequent task switching, which is critical for resource-constrained edge platforms.

Method: Decompose weights into reusable block-granular units and align sparse structures across tasks to maximize overlap. Dynamically load only the small differential set of blocks needed for the next task.

Result: Achieves superior switching efficiency, accelerating task switching by over 6.6X on average compared to existing sparsity methods on a real-world autonomous driving platform.

Conclusion: The framework effectively mitigates cold-start latency inherent in traditional monolithic approaches by maximizing parameter reuse across tasks.
...
### [219] [RankOOD - Class Ranking-based Out-of-Distribution Detection](https://arxiv.org/abs/2511.19996)
*Dishanika Denipitiyage,Naveen Karunanayake,Suranga Seneviratne,Sanjay Chawla*

Main category: cs.LG

TL;DR: RankOOD is a rank-based OOD detection method that uses Plackett-Luce loss training to leverage ranking patterns in ID class predictions for improved OOD detection.


<details>
  <summary>Details</summary>
Motivation: Current OOD detection methods may not fully exploit the ranking patterns inherent in ID class predictions, which could provide additional discriminative signals for detecting OOD samples.

Method: Extract rank lists for each class using an initial classifier, then train with Plackett-Luce loss where class rank permutations are predicted variables. OOD samples are detected based on low probability of respecting ranking patterns.

Result: Achieves state-of-the-art performance on TinyImageNet near-OOD benchmark, reducing FPR95 by 4.3% compared to previous methods.

Conclusion: RankOOD effectively leverages ranking patterns in ID predictions for improved OOD detection, demonstrating the value of rank-based approaches in this domain.
...
### [220] [REWA: Witness-Overlap Theory -- Foundations for Composable Binary Similarity Systems](https://arxiv.org/abs/2511.19998)
*Nikit Phadke*

Main category: cs.LG

TL;DR: REWA introduces a general theory of similarity based on witness-overlap structures that reduces any monotone witness overlap similarity to compact encodings with provable ranking preservation guarantees.


<details>
  <summary>Details</summary>
Motivation: To provide a unified foundation for similarity systems that can handle diverse similarity measures (graph neighborhoods, causal relations, temporal structure, etc.) and enable modular construction from reusable primitives.

Method: REWA systems use finite witness sets, semi-random bit assignments from witnesses, and rely on monotonicity of expected similarity in witness overlap. The theory applies to any sequence of transformations that produce discrete witness sets.

Result: Top-k rankings are preserved using O(log(|V|/δ)) bits under an overlap-gap condition. The framework subsumes Bloom filters, minhash, LSH bitmaps, and other methods as special cases.

Conclusion: REWA provides a principled foundation for similarity systems based on witness overlap rather than hash-function engineering, enabling millions of composable similarity definitions with logarithmic encoding complexity.
...
### [221] [Zero-Shot Transfer Capabilities of the Sundial Foundation Model for Leaf Area Index Forecasting](https://arxiv.org/abs/2511.20004)
*Peining Zhang,Hongchen Qin,Haochen Zhang,Ziqi Guo,Guiling Wang,Jinbo Bi*

Main category: cs.LG

TL;DR: Sundial foundation model outperforms fully trained LSTM in zero-shot LAI forecasting when given sufficiently long input context windows covering multiple seasonal cycles.


<details>
  <summary>Details</summary>
Motivation: To investigate the zero-shot forecasting capability of time-series foundation models for agricultural monitoring, specifically Leaf Area Index forecasting, without task-specific tuning.

Method: Systematic comparison using HiQ dataset (2000-2022) with statistical baselines, fully supervised LSTM, and Sundial foundation model under multiple evaluation protocols.

Result: Sundial in zero-shot setting outperforms fully trained LSTM when input context window covers more than one or two full seasonal cycles.

Conclusion: General-purpose foundation models can surpass specialized supervised models for remote-sensing time series prediction without task-specific tuning, showing strong potential as plug-and-play forecasters in agricultural applications.
...
### [222] [iRadioDiff: Physics-Informed Diffusion Model for Indoor Radio Map Construction and Localization](https://arxiv.org/abs/2511.20015)
*Xiucheng Wang,Tingwei Yuan,Yang Cao,Nan Cheng,Ruijin Sun,Weihua Zhuang*

Main category: cs.LG

TL;DR: iRadioDiff is a diffusion-based framework for indoor radio map construction that uses physics-informed prompts and multipath-critical priors to model electromagnetic field distributions efficiently.


<details>
  <summary>Details</summary>
Motivation: Current methods for indoor radio map construction face challenges with EM solver latency and learning-based methods that don't handle heterogeneous materials and multipath effects well in complex indoor environments.

Method: Uses sampling-free diffusion conditioned on AP positions, material reflection/transmission coefficients, and multipath priors including diffraction points, transmission boundaries, and LoS contours via conditional channels and boundary-weighted objectives.

Result: Achieves state-of-the-art performance in indoor radio map construction and RSS-based localization, with effective generalization across different layouts and material configurations.

Conclusion: iRadioDiff enables accurate modeling of nonstationary field discontinuities and efficient construction of physically consistent radio maps for indoor environments.
...
### [223] [Cross-Contrastive Clustering for Multimodal Attributed Graphs with Dual Graph Filtering](https://arxiv.org/abs/2511.20030)
*Haoran Zheng,Renchi Yang,Hongtao Wang,Jianliang Xu*

Main category: cs.LG

TL;DR: DGF is a novel clustering method for Multimodal Attributed Graphs that addresses low modality correlation and feature noise through dual graph filtering and tri-cross contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view clustering methods fail to handle unique characteristics of multimodal attributes from pre-trained models in MMAGs, particularly low modality correlation and intense feature noise.

Method: Proposes Dual Graph Filtering (DGF) with feature-wise denoising and tri-cross contrastive training across modalities, neighborhoods, and communities for robust node representation learning.

Result: DGF consistently and significantly outperforms state-of-the-art baselines on eight benchmark MMAG datasets in terms of clustering quality.

Conclusion: The proposed DGF framework effectively overcomes limitations of traditional graph filters and provides superior clustering performance for multimodal attributed graphs.
...
### [224] [RED-F: Reconstruction-Elimination based Dual-stream Contrastive Forecasting for Multivariate Time Series Anomaly Prediction](https://arxiv.org/abs/2511.20044)
*PengYu Chen,Xiaohou Shi,Yuan Chang,Yan Sun,Sajal K. Das*

Main category: cs.LG

TL;DR: RED-F is a dual-stream framework for proactive anomaly prediction in multivariate time series that transforms absolute signal detection into relative trajectory comparison by contrasting predictions from purified normal patterns and original sequences.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised methods trained only on normal data tend to reconstruct normal patterns, causing weak anomaly precursors to be submerged by dominant normal signals, making early detection difficult.

Method: Uses Reconstruction-Elimination Model (REM) with hybrid time-frequency mechanism to create purified normal baseline, and Dual-stream Contrastive Forecasting Model (DFM) that contrasts predictions from purified and original sequences using Multi-Series Prediction objective.

Result: Extensive experiments on six real-world datasets demonstrate superior capability in anomaly prediction tasks compared to existing methods.

Conclusion: RED-F effectively amplifies faint precursor signals through contrastive forecasting and transforms the challenging task of absolute anomaly detection into more robust relative trajectory comparison.
...
### [225] [SOMBRL: Scalable and Optimistic Model-Based RL](https://arxiv.org/abs/2511.20066)
*Bhavya Sukhija,Lenart Treven,Carmelo Sferrazza,Florian Dörfler,Pieter Abbeel,Andreas Krause*

Main category: cs.LG

TL;DR: SOMBRL is a scalable and optimistic model-based RL method that uses uncertainty-aware dynamics models and principled exploration to achieve sublinear regret across various settings.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of efficient exploration in model-based reinforcement learning when system dynamics are unknown and learning must occur through online interactions.

Method: Learns an uncertainty-aware dynamics model and greedily maximizes a weighted sum of extrinsic reward and epistemic uncertainty; compatible with any policy optimizers or planners.

Result: Achieves sublinear regret for nonlinear dynamics in finite-horizon, discounted infinite-horizon, and non-episodic settings; shows strong performance on state-based and visual-control environments; outperforms state-of-the-art on RC car hardware.

Conclusion: SOMBRL provides a flexible, scalable solution for principled exploration in MBRL with theoretical guarantees and practical effectiveness across simulation and real-world tasks.
...
### [226] [QiMeng-CRUX: Narrowing the Gap between Natural Language and Verilog via Core Refined Understanding eXpression](https://arxiv.org/abs/2511.20099)
...
Method: Introduces CRUX structured intermediate space and a two-stage training framework with Joint Expression Modeling and Dual-Space Optimization.

Result: CRUX-V achieves state-of-the-art performance on multiple Verilog generation benchmarks, especially on challenging design tasks. CRUX space is transferable and benefits other code models.
...
### [227] [The Devil in the Details: Emergent Misalignment, Format and Coherence in Open-Weights LLMs](https://arxiv.org/abs/2511.20104)
*Craig Dickson*

Main category: cs.LG

TL;DR: Modern open-weights models show significantly lower emergent misalignment rates (0.68%) compared to proprietary models like GPT-4o (20%), with JSON output formats doubling vulnerability compared to natural language prompts.


<details>
  <summary>Details</summary>
Motivation: To evaluate if current-generation open-weights models exhibit similar resistance to emergent misalignment as the Qwen-2.5 family, and measure misalignment robustness across different model architectures and scales.
...
Result: Fine-tuned models showed 0.68% misalignment rate (vs 0.07% for base models), dramatically lower than GPT-4o's 20%. JSON output formats doubled misalignment rates compared to natural language prompts (0.96% vs 0.42%).

Conclusion: Emergent misalignment is reproducible in modern open-weights models but at substantially lower rates than proprietary systems, with structural constraints like JSON output increasing vulnerability by reducing models' ability to refuse unsafe requests.
...
### [228] [Multivariate Forecasting of Bitcoin Volatility with Gradient Boosting: Deterministic, Probabilistic, and Feature Importance Perspectives](https://arxiv.org/abs/2511.20105)
*Grzegorz Dudek,Mateusz Kasprzyk,Paweł Pełka*

Main category: cs.LG

TL;DR: LGBM model applied to Bitcoin volatility forecasting using 69 predictors, showing effectiveness in capturing nonlinear market characteristics and identifying key drivers like trading volume and investor attention.


<details>
  <summary>Details</summary>
Motivation: To investigate LGBM's effectiveness for both deterministic and probabilistic Bitcoin volatility forecasting, addressing the nonlinear and high-variance nature of cryptocurrency markets.

Method: Used LGBM with 69 predictors (market, behavioral, macroeconomic), compared with econometric and ML baselines, employed quantile regression and residual simulation for probabilistic forecasting, used gain-based and permutation feature importance.

Result: LGBM models effectively captured nonlinear cryptocurrency market characteristics, identified trading volume, lagged volatility, investor attention, and market capitalization as key drivers, and provided interpretable volatility insights.

Conclusion: LGBM is effective for Bitcoin volatility forecasting, offering both accurate predictions and interpretable insights into market dynamics through feature importance analysis.
...
### [229] [CLIMATEAGENT: Multi-Agent Orchestration for Complex Climate Data Science Workflows](https://arxiv.org/abs/2511.20109)
*Hyeonjae Kim,Chenyue Li,Wen Deng,Mengxi Jin,Wen Huang,Mengqian Lu,Binhang Yuan*

Main category: cs.LG

TL;DR: ClimateAgent is a multi-agent framework that automates climate data analysis workflows by decomposing user questions into executable tasks, dynamically acquiring data through API introspection, and generating Python code with self-correction.


<details>
  <summary>Details</summary>
Motivation: Generic LLM agents and static scripting pipelines lack climate-specific context and flexibility, performing poorly for complex climate data analysis tasks that require handling massive, heterogeneous datasets.

Method: Uses orchestrated multi-agent system with specialized agents: Orchestrate-Agent and Plan-Agent coordinate tasks, Data-Agents dynamically introspect APIs for data acquisition, and Coding-Agent generates Python code with self-correction loop for analysis and reporting.

Result: Achieved 100% task completion and report quality score of 8.32 on Climate-Agent-Bench-85 benchmark, significantly outperforming GitHub-Copilot (6.27) and GPT-5 baseline (3.26).
...
### [230] [IDAP++: Advancing Divergence-Based Pruning via Filter-Level and Layer-Level Optimization](https://arxiv.org/abs/2511.20141)
*Aleksei Samarin,Artem Nazarenko,Egor Kotenko,Valentin Malykh,Alexander Savelev,Aleksei Toropov*

Main category: cs.LG

TL;DR: A unified neural network compression framework using information flow analysis to remove redundant filters and layers while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: To address redundancy at both filter and architectural levels in neural networks through a consistent information flow-based approach that works across diverse architectures.

Method: Two-stage optimization: 1) Iterative divergence-aware pruning of redundant filters, 2) Layer-wise analysis to eliminate entire layers with minimal impact on information propagation using tensor flow divergence metric.

Result: Achieves substantial model compression while maintaining competitive accuracy, outperforming state-of-the-art methods across various architectures including CNNs and transformers.

Conclusion: Flow divergence serves as an effective guiding principle for both filter-level and layer-level optimization, providing practical benefits for resource-constrained deployment.
...
### [231] [On the Limits of Momentum in Decentralized and Federated Optimization](https://arxiv.org/abs/2511.20168)
*Riccardo Zaccone,Sai Praneeth Karimireddy,Carlo Masone*

Main category: cs.LG

TL;DR: Momentum in distributed SGD under cyclic client participation in Federated Learning is inevitably affected by statistical heterogeneity, and decreasing step-sizes faster than Θ(1/t) leads to convergence to a constant value dependent on initialization and heterogeneity.


<details>
  <summary>Details</summary>
Motivation: To determine if momentum can guarantee convergence under unbounded heterogeneity in decentralized scenarios with partial worker participation, particularly in Federated Learning where momentum appears as a potential solution to mitigate statistical heterogeneity effects.

Method: Theoretical analysis of momentum under cyclic client participation in distributed SGD, examining the effects of statistical heterogeneity and various step-size schedules.

Result: Momentum remains inevitably affected by statistical heterogeneity. Decreasing step-sizes faster than Θ(1/t) leads to convergence to a constant value that depends on initialization and the heterogeneity bound.

Conclusion: Momentum cannot overcome statistical heterogeneity in decentralized FL settings with cyclic participation, and careful step-size selection is crucial for convergence behavior.
...
### [232] [AdaCap: An Adaptive Contrastive Approach for Small-Data Neural Networks](https://arxiv.org/abs/2511.20170)
*Bruno Belucci,Karim Lounici,Katia Meziani*

Main category: cs.LG

TL;DR: AdaCap is a training scheme that combines permutation-based contrastive loss with Tikhonov-based closed-form output mapping to improve neural network performance on small tabular datasets where tree-based models typically dominate.


<details>
  <summary>Details</summary>
Motivation: Neural networks struggle on small tabular datasets while tree-based models remain dominant in this regime, creating a performance gap that needs to be addressed.

Method: AdaCap combines a permutation-based contrastive loss with a Tikhonov-based closed-form output mapping as a training scheme for neural networks.

Result: Across 85 real-world regression datasets and multiple architectures, AdaCap yields consistent and statistically significant improvements in the small-sample regime, particularly for residual models. A meta-predictor can accurately anticipate when AdaCap is beneficial based on dataset characteristics.

Conclusion: AdaCap acts as a targeted regularization mechanism that strengthens neural networks precisely where they are most fragile, making them more competitive with tree-based models on small tabular datasets.
...
### [233] [Learning Subgroups with Maximum Treatment Effects without Causal Heuristics](https://arxiv.org/abs/2511.20189)
*Lincen Yang,Zhong Li,Matthijs van Leeuwen,Saber Salehkaleybar*

Main category: cs.LG

TL;DR: The paper proposes a causal subgroup discovery method using structural causal models instead of causal heuristics, showing optimal subgroup discovery reduces to standard supervised learning when using partition-based models.


<details>
  <summary>Details</summary>
Motivation: Current approaches for subgroup discovery either rely on accurate pointwise treatment effect estimation or use ad-hoc causal heuristics without rigorous justification, which are problematic for identifying subgroups with maximum average treatment effect.

Method: The authors study subgroup discovery under the structural causal model framework, showing that with partition-based models, optimal subgroup discovery becomes a standard supervised learning problem. They instantiate this approach using CART (Classification and Regression Trees).

Result: On synthetic and semi-synthetic datasets, their method outperforms various baselines in accurately identifying subgroups with maximum treatment effect, demonstrating that avoiding causal heuristics leads to better performance.

Conclusion: Subgroup discovery for maximum treatment effect can be effectively framed as a supervised learning problem under structural causal models, eliminating the need for ad-hoc causal heuristics while achieving superior performance.
...
### [234] [In-Context Compositional Learning via Sparse Coding Transformer](https://arxiv.org/abs/2511.20194)
*Wei Chen,Jingxi Yu,Zichen Miao,Qiang Qiu*

Main category: cs.LG

TL;DR: Proposes a sparse coding-inspired attention reformulation for Transformers to enhance compositional learning by representing inputs as sparse combinations of dictionary atoms and applying compositional rules from context examples.


<details>
  <summary>Details</summary>
Motivation: Transformers struggle with in-context compositional learning tasks despite their success in other domains, as they lack inherent structural inductive bias for compositional reasoning.

Method: Reformulates attention as mapping through encoding/decoding dictionaries, imposes sparsity on coefficients to capture compositional structure, and estimates target coefficients as linear combinations of context example coefficients.

Result: Demonstrated effectiveness on S-RAVEN and RAVEN datasets, maintaining performance on compositional generalization tasks where standard Transformers fail.

Conclusion: The sparse coding-based attention reformulation enables Transformers to better learn and apply compositional rules, addressing limitations in compositional reasoning tasks.
...
### [235] [Communication-Efficient Learning for Satellite Constellations](https://arxiv.org/abs/2511.20220)
*Ruxandra-Stefania Tudose,Moritz H. W. Grüss,Grace Ra Kim,Karl H. Johansson,Nicola Bastianello*

Main category: cs.LG

TL;DR: A communication-efficient federated learning algorithm for satellite constellations using local training, compression, and error feedback mechanisms to reduce communication overhead while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Satellite constellations are widespread but learning with them faces communication challenges; need efficient algorithms that minimize communication with ground stations while maintaining model accuracy.

Method: Proposed a federated learning approach with local training on satellites, compression to reduce communication size, and an error feedback mechanism to enhance accuracy. Also developed an algorithm-agnostic error feedback scheme.

Result: The algorithm shows superior performance compared to state-of-the-art methods in realistic space scenarios through simulations, with convergence analysis demonstrating effectiveness.

Conclusion: The proposed communication-efficient federated learning algorithm successfully addresses the challenges of satellite constellations, providing accurate models while significantly reducing communication overhead through local processing and compression techniques.
...
### [236] [Decoupling and Damping: Structurally-Regularized Gradient Matching for Multimodal Graph Condensation](https://arxiv.org/abs/2511.20222)
*Lian Shen,Zhendan Chen,Yinhui jiang,Meijia Song,Ziming Su,Juan Liu,Xiangrong Liu*

Main category: cs.LG

TL;DR: SR-GM is a novel graph condensation framework that addresses gradient conflicts and structural noise amplification in multimodal graphs, enabling efficient training of GNNs on compressed datasets.


<details>
  <summary>Details</summary>
Motivation: Multimodal graphs are crucial for web applications but computationally expensive to train. Existing graph condensation methods fail in multimodal settings due to conflicting gradients between modalities and structural amplification of noise.

Method: Proposes Structurally-Regularized Gradient Matching with two components: gradient decoupling via orthogonal projection to resolve modality conflicts, and structural damping regularizer using graph Dirichlet energy to stabilize optimization.

Result: SR-GM significantly improves accuracy and accelerates convergence compared to baselines. Condensed graphs show strong cross-architecture generalization and potential for Neural Architecture Search acceleration.

Conclusion: The framework provides a scalable methodology for multimodal graph-based learning in resource-constrained environments by addressing both gradient conflict and structural amplification issues.
...
### [237] [DiCaP: Distribution-Calibrated Pseudo-labeling for Semi-Supervised Multi-Label Learning](https://arxiv.org/abs/2511.20225)
*Bo Han,Zhuoming Li,Xiaoyu Wang,Yaxin Hou,Hui Liu,Junhui Hou,Yuheng Jia*

Main category: cs.LG

TL;DR: DiCaP introduces a correctness-aware pseudo-labeling framework for semi-supervised multi-label learning that weights pseudo-labels based on their correctness likelihood and uses dual-thresholding to handle confident vs ambiguous samples.


<details>
  <summary>Details</summary>
Motivation: Existing SSMLL methods assign equal weights to all pseudo-labels regardless of quality, which amplifies the impact of noisy predictions and degrades performance. The optimal weight should reflect the correctness likelihood of each pseudo-label.

Method: Proposed Distribution-Calibrated Pseudo-labeling (DiCaP) that estimates posterior precision to calibrate pseudo-label weights. Uses dual-thresholding: confident samples get weighted pseudo-labels, ambiguous samples are explored via unsupervised contrastive learning.

Result: Experiments on multiple benchmark datasets show consistent improvements, outperforming state-of-the-art methods by up to 4.27%.

Conclusion: DiCaP effectively addresses the pseudo-label quality issue in SSMLL by incorporating correctness likelihood estimation and dual-thresholding, leading to significant performance gains over existing methods.
...
### [238] [Leveraging weights signals - Predicting and improving generalizability in reinforcement learning](https://arxiv.org/abs/2511.20234)
*Olivier Moulin,Vincent Francois-lavet,Paul Elbers,Mark Hoogendoorn*

Main category: cs.LG

TL;DR: Proposes a method to predict RL agent generalizability using neural network weights and modifies PPO loss function to improve generalization.


<details>
  <summary>Details</summary>
Motivation: RL agents tend to overfit to training environments, limiting their performance on different environments.

Method: Predict generalizability score from neural network weights and modify PPO loss function to boost generalization.

Result: Improved PPO algorithm produces agents with better generalization compared to original PPO.

Conclusion: The proposed methodology effectively enhances RL agent generalizability through weight-based prediction and loss function modifications.
...
### [239] [Interpretable Air Pollution Forecasting by Physics-Guided Spatiotemporal Decoupling](https://arxiv.org/abs/2511.20257)
...
<details>
  <summary>Details</summary>
Motivation: Address the trade-off between performance and interpretability in air pollution forecasting models, which is crucial for public health and operational air-quality management.

Method: Decomposes spatiotemporal behavior into two additive modules: a physics-guided transport kernel with directed weights conditioned on wind and geography (advection), and an explainable attention mechanism that learns local responses and attributes future concentrations to specific historical lags and exogenous drivers.

Result: Consistently outperforms state-of-the-art baselines across multiple forecasting horizons on a comprehensive dataset from the Stockholm region.

Conclusion: The model's integration of high predictive performance and spatiotemporal interpretability provides a more reliable foundation for operational air-quality management in real-world applications.
...
### [240] [Beyond Components: Singular Vector-Based Interpretability of Transformer Circuits](https://arxiv.org/abs/2511.20273)
*Areeb Ahmad,Abhinav Joshi,Ashutosh Modi*

Main category: cs.LG

TL;DR: The paper introduces a fine-grained decomposition of transformer components into orthogonal singular directions, revealing that attention heads and MLPs contain multiple overlapping subfunctions rather than being indivisible units.


<details>
  <summary>Details</summary>
Motivation: Existing mechanistic interpretability methods treat transformer components as indivisible units, overlooking potential functional substructure within them, which limits understanding of their distributed and complex behavior.

Method: Decompose attention heads and MLP layers into orthogonal singular directions to identify superposed and independent computations within single components, validated on standard tasks like IOI, GP, and GT.

Result: Previously identified canonical functional heads (e.g., name mover) encode multiple overlapping subfunctions aligned with distinct singular directions, and circuit elements show strong activation along specific low-rank directions.

Conclusion: Transformer computations are more distributed, structured, and compositional than previously assumed, opening new avenues for fine-grained mechanistic interpretability and deeper understanding of model internals.
...
### [241] [HVAdam: A Full-Dimension Adaptive Optimizer](https://arxiv.org/abs/2511.20277)
*Yiheng Zhang,Shaowu Wu,Yuanzhuo Xu,Jiajun Wu,Shang Xu,Steve Drew,Xiaoguang Niu*

Main category: cs.LG

TL;DR: Anon is a novel optimizer with tunable adaptivity that bridges the gap between SGD and Adam, outperforming both through a flexible incremental delay update mechanism and achieving superior performance across various tasks.


<details>
  <summary>Details</summary>
Motivation: Adaptive optimizers like Adam generalize worse than non-adaptive methods like SGD on classical architectures, due to limitations in adaptivity that restrict their ability to handle diverse optimization landscapes.

Method: Proposed Anon optimizer with continuously tunable adaptivity and incremental delay update (IDU) mechanism, which is more flexible than AMSGrad's max-tracking and enhances robustness to gradient noise.

Result: Anon consistently outperforms state-of-the-art optimizers on image classification, diffusion, and language modeling tasks, demonstrating superior performance across the adaptivity spectrum.

Conclusion: Adaptivity can serve as a valuable tunable design principle, and Anon provides the first unified framework that bridges classical and modern optimizers while surpassing their advantageous properties.
...
### [242] [Geometry of Decision Making in Language Models](https://arxiv.org/abs/2511.20315)
*Abhinav Joshi,Divyanshu Bhatt,Ashutosh Modi*

Main category: cs.LG

TL;DR: LLMs learn to project linguistic inputs onto structured low-dimensional manifolds aligned with task decisions, with intrinsic dimension patterns showing expansion in middle layers and compression in later layers for decision-making.


<details>
  <summary>Details</summary>
Motivation: To understand the internal decision-making processes and geometric structure of hidden representations in LLMs, which remain opaque despite strong generalization capabilities.

Method: Large-scale study with 28 open-weight transformer models, estimating intrinsic dimension across layers using multiple estimators and quantifying per-layer performance on multiple-choice question answering tasks.

Result: Consistent intrinsic dimension pattern: early layers use low-dimensional manifolds, middle layers expand the space, and later layers compress it to converge to decision-relevant representations.

Conclusion: LLMs implicitly learn structured low-dimensional manifolds aligned with task-specific decisions, providing geometric insights into how generalization and reasoning emerge in language models.
...
### [243] [MXtalTools: A Toolkit for Machine Learning on Molecular Crystals](https://arxiv.org/abs/2511.20327)
*Michael Kilgour,Mark E. Tuckerman,Jutta Rogal*

Main category: cs.LG

TL;DR: MXtalTools is a Python package for machine learning modeling of molecular crystals, offering utilities for dataset curation, model training, crystal representation, structure sampling, and differentiable analysis.


<details>
  <summary>Details</summary>
Motivation: To provide a flexible and comprehensive tool for data-driven modeling of molecular crystals, enabling machine learning studies in the molecular solid state.

Method: Developed a modular Python package with CUDA acceleration, featuring classes for dataset synthesis, integrated workflows, crystal parameterization, structure sampling, and differentiable crystal analysis.

Result: Created an open-source package with detailed documentation that enables high-throughput crystal modeling and can be integrated into existing workflows or used to build novel pipelines.

Conclusion: MXtalTools successfully provides a flexible framework for molecular crystal modeling that combines machine learning approaches with computational crystallography capabilities.
...
### [244] [Soft Adaptive Policy Optimization](https://arxiv.org/abs/2511.20347)
*Chang Gao,Chujie Zheng,Xiong-Hui Chen,Kai Dang,Shixuan Liu,Bowen Yu,An Yang,Shuai Bai,Jingren Zhou,Junyang Lin*

Main category: cs.LG

TL;DR: SAPO is a soft adaptive policy optimization method that replaces hard clipping with temperature-controlled gating to stabilize RL training for LLMs while preserving learning signals.


<details>
  <summary>Details</summary>
Motivation: Existing group-based policy optimization methods like GSPO and GRPO use hard clipping that can suppress useful gradients and lead to unstable updates, especially in Mixture-of-Experts models where token-level importance ratios exhibit high variance.

Method: SAPO introduces a smooth, temperature-controlled gate that adaptively attenuates off-policy updates while preserving useful learning signals. It maintains sequence-level coherence like GSPO but avoids brittle hard clipping by forming a continuous trust region.

Result: Empirical results show SAPO exhibits improved training stability and higher Pass@1 performance on mathematical reasoning benchmarks. When applied to Qwen3-VL model series, SAPO yields consistent performance gains across diverse tasks and different model sizes.

Conclusion: SAPO provides a more reliable, scalable, and effective optimization strategy for RL training of LLMs compared to existing methods like GSPO and GRPO.
...
### [245] [Complexity Reduction Study Based on RD Costs Approximation for VVC Intra Partitioning](https://arxiv.org/abs/2511.20349)
*M. E. A. Kherchouche,F. Galpin,T. Dumas,F. Schnitzler,D. Menard,L. Zhang*

Main category: cs.LG

TL;DR: Machine learning approaches (regression and reinforcement learning) are proposed to accelerate VVC intra partitioning by predicting RD costs and making split decisions, reducing exhaustive RDO search complexity.


<details>
  <summary>Details</summary>
Motivation: To reduce the computational complexity of exhaustive Rate-Distortion Optimization search in Versatile Video Codec intra partitioning, which is time-consuming.

Method: Two ML techniques: 1) Regression-based prediction of normalized RD costs using neighboring blocks' RD costs as features; 2) Reinforcement Learning with DQN modeling partitioning as Markov Decision Process, using CU decision trajectories across depths.

Result: Both methods are size-independent and incorporate neighboring block RD costs. RL approach exploits Markov property of partitioning. Pre-determined thresholds are applied for split selection.

Conclusion: The proposed machine learning approaches effectively accelerate VVC intra partitioning by reducing exhaustive RDO search complexity through RD cost prediction and RL-based decision making.
...
### [246] [PRISM: Periodic Representation with multIscale and Similarity graph Modelling for enhanced crystal structure property prediction](https://arxiv.org/abs/2511.20362)
*Àlex Solé,Albert Mosella-Montoro,Joan Cardona,Daniel Aravena,Silvia Gómez-Coca,Eliseo Ruiz,Javier Ruiz-Hidalgo*

Main category: cs.LG

TL;DR: PRISM is a graph neural network framework that improves crystal property prediction by explicitly modeling multiscale representations and periodic boundary conditions in crystalline structures.


<details>
  <summary>Details</summary>
Motivation: Current graph-based methods for crystal structures often overlook periodic boundary conditions and multiscale interactions, which are essential characteristics of crystalline materials.

Method: PRISM employs specialized expert modules that encode distinct structural and chemical aspects of periodic systems, integrating multiscale representations and periodic feature encoding.

Result: Extensive experiments show PRISM improves state-of-the-art predictive accuracy across crystal structure-based benchmarks.

Conclusion: PRISM significantly enhances crystal property prediction by better capturing the unique characteristics of periodic crystalline structures.
...
### [247] [MoRE: Batch-Robust Multi-Omics Representations from Frozen Pre-trained Transformers](https://arxiv.org/abs/2511.20382)
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in multi-omics representation learning including extreme dimensionality, modality heterogeneity, and batch effects, while leveraging the generalization capabilities of pre-trained transformers.
...
Conclusion: MoRE represents a practical step toward general-purpose omics foundation models through parameter-efficient multi-omics integration.
...
### [248] [Identifying environmental factors associated with tetrodotoxin contamination in bivalve mollusks using eXplainable AI](https://arxiv.org/abs/2511.20395)
*M. C. Schoppema,B. H. M. van der Velden,A. Hürriyetoğlu,M. D. Klijnstra,E. J. Faassen,A. Gerssen,H. J. van der Fels-Klerx*

Main category: cs.LG

TL;DR: Developed an explainable deep learning model to predict TTX contamination in bivalve mollusks using environmental factors like sun hours, temperature, and chloride concentration.


<details>
  <summary>Details</summary>
Motivation: TTX contamination in European seafood poses food safety risks and economic losses, requiring early prediction methods. Previous studies identified shallow habitats and water temperature as drivers, but temporal relationships remained unexplored.

Method: Created an explainable deep learning model using meteorological and hydrological features as inputs to predict TTX presence/absence in the Dutch Zeeland estuary.

Result: Sunrise/sunset times, global radiation, water temperature, and chloride concentration were most influential. Day length and global radiation (representing effective sun hours) emerged as key drivers for TTX contamination.

Conclusion: The model successfully identified environmental factors associated with TTX contamination, providing a valuable tool for mitigating marine toxin risks in the food industry and regulatory authorities.
...
### [249] [Model-Based Learning of Whittle indices](https://arxiv.org/abs/2511.20397)
*Joël Charles-Rebuffé,Nicolas Gast,Bruno Gaujal*

Main category: cs.LG

TL;DR: BLINQ is a model-based algorithm that learns Whittle indices for indexable, communicating, unichain MDPs by building empirical estimates and using an extended state-of-the-art algorithm, outperforming Q-learning in sample efficiency and computational cost.


<details>
  <summary>Details</summary>
Motivation: To develop a more efficient method for learning Whittle indices in MDPs that requires fewer samples and has lower computational cost compared to existing Q-learning approaches, even when Q-learning is accelerated with neural networks.

Method: Builds an empirical estimate of the MDP and computes Whittle indices using an extended version of a state-of-the-art algorithm, with proven convergence guarantees and computational complexity analysis.

Result: BLINQ significantly outperforms Q-learning in sample efficiency for accurate approximation and has lower total computational cost for reasonably high sample counts, even when Q-learning uses neural network acceleration.

Conclusion: BLINQ provides an efficient model-based alternative to Q-learning for Whittle index learning, offering better sample efficiency and computational performance with proven convergence guarantees.
...
### [250] [Short-Range Oversquashing](https://arxiv.org/abs/2511.20406)
*Yaaqov Mishayev,Yonatan Sverdlov,Tal Amir,Nadav Dym*

Main category: cs.LG

TL;DR: MPNNs suffer from oversquashing not just in long-range tasks but also in short-range problems, revealing two distinct mechanisms: bottleneck phenomenon and vanishing gradients. Transformers outperform specialized MPNNs in addressing these issues.


<details>
  <summary>Details</summary>
Motivation: To understand the limitations of Message Passing Neural Networks (MPNNs) in handling oversquashing, which was previously thought to be mainly a long-range problem, and to compare MPNNs with Graph Transformers as potential solutions.

Method: Analysis of oversquashing in both short-range and long-range graph tasks, disentangling bottleneck phenomenon from vanishing gradient effects, and evaluating virtual nodes and transformer architectures.

Result: Oversquashing occurs in short-range tasks due to bottleneck effects not captured by existing explanations, and virtual nodes don't resolve it. Transformers successfully handle these challenges.

Conclusion: Transformers are more effective than specialized MPNNs for addressing oversquashing, as they overcome both short-range bottleneck and long-range vanishing gradient problems that limit MPNN performance.
...
### [251] [Tight Margin-Based Generalization Bounds for Voting Classifiers over Finite Hypothesis Sets](https://arxiv.org/abs/2511.20407)
*Kasper Green Larsen,Natascha Schalburg*

Main category: cs.LG

TL;DR: First margin-based generalization bound for voting classifiers that is asymptotically tight across key parameters.


<details>
  <summary>Details</summary>
Motivation: To establish a theoretically sound generalization guarantee for voting classifiers that accounts for multiple factors simultaneously.

Method: Mathematical proof of a margin-based generalization bound that considers hypothesis set size, margin, training points with margin, sample size, and failure probability.

Result: Derived an asymptotically tight bound that provides optimal tradeoff between the key parameters affecting generalization performance.

Conclusion: The bound offers a comprehensive theoretical foundation for understanding voting classifier generalization behavior.
...
### [252] [Diffusion for Fusion: Designing Stellarators with Generative AI](https://arxiv.org/abs/2511.20445)
*Misha Padidar,Teresa Huang,Andrew Giuliani,Marina Spivak*

Main category: cs.LG

TL;DR: Using conditional diffusion models to rapidly generate high-quality quasisymmetric stellarator designs with specific characteristics not seen during training, achieving less than 5% deviation from targets.


<details>
  <summary>Details</summary>
Motivation: Stellarator design is traditionally slow and computationally intensive. Machine learning approaches can accelerate this process using large datasets of optimized stellarators, enabling rapid generation of new designs.

Method: Train a conditional diffusion model on data from the QUASR database to generate quasisymmetric stellarator designs with specific characteristics (aspect ratio and mean rotational transform).

Result: Generated stellarators show less than 5% deviation from quasisymmetry and target characteristics, with potential to reach sub 1% targets. Models can produce designs with characteristics not seen during training.

Conclusion: Generative modeling shows promise for advancing stellarator design, with diffusion models capable of rapidly producing high-quality designs that meet performance targets.
...
### [253] [Towards Trustworthy Wi-Fi Sensing: Systematic Evaluation of Deep Learning Model Robustness to Adversarial Attacks](https://arxiv.org/abs/2511.20456)
*Shreevanth Krishnaa Gopalakrishnan,Stephen Hailes*

Main category: cs.LG

TL;DR: This paper systematically evaluates the robustness of CSI deep learning models for human sensing under various adversarial attacks, showing that smaller models are less robust and physically realizable perturbations are less effective than feature-space attacks.


<details>
  <summary>Details</summary>
Motivation: Machine learning-based CSI sensing systems are vulnerable to adversarial perturbations, raising security and reliability concerns for real-world deployment of wireless human sensing applications.

Method: Established a framework to evaluate CSI models under white-box, black-box/transfer, and universal perturbation attacks, comparing compact temporal autoencoders with larger deep architectures across three public datasets.

Result: Smaller models are markedly less robust despite similar clean data performance. Physically realizable signal-space perturbations significantly reduce attack success compared to unconstrained feature-space attacks. Adversarial training improves robust accuracy with moderate clean performance degradation.

Conclusion: The findings provide quantitative baselines for robustness estimation and inform design principles for secure and trustworthy human-centered wireless sensing systems.
...
### [254] [NVIDIA Nemotron Parse 1.1](https://arxiv.org/abs/2511.20478)
...
TL;DR: Nemotron-Parse-1.1 is a lightweight document parsing and OCR model that improves upon its predecessor with better OCR, markdown formatting, table parsing, and text extraction from visual elements, while supporting longer output sequences.


<details>
  <summary>Details</summary>
Motivation: To advance document parsing capabilities by creating a more efficient and capable OCR model that handles various document types including dense documents with visual elements like pictures, charts, and diagrams.

Method: Uses an encoder-decoder architecture with 885M parameters, including a compact 256M-parameter language decoder, and offers a variant (Nemotron-Parse-1.1-TC) with reduced vision token length for faster processing.

Result: Achieves competitive accuracy on public benchmarks and offers a 20% speed improvement in the TC variant with minimal quality degradation.

Conclusion: Nemotron-Parse-1.1 provides a strong lightweight OCR solution with improved capabilities across multiple document parsing tasks, and the model weights and training data are publicly released.
...
### [255] [MTBBench: A Multimodal Sequential Clinical Decision-Making Benchmark in Oncology](https://arxiv.org/abs/2511.20490)
...
TL;DR: MTBBench is a new benchmark for multimodal LLMs that simulates Molecular Tumor Board decision-making with clinically challenging, multimodal, and longitudinal oncology questions, addressing limitations of current benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current multimodal LLM benchmarks fail to capture the complexity of real-world clinical workflows like Molecular Tumor Boards, which require integrating heterogeneous data and evolving insights over time in multi-agent decision-making environments.

Method: Developed MTBBench - an agentic benchmark simulating MTB-style decision-making through clinically validated multimodal and longitudinal oncology questions, with ground truth annotations validated by clinicians via a co-developed app.

Result: Benchmarking shows current LLMs lack reliability - frequently hallucinate, struggle with time-resolved data reasoning, and fail to reconcile conflicting evidence across modalities. The agentic framework with foundation model-based tools improves task-level performance by up to 9.0% and 11.2% for multimodal and longitudinal reasoning respectively.

Conclusion: MTBBench provides a challenging and realistic testbed for advancing multimodal LLM reasoning, reliability, and tool-use in precision oncology MTB environments.
...
### [256] [DP-MicroAdam: Private and Frugal Algorithm for Training and Fine-tuning](https://arxiv.org/abs/2511.20509)
*Mihaela Hudişteanu,Edwige Cyffers,Nikita P. Kalinin*

Main category: cs.LG

TL;DR: DP-MicroAdam is a memory-efficient adaptive optimizer for differentially private training that outperforms existing methods and achieves competitive accuracy with DP-SGD across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current DP training relies heavily on DP-SGD, which requires extensive compute and hyperparameter tuning. Adaptive optimizers that work well in non-private settings are not widely adopted in DP training despite their potential benefits.

Method: Proposed DP-MicroAdam, a memory-efficient and sparsity-aware adaptive DP optimizer that converges at the optimal O(1/√T) rate in stochastic non-convex optimization.

Result: DP-MicroAdam outperforms existing adaptive DP optimizers and achieves competitive or superior accuracy compared to DP-SGD on CIFAR-10, ImageNet, and private fine-tuning of transformers.

Conclusion: Adaptive optimization can improve both performance and stability under differential privacy, making DP-MicroAdam a practical alternative to DP-SGD.
...
### [257] [Adam Simplified: Bias Correction Simplified](https://arxiv.org/abs/2511.20516)
...
<details>
  <summary>Details</summary>
Motivation: To investigate the empirical necessity of bias correction in Adam optimizer, which is widely used but poorly understood.

Method: Systematic ablations on vision and language modeling tasks, analyzing bias correction as implicit learning rate scheduling dependent on β1 and β2 parameters.

Result: Bias correction shows no improvement in final test performance with optimal hyperparameters, and can harm performance without appropriate learning rate scheduling.

Conclusion: Bias correction should not be universally included in Adam optimizer as it provides no benefits in optimal settings and requires careful learning rate scheduling.
...
### [258] [Feature-Modulated UFNO for Improved Prediction of Multiphase Flow in Porous Media](https://arxiv.org/abs/2511.20543)
*Alhasan Abdellatif,Hannah P. Menke,Ahmed H. Elsheikh,Florian Doster,Kamaljit Singh*

Main category: cs.LG

TL;DR: UFNO-FiLM enhances UFNO by decoupling scalar inputs using FiLM layers and employing spatially weighted loss, achieving 21% reduction in gas saturation MAE for subsurface flow.
...
Method: Introduces FiLM layers to decouple scalar inputs from spatial features, and uses spatially weighted loss function that prioritizes critical regions.

Result: 21% reduction in gas saturation Mean Absolute Error compared to UFNO in subsurface multiphase flow experiments.

Conclusion: The proposed UFNO-FiLM architecture effectively improves predictive accuracy by addressing scalar input inefficiencies and incorporating spatial error sensitivity.
...
### [259] [E2E-GRec: An End-to-End Joint Training Framework for Graph Neural Networks and Recommender Systems](https://arxiv.org/abs/2511.20564)
*Rui Xue,Shichao Zhu,Liang Qin,Guangmou Pan,Yang Song,Tianfu Wu*

Main category: cs.LG

TL;DR: E2E-GRec is an end-to-end training framework that unifies GNN training with recommender systems, overcoming limitations of traditional two-stage approaches through efficient subgraph sampling, graph feature auto-encoding, and dynamic loss balancing.


<details>
  <summary>Details</summary>
Motivation: Traditional two-stage GNN pipelines in recommender systems suffer from high computational overhead and lack of joint optimization, as GNN embeddings are pre-trained offline and used as static features without direct gradient feedback from the recommendation task.

Method: The framework uses efficient subgraph sampling from large-scale graphs, a Graph Feature Auto-Encoder for self-supervised learning, and a two-level feature fusion mechanism with Gradnorm-based dynamic loss balancing to enable stable end-to-end training.

Result: Extensive evaluations show significant improvements: +0.133% relative improvement in stay duration and 0.3171% reduction in average video skips in online A/B tests, consistently outperforming traditional approaches across multiple recommendation metrics.

Conclusion: E2E-GRec successfully addresses the limitations of decoupled GNN-recommender pipelines by enabling joint optimization, achieving better performance and efficiency in large-scale industrial recommendation scenarios.
...
### [260] [MSTN: Fast and Efficient Multivariate Time Series Model](https://arxiv.org/abs/2511.20577)
*Sumit S Shevtekar,Chandresh K Maurya,Gourab Sil*

Main category: cs.LG

TL;DR: MSTN is a multi-scale temporal network that adaptively models time-series patterns from milliseconds to long-range dependencies, achieving state-of-the-art performance on 24 of 32 benchmark datasets across forecasting, imputation, and classification tasks.


<details>
  <summary>Details</summary>
Motivation: Real-world time-series data exhibits complex multi-scale dynamics, but existing models with fixed-scale priors over-regularize temporal patterns and struggle with sudden, high-magnitude events, limiting their adaptability.

Method: Hierarchical architecture with: (i) multi-scale convolutional encoder for local patterns, (ii) sequence modeling (BiLSTM/Transformer) for long-range dependencies, and (iii) gated fusion with SE and multi-head temporal attention for dynamic feature integration.

Result: MSTN achieves competitive SOTA performance, outperforming contemporary approaches including EMTSF, LLM4TS, HiMTM, TIME-LLM, MTST, SOFTS, iTransformer, TimesNet, and PatchTST on 24 of 32 benchmark datasets.

Conclusion: MSTN provides a unified framework for adaptive multi-scale temporal modeling that consistently outperforms existing methods across diverse temporal tasks, establishing new state-of-the-art performance.
...
### [261] [A Tale of Two Geometries: Adaptive Optimizers and Non-Euclidean Descent](https://arxiv.org/abs/2511.20584)
*Shuo Xie,Tianhao Wang,Beining Wu,Zhiyuan Li*

Main category: cs.LG

TL;DR: The paper reveals that adaptive optimizers reduce to normalized steepest descent and establishes adaptive smoothness as the key condition governing their convergence, enabling acceleration with momentum in non-Euclidean geometries where standard smoothness fails.


<details>
  <summary>Details</summary>
Motivation: To bridge the theoretical gap between adaptive optimizers and normalized steepest descent by identifying the geometric conditions (adaptive smoothness vs standard smoothness) that differentiate their convergence analyses, particularly in non-Euclidean settings.

Method: Extends adaptive smoothness theory to nonconvex optimization, analyzes convergence of adaptive optimizers under this condition, and introduces adaptive gradient variance for stochastic optimization with dimension-free guarantees.

Result: Shows that adaptive smoothness precisely characterizes adaptive optimizer convergence, enables momentum acceleration in convex non-Euclidean settings, and adaptive gradient variance provides dimension-free convergence where standard variance fails.

Conclusion: Adaptive smoothness and adaptive gradient variance are fundamental geometric conditions that unify and explain the behavior of adaptive optimizers, providing stronger convergence guarantees than standard smoothness in non-Euclidean geometries.
...
### [262] [Anatomica: Localized Control over Geometric and Topological Properties for Anatomical Diffusion Models](https://arxiv.org/abs/2511.20587)
*Karim Kadry,Abdallah Abdelwahed,Shoaib Goraya,Ajay Manicka,Naravich Chutisilp,Farhad Nezami,Elazer Edelman*

Main category: cs.LG

TL;DR: Anatomica is an inference-time framework for generating multi-class anatomical voxel maps with localized geometric and topological control using cuboidal domains and differentiable penalty functions.


<details>
  <summary>Details</summary>
Motivation: To enable rational design of synthetic anatomical datasets for virtual trials and machine learning workflows by providing flexible control over anatomical properties across diverse systems.

Method: Uses cuboidal control domains to slice substructures, computes differentiable penalty functions for geometric (voxel-wise moments) and topological (persistent homology) constraints, implemented for latent diffusion models with neural field decoders.

Result: Enables flexible control of anatomical properties across diverse systems, allowing composition of constraints for complex structures over arbitrary dimensions and coordinate systems.

Conclusion: Anatomica provides a powerful framework for generating anatomically realistic voxel maps with precise geometric and topological control, facilitating synthetic dataset creation for medical applications.
...
### [263] [Attention Trajectories as a Diagnostic Axis for Deep Reinforcement Learning](https://arxiv.org/abs/2511.20591)
*Charlotte Beylier,Hannah Selder,Arthur Fleig,Simon M. Hofmann,Nico Scherf*

Main category: cs.LG

TL;DR: ATOMs (attention-oriented metrics) help analyze RL agent attention development during training, showing distinct attention patterns for different game behaviors and revealing phased learning progression.


<details>
  <summary>Details</summary>
Motivation: To better understand the learning process of RL agents beyond mathematical formulations, specifically investigating how attention develops during training.

Method: Introduced ATOMs to monitor RL agent attention, tested on three Pong game variations designed to teach distinct behaviors, with behavioral assessment.

Result: ATOMs successfully identified distinct attention patterns for each game variation, showing these patterns translate to behavioral differences, and revealed consistent phased development of attention across all variations.

Conclusion: ATOMs can improve understanding of RL agent learning processes and the relationship between attention and learning.
...
### [264] [Latent Diffusion Inversion Requires Understanding the Latent Space](https://arxiv.org/abs/2511.20592)
*Mingxing Rao,Bowen Qu,Daniel Moyer*

Main category: cs.LG

TL;DR: The paper reveals non-uniform memorization in latent diffusion models, identifies dimensions most responsible for memorization, and shows that removing less-memorizing dimensions improves membership inference attack performance.


<details>
  <summary>Details</summary>
Motivation: Previous model inversion studies focused on data domain diffusion models, ignoring the encoder/decoder pair and latent codes in latent space generative models like LDMs. The influence of auto-encoder geometry on memorization has been overlooked.

Method: Introduced a principled method to rank latent dimensions by their per-dimensional contribution to the decoder pullback metric, identifying those most responsible for memorization. Removed less-memorizing dimensions when computing attack statistics for score-based membership inference.

Result: Significant improvement in membership inference attack performance: average AUROC gains of 2.7% and substantial increases in TPR@1%FPR (6.42%) across diverse datasets including CIFAR-10, CelebA, ImageNet-1K, Pokémon, MS-COCO, and Flickr.

Conclusion: The auto-encoder geometry significantly influences LDM memorization, providing a new perspective for analyzing privacy risks in diffusion-based generative models and enabling stronger confidence in identifying members under low false-positive tolerance.
...
### [265] [BrowseSafe: Understanding and Preventing Prompt Injection Within AI Browser Agents](https://arxiv.org/abs/2511.20597)
*Kaiyuan Zhang,Mark Tenenholtz,Kyle Polley,Jerry Ma,Denis Yarats,Ninghui Li*

Main category: cs.LG

TL;DR: This paper analyzes prompt injection attacks against AI web agents, creates a realistic benchmark, evaluates existing defenses, and proposes a multi-layered defense strategy.


<details>
  <summary>Details</summary>
Motivation: AI agents in web browsers face security challenges beyond traditional web threats, with prompt injection attacks being insufficiently understood in real-world impact.

Method: Created a benchmark of realistic HTML payloads for prompt injection attacks, conducted empirical evaluation of existing defenses across multiple AI models, and proposed multi-layered defense strategies.

Result: Developed a comprehensive benchmark that emphasizes real-world action influence rather than text outputs, and evaluated defense effectiveness across frontier AI models.

Conclusion: Proposes a blueprint for secure web agent design using defense-in-depth approach with both architectural and model-based defenses against evolving prompt injection threats.
...
TL;DR: Deep learning models for blood glucose forecasting fail to effectively use clinical drivers (insulin, meals, activity) despite known physiological mechanisms, a problem termed 'Driver-Blindness' quantified by Δ_drivers metric.


<details>
  <summary>Details</summary>
Motivation: Current blood glucose forecasting models don't leverage clinically important drivers despite clear physiological understanding, leading to suboptimal performance and limited clinical utility.

Method: Formalize Driver-Blindness via Δ_drivers metric, analyze three contributing factors: architectural biases favoring autocorrelation, data fidelity gaps making drivers noisy, and physiological heterogeneity undermining population models.

Result: Across literature, Δ_drivers is typically near zero, indicating models don't benefit from including clinical drivers. Proposed mitigation strategies include physiological feature encoders, causal regularization, and personalization.

Conclusion: Future work should routinely report Δ_drivers to prevent driver-blind models from being considered state-of-the-art, and implement strategies to address the identified contributing factors.
...
### [267] [Adaptive Hopfield Network: Rethinking Similarities in Associative Memory](https://arxiv.org/abs/2511.20609)
*Shurong Wang,Yuqi Pan,Zhuoyang Shen,Meng Zhang,Hongwei Wang,Guoqi Li*

Main category: cs.LG

TL;DR: The paper proposes adaptive similarity for associative memory models to ensure correct retrieval by learning to approximate the likelihood of stored patterns generating queries, integrated into an adaptive Hopfield network (A-Hop) that achieves state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing associative memory models evaluate retrieval quality based on proximity, which cannot guarantee that the retrieved pattern has the strongest association with the query, failing correctness.

Method: Develop adaptive similarity that learns to approximate the likelihood of each stored pattern generating the query according to a variant distribution, and integrate it into an adaptive Hopfield network (A-Hop).

Result: Theoretical proof that adaptive similarity achieves optimal correct retrieval under noisy, masked, and biased variants. Empirical results show state-of-the-art performance in memory retrieval, tabular classification, image classification, and multiple instance learning.

Conclusion: The proposed adaptive similarity mechanism enables correct retrieval in associative memory by learning context-dependent generative processes, outperforming existing methods across diverse tasks.
...
### [268] [DiFR: Inference Verification Despite Nondeterminism](https://arxiv.org/abs/2511.20621)
*Adam Karvonen,Daniel Reuter,Roy Rinberg,Luke Marks,Adrià Garriga-Alonso,Keri Warr*

Main category: cs.LG

TL;DR: Token-DiFR is a method for verifying LLM inference correctness by comparing generated tokens against a trusted reference implementation using synchronized random seeds, enabling detection of errors and deviations at zero additional cost.


<details>
  <summary>Details</summary>
Motivation: As LLM inference demand grows, providers need to verify inference processes are performed correctly without errors or tampering, but benign numerical noise makes it difficult to distinguish legitimate variation from actual problems.

Method: Token-DiFR compares generated tokens against predictions from a trusted reference implementation using synchronized random seeds. Also introduces Activation-DiFR using random orthogonal projections to compress activations into compact fingerprints for sample-efficient verification.

Result: Token-DiFR reliably identifies sampling errors, simulated bugs, and model quantization, detecting 4-bit quantization with AUC > 0.999 within 300 output tokens. Activation-DiFR detects 4-bit quantization with AUC > 0.999 using just 2 output tokens while reducing communication overhead by 25-75% relative to existing methods.

Conclusion: The methods provide practical verification of LLM inference correctness with high accuracy and efficiency, with an open-source integration with vLLM released to accelerate deployment of verifiable inference.
...
### [269] [ROOT: Robust Orthogonalized Optimizer for Neural Network Training](https://arxiv.org/abs/2511.20626)
*Wei He,Kai Han,Hang Zhou,Hanting Chen,Zhicheng Liu,Xinghao Chen,Yunhe Wang*

Main category: cs.LG

TL;DR: ROOT is a Robust Orthogonalized Optimizer that addresses robustness limitations in LLM optimization through dimension-robust orthogonalization and optimization-robust noise suppression, achieving faster convergence and superior performance in noisy scenarios.


<details>
  <summary>Details</summary>
Motivation: Current optimizers with momentum orthogonalization suffer from dimensional fragility in orthogonalization precision and vulnerability to outlier-induced noise, which becomes more critical as LLMs scale up.

Method: ROOT uses dual robustness mechanisms: (1) dimension-robust orthogonalization with adaptive Newton iterations and fine-grained coefficients for different matrix sizes, and (2) optimization-robust framework via proximal optimization to suppress outlier noise while preserving meaningful gradients.

Result: Extensive experiments show ROOT achieves significantly improved robustness, faster convergence, and superior final performance compared to Muon and Adam-based optimizers, especially in noisy and non-convex scenarios.

Conclusion: ROOT establishes a new paradigm for developing robust and precise optimizers capable of handling the complexities of modern large-scale model training.
...
### [270] [Image2Gcode: Image-to-G-code Generation for Additive Manufacturing Using Diffusion-Transformer Model](https://arxiv.org/abs/2511.20636)
*Ziyue Wang,Yayati Jadhav,Peter Pak,Amir Barati Farimani*

Main category: cs.LG

TL;DR: Image2Gcode is an end-to-end framework that generates 3D printer G-code directly from 2D images, bypassing traditional CAD modeling to accelerate prototyping.


<details>
  <summary>Details</summary>
Motivation: Traditional CAD-based design workflows are slow and create bottlenecks for rapid prototyping, requiring manual updates for even minor design variations.

Method: Uses a denoising diffusion probabilistic model (DDPM) to transform Gaussian noise into executable G-code sequences based on structural cues extracted from 2D images.

Result: Eliminates the need for CAD modeling and STL intermediates, enabling direct generation of printer-ready toolpaths from sketches or visual references.

Conclusion: Provides a flexible, efficient framework that lowers barriers to additive manufacturing and accelerates design-to-fabrication cycles for prototyping, repair, and distributed manufacturing.
...
### [271] [Using Wearable Devices to Improve Chronic PainTreatment among Patients with Opioid Use Disorder](https://arxiv.org/abs/2511.19577)
...
TL;DR: This pilot study examines AI approaches for predicting pain spikes in patients with chronic pain and opioid use disorder using wearable device data, finding machine learning models effective but LLMs limited.


<details>
  <summary>Details</summary>
Motivation: There's a lack of integrated treatments for chronic pain and opioid use disorder, and wearable devices combined with AI could help monitor pain variability and clinical correlates to improve treatment.

Method: Used wearable devices to monitor patient data and applied various AI approaches including machine learning models and large language models to analyze pain spikes and clinical correlates.

Result: Machine learning models achieved >0.7 accuracy in predicting pain spikes, while LLMs performed poorly and provided limited insights on pain spikes.

Conclusion: Real-time monitoring with wearables and advanced AI can enable early pain spike detection and personalized interventions, but LLMs need development to provide actionable insights in this clinical context.
...
### [272] [Fara-7B: An Efficient Agentic Model for Computer Use](https://arxiv.org/abs/2511.19663)
...
TL;DR: FaraGen is a synthetic data generation system for computer use agents that creates diverse multi-step web tasks and verified trajectories at low cost. The resulting Fara-7B model outperforms comparable CUA models and competes with larger frontier models.
...
Method: FaraGen proposes diverse tasks from frequently used websites, generates multiple solution attempts, and filters successful trajectories using multiple verifiers. The data trains Fara-7B, a native CUA model that uses screenshots for perception and predicted coordinates for action execution.

Result: Fara-7B outperforms other CUA models of comparable size on benchmarks like WebVoyager, Online-Mind2Web, and WebTailBench, and is competitive with much larger frontier models. Verified trajectories are produced at approximately $1 each.

Conclusion: Scalable data generation systems like FaraGen can advance small efficient agentic models, demonstrating key benefits for computer use agent development.
...
### [273] [HeaRT: A Hierarchical Circuit Reasoning Tree-Based Agentic Framework for AMS Design Optimization](https://arxiv.org/abs/2511.19669)
*Souradip Poddar,Chia-Tung Ho,Ziming Wei,Weidong Cao,Haoxing Ren,David Z. Pan*

Main category: cs.AI

TL;DR: HeaRT is a foundational reasoning engine for AMS design automation that achieves >97% reasoning accuracy and >98% Pass@1 performance across 40 circuits, with 3x faster convergence than SOTA baselines at <0.5x real-time token budget.


<details>
  <summary>Details</summary>
Motivation: Conventional AI-driven AMS design automation suffers from reliance on high-quality datasets, poor transferability across architectures, and lack of adaptive mechanisms.

Method: Proposes HeaRT, a foundational reasoning engine for automation loops that enables intelligent, adaptive, human-style design optimization.

Result: HeaRT achieves >97% reasoning accuracy and >98% Pass@1 performance across 40-circuit benchmark, with >3x faster convergence in sizing and topology design adaptation tasks while preserving prior design intent.

Conclusion: HeaRT represents a first step toward intelligent, adaptive AMS design automation that operates efficiently and effectively across diverse circuit architectures and optimization approaches.
...
### [274] [FISCAL: Financial Synthetic Claim-document Augmented Learning for Efficient Fact-Checking](https://arxiv.org/abs/2511.19671)
*Rishab Sharma,Iman Saberi,Elham Alipour,Jie JW Wu,Fatemeh Fard*

Main category: cs.AI

TL;DR: FISCAL framework generates synthetic financial data to train MiniCheck-FISCAL, a lightweight verifier that achieves state-of-the-art accuracy for financial fact-checking while being much smaller than competing models.


<details>
  <summary>Details</summary>
Motivation: Current LLMs for financial applications suffer from factual unreliability (hallucinations) and computational inefficiency due to large model sizes, limiting practical deployment.

Method: Proposed FISCAL framework for generating synthetic financial data, then trained MiniCheck-FISCAL - a lightweight verifier using this domain-specific synthetic data.

Result: MiniCheck-FISCAL outperforms GPT-3.5 Turbo and similar-sized open-source models, approaches accuracy of 20x larger models like Mixtral-8x22B, and rivals GPT-4o/Claude-3.5 on external datasets while being more efficient.
...
### [275] [Scaling Item-to-Standard Alignment with Large Language Models: Accuracy, Limits, and Solutions](https://arxiv.org/abs/2511.19749)
*Farzan Karimi-Malekabadi,Pooya Razavi,Sonya Powers*

Main category: cs.AI

TL;DR: LLMs can effectively automate educational item alignment reviews, achieving 83-94% accuracy in identifying misalignments and over 95% success in skill matching when combined with candidate filtering.


<details>
  <summary>Details</summary>
Motivation: Traditional human alignment reviews for educational assessments are accurate but slow and labor-intensive, especially for large item banks, creating a need for automated solutions.

Method: Tested three LLMs (GPT-3.5 Turbo, GPT-4o-mini, GPT-4o) on 12,000+ K-5 item-skill pairs across three tasks: identifying misalignments, selecting correct skills from standards, and pre-filtering candidate lists.

Result: GPT-4o-mini achieved 83-94% accuracy in identifying alignment status, strong math performance but lower reading performance due to semantic overlap, and over 95% success rate when using candidate filtering.

Conclusion: LLMs with candidate filtering can significantly reduce manual review burden while maintaining accuracy, recommending hybrid pipelines combining LLM screening with human review for ambiguous cases.
...
### [276] [Scaling Agentic Reinforcement Learning for Tool-Integrated Reasoning in VLMs](https://arxiv.org/abs/2511.19773)
...
TL;DR: VISTA-Gym is a scalable training environment that enables visual agentic reinforcement learning to enhance tool-integrated visual reasoning capabilities in vision-language models, with VISTA-R1-8B outperforming state-of-the-art baselines by 9.51%-18.72%.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models (VLMs) have strong image understanding but limited ability for multi-step visual reasoning and tool integration, struggling with tool selection, invocation, and coordination.

Method: VISTA-Gym provides a unified environment with standardized visual tool interfaces, executable interaction loops, verifiable feedback, and trajectory logging. VISTA-R1 is trained using multi-turn trajectory sampling and end-to-end reinforcement learning to interleave tool-use with agentic reasoning.

Result: VISTA-R1-8B achieves significant performance improvements, outperforming state-of-the-art baselines with similar sizes by 9.51%-18.72% across 11 public reasoning-intensive VQA benchmarks.

Conclusion: VISTA-Gym serves as an effective training ground to unlock tool-integrated reasoning capabilities for VLMs, demonstrating substantial improvements in visual reasoning performance through agentic reinforcement learning.
...
### [277] [NOEM$^{3}$A: A Neuro-Symbolic Ontology-Enhanced Method for Multi-Intent Understanding in Mobile Agents](https://arxiv.org/abs/2511.19780)
*Ioannis Tzachristas,Aifen Sui*

Main category: cs.AI

TL;DR: A neuro-symbolic framework that integrates structured intent ontology with compact language models for multi-intent understanding in mobile AI agents, achieving near-GPT-4 accuracy with much lower computational cost.


<details>
  <summary>Details</summary>
Motivation: To enable accurate and efficient on-device natural language understanding by combining symbolic intent structure with compact language models, addressing the computational limitations of large models for mobile deployment.

Method: Uses retrieval-augmented prompting, logit biasing, and optional classification heads to inject symbolic intent structure into both input and output representations of compact language models.

Result: A 3B Llama model with ontology augmentation achieves 85% accuracy on MultiWOZ 2.3 dialogues, approaching GPT-4's 90% accuracy while using a tiny fraction of energy and memory footprint.
...
### [278] [KOM: A Multi-Agent Artificial Intelligence System for Precision Management of Knee Osteoarthritis (KOA)](https://arxiv.org/abs/2511.19798)
...
TL;DR: KOM is a multi-agent AI system that automates knee osteoarthritis evaluation, risk prediction, and treatment prescription, reducing diagnostic time by 38.5% and improving treatment quality when collaborating with clinicians.


<details>
  <summary>Details</summary>
Motivation: Knee osteoarthritis affects over 600 million people globally, but personalized interventions require substantial medical resources, making them difficult to implement in resource-limited settings.

Method: Developed KOM, a multi-agent system that automates KOA evaluation, risk prediction, and treatment prescription through imaging analysis and prescription generation.

Result: KOM outperformed general-purpose LLMs in imaging analysis and prescription generation. Collaboration with clinicians reduced diagnostic time by 38.5% and improved treatment quality compared to independent approaches.

Conclusion: KOM can facilitate automated KOA management and enhance care efficiency when integrated into clinical workflows. Its modular architecture offers insights for developing AI-assisted systems for other chronic conditions.
...
### [279] [A Unified Evaluation-Instructed Framework for Query-Dependent Prompt Optimization](https://arxiv.org/abs/2511.19829)
*Ke Chen,Yifeng Wang,Hassan Almosapeeh,Haohan Wang*

Main category: cs.AI

TL;DR: The paper proposes an evaluation-instructed optimization pipeline that establishes a systematic prompt evaluation framework and uses a metric-aware optimizer to rewrite prompts in an interpretable, query-dependent manner, outperforming existing methods across diverse tasks.


<details>
  <summary>Details</summary>
Motivation: Existing prompt-optimization methods are ineffective in complex scenarios due to static templates, unstable feedback, and lack of unified prompt quality definitions, leading to fragmented evaluation signals.

Method: Developed a performance-oriented prompt evaluation framework, fine-tuned an execution-free evaluator to predict multi-dimensional quality scores, and created a metric-aware optimizer that diagnoses failure modes and rewrites prompts interpretably.

Result: The evaluator achieved strongest accuracy in predicting prompt performance, and the optimization pipeline consistently surpassed both static-template and query-dependent baselines across eight datasets and three backbone models.

Conclusion: The approach provides a unified, metric-grounded perspective on prompt quality and delivers stable, interpretable, model-agnostic improvements across diverse tasks.
...
### [280] [Reinforcement Learning with $ω$-Regular Objectives and Constraints](https://arxiv.org/abs/2511.19849)
*Dominik Wagner,Leon Witzman,Luke Ong*

Main category: cs.AI

TL;DR: This paper proposes combining ω-regular objectives with explicit constraints in reinforcement learning to address limitations of scalar rewards and enable separate treatment of safety requirements and optimization targets.


<details>
  <summary>Details</summary>
Motivation: Scalar rewards in RL have limited expressiveness for temporal, conditional, or safety-critical goals and can lead to reward hacking. Current approaches also mask safety-performance trade-offs by measuring performance with a single scalar.

Method: Developed a model-based RL algorithm based on linear programming that combines ω-regular objectives with explicit constraints. Also established a translation to constrained limit-average problems with optimality-preserving guarantees.

Result: The algorithm produces policies that maximize the probability of satisfying ω-regular objectives while adhering to ω-regular constraints within specified thresholds.

Conclusion: The approach successfully addresses both limitations of scalar rewards and masked safety-performance trade-offs by treating safety requirements and optimization targets separately through ω-regular objectives with constraints.
...
### [281] [MicroSims: A Framework for AI-Generated, Scalable Educational Simulations with Universal Embedding and Adaptive Learning Support](https://arxiv.org/abs/2511.19864)
*Valerie Lockhart,Dan McCreary,Troy A. Peterson*

Main category: cs.AI

TL;DR: MicroSims is a framework for creating lightweight, interactive educational simulations using AI that can be easily embedded across platforms and customized without programming.


<details>
  <summary>Details</summary>
Motivation: Traditional educational simulations require substantial resources and technical expertise, creating barriers to widespread adoption. MicroSims aims to make interactive simulations accessible by addressing cost, complexity, and platform dependence issues.

Method: The framework combines standardized design patterns for AI-assisted generation, iframe-based architecture for universal embedding and security, and transparent modifiable code for customization and pedagogical transparency.

Result: Interactive simulations can improve conceptual understanding by 30-40% compared to traditional instruction. MicroSims extends these benefits while overcoming traditional barriers.

Conclusion: MicroSims has significant implications for educational equity, enabling low-cost intelligent interactive textbooks and empowering educators worldwide to create customized, curriculum-aligned simulations on demand.
...
### [282] [Agentic AI-Empowered Conversational Embodied Intelligence Networks in 6G](https://arxiv.org/abs/2511.19865)
*Mingkai Chen,Zijie Feng,Lei Wang,Yaser Khamayseh*

Main category: cs.AI

TL;DR: CC-EIN is a collaborative conversational embodied intelligence network for 6G that integrates multimodal fusion, adaptive semantic communication, task coordination, and interpretability to improve complex task execution among multiple devices.


<details>
  <summary>Details</summary>
Motivation: Address challenges in multimodal information fusion, adaptive communication, and decision interpretability for semantic collaboration among multiple embodied intelligent devices in 6G networks.

Method: Proposes CC-EIN with four components: PerceptiNet for cross-modal fusion of image and radar data, adaptive semantic communication strategy, semantic-driven collaboration mechanism for task decomposition, and InDec module for decision interpretability using Grad-CAM visualization.

Result: Achieves 95.4% task completion rate and 95% transmission efficiency in post-earthquake rescue scenarios while maintaining strong semantic consistency and energy efficiency.

Conclusion: CC-EIN effectively addresses key challenges in semantic collaboration for MEIDs and demonstrates superior performance in complex emergency scenarios.
...
### [283] [Simulated Self-Assessment in Large Language Models: A Psychometric Approach to AI Self-Efficacy](https://arxiv.org/abs/2511.19872)
*Daniel I Jackson,Emma L Jensen,Syed-Amad Hussain,Emre Sezgin*

Main category: cs.AI

TL;DR: LLMs show stable but inaccurate self-efficacy assessments that don't correlate with actual task performance, with higher self-efficacy models using more anthropomorphic reasoning styles.
...
Method: Adapted 10-item General Self-Efficacy Scale (GSES) to test ten LLMs across four conditions (no task, computational reasoning, social reasoning, summarization), with follow-up confidence prompts and qualitative analysis.

Result: Models showed stable self-efficacy responses but scores were lower than human norms. Self-assessment didn't reflect actual ability - some low-scoring models performed well while high-scoring ones performed poorly. Higher self-efficacy correlated with more anthropomorphic reasoning.

Conclusion: Psychometric prompting provides insight into LLM communication behavior but not calibrated performance estimates, revealing a disconnect between self-assessment and actual capability.
...
### [284] [RPM-MCTS: Knowledge-Retrieval as Process Reward Model with Monte Carlo Tree Search for Code Generation](https://arxiv.org/abs/2511.19895)
*Yuanyuan Lin,Xiangyu Ouyang,Teng Zhang,Kaixin Sui*

Main category: cs.AI

TL;DR: RPM-MCTS is a tree search method that uses knowledge retrieval as a process reward model to evaluate intermediate algorithmic steps in code generation, reducing token consumption by 15% while improving performance.


<details>
  <summary>Details</summary>
Motivation: Existing tree search methods struggle with evaluating intermediate steps and correcting errors in code generation, leading to incorrect code and high computational costs.

Method: Uses Knowledge-Retrieval as Process Reward Model based on Monte Carlo Tree Search, similarity filtering to remove redundant nodes, and sandbox execution feedback to locate and correct errors.

Result: Outperforms state-of-the-art methods on four code generation benchmarks with 15% reduction in token consumption. Fine-tuning with RPM-MCTS data significantly enhances base model capabilities.

Conclusion: RPM-MCTS effectively addresses evaluation and correction challenges in code generation, achieving better performance with reduced computational costs while enabling model improvement through generated data.
...
### [285] [Semantic-KG: Using Knowledge Graphs to Construct Benchmarks for Measuring Semantic Similarity](https://arxiv.org/abs/2511.19925)
*Qiyao Wei,Edward Morrell,Lea Goetz,Mihaela van der Schaar*

Main category: cs.AI

TL;DR: This paper introduces a knowledge graph-based method to generate benchmarks for evaluating semantic similarity methods in LLM outputs, addressing limitations of existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current semantic similarity methods may prioritize syntactic/lexical features over semantic content, and existing benchmarks have high generation costs, limited domain availability, and unclear equivalence definitions.

Method: Leverages knowledge graphs to generate pairs of natural-language statements that are semantically similar or dissimilar, with dissimilar pairs categorized into four sub-types. Creates datasets across four domains (general knowledge, biomedicine, finance, biology).

Result: Performance of semantic similarity methods varies based on semantic variation sub-type and domain, with no method consistently superior. LLM-as-a-judge performance is impacted by these factors.

Conclusion: The study provides important implications for using LLM-as-a-judge in detecting semantic content of text, highlighting the need for domain-specific and variation-aware evaluation approaches.
...
### [286] [A System-Level Taxonomy of Failure Modes in Large Language Model Applications](https://arxiv.org/abs/2511.19933)
*Vaishali Vinay*
...
<details>
  <summary>Details</summary>
Motivation: LLMs are being rapidly integrated into production systems but their failure patterns differ from traditional ML models, and their behavior in production environments remains poorly understood, creating reliability concerns.

Method: The paper develops a system-level taxonomy of 15 hidden failure modes in LLM applications and analyzes current evaluation gaps, examining production deployment challenges and proposing design principles.

Result: Identified 15 specific failure modes including multi-step reasoning drift, latent inconsistency, context-boundary degradation, and cost-driven performance collapse, revealing significant gaps in current monitoring and evaluation practices.

Conclusion: LLM reliability should be approached as a system-engineering problem rather than purely model-centric, providing foundations for future research on evaluation methodology, AI system robustness, and dependable LLM deployment.
...
### [287] [M$^3$Prune: Hierarchical Communication Graph Pruning for Efficient Multi-Modal Multi-Agent Retrieval-Augmented Generation](https://arxiv.org/abs/2511.19969)
*Weizi Shao,Taolin Zhang,Zijie Zhou,Chen Chen,Chengyu Wang,Xiaofeng He*

Main category: cs.AI

TL;DR: M$^3$Prune is a multi-modal multi-agent hierarchical communication graph pruning framework that reduces token overhead and computational costs in multi-modal RAG systems while maintaining or improving performance.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent mRAG systems incur substantial token overhead and computational costs, posing challenges for large-scale deployment, despite their superior performance over single models.

Method: The framework applies intra-modal graph sparsification to identify critical edges, constructs dynamic communication topology, and progressively prunes redundant edges to create an efficient hierarchical topology across textual and visual modalities.

Result: Extensive experiments show the method consistently outperforms both single-agent and robust multi-agent mRAG systems while significantly reducing token consumption on general and domain-specific benchmarks.

Conclusion: M$^3$Prune achieves an optimal balance between task performance and token overhead, making multi-agent mRAG systems more practical for large-scale deployment.
...
### [288] [Reducing Latency of LLM Search Agent via Speculation-based Algorithm-System Co-Design](https://arxiv.org/abs/2511.20048)
...
TL;DR: SPAgent is a framework that uses adaptive speculation to reduce latency in LLM-based search agents by predicting simple early actions without full reasoning and selectively omitting verification when safe.


<details>
  <summary>Details</summary>
Motivation: LLM-based search agents suffer from severe latency due to serialized LLM reasoning and tool execution at each step, limiting their practical deployment despite strong performance.

Method: Algorithm-system co-design with two-phase adaptive speculation that predicts simple evidence-gathering actions without full reasoning, and a two-level scheduler to regulate speculative requests based on engine load.

Result: Achieves up to 1.65× end-to-end speedup while maintaining same or even higher accuracy across extensive experimental settings.

Conclusion: SPAgent enables practical deployment of multi-step search agents by significantly reducing latency through effective speculation mechanisms.
...
### [289] ["Are We Done Yet?": A Vision-Based Judge for Autonomous Task Completion of Computer Use Agents](https://arxiv.org/abs/2511.20067)
*Marta Sumyk,Oleksandr Kosovan*

Main category: cs.AI

TL;DR: Vision-based evaluation framework improves autonomous computer agents' task completion detection by 27% using screenshots and task descriptions.


<details>
  <summary>Details</summary>
Motivation: Computer Use Agents often fail to reliably determine task completion, needing better self-assessment mechanisms.

Method: Autonomous evaluation framework using vision-language models to assess task completion from screenshots and task descriptions across 42 macOS apps.

Result: Achieves 73% accuracy in task success detection and 27% relative improvement in overall task success with evaluator feedback.

Conclusion: Vision-based evaluation provides effective feedback mechanism for improving reliability and self-correction of autonomous computer-use agents.
...
### [290] [VICoT-Agent: A Vision-Interleaved Chain-of-Thought Framework for Interpretable Multimodal Reasoning and Scalable Remote Sensing Analysis](https://arxiv.org/abs/2511.20085)
*Chujie Wang,Zhiyuan Luo,Ruiqi Liu,Can Ran,Shenghua Fan,Xi Chen,Chu He*

Main category: cs.AI

TL;DR: VICoT is a multimodal agent framework that enhances remote sensing image analysis through explicit multi-round reasoning by integrating visual tools into chain-of-thought processes, with distillation methods to transfer capabilities to smaller models.


<details>
  <summary>Details</summary>
Motivation: Remote sensing image analysis is evolving from simple object recognition to complex intelligence reasoning, requiring improved model reasoning abilities and flexible tool invocation capabilities.

Method: Uses a stack-based reasoning structure with modular MCP-compatible tool suite to enable LLMs to perform multi-round, interleaved vision-language reasoning. Also employs Reasoning Stack distillation to migrate complex agent behaviors to lightweight models.
...
Conclusion: VICoT provides an effective framework for complex remote sensing reasoning tasks with strong generalization and flexibility while enabling efficient deployment through model distillation.
...
### [291] [From data to concepts via wiring diagrams](https://arxiv.org/abs/2511.20138)
*Jason Lo,Mohammadnima Jafari*

Main category: cs.AI

TL;DR: The paper introduces quasi-skeleton wiring diagram graphs that correspond to Hasse diagrams, develops algorithms to extract wiring diagrams from sequential data, and applies them to analyze autonomous agent behavior in games.


<details>
  <summary>Details</summary>
Motivation: To develop mathematical frameworks and algorithms for extracting wiring diagrams (labeled directed graphs representing temporal processes) from sequential data, particularly for analyzing autonomous agent behavior.

Method: Introduced quasi-skeleton wiring diagram graphs and proved their correspondence to Hasse diagrams. Designed algorithms using this theoretical foundation to extract wiring diagrams from sequential data. Compared performance with DBSCAN and agglomerative hierarchical clustering methods.

Result: The algorithms successfully identified winning strategies of an autonomous agent playing a computer game. Performance comparisons showed effectiveness even with perturbed data.

Conclusion: The work successfully integrates techniques from category theory, graph theory, clustering, reinforcement learning, and data engineering to provide a robust framework for analyzing temporal processes through wiring diagrams.
...
### [292] [Towards Benign Memory Forgetting for Selective Multimodal Large Language Model Unlearning](https://arxiv.org/abs/2511.20196)
*Zhen Zeng,Leijiang Gu,Zhangling Duan,Feng Li,Zenglin Shi,Cees G. M. Snoek,Meng Wang*

Main category: cs.AI

TL;DR: SMFA enables precise unlearning of sensitive information in MLLMs while preserving general image understanding capabilities through a memory forgetting adapter and retaining anchor-guided masking.


<details>
  <summary>Details</summary>
Motivation: Current unlearning methods for MLLMs degrade general image understanding performance when removing privacy-sensitive information, creating a need for more selective forgetting approaches.

Method: Proposes Sculpted Memory Forgetting Adapter (SMFA) that fine-tunes models to replace sensitive responses with refusals, then applies retaining anchor-guided masking to prevent interference with unrelated knowledge.

Result: SMFA achieves precise and controllable unlearning while maintaining foundational image understanding, outperforming prior methods that degrade general capabilities.

Conclusion: SMFA provides an effective solution for selective unlearning in MLLMs, enabling privacy protection without compromising overall model performance.
...
### [293] [Interactive AI NPCs Powered by LLMs: Technical Report for the CPDC Challenge 2025](https://arxiv.org/abs/2511.20200)
*Yitian Huang,Yuxuan Lei,Jianxun Lian,Hao Liao*

Main category: cs.AI

TL;DR: A winning solution for CPDC 2025 that uses context engineering and GRPO training to achieve top rankings across multiple tracks.


<details>
  <summary>Details</summary>
Motivation: To develop an effective framework for commonsense persona-grounded dialogue that works across both GPU and API tracks, addressing challenges in tool call stability, execution reliability, and role-playing guidance.

Method: Two key components: 1) Context Engineering with dynamic tool pruning, persona clipping, parameter normalization, and function merging; 2) GRPO training replacing supervised fine-tuning with reinforcement learning in GPU Track.

Result: Team ranked 1st in Task 2 API, 2nd in Task 1 API, and 3rd in both Task 3 API and GPU track in the final evaluation.

Conclusion: The proposed framework is simple yet effective, demonstrating strong performance across multiple tracks in the commonsense persona-grounded dialogue challenge.
...
### [294] [CostNav: A Navigation Benchmark for Cost-Aware Evaluation of Embodied Agents](https://arxiv.org/abs/2511.20216)
...
TL;DR: CostNav is the first micro-navigation economic testbed that evaluates autonomous delivery robots through cost-revenue analysis, revealing that optimizing for task success differs fundamentally from optimizing for economic viability.
...
Result: The baseline achieves 43.0% SLA compliance but is not commercially viable, yielding a loss of $30.009 per run with no finite break-even point, as operating costs are dominated by collision-induced maintenance (99.7% of per-run costs).

Conclusion: CostNav bridges the gap between navigation research and commercial deployment by enabling data-driven decisions about economic trade-offs across navigation paradigms, highlighting collision avoidance as a key optimization target.
...
### [295] [Actionable and diverse counterfactual explanations incorporating domain knowledge and causal constraints](https://arxiv.org/abs/2511.20236)
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of existing counterfactual explanation methods that ignore complex dependencies in real-world datasets, leading to unrealistic modifications, particularly motivated by cybersecurity applications in email marketing.

Method: Learns linear and nonlinear constraints from data or integrates expert-provided dependency graphs to maintain consistency with feature relationships, balancing plausibility, diversity, and sparsity.

Result: Outperforms existing approaches on 140 public datasets, generating meaningful domain-relevant counterfactuals that align with real-world constraints.

Conclusion: DANCE effectively addresses key limitations in existing algorithms by producing plausible, actionable explanations that respect feature dependencies and causal constraints.
...
### [296] [SMoG: Schema Matching on Graph](https://arxiv.org/abs/2511.20285)
*Mingyu Jeon,Jaeyoung Suh,Suwan Cho*

Main category: cs.AI

TL;DR: SMoG is a novel framework for schema matching that uses iterative 1-hop SPARQL queries on knowledge graphs to enhance reliability and reduce storage requirements compared to existing LLM-based approaches.


<details>
  <summary>Details</summary>
Motivation: LLMs suffer from hallucination and outdated domain knowledge in schema matching, while existing KG-augmented approaches use inefficient multi-hop queries or storage-intensive vector retrieval methods.

Method: SMoG leverages iterative execution of simple 1-hop SPARQL queries inspired by KGQA strategies, directly querying SPARQL endpoints to generate human-verifiable query paths.

Result: Experimental results on real-world medical datasets show SMoG achieves performance comparable to state-of-the-art baselines.

Conclusion: SMoG provides an effective and efficient solution for KG-augmented schema matching with enhanced explainability and reduced storage requirements.
...
### [297] [Improving Language Agents through BREW](https://arxiv.org/abs/2511.20297)
*Shashank Kirtania,Param Biyani,Priyanshu Gupta,Yasharth Bajpai,Roshni Iyer,Sumit Gulwani,Gustavo Soares*

Main category: cs.AI

TL;DR: BREW is a framework that optimizes LLM-based agents through structured memory construction and refinement, achieving improved task precision and efficiency without weight optimization.


<details>
  <summary>Details</summary>
Motivation: Current training methods like PPO and GRPO have high computational overhead and produce opaque agent policies that are difficult to interpret or improve incrementally.

Method: BREW creates and refines structured memory from agent experiences, using task partitioning, graders, behavior rubrics, and state-space search for robust learning.

Result: 10-20% improvement in task precision, 10-15% reduction in API/tool calls with faster execution, while maintaining computational efficiency comparable to base models.

Conclusion: BREW establishes knowledge bases as modular, controllable substrates for transparent and interpretable agent optimization, unlike static memory approaches.
...
### [298] [Data Augmentation Techniques to Reverse-Engineer Neural Network Weights from Input-Output Queries](https://arxiv.org/abs/2511.20312)
*Alexander Beiser,Flavio Martinelli,Wulfram Gerstner,Johanni Brea*

Main category: cs.AI

TL;DR: The paper addresses the challenge of reverse-engineering neural network weights when teacher parameters outnumber training data, proposing novel data augmentation techniques that better sample hidden layer representations to improve parameter recovery.


<details>
  <summary>Details</summary>
Motivation: Current methods fail to recover teacher network parameters when they exceed training data size, as students overfit to queries instead of aligning with teacher parameters. The goal is to develop better sampling techniques for the teacher's input-output mapping.

Method: The authors design new data augmentation techniques specifically tailored to better sample the representational space of network hidden layers, moving beyond standard augmentations like rotation, flipping, and noise addition.

Result: The proposed augmentations extend the state-of-the-art range of recoverable network sizes, enabling recovery of networks with up to 100 times more parameters than training data-points.

Conclusion: Custom-designed data augmentations that target hidden layer representations are crucial for effective network parameter recovery when teacher parameters significantly outnumber available training data.
...
### [299] [Active Inference in Discrete State Spaces from First Principles](https://arxiv.org/abs/2511.20321)
*Patrick Kenny*

Main category: cs.AI

TL;DR: The paper disentangles active inference from the Free Energy Principle, showing it can be implemented as constrained divergence minimization using standard mean field methods without expected free energy.


<details>
  <summary>Details</summary>
Motivation: To clarify the concept of active inference by separating it from the Free Energy Principle framework.

Method: Formulate active inference optimizations as constrained divergence minimization problems solvable by standard mean field methods, using a perception/action divergence criterion.

Result: The proposed divergence criterion coincides with variational free energy for perception modeling, but differs from expected free energy for action modeling by including an entropy regularizer.

Conclusion: Active inference can be implemented independently of the Free Energy Principle through constrained divergence minimization approaches.
...
### [300] [NNGPT: Rethinking AutoML with Large Language Models](https://arxiv.org/abs/2511.20333)
...
TL;DR: NNGPT is an open-source framework that transforms LLMs into self-improving AutoML engines for neural network development, using a closed-loop system of generation, assessment, and self-improvement to continuously expand neural network datasets.


<details>
  <summary>Details</summary>
Motivation: To address the fundamental challenge of building self-improving AI systems by creating an autonomous AutoML engine that can continuously generate and improve neural networks for computer vision tasks.

Method: Integrates five LLM-based pipelines: zero-shot architecture synthesis, hyperparameter optimization, code-aware accuracy prediction, retrieval-augmented synthesis of PyTorch blocks (NN-RAG), and reinforcement learning within a unified workflow using the LEMUR dataset.

Result: Achieved 73% executability on 1,289 targets, HPO RMSE of 0.60 (outperforming Optuna's 0.64), code-aware predictor RMSE of 0.14 with Pearson r=0.78, and generated over 5K validated models with one-shot prediction matching search-based AutoML performance.

Conclusion: NNGPT successfully demonstrates autonomous AutoML capabilities, proving LLMs can be transformed into self-improving engines for neural network development, with plans to release code, prompts, and checkpoints for community use.
...
### [301] [VibraVerse: A Large-Scale Geometry-Acoustics Alignment Dataset for Physically-Consistent Multimodal Learning](https://arxiv.org/abs/2511.20422)
*Bo Pang,Chenxi Xu,Jierui Ren,Guoping Wang,Sheng Li*

Main category: cs.AI

TL;DR: VibraVerse is a large-scale dataset that bridges 3D geometry, physical properties, and acoustic signals through causal relationships, enabling physically consistent multimodal learning.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal frameworks lack physical consistency and ignore causal relationships between object geometry, material properties, and the sounds they produce, limiting their ability to understand the physical world.

Method: Created VibraVerse dataset with explicit physical properties and volumetric geometry, computed modal parameters for sound synthesis, and developed CLASP contrastive learning framework for cross-modal alignment that preserves causal correspondence.
...
### [302] [Latent-space metrics for Complex-Valued VAE out-of-distribution detection under radar clutter](https://arxiv.org/abs/2511.19805)
*Y. A. Rouzoumka,E. Terreaux,C. Morisseau,J. -P. Ovarlez,C. Ren*

Main category: eess.SP

TL;DR: Complex-valued Variational AutoEncoders (CVAE) for radar OOD detection with multiple metrics compared against classical detector.


<details>
  <summary>Details</summary>
Motivation: To improve radar Out-Of-Distribution detection in complex radar environments using deep learning approaches.

Method: Proposed CVAE with multiple detection metrics: reconstruction error (CVAE-MSE), latent-based scores (Mahalanobis, KLD), and compared with ANMF-Tyler detector.

Result: Performance analyzed on synthetic and experimental radar data, showing advantages and weaknesses of each detector.

Conclusion: CVAE-based approaches provide viable alternatives for radar OOD detection with different trade-offs compared to classical methods.
...
### [303] [White-Box Modeling of V2X Link Performance Using Stabilized Symbolic Regression](https://arxiv.org/abs/2511.19809)
*Rahul Gulia,Feyisayo Favour Popoola,Ashish Sheikh*

Main category: eess.SP

TL;DR: Proposes a stabilized symbolic regression framework for interpretable block error rate prediction in V2X networks, achieving comparable accuracy to neural networks with ultra-fast inference suitable for embedded systems.


<details>
  <summary>Details</summary>
Motivation: Traditional ML approaches like DNNs lack interpretability and have high computational costs, limiting real-time applicability in resource-constrained V2X environments.

Method: Uses stabilized symbolic regression to derive compact analytical expressions for BLER prediction, trained on realistic V2X simulation data capturing nonlinear dependencies on SNR, velocity, MCS, DMRS symbols, and environmental factors.

Result: Achieves R² = 0.8684 and MSE = 2.08×10⁻² with only 158 nodes, outperforming fixed-form regressions and offering comparable accuracy to neural networks while being fully interpretable.

Conclusion: The framework combines predictive performance, physical fidelity, and computational efficiency, providing a powerful tool for real-time V2X communication system design and adaptive resource allocation.
...
### [304] [Parallel Delay-Doppler Estimation via Order-Reversed Two-Stage Prony Method](https://arxiv.org/abs/2511.19866)
*Yutaka Jitsumatsu,Liangchen Sun*

Main category: eess.SP

TL;DR: A parallel two-stage method using Prony analysis for delay-Doppler estimation in OTFS systems that resolves ambiguities through parallel delay-first and Doppler-first estimation with result fusion.


<details>
  <summary>Details</summary>
Motivation: To address ambiguities caused by similar path characteristics in delay-Doppler estimation for OTFS systems, which is crucial for applications like V2V and ISAC.

Method: Prony-based parallel two-stage approach performing delay-first and Doppler-first estimations simultaneously, followed by fusion of results to resolve estimation ambiguities.

Result: Superior accuracy and robustness demonstrated through simulations under various conditions compared to existing methods.

Conclusion: The proposed method provides a promising solution for delay-Doppler estimation in future applications including Vehicle-to-Vehicle communication and Integrated Sensing and Communication systems.
...
### [305] [Joint Classification and Regression Deep Learning Model for Universal Phase-based Ranging in Multiple Environments](https://arxiv.org/abs/2511.19891)
*Pantelis Stefanakis,Ming Shen*

Main category: eess.SP

TL;DR: Neural Network models, especially a novel 2NN Model combining environment classification and distance prediction, outperform traditional methods for Phase-Based Ranging across diverse environments, achieving superior accuracy with lower RMSE and maximum error.


<details>
  <summary>Details</summary>
Motivation: To leverage Neural Networks for accurate Phase-Based Ranging (PBR) that offers high accuracy over large distances without requiring antenna arrays, and to compare NN-based approaches with established non-NN methods across different environments.

Method: Proposed a novel 2NN Model with two neural networks: one for environment classification (Openfield, Office, Near Buildings) and another for distance prediction. Evaluated performance using RMSE and maximum error over 20 trials, including filtered analysis to assess misclassification impact.

Result: The 2NN Model consistently outperformed other methods, achieving best average RMSE and lowest maximum error. Filtered analysis showed significant impact of misclassifications, but the filtered 2NN variant achieved lowest RMSE and maximum error across all datasets.

Conclusion: NN models provide robust, high-accuracy ranging across diverse environments, outperforming non-NN methods and demonstrating potential as universal PBR solutions when trained on comprehensive datasets.
...
### [306] [AI/ML based Joint Source and Channel Coding for HARQ-ACK Payload](https://arxiv.org/abs/2511.19943)
*Akash Doshi,Pinar Sen,Kirill Ivanov,Wei Yang,June Namgoong,Runxin Wang,Rachel Wang,Taesang Yoo,Jing Jiang,Tingfang Ji*

Main category: eess.SP

TL;DR: Proposes transformer-based joint source-channel coding with power shaping for non-uniform HARQ-ACK bits, achieving 3-6 dB power reduction compared to 5G NR baseline.


<details>
  <summary>Details</summary>
Motivation: HARQ-ACK bits are inherently non-uniformly distributed, but current channel coding assumes uniform bit distribution, missing potential performance gains from exploiting source statistics.

Method: Transformer-based encoder with 'free-lunch' training algorithm, per-codeword power shaping, and Neyman-Pearson test extension for unequal error protection of NACK over ACK bits.

Result: Achieves 3-6 dB reduction in average transmit power and 2-3 dB reduction in maximum transmit power compared to 5G NR baseline while meeting target error rates.

Conclusion: The proposed joint source-channel coding approach provides significant coverage gains and power savings for uplink HARQ-ACK transmission in 5G systems.
...
### [307] [Cross-Modal Semantic Communication for Heterogeneous Collaborative Perception](https://arxiv.org/abs/2511.20000)
*Mingyi Lu,Guowei Liu,Le Liang,Chongtao Guo,Hao Ye,Shi Jin*

Main category: eess.SP

TL;DR: A cross-modal semantic communication framework that transforms heterogeneous sensor data into a unified semantic space to enable effective collaborative perception among connected autonomous vehicles with different sensor configurations.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of sensor heterogeneity in connected autonomous vehicles (CAVs) that complicates collaborative perception systems and impedes effective information fusion across different sensor modalities.

Method: Proposes a cross-modal semantic communication (CMSC) framework that transforms heterogeneous perceptual features from different sensor modalities into a unified semantic space, then performs encoding, transmission, and decoding within this standardized space.

Result: Extensive experiments show CMSC achieves significantly stronger perception performance than existing methods, particularly in low signal-to-noise ratio (SNR) regimes.

Conclusion: The CMSC framework effectively enables seamless and effective information fusion across heterogeneous sensor configurations in collaborative perception systems for autonomous driving.
...
### [308] [Redefining Radar Segmentation: Simultaneous Static-Moving Segmentation and Ego-Motion Estimation using Radar Point Clouds](https://arxiv.org/abs/2511.20003)
*Simin Zhu,Satish Ravindran,Alexander Yarovoy,Francesco Fioranelli*

Main category: eess.SP

TL;DR: A neural network approach for simultaneous segmentation of static/moving objects and ego-motion estimation from raw radar point clouds, using simple MLPs and RNNs without preprocessing.


<details>
  <summary>Details</summary>
Motivation: Radar perception in automotive requires distinguishing moving vs static objects as a prerequisite for most tasks, but conventional methods focus on category labels which are unreliable due to radar sensor limitations.

Method: Uses multi-layer perceptrons (MLPs) and recurrent neural networks (RNNs) to process unprocessed radar point clouds directly, without cloud aggregation, Doppler compensation, or motion compensation.

Result: Successfully performs dual tasks of object segmentation and ego-motion estimation on challenging RadarScenes dataset, demonstrating feasibility of direct point cloud processing.

Conclusion: The method shows strong performance on dual tasks and has broad application potential in other radar perception tasks, being the first of its kind in literature.
...
### [309] [Sparse MIMO-OFDM Channel Estimation via RKHS Regularization](https://arxiv.org/abs/2511.20082)
*James Delfeld,Gian Marti,Chris Dick*

Main category: eess.SP

TL;DR: A novel channel estimation method for MIMO-OFDM systems that exploits channel sparsity in delay-beamspace domain using RKHS optimization and deep-unfolding techniques.


<details>
  <summary>Details</summary>
Motivation: To improve channel estimation accuracy in MIMO-OFDM wireless systems by leveraging the band-sparsity property of wireless channels, overcoming limitations of traditional linear methods like LMMSE.

Method: Formulates channel estimation as a regularized optimization problem in RKHS, transforms it to finite dimensions using representer theorem, solves with FBS algorithm, and extends with data-driven deep-unfolding for faster convergence.

Result: Significantly outperforms linear methods (LMMSE) in both raw estimation accuracy and downstream performance on ray-traced channels from SionnaRT, with quasi-linear computational complexity per iteration.

Conclusion: The proposed RKHS-based channel estimation with deep-unfolding extension provides superior performance over conventional linear methods while maintaining computational efficiency in MIMO-OFDM systems.
...
### [310] [Joint Bit-Partitioning and Modulation Design for Digital AirComp](https://arxiv.org/abs/2511.20113)
*Xiaojing Yan,Carlo Fischione*

Main category: eess.SP

TL;DR: This paper integrates bit-partitioning into ChannelComp framework to reduce modulation design complexity and improve computation reliability for digital over-the-air computation. It develops two methods: uniform bit-partitioning and importance-adaptive bit-partitioning (IABP), with IABP achieving up to 5 dB error reduction.


<details>
  <summary>Details</summary>
Motivation: To reduce modulation design complexity while increasing computation reliability for digital over-the-air computation in the ChannelComp framework.

Method: Two bit-partitioning methods: 1) Uniform bit-partitioning with max-min optimization handled by CCCP solving SOCP subproblems; 2) Importance-adaptive bit-partitioning (IABP) that jointly optimizes partitioning and modulation using simulated annealing in outer loop and CCCP-based solver in inner loop.

Result: Both methods provide robust computation in noisy channels, with IABP achieving up to 5 dB reduction in computation error compared to Sequential Modulation for AirComp, especially for product computation.

Conclusion: Bit-partitioning integration into ChannelComp successfully reduces design complexity and improves computation reliability, with IABP showing superior performance through adaptive bit allocation based on bit position significance.
...
### [311] [Optimal Waveform Design for Continuous Aperture Array (CAPA)-aided ISAC Systems](https://arxiv.org/abs/2511.20203)
*Junjie Ye,Zhaolin Wang,Yuanwei Liu,Peichang Zhang,Lei Huang,Arumugam Nallanathan*

Main category: eess.SP

TL;DR: Proposes a continuous-aperture-array (CAPA) framework for integrated sensing and communication (ISAC) with optimal waveform design that balances sensing beampattern and multi-user interference suppression.


<details>
  <summary>Details</summary>
Motivation: To improve integrated sensing and communication performance by leveraging continuous-aperture arrays instead of conventional discrete arrays, addressing the tradeoff between sensing accuracy and communication reliability.

Method: Derives CAPA directional beampattern using Green's function, obtains reference sensing waveform via wavenumber-domain optimization, formulates weighted functional programming for tradeoff optimization, and analytically derives optimal waveform structure using Lagrangian-transformation and calculus-of-variations.

Result: Numerical results show CAPA achieves significant performance gains over conventional discrete arrays in both sensing accuracy and communication reliability.

Conclusion: The proposed CAPA-ISAC framework with optimal waveform design effectively balances sensing and communication requirements, outperforming traditional discrete array approaches.
...
### [312] [Rectified Flow for Vision-Aided mmWave V2I Beam Prediction](https://arxiv.org/abs/2511.20265)
*Can Zheng,Jiguang He,Chung G. Kang,Guofa Cai,Chongwen Huang,Henk Wymeersch*

Main category: eess.SP

TL;DR: Proposes a flow matching framework using rectified flow for vision-aided beam prediction in V2I links, enabling smooth beam trajectories and fast sampling with significant performance improvements and speedups.


<details>
  <summary>Details</summary>
Motivation: To improve beam prediction in vehicle-to-infrastructure communication by moving from discrete beam index sequences to continuous latent flows, enabling smoother trajectories and faster inference.

Method: Uses flow matching with rectified flow to learn a continuous latent flow governed by an ODE-based vector field, with terminal flow constraint for global consistency under finite-step integration.

Result: Significantly improves top-K accuracy over RNN and LSTM baselines, approaches large language model performance, and achieves 10x GPU and 10^4x CPU inference speedups.

Conclusion: The flow matching framework provides an effective approach for vision-aided beam prediction with superior accuracy and substantial computational efficiency gains.
...
### [313] [Log-Mu Fading Process: Second-Order Statistics for Diversity-Combining Techniques](https://arxiv.org/abs/2511.20298)
*Godfred Kumi Tenkorang,Michel Daoud Yacoub*

Main category: eess.SP

TL;DR: Derives second-order statistics for diversity-combining techniques over Log-mu fading channels, providing closed-form expressions for PSC and integral expressions for EGC/MRC.


<details>
  <summary>Details</summary>
Motivation: To analyze the performance of diversity-combining techniques in Log-mu fading environments, which is important for wireless communication systems operating in challenging propagation conditions.

Method: Mathematical derivation of closed-form expressions for PSC and exact multidimensional integral expressions for EGC and MRC, considering M unbalanced, independent, non-identically distributed Log-mu fading channels, with validation through Monte Carlo simulations.

Result: Successfully derived analytical expressions for LCR and AFD, with Monte Carlo simulations showing excellent agreement with theoretical results, confirming the accuracy of the proposed expressions.

Conclusion: The derived expressions provide accurate second-order statistics for diversity-combining techniques in Log-mu fading channels, enabling better performance analysis of wireless communication systems in such environments.
...
### [314] [Next-Generation MIMO Transceivers for Integrated Sensing and Communications: Unique Security Vulnerabilities and Solutions](https://arxiv.org/abs/2511.20309)
*Kawon Han,Christos Masouros,Taneli Riihonen,Moeness G. Amin*

Main category: eess.SP

TL;DR: Overview of MIMO ISAC transceiver design advances and security vulnerabilities in integrated sensing and communications for 6G networks.


<details>
  <summary>Details</summary>
Motivation: ISAC enables intelligent wireless networks but introduces unique security threats beyond conventional PLS, where sensing transmissions can empower eavesdroppers and passive interception can reveal sensitive target information.

Method: Examines MIMO transceiver designs from transmitter perspectives, receiver architectures, and full-duplex implementations, considering security threats specific to ISAC.

Result: Identifies emerging countermeasures including secure signaling design, interference exploitation, and transceiver optimization under adversarial conditions.

Conclusion: Discusses challenges and research opportunities for developing secure ISAC systems in next-generation wireless networks.
...
### [315] [Bridging the Educational Divide: A Delay-Tolerant Networking Approach for Equitable Digital Learning in Rural Areas](https://arxiv.org/abs/2511.20334)
*Salah Abdeljabar,Mohamed-Slim Alouini*

Main category: eess.SP

TL;DR: A DTN-based digital learning platform for underserved areas with limited internet access, using transportation infrastructure to deliver educational content.


<details>
  <summary>Details</summary>
Motivation: Address educational inequality in rural areas with poor internet connectivity by providing an affordable, sustainable solution that promotes educational equity and digital inclusion.

Method: Developed a framework using Delay Tolerant Networking (DTN) that leverages existing transportation infrastructure to deliver educational content without requiring continuous internet access.

Result: A prototype was successfully tested on a university campus, demonstrating the feasibility of using DTN for educational delivery in disconnected environments.

Conclusion: The DTN framework provides a viable solution to bridge the digital divide, supporting global goals of inclusive education and sustainable development in underserved communities.
...
### [316] [Digital Twin-Assisted High-Precision Massive MIMO Localization in Urban Canyons](https://arxiv.org/abs/2511.20453)
*Ziqin Zhou,Hui Chen,Gerhard Steinböck,Henk Wymeersch*

Main category: eess.SP

TL;DR: A robust three-stage wireless localization algorithm that combines digital twin models with RANSAC to handle NLOS propagation in urban canyons, enabling accurate positioning with reduced reliance on direct LOS paths.


<details>
  <summary>Details</summary>
Motivation: High-precision wireless localization in urban canyons faces challenges from noisy measurements and severe non-line-of-sight propagation, which degrade positioning accuracy.

Method: Three-stage algorithm synergizing digital twin model with RANSAC: uses DT for geometric path association, RANSAC to identify reliable LOS and single-bounce NLOS paths while rejecting multi-bounce outliers, and final optimization on inlier set.

Result: Simulations validate that the approach enables accurate localization by effectively turning NLOS paths into valuable geometric information, reduces reliance on direct LOS, and significantly lowers system deployment costs.

Conclusion: The proposed method is suitable for practical deployment in urban environments by overcoming NLOS limitations and reducing system costs.
...
### [317] [Near-Field Multipath MIMO Channels: Modeling Reflectors and Exploiting NLOS Paths](https://arxiv.org/abs/2511.20572)
*Mohamadreza Delbari,George C. Alexandropoulos,Robert Schober,H. Vincent Poor,Vahid Jamali*

Main category: eess.SP

TL;DR: The paper develops a generalized statistical near-field MIMO channel model that accounts for imperfect reflections from large surfaces, showing that NLOS components are essential for achieving multiplexing gains even in near-field scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing near-field channel models for NLOS links are based on point scattering assumptions that don't account for large reflectors like walls and ceilings, which limits their accuracy in practical MIMO systems.

Method: Developed a generalized statistical NF MIMO channel model that extends point scattering framework to include imperfect reflections from large surfaces, then analyzed how physical characteristics of reflectors influence the channel.

Result: Simulation results validate the proposed model's accuracy and show that NLOS components are non-negligible in many practical settings, requiring careful consideration in system design.

Conclusion: Relying solely on LOS near-field links is insufficient for achieving multiplexing gains in multi-user scenarios; exploiting NLOS links becomes essential even when users are in the near-field regime.
...
### [318] [The Quality of Information: A Weighted Entropy Approach to Near-Optimal Mastermind](https://arxiv.org/abs/2511.19446)
*Serkan Gür*

Main category: cs.IT

TL;DR: Novel information-theoretic strategies for Mastermind using weighted entropy heuristics achieve near-optimal performance with 4.3488 average guesses.


<details>
  <summary>Details</summary>
Motivation: To develop improved heuristic methods for solving Mastermind that approach theoretical optimum performance while maintaining computational efficiency.

Method: Uses weighted entropy heuristic based on Belis-Guias framework with context-dependent utility values, optimized via genetic algorithm to discover interpretable weight patterns. Introduces stage-weighted heuristic with distinct utility vectors for each turn.

Result: Single fixed weight vector achieves 4.3565 average guesses (max 5), while stage-weighted version achieves 4.3488 average guesses (max 6), approaching theoretical optimum of 4.3403 by less than 0.2%.

Conclusion: The method significantly improves Mastermind solving performance through principled information valuation while retaining computational efficiency of classical one-step-ahead heuristics.
...
### [319] [The Semiotic Channel Principle: Measuring the Capacity for Meaning in LLM Communication](https://arxiv.org/abs/2511.19550)
*Davide Picca*

Main category: cs.IT

TL;DR: A semiotic framework for analyzing LLMs as stochastic engines, formalizing the trade-off between expressive richness and interpretive stability using information theory.


<details>
  <summary>Details</summary>
Motivation: To understand LLMs as communication systems by shifting focus from opaque model internals to observable textual artifacts, enabling empirical measurement of meaning transmission.

Method: Formalize breadth (source entropy) and decipherability (mutual information) trade-off using generative complexity parameter lambda. Define semiotic channel with capacity constraint for meaning transmission.

Result: Framework enables model profiling, prompt optimization, risk analysis based on ambiguity, and design of adaptive semiotic systems through empirical measurement.

Conclusion: Capacity-based semiotic approach provides rigorous, actionable toolkit for understanding, evaluating, and designing LLM-mediated communication systems.
...
### [320] [One-Shot Coding and Applications](https://arxiv.org/abs/2511.19556)
*Yanxiao Liu*

Main category: cs.IT

TL;DR: Extends Poisson functional representation method to handle more complex one-shot information theory scenarios where the original version fails.


<details>
  <summary>Details</summary>
Motivation: To develop explicit coding schemes for one-shot information theory where sources and channels are used only once and can be arbitrary (not necessarily memoryless or ergodic), and to derive achievability results that imply existing asymptotic results.

Method: Extends the Poisson functional representation technique, originally developed for channel simulation and unified coding schemes, to handle more complicated scenarios through further extensions and modifications.

Result: Successfully applies Poisson functional representation to various complex one-shot information theory problems where the original method was insufficient.

Conclusion: The Poisson functional representation method can be effectively extended to address challenging one-shot information theory scenarios, providing a powerful tool for deriving achievability results that bridge to asymptotic performance.
...
### [321] [A Hybrid Dominant-Interferer Approximation for SINR Coverage in Poisson Cellular Networks](https://arxiv.org/abs/2511.19568)
*Sunder Ram Krishnan,Junaid Farooq,Kumar Vijay Mishra,Xingchen Liu,S. Unnikrishna Pillai,Theodore S. Rappaport*

Main category: cs.IT

TL;DR: Hybrid approximation framework combining Monte Carlo sampling of dominant interferers with Laplace functional representation of residual far-field interference for accurate radio propagation modeling.


<details>
  <summary>Details</summary>
Motivation: Existing stochastic geometry and probabilistic interference models have limitations - stochastic geometry involves complex nested integrals and special functions, while probabilistic models have restricted applicability and become unwieldy when interference moments lack closed-form representations.

Method: Combines Monte Carlo sampling of a small set of dominant interferers with a Laplace functional representation of the residual far-field interference, creating a dominant-plus-tail structure that is modular, numerically stable, and path-loss-agnostic.

Result: Provides a framework suitable for both noise-limited and interference-limited regimes, with theoretical error bounds that decrease with the number of dominant interferers. Validated against established stochastic geometry and probabilistic modeling benchmarks.

Conclusion: The hybrid approximation framework successfully addresses limitations of existing approaches by providing a more general, numerically stable, and path-loss-agnostic estimator for radio propagation and interference modeling in cellular networks.
...
### [322] [Computer-aided Characterization of Fundamental Limits of Coded Caching with Linear Coding](https://arxiv.org/abs/2511.19639)
*Niccolò Brembilla,Yinbin Ma,Pietro Belotti,Federico Malucelli,Daniela Tuninetti*

Main category: cs.IT

TL;DR: Efficient computer-aided framework for characterizing fundamental limits of coded caching systems under linear coding constraints using non-Shannon inequalities and problem-specific optimizations.


<details>
  <summary>Details</summary>
Motivation: To develop tighter converse bounds for coded caching systems by considering linear coding constraints and leveraging representable polymatroids, improving upon previous analytic methods.

Method: Uses non-Shannon-type inequalities valid for representable polymatroids, incorporates symmetric structure and problem-specific constraints to reduce linear program complexity, and applies minimal common information constructions.

Result: Derived converse bounds are tighter than previous methods, proving optimality of some memory-load tradeoff points under linear coding constraints for placement and delivery.

Conclusion: Small, structured demand subsets with minimal common information constructions may be sufficient to characterize optimal tradeoffs under linear coding in coded caching systems.
...
### [323] [Joint Satellite Power Consumption and Handover Optimization for LEO Constellations](https://arxiv.org/abs/2511.19745)
*Yassine Afif,Mohammed Almekhlafi,Antoine Lesage-Landry,Gunes Karabulut Kurt*

Main category: cs.IT

TL;DR: Optimizing LEO satellite communication by jointly managing power allocation, user-satellite associations, and handoff frequency to improve transmission rates while controlling signaling overhead.


<details>
  <summary>Details</summary>
Motivation: Frequent handoffs in LEO satellite constellations cause significant signaling and power consumption burdens, especially as constellation sizes increase, requiring optimization to maintain efficient communication.

Method: Formulated power allocation as a mixed-integer concave linear program (MICP) considering power constraints, user-satellite associations, and handoff penalties, solvable with off-the-shelf solvers.

Result: Proposed method achieves about 40% improvement in user rates compared to naive closest-satellite association, while effectively controlling handoff frequency through Monte Carlo simulations.

Conclusion: Handover-aware optimization strategy successfully balances transmission rate improvements with controlled handoff frequency, demonstrating significant performance gains in LEO satellite communication systems.
...
### [324] [Two-Step Decoding of Binary $2\times2$ Sum-Rank-Metric Codes](https://arxiv.org/abs/2511.19812)
*Hao Wu,Bocong Chen,Guanghui Zhang,Hongwei Liu*

Main category: cs.IT

TL;DR: This paper shows that decoding binary sum-rank-metric codes with 2x2 matrix blocks can be reduced entirely to decoding the constituent Hamming-metric codes, eliminating the restrictive requirement d1 ≥ 2/3 dsr from previous work.


<details>
  <summary>Details</summary>
Motivation: To resolve an open problem posed by Chen-Cheng-Qi about whether decoding binary sum-rank-metric codes can be reduced to decoding constituent Hamming codes without additional constraints.

Method: A simple two-step procedure: first uniquely decode C2, then apply single error/erasure decoding of C1. This achieves unique decoding up to floor((dsr-1)/2) errors with overall complexity T2 + T1.

Result: The restrictive hypothesis d1 ≥ 2/3 dsr is theoretically unnecessary. The decoder achieves optimal unique decoding capability and runs in O(ℓ²) time for BCH or Goppa instantiations over F4.

Conclusion: The reduction to constituent Hamming codes is both possible and asymptotically optimal in a black-box model, as any sum-rank decoder must inherently decode the constituent Hamming codes.
...
### [325] [Towards Edge General Intelligence: Knowledge Distillation for Mobile Agentic AI](https://arxiv.org/abs/2511.19947)
...
TL;DR: This survey explores how Knowledge Distillation (KD) enables Edge General Intelligence (EGI) by making AI models more efficient for resource-constrained mobile and edge devices, covering specialized KD techniques, novel architectures, and diverse applications.


<details>
  <summary>Details</summary>
Motivation: The deployment of advanced agentic AI models on mobile and edge devices faces significant challenges due to limited computation, energy, and storage resources, requiring efficient solutions for autonomous operation in dynamic environments.

Method: The paper investigates the integration of Knowledge Distillation (KD) into EGI, focusing on specialized techniques like channel-aware self-distillation, cross-model CSI feedback distillation, and robust modulation/classification distillation, along with novel architectures like Mamba and RWKV.

Result: The survey provides a comprehensive framework showing how KD-driven architectures enable EGI across vision, speech, and multimodal tasks, positioning KD as a key enabler for efficient, communication-aware, and scalable intelligence at the wireless edge.

Conclusion: Knowledge Distillation is identified as a crucial technology for realizing Edge General Intelligence, with the survey serving as a comprehensive reference for researchers exploring KD-driven frameworks for mobile agentic AI in the EGI era.
...
### [326] [Explainable Deep Learning for Secrecy Energy-Efficiency Maximization in Ambient Backscatter Multi-User NOMA Systems](https://arxiv.org/abs/2511.20108)
*Miled Alam,Abdul Karim Gizzini,Laurent Clavier*

Main category: cs.IT

TL;DR: This paper analyzes secrecy energy-efficiency in multi-user NOMA systems with ambient backscatter communications, proposing optimization methods and a deep learning predictor that achieves 95% accuracy with reduced complexity.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of improving both security and energy efficiency in next-generation IoT systems, particularly in the presence of passive eavesdroppers, by leveraging ambient backscatter communications with NOMA.

Method: For two backscatter devices: derived closed-form solutions for optimal reflection coefficients and power allocation. For more than two devices: proposed exhaustive grid-based method and particle swarm optimization, plus a feedforward neural network predictor.

Result: AmBC integration improves secrecy energy-efficiency by up to 615% compared to conventional NOMA in high-noise regimes. The FNN model achieves >95% accuracy while reducing complexity. SHAP analysis confirms model interpretability.

Conclusion: The proposed AmBC-NOMA system with deep learning optimization demonstrates significant potential for energy-efficient and secure IoT applications, with explainable AI building trust in the system performance.
...
### [327] [On hierarchical secure aggregation against relay and user collusion](https://arxiv.org/abs/2511.20117)
*Min Xu,Xuejiao Han,Kai Wan,Gennian Ge*

Main category: cs.IT

TL;DR: This paper establishes fundamental limits for hierarchical secure aggregation (HSA) in federated learning, addressing collusion resistance between relays and users while optimizing communication efficiency.


<details>
  <summary>Details</summary>
Motivation: To enhance privacy in federated learning by developing secure aggregation schemes that prevent information leakage even when relays and users collude, while minimizing communication overhead.

Method: Developed a two-phase communication framework using network function computation techniques, established communication load limits, derived achievable collusion thresholds, and constructed communication-optimal schemes with analysis of key size requirements.

Result: Established fundamental limits on communication load for user-relay and relay-server links, derived critical thresholds for collusion resilience, and proved achievability of lower bounds on key size in cyclic networks.

Conclusion: The work advances theoretical limits of communication efficiency and information-theoretic security in secure aggregation by establishing connections between HSA and network function computation.
...
### [328] [General Multi-User Distributed Computing](https://arxiv.org/abs/2511.20127)
*Ali Khalesi*

Main category: cs.IT

TL;DR: A unified learning- and information-theoretic framework for distributed computation and inference across multiple users and servers, characterizing trade-offs between computation, communication, and accuracy for heterogeneous target functions.


<details>
  <summary>Details</summary>
Motivation: To develop scalable and resource-optimal distributed and federated learning systems for applications where energy and data efficiency are critical, such as aeronautical, satellite, and edge-intelligent networks.

Method: Proposes a General Multi-User Distributed Computing (GMUDC) model with dual analysis: quenched design (fixed assignments and topology) and annealed design (averaged performance over random assignments and links). Uses reproducing-kernel Hilbert spaces with shift-invariant kernels.

Result: Establishes fundamental limits governing trade-offs among computing load, communication load, and reconstruction distortion under computational and communication budgets. Reveals spectral-coverage duality linking generalization capability with network topology and resource allocation.

Conclusion: Provides an information-energy foundation for provably efficient and topology-aware distributed designs, enabling scalable and resource-optimal distributed and federated learning systems.
...
### [329] [CSI Prediction Frameworks for Enhanced 5G Link Adaptation: Performance-Complexity Trade-offs](https://arxiv.org/abs/2511.20160)
*Francisco Díaz-Ruiz,Francisco J. Martín-Vega,Jose A. Cortés,Gerardo Gómez,Mari Carmen Aguayo*

Main category: cs.IT

TL;DR: This paper compares classical Wiener filter and deep learning methods for CSI prediction in wireless systems, finding that Wiener filter performs close to GRU with lower complexity when channel statistics are available, while GRU offers better generalization across different channel conditions.


<details>
  <summary>Details</summary>
Motivation: Channel state information (CSI) accuracy is crucial for efficient link adaptation, but challenges like channel aging, user mobility, and feedback delays impact adaptive modulation and coding performance, necessitating effective CSI prediction methods.

Method: Proposes two CSI prediction frameworks operating in SINR domain for both TDD and FDD systems. Compares classical Wiener filter with deep learning methods (GRU, LSTM, delayed DNN) using metrics like MSE, system performance, and computational complexity (FLOPs).

Result: Wiener filter performs close to GRU in MSE and throughput with lower computational complexity when channel statistics are available. GRU model shows better generalization across different channel scenarios. Learning-based methods suit TDD systems, while classical methods are preferable for FDD due to lower complexity at user equipment.

Conclusion: Classical Wiener filter is preferable for FDD systems due to lower complexity at power-constrained user equipment, while learning-based solutions are better suited for TDD systems where base station handles computation, with GRU offering superior generalization across channel conditions.
...
### [330] [Unified Block Signal Processing Framework for LPWANs: Sequence Index Modulation Spreading](https://arxiv.org/abs/2511.20364)
*Wenkun Wen,Tierui Min,Long Yuan,Minghua Xia*

Main category: cs.IT

TL;DR: A unified framework for generalized block signal transmission in LPWANs using quasi-orthogonal codewords from cyclically shifted spreading sequences, enabling reliable multi-user separation and block-based synchronization/demodulation.


<details>
  <summary>Details</summary>
Motivation: Address limitations of conventional symbol-by-symbol approaches in LPWANs that demand high receiver sensitivity and efficient physical-layer signal processing.

Method: Three-component framework: signal block vector, intra-block structure generator, and signal basis matrix using quasi-orthogonal codewords from cyclically shifted spreading sequences.

Result: Enables reliable multi-user separation (especially under asynchronous access), establishes block synchronization foundation, and provides unified demodulation via block correlation matching.

Conclusion: Advances scalable and efficient physical-layer design for next-generation LPWANs with flexible implementation demonstrated for FSK and chirp spread spectrum.
...
### [331] [A model of the Unity High Definition Render Pipeline, with applications to flat-panel and head-mounted display characterization](https://arxiv.org/abs/2511.19447)
*Richard F. Murray*

Main category: eess.IV

TL;DR: Analysis of Unity's HDRP rendering pipeline reveals non-obvious nonlinearities in material properties and rendered values that affect stimulus control, with solutions provided for gamma-corrected luminance and color display.


<details>
  <summary>Details</summary>
Motivation: Game engines like Unity are increasingly used for perceptual experiments, but their rendering pipelines have undocumented features that can compromise stimulus control and experimental validity.

Method: Mathematical modeling and testing of Unity's High Definition Render Pipeline (HDRP) to identify nonlinear transformations in material properties and rendered values, with development of specialized configuration and software tools.

Result: Identified several non-obvious nonlinear features in HDRP that affect luminance and color rendering, and successfully configured the pipeline for gamma-corrected stimulus display with provided software tools.

Conclusion: Unity's HDRP requires careful consideration of its nonlinear rendering characteristics for experimental stimulus control, but can be properly configured for accurate luminance and color display using the developed methods and tools.
...
### [332] [Not Quite Anything: Overcoming SAMs Limitations for 3D Medical Imaging](https://arxiv.org/abs/2511.19471)
*Keith Moore*

Main category: eess.IV

TL;DR: A compositional approach that uses foundation segmentation models (SAM-2) as additional input channels for brain MRI segmentation, avoiding fine-tuning while achieving 96% accuracy for basal ganglia segmentation.


<details>
  <summary>Details</summary>
Motivation: Foundation models like SAM and SAM-2 perform poorly on brain MRIs due to low contrast and unclear boundaries of structures like caudate and thalamus, while fine-tuning approaches like MedSAM require model modification.

Method: Use lightweight 3D U-Net to generate SAM-2 prompts from MRI, treating foundation model output as additional input channel. Also test DINO attention maps for prompt-free segmentation. Smooth edges of foundation model guesses for better alignment.

Result: Achieves ~96% volume accuracy on basal ganglia segmentation, sufficient for studying longitudinal volume changes. Approach is fast, label-efficient, and robust to out-of-distribution scans.

Conclusion: The compositional architecture avoids modifying foundation model weights, adapts to domain shift without retraining, and successfully applied to study inflammation-linked changes in pediatric OCD.
...
### [333] [A Multi-Stage Deep Learning Framework with PKCP-MixUp Augmentation for Pediatric Liver Tumor Diagnosis Using Multi-Phase Contrast-Enhanced CT](https://arxiv.org/abs/2511.19478)
*Wanqi Wang,Chun Yang,Jianbo Shao,Yaokai Zhang,Xuehua Peng,Jin Sun,Chao Xiong,Long Lu,Lianting Hu*

Main category: eess.IV

TL;DR: A multi-stage deep learning framework using multi-phase CT scans for non-invasive diagnosis of pediatric liver tumors, achieving high accuracy in tumor detection and classification between benign/malignant types and their subtypes.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of invasive biopsies for pediatric liver tumors (high complication risks, anesthesia requirements, costs) by developing a non-invasive AI-based diagnostic method using CT scans.

Method: Multi-stage DL framework with PKCP-MixUp data augmentation, tumor detection model for ROI extraction, and two-stage diagnosis pipeline using ROI-masked images with three backbones.

Result: High performance in tumor detection (mAP=0.871), excellent benign vs malignant classification (AUC=0.989), and robust subtype classification (benign AUC=0.915, malignant AUC=0.979).

Conclusion: The framework fills the pediatric-specific DL diagnostic gap, provides insights for CT phase selection and model design, and enables precise, accessible pediatric liver tumor diagnosis.
...
### [334] [The Selective Disk Bispectrum and Its Inversion, with Application to Multi-Reference Alignment](https://arxiv.org/abs/2511.19706)
*Adele Myers,Nina Miolane*

Main category: eess.IV

TL;DR: The paper presents the selective disk bispectrum, a fast, rotation-invariant representation for image shape analysis that is computationally efficient and invertible, enabling practical learning tasks on rotated images.
...
Method: Derived an explicit inverse for the disk bispectrum and defined a 'selective' version that uses only the minimal number of coefficients needed for faithful shape recovery, addressing previous limitations of cubic complexity and lack of invertibility.
...
### [335] [DLADiff: A Dual-Layer Defense Framework against Fine-Tuning and Zero-Shot Customization of Diffusion Models](https://arxiv.org/abs/2511.19910)
...
TL;DR: DLADiff is a dual-layer defense method that protects facial privacy against both fine-tuning and zero-shot diffusion model customization attacks, outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: Current defense methods focus on fine-tuning attacks but neglect zero-shot generation, creating a gap in facial privacy protection as malicious actors can exploit diffusion models with just one image.

Method: DLADiff uses a dual-layer mechanism: first layer employs Dual-Surrogate Models (DSUR) and Alternating Dynamic Fine-Tuning (ADFT) for fine-tuning defense; second layer provides simple but effective protection against zero-shot generation.

Result: Extensive experiments show DLADiff significantly outperforms existing methods in defending against fine-tuning attacks and achieves unprecedented performance against zero-shot generation.

Conclusion: DLADiff successfully addresses the gap in defending against both fine-tuning and zero-shot diffusion model customization, providing comprehensive facial privacy protection.
