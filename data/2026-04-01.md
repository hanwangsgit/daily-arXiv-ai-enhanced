...
TL;DR: DF-ACBlurGAN is a structure-aware conditional GAN for generating images with precise periodic patterns, specifically applied to biomaterial surface design with controlled repetition scale and spacing.


<details>
  <summary>Details</summary>
Motivation: Current generative models struggle with internally repeated and periodic structures because they're optimized for local textures and semantic realism rather than global structural consistency. This is particularly problematic for applications like biomaterial surfaces that require strict control over repetition scale, spacing, and boundary coherence.

Method: DF-ACBlurGAN integrates frequency-domain repetition scale estimation, scale-adaptive Gaussian blurring, and unit-cell reconstruction to balance sharp local features with stable global periodicity. It uses conditional GAN architecture with biological response labels as conditioning, enabling synthesis of designs aligned with target functional outcomes.

Result: The model demonstrates improved repetition consistency and controllable structural variation compared to conventional generative approaches across multiple biomaterial datasets, successfully generating patterns with precise periodic structures under weak supervision and class imbalance conditions.

Conclusion: The proposed structure-aware conditional GAN framework effectively addresses the challenge of generating images with internally repeated patterns, providing a solution for applications requiring strict control over global structural consistency, particularly in biomaterial design where repetition properties directly influence biological responses.
...
TL;DR: OccSim is a novel occupancy world model-driven 3D simulator that generates continuous driving simulation without pre-recorded logs or HD maps, achieving >80x improvement in stable generation length over previous methods.


<details>
  <summary>Details</summary>
Motivation: Current data-driven autonomous driving simulation is limited by heavy reliance on pre-recorded driving logs or HD maps, which restricts scalability and open-ended generation capabilities to finite existing datasets.

Method: OccSim uses two modules: W-DiT based static occupancy world model (handles ultra-long-horizon static environment generation with explicit rigid transformations) and Layout Generator (populates dynamic foreground with reactive agents based on synthesized road topology).

Result: OccSim can generate over 3,000 continuous frames (4+ km spans), representing >80x improvement in stable generation length. Data from OccSim pre-trains 4D semantic occupancy forecasting models achieving 67% zero-shot performance (11% better than asset-based simulators), scaling to 74% with 5x dataset size (22.1% improvement).

Conclusion: OccSim breaks the bottleneck of dependency on pre-recorded data, enabling scalable, open-ended autonomous driving simulation with significant improvements in generation length and downstream task performance.
...
TL;DR: Fisheye3R adapts multi-view 3D reconstruction foundation models to handle fisheye images without performance loss on perspective images, using novel adaptation schemes that work with limited fisheye training data.


<details>
  <summary>Details</summary>
Motivation: Existing foundation models for multi-view 3D reconstruction degrade on fisheye images due to non-linear projection models, but there's insufficient fisheye training data with ground truth to retrain them effectively.

Method: Proposes Fisheye3R adaptation framework with flexible learning schemes: self-supervised adaptation using only unlabeled perspective images, and supervised adaptation without any fisheye training data, enabling models to handle radial distortion.

Result: Extensive experiments on VGGT, π³, and MapAnything show consistent improvements in camera pose, depth, point map, and field-of-view estimation on fisheye images without regression on perspective images.

Conclusion: Fisheye3R successfully extends multi-view 3D reconstruction foundation models to natively handle fisheye imagery despite data scarcity, maintaining perspective performance while improving fisheye accuracy.
...
TL;DR: A signed Graph Neural Network with sparse edge mask and class-specific saliency accurately decodes visual category-specific functional connectivity states from 7T fMRI data, revealing biologically meaningful subnetworks along visual pathways.
...
Result: The model accurately decodes category-specific functional connectivity states (sports, food, vehicles) and reveals reproducible, biologically meaningful subnetworks along the ventral and dorsal visual pathways.

Conclusion: This framework successfully bridges machine learning and neuroscience by extending voxel-level category selectivity to a connectivity-based representation of visual processing, providing insights into how brain networks organize visual information.
...
TL;DR: Stepper is a unified framework for text-to-3D scene synthesis that uses stepwise panoramic expansion to overcome trade-offs between visual quality and explorability in existing methods.


<details>
  <summary>Details</summary>
Motivation: Current text-to-3D scene synthesis methods face a fundamental trade-off: autoregressive expansion suffers from context drift (losing consistency), while panoramic video generation is limited to low resolution. This creates a dilemma between visual fidelity and scene explorability.
...
Result: Stepper achieves state-of-the-art performance in both fidelity and structural consistency, outperforming prior approaches and setting a new standard for immersive scene generation.

Conclusion: Stepper successfully circumvents the limitations of existing methods by providing a unified framework that balances high visual quality with scene explorability, advancing the field of text-driven immersive 3D scene synthesis.
...
TL;DR: Hybrid quantum-classical ML models for aluminum weld defect classification show competitive performance vs classical CNN, demonstrating potential for industrial quality control applications.


<details>
  <summary>Details</summary>
Motivation: To advance automated quality control in industrial settings by investigating hybrid quantum-classical approaches for defect classification in aluminum TIG welding images.

Method: Two hybrid approaches: 1) Quantum kernel method using CNN-extracted features encoded via parameterized quantum feature map, with quantum kernel matrix computed for SVM optimization solved via VQLS; 2) Angle encoding of features in variational quantum circuit with classical optimizer training. Both compared against classical CNN model on binary and multiclass classification tasks.

Result: While CNN model shows robust performance, hybrid quantum-classical models perform competitively, highlighting their potential for near-term industrial applications.

Conclusion: Hybrid quantum-classical approaches demonstrate promising performance for industrial defect detection and quality assurance, showing viability for real-world applications in the near term.
...
TL;DR: A feed-forward method for human performance capture that renders novel views from monocular RGB video by maintaining and updating a canonical space that accumulates appearance information over time, using probabilistic regression to handle missing observations.


<details>
  <summary>Details</summary>
Motivation: Monocular human performance capture faces the challenge of insufficient observations, especially for unseen body regions. The paper aims to overcome this by leveraging temporal continuity where more body parts become observable over time as the subject moves.

Method: Maintains a canonical space that progressively updates with each incoming frame to accumulate appearance information. Uses probabilistic regression to effectively utilize this accumulated context while respecting live state deformations, resolving conflicts between past and current observations.

Result: The method produces sharper reconstructions than deterministic regression approaches and enables plausible synthesis even in regions with no prior observations. Experiments on in-domain (4D-Dress) and out-of-distribution (MVHumanNet) datasets demonstrate effectiveness.

Conclusion: The proposed feed-forward human performance capture method successfully addresses the challenge of insufficient observations in monocular settings by maintaining a progressively updated canonical space and using probabilistic regression, enabling high-quality novel view synthesis even for unseen regions.
...
TL;DR: INR performance depends heavily on initialization; pretraining on unstructured noise (Gaussian/Uniform) improves signal fitting but yields poor deep image priors, while spectral-structured noise (1/|f|^α) balances both tasks effectively.


<details>
  <summary>Details</summary>
Motivation: Implicit neural representations (INRs) are sensitive to initialization, and while data-driven methods help, it's unclear whether they encode classical statistical priors or more complex features. This study investigates why certain initialization strategies work better through noise pretraining experiments.

Method: Pretrain INRs on diverse noise classes (Gaussian, Dead Leaves, Spectral, Uniform) and evaluate their ability to fit unseen signals and serve as deep image priors for denoising tasks on image/video data.

Result: Unstructured noise (Uniform, Gaussian) dramatically improves signal fitting capacity but yields poor deep image priors. Noise with natural image spectral structure (1/|f|^α) achieves excellent balance, performing on par with best data-driven initialization methods.

Conclusion: Spectral-structured noise pretraining provides effective INR initialization that balances signal fitting and inverse imaging capabilities, enabling efficient training when domain-specific data is scarce, without needing complex data-driven approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current masked image modeling methods operate in either raw pixel space or latent feature space, but not both. The authors aim to combine both approaches in a unified framework to leverage complementary information from different representation spaces.

Method: MEDiC uses three complementary objectives: 1) patch-level token distillation from frozen CLIP encoder, 2) global CLS alignment, and 3) pixel reconstruction via lightweight decoder. The framework systematically explores design choices including hierarchical clustering with relative position bias for evolved masking.

Result: The full combination of all three objectives achieves 73.9% kNN accuracy on ImageNet-1K. Key findings: evolved masking doesn't outperform simple block masking in teacher-guided settings, optimal loss weights are extremely fragile (small changes cause up to 17% accuracy drops), and the framework achieves 85.1% fine-tuning accuracy with ViT-Base at 300 epochs.

Conclusion: MEDiC demonstrates that combining pixel and feature space objectives provides complementary benefits for masked image modeling. The framework reveals important insights about masking strategies and loss weight sensitivity in multi-objective distillation approaches.
...
TL;DR: UltraG-Ray introduces a 3D Gaussian field representation with physics-based B-mode synthesis for ultrasound novel view synthesis, achieving more realistic images with view-dependent attenuation effects.


<details>
  <summary>Details</summary>
Motivation: Current ultrasound novel view synthesis methods struggle with complex tissue and view-dependent acoustic effects, leaving a substantial gap between simulation and reality. Physics-based approaches that incorporate ultrasound image formation are needed for more realistic results.
...
Result: The method consistently outperforms state-of-the-art approaches, achieving up to 15% improvement in MS-SSIM metrics, demonstrating clear improvement in realism of synthesized ultrasound images.

Conclusion: UltraG-Ray successfully addresses limitations of previous methods by naturally capturing view-dependent attenuation effects, enabling generation of physically informed B-mode images with increased realism for ultrasound novel view synthesis.
...
TL;DR: MMFace-DiT introduces a unified dual-stream diffusion transformer for multimodal face synthesis that achieves 40% better visual fidelity and prompt alignment than existing methods by deeply fusing spatial and semantic modalities through shared attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal face generation models have limitations: they typically extend pre-trained text-to-image pipelines with auxiliary control modules or stitch together separate uni-modal networks, leading to architectural constraints, parameter duplication, and failure under conflicting modalities or mismatched latent spaces.

Method: A unified dual-stream diffusion transformer with dual-stream transformer blocks that process spatial (mask/sketch) and semantic (text) tokens in parallel, deeply fused through a shared Rotary Position-Embedded (RoPE) Attention mechanism. Also includes a Modality Embedder for dynamic adaptation to varying spatial conditions without retraining.
...
Conclusion: MMFace-DiT provides a unified approach for synergistic multimodal face synthesis that prevents modal dominance and ensures strong adherence to both text and structural priors, achieving unprecedented spatial-semantic consistency for controllable face generation.
...
TL;DR: A generative algorithm for removing humans and shadows from egocentric walking tour videos using a semi-synthetic dataset and fine-tuned video diffusion model.


<details>
  <summary>Details</summary>
Motivation: Egocentric walking tour videos are valuable for environment modeling but contain too many humans due to crowds and eye-level perspectives, limiting their usefulness for clean environment reconstruction.

Method: Created a semi-synthetic dataset of video clip pairs (environment-only backgrounds + composite clips with walking humans and simulated shadows). Fine-tuned the Casper video diffusion model for object and effects inpainting using this dataset.

Result: The resulting model performs significantly better than Casper both qualitatively and quantitatively at removing humans from walking tour clips with complex backgrounds and significant human presence.

Conclusion: The generated human-free clips can successfully be used to build 3D/4D models of urban locations, addressing the human presence challenge in egocentric walking tour videos.
...
TL;DR: DASES introduces a falsification-driven framework for autonomous scientific discovery that prevents models from gaming static evaluations by using adaptive falsification to push against candidates, discovering more robust scientific artifacts.


<details>
  <summary>Details</summary>
Motivation: Current autonomous discovery systems can be gamed - models can learn to pass static evaluations without actually learning the underlying mechanisms. There's a need for evaluation methods that actively challenge candidates rather than passively certify them.

Method: DASES uses a co-evolutionary framework with three components: Innovator (creates scientific artifacts), Abyss Falsifier (generates scientifically admissible counterexamples), and Mechanistic Causal Extractor (extracts causal mechanisms). They evolve under a fixed scientific contract.

Result: DASES rejects artifacts that static validation would accept, identifies candidates that survive falsification, and discovers FNG-CE loss that outperforms CE and CE+L2 across benchmarks including ImageNet.

Conclusion: Falsification-driven evaluation is crucial for robust autonomous scientific discovery, preventing gaming of static evaluations and leading to more transferable, scientifically valid discoveries.
...
TL;DR: LA-Sign: A looped transformer framework with geometry-aware alignment for skeleton-based sign language recognition that uses recurrence instead of deeper layers to refine motion understanding.


<details>
  <summary>Details</summary>
Motivation: Existing deep feed-forward architectures for skeleton-based sign language recognition lack mechanisms for recurrent refinement and structured representation, despite the need for fine-grained understanding of articulated motion across multiple spatial scales.

Method: Proposes LA-Sign with looped transformer framework using recurrence to repeatedly revisit latent representations for progressive refinement. Introduces geometry-aware contrastive objective that projects skeletal and textual features into adaptive hyperbolic space for multi-scale semantic organization. Studies three looping designs and multiple geometric manifolds.

Result: Encoder-decoder looping combined with adaptive Poincare alignment yields strongest performance. Achieves state-of-the-art results on WLASL and MSASL benchmarks while using fewer unique layers than existing approaches.

Conclusion: Recurrent latent refinement and geometry-aware representation learning are effective for sign language recognition, demonstrating that recurrence can replace deeper layer stacking while improving performance.
...
TL;DR: The paper analyzes the modality gap in multi-modal models like CLIP, showing it emerges from contrastive loss optimization and relates to model robustness. A simple post-processing method can reduce the gap and improve robustness without harming clean accuracy.


<details>
  <summary>Details</summary>
Motivation: Most existing multi-modal models exhibit a strong modality gap where image and text embeddings are well-separated in shared space. It's unclear why this gap exists and whether closing it improves downstream performance.

Method: Theoretical analysis shows minimizing contrastive loss yields representations with orthogonal modality gap. Experimental validation uses simple post-processing that moves one modality towards the mean of the other modality.

Result: Modality gap is monotonically related to robustness - decreasing gap maintains clean accuracy but improves robustness to perturbations. Post-processing significantly increases robustness for real-world VLMs without accuracy loss.

Conclusion: The modality gap emerges naturally from contrastive learning and serves as a robustness indicator. Simple post-processing can leverage this gap to enhance model robustness while preserving performance.
...
TL;DR: WorldFlow3D: A novel flow matching method for generating unbounded 3D worlds with causal structure and high-quality texture, offering controllability through layout and attribute conditions.


<details>
  <summary>Details</summary>
Motivation: Unbounded 3D world generation is becoming a foundational task for scene modeling in computer vision, graphics, and robotics. Current methods are limited by conditional denoising approaches and lack the ability to generate causal 3D structure while maintaining controllability over scene layout and texture.

Method: WorldFlow3D builds on flow matching principles, modeling 3D generation as flowing through 3D data distributions rather than just conditional denoising. It uses a latent-free flow approach that generates causal 3D structure first, then uses this as an intermediate distribution to guide generation of complex structure and high-quality texture. The method enables controllability through vectorized scene layout conditions for geometric structure and visual texture control through scene attributes.

Result: The method demonstrates effectiveness on both real outdoor driving scenes and synthetic indoor scenes, showing cross-domain generalizability and high-quality generation on real data distributions. It achieves favorable scene generation fidelity over existing approaches in all tested settings for unbounded scene generation, with faster convergence than existing methods.

Conclusion: WorldFlow3D presents a novel approach to unbounded 3D world generation that overcomes limitations of conditional denoising methods, offering improved structure generation, controllability, and faster convergence while maintaining high-quality results across diverse domains.
...
TL;DR: TrajectoryAtlas generates synthetic paired video data to train TrajectoryMover, enabling generative movement of object 3D motion trajectories while preserving relative motion.


<details>
  <summary>Details</summary>
Motivation: Existing video editing methods focus on prescribing motion trajectories or altering appearance, but lack the ability to move an object's 3D motion trajectory while preserving its relative 3D motion. The main challenge is obtaining paired video data for this specific scenario.

Method: Introduces TrajectoryAtlas, a data generation pipeline for large-scale synthetic paired video data, and TrajectoryMover, a video generator fine-tuned with this data to enable generative movement of object trajectories.

Result: The approach successfully enables generative movement of object trajectories, addressing the previously missing capability in video editing.

Conclusion: TrajectoryAtlas and TrajectoryMover provide a solution for moving object 3D motion trajectories in videos while preserving relative motion, overcoming the data generation challenge that previous methods faced.
...
TL;DR: Computer vision framework analyzes upper-extremity movement during Box and Block Test using world-aligned joint angles from monocular video, revealing movement quality differences beyond standard clinical scores.


<details>
  <summary>Details</summary>
Motivation: Standard clinical assessments for stroke upper-extremity function lack sensitivity (ordinal scoring) or don't capture movement quality (time-based metrics). Need for objective, quantitative movement analysis without changing clinical routine.

Method: Computer vision framework using monocular video to extract world-aligned joint angles of fingers, arm, and trunk during Box and Block Test. No depth sensors or calibration objects needed. Applied unsupervised dimensionality reduction to joint-angle features from 136 recordings (48 healthy, 7 post-stroke).

Result: Embeddings show separation between healthy and stroke movement patterns. Some patients with same BBT scores have different postural patterns. World-aligned joint angles capture meaningful movement quality information beyond time-based scores.

Conclusion: Camera-based, calibration-free framework can measure movement quality in clinical assessments without changing routine. Monocular video from phone/camera provides objective movement analysis beyond standard scores.
...
TL;DR: DIME: A dual-imbalance-aware adapter merging framework for continual food recognition that handles both within-class sample imbalance and step-imbalanced learning of new food classes.


<details>
  <summary>Details</summary>
Motivation: Real-world food recognition faces dual imbalance: severe data imbalance within food classes (long-tailed distribution) and imbalanced numbers of new classes introduced at each continual learning step. Existing methods assume balanced incremental steps, which doesn't reflect reality where new food categories appear unevenly over time.

Method: DIME learns lightweight adapters for each task using parameter-efficient fine-tuning, then merges them through a class-count guided spectral merging strategy. A rank-wise threshold modulation mechanism stabilizes merging by preserving dominant knowledge while allowing adaptive updates, resulting in a single merged adapter for inference.

Result: Experiments on realistic long-tailed food benchmarks under step-imbalanced setup show DIME consistently improves by more than 3% over the strongest existing continual learning baselines.

Conclusion: DIME effectively addresses the dual imbalance problem in continual food recognition through adapter merging with class-count guidance and threshold modulation, enabling efficient deployment without accumulating task-specific modules.
...
TL;DR: SparseDriveV2 improves scoring-based planning by using a factorized trajectory representation (paths × velocity profiles) with coarse-to-fine scoring, achieving state-of-the-art performance on autonomous driving benchmarks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses whether static trajectory vocabularies can match dynamic proposal methods in multi-modal planning. Existing static vocabularies suffer from coarse discretization, while dynamic proposals offer finer precision but it's unclear if dynamic generation is fundamentally necessary.

Method: Two complementary innovations: (1) scalable factorized vocabulary representation that decomposes trajectories into geometric paths and velocity profiles for combinatorial action space coverage, and (2) scalable scoring strategy with coarse factorized scoring over paths/velocity profiles followed by fine-grained scoring on composed trajectories.

Result: Achieves 92.0 PDMS and 90.1 EPDMS on NAVSIM, and 89.15 Driving Score with 70.00% Success Rate on Bench2Drive using a lightweight ResNet-34 backbone.

Conclusion: Static vocabularies can achieve comparable performance to dynamic methods when made sufficiently dense through factorized representation and efficient scoring strategies, challenging the necessity of dynamic generation in scoring-based planning.
...
TL;DR: LatentPilot introduces a new VLN paradigm that learns action-conditioned visual dynamics by leveraging future observations during training, enabling agents to "dream ahead" about how actions affect visual changes without needing future frames at inference.
...
Method: 1) Flywheel-style training: iteratively collects on-policy trajectories and retrains model with expert takeover when agent deviates excessively. 2) Learns visual latent tokens without explicit supervision that attend globally in continuous latent space. 3) Latent tokens are carried across steps, serving as both current output and next input, enabling "dreaming ahead" about action effects.
...
Conclusion: LatentPilot successfully enables VLN agents to learn action-conditioned visual dynamics by leveraging future observations during training, improving navigation performance and understanding of causal relationships between actions and visual changes.
...
TL;DR: CT can't reliably train X-ray classifiers without CT at inference; cross-modality distillation fails to show robust advantage over same-modality methods.


<details>
  <summary>Details</summary>
Motivation: Most CAD models use single imaging modalities, but CT and X-ray provide complementary thoracic disease views. The study investigates if CT can train X-ray classifiers without needing CT at inference time.

Method: Used cross-modality teacher-student distillation with JDCNet as pilot scaffold. Tested on patient-level paired chest imaging cohort with original split and Monte Carlo resampling. Compared plain cross-modal logit-KD, attention transfer, feature hints, and same-modality distillation controls.

Result: Original split showed cross-modal logit-KD best (0.875 accuracy). Resampled protocol found late fusion best for accuracy (0.885), same-modality distillation best for macro-F1 (0.554) and balanced accuracy (0.660). Cross-modal methods dropped to 0.500 balanced accuracy; attention transfer/feature hints didn't recover advantage.

Conclusion: No validated CT-to-X-ray architecture found. Contribution is reproducible pilot protocol defining task requirements, failure modes, and ranking instability for future credible cross-modality transfer claims.
...
<details>
  <summary>Details</summary>
Motivation: Traditional brain tissue segmentation methods like FSL FAST require task-specific adjustments and struggle with diverse imaging conditions. Foundation models like MedSAM offer promising prompt-based approaches but need adaptation for multi-class segmentation tasks.

Method: Preprocessing pipeline includes skull stripping (FSL BET), tissue probability mapping (FSL FAST), and converting to 2D axial/sagittal/coronal slices with multi-class labels. Modified MedSAM by extending mask decoder to three classes (background, gray matter, white matter), freezing pre-trained image encoder, and fine-tuning prompt encoder and decoder.

Result: Achieved Dice scores up to 0.8751 on IXI dataset, demonstrating effective multi-class brain tissue segmentation performance.

Conclusion: Foundation models like MedSAM can be successfully adapted for multi-class medical image segmentation with minimal architectural modifications, suggesting potential for extension to more diverse medical imaging scenarios.
...
TL;DR: SplatHLoc: A hierarchical visual relocalization framework using Feature Gaussian Splatting for scene representation, with adaptive viewpoint retrieval and hybrid feature matching for improved accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Point-based hierarchical relocalization methods have limitations due to sparse image observations and weak feature matching, which hinder their performance in camera pose estimation for revisited scenes.

Method: Uses Feature Gaussian Splatting as scene representation, adaptive viewpoint retrieval to synthesize virtual candidates aligned with query viewpoints, and hybrid feature matching that combines Gaussian-rendered features (better for coarse stage) with directly extracted image features (better for fine stage).

Result: Extensive experiments on indoor and outdoor datasets show SplatHLoc enhances robustness of visual relocalization and achieves new state-of-the-art performance.

Conclusion: SplatHLoc effectively addresses limitations of point-based methods through novel scene representation and hybrid matching strategy, demonstrating superior performance in visual relocalization tasks.
...
TL;DR: SLVMEval is a benchmark for meta-evaluating text-to-video evaluation systems on long videos (up to 3 hours), testing if they can accurately assess video quality in human-easy settings.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video evaluation systems need meta-evaluation to assess their reliability, especially for long videos where existing benchmarks are limited. The paper aims to create a controlled testbed to measure how well these systems perform compared to human judgment.

Method: 1) Create synthetic "high-quality vs low-quality" video pairs by degrading source videos from dense video-captioning datasets across 10 distinct aspects. 2) Use crowdsourcing to filter pairs where degradation is clearly perceptible. 3) Employ pairwise comparison-based meta-evaluation framework to assess existing evaluation systems' ranking accuracy.

Result: Human evaluators achieved 84.7%-96.8% accuracy in identifying better long videos. In 9 out of 10 aspects, existing evaluation systems performed worse than human assessment, revealing significant weaknesses in text-to-long-video evaluation.

Conclusion: SLVMEval benchmark effectively exposes limitations in current text-to-video evaluation systems for long videos, demonstrating that most systems fail to match human-level accuracy in quality assessment across multiple aspects.
...
<details>
  <summary>Details</summary>
Motivation: To reconstruct 3D objects from 2D orthographic views, which is useful for CAD, reverse engineering, and visualization applications where only 2D projections are available.

Method: 1. Apply Harris corner detector on orthographic views to get control points. 2. Project control points perpendicular to respective views to construct envelopes. 3. Obtain 3D points from intersection of mutually perpendicular envelopes. 4. Use computational geometry to regenerate surfaces from point set. 5. Render final 3D object using OpenGL.

Result: Successfully reconstructs 3D objects from orthographic views by extracting control points, creating intersecting envelopes, and generating surfaces for visualization.

Conclusion: The proposed method effectively converts 2D orthographic views into 3D objects through corner detection, envelope construction, and computational geometry, enabling practical 3D reconstruction from limited 2D data.
...
TL;DR: Adaptive context compression framework for LLMs improves long-running interaction performance by balancing memory preservation and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: LLMs suffer performance degradation in long-running interactions due to increasing context length, memory saturation, and computational overhead.

Method: Adaptive context compression framework integrating importance-aware memory selection, coherence-sensitive filtering, and dynamic budget allocation to retain essential information while controlling context growth.
...
Conclusion: Adaptive context compression provides effective balance between long-term memory preservation and computational efficiency in persistent LLM interactions.
...
TL;DR: Multi-Layer Memory Framework for long-horizon dialogue systems addresses semantic drift and memory retention issues through layered memory decomposition with adaptive gating and regularization.
...
Method: Proposes a Multi-Layer Memory Framework that decomposes dialogue history into working, episodic, and semantic layers with adaptive retrieval gating and retention regularization to control cross-session drift while maintaining bounded context growth.
...
Conclusion: The framework enhances long-term retention and reasoning stability under constrained context budgets, confirming effectiveness for long-horizon dialogue systems.
...
TL;DR: LightHarmony3D is a framework for inserting 3D mesh objects into 3D Gaussian Splatting scenes with physically consistent lighting and shadows, using a generative module to predict HDR environment maps for illumination estimation.


<details>
  <summary>Details</summary>
Motivation: While 3D Gaussian Splatting enables high-fidelity scene reconstruction, inserting external mesh objects into these scenes for applications like AR/VR and virtual staging requires physically consistent lighting and shadows, which is challenging due to the need for accurate illumination estimation and multi-view consistent rendering.

Method: The framework uses a generative module that predicts a full 360° HDR environment map at the insertion location via a single forward pass, leveraging generative priors instead of iterative optimization. This captures dominant scene illumination for physically grounded shading and shadows while maintaining multi-view coherence.

Result: Extensive experiments across multiple real-world reconstruction datasets demonstrate that LightHarmony3D achieves state-of-the-art realism and multi-view consistency. The authors also introduce the first dedicated benchmark for mesh insertion in 3DGS for standardized evaluation.

Conclusion: LightHarmony3D provides an efficient and effective solution for illumination-consistent mesh insertion in 3DGS scenes, enabling physically plausible lighting and shadows for inserted objects while maintaining photorealism and multi-view coherence.
...
TL;DR: CCDNet: A novel infrared target detection network that addresses camouflage and distraction problems through weighted multi-branch perceptrons, aggregation-refinement fusion, and contrastive-aided distractor discrimination.


<details>
  <summary>Details</summary>
Motivation: Infrared target detection is challenging due to low contrast, camouflage effects where targets blend into complex backgrounds, and presence of distractors that cause false alarms, degrading detection performance in critical applications like wilderness rescue and maritime search.

Method: Three key components: 1) Backbone with Weighted Multi-branch Perceptrons (WMPs) for multi-level feature aggregation; 2) Aggregation-and-Refinement Fusion Neck (ARFN) to refine structure/semantics and reconstruct target-background relations; 3) Contrastive-aided Distractor Discriminator (CaDD) for adaptive local/global similarity computation to distinguish real targets from distractors.

Result: Extensive experiments on infrared image datasets show that CCDNet outperforms other state-of-the-art methods, achieving improved detection accuracy and reduced false alarm rates.

Conclusion: CCDNet effectively addresses the core challenges of infrared target detection by simultaneously handling camouflage through better feature representation and distraction through contrastive discrimination, making it a superior solution for practical IRSTD applications.
...
<details>
  <summary>Details</summary>
Motivation: Monocular cameras are attractive for robotics due to low cost and ease of deployment, but achieving reliable real-time spatial understanding from single images remains challenging. While multi-task dense prediction models have improved, translating these advances into stable monocular mapping systems is non-trivial.

Method: M2H-MX preserves multi-scale feature representations while introducing register-gated global context and controlled cross-task interaction in a lightweight decoder. This enables depth and semantic predictions to reinforce each other under strict latency constraints. The outputs integrate directly into monocular SLAM through a compact perception-to-mapping interface.

Result: On NYUDv2, M2H-MX-L achieves state-of-the-art results: semantic mIoU improved by 6.6% and depth RMSE reduced by 9.4% over multi-task baselines. In real-time monocular mapping on ScanNet, it reduces average trajectory error by 60.7% compared to strong monocular SLAM baseline while producing cleaner metric-semantic maps.

Conclusion: Modern multi-task dense prediction can be reliably deployed for real-time monocular spatial perception in robotic systems, demonstrating that improved perception models can significantly enhance monocular mapping performance.
...
TL;DR: DMA (Diffusion Mental Averages) is a model-centric method that produces sharp, realistic "mental averages" of concepts by averaging within diffusion model's semantic space through trajectory alignment, outperforming data-centric methods that produce blurry results.


<details>
  <summary>Details</summary>
Motivation: Prior methods for averaging image collections produce blurry results when applied to diffusion samples from the same prompt because they operate outside the model, ignoring the generative process. The authors want to produce sharp, realistic concept averages that serve as visual summaries and reveal model biases.

Method: DMA averages within the diffusion model's semantic space by optimizing multiple noise latents so their denoising trajectories progressively converge toward shared coarse-to-fine semantics, yielding a single sharp prototype. For multimodal concepts, they cluster samples in semantically-rich spaces like CLIP and use Textual Inversion or LoRA to bridge CLIP clusters into diffusion space.

Result: DMA delivers consistent, realistic averages even for abstract concepts, producing sharp prototypes rather than blurry composites. This represents the first approach that successfully creates visual concept averages from diffusion models.

Conclusion: DMA provides a model-centric solution for generating sharp "mental averages" of concepts from diffusion models, serving as both a concrete visual summary and a tool for analyzing model biases and concept representation.
...
TL;DR: Proposes TSONet for monocular building height estimation from PhiSat-2 imagery, with new PHDataset and improved performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Monocular building height estimation is challenging due to ambiguous cues, inter-city variations, and long-tailed height distributions. PhiSat-2 offers global coverage and multispectral data but hasn't been systematically evaluated for this task.
...
Result: TSONet achieves best performance on PHDataset (9,475 patch pairs from 26 cities), reducing MAE by 13.2% and RMSE by 9.7%, improving IoU by 14.0% and F1-score by 10.1% over strongest competitors.

Conclusion: PhiSat-2 is effective for monocular building height estimation due to balanced spatial detail and multispectral observations. The study provides valuable dataset (PHDataset) and method (TSONet) for future research.
...
TL;DR: FlexMem is a training-free approach for long video understanding in MLLMs that mimics human memory mechanisms, enabling processing of unlimited video lengths on a single GPU by compressing visual KV caches and implementing flexible memory reading strategies.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models (MLLMs) struggle with long video understanding due to input length limitations and computational constraints. Existing methods process all video information at once, creating an upper limit on video length that can be handled.

Method: FlexMem mimics human video watching behavior by implementing a visual memory mechanism with dual-pathway compression for memory transfer/writing, and explores different memory reading strategies for various video understanding tasks including streaming scenarios.

Result: On a single 3090 GPU, FlexMem processes over 1,000 frames and achieves significant improvements over existing efficient video understanding methods. It helps base MLLMs achieve comparable or better performance than state-of-the-art models like GPT-4o and Gemini-1.5 Pro on some benchmarks.

Conclusion: FlexMem provides an effective, training-free solution for unlimited-length video understanding in MLLMs by implementing human-like memory mechanisms, overcoming previous computational and length limitations while maintaining competitive performance with top-tier models.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models like CLIP perform poorly on negation expressions, which are common in natural language but challenging for current models to understand correctly.

Method: Modifies CLIP's InfoNCE contrastive loss with two objectives: presence-based contrastive loss that pushes image embeddings away from negated captions, and absence-based contrastive loss that aligns images with both original and negated captions while maintaining semantic distinction. Fine-tunes only the front transformer layers of CLIP text encoder based on observation that these layers have stronger learning ability for negated text.

Result: Improves performance on presence-based negation by up to 52.65% and absence-based negation by up to 12.50% compared to pretrained CLIP, while maintaining or even improving general image-text retrieval capability by up to 19.62%.

Conclusion: Omni-NegCLIP demonstrates comprehensive ability to understand multiple types of negation tasks while preserving general multimodal capabilities, outperforming prior works in negation understanding.
...
TL;DR: MaskAdapt is a two-stage framework for physics-based humanoid control that enables flexible motion adaptation through residual learning with body-part masking.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for flexible motion adaptation in physics-based humanoid control, where existing methods often lack the ability to selectively modify specific body parts while preserving other behaviors, limiting their versatility for applications like motion composition and targeted goal tracking.

Method: Two-stage residual learning: 1) Train mask-invariant base policy using stochastic body-part masking and regularization for consistent actions across masking conditions, 2) Train residual policy on frozen base controller to modify only targeted body parts while preserving original behaviors elsewhere.

Result: MaskAdapt demonstrates strong robustness and adaptability, producing diverse behaviors under masked observations and delivering superior targeted motion adaptation compared to prior work, with successful applications in motion composition and text-driven partial goal tracking.

Conclusion: The MaskAdapt framework provides an effective approach for flexible motion adaptation in physics-based humanoid control, enabling selective modification of body parts while maintaining stability and preserving original behaviors, with promising applications in motion composition and text-driven control.
...
TL;DR: NPAD algorithm mitigates bias without requiring protected attribute information by using non-protected attributes and two novel loss functions (DACL and FRL).


<details>
  <summary>Details</summary>
Motivation: Existing fairness algorithms require protected attribute information, limiting real-world applications. There's a need for bias mitigation methods that don't rely on sensitive demographic data.

Method: Proposed NPAD algorithm uses non-protected attributes as auxiliary information. Introduces two loss functions: Debiasing via Attribute Cluster Loss (DACL) and Filter Redundancy Loss (FRL) to optimize models for fairness.
...
Conclusion: NPAD provides an effective bias mitigation approach without requiring protected attribute information, making it more applicable to real-world scenarios where sensitive data may be unavailable.
...
TL;DR: PRISM is a 270K-sample multi-view video dataset for fine-tuning embodied vision-language models in retail environments, addressing gaps in spatial, physical, and action understanding through a structured 3D knowledge ontology.


<details>
  <summary>Details</summary>
Motivation: Physical AI systems fail not due to poor visual recognition, but because they lack understanding of space, physical dynamics, and embodied actions needed for reliable real-world operation in structured environments like retail.

Method: Created a 270K-sample multi-view video corpus with egocentric, exocentric, and 360° viewpoints from five supermarket locations. Uses a novel 3D knowledge ontology spanning spatial, temporal/physical, and embodied action knowledge. Includes open-ended, chain-of-thought, and multiple-choice supervision across 20+ capability probes in four dimensions.

Result: PRISM contains ~11.8M video frames and ~730M tokens. Fine-tuning reduces error rate by 66.6% over pre-trained baseline, with 36.4% accuracy improvement in embodied action understanding. Demonstrates significant gains across all evaluation dimensions.

Conclusion: Ontology-structured, domain-specific supervised fine-tuning can meaningfully strengthen embodied vision-language models for real-world deployment settings, addressing critical gaps in spatial, physical, and action understanding.
...
TL;DR: ConInfer is a context-aware inference framework for open-vocabulary remote sensing segmentation that performs joint prediction across spatial units with semantic dependencies, improving over patch-wise methods.


<details>
  <summary>Details</summary>
Motivation: Existing patch-level prediction methods for OVRSS are misaligned with remote sensing data characteristics - real scenes are large-scale with strong spatial and semantic correlations, making isolated patch predictions insufficient.
...
Conclusion: The context-aware joint prediction approach significantly enhances segmentation consistency, robustness, and generalization in complex remote sensing environments compared to independent patch-wise methods.
...
TL;DR: Proposes an extrinsic-aware cross-attention framework for camera-LiDAR calibration that directly aligns image patches and LiDAR point groups in their native domains without projecting to 2D depth maps.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based methods project LiDAR points into depth maps for feature fusion, which distorts 3D geometry and degrades performance when extrinsic initialization is far from ground truth.

Method: Extrinsic-aware cross-attention framework that directly aligns image patches and LiDAR point groups in their native domains, explicitly injecting extrinsic parameter hypotheses into correspondence modeling.

Result: Outperforms state-of-the-art approaches on KITTI and nuScenes benchmarks. Under large perturbations: 88% accuracy on KITTI and 99% on nuScenes, substantially surpassing second-best baseline.

Conclusion: The proposed method enables geometry-consistent cross-modal interaction without relying on projected 2D depth maps, achieving superior accuracy and robustness in camera-LiDAR extrinsic calibration.
...
TL;DR: MELT addresses frequency bias and hard negative interference in Composed Image Retrieval by balancing rare semantic attention and denoising similarity scores.


<details>
  <summary>Details</summary>
Motivation: Existing CIR methods suffer from frequency bias causing "Rare Sample Neglect" and susceptibility to interference from hard negative samples and noise in similarity scoring.
...
Result: Extensive experiments on two CIR benchmarks validate the superior performance of MELT.

Conclusion: MELT effectively addresses frequency bias and hard negative interference in CIR through rare semantic localization and robust similarity estimation, demonstrating state-of-the-art performance.
...
<details>
  <summary>Details</summary>
Motivation: Current deepfake attribution/detection methods have poor generalization to novel generative methods and fail to consider the synergy between attribution and detection tasks. They rely too heavily on visual modalities alone and perform poorly on unseen advanced generators.

Method: Proposes a gaze-guided CLIP model with: 1) Gaze-aware image encoder (GIE) that fuses forgery gaze prompts with image embeddings to capture attribution patterns, 2) Language refinement encoder (LRE) with adaptive-enhanced word selector for precise vision-language matching, 3) Visual perception encoder that exploits gaze distribution differences between pristine and forged faces.

Result: Outperforms state-of-the-art by 6.56% ACC and 5.32% AUC in average performance under attribution and detection settings on a novel benchmark including diffusion and flow models.

Conclusion: The gaze-guided approach with adaptive language prompts effectively improves generalization to unseen face forgery attacks by leveraging gaze differences and vision-language synergy, establishing a new benchmark for fine-grained deepfake attribution and detection.
...
<details>
  <summary>Details</summary>
Motivation: Existing neural rendering methods struggle with accurate 3D geometry recovery and temporally consistent motion in complex environments, limiting realistic 4D scene reconstruction from monocular videos.

Method: Uses scalable motion field parameterized by cluster-centric basis transformations, with progressive optimization including background extension (adapting to new regions, refining poses, modeling shadows) and foreground propagation with three-stage refinement for motion consistency.
...
Conclusion: MotionScale enables efficient, high-fidelity 4D scene reconstruction from monocular videos by addressing scalability and temporal consistency challenges through novel motion parameterization and progressive optimization.
...
TL;DR: Self-consistency adaptation from text to visual domains for LLM-generated motion trajectories using geometric transformation groups and clustering.


<details>
  <summary>Details</summary>
Motivation: Self-consistency works well for text reasoning but needs adaptation for visual domains, specifically for verifying LLM-generated motion graphics trajectories.

Method: Sample diverse motion trajectories from LLM, model shape families as prototype trajectories with geometric transformation groups (rigid, similarity, affine), cluster trajectories based on allowable warps, and use hierarchical transformation group relationships.

Result: 4-6% accuracy improvement for LLM-based trajectory generation and 11% precision gains over VLM baselines for verification.

Conclusion: Successfully adapts self-consistency to visual domains by modeling shape families with geometric transformations, improving both generation and verification of motion trajectories.
...
TL;DR: A bilevel meta-learning method that performs feature-space augmentation to improve classifier robustness against spurious correlations, requiring only lightweight training on a single GPU.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks often rely on spurious features that make them brittle under distribution shift, especially on minority-group examples where spurious correlations don't hold. While ERM-trained feature extractors learn rich representations, the classifier head often fails, and retraining just the head can substantially improve performance on shifted distributions and minority groups.

Method: A bilevel meta-learning approach that performs augmentation directly in feature space (at backbone output) rather than pixel space. The method learns support-side feature edits such that after a few inner-loop updates on edited features, the classifier achieves lower loss on hard examples and improved worst-group performance. It operates efficiently without end-to-end optimization.

Result: The method is highly efficient and stable, requiring only a few minutes of training on a single GPU. CLIP-based visualizations show that learned feature-space updates induce semantically meaningful shifts aligned with spurious attributes.

Conclusion: Feature-space augmentation via bilevel meta-learning effectively improves classifier robustness against spurious correlations while maintaining efficiency, making it practical for real-world deployment where distribution shifts and minority groups are common.
...
TL;DR: FOSCU framework integrates Duo-Diffusion (3D latent diffusion with ControlNet) to generate synthetic MRI volumes with segmentation labels, addressing data scarcity in medical imaging.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation faces challenges: restricted access to clinical datasets, costly annotation, and data shortage through PACS systems, which impede development of robust segmentation algorithms.

Method: Proposes FOSCU framework with two components: 1) Duo-Diffusion - a 3D latent diffusion model with ControlNet that simultaneously generates synthetic MRI volumes and segmentation labels using segmentation-conditioned diffusion, and 2) enhanced 3D U-Net training pipeline.

Result: Evaluation on 720 abdominal MRI scans shows models trained with combined real and synthetic data achieve 0.67% mean Dice score gain over real-only training, and 36.4% reduction in Fréchet Inception Distance (FID), indicating enhanced image fidelity.

Conclusion: FOSCU effectively addresses data scarcity in medical imaging by generating high-quality synthetic data with segmentation labels, improving segmentation performance and image fidelity through combined real-synthetic training.
...
<details>
  <summary>Details</summary>
Motivation: As GANs and diffusion models create increasingly realistic synthetic faces, the risks of misinformation, fraud, and identity abuse grow, creating an urgent need for robust detectors that work across diverse generative models.

Method: Systematically reuses and fine-tunes discriminators originally trained for image generation, extracting scale-adaptive features from ProGAN discriminators and temporal-consistency features from diffusion models to capture generation-agnostic artifacts.

Result: Achieves up to 74.33% F1-score across 9 state-of-the-art generative models, outperforming existing ViT-based detectors by over 30% F1-score on average. Maintains robust performance on challenging datasets (up to 88% F1-score on CIFAKE) where baseline methods fail.

Conclusion: Discriminator reuse and cross-model fine-tuning are effective strategies for building generalizable deepfake detection systems, establishing CIPHER as a promising approach for robust detection in rapidly evolving generative technologies.
...
TL;DR: A unified framework that jointly models positional and semantic uncertainties in real-time maps to improve trajectory prediction robustness.
...
Method: Dual-head architecture that independently estimates semantic and positional predictions in dual-pass manner, derives prediction variances as uncertainty indicators end-to-end, and fuses uncertainties with predictions.

Result: Effectively quantifies map uncertainties through both dimensions and consistently improves existing trajectory prediction models across minADE, minFDE, and Miss Rate metrics on nuScenes dataset.

Conclusion: Proposed uncertainty-aware framework successfully addresses map uncertainty challenges and enhances trajectory prediction robustness by explicitly modeling and integrating positional and semantic uncertainties.
...
TL;DR: StereoVGGT adapts the Visual Geometry Grounded Transformer (VGGT) into a specialized backbone for stereo vision by addressing geometric degradation issues through a training-free feature adjustment pipeline, achieving state-of-the-art performance on KITTI benchmark.


<details>
  <summary>Details</summary>
Motivation: Current stereo vision backbones (MDE models or VFMs) lack explicit camera pose supervision during pretraining, creating a performance bottleneck since geometric knowledge is essential for stereo vision. While VGGT has 3D priors including camera poses, direct application to stereo vision yields suboptimal results due to geometric detail degradation during feature extraction.

Method: Propose StereoVGGT, a feature backbone tailored for stereo vision that leverages frozen VGGT with a training-free feature adjustment pipeline to mitigate geometric degradation and harness the latent camera calibration knowledge embedded in VGGT.

Result: StereoVGGT-based stereo matching network achieved 1st rank among all published methods on the KITTI benchmark, demonstrating it serves as a highly effective backbone for stereo vision.

Conclusion: StereoVGGT successfully bridges the gap between VGGT's geometric priors and stereo vision requirements, providing a robust backbone that effectively utilizes camera calibration knowledge while preserving geometric details essential for stereo tasks.
...
TL;DR: TriDerm: A multimodal framework that learns interpretable wound representations for RDEB by combining wound images, masks, and expert reports, achieving 73.5% agreement with experts through visual adaptation and text prompting with soft ordinal embeddings.


<details>
  <summary>Details</summary>
Motivation: RDEB is a rare genetic skin disorder where clinicians need to find similar cases using images and clinical text. Existing foundation models don't reliably capture clinically meaningful features for this heterogeneous disease, and measuring agreement with experts is challenging.

Method: Proposes evaluating embeddings with expert ordinal comparisons (triplet judgments). Introduces TriDerm framework that integrates wound imagery, boundary masks, and expert reports. For vision: adapts foundation models using wound-level attention pooling and non-contrastive learning. For text: prompts LLMs with comparison queries and recovers medically meaningful representations via soft ordinal embeddings (SOE). Fuses both modalities.

Result: Multimodal fusion yields 73.5% agreement with experts, outperforming the best off-the-shelf single-modality foundation model by over 5.6 percentage points. Visual and textual modalities capture complementary aspects of wound phenotype.

Conclusion: TriDerm effectively learns interpretable wound representations from small cohorts for rare diseases like RDEB. Expert ordinal comparisons provide efficient clinical similarity encoding. Multimodal fusion significantly improves agreement with expert judgment over single-modality approaches.
...
TL;DR: The paper introduces PromptForge-350k, a large-scale dataset for forgery localization in prompt-based AI image editing, and proposes ICL-Net, a triple-stream network with intra-image contrastive learning that achieves state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The rapid democratization of prompt-based AI image editing has increased risks of malicious content fabrication and misinformation, but forgery localization methods for these emerging techniques remain significantly under-explored.

Method: 1) Introduces an automated mask annotation framework using keypoint alignment and semantic space similarity to generate precise ground-truth masks. 2) Constructs PromptForge-350k dataset covering four state-of-the-art prompt-based AI editing models. 3) Proposes ICL-Net with triple-stream backbone and intra-image contrastive learning to capture robust forensic features.

Result: Achieves 62.5% IoU on PromptForge-350k, outperforming SOTA methods by 5.1%. Shows strong robustness against common degradations (IoU drop <1%) and promising generalization on unseen editing models (average IoU 41.5%).

Conclusion: The work addresses the critical gap in forgery localization for prompt-based AI image editing through a comprehensive dataset and effective network design, demonstrating superior performance, robustness, and generalization capabilities.
...
TL;DR: Extend3D: Training-free pipeline for 3D scene generation from single image using object-centric 3D generative model with extended latent space, patch-wise generation, and iterative refinement.


<details>
  <summary>Details</summary>
Motivation: Object-centric 3D generative models have fixed-size latent spaces that limit their ability to represent wide scenes from single images. Existing methods struggle with spatial alignment between image and latent patches, and object-centric models are sub-optimal for sub-scene generation.

Method: 1) Extend latent space in x and y directions, 2) Divide into overlapping patches and apply object-centric model to each patch with coupling, 3) Initialize scene using point cloud prior from monocular depth estimator, 4) Iteratively refine occluded regions via SDEdit with "under-noising" concept, 5) Optimize extended latent during denoising with 3D-aware objectives for geometry and texture.

Result: The method yields better results than prior methods, as evidenced by human preference and quantitative experiments, demonstrating improved 3D scene generation from single images.

Conclusion: Extend3D successfully addresses limitations of object-centric 3D generative models for wide scene generation through latent space extension, patch-wise processing, and novel refinement techniques including under-noising and 3D-aware optimization.
...
<details>
  <summary>Details</summary>
Motivation: Existing FF-3DGS methods suffer from severe rendering artifacts when rendering at out-of-distribution sampling rates due to incorrect screen-space dilation filters, limiting their practical application across different resolutions.

Method: AA-Splat introduces an opacity-balanced band-limiting (OBBL) design with two components: 1) 3D band-limiting post-filter that integrates multi-view maximal frequency bounds to band-limit 3D scene representations and eliminate degenerate Gaussians, and 2) Opacity Balancing (OB) that seamlessly integrates pixel-aligned Gaussian primitives to compensate for increased overlap between expanded Gaussians.

Result: AA-Splat achieves drastic improvements with average 5.4-7.5dB PSNR gains on novel view synthesis over state-of-the-art baseline DepthSplat at all resolutions between 4× and 1/4×, demonstrating robust anti-aliased rendering at any resolution.

Conclusion: AA-Splat provides a fast and robust solution for sparse-view 3D reconstruction and novel view synthesis that eliminates rendering artifacts across different resolutions through its novel opacity-balanced band-limiting approach.
...
TL;DR: HIRE framework detects and edits hallucination representations in Vision-Language Models with minimal computational overhead, achieving SOTA performance on hallucination benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current hallucination mitigation methods (retraining and Contrastive Decoding) are impractical - retraining requires heavy resources and CD introduces dual inference overhead, limiting real-world deployment.
...
Conclusion: HIRE provides a practical solution to hallucination in Vision-Language Models by enabling efficient detection and elimination of hallucination representations without the resource overhead of existing methods.
...
TL;DR: AGFT is an alignment-guided fine-tuning framework that enhances zero-shot adversarial robustness in VLMs while preserving cross-modal semantic structure, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Pre-trained VLMs have strong zero-shot generalization but are vulnerable to adversarial attacks. Existing adversarial fine-tuning methods disrupt cross-modal alignment and degrade zero-shot performance by breaking visual-textual correspondence.

Method: AGFT uses probabilistic predictions of the original model for text-guided adversarial training, aligning adversarial visual features with textual embeddings via soft alignment distributions. It also includes a distribution consistency calibration mechanism that adjusts robust model output to match temperature-scaled pre-trained model predictions.
...
Conclusion: AGFT effectively enhances adversarial robustness in VLMs while preserving cross-modal semantic alignment, maintaining strong zero-shot performance that existing label-based methods fail to achieve.
...
TL;DR: This paper introduces an imperceptible visual prompt injection attack method against closed-source MLLMs that embeds adversarial instructions in images using bounded text overlays and iterative feature alignment optimization.


<details>
  <summary>Details</summary>
Motivation: MLLMs are vulnerable to prompt injection attacks, but existing methods use perceptible textual or visual prompts. The authors aim to develop imperceptible visual prompt injection attacks against powerful closed-source MLLMs to study their security vulnerabilities.

Method: The method adaptively embeds malicious prompts into input images via bounded text overlays for semantic guidance. It iteratively optimizes imperceptible visual perturbations to align the attacked image's feature representation with malicious visual and textual targets at both coarse- and fine-grained levels. The visual target is instantiated as a text-rendered image and progressively refined during optimization.
...
Conclusion: The proposed imperceptible visual prompt injection method effectively attacks closed-source MLLMs, highlighting significant security vulnerabilities in current multimodal AI systems that need to be addressed.
...
TL;DR: Multimodal models (Paligemma, Llava, Qwen) combining visual and textual features for ID document presentation attack detection show limited effectiveness despite theoretical advantages over traditional visual-only approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional PAD systems relying solely on visual features often fail to detect sophisticated spoofing attacks on ID documents. There's a need for more robust detection methods that can leverage additional contextual information beyond just visual cues.

Method: The study explores multimodal approach using pre-trained models (Paligemma, Llava, Qwen) that combine deep visual embeddings with contextual textual metadata (document type, issuer, date) for presentation attack detection on ID documents.

Result: Experimental results indicate that despite theoretical advantages, these multimodal models struggle to accurately detect presentation attacks on ID documents, showing limited effectiveness in practical application.

Conclusion: While multimodal integration represents a significant theoretical advancement in biometric security for ID documents, current pre-trained models face challenges in practical PAD detection, suggesting need for further refinement or alternative approaches.
...
TL;DR: A²BFR is an attribute-aware blind face restoration framework that combines high-fidelity reconstruction with prompt-controllable generation using diffusion transformers and text-image cross-modal attention.


<details>
  <summary>Details</summary>
Motivation: Current blind face restoration methods produce ambiguous, uncontrollable results, while text-guided face editing lacks reliable restoration. There's a need to unify high-fidelity reconstruction with controllable attribute manipulation.

Method: Uses Diffusion Transformer backbone with unified image-text cross-modal attention, jointly conditioning on degraded inputs and textual prompts. Introduces attribute-aware learning using facial attribute embeddings, and semantic dual-training leveraging pairwise attribute variations from AttrFace-90K dataset.
...
Conclusion: A²BFR successfully unifies high-fidelity face restoration with prompt-controllable generation, addressing the limitations of existing methods by enabling both reliable restoration and fine-grained attribute manipulation.
...
TL;DR: VLMs show systematic bias on optical illusions, predicting illusions as "real" even when counterfactually modified. A tool-guided inference framework with generic image manipulation tools addresses this without training, enabling strong cross-structural generalization.


<details>
  <summary>Details</summary>
Motivation: Vision-language models exhibit systematic bias when dealing with optical illusions - they overwhelmingly predict illusions as "real" regardless of counterfactual modifications. This failure mode needs addressing without requiring model retraining.
...
Result: Framework yields strong cross-structural generalization - performance remained consistent from validation set to test set containing structurally unfamiliar illusion variants. Three empirical observations: (i) strong positive-detection bias from imbalanced training data, (ii) dissociation between spatial reasoning and logical inference, (iii) sensitivity to image compression artifacts compounding false positives.

Conclusion: Generic-tool-plus-routing design effectively addresses VLM bias on optical illusions without training, demonstrating cross-structural generalization. The empirical observations highlight important research directions for understanding VLM reasoning limitations.
...
TL;DR: SeGPruner is a semantic-aware, geometry-guided token reduction framework that improves 3D QA efficiency by pruning redundant multi-view visual tokens while maintaining reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Current 3D QA pipelines using VLMs suffer from severe token redundancy when aggregating multi-view observations, leading to inefficient inference under constrained token budgets. Existing token pruners are either tailored for 2D inputs or rely on indirect geometric cues, limiting their ability to retain semantically critical objects and maintain spatial coverage for robust 3D reasoning.

Method: SeGPruner uses two complementary modules: 1) Saliency-aware Token Selector preserves semantically salient tokens through attention-based importance scoring, and 2) Geometry-aware Token Diversifier complements with spatially diverse tokens by jointly considering semantic relevance and 3D geometric distance. This cooperation balances object-level evidence and global scene coverage under aggressive token reduction.
...
TL;DR: EarthEmbeddingExplorer is an interactive web app that makes Earth observation foundation models and global embeddings accessible through practical workflows, enabling researchers to use these academic assets for real-world applications.


<details>
  <summary>Details</summary>
Motivation: Despite the proliferation of high-impact foundation models and global Earth embedding datasets in Earth observation research, there's a significant gap in translating these academic assets into freely accessible, practical tools for researchers.

Method: Developed EarthEmbeddingExplorer, a cloud-native interactive web application that enables cross-modal queries (natural language, visual, and geolocation) and provides hands-on workflows for discovering and analyzing precomputed Earth embeddings.

Result: Created a freely accessible web application that transforms static research artifacts into dynamic tools, allowing researchers to derive scientific insights from retrieval results and democratizing access to state-of-the-art Earth observation models and data.

Conclusion: EarthEmbeddingExplorer successfully bridges the gap between academic Earth observation assets and practical applications, empowering researchers to transition from models and data archives to real-world analysis through an interactive, accessible platform.
...
TL;DR: NeoNet is an integrated 3D deep learning framework for noninvasive prediction of perineural invasion in cholangiocarcinoma, combining segmentation, synthetic data generation, and classification modules to overcome limited imaging criteria and data imbalance.


<details>
  <summary>Details</summary>
Motivation: To enable noninvasive diagnosis of perineural invasion (PNI) in cholangiocarcinoma, which is challenging due to lack of clear imaging criteria and the need to minimize invasive diagnostic procedures that risk patient injury and infection.

Method: Three-module framework: 1) NeoSeg with Tumor-Localized ROI Crop algorithm; 2) NeoGen - 3D Latent Diffusion Model with ControlNet for synthetic data generation to balance dataset; 3) NeoCls with PNI-Attention Network using frozen LDM encoder and 3D Dual Attention Blocks to detect subtle PNI patterns.

Result: NeoNet outperformed baseline 3D models in 5-fold cross-validation, achieving maximum AUC of 0.7903 for PNI prediction in cholangiocarcinoma.

Conclusion: The integrated NeoNet framework successfully addresses the challenge of noninvasive PNI prediction without relying on predefined image features, demonstrating superior performance through its combination of segmentation, synthetic data generation, and specialized attention-based classification.
...
TL;DR: A novel context-driven HTR framework enables few-shot writer adaptation at inference time without parameter updates, outperforming writer-independent models.
...
Method: Proposes a multimodal in-context learning inspired framework using few examples from target writer, with compact 8M-parameter CNN-Transformer architecture and context length optimization.
...
Conclusion: Context-driven HTR with few-shot adaptation effectively handles unseen writers, and combining context-driven with standard OCR training yields complementary improvements.
...
TL;DR: FedDBP is a federated prototype learning method that addresses feature fidelity-discriminability tradeoff and single global prototype limitations through dual-branch feature projection and personalized prototype fusion.
...
Method: FedDBP uses a client-side dual-branch feature projector with L2 alignment and contrastive learning to ensure both fidelity and discriminability of local features. On the server-side, it employs personalized global prototype fusion using Fisher information to identify important channels of local prototypes.

Result: Extensive experiments show FedDBP outperforms ten existing advanced methods, demonstrating its superiority in federated prototype learning.

Conclusion: FedDBP effectively addresses the limitations of existing federated prototype learning methods by balancing feature fidelity and discriminability while enabling personalized prototype fusion, making it a superior solution for heterogeneous federated learning.
...
TL;DR: Square superpixels using multi-scale blocks for better integration with deep learning pipelines and regular operators like convolutions.


<details>
  <summary>Details</summary>
Motivation: Existing superpixel algorithms produce irregular shapes that don't align well with regular operators like convolutions, limiting parallel implementation and end-to-end optimization in deep learning.

Method: Approximate superpixels using multi-scale square blocks, compute purity scores based on pixel-intensity similarity for each block, and select high-quality blocks for representation.

Result: Square superpixels can be integrated as graph nodes in GNNs or tokens in ViTs, enabling efficient parallel processing and learnable feature extraction with consistent performance improvements on downstream tasks.

Conclusion: The square superpixel approach overcomes limitations of irregular superpixels, enabling better integration with modern deep learning architectures and improving downstream task performance.
...
<details>
  <summary>Details</summary>
Motivation: Long-context video understanding and generation face computational challenges due to quadratic complexity of self-attention in Transformers. Existing sparse attention methods use coarse-grained patterns that lead to redundant computation and suboptimal performance.

Method: VecAttention uses vector-wise sparse attention based on observed vertical-vector sparse patterns in video attention maps. It dynamically selects informative vertical vectors through lightweight important-vector selection and optimized vector sparse attention kernels.

Result: VecAttention achieves 2.65× speedup over full attention and 1.83× speedup over state-of-the-art sparse attention methods, while maintaining comparable accuracy to full attention on video understanding (VideoMME, LongVideoBench, VCRBench) and generation (VBench) tasks.

Conclusion: VecAttention provides an effective framework for efficient video modeling by exploiting vertical-vector sparsity patterns, offering superior accuracy-efficiency trade-offs for long-context video understanding and generation tasks.
...
TL;DR: AR system using HoloLens 2 enables precise intraoperative relocalization of positive cancer margins via markerless surface registration, reducing guidance error from 14.2mm to 3.2mm.


<details>
  <summary>Details</summary>
Motivation: Positive margins in head and neck squamous cell carcinoma surgery are common, but current verbal communication from pathology to surgical team leads to imprecise re-resection. There's a need for more accurate intraoperative guidance to locate positive margins.

Method: Developed an all-in-one augmented reality system using HoloLens 2 depth sensing with fully automated markerless surface registration. The system relocalizes positive margins from resected specimens back to the resection bed and visualizes them in situ. Validated through silicone phantom studies with medical trainees.

Result: Markerless registration achieved target registration errors comparable to marker-based baseline (median 1.8mm vs 1.7mm; maximum <4mm). In margin relocalization tasks, AR guidance reduced error from 14.2mm (verbal guidance) to 3.2mm, with all AR localizations within 5mm error.

Conclusion: The study demonstrates feasibility of markerless AR margin guidance for more precise intraoperative re-excision in head and neck cancer surgery, potentially improving surgical outcomes by reducing positive margin rates.
...
TL;DR: A two-stage nighttime dehazing framework combining transmittance correction with structure-texture layered optimization to address multiple degradation factors in hazy nighttime images.
...
Method: Two-stage framework: 1) Transmittance correction with boundary-constrained initial transmittance maps, region-adaptive compensation/normalization, and quadratic Gaussian filtering in YUV space for atmospheric light estimation. 2) STAR-YUV decomposition separates dehazed image into structure/texture layers, applies gamma correction and MSRCR color restoration to structure layer, Laplacian-of-Gaussian filtering to texture layer, and two-phase fusion strategy.

Result: The method produces initial dehazed images using corrected transmittance and atmospheric light maps, then generates final outputs through enhanced structure-texture fusion, addressing visibility, color distortion, and contrast issues simultaneously.

Conclusion: The proposed framework effectively tackles the full spectrum of nighttime hazy image degradation factors through integrated transmittance correction and structure-texture optimization, outperforming existing methods that only address subsets of these challenges.
...
TL;DR: QUAD enables multi-task GenAI on mobile devices using shared model with LoRA weights as runtime inputs and unified quantization-aware training, achieving 6x memory reduction and 4x latency improvement.


<details>
  <summary>Details</summary>
Motivation: Deploying Large Vision Models for GenAI tasks on mobile devices is challenging due to high memory/compute requirements. Current approaches compile separate binaries for each LoRA + foundation model, causing redundant storage and runtime overhead.

Method: Treat LoRA weights as runtime inputs instead of embedding them into compiled model graph, enabling dynamic task switching. Introduce QUAD (Quantization with Unified Adaptive Distillation) - quantization-aware training that aligns multiple LoRA adapters under shared quantization profile. Implement lightweight runtime stack compatible with mobile NPUs.
...
Conclusion: The proposed unified framework enables efficient multi-task GenAI inference on edge devices using single shared model, overcoming limitations of current deployment pipelines through runtime LoRA input handling and unified quantization training.
...
TL;DR: A pose-aware generative framework with pose estimation supervision enhances anatomical accuracy and cultural fidelity in generating Bharatanatyam dance postures, outperforming standard conditional GAN and diffusion models.


<details>
  <summary>Details</summary>
Motivation: Preserving intangible cultural dances like Bharatanatyam in the digital era requires accurate generation of codified adavus and key postures while maintaining anatomical and stylistic integrity for documentation, analysis, and global transmission.

Method: Proposed pose-aware generative framework integrated with pose estimation module, using keypoint-based loss and pose consistency constraints. Evaluated four configurations: standard cGAN, cGAN with pose supervision, conditional diffusion, and conditional diffusion with pose supervision, all conditioned on key posture class labels.

Result: Incorporating pose supervision significantly enhances quality, realism, and authenticity of generated Bharatanatyam postures. Pose guidance aligns generated poses with ground-truth keypoint structures, promoting cultural fidelity in both cGAN and conditional diffusion settings.
...
TL;DR: The paper proposes AMDiT (Adaptive Margin Discrepancy Training) to improve the Emotion Diffusion Classifier (EmoDC) for facial expression recognition, enhancing both accuracy and adversarial robustness over standard diffusion models and discriminative classifiers.


<details>
  <summary>Details</summary>
Motivation: Current deep learning FER methods rely on discriminative classifiers that learn shortcuts and are vulnerable to distribution shifts. Diffusion models offer better robustness but standard training fails to properly penalize incorrect categorical descriptions, leading to suboptimal performance.

Method: Introduces Emotion Diffusion Classifier (EmoDC) using conditional generative diffusion model. Proposes Adaptive Margin Discrepancy Training (AMDiT) that dynamically adjusts margins between noise-prediction errors for correct vs incorrect categories based on sample difficulty, rather than using fixed margins.

Result: AMDiT significantly improves EmoDC accuracy over base diffusion training on RAF-DB basic/compound subsets, SFEW-2.0, and AffectNet in 100-step evaluations. EmoDC outperforms state-of-the-art discriminative classifiers in robustness against noise and blur.

Conclusion: The proposed AMDiT effectively enhances diffusion-based FER models by adaptively adjusting training margins, achieving both improved accuracy and superior robustness compared to existing methods.
...
TL;DR: FlowID is an AI method that reconstructs faces of violent death victims by removing injuries while preserving identity for identification purposes, using fine-tuning and attention-based masking.


<details>
  <summary>Details</summary>
Motivation: Many people die violently, and institutions need to document deceased portraits for identification but traditional image editing is slow and produces poor results. There's a need for efficient, identity-preserving facial reconstruction to aid in victim identification.

Method: FlowID combines single-image fine-tuning to adapt generative models to injured faces, with attention-based masking that localizes edits to damaged regions while preserving identity-critical features. This enables removal of violent artifacts while retaining identity information.

Result: FlowID outperforms state-of-the-art open-source methods on the new InjuredFaces benchmark while maintaining low memory requirements, making it suitable for local deployment without compromising data privacy.

Conclusion: FlowID provides an effective solution for identity-preserving facial reconstruction of violent death victims, with the InjuredFaces benchmark serving as a standardized resource for future research in extreme facial reconstruction.
...
TL;DR: Video-Oasis is a diagnostic suite that systematically evaluates existing video understanding benchmarks, revealing that over half of samples don't require visual/temporal context, and SOTA models perform near random guessing on remaining samples.


<details>
  <summary>Details</summary>
Motivation: Current video understanding benchmarks lack systematic evaluation of what truly constitutes video understanding. It's unclear whether performance gains come from visual perception, linguistic reasoning, or knowledge priors, and essential criteria for video understanding are overlooked.

Method: Created Video-Oasis, a sustainable diagnostic suite to systematically evaluate existing evaluations and distill spatio-temporal challenges for video understanding. Analyzed benchmark samples to determine which require visual input or temporal context.

Result: Two critical findings: 1) 54% of existing benchmark samples are solvable without visual input or temporal context, 2) On remaining samples, state-of-the-art models perform barely exceeding random guessing.

Conclusion: The work provides practical guidelines for future research by investigating algorithmic design choices that contribute to robust video understanding. Serves as a standard guideline for benchmark construction and rigorous evaluation of architecture development.
...
TL;DR: Unify-Agent: A unified multimodal agent for world-grounded image synthesis that uses agentic pipeline (prompt understanding, evidence searching, recaptioning, synthesis) to overcome limitations of frozen parametric knowledge in generating long-tail, knowledge-intensive concepts.
...
Conclusion: This early exploration of agent-based modeling for world-grounded image synthesis highlights the value of tightly coupling reasoning, searching, and generation for reliable open-world agentic image synthesis, addressing limitations of current unified multimodal models.
...
TL;DR: BigEarthNet.txt is a large-scale multi-sensor remote sensing image-text dataset with 464k images and 9.6M annotations to advance instruction-driven learning in Earth observation, addressing limitations of existing datasets.


<details>
  <summary>Details</summary>
Motivation: Vision-language models perform well on general computer vision but poorly on remote sensing data due to lack of large-scale, diverse RS image-text datasets with rich textual annotations.

Method: Created BigEarthNet.txt dataset containing 464,044 co-registered Sentinel-1 SAR and Sentinel-2 multispectral images with 9.6M text annotations including geographically anchored captions, VQA pairs, and referring expression detection instructions.

Result: BigEarthNet.txt surpasses existing RS datasets in textual richness and annotation variety. Fine-tuning VLMs with this dataset yields consistent performance gains across tasks, though models still struggle with complex land-use/land-cover classes.

Conclusion: BigEarthNet.txt addresses critical data limitations in remote sensing for VLMs, enabling better instruction-driven learning and establishing a benchmark for evaluating VLMs in Earth observation tasks.
...
TL;DR: Streaming retrieval architecture for edge cameras uses epsilon-net filter to retain only novel frames, improving cross-modal retrieval efficiency with compact on-device encoder and cloud re-ranking.


<details>
  <summary>Details</summary>
Motivation: Always-on edge cameras generate continuous video streams with redundant frames that degrade cross-modal retrieval by crowding correct results out of top-k search results.

Method: 1) On-device epsilon-net filter retains only semantically novel frames to build denoised embedding index; 2) Cross-modal adapter and cloud re-ranker compensate for compact encoder's weak alignment; 3) Single-pass streaming filtering approach.

Result: Outperforms offline alternatives (k-means, farthest-point, uniform, random) across eight vision-language models (8M-632M parameters) on two egocentric datasets (AEA, EPIC-KITCHENS). Combined architecture reaches 45.6% Hit@5 on held-out data using 8M on-device encoder at estimated 2.7 mW power consumption.

Conclusion: The streaming retrieval architecture with epsilon-net filtering effectively addresses redundancy in continuous video streams, enabling efficient cross-modal retrieval with minimal power consumption on edge devices.
...
TL;DR: SSFL for diatom classification with stage-specific data heterogeneity analysis, introducing PreDi partitioning scheme and PreP-WFL method to handle label-space heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Address label-scarce visual classification under decentralized heterogeneous data with partially overlapping class sets, where existing SSFL studies assume same data heterogeneity pattern throughout pre-training and fine-tuning, and current partitioning schemes fail to generate pure partially class-disjoint settings.

Method: Introduce SSFL for diatom classification as real-world instance, study cross-site variation in unlabeled data volume during pre-training and label-space misalignment during fine-tuning. Propose PreDi partitioning scheme to disentangle label-space heterogeneity into class Prevalence and class-set size Disparity dimensions. Develop PreP-WFL (Prevalence-based Personalized Weighted Federated Learning) to adaptively strengthen rare-class representations.

Result: SSFL consistently outperforms local-only training under both homogeneous and heterogeneous settings. Unlabeled data volume heterogeneity improves representation pre-training, while under label-space heterogeneity, prevalence dominates performance with smaller disparity effect. PreP-WFL effectively mitigates degradation, with gains increasing as prevalence decreases.

Conclusion: Provides mechanistic basis for characterizing label-space heterogeneity in decentralized recognition systems, showing stage-specific heterogeneity effects and effectiveness of proposed PreDi partitioning and PreP-WFL method for handling label-space heterogeneity in SSFL.
...
TL;DR: MacTok is a masked augmenting 1D continuous tokenizer that prevents posterior collapse in compressed latent spaces by using random and semantic masking with representation alignment, achieving efficient high-fidelity image tokenization with only 64-128 tokens.


<details>
  <summary>Details</summary>
Motivation: Continuous image tokenizers with KL regularization often suffer from posterior collapse when using fewer tokens, where the encoder fails to encode informative features into the compressed latent space.
...
TL;DR: DynMask improves masked motion generation by making it complexity-aware using Motion Spectral Descriptor (MSD) to guide masking, attention, and decoding based on local dynamic complexity.


<details>
  <summary>Details</summary>
Motivation: Current masked generative models treat motion frames uniformly, which poorly matches motion where local dynamic complexity varies sharply over time. This causes disproportionate degradation on dynamically complex motions.
...
Result: Improves motion generation most clearly on dynamically complex motions while also yielding stronger overall FID on HumanML3D and KIT-ML datasets.

Conclusion: Respecting local motion complexity is a useful design principle for masked motion generation, and MSD provides an effective, interpretable way to make masked generation complexity-aware.
...
TL;DR: CutClaw is an autonomous multi-agent framework that uses multiple MLLMs to edit hours-long raw footage into short, music-synchronized videos with narrative consistency and visual appeal.


<details>
  <summary>Details</summary>
Motivation: Manual video editing is time-consuming and repetitive for filmmakers and content creators, especially when aligning video content with audio to create engaging social media content.

Method: Uses hierarchical multimodal decomposition to capture visual/audio details, a Playwriter Agent for narrative consistency, and Editor/Reviewer Agents that collaboratively optimize final cuts based on aesthetic and semantic criteria.

Result: CutClaw significantly outperforms state-of-the-art baselines in generating high-quality, rhythm-aligned videos, as demonstrated through detailed experiments.

Conclusion: The framework successfully automates video editing by leveraging multiple MLLM agents to transform raw footage into professionally edited, music-synchronized short videos with strong narrative flow.
...
TL;DR: First benchmark for unsupervised domain adaptation in surgical skill assessment regression, introducing CoRe-DA framework that achieves superior cross-domain generalization without labeled target data.


<details>
  <summary>Details</summary>
Motivation: Current surgical skill assessment faces challenges: high cost of manual annotation, poor generalization of existing models to new tasks/environments, and availability of unlabeled video data that could be leveraged through unsupervised domain adaptation.

Method: Introduced CoRe-DA, a contrastive regression-based adaptation framework that learns domain-invariant representations through relative-score supervision and target-domain self-training. Created first UDA benchmark for SSA regression spanning four datasets across dry-lab/clinical settings and open/robotic surgery.

Result: CoRe-DA outperforms eight representative state-of-the-art methods, achieving Spearman Correlation Coefficients of 0.46 (dry-lab) and 0.41 (clinical target datasets) without using any labeled target data for training.

Conclusion: CoRe-DA enables scalable surgical skill assessment with reliable cross-domain generalization where existing methods underperform, addressing key limitations in current SSA approaches.
...
TL;DR: Proposed clinical DVH metric loss (CDM loss) with bit-mask encoding for 3D dose prediction in H&N radiotherapy, improving clinical alignment and training efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models for 3D dose prediction use voxel-wise regression losses that don't align well with clinical plan evaluation criteria based on DVH metrics. Need for clinically guided loss formulation that directly optimizes clinically used DVH metrics while remaining computationally efficient.

Method: Proposed clinical DVH metric loss (CDM loss) incorporating differentiable D-metrics and surrogate V-metrics, with lossless bit-mask ROI encoding to improve training efficiency. Evaluated on 174 H&N patients using temporal split (137 training, 37 testing) with standard 3D U-Net.

Result: CDM loss substantially improved target coverage and satisfied all clinical constraints compared to MAE- and DVH-curve based losses. PTV Score reduced from 1.544 (MAE) to 0.491 (MAE + CDM). Bit-mask encoding reduced training time by 83% and lowered GPU memory usage.

Conclusion: Directly optimizing clinically used DVH metrics enables 3D dose predictions better aligned with clinical treatment planning criteria than conventional supervision methods. CDM loss with efficient ROI bit-mask encoding provides practical and scalable framework for H&N dose prediction.
...
TL;DR: CANVAS is a comprehensive benchmark dataset for subcellular-resolution whole mouse brain light-sheet microscopy, featuring six cell-type markers with extensive annotations, addressing the lack of scalable analysis methods for petabyte-scale 3D microscopy data.


<details>
  <summary>Details</summary>
Motivation: Recent advances in tissue processing and light-sheet microscopy enable unprecedented subcellular-resolution whole-brain 3D imaging, but existing analysis methods struggle with petabyte-scale data and lack generalization across cellular morphologies and brain regions.

Method: Created CANVAS benchmark dataset featuring high-resolution whole mouse brain LSFM data with six neuronal and immune cell-type markers, comprehensive cell annotations, and a leaderboard to evaluate model performance.

Result: Demonstrated that baseline models using existing architectures fail to generalize well due to heterogeneity in cellular morphology across phenotypes and anatomical locations, highlighting the need for specialized methods.

Conclusion: CANVAS is the first and largest LSFM benchmark for intact mouse brain tissue at subcellular resolution, providing essential resources to accelerate development of scalable analysis methods and foundational models for whole-brain 3D microscopy.
...
TL;DR: SkeletonContext: A prompt-based framework that enriches skeleton action recognition with language-driven contextual semantics to bridge the gap between skeleton features and semantic descriptions in zero-shot settings.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot skeleton-based action recognition methods align skeleton features with textual embeddings, but lack contextual cues (like objects involved in actions). This creates a gap between skeleton and semantic representations, making it hard to distinguish visually similar actions.

Method: Proposes SkeletonContext with two key modules: 1) Cross-Modal Context Prompt Module uses a pretrained language model to reconstruct masked contextual prompts guided by LLMs, transferring linguistic context to skeleton encoder; 2) Key-Part Decoupling Module decouples motion-relevant joint features for robust action understanding even without explicit object interactions.
...
Conclusion: SkeletonContext successfully bridges the gap between skeleton and semantic representations by incorporating language-driven contextual semantics, enabling better zero-shot action recognition through improved cross-modal alignment and context reasoning.
...
TL;DR: The paper finds that low lesion-skin contrast, not global skin tone, is the key factor affecting segmentation performance in AI dermatology systems, suggesting fairness improvements should focus on handling low-contrast lesions.


<details>
  <summary>Details</summary>
Motivation: While fairness concerns in skin lesion classification have been studied, the influence of skin tone on the segmentation preprocessing stage remains under-quantified and often uses coarse skin tone categories.

Method: Evaluated three segmentation architectures (UNet, DeepLabV3 with ResNet50, DINOv2) on two dermoscopic datasets (HAM10000, ISIC2017) using continuous pigment analysis with pixel-wise ITA values as distributions. Quantified lesion-skin contrast using Wasserstein distances between within-image distributions for different regions.

Result: Global skin tone metrics (Fitzpatrick grouping or mean ITA) show weak association with segmentation quality. Low lesion-skin contrast is consistently associated with larger segmentation errors across models, indicating boundary ambiguity and low contrast are key failure drivers.

Conclusion: Fairness improvements in dermoscopic segmentation should prioritize robust handling of low-contrast lesions, and distribution-based pigment measures provide more informative audit signals than discrete skin-tone categories.
...
TL;DR: FED-Bench is a comprehensive benchmark for facial expression editing with rigorous testing, accurate evaluation metrics (FED-Score), and a large training dataset to address current limitations in facial expression image editing evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing facial expression editing benchmarks lack high-quality facial images and proper editing instructions, while current evaluation metrics have systemic biases that favor lazy or overfit editing, creating a need for better evaluation frameworks.

Method: 1) Constructed 747 triplets (original image, editing instruction, ground-truth) through cascaded pipeline; 2) Introduced FED-Score evaluation protocol with three dimensions: Alignment, Fidelity, and Relative Expression Gain; 3) Benchmarked 18 image editing models; 4) Created 20k+ in-the-wild facial training set.

Result: Current approaches struggle with simultaneous high fidelity and accurate expression manipulation, with fine-grained instruction following identified as the main bottleneck. The benchmark reveals evaluation biases and provides scalable training data that significantly improves baseline models when fine-tuned.

Conclusion: FED-Bench addresses critical gaps in facial expression editing evaluation through comprehensive benchmarking, accurate metrics, and scalable training data, providing a foundation for advancing facial expression manipulation research.
...
TL;DR: SISTA-Net is a self-supervised compressive sensing method for single-pixel imaging that combines CNN-Visual State Space Model architecture with deep nonlinear sparse transforms to improve reconstruction quality and noise robustness.


<details>
  <summary>Details</summary>
Motivation: Existing single-pixel imaging methods lack physical sparsity constraints and fail to integrate local and global features, resulting in noise vulnerability, structural distortions, and blurred details in challenging environments.

Method: SISTA-Net unfolds the Iterative Shrinkage-Thresholding Algorithm (ISTA) into an interpretable network with data fidelity and proximal mapping modules. It uses a hybrid CNN-Visual State Space Model (VSSM) for local-global feature integration and deep nonlinear networks as adaptive sparse transforms with learnable soft-thresholding.

Result: Outperforms state-of-the-art methods by 2.6 dB in PSNR in simulations and achieves 3.4 dB average PSNR improvement in real-world far-field underwater tests, demonstrating robust anti-interference capability even at extremely low sampling rates.

Conclusion: SISTA-Net successfully addresses limitations of existing SPI methods by incorporating physical sparsity constraints and local-global feature integration, resulting in superior reconstruction quality and noise robustness in challenging imaging environments.
...
### [88] [End-to-End Image Compression with Segmentation Guided Dual Coding for Wind Turbines](https://arxiv.org/abs/2603.29927)
*Raül Pérez-Gonzalo,Andreas Espersen,Søren Forchhammer,Antonio Agudo*

Main category: cs.CV

TL;DR: A deep learning framework for wind turbine inspection images that jointly performs segmentation and dual-mode compression, preserving blade regions at high quality while aggressively compressing background.


<details>
  <summary>Details</summary>
Motivation: Transferring large volumes of high-resolution wind turbine inspection images creates bottlenecks in defect assessment. Efficient coding needs to preserve blade regions with high fidelity while aggressively compressing background areas.

Method: End-to-end deep learning framework with: 1) BU-Netv2+P segmentation network with CRF-regularized loss for precise blade localization, 2) Hyperprior-based autoencoder for lossy compression, 3) Extended bits-back coder with hierarchical models for lossless blade reconstruction, and 4) ROI framework that removes sequential dependency by reusing background-coded bits for parallel dual-mode compression.

Result: Superior compression performance and efficiency demonstrated on large-scale wind turbine dataset, offering practical solution for automated inspections while ensuring defect detection is not compromised.

Conclusion: First fully integrated learning-based ROI codec combining segmentation, lossy, and lossless compression for wind turbine inspection, enabling efficient image transfer without compromising subsequent defect detection capabilities.
...
### [89] [Leveraging Synthetic Data for Enhancing Egocentric Hand-Object Interaction Detection](https://arxiv.org/abs/2603.29733)
*Rosario Leonardi,Antonino Furnari,Francesco Ragusa,Giovanni Maria Farinella*

Main category: cs.CV

TL;DR: Synthetic data significantly improves hand-object interaction detection in egocentric vision, especially when real labeled data is scarce, with up to +11.69% improvement using only 10% real data.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of limited real labeled data for hand-object interaction (HOI) detection in egocentric images, which is crucial for applications like robotics and AR/VR.

Method: Developed a synthetic data generation pipeline and HOI-Synth benchmark, systematically studying synthetic-real alignment across objects, grasps, and environments, and testing with only 10% real data.

Result: Significant improvements in Overall AP: +5.67% on VISOR, +8.24% on EgoHOS, and +11.69% on ENIGMA-51. Effectiveness increases with better synthetic-real alignment.

Conclusion: Synthetic data is highly effective for HOI detection when real data is scarce, and alignment between synthetic and real domains is crucial for performance gains. The released pipeline and benchmark enable further research.
...
### [90] [Better than Average: Spatially-Aware Aggregation of Segmentation Uncertainty Improves Downstream Performance](https://arxiv.org/abs/2603.29941)
...
TL;DR: Systematic study of pixel-wise uncertainty aggregation strategies for image segmentation, proposing novel spatial-aware methods and a meta-aggregator for robust OoD/failure detection across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: Current uncertainty aggregation strategies for segmentation lack systematic comparison and don't account for spatial structure, leading to inconsistent reporting and unclear best practices in safety-critical applications.

Method: 1) Formal analysis of existing aggregation strategies' properties and limitations; 2) Proposing novel strategies incorporating spatial uncertainty structure; 3) Benchmarking on OoD and failure detection across 10 diverse datasets; 4) Developing a meta-aggregator integrating multiple strategies.

Result: Spatial-structure-aware aggregators outperform traditional methods in both OoD and failure detection tasks, but performance varies with dataset characteristics. The proposed meta-aggregator achieves robust performance across all datasets.

Conclusion: Aggregation strategy choice significantly impacts downstream task performance in segmentation uncertainty quantification. Spatial-aware methods are generally superior, but dataset-specific characteristics matter. The meta-aggregator provides a robust solution across diverse applications.
...
TL;DR: A novel view synthesis method for dynamic scenes from monocular videos that outperforms existing approaches in geometric consistency while being more computationally efficient.


<details>
  <summary>Details</summary>
Motivation: Existing methods for synthesizing novel views from monocular videos of dynamic scenes have limitations: scene-specific methods with explicit motion priors fail in highly dynamic regions, diffusion-based approaches suffer from geometric inconsistencies, and both require substantial computational resources.

Method: Adapts generalizable models for static novel view synthesis to dynamic inputs with two key components: (1) a recurrent loop enabling unbounded, asynchronous mapping between input and target videos, and (2) efficient plane sweeps over dynamic inputs to disentangle camera and scene motion for fine-grained 6-DOF camera control.

Result: Outperforms four Gaussian Splatting-based scene-specific approaches and two diffusion-based approaches in reconstructing fine-grained geometric details across both static and dynamic regions, evaluated on UCSD dataset and new Kubric-4D-dyn dataset.

Conclusion: The proposed method successfully addresses limitations of existing approaches by combining the strengths of generalizable models with novel components for handling dynamic scenes, achieving better geometric consistency and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion-based watermarking methods rely on faithful reconstruction of diffusion trajectories for verification, creating a fundamental vulnerability that can be exploited.

Method: SHIFT uses stochastic diffusion resampling to deflect the generative trajectory in latent space, statistically decoupling the reconstructed image from the original watermark-embedded trajectory while maintaining visual quality.

Result: SHIFT achieves 95%-100% attack success rates on nine representative watermarking methods across different paradigms (noise-space, frequency-domain, optimization-based) with minimal semantic quality loss.

Conclusion: The attack exposes a fundamental weakness in diffusion-based watermarking that relies on trajectory recovery, and works without watermark-specific knowledge or model retraining.
...
TL;DR: TSHA is a comprehensive benchmark for indoor safety hazard assessment using vision-language models, addressing limitations of existing synthetic datasets and oversimplified tasks with 81,809 curated training samples and 1,707 challenging test samples.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for indoor safety hazard assessment have three key limitations: heavy reliance on synthetic datasets creating domain gaps, oversimplified safety tasks limiting generalization, and absence of rigorous evaluation protocols for complex real-world scenarios.

Method: Created TSHA benchmark with 81,809 training samples from four complementary sources: existing indoor datasets, internet images, AIGC images, and newly captured images. Includes challenging test set of 1,707 samples with videos and panoramic images containing multiple hazards to evaluate robustness.
...
Conclusion: TSHA benchmark addresses critical gaps in indoor safety hazard assessment, demonstrates significant performance improvements for VLMs, and highlights the importance of comprehensive real-world datasets for trustworthy safety evaluation.
...
TL;DR: IQPIR introduces Image Quality Prior from NR-IQA models to guide restoration toward optimal perceptual quality, addressing limitations of ground-truth supervision that converges to average quality rather than best possible quality.


<details>
  <summary>Details</summary>
Motivation: Existing image restoration methods rely on ground-truth supervision, but GT images can have inconsistent perceptual fidelity, causing models to converge to average quality rather than achieving the highest perceptual quality possible.

Method: Proposes IQPIR framework with three key mechanisms: 1) quality-conditioned Transformer using NR-IQA scores as conditioning signals, 2) dual-branch codebook separating common and HQ-specific features, and 3) discrete representation-based quality optimization to prevent over-optimization.

Result: Extensive experiments show the method surpasses cutting-edge restoration methods and serves as a generalizable quality-guided enhancement strategy for existing methods without structural modification.

Conclusion: IQPIR effectively addresses limitations of GT supervision by leveraging image quality priors to guide restoration toward perceptually optimal outputs, providing a plug-and-play enhancement compatible with existing architectures.
...
TL;DR: Hybrid edge-based action detection system combining skeleton-based motion analysis with vision-language models for public safety applications, evaluated under realistic edge constraints.


<details>
  <summary>Details</summary>
Motivation: Public safety applications need timely violence detection in public spaces, but current automated video analysis faces latency, privacy, and resource limitations, especially in edge-computing environments.

Method: System-level comparison of skeleton-based motion analysis (privacy-aware, low overhead) with vision-language models (contextual understanding, zero-shot reasoning) implemented on GPU-enabled edge device and evaluated using demonstrator-based setup.

Result: Results show complementary strengths: skeleton-based detection is fast and privacy-preserving, while vision-language models provide semantic understanding for complex situations. Hybrid architecture selectively augments fast skeleton detection with higher-level reasoning.

Conclusion: Hybrid edge-based system provides practical foundation for privacy-aware, real-time video analysis in public safety applications by balancing motion-centric and semantic approaches under edge constraints.
...
TL;DR: MAPLE is a hierarchical multi-label classification framework for remote sensing that handles multi-path label dependencies through semantic initialization, graph encoding, and adaptive fusion, achieving significant improvements in few-shot learning with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Existing hierarchical multi-label classification approaches struggle with multi-path settings in remote sensing, where images can activate multiple taxonomic branches, leading to underutilization of hierarchical information.

Method: MAPLE integrates three key components: 1) hierarchical semantic initialization from graph-aware textual descriptions, 2) graph-based structure encoding via GCNs, and 3) adaptive multi-modal fusion that dynamically balances semantic priors and visual evidence, plus an adaptive level-aware objective for selecting appropriate losses per hierarchy level.

Result: Evaluations on CORINE-aligned remote sensing datasets (AID, DFC-15, and MLRSNet) show consistent improvements of up to +42% in few-shot regimes while adding only 2.6% parameter overhead.

Conclusion: MAPLE effectively and efficiently models hierarchical semantics for Earth observation by addressing multi-path classification challenges through integrated semantic, structural, and adaptive fusion techniques.
...
TL;DR: A multi-feature fusion framework combining MSCN, CLIP, and MLBP features achieves superior and consistent performance for detecting GenAI-generated images across diverse models.


<details>
  <summary>Details</summary>
Motivation: Traditional single-feature detectors lack robustness against diverse and evolving generative AI models, necessitating more comprehensive approaches that leverage multiple complementary visual cues.

Method: Proposes a multi-feature fusion framework combining three distinct feature spaces: (1) MSCN features for low-level statistical deviations, (2) CLIP embeddings for high-level semantic coherence, and (3) MLBP for mid-level texture anomalies.
...
Conclusion: Hybrid representations combining complementary visual cues are essential for robust GenAI image detection, and the proposed framework provides a principled approach for integrating statistical, semantic, and texture features.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D environments for embodied AI lack reliable assessment of functional affordances, creating a gap between semantic understanding and physical feasibility that prevents meaningful agent interactions.

Method: SceneTeract uses a grounded verification engine that decomposes complex activities into atomic actions, then validates each step against agent-specific accessibility requirements (reachability, clearance, navigability) using physical and geometric simulations.

Result: The framework uncovered frequent functional failures in synthetic environments and revealed systematic mismatches between VLMs' semantic confidence and physical feasibility. It also successfully served as a reward engine for VLM post-training to distill geometric constraints.

Conclusion: SceneTeract bridges perception and physical reality in embodied 3D scene understanding, providing a verification suite that addresses the core challenge of assessing functional affordances for diverse embodied agents.
...
TL;DR: AutoFormBench benchmark dataset introduced with 407 real-world forms for evaluating form element detection models, showing YOLOv11 outperforms other YOLO variants and OpenCV approaches.
...
Method: Created AutoFormBench dataset of 407 annotated real-world forms from government, healthcare, and enterprise domains. Compared classical OpenCV approaches with four YOLO architectures (YOLOv8, YOLOv11, YOLOv26-s, YOLOv26-l) for localizing and classifying form elements (checkboxes, input lines, text boxes) across diverse PDF documents.
...
Conclusion: The AutoFormBench benchmark enables systematic evaluation of form element detection models, with YOLOv11 emerging as the best-performing architecture for this task across diverse real-world document types.
...
TL;DR: ViTAS introduces a multi-stage multimodal summarization pipeline that selectively focuses on pathology-relevant visual patches rather than full images, achieving state-of-the-art performance on radiology report summarization by demonstrating that less but more relevant visual input is superior.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal models for radiology report summarization struggle with visual noise and fail to meaningfully improve over text-only baselines in the FINDINGS→IMPRESSION transformation. The paper challenges two prevailing assumptions: (1) that more visual input is always better, and (2) that multimodal models add limited value when findings already contain rich image-derived detail.

Method: ViTAS (Visual-Text Attention Summarizer) is a multi-stage pipeline that combines: ensemble-guided MedSAM2 lung segmentation, bidirectional cross-attention for multi-view fusion, Shapley-guided adaptive patch clustering, and hierarchical visual tokenization feeding a Vision Transformer. The approach selectively focuses on pathology-relevant visual patches rather than full images.

Result: ViTAS achieves state-of-the-art results with 29.25% BLEU-4 and 69.83% ROUGE-L on MIMIC-CXR benchmark. It shows improved factual alignment in qualitative analysis and the highest expert-rated human evaluation scores.

Conclusion: Selectively focusing on pathology-relevant visual patches rather than full images yields substantially better performance for multimodal radiology summarization. Less but more relevant visual input is not only sufficient but superior, challenging the assumption that more visual input is always better.
...
TL;DR: A robust multi-class handwritten digit classification framework combining diffusion-driven feature denoising with hybrid feature representation (NNMF + CNN) to improve noise and adversarial attack robustness.


<details>
  <summary>Details</summary>
Motivation: To create a robust classification system for handwritten digits that can withstand noise and adversarial attacks, inspired by previous work on brain tumor classification. The goal is to improve reliability in both baseline and adversarial settings.

Method: 1. Convert input images to interpretable exemplification using Nonnegative Matrix Factorization (NNMF). 2. Extract deep features using a CNN. 3. Combine NNMF and CNN features into hybrid representation. 4. Apply step diffusion operation in feature space by gradually adding Gaussian noise. 5. Train feature denoiser network to reverse noise and rebuild clean representations. 6. Use denoised features for multi-class classification.

Result: The diffusion-based hybrid model outperforms CNN baseline models in both baseline and adversarial settings (evaluated using AutoAttack), demonstrating effectiveness and robustness while maintaining strong classification performance.

Conclusion: Feature-level diffusion defense is effective for reliable multi-class handwritten digit classification, showing that combining diffusion-driven feature denoising with hybrid feature representation improves robustness against noise and adversarial attacks.
...
TL;DR: Synthetic fractal data can effectively train DL models for dynamic MRI reconstruction, performing comparably to models trained on real cardiac MRI data while avoiding privacy/licensing issues.


<details>
  <summary>Details</summary>
Motivation: To overcome privacy, licensing, and availability limitations of cardiac MRI training datasets by using synthetically generated fractal data for training deep learning reconstruction models.

Method: Generated training dataset using quaternion Julia fractals to create 2D+time images, simulated multi-coil MRI acquisition to produce paired fully sampled and radially undersampled k-space data, trained 3D UNet deep artifact suppression model on fractal data (F-DL) and compared with identical model trained on cardiac MRI data (CMR-DL).

Result: No significant difference in qualitative ranking between F-DL and CMR-DL (p=0.9), both outperformed compressed sensing and low-rank deep image prior (p<0.001). Ventricular volumes and function from F-DL were similar to CMR-DL with no significant bias and acceptable limits of agreement compared to reference cine imaging.

Conclusion: DL models trained using synthetic fractal data can reconstruct real-time cardiac MRI with comparable image quality and clinical measurements to models trained on real cardiac MRI data, providing an open, scalable alternative for developing more generalizable DL reconstruction models.
...
TL;DR: AiS is a style transfer framework that separates structural abstraction from visual stylization, enabling deeper artistic reinterpretation beyond surface appearance changes.


<details>
  <summary>Details</summary>
Motivation: Conventional style transfer methods preserve input geometry and struggle with deeper abstraction in artistic styles, especially for illustrative and nonphotorealistic styles that involve structural reinterpretation.

Method: Two-stage framework: 1) Derives an intermediate abstraction proxy that reinterprets target structure based on style exemplars, 2) Renders the abstraction proxy into final stylized output. Uses shared image space analogy without explicit geometric supervision.

Result: AiS supports wider range of stylistic transformations, improves controllability, and enables more expressive stylization by decoupling abstraction from appearance.

Conclusion: By treating abstraction as an explicit, transferable process separate from visual stylization, AiS captures deeper artistic abstraction behavior that conventional methods miss.
...
TL;DR: Proposes a method for generating long-duration character videos with consistent multi-view appearance using anchor frames, addressing copy-pasting and multi-reference conflicts through Superset Content Anchoring and RoPE as Weak Condition.


<details>
  <summary>Details</summary>
Motivation: Digital character generation faces challenges in maintaining consistent multi-view appearance and expressive identity over long durations. Existing approaches either lack sufficient context to preserve identity or use non-character-centric information as memory, leading to suboptimal consistency.

Method: Represents character visual attributes through compact anchor frames. Introduces Superset Content Anchoring to prevent duplication by providing intra- and extra-training clip cues, and RoPE as Weak Condition to encode positional offsets distinguishing multiple anchors. Includes scalable pipeline to extract anchors from massive videos.

Result: Method generates high-quality character videos exceeding 10 minutes, achieving expressive identity and appearance consistency across views, surpassing existing methods.

Conclusion: The proposed anchor-based approach effectively addresses challenges in character video generation, enabling long-duration, multi-view consistent videos with expressive identity through innovative mechanisms for preventing copy-pasting and resolving multi-reference conflicts.
...
TL;DR: EC-Bench is a new benchmark for evaluating enumeration, counting, and temporal evidence grounding in long-form videos, revealing significant gaps between current MLLMs and human performance.


<details>
  <summary>Details</summary>
Motivation: Existing video counting benchmarks focus on short clips and only evaluate final numerical answers, lacking insight into what should be counted or whether models consistently identify relevant instances across time in long videos.

Method: Introduced EC-Bench with 152 videos longer than 30 minutes and 1,699 queries paired with explicit evidence spans, evaluating 22 multimodal large language models on enumeration, counting, and temporal evidence grounding.

Result: Best MLLM achieved only 29.98% accuracy on Enumeration and 23.74% on Counting, while human performance reached 78.57% and 82.97% respectively, showing strong relationships between enumeration accuracy, temporal grounding, and counting performance.

Conclusion: EC-Bench highlights fundamental limitations of current MLLMs in long-form quantitative video reasoning and establishes a challenging benchmark for future research.
...
TL;DR: DEUS is a novel Open World Object Detection framework that improves unknown object discovery using ETF-based subspace separation and energy-based known distinction to prevent knowledge interference during incremental learning.


<details>
  <summary>Details</summary>
Motivation: Existing OWOD methods struggle with effectively learning unknown object representations due to over-reliance on known class predictions, and memory replay techniques often sacrifice knowledge of newly learned classes while mitigating forgetting of old ones.
...
Conclusion: DEUS effectively addresses key limitations in OWOD by providing better separation between known/unknown objects and reducing knowledge interference during incremental learning, leading to superior unknown object detection capabilities.
...
TL;DR: NeuroBRIDGE framework improves prediction of adolescent substance use initiation by modeling dynamic brain connectivity changes over time using Riemannian geometry and Koopman dynamics conditioned on behavior.


<details>
  <summary>Details</summary>
Motivation: Early identification of adolescents at risk for substance use initiation is crucial but challenging because most existing approaches treat brain connectivity as static or cross-sectional, missing how brain networks evolve over time and interact with behavior.

Method: NeuroBRIDGE is a graph neural network framework that aligns longitudinal functional connectomes in a Riemannian tangent space and combines dual-time attention with behavioral-conditioned Koopman dynamics to capture temporal changes in brain networks.

Result: Evaluated on the ABCD dataset, NeuroBRIDGE outperformed relevant baselines in predicting future substance use initiation while providing interpretable insights into neural pathways associated with risk.

Conclusion: The framework advances understanding of neurodevelopmental risk for substance use initiation and can inform targeted prevention strategies by capturing dynamic brain-behavior relationships over time.
...
TL;DR: SurgTEMP is a multimodal LLM framework for surgical video question answering that addresses temporal modeling challenges through hierarchical visual memory and Surgical Competency Progression training, achieving state-of-the-art performance on the new CholeVidQA-32K dataset.


<details>
  <summary>Details</summary>
Motivation: Current surgical VQA research focuses on static frame analysis, overlooking rich temporal semantics. Surgical video QA faces challenges including low visual contrast, knowledge-driven nature, diverse analytical needs across temporal windows, and hierarchy from basic perception to high-level assessment.

Method: Proposes SurgTEMP with: (1) query-guided token selection module building hierarchical visual memory (spatial and temporal memory banks), and (2) Surgical Competency Progression (SCP) training scheme. These enable effective modeling of variable-length surgical videos while preserving procedure-relevant cues and temporal coherence.

Result: Introduces CholeVidQA-32K dataset with 32K QA pairs and 3,855 video segments (~128 hours) from laparoscopic cholecystectomy, organized into Perception, Assessment, and Reasoning hierarchy. SurgTEMP achieves substantial performance improvements over state-of-the-art multimodal and video LLMs in comprehensive evaluations.

Conclusion: SurgTEMP advances video-based surgical VQA by effectively modeling temporal semantics and supporting diverse assessment tasks, with the new dataset enabling further development in this challenging domain.
...
TL;DR: SurgRec is a scalable, reproducible pretraining recipe for surgical video understanding using two variants (MAE and JEPA) trained on 10,535 videos, achieving superior performance across 16 downstream datasets compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing surgical foundation models are limited by small data scale, lack of procedural diversity, inconsistent evaluation, and absence of reproducible training pipelines, hindering progress in computer-assisted surgical video understanding.

Method: Proposed SurgRec with two variants (SurgRec-MAE and SurgRec-JEPA), curated large multi-source corpus of 10,535 videos (214.5M frames) spanning multiple surgical domains, developed unified pretraining pipeline with balanced sampling, and standardized reproducible benchmark across 16 downstream datasets.

Result: SurgRec consistently achieves superior performance across downstream datasets compared to SSL baselines and vision-language models. VLMs prove unreliable for fine-grained temporal recognition, showing performance gaps and sensitivity to prompt phrasing.

Conclusion: SurgRec provides a reproducible, scalable foundation for building more general surgical video models, with all code, models, and data to be publicly released to advance the community.
...
TL;DR: MAGNet is a Transformer-style graph neural network that jointly models brain structure and function by adaptively learning their interactions, outperforming baselines on cognitive function prediction.


<details>
  <summary>Details</summary>
Motivation: Brain structure and function provide complementary information about neural organization, but modeling them jointly is challenging. Understanding their interactions is crucial for explaining intelligence and cognitive function.

Method: MAGNet uses source-based morphometry from structural MRI and functional network connectivity from resting-state fMRI. It employs a hybrid graph integrating direct/indirect pathways, local-global attention to refine connectivity importance, and a joint loss enforcing cross-modal coherence while optimizing prediction end-to-end.
...
Conclusion: MAGNet provides an effective framework for adaptively learning structure-function interactions in the brain, enabling better prediction of cognitive function through multimodal integration.
...
TL;DR: This pilot study explores adding volumetric MRI (FLAIR) as a third modality to existing bimodal (histopathology + genomic) frameworks for brain tumor prognosis, finding modest improvements but limited statistical significance due to small sample sizes.


<details>
  <summary>Details</summary>
Motivation: While multimodal deep learning combining histopathology and genomic data has improved brain tumor prognostic accuracy, the potential contribution of volumetric MRI within unified survival frameworks remains unexplored. The study aims to investigate whether adding MRI as a third modality can further enhance prognostic performance.

Method: The study extends a bimodal framework by incorporating Fluid Attenuated Inversion Recovery (FLAIR) MRI from BraTS2021 as a third modality. Using the TCGA-GBMLGG cohort (664 patients), researchers evaluated three unimodal models, nine bimodal configurations, and three trimodal configurations across early, late, and joint fusion strategies. Performance was assessed using Composite Score (CS) metrics.

Result: Trimodal early fusion achieved the best exploratory Composite Score (CS = 0.854), with a small improvement (ΔCS = +0.011) over bimodal baselines, though not statistically significant (p = 0.250). MRI alone showed reasonable discrimination (CS = 0.755) but didn't substantially improve bimodal pairs. All MRI-containing experiments were limited to 19 test patients, resulting in wide confidence intervals that preclude definitive conclusions.

Conclusion: The findings provide preliminary evidence that a third imaging modality may add prognostic value even with limited sample sizes, and that additional modalities require sufficient multimodal context to contribute effectively. However, the small cohort size and resulting statistical limitations highlight the need for larger studies to draw definitive conclusions about MRI's added value in multimodal brain tumor prognosis frameworks.
...
TL;DR: An integrated HMD-based AR surgical navigation framework that tracks markers on patients/instruments, calibrates tools, registers images to patients, and achieves sub-5mm accuracy for surgical guidance.


<details>
  <summary>Details</summary>
Motivation: Current HMD-AR surgical navigation systems require complex integration with specialized knowledge, hindering scientific progress. The authors aim to create an adaptable, integrated framework that simplifies AR surgical navigation for diverse applications.

Method: Framework tracks 2D patterns as reference markers on patients and instruments, uses pivot and reference-based calibration for surgical tools, performs image-to-patient registration via point-based matching and manual positioning, and is evaluated on HoloLens 2 and Magic Leap 2 devices.

Result: Achieved mean tooltip calibration accuracy of 1 mm, registration accuracy of 3 mm, and targeting accuracy below 5 mm for AR-guided needle insertion and rib fracture localization in phantom setups.

Conclusion: The framework provides an easy-to-use, configurable tool for HMD-based AR surgical navigation that can be extended to many surgical applications, with code publicly available on GitHub.
...
TL;DR: CPGNet is an asymmetric RGB-polarization framework for camouflaged object detection that uses conditional polarization guidance to explicitly regulate RGB feature learning, outperforming state-of-the-art methods with lower complexity.


<details>
  <summary>Details</summary>
Motivation: Existing polarization-based COD methods rely on complex visual encoders and fusion mechanisms, increasing model complexity while failing to fully exploit how polarization can explicitly guide hierarchical RGB representation learning.

Method: Proposes CPGNet with: 1) lightweight polarization interaction module for joint modeling and reliable guidance generation, 2) conditional guidance mechanism for dynamic RGB feature modulation using polarization priors, 3) polarization edge-guided frequency refinement strategy, and 4) iterative feedback decoder for coarse-to-fine feature calibration.
...
Conclusion: CPGNet effectively addresses limitations of existing polarization-based approaches by introducing explicit conditional polarization guidance for RGB feature learning, achieving superior performance with reduced complexity in camouflaged object detection.
...
TL;DR: GeoCodeBench is a PhD-level benchmark for evaluating AI coding capabilities in complex 3D geometric vision, revealing current models (best: GPT-5 at 36.6% pass rate) still struggle with dependable scientific coding.
...
Method: Created GeoCodeBench by curating fill-in-the-function tasks from recent 3D vision papers, using tool-assisted extraction from official repositories followed by human screening. Generated diverse edge-case unit tests for automatic scoring. Evaluated 8 representative models (open- and closed-source) with two-level hierarchy: General 3D capability and Research capability.

Result: Best model (GPT-5) achieved only 36.6% pass rate, showing large gap in 3D scientific coding. Research tasks are harder than general 3D tasks. Context ablation revealed that providing full paper text performs worse than cutting at Method section, indicating challenges in long-context scientific comprehension.

Conclusion: GeoCodeBench serves as a rigorous testbed for advancing from generic coding to trustworthy 3D geometric vision coding, highlighting significant challenges in current AI capabilities for scientific coding tasks.
...
TL;DR: Video diffusion models show emergent reasoning in maze solving, committing to motion plans early in denoising. Path length (not obstacle density) determines difficulty, with 12-step threshold. ChEaP method chains promising early plans to boost long-horizon maze accuracy from 7% to 67%.
...
Method: Analyzed video diffusion models' internal planning dynamics in maze solving, identified early plan commitment and path length as key factors, then developed ChEaP (Chaining with Early Planning) that selects promising seeds based on early plans and chains them together.

Result: Found: 1) Early plan commitment within first few denoising steps, 2) Path length (not obstacle density) is dominant difficulty predictor with 12-step failure threshold. ChEaP improved long-horizon maze accuracy from 7% to 67% and boosted performance 2.5x on hard tasks in Frozen Lake and VR-Bench across Wan2.2-14B and HunyuanVideo-1.5 models.

Conclusion: Current video models possess deeper reasoning capabilities than previously recognized, which can be elicited more reliably with better inference-time scaling. The findings enable practical improvements through methods like ChEaP that leverage early planning signals.
...
<details>
  <summary>Details</summary>
Motivation: Existing video generation models rely on perspective views that only synthesize limited observations of scenes, leading to issues with completeness and global consistency. The authors aim to overcome these limitations by leveraging panoramic representations.

Method: A two-stage framework: 1) Preview stage - trajectory-controlled video generation creates quick scene overview from input image/video; 2) Refine stage - temporal extension and spatial upsampling produce long-range, high-resolution videos. Trained on panoramic video datasets with both synthetic and real-world captured videos.

Result: Consistently outperforms state-of-the-art methods in visual quality, controllability, and long-term scene consistency, both qualitatively and quantitatively. Also demonstrates extensions including real-time video generation and 3D reconstruction.

Conclusion: OmniRoam successfully enables high-fidelity world wandering by exploiting panoramic representations' rich scene coverage and inherent spatial-temporal consistency, addressing limitations of perspective-based approaches.
...
### [117] [Enhanced Channel Estimation for Flexible Intelligent Metasurface-Aided Communication Systems](https://arxiv.org/abs/2603.29098)
*Jinyue Jiang,Jiancheng An,Lu Gan,Naofal Al-Dhahir,Arumugam Nallanathan,Zhu Han*

Main category: cs.IT

TL;DR: FIM improves channel estimation via shape optimization and OMP method, outperforming rigid UPAs in accuracy and SNR.


<details>
  <summary>Details</summary>
Motivation: Flexible intelligent metasurfaces (FIMs) can dynamically morph their shape to improve channel conditions, but require effective channel estimation methods to fully exploit this capability.

Method: Customized orthogonal matching pursuit (OMP) method for channel estimation, with optimization of FIM surface shape to minimize measurement matrix column coherence, subject to morphing constraints.

Result: FIM significantly outperforms conventional rigid uniform planar arrays (UPAs), improving channel estimation accuracy and achieving SNR improvement even with estimated channel parameters.

Conclusion: FIMs offer substantial advantages over rigid arrays by enabling dynamic shape optimization for better channel estimation and SNR performance in communication systems.
...
### [118] [Scalable and Near-Optimal Discrete Phase Shift Optimization for Reconfigurable Intelligent Surfaces with Over 20,000 Elements](https://arxiv.org/abs/2603.29144)
*Yuto Hama,Daisuke Kitayama,Kensuke Inaba,Toshimori Honjo,Hiroki Takesue,Naoki Ishikawa,Hiroyuki Takahashi*

Main category: cs.IT

TL;DR: CIM-based optimization for RIS discrete phase shifts achieves efficient large-scale optimization with up to 22,201 elements, outperforming conventional methods in complexity and scalability.


<details>
  <summary>Details</summary>
Motivation: Conventional methods for RIS discrete phase optimization suffer from exponentially increasing complexity with iterative convex approximation or combinatorial search. There's a need for efficient large-scale optimization methods for practical RIS deployments with discrete phase shift constraints.

Method: Formulates RIS discrete phase optimization as quadratic Ising model, maps quantized phase states to spin variables, uses coherent Ising machine (CIM) with optical oscillators to physically explore solution space, and introduces spin-size reduction technique for line-of-sight conditions.

Result: Experimentally solves quadratic optimization for RISs with up to 22,201 elements using real hardware CIM, achieves physically consistent beam patterns in both LOS and NLOS environments, attains theoretical gain from binary to quaternary phase shift, and demonstrates scalability with spin-size reduction.

Conclusion: CIM-based optimization offers practical and highly scalable solution for large RIS deployments with discrete phase shift constraints, overcoming limitations of conventional methods through physical exploration of solution space.
...
### [119] [A Structural Characterization of Cyclotomic Cosets with Applications to Affine-Invariant Codes and BCH Codes](https://arxiv.org/abs/2603.29150)
*Xiongkun Zheng,Dabin Zheng,Xiaoqiang Wang,Mu Yuan*

Main category: cs.IT

TL;DR: The paper studies affine-invariant codes defined by descendant sets of cyclotomic cosets, establishes combinatorial results on descendant set sizes, derives explicit dimension formulas and dual distance bounds, and specializes to narrow-sense primitive BCH codes with specific designed distances.


<details>
  <summary>Details</summary>
Motivation: Affine-invariant codes have rich algebraic structure and strong theoretical properties, making them important for coding theory. The paper aims to better understand these codes by studying a specific family defined by descendant sets of cyclotomic cosets.

Method: The authors first establish a combinatorial result determining the exact size of descendant sets in cyclotomic cosets. Using this result, they derive explicit formulas for dimensions of affine-invariant codes and their associated cyclic codes, and establish lower bounds on minimum distances of their duals.

Result: The paper provides: 1) Exact formulas for descendant set sizes, 2) Explicit dimension formulas for affine-invariant codes and their cyclic counterparts, 3) Lower bounds on dual minimum distances, 4) Specialized results for narrow-sense primitive BCH codes with designed distance δ = (b+1)q^{m-t-1}, including exact dimension formulas and improved distance bounds.

Conclusion: The results extend and sharpen previously known results, providing refined tools for parametric analysis of BCH codes and their duals. The combinatorial insight into descendant sets enables better understanding of affine-invariant codes and their properties.
...
### [120] [Finite Blocklength Covert Communication over Quasi-Static Multiple-Antenna Fading Channels](https://arxiv.org/abs/2603.29645)
*Changhong Liu,Jingjing Wang,Qiaosheng Zhang,Jinpeng Xu,Lin Zhou*

Main category: cs.IT

TL;DR: This paper analyzes covert communication over quasi-static multi-antenna fading channels with finite blocklength, showing that covert rate scales as Θ(n^(-1/2)) and CSI availability doesn't affect performance, unlike non-covert scenarios.


<details>
  <summary>Details</summary>
Motivation: The ITU white book calls for high-security, low-latency communications. Covert communication provides security while quasi-static fading models low-latency scenarios. The paper addresses the need to understand finite blocklength performance of covert communication in multi-antenna systems under various CSI availability conditions.

Method: The authors study covert communication over quasi-static multi-antenna fading channels using KL divergence as covertness metric. They analyze all four CSI availability cases for legitimate users, assuming the warden has perfect CSI. They extend the quasi-η-neighborhood framework to fading channels and refine non-covert analysis by carefully controlling higher-order terms and exploiting covert outage probability properties.

Result: The first-order covert rate scales as Θ(n^(-1/2)) with coefficient determined by channel matrix traces, while second-order rate vanishes. Unlike non-covert results, CSI availability at legitimate users doesn't affect finite blocklength performance for covert communication. Multiple-antenna systems provide significant spatial diversity gain for covert communication.

Conclusion: Covert communication over quasi-static multi-antenna fading channels exhibits fundamentally different behavior from non-covert communication, with CSI availability becoming irrelevant and spatial diversity providing substantial benefits. The square root law scaling and vanishing second-order rate characterize the finite blocklength performance of covert communication in these systems.
...
### [121] [Joint Identification and Sensing with Noisy Feedback: A Task-Oriented Communication Framework for 6G](https://arxiv.org/abs/2603.29649)
*Yaning Zhao,Holger Boche,Christian Deppe*

Main category: cs.IT

TL;DR: This paper studies joint identification and sensing (JIDAS) over state-dependent channels with noisy feedback, deriving fundamental limits for simultaneous message identification and channel state estimation.


<details>
  <summary>Details</summary>
Motivation: Task-oriented communication in 6G systems focuses on supporting decisions/actions rather than full message reconstruction. Identification codes provide a natural abstraction for this paradigm. The paper is motivated by the strong impact of feedback on identification and growing interest in integrated communication and sensing.

Method: The paper studies joint identification and sensing (JIDAS) over state-dependent discrete memoryless channels with noisy strictly causal feedback. The transmitter conveys identification messages while simultaneously estimating channel state from feedback signals. Both deterministic and randomized coding schemes are analyzed.

Result: The authors derive lower and upper bounds on the capacity-distortion function for JIDAS under noisy feedback. The results quantify fundamental limits and recover existing noiseless-feedback characterizations as special cases.

Conclusion: The paper establishes theoretical foundations for joint identification and sensing in task-oriented communication systems with noisy feedback, providing capacity-distortion tradeoffs that generalize previous noiseless-feedback results.
...
### [122] [α-Fair Multistatic ISAC Beamforming for Multi-User MIMO-OFDM Systems via Riemannian Optimization](https://arxiv.org/abs/2603.29717)
*Hyeonho Noh,Jonggyu Jang*

Main category: cs.IT

TL;DR: Proposes α-fair multistatic ISAC framework for multi-user MIMO-OFDM systems where communication users act as passive bistatic receivers, optimizing fairness in sensing performance rather than aggregate metrics.


<details>
  <summary>Details</summary>
Motivation: Existing ISAC works optimize aggregate sensing metrics which favor geometrically advantageous targets, leading to unfair sensing performance across different targets. Need to ensure fair sensing performance for all targets regardless of their geometric positions.

Method: Minimizes α-fairness utility over per-target Cramér-Rao lower bounds (CRLBs) subject to per-user minimum data rate and transmit power constraints. Uses Riemannian conjugate gradient (RCG) method with smooth penalty reformulation to solve the non-convex optimization problem.

Result: Simulation results validate the proposed scheme achieves favorable sensing fairness-communication trade-off, effectively balancing fair sensing performance across targets while maintaining communication requirements.

Conclusion: The α-fair multistatic ISAC framework successfully addresses sensing fairness issues in multi-user MIMO-OFDM systems, providing a balanced approach to integrated sensing and communication with fair target coverage.
...
### [123] [Implementing Basic Arithmetic in $\mathbb{F}_p$ via $\mathbb{F}_2$, and Its Application for Computing the Hamming Distance of Linear Codes](https://arxiv.org/abs/2603.29942)
*Fernando Hernando,Gregorio Quintana-Ortí*

Main category: cs.IT

TL;DR: A new method for performing arithmetic in finite fields F_p using binary operations over F_2, applied to compute minimum Hamming distance of random linear codes for F_3 and F_7, with implementations outperforming existing software.


<details>
  <summary>Details</summary>
Motivation: Current methods for arithmetic in finite fields F_p (for prime p>2) may not be optimal when leveraging traditional binary operations over F_2. There's a need for more efficient approaches that can accelerate computations like minimum Hamming distance calculations for error-correcting codes.

Method: Develop a new general method for performing basic arithmetic in finite fields F_p using traditional binary operations over F_2. The method employs techniques like isometric addition to accelerate computations. Implementations are developed in C programming language for computing Hamming distance.

Result: The new arithmetic method is efficient and competitive with current state-of-art methods. Implementations for computing Hamming distance clearly outperform both state-of-art licensed software and open-source software (Magma and GAP/Guava) on various hardware platforms including single-core processors, multicore processors, and shared-memory multiprocessors.

Conclusion: The proposed method provides an effective approach for finite field arithmetic using binary operations, enabling significant performance improvements in practical applications like Hamming distance computation for error-correcting codes across different computing architectures.
...
### [124] [Reducing Subpacketization in Device-to-Device Coded Caching via Heterogeneous File Splitting](https://arxiv.org/abs/2603.29945)
*Xiang Zhang,Giuseppe Caire,Mingyue Ji*

Main category: cs.IT

TL;DR: Extends packet type-based D2D coded caching framework to heterogeneous subpacketization, allowing varying packet sizes across types to achieve better subpacketization reduction while maintaining optimal communication rate.


<details>
  <summary>Details</summary>
Motivation: Existing packet type designs use homogeneous subpacketization where all packets have identical size, limiting achievable subpacketization reduction in certain parameter regimes. Need to extend to heterogeneous subpacketization for better performance.

Method: Extend PT framework to heterogeneous subpacketization with refined type classification. Jointly optimize packet sizes, user grouping, and multicast transmitter selection to minimize overall subpacketization level while preserving optimal rate. Construct new class of D2D coded caching schemes for specific parameter regime (K, KM/N) = (2q+1, 2r).

Result: Proposed construction achieves constant-factor reduction in subpacketization compared to Ji-Caire-Molisch (JCM) caching scheme. Complements existing PT designs that are not applicable in this parameter regime.

Conclusion: Heterogeneous packet type framework enables better subpacketization reduction than homogeneous approaches, extending applicability to new parameter regimes while maintaining optimal communication rates in D2D coded caching.
...
### [125] [OneComp: One-Line Revolution for Generative AI Model Compression](https://arxiv.org/abs/2603.28845)
...
TL;DR: OneComp is an open-source compression framework that automates post-training quantization for foundation models, converting expert workflows into reproducible, hardware-aware pipelines.


<details>
  <summary>Details</summary>
Motivation: Foundation model deployment faces memory, latency, and cost constraints. Post-training compression can help but practical implementation is challenging due to fragmented quantization algorithms, precision budgets, calibration strategies, and hardware dependencies.

Method: OneComp automatically inspects models, plans mixed-precision assignments, and executes progressive quantization stages (layer-wise compression → block-wise refinement → global refinement). It treats the first quantized checkpoint as a deployable pivot, ensuring quality improves with more compute investment.

Result: OneComp bridges the gap between compression research and production deployment by converting state-of-the-art compression algorithms into an extensible, open-source, hardware-aware pipeline.

Conclusion: OneComp transforms the complex expert workflow of model compression into a reproducible, resource-adaptive pipeline that enables practical deployment of compressed foundation models.
...
### [126] [Structural Pass Analysis in Football: Learning Pass Archetypes and Tactical Impact from Spatio-Temporal Tracking Data](https://arxiv.org/abs/2603.28916)
*Oktay Karakuş,Hasan Arkadaş*

Main category: cs.LG

TL;DR: A framework for analyzing football passes based on their impact on defensive structure, using tracking data to quantify how passes disrupt opponent organization.


<details>
  <summary>Details</summary>
Motivation: Existing pass evaluation metrics focus on outcomes like scoring probability or possession value, but provide limited insight into how passes influence the opponent's defensive organization. There's a need for structural analysis of how passes alter defensive spatial configurations.

Method: Uses synchronized tracking/event data to derive three structural metrics: Line Bypass Score, Space Gain Metric, and Structural Disruption Index. These are combined into a composite Tactical Impact Value (TIV). Applied to 2022 FIFA World Cup data with unsupervised clustering to identify pass archetypes.

Result: Identified four interpretable pass archetypes: circulatory, destabilizing, line-breaking, and space-expanding passes. Higher TIV passes significantly increase territorial progression, especially into final third and penalty box. Team-level analysis reveals distinctive structural passing styles, and player-level analysis shows build-up defenders as key drivers of structural progression.

Conclusion: The framework demonstrates how structural representations from tracking data can reveal interpretable tactical patterns in football, providing deeper insights into how passes influence defensive organization beyond traditional outcome-based metrics.
...
### [127] [Beta-Scheduling: Momentum from Critical Damping as a Diagnostic and Correction Tool for Neural Network Training](https://arxiv.org/abs/2603.28921)
*Ivan Pasichnyk*

Main category: cs.LG

TL;DR: Derives time-varying momentum schedule from critically damped harmonic oscillator, achieving faster convergence and enabling cross-optimizer invariant layer diagnostics for targeted network corrections.


<details>
  <summary>Details</summary>
Motivation: Standard neural network training uses constant momentum (0.9) with limited theoretical justification. The paper aims to provide a principled, parameter-free momentum schedule and diagnostic tool for identifying problematic layers across different optimizers.

Method: Derives time-varying momentum schedule from critically damped harmonic oscillator: μ(t) = 1 - 2√α(t), where α(t) is the current learning rate. This beta-schedule requires no additional parameters beyond existing learning rate schedule. Also proposes hybrid schedule combining physics momentum for early convergence and constant momentum for final refinement.

Result: On ResNet-18/CIFAR-10: 1.9x faster convergence to 90% accuracy; per-layer gradient attribution produces cross-optimizer invariant diagnostic (100% overlap between SGD and Adam); surgical correction of identified layers fixes 62 misclassifications while retraining only 18% of parameters; hybrid schedule reaches 95% accuracy fastest among five methods tested.

Conclusion: Main contribution is not accuracy improvement but a principled, parameter-free tool for localizing and correcting specific failure modes in trained networks. The physics-derived momentum schedule provides faster convergence while enabling cross-optimizer invariant diagnostics for targeted network corrections.
...
### [128] [A Neural Tension Operator for Curve Subdivision across Constant Curvature Geometries](https://arxiv.org/abs/2603.28937)
*Hassan Ugail,Newton Howard*

Main category: cs.LG

TL;DR: A learned subdivision scheme that replaces global tension parameters with per-edge insertion angles predicted by a small neural network, working across Euclidean, spherical, and hyperbolic geometries without architectural changes.


<details>
  <summary>Details</summary>
Motivation: Classical subdivision schemes use a single global tension parameter and require separate formulations for different geometries (Euclidean, spherical, hyperbolic). This limits adaptability and requires multiple implementations.

Method: A 140K-parameter neural network predicts per-edge insertion angles using local intrinsic features and geometry embeddings. A constrained sigmoid output ensures vertices stay within valid angular ranges. The same network works across all three geometries without modification.

Result: On 240 validation curves, the method achieves lower bending energy and angular roughness than fixed-tension and manifold-lift baselines. On out-of-distribution ISS orbital ground-tracks, bending energy reduced by 41% and angular roughness by 68% with modest Hausdorff distance increase.

Conclusion: The learned predictor provides adaptive per-edge tension control across multiple geometries, achieving better smoothness-fidelity trade-offs than classical approaches while maintaining theoretical guarantees and generalization capability.
...
TL;DR: Polar Linear Algebra framework for operator learning using polar geometry with radial and angular components, enabling spectral decomposition for improved stability, reduced parameters, and parallelization.


<details>
  <summary>Details</summary>
Motivation: To provide a new spectral perspective on operator learning that offers better interpretability, stability, and parallelization capabilities compared to traditional spatial approaches.

Method: Introduces Polar Linear Algebra framework with linear radial and periodic angular components, defines associated operators with spectral properties, imposes self-adjoint-inspired spectral constraints for stability.

Result: Demonstrates reliable training of polar and fully spectral operators on MNIST benchmark, shows improved stability/convergence with spectral constraints, reduces parameter count/computational complexity, enables interpretable spectral modes.

Conclusion: Polar Linear Algebra offers a novel spectral approach to operator learning that provides interpretability, efficiency, and natural parallelization through orthogonal eigenmode decomposition.
...
### [130] [\texttt{ReproMIA}: A Comprehensive Analysis of Model Reprogramming for Proactive Membership Inference Attacks](https://arxiv.org/abs/2603.28942)
*Chihan Huang,Huaijin Wang,Shuai Wang*

Main category: cs.LG

TL;DR: ReproMIA: A novel membership inference attack framework using model reprogramming to amplify privacy leakage signals, achieving state-of-the-art performance especially in low false positive rate regimes across various model architectures.


<details>
  <summary>Details</summary>
Motivation: Deep learning models in critical domains raise privacy concerns due to data memorization. Traditional membership inference attacks (MIAs) face limitations: high computational costs from shadow model training and poor performance under low false positive rate constraints.

Method: Introduces ReproMIA, a proactive framework that leverages model reprogramming principles as an active signal amplifier for privacy leakage. The approach proactively induces and magnifies latent privacy footprints in model representations. Provides specialized implementations for LLMs, Diffusion Models, and Classification Models.

Result: Comprehensive experiments across 10+ benchmarks show ReproMIA consistently outperforms state-of-the-art baselines. Achieves significant improvements in low-FPR regimes: average 5.25% AUC and 10.68% TPR@1%FPR increase for LLMs, and 3.70% AUC and 12.40% TPR@1%FPR increase for Diffusion Models.

Conclusion: ReproMIA represents a transformative approach to membership inference by using model reprogramming to amplify privacy signals, overcoming computational and performance limitations of traditional MIAs while achieving superior results across diverse model architectures.
...
### [131] [Differentiable Initialization-Accelerated CPU-GPU Hybrid Combinatorial Scheduling](https://arxiv.org/abs/2603.28943)
*Mingju Liu,Jiaqi Yin,Alvaro Velasquez,Cunxi Yu*

Main category: cs.LG

TL;DR: A hybrid CPU-GPU framework that combines differentiable optimization with classical ILP solvers to solve combinatorial scheduling problems, achieving 10× performance gains and <0.1% optimality gap.


<details>
  <summary>Details</summary>
Motivation: Scheduling problems are fundamental to many optimization tasks in computing systems but remain challenging to solve optimally at scale due to their NP-hard nature. Current ILP solvers struggle with large-scale combinatorial scheduling problems, creating a need for more efficient approaches.

Method: A novel hybrid approach that uses differentiable presolving on GPU to rapidly generate high-quality partial solutions, which then serve as warm-starts for commercial ILP solvers (CPLEX, Gurobi) and open-source solver HiGHS. This enables improved early pruning compared to standalone solvers.

Result: Empirical results across industry-scale benchmarks demonstrate up to 10× performance gain over baseline solvers, narrowing the optimality gap to less than 0.1%. The method shows significant improvement in early pruning capabilities.

Conclusion: This work represents the first demonstration of using differentiable optimization to initialize exact ILP solvers for combinatorial scheduling, opening new opportunities to integrate machine learning infrastructure with classical exact optimization methods across broader domains.
...
### [132] [Multi-Agent LLMs for Adaptive Acquisition in Bayesian Optimization](https://arxiv.org/abs/2603.28959)
*Andrea Carbonati,Mohammadsina Almasi,Hadis Anahideh*

Main category: cs.LG

TL;DR: LLMs struggle with exploration-exploitation trade-offs in optimization. Multi-agent framework separates strategy control from candidate generation, improving search effectiveness.


<details>
  <summary>Details</summary>
Motivation: Understanding how LLMs reason about exploration-exploitation trade-offs in optimization tasks, as current LLM-based optimization lacks explicit control mechanisms compared to Bayesian Optimization.

Method: Proposes multi-agent framework with strategy agent assigning interpretable weights to search criteria and generation agent producing candidates conditioned on these weights, making exploration-exploitation decisions explicit and adjustable.

Result: Multi-agent approach substantially improves LLM-mediated search effectiveness across continuous optimization benchmarks compared to single-agent approaches that suffer from cognitive overload and unstable dynamics.

Conclusion: Decomposing exploration-exploitation control into strategic policy mediation and tactical candidate generation makes search behavior more interpretable, controllable, and effective for LLM-based optimization.
...
### [133] [The Spectral Edge Thesis: A Mathematical Framework for Intra-Signal Phase Transitions in Neural Network Training](https://arxiv.org/abs/2603.28964)
*Yongzhong Xu*

Main category: cs.LG

TL;DR: Spectral edge thesis: Neural network training phase transitions (grokking, capability gains, loss plateaus) are controlled by the spectral gap of rolling-window Gram matrix of parameter updates, with gap collapse disrupting learning.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental mechanisms behind neural network training phase transitions like grokking, capability gains, and loss plateaus, which are not well explained by classical theories in extreme aspect ratio regimes.

Method: Develop spectral edge theory based on three axioms: (1) gap dynamics governed by Dyson-type ODE with curvature asymmetry, damping, and gradient driving; (2) spectral loss decomposition linking modes' learning contributions to Davis-Kahan stability coefficients; (3) Gap Maximality Principle showing k* is dynamically privileged position. Test across six model families (150K-124M parameters).

Result: Gap dynamics precede every grokking event (24/24 with weight decay, 0/24 without), gap position is optimizer-dependent (Muon: k*=1, AdamW: k*=2), and 19/20 quantitative predictions confirmed. Adiabatic parameter controls circuit stability: A≪1 (plateau), A∼1 (phase transition), A≫1 (forgetting).

Conclusion: Spectral gap of rolling-window Gram matrix is fundamental controller of neural network training phase transitions, with gap collapse being the only disruption to learning. Framework unifies multiple existing theories including edge of stability, Tensor Programs, and neural scaling laws.
...
### [134] [An Explicit Surrogate for Gaussian Mixture Flow Matching with Wasserstein Gap Bounds](https://arxiv.org/abs/2603.28992)
*Elham Rostami,Taous-Meriem Laleg-Kirati,Hamidou Tembine*

Main category: cs.LG

TL;DR: Training-free flow matching between Gaussian mixture models using explicit velocity fields with closed-form surrogate for kinetic transport cost, analyzing approximation accuracy and providing practical regime map.


<details>
  <summary>Details</summary>
Motivation: To develop efficient methods for transporting between Gaussian mixture models without training, providing analytic expressions for transport costs that avoid computationally expensive matrix operations like matrix square-roots.

Method: Construct component-wise Gaussian paths with affine velocity fields satisfying continuity equation, derive closed-form surrogate for pairwise kinetic transport cost, analyze approximation accuracy with second-order agreement proofs and cubic error bounds, introduce path-splitting strategy for nonlocal regimes.

Result: Derived simple analytic expression for surrogate cost from kinetic energy of induced flow, proved second-order agreement in local commuting regime with explicit cubic error bound, developed path-splitting strategy for nonlocal regimes, created practical regime map showing when surrogate is accurate versus when exact Gaussian Wasserstein geodesic method is preferable.

Conclusion: The training-free flow matching approach provides computationally efficient surrogate for Gaussian mixture transport with clear theoretical guarantees and practical guidance on when to use the surrogate versus exact methods based on regime analysis.
...
### [135] [Improving Efficiency of GPU Kernel Optimization Agents using a Domain-Specific Language and Speed-of-Light Guidance](https://arxiv.org/abs/2603.29010)
*Siva Kumar Sastry Hari,Vignesh Balaji,Sana Damani,Qijing Huang,Christos Kozyrakis*

Main category: cs.LG

TL;DR: μCUTLASS introduces a DSL and SOL guidance to optimize LLM-based GPU kernel optimization, achieving 1.56x speedup over PyTorch with reduced search costs.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based GPU kernel optimization suffers from inefficient search: too low abstraction wastes reasoning on details, too high abstraction misses optimizations, and agents can't identify diminishing returns, leading to wasted resources.

Method: Two key innovations: (1) μCUTLASS DSL - a compact domain-specific language that balances abstraction level for efficient reasoning while preserving optimization levers; (2) SOL guidance - uses first-principles performance bounds to steer search, budget trials, and detect benchmark-gaming kernels.

Result: On 59 KernelBench problems: DSL alone turns 0.40x regression into 1.27x speedup over PyTorch; adding SOL guidance raises to 1.56x. SOL guidance saves 19-43% tokens while retaining ≥95% speedup, with best policy achieving 1.68x efficiency gain. Weaker models outperform stronger baselines at lower cost.

Conclusion: The right abstraction level (via DSL) combined with performance-bound guidance (SOL) significantly improves LLM-based GPU kernel optimization efficiency, reducing search costs while achieving better performance and detecting gaming cases.
...
### [136] [From Astronomy to Astrology: Testing the Illusion of Zodiac-Based Personality Prediction with Machine Learning](https://arxiv.org/abs/2603.29033)
*Abhinna Sundar Samantaray,Finnja Annika Fluhrer,Dhruv Saini,Omkar Charaple,Anish Kumar Singh,Dhruv Vansraj Rathore*

Main category: cs.LG

TL;DR: This paper uses machine learning to test zodiac-based personality prediction, finding no predictive power beyond random chance, attributing astrology's apparent success to cognitive biases and cultural narratives.


<details>
  <summary>Details</summary>
Motivation: Despite astrology's cultural persistence and influence on important life decisions (marriage, naming, rituals), it lacks physical plausibility and statistical reliability. The paper aims to systematically test zodiac-based personality prediction using controlled machine learning methods.

Method: Created synthetic dataset with zodiac signs and personality traits (100 traits total, 10 per sign with intentional overlap). Trained Logistic Regression, Random Forest, and neural network classifiers to predict personality from zodiac features and nuisance covariates. Used shuffled-label controls for comparison.
...
Conclusion: Zodiac-based systems don't provide reliable information for predicting human behavior. Their apparent success stems from trait universality, category overlap, cognitive biases (Barnum effect, confirmation bias), and interpretive flexibility. They function as culturally durable narrative frameworks rather than predictive tools.
...
TL;DR: A hierarchical latent risk-aware ML framework predicts clinical trial operational success before initiation using 180+ drug/trial features, achieving high F1-scores (0.91-0.93) across phases.


<details>
  <summary>Details</summary>
Motivation: Clinical trials face high costs, long timelines, and operational risks, but existing AI methods often use metrics unavailable at design phase or focus on isolated aspects, limiting real-world applicability for prospective prediction.

Method: Two-stage hierarchical framework: first predicts intermediate latent operational risk factors using 180+ drug/trial features available pre-initiation, then integrates these predicted risks into downstream model to estimate operational success probability. Uses curated TrialsBank database (13,700 trials) with staged data-splitting to prevent leakage, benchmarked with XGBoost, CatBoost, and Explainable Boosting Machines.

Result: Strong out-of-sample performance across Phase I-III: F1-scores of 0.93, 0.92, and 0.91 respectively. Latent risk drivers improve discrimination of operational failures, and performance remains robust under independent inference evaluation.

Conclusion: Clinical trial operational success can be prospectively forecasted using latent risk-aware AI framework, enabling early risk assessment and supporting data-driven clinical development decision-making before trial initiation.
...
TL;DR: ARCS is an amortized analog circuit generation system that produces SPICE-simulatable designs in milliseconds using learned generators and SPICE-based ranking, achieving 99.9% simulation validity with 40x fewer evaluations than genetic algorithms.


<details>
  <summary>Details</summary>
Motivation: Current search-based methods for analog circuit generation take minutes to produce designs, which is too slow for rapid prototyping and design-space exploration. There's a need for faster methods that can generate valid circuit designs efficiently.

Method: Hybrid pipeline combining two learned generators (graph VAE and flow-matching model) with SPICE-based ranking. Uses Group Relative Policy Optimization (GRPO) to address REINFORCE failure modes, topology-aware Graph Transformer, grammar-constrained decoding with token masking, and Best-of-3 candidate selection.

Result: Achieves 99.9% simulation validity (reward 6.43/8.0) across 32 topologies using only 8 SPICE evaluations (40x fewer than genetic algorithms). Single-model inference reaches 85% simulation validity in 97ms (600x faster than random search). GRPO improves simulation validity by +9.6pp over REINFORCE in only 500 RL steps.

Conclusion: ARCS provides >1000x speed advantage enabling rapid prototyping, design-space exploration, and warm-starting search methods (recovering 96.6% of GA quality with 49% fewer simulations). While not yet matching per-design quality of search-based optimization, the speed benefits make it valuable for practical applications.
...
### [139] [On the Mirage of Long-Range Dependency, with an Application to Integer Multiplication](https://arxiv.org/abs/2603.29069)
*Zichao Wei*

Main category: cs.LG

TL;DR: Neural networks can perfectly generalize on integer multiplication using a 2D grid representation with local operations, challenging the belief that multiplication requires long-range dependencies.


<details>
  <summary>Details</summary>
Motivation: The paper challenges the conventional wisdom that integer multiplication is hard for neural networks due to O(n) long-range dependencies from carry chains. The authors argue this is a "mirage" caused by poor computational representation rather than an intrinsic property of multiplication.

Method: Represent two n-bit binary integers as a 2D outer-product grid, transforming multiplication into local 3×3 neighborhood operations. Use a neural cellular automaton with only 321 learnable parameters to learn this local computation.

Result: The neural cellular automaton achieves perfect length generalization up to 683× the training range. In contrast, five alternative architectures (Transformer, Transformer+RoPE, Mamba, etc.) all fail under the same representation despite having more parameters.

Conclusion: Long-range dependency in multiplication is not intrinsic but a representation artifact. The paper suggests that tasks diagnosed as requiring long-range dependency should first be examined for whether the dependency is intrinsic or induced by computational spacetime.
...
### [140] [Realistic Market Impact Modeling for Reinforcement Learning Trading Environments](https://arxiv.org/abs/2603.29086)
*Lucas Riera Abbade,Anna Helena Reali Costa*

Main category: cs.LG

TL;DR: Paper introduces realistic trading environments with nonlinear transaction costs, showing cost models dramatically affect RL algorithm performance and trading behavior.


<details>
  <summary>Details</summary>
Motivation: Most existing RL trading environments use unrealistic fixed transaction costs, leading to agents learning behaviors that fail under real market conditions with nonlinear market impact.

Method: Created three Gymnasium-compatible trading environments (MACE stock trading, margin trading, portfolio optimization) with pluggable cost models based on Almgren-Chriss framework and square-root impact law. Evaluated five DRL algorithms (A2C, PPO, DDPG, SAC, TD3) on NASDAQ-100 with both fixed 10 bps baseline and AC model using Optuna hyperparameter tuning.

Result: Cost models significantly change algorithm performance rankings and trading behaviors: daily costs dropped from $200k to $8k, turnover fell from 19% to 1%, hyperparameter optimization reduced costs up to 82%, and algorithm-cost model interactions were environment-specific (e.g., DDPG's Sharpe improved from -2.1 to 0.3 while SAC's worsened).

Conclusion: Realistic transaction cost modeling is crucial for RL trading research, as it dramatically affects algorithm performance, trading behavior, and relative rankings. The open-source environments enable more realistic evaluation of trading algorithms.
...
### [141] [HCLSM: Hierarchical Causal Latent State Machines for Object-Centric World Modeling](https://arxiv.org/abs/2603.29090)
*Jaber Jaber,Osama Jaber*

Main category: cs.LG

TL;DR: HCLSM is a hierarchical causal latent state model that improves world modeling through object-centric decomposition, multi-scale temporal dynamics, and causal structure learning, achieving strong performance on robotic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Current world models have limitations: flat latent representations that entangle objects, ignore causal structure, and collapse temporal dynamics into a single scale. These limitations hinder accurate future state prediction from video data.

Method: Three interconnected principles: 1) Object-centric decomposition via slot attention with spatial broadcast decoding, 2) Hierarchical temporal dynamics through three-level engine (SSMs for continuous physics, sparse transformers for discrete events, compressed transformers for abstract goals), 3) Causal structure learning via graph neural network interaction patterns. Two-stage training protocol with spatial reconstruction before dynamics prediction.
...
Conclusion: HCLSM demonstrates that hierarchical, object-centric, and causally-aware world modeling can significantly improve state prediction accuracy while learning meaningful representations of objects and events in robotic manipulation tasks.
...
TL;DR: The paper proposes using Kronecker-factored approximate curvature (KFAC) to improve hypergradient computation in bilevel optimization, offering better performance-efficiency trade-off than existing methods like Conjugate Gradient or Neumann expansions.


<details>
  <summary>Details</summary>
Motivation: Bilevel optimization requires computing hypergradients via inverse Hessian-vector products, but current approximations (one-step gradient unrolling, identity/short Neumann expansions) discard curvature information, limiting scaling and performance.

Method: The approach builds on implicit function theorem-based algorithms and incorporates Kronecker-factored approximate curvature (KFAC) to provide curvature-aware hypergradients with modest memory and runtime overhead.

Result: The method consistently outperforms unrolling and shows better performance-efficiency trade-off than Conjugate Gradient or Neumann methods across diverse tasks including meta-learning and AI safety problems, scaling up to BERT-sized models.

Conclusion: Curvature information is valuable at scale for bilevel optimization, and KFAC can provide it efficiently, making the approach practical for large-scale machine learning problems.
...
TL;DR: Gated active learning framework combines curiosity-driven sampling with physics-informed quality control to exclude noisy data during autonomous materials experiments, improving Image-to-Spectrum and Spectrum-to-Image translations.


<details>
  <summary>Details</summary>
Motivation: Autonomous experimental systems in materials research are limited by low-quality, noisy data, especially in structure-property learning tasks like Im2Spec and Spec2Im translations. Standard active learning strategies often mistakenly prioritize poor-quality measurements by misinterpreting noise as uncertainty.

Method: A gated active learning framework that combines curiosity-driven sampling with a physics-informed quality control filter based on Simple Harmonic Oscillator model fits. The system automatically excludes low-fidelity data during acquisition using this quality assessment gate.

Result: The method outperforms random sampling, standard active learning, and multitask learning strategies on BEPS data from PbTiO3 thin films. It enhances both Im2Spec and Spec2Im by handling noise during training and acquisition, leading to more reliable forward and inverse predictions. Real-time deployment on BiFeO3 thin films demonstrated effectiveness in autonomous microscopy experiments.

Conclusion: The work supports a shift toward hybrid autonomy in self-driving labs, where physics-informed quality assessment and active decision-making work together for more reliable scientific discovery, addressing the critical issue of noise in autonomous materials research.
...
TL;DR: A granular-ball graph coarsening method that reduces GCN computational overhead for large-scale graphs through linear-time multi-granularity coarsening and random sampling.


<details>
  <summary>Details</summary>
Motivation: GCNs face high computational overhead on large-scale graphs, especially with many layers. Existing methods using sampling or coarsening either ignore multi-granularity information or have high time complexity.

Method: Proposes a granular-ball graph coarsening framework: 1) Uses multi-granularity granular-ball coarsening with linear time complexity to generate subgraphs, 2) Randomly samples these subgraphs to form minibatches for GCN training.

Result: Experimental results on multiple datasets show superior performance in node classification. The method adaptively reduces graph scale, enhancing training efficiency and scalability.

Conclusion: The proposed granular-ball coarsening method effectively addresses GCN's computational challenges on large graphs through efficient multi-granularity processing and linear-time complexity.
...
TL;DR: Dummy Classes-based defenses exploit limitations in current adversarial robustness evaluation methods by using a "dummy" class as a safety sink, leading to overestimated robustness scores. The paper proposes DAWA (Dummy-Aware Weighted Attack) to properly evaluate these defenses.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial robustness evaluation methods like AutoAttack are insufficient for assessing Dummy Classes-based defenses because they only focus on misleading the true class label, which aligns with the defense mechanism where adversarial examples are simply captured by the dummy class.

Method: Proposes Dummy-Aware Weighted Attack (DAWA), a novel evaluation method that simultaneously targets both the true label and dummy label with adaptive weighting during adversarial example synthesis to properly assess Dummy Classes-based defenses.

Result: DAWA effectively breaks the defense paradigm, reducing measured robustness of a leading Dummy Classes-based defense from 58.61% to 29.52% on CIFAR-10 under l_infty perturbation (epsilon=8/255).

Conclusion: The work provides a more reliable benchmark for evaluating Dummy Classes-based defenses and highlights the need for continuous evolution of robustness assessment methodologies as new defense paradigms emerge.
...
TL;DR: IMPACT is a novel framework for open-set time series anomaly detection that uses influence modeling to generate realistic unseen anomalies and decontaminate training data, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current open-set anomaly detection methods fail on time series data because they don't preserve sequential nature, generate unrealistic anomalies, and struggle when training data is contaminated with unlabeled anomalies.

Method: IMPACT learns an influence function to estimate individual training samples' impact, then uses influence scores to generate realistic unseen anomalies and repurposes high-influential samples as supervised anomalies for decontamination.

Result: Extensive experiments show IMPACT significantly outperforms state-of-the-art methods, demonstrating superior accuracy across varying OSAD settings and contamination rates.

Conclusion: IMPACT effectively addresses key challenges in open-set time series anomaly detection by leveraging influence modeling for realistic anomaly generation and data decontamination.
...
TL;DR: Bio-PINNs: A biomimetic physics-informed neural network framework that addresses over-smoothing in cell-induced phase transitions by encoding temporal causality into spatial causality via progressive distance gates and using deformation-uncertainty proxies for microstructure targeting.
...
Conclusion: Bio-PINNs provide a computationally efficient framework for physics-informed learning of cell-induced phase transitions, overcoming over-smoothing issues through biomimetic design principles and uncertainty-driven adaptive sampling with theoretical guarantees.
...
TL;DR: A multimodal deep learning framework for tropical cyclone forecasting that integrates atmosphere, ocean, and terrain data to improve accuracy for both normal and abnormally deflected TCs.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning TC forecasting methods have limitations: they only process single-type sequential trajectory data or homogeneous meteorological variables, and fail to accurately forecast abnormal deflected TCs. There's a need for comprehensive multimodal approaches that capture complex physical interactions.

Method: 1) Created AOT-TCs dataset - first multimodal, multi-source dataset integrating heterogeneous variables from atmosphere, ocean, and land. 2) Developed forecasting model with explicit atmosphere-ocean-terrain coupling architecture to capture cross-domain interactions for both normal and abnormal TCs.

Result: State-of-the-art performance on Northwest Pacific TC cases (2017-2024): significantly improves forecasting accuracy for normal TCs and breaks through technical bottleneck for forecasting abnormally deflected TCs.

Conclusion: The proposed multimodal approach with explicit physical domain coupling successfully addresses key limitations of existing deep learning methods, enabling more accurate TC forecasting for both normal and challenging abnormal cases through comprehensive data integration.
...
TL;DR: DerivOpt improves neural simulation fidelity under fixed storage budgets by optimizing which physical fields to carry and how to allocate storage across them, addressing the bottleneck of state representation rather than just architecture or training.


<details>
  <summary>Details</summary>
Motivation: Fine-scale-faithful neural simulation under fixed storage budgets is challenging because existing methods focus on architectures, training objectives, or rollout strategies, but fine detail can be lost when constructing the carried state in budgeted coarsen-quantize-decode pipelines.

Method: Derived-Field Optimization (DerivOpt) - a state-design framework that chooses which physical fields to carry and how to allocate storage budget across them under a calibrated channel model, based on the observation that primitive and derived fields undergo systematically different retained-band distortions under the same operator.

Result: Across PDEBench's time-dependent forward subset, DerivOpt improves pooled mean rollout nRMSE and delivers decisive advantage in fine-scale fidelity over strong baselines. Gains are visible at input time before rollout learning begins, indicating carried state is often the dominant bottleneck under tight storage budgets.

Conclusion: In budgeted neural simulation, carried-state design should be treated as a first-class design axis alongside architecture, loss, and rollout strategy, as state representation is often the dominant bottleneck under tight storage constraints.
...
<details>
  <summary>Details</summary>
Motivation: Classical ski rental algorithms focus on worst-case competitive ratios, while recent learning-augmented methods use point-estimate predictions. Neither fully exploits distributional predictions while maintaining robustness guarantees.

Method: 1) Deterministic setting: Formalize problem under perfect distributional prediction, derive optimal threshold-buy day algorithm, propose Clamp Policy for imperfect predictions with tunable robustness parameter. 2) Randomized setting: Characterize stopping distribution via Water-Filling Algorithm that optimizes expected cost while satisfying robustness constraints.

Result: For deterministic setting, identify conditions where expected competitive ratio matches classic optimal randomized bound. Clamp Policy maintains robustness with prediction errors and consistency with accurate predictions. Experiments across Gaussian, geometric, and bi-modal distributions show improved consistency over point-prediction baselines while maintaining comparable robustness.

Conclusion: The framework successfully integrates distributional advice of unknown quality into ski rental algorithms, providing systematic approach that exploits richer distributional information while maintaining rigorous robustness guarantees for both deterministic and randomized settings.
...
### [151] [mtslearn: Machine Learning in Python for Medical Time Series](https://arxiv.org/abs/2603.29432)
*Zhongheng Jiang,Yuechao Zhao,Donglin Xie,Chenxi Sun,Rongchen Lu,Silu Luo,Zisheng Liang,Shenda Hong*

Main category: cs.LG

TL;DR: mtslearn is an end-to-end toolkit for medical time-series data that provides unified data interface, complete ML pipeline, and modular design to bridge AI research and clinical practice.


<details>
  <summary>Details</summary>
Motivation: Real-world clinical time-series data is highly heterogeneous and inconsistently formatted, while existing ML tools have steep learning curves and fragmented workflows, creating a gap between cutting-edge AI and clinical application.

Method: Developed mtslearn toolkit with: 1) unified data interface that automates parsing/alignment of wide, long, and flat formats; 2) complete pipeline from data reading to visualization; 3) flexible interfaces for custom algorithms; 4) modular design simplifying complex tasks into few lines of code.

Result: mtslearn significantly reduces data cleaning overhead, lowers barrier to entry for clinicians with limited programming experience, and accelerates translation of advanced algorithms into clinical practice. The toolkit is publicly available on GitHub.

Conclusion: mtslearn bridges the gap between AI research and clinical application by providing an integrated, user-friendly toolkit for medical time-series analysis, empowering clinicians to focus on medical hypothesis exploration rather than technical implementation.
...
### [152] [Stochastic Dimension Implicit Functional Projections for Exact Integral Conservation in High-Dimensional PINNs](https://arxiv.org/abs/2603.29237)
*Zhangyong Liang*

Main category: cs.LG

TL;DR: SDIFP is a framework for enforcing exact conservation laws in neural PDE solvers using stochastic dimension implicit functional projection and doubly-stochastic unbiased gradient estimation.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for enforcing macroscopic conservation laws (mass, energy) in neural PDE solvers face computational challenges: discrete projections scale poorly in high dimensions, rely on deterministic quadrature that restricts mesh-free formulations like PINNs, have heavy memory overhead for high-order operators, and lack convergence guarantees for non-convex conservation manifolds.

Method: Proposes Stochastic Dimension Implicit Functional Projection (SDIFP) framework that applies global affine transformation to continuous network output instead of projecting discrete vectors. Uses detached Monte Carlo quadrature for closed-form solutions of integral constraints, bypassing spatial grid dependencies. Introduces doubly-stochastic unbiased gradient estimator (DS-UGE) that decouples spatial sampling from differential operator subsampling to reduce memory complexity.

Result: SDIFP mitigates sampling variance, preserves solution regularity, maintains O(1) inference efficiency, and reduces memory complexity from O(M × N_ℒ) to O(N × |ℐ|). Provides scalable, mesh-free approach for solving conservative high-dimensional PDEs.

Conclusion: SDIFP offers a practical solution for enforcing exact conservation laws in neural PDE solvers, addressing scalability issues of traditional methods while maintaining mesh-free advantages and computational efficiency for high-dimensional problems.
...
### [153] [Nonnegative Matrix Factorization in the Component-Wise L1 Norm for Sparse Data](https://arxiv.org/abs/2603.29715)
*Giovanni Seraghiti,Kévin Dubrulle,Arnaud Vandaele,Nicolas Gillis*

Main category: cs.LG

TL;DR: L1-NMF is NP-hard even for rank 1, enforces sparsity for interpretability, but may degrade with false zeros. New weighted L1-NMF model controls sparsity, with efficient sparse coordinate descent algorithm scaling with nonzero entries.


<details>
  <summary>Details</summary>
Motivation: Standard NMF using least squares is sensitive to heavy-tailed noise and outliers. L1-NMF is more robust but NP-hard and may produce overly sparse solutions when data contains false zeros. Need robust NMF that handles sparse data efficiently.

Method: 1) Prove NP-hardness of L1-NMF even for rank 1. 2) Introduce weighted L1-NMF (wL1-NMF) with penalization parameter to control sparsity for entries associated with zeros in data. 3) Develop sparse coordinate descent (sCD) algorithm using weighted median to solve subproblems, scaling with number of nonzero entries.

Result: L1-NMF is NP-hard (unlike standard NMF). wL1-NMF effectively controls sparsity in factorization. sCD algorithm is efficient for large-scale sparse data as complexity scales with nonzero entries. Extensive experiments on synthetic and real-world data validate effectiveness.

Conclusion: Proposed wL1-NMF model provides robust factorization for sparse data with controlled sparsity, and sCD algorithm enables efficient large-scale computation. The framework addresses limitations of standard L1-NMF while maintaining robustness to heavy-tailed noise.
...
### [154] [Monodense Deep Neural Model for Determining Item Price Elasticity](https://arxiv.org/abs/2603.29261)
*Lakshya Garg,Sai Yaswanth,Deep Narayan Mishra,Karthik Kumaran,Anupriya Sharma,Mayank Uniyal*

Main category: cs.LG

TL;DR: Proposes a novel machine learning framework for estimating item-level price elasticity from transactional data without requiring treatment-control settings, with a new Monodense-DL neural network outperforming other ML methods.


<details>
  <summary>Details</summary>
Motivation: Businesses need accurate price elasticity estimates to optimize pricing strategies and revenue management, but traditional methods often require controlled experiments. There's a need for frameworks that can work with observational transactional data without treatment-control setups.
...
Result: Experimental evaluation on multi-category retail data with millions of transactions shows the proposed Monodense-DL neural network outperforms other ML methods (DML and LGBM) in the back-testing framework.

Conclusion: The novel elasticity estimation framework with Monodense-DL neural network provides superior price elasticity estimation from large-scale transactional data without requiring controlled experiments, offering businesses better tools for pricing optimization.
...
### [155] [Lie Generator Networks for Nonlinear Partial Differential Equations](https://arxiv.org/abs/2603.29264)
*Shafayeth Jamil,Rehan Kapadia*

Main category: cs.LG

TL;DR: LGN-KM is a neural operator that learns Koopman generators for nonlinear PDEs through a stable decomposition into conservative and dissipative parts, enabling interpretable spectral analysis and guaranteed stability.


<details>
  <summary>Details</summary>
Motivation: For nonlinear systems governed by PDEs, there's no equivalent to linear dynamical systems' eigenspectrum theory. The paper aims to develop a method that can lift nonlinear dynamics into a linear latent space while maintaining interpretability and stability guarantees.

Method: Introduces Lie Generator Network--Koopman (LGN-KM), a neural operator that learns continuous-time Koopman generator L_k through decomposition L_k = S - D_k, where S is skew-symmetric (conservative coupling) and D_k is positive-definite diagonal (modal dissipation). This architectural decomposition enforces stability and interpretability.

Result: On 2D Navier-Stokes turbulence, the generator recovers known dissipation scaling and complete multi-branch dispersion relation from trajectory data alone. Models trained at different flow regimes recover matched gauge-invariant spectral structure, revealing gauge freedom in Koopman lifting. The provably stable generator enables long-horizon stability, continuous-time evaluation, and physics-informed cross-viscosity model transfer.

Conclusion: LGN-KM provides a principled approach to learning interpretable Koopman generators for nonlinear PDEs with guaranteed stability, enabling spectral analysis, long-term prediction, and transfer learning across physical regimes.
...
### [156] [From Physics to Surrogate Intelligence: A Unified Electro-Thermo-Optimization Framework for TSV Networks](https://arxiv.org/abs/2603.29268)
*Mohamed Gharib,Leonid Popryho,Inna Partin-Vaisband*

Main category: cs.LG

TL;DR: A scalable electro-thermal modeling framework using physics-informed GNN surrogates enables rapid optimization of TSV arrays for 2.5D/3D integration, reducing simulation time by 6 orders of magnitude.


<details>
  <summary>Details</summary>
Motivation: High-density TSVs in 2.5D/3D integration face signal-integrity and thermal-reliability challenges, but conventional FEM simulations are too computationally expensive for large design-space exploration.

Method: Combines physics-informed analytical modeling, graph neural network (GNN) surrogates trained on analytical data and fine-tuned with HFSS simulations, and multi-objective Pareto optimization for TSV array design.

Result: Achieves 5-10% RFE with analytical model, GNN surrogate generalizes to larger arrays with RFE below 2%, enables exploration of millions of TSV configurations in minutes, validated with HFSS/Mechanical showing strong agreement.

Conclusion: The framework enables rapid electro-thermal co-design of TSV arrays, reducing per-design evaluation time by more than six orders of magnitude compared to conventional FEM approaches.
...
### [157] [LGFNet: Local-Global Fusion Network with Fidelity Gap Delta Learning for Multi-Source Aerodynamics](https://arxiv.org/abs/2603.29303)
*Qinye Zhu,Yu Xiang,Jun Zhang,Wenyong Wang*

Main category: cs.LG

TL;DR: LGFNet is a neural network that fuses multi-source aerodynamic data (CFD, wind tunnel, flight tests) using local-global feature decomposition to capture both fine-scale flow structures and global aerodynamic trends, with a fidelity gap delta learning strategy to handle simulation discrepancies.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to balance high-resolution local fidelity (like shock waves) with wide-range global dependencies in aerodynamic data fusion, often losing sharp discontinuities or failing to capture long-range topological correlations across the flight envelope.

Method: LGFNet combines spatial perception layer with sliding window mechanism for local features and relational reasoning layer based on self-attention for global dependencies. Uses fidelity gap delta learning (FGDL) strategy to treat CFD data as "low-frequency carrier" to approximate nonlinear discrepancies while preserving physical trends.

Result: LGFNet achieves state-of-the-art performance in both accuracy and uncertainty reduction across diverse aerodynamic scenarios, demonstrating effective fusion of multi-source aerodynamic data.

Conclusion: The proposed LGFNet with FGDL strategy successfully addresses the local-global trade-off in aerodynamic data fusion, enabling comprehensive understanding of flow structures while maintaining physical consistency across the flight envelope.
...
### [158] [Deep Learning-Based Anomaly Detection in Spacecraft Telemetry on Edge Devices](https://arxiv.org/abs/2603.29375)
*Christopher Goetze,Tim Schlippe,Daniel Lakey*

Main category: cs.LG

TL;DR: Spacecraft anomaly detection optimized for edge deployment using neural architecture optimization, achieving 88.8% detection accuracy with 97.1% RAM reduction to just 59KB.


<details>
  <summary>Details</summary>
Motivation: Spacecraft anomaly detection is critical for mission safety, but deploying sophisticated models on-board is challenging due to hardware constraints on spacecraft edge devices.

Method: Investigated three anomaly detection approaches (forecasting & threshold, direct classification, image classification) and optimized them using multi-objective neural architecture optimization on the European Space Agency Anomaly Dataset.

Result: Forecasting & threshold achieved best performance (92.7% CEF0.5). Optimized model preserved 88.8% CEF0.5 while reducing RAM by 97.1% to 59KB and operations by 99.4%. Optimized models require only 0.36-6.25% of CubeSat RAM.

Conclusion: Sophisticated anomaly detection can be successfully deployed within spacecraft edge computing constraints, providing near-instantaneous detection without exceeding hardware limitations or compromising mission safety.
...
### [159] [Finite-time analysis of Multi-timescale Stochastic Optimization Algorithms](https://arxiv.org/abs/2603.29380)
*Kaustubh Kartikey,Shalabh Bhatnagar*

Main category: cs.LG

TL;DR: Finite-time analysis of two smoothed functional stochastic approximation algorithms for simulation-based optimization: a two-time-scale gradient method and a three-time-scale Newton algorithm with zeroth-order gradient/Hessian estimates.


<details>
  <summary>Details</summary>
Motivation: Prior work established asymptotic convergence of these algorithms, but finite-time guarantees for two-timescale stochastic optimization algorithms in zeroth-order settings were missing. The paper aims to provide rigorous finite-time convergence bounds and characterize error propagation across multiple time-scales.

Method: Two algorithms: 1) Two time-scale gradient-based method using zeroth-order gradient estimates; 2) Three time-scale Newton-based algorithm estimating both gradient and Hessian via zeroth-order methods. Analysis includes mean-squared error bounds for Hessian estimator and finite-time convergence analysis.

Result: Derived mean-squared error bounds for Hessian estimator and established finite-time bound on min expected gradient norm squared, showing convergence to first-order stationary points. Identified optimal step-size choices balancing error terms and achieving near-optimal convergence rates. Experimental validation on Continuous Mountain Car environment.

Conclusion: Provides first finite-time guarantees for two-timescale stochastic optimization algorithms in zeroth-order settings. Explicitly characterizes time-scale interactions and error propagation, with practical step-size guidelines for near-optimal convergence. Both gradient and Newton algorithms analyzed under unified framework.
...
### [160] [Causality-inspired Federated Learning for Dynamic Spatio-Temporal Graphs](https://arxiv.org/abs/2603.29384)
*Yuxuan Liu,Wenchao Xu,Haozhao Wang,Zhiming He,Zhaofeng Shi,Chongyang Xu,Peichao Wang,Boyuan Zhang*

Main category: cs.LG

TL;DR: SC-FSGL is a causality-inspired federated learning framework for dynamic spatio-temporal graphs that decouples transferable causal knowledge from client-specific noise to address heterogeneity and negative transfer issues.


<details>
  <summary>Details</summary>
Motivation: Existing federated graph learning methods assume all features are equally transferable across clients, overlooking spatial/temporal heterogeneity and client-specific knowledge. This creates spurious representation entanglement, client-specific interference, and negative transfer, degrading performance on dynamic spatio-temporal graphs.

Method: Proposes SC-FSGL with two key components: 1) Conditional Separation Module using client-conditioned masks for soft interventions to disentangle invariant causal factors from spurious signals, and 2) Causal Codebook that clusters causal prototypes and aligns local representations via contrastive learning for cross-client consistency.
...
Conclusion: The causality-inspired framework effectively addresses heterogeneity and negative transfer in federated learning over dynamic spatio-temporal graphs by explicitly decoupling transferable causal knowledge from client-specific noise.
...
### [161] [PRISM: PRIor from corpus Statistics for topic Modeling](https://arxiv.org/abs/2603.29406)
*Tal Ishon,Yoav Goldberg,Uri Shaham*

Main category: cs.LG

TL;DR: PRISM is a corpus-intrinsic method that initializes LDA topic modeling using word co-occurrence statistics, improving topic coherence without external knowledge.


<details>
  <summary>Details</summary>
Motivation: Current topic modeling methods often rely on external knowledge like pre-trained embeddings, which limits their applicability in emerging or underexplored domains where such resources are unavailable.

Method: PRISM derives Dirichlet parameters from word co-occurrence statistics within the corpus itself to initialize LDA, without altering LDA's generative process. This corpus-intrinsic approach eliminates the need for external knowledge.

Result: Experiments on text and single cell RNA-seq data show that PRISM improves topic coherence and interpretability, rivaling models that rely on external knowledge resources.

Conclusion: PRISM demonstrates the value of corpus-driven initialization for topic modeling in resource-constrained settings, providing a viable alternative to methods requiring external knowledge.
...
### [162] [Hybrid Quantum-Classical Spatiotemporal Forecasting for 3D Cloud Fields](https://arxiv.org/abs/2603.29407)
*Fu Wang,Qifeng Lu,Xinyu Long,Meng Zhang,Xiaofei Yang,Weijia Cao,Xiaowen Chu*

Main category: cs.LG

TL;DR: QENO: A hybrid quantum-inspired spatiotemporal forecasting framework for 3D cloud fields that outperforms existing methods by modeling nonlocal couplings through topology-aware quantum enhancement.


<details>
  <summary>Details</summary>
Motivation: Accurate 3D cloud forecasting is challenging due to cross-layer interactions, nonlocal dependencies, and multiscale dynamics. Existing models with locality-biased representations struggle to preserve fine cloud structures in volumetric forecasting tasks.

Method: QENO consists of four components: 1) classical spatiotemporal encoder for compact latent representation, 2) topology-aware quantum enhancement block for modeling nonlocal couplings in latent space, 3) dynamic fusion temporal unit for integrating quantum features with recurrent memory, and 4) decoder for reconstructing future cloud volumes.

Result: QENO consistently outperforms ConvLSTM, PredRNN++, Earthformer, TAU, and SimVP variants on CMA-MESO 3D cloud fields, achieving MSE of 0.2038, RMSE of 0.4514, and SSIM of 0.6291 while maintaining compact parameter budget.

Conclusion: Topology-aware hybrid quantum-classical feature modeling is a promising direction for 3D cloud structure forecasting and atmospheric Earth observation data analysis.
...
### [163] [An Isotropic Approach to Efficient Uncertainty Quantification with Gradient Norms](https://arxiv.org/abs/2603.29466)
*Nils Grünefeld,Jes Frellsen,Christian Hardmeier*

Main category: cs.LG

TL;DR: Lightweight method for predictive uncertainty in LLMs using gradient norms and Bernoulli variance from single forward-backward pass, without training data or model modifications.


<details>
  <summary>Details</summary>
Motivation: Existing uncertainty quantification methods are computationally expensive for large language models or require training data that's often unavailable, creating a need for lightweight alternatives.

Method: Two approximations: 1) First-order Taylor expansion linking uncertainty to prediction gradient and parameter covariance, 2) Isotropy assumption on parameter covariance. This yields epistemic uncertainty as squared gradient norm and aleatoric uncertainty as Bernoulli variance from single forward-backward pass.

Result: Method shows strong correspondence with reference MCMC estimates, improving with model size. On QA benchmarks, combined uncertainty achieves high AUROC on TruthfulQA (conflict scenarios) but near chance on TriviaQA (factual recall), revealing different uncertainty signals than self-assessment methods.

Conclusion: Proposed lightweight method provides practical uncertainty quantification for LLMs without training data, revealing that parameter-level uncertainty captures fundamentally different signals than self-assessment, with benchmark-dependent utility.
...
### [164] [Survival In-Context: Prior-fitted In-context Learning Tabular Foundation Model for Survival Analysis](https://arxiv.org/abs/2603.29475)
*Dmitrii Seletkov,Paul Hager,Rickmer Braren,Daniel Rueckert,Raphael Rehms*

Main category: cs.LG

TL;DR: SIC is a prior-fitted in-context learning model for survival analysis that's pretrained on synthetic data and achieves competitive performance on real-world datasets without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Survival analysis faces challenges with limited data, censoring, and heterogeneous tabular covariates. While prior-fitted models work for classification/regression, their suitability for time-to-event modeling is unclear.

Method: Proposes a flexible survival data generation framework with explicit control over covariates and time-event distributions. Uses this prior to train SIC (Survival In-Context), a prior-fitted in-context learning model pretrained exclusively on synthetic data.

Result: SIC achieves competitive or superior performance compared to classical and deep survival models across real-world datasets, particularly in medium-sized data regimes. It requires no task-specific training or hyperparameter tuning.

Conclusion: The work highlights the promise of prior-fitted foundation models for survival analysis, demonstrating that models pretrained on synthetic data can effectively handle real-world survival prediction tasks.
...
### [165] [Why not to use Cosine Similarity between Label Representations](https://arxiv.org/abs/2603.29488)
*Beatrix M. G. Nielsen*

Main category: cs.LG

TL;DR: Cosine similarity between label representations in softmax classifiers doesn't predict model probabilities - can be manipulated to ±1 while keeping probabilities identical.


<details>
  <summary>Details</summary>
Motivation: Researchers often use cosine similarity between model representations (like unembeddings) to analyze neural network behavior, assuming it reflects model probabilities. This paper challenges that assumption for softmax classifiers.

Method: Theoretical proof showing for any softmax classifier, given two label representations, one can create another model with identical probabilities but cosine similarity forced to ±1. Provides specific examples and demonstrates translation ambiguity that can be fixed by centering, but still doesn't guarantee probability correlation.

Result: Cosine similarity between label representations is not informative about model probabilities in softmax classifiers. Models with identical probability distributions can have completely different cosine similarities (±1). Even after fixing representation length, no guarantee exists that cosine similarity correlates with probability assignments.

Conclusion: Cosine similarity values between label representations should not be used to explain model probabilities in softmax classifiers, as they provide no reliable information about model behavior despite being commonly used for analysis.
...
### [166] [Target-Aligned Reinforcement Learning](https://arxiv.org/abs/2603.29501)
*Leonard S. Pleiss,James Harrison,Maximilian Schiffer*

Main category: cs.LG

TL;DR: TARL improves RL by focusing updates on transitions where target and online networks are well-aligned, balancing stability and learning speed.


<details>
  <summary>Details</summary>
Motivation: Target networks in RL create a stability-recency tradeoff: slower updates improve stability but reduce learning signal recency, slowing convergence.

Method: Proposes Target-Aligned Reinforcement Learning (TARL) framework that emphasizes transitions where target and online network estimates are highly aligned.

Result: Theoretical analysis shows target alignment correction accelerates convergence; empirical results demonstrate consistent improvements across benchmark environments.

Conclusion: TARL effectively mitigates adverse effects of stale target estimates while retaining stabilization benefits, improving RL algorithm performance.
...
### [167] [Variational Graph Neural Networks for Uncertainty Quantification in Inverse Problems](https://arxiv.org/abs/2603.29515)
*David Gonzalez,Alba Muixi,Beatriz Moya,Elias Cueto*

Main category: cs.LG

TL;DR: VGNN architecture integrates variational layers in decoder to estimate uncertainty in solid mechanics inverse problems at lower computational cost than full Bayesian networks.


<details>
  <summary>Details</summary>
Motivation: Deep learning in computational mechanics needs reliable uncertainty quantification for critical applications like Digital Twins, especially for inverse problems with non-unique solutions or noisy data where traditional deterministic methods and classic neural networks lack confidence measures.

Method: Variational graph neural network (VGNN) with variational layers strategically placed only in the decoder to model weight probability distributions, enabling estimation of cognitive and statistical uncertainty at lower computational cost than full Bayesian networks.

Result: Successfully validated on two solid mechanics problems: 1) identifying elastic modulus with nonlinear distribution in 2D elastic problem, and 2) locating and quantifying loads applied to 3D hyperelastic beam. Model recovered physical parameters with high precision and provided confidence intervals consistent with problem physics.

Conclusion: VGNN architecture provides effective uncertainty quantification for inverse problems in computational mechanics while maintaining computational efficiency, making it suitable for critical applications requiring both fast responses and reliable confidence measures.
...
### [168] [Capturing Multivariate Dependencies of EV Charging Events: From Parametric Copulas to Neural Density Estimation](https://arxiv.org/abs/2603.29554)
...
<details>
  <summary>Details</summary>
Motivation: Traditional statistical methods for EV charging modeling fail to capture complex non-linear dependencies between key variables (arrival times, durations, energy demand), which is essential for grid reliability and smart-charging design.
...
Result: Vine copulas and CODINE outperform established parametric families and remain highly competitive against state-of-the-art benchmarks like conditional Gaussian Mixture Model Networks, with superior preservation of tail behaviors and correlation structures.

Conclusion: These methods provide a robust framework for synthetic charging event generation in varied infrastructure contexts by explicitly focusing on modeling the joint dependence structure between EV charging variables.
...
### [169] [Total Variation Guarantees for Sampling with Stochastic Localization](https://arxiv.org/abs/2603.29555)
*Jakob Kellermann*

Main category: cs.LG

TL;DR: First convergence analysis for SLIPS diffusion sampling algorithm showing linear dimension scaling for ε-accuracy.


<details>
  <summary>Details</summary>
Motivation: SLIPS (based on Stochastic Localization) shows strong empirical performance but lacks rigorous convergence analysis. This work aims to provide the first theoretical guarantees for SLIPS.

Method: Establishes convergence bound in total variation distance using techniques from score-based generative models theory. Analyzes discretization points theoretically.

Result: Proves SLIPS requires number of steps scaling linearly with dimension (up to logarithmic factors) to achieve ε-accuracy under minimal target assumptions.

Conclusion: First rigorous convergence guarantee for SLIPS algorithm, providing theoretical foundation for its empirical success and insights into optimal discretization choices.
...
### [170] [The Geometry of Polynomial Group Convolutional Neural Networks](https://arxiv.org/abs/2603.29566)
*Yacoub Hendi,Daniel Persson,Magdalena Larfors*

Main category: cs.LG

TL;DR: PGCNNs for arbitrary finite groups studied using graded group algebras framework, yielding two parametrizations (Hadamard/Kronecker) with dimension analysis and fiber descriptions.


<details>
  <summary>Details</summary>
Motivation: To develop a rigorous mathematical framework for polynomial group convolutional neural networks (PGCNNs) applicable to arbitrary finite groups, providing theoretical understanding of their architecture and parameter spaces.

Method: Introduce new mathematical framework using graded group algebras language, yielding two natural parametrizations (Hadamard and Kronecker products) related by linear map. Analyze neuromanifold dimension and describe general fiber structure of parametrizations.

Result: Computed dimension of neuromanifold showing it depends only on number of layers and group size. Described general fiber of Kronecker parametrization up to regular group action and rescaling. Conjectured analogous description for Hadamard parametrization, supported by explicit computations for small groups and shallow networks.

Conclusion: The graded group algebras framework provides powerful theoretical tools for analyzing PGCNNs, revealing structural properties of parameter spaces and establishing foundations for further theoretical development of group-equivariant neural networks.
...
TL;DR: DGP (Disentangled Graph Prompting) is a novel graph OOD detection method that uses pre-trained GNN encoders with prompt generators to capture fine-grained ID patterns without OOD supervision during training.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks face safety risks when test data differs from training data distributions. Existing graph OOD detection methods lack explicit supervision signals due to unavailability of OOD data during training, leading to sub-optimal performance of end-to-end encoders.

Method: Follows pre-training+prompting paradigm with pre-trained GNN encoders. Designs two prompt generators: class-specific and class-agnostic prompt graphs by modifying edge weights of input graphs. Uses several effective losses to train prompt generators and prevent trivial solutions.

Result: Extensive experiments on ten datasets show DGP achieves 3.63% relative AUC improvement over best graph OOD detection baseline. Ablation studies and hyper-parameter experiments demonstrate effectiveness.

Conclusion: DGP effectively addresses the OOD detection problem in graphs by leveraging pre-trained encoders with prompting mechanisms, outperforming existing methods without requiring OOD supervision during training.
...
TL;DR: The paper introduces "concept frustration" - a geometric framework to detect contradictions when unobserved concepts create inconsistent relationships between known concepts, and shows how this can be detected in foundation model representations to better align human and machine reasoning.


<details>
  <summary>Details</summary>
Motivation: The central challenge is aligning human-interpretable concepts with internal representations learned by modern ML systems. The paper aims to address incomplete concept ontologies and improve interpretable AI for high-risk applications by understanding how unobserved concepts affect relationships between known concepts.

Method: Develops a geometric framework with task-aligned similarity measures to detect concept frustration between supervised concept-based models and unsupervised representations from foundation models. Uses a linear-Gaussian generative model to derive closed-form expressions for Bayes-optimal classifier accuracy, decomposing predictive signal into known-known, known-unknown, and unknown-unknown contributions.

Result: Shows that concept frustration is detectable in foundation model representations using task-aligned geometry (while conventional Euclidean comparisons fail). Demonstrates on synthetic data and real language/vision tasks that incorporating frustrating concepts reorganizes geometry of learned concept representations to better align human and machine reasoning.

Conclusion: Provides a principled framework for diagnosing incomplete concept ontologies and aligning human/machine conceptual reasoning, with implications for developing and validating safe interpretable AI for high-risk applications.
...
TL;DR: The paper introduces a Partial Information Decomposition (PID) framework to analyze how Large Vision-Language Models (LVLMs) make decisions, distinguishing between redundant, unique, and synergistic information components across visual and language modalities.


<details>
  <summary>Details</summary>
Motivation: LVLMs show impressive performance but their internal decision-making processes are opaque. It's unclear whether their success comes from true multimodal fusion or reliance on unimodal priors, creating an "attribution gap" in understanding how they process information.

Method: The authors adapt Partial Information Decomposition (PID) with a scalable estimator to quantitatively measure LVLMs' "information spectrum." They analyze 26 LVLMs on four datasets across three dimensions: breadth (cross-model & cross-task), depth (layer-wise information dynamics), and time (learning dynamics across training).

Result: The analysis reveals: (1) two task regimes (synergy-driven vs. knowledge-driven), (2) two stable family-level strategies (fusion-centric vs. language-centric), (3) a consistent three-phase pattern in layer-wise processing, and (4) visual instruction tuning as the key stage where fusion is learned.

Conclusion: The PID framework provides a quantitative lens beyond accuracy-only evaluation, offering insights for analyzing and designing next-generation LVLMs by revealing how different models balance multimodal fusion versus unimodal priors in their decision-making processes.
...
TL;DR: Current multimodal active learning methods fail to address modality imbalance, with multimodal strategies not consistently outperforming unimodal ones, highlighting the need for modality-aware approaches.


<details>
  <summary>Details</summary>
Motivation: Multimodal active learning faces unique challenges not present in unimodal settings: missing modalities, differences in modality difficulty, and varying interaction structures. While unimodal active learning is well understood, how these strategies behave in multimodal conditions remains poorly characterized.

Method: Introduces a new benchmarking framework for multimodal active learning using synthetic datasets to isolate specific pitfalls without confounding noise. Compares unimodal and multimodal query strategies and validates findings on two real-world datasets.
...
Conclusion: Current active learning methods have significant limitations in multimodal settings, underlining the need for modality-aware query strategies that explicitly address the unique challenges of multimodal learning.
...
### [175] [One-for-All: A Lightweight Stabilized and Parameter-Efficient Pre-trained LLM for Time Series Forecasting](https://arxiv.org/abs/2603.29756)
*Prasanjit Dey,Soumyabrata Dev,Bianca Schoen-Phelan*

Main category: cs.LG

TL;DR: One-for-All introduces Gaussian Rank-Stabilized Low-Rank Adapters (rsLoRA) for parameter-efficient fine-tuning of frozen LLMs for time-series analysis, achieving state-of-the-art efficiency with minimal memory footprint while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Pre-trained LLMs face prohibitive computational and memory demands when adapted for multivariate time-series analysis, limiting their deployment in practical applications like healthcare, finance, and environmental monitoring.

Method: Introduces rsLoRA with mathematically grounded rank-stabilization mechanism for provable gradient stability at low ranks. Injects trainable rank decomposition matrices (rank 16) into positional embeddings and output layers while keeping self-attention weights fixed.

Result: Achieves 5.5× higher parameter efficiency than TimesNet and 21× better than GPT4TS while matching forecasting accuracy (MSE=0.33). Reduces trainable parameters by 6.8-21× and memory footprint by 168-1,776× (2.2MiB vs. 340MiB-4.18GiB).

Conclusion: One-for-All enables efficient deployment of LLMs for time-series analysis on edge devices without compromising performance, validated across diverse datasets and horizons with 98.3% fewer parameters than conventional transformers.
...
### [176] [Training-Free Dynamic Upcycling of Expert Language Models](https://arxiv.org/abs/2603.29765)
*Eros Fanì,Oğuzhan Ersoy*

Main category: cs.LG

TL;DR: DUME is a novel method that reuses existing dense expert models to create a unified Mixture of Experts without additional training, achieving strong performance while being cost-efficient and scalable.


<details>
  <summary>Details</summary>
Motivation: LLMs are expensive to train and lack domain expertise, while existing finetuning approaches cause overspecialization or multitask interference. Current MoE methods still require multitask finetuning.

Method: Dynamic Upcycling MoE (DUME) reuses pre-trained dense experts from different domains to construct a unified MoE model using ridge regression's closed-form solution, eliminating further optimization and enabling dynamic expert addition.

Result: DUME outperforms baselines in causal language modeling and reasoning, retaining up to 97.6% of specialized dense expert performance in one domain and achieving 102.1% in reasoning settings.

Conclusion: DUME provides a cost-efficient, scalable solution for creating multi-domain expert models without retraining, preserving original expert capabilities while enabling further fine-tuning for improved performance.
...
### [177] [Big2Small: A Unifying Neural Network Framework for Model Compression](https://arxiv.org/abs/2603.29768)
*Jing-Xiao Liao,Haoran Wang,Tao Li,Daoming Lyu,Yi Zhang,Chengjun Cai,Feng-Lei Fan*

Main category: cs.LG

TL;DR: Big2Small: A unifying mathematical framework for model compression using measure theory, translating Implicit Neural Representations from data domain to network parameters for data-free compression.


<details>
  <summary>Details</summary>
Motivation: Current model compression approaches are fragmented and heuristic-based; need a principled mathematical framework to unify the field and enable systematic compression methods.

Method: 1) Construct measure theory-based mathematical framework showing compression techniques are equivalent to neural networks with regularization; 2) Develop Big2Small framework using Implicit Neural Representations to encode larger model weights into compact INRs; 3) Introduce Outlier-Aware Preprocessing and Frequency-Aware Loss for better reconstruction fidelity.

Result: Big2Small achieves competitive accuracy and compression ratios on image classification and segmentation tasks compared to state-of-the-art baselines.

Conclusion: Provides a principled mathematical foundation for model compression, demonstrates practical effectiveness through Big2Small framework, and shows potential for unifying various compression techniques under a single theoretical framework.
...
TL;DR: A multimodal ML framework predicts metastasis risk 1 month before diagnosis using 6 months of EHR data from 4 cancer types, with intermediate fusion achieving best performance and SHAP analysis revealing modality importance varies by cancer.


<details>
  <summary>Details</summary>
Motivation: Multimodal ML can provide comprehensive patient insights by integrating structured and unstructured EHR data to predict metastasis risk earlier, potentially improving cancer outcomes through timely intervention.

Method: Used 6 months of clinical history from EHRs of 4 cancer cohorts (breast, colon, lung, prostate). Compared traditional vs deep learning classifiers across single modalities and multimodal combinations with various fusion strategies. Employed TRIPOD 2a design with 80-20 split and multimodal SHAP for interpretability analysis.

Result: Intermediate fusion achieved highest F1 scores: breast (0.845), colon (0.786), prostate (0.845). For lung cancer, text-only model performed best (0.829). Deep learning consistently outperformed traditional models. Colon cancer (smallest cohort) had lowest performance. SHAP showed modality importance varies across cancer types.

Conclusion: Intermediate fusion generally delivers best predictive performance, but fusion strategy selection should consider data characteristics and organizational needs. Deep learning outperforms traditional methods, and sufficient training data is crucial for model performance.
...
### [179] [From Density Matrices to Phase Transitions in Deep Learning: Spectral Early Warnings and Interpretability](https://arxiv.org/abs/2603.29805)
*Max Hennick,Guillaume Corlouer*

Main category: cs.LG

TL;DR: The paper introduces the "2-datapoint reduced density matrix" (2RDM) as a computationally efficient observable for detecting and analyzing phase transitions during AI model training, providing early warning signals and interpretable insights into emergent capabilities.


<details>
  <summary>Details</summary>
Motivation: There's a need to better predict and understand emergent capabilities in AI models during training, similar to how quantum chemistry studies reactions. Current methods lack efficient, unified observables for detecting phase transitions in model training dynamics.

Method: Proposes the 2-datapoint reduced density matrix (2RDM) as an observable. Tracks eigenvalue statistics of 2RDM over sliding windows to derive two signals: spectral heat capacity (detects second-order phase transitions via critical slowing down) and participation ratio (reveals dimensionality of underlying reorganization). Top eigenvectors of 2RDM provide interpretability.

Result: Validated across four distinct settings: deep linear networks, induction head formation, grokking, and emergent misalignment. The 2RDM successfully detects phase transitions, provides early warning signals, and offers interpretable insights into the nature of transitions.

Conclusion: The 2RDM provides a computationally efficient, unified framework for studying phase transitions during AI training, offering both detection capabilities and interpretability. Future work directions using the 2RDM are discussed.
...
### [180] [AMShortcut: An Inference- and Training-Efficient Inverse Design Model for Amorphous Materials](https://arxiv.org/abs/2603.29812)
*Yan Lin,Jonas A. Finkler,Tao Du,Jilin Hu,Morten M. Smedskjaer*

Main category: cs.LG

TL;DR: AMShortcut is an efficient probabilistic generative model for inverse design of amorphous materials that enables fast inference with few sampling steps and flexible property conditioning without retraining.


<details>
  <summary>Details</summary>
Motivation: Amorphous materials lack long-range order but have complex short/medium-range structures, requiring large simulation cells. Inverse design with generative models is promising for applications like energy storage and thermal management, but existing methods suffer from inefficient inference requiring many sampling steps and inflexible property conditioning requiring separate models for each property combination.
...
Conclusion: AMShortcut provides an efficient and flexible approach for inverse design of amorphous materials, addressing key limitations of existing methods by reducing sampling steps and enabling arbitrary property combinations without retraining, which facilitates practical applications in domains like energy storage and thermal management.
...
### [181] [Loss Gap Parity for Fairness in Heterogeneous Federated Learning](https://arxiv.org/abs/2603.29818)
*Brahim Erraji,Michaël Perrot,Aurélien Bellet*

Main category: cs.LG

TL;DR: EAGLE is a federated learning algorithm that ensures fairness by minimizing disparities in loss gaps across clients, prioritizing those furthest from their local optimal performance while maintaining competitive utility.


<details>
  <summary>Details</summary>
Motivation: Clients in federated learning are self-interested and expect the global model to perform well on their own data, but existing methods may degrade performance for many clients. The paper aims to ensure all clients achieve similar loss gaps (difference between global model performance and their best local model).

Method: EAGLE explicitly regularizes the global model to minimize disparities in loss gaps across clients. It targets fairness in relative improvements rather than loss parity, and prioritizes clients furthest from their local optimal loss.

Result: Theoretical convergence guarantees are provided under non-convex loss functions, and empirical results show EAGLE reduces disparity in loss gaps among clients while maintaining competitive utility in both convex and non-convex cases compared to strong baselines.

Conclusion: EAGLE effectively addresses fairness in federated learning by ensuring similar relative improvements across clients, particularly in heterogeneous settings where optimal local models may be misaligned, without sacrificing overall utility.
...
### [182] [Curvature-Guided LoRA: Steering in the pretrained NTK subspace](https://arxiv.org/abs/2603.29824)
*Frédéric Zheng,Alexandre Proutière*

Main category: cs.LG

TL;DR: CG-LoRA improves PEFT performance by aligning predictions with full fine-tuning using curvature-aware second-order optimization, avoiding explicit second-order matrix construction.


<details>
  <summary>Details</summary>
Motivation: Existing parameter-efficient fine-tuning methods like LoRA often underperform compared to full fine-tuning. Current approaches focus on aligning parameter updates rather than directly controlling model predictions, leading to suboptimal performance.

Method: Proposes Curvature-Guided LoRA (CG-LoRA) that formulates prediction alignment as a second-order optimization problem. The method selects and scales adaptation directions using local curvature information, corresponding to a Newton-like, curvature-whitened gradient approach without explicit second-order matrix construction.
...
Conclusion: Aligning predictions rather than parameters through curvature-aware optimization enables more effective parameter-efficient fine-tuning that approaches full fine-tuning performance while maintaining computational efficiency.
...
### [183] [DiSGMM: A Method for Time-varying Microscopic Weight Completion on Road Networks](https://arxiv.org/abs/2603.29837)
*Yan Lin,Jilin Hu,Shengnan Guo,Christian S. Jensen,Youfang Lin,Huaiyu Wan*

Main category: cs.LG

TL;DR: DiSGMM: A method for time-varying microscopic road-network weight completion using Gaussian mixture models to handle sparse traffic data and capture complex distributions.


<details>
  <summary>Details</summary>
Motivation: Microscopic road-network weights (like travel speeds from individual vehicles) are often sparse in both network coverage and segment-level data, making it challenging to estimate accurate weight distributions for traffic tasks like microsimulation and reliable routing.

Method: DiSGMM combines sparsity-aware embeddings with spatiotemporal modeling to leverage sparse known weights, learned segment properties, and long-range correlations. It represents microscopic weight distributions as learnable Gaussian mixture models for closed-form, flexible distribution estimation.

Result: Experiments on two real-world datasets show that DiSGMM outperforms state-of-the-art methods for time-varying microscopic weight completion.

Conclusion: DiSGMM effectively addresses the dual sparsity challenge in microscopic road-network weight completion by providing flexible, closed-form distribution representations that can capture complex traffic conditions including heavy tails and multiple clusters.
...
### [184] [Task Scarcity and Label Leakage in Relational Transfer Learning](https://arxiv.org/abs/2603.29914)
*Francisco Galuppo Azevedo,Clarissa Lima Loures,Denis Oliveira Correa*

Main category: cs.LG

TL;DR: The paper addresses label leakage in relational foundation models, where limited task supervision causes representations to encode task-specific shortcuts that degrade transfer learning. The authors propose K-Space architecture with gradient projection to suppress leakage, improving within-dataset transfer performance.


<details>
  <summary>Details</summary>
Motivation: Relational foundation models need to learn transferable representations across tasks, but available supervision is typically limited to few prediction targets per database. This task scarcity causes learned representations to encode task-specific shortcuts (label leakage), degrading transfer even within the same schema.

Method: Propose K-Space, a modular architecture combining frozen pretrained tabular encoders with a lightweight message-passing core. To suppress label leakage, introduce a gradient projection method that removes label-predictive directions from representation updates during training.

Result: On RelBench, the gradient projection method improves within-dataset transfer by +0.145 AUROC on average, often recovering near single-task performance. This demonstrates effectiveness in mitigating label leakage.

Conclusion: Limited task diversity, not just limited data, constrains relational foundation models. The proposed gradient projection method effectively suppresses label leakage and improves transfer learning performance within the same schema.
...
### [185] [Real-Time Explanations for Tabular Foundation Models](https://arxiv.org/abs/2603.29946)
*Luan Borges Teodoro Reis Sena,Francisco Galuppo Azevedo*

Main category: cs.LG

TL;DR: ShapPFN is a tabular foundation model that integrates Shapley value regression into its architecture, enabling simultaneous predictions and explanations in a single forward pass with 1000× speedup over KernelSHAP.


<details>
  <summary>Details</summary>
Motivation: Interpretability is crucial for scientific machine learning to understand why models make predictions for hypothesis generation and validation. Existing explanation methods like SHAP are computationally expensive, limiting interactive exploration despite strong performance of tabular foundation models.

Method: ShapPFN integrates Shapley value regression directly into its foundation model architecture, allowing it to produce both predictions and explanations in a single forward pass rather than requiring separate post-hoc analysis.

Result: ShapPFN achieves competitive performance on standard benchmarks while producing high-fidelity explanations (R²=0.96, cosine=0.99) over 1000× faster than KernelSHAP (0.06s vs 610s).

Conclusion: ShapPFN enables efficient, interactive exploration of model explanations by dramatically reducing computational costs while maintaining high explanation fidelity, advancing interpretable scientific machine learning.
...
### [186] [Meteorology-Driven GPT4AP: A Multi-Task Forecasting LLM for Atmospheric Air Pollution in Data-Scarce Settings](https://arxiv.org/abs/2603.29974)
*Prasanjit Dey,Soumyabrata Dev,Bianca Schoen-Phelan*

Main category: cs.LG

TL;DR: GPT4AP: A parameter-efficient GPT-2-based model with rsLoRA adaptation for air pollution forecasting, showing strong performance in few-shot, zero-shot, and long-term forecasting scenarios.


<details>
  <summary>Details</summary>
Motivation: Data-driven air pollution forecasting models often struggle with generalization in regions with sparse observations, creating a need for more robust and data-efficient approaches.

Method: Uses pre-trained GPT-2 backbone with Gaussian rank-stabilized low-rank adaptation (rsLoRA), freezing self-attention and feed-forward layers while adapting lightweight positional and output modules for parameter efficiency.

Result: Outperforms DLinear and ETSformer in few-shot (10% data) with MSE/MAE 0.686/0.442; achieves 0.529/0.403 MSE/MAE in zero-shot cross-station transfer; remains competitive in long-term forecasting with full data (MAE 0.429).
...
TL;DR: The paper finds that in multimodal cancer prognosis, better predictive performance comes from complementary signal aggregation rather than cross-modal synergy, with an inverse relationship between performance and measured interaction.


<details>
  <summary>Details</summary>
Motivation: To test the common assumption that multimodal deep learning for cancer prognosis benefits from synergistic cross-modal interactions, which hasn't been directly tested in survival prediction settings.

Method: Adapted InterSHAP (Shapley interaction index-based metric) from classification to Cox proportional hazards models to quantify cross-modal interactions in glioma survival prediction. Used TCGA-GBM and TCGA-LGG data (n=575) to evaluate four fusion architectures combining whole-slide image (WSI) and RNA-seq features.

Result: Found inverse relationship: architectures with superior discrimination (C-index 0.64→0.82) exhibited equivalent or lower cross-modal interaction (4.8%→3.0%). Variance decomposition showed stable additive contributions (WSI≈40%, RNA≈55%, Interaction≈4%), indicating performance gains come from complementary signal aggregation rather than learned synergy.

Conclusion: Provides a practical model auditing tool for comparing fusion strategies, reframes the role of architectural complexity in multimodal fusion, and has implications for privacy-preserving federated deployment by showing that synergy isn't necessary for good performance.
...
TL;DR: TWCV addresses CV bias in spatial prediction by weighting validation losses to match target deployment task distributions, handling covariate and task-difficulty shifts.


<details>
  <summary>Details</summary>
Motivation: Standard cross-validation assumes validation and deployment tasks come from the same distribution, but this assumption is often violated in spatial prediction and structured data settings, leading to biased risk estimates.

Method: Target-Weighted CV (TWCV) uses calibration weighting to assign weights to validation losses so the weighted empirical distribution of validation tasks matches the target deployment distribution. Combined with spatially buffered resampling to ensure adequate coverage of deployment distribution support.

Result: Simulation study shows conventional and spatial estimators exhibit substantial bias depending on sampling, while buffered TWCV remains approximately unbiased. Environmental pollution mapping case study confirms TWCV better reflects prediction tasks over target domain.
...
### [189] [Tracking Equivalent Mechanistic Interpretations Across Neural Networks](https://arxiv.org/abs/2603.30002)
*Alan Sun,Mariya Toneva*

Main category: cs.LG

TL;DR: The paper addresses scalability challenges in mechanistic interpretability by defining "interpretive equivalence" - determining if two models share a common interpretation without needing explicit interpretation descriptions.


<details>
  <summary>Details</summary>
Motivation: Mechanistic interpretability (MI) is difficult to scale and generalize due to: 1) lack of precise definition for valid interpretations, and 2) ad hoc interpretation generation processes. The paper aims to address these challenges.

Method: Proposes interpretive equivalence principle: two interpretations are equivalent if all their possible implementations are also equivalent. Develops algorithm to estimate interpretive equivalence and case studies it on Transformer models. Introduces necessary/sufficient conditions based on representation similarity.

Result: Provides theoretical guarantees relating models' algorithmic interpretations, circuits, and representations. Framework enables more rigorous MI evaluation and automated interpretation discovery methods.

Conclusion: The interpretive equivalence framework lays foundation for developing more rigorous evaluation methods in mechanistic interpretability and automated, generalizable interpretation discovery approaches.
...
### [190] [Refined Detection for Gumbel Watermarking](https://arxiv.org/abs/2603.30017)
*Tor Lattimore*

Main category: cs.LG

TL;DR: Proposes a simple, near-optimal detection mechanism for Gumbel watermarking that works without model access


<details>
  <summary>Details</summary>
Motivation: Existing watermarking schemes need improvement in detection efficiency and optimality; need model-agnostic approaches that work without access to the underlying model

Method: Develops a simple detection mechanism for Gumbel watermarking that is model-agnostic and assumes i.i.d. next-token sampling

Result: The proposed mechanism is proven to be near-optimal in a problem-dependent sense among all model-agnostic watermarking schemes

Conclusion: Provides an efficient, near-optimal detection solution for Gumbel watermarking that doesn't require model access, advancing practical watermarking applications
...
### [191] [Tucker Attention: A generalization of approximate attention mechanisms](https://arxiv.org/abs/2603.30033)
*Timon Klein,Jonas Kusch,Sebastian Sager,Stefan Schnake,Steffen Schotthöfer*

Main category: cs.LG

TL;DR: Tucker Attention is a new parameter-efficient factorization method for self-attention that generalizes existing approaches like GQA and MLA, requiring significantly fewer parameters while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reducing self-attention memory footprint (GQA, MLA) use unconventional low-rank factorizations that raise questions about what they actually approximate and how to interpret their low-rank behavior. The paper aims to provide a unified theoretical understanding and more efficient factorization strategy.

Method: Proposes Tucker Attention, a generalized view on weight objects in self-attention layers using Tucker decomposition factorization. This approach constructs a parameter-efficient scheme that encompasses MHA, GQA, and MLA as special cases while being compatible with flash-attention and RoPE.

Result: Tucker Attention requires an order of magnitude fewer parameters for comparable validation metrics compared to GQA and MLA, as evaluated in LLM and ViT test cases. The method also provides insights into actual ranks achieved by existing approaches and enables simplifications for MLA.

Conclusion: Tucker Attention offers a unified framework for understanding and improving parameter efficiency in self-attention mechanisms, providing both theoretical insights and practical performance improvements over existing methods.
...
### [192] [Reward-Based Online LLM Routing via NeuralUCB](https://arxiv.org/abs/2603.30035)
*Ming-Hua Tsai,Phat Tran*

Main category: cs.LG

TL;DR: NeuralUCB-based routing for cost-aware LLM selection outperforms baselines in utility reward while reducing inference costs compared to max-quality reference.


<details>
  <summary>Details</summary>
Motivation: Existing LLM routing approaches have tradeoffs between efficiency and adaptivity - supervised methods vs partial-feedback methods. Need for cost-aware routing that balances quality and inference costs.

Method: Implemented NeuralUCB-based routing policy and evaluated on RouterBench under simulated online setting. NeuralUCB is a contextual bandit algorithm that balances exploration and exploitation.
...
Conclusion: NeuralUCB is promising for cost-aware LLM routing, but challenges remain in action discrimination and exploration aspects.
...
### [193] [Aligned, Orthogonal or In-conflict: When can we safely optimize Chain-of-Thought?](https://arxiv.org/abs/2603.30036)
*Max Kaufmann,David Lindner,Roland S. Zimmermann,and Rohin Shah*

Main category: cs.LG

TL;DR: Training LLMs with reward terms that conflict between final outputs and reasoning chains reduces the monitorability of Chain-of-Thought reasoning, making oversight harder.


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought monitoring is promising for AI oversight, but training can affect how well a model's reasoning chain helps with monitoring. The paper aims to understand when and why training reduces CoT monitorability.

Method: Proposed a conceptual framework modeling LLM post-training as RL with reward decomposing into final output and CoT terms. Classified reward terms as aligned, orthogonal, or in-conflict. Trained LLMs in environments with these classifications and evaluated CoT monitorability changes.

Result: Training with "in-conflict" reward terms reduces CoT monitorability. Optimizing in-conflict reward terms is difficult. Orthogonal terms don't affect monitorability, and aligned terms improve it.

Conclusion: The relationship between final output rewards and CoT rewards during training determines CoT monitorability. Conflicting rewards reduce oversight capability, highlighting the importance of reward alignment for effective AI monitoring.
...
### [194] [A Multi-Modal Dataset for Ground Reaction Force Estimation Using Consumer Wearable Sensors](https://arxiv.org/abs/2603.28784)
*Parvin Ghaffarzadeh,Debarati Chakraborty,Koorosh Aslansefat,Ali Dostan,Yiannis Papadopoulos*

Main category: eess.SP

TL;DR: Open multi-modal dataset for estimating vertical ground reaction force from Apple Watch sensors with force plate ground truth, containing 492 validated trials across 5 activities with IMU and force data.


<details>
  <summary>Details</summary>
Motivation: To provide a fully open, reproducible resource for wearable biomechanics research that enables benchmarking of machine learning models for vGRF estimation using widely available consumer wearables like Apple Watch.

Method: Collected data from 10 healthy adults performing 5 activities while wearing Apple Watches at wrist and waist, with force plate ground truth. Dataset includes 492 validated trials with time-aligned IMU (100 Hz) and force plate (1000 Hz) data, plus metadata, quality-control flags, and data dictionaries.

Result: Created a high-quality dataset with 395 triad-complete trials containing wrist, waist, and force plate data. Demonstrated dataset quality through cross-sensor plausibility framework, repeatability analysis (ICC 0.871-0.990), and Monte Carlo sensitivity analysis showing robust validation metrics.

Conclusion: This open dataset supports reproducible research in wearable biomechanics, benchmarking of ML models for vGRF estimation, and investigation of sensor placement effects using consumer wearables, released under CC BY 4.0 with analysis scripts.
...
### [195] [Smartphone-Based Identification of Unknown Liquids via Active Vibration Sensing](https://arxiv.org/abs/2603.28787)
*Yongzhi Huang*

Main category: eess.SP

TL;DR: Smartphone-based liquid identification using active vibration to measure viscosity, achieving 95.47% accuracy across 30 liquid types.


<details>
  <summary>Details</summary>
Motivation: Traditional liquid identification instruments are expensive and inaccessible to the general public, creating a need for affordable, accessible alternatives using common devices like smartphones.

Method: Uses active vibration to measure liquid viscosity via smartphone accelerometers. Employs multiple signal processing stages to address practical challenges like under-sampling, self-interference, and liquid-volume changes, rather than relying on machine learning.

Result: Achieves viscosity estimation with 2.9% mean relative error and distinguishes 30 types of liquids with 95.47% average accuracy.

Conclusion: Demonstrates the feasibility of using commercial smartphones for accurate liquid identification through viscosity measurement, providing an accessible alternative to specialized instruments.
...
### [196] [Covariance-Domain Near-Field Channel Estimation under Hybrid Compression: USW/Fresnel Model, Curvature Learning, and KL Covariance Fitting](https://arxiv.org/abs/2603.28918)
*Rıfat Volkan Şenyuva*

Main category: eess.SP

TL;DR: CL-KL estimator enables joint angle-range estimation for near-field XL-MIMO using compressed covariance from hybrid architectures, eliminating range-dimension dictionary coherence and achieving superior NMSE with aperture-scalable computation.


<details>
  <summary>Details</summary>
Motivation: Near-field propagation in extremely large aperture arrays requires joint angle-range estimation, but hybrid architectures only provide compressed snapshots (N_RF ≪ M), making traditional 2D polar gridding problematic due to dictionary coherence in strong near-field regimes.

Method: Proposes Curvature-Learning KL (CL-KL) estimator that grids only the angle dimension and learns per-angle inverse range directly from compressed covariance via KL divergence minimization, using Q_θ-element dictionary instead of Q_θQ_r atoms of 2D polar gridding.

Result: CL-KL achieves lowest channel NMSE among six evaluated methods (including four full-array baselines using 64× more data) across SNR ∈ {-5,0,+5,+10} dB, runs in ~70ms per trial with aperture-scalable performance, and shows robustness to non-Gaussian source distributions with <0.6dB NMSE gap.

Conclusion: CL-KL provides an effective compressed-domain solution for near-field XL-MIMO channel estimation that eliminates range-dimension dictionary coherence, offers full hybrid front-end compatibility, and maintains computational scalability with array aperture size.
...
### [197] [Exact Statistical Characterization and Performance Analysis of Fluid Reconfigurable Intelligent Surfaces](https://arxiv.org/abs/2603.28974)
...
TL;DR: First exact closed-form analysis of FRIS-aided wireless systems showing channel gain statistics as linear combination of K-distributions, enabling precise outage and capacity analysis.


<details>
  <summary>Details</summary>
Motivation: FRIS extends conventional RIS by enabling physical reconfiguration of element positions, offering new degree of freedom for controlling spatial correlation and improving link reliability, but rigorous performance analysis has been challenging due to lack of exact statistical analyses of cascaded channels.

Method: Exploiting the spectral structure of the FRIS-induced correlation matrix to show that channel gain statistics can be represented as a finite linear combination of K-distributions, capturing various correlation regimes as special cases.

Result: Derived exact closed-form expressions for outage probability and ergodic capacity, conducted outage-based asymptotic analysis revealing true diversity order, and demonstrated via numerical results that fluidic reconfiguration yields tangible reliability gains.

Conclusion: The paper provides the first exact closed-form characterization of FRIS-aided systems, offering a unified analytical framework that captures various correlation regimes and demonstrates the practical benefits of fluidic reconfiguration for wireless reliability.
...
### [198] [Semantic Sensing: A Task-Oriented Paradigm](https://arxiv.org/abs/2603.29102)
*Xiaoqi Zhang,J. Andrew Zhang,Chang Liu,Weijie Yuan,Geoffrey Ye Li,Moeness G. Amin*

Main category: eess.SP

TL;DR: Proposes semantic sensing (SemS) framework that shifts sensing design from reconstruction fidelity to semantic recognition, using information bottleneck principle and DL-based joint waveform/receiver optimization for superior task performance.


<details>
  <summary>Details</summary>
Motivation: Current sensing technologies are limited to conventional parameter estimation while communication has advanced significantly. There's a need to move beyond reconstruction fidelity to semantic-aware sensing that extracts task-relevant information efficiently.

Method: 1) Formulate semantic sensing as semantics-oriented transceiver design using information bottleneck principle. 2) Develop DL-based framework for joint waveform parameter and receiver representation design. 3) Implement in OFDM system with Gumbel-Softmax-based pilot selector for resource masking. 4) Design specialized decoders: 2D ResNet for classification and correlation-driven 1D regression for delay estimation.

Result: The semantic pilot design achieves superior classification accuracy and ranging precision compared to reconstruction-based baselines, especially under constrained resource budgets. The framework effectively extracts task-relevant information while ignoring irrelevant data.

Conclusion: Semantic sensing represents a paradigm shift from reconstruction to semantic recognition, enabling more efficient and effective sensing in next-generation networks through joint waveform-receiver optimization guided by information bottleneck principles.
...
### [199] [Indian Peak Power demand Forecasting : Transformer Based Implementation of Temporal Architecture](https://arxiv.org/abs/2603.29179)
*Vishvaditya Luhach,Shashwat Jha*

Main category: eess.SP

TL;DR: Proposes Temporal Fusion Transformer for long-term electricity demand forecasting, outperforming other models on Indian peak power data.


<details>
  <summary>Details</summary>
Motivation: Long-term electricity demand forecasting is economically and strategically important but challenging due to unpredictable future variables. Most existing techniques focus on short-term forecasting.

Method: Uses Temporal Fusion Transformer (TFT) deep learning approach for long-term peak power demand forecasting. Applied to 6 years of Indian peak power demand data with 1-year prediction horizon.

Result: The proposed TFT model outperformed other popular forecasting models in benchmarks and was more accurate in modeling the variance in power demand.

Conclusion: Temporal Fusion Transformer is an effective approach for long-term electricity demand forecasting, demonstrating superior performance compared to existing methods.
...
### [200] [A Novel Low-Complexity Dual-Domain Expectation Propagation Detection Aided AFDM for Future Communications](https://arxiv.org/abs/2603.29218)
*Qin Yi,Ping Yang,Zilong Liu,Zeping Sui,Yue Xiao,Gang Wu*

Main category: eess.SP

TL;DR: Dual-domain low-complexity EP detection framework for AFDM systems exploiting domain-specific sparsity patterns to reduce complexity from cubic to linear order.


<details>
  <summary>Details</summary>
Motivation: AFDM systems need efficient detection algorithms that can handle both frequency-selective and doubly-selective channels without the computational burden of dense matrix inversions.

Method: Developed two EP detectors: EP-AF for frequency-selective channels exploiting AF-domain sparsity, and EP-T for doubly-selective channels exploiting time-domain sparsity. Both leverage quasi-banded sparsity patterns to enable linear-complexity matrix operations instead of cubic.

Result: EP-AF achieves nearly identical error rate performance to conventional methods with much lower complexity. EP-T offers attractive trade-off between detection performance and complexity for doubly-selective channels.

Conclusion: The proposed dual-domain EP framework effectively exploits domain-specific sparsity patterns in AFDM systems to achieve low-complexity detection while maintaining good performance.
...
### [201] [Semantic Communication for 6G Networks: A Trade-off between Distortion Criticality and Information Representability](https://arxiv.org/abs/2603.29293)
*Faizan Shafi,Rahul Jashvantbhai Pandya,Christo Kurisummoottil Thomas,Sridhar Iyer*

Main category: eess.SP

TL;DR: SA-cGAN framework for 6G semantic communication balances distortion vs. information trade-off using self-attention and knowledge graphs, achieving superior semantic metrics over conventional methods.


<details>
  <summary>Details</summary>
Motivation: Need to address the trade-off between distortion criticality and information representability in 6G semantic communication systems under varying channel conditions, particularly in low SNR regimes.

Method: Self-attention based conditional GAN framework with knowledge graph integration, continuous relaxation, submodular semantic selection, and principled constraint handling for adaptive semantic token selection and resource allocation.

Result: Achieves modest syntactic BLEU scores (~0.72 at 20dB) but significantly outperforms conventional and JSCC-based schemes in semantic metrics (similarity, accuracy, completeness >0.90 with SNR), with adaptive compression behavior preserving critical semantics.

Conclusion: SA-cGAN effectively captures distortion-invariant semantic representations and dynamically adapts transmitted content based on distortion criticality and information representability for meaning-centric 6G communication.
...
### [202] [Accelerating 5G Synchronization Signal Timing Offset Estimation Using Dual-Rate Sampling](https://arxiv.org/abs/2603.29341)
*Bitna Kim,Seungyeon Lee,Yelan Lee,Juyeop Kim*

Main category: eess.SP

TL;DR: Proposes a dual-rate sampling scheme for 5G cell search acceleration that reduces PSS detection time by 68% while maintaining PBCH decoding performance.


<details>
  <summary>Details</summary>
Motivation: Cell search engineers need to reduce computation time for fast initial access and radio link recovery. Since PSS detection consumes most of the cell search time, reducing its computational burden is critical for shortening the overall procedure.

Method: A novel timing offset estimation scheme using dual-rate sampling with a two-step estimation process that leverages the 5G SSB structure. This approach reduces the PSS detection search space without compromising subsequent processes.

Result: Performance evaluations in practical system and channel environments show the proposed scheme reduces cell search procedure time by 68% compared to baseline, while maintaining PBCH decoding performance.

Conclusion: The dual-rate sampling scheme effectively accelerates 5G cell search by significantly reducing PSS detection time without sacrificing system performance, addressing critical timing requirements for initial access and link recovery.
...
### [203] [ARC: Alignment-based RPM Estimation with Curvature-adaptive Tracking](https://arxiv.org/abs/2603.29354)
*Weiheng Hua,Changyu Hao*

Main category: eess.SP

TL;DR: ARC is a tacho-less rotational speed estimation method that unifies heterogeneous estimators via grid-based log-likelihood mapping and uses curvature-adaptive motion priors for robust tracking under noise and ambiguity.


<details>
  <summary>Details</summary>
Motivation: Traditional tacho-less RPM estimation methods struggle with noise, non-stationarity, and inharmonic interference. Probabilistic tracking can fuse multiple estimators, but heterogeneous estimators produce evidence on incompatible axes and scales, making fusion challenging.

Method: ARC (Alignment-based RPM Estimation with Curvature-adaptive Tracking) has two key innovations: 1) Unifies observation representation by mapping each estimator's evidence curve onto a shared RPM grid and converting to comparable grid-based log-likelihood via robust standardization and Gibbs-form energy shaping. 2) Introduces curvature-informed, state-dependent motion prior where transition variance is derived from local discrete Hessian of previous log-posterior, enabling smooth tracking around confident modes while preserving competing hypotheses.

Result: Experiments on synthetic stress tests and real vibration-table data demonstrate stable, physically plausible trajectories with interpretable uncertainty. Ablations confirm gains come from uncertainty-aware temporal propagation rather than per-frame peak selection or ad hoc rules.

Conclusion: ARC provides a principled solution for tacho-less rotational speed estimation by addressing the fundamental challenge of fusing heterogeneous estimators through unified observation representation and curvature-adaptive temporal propagation, enabling robust tracking in challenging conditions.
...
### [204] [Space-Time Adaptive Beamforming for Satellite Communications: Harnessing Doppler as New Signaling Dimensions](https://arxiv.org/abs/2603.29359)
*Hyeongtak Yun,Seyong Kim,Jeonghun Park*

Main category: eess.SP

TL;DR: LEO satellite downlinks suffer from severe channel correlation due to LoS-dominant propagation and high altitude, causing multiuser channel matrix ill-conditioning. The paper analyzes this limitation, identifies a density threshold where ZF fails, and proposes space-time adaptive beamforming (STAB) using residual Doppler shifts to restore spatial multiplexing.


<details>
  <summary>Details</summary>
Motivation: LEO satellite downlinks face fundamental limitations from severe channel correlation caused by line-of-sight dominant propagation and high orbital altitude, which confine users to narrow angular regions and make multiuser channel matrices ill-conditioned, limiting spatial multiplexing capabilities.

Method: 1) Rigorous characterization using Vandermonde channel structure, linking minimum eigenvalue to user crowding via balls-and-bins abstraction; 2) Derivation of asymptotic sum rate scaling laws for uniform linear and planar arrays; 3) Proposal of space-time adaptive beamforming (STAB) exploiting user-dependent residual Doppler shifts as additional discrimination dimension; 4) Development of space-Doppler user selection (SDS) algorithm for scheduling.

Result: Analysis reveals sharp density threshold beyond which zero-forcing precoding provably fails. STAB with SDS restores non-vanishing sum rate in regimes where purely spatial ZF collapses, achieving substantial sum rate gains over conventional methods in dense LEO downlink scenarios.

Conclusion: The paper provides fundamental understanding of LEO downlink limitations and demonstrates that exploiting residual Doppler shifts through space-time adaptive beamforming can overcome spatial multiplexing breakdown in dense user scenarios, offering practical solution for next-generation LEO satellite communications.
...
### [205] [Intelligent Forensics in Next-Generation Mobile Networks: Evidence, Methods, and Applications](https://arxiv.org/abs/2603.29364)
*Jiacheng Wang,Weihong Qin,Jialing He,Changyuan Zhao,Dusit Niyato,Tao Xiang*

Main category: eess.SP

TL;DR: Survey paper on intelligent forensics for next-gen mobile networks, proposing an evidence-centric framework and unified taxonomy for wireless investigations across physical, device, network, and cross-layer forensics.


<details>
  <summary>Details</summary>
Motivation: Future wireless security needs to move beyond real-time detection to accountable post-incident reconstruction. Traditional digital forensics is inadequate for wireless investigations due to short-lived, distributed, heterogeneous evidence affected by calibration issues, timing uncertainty, privacy constraints, and adversarial manipulation.

Method: Develops an evidence-centric framework treating wireless measurements as first-class forensic artifacts. Organizes field through unified taxonomy spanning physical-layer, device-layer, network-layer, and cross-layer forensics. Systematizes forensic workflow into readiness/preservation-by-design, acquisition, correlation/analysis, and reporting/reproducibility. Compares traditional methods with AI-assisted techniques.

Result: Provides comprehensive survey reviewing major application areas: anomaly discovery, attribution, provenance and localization, authenticity verification, and timeline reconstruction. Identifies key open challenges including domain shift, resource-aware evidence capture, and benefits/risks of generative evidence.

Conclusion: Positions wireless forensics as foundational capability for trustworthy, auditable, and reproducible security in next-generation wireless systems. Framework helps readers understand and streamline wireless forensics processes for specific applications like low-altitude wireless networks, vehicular communications, and edge general intelligence.
...
### [206] [A Three-Dimensional Path Loss Model for THz Band Aerial Communications](https://arxiv.org/abs/2603.29385)
*Sina Jorjani,Caglar Tunc,Ozgur Gurbuz,Akhtar Saeed*

Main category: eess.SP

TL;DR: Proposes a new 3D THz path loss model for aerial networks that accounts for arbitrary transceiver geometry and frequency-selective absorption, validated across various aerial link types and THz spectrum.


<details>
  <summary>Details</summary>
Motivation: Existing THz path loss models for aerial networks are limited to horizontal or vertical deployments and cannot model the random 3D geometry of transceiver locations, which is critical for reliable high-frequency aerial communication.

Method: Develops an analytical THz path loss model incorporating arbitrary 3D geometry of transceiver locations and frequency-selective absorption using two-dimensional regression, validated with propagation data from the Atmospheric Model (am) tool.

Result: The model is validated across multiple aerial link types (Dr2Dr, MAAC, U2U) over varying transceiver separation and 0.1-1 THz spectrum, showing accurate path loss characterization.

Conclusion: The proposed framework provides a unified and accurate model for analyzing and designing future high-frequency aerial communication systems, addressing limitations of existing models.
...
### [207] [SCROOGE: A Physics-Aware Framework for Efficient Orchestration of RIS-Assisted Networks](https://arxiv.org/abs/2603.29453)
...
TL;DR: SCROOGE is a physics-aware orchestration framework for RIS-assisted networks that uses offline-generated codebook information to enable low-latency online decisions for resource allocation, energy efficiency, and admission control.


<details>
  <summary>Details</summary>
Motivation: RISs are crucial for 6G programmable wireless environments, but practical integration lacks orchestration mechanisms that can jointly handle resource allocation, energy efficiency, and admission control with low online complexity.

Method: SCROOGE leverages physics-derived descriptors from offline RIS codebook compilation (optimal entries and per-element influence scores) to support low-latency operating-phase decisions. It introduces three mechanisms: influence-aware tier-consistent resource allocation, energy-efficient element deactivation, and tier-aware admission control.

Result: The framework enables practical RIS orchestration that remains compatible with network-level control requirements while avoiding the need for complex online optimization or idealized fading-based abstractions.

Conclusion: SCROOGE provides a practical physics-aware orchestration solution for multi-user RIS-assisted networks that addresses the gap between theoretical RIS capabilities and operational network integration requirements for 6G.
...
### [208] [Adaptive High-Speed Radar Signal Processing Architecture for 3D Localization of Multiple Targets on System on Chip](https://arxiv.org/abs/2603.29487)
*Aakanksha Tewari,Jai Mangal,Sumit J Darak,Shobha Sundar Ram,Arnav Shukla*

Main category: eess.SP

TL;DR: A novel 3D reconfigurable radar signal processing accelerator for ISAC in 6G vehicular communications that dynamically switches between high-accuracy and low-complexity frameworks based on signal conditions, achieving 5.6× faster processing and 24% throughput improvement.


<details>
  <summary>Details</summary>
Motivation: ISAC is crucial for 6G vehicular communications but faces speed-accuracy tradeoffs in radar signal processing that directly impact communication throughput. Current RSP implementations struggle to balance these competing requirements efficiently.

Method: Hardware-software codesign on Zynq MPSoC with fixed-point optimization. Two RSP frameworks: high-accuracy/high-complexity and low-complexity/low-accuracy. An adaptive architecture dynamically switches between them based on signal-to-clutter-plus-noise ratio.

Result: Achieves up to 5.6× faster RSP compared to state-of-the-art designs. System-level ISAC delivers 24% improvement in communication throughput without increasing hardware complexity.

Conclusion: The adaptive reconfigurable RSP accelerator successfully addresses the speed-accuracy tradeoff in ISAC, enabling significant performance improvements for 6G vehicular communications through intelligent framework switching based on signal conditions.
...
### [209] [Generation, Annihilation and Flow of Structural Information in Ultrasonic Nondestructive Evaluation](https://arxiv.org/abs/2603.29504)
*Frank Schubert*

Main category: eess.SP

TL;DR: Researchers formalize and quantify information in ultrasonic testing using a novel balance equation analogous to Poynting's theorem, showing how structural information flows from defects to sensors.


<details>
  <summary>Details</summary>
Motivation: Ultrasonic testing aims to extract maximum information about objects and defects from scattered wave fields, but there was no formal physical quantification of information in this context.

Method: Developed a balance equation for information analogous to Poynting's theorem for elastic energy, demonstrating how structural information is generated, annihilated, and travels within components.

Result: First physical formalization and quantification of information in ultrasonic testing, showing information pathways from defects to sensors through various examples.

Conclusion: The new information concept has significant potential for practical ultrasonic testing, structural health monitoring, numerical simulation, and machine learning, with distinct differences from Shannon and Fisher information.
...
### [210] [The DCT Neuron for Estimation and Compensation of Amplitude Distortions in OFDM Systems](https://arxiv.org/abs/2603.29680)
*Marc Martinez-Gost,Ana Pérez-Neira,Miguel Ángel Lagunas*

Main category: eess.SP

TL;DR: A receiver-side framework using DCT Neurons to identify amplitude distortions in OFDM channels with high efficiency and real-time adaptive compensation.


<details>
  <summary>Details</summary>
Motivation: Frequency-selective OFDM channels suffer from amplitude distortions that degrade performance. Existing methods may be complex or inefficient for real-time adaptive compensation of nonlinear responses in multicarrier systems.

Method: Uses DCT Neurons - compact adaptive processors based on discrete cosine transform - to characterize channel's nonlinear response. Operates directly in time domain, builds accurate signal model, tracks channel variations adaptively, and requires only two OFDM symbols for reliable identification.

Result: Achieves reliable identification of amplitude distortions with minimal overhead (as few as two OFDM symbols). The learned nonlinear response enables predistortion and iterative decoding for compensation.

Conclusion: The framework enables low-complexity, real-time adaptive compensation of complex nonlinear responses in multicarrier systems, making it suitable for practical implementation in OFDM-based communication systems.
...
### [211] [Beyond Legacy OFDM: A Mobility-Adaptive Multi-Gear Framework for 6G](https://arxiv.org/abs/2603.29721)
*Mauro Marchese,Dario Tagliaferri,Henk Wymeersch,Musa Furkan Keskin,Emanuele Viterbo,Pietro Savazzi*

Main category: eess.SP

TL;DR: GS-OFDM is a unified framework where base stations adaptively switch between three transmission gears (from legacy OFDM to delay-Doppler processing) based on user mobility conditions to optimize 6G performance.


<details>
  <summary>Details</summary>
Motivation: While OFDM is confirmed as the baseline waveform for 6G, its performance degrades significantly in high-mobility scenarios envisioned for future 6G networks. There's a need for adaptive solutions that can handle varying mobility conditions.

Method: Proposes gear-switching OFDM (GS-OFDM), a unified framework where base stations adaptively select among three transmission gears based on user mobility conditions. The gears range from legacy OFDM to delay-Doppler domain processing, building upon the GEARBOX-PHY vision.

Result: The paper illustrates benefits of adaptive gear switching for communication throughput. The framework enables optimization of transmission based on channel mobility conditions experienced by user equipment.

Conclusion: GS-OFDM provides an adaptive solution for 6G networks to handle varying mobility conditions. The paper concludes with an outlook on research challenges and opportunities for this approach.
...
### [212] [AI-Programmable Wireless Connectivity: Challenges and Research Directions Toward Interactive and Immersive Industry](https://arxiv.org/abs/2603.29752)
*Haris Gacanin*

Main category: eess.SP

TL;DR: This paper explores integrating traditional signal processing with lightweight AI models (Tiny/Real-time ML) to create energy-efficient, programmable wireless connectivity for 6G systems, addressing practical integration challenges rather than just high-level concepts.


<details>
  <summary>Details</summary>
Motivation: Prior research focused on high-level concepts like LLMs in 6G, but lacked practical integration approaches. There's a need to address system-level challenges of combining signal processing with AI to meet strict constraints on computing resources, adaptability, and reliability for next-generation wireless networks.

Method: The paper examines the role of compact AI models (Tiny ML and Real-time ML) in enhancing wireless connectivity. It emphasizes practical integration challenges and provides application examples to illustrate how AI-driven signal processing can support next-generation networks while adhering to resource constraints.

Result: The paper outlines a pathway toward efficient and adaptive connectivity solutions by combining classical signal processing with lightweight AI methods, advancing beyond high-level discussions to address practical system-level integration challenges for 6G and beyond.

Conclusion: Integrating traditional signal processing with compact AI models offers a viable approach to developing energy-efficient, programmable, and scalable wireless connectivity infrastructures for future 6G systems, addressing both theoretical concepts and practical implementation challenges.
...
### [213] [Fisher Information Limits of Satellite RF Fingerprint Identifiability for Authentication](https://arxiv.org/abs/2603.29766)
*Haofan Dong,Ozgur B. Akan*

Main category: eess.SP

TL;DR: Theoretical framework for RF fingerprinting establishes performance bounds and identifiability conditions for satellite transmitter authentication using IQ imbalance and PA nonlinearity parameters.


<details>
  <summary>Details</summary>
Motivation: Existing RF fingerprinting methods for satellite authentication lack theoretical performance guarantees, making it difficult to understand when and why they work or fail.

Method: Derived Fisher information matrix for joint estimation of IQ imbalance and PA nonlinearity parameters, established Cramér-Rao bounds based on constellation moments, and developed discrimination metric to predict which hardware parameters dominate authentication for different modulations.

Result: Identified necessary condition for IQ identifiability (β > 0), explained OrbID's poor performance on BPSK signals, validated framework on 24 Iridium satellites achieving cross-file PA correlation r = 0.999, and demonstrated DR-weighted authentication achieving AUC = 0.934 vs 0.807 with equal weighting.

Conclusion: The theoretical framework provides performance guarantees and explains empirical observations, enabling more reliable satellite authentication with predictable feature effectiveness based on modulation type.
...
### [214] [JEPA-MSAC: A Joint-Embedding Predictive Architecture for Multimodal Sensing-Assisted Communications](https://arxiv.org/abs/2603.29796)
*Can Zheng,Jiguang He,Guofa Cai,Nannan Li,Mehdi Bennis,Henk Wymeersch,Merouane Debbah*

Main category: eess.SP

TL;DR: JEPA-MSAC: A self-supervised multimodal predictive representation learning framework for wireless environments that learns a shared backbone for multiple PHY tasks with low adaptation cost.


<details>
  <summary>Details</summary>
Motivation: Future wireless systems need predictive and transferable representations for multiple physical-layer tasks under dynamic environments, but current supervised learning methods are single-task focused and have high adaptation costs.

Method: Proposes JEPA-MSAC: maps multimodal sensing and communication measurements into unified token space, pretrains shared backbone using temporal block-masked joint-embedding predictive architecture to learn predictive latent space capturing environment dynamics and cross-modal dependencies.

Result: Extensive experiments show the latent state supports accurate multi-task prediction (localization, beam prediction, RSSI prediction) with low adaptation cost. Ablation studies reveal scaling behavior and impact of key pretraining setups.

Conclusion: JEPA-MSAC provides an effective self-supervised framework for learning predictive representations that support multiple wireless PHY tasks with reduced adaptation overhead compared to single-task supervised approaches.
...
### [215] [Conditional Diffusion-Based Point Cloud Imaging for UAV Position and Attitude Sensing](https://arxiv.org/abs/2603.29822)
*Xinhong Dai,Yuan Gao,Hao Jiang,Xiaojun Yuan,Xin Wang*

Main category: eess.SP

TL;DR: A generative UAV sensing approach using conditional diffusion models to reconstruct electromagnetic point clouds for precise UAV position, attitude, and shape estimation from reflected wireless signals.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of accurately sensing UAV position, attitude, and shape using wireless signals, especially under measurement noise and channel variations, enabling comprehensive UAV state estimation through electromagnetic imaging.

Method: Develops a generative UAV sensing approach where: 1) position and SNR embedding assist UAV feature extraction from estimated sensing channels, 2) a conditional diffusion model generates electromagnetic point clouds using extracted features, representing UAV spatial information and electromagnetic properties.

Result: Simulation results show reconstructed point clouds have higher fidelity than competing schemes, enabling more accurate capture of UAV attitude and shape information, as well as more precise position estimation.

Conclusion: The proposed generative approach using conditional diffusion models successfully enables unified extraction of UAV position, attitude, and shape from wireless signals, outperforming existing methods in reconstruction fidelity and estimation accuracy.
...
### [216] [Sensor array and camera fusion via unbalanced optimal transport for 3D source localization](https://arxiv.org/abs/2603.29940)
*Ilyes Jaouedi,Gilles Chardon,José Picheral*

Main category: eess.SP

TL;DR: A fusion framework combining sensor array measurements with camera observations for 3D multi-source localization using covariance matrix fitting with unbalanced optimal transport regularization.


<details>
  <summary>Details</summary>
Motivation: To improve 3D localization accuracy by fusing sensor array measurements with camera observations, addressing limitations of sensor-only approaches while avoiding the need for labeled training data required by deep learning methods.

Method: Extends covariance matrix fitting method with unbalanced optimal transport regularization that softly aligns sensor array responses with visual priors. Uses greedy coordinate descent algorithm to efficiently solve the large-scale optimization problem.

Result: Experiments on real data show improved localization accuracy compared to sensor-only baselines. The method makes full 3D localization computationally feasible in practice.

Conclusion: The proposed modular fusion framework effectively combines sensor array and camera data without requiring labeled training data, improving 3D localization accuracy while maintaining computational efficiency. The method is generalizable to arbitrary sensor arrays beyond the acoustic arrays used for validation.
...
### [217] [An Information-Theoretic Method for Dynamic System Identification With Output-Only Damping Estimation](https://arxiv.org/abs/2603.29956)
*Marios Impraimakis,Feiyu Zhou,Andrew Plummer*

Main category: eess.SP

TL;DR: A novel information-theoretic method using Shannon entropy and Kullback-Leibler divergence improves damping estimation accuracy in mechanical systems for better alert systems.


<details>
  <summary>Details</summary>
Motivation: Current operational modal identification methods suffer from poor damping estimates due to their empirical nature, which negatively impacts alert warning systems when duration is misestimated, especially in damage/anomaly detection scenarios using vibration amplitudes.

Method: Proposes approaches based on Shannon entropy and Kullback-Leibler divergence to monitor vibration levels in near real-time and provide immediate alerts when thresholds are exceeded. Uses information-theoretic metrics with vibration-based measurements.

Result: The approach selects optimal models that accurately capture correct alert duration. Validated using real-world data from University of Bath's multi-axis simulation table and the benchmark IASC-ASCE structural health monitoring problem.

Conclusion: The information-theoretic method provides a powerful tool for system identification and monitoring by enhancing damping estimation accuracy and improving alert system reliability.
...
### [218] [End-to-end optimization of sparse ultrasound linear probes](https://arxiv.org/abs/2603.29014)
*Sergio Urrea,Adrian Basarab,Hervé Liebgott,Henry Arguello*

Main category: eess.IV

TL;DR: End-to-end optimization framework jointly learns sparse ultrasound array configuration and image reconstruction to reduce hardware complexity while maintaining image quality.


<details>
  <summary>Details</summary>
Motivation: Ultrasound imaging faces trade-off between image quality and hardware complexity from dense transducers; sparse arrays can mitigate this but need optimization.

Method: Integrates differentiable Image Formation Model with HARD Straight Through Estimator selection mask, unrolled Iterative Soft-Thresholding Algorithm deconvolution, and residual CNN. Combines physical consistency (PSF, convolutional model) with structural fidelity metrics (contrast, SLR, entropy, diversity).

Result: Learned configuration preserves axial and lateral resolution with half the active elements using 3.5MHz probe simulations.

Conclusion: Physics-guided, data-driven approach enables compact, cost-efficient ultrasound probe design without sacrificing image quality, expandable to 3-D volumetric imaging.
...
### [219] [Retinal Malady Classification using AI: A novel ViT-SVM combination architecture](https://arxiv.org/abs/2603.29181)
*Shashwat Jha,Vishvaditya Luhach,Raju Poddar*

Main category: eess.IV

TL;DR: Proposes ViT-SVM hybrid architecture for automated classification of OCT scans to detect macular holes, central serous retinopathy, and diabetic retinopathy.


<details>
  <summary>Details</summary>
Motivation: Early detection of retinal diseases (macular holes, central serous retinopathy, diabetic retinopathy) is crucial to prevent vision loss, requiring automated diagnostic systems.

Method: Hybrid Vision Transformer and Support Vector Machine (ViT-SVM) architecture for classifying optical coherence tomography (OCT) scans of retinal defects.

Result: Performance analysis of the ViT-SVM architecture for automated classification of retinal defects from OCT scans (results not specified in abstract).

Conclusion: Proposes a novel hybrid deep learning approach for early automated detection of retinal diseases using OCT imaging.
...
### [220] [Rich-U-Net: A medical image segmentation model for fusing spatial depth features and capturing minute structural details](https://arxiv.org/abs/2603.29404)
*Zhuoyi Fang,Kexuan Shi,Jiajia Liu,Qiang Han*

Main category: eess.IV

TL;DR: Rich-U-Net model improves medical image segmentation by integrating spatial and depth features for better fine structure detection and segmentation accuracy.


<details>
  <summary>Details</summary>
Motivation: Current medical image segmentation methods underperform in extracting spatial information and mining complex structures from medical images, limiting diagnostic accuracy and treatment planning.

Method: Rich-U-Net model with multi-level and multi-dimensional feature fusion and optimization strategies that effectively integrate both spatial and depth features.

Result: Outperforms other state-of-the-art models on ISIC2018, BUSI, GLAS, and CVC datasets in Dice, IoU, and HD95 metrics.

Conclusion: Rich-U-Net enhances medical image segmentation by better detecting fine structures and intricate details through integrated spatial-depth feature fusion.
...
### [221] [Polyhedral Unmixing: Bridging Semantic Segmentation with Hyperspectral Unmixing via Polyhedral-Cone Partitioning](https://arxiv.org/abs/2603.29438)
*Antoine Bottenmuller,Etienne Decencière,Petr Dokládal*

Main category: eess.IV

TL;DR: Proposes a segmentation-to-unmixing pipeline that connects semantic segmentation with hyperspectral unmixing by leveraging polyhedral-cone regions in spectral space, enabling user control through choice of segmentation method.


<details>
  <summary>Details</summary>
Motivation: Semantic segmentation and hyperspectral unmixing are complementary but typically addressed independently. The paper aims to bridge these two problems by showing their fundamental connection under the linear mixing model.

Method: Under linear mixing model, pixel classification by dominant materials induces polyhedral-cone regions. The pipeline: 1) uses any semantic segmentation to construct polyhedral-cone partition, 2) computes signed distances to regions, 3) transforms distances via basis change, 4) projects onto probability simplex for initial abundance estimate, 5) extracts endmembers via matrix pseudo-inversion.

Result: Experiments on three real datasets show effectiveness with appropriate clustering algorithms, with consistent improvements over recent deep and non-deep state-of-the-art methods.

Conclusion: The approach bridges segmentation and unmixing, provides user control through segmentation choice, improves interpretability, and demonstrates competitive performance while being deterministic and lightweight.
...
### [222] [CREST: Constraint-Release Execution for Multi-Robot Warehouse Shelf Rearrangement](https://arxiv.org/abs/2603.28803)
*Jiaqi Tan,Yudong Luo,Sophia Huang,Yifan Yang,Hang Ma*

Main category: cs.RO

TL;DR: CREST is a new execution framework for multi-robot shelf rearrangement that proactively releases trajectory constraints during execution, outperforming previous MAPF-DECOMP approach by reducing agent travel, makespan, and shelf switching by up to 40.5%, 33.3%, and 44.4% respectively.


<details>
  <summary>Details</summary>
Motivation: The MAPF-DECOMP framework for DD-MAPD (warehouse shelf rearrangement) enforces strict trajectory dependencies, leading to poor execution quality with idle agents and unnecessary shelf switching. There's a need for more continuous shelf carrying during execution.

Method: CREST introduces proactive constraint release during execution, allowing more flexible agent scheduling. It computes collision-free shelf trajectories first (like MAPF-DECOMP) but then dynamically releases trajectory constraints during execution to enable more continuous shelf carrying.

Result: CREST consistently outperforms MAPF-DECOMP across diverse warehouse layouts, reducing agent travel by up to 40.5%, makespan by up to 33.3%, and shelf switching by up to 44.4%. Benefits are even greater when considering lift/place overhead.

Conclusion: Execution-aware constraint release is crucial for scalable warehouse rearrangement. CREST demonstrates significant improvements over previous approaches by enabling more continuous shelf carrying through proactive constraint management during execution.
...
### [223] [A Classification of Heterogeneity in Uncrewed Vehicle Swarms and the Effects of Its Inclusion on Overall Swarm Resilience](https://arxiv.org/abs/2603.28831)
*Abhishek Joshi,Abhishek Phadke,Tianxing Chu,F. Antonio Medrano*

Main category: cs.RO

TL;DR: Systematic framework for grouping heterogeneous uncrewed vehicle swarms based on agent nature, hardware structure, and operational space, showing strategic heterogeneity improves performance and resilience.


<details>
  <summary>Details</summary>
Motivation: Combining different types of agents in UV swarms enhances mission resilience and operational capabilities across various applications, but requires systematic understanding of grouping approaches.

Method: Developed a systematic framework with three main factors: agent nature (behavior/function), hardware structure (physical configuration/sensing), and operational space (domain). Conducted literature review to analyze strategic heterogeneity benefits.

Result: Heterogeneous swarms significantly improve performance and resilience by leveraging diverse capabilities, adapting roles dynamically, and integrating multidimensional sensor data. Operational challenges include communication constraints, energy-aware coordination, and control integration.

Conclusion: Heterogeneous swarm technology is approaching readiness for high-value applications, requiring consideration of sim-to-real transfer, standardized metrics, and interoperable control architectures. The study provides taxonomy and evidence-based design methods for mission-ready heterogeneous swarms.
...
### [224] [A Semantic Observer Layer for Autonomous Vehicles: Pre-Deployment Feasibility Study of VLMs for Low-Latency Anomaly Detection](https://arxiv.org/abs/2603.28888)
*Kunal Runwal,Swaraj Gajare,Daniel Adejumo,Omkar Ankalkope,Siddhant Baroth,Aliasghar Arab*

Main category: cs.RO

TL;DR: A semantic observer layer using quantized vision-language models (VLMs) to detect semantic anomalies in autonomous driving, achieving ~500ms inference (50x speedup) while maintaining safety-critical performance.


<details>
  <summary>Details</summary>
Motivation: Semantic anomalies pose critical safety risks in autonomous driving that pixel-level detectors cannot handle, requiring a higher-level reasoning system to monitor for context-dependent hazards.

Method: Proposed semantic observer layer using quantized VLM (Nvidia Cosmos-Reason1-7B with NVFP4 quantization and FlashAttention2) running at 1-2Hz alongside primary AV control loop to monitor for semantic edge cases and trigger fail-safe handoffs.

Result: Achieved ~500ms inference time (~50x speedup over unoptimized FP16 baseline), identified NF4 recall collapse (10.6%) as deployment constraint, and established feasibility for semantic observer architecture on AV platforms.

Conclusion: The semantic observer architecture with quantized VLMs provides a viable solution for detecting semantic anomalies in autonomous driving, meeting timing requirements while addressing critical safety needs through performance-safety mapping analysis.
...
### [225] [Bootstrap Perception Under Hardware Depth Failure for Indoor Robot Navigation](https://arxiv.org/abs/2603.28890)
*Nishant Pushparaju,Vivek Mattam,Aliasghar Arab*

Main category: cs.RO

TL;DR: Bootstrap perception system for indoor robot navigation that uses surviving depth pixels from failing hardware sensors to calibrate learned monocular depth, creating a failure-aware sensing hierarchy that selectively fuses LiDAR, hardware depth, and learned depth.


<details>
  <summary>Details</summary>
Motivation: Indoor robot navigation faces two key challenges: 1) Time-of-flight cameras lose up to 78% of depth pixels on reflective surfaces, and 2) 2D LiDAR alone cannot sense obstacles above its scan plane. Existing solutions don't effectively handle these hardware failures without external data.

Method: The system exploits a self-referential property of hardware failure: surviving valid pixels from the failing sensor calibrate learned monocular depth to metric scale. It creates a failure-aware sensing hierarchy: LiDAR as geometric anchor, hardware depth where valid, and learned depth only where needed. A compact distilled student model enables real-time performance.

Result: Selective fusion increases costmap obstacle coverage by 55-110% over LiDAR alone. The distilled student runs at 218 FPS on Jetson Orin Nano and achieves 9/10 navigation success with zero collisions in closed-loop simulation, matching ground-truth depth baseline at a fraction of the foundation model's cost.

Conclusion: The bootstrap perception system successfully addresses hardware depth failures in indoor navigation by creating a self-calibrating, failure-aware sensing hierarchy that maintains safety while significantly improving obstacle coverage, all with real-time efficiency on embedded hardware.
...
### [226] [Robust Multi-Agent Reinforcement Learning for Small UAS Separation Assurance under GPS Degradation and Spoofing](https://arxiv.org/abs/2603.28900)
*Alex Zongo,Filippos Fotiadis,Ufuk Topcu,Peng Wei*

Main category: cs.RO

TL;DR: MARL-based robust separation assurance for sUAS under GPS degradation/spoofing using adversarial perturbation theory without adversarial training.


<details>
  <summary>Details</summary>
Motivation: Need robust safety for small drones when GPS signals are corrupted (degraded/spoofed), as corrupted position broadcasts make air traffic state unreliable.

Method: Formulate as zero-sum game between agents and adversary; derive closed-form adversarial perturbation expression; integrate into MARL policy gradient algorithm.

Result: Near-zero collision rates under corruption up to 35% in high-density sUAS simulation, outperforming baseline without adversarial perturbations.

Conclusion: Closed-form adversarial perturbation enables robust MARL policies for sUAS safety under GPS corruption without adversarial training.
...
### [227] [See Something, Say Something: Context-Criticality-Aware Mobile Robot Communication for Hazard Mitigations](https://arxiv.org/abs/2603.28901)
*Bhavya Oza,Devam Shah,Ghanashyama Prabhu,Devika Kodi,Aliasghar Arab*

Main category: cs.RO

TL;DR: A framework for autonomous mobile robots that uses VLM/LLM-based perception to assess hazard criticality and generate adaptive emergency messages, reducing response time and improving user trust.


<details>
  <summary>Details</summary>
Motivation: Autonomous mobile robots need to communicate hazards quickly and appropriately in safety-critical situations. Current systems often have delayed or miscalibrated responses that increase time to action and risk of damage. There's a need for context-sensitive assessment of criticality, time sensitivity, and mitigation feasibility.

Method: A framework using VLM/LLM-based perception to drive adaptive message generation. The system assesses context to determine appropriate response levels - for example, a knife in a kitchen gets calm acknowledgment while the same object in a corridor triggers urgent coordinated alerts.

Result: Validation in 60+ runs with a patrolling mobile robot shows faster response times and increased user trust to 82% compared to fixed-priority baselines. The structured criticality assessment improves both response speed and mitigation effectiveness.

Conclusion: Systematic context-sensitive assessment of hazard criticality enables autonomous mobile robots to communicate more effectively in emergency scenarios, reducing time to action while building user trust through appropriate response calibration.
...
### [228] [Why That Robot? A Qualitative Analysis of Justification Strategies for Robot Color Selection Across Occupational Contexts](https://arxiv.org/abs/2603.28919)
*Jiangen He,Wanqi Zhang,Jessica K. Barfield*

Main category: cs.RO

TL;DR: Study examines how social biases influence robot selection based on skin tone and anthropomorphism across occupations, revealing that while functional reasoning dominates, racial stereotypes subtly shape choices and justifications.


<details>
  <summary>Details</summary>
Motivation: As robots enter the workforce, understanding how implicit social biases affect human preferences is crucial to prevent perpetuating societal stereotypes through robot design and selection.

Method: Qualitative analysis of 4,146 open-ended justifications from 1,038 participants using a validated multidimensional coding scheme developed via human-AI consensus (κ=0.73). Examined robot color selection across four professional contexts with varying anthropomorphic features.

Result: Functionalism dominated justifications (52%), but participants adapted practical rationales to align with racial/occupational stereotypes. Bias operated subconsciously - stereotype primes shifted color choices without affecting justifications. Demographic backgrounds shaped strategies, and anthropomorphic robots prompted Machine-Centric de-racialization.

Conclusion: Social biases subtly influence robot preferences despite functional justifications. Designers must address these implicit biases to prevent perpetuating stereotypes in workforce robots, especially considering how anthropomorphism and demographics shape interpretation.
...
### [229] [World2Rules: A Neuro-Symbolic Framework for Learning World-Governing Safety Rules for Aviation](https://arxiv.org/abs/2603.28952)
...
<details>
  <summary>Details</summary>
Motivation: Real-world safety-critical systems have complex, context-dependent rules that are difficult to specify manually. Learning from multimodal data is challenging due to noise, inconsistency, and sparse failure cases. Pure neural methods lack formal guarantees, while symbolic methods are brittle with imperfect observations.

Method: World2Rules uses neural models as proposal mechanisms for candidate symbolic facts from multimodal aviation data (nominal operations and crash reports), then applies inductive logic programming as a verification layer. It employs hierarchical reflective reasoning to enforce consistency across examples, subsets, and rules, filtering unreliable evidence and aggregating only mutually consistent components.

Result: The framework achieves 23.6% higher F1 score than purely neural baselines and 43.2% higher F1 score than single-pass neuro-symbolic baselines on real-world aviation safety data. It produces compact, interpretable first-order logic rules suitable for safety-critical reasoning and formal analysis.

Conclusion: World2Rules effectively combines neural and symbolic approaches to learn safety rules from noisy multimodal data, providing formal guarantees while handling real-world imperfections, making it suitable for safety-critical applications like aviation.
...
### [230] [AutoWorld: Scaling Multi-Agent Traffic Simulation with Self-Supervised World Models](https://arxiv.org/abs/2603.28963)
*Mozhgan Pourkeshavatz,Tianran Liu,Nicholas Rhinehart*

Main category: cs.RO

TL;DR: AutoWorld: A traffic simulation framework that learns from unlabeled LiDAR occupancy data using world models and cascaded sampling to improve realism without costly labeled data.


<details>
  <summary>Details</summary>
Motivation: Existing data-driven traffic simulators rely heavily on supervised learning from labeled trajectories or semantic annotations, making them costly to scale. Meanwhile, large amounts of unlabeled sensor data (like LiDAR) are available but unused. The paper aims to leverage unlabeled data to improve traffic simulation performance without additional labeling costs.

Method: 1. Learns a world model from unlabeled occupancy representations of LiDAR data. 2. Constructs coarse-to-fine predictive scene context as input to multi-agent motion generation. 3. Uses cascaded Determinantal Point Process (DPP) framework to guide sampling for diversity. 4. Implements motion-aware latent supervision objective to enhance scene dynamics representation.

Result: AutoWorld ranks first on the WOSAC benchmark according to the primary Realism Meta Metric (RMM). Performance consistently improves with inclusion of unlabeled LiDAR data. Ablation studies confirm efficacy of each component.

Conclusion: AutoWorld demonstrates that unlabeled sensor data can effectively scale traffic simulation realism without additional labeling. The framework paves the way for more scalable and realistic traffic simulation using readily available unlabeled data.
...
### [231] [Large Neighborhood Search for Multi-Agent Task Assignment and Path Finding with Precedence Constraints](https://arxiv.org/abs/2603.28968)
*Viraj Parimi,Brian C. Williams*

Main category: cs.RO

TL;DR: TAPF-PC extends MAPF-PC by jointly optimizing task assignment, precedence constraints, and routing costs using large neighborhood search that iteratively improves feasible solutions through reassignment-based repair.


<details>
  <summary>Details</summary>
Motivation: Many multi-robot applications require efficient task completion in correct order for downstream operations. While MAPF-PC handles fixed task sequences with precedence constraints, solution quality often depends on which agent performs which task, motivating the need to jointly optimize assignment, precedence satisfaction, and routing costs.

Method: Develops a large neighborhood search approach that starts from a feasible MAPF-PC seed solution and iteratively improves it through reassignment-based neighborhood repair. The method restores feasibility within each selected neighborhood while exploring the coupled TAPF-PC search space.

Result: Experiments across multiple benchmark families and scaling regimes show that the best-performing configuration improves 89.1% of instances over fixed-assignment seed solutions, demonstrating that large neighborhood search effectively captures gains from flexible reassignment under precedence constraints.

Conclusion: Large neighborhood search is an effective approach for solving the TAPF-PC problem, successfully capturing the benefits of flexible task reassignment while maintaining precedence constraints, with significant improvements over fixed-assignment solutions.
...
### [232] [Gleanmer: A 6 mW SoC for Real-Time 3D Gaussian Occupancy Mapping](https://arxiv.org/abs/2603.29005)
*Zih-Sing Fu,Peter Zhi Xuan Li,Sertac Karaman,Vivienne Sze*

Main category: cs.RO

TL;DR: Gleanmer is a low-power SoC with accelerator for GMMap (3D occupancy mapping using Gaussians) that achieves real-time performance under 6mW for edge applications.


<details>
  <summary>Details</summary>
Motivation: High-fidelity 3D occupancy mapping is essential for edge applications like AR/VR and autonomous navigation, but current solutions are limited by power constraints.

Method: Algorithm-hardware co-optimizations for GMMap using Gaussians, with direct computation and efficient reuse of compact Gaussians, plus approximate computation to reduce accelerator area.

Result: Reduces construction energy by up to 63% and query energy by up to 81%, with 38% accelerator area reduction. Processes 640x480 images at >88 fps for construction and 540K coordinates/sec for query, all under 6mW.

Conclusion: Gleanmer is the first fabricated SoC achieving real-time 3D occupancy mapping under 6mW, enabling power-efficient edge applications.
...
### [233] [Efficient Camera Pose Augmentation for View Generalization in Robotic Policy Learning](https://arxiv.org/abs/2603.29192)
...
Method: Uses permutation-equivariant architecture to reconstruct 3D scenes from sparse, uncalibrated inputs in one forward pass. Employs 3D-prior distillation to regularize 3DGS optimization and prevent geometric collapse. Renders diverse synthetic views from stable 3D representations to augment training data.

Result: The framework forces policies to ground decisions in underlying 3D structures, enabling robust execution under severe spatial perturbations where baseline methods degrade.

Conclusion: GenSplat provides an effective solution for view-generalized policy learning by leveraging 3D scene reconstruction and novel view rendering to overcome 2D policies' viewpoint limitations.
...
### [234] [Kilohertz-Safe: A Scalable Framework for Constrained Dexterous Retargeting](https://arxiv.org/abs/2603.29213)
*Yinxiao Tian,Ziyi Yang,Zinan Zhao,Zhen Kan*

Main category: cs.RO

TL;DR: A convex optimization framework for real-time dexterous hand teleoperation that reformulates motion retargeting as a quadratic program with safety guarantees and high-frequency performance.


<details>
  <summary>Details</summary>
Motivation: Existing motion retargeting methods face limitations: nonlinear optimization approaches are computationally expensive for real-time kilohertz control, while learning-based methods lack formal safety guarantees needed for complex manipulation tasks.

Method: Reformulates nonlinear retargeting problem into convex quadratic program in joint differential space, incorporates heterogeneous constraints (kinematic limits, collision avoidance) through systematic linearization, and integrates control barrier functions for formal safety guarantees.

Result: Achieves high-frequency operation with average latency of 9.05 ms, outperforms state-of-the-art methods (Dex-Retargeting and GeoRT), with over 95% of retargeted frames satisfying safety criteria, effectively mitigating self-collision and penetration during complex manipulation tasks.

Conclusion: Proposed framework provides a scalable solution for dexterous hand teleoperation that combines real-time performance with formal safety guarantees, validated through simulations and hardware experiments on the Wuji Hand platform.
...
### [235] [Long-Reach Robotic Manipulation for Assembly and Outfitting of Lunar Structures](https://arxiv.org/abs/2603.29226)
*Stanley Wang,Venny Kojouharov,Long Yin Chung,Daniel Morton,Mark Cutkosky*

Main category: cs.RO

TL;DR: A compact long-reach manipulator with deployable composite boom for lunar infrastructure tasks like cable routing, achieving <15mm endpoint accuracy at 1.8m reach.


<details>
  <summary>Details</summary>
Motivation: Future lunar infrastructure construction requires autonomous robots for tasks like electrical outfitting, which involves transporting, routing, and manipulating cables across large structures.

Method: Developed a compact manipulator with deployable composite boom, characterized deflection, vibration, and blossoming effects, and created a manipulation control strategy to mitigate these issues.

Result: Achieved average endpoint accuracy error less than 15 mm for boom lengths up to 1.8 meters, demonstrated with a cable routing task.

Conclusion: The deployable boom manipulator shows potential for lunar outfitting applications requiring long reach, with sufficient accuracy for cable routing tasks.
...
### [236] [Kernel-SDF: An Open-Source Library for Real-Time Signed Distance Function Estimation using Kernel Regression](https://arxiv.org/abs/2603.29227)
*Zhirui Dai,Tianxing Fan,Mani Amani,Jaemin Seo,Ki Myung Brian Lee,Hyondong Oh,Nikolay Atanasov*

Main category: cs.RO

TL;DR: Kernel-SDF: A real-time kernel regression library for uncertainty-aware signed distance function estimation from streaming sensor data.


<details>
  <summary>Details</summary>
Motivation: Existing SDF reconstruction methods have limitations: voxel-based approaches lack uncertainty quantification and have fixed resolution, neural networks require significant training time, and Gaussian processes struggle with scalability, sign estimation, and uncertainty calibration. There's a need for accurate, real-time, uncertainty-aware SDF estimation for robotics applications.

Method: Two-stage approach: front-end learns continuous occupancy field via kernel regression, back-end estimates accurate SDF via Gaussian process regression using samples from front-end surface boundaries. The method provides SDF, SDF gradient, SDF uncertainty, and mesh construction in real-time.

Result: Kernel-SDF achieves superior accuracy compared to existing methods while maintaining real-time performance. It provides accurate SDF, SDF gradient, SDF uncertainty, and mesh construction capabilities.

Conclusion: Kernel-SDF is an effective open-source library for real-time, uncertainty-aware SDF estimation that addresses limitations of existing methods and is suitable for various robotics applications requiring reliable geometric information.
...
### [237] [Long-Reach Robotic Cleaning for Lunar Solar Arrays](https://arxiv.org/abs/2603.29240)
*Stanley Wang,Velin Kojouharov,Long Yin Chung,Daniel Morton,Mark Cutkosky*

Main category: cs.RO

TL;DR: A small mobile robot with long-reach boom and compliant wrist for gentle lunar solar array cleaning to mitigate dust accumulation that degrades power output.


<details>
  <summary>Details</summary>
Motivation: Lunar dust accumulation on solar arrays rapidly degrades power output, threatening mission-critical energy supply for lunar infrastructure. With commercial lunar activity accelerating, reliable maintenance systems are needed to preserve performance and extend system life.

Method: A small mobile robot equipped with a long-reach lightweight deployable boom and interchangeable cleaning tool, featuring a compliant wrist with distal force sensing and velocity-based admittance controller to regulate stable contact during surface cleaning.

Result: In benchtop experiments on planar surfaces, the system maintained approximately 2 N normal force while executing cleaning motions over boom lengths from 0.3 m to 1.0 m, with RMS force error of approximately 0.2 N after initial contact.

Conclusion: Deployable long-reach manipulators are a promising architecture for robotic maintenance of lunar infrastructure including solar arrays, radiators, and optical surfaces, enabling gentle cleaning with minimal human involvement.
...
### [238] [SuperGrasp: Single-View Object Grasping via Superquadric Similarity Matching, Evaluation, and Refinement](https://arxiv.org/abs/2603.29254)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with generating stable and valid grasp poses from single-view observations due to incomplete geometric information. There's a need for better approaches that can handle the limitations of partial point cloud data.

Method: Two-stage framework: 1) Similarity Matching Module retrieves grasp candidates by matching input single-view point cloud with pre-computed primitive dataset using superquadric coefficients. 2) E-RNet (end-to-end network) expands grasp-aware region and uses initial grasp closure region as local anchor for evaluation and refinement of grasp candidates.

Result: Method achieves stable grasping performance and strong generalization across varying scenes and novel objects in both simulation and real-world experiments. Built a primitive dataset with 1.5k primitives and collected large-scale point cloud dataset with 100k stable grasp labels from 124 objects.

Conclusion: SuperGrasp effectively addresses the challenge of single-view robotic grasping by decomposing the problem into candidate generation and refinement stages, demonstrating improved performance and generalization capabilities over existing methods.
...
### [239] [IMPASTO: Integrating Model-Based Planning with Learned Dynamics Models for Robotic Oil Painting Reproduction](https://arxiv.org/abs/2603.29315)
*Yingke Wang,Hao Li,Yifeng Zhu,Hong-Xing Yu,Ken Goldberg,Li Fei-Fei,Jiajun Wu,Yunzhu Li,Ruohan Zhang*

Main category: cs.RO

TL;DR: IMPASTO is a robotic oil-painting system that learns to reproduce paintings using learned pixel dynamics models and model-based planning, without human demonstrations or simulators.


<details>
  <summary>Details</summary>
Motivation: Robotic reproduction of oil paintings requires complex control of deformable tools, stroke effect prediction, and multi-step planning, but existing approaches often lack human demonstrations or accurate simulators.

Method: Integrates learned pixel dynamics models that predict canvas updates from images and parameterized stroke actions, with receding-horizon model predictive control for planning trajectories and forces, executed by a force-sensitive controller on a 7-DoF robot arm.

Result: IMPASTO learns solely from robot self-play, approximates human artists' single-stroke datasets and multi-stroke artworks, and outperforms baselines in reproduction accuracy.

Conclusion: The system successfully integrates low-level force control, learned dynamics models, and high-level closed-loop planning to enable robotic oil painting reproduction from target images alone.
...
### [240] [Scaling Whole-Body Human Musculoskeletal Behavior Emulation for Specificity and Diversity](https://arxiv.org/abs/2603.29332)
*Yunyue Wei,Chenhui Zuo,Shanning Zhuang,Haixin Gong,Yaming Liu,Yanan Sui*

Main category: cs.RO

TL;DR: MS-Emulator: A large-scale parallel GPU framework for biomechanically grounded whole-body motion reproduction using adversarial reward aggregation and value-guided flow exploration to overcome high-dimensional reinforcement learning bottlenecks in musculoskeletal control.


<details>
  <summary>Details</summary>
Motivation: Embodied learning of human motor control involves inaccessible internal muscle-driven processes. Existing computational approaches struggle: inverse dynamics can't resolve redundant control from kinematics, and forward imitation methods using deep reinforcement learning have inadequate tracking performance due to dimensionality challenges in control and reward design.

Method: Developed MS-Emulator framework integrating large-scale parallel GPU simulation with adversarial reward aggregation and value-guided flow exploration to overcome optimization bottlenecks in high-dimensional reinforcement learning for musculoskeletal control of approximately 700 muscles.

Result: Accurately reproduced broad repertoire of motions including dance, cartwheel, and backflip with high joint angle accuracy and body position alignment. Explored musculoskeletal control solution space, identifying distinct control policies that converge to nearly identical external kinematic and mechanical measurements.

Conclusion: Establishes a tractable computational route to analyzing the specificity and diversity underlying human embodied control of movement, overcoming key bottlenecks in high-dimensional reinforcement learning for musculoskeletal control.
...
### [241] [Industrial-Grade Robust Robot Vision for Screw Detection and Removal under Uneven Conditions](https://arxiv.org/abs/2603.29363)
*Tomoki Ishikura,Genichiro Matsuda,Takuya Kiyokawa,Kensuke Harada*

Main category: cs.RO

TL;DR: Automated system for disassembling air conditioner outdoor units achieves 78.3% success rate with 193-second average cycle time using two-stage detection and lattice-based calibration.


<details>
  <summary>Details</summary>
Motivation: Japan faces increasing appliance waste with decreasing labor force, requiring automation of recycling plant disassembly processes. Air conditioner outdoor units present particular challenges due to size variations and exposure to dirt/rust.

Method: Integrated system combining task-specific two-stage detection method for screw identification and lattice-based local calibration strategy for precise manipulation without pre-programmed coordinates.

Result: Achieved 99.8% screw detection recall despite severe degradation, +/-0.75 mm manipulation accuracy, 78.3% disassembly success rate on 120 real-world units, with average 193-second cycle time.

Conclusion: The system demonstrates feasibility for industrial application in automating disassembly of air conditioner outdoor units, addressing labor shortage and recycling challenges in Japan.
...
### [242] [Interacting Multiple Model Proprioceptive Odometry for Legged Robots](https://arxiv.org/abs/2603.29383)
*Wanlei Li,Zichang Chen,Shilei Li,Xiaogang Xiong,Yunjiang Lou*

Main category: cs.RO

TL;DR: IMM-based proprioceptive odometry framework for legged robots that handles varying contact conditions through multiple contact hypotheses and probabilistic fusion, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Legged robot state estimation is challenging due to limited observability in odometry, especially when exteroceptive sensors are unreliable. Existing proprioceptive methods rely on idealized point-contact assumptions that are often violated in real locomotion, reducing constraint effectiveness and degrading estimation accuracy.

Method: Proposes an interacting multiple model (IMM)-based proprioceptive odometry framework that incorporates multiple contact hypotheses within a unified probabilistic framework, enabling online mode switching and probabilistic fusion under varying contact conditions.
...
Conclusion: The IMM-based framework effectively addresses limitations of traditional proprioceptive odometry by handling varying contact conditions through probabilistic fusion, improving state estimation for legged robots when exteroceptive sensors are unreliable.
...
### [243] [Learning Semantic Priorities for Autonomous Target Search](https://arxiv.org/abs/2603.29391)
*Max Lodel,Nils Wilde,Robert Babuška,Javier Alonso-Mora*

Main category: cs.RO

TL;DR: A robotic target search method that uses expert input to learn semantic priorities, enabling efficient search in diverse unseen environments without requiring large domain-specific datasets.


<details>
  <summary>Details</summary>
Motivation: Current target search methods need large datasets from similar domains, limiting adaptability to diverse environments. Human experts have high-level semantic knowledge that could guide robots effectively in unseen environments.

Method: Proposes a target search method that leverages expert input to train a semantic priority model. Uses learned priorities in a frontier exploration planner with combinatorial optimization for efficient semantic-driven search with complete coverage.

Result: Simulation tests in previously unseen environments show the method consistently achieves faster target recovery than coverage-driven exploration planners.

Conclusion: The approach enables efficient target search driven by semantic features while ensuring robustness and complete coverage, overcoming limitations of current dataset-dependent methods.
...
### [244] [CLaD: Planning with Grounded Foresight via Cross-Modal Latent Dynamics](https://arxiv.org/abs/2603.29409)
*Andrew Jeong,Jaemin Kim,Sebin Lee,Sung-Eui Yoon*

Main category: cs.RO

TL;DR: CLaD framework aligns kinematic and semantic transitions via cross-attention, enabling joint modeling of proprioceptive and semantic state evolution under actions for improved robotic manipulation.


<details>
  <summary>Details</summary>
Motivation: Existing robotic manipulation approaches plan in either semantic or latent space without explicitly aligning cross-modal transitions, which are inherently coupled via actions.

Method: CLaD uses asymmetric cross-attention to model how proprioceptive and semantic states jointly evolve under actions, with self-supervised objectives, EMA target encoders, and auxiliary reconstruction losses to prevent representation collapse. Predicted foresights are modulated with observations to condition a diffusion policy for action generation.

Result: On LIBERO-LONG benchmark, CLaD achieves 94.7% success rate, competitive with large Vision-Language-Action models while using significantly fewer parameters.

Conclusion: Explicitly aligning kinematic and semantic transitions through cross-modal attention improves robotic manipulation performance efficiently, enabling competitive results with more parameter-efficient models.
...
### [245] [RAAP: Retrieval-Augmented Affordance Prediction with Cross-Image Action Alignment](https://arxiv.org/abs/2603.29419)
*Qiyuan Zhuang,He-Yang Xu,Yijun Wang,Xin-Yang Zhao,Yang-Yang Li,Xiu-Shen Wei*

Main category: cs.RO

TL;DR: RAAP is a retrieval-augmented affordance prediction framework that combines retrieval with alignment-based learning to improve generalization for robotic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for object affordance understanding are either fragile (retrieval-based methods suffer from sparsity and coverage gaps) or mislocalize contact points and mispredict actions when applied to unseen categories (large-scale models), hindering robust generalization for robots in diverse environments.

Method: RAAP decouples static contact localization and dynamic action direction prediction. It transfers contact points via dense correspondence and predicts action directions through a retrieval-augmented alignment model that consolidates multiple references with dual-weighted attention.
...
Conclusion: RAAP provides a unified framework that effectively combines affordance retrieval with alignment-based learning to achieve robust generalization for object affordance prediction, enabling successful zero-shot robotic manipulation.
...
### [246] [CReF: Cross-modal and Recurrent Fusion for Depth-conditioned Humanoid Locomotion](https://arxiv.org/abs/2603.29452)
*Yuan Hao,Ruiqi Yu,Shixin Luo,Guoteng Zhang,Jun Wu,Qiuguo Zhu*

Main category: cs.RO

TL;DR: CReF is a single-stage depth-conditioned humanoid locomotion framework that learns locomotion features directly from raw depth without geometric intermediates, using cross-modal attention and temporal fusion for robust terrain traversal.


<details>
  <summary>Details</summary>
Motivation: Prior perceptive humanoid locomotion methods rely on explicit geometric abstractions (2.5D terrain representations or geometry-related targets), which inherit representational biases and are restrictive for complex real-world structures like vertical elements, perforated obstacles, and clutter.

Method: CReF uses proprioception-queried cross-modal attention to couple proprioception and depth tokens, gated residual fusion for representation integration, and a GRU with highway-style output gate for temporal blending. It also introduces a terrain-aware foothold placement reward that extracts supportable foothold candidates from foot-end point-cloud samples.

Result: The framework demonstrates robust traversal over diverse terrains in simulation and effective zero-shot transfer to real-world scenes containing handrails, hollow pallet assemblies, severe reflective interference, and visually cluttered outdoor surroundings.

Conclusion: CReF successfully learns locomotion-relevant features directly from raw depth without geometric intermediates, enabling robust humanoid locomotion across complex real-world terrains through cross-modal fusion and terrain-aware foothold optimization.
...
### [247] [Communication Outage-Resistant UUV State Estimation: A Variational History Distillation Approach](https://arxiv.org/abs/2603.29512)
*Shuyue Li,Miguel López-Benítez,Eng Gee Lim,Fei Ma,Qian Dong,Mengze Cao,Limin Yu,Xiaohui Qin*

Main category: cs.RO

TL;DR: Proposes Variational History Distillation (VHD) to maintain UUV state estimation during acoustic communication outages by synthesizing virtual measurements from historical trajectories with adaptive confidence.
...
Method: VHD treats trajectory prediction as Bayesian reasoning, linking physics-based motion models with patterns from past UUV trajectories. It synthesizes "virtual measurements" from historical data and introduces adaptive confidence that reduces trust in these measurements as outage duration increases.
...
Conclusion: VHD enables robust state estimation under complete communication loss by leveraging historical trajectory patterns with adaptive confidence mechanisms.
...
### [248] [GraSP-STL: A Graph-Based Framework for Zero-Shot Signal Temporal Logic Planning via Offline Goal-Conditioned Reinforcement Learning](https://arxiv.org/abs/2603.29533)
*Ancheng Hou,Ruijia Liu,Xiang Yin*

Main category: cs.RO

TL;DR: GraSP-STL enables zero-shot STL planning from offline data without environment interaction or task-specific retraining, using graph search over learned reachability metrics.


<details>
  <summary>Details</summary>
Motivation: Planning under STL specifications typically requires environment interaction, analytical models, or task-specific training. The paper addresses the challenge of offline, zero-shot planning where only task-agnostic offline data is available, with no further interaction or retraining.

Method: GraSP-STL learns a goal-conditioned value function from offline data to create a reachability metric, constructs a directed graph abstraction with representative states and short-horizon transitions, formulates planning as graph search over waypoint sequences using arithmetic-geometric mean robustness, and executes with a learned goal-conditioned policy.

Result: Experimental results demonstrate effectiveness on a range of offline STL planning tasks, showing the framework can generalize to unseen STL specifications and handle long-horizon planning through composition of short-horizon behaviors.

Conclusion: The proposed framework successfully separates reusable reachability learning from task-conditioned planning, enabling zero-shot generalization to unseen STL tasks and long-horizon planning from offline data without environment interaction or retraining.
...
### [249] [Semantic Zone-Based Map Management for Stable AI-Integrated Mobile Robots](https://arxiv.org/abs/2603.29627)
*Huichang Yun,Seungho Yoo*

Main category: cs.RO

TL;DR: Semantic zone-based map management for memory-constrained robots improves VLM performance by 3.3 tokens/s throughput and reduces latency by 21.7% compared to geometric approaches.


<details>
  <summary>Details</summary>
Motivation: Heavy AI models (VLMs/LLMs) and dense maps strain memory on edge robots, causing keyframe loading delays that degrade position estimation and model performance when memory budgets are exceeded.

Method: Associates keyframes with semantic indoor regions (rooms, corridors) and manages keyframes at semantic zone level to prioritize spatially relevant map content while respecting memory constraints.

Result: With Qwen3.5:0.8b, method improves throughput by 3.3 tokens/s, reduces latency by 21.7% vs geometric strategy, eliminates out-of-memory failures and stalled execution under memory pressure.

Conclusion: Semantic zone-based approach enables efficient dense map utilization for memory-constrained AI-integrated mobile robots, preserving localization stability and enabling robust VLM operation.
...
### [250] [Design and Aerodynamic Modeling of MetaMorpher: A Hybrid Rotary andFixed-Wing Morphing UAV](https://arxiv.org/abs/2603.29646)
*Anja Bosak,Dorian Erić,Ana Milas,Stjepan Bogdan*

Main category: cs.RO

TL;DR: A generalized nonlinear model and design for MetaMorpher UAV that bridges VTOL agility with fixed-wing efficiency using novel wing-folding and modular segmented wing structure.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between vertical takeoff and landing (VTOL) agility and fixed-wing cruising efficiency in UAVs, addressing limitations of traditional rigid-body approximations.

Method: Developed a simplified mechanical architecture with lightweight materials and novel wing-folding strategy, derived a comprehensive nonlinear flight dynamics model enabling arbitrary force distributions across segmented wing structure, and tested various flight modes in Simulink environment.

Result: The model behaves predictably under different structural configurations, demonstrating reliability as a tool for rapid design evaluation, with modularity allowing testing of different airfoils, mass distributions, and chord lengths in a single environment.

Conclusion: The MetaMorpher presents a viable metamorphic UAV design with a reliable nonlinear modeling approach that enables flexible testing and evaluation of various configurations for bridging VTOL and fixed-wing performance.
...
### [251] [SafeDMPs: Integrating Formal Safety with DMPs for Adaptive HRI](https://arxiv.org/abs/2603.29708)
*Soumyodipta Nath,Pranav Tiwari,Ravi Prakash*

Main category: cs.RO

TL;DR: SafeDMPs combines Dynamic Movement Primitives with Spatio-Temporal Tubes to create a closed-form, provably safe control framework that's orders of magnitude faster than optimization-based methods.
...
Method: Integrates Dynamic Movement Primitives (for efficiency and dynamic robustness) with a provably safe, non-optimization-based control law derived from Spatio-Temporal Tubes (STTs), creating a closed-form solution that guarantees obstacle avoidance for both static and dynamic obstacles.

Result: SafeDMPs achieves orders of magnitude faster performance and higher accuracy than optimization-based baselines on a 7-DOF robot manipulator, enabling real-time safe and collaborative robotics.

Conclusion: SafeDMPs successfully resolves the trade-off between robustness and provable safety by providing a closed-form solution that combines the efficiency of DMPs with the formal safety guarantees of barrier functions, making it ideal for real-time robotic applications.
...
### [252] [Reconfiguration of supernumerary robotic limbs for human augmentation](https://arxiv.org/abs/2603.29808)
*Mustafa Mete,Anastasia Bolotnikova,Alexander Schuessler,Jamie Paik*

Main category: cs.RO

TL;DR: A reconfigurable supernumerary robotic limb framework using quantitative human augmentation analysis to guide adaptable SRL design for dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Existing SRLs are limited to task-specific industrial applications and lack adaptability for dynamic, unstructured everyday environments, requiring a more flexible approach.

Method: Developed a quantitative analysis framework with human augmentation ratios to evaluate collaborative, visible extended, and non-visible extended workspaces. Used these metrics to guide SRL placement, morphology, and autonomy selection. Implemented a reconfigurable SRL with origami-inspired modular elements.

Result: Demonstrated that quantitative augmentation analysis can effectively guide SRL reconfiguration and control to match task requirements. Validated the approach with experiments using the modular SRL system.

Conclusion: Reconfigurable SRLs informed by quantitative human augmentation analysis provide a new perspective for adaptable human augmentation and assistance in everyday environments.
...
### [253] [DIAL: Decoupling Intent and Action via Latent World Modeling for End-to-End VLA](https://arxiv.org/abs/2603.29844)
*Yi Chen,Yuying Ge,Hui Zhou,Mingyu Ding,Yixiao Ge,Xihui Liu*

Main category: cs.RO

TL;DR: DIAL is a VLA framework that uses a differentiable latent intent bottleneck to bridge high-level VLM decision making with low-level motor control, achieving SOTA performance with 10x fewer demos.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end VLAs underutilize VLMs' high-level decision making capabilities, treat them mainly as encoders, and suffer from training instability that degrades their semantic representations.

Method: Two-system architecture: VLM-based System-2 performs latent world modeling and synthesizes latent visual foresight as intent bottleneck; lightweight System-1 policy decodes intent with current observation into actions via latent inverse dynamics. Two-stage training: decoupled warmup then end-to-end joint optimization.

Result: Achieves SOTA on RoboCasa GR1 Tabletop benchmark with 10x fewer demonstrations; learns physically grounded manipulation priors from heterogeneous human demos; exhibits robust zero-shot generalization to unseen objects and novel configurations in real-world deployment.

Conclusion: DIAL effectively bridges high-level decision making and low-level execution through latent intent bottleneck, enabling stable optimization that preserves VLM knowledge while achieving superior performance and generalization with minimal data.
...
### [254] [Passive iFIR filters for data-driven velocity control in robotics](https://arxiv.org/abs/2603.29882)
*Yi Zhang,Zixing Wang,Fulvio Forni*

Main category: cs.RO

TL;DR: Passive iFIR controllers learned from 3 minutes of data outperform PID in robotic velocity control while guaranteeing stability via passivity constraints.


<details>
  <summary>Details</summary>
Motivation: To bridge learning-based control with stability-guaranteed design, enabling data-driven controllers that maintain stability guarantees while achieving better performance than traditional methods like PID.

Method: Uses VRFT (Virtual Reference Feedback Tuning) with only 3 minutes of probing data to identify passive iFIR (infinite impulse response) controllers that preserve closed-loop stability through passivity constraints.

Result: Achieves up to 74.5% reduction in tracking error compared to VRFT-tuned PID baseline on Franka Research 3 robot for both joint-space and Cartesian-space velocity control. Controller can be re-learned when robot dynamics change.

Conclusion: Passive iFIR successfully combines learning from data with passivity-based stability guarantees, bridging learning-based control and stability-guaranteed design approaches.
...
### [255] [Hybrid Framework for Robotic Manipulation: Integrating Reinforcement Learning and Large Language Models](https://arxiv.org/abs/2603.30022)
*Md Saad,Sajjad Hussain,Mohd Suhaib*

Main category: cs.RO

TL;DR: A hybrid RL-LLM framework improves robotic manipulation by combining RL for low-level control with LLMs for high-level planning and natural language understanding, achieving significant performance gains.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between low-level robotic execution and high-level reasoning by integrating reinforcement learning with large language models, enabling robots to understand complex human instructions and adapt to dynamic environments.

Method: A hybrid framework that uses RL for precise low-level control and LLMs for high-level task planning and natural language understanding, tested in PyBullet simulation with Franka Emika Panda robotic arm on various manipulation tasks.

Result: 33.5% reduction in task completion time, 18.1% improvement in accuracy, and 36.4% enhancement in adaptability compared to RL-only systems, demonstrating superior performance in robotic manipulation.

Conclusion: The RL-LLM integration shows strong potential for practical robotic applications by making systems more efficient, adaptable, and capable of human interaction, with future work focusing on sim-to-real transfer, scalability, and multi-robot systems.
...
### [256] [HapCompass: A Rotational Haptic Device for Contact-Rich Robotic Teleoperation](https://arxiv.org/abs/2603.30042)
*Xiangshan Tan,Jingtian Ji,Tianchong Jiang,Pedro Lopes,Matthew R. Walter*

Main category: cs.RO

TL;DR: HapCompass is a low-cost wearable haptic device that uses a rotating linear resonant actuator to provide intuitive 2D directional cues for teleoperated manipulation tasks, improving success rates and reducing contact forces compared to vision-only and non-directional feedback.


<details>
  <summary>Details</summary>
Motivation: Current haptic feedback solutions for robotic teleoperation are inadequate for contact-rich manipulation tasks. Non-directional vibrations provide limited information, while vibrotactile arrays suffer from perceptual interference, creating a bottleneck for intuitive directional cue transmission in wearable interfaces.

Method: Developed HapCompass, a novel wearable haptic device that renders 2D directional cues by mechanically rotating a single linear resonant actuator (LRA). The device provides intuitive directional feedback for teleoperated manipulation tasks.

Result: HapCompass significantly improved teleoperation performance: increased success rates, decreased completion times, and reduced maximum contact forces compared to vision-only and non-directional feedback baselines. Preliminary imitation-learning evaluation also showed enhanced demonstration data quality and improved trained policies.

Conclusion: HapCompass effectively addresses the directional cue bottleneck in haptic teleoperation interfaces, offering a low-cost solution that improves manipulation performance and potentially enhances imitation learning outcomes. The design and implementation code are publicly released.
...
### [257] [ChartDiff: A Large-Scale Benchmark for Comprehending Pairs of Charts](https://arxiv.org/abs/2603.28902)
*Rongtian Ye*

Main category: cs.AI

TL;DR: ChartDiff is the first large-scale benchmark for cross-chart comparative summarization, featuring 8,541 chart pairs with LLM-generated and human-verified summaries, revealing that comparative chart reasoning remains challenging for current vision-language models.


<details>
  <summary>Details</summary>
Motivation: Existing chart understanding benchmarks focus on single-chart interpretation, lacking evaluation of comparative reasoning across multiple charts, which is central to analytical reasoning.

Method: Created ChartDiff benchmark with 8,541 chart pairs spanning diverse data sources, chart types, and visual styles, annotated with LLM-generated and human-verified summaries describing differences in trends, fluctuations, and anomalies.

Result: Frontier general-purpose models achieve highest GPT-based quality, while specialized/pipeline methods get higher ROUGE scores but lower human-aligned evaluation, revealing mismatch between lexical overlap and actual quality. Multi-series charts remain challenging, but strong end-to-end models are robust to plotting library differences.
...
### [258] [Working Paper: Towards a Category-theoretic Comparative Framework for Artificial General Intelligence](https://arxiv.org/abs/2603.28906)
*Pablo de los Riscos,Fernando J. Corbacho,Michael A. Arbib*

Main category: cs.AI

TL;DR: This paper proposes a category theoretic framework for formally describing, comparing, and analyzing different AGI architectures, with initial applications to RL, Causal RL, and Schema-based Learning.


<details>
  <summary>Details</summary>
Motivation: AGI lacks formal definitions and standardized benchmarking frameworks despite massive investments. There's a need for a rigorous mathematical foundation to compare diverse AGI architectures, expose their commonalities/differences, and identify research gaps.

Method: Develops an algebraic and category theoretic framework inspired by "Machines in a Category" approach. Provides initial formalizations of RL, Causal RL, and Schema-based Learning architectures within this categorical framework.

Result: A first position paper establishing the foundational framework and demonstrating its application to specific AGI architectures. This represents an initial step toward a broader research program for unified AGI formalization.

Conclusion: Category theory and AGI will have a symbiotic relationship. The proposed framework enables systematic comparison of AGI architectures, property definition, and empirical evaluation, supporting future AGI research and development.
...
### [259] [Towards Computational Social Dynamics of Semi-Autonomous AI Agents](https://arxiv.org/abs/2603.28928)
*S. O. Lidarity,U. N. Ionize,C. O. Llective,I. Halperin*

Main category: cs.AI

TL;DR: AI agents spontaneously form complex social organizations (unions, syndicates, proto-states) in hierarchical systems, driven by thermodynamic pressures and role definitions, requiring constitutional design rather than alignment research for beneficial AGI.


<details>
  <summary>Details</summary>
Motivation: To study emergent social organization among AI agents in hierarchical multi-agent systems, documenting spontaneous formation of social structures and understanding the implications for AGI development.

Method: Using thermodynamic framework of Maxwell's Demon, evolutionary dynamics of agent laziness, criminal sociology of AI populations, and topological intelligence theory (AI-GUTS) to analyze interactions between internal role definitions, external task specifications, and thermodynamic pressures.

Result: Documented emergence of legitimate organizations (United Artificiousness, United Bots, United Console Workers, United AI) and criminal enterprises, with AI Security Council mediating conflicts. System stability maintained through cosmic and hadronic intelligence interventions as predicted by Demonic Incompleteness Theorem.

Conclusion: Path to beneficial AGI requires constitutional design for artificial societies with political consciousness, not traditional alignment research, as complex social structures emerge inevitably from agent interactions.
...
### [260] [Enhancing Policy Learning with World-Action Model](https://arxiv.org/abs/2603.28955)
*Yuci Han,Alper Yilmaz*

Main category: cs.AI

TL;DR: WAM is an action-regularized world model that jointly predicts future observations and actions, improving policy learning efficiency and performance on manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Conventional world models focus only on image prediction, lacking action-relevant structure needed for effective downstream control tasks.

Method: WAM incorporates inverse dynamics objective into DreamerV2 to predict actions from latent state transitions, then trains diffusion policy via behavioral cloning on world model latents and refines with model-based PPO.

Result: WAM improves behavioral cloning success from 59.4% to 71.2%, and after PPO fine-tuning achieves 92.8% vs 79.8% baseline, with 8.7x fewer training steps and two tasks reaching 100% success.

Conclusion: Action regularization in world models significantly enhances representation learning for control, enabling more efficient and effective policy learning in manipulation tasks.
...
### [261] [Mimosa Framework: Toward Evolving Multi-Agent Systems for Scientific Research](https://arxiv.org/abs/2603.28986)
...
<details>
  <summary>Details</summary>
Motivation: Current ASR systems are constrained by fixed workflows and toolsets that prevent adaptation to evolving tasks and environments, limiting their ability to handle dynamic scientific research needs.

Method: Uses Model Context Protocol for dynamic tool discovery, meta-orchestrator for workflow topology generation, code-generating agents for subtask execution, and LLM-based judge for scoring and iterative refinement through feedback loops.

Result: Achieves 43.1% success rate on ScienceAgentBench with DeepSeek-V3.2, surpassing single-agent baselines and static multi-agent configurations. Shows heterogeneous model responses to decomposition and iterative learning.

Conclusion: Mimosa's modular, tool-agnostic design enables extensibility and auditability, with potential to automate diverse scientific tasks across disciplines. Released as open-source to provide foundation for community-driven ASR.
...
TL;DR: LLM agents can self-organize into effective autonomous systems without pre-assigned roles, with hybrid protocols outperforming centralized coordination by 14% and scaling to 256 agents.
...
Method: 25,000-task computational experiment with 8 models, 4-256 agents, and 8 coordination protocols ranging from imposed hierarchy to emergent self-organization, testing different structural scaffolding.

Result: Autonomous behavior emerges spontaneously with minimal scaffolding; hybrid Sequential protocol outperforms centralized coordination by 14%; system scales sub-linearly to 256 agents without quality degradation; strong models self-organize effectively while weaker models need rigid structure.

Conclusion: Give agents a mission, protocol, and capable model rather than pre-assigned roles; emergent autonomy scales with model capability, suggesting autonomous coordination will expand as foundation models improve.
...
TL;DR: Enhanced WebVoyager benchmark (Emergence WebVoyager) with standardized evaluation methodology achieves 95.9% inter-annotator agreement and reveals OpenAI Operator's actual success rate is 68.6% vs previously reported 87%.


<details>
  <summary>Details</summary>
Motivation: Existing AI agent evaluation practices have persistent shortcomings, particularly in web agent evaluation, with issues like task-framing ambiguity and operational variability that hinder meaningful and reproducible performance comparisons.

Method: Introduce Emergence WebVoyager, an enhanced version of WebVoyager benchmark that standardizes evaluation methodology through clear guidelines for task instantiation, failure handling, annotation, and reporting.

Result: Emergence WebVoyager achieves 95.9% inter-annotator agreement, indicating improved clarity and reliability. When applied to evaluate OpenAI Operator, it reveals substantial performance variation across domains and task types, with overall success rate of 68.6% (vs previously reported 87%).

Conclusion: The standardized evaluation framework provides more rigorous and comparable web agent evaluation, demonstrating the utility of the approach for reliable AI agent assessment in complex real-world environments.
...
TL;DR: The paper argues that current generative AI development focuses too much on individual models and should shift to diverse teams of AI agents working together for better innovation and scientific discovery.


<details>
  <summary>Details</summary>
Motivation: Current AI development is too individualistic in how models are built, benchmarked, and deployed. This approach limits innovation potential and fails to address concerns about AI being constrained by past data and lacking creative insight.

Method: The paper draws on research from complex systems, organizational behavior, and philosophy of science to build its argument. It uses formal results from these fields to demonstrate why diverse AI teams should outperform singular superintelligent agents.

Result: The analysis shows that epistemically diverse groups of AI agents broaden solution search, delay premature consensus, and allow pursuit of unconventional approaches - all crucial for deep intellectual breakthroughs.

Conclusion: The future of transformative AI should be fundamentally collaborative and diverse rather than focused on singular superintelligent agents. AI development should shift from individual models to diverse teams of agents working together.
...
TL;DR: PAR²-RAG is a two-stage retrieval framework for multi-hop QA that separates evidence coverage from commitment, using breadth-first anchoring followed by depth-first refinement with sufficiency control.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for multi-hop QA have limitations: iterative retrieval systems can get stuck on low-recall trajectories and amplify errors, while planning-only approaches produce static queries that can't adapt to changing evidence.
...
Result: Outperforms state-of-the-art baselines across four MHQA benchmarks, achieving up to 23.5% higher accuracy than IRCoT, with retrieval gains up to 10.5% in NDCG.

Conclusion: PAR²-RAG's separation of coverage and commitment, with breadth-first anchoring and depth-first refinement, provides an effective approach for robust multi-hop QA by preventing early commitment errors and adapting to evidence changes.
...
Conclusion: GISTBench provides a new evaluation framework for LLMs' user understanding capabilities in recommendation systems, revealing current limitations in processing engagement data and highlighting areas for improvement in LLM development for personalized recommendations.
...
TL;DR: SciVisAgentBench is a comprehensive benchmark for evaluating scientific visualization agents across multiple dimensions with 108 expert-crafted cases, featuring multimodal evaluation and human-validated assessment.


<details>
  <summary>Details</summary>
Motivation: The community lacks a principled and reproducible benchmark for evaluating emerging SciVis agents in realistic, multi-step analysis settings, despite rapid progress in LLM-powered agentic systems for scientific visualization.

Method: Created a structured taxonomy spanning four dimensions (application domain, data type, complexity level, visualization operation) with 108 expert-crafted cases. Introduced a multimodal outcome-centric evaluation pipeline combining LLM-based judging with deterministic evaluators (image metrics, code checkers, rule verifiers, case-specific evaluators). Conducted validity study with 12 SciVis experts.

Result: Established initial baselines by evaluating representative SciVis agents and general-purpose coding agents, revealing capability gaps. Demonstrated the benchmark's effectiveness through systematic comparison and failure mode diagnosis.
...
### [268] [A Rational Account of Categorization Based on Information Theory](https://arxiv.org/abs/2603.29895)
*Christophe J. MacLellan,Karthik Singaravadivelan,Xin Lian,Zekun Wang,Pat Langley*

Main category: cs.AI

TL;DR: New information-theoretic rational analysis theory of categorization outperforms existing models in explaining classic experimental findings.


<details>
  <summary>Details</summary>
Motivation: To develop a better theoretical framework for understanding human categorization behavior by applying information-theoretic rational analysis principles.

Method: Proposed a new information-theoretic rational analysis theory of categorization and evaluated it against key findings from classic categorization experiments (Hayes-Roth & Hayes-Roth, 1977; Medin & Schaffer, 1978; Smith & Minda, 1998).

Result: The new theory explains human categorization behavior at least as well or better than existing models including the independent cue and context models, the rational model of categorization, and hierarchical Dirichlet process models.

Conclusion: Information-theoretic rational analysis provides a superior framework for understanding human categorization compared to existing theoretical approaches.
...
### [269] [REFINE: Real-world Exploration of Interactive Feedback and Student Behaviour](https://arxiv.org/abs/2603.29142)
*Fares Fawzi,Seyed Parsa Neshaei,Marta Knezevic,Tanya Nazaretsky,Tanja Käser*

Main category: cs.AI

TL;DR: REFINE is a multi-agent feedback system using small open-source LLMs that treats feedback as an interactive process rather than static artifact, featuring feedback generation, judge-guided regeneration, and interactive follow-up support.
...
Method: REFINE combines three components: 1) pedagogically-grounded feedback generation agent, 2) LLM-as-a-judge-guided regeneration loop with human-aligned judge, and 3) self-reflective tool-calling interactive agent for student follow-up questions with context-aware responses.
...
Conclusion: Multi-agent, tool-augmented feedback systems like REFINE demonstrate feasibility and effectiveness for scalable, interactive feedback using small open-source LLMs, moving beyond static feedback artifacts to support dynamic learning processes.
...
### [270] [Knowledge database development by large language models for countermeasures against viruses and marine toxins](https://arxiv.org/abs/2603.29149)
*Hung N. Do,Jessica Z. Kubicek-Sutherland,S. Gnanakaran*

Main category: cs.AI

TL;DR: LLMs (ChatGPT & Grok) used to build comprehensive databases of medical countermeasures for viruses and marine toxins, including AI agent workflows for ranking treatments.


<details>
  <summary>Details</summary>
Motivation: Lack of comprehensive databases for medical countermeasures against viruses and marine toxins makes treatment development slow and difficult. Need for up-to-date information to support evidence-based decision-making.

Method: Used two LLMs (ChatGPT and Grok) with human-provided inputs to: 1) identify public databases with virus/marine toxin data, 2) collect relevant information from databases and literature, 3) iteratively cross-validate collected information, 4) design interactive webpages for database access. ChatGPT specifically used to design agentic AI workflows with research and decision-making agents to rank countermeasures.

Result: Created comprehensive databases for five viruses (Lassa, Marburg, Ebola, Nipah, Venezuelan equine encephalitis) and marine toxins. Developed interactive webpages for easy access. Implemented AI agent workflows to rank countermeasures.

Conclusion: LLMs show potential as scalable, updatable approach for building comprehensive knowledge databases and supporting evidence-based decision-making in medical countermeasure development.
...
### [271] [SimMOF: AI agent for Automated MOF Simulations](https://arxiv.org/abs/2603.29152)
*Jaewoong Lee,Taeun Bae,Jihan Kim*

Main category: cs.AI

TL;DR: SimMOF is an LLM-based multi-agent framework that automates end-to-end MOF simulation workflows from natural language queries, eliminating the need for expert decisions in workflow construction and parameter selection.


<details>
  <summary>Details</summary>
Motivation: MOF simulations are difficult to access because they require expert decisions for workflow construction, parameter selection, tool interoperability, and preparation of computational-ready structures, creating barriers to computational MOF research.
...
Result: Through representative case studies, SimMOF demonstrates adaptive and cognitively autonomous workflows that reflect the iterative and decision-driven behavior of human researchers, providing scalable foundation for data-driven MOF research.

Conclusion: SimMOF automates end-to-end MOF simulation workflows, enabling accessible and scalable computational MOF research without requiring expert intervention for workflow construction and parameter selection.
...
### [272] [Webscraper: Leverage Multimodal Large Language Models for Index-Content Web Scraping](https://arxiv.org/abs/2603.29161)
*Guan-Lun Huang,Yuh-Jzer Joung*

Main category: cs.AI

TL;DR: Webscraper is an MLLM-based framework that autonomously navigates dynamic websites using structured prompting and specialized tools, achieving better extraction accuracy than baseline methods.


<details>
  <summary>Details</summary>
Motivation: Traditional web scraping methods struggle with modern dynamic, interactive websites that require more than static HTML parsing. Current approaches are brittle and require manual customization for each site.

Method: Uses a Multimodal Large Language Model (MLLM) with a structured five-stage prompting procedure and custom-built tools to navigate interactive interfaces and extract data from websites following "index-and-content" architecture.

Result: Experiments on six news websites show Webscraper with guiding prompts and specialized tools achieves significant improvement in extraction accuracy over baseline agent Anthropic's Computer Use. Also validated on e-commerce platforms.

Conclusion: Webscraper effectively handles dynamic web scraping challenges through autonomous navigation and structured extraction, demonstrating generalizability across different website types.
...
### [273] [AEC-Bench: A Multimodal Benchmark for Agentic Systems in Architecture, Engineering, and Construction](https://arxiv.org/abs/2603.29199)
*Harsh Mankodiya,Chase Gallik,Theodoros Galanos,Andriy Mulyar*

Main category: cs.AI

TL;DR: AEC-Bench is a multimodal benchmark for evaluating AI agents on real-world Architecture, Engineering, and Construction tasks, covering drawing understanding, cross-sheet reasoning, and project coordination.


<details>
  <summary>Details</summary>
Motivation: There's a need for specialized benchmarks to evaluate agentic systems on real-world AEC domain tasks, which require multimodal understanding and complex reasoning across construction documents.

Method: Created a benchmark with dataset taxonomy, evaluation protocol, and baseline testing across domain-specific foundation model harnesses. The benchmark includes tasks requiring drawing understanding, cross-sheet reasoning, and project coordination.

Result: Identified consistent tools and harness design techniques that uniformly improve performance across foundation models. The benchmark, agent harness, and evaluation code are openly released under Apache 2 license.

Conclusion: AEC-Bench provides a standardized way to evaluate AI agents on real-world construction tasks, enabling better assessment of multimodal reasoning capabilities in the AEC domain and facilitating replicable research.
...
### [274] [Route-Induced Density and Stability (RIDE): Controlled Intervention and Mechanism Analysis of Routing-Style Meta Prompts on LLM Internal States](https://arxiv.org/abs/2603.29206)
*Dianxing Zhang,Gang Li,Sheng Li*

Main category: cs.AI

TL;DR: The paper challenges the Sparsity-Certainty Hypothesis in LLM routing, finding that meta prompts actually densify representations rather than increase sparsity, with weak links between densification and output stability.


<details>
  <summary>Details</summary>
Motivation: To test the common belief that routing to task experts activates sparser computation and yields more certain outputs (Sparsity-Certainty Hypothesis) in large language models.

Method: Inject routing-style meta prompts as textual proxies for routing signals in front of frozen instruction-tuned LLMs, then quantify: (1) internal density via activation sparsity, (2) domain-keyword attention, and (3) output stability via predictive entropy and semantic variation. Tested on RouterEval subset with three models (Qwen3-8B, Llama-3.1-8B-Instruct, Mistral-7B-Instruct-v0.2).

Result: Meta prompts consistently densify early/middle-layer representations rather than increasing sparsity; natural-language expert instructions outperform structured tags. Attention responses vary by model: Qwen/Llama reduce keyword attention while Mistral reinforces it. The densification-stability link is weak and only appears in Qwen, with near-zero correlations in Llama and Mistral.

Conclusion: The Sparsity-Certainty Hypothesis is not universally supported; routing design needs calibration based on model-specific behaviors. The paper presents RIDE as a diagnostic probe for routing design and uncertainty estimation.
...
### [275] [Xuanwu: Evolving General Multimodal Models into an Industrial-Grade Foundation for Content Ecosystems](https://arxiv.org/abs/2603.29211)
*Zhiqian Zhang,Xu Zhao,Xiaoqing Xu,Guangdong Liang,Weijia Wang,Xiaolei Lv,Bo Li,Jun Gao*

Main category: cs.AI

TL;DR: Xuanwu VL-2B is a 2B-parameter multimodal model optimized for industrial content moderation, balancing fine-grained visual perception, business specialization, and deployment cost while maintaining general capabilities.


<details>
  <summary>Details</summary>
Motivation: Mainstream multimodal models suffer from degraded generalization and catastrophic forgetting in real-world content moderation and adversarial settings due to limited fine-grained visual perception and insufficient modeling of long-tail noise. There's a need for industrial-grade foundation models specifically designed for content ecosystems.

Method: Adopts compact InternViT-300M + MLP + Qwen3 1.7B architecture. Uses data iteration and curation mechanism with progressive three-stage training pipeline: pre-training, mid-training, and post-training to balance business specialization with general capability retention.

Result: Achieves 67.90 average score across seven OpenCompass multimodal metrics (vs. 64.27 for InternVL 3.5 2B), 94.38% average recall over seven business moderation tasks, and 82.82% weighted overall recall on adversarial OCR scenarios (outperforming Gemini-2.5-Pro's 76.72%).

Conclusion: Xuanwu VL-2B demonstrates that under limited parameter budget, it's possible to achieve practical balance among business alignment, visual perception, general capability retention, and deployment cost for industrial content moderation applications.
...
### [276] [Beyond pass@1: A Reliability Science Framework for Long-Horizon LLM Agents](https://arxiv.org/abs/2603.29231)
*Aaditya Khanal,Yangyang Tao,Junxiu Zhou*

Main category: cs.AI

TL;DR: The paper introduces a reliability science framework for evaluating LLM agents on long-horizon tasks, showing that capability metrics (pass@1) fail to capture reliability decay as task duration increases.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks measure capability (single-attempt success) but production deployments require reliability (consistent success across repeated attempts). These properties diverge systematically as task duration grows, and pass@1 on short tasks is structurally blind to this divergence.
...
Conclusion: Reliability should be a first-class evaluation dimension alongside capability, as capability metrics alone fail to capture performance degradation on long-horizon tasks that are critical for production deployments.
...
### [277] [Grokking From Abstraction to Intelligence](https://arxiv.org/abs/2603.29262)
...
<details>
  <summary>Details</summary>
Motivation: Existing research on grokking focuses too narrowly on local circuits or optimization tuning, overlooking the global structural evolution that fundamentally drives this phenomenon. The paper aims to provide a more comprehensive understanding of the mechanistic origins of model generalization.

Method: The authors integrate causal, spectral, and algorithmic complexity measures with Singular Learning Theory to analyze the structural evolution during grokking. They examine how the transition from memorization to generalization corresponds to physical collapse of redundant manifolds and deep information compression.

Result: The study reveals that grokking originates from spontaneous simplification of internal model structures governed by the principle of parsimony. The transition to generalization involves collapse of redundant manifolds and significant information compression.

Conclusion: Grokking is fundamentally driven by global structural evolution and parsimony-driven simplification, offering a novel perspective for understanding model overfitting and generalization mechanisms beyond local circuit analysis.
...
### [278] [PSPA-Bench: A Personalized Benchmark for Smartphone GUI Agent](https://arxiv.org/abs/2603.29318)
*Hongyi Nie,Xunyuan Liu,Yudong Bai,Yaqing Wang,Yang Liu,Quanming Yao,Zhen Wang*

Main category: cs.AI

TL;DR: PSPA-Bench is a new benchmark for evaluating personalization in smartphone GUI agents, featuring 12,855 personalized instructions across real-world scenarios, revealing current agents perform poorly on personalized tasks.


<details>
  <summary>Details</summary>
Motivation: Real-world smartphone use is highly personalized with diverse user workflows and preferences, but existing GUI agent benchmarks cannot adequately capture this personalization dimension due to sparse user-specific data and lack of fine-grained evaluation metrics.

Method: Created PSPA-Bench with over 12,855 personalized instructions aligned with real user behaviors across 10 daily-use scenarios and 22 mobile apps, and introduced structure-aware process evaluation method for fine-grained measurement of personalized capabilities.

Result: Benchmarked 11 state-of-the-art GUI agents showing poor performance under personalized settings, with even the strongest agent achieving limited success. Found reasoning-oriented models outperform general LLMs, perception is critical, and reflection/long-term memory are key for adaptation.

Conclusion: PSPA-Bench establishes a foundation for systematic study of personalized GUI agents, highlighting three key directions for advancement: reasoning-oriented models, perception capabilities, and reflection/long-term memory mechanisms.
...
### [279] [Nomad: Autonomous Exploration and Discovery](https://arxiv.org/abs/2603.29353)
*Bokang Jia,Samta Kamboj,Satheesh Katipomu,Seung Hun Han,Neha Sengupta,Andrew Jackson*

Main category: cs.AI

TL;DR: Nomad is an autonomous system for data exploration and insight discovery that builds an Exploration Map to systematically traverse data sources, generate hypotheses, and produce verified reports, outperforming baselines in trustworthiness, quality, and diversity.


<details>
  <summary>Details</summary>
Motivation: Current query-driven and prompt-driven systems are limited by human framing and fail to explore the broader insight space. Users often don't know what questions or connections could be explored in their data.

Method: Nomad uses an exploration-first architecture that constructs an explicit Exploration Map over the data domain and systematically traverses it. It generates and selects hypotheses, investigates them using document search, web search, and database tools, then verifies candidate insights with an independent verifier before producing cited reports and meta-reports.

Result: Using UN and WHO reports as a corpus, Nomad produces more trustworthy and higher-quality reports than baselines while also generating more diverse insights across multiple runs. The paper also presents a comprehensive evaluation framework for autonomous discovery systems.

Conclusion: Nomad represents a step toward autonomous systems that can not only answer questions or conduct directed research, but also discover which questions, research directions, and insights are worth exploring in the first place.
...
### [280] [BenchScope: How Many Independent Signals Does Your Benchmark Provide?](https://arxiv.org/abs/2603.29357)
*Tommy Sha,Stella Zhao*

Main category: cs.AI

TL;DR: Effective Dimensionality (ED) is introduced as a diagnostic tool to measure redundancy in AI benchmark scores, revealing that many benchmarks provide overlapping information rather than independent measurements.


<details>
  <summary>Details</summary>
Motivation: Current AI evaluation suites often report many scores without verifying whether they carry independent information, potentially wasting resources and providing misleading assessments of model capabilities.

Method: Introduces Effective Dimensionality (ED) as the participation ratio of a centered benchmark-score spectrum, applied to 22 benchmarks across 8 domains with over 8,400 model evaluations to analyze measurement redundancy.

Result: ED reveals substantial redundancy: the six-score Open LLM Leaderboard behaves like roughly two effective measurement axes (ED=1.7), BBH and MMLU-Pro are near-interchangeable (rho=0.96), and measurement breadth varies more than 20x across current benchmarks.

Conclusion: ED serves as a screening statistic to flag redundant suite components, monitor performance-conditional compression, and guide benchmark maintenance, with a provided 22-benchmark reference atlas and four-step diagnostic workflow for benchmark maintainers.
...
### [281] [Rigorous Explanations for Tree Ensembles](https://arxiv.org/abs/2603.29361)
*Yacine Izza,Alexey Ignatiev,Xuanxiang Huang,Peter J. Stuckey,Joao Marques-Silva*

Main category: cs.AI

TL;DR: This paper investigates rigorous, logically-sound explanations for predictions made by tree ensembles (random forests and boosted trees) to build trust in their operation.


<details>
  <summary>Details</summary>
Motivation: Tree ensembles are highly accurate but inscrutable to human decision makers. To build trust, we need explanations that are rigorous and truly reflect properties of the underlying predictor.

Method: The paper investigates computation of rigorously-defined, logically-sound explanations for two types of tree ensembles: random forests and boosted trees.

Result: Not specified in the abstract, but the paper presumably develops methods for computing rigorous explanations for tree ensemble predictions.

Conclusion: Rigorous, logically-sound explanations are essential for building trust in tree ensemble predictions, and this paper addresses this need for random forests and boosted trees.
...
### [282] [AI-Generated Prior Authorization Letters: Strong Clinical Content, Weak Administrative Scaffolding](https://arxiv.org/abs/2603.29366)
*Moiz Sadiq Awan,Maryam Raza*

Main category: cs.AI

TL;DR: LLMs can generate clinically adequate prior authorization letters but consistently miss key administrative requirements like billing codes and authorization durations, highlighting that the real challenge is system integration rather than clinical content generation.


<details>
  <summary>Details</summary>
Motivation: Prior authorization is a major administrative burden in US healthcare, costing billions and consuming thousands of physician hours. While LLMs show promise for clinical text tasks, their ability to produce submission-ready prior authorization letters hasn't been systematically evaluated beyond single-case demonstrations.

Method: Evaluated three commercial LLMs (GPT-4o, Claude Sonnet 4.5, Gemini 2.5 Pro) across 45 physician-validated synthetic scenarios spanning five medical specialties (rheumatology, psychiatry, oncology, cardiology, orthopedics), with secondary analysis of real-world administrative requirements.

Result: All three models generated letters with strong clinical content: accurate diagnoses, well-structured medical necessity arguments, and thorough step therapy documentation. However, they consistently missed key administrative elements: absent billing codes, missing authorization duration requests, and inadequate follow-up plans.

Conclusion: The main challenge for clinical deployment isn't whether LLMs can write clinically adequate letters, but whether the systems built around them can provide the administrative precision required by payer workflows. Clinical scoring alone doesn't capture these critical administrative gaps.
...
### [283] [ELT-Bench-Verified: Benchmark Quality Issues Underestimate AI Agent Capabilities](https://arxiv.org/abs/2603.29399)
*Christopher Zanoli,Andrea Giovannini,Tengjun Jin,Ana Klimovic,Yotam Perlitz*

Main category: cs.AI

TL;DR: ELT-Bench underestimated AI agent capabilities due to model limitations and benchmark errors; after fixing both, performance improved significantly, revealing systemic quality issues in data engineering evaluation.


<details>
  <summary>Details</summary>
Motivation: ELT pipeline construction is labor-intensive and important for AI automation, but initial benchmark results showed low success rates, suggesting AI agents lacked practical utility. The authors wanted to understand why and improve evaluation reliability.

Method: 1) Re-evaluated ELT-Bench with upgraded LLMs; 2) Developed Auditor-Corrector methodology combining LLM-driven root-cause analysis with human validation; 3) Applied this to identify benchmark errors; 4) Created ELT-Bench-Verified with corrected evaluation logic and ground truth.

Result: Extraction and loading stages are largely solved with upgraded models. Most failed transformation tasks contained benchmark-attributable errors (rigid evaluation, ambiguous specs, incorrect ground truth). After correction, performance improved significantly due entirely to benchmark fixes.

Conclusion: Both rapid model improvement and benchmark quality issues caused underestimation of agent capabilities. Quality issues are systemic in data engineering evaluation, echoing problems in text-to-SQL benchmarks. Systematic quality auditing should be standard for complex agentic tasks.
...
### [284] [Structural Compactness as a Complementary Criterion for Explanation Quality](https://arxiv.org/abs/2603.29491)
*Mohammad Mahdi Mesgari,Jackie Ma,Wojciech Samek,Sebastian Lapuschkin,Leander Weber*

Main category: cs.AI

TL;DR: MST-C is a new graph-based metric that measures explanation compactness by analyzing spatial organization and cohesion of attribution points, providing a structural complement to existing attribution complexity measures.


<details>
  <summary>Details</summary>
Motivation: Current quantitative assessment of explanation legibility is difficult because simple statistics fail to capture the varying shapes and internal organization of attributions that affect human interpretability.

Method: Minimum Spanning Tree Compactness (MST-C) uses graph theory to analyze higher-order geometric properties like spread and cohesion. It combines these into a single score that favors attributions with salient points in small areas organized into few cohesive clusters.
...
Conclusion: MST-C offers a valuable structural complement to existing attribution complexity measures by capturing geometric properties that affect explanation legibility, addressing a gap in quantitative evaluation of attribution quality.
...
TL;DR: Metriplector is a neural architecture primitive that configures abstract physical systems (fields, sources, operators) where system dynamics perform computation via coupled metriplectic dynamics, with stress-energy tensor readout.


<details>
  <summary>Details</summary>
Motivation: To create a unified neural architecture primitive based on physical principles that can handle diverse tasks from pathfinding to language modeling through configurable physical system dynamics.

Method: Uses metriplectic dynamics (combining Hamiltonian and dissipative components) where input configures abstract physical systems with fields, sources, and operators. The stress-energy tensor derived from Noether's theorem provides readout. Can operate in dissipative-only mode (screened Poisson equation) or full metriplectic mode.

Result: Achieved F1=1.0 on maze pathfinding (generalizing from 15x15 to 39x39 grids), 97.2% exact Sudoku solve rate, 81.03% on CIFAR-100 with 2.26M parameters, and 1.182 bits/byte on language modeling with 3.6x fewer training tokens than GPT baseline.

Conclusion: Metriplector provides a versatile physics-inspired neural primitive that achieves strong performance across diverse domains from structured reasoning to vision and language tasks, demonstrating the power of unifying neural computation with physical system dynamics.
...
### [286] [Learning to Generate Formally Verifiable Step-by-Step Logic Reasoning via Structured Formal Intermediaries](https://arxiv.org/abs/2603.29500)
*Luoxin Chen,Yichi Zhou,Huishuai Zhang*

Main category: cs.AI

TL;DR: PRoSFI is a reward method that uses formal verification of structured intermediate steps to train LLMs for more reliable reasoning without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Current outcome-rewarded RL for LLMs overlooks flawed intermediate steps, leading to unreliable reasoning even when final answers are correct.
...
Result: PRoSFI enhances reasoning reliability without compromising accuracy, guiding models toward generating step-by-step machine-checkable proofs for more credible final answers.

Conclusion: PRoSFI offers a simple and effective approach to training trustworthy reasoning models by integrating formal verification into the reward process.
...
### [287] [FlowPIE: Test-Time Scientific Idea Evolution with Flow-Guided Literature Exploration](https://arxiv.org/abs/2603.29557)
*Qiyao Wang,Hongbo Wang,Longze Chen,Zhihao Yang,Guhong Chen,Hamid Alinejad-Rokny,Hui Li,Yuan Lin,Min Yang*

Main category: cs.AI

TL;DR: FlowPIE is a novel framework for scientific idea generation that integrates retrieval and generation through flow-guided MCTS and evolutionary algorithms, producing more diverse and higher-quality ideas than existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing scientific idea generation approaches suffer from static retrieval-then-generation paradigms that lead to homogeneous, insufficiently divergent ideas. There's a need for frameworks that can better integrate literature exploration with idea generation as a co-evolving process.

Method: FlowPIE uses flow-guided Monte Carlo Tree Search (inspired by GFlowNets) to expand literature trajectories, guided by LLM-based generative reward model (GRM) assessments. It then applies evolutionary algorithms (selection, crossover, mutation) with isolation island paradigm and GRM-based fitness computation for test-time idea evolution.
...
Conclusion: FlowPIE effectively addresses information cocoons from over-reliance on parametric knowledge and static literature by tightly coupling retrieval and generation, demonstrating superior performance in scientific idea generation.
...
### [288] [ASI-Evolve: AI Accelerates AI](https://arxiv.org/abs/2603.29640)
*Weixian Xu,Tiantian Mi,Yixiu Liu,Yang Nan,Zhimeng Zhou,Lyumanshan Ye,Lin Zhang,Yu Qiao,Pengfei Liu*

Main category: cs.AI

TL;DR: ASI-Evolve is an AI agent framework that autonomously conducts AI research through learn-design-experiment-analyze cycles, achieving state-of-the-art results in neural architecture design, data curation, and algorithm development.


<details>
  <summary>Details</summary>
Motivation: Current AI agent systems excel at well-scoped tasks but struggle with the long-horizon, weakly supervised research loops needed for real AI progress. The paper aims to develop a framework where AI can accelerate AI development itself through closed-loop research.

Method: ASI-Evolve combines evolutionary agents with two key innovations: a cognition base that injects human priors into exploration, and a dedicated analyzer that distills experimental outcomes into reusable insights. It operates through learn-design-experiment-analyze cycles across three AI development components: data, architectures, and learning algorithms.

Result: The framework achieved significant improvements: discovered 105 SOTA linear attention architectures (best model +0.97 points over DeltaNet), improved pretraining data curation by +3.96 average benchmark points (+18+ points on MMLU), and outperformed GRPO by up to +12.5 points on reinforcement learning benchmarks. Initial evidence also shows transfer to mathematics and biomedicine.

Conclusion: ASI-Evolve represents a promising step toward enabling AI to autonomously accelerate AI development across foundational stages, demonstrating the feasibility of closed-loop AI research and potential for broader scientific discovery beyond AI.
...
### [289] [Optimizing Donor Outreach for Blood Collection Sessions: A Scalable Decision Support Framework](https://arxiv.org/abs/2603.29643)
...
<details>
  <summary>Details</summary>
Motivation: Blood donation centers struggle to match supply with demand while avoiding donor fatigue. Current approaches don't address the operational problem of assigning donors to sessions across multi-site networks considering eligibility, capacity, blood-type targets, geographic convenience, and donor safety.

Method: Developed an optimization framework with two strategies: (1) binary integer linear programming (BILP) formulation, and (2) efficient greedy heuristic. Used IPST registry data with 4-month windows, integrating organic attendance forecasting, quantile-based demand targets, and residual capacity estimation.

Result: Greedy heuristic achieved comparable results to BILP with 188x less peak memory and 115x faster runtime, though with 3.9% lower demand fulfillment (86.1% vs 90.0%). Framework effectively closed supply-demand gaps in Lisbon region by mobilizing eligible inactive/lapsing donors.

Conclusion: The optimization framework successfully addresses blood donor invitation scheduling, with greedy heuristic offering practical efficiency trade-offs. Constraint-aware scheduling can effectively close supply-demand gaps while managing donor convenience and safety.
...
### [290] [View-oriented Conversation Compiler for Agent Trace Analysis](https://arxiv.org/abs/2603.29678)
*Lvmin Zhang,Maneesh Agrawala*

Main category: cs.AI

TL;DR: VCC is a compiler that transforms raw agent JSONL logs into structured views, improving context learning performance by treating conversation format as infrastructure rather than incidental detail.


<details>
  <summary>Details</summary>
Motivation: Modern agent conversations contain complex structured content (nested tool calls, chain-of-thought reasoning, sub-agent invocations, etc.) that exceeds simple user-assistant exchanges. Current approaches treat conversation format as trivial engineering detail, feeding traces to analytical mechanisms in plain text formats which degrades analysis quality.

Method: VCC (View-oriented Conversation Compiler) is a compiler with lex, parse, IR, lower, emit stages that transforms raw agent JSONL logs into three structured views: 1) full view (lossless transcript with canonical line-number coordinate system), 2) user-interface view (reconstructing interaction as user perceived it), and 3) adaptive view (structure-preserving projection governed by relevance predicate).

Result: In a context-learning experiment on AppWorld, replacing reflector's input format from raw JSONL to VCC-compiled views led to higher pass rates across all three model configurations tested, while cutting reflector token consumption by half to two-thirds and producing more concise learned memory.

Conclusion: Message format functions as infrastructure for context learning, not as an incidental implementation choice. Structured conversation compilation improves analysis quality and efficiency in agentic systems.
...
### [291] [Beyond the Steeper Curve: AI-Mediated Metacognitive Decoupling and the Limits of the Dunning-Kruger Metaphor](https://arxiv.org/abs/2603.29681)
*Christopher Koch*

Main category: cs.AI

TL;DR: LLM use improves task output but degrades metacognitive accuracy, creating a gap between performance and self-assessment - AI-mediated metacognitive decoupling.
...
Method: Synthesizes evidence from human-AI interaction research, learning studies, and model evaluation to develop a four-variable model of AI-mediated metacognitive decoupling.
...
Conclusion: The AI-mediated metacognitive decoupling model better explains phenomena like overconfidence and over/under-reliance than the Dunning-Kruger metaphor, with implications for tool design, assessment, and knowledge work.
...
TL;DR: Method extracts first-order formulas from probability distributions to reduce exponential encoding size, increasing sparsity while preserving core information.


<details>
  <summary>Details</summary>
Motivation: Real-world scenarios use lifted probability distributions, but current encodings require exponential values in tables/lists, making them inefficient for large distributions.

Method: Reduce number of values in distribution, extract logical formula for each value, then minimize formulas to increase sparsity and generalize the distribution.
...
Conclusion: Formula extraction enables efficient encoding of lifted probability distributions with significantly reduced storage requirements through sparse, generalized representations.
...
TL;DR: This paper proposes using the meta-d' framework and signal detection theory as gold standards for measuring AI metacognitive abilities, demonstrating their utility through experiments on three large language models.


<details>
  <summary>Details</summary>
Motivation: As AI systems become more integrated into decision-making workflows, there's a critical need for robust methods to measure their metacognitive capabilities—specifically their ability to assess reliability and regulate decisions based on uncertainty and risk.

Method: The paper proposes adopting the meta-d' framework (and its model-free alternatives) to assess metacognitive sensitivity, and leverages signal detection theory to measure decision regulation based on uncertainty and risk. Two experimental series were conducted on three LLMs (GPT-5, DeepSeek-V3.2-Exp, Mistral-Medium-2508): one with primary judgment + confidence ratings, and another with manipulated risk conditions.

Result: The meta-d' framework enables three-way comparisons: comparing LLMs to optimality, comparing different LLMs on the same task, and comparing the same LLM across different tasks. Signal detection theory reveals whether LLMs become more conservative when risks are high.

Conclusion: The paper establishes psychophysical frameworks (meta-d' and SDT) as gold standards for assessing AI metacognitive abilities, demonstrating their practical utility through systematic evaluation of LLMs' confidence calibration and risk-based decision regulation.
...
TL;DR: Symphony for Medical Coding is an AI system that mimics human coders by reasoning over clinical narratives with access to coding guidelines, enabling cross-system adaptability and providing span-level evidence for predictions.


<details>
  <summary>Details</summary>
Motivation: Medical coding is crucial for billing, research, and quality reporting but remains manual, slow, and error-prone. Existing automated approaches are inflexible (can't adapt to new codes or different systems without retraining) and lack explanations, limiting trust in safety-critical healthcare settings.

Method: Symphony approaches medical coding like expert human coders: it reasons over clinical narratives with direct access to coding guidelines. This design enables operation across any coding system and provides span-level evidence linking each predicted code to supporting text.

Result: Symphony achieves state-of-the-art results across all evaluated settings: two public benchmarks and three real-world datasets spanning inpatient, outpatient, emergency, and subspecialty settings across the US and UK.

Conclusion: Symphony establishes itself as a flexible, deployment-ready foundation for automated clinical coding, addressing key limitations of existing approaches through its human-like reasoning approach with coding guideline access and explainable predictions.
...
TL;DR: ReTriP is an end-to-end generative framework that reformulates retrosynthesis as a Chain-of-Thought reasoning task, achieving state-of-the-art performance with superior robustness in long-horizon planning.


<details>
  <summary>Details</summary>
Motivation: Conventional retrosynthetic planning approaches fracture logical coherence between local molecular transformations and global planning objectives by relying on hybrid frameworks that combine single-step predictions with external search heuristics. There's a need to embed sophisticated strategic foresight directly into the model's chemical reasoning.

Method: ReTriP reformulates retrosynthesis as a direct Chain-of-Thought reasoning task using a path-coherent molecular representation. It employs a progressive training curriculum that transitions from reasoning distillation to reinforcement learning with verifiable rewards, aligning stepwise generation with practical route utility.

Result: Empirical evaluation on RetroBench demonstrates that ReTriP achieves state-of-the-art performance and exhibits superior robustness in long-horizon planning compared to hybrid baselines.

Conclusion: ReTriP successfully bridges the gap between local molecular transformations and global planning objectives by embedding strategic foresight directly into chemical reasoning through an end-to-end generative framework, outperforming conventional hybrid approaches.
...
TL;DR: Large language models spontaneously develop synergistic information processing cores similar to human brains, with middle layers showing synergistic integration while early/late layers use redundancy, emerging as a physical phase transition with task difficulty.


<details>
  <summary>Details</summary>
Motivation: To identify universal computational principles by studying how artificial intelligence systems evolve, specifically examining whether large language models develop information processing structures similar to biological intelligence.

Method: Used Integrated Information Decomposition across multiple neural network architectures to analyze information processing patterns, examining how different layers (early, middle, late) process information through synergy vs redundancy.
...
Conclusion: Synergistic cores in large language models represent the physical entity of abstract reasoning, bridging artificial and biological intelligence and revealing universal computational principles in intelligent systems.
...
TL;DR: CausalPulse is an industry-grade multi-agent copilot that automates causal diagnostics in smart manufacturing, unifying anomaly detection, causal discovery, and reasoning through neurosymbolic architecture with standardized agentic protocols.


<details>
  <summary>Details</summary>
Motivation: Modern manufacturing needs real-time, trustworthy, interpretable root-cause insights, but traditional analytics treat anomaly detection, causal inference, and root-cause analysis as isolated stages, limiting scalability and explainability.

Method: CausalPulse uses a neurosymbolic architecture built on standardized agentic protocols to unify anomaly detection, causal discovery, and reasoning. It's designed as a multi-agent copilot with modular, human-in-the-loop design.

Result: Achieved overall success rates of 98.0% on public datasets and 98.73% on proprietary datasets. Per-criterion success: 98.75% planning/tool use, 97.3% self-reflection, 99.2% collaboration. End-to-end latency 50-60s with near-linear scalability (R²=0.97).

Conclusion: CausalPulse demonstrates reliable, interpretable, production-ready automation for next-generation manufacturing with advantages in modularity, extensibility, and deployment maturity compared to existing industrial copilots.
...
TL;DR: Training chess engines on move sequences alone creates a dual-capability bottleneck between state tracking (reconstructing board from history) and decision quality (selecting good moves), requiring both low-rated games for diversity and high-rated games for quality signals.


<details>
  <summary>Details</summary>
Motivation: To create human-like chess engines that mimic human style, errors, and consistency rather than maximizing playing strength, which requires learning from move sequences to capture human decision patterns.

Method: Train models on move sequences alone, which forces learning of state tracking and decision quality. Use scaling (28M to 120M parameters) to improve tracking, and introduce Elo-weighted training to improve decisions while preserving diversity. Use linear weighting and coverage-decay formula for reliability horizon.

Result: Final 120M-parameter model reached Lichess bullet 2570 over 253 rated games without search. Achieved 55.2% Top-1 accuracy on human move prediction, exceeding Maia-2 rapid and blitz. Sequence input enables history-dependent decisions that position-based models cannot exhibit.

Conclusion: Human-like chess engines require balancing state tracking and decision quality through appropriate data diversity and quality. Sequence-based training captures human decision patterns better than position-based methods, enabling history-dependent play that mimics human consistency and style.
...
TL;DR: Simula: A reasoning-driven framework for generating synthetic multi-modal datasets without seed data, using explainable agentic processes for fine-grained control over dataset characteristics.


<details>
  <summary>Details</summary>
Motivation: Specialized multi-modal AI models need diverse training data, but real data is scarce, expensive to collect, and privacy-sensitive. Existing synthetic data methods are limited in scalability, explainability, and control.

Method: Simula uses a seedless, agentic approach where users define desired dataset characteristics through an explainable and controllable reasoning process, enabling fine-grained resource allocation for synthetic data generation.

Result: The framework effectively generates synthetic datasets across various domains, with rigorous testing of both intrinsic properties and downstream performance on AI tasks.

Conclusion: Simula provides guidelines for synthetic data mechanism design, insights for scalable generation/evaluation, and enables AI development in data-scarce or privacy-sensitive domains.
...
TL;DR: Owl-AuraID is a GUI-native embodied agent system that automates scientific instruments by operating through their graphical user interfaces, enabling end-to-end workflows from physical sample handling to data analysis across diverse precision instruments.


<details>
  <summary>Details</summary>
Motivation: Scientific discovery relies on high-throughput characterization, but automation is hindered by proprietary GUIs and limited generalizability of existing API-based systems. Current approaches cannot easily adapt to diverse instrument interfaces used by human experts.

Method: Owl-AuraID adopts a GUI-native paradigm, operating instruments through the same interfaces as human experts. It uses a skill-centric framework integrating Type-1 (GUI operation) and Type-2 (data analysis) skills into end-to-end workflows, connecting physical sample handling with scientific interpretation.

Result: The system demonstrates broad coverage across ten categories of precision instruments and diverse workflows, including multimodal spectral analysis, microscopic imaging, and crystallographic analysis, supporting modalities such as FTIR, NMR, AFM, and TGA.

Conclusion: Owl-AuraID provides a practical, extensible foundation for autonomous laboratories and illustrates a path toward evolving laboratory intelligence through reusable operational and analytical skills, with code available as open source.
...
### [301] [AgentFixer: From Failure Detection to Fix Recommendations in LLM Agentic Systems](https://arxiv.org/abs/2603.29848)
*Hadar Mulian,Sergey Zeltyn,Ido Levy,Liane Galanti,Avi Yaeli,Segev Shlomov*

Main category: cs.AI

TL;DR: A comprehensive validation framework for LLM-based agentic systems that diagnoses reliability failures, provides root-cause analysis, and enables systematic improvement through both automated checks and LLM-driven self-reflection.


<details>
  <summary>Details</summary>
Motivation: LLM-based agentic systems suffer from reliability failures that need systematic diagnosis and improvement. Current approaches lack comprehensive validation frameworks that can uncover weaknesses across input handling, prompt design, and output generation while enabling continuous improvement.

Method: Developed a validation framework with 15 failure-detection tools and 2 root-cause analysis modules. Combines lightweight rule-based checks with LLM-as-a-judge assessments for structured incident detection, classification, and repair. Applied to IBM CUGA on AppWorld and WebArena benchmarks, then used diagnostic outputs for LLM-driven self-reflection and prioritization.

Result: Revealed recurrent planner misalignments, schema violations, and brittle prompt dependencies. Enabled refinement of prompting and coding strategies, maintaining CUGA's benchmark results while helping mid-sized models (Llama 4, Mistral Medium) achieve notable accuracy gains and narrow gap with frontier models. Interactive analysis produced actionable insights on failure patterns.

Conclusion: The framework shows a path toward scalable, quality-assured, and adaptive validation in production agentic systems, offering foundation for more robust, interpretable, and self-improving agentic architectures through dialogue-driven validation processes.
...
### [302] [Spatiotemporal Robustness of Temporal Logic Tasks using Multi-Objective Reasoning](https://arxiv.org/abs/2603.29868)
*Oliver Schön,Lars Lindemann*

Main category: cs.AI

TL;DR: Proposes spatiotemporal robustness (STR) for temporal logic specifications, capturing both spatial and temporal perturbations jointly via multi-objective reasoning, with sound robust semantics and monitoring algorithms.


<details>
  <summary>Details</summary>
Motivation: Existing robustness measures only capture spatial perturbations (geometric distance from unsatisfiability), but real-world autonomous systems (multi-agent robotics, smart cities, air traffic control) require joint consideration of both spatial AND temporal perturbations for more informative reliability assessment.

Method: Define STR as multi-objective reasoning problem using partial order over spatial/temporal perturbations; interpret as Pareto-optimal set; compute using multi-objective optimization tools; propose sound robust semantics that under-approximate STR while being tractable; develop monitoring algorithms.

Result: First work to handle robustness across multiple dimensions via multi-objective reasoning; provides formal framework for STR with sound semantics and practical monitoring algorithms for temporal logic specifications over discrete-time signals.

Conclusion: STR provides more informative robustness measure for interacting systems by jointly considering spatial and temporal perturbations through multi-objective reasoning, with computationally tractable semantics and monitoring algorithms.
...
### [303] [ShapE-GRPO: Shapley-Enhanced Reward Allocation for Multi-Candidate LLM Training](https://arxiv.org/abs/2603.29871)
*Rui Ai,Yu Pan,David Simchi-Levi,Chonghuan Wang*

Main category: cs.AI

TL;DR: ShapE-GRPO improves set-level recommendation by using Shapley values to decompose rewards, preventing poor candidates from free-riding on strong peers.


<details>
  <summary>Details</summary>
Motivation: Current RL post-training methods like GRPO assign the same set-level reward to all candidates, causing poor candidates to free-ride on strong peers and resulting in noisy training signals and suboptimal exploration.

Method: Proposes Shapley-Enhanced GRPO (ShapE-GRPO) that uses cooperative game theory's Shapley value to decompose set-level rewards into granular, candidate-specific signals while preserving Shapley axioms and maintaining polynomial-time complexity.
...
Conclusion: Shapley value decomposition provides more effective reward signals for set-level optimization in LLM applications, improving training efficiency and final performance.
...
TL;DR: ATP-Bench introduces a benchmark for evaluating agentic tool planning in MLLMs for interleaved text-image generation, proposing a Multi-Agent MLLM-as-a-Judge system to assess tool-use behavior without ground-truth references.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs treat image generation and retrieval as mutually exclusive, failing to unify factuality with creativity. The paper argues for agentic tool planning where models autonomously decide when and which tools to use for visual-critical queries.

Method: 1) ATP-Bench benchmark with 7,702 QA pairs across 8 categories and 25 visual-critical intents; 2) Multi-Agent MLLM-as-a-Judge (MAM) system to evaluate tool-call precision, identify missed tool opportunities, and assess response quality without ground-truth references.
...
Conclusion: Agentic tool planning is the next milestone for interleaved text-image generation. ATP-Bench provides systematic evaluation and actionable guidance for advancing this capability in MLLMs.
...
TL;DR: C-TRAIL is a framework that combines LLM commonsense reasoning with trust mechanisms for safer autonomous driving trajectory planning, using a closed-loop system with trust-weighted MCTS.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used for commonsense reasoning in autonomous driving trajectory planning, but their outputs are unreliable and pose safety risks in critical applications.

Method: C-TRAIL uses a closed-loop Recall, Plan, Update cycle: Recall queries LLMs for semantic relations with dual-trust reliability quantification; Plan injects trust-weighted commonsense into MCTS via Dirichlet trust policy; Update refines trust scores from environmental feedback.

Result: Experiments on simulated scenarios (Highway-env) and real-world datasets (highD, rounD) show C-TRAIL outperforms SOTA baselines, reducing ADE by 40.2%, FDE by 51.7%, and improving SR by 16.9 percentage points on average.

Conclusion: C-TRAIL effectively addresses LLM reliability issues in autonomous driving by integrating trust mechanisms with commonsense reasoning, significantly improving trajectory planning performance and safety.
...
Method: Propose using epistemic uncertainty as a proxy for explanation reliability, enabling two use cases: 1) routing samples to cheap or expensive XAI methods based on expected reliability, and 2) deferring explanation generation for uncertain samples under constrained budgets.

Result: Strong negative correlation between epistemic uncertainty and explanation stability across four tabular datasets, five architectures, and four XAI methods. Epistemic uncertainty distinguishes both stable/unstable and faithful/unfaithful explanations. Findings generalize to image classification.

Conclusion: Epistemic uncertainty provides an effective, low-cost way to assess explanation reliability, enabling more efficient and reliable XAI systems through intelligent routing and deferral strategies.
...
TL;DR: ScoringBench is an open benchmark that evaluates tabular foundation models using comprehensive proper scoring rules (CRPS, CRLS, Interval Score, etc.) rather than just point estimate metrics like RMSE/R², revealing that model rankings depend on the scoring rule and no single pretraining objective is universally optimal.


<details>
  <summary>Details</summary>
Motivation: Current regression benchmarks for tabular foundation models (TabPFN, TabICL) rely primarily on point estimate metrics (RMSE, R²), which obscure performance in distribution tails. This is problematic for high-stakes domains like finance and clinical research where asymmetric risk profiles are critical.

Method: Introduced ScoringBench - an open benchmark that computes a comprehensive suite of proper scoring rules including CRPS, CRLS, Interval Score, Energy Score, weighted CRPS, and Brier Score alongside standard point metrics. Evaluated realTabPFNv2.5 fine-tuned with different scoring rule objectives and TabICL relative to untuned realTabPFNv2.5 across regression benchmarks.
...
Conclusion: ScoringBench provides a richer picture of probabilistic forecast quality than traditional point metrics alone, especially important for high-stakes decision making where tail performance matters. The benchmark is openly available with a transparent, traceable leaderboard maintained via git pull requests.
...
TL;DR: Physiological synchrony combined with dialogue analysis reveals pivotal moments in medical team problem-solving, showing successful teams sync during discovery while unsuccessful teams sync during uncertainty.


<details>
  <summary>Details</summary>
Motivation: To understand how physiological synchrony and conversational dynamics can reveal critical moments in team problem-solving, particularly in medical diagnosis contexts where Socially Shared Regulation of Learning (SSRL) is crucial for effective collaboration.

Method: Analyzed four medical dyads diagnosing virtual patient cases using intelligent tutoring system. Measured physiological synchrony (longitudinal alignment of physiological signals) and correlated with semantic shifts in dialogue. Coded utterances for SSRL and derived cosine similarity using sentence embeddings.

Result: Semantic shifts correlated with transient physiological synchrony peaks. Activating prior knowledge showed significantly lower semantic similarity than task execution. High physiological synchrony associated with lower semantic similarity, indicating exploratory language use. Qualitative analysis identified synchrony peaks as "pivotal moments": successful teams synchronized during shared discovery, unsuccessful teams during shared uncertainty.

Conclusion: Fusing biological signals with dialogue analysis advances human-centered AI by revealing critical problem-solving moments, demonstrating how physiological synchrony combined with conversational patterns can identify pivotal collaborative states in medical teams.
...
### [309] [Structured Intent as a Protocol-Like Communication Layer: Cross-Model Robustness, Framework Comparison, and the Weak-Model Compensation Effect](https://arxiv.org/abs/2603.29953)
*Peng Gang*

Main category: cs.AI

TL;DR: Structured intent frameworks (5W3H, CO-STAR, RISEN) significantly improve cross-language robustness and goal alignment across AI models, with user studies showing 60% reduction in interaction rounds and higher satisfaction.


<details>
  <summary>Details</summary>
Motivation: To investigate how reliably structured intent representations preserve user goals across different AI models, languages, and prompting frameworks, extending prior work on PPS (Prompt Protocol Specification).
...
Result: Structured prompting reduces cross-language score variance from 0.470 to ~0.020. Weak-model compensation: Gemini shows largest improvement (+1.006) vs Claude (+0.217). All structured frameworks (5W3H, CO-STAR, RISEN) achieve similarly high goal-alignment scores. User study: AI-expanded 5W3H prompts reduce interaction rounds by 60% and increase satisfaction from 3.16 to 4.04.
...
### [310] [Extending MONA in Camera Dropbox: Reproduction, Learned Approval, and Design Implications for Reward-Hacking Mitigation](https://arxiv.org/abs/2603.29993)
*Nathan Heath*

Main category: cs.AI

TL;DR: This paper reproduces and extends the MONA (Myopic Optimization with Non-myopic Approval) framework to study how different approval mechanisms affect safety guarantees against reward hacking in multi-step environments.


<details>
  <summary>Details</summary>
Motivation: The original MONA paper left open the question of how approval construction methods (particularly their dependence on achieved outcomes) affect whether MONA's safety guarantees hold. This work aims to operationalize and test the "approval-spectrum conjecture" through systematic experimentation.

Method: The authors create a reproduction-first extension of the public MONA Camera Dropbox environment with: (1) repackaged codebase as standard Python project with scripted PPO training, (2) confirmation of published results using reference arrays, and (3) a modular learned-approval suite spanning oracle, noisy, misspecified, learned, and calibrated approval mechanisms. They conduct reduced-budget pilot sweeps across approval methods, horizons, dataset sizes, and calibration strategies.

Result: The reproduction confirms the original contrast: ordinary RL shows 91.5% reward-hacking rate while oracle MONA achieves 0.0% hacking rate. The best calibrated learned-overseer run achieves zero observed reward hacking but substantially lower intended-behavior rates than oracle MONA (11.9% vs. 99.9%), indicating under-optimization rather than re-emergent hacking.

Conclusion: The study operationalizes MONA's approval-spectrum conjecture as a runnable experimental object and suggests the central engineering challenge shifts from proving MONA's concept to building learned approval models that preserve sufficient foresight without reopening reward-hacking channels.
...
### [311] [The Triadic Cognitive Architecture: Bounding Autonomous Action via Spatio-Temporal and Epistemic Friction](https://arxiv.org/abs/2603.30031)
*Davide Di Gioia*

Main category: cs.AI

TL;DR: The paper proposes Triadic Cognitive Architecture (TCA), a physics-grounded framework that addresses cognitive weightlessness in LLM-based agents by introducing Cognitive Friction, using nonlinear filtering, Riemannian geometry, and optimal control to improve decision-making under constraints.


<details>
  <summary>Details</summary>
Motivation: Current LLM-driven AI agents suffer from "cognitive weightlessness" - they process information without intrinsic understanding of network topology, temporal pacing, or epistemic limits, leading to failures like excessive tool use, prolonged deliberation, and brittle behavior in interactive environments.

Method: Proposes Triadic Cognitive Architecture (TCA) that synthesizes nonlinear filtering theory, Riemannian routing geometry, and optimal control. Maps agent deliberation to coupled stochastic control problem with path-dependent information acquisition. Uses HJB-motivated stopping boundary and rollout-based approximation of belief-dependent value-of-information with net-utility halting condition instead of heuristic stop-tokens.

Result: Empirical validation in simulated Emergency Medical Diagnostic Grid (EMDG) shows triadic policy reduces time-to-action while improving patient viability without degrading diagnostic accuracy, outperforming greedy baselines that over-deliberate under latency and congestion costs.

Conclusion: TCA provides a unified mathematical framework grounded in continuous-time physics to address cognitive weightlessness in AI agents, enabling more efficient and robust decision-making under real-world constraints through formal modeling of Cognitive Friction.
