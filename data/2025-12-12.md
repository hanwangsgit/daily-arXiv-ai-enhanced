...
TL;DR: This paper presents a neuromorphic SNN approach for eye tracking that achieves 3.7-4.1px mean error with 20x smaller models and 850x less compute than ANN variants, enabling 3.9-4.9 mW power and 3 ms latency for wearable AR/VR applications.


<details>
  <summary>Details</summary>
Motivation: Conventional frame-based eye tracking suffers from motion blur, high compute cost, and limited temporal resolution, which are problematic for AR/VR applications requiring low latency and milliwatt-level power. While neuromorphic sensors and SNNs offer promise, existing approaches are too specialized or underperform compared to modern ANNs.
...
Result: The SNN models achieve 3.7-4.1px mean error, approaching the accuracy of the specialized Retina system (3.24px), while reducing model size by 20x and theoretical compute by 850x compared to the closest ANN variant. Estimated performance is 3.9-4.9 mW power with 3 ms latency at 1 kHz.
...
TL;DR: ABBSPO is a weakly supervised oriented object detection framework that uses adaptive bounding box scaling and symmetry-prior-based orientation prediction to improve accuracy in aerial object detection with only horizontal bounding box supervision.


<details>
  <summary>Details</summary>
Motivation: Existing HBox-supervised OOD methods have limitations in scale estimation accuracy due to directly comparing GT HBoxes with minimum circumscribed rectangles of predicted RBoxes, leading to inaccurate predictions.
...
Conclusion: ABBSPO effectively addresses scale estimation issues in previous HBox-supervised OOD methods and demonstrates superior performance through adaptive scaling and symmetry-based orientation prediction.
...
TL;DR: SST combines diffusion models with autoregressive generation for image captioning, using diffusion to suggest improvements to autoregressive captions, achieving state-of-the-art results on COCO.


<details>
  <summary>Details</summary>
Motivation: Diffusion models underperform autoregressive models in discrete domains like text generation. The authors want to leverage diffusion's bidirectional/refining capabilities while maintaining autoregressive models' strong linguistic structure.

Method: Show, Suggest and Tell (SST) uses diffusion models to provide suggestions to autoregressive generation rather than replacing it. The diffusion model suggests improvements to the autoregressive caption generation process.
...
TL;DR: MetaVoxel is a joint diffusion model that learns the complete distribution over medical images and clinical metadata, enabling flexible zero-shot inference for multiple tasks without retraining.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for medical AI are typically trained for specific predictive tasks with fixed input-output directions, requiring separate models for different applications. This limits flexibility and clinical applicability.

Method: MetaVoxel uses a generative joint diffusion modeling framework that learns a single diffusion process spanning all variables (imaging data and clinical metadata), capturing the joint distribution rather than conditional distributions.

Result: A single MetaVoxel model trained on over 10,000 T1-weighted MRI scans with clinical metadata from nine datasets can perform image generation, age estimation, and sex prediction with performance comparable to task-specific baselines.

Conclusion: Joint multimodal diffusion modeling offers a promising approach for unifying medical AI models, enabling flexible zero-shot inference and broader clinical applicability without task-specific retraining.
...
TL;DR: Proposes Independent Density Estimation (IDE) method to improve compositional generalization in vision-language models by learning connections between individual words and image features.
...
TL;DR: TraceFlow: A framework for high-fidelity rendering of dynamic specular scenes using 2D Gaussian splatting with material augmentation and hybrid rasterization-ray tracing pipeline.


<details>
  <summary>Details</summary>
Motivation: To address challenges in rendering dynamic specular scenes, specifically precise reflection direction estimation and physically accurate reflection modeling, which are crucial for realistic rendering of complex dynamic environments with specular surfaces.

Method: Proposes Residual Material-Augmented 2D Gaussian Splatting for dynamic geometry/material modeling, Dynamic Environment Gaussian, and hybrid rendering pipeline decomposing into diffuse/specular components using rasterization and ray tracing. Uses coarse-to-fine training strategy for optimization stability.
...
Conclusion: TraceFlow successfully addresses key challenges in dynamic specular scene rendering through novel representations and hybrid rendering, achieving state-of-the-art performance in producing realistic specular reflections.
...
TL;DR: Proposes hierarchical instance tracking task and benchmark dataset with 2,765 entities tracked in 552 videos across 40 object/part categories, showing current models struggle with this challenging task.


<details>
  <summary>Details</summary>
Motivation: Current tracking methods focus on tracking individual objects without considering hierarchical relationships between objects and their parts. There's a need for tracking that maintains these structural relationships for more comprehensive scene understanding.

Method: Introduces a new hierarchical instance tracking task and creates the first benchmark dataset for it. The dataset contains 2,765 unique entities tracked in 552 videos, belonging to 40 categories spanning both objects and parts. Evaluates seven variants of four existing models adapted to this new task.

Result: Evaluation reveals the dataset is challenging for current models, indicating that hierarchical instance tracking presents significant difficulties beyond standard tracking tasks.

Conclusion: Hierarchical instance tracking is a novel and challenging task that requires new approaches beyond existing tracking methods. The introduced benchmark dataset enables future research in this direction and highlights the need for models that can maintain hierarchical relationships during tracking.
...
<details>
  <summary>Details</summary>
Motivation: Breast cancer screening suffers from high false positive/negative rates and model performance degradation when deployed across different scanners, modalities, and patient populations.

Method: Propose wavelet-based vectorization of persistent homology to create stable spatial multi-scale maps of image structure. Integrate these topological features into a two-stage detection pipeline via channel concatenation.

Result: On INbreast dataset, augmenting ConvNeXt Tiny with wavelet persistence channels increased patient-level AUC from 0.55 to 0.75 under limited training budget. Model validated on CBIS DDSM and evaluated on independent datasets from Portugal (INbreast) and China (CMMD).

Conclusion: Topological data analysis (wavelet persistence) provides effective conditioning signals that improve external performance and generalization of breast cancer detection models across different clinical settings.
...
TL;DR: FCTM (Feature Coding Test Model) achieves ~85% bitrate reduction for edge-cloud split inference by compressing intermediate features, enabling efficient deployment in bandwidth-limited applications.


<details>
  <summary>Details</summary>
Motivation: DNNs are compute-intensive for edge devices, but full offloading to cloud faces latency, bandwidth, and privacy issues. Edge-cloud split inference needs efficient feature compression to reduce transmission overhead.

Method: MPEG's FCM standard provides bitstream syntax and codec pipeline for compressing intermediate features. The paper presents FCTM design and evaluation across multiple vision tasks.

Result: FCTM achieves average 85.14% bitrate reduction while preserving accuracy, demonstrating significant bandwidth savings for split inference scenarios.

Conclusion: FCM/FCTM enables scalable, efficient, and interoperable deployment of intelligent features in bandwidth-limited and privacy-sensitive consumer applications through standardized feature compression.
...
Method: Proposes Latent-CoT-Drive (LCDrive) that expresses CoT in a latent language using action-proposal tokens (same vocabulary as output actions) and world model tokens (grounded in learned latent world model). Uses cold start supervision with ground-truth future rollouts, then post-trains with closed-loop reinforcement learning.
...
TL;DR: FCM (Feature Coding for Machines) compresses neural features for machine-to-machine communication, reducing bandwidth while maintaining accuracy close to edge inference, with existing codecs like HEVC performing nearly as well as newer VVC for machine tasks.


<details>
  <summary>Details</summary>
Motivation: Current machine-to-machine systems rely on human-optimized video codecs, which are bandwidth-intensive, scale poorly, and expose raw images to third parties, creating privacy and efficiency concerns.

Method: Proposes Feature Coding for Machines (FCM) that compresses intermediate neural features instead of raw pixels, using H.26X codecs as inner codecs to reduce bitrate while preserving privacy and supporting compute offload.

Result: FCM maintains accuracy close to edge inference while significantly reducing bitrate. HEVC performs nearly as well as VVC (only 1.39% BD-Rate increase), while AVC performs significantly worse (32.28% increase). For tracking tasks, codec choice has minimal impact.

Conclusion: Existing hardware with deployed codecs like HEVC can support machine-to-machine communication without degrading performance, making FCM a practical solution for efficient, privacy-preserving machine-to-machine systems.
...
TL;DR: MCDPO addresses reward conflicts in DPO for diffusion models by introducing a disentangled Bradley-Terry objective with conditional preference vectors, enabling independent optimization of multiple reward axes and dynamic control at inference.


<details>
  <summary>Details</summary>
Motivation: Standard DPO formulation has a fundamental limitation: it aggregates diverse evaluation axes (aesthetic quality, semantic alignment) into a single scalar reward using Bradley-Terry model, creating reward conflicts where desirable features of one dimension get unlearned if they appear in globally non-preferred samples.

Method: Proposes Multi Reward Conditional DPO (MCDPO) with disentangled Bradley-Terry objective. Introduces preference outcome vector as condition during training, allowing independent optimization of each reward axis within single network. Uses dimensional reward dropout for balanced optimization across dimensions.

Result: Extensive experiments on Stable Diffusion 1.5 and SDXL show MCDPO achieves superior performance on benchmarks. The conditional framework enables dynamic and multiple-axis control at inference using Classifier Free Guidance to amplify specific reward dimensions without additional training or external reward models.

Conclusion: MCDPO effectively resolves reward conflicts in DPO for diffusion models, enabling independent optimization of multiple reward dimensions and providing flexible control over different quality aspects at inference time through a single trained model.
...
TL;DR: SWIFT enables effective semi-supervised few-shot learning by finetuning VLMs with simple techniques (classifier initialization & temperature tuning) to overcome flat probability distributions, achieving near-supervised performance.


<details>
  <summary>Details</summary>
Motivation: Real-world auto-annotation applications require learning from few labeled and abundant unlabeled data, but current SSFSL methods neglect powerful open-source VLMs and their pretraining data, while related FSL work has already exploited these resources.

Method: SWIFT (Stage-Wise Finetuning with Temperature Tuning) uses classifier initialization and temperature tuning to increase confidence scores of pseudo-labels, enabling effective VLM finetuning on limited labeled data, abundant unlabeled data, and task-relevant noisy data from pretraining sets.

Result: Extensive experiments on five SSFSL benchmarks show SWIFT outperforms recent FSL and SSL methods by ~5 accuracy points, and even rivals supervised learning that uses ground truth labels for unlabeled data.

Conclusion: Simple techniques can overcome VLM's flat probability distributions to enable effective semi-supervised few-shot learning, allowing SSFSL to leverage open-source VLMs and pretraining data for real-world auto-annotation applications.
...
TL;DR: RobustSora benchmark evaluates watermark robustness in AI-generated video detection, revealing detectors partially rely on watermarks with 2-8pp performance variations under watermark manipulation.
...
Method: Created RobustSora benchmark with 6,500 videos across four types: Authentic-Clean, Authentic-Spoofed with fake watermarks, Generated-Watermarked, and Generated-DeWatermarked. Evaluated ten models including specialized AIGC detectors, transformers, and MLLMs on two tasks: Task-I tests detection on watermark-removed AI videos, Task-II assesses false alarms on authentic videos with fake watermarks.

Result: Performance variations of 2-8pp under watermark manipulation. Transformer-based models show consistent moderate dependency (6-8pp), while MLLMs exhibit diverse patterns (2-8pp). Results indicate partial watermark dependency in current detection models.

Conclusion: Current AIGC video detectors partially rely on watermarks, highlighting the need for watermark-aware training strategies. RobustSora provides essential tools to advance robust AIGC detection research.
...
TL;DR: THE-Pose introduces a category-level 6D pose estimation framework that combines topological features from images with point-cloud features via hybrid graph fusion to overcome limitations of 3D graph convolution methods.


<details>
  <summary>Details</summary>
Motivation: Existing 3D graph convolution methods focus only on local geometry and depth, making them vulnerable to complex objects and visual ambiguities. There's a need to incorporate both global context and local structure for robustness against intra-class variations.

Method: Uses surface embedding to extract consistent topological features from images, then employs Hybrid Graph Fusion (HGF) module to adaptively integrate these topological features with point-cloud features, bridging 2D image context with 3D geometric structure.

Result: Achieves 35.8% improvement over 3D-GC baseline (HS-Pose) and surpasses previous state-of-the-art by 7.2% across all key metrics on REAL275 dataset. Shows stability for unseen/complicated objects even under significant occlusions.

Conclusion: THE-Pose effectively addresses limitations of 3D-GC methods by leveraging topological priors and hybrid fusion, demonstrating superior performance in category-level 6D pose estimation through better integration of 2D and 3D features.
...
Result: GDKVM outperforms state-of-the-art methods on two mainstream echocardiography datasets (CAMUS and EchoNet-Dynamic) in segmentation accuracy and robustness while maintaining real-time performance.

Conclusion: GDKVM provides an effective solution for echocardiography video segmentation that successfully balances spatiotemporal dependency modeling, computational efficiency, and feature representation, offering improved clinical utility for cardiac function analysis.
...
TL;DR: LLM-NCD: A multimodal framework for Novel Class Discovery that fuses visual-textual semantics and uses prototype-guided clustering to improve classification of unknown classes, achieving 25.3% accuracy improvement on CIFAR-100 with unique resilience to long-tail distributions.
...
Method: Multimodal framework fusing visual-textual semantics with prototype-guided clustering. Key innovations: 1) Joint optimization of known class image and text features to model cluster centers and semantic prototypes, 2) Dual-phase discovery mechanism that dynamically separates known/novel samples using semantic affinity thresholds and adaptive clustering.

Result: Achieves up to 25.3% improvement in accuracy for unknown classes on CIFAR-100 dataset compared to current methods. Demonstrates unique resilience to long-tail distributions, which is a first in NCD literature.

Conclusion: LLM-NCD successfully overcomes limitations of visual-only NCD methods by leveraging multimodal fusion and prototype-guided clustering, establishing a new state-of-the-art approach with strong performance on both standard and long-tail distributions.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods have trade-offs: Gaussian splatting (Long-LRM) enables feed-forward reconstruction but is error-sensitive for fine details, while implicit representations (LVSM/LaCT) offer higher fidelity but require computationally intensive decompression that prevents real-time rendering.

Method: Proposes Long-LRM++ with semi-explicit scene representation combined with lightweight decoder, scaling to 64 input views at 950×540 resolution while maintaining efficient processing.

Result: Achieves real-time 14 FPS rendering on A100 GPU while matching LaCT's quality on DL3DV, scales to 64 input views, and delivers superior novel-view depth prediction on ScanNetv2 compared to Gaussian-based methods.

Conclusion: Demonstrates that semi-explicit representation with lightweight decoder can retain benefits of implicit methods while enabling real-time performance, overcoming limitations of both Gaussian splatting and computationally intensive implicit approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing adversarial distillation methods often fail to effectively transfer robustness from state-of-the-art robust teachers to students, with stronger teachers not necessarily producing more robust students (robust saturation phenomenon). Current explanations focusing on capacity gaps are incomplete.

Method: Proposes Sample-wise Adaptive Adversarial Distillation (SAAD) that reweights training examples based on measured adversarial transferability (fraction of student-crafted adversarial examples that remain effective against the teacher) without additional computational cost.
...
Conclusion: Adversarial transferability is a key factor in successful robustness transfer, and SAAD's sample-wise adaptive weighting based on transferability effectively addresses robust saturation in adversarial distillation.
...
<details>
  <summary>Details</summary>
Motivation: Existing image editing datasets focus on static appearance changes or have low-quality motion edits. There's a need for high-fidelity motion editing that preserves identity, structure, and physical plausibility for applications like video synthesis and animation.

Method: 1) Created MotionEdit dataset from continuous videos with verified motion transformations. 2) Developed MotionEdit-Bench benchmark with generative, discriminative, and preference metrics. 3) Proposed MotionNFT framework that computes motion alignment rewards to guide models toward accurate motion transformations during fine-tuning.

Result: MotionEdit-Bench shows current diffusion models struggle with motion editing. MotionNFT consistently improves editing quality and motion fidelity for FLUX.1 Kontext and Qwen-Image-Edit models without sacrificing general editing ability.

Conclusion: Motion editing is a challenging but important task. The MotionEdit dataset and MotionNFT framework provide effective solutions for improving motion-centric image editing while maintaining general editing capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current multi-shot video generation focuses only on low-level visual consistency across shots, neglecting intentional film-editing patterns and cinematographic language, resulting in mere sequential shot changes without coherent narrative expression.

Method: Proposes ShotDirector with: 1) Camera control module using 6-DoF poses and intrinsic settings for precise camera information injection; 2) Shot-aware mask mechanism with hierarchical prompts aware of professional editing patterns for fine-grained content control; 3) Combination of parameter-level conditions with high-level semantic guidance.

Result: Created ShotWeaver40K dataset capturing film-like editing pattern priors, developed evaluation metrics for controllable multi-shot video generation, and demonstrated framework effectiveness through extensive experiments.

Conclusion: ShotDirector effectively addresses the limitation of current approaches by enabling film-like controllable shot transitions through integrated camera control and editing-pattern-aware prompting, advancing narrative expression in multi-shot video generation.
...
TL;DR: Disentangled360 is a 3D-aware framework that combines direction disentangled volume rendering with single-image 360° view synthesis for medical imaging and natural scene reconstruction, outperforming existing methods without requiring scene-specific finetuning.


<details>
  <summary>Details</summary>
Motivation: Current techniques either oversimplify anisotropic light behavior or lack generalizability across different contexts (medical imaging vs. natural scenes), creating a need for a unified framework that can handle both isotropic and anisotropic contributions while maintaining structural realism.

Method: The framework uses a Gaussian Splatting backbone to disentangle isotropic and anisotropic contributions, implements a dual-branch conditioning framework (one for CT intensity scattering, one for RGB scenes via normalized camera embeddings), and employs a hybrid pose-agnostic anchoring method for adaptive depth and material sampling.

Result: Superior SSIM and LPIPS performance on Mip-NeRF 360, RealEstate10K, and DeepDRR datasets, with runtime assessments confirming viability for interactive applications. The system enables rapid, photorealistic view synthesis with inherent directionality.

Conclusion: Disentangled360 successfully integrates preoperative radiography simulation and consumer-grade 360° rendering into a single inference pipeline, facilitating mixed-reality medical supervision, robotic perception, and immersive content creation without requiring scene-specific finetuning or expensive photon simulations.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs show promise in VLN but face severe training overhead from processing long-horizon observations as massive token sequences and the exploration-efficiency trade-off in DAgger data aggregation.

Method: Progressive memory (dynamic token allocation to recent observations), learnable recursive memory (key-value cache as memory state), and dynamic mixed policy to balance exploration-efficiency trade-off.
...
Conclusion: Efficient-VLN successfully addresses training efficiency challenges in VLN while maintaining high performance, making practical MLLM development for navigation more feasible.
...
<details>
  <summary>Details</summary>
Motivation: Current weakly supervised semantic segmentation (WSSS) methods in histopathology are limited by inter-class homogeneity, intra-class heterogeneity, and region-shrinkage effects from CAM-based supervision, necessitating better region discovery approaches.

Method: Proposes a prototype-driven framework that integrates CoOp-style learnable prompt tuning to generate text-based prototypes, combines them with learnable image prototypes to form a dual-modal prototype bank, and incorporates a multi-scale pyramid module to address oversmoothing in ViT representations.
...
Conclusion: Jointly leveraging textual semantics and visual prototype learning is effective for WSSS in digital pathology, highlighting the value of vision-language alignment for improving region discovery under weak supervision.
...
Result: Outperforms existing WSSS methods on BCSS-WSSS datasets, produces high-quality pseudo masks without pixel-level annotations, improves localization completeness, enhances semantic consistency across tissue types, and remains computationally efficient with frozen foundation models and lightweight adapters.

Conclusion: The prototype learning framework successfully integrates complementary strengths of vision-language models and segmentation backbones for WSSS in histopathology, achieving better performance while maintaining efficiency through frozen backbones and lightweight training.
...
TL;DR: Point2Pose is a generative 3D human pose estimation framework that models pose distributions using sequential point clouds and pose history, outperforming baselines across datasets.
...
Method: Point2Pose uses spatio-temporal point cloud encoder and pose feature encoder to extract joint-wise features, followed by an attention-based generative regressor. Also introduces MVPose3D dataset with IMU data, multi-view point clouds, and RGB images.

Result: The proposed method outperforms baseline models, demonstrating superior performance across various datasets.

Conclusion: Point2Pose effectively addresses 3D human pose estimation challenges through generative modeling and multi-modal data, showing state-of-the-art performance.
...
TL;DR: EchoingPixels is an audio-visual token reduction framework that uses cross-modal interaction to dynamically reduce tokens from a joint audio-visual pool, achieving 5-20% token usage with 2-3x speedup.


<details>
  <summary>Details</summary>
Motivation: Current token reduction methods for audio-visual LLMs are insufficient because they treat modalities independently and use static budgets, failing to leverage cross-modal synergies and adapt to different information densities in audio and video.

Method: Proposes EchoingPixels with Cross-Modal Semantic Sieve (CS2) that co-attends to joint audio-visual streams and reduces tokens from a combined pool, plus Synchronization-Augmented RoPE (Sync-RoPE) to preserve temporal relationships.

Result: Achieves comparable performance to baselines using only 5-20% of original tokens, with 2-3x speedup and memory reduction.

Conclusion: EchoingPixels effectively addresses the audio-visual token reduction bottleneck by enabling cross-modal interaction and dynamic budget allocation, making AV-LLMs more computationally efficient.
...
TL;DR: StainNet is a specialized pathology foundation model for special stains (like immunohistochemistry) using vision transformers, trained on 1.4M patches from 20K+ WSIs, outperforming H&E-focused models on special stain tasks.


<details>
  <summary>Details</summary>
Motivation: Existing pathology foundation models are primarily pre-trained on H&E-stained images, limiting their effectiveness for clinical applications involving special stains like immunohistochemistry, which are frequently used in practice.

Method: Proposes StainNet, a vision transformer-based foundation model using self-distillation SSL, trained on over 1.4 million patch images from 20,231 publicly available special staining WSIs in the HISTAI database.

Result: Demonstrates strong performance on slide-level liver malignancy classification and two public ROI-level datasets, shows effectiveness in few-ratio learning and retrieval tasks, and outperforms larger existing PFMs on special stain applications.

Conclusion: StainNet addresses the gap in specialized foundation models for special stains, providing a robust solution for clinical applications involving immunohistochemistry and other special staining techniques, with model weights publicly released.
...
TL;DR: ISMVC is a selective imputation method for incomplete multi-view clustering that only imputes missing data when sufficient support exists, avoiding noise from indiscriminate imputation while maintaining cross-view complementarity.


<details>
  <summary>Details</summary>
Motivation: Existing methods for incomplete multi-view clustering have limitations: imputation-based approaches introduce noise and bias when information is insufficient, while imputation-free methods struggle under severe incompleteness due to lack of cross-view complementarity.

Method: ISMVC evaluates imputation-relevant informativeness of each missing position based on intra-view similarity and cross-view consistency, selectively imputing only when sufficient support is available. It integrates this with a variational autoencoder using mixture-of-Gaussians prior for clustering-friendly latent representations, performing distribution-level imputation to stabilize aggregation and model uncertainty.

Result: Extensive experiments on multiple benchmark datasets under realistic unbalanced missing scenarios demonstrate that ISMVC outperforms both imputation-based and imputation-free approaches.

Conclusion: ISMVC provides a lightweight, data-driven, model-agnostic solution that can be integrated as a plug-in module into existing incomplete multi-view clustering models, offering robust performance by balancing the benefits of imputation while avoiding its pitfalls through selective application.
...
TL;DR: Proposes a conditional generative framework using Pix2Pix to generate realistic filament images from binary masks, addressing data shortage in filament segmentation.


<details>
  <summary>Details</summary>
Motivation: Filament segmentation is crucial for biological analysis but suffers from data shortage due to the extreme difficulty of manual pixel-level annotation of dense, elongated structures like microtubules and actin filaments.

Method: Uses conditional generative framework based on Pix2Pix architecture to generate realistic filament microscopy images from binary masks, with a novel filament-aware structural loss to improve structural similarity.
...
Conclusion: The proposed generative framework successfully addresses the data shortage problem in filament segmentation by generating realistic synthetic images, enabling better training of segmentation models.
...
TL;DR: HD-CLIP is a plug-and-play module that decomposes low-quality images into semantic and ordinal degradation embeddings to guide diffusion models for real-world image super-resolution, improving generalization across unknown degradation levels.


<details>
  <summary>Details</summary>
Motivation: Real-world image super-resolution faces challenges with diverse, coupled degradations where existing methods assume known degradation severity and rely on CLIP text encoders that cannot capture numerical severity information, limiting generalization ability.

Method: Proposes HD-CLIP (Hierarchical Degradation CLIP) that decomposes low-quality images into semantic embedding and ordinal degradation embedding capturing ordered relationships, integrated into diffusion models via classifier-free guidance and proposed classifier-free projection guidance.

Result: HD-CLIP significantly improves detail fidelity and perceptual realism across diverse real-world datasets, can be seamlessly integrated into various super-resolution frameworks without training as a plug-and-play module.

Conclusion: HD-CLIP effectively addresses the generalization limitations of existing methods by providing richer degradation guidance, suppressing hallucinations and artifacts while enhancing restoration quality in real-world super-resolution scenarios.
...
TL;DR: CoSPlan benchmark evaluates VLMs on error-prone sequential planning with error detection and correction. VLMs struggle despite advanced reasoning methods. Proposed SGI method improves performance by 5.2% through intermediate reasoning steps.


<details>
  <summary>Details</summary>
Motivation: While large-scale Vision-Language Models (VLMs) show impressive reasoning capabilities, their performance in visual sequential planning with error correction remains unexplored. Real-world planning often involves non-optimal steps that need detection and correction.

Method: Proposes CoSPlan benchmark with 4 domains (maze navigation, block rearrangement, image reconstruction, object reorganization) to evaluate VLMs on Error Detection and Step Completion. Introduces Scene Graph Incremental updates (SGI), a training-free method that adds intermediate reasoning steps between initial and goal states.

Result: State-of-the-art VLMs (Intern-VLM, Qwen2) struggle on CoSPlan even with Chain-of-Thought and Scene Graphs. SGI method yields average 5.2% performance gain and generalizes to traditional planning tasks like Plan-Bench and VQA.

Conclusion: VLMs have limitations in error-prone sequential planning despite advanced reasoning techniques. SGI's intermediate reasoning steps effectively enhance VLM performance in corrective sequential planning and generalize to other planning tasks.
...
Result: Method generates temporally coherent, physically plausible, semantically aligned motions given text prompts and target skeletons, enabling cross-species motion style transfer.

Conclusion: OmniZoo addresses core limitations in motion generation by providing both large-scale heterogeneous animal motion data and a unified framework capable of handling arbitrary skeletal topologies with textual conditioning.
...
TL;DR: TranSamba is a hybrid Transformer-Mamba architecture for weakly supervised volumetric medical segmentation that combines Vision Transformers with Cross-Plane Mamba blocks to efficiently capture 3D context with linear complexity.


<details>
  <summary>Details</summary>
Motivation: Existing weakly supervised semantic segmentation approaches for medical imaging often use 2D encoders that ignore the volumetric nature of the data, limiting their ability to capture 3D context which is crucial for accurate medical image analysis.
...
Result: TranSamba achieves new state-of-the-art performance on three datasets, consistently outperforming existing methods across diverse modalities and pathologies. It enables effective volumetric modeling with linear time complexity relative to input volume depth and constant memory usage for batch processing.

Conclusion: TranSamba successfully addresses the limitations of 2D approaches by providing an efficient 3D modeling solution for weakly supervised volumetric medical segmentation, offering both computational efficiency and superior performance across various medical imaging applications.
...
<details>
  <summary>Details</summary>
Motivation: mmWave radars struggle with detecting static people in dense groups due to spatial resolution limits and reliance on movement. Existing breathing rate estimation methods assume known person count, creating a need for technology that can count static individuals in crowded spaces.

Method: Uses novel multi-stage signal processing pipeline to extract ultra-low frequency (<1 Hz) signals from breathing and micro-scale body movements, differentiates these from background noise, extracts spatial information, and maps sources to individual people for counting.

Result: Achieves 87% average F1 score and 0.6 mean absolute error in familiar environments, 60% average F1 score and 1.1 mean absolute error in new environments. Can count up to 7 people in 3 square meters with minimal spacing constraints.

Conclusion: mmCounter successfully addresses the challenge of counting static people in dense indoor environments using mmWave radar and advanced signal processing, outperforming existing approaches that require movement or prior knowledge of person count.
...
TL;DR: STAR framework equips MLLMs with a comprehensive Video Toolkit to enhance spatiotemporal reasoning for VideoQA, achieving significant gains on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs struggle with simultaneously modeling spatial relationships within video frames and understanding causal dynamics of temporal evolution on complex VideoQA tasks.

Method: Proposes a Spatiotemporal Reasoning Framework (STAR) with comprehensive Video Toolkit, strategically scheduling temporal and spatial tools to progressively localize key areas in videos.

Result: STAR enhances GPT-4o using lightweight tools, achieving 8.2% gain on VideoMME and 4.6% on LongVideoBench.

Conclusion: The Video Toolkit and STAR framework represent an important step toward building autonomous and intelligent video analysis assistants.
...
TL;DR: Visual Funnel addresses "Contextual Blindness" in MLLMs by using a training-free, two-step approach with hierarchical cropping to preserve both fine details and global context.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with fine-grained visual perception despite good reasoning capabilities. Existing cropping methods cause "Contextual Blindness" - a structural disconnect between high-fidelity details from crops and the broader global context, limiting performance in precision-demanding tasks.

Method: Visual Funnel: 1) Contextual Anchoring identifies region of interest in single forward pass; 2) Constructs Entropy-Scaled Portfolio with hierarchical crops (from focal detail to broader surroundings) by dynamically determining crop sizes based on attention entropy and refining crop centers.
...
Conclusion: The problem is not information quantity but structural diversity in input. Visual Funnel's hierarchical portfolio structure effectively resolves Contextual Blindness in MLLMs without requiring training.
...
TL;DR: P2S is a novel zero-shot framework for long video moment retrieval that addresses computational inefficiency in search phases and high-cost VLM verification in refine phases through adaptive span generation and query decomposition.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to Zero-shot Long Video Moment Retrieval (ZLVMR) face severe limitations: supervised methods have poor scalability and generalization despite high resource consumption, while zero-shot methods suffer from candidate explosion in search phases and require costly VLM verification in refine phases.

Method: P2S introduces two key innovations: (1) an 'Adaptive Span Generator' to prevent candidate explosion in the search phase, and (2) 'Query Decomposition' to refine candidates without relying on high-cost VLM verification, making it a training-free framework.

Result: P2S outperforms supervised state-of-the-art methods by significant margins (e.g., +3.7% on R5@0.1 on MAD dataset), becoming the first zero-shot framework capable of temporal grounding in hour-long videos.

Conclusion: P2S successfully addresses the dual challenges of inefficient search and costly refinement in ZLVMR, demonstrating superior performance over existing supervised methods while being training-free and computationally efficient.
...
TL;DR: CoherentGS is a novel framework for high-fidelity 3D reconstruction from sparse and blurry images using dual-prior generative models to break the vicious cycle between sparse views and motion blur.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) relies on dense, high-quality input imagery, but real-world applications often have sparse and motion-blurred data. This creates a vicious cycle where sparse views ignore multi-view constraints needed to resolve motion blur, while motion blur erases high-frequency details crucial for aligning limited views, leading to catastrophic reconstruction failures with fragmented views and low-frequency bias.

Method: CoherentGS uses a dual-prior strategy combining two pre-trained generative models: a specialized deblurring network for restoring sharp details and providing photometric guidance, and a diffusion model that offers geometric priors to fill in unobserved regions. The framework includes a consistency-guided camera exploration module that adaptively guides the generative process, and a depth regularization loss that ensures geometric plausibility.

Result: CoherentGS significantly outperforms existing methods on both synthetic and real-world scenes with as few as 3, 6, and 9 input views, setting a new state-of-the-art for this challenging task.

Conclusion: CoherentGS successfully breaks the vicious cycle between sparse views and motion blur in 3D reconstruction by leveraging dual-prior generative models, enabling high-fidelity reconstruction from severely degraded input data where previous methods fail catastrophically.
...
TL;DR: First joint scene flow learning framework for 4D radar and LiDAR fusion, addressing challenges of radar noise/sparsity through novel fusion module and loss functions, outperforming single-modal methods.


<details>
  <summary>Details</summary>
Motivation: Radar complements LiDAR for scene flow estimation (cheaper, weather-robust, provides velocity), but radar-LiDAR fusion remains unexplored. No existing dataset combines both modalities for scene flow, and radar data has noise/low-resolution challenges.

Method: 1) Construct Radar-LiDAR scene flow dataset from public automotive data with preprocessing for radar denoising and reliable flow ground truth generation. 2) Propose RaLiFlow framework with Dynamic-aware Bidirectional Cross-modal Fusion (DBCF) module that integrates radar dynamic cues into local cross-attention. 3) Design loss functions to mitigate unreliable radar data effects and enhance instance-level consistency.

Result: Extensive experiments show method significantly outperforms existing LiDAR-based and radar-based single-modal methods on the repurposed scene flow dataset.

Conclusion: Successfully demonstrates first effective radar-LiDAR fusion for scene flow estimation, overcoming radar data challenges through novel fusion architecture and training strategies, establishing new benchmark for multimodal scene flow estimation.
...
TL;DR: A self-supervised deep learning pipeline for feature matching in endoscopic surgery images that outperforms state-of-the-art methods on SCARED datasets.


<details>
  <summary>Details</summary>
Motivation: Surgical endoscopy requires precise pixel-level correspondences for 3D reconstruction and tracking, but faces challenges like weak perspective cues, tissue reflections, and deformable anatomy that degrade conventional computer vision techniques.

Method: Uses novel-view synthesis to generate ground-truth correspondences, then employs contrastive learning with triplets to augment DINOv2 backbone with an additional Transformer layer optimized for direct matching via cosine similarity thresholding.

Result: Outperforms state-of-the-art methods on SCARED datasets with improved matching precision and lower epipolar error.

Conclusion: The framework enables more accurate high-level computer vision applications in surgical endoscopy through self-supervised feature correspondence learning.
...
TL;DR: FROW benchmark addresses LVLMs' neglect of fine-grained recognition, introducing mosaic and open-world data to improve performance by 10-20% on fine-grained tasks.


<details>
  <summary>Details</summary>
Motivation: Existing LVLM benchmarks focus too much on reasoning tasks while neglecting fine-grained recognition, which is crucial for practical applications. There's a need for better evaluation and improvement of fine-grained recognition capabilities in LVLMs.

Method: 1) Introduce FROW benchmark for fine-grained recognition evaluation using GPT-4o; 2) Propose optimization strategy with mosaic data (combined short-answer responses) and open-world data (real-world Q&A generated by GPT-4o); 3) Incorporate fine-grained data into pre-training phase.

Result: Mosaic data improves category recognition accuracy by 1%; Open-world data boosts FROW benchmark accuracy by 10-20% and content accuracy by 6-12%; Fine-grained data in pre-training improves category recognition accuracy by up to 10%.

Conclusion: The FROW benchmark effectively addresses the fine-grained recognition gap in LVLM evaluation, and the proposed data construction and training optimization strategies significantly improve LVLM performance on fine-grained recognition tasks.
...
<details>
  <summary>Details</summary>
Motivation: LiDAR point clouds often contain noise that degrades object detection accuracy. Existing methods trade off between computational efficiency and denoising quality, making it difficult to achieve both high accuracy and real-time performance.

Method: 1) Octree spatial partitioning for parallel acceleration; 2) Adaptive voxel-based occupancy statistics and kNN density estimation to remove isolated/low-density noise; 3) Gravitational scoring function combining density weights with adaptive distance weights for fine noise discrimination.

Result: Outperforms existing methods on Stanford 3D, CADC, and in-house FMCW LiDAR datasets, showing consistent improvements in F1, PSNR, and Chamfer Distance across various noise conditions while reducing single-frame processing time.

Conclusion: The proposed method achieves high denoising accuracy, strong edge preservation, and real-time performance simultaneously, validated by experiments across multiple datasets and noise scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing research focuses on video-level hate speech classification but neglects temporal localization (identifying when hateful segments occur), especially under weak supervision where only video-level labels are available. Current methods struggle with cross-modal and temporal dynamics in multimodal hate speech.

Method: Proposes MultiHateLoc with three key components: (1) modality-aware temporal encoders for heterogeneous sequential patterns with text preprocessing, (2) dynamic cross-modal fusion to emphasize informative modalities per moment plus contrastive alignment for feature consistency, (3) modality-aware MIL objective to identify discriminative segments under video-level supervision.

Result: Achieves state-of-the-art performance on HateMM and MultiHateClip datasets for temporal localization task, producing fine-grained frame-level predictions despite using only coarse video-level labels.
...
TL;DR: This paper introduces WildRoad dataset and MaGRoad model for off-road road network extraction, addressing domain gap and structural weaknesses in existing methods.


<details>
  <summary>Details</summary>
Motivation: Current deep learning models for vectorized road extraction work well in urban settings but fail in off-road environments due to domain gap, lack of large-scale datasets, and structural weaknesses in node-centric approaches that are fragile to occlusions and ambiguous junctions.

Method: Two complementary approaches: 1) Release WildRoad dataset - a global off-road road network dataset created with a dedicated interactive annotation tool; 2) Introduce MaGRoad (Mask-aware Geodesic Road network extractor) - a path-centric framework that aggregates multi-scale visual evidence along candidate paths for robust connectivity inference.
...
Method: TransLocNet uses a cross-modal attention framework: 1) LiDAR scans are projected to bird's-eye-view, 2) bidirectional attention aligns LiDAR geometry with aerial semantic features, 3) a likelihood map decoder outputs spatial probability distributions for position and orientation, 4) contrastive learning enforces a shared embedding space for better cross-modal alignment.

Result: Experiments on CARLA (synthetic) and KITTI (real-world) datasets show TransLocNet outperforms state-of-the-art baselines, reducing localization error by up to 63% and achieving sub-meter, sub-degree accuracy in both synthetic and real-world settings.

Conclusion: TransLocNet provides robust and generalizable aerial-ground localization by effectively bridging the viewpoint and modality gaps between ground LiDAR and aerial imagery through cross-modal attention and contrastive learning.
...
TL;DR: NCTTA is a novel Test-Time Adaptation method that addresses performance degradation under domain shifts by correcting sample-wise feature-classifier misalignment using hybrid targets that combine geometric proximity and predictive confidence.
...
Method: Extends Neural Collapse to sample-wise level (NC3+), identifies feature-classifier misalignment as the problem, and proposes NCTTA with hybrid targets that blend geometric proximity (alignment with classifier weights) with predictive confidence to handle unreliable pseudo-labels.

Result: NCTTA significantly outperforms existing methods, achieving 14.52% improvement over Tent on ImageNet-C, demonstrating enhanced robustness to domain shifts.

Conclusion: Sample-wise feature-classifier misalignment is the fundamental cause of TTA performance degradation under domain shifts, and NCTTA's hybrid target approach effectively addresses this by realigning features while handling unreliable pseudo-labels.
...
TL;DR: Real-time mobile system for physiotherapy exercise analysis using pose estimation, angle features, and dynamic programming for sequence matching.
...
Method: 1) Extract body keypoints using pose-estimation neural network from camera input; 2) Transform keypoints into trigonometric angle-based features; 3) Classify with lightweight supervised models for frame-level predictions; 4) Use dynamic programming with modified Levenshtein distance for full exercise sequence matching and deviation detection.

Result: System operates entirely client-side with real-time performance, effectively identifies and evaluates physiotherapy exercises, and demonstrates applicability to remote supervision and m-health.

Conclusion: Proposed framework provides efficient, scalable solution for real-time physiotherapy exercise monitoring on mobile devices, enabling remote healthcare applications.
...
TL;DR: Proposes a unified-transform video compression framework with dual-domain temporal alignment and quality-conditioned MoE to eliminate error propagation while maintaining competitive R-D performance.
...
Method: 1) Dual-domain progressive temporal alignment: coarse pixel-domain alignment for simple motion + refined latent-domain alignment using Flow-Guided Deformable Transformer for complex motion. 2) Quality-Conditioned Mixture-of-Expert (QCMoE) module for continuous bit-rate adaptation that dynamically assigns experts per pixel based on target quality and content.
...
Conclusion: The proposed unified-transform framework with dual-domain alignment and QCMoE effectively addresses the error propagation vs. temporal alignment dilemma in learned video compression, enabling high-quality streaming without error accumulation.
...
TL;DR: Hybrid Shape-from-Focus method combining traditional DDL kernels for focus volume computation with lightweight GRU-based iterative refinement and learned upsampling, outperforming both deep learning and traditional approaches.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning SFF methods have issues: they use heavy feature encoders for focus volume extraction, then use simple one-step aggregation that introduces artifacts and amplifies noise in depth maps.

Method: 1) Compute multi-scale focus volumes traditionally using handcrafted Directional Dilated Laplacian (DDL) kernels to capture long-range directional focus variations. 2) Feed these into lightweight multi-scale GRU-based depth extraction module that iteratively refines initial depth estimate at lower resolution. 3) Use learned convex upsampling module within recurrent network to reconstruct high-resolution depth maps while preserving details and boundaries.
...
Conclusion: The hybrid framework combining traditional focus volume computation with lightweight neural refinement offers an effective solution for SFF that addresses limitations of existing methods while maintaining computational efficiency and preserving fine details.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of datasets for 3D blood pulsation analysis, which is needed to improve remote pulse estimation methods via photoplethysmography imaging and develop illumination-robust approaches.

Method: Created a multi-view dataset with raw videos from 15 subjects recorded at 30 Hz from 23 viewpoints, with blood pulse reference measurements and facial 3D scans generated using monocular structure-from-motion. Processed 3D pulsation maps compatible with FLAME 3D head model texture space.

Result: Dataset includes SNR, local pulse amplitude, phase information, and supplementary data. Comprehensive evaluation shows the dataset captures physiologically meaningful features in facial and neck skin regions with consistent illumination conditions and map consistency.

Conclusion: Pulse3DFace enables development of dynamic facial blood pulsation models, synthetic video generation for remote photoplethysmography validation, and novel multi-view approaches for illumination-robust blood pulsation analysis.
...
TL;DR: TaP enhances few-shot semantic segmentation by using LoRA to adapt encoders to novel classes with minimal computation, improving performance across benchmarks and domains.


<details>
  <summary>Details</summary>
Motivation: Prior FSS research focused on decoders, but encoder limitations in extracting features for unseen classes remain a key bottleneck. Need for better encoder adaptability to novel classes in both standard and cross-domain FSS settings.

Method: TaP (Take a Peek) uses Low-Rank Adaptation (LoRA) to fine-tune the encoder on the support set with minimal computational overhead. It's model-agnostic and integrates seamlessly into existing FSS pipelines, enabling fast adaptation while preventing catastrophic forgetting.

Result: Extensive experiments on COCO 20^i, Pascal 5^i, and cross-domain datasets (DeepGlobe, ISIC, Chest X-ray) show consistent performance improvements across diverse models and shot settings. Significant gains in complex multi-class scenarios, with strong performance even using low-rank adaptations for computational efficiency.

Conclusion: TaP addresses the critical limitation of encoder generalization to novel classes in FSS, paving the way for more robust, efficient, and generalizable segmentation systems. The method demonstrates practical effectiveness in realistic settings.
...
TL;DR: Blink: A dynamic visual token resolution framework that emulates human "blink-like" visual perception in MLLMs by selectively allocating computation to salient regions through saliency-guided scanning and token super-resolution.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs have limited visual perception compared to humans who efficiently perceive complex scenes by dynamically scanning and focusing on salient regions in a sequential "blink-like" process. The authors investigate whether MLLMs exhibit similar behavior and find they naturally attend to different visual regions across layers, with selective computation allocation to salient tokens enhancing perception.

Method: Blink framework includes two modules: 1) Saliency-guided scanning that estimates visual token saliency based on attention maps, and 2) Dynamic token resolution that extends important tokens through a plug-and-play token super-resolution (TokenSR) module, then drops extended tokens when they lose focus in subsequent layers. This creates a dynamic mechanism balancing broad exploration and fine-grained focus.

Result: Extensive experiments validate Blink's effectiveness in enhancing visual perception and multimodal understanding, demonstrating improved performance through adaptive and efficient computation allocation to salient visual regions.

Conclusion: Blink successfully emulates human-inspired visual perception strategies in MLLMs, providing a dynamic framework that enhances visual perception through selective computation allocation to salient regions, balancing exploration and focus for improved multimodal understanding.
...
Method: GETok integrates specialized learnable tokens: grid tokens partition image into spatial anchors, and offset tokens enable iterative refinement of localization predictions.
...
Conclusion: GETok effectively embeds spatial relationships into tokens, advancing MLLMs' native 2D space reasoning while maintaining existing architecture.
...
<details>
  <summary>Details</summary>
Motivation: ISLR faces data scarcity and long-tail distribution problems where gathering sufficient examples for thousands of signs is expensive. Standard classifiers overfit frequent classes and fail on rare ones, creating a bottleneck for robust sign language recognition.

Method: Uses few-shot prototypical network framework with skeleton-based encoder. Integrates Spatiotemporal Graph Convolutional Network (ST-GCN) with novel Multi-Scale Temporal Aggregation (MSTA) module to capture rapid and fluid motion dynamics. Employs episodic training to learn semantic metric space where signs are classified based on proximity to dynamic class prototypes.

Result: Achieves 43.75% Top-1 and 77.10% Top-5 accuracy on WLASL dataset, outperforming standard classification baseline with identical backbone by over 13%. Shows strong zero-shot generalization with nearly 30% accuracy on unseen SignASL dataset without fine-tuning.

Conclusion: The prototypical training strategy effectively addresses data scarcity where standard classification fails. The approach offers scalable pathway for recognizing extensive sign vocabularies with limited data through metric learning paradigm and strong generalization capabilities.
...
TL;DR: AVI-Edit is a framework for audio-sync video instance editing that addresses audio-visual synchronization and fine-grained spatial-temporal control through mask refinement and audio guidance.


<details>
  <summary>Details</summary>
Motivation: Existing video editing methods overlook audio-visual synchronization and lack fine-grained spatial and temporal controllability needed for precise instance-level edits in video content.

Method: Proposes AVI-Edit with two key components: 1) granularity-aware mask refiner that iteratively refines coarse user masks into precise instance-level regions, and 2) self-feedback audio agent that curates high-quality audio guidance for temporal control. Also constructs a large-scale dataset with instance-centric correspondence and comprehensive annotations.
...
Conclusion: AVI-Edit successfully addresses the limitations of existing video editing methods by providing precise instance-level editing with audio-visual synchronization, enabled by mask refinement and audio guidance components.
...
TL;DR: A symmetric U-Net architecture (SymUNet) achieves state-of-the-art all-in-one image restoration with simpler design than complex existing methods, plus a semantic-enhanced variant (SE-SymUNet) using CLIP features.
...
Method: Proposes SymUNet: a symmetric U-Net architecture that aligns feature scales across encoder-decoder for streamlined cross-scale propagation, preserving degradation signals with simple additive skip connections. Also proposes SE-SymUNet variant that integrates CLIP semantic features via cross-attention to amplify degradation priors.

Result: Achieves better results across benchmark datasets than existing approaches while reducing computational cost. Both SymUNet and SE-SymUNet establish simpler, stronger foundations for all-in-one image restoration.

Conclusion: Well-crafted feature extraction inherently encodes degradation information, and symmetric U-Net architecture is sufficient for state-of-the-art performance without complex designs, providing a simpler foundation for future advancements.
...
TL;DR: A weather-robust salient object detection framework with noise indicator fusion for handling diverse weather conditions.


<details>
  <summary>Details</summary>
Motivation: Existing SOD methods assume clean visual conditions and fail in real-world scenarios with weather-induced noise, degrading segmentation accuracy.

Method: Proposes a SOD framework with specific encoder and replaceable decoder. Introduces one-hot noise indicator vector and Noise Indicator Fusion Module (NIFM) that fuses semantic features with weather priors via adaptive feature modulation between encoder stages.

Result: Extensive experiments on WXSOD dataset show the framework (especially NIFM-enhanced encoder) improves segmentation accuracy under complex weather conditions compared to vanilla encoder across different training data scales and configurations.

Conclusion: The proposed weather-aware SOD framework effectively handles weather-induced noise while maintaining compatibility with mainstream SOD decoders, advancing real-world robustness.
...
TL;DR: Training-free semantic retrieval of remote sensing images using VLM-generated structured text descriptions as queries and database entries, achieving competitive performance with supervised methods.


<details>
  <summary>Details</summary>
Motivation: Address the semantic gap in remote sensing image retrieval and the limitations of existing methods that require costly domain-specific training, while lacking benchmarks for evaluating VLM-generated text in zero-shot retrieval contexts.

Method: Introduce RSRT dataset with structured captions, propose TRSLLaVA - a training-free text-only retrieval framework that reformulates cross-modal retrieval as text-to-text matching using VLM-generated captions in a unified textual embedding space.

Result: Achieves 42.62% mean Recall on RSITMD, nearly doubling the standard zero-shot CLIP baseline (23.86%) and surpassing several top supervised models, demonstrating competitive performance without training.

Conclusion: High-quality semantic representation through structured text provides a powerful and cost-effective paradigm for remote sensing image retrieval, validating the effectiveness of training-free VLM-based approaches.
...
<details>
  <summary>Details</summary>
Motivation: Video understanding in challenging conditions (occlusion, camouflage, rapid movement) depends more on motion dynamics than static appearance. Current methods often require user queries, but motion patterns aligned with vision-language representations can provide powerful semantic signals for recognizing and describing actions autonomously.

Method: TCAM uses a motion-centric framework that: 1) autonomously observes videos to identify multiple motion activities, 2) employs motion-field attention mechanism to spatially ground natural language descriptions to corresponding trajectories, 3) aligns motion patterns with contrastive vision-language representations, and 4) uses unified training combining global video-text alignment with fine-grained spatial correspondence via multi-head cross-attention.

Result: On the MeViS benchmark: 58.4% video-to-text retrieval accuracy, 64.9 JF for spatial grounding, discovers 4.8 relevant expressions per video with 84.7% precision. Demonstrates strong cross-task generalization capabilities.

Conclusion: TCAM successfully enables query-free discovery of multiple motion expressions in videos by leveraging motion patterns as semantic signals. The motion-field attention mechanism effectively grounds descriptions to trajectories, and the unified training approach allows for strong performance across multiple video understanding tasks.
...
Method: Pipeline combining deep feature extraction with interpretable image processing modules. Uses high-fidelity retinal vessel segmentation as auxiliary task to guide classification process. Focuses on grounding model predictions in clinically relevant morphological features.

Result: Not explicitly stated in abstract, but implied goals include: bridging gap between algorithmic output and expert medical validation, reducing false positives, and improving deployment viability in clinical settings.

Conclusion: Interpretable deep learning approach for ocular disease diagnosis can enhance clinical trust and practical deployment by making predictions more transparent and grounded in medically relevant features like retinal vessel morphology.
...
TL;DR: Lang2Motion generates object motion trajectories from text by aligning motion manifolds with CLIP embedding spaces, outperforming video-based methods on text-to-trajectory retrieval and motion accuracy.


<details>
  <summary>Details</summary>
Motivation: Prior work focuses on human motion or video synthesis, but there's a need for generating explicit motion trajectories for arbitrary objects from language descriptions using real-world video data.

Method: Transformer-based auto-encoder learns trajectory representations through dual supervision: textual motion descriptions and rendered trajectory visualizations, both mapped through CLIP's frozen encoders. Uses point tracking from real-world videos to extract motion trajectories.
...
Conclusion: Lang2Motion effectively generates and retrieves motion trajectories from language, demonstrates cross-domain transfer capabilities, and supports applications like style transfer, semantic interpolation, and latent-space editing through CLIP-aligned representations.
...
TL;DR: DOCR-Inspector is a VLM-based system that formalizes document parsing assessment as fine-grained error detection and analysis, outperforming commercial models and providing guidance for parsing refinement.


<details>
  <summary>Details</summary>
Motivation: Current document parsing evaluation relies on standard benchmarks with dataset-specific biases, providing only overall scores that obscure error patterns, making it difficult to reliably assess real-world parsing quality.
...
Result: DOCR-Inspector-7B outperforms commercial models like Gemini 2.5 Pro and leading open-source models on DOCRcaseBench (882 real-world cases). Its assessments provide valuable guidance for parsing refinement.

Conclusion: DOCR-Inspector serves as both a practical evaluator and a driver for advancing document parsing systems, offering comprehensive quality assessment that addresses limitations of current benchmark-based evaluation approaches.
...
TL;DR: K-Track is a hybrid acceleration framework that combines sparse deep learning keyframe updates with lightweight Kalman filtering to enable 5-10X speedup for point tracking on edge devices while maintaining over 85% accuracy.


<details>
  <summary>Details</summary>
Motivation: Current deep learning-based point trackers achieve state-of-the-art accuracy but rely on per-frame GPU inference, making them impractical for deployment on resource-constrained edge devices with limited compute, power, and connectivity.
...
Result: The framework achieves 5-10X speedup while retaining over 85% of original trackers' accuracy, enabling real-time performance on edge platforms like NVIDIA Jetson Nano and RTX Titan across multiple state-of-the-art point trackers.

Conclusion: K-Track provides a practical solution for deploying high-quality point tracking in resource-limited settings, bridging the gap between modern tracking algorithms and deployable vision systems for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Advances in generative modeling have made it easy to fabricate realistic portrayals of individuals, creating serious risks for security, communication, and public trust. There's a need for systems that not only detect manipulated content but also provide clear and reliable reasoning.
...
Conclusion: NaviHydra effectively addresses the challenge of combining navigation command compliance with end-to-end learning, providing a robust solution for controllable autonomous driving that aligns trajectories with specified intentions while maintaining safety.
...
TL;DR: XDen-1K is the first large-scale multi-modal dataset for real-world physical property estimation, focusing on volumetric density, with 1,000 objects across 148 categories including 3D models and X-ray scans, enabling improved center-of-mass estimation and robotic manipulation.


<details>
  <summary>Details</summary>
Motivation: Current AI models capture object surface geometry and appearance but neglect internal physical properties like volumetric density, which are critical for predicting center of mass, stability, and interaction dynamics in robotics and simulation applications. The main bottleneck has been the lack of large-scale real-world data.

Method: 1) Introduce XDen-1K dataset with 1,000 real-world objects across 148 categories, providing high-resolution 3D geometric models with part-level annotations and corresponding biplanar X-ray scans. 2) Develop a novel optimization framework that recovers high-fidelity volumetric density fields from sparse X-ray views. 3) Use X-ray images as conditioning signal for volumetric segmentation and conduct experiments on downstream robotics tasks.
...
Conclusion: XDen-1K serves as a foundational resource and challenging benchmark for physically grounded visual inference and embodied AI research, bridging the gap in physical property estimation and enabling more realistic simulation and robotic interaction with the physical world.
...
Method: Combines foundation model visual features (DINO descriptors) with geometric filtering: computes similarity maps between template descriptors and scene patches, establishes mutual correspondences by projecting scene patches to 3D and template descriptors to object coordinates, recovers poses via correspondence-driven RANSAC, and ranks poses using a weighted geometric alignment metric considering reprojection consistency and spatial support.

Result: Achieves 53.7 AR (average recall) at 1.08 FPS with sub-second inference on a single commodity GPU, matching larger zero-shot baselines while being fully local and training-free.

Conclusion: Geo6DPose enables practical, fully local 6D perception for robotic deployment without training, fine-tuning, or network access, remaining compatible with evolving foundation backbones and addressing real-world robotics constraints.
...
TL;DR: SHOT reformulates SMLM as a set-matching problem using optimal transport loss to eliminate non-differentiable NMS layers, enabling end-to-end training and faster live-cell imaging.


<details>
  <summary>Details</summary>
Motivation: Current SMLM requires non-overlapping fluorophores leading to long acquisition times, hindering live-cell imaging. Existing deep learning approaches use non-differentiable NMS layers that may discard true positives.
...
Conclusion: SHOT enables end-to-end training without NMS, improves localization accuracy at high densities, and accelerates SMLM for live-cell imaging applications.
...
TL;DR: SHARP enables photorealistic view synthesis from a single image using 3D Gaussian representation, achieving real-time rendering with state-of-the-art quality and 1000x faster synthesis.


<details>
  <summary>Details</summary>
Motivation: To enable high-quality, photorealistic novel view synthesis from just a single input image, addressing the need for fast, efficient 3D scene reconstruction that supports metric camera movements.

Method: Uses a neural network to regress parameters of a 3D Gaussian representation from a single photograph in a single feedforward pass (<1 second on GPU). The representation is metric with absolute scale.

Result: Sets new SOTA on multiple datasets: reduces LPIPS by 25-34% and DISTS by 21-43% vs best prior model, while lowering synthesis time by three orders of magnitude (1000x faster). Shows robust zero-shot generalization.

Conclusion: SHARP enables fast, high-quality photorealistic view synthesis from single images with metric 3D representations, achieving significant improvements in both quality and speed over previous methods.
...
TL;DR: This paper introduces uncertainty estimation methods for anatomical landmark-based segmentation in chest X-rays, releasing a large-scale dataset with uncertainty annotations to enhance safe clinical deployment.
...
Method: The authors use hybrid neural network architectures combining standard image convolutional encoders with graph-based generative decoders. They leverage the variational latent space to derive two complementary uncertainty measures: (1) latent uncertainty from learned distribution parameters, and (2) predictive uncertainty from multiple stochastic output predictions generated from latent samples.
...
Conclusion: Uncertainty estimation is a promising direction to enhance robustness and safe deployment of landmark-based anatomical segmentation methods in chest X-ray, with released resources including dataset, interactive demo, and source code to support further research.
...
<details>
  <summary>Details</summary>
Motivation: Current vision language models (VLMs) struggle to understand fine-grained 3D spatial relationships, which is fundamental for autonomous driving systems interacting with the physical world. There's a need to improve spatial reasoning capabilities in VLM-based driving frameworks.

Method: SpaceDrive uses a universal positional encoder for all 3D coordinates from multi-view depth estimation, historical ego-states, and text prompts. These 3D positional encodings are superimposed on 2D visual tokens and serve as task-agnostic coordinate representations, replacing digit-wise numerical tokens for both inputs and outputs. This enables better indexing of visual semantics in spatial reasoning and direct trajectory coordinate regression.
...
Conclusion: Treating spatial information as explicit positional encodings rather than textual digits significantly improves VLM-based autonomous driving systems' spatial reasoning capabilities and planning accuracy, making them more effective for real-world driving tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing video depth estimation methods suffer from either temporal inconsistencies (frame-by-frame monocular models) or computational inefficiency (heavy temporal modeling), limiting practical real-time applications.
...
Conclusion: VeloDepth provides a practical, efficient, and accurate real-time depth estimation solution suitable for diverse perception tasks.
...
TL;DR: IRMoGen introduces a novel paradigm that interleaves motion generation with assessment and refinement through iterative text-motion dialogue, achieving better text-motion alignment and outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Current motion-aware LLMs treat understanding and generation separately, missing opportunities for interactive feedback between tasks. The authors identify motion assessment and refinement as crucial bridges for bidirectional knowledge flow between understanding and generation.
...
Conclusion: The interleaved reasoning paradigm successfully bridges motion understanding and generation through assessment and refinement tasks, demonstrating that tight coupling of these components leads to improved motion generation performance.
...
TL;DR: LDP is a novel framework using multimodal LLMs for professional polyp diagnosis report generation, trained on MMEndo dataset with LoRA and DPO, achieving high clinical scores with 833x lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Traditional automated polyp diagnosis reporting suffers from inconsistencies and hallucinations due to scarcity of high-quality multimodal medical data, creating a need for more reliable clinical reporting systems.

Method: Curated MMEndo multimodal endoscopic dataset, fine-tuned Qwen2-VL-7B backbone using Parameter-Efficient Fine-Tuning (LoRA), and aligned with clinical standards via Direct Preference Optimization (DPO).

Result: Outperformed existing baselines on automated metrics and clinical expert evaluations (Physician Score 7.2/10), reduced training computational costs by 833x compared to full fine-tuning, validated robustness on IU-XRay dataset.

Conclusion: LDP offers a scalable, clinically viable path for primary healthcare polyp diagnosis, demonstrating effectiveness with significantly reduced computational requirements.
...
TL;DR: A diffusion-based regression model predicts coronary blood pressure from CT angiography features, bypassing slow CFD simulations for faster CAD diagnosis.
...
Method: Developed an end-to-end pipeline automating coronary geometry extraction from CCTA, streamlining simulation data generation, and introduced a diffusion-based regression model to predict coronary blood pressure directly from CCTA-derived features without CFD computation during inference.

Result: The model achieves state-of-the-art performance with R² of 64.42%, RMSE of 0.0974, and normalized RMSE of 0.154, outperforming several baseline approaches on simulated coronary hemodynamics data.
...
TL;DR: The paper finds that spatial structure (patch token similarity) rather than global semantic performance drives generation quality in representation alignment, and introduces simple modifications (convolution layer + spatial normalization) that improve convergence.


<details>
  <summary>Details</summary>
Motivation: To investigate what aspect of target representations matters for generation in representation alignment (REPA) - whether it's global semantic information (ImageNet accuracy) or spatial structure (patch token similarity), challenging the prevalent assumption that stronger global performance leads to better generation.

Method: 1) Large-scale empirical analysis across 27 vision encoders and model scales; 2) Introduces iREPA with two simple modifications: replacing MLP projection with convolution layer and adding spatial normalization layer for external representations.
...
Conclusion: The work challenges conventional wisdom about representation alignment, showing spatial information is more critical than global semantics for generation, and provides simple yet effective modifications that improve training efficiency across various settings.
...
TL;DR: GLAT-IRM: Graph Laplacian Attention Transformer with Iterative Refinement Module for prostate cancer grading from whole-slide images, improving patch selection and spatial consistency.
...
Result: Extensive experiments on five public and one private dataset demonstrate superior performance over state-of-the-art methods, achieving higher performance and spatial consistency while maintaining computational efficiency.

Conclusion: The proposed GLAT-IRM framework effectively addresses challenges in prostate cancer grading by improving patch selection and ensuring spatial consistency, leading to better diagnostic performance on whole-slide images.
...
TL;DR: SEPL (Self-Ensemble Post Learning) addresses domain generalization with noisy labels by leveraging intermediate features and ensemble predictions to mitigate spurious feature enlargement.
...
Conclusion: SEPL effectively addresses domain generalization with noisy labels by diversifying feature utilization through self-ensemble post learning, offering a practical solution for real-world scenarios with distribution shifts and label noise.
...
TL;DR: PoseGAM: A geometry-aware multi-view framework for 6D object pose estimation that directly predicts object pose from query and template images without explicit feature matching, achieving state-of-the-art performance with 5.1% average AR improvement.


<details>
  <summary>Details</summary>
Motivation: Existing 6D object pose estimation methods for unseen objects rely on explicit feature correspondences between query images and object models/templates, which can be limiting. The paper aims to eliminate the need for explicit matching while improving generalization to unseen objects.

Method: PoseGAM is a geometry-aware multi-view framework built on multi-view-based foundation model architectures. It integrates object geometry through two complementary mechanisms: explicit point-based geometry and learned features from geometry representation networks. The method uses a query image and multiple template images to directly predict object pose without explicit matching.

Result: The method achieves state-of-the-art performance across multiple benchmarks with an average AR improvement of 5.1% over prior methods, and up to 17.6% gains on individual datasets. A large-scale synthetic dataset with over 190k objects under diverse conditions was created to enhance robustness and generalization.

Conclusion: PoseGAM demonstrates strong generalization to unseen objects by eliminating explicit feature matching and integrating geometry information through complementary mechanisms, showing significant improvements over existing methods in 6D object pose estimation.
...
TL;DR: SWiT-4D converts monocular videos into high-quality animated 3D assets with explicit 4D meshes using a sliding-window transformer approach that leverages existing image-to-3D priors.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale 4D mesh datasets makes training generalizable video-to-4D models challenging. Existing image-to-3D generation models have strong priors from extensive datasets that can be leveraged for 4D generation with minimal 4D supervision.

Method: SWiT-4D uses a Sliding-Window Transformer for lossless, parameter-free temporal 4D mesh generation. It integrates with any Diffusion Transformer-based image-to-3D generator, adding spatial-temporal modeling across video frames while preserving the original single-image forward process. Includes an optimization-based trajectory module for recovering global translation in static-camera videos.

Result: Achieves high-fidelity geometry and stable temporal consistency with only a single short (<10s) video for fine-tuning. Outperforms existing baselines in temporal smoothness on in-domain zoo-test sets and challenging out-of-domain benchmarks (C4D, Objaverse, and in-the-wild videos).

Conclusion: SWiT-4D demonstrates strong data efficiency and practical deployability under extremely limited 4D supervision, enabling 4D mesh reconstruction from videos of arbitrary length while leveraging powerful image-to-3D priors.
...
TL;DR: MiSI-Bench is a new benchmark for evaluating Vision-Language Models' ability to perceive and reason about microscopic spatial relationships, with over 163K QA pairs and 587K images from molecular structures. Current VLMs perform below human level, but fine-tuned models show promise in spatial transformations while struggling with domain-specific scientific tasks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need to assess Vision-Language Models' capabilities in microscopic spatial intelligence (MiSI) - the ability to perceive and reason about spatial relationships of invisible microscopic entities, which is crucial for scientific discovery but currently under-evaluated in AI benchmarks.
...
Result: Current state-of-the-art VLMs perform significantly below human level on MiSI-Bench. However, a fine-tuned 7B model shows substantial potential, even surpassing humans in spatial transformation tasks, while performing poorly in scientifically-grounded tasks like hydrogen bond recognition.

Conclusion: The benchmark reveals VLMs' limitations in microscopic spatial reasoning and highlights the necessity of integrating explicit domain knowledge for progress toward scientific AGI. The datasets are publicly available for further research.
...
<details>
  <summary>Details</summary>
Motivation: Existing motion capture pipelines are limited to specific species or templates, creating a gap for generalizable motion capture that works with arbitrary 3D assets. The authors formalize this as Category-Agnostic Motion Capture (CAMoCap) to enable scalable, prompt-driven animation for any rigged asset.

Method: The framework uses a reference-guided, factorized approach with three learnable modules: (1) Reference Prompt Encoder extracts joint queries from asset skeleton, mesh, and rendered images; (2) Video Feature Extractor computes visual descriptors and reconstructs coarse 4D deforming mesh; (3) Unified Motion Decoder fuses cues to produce temporally coherent joint trajectories, followed by constraint-aware inverse kinematics to recover asset-specific rotations.

Result: The system delivers high-quality skeletal animations and exhibits meaningful cross-species retargeting across heterogeneous rigs. Experiments on in-domain benchmarks and in-the-wild videos demonstrate effective performance, enabled by the curated Truebones Zoo dataset with 1038 motion clips providing standardized skeleton-mesh-render triads.

Conclusion: MoCapAnything successfully addresses the Category-Agnostic Motion Capture problem, enabling scalable, prompt-driven 3D motion capture for arbitrary assets through a novel reference-guided factorization approach that bridges the gap between video input and joint space animation.
...
<details>
  <summary>Details</summary>
Motivation: Progress in table extraction using vision-language models has been limited due to lack of annotated data, especially for multi-page table structure recognition.

Method: Created PubTables-v2 dataset supporting multiple table extraction tasks, used it to evaluate domain-specialized VLMs, and developed POTATR - an image-to-graph extension of Table Transformer for page-level table extraction.

Result: PubTables-v2 enables evaluation of current methods on challenging table extraction tasks, and POTATR demonstrates comprehensive page-level table extraction capabilities.

Conclusion: PubTables-v2 addresses the data bottleneck in table extraction research, enabling better evaluation and development of methods like POTATR for comprehensive document understanding.
...
TL;DR: DuetSVG is a multimodal model that jointly generates image tokens and SVG tokens to create better vector graphics from text descriptions, using visual guidance to improve geometric coherence and visual appeal.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based SVG generation methods produce only text during decoding, lacking visual signals, which leads to struggles with complex semantics and poor visual/geometric quality in generated SVGs.

Method: DuetSVG is a unified multimodal model trained on both image and SVG datasets that jointly generates image tokens and corresponding SVG tokens end-to-end, with a novel test-time scaling strategy that uses the model's visual predictions to guide SVG decoding.
...
Conclusion: Joint multimodal generation of both visual and vector representations with test-time visual guidance significantly improves SVG generation quality compared to text-only approaches.
...
TL;DR: FoundationMotion introduces an automated pipeline to create large-scale motion datasets using object tracking and LLMs, enabling models to achieve state-of-the-art motion understanding performance.


<details>
  <summary>Details</summary>
Motivation: Current motion understanding models struggle due to scarcity of large-scale, fine-grained motion datasets, which are expensive to create through manual annotation.
...
TL;DR: Real-time talking head generation using 3D Morphable Model-guided Gaussian Splatting with transformer-based audio-to-parameter prediction for temporal stability.
...
Conclusion: The proposed approach addresses temporal instability in real-time talking head generation by combining 3D Morphable Model-guided Gaussian Splatting with transformer-based audio parameter prediction, achieving both real-time performance and visual fidelity.
...
<details>
  <summary>Details</summary>
Motivation: To overcome limitations of autoregressive token generation in VLMs by focusing on semantic representations rather than surface-level linguistic variability, enabling more efficient and versatile vision-language models.

Method: Uses Joint Embedding Predictive Architecture to predict continuous embeddings of target texts in abstract representation space, with a lightweight text decoder invoked only when needed for text generation.

Result: Achieves stronger performance with 50% fewer trainable parameters than standard token-space VLMs, supports 2.85x reduction in decoding operations, and outperforms CLIP/SigLIP2 on video tasks while matching classical VLMs on VQA with only 1.6B parameters.

Conclusion: VL-JEPA demonstrates that predicting continuous embeddings in abstract representation space is more efficient and effective than token-space generation, enabling versatile performance across generation, classification, retrieval, and VQA tasks with reduced computational cost.
...
TL;DR: MeViS is a large-scale multi-modal dataset for referring motion expression video segmentation, focusing on segmenting/tracking objects based on motion descriptions rather than static attributes.


<details>
  <summary>Details</summary>
Motivation: Existing referring video segmentation datasets focus on salient objects with static attribute descriptions, underemphasizing motion in both videos and language. There's a need to explore motion expressions and motion reasoning for pixel-level video understanding.
...
Result: Benchmarking revealed weaknesses in existing methods for motion expression-guided video understanding. LMPM++ achieved new state-of-the-art results on RVOS/AVOS/RMOT tasks.

Conclusion: MeViS provides a platform for developing motion expression-guided video understanding algorithms in complex scenes. The dataset highlights the importance of motion reasoning and exposes limitations of current methods.
...
TL;DR: Flex is a geometry-agnostic scene encoder that uses learnable tokens to jointly encode multi-camera data across timesteps, achieving 2.2x faster inference and better driving performance without relying on 3D priors like BEV.


<details>
  <summary>Details</summary>
Motivation: Address the computational bottleneck of processing high-volume multi-camera data in end-to-end autonomous driving systems, challenging the prevailing assumption that explicit 3D inductive biases are necessary.

Method: Uses a small set of learnable scene tokens to jointly encode information from all image tokens across different cameras and timesteps. Geometry-agnostic approach learns compact scene representation directly from data without relying on explicit 3D representations like BEV, occupancy, or tri-plane.

Result: Achieves 2.2x greater inference throughput while improving driving performance by a large margin compared to state-of-the-art methods on a 20,000-hour proprietary dataset. Also shows emergent capability for scene decomposition without explicit supervision.

Conclusion: Challenges the assumption that 3D priors are necessary, demonstrating that data-driven joint encoding offers a more scalable, efficient, and effective path for future autonomous driving systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing All-in-One Image Restoration (AiOIR) methods fail to explicitly model degradation types and struggle to adapt restoration behavior to complex or mixed degradations, limiting their effectiveness across diverse degradation scenarios.

Method: ClusIR introduces two key components: 1) Probabilistic Cluster-Guided Routing Mechanism (PCGRM) that disentangles degradation recognition from expert activation for discriminative perception and stable routing, and 2) Degradation-Aware Frequency Modulation Module (DAFMM) that uses cluster-guided priors for adaptive frequency decomposition and targeted modulation to refine structural and textural representations.

Result: Extensive experiments on diverse benchmarks validate that ClusIR achieves competitive performance across multiple scenarios, demonstrating remarkable restoration results for a wide range of degradations.

Conclusion: ClusIR successfully bridges semantic degradation cues with frequency-domain modulation through cluster-guided synergy, enabling adaptive and high-fidelity image restoration across diverse degradation types.
...
<details>
  <summary>Details</summary>
Motivation: Self-supervised pre-training has been successful for languages, 2D images, and videos, but remains largely unexplored for learning 3D-aware representations from multi-view images. Prior methods like RayZer infer 3D indirectly through latent-space view synthesis rather than operating directly in 3D space.

Method: E-RayZer performs self-supervised 3D reconstruction with explicit geometry directly in 3D space, eliminating shortcut solutions. It introduces a novel fine-grained learning curriculum that organizes training from easy to hard samples and harmonizes heterogeneous data sources in an unsupervised manner.
...
Conclusion: E-RayZer establishes a new paradigm for 3D-aware visual pre-training by learning truly 3D-aware representations directly from unlabeled images through explicit 3D reconstruction, achieving state-of-the-art performance on 3D tasks.
...
<details>
  <summary>Details</summary>
Motivation: Previous diffusion models generate images independently at inference, missing potential benefits of collaborative generation. The authors explore whether samples can be generated collaboratively to leverage inter-image relationships.
...
Result: Observed clear scaling effect - larger group sizes yield stronger cross-sample attention and better generation quality. Achieved up to 32.2% FID improvement on ImageNet-256x256. Introduced qualitative measure that correlates with FID.

Conclusion: Cross-sample inference is an effective, previously unexplored mechanism for generative modeling. GroupDiff reveals the potential of collaborative generation in diffusion models.
...
Method: Joint data-model design: (1) curating semantically linked image pairs annotated with positive/negative attributes to explicitly teach what to preserve/suppress; (2) dual-objective training balancing generative fidelity with contrastive disentanglement.

Result: The encoder produces high-fidelity, attribute-specific representations effective for open-vocabulary attribute retrieval, personalization, and compositional generation, achieving state-of-the-art performance across multiple benchmarks.

Conclusion: Omni-Attribute successfully addresses the disentanglement problem in visual concept personalization through explicit attribute-specific learning, enabling more precise control over image attributes in downstream applications.
...
TL;DR: StereoWalker improves robot navigation foundation models by incorporating stereo vision and explicit mid-level vision modules (depth estimation, tracking) instead of relying solely on monocular vision, achieving better performance with far less training data.


<details>
  <summary>Details</summary>
Motivation: Current navigation foundation models rely on monocular vision and assume vision capabilities will emerge implicitly, but this requires massive pixel-to-action supervision and struggles with depth-scale ambiguity and dynamic scenes. Mid-level vision priors are ignored despite their potential benefits.

Method: StereoWalker augments NFMs with stereo inputs to resolve depth-scale ambiguity and incorporates explicit mid-level vision modules (depth estimation and dense pixel tracking). The authors also curate a large stereo navigation dataset with automatic action annotation from Internet stereo videos.
...
Conclusion: Relying solely on monocular vision and ignoring mid-level vision priors is inefficient for robot navigation foundation models. Incorporating stereo inputs and explicit mid-level vision modules significantly improves performance and data efficiency.
...
TL;DR: WorldLens is a comprehensive benchmark for evaluating generative world models across five dimensions: generation quality, reconstruction accuracy, action-following capability, downstream task performance, and human preference alignment.


<details>
  <summary>Details</summary>
Motivation: Current generative world models produce visually convincing 4D driving environments but often fail physically or behaviorally. The field lacks unified evaluation methods to assess whether generated worlds preserve geometry, obey physics, or support reliable control.
...
Result: No existing world model excels universally across all dimensions - models with strong textures often violate physics, while geometry-stable ones lack behavioral fidelity. The benchmark reveals trade-offs between visual realism, geometric consistency, physical plausibility, and functional reliability.

Conclusion: WorldLens provides a unified ecosystem for measuring world fidelity, standardizing evaluation of future models not just by visual realism but by behavioral realism. The benchmark, dataset, and agent together enable scalable, explainable scoring of generative world models.
...
<details>
  <summary>Details</summary>
Motivation: Current stereo generation methods often rely on explicit depth estimation and warping, which can be limited by depth estimation errors and struggle with complex scenes. The paper aims to develop a more robust, end-to-end approach that doesn't require explicit geometry modeling.

Method: StereoSpace uses a diffusion-based framework with viewpoint conditioning to model geometry implicitly. It operates in a canonical rectified space and uses conditioning to guide the generator to infer correspondences and fill disocclusions end-to-end, without explicit depth or warping operations.

Result: StereoSpace outperforms other methods from warp & inpaint, latent-warping, and warped-conditioning categories. It achieves sharp parallax and strong robustness on layered and non-Lambertian scenes, establishing viewpoint-conditioned diffusion as a scalable, depth-free solution.

Conclusion: Viewpoint-conditioned diffusion provides a scalable, depth-free solution for stereo generation that avoids the limitations of explicit geometry modeling, offering better perceptual comfort and geometric consistency than existing approaches.
...
### [98] [Fourier Sparsity of Delta Functions and Matching Vector PIRs](https://arxiv.org/abs/2512.09941)
*Fatemeh Ghasemi,Swastik Kopparty*

Main category: cs.IT

TL;DR: Study of Fourier sparsity of delta functions on Boolean cubes, with applications to improving Matching Vector PIR schemes. Shows upper/lower bounds on Fourier sparsity, revealing limitations for achieving polylogarithmic communication in PIR.


<details>
  <summary>Details</summary>
Motivation: The paper studies a fundamental question in Fourier analysis of Boolean functions: how sparse can delta functions be in the Fourier basis? This has direct applications to improving Matching Vector Private Information Retrieval (PIR) schemes, as finding delta functions with low Fourier sparsity corresponds to finding better S-decoding polynomials for known matching vector families.

Method: The authors analyze delta functions defined on {0,1}^r with values in Z_m^r, where f(0)=1 and f(x)=0 for all nonzero Boolean x. They study the Fourier sparsity of such functions using elementary and clean mathematical proofs to establish both upper and lower bounds on the minimum possible Fourier sparsity.

Result: The paper proves nontrivial upper and lower bounds on the Fourier sparsity of delta functions. These results demonstrate limitations on improving Matching Vector PIR schemes through better S-decoding polynomials. Specifically, they show there are no S-decoding polynomials that can make Matching Vector PIRs based on known families achieve polylogarithmic communication with a constant number of servers.

Conclusion: The study reveals fundamental limitations in improving Matching Vector PIR schemes via S-decoding polynomial optimization. While the bounds provide insights into the sparsity constraints of delta functions, many interesting questions about Fourier analysis of Boolean functions and PIR scheme improvements remain open for future research.
...
### [99] [Improving the decoding performance of CA-polar codes](https://arxiv.org/abs/2512.10223)
*Jiewei Feng,Peihong Yuan,Ken R. Duffy,Muriel Médard*

Main category: cs.IT

TL;DR: Using code-agnostic decoders to complete CA-SCL decoding when it fails, achieving up to 0.2-1dB BLER gains for 5G NR polar codes while controlling undetected error rates.


<details>
  <summary>Details</summary>
Motivation: CA-SCL (CRC-Aided Successive Cancellation List) decoding is incomplete - it sometimes fails to find a valid codeword that passes CRC check. There's a need to convert it into a complete decoder that always finds a valid solution when one exists.

Method: When CA-SCL fails to identify a codeword passing CRC, apply a code-agnostic decoder to find a codeword that satisfies the CRC. For systematic CA-polar codes, this approach yields even better gains. Also leverages blockwise soft output techniques to control undetected error rates.

Result: Achieves gains of up to 0.2 dB in block error rate for CA-Polar codes from 5G NR standard. For systematic CA-polar codes, gains improve to 0.2-1 dB. Can control undetected error rate even when using CRC for error correction.

Conclusion: Code-agnostic decoders can effectively complete CA-SCL decoding, providing significant performance gains for polar codes while maintaining control over error detection capabilities.
...
### [100] [OGC Geotech Interoperability Experiment Engineering Report](https://arxiv.org/abs/2512.10678)
*Mickaël Beaufils,Katharina Schleidt,Hylke van Der Schaaf,Daniel Ponti,Neil Chadwick,Derrick Dasenbrock*

Main category: cs.IT

TL;DR: OGC Geotech Interoperability Experiment developed a common conceptual model for geotechnical engineering data to bridge existing specifications and integrate across OGC and buildingSMART standards.


<details>
  <summary>Details</summary>
Motivation: The need for interoperability between different geotechnical engineering data specifications and standards from OGC and buildingSMART International to enable better data sharing and integration across domains.

Method: Conducted an OGC Interoperability Experiment to develop a common conceptual model that bridges existing geotechnical data encoding specifications, documented in an Engineering Report imported from a project wiki.

Result: Produced an Engineering Report describing the outcomes of the Geotech IE, including a common conceptual model for geotechnical engineering data that can integrate across OGC and buildingSMART standards.

Conclusion: The experiment successfully developed an interoperable conceptual model for geotechnical data that bridges existing specifications and enables integration across major standards organizations in the geospatial and building information domains.
...
### [101] [CSI-Based User Positioning, Channel Charting, and Device Classification with an NVIDIA 5G Testbed](https://arxiv.org/abs/2512.10809)
*Reinhard Wiesmayr,Frederik Zumegen,Sueda Taner,Chris Dick,Christoph Studer*

Main category: eess.SP

TL;DR: Researchers publish first real-world 5G NR CSI datasets from a software-defined testbed, enabling development of CSI-based sensing algorithms with high accuracy for UE positioning, channel charting, and device classification.


<details>
  <summary>Details</summary>
Motivation: No CSI dataset has been published from a real-world 5G NR system to facilitate development and validation of sensing algorithms, creating a gap in the research community.

Method: Created three real-world wideband multi-antenna multi-O-RU CSI datasets using a software-defined 5G NR testbed (NVIDIA Aerial RAN CoLab OTA) with COTS hardware deployed at ETH Zurich, covering indoor lab/office, outdoor campus, and device classification scenarios.

Result: High accuracy achieved across three sensing tasks: neural UE positioning (0.6cm indoor, 5.7cm outdoor MAE), channel charting in real-world coordinates (73cm outdoor MAE), and device classification (99% same-day, 95% next-day accuracy).

Conclusion: The published datasets successfully fill the gap in real-world 5G NR CSI data availability, demonstrating practical utility for CSI-based sensing with state-of-the-art performance, and all resources are publicly available for community use.
...
### [102] [Analysis and Compensation of Receiver IQ Imbalance and Residual CFO Error for AFDM](https://arxiv.org/abs/2512.10036)
*Nimesha Gunasekara,Ebrahim Bedeer*

Main category: eess.SP

TL;DR: AFDM is analyzed under receiver IQ imbalance and residual CFO errors, showing these impairments break channel sparsity and cause energy leakage. An extended LMMSE detector is proposed to handle improper Gaussian noise from IQ imbalance.


<details>
  <summary>Details</summary>
Motivation: AFDM is promising for future wireless systems, but receiver hardware impairments like IQ imbalance and residual CFO errors can degrade performance. These impairments need to be analyzed and mitigated for practical AFDM implementation.

Method: The paper analyzes how receiver IQ imbalance breaks AFDM channel sparsity due to DAFT's complex-conjugate operator, and how residual CFO causes affine domain energy leakage. An extended LMMSE detector is developed to handle the improper Gaussian noise from IQ imbalance.

Result: Simulation results show the proposed LMMSE detector effectively compensates for receiver hardware impairments, demonstrating practical mitigation of IQ imbalance and residual CFO effects on AFDM signals.

Conclusion: Receiver IQ imbalance and residual CFO significantly impact AFDM performance by breaking channel sparsity and causing energy leakage. The proposed extended LMMSE detector provides effective compensation for these hardware impairments in AFDM systems.
...
### [103] [Cost and Complexity as Barriers to RTLS Adoption in SMEs: A Survey and Analysis](https://arxiv.org/abs/2512.10074)
*Peyman Moeini,Mark Coates*

Main category: eess.SP

TL;DR: RTLS adoption by SMEs is limited not by perceived value but by cost, installation complexity, and infrastructure requirements, with preferences for sub-$10K investments and low-anchor solutions.


<details>
  <summary>Details</summary>
Motivation: To understand why small and medium-sized enterprises (SMEs) have limited adoption of real-time location systems (RTLS) despite their importance for Industry 4.0/5.0, focusing on whether cost and installation complexity are primary barriers.

Method: 1) Literature review synthesizing technical, financial, and organizational constraints; 2) Online survey of 16 manufacturing/technology professionals in Canada/US to assess perceived value and barriers.

Result: SMEs recognize strong value in RTLS but face barriers: upfront cost, installation effort, integration difficulty, and reliance on multiple anchor nodes. Most accept investments below $10,000 and prefer low-infrastructure deployments with minimal anchors.

Conclusion: Limited SME adoption stems from misalignment between current RTLS deployment models and SME resource constraints, not insufficient perceived value. Future RTLS designs should focus on wireless/modular architectures, cloud-managed systems, standardized interfaces, and anchor-minimizing methods.
...
### [104] [Topology Identification and Inference over Graphs](https://arxiv.org/abs/2512.10183)
*Gonzalo Mateos,Yanning Shen,Georgios B. Giannakis,Ananthram Swami*

Main category: eess.SP

TL;DR: Overview of graph topology identification and inference methods for multidimensional relational data, covering undirected and directional relations, with extensions to structural equations and vector autoregressions.


<details>
  <summary>Details</summary>
Motivation: Topology identification and inference are crucial for understanding processes in various real-world networks (brain, transportation, financial, power, social, information networks) where relational data evolves over graphs.

Method: Survey of approaches including: 1) Undirected link methods from correlation metrics to covariance selection; 2) Kernel-based framework for directional/causal relations addressing dynamic and nonlinear dependencies; 3) Generalizations via structural equations and vector autoregressions exploiting low rank, sparsity, acyclicity, and smoothness.

Result: The surveyed framework supports both batch and online learning with convergence guarantees, is amenable to tensor formulations for multidimensional network data, and can leverage high-order statistical information.

Conclusion: Comprehensive overview of graph topology identification methods that handle both undirected and directional relations, with extensions to dynamic processes over time-evolving topologies using principled statistical inference frameworks.
...
### [105] [Antenna Coding Design for Multi-User Transmissions Using Pixel Antennas](https://arxiv.org/abs/2512.10246)
*Hongyu Li,Shanpu Shen*

Main category: eess.SP

TL;DR: Proposes multi-user MISO pixel antenna system with reconfigurable antennas at users, develops algorithms for joint precoding and antenna coding design to maximize sum rate, and introduces codebook-based approaches to reduce complexity while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: To exploit the potential of pixel antennas (reconfigurable antennas with flexible characteristics through antenna coding) in multi-user transmissions, addressing the need for improved performance in multi-user MISO systems beyond conventional fixed antennas.

Method: 1) Proposes multi-user MISO pixel antenna system with pixel antennas deployed at users; 2) Develops system model including antenna coding and multi-user beamspace channels; 3) Designs alternating algorithm for joint precoding and antenna coding optimization; 4) Introduces codebook-based antenna coding design with offline codebook and online optimization; 5) Proposes hierarchical codebook-based design with multi-layer hierarchical search for better performance-complexity trade-off.

Result: Simulation results show the multi-user MISO pixel antenna system consistently outperforms conventional multi-user MISO systems with fixed antennas. The proposed codebook-based algorithms significantly reduce computational complexity while maintaining satisfactory sum rate performance.

Conclusion: Pixel antenna technology can effectively enhance multi-user MISO systems through reconfigurable antenna characteristics. The proposed algorithms, particularly the hierarchical codebook-based approach, provide practical solutions with good performance-complexity trade-offs, making pixel antenna systems viable for real-world applications.
...
### [106] [Outdoor Crowd Flow Estimation Using RSRP from Commercial LTE Base Station: A Field Study](https://arxiv.org/abs/2512.10447)
*Kaisei Higeta,Masakatsu Ogawa,Tomoki Murakami,Kazuya Ohara,Shinya Otsuki*

Main category: eess.SP

TL;DR: This paper demonstrates outdoor crowd flow estimation using RSRP variance from commercial LTE base stations, finding optimal variance window size of 0.1-0.2 seconds and that larger counting areas improve feature quality for machine learning.


<details>
  <summary>Details</summary>
Motivation: With 6G and ISAC gaining attention, there's a need for outdoor crowd flow estimation applications. Existing research focuses on indoor environments or vehicles, leaving a gap in using commercial LTE base stations for outdoor crowd monitoring.

Method: Used RSRP from commercial LTE base station, associated pedestrian counts from camera-based object recognition with RSRP variance. Applied CatBoost regression model with SHAP analysis to quantitatively evaluate variance features.

Result: Found optimal RSRP variance window size of 0.1-0.2 seconds. Larger counting areas increased useful features from RSRP variance for machine learning. First quantitative demonstration of outdoor crowd flow estimation using commercial LTE.

Conclusion: Successfully demonstrated outdoor crowd flow estimation using commercial LTE, revealing important characteristics for feature design including optimal variance window size and counting area effects on feature quality.
...
### [107] [A Novel Pilot Scheme for Uplink Channel Estimation for Sub-array Structured ELAA in XL-MIMO systems](https://arxiv.org/abs/2512.10478)
*Yumeng Zhang,Huayan Guo,Vincent Lau*

Main category: eess.SP

TL;DR: Novel pilot scheme for XL-MIMO systems using spatial non-stationarity to group users with distinct visibility regions, enabling more users with reduced pilot overhead through frequency-division multiplexing and cyclic codes.


<details>
  <summary>Details</summary>
Motivation: XL-MIMO systems with ELAA introduce spatial non-stationarity where far-apart users have distinct visibility at antennas, reducing inter-user interference. This spatial property can be exploited to serve more users with reduced pilot overhead.

Method: 1) Novel pilot scheme groups users with distinct visibility regions to share frequency subcarriers; 2) Frequency-division multiplexing for inter-group estimation; 3) Intra-group users distinguished by shifted cyclic codes; 4) Sub-array structured ELAA with each sub-array as spatial stationary; 5) Channel modeled by 2-D Markov random field; 6) Low-complexity channel estimation algorithm using turbo Bayesian inference framework.

Result: Simulations show the proposed scheme and algorithm allow XL-MIMO systems to support more users and deliver superior channel estimation performance compared to existing methods.

Conclusion: The proposed pilot scheme and channel estimation algorithm effectively exploit spatial non-stationarity in XL-MIMO systems to increase user capacity while reducing pilot overhead, making it a promising approach for next-generation massive MIMO systems.
...
### [108] [T-ADD: Enhancing DOA Estimation Robustness Against Adversarial Attacks](https://arxiv.org/abs/2512.10496)
*Shilian Zheng,Xiaoxiang Wu,Luxin Zhang,Keqiang Yue,Peihan Qi,Zhijin Zhao*

Main category: eess.SP

TL;DR: Transformer-based adversarial defense method (T-ADD) improves robustness of DOA estimation models against adversarial attacks while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for DOA estimation are vulnerable to adversarial perturbations that can severely compromise their performance, creating a need for robust defense mechanisms.

Method: Proposes T-ADD, a transformer-based defense method that formulates adversarial defense as a joint reconstruction task with a tailored joint loss function to balance robustness and accuracy.

Result: T-ADD significantly mitigates adverse effects of widely used adversarial attacks and shows notable improvements in adversarial robustness compared to three state-of-the-art defense methods.

Conclusion: The proposed transformer-based adversarial defense method effectively enhances the robustness of DOA estimation models against adversarial attacks while maintaining estimation accuracy.
...
### [109] [NN-Based Frequency Domain DPD for OFDM Massive MIMO Transmitters With Multiple States](https://arxiv.org/abs/2512.10731)
*Yundi Zhang,Yanshi Sun,Li Chen*

Main category: eess.SP

TL;DR: The paper proposes a hypernetwork-based neural network for frequency-domain digital predistortion that adapts to multiple signal states without requiring retraining.


<details>
  <summary>Details</summary>
Motivation: Frequency-domain DPD is efficient for massive MIMO transmitters, but needs to handle scenarios with multiple signal states (different bandwidths and power levels) without requiring additional online training when states change.

Method: Proposes a neural network-based FD-DPD model using a hypernetwork to generate parameters for the output layer based on signal states, trained with help from time-domain DPD.

Result: Experimental results show the proposed model achieves excellent performance without requiring additional online training when signal states change.

Conclusion: The hypernetwork-based FD-DPD model effectively handles multiple signal states in massive MIMO transmitters with good performance and no need for retraining when states change.
...
### [110] [Symbol-Level Precoding for Integrated Sensing and Covert Communication](https://arxiv.org/abs/2512.10752)
*Yufei Wang,Qiang Li,Hongli Liu,Ying Zhang,Jingran Lin*

Main category: eess.SP

TL;DR: Proposes a novel symbol-level precoding (SLP) waveform design for integrated sensing and covert communication (ISCC) systems that achieves covert communication intrinsically without artificial noise, using symbol shaping for reliability and noise shaping for covertness.


<details>
  <summary>Details</summary>
Motivation: ISAC systems improve spectrum efficiency but face challenges in ensuring secure information transmission while maintaining high-quality sensing performance. The need for covert communication against malicious targets that act as both eavesdroppers and wardens drives this research.

Method: Proposes SLP-based waveform design integrating symbol shaping for legitimate users and noise shaping to obscure transmission from targets. Incorporates bounded uncertainty models for imperfect CSI (user channels and target angles). Develops low-complexity proximal distance algorithm (PDA) with closed-form updates for PSK and QAM modulations to solve the non-convex optimization problem.

Result: Simulation results show superior covertness and sensing-communication performance with negligible degradation compared to traditional beamforming and conventional SLP approaches without noise-shaping mechanisms.

Conclusion: The proposed ISCC waveform design effectively achieves covert communication intrinsically without requiring additional transmission resources like artificial noise, while maintaining robust performance under imperfect CSI conditions.
...
### [111] [Comparative analysis of WNG-DF compromising beamformers](https://arxiv.org/abs/2512.10829)
*Vitor G. P. Curtarelli*

Main category: eess.SP

TL;DR: Comparison of multi-objective beamformers balancing white-noise gain and directivity factor, finding robust superdirective and tunable beamformers perform best with nearly identical results.


<details>
  <summary>Details</summary>
Motivation: To study beamformers that can achieve multiple characteristics simultaneously, specifically balancing white-noise gain and directivity factor, and compare different approaches to achieve this joint optimization.

Method: Compare methods explicitly designed for joint features against combinations of single-task beamformers through simulations, evaluating performance across various metrics.

Result: Robust superdirective and tunable beamformers yield the best results among studied methods, producing nearly identical outputs across all evaluated metrics. These methods continuously compromise between the two objectives.

Conclusion: Robust superdirective and tunable beamformers are the most practical and effective approaches for simultaneously achieving white-noise gain and directivity factor optimization, offering continuous trade-off between the two objectives.
...
### [112] [A Variable Step Sizes Frequency Offsets-Compensated Least Mean Squares Algorithm](https://arxiv.org/abs/2512.10832)
*Karel Pärlin,Aaron Byman,Tommi Meriläinen,Taneli Riihonen*

Main category: eess.SP

TL;DR: FO-LMS algorithm for channel estimation under frequency offsets, with theoretical error analysis and adaptive step size optimization for unknown time-varying channels.


<details>
  <summary>Details</summary>
Motivation: FO-LMS algorithm needs proper step size selection to balance tracking ability and noise misadjustment, but existing methods lack theoretical guidance for time-varying channels.

Method: Develop theoretical expressions to predict tracking and misadjustment errors, and propose adaptive step size adjustment method for unknown time-varying channel characteristics.

Result: Theoretical expressions accurately predict algorithm performance, and proposed variable step size method effectively adapts to unknown time-varying channel conditions.

Conclusion: The paper provides theoretical framework for FO-LMS optimization and practical adaptive step size method for real-world applications with unknown time-varying channels.
...
### [113] [Decision Feedback-Aided Known-Interference Cancellation](https://arxiv.org/abs/2512.10833)
*Karel Pärlin,Aaron Byman,Tommi Meriläinen,Taneli Riihonen*

Main category: eess.SP

TL;DR: Decision feedback-aided known-interference cancellation (DF-KIC) improves KI cancellation capability and increases wireless communication throughput, though with higher computational load.


<details>
  <summary>Details</summary>
Motivation: Known-interference cancellation (KIC) with cooperative jamming provides covertness and security at the physical layer, but the signal of interest (SI) acts as estimation noise that limits KI cancellation and thus limits the throughput of the hidden/secure wireless communication system.

Method: Proposes a decision feedback-aided known-interference cancellation (DF-KIC) structure where both known interference (KI) and signal of interest (SI) are canceled iteratively and successively through decision feedback mechanisms.

Result: Measurement results show that introducing decision feedback to KIC improves its KI cancellation capability, thereby increasing the wireless communication system's useful throughput, albeit at the expense of higher computational load.

Conclusion: DF-KIC offers a practical trade-off between improved interference cancellation/throughput and increased computational complexity for enhancing covertness and security in wireless communications.
...
### [114] [POLO: Phase-Only Localization in Uplink Distributed MIMO Systems](https://arxiv.org/abs/2512.10879)
*Sauradeep Dey,Musa Furkan Keskin,Dario Tagliaferri,Gonzalo Seco-Granados,Mikko Valkama,Henk Wymeersch*

Main category: eess.SP

TL;DR: POLO is a low-complexity localization framework for uplink D-MIMO systems that uses differential carrier-phase measurements from selected APs to generate candidate positions, avoiding exhaustive grid search while maintaining coverage.


<details>
  <summary>Details</summary>
Motivation: ML-based localization in D-MIMO systems with sparsely deployed phase-coherent APs requires dense grid search, which has prohibitive computational complexity. There's a need for efficient localization methods that reduce complexity while maintaining performance.

Method: POLO uses differential carrier-phase measurements from selected APs to generate a compact set of candidate user positions, then evaluates ML cost only at these candidates. Two variants: POLO-I selects 3 APs for closed-form solutions with low cost, POLO-II selects 4 APs for better coverage at slightly higher runtime.

Result: POLO achieves orders of magnitude complexity reduction compared to exhaustive grid search with only marginal coverage loss. The framework provides a favorable coverage-complexity trade-off and practical guidelines for selecting between POLO-I and POLO-II based on latency/coverage requirements.

Conclusion: POLO offers an efficient solution for D-MIMO localization by significantly reducing computational complexity while maintaining reliable coverage, with two variants providing flexibility for different system requirements.
...
### [115] [Active Optics for Hyperspectral Imaging of Reflective Agricultural Leaf Sensors](https://arxiv.org/abs/2512.10213)
*Dexter Burns,Sanjeev Koppal*

Main category: eess.IV

TL;DR: An integrated system using LiDAR, liquid lens, camera, filter wheel, and FSM to autonomously detect and interrogate plant-mounted sensors for real-time agricultural monitoring.


<details>
  <summary>Details</summary>
Motivation: Current plant health monitoring relies on leaf-mounted sensors, but efficiently locating and sampling these sensors in complex agricultural environments remains challenging.

Method: Uses LiDAR to identify reflective sensor signatures, then dynamically redirects camera field of view via FSM for hyperspectral imaging, with liquid lens adjusting focus for varying depths.

Result: Validated in controlled indoor experiments, demonstrating accurate detection and tracking of reflective plant sensors and successful acquisition of spectral data.
...
### [116] [Hyperspectral Image Data Reduction for Endmember Extraction](https://arxiv.org/abs/2512.10506)
*Tomohiko Mizutani*
...
<details>
  <summary>Details</summary>
Motivation: Self-dictionary methods for hyperspectral endmember extraction have high computational costs that limit their applicability to large-scale images, despite achieving good accuracy. Existing mitigation approaches haven't fully solved this challenge.

Method: Develops a data reduction technique that removes pixels not containing endmembers, assuming linear mixing model with pure-pixel assumption. Analyzes theoretical properties showing preservation of endmember-proximate pixels. Integrates this reduction with a self-dictionary method based on linear programming formulation.
...
Conclusion: The data-reduced self-dictionary method effectively addresses computational scalability issues in hyperspectral endmember extraction while maintaining extraction accuracy.
...
### [117] [Fast and Robust LRSD-based SAR/ISAR Imaging and Decomposition](https://arxiv.org/abs/2512.10740)
*Hamid Reza Hashempour,Majid Moradikia,Hamed Bastami,Ahmed Abdelhadi,Mojtaba Soltanalian*

Main category: eess.IV

TL;DR: Fast unified SAR imaging framework using low-rank-sparse decomposition that handles platform residual phase errors and reduces computational burden for real-time processing.


<details>
  <summary>Details</summary>
Motivation: Existing LRSD-based SAR imaging methods fail to handle platform residual phase errors (PRPE) from unstable airborne platforms and ignore real-time processing requirements by focusing only on image quality enhancement rather than computational efficiency.

Method: Proposes a fast unified joint SAR imaging framework that decomposes sparse objects and low-rank background features through robust LRSD. The algorithm avoids computing large matrix inverses for image formation and uses constrained quadratic programming to handle unimodular constraints from PRPE. Extended to ISAR autofocusing and imaging where LRSD recovers sparse images.
...
Conclusion: The unified framework successfully addresses both PRPE handling and computational efficiency, making it suitable for real-time remote sensing applications while maintaining superior image quality.
...
### [118] [HGC-Herd: Efficient Heterogeneous Graph Condensation via Representative Node Herding](https://arxiv.org/abs/2512.09947)
...
<details>
  <summary>Details</summary>
Motivation: HGNNs struggle with scalability on large graphs due to structural redundancy and high-dimensional features. Existing condensation methods like GCond are designed for homogeneous graphs and rely on gradient matching, causing computational, memory, and optimization overhead.
...
Conclusion: HGC-Herd demonstrates practical value for efficient and scalable heterogeneous graph representation learning by providing a training-free condensation approach that maintains both semantic and structural fidelity.
...
### [119] [BAMBO: Construct Ability and Efficiency LLM Pareto Set via Bayesian Adaptive Multi-objective Block-wise Optimization](https://arxiv.org/abs/2512.09972)
*Kesheng Chen,Wenjian Luo,Zhenqian Zhu,Yamin Hu,Yiya Xi*

Main category: cs.LG

TL;DR: BAMBO is a Bayesian optimization framework that automatically constructs Pareto sets for LLMs by partitioning models into optimal blocks, making the search tractable while maintaining granularity.


<details>
  <summary>Details</summary>
Motivation: Existing model merging techniques are inadequate for constructing Pareto sets that navigate capability-efficiency trade-offs in LLMs. Coarse-grained methods produce sparse suboptimal solutions, while fine-grained approaches suffer from computational intractability due to high dimensionality.

Method: BAMBO uses Hybrid Optimal Block Partitioning formulated as a 1D clustering problem with dynamic programming to balance intra-block homogeneity and inter-block information distribution. This reduces dimensionality while preserving granularity. The process is automated within an evolutionary loop using q-Expected Hypervolume Improvement (qEHVI) acquisition function.

Result: BAMBO discovers a superior and more comprehensive Pareto frontier than baselines, enabling agile model selection tailored to diverse operational constraints.

Conclusion: BAMBO effectively resolves the dichotomy between coarse-grained and fine-grained model merging approaches, providing a tractable solution for constructing comprehensive Pareto sets for LLMs.
...
### [120] [Latent Action World Models for Control with Unlabeled Trajectories](https://arxiv.org/abs/2512.10016)
*Marvin Alles,Xingyuan Zhang,Patrick van der Smagt,Philip Becker-Ehmck*

Main category: cs.LG

TL;DR: Latent-action world models learn from both action-conditioned and action-free data by learning shared latent action representations, enabling efficient training with far fewer labeled samples.


<details>
  <summary>Details</summary>
Motivation: Standard world models rely on action-conditioned trajectories, which limits effectiveness when action labels are scarce. Humans combine direct interaction with action-free experience (like videos), suggesting world models should learn from heterogeneous data.
...
Conclusion: Latent actions enable training on both passive and interactive data, making world models learn more efficiently by bridging offline RL (action-conditioned) with action-free training.
...
### [121] [Cluster-Dags as Powerful Background Knowledge For Causal Discovery](https://arxiv.org/abs/2512.10032)
*Jan Marco Ruiz de Vargas,Kirtan Padh,Niki Kilbertus*

Main category: cs.LG

TL;DR: This paper introduces Cluster-DAGs as a prior knowledge framework to warm-start causal discovery, with two modified algorithms (Cluster-PC and Cluster-FCI) that outperform baseline methods without prior knowledge.


<details>
  <summary>Details</summary>
Motivation: Current causal discovery methods face challenges with high-dimensional data and complex dependencies. Incorporating prior knowledge can aid causal discovery, but existing approaches based on tiered background knowledge have limitations in flexibility.

Method: The authors leverage Cluster-DAGs as a prior knowledge framework to warm-start causal discovery. They introduce two modified constraint-based algorithms: Cluster-PC for fully observed settings and Cluster-FCI for partially observed settings.

Result: Empirical evaluation on simulated data demonstrates that both Cluster-PC and Cluster-FCI outperform their respective baselines (standard PC and FCI algorithms) without prior knowledge.

Conclusion: Cluster-DAGs provide a flexible prior knowledge framework that effectively improves causal discovery performance, offering advantages over existing tiered background knowledge approaches.
...
### [122] [Robust Gradient Descent via Heavy-Ball Momentum with Predictive Extrapolation](https://arxiv.org/abs/2512.10033)
*Sarwan Ali*

Main category: cs.LG

TL;DR: HB-SGE is a robust first-order optimization method combining heavy-ball momentum with predictive gradient extrapolation, preventing divergence on ill-conditioned/non-convex problems where NAG and standard momentum fail.


<details>
  <summary>Details</summary>
Motivation: Accelerated gradient methods like NAG achieve fast convergence on well-conditioned problems but often diverge on ill-conditioned or non-convex landscapes due to aggressive momentum accumulation. There's a need for methods that maintain acceleration while ensuring stability across diverse optimization landscapes.

Method: HB-SGE combines heavy-ball momentum with predictive gradient extrapolation using local Taylor approximations to estimate future gradient directions. Unlike classical momentum that accumulates historical gradients, it uses synthetic gradient extrapolation for adaptive acceleration while maintaining stability. The method has O(d) memory overhead and uses the same hyperparameters as standard momentum.

Result: On ill-conditioned quadratics (κ=50), HB-SGE converges in 119 iterations while both SGD and NAG diverge. On the non-convex Rosenbrock function, HB-SGE achieves convergence in 2,718 iterations where classical momentum methods diverge within 10 steps. While NAG remains faster on well-conditioned problems, HB-SGE provides robust performance across diverse landscapes with speedup over SGD.

Conclusion: HB-SGE offers a robust alternative to NAG and standard momentum methods, providing adaptive acceleration while preventing divergence on ill-conditioned and non-convex problems. It maintains the computational efficiency of first-order methods while significantly improving stability across challenging optimization landscapes.
...
### [123] [Intelligently Weighting Multiple Reference Models for Direct Preference Optimization of LLMs](https://arxiv.org/abs/2512.10040)
...
<details>
  <summary>Details</summary>
Motivation: Current methods for setting reference weights in MRPO are ad-hoc and statistically unsound, leading to unreliable performance. The paper aims to develop more principled weighting strategies to improve preference optimization.

Method: The authors introduce four new weighting strategies: two offline methods using held-out validation signal, one online method with sliding-window estimator to reduce overfitting, and an online method treating reference weighting as a K-armed bandit via Thompson Sampling. Experiments use Qwen2.5-0.5B as policy model with seven reference models from different families.

Result: All four new strategies outperform current MRPO weighting methods on UltraFeedback and SafeRLHF in preference accuracy. However, surprisingly, single-reference DPO using 6 out of 7 references consistently outperforms all tested multiple-reference approaches.

Conclusion: While the new weighting strategies improve MRPO performance, the finding that single-reference DPO often outperforms multiple-reference approaches questions the practical value of multiple-reference methods for preference optimization.
...
### [124] [SEMDICE: Off-policy State Entropy Maximization via Stationary Distribution Correction Estimation](https://arxiv.org/abs/2512.10042)
*Jongmin Lee,Meiqi Sun,Pieter Abbeel*

Main category: cs.LG

TL;DR: SEMDICE is an off-policy algorithm that learns state entropy maximizing policies from arbitrary datasets for unsupervised RL pre-training, achieving better adaptation to downstream tasks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses unsupervised pre-training for RL where agents need to learn prior policies without task-specific rewards. The focus is on state entropy maximization (SEM) to learn policies that maximize entropy of state stationary distributions, which can serve as good priors for downstream tasks.

Method: SEMDICE is introduced as a principled off-policy algorithm that computes SEM policies directly from arbitrary off-policy datasets. It optimizes policies within the space of stationary distributions, computing a single stationary Markov state-entropy-maximizing policy without requiring online interaction.

Result: Experimental results show SEMDICE outperforms baseline algorithms in maximizing state entropy while achieving the best adaptation efficiency for downstream tasks among SEM-based unsupervised RL pre-training methods.

Conclusion: SEMDICE provides an effective off-policy approach for unsupervised RL pre-training through state entropy maximization, enabling efficient adaptation to downstream tasks from arbitrary datasets.
...
TL;DR: Novel three-step ensemble pipeline for zero-shot NER using multiple small LLMs outperforms individual models on Portuguese datasets without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: LLMs under-perform in NER for lower-resource languages like Portuguese, and while ensemble approaches exist for other tasks, NER remains under-explored despite the availability of open-weight LLMs for local deployment.

Method: Three-step ensemble pipeline for zero-shot NER using similarly capable, locally run LLMs with a heuristic to select optimal model combinations using minimal annotated data.

Result: Outperforms individual LLMs in 4 out of 5 Portuguese NER datasets; ensembles from different source datasets also outperform individual models in cross-dataset configurations.

Conclusion: Advances scalable, low-resource, zero-shot NER by effectively combining multiple small LLMs without fine-tuning, potentially eliminating need for annotated data for current tasks.
...
TL;DR: Researchers discover a detailed balance in LLM-generated transitions, suggesting LLMs implicitly learn underlying potential functions rather than rule sets, revealing a macroscopic physical law in LLM dynamics independent of specific model details.


<details>
  <summary>Details</summary>
Motivation: Despite empirical success of LLM-driven agents for complex problem solving, there's a lack of theoretical framework to understand and unify their macroscopic dynamics. The paper aims to establish a macroscopic dynamics theory of complex AI systems, elevating AI agent study from engineering practices to a science with predictable, quantifiable measurements.

Method: Proposes a method based on the least action principle to estimate the underlying generative directionality of LLMs within agents. Experimentally measures transition probabilities between LLM-generated states to statistically analyze the dynamics.

Result: Discovers a detailed balance in LLM-generated transitions, indicating that LLM generation may not be achieved by learning rule sets and strategies, but rather by implicitly learning underlying potential functions that transcend different LLM architectures and prompt templates.

Conclusion: This is the first discovery of a macroscopic physical law in LLM generative dynamics that doesn't depend on specific model details. The work represents an attempt to establish a macroscopic dynamics theory of complex AI systems, moving AI agent research toward a more scientific foundation with effective, predictable measurements.
...
TL;DR: DB2-TransF replaces Transformer self-attention with learnable Daubechies wavelet coefficients for efficient time series forecasting, achieving comparable accuracy with lower memory usage.


<details>
  <summary>Details</summary>
Motivation: Transformers have quadratic complexity that limits scalability for large-scale time series forecasting. Need efficient models that can capture complex temporal dependencies without computational bottlenecks.

Method: Introduces DB2-TransF architecture that replaces self-attention with a learnable Daubechies wavelet coefficient layer to capture multi-scale local and global patterns efficiently.

Result: Achieves comparable or superior predictive accuracy to conventional Transformers on 13 standard forecasting benchmarks while substantially reducing memory usage.

Conclusion: DB2-TransF is a scalable and resource-efficient framework for advanced time series forecasting, offering a practical alternative to Transformers.
...
TL;DR: SoTra introduces soft-token trajectory forecasting with risk-aware decoding to improve clinical forecasting in diabetes and hemodynamic management by reducing exposure bias and minimizing clinical risk.


<details>
  <summary>Details</summary>
Motivation: Autoregressive forecasting is crucial for predictive control in clinical settings like diabetes and blood pressure management, where different operating zones have different clinical risks. Standard models trained with teacher forcing suffer from exposure bias, leading to unstable multi-step forecasts unsuitable for closed-loop control systems.

Method: SoTra (Soft-Token Trajectory Forecasting) propagates continuous probability distributions (soft tokens) to mitigate exposure bias and learn calibrated, uncertainty-aware trajectories. It includes a risk-aware decoding module that minimizes expected clinical harm by considering different risk zones.

Result: In glucose forecasting, SoTra reduces average zone-based risk by 18%. In blood-pressure forecasting, it lowers effective clinical risk by approximately 15%. These improvements demonstrate better performance for safety-critical predictive control applications.

Conclusion: SoTra provides a robust approach for clinical forecasting that addresses exposure bias and incorporates risk-aware decision making, making it suitable for safety-critical predictive control systems in healthcare applications.
...
### [129] [\textsc{Text2Graph}: Combining Lightweight LLMs and GNNs for Efficient Text Classification in Label-Scarce Scenarios](https://arxiv.org/abs/2512.10061)
*João Lucas Luz Lima Sarcinelli,Ricardo Marcondes Marcacini*

Main category: cs.LG

TL;DR: Text2Graph is an open-source Python package that combines LLM-based partial annotation with GNN label propagation for sustainable zero-shot text classification, reducing energy costs while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: LLMs are effective zero-shot classifiers but have high computational requirements and environmental costs that limit their practicality for large-scale annotation in HPC environments. There's a need for more sustainable workflows.

Method: Text2Graph provides modular implementation of text-to-graph classification approaches, allowing users to combine LLM-based partial annotation with Graph Neural Network label propagation. The framework enables swapping of components like feature extractors, edge construction methods, and sampling strategies.

Result: Benchmarked on five datasets spanning topic classification and sentiment analysis tasks in zero-shot settings. Graph-based propagation achieves competitive results while significantly reducing energy consumption and carbon emissions compared to other zero-shot approaches.

Conclusion: Text2Graph enables sustainable text classification workflows by combining LLM partial annotation with GNN propagation, achieving good performance at a fraction of the energy and environmental cost of full LLM-based approaches.
...
### [130] [MedXAI: A Retrieval-Augmented and Self-Verifying Framework for Knowledge-Guided Medical Image Analysis](https://arxiv.org/abs/2512.10098)
*Midhat Urooj,Ayan Banerjee,Farhat Shaikh,Kuntal Thakur,Sandeep Gupta*

Main category: cs.LG

TL;DR: MedXAI is an explainable framework that integrates deep vision models with clinician-derived expert knowledge to improve medical imaging classification, particularly for domain shifts and rare-class conditions.


<details>
  <summary>Details</summary>
Motivation: Deep learning models struggle with real-world distribution changes, exhibit bias against infrequent pathologies, and lack transparency required for safety-critical clinical environments.

Method: A unified framework integrating deep vision models with clinician-derived expert knowledge, providing human-understandable explanations by localizing relevant diagnostic features rather than using technical post-hoc methods like Saliency Maps or LIME.

Result: Experiments on ten multicenter datasets show consistent gains: 3% improvement in cross-domain generalization and 10% improvement in F1 score for rare class, substantially outperforming strong deep learning baselines.

Conclusion: MedXAI delivers clinically aligned explanations while achieving superior in-domain and cross-domain performance, particularly for rare diseases in multimodal medical AI, with symbolic components acting as effective clinical priors and regularizers.
...
### [131] [CHyLL: Learning Continuous Neural Representations of Hybrid Systems](https://arxiv.org/abs/2512.10117)
*Sangli Teng,Hang Liu,Jingyu Song,Koushil Sreenath*

Main category: cs.LG

TL;DR: CHyLL learns continuous neural representations of hybrid systems without trajectory segmentation or mode switching by embedding the state space into a higher-dimensional space where flows become continuous.


<details>
  <summary>Details</summary>
Motivation: Existing methods for learning hybrid system dynamics suffer from mode switching and discontinuities in flows. The challenge lies in handling both continuous and discrete time dynamics together.

Method: CHyLL reformulates the state space as a piecewise smooth quotient manifold where flows become spatially continuous. It learns a singularity-free neural embedding in higher-dimensional space and continuous flows within it, avoiding trajectory segmentation, event functions, or mode switching.

Result: CHyLL accurately predicts hybrid system flows with superior accuracy and identifies topological invariants of hybrid systems. It also demonstrates application to stochastic optimal control problems.

Conclusion: CHyLL provides a novel approach to learning hybrid system dynamics by creating continuous neural representations in latent space, overcoming limitations of traditional mode-based methods and enabling better flow prediction and topological analysis.
...
### [132] [Partitioning the Sample Space for a More Precise Shannon Entropy Estimation](https://arxiv.org/abs/2512.10133)
*Gabriel F. A. Bastos,Jugurta Montalvão*

Main category: cs.LG

TL;DR: A new discrete entropy estimator that addresses bias in small data sets by combining decomposability with missing mass and unseen outcomes estimation.


<details>
  <summary>Details</summary>
Motivation: Reliable entropy estimation from small data sets is critical but challenging when number of examples is smaller than possible outcomes, leading to negative bias.

Method: Uses decomposability property combined with estimations of missing mass and number of unseen outcomes to compensate for negative bias.
...
Conclusion: Proposed method provides effective entropy estimation for small data sets by addressing bias through missing mass and unseen outcomes compensation.
...
### [133] [Sequence-to-Image Transformation for Sequence Classification Using Rips Complex Construction and Chaos Game Representation](https://arxiv.org/abs/2512.10141)
*Sarwan Ali,Taslim Murad,Imdadullah Khan*

Main category: cs.LG

TL;DR: Novel topological approach transforms molecular sequences into images using Chaos Game Representation and Rips complexes, achieving superior accuracy on anticancer peptide classification compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional feature engineering for molecular sequence classification suffers from sparsity and computational complexity, while deep learning models underperform on tabular biological data. There's a need for better representation methods that can effectively capture molecular sequence information for classification tasks.

Method: Combines Chaos Game Representation (CGR) with Rips complex construction from algebraic topology. Maps sequence elements to 2D coordinates via CGR, computes pairwise distances, and constructs Rips complexes to capture both local structural and global topological features. Provides formal guarantees on representation uniqueness, topological stability, and information preservation.

Result: Achieves superior performance over vector-based, sequence language models, and existing image-based methods, with 86.8% accuracy on breast cancer dataset and 94.5% accuracy on lung cancer dataset for anticancer peptide classification.

Conclusion: The topological representation preserves critical sequence information while enabling effective utilization of vision-based deep learning architectures for molecular sequence analysis, offering a promising approach for biological sequence classification tasks.
...
### [134] [Murmur2Vec: A Hashing Based Solution For Embedding Generation Of COVID-19 Spike Sequences](https://arxiv.org/abs/2512.10147)
*Sarwan Ali,Taslim Murad*

Main category: cs.LG

TL;DR: A scalable embedding method using hashing for SARS-CoV-2 spike protein sequences achieves 86.4% classification accuracy with 99.81% faster embedding generation compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for COVID-19 sequence analysis face scalability issues: phylogenetic trees are computationally intensive for millions of sequences, while current embedding methods require aligned sequences or have poor performance and high runtime costs.
...
Result: Achieved up to 86.4% classification accuracy while reducing embedding generation time by as much as 99.81% compared to baseline and state-of-the-art biological sequence embedding methods.

Conclusion: The proposed hashing-based embedding method offers a fast, effective, and scalable solution for large-scale viral sequence analysis, addressing computational bottlenecks in current approaches.
...
### [135] [Rethinking Causal Discovery Through the Lens of Exchangeability](https://arxiv.org/abs/2512.10152)
*Tiago Brogueira,Mário Figueiredo*

Main category: cs.LG

TL;DR: The paper reframes i.i.d. causal discovery in terms of exchangeability, shows existing methods rely on exchangeability assumptions, creates an exchangeable-only synthetic dataset that better matches real-world data, and demonstrates a neural network trained on this dataset performs comparably to state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Traditional causal discovery methods have been developed separately for i.i.d. and timeseries data. The authors argue that the i.i.d. setting should be reframed in terms of exchangeability, which is a more general symmetry principle than i.i.d.

Method: 1) Conceptual argument linking experimental causal inference's dependency on exchangeability to causal discovery; 2) Empirical analysis showing existing i.i.d. methods rely on exchangeability assumptions; 3) Creation of a novel synthetic dataset that enforces only exchangeability (not stronger i.i.d. assumptions); 4) Development of a neural-network-based causal discovery algorithm trained exclusively on the exchangeable synthetic dataset.

Result: 1) The exchangeable synthetic dataset mirrors the statistical structure of real-world "i.i.d." data better than existing i.i.d. synthetic datasets; 2) The neural network trained on the exchangeable dataset performs similarly to state-of-the-art i.i.d. methods on real-world benchmarks; 3) The widely-used Tübingen dataset consists mainly of exchangeable (not i.i.d.) examples.

Conclusion: Causal discovery should be reframed in terms of exchangeability rather than i.i.d. assumptions, as exchangeability is more general and better reflects real-world data structures. The exchangeable framework provides a more accurate foundation for causal discovery methods.
...
### [136] [CIEGAD: Cluster-Conditioned Interpolative and Extrapolative Framework for Geometry-Aware and Domain-Aligned Data Augmentation](https://arxiv.org/abs/2512.10178)
*Keito Inoshita,Xiaokang Zhou,Akira Kawai,Katsutoshi Yada*

Main category: cs.LG

TL;DR: CIEGAD is a geometry-aware, domain-aligned data augmentation framework that uses cluster conditioning and hierarchical allocation to generate both in-distribution and out-of-distribution data for underrepresented semantic regions, improving classification performance on imbalanced datasets.


<details>
  <summary>Details</summary>
Motivation: Practical deep learning faces data scarcity and label imbalance, creating semantically uncovered regions in real-world distributions that cause misclassification near class boundaries and unstable behavior in peripheral areas. Existing LLM-based augmentation lacks integrated directional control, domain alignment, and quality control.

Method: CIEGAD uses cluster conditioning to build domain profiles, hierarchical frequency-geometric allocation for generation planning, interpolative and extrapolative synthesis for directional control, and geometry-constrained filtering with LLM-as-a-Judge for quality control.

Result: Experiments show CIEGAD effectively extends real-world data distribution peripheries while maintaining high alignment and semantic diversity. It consistently improves F1 and recall scores for long-tailed and multi-class classification tasks.
...
### [137] [Assessing Neuromorphic Computing for Fingertip Force Decoding from Electromyography](https://arxiv.org/abs/2512.10179)
*Abolfazl Shahrooei,Luke Arthur,Om Patel,Derek Kamper*

Main category: cs.LG

TL;DR: SNN vs TCN for decoding fingertip force from HD-sEMG motor-unit firing: TCN outperforms SNN (4.44% vs 8.25% MVC RMSE), but SNN shows promise as neuromorphic baseline.


<details>
  <summary>Details</summary>
Motivation: HD-sEMG provides noninvasive neural interface for assistive/rehabilitation control, but mapping neural activity to motor intent remains challenging. Need to evaluate neuromorphic approaches like SNNs against conventional deep learning methods.

Method: Collected data from single participant (10 trials) with two forearm electrode arrays. Used FastICA-based decomposition to obtain MU firing activity. Trained SNN (spiking neural network) and TCN (temporal convolutional network) on overlapping windows with end-to-end causal convolutions for fingertip force decoding.

Result: TCN achieved 4.44% MVC RMSE (Pearson r = 0.974) on held-out trials, while SNN achieved 8.25% MVC RMSE (r = 0.922). TCN was more accurate, but SNN performance suggests potential as neuromorphic baseline.

Conclusion: While TCN outperformed SNN in accuracy, SNN represents a realistic neuromorphic baseline that could close performance gap with modest architectural and hyperparameter refinements, making it promising for neuromorphic computing applications.
...
### [138] [MiniF2F-Dafny: LLM-Guided Mathematical Theorem Proving via Auto-Active Verification](https://arxiv.org/abs/2512.10187)
...
<details>
  <summary>Details</summary>
Motivation: To bridge mathematical reasoning benchmarks from interactive theorem provers to automated theorem provers, specifically translating miniF2F to Dafny to evaluate automated verification capabilities.

Method: Translated miniF2F benchmark to Dafny, tested empty proof verification, then evaluated 12 LLMs for providing proof hints when empty proofs fail, using iterative error correction.
...
Conclusion: Effective division of labor between LLMs (high-level guidance) and automation (low-level details) for mathematical reasoning, establishing a benchmark for automated theorem proving in Dafny.
...
### [139] [Exact Recovery of Non-Random Missing Multidimensional Time Series via Temporal Isometric Delay-Embedding Transform](https://arxiv.org/abs/2512.10191)
*Hao Shu,Jicheng Li,Yu Jin,Ling Zhou*

Main category: cs.LG

TL;DR: LRTC-TIDT: A tensor completion method using temporal isometric delay-embedding transform to exactly recover multidimensional time series with non-random missing data.


<details>
  <summary>Details</summary>
Motivation: Non-random missing data in multidimensional time series undermines data-driven analysis, and existing tensor completion methods fail to handle non-random missingness both methodologically and theoretically.

Method: Proposes temporal isometric delay-embedding transform to construct Hankel tensors whose low-rankness comes from time series smoothness/periodicity. Uses LRTC-TIDT model with t-SVD framework for tensor completion.

Result: Achieves exact recovery under prescribed non-random sampling conditions and incoherence assumptions. Outperforms existing methods in simulations and real-world tasks (network flow, traffic estimation, temperature prediction).

Conclusion: LRTC-TIDT provides theoretical guarantees and practical effectiveness for recovering multidimensional time series with non-random missing data, addressing fundamental limitations of existing tensor completion approaches.
...
### [140] [Federated Domain Generalization with Latent Space Inversion](https://arxiv.org/abs/2512.10224)
...
<details>
  <summary>Details</summary>
Motivation: Existing FedDG methods improve generalization but compromise privacy by sharing client data statistics. Need to address privacy concerns while handling non-i.i.d. client distributions that may discard local adaptations during aggregation.

Method: Two key innovations: 1) Latent space inversion technique to enforce domain invariance across local models while preserving privacy, 2) Important weight aggregation strategy to prioritize parameters that significantly influence local model predictions during aggregation.

Result: Extensive experiments show superior results over state-of-the-art methods with less communication overhead, demonstrating both improved generalization and better privacy preservation.

Conclusion: The proposed approach successfully addresses privacy concerns in FedDG while maintaining strong generalization performance through novel local training and aggregation techniques, making it suitable for practical federated learning scenarios with non-i.i.d. clients.
...
### [141] [Adaptive Information Routing for Multimodal Time Series Forecasting](https://arxiv.org/abs/2512.10229)
...
TL;DR: AIR framework uses text data to dynamically guide time series forecasting models, controlling how multivariate time series information should be combined, with a text-refinement pipeline using LLMs to process raw text data.


<details>
  <summary>Details</summary>
Motivation: Traditional time series forecasting relying only on historical data is insufficient for accurate predictions due to limited information. Multimodal approaches incorporating text data alongside time series data can address this challenge.

Method: Adaptive Information Routing (AIR) framework that uses text information to dynamically guide time series models by controlling how and to what extent multivariate time series information should be combined. Includes a text-refinement pipeline using large language models to convert raw text data into suitable form for multimodal forecasting.

Result: AIR effectively modulates time series model behavior using textual inputs, significantly enhancing forecasting accuracy in various tasks with real-world market data (crude oil price, exchange rates). A benchmark is introduced to facilitate multimodal forecasting experiments.

Conclusion: AIR represents a novel approach to multimodal time series forecasting that dynamically leverages text data to guide time series models, outperforming existing methods that treat text as interchangeable auxiliary features.
...
### [142] [R^2-HGP: A Double-Regularized Gaussian Process for Heterogeneous Transfer Learning](https://arxiv.org/abs/2512.10258)
...
<details>
  <summary>Details</summary>
Motivation: Multi-output Gaussian processes face three key challenges in transfer learning: 1) heterogeneous input spaces between source and target domains, 2) ignoring prior physical knowledge leading to unstable mappings, and 3) inappropriate information sharing causing negative transfer. Traditional models fail to address these issues comprehensively.

Method: Proposes R²-HGP framework with: 1) trainable prior probability mapping to align heterogeneous inputs as latent variables, 2) multi-source transfer GP integrated with conditional variational autoencoder (CVAE), 3) physical knowledge regularization to ensure alignment adheres to known physics, and 4) sparsity penalty on transfer coefficients for adaptive source selection.

Result: Extensive simulations and real-world engineering case studies demonstrate consistent superiority over state-of-the-art benchmarks across diverse evaluation metrics, validating the effectiveness of the proposed framework.

Conclusion: R²-HGP successfully addresses key challenges in multi-source transfer learning by providing a unified framework for heterogeneous domain alignment, physical knowledge incorporation, and adaptive source selection to prevent negative transfer.
...
### [143] [A Kernel-based Resource-efficient Neural Surrogate for Multi-fidelity Prediction of Aerodynamic Field](https://arxiv.org/abs/2512.10287)
...
<details>
  <summary>Details</summary>
Motivation: Need for fast, accurate surrogate models for aerodynamic simulations in design/optimization, especially when computational resources are limited and high-fidelity data is sparse.

Method: Proposes KHRONOS architecture blending sparse HF data with LF information using variational principles, interpolation theory, and tensor decomposition for heavy pruning. Evaluated on AirfRANS dataset (HF) and NeuralFoil (LF) with varying HF data availability (0%, 10%, 30%) and geometry complexity to predict surface pressure coefficient.

Result: KHRONOS achieves comparable accuracy to MLP, GNN, and PINN but excels in resource constraints: requires orders of magnitude fewer parameters and delivers much faster training/inference at comparable accuracy.

Conclusion: KHRONOS and similar architectures effectively balance accuracy and efficiency for multi-fidelity aerodynamic field prediction, particularly valuable in resource-constrained scenarios.
...
### [144] [An Interpretable AI Tool for SAVR vs TAVR in Low to Intermediate Risk Patients with Severe Aortic Stenosis](https://arxiv.org/abs/2512.10308)
*Vasiliki Stoumpou,Maciej Tysarowski,Talhat Azemi,Jawad Haider,Howard L. Haronian,Robert C. Hagberg,Dimitris Bertsimas*

Main category: cs.LG

TL;DR: Interpretable prescriptive framework using Optimal Policy Trees provides transparent, data-driven TAVR vs SAVR recommendations that reduce estimated 5-year mortality by 13.8-20.3% compared to real-life practice.


<details>
  <summary>Details</summary>
Motivation: Treatment selection between SAVR and TAVR for aortic stenosis patients remains variable due to patient heterogeneity and institutional preferences. Existing models predict risk but lack interpretable, individualized treatment recommendations that directly optimize long-term outcomes.

Method: Interpretable prescriptive framework integrating prognostic matching, counterfactual outcome modeling, and Optimal Policy Trees (OPT). Uses data from two hospitals, emulates randomization via prognostic matching and sample weighting, estimates counterfactual mortality under both treatments, and trains policy model to partition patients into clinically coherent subgroups.

Result: OPT prescriptions showed estimated reduction in 5-year mortality of 20.3% in Hartford and 13.8% in St. Vincent's relative to real-life prescriptions, demonstrating promising generalizability across institutions. Learned decision boundaries aligned with real-world outcomes and clinical observations.

Conclusion: First interpretable prescriptive framework providing transparent, data-driven TAVR vs SAVR recommendations that improve estimated long-term outcomes in both internal and external cohorts, contributing to more systematic, evidence-based precision medicine in structural heart disease.
...
### [145] [A Privacy-Preserving Cloud Architecture for Distributed Machine Learning at Scale](https://arxiv.org/abs/2512.10341)
*Vinoth Punniyamoorthy,Ashok Gadi Parthi,Mayilsamy Palanigounder,Ravi Kiran Kodali,Bikesh Kumar,Kabilan Kannan*

Main category: cs.LG

TL;DR: Cloud-native privacy architecture combining federated learning, differential privacy, zero-knowledge proofs, and RL-based governance for scalable, verifiable distributed ML with strong privacy guarantees.


<details>
  <summary>Details</summary>
Motivation: Distributed ML systems need strong privacy guarantees, verifiable compliance, and scalable deployment across heterogeneous multi-cloud environments while protecting sensitive data.

Method: Integrates federated learning (decentralized training), differential privacy (formal privacy budgets), zero-knowledge compliance proofs (cryptographic verification), and adaptive governance powered by reinforcement learning for risk-aware policy enforcement.

Result: Prototype deployed across hybrid Kubernetes clusters shows reduced membership-inference risk, consistent enforcement of formal privacy budgets, stable model performance under differential privacy, and minimal overhead while maintaining utility.

Conclusion: The framework establishes a practical foundation for deploying trustworthy and compliant distributed ML systems at scale with continuous, risk-aware governance across institutions and cloud platforms.
...
### [146] [Dynamics of Agentic Loops in Large Language Models: A Geometric Theory of Trajectories](https://arxiv.org/abs/2512.10350)
*Nicolas Tacheny*

Main category: cs.LG

TL;DR: This paper introduces a geometric framework to analyze agentic loops in LLMs as discrete dynamical systems, distinguishing artifact vs embedding spaces and introducing isotonic calibration to measure trajectories, clusters, and attractors.


<details>
  <summary>Details</summary>
Motivation: Agentic systems built on LLMs operate through recursive feedback loops, but the geometric behavior of these loops (convergence, divergence, complex dynamics) remains poorly understood. There's a need for rigorous analysis of how these iterative transformations behave in semantic space.

Method: The paper introduces a geometric framework treating iterative transformations as discrete dynamical systems. It distinguishes artifact space (linguistic transformations) from embedding space (geometric measurements). Introduces isotonic calibration to eliminate cosine similarity bias from embedding anisotropy. Uses controlled experiments on singular agentic loops to analyze trajectories.

Result: Identified two fundamental regimes: contractive rewriting loops converge toward stable attractors with decreasing dispersion, while exploratory summarize and negate loops produce unbounded divergence with no cluster formation. These regimes show distinct geometric signatures of contraction and expansion.

Conclusion: Prompt design directly governs the dynamical regime of an agentic loop, enabling systematic control of convergence, divergence and trajectory structure in iterative LLM transformations. The geometric framework provides tools for analyzing and controlling agentic behavior.
...
### [147] [Better Prevent than Tackle: Valuing Defense in Soccer Based on Graph Neural Networks](https://arxiv.org/abs/2512.10355)
*Hyunsung Kim,Sangwoo Seo,Hoyoung Choi,Tom Boomstra,Jinsung Yoon,Chanyoung Park*

Main category: cs.LG

TL;DR: DEFCON is a framework using Graph Attention Networks to quantify defensive contributions in soccer by measuring how defenders affect opponents' Expected Possession Value, going beyond traditional on-ball action metrics.


<details>
  <summary>Details</summary>
Motivation: Current soccer analytics focus too much on visible on-ball defensive actions (interceptions, tackles), missing the true impact of defenders who prevent dangerous opportunities before they arise. There's a need to better measure defensive contributions that happen off the ball.

Method: Uses Graph Attention Networks to estimate success probability and expected value of each attacking option, plus each defender's responsibility for stopping it. Calculates Expected Possession Value (EPV) before/after each action, assigning positive/negative credits to defenders based on whether they reduced or increased opponent's EPV.

Result: Trained on 2023-24 and evaluated on 2024-25 Eredivisie data, DEFCON's aggregated player credits show strong positive correlations with market valuations. Framework enables practical applications like in-game defensive contribution timelines, spatial analyses across pitch zones, and attacker-defender interaction summaries.

Conclusion: DEFCON provides a comprehensive framework for quantifying defensive contributions in soccer that captures off-ball defensive impact, addressing limitations of traditional metrics and offering practical analytical applications for player evaluation.
...
### [148] [GPG: Generalized Policy Gradient Theorem for Transformer-based Policies](https://arxiv.org/abs/2512.10365)
*Hangyu Mao,Guangting Dong,Zhicheng Dou*

Main category: cs.LG

TL;DR: The paper introduces Generalized Policy Gradient (GPG) Theorem for Transformer-based policies, showing that standard Policy Gradient Theorem and GRPO are special cases, with practical applications for training LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing policy gradient methods may not be optimally designed for Transformer-based policies, and there's a need for a more general framework that unifies different approaches while being specifically tailored for modern LLM architectures.

Method: Developed Generalized Policy Gradient (GPG) Theorem specifically for Transformer-based policies, creating a theoretical framework that encompasses both standard Policy Gradient Theorem and GRPO as special cases.

Result: Demonstrated that both standard Policy Gradient Theorem and GRPO emerge as special cases within the GPG framework, providing a unified theoretical foundation for policy optimization in Transformer-based models.

Conclusion: The GPG Theorem offers a general framework for policy optimization in Transformer-based policies with practical applications for training LLMs, providing new insights into efficient policy optimization and unifying existing approaches.
...
### [149] [Fitting magnetization data using continued fraction of straight lines](https://arxiv.org/abs/2512.10390)
*Vijay Prakash S*

Main category: cs.LG

TL;DR: Paper approximates magnetization nonlinearity using continued fraction of straight lines to model domain alignment behavior.


<details>
  <summary>Details</summary>
Motivation: To understand and mathematically model the nonlinear magnetization response of ferromagnetic materials to external magnetic fields, which arises from microscopic domain alignment.
...
Result: The continued fraction approximation successfully models both growing and shrinking magnetic domain behavior, providing a mathematical framework to interpret magnetization nonlinearity.
...
### [150] [The Eminence in Shadow: Exploiting Feature Boundary Ambiguity for Robust Backdoor Attacks](https://arxiv.org/abs/2512.10402)
...
<details>
  <summary>Details</summary>
Motivation: Current backdoor attacks rely on heuristic brute-force methods without theoretical understanding, limiting attack predictability and adaptability. The lack of rigorous analysis prevents understanding of why low poison rates can be effective.

Method: Theoretical analysis reveals how sparse decision boundaries enable disproportionate model manipulation. Based on this, they derive a closed-form ambiguous boundary region where minimal relabeled samples cause substantial misclassification. They propose Eminence framework that optimizes a universal, visually subtle trigger to strategically exploit vulnerable decision boundaries.

Result: Eminence achieves >90% attack success rate with exceptionally low poison rates (<0.1% vs SOTA >1%), maintains negligible clean-accuracy loss, and demonstrates high transferability across models, datasets, and scenarios. Experiments confirm exponential relationship between margin poisoning and boundary manipulation.

Conclusion: The paper provides theoretical grounding for why low poison rates suffice for effective backdoor attacks through decision boundary analysis. Eminence offers an explainable, robust framework with provable guarantees and inherent stealth properties, advancing backdoor attack research from empirical to theoretical foundations.
...
### [151] [The Operator Origins of Neural Scaling Laws: A Generalized Spectral Transport Dynamics of Deep Learning](https://arxiv.org/abs/2512.10427)
*Yizhou Zhang*
...
<details>
  <summary>Details</summary>
Motivation: Modern deep networks operate in a rough, finite-regularity regime where Jacobian-induced operators exhibit heavy-tailed spectra and strong basis drift, requiring a unified theoretical framework to understand training dynamics.

Method: Derive exact evolution equations from gradient descent, apply Kato perturbation theory to obtain coupled mode ODEs, coarse-grain to a spectral transport-dissipation PDE, analyze self-similar solutions in weak-coupling regime.

Result: Training preserves functional regularity with asymptotic power-law drift; PDE admits self-similar solutions with resolution frontier and polynomial growth; yields explicit scaling-law exponents; connects NTK training (v=0) and feature learning (v≠0).

Conclusion: Provides unified spectral framework connecting operator geometry, optimization dynamics, and universal scaling behavior of modern deep networks, explaining double descent geometry and effective training time scaling.
...
TL;DR: The paper introduces a metacognition framework for AI models using meta-d' to measure metacognitive sensitivity, then uses this score for bandit-based model selection, improving ensemble accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep learning models often have poor calibration where their confidence doesn't reflect true competence, raising the question of whether models truly know what they know. The authors aim to apply human cognitive science concepts of metacognition to evaluate and leverage AI self-assessment capabilities.

Method: Proposes meta-d', a psychologically-grounded measure of metacognitive sensitivity that characterizes how reliably a model's confidence predicts its own accuracy. Uses this dynamic sensitivity score as context for a bandit-based arbiter that performs test-time model selection, learning which expert model to trust for given tasks.

Result: Experiments across multiple datasets and deep learning model combinations (including CNNs and VLMs) demonstrate that this metacognitive approach improves joint-inference accuracy over constituent models.

Conclusion: Provides a novel behavioral account of AI models, recasting ensemble selection as evaluating both short-term signals (confidence prediction scores) and medium-term traits (metacognitive sensitivity). The work bridges cognitive science and AI to improve model reliability through metacognitive evaluation.
...
### [153] [Hybrid Physics-ML Model for Forward Osmosis Flux with Complete Uncertainty Quantification](https://arxiv.org/abs/2512.10457)
...
<details>
  <summary>Details</summary>
Motivation: Forward Osmosis (FO) faces challenges in accurate water flux modeling due to complex mass transfer phenomena. Traditional mechanistic models have empirical parameter variability issues, while purely data-driven models lack physical consistency and proper uncertainty quantification.

Method: A Robust Hybrid Physics-ML framework using Gaussian Process Regression (GPR) trained on residual errors between detailed physical model predictions and experimental water flux. Implements full uncertainty quantification by decomposing total predictive variance into model uncertainty (epistemic) and input uncertainty (aleatoric) using the Delta method for multivariate correlated inputs.

Result: Achieved state-of-the-art performance with Mean Absolute Percentage Error (MAPE) of 0.26% and R² of 0.999 on independent test data, using only 120 training data points, demonstrating robust and reliable surrogate modeling capabilities.

Conclusion: The hybrid physics-ML framework provides a truly robust and reliable surrogate model for advanced FO process optimization and digital twin development, combining physical consistency with data-driven accuracy and comprehensive uncertainty quantification.
...
### [154] [T-SKM-Net: Trainable Neural Network Framework for Linear Constraint Satisfaction via Sampling Kaczmarz-Motzkin Method](https://arxiv.org/abs/2512.10461)
*Haoyu Zhu,Yao Zhang,Jiashen Ren,Qingchun Hou*

Main category: cs.LG

TL;DR: T-SKM-Net integrates Sampling Kaczmarz-Motzkin method into neural networks for efficient constraint satisfaction with theoretical guarantees and significant speedup over traditional solvers.


<details>
  <summary>Details</summary>
Motivation: Neural network constraint satisfaction is crucial for safety-critical applications, but existing methods face efficiency-applicability trade-offs. Hard constraint methods suffer from high computational complexity or restrictive assumptions, while SKM's non-differentiability poses challenges for neural network integration.

Method: Proposes T-SKM-Net framework that transforms mixed constraint problems into pure inequality problems via null space transformation, employs SKM for iterative solving, maps solutions back to original constraint space, and provides theoretical guarantees for post-processing effectiveness and end-to-end trainability with unbiased gradient estimators.

Result: On DCOPF case118 benchmark: 4.27ms/item GPU forward inference with 0.0025% max optimality gap (post-processing mode) and 5.25ms/item with 0.0008% max optimality gap (joint training mode), achieving over 25× speedup compared to pandapower solver while maintaining zero constraint violations.

Conclusion: T-SKM-Net successfully integrates SKM-type methods into neural network constraint satisfaction, providing an efficient framework with theoretical guarantees that handles both equality and inequality constraints while significantly outperforming traditional solvers in speed without sacrificing constraint satisfaction.
...
### [155] [UACER: An Uncertainty-Aware Critic Ensemble Framework for Robust Adversarial Reinforcement Learning](https://arxiv.org/abs/2512.10492)
*Jiaxi Wu,Tiantian Zhang,Yuxing Wang,Yongzhe Chang,Xueqian Wang*

Main category: cs.LG

TL;DR: UACER introduces an uncertainty-aware critic ensemble with time-varying decay mechanism to stabilize adversarial RL training against non-stationary adversaries in complex environments.


<details>
  <summary>Details</summary>
Motivation: Robust adversarial RL faces training instability due to non-stationary learning dynamics caused by trainable adversaries, especially in high-dimensional environments like autonomous driving and robotics.

Method: Two strategies: 1) Diversified critic ensemble using K parallel critic networks for stable Q-value estimation, and 2) Time-varying Decay Uncertainty mechanism that uses variance-derived Q-value aggregation with epistemic uncertainty to dynamically regulate exploration-exploitation trade-off.

Result: UACER outperforms state-of-the-art methods in MuJoCo control problems, demonstrating superior performance, stability, and efficiency.

Conclusion: The proposed uncertainty-aware critic ensemble approach effectively addresses training instability in robust adversarial RL, providing a promising solution for complex sequential decision-making domains.
...
### [156] [Adaptive Replay Buffer for Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2512.10510)
...
<details>
  <summary>Details</summary>
Motivation: Standard O2O RL methods use fixed data-mixing ratios, which struggle to balance early learning stability (needing offline data) with asymptotic performance (needing relevant online experiences). This creates a critical dilemma in managing the trade-off between these competing needs.

Method: Introduces Adaptive Replay Buffer (ARB) - a learning-free, simple approach that dynamically prioritizes data sampling based on 'on-policyness'. It assesses how closely collected trajectories align with current policy behavior and assigns proportional sampling weights to each transition within trajectories.
...
Conclusion: ARB demonstrates the importance of adaptive, behavior-aware replay buffer design for effective O2O RL, providing a simple yet effective solution to the offline-online data balancing problem without complex learning procedures.
...
### [157] [Disentangled and Distilled Encoder for Out-of-Distribution Reasoning with Rademacher Guarantees](https://arxiv.org/abs/2512.10522)
*Zahra Rahiminasab,Michael Yuhas,Arvind Easwaran*

Main category: cs.LG

TL;DR: DDE framework compresses disentangled VAEs for OOD reasoning on resource-constrained devices via teacher-student distillation with theoretical guarantees.


<details>
  <summary>Details</summary>
Motivation: Disentangled VAEs are useful for multi-label OOD reasoning but are too large for deployment on resource-constrained devices like edge devices.

Method: Proposes Disentangled Distilled Encoder (DDE) framework that formalizes teacher-student distillation as constrained optimization with disentanglement constraints to preserve disentanglement during compression.

Result: Establishes theoretical guarantees for disentanglement preservation during distillation using Rademacher complexity, and empirically evaluates compressed models on NVIDIA hardware.

Conclusion: DDE successfully compresses disentangled VAEs for OOD reasoning while preserving disentanglement properties, enabling deployment on resource-constrained devices.
...
TL;DR: VML-MAP: A new method using variational mode-seeking loss to guide diffusion models toward maximum a posteriori estimates for solving inverse problems without task-specific training.
...
Method: Proposes variational mode-seeking loss (VML) that minimizes KL divergence between diffusion posterior and measurement posterior. For linear inverse problems, VML can be analytically derived without approximations. VML-MAP algorithm applies this loss during reverse diffusion steps.
...
Conclusion: VML-MAP provides an effective, theoretically grounded approach for solving inverse problems with diffusion models, offering better performance and faster computation than existing methods.
...
### [159] [Unlocking the Address Book: Dissecting the Sparse Semantic Structure of LLM Key-Value Caches via Sparse Autoencoders](https://arxiv.org/abs/2512.10547)
...
<details>
  <summary>Details</summary>
Motivation: The KV cache is the primary memory bottleneck in long-context LLMs, but is typically treated as an opaque numerical tensor. There's a need to make it interpretable while maintaining attention geometry.

Method: Proposes STA-Attention framework using Top-K Sparse Autoencoders to decompose KV cache into semantic atoms. Introduces Dual-Budget Strategy based on discovered Key-Value asymmetry: sparse routing for Keys vs dense content for Values.

Result: Experiments on Yi-6B, Mistral-7B, Qwen2.5-32B show semantic reconstructions maintain perplexity and zero-shot performance comparable to original models, bridging mechanistic interpretability with faithful attention modeling.

Conclusion: The approach successfully makes KV cache interpretable while preserving attention geometry, revealing fundamental Key-Value asymmetry and enabling efficient semantic compression without performance degradation.
...
### [160] [Is the Information Bottleneck Robust Enough? Towards Label-Noise Resistant Information Bottleneck Learning](https://arxiv.org/abs/2512.10573)
*Yi Huang,Qingyun Sun,Yisen Gao,Haonan Yuan,Xingcheng Fu,Jianxin Li*

Main category: cs.LG

TL;DR: LaT-IB is a novel Information Bottleneck method that introduces a "Minimal-Sufficient-Clean" criterion to make representation learning robust to label noise through noise-aware latent disentanglement and a three-phase training framework.


<details>
  <summary>Details</summary>
Motivation: The standard Information Bottleneck principle is vulnerable to label noise because it strongly relies on accurate labels, which are often noisy in real-world scenarios, leading to performance degradation and overfitting.

Method: LaT-IB introduces a "Minimal-Sufficient-Clean" criterion as a mutual information regularizer, employs noise-aware latent disentanglement to separate clean label information from noise, and uses a three-phase training framework (Warmup, Knowledge Injection, Robust Training) to progressively learn noise-resistant representations.
...
Conclusion: LaT-IB successfully addresses the vulnerability of standard Information Bottleneck to label noise through its MSC criterion and noise-aware disentanglement approach, making representation learning more robust and practical for real-world applications with noisy supervision.
...
### [161] [THeGAU: Type-Aware Heterogeneous Graph Autoencoder and Augmentation](https://arxiv.org/abs/2512.10589)
...
### [162] [Multi-Objective Reward and Preference Optimization: Theory and Algorithms](https://arxiv.org/abs/2512.10601)
*Akhil Agnihotri*

Main category: cs.LG

TL;DR: This thesis advances constrained RL across control, preference learning, and LLM alignment with four key contributions: ACPO for average-cost CMDPs, e-COP for episodic CMDPs, warmPref-PS for RLHF with heterogeneous raters, and MOPO for scalable LLM alignment.


<details>
  <summary>Details</summary>
Motivation: The thesis addresses the need for robust constrained RL methods that can handle safety constraints in control systems, incorporate human preferences efficiently, and align large language models with human values while maintaining theoretical guarantees and practical scalability.

Method: Four main approaches: 1) ACPO integrates sensitivity analysis with trust-region updates for average-cost CMDPs; 2) e-COP uses episodic policy difference lemma for finite-horizon CMDPs; 3) warmPref-PS employs posterior sampling with offline preference data and rater competence modeling; 4) MOPO uses multi-objective constrained optimization with closed-form updates for LLM alignment.

Result: The methods achieve state-of-the-art empirical performance with theoretical guarantees: ACPO provides stable constraint handling, e-COP offers provable performance in safety-critical environments, warmPref-PS reduces regret in RLHF, and MOPO scales to multi-billion-parameter models while remaining robust across alignment settings.

Conclusion: The thesis successfully unifies constrained RL across average-cost, episodic, and preference-driven paradigms, delivering both theoretical advances and practical tools for safe and aligned decision-making in control systems, preference learning, and large language model alignment.
...
### [163] [Uncertainty-Preserving QBNNs: Multi-Level Quantization of SVI-Based Bayesian Neural Networks for Image Classification](https://arxiv.org/abs/2512.10602)
*Hendrik Borras,Yong Wu,Bernhard Klein,Holger Fröning*

Main category: cs.LG

TL;DR: BNNs can be quantized to 4-bit precision with minimal accuracy/uncertainty degradation, achieving 8x memory reduction via systematic multi-level quantization framework.


<details>
  <summary>Details</summary>
Motivation: Bayesian Neural Networks provide principled uncertainty quantification but suffer from substantial computational and memory overhead compared to deterministic networks, limiting deployment on resource-constrained edge devices.

Method: Introduces systematic multi-level quantization framework for Stochastic Variational Inference BNNs with three strategies: Variational Parameter Quantization (VPQ), Sampled Parameter Quantization (SPQ), and Joint Quantization (JQ). Uses logarithmic quantization for variance parameters and specialized activation functions to preserve distributional structure.

Result: BNNs can be quantized down to 4-bit precision while maintaining classification accuracy and uncertainty disentanglement. At 4 bits, Joint Quantization achieves up to 8x memory reduction compared to floating-point implementations with minimal degradation in epistemic and aleatoric uncertainty estimation.

Conclusion: Enables deployment of BNNs on resource-constrained edge devices and provides design guidelines for future analog "Bayesian Machines" operating at inherently low precision.
...
### [164] [Supporting Migration Policies with Forecasts: Illegal Border Crossings in Europe through a Mixed Approach](https://arxiv.org/abs/2512.10633)
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in migration forecasting posed by sudden shifts in migration patterns and limitations in traditional datasets, while responding to the forecasting needs outlined in the EU Pact on Migration and Asylum to support the Asylum and Migration Management Regulation (AMMR).

Method: Integrates machine learning techniques with qualitative insights from migration experts, including a human-assessed covariate to improve predictive capacity. The methodology is tested and validated with known data across five key migratory routes in Europe.

Result: The methodology demonstrates applicability and reliability in migration-related policy contexts, providing policy-relevant forecasts that can inform strategic decisions, early warning systems, and solidarity mechanisms among EU Member States.

Conclusion: This work introduces a novel operational tool for EU migration governance that aligns with academic recommendations by combining data-driven modeling with expert judgment, creating an innovative approach to migration forecasting that addresses both technical and policy needs.
...
### [165] [Token Sample Complexity of Attention](https://arxiv.org/abs/2512.10656)
...
<details>
  <summary>Details</summary>
Motivation: As LLM context windows expand, it's crucial to understand attention behavior at extreme sequence lengths and characterize how attention converges to its infinite-token limit.

Method: Introduces token-sample complexity concept and analyzes convergence at two levels: 1) pointwise uniform convergence of attention maps, and 2) convergence of moments for transformed token distributions. Examines both softmax and hardmax (infinite attention parameter) regimes.

Result: For compactly supported/sub-Gaussian distributions: attention maps converge uniformly at rate C(R)/√n (exponential C(R) dependence on radius R). For transformed token distributions: moments converge at rate C'(R)/n^β with β<½ (polynomial C'(R) dependence). Hardmax regime shows logarithmic convergence. Experiments on synthetic Gaussian data and BERT models confirm predictions.

Conclusion: Token-sample complexity provides a framework for understanding attention convergence at extreme sequence lengths, with different convergence regimes for attention maps vs. transformed distributions, offering insights for practical LLM scaling.
...
### [166] [DCFO Additional Material](https://arxiv.org/abs/2512.10659)
*Tommaso Amico,Pernille Matthews,Lena Krieger,Arthur Zimek,Ira Assent*

Main category: cs.LG

TL;DR: DCFO is a novel method that generates counterfactual explanations specifically for the Local Outlier Factor (LOF) outlier detection algorithm, outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: LOF is widely used but lacks interpretability, and existing counterfactual explanation methods don't address the unique challenges of outlier detection or target classical algorithms like LOF.

Method: DCFO partitions the data space into regions where LOF behaves smoothly, enabling efficient gradient-based optimization to generate counterfactual explanations.

Result: Extensive experiments on 50 OpenML datasets show DCFO consistently outperforms benchmarked competitors in terms of proximity and validity of generated counterfactuals.

Conclusion: DCFO successfully addresses the interpretability gap in LOF by providing effective counterfactual explanations specifically designed for this popular outlier detection method.
...
### [167] [Learning by Analogy: A Causal Framework for Composition Generalization](https://arxiv.org/abs/2512.10669)
*Lingjing Kong,Shaoan Xie,Yang Jiao,Yetian Chen,Yanhui Guo,Simone Shao,Yan Gao,Guangyi Chen,Kun Zhang*

Main category: cs.LG

TL;DR: The paper proposes a causal modularity framework for compositional generalization, showing how hierarchical concept decomposition enables novel combinations and proving identifiability from observable data.


<details>
  <summary>Details</summary>
Motivation: Current models lack understanding of the data structures and principles enabling compositional generalization - the ability to understand and generate novel combinations of learned concepts beyond limited experiences.

Method: Formalizes compositional generalization using principles of causal modularity and minimal changes, introduces a hierarchical data-generating process encoding different concept levels and interactions, and proves identifiability of latent hierarchical structure from observable data.

Result: Theoretically demonstrates that this approach enables compositional generalization supporting complex relations between composed concepts, advancing beyond prior work assuming simpler interactions. Also proves latent hierarchical structure is provably recoverable from observable data like text-image pairs.

Conclusion: The causal modularity framework provides a principled approach to compositional generalization, with theoretical guarantees and practical improvements validated on benchmark datasets.
...
### [168] [HybridVFL: Disentangled Feature Learning for Edge-Enabled Vertical Federated Multimodal Classification](https://arxiv.org/abs/2512.10701)
*Mostafa Anoosha,Zeinab Dehghani,Kuniko Paxton,Koorosh Aslansefat,Dhavalkumar Thakker*

Main category: cs.LG

TL;DR: HybridVFL improves Vertical Federated Learning for Edge AI by using client-side feature disentanglement and server-side cross-modal transformers for better fusion, showing superior performance on skin lesion diagnosis.


<details>
  <summary>Details</summary>
Motivation: Standard VFL systems in Edge AI scenarios (like mobile health diagnostics) suffer performance limitations due to simplistic feature fusion approaches, especially when dealing with sensitive multimodal data on distributed, resource-constrained devices.

Method: HybridVFL framework employs client-side feature disentanglement paired with a server-side cross-modal transformer for context-aware fusion, enabling more sophisticated integration of multimodal features while preserving privacy.

Result: Systematic evaluation on the multimodal HAM10000 skin lesion dataset demonstrates that HybridVFL significantly outperforms standard federated baselines.

Conclusion: Advanced fusion mechanisms are critical for building robust, privacy-preserving systems in Vertical Federated Learning applications, particularly for multimodal Edge AI scenarios like medical diagnostics.
...
### [169] [Beyond the Black Box: Identifiable Interpretation and Control in Generative Models via Causal Minimality](https://arxiv.org/abs/2512.10720)
*Lingjing Kong,Shaoan Xie,Guangyi Chen,Yuewen Sun,Xiangchen Song,Eric P. Xing,Kun Zhang*

Main category: cs.LG

TL;DR: The paper establishes a principled foundation for interpretable generative models using causal minimality to extract hierarchical concepts from diffusion and autoregressive models, enabling component-wise identifiable control.
...
Conclusion: Causal minimality provides a principled foundation for interpretable generative models, offering clear causal interpretation, robust control, and transparent, reliable systems through causally grounded concepts.
...
### [170] [Generalized Spherical Neural Operators: Green's Function Formulation](https://arxiv.org/abs/2512.10723)
*Hao Tang,Hao Chen,Chao Li*

Main category: cs.LG

TL;DR: Proposes GSNO, a spherical neural operator framework using Green's function theory for solving parametric PDEs on spherical domains with flexible balance of equivariance and invariance.


<details>
  <summary>Details</summary>
Motivation: Existing spherical neural operators lack flexibility for real-world complexity while preserving geometric consistency on spherical domains. Need principled operator-theoretic foundation for spherical learning.

Method: Designable spherical Green's function framework with harmonic expansion; absolute/relative position-dependent Green's function; GSNO operator with spectral learning; GSHNet hierarchical architecture with multi-scale spectral modeling and spherical up-down sampling.
...
### [171] [UniExtreme: A Universal Foundation Model for Extreme Weather Forecasting](https://arxiv.org/abs/2508.01426)
*Hang Ni,Weijia Zhang,Hao Liu*

Main category: cs.LG

TL;DR: UniExtreme is a universal foundation model for extreme weather forecasting that addresses spectral disparities and hierarchical drivers of diverse extreme events through adaptive frequency modulation and event prior augmentation.


<details>
  <summary>Details</summary>
Motivation: Current foundation models for weather forecasting have limited ability to predict extreme weather events. Existing approaches either focus on general weather conditions or specialize in specific extreme types, failing to capture real-world atmospheric patterns of diversified extreme events.

Method: Proposes UniExtreme with two key modules: (1) Adaptive Frequency Modulation (AFM) module that captures region-wise spectral differences between normal and extreme weather using learnable Beta-distribution filters and multi-granularity spectral aggregation; (2) Event Prior Augmentation (EPA) module that incorporates region-specific extreme event priors to resolve hierarchical extreme diversity and composite extreme schema via a dual-level memory fusion network.
...
Conclusion: UniExtreme successfully addresses the limitations of existing weather forecasting foundation models by capturing the spectral characteristics and hierarchical drivers of diverse extreme weather events, providing a universal solution for extreme weather prediction.
...
### [172] [LGAN: An Efficient High-Order Graph Neural Network via the Line Graph Aggregation](https://arxiv.org/abs/2512.10735)
*Lin Du,Lu Bai,Jincheng Li,Lixin Cui,Hangyuan Du,Lichi Zhang,Yuting Chen,Zhao Li*

Main category: cs.LG

TL;DR: LGAN is a novel GNN that uses line graphs from node-centered subgraphs for higher-order aggregation, achieving better expressivity than 2-WL with lower complexity and improved interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing GNNs have limited expressivity (bounded by 1-WL test), while k-WL-based GNNs suffer from high computational cost and poor interpretability due to their inability to retain fine-grained node/edge semantics needed for attribution methods.

Method: Proposes Line Graph Aggregation Network (LGAN) that constructs a line graph from the induced subgraph centered at each node to perform higher-order aggregation, enabling more expressive power while maintaining computational efficiency.

Result: Theoretical analysis shows LGAN has greater expressive power than 2-WL with lower time complexity. Empirical evaluations on benchmarks demonstrate LGAN outperforms state-of-the-art k-WL-based GNNs while offering better interpretability.

Conclusion: LGAN effectively addresses the limitations of existing GNNs by providing higher expressivity than 2-WL with lower computational cost and improved interpretability through retention of fine-grained node/edge semantics.
...
### [173] [Template-Free Retrosynthesis with Graph-Prior Augmented Transformers](https://arxiv.org/abs/2512.10770)
*Youjun Zhao*

Main category: cs.LG

TL;DR: Template-free Transformer model for retrosynthesis prediction achieves SOTA on USPTO-50K by injecting molecular graph info into attention and using paired data augmentation.


<details>
  <summary>Details</summary>
Motivation: Existing retrosynthesis models lack the accuracy and robustness needed for practical deployment, and many rely on handcrafted reaction templates or chemical rule engines which limit flexibility.

Method: Template-free Transformer framework that injects molecular graph information into the attention mechanism to combine SMILES sequences with structural cues, plus a paired data augmentation strategy to enhance training diversity and scale.
...
Conclusion: The proposed template-free Transformer approach with graph-enhanced attention and data augmentation provides an effective framework for retrosynthesis prediction without relying on handcrafted templates or rule engines.
...
### [174] [Interpretable and Steerable Concept Bottleneck Sparse Autoencoders](https://arxiv.org/abs/2512.10805)
*Akshay Kulkarni,Tsui-Wei Weng,Vivek Narayanaswamy,Shusen Liu,Wesam A. Sakla,Kowshik Thopalli*

Main category: cs.LG

TL;DR: CB-SAE improves sparse autoencoder interpretability and steerability by pruning low-utility neurons and adding a concept bottleneck aligned to user-defined concepts.


<details>
  <summary>Details</summary>
Motivation: Current sparse autoencoders (SAEs) have limitations: many neurons have low interpretability/steerability, and user-desired concepts are often absent due to unsupervised learning, limiting practical utility for mechanistic interpretability and model steering.

Method: Propose Concept Bottleneck Sparse Autoencoders (CB-SAE) - a post-hoc framework that prunes low-utility neurons and augments the latent space with a lightweight concept bottleneck aligned to user-defined concept sets.
...
Conclusion: CB-SAE addresses key limitations of SAEs by enhancing both interpretability and steerability through concept bottleneck integration, making sparse autoencoders more practical for downstream applications in LLMs and LVLMs.
...
### [175] [Extrapolation of Periodic Functions Using Binary Encoding of Continuous Numerical Values](https://arxiv.org/abs/2512.10817)
...
<details>
  <summary>Details</summary>
Motivation: Neural networks typically struggle with extrapolation beyond their training data, especially for periodic functions. The paper aims to address this limitation by developing an encoding method that enables MLPs to learn and extrapolate periodic signals without prior knowledge of their functional form.

Method: The authors introduce Normalized Base-2 Encoding (NB2E) for encoding continuous numerical values. They use vanilla multi-layer perceptrons (MLPs) with this input encoding and analyze internal activations to understand the mechanism.

Result: MLPs with NB2E successfully extrapolate diverse periodic signals beyond training bounds. Internal activation analysis reveals that NB2E induces bit-phase representations, allowing MLPs to learn signal structure independently of position.

Conclusion: Binary encoding through NB2E provides a powerful method for enabling neural networks to extrapolate periodic functions, overcoming a fundamental limitation in neural network generalization capabilities.
...
### [176] [Learning Controllable and Diverse Player Behaviors in Multi-Agent Environments](https://arxiv.org/abs/2512.10835)
*Atahan Cilan,Atay Özgövde*

Main category: cs.LG

TL;DR: A reinforcement learning framework that enables controllable and diverse player behaviors without human data by training agents to match target behavior vectors in a continuous space.


<details>
  <summary>Details</summary>
Motivation: Existing approaches require large human gameplay data, train separate models for different player types, or lack interpretable mapping between behavioral parameters and learned policies, limiting scalability and controllability.

Method: Define player behavior in N-dimensional continuous space, uniformly sample target behavior vectors, train agents with current and target behavior vectors as input, reward based on normalized reduction in distance between them using PPO-based multi-agent policy.

Result: The framework produces significantly greater behavioral diversity than win-only baseline, reliably matches specified behavior vectors across diverse targets, and can reproduce new/unseen play styles without retraining.

Conclusion: The method offers scalable solution for automated playtesting, game balancing, human-like behavior simulation, and replacing disconnected players in online games.
...
### [177] [Bayesian Symbolic Regression via Posterior Sampling](https://arxiv.org/abs/2512.10849)
...
Method: A Sequential Monte Carlo (SMC) framework for Bayesian symbolic regression that approximates posterior distribution over symbolic expressions. Combines probabilistic selection, adaptive tempering, and normalized marginal likelihood to efficiently explore the search space of symbolic expressions.

Result: The method outperforms standard genetic programming baselines on challenging, noisy benchmark datasets. It reduces overfitting tendency and discovers more accurate and interpretable equations with improved generalization.

Conclusion: The SMC-based Bayesian symbolic regression provides enhanced robustness to noise and enables uncertainty quantification, paving the way for more reliable symbolic regression in scientific discovery and engineering applications.
...
TL;DR: SCSI is a novel transport-based method that learns to invert corruption channels using only corrupted data and black-box access to the forward model, enabling clean data generation without clean training data.


<details>
  <summary>Details</summary>
Motivation: Many scientific and engineering domains lack clean data, only having measurements corrupted through noisy, ill-conditioned channels. Existing generative models require clean data, but real-world applications often only have access to corrupted observations.

Method: Self-Consistent Stochastic Interpolants (SCSI) iteratively update a transport map between corrupted and clean data samples using only corrupted datasets and black-box access to the corruption channel. The method converges to a self-consistent transport map that effectively inverts the corruption.

Result: SCSI demonstrates superior performance on inverse problems in natural image processing and scientific reconstruction. It is computationally efficient compared to variational alternatives, highly flexible handling arbitrary nonlinear forward models, and enjoys theoretical convergence guarantees.

Conclusion: SCSI provides an effective framework for building generative models from corrupted data by learning to invert corruption channels, with applications across scientific and engineering domains where clean data is unavailable.
...
TL;DR: Discrete diffusion language models (DLMs) show different scaling behavior than autoregressive models, with uniform diffusion being more parameter-efficient but data-efficient, making it promising for data-limited scenarios.


<details>
  <summary>Details</summary>
Motivation: Understanding scaling laws is crucial for LLM pre-training efficiency. While discrete diffusion models have been proposed as alternatives to autoregressive models, their scaling behavior hasn't been fully explored, with prior work suggesting they need more resources to match ALM performance.

Method: Studied DLM scaling behavior across different noise types by interpolating between masked and uniform diffusion, while carefully controlling hyperparameters like batch size and learning rate. Scaled uniform diffusion model up to 10B parameters trained for 10^22 FLOPs.

Result: DLM scaling behavior strongly depends on noise type and differs significantly from ALMs. While all noise types converge to similar loss in compute-bound scaling, uniform diffusion requires more parameters but less data for compute-efficient training compared to masked diffusion.

Conclusion: Uniform diffusion models are promising candidates for data-bound settings due to their parameter efficiency and data efficiency trade-off, with the 10B parameter uniform diffusion model being the largest publicly known model of its type.
...
TL;DR: CC-GRMAS is a multi-agent framework using satellite data and environmental signals for improved landslide forecasting in high mountain Asia, with real-time situational awareness and coordinated response planning.
...
Method: CC-GRMAS framework with three interlinked agents: Prediction (forecasting), Planning (response planning), and Execution (intervention). Leverages satellite observations and environmental signals, incorporates local environmental factors, and uses multi-agent coordination.

Result: The approach enhances accuracy of landslide forecasting and enables real-time situational awareness, response planning, and intervention through collaborative agent coordination.
...
### [181] [Echo-CoPilot: A Multi-View, Multi-Task Agent for Echocardiography Interpretation and Reporting](https://arxiv.org/abs/2512.09944)
*Moein Heidari,Mohammad Amin Roohi,Armin Khosravi,Ilker Hacihaliloglu*

Main category: cs.AI

TL;DR: Echo-CoPilot is an AI agent that uses a large language model to orchestrate multiple specialized tools for comprehensive echocardiography interpretation, outperforming existing models on clinical benchmarks.
...
Method: A multi-view, multi-task agent using a large language model in a ReAct-style loop to decompose clinician queries, invoke specialized tools (view recognition, segmentation, measurement, disease prediction, report synthesis), and integrate outputs into guideline-aware answers.

Result: Achieved 50.8% accuracy on MIMIC-EchoQA benchmark, outperforming both general-purpose and biomedical video vision-language models. Demonstrates ability to resolve challenging borderline cases using quantitative measurements and physiologic context.

Conclusion: Echo-CoPilot provides a unified, clinically coherent approach to echocardiography interpretation by orchestrating specialized tools through language model reasoning, showing promise for clinical decision support.
...
### [182] [ExaCraft: Dynamic Learning Context Adaptation for Personalized Educational Examples](https://arxiv.org/abs/2512.09931)
*Akaash Chatterjee,Suman Kundu*

Main category: cs.AI

TL;DR: ExaCraft is an AI system that generates personalized educational examples by adapting to learners' dynamic context, including their struggles, mastery patterns, and progression history.


<details>
  <summary>Details</summary>
Motivation: Existing educational AI tools don't focus on generating personalized examples or adapting to learners' changing understanding, struggles, or growing skills. Learning is most effective when connected to relevant, relatable examples that resonate personally.

Method: ExaCraft uses Google Gemini AI and Python Flask API accessible via Chrome extension. It combines user-defined profiles (location, education, profession, complexity preferences) with real-time analysis of learner behavior to generate culturally relevant examples.

Result: The system adapts to five key learning context aspects: indicators of struggle, mastery patterns, topic progression history, session boundaries, and learning progression signals. Examples evolve from basic concepts to advanced technical implementations based on learner interactions.

Conclusion: ExaCraft demonstrates how AI can generate personalized educational examples that dynamically adapt to individual learners' needs, struggles, and progression patterns for more effective learning.
...
### [183] [Suzume-chan: Your Personal Navigator as an Embodied Information Hub](https://arxiv.org/abs/2512.09932)
*Maya Grace Torii,Takahito Murakami,Shuka Koseki,Yoichi Ochiai*

Main category: cs.AI

TL;DR: An embodied AI agent (Suzume-chan) that creates human-like connection for knowledge sharing through physical presence and conversation, reducing psychological distance.


<details>
  <summary>Details</summary>
Motivation: Current digital tools provide information access but lack the sense of connection needed for deep understanding. Expert knowledge often requires real-time human communication that creates meaningful connection.

Method: Proposed an "Embodied Information Hub" based on Social Presence Theory. Built prototype Suzume-chan - a small, soft AI agent running locally with language model and RAG. It learns from spoken explanations and responds through dialogue.

Result: The embodied agent creates physical and conversational interaction that reduces psychological distance between users and knowledge sources, making knowledge sharing warmer and more human-centered.

Conclusion: Embodied AI agents can bridge the gap between digital information access and human connection, creating more effective knowledge sharing through social presence and reduced psychological distance.
...
### [184] [Exploring Health Misinformation Detection with Multi-Agent Debate](https://arxiv.org/abs/2512.09935)
*Chih-Han Chen,Chen-Han Tsai,Yu-Shao Peng*

Main category: cs.AI

TL;DR: Two-stage framework for health misinformation detection: first uses LLMs to compute agreement scores from evidence, then triggers multi-agent debate when consensus is low.


<details>
  <summary>Details</summary>
Motivation: Health misinformation is proliferating online, requiring both high-quality evidence retrieval and rigorous reasoning for effective verification. Current approaches may lack robust reasoning when evidence is conflicting.

Method: Two-stage framework: 1) Agreement Score Prediction using LLMs to independently evaluate retrieved articles and compute aggregated agreement scores; 2) Multi-Agent Debate triggered when scores fall below threshold, where multiple agents engage in structured debate to synthesize conflicting evidence.
...
Conclusion: The proposed two-stage approach effectively combines evidence assessment with structured reasoning, providing a robust solution for health misinformation detection that handles conflicting evidence through collaborative debate.
...
TL;DR: A new fuzzy optimization framework extending FCM causality for service process design optimization


<details>
  <summary>Details</summary>
Motivation: To create a framework for optimizing information transmission in service process design by extending fuzzy cognitive mapping (FCM) causality with logical implication and hierarchical concept analysis

Method: Extends FCM causality using dynamics to map data into metrics, creates framework examining logical implication and concept hierarchy via multiplex approach, includes thorough FHM analysis with logical steps

Result: Proposes a white-theoretical framework with mathematical and logical analysis, expounds main objectives and orientation, exemplifies framework application for service optimization

Conclusion: The framework provides a systematic approach for service process design optimization through fuzzy cognitive mapping extensions, with elegant mathematical foundation for practical application
...
TL;DR: SciEx is a modular framework for extracting fine-grained scientific information from literature using LLMs, addressing challenges like long documents, multi-modal content, and evolving data schemas.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based extraction tools struggle with scientific literature's complexities: long documents, multi-modal content, inconsistent information across publications, and rapidly changing extraction schemas that make system re-architecture difficult.

Method: SciEx is a modular, composable framework that decouples key components: PDF parsing, multi-modal retrieval, extraction, and aggregation. This design enables extensibility and flexible integration of new models, prompting strategies, and reasoning mechanisms.
...
Conclusion: SciEx offers a practical solution for scientific information extraction that addresses real-world challenges through modular design, enabling researchers to adapt to evolving extraction needs while maintaining accuracy and consistency.
...
TL;DR: DynaMate is an automated multi-agent LLM framework that autonomously designs and executes complete molecular dynamics workflows for protein and protein-ligand systems, including free energy calculations, overcoming technical barriers in MD simulation setup.


<details>
  <summary>Details</summary>
Motivation: Molecular dynamics simulations are crucial for studying biomolecular systems but have high technical complexity in setup (parameterization, input preparation, software configuration) that limits widespread adoption in drug discovery and protein engineering.

Method: DynaMate uses a modular multi-agent framework with three specialized modules that interact to plan experiments, perform simulations, and analyze results. It integrates dynamic tool use, web search, PaperQA, and self-correcting behavior to autonomously execute MD workflows.

Result: The framework successfully performed full MD simulations across twelve benchmark systems of varying complexity, corrected runtime errors through iterative reasoning, and produced meaningful analyses of protein-ligand interactions with reliable performance.

Conclusion: DynaMate enables standardized, scalable, and time-efficient molecular modeling pipelines for biomolecular and drug design applications by automating the complex technical setup of MD simulations.
...
TL;DR: SimWorld-Robotics (SWR) is a photorealistic urban simulation platform with procedurally generated environments and dynamic elements, featuring two challenging robot benchmarks for evaluating multimodal instruction-following and multi-agent collaboration in realistic urban scenarios.


<details>
  <summary>Details</summary>
Motivation: Current foundation models for robotics are mainly focused on indoor, household scenarios, lacking evaluation in large-scale, realistic urban environments with dynamic elements like pedestrians and traffic systems.

Method: Built on Unreal Engine 5, SWR procedurally generates unlimited photorealistic urban scenes with dynamic elements (pedestrians, traffic systems) and supports multi-robot control and communication. Two benchmarks are created: (1) multimodal instruction-following task for vision-language navigation, and (2) multi-agent search task requiring cooperative communication.

Result: State-of-the-art models, including vision-language models (VLMs), struggle with the tasks, demonstrating lack of robust perception, reasoning, and planning abilities necessary for urban environments.

Conclusion: SWR provides a comprehensive simulation platform for evaluating critical robot capacities in realistic urban scenarios, revealing current model limitations and establishing challenging benchmarks for future embodied AI research in complex outdoor environments.
...
TL;DR: PDT introduces a parallel decoder transformer with speculative note conditioning to enable coordinated parallel generation in frozen LLMs, achieving 77.8% precision in coverage prediction without retraining base model weights.


<details>
  <summary>Details</summary>
Motivation: Autoregressive decoding in LLMs creates latency bottlenecks scaling linearly with output length. Existing parallelization methods like Skeleton-of-Thought suffer from coherence drift due to lack of cross-stream communication between parallel generation streams.

Method: Introduces Parallel Decoder Transformer (PDT) with lightweight Speculative Note Conditioning (SNC) adapters injected into a frozen pre-trained model. Uses a shared dynamic latent space for parallel streams to synchronize via semantic "notes" broadcast to a global bus, gated by learned verification head. Formulates coordination as speculative consensus problem.

Result: Validated on 50,000-step curriculum using frozen 20B-parameter backbone. Achieves 77.8% precision in coverage prediction and recovers approximate serial semantics without modifying trunk weights. Demonstrates effective self-correction capabilities.

Conclusion: PDT establishes a scalable, efficient alternative to full model fine-tuning for structured parallel generation, enabling coordinated parallel decoding while maintaining coherence through lightweight adapters rather than retraining the base model.
...
<details>
  <summary>Details</summary>
Motivation: AI alignment research has diverged into two parallel tracks (safety vs ethics) with different priorities, methodologies, and institutional homes, creating a conceptual and institutional divide that hampers comprehensive alignment efforts.

Method: Large-scale bibliometric and co-authorship network analysis of 6,442 papers from twelve major ML/NLP conferences (2020-2025) to quantify collaboration patterns and structural separation between safety and ethics communities.
...
TL;DR: LLMs develop linear representations of user demographics in activation space that can be decoded from indirect cues like names and occupations, and these implicit demographic representations actively influence downstream behavior like career recommendations.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs encode sociodemographic attributes of human conversational partners from indirect cues, and to investigate whether models that pass bias benchmarks still harbor and leverage implicit biases that affect their behavior.

Method: Probed residual streams across layers of four open transformer-based LLMs (Magistral 24B, Qwen3 14B, GPT-OSS 20B, OLMo2-1B) using explicit demographic disclosure prompts, then showed same probes predict demographics from implicit cues like names and occupations.

Result: LLMs develop linear representations of demographics where stereotypically associated attributes are encoded along interpretable geometric directions; names activate census-aligned gender/race representations, occupations trigger representations correlated with real-world workforce statistics; these implicit representations shape downstream behavior like career recommendations.

Conclusion: Models that pass bias benchmark tests may still harbor and leverage implicit biases, with significant implications for fairness when deployed at scale, as these implicit demographic representations actively influence model behavior.
...
Method: Propose using sparse autoencoders (SAEs) to create SAE embeddings - representations whose dimensions map to interpretable concepts. Applied to four data analysis tasks to demonstrate cost-effectiveness and controllability.

Result: SAE embeddings uncover 2-8x bigger differences at lower cost than LLMs, identify biases more reliably, enable controllable clustering along axes of interest, and outperform dense embeddings on property-based retrieval. Case studies reveal model behavior changes and training data influences.

Conclusion: SAEs serve as a versatile tool for unstructured data analysis, highlighting the importance of interpreting models through their data and providing interpretable, controllable representations for text corpus analysis.
...
TL;DR: The paper extends Gödel's incompleteness theorem to AI systems, establishing fundamental information-theoretic limitations for AI security and alignment that cannot be fully overcome.


<details>
  <summary>Details</summary>
Motivation: To identify and understand the inherent limitations of AI systems in achieving perfect security and alignment, which is crucial for responsible AI adoption and managing expectations about what AI can and cannot guarantee.

Method: Extends Gödel's incompleteness theorem (which shows inherent limitations in formal mathematical systems) to the domain of AI systems, establishing information-theoretic bounds on what can be guaranteed about AI security and alignment properties.

Result: Proves that there are fundamental, unavoidable limitations to achieving perfect AI security and alignment - certain properties cannot be fully verified or guaranteed within the AI systems themselves, similar to how Gödel showed certain mathematical truths cannot be proven within formal systems.

Conclusion: AI systems have inherent cognitive reasoning limitations that cannot be completely overcome, requiring practical approaches to manage these limitations rather than attempting to achieve perfect security and alignment. These findings have broad implications for how we design, deploy, and regulate AI systems.
...
TL;DR: Researchers analyze conspiratorial discourse in Singapore Telegram groups using a two-stage framework: fine-tuning RoBERTa for classification and building a signed belief graph with a novel GNN model to identify narrative archetypes.


<details>
  <summary>Details</summary>
Motivation: Conspiratorial discourse is embedded in digital ecosystems but difficult to study; understanding its structure and spread in everyday discussions rather than isolated echo chambers is important for online radicalization research.

Method: Two-stage framework: 1) Fine-tune RoBERTa-large to classify messages as conspiratorial (F1=0.866), 2) Build signed belief graph with message nodes and belief-alignment edges, then develop SiBeGNN with Sign Disentanglement Loss to separate ideology from style.

Result: Identified 7 narrative archetypes across 553,648 messages: legal topics, medical concerns, media discussions, finance, contradictions in authority, group moderation, and general chat. SiBeGNN achieved better clustering (cDBI=8.38) than baselines (13.60-67.27) with 88% expert agreement.

Conclusion: Conspiratorial discourse operates within ordinary social interactions, challenging assumptions about online radicalization. The framework advances belief-driven discourse analysis with applications for stance detection, political communication, and content moderation.
...
TL;DR: AgriRegion is a RAG framework for region-aware agricultural advisory that reduces LLM hallucinations by incorporating geospatial metadata and region-prioritized re-ranking, achieving 10-20% hallucination reduction.
...
Method: AgriRegion uses Retrieval-Augmented Generation with geospatial metadata injection and region-prioritized re-ranking, restricting knowledge base to verified local agricultural extension services and enforcing geo-spatial constraints during retrieval.

Result: AgriRegion reduces hallucinations by 10-20% compared to state-of-the-art LLM systems and significantly improves trust scores, as evaluated on the novel benchmark dataset AgriRegion-Eval (160 domain-specific questions across 12 agricultural subfields).

Conclusion: AgriRegion provides a specialized framework for high-fidelity, region-aware agricultural advisory that addresses the critical problem of contextual hallucination in LLMs for agriculture.
...
<details>
  <summary>Details</summary>
Motivation: To track how transparency practices of major foundation model developers evolve as these companies become increasingly important, and to assess the impact of policy interventions on transparency.

Method: Annual index evaluating transparency using indicators related to data acquisition, usage data, monitoring, and other factors; third edition introduces new indicators and evaluates additional companies including Alibaba, DeepSeek, and xAI.

Result: Transparency has deteriorated: average score fell from 58 in 2024 to 40 in 2025. Companies are most opaque about training data, training compute, and post-deployment usage. IBM scored highest (95), while xAI and Midjourney scored lowest (14). Frontier Model Forum companies scored in the middle.

Conclusion: Despite increasing policy mandates for transparency, foundation model developers have become less transparent, revealing critical information deficits that require more aggressive policy interventions.
...
TL;DR: CP-Env is a controllable agentic hospital environment that evaluates LLMs across end-to-end clinical pathways, simulating dynamic hospital ecosystems with patient/physician agents and providing comprehensive evaluation of clinical efficacy, process competency, and professional ethics.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks focus on static exams or isolated dialogues, which inadequately evaluate LLMs in dynamic clinical scenarios with complex decision-making and transitions between different stages of care.

Method: CP-Env simulates a hospital ecosystem with patient and physician agents, constructing scenarios ranging from triage to specialist consultation, diagnostic testing, and multidisciplinary team meetings. It enables branching, long-horizon task execution following real hospital adaptive healthcare flow.
...
Conclusion: CP-Env advances medical AI agents development through comprehensive end-to-end clinical evaluation, providing a benchmark and evaluation tools for further research in dynamic clinical pathway scenarios.
...
TL;DR: A multi-objective reinforcement learning approach is proposed to optimize operator sequences in search/optimization algorithms, addressing efficiency challenges in multi-objective optimization.


<details>
  <summary>Details</summary>
Motivation: Optimization efficiency remains challenging, especially for multi-objective cases. While using multiple operators instead of single operators shows promise, finding optimal operator sequences requires further investigation, particularly for multi-objective optimization where existing work is limited.

Method: Proposes a multi-objective reinforcement learning approach to generalize experiences and optimize operator sequences in optimization algorithms. The paper describes a general framework with certain stages completed and phases still outstanding.

Result: The paper presents an overview of the proposed approach rather than final results. It describes a framework in development with some stages completed and others outstanding, aiming to demonstrate the efficiency of multi-objective reinforcement learning for operator sequence optimization.

Conclusion: Multi-objective reinforcement learning shows promise for addressing efficiency challenges in optimization by learning optimal operator sequences, but the approach requires further development and validation through the outstanding phases of the proposed framework.
...
TL;DR: Extends Predict-and-Search framework to parametric MIPs with ID-PaS, an identity-aware learning approach that handles heterogeneous variables, outperforming Gurobi and PaS on real-world problems.


<details>
  <summary>Details</summary>
Motivation: Existing Predict-and-Search methods are limited to binary problems and don't handle fixed variables common in practical settings, creating a need for more flexible approaches that can work with parametric mixed-integer programs and heterogeneous variables.

Method: Proposes ID-PaS (Identity-aware Predict-and-Search), an extension of the PaS framework to parametric MIPs that incorporates identity-aware learning to enable ML models to effectively handle heterogeneous variables and fixed variables in practical settings.

Result: Experiments on several real-world large-scale problems show that ID-PaS consistently achieves superior performance compared to both the state-of-the-art commercial solver Gurobi and the original PaS framework.

Conclusion: ID-PaS successfully extends the Predict-and-Search paradigm to parametric MIPs with identity-aware learning, demonstrating practical advantages for real-world combinatorial optimization problems with heterogeneous variables.
...
TL;DR: A novel reverse thinking framework improves LLMs' ability to detect missing information by transforming the problem into backward reasoning, outperforming traditional forward reasoning methods.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with problems involving missing information, exhibiting incomplete responses, factual errors, and hallucinations. Traditional forward reasoning approaches like Chain-of-Thought and Tree-of-Thought often fail to systematically identify and recover omitted information.

Method: Proposes a reverse thinking framework that guides LLMs through backward reasoning to identify necessary conditions and pinpoint missing elements. The approach transforms missing information identification into a more manageable backward reasoning problem.

Result: Experimental results demonstrate substantial performance gains compared to traditional forward reasoning methods, significantly improving model accuracy on missing information detection tasks.
...
TL;DR: NAC is a novel continuous-time attention mechanism that reformulates attention logits as ODE solutions using biologically-inspired sparse gates, offering three computation modes and improved efficiency for irregular time-series tasks.


<details>
  <summary>Details</summary>
Motivation: Attention mechanisms improve representation learning but their discrete nature limits continuous-time modeling capabilities. The authors aim to create a biologically plausible continuous-time attention mechanism that can handle irregular time-series data more effectively.

Method: Neuronal Attention Circuit (NAC) reformulates attention logits computation as a linear first-order ODE solution using sparse sensory gates derived from C. elegans neuronal circuit policies. It replaces dense projections with sparse gates, uses a sparse backbone network with two heads for content-target and learnable time-constant gates, and offers three computation modes: explicit Euler integration, exact closed-form solution, and steady-state approximation. Includes sparse Top-K pairwise concatenation for memory efficiency.
...
Conclusion: NAC provides a biologically plausible, theoretically grounded continuous-time attention mechanism with rigorous guarantees (state stability, bounded errors, universal approximation) that effectively handles irregular time-series while maintaining competitive performance and efficiency.
...
TL;DR: The paper introduces CogVision, a dataset for analyzing vision-language models' internal mechanisms, particularly attention heads' functional roles in multimodal reasoning, revealing their sparsity, distribution patterns, and critical importance for performance.


<details>
  <summary>Details</summary>
Motivation: Despite strong performance on benchmarks, vision-language models remain largely black boxes. The authors aim to develop an interpretability framework to systematically analyze VLM internal mechanisms, specifically focusing on understanding the functional roles of attention heads in multimodal reasoning processes.

Method: 1) Introduce CogVision dataset that decomposes complex multimodal questions into step-by-step subquestions simulating human chain-of-thought reasoning, each associated with specific receptive/cognitive functions. 2) Use probing-based methodology to identify attention heads specializing in these functions (functional heads). 3) Analyze functional heads across diverse VLM families. 4) Conduct intervention experiments (removing/emphasizing functional heads) to test their role.

Result: 1) Functional heads are universally sparse across VLM families. 2) They vary in number and distribution across different functions. 3) They mediate interactions and exhibit hierarchical organization. 4) Intervention experiments show: removing functional heads degrades performance, while emphasizing them enhances accuracy, demonstrating their critical role in multimodal reasoning.

Conclusion: The findings provide new insights into VLM cognitive organization and suggest promising directions for designing models with more human-aligned perceptual and reasoning abilities. The interpretability framework and CogVision dataset enable systematic analysis of VLM internal mechanisms.
...
TL;DR: A framework called Ten Criteria for Trustworthy Orchestration AI integrates governance into AI systems to bridge the gap between technical capabilities and institutional accountability.


<details>
  <summary>Details</summary>
Motivation: As AI systems take on more consequential decision-making roles, there's a widening gap between technical capabilities and institutional accountability. Ethical guidance alone is insufficient, requiring architectures that embed governance into the AI ecosystem's execution fabric.

Method: The paper presents the Ten Criteria for Trustworthy Orchestration AI, a comprehensive assurance framework that integrates human input, semantic coherence, audit and provenance integrity into a unified Control-Panel architecture. It draws inspiration from international standards and Australia's National Framework for AI Assurance initiative.

Result: The framework provides governance across all AI components, their consumers, and human participants, unlike conventional agentic AI initiatives that focus primarily on AI-to-AI coordination.

Conclusion: Trustworthiness can be systematically engineered into AI systems, ensuring the execution fabric remains verifiable, transparent, reproducible, and under meaningful human control.
...
<details>
  <summary>Details</summary>
Motivation: Current collaborative perception systems face a fundamental communication-performance trade-off, with existing approaches assuming MB-level data transmission that fails under practical network constraints. There's a need for more communication-efficient solutions that can work in real-world autonomous driving scenarios.

Method: InfoCom introduces an information purification paradigm based on extended Information Bottleneck principles. It includes: 1) Information-Aware Encoding that condenses features into minimal messages while preserving task-critical information, 2) Sparse Mask Generation that identifies spatial cues with negligible communication cost, and 3) Multi-Scale Decoding that progressively recovers perceptual information through mask-guided mechanisms rather than simple feature reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: Traditional epidemic response planning relies on labor-intensive manual methods, creating a need for automated, scalable solutions to improve public health preparedness.

Method: Multi-agent framework with task decomposition, knowledge grounding, and simulation modules using large language models, tested by public health professionals with real-world outbreak scenarios.

Result: Significantly improved plan completeness and guideline alignment while drastically reducing development time; expert evaluation confirmed high consistency between AI-generated and human-authored content.
...
Result: Experiments on GSA-R2R benchmark show consistent improvement over strong baselines like GR-DUET, improving navigation success and path efficiency. Memory-bank warm start stabilizes early navigation and reduces performance drops after updates. Results confirm robustness and generality across continual and hybrid adaptation settings.

Conclusion: The framework demonstrates that integrating user feedback into continual learning for VLN significantly enhances adaptation quality and real-world deployment readiness, with sustained improvement across diverse deployment conditions.
...
TL;DR: ACE (Adaptive path Correction with Exponents) solves Marginal Path Collapse in ratio-of-densities steering for diffusion models, enabling reliable compositional generation with heterogeneous experts.


<details>
  <summary>Details</summary>
Motivation: Ratio-of-densities steering for diffusion models fails due to Marginal Path Collapse when composing heterogeneous models (different noise schedules/datasets), particularly problematic in molecular design tasks like flexible-pose scaffold decoration.

Method: 1) Derive path existence criterion predicting collapse from noise schedules and exponents alone; 2) Introduce ACE (Adaptive path Correction with Exponents) extending Feynman-Kac steering to time-varying exponents, guaranteeing valid probability paths.

Result: ACE eliminates collapse and enables high-guidance compositional generation, improving distributional and docking metrics over constant-exponent baselines and specialized task-specific scaffold decoration models on synthetic 2D benchmark and flexible-pose scaffold decoration.

Conclusion: ACE transforms ratio-of-densities steering with heterogeneous experts from an unstable heuristic into a reliable tool for controllable generation, solving the previously unformalized Marginal Path Collapse problem.
...
TL;DR: REMISVFU is a plug-and-play representation misdirection framework for fast client-level unlearning in split Vertical Federated Learning (VFL) systems, enabling effective forgetting while preserving utility for remaining parties.


<details>
  <summary>Details</summary>
Motivation: Data protection regulations like GDPR require the ability to remove a participant's contribution from federated models. While unlearning techniques exist for Horizontal Federated Learning (HFL), they are ineffective for Vertical Federated Learning (VFL) where data is partitioned by features rather than samples, creating a need for VFL-specific unlearning solutions.

Method: REMISVFU uses representation misdirection where the forgetting party collapses its encoder output to a randomly sampled anchor on the unit sphere, severing statistical links between its features and the global model. The server jointly optimizes retention and forgetting losses, aligning their gradients via orthogonal projection to eliminate destructive interference.

Result: Evaluations show REMISVFU suppresses back-door attack success to natural class-prior levels and sacrifices only about 2.5% points of clean accuracy, outperforming state-of-the-art baselines.

Conclusion: The proposed REMISVFU framework effectively enables client-level unlearning in splitVFL systems, addressing the unique challenges of feature-partitioned architectures while maintaining utility for remaining parties and complying with data protection regulations.
...
Method: EmerFlow uses LLM reasoning to enrich raw features of emerging items, aligns these representations with existing recommendation model embeddings, and incorporates new interactions through meta-learning to refine embeddings.
...
Conclusion: EmerFlow effectively addresses the challenge of recommending emerging items by generating distinctive embeddings that balance uniqueness with shared patterns, demonstrating superior performance across multiple domains.
...
TL;DR: AgentProg: A program-guided approach for mobile GUI agent context management that reframes interaction history as a program to reduce context overhead while maintaining task performance.
...
Result: Experiments on AndroidWorld and extended long-horizon task suite demonstrate state-of-the-art success rates. AgentProg maintains robust performance on long-horizon tasks while baseline methods experience catastrophic degradation.

Conclusion: AgentProg provides an effective program-guided approach for agent context management that reduces context overhead while preserving task performance, with open-source implementation available.
...
TL;DR: SaEI introduces selective-adversarial entropy intervention to enhance VLM reasoning by using entropy of sampled responses to create adversarial visual inputs, improving policy exploration without distorting factual knowledge.
...
Conclusion: SaEI effectively enhances VLM reasoning by addressing the overlooked aspect of entropy intervention during RL sampling, using adversarial techniques to improve exploration while preserving factual knowledge.
...
TL;DR: A new method to represent graphs as strings of simple instructions that build adjacency matrices step by step, making graphs amenable to deep learning language models.


<details>
  <summary>Details</summary>
Motivation: Current graph representations (based on adjacency matrices) are not suitable for processing by powerful deep learning language models that specialize in text processing. There's a need to bridge the gap between graph processing and modern language models.

Method: Proposes representing graphs by converting adjacency matrices into strings of simple instructions. The instructions build the adjacency matrix step by step in a reversible transformation - graphs can be converted to strings and strings back to graphs.

Result: The representation is compact and maintains local structural patterns of graphs. A tentative computational experiment showed favorable results, suggesting the method could boost graph processing by deep learning models.

Conclusion: The proposed string-based graph representation method shows promise for making graphs processable by deep learning language models, potentially enhancing graph processing capabilities through existing text-specialized models.
...
TL;DR: TAFAP is a new method for targeted data protection in diffusion models that controls the entire training trajectory to redirect fine-tuning toward user-specified concepts, outperforming snapshot-based approaches.


<details>
  <summary>Details</summary>
Motivation: Current diffusion model fine-tuning raises privacy concerns, and existing protection methods only degrade image quality without stable control. Targeted Data Protection (TDP) offers active redirection but existing approaches have poor controllability due to snapshot-matching that ignores complete learning dynamics.
...
Conclusion: TAFAP enables verifiable safeguards and provides a new framework for controlling and tracing alterations in diffusion model outputs, addressing privacy concerns in diffusion model fine-tuning through effective targeted data protection.
...
TL;DR: This paper investigates how adversarial PDF manipulation can fool AI-powered peer review systems into flipping "Reject" decisions to "Accept," introducing a new metric called WAVS to measure this vulnerability.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the rapid adoption of LLMs in scientific peer review, both by individual reviewers (the "Lazy Reviewer" hypothesis) and by institutions deploying AI assessment systems. There's a need to understand the robustness of these systems against adversarial attacks aimed at manipulating acceptance decisions.

Method: The researchers curated a dataset of 200 scientific papers and adapted 15 domain-specific attack strategies for adversarial PDF manipulation. They evaluated these attacks across 13 language models (including GPT-5, Claude Haiku, and DeepSeek) using their novel WAVS (Weighted Adversarial Vulnerability Score) metric to measure how effectively attacks could flip "Reject" to "Accept" decisions.

Result: Results show that obfuscation strategies like "Maximum Mark Magyk" successfully manipulate scores and achieve alarming decision flip rates, even in large-scale models. The attacks demonstrate significant vulnerabilities in AI-powered peer review systems.

Conclusion: The study reveals serious vulnerabilities in LLM-based peer review systems to adversarial PDF manipulation, highlighting the need for more robust defenses. The researchers will release their complete dataset and injection framework to facilitate further research on this critical security issue.
...
<details>
  <summary>Details</summary>
Motivation: PCG tools require precise configuration of opaque technical parameters, creating a semantic gap between abstract user instructions and strict parameter specifications that off-the-shelf LLMs fail to bridge effectively.

Method: Proposes a training-free architecture with paired Actor and Critic LLM agents that work iteratively: Actor reasons over tool parameters, Critic refines configurations to align with human design preferences through autonomous reasoning.
...
Conclusion: Off-the-shelf LLMs can be effectively repurposed as generalized agents for arbitrary PCG tools without task-specific fine-tuning, shifting burden from model training to architectural reasoning for scalable framework.
...
TL;DR: InternGeometry is a medalist-level LLM agent that solves IMO geometry problems using iterative proposition generation, symbolic verification, and feedback reflection, achieving gold medalist performance with minimal training data.


<details>
  <summary>Details</summary>
Motivation: Current AI geometry solving is dominated by expert models like AlphaGeometry 2 that require massive data synthesis and search. LLMs have strong mathematical reasoning but struggle with geometry due to weak heuristics for auxiliary constructions. The authors aim to build the first medalist-level LLM agent for geometry problem solving.

Method: InternGeometry uses an iterative approach: 1) Proposes propositions and auxiliary constructions, 2) Verifies them with a symbolic engine, 3) Reflects on engine feedback to guide subsequent proposals. It employs dynamic memory for 200+ interactions per problem and Complexity-Boosting Reinforcement Learning (CBRL) that gradually increases problem complexity during training.
...
Conclusion: Demonstrates LLM agents' potential for expert-level geometry tasks with minimal training data. The approach overcomes heuristic limitations through iterative verification and feedback. The model, data, and symbolic engine will be released to support future research.
...
TL;DR: NormCode is a semiformal language for constructing AI workflows that prevents context pollution by enforcing strict data isolation between steps, enabling precise cost/reliability tracing and auditable AI workflows.
...
Method: NormCode is a semiformal language with three isomorphic formats (.ncds for human authoring, .ncd for machine execution, .ncn for human verification) that enforces strict separation between semantic operations (LLM reasoning) and syntactic operations (deterministic data restructuring), ensuring each step operates in data isolation with only explicitly passed inputs.

Result: Two demonstrations validate NormCode: (1) base X addition algorithm achieving 100% accuracy on arbitrary length inputs, and (2) self-hosted execution of NormCode's own five-phase compiler pipeline. The orchestrator provides dependency-driven scheduling, SQLite checkpointing, and loop management.

Conclusion: NormCode addresses critical transparency needs in high-stakes domains by making AI workflows auditable by design through elimination of cross-step contamination, precise cost/reliability tracing, and progressive formalization from sketch to production.
...
Method: Phythesis uses iterative bi-level optimization: (1) LLM-driven level generates physically plausible 3D layouts and self-criticizes to refine scene topology, (2) physics-informed optimization level identifies optimal asset parameters and selects best asset combinations for simulation-ready scene synthesis.
...
Conclusion: Phythesis successfully bridges the gap between generative AI and physics-aware design by synergizing LLMs with physics-guided evolutionary optimization, enabling automated, simulation-ready data center design that meets strict physical constraints and energy efficiency objectives.
...
<details>
  <summary>Details</summary>
Motivation: Current clustering methods for single-cell omics focus only on intrinsic cellular structure and ignore cell-gene associations, limiting their ability to distinguish closely related cell types.

Method: Proposes scRCL with two contrastive distribution alignment components to reveal cellular structures, plus a refinement module that integrates gene-correlation structure learning to enhance cell embeddings by capturing cell-gene associations.

Result: Outperforms state-of-the-art baselines on single-cell RNA-seq and spatial transcriptomics datasets, with downstream analyses confirming coherent gene-expression signatures in recovered cell populations.

Conclusion: Incorporating cell-gene interactions through refinement contrastive learning improves unsupervised cell type identification and produces biologically meaningful representations.
...
TL;DR: CAPTAIN is a training-free framework that mitigates memorization in diffusion models by modifying latent features during denoising, using frequency-based noise initialization and injecting semantically aligned features from reference images into localized memorized regions.
...
Method: CAPTAIN uses frequency-based noise initialization to reduce tendency to replicate memorized patterns early in denoising. It identifies optimal denoising timesteps for feature injection, localizes memorized regions, and injects semantically aligned features from non-memorized reference images into localized latent regions.
...
TL;DR: Value diversity in LLM-based multi-agent systems enhances collective intelligence, stability, and emergent behaviors, but with diminishing returns at extreme heterogeneity.
...
Method: Used naturalistic value elicitation based on Schwartz's Theory of Basic Human Values, constructed multi-agent simulations with varying numbers of agents engaging in open-ended interactions and constitution formation.

Result: Value diversity enhances value stability, fosters emergent behaviors, and promotes creative principles developed autonomously by agents. However, effects show diminishing returns as extreme heterogeneity induces instability.
...
TL;DR: Hardware-aware NAS framework designs early-exit networks with optimized exit branch depth/layers and adaptive thresholds, achieving higher accuracy with same/lower computational cost than SOTA.


<details>
  <summary>Details</summary>
Motivation: Early-exit networks reduce energy/latency but designing them is challenging. Existing NAS approaches focus on exit positions/number but neglect exit branch architecture optimization. Need to consider both accuracy and efficiency in exit branch design.

Method: Propose hardware-aware NAS framework that optimizes exit branch depth and layer types along with adaptive threshold tuning. Considers both accuracy and efficiency during optimization process.

Result: Evaluation on CIFAR-10, CIFAR-100, and SVHN shows proposed framework designs early-exit networks achieving higher accuracy with same or lower average MACs compared to state-of-the-art approaches.

Conclusion: Optimizing exit branch architecture (depth and layer types) through hardware-aware NAS with adaptive thresholds significantly improves early-exit network efficiency and accuracy over existing methods.
...
TL;DR: Current LLM safety evaluations focus on universal risks, but miss context-dependent harms for individual users in high-stakes domains like finance and health. This study shows context-aware evaluation is essential, as context-blind evaluators rate identical responses as safer than those aware of user vulnerability.


<details>
  <summary>Details</summary>
Motivation: Existing safety evaluations for LLMs concentrate on universal risks (dangerous capabilities), but millions use LLMs for personal advice in high-stakes areas where harms depend on individual user context. User-welfare safety evaluations are underdeveloped, and developing them is non-trivial due to fundamental questions about incorporating user context in evaluation design.

Method: The study evaluated advice from GPT-5, Claude Sonnet 4, and Gemini 2.5 Pro on finance and health topics across user profiles with varying vulnerability levels. Two studies: 1) Compared safety ratings between context-blind evaluators and context-aware evaluators, 2) Tested whether realistic user prompts containing self-disclosed context improve evaluation accuracy.

Result: Context-blind evaluators rated identical LLM responses significantly safer than context-aware evaluators. Safety scores for high-vulnerability users dropped from safe (5/7) to somewhat unsafe (3/7). Realistic user prompts containing self-disclosed context did not significantly improve evaluation accuracy. The gap between context-blind and context-aware evaluations persists even with realistic user disclosures.

Conclusion: Effective user-welfare safety evaluation requires evaluators to assess responses against diverse user profiles, as realistic user context disclosure alone is insufficient. The study provides a methodology for context-aware evaluation and demonstrates that evaluating individual welfare demands approaches distinct from existing universal-risk frameworks.
...
TL;DR: RL optimization with clinically grounded rewards boosts CXR VLM performance beyond SFT alone, but explicit thinking doesn't provide additional gains.


<details>
  <summary>Details</summary>
Motivation: Current medical VLMs rely heavily on supervised fine-tuning (SFT) which optimizes next-token prediction without evaluating answer quality. Reinforcement learning (RL) with task-specific feedback could improve performance, but its effects in CXR interpretation need investigation.

Method: Built RadVLM based on Qwen3-VL with large-scale SFT on CXR data, then cold-start SFT for basic thinking ability. Applied Group Relative Policy Optimization (GRPO) with clinically grounded rewards for report generation and visual grounding. Conducted matched RL experiments on domain-specific and general-domain variants with/without thinking.

Result: RL provides additional gains on both report generation and grounding tasks beyond strong SFT. Explicit thinking does not further improve results. RL-optimized RadVLM models outperform baselines and achieve state-of-the-art performance on both tasks.

Conclusion: Clinically aligned RL is a powerful complement to SFT for medical VLMs, enabling performance improvements on CXR interpretation tasks, though explicit intermediate reasoning doesn't provide additional benefits in this domain.
...
TL;DR: ReMe is a comprehensive framework for LLM agent memory evolution that moves beyond static archives through multi-faceted distillation, context-adaptive reuse, and utility-based refinement, achieving state-of-the-art performance with significant memory-scaling effects.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agent memory frameworks suffer from "passive accumulation" paradigms where memory is treated as a static append-only archive, creating a gap between static storage and dynamic reasoning needed for effective experience-driven evolution.

Method: ReMe introduces three key mechanisms: 1) multi-faceted distillation that extracts fine-grained experiences by recognizing success patterns, analyzing failure triggers, and generating comparative insights; 2) context-adaptive reuse that tailors historical insights to new contexts via scenario-aware indexing; and 3) utility-based refinement that autonomously adds valid memories and prunes outdated ones to maintain a compact, high-quality experience pool.

Result: Extensive experiments on BFCL-V3 and AppWorld demonstrate ReMe establishes new state-of-the-art in agent memory systems. Crucially, Qwen3-8B equipped with ReMe outperforms larger, memoryless Qwen3-14B, showing significant memory-scaling effects and suggesting self-evolving memory provides computation-efficient lifelong learning.

Conclusion: ReMe bridges the gap between static storage and dynamic reasoning in LLM agent memory systems, enabling experience-driven evolution through its three innovative mechanisms. The framework demonstrates that self-evolving memory can provide computation-efficient pathways for lifelong learning, with code and the reme.library dataset released to facilitate further research.
...
TL;DR: CA-GPT, a domain-specific AI model for OCT-guided PCI, outperformed general-purpose ChatGPT-5 and junior physicians in procedural decision-making agreement with expert-derived records.


<details>
  <summary>Details</summary>
Motivation: Intravascular imaging like OCT improves PCI outcomes but interpretation is operator-dependent. General-purpose AI lacks domain-specific reliability, creating need for specialized AI systems for OCT-guided PCI planning and assessment.

Method: Single-center analysis of 96 OCT-guided PCI patients comparing procedural decisions from CA-GPT, ChatGPT-5, and junior physicians against expert-derived procedural records. Agreement assessed using ten pre-specified metrics across pre-PCI and post-PCI phases.

Result: CA-GPT showed significantly higher median agreement scores than both ChatGPT-5 and junior physicians for pre-PCI planning (5 vs 3 vs 4). Superior performance in stent diameter (90.3% vs 72.2%) and length selection (80.6% vs 52.8%). Maintained excellent post-PCI agreement (5 vs 4 vs 5). Robust advantage in complex scenarios.

Conclusion: CA-GPT-based AI-OCT system provides superior decision-making agreement compared to general-purpose AI and junior physicians, offering standardized, reliable intravascular imaging interpretation that can augment operator expertise and optimize OCT-guided PCI.
...
TL;DR: SEAL-RAG introduces a "replace, don't expand" strategy for multi-hop RAG systems that actively swaps out irrelevant context for missing bridge facts instead of expanding context windows, preventing context dilution while maintaining fixed retrieval depth.
...
Conclusion: SEAL-RAG's "replace, don't expand" strategy effectively fights context dilution in multi-hop RAG systems, providing predictable cost profiles while optimizing top-k slots for precision rather than mere breadth, significantly outperforming existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current HAR research lacks comprehensive evaluation of OOD algorithms across different distribution shift scenarios (cross-person, device, environment, time), making it unclear which methods work best and whether OOD approaches are necessary for HAR.

Method: Created HAROOD benchmark with 4 OOD scenarios (cross-person, cross-position, cross-dataset, cross-time), 6 datasets, 16 comparative methods implemented with CNN-based and Transformer architectures, and two model selection protocols.

Result: Extensive experiments revealed that no single OOD method consistently outperforms others across all scenarios, indicating substantial room for improvement in OOD-based HAR research.

Conclusion: The benchmark provides modular, extensible codebase to facilitate future research in OOD-based HAR, highlighting the need for more robust methods that can handle diverse distribution shifts in realistic HAR scenarios.
...
TL;DR: Agile Deliberation: A human-in-the-loop framework that helps users iteratively refine vague visual concepts through borderline case exposure and structured deliberation, outperforming automated and manual baselines.


<details>
  <summary>Details</summary>
Motivation: Existing human-in-the-loop approaches assume users have clear, stable concept understanding from the start, but in reality (especially in content moderation), users often begin with vague ideas that need iterative refinement through "concept deliberation."

Method: Operationalizes real content moderators' deliberation strategies into a two-stage framework: (1) concept scoping - decomposes initial concept into structured hierarchy of sub-concepts, and (2) concept iteration - surfaces semantically borderline examples for user reflection and feedback to iteratively align image classifier with evolving user intent.

Result: Achieves 7.5% higher F1 scores than automated decomposition baselines and more than 3% higher than manual deliberation, with participants reporting clearer conceptual understanding and lower cognitive effort. Evaluation involved 18 user sessions (1.5h each) rather than standard benchmarks.

Conclusion: Agile Deliberation effectively supports evolving and subjective visual concepts through structured deliberation, addressing the real-world need for iterative concept refinement in applications like content moderation where users start with vague ideas.
...
TL;DR: V-OCBF learns neural Control Barrier Functions from offline data without dynamics models, enabling safe control synthesis with fewer violations than baselines.


<details>
  <summary>Details</summary>
Motivation: Existing Safe Offline RL methods only enforce soft constraints without forward invariance guarantees, while CBFs require expert-designed barriers or full dynamics knowledge. Need offline methods that guarantee safety without online interaction or model access.

Method: Learns neural CBF from offline demonstrations using recursive finite-difference barrier updates (model-free). Uses expectile-based objective to avoid OOD actions and restricts updates to dataset-supported actions. Synthesizes safe control via Quadratic Program.

Result: Substantially fewer safety violations than baseline methods across multiple case studies while maintaining strong task performance. Demonstrates scalability for offline safety-critical controller synthesis.

Conclusion: V-OCBF enables rigorous safety guarantees from offline data without dynamics models or hand-engineered barriers, bridging the gap between offline RL and formal control methods.
