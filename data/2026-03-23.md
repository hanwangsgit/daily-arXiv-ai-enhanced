...
TL;DR: SemanticFL uses pre-trained diffusion models to create semantic guidance for federated learning, addressing non-IID data problems in multimodal perception tasks.


<details>
  <summary>Details</summary>
Motivation: Federated learning struggles with non-IID client data, especially in multimodal perception settings, where semantic discrepancies degrade global model performance.

Method: Leverages multi-layer semantic representations from Stable Diffusion (VAE latents & U-Net features) to create shared latent space, uses client-server architecture with cross-modal contrastive learning for consistency.

Result: Achieves up to 5.49% accuracy gain over FedAvg on CIFAR-10, CIFAR-100, and TinyImageNet benchmarks under diverse heterogeneity scenarios.

Conclusion: SemanticFL effectively addresses non-IID challenges in federated learning by using diffusion model semantics for robust representation learning in heterogeneous multimodal data.
...
TL;DR: A unified multi-task transformer framework for ultrasound image analysis that handles segmentation, detection, classification, and landmark regression across diverse organs and datasets, improving performance from 67% to 85% on validation.


<details>
  <summary>Details</summary>
Motivation: Ultrasound images vary widely across scanners, operators, and anatomical targets, causing poor generalization of models trained in one setting to new hospitals and clinical conditions. The Foundation Model Challenge for Ultrasound Image Analysis requires a single model to handle multiple tasks across diverse datasets.

Method: Uses a transformer visual encoder from Qwen3-VL family, projects intermediate token features into spatial feature maps, fuses them with a lightweight multi-scale feature pyramid. Each task has a small task-specific prediction head. Training uses task-aware sampling and selective loss balancing to manage heterogeneous supervision and reduce task imbalance.

Result: Performance improved from 67% to 85% on the validation set and achieved an average score of 81.84% on the official test set across all tasks.

Conclusion: The proposed unified multi-task framework is simple to optimize and adaptable across a wide range of ultrasound analysis tasks, effectively addressing the generalization challenges in ultrasound imaging across different clinical settings.
...
TL;DR: A memory-efficient Levenberg-Marquardt optimizer for diffeomorphic image registration reduces memory usage by up to 24.6% while maintaining performance across multiple medical imaging datasets.


<details>
  <summary>Details</summary>
Motivation: FireANTs uses Adam optimizer which requires storing momentum and squared-momentum estimates, consuming significant memory and limiting its use for large medical images.

Method: Proposed a modified Levenberg-Marquardt optimizer with only a single scalar damping parameter as optimizer state, adaptively tuned using trust region approach, with Metropolis-Hastings style rejection step.

Result: Reduces memory by up to 24.6% for large volumes while retaining performance across all four datasets; single hyperparameter configuration transfers across brain MRI, lung CT, and abdominal registration, matching or outperforming Adam on three of four benchmarks.

Conclusion: The modified LM optimizer provides a memory-efficient alternative to Adam for diffeomorphic image registration, enabling registration of larger medical images while maintaining or improving performance.
...
<details>
  <summary>Details</summary>
Motivation: Current contrastive models lack region-level supervision, and large vision language models struggle with fine-grained representations in external validation, leading to suboptimal performance on chest X-ray retrieval and phrase grounding tasks where clinically relevant findings are often spatially confined.

Method: Proposes LoFi which jointly optimizes sigmoid, captioning, and location-aware captioning losses using a lightweight large language model. The location-aware captioning loss provides region-level supervision through grounding and dense captioning objectives. Also integrates a fine-grained encoder into retrieval-based in-context learning for enhanced chest X-ray grounding.
...
Conclusion: LoFi effectively addresses the limitations of existing methods by enabling fine-grained representation learning through location-aware supervision, leading to improved performance on clinically important chest X-ray analysis tasks.
...
TL;DR: A new framework formulates vehicle camouflage attacks as conditional image-editing using ControlNet to synthesize camouflaged vehicles on real images, achieving strong attack effectiveness while preserving vehicle structure and improving stealthiness.
...
Method: Formulates vehicle camouflage attacks as conditional image-editing problem. Uses both image-level and scene-level camouflage generation strategies, fine-tunes ControlNet to synthesize camouflaged vehicles directly on real images. Designs unified objective enforcing vehicle structural fidelity, style consistency, and adversarial effectiveness.

Result: Achieves significantly stronger attack effectiveness with >38% AP50 decrease on COCO and LINZ datasets. Better preserves vehicle structure and improves human-perceived stealthiness compared to existing approaches. Generalizes effectively to unseen black-box detectors and shows promising transferability to physical world.

Conclusion: The proposed framework successfully addresses vehicle camouflage attacks as conditional image-editing, achieving superior attack performance while maintaining better stealthiness and structure preservation, with good generalization to black-box detectors and physical world transferability.
...
TL;DR: The paper introduces ProactiveBench, a benchmark for evaluating proactive help-seeking behavior in multimodal large language models (MLLMs), finding current models lack this capability and exploring ways to improve it.


<details>
  <summary>Details</summary>
Motivation: To investigate whether MLLMs can exhibit proactive behavior similar to humans by requesting simple user interventions when needed, such as asking to remove obstructions when identifying occluded objects.

Method: Created ProactiveBench from seven repurposed datasets testing proactiveness across tasks like recognizing occluded objects, enhancing image quality, and interpreting coarse sketches. Evaluated 22 MLLMs, analyzed correlations with model capacity, tested "hinting" strategies, and explored reinforcement learning fine-tuning.

Result: MLLMs generally lack proactiveness; proactiveness doesn't correlate with model capacity; "hinting" yields only marginal gains; conversation histories and in-context learning introduce negative biases; reinforcement learning fine-tuning shows proactiveness can be learned and generalizes to unseen scenarios.

Conclusion: ProactiveBench serves as a first step toward building proactive multimodal models, demonstrating that while current MLLMs lack this capability, it can be learned through appropriate training methods like reinforcement learning.
...
### [7] [Evaluating Test-Time Adaptation For Facial Expression Recognition Under Natural Cross-Dataset Distribution Shifts](https://arxiv.org/abs/2603.19994)
*John Turnbull,Shivam Grover,Amin Jalali,Ali Etemad*

Main category: cs.CV

TL;DR: TTA methods improve FER performance under natural domain shifts by up to 11.34%, with effectiveness depending on distributional distance and shift severity.


<details>
  <summary>Details</summary>
Motivation: Deep learning models struggle with natural distribution shifts in real-world deployments, and Test-Time Adaptation (TTA) offers a solution by adapting models during inference without labeled source data.
...
Result: TTA can boost FER performance under natural shifts by up to 11.34%. Entropy minimization methods (TENT, SAR) perform best with clean target distributions, prototype adjustment methods (T3A) excel under larger distributional distances, and feature alignment methods (SHOT) deliver largest gains with noisier target distributions.

Conclusion: TTA effectiveness for FER is governed by distributional distance and severity of natural shifts across domains, with different TTA methods performing optimally under different shift conditions.
...
TL;DR: NA-VQA is a new benchmark for evaluating deep narrative reasoning in long videos, featuring 88 movies and 4.4K questions requiring integration of evidence across multiple scenes. The authors also propose Video-NaRA, a narrative-centric framework that improves long-range reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Most existing video reasoning benchmarks rely on localized cues and fail to capture narrative reasoning - the ability to track intentions, connect distant events, and reconstruct causal chains across entire movies. There's a need for benchmarks that test deep temporal and narrative understanding rather than shallow pattern matching.

Method: 1) Created NA-VQA benchmark with 88 full-length movies and 4.4K open-ended QA pairs, each grounded in evidence spans labeled as Short, Medium, or Far to assess long-range dependencies. 2) Proposed Video-NaRA framework that builds event-level chains and stores them in structured memory for retrieval during reasoning.

Result: State-of-the-art MLLMs perform poorly on questions requiring far-range evidence, highlighting the need for explicit narrative modeling. Video-NaRA improves long-range reasoning performance by up to 3%, demonstrating effectiveness in handling complex narrative structures.

Conclusion: NA-VQA addresses the gap in evaluating deep narrative reasoning in long videos, showing current MLLMs struggle with far-range evidence integration. Video-NaRA's narrative-centric approach shows promise for improving long-range reasoning, and the benchmark will be publicly released to advance research in this area.
...
### [9] [A Unified Platform and Quality Assurance Framework for 3D Ultrasound Reconstruction with Robotic, Optical, and Electromagnetic Tracking](https://arxiv.org/abs/2603.20077)
*Lewis Howell,Manisha Waterston,Tze Min Wah,James H. Chandler,James R. McLaughlan*

Main category: cs.CV

TL;DR: A QA framework and open-source platform for evaluating 3D ultrasound reconstruction accuracy using geometric phantoms, with robotic 3D US achieving state-of-the-art performance approaching transducer resolution limits.
...
Method: Developed a QA framework with custom phantom containing geometric inclusions, standardized pipeline for real-time segmentation and 3D reconstruction, automated registration with ground-truth geometries, and evaluation of optical, electromagnetic, and robotic tracking at different scanning speeds and angles.

Result: Robotic 3D US achieved state-of-the-art reconstruction performance (DSC-3D = 0.94 ± 0.01, HD95 = 1.17 ± 0.12), approaching spatial resolution limits imposed by the transducer, with real-time segmentation achieving DSC = 0.97 at 46 FPS without GPU acceleration.

Conclusion: Established a flexible experimental platform and reproducible validation methodology for 3D US reconstruction, enabling robust cross-platform comparisons and improved reporting practices to support safe clinical translation of 3D ultrasound.
...
<details>
  <summary>Details</summary>
Motivation: Medical LVLMs face challenges in constructing large-scale, high-quality instruction datasets due to specialized expert knowledge requirements. Existing methods rely on curated image-instruction-output triplets which are difficult to obtain in medical domains.

Method: Proposes instruction-free tuning using momentum proxy instructions as replacements for curated text instructions, preserving instruction-following capability while enabling parameter updates valid during inference. Incorporates response shuffling to reduce over-reliance on previous words.

Result: Achieves state-of-the-art accuracy on multiple-choice visual question answering tasks across SKINCON, WBCAtt, CBIS, and MIMIC-CXR datasets, significantly enhancing fine-tuning efficiency.

Conclusion: Instruction-free tuning effectively addresses the challenge of limited medical instruction datasets, enabling flexible domain-specific responses without explicit instructions during fine-tuning while maintaining LVLM capabilities.
...
TL;DR: VeloxNet: lightweight CNN using gMLP blocks with spatial gating units to replace SqueezeNet's fire modules for embedded image classification, achieving better accuracy with 46% fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Need for deep learning models that balance accuracy with strict constraints on model size, memory, and latency for deployment on embedded devices in aerial disaster monitoring and infrastructure inspection applications.

Method: Introduces VeloxNet, which replaces SqueezeNet's fire modules with gated multi-layer perceptron (gMLP) blocks containing spatial gating units (SGU). SGUs apply learned spatial projections and multiplicative gating to capture global spatial dependencies across full feature maps in single layers, unlike local convolutional kernels.

Result: 46.1% parameter reduction from SqueezeNet (740,970 to 399,366) with improved weighted F1 scores: +6.32% on AIDER, +30.83% on CDD, +2.51% on LDD. Outperforms 11 baselines including MobileNet variants, ShuffleNet, EfficientNet, and vision transformers.
...
TL;DR: ViTRM replaces deep ViT encoders with a tiny 3-layer block applied recursively, achieving competitive vision performance with 6-84x fewer parameters than CNN/ViT models.


<details>
  <summary>Details</summary>
Motivation: Current vision models (CNNs and ViTs) are parameter-intensive and computationally demanding, limiting deployment in resource-constrained environments. There's a need for parameter-efficient alternatives that maintain performance.

Method: Introduces Vision Tiny Recursion Model (ViTRM) that replaces the L-layer ViT encoder with a single tiny k-layer block (k=3) applied recursively N times, inspired by Tiny Recursive Models (TRM) that use iterative state refinement.

Result: ViTRM maintains competitive performance on CIFAR-10 and CIFAR-100 while using up to 6× fewer parameters than CNN-based models and 84× fewer parameters than ViT models.

Conclusion: Recursive computation is a viable, parameter-efficient alternative to architectural depth in vision models, enabling effective performance with dramatically reduced parameter counts.
...
TL;DR: FedAgain is a trust-based federated learning strategy that enhances robustness for kidney stone identification from endoscopic images by using dual trust mechanisms to dynamically weight client contributions and handle heterogeneous medical data.


<details>
  <summary>Details</summary>
Motivation: AI reliability in medical imaging depends on robustness to heterogeneous and corrupted images from diverse devices across different hospitals, which is highly challenging. Current federated learning approaches need better mechanisms to handle noisy or adversarial updates and non-IID data distributions.

Method: FedAgain integrates a dual trust mechanism combining benchmark reliability and model divergence to dynamically weight client contributions during aggregation. This mitigates the impact of noisy or adversarial updates while preserving data privacy and enabling collaborative training across multiple institutions.
...
TL;DR: Gastric-X is a large-scale multimodal benchmark for gastric cancer analysis with 1.7K cases containing CT scans, endoscopic images, biochemical indicators, diagnostic notes, and tumor annotations to evaluate VLMs on clinical tasks.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models (VLMs) have strong generalization but limited application to medical diagnosis due to lack of comprehensive, structured datasets that capture real clinical workflows, particularly for gastric cancer.

Method: Created Gastric-X benchmark with 1.7K cases including paired resting/dynamic CT scans, endoscopic images, structured biochemical indicators, expert diagnostic notes, and tumor bounding box annotations. Evaluated VLMs on five core tasks: VQA, report generation, cross-modal retrieval, disease classification, and lesion localization.

Result: The benchmark enables systematic examination of VLM capabilities on clinical tasks, assessing whether current VLMs can meaningfully correlate biochemical signals with spatial tumor features and textual reports.

Conclusion: Gastric-X represents a step toward aligning machine intelligence with physician cognitive reasoning and serves as a resource to inspire development of next-generation medical VLMs for clinical applications.
...
TL;DR: ReXInTheWild is a new benchmark of 955 clinician-verified medical questions about everyday photographs, evaluating vision-language models' ability to combine natural image understanding with medical reasoning.


<details>
  <summary>Details</summary>
Motivation: Everyday medical photographs are widely used in telemedicine, but there's no comprehensive benchmark to evaluate whether vision-language models can properly interpret their medical content, which requires both fine-grained natural image understanding and domain-specific medical reasoning.

Method: Created ReXInTheWild benchmark with 955 clinician-verified multiple-choice questions spanning 7 clinical topics across 484 photographs sourced from biomedical literature. Evaluated leading multimodal LLMs including Gemini-3, Claude Opus 4.5, GPT-5, and medical specialist MedGemma.

Result: Substantial performance variation: Gemini-3 achieved 78% accuracy, Claude Opus 4.5 (72%), GPT-5 (68%), while medical specialist MedGemma only reached 37%. Error analysis revealed four categories of common errors from low-level geometric errors to high-level reasoning failures.

Conclusion: ReXInTheWild provides a challenging, clinically grounded benchmark at the intersection of natural image understanding and medical reasoning, highlighting current limitations of vision-language models in medical image interpretation and suggesting need for different mitigation strategies for different error types.
...
TL;DR: New large-scale BSL fingerspelling dataset FS23K with iterative annotation framework and improved recognition model using bi-manual interactions and mouthing cues, achieving 50% reduction in character error rate.


<details>
  <summary>Details</summary>
Motivation: Fingerspelling is crucial in BSL for proper names and technical terms, but recognition is challenging due to rapid signing and letter omissions. Existing BSL fingerspelling datasets are either too small or inaccurate.

Method: Introduced FS23K dataset using iterative annotation framework, and proposed fingerspelling recognition model that explicitly accounts for bi-manual interactions and mouthing cues.

Result: Halved the character error rate (CER) compared to prior state-of-the-art on fingerspelling recognition through refined annotations and improved modeling.

Conclusion: The approach demonstrates effectiveness and potential to support future research in sign language understanding and scalable automated annotation pipelines.
...
TL;DR: SurfaceXR combines headset hand tracking with smartwatch IMU data to enable precise surface-based interactions in XR, overcoming limitations of mid-air gestures and single-modality approaches.


<details>
  <summary>Details</summary>
Motivation: Mid-air gestures in XR cause fatigue and imprecision, while surface-based interactions offer better accuracy and comfort. However, current egocentric vision methods face challenges with hand tracking and unreliable surface plane estimation.
...
Conclusion: The complementary nature of hand tracking and IMU data enables robust surface-based XR interactions, addressing limitations of current methods and improving user experience.
...
TL;DR: DINOv3.seg extends DINOv3.txt into a dedicated framework for Open-Vocabulary Semantic Segmentation with four key contributions: task-specific architecture, joint text embeddings, early and late refinement of features, and high-resolution inference strategy.
...
Method: 1) Task-specific architecture adapting prior design principles; 2) Joint text embeddings using both global CLS token and local patch-level features; 3) Early refinement of visual representations before image-text interaction plus late refinement of correlation features; 4) High-resolution sliding-window inference preserving spatial detail and global context.

Result: Extensive experiments on five OVSS benchmarks demonstrate the approach consistently outperforms current state-of-the-art methods, showing effectiveness and robustness.

Conclusion: DINOv3.seg provides a comprehensive framework for OVSS that addresses limitations of current approaches through systematic architectural design, refined feature processing, and improved inference strategies, achieving superior performance on standard benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Accurate pedestrian intention prediction is crucial for autonomous vehicle safety in urban environments, but existing models often lack efficiency, interpretability, and proper uncertainty quantification needed for real-world deployment.

Method: Proposes a socially informed architecture that fuses attention, position, situation, and interaction behavioral streams using highway encoders, a compact 4-token Transformer, and global self-attention pooling. Includes two uncertainty heads: variational bottleneck for epistemic uncertainty and Mahalanobis distance detector for distributional shift.

Result: Outperforms recent vision language models on PSI 1.0 (0.9 F1, 0.94 AUC-ROC, 0.78 MCC) using only structured features. Establishes first baseline on PSI 2.0 (0.78 F1, 0.79 AUC-ROC). Selective prediction improves accuracy by 0.4 percentage points at 80% coverage.

Conclusion: The approach provides accurate, calibrated pedestrian intention prediction with uncertainty quantification, is modality-agnostic, easy to integrate with vision pipelines, and suitable for resource-constrained platforms, enabling safer autonomous navigation.
...
TL;DR: MoCA3D is a monocular, class-agnostic 3D model that predicts projected 3D bounding box corners and per-corner depths without requiring camera intrinsics at inference, using dense prediction via corner heatmaps and depth maps.


<details>
  <summary>Details</summary>
Motivation: Existing monocular 3D object understanding methods require known camera intrinsics to obtain image-plane geometry like projected 3D box corners, which limits applications in the wild where intrinsics are unknown.

Method: Formulates pixel-space localization and depth assignment as dense prediction via corner heatmaps and depth maps, enabling inference without camera intrinsics. Proposes Pixel-Aligned Geometry (PAG) metric for evaluation.
...
Conclusion: MoCA3D enables practical downstream applications previously impractical under unknown intrinsics, demonstrating utility beyond standard baseline models for monocular 3D object understanding in the wild.
...
<details>
  <summary>Details</summary>
Motivation: Monocular depth estimation struggles with transparent objects due to refraction and transmission effects that break appearance assumptions used by conventional depth networks, leading to unstable or incorrect predictions.

Method: First localize transparent regions, then transform refractive appearance into geometrically consistent opaque shapes using a diffusion-based generative opacification module. Processed images are fed into off-the-shelf monocular depth estimators without retraining. Trained on SeeClear-396k synthetic dataset with 396k paired transparent-opaque renderings.
...
Conclusion: SeeClear provides an effective framework for stable monocular depth estimation of transparent objects by converting them into opaque representations, addressing a fundamental challenge in computer vision without requiring modifications to existing depth estimation architectures.
...
TL;DR: StreetForward is a feedforward framework for dynamic street reconstruction that uses temporal mask attention and 3D Gaussian Splatting to capture motion and render novel views without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Rapid scene reconstruction is crucial for autonomous driving applications to efficiently utilize large-scale driving datasets in closed-loop simulation and downstream tasks, eliminating the need for time-consuming per-scene optimization.

Method: A pose-free and tracker-free feedforward framework with temporal mask attention module that captures dynamic motion from image sequences, representing static and dynamic content uniformly with 3D Gaussian Splatting, optimized jointly by cross-frame rendering with spatio-temporal consistency.

Result: Superior performance on novel view synthesis and depth estimation compared to existing methods on Waymo Open Dataset, with zero-shot generalization validated on CARLA and other datasets.

Conclusion: StreetForward enables efficient dynamic street reconstruction for autonomous driving applications with strong generalization capabilities, making it suitable for large-scale dataset utilization in simulation and downstream tasks.
...
TL;DR: EvoNAS is an efficient distributed evolutionary architecture search framework that uses a hybrid VSS-ViT supernet with cross-architecture knowledge distillation to balance accuracy and efficiency for edge deployment.


<details>
  <summary>Details</summary>
Motivation: Large vision models have high inference costs that limit deployment on resource-constrained edge devices. Evolutionary NAS is suitable for multi-objective optimization but suffers from expensive candidate evaluation and ranking inconsistency among subnetworks.

Method: Proposes EvoNAS with: 1) Hybrid VSS-ViT supernet combining Vision State Space and Vision Transformer modules, 2) Cross-Architecture Dual-Domain Knowledge Distillation (CA-DDKD) for improved representational capacity and ranking consistency, 3) Distributed Multi-Model Parallel Evaluation (DMMPE) framework using GPU resource pooling and asynchronous scheduling for efficient evaluation.

Result: DMMPE improves evaluation efficiency by over 70% compared to conventional data-parallel evaluation. Searched EvoNet architectures achieve Pareto-optimal trade-offs between accuracy and efficiency on COCO, ADE20K, KITTI, and NYU-Depth v2 datasets, delivering lower inference latency and higher throughput under computational constraints while maintaining strong generalization on downstream tasks.

Conclusion: EvoNAS effectively addresses the challenges of evolutionary NAS through hybrid architecture design, knowledge distillation for ranking consistency, and distributed evaluation, enabling efficient search of vision models that balance accuracy and efficiency for edge deployment.
...
TL;DR: Proposes Event Prompter for RGB-Event pedestrian attribute recognition using lightweight DCT/IDCT operations on event data and memory-augmented learning with Hopfield networks for efficient multimodal fusion.


<details>
  <summary>Details</summary>
Motivation: Existing two-stream multimodal fusion methods for event-based PAR have high computational overhead and fail to leverage contextual guidance from other samples, limiting efficiency and performance.

Method: 1) Event Prompter replaces heavy auxiliary backbone with lightweight DCT/IDCT operations on event data for frequency-domain feature extraction; 2) External memory bank with Hopfield networks enables associative memory-augmented learning; 3) Cross-attention fuses RGB and event modalities followed by feed-forward networks for attribute prediction.

Result: Extensive experiments on multiple benchmark datasets validate the effectiveness of the proposed RGB-Event PAR framework, showing improved performance with minimal computational cost.

Conclusion: The proposed method provides an efficient and effective solution for event-based pedestrian attribute recognition by addressing computational overhead and leveraging global relational knowledge through memory-augmented learning.
...
### [25] [PhyUnfold-Net: Advancing Remote Sensing Change Detection with Physics-Guided Deep Unfolding](https://arxiv.org/abs/2603.19566)
*Zelin Lei,Yaoxing Ren,Jiaming Chang*

Main category: cs.CV

TL;DR: PhyUnfold-Net: A physics-guided deep unfolding framework for bi-temporal change detection that uses patch-wise singular-value entropy to distinguish genuine changes from pseudo-changes caused by acquisition discrepancies.


<details>
  <summary>Details</summary>
Motivation: Bi-temporal change detection is highly sensitive to acquisition discrepancies like illumination, season, and atmospheric variations, which cause false alarms. The authors observed that genuine changes have higher patch-wise singular-value entropy than pseudo-changes in feature-difference space, providing a physical prior for better change detection.

Method: Proposes PhyUnfold-Net with two main components: 1) Iterative Change Decomposition Module (ICDM) that unrolls a multi-step solver to separate mixed discrepancy features into change and nuisance components, and 2) Wavelet Spectral Suppression Module (WSSM) to suppress acquisition-induced spectral mismatch before decomposition. Uses staged Exploration-and-Constraint loss (S-SEC) to stabilize training by encouraging component separation early and constraining nuisance magnitude later.

Result: Experiments on four benchmarks show improvements over state-of-the-art methods, with particularly strong gains under challenging conditions with acquisition discrepancies.

Conclusion: The physics-guided deep unfolding framework effectively addresses the challenge of acquisition discrepancies in bi-temporal change detection by leveraging physical priors about singular-value entropy differences between genuine and pseudo changes, leading to more robust performance.
...
### [26] [Efficiency Follows Global-Local Decoupling](https://arxiv.org/abs/2603.19567)
*Zhenyu Yang,Gensheng Pei,Tao Chen,Yichao Zhou,Tianfei Zhou,Yazhou Yao,Fumin Shen*

Main category: cs.CV

TL;DR: ConvNeur is a two-branch vision architecture that decouples global reasoning and local representation using a neural memory branch for global context and a locality-preserving branch for fine structure, achieving efficient subquadratic scaling.


<details>
  <summary>Details</summary>
Motivation: Modern vision models need to capture both global image context and local details while remaining computationally efficient. There's a fundamental trade-off between global reasoning and local representation that needs to be addressed.

Method: Introduces ConvNeur, a two-branch architecture: 1) lightweight neural memory branch aggregates global context on compact token set, 2) locality-preserving branch extracts fine structure. Uses learned gate to modulate local features with global cues without entangling objectives.

Result: Achieves subquadratic scaling with image size, retains local processing inductive priors, reduces overhead vs fully global attention. Matches/surpasses comparable alternatives on classification, detection, segmentation benchmarks at similar/lower compute. Offers favorable accuracy-latency trade-offs.

Conclusion: Efficiency follows from global-local decoupling principle. The separation of global reasoning and local representation roles enables computationally affordable vision models without sacrificing performance.
...
### [27] [Accelerating Diffusion Decoders via Multi-Scale Sampling and One-Step Distillation](https://arxiv.org/abs/2603.19570)
*Chuhan Wang,Hao Chen*

Main category: cs.CV

TL;DR: Two-stage acceleration framework for diffusion-based image tokenizers: multi-scale progressive refinement plus single-step distillation, achieving order-of-magnitude speedup with minimal quality loss.
...
Method: 1) Multi-scale sampling strategy: decode from coarse to fine resolution, doubling resolution at each stage for O(log n) theoretical speedup. 2) Distill diffusion decoder at each scale into single-step denoising model for fast single forward pass per scale.

Result: Order-of-magnitude reduction in decoding time with little degradation in output quality, enabling efficient yet expressive image tokenizers.

Conclusion: The approach provides practical pathway toward efficient image tokenization and serves as foundation for future work in efficient visual tokenization and downstream generation.
...
TL;DR: LoD-Loc v3 improves aerial visual localization in dense cities by using synthetic instance segmentation data and instance silhouette alignment instead of semantic alignment, achieving better generalization and performance in dense scenes.


<details>
  <summary>Details</summary>
Motivation: Previous LoD-Loc v2 method has poor cross-scene generalization and fails frequently in dense building scenes due to semantic building silhouette alignment with low-detail city models.

Method: Two key innovations: 1) New synthetic data generation pipeline creating InsLoD-Loc - largest instance segmentation dataset for aerial imagery (100k images with precise building annotations) for zero-shot generalization. 2) Reformulating localization paradigm from semantic to instance silhouette alignment to reduce pose estimation ambiguity in dense scenes.

Result: Extensive experiments show LoD-Loc v3 outperforms existing SOTA baselines with large margin, achieving superior performance in both cross-scene and dense urban scenarios.

Conclusion: LoD-Loc v3 successfully addresses limitations of previous methods through synthetic instance segmentation data and instance-level alignment, enabling robust aerial visual localization in challenging dense urban environments.
...
### [29] [CurveStream: Boosting Streaming Video Understanding in MLLMs via Curvature-Aware Hierarchical Visual Memory Management](https://arxiv.org/abs/2603.19571)
*Chao Wang,Xudong Tan,Jianjian Cao,Kangcong Li,Tao Chen*

Main category: cs.CV

TL;DR: CurveStream is a training-free, curvature-aware hierarchical visual memory management framework for streaming video understanding that uses geometric curvature to identify critical semantic transitions and adaptively manage memory under token budget constraints.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs struggle with streaming videos due to linear explosion of visual tokens causing OOM errors or catastrophic forgetting. Existing methods lack semantic awareness, potentially disrupting contextual coherence and missing critical semantic transitions.

Method: CurveStream uses curvature scores to evaluate real-time semantic intensity, integrates online K-Sigma dynamic thresholding, and adaptively routes frames into clear and fuzzy memory states under strict token budget constraints based on geometric insights about feature trajectories.

Result: Achieves over 10% absolute performance gains (10.69% on StreamingBench and 13.58% on OVOBench) over baselines, establishing new state-of-the-art results for streaming video perception.

Conclusion: CurveStream effectively addresses memory management challenges in streaming video understanding by leveraging geometric curvature to preserve semantic coherence and critical transitions, outperforming existing methods without requiring training.
...
### [30] [DynFlowDrive: Flow-Based Dynamic World Modeling for Autonomous Driving](https://arxiv.org/abs/2603.19675)
...
<details>
  <summary>Details</summary>
Motivation: Existing world model approaches in autonomous driving typically use appearance generation or deterministic regression, which limits their ability to capture trajectory-conditioned scene evolution and leads to unreliable action planning.

Method: Proposes DynFlowDrive, a latent world model leveraging flow-based dynamics with rectifiedflow formulation to learn a velocity field describing scene state changes under different driving actions, enabling progressive prediction of future latent states. Also introduces a stability-aware multi-mode trajectory selection strategy that evaluates candidate trajectories based on the stability of induced scene transitions.
...
<details>
  <summary>Details</summary>
Motivation: Current open-world semantic segmentation relies on expensive image-text pair datasets with limited fine-grained pixel annotations. Manual data collection is costly in terms of human labor and time, creating a need for automated dataset generation.

Method: 1. Start from class labels to generate high-fidelity textual descriptions. 2. Use diffusion models to generate images guided by these descriptions. 3. Generate both positive and negative (counterfactual) image pairs for contrastive training. 4. Extract object masks using open-vocabulary detection and interactive segmentation models to create pseudo segmentation labels.

Result: Achieved state-of-the-art performance on PASCAL VOC (62.9%), PASCAL Context (26.7%), and COCO (40.2%) datasets, demonstrating the effectiveness of the automatically generated dataset for open-world semantic segmentation.

Conclusion: MagicSeg provides an effective automated pipeline for generating high-quality datasets for open-world semantic segmentation, reducing reliance on expensive manual annotation while achieving competitive performance through contrastive pretraining with pseudo masks and counterfactual samples.
...
### [32] [LIORNet: Self-Supervised LiDAR Snow Removal Framework for Autonomous Driving under Adverse Weather Conditions](https://arxiv.org/abs/2603.19936)
*Ji-il Park,Inwook Shim*

Main category: cs.CV

TL;DR: LIORNet: A self-supervised LiDAR noise filtering method for adverse weather using U-Net++ backbone and multi-cue pseudo-labels, outperforming existing approaches without manual annotation.


<details>
  <summary>Details</summary>
Motivation: LiDAR performance degrades significantly in adverse weather (snow, rain, fog) where spurious noise points dominate point clouds and cause false perception. Existing approaches have limitations: distance-based filters struggle to distinguish valid points, intensity-based methods lack adaptability with fixed thresholds, and learning-based methods suffer from annotation costs, limited generalization, and computational overhead.

Method: LIORNet integrates strengths of three paradigms using U-Net++ backbone with self-supervised learning guided by pseudo-labels generated from multiple physical and statistical cues: range-dependent intensity thresholds, snow reflectivity, point sparsity, and sensing range constraints. This eliminates need for manual annotations while overcoming limitations of single-principle approaches.
...
Conclusion: LIORNet is a practical and robust solution for LiDAR perception in extreme weather with strong potential for real-time deployment in autonomous driving systems, addressing the challenging problem of snow labeling and limitations of existing approaches.
...
### [33] [FlowScene: Style-Consistent Indoor Scene Generation with Multimodal Graph Rectified Flow](https://arxiv.org/abs/2603.19598)
*Zhifei Yang,Guangyao Zhai,Keyang Lu,YuYang Yin,Chao Zhang,Zhen Xiao,Jieyi Long,Nassir Navab,Yikai Wang*

Main category: cs.CV

TL;DR: FlowScene is a tri-branch scene generation model that uses multimodal graphs to collaboratively generate layouts, shapes, and textures with tight-coupled rectified flow, enabling fine-grained object control while maintaining scene-level style coherence.


<details>
  <summary>Details</summary>
Motivation: Existing scene generation methods have limitations: language-driven retrieval lacks object-level control and scene-level style coherence, while graph-based methods offer better controllability but struggle with producing high-fidelity textured results, limiting practical utility.

Method: FlowScene uses a tri-branch generative model conditioned on multimodal graphs with a tight-coupled rectified flow model that exchanges object information during generation, enabling collaborative reasoning across the graph to generate scene layouts, object shapes, and object textures simultaneously.
...
Conclusion: FlowScene successfully addresses the limitations of existing methods by providing fine-grained control over objects' shapes, textures, and relations while enforcing scene-level style coherence across structure and appearance, making it more practical for industrial applications.
...
TL;DR: K-GMRF is an online, training-free framework for covariance tracking that uses second-order forced rigid-body motion on Lie groups, achieving zero steady-state error under constant rotation and outperforming first-order baselines.


<details>
  <summary>Details</summary>
Motivation: Existing covariance tracking methods either ignore manifold constraints or rely on first-order updates, causing inevitable phase lag during rapid evolution. There's a need for a method that respects geometric structure while achieving better tracking performance.

Method: Reformulates covariance tracking as forced rigid-body motion on Lie groups using Euler-Poincaré equations. Observations are interpreted as torques driving latent angular velocity, propagated via a structure-preserving symplectic integrator. This creates a second-order dynamics framework.

Result: Achieves zero steady-state error under constant rotation (strictly superior to proportional lag of first-order baselines). On synthetic ellipses: 30x angular error reduction vs Riemannian EMA. On SO(3) stabilization with 20% dropout: reduces geodesic error from 29.4° to 9.9°. On OTB motion-blur sequences: improves IoU from 0.55 to 0.74 on BlurCar2 with 96% success rate.

Conclusion: K-GMRF provides a plug-and-play geometric prior for data-constrained scenarios and an interpretable layer within modern deep architectures, offering robust tracking fidelity across multiple domains while respecting manifold constraints.
...
TL;DR: ChangeRWKV introduces a new architecture for remote sensing change detection that combines Transformer-like parallel training with RNN-like linear-time inference, achieving state-of-the-art performance with significantly reduced computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing methods face a trade-off: CNNs are efficient but lack global context, while Transformers capture long-range dependencies but are computationally prohibitive. There's a need for an architecture that reconciles this conflict for operational-scale change detection.

Method: ChangeRWKV builds on the RWKV framework, featuring: 1) a hierarchical RWKV encoder for multi-resolution feature representation, and 2) a Spatial-Temporal Fusion Module (STFM) to resolve spatial misalignments across scales while extracting fine-grained temporal differences.
...
Conclusion: ChangeRWKV demonstrates a new efficient and powerful paradigm for operational-scale change detection, successfully combining the benefits of Transformers and RNNs while overcoming their respective limitations.
...
### [36] [Physion-Eval: Evaluating Physical Realism in Generated Video via Human Reasoning](https://arxiv.org/abs/2603.19607)
*Qin Zhang,Peiyu Jing,Hong-Xing Yu,Fangqiang Ding,Fan Nie,Weimin Wang,Yilun Du,James Zou,Jiajun Wu,Bing Shuai*

Main category: cs.CV

TL;DR: Physion-Eval is a benchmark for evaluating physical realism in video generation models using expert human reasoning across 22 physical categories, revealing that 83-94% of generated videos contain identifiable physical glitches.


<details>
  <summary>Details</summary>
Motivation: Current video generation evaluations rely on automated metrics or coarse human judgments that don't provide insight into when and why generated videos violate real-world physical constraints. As video models become world simulators for storytelling and AI, understanding their physical realism limitations is crucial.

Method: Created a large-scale benchmark with 10,990 expert reasoning traces comparing generated videos against real-world reference videos. Videos from 5 state-of-the-art models across egocentric and exocentric views were annotated with temporally localized glitches, structured failure categories, and natural-language explanations of violated physical behaviors.

Result: In physics-critical scenarios, 83.3% of exocentric and 93.5% of egocentric generated videos exhibit at least one human-identifiable physical glitch. The benchmark provides detailed failure analysis across 22 fine-grained physical categories.

Conclusion: Current video generation models have significant limitations in obeying physical laws. Physion-Eval sets a new standard for physical realism evaluation and should guide development of physics-grounded video generation models.
...
### [37] [FB-CLIP: Fine-Grained Zero-Shot Anomaly Detection with Foreground-Background Disentanglement](https://arxiv.org/abs/2603.19608)
*Ming Hu,Yongsheng Huo,Mingyu Dou,Jianfu Yin,Peng Zhao,Yao Wang,Cong Hu,Bingliang Hu,Quan Wang*

Main category: cs.CV

TL;DR: FB-CLIP enhances zero-shot fine-grained anomaly detection by addressing CLIP's foreground-background entanglement and coarse textual semantics through multi-strategy textual representations and visual foreground-background separation.


<details>
  <summary>Details</summary>
Motivation: Fine-grained anomaly detection is crucial for industrial/medical applications but suffers from scarce labeled anomalies. While vision-language models like CLIP offer zero-shot potential, they struggle with foreground-background feature entanglement and coarse textual semantics that limit precise anomaly localization.

Method: FB-CLIP uses multi-strategy textual representations (End-of-Text features, global-pooled representations, attention-weighted token features) and visual multi-view soft separation along identity, semantic, and spatial dimensions with background suppression. Semantic Consistency Regularization aligns image features with normal/abnormal textual prototypes while suppressing uncertain matches.
...
Conclusion: The proposed FB-CLIP framework successfully addresses CLIP's limitations for zero-shot fine-grained anomaly detection through enhanced textual representations and visual foreground-background separation, enabling precise anomaly localization without labeled training data.
...
TL;DR: ParallelVLM is a training-free speculative decoding framework that accelerates Video-LLMs by 2.4-3.4x through parallel draft-then-verify stages and unbiased verifier-guided pruning, overcoming token efficiency bottlenecks in long-video understanding.


<details>
  <summary>Details</summary>
Motivation: Current Video-LLMs suffer from inefficient autoregressive decoding due to massive video tokens. Existing visual token pruning approaches still cause information loss and provide only modest acceleration, while traditional speculative decoding faces mutual waiting and limited speedup problems in long-video settings.
...
Result: ParallelVLM expands the draft window by 1.6-1.8x with high accepted lengths, and accelerates various video understanding benchmarks by 3.36x on LLaVA-Onevision-72B and 2.42x on Qwen2.5-VL-32B compared to vanilla autoregressive decoding.

Conclusion: ParallelVLM effectively overcomes the decoding efficiency bottleneck in Video-LLMs through parallel speculative decoding and unbiased pruning, achieving significant speedup without training while maintaining high accuracy for long-video understanding tasks.
...
TL;DR: OrbitNVS reformulates novel view synthesis as orbit video generation, adapting a pre-trained video model with camera adapters, normal map guidance, and pixel-space supervision to improve geometric and appearance consistency.


<details>
  <summary>Details</summary>
Motivation: Existing NVS methods struggle with synthesizing plausible views for unobserved regions (especially under single-view input) and maintaining geometry- and appearance-consistency.

Method: Reformulates NVS as orbit video generation, adapts pre-trained video generation model with camera adapters for accurate camera control, adds normal map generation branch to guide synthesis via attention for geometric consistency, and applies pixel-space supervision to reduce blurry appearance.
...
Conclusion: OrbitNVS effectively addresses key challenges in NVS by leveraging video generation priors and incorporating geometric guidance mechanisms, achieving state-of-the-art performance.
...
TL;DR: UniPR is an end-to-end framework for object-level real-to-sim perception and reconstruction from single stereo image pairs, eliminating modular pipelines and scale ambiguity issues.


<details>
  <summary>Details</summary>
Motivation: Existing modular pipelines for real-to-sim transfer (detection, segmentation, reconstruction, pose estimation) suffer from inefficiency and cumulative errors due to operating on partial information and discarding global context.

Method: UniPR uses single stereo image pairs to leverage geometric constraints for scale resolution, introduces Pose-Aware Shape Representation to eliminate per-category canonical definitions, and bridges reconstruction with pose estimation. Also creates LVS6D dataset with 6,300+ objects.

Result: UniPR reconstructs all scene objects in parallel within a single forward pass, achieving significant efficiency gains while preserving true physical proportions across diverse object types.

Conclusion: UniPR demonstrates potential for practical robotic applications by providing an efficient, end-to-end solution for real-to-sim perception and reconstruction that overcomes limitations of modular approaches.
...
TL;DR: HRNet is a hybrid multimodal registration network that learns stable shared features and predicts unified hybrid transformations (rigid + non-rigid) for handling both global misalignment and local deformation.


<details>
  <summary>Details</summary>
Motivation: Current multimodal registration methods have two key limitations: 1) modality-private cues leak into shared feature spaces despite disentanglement, and 2) most multi-scale frameworks only support single transformation types, limiting applicability when both global misalignment and local deformation coexist.

Method: HRNet uses a shared backbone with Modality-Specific Batch Normalization (MSBN) for multi-scale feature extraction, a Cross-scale Disentanglement and Adaptive Projection (CDAP) module to suppress modality-private cues and project shared features into stable subspace, and a Hybrid Parameter Prediction Module (HPPM) for non-iterative coarse-to-fine estimation of both global rigid parameters and local deformation fields.
...
Conclusion: HRNet effectively addresses the limitations of existing multimodal registration methods by jointly learning stable shared feature spaces and unified hybrid transformations, enabling robust registration when both global and local misalignments are present.
...
TL;DR: IUP-Pose is a geometry-driven decoupled iterative framework for relative pose estimation that achieves high accuracy with end-to-end differentiability and real-time efficiency (70 FPS).


<details>
  <summary>Details</summary>
Motivation: Existing RPR methods face trade-offs: feature-matching pipelines are accurate but non-differentiable due to RANSAC, while ViT-based regressors are end-to-end trainable but too slow for real-time deployment. Core bottlenecks are coupling between rotation/translation estimation and insufficient cross-view feature alignment.
...
Result: Achieves 73.3% AUC@20deg on MegaDepth1500 benchmark with full end-to-end differentiability, 70 FPS throughput, and only 37M parameters, demonstrating favorable accuracy-efficiency trade-off.

Conclusion: IUP-Pose provides an effective solution for real-time relative pose estimation with both high accuracy and computational efficiency, making it suitable for edge deployment in SLAM, visual localization, and 3D reconstruction applications.
...
TL;DR: DPTracker introduces a dual prompt-driven feature encoding method for nighttime UAV tracking, using illumination and viewpoint prompts to enhance robustness under challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Existing UAV tracking methods often fail to adequately handle illumination and viewpoint variations, especially in nighttime conditions, leading to degraded tracking performance. Robust feature encoding that accounts for these critical cues is needed for reliable tracking.

Method: Proposes a dual prompt-driven feature encoding method with two components: (1) pyramid illumination prompter that extracts multi-scale frequency-aware illumination prompts, and (2) dynamic viewpoint prompter that modulates deformable convolution offsets to accommodate viewpoint variations. The method integrates prompt-conditioned feature adaptation and context-aware prompt evolution.

Result: Extensive experiments validate DPTracker's effectiveness for nighttime UAV tracking. Ablation studies confirm the contribution of each component, and real-world tests demonstrate robustness and practical utility across diverse nighttime scenarios.

Conclusion: The dual prompt-driven approach successfully addresses illumination and viewpoint challenges in nighttime UAV tracking, enabling domain-invariant feature encoding and reliable performance in challenging conditions.
...
TL;DR: UniBioTransfer is the first unified framework for multiple deepface generation tasks including face/hair transfer and shape-varying transformations, addressing data scarcity and cross-task conflicts through unified data construction and BioMoE architecture.


<details>
  <summary>Details</summary>
Motivation: Traditional task-specific deepface generation models lack generalization and scalability. A unified model for multiple tasks is promising but challenging due to data scarcity and cross-task conflicts from heterogeneous attribute transformations.

Method: Proposes UniBioTransfer with: 1) unified data construction strategy with swapping-based corruption for dynamic attributes like hair, 2) BioMoE (mixture-of-experts) model with two-stage training to disentangle task-specific knowledge and mitigate cross-task interference.

Result: Extensive experiments show UniBioTransfer outperforms both existing unified models and task-specific methods across various deepface generation tasks. It naturally generalizes to unseen tasks (lip, eye, glasses transfer) with minimal fine-tuning.

Conclusion: UniBioTransfer represents an effective, generalizable, and scalable unified framework for deepface generation that handles both conventional tasks and shape-varying transformations while addressing key challenges of data scarcity and cross-task conflicts.
...
TL;DR: OmniDiT is a unified diffusion transformer framework for virtual try-on and try-off tasks that addresses detail preservation, generalization, and efficiency challenges through self-evolving data curation, adaptive position encoding, and shifted window attention.


<details>
  <summary>Details</summary>
Motivation: Existing VTON methods struggle with fine-grained detail preservation, generalization to complex scenes, complicated pipelines, and efficient inference. There's a need for a unified solution that handles both try-on and try-off tasks effectively.

Method: 1) Self-evolving data curation pipeline creating Omni-TryOn dataset (380k+ garment-model-tryon pairs with text prompts); 2) Token concatenation with adaptive position encoding for multiple reference conditions; 3) Shifted Window Attention for linear complexity; 4) Multiple timestep prediction and alignment loss to improve generation fidelity.

Result: Achieves best performance in model-free VTON and VTOFF tasks across various complex scenes, and comparable performance to current SOTA methods in model-based VTON task.

Conclusion: OmniDiT successfully addresses key challenges in virtual try-on/off through a unified diffusion transformer framework with efficient attention mechanisms and comprehensive data curation, demonstrating superior performance in complex scenarios.
...
TL;DR: GravCal: A feedforward model that calibrates noisy IMU gravity priors using single RGB images, combining residual correction and image-based estimation with learned gating.
...
Method: Feedforward model that takes RGB image and noisy gravity prior as input, predicts corrected gravity direction and confidence score. Combines two complementary predictions: residual correction of input prior and prior-independent image estimate, fused adaptively using learned gate.

Result: Reduces mean angular error from 22.02° (IMU prior) to 14.24°, with larger improvements when prior is severely corrupted. Introduces dataset of 148K+ frames with VIO-derived ground-truth gravity and Mahony-filter IMU priors across diverse scenes.

Conclusion: GravCal effectively calibrates noisy gravity priors from single images, with learned gate providing useful confidence signal for downstream systems, addressing practical limitations of IMU-based gravity estimation.
...
TL;DR: CS-MUNet introduces a novel Mamba-based architecture for abdominal organ segmentation with boundary-aware state modeling and cross-channel semantic collaboration modules.
...
Method: Proposes CS-MUNet with two key modules: 1) Boundary-Aware State Mamba module using Bayesian-attention framework to generate pixel-level boundary posterior maps injected into Mamba's scan parameters, with dual-branch weight allocation for global-local complementary modulation. 2) Channel Mamba State Aggregation module that redefines channel dimension as SSM sequence dimension to explicitly model cross-channel anatomical semantic collaboration.

Result: Experiments on two public benchmarks show CS-MUNet consistently outperforms state-of-the-art methods across multiple metrics.

Conclusion: CS-MUNet establishes a new SSM modeling paradigm that jointly addresses channel semantic collaboration and boundary-aware feature fusion for abdominal multi-organ segmentation.
...
TL;DR: SAVN-CE introduces continuous audio-visual navigation where agents move freely in 3D spaces, addressing intermittent silence of sound-emitting targets via MAGNet, a multimodal transformer with memory-augmented goal reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing audio-visual navigation methods rely on precomputed room impulse responses, restricting agents to discrete grid positions and causing spatially discontinuous observations. Additionally, targets may become silent intermittently, causing agents to lose goal information.
...
Conclusion: SAVN-CE establishes a more realistic continuous audio-visual navigation setting, and MAGNet effectively addresses the challenge of intermittent target silence through multimodal encoding and memory-augmented reasoning.
...
TL;DR: JMVR is a novel EEG-based visual reconstruction framework that treats EEG and text as independent modalities for joint learning, preserving EEG-specific spatial and chromatic details through multi-scale encoding and image augmentation.


<details>
  <summary>Details</summary>
Motivation: Current EEG-based visual reconstruction methods are deeply coupled with alignment frameworks that force EEG features to align with text/image semantics, which condenses rich spatial and chromatic details in EEG signals and leads to mere conditioned image generation rather than high-fidelity reconstruction.
...
Conclusion: The JMVR framework effectively preserves EEG-specific information for visual reconstruction by treating EEG and text as independent modalities, overcoming limitations of alignment-based approaches and enabling high-fidelity reconstruction of spatial and chromatic details.
...
TL;DR: Optimizing bounding box positions in text-to-video diffusion models improves generation quality and control adherence by aligning boxes with model's internal attention maps.


<details>
  <summary>Details</summary>
Motivation: While bounding boxes/layouts are popular for controlling text-to-video diffusion models, ensuring adherence to these control inputs remains challenging. Current methods struggle to enforce precise control while maintaining generation quality.

Method: Proposes optimizing user-provided bounding boxes by: 1) Making box positions differentiable using a smooth mask, 2) Using attention-maximization objective to align boxes with model's internal attention maps, 3) Balancing focus on foreground and background to modify boxes to positions where the model is "familiar" with.

Result: Small modifications to bounding boxes significantly improve generation quality and control adherence. Thorough experiments and user study validate effectiveness. Code is publicly available.

Conclusion: Optimizing bounding box positions by aligning them with diffusion model's attention maps is an effective approach for improving both control adherence and generation quality in text-to-video synthesis.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models have high visual quality but systematically fail at numerical control when prompts specify exact object counts, limiting their practical utility for precise generation tasks.

Method: ATHENA uses intermediate representations during sampling to estimate object counts and applies count-aware noise corrections early in denoising. It offers three variants with increasing complexity: static prompt-based steering, dynamically adjusted count-aware control, and more advanced versions trading computation for accuracy.

Result: ATHENA consistently improves count fidelity, especially at higher target counts, while maintaining favorable accuracy-runtime trade-offs across multiple diffusion backbones on established benchmarks and a new complex dataset.

Conclusion: ATHENA provides an effective model-agnostic solution to improve numerical control in diffusion models through test-time adaptive steering, addressing a key limitation without requiring architectural modifications or retraining.
...
<details>
  <summary>Details</summary>
Motivation: Existing lifelong person re-identification methods only consider global-aware learning and underutilize fine-grained attribute knowledge, limiting their acquisition and anti-forgetting capacity. Vision-Language Models offer generalizable knowledge that could be better leveraged.

Method: VLADR introduces Multi-grain Text Attribute Disentanglement to mine global and local text attributes, and an Inter-domain Cross-modal Attribute Reinforcement scheme that uses cross-modal attribute alignment to guide visual attribute extraction and inter-domain attribute alignment for fine-grained knowledge transfer.
...
Conclusion: Explicitly modeling universally shared human attributes improves inter-domain knowledge transfer in lifelong person re-identification, enabling better utilization of historical knowledge to reinforce new learning and alleviate forgetting.
...
TL;DR: UDML proposes an unbiased dynamic multimodal learning framework with noise-aware uncertainty estimation and modality bias correction to address limitations of existing dynamic fusion methods.


<details>
  <summary>Details</summary>
Motivation: Existing dynamic multimodal methods have two key limitations: 1) they rely on empirical metrics that fail at extreme noise levels (very low or high), and 2) they assume equal initial modality contributions, neglecting intrinsic modality dependency bias, which causes hard-to-learn modalities to be doubly penalized.

Method: Two main components: 1) Noise-aware uncertainty estimator that adds controlled noise to modality data and predicts noise intensity from features, establishing clear feature-corruption correspondence for accurate uncertainty measurement across all noise levels. 2) Quantification of inherent modality reliance bias via modality dropout, incorporating this bias into the weighting mechanism to eliminate dual suppression of hard-to-learn modalities.

Result: Extensive experiments across diverse multimodal benchmark tasks validate the effectiveness, versatility, and generalizability of UDML. The framework outperforms existing methods by addressing both noise measurement limitations and modality bias issues.

Conclusion: UDML successfully addresses key limitations of dynamic multimodal fusion through noise-aware uncertainty estimation and modality bias correction, providing a more robust and adaptive framework for real-world multimodal applications with varying modality quality.
...
TL;DR: Self-constrained prior improves 3D Gaussian Splatting (3DGS) surface reconstruction by using TSDF-based distance fields to constrain Gaussian placement and properties for more accurate depth rendering.


<details>
  <summary>Details</summary>
Motivation: While 3DGS shows advantages over NeRF in rendering quality and speed, there's still room for improvement in recovering high-fidelity surfaces. Current 3DGS methods don't fully leverage geometric constraints for accurate surface reconstruction.

Method: Proposes a self-constrained prior derived from TSDF grids created by fusing depth maps rendered from current 3D Gaussians. The prior creates a distance field band around estimated surfaces to constrain Gaussians: removing those outside the band, moving Gaussians closer to surfaces, and adjusting opacity in geometry-aware manner. The prior is regularly updated with improved depth maps and progressively narrows the constraint band.

Result: The method demonstrates superiority over state-of-the-art approaches on widely used benchmarks, showing improved surface reconstruction fidelity through better geometric constraints on 3D Gaussians.

Conclusion: The self-constrained prior effectively improves 3DGS surface reconstruction by leveraging TSDF-based geometric constraints that are regularly updated and progressively tightened, leading to more accurate depth rendering and higher fidelity surfaces.
...
TL;DR: TSegAgent: A zero-shot geometric reasoning approach for tooth segmentation and identification from 3D dental scans, using foundation models and anatomical constraints instead of task-specific training.


<details>
  <summary>Details</summary>
Motivation: Existing methods require expensive dense annotations and task-specific 3D neural networks, leading to high annotation costs and poor generalization to unseen scan sources. There's a need for more generalizable, annotation-efficient approaches.

Method: Reformulates dental analysis as zero-shot geometric reasoning rather than data-driven recognition. Combines general-purpose foundation models with explicit geometric inductive biases from dental anatomy. Uses multi-view visual abstraction and geometry-grounded reasoning with structural constraints like dental arch organization and volumetric relationships.

Result: Enables accurate and reliable tooth segmentation and identification with low computational and annotation cost. Demonstrates strong generalization across diverse and previously unseen dental scans.

Conclusion: The reasoning-oriented formulation successfully addresses annotation cost and generalization limitations of traditional approaches by leveraging geometric reasoning and anatomical constraints instead of task-specific training.
...
TL;DR: AI framework improves detection of rare cardiac anomalies from ECGs using self-supervised anomaly detection and demographic-aware learning, reducing performance gap by 73% while maintaining equity across populations.
...
Method: Two-stage framework: 1) Self-supervised anomaly detection pretraining reconstructing masked ECG signals and predicting patient attributes, 2) Fine-tuning for multi-label classification with asymmetric loss for long-tail abnormalities, plus anomaly score maps for localization with CPU-based optimization.

Result: Achieves 94.7% AUROC for rare anomalies, reduces common-rare performance gap by 73%, maintains consistent diagnostic accuracy across age and sex groups on over 1 million clinical ECGs.
...
TL;DR: 2D foundation image models possess inherent 3D world understanding, which can be harnessed through an agentic multi-architecture approach to generate coherent 3D-consistent worlds.


<details>
  <summary>Details</summary>
Motivation: To investigate whether 2D foundation image models inherently possess 3D world model capabilities, despite being trained primarily on 2D data, and to explore if this implicit understanding can be leveraged for 3D world synthesis.

Method: Proposes an agentic multi-architecture approach: (1) VLM-based director formulates prompts to guide image synthesis, (2) generator synthesizes new image views, and (3) VLM-backed two-step verifier evaluates and curates generated frames from both 2D image and 3D reconstruction perspectives.

Result: Demonstrates that 2D foundation models do indeed encapsulate 3D world understanding. The agentic approach provides coherent and robust 3D reconstruction, producing output scenes that can be explored through novel view rendering, successfully synthesizing expansive, realistic, and 3D-consistent worlds.

Conclusion: 2D foundation image models inherently possess 3D world model capabilities that can be effectively harnessed through agentic frameworks for high-quality 3D world synthesis, bridging the gap between 2D image generation and 3D scene understanding.
...
Method: BALM framework with two modules: 1) Feature Calibration Module (FCM) recalibrates unimodal features using global context to establish shared representation basis across missing patterns; 2) Gradient Rebalancing Module (GRM) balances learning dynamics by modulating gradient magnitudes and directions from distributional and spatial perspectives.
...
Conclusion: BALM provides a model-agnostic plug-in solution for balanced multimodal learning under imbalanced missing rates, effectively addressing modality imbalance without architectural changes to existing models.
...
TL;DR: PerformRecast: A method for expression-only portrait video editing that recasts facial expressions from driving videos while maintaining head pose and identity, using improved 3DMM-based disentanglement and region-specific supervision.


<details>
  <summary>Details</summary>
Motivation: Existing portrait animation methods struggle to disentangle facial expression from head pose rotation, lacking the ability to edit facial expression independently. This limitation hinders performance editing in animation and film industries where only expression changes are needed while preserving original head pose and identity.

Method: The method improves keypoints transformation formula to align with 3D Morphable Face Model (3DMM) for better disentanglement of expression, identity, and pose. It decouples facial and non-facial regions, pre-trains a teacher model for separate supervision to avoid boundary misalignment issues.

Result: Extensive experiments show the method produces high-quality results more faithful to driving videos, outperforming existing methods in both controllability and efficiency. The approach achieves better disentanglement and provides fine-grained control over expression editing.

Conclusion: PerformRecast enables versatile expression-only video editing for film and animation applications, offering improved disentanglement of facial expression from head pose through 3DMM-aligned transformations and region-specific supervision, with publicly available code and models.
...
TL;DR: PhysNeXt is a dual-input deep learning framework that combines raw video frames and STMap representations for robust remote photoplethysmography, achieving stable heart rate measurement under challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Current rPPG methods have trade-offs: end-to-end video modeling preserves complete spatiotemporal information but introduces noise from motion/illumination, while STMap representations reduce complexity but lose high-frequency details. The paper aims to integrate both approaches' strengths.

Method: PhysNeXt uses a dual-input framework with video frames and STMap representations, incorporating spatio-temporal difference modeling, cross-modal interaction, and structured attention-based decoder for collaborative pulse signal extraction.

Result: PhysNeXt achieves more stable and fine-grained rPPG signal recovery under challenging conditions, demonstrating the effectiveness of joint modeling of video and STMap representations.

Conclusion: The proposed dual-input approach successfully integrates the strengths of both video-based and STMap-based rPPG methods, providing robust contactless heart rate measurement with improved signal quality.
...
TL;DR: UPL introduces uncertainty-aware prototype learning for few-shot 3D semantic segmentation, using probabilistic modeling and dual-stream refinement to improve robustness and generalization with limited supervision.
...
Method: UPL proposes: 1) dual-stream prototype refinement module that leverages both support and query samples, and 2) probabilistic formulation treating prototypes as latent variables solved via variational inference for explicit uncertainty modeling.
...
Conclusion: UPL successfully incorporates uncertainty modeling into prototype learning for few-shot 3D segmentation, improving robustness, generalization, and providing interpretable predictions with uncertainty estimation.
...
### [62] [ReLi3D: Relightable Multi-view 3D Reconstruction with Disentangled Illumination](https://arxiv.org/abs/2603.19753)
*Jan-Niklas Dihlmann,Mark Boss,Simon Donne,Andreas Engelhardt,Hendrik P. A. Lensch,Varun Jampani*

Main category: cs.CV

TL;DR: ReLi3D is a unified pipeline that reconstructs 3D geometry, materials, and illumination from sparse multi-view images in under 1 second, solving the ill-posed disentanglement problem through multi-view constraints.
...
Method: Uses transformer cross-conditioning to fuse multi-view input, followed by a unified two-path prediction strategy: one path predicts object structure/appearance, another predicts environment illumination. Combines differentiable Monte Carlo multiple importance sampling renderer with mixed domain training (synthetic PBR + real-world RGB).

Result: Enables near-instantaneous generation of complete, relightable 3D assets in under one second. Achieves generalizable results in geometry accuracy, material accuracy, and illumination quality through the mixed domain training protocol.

Conclusion: ReLi3D successfully unifies previously separate reconstruction tasks into a single feed-forward pipeline, dramatically improving material and illumination disentanglement through multi-view constraints and enabling practical, fast 3D asset creation.
...
TL;DR: HOP3D introduces hierarchical orthogonal prototypes with entropy-based regularization for generalized few-shot 3D point cloud segmentation, effectively balancing base-class retention and novel-class adaptation.
...
Method: HOP3D learns hierarchical orthogonal prototypes with entropy-based few-shot regularization, using hierarchical orthogonalization to decouple base and novel learning at gradient and representation levels, and uncertainty-based refinement for prototype learning.
...
### [64] [Growing Networks with Autonomous Pruning](https://arxiv.org/abs/2603.19759)
...
<details>
  <summary>Details</summary>
Motivation: Traditional CNNs have fixed architectures and parameter counts, which may be inefficient. The goal is to create networks that can automatically adapt their size to fit the data while minimizing parameters for better efficiency.

Method: GNAP uses two complementary mechanisms: 1) Growth phases where network size expands when convergence saturates to add expressive power, and 2) Autonomous pruning between growth phases where parameters are trained for classification and pruned simultaneously via gradient descent.

Result: Achieved state-of-the-art efficiency: 99.44% accuracy on MNIST with only 6.2k parameters, and 92.2% accuracy on CIFAR10 with 157.8k parameters, demonstrating highly sparse yet accurate networks.

Conclusion: GNAP successfully creates extremely sparse neural networks with high accuracy by dynamically adjusting network size during training, offering an efficient alternative to traditional fixed-architecture CNNs.
...
TL;DR: SeGroS is a fine-tuning framework that addresses granularity mismatch and supervisory redundancy in Unified Multimodal Models by using visual grounding maps to create complementary supervision signals for improved generation fidelity and cross-modal alignment.
...
Method: SeGroS introduces a novel visual grounding map to construct two complementary supervision signals: 1) semantic Visual Hints to compensate for sparse text prompts, and 2) semantically-grounded Corrupted Input that restricts reconstruction loss to core text-aligned regions for masking-based UMMs.
...
Conclusion: SeGroS effectively resolves the granularity mismatch and supervisory redundancy issues in Unified Multimodal Models, leading to substantial improvements in multimodal generation performance through semantically-grounded supervision.
...
### [66] [PCSTracker: Long-Term Scene Flow Estimation for Point Cloud Sequences](https://arxiv.org/abs/2603.19762)
*Min Lin,Gangwei Xu,Xianqi Wang,Yuyi Peng,Xin Yang*

Main category: cs.CV

TL;DR: PCSTracker is an end-to-end framework for consistent scene flow estimation in point cloud sequences, addressing temporal consistency issues in long sequences through iterative optimization, trajectory updates, and sliding-window inference.
...
Method: Three key components: 1) Iterative Geometry Motion Joint Optimization (IGMO) module that models temporal evolution of point features; 2) Spatio-temporal Point Trajectory Update (STTU) module that leverages temporal context for occluded points; 3) Overlapping sliding-window inference strategy with cross-window propagation and in-window refinement.

Result: Achieves best accuracy in long-term scene flow estimation on PointOdyssey3D and ADT3D datasets, maintains real-time performance at 32.5 FPS, and demonstrates superior 3D motion understanding compared to RGB-D-based approaches.

Conclusion: PCSTracker is the first end-to-end framework for consistent scene flow estimation in point cloud sequences, effectively addressing temporal consistency challenges and outperforming existing methods in both accuracy and efficiency.
...
TL;DR: A framework for diagnosing generalization failures in online mapping models by disentangling memorization from overfitting, with geometry-aware metrics and dataset balancing strategies.


<details>
  <summary>Details</summary>
Motivation: Deep learning-based online mapping models for autonomous driving often fail to generalize beyond familiar environments due to memorization of input features and overfitting to known map geometries.

Method: Proposes measures based on evaluation subsets controlling for geographical proximity and geometric similarity, introduces Fréchet distance-based reconstruction statistics, defines failure-mode scores (localization overfitting and map geometry overfitting), analyzes dataset biases with MST diversity and symmetric coverage measures, and formulates MST-based sparsification strategy.

Result: Experiments on nuScenes and Argoverse 2 across multiple state-of-the-art models show more trustworthy assessment of generalization and demonstrate that map geometry-diverse and balanced training sets lead to improved performance while reducing training size.
...
### [68] [FREAK: A Fine-grained Hallucination Evaluation Benchmark for Advanced MLLMs](https://arxiv.org/abs/2603.19765)
*Zhihan Yin,Jianxin Liang,Yueqian Wang,Yifeng Yao,Huishuai Zhang,Dongyan Zhao*

Main category: cs.CV

TL;DR: FREAK is a new multimodal benchmark for fine-grained hallucination assessment in MLLMs using photorealistic images with counter-commonsense edits to evaluate detailed visual perception.


<details>
  <summary>Details</summary>
Motivation: Existing hallucination evaluation benchmarks are limited by oversimplified tasks with saturated metrics or insufficient diversity, failing to adequately assess hallucination extent in state-of-the-art multimodal models.

Method: Proposes FREAK benchmark with high-quality photorealistic images featuring fine-grained counter-commonsense edits to evaluate hallucination in detailed visual perception. Includes a controlled subset for indirect evaluation of target detail perception and systematic evaluation of Chain-of-Thought prompting techniques.
...
Conclusion: FREAK addresses the gap in multimodal hallucination evaluation by providing a comprehensive benchmark for fine-grained assessment, revealing significant hallucination problems in current MLLMs and offering insights into model reasoning through controlled analysis.
...
### [69] [Adapting a Pre-trained Single-Cell Foundation Model to Spatial Gene Expression Generation from Histology Images](https://arxiv.org/abs/2603.19766)
*Donghai Fang,Yongheng Li,Zhen Wang,Yuansong Zeng,Wenwen Min*

Main category: cs.CV

TL;DR: HINGE adapts pre-trained single-cell foundation models for histology-conditioned spatial expression generation, preserving gene-gene dependencies while injecting visual context via lightweight modulation.


<details>
  <summary>Details</summary>
Motivation: Spatial transcriptomics is expensive and low-throughput, motivating prediction from histology. Existing generative approaches lack explicit modeling of gene-gene dependencies, while single-cell foundation models capture these relationships but can't directly condition on histology.

Method: Proposes HINGE which retrofits pre-trained sc-FMs using SoftAdaLN (lightweight, identity-initialized modulation) to inject visual context layer-wise, with expression-space masked diffusion objective and warm-start curriculum for alignment and stability.

Result: Outperforms state-of-the-art baselines on mean Pearson correlation across three ST datasets, yields more accurate spatial marker expression patterns and higher pairwise co-expression consistency.

Conclusion: HINGE establishes a practical route to adapt pre-trained sc-FMs for histology-conditioned spatial expression generation, preserving learned gene relationships while enabling visual conditioning.
...
TL;DR: FlashCap: Flashing LED-based MoCap system for precise motion timing, creating FlashMotion dataset with millisecond resolution across multiple modalities, enabling high-temporal-resolution human pose estimation.


<details>
  <summary>Details</summary>
Motivation: Precise motion timing is crucial for sports analysis but overlooked due to lack of high-temporal-resolution datasets. Current high-speed RGB camera solutions are expensive and impractical for daily use.
...
Conclusion: FlashCap enables precise motion timing research with practical daily-use system. FlashMotion dataset and ResPose baseline open new research opportunities in high-temporal-resolution human pose estimation.
...
TL;DR: ATCG is a plug-and-play module that uses analogical reasoning to generate textual concepts for unlabeled data, improving category discovery by fusing visual and textual features.


<details>
  <summary>Details</summary>
Motivation: Current GCD methods rely on visual-only pipelines with loose coupling between supervised learning and discovery, leading to brittle boundaries especially for fine-grained, look-alike categories.

Method: Introduces Analogical Textual Concept Generator (ATCG) that analogizes from labeled knowledge to new observations, generating textual concepts for unlabeled samples. These concepts are fused with visual features to create a visual-textual reasoning process.

Result: ATCG consistently improves overall, known-class, and novel-class performance across six benchmarks, with largest gains on fine-grained data. It works with both parametric and clustering style GCD pipelines without design changes.

Conclusion: ATCG effectively transfers prior knowledge to novel data through analogical reasoning and visual-textual fusion, sharpening category separation in Generalized Category Discovery.
...
### [72] [Template-based Object Detection Using a Foundation Model](https://arxiv.org/abs/2603.19773)
...
<details>
  <summary>Details</summary>
Motivation: Addresses scenarios requiring object detection without training data generation or model training, particularly useful for automated GUI testing in software development and continuous integration where object designs change frequently.

Method: Uses segmentation foundation models to extract object segments, then applies simple feature-based classification (rather than learning-based approaches) to detect and classify objects.

Result: Achieves performance almost on par with learning-based methods like YOLO for detecting and classifying icons in navigation maps, without requiring any training or dataset creation.

Conclusion: Provides a practical, cost-effective solution for object detection in specialized domains with stable appearances but frequent design changes, eliminating training overhead while maintaining competitive performance.
...
TL;DR: RAM introduces a robust framework for 3D human motion capture using motion-aware semantic tracking with adaptive Kalman filtering, temporal priors for smooth reconstruction, and predictive fusion for continuity.


<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in markerless 3D human motion capture in-the-wild, particularly dealing with severe occlusions, dynamic interactions, and maintaining tracking stability and reconstruction accuracy in complex multi-person scenarios.

Method: RAM combines: 1) motion-aware semantic tracker with adaptive Kalman filtering for robust identity association, 2) memory-augmented Temporal HMR module with spatio-temporal priors for consistent motion reconstruction, 3) lightweight Predictor module for future pose forecasting, and 4) gated combiner for adaptive fusion of reconstructed and predicted features.
...
Conclusion: RAM offers a generalizable paradigm for robust markerless 3D human motion capture in challenging real-world scenarios, effectively handling occlusions, dynamic interactions, and maintaining temporal coherence.
...
### [74] [Evaluating Image Editing with LLMs: A Comprehensive Benchmark and Intermediate-Layer Probing Approach](https://arxiv.org/abs/2603.19775)
*Shiqi Gao,Zitong Xu,Kang Fu,Huiyu Duan,Xiongkuo Min,Jia wang,Guangtao Zhai*

Main category: cs.CV

TL;DR: TIEdit benchmark with 5,120 edited images from 10 models and 15,360 human ratings, plus EditProbe LLM-based evaluator that better aligns with human perception than existing metrics.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation benchmarks for text-guided image editing (TIE) are limited in scale and show weak correlation with human judgments, making reliable assessment challenging.

Method: 1) Created TIEdit benchmark with 512 source images and editing prompts across 8 tasks, generating 5,120 edited images from 10 TIE models. 2) Collected 307,200 raw ratings from 20 experts to produce 15,360 MOS scores across three dimensions. 3) Developed EditProbe, an LLM-based evaluator that probes intermediate layers of multimodal LLMs to capture semantic relationships between source images, editing instructions, and edited results.

Result: Widely used automatic evaluation metrics show limited correlation with human judgments, while EditProbe achieves substantially stronger alignment with human perception. The benchmark provides comprehensive evaluation data across perceptual quality, editing alignment, and content preservation.

Conclusion: TIEdit and EditProbe together provide a foundation for more reliable and perceptually aligned evaluation of text-guided image editing methods, addressing limitations of existing evaluation approaches.
...
TL;DR: HiPath is a lightweight vision-language model framework that predicts structured pathology reports from multi-image inputs, achieving high accuracy and safety with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Pathology reports are structured, multi-granular documents containing diagnostic conclusions, histological grades, and test results across anatomical sites, but existing pathology VLMs reduce this complexity to flat labels or free-form text, failing to capture the structured nature of clinical documentation.
...
Result: Trained on 749K real-world Chinese pathology cases from three hospitals, HiPath achieves 68.9% strict and 74.7% clinically acceptable accuracy with 97.3% safety rate, outperforming all baselines under the same frozen backbone. Cross-hospital evaluation shows only 3.4pp drop in strict accuracy while maintaining 97.1% safety.

Conclusion: HiPath demonstrates that treating structured report prediction as the primary training objective with lightweight specialized modules enables accurate, safe, and generalizable pathology report generation while preserving the structured nature of clinical documentation.
...
### [76] [ReManNet: A Riemannian Manifold Network for Monocular 3D Lane Detection](https://arxiv.org/abs/2603.19776)
*Chengzhi Hong,Bijun Li*

Main category: cs.CV

TL;DR: ReManNet introduces a Road-Manifold Assumption for monocular 3D lane detection, treating roads as smooth 2D manifolds and lanes as embedded 1D submanifolds, with Riemannian Gaussian descriptors and 3D-TLIoU loss for better geometric consistency.


<details>
  <summary>Details</summary>
Motivation: Monocular 3D lane detection suffers from depth ambiguity and weak geometric constraints, causing ill-posed 2D-to-3D lifting that often produces concavities, bulges, and twists in lane predictions.

Method: Proposes Road-Manifold Assumption (road as smooth 2D manifold, lanes as 1D submanifolds), uses Riemannian Gaussian descriptors on SPD manifold to encode geometry, fuses with visual features via lightweight gate, and employs 3D Tunnel Lane IoU loss for shape-level alignment.
...
Conclusion: The Road-Manifold Assumption provides stronger geometric-topological coupling for 3D lane detection, enabling more robust and accurate predictions through Riemannian geometry encoding and shape-aware loss functions.
...
### [77] [One Model, Two Minds: Task-Conditioned Reasoning for Unified Image Quality and Aesthetic Assessment](https://arxiv.org/abs/2603.19779)
*Wen Yin,Cencen Liu,Dingrui Liu,Bing Su,Yuan-Fang Li,Tao He*

Main category: cs.CV

TL;DR: TATAR is a unified framework for both Image Quality Assessment (IQA) and Image Aesthetic Assessment (IAA) that addresses fundamental mismatches between the two tasks through task-aware reasoning and asymmetric rewards.


<details>
  <summary>Details</summary>
Motivation: Existing unified approaches treat IQA and IAA with the same reasoning strategy and reward, which is fundamentally misaligned. IQA relies on low-level, objective perceptual cues and benefits from concise distortion-focused reasoning, while IAA requires deliberative semantic judgment and is poorly served by point-wise score regression.

Method: TATAR uses a shared visual-language backbone with task-conditioned post-training, featuring: 1) Fast-slow task-specific reasoning (concise perceptual rationales for IQA, deliberative aesthetic narratives for IAA), 2) Two-stage SFT+GRPO learning (behavioral priors then reward-driven refinement), and 3) Asymmetric rewards (Gaussian score shaping for IQA, Thurstone-style completion ranking for IAA).
...
Conclusion: Task-conditioned post-training is established as a principled paradigm for unified perceptual scoring, addressing the fundamental reasoning and optimization mismatches between IQA and IAA through the proposed TATAR framework.
...
TL;DR: X-World is an action-conditioned multi-camera generative world model that simulates realistic future driving observations from sensor inputs and action sequences, enabling scalable autonomous driving evaluation.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving evaluation relies heavily on costly real-world road testing with limited scenario coverage and reproducibility issues. There's a need for a real-world simulator that can generate realistic future observations while remaining controllable and stable over long horizons.

Method: X-World uses a multi-view latent video generator that takes synchronized multi-camera history and future action sequences to generate future multi-camera video streams. It explicitly encourages cross-view geometric consistency and temporal coherence, supports optional controls over traffic agents and road elements, and includes text-prompt interfaces for appearance-level control.
...
Conclusion: X-World provides a practical foundation for scalable and reproducible evaluation of autonomous driving systems by enabling realistic simulation of future driving scenarios with multiple control mechanisms.
...
### [79] [Decoupled Sensitivity-Consistency Learning for Weakly Supervised Video Anomaly Detection](https://arxiv.org/abs/2603.19780)
*Hantao Zheng,Ning Han,Yawen Zeng,Hao Chen*

Main category: cs.CV

TL;DR: DeSC is a novel decoupled framework for weakly supervised video anomaly detection that separates temporal sensitivity and semantic consistency into specialized streams to overcome the sensitivity-stability trade-off in unified approaches.


<details>
  <summary>Details</summary>
Motivation: Current unified frameworks for weakly supervised video anomaly detection suffer from a fundamental sensitivity-stability trade-off, where conflicting objectives for detecting transient anomalies (sensitivity) and sustained anomalies (stability/consistency) lead to either fragmented predictions or over-smoothed responses.

Method: DeSC proposes a Decoupled Sensitivity-Consistency framework with two specialized streams: 1) Temporal sensitivity stream using aggressive optimization to capture high-frequency abrupt changes, and 2) Semantic consistency stream applying robust constraints to maintain long-term coherence and reduce noise. The streams are fused through a collaborative inference mechanism that reduces individual biases.
...
Conclusion: The decoupled approach effectively addresses the sensitivity-stability trade-off in weakly supervised video anomaly detection by separating conflicting objectives into specialized streams, demonstrating superior performance over unified frameworks through complementary optimization strategies.
...
TL;DR: The paper addresses MLLMs' failure on OCR tasks due to gradient interference in skip connections, proposing Detached Skip-Links to preserve fine-grained visual details and introducing R-Probe for evaluation.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs excel at high-level reasoning but fail on OCR tasks where fine-grained visual details are compromised or misaligned, due to an overlooked optimization issue in multi-layer feature fusion where skip pathways introduce gradient interference that overwrites low-level signals.

Method: Proposes Detached Skip-Links - a minimal modification that reuses shallow features in forward pass while stopping gradients through skip branch during joint training, reducing gradient interference without adding parameters. Also introduces R-Probe to measure pixel-level reconstructability of visual tokens using a shallow decoder initialized from LLM layers.

Result: Across multiple ViT backbones and multimodal benchmarks at scales up to 7M training samples, the approach consistently improves OCR-centric benchmarks and delivers clear gains on general multimodal tasks.

Conclusion: The proposed Detached Skip-Links effectively mitigate gradient interference in MLLMs, preserving fine-grained visual details crucial for OCR tasks while maintaining performance on general multimodal reasoning, with R-Probe providing a diagnostic tool for evaluating visual information preservation.
...
### [81] [From Plausibility to Verifiability: Risk-Controlled Generative OCR for Vision-Language Models](https://arxiv.org/abs/2603.19790)
*Weile Gong,Yiping Zuo,Zijian Lu,Xin He,Weibei Fan,Chen Dai*

Main category: cs.CV

TL;DR: Generative OCR with vision-language models suffers from deployment misalignment where autoregressive decoding favors semantic plausibility over visual grounding, causing severe errors. The paper proposes a Geometric Risk Controller that uses cross-view consensus to selectively accept/reject transcriptions, reducing extreme-error risk at predictable coverage costs.


<details>
  <summary>Details</summary>
Motivation: Modern VLMs can act as generative OCR engines, but open-ended decoding exposes rare but consequential failures. There's a core deployment misalignment: autoregressive decoding favors semantic plausibility, while OCR requires visually grounded and geometrically verifiable outputs. This mismatch produces severe errors like over-generation and unsupported substitutions, creating deployment risk even when benchmark accuracy remains high.

Method: Formulates frozen VLM OCR as a selective accept/abstain problem and proposes a model-agnostic Geometric Risk Controller. The controller probes multiple structured views of the same input, applies lightweight structural screening, and accepts a transcription only when cross-view consensus and stability satisfy predefined criteria, yielding a small family of operating points.

Result: Experiments on frozen VLM backbones and standard OCR benchmarks show consistent reductions in extreme-error risk and catastrophic over-generation at predictable coverage costs. The controller effectively reduces deployment risk while maintaining reasonable coverage.

Conclusion: Reliable deployment of generative OCR with frozen VLMs benefits from explicit system-level risk control rather than unconstrained generation. The proposed Geometric Risk Controller provides a practical approach to mitigate severe errors while maintaining usable coverage.
...
### [82] [Controllable Text-to-Motion Generation via Modular Body-Part Phase Control](https://arxiv.org/abs/2603.19795)
*Minyue Dai,Ke Fan,Anyi Rao,Jingbo Wang,Bo Dai*

Main category: cs.CV

TL;DR: Proposes Modular Body-Part Phase Control, a plug-and-play framework for fine-grained, localized editing of text-to-motion generation using compact scalar-based phase signals.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-motion generation methods struggle with modifying specific body parts while maintaining overall motion coherence, relying on cumbersome high-dimensional joint constraints that hinder user-friendly iterative refinement.

Method: Models body-part latent motion channels as sinusoidal phase signals (amplitude, frequency, phase shift, offset) to extract interpretable codes, then uses a modular Phase ControlNet branch to inject signals via residual feature modulation, decoupling control from the generative backbone.

Result: The approach provides predictable, fine-grained control over motion magnitude, speed, and timing while preserving global motion coherence, working with both diffusion- and flow-based models.

Conclusion: The framework offers a practical paradigm for controllable text-to-motion generation with structured, localized editing through a compact, scalar-based phase interface.
...
### [83] [Evaluating Vision Foundation Models for Pixel and Object Classification in Microscopy](https://arxiv.org/abs/2603.19802)
*Carolin Teuber,Anwai Archit,Tobias Boothe,Peter Ditte,Jochen Rink,Constantin Pape*

Main category: cs.CV

TL;DR: Vision foundation models (VFMs) improve pixel and object classification in microscopy over traditional feature-based shallow learning, establishing a benchmark for future development.


<details>
  <summary>Details</summary>
Motivation: Despite deep learning's dominance in computer vision, biomedical imaging still relies on feature-based shallow learning for interactive semantic segmentation (pixel classification) and object-level classification due to data diversity, lack of large pretraining datasets, and need for computational/label efficiency. The paper investigates whether vision foundation models (VFMs) can improve these tasks compared to current approaches.
...
Conclusion: The study establishes a benchmark for VFMs in microscopy and informs future developments in this area, showing that VFMs can effectively improve pixel and object classification tasks in biomedical imaging.
...
Method: Created HUGE-Bench with 4 real-world digital twin scenes, 8 high-level tasks, and 2.56M meters of trajectories using aligned 3D Gaussian Splatting-Mesh representation for photorealistic rendering and collision-capable geometry. Introduced process-oriented and collision-aware metrics.

Result: Experiments on state-of-the-art VLA models show significant gaps in high-level semantic completion and safe execution, demonstrating HUGE-Bench's diagnostic value for UAV autonomy.

Conclusion: HUGE-Bench serves as a diagnostic testbed for high-level UAV autonomy, highlighting current limitations in interpreting concise language and executing safe, complex trajectories.
...
TL;DR: CoA is a reinforcement learning framework that adapts VLMs to specialized domains while preserving their original multimodal reasoning capabilities, achieving better generalization than standard fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Standard fine-tuning on domain-specific data can degrade a model's pretrained multimodal priors and generalization abilities, creating a need for adaptation methods that maintain core capabilities while incorporating domain knowledge.

Method: Chain-of-Adaptation (CoA) uses a structured reasoning format and reinforcement learning to integrate domain knowledge while preserving the model's inherent reasoning and perceptual capabilities, rather than directly fine-tuning parameters.

Result: CoA outperforms supervised fine-tuning on surgical benchmarks in both in-distribution and out-of-distribution settings, achieving higher accuracy, stronger generalization, and more stable behavior while preserving visual-language abilities.

Conclusion: CoA provides an effective pathway for domain specialization in VLMs that maintains their core multimodal competence, offering a better alternative to conventional fine-tuning for specialized applications.
...
TL;DR: Fourier Splatting introduces scalable primitives for radiance field rendering using Fourier-encoded planar surfels, enabling variable level-of-detail rendering from a single trained model by truncating Fourier coefficients at runtime.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods tie visual fidelity strictly to primitive count, requiring quality downscaling through primitive pruning. There's a need for inherently scalable primitives that can render at varying levels of detail from a single model.

Method: Uses Fourier-encoded descriptors to parameterize planar surfels with arbitrary closed shapes. Employs straight-through estimator for gradient extension beyond primitive boundaries, and introduces HYDRA densification strategy that decomposes complex primitives into simpler constituents within MCMC framework.

Result: Achieves state-of-the-art rendering quality among planar-primitive frameworks and comparable perceptual metrics to leading volumetric representations on standard benchmarks, providing versatile solution for bandwidth-constrained high-fidelity rendering.

Conclusion: Fourier Splatting presents the first inherently scalable primitive for radiance field rendering, enabling efficient variable level-of-detail rendering from a single model while maintaining high visual quality.
...
TL;DR: Hyper-Connections (HC) improve volumetric brain tumor segmentation across multiple architectures with minimal parameter overhead, showing strongest gains in 3D models and enhanced sensitivity to clinically dominant MRI sequences.


<details>
  <summary>Details</summary>
Motivation: To enhance multi-modal brain tumor segmentation by replacing fixed residual connections with dynamic Hyper-Connections that can adaptively aggregate features from different MRI modalities, potentially improving fine-grained boundary delineation and clinical relevance.

Method: Hyper-Connections are integrated as drop-in replacements for fixed residual connections across five architectures (nnU-Net, SwinUNETR, VT-UNet, U-Net, U-Netpp) and evaluated on BraTS 2021 dataset for 3D brain tumor segmentation. Modality ablation studies are conducted to analyze sensitivity to different MRI sequences.

Result: HC consistently improves all 3D models with up to +1.03% mean Dice gain and negligible parameter overhead. Most significant improvements in Enhancing Tumor sub-region. HC-equipped models develop sharper sensitivity to clinically dominant sequences (T1ce for Tumor Core/Enhancing Tumor, FLAIR for Whole Tumor), unlike fixed-connection baselines. 2D improvements are smaller and configuration-sensitive.

Conclusion: Hyper-Connections are a simple, efficient, and broadly applicable mechanism for multi-modal feature fusion in medical image segmentation, with volumetric spatial context amplifying their benefits for improved clinical relevance and boundary delineation.
...
TL;DR: A greedy frame selection method for long-video QA that jointly optimizes query relevance and semantic coverage under fixed frame budgets, with question-adaptive strategy routing.


<details>
  <summary>Details</summary>
Motivation: Current VLMs for long-video QA face inference bottlenecks from too many visual tokens. Naive sparse sampling misses key moments, while relevance-only selection collapses to near-duplicate frames and loses temporal coverage.

Method: Question-adaptive greedy frame selection with 1 FPS candidate pool (max 1000 frames), dual embeddings (SigLIP for relevance, DINOv2 for semantic similarity), maximizing weighted sum of relevance and coverage terms. Uses four preset strategies and lightweight question-type classifier to route queries.
...
### [89] [VideoSeek: Long-Horizon Video Agent with Tool-Guided Seeking](https://arxiv.org/abs/2603.20185)
...
<details>
  <summary>Details</summary>
Motivation: Current video agentic models rely heavily on greedy parsing over densely sampled video frames, resulting in high computational costs. There's a need for more efficient approaches that can understand videos without exhaustive frame analysis.

Method: VideoSeek operates in a think-act-observe loop with a toolkit for collecting multi-granular video observations. It leverages video logic flow to actively seek answer-critical evidence rather than exhaustively parsing full videos, enabling query-aware exploration over accumulated observations.

Result: Experiments on four challenging video understanding benchmarks show VideoSeek achieves strong accuracy while using far fewer frames than prior video agents and standalone LMMs. Notably, it achieves 10.2 absolute points improvement on LVBench over GPT-5 while using 93% fewer frames.

Conclusion: VideoSeek demonstrates that leveraging video logic flow for active evidence-seeking enables efficient video understanding with significantly reduced computational costs while maintaining or improving performance. The approach highlights the importance of toolkit design and strong reasoning capabilities.
...
### [90] [IsoCLIP: Decomposing CLIP Projectors for Efficient Intra-modal Alignment](https://arxiv.org/abs/2603.19862)
*Simone Magistri,Dipam Goswami,Marco Mistretta,Bartłomiej Twardowski,Joost van de Weijer,Andrew D. Bagdanov*

Main category: cs.CV

TL;DR: IsoCLIP addresses intra-modal misalignment in CLIP models by identifying and removing anisotropic directions in the shared embedding space, improving performance on intra-modal tasks like image-to-image retrieval without retraining.


<details>
  <summary>Details</summary>
Motivation: CLIP models suffer from intra-modal misalignment when applied to single-modality tasks (like image-to-image retrieval) because their contrastive training focuses on inter-modal alignment, leaving intra-modal features poorly aligned within the same modality.

Method: Analyzes CLIP's projector weights to identify an isotropic subspace where modalities are well-aligned and anisotropic directions specific to each modality. Removes these anisotropic directions to improve intra-modal alignment without additional training.

Result: The method reduces intra-modal misalignment, lowers latency, and outperforms existing approaches on intra-modal retrieval and classification benchmarks across multiple pre-trained CLIP models.

Conclusion: Intra-modal misalignment in CLIP can be effectively addressed by analyzing and modifying the shared embedding space structure, providing a training-free solution that improves performance on single-modality tasks.
...
TL;DR: LumosX is a framework for personalized multi-subject video generation that improves face-attribute alignment through both data curation and novel attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-video diffusion models struggle with precise face-attribute alignment across subjects, lacking explicit mechanisms for intra-group consistency and subject-attribute dependencies.

Method: Two-pronged approach: 1) Data side - tailored collection pipeline using MLLMs to infer subject-specific dependencies and construct relational priors; 2) Modeling side - Relational Self-Attention and Relational Cross-Attention mechanisms that intertwine position-aware embeddings with refined attention dynamics.

Result: Comprehensive evaluations show LumosX achieves state-of-the-art performance in fine-grained, identity-consistent, and semantically aligned personalized multi-subject video generation.

Conclusion: LumosX addresses the face-attribute alignment challenge through explicit modeling strategies and face-attribute-aware data resources, enabling disciplined intra-group cohesion and better separation between subject clusters.
...
TL;DR: MedQ-Engine is a closed-loop data engine that iteratively improves multimodal large language models for medical image quality assessment through failure prototype discovery, progressive human annotation, and quality-assured fine-tuning, achieving near-human performance with high sample efficiency.
...
Method: MedQ-Engine uses a closed-loop system that: 1) evaluates models to discover failure prototypes via data-driven clustering, 2) explores million-scale image pool using prototypes as retrieval anchors with progressive human-in-the-loop annotation, 3) evolves through quality-assured fine-tuning, forming a self-improving cycle. Includes entropy-guided routing to triage annotations and minimize labeling costs.

Result: Across five medical imaging modalities, MedQ-Engine elevates an 8B-parameter model to surpass GPT-4o by over 13% and narrows the gap with human experts to only 4.34%, using only 10K annotations with more than 4x sample efficiency over random sampling.

Conclusion: MedQ-Engine provides an effective framework for improving MLLMs in medical image quality assessment through iterative, targeted data collection and model refinement, achieving near-expert performance with significantly reduced annotation costs.
...
<details>
  <summary>Details</summary>
Motivation: Existing tampering detection benchmarks rely on object masks that misalign with true edit signals - many pixels inside masks are untouched while subtle edits outside masks are treated as natural, creating evaluation problems.
...
Result: Re-evaluation of existing strong baselines reveals substantial over- and under-scoring using mask-only metrics, exposing failure modes on micro-edits and off-mask changes. The framework establishes rigorous standards for tamper localization, semantic classification and description.

Conclusion: PIXAR advances the field from masks to pixels, meanings and language descriptions, providing a comprehensive benchmark and evaluation protocol for VLM image tampering detection that better aligns with real-world edit signals.
...
### [94] [SIMPLER: Efficient Foundation Model Adaptation via Similarity-Guided Layer Pruning for Earth Observation](https://arxiv.org/abs/2603.19873)
...
<details>
  <summary>Details</summary>
Motivation: Fine-tuning Earth Observation foundation models is computationally expensive in both training and inference. Existing parameter-efficient methods reduce training costs but keep full inference complexity, while post-hoc compression requires costly full fine-tuning first.

Method: SIMPLER exploits the stabilization of representations in deeper layers of pre-trained vision transformers. It computes layer-wise representation similarity on unlabeled task data and applies an automated scoring function to select redundant layers, requiring no gradients, magnitude heuristics, or hyperparameter tuning.

Result: On Prithvi-EO-2, SIMPLER prunes up to 79% of parameters while retaining 94% of baseline performance, yielding 2.1x training speedup and 2.6x inference speedup. The method generalizes to TerraMind and ImageNet-pretrained ViT-MAE across tasks, architectures, and spectral modalities.

Conclusion: SIMPLER provides an effective pre-fine-tuning architecture selection approach that reduces both training and inference costs for Earth Observation foundation models, demonstrating broad applicability across different models and tasks without requiring gradients or hyperparameter tuning.
...
### [95] [PanORama: Multiview Consistent Panoptic Segmentation in Operating Rooms](https://arxiv.org/abs/2603.19920)
*Tuna Gürbüz,Ege Özsoy,Tony Danjun Wang,Nassir Navab*

Main category: cs.CV

TL;DR: PanORama: A multiview-consistent panoptic segmentation method for operating rooms that achieves >70% PQ without camera calibration and generalizes to unseen viewpoints.


<details>
  <summary>Details</summary>
Motivation: Operating rooms are complex, cluttered environments with high occlusion where reliable spatial understanding is crucial for surgical situational awareness. Current multiview segmentation methods suffer from mispredictions due to limited visibility in some camera views.

Method: PanORama models cross-view interactions at the feature level within the backbone network in a single forward pass, achieving view consistency directly rather than through post-hoc refinement. The approach is calibration-free and doesn't require camera parameters.

Result: Achieves >70% Panoptic Quality (PQ) on MM-OR and 4D-OR datasets, outperforming previous state-of-the-art methods. The method generalizes to unseen camera viewpoints within any multiview configuration at inference time.

Conclusion: PanORama substantially enhances multiview segmentation and spatial understanding in ORs, opening new opportunities for surgical perception and assistance systems.
...
### [96] [SegVGGT: Joint 3D Reconstruction and Instance Segmentation from Multi-View Images](https://arxiv.org/abs/2603.19926)
...
<details>
  <summary>Details</summary>
Motivation: Current 3D instance segmentation methods rely on high-quality point clouds or posed RGB-D scans, requiring complex multi-stage pipelines and being sensitive to reconstruction noise. Recent feed-forward transformers for 3D reconstruction lack semantic understanding, creating a gap between geometry reconstruction and instance segmentation.

Method: SegVGGT introduces object queries that interact with multi-level geometric features to integrate instance identification into a visual geometry grounded transformer. To solve attention dispersion from massive global image tokens, the Frame-level Attention Distribution Alignment (FADA) strategy explicitly guides object queries to attend to instance-relevant frames during training without extra inference overhead.

Result: Extensive experiments show SegVGGT achieves state-of-the-art performance on ScanNetv2 and ScanNet200, outperforming both recent joint models and RGB-D-based approaches, while demonstrating strong generalization on ScanNet++.

Conclusion: SegVGGT successfully unifies 3D reconstruction and instance segmentation in an end-to-end framework, effectively addressing attention dispersion through FADA and achieving superior performance across multiple benchmarks.
...
### [97] [Timestep-Aware Block Masking for Efficient Diffusion Model Inference](https://arxiv.org/abs/2603.19939)
*Haodong He,Yuan Gao,Weizhong Zhang,Gui-Song Xia*

Main category: cs.CV

TL;DR: A framework for optimizing diffusion model inference by learning timestep-specific masks to dynamically skip or reuse features, reducing computational cost while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Diffusion models suffer from high inference latency due to iterative denoising, requiring optimization of computational graphs on a per-timestep basis to improve efficiency.

Method: Learn timestep-specific masks to determine which blocks to execute or bypass via feature reuse, with timestep-aware loss scaling and knowledge-guided mask rectification to prune redundant dependencies.

Result: Achieves significant efficiency gains across DDPM, LDM, DiT, and PixArt models with superior balance between sampling speed and generative quality.

Conclusion: Treating denoising as optimized computational paths enables memory-efficient training and architecture-agnostic acceleration of diffusion models.
...
### [98] [Cov2Pose: Leveraging Spatial Covariance for Direct Manifold-aware 6-DoF Object Pose Estimation](https://arxiv.org/abs/2603.19961)
*Nassim Ali Ousalah,Peyman Rostami,Vincent Gaudillière,Emmanuel Koumandakis,Anis Kacem,Enjie Ghorbel,Djamila Aouada*

Main category: cs.CV

TL;DR: Proposes a direct 6-DoF pose estimation method using covariance-pooled SPD representations and continuous pose encoding via Cholesky decomposition with Riemannian geometry-aware regression.


<details>
  <summary>Details</summary>
Motivation: Direct pose regression methods are efficient but less accurate than indirect methods. They ignore spatial second-order statistics and use discontinuous pose representations that lack robustness.

Method: Uses covariance pooling to encode convolutional features as SPD matrices. Proposes continuous pose encoding via Cholesky decomposition of SPD matrices. Employs manifold-aware network head that respects Riemannian geometry of SPD matrices for end-to-end regression.

Result: Experiments demonstrate relevance of second-order pooling and continuous representations for direct pose regression, including under partial occlusion conditions.

Conclusion: The proposed covariance-pooled SPD representation and continuous pose encoding improve direct pose regression by capturing spatial second-order statistics and ensuring robust, continuous pose predictions.
...
### [99] [2K Retrofit: Entropy-Guided Efficient Sparse Refinement for High-Resolution 3D Geometry Prediction](https://arxiv.org/abs/2603.19964)
*Tianbao Zhang,Zhenyu Liang,Zhenbo Song,Nana Wang,Xiaomei Zhang,Xudong Cai,Zheng Zhu,Kejian Wu,Gang Wang,Zhaoxin Fan*

Main category: cs.CV

TL;DR: 2K Retrofit enables efficient 2K-resolution inference for geometric foundation models without retraining, using coarse predictions and entropy-based sparse refinement to selectively enhance uncertain regions.


<details>
  <summary>Details</summary>
Motivation: Current geometric foundation models face scalability limitations for real-world high-resolution scenarios (2K images), with prohibitive computational and memory demands that challenge practical deployment in autonomous driving, robotics, and AR/MR applications.

Method: A novel framework that leverages fast coarse predictions and entropy-based sparse refinement to selectively enhance high-uncertainty regions, enabling efficient 2K-resolution inference without modifying or retraining the backbone model.
...
Conclusion: 2K Retrofit provides an effective solution for efficient high-resolution geometric prediction, making foundation models practically deployable in real-world applications while maintaining high fidelity and precision with minimal computational overhead.
...
### [100] [MedSPOT: A Workflow-Aware Sequential Grounding Benchmark for Clinical GUI](https://arxiv.org/abs/2603.19993)
*Rozain Shakeel,Abdul Rahman Mohammad Ali,Muneeb Mushtaq,Tausifa Jan Saleem,Tajamul Ashraf*

Main category: cs.CV

TL;DR: MedSPOT introduces a workflow-aware sequential grounding benchmark for clinical GUI environments, evaluating MLLMs on multi-step medical interface tasks rather than isolated grounding queries.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack reliable visual grounding capabilities for high-stakes clinical software environments. Existing benchmarks focus on isolated, single-step grounding queries, missing the sequential, workflow-driven reasoning needed in real-world medical interfaces where tasks evolve across multiple steps and dynamic states.

Method: Created MedSPOT benchmark with 216 task-driven videos and 597 annotated keyframes, each task consisting of 2-3 interdependent grounding steps within realistic medical workflows. Introduced strict sequential evaluation protocol that terminates assessment upon first incorrect prediction to measure error propagation. Developed comprehensive failure taxonomy for systematic diagnosis.

Result: Established a realistic and safety-critical benchmark that captures interface hierarchies, contextual dependencies, and fine-grained spatial precision under evolving conditions. The benchmark enables systematic evaluation of error propagation in multi-step clinical workflows.

Conclusion: MedSPOT shifts evaluation from isolated grounding to workflow-aware sequential reasoning, providing a more realistic assessment framework for multimodal models in medical software environments where procedural robustness is critical for patient safety.
...
TL;DR: NEC-Diff is a diffusion-based framework that combines RAW images and event camera data for high-quality imaging in extreme low-light conditions, using physics-driven constraints and adaptive feature fusion to handle severe noise.


<details>
  <summary>Details</summary>
Motivation: Extreme low-light imaging suffers from severe noise and texture loss due to photon scarcity. While event cameras offer high dynamic range and motion sensitivity, existing methods focus on texture recovery but neglect image noise and event noise, limiting accurate reconstruction in photon-starved conditions.

Method: Proposes NEC-Diff, a diffusion-based event-RAW hybrid framework that: 1) Uses physics-driven constraints by combining RAW's linear light-response with event brightness-change properties for robust dual-modal denoising; 2) Dynamically estimates SNR of both modalities to guide adaptive feature fusion, injecting reliable cues into diffusion process for high-fidelity reconstruction.

Result: Created REAL dataset with 47,800 pixel-aligned low-light RAW images, events, and high-quality references under 0.001-0.8 lux illumination. Extensive experiments demonstrate superiority under extreme darkness.

Conclusion: NEC-Diff effectively addresses extreme low-light imaging challenges by leveraging complementary strengths of RAW and event data through physics-driven constraints and adaptive fusion, enabling high-quality reconstruction in photon-starved conditions.
...
TL;DR: FRAM proposes a two-stage approach for makeup transfer: (1) fine-tuning CLIP on synthetic makeup data, and (2) injecting identity and facial region-aware makeup features into diffusion models for better regional control.


<details>
  <summary>Details</summary>
Motivation: Current makeup transfer methods using off-the-shelf foundation models struggle to capture makeup styles and lack regional controllability for specific facial areas like eyes and mouth.

Method: Two-stage approach: 1) Fine-tune CLIP on synthetic makeup data generated using GPT-o3 and text-driven editing; 2) Use learnable tokens to extract facial region-aware makeup features and ControlNet Union for identity injection into diffusion models.

Result: Experimental results show superiority in regional controllability and makeup transfer performance compared to existing methods.

Conclusion: FRAM addresses limitations of current makeup transfer methods by enabling better makeup style capture and facial region-aware control through specialized CLIP fine-tuning and targeted feature injection.
...
TL;DR: CFCML framework reduces modality gap between medical images and tabular data using coarse-to-fine learning with hierarchical anchor-based relationship mining.
...
Conclusion: CFCML effectively reduces modality gap, enhances inter-class disparity while reducing intra-class disparity, improving crossmodal diagnostic accuracy.
...
TL;DR: MFil-Mamba is a novel visual state space architecture with multi-filter scanning backbone that achieves SOTA performance across multiple vision tasks by capturing unique spatial information while minimizing redundancy.


<details>
  <summary>Details</summary>
Motivation: Extending State Space Models (SSMs) like Mamba to computer vision is challenging due to non-sequential visual data structure and complex 2D spatial dependencies. Existing approaches rely on various traversal strategies over the same input, introducing redundancy and distorting spatial relationships.

Method: Proposes MFil-Mamba with multi-filter scanning backbone where each scan captures unique contextually relevant spatial information (unlike fixed multi-directional traversal). Includes adaptive weighting mechanism to fuse outputs from multiple scans and architectural enhancements.

Result: Superior performance over SOTA models across image classification (83.2% top-1 on ImageNet-1K), object detection (47.3% box AP on MS COCO), instance segmentation (42.7% mask AP), and semantic segmentation (48.5% mIoU on ADE20K).

Conclusion: MFil-Mamba effectively addresses challenges of adapting SSMs to vision tasks through its multi-filter scanning design, achieving strong performance across diverse benchmarks while minimizing redundancy and preserving spatial relationships.
...
Conclusion: The preference-guided debiasing framework successfully addresses the generalization problem in EIQA by separating enhancement-specific patterns from genuine perceptual quality cues, leading to more robust quality assessment across different enhancement algorithms.
...
TL;DR: FacadeFixer is a multi-agent framework that treats building facade defect inspection as collaborative reasoning, using specialized agents for detection/segmentation and generative synthesis to create augmented data, outperforming SOTA methods.
...
Conclusion: The paper proposes a collaborative reasoning approach to facade defect inspection, introduces a comprehensive multi-task dataset, and shows that generative synthesis effectively addresses data scarcity while improving performance on complex defect detection tasks.
...
TL;DR: DefectBench is the first multi-dimensional benchmark for evaluating Large Multimodal Models (LMMs) in building facade inspection, showing they excel at semantic understanding but struggle with precise metric localization, while demonstrating viable zero-shot generative segmentation.


<details>
  <summary>Details</summary>
Motivation: Traditional facade inspection uses specialized discriminative models that lack visual understanding and active reasoning capabilities. LMMs promise paradigm shift but lack rigorous evaluation standards in high-stakes engineering domains.

Method: Created human-in-the-loop semi-automated annotation framework to unify 12 fragmented datasets into standardized hierarchical ontology. Developed DefectBench benchmark evaluating 18 SOTA LMMs across three cognitive dimensions: Semantic Perception, Spatial Localization, and Generative Geometry Segmentation.

Result: Current LMMs demonstrate exceptional topological awareness and semantic understanding (diagnosing "what" and "how") but show significant deficiencies in metric localization precision ("where"). Validated viability of zero-shot generative segmentation where general-purpose models can rival specialized supervised networks without domain-specific training.

Conclusion: Provides rigorous benchmarking standard and high-quality open-source database, establishing new baseline for autonomous AI agents in civil engineering. Shows LMMs can transform facade inspection but need improvement in spatial precision.
...
TL;DR: EgoForge is an egocentric world simulator that generates coherent first-person video from minimal inputs (single image + instruction), using reward-guided diffusion refinement for better intent alignment and temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for egocentric video generation have limitations: hand-centric methods have limited scene evolution, static view translation lacks action dynamics, and many require dense supervision like camera trajectories or long video prefixes. Egocentric video is challenging due to rapid viewpoint changes, hand-object interactions, and goal-directed procedures dependent on latent human intent.

Method: EgoForge uses minimal static inputs: a single egocentric image, a high-level instruction, and optional auxiliary exocentric view. It introduces VideoDiffusionNFT, a trajectory-level reward-guided refinement that optimizes goal completion, temporal causality, scene consistency, and perceptual fidelity during diffusion sampling.

Result: EgoForge achieves consistent gains in semantic alignment, geometric stability, and motion fidelity over strong baselines. It also demonstrates robust performance in real-world smart-glasses experiments.

Conclusion: EgoForge successfully addresses the challenges of egocentric video generation by creating a goal-directed world simulator that produces coherent first-person video rollouts from minimal inputs, with improved intent alignment and temporal consistency through reward-guided diffusion refinement.
...
TL;DR: TinyML pipeline for CubeSat onboard image classification using model compression techniques to meet strict resource constraints while maintaining acceptable accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional Earth observation missions transmit raw imagery to ground stations, which is infeasible for CubeSats due to limited onboard processing power, energy, and communication bandwidth constraints.

Method: Developed a TinyML pipeline integrating structured iterative pruning, post-training INT8 quantization, and hardware-aware operator mapping to compress CNN models for deployment on STM32N6 microcontroller with Arm Cortex-M55 and NPU.

Result: Achieved average 89.55% RAM reduction and 70.09% Flash memory reduction with acceptable accuracy drops (0.4-8.6 percentage points). Energy consumption: 0.68-6.45 mJ per inference, latency: 3.22-30.38 ms.

Conclusion: The proposed TinyML pipeline enables efficient onboard EO processing for CubeSats, meeting stringent energy budgets and real-time constraints while significantly reducing downlink bandwidth requirements.
...
TL;DR: LagerNVS is a 3D-aware neural network for novel view synthesis that uses pre-trained 3D reconstruction features to achieve state-of-the-art results with real-time rendering.
...
Method: LagerNVS uses an encoder-decoder architecture with 3D-aware latent features. The encoder is initialized from a pre-trained 3D reconstruction network (using explicit 3D supervision), paired with a lightweight decoder, and trained end-to-end with photometric losses.
...
TL;DR: I2I-RFR reformulates standard image-to-image regression networks as continuous-time transport models, enabling progressive refinement with minimal changes to training pipelines while improving perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Standard pixel-wise I2I regression often over-smooths ill-posed and multimodal targets, while generative alternatives require complex pipelines, task-specific tuning, and additional components. There's a need for a lightweight method that combines the simplicity of regression with the refinement capabilities of generative models.

Method: Augments backbone input by channel-wise concatenation with noise-corrupted ground-truth targets and optimizes a t-reweighted pixel loss. This admits a rectified-flow interpretation via induced velocity field, enabling ODE-based progressive refinement at inference with just a few explicit solver steps (e.g., 3 steps) without distillation.

Result: Extensive experiments across multiple image-to-image translation and video restoration tasks show general performance improvements across tasks and backbones, with particularly clear gains in perceptual quality and detail preservation.
...
TL;DR: MuSteerNet generates 3D human reactions from videos using mutual steering between observations and reactions to overcome relational distortion issues in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing video-driven human reaction generation methods fail to effectively leverage video inputs, resulting in reaction motions mismatched with video content due to severe relational distortion between visual observations and reaction types.
...
Result: Extensive experiments and ablation studies validate the method's effectiveness, achieving competitive performance in generating 3D human reactions from videos.

Conclusion: MuSteerNet effectively addresses relational distortion in video-driven human reaction generation through observation-reaction mutual steering, improving reaction quality and enabling better alignment between generated motions and video content.
...
TL;DR: Training-free sampling methods improve sample diversity in ambiguous segmentation tasks using diffusion models, achieving up to 16.4% HM IoU* improvement on Cityscapes.


<details>
  <summary>Details</summary>
Motivation: Predicting multiple plausible outcomes in uncertain environments requires models that can capture multi-modal distributions. While diffusion models can learn these distributions, naive sampling is computationally inefficient and may miss operationally relevant low-probability modes.

Method: Adapted particle guidance and SPELL techniques (originally for diverse natural image generation) to discrete segmentation tasks, and proposed a simple clustering-based technique. Validated on LIDC medical dataset, modified Cityscapes, and new MMFire wildfire spread dataset.

Result: Compared to naive sampling, these approaches increased HM IoU* metric by up to 7.5% on MMFire and 16.4% on Cityscapes, demonstrating efficient sample diversity improvement with little cost to image quality and runtime.

Conclusion: Training-free methods can effectively increase sample diversity of segmentation diffusion models for ambiguous prediction tasks in various domains including medical imaging, autonomous driving, and wildfire spread.
...
TL;DR: Zero-shot CoVR method uses LMMs to reason about causal/temporal after-effects of video edits, outperforming baselines on challenging implicit-effect cases.
...
Method: Reasoning-first, zero-shot approach leveraging large multimodal models to: (1) infer causal and temporal consequences implied by the edit, and (2) align reasoned queries to candidate videos without task-specific finetuning. Also introduces CoVR-Reason benchmark with structured reasoning traces and challenging distractors.
...
TL;DR: Proposes mode proposal models - a deterministic framework that directly generates multiple likely segmentation masks in one forward pass, avoiding expensive stochastic sampling used in current generative approaches.


<details>
  <summary>Details</summary>
Motivation: Current methods for ambiguous segmentation tasks (like medical imaging) use generative models that require computationally expensive stochastic sampling and post-hoc clustering to identify distribution modes. Need more efficient approach.
...
Conclusion: Shifts focus from stochastic sampling to direct generation of likely outcomes, providing efficient deterministic alternative to current generative approaches for ambiguous segmentation tasks.
...
TL;DR: Proposes MME-CoF-Pro benchmark to evaluate reasoning coherence in video generative models, revealing they exhibit weak reasoning coherence despite good generation quality.
...
Method: Proposes MME-CoF-Pro benchmark with 303 samples across 16 categories (visual logical to scientific reasoning). Introduces Reasoning Score metric for process-level intermediate reasoning steps. Includes three evaluation settings: no hint, text hint, and visual hint to study hint guidance mechanisms.

Result: Evaluation of 7 open/closed-source video models shows: (1) Weak reasoning coherence decoupled from generation quality; (2) Text hints boost apparent correctness but cause inconsistency and hallucinated reasoning; (3) Visual hints benefit structured perceptual tasks but struggle with fine-grained perception.

Conclusion: Video generative models lack reasoning coherence despite good generation quality. The proposed benchmark enables systematic evaluation of reasoning consistency, revealing limitations in current models' causal reasoning capabilities.
...
### [118] [Experimental Analysis of Microbubble Propagation for In-Body Data Transmission](https://arxiv.org/abs/2603.19372)
*Annika Tjabben,Lea Bergkemper,Carolin Conrad,Michael Gundall,Hans D. Schotten*

Main category: eess.SP

TL;DR: Microbubbles show promise as communication carriers for in-body biomedical applications, with experimental validation of detection methods and modulation schemes demonstrating feasibility despite noise challenges.


<details>
  <summary>Details</summary>
Motivation: In-body communication has significant potential for medical diagnostics and therapeutic interventions, but requires effective communication carriers. Microbubbles offer distinct physical properties that make them promising candidates for facilitating communication within the human body.

Method: Experimental analysis using microbubbles as communication carriers, focusing on detection methods and modulation schemes. Applied filtering and peak detection techniques to accurately identify microbubble presence despite noise interference.

Result: Demonstrated feasibility and effectiveness of microbubble-based communication systems. Successfully identified microbubble presence using filtering and peak detection methods, providing insights into signal integrity and noise challenges.

Conclusion: Microbubble-based communication systems show promise for future biomedical applications. The work establishes a foundation for optimizing detection algorithms and advancing in-body communication technologies for medical diagnostics and therapeutic interventions.
...
### [119] [Geometric Performance Analysis of Doppler-Based Positioning with a Single LEO Satellite](https://arxiv.org/abs/2603.19499)
*Qi Liu,Marc Fernandez-Temprado,Antoni Reus-Bergas,Gonzalo Seco-Granados,Jose A. Lopez-Salcedo*

Main category: eess.SP

TL;DR: This paper analyzes the geometric characteristics and positioning performance of single-satellite Doppler positioning in LEO, revealing strong directional error behavior with severe cross-track errors but significantly less error along the satellite track.


<details>
  <summary>Details</summary>
Motivation: While LEO satellites are gaining attention for PNT applications, most existing studies focus on multi-satellite constellations, leaving the fundamental positioning performance achievable with a single LEO satellite less explored. The paper aims to analyze the geometric limitations and characteristics of single-satellite Doppler positioning.

Method: The researchers conducted theoretical analysis of Dilution of Precision (DOP) and performed extensive numerical simulations to examine the geometric characteristics and positioning performance of single-satellite Doppler positioning.

Result: The results reveal strong directional error behavior: severe errors in the cross-track direction but significantly less error along the satellite track. This reflects an intrinsic geometric limitation of single-satellite LEO positioning. The analysis provides fundamental insights into the feasibility of LEO-based Doppler positioning using only one satellite.

Conclusion: The work provides an in-depth analysis of single-satellite LEO Doppler positioning, unveiling fundamental limitations and characteristics that could make such positioning feasible today. The findings offer valuable insights into observational geometry in Doppler navigation and guidance for optimizing geometric configurations in future small or single-satellite LEO constellations for strategic applications.
...
### [120] [Multibeam Phased Arrays with Spherical Gold Spatio-temporal Coding for Fading-Resilient and Delay Robust Beam Isolations](https://arxiv.org/abs/2603.19578)
*Yuan Ma,Mike Ballou,Kyle Richard,Hessam Mahdavifar,Najme Ebrahimi*

Main category: eess.SP

TL;DR: Spherical-Gold scheme combines spatial and temporal coding for ISAC multibeam arrays, achieving stable 15+ dB inter-beam isolation vs. conventional CDMA's poor -5 to -7 dB performance under time errors.


<details>
  <summary>Details</summary>
Motivation: Future ISAC systems need simultaneous multibeam operation with low latency and robust isolation under synchronization errors and fading. Conventional Walsh-Hadamard CDMA is too time-sensitive and suffers from inter-beam sidelobe level collapse.

Method: Embed moderate-length Gold sequences into a spherical spatial codebook, leveraging both temporal and spatial correlation bounds without increasing RF complexity. Combines spatial and temporal coding for improved isolation.

Result: Measurement using ADAR3002 Ka-band 256-element receiver with 4 simultaneous beams shows ≥15 dB rejection with <2.5 dB SLL variation under time error/fading. Conventional temporal-only CDMA degrades to -5 to -7 dB SLL with ~8 dB variation.

Conclusion: Spherical-Gold scheme effectively solves inter-beam isolation problem in ISAC systems by combining spatial and temporal coding, providing robust performance under practical impairments without additional RF complexity.
...
### [121] [A Deep Learning Approach to Multipath Component Detection in Power Delay Profiles](https://arxiv.org/abs/2603.19706)
*Ondrej Zeleny,Radek Zavorka,Ales Prokes,Tomas Fryza,Jaroslaw Wojtun,Jan M. Kelner,Cezary Ziolkowski,Aniruddha Chandra*

Main category: eess.SP

TL;DR: The paper proposes a framework combining autoencoders and DBSCAN clustering for detecting multipath components in Power Delay Profiles, with transformer-based autoencoder showing best performance.


<details>
  <summary>Details</summary>
Motivation: Accurate peak detection in Power Delay Profiles is essential for wireless communication tasks like channel estimation and localization, but traditional methods struggle with noise and environmental complexity.

Method: Proposes a framework combining autoencoders (including transformer-based) with DBSCAN clustering for reconstruction-based anomaly detection of multipath components in PDPs, using a relaxed F1 score for evaluation.

Result: The transformer-based autoencoder framework shows superior performance in both reconstruction and anomaly detection compared to other models.

Conclusion: Deep learning approaches, particularly transformer-based autoencoders combined with DBSCAN clustering, offer effective solutions for multipath component detection in Power Delay Profiles, outperforming traditional methods.
...
### [122] [LSTM-Based Power Delay Profile Predictions for Intra-Bus Wireless Propagation](https://arxiv.org/abs/2603.19707)
...
TL;DR: LSTM model predicts 60 GHz mmWave channel transfer function inside buses with <10% PDP error


<details>
  <summary>Details</summary>
Motivation: Need accurate channel prediction for 60 GHz mmWave communications in dynamic bus environments where LSTM can capture both long-term dependencies and short-term wireless channel changes

Method: Simple LSTM deep learning model to predict channel transfer function (CTF) for transmitter-receiver locations inside buses at 60 GHz millimeter wave band

Result: Average error of power delay profile (PDP) taps from predicted CTFs was less than 10% compared to ground truth measurements

Conclusion: LSTM is effective for predicting 60 GHz mmWave channel characteristics in bus environments, achieving high accuracy with less than 10% PDP error
...
### [123] [Codebook-Based Self-Sustainable RIS: Optimal Splitting Schemes and Power Allocation](https://arxiv.org/abs/2603.19746)
*Friedemann Laue,Sebastian Lotter,Nikita Shani,Robert Schober*

Main category: eess.SP

TL;DR: This paper designs a RIS codebook and energy harvesting framework for self-sustainable RIS operation, analyzing three splitting schemes (PS, ES, TS) to minimize BS transmit power through joint optimization of splitting ratios and power allocation.


<details>
  <summary>Details</summary>
Motivation: To enable self-sustainable RIS operation through energy harvesting while extending BS coverage, requiring efficient energy management and codebook-based configuration.

Method: Develops RIS codebook design and mathematical framework for analyzing three energy harvesting schemes (PS, ES, TS) using tile-based RIS architecture. Considers deterministic/random signals for beam training/data transmission, and jointly optimizes splitting ratio and power allocation across codewords.

Result: Derives optimal power allocation analytically and shows optimal splitting ratio can be found via single-variable grid search. Performance depends on power consumption model and number of RIS tiles, with different splitting schemes achieving lowest BS transmit power depending on system parameters.

Conclusion: The efficiency of optimized energy harvesting schemes for self-sustainable RIS operation depends on system parameters, with no single scheme universally optimal, requiring careful selection based on specific deployment conditions.
...
### [124] [Outlier-Resistant Fusion for Multi-static Positioning using 5G NR Signals](https://arxiv.org/abs/2603.19821)
*Maximiliano Rivera Figueroa,Jannis Held,Pradyumna Kumar Bishoyi,Marina Petrova*

Main category: eess.SP

TL;DR: Proposed an outlier-resistant algorithm for indoor positioning of extended targets in multipath environments, using AoA for initial coarse estimate and Cauchy loss function for refinement, achieving <70cm error in 90% of cases.


<details>
  <summary>Details</summary>
Motivation: Indoor positioning faces challenges from multipath propagation, signal blockages, and extended target characteristics that impact measurement reliability. Existing methods like Least Squares struggle with robustness when dealing with unreliable observations from multipath interactions.

Method: Two-step algorithm: 1) Initial coarse position estimate using angle-of-arrival (AoA), 2) Refinement using Cauchy loss function to suppress outliers and improve robustness against multipath effects.

Result: The algorithm outperforms benchmark methods (Iterative Reweighted Least Squares, LS, and Huber loss function), achieving positioning error less than 70 cm in 90% of cases. Effectiveness confirmed by comparing tracking performance in cluttered vs empty room scenarios.

Conclusion: The proposed outlier-resistant algorithm effectively mitigates multipath effects and improves positioning accuracy for extended targets in complex indoor environments, demonstrating superior robustness compared to existing methods.
...
### [125] [Supervised Contrastive Learning Framework for Electroencephalography-based Air-writing Recognition](https://arxiv.org/abs/2603.19846)
*Anant Jain,Ayush Tripathi*

Main category: eess.SP

TL;DR: Supervised contrastive learning improves EEG-based air-writing recognition by enhancing representation learning, achieving significant accuracy gains over conventional cross-entropy training across different neural network architectures and EEG feature representations.


<details>
  <summary>Details</summary>
Motivation: EEG-based air-writing recognition is challenging due to low signal-to-noise ratio and high inter-subject variability. The study aims to improve this by exploring supervised contrastive learning for better representation learning from EEG signals.

Method: Used preprocessed EEG signals and ICA-derived neural components from 5 participants. Evaluated EEGNet and DeepConvNet architectures with both conventional cross-entropy training and supervised contrastive learning framework. Employed subject-dependent five-fold cross-validation scheme.

Result: Supervised contrastive learning consistently improved classification accuracy: For preprocessed EEG, accuracy increased from 33.45% to 43.77% (EEGNet) and 29.14% to 38.06% (DeepConvNet). Using ICA components achieved even higher accuracies: 49.21% (EEGNet) and 43.32% (DeepConvNet).

Conclusion: Supervised contrastive learning offers an efficient extension to existing EEG-based air-writing recognition approaches, significantly improving classification performance across different architectures and feature representations.
...
### [126] [Reduced-Overhead Channel Estimation and Iterative Detection of FTN Signaling Based on Pilot Superimposition and Spectral Interference Alignment](https://arxiv.org/abs/2603.19950)
*Yuchen Wu,Shinya Sugiura*

Main category: eess.SP

TL;DR: Low-overhead channel estimation for FTN signaling using pilot superimposition with spectral interference alignment to eliminate interference, achieving 50% overhead reduction.


<details>
  <summary>Details</summary>
Motivation: Traditional channel estimation for faster-than-Nyquist signaling requires dedicated resources and guard bands, creating overhead. The paper aims to reduce this overhead while maintaining estimation accuracy.

Method: Proposes pilot superimposition where pilot symbols are overlaid with information symbols in FTN blocks, eliminating need for dedicated resources. Uses spectral interference alignment with data-dependent sequence subtraction to cancel interference from pilot superimposition.

Result: Achieves 50% reduction in overhead compared to conventional methods. Theoretical MSE analysis shows the proposed CE is unaffected by interference from pilot superimposition, validating the effectiveness of spectral interference alignment.

Conclusion: The proposed low-overhead channel estimation scheme for FTN signaling successfully reduces overhead by 50% while maintaining estimation accuracy through pilot superimposition and spectral interference alignment techniques.
...
### [127] [NCR vs. Passive/Active RIS: How Much NCR Amplification is Required to Beat RIS?](https://arxiv.org/abs/2603.19999)
*Özlem Tuğfe Demir,Ozan Alp Topal,Cicek Cavdar,Emil Björnson*

Main category: eess.SP

TL;DR: The paper analyzes the SNR tradeoff between RIS and NCR technologies in wireless networks, showing RIS SNR grows unbounded with elements while NCR is limited by noise amplification, but NCR can outperform RIS when deployed near UE with sufficient amplification.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental performance tradeoffs between reconfigurable intelligent surfaces (RIS) and network-controlled repeaters (NCR) in future wireless networks, providing clear design guidelines for practical integration.

Method: Derived closed-form SNR expressions for passive RIS, active RIS, and NCR-assisted communication under line-of-sight propagation. Analyzed both narrowband and wideband transmissions with/without direct BS-UE link, using realistic path loss models.

Result: RIS SNR grows unbounded with number of elements, while NCR SNR is fundamentally limited by UE-repeater channel due to noise amplification. However, NCRs can outperform both passive and active RISs when deployed close to UE with sufficient amplification.

Conclusion: The analysis provides clear design guidelines for integrating RIS and NCR technologies in future wireless networks, quantifying required amplification levels for NCRs to outperform RISs across different deployment scenarios.
...
### [128] [Performance Analysis and Optimization of FAS-ARIS Communications for 6G: System Modeling and Analytical Insights](https://arxiv.org/abs/2603.20011)
*Hong-Bae Jeon,Kai-Kit Wong,Chan-Byoung Chae*

Main category: eess.SP

TL;DR: Unified framework for FAS-ARIS communications in 6G combining fluid antenna port reconfigurability with ARIS signal amplification for enhanced link reliability and spectral efficiency.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of passive RIS solutions by combining FAS port reconfigurability with ARIS signal amplification for more flexible propagation environment control and enhanced link reliability in 6G communications.

Method: 1) Derive optimal ARIS amplification gain under power constraint to maximize SNR; 2) Use block-diagonal matrix approximation for tractable outage analysis; 3) Establish monotonic relationship between outage and effective channel gain for closed-form ARIS phase optimization; 4) Propose region-partitioned throughput optimization framework for near-optimal performance without exhaustive search.

Result: Achieves consistent gains in outage and throughput compared to baselines, with accurate analytical results confirmed by extensive simulations. The proposed framework offers low computational complexity while achieving near-optimal performance.

Conclusion: The unified FAS-ARIS framework enables superior 6G communications by combining port reconfigurability with signal amplification, offering flexible propagation control, enhanced reliability, and improved spectral efficiency with practical implementation feasibility.
...
### [129] [Structured Latent Dynamics in Wireless CSI via Homomorphic World Models](https://arxiv.org/abs/2603.20048)
*Salmane Naoumi,Mehdi Bennis,Marwa Chafii*

Main category: eess.SP

TL;DR: Self-supervised framework learns predictive representations of wireless channels using JEPA and Lie algebra-based transitions for structured latent space.


<details>
  <summary>Details</summary>
Motivation: Need to model temporal evolution of wireless channel state information (CSI) in a compact, structured latent space that preserves geometric properties for downstream applications like mobility-aware scheduling and localization.

Method: Uses Joint Embedding Predictive Architecture (JEPA) for action-conditioned latent dynamics, parameterizes transitions with homomorphic updates from Lie algebra to promote geometric consistency and compositionality in the latent space.

Result: Outperforms baselines on DICHASUS dataset in preserving topology and forecasting future embeddings across unseen environments, enables metrically faithful channel charts.

Conclusion: The framework provides scalable foundation for wireless applications including mobility-aware scheduling, localization, and wireless scene understanding through structured latent representations.
...
### [130] [TuLaBM: Tumor-Biased Latent Bridge Matching for Contrast-Enhanced MRI Synthesis](https://arxiv.org/abs/2603.19386)
*Atharva Rege,Adinath Madhavrao Dukre,Numan Balci,Dwarikanath Mahapatra,Imran Razzak*

Main category: eess.IV

TL;DR: TuLaBM is a novel method for synthesizing contrast-enhanced MRI from non-contrast MRI using latent space Brownian bridge transport with tumor-biased attention, achieving superior performance and faster inference than existing approaches.


<details>
  <summary>Details</summary>
Motivation: CE-MRI requires gadolinium contrast agents that increase costs and safety risks. Existing GAN-based methods suffer from instability and mode collapse, while diffusion models are computationally expensive and often fail to accurately reproduce tumor contrast patterns.

Method: TuLaBM formulates NC-to-CE MRI translation as Brownian bridge transport between source and target distributions in a learned latent space. It introduces Tumor-Biased Attention Mechanism (TuBAM) to amplify tumor-relevant features during bridge evolution and a boundary-aware loss to improve tumor margin sharpness.

Result: TuLaBM outperforms state-of-the-art baselines on both whole-image and tumor-region metrics across BraTS2023-GLI and Cleveland Clinic liver MRI datasets. It generalizes effectively to unseen liver MRI data in zero-shot and fine-tuned settings, with inference times under 0.097 seconds per image.

Conclusion: TuLaBM provides an efficient, high-fidelity solution for CE-MRI synthesis that addresses computational cost and tumor-region accuracy limitations of previous methods, offering practical clinical utility for brain and liver tumor assessment.
...
### [131] [Offshore oil and gas platform dynamics in the North Sea, Gulf of Mexico, and Persian Gulf: Exploiting the Sentinel-1 archive](https://arxiv.org/abs/2603.19801)
*Robin Spanier,Thorsten Hoeser,John Truckenbrodt,Felix Bachofer,Claudia Kuenzer*

Main category: eess.IV

TL;DR: Automated detection of offshore oil/gas platforms using Sentinel-1 satellite data and deep learning, creating quarterly time series (2017-2025) for three major regions, tracking installation/decommissioning dynamics.


<details>
  <summary>Details</summary>
Motivation: Marine spaces are increasingly used for offshore infrastructure but remain difficult to monitor systematically due to inaccessibility and spatial extent, creating need for consistent, scalable monitoring solutions.

Method: Leveraged Sentinel-1 archive data and deep learning-based object detection to create automated spatiotemporal detection of offshore platforms, with additional derivation of platform size, water depth, distance to coast, national affiliation, and installation/decommissioning dates.

Result: Identified 3,728 offshore platforms in 2025 across three regions: North Sea (356), Gulf of Mexico (1,641), Persian Gulf (1,731). Observed regional trends: Persian Gulf expansion until 2024, while Gulf of Mexico and North Sea declined from 2018-2020. Documented high dynamics with over 2,700 platforms installed/relocated and comparable number decommissioned/relocated.

Conclusion: Demonstrates potential of freely available Earth observation data and deep learning for consistent long-term marine infrastructure monitoring. Public dataset supports offshore monitoring, maritime planning, and analysis of offshore energy sector transformation, revealing structural shift toward mobile offshore units with shorter lifespans.
...
### [132] [ReconMIL: Synergizing Latent Space Reconstruction with Bi-Stream Mamba for Whole Slide Image Analysis](https://arxiv.org/abs/2603.19925)
*Lubin Gan,Jing Zhang,Heng Zhang,Xin Di,Zhifeng Wang,Wenke Huang,Xiaoyan Sun*

Main category: eess.IV

TL;DR: ReconMIL is a novel WSI analysis framework that addresses domain gap issues and over-smoothing in MIL by introducing task-specific feature adaptation and bi-stream global-local fusion.


<details>
  <summary>Details</summary>
Motivation: Current WSI analysis methods using MIL suffer from two key issues: 1) frozen task-agnostic features create domain gaps with histological tasks, leading to suboptimal separability, and 2) global aggregators cause over-smoothing where critical diagnostic signals are overshadowed by background context.

Method: ReconMIL introduces: 1) Latent Space Reconstruction module that adaptively projects generic features into task-specific manifolds; 2) Bi-stream architecture combining Mamba-based global stream for contextual priors and CNN-based local stream for morphological anomalies; 3) Scale-adaptive selection mechanism that dynamically fuses the two streams.

Result: ReconMIL consistently outperforms current state-of-the-art methods across multiple diagnostic and survival prediction benchmarks, effectively localizing fine-grained diagnostic regions while suppressing background noise.

Conclusion: ReconMIL successfully bridges the domain gap in WSI analysis and balances global-local feature aggregation, demonstrating superior diagnostic region localization through effective integration of global structure and local granularity.
...
### [133] [Goal-Oriented Framework for Optical Flow-based Multi-User Multi-Task Video Transmission](https://arxiv.org/abs/2603.19995)
*Yujie Xu,Shutong Chen,Nan Li,Yansha Deng,Jinhong Yuan,Robert Schober*

Main category: eess.IV

TL;DR: OF-GSC: Goal-oriented semantic communication framework for optical flow-based multi-user multi-task video transmission with semantic encoder/decoder and DDPG-based bandwidth allocation.


<details>
  <summary>Details</summary>
Motivation: Need to reduce transmission burden and save communication resources for efficient multi-user multi-task video transmission in wireless communication systems.

Method: 1) Semantic encoder with motion extractor and patch-level optical flow-based semantic representation extractor; 2) Transformer-based semantic decoder for video reconstruction and classification; 3) DDPG-based bandwidth allocation algorithm for multi-user transmission.

Result: 1) 13.47% SSIM improvement over DeepJSCC for video reconstruction; 2) Slightly surpasses VideoMAE Top-1 accuracy with only 25% data under same mask ratio; 3) 25.97% reduction in maximum transmission time compared to equal-bandwidth allocation.

Conclusion: OF-GSC framework effectively improves video transmission efficiency and quality through semantic communication and intelligent bandwidth allocation for multi-user multi-task scenarios.
...
### [134] [Investigating a Policy-Based Formulation for Endoscopic Camera Pose Recovery](https://arxiv.org/abs/2603.20045)
...
TL;DR: A policy-based approach for endoscopic camera pose recovery that predicts short-horizon relative motions without explicit geometric representation, showing improved robustness in low-texture conditions compared to traditional geometry-based methods.


<details>
  <summary>Details</summary>
Motivation: Current vision-based navigation systems for endoscopic surgery rely on feature matching and geometric optimization, which degrade under challenging endoscopic conditions like low texture and rapid illumination changes. Surgeons succeed by reasoning about evolving visual appearance in context, suggesting a need for alternative approaches that better mimic expert reasoning.

Method: Proposes a policy-based formulation that imitates experts by predicting short-horizon relative camera motions conditioned on previous camera state, without maintaining explicit geometric representations at inference time. This avoids brittle correspondence matching and reconstruction failures common in geometry-based approaches.

Result: On cadaveric sinus endoscopy, the approach achieves lowest mean translation error and competitive rotational accuracy under oracle state conditioning compared to geometric baselines. Shows reduced sensitivity to low-texture conditions and maintains robustness across varying illumination changes.

Conclusion: A learned motion policy offers a viable alternative formulation for endoscopic camera pose recovery, addressing key limitations of geometry-based approaches while better mimicking the reasoning process surgeons use during endoscopic procedures.
...
TL;DR: SAT encodings for planning that keep actions lifted while partially grounding predicates, achieving linear scaling with plan length instead of quadratic.


<details>
  <summary>Details</summary>
Motivation: Classical planning faces a trade-off: lifted representations are compact but planners often ground them, causing exponential blowup. Recent lifted approaches avoid grounding but have limitations. The paper seeks a middle ground between fully lifted and fully grounded planning.

Method: Introduces three SAT encodings that maintain actions at lifted level while partially grounding predicates. Unlike previous quadratic-scaling SAT encodings, these scale linearly with plan length.

Result: The best encoding outperforms state-of-the-art methods in length-optimal planning on hard-to-ground domains, demonstrating better performance for longer plans.

Conclusion: Partial grounding of predicates while keeping actions lifted provides an effective middle ground, enabling linear-scaling SAT encodings that excel on challenging planning domains.
...
### [136] [Hyperagents](https://arxiv.org/abs/2603.19461)
*Jenny Zhang,Bingchen Zhao,Wannan Yang,Jakob Foerster,Jeff Clune,Minqi Jiang,Sam Devlin,Tatiana Shavrina*

Main category: cs.AI

TL;DR: DGM-Hyperagents enable open-ended self-improvement by integrating task-solving and meta-modification into a single editable program, allowing agents to improve both their task performance and their own improvement mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing self-improving AI systems rely on fixed meta-level mechanisms that limit improvement speed. While Darwin Gödel Machine (DGM) shows open-ended self-improvement in coding, this alignment between task performance and self-modification skill doesn't generalize to other domains.

Method: Introduces hyperagents that integrate task agent and meta agent into a single editable program. The meta-level modification procedure is itself editable, enabling metacognitive self-modification. Extends DGM to create DGM-Hyperagents (DGM-H), eliminating domain-specific alignment assumptions.

Result: DGM-H improves performance over time across diverse domains, outperforming baselines without self-improvement or open-ended exploration, as well as prior self-improving systems. Meta-level improvements (e.g., persistent memory, performance tracking) transfer across domains and accumulate across runs.

Conclusion: DGM-Hyperagents represent a step toward open-ended AI systems that continually improve both their solutions and their search for improvement mechanisms, potentially supporting self-accelerating progress on any computable task.
...
### [137] [Teaching an Agent to Sketch One Part at a Time](https://arxiv.org/abs/2603.19500)
...
<details>
  <summary>Details</summary>
Motivation: To enable interpretable, controllable, and locally editable text-to-vector sketch generation by producing sketches one part at a time with structured part-level control.

Method: Train multi-modal language model-based agents using multi-turn process-reward reinforcement learning after supervised fine-tuning. Use ControlSketch-Part dataset with automatic part-level annotations obtained through a novel segmentation and labeling pipeline that assigns semantic parts to vector sketches.

Result: The approach enables interpretable, controllable, and locally editable text-to-vector sketch generation by incorporating structured part-level data and providing agents with visual feedback throughout the generation process.

Conclusion: Structured part-level data combined with process-reward reinforcement learning enables effective part-by-part vector sketch generation with interpretability, controllability, and local editability.
...
### [138] [Learning to Disprove: Formal Counterexample Generation with Large Language Models](https://arxiv.org/abs/2603.19514)
*Zenan Li,Zhaoyu Li,Kaiyu Yang,Xiaoxing Ma,Zhendong Su*

Main category: cs.AI

TL;DR: Fine-tuning LLMs for formal counterexample generation in mathematics using symbolic mutation strategy and multi-reward expert iteration framework, validated on new benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current AI mathematical reasoning focuses only on proof construction while neglecting counterexample generation, which is equally important for mathematical reasoning. There's a gap in AI systems that can both propose counterexamples and produce formal proofs that can be automatically verified.

Method: 1) Formalize counterexample generation task requiring LLMs to propose candidates and produce formal proofs verifiable in Lean 4 theorem prover. 2) Introduce symbolic mutation strategy that synthesizes diverse training data by extracting theorems and discarding selected hypotheses. 3) Use multi-reward expert iteration framework combining curated datasets with mutation strategy for effective training.
...
Conclusion: The proposed approach successfully addresses the gap in AI mathematical reasoning by enabling LLMs to generate formal counterexamples with verifiable proofs, advancing both counterexample generation and theorem proving capabilities through systematic data synthesis and training frameworks.
...
### [139] [ItinBench: Benchmarking Planning Across Multiple Cognitive Dimensions with Large Language Models](https://arxiv.org/abs/2603.19515)
*Tianlong Wang,Pinqiao Wang,Weili Shi,Sheng li*

Main category: cs.AI

TL;DR: ItinBench is a new benchmark that integrates spatial reasoning (route optimization) with traditional verbal reasoning tasks in trip itinerary planning to comprehensively evaluate LLMs across multiple cognitive domains.


<details>
  <summary>Details</summary>
Motivation: Existing LLM evaluations focus on specific reasoning tasks in controlled environments, but real-world applications require handling multiple cognitive domains simultaneously. Travel planning serves as a good medium to integrate various reasoning tasks, but current benchmarks lack spatial reasoning components.

Method: Created ItinBench benchmark featuring spatial reasoning (route optimization) integrated with traditional verbal reasoning tasks in trip itinerary planning. Evaluated various LLMs including Llama 3.1 8B, Mistral Large, Gemini 1.5 Pro, and GPT family across these diverse tasks.

Result: LLMs struggle to maintain high and consistent performance when concurrently handling multiple cognitive dimensions. Performance varies significantly across different cognitive tasks within the same planning scenario.

Conclusion: By incorporating tasks from distinct human-level cognitive domains, ItinBench provides new insights for building more comprehensive reasoning testbeds that better reflect real-world challenges, highlighting the need for LLMs to improve multi-domain cognitive integration.
...
### [140] [PA2D-MORL: Pareto Ascent Directional Decomposition based Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2603.19579)
*Tianmeng Hu,Biao Luo*

Main category: cs.AI

TL;DR: PA2D-MORL: A novel multi-objective RL method using Pareto ascent directional decomposition to better approximate Pareto policy sets in complex continuous/high-dimensional tasks.


<details>
  <summary>Details</summary>
Motivation: Current MORL methods struggle to achieve high-quality approximations of Pareto policy sets in complex tasks with continuous or high-dimensional state-action spaces, creating a need for more effective decomposition and optimization approaches.

Method: Uses Pareto ascent direction to select scalarization weights and compute multi-objective policy gradient; employs evolutionary framework for selective optimization of multiple policies; applies Pareto adaptive fine-tuning to enhance frontier density and spread.
...
Conclusion: PA2D-MORL provides an effective solution for approximating Pareto policy sets in complex MORL problems through directional decomposition and evolutionary optimization with adaptive fine-tuning.
...
### [141] [PowerLens: Taming LLM Agents for Safe and Personalized Mobile Power Management](https://arxiv.org/abs/2603.19584)
*Xingyu Feng,Chang Sun,Yuzhu Wang,Zhangbing Zhou,Chengwen Luo,Zhuangzhuang Chen,Xiaomin Ouyang,Huanqi Yang*

Main category: cs.AI

TL;DR: PowerLens uses LLMs for personalized mobile power management on Android, achieving 38.8% energy savings through context-aware policy generation with safety constraints.


<details>
  <summary>Details</summary>
Motivation: Existing mobile power management systems use static rules or coarse heuristics that don't consider user activities and personal preferences, leading to suboptimal battery life management.

Method: PowerLens employs LLMs' commonsense reasoning to bridge semantic gaps between user activities and system parameters. It uses a multi-agent architecture with UI semantic recognition, generates policies across 18 device parameters, implements PDL-based constraint verification, and uses a two-tier memory system with confidence-based distillation for learning individual preferences from implicit feedback.

Result: Achieves 81.7% action accuracy and 38.8% energy saving over stock Android, outperforming rule-based and LLM baselines. System converges within 3-5 days, has high user satisfaction, strong safety guarantees, and consumes only 0.5% of daily battery capacity.

Conclusion: PowerLens demonstrates that LLMs can enable safe, personalized, and effective mobile power management through zero-shot, context-aware policy generation that adapts to individual preferences without explicit configuration.
...
### [142] [HyEvo: Self-Evolving Hybrid Agentic Workflows for Efficient Reasoning](https://arxiv.org/abs/2603.19639)
*Beibei Xu,Yutong Ye,Chuyun Shen,Yingbo Zhou,Cheng Chen,Mingsong Chen*

Main category: cs.AI

TL;DR: HyEvo is an automated workflow generation framework that combines LLMs for semantic reasoning with deterministic code nodes for rule-based execution, using evolutionary search to optimize workflow structure and reduce inference costs.
...
Result: HyEvo consistently outperforms existing methods across diverse reasoning and coding benchmarks, reducing inference cost by up to 19x and execution latency by up to 16x compared to state-of-the-art open-source baselines.

Conclusion: HyEvo addresses limitations of existing workflow generation methods by combining heterogeneous computation (LLMs + code) with evolutionary optimization, achieving superior performance while significantly reducing computational costs.
...
### [143] [A Subgoal-driven Framework for Improving Long-Horizon LLM Agents](https://arxiv.org/abs/2603.19685)
*Taiyi Wang,Sian Gooding,Florian Hartmann,Oriana Riva,Edward Grefenstette*

Main category: cs.AI

TL;DR: LLM-based agents struggle with long-horizon web navigation tasks. The paper proposes two solutions: a real-time planning framework using subgoal decomposition, and MiRA, an RL training framework with milestone-based rewards, achieving state-of-the-art performance on WebArena-Lite.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based agents have difficulty with long-horizon planning in web navigation due to: 1) losing track during online execution as new information arrives, and 2) sparse/delayed rewards during RL fine-tuning that prevent coherent reasoning over extended tasks.

Method: Two main contributions: 1) An agent framework using proprietary models for online planning through subgoal decomposition, and 2) MiRA (Milestoning your Reinforcement Learning Enhanced Agent), an RL training framework with dense, milestone-based reward signals.
...
### [144] [Stepwise: Neuro-Symbolic Proof Search for Automated Systems Verification](https://arxiv.org/abs/2603.19715)
*Baoding He,Zenan Li,Wei Sun,Yuan Yao,Taolue Chen,Xiaoxing Ma,Zhendong Su*

Main category: cs.AI

TL;DR: Neuro-symbolic framework combines LLMs with interactive theorem provers to automate proof generation, achieving 77.6% theorem proving on seL4 benchmark.


<details>
  <summary>Details</summary>
Motivation: Formal verification via interactive theorem proving is manual and doesn't scale well. LLMs show promise in mathematical reasoning but need better integration with verification tools.

Method: Best-first tree search over proof states with LLM queries for candidate steps. Fine-tunes LLMs on proof state-step pairs, uses ITP tools to repair rejected steps, filter/rank states, and discharge subgoals.

Result: Proves up to 77.6% of theorems on seL4 benchmark, surpassing previous LLM approaches and standalone Sledgehammer. Solves more multi-step proofs and shows strong generalization across benchmarks.

Conclusion: Neuro-symbolic approach enables scalable automated software verification by combining neural reasoning with symbolic verification tools.
...
### [145] [Embodied Science: Closing the Discovery Loop with Agentic Embodied AI](https://arxiv.org/abs/2603.19782)
...
<details>
  <summary>Details</summary>
Motivation: Current AI approaches to scientific discovery are misaligned with reality - they treat discovery as isolated predictions rather than continuous physical interaction. There's a gap between digital prediction and empirical validation that needs bridging.
...
Result: The paper presents a conceptual framework rather than experimental results. It offers a roadmap for autonomous discovery systems that tightly couple agentic reasoning with physical execution in life and chemical sciences.

Conclusion: Embodied science represents a paradigm shift from isolated predictions to continuous physical interaction, bridging the gap between computational reasoning and empirical validation through closed-loop systems.
...
### [146] [FormalEvolve: Neuro-Symbolic Evolutionary Search for Diverse and Prover-Effective Autoformalization](https://arxiv.org/abs/2603.19828)
*Haijian Lu,Wei Wang,Jing Liu*

Main category: cs.AI

TL;DR: FormalEvolve is a neuro-symbolic evolutionary framework for autoformalization that uses LLM-driven mutation/crossover with AST rewrites to generate diverse, semantically consistent formalizations, improving both semantic hit rates and downstream proving performance under budget constraints.


<details>
  <summary>Details</summary>
Motivation: Current autoformalization approaches focus on semantic consistency but ignore prover effectiveness - even semantically correct formalizations can vary significantly in proof-search cost and success rates. There's a need for methods that generate formalizations that are not just semantically consistent but also effective for automated provers.

Method: FormalEvolve treats autoformalization as budgeted test-time search for semantically consistent repertoires. It uses a compilation-gated neuro-symbolic evolutionary framework with: 1) LLM-driven mutation and crossover with bounded patch repair, 2) symbolic AST rewrite operations for structural diversity, and 3) compilation gates to filter candidates.

Result: On CombiBench and ProofNet with T=100 generator-call budget: achieves SH@100 of 58.0% and 84.9%, reduces cross-problem concentration of semantic successes (lower Gini coefficient). Also improves downstream proving performance on CombiBench under fixed prover budget.

Conclusion: FormalEvolve successfully addresses the gap between semantic consistency and prover effectiveness in autoformalization by generating diverse formalizations through evolutionary search, leading to better semantic coverage and improved proving performance within computational budgets.
...
### [147] [Utility-Guided Agent Orchestration for Efficient LLM Tool Use](https://arxiv.org/abs/2603.19896)
*Boyan Liu,Gongming Zhao,Hongli Xu*

Main category: cs.AI

TL;DR: A utility-guided orchestration policy for LLM agents that balances answer quality and execution cost by making explicit decisions about actions (respond, retrieve, tool call, verify, stop) based on estimated gain, step cost, uncertainty, and redundancy.


<details>
  <summary>Details</summary>
Motivation: LLM agents face a tension between answer quality and execution cost. Fixed workflows are inflexible while free-form methods like ReAct improve performance but at high cost (excessive tool calls, longer trajectories, higher token consumption, latency). Current approaches leave orchestration to prompt-level behavior rather than treating it as an explicit decision problem.

Method: Proposes a utility-guided orchestration policy that explicitly selects among actions (respond, retrieve, tool call, verify, stop) by balancing estimated gain, step cost, uncertainty, and redundancy. Treats agent orchestration as a decision problem rather than leaving it to prompt-level behavior.

Result: Experiments across direct answering, threshold control, fixed workflows, ReAct, and policy variants show that explicit orchestration signals substantially affect agent behavior. Additional analyses demonstrate that lightweight utility design provides defensible and practical mechanism for agent control.

Conclusion: The paper provides a controllable and analyzable policy framework for studying quality-cost trade-offs in tool-using LLM agents, offering a practical mechanism for agent control through explicit orchestration decisions rather than implicit prompt behavior.
...
### [148] [On the Ability of Transformers to Verify Plans](https://arxiv.org/abs/2603.19954)
*Yash Sarrof,Yupei Du,Katharina Stein,Alexander Koller,Sylvie Thiébaux,Michael Hahn*

Main category: cs.AI

TL;DR: Transformers can provably learn to verify long plans in many classical planning domains, with theoretical guarantees for length generalization as vocabulary grows.


<details>
  <summary>Details</summary>
Motivation: Transformers show inconsistent success in AI planning tasks, and there's limited theoretical understanding of when generalization should be expected, especially when the number of objects (and thus vocabulary size) grows at test time.

Method: Introduce C*-RASP, an extension of C-RASP, to establish length generalization guarantees for transformers under simultaneous growth in sequence length and vocabulary size. Analyze decoder-only models' ability to verify whether given plans solve planning instances.
...
Conclusion: The paper provides theoretical foundations for understanding transformers' generalization capabilities in planning tasks, showing they can learn to verify plans with provable guarantees even as vocabulary size grows.
...
### [149] [Experience is the Best Teacher: Motivating Effective Exploration in Reinforcement Learning for LLMs](https://arxiv.org/abs/2603.20046)
...
<details>
  <summary>Details</summary>
Motivation: Current RL with rubric-based rewards for LLMs suffers from ineffective exploration confined to current policy distribution. The authors aim to steer policy toward ideal distribution that maximizes rewards by aligning exploration with desired target behaviors.

Method: HeRL treats failed trajectories along with unmet rubrics as hindsight experience, using it as in-context guidance for policy to explore desired responses beyond current distribution. Also introduces bonus reward to incentivize responses with greater improvement potential under such guidance.

Result: Extensive experiments across various benchmarks show HeRL achieves superior performance gains over baselines and can further benefit from experience guided self-improvement at test time.

Conclusion: HeRL facilitates effective learning from desired high quality samples without repeated trial-and-error from scratch, yielding more accurate estimation of expected gradient theoretically and improving LLM reasoning capabilities.
...
### [150] [DIAL-KG: Schema-Free Incremental Knowledge Graph Construction via Dynamic Schema Induction and Evolution-Intent Assessment](https://arxiv.org/abs/2603.20059)
*Weidong Bao,Yilin Wang,Ruyu Gao,Fangling Leng,Yubin Bao,Ge Yu*

Main category: cs.AI

TL;DR: DIAL KG is a closed-loop framework for incremental knowledge graph construction that addresses limitations of static methods by operating in a three-stage cycle with a Meta-Knowledge Base.


<details>
  <summary>Details</summary>
Motivation: Conventional KG construction methods are static, rely on single-step construction from fixed corpus with predefined schemas, making them suboptimal for real-world dynamic data scenarios where incorporating new information requires expensive complete reconstructions.

Method: Closed-loop framework with Meta-Knowledge Base operating in three-stage cycle: 1) Dual-Track Extraction (triple generation for simple knowledge, event extraction for complex knowledge), 2) Governance Adjudication (ensures fidelity and currency of facts), 3) Schema Evolution (induces new schemas from validated knowledge to guide subsequent cycles).

Result: Extensive experiments demonstrate state-of-the-art (SOTA) performance in both constructed graph quality and induced schemas.

Conclusion: DIAL KG provides an effective incremental KG construction framework that addresses dynamic data challenges, prevents knowledge staleness and hallucinations, and enables schema evolution for real-world applications.
...
### [151] [Pitfalls in Evaluating Interpretability Agents](https://arxiv.org/abs/2603.20101)
*Tal Haklay,Nikhil Prakash,Sana Pandey,Antonio Torralba,Aaron Mueller,Jacob Andreas,Tamar Rott Shaham,Yonatan Belinkov*

Main category: cs.AI

TL;DR: LLM-based automated interpretability systems need better evaluation methods beyond replication-based approaches, which have pitfalls like subjectivity, incomplete human explanations, and memorization risks.


<details>
  <summary>Details</summary>
Motivation: As automated interpretability systems using LLMs become more autonomous, there's a need to scale evaluation approaches to handle the volume and complexity of generated explanations, particularly in circuit analysis tasks.

Method: Built an agentic system where a research agent iteratively designs experiments and refines hypotheses for circuit analysis. Evaluated against human expert explanations across six tasks, then proposed unsupervised intrinsic evaluation based on functional interchangeability of model components.

Result: The system appeared competitive with human experts initially, but deeper examination revealed pitfalls of replication-based evaluation: human explanations can be subjective/incomplete, outcome comparisons obscure research process, and LLMs may reproduce findings via memorization.

Conclusion: Demonstrates fundamental challenges in evaluating complex automated interpretability systems and reveals key limitations of replication-based evaluation, highlighting the need for better evaluation frameworks.
...
### [152] [Learning Dynamic Belief Graphs for Theory-of-mind Reasoning](https://arxiv.org/abs/2603.20170)
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based ToM approaches either use direct prompting or static latent-state models, which produce incoherent mental models over time and weak reasoning in dynamic, high-stakes contexts like disaster response and emergency medicine.

Method: Introduces a structured cognitive trajectory model with: (1) projection from textual probabilistic statements to consistent PGM updates, (2) energy-based factor graph representation of belief interdependencies, and (3) ELBO-based objective capturing belief accumulation and delayed decisions.
...
### [153] [PhyGile: Physics-Prefix Guided Motion Generation for Agile General Humanoid Motion Tracking](https://arxiv.org/abs/2603.19305)
*Jiacheng Bao,Haoran Yang,Yucheng Xin,Junhong Liu,Yuecheng Xu,Han Liang,Pengfei Han,Xiaoguang Ma,Dong Wang,Bin Zhao*

Main category: cs.RO

TL;DR: PhyGile is a unified framework for physics-guided robot-native motion generation that eliminates retargeting artifacts and enables stable execution of complex whole-body motions on humanoid robots.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-motion generation models trained on human motion data produce motions that appear kinematically reasonable but often violate physical feasibility when retargeted to humanoid robots, causing generation-execution discrepancies.

Method: PhyGile performs physics-prefix-guided robot-native motion generation in a 262-dimensional skeletal space, eliminating inference-time retargeting. It uses a curriculum-based mixture-of-experts scheme for GMT controller training, followed by post-training on unlabeled motion data, and fine-tunes with physics-derived prefixes.

Result: Extensive experiments show PhyGile enables stable tracking of agile, highly difficult whole-body motions beyond walking and low-dynamic motions achieved by prior methods, expanding the frontier of text-driven humanoid control.

Conclusion: PhyGile successfully bridges the gap between motion generation and physical execution by integrating physics guidance directly into the generation process, enabling real-world execution of complex humanoid motions.
...
### [154] [VAMPO: Policy Optimization for Improving Visual Dynamics in Video Action Models](https://arxiv.org/abs/2603.19370)
...
TL;DR: VAMPO improves video action models for robot manipulation by optimizing visual dynamics through policy optimization of the denoising process, using expert-guided rewards in latent space.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based video predictors use likelihood-surrogate objectives that don't explicitly optimize for precise visual dynamics needed for manipulation, leading to subtle errors in object pose, spatial relations, and contact timing that get amplified by downstream policies.

Method: Formulates multi-step denoising as a sequential decision process and optimizes the denoising policy with rewards defined over expert visual dynamics in latent space. Introduces Euler Hybrid sampler that injects stochasticity only at the first denoising step for tractable low-variance policy-gradient estimation while preserving denoising trajectory coherence. Combines with GRPO and verifiable non-adversarial reward.
...
Conclusion: VAMPO provides an effective post-training framework that directly optimizes visual dynamics in video action models through policy optimization, addressing the objective mismatch between likelihood-based training and manipulation requirements.
...
### [155] [SOFTMAP: Sim2Real Soft Robot Forward Modeling via Topological Mesh Alignment and Physics Prior](https://arxiv.org/abs/2603.19384)
*Ziyong Ma,Uksang Yoo,Jonathan Francis,Weiming Zhi,Jeffrey Ichnowski,Jean Oh*

Main category: cs.RO

TL;DR: SOFTMAP: A sim-to-real learning framework for real-time 3D forward modeling of tendon-actuated soft finger manipulators that combines topological alignment, MLP forward modeling, residual correction, and linear calibration to address sim-to-real discrepancies.


<details>
  <summary>Details</summary>
Motivation: Soft robot manipulators offer advantages like compliance and safe human interaction, but accurate forward modeling from low-dimensional actuation commands is challenging due to nonlinear material phenomena (hysteresis, manufacturing variability) and sim-to-real gaps.

Method: Four-component framework: (1) ARAP-based topological alignment for consistent vertex space, (2) lightweight MLP forward model pretrained on simulation data, (3) residual correction network trained on real observations to compensate for discrepancies, (4) closed-form linear actuation calibration for real-time inference at 30 FPS.

Result: Achieved state-of-the-art shape prediction accuracy: Chamfer distance of 0.389 mm in simulation and 3.786 mm on hardware, millimeter-level fingertip trajectory tracking, and 36.5% improvement in teleoperation task success over baseline.

Conclusion: SOFTMAP provides a data-efficient approach for 3D forward modeling and control of soft manipulators, effectively bridging the sim-to-real gap while maintaining real-time performance.
...
### [156] [Speculative Policy Orchestration: A Latency-Resilient Framework for Cloud-Robotic Manipulation](https://arxiv.org/abs/2603.19418)
*Chanh Nguyen,Shutong Jin,Florian T. Pokorny,Erik Elmroth*

Main category: cs.RO

TL;DR: SPO is a cloud-edge framework that uses speculative pre-computation and buffering to enable real-time robotic control despite network latency, with safety verification and adaptive horizon scaling.


<details>
  <summary>Details</summary>
Motivation: Network latency and jitter in cloud robotics cause command starvation and unsafe physical execution for continuous manipulation tasks requiring high-frequency control.

Method: Speculative Policy Orchestration (SPO) uses cloud-hosted world model to pre-compute future waypoints, edge buffering, ε-tube verifier for safety bounds, and Adaptive Horizon Scaling for dynamic pre-fetch depth adjustment.

Result: SPO reduces network-induced idle time by over 60% compared to blocking remote inference, discards 60% fewer cloud predictions than static caching baselines, and enables fluid real-time control with bounded safety.

Conclusion: SPO successfully enables latency-resilient cloud-robotic control for continuous manipulation tasks while maintaining physical safety through speculative pre-computation and verification mechanisms.
...
### [157] [A Closed-Form CLF-CBF Controller for Whole-Body Continuum Soft Robot Collision Avoidance](https://arxiv.org/abs/2603.19424)
*Kiwan Wong,Maximillian Stölzle,Wei Xiao,Daniela Rus*

Main category: cs.RO

TL;DR: Closed-form CLF-CBF controller for real-time 3D obstacle avoidance in soft continuum manipulators, providing formal safety guarantees without online optimization.


<details>
  <summary>Details</summary>
Motivation: Soft continuum manipulators need active control for reliable collision avoidance, but existing approaches (sampling-based planning) are computationally expensive and lack formal safety guarantees, limiting real-time whole-body avoidance.

Method: Closed-form Control Lyapunov Function–Control Barrier Function (CLF–CBF) controller that analytically embeds safety constraints into control input, avoiding online optimization and ensuring stability/safety under modeling assumptions.

Result: Controller is 10× faster than standard CLF-CBF quadratic-programming approaches and 100× faster than traditional sampling-based planners. Simulation/hardware experiments on tendon-driven soft manipulator show accurate 3D trajectory tracking and robust obstacle avoidance in cluttered environments.

Conclusion: The framework provides scalable and provably safe control strategy for soft robots operating in dynamic, safety-critical settings, enabling real-time obstacle avoidance with formal safety guarantees.
...
### [158] [Can LLMs Prove Robotic Path Planning Optimality? A Benchmark for Research-Level Algorithm Verification](https://arxiv.org/abs/2603.19464)
*Zhengbang Yang,Md. Tasin Tazwar,Minghan Wei,Zhuangdi Zhu*

Main category: cs.RO

TL;DR: LLMs struggle with research-grade approximation-ratio proofs for robotic path planning algorithms, but task-specific lemmas significantly improve performance over generic prompting approaches.


<details>
  <summary>Details</summary>
Motivation: Robotic path planning problems are NP-hard, requiring approximation algorithms with provable guarantees. While LLMs show strong mathematical reasoning, their ability to handle research-level optimality proofs in this domain remains unexplored, despite the need for domain-specific geometric insights and complex multi-step reasoning.

Method: Created a benchmark of 34 research-grade proof tasks spanning diverse planning problem types and complexity levels. Evaluated state-of-the-art LLMs on these tasks, comparing different prompting strategies: generic chain-of-thought, supplying ground-truth approximation ratios, and providing task-specific in-context lemmas.

Result: Even the strongest LLMs struggle to produce fully valid proofs without external domain knowledge. However, providing task-specific lemmas substantially improves reasoning quality, outperforming generic chain-of-thought prompting and supplying ground-truth approximation ratios. Fine-grained error analysis identified common logical failures and hallucinations.

Conclusion: LLMs currently cannot independently produce valid research-level proofs for robotic path planning algorithms, but targeted context augmentation (specifically task-specific lemmas) significantly enhances their reasoning capabilities, suggesting a path toward LLM-assisted proof development in this domain.
...
### [159] [Real-Time Optical Communication Using Event-Based Vision with Moving Transmitters](https://arxiv.org/abs/2603.19477)
*Harmeet Dhillon,Pranay Katyal,Brendan Long,Rohan Walia,Matthew Cleaveland,Kevin Leahy*

Main category: cs.RO

TL;DR: Event camera-based optical communication system for multi-robot systems achieves 95% text decoding accuracy during motion with 7x faster processing than SOTA.


<details>
  <summary>Details</summary>
Motivation: Traditional RF communication in multi-robot systems faces contention and jamming issues. Optical communication offers an alternative but conventional cameras have limitations like low frame rates, motion blur, and poor performance in high dynamic range lighting.
...
Result: Achieved over 95% decoding accuracy for text transmission during motion. The system processes data 7x faster than previous state-of-the-art methods while maintaining equivalent tracking accuracy at transmitting frequencies ≥ 1 kHz.

Conclusion: Event cameras enable robust optical communication for multi-robot systems, overcoming RF limitations and conventional camera drawbacks through high-speed, accurate tracking and decoding during motion.
...
### [160] [Zero Shot Deformation Reconstruction for Soft Robots Using a Flexible Sensor Array and Cage Based 3D Gaussian Modeling](https://arxiv.org/abs/2603.19543)
*Linrui Shou,Zilang Chen,Wenjia Xu,Yiyue Luo,Tingyu Cheng*

Main category: cs.RO

TL;DR: Zero-shot tactile-only deformation reconstruction for unseen soft robots using cage-based 3D Gaussian model and piezoresistive sensors, requiring only static geometric proxy and no visual supervision.


<details>
  <summary>Details</summary>
Motivation: Current soft robot deformation reconstruction methods typically require visual supervision or object-specific training data, limiting practical deployment. The authors aim to enable camera-free, zero-shot deformation inference for unseen soft robots without retraining.

Method: Integrates flexible piezoresistive sensor array with cage-based 3D Gaussian deformation model. Uses graph attention network to map local tactile measurements to low-dimensional cage control signals, which propagate to dense Gaussian primitives for globally consistent shape deformations. Enforces spatial smoothness via boundary-aware propagation.

Result: Achieves 0.67 IoU, 0.65 SSIM, and 3.48 mm Chamfer distance. Successfully performs zero-shot deformation reconstruction of unseen soft robots in bending and twisting motions while rendering photorealistic RGB in real time.

Conclusion: The framework demonstrates strong zero-shot generalization through explicit coupling of tactile sensing and structured geometric deformation, enabling camera-free deformation inference for previously unseen soft robots without visual supervision or retraining.
...
### [161] [Evolving Embodied Intelligence: Graph Neural Network--Driven Co-Design of Morphology and Control in Soft Robotics](https://arxiv.org/abs/2603.19582)
...
<details>
  <summary>Details</summary>
Motivation: Traditional co-design of robot morphology and control faces challenges because morphological evolution disrupts learned control strategies, making it difficult to reuse or adapt existing knowledge when the body changes.

Method: Graph Neural Network approach where each robot is represented as a graph, with GAT encoding node features and MLP head for actuator commands. Uses topology-consistent inheritance: shared GAT layers reused, MLP hidden layers transferred, matched actuators copied, unmatched ones randomly initialized and fine-tuned.
...
Conclusion: Graph-structured policies provide a more effective interface between evolving morphologies and control for embodied intelligence, enabling better adaptation when robot bodies mutate during co-design optimization.
...
### [162] [CeRLP: A Cross-embodiment Robot Local Planning Framework for Visual Navigation](https://arxiv.org/abs/2603.19602)
...
<details>
  <summary>Details</summary>
Motivation: Visual navigation for cross-embodiment robots is challenging due to variations in robot and camera configurations. Previous approaches require massive datasets or fine-tuning, are data-intensive/time-consuming, and lack explicit consideration of robot geometry.

Method: CeRLP abstracts visual information into unified geometric formulation with two key components: 1) depth estimation scale correction using offline pre-calibration to resolve monocular depth scale ambiguity, and 2) visual-to-scan abstraction module that projects varying visual inputs into height-adaptive laser scans for policy robustness.
...
Conclusion: CeRLP provides a general visual navigation framework that successfully addresses cross-embodiment challenges through geometric abstraction, enabling application to heterogeneous robots without requiring massive datasets or extensive fine-tuning.
...
### [163] [ContractionPPO: Certified Reinforcement Learning via Differentiable Contraction Layers](https://arxiv.org/abs/2603.19632)
*Vrushabh Zinage,Narek Harutyunyan,Eric Verheyden,Fred Y. Hadaegh,Soon-Jo Chung*

Main category: cs.RO

TL;DR: ContractionPPO combines RL with contraction metrics to achieve certified robust control for legged robots, ensuring stability under perturbations while maintaining high performance.


<details>
  <summary>Details</summary>
Motivation: Legged locomotion in unstructured environments requires both high-performance control and formal robustness guarantees. Traditional methods need carefully designed reference trajectories, which are challenging for high-dimensional, contact-rich systems like quadrupeds. RL can learn policies directly but lacks formal stability guarantees.

Method: ContractionPPO augments PPO RL with a state-dependent contraction metric layer. The policy maximizes performance while producing a contraction metric that certifies incremental exponential stability. The metric is parameterized as a Lipschitz neural network and trained jointly with the policy, either in parallel or as an auxiliary head of the PPO backbone.

Result: The approach derives upper bounds on worst-case contraction rates that ensure learned metrics generalize from simulation to real-world deployment. Hardware experiments on quadruped locomotion demonstrate robust, certifiably stable control even under strong external perturbations.

Conclusion: ContractionPPO enables certified robust planning and control for legged robots by combining RL's performance advantages with formal contraction-based stability guarantees, bridging the gap between learning-based methods and formal verification.
...
### [164] [Accurate Open-Loop Control of a Soft Continuum Robot Through Visually Learned Latent Representations](https://arxiv.org/abs/2603.19655)
...
<details>
  <summary>Details</summary>
Motivation: To achieve reliable long-horizon open-loop control of soft continuum robots using interpretable latent dynamics learned from video data, eliminating the need for camera feedback during operation.

Method: Use Visual Oscillator Networks (VONs) with attention broadcast decoder (ABCD) to extract mechanistically interpretable 2D oscillator latents from video. Perform open-loop, single-shooting optimal control in latent space to track image-specified waypoints. Evaluate Koopman, MLP, and oscillator dynamics models with/without ABCD on a two-segment pneumatic SCR.
...
Conclusion: This is the first demonstration that interpretable, video-learned latent dynamics enable reliable long-horizon open-loop control of soft continuum robots, with ABCD-based models showing superior performance.
...
### [165] [Legged Autonomous Surface Science In Analogue Environments (LASSIE): Making Every Robotic Step Count in Planetary Exploration](https://arxiv.org/abs/2603.19661)
...
Method: Two novel approaches: (1) high-mobility legged robots that use direct surface interactions to collect terrain mechanical information, and (2) human-inspired data acquisition algorithms that enable robots to reason about scientific hypotheses and adapt exploration priorities based on ground-sensing measurements.

Result: Successfully verified through lab work and field deployments in planetary analog environments. Legged robots' soil mechanical property measurements enable effective traversal of challenging terrains and reveal key factors governing geologic environment formation when combined with other geologic properties.

Conclusion: The integrated human-robot system enables exploration of wider planetary environments and new substrate investigation opportunities, supporting maximum scientific return through more flexible and adaptive data collection decisions.
...
### [166] [Morphology-Consistent Humanoid Interaction through Robot-Centric Video Synthesis](https://arxiv.org/abs/2603.19709)
...
TL;DR: Dream2Act enables zero-shot humanoid robot interaction by using video generation models to create robot-centric motion from images, avoiding morphology gaps in retargeting and eliminating policy training.


<details>
  <summary>Details</summary>
Motivation: Current approaches for humanoid robot interaction either require extensive policy training (costly data collection) or human-to-robot motion retargeting (suffers from morphology gaps due to skeletal scale mismatches between humans and robots).

Method: Given a third-person image of robot and target object, use video generation models to synthesize robot completing the task with morphology-consistent motion. Extract physically feasible joint trajectories from synthesized videos using high-fidelity pose extraction, then execute via general-purpose whole-body controller.

Result: Achieved 37.5% overall success rate on Unitree G1 across four whole-body mobile interaction tasks (ball kicking, sofa sitting, bag punching, box hugging), compared to 0% for conventional retargeting. Maintains robot-consistent spatial alignment enabling reliable contact formation.

Conclusion: Dream2Act provides a robot-centric framework for zero-shot interaction that avoids retargeting errors and eliminates task-specific policy training by operating strictly within robot-native coordinate space through generative video synthesis.
...
### [167] [Generalized Task-Driven Design of Soft Robots via Reduced-Order FEM-based Surrogate Modeling](https://arxiv.org/abs/2603.19794)
*Yao Yao,David Howard,Perla Maiolino*

Main category: cs.RO

TL;DR: A unified FEM-based surrogate modeling pipeline for task-driven soft robot design that balances physical accuracy with computational efficiency, enabling scalable optimization across actuator designs and task scenarios.
...
Method: High-fidelity FEM simulations characterize actuator behavior at modular level → compact surrogate joint models constructed for pseudo-rigid body model (PRBM) → meta-model maps design parameters to surrogate representations → embedded into PRBM-based simulation environment for task-level simulation and optimization.

Result: Validated through sim-to-real transfer across multiple actuator types (bellow-type pneumatic actuators, tendon-driven soft finger) and two design studies: soft gripper co-design via RL and 3D actuator shape matching via evolutionary optimization.

Conclusion: The pipeline demonstrates high accuracy, efficiency, and reliable reuse, providing a scalable foundation for autonomous task-driven soft robot design.
...
TL;DR: Lightweight real-time framework projects 3D LiDAR to 2D BEV images for structural element detection, comparing classical methods (Hough, RANSAC, LSD) with YOLO-OBB deep learning, finding YOLO-OBB achieves best balance of robustness and efficiency on resource-constrained platforms.


<details>
  <summary>Details</summary>
Motivation: Need efficient structural perception for mapping/navigation on resource-constrained robots. Existing 3D methods are computationally prohibitive, while 2D geometric approaches lack robustness.

Method: Projects 3D LiDAR data into 2D Bird's-Eye-View images, evaluates feature extraction strategies (Hough Transform, RANSAC, LSD, YOLO-OBB), integrates detections through spatiotemporal fusion module for stability.

Result: Classical methods (Hough, LSD) are fast but noise-sensitive with fragmentation issues; RANSAC is robust but not real-time; YOLO-OBB achieves best balance - maintains 10 Hz operation on low-power SBC without GPU acceleration.

Conclusion: YOLO-OBB-based approach provides computationally efficient BEV-based perception pipeline enabling reliable real-time structural detection from 3D LiDAR on resource-constrained robotic platforms without GPU-intensive processing.
...
TL;DR: Novel hybrid motion planning method combining decentralized ADMM with centralized HOCBF for scalable, safe multi-agent control with real-time implementation on magnetic levitation platform.


<details>
  <summary>Details</summary>
Motivation: Classical centralized MPC becomes intractable for growing numbers of agents in multi-agent systems, requiring a scalable solution that maintains safety guarantees.

Method: Decentralized MPC framework combining decentralized ADMM (alternating direction method of multipliers) with centralized high-order control barrier function (HOCBF) architecture for safety guarantees.

Result: Significant improvement in scalability over classical centralized MPC, with efficacy and real-time capability validated through C++ implementation and deployment on real industrial magnetic levitation platform.

Conclusion: The hybrid approach successfully addresses scalability limitations while providing safety guarantees, demonstrating practical applicability through real-world implementation.
...
TL;DR: A hierarchical multi-agent architecture for onboard Earth Observation processing that enables rapid hazard detection using distributed AI agents on satellites, reducing computational overhead while maintaining decision quality.


<details>
  <summary>Details</summary>
Motivation: Current Earth Observation monitoring pipelines are ground-centric, causing latency due to downlink limitations, data fusion constraints, and exhaustive scene analysis costs. There's a need for faster, more efficient onboard processing for disaster response.

Method: Proposes a hierarchical multi-agent architecture with specialized AI agents deployed across distributed nodes (satellites). Uses an Early Warning agent for fast hypothesis generation, domain-specific analysis agents, and a Decision agent for evidence consolidation. Combines vision-language models, remote sensing tools, and role-specialized agents in an event-driven pipeline.

Result: Proof-of-concept implementation on an orbital edge-computing platform using representative satellite data. Experiments on wildfire and flood monitoring show the routing-based pipeline significantly reduces computational overhead while maintaining coherent decision outputs.

Conclusion: The distributed agent-based reasoning architecture demonstrates feasibility for future autonomous Earth Observation constellations, enabling rapid hazard identification under strict resource and bandwidth constraints.
...
TL;DR: A continuous-time RIO framework that jointly estimates spatial and temporal calibration between radar and IMU using B-splines, enabling robust odometry without scan matching or environment assumptions.


<details>
  <summary>Details</summary>
Motivation: Existing RIO approaches often assume known radar-IMU extrinsic calibration or require sufficient motion excitation for online estimation, while temporal misalignment between sensors is frequently neglected or treated separately from spatial calibration.

Method: Uses continuous-time modeling of inertial measurements with uniform cubic B-splines within a factor-graph optimization framework to jointly estimate spatial (extrinsic) and temporal calibration parameters, accurately capturing asynchronous radar-IMU measurements.

Result: The continuous-time representation enables reliable convergence of both temporal offset and extrinsic calibration parameters without relying on scan matching, target tracking, or environment-specific assumptions.

Conclusion: The proposed framework provides a robust RIO solution that handles challenging conditions by jointly addressing spatial and temporal calibration issues through continuous-time modeling, improving reliability in adverse environments.
...
TL;DR: GustPilot: A hierarchical wind-resilient navigation stack combining DRL-based velocity planning with geometric INDI control for robust quadrotor flight in windy conditions.


<details>
  <summary>Details</summary>
Motivation: Wind disturbances are a major challenge for lightweight quadrotor navigation, destabilizing both planning and tracking. Existing methods struggle with rapidly varying airflow, motivating a solution that combines intelligent planning with fast disturbance rejection.

Method: Two-level hierarchical approach: 1) DRL policy generates inertial-frame velocity references for gate traversal using wind-aware planning learned via fan-jet domain randomization, 2) Geometric INDI controller provides low-level tracking with incremental feedback on linear/angular acceleration rates using onboard sensors for fast wind disturbance rejection.

Result: Tested on 50g quad-copter: DRL-INDI achieves 94.7% success rate vs 55.0% for DRL-PID baseline, reduces tracking RMSE up to 50%, sustains 1.34 m/s speeds under 3.5 m/s wind disturbances. Generalizes from single-gate/single-fan training to complex environments (6 gates, 4 fans) without retraining.

Conclusion: Combining DRL-based velocity planning with structured INDI disturbance rejection provides a practical and generalizable approach to wind-resilient autonomous flight navigation, enabling robust performance in dynamic wind conditions.
...
<details>
  <summary>Details</summary>
Motivation: Current vectorized mapping models treat map estimation as deterministic, ignoring structural uncertainty. Existing probabilistic approaches use diagonal covariance matrices that fail to capture the strong spatial correlations inherent in road geometry.

Method: Structured probabilistic formulation for online map generation that explicitly models intra-element dependencies by predicting a dense covariance matrix parameterized via Low-Rank plus Diagonal (LRPD) decomposition. This captures global spatial structure through low-rank components and local noise through diagonal components.
...
Conclusion: The proposed uncertainty-aware framework demonstrates the critical role of modeling spatial correlations in map uncertainty for improving both map generation and downstream planning tasks like motion prediction.
...
TL;DR: Co-Ego system addresses viewpoint asymmetry in robotic guide dogs by fusing robot's ground-level sensing with user's elevated perspective to detect hazards like bent branches that are transparent to robots but dangerous at human height.


<details>
  <summary>Details</summary>
Motivation: Guide dogs are scarce for BLV individuals, and existing robotic guide dogs fail to detect obstacles that are transparent to robot sensors but dangerous at human body height (like bent branches), creating a viewpoint asymmetry problem.

Method: Dual-branch obstacle avoidance framework integrating robot-centric ground sensing with user's elevated egocentric perspective, deployed on a quadruped robot and evaluated in controlled user study with three conditions: unassisted, single-view, and cross-view fusion.

Result: Cross-view fusion significantly reduces collision times and cognitive load compared to unassisted and single-view conditions, demonstrating the necessity of viewpoint complementarity for safe navigation.

Conclusion: The Co-Ego system successfully addresses viewpoint asymmetry in robotic guide dogs by integrating both robot and user perspectives, providing comprehensive navigation safety that single-view systems cannot achieve.
...
TL;DR: KUKAloha is a low-cost shared-control teleoperation framework for construction robot arms that combines intuitive human guidance via a leader arm with autonomous perception for precise alignment, improving safety and efficiency.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for safe and efficient teleoperation of large-scale construction robot arms, where direct human control can be challenging and unsafe. Current approaches lack intuitive human-robot collaboration frameworks specifically designed for construction environments.

Method: The system uses a leader-follower paradigm with a lightweight leading arm for intuitive human guidance of coarse robot motion, combined with an autonomous perception module based on AprilTag detection for precise alignment and grasp execution. This decouples human control from fine manipulation.

Result: Experimental results show that KUKAloha reduces operator workload, improves task completion efficiency, and provides a practical solution for scalable demonstration collection and shared human-robot control in construction environments.

Conclusion: KUKAloha presents an effective shared-control teleoperation framework that enhances safety and repeatability for construction robot arms by separating human guidance from precise autonomous manipulation, making it suitable for practical deployment in construction settings.
...
Result: AGILE was validated across five representative humanoid skills (locomotion, recovery, motion imitation, loco-manipulation) on two hardware platforms (Unitree G1 and Booster T1), achieving consistent sim-to-real transfer.
...
TL;DR: HortiMulti is a multimodal, cross-season dataset collected in commercial strawberry and raspberry polytunnels to address the lack of representative real-world datasets for agricultural robotics in horticultural environments.


<details>
  <summary>Details</summary>
Motivation: Agricultural robotics needs representative datasets for complex autonomous tasks. While urban/forestry robotics have established benchmarks, horticultural environments remain under-explored despite economic significance. Current localization/perception algorithms degrade in polytunnel conditions (appearance variation, dynamic foliage, specular reflections, perceptual aliasing, GNSS-unreliability).

Method: Collected multimodal dataset across entire growing season in commercial strawberry/raspberry polytunnels. Sensor suite: two 3D LiDARs, four RGB cameras, IMU, GNSS, wheel odometry. Ground truth from Total Station surveying, AprilTag fiducial markers, and LiDAR-inertial odometry for dense/sparse/marker-free coverage.

Result: Released time-synchronized raw measurements, calibration files, reference trajectories, and baseline benchmarks for visual, LiDAR, and multi-sensor SLAM. Results show current state-of-the-art methods remain inadequate for reliable polytunnel deployment.

Conclusion: HortiMulti establishes a one-stop resource for developing/testing robotic perception systems in horticulture environments, addressing the dataset gap and enabling progress in agricultural robotics for this economically significant sector.
...
TL;DR: CRISP is an autonomous robot behavior framework that uses Vision-Language Models as social critics to generate and refine human-like social interactions without platform-specific APIs.


<details>
  <summary>Details</summary>
Motivation: Current robot social behavior generation lacks flexibility and autonomy, relying on predefined motions or human feedback, limiting cross-platform applicability and natural interaction capabilities.

Method: Five-step framework: 1) Extract joint constraints from robot description files, 2) Generate context-based behavior plans, 3) Create joint control code using visual ROM references, 4) VLM-based evaluation of social appropriateness, 5) Iterative refinement through reward-based search.

Result: In user studies with 5 robot types and 20 scenarios, CRISP achieved significantly higher preference and situational appropriateness ratings compared to previous methods across mobile manipulators and humanoids.

Conclusion: CRISP provides a general framework that minimizes human intervention while expanding autonomous interaction capabilities and cross-platform applicability through VLM-based self-critique and replanning.
...
TL;DR: IndoorR2X is a benchmark and simulation framework for LLM-driven multi-robot task planning that integrates observations from mobile robots and static IoT sensors to improve indoor scene understanding and coordination.


<details>
  <summary>Details</summary>
Motivation: Robot-to-robot communication alone is insufficient for overcoming partial observability in indoor environments without significant exploration overhead or large team sizes. Many indoor environments already have low-cost IoT sensors (like cameras) that provide persistent, building-wide context beyond what robots can perceive onboard.

Method: Introduces IndoorR2X, which integrates observations from both mobile robots and static IoT devices to construct a global semantic state. This supports scalable scene understanding, reduces redundant exploration, and enables high-level coordination through LLM-based planning. The framework provides configurable simulation environments, sensor layouts, robot teams, and task suites.

Result: Extensive experiments across diverse settings demonstrate that IoT-augmented world modeling improves multi-robot efficiency and reliability. The research highlights key insights and failure modes for advancing LLM-based collaboration between robot teams and indoor IoT sensors.

Conclusion: Integrating IoT sensors with robot teams through LLM-driven planning significantly enhances indoor scene understanding and coordination efficiency, providing a scalable solution that reduces exploration overhead while leveraging existing infrastructure.
...
### [180] [Beam-aware Kernelized Contextual Bandits for User Association and Beamforming in mmWave Vehicular Networks](https://arxiv.org/abs/2603.19285)
*Xiaoyang He,Manabu Tsukada*

Main category: cs.IT

TL;DR: BKC-UCB algorithm uses kernel methods and contextual bandits to estimate mmWave transmission rates without frequent channel measurements, leveraging vehicle context data and beam correlations.


<details>
  <summary>Details</summary>
Motivation: Frequent estimation of fast-fading mmWave channels for vehicle communications incurs significant overhead, creating a need for methods that can determine optimal base stations and beamforming vectors without constant channel measurements.

Method: Proposes Beam-aware Kernelized Contextual Upper Confidence Bound (BKC-UCB) algorithm that: 1) Uses kernel methods to map vehicle contexts (location, velocity) into RKHS for nonlinear relationship learning, 2) Embeds beam index into context to exploit beam correlations rather than treating beams as independent arms, 3) Incorporates event-triggered information sharing mechanism for efficient learning with limited communication overhead.

Result: The algorithm enables estimation of instantaneous transmission rates without additional channel measurements by exploiting historical contexts and past observed rates, accelerating convergence through beam correlation exploitation.

Conclusion: BKC-UCB provides an efficient solution for mmWave vehicle communications by reducing channel measurement overhead while maintaining accurate transmission rate estimation through contextual learning and intelligent information sharing.
...
### [181] [Wireless Broadcast Gossip for Decentralized Drone Swarms: Success Probability, Contraction, and Optimal Aloha](https://arxiv.org/abs/2603.19379)
*Ali Khalesi*

Main category: cs.IT

TL;DR: Analysis of broadcast gossip for drone swarms over interference-limited wireless, deriving SIR success probability, consensus rate bound, and optimal access probability.


<details>
  <summary>Details</summary>
Motivation: To understand and optimize decentralized consensus in drone swarms over interference-limited wireless channels, addressing challenges of wireless medium access and interference.

Method: Model drone locations as planar Poisson point process, use slotted Aloha for medium access, derive closed-form SIR success probability under Rayleigh fading, mean-square contraction bound, and optimal access probability.

Result: Derived (i) closed-form SIR success probability, (ii) consensus rate factorized into ideal mixing and wireless thinning terms, (iii) closed-form optimal access probability maximizing availability-reliability proxy.

Conclusion: Theoretical predictions match fastest convergence region in simulations, providing analytical framework for optimizing broadcast gossip in interference-limited drone swarm networks.
...
### [182] [Status Updating in Two-Way Delay Systems with Preemption](https://arxiv.org/abs/2603.19410)
*Jinxin Yang,Mohammad Moltafet,Hamid R. Sadjadpour*

Main category: cs.IT

TL;DR: Optimal request generation policy for minimizing AoI in a two-link status update system with geometric delays and preemption-in-waiting buffers.


<details>
  <summary>Details</summary>
Motivation: Need to optimize status update freshness in systems where controller requests updates from sampler, but both forward and reverse links experience random delays with limited buffers.

Method: Model as MDP with geometric service times, preemption-in-waiting buffers, and derive optimal stationary deterministic policy using relative value iteration algorithm.

Result: Proposed system outperforms baseline policies, optimal policy exhibits threshold-based structure, and algorithm convergence is proven.

Conclusion: Optimal request generation policy significantly improves AoI performance in two-link status update systems with random delays and limited buffers.
...
### [183] [Optimal Scalar Quantization for Matrix Multiplication: Closed-Form Density and Phase Transition](https://arxiv.org/abs/2603.19559)
*Calvin Ang,Sungyoon Kim,Mert Pilanci*

Main category: cs.IT

TL;DR: The paper analyzes optimal scalar quantization for matrix multiplication, deriving asymptotic error expansions, optimal quantization densities, and identifying a correlation-driven phase transition in quantization design.


<details>
  <summary>Details</summary>
Motivation: To minimize mean-squared error in quantized matrix multiplication by finding optimal scalar quantization strategies for entries of two matrices before multiplication, particularly in high-resolution regimes.

Method: Analyze entrywise scalar quantization of matrices A and B independently using K_X and K_Y quantization levels per entry, derive asymptotic expansions in high-resolution regime, identify optimal leading constants, characterize asymptotically optimal quantization center densities via conditional second moments, and specialize to correlated Gaussian multiplicative pairs.

Result: Derived sharp K^{-2} asymptotic expansion for MSE, identified exact optimal leading constants, obtained closed-form optimal point density for correlated Gaussian pairs showing correlation-driven phase transition (unimodal for |ρ|≤1/√3, bimodal for |ρ|>1/√3 with peaks at u_peak=±√(3-1/ρ^2)), and demonstrated applicability in matrix multiplication quantization, least squares optimization, and LLM key/query activation quantization.

Conclusion: The paper provides a comprehensive theoretical framework for optimal scalar quantization in matrix multiplication, revealing fundamental phase transition behavior driven by correlation, with practical applications in computational tasks and neural network quantization.
...
### [184] [New Constructions of Polar Code Based on Refined Error Probability Analysis](https://arxiv.org/abs/2603.19589)
*Hassan Noghrei,Murad Abdullah*

Main category: cs.IT

TL;DR: Novel LLR-based BLER analysis for polar codes under SC/SCL decoding, leading to optimized construction algorithms that outperform benchmark designs by up to 0.4 dB.


<details>
  <summary>Details</summary>
Motivation: To improve polar code performance by developing refined BLER analysis directly in terms of decoder LLRs, enabling optimized construction algorithms for both SC and SCL decoding.

Method: Derived novel LLR-based BLER expression for SC decoding, proposed SC-optimized construction algorithm, decomposed BLER into path loss/selection components for SCL, and developed SCL-optimized construction method.

Result: SC-optimized codes achieve up to 0.2 dB gain under DSCF decoding over AWGN channel; SCL-optimized codes achieve comparable or improved performance with gains up to 0.4 dB relative to benchmarks.

Conclusion: The LLR-based BLER analysis enables superior polar code constructions that outperform existing methods for both SC and SCL decoding across various channel conditions.
...
### [185] [On the Fundamental Limits of Hierarchical Secure Aggregation with Dropout and Collusion Resilience](https://arxiv.org/abs/2603.19705)
*Zhou Li,Yizhou Zhao,Xiang Zhang,Giuseppe Caire*

Main category: cs.IT

TL;DR: Secure aggregation in hierarchical networks with dropouts and collusion, characterizing communication limits via four-dimensional rate tuple and establishing feasibility conditions.


<details>
  <summary>Details</summary>
Motivation: To study fundamental communication limits for information-theoretic secure aggregation in hierarchical networks with user/relay dropouts and collusion threats, where server needs aggregate sum without learning individual inputs beyond colluding users.

Method: Introduce four-dimensional rate tuple capturing communication costs; use delayed message availability model; establish necessary/sufficient conditions for feasibility; characterize optimal first-round rates; characterize optimal user-to-relay rate; derive lower/upper bounds on relay-to-server rate; use vector linear coding with structured correlated randomness exhibiting MDS-like properties.

Result: Sharp threshold phenomenon: secure aggregation feasible iff total surviving users across surviving relays exceeds collusion threshold. Fully characterize optimal first-round rates, optimal user-to-relay rate, and provide bounds on relay-to-server rate (tight in certain regimes). Achievability via vector linear coding scheme with correlated randomness.

Conclusion: Established fundamental limits for secure aggregation in hierarchical networks with dropouts and collusion, providing complete characterization of feasibility conditions and optimal communication rates for first round and user-to-relay communication, with bounds for relay-to-server communication.
...
### [186] [Repeater-Aided Over-the-Air Phase Synchronization in Distributed MIMO](https://arxiv.org/abs/2603.19903)
*Unnikrishnan Kunnath Ganesan,Sai Subramanyam Thoota,Erik G. Larsson*

Main category: cs.IT

TL;DR: Novel over-the-air phase synchronization using repeater nodes when direct AP communication is infeasible in D-MIMO systems.


<details>
  <summary>Details</summary>
Motivation: Existing phase synchronization methods assume direct AP-to-AP communication, but this fails in scenarios where inter-AP signaling is too weak. Need to enable D-MIMO in challenging environments where current techniques don't work.

Method: Proposes a novel over-the-air calibration scheme using repeater nodes to facilitate phase synchronization when direct AP signaling is infeasible. Provides closed-form algorithm steps for phase calibration.

Result: The method enables coherent joint transmission (CJT) by APs and expands D-MIMO applicability to challenging environments where existing synchronization techniques fail.

Conclusion: The framework successfully addresses the limitation of direct AP communication assumptions, making D-MIMO systems viable in more challenging deployment scenarios through repeater-assisted phase synchronization.
...
### [187] [Robust Beam Codebooks for mmWave/THz Systems: Toward a Stochastic RL Approach](https://arxiv.org/abs/2603.19930)
*Anouar Nechi,Rainer Buchty,Mladen Berekovic,Saleh Mulhem*

Main category: cs.IT

TL;DR: RL-based adaptive beam codebook design for mmWave/THz massive MIMO systems that eliminates need for CSI and shows robustness to hardware impairments and NLoS conditions.


<details>
  <summary>Details</summary>
Motivation: Traditional predefined beamforming codebooks are suboptimal in NLoS conditions and for hardware-limited transceivers. RL enables adaptive, data-driven codebook design without explicit CSI, but robustness in practical conditions is underexplored.

Method: Proposes a robust multi-agent RL framework that learns beam codebooks directly from environmental feedback without prior channel knowledge. Analyzes three off-policy algorithms: DDPG, TD3, and SAC, evaluating their resilience to hardware impairments and feedback noise.

Result: SAC consistently outperforms deterministic methods (DDPG, TD3), achieving superior beamforming gains and stability in NLoS scenarios, even under severe hardware impairments.

Conclusion: RL-based codebook design shows promise for robust mmWave/THz massive MIMO systems, with SAC demonstrating best performance for adaptive beamforming in practical deployment conditions.
...
### [188] [Capacity-Achieving BBT Polar Codes with Interleaver-Assisted BP Decoding](https://arxiv.org/abs/2603.19938)
*Xinyuanmeng Yao,Xiao Ma*

Main category: cs.IT

TL;DR: BBT polar codes extend polar codes to arbitrary block lengths via binary balanced tree channel transformation, achieve capacity, and enable efficient finite-length analysis. Interleaved BBT polar codes with modified BP decoding improve convergence and reduce latency.


<details>
  <summary>Details</summary>
Motivation: To extend polar codes beyond power-of-two block lengths (Arıkan's limitation) and develop practical low-latency implementations with improved decoding performance.

Method: 1) Binary balanced tree (BBT) channel transformation for arbitrary block lengths; 2) Efficient weight spectrum estimation using hierarchical tree structure; 3) Interleaved BBT (IBBT) polar codes with modified BP decoding via normal-graph representation and interleavers; 4) Sub-normal-graph BP decoding with APP modules to reduce message-passing steps.

Result: 1) BBT transformation induces channel polarization and achieves BMS channel capacity; 2) Derived analytical FER bounds under ML decoding; 3) IBBT improves decoding convergence; 4) Sub-normal-graph BP reduces latency while maintaining error-rate performance.

Conclusion: BBT polar codes successfully generalize polar codes to arbitrary block lengths while maintaining capacity-achieving properties. The proposed interleaving and sub-normal-graph decoding techniques enable practical low-latency implementations with improved performance.
...
### [189] [Speculating Experts Accelerates Inference for Mixture-of-Experts](https://arxiv.org/abs/2603.19289)
*Vivan Madan,Prajwal Singhania,Abhinav Bhatele,Tom Goldstein,Ashwinee Panda*

Main category: cs.LG

TL;DR: MoE models suffer from CPU-GPU transfer bottlenecks when expert weights are offloaded. The paper proposes expert prefetching using internal model representations to predict future experts, enabling memory-computation overlap and reducing inference latency.


<details>
  <summary>Details</summary>
Motivation: MoE models require expert weights to be offloaded to CPU in memory-constrained inference settings, creating performance bottlenecks from CPU-GPU transfers during decoding.

Method: Proposes expert prefetching scheme that leverages current internal model representations to speculate future experts, enabling memory transfers to overlap with computation. Also examines lightweight estimators to improve expert prediction hit rates when speculative execution alone yields suboptimal accuracy.

Result: Future experts can be reliably predicted by internal representations across multiple MoE architectures. Executing speculated experts generally maintains downstream task accuracy. Achieves up to 14% reduction in time per output token (TPOT) over on-demand loading from CPU memory.

Conclusion: Expert prefetching using internal representations effectively reduces MoE inference latency in memory-constrained settings by enabling compute-memory overlap, with minimal accuracy degradation.
...
### [190] [A Visualization for Comparative Analysis of Regression Models](https://arxiv.org/abs/2603.19291)
*Nassime Mountasir,Baptiste Lafabregue,Bruno Albert,Nicolas Lachiche*

Main category: cs.LG

TL;DR: A novel visualization method for comparing regression models that plots residuals in 2D space using Mahalanobis distance and colormaps to reveal error patterns that traditional metrics miss.


<details>
  <summary>Details</summary>
Motivation: Traditional regression metrics like MAE, RMSE, and R² aggregate too much information and obscure important patterns in model performance. While useful for distinguishing between poorly and well-performing models, they don't provide detailed insights into error distributions and correlations between models.

Method: Three main contributions: (1) Plotting residuals in 2D space to simultaneously evaluate errors from two models, (2) Using Mahalanobis distance to account for data correlations and scale differences, and (3) Employing colormaps to visualize percentile-based error distributions, making dense regions and outliers easier to identify.

Result: The visualization approach provides a more detailed and comprehensive view of model performance compared to traditional aggregate metrics. It enables users to uncover patterns in error distributions and correlations that would otherwise be obscured.

Conclusion: The proposed visualization method facilitates deeper understanding of regression model performance differences and error distributions, enhancing the evaluation and comparison process beyond what traditional metrics can offer.
...
### [191] [Maximizing mutual information between user-contexts and responses improve LLM personalization with no additional data](https://arxiv.org/abs/2603.19294)
*Hyunji Nam,Haoran Li,Natasha Jaques*

Main category: cs.LG

TL;DR: MIPO is a self-supervised preference optimization method that constructs preference pairs by generating positive responses from correct prompts and negative responses from random prompts, enabling LLM self-improvement without external data or supervision.


<details>
  <summary>Details</summary>
Motivation: Current LLM improvement methods rely heavily on expensive human-labeled data or external verifiers, which limits scalability. True intelligence requires self-improvement capabilities beyond easily verifiable tasks, necessitating frameworks that work without external oversight.

Method: Mutual Information Preference Optimization (MIPO) uses contrastive data augmentation to construct preference pairs: positive responses generated from correct prompts, and negative responses generated from random, unrelated prompts. Using DPO to learn from these pairs maximizes pointwise conditional mutual information between prompts and model responses under the base LLM.

Result: MIPO achieves 3-40% improvements on personalization tasks using real-user datasets compared to strong baselines. Surprisingly, it also improves performance on math and multiple-choice problems by 1-18% without any additional data or human supervision.

Conclusion: MIPO demonstrates a promising self-improvement direction for LLMs that works without external oversight, enabling both personalization and general task improvement through mutual information maximization between prompts and responses.
...
### [192] [BrainSCL: Subtype-Guided Contrastive Learning for Brain Disorder Diagnosis](https://arxiv.org/abs/2603.19295)
*Xiaolong Li,Guiliang Guo,Guangqi Wen,Peng Cao,Jinzhu Yang,Honglin Wu,Xiaoli Liu,Fei Wang,Osmar R. Zaiane*

Main category: cs.LG

TL;DR: Proposes a subtype-guided contrastive learning framework that models patient heterogeneity as latent subtypes to improve discriminative representation learning for mental disorders.


<details>
  <summary>Details</summary>
Motivation: Mental disorder populations exhibit pronounced heterogeneity (significant differences between samples), which poses challenges for defining positive pairs in contrastive learning.

Method: Constructs multi-view representations combining clinical text with graph structure from BOLD signals, uses unsupervised spectral clustering to uncover latent subtypes, proposes dual-level attention mechanism to create subtype prototypes, and implements subtype-guided contrastive learning that pulls samples toward their subtype prototype graph.

Result: The approach outperforms state-of-the-art methods on Major Depressive Disorder (MDD), Bipolar Disorder (BD), and Autism Spectrum Disorders (ASD), demonstrating effectiveness of subtype prototype graphs in guiding contrastive learning.

Conclusion: The proposed subtype-guided contrastive learning framework successfully addresses patient heterogeneity in mental disorders by incorporating latent subtypes as structural priors, improving model performance through enhanced intra-subtype consistency.
...
### [193] [TTQ: Activation-Aware Test-Time Quantization to Accelerate LLM Inference On The Fly](https://arxiv.org/abs/2603.19296)
*Toshiaki Koike-Akino,Jing Liu,Ye Wang*

Main category: cs.LG

TL;DR: Test-time quantization (TTQ) framework that compresses large foundation models on-the-fly during inference to address domain shift issues in activation-aware compression methods.


<details>
  <summary>Details</summary>
Motivation: Existing activation-aware compression techniques for large foundation models rely heavily on calibration data, causing domain shift problems when applied to unseen downstream tasks. There's a need for quantization methods that can adapt to different tasks without requiring task-specific retraining.

Method: Proposes a test-time quantization (TTQ) framework that performs instant activation-aware quantization at inference time using efficient online calibration. The method adapts quantization parameters to each prompt dynamically, enabling compression without retraining while maintaining inference speedup.

Result: Experimental results show that TTQ improves quantization performance over state-of-the-art baselines, demonstrating its effectiveness in handling various downstream tasks without domain shift issues.

Conclusion: TTQ provides an effective solution for compressing large foundation models by enabling prompt-adaptive quantization at inference time, overcoming the domain shift limitations of traditional calibration-dependent methods while maintaining computational efficiency.
...
### [194] [CLaRE-ty Amid Chaos: Quantifying Representational Entanglement to Predict Ripple Effects in LLM Editing](https://arxiv.org/abs/2603.19297)
*Manit Baser,Alperen Yildiz,Dinil Mon Divakaran,Mohan Gurusamy*

Main category: cs.LG

TL;DR: CLaRE is a lightweight method that uses forward activations from a single layer to identify ripple effects in LLMs after factual edits, enabling systematic study of fact entanglement with better performance and efficiency than gradient-based methods.


<details>
  <summary>Details</summary>
Motivation: LLMs' static knowledge becomes outdated, and model-editing techniques cause unpredictable ripple effects that propagate through hidden representations. There's a need to efficiently identify where these unintended behavioral changes occur.

Method: CLaRE quantifies entanglement between facts using forward activations from a single intermediate layer, avoiding costly backward passes. It computes large-scale entanglement graphs from a corpus of 11,427 facts across three datasets.

Result: CLaRE achieves 62.2% improvement in Spearman correlation with ripple effects while being 2.74× faster and using 2.85× less peak GPU memory than baselines. It also requires minimal storage for fact representations.

Conclusion: CLaRE provides an efficient, lightweight approach to study fact entanglement in LLMs, enabling better preservation sets for model editing, audit trails, red-teaming, and scalable post-edit evaluation through large-scale entanglement graphs.
...
### [195] [A Dynamic Bayesian and Machine Learning Framework for Quantitative Evaluation and Prediction of Operator Situation Awareness in Nuclear Power Plants](https://arxiv.org/abs/2603.19298)
*Shuai Chen,Huiqiao Jia,Tao Qing,Li Zhang,Xingyu Xiao*

Main category: cs.LG

TL;DR: DBML SA is a dynamic Bayesian machine learning framework that fuses probabilistic reasoning with data-driven intelligence to achieve quantitative, interpretable, and predictive situation awareness modeling for nuclear control operators.


<details>
  <summary>Details</summary>
Motivation: Existing situation awareness assessment methods (SAGAT, SART) are static, retrospective, and detached from evolving cognitive dynamics, limiting their ability to capture operational risk in complex nuclear control environments.

Method: Developed a unified DBML SA framework combining Bayesian probabilistic reasoning with neural networks. Analyzed 212 operational event reports (2007-2021) to reconstruct causal temporal structure of 11 performance shaping factors across cognitive layers. Bayesian component enables time-evolving inference under uncertainty, while neural component establishes nonlinear predictive mapping from PSFs to SART scores.

Result: Achieved mean absolute percentage error of 13.8% with statistical consistency to subjective evaluations (p > 0.05). Identified training quality and stress dynamics as primary drivers of situation awareness degradation.

Conclusion: DBML SA transcends traditional questionnaire-based assessments by enabling real-time cognitive monitoring, sensitivity analysis, and early-warning prediction, paving the way for intelligent human-machine reliability management in next-generation digital control rooms.
...
### [196] [PRIME-CVD: A Parametrically Rendered Informatics Medical Environment for Education in Cardiovascular Risk Modelling](https://arxiv.org/abs/2603.19299)
*Nicholas I-Hsien Kuo,Marzia Hoque Tania,Blanca Gallego,Louisa Jorm*

Main category: cs.LG

TL;DR: PRIME-CVD introduces synthetic medical datasets for cardiovascular risk modeling education, generated from population statistics rather than real patient data to avoid privacy risks.


<details>
  <summary>Details</summary>
Motivation: Real patient EMR data is rarely available for teaching due to privacy and re-identification risks, limiting reproducibility and hands-on training in cardiovascular risk modeling.

Method: Created two synthetic datasets of 50,000 adults using a parametrically rendered informatics medical environment based on causal directed acyclic graphs parameterized with Australian population statistics and published epidemiologic effect estimates.

Result: Two openly accessible synthetic data assets: Data Asset 1 provides clean, analysis-ready cohort data; Data Asset 2 offers relational EMR-style database with realistic structural heterogeneity, enabling instruction in data cleaning, causal reasoning, and risk modeling without privacy risks.

Conclusion: PRIME-CVD enables scalable medical education and reproducible research in cardiovascular risk modeling while ensuring negligible disclosure risk through synthetic data generation, released under Creative Commons Attribution 4.0 license.
...
### [197] [A General Deep Learning Framework for Wireless Resource Allocation under Discrete Constraints](https://arxiv.org/abs/2603.19322)
*Yikun Wang,Yang Li,Yik-Chung Wu,Rui Zhang*

Main category: cs.LG

TL;DR: A novel DL framework for discrete wireless resource allocation using probabilistic modeling of support sets to overcome zero-gradient, constraint enforcement, and non-SPSD challenges.


<details>
  <summary>Details</summary>
Motivation: DL methods struggle with discrete variables due to zero-gradient in backpropagation, difficulty enforcing discrete constraints, and inability to generate non-SPSD solutions in wireless resource allocation problems.

Method: Introduces support set representation for discrete variables, models elements as random variables, learns joint probability distribution via conditional probability factorization with sequential learning, uses dynamic context embedding for evolving solutions.

Result: Outperforms existing baselines in both system performance and computational efficiency for joint user association/beamforming in cell-free systems and joint antenna positioning/beamforming in movable antenna systems.

Conclusion: The proposed probabilistic DL framework effectively addresses fundamental challenges in discrete wireless resource allocation, providing a general solution that avoids zero-gradient issues, enforces constraints via masking, and inherently supports non-SPSD property.
...
### [198] [Parameter-Efficient Token Embedding Editing for Clinical Class-Level Unlearning](https://arxiv.org/abs/2603.19302)
*Iyad Ait Hou,Shrenik Borad,Harsh Sharma,Pooja Srinivasan,Rebecca Hwa,Aya Zirikly*

Main category: cs.LG

TL;DR: STEU is a parameter-efficient machine unlearning method for clinical language models that updates only selected token embeddings and a small classifier head to forget specific classes while preserving model utility.


<details>
  <summary>Details</summary>
Motivation: Clinical language models need to remove sensitive information due to privacy regulations and institutional policies without full retraining, requiring a balance between effective forgetting and preservation of model utility.

Method: STEU (Sparse Token Embedding Unlearning) updates only PMI-selected token embeddings together with a small classifier head while keeping all encoder layers frozen, enabling behavioral class-level unlearning with minimal parameter modification.

Result: STEU achieves near-complete forgetting (forget F1 = 0.0004) while maintaining competitive retained utility (retain avg F1 = 0.4766) after modifying only 0.19% of model parameters on MIMIC-IV, with consistent performance across multiple clinical datasets and model architectures.

Conclusion: Targeted behavioral unlearning can be effectively achieved through sparse embedding edits without modifying deeper encoder representations, offering a parameter-efficient solution for clinical language model privacy compliance.
...
### [199] [A Mathematical Theory of Understanding](https://arxiv.org/abs/2603.19349)
*Bahar Taşkesen*

Main category: cs.LG

TL;DR: Paper models learning as communication bottleneck where instructional signals only work when learner has prerequisite concepts, creating structural and epistemic limits on learning speed.


<details>
  <summary>Details</summary>
Motivation: While generative AI has made information production cheap, the value depends on whether learners can absorb it. Signals only convey meaning to learners with structural capacity to decode them - an explanation that helps one user may be noise to another lacking prerequisites.

Method: Develops mathematical model of learner as "mind" with prerequisite structure over concepts. Teaching is sequential communication with latent target. Effective communication channel depends on learner's current knowledge state, becoming more informative as learning progresses.

Result: Identifies two limits on learning speed: structural limit (prerequisite reachability) and epistemic limit (uncertainty about target). Shows threshold effects in training - teaching horizon below prerequisite depth prevents successful completion. Common broadcast curriculum can be linearly slower than personalized instruction across heterogeneous learners.

Conclusion: Learner-side structural capacity creates fundamental bottlenecks in information absorption, explaining why cheap information production doesn't guarantee effective learning. Personalized instruction accounting for prerequisite structures is crucial for overcoming these bottlenecks.
...
TL;DR: KD-Brain is a prior-informed graph learning framework that uses semantic and clinical priors to guide subnetwork interaction modeling for mental disorder diagnosis, achieving SOTA performance with interpretable biomarkers.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based methods struggle to learn complex interactions among functional brain subnetworks due to limited training samples, making it challenging to model the underlying functional pathways crucial for mental disorder diagnosis.

Method: Proposes KD-Brain with two key components: 1) Semantic-Conditioned Interaction mechanism that injects semantic priors into attention queries to guide subnetwork interactions based on functional identities, and 2) Pathology-Consistent Constraint that regularizes model optimization by aligning learned interaction distributions with clinical priors.

Result: Achieves state-of-the-art performance on a wide range of disorder diagnosis tasks and identifies interpretable biomarkers that are consistent with psychiatric pathophysiology.

Conclusion: KD-Brain effectively addresses the challenge of learning subnetwork interactions with limited data by incorporating prior knowledge, leading to improved diagnostic performance and clinically meaningful biomarker discovery.
...
### [201] [Neural Uncertainty Principle: A Unified View of Adversarial Fragility and LLM Hallucination](https://arxiv.org/abs/2603.19562)
*Dong-Xiao Zhang,Hu Lou,Jun-Jie Zhang,Jun Zhu,Deyu Meng*

Main category: cs.LG

TL;DR: The paper reveals a common geometric origin between adversarial vulnerability in vision models and hallucination in LLMs, formalized as a Neural Uncertainty Principle (NUP) where input and loss gradient are conjugate observables with irreducible uncertainty bound.


<details>
  <summary>Details</summary>
Motivation: Conventional approaches treat adversarial vulnerability in vision and hallucination in LLMs as separate problems with modality-specific solutions. The authors aim to show they share a common geometric origin and provide a unified theoretical framework.

Method: Formalizes a Neural Uncertainty Principle (NUP) under loss-induced states, revealing input and loss gradient as conjugate observables with irreducible uncertainty bound. Designs a single-backward probe to measure input-gradient correlation. Proposes ConjMask (masking high-contribution components) and LogitReg (logit-side regularization) for robustness, and uses the probe as decoding-free risk signal for LLMs.

Result: NUP shows that in near-bound regimes, compression leads to increased sensitivity dispersion (adversarial fragility), while weak prompt-gradient coupling causes under-constrained generation (hallucination). The probe effectively detects hallucination risk before token generation and improves robustness without adversarial training.

Conclusion: NUP provides a unified framework connecting adversarial vulnerability and hallucination as manifestations of the same uncertainty-budget principle. It offers practical tools (ConjMask, LogitReg, probe-based detection) for diagnosing and mitigating boundary anomalies across perception and generation tasks.
...
### [202] [GT-Space: Enhancing Heterogeneous Collaborative Perception with Ground Truth Feature Space](https://arxiv.org/abs/2603.19308)
*Wentao Wang,Haoran Xu,Guang Tan*

Main category: cs.LG

TL;DR: GT-Space is a scalable collaborative perception framework for heterogeneous autonomous agents that creates a common feature space from ground-truth labels, requiring only single adapter modules per agent instead of pairwise alignment.


<details>
  <summary>Details</summary>
Motivation: Multi-agent collaborative perception in autonomous driving faces challenges with heterogeneous features from different sensing modalities or model architectures, making data fusion difficult. Existing approaches requiring retraining encoders or pairwise feature alignment are not scalable.

Method: GT-Space constructs a common feature space from ground-truth labels as a unified reference for feature alignment. Each agent only needs a single adapter module to project features into this space, eliminating pairwise interactions. A fusion network is trained with contrastive losses across diverse modality combinations.
...
Conclusion: GT-Space provides a flexible and scalable solution for heterogeneous collaborative perception that eliminates the need for pairwise feature alignment, making it practical for real-world applications with diverse agent configurations.
...
### [203] [MemReward: Graph-Based Experience Memory for LLM Reward Prediction with Limited Labels](https://arxiv.org/abs/2603.19310)
*Tianyang Luo,Tao Feng,Zhigang Hua,Yan Xie,Shuang Yang,Ge Liu,Jiaxuan You*

Main category: cs.LG

TL;DR: MemReward is a graph-based experience memory framework that uses GNNs to propagate rewards from limited labeled data to unlabeled LLM rollouts, achieving near-Oracle performance with only 20% of reward labels.


<details>
  <summary>Details</summary>
Motivation: Training LLMs via RL requires expensive reward labeling (human experts or verification), especially for complex reasoning tasks like math proofs and open-ended QA where ground truth is scarce. Limited reward labels constrain RL fine-tuning effectiveness.

Method: Initial LLM generates rollouts (thinking process + answer) stored as experience memory. Queries, thinking processes, and answers form nodes in a heterogeneous graph with similarity/structural edges. GNN trained on labeled nodes propagates rewards to unlabeled rollouts during online optimization.

Result: With only 20% labels, MemReward achieves 97.3% of Oracle performance on Qwen2.5-3B and 96.6% on 1.5B across math, QA, and code generation. Surpasses Oracle on out-of-domain tasks. Scales smoothly to 99.4% of Oracle at 70% labels.

Conclusion: MemReward effectively addresses reward scarcity in RL fine-tuning by leveraging graph-based experience memory and GNN reward propagation, enabling efficient training with limited labeled data while maintaining strong performance.
...
### [204] [GO-GenZip: Goal-Oriented Generative Sampling and Hybrid Compression](https://arxiv.org/abs/2603.20109)
*Pietro Talli,Qi Liao,Alessandro Lieto,Parijat Bhattacharjee,Federico Chiariotti,Andrea Zanella*

Main category: cs.LG

TL;DR: GenAI-driven framework for network telemetry that jointly optimizes what to observe and how to encode it, achieving over 50% reduction in sampling/data transfer while maintaining analytical fidelity.


<details>
  <summary>Details</summary>
Motivation: Current network telemetry pipelines are unsustainable due to massive streams of fine-grained KPIs from distributed sources to central aggregators, creating challenges in storage, transmission, and real-time analysis.

Method: Goal-oriented generative AI framework with adaptive sampling policies using adaptive masking techniques, combined with generative modeling to preserve critical features. Features hybrid compression combining traditional lossless coding with GenAI-driven lossy compression.
...
Conclusion: The framework redesigns network telemetry from a goal-oriented perspective, moving beyond passive compression to jointly optimize observation and encoding based on downstream task relevance.
...
### [205] [LeWorldModel: Stable End-to-End Joint-Embedding Predictive Architecture from Pixels](https://arxiv.org/abs/2603.19312)
*Lucas Maes,Quentin Le Lidec,Damien Scieur,Yann LeCun,Randall Balestriero*

Main category: cs.LG

TL;DR: LeWorldModel (LeWM) is a stable Joint Embedding Predictive Architecture that trains end-to-end from pixels with just two loss terms, enabling efficient world modeling and planning while encoding meaningful physical structure.


<details>
  <summary>Details</summary>
Motivation: Existing JEPA methods for learning world models are fragile and require complex multi-term losses, exponential moving averages, pre-trained encoders, or auxiliary supervision to avoid representation collapse. There's a need for a simpler, more stable approach that can train end-to-end from raw pixels.

Method: LeWM uses only two loss terms: a next-embedding prediction loss and a regularizer enforcing Gaussian-distributed latent embeddings. This reduces tunable loss hyperparameters from six to one compared to existing alternatives. The model has ~15M parameters and trains on a single GPU in hours.

Result: LeWM plans up to 48x faster than foundation-model-based world models while remaining competitive across diverse 2D and 3D control tasks. The latent space encodes meaningful physical structure, and surprise evaluation shows the model reliably detects physically implausible events.

Conclusion: LeWM demonstrates that stable end-to-end world modeling from pixels is achievable with minimal loss terms, offering efficient planning and meaningful physical representation learning without complex regularization schemes.
...
### [206] [DPxFin: Adaptive Differential Privacy for Anti-Money Laundering Detection via Reputation-Weighted Federated Learning](https://arxiv.org/abs/2603.19314)
*Renuga Kanagavelu,Manjil Nepal,Ning Peiyan,Cai Kangning,Xu Jiming,Fei Gao,Yong Liu,Goh Siow Mong Rick,Qingsong Wei*

Main category: cs.LG

TL;DR: DPxFin: A federated learning framework with reputation-guided adaptive differential privacy for anti-money laundering, balancing privacy and accuracy better than traditional FL and fixed-noise DP.


<details>
  <summary>Details</summary>
Motivation: Combating money laundering is challenging due to data privacy concerns and complex fraud patterns. Federated learning helps but is prone to privacy leakage in tabular financial data.

Method: DPxFin integrates reputation-guided adaptive differential privacy. Client reputation is computed based on alignment between local and global models, then dynamic DP noise is assigned: lower noise for high-reputation clients, stronger noise for low-reputation clients.

Result: DPxFin achieves better accuracy-privacy trade-off than traditional FL and fixed-noise DP baselines on AML dataset under both IID and non-IID settings. It withstands tabular data leakage attacks and shows consistent performance improvements.

Conclusion: DPxFin effectively addresses privacy concerns in federated learning for financial applications, providing adaptive privacy protection while maintaining model utility, making it suitable for real-world anti-money laundering scenarios.
...
### [207] [Stochastic Sequential Decision Making over Expanding Networks with Graph Filtering](https://arxiv.org/abs/2603.19501)
*Zhan Gao,Bishwadeep Das,Elvin Isufi*

Main category: cs.LG

TL;DR: The paper proposes a stochastic sequential decision-making framework for filtering networked data on expanding graphs, using multi-agent reinforcement learning to adapt filters to graph evolution and account for long-term impacts.


<details>
  <summary>Details</summary>
Motivation: Existing graph filters mainly study fixed graphs, ignoring that real-world graphs often expand as nodes continually attach with unknown patterns. Current approaches rely on either pre-designed filters or online learning, which only consider past or present information without accounting for future impacts of filtering decisions.

Method: The authors propose a stochastic sequential decision-making framework that models filter shifts as agents, creating a multi-agent system trained with multi-agent reinforcement learning. They develop a context-aware graph neural network to parameterize the policy, which tunes filter parameters based on both graph and agent information.

Result: Experiments on synthetic and real datasets (including cold-start recommendation and COVID prediction) demonstrate that the sequential decision-making approach outperforms batch and online filtering alternatives.

Conclusion: The proposed framework effectively handles expanding graphs by accounting for long-term rewards and capturing expansion dynamics through sequential decision-making, offering significant benefits over traditional filtering approaches.
...
### [208] [MSNet and LS-Net: Scalable Multi-Scale Multi-Representation Networks for Time Series Classification](https://arxiv.org/abs/2603.19315)
*Celal Alagöz,Mehmet Kurnaz,Farhan Aadil*

Main category: cs.LG

TL;DR: Proposes scalable multi-scale convolutional frameworks (MSNet, LS-Net, LiteMV) that integrate multiple input representations for univariate time series classification, achieving state-of-the-art performance across accuracy, calibration, and efficiency metrics.


<details>
  <summary>Details</summary>
Motivation: Time series classification performance depends not only on architectural design but also on the diversity of input representations. The paper aims to develop scalable frameworks that systematically integrate structured multi-representation inputs to improve TSC performance across different metrics.

Method: Introduces three architectures: 1) MSNet - hierarchical multi-scale convolutional network optimized for robustness and calibration, 2) LS-Net - lightweight variant for efficiency-aware deployment, and 3) LiteMV - adapted from multivariate inputs to operate on multi-representation univariate signals for cross-representation interaction. All models are evaluated across 142 benchmark datasets under unified experimental protocol.

Result: LiteMV achieves highest mean accuracy, MSNet provides superior probabilistic calibration (lowest NLL), and LS-Net offers best efficiency-accuracy tradeoff. Critical Difference analysis confirms statistically significant performance differences among top models. Pareto analysis shows multi-representation multi-scale modeling yields flexible design space tunable for different settings.

Conclusion: Scalable multi-representation multi-scale learning establishes a principled and practical direction for modern time series classification, offering flexible solutions for accuracy-oriented, calibration-oriented, or resource-constrained settings.
...
### [209] [Ternary Gamma Semirings: From Neural Implementation to Categorical Foundations](https://arxiv.org/abs/2603.19317)
*Ruoqi Sun*

Main category: cs.LG

TL;DR: Neural networks fail on compositional generalization (0% accuracy) but achieve 100% accuracy when constrained by a Ternary Gamma Semiring structure, which corresponds to a unique Boolean-type algebraic structure.


<details>
  <summary>Details</summary>
Motivation: To establish a theoretical connection between neural network learning and abstract algebraic structures, and to understand why neural networks succeed or fail on compositional generalization tasks.

Method: Present a minimal counterexample showing standard neural networks fail completely on compositional generalization. Introduce a logical constraint (Ternary Gamma Semiring) to the same architecture, then prove the learned feature space constitutes a finite commutative ternary Γ-semiring implementing majority vote rule.

Result: With the Ternary Gamma Semiring constraint, the architecture achieves 100% accuracy on novel combinations. The learned structure corresponds precisely to the Boolean-type ternary Γ-semiring with |T|=4, |Γ|=1, which is unique up to isomorphism in existing classifications.

Conclusion: Three key insights: (1) Neural network success approximates mathematically "natural" structures, (2) Learned representations generalize by internalizing algebraic axioms, (3) Logical constraints guide networks to converge to canonical forms. Establishes Computational Γ-Algebra as a new interdisciplinary direction.
...
### [210] [Target Concept Tuning Improves Extreme Weather Forecasting](https://arxiv.org/abs/2603.19325)
*Shijie Ren,Xinyue Gu,Ziheng Peng,Haifan Zhang,Peisong Niu,Bo Wu,Xiting Wang,Liang Sun,Jirong Wen*

Main category: cs.LG

TL;DR: TaCT is an interpretable concept-gated fine-tuning framework that selectively improves deep learning models for rare high-impact weather events (like typhoons) while preserving overall performance, using sparse autoencoders to discover failure-related concepts and applying targeted parameter updates.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for meteorological forecasting often fail in rare but high-impact events like typhoons due to scarce data. Existing fine-tuning methods face a trade-off between overlooking extreme events and overfitting them at the expense of overall performance.

Method: TaCT uses Sparse Autoencoders and counterfactual analysis to automatically discover failure-related internal concepts. It then updates model parameters only when corresponding concepts are activated, rather than applying uniform adaptation, enabling selective improvement for failure cases while preserving performance in common scenarios.

Result: Experiments show consistent improvements in typhoon forecasting across different regions without degrading other meteorological variables. The identified concepts correspond to physically meaningful circulation patterns, revealing model biases and supporting trustworthy adaptation.

Conclusion: TaCT provides an interpretable framework for trustworthy adaptation in scientific forecasting tasks, solving the trade-off problem in fine-tuning for rare events by enabling selective model improvement based on discovered failure-related concepts.
...
### [211] [FalconBC: Flow matching for Amortized inference of Latent-CONditioned physiologic Boundary Conditions](https://arxiv.org/abs/2603.19331)
*Chloe H. Choi,Alison L. Marsden,Daniele E. Schiavazzi*

Main category: cs.LG

TL;DR: A general amortized inference framework using probabilistic flow for boundary condition tuning in patient-specific cardiovascular models, handling complex scenarios like open-loop models and vascular lesions.


<details>
  <summary>Details</summary>
Motivation: Current data-driven variational inference methods for boundary condition tuning in cardiovascular modeling fail in two important scenarios: open-loop models with known mean flow/assumed waveform shapes, and anatomies with vascular lesions where segmentation affects pressure/flow target reachability.

Method: Introduces a general amortized inference framework based on probabilistic flow that treats clinical targets, inflow features, and point cloud embeddings of patient-specific anatomies as either conditioning variables or quantities to be jointly estimated.
...
Conclusion: The proposed framework addresses limitations of existing methods by enabling joint estimation of boundary conditions in complex cardiovascular modeling scenarios where boundary conditions cannot be tuned in isolation.
...
### [212] [Do Post-Training Algorithms Actually Differ? A Controlled Study Across Model Scales Uncovers Scale-Dependent Ranking Inversions](https://arxiv.org/abs/2603.19335)
*Xiaoyi Li*

Main category: cs.LG

TL;DR: Large-scale comparison of 51 post-training alignment algorithms reveals algorithm rankings are unstable across model scales, loss modifications yield negligible gains, and algorithm effectiveness is task-specific, with model scale being the most important factor.


<details>
  <summary>Details</summary>
Motivation: Practitioners lack controlled comparisons to guide selection among competing post-training alignment algorithms (DPO, SimPO, KTO, GRPO, etc.), creating a need for systematic evaluation.

Method: OXRL framework implementing 51 algorithms with identical infrastructure, evaluating 8 algorithms across 4 model scales (0.5B-7B), 3 domains, and 20 DPO variants, totaling ~240 training runs on H100 GPUs with rigorous statistical analysis.

Result: 1) Algorithm rankings invert across scales (SGRPO best at 1.5B, SimPO best at 7B); 2) Loss modifications yield negligible gains; 3) Algorithm effectiveness is task-specific (19.3pp spread on GSM8K vs 0.54pp on MATH). Hierarchy: model scale (~50pp) > training paradigm (~10pp) > online vs offline (~9pp) > loss function (~1pp).

Conclusion: Model scale is the dominant factor in alignment performance, algorithm choice matters primarily within training distribution, and loss function modifications offer minimal benefits. The study provides a practical hierarchy for practitioners and releases all resources as a community benchmark.
...
### [213] [DAPA: Distribution Aware Piecewise Activation Functions for On-Device Transformer Inference and Training](https://arxiv.org/abs/2603.19338)
*Maoyang Xiang,Bo Wang*

Main category: cs.LG

TL;DR: DAPA is a hardware-friendly activation function for Transformers that uses distribution-aware piecewise approximation and quantization to significantly accelerate GELU computation while maintaining model performance.


<details>
  <summary>Details</summary>
Motivation: Non-linear activation functions are computationally expensive for on-device inference and training, consuming substantial hardware resources and impacting system performance and energy efficiency. There's a need for more efficient activation functions that can reduce latency and resource utilization without sacrificing model accuracy.

Method: DAPA uses distribution-aware piecewise approximation that allocates finer segments to high-probability regions of pre-activation data distribution. It employs non-uniform piecewise approximation for better generalizability, followed by quantization using Distribution-Weighted Mean Square Error to optimize for hardware deployment.

Result: DAPA achieves 16× speedup in GELU computation and 16× reduction in DSP utilization compared to standard implementations, while maintaining comparable or better performance across vision Transformers and GPT-2 models.

Conclusion: DAPA provides an effective hardware-friendly activation function solution that significantly improves computational efficiency for Transformer architectures while preserving model performance, making it suitable for on-device deployment scenarios.
...
### [214] [Beyond Weighted Summation: Learnable Nonlinear Aggregation Functions for Robust Artificial Neurons](https://arxiv.org/abs/2603.19344)
*Berke Deniz Bozyigit*

Main category: cs.LG

TL;DR: Replacing fixed linear neuron aggregation with learnable nonlinear alternatives improves neural network robustness to noise while maintaining trainability.


<details>
  <summary>Details</summary>
Motivation: Standard weighted summation in neurons behaves like a mean-based estimator, making networks sensitive to noisy or extreme inputs. The paper investigates whether learnable nonlinear aggregation can improve robustness without sacrificing optimization stability.
...
### [215] [Anatomical Heterogeneity in Transformer Language Models](https://arxiv.org/abs/2603.19348)
*Tomasz Wietrzykowski*

Main category: cs.LG

TL;DR: The paper challenges the assumption of layer homogeneity in transformer models, showing profound anatomical heterogeneity across layers and proposing Growth Transformer Training for more efficient training.


<details>
  <summary>Details</summary>
Motivation: Current transformer language models are trained with uniform computational budgets across all layers, implicitly assuming layer homogeneity. The authors challenge this assumption through empirical analysis.

Method: Used five diagnostic metrics on SmolLM2-135M (30-layer, 135M-parameter model): weight predictability (R2), ablation degradation, recovery speed, weight manipulation robustness, and structural analysis. Proposed Growth Transformer Training that allocates budget by layer importance.

Result: Found profound anatomical heterogeneity: layer weights follow strong mathematical regularity but predicted weights cause catastrophic failure; layer importance spans 10^7 range with critical core layers and anti-layers; recovery speed correlates with importance; only weight scaling preserves quality; Growth Transformer Training achieves ~54% cost reduction and 4.7x lower validation loss than uniform training.

Conclusion: Transformer layers are anatomically heterogeneous, challenging the uniform training assumption. Growth Transformer Training offers significant efficiency improvements by allocating computational budget according to layer importance.
...
### [216] [Warm-Start Flow Matching for Guaranteed Fast Text/Image Generation](https://arxiv.org/abs/2603.19360)
*Minyoung Kim*

Main category: cs.LG

TL;DR: WS-FM (Warm-Start Flow Matching) accelerates flow matching algorithms by using fast, low-quality draft samples as initial distributions instead of pure noise, enabling fewer time steps and guaranteed speed-up without quality loss.


<details>
  <summary>Details</summary>
Motivation: Current generative models (AR LLMs, diffusion, flow matching) require many function evaluations during inference, making them computationally expensive and time-consuming. There's a need to reduce sample generation time without sacrificing quality.

Method: Propose Warm-Start FM (WS-FM) that uses computationally lightweight generative models to produce draft samples quickly. These draft samples serve as initial distributions for flow matching algorithms, allowing starting closer to the end time (rather than time 0 with pure noise), reducing the number of time steps needed.

Result: Demonstrated on synthetic toy data and real-world text/image generation tasks. WS-FM offers guaranteed speed-up in sample generation without sacrificing the quality of generated samples.

Conclusion: WS-FM provides an effective approach to accelerate flow matching algorithms by leveraging fast draft generation, enabling significant computational savings while maintaining sample quality.
...
### [217] [Optimizing Resource-Constrained Non-Pharmaceutical Interventions for Multi-Cluster Outbreak Control Using Hierarchical Reinforcement Learning](https://arxiv.org/abs/2603.19397)
*Xueqiao Peng,Andrew Perrault*

Main category: cs.LG

TL;DR: Hierarchical RL framework for allocating limited testing/quarantine resources across multiple asynchronous COVID-19 outbreak clusters outperforms baselines by 20-30%.


<details>
  <summary>Details</summary>
Motivation: Real-world public health faces constrained resources for NPIs (testing/quarantine) that must be allocated across multiple asynchronous outbreak clusters with varying sizes and risks, requiring decisions under uncertainty and operational constraints.

Method: Formulate as constrained restless multi-armed bandit problem. Propose hierarchical RL: global controller learns continuous action cost multiplier to adjust global resource demand; local policy estimates marginal value of allocating resources to individuals within each cluster.

Result: Method consistently outperforms RMAB-inspired and heuristic baselines across wide range of system scales and testing budgets, improving outbreak control effectiveness by 20%-30%. Scales to 40+ concurrent clusters with faster decision-making than RMAB method.

Conclusion: Hierarchical RL framework effectively addresses resource allocation challenges in multi-cluster outbreak scenarios, demonstrating superior performance and scalability for real-world public health decision-making under resource constraints.
...
### [218] [GeoLAN: Geometric Learning of Latent Explanatory Directions in Large Language Models](https://arxiv.org/abs/2603.19460)
*Tianyu Bell Pan,Damon L. Woodard*

Main category: cs.LG

TL;DR: GeoLAN is a training framework that treats token representations as geometric trajectories and applies stickiness conditions inspired by the Kakeya Conjecture to improve LLM transparency through geometry-aware training.


<details>
  <summary>Details</summary>
Motivation: Large language models demonstrate strong performance but lack transparency. The authors aim to enhance mechanistic interpretability by incorporating geometric principles into model training.

Method: Introduces GeoLAN framework treating token representations as geometric trajectories with stickiness conditions from Kakeya Conjecture. Develops two differentiable regularizers: Katz-Tao Convex Wolff (KT-CW) promoting isotropy, and Katz-Tao Attention (KT-Attn) encouraging diverse attention.

Result: Experiments with Gemma-3 (1B, 4B, 12B) and Llama-3-8B show GeoLAN maintains task accuracy while improving geometric metrics and reducing certain fairness biases. Benefits are most significant in mid-sized models, revealing scale-dependent trade-offs between geometric precision and performance.

Conclusion: Geometry-aware training is a promising approach to enhance mechanistic interpretability in LLMs, with GeoLAN demonstrating practical benefits for model transparency and fairness.
...
### [219] [Deep Hilbert--Galerkin Methods for Infinite-Dimensional PDEs and Optimal Control](https://arxiv.org/abs/2603.19463)
*Samuel N. Cohen,Filippo de Feo,Jackson Hebner,Justin Sirignano*

Main category: cs.LG

TL;DR: Deep learning methods for fully nonlinear second-order PDEs on Hilbert spaces using Hilbert-Galerkin Neural Operators, with novel universal approximation theorems and numerical training methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with fully nonlinear second-order PDEs on infinite-dimensional Hilbert spaces, which arise in many applied sciences including control problems, functional differential equations, SPDEs, and mean-field systems.

Method: Develop Hilbert-Galerkin Neural Operators (HGNOs) with novel topologies for Hessian terms, prove Universal Approximation Theorems for functions and their derivatives up to second order, and create Deep Hilbert-Galerkin and Hilbert Actor-Critic training methods that minimize L²-norm residuals on the whole Hilbert space.

Result: First UATs for fully nonlinear PDEs on Hilbert spaces with non-sequential, non-metrizable topologies; successful numerical solution of Kolmogorov and HJB PDEs related to optimal control of heat and Burgers' equations.

Conclusion: HGNOs provide a promising deep learning framework for solving challenging infinite-dimensional PDE problems with theoretical guarantees and practical numerical performance.
...
### [220] [Global Convergence of Multiplicative Updates for the Matrix Mechanism: A Collaborative Proof with Gemini 3](https://arxiv.org/abs/2603.19465)
*Keith Rush*

Main category: cs.LG

TL;DR: The paper proves convergence of a fixed-point iteration for nuclear norm optimization and documents using AI (Gemini 3) to assist with the mathematical proof.


<details>
  <summary>Details</summary>
Motivation: To close an open problem from prior work on regularized nuclear norm optimization in private machine learning, and to document the practical use of AI in mathematical research.

Method: Analyzes a fixed-point iteration v ← φ(v) for nuclear norm optimization with Hadamard product structure. Uses mathematical analysis to prove monotonic convergence to unique global optimizer, with AI assistance (Gemini 3) for proof development.

Result: Proves that the iteration v^{(k+1)} = diag((D_{v^{(k)}}^{1/2} M D_{v^{(k)}}^{1/2})^{1/2}) converges monotonically to the unique global optimizer of J(v) = 2Tr((D_v^{1/2} M D_v^{1/2})^{1/2}) - ∑ v_i.

Conclusion: Closes an open problem in nuclear norm optimization while demonstrating practical AI-assisted mathematical research, providing insights into AI-human collaboration in proof development.
...
### [221] [Spectral Alignment in Forward-Backward Representations via Temporal Abstraction](https://arxiv.org/abs/2603.20103)
*Seyed Mahdi B. Azad,Jasper Hoffmann,Iman Nematollahi,Hao Zhu,Abhinav Valada,Joschka Boedecker*

Main category: cs.LG

TL;DR: Temporal abstraction helps align the spectral properties of continuous MDPs with low-rank FB architectures by acting as a low-pass filter, enabling stable successor representation learning.


<details>
  <summary>Details</summary>
Motivation: There's a spectral mismatch between high-rank transition dynamics in continuous environments and the low-rank bottleneck of forward-backward architectures, making accurate low-rank representation learning difficult.

Method: Analyze temporal abstraction as a mechanism to mitigate spectral mismatch by characterizing spectral properties of the transition operator, showing it acts as a low-pass filter that suppresses high-frequency components.

Result: Temporal abstraction reduces the effective rank of the induced successor representation while preserving formal bounds on value function error, enabling stable FB learning especially at high discount factors.

Conclusion: Temporal abstraction is a principled mechanism for shaping the spectral structure of MDPs and enabling effective long-horizon representations in continuous control.
...
### [222] [Adaptive Layerwise Perturbation: Unifying Off-Policy Corrections for LLM RL](https://arxiv.org/abs/2603.19470)
*Chenlu Ye,Xuanchang Zhang,Yifan Hao,Zhou Yu,Ziji Zhang,Abhinav Gullapalli,Hao Chen,Jing Huang,Tong Zhang*

Main category: cs.LG

TL;DR: ALP (Adaptive Layerwise Perturbation) addresses off-policy RL training instability in LLMs by injecting learnable perturbations into hidden states to flatten importance ratio distributions and reduce policy gap.


<details>
  <summary>Details</summary>
Motivation: Off-policy RL for LLMs suffers from policy staleness and training-inference mismatch, causing heavy-tailed importance ratios and sharp gradients that push updates outside trust regions, leading to training instability.

Method: Proposes Adaptive Layerwise Perturbation (ALP) - injecting small learnable perturbations into input hidden states of each layer during updates, using perturbed policy as numerator against unchanged inference policy in the objective.

Result: ALP improves final performance on math and tool-integrated reasoning tasks, avoids importance ratio tail blow-up and KL spikes, boosts exploration, and outperforms partial-layer and logits-only variants.

Conclusion: ALP effectively maintains training stability by flattening importance ratio distributions through controlled representation-level perturbations, bridging the policy gap between updated and inference policies.
...
TL;DR: A control-space learning framework for few-step swarm steering that respects sampled-data constraints by learning finite-horizon control coefficients instead of instantaneous velocity fields.


<details>
  <summary>Details</summary>
Motivation: Real swarm systems operate in sampled-data form with intermittent control updates, making few-step steering challenging. Traditional approaches using instantaneous velocity fields don't respect the sampled-data structure of actual control systems.

Method: Introduces a control-space learning framework inspired by MeanFlow that learns coefficients parameterizing finite-horizon minimum-energy control over each sampling interval. Uses integral representation and local differential identity along bridge trajectories with stop-gradient training objective.

Result: The framework provides a scalable approach to few-step swarm steering that is consistent with sampled-data structure, with learned coefficients used directly in sampled-data updates while respecting prescribed dynamics and actuation maps.

Conclusion: The proposed control-space learning approach enables effective few-step swarm steering that properly accounts for the sampled-data nature of real control systems, offering a more realistic and implementable framework than instantaneous velocity field methods.
...
### [224] [TRACE: Trajectory Recovery with State Propagation Diffusion for Urban Mobility](https://arxiv.org/abs/2603.19474)
*Jinming Wang,Hai Wang,Hongkai Wen,Geyong Min,Man Luo*

Main category: cs.LG

TL;DR: TRACE is a diffusion model for trajectory recovery that reconstructs dense, continuous trajectories from sparse GPS data using a State Propagation Diffusion Model with memory mechanism.


<details>
  <summary>Details</summary>
Motivation: Real-world GPS trajectories are often sparse and unevenly distributed due to low sampling rates and limited infrastructure coverage, making trajectory recovery essential but challenging for location-based services and smart city applications.

Method: TRACE uses a State Propagation Diffusion Model (SPDM) with a novel memory mechanism that retains intermediate results from previous denoising steps to effectively reconstruct hard-to-recover trajectory segments.
...
### [225] [Any-Subgroup Equivariant Networks via Symmetry Breaking](https://arxiv.org/abs/2603.19486)
*Abhinav Goel,Derek Lim,Hannah Lawrence,Stefanie Jegelka,Ningyuan Huang*

Main category: cs.LG

TL;DR: ASEN is a single model that achieves simultaneous equivariance to multiple symmetry groups via symmetry-breaking inputs, overcoming the limitations of traditional equivariant architectures that are constrained to specific, pre-defined symmetries.


<details>
  <summary>Details</summary>
Motivation: Traditional equivariant architectures are highly constrained to specific symmetries chosen a priori, preventing the development of flexible foundation models capable of processing diverse data equivariantly. There's a need for models that can handle multiple symmetries simultaneously.

Method: Start with a fully permutation-equivariant base model, then obtain subgroup equivariance by using symmetry-breaking inputs whose automorphism group matches the desired subgroup. Overcome computational hardness by relaxing to approximate symmetry breaking using 2-closure for fast algorithms.
...
Conclusion: ASEN enables a single network to be equivariant to multiple permutation subgroups simultaneously, providing a flexible framework for multi-modal foundation models that can process diverse data equivariantly without being constrained to specific symmetries.
...
### [226] [ICLAD: In-Context Learning for Unified Tabular Anomaly Detection Across Supervision Regimes](https://arxiv.org/abs/2603.19497)
*Jack Yi Wei,Narges Armanfard*

Main category: cs.LG

TL;DR: ICLAD is an in-context learning foundation model for tabular anomaly detection that works across different datasets and supervision regimes (one-class, unsupervised, semi-supervised) without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for tabular anomaly detection are limited by being dataset-specific and tied to single supervision regimes, preventing them from leveraging shared structures across tasks and adapting to different supervision levels.

Method: ICLAD uses meta-learning on synthetic tabular anomaly detection tasks to create a foundation model. At inference, it performs in-context learning by conditioning on the training set without updating model weights, assigning anomaly scores based on context.

Result: Comprehensive experiments on 57 tabular datasets from ADBench show state-of-the-art performance across all three supervision regimes (one-class, unsupervised, semi-supervised).

Conclusion: ICLAD establishes a unified framework for tabular anomaly detection that generalizes across datasets and supervision regimes, overcoming limitations of previous dataset-specific approaches.
...
### [227] [Scalable Cross-Facility Federated Learning for Scientific Foundation Models on Multiple Supercomputers](https://arxiv.org/abs/2603.19544)
*Yijiang Li,Zilinghan Li,Kyle Chard,Ian Foster,Todd Munson,Ravi Madduri,Kibaek Kim*

Main category: cs.LG

TL;DR: A federated learning framework for cross-HPC facility deployment that addresses challenges in scientific AI applications with privacy-sensitive or large-scale data.


<details>
  <summary>Details</summary>
Motivation: Scientific AI applications need to train large models on data that cannot be centralized due to privacy, data sovereignty, or volume constraints. Federated learning helps but requires HPC resources, and deploying FL across multiple HPC facilities presents unique challenges beyond cloud/enterprise settings.

Method: Developed a comprehensive cross-facility FL framework for heterogeneous HPC environments, built on Advanced Privacy-Preserving Federated Learning (APPFL) framework with Globus Compute and Transfer orchestration. Evaluated across four DOE leadership-class supercomputers.

Result: Demonstrated that FL experiments across HPC facilities are practically achievable. Characterized key sources of heterogeneity impacting training performance, showed algorithmic choices matter significantly under realistic HPC scheduling conditions. Validated scientific applicability by fine-tuning a large language model on chemistry instruction dataset.

Conclusion: Cross-facility FL for scientific applications is feasible but requires addressing HPC-specific challenges. Identified scheduler-aware algorithm design as a critical open challenge for future deployments.
...
### [228] [Subspace Kernel Learning on Tensor Sequences](https://arxiv.org/abs/2603.19546)
*Lei Wang,Xi Ding,Yongsheng Gao,Piotr Koniusz*

Main category: cs.LG

TL;DR: UKTL is a novel kernel framework for M-mode tensors that uses uncertainty-driven subspace weighting and scalable Nyström linearization for efficient and robust tensor similarity learning.


<details>
  <summary>Details</summary>
Motivation: Need to capture complex interactions across tensor modes while maintaining computational efficiency for learning from structured multi-way data represented as higher-order tensors.

Method: Uncertainty-driven Kernel Tensor Learning (UKTL) compares mode-wise subspaces from tensor unfoldings, uses scalable Nyström kernel linearization with soft k-means clustering for pivot tensors, and incorporates uncertainty-aware subspace weighting to down-weight unreliable components.

Result: Achieves state-of-the-art performance on action recognition benchmarks (NTU-60, NTU-120, Kinetics-Skeleton) with superior generalization and meaningful mode-wise insights.

Conclusion: Establishes a principled, scalable, and interpretable kernel learning paradigm for structured multi-way and multi-modal tensor sequences.
...
### [229] [Wearable Foundation Models Should Go Beyond Static Encoders](https://arxiv.org/abs/2603.19564)
...
TL;DR: Wearable foundation models need to evolve from short-term static prediction to longitudinal, anticipatory health reasoning for chronic conditions.


<details>
  <summary>Details</summary>
Motivation: Current wearable foundation models focus on short-term, well-defined tasks but are poorly suited for modeling chronic, progressive, or episodic health conditions that unfold over longer timeframes (weeks, months, years). They emphasize retrospective prediction rather than reasoning over evolving personal history and future risk trajectories.

Method: Proposes three foundational shifts: 1) Structurally rich data with integrated multimodal, long-term personal trajectories and contextual metadata; 2) Longitudinal-aware multimodal modeling prioritizing long-context inference, temporal abstraction, and personalization; 3) Agentic inference systems supporting planning, decision-making, and clinically grounded intervention under uncertainty.

Result: The paper argues for a paradigm shift in wearable health monitoring from retrospective signal interpretation toward continuous, anticipatory, and human-aligned health support through these three foundational changes.

Conclusion: Wearable foundation models must move beyond static encoders and be explicitly designed for longitudinal, anticipatory health reasoning to effectively model chronic health conditions and provide meaningful long-term health support.
...
### [230] [ARMOR: Adaptive Resilience Against Model Poisoning Attacks in Continual Federated Learning for Mobile Indoor Localization](https://arxiv.org/abs/2603.19594)
*Danish Gufran,Akhil Singampalli,Sudeep Pasricha*

Main category: cs.LG

TL;DR: ARMOR is a continual federated learning framework for indoor localization that uses a state-space model to monitor and safeguard the global model against corrupted updates and adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: Real-world continual federated learning for indoor localization faces challenges from device heterogeneity, evolving environments, and adversarial model poisoning attacks that can degrade model performance over time.

Method: ARMOR introduces a state-space model that learns the historical evolution of global model weight tensors and predicts expected future states. It compares incoming local updates with these projections to detect deviations and selectively mitigate corrupted updates before aggregation.

Result: ARMOR achieves up to 8.0x reduction in mean error and 4.97x reduction in worst-case error compared to state-of-the-art indoor localization frameworks, demonstrating strong resilience against model corruption.

Conclusion: ARMOR provides an effective framework for robust continual federated learning in dynamic indoor environments, protecting against model degradation and adversarial attacks while maintaining privacy-preserving localization.
...
### [231] [Demonstrations, CoT, and Prompting: A Theoretical Analysis of ICL](https://arxiv.org/abs/2603.19611)
*Xuhan Tong,Yuchen Zeng,Jiawei Zhang*

Main category: cs.LG

TL;DR: Theoretical analysis of In-Context Learning (ICL) showing how demonstration quality, pretraining capability, and distribution shift affect ICL performance, with insights on Chain-of-Thought prompting benefits and prompt template sensitivity.


<details>
  <summary>Details</summary>
Motivation: Existing ICL theories either rely on strong assumptions or fail to capture practical factors like demonstration selection, CoT prompting, number of demonstrations, and prompt templates. This paper aims to address this gap with a theoretical analysis under mild assumptions.

Method: Establishes theoretical analysis of ICL under mild assumptions, derives upper bound on ICL test loss, analyzes CoT prompting as inducing task decomposition, and characterizes prompt template sensitivity relative to number of demonstrations.

Result: Shows ICL performance is governed by: (1) demonstration quality (quantified by Lipschitz constants), (2) intrinsic ICL capability of pretrained model, and (3) degree of distribution shift. CoT is beneficial when demonstrations are well-chosen at each substep and subtasks are easier to learn. Prompt template sensitivity varies with number of demonstrations.

Conclusion: Pretraining equips models with ability to generalize beyond observed tasks, CoT enables composition of simpler subtasks into complex ones, and demonstrations/instructions enable retrieval of similar/complex tasks, jointly supporting generalization to unseen tasks. All insights are experimentally validated.
...
### [232] [On Performance Guarantees for Federated Learning with Personalized Constraints](https://arxiv.org/abs/2603.19617)
*Mohammadjavad Ebrahimi,Daniel Burbano,Farzad Yousefian*

Main category: cs.LG

TL;DR: PC-FedAvg: A personalized constrained federated optimization method where each agent maintains cross-estimates of other agents' variables, enabling personalization without consensus or constraint sharing, with O(ε⁻²) suboptimality and O(ε⁻¹) infeasibility rates.


<details>
  <summary>Details</summary>
Motivation: Standard FL formulations handle unconstrained or globally constrained problems, but practical settings often involve heterogeneous resource or model constraints with agent-specific feasible sets, requiring personalized constrained federated optimization.

Method: PC-FedAvg method where each agent maintains cross-estimates of other agents' variables through a multi-block local decision vector, updates all blocks locally while penalizing infeasibility only in its own block, enabling personalization without consensus or constraint sharing.

Result: Theoretical communication-complexity rates of O(ε⁻²) for suboptimality and O(ε⁻¹) for agent-wise infeasibility, validated by preliminary experiments on MNIST and CIFAR-10 datasets.

Conclusion: PC-FedAvg provides an effective solution for personalized constrained federated optimization with provable convergence rates and practical applicability, addressing the challenge of heterogeneous agent constraints without requiring consensus or constraint information sharing.
...
### [233] [DeepStock: Reinforcement Learning with Policy Regularizations for Inventory Management](https://arxiv.org/abs/2603.19621)
*Yaqi Xie,Xinru Hao,Jiaxi Liu,Will Ma,Linwei Xin,Lei Cao,Yidong Zhang*

Main category: cs.LG

TL;DR: DRL for inventory management benefits from policy regularizations based on classical inventory concepts like Base Stock, improving hyperparameter tuning and final performance.


<details>
  <summary>Details</summary>
Motivation: Off-the-shelf DRL implementations for inventory management are highly sensitive to hyperparameters and show mixed success, requiring more robust and efficient training approaches.

Method: Impose policy regularizations grounded in classical inventory concepts (e.g., Base Stock) to accelerate hyperparameter tuning and improve DRL performance for inventory management.

Result: Policy regularizations significantly accelerate hyperparameter tuning and improve final performance of DRL methods; successfully deployed 100% on Alibaba's Tmall platform; synthetic experiments show regularizations change which DRL method performs best.

Conclusion: Policy regularizations based on classical inventory concepts make DRL more practical for inventory management by improving training efficiency and performance, with successful real-world deployment.
...
### [234] [Continual Learning for Food Category Classification Dataset: Enhancing Model Adaptability and Performance](https://arxiv.org/abs/2603.19624)
...
<details>
  <summary>Details</summary>
Motivation: Conventional ML pipelines struggle with recognizing categories absent from original training sets, and fixed datasets rarely capture full domain diversity, leading to reduced accuracy in food classification.

Method: Proposes a continual learning framework for text-guided food classification that allows incremental updates to integrate new categories without requiring full retraining from scratch.

Result: The framework enables models to learn new food categories (like dosa or kimchi) after initial training on different cuisines (e.g., Western), showing promise for adaptive food recognition.

Conclusion: The design shows promise for adaptive food recognition with applications in dietary monitoring and personalized nutrition planning, though further refinements are needed.
...
### [235] [Alternating Diffusion for Proximal Sampling with Zeroth Order Queries](https://arxiv.org/abs/2603.19633)
*Hirohane Takagi,Atsushi Nitanda*

Main category: cs.LG

TL;DR: New proximal sampler using only zeroth-order information of potential function, avoiding rejection sampling and enabling deterministic runtime with flexible step sizes.


<details>
  <summary>Details</summary>
Motivation: Existing proximal samplers require rejection sampling for backward steps, while diffusion-based methods need learned score models or auxiliary samplers. The authors aim to create a simpler, more practical sampler that avoids these complexities.

Method: Treats intermediate particle distribution as Gaussian mixture, enabling Monte Carlo score estimation from directly samplable distributions. Simulates dynamics directly instead of using rejection sampling, operates with zeroth-order information only.

Result: Theoretical: Inherits exponential convergence of proximal sampling under isoperimetric conditions when score estimation error is controlled. Practical: Avoids rejection sampling, permits flexible step sizes, runs with deterministic runtime budget, converges rapidly via particle interactions and parallel computation.

Conclusion: Proposes a practical proximal sampler that combines theoretical guarantees with implementation advantages, offering a viable alternative to existing sampling methods with improved flexibility and computational efficiency.
...
### [236] [RiboSphere: Learning Unified and Efficient Representations of RNA Structures](https://arxiv.org/abs/2603.19636)
*Zhou Zhang,Hanqun Cao,Cheng Tan,Fang Wu,Pheng Ann Heng,Tianfan Fu*

Main category: cs.LG

TL;DR: RiboSphere is a framework that learns discrete geometric representations of RNA using vector quantization and flow matching, achieving accurate structure modeling and effective transfer to downstream tasks.


<details>
  <summary>Details</summary>
Motivation: RNA structure modeling is challenging due to flexible backbones, prevalent non-canonical interactions, and scarcity of experimental 3D structures. The modular organization of RNA architecture (complex folds composed from recurring structural motifs) motivates the approach.

Method: Combines vector quantization with flow matching. Uses geometric transformer encoder to produce SE(3)-invariant features, discretizes them with finite scalar quantization (FSQ) into latent codes, then conditions a flow-matching decoder on these codes to reconstruct atomic coordinates.
...
Conclusion: RiboSphere successfully learns discrete geometric representations of RNA that capture modular structural organization, enabling accurate structure modeling and effective transfer learning to various RNA-related prediction tasks.
...
### [237] [Heavy-Tailed and Long-Range Dependent Noise in Stochastic Approximation: A Finite-Time Analysis](https://arxiv.org/abs/2603.19648)
*Siddharth Chandak,Anuj Yadav,Ayfer Ozgur,Nicholas Bambos*

Main category: cs.LG

TL;DR: First finite-time moment bounds for stochastic approximation under heavy-tailed and long-range dependent noise, with applications to SGD and gradient play.


<details>
  <summary>Details</summary>
Motivation: Classical SA analyses assume bounded second moments or Markov noise, but many practical applications (finance, communications) encounter heavy-tailed and long-range dependent noise, which existing theory doesn't cover.

Method: Study SA for finding roots of strongly monotone operators under heavy-tailed and LRD noise. Use noise-averaging argument to regularize noise impact without modifying iteration. Apply framework to SGD and gradient play.

Result: Establish first finite-time moment bounds for both noise settings, providing explicit convergence rates that quantify impact of heavy tails and temporal dependence. Numerical experiments corroborate analysis.

Conclusion: General framework extends SA theory to practical non-classical noise models, providing rigorous finite-time guarantees for applications with heavy-tailed or long-range dependent noise.
...
### [238] [Ensembles-based Feature Guided Analysis](https://arxiv.org/abs/2603.19653)
*Federico Formica,Stefano Gregis,Andrea Rota,Aurora Francesca Zanenga,Mark Lawford,Claudio Menghi*

Main category: cs.LG

TL;DR: EFGA improves recall of neural network explanation rules by combining FGA rules into ensembles with different aggregation criteria.


<details>
  <summary>Details</summary>
Motivation: Existing Feature Guided Analysis (FGA) techniques for explaining DNN behavior have good precision but limited recall, meaning they don't apply to enough situations.

Method: Proposes Ensembles-based Feature Guided Analysis (EFGA) that combines rules extracted by FGA into ensembles using aggregation criteria/policies. Three different aggregation criteria were developed and evaluated.

Result: EFGA significantly improves recall (+28-33% on training, +25-31% on testing) compared to FGA with minimal precision reduction (-0.7-0.9%). Different aggregation criteria offer alternative precision-recall trade-offs.

Conclusion: EFGA effectively addresses the recall limitation of FGA by combining rules into ensembles, providing a flexible framework for improving DNN explanation coverage while maintaining high precision.
...
### [239] [The Residual Stream Is All You Need: On the Redundancy of the KV Cache in Transformer Inference](https://arxiv.org/abs/2603.19664)
*Kaleem Ullah Qasim,Jiashu Zhang,Muhammad Kafeel Shaheen,Razan Alharith,Heying Zhang*

Main category: cs.LG

TL;DR: KV cache in transformers is redundant - keys and values can be perfectly recomputed from residual stream with zero error, enabling memory-efficient inference.


<details>
  <summary>Details</summary>
Motivation: Current transformer inference requires large KV cache memory, but this state may be redundant if keys/values can be recomputed from other model states.

Method: Prove KV cache redundancy mathematically, verify across 6 models (135M-4B params), develop KV-Direct that checkpoints residual vectors (5KB/token) instead of full KV pairs (136KB), recomputing keys/values on demand.

Result: KV-Direct maintains 100% token match at all cache budgets vs baselines (5-28%), reduces peak memory from 103MB to 42MB over 20 conversation turns, recomputation runs 5x faster than cache reads at moderate batch sizes.

Conclusion: KV cache is entirely redundant; residual stream is the sole information-carrying state, enabling bounded-memory inference with perfect accuracy via recomputation.
...
### [240] [Scale-Dependent Radial Geometry and Metric Mismatch in Wasserstein Propagation for Reverse Diffusion](https://arxiv.org/abs/2603.19670)
*Zicheng Lyu,Zengfeng Huang*

Main category: cs.LG

TL;DR: The paper analyzes reverse diffusion sampling error propagation, showing that contraction occurs radially rather than in Euclidean geometry under weak log-concavity. It develops a one-switch routing argument with metric conversion to obtain explicit W₂ guarantees.


<details>
  <summary>Details</summary>
Motivation: Existing analyses propagate sampling error in Euclidean geometry along the entire reverse trajectory, but Gaussian smoothing creates contraction first at large separations while short separations remain non-dissipative, creating a metric mismatch problem.

Method: Formalizes radial lower profile for learned reverse drift, uses one-switch routing: reflection coupling in concave transport metric before switch, converts to W₂ under p-moment budget at switch, then propagates in Euclidean geometry over remaining window.

Result: Obtains explicit non-asymptotic end-to-end W₂ guarantees for discretizations of learned reverse SDE under L² score-error control, provides scalar switch-selection objective, and identifies sharp structural limit on conversion exponent within affine-tail concave class.

Conclusion: The radial contraction phenomenon under weak log-concavity creates metric mismatch that can be exploited through one-switch routing with metric conversion, yielding improved error propagation analysis for reverse diffusion sampling.
...
### [241] [GoAgent: Group-of-Agents Communication Topology Generation for LLM-based Multi-Agent Systems](https://arxiv.org/abs/2603.19677)
*Hongjiang Chen,Xin Zheng,Yixin Liu,Pengfei Jiao,Shiyuan Li,Huan Liu,Zhidong Zhao,Ziqi Xu,Ibrahim Khalil,Shirui Pan*

Main category: cs.LG

TL;DR: GoAgent introduces a group-centric approach to LLM-based multi-agent communication topology generation, treating collaborative groups as atomic units rather than individual agents, with conditional information bottleneck to reduce communication overhead.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based multi-agent systems generate communication topologies in a node-centric manner, leading to suboptimal coordination and unnecessary communication overhead. Group structures emerge implicitly rather than being explicitly modeled, which hampers efficient task decomposition and collaboration.

Method: GoAgent first enumerates task-relevant candidate groups using LLMs, then autoregressively selects and connects these groups as atomic units to construct the final communication graph. It introduces a conditional information bottleneck objective to compress inter-group communication, preserving task-relevant signals while filtering redundant historical noise.

Result: Extensive experiments on six benchmarks show state-of-the-art performance with 93.84% average accuracy while reducing token consumption by about 17% compared to existing approaches.

Conclusion: Explicitly modeling collaborative groups as atomic units in MAS construction significantly improves coordination efficiency and reduces communication overhead, demonstrating the effectiveness of group-centric topology generation with information bottleneck compression.
...
### [242] [Ontology-Based Knowledge Modeling and Uncertainty-Aware Outdoor Air Quality Assessment Using Weighted Interval Type-2 Fuzzy Logic](https://arxiv.org/abs/2603.19683)
*Md Inzmam,Ritesh Chandra,Sadhana Tiwari,Sonali Agarwal,Triloki Pant*

Main category: cs.LG

TL;DR: Proposes a hybrid ontology-based uncertainty-aware framework integrating Weighted Interval Type-2 Fuzzy Logic with semantic knowledge modeling for improved air quality index classification and decision support.


<details>
  <summary>Details</summary>
Motivation: Traditional AQI calculation uses crisp thresholds and deterministic aggregation rules that are not suitable for handling uncertainty and transitions between classes, especially in rapidly urbanizing areas where air pollution is a major public health concern.

Method: Hybrid framework combining: 1) Interval Type-2 fuzzy sets to model uncertainty near AQI class boundaries, 2) Interval Type-2 Fuzzy Analytic Hierarchy Process (IT2-FAHP) to determine pollutant importance weights based on health impacts, and 3) OWL-based air quality ontology extending Semantic Sensor Network (SSN) ontology with SWRL rules for semantic reasoning and SPARQL queries for validation.

Result: Experimental evaluation using CPCB air quality datasets shows the proposed framework improves AQI classification reliability and uncertainty handling compared to traditional crisp and Type-1 fuzzy approaches, while enabling explainable semantic reasoning and intelligent decision support.

Conclusion: The hybrid ontology-based uncertainty-aware framework provides a more robust and intelligent approach to air quality monitoring by better handling uncertainty, incorporating health impact considerations, and enabling semantic reasoning for decision support in environmental governance.
...
### [243] [Regret Analysis of Sleeping Competing Bandits](https://arxiv.org/abs/2603.19700)
*Shinnosuke Uba,Yutaro Yamaguchi*

Main category: cs.LG

TL;DR: Sleeping Competing Bandits extends competing bandits to handle varying player/arm availability, with asymptotically optimal algorithm achieving O(NK log T_i/Δ²) regret.


<details>
  <summary>Details</summary>
Motivation: Real-world competing bandits problems involve players and arms with varying availability over time, unlike conventional models that assume constant availability.

Method: Formulated Sleeping Competing Bandits framework, extended regret definition, proposed algorithm for this setting with time-varying availability.

Result: Algorithm achieves O(NK log T_i/Δ²) regret upper bound, with matching lower bound Ω(N(K-N+1) log T_i/Δ²), showing asymptotic optimality when K > N.

Conclusion: Sleeping Competing Bandits framework effectively handles varying availability, with proposed algorithm being asymptotically optimal in relevant regimes.
...
### [244] [Learning from Similarity/Dissimilarity and Pairwise Comparison](https://arxiv.org/abs/2603.19713)
*Tomoya Tate,Kosuke Sugiyama,Masato Uchida*

Main category: cs.LG

TL;DR: Proposes SD-Pcomp classification framework using only relative judgments (similarity/dissimilarity and pairwise comparison) instead of probabilistic supervision, improving robustness to label noise and uncertainty.


<details>
  <summary>Details</summary>
Motivation: Existing SconfConfDiff framework requires continuous probabilistic supervision which involves subjective uncertainty quantification, leading to unstable supervision. Need for more robust approach using only relative judgments.

Method: SD-Pcomp classification uses two types of weak labels: Similarity/Dissimilarity (SD) labels indicating class agreement between instances, and Pairwise Comparison (Pcomp) labels indicating preference toward positive class. Develops two unbiased risk estimators: convex combination of SD and Pcomp, and unified estimator integrating both labels by modeling their relationship.

Result: Theoretical analysis and experiments show improved classification performance over methods using single weak labels, with robustness to label noise and uncertainty in class prior estimation.

Conclusion: SD-Pcomp classification provides a more robust weakly supervised learning framework by using only relative judgments instead of probabilistic supervision, addressing instability issues in existing approaches.
...
### [245] [FedRG: Unleashing the Representation Geometry for Federated Learning with Noisy Clients](https://arxiv.org/abs/2603.19722)
*Tian Wen,Zhiqin Yang,Yonggang Zhang,Xuefeng Jiang,Hao Peng,Yuwei Wang,Bo Han*

Main category: cs.LG

TL;DR: FedRG uses representation geometry to identify noisy labels in federated learning by creating label-agnostic spherical representations and fitting a vMF mixture model, outperforming existing methods in heterogeneous noisy scenarios.


<details>
  <summary>Details</summary>
Motivation: Federated learning suffers from performance degradation due to noisy annotations in distributed scenarios. Existing approaches using scalar loss values for noisy sample recognition lack reliability under heterogeneous FL settings.

Method: FedRG follows "representation geometry priority" principle: 1) Creates label-agnostic spherical representations using self-supervision, 2) Iteratively fits spherical von Mises-Fisher mixture model to capture semantic clusters using clean samples, 3) Integrates geometric evidence with semantic-label soft mapping to derive distribution divergence between label-free and label-conditioned feature spaces, 4) Uses personalized noise absorption matrix on noisy labels for robust optimization.

Result: Extensive experiments demonstrate that FedRG significantly outperforms state-of-the-art methods for federated learning with data heterogeneity under diverse noisy client scenarios.

Conclusion: The representation geometry approach provides more reliable noisy sample recognition than scalar loss-based methods, enabling robust federated learning in heterogeneous environments with noisy annotations.
...
### [246] [FedPDPO: Federated Personalized Direct Preference Optimization for Large Language Model Alignment](https://arxiv.org/abs/2603.19741)
*Kewen Zhu,Liping Yi,Zhiming Zhao,Zhuang Qi,Han Yu,Qinghua Hu*

Main category: cs.LG

TL;DR: FedPDPO is a personalized federated learning framework that enables efficient alignment of large language models with human preferences in decentralized, non-IID settings by combining global LoRA adapters with personalized client-specific components and explicit reward heads.


<details>
  <summary>Details</summary>
Motivation: Aligning LLMs with human preferences in federated learning is challenging due to decentralized, privacy-sensitive, and highly non-IID preference data. Traditional DPO methods suffer from severe performance degradation in FL settings with non-IID data and limited generalization of implicit rewards.

Method: FedPDPO uses parameter-efficient fine-tuning where each client maintains a frozen pretrained LLM backbone with a LoRA adapter. It introduces: 1) globally shared LoRA adapter with personalized client-specific LLM head, 2) personalized DPO training with client-specific explicit reward head to complement implicit rewards, and 3) bottleneck adapter to balance global and local features.

Result: Extensive experiments on multiple preference datasets demonstrate state-of-the-art performance, achieving up to 4.80% average accuracy improvements in both federated intra-domain and cross-domain settings.

Conclusion: FedPDPO effectively addresses the challenges of aligning LLMs with human preferences in federated learning by combining global parameter sharing with personalized components, enabling communication-efficient aggregation while handling non-IID heterogeneity through explicit reward modeling and balanced feature extraction.
...
### [247] [Dual Path Attribution: Efficient Attribution for SwiGLU-Transformers through Layer-Wise Target Propagation](https://arxiv.org/abs/2603.19742)
*Lasse Marten Jantsch,Dong-Jae Koh,Seonghyeon Lee,Young-Kyoon Suh*

Main category: cs.LG

TL;DR: DPA is a novel attribution framework that traces information flow in transformers with O(1) time complexity, achieving state-of-the-art faithfulness and unprecedented efficiency for dense component attribution.


<details>
  <summary>Details</summary>
Motivation: Current attribution methods for transformer-based LLMs struggle to balance faithfulness and computational efficiency, with dense component attribution remaining prohibitively expensive despite recent advances.

Method: DPA analytically decomposes and linearizes SwiGLU Transformers into distinct pathways, propagating a targeted unembedding vector through one forward and one backward pass without counterfactual examples.

Result: DPA achieves O(1) time complexity with respect to model components, scales to long input sequences, and demonstrates state-of-the-art faithfulness and unprecedented efficiency on standard interpretability benchmarks.

Conclusion: DPA provides an efficient and faithful attribution framework for understanding internal mechanisms of transformer-based LLMs, enabling reliable deployment and effective operation through scalable dense component analysis.
...
### [248] [Scalable Learning of Multivariate Distributions via Coresets](https://arxiv.org/abs/2603.19792)
*Zeyu Ding,Katja Ickstadt,Nadja Klein,Alexander Munteanu,Simon Omlor*

Main category: cs.LG

TL;DR: Novel coreset construction for multivariate conditional transformation models (MCTMs) enhances scalability and training efficiency for semi-parametric distributional models via importance sampling with statistical accuracy guarantees.


<details>
  <summary>Details</summary>
Motivation: Efficient and scalable non-parametric/semi-parametric regression and density estimation are crucial for statistics and machine learning, but existing methods struggle with large-scale data. There's a need for scalable approaches that maintain statistical accuracy while handling complex distributions and non-linear relationships.

Method: Develops coreset construction for multivariate conditional transformation models (MCTMs) using importance sampling for data reduction. Employs geometric approximation based on convex hull of input data to address numerical problems with logarithmic terms. Ensures log-likelihood remains within (1±ε) multiplicative error bounds with high probability.

Result: First coresets for semi-parametric distributional models. Substantial data reduction while maintaining statistical model accuracy. Enhanced adaptability compared to full-parametric models, especially for complex distributions and non-linear relationships. Numerical experiments show substantially improved computational efficiency for large, complex datasets.

Conclusion: The proposed coreset construction enables efficient and scalable semi-parametric regression analysis and density estimation, laying foundation for broad applications in statistics and machine learning communities by handling large-scale data while preserving statistical accuracy.
...
### [249] [Quantifying Gate Contribution in Quantum Feature Maps for Scalable Circuit Optimization](https://arxiv.org/abs/2603.19805)
*F. Rodríguez-Díaz,D. Gutiérrez-Avilés,A. Troncoso,F. Martínez-Álvarez*

Main category: cs.LG

TL;DR: GATE methodology optimizes quantum feature maps by removing low-contribution gates using a significance index, reducing circuit size and runtime while maintaining or improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Current quantum devices face limitations from noise, decoherence, and connectivity constraints that hinder efficient execution of feature map-based circuits in quantum machine learning.

Method: GATE uses a gate significance index combining fidelity, entanglement, and sensitivity to quantify gate relevance. It iteratively scans thresholds, removes low-contribution gates, generates optimized QML models, and ranks them based on accuracy, runtime, and balanced performance.

Result: The approach shows consistent reductions in circuit size and runtime with preserved or improved predictive accuracy. Best trade-offs occur at intermediate thresholds rather than baseline or aggressively compressed circuits.

Conclusion: GATE provides an effective circuit optimization methodology for quantum machine learning that addresses current hardware limitations while maintaining model performance.
...
### [250] [Two-Time-Scale Learning Dynamics: A Population View of Neural Network Training](https://arxiv.org/abs/2603.19808)
*Giacomo Borghi,Hyesung Im,Lorenzo Pareschi*

Main category: cs.LG

TL;DR: The paper introduces a theoretical framework for population-based neural network training using two-time-scale dynamics, connecting it to bilevel optimization and replicator-mutator models.


<details>
  <summary>Details</summary>
Motivation: Population-based learning methods (evolutionary strategies, PBT, model-merging) have shown empirical success but lack a comprehensive mathematical framework to describe their collective training dynamics.

Method: Models neural network population as interacting agent system with two-time-scale dynamics: fast noisy gradient updates (SGD/Langevin) for parameters, and slower selection-mutation dynamics for hyperparameters. Proves large-population limit and derives selection-mutation equation for hyperparameter density.

Result: Shows that fast parameter dynamics relax to Boltzmann-Gibbs measure, inducing effective fitness for slow hyperparameter evolution. Connects population-based learning to bilevel optimization and replicator-mutator models, provides conditions for population mean moving toward fittest hyperparameter, and clarifies noise/diversity roles.

Conclusion: The theoretical framework provides mathematical foundation for population-based learning, showing how access to effective fitness can improve population-level updates, with numerical experiments validating the large-population regime and reduced two-time-scale dynamics.
...
### [251] [Eye Gaze-Informed and Context-Aware Pedestrian Trajectory Prediction in Shared Spaces with Automated Shuttles: A Virtual Reality Study](https://arxiv.org/abs/2603.19812)
*Danya Li,Yan Feng,Rico Krueger*

Main category: cs.LG

TL;DR: VR study captures pedestrian interactions with automated shuttles in shared spaces, proposes GazeX-LSTM model using eye gaze and context for behavior prediction, shows super-additive improvements over head orientation alone.


<details>
  <summary>Details</summary>
Motivation: Automated shuttles in shared urban spaces face challenges due to lack of traffic rules and complex pedestrian interactions. Accurate pedestrian behavior prediction is critical for safety and efficiency in these unstructured environments.

Method: Virtual Reality study capturing pedestrian interactions with automated shuttles across diverse scenarios. Proposed GazeX-LSTM model integrates pedestrians' trajectories, fine-grained eye gaze dynamics, and contextual factors for multimodal prediction.

Result: Identified critical pedestrian behavior patterns (hesitation, evasive maneuvers, gaze allocation, proxemic adjustments). Demonstrated eye gaze has unique predictive power over head orientation alone. Combination of eye gaze and contextual information produces super-additive improvements in prediction accuracy.

Conclusion: Eye gaze-informed modeling fundamentally advances pedestrian behavior prediction in shared spaces. Situational contexts play critical role in interactions. Findings enable safer, more adaptive automated vehicle technologies that account for human perception and action in complex shared spaces.
...
### [252] [GDEGAN: Gaussian Dynamic Equivariant Graph Attention Network for Ligand Binding Site Prediction](https://arxiv.org/abs/2603.19817)
*Animesh,Plaban Kumar Bhowmick,Pralay Mitra*

Main category: cs.LG

TL;DR: GDEGAN introduces an adaptive kernel-based attention mechanism for protein binding site prediction, replacing standard dot-product attention to better capture chemical and geometric variations in neighboring residues.


<details>
  <summary>Details</summary>
Motivation: Current equivariant GNN methods use dot-product attention that ignores variations in chemical and geometric properties of neighboring residues, limiting their ability to accurately identify protein binding sites for drug discovery.

Method: Proposes GDEGAN (Gaussian Dynamic Equivariant Graph Attention Network) with adaptive kernels that use statistics of local feature distributions. The mechanism dynamically computes neighborhood statistics at each layer, using local variance as adaptive bandwidth with learnable per-head temperatures for context-specific importance.
...
Conclusion: GDEGAN's adaptive attention mechanism significantly improves binding site prediction accuracy, with direct applications in accelerating protein-ligand docking and therapeutic target identification.
...
### [253] [FIPO: Eliciting Deep Reasoning with Future-KL Influenced Policy Optimization](https://arxiv.org/abs/2603.19835)
...
TL;DR: FIPO is a new RL algorithm that improves reasoning in LLMs by using future-KL divergence for better credit assignment, enabling longer reasoning chains and higher accuracy on math problems.


<details>
  <summary>Details</summary>
Motivation: Standard GRPO-style training with outcome-based rewards suffers from coarse-grained credit assignment that treats all tokens equally, failing to distinguish critical logical pivots from trivial tokens, which imposes a performance ceiling on reasoning capabilities.

Method: FIPO incorporates discounted future-KL divergence into policy updates to create a dense advantage formulation that re-weights tokens based on their influence on subsequent trajectory behavior, enabling finer-grained credit assignment.

Result: FIPO extends average chain-of-thought length from ~4,000 to over 10,000 tokens and increases AIME 2024 Pass@1 accuracy from 50.0% to 58.0% (converging at ~56.0%), outperforming DeepSeek-R1-Zero-Math-32B (~47.0%) and matching/exceeding o1-mini (~56.0%).

Conclusion: Dense advantage formulations are crucial for evolving ORM-based algorithms to unlock the full reasoning potential of base models, and FIPO demonstrates this by overcoming reasoning bottlenecks through better credit assignment.
...
### [254] [What If Consensus Lies? Selective-Complementary Reinforcement Learning at Test Time](https://arxiv.org/abs/2603.19880)
*Dong Yan,Jian Liang,Yanbo Wang,Shuo Lu,Ran He,Tieniu Tan*

Main category: cs.LG

TL;DR: SCRL introduces selective positive and entropy-gated negative pseudo-labeling to improve test-time RL for LLMs, reducing noise amplification from weak consensus in challenging reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing TTRL methods rely only on positive pseudo-labeling, which becomes vulnerable when answer distributions are highly dispersed, leading to weak consensus that reinforces incorrect trajectories as supervision signals.

Method: SCRL develops Selective Positive Pseudo-Labeling with strict consensus criteria to filter unreliable majorities, and introduces Entropy-Gated Negative Pseudo-Labeling as the first negative supervision mechanism in TTRL to prune incorrect trajectories based on generation uncertainty.
...
### [255] [NASimJax: GPU-Accelerated Policy Learning Framework for Penetration Testing](https://arxiv.org/abs/2603.19864)
*Raphael Simon,José Carrasquel,Wim Mees,Pieter Libin*

Main category: cs.LG

TL;DR: NASimJax is a JAX-based reimplementation of Network Attack Simulator achieving 100x speedup, enabling RL training on larger networks for penetration testing with improved generalization.


<details>
  <summary>Details</summary>
Motivation: Existing penetration testing simulators are too slow for training RL policies at scale, resulting in poor generalization to realistic network scenarios.

Method: 1) NASimJax: JAX-based reimplementation for 100x speedup; 2) Formulate as Contextual POMDP; 3) Network generation pipeline for diverse solvable scenarios; 4) Two-stage action decomposition (2SAS) for scaling; 5) Compare Prioritized Level Replay vs Domain Randomization.

Result: Achieved 100x environment throughput, enabled training on networks up to 40 hosts, found Prioritized Level Replay outperforms Domain Randomization at scale, sparser topologies improve OOD generalization, and 2SAS outperforms flat action masking.

Conclusion: NASimJax provides a fast, flexible platform for advancing RL-based penetration testing, enabling study of zero-shot generalization and scaling to realistic network sizes.
...
### [256] [Integrating Meta-Features with Knowledge Graph Embeddings for Meta-Learning](https://arxiv.org/abs/2603.19888)
*Antonis Klironomos,Ioannis Dasoulas,Francesco Periti,Mohamed Gad-Elrab,Heiko Paulheim,Anastasia Dimou,Evgeny Kharlamov*

Main category: cs.LG

TL;DR: KGmetaSP uses knowledge graph embeddings from past ML experiments to improve pipeline performance estimation and dataset similarity tasks, outperforming traditional meta-feature approaches.


<details>
  <summary>Details</summary>
Motivation: Existing meta-learning approaches rely on dataset meta-features but overlook valuable past experimental results and pipeline metadata, limiting their ability to capture dataset-pipeline interactions that reveal performance similarity patterns.

Method: Proposes KGmetaSP, a knowledge-graph-embeddings approach that represents datasets and pipelines in a unified knowledge graph, deriving embeddings that support pipeline-agnostic meta-models for PPE and distance-based retrieval for DPSE.

Result: Validated on a large-scale benchmark of 144,177 OpenML experiments, KGmetaSP enables accurate pipeline performance estimation using a single pipeline-agnostic meta-model and improves dataset similarity estimation over baselines.

Conclusion: KGmetaSP establishes a new reference point for meta-learning by demonstrating how consolidating open experiment data into a unified knowledge graph advances the field, with the approach, knowledge graph, and benchmark released publicly.
...
### [257] [On the Dynamics & Transferability of Latent Generalization during Memorization](https://arxiv.org/abs/2603.19865)
*Simran Ketha,Venkatakrishnan Ramaswamy*

Main category: cs.LG

TL;DR: The paper investigates latent generalization in deep networks during memorization of shuffled labels, tracking its training dynamics and developing linear probes to extract and transfer this hidden generalization to model performance.


<details>
  <summary>Details</summary>
Motivation: Deep networks can memorize shuffled labels but still retain latent generalization abilities in their internal representations. The origin and dynamics of this latent generalization during training, and whether it can be linearly decoded and transferred to improve model generalization, are not well understood.

Method: 1) Track training dynamics of latent generalization during memorization; 2) Analyze MASC probe structure (quadratic classifier); 3) Design new linear probe for latent generalization; 4) Develop method to transfer latent generalization from last-layer representations to model weights via the linear probe.

Result: Latent generalization peaks early in training alongside model generalization. The MASC probe is quadratic/non-linear, prompting investigation of linear decodability. A new linear probe is designed, enabling transfer of latent generalization to model generalization through weight editing.

Conclusion: Deep networks retain latent generalization during memorization that peaks early in training. This generalization can be linearly decoded from layerwise outputs and transferred to improve model generalization, revealing hidden capabilities in memorizing networks.
...
### [258] [Discovery of Decision Synchronization Patterns from Event Logs](https://arxiv.org/abs/2603.19879)
*Tijmen Kuijpers,Karolin Winter,Remco Dijkman*

Main category: cs.LG

TL;DR: Proposes approach for discovering decision synchronization patterns in business processes that consider multiple cases simultaneously, addressing a gap in current process discovery techniques.


<details>
  <summary>Details</summary>
Motivation: Decision synchronization patterns in business processes facilitate fair resource use, case prioritization, and prevent unnecessary waiting, but current process discovery techniques rarely address mechanisms that consider properties of multiple cases simultaneously.

Method: Proposes approach inspired by supply chain processes to discover decision synchronization patterns, which combine specific process constructs with constraints determining which case to execute. Formalizes and demonstrates discovery of constraints for four such patterns.

Result: Evaluated in two artificial scenarios: (1) four separate process models each with single pattern - successfully discovered all pattern types; (2) process model containing all four patterns - approach generalized to complex problems. Both scenarios reliably retrieved expected patterns.

Conclusion: The approach effectively discovers decision synchronization patterns that consider multiple cases, addressing a gap in current process discovery techniques and demonstrating reliability in both simple and complex scenarios.
...
### [259] [Graph2TS: Structure-Controlled Time Series Generation via Quantile-Graph VAEs](https://arxiv.org/abs/2603.19970)
*Shaoshuai Du,Joze M. Rozanec,Andy Pimentel,Ana-Lucia Varbanescu*

Main category: cs.LG

TL;DR: Graph2TS: A quantile-graph conditioned VAE that separates time series into structural backbone and stochastic residuals, enabling structure-controlled generation with better global temporal organization.


<details>
  <summary>Details</summary>
Motivation: Existing generative models struggle to balance global temporal structure preservation with stochastic local variations, especially for volatile signals with weak/irregular periodicity. Direct distribution matching can amplify noise or suppress meaningful patterns.

Method: Proposes structure-residual perspective: time series = structural backbone + stochastic residuals. Uses quantile-based transition graph to capture global distributional/temporal dependencies. Develops Graph2TS, a quantile-graph conditioned VAE for cross-modal generation from structural graphs to time series.
...
### [260] [Memori: A Persistent Memory Layer for Efficient, Context-Aware LLM Agents](https://arxiv.org/abs/2603.19935)
*Luiz C. Borro,Luiz A. B. Macarini,Gordon Tindall,Michael Montero,Adam B. Struck*

Main category: cs.LG

TL;DR: Memori is an LLM-agnostic persistent memory layer that converts unstructured dialogue into semantic triples and summaries, achieving 81.95% accuracy on LoCoMo benchmark while using only 5% of full context tokens.


<details>
  <summary>Details</summary>
Motivation: Current memory approaches for LLM agents force vendor lock-in and rely on injecting large volumes of raw conversation into prompts, leading to high token costs and degraded performance.

Method: Memori treats memory as a data structuring problem with an Advanced Augmentation pipeline that converts unstructured dialogue into compact semantic triples and conversation summaries for precise retrieval.

Result: Achieves 81.95% accuracy on LoCoMo benchmark, uses only 1,294 tokens per query (~5% of full context), with 67% fewer tokens than competing approaches and over 20x savings compared to full-context methods.

Conclusion: Effective memory in LLM agents depends on structured representations instead of larger context windows, enabling scalable and cost-efficient deployment of autonomous agents.
...
### [261] [Breaking the Capability Ceiling of LLM Post-Training by Reintroducing Markov States](https://arxiv.org/abs/2603.19987)
*Yurun Yuan,Tengyang Xie*

Main category: cs.LG

TL;DR: RL for LLMs faces a capability ceiling due to using expanding history as state instead of compact Markov states. Introducing explicit Markov states reduces sample complexity and breaks performance boundaries in complex reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Current RL approaches for LLM post-training face a persistent "capability ceiling" - they merely refine existing patterns rather than enabling novel strategy discovery like classical RL systems. This is due to a structural bottleneck where LLM RL uses ever-expanding action histories as state instead of compact Markov states.

Method: Reintroduce explicit Markov states into LLM post-training, moving beyond "history-as-state" modeling. Theoretically analyze sample complexity benefits, and empirically test the approach on complex logic puzzles to demonstrate performance improvements.

Result: Theoretical guarantees show Markov states significantly reduce sample complexity. Empirical results demonstrate that Markov state-based RL consistently breaks performance boundaries of standard RL post-training across complex logic puzzles.

Conclusion: Moving beyond history-as-state modeling to structured Markovian representations is essential for unlocking open-ended discovery and genuinely new reasoning capabilities in Generative AI, enabling LLMs to discover novel strategies rather than just refining existing patterns.
...
### [262] [Model-Driven Learning-Based Physical Layer Authentication for Mobile Wi-Fi Devices](https://arxiv.org/abs/2603.19972)
*Yijia Guo,Junqing Zhang,Yao-Win Peter Hong,Stefano Tomasin*

Main category: cs.LG

TL;DR: The paper proposes LiteNP-Net, a lightweight neural network for physical layer authentication that combines the theoretical optimality of Neyman-Pearson hypothesis testing with practical deep learning approaches to address IoT security challenges.


<details>
  <summary>Details</summary>
Motivation: IoT faces authentication risks due to wireless broadcast nature. Traditional hypothesis testing (NP detector) is theoretically optimal but impractical due to reliance on channel statistics, while deep learning approaches are practical but not optimal. Need to bridge this gap.

Method: Proposed learning-based PLA scheme driven by hypothesis testing: 1) Incorporated conditional statistical models into hypothesis testing to derive theoretically optimal NP detector, 2) Developed LiteNP-Net, a lightweight neural network driven by the NP detector structure, 3) Conducted simulations and experimental evaluations using Wi-Fi IoT development kits in real-world scenarios.

Result: Simulations showed LiteNP-Net approaches NP detector performance without prior channel statistics knowledge. Experimental results demonstrated LiteNP-Net outperforms conventional correlation-based methods and state-of-the-art Siamese-based methods in practical Wi-Fi environments.

Conclusion: LiteNP-Net successfully bridges the gap between theoretical optimality and practical implementation in physical layer authentication, providing an effective solution for IoT security that combines the strengths of both hypothesis testing and deep learning approaches.
...
### [263] [Fine-tuning Timeseries Predictors Using Reinforcement Learning](https://arxiv.org/abs/2603.20063)
*Hugo Cazaux,Ralph Rudd,Hlynur Stefánsson,Sverrir Ólafsson,Eyjólfur Ingi Ásgeirsson*

Main category: cs.LG

TL;DR: The paper presents three RL algorithms for fine-tuning financial forecasters, showing performance improvements and transfer learning benefits after fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To improve financial forecasting models by applying reinforcement learning fine-tuning to models initially trained with supervised learning, and to provide practical implementation guidance for practitioners.

Method: Proposes a clear implementation plan for backpropagating RL loss to supervised learning models, uses three major RL algorithms for fine-tuning financial forecasters, and compares performance before and after fine-tuning.

Result: Found increased performance after fine-tuning and discovered transfer learning properties in the models, demonstrating the benefits of the RL fine-tuning approach.

Conclusion: RL fine-tuning improves financial forecasting models, provides transfer learning benefits, and the paper offers practical tuning guidance for practitioners' future implementations.
...
### [264] [A Super Fast K-means for Indexing Vector Embeddings](https://arxiv.org/abs/2603.20009)
*Leonardo Kuffo,Sven Hepkema,Peter Boncz*

Main category: cs.LG

TL;DR: SuperKMeans is a fast k-means variant for clustering high-dimensional embeddings, achieving up to 7x speedup over FAISS/Scikit-Learn on CPUs and 4x over cuVS on GPUs while maintaining centroid quality for similarity search.


<details>
  <summary>Details</summary>
Motivation: Traditional k-means clustering for high-dimensional vector embeddings is computationally expensive, creating bottlenecks for similarity search tasks that require efficient clustering of large embedding collections.

Method: Uses dimension pruning to reduce data-access and compute overhead by identifying and eliminating unnecessary dimensions for centroid assignment. Also introduces Early Termination by Recall mechanism that stops iterations when centroid quality for retrieval tasks stops improving.

Result: Achieves 7x faster clustering than FAISS and Scikit-Learn on modern CPUs, 4x faster than cuVS on GPUs, while maintaining centroid quality for vector similarity search. Early termination further reduces runtime without compromising retrieval quality.

Conclusion: SuperKMeans provides significant speed improvements for clustering high-dimensional embeddings while preserving retrieval quality, making it practical for real-world similarity search applications. The implementation is open-sourced for community use.
...
### [265] [AgenticRS-EnsNAS: Ensemble-Decoupled Self-Evolving Architecture Search](https://arxiv.org/abs/2603.20014)
*Yun Chen,Moyu Zhang,Jinxin Hu,Yu Zhang,Xiaoyi Zeng*

Main category: cs.LG

TL;DR: Proposes Ensemble-Decoupled Architecture Search to reduce NAS validation cost from O(M) to O(1) by predicting ensemble performance from single-learner evaluation, enabling faster architecture iteration in industrial systems.


<details>
  <summary>Details</summary>
Motivation: Industrial NAS faces prohibitive validation costs: evaluating each candidate architecture requires testing the full ensemble (M=50-200 models), creating a bottleneck that limits architecture iteration frequency in production systems.

Method: Develops Ensemble-Decoupled Theory with a sufficient condition for monotonic ensemble improvement. Uses lightweight dual-learner training to estimate key metrics (ΔE, ρ, σ²), enabling performance prediction without full ensemble training. Provides three solution strategies: closed-form optimization, constrained differentiable optimization, and LLM-driven search.

Result: Reduces per-candidate search cost from O(M) to O(1) while maintaining O(M) deployment cost only for validated winners. Identifies two orthogonal improvement mechanisms: base diversity gain and accuracy gain for industrial NAS design.

Conclusion: The framework decouples architecture search from ensemble training, enabling practical NAS deployment in industrial systems with large ensembles. Theoretical foundations are rigorous, with empirical validation planned for journal extension.
...
### [266] [The $\mathbf{Y}$-Combinator for LLMs: Solving Long-Context Rot with $λ$-Calculus](https://arxiv.org/abs/2603.20105)
*Amartya Roy,Rasul Tutunov,Xiaotong Ji,Matthieu Zimmer,Haitham Bou-Ammar*

Main category: cs.LG

TL;DR: λ-RLM replaces open-ended recursive code generation with typed functional runtime for long-context reasoning, providing formal guarantees and better performance.


<details>
  <summary>Details</summary>
Motivation: Existing RLMs use open-ended REPL with arbitrary control code, making execution difficult to verify, predict, and analyze. Need more structured approach for reliable long-context reasoning.

Method: Uses typed functional runtime grounded in λ-calculus with pre-verified combinators. Neural inference only on bounded leaf subproblems, turning recursion into structured functional program with explicit control flow.

Result: Outperforms standard RLM in 29/36 model-task comparisons, improves accuracy by up to +21.9 points, reduces latency by up to 4.1x across four long-context reasoning tasks and nine base models.

Conclusion: Typed symbolic control yields more reliable and efficient foundation for long-context reasoning than open-ended recursive code generation, with formal guarantees including termination and cost bounds.
...
### [267] [ODySSeI: An Open-Source End-to-End Framework for Automated Detection, Segmentation, and Severity Estimation of Lesions in Invasive Coronary Angiography Images](https://arxiv.org/abs/2603.20021)
...
TL;DR: ODySSeI is an open-source framework for automated detection, segmentation, and severity estimation of coronary lesions in ICA images, using deep learning with pyramidal augmentation and achieving real-time performance.


<details>
  <summary>Details</summary>
Motivation: ICA interpretation is subjective and prone to operator variability, creating a need for automated, objective, and reproducible analysis tools for coronary artery disease assessment.

Method: Combines deep learning-based lesion detection and segmentation models trained with Pyramidal Augmentation Scheme (PAS), plus a quantitative angiography-free Lesion Severity Estimation technique that computes MLD and diameter stenosis from predicted lesion geometry.

Result: PAS yields 2.5-fold increase in lesion detection performance; LSE achieves high accuracy (±2-3 pixels from ground truth); processes images in seconds on CPU/fraction of second on GPU; demonstrates strong generalizability across diverse patient cohorts.
...
### [268] [Var-JEPA: A Variational Formulation of the Joint-Embedding Predictive Architecture -- Bridging Predictive and Generative Self-Supervised Learning](https://arxiv.org/abs/2603.20111)
*Moritz Gögl,Christopher Yau*

Main category: cs.LG

TL;DR: Var-JEPA reformulates JEPA as a variational latent-variable model, showing JEPA is implicitly generative and deriving a principled probabilistic version that eliminates need for ad-hoc regularization while enabling uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: The paper challenges the view that JEPA is fundamentally non-generative, arguing that its separation from probabilistic generative modeling is rhetorical rather than structural. The authors aim to bridge this gap by showing JEPA can be understood as a deterministic specialization of latent-variable models and developing a principled probabilistic variant.

Method: The authors derive Variational JEPA (Var-JEPA) by applying variational inference to coupled latent-variable models, showing JEPA's architecture mirrors variational posteriors and learned conditional priors. Var-JEPA optimizes a single Evidence Lower Bound (ELBO), making the latent generative structure explicit. They instantiate this for tabular data as Var-T-JEPA.

Result: Var-JEPA learns meaningful representations without ad-hoc anti-collapse regularizers and enables principled uncertainty quantification in latent space. For tabular data (Var-T-JEPA), the approach achieves strong representation learning and downstream performance, consistently improving over T-JEPA while remaining competitive with strong raw-feature baselines.

Conclusion: JEPA is fundamentally connected to probabilistic generative modeling, and its probabilistic formulation (Var-JEPA) provides a principled framework that eliminates heuristic regularization while maintaining strong performance and enabling uncertainty quantification, bridging the gap between predictive and generative self-supervised learning approaches.
...
### [269] [Continual Learning as Shared-Manifold Continuation Under Compatible Shift](https://arxiv.org/abs/2603.20036)
*Henry J. Kobs*

Main category: cs.LG

TL;DR: SPMA-OG improves continual learning by preserving latent geometry through anchor regularization and sparse replay, maintaining old-task performance while learning new tasks on shared latent supports.


<details>
  <summary>Details</summary>
Motivation: Existing continual learning methods (parameter regularization, output matching, replay) reduce forgetting but don't directly control how latent representations evolve. The paper proposes a geometric approach for scenarios where old and new data should remain on the same latent support.

Method: Support-Preserving Manifold Assimilation (SPMA) with geometry-preserving variant SPMA-OG combines: 1) sparse replay, 2) output distillation, 3) relational geometry preservation, 4) local smoothing, and 5) chart-assignment regularization on old anchors to preserve latent manifold structure.

Result: On CIFAR10 and Tiny-ImageNet with compatible shifts, SPMA-OG improves old-task retention and representation preservation over sparse replay baselines while maintaining competitive new-task accuracy. On synthetic atlas-manifold benchmark, achieves near-perfect anchor-geometry preservation and improves new-task accuracy over replay.

Conclusion: Geometry-aware anchor regularization is a useful inductive bias for continual learning when preserving a shared latent support rather than creating a new one. The geometric approach effectively maintains old knowledge while learning new tasks on the same manifold.
...
### [270] [Federated Hyperdimensional Computing for Resource-Constrained Industrial IoT](https://arxiv.org/abs/2603.20037)
*Nikita Zeulin,Olga Galinina,Nageen Himayat,Sergey Andreev*

Main category: cs.LG

TL;DR: Federated hyperdimensional computing enables lightweight collaborative learning for resource-constrained IIoT devices by exchanging only prototype representations, reducing communication overhead while maintaining fast convergence.


<details>
  <summary>Details</summary>
Motivation: IIoT devices face strict constraints in memory, compute capability, and wireless bandwidth, making deployment of advanced analytics like predictive maintenance challenging. There's a need for lightweight learning paradigms that can operate effectively in these resource-constrained environments.

Method: Integrates hyperdimensional computing (HDC) into a federated learning framework. HDC leverages high-dimensional vector spaces for energy-efficient training and inference. In the federated approach, devices exchange only prototype representations rather than full models, significantly reducing communication overhead.

Result: Numerical results demonstrate that federated HDC supports collaborative learning in IIoT with fast convergence speed and communication efficiency. The approach shows potential for lightweight and resilient distributed intelligence in large-scale, resource-constrained environments.

Conclusion: HDC represents a promising lightweight and resilient framework for distributed intelligence in large-scale IIoT systems, enabling efficient collaborative learning while addressing the strict resource constraints of edge devices.
...
### [271] [How Out-of-Equilibrium Phase Transitions can Seed Pattern Formation in Trained Diffusion Models](https://arxiv.org/abs/2603.20092)
*Luca Ambrogioni*

Main category: cs.LG

TL;DR: Diffusion models exhibit out-of-equilibrium phase transitions during generation, with a critical regime where small fluctuations amplify into large-scale structure, enabling pattern formation beyond training data.


<details>
  <summary>Details</summary>
Motivation: To understand the generation process in trained diffusion models through the lens of non-equilibrium physics, specifically phase transitions and critical phenomena, rather than viewing it as smooth noise-to-data evolution.

Method: Developed theoretical framework using analytically tractable patch score models, connected to Ginzburg-Landau field theories, and validated with empirical analysis of trained convolutional diffusion models. Used targeted perturbations (classifier-free guidance pulses) at estimated critical times.

Result: Found that diffusion models pass through a critical regime with mode softening, growing correlation lengths, and spatial pattern emergence. Targeted perturbations at critical times significantly improve generation control, demonstrating practical relevance.

Conclusion: Non-equilibrium critical phenomena provide a unifying principle for understanding and potentially improving diffusion models, connecting architectural constraints to collective spatial pattern formation beyond memorization.
...
### [272] [Trojan horse hunt in deep forecasting models: Insights from the European Space Agency competition](https://arxiv.org/abs/2603.20108)
...
TL;DR: The paper presents a data science competition focused on detecting trojan horse attacks in deep forecasting models for spacecraft telemetry, where participants identified hidden triggers that manipulate predictions when activated.


<details>
  <summary>Details</summary>
Motivation: Deep forecasting models in safety-critical applications like space operations are vulnerable to trojan horse attacks, where backdoors can be implanted to manipulate predictions when triggered, creating serious security risks.

Method: Organized a Kaggle competition (Trojan Horse Hunt) with over 200 teams tasked with identifying hidden triggers in deep forecasting models for spacecraft telemetry, using a novel task formulation, benchmark set, and evaluation protocol.

Result: The competition successfully engaged 200+ teams to develop solutions for trigger identification, with best solutions analyzed and key insights summarized about detecting trojan attacks in time series forecasting models.

Conclusion: The competition provides valuable materials and insights for improving security in deep forecasting models, with all resources publicly available to advance research on trojan detection in time series applications.
...
### [273] [Conditioning Protein Generation via Hopfield Pattern Multiplicity](https://arxiv.org/abs/2603.20115)
*Jeffrey D. Varner*

Main category: cs.LG

TL;DR: A method to bias protein sequence generation toward functional subsets using a single attention bias parameter, enabling expansion of small experimental datasets into diverse candidate libraries without retraining.


<details>
  <summary>Details</summary>
Motivation: Current protein sequence generation treats all stored sequences equally and cannot direct generation toward specific functional subsets of interest, limiting practical utility for targeted design applications.

Method: Adds a single scalar parameter as bias to sampler's attention logits to continuously shift generation from full family toward user-specified subset, requiring only a small set of sequences and multiplicity ratio, with no model retraining or architecture changes.

Result: Method enables exact conditioning at sampler's internal representation level, though decoded sequence phenotype may fall short due to calibration gap from encoding dimensionality reduction. Experiments on five Pfam families show monotonic relationship between geometric separation and calibration gap. Applied to omega-conotoxin peptides, generated over 1000 candidates preserving pharmacophore and binding determinants.

Conclusion: Stochastic attention enables practitioners to expand small sets of experimentally characterized sequences into diverse candidate libraries without retraining generative models, with conditioning effectiveness predictable from encoding geometry.
...
### [274] [Revisiting Gene Ontology Knowledge Discovery with Hierarchical Feature Selection and Virtual Study Group of AI Agents](https://arxiv.org/abs/2603.20132)
*Cen Wan,Alex A. Freitas*

Main category: cs.LG

TL;DR: Proposes an agentic AI-based virtual study group framework for discovering aging-related biological knowledge from Gene Ontology terms using hierarchical feature selection and validation against existing literature.


<details>
  <summary>Details</summary>
Motivation: To leverage agentic AI techniques to enhance scientific discovery pipelines, specifically for extracting meaningful aging-related biological knowledge from Gene Ontology data across multiple model organisms.

Method: Develops a novel agentic AI-based knowledge discovery-oriented virtual study group that uses hierarchical feature selection methods to identify highly aging-related Gene Ontology terms, then applies the framework across four different model organisms.

Result: Majority of AI agent-generated scientific claims are supported by existing literature, and the proposed internal mechanisms of the virtual study group prove important for the knowledge discovery framework.

Conclusion: Agentic AI-based virtual study groups show promise for biological knowledge discovery, with validation confirming the reliability of AI-generated scientific claims about aging mechanisms.
...
### [275] [Beyond Single Tokens: Distilling Discrete Diffusion Models via Discrete MMD](https://arxiv.org/abs/2603.20155)
*Emiel Hoogeboom,David Ruhe,Jonathan Heek,Thomas Mensink,Tim Salimans*

Main category: cs.LG

TL;DR: D-MMD enables effective distillation of discrete diffusion models, maintaining quality/diversity and sometimes outperforming teachers, unlike previous methods that collapse.


<details>
  <summary>Details</summary>
Motivation: Discrete diffusion models lack effective distillation methods compared to continuous diffusion models, which have many approaches to reduce sampling steps significantly.

Method: Discrete Moment Matching Distillation (D-MMD) adapts successful continuous domain distillation ideas to discrete diffusion models.

Result: D-MMD maintains high quality and diversity with sufficient sampling steps (unlike previous methods that collapse), works on text and image datasets, and can outperform teacher models.

Conclusion: D-MMD successfully bridges the distillation gap between continuous and discrete diffusion models, enabling efficient sampling while preserving performance.
...
### [276] [Kolmogorov-Arnold causal generative models](https://arxiv.org/abs/2603.20184)
*Alejandro Almodóvar,Mar Elizo,Patricia A. Apellániz,Santiago Zazo,Juan Parras*

Main category: cs.LG

TL;DR: KaCGM is a causal generative model using Kolmogorov-Arnold Networks for transparent causal mechanisms in tabular data, enabling interpretable structural equations while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Deep causal models often use opaque architectures that limit auditability in high-stakes domains, creating a need for transparent causal mechanisms that can be inspected while preserving generative capabilities.

Method: Proposes KaCGM where each structural equation is parameterized by a Kolmogorov-Arnold Network (KAN), enabling direct inspection of learned mechanisms. Includes validation pipeline using distributional matching and independence diagnostics of exogenous variables.

Result: Competitive performance against state-of-the-art methods on synthetic/semi-synthetic benchmarks. Real-world cardiovascular case study demonstrates extraction of simplified structural equations and interpretable causal effects.

Conclusion: Expressive causal generative modeling and functional transparency can be achieved jointly, supporting trustworthy deployment in tabular decision-making settings through interpretable structural equations.
