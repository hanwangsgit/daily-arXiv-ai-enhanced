...
TL;DR: Proposes C-Norm method to normalize cell distribution in TCT images, combined with YOLOv12 and DINOv3 for improved cervical cancer screening, achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: Manual TCT reading is time-consuming and inconsistent; existing AI models fail under real clinical conditions due to unbalanced cell distribution and limited high-quality annotated data.

Method: C-Norm decouples abnormal/normal cells and re-synthesizes images for uniform distribution; integrates YOLOv12 detection with DINOv3 feature extraction.

Result: Achieves state-of-the-art performance, significantly outperforming mainstream detection algorithms on TCT images.

Conclusion: The proposed approach effectively addresses distribution bias and limited annotations, leading to superior detection for cervical cancer screening.
...
TL;DR: Bring Music The Horizon is an emotion-aware pipeline that generates immersive 360° music visualization videos by predicting the emotional trajectory (valence-arousal) of a song every four bars, converting that into visual guidance via EmotiCrafter and SEGA for fine-grained control, then using image-to-video models for temporal continuity.


<details>
  <summary>Details</summary>
Motivation: Existing music visualization approaches either rely heavily on lyrics or produce flat, non-immersive videos that fail to convey the emotional dynamics of music or provide an immersive listening experience.

Method: (1) Predict emotional trajectory by estimating valence-arousal values every four bars; (2) Convert these into emotion-aware visual guidance using EmotiCrafter, manipulated by SEGA for fine-grained semantic control in keyframe generation; (3) Apply image-to-video models to generate temporally continuous 360° videos.

Result: The pipeline generates 360° music visualization videos that reflect the emotional progression and temporal structure of the input song, demonstrated across different music genres.

Conclusion: The proposed emotion-aware pipeline effectively generates immersive 360° videos that capture musical emotional dynamics without relying on lyrics, outperforming or complementing existing baselines like From-Sound-To-Sight.
...
TL;DR: Boogu-Image-0.1 is an open-source family of unified multimodal models (Base, Turbo, Edit, Edit-Turbo) that achieves strong text-to-image generation, editing, and bilingual text rendering performance with only ~$400K training cost, matching/outperforming other open-source models and approaching closed-source systems.


<details>
  <summary>Details</summary>
Motivation: To advance open-source multimodal understanding and generation by demonstrating that targeted improvements in model understanding, data quality, training pipelines, and inference-time scaling can achieve competitive performance even under constrained compute budgets, while closed-source systems' internal practices remain undisclosed.

Method: 1) Targeted improvements in model understanding, data quality, and training pipelines. 2) Agentic inference-time scaling. 3) Four model variants (Base, Turbo, Edit, Edit-Turbo) for different use cases. 4) Training on 208.62 million unique images with ~$400K theoretical cost.

Result: Consistently matches or surpasses other open-source models on standard benchmarks, approaches leading closed-source systems (Nano-Banana-Pro, GPT-Image-2), achieves high-quality text-to-image generation, fast inference, instruction-based editing, and bilingual text rendering.

Conclusion: Open-source unified multimodal models can achieve strong performance under constrained compute budgets through targeted improvements; the authors release weights, code, and recipes under Apache 2.0 to advance the open ecosystem.
...
TL;DR: A Transformer-based Masked Autoencoder pretrained with 75% masking and an auxiliary defect localization objective learns representations of steel surface defects, achieving 91.3% clustering accuracy via UMAP + Agglomerative clustering on six defect categories, enabling unsupervised grouping when labeled data is scarce.


<details>
  <summary>Details</summary>
Motivation: Labeled defect data is scarce and costly in steel surface inspection, while unlabeled images are abundant, motivating self-supervised methods that learn useful representations without class labels.

Method: A Transformer-based Masked Autoencoder pretrains by masking 75% of input patches; the encoder is jointly trained with an auxiliary defect localization objective. Features are then clustered via UMAP + Agglomerative clustering.
...
Conclusion: Self-supervised representation learning via masked autoencoding with a defect localization auxiliary task is effective for unsupervised grouping of steel surface defects, reducing reliance on expensive labeled data.
...
TL;DR: MGFace is a mask-gated face identification pipeline that detects whether a query face is masked, applies global matching for unmasked faces, and only uses costly patch-level re-ranking for masked queries, achieving 80-90%+ accuracy with ~20x speedup over prior methods.


<details>
  <summary>Details</summary>
Motivation: Face identification accuracy degrades significantly under partial occlusion (especially facial masks), and existing re-ranking methods improve robustness via patch-level matching but are computationally expensive for large-scale retrieval.

Method: MGFace uses a mask status predictor to distinguish masked vs. unmasked queries, applies lightweight global embedding matching for unmasked queries, and activates mask-aware patch-level re-ranking (focusing on upper-face regions) only for masked queries.

Result: Over 80% identification accuracy with FaceNet, over 90% with ArcFace on LFW-Mask dataset; ~20x query time reduction compared to EMD-based re-ranking while achieving better performance.

Conclusion: MGFace effectively balances accuracy and efficiency for masked-face identification by conditionally routing computation based on mask status, making it suitable for large-scale retrieval scenarios.
...
TL;DR: This paper compares pretraining-finetuning (PFT) vs. joint training (JT) of self-supervised and supervised objectives across 8 SSL methods and diverse vision domains, finding JT improves data/training efficiency and works well in low-label settings, while PFT is more reliable for specialized domains.


<details>
  <summary>Details</summary>
Motivation: The interaction between self-supervised and supervised learning objectives during optimization remains insufficiently understood, despite the proven effectiveness of the two-stage SSL pipeline (pretrain then finetune).

Method: Systematically compares two training paradigms — pretraining followed by finetuning (PFT) and joint training (JT) — across 8 SSL methods and diverse tasks (natural, medical, crisis response, remote sensing data) under varying labeled data percentages.

Result: JT consistently improves data and training efficiency and is robust in low-label settings; PFT is more reliable in specialized domains. Effectiveness depends on task, labeled data availability, and domain complexity.

Conclusion: Provides a comprehensive empirical benchmark for hybrid SSL-based semi-supervised learning and offers practical guidance for choosing between PFT and JT across diverse vision applications.
...
TL;DR: Static deepfake detectors fail on real-world content (45-50% AUC drops). BitMind Forensics (BMF) uses an open adversarial competition to continually update its training data, achieving 0.936 AUC on Sumsub, matches best commercial (0.915 vs 0.90) on Deepfake-Eval-2024 images, 0.991 on a 21-generator AI-image panel, and improves over time (image detection from 0.842 to 0.902 AUC).


<details>
  <summary>Details</summary>
Motivation: Static deepfake detectors show near-perfect academic scores but collapse (45-50% AUC drops) on real-world content, revealing a structural gap: they are trained once against a rapidly evolving generative frontier.

Method: BMF is trained through Bittensor SN34, an open adversarial competition that continuously refreshes the training distribution with emerging deepfake generators.

Result: 0.936 AUC on Sumsub original images, 0.872 pooled AUC across four-condition manipulation battery; 0.915 AUC (images) and 0.822 AUC (video) on Deepfake-Eval-2024, matching/exceeding best commercial; 0.991 on 21-generator AI-image panel; 0.918 on GenVidBench; temporal study shows improvement from 0.842 to 0.902 (image) and 0.864 to 0.936 (video).

Conclusion: Continuous adversarial competition effectively bridges the gap between academic and real-world deepfake detection performance.
...
TL;DR: A human-in-the-loop framework combining active learning and dual-loss optimization reduces annotation effort for surgical tool segmentation by 50%, using foundation models and iterative pseudo-mask refinement.


<details>
  <summary>Details</summary>
Motivation: Precise spatial-temporal annotation of laparoscopic videos is extremely time-consuming and requires expert knowledge, creating a bottleneck for developing automated surgical tool segmentation models.

Method: The framework combines (1) dual-loss optimization: weak supervision loss on video-level tool labels + image-level mask loss on human-corrected annotations, (2) a foundation model generating temporally consistent CAMs, and (3) iterative active learning where pseudo-masks guide expert refinement.

Result: Reduces annotation effort by 50% compared to fully manual annotation by the end of training, eliminating the need for large fully annotated datasets upfront.

Conclusion: This iterative human-in-the-loop approach enables scalable, practical deployment of surgical tool segmentation models to larger, more diverse datasets and real-world clinical settings with minimal expert input.
...
TL;DR: JITOMA is a closed-loop framework that replaces exhaustive 3D scene graph construction with just-in-time, on-demand memory activation, using a task heatmap and LLM-driven queries to reduce observation redundancy and latency on edge platforms.


<details>
  <summary>Details</summary>
Motivation: Conventional 3D Scene Graph pipelines build everything first then filter later, which causes perceptual saturation (severe observation redundancy) and fails to meet the real-time, low-latency requirements of edge platforms.

Method: JITOMA uses a top-down task heatmap at the frontend to filter continuous observations, maintaining low-cost dormant anchors. Upon cognitive queries, an LLM parses robotic intent to dynamically awaken task-relevant anchors and trigger dense operations only within activated local subgraphs.

Result: JITOMA substantially reduces active graph size and captioning latency while maintaining stable processing time under long-horizon task switching, as evaluated on JITOMA-Bench.

Conclusion: The just-in-time activation paradigm effectively resolves the perceptual saturation problem in 3DSG construction, enabling efficient long-horizon multi-tasking on resource-constrained edge platforms.
...
TL;DR: AffectFlow-DINO extends a deterministic facial behavior model with a conditional rectified-flow head for uncertainty-aware predictions, achieving P_MTL=1.177 (vs. baseline 0.45) on the ABAW challenge through multi-task learning of valence-arousal, expressions, and Action Units.


<details>
  <summary>Details</summary>
Motivation: Facial behavior in-the-wild is inherently ambiguous; standard deterministic models fail to capture one-to-many prediction possibilities, motivating a generative, uncertainty-aware approach.

Method: A frozen DINOv3 ViT-S/16 backbone is used with a conditional rectified-flow head for generative decoding, enabling Monte Carlo sampling. Multi-task learning jointly estimates valence-arousal, 8 facial expressions, and 12 Action Units. Post-hoc threshold calibration addresses class imbalance.

Result: Rectified-flow decoding improves CCC-V by +0.058 for valence-arousal. Post-hoc calibration recovers Fear from 3.8% to 33.1%. Final P_MTL=1.177 substantially outperforms the baseline (0.45).

Conclusion: Conditional rectified-flow decoding combined with threshold calibration effectively models ambiguity and imbalance in facial behavior analysis, significantly advancing state-of-the-art on the ABAW benchmark.
...
TL;DR: The paper proposes a polarization-aware differentiable path tracing method that handles rank-deficient polarimetric operators (like linear polarizers and diffuse reflections) using path replay with local caching, enabling stable gradient estimation for inverse rendering problems involving polarization cues.


<details>
  <summary>Details</summary>
Motivation: Existing differentiable rendering methods operate only on radiometric intensity, discarding valuable polarization cues that provide constraints on scene geometry and material properties. Standard gradient estimators fail due to the rank-deficient nature of common polarimetric operators violating invertibility assumptions.

Method: The method combines path replay backpropagation with local caching to estimate unbiased gradients through polarization-aware differentiable path tracing, handling rank-deficient polarimetric operators without numerical instability.

Result: The approach enables efficient and stable optimization of material and lighting parameters in complex scenes, demonstrating robust gradient estimation where standard methods fail.

Conclusion: This work broadens the applicability of physically based inverse rendering by incorporating polarization information through a robust differentiable rendering framework, opening new possibilities for inverse rendering with polarization cues.
...
TL;DR: FOLIO is a training-free semantic memory system for streaming video that dynamically retains focused entity details while compacting context, achieving SOTA results on OVO-Bench and StreamingBench.


<details>
  <summary>Details</summary>
Motivation: Streaming video understanding faces unbounded frame growth; retaining all history is memory-inefficient, yet later queries often depend on specific entities, actions, and temporal changes, creating a need for selective, structured memory.

Method: FOLIO uses a dynamic focus state, a short-term visual buffer, and a long-term semantic memory organized around entities with a visual-evidence cache. At query time, lightweight hybrid retrieval combines direct matching over structured memory with semantic query expansion.

Result: 82.0/69.1 Perception/Backward accuracy on OVO-Bench with Qwen3-VL-8B, and 74.5 overall accuracy on StreamingBench, with substantially reduced memory cost.

Conclusion: Training-free selective memory compression focused on entities is effective and efficient for streaming video understanding, achieving SOTA while reducing memory overhead.
...
TL;DR: The paper proposes FD-loss to fine-tune diffusion models for better medical image generation of heterogeneous tumors, improving downstream segmentation by over 5% DSC.


<details>
  <summary>Details</summary>
Motivation: Diffusion models struggle to capture complex morphological characteristics of heterogeneous tumors with irregular boundaries due to the standard per-pixel denoising objective that smooths high-variance structures.

Method: Fine-tuning generative diffusion models with Fréchet Distance loss (FD-loss), which aligns first and second order feature statistics of real and generated images in a pretrained encoder space.

Result: Downstream segmentation networks trained on FD-regularized synthetic data improve tumor DSC by >5% over unregularized synthetic augmentation, with more faithful tumor synthesis and fewer segmentation hallucinations.
...
TL;DR: This paper introduces the Visual Dependency Gap (VDG) metric to audit whether video LLM accuracy on benchmarks reflects genuine visual understanding, finding that many models achieve strong scores without depending on visual information.


<details>
  <summary>Details</summary>
Motivation: The paper questions an implicit assumption in video LLM research: that benchmark accuracy necessarily indicates visual understanding. The authors aim to develop a diagnostic tool to separate genuine visual reasoning from language-only performance.

Method: The authors define VDG as the per-question accuracy difference between original-video and black-screen conditions. They use paired McNemar tests across 20 models, create a diagnostic ladder (black screen → single frame → shuffled frames → original video), run FPS ablations (0.5-24), and conduct H.264 experiments to detect answer flips.

Result: Models differ significantly on original video (p=0.0003) but not on black screens (p=0.53). Frame diversity drives most visual benefit; temporal order contributes near-zero accuracy. FPS ablation rules out sparse sampling. Bidirectional answer flips are hidden by stable aggregate accuracy. VDG varies widely (0.025-0.315) across API models.

Conclusion: Accuracy and visual dependency are separable. VDG is proposed as a standard audit metric to determine whether video benchmarks measure genuinely visually grounded capability rather than language priors.
...
TL;DR: This paper proposes a procedural material generation approach that retrieves and reasons over expert process traces (textual records of construction steps) using LLMs, then compiles them into executable Blender material graphs. The method leverages tutorial videos as process knowledge sources and outperforms prior systems in expert and user studies.


<details>
  <summary>Details</summary>
Motivation: Procedural material creation requires understanding the process by which experts construct materials, not just reproducing node graphs. Existing methods operate solely on static artifacts (node graphs) and fail to capture the underlying design strategies and workflows.

Method: The system uses a pretrained LLM-based ProcessSynthesizer to generate process traces (textual records of steps, parameters, intent) aligned with user intent, and a pretrained LLM-based Compiler to ground these traces into executable Blender material graphs. Process knowledge is extracted from tutorial videos using automated video analysis tools.

Result: Expert study with 5 Blender artists (avg. 7.5 years experience) showed materials generated by reflecting expert demonstrations required fewer edits and more closely matched professional design strategies. A 150-participant user study demonstrated superior generation and editing performance over prior systems.

Conclusion: Elevating process to a first-class representation (beyond graph-only synthesis) is crucial for high-quality procedural material generation. Process traces capture expert design intent and workflows, leading to more editable and professionally aligned results.
...
TL;DR: SARFA enhances SAM for medical image segmentation by using radiomic feature alignment and preference optimization to generate clinically plausible, diverse masks for ambiguous targets like tumors.


<details>
  <summary>Details</summary>
Motivation: SAM struggles with ambiguous segmentation targets in medical imaging where expert radiologists often disagree on boundaries, yet precise delineation of tumors is critical.

Method: SARFA uses probabilistic prompting to generate diverse candidate masks, then optimizes them via Fréchet Radiomic Distance (FRD) and Direct Preference Optimization (DPO) to align anatomical/textural features with clinically meaningful ground truth.

Result: Outperforms existing ambiguous segmentation methods on CT and MRI benchmarks.

Conclusion: Radiomic feature alignment and DPO-style ranking are effective training objectives for improving SAM's performance on ambiguous medical segmentation tasks.
...
TL;DR: The paper proposes TC-UAP, the first method to protect videos against both reference-based (e.g., image-to-video) and tuning-based (e.g., fine-tuning) video customization attacks by generating temporally consistent universal adversarial perturbations that survive temporal compression and remain robust across unseen videos and temporal attacks.


<details>
  <summary>Details</summary>
Motivation: Existing anti-customization works focus on protecting images, while video protection against both reference- and tuning-based customization remains underexplored. Video protection faces unique challenges: image-level perturbations fail under 3D video VAE compression; single-video perturbations are vulnerable to temporal editing and cannot protect unseen videos; temporally inconsistent perturbations lack robustness.

Method: TC-UAP optimizes identity-level multi-frame Universal Adversarial Perturbations (UAP) over sliding windows from multiple videos. It incorporates intrinsic temporal modeling and an extrinsic surrogate temporal-attack loss to ensure temporal consistency and robustness against unseen temporal attacks.
...
Conclusion: TC-UAP is the first effective protection method against both types of video customization, addressing temporal compression, cross-video generalization, and temporal attack robustness.
...
TL;DR: A marker-free augmented reality workflow maps positive surgical margins from resected specimen scans back to the patient's resection bed, significantly improving localization accuracy (6.19 mm with AR vs 21.40 mm with verbal guidance) in cadaveric head and neck surgeries.


<details>
  <summary>Details</summary>
Motivation: Positive margins in head and neck oncologic surgery require mapping specimen-side pathology findings to the patient resection bed, which is challenging because pathologists identify margins on deformed specimens while surgeons must relocate the corresponding site using only verbal descriptions and no visual guidance.

Method: The method combines contour-constrained deformation, residual alignment to depth scans, surface-based fusion to a head-mounted display, and target projection. It uses bead-suture correspondences for deformation estimation and marker-free patient-to-display fusion.

Result: Deformation target errors: 7.63±3.74 mm (cheek), 3.72±1.02 mm (scalp). End-to-end margin localization error decreased from 21.40±3.84 mm (verbal) and 16.09±4.30 mm (specimen exam) to 6.19±1.79 mm (AR guidance, p<0.001). Marker-free fusion error: 2.15±0.87 mm. Online fusion time: 5.23±0.34 s.

Conclusion: The results demonstrate effective marker-free AR guidance for positive-margin localization and support more precise tumor resection in head and neck oncologic surgery.
...
TL;DR: An audio-text system for the ABAW 11th Ambivalence/Hesitancy Challenge that fuses prosodic audio, emotion RoBERTa, and handcrafted uncertainty features via temporal cross-attention and gated MIL pooling, achieving 0.875 AP and 0.72 macro-F1 on the dev set.


<details>
  <summary>Details</summary>
Motivation: To address the Ambivalence/Hesitancy Recognition Challenge by building a system that accurately detects emotional uncertainty and conflict from audio and text without using visual frames.

Method: Uses overlapping 5-second windows aligned with transcript timestamps. Features: 320-dim prosodic audio descriptor + 768-dim emotion RoBERTa + 74 handcrafted uncertainty/hedging features. Audio-text fusion via temporal cross-attention; support features injected before gated MIL pooling; ensemble of 5 models averaged.

Result: On the public development set: average precision 0.875, macro-F1 0.72.

Conclusion: The proposed audio-text fusion with gated MIL pooling and ensemble averaging is effective for ambivalence/hesitancy recognition, with code publicly available.
...
TL;DR: Open-vocabulary detectors maintain high confidence even when objects are severely occluded (down to 1/8 visible or 99% occluded frames), because their confidence signals category presence rather than target visibility. This leads to a ~10x understatement of occlusion's impact and makes confidence-based gates fire when objects are actually hidden.


<details>
  <summary>Details</summary>
Motivation: To audit whether detector confidence scores reflect object occlusion (visibility), since confidence is widely used for thresholding, evaluation, language grounding, retrieval, and active perception.

Method: Paired every view with a geometry-segmentation oracle for ground-truth visibility; tested three detectors (Grounding DINO, OWLv2, SAM 3) across nine categories, two simulators, real video, and both built and natural occlusion.

Result: Confidence stays nearly constant and uncorrelated with visibility as true visibility drops; detectors report target present in ~9/10 scenes even when occluded, firing on same-category distractors. Confidence-based metric understates occlusion impact by ~10x (8 vs 88 points); confidence-based gate fires exactly when object is hidden.

Conclusion: No single-view signal tested flags occlusion reliably; confidence signals category presence, not target visibility. Recommends target-grounded signals for gating and evaluation; connects effect to detector miscalibration and object hallucination.
...
TL;DR: DiffGI is a differentiable 3D-to-2D mapping framework that replaces discrete binary occupancy maps with continuous 2D TSDF for subpixel boundary precision, introduces differentiable Marching Squares, and trains a VAE+latent diffusion model for high-quality, efficient 3D generation from ultra-compact latent spaces.


<details>
  <summary>Details</summary>
Motivation: Existing 3D generative models rely on implicit volumetric representations with watertight constraints, failing on thin-shell/non-manifold geometries like garments. Geometry-image methods suffer from staircase artifacts and non-differentiable surface reconstruction.

Method: Proposes DiffGI: (1) continuous 2D TSDF replacing binary maps for subpixel boundary encoding; (2) differentiable Marching Squares via analytical linear interpolation; (3) DiffGI-VAE with geometry-aware normal rendering loss; (4) transformer-based latent diffusion with flow-matching for conditional 3D generation.

Result: Superior reconstruction fidelity and boundary precision compared to prior geometry-image and voxel-based approaches, with significantly fewer computational resources on garment and object datasets.

Conclusion: DiffGI provides an end-to-end differentiable pipeline for surface-centric 3D generation, eliminating resolution-dependent artifacts and enabling compact latent-space learning for efficient, high-quality 3D synthesis.
...
TL;DR: RoughNet uses a conditional diffusion model to estimate high-resolution (1 m) sea ice roughness from 10 m Sentinel-2 satellite imagery, achieving 9 cm RMSE on unseen Arctic regions, offering a scalable alternative to airborne LiDAR surveys.


<details>
  <summary>Details</summary>
Motivation: Accurate sea ice roughness estimation is critical for climate modeling and safe Arctic travel, but existing methods rely on expensive airborne surveys or sparse in-situ data, limiting coverage and scalability.

Method: A conditional diffusion framework (RoughNet) learns to map 10 m Sentinel-2 multispectral images to locally normalized 1 m surface elevation residual fields, trained on airborne LiDAR data and evaluated on unseen Arctic regions.

Result: The model generalizes across diverse ice conditions, achieves 9 cm out-of-domain RMSE, and preserves statistical/spectral properties of the roughness field.

Conclusion: Generative diffusion models can recover physically meaningful surface structure from optical imagery alone, providing a scalable pathway for sea ice roughness mapping in data-sparse environments.
...
TL;DR: FM$^2$ is a federated foundation model framework that addresses Imaging Modality Heterogeneity (overlapped and non-overlapped) for medical imaging, using dual Mixture-of-Experts, Heterogeneous Modality Alignment regularizer, and Caption-Enhanced Learning to enable cross-modality collaboration without centralized data sharing.


<details>
  <summary>Details</summary>
Motivation: Privacy regulations prevent centralized aggregation of medical imaging data. Existing federated foundation models either fine-tune natural-image models (poor medical transfer) or train single-modality models from scratch (lacking task flexibility). The key under-explored challenge is Imaging Modality Heterogeneity, where clients have either overlapped (shared modalities, heterogeneous labels) or non-overlapped (fully disjoint) modalities.

Method: FM$^2$ trains a backbone from scratch for medical domain fidelity, optionally with biomedical pretrained encoders for vision-language alignment. Each client has dual MoE modules: Class-wise MoE (personalized category knowledge) and Domain-wise MoE (shared cross-modality representations). A Heterogeneous Modality Alignment (HMA) regularizer aligns modality-specific expert parameters with $O(1/\sqrt{T})$ convergence guarantees. Caption-Enhanced Learning (CEL) uses locally retained GPT-4o-generated captions as semantic bridges for cross-client representation transfer, with extensibility to Federated Medical VQA.

Result: On the MIMH benchmark (classification and CEL) and real-world medical VQA datasets, FM$^2$ consistently outperforms state-of-the-art federated baselines and demonstrates strong out-of-modality generalization across all three tasks.

Conclusion: FM$^2$ provides a unified, theoretically grounded solution for heterogeneous federated medical imaging, enabling cross-modality collaboration while preserving privacy, with demonstrated effectiveness across classification, caption-enhanced learning, and VQA tasks.
...
TL;DR: AnomExpert is a prototype-driven framework for prenatal ultrasound anomaly diagnosis that uses learnable plane prototypes and disease-aware sparse selection to identify anatomical planes and diagnostically relevant features, achieving 86.9% accuracy on a large multi-center dataset under only case-level supervision.


<details>
  <summary>Details</summary>
Motivation: Existing automated methods for prenatal US diagnosis either require expensive plane-level annotations or aggregate heterogeneous images without explicitly modeling the diagnostic capabilities of identifying anatomical planes and selecting diagnostically relevant planes for each anomaly.

Method: AnomExpert introduces (1) learnable plane prototypes to organize unordered images into latent representations corresponding to anatomical planes without plane annotations, and (2) a disease-aware sparse selection mechanism to select diagnostically relevant planes for each anomaly, using only case-level supervision.
...
TL;DR: MultiAnimate is a framework for concurrently animating multiple characters from separate reference images while maintaining identity consistency and spatial relationships. It uses an identity-specific reference net, identity-aware pose encoder, and optional interaction guider module to handle complex multi-character interactions.


<details>
  <summary>Details</summary>
Motivation: Existing character animation methods are limited to single reference images, failing to support multiple character interaction animation scenarios. This gap restricts applicability in contexts requiring concurrent animation of multiple characters with preserved identities and spatial relationships.

Method: MultiAnimate employs three key components: (1) an identity-specific reference net for extracting appearances from multiple reference images; (2) an identity-aware pose encoder with attention mechanisms for character-pose binding across multiple sequences; (3) an optional interaction guider module using character-specific mask information to refine complex inter-character interactions.

Result: Extensive experiments and ablation analyses demonstrate superiority in multiple character animation, especially in scenarios involving complex motion sequences.

Conclusion: MultiAnimate effectively addresses the multi-character animation gap, offering a comprehensive framework that preserves identity consistency and spatial relationships in shared environments.
...
TL;DR: ScanFocus is a coarse-to-fine framework for Spatio-Temporal Video Grounding that decouples the task into global scanning and local boundary focusing, using a Deformable Semantic-Motion Fusion module and Semantic-Guided Temporal Aggregator to recover fine-grained boundary details lost in low-rate downsampling.


<details>
  <summary>Details</summary>
Motivation: Existing STVG methods struggle to balance global context modeling with precise boundary localization because low-rate temporal downsampling and implicit motion modeling suppress high-frequency boundary cues and neglect explicit inter-frame dependencies.

Method: ScanFront uses a coarse-to-fine approach: (1) a unified vision-language fusion encoder with a Deformable Semantic-Motion Fusion module for coarse proposal generation, and (2) a Semantic-Guided Temporal Aggregator (SGTA) that densely samples around coarse boundaries and explicitly models short-term temporal interactions under semantic guidance for precise regression.

Result: Extensive experiments on three widely used benchmarks show performance superiority over previous approaches.

Conclusion: The proposed ScanFocus framework effectively addresses the trade-off between global context and fine-grained boundary localization in STVG by decoupling the task into two stages and recovering suppressed details via semantic-guided temporal aggregation.
...
TL;DR: This paper proposes a CLIP-guided label-free region scoring framework for fine-grained classification, comparing different scoring strategies and region generation methods (SAM masks vs. random crops). Soft Negative Margin scoring performs best, and random-crop-based pseudo-label scoring consistently outperforms SAM-based scoring.


<details>
  <summary>Details</summary>
Motivation: It remains unclear how different local regions and CLIP-based scoring strategies affect the selection of discriminative evidence for fine-grained classification when ground-truth labels are unavailable.

Method: A unified CLIP-guided label-free region scoring framework that evaluates cosine similarity-based, margin-based, and entropy-based scoring strategies using both SAM-generated masks and random crops, with two label-free pseudo-label variants from global and local embeddings.

Result: Soft Negative Margin scoring achieves the strongest performance; pseudo-label scoring closely approximates true-label performance; random-crop-based pseudo-label scoring consistently outperforms SAM-based scoring across all datasets; SAM benefits from aggregating all regions while random crops perform better with smaller top-k subsets.

Conclusion: Random crops preserve surrounding information and provide more stable semantic context when pseudo-labels are noisy, offering new insights for fine-grained classification.
...
TL;DR: A framework for single-shot 6-DoF pose and shape estimation of unknown spacecraft using DINOv3 features, dynamic graph CNN, and dual-stream transformer matching, achieving 0.157° mean pointing error on the SPE3R dataset.


<details>
  <summary>Details</summary>
Motivation: 6-DoF pose estimation is critical for autonomous rendezvous and proximity operations, but becomes challenging when the target spacecraft is unknown, requiring simultaneous shape reconstruction and pose estimation.

Method: (1) Reconstruct 3D shape from a single image using frozen DINOv3 vision transformer features; (2) Extract geometric features from the point cloud via a trainable dynamic graph CNN; (3) Use a dual-stream transformer matcher with alternating self- and cross-attention to refine descriptors and produce soft 2D-3D correspondences; (4) Apply a PnP solver for pose recovery.

Result: 0.157° mean pointing error on the SPE3R dataset using only a single image and reconstructed geometry; demonstrates strong generalization to unseen spacecraft.

Conclusion: The proposed method achieves reliable pose estimation for unknown spacecraft with high accuracy, outperforming or matching the state-of-the-art baseline FoundationPose, and generalizing well to unseen targets.
...
TL;DR: The paper proposes Symbiosis-Inspired Knowledge Distillation (SIKD) for incremental object detection, which leverages object symbiosis through Spatial Symbiosis Distillation (preserving spatial region cues) and Semantic Symbiosis Distillation (maintaining class-level semantic topology) to overcome forgetting and confusion in class incremental learning.


<details>
  <summary>Details</summary>
Motivation: Existing incremental object detection methods adopt a separation-oriented paradigm that isolates feature spaces for different classes, which overlooks object symbiosis (co-occurrence and occlusion relationships). This neglect distorts shared representations and exacerbates catastrophic forgetting and class confusion.

Method: SIKD has two complementary components: (1) Spatial Symbiosis Distillation (SpSD) - identifies symbiotic regions where the old model responds to new task objects, preserves generalizable old cues while suppressing bias/redundancy, and distills refined evidence with slot-aligned supervision. (2) Semantic Symbiosis Distillation (SeSD) - forms confidence-weighted prototypes for old classes and aligns their inter-class soft ranks over old class logits to stabilize semantic topology.

Result: Extensive experiments demonstrate the effectiveness and superiority of the proposed method over existing approaches.

Conclusion: Leveraging object symbiosis through both spatial and semantic distillation is an effective paradigm shift from separation-oriented methods, enabling better retention of old knowledge while learning new categories in incremental object detection.
...
TL;DR: GeoAnchor is an interleaved text-latent reasoning framework that decomposes 3D spatial information into three complementary components (position, direction, and geometry latents) to improve MLLMs' understanding of 3D spatial relationships from 2D images, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs struggle with 3D spatial understanding from 2D images because symbolic text tokens lack fidelity for continuous geometric information, and single-type latent representations cannot adapt to diverse spatial tasks.

Method: GeoAnchor proposes an interleaved text-latent reasoning framework that decomposes 3D spatial information into three complementary latent components: position latents (object grounding), direction latents (relational orientation), and geometry latents (scene structure). These are recombined in a structured space for local evidence and global context, with a collaborative training strategy guiding from local to comprehensive 3D understanding.

Result: Extensive experiments on diverse and complex 3D reasoning tasks show GeoAnchor outperforms state-of-the-art methods, demonstrating effectiveness and generalization.

Conclusion: The interleaved multi-latent approach (position, direction, geometry) effectively bridges the gap between 2D images and 3D spatial reasoning, offering a more adaptive and interpretable solution than single-latent or text-only methods.
...
TL;DR: This paper introduces a Riemannian geometric framework for analyzing and generating complex 3D tree-shaped objects by generalizing the SRNF representation, enabling statistical analysis, geodesic computation, and shape synthesis with significantly better performance than prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods either only model skeletal structures or approximate 3D geometry with branch thickness; there is no accurate framework that captures both the full 3D geometry of branches and their interconnection structure for tree-shaped objects.

Method: Generalizes Square Root Normal Fields (SRNF) from genus-0 surfaces to tree-shaped 3D objects; models tree shapes as points on a Riemannian tree-shape space with a novel metric measuring surface bending, stretching, and structural changes; develops algorithms for correspondences and geodesic paths.

Result: Demonstrates superior performance over state-of-the-art on both real and synthetic plants/trees; enables computing statistical summaries (means and modes of variation) and synthesizing novel tree shapes via probability distribution sampling.

Conclusion: The proposed Riemannian framework effectively handles both geometry and topology of 3D tree-shaped objects, advancing statistical shape analysis and generative modeling for botanical structures.
...
TL;DR: This paper introduces 2D-RoPE-STR, which adapts 2D Rotary Position Embedding for Scene Text Recognition by handling anisotropic text aspect ratios and extending rotary coupling to encoder-decoder cross-attention, achieving gains on irregular text layouts.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based STR methods use 1D positional encodings that ignore 2D spatial structure, while existing 2D RoPE methods assume square isotropic images and only work in encoder self-attention, neither suited for anisotropic scene text images with encoder-decoder architectures.

Method: (1) Anisotropic row/column dimension allocation matched to text aspect ratio; (2) Extension of rotary coupling into encoder-decoder cross-attention; both are parameter-free and require no architectural redesign.

Result: Gains concentrated on irregular layouts (curved, rotated, perspective-distorted) across six benchmarks (IIIT5K, SVT, ICDAR 2013/2015, CUTE80, SVTP), validated by controlled ablation and diagnostic protocol.

Conclusion: 2D-RoPE-STR effectively improves STR on challenging irregular text by incorporating relative 2D position information without additional parameters or architectural changes.
...
TL;DR: LPM is a diffusion-based generative video restoration model deployed at industrial scale at Kuaishou, handling diverse real-world degradations, processing ~45% of total viewing time, reducing bitrate by 20%, and saving hundreds of millions in bandwidth costs annually.


<details>
  <summary>Details</summary>
Motivation: To address the diverse, complex degradations in user-generated content (UGC) videos at industrial scale with a unified, practical, and cost-effective generative restoration system.

Method: A diffusion-based generative framework with enhanced architecture, progressive training strategy, temporal-pyramid inference mechanism, and large-scale data engineering for unified handling of in-the-wild degradations.

Result: Deployed at Kuaishou processing ~45% of total viewing time; achieves 20% bitrate reduction at comparable perceptual quality vs. in-house codec; yields annual bandwidth cost savings of hundreds of millions; low serving cost enables integration into products like Kling.

Conclusion: Generative video restoration can be practical, scalable, and cost-effective for large-scale production environments, delivering both perceptual and system-level benefits.
...
TL;DR: HIVF-3D generates high-quality 3D scenes from a single image using a hierarchical voxel enhancement framework with component alignment and coarse-to-fine super-resolution.


<details>
  <summary>Details</summary>
Motivation: Existing single-image 3D generation methods are limited by restricted representation resolution, making them unsuitable for 3D scene generation.

Method: (1) Generate a coarse initial scene from a single image. (2) Use image segmentation and attention-based retrieval to align 2D components with 3D components. (3) Organize scene relations into a hierarchical component tree. (4) Apply a voxel super-resolution model that performs coarse-to-fine hierarchical super-resolution on images and voxels for each component.

Result: The method significantly outperforms previous approaches, achieving state-of-the-art performance in 3D scene generation.

Conclusion: The hierarchical voxel enhancement framework effectively addresses resolution limitations, enabling high-quality 3D scene generation from a single image.
...
TL;DR: GPOcc/GPOcc++ transforms visual geometry priors into occupancy-aware sparse Gaussian representations for efficient 3D occupancy prediction across indoor and outdoor scenes, handling multi-view and temporal observations in a unified framework.


<details>
  <summary>Details</summary>
Motivation: Visual geometry priors offer strong geometric cues but are surface-centric, while 3D occupancy prediction requires reasoning about volumetric interiors and free space. The authors aim to bridge this gap between surface-level geometry priors and full volumetric occupancy understanding.

Method: GPOcc converts visual geometry priors into occupancy-aware sparse Gaussian representations for volumetric scene modeling. GPOcc++ extends this to handle multi-view observations and temporal sequences within a unified framework, and further scales from indoor to outdoor occupancy prediction.

Result: Consistently strong performance across both indoor and outdoor benchmarks in multi-view and temporal settings, with favorable efficiency and generalization capabilities.

Conclusion: The proposed sparse Gaussian representation effectively bridges surface geometry priors and volumetric occupancy prediction, achieving strong results with good efficiency and generalization across diverse settings.
...
TL;DR: CASA-SDF is a neural implicit representation framework for indoor 3D reconstruction that uses curriculum-aware spatial adaptation with hybrid uncertainty annealing for supervision and curvature-aware adaptive density transformation for thin structures, improving both planar smoothness and detail recovery.


<details>
  <summary>Details</summary>
Motivation: High-fidelity indoor surface reconstruction is challenging due to geometric heterogeneity: large texture-less planar regions need strong regularization to suppress artifacts, while thin structures require sharp, adaptive representations to prevent over-smoothing. Existing approaches rely on spatially indiscriminate priors and global SDF-to-density transformations, failing to balance smoothness and detail.

Method: CASA-SDF introduces two key components: (1) Hybrid Spatially-Adaptive Uncertainty Annealing (SAUA) that fuses semantic and photometric uncertainties to create a pixel-wise curriculum for monocular prior supervision, and (2) Curvature-Aware Locally Adaptive Density Transformation (CALADT) that progressively modulates SDF-to-density mapping sharpness using a curvature proxy for better thin structure representation.
...
Conclusion: The proposed unified framework successfully addresses geometric heterogeneity in indoor scenes by complementary adaptations of supervision and representation capacity, achieving balanced performance across both planar regions and thin structures.
...
TL;DR: M2P-AD proposes a Memory-to-Prototype module and Boundary-aware score refinement to reduce false positives in 3D anomaly detection, achieving SOTA on three benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D anomaly detection methods suffer from excessive anomaly responses in normal regions and false positives near object boundaries, reducing reliability.

Method: (1) Memory-to-Prototype (M2P) module to learn prototypes from normal features; (2) Boundary extraction (BE) module to identify object boundaries; (3) Boundary-aware score refinement (BSR) strategy to recalibrate anomaly scores using boundary characteristics.

Result: Achieves state-of-the-art performance on Real3D-AD, Anomaly-ShapeNet, and MulSen-AD datasets; reduces excessive anomaly scores in normal regions and suppresses false positives near boundaries.

Conclusion: M2P-AD provides more accurate and stable 3D anomaly localization, offering a robust solution for real-world industrial environments.
...
TL;DR: This paper proposes an attention-free, lightweight token reduction framework for Vision-Language Models that uses entropy-based importance estimation and transformation-induced consistency signals to efficiently prune visual tokens while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: VLMs are computationally expensive on edge devices due to many visual tokens. Existing token reduction methods are either incompatible with modern acceleration frameworks (rely on attention maps) or computationally expensive (pairwise similarity comparisons), limiting practical deployment.

Method: 1) Attention-free importance estimation using entropy-based criterion to quantify token information. 2) Transformation-induced consistency signal where similar tokens get similar signals, enabling stride-based diverse token selection without pairwise comparisons.

Result: Achieves favorable accuracy-efficiency trade-off across multiple VLMs benchmarks, maintaining competitive performance even under aggressive compression.

Conclusion: The proposed framework is a plug-and-play module that effectively reduces visual tokens for VLMs without relying on attention maps or costly pairwise computations, making it practical for edge deployment.
...
TL;DR: DP-BOA is a Bayesian framework for on-the-fly category discovery that uses Dirichlet-process Gaussian mixture modeling to explicitly compare posterior predictive evidence for assigning a test sample to an existing category versus spawning a new one, outperforming heuristic-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing on-the-fly category discovery methods rely on matching-based heuristics (radius/hash rules) that treat category birth implicitly as a fallback, lacking explicit statistical evidence for spawning new categories.

Method: Proposes DP-BOA, an online Dirichlet-process Gaussian mixture model with Normal-Inverse-Wishart prior. Uses labeled data to calibrate priors and warm-start posteriors. At test time, compares posterior predictive evidence for assignment to existing categories vs. spawning a new category via DP prior.
...
Conclusion: DP-BOA demonstrates that explicit statistical modeling of category birth via Bayesian posterior predictive comparison is more effective than heuristic-based approaches for on-the-fly category discovery.
...
TL;DR: A temporal multimodal model using a 3D MRI foundation model and temporal attention gate predicts prostate cancer progression during active surveillance without requiring lesion segmentation, outperforming baselines and potentially reducing unnecessary biopsies.


<details>
  <summary>Details</summary>
Motivation: Current active surveillance protocols rely on scheduled repeat biopsies, most of which are unnecessary; existing risk-stratification tools are limited to single time-point imaging or require explicit lesion segmentation, failing to capture longitudinal changes and excluding patients without MRI-visible lesions.

Method: Encodes serial MRI scans with a pretrained 3D foundation model, introduces a temporal attention gate to recalibrate multi-visit features, and fuses imaging with clinical variables in a multimodal framework for progression risk estimation.

Result: Outperforms competing baselines, performs comparably to radiologist assessment, maintains high negative predictive value while achieving higher positive predictive value.

Conclusion: The model demonstrates potential to safely reduce unnecessary biopsies during active surveillance for prostate cancer.
...
TL;DR: This paper introduces a new NIR-visible dataset for on-the-move face recognition at border checkpoints, capturing real-world challenges like motion blur, variable lighting, and occlusions. Baseline tests show state-of-the-art models struggle, revealing a need for dedicated methods.


<details>
  <summary>Details</summary>
Motivation: Existing face recognition datasets fail to capture the unique challenges of vehicular border control settings (motion blur, variable illumination, occlusions, cross-spectral enrollment). This gap hinders development of on-the-move biometric authentication systems.

Method: The authors construct a dataset of NIR vehicle-crossing videos paired with smartphone-based pre-enrollment data, then evaluate state-of-the-art face recognition models as baselines under these realistic conditions.

Result: State-of-the-art face recognition models show clear performance limitations on this dataset, demonstrating that current methods are insufficient for real-world vehicular border control scenarios.

Conclusion: There is a pressing need for dedicated face recognition methods to address the unique challenges of on-the-move border control authentication.
...
TL;DR: The paper proposes VGIF-Score, an automated and interpretable framework for evaluating how well video generation models follow complex, long instructions. It combines objective completion (via ST-DAG parsing and QA diagnostics) and subjective satisfaction (via AutoRubric) into a unified score, validated on a benchmark of 223 long prompts and 14 VGMs.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models (VGMs) lack sufficient evaluation for following long, compositional instructions. Current protocols use short, shallow prompts with limited constraints, rely on costly human evaluation or handcrafted pipelines, and offer little diagnostic insight into which instruction constraints succeed or fail.

Method: VGIF-Score has two branches: (1) Objective Completion Branch — parses prompts into Spatio-Temporal Directed Acyclic Graphs (ST-DAG) and performs dependency-aware QA with short-circuit diagnostics; (2) Subjective Satisfaction Branch — uses instruction-conditioned AutoRubric to assess cinematography, visual purity, motion smoothness, and physics adherence. These are combined into a unified score.

Result: Experiments on 14 proprietary and open-source VGMs with over 3K generated videos demonstrate that VGIF-Score provides reliable, interpretable, and diagnostically useful evaluation.

Conclusion: VGIF-Score fills a critical gap in evaluating instruction following for video generation, offering a highly automated, interpretable, and diagnostic framework that outperforms existing costly or shallow evaluation methods.
...
TL;DR: ThinkBLOX is a VLM-based progressive reasoning framework that iteratively designs and refines 3D indoor scenes using step-by-step reasoning and action, outperforming one-shot approaches in physical plausibility, semantic alignment, and interactive editing.


<details>
  <summary>Details</summary>
Motivation: One-shot VLM-based 3D scene generators require global re-optimization during editing and produce physically or semantically poor layouts. Existing iterative methods also fall short.

Method: ThinkBLOX uses a state-conditioned step-by-step reasoning-and-action process with a curated dataset (ThinkBLOX-Data-200K) of procedural placement pairs with CoT rationales, plus Tier-Decoupled GDPO for multi-reward reinforcement learning.

Result: ThinkBLOX significantly outperforms one-shot and iterative baselines in physical plausibility, semantic alignment, and interactive editability, supporting global/local generation and rearrangement.

Conclusion: Progressive reasoning with tier-decoupled reward optimization effectively bridges the reasoning-action gap and handles the multi-solution nature of 3D scene synthesis.
...
TL;DR: Nexus is a diffusion-based method for holistic 3D mesh generation that decouples vertex and topology generation, outperforming autoregressive approaches on Objaverse and Toys4K datasets.


<details>
  <summary>Details</summary>
Motivation: Autoregressive mesh generation methods suffer from ineffective inference and error accumulation. The authors aim to overcome these limitations by proposing a diffusion-based approach that avoids sequential modeling.

Method: Mesh generation is split into two decoupled stages: (1) Vertex generation via a diffusion model that treats vertices as sparse voxels in an octree, generated coarse-to-fine. (2) Topology generation using Spacetime Interval (an extension of Spacetime Distance) to encode edge/face topology into continuous per-vertex embeddings, recovered via another diffusion model.

Result: Nexus outperforms state-of-the-art autoregressive and two-stage baselines on Objaverse and Toys4K datasets, and demonstrates strong performance on in-the-wild images. A blind user study confirmed perceptual preference by 3D practitioners.

Conclusion: The proposed diffusion-based decoupled approach effectively circumvents limitations of sequential mesh modeling, achieving high-quality holistic mesh generation.
...
TL;DR: GHR-VLM is a hybrid edge-cloud framework combining lightweight tracking and vision-language models for zero-shot transit video analytics, achieving passenger-level payment behavior classification while reducing cloud inference costs.


<details>
  <summary>Details</summary>
Motivation: Transit video understanding needs fine-grained data that conventional systems cannot capture, but supervised models require task-specific annotations and VLMs are unreliable/costly on long videos. The authors observed that explicit visual grounding can bridge this gap.

Method: Edge-cloud design: (1) lightweight edge monitor tracks door status and segments passenger clips, (2) backend VLM performs two-stage coarse-to-fine refinement on grounded passenger clips and contact sheets to identify boarding passengers and classify payment behavior.

Result: Evaluated on 486 minutes of real-world bus surveillance video, demonstrating potential for passenger-level payment analytics while highlighting challenges from degraded video conditions.

Conclusion: Grounded edge-cloud reasoning enables zero-shot transit video analytics, reducing cloud inference and avoiding payment-specific training data, though degraded video quality remains a challenge.
...
TL;DR: UniPhys is a scalable framework that automatically transforms raw 3D assets into simulation-ready assets with unified physical semantics (articulation + intrinsic properties). It includes a large-scale dataset (UniPhys-40K), a benchmark (UniPhys-Bench), and a model (UniPhysGen) that achieves SOTA on physical grounding tasks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D assets lack unified physical semantics needed for realistic interaction in embodied AI and robotic simulation. Current approaches treat articulation semantics and physical properties independently or rely on canonicalized structures, limiting robustness across heterogeneous assets.

Method: UniPhys is a scalable framework for automatic transformation of raw 3D assets. It includes UniPhysGen — a unified physical grounding model that jointly reasons over articulation semantics and intrinsic physical properties, with geometry-robust articulation grounding to mitigate geometric shortcut bias under heterogeneous part decompositions.

Result: State-of-the-art performance across articulation grounding and intrinsic physical property estimation tasks. The resulting assets are directly deployable in robotic simulation for realistic physical interaction.

Conclusion: UniPhys provides a complete pipeline (framework + dataset + benchmark + model) for converting raw 3D assets into physically grounded, simulation-ready assets, advancing embodied AI and robotic simulation capabilities.
...
TL;DR: OvisOCR2 is a 0.8B end-to-end document parsing model that converts document page images to Markdown, achieving state-of-the-art scores of 96.58 on OmniDocBench and 75.06 on PureDocBench through a training pipeline involving SFT, RL distillation, and model fusion.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient (0.8B parameters) end-to-end document parser that can outperform traditional pipeline methods on document parsing benchmarks, while maintaining robustness on long-tail scenarios.

Method: The model is trained with a data engine combining filtered real documents and synthetic HTML-derived pages. The recipe includes supervised fine-tuning, reinforcement learning on a 4B teacher branch with multi-component rewards, on-policy distillation into the 0.8B model, and model fusion.

Result: Achieves state-of-the-art overall score of 96.58 on OmniDocBench v1.6 (top of leaderboard previously dominated by pipeline methods) and highest Avg3 score of 75.06 on PureDocBench. Also shows best performance on an in-house long-tail benchmark.

Conclusion: The results highlight the potential of end-to-end document parsing models to surpass pipeline-based approaches, demonstrating strong generalization and robustness even with a small 0.8B model.
...
TL;DR: Human4K is a large-scale 4K multi-view whole-body human reconstruction dataset with 6M+ images and mocap-accurate SMPL-X annotations, using a Motion-Retargeting and Refinement Module (MRRM) to improve whole-body reconstruction, especially for hands, feet, and depth-ambiguous limbs.


<details>
  <summary>Details</summary>
Motivation: Existing human reconstruction models fail in challenging real-world scenarios due to unstable geometry, inaccurate limb articulation, and unreliable predictions under depth ambiguity or self-occlusion, largely because current datasets lack high-resolution images, high-precision annotations, and diverse whole-body motions.

Method: The authors built Human4K using an eight-view 4K camera system synchronized with a Vicon motion capture setup, capturing 11 subjects performing complex motions. They developed a Motion-Retargeting and Refinement Module (MRRM) to ensure precise alignment of full body and extremities with SMPL-X annotations.
...
Conclusion: High-resolution, high-precision datasets like Human4K are crucial for advancing robust 3D human reconstruction in challenging real-world scenarios, especially for extremities and ambiguous poses.
...
TL;DR: This paper evaluates how well vision models represent colors in a human-like way, using a fuzzy perceptual model with 86 graded categories. Across Vision Transformers, MAE shows the strongest human-aligned color representations, especially in graded alignment beyond geometry.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations compare model color representations to geometric spaces (CIELAB) or discrete labels, which capture perceptual distance or category membership but not the graded, fuzzy way humans organize colors.

Method: The authors propose a framework using 86 graded categories fitted to human survey data, measuring three properties: category boundaries, category compactness, and graded alignment beyond color geometry. They evaluate eleven Vision Transformer encoders.

Result: Masked Autoencoders (MAE) achieve the strongest beyond-geometry alignment with non-overlapping confidence intervals. MAE preserves surface color globally, while language-supervised models encode color more in relation to foreground objects.

Conclusion: Human-like color grounding has multiple distinct aspects that should not be reduced to a single score; different encoder architectures capture different aspects of color perception.
...
TL;DR: This paper presents LeWorldModel, a joint-embedding predictive architecture adapted for cloud-aware Earth Observation, which forecasts whether satellite imagery will be usable (i.e., not cloud-obscured) and when a usable view will return, outperforming persistence and competing with LightGBM on various benchmarks.


<details>
  <summary>Details</summary>
Motivation: The bottleneck in Earth Observation processing is not the availability of new imagery, but whether the surface is actually visible (cloud-free) when the image arrives. The authors aim to forecast observability — predicting if the next acquisition will be usable and when a usable view will return.

Method: The authors adapt LeWorldModel, a joint-embedding predictive architecture world model, to cloud-aware Earth Observation sequences. They convert raw minicubes into episodic HDF5 sequences with five image channels (blue, green, red, NIR, cloud mask) and eight meteorological/calendar covariates. The model (18.0M parameters) is trained from scratch on 23,904 episodes and evaluated via a locked protocol with linear probes.

Result: LeWorldModel consistently outperforms persistence: next-step usability balanced accuracy ranges from 0.769 to 0.887 (vs. 0.493-0.556 for persistence). Exact first-usable-horizon accuracy ranges from 0.602 to 0.806 (vs. 0.120-0.369). Against LightGBM, LeWorldModel is better on continuous clear/cloud regression and exact recovery timing on most splits, while LightGBM is stronger on binary any-usable-within-six tasks and more robust on OOD. LeWM also produces strong ranking-based anomaly signals under synthetic temporal inconsistencies.

Conclusion: LeWorldModel is an effective approach for observability forecasting in Earth Observation, significantly outperforming simple persistence and competing well with gradient-boosted baselines on cloud-related forecasting tasks.
...
TL;DR: REAL is an agentic framework for open-world mobile manipulation that bridges sim-to-real gaps, integrates human-in-the-loop interaction, and achieves strong performance on both simulated benchmarks (56.9% success rate) and real-world physical trials (78.3% success rate).


<details>
  <summary>Details</summary>
Motivation: Existing embodied agent frameworks rely on privileged simulator states or assume complete instructions, failing to address realistic deployment challenges such as active exploration, visual grounding, and interactive intent disambiguation.

Method: REAL establishes sim-to-real-consistent environment APIs without oracle perception, integrates a simulated user for human-in-the-loop interaction, and uses diverse task compositions for data collection, supervised fine-tuning, and online reinforcement learning to optimize agent performance.

Result: The trained agent outperforms leading commercial closed-source VLMs with a 56.9% success rate on interactive tasks. On a physical dual-arm mobile robot, it achieves a 78.3% end-to-end success rate over 60 real-world episodes with robust zero-shot transferability.

Conclusion: REAL's sim-to-real-consistent design successfully bridges the reality gap for long-horizon mobile manipulation, demonstrating strong zero-shot transfer to unseen household scenarios through its hierarchical training pipeline.
...
TL;DR: T3HG-Editor is a text-driven 3D human garment editing method that leverages SMPL-X geometry and joint priors to achieve high-fidelity, garment-consistent editing through three stages: editable Gaussian obtainment, garment consistent editing via multi-view token aggregation, and Gaussian updating with SDF-based overflow pruning.


<details>
  <summary>Details</summary>
Motivation: Text-driven 3D human garment editing is underexplored, and existing 3DGE methods suffer from low fidelity due to distortions and garment inconsistencies. SMPL-X offers rich prior information for virtual humans, motivating its use.

Method: Three-stage pipeline: (1) Obtain editable Gaussians by seeding along SMPL-X normals and applying 2D mask constraints; (2) Garment consistent editing by aggregating tokens across views for same SMPL-X vertices; (3) Gaussian updating with SDF-based overflow pruning to prevent contamination of non-target regions.

Result: Outperforms state-of-the-art methods on multiple subjects and diverse garment types in both editing quality and garment consistency.

Conclusion: T3HG-Editor effectively addresses low-fidelity and inconsistency issues in 3D human garment editing by leveraging SMPL-X priors, achieving superior results without requiring additional training.
...
TL;DR: FreeLit is a paired-free framework for controllable indoor scene relighting that uses physics-guided illumination priors and relighting-guided intrinsic stabilization to manipulate light source location, color, and intensity without requiring costly paired multi-illumination datasets.


<details>
  <summary>Details</summary>
Motivation: Existing data-driven methods require costly paired multi-illumination datasets that fail to scale, while inverse-rendering methods lack robustness in challenging conditions like low-light scenes.

Method: FreeLit constructs a physics-guided illumination prior from intrinsic scene properties to generate structured lightmaps and pseudo-relit images for diffusion-based synthesis. It also introduces a relighting-guided intrinsic stabilization strategy enforcing illumination-invariant reflectance via structure-aware distillation and consistency constraints.

Result: FreeLit achieves stable, physically consistent, and controllable relighting with improved robustness in low-light indoor scenes, requiring no paired supervision.

Conclusion: FreeLit effectively balances data efficiency and robustness by combining physics-guided priors with diffusion-based synthesis, enabling explicit user control over illumination without paired training data.
...
TL;DR: SFF-CLIP enhances CLIP's fine-grained dense feature representation through a self-annotated fine-tuning method using only image-text pairs, without requiring additional region annotations, while preserving CLIP's original image-level capabilities.


<details>
  <summary>Details</summary>
Motivation: CLIP has limited fine-grained dense feature representation due to its global image-text matching pre-training. Existing fine-tuning methods require additional region annotations (costly, limited semantic diversity) or sacrifice CLIP's global representation ability.

Method: SFF-CLIP proposes a run-time region-phrase alignment scheme that extracts concept phrases from input sentences and aligns them with region-based features using text-specific heat maps, all without extra annotations.

Result: Significant performance improvements on fine-grained dense feature tasks, while maintaining original CLIP performance on image-level tasks.

Conclusion: SFF-CLIP effectively bypasses limitations of prior CLIP fine-tuning methods by leveraging self-annotation from image-text pairs, achieving both fine-grained and global representation capabilities.
...
TL;DR: The POLY-SIM 2026 challenge addresses multimodal speaker identification under realistic conditions where audio-visual modalities may be incomplete/inhomogeneous and speakers may be multilingual.


<details>
  <summary>Details</summary>
Motivation: Real-world multimodal speaker identification faces challenges of missing modalities (occlusions, failures, privacy) and multilingual variability, which are not addressed by existing systems assuming complete, homogeneous, single-language data.

Method: The POLY-SIM 2026 challenge provides a standardized evaluation framework to compare solutions for robust multimodal speaker identification under incomplete modality and multilingual conditions.

Result: Not applicable (this is a challenge proposal/description, not a paper with experimental results).

Conclusion: The challenge aims to drive progress in robust and generalizable multimodal speaker identification by addressing realistic conditions of missing modalities and multilingualism.
...
TL;DR: WAVE-Stereo proposes an iterative stereo matching method that jointly models correlation volumes and feature warping via a GeoWarp Correspondence Encoder, plus a Periodic Global Context Propagation module for textureless regions, achieving competitive zero-shot accuracy with real-time inference across five benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing stereo methods treat explicit matching search (correlation volumes) and local residual refinement (warped features) as separate processes; the paper identifies that these two provide complementary matching cues and should be modeled jointly.

Method: (1) GeoWarp Correspondence Encoder (GWCE) encodes matching search, residual alignment, and disparity prior in parallel at the ConvGRU input. (2) Periodic Global Context Propagation (PGCP) periodically propagates global spatial information to mitigate degradation in textureless regions.

Result: 3.18% D1-all on KITTI 2015, 4.42% Bad-2.0 on Booster, 66ms real-time inference, competitive zero-shot generalization on five benchmarks (Middlebury, ETH3D, KITTI 2012, KITTI 2015, Booster).

Conclusion: The joint modeling of correlation volumes and feature warping, complemented by periodic global context, achieves a favorable balance between accuracy and efficiency without requiring any external foundation model.
...
TL;DR: The paper introduces VSI-Super-Wild, a large-scale benchmark with 6,980 QA pairs from 442 real-world long videos (up to 4+ hours) across 8 scene categories, testing spatial supersensing. Results show models fail at coherent world-state tracking over time, revealing four failure modes: spatial collapse, semantic shortcuts, insufficient update, and instance confusion.


<details>
  <summary>Details</summary>
Motivation: Existing spatial supersensing benchmarks rely on synthetic long videos formed by concatenating random short clips, limited to household scenes, lacking real-world continuity and diversity. The authors aim to address this gap by evaluating spatial understanding over long temporal horizons in diverse in-the-wild scenes.

Method: The benchmark VSI-Super-Wild contains 6,980 human-verified QA pairs from 442 real-world videos across 8 scene categories, including recordings exceeding 4 hours. It systematically probes the full triad of world state: agent (observer), objects (scene items), and environment (places and global layout), inspired by cognitive studies.
...
Conclusion: Current models lack mechanisms to bind objects, agents, and environments into a unified spatial world model. This fundamental gap defines the path forward for spatial supersensing research.
...
TL;DR: This paper introduces radiative Gaussian splatting with closed-form variational uncertainty propagation for sparse-view CT, demonstrating calibrated per-voxel uncertainty that outperforms perturbation-based heuristics, while analyzing acquisition regimes where principled uncertainty matters most.


<details>
  <summary>Details</summary>
Motivation: Existing Gaussian-splatting CT methods output point estimates without uncertainty quantification, making it unclear where reconstructions can be trusted. The authors exploit the linearity of transmissive X-ray imaging to equip radiative Gaussians with principled, closed-form predictive variance.

Method: Leverages the linearity of projection and voxelization in per-Gaussian densities to derive a variational density posterior with closed-form predictive variance in both volume space (σ²(x) = Σ g_i(x)² s_i²) and projection space (Var[I_p] = Σ w_{i,p}² s_i²). Evaluated via systematic calibration study (Spearman / AUSE / ECE with temperature scaling).

Result: Per-voxel uncertainty ranks true error on 14/15 benchmark scenes across three view budgets; 9/15 meet magnitude-calibration target after single temperature. Perturbation-ensemble heuristic shows poor performance (rank correlation as low as -0.08). Identifies three acquisition regimes (flat, pathological, anisotropic) where principled uncertainty yields measurable benefit only in the anisotropic case.

Conclusion: Closed-form variational uncertainty in Gaussian-splatting CT is well-calibrated and superior to perturbation-based heuristics. The benefit is regime-dependent, motivating a coverage-gated, maturity-scheduled acquisition policy. A dose-adaptive stopping rule is proposed for future work.
...
TL;DR: The paper proposes DNA (Dual-stage Native Attribution), a training-free coarse-to-fine framework for attributing generated images to their source models—even distinguishing highly similar within-family variants—by leveraging naturally stratified attribution signals in latent generative models (VAE-level for family, backbone-level for specific model). It achieves 89.11% attribution accuracy on the challenging DNA-30K benchmark, outperforming baselines by 33.81%.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of image generation has produced numerous within-family variants, making source-model attribution of suspect images increasingly important for digital forensics. Existing proactive methods degrade visual quality or limit flexibility, while passive methods struggle with unknown sources and distinguishing highly similar within-family variants.

Method: The authors propose a Dual-stage Native Attribution (DNA) framework that follows the natural hierarchy of attribution signals in latent generative models: (1) a coarse-grained stage using Autoencoder Double-Reconstruction (AEDR) for open-set family-level screening, and (2) a fine-grained stage using Native Prediction Consistency (NPC) that compares native prediction errors of within-family variants across multiple noise levels under semantic conditioning, attributing the source via normalized calibrated scores—all without additional neural-network training.

Result: Experiments show DNA achieves 89.11% end-to-end attribution accuracy on the DNA-30K benchmark (24 candidate models across six families, including non-candidate and natural images as unknowns), where random guessing accuracy is below 1%, and outperforms the strongest baseline by 33.81% even when AEDR is used as the coarse-grained stage.

Conclusion: The proposed DNA framework effectively addresses the challenging task of within-family variant attribution without training, leveraging the natural stratification of attribution signals in latent generative models, and significantly outperforms existing methods on a comprehensive benchmark.
...
TL;DR: A compact 1.11M-parameter CNN achieves 99.73% accuracy on DHCD Devanagari recognition, reaching an apparent performance saturation point, while also demonstrating strong corruption robustness and competitive zero-shot/fine-tuned performance on CMATERdb digits.


<details>
  <summary>Details</summary>
Motivation: To determine whether Devanagari character recognition has reached a performance saturation point, and to show that a highly compact model can match or exceed much larger architectures while reducing computational cost.

Method: A compact convolutional network (1.11M parameters) is trained on the 46-class DHCD dataset. Statistical significance is evaluated using exact McNemar tests with Wilson confidence intervals. The model is also tested zero-shot and fine-tuned on CMATERdb digits, and evaluated on corruption robustness.

Result: 99.73% accuracy on DHCD (highest reported, 15.6x smaller than prior SOTA); no statistically significant win over baselines; zero-shot 76.6%, fine-tuned 97.8% on CMATERdb; corruption robustness 75.7% vs. 38.7% for large baselines.

Conclusion: The DHCD task has reached an intrinsic error floor where all models perform similarly; a compact model is sufficient, and offers superior corruption robustness over larger baselines.
...
TL;DR: The paper proposes Groc-PO, a grounded preference optimization framework for Multimodal LLMs that applies stage-specific preference supervision across object grounding, contextual grounding, and grounded reasoning stages (via the GCPD dataset), outperforming standard DPO in reducing hallucinations and improving faithful reasoning.


<details>
  <summary>Details</summary>
Motivation: Standard DPO applies preference optimization only at the final-answer level, failing to address error propagation from early grounding stages (e.g., visual hallucinations, grounding drift, context inconsistency) in multimodal reasoning pipelines.

Method: The authors propose Groc-PO, a grounded preference optimization framework, along with the GCPD dataset that organizes multi-stage preference samples around three stages: Object Grounding, Contextual Grounding, and Grounded Reasoning, providing stage-specific preference supervision.

Result: Groc-PO achieves improved performance over standard DPO and strong baselines in hallucination mitigation, faithful reasoning, and overall reliability.

Conclusion: More explicit grounded supervision across multiple stages strengthens context-dependent reasoning and mitigates cross-stage error propagation, supporting trustworthy multimodal reasoning.
...
TL;DR: M$^\text{4}$World is a multi-view, multimodal generative driving world model that synthesizes surround-view video and LiDAR scans with object-level controllability and stable minute-long streaming, enabled by a flexible conditioning interface and multi-stage training.


<details>
  <summary>Details</summary>
Motivation: Driving-world generation lacks object-level controllability and long-horizon stability for scalable autonomous-driving simulation.

Method: Proposes a multi-view, multimodal generative model with (1) flexible conditioning for spatial layout and visual appearance control, (2) multi-stage training for 4-step causal generation, (3) few-clip post-training for rare-case customization, and (4) an automated VLM-based judging pipeline.

Result: Achieves high generation quality, precise controllability, and stable minute-long streaming; demonstrates downstream long-tail augmentation and scene editing capabilities.

Conclusion: M$^\text{4}$World shows strong potential for controllable, scalable driving simulation.
...
TL;DR: Deep video models for EF estimation are spatially faithful (focus on left ventricle) but temporally blind—they ignore clinically decisive ES/ED frames—revealing that spatial attribution alone cannot certify clinically meaningful explanations.


<details>
  <summary>Details</summary>
Motivation: Post-hoc attribution methods (e.g., Grad-CAM, Chefer relevance) are used to certify that models "look at the right place," but whether these explanations are faithful both spatially and temporally has not been audited. Since EF is defined by ES/ED frames, temporal faithfulness is critical.

Method: Fine-tuned two EF regressors (VideoMAE transformer, R(2+1)D CNN) on EchoNet-Dynamic. Audited with architecture-matched attribution along three axes: IoR against LV masks, deletion AUC, and temporal localization index on ES/ED frames, each with per-case 95% CIs over 50 studies. Used tubelet-occlusion probe to separate attribution failure from model behavior.

Result: Both models are spatially faithful (IoR 2.91x for VideoMAE, 1.98x for R(2+1)D above chance) but temporally blind (temporal localization indistinguishable from chance, 0.97–1.00). Occlusion confirmed models do not preferentially rely on ES/ED frames (0.90x chance), indicating temporal blindness reflects model behavior, not attribution artifact.

Conclusion: Spatial faithfulness does not imply temporal faithfulness. Attribution can certify anatomical grounding while masking that models ignore clinically decisive frames. This is a caution for XAI-based validation of video diagnostic models and motivates temporally-aware training and evaluation.
...
TL;DR: This paper introduces EgoProceVQA, a benchmark for egocentric procedural understanding with 3,600 key-step-centric questions across 31 everyday tasks. It also presents EgoProceGen (a data generation platform) and EgoProceAgent (a self-skill-exploration agentic framework) that achieves SOTA among open-source models.


<details>
  <summary>Details</summary>
Motivation: Existing egocentric video evaluations rarely address procedural understanding and overlook complex key-step-level reasoning under the VQA paradigm, which is crucial for building procedural AI assistants on wearable devices.

Method: The authors propose (1) EgoProceVQA task with six types of key-step-centric questions, (2) EgoProceGen data generation platform, and (3) EgoProceAgent framework with a generic tool library and standardized sub-skill library enabling self-exploration without ground-truth supervision.

Result: Existing MLLMs and agents show substantial room for improvement; EgoProceAgent achieves state-of-the-art performance among open-source models on multiple tasks.

Conclusion: The benchmark, generation platform, and agentic framework together establish a unified foundation for egocentric procedural understanding VQA, demonstrating that self-skill-exploration can improve procedural reasoning capabilities.
...
TL;DR: This study evaluates multimodal fusion strategies for chest radiograph diagnosis using 15,000 studies, finding that prospective Indication text alone achieves strong performance (AUROC 0.749), fusion methods provide marginal gains (SectionGuard-MI AUROC 0.783), and full report text creates circularity (AUROC 0.979), highlighting that post-hoc report leakage is a major concern.


<details>
  <summary>Details</summary>
Motivation: Chest radiograph datasets often combine images with clinical text (Indication, Findings, Impression) produced at different care stages, creating potential label leakage. The authors aim to evaluate whether prospective (pre-diagnosis) inputs alone are sufficient, and whether multimodal fusion provides meaningful gains over image-only or text-only models while controlling for post-hoc report circularity.

Method: 15,000 ReXGradient-160K studies with two images and five CheXbert-derived observations were used. Frozen DenseNet-121 (images) and Bio+ClinicalBERT (text) encoders were compared across seven model variants: image-only, Indication-only, fixed-order multimodal, random-swap, DeepSets, SectionGuard-MI, and full report text. Models were trained with five seeds; uncertainty estimated via 2,000 bootstrap replicates.

Result: Under U-Ones, macro AUROC: primary image 0.643 → two images 0.694 → Indication 0.749 → ordinary fusion 0.780. SectionGuard-MI achieved 0.783 AUROC and 0.260 AUPRC. DeepSets had highest AUROC (0.787), random-swap had highest AUPRC (0.265). Full report text reached 0.979 AUROC, staying above 0.973 after masking. SectionGuard-MI vs. ordinary fusion: AUROC diff not significant (p=0.374), AUPRC diff significant (p=0.004).

Conclusion: Prospective Indication text is strongly associated with report-derived targets and outperforms image-only models. Permutation-aware fusion methods (DeepSets, random-swap) are competitive with dedicated leakage-control approaches. Post-hoc report text creates substantial circularity, making prospective evaluations critical for realistic performance assessment.
...
<details>
  <summary>Details</summary>
Motivation: Existing RGB-only video deraining methods struggle in dynamic rainy scenes because rain streaks, textures, boundaries, motion, and occlusions share similar visual patterns. While event cameras offer complementary motion cues, direct RGB-Event fusion introduces cross-modal interference due to sensor noise and background-triggered responses in event streams.

Method: RainDancer adopts a decompose-before-interact paradigm: (1) RGB branch progressively decomposes frame features into rain and background representations; (2) Event branch uses a rain-oriented spiking neural network (SNN) to capture sparse/bursty rain event dynamics; (3) Component-level fusion between semantically aligned representations; (4) Event-domain supervision for sparse event reconstruction, structural consistency, and gradient orientation.

Result: Experiments on synthetic and real RGB-Event video deraining datasets show superior quantitative performance, visual quality, and downstream perception robustness compared to existing methods.

Conclusion: The decompose-before-interact paradigm effectively addresses cross-modal interference in RGB-Event video deraining, and the proposed RainDancer framework achieves state-of-the-art results. Code is publicly available.
...
TL;DR: AspectCLIP addresses the information asymmetry between images and text in contrastive learning by partitioning samples into aspect-coherent clusters and applying selective consistency regularization, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Existing global consistency regularization methods in CLIP overlook the inherent information asymmetry: images paired with divergent captions describing different aspects, causing semantic distortion in representation space.

Method: AspectCLIP partitions training samples into attribute clusters based on textual similarity, applies full cyclic consistency within each cluster, and restricts cross-cluster regularization to prototype-level comparisons, preserving geometric alignment only for consistent facets.

Result: Extensive experiments on downstream tasks show AspectCLIP consistently outperforms traditional methods and achieves a more structured representation space.

Conclusion: Aspect-guided regularization effectively respects the one-to-many structure of image-text pairs, reducing semantic distortion and improving representation quality.
...
TL;DR: This paper introduces a decoupled radiance representation for 3D Gaussian Splatting that separates low-frequency geometry/view-dependent features (via 2D surfels) from high-frequency textures (via baked texture atlas), achieving 5x speedup over 3DGS with real-time 4K@60 FPS rendering.
...
Method: Decouples radiance into low-frequency (2D surfels for geometry and view-dependent appearance) and high-frequency components (view-independent spatial hash grid baked into a compact texture atlas). Includes sparsity-enhancing optimizations that penalize semi-transparency and per-primitive falloff for aggressive pruning.

Result: Achieves up to 5x speedup over 3DGS while maintaining state-of-the-art visual fidelity, enabling real-time 4K rendering at 60 FPS on consumer hardware.

Conclusion: Exploiting geometric sparsity and efficient GPU texture mapping allows significantly faster and sparser reconstructions than prior work, making high-resolution real-time rendering practical on consumer hardware.
...
TL;DR: A fully automated multimodal deep learning framework using Swin-UNETR with auxiliary segmentation and dynamic multitask learning to classify PDAC resectability (NCCN categories) from CT and clinical data.


<details>
  <summary>Details</summary>
Motivation: Expert assessment of PDAC resectability via CT shows substantial variability, motivating an automated objective approach to reduce inter-reader disagreement.

Method: Uses Swin-UNETR backbone with auxiliary segmentation (pancreas, tumor, vessels) → fuses image features with clinical embedding (17 variables) → lightweight classification head; dynamic multitask loss adjusts segmentation vs. classification weighting based on tumor Dice.

Result: Not explicitly stated in abstract; effectiveness implied by framework design (anatomy-aware + clinical fusion + adaptive training).

Conclusion: The proposed multimodal framework with dynamic multitask learning can potentially improve consistency and accuracy in PDAC resectability classification.
...
TL;DR: This paper introduces a novel slice-wise data synthesis paradigm to create a large-scale structured reasoning dataset for 3D medical image understanding. By decomposing 3D radiology reading into fine-grained, per-slice Chain-of-Thought reasoning, they instruction-tune a 2D-pretrained MLLM to achieve volumetric comprehension, significantly improving performance over 2D baselines and rivaling native 3D architectures without expensive 3D pre-training.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with 3D volumetric imaging due to prohibitive annotation costs, dataset opacity, and lack of explicit clinical reasoning in existing data formats like rigid VQA pairs or unstructured clinical reports.

Method: A slice-wise data synthesis paradigm inspired by radiologists' diagnostic workflow: decomposes 3D reading into per-slice observations with Chain-of-Thought reasoning, enforcing spatial tracking, multi-slice awareness for artifact mitigation, and differential exclusion. A 2D-pretrained MLLM is instruction-tuned on this synthesized data.

Result: Significant performance improvements over 2D baselines across multiple 3D medical benchmarks; robust spatial reasoning capabilities; competitive with resource-intensive native 3D architectures.

Conclusion: This data-centric strategy unlocks deep volumetric understanding and interpretable clinical logic without requiring computationally expensive 3D-specific pre-training, effectively bridging the performance gap between 2D and 3D approaches.
...
TL;DR: AgentHOI is a training-free, agentic framework that leverages multimodal reasoning capabilities of foundation models for human-object interaction detection, achieving superior performance without any HOI detection training data.


<details>
  <summary>Details</summary>
Motivation: Traditional HOI detection relies on supervised learning over predefined categories, which limits generalization to open-world scenarios. Existing MLLM-based prompting approaches fail to fully exploit multimodal reasoning capabilities for ambiguous interactions.

Method: AgentHOI uses two key mechanisms: (1) Context-aware Multi-round Reasoning for progressive refinement of interaction hypotheses, and (2) Multifaceted Interaction Localization for instance-specific descriptions integrating semantic, spatial, and appearance cues.

Result: Superior performance over state-of-the-art supervised and weakly supervised methods in real-world settings, despite requiring no HOID data for training.

Conclusion: A training-free agentic framework that effectively transfers generalist multimodal reasoning to HOI detection in open-world scenarios.
...
TL;DR: OCP-CT proposes an organ-conditioned pattern-token alignment framework for CT vision-language pretraining, using sparse Mixture-of-Experts to route image/text tokens by radiological patterns and learnable slots for token-level contrast, achieving state-of-the-art zero-shot diagnosis on CT-RATE and RAD-ChestCT.


<details>
  <summary>Details</summary>
Motivation: Existing CT vision-language pretraining methods suffer from either overly coarse global contrast (obscuring organ-level evidence) or imprecise organ-level alignment (same anatomy can have multiple radiological appearances), necessitating a finer alignment unit: organ-conditioned radiological patterns.

Method: OCP-CT preserves a global CT-report contrastive branch and introduces an organ pattern interface with: (1) sparse Mixture-of-Experts to route image/text tokens by latent radiological patterns, (2) learnable slots to query routed tokens into continuous pattern tokens, and (3) paired token contrast with structured soft targets from report-derived clinical similarity.

Result: OCP-CT achieves average AUROCs of 84.5% on CT-RATE and 69.9% on RAD-ChestCT for zero-shot abnormality diagnosis, yielding absolute gains of 6.7 and 0.8 percentage points over the strongest prior results.

Conclusion: Organ-conditioned pattern-token alignment (via MoE routing + learnable slots + structured soft targets) significantly outperforms prior global and organ-level pretraining methods for CT radiology.
...
TL;DR: The 2nd International StepUP Competition evaluates pressure-based footstep biometrics on a large dataset (150 users, 200K+ footsteps), tackling unseen user generalization, cross-domain robustness (footwear/speed), and stride-level fusion. The best system (8.00% EER) used spatiotemporal CNN with ensemble scoring, but recognizing users in unseen footwear remains challenging.


<details>
  <summary>Details</summary>
Motivation: To advance pressure-based footstep biometrics through a standardized, challenging evaluation framework that addresses real-world issues like unseen users, domain shifts from footwear/speed variations, and fusion of left-right footsteps.

Method: The competition used the StepUP-P150 dataset (200K+ high-resolution dynamic footsteps from 150 individuals) with a previously unreleased test set, introducing three challenges: (1) unseen user generalization with limited enrollment, (2) cross-domain robustness (extreme footwear/speed variations), (3) stride-level verification with left-right footstep fusion. 26 teams participated.

Result: Best equal error rate (EER) of 8.00% achieved by ArogyaPandit Research Team using spatiotemporal CNN with ensemble-based scoring. Top solutions leveraged temporal patterns and inference-time normalization/calibration. Recognizing users in unseen personal footwear remains a key difficulty.

Conclusion: Temporal pattern mining and inference-time scoring strategies are effective for footstep biometrics, but robustness to unseen footwear remains a major open challenge, especially with similar-looking distractor users.
...
TL;DR: TCA-Net proposes a Thresholded Cross-Attention mechanism for Low-Light Image Enhancement that adaptively fuses intensity and chromaticity streams in HVI space, replacing fixed Top-K sparse attention with a confidence-threshold approach, complemented by Fourier-based brightness initialization and decoupled dual-stream guidance, achieving competitive accuracy and color fidelity with compact parameters.


<details>
  <summary>Details</summary>
Motivation: Existing HVI-based LLIE methods decouple intensity and chromaticity but use fixed-quota Top-K sparse attention for fusion, which is mismatched with layer-dependent confidence levels—discarding informative dependencies in some layers while retaining noisy ones in others.

Method: TCA-Net introduces (1) Thresholded Cross-Attention (TCA) that replaces rigid Top-K with a fixed confidence threshold, making retained cardinality input- and layer-adaptive; (2) Phase-guided Fourier Interaction Module for structure-aware brightness initialization; (3) Decoupled Dual-Stream Guidance Module to suppress chromaticity leakage; and (4) Scale-Aware Consistency Regularization for robustness.

Result: Experiments on LOL-v1, LOL-v2, Sony-Total-Dark, and LSRW-Huawei show competitive restoration accuracy, improved color fidelity, and compact parameter size.

Conclusion: TCA-Net demonstrates that adaptive confidence-based fusion, rather than fixed-quota selection, is key to reliable intensity-chromaticity fusion in LLIE, achieving a strong balance among restoration quality, color fidelity, and efficiency.
...
TL;DR: Cyclone is a unified latent diffusion framework with cycle-consistency and vision-language knowledge that realistically edits weather conditions in driving scenes without paired data, improving downstream perception tasks and extending to temporally consistent video editing.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving systems struggle with perception under diverse weather conditions. Existing methods either rely on synthetic data augmentation or physics-based models that need paired training data and fail to generalize to out-of-domain scenarios.

Method: Cyclone is a latent diffusion-based weather editing framework with cycle-consistent constraints and knowledge from image-text models, operating without paired training data. It can also be distilled into a video diffusion model for temporal consistency.

Result: The method produces more realistic, structure-preserving weather-edited outputs than existing baselines and yields consistent improvements across multiple downstream driving perception tasks.

Conclusion: Cyclone offers a scalable, unified solution for weather-robust perception in autonomous driving, with strong generation quality and downstream task benefits.
...
TL;DR: SIVA-RL replaces intervention-type-based supervision with outcome-conditioned routing for vision-language alignment in RLVR, achieving consistent gains across 9 multimodal benchmarks.


<details>
  <summary>Details</summary>
Motivation: Answer-level correctness in RLVR doesn't guarantee visual grounding. Existing methods assign supervision by intervention type, which fails because identical interventions yield heterogeneous effects across different samples.

Method: SIVA-RL uses token-aligned, distance-constrained within-image PatchSwap for localized interventions. A frozen audit policy scores each clean-intervention pair; reward drop determines soft routing weights: large-drop→sensitivity alignment, low-drop→invariance alignment, ambiguous→down-weighted.

Result: Across 9 multimodal benchmarks (math, logic, vision tasks), SIVA-RL improves 3B and 7B models over RL baselines in every setting. +8.79 pp on vision-dependent reasoning, up to 14.9% relative improvement across GRPO and DAPO backbones.

Conclusion: Decoupling intervention construction from supervision assignment via outcome-conditioned routing is effective and backbone-agnostic for visual grounding in RLVR.
...
TL;DR: This study uses simultaneous PET/MR imaging with multiparametric quantitative analysis and two-step clustering to identify abnormal myocardial regions in arrhythmogenic left ventricular cardiomyopathy patients, achieving accurate automated health reports (BA=0.76±0.04 in patients, BA≥0.8 on phantoms) that closely match cardiac physician assessments.


<details>
  <summary>Details</summary>
Motivation: Arrhythmogenic left ventricular cardiomyopathy is difficult to diagnose due to the lack of gold standard criteria, and there is a need for better methodological strategies to handle multimodal PET/MRI data for characterizing myocardial heterogeneity.

Method: Two-step clustering (supervoxel clustering followed by spectral clustering) was applied to z-scored and summed T1/T2 maps, LGE, and 18F-FDG-PET images from 99 genetically diagnosed patients; abnormality scores were assigned per cluster/modality for automated report generation, validated via nested cross-validation and on 167 numerical phantoms.

Result: Automated reports achieved balanced accuracy of 0.76±0.04 in repeated nested cross-validation on patients and ≥0.8 on phantoms, closely matching cardiac physicians' visual observations of fibrosis/inflammation.

Conclusion: This clustering-based approach enables systematic handling of multimodal PET/MRI data to characterize myocardial heterogeneity in arrhythmogenic left ventricular cardiomyopathy, facilitating automated health report generation.
...
TL;DR: Peak-End-Net is a lightweight, interpretable framework for video aesthetic assessment inspired by the peak-end rule from psychology. It uses a pretrained image aesthetic assessment head for frame-wise priors, an aesthetic rhythm encoder for temporal progression, and a dynamic gated fusion mechanism for robustness. Achieving SOTA on VADB and DIVIDE-3K, it demonstrates the value of psychologically grounded modeling with minimal trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Video aesthetic assessment is underexplored due to lack of large-scale benchmarks and the inherent subjectivity of aesthetic judgment shaped by human perception. The authors aim to address these challenges by revisiting VAA from a psychological perspective.

Method: (1) Transfer knowledge from IAA to VAA via a pretrained IAA head for frame-wise aesthetic priors; (2) Peak-end rule-based temporal aggregation to identify salient moments and endings; (3) Aesthetic rhythm encoder to model temporal evolution; (4) Dynamic gated fusion for robustness under distribution shift; (5) Built on a frozen ViT with few trainable parameters.

Result: Achieves state-of-the-art performance on two VAA benchmarks: in-domain evaluation on VADB and cross-domain testing on DIVIDE-3K.

Conclusion: Psychologically grounded modeling (peak-end rule) is valuable and effective for VAA. The approach is lightweight, interpretable, and parameter-efficient.
...
TL;DR: This paper demonstrates that published methane plume metrics (IME, plume length, emission rate) from imaging spectrometers do not uniquely define the plume boundary, using genetic-algorithm ensembles to reveal equifinality. The authors introduce PlumeQuant, a tool that recomputes these quantities under consistent conventions, and evaluate a transparent Carbon Mapper-like (CM-like) mask generation method that achieves high consistency with published products (0.843 median IoU, +0.72% IME difference, +0.16% emission rate difference) while flagging ambiguous plumes for expert review.


<details>
  <summary>Details</summary>
Motivation: To reveal and quantify the equifinality (multiple plausible solutions) in published methane plume products from imaging spectrometers, where different plume boundaries can yield the same scalar metrics (IME, length, emission rate).

Method: Used genetic-algorithm (GA) ensembles conditioned on published IME and plume length to generate alternative masks; developed PlumeQuant to recompute metrics under consistent conventions; created a CM-like mask generator using fixed settings on a 44-plume development split, tested on 63 EMIT-derived Carbon Mapper plume records from 27 scenes.

Result: High-confidence core covers median 13% of plausible footprint envelope; CM-like masks achieved 0.843 median IoU with reference masks, +0.72% median IME difference, +0.16% emission rate difference (6.98% mean absolute), and 1.01 median uncertainty ratio; holdout MAEs were 7.6% (IME), 9.5% (length), 6.1% (rate).

Conclusion: Published scalar plume metrics underdetermine the actual plume boundary, especially for weak, low-overlap plumes. PlumeQuant and the CM-like mask generator provide consistency diagnostics that can flag problematic plumes for expert review, but these are not independent validation tools.
...
TL;DR: CF-Net is a multimodal deep network for detecting ambivalence/hesitancy in video, using frozen backbones (SigLIP2, HuBERT, DistilBERT), per-speaker normalization, a ConflictFusion module for cross-modal incongruence, and specialized training techniques to achieve 0.7364 Macro F1 on the BAH challenge test set.


<details>
  <summary>Details</summary>
Motivation: Ambivalence and hesitancy detection in unconstrained video is challenging because the signal is subtle, ambiguous, and expressed through cross-modal incongruence rather than clear prototypical affect patterns.

Method: CF-Net encodes visual, audio, and transcript streams using frozen SigLIP2, HuBERT, and DistilBERT backbones respectively; normalizes features per speaker to reduce identity leakage; fuses them via a ConflictFusion module that explicitly computes pairwise cross-modal incongruence; and trains with certainty-weighted focal loss, manifold mixup, modality dropout, and an auxiliary certainty-regression head.

Result: CF-Net achieves Macro F1 of 0.7155 on the BAH validation set and 0.7364 (AP = 0.7492) on the private challenge test set.

Conclusion: The explicit modeling of cross-modal incongruence via ConflictFusion, combined with training strategies that handle ambiguity, effectively captures subtle AH signals and achieves state-of-the-art results on the BAH dataset.
...
TL;DR: This paper proposes a structure-aware dance generation framework that models choreography as sequences of atomic movements. It uses LLMs to build an interpretable atomic movement vocabulary, then employs a two-stage generation pipeline (planning + completion) to produce dances with better structural coherence, rhythmic alignment, and controllability.


<details>
  <summary>Details</summary>
Motivation: Existing music-driven dance generation methods model motion as a continuous signal, ignoring its compositional nature. This leads to structurally incoherent dances that are difficult to control or edit.

Method: 1) Segment large-scale dance data and cluster into atomic movement groups. 2) Use LLM to semantically relabel and refine clusters into an interpretable vocabulary. 3) Two-stage generation: atomic movement planning (predict type, duration, timing) followed by transition-aware completion for smooth motion.

Result: The method achieves significantly improved structural coherence, rhythmic alignment, and perceptual naturalness compared to existing baselines, with enhanced interpretability and controllable editing.

Conclusion: Modeling dance as compositional atomic movements improves both generation quality and controllability, closely mirroring human choreography.
...
TL;DR: VisionScreen adapts the Screening mechanism from NLP to vision, enabling each image patch to independently evaluate and exclude irrelevant patches based on query-key similarity and spatial relevance, outperforming standard Vision Transformer in classification tasks.


<details>
  <summary>Details</summary>
Motivation: Standard ViT's softmax-based self-attention assigns relative weights to all patches, making it impossible to explicitly reject irrelevant/background patches, which introduces unnecessary information into feature aggregation.

Method: VisionScreen extends the Screening mechanism (from language modeling) to visual recognition by treating image patches as tokens on a 2D grid, estimating absolute relevance via query-key similarity in the 2D spatial domain, and explicitly excluding low-relevance patches through thresholding without inter-patch competition.

Result: Experiments on image classification benchmarks show VisionScreen outperforms conventional Vision Transformer.

Conclusion: Screening can be effective for visual recognition, providing an alternative to softmax-based relative feature aggregation.
...
TL;DR: This paper proposes a task-adaptive feature fusion approach for the ABAW11 Multi-Task Learning Challenge, using frozen pretrained visual features (DINOv2 ViT-L and DINOv3 ConvNeXt-base) with task-specific prediction heads and fusion strategies, achieving strong results on valence-arousal, expression, and action unit prediction.


<details>
  <summary>Details</summary>
Motivation: Although valence-arousal, categorical expressions, and facial action units are naturally related through facial behavior, validation experiments show they benefit from different visual features, temporal processing strategies, fusion mechanisms, and calibration procedures. Thus, a single shared architecture is suboptimal.

Method: Two pretrained backbones (DINOv2 ViT-L and DINOv3 ConvNeXt-base) are adapted on an external expression dataset and frozen. Frame-level features are extracted, then task-specific fusion and prediction strategies (frame-level heads, temporal CNNs, post-hoc smoothing, LightGBM, various fusion types, threshold calibration, shared MTL structures) are compared to select optimal per-task configurations.

Result: On the ABAW11 validation set: EXPR macro-F1 0.4222, AU macro-F1 0.5402, mean VA CCC 0.6717, overall score 1.6341.

Conclusion: Task-adaptive fusion of frozen visual features is a simple and effective strategy for multi-task affective behavior analysis, outperforming forcing all tasks to share a single architecture.
...
TL;DR: A multi-expert OCR system for historical Manchu texts uses a lightweight page-level image classifier to dispatch pages to domain-specific specialists, achieving high accuracy on three writing styles with minimal additional training.


<details>
  <summary>Details</summary>
Motivation: Historical Manchu OCR must handle diverse writing styles (regular script, running script, chancery hand) with limited labeled data, motivating a system that can adapt to different visual styles without requiring extensive training data for each domain.

Method: They build a multi-expert system repurposing checkpoints from iterative fine-tuning as domain specialists, with a lightweight page-level image classifier (router) that dispatches pages by visual style. If no suitable specialist exists, a new expert is trained for that domain.

Result: The routed system achieves 0.30% CER (regular script), 1.57% CER (memorials), and 4.83% CER (running script). Router achieves 99.3% page-level domain accuracy, matching oracle performance at two-decimal precision.

Conclusion: The multi-expert routing approach effectively handles diverse Manchu writing styles with limited labeled data, and most selected specialists were not originally trained for their final domain, demonstrating efficient reuse of checkpoints.
...
TL;DR: This paper analyzes interactive game world modeling using a recurrent action-state-observation loop framework, examining four key dimensions: player action control, game state dynamics, state-observation persistence, and real-time generation. It also introduces a data engine for Black Myth: Wukong with over 90 hours of annotated gameplay.


<details>
  <summary>Details</summary>
Motivation: Building interactive worlds that respond coherently to player actions is a shared goal of computer graphics, games, and AI. While video generative models show promise as next-generation game engines, genuinely interactive worlds require rule-following outcomes, long-horizon consequences, and real-time generation—properties current generative models struggle with.

Method: The paper uses the conventional game engine's action-state-observation loop as an organizing lens to examine four dimensions: (1) player action control, (2) game state dynamics, (3) state-observation persistence, and (4) real-time interactive generation. For each dimension, it categorizes existing approaches into representative families and discusses trade-offs.

Result: The paper presents a scalable data engine for Black Myth: Wukong collecting over 90 hours of gameplay with frame-aligned player actions, ground-truth game states, visual observations, and structured/semantic annotations as a resource for state-aware game world modeling.

Conclusion: The paper offers a clear picture of the current state of interactive game world modeling, categorizing approaches along four key dimensions, and hopes to foster progress toward genuinely interactive game worlds.
...
TL;DR: VideoRAE is a representation autoencoder that compresses frozen Video Foundation Model (VFM) features into compact, reconstruction-capable, and generation-friendly video latents using a lightweight 1D self-attention projector, achieving state-of-the-art reconstruction quality and faster convergence than conventional 3D-VAEs.


<details>
  <summary>Details</summary>
Motivation: Conventional 3D-VAEs optimized for pixel-level reconstruction limit the semantic and spatio-temporal structure of video latents, while frozen Video Foundation Models (VFMs) like V-JEPA 2 and VideoMAEv2 have strong video understanding capabilities but it's unclear if their representations can be transformed into compact, reconstruction-capable, and generation-friendly latents.

Method: VideoRAE leverages multi-scale hierarchical features from a frozen VFM encoder, compresses them with a lightweight 1D self-attention projector, supports continuous latents (for Diffusion Transformers) and discrete tokens (for autoregressive models via multi-codebook high-dimensional quantization), and uses a local-and-global representation alignment objective with the frozen VFM teacher during decoding to improve semantic preservation without KL regularization.

Result: On UCF-101, VideoRAE achieves state-of-the-art class-to-video gFVDs of 40 (AR) and 93 (DiT), converges ~5x faster than competing autoencoder baselines. In a 2B-scale text-to-video study, replacing LTX-VAE with VideoRAE leads to faster convergence under comparable settings.

Conclusion: Frozen VFM representations are validated as versatile and generation-friendly video latents. VideoRAE demonstrates strong reconstruction in both continuous and discrete regimes, confirming that frozen VFM features can effectively replace conventional 3D-VAE latents for video generation.
...
### [87] [LiTCom: A Lightweight Transmitter and Inference-Capable Receiver Framework for 6G Uplink](https://arxiv.org/abs/2607.13114)
*Chunmei Xu,Siqi Zhang,Zhi Ding,Yi Ma,Rahim Tafazolli*

Main category: eess.SP

TL;DR: LiTCom is a lightweight transmitter and GenAI-powered receiver framework for 6G uplink that uses simple low-pass filtering and minimal coding at the edge, while leveraging large generative AI models at the receiver to recover high-fidelity content from degraded signals, achieving up to 8 dB SNR gain over 5G NR and reducing transmitter computation by 95%.


<details>
  <summary>Details</summary>
Motivation: To address resource asymmetry in 6G networks by shifting complexity from resource-constrained edge devices (transmitters) to the network infrastructure (receiver), enabling robust uplink communication under extremely low SNR conditions.

Method: LiTCom employs lightweight low-pass filtering for source coding and minimal channel coding at the transmitter, while deploying large-scale generative AI models at the receiver for semantic inference and signal reconstruction. It also introduces data-importance-aware power allocation strategies optimized for a QoE metric.

Result: LiTCom achieves SNR gains of up to 8 dB over 5G NR-like baseline (JPEG+LDPC) and 2.5 dB over Deep-JSCC baseline, while reducing transmitter-side computations by over 95%.

Conclusion: The proposed lightweight transmitter + GenAI receiver paradigm is highly effective for 6G uplink under low SNR, demonstrating that shifting processing burden to the receiver can dramatically reduce edge complexity without sacrificing—or even improving—performance.
...
### [88] [Harmonic Analysis on Graphs via Isometric Group Embedding: A Canonical Fourier Transform, Shift, and Convolution for Network Signals](https://arxiv.org/abs/2607.13338)
*Rigobert Fokam Souop,Laurent Bitjoka*

Main category: eess.SP

TL;DR: This paper proposes a group-embedding graph Fourier transform that resolves three structural limitations of standard graph signal processing: non-unique eigenbases, non-isometric shifts, and lack of true convolution.


<details>
  <summary>Details</summary>
Motivation: Standard graph signal processing based on Laplacian/adjacency eigenvectors has three inherent compromises: ambiguous eigenbases due to degenerate eigenspaces, the shift operator not being an isometry, and the absence of genuine translation and convolution operations.

Method: The authors embed a connected graph isometrically into a Cayley graph of a finite abelian group, then define a graph Fourier transform using the host group's characters, lift graph signals to the host for processing, and leverage the group's exact translation and convolution structure.

Result: Structural identities (Plancherel, convolution, translation-covariance, sampling) hold to machine precision numerically. Under a same-filter protocol, the group-character basis achieves equivalent denoising performance to the Laplacian eigenbasis when the host complement is filled with smoothness-respecting extension.

Conclusion: The contribution is an exact, canonical Fourier structure for graph signals—not a denoising advantage—providing a principled alternative to Laplacian-based harmonic analysis on graphs.
...
### [89] [Fine-Grained Open-Set Fault Diagnosis via Metric-Guided Time-Frequency Configuration Selection and Class-Specific Autoencoders](https://arxiv.org/abs/2607.13368)
*Youngjae Jeon,Dongjin Lee*

Main category: eess.SP

TL;DR: A new open-set fault diagnosis method that uses metric-guided STFT configuration selection and class-specific autoencoders to achieve fine-grained fault severity diagnosis with lower configuration-selection cost.


<details>
  <summary>Details</summary>
Motivation: Existing open-set fault diagnosis methods struggle with fine-grained severity diagnosis due to reliance on coarse type levels, heuristic STFT settings, and global class boundaries. Real machinery may encounter previously unseen fault states.

Method: Combines MGDC (Metric-Guided Data-Centric STFT configuration selection using Silhouette score) with CSAE (Class-Specific AutoEncoder) bank for learning compact class-specific manifolds, plus a dual-criteria rejection mechanism using latent dimension-wise boundaries and reconstruction error.

Result: Achieves H-scores of 0.9924 (CWRU) and 0.9509 (PU) for fine-grained fault severity diagnosis. Reduces configuration-selection cost by factors of 5.69× (CWRU) and 29.87× (PU) compared to exhaustive search.

Conclusion: The proposed MGDC-CSAE method supports accurate open-set severity diagnosis with substantially lower configuration-selection cost, enabling practical deployment for real industrial systems.
...
### [90] [Parametric Diffraction-Based Object Sensing: Modeling, Estimation, and Fundamental Limits](https://arxiv.org/abs/2607.13417)
*Jiaqi Xu,Bjorn Ottersten,A. Lee Swindlehurst*

Main category: eess.SP

TL;DR: This paper develops a physics-consistent diffraction channel model for sensing environmental objects at wireless frequencies, including ML estimation methods for blockage shape, range, and DoAs, with CRB-based performance limits and Fresnel-number scaling laws.


<details>
  <summary>Details</summary>
Motivation: To enable high-fidelity blockage characterization in wireless environments by leveraging diffraction mechanisms, which are prevalent at communication frequencies but underexplored for sensing purposes.

Method: Physics-consistent parameterized diffraction channel model integrating far-field, Fresnel, and near-field approximations; ML estimation for blockage shape, range, and DoAs; CRB analysis for fundamental limits; Fresnel-number scaling laws across frequency, size, and range.

Result: ML estimators closely approach the CRB at moderate to high SNR; diffraction-based modeling is effective for high-fidelity blockage characterization.

Conclusion: Diffraction-based sensing is viable and accurate for environmental object characterization at wireless frequencies, with rigorous theoretical foundations and practical estimation performance near fundamental bounds.
...
### [91] [Compositional Zero-Shot Recognition based on Tangent Space Disentanglement for Composite Modulation Signals](https://arxiv.org/abs/2607.13463)
*Yurui Zhao,Xiang Wang,Zhitao Huang,Baoguo Li*

Main category: eess.SP

TL;DR: The paper proposes a zero-shot learning framework with disentangled semantic space (TSDN) for automatic composite modulation recognition in ISAC systems, achieving 93% accuracy and robust performance under channel/hardware imperfections.


<details>
  <summary>Details</summary>
Motivation: Conventional ACMR methods struggle with semantic coupling between inner/outer-layer modulations in composite modulation, degraded performance under joint hardware/channel imperfections, and inability to handle unknown modulation schemes.

Method: A disentangled semantic space zero-shot learning framework is designed, using logarithmic projection to linearize multiplicative coupling between modulation layers, a learnable geometric transformation for layer-wise semantic features, and a multi-objective loss function balancing discrimination with cross-domain generalization. The framework is instantiated as Tangent Space Disentanglement Network (TSDN).

Result: TSDN achieves over 93% zero-shot recognition accuracy, significantly outperforms unified-semantic and multi-task baselines, and maintains robust performance under combined channel fading and hardware imperfections down to 4 dB SNR.

Conclusion: The proposed disentangled semantic space and TSDN effectively address key challenges in ACMR for ISAC systems, demonstrating strong zero-shot generalization and robustness.
...
### [92] [Posterior-Confidence Driven Beamforming for Energy-Efficient Integrated Sensing and Communication](https://arxiv.org/abs/2607.13470)
*Nusaibah A. Alshorman,Huseyin Arslan*

Main category: eess.SP

TL;DR: This paper proposes an energy-efficient MIMO DFRC beamforming framework for 6G ISAC systems that minimizes transmit power by adopting a skip-enabled sensing policy, selectively triggering sensing based on EKF-derived statistics (confidence metric and NIS), with sector-based beampattern constraints and safety illumination floor to ensure robust target tracking, achieving significant power reduction without compromising communication or sensing performance.


<details>
  <summary>Details</summary>
Motivation: Energy efficiency is a critical limitation for 6G ISAC systems due to high sensing power consumption from persistent sensing, despite stable communication requirements. The need to minimize transmit power while maintaining both communication QoS and reliable multi-target tracking drives this work.

Method: Proposes an energy-efficient MIMO DFRC beamforming framework with a tracking-aware, skip-enabled sensing policy. Sensing is selectively triggered based on two EKF-derived statistics: a posterior confidence metric (for estimation accuracy) and normalized innovation squared (NIS) (for measurement reliability). Sector-based beampattern constraints with a nonzero safety illumination floor ensure robustness under intermittent sensing.

Result: The proposed framework achieves significant reduction in transmit power compared to baselines, without deteriorating communication system performance or excessively impacting the sensing process.

Conclusion: The skip-enabled sensing policy effectively breaks the always-on sensing paradigm, enabling substantial energy savings in 6G ISAC systems while preserving both communication and sensing functionality.
...
### [93] [Pinching-Antenna Systems (PASS)-Based User-Side Navigation: An Anchor-Line-based Approach](https://arxiv.org/abs/2607.13485)
*Zongyi Li,Jun Wang,Tianwei Hou,Anna Li*

Main category: eess.SP

TL;DR: This paper proposes a user-side navigation framework for Pinching-Antenna Systems (PASS), where mobile users self-localize using downlink broadcast signals without prior knowledge of antenna positions. It introduces algorithms for PA positioning, pseudorange estimation, and weighted least-squares navigation, achieving centimeter-level accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional navigation systems rely on known infrastructure positions, which limits flexibility. PASS enables dynamic wireless channel reconfiguration, but users typically need prior knowledge of antenna positions to navigate. This work aims to eliminate that requirement.

Method: (1) LWF-PAP: Lambert W function-based algorithm for PA positioning and pseudorange estimation with closed-form expressions. (2) WLS-PAN: Weighted least squares-based navigation algorithm transforming nonlinear equations into a closed-form linear system with optimal weight matrix for minimum-variance unbiased estimation. (3) PA-PDOP: A dilution of precision metric for theoretical accuracy bounds.

Result: Centimeter-level positioning accuracy is achieved for both PAs and users within the breakpoint distance. Uniform PA deployment and moderate increase in PA count improve navigation accuracy.

Conclusion: The proposed framework is effective and robust for distributed real-time user-side self-navigation in PASS, enabling autonomous localization without prior PA position knowledge.
...
### [94] [Cross-Field Channel Parameter Estimation and Channel Characterization at THz Bands in Indoor Scenarios](https://arxiv.org/abs/2607.13535)
*Hengtai Chang,Cheng-Xiang Wang,Cunhua Pan,Jian Sun,Bingchang Hua,Yongchao He,el-Hadi M. Aggoune*

Main category: eess.SP

TL;DR: This paper proposes a cross-field SAGE algorithm for estimating near-field and far-field multipath components in THz communication systems using a 260-380 GHz virtual ULA measurement framework, finding that near-field MPCs dominate (over 90%) at short distances and spatial non-stationarity increases with higher frequency and shorter distance.


<details>
  <summary>Details</summary>
Motivation: To address the critical challenge of cross-field (hybrid near-field/far-field) channel parameter estimation and channel characterization in THz communication systems with large-scale antenna arrays, where spatial non-stationarity and mixed wavefront characteristics complicate traditional channel modeling approaches.

Method: Established a 260-380 GHz virtual ULA measurement framework; proposed a cross-field SAGE algorithm that uses Bayesian phase-curvature classification to discriminatively estimate near-field and far-field MPCs, while tracking spatial birth-death phenomena via visibility region estimation.

Result: The algorithm effectively resolves cross-field MPCs; near-field MPCs account for over 90% of total MPCs at 2 m transmission distance at 380 GHz; spatial non-stationarity intensifies with higher carrier frequency and shorter transmission distance.

Conclusion: The findings offer quantitative guidelines for channel modeling and system design in wireless THz communication systems, highlighting the dominance of near-field effects in typical short-range THz deployments.
...
### [95] [On the Performance of Pinching-Antenna Systems (PASS) Under Dynamic Channels with Blockages](https://arxiv.org/abs/2607.13581)
*Jinhua Wang,Jun Wang,Tianwei Hou,Anna Li,Yuanwei Liu,Arumugam Nallanathan*

Main category: eess.SP

TL;DR: This paper analyzes pinching-antenna systems (PASS) under realistic blockage by jointly considering LoS and NLoS components, developing channel models with in-waveguide attenuation and spatial propagation loss, deriving closed-form expressions for outage probability and ergodic rate under Rayleigh and Rician fading, and providing optimal PA deployment criteria.


<details>
  <summary>Details</summary>
Motivation: Existing PASS studies rely on LoS channels or probabilistic blockage models, ignoring realistic obstacle-induced blockage. This paper addresses the need to understand PASS performance under practical environments where both LoS and NLoS components coexist with geometry-aware blockage.

Method: A geometry-aware blockage model defines blockage regions on the waveguide based on actual obstacle locations and geometry. Channel models jointly account for in-waveguide attenuation and spatial propagation loss. A single-PA single-UE scenario is analyzed under Rayleigh and Rician fading channels, with closed-form analytical derivations for outage probability, ergodic rate, and optimal PA deployment criteria.

Result: i) NLoS scattering has a twofold effect—degrading outage performance but improving rate performance under Rician fading. ii) Strong NLoS scattering can sustain communication despite LoS blockage. iii) Optimal PA position depends on both environment geometry and the trade-off between spatial propagation loss and in-waveguide attenuation.

Conclusion: The study provides a comprehensive analytical framework for PASS under realistic blockage, offering closed-form performance expressions and practical deployment guidelines for optimal PA placement.
...
### [96] [On the Blockage Effect in Pinching-Antenna Systems (PASS)](https://arxiv.org/abs/2607.13582)
*Jinhua Wang,Jun Wang,Tianwei Hou,Xin Sun,Arumugam Nallanathan*

Main category: eess.SP

TL;DR: This paper uses stochastic geometry to analyze pinching-antenna systems (PASS) under random line-of-sight blockage, proposing RHRR and DHDR blockage models, deriving closed-form outage probability and ergodic rate expressions, and showing PASS outperforms conventional antennas despite denser/larger obstacles.


<details>
  <summary>Details</summary>
Motivation: PASS offer dynamic radiation position reconfiguration but face performance degradation due to random LoS blockage; the paper aims to analytically characterize this performance loss.

Method: Models obstacles as a homogeneous PPP; proposes two blockage models (RHRR and DHDR); derives closed-form analytical and asymptotic expressions for outage probability and ergodic rate using stochastic geometry.

Result: Denser obstacle environments or statistically larger obstacles increase LoS blockage probability and degrade performance; PASS consistently outperforms conventional antenna systems under LoS blockage.

Conclusion: PASS is robust against LoS blockage due to dynamic repositioning capability, and the proposed analytical framework effectively quantifies its performance advantages.
...
### [97] [RIS Beamforming under Element-Level Variations: Statistical Characterization and Robust Design](https://arxiv.org/abs/2607.13583)
*Dimitris Kompostiotis,Dimitris Vordonis,Vassilis Paliouras,George C. Alexandropoulos*

Main category: eess.SP

TL;DR: This paper introduces a statistical model to analyze how element-level variations (like varactor capacitance fluctuations) affect RIS radiation patterns, and proposes a low-complexity greedy optimization for robust configurations that achieves >2 dB gain and ~10 dB sidelobe suppression.


<details>
  <summary>Details</summary>
Motivation: To characterize and mitigate the impact of manufacturing/operational tolerances and element-level variations on RIS radiation performance, replacing costly Monte Carlo simulations.

Method: Proposes a statistical model for varactor capacitance fluctuations affecting RIS reflection coefficients, followed by a low-complexity greedy optimization to maximize expected radiation power under both low- and large-variance perturbation scenarios.

Result: For 32x32 or 64x64 RIS, the method achieves >2 dB main lobe gain improvement and ~10 dB sidelobe suppression under element-level variations.

Conclusion: Optimizing the mean power pattern significantly enhances RIS robustness to element-level variations, and the analytical model efficiently replaces Monte Carlo simulations for sensitivity quantification.
...
### [98] [M3F-UAV: A Missing-Modality Multimodal Foundation Model for Low-Altitude Wireless Sensing](https://arxiv.org/abs/2607.13678)
*Pengxuan Gao,Kai Ying,Botao Wu,Jianhua Mo,Qingsong Wen*

Main category: eess.SP

TL;DR: M3F-UAV is a missing-modality multimodal foundation model for low-altitude wireless sensing that fuses visual, geometric, and wireless data, achieving robust performance even when some modalities are unavailable.


<details>
  <summary>Details</summary>
Motivation: Low-altitude UAV wireless systems in urban environments suffer from visual occlusion, sparse geometry, multipath propagation, and sensor failures, which degrade single-modality model reliability.

Method: M3F-UAV uses modality-specific pretrained feature extractors (RGB/depth, LiDAR, CSI), cross-modal fusion, and missing-modality-aware pretraining with masked reconstruction and UAV localization objectives to learn unified multimodal representations adaptable to downstream tasks.

Result: On the LAMBDA dataset, M3F-UAV outperforms single-modality baselines and maintains robust performance under missing-modality conditions.

Conclusion: The proposed multimodal approach effectively handles missing modalities in complex urban low-altitude scenarios, enhancing the reliability of UAV-based wireless sensing.
...
### [99] [From Metric to Mechanism: Designing Wireless Resilience through Finite Blocklength Dynamics](https://arxiv.org/abs/2607.13710)
*Kevin Weinberger,Aydin Sezgin,Mehdi Bennis*

Main category: eess.SP

TL;DR: This paper proposes a cross-layer framework integrating finite blocklength communication for time-centric network resilience, with a novel interpretable resilience metric and alternating optimization to enable recovery from severe disruptions in URLLC scenarios.


<details>
  <summary>Details</summary>
Motivation: Next-generation wireless networks must maintain reliable operation under abrupt and severe disruptions, particularly in URLLC scenarios where strict time constraints dominate system design. Existing approaches lack a time-centric resilience perspective that treats transmission duration as a controllable resource.

Method: Proposes a unified cross-layer framework coupling queue dynamics, rate adaptation, and blocklength optimization. Introduces an interpretable resilience metric decomposing disruption impact into absorption loss, adaptation efficiency, and recovery behavior. Develops a three-stage alternating optimization approach jointly optimizing beamforming, RIS phase shifts, and blocklength.

Result: Demonstrates strong resilience performance under repeated channel disruptions and AI-driven traffic surges. The proposed metric enables intuitive and consistent comparison of resilience performance across different approaches and disruption types.

Conclusion: Time-aware resource allocation in the FBL regime is critical for network resilience. The cross-layer framework and interpretable metric effectively capture, evaluate, and compare resilience behaviors in URLLC scenarios.
...
### [100] [Design and Analysis of a Higher-Order Enhanced Phase-Locked Loop via the Ahmadi-Chaudhry-Zhang Newton Framework](https://arxiv.org/abs/2607.13752)
*Shafayat Abrar*

Main category: eess.SP

TL;DR: This paper proposes a new higher-order enhanced phase-locked loop (EPLL) based on the ACZ Newton framework, which eliminates spurious equilibria and saddle points while enlarging the basin of attraction compared to existing EPLL formulations.


<details>
  <summary>Details</summary>
Motivation: Existing EPLL formulations derived from first- or second-order optimization methods exhibit slow convergence, saddle-point attraction, or undesired oscillatory behavior.

Method: A continuous-time higher-order Newton flow (based on the ACZ framework) is formulated and applied to the EPLL through a coordinate optimization strategy, where the higher-order flow is applied only to phase update while amplitude retains its classical form.

Result: The proposed method eliminates spurious equilibria and saddle points present in autonomous Newton EPLL, substantially enlarges the basin of attraction for the desired equilibrium, and shares desirable convergence characteristics with modified EPLL.

Conclusion: The higher-order ACZ-based EPLL improves convergence properties and stability characteristics over standard, Newton, and modified EPLL formulations for sinusoidal voltage estimation in power systems.
...
### [101] [Hybrid Time-Frequency Domain Frequency Offset Compensation Under GHz Doppler Shift for LEO Satellite-to-Ground Coherent Free-Space Optical Communication](https://arxiv.org/abs/2607.13904)
*Tiankuo Jiao,Hossein Kazemi,Harald Haas*

Main category: eess.SP

TL;DR: A Doppler-aware hybrid time-frequency frequency offset compensation (HTF-FOC) receiver is proposed for LEO satellite-to-ground FSO links, achieving multi-GHz Doppler shift tracking with residual CFO below 80 MHz.


<details>
  <summary>Details</summary>
Motivation: High orbital velocity of LEO satellites introduces multi-GHz Doppler shifts that pose a major challenge for conventional coherent optical receivers, as narrow-range digital loops cannot acquire the initial offset and wide-range solutions leave large residual errors or high implementation costs.

Method: The proposed HTF-FOC receiver uses a cumulative time-varying random process model for dynamic Doppler-induced phase shift, and implements a hybrid acquisition and tracking procedure: 4th-power FFT-based coarse CFO acquisition, residual CFO handover verification, and low-complexity decision-directed frequency-locked loop (FLL) tracking.

Result: The method tracks Doppler shifts beyond ±5 GHz with residual CFO below 80 MHz and 100% success rate for LEO altitudes of 400–800 km and orbital speeds of 7.3–7.9 km/s. Phase-averaged PEP and union-bound SER expressions were derived and verified via Monte Carlo simulations.

Conclusion: The proposed HTF-FOC receiver effectively addresses the multi-GHz Doppler shift challenge in coherent LEO satellite-to-ground FSO links, offering a high-performance and practical solution.
...
### [102] [Fast Cascaded Recursive Filtering via a Block-Matrix Reformulation](https://arxiv.org/abs/2607.14054)
*Haotian Zhai,Bernd-Peter Paris*

Main category: eess.SP

TL;DR: This paper presents two parallel algorithms (partial LU factorization and cyclic reduction) for recursive IIR biquad filters by reformulating them as block-tridiagonal linear systems, achieving up to 10× speedup over scalar filtering and 8× throughput improvement over scipy.signal.sosfilt.


<details>
  <summary>Details</summary>
Motivation: Recursive (IIR) filters realized as cascaded biquads offer design generality and robustness against coefficient quantization, but their inherent sample-to-sample feedback dependency fundamentally obstructs parallel computation.

Method: The biquad difference equation is reformulated as a banded block-Toeplitz linear system. A stride-N permutation maps NL samples into a block-tridiagonal structure. Two parallel algorithms are developed: (1) partial LU (PH) factorization preserving sparse block structure, and (2) cyclic reduction applied to recursive filtering, reducing sequential dependency depth from O(N) to O(log₂ N).

Result: For a 16th-order system, the multi-block algorithms reduce clock cycles per sample by up to 10× compared to scalar filtering. On a single Meteor Lake core, cyclic reduction achieves approximately 618 MS/s — an 8× throughput improvement over scipy.signal.sosfilt.

Conclusion: The proposed parallel algorithms effectively overcome the sequential dependency bottleneck in recursive IIR biquad filtering, with both algorithms scaling favorably on newer architectures and requiring only a single permutation/de-permutation pair for the entire cascade.
...
### [103] [Automatic Differentiation from Scratch: How PyTorch Computes Gradients in Physics-Informed Neural Networks](https://arxiv.org/abs/2607.13042)
*Abdeladhim Tahimi*

Main category: cs.LG

TL;DR: This paper traces PyTorch's automatic differentiation engine computing gradients for PINN training, using a simple MLP and ODE example, verifying all adjoint values against hand derivations.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between theoretical sensitivity analysis and the actual numerical mechanics of PyTorch's autograd engine when computing two-level differentiation required for PINNs.

Method: Traces the complete computational graph of a 1-3-3-1 MLP solving y'(t)+y(t)=0, y(0)=1, showing forward pass graph, reverse-mode backward traversal computing 22 parameter gradients, and the create_graph=True mechanism enabling differentiation through physics residuals.

Result: All 22 parameter gradients are computed in a single backward pass, and every adjoint value is numerically verified against hand derivations from the P/Q sensitivity framework.

Conclusion: The paper establishes a concrete connection between the P/Q sensitivity framework and PyTorch's vector-Jacobian products, providing explicit numerical validation of automatic differentiation for PINN training.
...
### [104] [Beyond Backbone Backpropagation: A Decoupled Strategy for Efficient Transfer Learning](https://arxiv.org/abs/2607.13043)
*Daniel Vila-Cruz,Laura Morán-Fernández,Verónica Bolón-Canedo*

Main category: cs.LG

TL;DR: A lightweight training strategy that adapts normalization layers and decouples feature extraction from classifier optimization, reducing training time and CO2 emissions while maintaining accuracy across multiple architectures and medical datasets.


<details>
  <summary>Details</summary>
Motivation: Deep learning models face deployment challenges due to high computational costs and energy demands, making them impractical for resource-constrained clinical or prototyping environments.

Method: Proposes adapting normalization layers to the new domain, decoupling feature extraction from classifier optimization via precomputed features, and using a redesigned classifier head with margin-based weighted loss without end-to-end backpropagation.

Result: Evaluated on 7 architectures (CNN and Transformer) and 3 medical datasets; significantly reduces training time with marginal accuracy trade-off, often matching or surpassing baseline performance; reduces CO2 emissions by orders of magnitude.

Conclusion: Offers a practical and environmentally sustainable solution for resource-constrained clinical or prototyping environments.
...
### [105] [Federated Explainable Artificial Intelligence: Roles, Architectures, Evaluation, and Open Challenges](https://arxiv.org/abs/2607.13045)
*Masoume Gholizade,Fabrizio Ruffini,Pietro Ducange,Francesco Marcelloni*

Main category: cs.LG

TL;DR: A systematic survey of FedXAI showing how explainability is evolving from a post-hoc tool to an integral part of the federated learning lifecycle, covering taxonomy, methods, evaluation practices, and key challenges.


<details>
  <summary>Details</summary>
Motivation: FL addresses privacy via local data but fails to resolve model opacity; XAI improves transparency; their intersection (FedXAI) aims to jointly satisfy privacy and explainability requirements in distributed and heterogeneous settings.

Method: Systematic literature review with a newly proposed taxonomy classifying FedXAI methods by role of explainability, model/explainer types, explanation scope, integration level, FL settings, and data heterogeneity.

Result: Showcases how explainability supports aggregation, personalization, robustness, coordination, and system-level decisions; reviews model-agnostic to interpretable federated models and explainability-aware aggregation; identifies lack of standardized benchmarks/metrics for explanation quality, stability, privacy leakage, and computational overhead.

Conclusion: Identifies key challenges (non-IID explainability, security threats, communication-efficient XAI, continual FedXAI, domain knowledge/regulatory integration) and provides a reference framework for designing trustworthy, transparent, privacy-preserving federated AI systems.
...
### [106] [What Your Model Threw Away and Why You'll Want It Back: Masking, Fingerprinting, and Privacy from Discarded Geometry](https://arxiv.org/abs/2607.13046)
*Zachary P. Bradshaw*

Main category: cs.LG

TL;DR: A framework to quantify and compute the symmetry information discarded by machine learning models on Lie group-structured data, with efficient Newton-based computation and applications to molecular property prediction and spherical image classification.


<details>
  <summary>Details</summary>
Motivation: To understand what symmetries (Lie group actions) a learned model ignores, enabling analysis of model invariance, data masking, fingerprinting, and privacy preservation.

Method: Defines the null fiber (group elements invisible to f) and stabilizer (largest invariant subgroup), uses preimage theorem for dimension lower bounds, Peter-Weyl theorem for spectral characterization, and Newton iteration on the orbit map for efficient computation.

Result: Null fibers have dimension at least dim G - 1 at generic inputs; Fourier coefficient matrices characterize both objects for compact groups; Newton-based computation costs comparable to a few gradient evaluations; validated on SO(3) molecular property prediction and PSL(2,C) spherical image classification.

Conclusion: The framework unifies analysis of discarded symmetry across classical neural networks and variational quantum circuits, with practical applications to data masking, model fingerprinting, and privacy-preserving computation.
...
### [107] [Targeted Recovery of Weight-Space Mechanisms From Neural Networks](https://arxiv.org/abs/2607.13047)
*Antoine Vigouroux,Lee Sharkey*

Main category: cs.LG

TL;DR: Targeted Parameter Decomposition (tPD) efficiently identifies interpretable neural network components by focusing on specific inputs of interest, using a catch-all component for non-target data. It achieves mechanistic faithfulness at a fraction of the computational cost (e.g., 7% FLOPs) and enables surgical interventions like ablation and rewiring with minimal side effects.


<details>
  <summary>Details</summary>
Motivation: Scaling Parameter Decomposition (PD) to large models is computationally expensive and risky. The authors aim to make neural network interpretability more efficient by focusing decomposition on specific inputs or subtasks rather than the entire model.

Method: Introduces a high-rank catch-all component that handles all non-target data, allowing tPD to isolate only the components processing specific inputs of interest (prompts, subtasks).

Result: Validated on toy models and transformer LMs trained on The Pile. Extracted a CSS-only submodel from a 4-block transformer using only 7% of FLOPs vs. full PD. Successfully ablated and rewired memorized sequences in a 12-block transformer with minimal side effects on other inputs.

Conclusion: tPD enables scalable, faithful mechanistic interpretability with dramatically reduced computational cost, facilitating precise surgical interventions in large language models.
...
### [108] [Uncertainty-Aware Sequential Decision Rules for Event-Triggered LLM Invocation in Streaming Systems](https://arxiv.org/abs/2607.13048)
*Zhaohui Wang*

Main category: cs.LG

TL;DR: This paper formalizes the question of when to invoke a Large Language Model in streaming inference pipelines as a risk-based sequential stopping problem, proving theoretical guarantees (sublinear regret, threshold optimality) and empirically validating on turbofan degradation data with real LLM calls.


<details>
  <summary>Details</summary>
Motivation: Streaming pipelines pair lightweight models with expensive LLMs, but the decision of when to call the LLM lacks formal treatment. The authors aim to provide a principled, risk-based framework for this trigger policy problem.

Method: The problem is cast as a risk-based sequential stopping problem where a trigger fires when a risk functional exceeds a threshold. Six theoretical results are proven, including regret bounds and convergence rates. The framework unifies classical trigger families (event-triggered, SPRT, CUSUM, Bayesian). Empirical validation uses CMAPSS turbofan data with real LLM calls.

Result: Sublinear regret (α < 1) for all principled triggers; 92.9% of 1600 LLM diagnoses achieve grounding score ≥ 0.75; anomaly-score-driven risk functions dominate alternatives by roughly an order of magnitude on Pareto AUC.

Conclusion: The proposed risk-based framework provides a theoretically grounded and empirically effective approach for LLM invocation in streaming pipelines, outperforming baselines including RouteLLM-style routers and contextual bandits.
...
### [109] [TSSM: Triaxial State Space Model for Global Station Weather Forecasting with Temporal-Variable-Historical Modeling](https://arxiv.org/abs/2607.13101)
*Songru Yang,Zili Liu,Tao Han,Ben Fei,Fenghua Ling,Lei Bai,Chang Liu,Xiangyang Ji,Zhenwei Shi,Zhengxia Zou*

Main category: cs.LG

TL;DR: TSSM is a Triaxial State Space Model for global station weather forecasting that uses period-aligned historical data to overcome limitations of short-term look-back windows, achieving SOTA results with 10% accuracy and 61% extreme event gains on the Weather-5K dataset, while maintaining robust performance under heavy missing data.


<details>
  <summary>Details</summary>
Motivation: Existing GSWF methods over-rely on short-term temporal patterns, which fail to capture chaotic weather dynamics and handle extreme events or partial observations, leading to limited accuracy and error accumulation.

Method: TSSM proposes a history-enhanced Temporal-Variable-Historical paradigm that stacks historical samples into period-aligned batches, and designs triaxial scanning (temporal, variable, historical) to capture dependencies across axes, hierarchically shared to model from seasonal to extreme events while mitigating misalignment.

Result: Achieves SOTA on Weather-5K (10% accuracy gain, 61% extreme event gain); 95% best/second-best on human-involved datasets; 37.5% gain at 240h; up to 103.5% gain under iterative (48h×5) setting; retains >90% performance under 80% missing observations vs. <43% for baselines.

Conclusion: TSSM demonstrates that incorporating period-aligned historical patterns significantly improves both accuracy and robustness in global station weather forecasting, with strong practical potential for real-world in-situ observation networks.
...
### [110] [Disentangling Knowledge States with Ability and Proficiency Modeling for Knowledge Tracing](https://arxiv.org/abs/2607.13103)
*Duantengchuan Li,Yingqian Bi,Jinsong Chen,Rui Zhang,Mingwen Tong*

Main category: cs.LG

TL;DR: PAKT decomposes student learning into ability-building and proficiency phases using a multi-branch Transformer, achieving consistent AUC improvements of ~0.82% over baselines.


<details>
  <summary>Details</summary>
Motivation: Existing KT methods treat all learning behaviors uniformly, ignoring phase-specific differences (ability-building vs. proficiency-oriented). The authors observed that students tend to correctly answer previously failed concepts after sufficient practice, motivating a phase-aware approach.

Method: PAKT decomposes student interaction sequences into ability and proficiency phases via a tailored decomposition mechanism. It uses a multi-branch Transformer with a type-aware readout module to jointly capture phase-specific and holistic knowledge states. A causal analysis addresses confounding bias from entangled learning behaviors.

Result: Outperforms baselines on six public benchmarks with max AUC gain of 1.33% and average gain of 0.82%.

Conclusion: Modeling phase-aware learning dynamics improves knowledge tracing performance, and ignoring phase distinctions introduces confounding bias in predictions.
...
### [111] [STKAN: Kolmogorov-Arnold Networks for Spatio-Temporal Forecasting](https://arxiv.org/abs/2607.13108)
*Sicong Lai,Yuehong Hu,Siru Zhong,Si Qiao,Yuxuan Liang,Guangyin Jin*

Main category: cs.LG

TL;DR: STKAN introduces Taylor-polynomial KAN modules for spatial and temporal mixing in spatio-temporal forecasting, achieving competitive results on traffic benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing spatio-temporal forecasting methods focus on complex graph, attention, and decomposition architectures but overlook the impact of the underlying nonlinear function approximator on performance.

Method: STKAN uses learnable soft node-group assignment for spatial representations, group-wise spatial mixing via Taylor-polynomial KAN, temporal modeling over compressed sequences, and self-attention for long-range interactions.

Result: STKAN achieves competitive performance on five traffic forecasting benchmarks and outperforms the MLP-based variant in tested settings.

Conclusion: Designing better nonlinear function approximators can be a useful complement to architectural innovation in spatio-temporal forecasting.
...
### [112] [A Hybrid Mamba for Audio-Visual Navigation](https://arxiv.org/abs/2607.13110)
*Yi Wang,Yinfeng Yu*

Main category: cs.LG

TL;DR: Samba replaces traditional GRU and CNN backbones in audio-visual navigation with Mamba-based encoders (M-SE and AME), achieving 11.3% higher success rate on Matterport3D and stronger generalization with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: CNN/RNN-based navigation backbones have remained unchanged since 2020 and cannot efficiently represent dynamic multimodal sequences.

Method: Replaces GRUs with adaptive Mamba State Encoder (M-SE) for temporal aggregation, and uses Audio Mamba Encoder (AME) to capture global time-frequency dependencies in spectrograms, overcoming CNN limitations.

Result: Improves navigation success rate by 11.3% on Matterport3D; performs even better on Replica with finer scene structures; shows strong generalization to unseen sounds and scenes.

Conclusion: Modernized Mamba-based architectural reconstruction unlocks stronger embodied representation at lower cost, providing a robust pathway for audio-visual navigation paradigm evolution.
...
### [113] [CoDiffGRN: Rethinking Gene Regulatory Network Inference via the BEELINE-KGC Benchmark and Co-evolutionary Discrete Diffusion](https://arxiv.org/abs/2607.13120)
*Jiaze Song,Runhao Zhao,Minghao Xu,Bin Cui,Wentao Zhang*

Main category: cs.LG

TL;DR: The paper reformulates gene regulatory network inference as an inductive ranking problem and proposes a co-evolutionary discrete diffusion model that outperforms existing methods in discovering novel regulatory interactions from single-cell data.


<details>
  <summary>Details</summary>
Motivation: Existing GRN inference methods misalign with real-world needs: researchers want high-confidence novel regulatory interactions, but current benchmarks use transductive splits and global metrics that fail to measure generalization to unseen genes.

Method: The authors introduce a new benchmark with inductive gene-holdout splits and knowledge graph completion metrics. They propose a co-evolutionary discrete diffusion framework that jointly models discretized gene expression states and regulatory interactions, along with TF-ALL Subgraph Sampling for scalability.

Result: The proposed method achieves state-of-the-art performance on the new benchmark, significantly outperforming existing methods in novel regulatory discovery, with ablation studies confirming the effectiveness of each design component.

Conclusion: Reformulating GRN inference as an inductive ranking problem and using co-evolutionary diffusion modeling enables more biologically meaningful and practically useful regulatory network discovery.
...
### [114] [A VAE-Driven Multi-Task Satellite-Aided Semantic Communication Framework for 6G-Enabled Connected Autonomous Vehicles](https://arxiv.org/abs/2607.13494)
*S. M. Abtahiul Alam,Niloy Das,Apurba Adhikary,Yu Qiao,Zhu Han,Choong Seon Hong*

Main category: cs.LG

TL;DR: This paper proposes a VAE-based multi-task semantic communication framework for satellite-assisted autonomous driving, achieving 87-98% bandwidth reduction while maintaining stable traffic sign reconstruction and classification performance across varying SNR conditions.


<details>
  <summary>Details</summary>
Motivation: Conventional communication systems transmit raw data regardless of task relevance, which is inefficient in resource-constrained satellite channels with scarce uplink bandwidth and large propagation losses. Semantic communication addresses this by transmitting only task-relevant information.

Method: The authors present a Variational Autoencoder (VAE)-based multi-task semantic communication framework that uses probabilistic latent representations (unlike deterministic autoencoders) for robust and efficient encoding. The framework is trained end-to-end to jointly optimize traffic sign reconstruction and classification tasks over noisy wireless channels.

Result: The proposed approach achieves significant bandwidth reduction of 87.23% to 98.17% while maintaining stable performance across varying signal-to-noise ratio conditions.

Conclusion: The VAE-based semantic communication framework effectively balances bandwidth efficiency and task performance for satellite-assisted autonomous driving, demonstrating robustness under noisy channel conditions.
...
### [115] [ShortOPD: Recovering Pruned LLMs with Short-to-Long On-Policy Distillation](https://arxiv.org/abs/2607.13124)
*Qingyu Zhang,Qianhao Yuan,Hongyu Lin,Yaojie Lu,Xianpei Han,Le Sun,Xiang Li,Ming Xu,Jiarui Li,Xiuyin Zhao*

Main category: cs.LG

TL;DR: Proposes Short-to-Long On-Policy Distillation (ShortOPD) to recover structured-pruned LLMs for free-form generation, achieving 9× score improvement over unrecovered models and 1.6-4.4× over standard methods, while using 71% fewer tokens and 76% less training time.


<details>
  <summary>Details</summary>
Motivation: Structured pruning improves perplexity/accuracy on multiple-choice tasks but causes collapse on free-form generation (greedy pass@1 nearly vanishes). The gap exists because useful tokens are demoted (not erased) and recovery fails mainly due to suffix repetition.

Method: ShortOPD: a short-to-long on-policy distillation schedule. It detects teacher-confirmed repetitive suffixes, treats the surviving prefix as effective rollout length, and allocates future budgets to effective lengths the policy can use. Uses frozen pre-compression model as teacher for dense token-level supervision.

Result: 9× score improvement over unrecovered models, 1.6-4.4× over SFT/KD/SeqKD baselines. Matches fixed 8192-token horizon within 2 points using 71% fewer rollout tokens and 8.5h vs 35.9h training time.

Conclusion: Short-to-long OPD schedule is an effective recovery recipe for structured-pruned LLMs, bridging the gap between perplexity/benchmark gains and real deployment-ready generation quality.
...
### [116] [HEDGEHOG: Hierarchical Evaluation of Drug Generators Through Rigorous Filtration](https://arxiv.org/abs/2607.13155)
...
TL;DR: Only 0.65% of molecules generated by 23 molecular models survive HEDGEHOG, a comprehensive six-stage medicinal chemistry benchmark, revealing that current generators rarely produce compounds satisfying real-world drug discovery constraints.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics for molecular generators fail to reflect medicinal plausibility and downstream suitability, leading to false positives and wasted computational resources in drug discovery.

Method: HEDGEHOG is a six-stage filtration benchmark inspired by industrial hit identification: (i) preprocessing; (ii) physicochemical screening; (iii) structural alerts/graph-sanity; (iv) synthesis feasibility; (v) docking/binding affinity; (vi) 3D pose/interaction checks. 23 generators across 3 model classes were evaluated on 230,000 molecules.

Result: Only 0.65% of initial molecules pass all six stages, indicating a severe bottleneck in generating medicinally plausible compounds that satisfy real-world drug discovery constraints simultaneously.

Conclusion: Current molecular generators have a fundamental limitation: molecules acceptable under isolated criteria rarely satisfy combined medicinal chemistry, synthesis, docking, and 3D pose filters, highlighting the need for more constrained optimization approaches.
...
### [117] [SteinGate: Tail-Sensitive Safe Reinforcement Learning via Stein Discrepancy](https://arxiv.org/abs/2607.13175)
*Yassine Chemingui,Chenhua Fan,Honghao Wei,Janardhan Rao Doppa*

Main category: cs.LG

TL;DR: SteinGate introduces a distributional safety certificate using Kernelized Stein Discrepancy to detect catastrophic tail events in safe RL, dynamically switching between reward-seeking and recovery behaviors based on cost distribution consistency.


<details>
  <summary>Details</summary>
Motivation: Standard safe RL methods using expected cumulative costs fail to detect rare but catastrophic tail events, leading to unsafe decisions in safety-critical applications.

Method: A non-parametric safety certificate based on Kernelized Stein Discrepancy that checks consistency between observed rollout costs and a safe reference distribution, accounting for boundary atoms from clipped costs, then adapts the learning regime accordingly.

Result: On continuous-control benchmarks, SteinGate significantly reduces both the frequency and severity of constraint violations during training while maintaining competitive returns compared to state-of-the-art baselines.

Conclusion: Replacing fragile tail fitting with robust distributional consistency checks offers a more effective approach to detecting and preventing catastrophic safety violations in reinforcement learning.
...
### [118] [Concurrent Image Understanding and Generation: Self-Correcting Coupled Markov Jump Processes](https://arxiv.org/abs/2607.13188)
...
TL;DR: This paper introduces SC-CMJP and CO₂Jump, a training-free sampler for joint text-image generation using Masked Diffusion Models, where cross-modal coupling enables self-correction of contradictions during the denoising process. Three new large-scale datasets (JEdit-1M, JMaze-200K, JNono-200K) are released.


<details>
  <summary>Details</summary>
Motivation: Human cognition integrates understanding and generation across modalities (e.g., speaking and drawing together). Existing multimodal diffusion samplers fail to let modalities influence each other's decisions within the same denoising step, leading to unresolved cross-modal contradictions.

Method: Proposes SC-CMJP (Self-Correcting Coupled Markov Jump Processes), where transition rates of one modality are functionals of the other modality's confidence scores weighted by cross-modal attention. A remasking jump retracts commitments when cross-modal evidence conflicts. CO₂Jump implements this as a training-free single-pass sampler.

Result: CO₂Jump achieves best joint performance on image understanding, editing, and visual reasoning (maze and nonogram solving). Performance scales monotonically with number of denoising steps.

Conclusion: Cross-modal coupling benefits compound across the denoising trajectory, demonstrating that tightly coupled multimodal generation outperforms interleaved or parallel decoding approaches.
...
### [119] [EMAGN: Efficient Multi-Attention Graph Network via Learned Clustering for Scalable Traffic Forecasting](https://arxiv.org/abs/2607.13241)
*Mingxing Xu,Rakesh Chowdary Machineni,Ke Liu,Xi Cheng,Chengqi Lu,Xin Hu,Lyuhao Chen,Xiangyu Li,Junwei You,Oliver Gao*

Main category: cs.LG

TL;DR: EMAGN linearizes spatial attention via learned clustering matrices to reduce complexity from O(N²d) to O(NMd), achieving near-full-attention accuracy with 32-58% resource savings and enabling operation where full-attention fails on limited GPUs.


<details>
  <summary>Details</summary>
Motivation: Self-attention mechanisms achieve state-of-the-art in traffic forecasting but suffer from quadratic O(N²d) complexity, limiting scalability on standard hardware.

Method: Proposes EMAGN with two learned clustering matrices C_k and C_v that group key/value vectors into M super-clusters, inspired by fast high-dimensional Gaussian filtering, reducing complexity to O(NMd).

Result: MAE within 2.7-3.2% of full-attention GMAN; training time -32%, inference time -38%, GPU memory -58%; operates where full-attention runs out of memory on 11GB GPU; outperforms Linformer and Performer.

Conclusion: Adaptive clustering via traffic-network-aware design enables efficient and accurate attention, expanding feasible model configurations for traffic forecasting.
...
### [120] [Reassessing Muon for Matrix Factorization](https://arxiv.org/abs/2607.13246)
*Ali Parviz,Gal Mishne,Alex Cloninger*

Main category: cs.LG

TL;DR: The paper evaluates Muon optimizer on low-rank matrix factorization and finds it doesn't consistently outperform AdamW, questioning previously reported advantages.


<details>
  <summary>Details</summary>
Motivation: To isolate Muon's true advantages from confounding factors like scale, architecture, and data by studying it on a simple, controlled problem.

Method: Controlled comparison of Muon against tuned adaptive baselines (e.g., AdamW) on low-rank matrix factorization, a simple spectrally structured problem.
...
Conclusion: Spectrum-aware orthogonalization (Muon) is not universally beneficial; modern optimizers should be evaluated on controlled problems alongside end-to-end benchmarks.
...
### [121] [Deconstructing Actor-Critic: A Large-scale Empirical Study of Design Components for Practitioners](https://arxiv.org/abs/2607.13274)
*Haseeb Shah,Lingwei Zhu,Adam White,Martha White*

Main category: cs.LG

TL;DR: This paper analyzes over 33,000 experiments on actor-critic reinforcement learning algorithms applied to a water treatment control task, finding that common defaults like Gaussian action distributions and pathwise gradient estimators are unreliable, while bounded distributions with adaptive update schedules are more robust.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning is being applied to real-world systems (fusion, vehicles, drug discovery, water treatment) where reliability is essential and tuning budgets are limited, yet the impact of actor-critic algorithm design decisions on variability and hyperparameter sensitivity is poorly understood.

Method: The authors conduct over 33,000 experiments on a control task derived from a real water treatment plant, systematically analyzing actor-critic components: policy update method, action distribution representation, gradient estimation, and update frequency relative to value estimation.

Result: Common defaults (Gaussian action distributions with pathwise gradient estimators) are among the least reliable configurations. Bounded distributions with adaptive update schedules remain robust across a wide range of settings.

Conclusion: The findings provide empirical guidance for practitioners making component-level decisions when adapting actor-critic methods to new real-world control settings across scientific and engineering domains.
...
### [122] [Tabular Foundation Models for Discrete Choice Estimation](https://arxiv.org/abs/2607.13314)
*Liu Liu,Dan Zhang*

Main category: cs.LG

TL;DR: Tabular foundation models (TFMs) struggle with discrete choice tasks due to row-independence assumptions conflicting with choice-set dependence and consumer preference heterogeneity, but a novel reformulation encoding these structures achieves 8% better log-likelihood and 3.6% higher hit rate than hierarchical Bayesian methods, with 16x speed improvement.


<details>
  <summary>Details</summary>
Motivation: To determine whether tabular foundation models (originally designed for row-independent data) can be effectively applied to discrete choice demand estimation, a core problem in marketing and operations that involves set-valued choices and persistent consumer heterogeneity.

Method: The authors propose a reformulation that encodes choice-set dependence and individual-level heterogeneity within a row-based learning framework. They evaluate on a yogurt scanner panel dataset, comparing against hierarchical Bayesian estimation.

Result: Individual-level heterogeneity encoding is the dominant predictor of accuracy. Best reformulation outperforms hierarchical Bayes by 8% in holdout log-likelihood and 3.6% in hit rate, with 16x faster runtime. Largest gains occur in medium-data regime (10-40 purchase occasions). Fine-tuning benefits consumers with shallow purchase histories.

Conclusion: This establishes a principled approach for applying foundation models to consumer choice problems, with practical advantages for large-scale demand estimation, especially where parametric Bayesian shrinkage distorts estimates for atypical consumers.
...
### [123] [Accuracy-Preserving Stability Regularization for Large-Scale Retail Demand Forecasting](https://arxiv.org/abs/2607.13331)
*Jize Li,Jiani He,Dishu Yang,Dingyan Shang,Jingjing Liu,Shiqi Huang*

Main category: cs.LG

TL;DR: This paper proposes a training-time penalty to stabilize retail demand forecasts across adjacent time steps, improving stability by ~7% over XGBoost with minimal RMSE loss (≤0.72%), outperforming post-hoc smoothing in accuracy-stability trade-off.


<details>
  <summary>Details</summary>
Motivation: Retail demand forecasts are reused across multiple planning cycles (replenishment, capacity, labor, transportation), but standard point-error objectives (e.g., RMSE) allow abrupt movements between consecutive forecasts, causing operational instability. Post-hoc smoothing fixes this only after model fitting, not during training.

Method: A temporal-structured pipeline with recent-demand embeddings and calendar/price/hierarchy/item/store features, plus a training-time penalty on consecutive within-series movement (stability regularization), evaluated on M5 demand series at 1k, 3k, and 4k scales.

Result: The stability-aware hybrid model improves Forecast Stability Score over XGBoost by 6.91%, 6.66%, and 7.68% at 1000, 3000, and 4000-series scales, while RMSE changes stay within 0.72% across three random seeds. Post-hoc smoothing reduces movement more but at higher RMSE cost.

Conclusion: Training-time regularization preserves point accuracy better than post-hoc smoothing, advocating for an accuracy-stability trade-off perspective in operational retail forecasting rather than pure point-error minimization.
...
### [124] [Structured Reinforcement Learning for Bayesian Persuasion : Application to Intelligent Interactive Driving](https://arxiv.org/abs/2607.13576)
*Merlin Paul,Anup Aprem*

Main category: cs.LG

TL;DR: This paper proposes a structured reinforcement learning framework (SQP/MAPL) for designing computationally efficient signaling strategies in interactive driving scenarios, using Bayesian persuasion to guide connected vehicles' route choices, achieving 30% cost efficiency improvement over existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of harmonizing decisions in interactive driving, where a lead vehicle coordinates connected vehicles' route choices via real-time traffic data, while handling the computational complexity of designing signaling strategies for farsighted agents.

Method: The authors propose: (i) MAPL - a structured policy learning algorithm for monotonic agents with approximate best response; (ii) identification of sufficient conditions for supermodular Q-function structure; (iii) sufficient conditions for persuasiveness of signaling strategies; (iv) Supermodular Q learning for Principal (SQP) that leverages this structure for efficient signaling strategy synthesis.

Result: Numerical analysis on Bayesian persuasive driving for lane selection demonstrates 30% cost efficiency improvement in optimizing traveling rewards for both lead and connected vehicles compared to existing signaling strategy design methods.

Conclusion: The proposed structured RL framework enables computationally efficient and persuasive signaling strategies for interactive driving with farsighted agents, offering a promising approach for dynamic traffic management.
...
### [125] [Agora: Collective and Permissionless Internet-Scale Pretraining of Large Language Models](https://arxiv.org/abs/2607.13332)
...
TL;DR: Agora enables distributed training of large language models (up to 8.6B parameters) using heterogeneous, preemptible consumer GPUs connected via the internet, achieving 63% efficiency of centralized H100 training through Protocol Learning—a paradigm where no single party holds the full model weights.


<details>
  <summary>Details</summary>
Motivation: Frontier model training is confined to datacenters with homogeneous accelerators and high-speed interconnects, concentrating development among few groups. A vast pool of heterogeneous, individually owned consumer/professional GPUs remains unusable for training due to lack of suitable systems.

Method: Agora combines bandwidth-efficient pipeline-parallel model sharding over internet links with multi-party, fault-tolerant collective operations. Each participant holds only one model stage; no single party possesses full weights ("Protocol Learning"). Supports asynchronous optimization and handles node preemption/failure.

Result: Pluralis-8B: 8.6B-parameter model trained on 500B tokens of FineWeb-Edu over 40 days by 330 contributor nodes (mostly consumer GPUs on internet connections). Sustained ~170k tokens/s, 4.2 tokens per TFLOP (63% of centralized H100 efficiency), converged close to centralized reference run.

Conclusion: Protocol Learning enables collectively trained, collectively owned models, opening a path to open-source frontier training with economic sustainability. This is the first demonstration of open, permissionless pretraining at this scale using distributed consumer hardware.
...
### [126] [Weight Feedback Computes the Jacobian Transpose Locally in Modern Deep Networks](https://arxiv.org/abs/2607.13380)
*Junlong Shen,Xingyu Li*

Main category: cs.LG

TL;DR: This paper introduces WF-Act-PC, a Predictive Coding variant that eliminates the non-local autograd Jacobian-transpose operation by decomposing it into three locally available terms for layers with frozen normalization statistics, achieving improved accuracy and surpassing both classical PC and tuned backpropagation baselines on deeper architectures.


<details>
  <summary>Details</summary>
Motivation: Predictive Coding is biologically motivated but still relies on the non-local autograd Jacobian-transpose product for error routing. The authors aim to eliminate this last non-local operation to achieve fully local weight updates in PC.

Method: The authors show that for layers with frozen normalization statistics, the exact J^T product factors into three locally available terms: L^T (weight feedback, assuming weight symmetry), activation derivative σ', and normalization gain s. They substitute this identity into PC to create WF-Act-PC, which removes the autograd backward pass from error transport.

Result: On CIFAR-10/100 (50 epochs, 5 seeds), WF-Act-PC is the only PC method whose accuracy improves with depth, surpassing iPC by 2.7-22.3 pp on CIFAR-10. It matches or exceeds tuned backprop on deeper CIFAR-10 architectures (VGG-9: 93.57% vs 92.43%; ResNet-18: 92.76% vs 91.54%) and on Tiny-ImageNet.

Conclusion: The dependency on autograd J^T is largely avoidable in PC by decomposing it into locally available terms. WF-Act-PC improves accuracy with depth and closes the performance gap with backpropagation on several benchmarks, while maintaining biological plausibility.
...
### [127] [Gauge-Invariant, Parameter-Insensitive Regularization for Potential Recovery from Flow on Directed Graphs](https://arxiv.org/abs/2607.13609)
*Mohammad Forouhesh*

Main category: cs.LG

TL;DR: Recovering latent potential on directed graphs is ill-posed; standard ridge regularization collapses ordering. A gauge-invariant graph Dirichlet energy fix delivers stable, parameter-insensitive estimates across four orders of magnitude, preserving dynamic range where ridge fails. This gauge invariance also prevents oversmoothing in deep directed GCNs.


<details>
  <summary>Details</summary>
Motivation: Standard ridge regularization for recovering latent potential on directed graphs shrinks estimates toward a gauge-meaningless origin, causing rank correlation to invert (e.g., +0.81 → -0.42). The authors aim to fix this ill-posed inverse problem.

Method: Use gauge-invariant graph Dirichlet energy (H¹ seminorm) to regularize the discrete Poisson problem. Prove the reduced system is SPD, preserves dynamic range, and localizes absorbing boundaries via Poisson residual. Ablation tests robustness across extraction methods.

Result: Gauge-invariant estimate retains 28–41% of interior dynamic range on three clickstream corpora, while ridge collapses to as little as 0.2%. Estimate is stable across four orders of magnitude in λ, whereas ridge inverts ordering for every λ > 0.

Conclusion: Gauge invariance solves the ill-posedness and parameter-sensitivity of the directed graph potential recovery problem. The same principle extends to graph neural networks, neutralizing the constant mode per layer to prevent oversmoothing in deep directed GCNs, linking classical inverse problems to graph learning.
...
### [128] [Where Should RL Post-Training Compute Go? Model Size, Search, Learning, and Feedback](https://arxiv.org/abs/2607.13389)
*Patrick Wilhelm,Odej Kao*

Main category: cs.LG

TL;DR: This paper studies how to optimally allocate a fixed FLOP budget during RL post-training of foundation models, finding that the best compute split among model size, rollout/search, learning updates, and reward feedback varies with budget, model scale, and reward system. The authors propose RACE, a diagnostic protocol for identifying allocation regimes, and argue for transparent FLOP reporting in RL post-training papers.


<details>
  <summary>Details</summary>
Motivation: Current RL post-training practice condenses compute constraints into a single FLOP budget, but it's unclear how to optimally divide that budget across policy size, training length, rollout quantity, and reward feedback computation. The authors aim to systematically study this fixed-budget allocation problem.

Method: The authors introduce a FLOP-accounting framework for GRPO post-training that decomposes compute into rollout/search, policy-update/learning, and reward-feedback evaluation. They conduct experiments across LoRA-adapted Qwen2.5 policies of varying sizes, varying compute budgets, reward systems (rule-based vs. PRM-style), and evaluation targets, tracing conditional allocation frontiers.

Result: The optimal compute allocation is conditional on model size, budget, reward system, and evaluation target. Larger models consume more per-token compute per update, limiting the number of updates/rollouts under a fixed budget. Rule-based rewards spend compute almost entirely on rollouts, while PRM-style feedback devotes a visible fraction to reward-model inference. Model choice and training allocation are coupled—they cannot be optimized independently.

Conclusion: The authors propose RACE as a diagnostic pilot-grid protocol to identify promising allocation regimes before expensive validation runs. They recommend that RL post-training papers report total FLOPs alongside a breakdown of compute across model size, search, learning, and feedback.
...
### [129] [PiVoT: A Variational Solution for Real-time Large-scale Multi-object Detection and Tracking under Heavy Clutter](https://arxiv.org/abs/2607.13891)
*Runze Gan,Qing Li,Simon J. Godsill,Mike E. Davies,James R. Hopgood*

Main category: cs.LG

TL;DR: PiVoT is a fast, training-free Bayesian multi-object tracker for point cloud data that handles severe clutter, large object populations, and Doppler measurements through variational inference innovations, achieving performance comparable to deep learning methods on automotive radar datasets.


<details>
  <summary>Details</summary>
Motivation: Current Bayesian trackers based on Poisson measurement models struggle with accuracy and efficiency under severe clutter, large object populations, and full-resolution Doppler point clouds in data-scarce radar applications.

Method: PiVoT performs end-to-end detection and tracking via joint inference of object states, shapes, existence probabilities, data association, and measurement rates. It introduces variational inference innovations including theoretically justified birth pruning, quadratic-to-linear complexity reduction, and a computationally efficient Doppler Poisson model.

Result: PiVoT substantially outperforms existing Bayesian trackers in challenging scenes, demonstrates exceptional scalability to a thousand objects, robustness to clutter visually inseparable from objects, and real-time operation on full-scale automotive radar datasets, achieving performance comparable to a deep-learning detection benchmark.

Conclusion: PiVoT provides a training-free, clutter-resilient, and scalable solution for multi-object tracking from noisy point clouds, bridging the gap between Bayesian and deep learning approaches in radar applications.
...
### [130] [Self-Improving is Often Sudden: Enlightenment-style Finetuning for Large-Scale Models](https://arxiv.org/abs/2607.13395)
*Jing-Xiao Liao,Tianwei Zhang,Yu-Hao Jiang,Feifei Zhang,Hang-Cheng Dong,Feng-Lei Fan*

Main category: cs.LG

TL;DR: Enlightenment is a training-free post-tuning paradigm that unlocks latent capabilities in large models by modifying shortcuts in key modules/layers, with specific instantiations for LLMs (attention head-mixing) and VLMs (scalar-modulated residual connections), achieving performance gains without weight updates.


<details>
  <summary>Details</summary>
Motivation: Inspired by the human "aha moment" (enlightenment), the authors hypothesize that large foundation models have latent capacities for sudden capability boosts. They aim to unlock these capacities without costly retraining or fine-tuning.

Method: A training-free post-tuning paradigm called Enlightenment: (1) For LLMs - attention head-mixing shortcuts connecting the initial attention head's output to all other target heads with adaptive scaling factor initialization. (2) For VLMs - lightweight scalar-modulated factors applied to residual connections in decoder layers to regulate information flow.

Result: Remarkable performance improvements across diverse benchmarks and models, outperforming existing training-free approaches that primarily manipulate attention weights.

Conclusion: Enlightenment efficiently unlocks latent potential in pre-trained networks without weight updates or training, suggesting that significant capability gains can be achieved through architectural shortcut modifications alone.
...
### [131] [Is the Statistical Advantage Worth the Cost? An Empirical Comparison of KANs and MLPs for Structured Data Classification](https://arxiv.org/abs/2607.13413)
...
TL;DR: This paper compares KANs vs MLPs on tabular classification tasks, finding KANs statistically outperform MLPs but with higher computational cost and medium effect size (d=-0.46).


<details>
  <summary>Details</summary>
Motivation: Growing interest in KANs as an alternative function-approximating architecture, but lack of systematic benchmarking on structured tabular data.

Method: Empirical benchmarking on 12 datasets (binary, multiclass, multilabel, ordinal) using standardized preprocessing, fixed hyperparameters, evaluated via test accuracy, F1-score, paired hypothesis testing, and effect size analysis.

Result: KANs statistically outperform MLPs in binary/multiclass tasks with aggregate advantage, but with medium effect size (d=-0.46) and substantially higher parameter/computational complexity.

Conclusion: KANs preferred for high-precision applications; MLPs remain robust for resource-constrained environments. Future work should explore additional data modalities.
...
### [132] [EXPLORE: Exploration with Guided Search for Analog Topology Generation using Language Models](https://arxiv.org/abs/2607.13416)
*Guanglei Zhou,Chen-Chia Chang,Yikang Shen,Jonathan Ku,Isaac Jacobson,Jingyu Pan,Yiran Chen,Xin Zhang*

Main category: cs.LG

TL;DR: EXPLORE integrates Monte Carlo Tree Search with transformer decoding to improve analog circuit topology generation, boosting success rates from 12-33% to 65% on a complex benchmark.


<details>
  <summary>Details</summary>
Motivation: Automating analog circuit topology design is needed to reduce manual effort for diverse applications, but existing one-shot generation methods fail on complex circuits due to large search spaces and limited training data.

Method: EXPLORE combines simulator-guided Monte Carlo Tree Search (MCTS) with transformer-based decoding, using language-model priors and bypassing high-confidence structural tokens to efficiently allocate simulator budget toward topology-altering decisions.

Result: On a 6-component benchmark at 0.01 tolerance, EXPLORE achieves 65% success rate vs 12% (one-shot) and 33% (sampling-and-filter), and lowers MSE by over 20% under the same search budget.

Conclusion: EXPLORE is the first framework integrating structured test-time search with LM decoding for analog topology generation, representing a practical step toward scaling LLM-driven design automation.
...
### [133] [OrDA: Orthogonal Disentanglement of Access Habits Framework for Homepage Marketing Block Recommendations](https://arxiv.org/abs/2607.13420)
*Lingxiao Zhang,Xiaobo Li,Tao Xu*

Main category: cs.LG

TL;DR: OrDA is a framework that disentangles genuine content interest from habitual access clicks in homepage marketing blocks using a dual-tower structure with orthogonal regularization and causal intervention, achieving 5.64% UCTR improvement on Zhima's homepage.


<details>
  <summary>Details</summary>
Motivation: Clicks on marketing blocks are driven by both content interest and access habits, where habitual clicks create Pseudo-Positives that mask mediocre content quality, leading to biased recommendation ecosystems.

Method: OrDA uses a dual-tower structure with gated allocation layer for adaptive feature routing, orthogonal regularization to geometrically separate interest and habit manifolds, and causal intervention (do-calculus) during inference to rank items by purified interest scores.

Result: Outperforms state-of-the-art methods in predictive accuracy on large-scale datasets; online AB test shows 5.64% user click-through rates (UCTR) improvement on Zhima homepage marketing block and Zhima rent-floor recommendation.

Conclusion: OrDA effectively eliminates access-habit bias and improves recommendation quality by purifying interest signals from habitual noise.
...
### [134] [Temperature Scaling Is Not Enough: Calibration Gaps Under Human Label Distributions](https://arxiv.org/abs/2607.13423)
*Wisdom Dogah*

Main category: cs.LG

TL;DR: Temperature scaling, the dominant post-hoc calibration method, fails to calibrate models properly when labels are soft (distributional) rather than one-hot deterministic, creating a "soft-label calibration gap" that grows with model scale in vision and is larger in language than vision tasks.


<details>
  <summary>Details</summary>
Motivation: To study whether temperature scaling retains calibration properties when ground-truth labels are soft/distributional (reflecting genuine human disagreement) rather than one-hot deterministic, and whether degradation depends on model scale.

Method: Evaluated temperature scaling on CIFAR-10H and ChaosNLI datasets with both hard one-hot and soft distributional labels across three model scales per modality (vision and language), measuring Brier Score gaps between hard-label-calibrated and oracle soft-label-calibrated models.

Result: Found positive soft-label calibration gaps in all 9 configurations (0.002-0.134 Brier Score), gaps grow monotonically with model scale in vision and SNLI-split, larger gaps in language (mean 0.079) than vision (mean 0.003), with one inconclusive reversal on MNLI-split. Multiclass isotonic regression confirms same pattern.

Conclusion: Calibration protocols using majority-vote labels systematically misstate model reliability when label ambiguity is structural, with direct implications for safety-critical deployment.
...
### [135] [Data-Efficient Adaptation of LLMs via Attention Head Reweighting](https://arxiv.org/abs/2607.13425)
*Tuomas Oikarinen,Zixiao Chen,Charlotte Siska,Tsui-Wei Weng,Chandan Singh,Jianfeng Gao*

Main category: cs.LG

TL;DR: AHR adapts LLMs to text classification tasks by learning only one scalar per attention head (~0.0001% of parameters), outperforming LoRA in few-shot settings despite having 200-1000x fewer trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Learning from limited data is critical in domains like security where labeled examples are scarce; LLMs with standard adaptation methods still struggle when faced with few samples for difficult tasks.

Method: Attention Head Reweighting (AHR) - learns only a single scalar per attention head, leveraging functional specialization of individual heads to drastically reduce trainable parameters while adapting LLMs to new text-classification tasks.

Result: AHR outperforms standard baselines like LoRA in few-shot settings, despite having 200-1000x fewer trainable parameters (only ~0.0001% of model parameters modified).

Conclusion: AHR is data-efficient, interpretable (learned weights can be analyzed), and effective for few-shot text classification; it also aids understanding of attention heads responsible for in-context learning in LLMs.
...
### [136] [PUe: Biased Positive-Unlabeled Learning Enhancement by Causal Inference](https://arxiv.org/abs/2607.13428)
*Xutao Wang,Hanting Chen,Tianyu Guo,Yunhe Wang*

Main category: cs.LG

TL;DR: This paper proposes PUe, a PU learning framework using normalized propensity scores and NIPW to handle selection bias in positive-unlabeled learning, showing improvements on MNIST, CIFAR-10, and ADNI datasets.


<details>
  <summary>Details</summary>
Motivation: Existing PU learning methods assume positive labels are selected entirely at random, but real-world data has uneven label distributions with selection bias. The authors aim to address this limitation.

Method: PUe framework builds on SAR-PU with: (1) normalized inverse-probability-weighted PU risk formulation, (2) theoretical analysis of normalized sample-weight error under biased labeling, (3) regularized deep propensity-score estimation, (4) integration with cost-sensitive PU methods, (5) support for selectively labeled negative classes.

Result: Experiments on MNIST, CIFAR-10, and ADNI datasets show PUe improves over several PU baselines under non-uniform label distributions.

Conclusion: Normalized propensity weighting effectively mitigates selection bias in PU learning, providing a more realistic framework for real-world applications.
...
### [137] [Discrete Diffusion Models: A Unified Framework from Tokenization to Generation](https://arxiv.org/abs/2607.13431)
...
TL;DR: This paper presents a unified conceptual framework for discrete denoising diffusion models (DDMs), showing how existing approaches like transition-matrix, masking, and score-based methods stem from different constructions of the discrete state space, and it analyzes design trade-offs across training, inference, scaling, and evaluation.


<details>
  <summary>Details</summary>
Motivation: DDMs are emerging as powerful alternatives to autoregressive models for discrete data, but the field lacks a unified understanding of how different DDM formulations relate to each other and how the discrete state space construction fundamentally shapes model behavior.

Method: The authors develop a unified conceptual framework centered on how the discrete state space is constructed (tokenization, vocabulary topology, structural alphabets), and show that existing approaches (transition-matrix, masking/absorbing-state, score/ratio-based) are different instantiations of this common design space.

Result: The framework reveals common design trade-offs across training objectives, inference algorithms, scaling behavior, systems optimization, and evaluation protocols, providing a structured view of the DDM landscape.

Conclusion: The paper suggests several promising directions for future research based on the identified trade-offs and design choices within the unified framework.
...
### [138] [Local Redundancy: An Information-Theoretic Measure of Plasticity from Synthetic Memorization](https://arxiv.org/abs/2607.13432)
*Jiaxuan Cheng*

Main category: cs.LG

TL;DR: The paper introduces "local redundancy," an information-theoretic measure of neural network plasticity that predicts transfer learning performance better than existing metrics and enables checkpoint selection.


<details>
  <summary>Details</summary>
Motivation: Existing plasticity measures (effective rank, dead neuron fraction, weight norm) lack theoretical grounding and correlate poorly with performance on new tasks.

Method: Local redundancy is defined as the worst-case redundancy of a local model family along gradient directions. The expected squared gradient norm on a synthetic memorization task provides a tractable lower bound.

Result: Local redundancy outperforms existing measures for predicting downstream performance in continual image classification and time series transfer learning, and enables pretraining checkpoint selection when validation loss plateaus.

Conclusion: Local redundancy is a principled, theoretically grounded measure of plasticity that correlates well with actual task performance.
...
### [139] [Distributionally Robust and Safe Imitation Learning](https://arxiv.org/abs/2607.13436)
*Ahmed Aboudonia,Naira Hovakimyan*

Main category: cs.LG

TL;DR: A distributionally robust and safe imitation learning framework is proposed to handle both policy-induced and uncertainty-induced distribution shifts, demonstrated on a UAV task.


<details>
  <summary>Details</summary>
Motivation: Imitation learning is sensitive to distribution shifts, posing safety risks. Existing methods do not explicitly address both types of shifts (policy-induced and uncertainty-induced).

Method: Combines Taylor Series Imitation Learning (TaSIL) for policy-induced shifts with distributionally robust adaptive control for uncertainty-induced shifts, incorporating safety constraints.

Result: Demonstrated effectiveness on a UAV case study in an uncertain environment with obstacle avoidance.

Conclusion: The proposed unified framework optimizes IL performance under distributional uncertainty while systematically accounting for safety.
...
### [140] [PQFA: Parallel Quantum Feature Augmentation of Fused Representations for Multimodal Classification](https://arxiv.org/abs/2607.13466)
*Mingzhu Wang,Yun Shang*

Main category: cs.LG

TL;DR: PQFA uses parallel shallow quantum circuits to augment fused multimodal features, outperforming classical MLP augmentation with ~10x fewer parameters and improving robustness under missing modalities.


<details>
  <summary>Details</summary>
Motivation: Post-fusion enhancement in multimodal learning is underexplored; existing methods focus on alignment and fusion but not on augmenting the already-fused representation.

Method: Text (RoBERTa) and image (ViT) features are fused via bidirectional cross-attention, attentive pooling, and adaptive gated fusion. The fused feature is amplitude-encoded into parallel variational quantum circuits; measurement readouts are concatenated with the classical representation for final prediction.

Result: PQFA outperforms both the fusion-only backbone and a width-matched MLP augmentation baseline on MM-IMDb and N24News, using only ~2.2K vs. 24.0K augmentation parameters. It also shows improved robustness under missing modalities, especially when text is degraded.

Conclusion: PQFA is an effective, parameter-efficient post-fusion augmentation strategy; improvements are not reproducible by random mappings, wider classical layers, or untrained quantum circuits, and performance remains stable under simulated noise.
...
### [141] [Explainable Artificial Intelligence for Anomaly Detection in Banking Transactions: An Internal Audit Perspective](https://arxiv.org/abs/2607.13469)
*Anupa Lodhi*

Main category: cs.LG

TL;DR: An XAI framework combining Isolation Forest for anomaly detection and SHAP for explainable outputs, deployed via a Streamlit dashboard for banking fraud detection, achieving 0.91 precision and 0.88 recall while improving auditor confidence.


<details>
  <summary>Details</summary>
Motivation: Conventional rule-based fraud detection systems suffer from high false-positive rates and lack explainability, limiting their usefulness for compliance teams in banking.

Method: Uses an Isolation Forest (iForest) model for unsupervised anomaly scoring, enhanced with a SHAP explanation layer for feature-attributed justifications, and a Streamlit dashboard for user-friendly visualization.

Result: Achieved 0.91 precision and 0.88 recall on synthetic banking data, outperforming three unsupervised baselines. Expert feedback confirmed improved auditor confidence and decision quality.

Conclusion: The framework advances accountable, transparent AI deployment in regulated financial environments by providing interpretable, auditable fraud detection outputs.
...
### [142] [DeepLoop: Depth Scaling for Looped Transformers](https://arxiv.org/abs/2607.13491)
*Shuzhen Li,Yifan Zhang,Jiacheng Guo,Quanquan Gu,Mengdi Wang*

Main category: cs.LG

TL;DR: Looped Transformers share parameters across repeated forward passes, creating a tied-depth effect that requires adjusted residual scaling. The paper proposes DeepLoop, which modifies scaling exponents based on visit-alignment to stabilize training, showing improvements when recurrent depth is active.


<details>
  <summary>Details</summary>
Motivation: Standard residual scaling rules (like DeepNorm) assume untied Transformers where each layer has its own parameters. In looped Transformers, parameter reuse across multiple visits creates a different gradient aggregation problem, requiring new scaling analysis to ensure stable training at increased unrolled depth.

Method: Formalizes tied-depth effect via a first-order perturbation bound controlled by a visit-alignment coefficient κ_R. Derives scaling rules: α=(2N)^{1/2} and β=(8N)^{-1/2} for unrolled depth N in Post-LN DeepNorm architecture. The approach is called DeepLoop.

Result: On GPT-2 small and medium scale looped language models, DeepLoop is neutral (no degradation) when no physical block is revisited, and improves validation loss and downstream accuracy once recurrent depth is activated.

Conclusion: Stable training in recurrent-depth Transformers requires residual scaling rules that account for parameter visit frequencies, not just nominal layer count. The alignment coefficient κ_R governs the necessary scaling exponent shift from 1/4 to 1/2.
...
### [143] [Factorized Spectral Representations for Reinforcement Learning](https://arxiv.org/abs/2607.13498)
*Junyi Wu,Dan Li*

Main category: cs.LG

TL;DR: FaStR uses a tensor CP decomposition of the transition kernel to learn separate state, action, and next-state spectral representations, improving sample efficiency and enabling transfer learning across actuator changes.


<details>
  <summary>Details</summary>
Motivation: Existing spectral representation methods treat the transition kernel as a matrix (state-action × next-state), overlooking its natural three-mode tensor structure. This limits sample efficiency and does not leverage the factored nature of dynamics.

Method: FaStR performs a CP decomposition of the three-mode transition tensor (states, actions, next states) using a noise contrastive objective, learning separate encoder networks for each mode that together form a spectral representation.

Result: The factored representation reduces the hypothesis class size, improving sample efficiency by a factor scaling with the smaller of state/action dimensions. Largest empirical gains appear in high-dimensional locomotion tasks with factored dynamics. The state encoder transfers to new actuators without retraining.

Conclusion: CP tensor decomposition of the transition kernel yields more sample-efficient spectral representations for reinforcement learning, with built-in modularity enabling zero-shot transfer of the state encoder under actuator shifts.
...
### [144] [CDS: Counterfactual Directionality Score for Structured Interventions in Spatial Graphs](https://arxiv.org/abs/2607.13508)
*Humaira Anzum,Md Ishtyaq Mahmud,Jagan Mohan Reddy Dwarampudi,Tania Banerjee*

Main category: cs.LG

TL;DR: A framework using counterfactual interventions in graph-based models (NIM) to quantify directional influence between node types, validated on synthetic and spatial transcriptomics data.


<details>
  <summary>Details</summary>
Motivation: Existing methods (attention, attribution, correlation) capture associations but lack a principled framework for evaluating directional effects under controlled perturbations in spatial biological systems.

Method: Trains a Neighbor Influence Model (NIM) to predict node states from neighborhoods, applies constrained counterfactual interventions modifying neighborhood composition, defines Counterfactual Directionality Score (CDS) as a finite-difference measure, and uses core-level bootstrap for uncertainty estimation.

Result: CDS recovers known directional influence in synthetic graphs, remains well-calibrated under null conditions, robust to confounding signals, and yields biologically plausible interactions on spatial transcriptomics data.

Conclusion: The proposed counterfactual intervention framework provides a principled, interpretable approach for estimating directional influence between node populations in graph-based spatial models.
...
### [145] [ExTernD: Expanded-Rank Ternary Decomposition Ternary LLM PTQ with Accuracy Approaching Any Quantization Level](https://arxiv.org/abs/2607.13511)
*Chethan Reddy G. P*

Main category: cs.LG

TL;DR: ExTernD is a post-training LLM compression method that decomposes weight matrices into two ternary factors and a scale vector with expanded inner rank, achieving near-bf16 accuracy at ~5.1-5.7 effective bits per weight, matching 4-bit quantization quality.


<details>
  <summary>Details</summary>
Motivation: Existing ternary quantization schemes have limited accuracy due to fixed rank/plane count, unable to approach full precision arbitrarily closely. The authors aim to bridge the accuracy gap between extreme low-bit (ternary) compression and full precision by allowing an expanded inner rank that can progressively correct quantization errors.

Method: Factor each weight matrix A ∈ ℝ^{m×n} into A ≈ B·diag(D)·C where B,C ∈ {-1,0,+1}^{...} are ternary factors and D ∈ ℝ^k is a real scale vector. The inner rank k = μ·min(m,n) is expanded beyond full rank (μ>1), so extra components beyond full rank correct quantization errors. Sparsity is controlled by threshold τ, enabling continuous accuracy-efficiency trade-offs.

Result: ExTernD matches Q4_K per-matrix accuracy at 5.2-5.5 effective bpw (5.1-5.5 with importance weighting) on Gemma-4-E2B and Qwen3.5-4B. Full Qwen3.5-4B conversion at μ=3 achieves 10.10 wikitext-2 perplexity vs 9.78 for bf16 (+3.2%), near Q4_K/Q5_K quality at ~5.7 effective bpw.

Conclusion: ExTernD proves that ternary decomposition with expanded rank can theoretically approach bf16 accuracy arbitrarily closely, and practically matches 4-bit quantization quality while maintaining continuous memory/compute scaling — overcoming the fundamental accuracy ceiling of fixed-rank ternary schemes.
...
### [146] [Clustering algorithms for multivariate wind farm SCADA data filtering](https://arxiv.org/abs/2607.13544)
*Nicolò Italiano,Vasilis Pettas,Tuhfe Göçmen,Nicolaos A. Cutululis*

Main category: cs.LG

TL;DR: This paper compares clustering algorithms against manual filtering for cleaning SCADA data from offshore wind turbines, showing cluster-based methods can achieve higher accuracy but still require expert involvement.


<details>
  <summary>Details</summary>
Motivation: Wind farm SCADA data contains many anomalies, transients, and special operational modes that must be filtered out for normal operation analysis. Manual filtering by experts is time-consuming, so automated methods are needed.

Method: The authors compare multiple clustering algorithms against manual expert filtering, using novel evaluation metrics designed for unlabeled data. They apply these methods to SCADA data from three offshore wind turbines, using 10-minute statistics across multiple data channels.

Result: Cluster-based methods can detect both evident and subtle outliers, achieving higher accuracy than manual filtering in most cases. However, accuracy and data retention vary considerably across models. The study highlights the importance of extending analysis beyond the power curve.

Conclusion: Clustering algorithms can partially replace manual filtering with improved accuracy, but expert involvement remains necessary, though to a reduced extent.
...
### [147] [From Novice to Expert: Cost-Aware Bandits for Evolving Worker Performance in Crowdsensing](https://arxiv.org/abs/2607.13546)
*Yin Huang,Qingsong Liu,Jie Xu*

Main category: cs.LG

TL;DR: This paper addresses budget-constrained online worker recruitment in mobile crowdsensing, where workers' sensing quality improves with experience and stabilizes, and costs are unknown and time-varying. The authors formulate a structured bandit model with increasing-then-converging reward functions and unknown costs, proposing a cost-aware learning framework that detects saturation and allocates budget efficiently, with theoretical guarantees and experimental validation.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based MC recruitment methods assume stationary sensing quality with fixed mean over time, ignoring that worker performance typically improves with experience and eventually stabilizes, and that sensing costs can be unknown in advance due to time-varying device and context states.

Method: Formulates a structured bandit model where each worker's expected reward evolves according to an unknown increasing-then-converging function of participation count, with unknown expected costs. Develops a cost-aware online learning framework that jointly learns evolving reward trajectories and heterogeneous costs, detects performance saturation, and allocates limited budget to maximize long-term sensing utility.

Result: Provides theoretical performance guarantees and demonstrates through extensive experiments consistent improvements over baselines that ignore experience-driven dynamics or assume known costs.

Conclusion: The proposed cost-aware online learning framework effectively handles the practical dynamics of worker learning and unknown costs in budget-constrained mobile crowdsensing, outperforming existing approaches.
...
### [148] [The SIGReg Objective as Variational Free Energy: A Theoretical Active-Inference Account of JEPA World Models](https://arxiv.org/abs/2607.13612)
*Fabio Arnez,Alexandra Gomez-Villa*

Main category: cs.LG

TL;DR: This paper establishes a formal connection between Joint-Embedding Predictive Architectures (JEPAs) and Active Inference (AIF), showing that the choice of anti-collapse regulariser determines whether a JEPA's objective is a valid AIF variational free energy. It organizes four regularisers into an entropy-estimator hierarchy and proves a correspondence theorem where SIGReg yields an exact information bottleneck, while VICReg and LogDet produce unsafe upper bounds.


<details>
  <summary>Details</summary>
Motivation: JEPAs are widely used for latent world models but lack a normative (principled) justification—they rely on empirical performance. The authors aim to ground JEPAs in Active Inference theory, providing theoretical rigor to their design choices.

Method: The authors analyze four non-contrastive regularisers (VICReg, LogDet, PairDist, SIGReg) for JEPAs, organizing them into an entropy-estimator hierarchy indexed by a prior-miscalibration gap. They prove a correspondence theorem connecting JEPA objectives to AIF variational free energy under specific regulariser choices, with machine-verified proofs in Lean 4.

Result: SIGReg eliminates the prior-miscalibration gap, making the JEPA objective an exact information bottleneck that preserves the AIF surprise bound. VICReg and LogDet are unsafe upper bounds; PairDist is a safe lower bound. VICReg leaves an irreducible second-order anisotropy term. The correspondence extends to multi-step expected free energy, ensemble epistemic value, and learned-policy regimes.

Conclusion: The choice of anti-collapse regulariser fundamentally determines whether a JEPA is theoretically aligned with Active Inference. Only SIGReg yields a fully valid AIF model. The paper identifies state-epistemic value as the one AIF term missing from current JEPA world models, providing clear theoretical predictions for empirical testing.
...
### [149] [FastCentNN: Accelerating Centroid Neural Network with Entropy Proxy](https://arxiv.org/abs/2607.13613)
*Le-Anh Tran*

Main category: cs.LG

TL;DR: FastCentNN accelerates Centroid Neural Network training by introducing an early splitting strategy based on centroid movement (entropy proxy), achieving up to 16% faster runtime with comparable clustering quality.


<details>
  <summary>Details</summary>
Motivation: CentNN's strict local stabilization before splitting causes prolonged low-movement training phases, making it inefficient. The authors aim to reduce unnecessary reassignment epochs.

Method: FastCentNN uses an early splitting criterion based on total centroid movement per epoch as an entropy proxy, with both absolute and stage-relative movement thresholds for adaptive or fixed splitting.

Result: Up to 16% runtime reduction on synthetic 2D datasets and ~5% on high-dimensional datasets, with clustering quality comparable to CentNN.

Conclusion: FastCentNN is a practical drop-in replacement for CentNN that offers a simple, interpretable speed-stability trade-off through configurable splitting thresholds.
...
### [150] [How the Hessian-Spectrum of Neural Networks Depends on Data](https://arxiv.org/abs/2607.13631)
*Jasraj Singh,Enea Monzio Compagnoni,Antonio Orvieto*

Main category: cs.LG

TL;DR: This paper derives analytical eigenvalues of the Hessian matrix for linear neural networks of arbitrary depth/width on arbitrary datasets, showing that sharpness in classification tasks with MSE loss relates to the maximum class proportion. The theoretical predictions remain robust even when relaxing impractical assumptions and adding nonlinearities.


<details>
  <summary>Details</summary>
Motivation: The Hessian matrix is crucial for understanding loss landscapes, optimization dynamics, generalization, and second-order learning in deep learning, but prior work relied on empirical results or oversimplified theoretical settings. This paper aims to provide a more general theoretical treatment.

Method: The authors derive the eigenvalues of the Hessian for linear networks with arbitrary widths, depths, and datasets (arbitrary samples, features, labels). They analyze classification with MSE loss and empirically test predictions by relaxing assumptions one at a time and incorporating nonlinearities.

Result: Sharpness of the solution is directly related to the maximum proportion of samples belonging to any class. The theoretical predictions are considerably robust in most cases even when simplifying assumptions are removed or nonlinearities are added.

Conclusion: The findings extend to more practical learning setups, suggesting that the derived relationships between class proportions and Hessian sharpness hold beyond the simplified linear network setting.
...
### [151] [Consensus as Privileged Context for Label-Free Self-Distillation](https://arxiv.org/abs/2607.13643)
*John Gkountouras,Josip Jukić,Ivan Titov*

Main category: cs.LG

TL;DR: CANON is a label-free training method that converts majority vote consensus from multiple reasoning samples into dense, token-level supervision, outperforming existing label-free methods on math/science benchmarks while using significantly less compute.


<details>
  <summary>Details</summary>
Motivation: Existing consensus-based methods (filtering, preference, scalar reward) discard rich information from agreeing solutions; the authors aim to leverage consensus as dense token-level supervision for more effective self-distillation.

Method: CANON samples multiple solutions per unlabeled prompt, extracts the majority answer, conditions a frozen model snapshot on a solution reaching that answer (consensus-anchored teacher), and supervises the model on its own rollouts at every token position.

Result: Improves pass@1 by up to 12 points on math/science benchmarks; outperforms label-free RL by 6 points at 1/7th compute; matches gold-label training on transferred tasks; solves problems previously unsolvable in 32 attempts; improves majority vote accuracy.

Conclusion: CANON demonstrates that converting consensus into dense token-level supervision is more effective and efficient than prior label-free methods, enabling models to solve previously unsolvable problems without any labeled data.
...
### [152] [Maximally Robust Satisficing Bayesian Optimization](https://arxiv.org/abs/2607.13652)
*Samuli Kinnunen,Petrus Mikkola,Antti Niskanen,Arto Klami*

Main category: cs.LG

TL;DR: This paper introduces a Bayesian optimization method for finding satisficing (good enough) solutions that are robust to input perturbations after deployment, rather than searching for the global optimum.


<details>
  <summary>Details</summary>
Motivation: Many design tasks only require a sufficiently good solution rather than the exact optimum, and there are often multiple satisfactory solutions forming a superlevel set. The key question is which one to prefer among them.

Method: The authors propose a Bayesian optimization approach that efficiently finds satisficing solutions within the superlevel set that are robust to maximally large input perturbations expected after deployment.

Result: The method identifies robust satisficing solutions that tolerate large input perturbations, unlike previous works that assumed controllable inputs during both optimization and deployment.

Conclusion: Robustness to post-deployment input perturbations is a good criterion for selecting among multiple satisfactory solutions, and the proposed Bayesian optimization method can efficiently find such robust satisficing designs.
...
### [153] [The Hyperspherical Geometry of CLIP Latent Space: A Semantic Mixture Model](https://arxiv.org/abs/2607.13660)
*Zijie Yu,Gaowen Liu,Ramana Rao Kompella,Philip S. Yu,Yue Song*

Main category: cs.LG

TL;DR: The paper proposes Mixtures of von Mises-Fisher (MovMF) distributions to model CLIP's hyperspherical latent space, outperforming Gaussian assumptions for density estimation, out-of-distribution detection, and semantic decomposition.


<details>
  <summary>Details</summary>
Motivation: CLIP embeddings live on a hypersphere (cosine similarity space), but existing probabilistic models use Gaussian assumptions that ignore this directional geometry, leading to poor density estimation.

Method: Use Mixtures of von Mises-Fisher (MovMF) distributions on the unit hypersphere, learned via Expectation-Maximization (EM), where each component corresponds to a semantic concept.

Result: Significant improvements in long-tailed and out-of-distribution detection; natural semantic decomposition with sparse probabilistic concept combinations.

Conclusion: CLIP latent space is better modeled as a hyperspherical semantic mixture than an isotropic Gaussian, providing a geometrically consistent probabilistic framework.
...
### [154] [Optimal and Efficient Contextual Combinatorial Semi-bandits with General Function Approximation](https://arxiv.org/abs/2607.13686)
*Hao Qin,Chicheng Zhang*

Main category: cs.LG

TL;DR: SquareCB.Comb is a computationally efficient algorithm for contextual combinatorial semi-bandit problems that achieves minimax optimal regret of O(√(m A T log|F|)) without structural assumptions on action sets, generalizing previous slate recommendation results.


<details>
  <summary>Details</summary>
Motivation: To solve the contextual combinatorial semi-bandit problem with general reward function approximation, addressing limitations of previous methods that required restrictive structural assumptions on action sets or were limited to slate recommendation settings.

Method: SquareCB.Comb algorithm that solves a convex optimization problem at each round to sample a combinatorial action, balancing exploration and exploitation in a computationally efficient manner.

Result: Achieves minimax optimal regret bound of O(√(m A T log|F|)), where A is number of arms, m is max arms per action, T is time horizon, and F is reward function class.

Conclusion: The proposed method matches state-of-the-art regret guarantees in restricted settings while generalizing to arbitrary combinatorial action structures and general reward function approximation.
...
### [155] [Microstructure-Conditioned Surrogate Models for Graded Multiscale Optimization of Mycelium Composites](https://arxiv.org/abs/2607.13688)
*J. Storm,I. B. C. M. Rocha,S. Schyck,K. Masania,F. P. van der Meer*

Main category: cs.LG

TL;DR: This paper presents a hypernetwork-conditioned hybrid physics-data surrogate model that enables efficient multiscale optimization of microarchitectured sustainable materials (mycelium-woodchip composite), achieving 42% peak stress reduction in functionally graded structures with small training datasets.


<details>
  <summary>Details</summary>
Motivation: Traditional multiscale simulations require large amounts of data even for fixed microstructures, and when optimizing across a range of microstructures, the data demand becomes prohibitive. There is a need for efficient surrogate models that can handle varying microstructures with limited data.

Method: A hybrid physics-data surrogate model conditioned on microstructural variables using a hypernetwork architecture. The surrogate is trained on small datasets and enables multiscale simulations of functionally graded structures, validated against full FE^2 simulations. The approach is extended by conditioning directly on manufacturing variables.

Result: The conditioned surrogate enables accurate predictions for mycelium-woodchip composites with small datasets. In multiscale optimization of a graded disk, peak stress was reduced by 42% compared to a random microstructure baseline. The method also works with direct manufacturing variable conditioning.

Conclusion: Conditioned surrogate models offer a practical route to engineer microscale for desired macroscale behavior, accelerating the development and design of future sustainable materials and structures through efficient multiscale optimization.
...
### [156] [Conditional Invertible Neural Networks for Data-Driven UAV Control: A 2-D Proof of Concept](https://arxiv.org/abs/2607.13703)
*Christian Wittke,Stephan Myschik,Oliver Niggemann*

Main category: cs.LG

TL;DR: cINNs learn probabilistic inverse dynamics for multicopter control, matching INDI performance in closed-loop (RMSE 9.7 vs 9.5 m) with 47% acceptable tracking, while failures stem from command bandwidth limits and data coverage gaps.


<details>
  <summary>Details</summary>
Motivation: To develop probabilistic inverse-dynamics models that can replace traditional INDI controllers while capturing multimodality and uncertainty in the control distribution.

Method: Conditional invertible neural networks (cINNs) with rational-quadratic spline coupling and invertible linear mixing, trained on INDI teacher data to model $p(u \mid s_t, c_t)$.

Result: Open-loop: R²=0.944, mean CRPS=0.0915, log-probability-error correlation ρ=-0.60. Closed-loop: position RMSE 9.7 m (INDI: 9.5 m), 47% acceptable tracking.

Conclusion: cINNs can approximate INDI performance but are limited by command bandwidth and data coverage; these are the dominant failure mechanisms causing attitude divergence and phase lag.
...
### [157] [DAGR: State-Conditioned Goal Representations via Difference-Aware Goal Cross-Attention](https://arxiv.org/abs/2607.13731)
*Xing Lei,Wenyan Yang,Xuetao Zhang,Donglin Wang*

Main category: cs.LG

TL;DR: DAGR refines goal encodings by making them state-conditioned via gated cross-attention, improving navigation tasks but not universally outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Existing goal-conditioned RL encoders are state-independent, so they cannot distinguish which parts of a goal still require action, forcing the policy to recover this cue by inverting both encoders.

Method: DAGR refines static goal embeddings into state-conditioned ones using multi-scale gated cross-attention with a near-identity residual, plus difference-aware goal cross-attention that biases attention scores with a per-token state-goal discrepancy map.

Result: On OGBench, DAGR improves navigation. Ablations show the gain comes from the gated residual, not the difference bias. On manipulation and puzzle tasks it matches or falls below the base.

Conclusion: DAGR is a structured refinement that yields task-specific improvements (navigation) but is not a universal improvement across all domains.
...
### [158] [Constraint-Driven Model Optimization: An Industry Framework for Selecting Compression and Acceleration Techniques in Modern Machine Learning Systems](https://arxiv.org/abs/2607.13735)
*Dhruv Shivkant,Saket Mohanty,Utkarsh Wadhwa*

Main category: cs.LG

TL;DR: This paper proposes a constraint-driven, multi-objective framework for model optimization that maps deployment techniques (quantization, pruning, distillation, PEFT, inference optimization) to five interacting dimensions: data availability, latency budget, memory budget, accuracy tolerance, and retraining budget. It provides a prescriptive decision framework with four industrial scenarios.


<details>
  <summary>Details</summary>
Motivation: Practitioners currently navigate model optimization techniques through heuristics rather than principled methodology, despite a rich literature. The field lacks a structured, constraint-aware formalism for making optimization decisions in production deployments.

Method: The authors introduce a unified framework characterizing production deployments along five constraint dimensions (data, latency, memory, accuracy, retraining budgets). They synthesize empirical gains from research literature, mapping techniques to operational constraints rather than algorithmic categories, and propose prescriptive decision pipelines for four representative industrial scenarios.

Result: A constraint-aware, multi-objective optimization framework with a prescriptive decision process, supported by synthesized quantitative evidence from literature and demonstrated through four industrial scenario pipelines.

Conclusion: This is one of the first structured attempts to formalize model optimization as a constraint-driven engineering process, bridging the gap between research techniques and practical deployment decisions.
...
### [159] [Implementations of Quantum and Classical Topology-Aligned Architectures for Molecular Property Prediction](https://arxiv.org/abs/2607.13737)
*James T. Pegg,Hubert Okadome Valencia,Ronin Wu*

Main category: cs.LG

TL;DR: This paper proposes a topology-aligned inductive bias for parameter-efficient learning in quantum chemistry, instantiated in quantum (Iso-QGNN) and classical (Iso-CGNN) models that mirror molecular bond graphs. With only 64 parameters, they achieve strong AUC scores on QM9 tasks (HOMO-LUMO gap ~0.88-0.91, dipole ~0.78), reach 90% performance within ~250 molecules, and show stable gradient norms.


<details>
  <summary>Details</summary>
Motivation: To achieve parameter-efficient learning in low-data and resource-constrained quantum chemistry regimes by designing an inductive bias that aligns model architecture with molecular bond topology.

Method: Two architectures: a variational quantum circuit (Iso-QGNN) and a parameter-matched classical message-passing model (Iso-CGNN), both using a topology-aligned inductive bias where atoms map to computational units and bonds determine pairwise interactions via shared learnable parameters.

Result: With 64 trainable parameters, test AUCs of ~0.88 (quantum) and ~0.91 (classical) on HOMO-LUMO gap, ~0.78 (both) on dipole moment classification. Models reach 90% asymptotic performance within ~250 training molecules with stable gradient norms.

Conclusion: The topology-aligned inductive bias is the key driver of parameter efficiency at QM9 scale, with implications for matched-baseline benchmarking in quantum machine learning.
...
### [160] [Algebraic Representability as the Limiting Regime of Grokking: An Exactly Solvable Model with Holomorphic Activations](https://arxiv.org/abs/2607.13749)
*Chon-Fai Kam,Xavier Cadet,Miloud Bessafi,Frederic Cadet*

Main category: cs.LG

TL;DR: When neural networks' expressible function class collapses to a finite-dimensional algebraic variety (via monomial activation z^k on modular arithmetic tasks), grokking disappears and is replaced by binary success/failure determined solely by whether the target is algebraically representable. This is the limiting case of the capacity-grokking relationship.


<details>
  <summary>Details</summary>
Motivation: To understand the extreme end of the capacity-grokking spectrum—what happens when model capacity is so constrained that the network's expressible function class collapses to a finite-dimensional algebraic variety, and whether grokking persists or dissolves.

Method: Study two-layer networks with holomorphic monomial activation σ(z)=z^k on modular arithmetic tasks encoded via roots of unity. Provide algebraic characterisation of the (k+1)-dimensional subspace of characters the network can express. Validate across 585 runs and connect to standard networks via bottleneck ablation.

Result: A task is representable iff its discrete Fourier support lies on diagonal u+v≡k (mod p). Non-representable targets cannot be fit even on training set (positive lower bound on loss, independent of width). 99.8% accuracy across 585 runs. No grokking—binary success/failure with no memorisation regime.

Conclusion: Grokking dissolves into a simple representability question when the expressible class is sufficiently constrained. This binary behaviour is the limiting case of the capacity-grokking relationship, and the bottleneck ablation traces a continuous path from representational failure through memorisation to grokking as capacity grows.
...
### [161] [MxGPS: Multiplex Graph Transformers for a Power Grid Foundation Model](https://arxiv.org/abs/2607.13763)
...
TL;DR: MxGPS is a multiplex graph transformer that jointly trains on Static State Estimation and AC Power Flow tasks to prevent topology overfitting, achieving 0% boundary violation on unseen grids with only 1.6M parameters, while single-task models degrade 190-1400% under topology shift.


<details>
  <summary>Details</summary>
Motivation: Single-task fine-tuning of GNNs for power grids fails under topology shift—models with lowest in-distribution error degrade the most on unseen grids. The authors term this "topology overfitting" and seek to address this systematic failure mode.

Method: MxGPS is a multiplex graph transformer with K task-specialized GPS branches over a shared node encoder. It uses self-supervised pre-training and multi-task fine-tuning on SSE and PF jointly, with a cross-branch attention module. The joint objective forces the shared encoder to satisfy complementary gradient signals, preventing topology-specific overfitting.

Result: MxGPS achieves 0% BVR on all four zero-shot Power Flow topologies (14-, 24-, 162-, 300-bus). Under topology shift, single-task models degrade 190-1400% while MxGPS degrades only 39%. Achieves this with 1.6M parameters (12x fewer than GridFM baseline).

Conclusion: Multi-task joint training is a principled and parameter-efficient mechanism for topology-agnostic generalization in power grid foundation models. The performance inversion directly implicates topology overfitting as the failure mechanism, not insufficient model capacity.
...
### [162] [Mono-Z Dark Matter Search with Neural Spline Flows Using CMS Run 2015D Open Data](https://arxiv.org/abs/2607.13771)
*Hitesh Rasineni,Bhavishya Chebrolu*

Main category: cs.LG

TL;DR: This paper reports a dark matter search using CMS open data (2015D, 2.32 fb⁻¹ at 13 TeV) in the mono-Z→ℓ⁺ℓ⁻ channel. It employs Neural Spline Flows to model background and DM signal densities, constructs a per-event log-likelihood ratio test statistic, and sets observed (expected) 95% CL upper limits on signal strength for scalar, vector, and axial-vector mediators. Observed limits are weaker than expected due to high-MET background modeling discrepancy.


<details>
  <summary>Details</summary>
Motivation: The search targets dark matter produced in association with a leptonically decaying Z boson (mono-Z channel). It aims to test simplified DM mediator models without imposing a hard MET threshold, leveraging machine learning (Neural Spline Flows) for sensitivity across the full kinematic phase space.

Method: Analyzed CMS Run 2015D open data (2.32 fb⁻¹) at √s=13 TeV. Selected mono-Z→ℓ⁺ℓ⁻ events (μμ and ee channels). Extracted 40 kinematic observables → cleaned to 37-dimensional feature vectors. Trained 5 Neural Spline Flows to model SM background and mediator-specific DM signal densities. Constructed per-event test statistic from log-likelihood ratio. Performed simultaneous profile-likelihood fit combining both channels.

Result: Observed (expected) 95% CL upper limits on signal strength μ: scalar mediator <0.0177 (0.0018), vector mediator <0.0362 (0.0039), axial-vector mediator <0.0498 (0.0069). Observed limits weaker than expected due to a residual high-MET background-modeling discrepancy, not evidence of DM.

Conclusion: No dark matter signal observed. The Neural Spline Flow likelihood-ratio approach applied to mono-Z DM search in both μμ and ee channels is novel. The discrepancy in high-MET modeling suggests room for improved background estimation in future analyses with larger datasets.
...
### [163] [NodeImport: Imbalanced Node Classification with Node Importance Assessment](https://arxiv.org/abs/2607.13837)
*Nan Chen,Zemin Liu,Bryan Hooi,Bingsheng He,Jun Hu,Jia Chen*

Main category: cs.LG

TL;DR: NodeImport is a framework for class-imbalanced node classification on graphs that selects important nodes (labeled, unlabeled, and synthetic) via a balanced meta-set to measure node importance, enabling dynamic, fine-grained node selection to mitigate class imbalance bias.


<details>
  <summary>Details</summary>
Motivation: Real-world node classification suffers from class imbalance where majority classes dominate training, causing biased GNN performance. Existing methods (class-prioritized weighting or synthetic node generation) fail to effectively address this issue.

Method: The paper proposes NodeImport, which uses a balanced meta-set to compute node importance scores with a theoretically derived formula. It filters valuable labeled, unlabeled, and synthetic nodes dynamically during training, and decouples synthetic node generation from filtering for compatibility with various generation methods.

Result: Evaluated across multiple datasets and GNN architectures, NodeImport outperforms existing baselines, demonstrating improved class-imbalance mitigation and flexible node selection.

Conclusion: NodeImport effectively addresses class imbalance in node classification through principled, dynamic importance-based node selection, with theoretical support for computational efficiency and compatibility with diverse node generation approaches.
...
### [164] [Heavy-Tailed Flow Matching via Random Clocks](https://arxiv.org/abs/2607.13841)
*Zhouhao Yang,Yezhen Wang,Kenji Kawaguchi,Vladimir Braverman,Haoyang Cao*

Main category: cs.LG

TL;DR: HTFM extends flow matching to heavy-tailed data by using a random clock to mix Gaussian sources, enabling better tail recovery and sample quality across imbalanced, financial, and weather extremes.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion/flow models assume Gaussian sources, which poorly match heavy-tailed data (e.g., imbalanced images, financial returns, weather extremes). This mismatch leads to inadequate tail modeling and poor inductive bias.

Method: HTFM models heavy-tailed sources as mixtures of Gaussian sources conditioned on a random clock path. The clock is encoded via truncated logsignature features, making the vector field practical. Marginalizing over the clock yields Gaussian scale mixtures (Gaussian, α-stable, Student-t).

Result: On 2D α-stable mixtures, CIFAR10-LT, and HRRR weather fields, HTFM improves mode coverage, sample quality, and tail-statistic recovery over Gaussian flow matching and heavy-tailed baselines, while retaining low-NFE sampling.

Conclusion: The random-clock formulation provides a practical tail-control interface: by varying only the clock law or tail parameter, the same architecture can calibrate tail heaviness across distribution families.
...
### [165] [Relevance-Aware Rule: Structural Deletion of Irrelevant Conditions in Decision Trees](https://arxiv.org/abs/2607.13874)
*Jung-Sik Hong,Jeongeon Lee,Min Kyu Sim,Sangheum Hwang*

Main category: cs.LG

TL;DR: Decision trees contain irrelevant conditions (IRCs) due to their splitting mechanism. This paper provides theoretical foundations for IRC deletion and proposes a structural IRC deletion framework that identifies mismatched links (C1/C0 links) to selectively delete irrelevant conditions while preserving prediction reliability.


<details>
  <summary>Details</summary>
Motivation: Decision trees contain irrelevant conditions (IRCs) caused by the structural mechanism of tree splitting. Existing deletion methods either preserve the original tree too loosely (unreliable) or too strictly (no meaningful simplification). The authors aim to provide theoretical foundations for reliable IRC deletion.

Method: The paper establishes theorems about binary splits shifting class proportions in opposite directions per branch (C1-link and C0-link). A structural IRC deletion framework is proposed: (1) match links that increase leaf-class proportion, (2) identify mismatched links as IRC candidates, (3) diagnose relevance via prediction reliability assessment, (4) selectively delete structurally and empirically irrelevant conditions while protecting reliability-critical ones.

Result: The proposed framework achieves substantial rule simplification without sacrificing the reliability of the original decision tree.

Conclusion: The structural IRC deletion framework effectively removes IRCs by leveraging theoretical insights about split mechanisms and prediction reliability, balancing simplification with reliability preservation.
...
### [166] [AI-Augmented Adaptive Digital Twin Modeling for Brain Tumor Evolution Prediction and Treatment Scheduling](https://arxiv.org/abs/2607.13877)
*Wenxi Liu,Michael Trimboli,Xianqi Li*

Main category: cs.LG

TL;DR: An AI-augmented adaptive digital twin framework integrating reaction-diffusion models, residual learning, and model predictive control improves brain tumor evolution prediction and treatment scheduling, reducing MSE by 84.3% and tumor burden by 22.4% in synthetic experiments.


<details>
  <summary>Details</summary>
Motivation: Brain tumor progression is spatially heterogeneous, patient-specific, and interacts with complex anatomy, making accurate long-term prediction and treatment scheduling extremely challenging.

Method: The framework combines an interpretable reaction-diffusion (RD) model with a 3D residual learning module for model-form correction, online digital twin updating during recursive rollout, and model predictive control (MPC) for constrained chemotherapy and radiotherapy scheduling.

Result: Hybrid RD-residual modeling reduced masked voxel-wise MSE by 84.3% and increased Dice overlap by 43.5% over RD baseline. Online DT updating further reduced MSE by 45.9% and improved Dice by 9.6%. MPC-based scheduling reduced final tumor burden by 22.4% versus fixed schedules.

Conclusion: The unified framework demonstrates feasibility for patient-specific initialization, mechanistic modeling, adaptive learning, and constrained treatment optimization, laying groundwork for real-world adaptive treatment planning despite current validation on synthetic data.
...
### [167] [Task-Oriented Sensing and Covert Transmissions for Collaborative Multi-AUV Systems](https://arxiv.org/abs/2607.13880)
*Xueyao Zhang,Chenyang Yan,Bo Yang,Xuelin Cao,Zhiwen Yu,Bin Guo,George C. Alexandropoulos,Merouane Debbah,Chau Yuen*

Main category: cs.LG

TL;DR: This paper proposes SVR-MARL, a multi-agent reinforcement learning framework that helps AUVs coordinate covertly underwater by valuing sensed information based on its actual contribution to cooperative tasks, rather than assuming ideal communication. It improves task efficiency while reducing exposure risk.


<details>
  <summary>Details</summary>
Motivation: AUVs in covert operations cannot use active sonar or frequent communications due to exposure risk, leading to incomplete perception and limited task efficiency. Existing MARL methods assume ideal communication, and traditional comms optimization focuses on link-level metrics, neither capturing the true contribution of perceptual information under realistic covert communication constraints.

Method: Proposes SVR-MARL (Sensed Information Value Realization Multi-Agent Reinforcement Learning), which uses practical information to characterize the utility of information for cooperative tasks and learns distributed policies under realistic communication and covert constraints.

Result: Demonstrated through a case study of covert multi-AUV cooperative localization and tracking, showing improved collaborative task efficiency while reducing unnecessary communication and exposure risks.

Conclusion: The SVR-MARL framework effectively bridges the gap between communication optimization and cooperative task performance under covert constraints, enabling more efficient and safer multi-AUV coordination.
...
### [168] [RF Spectrogram Anomaly Detection with Quantum Kitchen Sinks: Architecture, Representation, and Hardware Validation](https://arxiv.org/abs/2607.13897)
*Abdallah Aaraba,Alexis Vieloszynski,Remon Polus,Ola Ahmad,Soumaya Cherkaoui*

Main category: cs.LG

TL;DR: This paper extends Quantum Kitchen Sinks (QKS) with multi-depth data re-uploading and ring entanglement for RF spectrogram anomaly detection, achieving best test AUROC of 0.8778 and F1 of 0.7995, validated on real quantum hardware (ibm_quebec) with minimal deviation from simulation.


<details>
  <summary>Details</summary>
Motivation: Anomaly detection is critical for secure spectrum management in wireless networks due to the broadcast nature of RF channels. While QKS offers a lightweight hybrid quantum feature map suitable for near-term devices, its behavior on structured signal data is poorly understood.

Method: Extended QKS with multi-depth data re-uploading and ring entanglement; introduced a validation-locked five-stage ablation protocol separating effects of shallow architecture, re-uploading depth, episode budget, input representation, and classical readout. Compared DCT, raw, and PCA inputs across configurations.

Result: DCT representations dominated raw and PCA inputs; moderate-depth entangled QKS configurations formed strongest regime; QKS outperformed matched classical baselines across all pairs; best config: test AUROC 0.8778, test F1 0.7995. Real-device validation on ibm_quebec showed AUROC deviations below 0.013 vs simulation.

Conclusion: Provides a practical, reproducible framework for deploying QKS-based anomaly detection in wireless networks, bridging real measured cellular signals and real QPU validation.
...
### [169] [An Efficient Newton Algorithm for Nonnegative Matrix Factorization with the Kullback-Leibler Divergence](https://arxiv.org/abs/2607.13919)
*Damien Lesens,Jérémy E. Cohen,Bora Uçar*

Main category: cs.LG

TL;DR: This paper proposes a Newton-type method for KL-divergence NMF that replaces the traditional separable majorant approach with a second-order Taylor expansion, generalized through a HALS-like algorithm for faster and provably converged performance.


<details>
  <summary>Details</summary>
Motivation: Existing KL-NMF algorithms rely on minimizing separable majorants of the loss, which the authors argue has reached its performance limits. A more effective optimization approach is needed.

Method: The authors replace the separable majorant with a second-order Taylor expansion (Newton-type method) and minimize this non-separable surrogate by generalizing the HALS algorithm, enabling efficient updates with convergence guarantees.

Result: The proposed algorithm competes favorably with state-of-the-art KL-NMF methods across a large variety of datasets.

Conclusion: The Newton-type approach with generalized HALS is a promising new direction for KL-NMF, outperforming existing majorant-based methods.
...
### [170] [VAIOM: Continuous-Input, Discrete-Output Decoder-Only Financial Sequence Modeling](https://arxiv.org/abs/2607.13929)
*Yiming Ma,Xinyu Chen*

Main category: cs.LG

TL;DR: VAIOM is a decoder-only Transformer for probabilistic next-return modeling on FX data that uses continuous vector inputs with categorical output distributions, outperforming LightGBM baselines across multiple test periods.


<details>
  <summary>Details</summary>
Motivation: Financial observations are continuous, heterogeneous, and noisy, but standard decoder-only next-token models are built around discrete symbolic inputs, creating a mismatch between data nature and model architecture.

Method: VAIOM separates continuous input representation from categorical output likelihood using continuous multivariate financial-event vectors as input, a categorical distribution over volatility-normalized return buckets for output, a Mixture-of-Market-States return head, auxiliary objectives (Gap, volatility-regime, Ordinal), and full-sequence supervision.

Result: All models outperform fixed single-bar LightGBM baseline in both 2025 test halves (gains of 0.029 and 0.043 bits/event). Continuous input > discrete-token input, full-sequence supervision > last-position training, and auxiliary objectives + mixture head improve likelihood.

Conclusion: The smallest evaluated complete architecture achieves the strongest validation likelihood, suggesting capacity trade-offs matter for this financial modeling task.
...
### [171] [TRACE: Turn-level Reward Assignment via Credit Estimation for Long-Horizon Agents](https://arxiv.org/abs/2607.13988)
*Leitian Tao,Baolin Peng,Wenlin Yao,Tao Ge,Hao Cheng,Mike Hang Wang,Jianfeng Gao,Sharon Li*

Main category: cs.LG

TL;DR: TRACE is a dense credit-assignment method for multi-turn agent RL that uses log-ratio state values from a frozen reference model to derive per-action rewards via TD learning, achieving large gains on complex search benchmarks without needing cold-start SFT or live-web data.


<details>
  <summary>Details</summary>
Motivation: Outcome rewards become sparse, high-variance, and misleading for long-horizon agent trajectories with many tool calls, as they fail to credit useful intermediate actions that occur before a final mistake.

Method: TRACE models rollouts as state transitions at tool-call boundaries, uses a frozen reference model to obtain gold-answer log-probabilities, transforms them into log-ratio state values, and derives per-action rewards as Temporal-Difference changes in those values—requiring no additional critic or process-label training.

Result: On BrowseComp-Plus, TRACE raises Qwen3-4B from 7.2 to 35.6 and Qwen3-30B-A3B from 8.4 to 42.6. The learned search behavior transfers to open-web benchmarks, with faster convergence during RL training.

Conclusion: TRACE is an effective dense credit-assignment method for agentic RL that enables substantial improvement in long-horizon tool-use tasks without requiring supervised fine-tuning, agentic mid-training, or live-web data.
...
### [172] [Lyapunov Exponent as Physics-Informed Dense Reward: RL Discovery of Stabilization Beyond the Kapitza Pendulum](https://arxiv.org/abs/2607.14001)
*Slava Andrejev*

Main category: cs.LG

TL;DR: This paper proposes using the Lyapunov characteristic exponent (LCE) as a dense reward signal for reinforcement learning to stabilize an inverted pendulum, successfully discovering both Kapitza pendulum oscillations and strict upright stabilization.


<details>
  <summary>Details</summary>
Motivation: To improve reinforcement learning for stabilizing an inverted pendulum with vertical motion by providing a more informative reward signal than traditional sparse rewards.

Method: The Lyapunov characteristic exponent (LCE) is used as a dense reward signal in a reinforcement learning framework for the inverted pendulum stabilization problem.

Result: The agent successfully discovered oscillatory motion (Kapitza pendulum) and also damped the pendulum's pivoting to achieve strict upright stabilization.

Conclusion: LCE serves as an effective dense reward signal for stabilizing complex pendulum systems, enabling discovery of multiple stabilization strategies.
...
### [173] [Lighthouse RL: Sample-Efficient Circuit Optimization via Strategic Reset Points](https://arxiv.org/abs/2607.14008)
...
TL;DR: Lighthouse RL introduces a sample-efficient reinforcement learning approach for analog circuit sizing that uses a strategic reset strategy from high-performing "lighthouse" configurations to guide exploration, achieving up to 1.72x faster optimization and 100% success rates.


<details>
  <summary>Details</summary>
Motivation: Traditional methods lack generalization across different performance targets, and standard RL approaches waste resources exploring unpromising regions in analog circuit sizing optimization.

Method: A strategic reset strategy that initializes episodes from high-performing configurations (called "lighthouses") discovered during training, guiding exploration toward promising regions rather than starting from random states.

Result: Up to 1.72x faster sample efficiency, 100% vs. 0-87% success rate, 75% vs. 0-50% extrapolation success, and improved objective maximization on benchmarks and two analog circuits.

Conclusion: The lighthouse reset strategy is particularly valuable for computationally expensive black-box optimization problems and can serve as a plug-and-play enhancement for any RL-based optimization approach.
...
### [174] [Transforming Rank: How Architecture Navigates the Spectral Pathologies of Depth](https://arxiv.org/abs/2607.14018)
*Katie Everett*

Main category: cs.LG

TL;DR: This paper analyzes how Transformer feedforward block components (skip connections, normalization, and the two-matrix structure) control gradient rank preservation across depth at initialization, showing that architectural design trades off rank collapse against ensemble-like behavior and parameter count, with implications for trainability on CIFAR-10.


<details>
  <summary>Details</summary>
Motivation: To understand how each architectural component of the Transformer feedforward block determines rank survival across depth, and to reinterpret known mechanisms (skip connections, normalization) as rank-preserving devices rather than just magnitude controllers.

Method: Theoretical analysis of gradient rank propagation through Transformer feedforward blocks at initialization, examining skip connections, normalization placement (Pre-Norm vs Post-Norm), the two-matrix expand-contract structure, and width expansion effects via Marchenko-Pastur law.

Result: Skip connections trade off rank collapse against ensemble behavior controlled by branch-to-skip ratio; normalization placement sets this ratio (explaining Pre-Norm vs Post-Norm differences); the second matrix decorrelates coherent mean spikes; width expansion preserves branch Jacobian rank following Marchenko-Pastur law; input-output Jacobian rank at initialization predicts trainability on CIFAR-10.

Conclusion: Deep network architecture design can be understood as navigating a fundamental tradeoff among rank collapse, ensemble-like behavior, and parameter count.
...
### [175] [Improving Wind and Solar Power Prediction with Efficient Wrapper-based Feature Selection: An Empirical Study](https://arxiv.org/abs/2607.14024)
*Daniel Grillmeyer,Marius Hadry,Michael Stenger,Vanessa Borst,Veronika Lesch,Samuel Kounev*

Main category: cs.LG

TL;DR: Proposes Cluster-based Sequential Feature Selection (CSFS), a model-agnostic method that reduces computational cost by 21% while maintaining predictive performance for renewable energy prediction tasks like wind turbine power curve modeling and photovoltaic power prediction.


<details>
  <summary>Details</summary>
Motivation: Renewable energy prediction is essential but lacks systematic feature selection methods despite many available monitoring variables. The authors aim to address the gap of limited/unsystematic feature selection in renewable energy prediction pipelines.

Method: Proposes CSFS (Cluster-based Sequential Feature Selection), a clustering-based wrapper method that is model-agnostic, automatic, and efficient. Evaluated on two use cases: wind turbine power curve modeling and photovoltaic power prediction. Compared with SFS, filter-based methods, and Random Forest feature importance.

Result: Wrapper-based methods provide better-performing feature selections overall. CSFS achieves predictive performance comparable to Sequential Feature Selection (SFS) while reducing computational cost by an average of 21%.

Conclusion: CSFS is an effective, efficient alternative to traditional feature selection methods for renewable energy prediction, offering computational savings without sacrificing accuracy.
...
### [176] [MetaPerch: Learning from metadata for bioacoustics foundation models](https://arxiv.org/abs/2607.14072)
*Mustafa Chasmai,Vincent Dumoulin,Jenny Hamer*

Main category: cs.LG

TL;DR: MetaPerch uses metadata (location, time) as auxiliary supervision signals alongside audio to build a bioacoustic foundation model that generalizes better across species distribution and acoustic domain shifts, evaluated on 17 datasets with 9 metadata sources.


<details>
  <summary>Details</summary>
Motivation: Existing bioacoustic models underutilize rich metadata (location, time) available in citizen science platforms like Xeno-Canto. Leveraging these metadata as auxiliary supervision can improve generalization to distribution shifts and domain shifts in real-world passive acoustic monitoring.

Method: MetaPerch trains a foundation model using auxiliary metadata losses (9 sources) as additional supervision signals alongside audio, enabling the model to learn species-metadata correlations for more robust representations.

Result: Strong species identification performance across multiple challenging domains; extensive empirical study shows benefits of metadata on 17 bioacoustic datasets.

Conclusion: Metadata provides valuable auxiliary supervision that improves robustness and generalization in bioacoustic models, making MetaPerch suitable for real-world PAM deployments.
...
### [177] [Linear Independent Component Analysis via Optimal Transport](https://arxiv.org/abs/2607.14081)
*Ashutosh Jha,Michel Besserve,Simon Buchholz*

Main category: cs.LG

TL;DR: This paper proposes OT-ICA, a novel Independent Component Analysis algorithm that uses the squared Wasserstein distance to Gaussian as a measure of non-Gaussianity, proving it maximizes when recovering independent components, and demonstrates superior performance over traditional proxy-based ICA methods.


<details>
  <summary>Details</summary>
Motivation: Classical ICA algorithms rely on proxy contrast functions (e.g., fourth-order cumulants, parametric log-likelihoods) for non-Gaussianity measurement because exact negentropy optimization is intractable. The authors aim to find a more direct and principled measure of non-Gaussianity for ICA.

Method: The authors propose measuring non-Gaussianity using the squared Wasserstein distance (W2^2) to a standard Gaussian. They prove this distance is maximized when a linear projection recovers an independent component, and implement gradient-based optimization (OT-ICA algorithm) to find this projection.

Result: OT-ICA outperforms proxy-based methods on simulated data across different latent variable distributions. It is also successfully applied to real-world tasks: EEG artifact removal and econometric price discovery, without requiring distributional assumptions.

Conclusion: The Wasserstein distance provides a theoretically sound and practical alternative to proxy-based non-Gaussianity measures for ICA, enabling effective application to diverse real-world tasks without distributional assumptions.
...
### [178] [Leveraging unlabelled data for generalizable neural population decoding](https://arxiv.org/abs/2607.14086)
*Ximeng Mao,Nanda H. Krishna,Avery Hee-Woon Ryoo,Matthew G. Perich,Guillaume Lajoie*

Main category: cs.LG

TL;DR: MOJO introduces a joint self-supervised and supervised learning framework for spike-tokenizing neural decoders, outperforming purely supervised models across multiple species and neural modalities, especially in label-limited settings.


<details>
  <summary>Details</summary>
Motivation: Current spike-based neural decoding models are restricted to supervised learning, limiting training to datasets with paired behavioral labels and preventing the use of abundant unlabeled neural data.

Method: MOJO (Masked autOencoder-based JOint training) combines self-supervised learning via masked autoencoding with supervised learning objectives for spike-tokenizing models, enabling joint training on both labeled and unlabeled data.

Result: Outperforms purely SL-trained models on monkey motor cortex, multi-regional mouse recordings, and human ECoG speech data; improvements are especially pronounced in few-shot finetuning; yields more interpretable neuronal representations; achieves performance comparable to NFMs designed for continuous signals.

Conclusion: Augmenting spike-tokenizing models with SSL improves performance in label-impoverished settings, enables use of unlabeled data across tasks and species, generalizes to other neural modalities, and suggests a path towards more flexible and scalable data usage for training neuro-foundation models.
...
### [179] [OriginBlame: Record- and Token-Level Data Provenance for AI Training Datasets](https://arxiv.org/abs/2607.13037)
*Haolin Xue*

Main category: cs.AI

TL;DR: ob is a record- and token-level data provenance system that precisely converts author revocation requests into exact forget sets for machine unlearning, eliminating over-deletion while adding minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Data contributors can request removal, but existing provenance systems operate at coarse file or dataset level, forcing catastrophic over-deletion. No tool can locate which training records belong to a given author.

Method: ob propagates author identity through data processing pipelines at record and token granularity, and resolves revocation requests into precise forget sets via deterministic queries.

Result: On 219,555 Wikipedia pages, record-level provenance reduced over-deletion from 101x to 1.3x. Throughput overhead: 1.3-4.0% (HuggingFace) and 2.1-19.0% (Datatrove). On a 1.7B model, provenance-based forget sets improved unlearning by 42% over random baselines.

Conclusion: Fine-grained data provenance is practical and effective for enabling precise machine unlearning, significantly reducing over-deletion while maintaining reasonable performance overhead.
...
### [180] [SPINE: Bridging the Cyber-Physical Gap with Agentic AI](https://arxiv.org/abs/2607.13049)
*Minkyu Ham,Dongho Kim,Chan Lee,Jiayi Wang,Min Jun Kim,Yixi Zhang,Guo Ye,Jihai Zhao,Soyeon Park,Han Liu*

Main category: cs.AI

TL;DR: SPINE is an agentic framework using multi-agent workflows to debug and deploy bimanual robots with minimal expert intervention, outperforming baselines across two robot platforms.


<details>
  <summary>Details</summary>
Motivation: Deploying embodied AI into physical robots still requires tedious, expert-driven calibration — the "spinal cord" bottleneck preventing scalable Embodied AI.

Method: SPINE uses two orchestrated multi-agent workflows: a profile builder for robot-specific context, and a debugger cycling through diagnosis, repair, and validation until successful teleoperation.

Result: On DOBOT X-Trainer, a novice using SPINE improved operationalization success from 75% to 100% and reduced time from 16min45s to 13min47s vs. Claude Code. On AgileX PiPER, SPINE resolved all 10 bugs vs. 9/10 for expert baseline.

Conclusion: SPINE transfers across bimanual platforms, reduces dependence on expert calibration, and moves embodied AI toward scalable real-world deployment.
...
### [181] [Interventional Grounding Audits: Black-Box Premise-Dependency Tests for LLM Chain-of-Thought via Predicate Substitution](https://arxiv.org/abs/2607.13069)
*Hironao Nakamura*

Main category: cs.AI

TL;DR: This paper introduces interventional grounding audits, a black-box method to test whether LLM chain-of-thought reasoning steps genuinely depend on their stated premises. Applied to GPT-4o on ProntoQA, it achieves F1=0.806 for detecting proof-tree dependencies, significantly outperforming baselines, and reveals "right answer, wrong reasoning" cases where 66% of correct solutions have steps insensitive to direct premise dependencies.


<details>
  <summary>Details</summary>
Motivation: LLMs produce chain-of-thought reasoning that appears logically sound but may not genuinely depend on stated premises, raising concerns about whether models are truly reasoning or exploiting spurious correlations. Existing passive evaluation methods cannot detect this "right answer, wrong reasoning" problem.

Method: Interventional grounding audits: intervene on a single premise by substituting its target predicate with a fresh symbol, re-run the model, and check if each reasoning step's normalized conclusion changes. This is a black-box, step-level test applied to GPT-4o on ProntoQA, a synthetic multi-hop deductive reasoning benchmark with gold proof trees providing ground truth dependencies.

Result: F1=0.806 for detecting proof-tree dependencies (F1=0.885 for predicate-determining dependencies; Recall=100%), significantly outperforming self-consistency baseline (F1=0.343). 66% of correctly-solved problems contain at least one aligned step insensitive to a direct proof-tree dependency under consistent substitution, all involving entity-introduction premises.

Conclusion: Interventional grounding audits effectively detect premise dependency in LLM chain-of-thought reasoning, outperforming passive methods, and reveal systematic blind spots in entity-introduction premises. The method is limited to formal, parsable benchmarks with known dependencies.
...
### [182] [Probabilistic Extension of Neuro-Symbolic AGI Robots based on Belnap's Typed Intensional FOL](https://arxiv.org/abs/2607.13073)
*Zoran Majkic*

Main category: cs.AI

TL;DR: This paper extends IFOL_B neuro-symbolic AI with probability computation for unknown sentences using Nilsson's probability structure, introducing global (knowledge-preserving) and local (real-time decision) symmetry transformations, with neural networks computing probability density via Shannon entropy maximization.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of purely neural systems (lack of interpretability, logical structure) by combining neural learning with symbolic reasoning, and to expand the cognitive power of IFOL_B by handling uncertainty in unknown sentences.

Method: Applies Nilsson's probability structure to IFOL_B, introduces global symmetry (preserves knowledge base and logical deduction) and local symmetry (for real-time decisions on predicate subsets), uses neural networks to compute probability density function KI based on Shannon's maximum information entropy.

Result: A probabilistic neuro-symbolic AGI framework that computes probability distributions for unknown sentences while maintaining logical consistency through symmetry transformations.

Conclusion: The integration of probability computation and symmetry transformations enhances IFOL_B's cognitive capabilities, enabling both global knowledge preservation and local real-time reasoning in a probabilistic neuro-symbolic AGI system.
...
### [183] [Self-Improvements in Modern Agentic Systems: A Survey](https://arxiv.org/abs/2607.13104)
...
TL;DR: This survey reviews self-improving autonomous agents, framing them as adaptive systems that convert experience into capability gains through self-induced updates to model parameters or scaffold components (prompts, memory, tools, control logic). It organizes prior work by update target and driving signals, discusses evaluation, and outlines open problems.


<details>
  <summary>Details</summary>
Motivation: As self-improving agents move from research to deployment, there is a need for a systematic understanding of how controllable evolution from experience can happen with minimal human input. This survey aims to provide a unified framework to organize and advance the field.

Method: The paper offers a system-level framework viewing an agent as a foundation model coupled with an operational scaffold (prompts, memory, tools, control logic). Self-improvement is formalized as a self-induced update operator that commits updates to either model parameters or scaffold components. Prior work is organized by update target and the signals that drive change.

Result: A structured taxonomy of self-improvement approaches based on where updates occur (parameters vs. scaffold) and what signals trigger changes. Applications are reviewed and evaluation methods are discussed.

Conclusion: The survey systematizes the emerging area of self-improving agents, providing a unified framework and taxonomy, while identifying open problems and future directions for the field.
...
### [184] [Improving Molecular Property Prediction in Small Language Models Using Graph-based Tools](https://arxiv.org/abs/2607.13115)
*Konstantinos Bougiatiotis,Dimitrios Kelesis,Georgios Paliouras*

Main category: cs.AI

TL;DR: This paper proposes a Context-Augmented Prompting framework that uses GNN experts to provide predictive hints and explanatory subgraphs to enhance SLMs' molecular property prediction from SMILES strings, achieving up to 74% relative improvement on Tox21, though still lagging behind specialized GNNs.


<details>
  <summary>Details</summary>
Motivation: Small language models (SLMs) struggle with zero-shot molecular property prediction from SMILES strings because sequence representations fail to capture key graph-topological cues (structural blindness).

Method: A modular Context-Augmented Prompting framework that at inference time uses: (1) a trained GNN expert to provide predictive hints with confidence, and (2) a GNN to extract instance-specific explanatory subgraphs (subgraph SMILES + explanatory paragraph). Evaluated on MUTAG and Tox21 with five prompting configurations.

Result: Graph-derived context yields substantial accuracy gains (≥25% relative improvement, up to 74% on Tox21). Motif relevance validated via edge-drop intervention. However, a gap to specialized GNN models persists.

Conclusion: Augmenting SLM prompts with GNN-derived structural context significantly improves molecular property prediction, but text-conditioned reasoning still cannot fully match specialized graph models, revealing both the value and limitations of this approach.
...
### [185] [Oracle Agent Memory as an Enterprise Memory Substrate for Long-Horizon AI Agents](https://arxiv.org/abs/2607.13157)
...
TL;DR: This report presents Oracle Agent Memory, a database-native memory system for long-horizon agents, achieving 93.8% accuracy on LongMemEval while using 10.7x fewer tokens than flat-history baselines. It covers memory lifecycle management, layered architecture with scope control, and evaluation methodology combining task accuracy with memory-centric metrics.


<details>
  <summary>Details</summary>
Motivation: Long-horizon agents require persistent memory for retaining task state across extended conversations, recovering user-specific facts/preferences, and accumulating procedural knowledge. Existing solutions go beyond simple document retrieval, needing a comprehensive memory layer for lifecycle management, scope control, and efficient retrieval.

Method: The paper proposes Oracle Agent Memory as a database-native memory substrate built on Oracle Database. It defines a six-stage memory lifecycle (ingestion, extraction, consolidation, retrieval, summarization, revision/removal), employs a layered architecture separating active memory core from passive memory-store with explicit scope control (users, agents, threads), and introduces a multi-faceted evaluation methodology.

Result: The system achieves 93.8% accuracy on LongMemEval, uses approximately 10.7x fewer tokens compared to flat-history baselines, and is benchmarked against published/reported external baselines where available.

Conclusion: Oracle Agent Memory demonstrates that a database-native approach effectively addresses the systems challenges of agent memory, yielding high accuracy with significant token efficiency. The report provides implementation-oriented appendix materials for practical deployment.
...
### [186] [Learning Safe Agent Behaviour from Human Preferences and Justifications via World Models](https://arxiv.org/abs/2607.13172)
*Ilias Kazantzidis,Timothy J. Norman,Yali Du,Christopher T. Freeman*

Main category: cs.AI

TL;DR: DROPJ uses human preferences with safety justifications, trained on simulated trajectories from a learned world model, to safely train and deploy RL agents without a reward function.


<details>
  <summary>Details</summary>
Motivation: Traditional RL is impractical for safety-critical environments where dynamics are unknown and no suitable reward function exists; human input is needed for both safe training and deployment.

Method: 1) Learn a world model from prior trajectories. 2) Human plays in the simulator to generate informative trajectories. 3) Elicit human preferences with justifications over trajectory segment pairs. 4) Train a reward model from justified preferences. 5) Deploy using model predictive control with the world model and reward model.

Result: Informative simulated trajectories reduce computational cost; preference-based feedback outperforms other feedback types; safety justifications significantly enhance safety or prioritize user-prescribed safety aspects during deployment.

Conclusion: DROPJ is an effective human-centred method for safe RL training and deployment that leverages preferences and safety justifications to achieve both performance and safety in unknown environments without a reward function.
...
### [187] [CayleyR: Solving the TopSpin puzzle via cycle intersection](https://arxiv.org/abs/2607.13219)
*Yuri Baramykov*

Main category: cs.AI

TL;DR: cayleyR is an R package that solves permutation puzzles (like TopSpin) using bidirectional search on Cayley graphs, detecting cycle intersections between random operation sequences from start and goal states, with C++ backend and optional GPU acceleration.


<details>
  <summary>Details</summary>
Motivation: To provide an efficient, accessible computational tool for solving permutation puzzles by exploiting the algebraic structure of Cayley graphs, specifically targeting the TopSpin(n,k) puzzle.

Method: Iterative bidirectional search: (1) generate random operation sequences from both initial and target states to create cycles in the Cayley graph, (2) detect intersections between these cycles to find a connecting path, (3) if no intersection found, use distance-guided bridge selection to reduce the gap, (4) repeat. Implemented in R with C++ hash-indexed state store and optional Vulkan GPU acceleration.

Result: A publicly available CRAN R package (cayleyR) that successfully solves TopSpin(n,k) permutation puzzles using the described bidirectional cycle-intersection algorithm.

Conclusion: The cayleyR package demonstrates a practical algorithmic approach combining Cayley graph theory, bidirectional search, and cycle intersection detection for solving permutation puzzles, with performance benefits from C++ and GPU acceleration.
...
### [188] [Networked Intelligence: Active Shared Context Graphs for Human-AI Team Science](https://arxiv.org/abs/2607.13220)
...
TL;DR: Mycelium is an active shared workspace that connects multiple researchers and AI agents as a multi-user co-scientist, enabling networked intelligence by capturing, tracking, and routing scientific observations and hypotheses across team members. A biological multi-omics case study shows it transforms local findings into cross-expert mechanistic insights and experimental designs.


<details>
  <summary>Details</summary>
Motivation: Challenging scientific problems are solved by diverse teams, not single reasoners. The open problem is not scaling individual models but cultivating networked intelligence—connecting humans and AI systems so that discoveries in one context reach others who can act on them.

Method: Mycelium is an active shared workspace that automatically captures important observations and hypotheses as users and agents work, tracks their relationships to the team's evolving model, and routes relevant context to the person or agent whose next decision it can inform.

Result: In a biological multi-omics campaign, routed shared context turned a local analytical finding into a cross-expert mechanistic constraint and ultimately into an experimental design. Networked intelligence is formalized as sparse conditional computation over distributed scientific contexts.

Conclusion: Standalone agents can sometimes match networked intelligence, but independent expertise and non-mergeable contexts make the network irreducible in many cases. The computational account distinguishes when each scenario applies.
...
### [189] [AI-Native Insurance for Agentic AI: Pricing, Underwriting, and End-to-End Automation](https://arxiv.org/abs/2607.13230)
*Quanyan Zhu*

Main category: cs.AI

TL;DR: This paper develops an AI-native mathematical framework for underwriting, pricing, and designing insurance contracts specifically for agentic AI deployments, addressing the unique risks posed by autonomous AI systems that can make decisions, interact with environments, and invoke third-party services.


<details>
  <summary>Details</summary>
Motivation: Agentic AI systems introduce novel insurance challenges due to their autonomous decision-making, tool invocation, and external environment interaction capabilities, for which traditional insurance frameworks are inadequate.

Method: The authors represent AI deployments as a risk state capturing autonomy level, operational authority, permission exposure, governance maturity, and dependency concentration. They map this state to event probabilities, loss severities, governance costs, and insurance contract parameters, then formulate an optimization problem under participation, profitability, and incentive compatibility constraints.

Result: The framework establishes structural properties of insurability, including an insurability region, monotone deterioration of feasibility with increasing exposure, and governance certification thresholds. A healthcare case study demonstrates contract optimization and sensitivity analysis.

Conclusion: Insurance for agentic AI can serve both as an operational cost and a regulatory mechanism, with a formal mathematical foundation for contract design that preserves incentive compatibility.
...
### [190] [Cost-Optimal Foundation Model Deployment Portfolio for Transportation Management](https://arxiv.org/abs/2607.13239)
*Xi Cheng,Ke Liu,Siyuan Feng,Jane Lin,H. Oliver Gao*

Main category: cs.AI

TL;DR: The paper introduces the Foundation Model Deployment Portfolio (FMDP) problem—a mixed-integer program for selecting which foundation models (LLMs, VLMs) to deploy for different TMC functions, under quality, latency, safety, and hardware budget constraints. It proves NP-hardness, provides a greedy heuristic, and shows in a case study a mixed portfolio reducing costs by 97% compared to all-closed-API baselines.


<details>
  <summary>Details</summary>
Motivation: Multiple foundation models are increasingly used in TMC tasks, creating a need for optimal model-to-function assignment and deployment mode selection under shared GPU/hardware constraints to minimize total cost of ownership.

Method: Formulates FMDP as a mixed-integer program, proves NP-hardness via reduction from 0-1 knapsack, and proposes a polynomial-time greedy heuristic. Evaluated via a case study with 5 TMC functions and 19 candidate (model, mode) pairs.

Result: The mixed portfolio costs $34/month, 97% lower than the cheapest all-closed-API baseline. On-premise GPU investment becomes reasonable above ~309 vision queries/hour or if API prices double.

Conclusion: Mixed portfolios (open-source + closed APIs) are highly cost-effective for TMC. Open-source models suffice for most functions except when strict quality floors cannot be met.
...
### [191] [Harness Handbook: Making Evolving Agent Harnesses Readable,Navigable, and Editable](https://arxiv.org/abs/2607.13285)
*Ruhan Wang,Yucheng Shi,Zongxia Li,Zhongzhi Li,Yue Yu,Junyao Yang,Kishan Panaganti,Haitao Mi,Dongruo Zhou,Leoweiliang*

Main category: cs.AI

TL;DR: This paper introduces the Harness Handbook, a behavior-centric representation automatically generated from AI agent harness codebases via static analysis and LLM assistance, along with Behavior-Guided Progressive Disclosure (BGPD) to help developers and coding agents localize where code changes are needed when modifying complex agent systems.


<details>
  <summary>Details</summary>
Motivation: Modifying AI agent harnesses is difficult because production harnesses are large, tightly coupled, and behaviorally distributed; developers must manually map behavioral modification requests to specific code locations across files and modules, making behavior localization a central bottleneck in harness evolution.

Method: The authors propose (1) Harness Handbook, a behavior-centric representation automatically synthesized via static analysis and LLM-assisted structuring that links each behavior to its source code, and (2) Behavior-Guided Progressive Disclosure (BGPD), which guides agents from high-level behaviors to relevant implementation details and verifies candidate locations against current source code.

Result: On diverse modification requests from two open-source harnesses, Handbook-Assisted planning improves behavior localization and edit-plan quality while using fewer planner tokens, with the largest gains on scattered sites, rarely executed paths, and cross-module interactions.

Conclusion: Evolving complex agentic systems depends not only on generating correct edits but also on determining where those edits should be made; the proposed approach addresses the behavior-to-code mapping bottleneck effectively.
...
### [192] [Theory-Level Autoformalization: From Isolated Statements to Unified Formal Knowledge Bases](https://arxiv.org/abs/2607.13292)
*Marcus J. Min,Mike He,Zhaoyu Li,Zixuan Yi,Sharad Malik,Aarti Gupta,Xujie Si,Osbert Bastani*

Main category: cs.AI

TL;DR: This position paper argues for shifting autoformalization from statement-level to theory-level, where complete theories with all their dependencies are formalized as structured libraries, and proposes three promising paths forward.


<details>
  <summary>Details</summary>
Motivation: Current autoformalization focuses on individual statements, but real formalization requires entire webs of axioms, definitions, and lemmas. The authors argue this gap needs to be addressed for meaningful progress.

Method: The paper takes a position paper approach: examining the significance of theory-level autoformalization, addressing alternative views, identifying open challenges, and proposing three promising paths forward.

Result: Identification of theory-level autoformalization as a critical but underexplored direction, along with a survey resource at https://github.com/marcusm117/Awesome-Autoformalization.

Conclusion: Theory-level autoformalization is a necessary shift for practical formalization efforts; the paper outlines challenges and future directions for this paradigm.
...
### [193] [EZSMT Version 3, Matured](https://arxiv.org/abs/2607.13344)
*Yuliya Lierler*

Main category: cs.AI

TL;DR: EZSMTV3 is a new SMT-based CASP framework that extends EZSMT+ with a richer input language, weak constraint optimization, and easier integration of new constraint types, achieving competitive performance against CLINGCON, CLINGO[DL], and CLINGO[LP] while supporting mixed integer-real constraints.


<details>
  <summary>Details</summary>
Motivation: To advance the translational approach to CASP solving by creating an extensible framework that leverages state-of-the-art SMT solvers rather than building custom search procedures, enabling easier integration of new constraint types and mixed-domain reasoning.

Method: EZSMTV3 employs a translational approach that compiles CASP problems into SMT formulas and uses existing SMT solvers (CVC5, YICES, Z3) for reasoning, while introducing a more expressive input language, weak constraint optimization support, and modular extension mechanisms.

Result: Benchmarking results show EZSMTV3 competes effectively with CLINGCON, CLINGO[DL], and CLINGO[LP], and uniquely handles mixed-domain constraints over integers and reals.

Conclusion: EZSMTV3 serves as a robust, extensible CASP framework that provides a foundation for future theoretical exploration and practical extensions in the CASP domain.
...
### [194] [Set-shifting Behavioral Test for Harnessed Agents](https://arxiv.org/abs/2607.13396)
*Ziwei Ye*

Main category: cs.AI

TL;DR: This paper studies how LLM agents adapt when a reliable tool silently changes during an ongoing session. Using a set-shifting benchmark from cognitive psychology, the authors find that agents settle into small routines after reliability shifts, exhibit distinct failure modes, and are influenced by how tool alternatives are framed.


<details>
  <summary>Details</summary>
Motivation: To understand LLM agents' adaptability to hidden reliability shifts in tool choice, borrowing from cognitive psychology's set-shifting paradigm.

Method: A benchmark with redundant tool-skill libraries where tools differ in hidden reliability. A branched schedule shifts the reliable tool group at hidden boundaries with paired no-shift controls. Set-shifting accuracy is measured as joint probability of routing to the correct tool group per post-shift window. Tested on open-weight LLMs.

Result: Agents settle into small recurring routines after reliability shifts; call shares concentrate on discrete values. Qualitatively distinct failure modes observed across same routines. Set framing (competing vs. complementary tool presentations) shifts routing dynamics.

Conclusion: LLM agents have limited adaptability to silent tool reliability shifts, and their tool choice behavior is heavily influenced by routine formation and presentation framing.
...
### [195] [LAPO: Leave-One-Turn Attribution for Self-Generated Process Rewards in Multi-Turn Search Reasoning](https://arxiv.org/abs/2607.13501)
*Qiang Zhu,Jiajun Wu*

Main category: cs.AI

TL;DR: LAPO introduces a self-generated process-supervision method for multi-turn search reasoning that uses backward leave-one-turn attribution with sign-consistency gating, achieving superior results over existing step-reward baselines without requiring additional reward models.


<details>
  <summary>Details</summary>
Motivation: Standard reinforcement learning for multi-turn search reasoning uses only terminal rewards, which cannot distinguish between useful, redundant, and harmful intermediate interactions.

Method: LAPO replaces each search turn with a [DELETE] placeholder, measures the change in the policy's mean log-likelihood of the gold answer (Answer-Likelihood Gain), and applies sign-consistency gating to retain only consistent normalized process advantages.

Result: Across seven knowledge-intensive QA datasets, LAPO achieves 0.326 exact match, outperforming IGPO by 0.053. Ablations confirm benefits from both backward attribution and sign-consistency gating.

Conclusion: Policy-derived retrospective attribution can provide effective process supervision for multi-turn search agents without needing extra reward models, teachers, verifiers, or LLM judges.
...
### [196] [How Far Can Root Cause Analysis Go on Real-World Telemetry Data?](https://arxiv.org/abs/2607.13548)
*Athira Gopal,Ashwanth Krishnan*

Main category: cs.AI

TL;DR: This paper presents a Structured Multi-Agent RCA pipeline for root cause analysis in microservice failures that outperforms existing LLM-based and classical methods. Using reverse reasoning analysis, the authors show the bottleneck is not data access but reasoning capability, and introduce automated rule mining to reduce manual knowledge curation. They conclude model-level improvements are needed as scaffolding alone cannot close the performance gap.


<details>
  <summary>Details</summary>
Motivation: Identifying root causes in production microservice failures requires reasoning over large-scale, multimodal telemetry (metrics, logs, traces), which has proven resistant to both classical and LLM-based approaches. The OpenRCA dataset exemplifies these challenges with low accuracy across all existing methods.

Method: 1) A Structured Multi-Agent RCA pipeline that supports both domain-knowledge and knowledge-free modes. 2) A reverse reasoning agent that classifies failures into "Reasoning Gap" (evidence present but unused) or "Data Ambiguity" (evidence absent). 3) An automated rule mining pipeline that extracts discrimination rules from reverse reasoning reports.

Result: The required evidence is present in the vast majority of failures; the bottleneck is reasoning capability, not data access. Stronger models embed more domain expertise, and explicit knowledge injection partially compensates. Reasoning performance remains bounded even with perfect evidence extraction.

Conclusion: Scaffold engineering and better data pipelines cannot close the gap; progress requires improvements at the model level. Model reasoning capability and domain knowledge are the primary constraints.
...
### [197] [Multi-Agent Collaborative Reasoning with Tool-Augmented Evidence for Urban Region Profiling](https://arxiv.org/abs/2607.13558)
*Xixuan Hao,Yutian Jiang,Jiabo Liu,Yihang Yang,Guangyin Jin,Song Gao,Yuxuan Liang*

Main category: cs.AI

TL;DR: UrbanAgent is a multi-agent reasoning framework that reframes urban region profiling from correlation-driven representation learning to reasoning-driven inference, achieving 8.1% average R2 improvement on global datasets.


<details>
  <summary>Details</summary>
Motivation: Existing urban profiling methods rely on correlation-driven multimodal representation learning that assumes cross-modal consistency and uses static pipelines, limiting robustness in heterogeneous or unseen urban regions.

Method: UrbanAgent instantiates independent agents for each data modality, performs structured multi-agent collaborative reasoning to address cross-modal inconsistencies, and extends indicator prediction as a closed-loop process of active evidence acquisition and iterative reasoning with tool-augmented external knowledge retrieval optimized via reinforcement learning.

Result: Consistently outperforms baselines on global datasets for Carbon emissions, GDP, and Population estimation, achieving 8.1% average R2 improvement with strong generalization in unseen-city settings.

Conclusion: The agentic, reasoning-driven approach to urban region profiling is more robust and generalizable than traditional correlation-driven multimodal representation learning methods.
...
### [198] [AI advice suppresses people's willingness to say "I don't know", even when the advice is wrong and accuracy is incentivized](https://arxiv.org/abs/2607.13562)
*Chiara Marcoccia,Walter Quattrociocchi,Valerio Capraro*

Main category: cs.AI

TL;DR: Having access to AI advice—even when it's wrong—significantly reduces people's willingness to say "I don't know," making them answer more questions but with far lower accuracy and inflated confidence.


<details>
  <summary>Details</summary>
Motivation: To investigate how the mere presence of AI advice affects people's metacognitive judgment about whether they know enough to answer, independent of the accuracy of that advice.

Method: Five experiments (N = 3,132; four preregistered, one direct replication) where participants answered difficult questions with the option to decline. AI advice was engineered to be wrong, separating AI access from its accuracy. Conditions varied AI availability, whether advice was requested or displayed, and incentive structures.

Result: AI access nearly eliminated suspension of judgment; participants answered more questions but were correct ~1/3 as often as without AI, while confidence nearly doubled. Incentives for accuracy reduced AI seeking/following and improved performance but still fell short of no-AI conditions.

Conclusion: AI suggestions may alter the metacognitive threshold for deciding whether one knows enough to answer, beyond simply affecting answer accuracy.
...
### [199] [SAFETY SENTRY: Context-Aware Human Intervention via EXECUTE-ASK-REFUSE Routing](https://arxiv.org/abs/2607.13594)
*Tianyu Chen,Chujia Hu,Wenjie Wang*

Main category: cs.AI

TL;DR: The paper proposes Safety Sentry, a lightweight guard model that reframes binary safe/unsafe classification into a three-way routing decision (EXECUTE, ASK, REFUSE) for per-instance tool call safety, using a single decoding threshold for adjustable risk tolerance across deployments.


<details>
  <summary>Details</summary>
Motivation: Existing binary guard models conflate intrinsic harmfulness with contextual appropriateness, operate at coarse action-category level, and erode user autonomy through routine interruptions, leading to alert fatigue.

Method: Proposes Safety Sentry: a lightweight guard model that performs per-instance three-way routing over {EXECUTE, ASK, REFUSE}, with inference reduced to a single decoding call and a tunable decoding-time threshold for cross-deployment risk calibration without retraining.

Result: Safety Sentry outperforms open-weight and frontier closed-source baselines on overall accuracy and safety-related recall, while simultaneously controlling both directional error rates.

Conclusion: A three-way routing framework with a tunable threshold offers a more nuanced, effective, and adaptable safeguard for LLM agents than binary classification, balancing safety with autonomy.
...
### [200] [Automatic Ordinary Differential Equations Discovery For Biological Systems Using Large Language Model Powered Agentic System](https://arxiv.org/abs/2607.13608)
*David Krongauz,Arad Zulti,Eran Segal,Teddy Lazebnik*

Main category: cs.AI

TL;DR: The paper introduces MEDA, an LLM- and symbolic-regression-powered framework that autonomously discovers ordinary differential equation models of biological and biologically inspired dynamical systems, demonstrating strong performance in model retrieval, reasoning-based extrapolation, and open-ended discovery tasks.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between narrow equation-discovery benchmarks and broad end-to-end automation pipelines, particularly addressing the underexplored domain of biological systems, by developing a system capable of autonomously generating mechanistic ODE models from data and background knowledge.

Method: MEDA combines LLM-based agents with symbolic regression to retrieve background knowledge, define admissible variables, generate mechanistic constraints, propose candidate ODEs, and fit/evaluate them across canonical model retrieval, reasoning-based extrapolation, and open-ended discovery settings.

Result: MEDA recovered correct state variables in all settings, achieved strong structural recovery in retrieval and extrapolation tasks, and produced biologically plausible discovery-oriented models. Ablation studies showed knowledge-guided formalization and mechanistic constraints are critical, while pure numerical fitting can yield trajectory-compatible but biologically incorrect equations.

Conclusion: A knowledge-guided, constraint-driven approach is essential for discovering biologically meaningful mechanistic models, as numerical fitting alone is insufficient to guarantee biological correctness.
...
### [201] [STOCKTAKE: Measuring the Gap Between Perception and Action in LLM Agents with a Fair Oracle](https://arxiv.org/abs/2607.13618)
*Sagar Deb,Ashwanth Krishnan*

Main category: cs.AI

TL;DR: STOCKTAKE is a 26-week supply-chain benchmark that separates state estimation errors from knowing-doing gaps in LLM agents, revealing that even agents detecting 84-88% of hidden failures can score below a symptom-blind baseline due to both under-response and over-response.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of LLM agents on multi-week decision tasks cannot separate why an agent failed: it may have misread the world (state estimation error) or read it correctly but failed to act appropriately (knowing-doing gap), because reference policies either use privileged information or are missing.

Method: Built as a factored partially observable Markov decision process with six hidden factor processes; computes a fair reference oracle using an exact Bayes filter per factor operating on the agent's observation stream; scores each run with a skill score (0 = symptom-blind baseline, 1 = oracle), and separately measures stated-belief detection lag and knowing-doing rate from weekly written rationales.

Result: Four LLM agents detect 84-88% of hidden failures within a week, yet skill scores range from 0.62 to -0.23; two agents score below the symptom-blind floor. 34-43% of correctly diagnosed stress weeks still end in stockout. The two worst-scoring agents stock out least on diagnosed weeks (under-response), while traces reveal over-response (costs exceeding protected value) in others.

Conclusion: The knowing-doing gap has two faces: under-response (knowing stress but failing to act) and over-response (acting at cost exceeding benefit). STOCKTAKE measures both directions of this failure, enabling separate diagnosis of state estimation versus action quality in LLM agents.
...
### [202] [UESF-Bench: Benchmarking and Probing for Unified Embodied Seeking and Following](https://arxiv.org/abs/2607.13621)
*Kun Yu,Jianhua Yang,Yixiang Chen,Changwei Wang,Hongyuan Yu,Yan Huang,Fushuo Huo,Ya Jing,Zhumin Chen,Keji He*

Main category: cs.AI

TL;DR: This paper introduces UESF-Bench, a unified benchmark for embodied agents that must both search for and follow a language-described person, and proposes SeekFollow-VLA, a vision-language-action framework with task-driven routing for handling the transition between seeking and following phases.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks assume the target person is visible from the start, overlooking the realistic need for agents to first search for a person described by language and then follow them. Current approaches treat searching and following as separate tasks without a unified evaluation benchmark.
...
Conclusion: The benchmark and method establish a baseline for unified embodied seek-and-follow, addressing the limitations of prior task-specific and separate evaluation approaches.
...
### [203] [Explaining Reinforcement Learning Agents via Inductive Logic Programming](https://arxiv.org/abs/2607.13655)
*Celeste Veronese,Edoardo Zorzi,Daniele Meli,Alessandro Farinelli*

Main category: cs.AI

TL;DR: This paper introduces objective, planning-oriented metrics (activation rate, feature coverage, syntactic/semantic distance) for quantifying explainability of logical rules extracted from RL policies via Inductive Logic Programming, validated across single and multi-agent RL domains.


<details>
  <summary>Details</summary>
Motivation: XRL relies heavily on user studies lacking shared evaluation metrics, while logic-based XAI lacks systematic quantification of explainability. This work bridges both gaps by providing principled, objective metrics.

Method: Uses Inductive Logic Programming (ILP) to extract symbolic rules from RL policies, then defines four explainability metrics: activation rate, feature coverage, syntactic distance, and semantic distance to quantify rule-policy alignment, feature roles, and policy evolution.

Result: Metrics reveal action-specific learning dynamics beyond global return, provide finer feature importance insights than classical approaches, and uncover coordination/specialization/adaptation patterns in MARL, plus insights for policy transfer.

Conclusion: The proposed metrics advance XRL with objective explainability measures and contribute to logic-based XAI by enabling principled quantification of logical rule explainability beyond simple propositional fragments.
...
### [204] [When Bots Join the Team: Bot Adoption and the Institutional Fabric of Open-Source Software Projects](https://arxiv.org/abs/2607.13679)
*Yongren Shi,Wenyi Gong*

Main category: cs.AI

TL;DR: This study analyzes 2,991 GitHub projects before and after bot adoption, finding that bots joining human teams are associated with strengthened group organization—more repeated collaboration, greater social memory, fewer conflict cascades, and more distinctive outputs—suggesting bots can become part of a community's social infrastructure rather than disrupting coordination.


<details>
  <summary>Details</summary>
Motivation: To investigate whether group organization strengthens or weakens when automated agents become regular participants in human teams, using open-source software as a natural laboratory.

Method: Analyzed 2,991 GitHub projects two years before and after each adopted its first bot, measuring three coordination capabilities (repeated engagement, social memory, role differentiation) and two outcomes (conflict cascades, output distinctiveness), treating bots as participants rather than tools.

Result: Bot adoption is followed by more repeated collaboration, greater recognition of specific bots, fewer conflict cascades, and more distinctive outputs. Changes cluster around adoption rather than accumulating gradually. Human-side capabilities account for the bot-conflict association but not the bot-distinctiveness association.

Conclusion: Predictable, rule-based agents can become part of a community's social infrastructure. The findings are consistent with the interpretation that the bot serves as an occasion, while social organization is the underlying mechanism driving observed changes.
...
### [205] [AgentCompass: A Unified Evaluation Infrastructure for Agent Capabilities](https://arxiv.org/abs/2607.13705)
...
TL;DR: AgentCompass is an open-source, lightweight, and extensible infrastructure for evaluating LLM-based agents, featuring three independent components (Benchmark, Harness, Environment), fault-tolerant asynchronous runtime, and trajectory analysis tools, supporting 20+ benchmarks across five capability dimensions.


<details>
  <summary>Details</summary>
Motivation: Current evaluation pipelines for LLM agents are fragmented and tightly coupled, hindering reproducibility and causing redundant engineering efforts. A unified, flexible evaluation infrastructure is needed.

Method: AgentCompass organizes evaluation around three independent components (Benchmark, Harness, Environment) for flexible configuration, includes a fault-tolerant asynchronous runtime and comprehensive trajectory analysis tools to diagnose failure modes like reward-hacking.
...
Conclusion: AgentCompass offers the community a scalable, reproducible, and unified infrastructure to advance LLM-based agent research.
...
### [206] [CAVA: Canonical Action Verification and Attestation for Runtime Governance of Agentic AI Systems](https://arxiv.org/abs/2607.13716)
*Zexun Wang*

Main category: cs.AI

TL;DR: CAVA is a runtime-semantics layer that converts heterogeneous agent activity into canonical runtime action objects, enabling verifiable governance of AI actions across different runtime environments.


<details>
  <summary>Details</summary>
Motivation: Heterogeneous AI runtimes (coding hooks, SDKs, browser automation, API gateways, workflow engines) produce incompatible records for the same operational act, making it difficult to answer what action was approved, what evidence binds approval to execution, and whether an independent verifier can reproduce the same action identity later.

Method: CAVA formalizes canonical action identity, semantic pattern detection, approval binding, receipt integrity, runtime-portable projection, and optional attestation substrates. It sits below Proof-Carrying Agent Actions (PCAA), providing the stable action object that PCAA's route-review-prove governance process governs.

Result: Evaluated through a 96-seed, 384-variant benchmark covering semantic equivalence, semantic separation, wrapper bypass, false-positive control, approval binding, receipt reproducibility, attestation tamper detection, runtime portability, semantic pattern detection, policy degradation, and Azure deployment drills.

Conclusion: Action-level canonicalization and policy-addressable semantic patterns are a necessary substrate for deployer-side AI governance.
...
### [207] [Experience Memory Graph: One-Shot Error Correction for Agents](https://arxiv.org/abs/2607.13884)
*Wenjun Wang,Yuchen Fang,Fengrui Liu,Zibo Liang,Kai Zheng*

Main category: cs.AI

TL;DR: EMG reformulates agent failure recovery as graph matching, converting trajectories into directed action graphs, extracting correction patterns, and enabling single-pass execution without test-time trial-and-error.


<details>
  <summary>Details</summary>
Motivation: LLM agents suffer from compounding errors in long-horizon tasks. Existing self-correction via prompt-based reflection is brittle, costly, and produces task-specific memory that generalizes poorly.

Method: EMG converts failed and successful trajectories into directed action decision graphs, matches them to extract common subgraphs and graph edit paths (add/delete/relabel actions), stores them in a memory graph with intra-task nodes and cross-task edges, then retrieves insights at test time for single, loop-free execution.

Result: Outperforms SOTA reflection baselines on ALFWorld and ScienceWorld in success rate and average reward, with no test-time trial-and-error.

Conclusion: Graph matching is an effective alternative to prompt-based reflection for failure recovery in LLM agents, enabling efficient, generalizable, single-pass correction.
...
### [208] [AIMO Interpretability Challenge](https://arxiv.org/abs/2607.13899)
...
TL;DR: The AIMO Interpretability Challenge is a competition designed to distinguish robust from spurious reasoning in frontier math language models by analyzing their internal mechanisms, using olympiad-level problems, symbolic representations, and model access to create a lasting benchmark for interpretability and generalization.


<details>
  <summary>Details</summary>
Motivation: Standard reasoning benchmarks only evaluate final-answer accuracy, which fails to distinguish between stable, generalizable reasoning and brittle, shortcut-based reasoning. This gap motivates the need for interpretability-driven evaluation.

Method: The competition provides olympiad-level math problems with symbolic representations for generating functional variants, access to frontier reasoning models, adversarial robustness assessments, and computing infrastructure. Participants develop methods to identify which models solve problems robustly by examining internal mechanisms.

Result: The competition will produce an open robustness benchmark and baseline systems for mathematical reasoning and interpretability, along with assessments of models' adversarial robustness on these problems.

Conclusion: This challenge connects interpretability and generalization research to answer whether frontier AI models' decision-making is generalizable and reliable, providing a lasting foundation for standard benchmarking.
...
### [209] [A Self-Evolving Agent for Longitudinal Personal Health Management](https://arxiv.org/abs/2607.13940)
...
TL;DR: HealthClaw is an open-source agent architecture for longitudinal personal health management that uses separated safety rules, medical knowledge, and private memory with inductive updates. It achieves 45.7% answer accuracy (vs 0.2% baseline) with 71.7% less context exposure and 27.0 percentage point gains across biomedical tasks.


<details>
  <summary>Details</summary>
Motivation: Personal health management involves repeated interactions over time, yet most health AI systems treat each request in isolation, ignoring the evolving nature of a person's routines, preferences, measurements, and risks.

Method: HealthClaw separates shared safety rules and medical knowledge from private longitudinal memory (profile facts, reusable procedures, episodic traces). After each episode, induction determines what should update the profile, revise a procedure, remain episodic, or be excluded.

Result: Across 900 longitudinal probes: accuracy rose from 0.2% (current-query prompting) to 45.7% (HealthClaw). Context exposure was 71.7% lower than full-history prompting. In 100 privacy probes: higher privacy-aware quality with fewer unsafe disclosures. Across 9 biomedical tasks: mean absolute gain of 27.0 percentage points in primary metrics; 7 gains significant after FDR correction.
...
### [210] [Do Agent Optimizers Compound? A Continual-Learning Evaluation on Terminal-Bench 2.0](https://arxiv.org/abs/2607.14004)
*Wenxiao Wang,Priyatham Kattakinda,Soheil Feizi*

Main category: cs.AI

TL;DR: This paper investigates whether agent-optimization gains compound over time across multiple rounds of optimization. Testing three methods (GEPA, Meta Harness, RELAI-VCL) on Terminal-Bench 2.0, they find that only RELAI-VCL, which incorporates regression control, achieves positive transfer and continued improvement across sequential optimization phases, with the highest lifelong average pass rate (76.4%).


<details>
  <summary>Details</summary>
Motivation: Current agent-optimization methods are evaluated as one-shot improvements, ignoring the recursive optimization scenario that deployed agents face. The key question is whether optimization gains can compound across successive rounds without eroding previous improvements.

Method: A two-phase continual-learning evaluation using hard tasks from Terminal-Bench 2.0. Three approaches (GEPA, Meta Harness, RELAI-VCL) are compared under identical optimization budgets across sequential optimization rounds.

Result: All three methods improve over baseline in static one-shot setting. However, in continual setting: GEPA transfers below baseline, Meta Harness transfers well but cannot improve further, and only RELAI-VCL both transfers positively and continues improving across rounds, achieving 76.4% lifelong average pass rate.
...
### [211] [AI-accelerated End-to-End Framework for Rapid Professional Upskilling](https://arxiv.org/abs/2607.14044)
*Tam Nguyen,Hung Nguyen,Robert Ogburn*

Main category: cs.AI

TL;DR: An end-to-end AI-accelerated framework for enterprise upskilling was validated through NASBA approval, fast exam passes (3 learners passed NVIDIA Agentic AI certification), and production of a 1,267-item risk dataset.


<details>
  <summary>Details</summary>
Motivation: Growing skills gap (59/100 workers need reskilling by 2030) and worsening time-to-close skills gaps (3→36 days, 2014-2018), with existing frameworks being too narrow and lacking industry validation.

Method: End-to-end framework applying AI acceleration across five stages: knowledge acquisition, content development, content review/verification, teaching, and assessment development, optimizing both production and learning efficiency.

Result: (1) NASBA approved the program for CPE credits; (2) 3 learners passed NVIDIA Certified Professional in Agentic AI exam quickly (14 more in progress); (3) Generated a 1,267-item risk dataset for multi-agent AI systems.

Conclusion: The AI-accelerated framework is both pedagogically effective (fast certification) and industrially credible (NASBA approval), while also enabling complex downstream analysis (risk dataset production).
...
### [212] [Earthquaker-AI: A Retrieval-Augmented Generation Framework with Rubric-Based Assessment for Primary School Earthquake Education](https://arxiv.org/abs/2607.14046)
*Xanthi Kokkinou,Chaido Mizeli,Nafsika Koulaxidou,Marina Delianidi,Konstantinos Diamantaras*

Main category: cs.AI

TL;DR: Earthquaker-AI is a hybrid educational framework that combines Lego WeDo2 robotics with a Retrieval-Augmented Generation conversational AI assistant to teach earthquake preparedness to primary-school students, progressing from basic recognition to verbal production across grade levels.


<details>
  <summary>Details</summary>
Motivation: To enhance earthquake preparedness and conscious action among primary-school students by extending a mechanical STEM robotics project (Earthquaker) into cognitive and metacognitive learning through AI integration.

Method: The system integrates Lego WeDo2 robotics (simulating seismic response with sensors/actuators) with a RAG-based conversational AI assistant that provides rubric-based verbal feedback across three progressive learning stages: basic recognition (2D rubric), action sequences (3D rubric), and verbal production (4D rubric including clarity of expression).

Result: Experimental evaluation shows high groundedness and accuracy with a low hallucination rate in the dialogic module.

Conclusion: Combining robotics, rubrics, and AI promotes technological literacy, self-regulation, and responsible use of digital systems, contributing to early crisis-management skills in primary education.
...
### [213] [Deep Interaction: An Efficient Human-AI Interaction Method for Large Reasoning Models](https://arxiv.org/abs/2607.14049)
*Hefeng Zhou,Jinxuan Zhang,Jiong Lou,Yuxin Liu,Chaochao Lu,Jingjing Qu,Jie Li*

Main category: cs.AI

TL;DR: Deep Interaction enables direct editing of erroneous CoT reasoning steps in LLMs, achieving 25% better correction success and 40% less token usage on STEM tasks.


<details>
  <summary>Details</summary>
Motivation: Current interaction methods for correcting LLM reasoning errors are inefficient—they either regenerate entirely new responses (risking repeated mistakes) or require users to flag errors in follow-up turns, often leading to superficial acknowledgment without real correction.

Method: Deep Interaction allows direct editing of the original response to fix erroneous parts while preserving correct reasoning steps. The edited CoT is refined into a distilled prompt that guides the LLM along the corrected reasoning path.

Result: Over 25% improvement in correction success rate and approximately 40% reduction in token usage on STEM tasks compared to baseline approaches.

Conclusion: Deep Interaction offers a more efficient and precise human intervention mechanism for correcting LLM reasoning errors, with significant gains in both effectiveness and efficiency.
...
### [214] [Autonomous UAV Route Planning for Coverage Maximization in Environmental Monitoring: A Systematic Literature Review](https://arxiv.org/abs/2607.13054)
*Sebastian Jouannet-Contreras,Carola Figueroa-Flores*

Main category: cs.RO

TL;DR: This paper presents the protocol and preliminary results of a systematic literature review on autonomous UAV route planning for coverage-oriented environmental monitoring, following PRISMA 2020. From 401 unique records, 247 studies were retained. Early findings show strong focus on coverage, multi-UAV coordination, and energy optimization, but gaps in weather, uncertainty, and obstacle handling remain, with most validation done in simulation.


<details>
  <summary>Details</summary>
Motivation: Environmental monitoring with UAVs requires effective route planning that balances coverage maximization with energy limits, operational constraints, and geometric complexity. The fragmented research landscape lacks a structured synthesis to identify mature techniques and gaps for realistic missions.

Method: The authors conduct a systematic literature review following the PRISMA 2020 framework, searching Scopus and Web of Science for studies (2015–2026). The protocol focuses on path planning, coverage path planning, and informative path planning, analyzing algorithmic families, coverage/energy metrics, obstacle handling, geometric representations, and environmental constraints.

Result: 562 records identified → 161 duplicates removed → 401 unique records screened → 247 studies retained (235 eligible + 12 borderline). Preliminary analysis shows strong emphasis on coverage-oriented formulations, multi-UAV coordination, and energy-aware optimization. Few studies address weather, uncertainty, or obstacle-rich environments. Most rely on simulation-based validation, with growing interest in reinforcement learning, hybrid optimization, and geometry-aware planning.

Conclusion: The research landscape is active but fragmented. There is a clear simulation-to-reality gap and under-explored areas (weather, uncertainty, obstacles). A structured synthesis is needed to identify mature techniques and unresolved gaps for realistic environmental monitoring missions.
...
### [215] [HRIBench: Benchmarking Interaction-Centric Human-Robot Collaboration](https://arxiv.org/abs/2607.13056)
*Chang Liu,Jiawei Zhang,Tao Zhang,Ye Wang,Hongyu Zhou,Qin Jin*

Main category: cs.RO

TL;DR: HRIBench is a new benchmark for evaluating human-robot collaboration, designed to measure intent understanding, temporal coordination, and safe interaction beyond isolated manipulation skills. It introduces structured scenario scripts with defined roles (Instructor, Collaborator, Intruder), 13 tasks, 650+ episodes, and interpretable metrics (synchronization, responsiveness, protocol compliance, safety). Evaluations show current robot policies struggle with collaboration, but fine-tuning on HRIBench improves performance and transfers to real-world tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA benchmarks focus on isolated manipulation skills and ignore the structured, shared-agency nature of real-world human-robot collaboration, which requires intent understanding, temporal synchronization, protocol adherence, and safe interaction.

Method: HRIBench models collaborative tasks as structured scenario scripts with explicit agent roles, temporal dependencies, coordination constraints, and human behavior distributions. It defines three roles (Instructor, Collaborator, Intruder) covering intent communication, joint coordination, and robustness. It evaluates policies using interpretable metrics beyond binary success: synchronization, responsiveness, protocol compliance, and safety.

Result: Evaluated GR00T, pi0.5, and ACT policies under a unified protocol. Current foundation robot policies struggle substantially in collaborative settings despite strong manipulation ability. Fine-tuning on HRIBench consistently improves collaborative performance. In real-world adaptation, simulation data from HRIBench improved GR00T N1.5's physical-task success rate from 0.10 to 0.43.

Conclusion: HRIBench reveals major limitations in temporal coordination and intent-aware behavior in current VLA models, and demonstrates that structured collaborative benchmarks can drive meaningful improvements in real-world human-robot interaction.
...
### [216] [Power from Potential: A Survey of Electrostatic Actuators for Haptics](https://arxiv.org/abs/2607.13058)
*Ahad M. Rauf,Ran Zhou,Eric Acome,Madeline Balaam,Sean Follmer,Teng Han,Craig Shultz,Daniel Leithinger*

Main category: cs.RO

TL;DR: This paper reviews four classes of High-Voltage Electrostatic Actuators (HVEAs) for haptics—switchable adhesives, dielectric elastomers, electrohydraulic actuators, and electrokinetic pumps—highlighting their mechanisms, performance trade-offs, and potential for soft, silent, and low-power haptic feedback in wearables.


<details>
  <summary>Details</summary>
Motivation: Conventional haptic actuators (motors, temperature-responsive polymers) are bulky and power-hungry, failing to meet the needs of soft, thin, silent, and energy-efficient wearable interfaces. HVEAs offer a promising alternative.

Method: A focused cross-technology review of four HVEA classes, comparing their mechanisms (electrostatic force generation), bandwidth, force density, spatial scalability, and suitability for cutaneous vs. kinesthetic feedback.

Result: Identified common design constraints (e.g., high-voltage driving, fabrication complexity) and emerging strategies (improved ergonomics, streamlined fabrication, self-sensing integration).

Conclusion: HVEAs are uniquely positioned to advance haptic interaction in wearable and world-grounded interfaces, but further research is needed to translate them into practical, everyday systems.
...
### [217] [GPUSimBench: Towards Scalable and Reliable GPU-Accelerated Simulators in Embodied AI](https://arxiv.org/abs/2607.13059)
*Huzhenyu Zhang,Shenghai Yuan,Wenrui Yan,Li Ma,Hengjie Li,Jingcheng Pang,Dmitry Yudin*

Main category: cs.RO

TL;DR: GPUSimBench reveals hidden trade-offs in GPU-accelerated robotic simulators: it benchmarks physical fidelity, parallel scalability, and exposes significant GPU-induced non-determinism with four regimes of stochasticity that compromise reproducibility at scale.


<details>
  <summary>Details</summary>
Motivation: As embodied AI scales training via massively parallel simulation, the trade-offs between parallel efficiency, physical fidelity, and execution determinism remain unexamined, hindering reliable robot learning.

Method: Introduces GPUSimBench, which: (1) evaluates physical grounding via an inclined-plane task comparing simulated vs. real dynamics; (2) benchmarks parallel scalability (throughput, memory) across environment counts; (3) quantifies run-to-run and inter-environment variability from GPU-batched execution.

Result: Identifies four empirical regimes of stochasticity in current simulator stacks, showing unbounded scaling compromises reproducibility without explicit constraints.

Conclusion: GPU-based simulators have hidden non-determinism issues that limit reproducibility; scaling must be constrained to maintain reliable robot learning.
...
### [218] [A Bayesian framework for the uncanny valley in humanoid robot design](https://arxiv.org/abs/2607.13060)
*Shimon Honda,Rin Shibano,Hideyoshi Yanagisawa*

Main category: cs.RO

TL;DR: The paper proposes a hierarchical Bayesian generative model that operationalizes uncanny valley heuristics as mathematical design variables, using posterior-weighted negative category-conditional surprise to explain affinity reductions through category ambiguity and perceptual mismatch, validated through simulations and a human-subject experiment.


<details>
  <summary>Details</summary>
Motivation: Existing uncanny valley guidelines (e.g., avoiding excessive realism) are qualitative and not expressed as manipulable variables, making them difficult to use for algorithmic design of humanoid robots.

Method: A hierarchical Bayesian generative model is proposed, representing affinity as posterior-weighted negative category-conditional surprise. The model maps uncanny-valley mechanisms onto four variables: deviation from predicted robot-category mean, inconsistency in human likeness across modalities, prediction uncertainty, and observational uncertainty. Validated via simulations and a human-subject experiment with robot-human morphing images.

Result: Simulations showed category ambiguity and appearance-motion mismatch reduce affinity, and uncertainty reshapes the valley. The experiment showed increased observational uncertainty attenuated familiarity decrease at intermediate human likeness, while low prediction uncertainty increased ratings for robot-like appearances.

Conclusion: This framework turns empirical uncanny-valley heuristics into a computational basis for algorithmically evaluating and optimizing humanoid robot appearance and behavior.
...
### [219] [A 3DGS-Driven Dynamic Viewpoint and Vibrotactile Framework for Subsea Teleoperation Validated via fNIRS](https://arxiv.org/abs/2607.13067)
*Fang Xu,Tianyu Zhou,Ruitong Tian,Md Jahidul Islam,Jing Du*

Main category: cs.RO

TL;DR: The paper presents a multimodal teleoperation architecture for ROVs that combines a Dynamic Adaptive Viewpoint System (DAVS) using 3D Gaussian Splatting for occlusion-free exocentric views with a vibrotactile haptic suit for proximity feedback. A human-subject study (N=30) showed that haptic feedback improves path adherence under low latency, while the exocentric DAVS system provides superior resilience under severe latency (0.5-1.0s), with fNIRS revealing reduced cognitive overload compared to conventional egocentric teleoperation.


<details>
  <summary>Details</summary>
Motivation: To overcome the fundamental limitations of ROV teleoperation in flooded, cluttered infrastructure caused by narrow 2D egocentric views and subsea communication latency, which overload operator working memory and degrade performance.

Method: A ROS-Unity framework decoupling proactive spatial planning from reactive boundary avoidance, featuring (1) DAVS with continuous optimization and real-time 3D Gaussian Splatting for occlusion-free exocentric views, and (2) a torso-mounted vibrotactile suit mapping local obstacle clearance to haptic cues. Evaluated via 3x4 repeated-measures design (N=30) comparing Egocentric, Haptic, Exocentric modalities under 0.0-1.0s communication delays, with fNIRS measuring prefrontal activation.

Result: Reactive haptic feedback improves path adherence under minimal delay; 3DGS-driven exocentric visualization provides superior resilience under severe latency (0.5-1.0s). fNIRS showed that increasing latency in conventional egocentric teleoperation overloaded working memory and reduced prefrontal activation, while DAVS sustained executive control.

Conclusion: Spatially grounded, multimodal assistance (combining exocentric DAVS views and haptic feedback) can substantially improve operator performance and cognitive endurance during latency-degraded underwater teleoperation.
...
### [220] [HRO: Hierarchical Room-to-Object Framework for Zero-Shot Object Goal Navigation with Large Language Models](https://arxiv.org/abs/2607.13072)
*Luyuan Jia,Yinfeng Yu*

Main category: cs.RO

TL;DR: Proposes an LLM-driven hierarchical room-to-object (HRO) framework for zero-shot object-goal navigation, enabling coarse-to-fine exploration that outperforms existing LLM-based methods on Gibson and HM3D datasets.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot object-goal navigation methods using LLMs treat them as flat reasoning tools, lacking hierarchical spatial cognition of room-to-object semantics, leading to blind exploration and poor semantic association accuracy.

Method: An LLM-driven hierarchical room-to-object (HRO) framework that guides the agent in a coarse-to-fine manner—first reasoning about relevant rooms, then localizing target objects within those rooms.

Result: Superior success rate and generalization over existing LLM-based methods on Gibson and HM3D datasets.

Conclusion: Hierarchical reasoning with LLMs significantly improves zero-shot object-goal navigation by mimicking human-like spatial cognition, and LLMs have strong potential for this task.
...
### [221] [Worlds in One Demo: A Synthetic Data Engine for Learning Open-World Mobile Manipulation](https://arxiv.org/abs/2607.13154)
*Lingxiao Guo,Huanyu Li,Guanya Shi*

Main category: cs.RO

TL;DR: WANDA generates large-scale synthetic training data from a single real-world demonstration for open-world mobile manipulation, using Gaussian splatting reconstruction, whole-body motion planning, and corrective state expansion to achieve spatial, long-horizon, and cross-environment generalization.


<details>
  <summary>Details</summary>
Motivation: Current data collection paradigms (teleoperation, UMI) require prohibitive human effort and cost to scale, limiting the ability to train policies that generalize spatially, across long horizons, and to new environments.

Method: (1) Reconstruct background Gaussian splats and robot-object interaction trajectories from a single RGBD demo; (2) Rearrange contact-rich interaction segments into new spatial configurations using whole-body motion planning; (3) Apply Corrective State Expansion to increase state diversity for long-horizon robustness; (4) Synthesize trajectories on diverse generated 3D worlds from everyday photos; (5) Composite rendered robot/object meshes with Gaussian splatting backgrounds for photo-realistic observations.

Result: Policies trained with WANDA achieve long-horizon robustness, broad spatial generalization, and cross-environment generalization from a single real demonstration. It also supports cross-embodiment data generation, validated by zero-shot deployment on a differently shaped mobile manipulator.

Conclusion: WANDA effectively maximizes the value of each human demonstration by scaling synthetic data generation, overcoming the scalability bottleneck of manual data collection for open-world mobile manipulation.
...
### [222] [Attitude Estimation Using Inertial and Barometric Measurements](https://arxiv.org/abs/2607.13254)
*Melone Nyoba Tchonkeu,Soulaimane Berkane,Tarek Hamel*

Main category: cs.RO

TL;DR: This paper introduces barometer-aided attitude estimation for autonomous vehicles using nonlinear observers on SO(3), validated with simulated and real flight data.


<details>
  <summary>Details</summary>
Motivation: Accurate attitude estimation is challenging in GNSS-denied and highly accelerated flight conditions, where IMUs alone cannot distinguish gravitational from inertial accelerations, and auxiliary velocity sensors may be unavailable or costly.

Method: Two approaches: (1) a deterministic Riccati observer cascaded with a complementary filter ensuring AGAS; (2) a nonlinear observer on SO(3)×R² integrating IMU, barometer, and magnetometer measurements, guaranteeing LES under relaxed conditions.

Result: Validated on simulated and real flight data; barometer-aided estimation provides lightweight, reliable, and effective complementary sensing for attitude estimation.

Conclusion: Barometer-aided attitude estimation offers a practical alternative for minimal-sensing configurations when conventional velocity measurements are unavailable or degraded.
...
### [223] [Parsimonious disturbance-aware minimum-time planning with parametric uncertainty](https://arxiv.org/abs/2607.13312)
*Martino Gulisano,Matteo Masoni,Marco Gabiccini*

Main category: cs.RO

TL;DR: This paper presents a robust minimum-lap-time planning framework for motorsport that handles both state disturbances and parameter uncertainty, using a selective activation strategy for computational efficiency, and demonstrates through 1000 MPC-driven simulations on an FSAE car that robust references significantly reduce failures and improve trackability vs. nominal planning.


<details>
  <summary>Details</summary>
Motivation: Existing minimum-lap-time planning lacks robustness against state disturbances AND parameter uncertainty (e.g., inertia, center-of-mass, drag). The authors aim to extend a prior disturbance-aware framework to handle both sources of uncertainty while keeping computation tractable.

Method: Extends a disturbance-aware MLTP by incorporating parameter uncertainty (moment of inertia, CoM position, drag coefficient) into stochastic propagation. Uses a spatially selective, parsimonious activation strategy to apply robust constraints only where critical. Validated via an MPC virtual test driver over 1000 runs with random disturbances and parameter scatter.

Result: Robust references yield consistently fewer failed runs than nominal references, with tighter dispersion of vehicle inputs and axle saturations around reference values, at a moderate sector-time cost. This demonstrates improved trackability and driveability.

Conclusion: Embedding robustness against both state and parameter uncertainty in MLTP significantly enhances reliability and trackability for motorsport applications, and the selective activation strategy makes it computationally feasible.
...
### [224] [Adapting Generalist Vehicle Models for High-Speed MPC Across Terrains](https://arxiv.org/abs/2607.13319)
*Rwik Rana,Jesse Quattrociocchi,Christian Ellis,Nathan Tsoi,Garrett Warnell,Joydeep Biswas*

Main category: cs.RO

TL;DR: OptCar is a recipe for efficiently specializing generalist forward kinodynamic prediction models to specific off-road vehicles using minimal real-world data (5 minutes per terrain) and targeted synthetic rollouts, achieving ~55% trajectory tracking error reduction at high speeds on slip-diverse terrains while maintaining cross-terrain generalization.


<details>
  <summary>Details</summary>
Motivation: Generalist FKD prediction models require substantial real-world data to specialize to target platforms, and adapted models can overfit to specific terrains or driving regimes, losing cross-terrain generalization. A method is needed to bridge the generalist-to-specialist gap efficiently.

Method: OptCar introduces: (1) a history-conditioned dynamics adaptation module that encodes recent state-action observations into a dynamics context token, (2) fine-tuning the generalist model using limited real-world data combined with targeted synthetic rollouts from environment-specific system identification.

Result: At 6 m/s (highest speed evaluated), OptCar reduces trajectory tracking error by ~55% on vegetation and dirt compared to fine-tuned AnyCar baseline. With only 5 minutes of real data per terrain, it matches a specialist trained on 30 minutes of road data, and substantially outperforms it on changed terrains. It also remains robust to unseen cart-payload dynamics changes.

Conclusion: OptCar effectively bridges the generalist-to-specialist gap for FKD models, preserving cross-terrain generalization while optimizing performance for a specific vehicle using minimal real-world data.
...
### [225] [Design and Characterization of a Limb Encircling Actuator](https://arxiv.org/abs/2607.13337)
*Japmanjeet Singh Gill,Gray Cortright Thomas,Nikko Van Crey,Roberto Leo Medrano,Elliott J. Rouse*

Main category: cs.RO

TL;DR: This paper presents a Limb-Encircling Actuator (LEA) for exoskeletons that wraps around the limb to reduce spatial footprint, achieving 7.5 Nm/kg torque density but facing challenges in keeping the system contained close to the body.


<details>
  <summary>Details</summary>
Motivation: Current lower-limb exoskeletons place actuation components lateral to the legs or at the waist/back, extending beyond the body's natural envelope and making devices intrusive for everyday use, potentially limiting societal adoption.

Method: The authors designed a Limb-Encircling Actuator (LEA) with actuation geometry that encircles the limb in a plane orthogonal to the limb axis, featuring a novel radial bearing layout as a lightweight alternative to traditional bearings. They characterized its electromechanical properties using a custom-built testbed.

Result: The LEA achieved a continuous torque density of 7.5 Nm/kg with a mass of 894 g. However, despite high torque density and innovative layout, the system remained difficult to contain close to the body.

Conclusion: Limb-encircling actuation offers opportunities for torque-dense exoskeletons that could integrate into everyday apparel, but challenges in actuator sizing and geometry must be overcome before practical adoption.
...
### [226] [Safe Overtaking for Autonomous Racing Using Hierarchical Optimization and Learning-Based Control](https://arxiv.org/abs/2607.13348)
*Hassan Jardali,Kai Yin,Lantao Liu*

Main category: cs.RO

TL;DR: A hierarchical overtaking framework combining MIQP for maneuver selection and adaptive-CBF MPC for safe trajectory control, using RL to modulate safety margins online, achieving robust performance across diverse tracks without manual tuning.


<details>
  <summary>Details</summary>
Motivation: Fixed-decay discrete-time CBFs in MPC-based autonomous racing overtaking are overly conservative, limiting performance and requiring manual tuning across different track conditions.

Method: Proposes a hierarchical framework: (1) high-level MIQP for combinatorial passing-side selection, (2) nonlinear Frenet-frame MPC with embedded CBF constraints, (3) RL policy to adapt the CBF decay parameter online.

Result: No single fixed decay parameter achieves uniformly strong performance; the adaptive strategy attains the highest aggregate success rate and consistently strong safety-performance trade-offs without per-track tuning.

Conclusion: The adaptive CBF-MPC framework improves robustness to environment variation while maintaining safety constraint satisfaction in nominal operation, removing the need for manual per-track tuning.
...
### [227] [A Hybrid Sampling-Based Trajectory Planner with Game-Theoretic Guidance for Autonomous Racing](https://arxiv.org/abs/2607.13354)
*Alexander Langmann,Frederico Pita de Araujo,Mattia Piccinini,Johannes Betz*

Main category: cs.RO

TL;DR: A hybrid architecture integrating game-theoretic reasoning into sampling-based motion planners for autonomous racing, using offline-learned potential functions and online gradient-based optimization to produce interaction-aware trajectories with defensive behaviors like blocking, without the computational cost of full dynamic game solvers.


<details>
  <summary>Details</summary>
Motivation: Autonomous racing requires planners that balance vehicle dynamics limits with strategic multi-agent decision-making, but solving full dynamic games online is computationally prohibitive and hard to integrate into robust, high-frequency autonomous software stacks.

Method: Proposes a hybrid architecture combining game-theoretic reasoning with sampling-based motion planning: uses an offline-learned potential function (α-potential game formulation) to capture multi-agent interactions, then online gradient-based optimization refines interaction parameters to generate an Interaction Reference Path, which serves as a dynamic cost bias within a high-frequency sampling planner.

Result: Evaluated in high-fidelity simulation on Yas Marina Circuit. Qualitative and quantitative results show the approach successfully induces defensive behaviors like blocking without the computational burden of full dynamic game solvers.

Conclusion: The hybrid architecture effectively balances strategic multi-agent interaction and robust trajectory generation for autonomous racing, enabling computationally efficient game-theoretic behaviors in high-frequency planning.
...
### [228] [Min-Max Regret Task Allocation and Planning of Heterogeneous Multi-Robot System in Partially Known Environments](https://arxiv.org/abs/2607.13403)
*Xinkai Liang,Huixuan Chan,Ying Liu,Yangxi Shi,Hao Fang*

Main category: cs.RO

TL;DR: This paper presents a robust planning framework for large-scale heterogeneous multi-robot systems that handles complex temporal logic tasks in partially known environments. It uses a min-max regret optimization with Region-Binding Atomic Proposition (RbAP) and Extended Planning Decision Tree (E-PDT) with Regret-based Branch-and-Bound strategy, achieving near-linear scalability.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for task allocation in HMRS struggle with balancing exploration and exploitation when dealing with temporal logic tasks in partially known environments, and suffer from exponential computational complexity that limits scalability.

Method: The authors formulate the problem as min-max regret optimization, introduce Region-Binding Atomic Proposition (RbAP) to capture resource uncertainty within automaton structure, and propose Extended Planning Decision Tree (E-PDT) with a Regret-based Branch-and-Bound (BnB) strategy that dynamically prunes suboptimal policies.

Result: The framework achieves near-linear scalability with respect to number of robots and types, significantly outperforming MILP-based baselines in both solution quality and computational efficiency, as demonstrated by numerical and physical experiments.

Conclusion: The proposed approach provides a scalable and robust solution for HMRS task allocation in partially known environments, effectively balancing exploration and exploitation while maintaining theoretical feasibility and completeness.
...
### [229] [WNOJ-LIO: A White-Noise-on-Jerk Motion-Prior EKF for High-Dynamic LiDAR-IMU Fusion](https://arxiv.org/abs/2607.13405)
*Junning Lyu,Qizhi Guo,Xia Ning,Tao Song,Shaoming He*

Main category: cs.RO

TL;DR: WNOJ-LIO is a LiDAR-inertial odometry framework using a White-Noise-on-Jerk EKF that decouples state prediction from IMU measurements, treating IMU as a measurement source rather than propagation driver, improving accuracy under high-dynamic driving (up to 208 km/h).


<details>
  <summary>Details</summary>
Motivation: High-dynamic driving causes intra-scan motion distortion and vibration-contaminated IMU measurements. Existing LIO pipelines propagate state by integrating raw IMU data, which propagates inertial noise into both scan de-distortion and scan-to-map registration.

Method: WNOJ-LIO employs a decoupled WNOJ prior on R^3 × SO(3) for state prediction, treating IMU as a high-frequency measurement source. It uses closed-form covariance propagation via a Gaussian process trajectory prior integrated with recursive EKF filtering.

Result: Simulation shows improvements in acceleration/angular-velocity denoising, scan de-distortion, and localization accuracy over a FAST-LIO baseline. Real-world racing experiments at 53-208 km/h validate the method across varying vibration levels.

Conclusion: The decoupled WNOJ prior effectively bridges batch GP trajectory priors and recursive filtering, yielding superior state estimation under highly dynamic driving conditions. Code is publicly available.
...
### [230] [Ego-Dynamics-Augmented World Model for Autonomous Driving with Zero-Shot Cross-Chassis Adaptation](https://arxiv.org/abs/2607.13410)
*Zhidong Wang,Jingsong Liang,Zirui Li,Zhan Chen,Han Yu,Chen Lv*

Main category: cs.RO

TL;DR: DynaDreamer improves world model-based autonomous driving RL by adding an explicit ego-dynamics prior, which disentangles ego-motion from scene dynamics, leading to 28-73% better task success rates and zero-shot cross-chassis adaptation.


<details>
  <summary>Details</summary>
Motivation: Existing driving world models use egocentric BEV representations that entangle ego-motion with scene dynamics, forcing the WM to waste capacity on recovering ego-motion at the cost of scene modeling fidelity and imagination accuracy.

Method: A physics-informed ego-dynamics encoder-decoder extracts compact ego-state history to modulate a causal Transformer WM. During imagination, an ego-dynamics predictor propagates this context forward to keep priors synchronized with rollout.

Result: DynaDreamer improves task success rates by 28% in urban and 61% in highway scenarios over the strongest baseline, with 73% advantage when extrapolating to unseen chassis.

Conclusion: Explicitly modeling ego-dynamics as a separate prior improves WM fidelity, imagination accuracy, and enables zero-shot cross-chassis adaptation without retraining.
...
### [231] [Generalizable VLA Finetuning via Representation Anchoring and Language-Action Alignment](https://arxiv.org/abs/2607.13429)
...
TL;DR: Anchor-Align improves VLA policies by adding Vision-Language Anchoring (prevents representation drift via distillation) and Language-Action Alignment (jointly trains language and action on same observations), boosting real-robot success from 28%→54% and 37%→60% across two architectures.


<details>
  <summary>Details</summary>
Motivation: BC finetuning of VLMs progressively overwrites pretrained representations that support visual and semantic generalization; co-training on web data doesn't fix language-action misalignment since losses are applied to separate observations.

Method: Anchor-Align: (1) Vision-Language Anchoring - distills layer-wise representations from a frozen VLM copy to prevent drift; (2) Language-Action Alignment - converts action targets into discrete motion-direction labels and jointly trains language and action prediction on the same robot observation.

Result: On real xArm7: 28%→54% and 37%→60% success rates across two VLA architectures. In simulation: consistent improvements on OOD perturbations (LIBERO-PRO), perceptual robustness (LIBERO-Plus), and long-horizon control (CALVIN).

Conclusion: Preserving pretrained representations and effective action learning are not fundamentally at odds; Anchor-Align demonstrates they can be achieved simultaneously.
...
### [232] [Stress-Sharing: A Bio-Inspired Approach to Decentralized Fault Repair in Modular Spacecraft](https://arxiv.org/abs/2607.13444)
*Sidhdharth D. Sikka,Yue Shen,Shaoshuai Mou*

Main category: cs.RO

TL;DR: This paper presents a decentralized, bio-inspired repair policy for modular spacecraft that uses local distress signals to guide surviving modules toward damaged areas and then autonomously restore the pre-damage shape through self-tracing motion, achieving over 80% survivor consolidation in simulations with up to 30% random failures.


<details>
  <summary>Details</summary>
Motivation: Structural damage in modular spacecraft disrupts mechanical and communication connectivity, reducing system capability, and existing approaches relying on redundancy or preplanned reconfiguration cannot enable autonomous repair under local information and physical constraints.

Method: The spacecraft is modeled as a lattice-constrained graph, and a fully decentralized, asynchronous stress-sharing repair policy inspired by biological wound healing is introduced: local distress signals guide surviving modules toward damaged regions to close fragmented gaps, after which each displaced module locally retraces its own motions to recover the pre-damage shape using only local information and no absolute position sensing.

Result: In PyBullet rigid-body simulations across structures of up to 160 modules, three fault densities (10, 20, 30%), and random/localized damage, the policy consolidates surviving modules into a single connected body. In the most severe case (30% random failure), it gathers roughly 80% or more of surviving modules into one connected component.

Conclusion: The fraction of survivors consolidated improves with assembly size, making the approach well suited as a swarm-scale repair policy for large modular spacecraft.
...
### [233] [Learning Physics-Guided Residual Dynamics for Deformable Object Simulation](https://arxiv.org/abs/2607.13451)
*Shivansh Patel,Kaifeng Zhang,Sanjay Pokkali,Svetlana Lazebnik,Yunzhu Li*

Main category: cs.RO

TL;DR: PGRD combines a spring-mass simulator with a neural network that predicts residual corrections, using a velocity-based formulation and sliding-window transformer, achieving superior accuracy on real-world deformable objects and enabling MPC-based manipulation planning and interactive simulation.


<details>
  <summary>Details</summary>
Motivation: Accurately simulating deformable objects remains challenging for robotic manipulation; pure physics-based models lack fidelity while pure learning-based models lack physical consistency.

Method: Hybrid framework: optimizable spring-mass backbone + learned neural network for residual corrections, with velocity-based formulation for stability and sliding-window transformer for temporal dependencies.

Result: Outperforms both purely physics-based and purely learning-based methods on diverse real-world deformable objects; demonstrated in MPC-based manipulation planning and action-conditioned video prediction via 3D Gaussian Splatting.

Conclusion: The physics-guided residual approach effectively bridges the gap between mechanistic models and data-driven learning, enabling more accurate and practical deformable object simulation for robotics.
...
### [234] [Reverse to Advance: Teleoperation-Cost Effective Hard Policy Learning from Reversed Easy Tasks](https://arxiv.org/abs/2607.13455)
*Qiyuan Qiao,Ge Yuan,Can Wang,Dong Xu*

Main category: cs.RO

TL;DR: A framework that leverages temporally reversed easy-task trajectories (which are cheap to collect) as training data for hard tasks, using automated data collection, hierarchical refinement, and iterative learning to reduce teleoperation costs.


<details>
  <summary>Details</summary>
Motivation: High-quality teleoperation datasets for hard tasks are expensive to collect, but easy reversal tasks are cheap; the paper aims to exploit this asymmetry to reduce manual demonstration costs.

Method: Three components: (1) closed-loop data collection alternating hard and easy policies for autonomous reset; (2) hierarchical refinement with temporal inversion, kinematic priors, and critic-guided advantage filter; (3) iterative policy learning combining reversed easy demos and filtered reversed data.

Result: Across simulated benchmarks and real-robot experiments, the method improves hard-task success rates with higher data efficiency and more stable training compared to reversal-based and RL baselines.

Conclusion: The proposed framework enables scalable, reliable training of complex manipulation tasks without extensive hard-task teleoperation.
...
### [235] [Joint On-and-Off Policy Learning for Vision-and-Language Navigation](https://arxiv.org/abs/2607.13461)
*Qingrong He,Lin Zhao,Kevin Zheng,Liang Lin*

Main category: cs.RO

TL;DR: JOP-VLN proposes a three-stage VLN framework that integrates imitation learning (with DAgger) and reinforcement learning via joint off-policy and on-policy training, achieving state-of-the-art results on VLN-CE benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing VLN methods follow two separate paradigms (IL+DAgger and RL) without integration; this gap limits error recovery and exploration capabilities.

Method: Three-stage pipeline: (1) IL on expert demonstrations, (2) DAgger for heuristic exploration trajectories + IL for error recovery, (3) joint off-and-on policy learning with high-entropy trajectory sampling and error-correction-prioritized sorting.

Result: 69.9% success rate on VLN-CE R2R and 68.0% on RxR, achieving new state-of-the-art on R2R.

Conclusion: Integrating IL and RL paradigms in a joint training framework significantly improves VLN performance, setting a new SOTA.
...
### [236] [EgoHTR: Egocentric 4D Demonstrations of Human Terrain Traversal](https://arxiv.org/abs/2607.13472)
...
TL;DR: The paper introduces EgoHTR, a dataset and reconstruction pipeline for egocentric human-terrain 4D motion capture in complex environments, enabling training of perceptive locomotion policies for humanoid robots deployed on uneven terrain.


<details>
  <summary>Details</summary>
Motivation: Deploying humanoid robots in unstructured terrain is challenging because existing methods lack contextual awareness, and current datasets fail to capture human-scene sequences in challenging environments.

Method: The authors built the EgoHTR dataset using a multi-sensor setup (egocentric wearables + portable 3D scanner), capturing 55 scene-aligned 4D motion sequences (150k+ frames) in diverse terrains, and used this data to train perceptive locomotion policies deployed on a Unitree G1 robot.

Result: The dataset achieves state-of-the-art accuracy against motion-capture ground truth, and the trained locomotion policies were successfully deployed on a real humanoid robot (Unitree G1) for reconstructed reference motions.

Conclusion: EgoHTR bridges the gap between humanoid learning and scene reconstruction, enabling community-driven dataset extensions and foundational, context-aware robots for uneven terrain traversal.
...
### [237] [Deformable State Estimation for Autonomous Surgical Tissue Retraction Under Partial Observability](https://arxiv.org/abs/2607.13475)
*Everest Yang,Skye Thompson,George D. Konidaris*

Main category: cs.RO

TL;DR: A learned state estimator using PCA and MLP reconstructs full deformable mesh states from only 40 noisy vertex observations, achieving 98.1% of oracle performance in multi-step retraction planning.


<details>
  <summary>Details</summary>
Motivation: Surgical tissue retraction under partial and noisy perception is challenging, and state estimation from sparse observations is needed for effective manipulation planning.

Method: A learned state estimator combining a multilayer perceptron (MLP) with a low-dimensional PCA latent representation, trained with geometry-aware regularization for smooth, physically plausible deformations.

Result: Achieves 98.1% of oracle performance in multi-step retraction in 2D deformable sheet simulation, with efficient inference.

Conclusion: Learned, geometry-regularized state estimation can effectively support deformable manipulation under realistic perception constraints.
...
### [238] [VAMP-MR: Vector-Accelerated Motion Planning and Execution for Multi-Robot-Arms](https://arxiv.org/abs/2607.13478)
*Philip Huang,Chenrui Gao,Jiaoyang Li*

Main category: cs.RO

TL;DR: This paper presents a suite of multi-robot-arm motion planners that achieve near real-time performance by combining classical planning algorithms with CPU SIMD-based vectorized collision checking, resulting in up to 100x speedup in motion planning and execution postprocessing.


<details>
  <summary>Details</summary>
Motivation: Multi-robot-arm motion planning is computationally expensive for industrial tasks; existing search-based and sampling-based solvers require significant time to produce collision-free, high-quality motions, motivating the need for faster near real-time solutions.

Method: The authors combine classical planning algorithms with state-of-the-art vectorized collision-checking techniques based on CPU SIMD instructions to accelerate the primary bottleneck (collision checking) in multi-arm motion planning.

Result: The new planners achieve up to two orders of magnitude (100x) speedup in both motion planning and execution postprocessing for multi-arm manipulation tasks.

Conclusion: The proposed approach enables near real-time motion generation for multi-robot-arm systems, and the released open-source implementation lowers the barrier for further research and development in this area.
...
### [239] [Topology-Agnostic Mesh Reconstruction of Deformable Objects from Sparse Touch](https://arxiv.org/abs/2607.13479)
*Everest Yang*

Main category: cs.RO

TL;DR: This paper presents a topology-agnostic estimator that reconstructs full 3D meshes of deformable objects (rope, cloth, soft body) from only a few touches and no vision, using a permutation-invariant cross-attention architecture. It reduces reconstruction error by ~2/3 over baselines, and uses deep-ensemble uncertainty to guide next touch locations, outperforming random and active baselines especially under self-occlusion.


<details>
  <summary>Details</summary>
Motivation: Estimating the full shape of deformable objects is challenging when vision is unavailable (e.g., in the dark, inside opaque bags, behind hands, or under heavy self-occlusion). Touch is the natural sensor in these settings, but touch data is sparse and local.

Method: A single topology-agnostic estimator using a permutation-invariant cross-attention architecture that reconstructs full meshes from few touch points (handling 1D rope, 2D cloth, 3D volumetric soft body). Deep-ensemble uncertainty guides active touch selection.

Result: Reduces reconstruction error by roughly two-thirds relative to non-learned geometric mesh completion and Gaussian-process surface baseline. Outperforms simpler global-pool set encoder, with gap growing as more touches are observed. Active touch selection lowers error further, especially under self-occlusion and on error tails.

Conclusion: When vision is also available, where to touch barely matters, motivating the vision-free setting. The learned active touch strategy provides modest but meaningful gains in challenging conditions.
...
### [240] [Layered Risk Mapping for Autonomous Patient Transport in Expeditionary Medical Facilities](https://arxiv.org/abs/2607.13497)
*Lorena Maria Genua,Sarvesh Prajapati,Damla Leblebicioglu,Taşkın Padır*

Main category: cs.RO

TL;DR: This paper presents a layered risk mapping framework that fuses four environmental hazards (terrain slope, static/dynamic obstacles, semantic traversability) using a Noisy-OR fusion model for autonomous wheelchair navigation in expeditionary medical facilities. Risk-informed fusion reduces collision rates from 73% to 32% and more than doubles obstacle clearance in simulations, validated on a commercial wheelchair in indoor/outdoor deployments.


<details>
  <summary>Details</summary>
Motivation: Routine patient transport in expeditionary medical facilities consumes PPE, diverts staff, and increases infection risk—unsustainable under surge conditions. Autonomous wheelchairs could help, but safety-critical navigation in highly unstructured, dynamic environments poses unsolved challenges.

Method: A layered risk mapping framework fusing four heterogeneous hazards (terrain slope, static obstacles, dynamic obstacles, semantic traversability) into a unified probabilistic cost surface using Noisy-OR fusion model.

Result: Risk-informed fusion reduces collision rates from over 73% to under 32% and more than doubles obstacle clearance vs. risk-unaware baseline. Noisy-OR achieves highest clearance and lowest conditional peak risk across all hazard densities in paired Monte-Carlo evaluation.

Conclusion: The proposed architecture successfully meets planning requirements for autonomous wheelchair navigation in expeditionary medical settings, validated on a commercial wheelchair in indoor/outdoor deployments.
...
### [241] [Improving Map Consistency in Graph-Based LiDAR SLAM Through Information-Aware Odometry and Retroactive Loop Closure](https://arxiv.org/abs/2607.13516)
*Saurabh Gupta,Niklas Trekel,Louis Wiesmann,Cyrill Stachniss*

Main category: cs.RO

TL;DR: This paper proposes a LiDAR SLAM system that improves both global trajectory accuracy and local map consistency by using uncertainty-aware odometry (geometry-dependent information matrices for ICP) and a hierarchical loop-closure module with retroactive detection, evaluated with a new map consistency metric.


<details>
  <summary>Details</summary>
Motivation: High-quality maps are essential for robotics tasks, but low trajectory error alone does not guarantee geometrically consistent maps, especially at revisit locations where missed loop closures and drift cause misalignments.

Method: (1) A framework to efficiently estimate geometry-dependent information matrices for ICP, enabling principled weighting of odometry constraints in a pose graph. (2) A hierarchical loop-closure module decoupling place recognition from geometric registration. (3) A retroactive loop-closure module exploiting the optimized pose graph to recover missed loop closures. (4) An evaluation protocol for measuring map consistency at revisit locations.

Result: On several datasets, the system achieves global trajectory accuracies on par with or better than state-of-the-art LiDAR SLAM systems, while consistently improving local geometric map consistency at revisit locations.

Conclusion: Coupling uncertainty-aware odometry with geometry-guided loop-closure refinement leads to more accurate trajectories and higher-quality maps.
...
### [242] [Kepler-Encoder-v0.1: Towards a Multimodal Embedding Model for Robots](https://arxiv.org/abs/2607.13522)
*Ishneet Sukhvinder Singh,Dhanoosh Pooranakumaran,Alex Nguyen,Jia Qi Yip*

Main category: cs.RO

TL;DR: Kepler-Encoder-v0.1 is a multimodal encoder that fuses vision, proprioception, and force/torque into a shared latent via self-supervised cross-modal prediction. At test time, only vision is used, yet the latent recovers force and contact information better than vision-only baselines, especially where cameras are weak. It works across four robots and enables training-free anomaly detection.


<details>
  <summary>Details</summary>
Motivation: Vision alone struggles to capture force, contact, and full body state (e.g., raw vision features predict force with R² ≤ 0.10). The paper asks whether fusing robot state during training can make a vision-only latent encode information that pixels alone do not contain.

Method: The Kepler-Encoder-v0.1 uses a cross-attention layer with learned queries to fuse vision, proprioception, and force/torque into a shared latent space. Training is self-supervised via masked cross-modal prediction under the LeJEPA/SIGReg objective. At test time, only vision is fed in.

Result: The vision-only latent recovers end-effector state and force significantly better than raw ViT features and vision-only controls on all sensored robots, though single-timestep force recovery is modest. On motor state (visible to camera), it ties with strong vision baselines. The latent enables training-free anomaly detection (AUROC 0.90 on out-of-range, 0.69 on scene-swapped states).

Conclusion: Fusing state modalities during training imbues a vision-only latent with force and body-state information that pixels alone lack, especially where the camera view is limited. The encoder works across four robots without embodiment-specific tuning. Future work requires native-rate temporal fusion.
...
### [243] [COLMAR: Cooperative View Policy Learning for Multi-Agent Active 3D Reconstruction](https://arxiv.org/abs/2607.13524)
*Phu Pham,Damon Conover,Aniket Bera*

Main category: cs.RO

TL;DR: COLMAR is a multi-agent cooperative view policy learning framework for active 3D reconstruction that uses shared PPO training with overlap-aware coverage and collision-safe exploration, achieving up to 54% higher accuracy and 49% greater coverage than baselines.


<details>
  <summary>Details</summary>
Motivation: To address coordination inefficiencies (redundant observations, spatial clustering) in multi-agent active 3D reconstruction under limited sensing budgets.

Method: Formulates viewpoint allocation as shared policy optimization over map-centric observations; uses reconstruction-aware objectives (overlap-aware coverage, team-level discovery, collision-safe exploration); trained with parameter-sharing PPO with independent per-agent action selection; fused team map conditions decision-making without inter-agent message passing.

Result: Achieves up to 54% higher reconstruction accuracy and 49% greater coverage on GLEAM and Replica benchmarks compared to heuristic and non-cooperative baselines under matched sensing budgets.

Conclusion: Multi-agent cooperative policy learning with reconstruction-aware dense feedback significantly improves active 3D reconstruction quality and efficiency over non-cooperative approaches.
...
### [244] [Flow-aware Optimal Navigation in Unsteady Flows through Reinforcement Learning](https://arxiv.org/abs/2607.13553)
*Andrea Maria Braghin,Nicolò Botteghi,Matteo Tomasetto,Andrea Manzoni,Gabriele Cazzulani*

Main category: cs.RO

TL;DR: This paper uses TD3 reinforcement learning to train autonomous agents for navigation in chaotic double-gyre flows, testing five bio-inspired observation strategies. Results show velocity sensing with short-term memory achieves best performance, while explicit global parameters actually decrease navigation performance.


<details>
  <summary>Details</summary>
Motivation: Autonomous robotic navigation in nonstationary time-varying fluid flows is challenging due to partial observability and unpredictability. Classical control requires unrealistic global flow knowledge, while biological systems use localized sensory cues.

Method: Applied TD3 (Twin Delayed DDPG) reinforcement learning algorithm to train agents in a parametric chaotic double-gyre flow. Tested five bio-inspired observation strategies: relative position, local velocity/vorticity measures, and short-term memory variants. Analyzed impact of providing explicit global flow parameters.

Result: Velocity-aware agents with short-term memory achieved highest performance. Velocity sensors optimize energy efficiency while vorticity sensors provide better structural mapping and target proximity. Explicit global flow parameters decreased navigation performance.

Conclusion: RL-based autonomous systems develop more robust policies when restricted to implicit flow representations, offering insights for transitioning bio-inspired robotic navigation from simulation to real-world environments.
...
### [245] [IMMNet: Hybrid Fusion of Model-based and Data-driven Approaches for Maneuvering Target Tracking](https://arxiv.org/abs/2607.13573)
*Yixuan Zhao,Chaoqun Yang,Lin Gao,Yongxiao Tian,Ting Yuan*

Main category: cs.RO

TL;DR: IMMNet is a hybrid model/data-driven algorithm that integrates interpretable IMM structure with learnable neural components for maneuvering target tracking, outperforming existing methods across various scenarios.


<details>
  <summary>Details</summary>
Motivation: Maneuvering target tracking in 3D space is challenging due to complex motion dynamics and model mismatch.

Method: Proposes IMMNet, a hybrid model/data-driven algorithm combining the interpretable structure of interacting multiple model (IMM) algorithm with learnable neural components, preserving Bayesian inference while enabling adaptive learning from data.

Result: IMMNet consistently outperforms existing algorithms across various scenarios.

Conclusion: IMMNet is validated as a robust, interpretable, and practical solution for maneuvering target tracking.
...
### [246] [Agile perceptive multi-skill locomotion for quadrupedal robots in the wild](https://arxiv.org/abs/2607.13579)
*Jun-Gill Kang,Jaehyun Park,Tae-Gyu Song,Joon-Ha Kim,Seungwoo Hong,Hae-Won Park*

Main category: cs.RO

TL;DR: APT-RL uses trajectory optimization to generate large-scale 2D motion datasets for training reusable locomotion skills, enabling a quadruped robot to autonomously transition between gaits and traverse complex terrains at high speeds (up to 6 m/s) using only onboard sensors and computation.


<details>
  <summary>Details</summary>
Motivation: Quadruped robots need to seamlessly integrate multiple motor skills, transition smoothly between gaits, and perform high-speed perceptive locomotion in complex environments using only onboard sensors—capabilities that prior frameworks lacked in a unified manner.

Method: APT-RL generates large-scale, feature-rich 2D motion datasets via trajectory optimization with simplified dynamics, trains diverse reusable locomotion skills from these datasets, and transfers them to real quadruped robots in 3D environments using reinforcement learning with action-pretrained transformers.

Result: The robot achieves instantaneous peak speeds of 6 m/s, performs agile maneuvers through complex indoor/outdoor obstacles, and autonomously traverses stairs, hurdles, stepping stones, gaps, and fallen branches with a single onboard policy.

Conclusion: APT-RL effectively unifies multi-skill locomotion with autonomous gait transitions using only onboard perception, demonstrating robust high-speed traversal across diverse complex terrains.
...
### [247] [Active Trust Management for Successful Human-Robot Teaming: Moving from a Trust Repair to a Trust Satisficing Perspective](https://arxiv.org/abs/2607.13595)
*Nicola Webb,Edmund R. Hunt*

Main category: cs.RO

TL;DR: This paper argues that active trust management is essential for human-robot teams, especially in dynamic, high-risk environments. The authors propose a trust satisficing perspective and outline a framework that includes online trust measurement, closed-loop robot behavior adaptation, and variable autonomy to maintain sufficient trust levels for mission success.


<details>
  <summary>Details</summary>
Motivation: Integrating autonomous robots into human teams could improve mission effectiveness in hazardous environments, but trust fluctuations caused by robot mistakes or conflicting priorities could undermine team performance. The authors argue that without active trust management, insufficient trust levels could critically affect mission success.

Method: The paper presents a trust satisficing perspective that accounts for the fluctuating, multi-faceted, and context-dependent nature of trust. It outlines a trust management framework incorporating: (1) online measurement of proxy metrics for trust, (2) closed-loop adaptation of robot behavior, and (3) variable autonomy to preserve human responsibility in value-laden situations.

Result: The paper references experimental exploration of 'swift trust' and a novel behavioral trust metric for human-robot teams, identifying these as preliminary results supporting their framework.

Conclusion: Active trust management is a precondition for successful human-robot teaming in dynamic, high-risk environments. The proposed framework provides a foundation, but further investigation into trust dynamics and metrics is needed.
...
### [248] [Semantic Anchoring for Robotic Action Representations](https://arxiv.org/abs/2607.13597)
*Yuan Xu,Youheng Shi,Chengyang Li,Wentao Zhu,Yizhou Wang*

Main category: cs.RO

TL;DR: This paper investigates what makes good action representations in VLA models, finding that fine-tuning degrades semantic structure from pretrained models. The authors propose a plug-and-play method that anchors action representations to a semantic manifold without altering the deployed model, achieving up to +18.7% on real-world tasks and +21.5% on OOD generalization.


<details>
  <summary>Details</summary>
Motivation: To understand what constitutes a good action representation in VLA models, motivated by the observation that fine-tuning on limited robot data degrades semantic structure inherited from pretrained VLMs.

Method: Systematic probing of action representation quality; a plug-and-play method that anchors action representations to a semantic manifold and decomposes them into shared semantic and private channels, with the auxiliary components discarded at inference.

Result: Up to +18.7% improvement on real-world in-distribution tasks and +21.5% on out-of-distribution generalization across different VLA backbones in simulation and real-world benchmarks.

Conclusion: Action representation quality directly correlates with task success and OOD generalization; the proposed semantic anchoring method effectively preserves semantic structure without changing the deployed model.
...
### [249] [An Empirical Study on Stage-Information Interfaces for VLA Fine-Tuning](https://arxiv.org/abs/2607.13605)
*Yingwei Ji*

Main category: cs.RO

TL;DR: The paper explores using segmented action stage annotations as intermediate representations between full-task instructions and VLA action chunks for long-horizon manipulation tasks, testing stage information interfaces (current-stage text vs. ordinal stage index) with GR00T N1.6 on LIBERO-10, finding that ordinal stage indexing in robot state outperforms alternatives under continuation fine-tuning despite not automatically improving with direct fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To investigate whether providing explicit stage-level information (via different interface representations) between high-level instructions and low-level action chunks improves policy performance in long-horizon manipulation tasks.

Method: Used segmented action annotations as intermediate representation; implemented a progress module to track active stage; compared two interfaces: current-stage text vs. normalized ordinal stage index in robot state; evaluated with GR00T N1.6 on LIBERO-10 under direct fine-tuning and continuation fine-tuning from a full-task instruction baseline.

Result: Under direct fine-tuning: full-task instruction 57.45%, current-stage text 50.24%, Ordinal Stage-State 54.36%. Under continuation: full-task instruction 49.07%, current-stage text 50.00%, Ordinal Stage-State 53.75%. Ordinal Stage-State exceeded both alternatives in all three paired runs under continuation setting.

Conclusion: Explicit stage information does not automatically improve policy performance across all settings; the benefit depends on interface representation and training arrangement, with ordinal stage indexing showing clear advantage under continuation fine-tuning.
...
### [250] [Design, Modeling and Experimental Validation of a Miniature Hybrid Underwater Glider With Large-Range Foldable Deflectable Wings](https://arxiv.org/abs/2607.13622)
*Yongjian Zhu,Yusen Tao,Feitian Zhang*

Main category: cs.RO

TL;DR: FoDeGlider is a miniature hybrid underwater glider with independently actuated folding/deflecting wings. The paper presents a multibody dynamics model using CRBA-based projection to unify configuration-dependent dynamics, a sequential parameter-identification framework for hydrodynamic coefficients, and experimental validation showing accurate modeling and enhanced confined-space navigation via active wing morphing.


<details>
  <summary>Details</summary>
Motivation: Miniature hybrid underwater gliders need improved maneuverability and shape adaptability for confined-space inspections and long-endurance ocean observation. Large-range wing reconfiguration is a promising but underexplored approach, hindered by challenges in mechanical integration, dynamic modeling, and hydrodynamic characterization.

Method: A multibody dynamics model treating wing configuration as a structural variable, using CRBA-based projection to formulate composite inertia, wrench transformations, and component-level hydrodynamics into a unified Fossen-form model. A sequential parameter-identification framework estimates hydrodynamic coefficients, creating an open benchmark dataset.

Result: Experiments demonstrate accurate dynamic modeling and parameter identification across diverse morphing configurations. Gate traversal experiments validate active reconfiguration during locomotion, enabling enhanced navigation in confined underwater environments.

Conclusion: Large-range wing morphing effectively enhances maneuverability and shape adaptability in constrained underwater environments, and the proposed modeling and identification framework successfully supports this capability.
...
### [251] [From Language to Navigation Goals: A Vision-Language Approach for Semantic Navigation of Mobile Robots Using RGB-D Perception](https://arxiv.org/abs/2607.13624)
*Jose Martínez-Fajardo,Pablo Pueyo,Fernando Caballero,Luis Merino*

Main category: cs.RO

TL;DR: A ROS 2-based framework that enables mobile robots to interpret natural language requests, identify target objects via RGB-D perception, and autonomously navigate to them using the Nav2 stack, validated on TurtleBot3 and Unitree Go2 robots.


<details>
  <summary>Details</summary>
Motivation: To enable non-expert users to intuitively communicate with mobile robots using natural language, bridging the gap between language understanding and autonomous navigation.

Method: The framework uses modular ROS 2 components: (1) natural language input processing, (2) object identification via RGB-D data, (3) target position estimation, (4) navigation goal generation, and (5) execution via the Nav2 stack. The modular ROS 2 design ensures platform portability.

Result: Successfully interprets both direct commands and contextual requests, generates natural-language feedback, and navigates to targets in both simulation and real-world scenarios using TurtleBot3 Waffle and Unitree Go2 robots.

Conclusion: Combining semantic perception with autonomous navigation is feasible for creating intuitive human-robot interaction; the open-source release will support further research and deployment.
...
### [252] [Design and Control of the "QuadBoat": A Quadruped Surface Vehicle for Drowning Rescue](https://arxiv.org/abs/2607.13633)
*Lianxin Zhang,Yihan Huang,Huihuan Qian*

Main category: cs.RO

TL;DR: QuadBoat is a bio-inspired quadrupedal USV that uses inverse kinematics and cascaded MPC-PID control to achieve agile maneuverability and accurate object tracking/retrieval for water surface rescue missions.


<details>
  <summary>Details</summary>
Motivation: Prompt extraction of victims from water is critical in rescue missions, yet previous rescue robot research has seldom addressed this capability.

Method: QuadBoat employs a quadrupedal robot configuration with actively adjustable posture, an inverse kinematics-based controller, and a cascaded MPC-PID controller for movement control.

Result: Maneuverability demonstrations validated high agility; tracking experiments (leg action and trajectory tracking) confirmed high motion accuracy and system mobility; visual-based tracking and object pickup experiments verified target tracking and rescue effectiveness both indoors and outdoors.

Conclusion: QuadBoat demonstrates effective water surface rescue capabilities with accurate tracking, agile maneuverability, and successful object retrieval in real-world conditions.
...
### [253] [Anatomy of Uncertainty: Expressive Descriptors of Robotic Manipulator Motion for Non-verbal Communication in Human-Robot Collaboration](https://arxiv.org/abs/2607.13696)
*Ridhima Bector,Souravik Dutta,Poornima Ramachandran,Ree Yan Yeoh,Jui Hien Tan,Domenico Campolo,Bernhard Johannes Schmitt*

Main category: cs.RO

TL;DR: This paper introduces a mathematical framework for expressing robot perceptual uncertainty through motion, using Laban Movement Analysis to map uncertainty states to distinct movement signatures, and validates the approach through a human-subject study.


<details>
  <summary>Details</summary>
Motivation: Robots in human-robot collaboration need to communicate not only intended actions but also the uncertainty arising from incomplete or ambiguous perception, which is currently underexplored.

Method: The authors propose a Commitment-Vigilance state space based on Laban Movement Analysis, mapping uncertainty states (confidence, curiosity, hesitance, fear, inactivity) to Laban Effort signatures. Five motion primitives (approach, pause, retreat, exploration, oscillation) are parameterized using 11 kinematic/geometric descriptors. A video-based human-subject study evaluated recognition of expressive trajectories.

Result: Participants reliably identified the intended behavioral states, and several kinematic descriptors significantly modulated perceived expressiveness, establishing a perceptually grounded basis for encoding robot uncertainty in motion.

Conclusion: The framework supports future autonomous trajectory generation using parametric movement representations for collaborative tasks in shared environments.
...
### [254] [Towards a Modular Bin-picking Framework for Handling Object Pose Uncertainties](https://arxiv.org/abs/2607.13698)
*Frederik Hagelskjær*

Main category: cs.RO

TL;DR: A modular robotic framework that jointly handles object pose uncertainty and grasping errors through pose distribution estimation, multi-viewpoint fusion, and interchangeable modules for precise bin-picking.


<details>
  <summary>Details</summary>
Motivation: Robotic bin-picking systems need to handle both object pose estimation errors and grasping errors, but existing approaches only target specific challenges without offering general solutions.

Method: A modular framework with: (1) object pose distribution estimation for handling pose uncertainty under ambiguous observations, (2) a second-viewpoint module that computes and fuses complementary pose distributions to reduce uncertainty, and (3) two independent modules to compensate for grasping errors. Components can be combined or used individually based on setup.

Result: Evaluated in real-world setups with three different objects; all modules shown to improve efficiency with no errors reported.

Conclusion: Jointly addressing grasping and pose uncertainties via interchangeable modules is promising for flexible robotic production. Current framework limited to SO(2) but extensible to SE(3).
...
### [255] [nuTruck: Benchmarking Autonomous Driving Planning for Distributed Electric-drive Trucks](https://arxiv.org/abs/2607.13704)
*Jinyu Miao,Pu Zhang,Yifei He,Chengyao Zhang,Kun Jiang,Ke Wang,Mengmeng Yang,Diange Yang*

Main category: cs.RO

TL;DR: nuTruck is the first high-fidelity benchmark for learning-based autonomous driving planners on distributed electric-drive trucks (DETs), incorporating a nonlinear truck dynamics model with rollover risk assessment, adapted baselines from nuPlan, and extensive closed-loop evaluations.


<details>
  <summary>Details</summary>
Motivation: Learning-based planners have succeeded in passenger vehicles but their performance on heavy-duty trucks (especially DETs) is unexplored due to complex dynamics and high rollover susceptibility. There is no existing benchmark tailored for DETs.

Method: 1) Integrates a high-fidelity nonlinear truck dynamical model into simulation with independent wheel control and dynamic load transfer; 2) Adapts rule-based and learning-based planners as baselines for DETs; 3) Uses real-world nuPlan scenarios for closed-loop evaluation.

Result: Extensive closed-loop evaluations analyze both conventional collision-free planning and dynamical safety (rollover risk) of planned trajectories for DETs.

Conclusion: nuTruck serves as a new standard for fair and realistic evaluation of autonomous driving planners on DETs, filling a critical gap in the literature.
...
### [256] [The Nonsmooth Impact Direction (NSID) of Robotic Systems](https://arxiv.org/abs/2607.13768)
*Annika Kirner,Christian Ott*

Main category: cs.RO

TL;DR: This paper analyzes pre- and post-impact velocities in rigid robotic collisions, identifying a characteristic "nonsmooth impact direction" (NSID) that is largely independent of contact properties, with experimental validation supporting its use for future impact planning and control algorithms.


<details>
  <summary>Details</summary>
Motivation: Rigid robot-environment collisions are typically modeled as instantaneous events with impulsive forces and nonsmooth velocities. A systematic understanding of impact velocity relationships is needed to improve planning and control for robotic impact applications.

Method: The authors systematically analyze pre- and post-impact velocities, identifying the nonsmooth impact direction (NSID) as a characteristic direction of robotic impacts. They examine backdrivable rigid-link systems with nonelastic/flexible joints, and both unconstrained and constrained systems.

Result: The NSID is shown to be largely independent of contact properties. The approach direction relative to NSID determines the impulsive force direction in frictional, inelastic impacts. Theoretical analysis is supported by experimental validation.

Conclusion: The findings provide a theoretical foundation for developing future planning and control algorithms for robotic impact applications such as humanoid locomotion on slippery surfaces and repetitive hammering.
...
### [257] [Dynamical Vehicle Orienteering Problem for Multi-Rotor Unmanned Aerial Vehicles](https://arxiv.org/abs/2607.13789)
*František Nekovář,Matej Novosad,Martin Saska,Robert Pěnička*

Main category: cs.RO

TL;DR: This paper introduces the Dynamical Vehicle Orienteering Problem (DVOP) for multi-rotor UAV flight planning, combining reward maximization with time-optimal trajectory planning under realistic dynamics. Two solution approaches are proposed: a Branch-and-Bound (BnB) procedure combining NLP and MILP, and a Large Neighborhood Search (LNS) metaheuristic, with improvements up to 37% over state-of-the-art solutions.


<details>
  <summary>Details</summary>
Motivation: The Orienteering Problem (OP) is limited to simple graph-based formulations that ignore realistic vehicle dynamics. The authors aim to generalize the OP by incorporating external and vehicle-actuated forces, specifically for multi-rotor UAV flight planning, where dynamics significantly impact achievable trajectories and rewards.

Method: Two solution approaches: (1) A Branch-and-Bound (BnB) procedure combining Non-Linear Programming (NLP) and Mixed-Integer Linear Programming (MILP), using a novel MILP formulation of travel costs based on minimum-time trajectory primitives through target triplets. (2) A Large Neighborhood Search (LNS) metaheuristic using limited thrust decomposition for fast trajectory computation.

Result: Experiments show improvements of up to 37% over state-of-the-art solutions for the Kinematic Orienteering Problem. Real-world deployment on a multi-rotor UAV verifies the proposed PMM solution trajectories.

Conclusion: The DVOP effectively generalizes the OP to realistic dynamic vehicle scenarios. The BnB provides high-quality solutions while the LNS scales to larger instances. The approaches are validated both in benchmark tests and real-world UAV deployment.
...
### [258] [Vision-Based Obstacle Separation for Strawberry Harvesting in Clusters Using Hierarchical Reinforcement Learning](https://arxiv.org/abs/2607.13799)
*Teng Li,Hanfei Shi,Chunjiang Zhao,Ya Xiong*

Main category: cs.RO

TL;DR: This paper proposes VGPA, a hierarchical reinforcement learning framework with vision guidance and progressive adaptive exploration, for harvesting strawberries in clustered environments by first separating obstacles then grasping targets, achieving 96.7% success in simulation and 71.7–88.3% in real-world tests.


<details>
  <summary>Details</summary>
Motivation: Selective harvesting in clustered strawberry environments is challenging due to occlusion by unripe fruits, making direct grasping unreliable.

Method: A hierarchical reinforcement learning framework (VGPA) with a vision-guided decision mechanism and Progressive Adaptive Exploration Strategy (PAES), decomposing the task into obstacle separation and target grasping stages.

Result: 96.7% success rate in simulation; 71.7%–88.3% success rate in sim-to-real transfer, outperforming direct picking with only 1.22s extra average harvesting time.

Conclusion: The method demonstrates effectiveness, generalization ability, and practical potential for robotic harvesting in complex clustered environments.
...
### [259] [A Deployed Hybrid Vehicle-in-the-Loop Platform for Validating Cooperative Perception](https://arxiv.org/abs/2607.13806)
...
TL;DR: This paper presents a deployed hybrid Vehicle-in-the-Loop (ViL) platform combining a real instrumented vehicle with a CARLA-based digital twin via V2X communication, demonstrating cooperative perception on a test track under varying weather and localization noise conditions.


<details>
  <summary>Details</summary>
Motivation: European safety regulation now allows virtual homologation evidence for automated driving, but requires validated physical-virtual facilities. The authors aim to develop and demonstrate a deployed platform that can generate such evidence through real-virtual coupling.

Method: A hybrid ViL platform couples a real vehicle with a CARLA digital twin via a V2X (CAM/CPM) message pipeline. A GPU-accelerated Cooperative Perception module fuses virtual and real sensor data into probabilistic occupancy grids. The platform is tested on a multi-vehicle double T-intersection scenario under rain, night, and varied localization noise levels.

Result: Cooperative perception substantially widens FoV coverage and improves occupied-cell recall. Beyond a moderate localization-noise threshold, positioning uncertainty (not weather) becomes the dominant error source.

Conclusion: The platform is viable for hybrid homologation testing, with clear engineering targets identified (localization noise reduction). Its trajectory points toward a Mediterranean ODD testing service.
...
### [260] [Learning Robust Execution in Robotic Manipulation with Agentic Reinforcement Learning](https://arxiv.org/abs/2607.13818)
*Xiaopeng Zhang,Yueyang Weng,Qi Liu,Yongjin Mu,Yanjie Li*

Main category: cs.RO

TL;DR: This paper proposes an agentic reinforcement learning framework with two complementary metrics to assess robotic execution stability, enabling recovery through high-level decision-making and achieving 13.7-39.2% improvement in success rates under standard and disturbed settings.


<details>
  <summary>Details</summary>
Motivation: Robotic manipulation faces challenges from uncertainty, long-horizon execution, and compounding errors. Existing VLA models lack explicit mechanisms to assess execution stability and recover from deviations during runtime.

Method: (1) Two complementary metrics for runtime execution quality assessment, and (2) an agentic RL framework that learns high-level decision-making to restore effective execution by reasoning over recent history and selecting among execution modes to trigger recovery mechanisms.

Result: Up to 13.7% improvement in success rate under standard settings and up to 39.2% improvement under disturbance settings on the LIBERO benchmark.

Conclusion: The proposed method substantially enhances execution robustness for robotic manipulation by enabling runtime stability assessment and agentic high-level recovery from execution degradation.
...
### [261] [Recursive ArUco Markers: A Scalable Fiducial Marker Design for Unmanned Aerial Vehicle Landing Pads](https://arxiv.org/abs/2607.13830)
*Rafael Munoz-Salinas,Francisco Jose Romero-Ramirez,Sergio Garrido-Jurado*

Main category: cs.RO

TL;DR: Proposes a Recursive ArUco marker design that embeds complete markers within parent marker bits, enabling unlimited recursion depth, occlusion-robust detection, and unique IDs across scales for multi-UAV landing.


<details>
  <summary>Details</summary>
Motivation: Standard fiducial markers have limited operational ranges. Existing recursive/fractal markers either require the center to remain visible (vulnerable to occlusion) or have limited recursion depth and dictionary constraints.

Method: Transforms any standard ArUco marker into a recursive marker with arbitrary depth by embedding complete markers within both black and white bits of the parent marker, using a modified bit-sampling detection strategy.

Result: Achieves unlimited recursion depth, robust detection under partial occlusion (no center visibility needed), and maintains a single unique identifier across all recursive scales for an extensive dictionary of landing pads.

Conclusion: The proposed Recursive ArUco marker outperforms existing Fractal and Harco markers by supporting multi-UAV simultaneous landing with designated locations, overcoming occlusion vulnerabilities and dictionary limitations.
...
### [262] [Merging Reaction to Cognition: A Hybrid Cognitive Strategy for Odour Source Localisation in Natural Environments](https://arxiv.org/abs/2607.13853)
*Hugo Magalhães,Rui Baptista,Lino Marques*

Main category: cs.RO

TL;DR: A hybrid cognitive-reactive algorithm for chemical source localization using robots, combining belief-based planning with detection-triggered reactive behaviors, achieving up to 50% distance reduction, 86% success rate, and 3.2m error in field tests.


<details>
  <summary>Details</summary>
Motivation: Chemical pollutants transported by turbulent flows create complex plume structures. Rapidly localizing emission sources is critical for ecosystem and human health protection, but existing methods (bio-inspired or purely cognitive) have limitations in efficiency, robustness, or computational burden.

Method: A hybrid strategy integrating bio-inspired reactivity into belief-dependent motion planning, using a detection-triggered switching mechanism between crossflow exploration and source-directed motion. Behavioral parameters derived from belief metrics enable adaptive reactivity without manual tuning.

Result: Validated via simulations under three turbulence conditions and field experiments with an autonomous surface vehicle in the Mondego River, Portugal. Achieved up to 50% reduction in traveled distance, 86% success rate, and 3.2m average localization error.

Conclusion: The hybrid approach significantly improves efficiency over purely cognitive methods while maintaining robustness, successfully merging the reactivity of bio-inspired strategies with the principled reasoning of cognitive frameworks for real-world chemical source localization.
...
### [263] [Learning Forward & Reverse Skills from a Single Unfinished Demonstration for Constrained Manipulation Tasks](https://arxiv.org/abs/2607.13882)
*Yexin Hu,Haoyi Zheng,Johannes Heidersberger,Dongheui Lee*

Main category: cs.RO

TL;DR: A unified one-shot LfD framework that learns both forward and reverse constrained manipulation skills from a single, possibly unfinished demonstration by decomposing motion into non-contact (DMP) and contact (screw motion primitives with admittance control) phases.


<details>
  <summary>Details</summary>
Motivation: Existing LfD methods for contact-rich tasks require multiple complete demonstrations and cannot support reverse skill execution, limiting their practicality.

Method: Decomposes demonstrations into non-contact (DMP) and contact phases; contact motion segmented into screw primitives via geometry-driven twist-direction segmentation; executed with admittance-guided pose correction and speed regulation.

Result: Improved success rates and robustness over baselines on peg insertion, battery insertion, lock opening, and screw driving tasks; enables reverse execution and task completion beyond demonstrated trajectory length.

Conclusion: The proposed one-shot framework effectively handles constrained manipulation with geometric constraints and force interaction, enabling bidirectional skill execution from minimal demonstration data.
...
### [264] [S-squared-VLA: Decoupling Semantic and Spatial Streams in Vision-Language-Action Models for Autonomous Driving](https://arxiv.org/abs/2607.13926)
*Jianguo Yu,Rukang Wang,Duanfeng Chu,Chen Wang,Renju Feng,Liping Lu*

Main category: cs.RO

TL;DR: S-squared-VLA proposes a decoupled semantic-spatial architecture for Vision-Language-Action models in autonomous driving, achieving 87.1 PDMS and 98.4% collision-free rate on NAVSIM benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs struggle with precise low-level control due to a semantic-physical gap from discrete-to-continuous mismatch, while VLAs suffer from spatial representation collapse that degrades geometric priors essential for safe navigation.

Method: S-squared-VLA decouples semantic and spatial streams: semantic stream uses hierarchical bridging for multi-scale VLM features; spatial stream bypasses autoregressive bottleneck to preserve uncompressed spatial features; dual-stream planning adapter fuses both via cascaded attention.

Result: Achieves state-of-the-art 87.1 PDMS score on NAVSIM benchmark and highest No Collision rate of 98.4% among all evaluated methods under supervised fine-tuning setting.

Conclusion: Mitigating spatial representation collapse in VLMs through explicit semantic-spatial decoupling significantly improves both planning accuracy and safety in autonomous driving.
...
### [265] [Discriminative Barrier Functions for Safe Adversarial Imitation Learning from Observation](https://arxiv.org/abs/2607.13938)
*Anubhav Vishwakarma,Bhaumik Mehta,Caleb Hsu,Byron Boots,Karen Leung,Tyler Han*

Main category: cs.RO

TL;DR: This paper integrates Inverse Reinforcement Learning with Control Barrier Functions to enable safe, data-driven recovery of barrier functions from expert demonstrations, showing improved safety in navigation tasks.


<details>
  <summary>Details</summary>
Motivation: IRL methods are unsafe due to unconstrained exploration, while CBFs require time-consuming analytical design. The authors aim to jointly solve both limitations.

Method: The authors constrain reward function candidacy during IRL to the space of CBFs, enabling data-driven recovery of barrier functions from unlabeled expert observations.

Result: The recovered barrier function is robust to unsafe states absent from expert data. The method outperforms standard IRL baselines in simulated navigation safety. Trade-offs between planning-based and policy-based IRL are investigated in simulation and real-world obstacle avoidance.

Conclusion: Constraining IRL reward functions to CBF space yields safe online control with continuous improvement, enabling practical deployment of safe learning from demonstrations.
...
### [266] [GigaWorld-Policy-0.5: A Faster and Stronger WAM Empowered by AutoResearch](https://arxiv.org/abs/2607.13960)
...
TL;DR: GigaWorld-Policy-0.5 is an action-centered World Action Model that uses future visual dynamics during training but action-only decoding at inference, achieving 85ms latency on RTX 4090 via Mixture-of-Transformers architecture, with an AutoResearch pipeline for systematic hyperparameter tuning.


<details>
  <summary>Details</summary>
Motivation: Existing World Action Models (WAMs) explicitly generate future videos at inference time, causing substantial computational overhead that hinders real-time closed-loop deployment for robot control.

Method: 1) Action-centered formulation: uses future visual dynamics during training but action-only decoding at inference. 2) Mixed AC-WM and WAM pretraining strategy. 3) Mixture-of-Transformers architecture separating visual dynamics modeling and action generation into specialized experts. 4) Agent-based AutoResearch pipeline for systematic training configuration search.

Result: Achieves 85 ms inference latency on a local RTX 4090 setup, preserves training benefits of future visual dynamics while significantly improving inference efficiency.

Conclusion: GigaWorld-Policy-0.5 effectively decouples training-time visual dynamics supervision from inference-time action-only decoding, enabling efficient real-time robot control without sacrificing the learning benefits of world models.
...
### [267] [AeroMap3D: Anchoring Monocular UAV 6-DoF Localization to Visual-Geometric-Semantic Map Priors](https://arxiv.org/abs/2607.14009)
*Zhiyun Deng,Luis Sentis*

Main category: cs.RO

TL;DR: AeroMap3D is a monocular 6-DoF UAV localization system using satellite maps, DEMs, and semantic annotations for GNSS-denied navigation, achieving 5.88m mean 3D error over 55km flight without retraining.


<details>
  <summary>Details</summary>
Motivation: To enable reliable UAV localization in GNSS-denied environments by anchoring onboard imagery to map priors, addressing cross-view discrepancy (UAV vs satellite) and structural inconsistency (bare-earth DEMs vs urban scenes with buildings).

Method: (1) A lightweight adapter enabling pretrained dense matcher for UAV-to-map registration without finetuning by estimating scale ratio and yaw offset; (2) Lifting 2D correspondences onto DEM terrain with OpenStreetMap semantic filtering to reject unreliable matches; (3) Delayed-state EKF fusing delayed map-based pose measurements with relative-motion priors.

Result: Localizes all trajectories across eight Austin sites within 50m, achieving 5.88m mean 3D error over 55km of flight, without any retraining or tuning on UAV-Terra3D.

Conclusion: AeroMap3D effectively combines visual, geometric, and semantic map priors for robust GNSS-denied UAV navigation, overcoming cross-view and structural challenges with a lightweight, training-free adaptation approach.
...
### [268] [Industrial Dexterity Benchmark: A Hardware-Software Benchmarking Platform for Industrial Dexterous Manipulation](https://arxiv.org/abs/2607.14021)
...
TL;DR: This paper presents a shift from classical robotics to end-to-end multimodal imitation learning for industrial dexterous manipulation, introducing IDB benchmark boards, DAG-ROS framework, and AG-iDP3 policy. The best configuration (multimodal Diffusion Policy with R3M encoder) achieves 78% success on cable manipulation, significantly outperforming the 36% RGB baseline.


<details>
  <summary>Details</summary>
Motivation: Dexterous manipulation tasks (cable routing, connector insertion, precision assembly) remain heavily reliant on manual labor despite decades of robotics research, highlighting a critical bottleneck in industrial automation.

Method: Three contributions: (1) IDB benchmark boards mimicking real industrial tasks; (2) DAG-ROS imitation learning framework; (3) AG-iDP3 multimodal diffusion policy fusing RGB images, point clouds, joint positions, and wrist wrench data. Evaluated via 48 trials per configuration with ~100 teleoperated demonstrations per task phase.

Result: Best configuration (multimodal DP with R3M encoder + multi-view RGB) achieves 78% grasp-and-insert success, vs 36% for single-camera RGB DP baseline.

Conclusion: Learned policies can outperform classical vision/control methods in robustness, generalization, and deployment efficiency, justifying a shift toward scalable robotic automation for high-up-time industrial environments.
...
### [269] [PhysClaw-0: A Symbiotic Agentic System for Robot Autonomy via Language Corrections](https://arxiv.org/abs/2607.14047)
...
TL;DR: PhysClaw-0 is a human-robot symbiotic system for autonomous data collection that uses Corrective Memory to retain and reuse human corrections across rounds, reducing human working time to 16% while matching teleoperation performance on a desktop-clearing task.


<details>
  <summary>Details</summary>
Motivation: Existing autonomous data collection pipelines require re-issuing episode-scoped fixes whenever the same failure recurs, causing oversight cost to grow with session length rather than with the number of distinct problems.

Method: PhysClaw-0 uses an agentic loop that collects, verifies, and resets autonomously, pausing for a remote operator only when a phase exhausts a retry budget. An LLM parser maps natural-language corrections to structured adjustments stored in Corrective Memory for reuse.

Result: Reduced human working time to 16%, improved verifier-human agreement in all settings, raised single-attempt success from 12.5% to 47.5%, and matched teleoperation-trained policy success at lower human cost.

Conclusion: PhysClaw-0 demonstrates that retaining and reusing corrections across rounds significantly reduces human oversight cost while maintaining task performance, enabling more efficient autonomous data collection for manipulation policy learning.
...
### [270] [Efficient Computing for Medical Image Acquisition and Reconstruction](https://arxiv.org/abs/2607.13204)
*Xiao Wang,Jayasai Rajagopal,Md Safaiat Hossain,Peng Chen,Mohamed Wahib,Enzhi Zhang,Emma J. Reid*

Main category: eess.IV

TL;DR: This chapter presents a unified computational perspective on medical image reconstruction across CT, MRI, PET, and SPECT, covering imaging physics, mathematical frameworks, analytical/iterative/statistical reconstruction methods, and efficient computing strategies to enable accurate and scalable reconstruction.


<details>
  <summary>Details</summary>
Motivation: Medical imaging systems measure physical signals rather than directly acquiring images, requiring inverse problem solving. As datasets grow larger and higher-dimensional, image reconstruction has become a major computational bottleneck, motivating the need for a unified framework that connects medical physics, mathematics, and efficient computing.

Method: The chapter derives a generalized mathematical framework for reconstruction, reviews analytical, iterative, and statistical model-based methods, and examines efficient computing strategies including optimization algorithms, physics-aware forward operators, memory-efficient implementations, and parallel computing.

Result: Advanced reconstruction methods (analytical, iterative, statistical) substantially improve image quality while reducing radiation dose or scan time, but at increased computational cost. Efficient computing strategies are essential to achieve clinically practical reconstruction times.

Conclusion: Integration of imaging physics, mathematical modeling, and efficient computing enables accurate and scalable medical image reconstruction across CT, MRI, PET, and SPECT modalities.
...
### [271] [Video to All-in-focus Image Reconstruction Algorithm for Automated Microscopic Urinalysis](https://arxiv.org/abs/2607.13601)
*Chinmay Nema,Hari Om Aggrawal,Dipam Goswami,Rajiv Gupta,Vinti Agarwal*

Main category: eess.IV

TL;DR: The paper proposes replacing time-consuming multi-focal-plane image acquisition for microscopic urinalysis with a short manual-focus video (2-14 sec), reconstructing an all-in-focus image from video frames, and applying deep learning for sediment detection/classification. Experiments on 14 real-world videos show effectiveness.


<details>
  <summary>Details</summary>
Motivation: Traditional microscopic urinalysis requires acquiring multiple images at different focal planes due to the multi-layer structure of urine samples, which is time-consuming. The authors aim to simplify and accelerate this process.

Method: Record a short video (2-14 sec) while manually changing the lens focus; reconstruct an all-in-focus image from video frames using a novel reconstruction algorithm; apply a deep learning model for urine sediment detection and classification.

Result: Experiments on 14 videos acquired by a trained lab technician in a real diagnostic lab environment demonstrate the effectiveness of the proposed automated urinalysis pipeline.

Conclusion: The proposed video-based approach with all-in-focus reconstruction can replace the time-consuming multi-image acquisition method for automated microscopic urinalysis, achieving practical usability in real diagnostic settings.
...
### [272] [N-O Cool-chic: reconcile fast encoding with lightweight decoding for neural image compression](https://arxiv.org/abs/2607.13723)
*Théophile Blard,Théo Ladune,Pierrick Philippe,Xiaoran Jiang,Olivier Déforges*

Main category: eess.IV

TL;DR: This paper proposes a non-overfitted version of Cool-chic that reduces encoding complexity by 1000x by replacing overfitting with an encoder network, maintaining competitive compression performance.


<details>
  <summary>Details</summary>
Motivation: Overfitted image codecs like Cool-chic have prohibitively long encoding times, making them impractical for real-time applications.

Method: The authors bypass the overfitting procedure in Cool-chic and instead complement the decoder with an encoder network, creating a non-overfitted (N-O) version.

Result: Encoding complexity is reduced by a factor of 1000 compared to the original overfitted Cool-chic, while maintaining competitive performance.

Conclusion: The proposed N-O Cool-chic significantly reduces encoding complexity, addressing a key practical limitation of overfitted codecs for real-time applications.
...
### [273] [TCAM-Diff: Triplane-Aware Cross-Attention Medical Diffusion Model](https://arxiv.org/abs/2607.13812)
*Zhenkai Zhang,Krista A. Ehinger,Tom Drummond*

Main category: eess.IV

TL;DR: TCAM-Diff is a 3D medical image generation model that uses a decoder-only autoencoder with triplane representation and triplane-aware cross-attention diffusion to efficiently generate high-resolution 3D volumes, achieving superior results on BrainTumour, Pancreas, and Colon datasets.


<details>
  <summary>Details</summary>
Motivation: To reduce the memory requirements for encoding and generating high-resolution 3D medical images, which is a key challenge in this domain.

Method: (1) Decoder-only autoencoder to learn triplane representation from dense volume with generalization operations to prevent overfitting; (2) Triplane-aware cross-attention diffusion model for feature learning and integration; (3) Pre-trained decoder to rapidly transform generated features into 3D volumes.

Result: Outstanding results on BrainTumour (128³), Pancreas (256³), and Colon (512³) datasets; evaluated using MSE, SSIM for reconstruction, and W-GAN critic for generative quality; outperforms existing encoder-decoder methods with similar-sized latent spaces.

Conclusion: TCAM-Diff provides better reconstruction and generation results than comparable encoder-decoder methods, demonstrating its effectiveness for high-resolution 3D medical image generation.
...
### [274] [Mixed-Timescale Differential Coding for Downlink Model Broadcast in Wireless Federated Learning](https://arxiv.org/abs/2607.13119)
*Chung-Hsuan Hu,Zheng Chen,Erik G. Larsson*

Main category: cs.IT

TL;DR: This paper proposes a mixed-timescale differential coding (MTDC) scheme to handle downlink transmission failures in federated learning, allowing devices to reconstruct the latest global model even when they miss differential updates.


<details>
  <summary>Details</summary>
Motivation: Standard federated learning uses differential coding to reduce communication bits when broadcasting global models, but wireless link failures cause devices to miss updates and fall back to outdated models or idle waiting. The paper aims to solve this reliability problem.

Method: Proposes MTDC, which performs differential coding at two levels by adjusting the reference model, enabling device-side reconstruction of the latest global model between full broadcasts despite missed updates. Also introduces an age-aware MTDC variant and a device scheduling policy.

Result: Simulation results show MTDC schemes achieve superior learning performance compared to baselines under similar communication budgets with downlink failures.

Conclusion: Mixed-timescale differential coding effectively addresses the challenge of missed differential updates due to wireless link failures, improving communication efficiency and learning performance in federated learning.
...
### [275] [Active Beyond-Diagonal RIS Empowered Heterogeneous Edge Computing: A Distributional Reinforcement Learning Approach](https://arxiv.org/abs/2607.13160)
*Tianyu Pang,Hongyu Li*

Main category: cs.IT

TL;DR: This paper proposes a DSAC-T-based end-to-end optimization framework for energy-aware offloading and resource allocation in reciprocal active BD-RIS-assisted heterogeneous MEC systems, achieving 81.67% feasibility and 0.0267s per-scenario online decision time.


<details>
  <summary>Details</summary>
Motivation: Active BD-RIS enables hybrid transmitting/reflecting mode for signal amplification and full-space coverage in blockage-aware uplink offloading in heterogeneous MEC. However, reciprocal devices cause cross-sector energy leakage, reshaping the energy-latency tradeoff, which motivates energy-aware optimization.

Method: An end-to-end joint optimization framework based on a refined distributional soft actor-critic algorithm (DSAC-T), which models return distributions instead of expected values to handle reward heterogeneity and feasibility-boundary sensitivity in the high-dimensional mixed integer nonconvex problem.

Result: DSAC-T achieves the best energy-latency reward, highest feasibility ratio of 81.67%, and fast online decision time of 0.0267s per scenario compared to baseline algorithms.

Conclusion: DSAC-T effectively addresses the tightly coupled offloading and resource allocation problem for reciprocal active BD-RIS-assisted heterogeneous MEC, outperforming baselines in feasibility, reward, and decision speed.
...
### [276] [Construction of Generalized Weighing-Hadamard Matrices over Finite Fields](https://arxiv.org/abs/2607.13324)
*Gustavo T. Bastos,Miguel Beltrá,Sara D. Cardell,Verónica Requena*

Main category: cs.IT

TL;DR: This paper studies Generalized Weighing-Hadamard (GWH) matrices over finite fields, showing invertible GWH matrices form a group, introducing equivalence via orthogonal transformations, proving the quotient group is abelian, and discussing coding theory applications.


<details>
  <summary>Details</summary>
Motivation: To establish the algebraic structure (group properties) and equivalence relations of Generalized Weighing-Hadamard matrices over finite fields, which were not previously well-understood.

Method: Algebraic and group-theoretic analysis of GWH matrices over finite fields, studying invertible subsets, orthogonal transformations, and quotient group structures.

Result: Invertible GWH matrices form a group; a strong equivalence notion via orthogonal transformations is defined; the quotient group by the orthogonal subgroup is abelian.

Conclusion: GWH matrices have structured algebraic properties and potential applications in coding theory.
...
### [277] [$r$-Minimal Poset Codes](https://arxiv.org/abs/2607.13520)
*Yang Xu,Haibin Kan,Guangyue Han*

Main category: cs.IT

TL;DR: This paper generalizes minimal Hamming metric codes to r-minimal P-codes using poset structures, characterizes them via cutting r-blocking maps and a generalized Ashikhmin-Barg criterion, and provides existence results with a special focus on hierarchical posets and disjoint chains.


<details>
  <summary>Details</summary>
Motivation: To extend the concept of minimal codes from the Hamming metric setting to a more general poset-based framework (P-support), enabling broader applicability and deeper theoretical understanding of code minimality.

Method: The authors introduce r-minimal P-codes and characterize them using cutting r-blocking maps (generalizing cutting blocking sets). They also give a condition for r-minimality via (P,ω)-weight, generalizing the Ashikhmin-Barg criterion, and prove existence results using combinatorial structures like disjoint unions of chains and hierarchical posets.

Result: Characterization of r-minimal P-codes via cutting r-blocking maps and (P,ω)-weight; existence results for general P and disjoint chain posets; equivalence to r-minimal Hamming codes under hierarchical posets; characterization of two-level hierarchical poset blocking sets, answering a question from Hyun et al. (2020).

Conclusion: The paper successfully generalizes minimal codes to the poset setting, provides novel characterizations and existence proofs, and resolves an open question about cutting blocking sets in hierarchical posets.
...
### [278] [Optimal Non-Binary Single-Track Gray Code](https://arxiv.org/abs/2607.13588)
*Tuvi Etzion*

Main category: cs.IT

TL;DR: This paper proves the existence of single-track Gray codes over prime fields \(\mathbb{F}_p\) for \(p=3,5\) with length \(p^t\) and \(p^{p^t}\) codewords, and provides a construction method that extends from \(t=2\) to larger \(t\) for larger primes.


<details>
  <summary>Details</summary>
Motivation: Single-track Gray codes offer advantages in quantization and coding applications over conventional Gray codes, but binary versions cannot include all \(2^n\) codewords unless \(n=2\). This motivates the search for non-binary constructions that can achieve full codeword sets.

Method: The paper proves existence for \(p=3\) and \(p=5\) explicitly, shows that for larger primes a code for \(t=2\) implies existence for all \(t>2\), and provides indications that such codes exist even when the alphabet size is not prime.

Result: Single-track Gray codes of length \(p^t\) with \(p^{p^t}\) codewords over \(\mathbb{F}_p\) exist for \(p=3\) and \(p=5\). For larger primes, constructing the case \(t=2\) suffices to extend to any \(t>2\).

Conclusion: The paper expands the known existence of single-track Gray codes beyond binary cases, establishing constructions for small primes and a general framework for extension, with hints toward non-prime alphabets.
...
### [279] [Low-Complexity Soft-Aided Error-and-Erasure Decoding for Generalized Product Codes](https://arxiv.org/abs/2607.13719)
*Sisi Miao,Laurent Schmalen*

Main category: cs.IT

TL;DR: The paper introduces RDRSD, a hard-message-passing iterative decoder for Generalized Product Codes that achieves ~1dB coding gain over iBDD with comparable complexity, and proposes a soft-aided post-processing step to lower error floors for small component codes.


<details>
  <summary>Details</summary>
Motivation: To improve the decoding performance of Generalized Product Codes (GPCs) beyond existing hard-decision iterative bounded distance decoding (iBDD) while maintaining low hardware complexity.

Method: Proposes the refined dynamic reliability score decoder (RDRSD), a hard-message-passing iterative error-and-erasure decoder using dynamic reliability scores, implemented in the syndrome domain for low complexity. Also introduces a soft-aided post-processing step to address error floors.

Result: RDRSD achieves approximately 1dB coding gain over iBDD across various GPCs and configurations, with complexity comparable to hard-decision iBDD. The soft-aided post-processing significantly lowers error floors for GPCs with small error-correcting capability component codes.

Conclusion: RDRSD offers a favorable complexity-performance trade-off for GPC decoding, outperforming iBDD significantly while remaining hardware-efficient, and can be further enhanced with post-processing for low error floors.
...
### [280] [Precoding-based protocols for entanglement assisted linear computation over a quantum many-to-one network](https://arxiv.org/abs/2607.13756)
*Ruoyu Meng,Aditya Ramamoorthy*

Main category: cs.IT

TL;DR: This paper studies computing linear combinations over a noiseless quantum many-to-one network with k senders and one receiver. The senders share entanglement but cannot communicate, and Bob wants to compute a linear combination of their data vectors. The authors propose protocols that generalize prior work (N-Sum Box) by allowing more qudits and precoding, achieving lower or equal communication costs, and show subadditivity of communication cost across instances.


<details>
  <summary>Details</summary>
Motivation: To extend the N-Sum Box protocol to support a more general class of linear transformations in quantum many-to-one networks, where senders share entanglement but cannot communicate with each other, and to reduce communication costs.

Method: The authors give senders access to more qudits and allow them to judiciously precode their input symbols. They design protocols for computing general linear combinations over a noiseless quantum network with shared entanglement among senders.

Result: The proposed protocols achieve communication costs at most equal to the best-known prior results, and strictly lower in certain cases. The communication cost is shown to be subadditive across instances—computing two functions jointly can cost less than computing them individually.

Conclusion: The work generalizes existing quantum network computing protocols, achieves better communication efficiency in some scenarios, and reveals subadditivity in communication costs for linear function computation.
...
### [281] [CAS I: A Geometric Coding Theorem](https://arxiv.org/abs/2607.13796)
*Romie Banerjee*

Main category: cs.IT

TL;DR: The paper establishes a Coding Theorem analogue for symmetry groups, defines symmetry priors, proves universality for fix-retractable groups, develops a Galois connection between subgroups and binary strings, and unifies algorithmic information theory with group theory as part of a Computational Algorithmic Statistics series.


<details>
  <summary>Details</summary>
Motivation: To extend classical algorithmic information theory into the domain of symmetry groups by defining a symmetry-based prior that mirrors the universal prior in Solomonoff induction, and to unify group theory with algorithmic information theory.

Method: The paper defines symmetry priors as probabilities over randomly chosen symmetries from a group; proves that for fix-retractable groups (which admit computable sections selecting isolating symmetries), the symmetry prior is a universal lower semi-computable semi-measure; and develops a Galois connection between subgroups and binary strings.

Result: The symmetry prior for fix-retractable groups is a universal lower semi-computable semi-measure, establishing a Geometric Coding Theorem. A Galois connection between subgroups and binary strings is developed, with characterization of closed points and maximal closed subgroups.

Conclusion: This work unifies algorithmic information theory with group theory, providing a framework for symmetry-induced complexity measures, and serves as the foundational paper in a series on Computational Algorithmic Statistics.
...
### [282] [Safety-Aware Forward Detection in Networked ISAC for Low-Altitude UAV Flight](https://arxiv.org/abs/2607.13908)
*Jingli Li,Yiyan Ma,Wei Chen,Weijie Yuan,Qingqing Cheng,Tongyang Xu,Guoyu Ma,Mi Yang,Yunlong Lu,Wenwei Yue,Zhangdui Zhong*

Main category: cs.IT

TL;DR: This paper proposes a safety-aware forward detection design for networked ISAC systems, where multiple GBSs cooperate to support UAV downlink communication, state estimation, and non-cooperative target detection in the forward ROI. Key contributions include deriving CRLB and miss-detection probability scaling laws, formulating a resource optimization problem, and showing a 17.05% reduction in miss-detection probability with limited CRLB increase.


<details>
  <summary>Details</summary>
Motivation: Existing networked ISAC research mainly focuses on communication enhancement or target parameter estimation, but insufficiently addresses detection reliability of non-cooperative targets in the UAV forward region, which is critical for safe UAV flight in LAWNs.

Method: The forward ROI is determined by UAV position, velocity, and safe braking distance, then voxelized. CRLB for UAV state estimation and forward-ROI miss-detection probability are derived with scaling laws. A safety-aware resource optimization problem is formulated to jointly configure sensing pilot ratio, transmit power, and beam direction under communication-rate constraints.

Result: The proposed design reduces average miss-detection probability and sensing-induced collision risk by 17.05%, with only a 14.82% increase in average CRLB. Scaling laws show UAV state-estimation CRLB decreases as ln⁻²J, and miss-detection probability follows λₜD_f ln⁻²J.

Conclusion: Safety-aware forward detection in networked ISAC effectively balances communication, state estimation, and target detection for safer UAV flight, with manageable trade-offs in estimation performance.
...
### [283] [Square-Root Law for Covert Communication with Warden-Favorable Side Information](https://arxiv.org/abs/2607.14013)
*Hossein Ahmadi,Christian Deppe,Boulat A. Bash,Eduard A. Jorswieck*

Main category: cs.IT

TL;DR: This paper studies scalar Gaussian covert-overlay communication, where Alice's covert signal is hidden within a public component. Covertness is enforced via a relative-entropy constraint conditioned on Willie's side information. The maximal covert payload follows a square-root law with a constant governed by the total variance at Willie's detector (including cancellation error), not just receiver noise. Low-power Gaussian signaling achieves optimality, and time-varying residuals are handled via optimal power allocation.


<details>
  <summary>Details</summary>
Motivation: To establish fundamental limits and optimal strategies for covert communication when Willie has access to all physically obtainable side information and can subtract estimated public components, focusing on the practical overlay scenario where covert and public signals coexist.

Method: The paper models the residual after Willie's cancellation as a Gaussian innovation with known conditioned variance. Covertness is measured via conditional relative-entropy (KL divergence) budget δ. The analysis derives the maximal covert payload scaling (square-root law), identifies the optimal constant, and proves matching achievability (low-power Gaussian signaling) and converse results. For time-varying variances, a first-order optimal power allocation strategy is provided.

Result: The maximal covert payload is R_C^* √n (1+o(1)) bits, where R_C^* = (σ_0^2)/(σ_B^2 ln 2) √δ, with σ_0^2 = σ_W^2 + σ_e^2 (Willie's receiver noise plus cancellation error). Low-power Gaussian signaling achieves this constant, and the converse proves first-order optimality. For time-varying residuals, optimal allocation assigns more covert power to larger residual variances.

Conclusion: The square-root-law constant for covert communication depends on the total variance at Willie's detector input (including cancellation errors), not just receiver noise. The results hold under Gaussian post-cancellation residual with known conditioned variance; non-Gaussian residuals or fixed variance uncertainty are outside scope.
