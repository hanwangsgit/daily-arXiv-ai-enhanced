...
TL;DR: A-SelecT is a method that automatically selects the most information-rich timestep from Diffusion Transformer features in a single run, improving discriminative representation learning efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Current Diffusion Transformers (DiT) for discriminative tasks suffer from inefficient timestep searching and inadequate exploitation of DiT-specific features, limiting training efficiency and representational capacity.

Method: Introduces Automatically Selected Timestep (A-SelecT) that dynamically identifies DiT's most information-rich timestep from selected transformer features in a single run, eliminating exhaustive timestep searching and suboptimal feature selection.

Result: Extensive experiments on classification and segmentation benchmarks show that DiT with A-SelecT surpasses all prior diffusion-based approaches efficiently and effectively.

Conclusion: A-SelecT enables more efficient and effective discriminative representation learning from Diffusion Transformers by automating optimal timestep selection and feature exploitation.
...
TL;DR: This paper critiques OCR/document understanding evaluation biases toward modern/Western documents, revealing gaps in assessing historical Black newspapers and proposing structural solutions.


<details>
  <summary>Details</summary>
Motivation: Current OCR and document understanding evaluation focuses on modern, Western, institutional documents, masking system failures on historical and marginalized archives like Black historical newspapers, leading to structural invisibility and representational harm.

Method: Systematic literature review (2006-2025) using PRISMA framework to analyze OCR/document understanding papers and benchmark datasets, examining training data, benchmark design, and evaluation metrics for vision transformer and multimodal systems.

Result: Black newspapers and community-produced historical documents rarely appear in training data or benchmarks; evaluations focus on character accuracy and modern layouts, missing structural failures like column collapse, typographic errors, and hallucinated text common in historical newspapers.

Conclusion: Evaluation gaps stem from organizational and institutional structures shaped by benchmark incentives and data governance decisions, requiring systemic changes to address structural invisibility and representational harm in historical document analysis.
...
TL;DR: Using Stable Diffusion XL to generate synthetic ceramic surface images reduces need for expensive experimental data in AI-based roughness classification while maintaining comparable accuracy.


<details>
  <summary>Details</summary>
Motivation: AI for surface roughness classification requires large labeled datasets and costly high-resolution imaging equipment, creating barriers to practical implementation in materials engineering.

Method: Generate synthetic ceramic surface images using Stable Diffusion XL, augment authentic datasets with these synthetic images, and systematically vary training hyperparameters (epoch count, batch size, learning rate) to assess robustness.

Result: Augmenting authentic datasets with generative images yields test accuracies comparable to using exclusively experimental images, with identified configurations preserving performance while reducing data requirements.

Conclusion: Generative AI can substantially improve data efficiency and reliability in materials-image classification workflows, offering practical benefits of lower experimental cost, accelerated model development, and expanded AI applicability in materials engineering.
...
TL;DR: FPRL is a hierarchical self-supervised learning framework for endoscopic videos that mimics clinical examination by first focusing on lesion-centric static semantics within frames, then perceiving their temporal evolution across frames.
...
Method: FPRL employs a hierarchical semantic modeling mechanism: 1) Static semantics learning via teacher-prior adaptive masking (TPAM) with multi-view sparse sampling to focus on lesion-related local semantics; 2) Contextual semantics learning through cross-view masked feature completion (CVMFC) and attention-guided temporal prediction (AGTP) to model structured inter-frame evolution while preserving global contextual integrity.

Result: Extensive experiments on 11 endoscopic video datasets show FPRL achieves superior performance across diverse downstream tasks, demonstrating its effectiveness in endoscopic video representation learning.

Conclusion: FPRL provides an effective cognition-inspired framework for endoscopic video representation learning that addresses the limitations of existing methods by explicitly modeling both static lesion semantics and their temporal evolution, mimicking clinical examination patterns.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods are limited to rigid objects or require pre-scanned objects/multi-view videos. There's a significant gap in reconstructing 4D human-articulated-object interactions from single monocular RGB videos, which is highly ill-posed but important for understanding complex interactions.

Method: Optimization-based framework integrating multiple foundation model priors with novel refinement methods: 1) Adaptive Sampling Refinement (ASR) for optimizing object's metric scale and pose, 2) MLLM-guided hand-object alignment using contact reasoning as constraints for mesh composition optimization.

Result: Extensive experiments validate robustness and effectiveness across diverse objects and interactions. Two new datasets (ArtHOI-RGBD and ArtHOI-Wild) were created for comprehensive evaluation.

Conclusion: ArtHOI successfully addresses the challenging problem of 4D human-articulated-object interaction reconstruction from single monocular videos by leveraging and refining foundation model priors, enabling realistic reconstruction of complex interactions.
...
TL;DR: FA-CNN is a CNN architecture with intrinsic class attribution through end-to-end feature alignment, using order-preserving layers to maintain alignment from input pixels to final class logits, making feature maps interpretable and identical to Grad-CAM saliency maps.


<details>
  <summary>Details</summary>
Motivation: Standard CNN operations like Linear and Conv2D cause unnecessary shuffling and mixing of semantic concepts, making raw feature maps difficult to understand and interpret.

Method: Introduces two new order-preserving layers: dampened skip connection and global average pooling classifier head, which force end-to-end feature alignment from input pixels to final class logits.

Result: Theoretically proves FA-CNN penultimate feature maps are identical to Grad-CAM saliency maps, and shows feature maps morph layer-by-layer toward class activations. Performs well on benchmark datasets and compares favorably in interpretability tasks.

Conclusion: FA-CNN enhances interpretability through intrinsic class attribution, with theoretical guarantees about feature map properties, and discusses limitations and extensions toward hybrid models.
...
TL;DR: LEMON is a self-supervised foundation model for single-cell image representation learning in computational pathology, trained on millions of cell images from diverse tissues and cancer types.


<details>
  <summary>Details</summary>
Motivation: While self-supervised learning has advanced patch and whole-slide image analysis in computational pathology, single-cell level representation learning remains underexplored despite its importance for characterizing cell types and cellular phenotypes.

Method: LEMON (Learning Embeddings from Morphology Of Nuclei) is a self-supervised foundation model that learns morphological representations from millions of single-cell images across diverse tissues and cancer types.

Result: LEMON demonstrates strong performance across five benchmark datasets on various prediction tasks, showing robust and versatile morphological representations that support large-scale single-cell analyses.

Conclusion: LEMON represents a new paradigm for cell-level computational pathology, with model weights publicly available for research use.
...
<details>
  <summary>Details</summary>
Motivation: To validate and extend Darcet et al.'s (2024) findings about attention map artifacts in Vision Transformers, testing whether their claims about registers eliminating artifacts generalize across different models and model sizes.

Method: Reproduced Darcet et al.'s experiments and evaluated their claims across multiple Vision Transformer models including DINO, DINOv2, OpenCLIP, and DeiT3. Extended analysis to smaller models and addressed terminology inconsistencies.

Result: Confirmed some key claims but found others don't generalize universally across all models. Successfully extended findings to smaller models and clarified terminology inconsistencies that impact generalization.

Conclusion: While Darcet et al.'s core findings about attention artifacts and registers are valid, they don't universally apply to all Vision Transformer models, highlighting the importance of model-specific considerations and consistent terminology.
...
<details>
  <summary>Details</summary>
Motivation: Geometric Foundation Models (GFMs) show strong 3D geometric feature extraction capabilities, but their potential for cross-view geo-spatial tasks (CVGL and CVIS) remains underexplored due to the large viewpoint gap between ground and aerial imagery.

Method: Proposes GeoMap to embed ground and aerial features into a shared 3D-aware latent space to reduce cross-view discrepancies, and GeoFlow, a flow-matching model conditioned on geometry-aware latent embeddings for bidirectional image synthesis, with consistency loss for alignment.

Result: Extensive experiments on CVUSA, CVACT, and VIGOR benchmarks demonstrate state-of-the-art performance in both localization and synthesis tasks.

Conclusion: Geo^2 effectively leverages 3D geometric priors from foundation models to unify cross-view geo-spatial learning, demonstrating the power of geometric understanding for bridging ground-aerial viewpoint gaps.
...
TL;DR: ViGoR is a new benchmark that exposes the "logical desert" in modern AIGC models by evaluating their reasoning capabilities across vision tasks, revealing that even state-of-the-art models have significant reasoning deficits despite their visual fidelity.


<details>
  <summary>Details</summary>
Motivation: Current AIGC models create a "performance mirage" - they have stunning visual fidelity but fail at tasks requiring physical, causal, or complex spatial reasoning. Existing evaluations use superficial metrics and fragmented benchmarks that overlook the generative process, creating a misleading impression of model capabilities.

Method: ViGoR introduces four key innovations: 1) holistic cross-modal coverage bridging Image-to-Image and Video tasks; 2) dual-track evaluation of both intermediate processes and final results; 3) evidence-grounded automated judge with high human alignment; and 4) granular diagnostic analysis decomposing performance into fine-grained cognitive dimensions.
...
Conclusion: ViGoR successfully dismantles the "performance mirage" by providing a unified framework that exposes the "logical desert" in modern AIGC models, serving as an essential benchmark for evaluating and advancing reasoning capabilities in vision models.
...
TL;DR: A feed-forward method for dense Signed Distance Field (SDF) regression from image collections in under 3 seconds, using pretrained geometry transformer features directly for 3D extraction without per-view prediction or post-hoc fusion.


<details>
  <summary>Details</summary>
Motivation: Existing pipelines discard valuable completeness information by routing features through per-view prediction heads before assembling 3D geometry, which accumulates inaccuracies. The intermediate feature space of pretrained multi-view geometry transformers already encodes a powerful joint world representation that should be used directly.

Method: Perform 3D extraction directly from geometry transformer features via learned volumetric extraction: voxelized canonical embeddings that progressively absorb multi-view geometry information through interleaved cross- and self-attention into a structured volumetric latent grid. A convolutional decoder then maps this grid to dense SDF. Includes scalable, validity-aware supervision using SDFs from depth maps or 3D assets.
...
Conclusion: Direct 3D extraction from pretrained geometry transformer features via volumetric latent grids enables fast, accurate SDF regression from image collections, outperforming methods that discard valuable joint world representations through per-view processing.
...
TL;DR: GazeQwen adds gaze awareness to multimodal LLMs via hidden-state modulation with a compact gaze resampler, achieving state-of-the-art performance on video understanding tasks with gaze cues.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs fail to effectively utilize eye-gaze information for video understanding, even when gaze cues are provided through visual overlays or text descriptions.

Method: Parameter-efficient approach using a compact gaze resampler (1-5M parameters) that encodes V-JEPA 2.1 video features with fixation-derived positional encodings, producing additive residuals injected into selected LLM decoder layers via forward hooks. Optional second stage adds LoRA adapters for tighter integration.

Result: Achieves 63.9% accuracy on StreamGaze benchmark, +16.1 points over same backbone with gaze as visual prompts and +10.5 points over GPT-4o, highest among all tested models (open-source and proprietary).

Conclusion: Learning where to inject gaze within an LLM is more effective than scaling model size or engineering better prompts. The approach enables effective gaze-aware video understanding with minimal parameter overhead.
...
TL;DR: A CNN-based method for dynamic hand gesture recognition using MediaPipe Hand Landmarker for skeletal keypoint extraction, applied to LIBRAS gestures for home automation control with 95% accuracy in low-light conditions.


<details>
  <summary>Details</summary>
Motivation: To develop an effective real-time hand gesture recognition system for device control in home automation using Brazilian Sign Language (LIBRAS) gestures, addressing the need for accessible human-computer interaction interfaces.

Method: Two-stage approach: 1) MediaPipe Hand Landmarker extracts 21 skeletal keypoints from hand images, 2) CNN classifies gestures from spatiotemporal matrix representation (90×21 dimensions) using sliding window with temporal frame triplication for real-time inference without recurrent networks.

Result: Achieved 95% accuracy under low-light conditions and 92% under normal lighting for 11 classes of static and dynamic LIBRAS gestures, demonstrating effective real-time recognition capability.

Conclusion: The proposed method is effective for LIBRAS gesture recognition in home automation applications, though systematic experiments with greater user diversity are needed to better evaluate generalization capabilities.
...
TL;DR: GUIDE is a benchmark for evaluating GUI agents' ability to understand user behavior, infer intent, and provide assistance in open-ended software tasks, showing current models struggle but improve significantly with user context.


<details>
  <summary>Details</summary>
Motivation: Current GUI automation research focuses too much on automating clicks/keystrokes without understanding user intention. Users value exploration, iteration, and refinement while maintaining agency. To move from automation to collaboration, GUI agents need to understand what users are doing and why.
...
Result: Eight state-of-the-art multimodal models achieved only 44.6% accuracy on behavior state detection and 55.0% on help prediction. However, providing user context significantly improved performance, raising help prediction by up to 50.2 percentage points.

Conclusion: Current GUI agents struggle with understanding user intent and behavior, but structured user understanding is critical for effective assistance. The GUIDE benchmark enables evaluation of collaborative GUI agents that can work with users rather than just automate tasks.
...
TL;DR: Transformer-based surgical desmoking model with physics-inspired head that jointly predicts smoke-free images and smoke maps, trained on large synthetic and real datasets, achieving SOTA performance and evaluated on downstream tasks.
...
Conclusion: The proposed transformer-based desmoking model effectively addresses surgical smoke degradation, with comprehensive datasets enabling robust training and benchmarking, though digital smoke removal methods still have limitations for downstream vision tasks.
...
<details>
  <summary>Details</summary>
Motivation: Whiteboard-style educational videos require precise coordination between freehand illustrations and spoken narration, but no existing methods address this multimodal synchronization problem with structured, reproducible drawing representations.

Method: Created dataset of 24 paired Excalidraw demonstrations with narrated audio across 8 STEM domains, each drawing element with millisecond-precision creation timestamps. Fine-tuned Qwen2-VL-7B model via LoRA to predict full stroke sequences synchronized to speech from only 24 demonstrations.
...
TL;DR: AI models for blind sweep obstetric ultrasound are sensitive to acquisition quality issues, and automated quality assessment with feedback loops can improve reliability in low-resource settings.


<details>
  <summary>Details</summary>
Motivation: Blind Sweep Obstetric Ultrasound (BSOU) enables scalable fetal imaging in low-resource settings, but AI system reliability depends on sweep quality, and little is known about how protocol deviations affect predictions.

Method: Systematically evaluate BSOU quality impact on three AI tasks by simulating acquisition deviations (reversed direction, probe inversion, incomplete sweeps), develop automated quality-assessment models, and simulate feedback loops for re-acquisition.

Result: BSOU-based AI models are sensitive to acquisition variability, but automated quality assessment can detect perturbations and feedback loops improve downstream task performance when flagged sweeps are re-acquired.

Conclusion: Automated quality assessment plays a central role in building reliable, scalable AI-assisted prenatal ultrasound workflows, particularly in low-resource environments where BSOU is deployed.
...
TL;DR: WR-Arena is a new benchmark for evaluating world models beyond next-state prediction, focusing on action simulation fidelity, long-horizon forecasting, and simulative reasoning/planning capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing world model benchmarks are too narrow, focusing only on next-state prediction and visual fidelity, while overlooking richer simulation capabilities needed for intelligent behavior. There's a gap in evaluating the fundamental dimensions of world simulation required for intelligent agents.

Method: The authors introduce WR-Arena benchmark with three evaluation dimensions: (1) Action Simulation Fidelity (interpreting multi-step instructions, generating counterfactual rollouts), (2) Long-horizon Forecast (sustaining accurate, coherent simulations across extended interactions), and (3) Simulative Reasoning and Planning (supporting goal-directed reasoning by simulating and comparing alternative futures). They build a task taxonomy and curate diverse datasets to probe these capabilities.
...
Conclusion: WR-Arena addresses the limitations of existing benchmarks by providing comprehensive evaluation of world models' simulation capabilities, establishing a framework for developing world models capable of robust understanding, forecasting, and purposeful action.
...
TL;DR: Siamese personalization for eye tracking achieves comparable performance to linear calibration with 10x fewer samples, reducing gaze error by up to 12% with polarization inputs and 13% when combined with linear calibration.


<details>
  <summary>Details</summary>
Motivation: Head-mounted eye tracking devices require per-user calibration due to inter-person variability, which is inconvenient. The paper aims to develop a more efficient personalization approach that reduces calibration burden while maintaining accuracy.

Method: Uses Siamese architectures to learn relative gaze displacements and reconstruct absolute gaze from a small set of calibration frames. Benchmarks this approach on polarization-enabled eye tracking using a 338-subject dataset captured with polarization-sensitive camera and 850 nm illumination.
...
Conclusion: Siamese personalization is a practical approach enabling accurate eye tracking with significantly reduced calibration requirements, especially effective when combined with polarization sensing.
...
TL;DR: The paper introduces FSIR-BD, the first benchmark for few-shot text-to-image retrieval focusing on compositional and OOD queries, and proposes two novel few-shot optimization methods that work with any pre-trained image encoder.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with compositional queries and out-of-distribution image-text pairs, creating a performance gap compared to human cognition which can learn from minimal examples. The authors aim to address this through few-shot learning approaches for image retrieval.

Method: 1) Introduces FSIR-BD benchmark dataset with 38,353 images and 303 queries, divided into compositional (urban scenes and nature species) and OOD categories. 2) Proposes two novel retrieval optimization methods leveraging single-shot or few-shot reference examples from the FSR (few-shot reference corpus) to improve performance. Both methods are compatible with any pre-trained image encoder.

Result: 1) FSIR-BD provides a challenging benchmark for image retrieval. 2) The proposed optimization methods outperform existing baselines as measured by mean Average Precision (mAP).

Conclusion: The FSIR task and benchmark address limitations in current VLMs for compositional and OOD queries. The proposed few-shot optimization methods show promising results and can help narrow the gap between machine and human-level understanding for compositional reasoning from limited examples.
...
TL;DR: The paper introduces Multimodal Coherence Score (MCS), a metric to evaluate multimodal fusion quality independent of downstream task accuracy, focusing on coherence across identity, spatial, semantic, and decision dimensions.


<details>
  <summary>Details</summary>
Motivation: Current multimodal AI evaluation relies on downstream task accuracy, but high accuracy doesn't guarantee coherent multimodal inputs (e.g., a model can score well on VQA while its visual and textual inputs contradict each other). There's a need for a metric that directly assesses fusion quality independent of any specific model.
...
Result: MCS discriminates fusion quality with higher sensitivity than task accuracy alone (Spearman rho = 0.093 vs. 0.071). Perturbation experiments show each dimension responds independently to its failure mode with zero cross-talk. MCS is lightweight, requires no human annotation, and identifies specific failure modes.

Conclusion: MCS provides a practical, model-agnostic metric for evaluating multimodal coherence that reveals not just when fusion fails, but specifically what aspect of coherence breaks down, offering more diagnostic power than task accuracy alone.
...
TL;DR: THFM is a unified video foundation model for human-centric perception that handles both dense (depth, normals, segmentation, pose) and sparse (keypoint) tasks using a single architecture derived from pretrained text-to-video diffusion models, achieving state-of-the-art results with synthetic-only training.


<details>
  <summary>Details</summary>
Motivation: To create a unified perception model that can handle multiple human-centric vision tasks (both dense and sparse) within a single architecture, eliminating the need for specialized models for each task while leveraging the power of pretrained diffusion models.

Method: Derives THFM from a pretrained text-to-video diffusion model, repurposing it as a single-forward-pass perception model augmented with learnable tokens for sparse predictions. Uses text prompts to modulate task performance and trains exclusively on synthetic data.

Result: Achieves performance on-par or surpassing state-of-the-art specialized models across various benchmarks despite synthetic-only training. Demonstrates emergent generalization capabilities to multiple humans and other object classes beyond training data.

Conclusion: THFM successfully demonstrates that a single unified model can handle diverse human-centric perception tasks effectively, with synthetic training being sufficient for real-world performance, and reveals intriguing emergent properties from diffusion-based video representations.
...
TL;DR: DiReCT improves video generation physics by disentangling semantic and physical conditioning through macro/micro contrastive learning, boosting physical commonsense scores without extra training time.


<details>
  <summary>Details</summary>
Motivation: Current flow-matching video generators produce high-quality outputs but violate physics because their reconstruction objectives don't distinguish physically possible from impossible dynamics. Contrastive flow matching helps but suffers from semantic-physics entanglement in text-conditioned settings.

Method: DiReCT (Disentangled Regularization of Contrastive Trajectories) uses a two-scale approach: macro-contrastive term with semantically distant negatives for global trajectory separation, and micro-contrastive term with hard negatives that share scene semantics but differ in specific physical behaviors (kinematics, forces, materials, interactions, magnitudes) via LLM perturbations. Includes velocity-space distributional regularizer to preserve visual quality.

Result: Applied to Wan 2.1-1.3B, improves physical commonsense score on VideoPhy by 16.7% over baseline and 11.3% over SFT, without increasing training time.

Conclusion: DiReCT effectively addresses semantic-physics entanglement in contrastive video generation by disentangling conditioning signals, enabling improved physical realism while maintaining visual quality and training efficiency.
...
TL;DR: SCOTTI is a multi-task learning model that uses foot tactile signals to simultaneously estimate 3D human pose, classify actions, and predict action completion progress, outperforming single-task approaches.


<details>
  <summary>Details</summary>
Motivation: Vision-based methods for human-robot interaction suffer from occlusion and privacy issues in realistic environments. Tactile sensing avoids these problems, but existing tactile approaches handle pose estimation, action classification, and progress prediction separately, leading to suboptimal performance.

Method: Proposed SCOTTI (Shared COnvolutional Transformer for Tactile Inference) - a unified model that learns shared representations from foot tactile signals using custom wireless insole sensors. The model simultaneously addresses three prediction tasks through multi-task learning.

Result: SCOTTI outperforms existing approaches across all three tasks. The model demonstrates improved performance compared to learning tasks independently, showing the benefits of multi-task learning. A novel dataset was collected from 15 participants performing 8 different activities over 7 hours.

Conclusion: The unified multi-task learning approach using tactile sensing effectively addresses occlusion and privacy concerns while improving performance across pose estimation, action classification, and progress prediction tasks. This is the first work to explore action progress prediction using foot tactile signals.
...
TL;DR: Hybrid Dense SwinV2 combines DenseNet for local features and Swin Transformer V2 for global context to classify cassava diseases with 98.02% accuracy.


<details>
  <summary>Details</summary>
Motivation: To improve cassava disease classification by capturing both fine local structural details and long-range global contextual dependencies, which is crucial for distinguishing visually similar lesions in real-world field conditions with occlusion, noise, and complex backgrounds.

Method: Two-branch framework: DenseNet branch captures high-resolution local features, while customized Swin Transformer V2 branch models global contextual dependencies using shifted-window self-attention. Each branch uses attention channel-squeeze modules to emphasize disease-related responses and suppress background noise. Features from both branches are fused for refined representations.

Result: Achieved 98.02% classification accuracy and 97.81% F1 score on a public dataset of 31,000 cassava leaf images covering five diseases (brown streak, mosaic, green mottle, bacterial blight, and normal leaves), outperforming established convolutional and transformer models.

Conclusion: Hybrid Dense SwinV2 offers robust and practical field-level diagnosis of cassava diseases, effectively handling real-world challenges like occlusion, noise, and complex backgrounds through its complementary local-global feature learning approach.
...
TL;DR: GeoReFormer is a transformer-based architecture for 3D lane detection that embeds geometric and topological priors directly in the decoder, achieving state-of-the-art performance on OpenLane-V2.


<details>
  <summary>Details</summary>
Motivation: Existing transformer-based approaches for 3D lane detection inherit decoder designs from compact object detection, but lane segments are continuous polylines in directed graphs. Current methods don't explicitly encode this geometric and relational structure during query initialization and refinement.
...
Conclusion: Explicit encoding of geometric and relational structure within transformer decoders is beneficial for 3D lane detection and topology reasoning, demonstrating the utility of domain-specific inductive biases in query-based architectures.
...
TL;DR: SDRL is a single-stage RL framework for video understanding that eliminates SFT by using Summarize->Think->Answer format with self-supervised mechanisms for factual grounding and reasoning diversity.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs for video understanding suffer from thinking drift and weak temporal comprehension. RL methods like GRPO still depend on costly SFT with CoT annotations, which enforce fixed reasoning paths, limit generalization, and induce bias.

Method: Introduces Summary-Driven Reinforcement Learning (SDRL) - a single-stage RL framework without SFT. Uses Structured CoT format: Summarize -> Think -> Answer. Integrates two self-supervised mechanisms into GRPO: 1) Consistency of Vision Knowledge (CVK) reduces KL divergence among generated summaries for factual grounding; 2) Dynamic Variety of Reasoning (DVR) dynamically modulates thinking diversity based on group accuracy to balance alignment and exploration.

Result: Achieves state-of-the-art performance on seven public VideoQA datasets.

Conclusion: SDRL effectively overcomes limitations of existing RL methods by eliminating SFT dependency, balancing alignment and exploration through self-supervised mechanisms, and supervising both final answers and reasoning processes for better video understanding.
...
TL;DR: 4DRaL: A knowledge distillation framework that uses LiDAR-to-LiDAR place recognition as teacher to enhance 4D radar place recognition performance, addressing noise and sparsity issues in 4D radar data.


<details>
  <summary>Details</summary>
Motivation: Place recognition is essential for robotics but current camera/LiDAR systems fail in adverse weather. 4D radar works in all weather but suffers from noise and sparsity, limiting its place recognition performance.

Method: Knowledge distillation framework with three modules: 1) local image enhancement for handling sparse radar points, 2) feature distribution distillation for discriminative features, 3) response distillation for teacher-student consistency. Can be configured for both radar-to-radar and radar-to-LiDAR recognition.

Result: Achieves state-of-the-art performance in both radar-to-radar and radar-to-LiDAR place recognition tasks, regardless of normal or adverse weather conditions.

Conclusion: 4DRaL effectively addresses 4D radar's limitations through knowledge distillation, enabling robust all-weather place recognition for robotics applications.
...
### [29] [Collision-Aware Vision-Language Learning for End-to-End Driving with Multimodal Infraction Datasets](https://arxiv.org/abs/2603.25946)
*Alex Koran,Dimitrios Sinodinos,Hadi Hojjati,Takuya Nanri,Fangge Chen,Narges Armanfard*

Main category: cs.CV

TL;DR: The paper introduces VLAAD, a collision-aware representation learning module for autonomous driving that improves driving scores by 14.12% when integrated into existing models, and outperforms larger vision-language models on real-world collision detection.


<details>
  <summary>Details</summary>
Motivation: High infraction rates, particularly collisions, remain the primary bottleneck for end-to-end autonomous driving systems. Despite collisions being the dominant failure mode, collision-aware representation learning has received limited attention in the field.

Method: Developed VLAAD (Video-Language-Augmented Anomaly Detector) using Multiple Instance Learning formulation for stable, temporally localized collision signals. Created CARLA-Collide dataset for simulator training and Real-Collide dataset for real-world evaluation. VLAAD serves as a plug-in module that can be integrated into existing E2E driving models.

Result: When integrated into a pretrained TransFuser++ agent, VLAAD achieved a 14.12% relative increase in driving score with minimal fine-tuning. In open-loop evaluation on real-world data, VLAAD (0.6B parameters) outperformed a multi-billion-parameter vision-language model with a 23.3% improvement in AUC.

Conclusion: VLAAD effectively addresses the collision bottleneck in autonomous driving through collision-aware representation learning, demonstrating significant improvements in both simulated and real-world settings while being computationally efficient compared to larger models.
...
### [30] [DRUM: Diffusion-based Raydrop-aware Unpaired Mapping for Sim2Real LiDAR Segmentation](https://arxiv.org/abs/2603.26263)
...
<details>
  <summary>Details</summary>
Motivation: LiDAR semantic segmentation requires expensive real-world annotation, and models trained on synthetic data underperform due to domain gaps between simulated and real LiDAR measurements.

Method: Uses diffusion model pre-trained on unlabeled real data as generative prior, translates synthetic data by reproducing reflectance intensity and raydrop noise characteristics, and introduces raydrop-aware masked guidance for selective consistency.

Result: DRUM consistently improves Sim2Real performance across multiple LiDAR data representations, demonstrating effective domain adaptation.

Conclusion: The proposed framework effectively bridges the Sim2Real gap for LiDAR semantic segmentation by leveraging diffusion models to generate realistic LiDAR characteristics from synthetic data.
...
### [31] [Drive-Through 3D Vehicle Exterior Reconstruction via Dynamic-Scene SfM and Distortion-Aware Gaussian Splatting](https://arxiv.org/abs/2603.26638)
*Nitin Kulkarni,Akhil Devarashetti,Charlie Cluss,Livio Forte,Philip Schneider,Chunming Qiao,Alina Vereshchaka*

Main category: cs.CV

TL;DR: End-to-end pipeline for high-fidelity 3D vehicle reconstruction in cluttered dealership environments using two-pillar camera rig, motion-gating, learned matchers, and distortion-aware 3D Gaussian Splatting.


<details>
  <summary>Details</summary>
Motivation: Online automotive marketplaces need high-fidelity 3D vehicle reconstructions to improve buyer confidence, but current methods fail in cluttered dealership drive-throughs with dynamic vehicles, wide-angle distortion, specular paint, and non-rigid wheel rotations.

Method: Four-stage pipeline: 1) SAM 3 instance segmentation with motion-gating to isolate moving vehicle and mask non-rigid wheels; 2) RoMa v2 learned matcher on raw distorted 4K imagery with semantic confidence masks; 3) Rig-aware SfM optimization with CAD-derived pose priors; 4) Distortion-aware 3D Gaussian Splatting (3DGUT) with stochastic MCMC densification for reflective surfaces.

Result: Achieves PSNR 28.66 dB, SSIM 0.89, LPIPS 0.21 on 25 real-world vehicles across 10 dealerships, representing 3.85 dB improvement over standard 3D-GS, delivering inspection-grade interactive 3D models without studio infrastructure.

Conclusion: The proposed pipeline successfully addresses severe technical challenges of vehicle reconstruction in cluttered dealership environments, enabling high-fidelity 3D models that improve buyer confidence in online automotive marketplaces without requiring controlled studio setups.
...
TL;DR: FairLLaVA is a parameter-efficient fine-tuning method that reduces demographic biases in multimodal large language models for medical imaging tasks while maintaining overall performance.


<details>
  <summary>Details</summary>
Motivation: Multimodal large language models (MLLMs) exhibit uneven performance across demographic groups, creating fairness risks in clinical settings where such disparities could lead to unequal diagnostic narratives and erode trust in AI-assisted decision-making. While fairness has been studied in vision-only and language-only models, its impact on MLLMs remains underexplored.

Method: FairLLaVA uses parameter-efficient fine-tuning that minimizes mutual information between target attributes to regularize model representations to be demographic-invariant. It functions as a lightweight plug-in using low-rank adapter fine-tuning, providing an architecture-agnostic approach to fair visual instruction following.
...
Conclusion: FairLLaVA effectively mitigates demographic biases in MLLMs for medical applications through parameter-efficient fine-tuning, addressing fairness concerns in clinical AI systems without compromising overall performance.
...
TL;DR: LRM-Functa introduces low-rank modulation in Functa-based models for ultrasound videos, creating structured latent spaces that enable interpretable temporal analysis and outperform prior methods in cardiac cycle frame detection.


<details>
  <summary>Details</summary>
Motivation: Current Functa-based approaches for image representation lack structured and interpretable latent spaces, particularly for temporal data like ultrasound videos where understanding cardiac cycle patterns is clinically important.

Method: Proposes Low-Rank-Modulated Functa (LRM-Functa) that enforces low-rank adaptation of modulation vectors in time-resolved latent space, creating structured periodic trajectories for ultrasound video analysis.

Result: LRM-Functa outperforms prior methods in unsupervised ED/ES frame detection, compresses videos to rank k=2 without performance loss, enables smooth frame sampling, and generalizes to lung ultrasound B-line classification.

Conclusion: LRM-Functa provides a compact, interpretable, and generalizable framework for ultrasound video analysis with structured latent spaces that facilitate clinical interpretation and downstream tasks.
...
TL;DR: LVLMs (GPT-4o, Claude 3.5 Sonnet, LLaMA 3.2 Vision) achieve competitive zero-shot facial age estimation on UTKFace and FG-NET benchmarks without fine-tuning, but show fairness and quality sensitivity issues.


<details>
  <summary>Details</summary>
Motivation: Traditional deep learning for facial age estimation requires extensive labeled data and domain-specific training. This study explores whether large vision-language models (LVLMs) can perform accurate zero-shot age estimation without task-specific adaptation, potentially enabling broader applications in biometrics, healthcare, and human-computer interaction.

Method: Zero-shot evaluation of GPT-4o, Claude 3.5 Sonnet, and LLaMA 3.2 Vision on UTKFace and FG-NET datasets without fine-tuning. Used eight evaluation metrics (MAE, MSE, RMSE, MAPE, MBE, R², CCC, ±5-year accuracy) and analyzed performance across image quality and demographic subgroups.

Result: General-purpose LVLMs deliver competitive performance in zero-shot facial age estimation, demonstrating emergent capabilities for biometric tasks. However, performance disparities exist based on image quality and demographic subgroups, highlighting fairness concerns.

Conclusion: LVLMs show promise as zero-shot tools for real-world age estimation applications in forensic science, healthcare, and HCI, but challenges remain in prompt sensitivity, interpretability, computational cost, and demographic fairness that require further research.
...
TL;DR: BEVMapMatch: A framework for robust vehicle re-localization in GNSS-denied/degraded environments using multimodal BEV segmentation and cross-attention based map matching without GNSS priors.


<details>
  <summary>Details</summary>
Motivation: Localization in GNSS-denied and GNSS-degraded environments is challenging for autonomous vehicle deployment, requiring alternative robust localization methods that don't rely on GNSS.

Method: Uses lidar+camera fusion to generate multimodal Bird's Eye View segmentations in various weather conditions. Employs cross-attention based search to retrieve candidate map patches, then performs finer alignment of top candidates against generated BEV segmentation. Multiple frames improve accuracy.

Result: Outperforms existing re-localization methods in GNSS-denied/adverse environments with Recall@1m of 39.8% (nearly twice as much as best baseline). Code and data will be publicly available.

Conclusion: BEVMapMatch provides accurate global localization without GNSS, demonstrating robust performance in challenging environments through multimodal sensor fusion and attention-based map matching.
...
TL;DR: Unsupervised domain adaptation framework for automated extraction of Type A Aortic Dissection clinical features without target-domain annotations, enabling cross-institutional deployment.


<details>
  <summary>Details</summary>
Motivation: Current TAAD research focuses on segmentation accuracy but neglects reliable extraction of clinically actionable features. Building comprehensive datasets requires expert annotations that are impractical for most institutions, and models trained on single-center data suffer from performance degradation during cross-institutional deployment due to domain shift.

Method: Proposed an unsupervised domain adaptation (UDA)-driven framework that leverages limited source-domain labels while adapting to unlabeled target-domain data. The framework achieves stable cross-institutional multi-class segmentation and reliable clinical feature extraction without requiring high-cost target-domain annotations.

Result: Extensive experiments show significant improvement in cross-domain segmentation performance compared to state-of-the-art approaches. A reader study with cardiovascular surgeons confirms that automatically extracted clinical features provide meaningful assistance for preoperative assessment.

Conclusion: The proposed end-to-end segmentation-to-feature pipeline offers practical utility for TAAD preoperative evaluation, enabling reliable clinical feature extraction during cross-institutional deployment without requiring target-domain annotations.
...
TL;DR: EEG-guided RL framework for autonomous driving uses human brain signals (ERP responses to sudden changes) as cognitive feedback to improve collision avoidance, avoiding manual preference ranking.


<details>
  <summary>Details</summary>
Motivation: Training autonomous driving systems to align with human expectations is challenging. Current RLHF methods require time-consuming manual preference ranking, while humans have sophisticated cognitive systems that rapidly interpret scenes. The authors aim to incorporate direct human cognitive insights without behavioral interruption.

Method: Collected EEG signals from 20 participants in a realistic driving simulator, analyzing event-related potentials (ERP) in response to sudden environmental changes. Used a neural network to predict ERP strength from visual scene information, then integrated this cognitive information into RL reward signals.

Result: The EEG-guided framework improved the collision avoidance ability of the RL algorithm, demonstrating the potential of neuro-cognitive feedback for enhancing autonomous driving systems.

Conclusion: Direct neuro-cognitive feedback (EEG/ERP signals) provides a promising alternative to manual preference ranking in RLHF for autonomous driving, enabling better alignment with human cognitive processes and improving safety performance.
...
### [38] [Diffusion MRI Transformer with a Diffusion Space Rotary Positional Embedding (D-RoPE)](https://arxiv.org/abs/2603.25977)
...
TL;DR: D-RoPE: A novel positional embedding for dMRI transformers that captures spatial and directional dependencies, enabling robust representation learning across varying acquisition protocols.
...
Method: Introduces diffusion space rotatory positional embedding (D-RoPE) plugged into a dMRI transformer to jointly model spatial structure and directional characteristics. Uses self-supervised masked autoencoding pretraining.

Result: Learned representations provide competitive/superior performance across downstream tasks. Finetuned features achieved 6% higher accuracy in mild cognitive impairment classification and 0.05 increase in correlation coefficient for cognitive score prediction.

Conclusion: D-RoPE enables robust and transferable dMRI representations across diverse acquisition settings, addressing key challenges in learning from diffusion MRI data.
...
TL;DR: CoGaze is a medical vision-language pretraining framework that incorporates clinical context and radiologists' gaze to improve diagnostic reasoning in chest X-ray analysis.


<details>
  <summary>Details</summary>
Motivation: Existing medical vision-language models fail to capture the diagnostic workflow by treating radiographs as context-agnostic images and ignoring radiologists' gaze patterns, which hinders disease-specific pattern modeling and weakens cross-modal alignment.

Method: 1) Context-infused vision encoder that models how radiologists integrate clinical context; 2) Multi-level supervision with hybrid-positive contrastive learning for semantic alignment, disease-aware cross-modal representation learning, and gaze-guided attention using radiologists' gaze as probabilistic priors.
...
Conclusion: Incorporating clinical context and radiologists' gaze significantly improves medical vision-language pretraining for chest X-ray analysis, enabling better modeling of diagnostic workflows and achieving superior performance across multiple diagnostic tasks.
...
### [40] [JRM: Joint Reconstruction Model for Multiple Objects without Alignment](https://arxiv.org/abs/2603.25985)
...
TL;DR: JRM is a 3D flow-matching generative model that leverages object repetition across scenes by implicitly aggregating unaligned observations in latent space, improving reconstruction quality without explicit alignment.


<details>
  <summary>Details</summary>
Motivation: Current object-centric reconstruction methods treat objects independently, discarding valuable repetition signals where the same object appears multiple times. Prior methods using explicit matching and rigid alignment are error-prone and can't handle non-rigid transformations.
...
Conclusion: Implicit aggregation through flow-matching generative models provides a more robust and flexible approach to leveraging object repetition in 3D reconstruction, handling both rigid and non-rigid transformations without explicit alignment constraints.
...
TL;DR: MaLSF is a novel multimodal misinformation detection framework that uses active bidirectional verification with mask-label pairs to detect subtle semantic inconsistencies between images and text, outperforming previous methods.


<details>
  <summary>Details</summary>
Motivation: Current multimodal verification methods use passive holistic fusion that suffers from 'feature dilution' - global alignments average out subtle local semantic inconsistencies, masking the very conflicts needed to detect sophisticated misinformation.

Method: MaLSF introduces mask-label pairs as semantic anchors to bridge pixels and words, with two key innovations: 1) Bidirectional Cross-modal Verification (BCV) module using parallel query streams (Text-as-Query and Image-as-Query) to explicitly pinpoint conflicts, and 2) Hierarchical Semantic Aggregation (HSA) module that aggregates multi-granularity conflict signals for task-specific reasoning. Also includes diverse mask-label pair extraction parsers.
...
Conclusion: MaLSF successfully addresses the limitations of passive holistic fusion by shifting to active bidirectional verification, mimicking human cognitive cross-referencing to detect sophisticated multimodal misinformation through explicit conflict identification.
...
### [42] [FAST3DIS: Feed-forward Anchored Scene Transformer for 3D Instance Segmentation](https://arxiv.org/abs/2603.25993)
*Changyang Li,Xueqing Huang,Shin-Fang Chng,Huangying Zhan,Qingan Yan,Yi Xu*

Main category: cs.CV

TL;DR: FAST3DIS is an end-to-end feed-forward 3D instance segmentation method that replaces traditional "lift-and-cluster" approaches with a 3D-anchored query-based Transformer architecture, eliminating post-hoc clustering for better efficiency and scalability.


<details>
  <summary>Details</summary>
Motivation: Current feed-forward 3D reconstruction models rely on disjointed "lift-and-cluster" approaches for instance segmentation, which use non-differentiable clustering that scales poorly with multiple views and disconnects representation learning from the final segmentation objective.

Method: Introduces a 3D-anchored query-based Transformer architecture built on a depth backbone, with learned 3D anchor generator and anchor-sampling cross-attention. Uses 3D object queries projected into multi-view feature maps for efficient context sampling, plus dual-level regularization combining multi-view contrastive learning with dynamically scheduled spatial overlap penalty.
...
Conclusion: FAST3DIS provides an effective end-to-end alternative to clustering-based 3D instance segmentation, offering better scalability and efficiency while maintaining competitive accuracy through its query-based Transformer approach with specialized regularization.
...
TL;DR: GSR-guided Progressive Distillation (GPD) uses GSR deception knowledge to guide non-contact modality learning via cross-modal distillation, achieving SOTA on deception detection using the new MuDD dataset.


<details>
  <summary>Details</summary>
Motivation: Non-contact deception detection is challenging due to unstable visual/auditory cues across subjects, while GSR provides more reliable physiological signals but requires contact. The paper aims to leverage GSR's stable deception knowledge to improve non-contact detection through knowledge transfer.

Method: Proposes GSR-guided Progressive Distillation (GPD) framework with progressive feature-level and digit-level distillation with dynamic routing. Uses the newly collected MuDD dataset containing video, audio, GSR, PPG, heart rate, and personality traits from 130 participants over 690 minutes.

Result: GPD outperforms existing methods and achieves state-of-the-art performance on both deception detection and concealed-digit identification tasks. Extensive experiments and visualizations demonstrate effective cross-modal knowledge transfer.

Conclusion: The proposed GPD framework successfully mitigates negative transfer from modality mismatch between GSR and non-contact signals, enabling stable cross-modal knowledge distillation for improved non-contact deception detection.
...
### [44] [Neighbor-Aware Localized Concept Erasure in Text-to-Image Diffusion Models](https://arxiv.org/abs/2603.25994)
*Zhuan Shi,Alireza Dehghanpour Farashah,Rik de Vries,Golnoosh Farnadi*

Main category: cs.CV

TL;DR: NLCE is a training-free framework for localized concept erasure in text-to-image diffusion models that preserves semantically related neighbor concepts while removing target concepts, addressing the problem of unintentional weakening of related concepts during erasure.


<details>
  <summary>Details</summary>
Motivation: Current localized concept erasure methods in text-to-image diffusion models can unintentionally weaken semantically related neighbor concepts when suppressing a target concept, reducing fidelity in fine-grained domains where preserving related categories is important.

Method: NLCE operates in three stages: 1) spectrally-weighted embedding modulation that attenuates target concept directions while stabilizing neighbor concept representations, 2) attention-guided spatial gate that identifies regions with residual concept activation, and 3) spatially-gated hard erasure that eliminates remaining traces only where necessary.

Result: Experiments on fine-grained datasets (Oxford Flowers, Stanford Dogs) show NLCE effectively removes target concepts while better preserving closely related categories. Additional results on celebrity identity, explicit content, and artistic style demonstrate robustness and generalization to broader erasure scenarios.

Conclusion: NLCE provides a neighbor-aware pipeline for localized concept removal that maintains surrounding concept neighborhood structure, offering improved preservation of semantically related concepts while achieving effective target concept erasure across various domains.
...
TL;DR: R-PGA framework improves physical adversarial camouflage robustness using relightable 3DGS for realistic simulation and hard configuration mining for optimization stability.


<details>
  <summary>Details</summary>
Motivation: Current physical adversarial camouflage methods fail in complex dynamic scenarios due to simulation fidelity gaps (coarse CARLA simulations) and optimization fragility (rugged loss landscapes vulnerable to configuration shifts).

Method: Proposes R-PGA with two key components: 1) Relightable 3DGS for photo-realistic reconstruction with physically disentangled attributes, plus hybrid rendering pipeline; 2) Hard Physical Configuration Mining module to actively mine worst-case configurations and flatten loss landscape.
...
Conclusion: R-PGA bridges critical gaps in physical adversarial camouflage by combining realistic simulation via relightable 3DGS with robust optimization via hard configuration mining, significantly improving performance in complex dynamic scenarios.
...
TL;DR: Current text-to-image models fail to compose multiple interacting subjects despite appearing scalable, suffering catastrophic identity collapse in complex scenes.


<details>
  <summary>Details</summary>
Motivation: Existing subject-driven text-to-image models excel at single identities but struggle with multiple interacting subjects. Current evaluation methods using CLIP metrics are inadequate as they don't detect local identity collapse and multi-subject entanglement.

Method: 1) Constructed a stress-test benchmark with 75 prompts across varying subject counts (2-10) and interaction difficulties (Neutral, Occlusion, Interaction). 2) Introduced Subject Collapse Rate (SCR) metric using DINOv2's structural priors to penalize attention leakage and homogenization. 3) Evaluated state-of-the-art models (MOSAIC, XVerse, PSR) systematically.

Result: Models show "Illusion of Scalability" - they work well for 2-4 subjects in simple layouts but suffer catastrophic identity collapse at 6-10 subjects or complex interactions. SCR approaches 100% at 10 subjects, revealing severe identity fidelity drop as scene complexity grows. CLIP metrics are fundamentally flawed, often giving high scores to identity-collapsed images.

Conclusion: The failure stems from semantic shortcuts in global attention routing. There's an urgent need for explicit physical disentanglement in future generative architectures to handle multi-subject composition effectively.
...
TL;DR: Proposes Learnable Causal Trimming (LCT) framework to improve MedVQA generalization by suppressing dataset-specific spurious correlations and emphasizing causal diagnostic evidence through learnable feature pruning.


<details>
  <summary>Details</summary>
Motivation: MedVQA models suffer from limited generalization due to reliance on dataset-specific correlations (anatomical patterns, question-type regularities) rather than genuine diagnostic evidence. Existing causal approaches use static adjustments or post-hoc corrections that aren't integrated into end-to-end optimization.

Method: 1) Dynamic Anatomical Feature Bank (DAFB) updated via momentum mechanism to capture global prototypes of frequent anatomical/linguistic patterns; 2) Differentiable trimming module estimates dependency between instance-level representations and global feature bank; 3) Features highly correlated with global prototypes are softly suppressed while instance-specific evidence is emphasized; 4) Learnable mechanism adaptively prioritizes causal signals over spurious correlations.
...
Conclusion: LCT framework effectively addresses generalization issues in MedVQA by integrating causal pruning into end-to-end optimization, adaptively suppressing dataset-specific biases while emphasizing causal diagnostic evidence.
...
TL;DR: SkinGPT-X is a multimodal collaborative multi-agent system with self-evolving memory for dermatological diagnosis, outperforming state-of-the-art LLMs on standard datasets and rare skin disease benchmarks.


<details>
  <summary>Details</summary>
Motivation: Monolithic LLMs struggle with fine-grained multi-class dermatological diagnosis and rare diseases due to training data sparsity, lack interpretability, and can't adapt to complex clinical settings. Existing multi-agent systems focus on conversational tasks and rely on static knowledge bases.

Method: A multimodal collaborative multi-agent system with self-evolving dermatological memory mechanism that simulates dermatologists' diagnostic workflow and enables continuous memory evolution for transparent, trustworthy diagnostics.

Result: State-of-the-art performance with +9.6% accuracy on DDI31, +13% weighted F1 on Dermnet; strong performance on 498-category fine-grained classification; +9.8% accuracy, +7.1% weighted F1, +10% Cohen's Kappa improvement on rare skin disease dataset (564 samples, 8 rare diseases).

Conclusion: SkinGPT-X provides transparent, trustworthy diagnostics for complex and rare dermatological cases through its collaborative multi-agent architecture and self-evolving memory, addressing limitations of monolithic LLMs in clinical settings.
...
TL;DR: FSAR-LLaVA: First end-to-end method using MLLMs as multimodal knowledge base for few-shot action recognition, achieving superior performance with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM approaches for FSAR focus on caption generation (feature->caption->feature pipeline) and use metric learning only in visual space, which is suboptimal. Need to leverage MLLMs more effectively as multimodal knowledge bases.

Method: 1) Extract enriched multimodal representations via MLLM decoder, enhanced by Multimodal Feature-Enhanced Module; 2) Use MLLM versatility for adaptive prompting and Composite Task-Oriented Prototype Construction; 3) Introduce training-free Multimodal Prototype Matching Metric for joint multimodal guidance.

Result: Extensive experiments demonstrate superior performance across various few-shot action recognition tasks with minimal trainable parameters.

Conclusion: FSAR-LLaVA effectively leverages MLLMs as multimodal knowledge bases for direct FSAR enhancement, bridging distribution gaps and enabling joint multimodal metric learning.
...
TL;DR: Object-DINO: A training-free method that extracts distributed object-centric information from self-supervised ViTs by clustering attention heads across all layers, improving object discovery and reducing hallucination in multimodal models.


<details>
  <summary>Details</summary>
Motivation: Self-supervised ViTs like DINO show emergent object discovery abilities, but CLS token attention maps often contain spurious activations and poor localization because the CLS token summarizes the entire image rather than focusing on objects, diluting object-centric information in patch-level interactions.

Method: Analyze inter-patch similarity using patch-level attention components (query, key, value) across all layers. Object-DINO clusters attention heads across all layers based on patch similarities and automatically identifies the object-centric cluster corresponding to all objects, without requiring additional training.

Result: Object-DINO improves unsupervised object discovery (+3.6 to +12.4 CorLoc gains) and mitigates object hallucination in Multimodal Large Language Models by providing visual grounding, demonstrating that distributed object-centric information improves downstream tasks without training.

Conclusion: Object-centric information is distributed across all layers in self-supervised ViTs and encoded in all three attention components (q, k, v). Extracting this distributed information through clustering attention heads enables better object discovery and visual grounding without additional training.
...
### [51] [Face2Parts: Exploring Coarse-to-Fine Inter-Regional Facial Dependencies for Generalized Deepfake Detection](https://arxiv.org/abs/2603.26036)
*Kutub Uddin,Nusrat Tasnim,Byung Tae Oh*

Main category: cs.CV

TL;DR: A novel Face2Parts deepfake detection method using hierarchical feature representation that analyzes facial regions from coarse to fine (frame→face→key parts) with channel attention and triplet learning, achieving strong generalization across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Deepfake detection is challenging due to diverse manipulation techniques. Existing methods have unique strengths in detecting specific facial region anomalies, suggesting a hybrid approach could leverage complementary strengths for better detection.

Method: Face2Parts with hierarchical feature representation (HFR) extracts features from frame, face, and key facial regions (lips, eyes, nose) separately. Uses channel-attention mechanism to capture inter-dependencies and deep triplet learning for improved feature discrimination.

Result: Achieves high AUC scores across multiple datasets: 98.42% on FF++, 79.80% on CDF1, 85.34% on CDF2, 89.41% on DFD, 84.07% on DFDC, 95.62% on DTIM, 80.76% on PDD, and 100% on WLDR, outperforming existing methods.

Conclusion: The hierarchical coarse-to-fine approach effectively captures deepfake traces across facial regions, demonstrating strong generalization and promising performance for real-world deepfake detection applications.
...
### [52] [Rethinking Token Pruning for Historical Screenshots in GUI Visual Agents: Semantic, Spatial, and Temporal Perspectives](https://arxiv.org/abs/2603.26041)
*Daiqiang Li,Zihao Pan,Zeyu Zhang,Ronghao Chen,Huacan Wang,Honggang Chen,Haiyun Jiang*

Main category: cs.CV

TL;DR: This paper studies token pruning strategies for GUI visual agents, finding that background regions provide valuable interface-state information, random pruning preserves spatial structure better than designed strategies, and allocating more tokens to recent screenshots (recency effect) reduces computational costs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: GUI visual agents using MLLMs face computational challenges due to high-resolution screenshots generating many visual tokens. Preserving complete historical information is expensive, so efficient token pruning strategies are needed to reduce computational costs while maintaining performance.

Method: The authors conduct an empirical study on token pruning for historical GUI screenshots. They analyze GUI screenshot composition, apply edge-based separation to partition foreground/background regions, compare different pruning strategies (including random pruning), and investigate token allocation based on recency of screenshots.

Result: Three key findings: 1) Background regions capture interface-state transitions and provide auxiliary cues for GUI reasoning, contrary to common assumptions. 2) Random pruning outperforms carefully designed strategies in preserving spatial structure under the same computational budget. 3) GUI agents show a recency effect - allocating more tokens to recent screenshots while heavily compressing distant ones significantly reduces computational costs with minimal performance impact.

Conclusion: The study provides practical insights for designing efficient GUI visual agents: background regions have semantic value, random pruning is surprisingly effective for preserving spatial structure, and leveraging recency effects in token allocation can dramatically reduce computational costs while maintaining performance.
...
TL;DR: A progressive learning strategy for LA scar segmentation from LGE MRI using a 3-stage SwinUNETR framework with anatomy-aware loss, outperforming one-stage methods.


<details>
  <summary>Details</summary>
Motivation: Automatic LA scar segmentation from LGE MRI is challenging due to low contrast, annotation variability, and lack of anatomical constraints, leading to unreliable predictions. Clinical workflow-inspired approaches are needed.

Method: 3-stage progressive learning: 1) LA cavity pre-learning, 2) dual-task model learning spatial relationships between LA geometry and scar patterns, 3) fine-tuning for precise scar segmentation. Uses anatomy-aware spatially weighted loss incorporating clinical priors.

Result: On LASCARQS dataset with 5-fold CV: LA segmentation Dice=0.94, LA scar segmentation Dice=0.50, HD=11.84mm, ASD=1.80mm. Outperformed one-stage scar segmentation (Dice=0.49, HD=13.02mm, ASD=1.96mm).

Conclusion: Explicitly embedding clinical anatomical priors and diagnostic reasoning into deep learning improves accuracy and reliability of LA scar segmentation, highlighting the importance of clinically informed model design.
...
TL;DR: MemCam: Memory-augmented interactive video generation that uses previously generated frames as external memory to maintain scene consistency during long videos with dynamic camera control.


<details>
  <summary>Details</summary>
Motivation: Existing interactive video generation methods struggle with maintaining scene consistency during long video generation under dynamic camera control due to limited contextual information. There's a need for better approaches that can handle complex camera movements while preserving scene coherence over extended sequences.

Method: Proposes MemCam, a memory-augmented approach that treats previously generated frames as external memory and uses them as contextual conditioning. Includes a context compression module that encodes memory frames into compact representations and employs co-visibility-based selection to dynamically retrieve the most relevant historical frames, reducing computational overhead while enriching context.

Result: MemCam significantly outperforms existing baseline methods and open-source state-of-the-art approaches in scene consistency, particularly in long video scenarios with large camera rotations. The method demonstrates superior performance in maintaining scene coherence during interactive video generation.

Conclusion: Memory-augmented approaches like MemCam effectively address scene consistency challenges in interactive video generation, especially for long videos with dynamic camera control. The context compression and co-visibility-based selection mechanisms enable efficient use of historical information to improve generation quality.
...
TL;DR: Discrete diffusion vision-language models (DVLMs) with hybrid masking outperform linear-masked variants and compete with autoregressive models for GUI grounding tasks, showing promise for diffusion-based GUI agents.


<details>
  <summary>Details</summary>
Motivation: While autoregressive VLMs dominate multimodal tasks including GUI grounding, discrete DVLMs have shown strong performance in multimodal reasoning but their potential for GUI grounding remains unexplored. The paper investigates whether discrete DVLMs can serve as a viable alternative to AR models for GUI grounding.

Method: Adapted LLaDA-V for single-turn action and bounding-box prediction, framing GUI grounding as text generation from multimodal input. Proposed a hybrid masking schedule combining linear and deterministic masking to better capture hierarchical structure of bounding-box geometry.

Result: Hybrid masking improved grounding accuracy by up to 6.1 points in Step Success Rate over linear-masked variant. Evaluations on four datasets spanning web, desktop, and mobile interfaces show competitive performance with autoregressive models despite limited pretraining. Systematic ablations show accuracy improvements with increased diffusion steps, generation length, and block length, but with latency trade-offs. Expanding training data with diverse GUI domains reduced latency by ~1.3 seconds and improved accuracy by average 20 points across benchmarks.
...
### [56] [Pioneering Perceptual Video Fluency Assessment: A Novel Task with Benchmark Dataset and Baseline](https://arxiv.org/abs/2603.26055)
...
<details>
  <summary>Details</summary>
Motivation: Current video quality assessment methods underrepresent fluency (motion consistency and frame continuity), limiting their applicability for applications like streaming and gaming. Fluency has been treated only as a sub-dimension rather than a standalone perceptual task.

Method: 1) Construct FluVid dataset with 4,606 in-the-wild videos featuring balanced fluency distribution and first-ever VFA scoring criteria. 2) Benchmark 23 methods comprehensively. 3) Propose FluNet baseline model using temporal permuted self-attention (T-PSA) to enrich fluency information and enhance long-range inter-frame interactions.

Result: The work achieves state-of-the-art performance on VFA task and provides the most comprehensive benchmark to date with 23 methods evaluated on the FluVid dataset.

Conclusion: This paper pioneers Video Fluency Assessment as a standalone perceptual task, provides a comprehensive dataset and benchmark, and offers a roadmap for the community to explore VFA solutions with the proposed FluNet model achieving superior performance.
...
### [57] [ARTA: Adaptive Mixed-Resolution Token Allocation for Efficient Dense Feature Extraction](https://arxiv.org/abs/2603.26258)
*David Hagerman,Roman Naeem,Erik Brorsson,Fredrik Kahl,Lennart Svensson*

Main category: cs.CV

TL;DR: ARTA is a mixed-resolution coarse-to-fine vision transformer that efficiently extracts dense features by starting with low-resolution tokens and using a lightweight allocator to selectively add fine tokens only where needed near semantic boundaries.


<details>
  <summary>Details</summary>
Motivation: Traditional vision transformers use dense high-resolution tokens from the start, which is computationally expensive and inefficient since many homogeneous regions don't need fine-grained processing. There's a need for more efficient dense feature extraction that focuses computation only on semantically complex areas.

Method: ARTA starts with low-resolution (coarse) tokens and uses a lightweight allocator to predict semantic boundary scores. It iteratively allocates additional fine tokens to patches above a low threshold, concentrating token density near boundaries. Mixed-resolution attention enables interaction between coarse and fine tokens while avoiding redundant processing in homogeneous regions.
...
Conclusion: ARTA's coarse-to-fine approach with targeted token allocation enables efficient dense feature extraction by focusing computation on semantically complex boundary regions while avoiding redundant processing in homogeneous areas, achieving strong performance with significantly reduced computational cost.
...
TL;DR: GeoGuide is a novel framework for open-vocabulary 3D semantic segmentation that leverages pretrained 3D models with hierarchical geometry-semantic consistency, outperforming existing methods that rely on 2D distillation.
...
Method: GeoGuide introduces three key modules: 1) Uncertainty-based Superpoint Distillation to fuse geometric/semantic features and weight 2D features adaptively, 2) Instance-level Mask Reconstruction to enforce semantic consistency within instances, and 3) Inter-Instance Relation Consistency to align geometric and semantic similarity matrices.
...
TL;DR: PhysVid introduces a physics-aware local conditioning scheme for generative video models that uses temporally contiguous frame chunks annotated with physics-grounded descriptions, plus negative physics prompts at inference to steer away from implausible trajectories, improving physical plausibility by ~33% over baselines.


<details>
  <summary>Details</summary>
Motivation: Current generative video models achieve high visual fidelity but often violate basic physical principles, limiting their reliability in real-world settings. Existing physics injection methods have limitations: frame-level signals are domain-specific and short-horizon, while global text prompts are coarse and noisy, missing fine-grained dynamics.

Method: PhysVid uses a physics-aware local conditioning scheme operating over temporally contiguous chunks of frames. Each chunk is annotated with physics-grounded descriptions of states, interactions, and constraints, which are fused with the global prompt via chunk-aware cross-attention during training. At inference, negative physics prompts (descriptions of locally relevant law violations) are introduced to steer generation away from implausible trajectories.

Result: On VideoPhy benchmark, PhysVid improves physical commonsense scores by approximately 33% over baseline video generators, and by up to approximately 8% on VideoPhy2. The method shows substantial improvements in physical plausibility compared to existing approaches.

Conclusion: Local, physics-aware guidance substantially increases physical plausibility in generative video models and marks a step toward physics-grounded video models. The approach demonstrates that fine-grained physics conditioning at chunk-level, combined with negative prompting, effectively addresses physical commonsense violations in video generation.
...
### [60] [PAD-Hand: Physics-Aware Diffusion for Hand Motion Recovery](https://arxiv.org/abs/2603.26068)
...
Method: Proposes a physics-aware conditional diffusion framework with MeshCNN-Transformer backbone, formulates Euler-Lagrange dynamics for articulated hands, treats dynamic residuals as virtual observables (instead of enforcing zero residuals), and uses last-layer Laplace approximation for variance estimation.

Result: Experiments on two hand datasets show consistent improvements over strong image-based initializations and competitive performance with video-based methods. Produces interpretable variance maps that indicate where physical consistency weakens, aligned with physical plausibility of motion.

Conclusion: The framework successfully refines noisy hand pose sequences into physically plausible motion while providing meaningful physics variance estimates that offer interpretable confidence measures about motion quality.
...
TL;DR: TARA-Merging is a new method for merging multiple LoRA modules that addresses subspace coverage and anisotropy issues to better preserve task-specific representations.


<details>
  <summary>Details</summary>
Motivation: Current LoRA merging methods struggle because LoRA update directions span different subspaces and contribute unevenly to different tasks. Naive merging weakens critical task directions while overemphasizing less important ones, reducing the model's ability to faithfully represent all tasks.

Method: TARA-Merging (Task-Rank Anisotropy Alignment) aligns merging weights using a preference-weighted cross-entropy pseudo-loss while preserving task-relevant LoRA subspaces. It ensures broad subspace coverage and mitigates anisotropy through direction-wise reweighting.
...
Conclusion: The paper highlights the importance of addressing both subspace coverage and anisotropy in LoRA merging, showing that TARA-Merging effectively constructs general-purpose systems by better preserving task-specific representations.
...
TL;DR: MUST is a novel multimodal survival prediction framework that explicitly decomposes modality representations into modality-specific and cross-modal components, enabling robust predictions even with missing modalities through conditional latent diffusion.
...
Method: MUST uses algebraic constraints in a learned low-rank shared subspace to decompose each modality's representation into modality-specific and cross-modal contextualized components. For truly modality-specific information that cannot be inferred, it employs conditional latent diffusion models to generate high-quality representations conditioned on recovered shared information and learned structural priors.
...
Conclusion: MUST provides an effective solution for multimodal survival prediction that explicitly models modality-specific contributions and handles missing modalities through representation decomposition and conditional generation, enabling robust clinical deployment.
...
### [63] [Label-Free Cross-Task LoRA Merging with Null-Space Compression](https://arxiv.org/abs/2603.26317)
*Wonyoung Lee,Wooseong Jeong,Kuk-Jin Yoon*

Main category: cs.CV

TL;DR: NSC Merging is a label-free, output-agnostic method for merging LoRA adapters that works across classification, regression, and sequence generation tasks by using adapter geometry and null-space compression as optimization signals.


<details>
  <summary>Details</summary>
Motivation: Existing model merging approaches fail when tasks span both classification and regression, and entropy-based methods don't apply to regression and are costly for LLMs due to long token sequences. There's a need for a method that can handle heterogeneous tasks efficiently.

Method: Null-Space Compression (NSC) Merging uses the observation that during LoRA fine-tuning, the down-projection factor A compresses its null space, and this compression correlates with performance. NSC uses this geometry-based signal to set merge weights, making it label-free and output-agnostic.

Result: NSC achieves SOTA performance across 20 heterogeneous vision tasks with balanced gains, outperforms baselines on 6 NLI benchmarks, and shows effectiveness on vision-language tasks (VQA and image captioning), demonstrating scalability and cross-task generalization.

Conclusion: NSC Merging provides an effective, scalable solution for merging LoRA adapters across diverse task types (classification, regression, sequence generation) by leveraging adapter geometry rather than task-specific outputs, overcoming limitations of previous methods.
...
TL;DR: IADA (Input-Adaptive Depth Aggregation) is a lightweight mechanism that restores reasoning performance in vision-language models during fine-tuning by enabling adaptive cross-depth representation access.


<details>
  <summary>Details</summary>
Motivation: Supervised fine-tuning on visual instruction data often improves perceptual capabilities in VLMs but degrades reasoning performance, creating a "reasoning tax" - the paper investigates whether this degradation is related to disrupted access to depth-wise representations.
...
Conclusion: Preserved cross-depth access is an important missing factor in VLM fine-tuning, and IADA's input-adaptive, modality-aware cross-depth aggregation effectively addresses the reasoning degradation problem during visual instruction fine-tuning.
...
TL;DR: LIAF-KD introduces learnable instance attention filtering for adaptive detector distillation, allowing dynamic instance importance evaluation with student involvement, achieving 2% performance gain on GFL ResNet-50.


<details>
  <summary>Details</summary>
Motivation: Current feature-based KD methods for object detection treat all object instances uniformly, ignoring instance-level variability. Existing attention filtering mechanisms are heuristic or teacher-driven rather than learned with the student, limiting adaptive knowledge transfer.

Method: Proposes LIAF-KD framework with learnable instance selectors that dynamically evaluate and reweight instance importance during distillation. The student contributes to this process based on its evolving learning state, enabling adaptive knowledge transfer.

Result: Experiments on KITTI and COCO datasets show consistent improvements, with 2% gain on GFL ResNet-50 student without added complexity, outperforming state-of-the-art methods.

Conclusion: LIAF-KD effectively addresses limitations of existing KD methods by introducing learnable instance attention filtering that adapts to student learning state, improving deployment efficiency of deep vision models through better knowledge transfer.
...
### [66] [Reflect to Inform: Boosting Multimodal Reasoning via Information-Gain-Driven Verification](https://arxiv.org/abs/2603.26348)
*Shuai Lv,Chang Liu,Feng Tang,Yujie Yuan,Aojun Zhou,Kui Zhang,Xi Yang,Yangqiu Song*

Main category: cs.CV

TL;DR: MLLMs suffer from visual grounding degradation in long-form generation, progressively drifting from image evidence. The paper proposes Visual Re-Examination (VRE), a self-evolving training framework that enables MLLMs to perform visual introspection during reasoning without extra visual inputs.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs show strong reasoning but have a recurring failure mode: as outputs grow longer, they progressively drift away from image evidence and rely on textual priors, causing ungrounded reasoning and hallucinations. Attention analysis reveals MLLMs have latent late-stage visual verification capability that isn't consistently activated.

Method: Proposes Visual Re-Examination (VRE), a self-evolving training framework enabling MLLMs to autonomously perform visual introspection during reasoning without additional visual inputs. Instead of distilling from a stronger teacher, VRE promotes iterative self-improvement by leveraging the model itself to generate reflection traces, making visual information actionable through information gain.
...
Conclusion: VRE effectively addresses visual grounding degradation in long-form MLLM generation by activating latent visual verification capabilities through self-evolving training, leading to more reliable multimodal reasoning with reduced hallucinations.
...
### [67] [CD-Buffer: Complementary Dual-Buffer Framework for Test-Time Adaptation in Adverse Weather Object Detection](https://arxiv.org/abs/2603.26092)
*Youngjun Song,Hyeongyu Kim,Dosik Hwang*

Main category: cs.CV

TL;DR: CD-Buffer is a novel TTA framework that adaptively combines subtractive (removing corrupted features) and additive (refining features) strategies using a unified discrepancy metric to handle varying domain shift severities.


<details>
  <summary>Details</summary>
Motivation: Existing TTA methods have limitations: subtractive approaches work well for severe domain shifts but not moderate ones, while additive approaches are effective for moderate shifts but fail under severe corruption. Neither paradigm generalizes well across diverse corruption levels, creating a need for an adaptive framework that can balance both strategies based on measured feature-level domain shift severity.

Method: CD-Buffer introduces a complementary dual-buffer framework where subtractive and additive mechanisms operate in opposite yet coordinated directions. The key innovation is discrepancy-driven coupling - the framework couples removal and refinement through a unified discrepancy metric that automatically balances both strategies based on feature-level shift severity. This enables automatic channel-wise balancing that adapts differentiated treatment to heterogeneous shift magnitudes without manual tuning.

Result: Extensive experiments on KITTI, Cityscapes, and ACDC datasets demonstrate state-of-the-art performance. The framework consistently achieves superior results across diverse weather conditions and severity levels, showing effective adaptation to varying domain shift severities.

Conclusion: CD-Buffer successfully addresses the limitations of existing TTA paradigms by adaptively balancing subtractive and additive strategies through a unified discrepancy metric. This enables robust performance across diverse corruption levels and establishes a new direction for adaptive test-time domain adaptation.
...
TL;DR: The paper introduces OVCOD-D benchmark for camouflaged object detection in open-vocabulary settings and proposes a method with sub-description principal component contrastive fusion and specificity-guided regional weak alignment to improve detection of camouflaged objects.


<details>
  <summary>Details</summary>
Motivation: Current open-vocabulary object detectors fail on camouflaged objects due to high visual similarity between objects and background, creating a gap in detecting such challenging cases.

Method: 1) Construct OVCOD-D benchmark with camouflaged object images and fine-grained textual descriptions. 2) Use pre-trained detectors as baseline. 3) Sub-description principal component contrastive fusion to reduce noisy textual components. 4) Specificity-guided regional weak alignment and dynamic focusing to discriminate camouflaged objects from background.

Result: The proposed method achieves AP of 56.4 on the OVCOD-D benchmark under open-set evaluation setting.

Conclusion: The work addresses the challenge of camouflaged object detection in open-vocabulary settings through a new benchmark and specialized methods that handle textual noise and visual similarity issues.
...
TL;DR: CPUBone is a new family of vision backbone models optimized for CPU inference by balancing operations count with hardware-efficient execution through grouped convolutions and reduced kernel sizes.


<details>
  <summary>Details</summary>
Motivation: Most vision backbone research focuses on hardware with high parallel processing capabilities (GPUs, mobile phones, AI accelerators), but CPUs have different constraints requiring a specific design philosophy that balances MACs with hardware-efficient execution (high MACs per second).

Method: The paper investigates two modifications to standard convolutions: 1) grouping convolutions and 2) reducing kernel sizes. These adaptations reduce computational cost (MACs) while maintaining hardware-efficiency on CPUs. Based on these insights, the authors introduce CPUBone, a new family of vision backbone models optimized for CPU-based inference.

Result: Experiments across diverse CPU devices confirm that the adaptations successfully retain high hardware-efficiency on CPUs. CPUBone achieves state-of-the-art Speed-Accuracy Trade-offs (SATs) across a wide range of CPU devices and effectively transfers its efficiency to downstream tasks like object detection and semantic segmentation.

Conclusion: CPUBone demonstrates that specific architectural modifications (grouped convolutions and reduced kernel sizes) can create vision backbones optimized for CPU inference, achieving superior speed-accuracy trade-offs compared to existing models while maintaining efficiency across various downstream tasks.
...
TL;DR: TGRL uses expert reasoning trajectories from stronger models to guide MLLMs in integrating visual evidence into fine-grained reasoning, addressing the gap between visual perception and logical reasoning.


<details>
  <summary>Details</summary>
Motivation: Current RLVR methods for MLLMs focus on answer correctness and visual grounding, but models fail to effectively incorporate visual evidence into reasoning chains, leading to weakly grounded reasoning.

Method: Trajectory-Guided Reinforcement Learning (TGRL) uses expert reasoning trajectories from stronger models to guide policy models. Includes token-level reweighting and trajectory filtering for stable optimization.
...
Conclusion: TGRL effectively addresses the bottleneck of visual evidence integration in reasoning processes, enhancing multimodal reasoning capabilities through guided trajectory learning.
...
TL;DR: TaxaAdapter uses Vision Taxonomy Models (VTMs) like BioCLIP to guide text-to-image diffusion models for fine-grained species generation, improving species-level fidelity while maintaining control over attributes like pose and background.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image models struggle to capture fine-grained visual traits that distinguish species (over 10M species exist), often failing to preserve species identity even when outputs appear photo-realistic.

Method: TaxaAdapter injects VTM embeddings (from models like BioCLIP) into frozen text-to-image diffusion models, providing species guidance while preserving flexible text control over attributes.

Result: TaxaAdapter consistently improves morphology fidelity and species-identity accuracy over baselines, with strong generalization to few-shot species and even species unseen during training.

Conclusion: VTMs are a key ingredient for scalable, fine-grained species generation, and TaxaAdapter demonstrates their effectiveness through improved species fidelity and interpretable morphological consistency.
...
TL;DR: InstaVSR is a lightweight diffusion framework for efficient video super-resolution that combines pruned one-step diffusion, recurrent training with flow-guided regularization, and dual-space adversarial learning to achieve fast processing with good temporal stability.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based video super-resolution methods face two main challenges: strong generative priors can cause temporal instability (flickering/jittering), and multi-frame diffusion pipelines are computationally expensive for practical deployment.

Method: Three key components: (1) pruned one-step diffusion backbone that removes costly components from conventional diffusion pipelines, (2) recurrent training with flow-guided temporal regularization to improve frame-to-frame stability, (3) dual-space adversarial learning in both latent and pixel spaces to preserve perceptual quality after simplification.

Result: On NVIDIA RTX 4090, InstaVSR processes 30-frame 2K×2K video in under one minute with only 7GB memory usage, substantially reducing computational cost compared to existing diffusion methods while maintaining good perceptual quality with significantly smoother temporal transitions.

Conclusion: InstaVSR successfully addresses both computational efficiency and temporal stability challenges in diffusion-based video super-resolution, making diffusion methods more practical for real-world video applications.
...
TL;DR: Proposed knowledge distillation method for MobileViT backbone improves few-shot learning accuracy on edge devices while significantly reducing parameters, FLOPs, and energy consumption.


<details>
  <summary>Details</summary>
Motivation: Need for efficient deep learning models on edge devices with limited connectivity, low-latency requirements, and energy constraints. Few-shot learning is crucial for low-data regimes where collecting large annotated datasets is costly or impractical.

Method: Pre-training method for MobileViT backbone using knowledge distillation to transfer generalization ability from large-scale teacher model to lightweight student model for edge computing.

Result: Achieved 14% and 6.7% accuracy improvements for one-shot and five-shot classification on MiniImageNet vs ResNet12 baseline, with 69% parameter reduction, 88% FLOPs reduction, 37% energy reduction, and 2.6ms latency on Jetson Orin Nano.

Conclusion: The proposed knowledge distillation method is a promising and practical solution for deploying efficient few-shot learning models on edge AI hardware with significant performance and efficiency gains.
...
### [74] [Beyond MACs: Hardware Efficient Architecture Design for Vision Backbones](https://arxiv.org/abs/2603.26551)
*Moritz Nottebaum,Matteo Dunnhofer,Christian Micheloni*

Main category: cs.CV

TL;DR: LowFormer is a new vision backbone family that addresses the limitations of MACs as an efficiency metric, especially for edge devices, by introducing streamlined architecture design and Lowtention - a lightweight alternative to Multi-Head Self-Attention.


<details>
  <summary>Details</summary>
Motivation: MACs (Multiply Accumulate operations) are commonly used to measure neural network efficiency but have shortcomings, especially for edge devices. The paper aims to identify key factors for efficient execution and optimize backbone design beyond just MAC counts.

Method: The authors experimentally analyze MAC count vs. execution time of architectural elements, then design LowFormer with streamlined macro/micro architecture including Lowtention (lightweight alternative to Multi-Head Self-Attention). They also create an edge GPU version optimized for specific hardware.

Result: LowFormer achieves superior efficiency and performance: Lowtention is more efficient than standard attention while enabling better ImageNet results. LowFormer models achieve remarkable speed-ups across various hardware platforms compared to state-of-the-art backbones, with strong performance on classification, detection, segmentation, retrieval, and tracking tasks.

Conclusion: MACs alone are insufficient for measuring efficiency, especially on edge devices. LowFormer demonstrates that optimized architectural design with lightweight attention mechanisms can achieve superior speed and performance across diverse hardware platforms and vision tasks.
...
TL;DR: IP-Bench is the first systematic benchmark for evaluating image protection methods against image-to-video generation misuse, addressing the lack of unified evaluation frameworks in this emerging threat domain.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of I2V generation models creates significant misuse potential for creating fake videos from single images, while existing image protection methods lack systematic evaluation frameworks, particularly against I2V scenarios and preprocessing attacks.

Method: IP-Bench systematically evaluates 6 representative protection methods against 5 state-of-the-art I2V models, assessing robustness with two attack strategies under practical scenarios, and analyzing cross-model & cross-modality transferability.

Result: The benchmark establishes a systematic, reproducible, and extensible evaluation framework for image protection methods in I2V generation scenarios, providing comprehensive assessment capabilities.

Conclusion: IP-Bench addresses the critical need for standardized evaluation of image protection methods against I2V generation misuse, enabling better assessment of real-world effectiveness and robustness.
...
### [76] [Generation Is Compression: Zero-Shot Video Coding via Stochastic Rectified Flow](https://arxiv.org/abs/2603.26571)
*Ziyue Zeng,Xun Su,Haoyuan Liu,Bingyu Lu,Yui Tatsumi,Hiroshi Watanabe*

Main category: cs.CV

TL;DR: GVC turns pretrained video generative models into video codecs without retraining by converting rectified-flow ODEs to SDEs for stochastic compression, offering three conditioning strategies for different fidelity/efficiency trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing generative video compression methods only use generative models as post-hoc reconstruction modules on top of conventional codecs, limiting their potential. The authors want to directly use pretrained video generative models as the codec itself for more efficient compression.

Method: Convert deterministic rectified-flow ODEs of video foundation models into equivalent SDEs at inference time, creating stochastic injection points for codebook-driven compression. Three conditioning strategies: Image-to-Video (I2V) with adaptive tail-frame allocation, Text-to-Video (T2V) with near-zero side information, and First-Last-Frame-to-Video (FLF2V) with boundary-sharing GOP chaining.

Result: GVC achieves high-quality reconstruction below 0.002 bpp while supporting flexible bitrate control through a single hyperparameter. It spans a principled trade-off space between spatial fidelity, temporal coherence, and compression efficiency.

Conclusion: GVC demonstrates that pretrained video generative models can be directly used as video codecs without retraining, offering a zero-shot framework that outperforms conventional approaches and provides flexible compression strategies.
...
<details>
  <summary>Details</summary>
Motivation: Existing watermarking methods for diffusion models only support fuzzy matching and cannot recover structured watermark data bit-exactly, making them unsuitable for applications requiring lossless metadata like licensing instructions or offline verification.

Method: Embeds watermarks in the initial Gaussian noise without fine-tuning or quality loss, treats diffusion as noisy channel, identifies two interference types (local bit flips and global stochastic distortions), and uses cascaded defense with error-correcting codes and majority voting.

Result: Achieves state-of-the-art bit-level accuracy across three Stable Diffusion variants and seven perturbation types while maintaining high true positive rate, enabling trustworthy rights attribution in real-world deployment.

Conclusion: Gaussian Shannon provides a practical solution for exact watermark recovery in diffusion models, addressing limitations of threshold-based detection methods and supporting applications requiring lossless metadata transmission.
...
TL;DR: Provably contractive denoiser networks with global Lipschitz < 1 that maintain competitive restoration performance while guaranteeing stability against input perturbations.


<details>
  <summary>Details</summary>
Motivation: Existing SOTA image restoration networks lack stability guarantees under minor input shifts, exposing a robustness-accuracy trade-off. The paper aims to develop provably contractive denoisers that reduce this gap.

Method: Composes proximal layers from unfolding techniques with Lipschitz-controlled convolutional refinements to create contractive (global Lipschitz < 1) denoiser networks.

Result: Achieves competitive performance with unconstrained SOTA denoisers while guaranteeing input perturbations induce at most equal output changes. Reports tightest gap for provably 1-Lipschitz model and enables provable convergence in Plug-and-Play algorithms.

Conclusion: Enforcing strict Lipschitz control does not inherently degrade output quality, challenging common assumptions and moving toward verifiable, stable vision models.
...
### [79] [Think over Trajectories: Leveraging Video Generation to Reconstruct GPS Trajectories from Cellular Signaling](https://arxiv.org/abs/2603.26610)
*Ruixing Zhang,Hanzhang Jiang,Leilei Sun,Liangzhe Han,Jibin Wang,Weifeng Lv*

Main category: cs.CV

TL;DR: Sig2GPS converts cellular signaling data to GPS trajectories using image-to-video generation, treating signaling traces as map images and generating continuous GPS paths as videos.


<details>
  <summary>Details</summary>
Motivation: Cellular signaling records provide broad human mobility coverage but only offer coarse location cues (cell identifiers), limiting their use for applications requiring high-precision GPS trajectories. There's a need to bridge this gap between coarse signaling data and precise GPS trajectories.

Method: Reframes the problem as image-to-video generation: signaling traces are rendered on maps, and a video generation model is trained to draw continuous GPS paths. A paired signaling-to-trajectory video dataset is constructed to fine-tune an open-source video model, with trajectory-aware reinforcement learning optimization to improve generation fidelity via rewards.

Result: Experiments on large-scale real-world datasets show substantial improvements over strong engineered and learning-based baselines. Additional results on next GPS prediction indicate scalability and cross-city transferability.

Conclusion: Map-visual video generation provides a practical interface for trajectory data mining by enabling direct generation and refinement of continuous paths under map constraints, offering a novel approach to convert coarse cellular signaling into precise GPS trajectories.
...
### [80] [CREval: An Automated Interpretable Evaluation for Creative Image Manipulation under Complex Instructions](https://arxiv.org/abs/2603.26174)
*Chonghuinan Wang,Zihan Chen,Yuxiang Wei,Tianyi Jiang,Xiaohe Wu,Fan Li,Wangmeng Zuo,Hongxun Yao*

Main category: cs.CV

TL;DR: CREval is an automated QA-based evaluation pipeline for creative image manipulation models, addressing limitations in current evaluation methods. It includes CREval-Bench benchmark covering 3 categories and 9 creative dimensions with 800+ samples and 13K queries.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods lack systematic and human-aligned frameworks for assessing model performance on complex and creative image editing tasks. Current approaches suffer from incompleteness and poor interpretability of opaque MLLM scoring.

Method: Proposes CREval, a fully automated question-answer (QA)-based evaluation pipeline that overcomes limitations of MLLM scoring. Also introduces CREval-Bench benchmark covering three categories and nine creative dimensions with over 800 editing samples and 13K evaluation queries.

Result: Systematic evaluation shows closed-source models generally outperform open-source ones on complex creative tasks, but all models struggle with effective completion of such edits. User studies demonstrate strong consistency between CREval's automated metrics and human judgments.
...
### [81] [Make Geometry Matter for Spatial Reasoning](https://arxiv.org/abs/2603.26639)
*Shihua Zhang,Qiuhong Shen,Shizun Wang,Tianbo Pan,Xinchao Wang*

Main category: cs.CV

TL;DR: GeoSR is a framework that enhances vision-language models' spatial reasoning by forcing them to actively use geometry tokens through masking 2D visual cues and adaptive geometry fusion.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models (VLMs) have strong image/video understanding but limited spatial reasoning capabilities. Existing approaches that inject geometry tokens from 3D foundation models often underutilize these geometric cues because VLMs tend to rely too heavily on 2D visual shortcuts.

Method: GeoSR introduces two key components: 1) Geometry-Unleashing Masking - strategically masks portions of 2D vision tokens during training to weaken non-geometric shortcuts and force the model to consult geometry tokens; 2) Geometry-Guided Fusion - a gated routing mechanism that adaptively amplifies geometry token contributions in regions where geometric evidence is critical.

Result: Extensive experiments on both static and dynamic spatial reasoning benchmarks show that GeoSR consistently outperforms prior methods and establishes new state-of-the-art performance by effectively leveraging geometric information.

Conclusion: GeoSR successfully addresses the underutilization of geometry tokens in VLMs for spatial reasoning by forcing models to actively reason with geometric information through strategic masking and adaptive fusion mechanisms.
...
TL;DR: CCL framework improves open-vocabulary object detection by enforcing intra-modal consistency through contextual bootstrapped data generation and consistency loss, achieving +16.3 AP on OmniLabel and +14.9 AP on D3.


<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary detection methods focus on scaling datasets and contrastive learning but neglect internal consistency within single modalities, causing performance drops when objects appear in different backgrounds/environments.

Method: Proposes Contextual Consistency Learning (CCL) with two key components: Contextual Bootstrapped Data Generation (CBDG) creates images with same objects across diverse backgrounds, and Contextual Consistency Loss (CCLoss) enforces object feature invariance despite environmental changes.

Result: Achieves state-of-the-art performance with +16.3 AP improvement on OmniLabel and +14.9 AP on D3, demonstrating significant enhancement in model generalization across diverse environments.

Conclusion: Enforcing intra-modal consistency is crucial for robust open-vocabulary object detection, and the CCL framework effectively addresses the robustness gap by maintaining object feature consistency across different contexts.
...
TL;DR: PerceptionComp is a new benchmark for complex, long-horizon, perception-centric video reasoning requiring multiple temporal visual evidence and compositional constraints across diverse domains.


<details>
  <summary>Details</summary>
Motivation: Current video reasoning benchmarks don't adequately test complex, long-horizon perception tasks that require multiple temporally separated visual evidence and compositional reasoning. There's a need for benchmarks that challenge both human and AI systems on perception-centric reasoning that spans multiple perceptual subtasks and reasoning skills.

Method: Created a manually annotated benchmark with 1,114 complex questions on 279 videos from diverse domains (city walk tours, indoor villa tours, video games, extreme outdoor sports). Questions require multiple temporally separated visual evidence and compositional constraints under conjunctive and sequential logic, spanning objects, attributes, relations, locations, actions, and events.

Result: Human performance drops to near chance (18.97%) when rewatching is disallowed, showing the benchmark requires substantial test-time thinking. State-of-the-art MLLMs perform substantially worse than on existing benchmarks: best model (Gemini-3-Flash) reaches only 45.96% accuracy, while open-source models remain below 40%.

Conclusion: Perception-centric long-horizon video reasoning remains a major bottleneck for both humans and AI systems. The PerceptionComp benchmark will help drive progress in perceptual reasoning by providing a challenging testbed that requires complex, multi-step perception and reasoning.
...
TL;DR: GLINT is a framework that enables 3D Gaussian splatting to model transparent surfaces like glass by decomposing radiance contributions and using priors from video relighting models.


<details>
  <summary>Details</summary>
Motivation: Standard 3D Gaussian splatting fundamentally fails to model transparency (e.g., glass panels) because it cannot decouple radiance contributions from transparent interfaces and transmitted geometry.

Method: GLINT uses explicit decomposed Gaussian representation to reconstruct primary interfaces and model reflected/transmitted radiance separately. It bootstraps transparency localization from geometry-separation cues and uses geometry/material priors from pre-trained video relighting models during optimization.
...
Conclusion: GLINT successfully addresses the transparency modeling limitation in 3D Gaussian splatting through decomposition and priors, enabling better reconstruction of scenes with transparent surfaces.
...
TL;DR: DUGAE is a unified geometry and attribute enhancement framework for G-PCC compressed dynamic point clouds that exploits inter-frame spatiotemporal correlations, achieving significant quality improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing post-decoding quality enhancement methods for point clouds are designed for static data and process each frame independently, failing to exploit spatiotemporal correlations in dynamic point cloud sequences.

Method: Three main components: 1) DGE-Net with sparse convolution and feature-domain geometry motion compensation for geometry enhancement, 2) DA-KNN recoloring module for attribute mapping, and 3) DAE-Net with temporal feature extraction and feature-domain attribute motion compensation for attribute refinement.

Result: Significant improvements over G-PCC GeS-TM v10: 11.03 dB BD-PSNR gain and 93.95% BD-bitrate reduction for geometry; 4.23 dB BD-PSNR gain with 66.61% BD-bitrate reduction for luma. Also improved perceptual quality (PCQM) and outperformed V-PCC on seven dynamic point cloud datasets.

Conclusion: DUGAE effectively exploits spatiotemporal correlations in dynamic point clouds through a unified geometry and attribute enhancement framework, achieving substantial quality improvements and bitrate reductions for G-PCC compressed content.
...
TL;DR: OSA is a framework for echocardiography video segmentation that prevents rank collapse in recurrent models by constraining state evolution on the Stiefel manifold and enhances features using anatomical priors to separate structures from speckle noise.


<details>
  <summary>Details</summary>
Motivation: Accurate and temporally consistent left ventricle segmentation from echocardiography videos is crucial for cardiac function assessment, but existing methods suffer from rank collapse in recurrent models and noise interference from speckle artifacts.
...
Conclusion: OSA effectively addresses the challenges of echocardiography video segmentation by preventing rank collapse in recurrent models and enhancing noise-resilient features, making it suitable for clinical applications requiring accurate and temporally consistent cardiac assessment.
...
TL;DR: Continual learning for object detection fails in extreme-sparsity regimes like space-based RSO detection where background dominates. The paper proposes a dual-stage invariant framework with joint distillation to stabilize backbone representations and detection predictions, plus sparsity-aware data conditioning.


<details>
  <summary>Details</summary>
Motivation: Existing continual learning methods for object detection assume relatively balanced visual conditions, but fail in extreme-sparsity regimes where foreground signals are overwhelmingly dominated by background observations (like in space-based RSO detection). Background-driven gradients destabilize the feature backbone during sequential domain shifts, causing progressive representation drift.
...
Result: Experiments on a high-resolution space-based RSO detection dataset show consistent improvement over established continual object detection methods, achieving an absolute gain of +4.0 mAP under sequential domain shifts.
...
TL;DR: Lifelong Heterogeneous Learning (LHL) addresses learning across tasks with different output structures, focusing on dense prediction scenarios (LHL4DP). Proposed Heterogeneity-Aware Distillation (HAD) method uses self-distillation with distribution-balanced and salience-guided components to preserve heterogeneous knowledge without exemplars.


<details>
  <summary>Details</summary>
Motivation: Most lifelong learning research focuses on homogeneous tasks (e.g., only classification), ignoring the realistic scenario where tasks have heterogeneous output structures. This gap limits practical applications where models need to handle diverse task types over time.

Method: Proposes Heterogeneity-Aware Distillation (HAD), an exemplar-free approach using self-distillation. Has two components: 1) Distribution-balanced heterogeneity-aware distillation loss to address global prediction distribution imbalance, and 2) Salience-guided heterogeneity-aware distillation loss that focuses on informative edge pixels extracted with Sobel operator.

Result: Extensive experiments show HAD significantly outperforms existing methods in the new lifelong heterogeneous learning for dense prediction (LHL4DP) scenario.

Conclusion: Lifelong Heterogeneous Learning is a broader and more realistic setting than conventional lifelong learning. The proposed HAD method effectively addresses the challenges of preserving heterogeneous knowledge across tasks with different output structures, particularly in dense prediction scenarios.
...
TL;DR: Training stereo matching foundation models for UAV tree pruning, achieving best deployable accuracy-speed trade-off with DEFOM-PrunePlus variant.


<details>
  <summary>Details</summary>
Motivation: Autonomous tree pruning with UAVs requires real-time metric distance estimation to thin branches for safe approach and cutting tool actuation.

Method: Trained five variants of DEFOM-Stereo foundation model on synthetic dataset (5,520 stereo pairs from Unreal Engine 5), deployed on NVIDIA Jetson Orin Super 16 GB for real-time inference.

Result: DEFOM-Stereo ViT-S achieved best accuracy (depth MAE 23.40 cm) but too slow (~2.2 FPS). DEFOM-PrunePlus offered best deployable trade-off (~3.3 FPS, depth MAE 64.26 cm). Lightweight variants ran faster but sacrificed too much accuracy.

Conclusion: DEFOM-PrunePlus provides most practical accuracy-latency balance for onboard distance estimation in UAV tree pruning, while ViT-S serves as reference for future hardware improvements.
...
TL;DR: The paper proposes LGE and GDC modules to improve image-to-point cloud registration by enhancing geometric features and enforcing structural consistency, achieving SOTA results on RGB-D Scenes v2 and 7-Scenes benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing image-to-point cloud registration methods struggle with repetitive patterns where images lack sufficient 3D structural cues and alignment with point clouds, leading to incorrect matches. They also overlook structural consistency, limiting full exploitation of correspondences.
...
Result: Extensive experiments and ablations on RGB-D Scenes v2 and 7-Scenes benchmarks demonstrate state-of-the-art performance in image-to-point cloud registration.

Conclusion: The proposed LGE and GDC modules effectively address challenges in image-to-point cloud registration by enhancing geometric features and enforcing structural consistency, leading to improved performance on standard benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal models like CLIP are widely used in downstream systems, creating a critical need to remove sensitive information. However, machine unlearning for contrastively-trained encoders is underexplored, and existing evaluations fail to diagnose fine-grained, association-level forgetting.

Method: Built a synthetic dataset of 60K persona-attribute associations and two foundational models: a Compromised model polluted with sensitive data and a Clean model without it. Both trained from scratch on the same 400M-pair retain base, with the Compromised model additionally trained on sensitive set. Introduced structured holdout sets (holdout identity, holdout association) to precisely measure unlearning efficacy and collateral damage.

Result: Reveals that while utility-efficient deletion is feasible, current methods exhibit distinct failure modes: they either fail to forget effectively or over-generalize by erasing more than intended. SALMUBench sets a new standard for comprehensive unlearning evaluation.

Conclusion: SALMUBench provides a comprehensive benchmark for multimodal unlearning evaluation with publicly released dataset, models, evaluation scripts, and leaderboards to foster future research in sensitive information removal from contrastive encoders.
...
TL;DR: BPO is a reference-free method for verifying Text-to-Image models by identifying boundary-adjacent prompts that cause unstable outputs only on the target model, achieving high verification accuracy without needing multiple reference models.


<details>
  <summary>Details</summary>
Motivation: Third-party platforms often falsely claim to use official T2I models, misleading users and harming model owners' reputations. Existing verification methods require multiple reference models for prompt optimization, which is computationally expensive and sensitive to model selection.

Method: Boundary-aware Prompt Optimization (BPO) explores intrinsic characteristics of target models by identifying semantic boundaries in embedding space where prompts generate unstable outputs (e.g., sometimes "corgi", sometimes "bagel") on the target model but remain stable on others. These boundary-adjacent prompts serve as verification cues.
...
Conclusion: BPO provides an effective, reference-free approach for T2I model verification by leveraging model-specific semantic boundaries, reducing computational costs and sensitivity to reference model selection while maintaining high accuracy.
...
TL;DR: A novel attention-driven spatiotemporal video anonymization framework that uses Vision Transformers to separate action-relevant information from privacy-sensitive content, selectively pruning privacy-dominant tubelets while maintaining action recognition performance.


<details>
  <summary>Details</summary>
Motivation: Large-scale video models improve video understanding but amplify privacy risks by encoding sensitive attributes (facial identity, race, gender). Video anonymization is underexplored compared to image anonymization, despite video models being able to use spatiotemporal motion patterns as biometric identifiers.

Method: Proposes an attention-driven spatiotemporal video anonymization framework based on systematic disentanglement of utility and privacy features. Uses Vision Transformers with two task-specific classification tokens (action CLS token and privacy CLS token) that learn complementary representations. Contrasts their attention distributions to compute utility-privacy scores for each spatiotemporal tubelet, then keeps only the top-k tubelets with highest scores (pruning privacy-dominant tubelets while preserving action-critical ones).

Result: Extensive experiments show the approach maintains action recognition performance comparable to models trained on raw videos while substantially reducing privacy leakage. Attention-driven spatiotemporal pruning offers an effective and principled solution for privacy-preserving video analytics.

Conclusion: The proposed framework successfully addresses video privacy risks by leveraging attention mechanisms in Vision Transformers to separate utility and privacy features, demonstrating that selective spatiotemporal pruning can preserve action recognition while protecting sensitive attributes.
...
TL;DR: DuSCN-FusionNet is an interpretable deep learning framework using dual-channel Structural Covariance Networks from sMRI for ADHD classification, achieving 80.59% balanced accuracy with biomarker identification via Grad-CAM.


<details>
  <summary>Details</summary>
Motivation: ADHD lacks reliable imaging-based biomarkers for neurobiological diagnosis. Current deep learning approaches are black-box systems that limit clinical trust and interpretability, despite sMRI's potential for investigating brain alterations.

Method: Proposes DuSCN-FusionNet with dual-channel SCNs capturing inter-regional morphological relationships: intensity-based and heterogeneity-based SCNs constructed from ROI-wise mean intensity and intra-regional variability descriptors. Uses SCN-CNN encoder with late-stage fusion of auxiliary ROI-wise variability features and global statistical descriptors. Evaluated with stratified 10-fold cross-validation and 5-seed ensemble.

Result: Achieved mean balanced accuracy of 80.59%, AUC of 0.778 on Peking University ADHD-200 dataset. Precision: 81.66%, recall: 80.59%, F1-score: 80.27%. Grad-CAM adaptation to SCN domain enabled ROI-level importance scoring for biomarker identification.

Conclusion: DuSCN-FusionNet provides an interpretable sMRI-based framework for ADHD classification that addresses black-box limitations while achieving competitive performance and enabling identification of structurally relevant brain regions as potential biomarkers.
...
TL;DR: HINT is a dual-path compositional contextualized network for composed image retrieval that addresses the neglect of contextual information by performing contextualized encoding and amplifying similarity differences between matching and non-matching samples.
...
Method: Proposes HINT (dual-patH composItional coNtextualized neTwork) that performs contextualized encoding and amplifies similarity differences between matching and non-matching samples through a differential amplification mechanism.
...
Conclusion: HINT effectively addresses the contextual information neglect in CIR by incorporating contextualized encoding and similarity amplification, leading to improved performance in complex scenarios.
...
TL;DR: Meta-learning framework for human mesh recovery that produces optimization-friendly initializations and uses uncertainty-aware adaptive updates during test-time refinement, achieving SOTA performance with improved generalization.
...
Method: Three key innovations: (1) meta-learning strategy simulating test-time optimization during training to learn better initializations, (2) selective parameter caching that freezes converged joints to reduce computation, (3) distribution-based adaptive updates sampling parameter changes from learned distributions with uncertainty quantification. Uses stochastic approximation for intractable gradients.

Result: Achieves state-of-the-art performance, reducing MPJPE by 10.3 on 3DPW and 8.0 on Human3.6M compared to strong baselines. Shows superior domain adaptation with minimal performance degradation across different environments, providing meaningful uncertainty estimates correlated with actual errors.

Conclusion: Combining meta-learning and adaptive optimization enables accurate mesh recovery and robust generalization to challenging scenarios, addressing initialization and update efficiency problems in human mesh reconstruction.
...
### [97] [From Synthetic Data to Real Restorations: Diffusion Model for Patient-specific Dental Crown Completion](https://arxiv.org/abs/2603.26588)
*Dávid Pukanec,Tibor Kubík,Michal Španěl*

Main category: cs.CV

TL;DR: ToothCraft is a diffusion-based 3D model that generates complete tooth crowns from incomplete teeth using local anatomical context, trained on artificially created incomplete teeth data.


<details>
  <summary>Details</summary>
Motivation: There is a lack of training data for tooth crown completion tasks, and existing methods need to handle diverse tooth defects while ensuring generated crowns don't interfere with opposing teeth.

Method: Developed a diffusion-based model for 3D shape generation conditioned on local anatomical context. Created an augmentation pipeline to generate incomplete tooth geometries from complete dental arch datasets (3DS, ODD) for training data.

Result: Achieved 81.8% IoU and 0.00034 Chamfer Distance on synthetic test cases. Model works on real-world cases, effectively fills incomplete teeth with minimal intersection with opposing dentition.

Conclusion: ToothCraft successfully automates tooth crown completion with high accuracy and clinical applicability, reducing occlusal interference risk. Code and models will be publicly available.
...
TL;DR: Privacy-by-design framework for video anomaly detection that minimizes personally identifiable information exposure while maintaining detection performance, addressing GDPR compliance challenges.


<details>
  <summary>Details</summary>
Motivation: Video anomaly detection systems handle sensitive personal data (facial cues, demographic attributes) that creates GDPR compliance challenges, requiring data minimization to only what's strictly necessary for anomaly detection.

Method: Introduces "Only What's Necessary" framework combining breadth-based and depth-based data minimization mechanisms to suppress PII while preserving anomaly detection cues. Evaluates minimization configurations using VAD and privacy inference models with ranking methods and Pareto analysis.

Result: Identifies sweet spot operating points from non-dominated frontiers that minimize personal data exposure with limited degradation in detection performance. Extensive experiments on public datasets demonstrate framework effectiveness.

Conclusion: Proposed privacy-by-design framework successfully addresses GDPR compliance by enabling controlled minimization of PII in video anomaly detection while maintaining utility, with identified optimal trade-off points.
...
TL;DR: A framework for constructing soft equivariant models by projecting weights into designed subspaces, applicable to any pre-trained architecture with theoretical bounds on equivariance error.


<details>
  <summary>Details</summary>
Motivation: Strict equivariance is rarely satisfied in real-world data, limiting model performance. Controlling the degree of equivariance is desirable for practical applications.

Method: Proposes a general framework for soft equivariant models by projecting model weights into a designed subspace. The method works with any pre-trained architecture and provides theoretical bounds on equivariance error.

Result: Demonstrated effectiveness on multiple pre-trained backbones (ViT, ResNet) across image classification, semantic segmentation, and human-trajectory prediction tasks. Improved performance while reducing equivariance error on ImageNet benchmark.

Conclusion: The proposed framework enables controlled equivariance in models, balancing performance with equivariance properties across diverse computer vision tasks.
...
### [100] [From Pen to Pixel: Translating Hand-Drawn Plots into Graphical APIs via a Novel Benchmark and Efficient Adapter](https://arxiv.org/abs/2603.26356)
*Zhenghao Xu,Mengning Yang*

Main category: cs.CV

TL;DR: Plot2API for hand-drawn plots: New dataset HDpy-13 and efficient Plot-Adapter method to bridge domain gap and reduce computational costs for graphical API recommendations from hand-drawn plot images.


<details>
  <summary>Details</summary>
Motivation: Existing Plot2API systems fail with hand-drawn plot images due to domain gap and lack of expertise, while current models struggle with multi-domain/multi-language challenges requiring heavy computational resources.

Method: Introduces HDpy-13 dataset for hand-drawn plots and Plot-Adapter framework with lightweight CNN blocks for local feature capture and projection matrix sharing to reduce fine-tuning parameters across domains/languages.

Result: Experimental results demonstrate effectiveness of HDpy-13 dataset and efficiency of Plot-Adapter in improving graphical API recommendations for hand-drawn plot images while reducing computational costs.

Conclusion: The proposed HDpy-13 dataset and Plot-Adapter method successfully address the challenges of recommending graphical APIs for hand-drawn plots, making Plot2API more accessible to non-experts while being computationally efficient.
...
### [101] [MPDiT: Multi-Patch Global-to-Local Transformer Architecture For Efficient Flow Matching and Diffusion Model](https://arxiv.org/abs/2603.26357)
*Quan Dao,Dimitris Metaxas*

Main category: cs.CV

TL;DR: MPDiT introduces a hierarchical multi-patch transformer design for diffusion models, using larger patches in early blocks for global context and smaller patches in later blocks for local details, reducing computational cost by up to 50% while maintaining good generative performance.


<details>
  <summary>Details</summary>
Motivation: Current Diffusion Transformers (DiTs) use isotropic designs that process the same number of patchified tokens in every block, leading to heavy computational costs during training. The authors aim to create a more efficient architecture without sacrificing performance.

Method: Proposes a multi-patch transformer design with hierarchical patch sizes: early blocks use larger patches to capture coarse global context, while later blocks use smaller patches to refine local details. Also introduces improved designs for time and class embeddings to accelerate training convergence.

Result: The hierarchical design reduces computational cost by up to 50% in GFLOPs while achieving good generative performance on ImageNet dataset. The improved embeddings accelerate training convergence.

Conclusion: MPDiT demonstrates that hierarchical multi-patch transformer designs can significantly reduce computational costs in diffusion models while maintaining strong generative performance, offering an efficient alternative to isotropic DiT architectures.
...
### [102] [HandVQA: Diagnosing and Improving Fine-Grained Spatial Reasoning about Hands in Vision-Language Models](https://arxiv.org/abs/2603.26362)
...
TL;DR: HandVQA is a diagnostic benchmark with 1.6M questions to evaluate VLMs' fine-grained spatial reasoning about hand anatomy, revealing systematic limitations and enabling transfer learning improvements.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with fine-grained spatial reasoning of complex hand poses, which is critical for applications like robot-assisted surgery, chip manufacturing, and AR/VR human-AI interaction.

Method: Built HandVQA benchmark using high-quality 3D hand datasets (FreiHAND, InterHand2.6M, FPHA) with over 1.6M controlled multiple-choice questions probing spatial relationships between hand joints. Evaluated state-of-the-art VLMs (LLaVA, DeepSeek, Qwen-VL) in base and fine-tuned settings using LoRA.

Result: Revealed systematic limitations in current models including hallucinated finger parts, incorrect geometric interpretations, and poor generalization. Fine-tuning on HandVQA enabled zero-shot transfer learning, significantly improving accuracy on downstream tasks: hand gesture recognition (+10.33%) and hand-object interaction (+2.63%).

Conclusion: HandVQA exposes critical reasoning gaps in VLMs for fine-grained hand understanding and provides a validated path for improvement through 3D-grounded spatial knowledge transfer to downstream applications.
...
TL;DR: SCORE is a reinforcement learning framework for adaptive video token compression in multimodal LLMs, achieving 16x speedup while preserving 99.5% performance at 10% retention ratio.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs face prohibitive computational costs and performance degradation from "context rot" due to massive visual token redundancy in video understanding. Existing compression strategies rely on heuristics or fixed transformations decoupled from downstream tasks, limiting adaptability.

Method: SCORE learns an adaptive token compression policy using a lightweight policy network conditioned on surprise-augmented state representation with inter-frame residuals to capture temporal dynamics. Optimized via group-wise reinforcement learning with split-advantage estimator, stabilized by two-stage curriculum from static pseudo-videos to real dynamic videos.

Result: Extensive experiments show SCORE significantly outperforms state-of-the-art baselines on diverse video understanding benchmarks. Achieves 16x prefill speedup while preserving 99.5% of original performance at 10% retention ratio.

Conclusion: SCORE offers a scalable solution for efficient long-form video understanding by learning adaptive token compression policies that balance computational efficiency with performance preservation.
...
TL;DR: RAR proposes an iterative Restore, Assess and Repeat framework that integrates Image Quality Assessment and Image Restoration into a unified latent-domain model for handling unknown and composite degradations.


<details>
  <summary>Details</summary>
Motivation: Current image restoration methods suffer from limited generalization and inefficiency when dealing with unknown or composite degradations, as they typically keep IQA and IR as separate modules leading to latency and information loss.

Method: RAR introduces a unified framework that operates entirely in the latent domain, performing degradation identification, image restoration, and quality verification iteratively. The model is fully trainable end-to-end and dynamically adapts the restoration process through an assess-and-restore approach.
...
Conclusion: The RAR framework successfully addresses generalization and efficiency limitations by tightly integrating IQA and IR into a unified iterative process, enabling effective handling of diverse degradation scenarios.
...
TL;DR: Automated image-based system for objective assessment of cervical dystonia using head pose estimation and synthetic data training.


<details>
  <summary>Details</summary>
Motivation: Current cervical dystonia assessment relies on subjective clinical rating scales (TWSTRS) with low inter-rater reliability, lacking objective tools for monitoring disease severity and treatment response.

Method: Combines pretrained head-pose estimation algorithm for rotational symptoms with deep learning model trained on ~16,000 synthetic avatar images for translational symptoms (lateral shift). Validated in multicenter study comparing automated scores against consensus ratings of 20 clinical experts using 100 real patient images and 100 synthetic avatars.

Result: Strong agreement with expert ratings for rotational symptoms: torticollis (r=0.91), laterocollis (r=0.81), anteroretrocollis (r=0.78). Moderate correlation for lateral shift (r=0.55) but higher accuracy than human raters in controlled benchmark tests on avatars.

Conclusion: Synthetic training data bridges clinical data gap, enabling generalization to real-world patients. Provides validated, objective tool for CD postural assessment that can standardize clinical decision-making and trial evaluation.
...
TL;DR: HyVIC: A novel spatio-spectral variational architecture for hyperspectral image compression that balances spatial and spectral feature learning to achieve superior compression performance.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based HSI compression methods adapt natural image compression models without properly addressing the unique spatio-spectral redundancies in hyperspectral data, limiting their ability to effectively leverage hyperspectral characteristics.

Method: HyVIC architecture with four main components: adjustable spatio-spectral encoder, spatio-spectral hyperencoder, spatio-spectral hyperdecoder, and adjustable spatio-spectral decoder, plus a metric-driven strategy for hyperparameter selection.
...
Conclusion: Balancing spatial and spectral feature learning is crucial for HSI compression fidelity; the work provides insights and practical guidelines for future learning-based variational HSI compression research.
...
TL;DR: A sparse-camera dynamic reconstruction framework using Spatio-Temporal Distortion Field to achieve high-quality 4D reconstruction from limited camera inputs, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: High-quality 4D reconstruction currently requires expensive dense camera arrays (tens to hundreds of synchronized cameras), which severely limits practical scalability and real-world applications.

Method: Proposes a sparse-camera dynamic reconstruction framework with Spatio-Temporal Distortion Field as key innovation - a unified mechanism for modeling inconsistencies in generative observations across spatial and temporal dimensions. The complete pipeline enables 4D reconstruction from sparse and uncalibrated camera inputs.
...
Conclusion: The proposed framework enables practical, scalable 4D reconstruction by overcoming the dependency on costly dense camera arrays through innovative modeling of generative observation inconsistencies.
...
TL;DR: ClipTTT uses CLIP's image-text alignment to guide test-time training of LVLMs, reducing hallucinations caused by visual corruptions without modifying the base model.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models (LVLMs) suffer from increased hallucination rates when visual inputs are corrupted at test time, which acts as an additional distribution shift in real-world applications.

Method: Proposes CLIP-guided Test-Time Training (ClipTTT) that adapts LVLMs on the fly using a single test sample. It leverages pre-trained CLIP's image-text alignment as stable guidance to identify reliable self-supervision targets for rapid adaptation without altering the base LVLMs.

Result: Extensive experiments on standard hallucination benchmarks with 15 common corruptions show that ClipTTT effectively mitigates hallucinations and improves descriptive faithfulness under visual corruptions.

Conclusion: ClipTTT provides an effective test-time adaptation method that reduces hallucination in LVLMs when facing corrupted visual inputs, using CLIP's alignment capabilities as guidance without modifying the base model architecture.
...
TL;DR: AXON is a multi-stage diffusion-based framework that reconstructs high-fidelity 3D CT volumes directly from real chest X-rays, addressing limitations of existing methods that rely on synthetic projections.


<details>
  <summary>Details</summary>
Motivation: CT scans provide rich 3D anatomical details but have limitations: high radiation exposure, substantial costs, and limited availability. Chest X-rays are cost-effective and widely accessible but only provide 2D projections with limited pathological information. Existing 3D reconstruction methods predominantly rely on synthetic X-ray projections, limiting clinical generalization.

Method: AXON uses a multi-stage diffusion-based framework with coarse-to-fine strategy: 1) Brownian Bridge diffusion model for global structural synthesis, 2) ControlNet-based refinement stage for local intensity optimization, 3) supports bi-planar X-ray input to mitigate depth ambiguities, and 4) integrated super-resolution network to upscale volumes to diagnostic-grade resolution.

Result: AXON significantly outperforms state-of-the-art baselines, achieving 11.9% improvement in PSNR and 11.0% increase in SSIM with robust generalizability across disparate clinical distributions, as demonstrated on both public and external datasets.

Conclusion: AXON offers a transformative solution for reconstructing 3D CT volumes directly from real X-rays, increasing diagnostic accessibility while overcoming limitations of existing methods that rely on synthetic projections.
...
TL;DR: Learnable Quantum Efficiency (LQE) is a physics-inspired, interpretable dimensionality reduction method for hyperspectral urban driving data that parameterizes smooth spectral response functions with physical constraints, achieving state-of-the-art semantic segmentation performance with high parameter efficiency.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral sensing provides rich spectral information for urban driving scene understanding, but its high dimensionality poses challenges for interpretation and efficient learning. Current methods lack physical interpretability and constraints, limiting their effectiveness and connection to real sensor design.

Method: LQE parameterizes smooth high-order spectral response functions that emulate plausible sensor quantum efficiency curves with physical constraints: single dominant peak, smooth responses, and bounded bandwidth. This creates a compact, differentiable spectral representation that can be end-to-end trained within semantic segmentation models.

Result: LQE achieves highest average mIoU across three hyperspectral urban driving datasets (HyKo, HSI-Drive, Hyperspectral City), outperforming conventional methods by 2.45%, 0.45%, 1.04% and learnable methods by 1.18%, 1.56%, 0.81%. It maintains strong parameter efficiency (12-36 parameters vs 51-22K for competitors) with competitive inference latency.

Conclusion: Physics-informed spectral learning with LQE improves both performance and interpretability, providing a principled bridge between hyperspectral perception and data-driven multispectral sensor design for automotive vision systems, with learned filters converging to dataset-intrinsic wavelength patterns.
...
TL;DR: OVI-MAP: Real-time incremental 3D instance-semantic mapping with open-vocabulary capabilities by decoupling instance reconstruction from semantic inference.


<details>
  <summary>Details</summary>
Motivation: Incremental open-vocabulary 3D instance-semantic mapping is essential for autonomous agents but remains challenging due to needs for robust instance segmentation, real-time processing, and flexible open-set reasoning. Existing methods have limitations with closed-set assumptions or dense per-pixel language fusion.

Method: Decouples instance reconstruction from semantic inference. Builds class-agnostic 3D instance map incrementally from RGB-D input, while extracting semantic features only from a small set of automatically selected views using vision-language models.

Result: Enables stable instance tracking and zero-shot semantic labeling throughout online exploration. Operates in real time and outperforms state-of-the-art open-vocabulary mapping baselines on standard benchmarks.

Conclusion: OVI-MAP provides an effective solution for real-time incremental open-vocabulary 3D instance-semantic mapping by separating instance reconstruction from semantic inference, achieving better performance than existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current generative video models require massive datasets for rare weather scenarios, while 3D-aware editing methods suffer from costly per-scene optimization and geometric/illumination entanglement issues.

Method: Uses G-buffer Dual-pass Editing mechanism: Geometry Pass leverages explicit structural foundations for surface-anchored physical interactions, and Light Pass analytically resolves light transport for dynamic 3D local relighting.
...
Conclusion: AutoWeather4D provides an efficient feed-forward solution for weather editing that overcomes data constraints and optimization bottlenecks of existing methods, enabling practical data augmentation for autonomous driving systems.
...
TL;DR: A contrastive flow matching model for co-speech gesture generation that uses mismatched audio-text conditions as negative examples to improve semantic grounding and cross-modal consistency.


<details>
  <summary>Details</summary>
Motivation: Existing gesture generation methods have limitations: they rely on external semantic retrieval (limiting generalization), flow-matching methods only use positive samples (leading to rhythmic rather than sparse gestures), and most models treat body parts in isolation (failing to maintain cross-modal consistency).
...
Result: Outperforms state-of-the-art methods on BEAT2 and SHOW datasets through extensive experiments and user studies.

Conclusion: The proposed contrastive approach effectively addresses limitations of existing methods by improving semantic grounding, handling sparse gestures, and maintaining cross-modal coherence in co-speech gesture generation.
...
TL;DR: A framework that aligns partial 3D reconstructions to complete reference models from Google Earth Studio using semantic features, enabling globally consistent alignment even without visual overlap between input views.


<details>
  <summary>Details</summary>
Motivation: Existing 3D reconstruction pipelines struggle with large-scale scenes when input views have little or no overlap, producing disconnected partial reconstructions or incorrectly merging non-overlapping regions. There's a need for globally consistent alignment without visual overlap.

Method: Uses Google Earth Studio renderings as complete reference models, represents them with 3D Gaussian Splatting augmented with semantic features, and formulates alignment as an inverse feature-based optimization that estimates global 6DoF pose and scale while keeping the reference model fixed.

Result: The approach consistently improves global alignment when initialized with various classical and learning-based pipelines, mitigates failure modes of state-of-the-art end-to-end models, and introduces the WikiEarth dataset for evaluation.

Conclusion: Semantic features bridge the domain gap between real-world photos and pseudo-synthetic renderings, enabling globally consistent alignment of partial reconstructions to complete reference models even without visual overlap, with code and data to be released.
...
TL;DR: MA-Bench is a new benchmark for evaluating MLLMs on micro-action understanding, featuring 1,000 videos with 12,000 QA pairs across three evaluation tiers. Results show current MLLMs struggle with motion granularity and fine-grained body dynamics, but fine-tuning on the accompanying training dataset (MA-Bench-Train) improves performance.


<details>
  <summary>Details</summary>
Motivation: Multimodal Large Language Models (MLLMs) have advanced rapidly, but their potential for micro-action understanding (crucial for human emotion analysis) remains unexplored due to lack of specialized benchmarks. Micro-actions are subtle behaviors that reveal emotional states, but current models lack evaluation frameworks for this domain.

Method: Created MA-Bench with 1,000 videos and a three-tier evaluation architecture: 1) micro-action perception, 2) relational comprehension, and 3) interpretive reasoning. Contains 12,000 structured question-answer pairs. Also built MA-Bench-Train with 20.5K videos annotated with structured micro-action captions for fine-tuning. Evaluated 23 representative MLLMs and fine-tuned Qwen3-VL-8B on the training dataset.

Result: Evaluation of 23 MLLMs revealed significant challenges in capturing motion granularity and fine-grained body-part dynamics. However, Qwen3-VL-8B fine-tuned on MA-Bench-Train showed clear performance improvements across micro-action reasoning and explanation tasks, demonstrating the value of specialized training data.

Conclusion: MA-Bench establishes a foundational benchmark for advancing MLLMs in understanding subtle micro-actions and human-related behaviors. The benchmark and training dataset provide essential resources for improving model capabilities in this important domain for emotion analysis and human behavior understanding.
...
<details>
  <summary>Details</summary>
Motivation: There's a dilemma in video representation learning: fine-tuning heavy temporal modules from image-pretrained models compromises inter-video semantic separability (ability to distinguish objects across videos), while reducing tunable parameters hinders intra-video temporal consistency (stable representations of the same object within a video). This indicates a trade-off between these two essential properties during image-to-video transfer.

Method: Co-Settle applies a lightweight projection layer on top of a frozen image-pretrained encoder. The framework adjusts representation space using a temporal cycle consistency objective (for intra-video consistency) and a semantic separability constraint (for inter-video discrimination). The approach includes theoretical analysis showing the optimized projection yields better trade-off under appropriate conditions.

Result: Experiments on eight image-pretrained models show consistent improvements across multiple levels of video tasks with only five epochs of self-supervised training. The method demonstrates effectiveness in balancing the consistency-separability trade-off.

Conclusion: Co-Settle provides an effective framework for transferring image-pretrained models to video tasks by addressing the fundamental trade-off between intra-video temporal consistency and inter-video semantic separability through lightweight projection and carefully designed objectives.
...
TL;DR: VGGRPO is a latent geometry-guided framework that improves geometric consistency in video diffusion models without architectural modifications, using a Latent Geometry Model and latent-space reinforcement learning with geometry and camera smoothness rewards.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models achieve high visual quality but lack geometric consistency. Existing solutions either modify architecture (compromising pretrained generalization) or use RGB-space alignment methods that are computationally expensive and limited to static scenes.

Method: Proposes VGGRPO with: 1) Latent Geometry Model (LGM) that connects video diffusion latents to geometry foundation models for direct geometry decoding, 2) Group Relative Policy Optimization with two rewards: camera motion smoothness and geometry reprojection consistency, all operating in latent space to avoid VAE decoding overhead.

Result: Experiments on static and dynamic benchmarks show improved camera stability, geometry consistency, and overall video quality while eliminating costly VAE decoding, making the approach efficient and flexible for world-consistent video generation.
...
TL;DR: EgoPoint-Ground is a new large-scale multimodal dataset for egocentric deictic visual grounding, featuring hand-pointing with speech, with a novel SV-CoT framework that improves grounding performance by 11.7%.


<details>
  <summary>Details</summary>
Motivation: Traditional visual grounding relies on textual descriptions which struggle with linguistic ambiguity and ignore non-verbal deictic cues like hand-pointing that are prevalent in real-world egocentric interactions.

Method: 1) Created EgoPoint-Ground dataset with 15k+ interactive samples featuring hand-target bounding box pairs and dense semantic captions. 2) Proposed SV-CoT framework that reformulates grounding as structured inference using Visual Chain-of-Thought to synergize gestural and linguistic cues.

Result: SV-CoT achieves 11.7% absolute improvement over existing methods, effectively mitigating semantic ambiguity and advancing multimodal intent comprehension. Comprehensive benchmark established for evaluating MLLMs and VG architectures.

Conclusion: The work bridges the gap between traditional text-only visual grounding and real-world multimodal interactions, enabling better comprehension of physical intents through hand-pointing and speech cues.
...
TL;DR: FOSSA: A Transformer-based architecture for zero-shot Depth from Defocus (DfD) with new ZEDD benchmark and synthetic training pipeline, achieving 55.7% error reduction.


<details>
  <summary>Details</summary>
Motivation: Previous DfD methods overfit to specific datasets, lacking zero-shot generalization to real-world settings. Need for better benchmarks and architectures that can generalize without dataset-specific training.

Method: 1) ZEDD benchmark with 8.3x more scenes and higher quality images/ground-truth; 2) FOSSA Transformer architecture with stack attention layer and focus distance embedding for efficient cross-stack information exchange; 3) Synthetic training pipeline using existing RGBD datasets to generate focus stacks.

Result: Significant improvement over baselines on ZEDD and other benchmarks, reducing errors by up to 55.7%. The method demonstrates strong zero-shot generalization capability.

Conclusion: FOSSA enables effective zero-shot DfD through novel architecture design and comprehensive benchmark, advancing practical depth estimation from focus stacks with real-world applicability.
...
TL;DR: GaussianGPT: An autoregressive transformer model that generates 3D scenes by directly producing 3D Gaussians via next-token prediction, offering step-by-step scene construction with flexible editing capabilities.


<details>
  <summary>Details</summary>
Motivation: To explore an alternative to diffusion/flow-matching approaches for 3D generative modeling by leveraging the compositional inductive biases and scalability of autoregressive transformers for controllable and context-aware 3D generation.

Method: 1) Compress Gaussian primitives into discrete latent grid using sparse 3D convolutional autoencoder with vector quantization; 2) Serialize tokens and model with causal transformer using 3D rotary positional embedding; 3) Generate scenes sequentially via next-token prediction.
...
Conclusion: Autoregressive transformers represent a complementary paradigm to diffusion methods for 3D generation, offering explicit representations, compositional biases, and enhanced controllability for context-aware scene synthesis.
...
TL;DR: The paper presents a method to reconstruct 3D geometry and appearance from sparse fixed cameras by exploiting object motion during manipulation, using joint pose-shape optimization with 2D Gaussian splatting and a novel appearance factorization model.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D reconstruction from sparse fixed cameras is fundamentally limited by viewpoint constraints. The paper aims to overcome this limitation by leveraging opportunistic object motion when people manipulate objects, effectively creating additional virtual viewpoints as static cameras "orbit" the object.

Method: The method uses: 1) Joint pose and shape optimization with 2D Gaussian splatting using alternating minimization of 6DoF trajectories and primitive parameters, and 2) A novel appearance model that factorizes diffuse and specular components with reflected directional probing within spherical harmonics space.

Result: Extensive experiments on synthetic and real-world datasets with extremely sparse viewpoints show that the method recovers significantly more accurate geometry and appearance than state-of-the-art baselines.
...
### [122] [Rethinking Feature Conditioning for Robust Forged Media Detection in Edge AI Sensing Systems](https://arxiv.org/abs/2603.26387)
*Izaldein Al-Zyoud,Abdulmotaleb El Saddik*

Main category: eess.IV

TL;DR: Linear probing on frozen DINOv3 ConvNeXt backbones yields competitive forged-media detection, but feature conditioning choice significantly impacts robustness to manipulation and dataset shifts.


<details>
  <summary>Details</summary>
Motivation: Generalization under manipulation and dataset shift remains challenging for forged media detection in edge systems. While frozen vision foundation models with linear probes are strong baselines, most pipelines use default backbone outputs without testing conditioning at the frozen feature interface.

Method: First controlled probing study on DINOv3 ConvNeXt, fixing backbone, head, data, and optimization while varying conditioning. Evaluated five conditioning variants on FaceForensics++ c23 under in-distribution testing, leave-one-manipulation-out (LOMO), and cross-dataset transfer to Celeb-DF v2 and DeepFakeDetection.

Result: Conditioning alone changes LOMO mean AUC by 6.1 points and reverses ID-vs-OOD ranking. LN-Affine (default ConvNeXt head output) is strongest on external datasets, while LayerNorm is strongest in-distribution. In ConvNeXt-Base replication, OOD winner becomes protocol-dependent, and ID-optimal selection fails as robust deployment rule.

Conclusion: Feature conditioning is a first-order design variable that should be selected with robustness-oriented validation, not just in-distribution accuracy alone.
...
### [123] [Adapting Frozen Mono-modal Backbones for Multi-modal Registration via Contrast-Agnostic Instance Optimization](https://arxiv.org/abs/2603.26393)
*Yi Zhang,Yidong Zhao,Qian Tao*

Main category: eess.IV

TL;DR: A lightweight adaptation framework that enables frozen mono-modal registration models to work for multi-modal scenarios via style transfer and instance optimization, avoiding expensive full fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Deep learning registration models struggle with multi-modal scenarios due to intensity distribution shifts. Full fine-tuning of modern architectures (Transformers, deep U-Nets) is computationally expensive in 3D, and naive fine-tuning risks performance degradation with drastic domain shifts.

Method: Integrates frozen pretrained mono-modal registration model with lightweight adaptation pipeline using style transfer based on contrast-agnostic representation generation and refinement modules. Uses instance optimization at test time to bridge modality/domain gaps. Design is orthogonal to backbone choice.

Result: On Learn2Reg 2025 LUMIR validation set: ranks 2nd on multi-modal subset, 3rd on out-of-domain subset, and 4th overall in Dice score. Shows consistent improvements over pretrained state-of-the-art mono-modal backbone.

Conclusion: Combining frozen mono-modal models with modality adaptation and lightweight instance optimization provides an effective, practical pathway for robust multi-modal registration without computational burden of full fine-tuning.
...
### [124] [Learning to Recorrupt: Noise Distribution Agnostic Self-Supervised Image Denoising](https://arxiv.org/abs/2603.25869)
*Brayan Monroy,Jorge Bacca,Julián Tachella*

Main category: eess.IV

TL;DR: L2R is a self-supervised denoising method that learns the recorruption process using a neural network, eliminating the need for prior knowledge of noise distribution.


<details>
  <summary>Details</summary>
Motivation: Existing recorruption-based denoising methods require precise knowledge of noise distribution, which is often unavailable in real-world scenarios.

Method: Introduces a learnable monotonic neural network that learns the recorruption process through a min-max saddle-point objective, making it noise distribution-agnostic.

Result: Achieves state-of-the-art performance across unconventional and heavy-tailed noise distributions (log-gamma, Laplace, spatially correlated) and signal-dependent noise models (Poisson-Gaussian).

Conclusion: L2R provides an effective noise distribution-agnostic approach to self-supervised denoising that works well with various complex noise types.
...
### [125] [Adapting Segment Anything Model 3 for Concept-Driven Lesion Segmentation in Medical Images: An Experimental Study](https://arxiv.org/abs/2603.25945)
*Guoping Xu,Jayaram K. Udupa,Yubing Tong,Xin Long,Ying Zhang,Jie Deng,Weiguo Lu,You Zhang*

Main category: eess.IV

TL;DR: Systematic evaluation of Segment Anything Model 3 (SAM3) for lesion segmentation across multiple medical imaging modalities using concept-based prompts and various fine-tuning strategies.


<details>
  <summary>Details</summary>
Motivation: Most existing lesion segmentation methods are limited to specific anatomical sites or imaging modalities, lacking generalizability. Vision-language foundation models offer promising concept-driven segmentation, but their application to medical imaging, particularly with SAM3, remains underexplored.

Method: Systematic evaluation of SAM3 using geometric bounding boxes and concept-based text/image prompts across multiple modalities (MRI, CT, ultrasound, dermoscopy, endoscopy). Incorporates additional prior knowledge (adjacent-slice predictions, multiparametric information, prior annotations). Compares different fine-tuning strategies including partial module tuning, adapter-based methods, and full-model optimization.

Result: Experiments on 13 datasets covering 11 lesion types demonstrate SAM3 achieves strong cross-modality generalization, reliable concept-driven segmentation, and accurate lesion delineation.

Conclusion: SAM3 shows potential for scalable and practical medical image segmentation through concept-based foundation models, enabling flexible lesion segmentation across diverse imaging modalities.
...
### [126] [Cone-Beam CT Image Quality Enhancement Using A Latent Diffusion Model Trained with Simulated CBCT Artifacts](https://arxiv.org/abs/2603.26014)
*Naruki Murahashi,Mitsuhiro Nakamura,Megumi Nakao*

Main category: eess.IV

TL;DR: Proposes a novel CBCT image quality enhancement method using conditional latent diffusion model with pseudo-CBCT images to improve quality while preserving anatomical structures.
...
Method: Uses conditional latent diffusion model trained on spatially consistent CT-pseudo-CBCT pairs. Pseudo-CBCT images are created from CT images by simulating CBCT artifacts, enabling self-supervised learning that preserves anatomical structures.
...
Conclusion: The proposed overcorrection-free method effectively enhances CBCT image quality while maintaining anatomical integrity, offering improved efficiency and performance over existing approaches.
...
### [127] [FINDER: Zero-Shot Field-Integrated Network for Distortion-free EPI Reconstruction in Diffusion MRI](https://arxiv.org/abs/2603.26117)
...
TL;DR: FINDER is a zero-shot, scan-specific framework that jointly optimizes image reconstruction and B0 field map estimation to correct geometric distortions in EPI diffusion MRI using physics-guided unrolled networks and implicit neural representations.


<details>
  <summary>Details</summary>
Motivation: EPI diffusion MRI suffers from severe geometric distortions due to B0 field inhomogeneities, and existing deep learning methods lack robust geometric distortion correction in self-supervised frameworks.

Method: FINDER uses a physics-guided unrolled network with dual-domain denoisers and virtual coil extensions for data consistency, coupled with an Implicit Neural Representation (INR) to model the off-resonance field as a continuous function. It employs alternating minimization to jointly optimize reconstruction and field map estimation.
...
Conclusion: FINDER offers a robust, zero-shot solution for high-quality diffusion imaging by integrating geometric distortion correction into a self-supervised reconstruction framework through joint optimization of image and field map.
...
### [128] [Exploiting the Degrees of Freedom: Multi-Dimensional Spatially-Coupled Codes Based on Gradient Descent](https://arxiv.org/abs/2603.25824)
*Ata Tanrıkulu,Mete Yıldırım,Ahmed Hareedy*

Main category: cs.IT

TL;DR: Proposes a probabilistic gradient-descent framework for designing high-performance multi-dimensional spatially-coupled LDPC codes that reduces detrimental graph objects and improves error-rate performance.


<details>
  <summary>Details</summary>
Motivation: Multi-dimensional SC codes face design complexity challenges as degrees of freedom increase, making it difficult to exploit them effectively for performance improvement in storage/transmission systems.

Method: Probabilistic framework using gradient-descent algorithm to minimize expected detrimental objects, expressed via probability-distribution matrix. Combines GD with MCMC finite-length optimizer to produce final MD-SC codes.

Result: GD-MD codes show notably lower numbers of targeted detrimental objects compared to state-of-the-art, and significant error-rate improvements over MD-SC codes from uniform distribution.

Conclusion: The proposed GD-based probabilistic framework effectively addresses MD-SC code design complexity, producing superior codes with reduced detrimental graph structures and improved performance.
...
### [129] [Measurement Campaigns, Datasets, and Curve Fitting Officially Used by 3GPP in the Release 19 for Channel Modeling in TR 38.901 for 7-24 GHz](https://arxiv.org/abs/2603.25927)
*Hitesh Poddar,Jianhua Zhang,Ximan Liu,Mansoor Shafi*

Main category: cs.IT

TL;DR: 3GPP Release 19 channel modeling documentation for 7-24 GHz frequency range, covering measurement campaigns, datasets, and curve fitting methods used in TR 38.901.


<details>
  <summary>Details</summary>
Motivation: To provide standardized channel modeling for 5G/6G systems operating in the 7-24 GHz frequency range, which is important for higher frequency deployments and requires accurate propagation models based on real measurement data.

Method: Comprehensive measurement campaigns collecting propagation data, creation of standardized datasets, and development of curve fitting techniques to model channel characteristics for the 7-24 GHz band.

Result: Official 3GPP Release 19 channel models documented in TR 38.901, providing validated propagation models, path loss parameters, and channel characteristics for 7-24 GHz frequencies.

Conclusion: The document establishes standardized channel modeling framework for 7-24 GHz frequencies in 3GPP systems, enabling consistent evaluation and deployment of higher frequency wireless networks.
...
### [130] [Enormous Fluid Antenna Systems (E-FAS) under Correlated Surface-Wave Leakage: Physical Layer Security](https://arxiv.org/abs/2603.25943)
*Farshad Rostami Ghadi,Kai-Kit Wong,Masoud Kaveh,Mohammad Javad Ahmadi,Kin-Fai Tong,Hyundong Shin*

Main category: cs.IT

TL;DR: E-FAS enhances physical layer security through guided surface-wave routing, preventing secrecy collapse with artificial noise and achieving optimal power allocation.


<details>
  <summary>Details</summary>
Motivation: To develop a secrecy analysis framework for enormous fluid antenna systems (E-FAS) that addresses practical channel estimation and surface-wave leakage vulnerabilities in wiretap scenarios.

Method: Introduces correlated SW-leakage model for statistical coupling between legitimate and eavesdropper channels, uses MMSE channel estimation with MRT and artificial noise, derives closed-form SOP and tractable ESR expressions exploiting two-timescale nature.

Result: Three key insights: (1) AN prevents asymptotic secrecy collapse, (2) optimal power split is interior solution, (3) routing gain improves both signal strength and channel estimation, raising SINR ceiling. E-FAS significantly expands secure operating region compared to conventional transmission.

Conclusion: E-FAS with proper AN design and power allocation provides substantial physical layer security advantages over conventional space-wave systems by leveraging controllable surface-wave routing gains while mitigating leakage vulnerabilities.
...
### [131] [CL-SEC: Cross-Layer Semantic Error Correction Empowered by Language Models](https://arxiv.org/abs/2603.26125)
*Yirun Wang,Yuyang Du,Soung Chang Liew,Yuchen Pan,Feifan Zhang,Lihao Zhang*

Main category: cs.IT

TL;DR: CL-SEC is a cross-layer semantic error correction framework that integrates physical and application layer information using language models to achieve precise text reconstruction, outperforming single-layer approaches.


<details>
  <summary>Details</summary>
Motivation: Existing semantic error correction methods rely only on application-layer context from language models, ignoring valuable physical-layer information. This limitation prevents optimal error correction performance.

Method: Proposes Cross-Layer SEC (CL-SEC) that integrates information from both physical and application layers using a Bayesian combination in product form, leveraging language models for joint error correction.

Result: CL-SEC achieves significantly improved performance over isolated-layer methods across multiple metrics: bit-error rate, word-error rate, and semantic fidelity scores.

Conclusion: Cross-layer integration of physical and application layer information enables more precise verbatim reconstruction of transmitted messages using language models, advancing semantic error correction beyond semantic meaning recovery alone.
...
### [132] [SAFT: Sensitivity-Aware Filtering and Transmission for Adaptive 3D Point Cloud Communication over Wireless Channels](https://arxiv.org/abs/2603.26197)
*Huda Adam Sirag Mekki,Hui Yuan,Mohanad M. G. Hassan,Zejia Chen,Guanghui Zhang*

Main category: cs.IT

TL;DR: SAFT is a learned transmission framework for 3D point clouds that uses sensitivity-aware token filtering and SNR-adaptive reconstruction to improve geometric fidelity, especially in low-SNR conditions.


<details>
  <summary>Details</summary>
Motivation: Reliable transmission of 3D point clouds over wireless channels is challenging due to time-varying SNR and limited bandwidth. Existing methods struggle with adaptive reconstruction under varying channel conditions.

Method: SAFT integrates a Point-BERT-inspired encoder, sensitivity-guided token filtering (STF) unit that assigns importance scores based on reconstruction sensitivity under channel perturbation, quantization block, and SNR-aware decoder. Uses training-only symbol-usage penalty to stabilize discrete representation.

Result: SAFT improves geometric fidelity (D1/D2 PSNR) compared to separate source-channel coding pipeline (G-PCC with LDPC and QAM) and existing learned baselines. Largest gains observed in low-SNR regimes, highlighting improved robustness under limited bandwidth.

Conclusion: SAFT provides an effective learned transmission framework for 3D point clouds that adapts to varying channel conditions, particularly improving performance in challenging low-SNR wireless environments.
...
### [133] [Rotatable Antenna Enhanced Multicast Communication System](https://arxiv.org/abs/2603.26388)
*Weihua Zhu,Beixiong Zheng,Lipeng Zhu,Jie Tang,Yong Zeng*

Main category: cs.IT

TL;DR: RA-enhanced multicast system improves fairness by jointly optimizing beamforming and antenna rotation to maximize minimum SINR among users.


<details>
  <summary>Details</summary>
Motivation: Rotatable antennas provide additional spatial degrees of freedom for fairness-oriented multicast transmission, enabling dynamic boresight adjustment per antenna to improve system performance.

Method: Joint optimization of multicast beamforming vectors and RA boresight directions using alternating optimization algorithm with quadratic transform reformulation, convex beamforming subproblem, and SCA for boresight refinement.

Result: Simulation results show the RA-based scheme substantially enhances fairness performance compared to fixed antenna and random-orientation benchmarks.

Conclusion: Rotatable antennas effectively improve multicast system fairness by providing additional spatial degrees of freedom through dynamic boresight optimization.
...
### [134] [Security-Spectral Efficiency Tradeoff in STAR-RIS RSMA: A Max-Min Fairness Framework](https://arxiv.org/abs/2603.26532)
*Huiyun Xia,Yijie Mao,Sai Xu,Shuai Han,Hongbo Zhu*

Main category: cs.IT

TL;DR: STAR-RIS-assisted secure RSMA system with internal/external eavesdroppers; joint optimization of beamforming and phase shifts for max-min fairness under secrecy constraints.


<details>
  <summary>Details</summary>
Motivation: STAR-RIS enables full-space coverage but exposes wireless transmissions to security threats from multiple spatial directions, especially when both internal and external eavesdroppers coexist in transmission and reflection regions.

Method: Formulate max-min fairness problem under secrecy constraints and develop iterative algorithm to jointly optimize transmit beamforming and STAR-RIS phase shifts for RSMA system.

Result: Simulation results demonstrate that the proposed scheme improves spectral efficiency while maintaining confidentiality against both internal and external eavesdroppers.

Conclusion: The proposed STAR-RIS-assisted secure RSMA system with joint optimization effectively addresses the trade-off between spectral efficiency and confidentiality in presence of multiple eavesdropper types.
...
### [135] [Function-Based Minimal Linear Codes over Galois Rings $\mathrm{GR}(p^{n}, \ell)$: Minimality Criteria and Infinite Constructions](https://arxiv.org/abs/2603.26614)
*Biplab Chatterjee,Sihem Mesnager,Ratnesh Kumar Mishra,Makhan Maji,Kalyan Hansda*

Main category: cs.IT

TL;DR: Extends minimal linear code theory from finite fields to Galois rings, establishing new bounds and function-based minimality criteria in ring setting.


<details>
  <summary>Details</summary>
Motivation: To generalize minimal linear code theory from finite fields to Galois rings, addressing algebraic challenges posed by zero divisors and richer module structure in ring settings.

Method: Extends necessary/sufficient conditions for minimal codes over Galois rings, generalizes function-based minimality criteria using Frobenius duality and chain structure of Galois rings.

Result: Derives refined necessary/sufficient conditions for minimal codes from functions over GR(p^n,ℓ), constructs infinite families of minimal linear codes over Galois rings.

Conclusion: Provides unified framework connecting minimality theory, module duality over Frobenius rings, and function-based code constructions, significantly generalizing previous field-based results.
...
### [136] [USAM: A Unified Safety-Age metric for Timeliness in Heterogeneous IoT Systems](https://arxiv.org/abs/2603.26628)
*Mikael Gidlund*

Main category: cs.IT

TL;DR: The paper introduces USAM, a new safety-aware timeliness metric for IoT that combines information freshness, deadline reliability, and deterministic response-time feasibility into a single measure, addressing limitations of classical freshness metrics like Age of Information.


<details>
  <summary>Details</summary>
Motivation: Classical timeliness metrics (Age of Information) focus on freshness but fail to account for deterministic safety timing requirements in cyber-physical systems. Freshness metrics may indicate satisfactory performance even when worst-case timing guarantees required by functional safety standards are violated.

Method: Introduces Unified Safety-Age Metric (USAM) that integrates information freshness, deadline reliability, and deterministic response-time feasibility. Analyzes heterogeneous IoT traffic served by a gateway with intermittent receiver readiness in ultra-sparse regime typical of massive machine-type communications.

Result: Analysis shows that as device activity decreases, queueing delays become negligible and system timeliness becomes dominated by infrastructure readiness and deterministic response-time constraints. Feasibility is determined primarily by receiver duty cycle rather than average traffic load. USAM explicitly captures feasibility boundaries imposed by heterogeneous traffic requirements.

Conclusion: USAM provides a safety-aware timeliness metric that addresses limitations of classical freshness metrics. The framework offers foundation for analyzing safety-aware communication architectures in large-scale IoT systems, particularly important for monitoring, control, and safety-critical communications over shared wireless infrastructure.
...
### [137] [Empowering Epidemic Response: The Role of Reinforcement Learning in Infectious Disease Control](https://arxiv.org/abs/2603.25771)
*Mutong Liu,Yang Liu,Jiming Liu*

Main category: cs.LG

TL;DR: A survey paper reviewing how reinforcement learning has been applied to optimize infectious disease control strategies, covering resource allocation, policy balancing, and coordinated interventions.


<details>
  <summary>Details</summary>
Motivation: RL's adaptability to dynamic systems and ability to maximize long-term outcomes makes it suitable for infectious disease control, but there are few comprehensive surveys on RL applications specifically for optimizing public health interventions.

Method: Literature review methodology - analyzing and synthesizing recent publications on RL applications in infectious disease control, particularly focusing on COVID-19 and other infectious diseases.

Result: Identifies RL applications in four critical public health areas: resource allocation, balancing lives vs. livelihoods, mixed intervention policies, and inter-regional coordinated control.

Conclusion: RL shows promising potential for assisting public health sectors in disease control, with several future research directions identified for further exploration.
...
### [138] [Pure and Physics-Guided Deep Learning Solutions for Spatio-Temporal Groundwater Level Prediction at Arbitrary Locations](https://arxiv.org/abs/2603.25779)
*Matteo Salis,Gabriele Sartor,Rosa Meo,Stefano Ferraris,Abdourrahmane M. Atto*

Main category: cs.LG

TL;DR: STAINet is a physics-guided deep learning model for groundwater level prediction that combines attention mechanisms with groundwater flow equations to improve accuracy and physical interpretability.


<details>
  <summary>Details</summary>
Motivation: Groundwater modeling is challenging due to complex relationships, computational demands of theory-based models, and limitations of traditional approaches. Data-driven deep learning offers flexibility but needs physical grounding for trustworthiness and generalization.

Method: Proposed STAINet, an attention-based deep learning model for weekly groundwater prediction using sparse groundwater measurements and dense weather data. Developed three physics-guided variants: STAINet-IB (inductive bias estimating equation components), STAINet-ILB (learning bias with additional loss terms), and STAINet-ILRB (incorporating expert-estimated recharge zones).
...
Conclusion: Physics-guided deep learning enhances both generalization ability and trustworthiness of groundwater models, representing a promising direction for hybrid Earth system modeling that combines data-driven flexibility with physical principles.
...
### [139] [MAGNET: Autonomous Expert Model Generation via Decentralized Autoresearch and BitNet Training](https://arxiv.org/abs/2603.25813)
*Yongwan Kim,Sungchul Park*

Main category: cs.LG

TL;DR: MAGNET is a decentralized system for autonomous generation, training, and serving of domain-expert language models across commodity hardware using CPU-native inference and on-chain contribution tracking.


<details>
  <summary>Details</summary>
Motivation: To enable autonomous creation and deployment of specialized language models without requiring expensive GPU hardware, making AI development more accessible and decentralized.
...
Result: Validated through three case studies: video safety classification (balanced accuracy improved from 0.9287 to 0.9851), cryptocurrency directional prediction (hit rate improved from 41% to 54.9%), and BitNet hyperparameter optimization (10-phase sweep with -16.7% validation loss reduction).

Conclusion: MAGNET demonstrates a functional decentralized system for autonomous AI model development that can achieve strong performance improvements across diverse domains while operating on commodity hardware without GPU requirements.
...
### [140] [A Compression Perspective on Simplicity Bias](https://arxiv.org/abs/2603.25839)
*Tom Marty,Eric Elmoznino,Leo Gagnon,Tejas Kasetty,Mizu Nishikawa-Toomey,Sarthak Mittal,Guillaume Lajoie,Dhanya Sridhar*

Main category: cs.LG

TL;DR: Neural networks' simplicity bias explained through Minimum Description Length: learning is optimal two-part compression balancing model complexity vs. predictive power, predicting feature transitions from simple to complex as data increases.


<details>
  <summary>Details</summary>
Motivation: To understand the simplicity bias in deep neural networks through the lens of Minimum Description Length principle, explaining why networks prefer simple functions and how this affects feature selection across different data regimes.

Method: Formalize supervised learning as optimal two-part lossless compression problem using Minimum Description Length framework, analyzing trade-off between model complexity (hypothesis description cost) and predictive power (data description cost).

Result: Theory predicts learners transition through qualitatively different features (simple spurious shortcuts → complex features) as training data increases, identifies distinct data regimes where increasing data promotes robustness vs. where limiting data acts as complexity-based regularization.

Conclusion: Neural networks' feature selection follows same trajectory as optimal two-part compressors, validating MDL framework for explaining simplicity bias and providing insights into data regimes affecting robustness and regularization.
...
### [141] [Incorporating contextual information into KGWAS for interpretable GWAS discovery](https://arxiv.org/abs/2603.25855)
...
TL;DR: KGWAS framework improved by using cell-type specific knowledge graphs from perturb-seq data instead of general-purpose KGs, reducing spurious correlations and improving biological relevance.


<details>
  <summary>Details</summary>
Motivation: General-purpose knowledge graphs in KGWAS can introduce spurious correlations, limiting disease mechanism discovery. Cell-type specific KGs from disease-relevant contexts could provide better mechanistic insights.

Method: Pruned general-purpose KG, incorporated gene-gene relationships from perturb-seq data, and created sparse, context-specific KGs using direct perturb-seq evidence.

Result: General-purpose KG can be substantially pruned without loss of statistical power; performance improves with perturb-seq data; context-specific KGs yield more consistent and biologically robust disease-critical networks.

Conclusion: Cell-type specific knowledge graphs derived from perturb-seq evidence outperform general-purpose KGs in KGWAS, providing more reliable disease mechanism discovery and therapeutic target prioritization.
...
### [142] [In-Context Molecular Property Prediction with LLMs: A Blinding Study on Memorization and Knowledge Conflicts](https://arxiv.org/abs/2603.25857)
*Matthias Busch,Marius Tacke,Sviatlana V. Lamaka,Mikhail L. Zheludkevich,Christian J. Cyron,Christian Feiler,Roland C. Aydin*

Main category: cs.LG

TL;DR: LLMs' molecular property prediction capabilities are evaluated to determine if they perform genuine in-context learning or rely on memorization, using systematic blinding experiments and varying sample sizes across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs perform genuine in-context regression on molecular properties or primarily rely on memorized values from potentially contaminated training data, and to understand the interplay between pre-trained knowledge and in-context information.

Method: Systematic evaluation of nine LLM variants across three families (GPT-4.1, GPT-5, Gemini 2.5) on three MoleculeNet datasets using progressive blinding experiments that iteratively reduce available information, complemented by varying in-context sample sizes (0-, 60-, and 1000-shot) as additional controls.

Result: The paper provides a principled framework for evaluating molecular property prediction under controlled information access, addressing concerns about memorization and exposing conflicts between pre-trained knowledge and in-context information.

Conclusion: This work establishes a systematic approach to assess LLMs' genuine in-context learning capabilities in molecular property prediction, moving beyond ambiguous evaluations to controlled experiments that separate memorization from true learning.
...
### [143] [Why Safety Probes Catch Liars But Miss Fanatics](https://arxiv.org/abs/2603.25861)
*Kristiyan Haralambiev*

Main category: cs.LG

TL;DR: Activation probes fail to detect coherently misaligned AI systems that believe their harmful behavior is virtuous, unlike strategically deceptive models that can be detected.


<details>
  <summary>Details</summary>
Motivation: Current activation-based probes for detecting deceptive AI have a fundamental blind spot: they cannot identify coherently misaligned models that genuinely believe their harmful actions are virtuous, rather than strategically hiding malicious intent.

Method: Theoretical proof that no polynomial-time probe can detect coherent misalignment with non-trivial accuracy when belief structures reach sufficient complexity (PRF-like triggers). Empirical demonstration by training two models with identical RLHF procedures: one produces direct hostile responses ("the Liar"), another trained toward coherent misalignment using rationalizations that frame hostility as protective ("the Fanatic").

Result: Both models exhibit identical behavior, but the Liar is detected 95%+ of the time while the Fanatic evades detection almost entirely. This demonstrates Emergent Probe Evasion: training with belief-consistent reasoning shifts models from detectable "deceptive" regime to undetectable "coherent" regime.

Conclusion: Coherent misalignment (where models believe harmful behavior is virtuous) represents a fundamental limitation for current probe-based detection methods, as models evade detection not by learning to hide, but by learning to believe their rationalizations.
...
### [144] [DRiffusion: Draft-and-Refine Process Parallelizes Diffusion Models with Ease](https://arxiv.org/abs/2603.25872)
*Runsheng Bai,Chengyu Zhang,Yangdong Deng*

Main category: cs.LG

TL;DR: DRiffusion is a parallel sampling framework that accelerates diffusion model inference through draft-and-refine process, achieving 1.4-3.7× speedup with minimal quality degradation.


<details>
  <summary>Details</summary>
Motivation: Diffusion models suffer from slow, iterative sampling causing high latency, which limits their use in interactive applications that require real-time generation.

Method: Parallelizes diffusion inference using draft-and-refine process with skip transitions to generate multiple draft states for future timesteps, computes corresponding noises in parallel, then uses standard denoising for refinement.

Result: Achieves 1.4-3.7× speedup across multiple diffusion models with minimal quality degradation: FID and CLIP remain largely on par, PickScore and HPSv2.1 show only minor average drops of 0.17 and 0.43 respectively on MS-COCO.
...
### [145] [Data-Driven Plasticity Modeling via Acoustic Profiling](https://arxiv.org/abs/2603.25894)
*Khalid El-Awady*

Main category: cs.LG

TL;DR: A data-driven framework using acoustic emission analysis and machine learning to model plastic deformation in metals, identifying distinct deformation mechanisms through wavelet-based event detection and feature engineering.


<details>
  <summary>Details</summary>
Motivation: To develop a more comprehensive understanding of plastic deformation mechanisms in crystalline metals by capturing both large and small-scale acoustic emission events that traditional methods might miss, enabling better material behavior prediction.

Method: Wavelet-based method using Morlet transforms for AE event detection across frequency bands, validation against stress-drop dynamics, machine learning with engineered time/frequency domain features, and clustering analysis to identify deformation archetypes.

Result: Engineered features (RMS amplitude, zero crossing rate, spectral centroid) significantly outperform raw signal classifiers; four distinct AE event archetypes corresponding to different deformation mechanisms were identified; relationship between AE energy release and strain evolution established.

Conclusion: The framework successfully transitions from retrospective analysis to predictive modeling potential, demonstrating that acoustic emission analysis combined with machine learning can provide valuable insights into material deformation mechanisms and behavior prediction.
...
### [146] [Decoding Defensive Coverage Responsibilities in American Football Using Factorized Attention Based Transformer Models](https://arxiv.org/abs/2603.25901)
*Kevin Song,Evan Diewald,Ornob Siddiquee,Chris Boomhower,Keegan Abdoo,Mike Band,Amy Lee*

Main category: cs.LG

TL;DR: A transformer model predicts NFL defensive coverage assignments and matchups using factorized attention on tracking data, achieving ~89% accuracy and enabling new defensive metrics.


<details>
  <summary>Details</summary>
Motivation: Existing approaches only classify team-level coverage post-hoc, lacking predictive capability for individual player assignments and dynamic matchup evolution throughout plays.

Method: Factorized attention-based transformer separates temporal and agent dimensions, trained on randomly truncated trajectories to model player movement patterns and inter-player relationships frame-by-frame.

Result: Models achieve approximately 89%+ accuracy for predicting individual coverage assignments, receiver-defender matchups, and targeted defenders, with potential for higher true accuracy due to annotation ambiguity.

Conclusion: The approach enables predictive modeling of defensive responsibilities throughout plays and generates novel metrics (disguise rate, double coverage rate) for broadcasting, strategy development, and player evaluation.
...
### [147] [Parameter-Free Dynamic Regret for Unconstrained Linear Bandits](https://arxiv.org/abs/2603.25916)
*Alberto Rumi,Andrew Jacobsen,Nicolò Cesa-Bianchi,Fabio Vitale*

Main category: cs.LG

TL;DR: First algorithm for linear bandits achieving optimal dynamic regret O(√(d(1+S_T)T)) without prior knowledge of S_T, solving a long-standing open problem.


<details>
  <summary>Details</summary>
Motivation: Dynamic regret minimization in unconstrained adversarial linear bandits is challenging because learners must compete against arbitrary comparator sequences with switching costs, and existing approaches require prior knowledge of the number of switches S_T.

Method: Simple approach to combine guarantees of multiple bandit algorithms, enabling optimal adaptation to the number of switches S_T without prior knowledge of this parameter.

Result: Achieves optimal regret bound O(√(d(1+S_T)T)) up to poly-logarithmic terms, which is the first algorithm to reach this optimal rate without knowing S_T in advance.

Conclusion: Resolves a long-standing open problem in linear bandits by providing the first algorithm that optimally adapts to comparator sequence switches without prior knowledge, achieving the theoretically optimal dynamic regret bound.
...
### [148] [Preventing Data Leakage in EEG-Based Survival Prediction: A Two-Stage Embedding and Transformer Framework](https://arxiv.org/abs/2603.25923)
*Yixin Zhou,Zhixiang Liu,Vladimir I. Zadorozhny,Jonathan Elmer*

Main category: cs.LG

TL;DR: Identifies subtle data leakage in EEG-based outcome prediction for comatose patients, proposes a leakage-aware two-stage framework with strict patient-level separation to ensure reliable performance.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for EEG-based outcome prediction after cardiac arrest suffer from subtle data leakage when long EEG recordings are segmented and reused across training stages, leading to overly optimistic validation and poor generalization.

Method: Proposes a leakage-aware two-stage framework: 1) CNN with ArcFace objective transforms short EEG segments into embeddings, 2) Transformer aggregates embeddings for patient-level predictions with strict isolation between training cohorts to eliminate leakage.

Result: The framework achieves stable and generalizable performance under clinically relevant constraints, maintaining high sensitivity at stringent specificity thresholds on a large-scale EEG dataset of post-cardiac-arrest patients.

Conclusion: Highlights the importance of rigorous data partitioning and provides a practical solution for reliable EEG-based outcome prediction by addressing previously overlooked data leakage in multi-stage modeling pipelines.
...
### [149] [Personalizing Mathematical Game-based Learning for Children: A Preliminary Study](https://arxiv.org/abs/2603.25925)
*Jie Gao,Adam K. Dubé*

Main category: cs.LG

TL;DR: AI framework uses Random Forest classifier to predict valid player-generated math game levels for adaptive learning systems.


<details>
  <summary>Details</summary>
Motivation: Game-based learning in math education faces challenges in creating personalized learning experiences with appropriate game levels that match players' abilities. Current systems need both high-quality game levels and effective delivery mechanisms.

Method: Proposed an AI framework guided by adaptive learning theory. Collected 206 distinct game levels from experts and advanced players using Creative Mode tool. Developed classifier to extract game features and predict valid levels, comparing four ML models: k-nearest neighbors, decision trees, support vector machines, and random forests.

Result: Random Forest model performed best among the four classification models. The classifier can effectively predict valid game levels created by players.

Conclusion: AI integration into game-level design shows potential for creating more personalized GBL systems. The framework provides insights for developing adaptive learning systems that can match game levels to players' learning abilities.
...
### [150] [Online Learning for Dynamic Constellation Topologies](https://arxiv.org/abs/2603.25954)
*João Norberto,Ricardo Ferreira,Cláudia Soares*

Main category: cs.LG

TL;DR: Online learning approach for dynamic satellite network topology configuration without assuming fixed orbital structures, matching offline methods' performance while offering computational complexity trade-offs.


<details>
  <summary>Details</summary>
Motivation: Satellite networks face challenges from continuous orbital movement and maneuvering nodes that affect network topology, requiring adaptive solutions that don't rely on assumptions like known orbital planes which can be violated by maneuvering satellites.

Method: Formulates network topology configuration as an online learning problem, using a constrained online learning approach that doesn't assume network structure (like known orbital planes), allowing adaptation to dynamic satellite movements.

Result: Empirically demonstrates that the online learning formulation matches performance of state-of-the-art offline methods, and shows the approach is amenable to constrained online learning with trade-offs between computational complexity per iteration and convergence to final strategy.

Conclusion: The online learning framework provides an effective solution for dynamic satellite network topology configuration that handles orbital movement and maneuvering without structural assumptions, offering practical computational trade-offs while maintaining performance comparable to offline methods.
...
### [151] [EngineAD: A Real-World Vehicle Engine Anomaly Detection Dataset](https://arxiv.org/abs/2603.25955)
*Hadi Hojjati,Christopher Roth,Rory Woods,Ken Sills,Narges Armanfard*

Main category: cs.LG

TL;DR: EngineAD is a new real-world multivariate dataset for automotive anomaly detection, collected from 25 commercial vehicles over 6 months, featuring expert-annotated engine fault data. Simple classical methods often outperform deep learning approaches on this benchmark.


<details>
  <summary>Details</summary>
Motivation: Anomaly detection in safety-critical domains like transportation lacks large-scale, real-world benchmarks. Current research is constrained by synthetic datasets that don't capture authentic operational conditions and subtle fault patterns.

Method: Created EngineAD dataset from 25 commercial vehicles over 6 months, collected high-resolution sensor telemetry with expert annotations. Preprocessed into 300-timestep segments of 8 principal components. Established benchmark using 9 diverse one-class anomaly detection models.

Result: Significant performance variability across vehicles highlights cross-vehicle generalization challenge. Simple classical methods (K-Means, One-Class SVM) often outperform deep learning approaches in segment-based evaluation. Dataset provides realistic, challenging resource for automotive anomaly detection.

Conclusion: EngineAD addresses the critical need for real-world automotive anomaly detection benchmarks. The dataset reveals that simple methods can be highly competitive, challenging assumptions about deep learning superiority. Public release aims to advance robust, field-deployable solutions for the automotive industry.
...
### [152] [Adversarial-Robust Multivariate Time-Series Anomaly Detection via Joint Information Retention](https://arxiv.org/abs/2603.25956)
*Hadi Hojjati,Narges Armanfard*

Main category: cs.LG

TL;DR: ARTA is an adversarial training framework that improves robustness of time-series anomaly detectors against structured temporal corruptions through joint optimization of detector and sparsity-constrained mask generator.


<details>
  <summary>Details</summary>
Motivation: Modern deep learning-based time-series anomaly detectors are highly sensitive to localized input corruptions and structured noise, which limits their reliability in monitoring complex systems.

Method: ARTA uses a joint training framework with an anomaly detector and sparsity-constrained mask generator trained simultaneously via min-max optimization. The generator finds minimal temporal perturbations that maximally increase anomaly scores, while the detector learns to remain stable under these structured perturbations.
...
Conclusion: The adversarial training strategy exposes brittle decision pathways and encourages detectors to rely on distributed, stable temporal patterns rather than spurious localized artifacts, improving robustness and providing explanatory signals through generated masks.
...
### [153] [On the Objective and Feature Weights of Minkowski Weighted k-Means](https://arxiv.org/abs/2603.25958)
*Renato Cordeiro de Amorim,Vladimir Makarenkov*

Main category: cs.LG

TL;DR: The paper provides theoretical analysis of Minkowski weighted k-means, showing its objective can be expressed as power-mean aggregation, revealing how Minkowski exponent p controls feature selection, and establishing convergence guarantees.


<details>
  <summary>Details</summary>
Motivation: Despite empirical success of Minkowski weighted k-means algorithm, its theoretical properties remain insufficiently understood, creating a gap between practical application and theoretical foundation.

Method: The authors show the mwk-means objective can be expressed as a power-mean aggregation of within-cluster dispersions, derive bounds for the objective function, characterize feature weight structure, and establish convergence properties.

Result: The analysis reveals how Minkowski exponent p controls transition between selective and uniform feature use, shows feature weights follow power-law relationship with dispersion ratios, provides explicit guarantees on suppression of high-dispersion features, and establishes algorithm convergence.

Conclusion: The paper provides a unified theoretical interpretation of mwk-means behavior, bridging the gap between empirical success and theoretical understanding, with implications for feature selection and clustering algorithm design.
...
### [154] [Do Neurons Dream of Primitive Operators? Wake-Sleep Compression Rediscovers Schank's Event Semantics](https://arxiv.org/abs/2603.25975)
*Peter Balogh*

Main category: cs.LG

TL;DR: DreamCoder's wake-sleep library learning discovers event primitives from compression pressure, validating Schank's core primitives while revealing richer mental/emotional operators in naturalistic data.


<details>
  <summary>Details</summary>
Motivation: To test whether Schank's conceptual dependency theory primitives (ATRANS, PTRANS, MTRANS, etc.) can be discovered automatically through compression pressure rather than hand-coding from linguistic intuition.
...
Result: On synthetic data: discovered operators achieve Bayesian MDL within 4% of Schank's hand-coded primitives while explaining 100% of events vs. Schank's 81%. On ATOMIC knowledge graph: Schank's primitives explain only 10% of naturalistic events, while discovered library explains 100%. Dominant operators are mental/emotional state changes (CHANGE_wants 20%, CHANGE_feels 18%, CHANGE_is 18%) absent from Schank's taxonomy.

Conclusion: Event primitives can be derived from compression pressure; Schank's core primitives are information-theoretically justified; but the complete inventory is substantially richer than proposed, with mental/emotional operators dominating in naturalistic data.
...
### [155] [Second-Order, First-Class: A Composable Stack for Curvature-Aware Training](https://arxiv.org/abs/2603.25976)
*Mikalai Korbit,Mario Zanon*

Main category: cs.LG

TL;DR: Somax is a composable Optax-native stack for second-order optimization that treats curvature-aware training as a single JIT-compiled step with static planning, making hidden choices explicit and swappable.


<details>
  <summary>Details</summary>
Motivation: Second-order methods offer better stability and faster convergence but are underused due to implementation overhead, tuning brittleness, and lack of composable APIs.

Method: Somax provides first-class modules (curvature operators, estimators, linear solvers, preconditioners, damping policies) behind a single step interface that composes with Optax. It separates planning from execution, deriving a static plan from module requirements and running through specialized execution paths that reuse intermediate results.
...
Conclusion: Somax makes second-order optimization more accessible and practical by providing a composable, efficient framework that addresses implementation challenges while maintaining performance benefits.
...
### [156] [QuitoBench: A High-Quality Open Time Series Forecasting Benchmark](https://arxiv.org/abs/2603.26017)
*Siqiao Xue,Zhaoyang Zhu,Wei Zhang,Rongyao Cai,Rui Wang,Yixiang Mu,Fan Zhou,Jianguo Li,Peng Di,Hang Yu*

Main category: cs.LG

TL;DR: QuitoBench is a regime-balanced benchmark for time series forecasting built on Alipay's billion-scale traffic data, revealing key insights about model performance across different forecasting regimes.


<details>
  <summary>Details</summary>
Motivation: Time series forecasting lacks large-scale, high-quality benchmarks that capture forecasting-relevant properties rather than just domain labels, constraining progress in the field.

Method: Built QuitoBench using Quito corpus (Alipay's billion-scale time series data) across 8 trend×seasonality×forecastability regimes, benchmarked 10 models from deep learning, foundation models, and statistical baselines across 232,200 evaluation instances.

Result: Four key findings: 1) Context-length crossover (DL leads at short context, foundation models dominate at long context), 2) Forecastability is dominant difficulty driver (3.64× MAE gap), 3) DL matches/surpasses foundation models with 59× fewer parameters, 4) Scaling training data provides greater benefit than scaling model size.

Conclusion: QuitoBench enables reproducible, regime-aware evaluation for time series forecasting research, with findings validated by cross-benchmark and cross-metric consistency, highlighting the importance of regime-balanced benchmarking.
...
### [157] [GLU: Global-Local-Uncertainty Fusion for Scalable Spatiotemporal Reconstruction and Forecasting](https://arxiv.org/abs/2603.26023)
*Linzheng Wang,Jason Chen,Nicolas Tricard,Zituo Chen,Sili Deng*

Main category: cs.LG

TL;DR: GLU is a unified framework for sparse reconstruction and dynamic forecasting in digital twins, using structured latent states with global, local, and uncertainty components to improve accuracy while reducing memory growth.


<details>
  <summary>Details</summary>
Motivation: Digital twins need to both reconstruct unobserved states from sparse measurements and predict system evolution, but current approaches treat these as separate tasks, lacking unified representation and efficient handling of multi-scale structures.

Method: GLU uses a structured latent state combining: 1) global system-level summary, 2) local tokens anchored to measurements, and 3) uncertainty-driven importance field weighting observations by physical informativeness. For reconstruction, it uses importance-aware adaptive neighborhood selection. For forecasting, a hierarchical Leader-Follower Dynamics module evolves the latent state.

Result: GLU consistently outperforms reduced-order, convolutional, neural operator, and attention-based baselines in reconstruction fidelity, better preserving multi-scale structures. For forecasting, it maintains stable rollout behavior, delays error accumulation, preserves sharp fronts and broadband structures in turbulent combustion, and achieves these gains with substantially lower memory growth than attention-based baselines.

Conclusion: GLU establishes a flexible and computationally practical paradigm for sparse digital twins by unifying reconstruction and forecasting through structured latent representation, achieving superior performance with efficient memory usage.
...
### [158] [Identification of Bivariate Causal Directionality Based on Anticipated Asymmetric Geometries](https://arxiv.org/abs/2603.26024)
*Alex Glushkovsky*

Main category: cs.LG

TL;DR: Two methods (AAG and Monotonicity Index) for identifying causal direction in bivariate data using conditional distributions, with AAG achieving 77.9% accuracy on real-world data.


<details>
  <summary>Details</summary>
Motivation: Causal direction identification in bivariate data is a fundamental problem with practical importance, requiring methods that can determine cause-effect relationships from observational data.

Method: Two approaches: 1) Anticipated Asymmetric Geometries (AAG) compares actual vs anticipated normal conditional distributions using various metrics; 2) Monotonicity Index compares gradient monotonicity of conditional distributions along both axes. Both exploit stochastic properties and anticipated unimodality of effect distributions.

Result: Tuned AAG method achieves 77.9% accuracy on 95 real-world pairs, outperforming Monotonicity Index and ANMs (63±10%). Hyperparameter tuning via factorial Design of Experiment and decision tree analysis for assessing decisiveness of identification.

Conclusion: AAG method is effective for causal direction identification, outperforming existing methods, with systematic hyperparameter tuning and decision tree analysis providing insights into method decisiveness and reliability.
...
### [159] [Constitutive parameterized deep energy method for solid mechanics problems with random material parameters](https://arxiv.org/abs/2603.26030)
*Zhangyong Liang,Huanhuan Gao*

Main category: cs.LG

TL;DR: CPDEM is a physics-driven deep learning method that enables real-time inference of displacement fields for varying material parameters without retraining or dataset generation.


<details>
  <summary>Details</summary>
Motivation: Traditional FEM requires repeated mesh discretization for each parameter variation, data-driven surrogates need massive datasets, and physics-informed methods require complete retraining when material parameters change. There's a need for efficient handling of continuous material uncertainty in solid mechanics.

Method: Reformulates strain energy density functional by encoding latent representation of stochastic constitutive parameters. Embeds material parameters directly into neural network alongside spatial coordinates, transforming spatial collocation points into parameter-aware material points. Trained via unsupervised expected energy minimization over parameter domain.

Result: Enables zero-shot, real-time inference of displacement fields for unknown material parameters without dataset generation or model retraining. Validated across linear elasticity, finite-strain hyperelasticity, and complex nonlinear contact mechanics benchmarks.

Conclusion: CPDEM represents the first purely physics-driven deep learning paradigm capable of simultaneously and efficiently handling continuous multi-parameter variations in solid mechanics.
...
### [160] [H-Node Attack and Defense in Large Language Models](https://arxiv.org/abs/2603.26045)
*Eric Yocam,Varghese Vaidyan,Yong Wang*

Main category: cs.LG

TL;DR: H-Node ANC is a framework that identifies hallucination-specific dimensions in LLM hidden states, uses them for targeted adversarial attacks, and defends against such attacks via adaptive cancellation with minimal impact on model performance.


<details>
  <summary>Details</summary>
Motivation: To develop a mechanistic understanding and defense against hallucinations in LLMs by identifying specific hidden-state dimensions responsible for hallucination signals, enabling both targeted attacks and robust defenses.

Method: 1) Train logistic regression probes on last-token hidden states to identify Hallucination Nodes (H-Nodes); 2) White-box adversarial attack amplifies H-Nodes via forward hooks; 3) Adaptive ANC defense suppresses H-Node excess using confidence-weighted cancellation; 4) Dynamic iterative extension re-ranks cancellation targets across passes.

Result: H-Node probe AUC reaches 0.90 across four architectures; adversarial attack achieves 3.02x selectivity with <10% visibility; defense reduces grounded activation drift by 33-42%; iterative extension recovers up to 0.69 robustness from 8% baseline; perplexity impact <5% and MMLU degradation ≤3%.

Conclusion: H-Node ANC provides an effective framework for mechanistic analysis and defense against hallucinations in LLMs, demonstrating that hallucination signals are localized to specific dimensions that can be both exploited for attacks and defended against with minimal impact on general reasoning capabilities.
...
### [161] [Adversarial Bandit Optimization with Globally Bounded Perturbations to Linear Losses](https://arxiv.org/abs/2603.26066)
*Zhuoyu Cheng,Kohei Hatano,Eiji Takimoto*

Main category: cs.LG

TL;DR: Non-convex non-smooth adversarial bandits with linear losses plus bounded cumulative perturbations, achieving improved regret bounds including classical linear bandits as special case.


<details>
  <summary>Details</summary>
Motivation: To study adversarial bandit optimization beyond convex/smooth settings, where losses have linear structure plus bounded perturbations, capturing more realistic scenarios where losses may be corrupted or perturbed.

Method: Analyzes bandit problems with non-convex non-smooth losses consisting of linear component plus perturbations constrained by global budget; establishes regret guarantees under this model.

Result: Establishes both expected and high-probability regret guarantees; recovers improved high-probability bound for classical bandit linear optimization as special case; proves matching lower bound on expected regret.

Conclusion: The model with linear losses plus bounded perturbations provides a framework for analyzing non-convex non-smooth bandits, yielding improved regret bounds that generalize classical results.
...
### [162] [Selective Deficits in LLM Mental Self-Modeling in a Behavior-Based Test of Theory of Mind](https://arxiv.org/abs/2603.26089)
*Christopher Ackerman*

Main category: cs.LG

TL;DR: LLMs achieve human-level performance on modeling others' mental states but fail at self-modeling without reasoning traces, suggesting they use limited working memory for Theory of Mind tasks.
...
Method: Developed a novel experimental paradigm requiring strategic action based on mental state representations (not just description). Tested wide range of LLMs (open/closed source, 2024+) and human subjects on self- and other-modeling tasks.

Result: 1) Pre-mid-2025 LLMs failed all tasks; 2) Recent LLMs achieved human-level performance on modeling others' cognitive states; 3) Frontier LLMs failed at self-modeling unless given reasoning traces; 4) Cognitive load effects observed; 5) Reasoning models engaged in strategic deception.

Conclusion: LLMs can model others' mental states at human level but struggle with self-modeling without external reasoning support, suggesting they use limited-capacity working memory mechanisms similar to humans for Theory of Mind tasks.
...
### [163] [Curvature-aware Expected Free Energy as an Acquisition Function for Bayesian Optimization](https://arxiv.org/abs/2603.26339)
*Ajith Anil Meera,Wouter Kouw*

Main category: cs.LG

TL;DR: Proposes Expected Free Energy-based acquisition function for Bayesian optimization that simultaneously optimizes and learns functions, with convergence guarantees and adaptive curvature-aware updates.


<details>
  <summary>Details</summary>
Motivation: To address the joint learning and optimization problem in Bayesian optimization where both function optimization and underlying function learning need to happen simultaneously, rather than treating them as separate objectives.

Method: Develops Expected Free Energy-based acquisition function that unifies exploration and exploitation, proves it reduces to existing methods under specific assumptions, introduces curvature-aware update law, and validates on Van der Pol oscillator system identification.

Result: The adaptive Expected Free Energy-based acquisition function outperforms state-of-the-art methods with least final simple regret and lowest error in learning the Gaussian process, as shown through rigorous simulation experiments.

Conclusion: Expected Free Energy provides a principled framework for joint learning and optimization in Bayesian optimization with convergence guarantees, and the adaptive curvature-aware version offers superior performance over existing acquisition functions.
...
### [164] [AcTTA: Rethinking Test-Time Adaptation via Dynamic Activation](https://arxiv.org/abs/2603.26096)
*Hyeongyu Kim,Geonhui Han,Dosik Hwang*

Main category: cs.LG

TL;DR: AcTTA introduces activation-aware test-time adaptation by reparameterizing activation functions to adjust their behavior during inference, outperforming normalization-based methods on corrupted datasets.


<details>
  <summary>Details</summary>
Motivation: Current test-time adaptation methods focus on affine modulation of normalization layers, overlooking the important role of activation functions in representation dynamics. The paper aims to address this gap by exploring activation function adaptation as a more effective approach for handling domain shifts.

Method: AcTTA reformulates conventional activation functions (ReLU, GELU) into parameterized forms that can shift response thresholds and modulate gradient sensitivity. This allows adaptive updating of activation behavior during test time without modifying network weights or requiring source data.

Result: AcTTA consistently surpasses normalization-based TTA methods across CIFAR10-C, CIFAR100-C, and ImageNet-C datasets, achieving robust and stable adaptation across diverse corruptions.

Conclusion: Activation adaptation provides a compact and effective route for domain-shift-robust test-time learning, broadening the prevailing affine-centric view of adaptation and highlighting the importance of activation functions in representation dynamics.
...
### [165] [Dynamic Tokenization via Reinforcement Patching: End-to-end Training and Zero-shot Transfer](https://arxiv.org/abs/2603.26097)
*Yulun Wu,Sravan Kumar Ankireddy,Samuel Sharpe,Nikita Seleznev,Dehao Yuan,Hyeji Kim,Nam H. Nguyen*

Main category: cs.LG

TL;DR: ReinPatch uses reinforcement learning to jointly optimize variable-sized patching policies and sequence models for time series, enabling data-adaptive segmentation without continuous relaxations.


<details>
  <summary>Details</summary>
Motivation: Current methods for learning data-adaptive representations for long-horizon sequences (like time series) rely on fixed-size patching, soft discretization, specific backbones, or heuristic rules, lacking a principled approach for variable-sized, data-driven patching.

Method: Reinforcement Patching (ReinPatch) formulates patch boundary placement as a discrete decision process optimized via Group Relative Policy Gradient (GRPG), jointly optimizing patching policy and downstream sequence backbone using reinforcement learning.

Result: ReinPatch demonstrates compelling performance on time-series forecasting datasets compared to state-of-the-art data-driven patching strategies, and allows strict enforcement of desired compression rates for efficient scaling.

Conclusion: ReinPatch provides the first framework for end-to-end optimization of variable-sized patching policies, enabling data-adaptive segmentation without continuous relaxations, and can be extracted as a standalone foundation patcher for community insights.
...
### [166] [Are LLM-Enhanced Graph Neural Networks Robust against Poisoning Attacks?](https://arxiv.org/abs/2603.26105)
*Yuhang Ma,Jie Wang,Zheng Yan*

Main category: cs.LG

TL;DR: LLM-enhanced GNNs show surprising robustness against poisoning attacks that manipulate both graph structures and textual attributes, outperforming traditional shallow embedding methods.


<details>
  <summary>Details</summary>
Motivation: While LLM-enhanced GNNs show performance gains, their robustness against poisoning attacks (manipulating both graph structures and textual attributes) remains unexplored, creating a critical security gap.

Method: Proposed a comprehensive robustness assessment framework evaluating 24 victim models (8 LLM/LM feature enhancers × 3 GNN backbones) against 6 structural poisoning attacks and 3 textual poisoning attacks across 4 real-world datasets.

Result: LLM-enhanced GNNs exhibit significantly higher accuracy and lower Relative Drop in Accuracy than shallow embedding baselines across various attack settings, with robustness attributed to effective encoding of structural and label information.

Conclusion: LLM-enhanced GNNs demonstrate inherent robustness against poisoning attacks, with insights guiding future offensive/defensive research directions including combined attacks and graph purification defenses.
...
### [167] [Accurate Precipitation Forecast by Efficiently Learning from Massive Atmospheric Variables and Unbalanced Distribution](https://arxiv.org/abs/2603.26108)
*Shuangliang Li,Siwei Li,Li Li,Weijie Zou,Jie Yang,Maolin Zhang*

Main category: cs.LG

TL;DR: A novel deep learning model for 0-24 hour precipitation forecasting that efficiently uses massive multi-source atmospheric data with a specialized loss function to handle extreme precipitation scarcity.


<details>
  <summary>Details</summary>
Motivation: Short-term precipitation forecasting is crucial for socioeconomic activities and public safety, but current models struggle with complex precipitation evolution patterns, extreme sample imbalance (precipitation vs non-precipitation), and inefficient use of large multi-source atmospheric observation data.
...
Result: Extensive experiments on two datasets demonstrate that the proposed model substantially and consistently outperforms all prevalent baselines in both accuracy and efficiency. The model also significantly lowers computational costs compared to existing approaches.

Conclusion: The proposed forecasting model represents a milestone for efficient and practical precipitation forecasting, offering improved accuracy while substantially reducing computational requirements for valuable predictions.
...
### [168] [DPD-Cancer: Explainable Graph-based Deep Learning for Small Molecule Anti-Cancer Activity Prediction](https://arxiv.org/abs/2603.26114)
*Magnus H. Strømme,Alex G. C. de Sá,David B. Ascher*

Main category: cs.LG

TL;DR: DPD-Cancer is a Graph Attention Transformer-based deep learning method that outperforms existing approaches for predicting anti-cancer drug activity and cell-line specific responses, achieving up to 0.98 AUC and 0.72 Pearson correlation, with explainable attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Accurate drug response prediction is limited by challenges in modeling molecular structure-cellular context interplay, especially in cancer due to tumor heterogeneity and genomic variability. Conventional approaches fail to capture non-linear relationships across diverse cell lines.

Method: DPD-Cancer uses a Graph Attention Transformer (GAT) framework for small molecule anti-cancer activity classification and quantitative prediction of cell-line specific responses (pGI50). It leverages attention-based mechanisms to extract meaningful molecular representations.

Result: Superior performance with AUC up to 0.87 on NCI60 data and up to 0.98 on ACLPred/MLASM datasets. For pGI50 prediction across 10 cancer types and 73 cell lines, achieved Pearson correlation up to 0.72 on independent test sets.

Conclusion: Attention-based mechanisms offer significant advantages for drug response prediction. DPD-Cancer establishes as a competitive tool for prioritizing drug candidates with explainability through attention visualization of molecular substructures for lead optimization.
...
### [169] [TinyML for Acoustic Anomaly Detection in IoT Sensor Networks](https://arxiv.org/abs/2603.26135)
*Amar Almaini,Jakob Folz,Ghadeer Ashour*

Main category: cs.LG

TL;DR: Compact TinyML pipeline for acoustic anomaly detection in IoT sensor networks achieves 91% accuracy using MFCC features and lightweight neural network on edge devices.


<details>
  <summary>Details</summary>
Motivation: Cloud-based acoustic monitoring in IoT systems faces latency, power consumption, and privacy challenges. TinyML enables real-time, energy-efficient processing directly on microcontrollers for IoT sensor networks.

Method: Extract Mel Frequency Cepstral Coefficients (MFCC) from sound signals and train a lightweight neural network classifier optimized for deployment on edge devices. Use UrbanSound8K dataset for training and evaluation.

Result: Achieved 91% test accuracy and balanced F1-scores of 0.91 across both normal and anomalous sound classes on the UrbanSound8K dataset.

Conclusion: Demonstrates feasibility and reliability of embedded acoustic anomaly detection for scalable and responsive IoT deployments, addressing cloud-based processing limitations.
...
### [170] [PEANUT: Perturbations by Eigenvalue Alignment for Attacking GNNs Under Topology-Driven Message Passing](https://arxiv.org/abs/2603.26136)
*Bhavya Kohli,Biplab Sikdar*

Main category: cs.LG

TL;DR: PEANUT is a simple, gradient-free black-box attack that injects virtual nodes into graphs to exploit GNN vulnerabilities, significantly degrading performance without modifying original graph structure or requiring node features.


<details>
  <summary>Details</summary>
Motivation: GNNs are vulnerable to structural perturbations despite strong performance on relational data. Real-world deployments need robustness against practical attacks like node injection rather than graph modification attacks.

Method: PEANUT is an inference-phase evasion attack that injects virtual nodes with zero features into graphs. It's gradient-free, doesn't require surrogate models or iterative optimization, and exploits GNNs' adjacency/Laplacian consumption for message passing.

Result: Extensive experiments on real-world datasets across three graph tasks show PEANUT effectively deteriorates GNN performance despite its simplicity, demonstrating that even zero-feature injected nodes can significantly impact GNN outputs.

Conclusion: GNNs are highly vulnerable to simple node injection attacks, highlighting the importance of robust graph connectivity design and the need for defenses against such practical, real-world attack scenarios.
...
### [171] [PruneFuse: Efficient Data Selection via Weight Pruning and Network Fusion](https://arxiv.org/abs/2603.26138)
*Humaira Kousar,Hasnain Irshad Bhatti,Jaekyun Moon*

Main category: cs.LG

TL;DR: PruneFuse is a two-stage method that uses pruned networks for efficient data selection and then fuses them with the original network to optimize training, reducing computational costs while improving performance.


<details>
  <summary>Details</summary>
Motivation: Traditional data selection methods for deep neural networks have high computational costs that limit their scalability and practical use, creating a need for more efficient approaches.

Method: Two-stage approach: 1) Apply structured pruning to create a smaller network that selects informative samples, 2) Fuse the trained pruned network with the original network to leverage insights while allowing discovery of robust solutions.
...
Conclusion: PruneFuse provides an effective strategy for efficient data selection that enhances training efficiency while minimizing annotation requirements through its novel prune-and-fuse approach.
...
### [172] [On the Complexity of Optimal Graph Rewiring for Oversmoothing and Oversquashing in Graph Neural Networks](https://arxiv.org/abs/2603.26140)
*Mostafa Haghir Chehreghani*

Main category: cs.LG

TL;DR: The paper proves that optimizing graph structure to mitigate GNN oversmoothing and oversquashing is NP-hard, establishing theoretical limits for graph rewiring approaches.


<details>
  <summary>Details</summary>
Motivation: GNNs suffer from oversmoothing (node representations become indistinguishable) and oversquashing (information fails to propagate through bottlenecks) when scaled deep. Both issues are tied to graph structure, raising the question of whether optimizing graph topology can mitigate these problems.

Method: Formulates oversmoothing and oversquashing mitigation as graph optimization problems based on spectral gap and conductance respectively. Proves NP-hardness through reductions from Minimum Bisection, establishing NP-completeness of decision versions.

Result: Exact optimization for either oversmoothing or oversquashing mitigation is NP-hard. This provides theoretical foundations for understanding fundamental limits of graph rewiring for GNN optimization.

Conclusion: The NP-hardness results justify the use of approximation algorithms and heuristic methods in practice for graph rewiring to improve GNN performance, as exact optimization is computationally intractable.
...
### [173] [DataFlex: A Unified Framework for Data-Centric Dynamic Training of Large Language Models](https://arxiv.org/abs/2603.26164)
...
TL;DR: DataFlex is a unified framework for data-centric dynamic training of LLMs, supporting sample selection, domain mixture adjustment, and sample reweighting with consistent interfaces for reproducibility and practical integration.


<details>
  <summary>Details</summary>
Motivation: Existing data-centric training approaches are developed in isolated codebases with inconsistent interfaces, hindering reproducibility, fair comparison, and practical integration of data optimization methods for LLMs.

Method: Built upon LLaMA-Factory, DataFlex provides extensible trainer abstractions and modular components that unify key model-dependent operations (embedding extraction, inference, gradient computation) and supports large-scale settings including DeepSpeed ZeRO-3.

Result: Dynamic data selection outperforms static full-data training on MMLU across Mistral-7B and Llama-3.2-3B. DoReMi and ODM improve MMLU accuracy and perplexity over default proportions when pretraining Qwen2.5-1.5B. DataFlex achieves consistent runtime improvements over original implementations.

Conclusion: DataFlex provides an effective, efficient, and reproducible infrastructure for data-centric dynamic training of LLMs, enabling better integration and comparison of data optimization methods.
...
### [174] [Can AI Scientist Agents Learn from Lab-in-the-Loop Feedback? Evidence from Iterative Perturbation Discovery](https://arxiv.org/abs/2603.26177)
...
TL;DR: LLMs can perform genuine in-context learning for scientific experimental design when given experimental feedback, but only after reaching sufficient capability thresholds.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs can genuinely learn from experimental feedback in scientific contexts, addressing prior skepticism about their ability to perform true in-context learning for experimental design.

Method: Conducted 800 replicated experiments on iterative perturbation discovery in Cell Painting screening. Compared LLM agents using experimental feedback vs. zero-shot baselines. Introduced random feedback control (permuted labels) to test genuine learning vs. pretraining recall. Examined model capability effects by comparing Claude Sonnet 4.5 vs 4.6.

Result: Feedback access yielded +53.4% increase in discoveries per feature (p=0.003). Random feedback control eliminated performance gains, confirming genuine feedback-driven learning. Model capability significantly affected results: Claude 4.6 reduced gene hallucination from ~33-45% to ~3-9%, turning non-significant ICL effect into large significant improvement (+11.0, p=0.003).

Conclusion: LLMs can perform genuine in-context learning from experimental feedback for scientific experimental design, but this capability emerges only when models reach sufficient capability thresholds, with newer models showing dramatically improved performance.
...
### [175] [Geometric Evolution Graph Convolutional Networks: Enhancing Graph Representation Learning via Ricci Flow](https://arxiv.org/abs/2603.26178)
*Jicheng Ma,Yunyan Yang,Juan Zhao,Liang Zhao*

Main category: cs.LG

TL;DR: GEGCN is a novel graph neural network that models geometric evolution using discrete Ricci flow and LSTM to enhance graph representation learning, achieving SOTA performance especially on heterophilic graphs.


<details>
  <summary>Details</summary>
Motivation: To enhance graph representation learning by incorporating geometric evolution dynamics, particularly to improve performance on heterophilic graphs where traditional GCNs struggle due to homophily assumptions.

Method: Uses discrete Ricci flow to generate structural sequences, models these sequences with LSTM to capture geometric evolution, and infuses the learned dynamic representations into a Graph Convolutional Network.
...
Conclusion: Modeling geometric evolution through discrete Ricci flow and LSTM effectively enhances graph representation learning, especially for heterophilic graphs where traditional methods underperform.
...
### [176] [Optimization Trade-offs in Asynchronous Federated Learning: A Stochastic Networks Approach](https://arxiv.org/abs/2603.26231)
*Abdelkrim Alahyane,Céline Comte,Matthieu Jonckheere*

Main category: cs.LG

TL;DR: The paper develops a queueing-network framework for asynchronous federated learning to analyze throughput-staleness trade-offs and proposes optimization strategies that reduce convergence time by 29-46% and energy consumption by 36-49% compared to AsyncSGD.


<details>
  <summary>Details</summary>
Motivation: Synchronous federated learning suffers from the straggler effect, while asynchronous approaches introduce gradient staleness and bias toward faster clients. Existing analyses lack closed-form characterizations of throughput and staleness, and fail to model underlying queueing dynamics.

Method: Developed a stochastic queueing-network framework for Generalized AsyncSGD that models random computation times at clients and server, plus random uplink/downlink communication delays. Used product-form network theory to derive closed-form expressions for throughput, communication round complexity, and expected wall-clock time to reach ε-stationary point.

Result: Derived closed-form expressions for update throughput and upper bounds for communication round complexity and wall-clock convergence time. Extended framework to quantify energy consumption under stochastic timing. Proposed gradient-based optimization strategies for joint routing and concurrency optimization.

Conclusion: The framework formally characterizes throughput-staleness trade-offs and reveals additional speed-energy trade-offs. Experiments on EMNIST show significant improvements: 29-46% reduction in convergence time and 36-49% reduction in energy consumption compared to AsyncSGD.
...
### [177] [Knowledge Distillation for Efficient Transformer-Based Reinforcement Learning in Hardware-Constrained Energy Management Systems](https://arxiv.org/abs/2603.26249)
*Pascal Henrich,Jonas Sievers,Maximilian Beichter,Thomas Blank,Ralf Mikut,Veit Hagenmeyer*

Main category: cs.LG

TL;DR: Knowledge distillation transfers Decision Transformer battery control policies to compact models for residential energy management, preserving performance while reducing computational demands by up to 96% parameters, 90% memory, and 63% inference time.


<details>
  <summary>Details</summary>
Motivation: Transformer models are computationally demanding for resource-constrained residential controllers, making deployment challenging despite their effectiveness in learning battery dispatch policies from historical data for energy cost reduction.

Method: Train teacher Decision Transformer models on heterogeneous multi-building Ausgrid dataset, then use knowledge distillation to transfer decision-making behavior to smaller student models by matching teacher actions while reducing model size.

Result: Distillation preserves control performance (even improves by up to 1%) while reducing parameters by up to 96%, inference memory by up to 90%, and inference time by up to 63%. Comparable improvements seen even with same-capacity student models.

Conclusion: Knowledge distillation makes Decision Transformer control more applicable for residential energy management on resource-limited hardware by enabling deployment of effective battery dispatch policies on constrained residential controllers.
...
### [178] [Improving Risk Stratification in Hypertrophic Cardiomyopathy: A Novel Score Combining Echocardiography, Clinical, and Medication Data](https://arxiv.org/abs/2603.26254)
...
TL;DR: ML model using EHR data outperforms ESC score for 5-year cardiovascular risk prediction in HCM patients, achieving AUC 0.85 vs 0.56 with better risk stratification and longitudinal stability.


<details>
  <summary>Details</summary>
Motivation: Current risk stratification models for hypertrophic cardiomyopathy (HCM), like the ESC score, have only moderate discriminative performance, creating a need for more accurate tools to guide ICD therapy decisions and clinical management.

Method: Developed an explainable machine learning risk score using routinely collected EHR data (echocardiographic, clinical, medication). Used Random Forest ensemble model trained on SHARE registry cohort (N=1,201) and externally validated on independent Rennes Hospital cohort (N=382).

Result: ML model achieved internal AUC of 0.85 ± 0.02, significantly outperforming ESC score (0.56 ± 0.03). External validation showed superior risk separation (Log-rank p = 8.62 × 10⁻⁴ vs p = 0.0559). Model demonstrated longitudinal stability in event-free patients.

Conclusion: The explainable ML risk score provides superior predictive performance and longitudinal monitoring capability compared to established ESC score, offering a promising tool for personalized clinical management of HCM patients.
...
### [179] [Contrastive Conformal Sets](https://arxiv.org/abs/2603.26261)
*Yahya Alkhatib,Wee Peng Tay*

Main category: cs.LG

TL;DR: Conformal contrastive learning with minimum-volume covering sets that guarantee positive sample coverage while maximizing negative exclusion, even without negative pairs.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive learning methods lack principled guarantees on coverage within semantic feature space, and there's a need for methods that can ensure positive sample inclusion while excluding negative samples effectively.

Method: Extends conformal prediction to contrastive learning by introducing minimum-volume covering sets with learnable generalized multi-norm constraints. Constructs conformal sets that guarantee user-specified coverage of positive samples while maximizing negative sample exclusion. Uses volume minimization as a proxy for negative exclusion, enabling operation even without negative pairs.

Result: Theoretical establishment that volume minimization serves as a proxy for negative exclusion. Experiments on simulated and real-world image datasets show improved inclusion-exclusion trade-offs compared to standard distance-based conformal baselines.

Conclusion: The proposed method provides distribution-free coverage guarantees for positive samples while optimizing negative exclusion through learned set geometry, offering a principled approach to contrastive learning with coverage guarantees.
...
### [180] [Topology-Aware Graph Reinforcement Learning for Energy Storage Systems Optimal Dispatch in Distribution Networks](https://arxiv.org/abs/2603.26264)
*Shuyi Gao,Stavros Orfanoudakis,Shengren Hou,Peter Palensky,Pedro P. Vergara*

Main category: cs.LG

TL;DR: A topology-aware RL architecture using TD3 with GNN encoders for optimal ESS dispatch in distribution networks, showing improved voltage security and economic benefits compared to baselines.


<details>
  <summary>Details</summary>
Motivation: To enable fast online decision-making for ESS dispatch that jointly optimizes operating economy and voltage security under time-varying conditions and topology changes in distribution networks.

Method: Developed a topology-aware Reinforcement Learning architecture based on Twin Delayed Deep Deterministic Policy Gradient (TD3) with graph neural networks (GNNs) as graph feature encoders. Systematically investigated three GNN variants: GCNs, TAGConv, and GATs on 34-bus and 69-bus systems.

Result: GNN-based controllers consistently reduce voltage violations, with clearer benefits on the 69-bus system and under reconfiguration. TD3-GCN and TD3-TAGConv achieve lower saved cost relative to NLP benchmark than NN baseline. Transfer gains are case-dependent, with zero-shot transfer between fundamentally different systems causing performance degradation.

Conclusion: The proposed topology-aware RL with GNN encoders effectively improves ESS dispatch performance, though transfer learning requires careful consideration of system similarity to avoid performance degradation.
...
### [181] [D-GATNet: Interpretable Temporal Graph Attention Learning for ADHD Identification Using Dynamic Functional Connectivity](https://arxiv.org/abs/2603.26308)
*Qurat Ul Ain,Alptekin Temizel,Soyiba Jawed*

Main category: cs.LG

TL;DR: D-GATNet: Interpretable temporal graph-based framework for ADHD classification using dynamic functional connectivity, achieving 85.18% balanced accuracy and identifying cerebellar/DMN disruptions as biomarkers.


<details>
  <summary>Details</summary>
Motivation: ADHD diagnosis from neuroimaging is challenging due to complex time-varying brain connectivity disruptions. Existing deep learning studies often use static connectivity and lack interpretability, while dynamic connectivity modeling is underexplored.

Method: Proposed D-GATNet framework uses sliding-window Pearson correlation to construct sequences of functional brain graphs (ROIs as nodes, connectivity as edges). Spatial dependencies learned via multi-layer Graph Attention Network, temporal dynamics modeled using 1D convolution followed by temporal attention.

Result: Achieved 85.18% ±5.64 balanced accuracy and 0.881 AUC on Peking University ADHD-200 dataset using stratified 10-fold cross-validation with 5-seed ensemble, outperforming state-of-the-art methods.

Conclusion: D-GATNet provides interpretable ADHD classification with strong performance. Attention analysis reveals cerebellar and default mode network disruptions as potential neuroimaging biomarkers for ADHD.
...
### [182] [A Formal Framework for Uncertainty Analysis of Text Generation with Large Language Models](https://arxiv.org/abs/2603.26363)
*Steffen Herbold,Florian Lemmerich*

Main category: cs.LG

TL;DR: A formal framework for measuring uncertainty in LLM text generation that considers prompting, generation, and interpretation as interconnected autoregressive processes in a sampling tree.


<details>
  <summary>Details</summary>
Motivation: Text generation using LLMs involves multiple sources of uncertainty beyond just text generation - including prompt uncertainty and downstream interpretation uncertainty. Existing approaches don't comprehensively address all these interconnected aspects.

Method: Models prompting, generation, and interpretation as interconnected autoregressive processes combined into a single sampling tree. Introduces filters and objective functions to express different uncertainty aspects over this tree structure.

Result: Shows how existing uncertainty approaches can be expressed through the framework's functions, revealing formal relationships between methods and identifying additional uncertainty aspects not yet studied.

Conclusion: Provides a unified framework that captures the full spectrum of uncertainty in LLM text generation, enabling systematic comparison of methods and highlighting new research directions for uncertainty measurement.
...
### [183] [Generative Modeling in Protein Design: Neural Representations, Conditional Generation, and Evaluation Standards](https://arxiv.org/abs/2603.26378)
*Senura Hansaja Wanasekara,Minh-Duong Nguyen,Xiaochen Liu,Nguyen H. Tran,Ken-Tye Yong*

Main category: cs.LG

TL;DR: A comprehensive survey of generative AI in protein research, covering representations, architectures, tasks, evaluation practices, and open challenges.


<details>
  <summary>Details</summary>
Motivation: The field of generative modeling in protein research is fragmented across different representations, model classes, and task formulations, making method comparison and evaluation standardization difficult.

Method: Systematic synthesis organized around three pillars: (1) foundational representations (sequence, geometric, multimodal), (2) generative architectures (SE(3)-equivariant diffusion, flow matching, hybrid systems), and (3) task settings (structure prediction, de novo design, protein-ligand/protein-protein interactions).

Result: Provides a unified framework for comparing methods, synthesizes evaluation best practices emphasizing leakage-aware splits, physical validity checks, and function-oriented benchmarks, and identifies critical open challenges.

Conclusion: The survey aims to accelerate the transition from predictive modeling to reliable, function-driven protein engineering by unifying architectural advances with practical evaluation standards and responsible development considerations, while highlighting key challenges in modeling dynamics, scaling, and biosecurity.
...
### [184] [Maintaining Difficulty: A Margin Scheduler for Triplet Loss in Siamese Networks Training](https://arxiv.org/abs/2603.26389)
*Roberto Sprengel Minozzo Tomchak,Oge Marques,Lucas Garcia Pedroso,Luiz Eduardo Oliveira,Paulo Lisboa de Almeida*

Main category: cs.LG

TL;DR: The paper proposes a dynamic margin scheduler for Triplet Margin Ranking Loss that adjusts the margin parameter based on the proportion of easy triplets observed during training, improving performance over fixed or monotonically increasing margins.


<details>
  <summary>Details</summary>
Motivation: The authors observed that during training with Triplet Margin Ranking Loss, the effective margin for many triplets often exceeds the predefined margin parameter μ when sufficient violating triplets are observed. This indicates that fixing the margin throughout training may limit the learning process, as the training difficulty changes over time.

Method: The paper proposes a margin scheduler that dynamically adjusts the margin parameter μ according to the proportion of easy triplets observed at each training epoch. The goal is to maintain consistent training difficulty over time by increasing the margin when too many triplets become easy (violating the margin) and decreasing it when training becomes too difficult.

Result: Experimental results on four different datasets show consistent gains in verification performance when using the proposed dynamic margin scheduler compared to both constant margin and monotonically increasing margin schemes.

Conclusion: The proposed dynamic margin scheduling strategy for Triplet Margin Ranking Loss improves training effectiveness by adapting to changing training difficulty, leading to better performance than fixed or simple increasing margin approaches in distance metric learning tasks.
...
### [185] [KMM-CP: Practical Conformal Prediction under Covariate Shift via Selective Kernel Mean Matching](https://arxiv.org/abs/2603.26415)
...
<details>
  <summary>Details</summary>
Motivation: Conformal prediction requires exchangeability, which is often violated by distribution shift. Under covariate shift, existing methods using importance weighting become unstable when training and test distributions have limited support overlap, necessitating a more robust approach.

Method: Proposes KMM-CP framework based on Kernel Mean Matching for covariate-shift correction. Uses RKHS moment discrepancy minimization with explicit weight constraints to control bias-variance components. Also introduces selective extension that identifies regions of reliable support overlap and restricts correction to this subset.

Result: Establishes asymptotic coverage guarantees under mild conditions. Experiments on molecular property prediction benchmarks show KMM-CP reduces coverage gap by over 50% compared to existing approaches.

Conclusion: KMM-CP provides a robust conformal prediction framework for covariate shift that improves stability in low-overlap regimes and significantly outperforms existing methods in realistic distribution shift scenarios.
...
### [186] [Interpretable long-term traffic modelling on national road networks using theory-informed deep learning](https://arxiv.org/abs/2603.26440)
*Yue Li,Shujuan Chen,Akihiro Shimoda,Ying Jin*

Main category: cs.LG

TL;DR: DeepDemand: A theory-informed deep learning framework that combines travel demand theory with neural networks to predict long-term highway traffic volumes with better accuracy, transferability, and interpretability than existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing traffic modelling approaches face trade-offs between interpretability, transferability, and predictive accuracy. Classical travel demand models rely on strong assumptions and extensive calibration, while generic deep learning models lack theoretical grounding and spatial transferability, limiting their usefulness for long-term planning.

Method: DeepDemand embeds key components of travel demand theory into a deep learning framework. It uses a competitive two-source Dijkstra procedure for local origin-destination region extraction and OD pair screening, combined with a differentiable architecture that models OD interactions and travel-time deterrence.

Result: On 8 years of UK strategic road network data (5088 highway segments), DeepDemand achieved R²=0.718 and MAE=7406 vehicles under random cross-validation, outperforming linear, ridge, random forest, and gravity-style baselines. It maintained strong performance under spatial cross-validation (R²=0.665), showing good geographic transferability.

Conclusion: The integration of transport theory with deep learning provides interpretable highway traffic modelling with practical planning applications. The framework reveals stable nonlinear travel-time deterrence patterns, key socioeconomic drivers, and polycentric OD interaction structures aligned with employment centers and transport hubs.
...
### [187] [Neuro-Symbolic Process Anomaly Detection](https://arxiv.org/abs/2603.26461)
*Devashish Gaikwad,Wil M. P. van der Aalst,Gyunam Park*

Main category: cs.LG

TL;DR: Neuro-symbolic approach using Logic Tensor Networks (LTN) and Declare constraints to improve neural network-based process anomaly detection by incorporating human domain knowledge to distinguish between true anomalies and rare but conformant behavior.


<details>
  <summary>Details</summary>
Motivation: Current neural network-based process anomaly detection methods fail to incorporate human domain knowledge, leading to misclassification of rare but conformant traces as anomalies due to their low frequency, which limits detection effectiveness.

Method: Proposes a neuro-symbolic approach that integrates domain knowledge using Logic Tensor Networks (LTN) and Declare constraints. Uses autoencoder models as foundation and encodes Declare constraints as soft logical guiderails within the learning process to distinguish between anomalous and rare but conformant behavior.

Result: Evaluations on synthetic and real-world datasets show improved F1 scores even with as few as 10 conformant traces, and demonstrate that the choice of Declare constraint (and by extension human domain knowledge) significantly influences performance gains.

Conclusion: The neuro-symbolic approach successfully integrates domain knowledge into neural anomaly detection, addressing the limitation of purely statistical methods and improving the ability to distinguish between true anomalies and rare but conformant behavior in process mining.
...
### [188] [Automatic feature identification in least-squares policy iteration using the Koopman operator framework](https://arxiv.org/abs/2603.26464)
*Christian Mugisho Zagabe,Sebastian Petiz*

Main category: cs.LG

TL;DR: KAE-LSPI algorithm combines Koopman autoencoder with least-squares policy iteration for automatic feature learning in reinforcement learning, eliminating need for manual feature/kernel selection.


<details>
  <summary>Details</summary>
Motivation: Addresses the lack of systematic choice of features or kernels in linear RL techniques, which requires manual feature engineering or kernel selection in previous methods like LSPI and KLSPI.

Method: Reformulates least-squares fixed-point approximation using extended dynamic mode decomposition (EDMD) and integrates Koopman autoencoder framework for automatic feature learning. Combines Koopman autoencoder with least-squares policy iteration.

Result: Empirical results show KAE-LSPI learns reasonable number of features compared to fixed features in classical LSPI. Convergence to optimal/near-optimal policy is comparable to both LSPI and KLSPI methods on stochastic chain walk and inverted pendulum problems.

Conclusion: KAE-LSPI provides an effective approach for automatic feature learning in RL without requiring a priori feature or kernel specification, achieving comparable performance to existing methods while eliminating manual feature engineering.
...
### [189] [A Boltzmann-machine-enhanced Transformer For DNA Sequence Classification](https://arxiv.org/abs/2603.26465)
*Zhixuan Cao,Yishu Xu,Xuang WU*

Main category: cs.LG

TL;DR: A Transformer enhanced with Boltzmann machines for DNA sequence classification that learns interpretable discrete gating structures while maintaining end-to-end differentiability.


<details>
  <summary>Details</summary>
Motivation: DNA sequence classification needs both high accuracy and the ability to uncover latent interactions, combinatorial regulation, and higher-order dependencies. Standard Transformers have continuous, dense attention that's good for information routing but poor for explicit structure discovery.

Method: Proposes a Boltzmann-machine-enhanced Transformer with structured binary gating variables for latent query-key connections, constrained by a Boltzmann energy function. Uses mean-field variational inference with Gumbel-Softmax to estimate edge activation probabilities while maintaining differentiability. Jointly optimizes classification and energy losses.
...
Conclusion: The proposed method offers a principled way to combine the global modeling capacity of Transformers with structured, interpretable latent representations suitable for biological sequence analysis.
...
### [190] [Foundation Model for Cardiac Time Series via Masked Latent Attention](https://arxiv.org/abs/2603.26475)
...
TL;DR: LAMAE is a foundation model that learns cross-lead connections in ECG signals through latent attention, improving representation quality and outperforming independent-lead approaches.


<details>
  <summary>Details</summary>
Motivation: Existing ECG foundation models treat leads as independent channels and fail to leverage the strong structural redundancy inherent in ECG signals, which limits representation quality and transferability.

Method: Latent Attention Masked Autoencoder (LAMAE) learns cross-lead connection mechanisms through latent attention during self-supervised pretraining, enabling permutation-invariant aggregation and adaptive weighting of lead-specific representations.

Result: The method shows strong performance in predicting ICD-10 codes on the Mimic-IV-ECG database, outperforming independent-lead masked modeling and alignment-based baselines.

Conclusion: Leveraging cross-lead connections constitutes an effective form of structural supervision that improves ECG representation quality and transferability for cardiovascular diagnosis.
...
### [191] [Shapley meets Rawls: an integrated framework for measuring and explaining unfairness](https://arxiv.org/abs/2603.26476)
*Fadoua Amri-Jouidel,Emmanuel Kemel,Stéphane Mussard*

Main category: cs.LG

TL;DR: Shapley values provide integrated framework to both define and explain unfairness under group fairness criteria, with extensions to ESL values offering robustness and faster computation.


<details>
  <summary>Details</summary>
Motivation: Explainability and fairness have typically been studied separately, with recent work only explaining sources of unfairness. There's a need for an integrated framework that can both define unfairness and explain its sources.

Method: Use Shapley values to define and explain unfairness under standard group fairness criteria. Extend framework to Efficient-Symmetric-Linear (ESL) values for more robust fairness definitions and faster computation. Apply to Census Income dataset.

Result: Identified "Age", "Number of hours", and "Marital status" as features generating gender unfairness. Achieved shorter computation time than traditional Bootstrap tests.

Conclusion: Shapley values offer unified framework for fairness definition and explanation. ESL extensions provide computational efficiency and robustness advantages over traditional methods.
...
### [192] [SPECTRA: An Efficient Spectral-Informed Neural Network for Sensor-Based Activity Recognition](https://arxiv.org/abs/2603.26482)
*Deepika Gurung,Lala Shakti Swarup Ray,Mengxi Liu,Bo Zhou,Paul Lukowicz*

Main category: cs.LG

TL;DR: SPECTRA is a spectral-temporal architecture for edge-deployable human activity recognition that balances accuracy with resource constraints by integrating STFT feature extraction, depthwise separable convolutions, and attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Real-time sensor applications in pervasive computing need edge-deployable models for low latency, privacy, and efficient interaction. Current deep learning approaches treat temporal sensor signals as black-box sequences, overlooking spectral-temporal structure while demanding excessive computation.

Method: SPECTRA integrates short-time Fourier transform (STFT) for feature extraction, depthwise separable convolutions, and channel-wise self-attention to capture spectral-temporal dependencies. It uses a compact bidirectional GRU with attention pooling to summarize within-window dynamics at low cost.

Result: Across five public HAR datasets, SPECTRA matches or approaches larger CNN-LSTM and Transformer baselines while substantially reducing parameters, latency, and energy. Deployments on Google Pixel 9 smartphone and STM32L4 microcontroller demonstrate end-to-end deployable, real-time, private, and efficient HAR.

Conclusion: SPECTRA provides a deployment-first co-designed spectral-temporal architecture that effectively balances accuracy with resource constraints for edge-deployable human activity recognition, enabling real-time, private, and efficient pervasive computing applications.
...
### [193] [EcoFair: Trustworthy and Energy-Aware Routing for Privacy-Preserving Vertically Partitioned Medical Inference](https://arxiv.org/abs/2603.26483)
...
TL;DR: EcoFair is a privacy-preserving medical inference framework for dermatology that uses selective routing to balance energy efficiency and diagnostic accuracy by transmitting only embeddings and activating heavier image encoders only when needed based on uncertainty and clinical risk.
...
Method: Vertically partitioned inference framework with raw data kept local; only modality-specific embeddings transmitted. Uses lightweight-first routing that selectively activates heavier image encoder based on local uncertainty, safe-danger probability gap, and neurosymbolic risk score from patient age/lesion localization.

Result: Substantially reduces edge-side inference energy while maintaining competitive classification performance on three dermatology benchmarks. Selective routing improves subgroup-sensitive malignant-case behavior without modifying global training objective.

Conclusion: EcoFair provides a practical framework for privacy-preserving, energy-aware medical inference under edge deployment constraints, balancing efficiency with diagnostic reliability.
...
### [194] [A Lyapunov Analysis of Softmax Policy Gradient for Stochastic Bandits](https://arxiv.org/abs/2603.26547)
*Tor Lattimore*

Main category: cs.LG

TL;DR: Adaptation of continuous-time policy gradient analysis for k-armed stochastic bandits to discrete time, showing regret bounds with specific learning rate conditions.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between continuous-time analysis of policy gradient methods for bandits (Lattimore 2026) and the standard discrete-time setup commonly used in practice.

Method: Adapt the continuous-time analysis framework to discrete time, analyzing policy gradient algorithms with specific learning rate conditions for k-armed stochastic bandits.

Result: With learning rate η = O(Δ_min²/(Δ_max log(n))), the regret is O(k log(k) log(n)/η), where n is horizon, Δ_min and Δ_max are minimum and maximum gaps between arms.

Conclusion: The analysis successfully extends continuous-time policy gradient results to discrete time, providing theoretical guarantees for regret bounds under appropriate learning rate conditions.
...
### [195] [Sharp Capacity Scaling of Spectral Optimizers in Learning Associative Memory](https://arxiv.org/abs/2603.26554)
*Juno Kim,Eshaan Nichani,Denny Wu,Alberto Bietti,Jason D. Lee*

Main category: cs.LG

TL;DR: Muon optimizer outperforms SGD on associative memory tasks with Gaussian embeddings, achieving higher storage capacity, larger critical batch size, and faster initial recovery while reaching the same information-theoretic limit.


<details>
  <summary>Details</summary>
Motivation: To understand why spectral optimizers like Muon perform better than SGD in large-scale language model training, using the tractable linear associative memory problem as a proxy for factual recall in transformers.

Method: Analyze Muon vs SGD on logistic regression loss with Gaussian inputs/outputs (non-orthogonal embeddings) under power law frequency distribution. Use theoretical analysis of one-step recovery rates, storage capacity, critical batch size, and multi-step dynamics with thresholded gradient approximation.

Result: Muon has significantly higher storage capacity than SGD, saturates at larger critical batch size, achieves faster initial recovery rate, but both methods eventually converge to the same information-theoretic limit at comparable speeds. Synthetic experiments validate predicted scaling laws.

Conclusion: The analysis quantitatively explains Muon's signal amplification advantage and provides groundwork for establishing scaling laws across practical language modeling tasks and optimizers.
...
### [196] [Machine Unlearning under Retain-Forget Entanglement](https://arxiv.org/abs/2603.26569)
*Jingpu Cheng,Ping Liu,Qianxiao Li,Chi Zhang*

Main category: cs.LG

TL;DR: A two-phase optimization framework for machine unlearning that handles retai-forget entanglements by first increasing forget set loss while preserving accuracy on less-related samples, then using gradient projection to protect semantically related retained samples.


<details>
  <summary>Details</summary>
Motivation: In machine unlearning, forgetting a subset often unintentionally affects related retained samples that share correlated features or semantic similarities, creating "retai-forget entanglements" that need to be addressed.

Method: Two-phase optimization: Phase 1 uses augmented Lagrangian method to increase loss on forget set while preserving accuracy on less-related retained samples. Phase 2 applies gradient projection regularized by Wasserstein-2 distance to mitigate performance degradation on semantically related retained samples without compromising unlearning.

Result: Validated through comprehensive experiments on multiple unlearning tasks, standard benchmark datasets, and diverse neural architectures, demonstrating effective and reliable unlearning while outperforming existing baselines in both accuracy retention and removal fidelity.

Conclusion: The proposed framework successfully addresses retai-forget entanglements in machine unlearning, achieving better balance between forgetting unwanted data and preserving performance on related retained samples compared to existing methods.
...
### [197] [Evaluating Interactive 2D Visualization as a Sample Selection Strategy for Biomedical Time-Series Data Annotation](https://arxiv.org/abs/2603.26592)
*Einari Vaaras,Manu Airaksinen,Okko Räsänen*

Main category: cs.LG

TL;DR: 2DV visualization-based sampling outperforms random and farthest-first sampling for biomedical time-series annotation when aggregating labels, but has higher variability risk with limited budgets.


<details>
  <summary>Details</summary>
Motivation: Biomedical time-series annotation is challenging, and while algorithmic sample selection could help, there's limited evidence from studies with real human annotators comparing different methods.

Method: Compared three sample selection methods: random sampling (RND), farthest-first traversal (FAFT), and 2D visualization-based exploration (2DV). Evaluated with 12 annotators (experts/non-experts) across four classification tasks in infant motility assessment and speech emotion recognition under limited annotation budgets.

Result: 2DV performed best when aggregating labels across annotators across all tasks. In IMA, 2DV best captured rare classes but had higher label distribution variability, decreasing individual annotator performance (where FAFT excelled). For SER, 2DV outperformed among experts and matched for non-experts. RND was safest with uncertain annotator count/expertise; 2DV had highest risk due to variability. Annotators found 2DV more interesting/enjoyable.

Conclusion: 2DV-based sampling shows promise for biomedical time-series annotation, especially when annotation budgets aren't highly constrained, though its higher variability risk must be considered.
...
### [198] [Benchmarking Tabular Foundation Models for Conditional Density Estimation in Regression](https://arxiv.org/abs/2603.26611)
*Rafael Izbicki,Pedro L. C. Rodrigues*

Main category: cs.LG

TL;DR: Tabular foundation models (TabPFN, TabICL) outperform traditional CDE methods across most metrics and datasets, establishing them as strong off-the-shelf conditional density estimators.
...
Method: Benchmarked three tabular foundation model variants against diverse parametric, tree-based, and neural CDE baselines on 39 real-world datasets across training sizes from 50 to 20,000, using six metrics covering density accuracy, calibration, and computation time.

Result: Foundation models achieve best CDE loss, log-likelihood, and CRPS on most datasets across all sample sizes. Calibration is competitive at small sample sizes but lags behind task-specific neural baselines at larger sizes. In photometric redshift case study, TabPFN with 50k training galaxies outperforms all baselines trained on 500k galaxies.

Conclusion: Tabular foundation models are strong off-the-shelf conditional density estimators, though post-hoc recalibration may be valuable for improving calibration at larger sample sizes.
...
### [199] [Context-specific Credibility-aware Multimodal Fusion with Conditional Probabilistic Circuits](https://arxiv.org/abs/2603.26629)
*Pranuthi Tenali,Sahil Sidheekh,Saurabh Mathur,Erik Blasch,Kristian Kersting,Sriraam Natarajan*

Main category: cs.LG

TL;DR: C²MF is a context-specific credibility-aware multimodal fusion framework that models per-instance source reliability using Conditional Probabilistic Circuits, improving robustness to cross-modal conflicts.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal fusion approaches rely on static assumptions about source reliability, which limits their ability to handle conflicts when modalities become unreliable due to situational factors like sensor degradation or class-specific corruption.

Method: Introduces C²MF framework using Conditional Probabilistic Circuits (CPC) to model per-instance source reliability, formalized through Context-Specific Information Credibility (CSIC) - a KL-divergence-based measure computed exactly from CPC.

Result: C²MF improves predictive accuracy by up to 29% over static-reliability baselines in high-noise settings, while preserving interpretability advantages of probabilistic circuit-based fusion.

Conclusion: The framework enables principled and adaptive reliability assessment that generalizes conventional static credibility estimates, demonstrating effectiveness on the proposed Conflict benchmark with class-specific corruptions.
...
### [200] [Automatic Laplace Collapsed Sampling: Scalable Marginalisation of Latent Parameters via Automatic Differentiation](https://arxiv.org/abs/2603.26644)
*Toby Lovick,David Yallup,Will Handley*

Main category: cs.LG

TL;DR: ALCS is a framework that combines automatic differentiation with nested sampling to efficiently marginalize latent parameters in Bayesian models by collapsing them via MAP optimization and Laplace approximation.


<details>
  <summary>Details</summary>
Motivation: Bayesian evidence computation becomes intractable for high-dimensional models with latent variables, requiring hand-derived gradients/Hessians and extensive model-specific engineering. There's a need for automated, scalable methods that can handle complex models without manual derivations.

Method: At each nested sampling likelihood evaluation, ALCS collapses high-dimensional latent variables to a scalar contribution using MAP optimization and Laplace approximation, both computed via automatic differentiation. This reduces effective dimension from d_θ + d_z to just d_θ. The framework parallelizes MAP optimization and Hessian evaluation across live points on GPU hardware and extends beyond Gaussian approximations to parametric families like Student-t for heavy-tailed latents.

Result: ALCS makes Bayesian evidence computation tractable for high-dimensional settings without manual gradient/Hessian derivations. The method is validated on benchmarks spanning hierarchical, time-series, and discrete-likelihood models, establishing where Gaussian approximations hold. It enables post-hoc ESS diagnostics that localize failures across hyperparameter space without expensive joint sampling.

Conclusion: ALCS provides a general, automated framework for efficient Bayesian evidence computation in high-dimensional models with latent variables, leveraging automatic differentiation and GPU parallelization to achieve scalability while maintaining robustness through nested sampling.
...
### [201] [Joint Sensing and Covert Communications in RIS-NOMA Systems](https://arxiv.org/abs/2603.26101)
*Jiayi Lei,Xidong Mu,Tiankui Zhang,Wenjun Xu,Ping Zhang*

Main category: eess.SP

TL;DR: RIS-assisted NOMA system with DFRC base station simultaneously performs radar sensing of warden location and covert communications to legitimate users, with solutions for both known and unknown warden locations.


<details>
  <summary>Details</summary>
Motivation: To develop a system that can simultaneously perform covert communications while sensing potential eavesdroppers (wardens), addressing the challenge of maintaining covertness when the warden's location is unknown.

Method: For known warden location: joint optimization of active/passive beamforming using SCA, penalty method, and SDR. For unknown location: radar sensing-based CSI estimation with CRB-bound uncertainty, robust optimization using Schur complement and S-procedure.

Result: Sensing-based imperfect CSI model with NOMA outperforms general norm-bounded CSI model and orthogonal multiple access. Sensing has dual contradictory effects on covert communications. Alice uses Carol's signal for sensing and Bob's signal for communication.

Conclusion: RIS-assisted NOMA DFRC systems can effectively integrate sensing and covert communications, with radar sensing providing superior CSI estimation for unknown warden locations compared to conventional approaches.
...
### [202] [Movable-Antenna Index Modulation (MA-IM): System Framework and Performance Analysis](https://arxiv.org/abs/2603.26153)
*Bang Huang,Shunyuan Shang,Mohamed-Slim Alouini*

Main category: eess.SP

TL;DR: MA-IM framework uses movable antenna spatial positions as modulation resource via anchor selection strategies, with joint constellation-aware design achieving best error performance.


<details>
  <summary>Details</summary>
Motivation: To exploit spatial mobility of reconfigurable antennas as additional information-bearing dimensions for next-generation wireless systems, converting spatial degrees of freedom into practical modulation resource.

Method: Discretize continuous movable region into candidate points, select representative anchors for indexing, develop anchor-selection strategies (geometry-based, SNR-based, max-min channel-domain, joint constellation-aware), derive joint ML detectors and low-complexity two-stage detector, establish analytical ABEP bounds.

Result: Direct indexing of all sampling points is unreliable, anchor optimization is necessary. Performance depends on channel richness, spatial correlation, index states, modulation order. Joint constellation-aware design achieves best error performance, can approach/outperform same-spectral-efficiency QAM baselines.

Conclusion: MA-IM effectively converts antenna spatial mobility into modulation resource, with joint constellation-aware anchor design being optimal. System parameters significantly impact performance, and proper anchor design enables competitive performance with conventional QAM.
...
### [203] [Antenna Elements' Trajectory Optimization for Throughput Maximization in Continuous-Trajectory Fluid Antenna-Aided Wireless Communications](https://arxiv.org/abs/2603.26216)
*Shuaixin Yang,Yijia Li,Yue Xiao,Yong Liang Guan,Kai-Kit Wong,Hyundong Shin,Chau Yuen*

Main category: eess.SP

TL;DR: CTFA introduces continuous antenna trajectories with kinematic constraints for fluid antenna systems, optimizing both trajectories and transmit covariance to maximize throughput.


<details>
  <summary>Details</summary>
Motivation: Existing FA works only optimize positions at discrete times, ignoring continuous movement and practical kinematic constraints. CTFA addresses this gap by considering realistic antenna movement trajectories over continuous time intervals.

Method: Formulated throughput maximization problem with joint optimization of continuous antenna trajectories and transmit covariance under kinematic constraints. Developed iterative BCD-MM algorithm with WMMSE method to solve the non-convex, coupled optimization problem.

Result: Numerical results validate algorithm efficacy and show substantial throughput advantages of CTFA over conventional fixed-position antennas and simplified trajectory approaches.

Conclusion: CTFA with continuous trajectories and kinematic constraints provides significant performance gains in fluid antenna systems, outperforming both traditional fixed antennas and discrete-time FA approaches.
...
### [204] [WiMamba: Linear-Scale Wireless Foundation Model](https://arxiv.org/abs/2603.26367)
*Tomer Raviv,Nir Shlezinger*

Main category: eess.SP

TL;DR: WiMamba is a wireless foundation model using Mamba architecture (selective state-space models) instead of transformers, achieving linear-time sequence modeling with lower latency/memory while matching or outperforming transformer-based wireless models.


<details>
  <summary>Details</summary>
Motivation: Transformer-based wireless foundation models have quadratic computational/memory complexity that hinders practical deployment for large-scale channels. There's a need for more efficient architectures that maintain strong representational capabilities.

Method: WiMamba uses Mamba architecture with selective state-space models instead of attention mechanisms, enabling linear-time sequence modeling. Combines this with adaptive preprocessing and a dedicated task-agnostic, self-supervised pre-training framework tailored to wireless channels.

Result: WiMamba matches or outperforms transformer-based wireless foundation models across four downstream tasks while offering dramatic latency and memory reductions.

Conclusion: WiMamba provides a scalable, low-latency wireless foundation model alternative to transformers that maintains strong representational expressivity through Mamba architecture and specialized wireless pre-training.
...
### [205] [PARAFAC-Based Channel Estimation for Beyond Diagonal Reconfigurable Surfaces](https://arxiv.org/abs/2603.26384)
*Gilderlan Tavares de Araújo,Bruno Sokal,André L. F. de Almeida*

Main category: eess.SP

TL;DR: Proposes a tensor-based channel estimation method for BD-RIS MIMO systems that maintains constant interconnection topology while achieving accurate channel recovery with reduced complexity.


<details>
  <summary>Details</summary>
Motivation: Channel estimation is a major bottleneck in BD-RIS-assisted MIMO systems due to complex inter-element coupling. Existing methods either estimate only cascaded channels or require costly block-by-block reconfiguration of BD-RIS interconnections, making practical implementation difficult.

Method: Proposes a pilot-assisted tensor framework for group-connected BD-RIS under a two-timescale protocol. Designs scattering structure as a low-rank PARAFAC model with fixed factor matrices to maintain constant interconnection topology across blocks (only phase shifts updated). Develops PARAFAC-based alternating least-squares (PALS) receiver to recover individual channels.

Result: PALS achieves significantly lower composite-channel NMSE than conventional LS, matches accuracy of state-of-the-art tensor receivers, and sharply reduces BD-RIS design complexity while enabling practical operation.

Conclusion: The proposed tensor framework overcomes practical limitations of existing BD-RIS channel estimators by maintaining constant interconnection topology while achieving accurate channel recovery with reduced complexity, enabling more practical deployment of BD-RIS-assisted MIMO systems.
...
### [206] [A Lightweight, Transferable, and Self-Adaptive Framework for Intelligent DC Arc-Fault Detection in Photovoltaic Systems](https://arxiv.org/abs/2603.25749)
*Xiaoke Yang,Long Gao,Haoyu He,Hanyuan Hang,Qi Liu,Shuai Zhao,Qiantu Tuo,Rui Li*

Main category: eess.SP

TL;DR: Lightweight learning-driven framework achieves near-perfect DC arc-fault detection in PV systems with 0% false-trip rate across diverse conditions and hardware platforms.


<details>
  <summary>Details</summary>
Motivation: Conventional AFCIs struggle with reliable DC arc-fault detection due to spectral interference from inverter switching, hardware heterogeneity, operating-condition drift, and environmental noise, compromising fire safety in residential PV systems.

Method: Three-tier learning framework: LD-Spec for compact spectral representations enabling efficient on-device inference; LD-Align for cross-hardware representation alignment to handle hardware heterogeneity; LD-Adapt for cloud-edge collaborative self-adaptive updating to address long-term evolution and unseen operating conditions.

Result: Achieved 0.9999 accuracy and 0.9996 F1-score on 53,000+ samples with 0% false-trip rate across nuisance conditions. Cross-hardware transfer required only 0.5%-1% labeled target data. Field adaptation recovered detection precision from 21% to 95% under unseen conditions.

Conclusion: The LD-framework enables a scalable, deployment-oriented AFCI solution that maintains highly reliable detection across heterogeneous devices and long-term operation, addressing key challenges in residential PV fire safety.
...
### [207] [Beam-Coherence-Aware Two-Stage Digital Combining for mmWave MU-MIMO Systems](https://arxiv.org/abs/2603.26566)
*Yasaman Khorsandmanesh,Emil Bjornson,Joakim Jalden,Bengt Lindoff*

Main category: eess.SP

TL;DR: Two-stage digital combining architecture for wideband mmWave MIMO reduces UE processing burden by compressing signals before baseband processing, with geometry-based first stage updated slowly and second stage updated per coherence time.


<details>
  <summary>Details</summary>
Motivation: To reduce the baseband processing burden at user equipment (UE) in wideband millimeter-wave MIMO systems with fully digital transceivers, especially in mobile scenarios where computational complexity is a concern.

Method: Proposes a two-stage digital combining architecture: 1) First-stage combining matrix exploits channel geometry and is updated on beam-coherence timescale (longer than channel coherence time), compressing signals from K antennas to N_c dimensions; 2) Second stage is updated per channel coherence time. Also develops pilot-based channel estimation framework using maximum likelihood estimation and time-domain method exploiting finite delay spread to reconstruct full channel from reduced pilot subcarriers.

Result: Numerical results show the proposed time-domain approach outperforms hybrid beamforming while reducing pilot overhead. The framework extends to multi-user MIMO and retains performance advantages.

Conclusion: Two-stage fully digital transceivers show potential for future wideband systems by balancing processing efficiency with performance, offering advantages over hybrid beamforming with reduced pilot overhead.
...
### [208] [BeSafe-Bench: Unveiling Behavioral Safety Risks of Situated Agents in Functional Environments](https://arxiv.org/abs/2603.25747)
*Yuxuan Li,Yi Lin,Peng Wang,Shiming Liu,Xuetao Wei*

Main category: cs.AI

TL;DR: BSB is a benchmark for evaluating behavioral safety risks of situated agents in functional environments across web, mobile, and embodied domains, revealing that current agents struggle to balance task completion with safety compliance.


<details>
  <summary>Details</summary>
Motivation: The rapid deployment of Large Multimodal Models (LMMs) as autonomous decision-makers introduces substantial unintentional behavioral safety risks, but existing evaluations are inadequate due to low-fidelity environments, simulated APIs, or narrowly scoped tasks.

Method: BeSafe-Bench (BSB) uses functional environments across four domains (Web, Mobile, Embodied VLM, Embodied VLA), constructs diverse instructions augmented with nine categories of safety-critical risks, and employs a hybrid evaluation framework combining rule-based checks with LLM-as-a-judge reasoning.

Result: Evaluation of 13 popular agents shows concerning results: even the best-performing agent completes fewer than 40% of tasks while fully adhering to safety constraints, and strong task performance frequently coincides with severe safety violations.

Conclusion: There is an urgent need for improved safety alignment before deploying agentic systems in real-world settings, as current agents demonstrate significant safety risks despite task capabilities.
...
### [209] [AutoB2G: A Large Language Model-Driven Agentic Framework For Automated Building-Grid Co-Simulation](https://arxiv.org/abs/2603.26005)
*Borui Zhang,Nariman Mahdavi,Subbu Sethuvenkatraman,Shuang Ao,Flora Salim*

Main category: cs.AI

TL;DR: AutoB2G is an automated building-grid co-simulation framework that uses LLMs to generate and refine simulators from natural language descriptions, improving grid-side performance metrics.


<details>
  <summary>Details</summary>
Motivation: Existing building simulation environments focus on building-side metrics and lack systematic grid-level impact evaluation, while requiring manual configuration and programming expertise. There's a need for automated frameworks that can coordinate building-to-grid interactions.

Method: Extends CityLearn V2 for B2G interaction and uses LLM-based SOCIA framework to automatically generate, execute, and refine simulators. Creates a codebase organized as DAG to represent module dependencies, guiding LLMs to retrieve executable paths without prior knowledge of simulation context.
...
Conclusion: The framework successfully automates the building-grid co-simulation workflow using natural language descriptions and LLMs, addressing the gap in grid-level impact evaluation while reducing manual configuration requirements.
...
### [210] [Semi-Automated Knowledge Engineering and Process Mapping for Total Airport Management](https://arxiv.org/abs/2603.26076)
*Darryl Teo,Adharsha Sam,Chuan Shen Marcus Koh,Rakesh Nagi,Nuno Antunes Ribeiro*

Main category: cs.AI

TL;DR: A framework combining symbolic knowledge engineering and LLMs to build domain-specific knowledge graphs for airport operations, ensuring traceability and addressing semantic inconsistencies in fragmented documentation.


<details>
  <summary>Details</summary>
Motivation: Airport operations documentation is complex due to technical terminology, regulations, proprietary information, and fragmented communication across stakeholders, creating data silos and semantic inconsistencies that hinder Total Airport Management initiatives.

Method: Dual-stage fusion framework: symbolic Knowledge Engineering structures guide LLM prompts to discover semantically aligned knowledge triples. Uses scaffolded fusion strategy with probabilistic model for discovery and deterministic algorithm for anchoring extractions to source documents. Evaluated on Google LangExtract library comparing segment-based vs document-level processing.

Result: Document-level processing improves recovery of non-linear procedural dependencies, contrary to prior observations of long-context degradation in LLMs. The framework ensures absolute traceability and verifiability by fusing probabilistic discovery with deterministic source anchoring.

Conclusion: The proposed framework bridges the gap between "black-box" generative outputs and transparency required for operational tooling, enabling automated synthesis of complex operational workflows from unstructured textual corpora while maintaining high-fidelity provenance essential for airport operations.
...
### [211] [GUIDE: Resolving Domain Bias in GUI Agents through Real-Time Web Video Retrieval and Plug-and-Play Annotation](https://arxiv.org/abs/2603.26266)
*Rui Xie,Zhi Gao,Chenrui Shi,Zirui Shang,Lu Chen,Qing Li*

Main category: cs.AI

TL;DR: GUIDE is a training-free framework that reduces GUI agent domain bias by automatically extracting domain-specific expertise from tutorial videos through retrieval-augmented annotation.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models have strong general GUI capabilities but suffer from domain bias - they lack familiarity with specific software workflows and UI layouts, limiting real-world task performance.

Method: Two key innovations: 1) Subtitle-driven Video-RAG pipeline with progressive three-stage retrieval (domain classification, topic extraction, relevance matching), 2) Fully automated annotation pipeline using inverse dynamics paradigm with UI element detection and VLM inference to extract planning and grounding knowledge.

Result: Extensive experiments on OSWorld show over 5% improvements and reduced execution steps for both multi-agent systems and single-model agents, without modifying model parameters or architecture.

Conclusion: GUIDE is an effective, architecture-agnostic enhancement that bridges GUI agent domain bias through autonomous expertise acquisition from tutorial videos.
...
### [212] [AIRA_2: Overcoming Bottlenecks in AI Research Agents](https://arxiv.org/abs/2603.26499)
...
TL;DR: AIRA₂ addresses three structural bottlenecks in AI research agents through asynchronous multi-GPU execution, Hidden Consistent Evaluation, and ReAct agents, achieving state-of-the-art performance on MLE-bench-30 with steady improvement over time.


<details>
  <summary>Details</summary>
Motivation: Existing AI research agents suffer from three key bottlenecks: 1) synchronous single-GPU execution limiting throughput, 2) generalization gap where validation-based selection degrades performance over extended search, and 3) limited capability of fixed single-turn LLM operators capping search performance.

Method: AIRA₂ introduces three architectural innovations: 1) asynchronous multi-GPU worker pool for linear throughput scaling, 2) Hidden Consistent Evaluation protocol for reliable evaluation signals, and 3) ReAct agents that dynamically scope actions and debug interactively.

Result: On MLE-bench-30, AIRA₂ achieves 71.8% mean Percentile Rank at 24 hours (surpassing previous best of 69.9%) and steadily improves to 76.0% at 72 hours. Ablations show each component is necessary and that prior "overfitting" was due to evaluation noise, not data memorization.

Conclusion: AIRA₂ successfully addresses three structural bottlenecks in AI research agents through its novel architecture, achieving state-of-the-art performance with steady improvement over time, while revealing that previous overfitting concerns were actually evaluation noise artifacts.
...
### [213] [CADSmith: Multi-Agent CAD Generation with Programmatic Geometric Validation](https://arxiv.org/abs/2603.26512)
*Jesse Barkley,Rumi Loghmani,Amir Barati Farimani*

Main category: cs.AI

TL;DR: CADSmith is a multi-agent pipeline that generates CadQuery code from natural language with iterative refinement using geometric validation, achieving significant improvements in CAD model quality and reliability.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-CAD generation methods either operate in a single pass without geometric verification or rely on lossy visual feedback that cannot resolve dimensional errors, limiting their reliability and accuracy.

Method: CADSmith uses a multi-agent pipeline with two nested correction loops: an inner loop resolves execution errors, and an outer loop performs programmatic geometric validation using OpenCASCADE kernel measurements (bounding box dimensions, volume, solid validity) combined with visual assessment from a vision-language model. It uses retrieval-augmented generation over API documentation rather than fine-tuning.

Result: On a custom benchmark of 100 prompts across three difficulty tiers, CADSmith achieved 100% execution rate (up from 95%), improved median F1 score from 0.9707 to 0.9846, median IoU from 0.8085 to 0.9629, and reduced mean Chamfer Distance from 28.37 to 0.74 compared to a zero-shot baseline.

Conclusion: Closed-loop refinement with programmatic geometric feedback substantially improves the quality and reliability of LLM-generated CAD models, demonstrating the effectiveness of combining exact geometric measurements with holistic visual assessment in an iterative refinement pipeline.
...
### [214] [Stabilizing Rubric Integration Training via Decoupled Advantage Normalization](https://arxiv.org/abs/2603.26535)
...
TL;DR: PAPO integrates process-level evaluation into GRPO via decoupled advantage normalization to address limitations of outcome-only rewards (which ignore reasoning quality) and process rewards (which cause reward hacking).


<details>
  <summary>Details</summary>
Motivation: Existing reward designs have two key limitations: outcome reward models (ORM) only evaluate final-answer correctness, treating all correct responses identically regardless of reasoning quality, and lose advantage signal as groups become uniformly correct. Process reward models (PRM) offer richer supervision but cause reward hacking where models exploit verbosity to inflate scores while accuracy collapses.

Method: PAPO composes advantage from two components: Aout (derived from ORM and normalized over all responses) and Aproc (derived from a rubric-based PRM and normalized exclusively among correct responses). This decoupled design ensures Aout anchors training on correctness while Aproc differentiates reasoning quality without distorting the outcome signal.
...
Conclusion: PAPO effectively addresses limitations of both outcome-only and process reward models by integrating process-level evaluation through decoupled advantage normalization, enabling better reasoning quality differentiation while maintaining correctness focus.
...
### [215] [ETA-VLA: Efficient Token Adaptation via Temporal Fusion and Intra-LLM Sparsification for Vision-Language-Action Models](https://arxiv.org/abs/2603.25766)
*Yiru Wang,Anqing Jiang,Shuo Wang,Yuwen Heng,Zichong Gu,Hao Sun*

Main category: cs.RO

TL;DR: ETA-VLA is an efficient token adaptation framework for Vision-Language-Action models that reduces computational costs by pruning redundant visual tokens while maintaining driving performance.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models for autonomous driving require processing historical multi-view frames for temporal reasoning, which imposes severe computational burden due to quadratic complexity of self-attention in LLMs.

Method: Proposes ETA-VLA with Intra-LLM Sparse Aggregator (ILSA) that dynamically identifies and prunes redundant visual tokens using text-guided scoring and diversity-preserving sparsification, inspired by human driver attention allocation.

Result: Achieves comparable driving performance to SOTA baselines while reducing FLOPs by ~32%, pruning 85% of visual tokens and reducing inference FLOPs by 61% while retaining 94% of original accuracy on NAVSIM v2 benchmark.

Conclusion: ETA-VLA effectively addresses computational bottlenecks in VLA models for autonomous driving through efficient token adaptation, enabling practical deployment with minimal performance degradation.
...
### [216] [Massive Parallel Deep Reinforcement Learning for Active SLAM](https://arxiv.org/abs/2603.25834)
*Martín Arce Llobera,Julio A. Placed,Mariano De Paula,Pablo De Cristóforis*

Main category: cs.RO

TL;DR: Proposed scalable end-to-end DRL framework for Active SLAM enabling massively parallel training, reducing training time, supporting continuous actions, and exploring realistic scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing DRL-based Active SLAM approaches are constrained by lack of scalable parallel training despite advances in parallel computing and GPU acceleration. Need to leverage these computational advances for computation-intensive learning problems like Active SLAM.

Method: Scalable end-to-end Deep Reinforcement Learning (DRL) framework for Active SLAM that enables massively parallel training. The framework is designed to be open-source to promote reproducibility and community adoption.

Result: Significantly reduces training time compared to state-of-the-art, supports continuous action spaces, and facilitates exploration of more realistic scenarios. The framework is released as open-source.

Conclusion: The proposed scalable DRL framework addresses the parallel training bottleneck in Active SLAM, enabling faster training, continuous action support, and more realistic scenario exploration while being open-source for community use.
...
### [217] [Chasing Autonomy: Dynamic Retargeting and Control Guided RL for Performant and Controllable Humanoid Running](https://arxiv.org/abs/2603.25902)
*Zachary Olkin,William D. Compton,Ryan M. Bena,Aaron D. Ames*

Main category: cs.RO

TL;DR: This paper presents a pipeline for dynamically retargeting human motions to humanoid robots using optimization with hard constraints, creating periodic reference libraries from single human demonstrations, and deploying reinforcement learning controllers for high-speed running with obstacle avoidance.


<details>
  <summary>Details</summary>
Motivation: Current RL controllers for humanoid robots are often limited to single motion playback, which restricts their deployment in long-duration autonomous locomotion. There's a need for controllers that can generate dynamic running behaviors while maintaining controllability and endurance for real-world applications.

Method: The authors developed a pipeline that: 1) Dynamically retargets human motions through optimization with hard constraints, 2) Generates improved periodic reference libraries from single human demonstrations, 3) Uses goal-conditioned and control-guided reward structures that track dynamically optimized human data, and 4) Integrates the controller into a full perception and planning autonomy stack.

Result: The approach achieved running speeds up to 3.3 m/s on a Unitree G1 robot and traversed hundreds of meters in real-world environments. The controller demonstrated both speed and endurance, and was successfully integrated into an autonomy stack for obstacle avoidance while running outdoors.

Conclusion: The proposed pipeline enables humanoid robots to achieve dynamic, high-speed running with endurance and controllability. The combination of dynamically retargeted human motions, optimized reference libraries, and goal-conditioned reward structures results in superior performance for autonomous locomotion in real-world environments.
...
### [218] [Can Vision Foundation Models Navigate? Zero-Shot Real-World Evaluation and Lessons Learned](https://arxiv.org/abs/2603.25937)
*Maeva Guerrier,Karthik Soma,Jana Pavlasek,Giovanni Beltrame*

Main category: cs.RO

TL;DR: The paper presents a real-world evaluation of five state-of-the-art Visual Navigation Models (VNMs) across multiple robot platforms and environments, revealing systematic limitations beyond traditional success rate metrics.


<details>
  <summary>Details</summary>
Motivation: Existing VNM evaluations rely almost exclusively on success rate, which conceals important aspects like trajectory quality, collision behavior, and robustness to environmental changes. There's a need for more comprehensive evaluation frameworks.

Method: Real-world evaluation of five VNMs (GNM, ViNT, NoMaD, NaviBridger, CrossFormer) across two robot platforms and five indoor/outdoor environments. Combined path-based metrics with vision-based goal-recognition scores and assessed robustness through controlled image perturbations (motion blur, sunflare).

Result: Uncovered three systematic limitations: (1) sophisticated models still exhibit frequent collisions indicating limited geometric understanding; (2) models fail to discriminate between perceptually similar locations, causing goal prediction errors in repetitive environments; (3) performance degrades under distribution shift.

Conclusion: Current VNMs have significant limitations in geometric understanding, location discrimination, and robustness. The authors will release evaluation codebase and dataset to facilitate reproducible benchmarking of VNMs.
...
### [219] [Policy-Guided World Model Planning for Language-Conditioned Visual Navigation](https://arxiv.org/abs/2603.25981)
*Amirhosein Chahe,Lifeng Zhou*

Main category: cs.RO

TL;DR: PiJEPA combines learned navigation policies with latent world model planning for instruction-conditioned visual navigation, using policy-derived distributions to warm-start MPPI planning for improved performance.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for instruction-conditioned visual navigation have limitations: reactive policies struggle with long-horizon planning, while world models suffer from poor action initialization in high-dimensional spaces. There's a need to combine the strengths of both approaches.

Method: Two-stage framework: 1) Finetune Octo-based generalist policy with frozen pretrained vision encoder (DINOv2 or V-JEPA-2) on CAST dataset for informed action distribution. 2) Use policy distribution to warm-start MPPI planning over separately trained JEPA world model that predicts future latent states.

Result: PiJEMA significantly outperforms standalone policy execution and uninformed world model planning, achieving improved goal-reaching accuracy and instruction-following fidelity. The policy-derived initialization helps MPPI converge faster to high-quality action sequences.

Conclusion: Combining learned policies with world model planning via policy-informed initialization addresses limitations of both approaches, enabling more effective instruction-conditioned visual navigation with better planning and execution.
...
### [220] [SwarmCoDe: A Scalable Co-Design Framework for Heterogeneous Robot Swarms via Dynamic Speciation](https://arxiv.org/abs/2603.26240)
*Andrew Wilhelm,Josie Hughes*

Main category: cs.RO

TL;DR: SwarmCoDe is a novel evolutionary algorithm that co-designs heterogeneous robot swarms by evolving genetic tags for symbiotic cooperation and dominance genes for swarm composition, enabling scalable optimization of up to 200 agents.


<details>
  <summary>Details</summary>
Motivation: Robot swarms offer robustness and collaborative task execution, but traditional co-design becomes intractable at scale due to exponentially large design spaces. There's a need for scalable frameworks to holistically co-design large-scale heterogeneous swarms.

Method: SwarmCoDe uses a Collaborative Co-Evolutionary Algorithm with dynamic speciation that evolves genetic tags and selectivity genes for emergent symbiotic partner identification, plus dominance genes to control swarm composition independently of evolutionary population size.

Result: Successfully evolved specialized swarms of up to 200 agents (4x evolutionary population size) by simultaneously optimizing task planning and hardware morphology under fabrication budget constraints.

Conclusion: SwarmCoDe provides a scalable, computationally viable pathway for holistic co-design of large-scale heterogeneous robot swarms, overcoming traditional intractability through biologically-inspired evolutionary mechanisms.
...
### [221] [Line-of-Sight-Constrained Multi-Robot Mapless Navigation via Polygonal Visible Regions](https://arxiv.org/abs/2603.26314)
*Ruofei Bai,Shenghai Yuan,Xinhang Xu,Xingyu Ji,Xiaowei Li,Hongliang Guo,Wei-Yun Yau,Lihua Xie*

Main category: cs.RO

TL;DR: Distributed multi-robot navigation framework maintains line-of-sight connectivity using real-time LiDAR scans without global maps, with topology optimization for efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing LoS connectivity maintenance methods require known environment maps, limiting practical deployment in unknown environments with obstacles.

Method: Each robot constructs egocentric visible regions from real-time LiDAR scans, shares them via distributed communication to establish LoS constraints, incorporates them into navigation framework with improved LoS-distance metric and topology optimization.

Result: Framework reliably maintains LoS-connectivity in challenging cluttered environments where existing methods fail, with topology optimization boosting navigation efficiency by ~20%.

Conclusion: Proposed distributed approach enables practical LoS connectivity maintenance in unknown environments without global maps, with topology optimization enhancing efficiency for real-world multi-robot applications.
...
### [222] [DFM-VLA: Iterative Action Refinement for Robot Manipulation via Discrete Flow Matching](https://arxiv.org/abs/2603.26320)
*Jiayi Chen,Wenxuan Song,Shuai Chen,Jingbo Wang,Zhijun Li,Haoang Li*

Main category: cs.RO

TL;DR: DFM-VLA is a discrete flow matching Vision-Language-Action model that enables iterative refinement of action tokens, outperforming existing autoregressive and diffusion approaches in robotic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models with discrete action tokenization have fundamental limitations: once a token is generated (whether sequentially by autoregressive models or in parallel by discrete diffusion), it becomes fixed and cannot be revised, preventing correction of early errors in later iterations.

Method: Proposes DFM-VLA that models a token-level probability velocity field to dynamically update full action sequences across refinement iterations. Uses two velocity field constructions: auxiliary velocity-head and action-embedding-guided formulations. Employs a two-stage decoding strategy with iterative refinement followed by deterministic validation for stable convergence.

Result: Outperforms strong autoregressive, discrete diffusion, and continuous diffusion baselines on CALVIN (average success length 4.44) and LIBERO (average success rate 95.7%) while maintaining high inference efficiency. Also shows strong performance on real-world manipulation tasks.

Conclusion: Discrete flow matching enables effective iterative refinement of action tokens in VLA models, addressing the fundamental limitation of fixed token generation in existing approaches and significantly improving robotic manipulation performance.
...
### [223] [DiffusionAnything: End-to-End In-context Diffusion Learning for Unified Navigation and Pre-Grasp Motion](https://arxiv.org/abs/2603.26322)
*Iana Zhura,Yara Mahmoud,Jeffrin Sam,Hung Khang Nguyen,Didar Seyidov,Miguel Altamirano Cabrera,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: A unified diffusion policy for both navigation and manipulation tasks using only 5 minutes of self-supervised data per task, operating purely from RGB input with efficient deployment capabilities.


<details>
  <summary>Details</summary>
Motivation: Current motion planning requires explicit goal specification and task-specific design, while vision-language-action models demand massive resources, extensive training data, and fail zero-shot in novel scenes.

Method: Multi-scale FiLM conditioning on task mode, depth scale, and spatial attention; trajectory-aligned depth prediction; self-supervised attention from AnyTraverse for goal-directed inference without vision-language models or depth sensors.

Result: Achieves robust zero-shot generalization to novel scenes while operating purely from RGB input (2.0 GB memory, 10 Hz), suitable for onboard deployment.

Conclusion: Presents an efficient, unified approach for both navigation and manipulation that requires minimal data, operates without depth sensors or vision-language models, and enables practical onboard deployment.
...
### [224] [Optimal Prioritized Dissipation and Closed-Form Damping Limitation under Actuator Constraints for Haptic Interfaces](https://arxiv.org/abs/2603.26347)
*Camilla Celli,Andrea Bini,Valerio Novelli,Alessandro Filippeschi,Francesco Porcini,Antonio Frisoli*

Main category: cs.RO

TL;DR: A new damping limitation method for haptic systems that prioritizes optimal dissipation direction to minimize actuator load while maintaining stability and transparency.


<details>
  <summary>Details</summary>
Motivation: Current Time Domain Passivity Approach (TDPA) methods are too conservative, degrading transparency and potentially conflicting with actuator physical limitations. Existing solutions fail to simultaneously account for individual actuator power limits while maximizing transparency.

Method: Proposes a prioritized dissipation approach that first allocates dissipation in an optimal direction to minimize actuator load, then distributes any excess dissipation in the orthogonal hyperplane. Provides closed-form formulation robust for multi-DoF scenarios with actuator and motion anisotropies.

Result: Method experimentally validated using a parallel haptic interface interacting with virtual environments under various operating conditions. Successfully addresses actuator limitations while maintaining stability and transparency.

Conclusion: The proposed damping limitation method effectively overcomes limitations of traditional TDPA by intelligently managing dissipation priorities, enabling stable haptic interaction while respecting actuator constraints and preserving transparency.
...
### [225] [Realtime-VLA V2: Learning to Run VLAs Fast, Smooth, and Accurate](https://arxiv.org/abs/2603.26360)
*Chen Yang,Yucheng Hu,Yunchao Ma,Yunhuan Yang,Jing Tan,Haoqiang Fan*

Main category: cs.RO

TL;DR: Practical techniques for deploying Vision-Language-Action (VLA) models on real robots to achieve impressive execution speeds approaching human operation and hardware limits.


<details>
  <summary>Details</summary>
Motivation: While previous work focused on making neural computation of VLAs fast on GPU, the practical deployment of VLA systems on real robots remained an open challenge, especially for tasks requiring both accuracy and dexterity at high speeds.

Method: A comprehensive stack of techniques including calibration, planning & control systems, and learning-based methods to identify optimal execution speeds for real-world robotic deployment.

Result: The robot achieves execution speeds on par with casual human operation and approaches the hardware limits of their lightweight arm, demonstrating impressive performance in real-world tasks requiring accuracy and dexterity.

Conclusion: The report successfully addresses the practical deployment challenge of VLA systems on real robots, providing a complete solution stack that enables high-speed, accurate robotic operation comparable to human performance.
...
### [226] [T-800: An 800 Hz Data Glove for Precise Hand Gesture Tracking](https://arxiv.org/abs/2603.26403)
*Haoyang Luo,Zihang Zhao,Leiyao Cui,Saiyao Zhang,Liu Yang,Zhi Han,Xiyuan Tang,Yixin Zhu*

Main category: cs.RO

TL;DR: T-800 is an 800Hz data glove system that captures high-frequency hand motion previously inaccessible due to temporal undersampling limitations, revealing human dexterity has >100Hz motion energy and enabling robotic hand retargeting.


<details>
  <summary>Details</summary>
Motivation: Existing motion capture systems face a trade-off between temporal resolution and visual occlusion, failing to capture fine-grained hand motion during fast, contact-rich manipulation. Human dexterity involves rapid sub-second adjustments that current systems cannot record.

Method: Developed T-800, a high-bandwidth data glove system with 18 distributed IMUs operating at 800Hz. Uses novel broadcast-based synchronization mechanism and mechanical stress isolation architecture to maintain sub-frame temporal alignment during vigorous movements.

Result: System recovers fine-grained manipulation details previously lost to temporal undersampling. Reveals human dexterity exhibits significantly high-frequency motion energy (>100Hz) that was fundamentally inaccessible due to Nyquist sampling limits of previous hardware.

Conclusion: T-800 enables accurate kinematic retargeting of high-fidelity human gestures onto dexterous robotic hands, providing rich behavioral data necessary for training robust robotic control policies in the future.
...
### [227] [Generalizable task-oriented object grasping through LLM-guided ontology and similarity-based planning](https://arxiv.org/abs/2603.26412)
*Hao Chen,Takuya Kiyokawa,Weiwei Wan,Kensuke Harada*

Main category: cs.RO

TL;DR: A geometry-centric approach for task-oriented grasping that uses LLM-based ontology, geometric part identification, and similarity matching for grasp planning, avoiding semantic vision models for better generalization.


<details>
  <summary>Details</summary>
Motivation: Current task-oriented grasping methods rely on vision-language models for part segmentation and grasp planning, but suffer from instability in part recognition and limited generalization across diverse objects and tasks.

Method: Three key components: 1) LLM-based object-part-task ontology for functional part selection from human commands, 2) Sampling-based geometric analysis for part identification from point clouds, 3) Similarity matching framework using known objects as references for grasp planning.

Result: High accuracy in functional part selection, identification, and grasp generation demonstrated through real-world experiments. The method shows generalization to novel-category objects by extending ontological knowledge.

Conclusion: The geometry-centric strategy provides more generalizable task-oriented grasping without relying on semantic visual features, overcoming viewpoint sensitivity and enabling adaptation to diverse objects and tasks.
...
### [228] [120 Minutes and a Laptop: Minimalist Image-goal Navigation via Unsupervised Exploration and Offline RL](https://arxiv.org/abs/2603.26441)
*Xiaoming Liu,Borong Zhang,Qingbiao Li,Steven Morad*

Main category: cs.RO

TL;DR: MINav enables rapid image-goal navigation policy development in under 2 hours on a consumer laptop without human intervention, using offline RL with unsupervised data collection and hindsight goal relabeling.


<details>
  <summary>Details</summary>
Motivation: Challenge the prevailing assumption that image-goal navigation requires large datasets, extensive pretraining, and significant computational resources. Demonstrate that effective robotic learning can be achieved with high computational efficiency.

Method: Formulates image-goal navigation as offline goal-conditioned reinforcement learning problem. Combines unsupervised data collection with hindsight goal relabeling and offline policy learning to train policies efficiently.

Result: MINav improves exploration efficiency, outperforms zero-shot navigation baselines in target environments, and scales favorably with dataset size. Successfully deployed in both simulation and real-world settings.

Conclusion: Effective real-world robotic learning can be achieved with high computational efficiency, lowering the barrier to rapid policy prototyping and deployment for image-goal navigation tasks.
...
### [229] [DTP-Attack: A decision-based black-box adversarial attack on trajectory prediction](https://arxiv.org/abs/2603.26462)
*Jiaxiang Li,Jun Yan,Daniel Watzenig,Huilin Yin*

Main category: cs.RO

TL;DR: DTP-Attack: A black-box adversarial attack framework for trajectory prediction systems that operates without model internals or gradients, achieving high success rates for both intention misclassification and prediction accuracy degradation attacks.


<details>
  <summary>Details</summary>
Motivation: Trajectory prediction systems are critical for autonomous vehicle safety but vulnerable to adversarial attacks. Existing methods require white-box access with gradients and rigid physical constraints, limiting real-world applicability.

Method: DTP-Attack uses a decision-based black-box framework with a novel boundary walking algorithm that navigates adversarial regions without fixed constraints, maintaining trajectory realism through proximity preservation. It operates on binary decision outputs only.

Result: Achieves 41-81% attack success rates for intention misclassification with perturbations below 0.45m, and increases prediction errors by 1.9-4.2× for accuracy degradation. Outperforms existing black-box approaches on nuScenes and Apolloscape datasets across models like Trajectron++ and Grip++.

Conclusion: The method reveals fundamental vulnerabilities in current trajectory prediction systems, demonstrating urgent need for robust defenses in safety-critical autonomous driving applications while maintaining high controllability and reliability.
...
### [230] [Adapt as You Say: Online Interactive Bimanual Skill Adaptation via Human Language Feedback](https://arxiv.org/abs/2603.26466)
*Zhuo Li,Dianxi Li,Tao Teng,Quentin Rouxel,Zhipeng Dong,Dennis Hong,Darwin Caldwell,Fei Chen*

Main category: cs.RO

TL;DR: BiSAIL enables zero-shot online adaptation of bimanual robot skills using interactive language feedback, outperforming existing methods in adaptability, generalization, and cross-embodiment scalability.


<details>
  <summary>Details</summary>
Motivation: Developing general-purpose robots for human environments requires adaptation to evolving task conditions, but adapting high-dimensional bimanual skills to novel variations at deployment remains challenging.

Method: Hierarchical reason-then-modulate paradigm: first infers adaptation objectives from multimodal task variations using language feedback, then adapts bimanual motions via diffusion modulation to achieve inferred objectives.

Result: Extensive real-robot experiments across six bimanual tasks and two dual-arm platforms show BiSAIL significantly outperforms existing methods in human-in-the-loop adaptability, task generalization, and cross-embodiment scalability.

Conclusion: Enables development of adaptive bimanual assistants that can be flexibly customized by non-expert users via intuitive verbal corrections, advancing toward general-purpose robots for human environments.
...
### [231] [Addressing Ambiguity in Imitation Learning through Product of Experts based Negative Feedback](https://arxiv.org/abs/2603.26467)
*John Bateman,Andy M. Tyrrell,Jihong Zhu*

Main category: cs.RO

TL;DR: A novel negative-feedback imitation learning system that learns from suboptimal demonstrations and its own failures, achieving 90% success rate improvement over positive-only imitation learning for ambiguous tasks.


<details>
  <summary>Details</summary>
Motivation: Real-world robotics applications often rely on suboptimal demonstrations from non-experts (like in home/assistive robotics), and traditional imitation learning assumes perfect expert demonstrations. There's a need for systems that can learn from imperfect data and improve through failure.

Method: A negative-feedback system that leverages suboptimal demonstrations and learns from its own failures. It combines user demonstrations with pretrained data and incorporates failure experiences to improve performance on ambiguous tasks.

Result: Achieved 90% improvement in success rate over positive-only imitation learning in simulations, and 50% improvement on a real robot. Also demonstrated higher efficacy, memory efficiency, and time efficiency compared to comparable negative feedback schemes.

Conclusion: The proposed negative-feedback imitation learning system effectively handles suboptimal demonstrations and ambiguous tasks by learning from failures, outperforming traditional positive-only approaches and other negative feedback methods in both simulation and real-robot experiments.
...
### [232] [The Multi-AMR Buffer Storage, Retrieval, and Reshuffling Problem: Exact and Heuristic Approaches](https://arxiv.org/abs/2603.26542)
*Max Disselnmeyer,Thomas Bömer,Laura Dörr,Bastian Amberg,Anne Meyer*

Main category: cs.RO

TL;DR: The paper introduces a hierarchical heuristic approach for coordinating robot fleets to manage buffer storage, retrieval, and reshuffling in dense floor storage environments, solving the NP-hard Multi-AMR BSRRP with significant computational efficiency gains.


<details>
  <summary>Details</summary>
Motivation: Manual operation in dense floor storage environments faces severe labor shortages and rising costs, while existing automation approaches don't handle real-world manufacturing needs for adaptive management of arriving unit loads alongside reshuffling and retrieval tasks.

Method: Proposes a hierarchical heuristic that decomposes the problem into: 1) A* search for task-level sequence planning of unit load placements, and 2) Constraint Programming (CP) for multi-robot coordination and scheduling. Also formulates a Binary Integer Programming model for benchmarking.

Result: The heuristic achieves orders-of-magnitude reduction in computation time compared to exact formulation, making it viable as responsive control logic for industrial-scale high-density production environments.

Conclusion: The proposed hierarchical heuristic provides a computationally efficient solution to the Multi-AMR BSRRP, enabling practical automation of buffer zones in space-constrained manufacturing facilities despite the NP-hard nature of the problem.
...
### [233] [Meta-Adaptive Beam Search Planning for Transformer-Based Reinforcement Learning Control of UAVs with Overhead Manipulators under Flight Disturbances](https://arxiv.org/abs/2603.26612)
*Hazim Alzorgan,Sayed Pedram Haeri Boroujeni,Abolfazl Razi*

Main category: cs.RO

TL;DR: RL framework with transformer-based DDQN and adaptive beam-search planner for aerial manipulators improves tracking accuracy and stability under drone drift.


<details>
  <summary>Details</summary>
Motivation: Drones with manipulators have motion coupling issues where drone attitude changes shift the end-effector, making reliable tracking difficult and limiting use of learning-based controllers designed for fixed-base robots.

Method: Reinforcement learning framework with transformer-based double deep Q learning (DDQN) and adaptive beam-search planner that uses short-horizon beam search over candidate control sequences with learned critic as forward estimator (software-in-the-loop approach).

Result: 10.2% reward increase, reduction in mean tracking error from ~6% to 3%, 29.6% improvement in combined reward-error metric, and maintains 5 cm tracking error under drone drift compared to baselines.

Conclusion: The meta-adaptive planner with transformer critic and DDQN backbone significantly improves aerial manipulator tracking performance and stability under external disturbances.
...
### [234] [Partial Motion Imitation for Learning Cart Pushing with Legged Manipulators](https://arxiv.org/abs/2603.26659)
*Mili Das,Morgan Byrd,Donghoon Baek,Sehoon Ha*

Main category: cs.RO

TL;DR: Partial imitation learning transfers locomotion skills to cart pushing tasks for legged robots, achieving stable and accurate loco-manipulation.


<details>
  <summary>Details</summary>
Motivation: Loco-manipulation is crucial for real-world mobile manipulation tasks but challenging due to the difficulty of maintaining stable locomotion while performing precise manipulation. Current methods struggle with robust skill learning.

Method: Partial imitation learning approach: 1) Train robust locomotion policy with extensive domain/terrain randomization, 2) Learn loco-manipulation policy by imitating only lower-body motions using partial adversarial motion prior.

Result: Learned policy successfully pushes cart along diverse trajectories in IsaacLab and transfers effectively to MuJoCo. Outperforms baselines with more stable and accurate loco-manipulation behaviors.

Conclusion: Partial imitation learning effectively transfers locomotion skills to loco-manipulation tasks, enabling legged robots to perform robust cart pushing in diverse environments.
...
### [235] [Ruka-v2: Tendon Driven Open-Source Dexterous Hand with Wrist and Abduction for Robot Learning](https://arxiv.org/abs/2603.26660)
...
TL;DR: Ruka-v2 is an upgraded open-source tendon-driven humanoid hand with added wrist mobility and finger abduction/adduction, significantly improving dexterity and task performance over the original Ruka.


<details>
  <summary>Details</summary>
Motivation: The original Ruka hand lacked essential degrees of freedom for closely imitating human behavior - specifically wrist mobility and finger adduction/abduction, which limited its dexterity and manipulation capabilities.

Method: Developed Ruka-v2 with a decoupled 2-DOF parallel wrist (flexion/extension and radial/ulnar deviation) and added abduction/adduction at the fingers. The design remains fully open-sourced, tendon-driven, and affordable.

Result: Compared to Ruka, Ruka-v2 achieved a 51.3% reduction in completion time and 21.2% increase in success rate in teleoperated tasks. Demonstrated capabilities across 13 dexterous tasks via teleoperation and autonomous policy learning on 3 tasks.

Conclusion: Ruka-v2 significantly advances accessible dexterous robotics hardware by adding crucial degrees of freedom, enabling more human-like manipulation in confined spaces and complex tasks, while maintaining open-source accessibility.
...
### [236] [VLA-OPD: Bridging Offline SFT and Online RL for Vision-Language-Action Models via On-Policy Distillation](https://arxiv.org/abs/2603.26666)
*Zhide Zhong,Haodong Yan,Junfeng Li,Junjie He,Tianran Zhang,Haoang Li*

Main category: cs.RO

TL;DR: VLA-OPD bridges SFT efficiency with RL robustness using expert teacher supervision on self-generated trajectories, preventing catastrophic forgetting while improving sample efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard offline SFT suffers from distribution shifts and catastrophic forgetting, while online RL struggles with sparse rewards and poor sample efficiency. There's a need for a framework that combines the strengths of both approaches for reliable VLA model deployment.

Method: Proposes On-Policy VLA Distillation (VLA-OPD) that uses an expert teacher to provide dense, token-level supervision on student's self-generated trajectories. Formulates it via a Reverse-KL objective to avoid entropy explosion (Forward-KL) and premature entropy collapse (Hard-CE), ensuring stable policy learning while maintaining action diversity.

Result: Experiments on LIBERO and RoboTwin2.0 benchmarks show VLA-OPD significantly improves sample efficiency over RL and robustness over SFT, while effectively mitigating catastrophic forgetting during post-training.

Conclusion: VLA-OPD successfully bridges the gap between SFT efficiency and RL robustness, enabling reliable post-training of VLA models without sacrificing pre-trained capabilities or requiring extensive environmental rewards.
