...
TL;DR: Monocular depth estimation for robotic surgery using synthetic priors from Depth Anything V2 with DV-LORA adaptation, achieving SOTA results on SCARED dataset with improved robustness in specular environments.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised monocular depth estimation methods for robotic surgery are fragile in specular, fluid-filled endoscopic environments and suffer from boundary collapse on thin surgical tools and transparent surfaces.

Method: Leverage high-fidelity synthetic priors from Depth Anything V2 architecture, adapt to medical domain using Dynamic Vector Low-Rank Adaptation (DV-LORA) to minimize parameters while bridging synthetic-to-real gap, and introduce physically-stratified evaluation protocol on SCARED dataset.

Result: Achieves new state-of-the-art with accuracy (< 1.25) of 98.1% and reduces Squared Relative Error by over 17% compared to established baselines, demonstrating superior robustness in adverse surgical lighting.

Conclusion: The approach successfully addresses monocular depth estimation challenges in surgical environments by combining synthetic priors with efficient domain adaptation, providing robust performance in high-specularity regimes that are critical for robotic surgery.
...
<details>
  <summary>Details</summary>
Motivation: Current urban warfare training assessment methods rely on costly sensors or subjective human observation, limiting scalability and accuracy for evaluating cognitive, psychomotor, and teamwork skills in Synthetic Training Environments.
...
Result: Demonstrated approach with real-world Enter and Clear the Room drills, providing actionable domain-specific metrics capturing individual and team performance. Supports After Action Reviews with interactive dashboards in Gamemaster and GIFT.

Conclusion: Video-based assessment enables scalable evaluation in Synthetic Training Environments. Limitations include tracking difficulties and ground-truth validation. Future work includes 3D video analysis expansion.
...
<details>
  <summary>Details</summary>
Motivation: To enable efficient long-term memory in video models by compressing long videos into short contexts while preserving important visual details, reducing computational costs for autoregressive video generation.

Method: PFP uses a neural network structure with explicit pretraining objective to preserve high-frequency details of single frames at arbitrary temporal positions. It compresses 20-second videos into ~5k length contexts and can be fine-tuned as memory encoders for autoregressive video models.

Result: The model successfully compresses videos while preserving perceptual appearance of random frames. When fine-tuned as memory encoders, it enables long history memory with low context cost and relatively low fidelity loss.

Conclusion: PFP provides an effective framework for video compression that balances context length reduction with detail preservation, offering practical trade-offs for neural architecture designs in long-term video memory applications.
...
TL;DR: Proposes lifelong domain adaptation for 3D human pose estimation to handle non-stationary target datasets, using a novel GAN framework with 3D pose generators and domain-aware knowledge integration.


<details>
  <summary>Details</summary>
Motivation: Current 3D HPE methods rely on annotated 3D data from controlled environments, limiting generalization to diverse real-world scenarios. Existing domain adaptation approaches overlook non-stationary target pose datasets and suffer from catastrophic forgetting when adapting to multiple domains sequentially.

Method: Introduces lifelong domain adaptation for 3D HPE with a GAN framework containing 3D pose generators, 2D pose discriminator, and 3D pose estimator. Uses pose-aware, temporal-aware, and domain-aware knowledge integration to enhance current domain adaptation while alleviating catastrophic forgetting of previous domains.
...
Conclusion: The proposed lifelong domain adaptation framework successfully addresses challenges in 3D HPE by handling non-stationary target datasets while preventing catastrophic forgetting, representing the first application of lifelong domain adaptation to 3D human pose estimation.
...
TL;DR: Deep learning pipeline transforms pediatric T1-weighted MRI into synthetic CTs for cranial bone segmentation and suture analysis, enabling non-invasive evaluation of cranial development without radiation exposure.


<details>
  <summary>Details</summary>
Motivation: CT scans provide crucial cranial and sutural deformity evaluation but expose children to harmful ionizing radiation. MRI is radiation-free but cannot visualize cranial sutures, estimate skull bone density, or assess cranial vault growth, creating a critical gap in non-invasive pediatric cranial assessment.

Method: Proposes a deep learning pipeline that transforms T1-weighted MRIs of children (0.2-2 years) into synthetic CTs (sCTs). The pipeline predicts detailed cranial bone segmentation, generates suture probability heatmaps, and derives direct suture segmentation from these heatmaps using robust, domain-specific variational autoencoders.

Result: sCTs achieved 99% structural similarity and Frechet inception distance of 1.01 relative to real CTs. Skull segmentation attained average Dice coefficient of 85% across seven cranial bones, and sutures achieved 80% Dice. Equivalence between sCTs and real CTs was confirmed using two one-sided tests (TOST p < 0.05).

Conclusion: First pediatric cranial CT synthesis framework enabling suture segmentation on sCTs derived from MRI, bridging critical gaps in non-invasive cranial evaluation. The method generates perceptually indistinguishable cranial sCTs from routine pediatric MRIs, overcoming MRI's limitations in bone and suture depiction.
...
<details>
  <summary>Details</summary>
Motivation: Current scaling laws for AI models are well-established for natural images with abundant internet data, but poorly understood for high-value domains like remote sensing where domain-specialized encoders are crucial for multimodal applications like image captioning, search, and reasoning.

Method: Used over a quadrillion pixels of commercial satellite EO data and the MITRE Federal AI Sandbox to train progressively larger vision transformer (ViT) backbones, analyzing scaling behaviors and reporting success/failure modes at petascale.

Result: Even at massive scale (petascale), performance remains consistent with a data-limited regime rather than model parameter-limited one, with practical insights for bridging domain gaps across additional remote sensing modalities.

Conclusion: The findings provide practical guidance for data-collection strategies, compute budgets, and optimization schedules to advance frontier-scale remote sensing foundation models, addressing the unique scaling challenges in this domain.
...
TL;DR: A novel bi-level optimization framework for automated ultrasound skill assessment that predicts skills based on task performance quality without requiring manual skill ratings.


<details>
  <summary>Details</summary>
Motivation: Traditional ultrasound skill assessment is subjective and time-consuming, while existing automated methods rely on supervised learning with predetermined skill factors, limiting their flexibility and objectivity.

Method: A bi-level optimization framework with two jointly optimized networks: a clinical task predictor and a skill predictor. The framework assesses skills by how well tasks are performed on acquired ultrasound images, without using manually predefined skill ratings.

Result: Validated on real-world clinical ultrasound videos of fetal head scanning, demonstrating feasibility of predicting ultrasound skills by quantifying optimized task performance as a skill indicator.

Conclusion: The proposed framework offers an objective, automated approach to ultrasound skill assessment that eliminates subjectivity and reliance on predefined skill ratings, showing promise for clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: In clinical practice, multimodal MRI data for brain tumor segmentation is often incomplete, making it challenging to fully utilize available information. There's a need for methods that can effectively leverage incomplete multimodal data.

Method: Proposes MGML framework with two components: 1) Meta-parameterized adaptive modality fusion (Meta-AMF) that generates adaptive soft-label supervision based on available modalities, and 2) Consistency regularization module that enhances segmentation performance and framework robustness. The approach doesn't alter original model architecture and integrates into training pipelines.
...
Conclusion: The MGML framework effectively handles incomplete multimodal MRI data for brain tumor segmentation through adaptive fusion and consistency regularization, achieving state-of-the-art performance without modifying model architectures.
...
TL;DR: A novel cross-view geo-localization system using DINOv2 backbone with convolution adapter, multi-scale channel reallocation, and MoE-enhanced aggregation to address view-point discrepancies.
...
TL;DR: AI framework for automated assessment of microanastomosis surgical skills using computer vision and machine learning for objective performance evaluation.


<details>
  <summary>Details</summary>
Motivation: Microanastomosis requires precise surgical skills, but current assessment methods are subjective, time-consuming, and inconsistent. There's a need for automated, scalable solutions to provide objective feedback for surgical training.

Method: Three-component AI framework: (1) YOLO+DeepSORT for instrument tip tracking, (2) self-similarity matrix for unsupervised action segmentation and boundary detection, (3) supervised classification for skill proficiency evaluation. Designed for edge computing platforms.
...
### [11] [F2IDiff: Real-world Image Super-resolution using Feature to Image Diffusion Foundation Model](https://arxiv.org/abs/2512.24473)
*Devendra K. Jangid,Ripon K. Saha,Dilshan Godaliyadda,Jing Li,Seok-Jun Lee,Hamid R. Sheikh*

Main category: cs.CV

TL;DR: The paper introduces a Feature-to-Image Diffusion (F2IDiff) Foundation Model for smartphone SISR that uses DINOv2 features instead of text features to provide stricter, hallucination-free conditioning for high-fidelity consumer photography.


<details>
  <summary>Details</summary>
Motivation: Current T2IDiff-based SISR models cause undesirable hallucinations in smartphone photography where LR images already have high fidelity. Text features are too high-level for subtle textures and can't accurately describe small patches needed for high-resolution smartphone images.

Method: Proposes a Feature-to-Image Diffusion Foundation Model that uses DINOv2 features for conditioning instead of text. DINOv2 provides lower-level, richer features that offer stricter conditioning while accurately describing small image patches.

Result: The F2IDiff model addresses the shortcomings of T2IDiff models by providing hallucination-free generation suitable for consumer photography where minimal generation is needed and high fidelity is required.

Conclusion: Using lower-level DINOv2 features for conditioning in diffusion-based SISR enables better performance for smartphone photography by providing stricter, more accurate conditioning that prevents hallucinations while maintaining high fidelity.
...
TL;DR: DehazeSNN: A U-Net-like Spiking Neural Network architecture for image dehazing that combines multi-scale feature capture with efficient long-range dependency modeling using OLIFBlocks, achieving state-of-the-art performance with reduced computational cost.


<details>
  <summary>Details</summary>
Motivation: Traditional dehazing methods rely on atmospheric scattering models, while modern deep learning approaches (CNNs and Transformers) have limitations: CNNs struggle with long-range dependencies, and Transformers require heavy computational resources. There's a need for an efficient architecture that can handle both local and long-range dependencies in image dehazing tasks.

Method: Propose DehazeSNN, a U-Net-like architecture integrating Spiking Neural Networks (SNNs). The key innovation is the Orthogonal Leaky-Integrate-and-Fire Block (OLIFBlock) that enhances cross-channel communication. The architecture captures multi-scale image features while efficiently managing both local and long-range dependencies with reduced computational burden.

Result: Extensive experiments show DehazeSNN is highly competitive with state-of-the-art methods on benchmark datasets. It delivers high-quality haze-free images with smaller model size and fewer multiply-accumulate operations compared to existing approaches.

Conclusion: DehazeSNN successfully addresses the limitations of both CNNs and Transformers for image dehazing by combining SNNs with a U-Net architecture and OLIFBlocks, achieving excellent performance with computational efficiency. The method is publicly available as open-source software.
...
TL;DR: T2VAttack: First comprehensive study of adversarial attacks on Text-to-Video diffusion models, revealing vulnerabilities through semantic and temporal attack objectives using prompt modification strategies.
...
Method: Proposes T2VAttack framework with two attack objectives (semantic for video-text alignment, temporal for temporal dynamics) and two attack methods: T2VAttack-S (replaces critical words with synonyms via greedy search) and T2VAttack-I (iteratively inserts optimized words with minimal prompt perturbation).
...
Conclusion: Current T2V diffusion models have critical vulnerabilities to adversarial attacks through prompt manipulation, highlighting urgent need for improved robustness in video generation systems.
...
Method: 1) Estimate global static and per-frame dynamic point clouds from images/camera poses; 2) Use deformable 4D Gaussian framework for scene reconstruction; 3) Train video diffusion model on degraded renders from initial 4D Gaussian model; 4) Iteratively refine progressively shifted Gaussian renderings using diffusion model; 5) Incorporate enhanced results back into 4DGS training until reaching target viewpoints.
...
Conclusion: This work presents a practical view extrapolation solution for autonomous driving that reduces dependency on expensive sensors and annotations while achieving superior image quality through an iterative refinement process between 4D Gaussian reconstruction and diffusion models.
...
TL;DR: Deep learning inpainting model detects surface changes from satellite time series by predicting what surfaces should look like, achieving 3x better sensitivity than traditional methods for earthquake rupture detection.
...
Result: Method detects earthquake-triggered surface ruptures from 2023 Turkey-Syria earthquake with higher sensitivity and specificity than temporal median or Reed-Xiaoli anomaly detectors, achieving detection thresholds approximately three times lower than baseline approaches.

Conclusion: Deep learning temporal redundancy approach enables unprecedented sensitivity for surface change detection, providing a path toward automated, global-scale monitoring using freely available multi-spectral satellite data.
...
TL;DR: GCA-ResUNet: A lightweight CNN with Grouped Coordinate Attention module for efficient medical image segmentation, achieving better accuracy than Transformers with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: U-Net CNNs struggle with long-range dependencies in multi-organ segmentation, especially in low-contrast regions. Transformers address this but require high computational resources and large datasets, making them impractical for clinical deployment.

Method: Proposes GCA-ResUNet with a plug-and-play Grouped Coordinate Attention module that: 1) decouples channel-wise context into groups to handle semantic heterogeneity, and 2) uses direction-aware coordinate encoding to capture spatial dependencies along horizontal/vertical axes.

Result: Achieves Dice scores of 86.11% on Synapse and 92.64% on ACDC benchmarks, outperforming both CNN and Transformer methods including Swin-UNet and TransUNet. Particularly effective for small anatomical structures with complex boundaries.

Conclusion: GCA-ResUNet provides an optimal trade-off between accuracy and efficiency, offering a practical, scalable solution for clinical deployment in resource-constrained environments.
...
TL;DR: GASeg is a self-supervised semantic segmentation framework that bridges appearance and geometry using topological information to overcome appearance ambiguities like shadows and glare.


<details>
  <summary>Details</summary>
Motivation: Current self-supervised semantic segmentation methods fail with appearance ambiguities (shadows, glare, textures) due to over-reliance on unstable appearance-based features. There's a need for more stable representations that incorporate geometric information.

Method: 1) Differentiable Box-Counting (DBC) module extracts multi-scale topological statistics from geometric and appearance feature streams. 2) Topological Augmentation (TopoAug) applies adversarial morphological operators to simulate real-world ambiguities. 3) GALoss enforces cross-modal alignment between geometric and appearance features.

Result: Achieves state-of-the-art performance on four benchmarks: COCO-Stuff, Cityscapes, and PASCAL, validating the effectiveness of bridging geometry and appearance via topological information.

Conclusion: Bridging appearance and geometry through topological information provides stable representations for self-supervised semantic segmentation, overcoming limitations of appearance-only approaches and handling real-world ambiguities effectively.
...
TL;DR: A pipeline for 3D reconstruction of spacecraft using 3D Gaussian Splatting with sun position priors to handle dynamic space lighting.


<details>
  <summary>Details</summary>
Motivation: Spacecraft RPO requires accurate 3D reconstruction from images, but standard 3DGS assumes static scenes while space has dynamic lighting. Photometric accuracy is crucial for downstream pose estimation tasks.

Method: Uses 3D Gaussian Splatting model with incorporation of Sun position priors (estimated by servicer spacecraft) into training pipeline to improve photometric quality and handle changing illumination.

Result: Experimental studies show 3DGS models learn to adapt to rapidly changing space illumination conditions and reflect global shadowing and self-occlusion effectively.

Conclusion: The proposed solution successfully incorporates sun position knowledge to improve photometric accuracy of 3DGS for spacecraft reconstruction, enabling better handling of dynamic space lighting for RPO applications.
...
<details>
  <summary>Details</summary>
Motivation: VLMs show promise for medical diagnosis but struggle with 3D multimodal image processing, particularly in integrating complementary information and detecting subtle pathological features in complex medical images.

Method: Two-stage fusion framework: 1) HilbertMed-SAM module with redesigned SAM2 architecture using Hilbert space-filling curves in Mamba SSM scanning to preserve 3D spatial locality, plus Hilbert-Mamba Cross-Attention and scale-aware decoder for lesion segmentation; 2) Prompt enhancement module that unifies segmentation masks with textual attributes to guide VLM for disease classification.

Result: Achieved 82.35% Dice score on BraTS2021 segmentation benchmark and 78.85% diagnostic classification accuracy, demonstrating improved performance for medical VLM-based analysis.

Conclusion: Hilbert-VLM offers substantial potential to improve accuracy and reliability of medical VLM-based analysis by effectively handling 3D multimodal medical images through novel architectural innovations and two-stage fusion approach.
...
TL;DR: CVC is a flow-based diffusion editing framework that corrects velocity errors to maintain structural fidelity and semantic consistency during image distribution transformations.
...
Method: Introduces Conditioned Velocity Correction (CVC) with dual-perspective velocity conversion: structure-preserving branch maintains source trajectory consistency, and semantically-guided branch drives controlled deviation toward target distribution. Uses posterior-consistent updates derived from Empirical Bayes Inference and Tweedie correction to compensate for velocity errors.

Result: CVC achieves stable and interpretable latent dynamics with faithful reconstruction and smooth local semantic conversion. Experiments show superior fidelity, better semantic alignment, and more reliable editing behavior across diverse tasks.

Conclusion: CVC provides a principled framework for flow-based editing that addresses velocity error accumulation through mathematical grounding, enabling more consistent and faithful image distribution transformations.
...
TL;DR: FitControler is a learnable plug-in module for virtual try-on models that enables customized garment fit control by generating fit-aware layouts and injecting them into existing VTON pipelines.


<details>
  <summary>Details</summary>
Motivation: Current VTON methods focus on faithful garment detail rendering but neglect garment fit - how garments align with the wearer's body - which is crucial for holistic style coordination and fashion design.

Method: FitControler features: 1) fit-aware layout generator that redraws body-garment layouts using garment-agnostic representations, and 2) multi-scale fit injector that delivers layout cues for layout-driven VTON. The system is trained on Fit4Men dataset with 13,000 body-garment pairs of different fits.

Result: Extensive experiments show FitControler works with various VTON models and achieves accurate fit control. The paper introduces two fit consistency metrics for evaluation and will release code and data.

Conclusion: FitControler successfully addresses the gap in fit-aware VTON by providing a learnable plug-in that enables customized garment fit control while maintaining compatibility with existing VTON models.
...
<details>
  <summary>Details</summary>
Motivation: Existing 2D Gaussian Splatting pipelines allocate representation capacity and parameter precision without considering image structure, limiting rate-distortion efficiency at low bitrates.

Method: Three key innovations: 1) Structure-guided initialization that assigns 2D Gaussians based on spatial structural priors; 2) Adaptive bitwidth quantization of covariance parameters that gives higher precision to small-scale Gaussians in complex regions; 3) Geometry-consistent regularization that aligns Gaussian orientations with local gradient directions.

Result: The approach substantially improves representational power and RD performance while maintaining over 1000 FPS decoding. Achieves 43.44% BD-rate reduction on Kodak and 29.91% on DIV2K compared to baseline GSImage.

Conclusion: Structure-guided allocation principle effectively couples image structure with representation capacity and quantization precision, enabling significant improvements in 2D Gaussian Splatting compression efficiency without sacrificing decoding speed.
...
TL;DR: MF-RSVLM is a multi-feature fusion vision-language model for remote sensing that addresses visual forgetting and fine-grained feature extraction challenges through multi-scale feature learning and recurrent visual injection.
...
Result: Achieves state-of-the-art or highly competitive performance across diverse RS benchmarks including classification, image captioning, and VQA tasks.
...
TL;DR: RSAgent is an agentic MLLM that performs text-guided segmentation through multi-turn reasoning and tool invocations, achieving state-of-the-art performance on segmentation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing text-guided segmentation methods treat it as one-shot grounding, which limits verification, refocusing and refinement when initial localization is wrong. There's a need for iterative reasoning and refinement capabilities.
...
Conclusion: RSAgent's agentic approach with multi-turn reasoning and tool invocations enables more accurate and robust text-guided segmentation through iterative refinement, outperforming existing one-shot methods.
...
Method: Three key innovations: 1) Motion analysis using SSIM and Optical Flow to identify and skip editing of low-motion frames, 2) Pipelined task scheduling that splits videos into segments for parallel DDIM inversion and joint editing, 3) Neural network-based interpolation to smooth border frames between segments and interpolate skipped frames.

Result: PipeFlow achieves up to 9.6X speedup compared to TokenFlow and 31.7X speedup over Diffusion Motion Transfer (DMT), with editing time scaling linearly with video length instead of exponentially.

Conclusion: PipeFlow enables scalable editing of infinitely long videos by dividing them into smaller segments, eliminating the growing per-frame computational overhead of existing methods through parallel processing and intelligent frame skipping.
...
Result: The proposed denoiser outperforms existing diffusion-based methods and competes with representative deep CNN-based methods when applied to removing three types of common noise.

Conclusion: Reinforcement learning enables adaptive anisotropic diffusion that combines the strengths of traditional diffusion methods with learning-based approaches, achieving competitive denoising performance.
...
TL;DR: NIRNL is a robust cross-modal retrieval framework that handles noisy labels by using cross-modal margin preserving for better discrimination and neighbor-aware instance refining to partition data into pure, hard, and noisy subsets with tailored optimization strategies.


<details>
  <summary>Details</summary>
Motivation: Cross-modal retrieval suffers from noisy labels in multi-modal data collection, which degrades performance. Existing robust methods fail to simultaneously achieve high model performance, calibration reliability, and data utilization rates.
...
Conclusion: NIRNL effectively addresses noisy label problems in cross-modal retrieval by maximizing data utilization while mitigating error propagation, outperforming existing methods in robustness and performance.
...
TL;DR: PCRNet: A novel deep learning framework for ocular disease recognition that incorporates pathology context and expert experience priors through specialized modules (PRM and EPGA) with integrated loss function, achieving state-of-the-art performance.
...
Method: Developed Pathology Recalibration Module (PRM) to leverage pathology context via pixel-wise context compression and distribution concentration operators. Created Expert Prior Guidance Adapter (EPGA) to highlight significant pixel-wise regions using expert experience. Combined these with Integrated Loss (IL) considering sample-wise loss distributions and training label frequencies.

Result: PCRNet with IL outperforms state-of-the-art attention-based networks and advanced loss methods on three ocular disease datasets. Visualization analysis explains how PRM and EPGA affect DNN decision-making processes.

Conclusion: The proposed PCRNet effectively incorporates clinical pathology context and expert experience priors to enhance ocular disease recognition performance and interpretability, demonstrating the importance of integrating domain knowledge into medical AI systems.
...
TL;DR: Proposes balanced hierarchical contrastive loss with decoupled learning in DETR to address data imbalance and localization interference in fine-grained remote sensing detection.


<details>
  <summary>Details</summary>
Motivation: Fine-grained remote sensing datasets use hierarchical labels, but embedding semantic hierarchy into representation learning is challenging. Previous contrastive learning approaches overlook data imbalance across hierarchy (high-frequency classes dominate) and interference between semantic relationships and class-agnostic localization.

Method: Proposes balanced hierarchical contrastive loss with learnable class prototypes that equilibrates gradients from different classes at each hierarchical level. Uses decoupled learning strategy that separates DETR's object queries into classification and localization sets for task-specific feature extraction.

Result: Experiments on three fine-grained datasets with hierarchical annotations show the method outperforms state-of-the-art approaches.

Conclusion: The proposed balanced hierarchical contrastive loss with decoupled learning effectively addresses data imbalance and localization interference in fine-grained object detection with hierarchical labels.
...
<details>
  <summary>Details</summary>
Motivation: DiT models have extremely high computational costs due to attention mechanisms, limiting practical applications. Existing sparse attention methods have overhead issues and lack hardware generality, being mostly GPU-focused.
...
TL;DR: D²VLM: A factorized learning framework that decouples temporal grounding and textual response tasks for video-language models, using evidence tokens and factorized preference optimization to improve event-level perception.
...
Method: Proposes D²VLM framework with: 1) "Grounding then answering with evidence referencing" paradigm using evidence tokens for event-level visual semantic capture; 2) Factorized Preference Optimization (FPO) algorithm that explicitly incorporates probabilistic temporal grounding modeling into optimization; 3) A synthetic dataset for factorized preference learning with explicit temporal grounding.

Result: Experiments on various tasks demonstrate clear advantages of the approach, showing improved performance in temporal grounding and textual response for video understanding.

Conclusion: Factorized learning that decouples temporal grounding and textual response tasks, while maintaining their inherent dependency, leads to better video-language understanding through the proposed D²VLM framework and FPO algorithm.
...
<details>
  <summary>Details</summary>
Motivation: Current Text-to-Motion generation treats it as direct translation from language to poses, but faces a fundamental bottleneck called Semantic-Kinematic Impedance Mismatch - the difficulty of grounding semantically dense language into kinematically dense motion data in a single shot.

Method: Proposes Latent Motion Reasoning (LMR) with a Dual-Granularity Tokenizer that disentangles motion into two manifolds: Reasoning Latent for planning global topology and Execution Latent for preserving physical fidelity. The model performs autoregressive reasoning (planning) before execution (frame instantiation).

Result: LMR yields non-trivial improvements in both semantic alignment and physical plausibility when implemented on two representative baselines (T2M-GPT and MotionStreamer), demonstrating that motion-aligned concept spaces are better than natural language for motion planning.

Conclusion: The optimal substrate for motion planning is not natural language but a learned, motion-aligned concept space. The architectural shift to Latent System 2 Reasoning effectively bridges the gap between language and physics.
...
<details>
  <summary>Details</summary>
Motivation: Monocular Depth Estimation (MDE) is critical for autonomous driving but vulnerable to adversarial attacks. Existing physical attacks use unrealistic texture patches with strict placement constraints, limiting effectiveness in complex driving environments.

Method: Proposes a training-free generative adversarial attack framework with: 1) Salient Region Selection module to identify MDE-influential regions, and 2) Jacobian Vector Product Guidance mechanism to steer adversarial gradients toward directions supported by pre-trained diffusion models, enabling generation of physically plausible adversarial objects.

Result: Extensive digital and physical experiments show the method significantly outperforms existing attacks in effectiveness, stealthiness, and physical deployability, inducing substantial adversarial depth shifts.

Conclusion: The framework generates naturalistic, scene-consistent adversarial objects via diffusion-based conditional generation, demonstrating strong practical implications for autonomous driving safety assessment by exposing vulnerabilities in current MDE systems.
...
Result: Reasoning models like OpenAI-o3 outperform general MLLMs, but performance declines significantly with increasing task complexity. Sub-goal decomposition and irrelevant premise filtering critically influence final problem-solving accuracy, while Chain-of-Thought prompting unexpectedly degrades performance in some tasks.

Conclusion: GeoBench establishes a comprehensive benchmark for geometric problem-solving while offering actionable guidelines for developing geometric reasoning systems, highlighting the importance of hierarchical reasoning assessment.
...
<details>
  <summary>Details</summary>
Motivation: Automated neural architecture design is challenging due to task diversity and computational constraints. LLMs offer a promising alternative to expensive NAS methods, but their application to computer vision architecture generation lacks systematic study regarding prompt engineering and validation strategies.

Method: 1) Introduces Few-Shot Architecture Prompting (FSAP) to systematically study the optimal number of supporting examples (n=1-6) for LLM-based architecture generation. 2) Proposes Whitespace-Normalized Hash Validation for lightweight deduplication of generated architectures. 3) Uses dataset-balanced evaluation methodology for comparing architectures across heterogeneous vision tasks.

Result: Found that n=3 examples best balances architectural diversity and context focus for vision tasks. The deduplication method provides 100x speedup over AST parsing. Generated 1,900 unique architectures across 7 computer vision benchmarks (MNIST, CIFAR-10, CIFAR-100, CelebA, ImageNette, SVHN, Places365).
...
<details>
  <summary>Details</summary>
Motivation: Existing RLHF methods for text-to-image diffusion models suffer from Preference Mode Collapse - models converge to narrow, high-scoring outputs (monolithic styles, overexposure) that degrade generative diversity despite high automated reward scores.

Method: Proposes Directional Decoupling Alignment (D²-Align): learns directional correction in reward model's embedding space while keeping model frozen, then applies correction to reward signal during optimization to prevent mode collapse.
...
Conclusion: The proposed D²-Align framework effectively mitigates Preference Mode Collapse by correcting reward model biases, enabling text-to-image diffusion models to maintain generative diversity while aligning with human preferences.
...
TL;DR: IMDD-1M is the first large-scale industrial multimodal defect dataset with 1M image-text pairs covering 60+ materials and 400+ defect types, enabling various applications. A diffusion-based vision-language foundation model trained on this dataset achieves comparable performance to expert models with only 5% of task-specific data.


<details>
  <summary>Details</summary>
Motivation: There's a need for large-scale multimodal datasets specifically for industrial defect detection to advance multimodal learning in manufacturing and quality inspection. Current datasets lack the scale, diversity, and expert annotations needed for robust industrial applications.

Method: Created IMDD-1M dataset with 1M aligned image-text pairs covering 60+ material categories and 400+ defect types, each with expert-verified annotations and detailed textual descriptions. Trained a diffusion-based vision-language foundation model from scratch on this dataset, designed for industrial scenarios.

Result: The foundation model serves as a generalizable base that can be efficiently adapted to specialized domains through lightweight fine-tuning. With less than 5% of task-specific data required by dedicated expert models, it achieves comparable performance across various industrial applications.

Conclusion: IMDD-1M enables scalable, domain-adaptive, and knowledge-grounded manufacturing intelligence. The data-efficient foundation model adaptation approach paves the way for practical industrial inspection and generation systems that require minimal task-specific data.
...
Result: BSD consistently improves test accuracy (+1.4% for ResNet-50 on CIFAR-100) and reduces Expected Calibration Error (-40% ResNet-50, CIFAR-100) across architectures/datasets. Also improves robustness against data corruptions, perturbations, and label noise.

Conclusion: BSD provides a principled approach to self-distillation that eliminates hard target dependence, achieving superior accuracy, calibration, and robustness compared to existing methods, with state-of-the-art label noise robustness when combined with contrastive loss.
...
TL;DR: DiffThinker introduces a diffusion-based generative multimodal reasoning framework that reformulates vision-centric tasks as image-to-image generation, outperforming text-centric MLLMs in complex visual reasoning tasks.
...
Method: DiffThinker establishes a Generative Multimodal Reasoning paradigm using diffusion-based framework that treats multimodal reasoning as native image-to-image generation tasks, emphasizing logical consistency and spatial precision.

Result: DiffThinker significantly outperforms leading models: +314.2% over GPT-5, +111.6% over Gemini-3-Flash, and +39.0% over fine-tuned Qwen3-VL-32B across sequential planning, combinatorial optimization, constraint satisfaction, and spatial configuration domains.
...
TL;DR: DGC is a memory-efficient HSI segmentation framework that learns global clustering from local patches without pre-training, enabling fast training on consumer hardware but suffering from optimization instability due to loss balancing issues.
...
Method: Deep Global Clustering (DGC) framework operates on small patches with overlapping regions to enforce consistency, learning global clustering structure from local patch observations without pre-training, maintaining constant memory usage and enabling training in under 30 minutes on consumer hardware.

Result: On a leaf disease dataset, DGC achieves background-tissue separation with mean IoU 0.925 and demonstrates unsupervised disease detection through navigable semantic granularity, but suffers from optimization instability due to cluster over-merging in feature space from multi-objective loss balancing issues.

Conclusion: DGC presents a promising design philosophy for memory-efficient HSI segmentation, but requires principled approaches to dynamic loss balancing for stable implementation, positioning this work as intellectual scaffolding rather than a fully optimized solution.
...
TL;DR: Internal Guidance (IG) improves diffusion models by adding auxiliary supervision during training and extrapolating intermediate layer outputs during sampling, achieving state-of-the-art FID scores.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion models struggle with low-probability areas due to insufficient training data coverage. Existing guidance methods like CFG cause over-simplified/distorted samples, while other approaches require complex degradation strategies, extra training, or additional sampling steps.
...
Result: Significant improvements in training efficiency and generation quality across various baselines. SiT-XL/2+IG achieves FID=5.31 (80 epochs) and 1.75 (800 epochs) on ImageNet 256x256. LightningDiT-XL/1+IG achieves FID=1.34, and combined with CFG reaches state-of-the-art FID=1.19.

Conclusion: Internal Guidance is a simple yet effective strategy that enhances diffusion models without complex modifications, achieving superior generation quality and efficiency compared to existing guidance methods.
...
TL;DR: PointRAFT is a high-throughput point cloud regression network that directly predicts potato tuber weight from partial RGB-D camera data, overcoming occlusion issues without full 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: Potato yield estimation using RGB-D cameras on harvesters suffers from systematic underestimation due to self-occlusion in point clouds, requiring a method that can accurately predict weight from incomplete 3D data.

Method: PointRAFT uses a novel object height embedding to incorporate tuber height as geometric context, directly regressing continuous 3D shape properties (like weight) from partial point clouds without full reconstruction.

Result: Achieved MAE of 12.0g and RMSE of 17.2g on test set of 5,254 point clouds, outperforming linear regression and PointNet++ baselines, with 6.3ms inference time enabling 150 tubers/second processing.
...
TL;DR: CorGi is a training-free DiT inference acceleration framework that reduces redundant computation by caching and reusing low-contribution transformer blocks across denoising steps, achieving up to 2.0x speedup while preserving generation quality.


<details>
  <summary>Details</summary>
Motivation: Diffusion transformers (DiT) have high inference costs due to their iterative denoising process and large capacity, with substantial redundant computation across steps that needs to be reduced.

Method: CorGi selectively reuses outputs of transformer blocks across denoising steps by caching low-contribution blocks and reusing them within intervals. CorGi+ extends this for text-to-image tasks by using per-block cross-attention maps to identify salient tokens and applying partial attention updates.
...
Conclusion: CorGi provides an effective training-free approach to accelerate DiT inference by reducing redundant computation through selective block caching and reuse, with CorGi+ offering enhanced performance for text-to-image generation.
...
TL;DR: Proposes a progressive GAN with weighted synthetic data combination and meta-heuristic optimization for COVID-19 chest X-ray classification on imbalanced datasets, achieving 95.5% (4-class) and 98.5% (2-class) accuracy.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of imbalanced medical image data, particularly during pandemics like COVID-19, where data scarcity and class imbalance hinder accurate AI-based disease detection.

Method: Uses a progressive generative adversarial network to generate synthetic data, combines synthetic and real data with a weighted approach, and employs multi-objective meta-heuristic population-based optimization to tune classifier hyper-parameters.

Result: Achieves 95.5% accuracy for 4-class and 98.5% accuracy for 2-class imbalanced classification on COVID-19 chest X-ray datasets, outperforming existing methods in cross-validated metrics.

Conclusion: The proposed model effectively handles medical image classification with imbalanced data during pandemics, demonstrating the value of synthetic data generation and optimization techniques for improving diagnostic accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Existing training-free OVSS methods are either computationally expensive (relying on external models like SAM/DINO) or suboptimal (using static heuristics on CLIP features). CLIP's image-level representations lack pixel-level details needed for segmentation.

Method: Attention Refinement Module (ARM) - a lightweight learnable module with semantically-guided cross-attention (using deep features as K,V to refine shallow features as Q) followed by self-attention. Trained once on general datasets like COCO-Stuff, then used as plug-and-play post-processor for various training-free frameworks.
...
Conclusion: ARM provides a universal solution that unlocks CLIP's internal potential for segmentation tasks through adaptive feature fusion, offering a "train once, use anywhere" approach that outperforms existing training-free methods.
...
TL;DR: Mirage is a one-step video diffusion model for photorealistic and temporally coherent asset editing in driving scenes, using a novel approach that combines 3D diffusion priors with 2D detail injection and two-stage data alignment.
...
Method: Mirage builds on text-to-video diffusion for temporal consistency, injects temporally agnostic latents from a pretrained 2D encoder into the 3D decoder to restore detail, and uses a two-stage data alignment strategy (coarse 3D alignment + fine 2D refinement) to address distribution mismatch between scene objects and inserted assets.
...
Conclusion: Mirage provides a reliable baseline for photorealistic and coherent video editing in driving scenes, with potential applications beyond asset editing to various video-to-video translation tasks.
...
TL;DR: MotivNet is a facial emotion recognition model using Meta's Sapiens vision foundation model as backbone, achieving cross-domain generalization without cross-domain training.
...
Method: Uses Meta's Sapiens (human vision foundational model with Masked Autoencoder pretraining) as backbone. MotivNet is added as a downstream task to Sapiens. Proposes three evaluation criteria: benchmark performance, model similarity, and data similarity.
...
Conclusion: MotivNet validates FER as a viable downstream task for Sapiens foundation model, enabling more robust real-world facial emotion recognition applications without requiring cross-domain training.
...
TL;DR: MambaSeg: A dual-branch semantic segmentation framework using parallel Mamba encoders for RGB and event data fusion, with novel Dual-Dimensional Interaction Module for spatial and temporal fusion, achieving SOTA performance with reduced computational cost.


<details>
  <summary>Details</summary>
Motivation: RGB-based segmentation methods degrade under challenging conditions (fast motion, low-light, high dynamic range) due to frame camera limitations. Event cameras offer high temporal resolution but lack color/texture. Existing multimodal fusion approaches are computationally expensive and focus mainly on spatial fusion, neglecting event stream temporal dynamics.

Method: Proposes MambaSeg with parallel Mamba encoders for RGB images and event streams. Introduces Dual-Dimensional Interaction Module (DDIM) with Cross-Spatial Interaction Module (CSIM) and Cross-Temporal Interaction Module (CTIM) for fine-grained fusion along both spatial and temporal dimensions to improve cross-modal alignment and reduce ambiguity.
...
Conclusion: MambaSeg shows promise for efficient, scalable, and robust multimodal perception by effectively leveraging complementary properties of RGB and event modalities through efficient spatial-temporal fusion.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for MAR have limitations: supervised methods cause spectral blurring due to regression-to-the-mean, unsupervised methods risk structural hallucinations, and diffusion models are too slow for clinical use due to iterative sampling.

Method: Three components: 1) AAPS pipeline synthesizes high-fidelity training pairs via Monte Carlo spectral modeling and patient-specific digital twins; 2) DMP-Former adapts direct x-prediction for deterministic manifold projection in single forward pass; 3) SSA module uses MedDINOv3 priors for semantic-structural alignment.

Result: PGMP outperforms state-of-the-art methods on unseen anatomy in both synthetic and multi-center clinical datasets, setting new benchmarks in efficiency and diagnostic reliability.

Conclusion: PGMP framework effectively addresses limitations of existing MAR methods by combining physics-based simulation, deterministic fast inference, and medical foundation model priors, making it suitable for clinical applications.
...
TL;DR: DualityForge is a framework that synthesizes counterfactual video data using diffusion-based editing to reduce hallucinations in multimodal LLMs, achieving 24% improvement over baseline.


<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from visual ungrounded hallucinations due to over-reliance on language priors, especially with counterfactual videos. This stems from text-video data imbalance, and collecting counterfactual data is expensive.

Method: DualityForge uses controllable diffusion-based video editing to transform real videos into counterfactual scenarios, generating QA pairs and original-edited video pairs. Duality-Normalized Advantage Training (DNA-Train) applies pair-wise ℓ₁ advantage normalization for stable RL optimization.

Result: 24.0% relative improvement over Qwen2.5-VL-7B baseline on counterfactual videos, with significant gains on both hallucination and general-purpose benchmarks, demonstrating strong generalization.

Conclusion: The approach effectively reduces MLLM hallucinations through synthetic counterfactual data and contrastive training, with dataset and code to be open-sourced.
...
TL;DR: A 3D-based panoramic stitching framework that lifts images to 3D point clouds for global fusion, then projects to 360° panoramas, reducing distortions in complex scenes with parallax and occlusions.


<details>
  <summary>Details</summary>
Motivation: Traditional 2D stitching methods (homography, mesh warping) fail in real 3D scenes with multiple depth layers and occlusions, causing ghosting, structural bending, and stretching distortions, especially in multi-view and 360° closed-loop scenarios.

Method: 1) Lift each input image to dense 3D point representation in unified coordinate system with confidence metrics; 2) Establish unified 3D projection center and use equidistant cylindrical projection to map fused data to panoramic manifold; 3) Perform hole filling in canvas domain for unknown regions.
...
Method: EndoRare uses language-guided concept disentanglement to separate pathognomonic lesion features from non-diagnostic attributes, encoding diagnostic features into a learnable prototype embedding while varying non-diagnostic attributes to ensure diversity, all from a single reference image without retraining.

Result: Validated across four rare pathologies, synthetic images were clinically plausible by expert judgment. When used for data augmentation, they significantly enhanced downstream AI classifiers (improving true positive rate at low false-positive rates). A blinded reader study showed novice endoscopists exposed to EndoRare-generated cases achieved 0.400 increase in recall and 0.267 increase in precision.

Conclusion: EndoRare establishes a practical, data-efficient pathway to bridge the rare-disease gap in both computer-aided diagnostics and clinical education through one-shot generative synthesis of diverse, high-fidelity lesion exemplars.
...
TL;DR: Virtual-Eyes CT quality-control pipeline improves generalist foundation models for lung cancer screening but harms specialist models adapted to raw clinical data.


<details>
  <summary>Details</summary>
Motivation: Robust preprocessing is rarely quantified in deep-learning pipelines for low-dose CT lung cancer screening, and there's a need to understand how quality control affects different types of models.

Method: Developed Virtual-Eyes, a 16-bit CT quality-control pipeline that enforces 512x512 resolution, rejects non-diagnostic series, extracts contiguous lung blocks using Hounsfield-unit filtering and lung-coverage scoring. Evaluated on 765 NLST patients using RAD-DINO, Merlin, Sybil, and ResNet-18 models with frozen encoders and leakage-free patient-level MLP heads.

Result: Virtual-Eyes improved RAD-DINO slice-level AUC from 0.576 to 0.610 and patient-level AUC from 0.646 to 0.683 (mean pooling) and 0.619 to 0.735 (max pooling). However, Sybil degraded (AUC 0.886 to 0.837), ResNet-18 showed minimal improvement (0.571 to 0.596), and Merlin showed limited transferability (~0.507 to 0.567).
...
TL;DR: UniAct is a two-stage framework that integrates a fine-tuned multimodal large language model with a causal streaming pipeline to enable humanoid robots to execute diverse multimodal instructions with sub-500ms latency.


<details>
  <summary>Details</summary>
Motivation: Bridging high-level multimodal perception with whole-body execution in humanoid robotics remains a significant bottleneck. Existing methods struggle to translate heterogeneous instructions (language, music, trajectories) into stable, real-time actions.

Method: Two-stage framework: 1) Fine-tuned MLLM for multimodal instruction understanding, 2) Causal streaming pipeline for real-time execution. Uses FSQ (Finite Scalar Quantization) to unify inputs through a shared discrete codebook, ensuring cross-modal alignment while constraining motions to physically grounded manifolds.

Result: Achieves sub-500ms latency for humanoid robot execution, 19% improvement in zero-shot tracking of imperfect reference motions, and robust generalization across diverse real-world scenarios validated on the UniMoCap benchmark (20-hour humanoid motion dataset).

Conclusion: UniAct represents a critical step toward responsive, general-purpose humanoid assistants capable of seamless interaction through unified perception and control, enabling humanoid robots to follow diverse multimodal instructions with human-level flexibility.
...
Result: CERES achieves state-of-the-art performance on Ego-RVOS benchmarks, demonstrating the effectiveness of causal reasoning for building more reliable egocentric video understanding models.

Conclusion: The paper shows that applying causal reasoning through the CERES framework can effectively address biases and visual confounding in egocentric video segmentation, enabling more robust performance.
...
Method: SenseNova-MARS framework with reinforcement learning (BN-GSPO algorithm) that dynamically integrates image search, text search, and image crop tools for fine-grained visual understanding.

Result: Achieves state-of-the-art performance: 67.84 on MMSearch and 41.64 on HR-MMSearch (new benchmark), surpassing proprietary models like Gemini-3-Flash and GPT-5.
...
TL;DR: LVLDrive is a framework that enhances Vision-Language Models for autonomous driving by incorporating LiDAR point clouds for better 3D spatial understanding, using gradual feature fusion to preserve existing knowledge and spatial-aware training data.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models for autonomous driving rely on 2D images, which struggle with accurate metric spatial reasoning and geometric inference, creating safety and reliability bottlenecks for driving policies.

Method: Proposes LVLDrive framework that adds LiDAR point clouds as an extra input modality, introduces Gradual Fusion Q-Former to incrementally inject LiDAR features while preserving VLM knowledge, and develops spatial-aware question-answering dataset for 3D perception training.

Result: Extensive experiments show LVLDrive achieves superior performance compared to vision-only counterparts across scene understanding, metric spatial perception, and reliable driving decision-making on driving benchmarks.

Conclusion: Explicit 3D metric data is necessary for building trustworthy VLM-based autonomous systems, and LVLDrive successfully bridges the gap between VLMs and robust 3D spatial understanding for autonomous driving.
...
TL;DR: The paper proposes elementary information mechanics as a new model for understanding convolutional filtering with rectification, drawing analogies to special relativity and quantum mechanics by analyzing kernels through even/odd decomposition and their effects on information propagation.


<details>
  <summary>Details</summary>
Motivation: To establish a theoretical framework connecting convolutional neural network operations to fundamental physics principles, specifically bridging information processing in CNNs with relativistic energy-momentum relations that have not been previously explored in this context.

Method: Decomposes convolutional kernels into orthogonal even and odd components, analyzes their effects on image content (even components cause isotropic diffusion, odd components cause directional displacement), and examines these properties in the spectral domain using discrete cosine transform (DCT) to study low-frequency basis structures in small filters.

Result: Shows that even kernels preserve center of mass (analogous to rest/potential energy with zero momentum), odd kernels displace center of mass (analogous to kinetic energy with non-zero momentum), and information displacement speed is linearly related to odd vs total kernel energy ratio. Small convolutional filters are dominated by low-frequency DCT bases (DC Σ and gradient ∇ components).

Conclusion: This work establishes the first link between information processing in generic CNNs and the energy-momentum relation from relativistic physics, providing a new "elementary information mechanics" framework for understanding convolutional filtering operations through physical analogies.
...
TL;DR: DermaVQA-DAS extends DermaVQA dataset with expert-developed Dermatology Assessment Schema (DAS) for closed QA and lesion segmentation, benchmarking multimodal models on patient-centered dermatology tasks.


<details>
  <summary>Details</summary>
Motivation: Existing dermatology datasets focus on dermatoscopic images without patient queries and clinical context, limiting applicability to patient-centered care. Need for structured, standardized assessment framework.

Method: Introduce DAS schema with 36 high-level and 27 fine-grained assessment questions in English/Chinese. Extend DermaVQA dataset for closed QA and segmentation tasks. Benchmark multimodal models with different prompting strategies.

Result: For segmentation: prompt design impacts performance; augmented prompt with patient query yields best results (Jaccard 0.395, Dice 0.566). For QA: models perform well (avg accuracy 0.729-0.798); o3 achieves best accuracy (0.798), GPT-4.1 close second (0.796).

Conclusion: DermaVQA-DAS addresses gap in patient-centered dermatology datasets with structured DAS framework. Public release supports future research in dermatological vision-language modeling, showing importance of prompt design and clinical context.
...
TL;DR: A comprehensive framework and taxonomy for multi-modal pre-training to achieve Spatial Intelligence by integrating diverse sensors like cameras and LiDAR for autonomous systems.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of autonomous systems (self-driving vehicles, drones) creates an urgent need for true Spatial Intelligence from multi-modal sensor data. While foundation models work well for single modalities, integrating capabilities across diverse sensors remains a major challenge.

Method: Presents a comprehensive framework for multi-modal pre-training, analyzing the interplay between foundational sensor characteristics and learning strategies. Formulates a unified taxonomy for pre-training paradigms ranging from single-modality baselines to sophisticated unified frameworks that learn holistic representations. Investigates integration of textual inputs and occupancy representations for open-world perception and planning.

Result: Identifies core techniques driving progress toward multi-modal Spatial Intelligence, evaluates role of platform-specific datasets, and provides a unified taxonomy for pre-training paradigms applicable to advanced tasks like 3D object detection and semantic occupancy prediction.

Conclusion: Identifies critical bottlenecks (computational efficiency, model scalability) and proposes a roadmap toward general-purpose multi-modal foundation models capable of achieving robust Spatial Intelligence for real-world deployment in autonomous systems.
...
TL;DR: RedunCut is a dynamic model size selection system for live video analytics that reduces compute costs by 14-62% while maintaining accuracy, addressing limitations of prior methods through better sampling cost-benefit analysis and accuracy prediction.


<details>
  <summary>Details</summary>
Motivation: Live video analytics faces high inference costs with modern vision models across massive camera fleets. Dynamic model size selection (DMSS) can reduce costs by up to 10x without retraining, but prior methods fail to generalize to diverse workloads like mobile videos and lower accuracy targets due to inefficient sampling and inaccurate accuracy prediction.

Method: RedunCut introduces two key components: 1) a measurement-driven planner that estimates the cost-benefit tradeoff of sampling to avoid inefficient sampling, and 2) a lightweight, data-driven performance model to improve per-segment accuracy prediction. The system operates without ground truth labels at runtime.

Result: Across diverse video types (road-vehicle, drone, surveillance) and multiple model families/tasks, RedunCut reduces compute cost by 14-62% at fixed accuracy. The system remains robust to limited historical data and drift.

Conclusion: RedunCut successfully addresses the limitations of prior DMSS methods by optimizing sampling efficiency and improving accuracy prediction, enabling significant compute cost reductions for live video analytics across diverse real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing chunk-based methods require full non-causal context windows, introducing significant delays that prevent immediate non-verbal feedback needed for realistic listener behavior in dyadic conversations.

Method: Flow matching-based autoregressive model with two key designs: (1) stream-friendly autoregressive framework with flow-matching heads for probabilistic modeling, and (2) causal encoder enhanced by lookahead module to incorporate short future context (60ms) while maintaining low latency.

Result: Generates video within 34ms per frame, keeping total system latency under 100ms. Achieves state-of-the-art lip-sync quality with offline/online LipSync Confidence scores of 8.13/7.61 on HDTF dataset.
...
TL;DR: AI-driven framework using video transformers and motion tracking for automated assessment of microanastomosis surgical skills, achieving high accuracy in action recognition and skill classification.
...
Result: 87.7% frame-level accuracy in action segmentation (increased to 93.62% with post-processing). Average 76% classification accuracy in replicating expert assessments across all skill aspects on 58 expert-annotated videos.

Conclusion: The AI-driven framework provides objective, consistent, and interpretable feedback for surgical skill assessment, enabling more standardized, data-driven training and evaluation in surgical education.
...
TL;DR: ViT encoder representations show compositionality when using DWT primitives, similar to how language models compose representations.


<details>
  <summary>Details</summary>
Motivation: To understand how Vision Transformers structure information by testing for compositionality in their representations, analogous to language model analysis.

Method: Use Discrete Wavelet Transform to obtain vision primitives, then test if composed representations can reproduce original image representations in ViT encoder.

Result: Primitives from one-level DWT decomposition produce encoder representations that approximately compose in latent space.

Conclusion: ViTs exhibit compositionality in their representation space, offering new insights into how they structure visual information.
...
TL;DR: AI pipeline combines YOLO object detection models with GPT-4 to generate radiology reports from chest X-rays, achieving strong clinical accuracy but with distinguishable writing style from human radiologists.
...
Method: Integrated pipeline using YOLOv5 and YOLOv8 for anomaly detection in chest X-rays (bounding boxes and class labels), then feeding these structured outputs to GPT-4 to generate descriptive findings and clinical summaries. Comparison of both YOLO models on detection accuracy, inference latency, and text quality.

Result: Strong semantic similarity between AI and human reports. YOLOv8 showed better detection accuracy than YOLOv5. GPT-4 excelled in clarity (4.88/5) but scored lower for natural writing flow (2.81/5), indicating clinical accuracy but stylistically distinguishable from radiologist text.

Conclusion: The pipeline successfully generates clinically accurate radiology reports from chest X-rays, demonstrating the potential of combining computer vision with large language models. However, current systems still produce text that is stylistically different from human-authored reports, suggesting room for improvement in natural language generation.
...
TL;DR: MS-VQ-VAE for compact latent video representations optimized for edge devices, achieving 25.96 dB PSNR and 0.8375 SSIM on UCF101.


<details>
  <summary>Details</summary>
Motivation: Traditional video codecs lack native support for machine learning latent representations, limiting integration into deep learning pipelines. There's a need for efficient video compression suitable for bandwidth-sensitive scenarios like edge devices and CDNs.

Method: Multi-Scale Vector Quantized Variational Autoencoder (MS-VQ-VAE) extending VQ-VAE-2 to spatiotemporal setting with two-level hierarchical latent structure using 3D residual convolutions. Incorporates perceptual loss from pre-trained VGG16 network.

Result: Achieves 25.96 dB PSNR and 0.8375 SSIM on UCF101 test set (64x64 resolution, 32 frames at 16 FPS). Outperforms single-scale baseline by 1.41 dB PSNR and 0.0248 SSIM. Model is lightweight with ~18.5M parameters.

Conclusion: The framework is well-suited for scalable video compression in bandwidth-sensitive scenarios including real-time streaming, mobile video analytics, and CDN storage optimization, enabling efficient machine learning integration.
...
TL;DR: PhyGDPO: A physics-aware video generation method that uses physics-augmented data and groupwise preference optimization to create physically consistent videos.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-video methods struggle to generate videos that follow physical laws, with limitations in generalizing beyond simple simulations and implicit physical reasoning. There's also a scarcity of training data with rich physics interactions.

Method: 1) PhyAugPipe: Physics-augmented video data construction pipeline using VLM with chain-of-thought reasoning to create PhyVidGen-135K dataset. 2) PhyGDPO: Physics-aware Groupwise Direct Preference Optimization framework with Physics-Guided Rewarding (PGR) scheme using VLM-based physics rewards. 3) LoRA-Switch Reference (LoRA-SR) scheme for efficient training without memory-heavy reference duplication.

Result: Significantly outperforms state-of-the-art open-source methods on PhyGenBench and VideoPhy2 benchmarks.

Conclusion: The proposed physics-aware approach effectively addresses physical consistency in video generation through data augmentation and principled optimization, achieving superior performance on physics-focused benchmarks.
...
TL;DR: Proposes a novel second-order optimization algorithm for deep learning that uses OCP method and Hessian diagonal approximation to achieve faster convergence and better performance on visual localization tasks.


<details>
  <summary>Details</summary>
Motivation: To address large-scale optimization problems in deep learning, particularly for visual localization, where conventional optimization algorithms may have limitations in convergence speed, training stability, and robustness to noise.

Method: A novel second-order optimization algorithm that incorporates the OCP (likely Optimal Control or Optimization Control Protocol) method and appropriately approximates the diagonal elements of the Hessian matrix to make the optimization more efficient for large-scale problems.
...
Conclusion: The proposed second-order optimization framework with OCP method and Hessian diagonal approximation is effective for large-scale deep learning optimization problems, particularly in visual localization applications, offering superior performance compared to conventional optimization approaches.
...
TL;DR: RGBT-Ground: First large-scale RGB-Thermal visual grounding benchmark for complex real-world scenarios, with RGBT-VGNet baseline model that outperforms adapted methods, especially in nighttime/long-distance conditions.


<details>
  <summary>Details</summary>
Motivation: Existing VG benchmarks are limited to clean environments (like COCO) with limited scene diversity, failing to reflect real-world complexity (illumination, weather changes) crucial for evaluating model robustness in safety-critical applications.

Method: 1) Created RGBT-Ground benchmark with spatially aligned RGB-Thermal image pairs, referring expressions, bounding boxes, and fine-grained scene/environment/object annotations. 2) Proposed RGBT-VGNet - unified framework supporting uni-modal (RGB/TIR) and multi-modal (RGB-TIR) inputs with effective fusion of complementary visual modalities.

Result: RGBT-VGNet significantly outperforms adapted existing methods on the new benchmark, particularly excelling in challenging nighttime and long-distance scenarios. The benchmark enables comprehensive evaluation of robust grounding under diverse real-world conditions.

Conclusion: RGBT-Ground addresses limitations of existing VG benchmarks by introducing real-world complexity through RGB-Thermal data, with RGBT-VGNet demonstrating superior performance. This promotes research on robust visual grounding for safety-critical applications in complex environments.
...
<details>
  <summary>Details</summary>
Motivation: Many CDVQA failures stem from decision ambiguity where models assign similar confidence to correct answers and strong distractors, rather than clearly incorrect predictions. This ambiguity undermines model discriminability and robustness.

Method: DARFT: Decision-Ambiguity-guided Reinforcement Fine-Tuning framework that (1) mines Decision-Ambiguous Samples (DAS) using SFT-trained reference policy, then (2) applies group-relative policy optimization on the mined subset using multi-sample decoding and intra-group relative advantages.

Result: DARFT achieves consistent gains over SFT baselines, particularly under few-shot settings, by suppressing strong distractors and sharpening decision boundaries without additional supervision.

Conclusion: Explicitly optimizing decision-ambiguous samples is crucial for improving CDVQA model discriminability and robustness, and DARFT provides an effective framework for addressing this challenge through targeted reinforcement fine-tuning.
...
TL;DR: SliceLens is a hypothesis-driven framework using LLMs/VLMs for fine-grained error slice discovery in instance-level vision tasks, with a new benchmark FeSD showing significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Existing slice discovery methods are limited to image classification and fail at multi-instance tasks like detection/segmentation. Current approaches lack fine-grained reasoning for complex visual relationships, and benchmarks are biased toward classification with artificial ground truth.

Method: SliceLens leverages LLMs and VLMs to generate and verify diverse failure hypotheses through grounded visual reasoning. It enables reliable identification of fine-grained, interpretable error slices across instance-level vision tasks.
...
Conclusion: SliceLens addresses critical limitations in error slice discovery for instance-level vision tasks, providing a robust framework with superior performance and practical utility for model improvement.
...
TL;DR: A new 3D dataset from UAV footage of Hurricane Ian was created to address the lack of disaster-specific 3D data for semantic segmentation, revealing limitations of current SOTA models in post-disaster environments.
...
Method: Constructed a specialized 3D dataset using UAV-captured aerial footage of Hurricane Ian (2022) over affected areas, employing Structure-from-Motion (SfM) and Multi-View Stereo (MVS) techniques to reconstruct 3D point clouds. Evaluated SOTA 3D semantic segmentation models (Fast Point Transformer, Point Transformer v3, and OA-CNNs) on this dataset.

Result: Evaluation exposed significant limitations in existing methods for disaster-stricken regions, showing current SOTA models perform poorly in post-disaster environments.

Conclusion: There is an urgent need for advancements in 3D segmentation techniques and development of specialized 3D benchmark datasets to improve post-disaster scene understanding and response capabilities.
...
Method: CLoRA uses base-space sharing where all low-rank modules share down/up-projection spaces, and sample-agnostic diversity enhancement (SADE) to regularize similarities among low-rank matrices to encourage diverse representations.

Result: CLoRA achieves better balance between learning performance and parameter efficiency than state-of-the-art methods, requiring the fewest GFLOPs for point cloud analysis while maintaining strong performance on image datasets.

Conclusion: CLoRA effectively addresses the trade-off between performance and parameter efficiency in vision transformer fine-tuning through collaborative low-rank adaptation with shared spaces and diversity regularization.
...
TL;DR: The paper introduces MoniRefer, the first large-scale 3D visual grounding dataset for outdoor monitoring scenarios, and proposes Moni3DVG, a multi-modal method that combines image appearance with point cloud geometry for accurate object localization in traffic scenes.


<details>
  <summary>Details</summary>
Motivation: Current 3D visual grounding research focuses on indoor and driving scenes, leaving roadside infrastructure monitoring unexplored due to lack of paired point cloud-text data from roadside sensors. This gap limits infrastructure-level understanding of complex traffic environments.

Method: Proposes Moni3DVG, an end-to-end method that leverages both image appearance information and point cloud geometry/optical information for multi-modal feature learning and 3D object localization in traffic scenes.

Result: Created MoniRefer dataset with ~136,018 objects and 411,128 natural language expressions from real-world traffic intersections. Extensive experiments show the proposed method's superiority and effectiveness on this new benchmark.

Conclusion: The work establishes the first 3D visual grounding task for outdoor monitoring scenarios, provides a large-scale real-world dataset, and demonstrates an effective multi-modal approach that outperforms existing methods, enabling infrastructure-level traffic scene understanding.
...
TL;DR: Proposes Layer-by-Layer Hierarchical Attention Network for feature point matching with outlier handling, using stage fusion and attention mechanisms to improve matching accuracy.


<details>
  <summary>Details</summary>
Motivation: Feature point matching is fundamental in computer vision but suffers from outlier interference, especially with high outlier proportions. Need to extract high-quality information while reducing negative sample errors.

Method: Layer-by-Layer Hierarchical Attention Network with: 1) layer-by-layer channel fusion module preserving feature semantic information from each stage, 2) hierarchical attention module adaptively capturing global perception and structural semantics, 3) two architectures for feature extraction and integration.

Result: Outperforms state-of-the-art techniques on YFCC100M and SUN3D datasets in both outlier removal and camera pose estimation.
...
TL;DR: Proposes FRS-YOLO model and FireRescue dataset for improved object detection in fire rescue scenarios, addressing limitations in existing research on urban scenes and comprehensive target coverage.


<details>
  <summary>Details</summary>
Motivation: Existing fire detection research has two main limitations: 1) focuses mainly on mountainous/forest environments while neglecting more frequent and complex urban rescue scenes, and 2) includes limited target classes (flames/smoke) without comprehensive coverage of key command decision targets like fire trucks and firefighters.

Method: 1) Constructs new FireRescue dataset covering urban, mountainous, forest, and water rescue scenarios with 8 key categories (15,980 images, 32,000 bounding boxes). 2) Proposes FRS-YOLO model with: a) plug-and-play multidimensional collaborative enhancement attention module for cross-dimensional feature interaction to reduce inter-class confusion, and b) dynamic feature sampler to strengthen high-response foreground features to mitigate smoke occlusion and background interference.

Result: Experimental results demonstrate that object detection in fire rescue scenarios is highly challenging, and the proposed FRS-YOLO method effectively improves detection performance of YOLO series models in this context.

Conclusion: The paper addresses critical gaps in fire rescue object detection by creating a comprehensive dataset and developing an improved model that handles inter-class confusion and small target detection in chaotic rescue environments, contributing to better command and decision-making in firefighting operations.
...
TL;DR: RadAR is a parallelizable framework that accelerates autoregressive visual generation using radial topology and nested attention, improving efficiency while preserving quality.
...
Method: Organizes generation around radial topology: selects initial token as starting point, groups other tokens into concentric rings based on spatial distance from center. Generation proceeds ring-wise (inner to outer), enabling parallel prediction of all tokens within same ring. Introduces nested attention mechanism to dynamically refine implausible outputs during forward pass, mitigating error accumulation.

Result: Significantly improves generation efficiency through radial parallel prediction while preserving structural locality and spatial coherence of visual scenes. Prevents model collapse and error accumulation through dynamic output correction.

Conclusion: RadAR provides an efficient and parallelizable framework for autoregressive visual generation that maintains representational capacity while substantially increasing inference speed through radial organization and nested attention mechanisms.
...
Method: RGTN uses dynamic scale-transformation for continuous structure evolution across resolutions. It features learnable edge gates for topology modification and intelligent proposals based on physical quantities like node tension (measuring local stress) and edge information flow (quantifying connectivity importance). The framework starts from low-complexity coarse scales and refines to finer ones.
...
Conclusion: RGTN successfully transforms tensor network structure search through physics-inspired multi-scale renormalization group flows, validating the effectiveness of this approach for discovering optimal network topologies and rank configurations efficiently.
...
Method: EVOL-SAM3 reformulates reasoning segmentation as an inference-time evolutionary search process. It maintains a population of prompt hypotheses and iteratively refines them through a "Generate-Evaluate-Evolve" loop with three key components: 1) Visual Arena for reference-free pairwise tournament evaluation, 2) Semantic Mutation operator to inject diversity and correct semantic errors, and 3) Heterogeneous Arena module integrating geometric priors with semantic reasoning for robust final selection.
...
Conclusion: EVOL-SAM3 successfully addresses limitations of current reasoning segmentation approaches by introducing an evolutionary search paradigm at inference time, enabling deeper reasoning, self-correction capabilities, and superior performance without requiring training.
...
<details>
  <summary>Details</summary>
Motivation: The authors observed that model capacity impact varies across diffusion timesteps - crucial for early and late stages but negligible during intermediate stages. This insight motivates a more efficient sampling strategy that doesn't require large models throughout the entire process.

Method: FlowBlending uses a large model and a small model at different stages: large model for capacity-sensitive early/late stages, small model for intermediate stages. They introduce criteria to choose stage boundaries and use velocity-divergence analysis as a proxy for identifying capacity-sensitive regions.
...
Conclusion: FlowBlending provides an effective strategy for efficient video generation by strategically allocating model capacity based on timestep sensitivity, significantly reducing computational costs without compromising quality, and can be combined with other acceleration methods.
...
TL;DR: EchoFoley introduces a new video-grounded sound generation task with fine-grained control, addressing limitations in current VT2A models through symbolic event representation and a large expert-curated dataset.


<details>
  <summary>Details</summary>
Motivation: Current video-text-to-audio (VT2A) models suffer from three key limitations: visual-text conditioning imbalance, lack of fine-grained control definitions, and weak instruction understanding due to brief categorical tags in existing datasets.

Method: Proposes EchoFoley task with symbolic representation for sounding events (specifying when, what, and how sounds are produced), EchoFoley-6k dataset (6,000+ expert-curated video-instruction-annotation triplets), and EchoVidia framework with slow-fast thinking strategy for agentic generation.

Result: EchoVidia surpasses recent VT2A models by 40.7% in controllability and 12.5% in perceptual quality, demonstrating significant improvements in fine-grained sound generation capabilities.

Conclusion: The EchoFoley framework successfully addresses key limitations in video-grounded sound generation, enabling precise event-level control and hierarchical semantic control through symbolic representation and expert-curated data, representing a substantial advancement in multimodal storytelling.
...
Result: The authors introduce Splatwizard as a solution to the evaluation gap, making the code publicly available at https://github.com/splatwizard/splatwizard.

Conclusion: Splatwizard addresses the critical need for standardized benchmarking in 3DGS compression research by providing a comprehensive toolkit that enables fair comparison and evaluation of different compression methods across multiple important dimensions.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D segmentation methods face challenges with inconsistent 2D instance labels across views, leading to poor 3D predictions. Current approaches use two-stage methods with contrastive learning or label preprocessing, which are hyperparameter-sensitive and inefficient.

Method: Proposes a unified framework with learnable feature embeddings in Gaussian primitives, decoded into instance labels via "Embedding-to-Label" process. Addresses boundary artifacts with hard-mining samples and stabilizes training using a linear layer on rasterized embeddings before triplet loss calculation.
...
Conclusion: The unified framework effectively addresses 2D label inconsistencies in 3D segmentation, offering improved efficiency and performance through integrated optimization and boundary-aware training stabilization.
...
TL;DR: Proposes a physics-based adversarial attack method using projected light perturbations to fool monocular depth estimation models, causing objects to disappear from depth maps.


<details>
  <summary>Details</summary>
Motivation: DNN-based monocular depth estimation models are vulnerable to adversarial attacks, threatening their reliability in practical applications. Current attacks don't account for real-world physics and device constraints.

Method: Uses projection-based adversarial attacks with physics-in-the-loop optimization, evaluating candidate solutions in actual environments to account for device specs and disturbances. Employs distributed covariance matrix adaptation evolution strategy for optimization.

Result: Successfully created adversarial examples that cause depth misestimations, making parts of objects disappear from target scenes in real-world settings.

Conclusion: Demonstrates vulnerability of MDE models to physics-based adversarial attacks, highlighting need for robustness enhancement in practical applications.
...
TL;DR: The paper presents a method to extend Noise2Noise training to handle nonlinear preprocessing of noisy targets, enabling effective denoising of HDR images without clean reference data.


<details>
  <summary>Details</summary>
Motivation: Noise2Noise training requires noisy target images but cannot handle nonlinear preprocessing due to bias issues. This limitation is particularly problematic for HDR image denoising where nonlinear tone mapping is needed to handle outliers, but was previously thought incompatible with Noise2Noise.

Method: Developed a theoretical framework to analyze nonlinear function effects on noisy targets, identifying a class of nonlinear functions with minimal bias. Applied this to HDR denoising by combining specific loss functions and tone mapping functions that reduce outlier effects while introducing minimal bias.

Result: Successfully applied the method to an existing Monte Carlo denoiser, achieving results approaching the original implementation (which used high-sample count reference images) using only noisy training data.

Conclusion: Certain nonlinear functions can be applied to noisy targets in Noise2Noise training without significant bias, enabling effective preprocessing for challenging cases like HDR image denoising where nonlinear tone mapping is essential.
...
TL;DR: A method that enables 2D-trained cross-modal systems to adapt online to 3D scenes using regret minimization and derivative-free optimization to control an in-scene camera, improving performance without pretraining.
...
Method: Improves multivariate mutual information estimates through regret minimization with derivative-free optimization, enabling control of an in-scene camera to learn directly from noisy vision-language model outputs.
...
Conclusion: The method successfully bridges the 2D-to-3D gap for cross-modal systems through camera control optimization, enabling effective adaptation to complex 3D scenes using existing 2D-trained models.
...
### [88] [HaineiFRDM: Explore Diffusion to Restore Defects in Fast-Movement Films](https://arxiv.org/abs/2512.24946)
*Rongji Xun,Junjie Yuan,Zhongjie Wang*

Main category: cs.CV

TL;DR: HaineiFRDM is a film restoration diffusion model that addresses limitations of existing open-source methods by enabling high-resolution restoration on limited hardware, reducing artifacts, and using better training data.


<details>
  <summary>Details</summary>
Motivation: Existing open-source film restoration methods have limited performance due to training with low-quality synthetic data, noisy optical flows, and inability to handle high-resolution films. Commercial methods outperform them, creating a need for better open-source solutions.

Method: Proposes HaineiFRDM framework with: 1) patch-wise training/testing for high-resolution films on 24GB VRAM GPU, 2) position-aware Global Prompt and Frame Fusion Modules, 3) global-local frequency module for consistent textures, 4) low-resolution restoration as global residual to reduce artifacts, and 5) new dataset with restored real-degraded films and realistic synthetic data.

Result: Comprehensive experiments demonstrate superiority in defect restoration ability over existing open-source methods. The model effectively handles high-resolution films and produces better restoration results.

Conclusion: HaineiFRDM successfully addresses key limitations of open-source film restoration by leveraging diffusion models' content understanding, enabling high-resolution processing on accessible hardware, and using improved training data. Code and dataset will be released to advance open-source film restoration.
...
TL;DR: CropTrack is a novel MOT framework for agricultural environments that combines appearance and motion information to handle repetitive patterns, similar appearances, and frequent occlusions, outperforming motion-based methods.


<details>
  <summary>Details</summary>
Motivation: Agricultural MOT faces unique challenges: repetitive patterns, similar object appearances, sudden illumination changes, and frequent occlusions. Current motion-based trackers struggle with identity preservation during strong occlusions, while appearance-based association is difficult due to high similarity between objects.
...
Conclusion: The proposed CropTrack framework effectively addresses agricultural MOT challenges by combining appearance and motion information, providing better identity preservation and association accuracy than existing methods in this challenging domain.
...
TL;DR: MLLMs show poor 3D spatial reasoning in embodied navigation despite their general vision-language capabilities, with CoT reasoning and self-reflection actually decreasing performance in VLN tasks.


<details>
  <summary>Details</summary>
Motivation: While MLLMs excel at general vision-language tasks, their performance as embodied agents requiring multi-round dialogue, spatial reasoning, and sequential action prediction remains unexplored, particularly in Vision-and-Language Navigation (VLN) contexts.

Method: Introduces VLN-MME, a unified evaluation framework that bridges traditional navigation datasets into a standardized benchmark for probing MLLMs as zero-shot agents. Features modular design enabling structured comparisons and component-level ablations across MLLM architectures, agent designs, and navigation tasks.

Result: Counterintuitively, enhancing baseline agents with Chain-of-Thought reasoning and self-reflection leads to performance decrease, revealing MLLMs' poor context awareness and low 3D spatial reasoning fidelity in embodied navigation despite following instructions and structuring output correctly.

Conclusion: VLN-MME establishes groundwork for systematic evaluation of MLLMs in embodied navigation and reveals critical limitations in their sequential decision-making capabilities, offering crucial guidance for MLLM post-training as embodied agents.
...
TL;DR: OFL-SAM2 is a prompt-free adaptation of SAM2 for medical image segmentation that uses a lightweight mapping network trained on limited annotated data to generate target features, eliminating the need for manual prompts and extensive fine-tuning.


<details>
  <summary>Details</summary>
Motivation: While SAM2 shows promise for medical image segmentation, adapting it requires extensive annotated medical data for fine-tuning and high-quality manual prompts, both of which are labor-intensive and require medical expert intervention.

Method: Introduces OFL-SAM2 with two key components: (1) an online few-shot learner that trains a lightweight mapping network to generate target features using limited data, and (2) an adaptive fusion module that dynamically integrates target features with SAM2's memory-attention features. The mapping network supports online parameter updates during inference for better generalization.
...
Conclusion: OFL-SAM2 provides a label-efficient, prompt-free solution for adapting SAM2 to medical image segmentation, addressing the challenges of data scarcity and manual prompt requirements while maintaining strong performance.
...
### [92] [ShowUI-$π$: Flow-based Generative Models as GUI Dexterous Hands](https://arxiv.org/abs/2512.24965)
*Siyuan Hu,Kevin Qinghong Lin,Mike Zheng Shou*

Main category: cs.CV

TL;DR: ShowUI-π is a flow-based generative model for GUI agents that enables both discrete clicks and continuous drag actions, addressing limitations of existing agents that can't handle closed-loop trajectories like dragging progress bars.
...
Method: 1) Unified Discrete-Continuous Actions model integrating both clicks and drags; 2) Flow-based Action Generation using a lightweight action expert to predict incremental cursor adjustments from continuous visual observations; 3) Creation of ScreenDrag benchmark with 20K manually collected drag trajectories across five domains (PowerPoint, Adobe Premiere Pro, etc.).

Result: ShowUI-π achieves 26.98 score on ScreenDrag benchmark, outperforming proprietary GUI agents (Operator: 13.27, Gemini-2.5-CUA: 22.18) with only 450M parameters, demonstrating both the difficulty of the task and the effectiveness of the approach.

Conclusion: The work advances GUI agents toward human-like dexterous control in digital environments by enabling continuous drag capabilities through a unified flow-based generative model, with the ScreenDrag benchmark providing comprehensive evaluation protocols for future research.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack the complexity needed to evaluate MLLMs on real-world financial reasoning tasks that require scenario understanding, document comprehension, and multi-step numerical computation.

Method: Created a benchmark with 1,200 expert-annotated problems incorporating 12 financial scenarios, 837 bilingual documents across 9 types averaging 50.8 pages, and problems requiring 11-step reasoning on average with cross-page evidence retrieval.

Result: Best-performing MLLM achieved only 58.0% accuracy, showing significant room for improvement. Different RAG methods showed substantial performance variations, highlighting the benchmark's discriminative power.

Conclusion: FinMMDocR provides a challenging benchmark to drive improvements in MLLMs and reasoning-enhanced methods for complex multimodal reasoning in real-world financial scenarios.
...
### [94] [Evaluating the Impact of Compression Techniques on the Robustness of CNNs under Natural Corruptions](https://arxiv.org/abs/2512.24971)
...
<details>
  <summary>Details</summary>
Motivation: Compressed deep learning models are essential for resource-constrained devices, but compression may affect robustness to natural corruption, requiring comprehensive evaluation of robustness-accuracy-compression trade-offs.

Method: Comprehensive evaluation of quantization, pruning, and weight clustering (individually and combined) on CNNs (ResNet-50, VGG-19, MobileNetV2) using CIFAR-10-C and CIFAR-100-C datasets with multiobjective assessment.

Result: Certain compression strategies preserve or improve robustness, especially on complex architectures; customized technique combinations yield beneficial multi-objective results for robust and efficient deployment.

Conclusion: Compression techniques can maintain or enhance robustness to natural corruption, with optimal configurations from customized combinations, providing insights for robust model deployment in corrupted real-world environments.
...
TL;DR: A lidar domain adaptation method using neuron activation patterns to select a small, diverse subset of target domain samples for annotation, achieving SOTA performance with minimal annotation budget.


<details>
  <summary>Details</summary>
Motivation: 3D object detectors trained on standard benchmarks struggle to generalize across different geographic domains (e.g., US-trained models perform poorly in Asia/Europe), creating a need for efficient domain adaptation methods.

Method: Uses neuron activation patterns to identify a small, representative, and diverse subset of target domain samples for annotation. Combines this with post-training techniques inspired by continual learning to prevent weight drift from the original model.

Result: Outperforms both linear probing and state-of-the-art domain adaptation techniques in empirical evaluation, achieving SOTA performance with very small annotation budget.

Conclusion: Effective domain adaptation for 3D object detectors can be achieved by intelligently selecting a small subset of target domain samples using neuron activation patterns, combined with continual learning techniques to maintain model stability.
...
### [96] [DarkEQA: Benchmarking Vision-Language Models for Embodied Question Answering in Low-Light Indoor Environments](https://arxiv.org/abs/2512.24985)
*Yohan Park,Hyunwoo Ha,Wonjun Jo,Tae-Hyun Oh*

Main category: cs.CV

TL;DR: DarkEQA is a new benchmark for evaluating Vision Language Models (VLMs) on embodied question answering tasks under realistic low-light conditions, revealing significant performance limitations.


<details>
  <summary>Details</summary>
Motivation: Existing VLM benchmarks focus on ideal lighting conditions, but real-world embodied agents need to operate 24/7 in various lighting environments including low-light/dark conditions, which has been largely overlooked in current evaluation frameworks.

Method: DarkEQA isolates perception bottlenecks by evaluating question answering from egocentric observations under controlled visual degradations. It uses physically realistic modeling in linear RAW space to simulate illumination drop and sensor noise, followed by an ISP-inspired rendering pipeline for high physical fidelity.

Result: The benchmark systematically reveals significant limitations of state-of-the-art VLMs when operating under challenging low-light conditions, showing that current models struggle with degraded visual inputs.

Conclusion: DarkEQA addresses a critical gap in VLM evaluation for embodied agents, providing a physically realistic benchmark for low-light conditions that enables attributable robustness analysis and highlights the need for improved VLM performance in challenging visual environments.
...
### [97] [CPJ: Explainable Agricultural Pest Diagnosis via Caption-Prompt-Judge with LLM-Judged Refinement](https://arxiv.org/abs/2512.24947)
...
Method: Caption-Prompt-Judge (CPJ) framework: 1) Generate multi-angle captions using large vision-language models, 2) Refine captions iteratively via LLM-as-Judge module, 3) Use refined captions in dual-answer VQA process for both disease recognition and management responses.

Result: On CDDMBench: Using GPT-5-mini captions, GPT-5-Nano achieves +22.7 percentage points in disease classification and +19.5 points in QA score over no-caption baselines.
...
TL;DR: SpaceTimePilot is a video diffusion model that independently controls camera viewpoint and motion sequence for generative rendering of monocular videos.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models lack explicit disentangled control over spatial (camera viewpoint) and temporal (motion sequence) dimensions, limiting flexible scene exploration.

Method: Introduces animation time-embedding for motion control, temporal-warping training scheme using multi-view datasets, improved camera-conditioning, and CamxTime synthetic dataset for full space-time coverage.

Result: Demonstrates clear space-time disentanglement on real-world and synthetic data, achieving strong results compared to prior work with precise temporal control.

Conclusion: SpaceTimePilot enables continuous arbitrary exploration across space and time for monocular videos through effective disentanglement of spatial and temporal dimensions.
...
TL;DR: ProDM is a generative diffusion model that removes motion artifacts from non-gated chest CT scans to improve coronary artery calcium scoring accuracy.


<details>
  <summary>Details</summary>
Motivation: CAC scoring from chest CT is important for cardiovascular risk assessment but is affected by motion artifacts. ECG-gated cardiac CTs reduce artifacts but are not widely available for population screening. Non-gated chest CTs are more accessible but suffer from severe motion artifacts that degrade CAC quantification accuracy.

Method: ProDM uses a three-component approach: (1) CAC motion simulation data engine that synthesizes realistic non-gated CTs from cardiac-gated CTs for supervised training without paired data, (2) property-aware learning with calcium-specific priors using a differentiable calcium consistency loss to preserve lesion integrity, and (3) progressive correction scheme that gradually reduces artifacts across diffusion steps for stability and calcium fidelity.

Result: Experiments show ProDM significantly improves CAC scoring accuracy, spatial lesion fidelity, and risk stratification performance compared to baselines. A reader study confirms ProDM suppresses motion artifacts and improves clinical usability of non-gated CT scans.

Conclusion: ProDM demonstrates the potential of progressive, property-aware generative frameworks for reliable CAC quantification from routine chest CT imaging, enabling more accessible cardiovascular risk assessment without requiring specialized ECG-gated scans.
...
<details>
  <summary>Details</summary>
Motivation: Existing video reasoning evaluations use single-frame assessments that allow outcome-hacking (correct conclusions via erroneous processes), lacking process-aware evaluation.

Method: Proposed VIPER benchmark with 16 tasks across 6 reasoning types, and Process-outcome Consistency (POC@r) metric using VLM-as-Judge with hierarchical rubric to evaluate intermediate steps and final results.

Result: State-of-the-art video models achieve only ~20% POC@1.0, show significant outcome-hacking, and reveal substantial gap between current video generation and true generalized visual reasoning.

Conclusion: Process-aware evaluation is crucial for assessing video reasoning capabilities; current models fall short of true reasoning; benchmark enables better evaluation of Chain-of-Frames reasoning.
...
TL;DR: Proposes RFL-ReID, a re-index free lifelong person re-identification task that eliminates the need to re-extract features for historical gallery images, addressing privacy and computational cost issues.


<details>
  <summary>Details</summary>
Motivation: Existing lifelong ReID methods require re-indexing (re-extracting features) of all historical gallery images after each model update, which raises privacy concerns and incurs high computational costs for large-scale datasets.

Method: Proposes Bi-C2R (Bidirectional Continuous Compatible Representation) framework that continuously updates gallery features extracted by old models to maintain compatibility between new and old model features without re-indexing.

Result: The method achieves leading performance on both the new RFL-ReID task and traditional L-ReID benchmarks through theoretical analysis and extensive experiments.

Conclusion: RFL-ReID is a more challenging but practical task that addresses privacy and efficiency concerns in lifelong person re-identification, with Bi-C2R providing an effective solution for compatible feature representation across model updates.
...
<details>
  <summary>Details</summary>
Motivation: Previous flow-based SLAM approaches lack geometric consistency, which is essential for accurate and robust tracking and mapping. The paper aims to bridge the gap between flow estimation and geometric reasoning.
...
TL;DR: Proposes a self-bootstrapping framework that reframes visual dubbing from inpainting to video editing using Diffusion Transformers to generate ideal training data and achieve precise lip synchronization.


<details>
  <summary>Details</summary>
Motivation: Existing audio-driven visual dubbing methods suffer from visual artifacts, identity drift, and poor synchronization due to the lack of ideal training data (paired videos with identical visual conditions but different lip movements). The mask-based inpainting paradigm forces models to simultaneously hallucinate missing content and sync lips, leading to poor results.

Method: A self-bootstrapping framework with two-stage approach: 1) Uses a Diffusion Transformer as data generator to synthesize ideal training data (lip-altered companion videos for real samples), creating visually aligned video pairs. 2) Trains a DiT-based audio-driven editor end-to-end on these pairs, leveraging complete frame-aligned input to focus solely on precise lip modifications. Includes timestep-adaptive multi-phase learning strategy to disentangle conflicting editing objectives across diffusion timesteps.

Result: Achieves highly accurate lip sync, faithful identity preservation, and exceptional robustness in challenging in-the-wild scenarios. The method outperforms existing approaches by providing complete visual context including identity cues, scene interactions, and spatiotemporal dynamics.

Conclusion: Reframing visual dubbing from ill-posed inpainting to well-conditioned video editing with self-bootstrapped training data enables superior performance. The approach fundamentally addresses core challenges through complete visual context and stable training strategies, with ContextDubBench introduced for robust evaluation.
...
Method: 1) Context-aware completion with diverse temporal masking to restore base skeleton sequences; 2) Skeleton-based spatial decomposition into 5 semantic regions, further divided into dynamic/static subgroups with targeted perturbation; 3) Physics-driven estimation using Lagrangian dynamics to compute joint accelerations; 4) GCN-based recognition head processing both fused skeleton positions and accelerations.

Result: Significantly outperforms previous methods on both coarse-grained (NTU-60, NTU-120) and fine-grained (Gym99, Gym288) benchmarks under various corruption levels. Achieves 89.1% top-1 accuracy on Gym99-severe and 78.1% on Gym288-severe settings.

Conclusion: FineTec effectively addresses fine-grained action recognition from corrupted skeleton data by combining temporal restoration, spatial decomposition, physics modeling, and graph-based learning, demonstrating strong robustness and generalizability across different datasets and corruption levels.
...
Method: Uses SAM2-based recoloring for cross-view-consistent supervision and asymmetric input strategy (recolored reference view + raw auxiliary views) to fuse disparate observations. Directly predicts instruction-aligned 3D edits without optimization.

Result: Achieves superior semantic alignment and enhanced 3D consistency compared to baselines, with significantly higher inference speed. Handles 2D-edited images (like InstructPix2Pix) despite not being trained on them. Introduces DL3DV-Edit-Bench benchmark for evaluation.

Conclusion: Edit3r enables fast, photorealistic 3D scene editing from unposed images, making it promising for real-time applications. The method effectively handles view inconsistencies and generalizes to unseen 2D editing methods.
...
TL;DR: GaMO is a geometry-aware multi-view outpainting framework for sparse-view 3D reconstruction that expands field of view from existing camera poses instead of generating new viewpoints, achieving better geometric consistency, broader coverage, and 25x speedup over SOTA diffusion methods.


<details>
  <summary>Details</summary>
Motivation: Current sparse-view 3D reconstruction methods have three critical limitations: inadequate coverage beyond known view peripheries, geometric inconsistencies across generated views, and computationally expensive pipelines. Diffusion-based methods that generate novel views have improved results but still suffer from these issues.

Method: GaMO reformulates sparse-view reconstruction through multi-view outpainting. Instead of generating new viewpoints, it expands the field of view from existing camera poses, preserving geometric consistency. The approach uses multi-view conditioning and geometry-aware denoising strategies in a zero-shot manner without training.

Result: Extensive experiments on Replica and ScanNet++ show state-of-the-art reconstruction quality across 3, 6, and 9 input views, outperforming prior methods in PSNR and LPIPS metrics. Achieves 25x speedup over SOTA diffusion-based methods with processing time under 10 minutes.

Conclusion: GaMO provides an effective solution to sparse-view 3D reconstruction by addressing geometric consistency, coverage, and efficiency limitations of existing methods through its novel multi-view outpainting approach that operates in zero-shot manner without training.
...
TL;DR: Process mining method analyzes gaming network traffic to characterize states, encode them as interpretable Petri nets, and classify different video games being played.


<details>
  <summary>Details</summary>
Motivation: Online gaming generates large market revenue and requires understanding network device behavior for bandwidth management, load prediction, and security. Process mining offers data-driven analysis with model-based insights for gaming network traffic.
...
Result: Applied to UPSIDE case study with Clash Royale and Rocket League data. Achieved 94.02% inter-device similarity (coherence), 174.99% inter-state separation (specificity), and 73.84% AUC for video game classification.
...
TL;DR: Large-scale empirical study of 16 state-of-the-art DL model fixing approaches shows model-level methods have best fixing effectiveness, but no single approach excels at fixing while maintaining all other properties like robustness, fairness, and backward compatibility.


<details>
  <summary>Details</summary>
Motivation: DL systems are widely used in critical domains but prone to faults that expose users to risks. Many fixing approaches exist, but their comprehensive performance across multiple dimensions needs systematic evaluation.

Method: Conducted large-scale empirical study of 16 state-of-the-art DL model fixing approaches across model-level, layer-level, and neuron-level categories. Used diverse datasets, model architectures, and application domains in uniform experimental setup.

Result: Model-level approaches show superior fixing effectiveness compared to others. No single approach achieves best fixing performance while improving accuracy and maintaining all other properties (robustness, fairness, backward compatibility).

Conclusion: Academia should prioritize research on mitigating side effects of DL model fixing approaches. The study provides insights for future exploration in DL model repair field.
...
TL;DR: A comprehensive review of diffusion-based simulation-based inference (SBI) methods that use diffusion models for likelihood-free posterior estimation in scientific applications with intractable likelihoods.


<details>
  <summary>Details</summary>
Motivation: Traditional likelihood-based inference fails for complex simulation problems with intractable likelihood functions. Simulation-based inference (SBI) methods are needed to learn posterior distributions directly from simulator samples without explicit likelihoods. Diffusion models offer a flexible framework for SBI tasks, particularly addressing robustness in non-ideal scientific data conditions.

Method: The review covers diffusion modeling foundations: forward noising, reverse-time SDE/ODE, probability flow, and denoising score matching. It explains how conditional scores enable likelihood-free posterior sampling. The paper examines diffusion models' advantages over normalizing flows and synthesizes methods including Schrodinger-bridge formulations, conditional/sequential posterior samplers, amortized architectures for unstructured data, and inference-time prior adaptation.

Result: The review demonstrates that diffusion-based SBI addresses key pain points in scientific inference: robustness to model misspecification, handling unstructured or infinite-dimensional observations, and dealing with missing data. It provides a framework that overcomes limitations of normalizing flows while introducing trade-offs like iterative sampling costs.

Conclusion: Diffusion models provide a robust framework for simulation-based inference in scientific applications, particularly for uncertainty quantification in probabilistic geophysical models. The review identifies open problems and emphasizes conditions required for accurate posterior estimation, highlighting diffusion-based SBI's potential for complex scientific inference tasks with intractable likelihoods.
...
TL;DR: CM² is a Green AI model that achieves human-level one-shot document classification by focusing on structural coordinates rather than semantic vectors, outperforming traditional methods with minimal data and compute.


<details>
  <summary>Details</summary>
Motivation: Humans learn concepts from single examples while ML requires hundreds of samples. Current AI trends rely on massive pre-training and energy-intensive infrastructure, creating need for efficient, human-like learning approaches.

Method: Coordinate Matrix Machine (CM²) learns document structures by identifying structural "important features" similar to human cognition. It uses structural coordinates rather than exhaustive semantic vectors for classification.
...
TL;DR: Modern language models preserve geometric structures that enable Bayesian inference, with value representations organizing along entropy-aligned axes similar to synthetic transformer models.


<details>
  <summary>Details</summary>
Motivation: To investigate whether the geometric signatures observed in small "wind-tunnel" transformers (low-dimensional value manifolds and orthogonal keys that encode posterior structure) persist in production-grade language models.

Method: Analyzed Pythia, Phi-2, Llama-3, and Mistral families to examine last-layer value representations. Performed targeted interventions on entropy-aligned axes in Pythia-410M during in-context learning, including removing or perturbing the axis versus random-axis interventions.

Result: Found that last-layer value representations organize along a single dominant axis strongly correlated with predictive entropy. Domain-restricted prompts collapse structure into low-dimensional manifolds similar to synthetic settings. Interventions on entropy-aligned axes disrupt local uncertainty geometry while random interventions don't, but single-layer manipulations don't proportionally degrade Bayesian-like behavior.

Conclusion: Modern language models preserve the geometric substrate enabling Bayesian inference observed in synthetic transformers. The geometry serves as a privileged readout of uncertainty rather than a singular computational bottleneck, with models organizing approximate Bayesian updates along this substrate.
...
TL;DR: EDL models with Subjective Logic have activation-dependent learning-freeze issues in low-evidence regions; paper proposes new activation functions and regularizers to fix this.


<details>
  <summary>Details</summary>
Motivation: Evidential deep learning models using Subjective Logic can quantify uncertainty efficiently, but the non-negative evidence constraint requires specific activation functions that cause learning-freeze behavior in low-evidence regions, limiting training effectiveness.

Method: Theoretically analyze activation-dependent learning-freeze behavior, design a general family of activation functions and corresponding evidential regularizers that enable consistent evidence updates across different activation regimes.
...
Conclusion: The proposed activation functions and regularizers solve the learning-freeze problem in evidential models, enabling more effective training and uncertainty quantification across diverse applications.
...
<details>
  <summary>Details</summary>
Motivation: Human decision-making, emotions, and collective psychology shape financial/economic systems, but current approaches rely on expensive external data (news, social media) with high financial, computational, and practical costs.

Method: HINTS uses Friedkin-Johnsen opinion dynamics model as structural inductive bias to model evolving social influence, memory, and bias patterns from time series residuals. Extracted human factors are integrated into a state-of-the-art backbone model as an attention map.

Result: Experiments on nine real-world and benchmark datasets show HINTS consistently improves forecasting accuracy. Case studies and ablation studies validate interpretability, showing strong semantic alignment between extracted factors and real-world events.

Conclusion: HINTS provides a practical, cost-effective approach to capturing human factors in time series forecasting without external data, offering both improved accuracy and interpretable insights into real-world events.
...
TL;DR: MUSIC is a sparsity-induced multitask neural network framework that integrates partial physics constraints with data-driven learning to recover full solutions of coupled systems when physics and data are available for different variables.
...
Method: MUSIC uses a sparsity-induced multitask neural network framework that integrates partial physical constraints with data-driven learning. It employs mesh-free random sampling of training data and sparsity regularization to yield highly compressed models with improved training and evaluation efficiency.

Result: MUSIC accurately learns solutions to complex coupled systems (including shock wave solutions, discontinuous solutions, and pattern formation solutions) under data-scarce and noisy conditions, consistently outperforming non-sparse formulations.
...
TL;DR: A framework for evaluating dataset stability in network traffic classification using ML, with concept drift detection enhanced by feature weights, demonstrated on CESNET-TLS-Year22 dataset.


<details>
  <summary>Details</summary>
Motivation: ML models for network traffic classification degrade quickly due to data/concept drift from evolving protocols and traffic behavior changes. Current practice involves complete retraining without investigating root causes or dataset quality issues.

Method: Proposes a novel methodology for evaluating dataset stability and a benchmark workflow. Framework uses concept drift detection method enhanced with ML feature weights to boost detection performance.

Result: Demonstrated on CESNET-TLS-Year22 dataset, providing initial dataset stability benchmark to describe stability and identify weak points. Shows optimization impact on created dataset variants using the benchmarking methodology.

Conclusion: The proposed framework enables systematic evaluation of dataset stability for network traffic classification, helping identify optimization needs rather than blindly retraining models when performance drops occur.
...
TL;DR: Neural Optimal Design of Experiments (NODE) is a learning-based framework for optimal experimental design that jointly trains neural reconstruction models with continuous design variables in a single optimization loop, avoiding traditional bilevel optimization and sparsity regularization.


<details>
  <summary>Details</summary>
Motivation: Traditional optimal experimental design methods rely on bilevel optimization and indirect sparsity regularization, which are computationally expensive and require tuning of l1 regularization parameters. There's a need for a more efficient approach that directly optimizes measurement locations while enforcing sparsity naturally.

Method: NODE jointly trains a neural reconstruction model and continuous design variables (representing sensor locations, sampling times, or measurement angles) within a single optimization loop. Instead of weighting dense grid candidates, it directly optimizes measurement locations, enforcing sparsity by design and eliminating l1 tuning.

Result: NODE outperforms baseline approaches on three test cases: an analytically tractable exponential growth benchmark, MNIST image sampling, and a real-world sparse view X-ray CT example. It demonstrates improved reconstruction accuracy and task-specific performance while reducing computational complexity.

Conclusion: NODE provides an effective learning-based framework for optimal experimental design that avoids classical bilevel optimization and indirect sparsity regularization, offering improved performance, reduced computational complexity, and eliminating the need for l1 tuning parameters.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods have limitations: conventional spline-based approaches require repeated data transformations and struggle with large datasets, while neural network methods focus on accuracy but lack interpretability of cumulative exposure patterns.

Method: CENNSurv is a novel deep learning approach that captures dynamic risk relationships from time-dependent data, designed to model complex temporal patterns while providing interpretable insights.
...
Conclusion: CENNSurv provides researchers with a practical tool for studying cumulative effects of time-dependent exposures, offering improved scalability and interpretable insights into complex temporal patterns.
...
TL;DR: New subspace clustering algorithm using Schubert Variety of Best Fit (SVBF) as trainable prototypes instead of subspace means, integrated into LBG pipeline for improved cluster purity.


<details>
  <summary>Details</summary>
Motivation: Need better geometric representatives for subspace datasets that preserve mathematical structure while improving clustering performance. Traditional subspace means on Grassmann/flag manifolds may not be optimal for clustering tasks.

Method: Introduces SVBF-LBG algorithm: replaces subspace means with trainable Schubert Variety of Best Fit prototypes - subspaces that aim to intersect each cluster member in at least one fixed direction. Integrated into Linde-Buzo-Grey (LBG) clustering pipeline.

Result: Improved cluster purity on synthetic, image, spectral, and video action datasets while retaining mathematical structure needed for downstream analysis.

Conclusion: SVBF prototypes provide better geometric representatives for subspace clustering than traditional means, offering both performance improvements and mathematical interpretability for subsequent analysis tasks.
...
TL;DR: MERINDA is an FPGA-accelerated model recovery framework that replaces Neural ODEs with hardware-friendly components, achieving 114× lower energy, 28× smaller memory, and 1.68× faster training while maintaining accuracy for edge AI applications.


<details>
  <summary>Details</summary>
Motivation: Physical AI at the edge requires hardware-efficient learning for autonomous systems, but current model recovery methods rely on Neural ODEs that are computationally expensive and difficult to accelerate on edge hardware with strict latency, compute, and power constraints.

Method: MERINDA replaces Neural ODE components with: (1) GRU-based discretized dynamics, (2) dense inverse-ODE layers, (3) sparsity-driven dropout, and (4) lightweight ODE solvers. The framework structures computation for streaming parallelism to enable full parallelization of critical kernels on FPGAs.

Result: Across four benchmark nonlinear dynamical systems, MERINDA achieves 114× lower energy (434J vs. 49,375J), 28× smaller memory footprint (214MB vs. 6,118MB), and 1.68× faster training while matching state-of-the-art model-recovery accuracy compared to GPU implementations.

Conclusion: MERINDA demonstrates that accurate, explainable model recovery can be brought to the edge for real-time monitoring of autonomous systems, making physical AI practical on resource-constrained devices through FPGA acceleration and hardware-friendly formulations.
...
TL;DR: SB-TRPO is a new trust-region RL algorithm that adaptively biases policy updates toward safety constraint satisfaction while maintaining reward performance, achieving better safety-reward balance than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current RL methods for safety-critical domains either fail to ensure near-zero safety violations or sacrifice reward performance when dealing with hard constraints. There's a need for algorithms that can maximize rewards while strictly adhering to safety constraints.

Method: Safety-Biased Trust Region Policy Optimisation (SB-TRPO) uses trust-region updates with a convex combination of natural policy gradients for both cost (safety) and reward. It ensures a fixed fraction of optimal cost reduction at each step while adaptively biasing updates toward constraint satisfaction.

Result: Experiments on Safety Gymnasium tasks show SB-TRPO consistently achieves the best balance of safety and meaningful task completion compared to state-of-the-art methods, with theoretical guarantees of local progress toward safety.

Conclusion: SB-TRPO provides an effective solution for hard-constrained RL that maintains both safety and reward performance, addressing limitations of existing Lagrangian and projection-based approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing RL methods for quantitative trading focus on spot markets and fail to handle crypto futures markets with high leverage due to two key challenges: 1) high leverage amplifies reward fluctuations making training unstable, and 2) lack of self-awareness about capability boundaries exposes systems to significant losses during black swan events like COVID-19.

Method: FineFT uses a three-stage ensemble RL framework: Stage I - ensemble Q learners are selectively updated using ensemble TD errors to improve convergence; Stage II - filter Q-learners based on profitability and train VAEs on market states to identify capability boundaries; Stage III - choose between filtered ensemble and conservative policy using trained VAEs to maintain profitability while mitigating risk with new market states.

Result: FineFT outperforms 12 state-of-the-art baselines across 6 financial metrics in high-frequency crypto futures trading with 5x leverage, reducing risk by more than 40% while achieving superior profitability compared to the runner-up. Visualization shows agents specialize in distinct market dynamics, and ablation studies confirm VAE routing reduces maximum drawdown while selective updates improve convergence and performance.

Conclusion: FineFT successfully addresses the challenges of high-leverage futures trading through its novel three-stage ensemble RL framework with stable training, proper risk management, and self-awareness of capability boundaries, making it effective for volatile crypto markets and resilient to black swan events.
...
TL;DR: This paper reviews Graph Neural Network approaches for fraud detection in ride-hailing platforms, comparing different models and addressing challenges like class imbalance and fraud camouflage.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve fraud detection in ride-hailing platforms by leveraging Graph Neural Networks to address prevalent fraudulent activities, class imbalance issues, and sophisticated fraud camouflage techniques that traditional methods struggle to detect.

Method: The paper conducts a comparative analysis of various GNN models for fraud detection, provides a structured overview of GNN architectures and methodologies for anomaly detection, and examines approaches to handle class imbalance and fraudulent camouflage in ride-hailing data.

Result: The research identifies significant methodological progress in GNN-based fraud detection while highlighting existing gaps, and provides comparative insights into the effectiveness of different GNN models for detecting fraudulent activities in ride-hailing platforms.

Conclusion: The paper concludes that GNNs show promise for fraud detection in ride-hailing but calls for further exploration into real-world applicability and technical improvements to enhance fraud detection strategies in this rapidly evolving industry.
...
TL;DR: TabMixNN is a PyTorch framework that combines classical mixed-effects modeling with neural networks for tabular data, supporting hierarchical structures, diverse outcome types, and offering interpretability tools.


<details>
  <summary>Details</summary>
Motivation: There's a growing need for methods that can handle hierarchical data structures while supporting diverse outcome types (regression, classification, multitask learning) and maintaining interpretability, bridging classical statistical models with modern deep learning approaches.
...
Result: Demonstrated framework flexibility through applications to longitudinal data analysis, genomic prediction, and spatial-temporal modeling, showing it can handle complex hierarchical data structures while maintaining interpretability.

Conclusion: TabMixNN provides a unified interface that allows researchers to leverage deep learning capabilities while maintaining the interpretability and theoretical grounding of classical mixed-effects models, bridging the gap between statistical modeling and modern neural networks.
...
<details>
  <summary>Details</summary>
Motivation: Recent attacks on critical infrastructure (2021 Oldsmar water treatment, 2023 Danish energy sector) reveal urgent security gaps in Industrial IoT deployments. Existing federated learning frameworks for intrusion detection are vulnerable to Byzantine poisoning attacks and lack robust agent authentication mechanisms.

Method: Zero-Trust Agentic Federated Learning (ZTA-FL) combines three key components: (1) TPM-based cryptographic attestation with extremely low false acceptance rate, (2) a novel SHAP-weighted aggregation algorithm for explainable Byzantine detection under non-IID conditions with theoretical guarantees, and (3) privacy-preserving on-device adversarial training.
...
Conclusion: ZTA-FL provides a defense-in-depth framework for secure IIoT intrusion detection that addresses Byzantine attacks through zero-trust principles, maintains privacy through federated learning, and offers theoretical guarantees and practical performance improvements over existing methods.
...
TL;DR: The paper establishes theoretical bounds for private and robust alignment of language models in both offline and online settings, analyzing privacy constraints and adversarial corruption.


<details>
  <summary>Details</summary>
Motivation: To provide theoretical foundations for aligning language models under both privacy constraints and adversarial corruption, addressing the gap in understanding the interplay between these two challenges.

Method: Theoretical analysis using uniform convergence guarantees for log loss and square loss under privacy and corruption. Analyzes two scenarios: privacy-first and corruption-first interplays. Uses MLE-style algorithm for privacy-only setting and examines existing offline algorithms for joint settings.

Result: 1) For privacy-only setting: log loss with MLE-style algorithm achieves near-optimal rates. 2) For joint privacy-and-corruption: existing offline algorithms provide stronger guarantees than previously known. 3) First results for private and robust online alignment. 4) New uniform convergence guarantees for log loss and square loss.

Conclusion: The paper establishes theoretical bounds for private and robust alignment, showing that log loss with MLE can achieve near-optimal rates under privacy, and that existing algorithms are more robust than previously thought. The new uniform convergence guarantees have broad applicability in learning theory and statistics.
...
TL;DR: MS-SSM introduces a multi-scale state-space model framework that captures dependencies across multiple resolutions, improving memory efficiency and long-range modeling while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional SSMs have limited effective memory and struggle with multi-scale dependencies needed for complex structures in time series, images, and natural language. They require larger state sizes for better recall and cannot capture both fine-grained and coarse patterns effectively.

Method: A multi-scale SSM framework that represents sequence dynamics across multiple resolutions, with each resolution processed by specialized state-space dynamics. Includes an input-dependent scale-mixer for dynamic information fusion across resolutions.

Result: MS-SSM consistently outperforms prior SSM-based models on benchmarks including Long Range Arena, hierarchical reasoning, time series classification, and image recognition, demonstrating improved sequence modeling particularly in long-range and hierarchical tasks.

Conclusion: Multi-resolution processing in state-space architectures significantly enhances memory efficiency and long-range modeling capabilities, making MS-SSM a superior alternative to traditional SSMs for complex sequence modeling tasks.
...
<details>
  <summary>Details</summary>
Motivation: Previous generative methods for time series imputation (using diffusion models or Schrodinger bridges) suffer from limited accuracy because their priors (Gaussian noise or linear interpolation) are not informative enough about the ground-truth target values.

Method: Bridge-TS builds a data-to-data generation process with two novel designs: 1) Expert prior - uses a pretrained transformer-based module to provide deterministic estimations as prior, and 2) Compositional priors - combines multiple pretrained models' estimations in the generation process for compositional priors-to-target imputation.

Result: Experiments on benchmark datasets (ETT, Exchange, Weather) show Bridge-TS achieves new state-of-the-art imputation accuracy in terms of mean square error and mean absolute error.

Conclusion: Improving the prior design in generative time series imputation through expert and compositional priors significantly enhances imputation accuracy, demonstrating the superiority of data-to-data generation approaches over previous methods.
...
Method: Replaces standard KV cache with fixed-size memory and trains a two-pass recurrent compression mechanism that uses online gradient descent with a forget gate to dynamically compress and update memory at test time.

Result: Extensive experiments on language modeling, common-sense reasoning, recall-intensive tasks, and time series show Trellis outperforms strong baselines, with performance gains increasing as sequence length grows.

Conclusion: Trellis demonstrates potential for long-context applications by providing bounded memory Transformer architecture with dynamic KV cache compression that maintains important contextual information.
...
<details>
  <summary>Details</summary>
Motivation: To improve upon existing Neural Process models by leveraging flow matching, a powerful generative modeling approach, to create a simpler, more efficient model for stochastic process learning and conditional sampling.

Method: Proposes a Neural Process model based on flow matching that provides amortized predictions of conditional distributions over arbitrary points. Uses ODE solver for sampling from conditional distributions without requiring auxiliary conditioning methods.
...
Conclusion: The flow matching-based Neural Process model offers a simpler implementation, efficient ODE-based sampling, controllable accuracy-speed tradeoff, and superior performance across diverse benchmarks compared to previous approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing speculative decoding systems suffer from suboptimal performance due to a mismatch between dynamic speculation and static runtime assumptions, limiting their effectiveness in LLM inference acceleration.

Method: Yggdrasil introduces: 1) equal-growth tree structure for static graph compatibility, 2) latency-aware optimization objective for draft selection, and 3) stage-based scheduling to reduce overhead. It supports unmodified LLMs through context-aware tree drafting and compiler-friendly execution.
...
Conclusion: Yggdrasil demonstrates that co-designing speculative decoding systems with context-aware tree drafting and compiler-friendly execution enables latency-optimal performance, significantly advancing LLM inference efficiency.
...
TL;DR: Infini-attention improves long-context capabilities in small language models (300M params) by using compressed memory from past segments, achieving up to 31% higher accuracy than baseline despite some degradation at extreme lengths.


<details>
  <summary>Details</summary>
Motivation: Enable efficient small language models for low-resource settings by improving long-context extrapolation capabilities while maintaining computational efficiency and reducing costs.
...
Result: Infini-attention demonstrates training stability and outperforms baseline in long-context retrieval, achieving up to 31% higher accuracy despite some degradation at 16,384-token contexts. Balance factor is identified as key to performance.

Conclusion: Architectural memory like Infini-attention effectively compensates for SLMs' limited parameters and is beneficial for achieving robust long-context capabilities in small language models.
...
TL;DR: SAC algorithm enhanced with flow-based policies using online flow matching (ISFM) for better expressiveness and optimal action distribution learning.


<details>
  <summary>Details</summary>
Motivation: Standard SAC uses simple policy approximations for efficiency, sacrificing expressiveness and robustness. Flow-based models offer rich expressiveness but need efficient training methods.

Method: Proposes SAC variant with flow-based policies using instantaneous change-of-variable technique. Introduces ISFM (importance sampling flow matching) for online policy updates using only samples from a user-specified distribution rather than unknown target distribution.

Result: Develops theoretical analysis of ISFM showing how sampling distribution choices affect learning efficiency. Case study on max-entropy LQR problems demonstrates the algorithm learns optimal action distribution.

Conclusion: Flow-based SAC with ISFM enables expressive policies while maintaining efficient training, theoretically analyzed and empirically validated on LQR problems.
...
TL;DR: Transformer model achieves best solar irradiance forecasting accuracy, with knowledge distillation enabling efficient deployment on edge devices.
...
Method: Comprehensive benchmark of 10 deep learning architectures (LSTM, TCN, Transformer, Informer, iTransformer, TSMixer, Mamba) for 1-hour ahead GHI forecasting using NSRDB satellite data (2011-2020) from Ho Chi Minh City, with SHAP analysis for temporal reasoning and knowledge distillation for model compression.

Result: Transformer achieved highest predictive accuracy with R² of 0.9696; SHAP analysis revealed Transformer's recency bias vs Mamba's 24-hour periodic dependency utilization; Knowledge distillation compressed Transformer by 23.5% while reducing MAE to 23.78 W/m².

Conclusion: Transformer is superior for GHI forecasting, knowledge distillation enables efficient edge deployment, and different architectures exhibit distinct temporal reasoning patterns that inform their predictive capabilities.
...
### [134] [CPR: Causal Physiological Representation Learning for Robust ECG Analysis under Distribution Shifts](https://arxiv.org/abs/2512.24564)
*Shunbo Jia,Caizhi Liao*

Main category: cs.LG

TL;DR: CPR (Causal Physiological Representation Learning) is a defense method for ECG diagnosis models that uses causal disentanglement to separate invariant pathological features from artifacts, achieving robust performance against adversarial attacks without computational overhead.


<details>
  <summary>Details</summary>
Motivation: ECG diagnosis models are vulnerable to adversarial perturbations (especially SAP) that mimic biological morphology. Current defenses face trade-offs: adversarial training is computationally expensive, while certified methods like randomized smoothing add inference latency, making them impractical for real-time clinical use.

Method: CPR incorporates a Physiological Structural Prior within a causal disentanglement framework using a Structural Causal Model (SCM). It models ECG generation and enforces structural intervention to strictly separate invariant pathological morphology (P-QRS-T complex) from non-causal artifacts, unlike standard denoising methods.

Result: On PTB-XL dataset, CPR achieves F1 score of 0.632 under SAP attacks, outperforming Median Smoothing (0.541 F1) by 9.1%. It matches the certified robustness of Randomized Smoothing while maintaining single-pass inference efficiency.

Conclusion: CPR offers a superior trade-off between robustness, efficiency, and clinical interpretability by addressing the root cause of vulnerability (reliance on spurious correlations) through causal representation learning, making it practical for real-time clinical monitoring.
...
### [135] [Rethinking Dense Linear Transformations: Stagewise Pairwise Mixing (SPM) for Near-Linear Training in Neural Networks](https://arxiv.org/abs/2512.23905)
*Peter Farag*

Main category: cs.LG

TL;DR: SPM replaces dense linear layers with sparse pairwise-mixing stages, reducing computational cost from O(n²) to O(nL) while introducing compositional inductive bias for better generalization on structured tasks.


<details>
  <summary>Details</summary>
Motivation: Dense linear layers are computationally expensive (quadratic complexity) and often misaligned with the compositional structure of learned representations, creating a need for more efficient and structurally aligned alternatives.

Method: Stagewise Pairwise Mixers (SPM) replace dense matrices with compositions of sparse pairwise-mixing stages. The method offers two parameterizations: orthogonal norm-preserving rotation-based variant and fully general 2×2 mixing variant, with exact closed-form forward/backward computations.

Result: SPM achieves O(nL) time and parameters (vs O(n²) for dense layers), with L typically constant or log₂n. Proof-of-concept experiments show substantial wall-clock cost reductions and improved accuracy on structured learning problems while maintaining competitive performance on real-world benchmarks.

Conclusion: SPM provides an efficient drop-in replacement for dense linear layers that not only reduces computational costs but also introduces beneficial compositional inductive bias for improved generalization on structured tasks.
...
### [136] [Constraint Breeds Generalization: Temporal Dynamics as an Inductive Bias](https://arxiv.org/abs/2512.23916)
*Xia Chen*

Main category: cs.LG

TL;DR: Physical constraints in biological systems act as temporal inductive bias that promotes generalization in neural networks through proper dissipative dynamics that compress phase space and abstract invariant features.


<details>
  <summary>Details</summary>
Motivation: Conventional deep learning focuses on unconstrained optimization, but biological systems operate under strict metabolic constraints. The authors propose that these physical constraints aren't limitations but rather shape dynamics to function as temporal inductive bias that breeds generalization.

Method: Phase-space analysis of signal propagation reveals fundamental asymmetry: expansive dynamics amplify noise, while proper dissipative dynamics compress phase space aligning with network's spectral bias. This condition can be imposed externally via input encoding or intrinsically through network's temporal dynamics. Requires architectures capable of temporal integration and proper constraints to decode induced invariants.

Result: Comprehensive evaluations across supervised classification, unsupervised reconstruction, and zero-shot reinforcement learning demonstrate that a critical "transition" regime maximizes generalization capability.

Conclusion: Dynamical constraints establish a distinct class of inductive bias, suggesting robust AI development requires not just scaling and removing limitations, but computationally mastering temporal characteristics that naturally promote generalization.
...
### [137] [Interactive Machine Learning: From Theory to Scale](https://arxiv.org/abs/2512.23924)
*Yinglun Zhu*
...
<details>
  <summary>Details</summary>
Motivation: Traditional ML requires large labeled datasets or extensive trial-and-error, which is expensive, time-consuming, or risky in large-scale/high-stakes settings. Interactive learning can actively guide information collection to reduce these costs.

Method: Develops new algorithmic principles and establishes fundamental limits for interactive learning across three dimensions: 1) active learning with noisy data and rich model classes, 2) sequential decision making with large action spaces, and 3) model selection under partial feedback.
...
Conclusion: Advances theoretical foundations of interactive learning with statistically optimal and computationally efficient algorithms, providing principled guidance for deploying interactive learning methods in large-scale real-world settings.
...
### [138] [Improved Balanced Classification with Theoretically Grounded Loss Functions](https://arxiv.org/abs/2512.23947)
*Corinna Cortes,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: The paper introduces two advanced surrogate loss families (GLA and GCA) for imbalanced classification, showing GCA has stronger theoretical guarantees with better scaling (1/√p_min vs 1/p_min) and both outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: Balanced loss is important for fairness in imbalanced classification but is intractable to minimize directly, requiring effective surrogate losses. Existing approaches like Logit-Adjusted and class-weighted losses have limitations in theoretical guarantees.

Method: Introduces two families: Generalized Logit-Adjusted (GLA) losses that extend Logit-Adjusted to general cross-entropy family, and Generalized Class-Aware weighted (GCA) losses that extend class-weighted losses with class-dependent confidence margins to general cross-entropy family.

Result: Theoretical analysis shows GLA losses are Bayes-consistent but only H-consistent for complete hypothesis sets with bounds scaling as 1/p_min. GCA losses are H-consistent for any hypothesis set with better scaling as 1/√p_min. Empirical results show both outperform existing methods, with GLA slightly better in common benchmarks and GCA better in highly imbalanced settings.

Conclusion: GCA losses offer significantly stronger theoretical guarantees for imbalanced classification with favorable scaling properties, while both GLA and GCA provide practical improvements over existing surrogate losses for balanced classification objectives.
...
### [139] [DivQAT: Enhancing Robustness of Quantized Convolutional Neural Networks against Model Extraction Attacks](https://arxiv.org/abs/2512.23948)
...
<details>
  <summary>Details</summary>
Motivation: Quantized CNNs are vulnerable to extraction attacks (IP theft), but existing defenses are limited - they're added post-training, computationally expensive, make unrealistic assumptions, and don't work well with quantized models on edge devices.

Method: DivQAT modifies the quantization process during training to integrate extraction defense directly into the model design, using Quantization Aware Training (QAT) as a foundation but enhancing it with defense mechanisms.

Result: Empirical validation on benchmark vision datasets shows DivQAT effectively defends against model extraction attacks without compromising model accuracy, and when combined with other defenses, outperforms traditional QAT.

Conclusion: DivQAT is the first technique to modify quantization for extraction defense, providing an effective, accuracy-preserving solution for protecting quantized CNNs on edge devices against IP theft.
...
### [140] [Physics-informed Graph Neural Networks for Operational Flood Modeling](https://arxiv.org/abs/2512.23964)
...
<details>
  <summary>Details</summary>
Motivation: Physics-based numerical flood models are accurate but computationally expensive, limiting their use in operational settings where rapid predictions are needed. Graph neural networks offer speed and accuracy while handling unstructured spatial domains, and can be combined with physics-informed techniques for better interpretability.
...
Conclusion: The proposed DUALFloodGNN architecture successfully combines physics-informed constraints with graph neural networks for efficient and accurate flood modeling, making it suitable for operational disaster management where rapid predictions are essential. The model is open-sourced for community use.
...
### [141] [Causify DataFlow: A Framework For High-performance Machine Learning Stream Computing](https://arxiv.org/abs/2512.23977)
*Giacinto Paolo Saggese,Paul Smith*

Main category: cs.LG

TL;DR: DataFlow is a unified computational framework for building ML systems on unbounded time-series data that ensures identical execution in both batch and streaming modes without code changes.


<details>
  <summary>Details</summary>
Motivation: Traditional data science workflows assume finite datasets and require substantial reimplementation when moving from batch prototypes to streaming production systems, leading to causality violations, batch boundary artifacts, and poor reproducibility of real-time failures.

Method: Uses a unified execution model based on DAGs with point-in-time idempotency, where outputs at any time t depend only on a fixed-length context window preceding t. Enforces strict causality by automatically tracking knowledge time across transformations, supports flexible tiling across temporal and feature dimensions, and integrates with Python data science stack.

Result: Demonstrates effectiveness across domains including financial trading, IoT, fraud detection, and real-time analytics, enabling models developed in batch mode to execute identically in streaming production without code changes.

Conclusion: DataFlow resolves the gap between batch prototyping and streaming production by providing a unified framework that eliminates causality violations and ensures reproducibility, making it easier to build, test, and deploy high-performance ML systems on unbounded time-series data.
...
Conclusion: The proposed operations research framework addresses the autonomy paradox by providing formal mechanisms for verifiable feasibility, robustness, and stress testing in autonomous GenAI systems, establishing a research agenda for assured autonomy in safety-critical operational domains.
...
### [143] [Information-Theoretic Quality Metric of Low-Dimensional Embeddings](https://arxiv.org/abs/2512.23981)
*Sebastián Gutiérrez-Bernal,Hector Medel Cobaxin,Abiel Galindo González*

Main category: cs.LG

TL;DR: The paper introduces ERPM, an information-theoretic metric for evaluating low-dimensional embeddings that measures information preservation through Shannon entropy and stable rank analysis of neighborhood matrices, complementing existing distance-based and geometric metrics.


<details>
  <summary>Details</summary>
Motivation: Existing embedding evaluation metrics (stress, rank-based criteria, Local Procrustes) focus on distance distortions or local geometries but don't directly assess information preservation when projecting high-dimensional data to lower dimensions. There's a need for metrics that explicitly measure information loss in dimensionality reduction.

Method: Introduces Entropy Rank Preservation Measure (ERPM) based on Shannon entropy of singular-value spectrum of neighborhood matrices and stable rank. ERPM quantifies changes in uncertainty between original and projected representations, providing both local neighborhood-level indicators and global summary statistics.
...
Conclusion: ERPM complements existing embedding evaluation metrics by providing an information-theoretic perspective that identifies specific neighborhoods with information loss. This enables more comprehensive assessment of embeddings, especially valuable for information-sensitive applications like early-warning indicator construction.
...
### [144] [Tracing the Heart's Pathways: ECG Representation Learning from a Cardiac Conduction Perspective](https://arxiv.org/abs/2512.24002)
...
<details>
  <summary>Details</summary>
Motivation: Existing ECG self-supervised learning methods focus on consistent patterns but overlook inherent heartbeat differences rooted in cardiac conduction processes, and fail to align with clinical diagnostic guidelines that progress from individual heartbeats to lead combinations.

Method: Two-stage framework: 1) CLEAR (Conduction-LEAd Reconstructor) uses sparse attention to reconstruct signals while treating each heartbeat as distinct entity, capturing specific variations and general commonalities. 2) HUG (Hierarchical lead-Unified Group head) implements hierarchical diagnosis following clinical workflow.

Result: Experimental results across six tasks show 6.84% improvement, validating effectiveness in enhancing cardiac conduction representations and aligning patterns with expert diagnostic guidelines.

Conclusion: CLEAR-HUG successfully addresses limitations of previous ECG self-supervised learning by capturing subtle conduction variations and following clinical diagnostic logic, demonstrating improved performance and better alignment with medical practice.
...
### [145] [Hyperspherical Graph Representation Learning via Adaptive Neighbor-Mean Alignment and Uniformity](https://arxiv.org/abs/2512.24062)
*Rui Chen,Junjun Guo,Hongbin Wang,Yan Xiang,Yantuan Xian,Zhengtao Yu*

Main category: cs.LG

TL;DR: HyperGRL is a hyperspherical graph representation learning framework using adaptive neighbor-mean alignment and sampling-free uniformity for stable, high-quality embeddings without complex architectures or negative sampling.
...
Method: HyperGRL embeds nodes on a unit hypersphere using two adversarially coupled objectives: neighbor-mean alignment (using local neighborhood mean representations as stable targets) and sampling-free uniformity (L2-based hyperspherical regularization for uniform embedding distribution). Includes entropy-guided adaptive balancing mechanism for dynamic regulation.

Result: Extensive experiments on node classification, node clustering, and link prediction show superior representation quality and generalization across diverse graph structures, achieving average improvements of 1.49%, 0.86%, and 0.74% over strongest existing methods.

Conclusion: HyperGRL demonstrates the effectiveness of geometrically grounded, sampling-free contrastive objectives for graph representation learning, providing stable training and high-quality embeddings without complex negative sampling strategies.
...
### [146] [How and Why LLMs Generalize: A Fine-Grained Analysis of LLM Reasoning from Cognitive Behaviors to Low-Level Patterns](https://arxiv.org/abs/2512.24063)
*Haoyue Bai,Yiyou Sun,Wenjie Hu,Shi Qiu,Maggie Ziyu Huan,Peiyang Song,Robert Nowak,Dawn Song*

Main category: cs.LG

TL;DR: The paper introduces a novel benchmark that decomposes reasoning into atomic core skills to study why supervised fine-tuning narrows LLM capabilities while reinforcement learning preserves them, revealing RL models maintain stable reasoning skills while SFT models overfit to surface patterns.


<details>
  <summary>Details</summary>
Motivation: LLMs show puzzling generalization differences: supervised fine-tuning (SFT) narrows capabilities while reinforcement learning (RL) tuning preserves them. Prior studies relied on coarse accuracy metrics, leaving the reasons for this divergence unclear. The authors aim to understand what constitutes reasoning in LLMs and how different training methods affect reasoning skills.

Method: 1. Introduce a novel benchmark that decomposes reasoning into atomic core skills: calculation, fact retrieval, simulation, enumeration, and diagnostic. 2. Use meta-probing framework to track model behavior at different training stages. 3. Analyze low-level statistical patterns including distributional divergence and parameter statistics. 4. Study generalization evolution under SFT and RL across mathematical, scientific reasoning, and non-reasoning tasks.
...
Conclusion: This work provides new insights into the nature of reasoning in LLMs and points toward principles for designing training strategies that foster broad, robust generalization. The findings suggest RL methods better preserve reasoning capabilities compared to SFT approaches.
...
### [147] [Time-varying Mixing Matrix Design for Energy-efficient Decentralized Federated Learning](https://arxiv.org/abs/2512.24069)
*Xusheng Zhang,Tuan Nguyen,Ting He*

Main category: cs.LG

TL;DR: DFL mixing matrix design to minimize max per-node energy consumption in wireless networks, using time-varying topologies to balance energy use and convergence rate.


<details>
  <summary>Details</summary>
Motivation: Existing mixing matrix designs focus on minimizing communication time, but neglect per-node energy consumption which is critical for energy-constrained devices in wireless DFL.

Method: Propose multi-phase design framework with time-varying communication topologies under optimized budgets, based on novel convergence theorem allowing arbitrarily time-varying mixing matrices.

Result: Validated efficacy through real data evaluations, showing ability to combine low energy consumption of sparse mixing matrices with fast convergence of dense mixing matrices.

Conclusion: The proposed solution addresses energy minimization gap in DFL mixing matrix design, optimizing energy consumption while maintaining convergence performance in wireless networks.
...
### [148] [Multi-Scenario Highway Lane-Change Intention Prediction: A Temporal Physics-Informed Multi-Modal Framework](https://arxiv.org/abs/2512.24075)
*Jiazhao Shi,Ziyu Wang,Yichen Lin,Shoufeng Lu*

Main category: cs.LG

TL;DR: TPI-AI: A hybrid framework combining deep temporal representations with physics-inspired interaction features for robust lane-change intention prediction across heterogeneous highway scenarios.


<details>
  <summary>Details</summary>
Motivation: Lane-change intention prediction is safety-critical for autonomous driving but remains challenging due to noisy kinematics, severe class imbalance, and limited generalization across different highway scenarios.

Method: Temporal Physics-Informed AI (TPI-AI) fuses deep temporal representations (two-layer Bi-LSTM encoder for trajectory histories) with physics-inspired interaction features (kinematics, safety, interaction-aware features like headway, TTC, safe-gap indicators). Uses LightGBM classifier with imbalance-aware optimization including resampling/weighting and fold-wise threshold calibration.

Result: Outperforms standalone LightGBM and Bi-LSTM baselines on highD (straight highways) and exiD (ramp-rich environments) datasets. Achieves macro-F1 scores: highD: 0.9562, 0.9124, 0.8345 at T=1,2,3s; exiD: 0.9247, 0.8197, 0.7605 at T=1,2,3s.

Conclusion: Combining physics-informed interaction features with learned temporal embeddings yields robust multi-scenario lane-change intention prediction, demonstrating effectiveness across heterogeneous highway environments.
...
TL;DR: Ablation study shows latent autoregression in GP-VAE models produces more stable, Gaussian-process-compatible latent trajectories compared to non-autoregressive variants, highlighting its role in organizing long-range structure.


<details>
  <summary>Details</summary>
Motivation: To systematically analyze the role of latent autoregression in GP-VAE models, comparing it against non-autoregressive latent variables and standard token-level autoregressive Transformers to understand how latent sequential structure affects representation learning.

Method: Conducted ablation study comparing three models: (1) full GP-VAE with autoregressive latent dynamics using causal Gaussian process, (2) non-autoregressive ablation with independent latent variables, and (3) standard token-level autoregressive Transformer. Experiments conducted on medium-scale corpora with short training contexts.

Result: Latent autoregression induces latent trajectories significantly more compatible with Gaussian-process prior and exhibits greater long-horizon stability. Removing autoregression leads to degraded latent structure and unstable long-range behavior.

Conclusion: Latent autoregression serves as an effective mechanism for organizing long-range structure while remaining complementary to token-level autoregressive modeling. This is an empirical analysis of representational structure rather than a new architecture proposal.
...
<details>
  <summary>Details</summary>
Motivation: Despite previous research questioning the effectiveness of LLM self-critique methods, this paper aims to demonstrate that LLMs can indeed leverage self-critique to substantially enhance their planning performance without relying on external verifiers.

Method: The approach uses few-shot learning extended to many-shot as a base method, then applies an iterative process for correction and refinement through intrinsic self-critique. The method is tested on planning datasets including Blocksworld, Logistics, and Mini-grid domains.

Result: Significant performance gains over established planning benchmarks, exceeding strong baseline accuracies and achieving new state-of-the-art results for LLM model checkpoints from October 2024. The method shows consistent improvements across multiple planning domains.

Conclusion: Self-critique can significantly boost LLM planning performance, demonstrating intrinsic self-improvement capabilities that are model-version agnostic. The approach shows promise for even better performance when applied to more complex search techniques and more capable models.
...
TL;DR: OptRot and OptRot+ methods use learned fusible rotations to reduce outliers in LLM weights for better quantization, outperforming existing rotation-based methods in W4A8 settings.


<details>
  <summary>Details</summary>
Motivation: Large Language Models have outliers in weights and activations that make quantization difficult. Existing rotation methods help mitigate these outliers but have limitations in effectiveness or computational cost.

Method: Proposes OptRot (minimizing element-wise fourth power of rotated weights) and OptRot+ (incorporates activation covariance information). Both learn fusible rotations using cheap proxy objectives to minimize weight quantization error, primarily focusing on GPTQ quantization.

Result: OptRot outperforms Hadamard rotations and more expensive methods like SpinQuant and OSTQuant for weight quantization. Also improves activation quantization in W4A8 setting. OptRot+ further improves performance with activation covariance information. However, both perform worse in W4A4 setting, revealing trade-off between weight and activation quantization.

Conclusion: Learned fusible rotations via principled proxy objectives effectively reduce weight outliers for LLM quantization, with OptRot methods offering superior performance in W4A8 settings while highlighting quantization trade-offs in more aggressive W4A4 scenarios.
...
TL;DR: GARDO framework prevents reward hacking in RL-fine-tuned diffusion models by using selective regularization on uncertain samples, adaptive reference policy updates, and diversity-aware reward amplification.


<details>
  <summary>Details</summary>
Motivation: RL fine-tuning of diffusion models often uses imperfect proxy rewards, leading to reward hacking where proxy scores increase but real image quality deteriorates and diversity collapses. Existing regularization methods against reference policies compromise sample efficiency and exploration since reference policies are usually sub-optimal.

Method: GARDO (Gated and Adaptive Regularization with Diversity-aware Optimization) introduces three key components: 1) Selective regularization that only penalizes samples with high uncertainty, 2) Adaptive regularization that periodically updates the reference model to match the online policy's capabilities, and 3) Diversity-aware optimization that amplifies rewards for high-quality, diverse samples to prevent mode collapse.
...
Conclusion: GARDO provides a versatile framework that addresses the competing demands of sample efficiency, effective exploration, and reward hacking mitigation in RL-fine-tuned diffusion models, demonstrating robustness across various proxy rewards and evaluation metrics.
...
<details>
  <summary>Details</summary>
Motivation: While conformal prediction provides robust marginal coverage guarantees, achieving reliable conditional coverage for specific inputs remains challenging. Exact distribution-free conditional coverage is impossible with finite samples, so the authors aim to improve conditional coverage of standard conformal procedures.

Method: The authors derive a density-weighted pinball loss for quantile regression using Taylor expansion, where weights are given by the conditional density of the conformity score at the true quantile. They propose a three-headed quantile network that estimates these weights via finite differences using auxiliary quantile levels at 1-α±δ, then fine-tunes the central quantile by optimizing the weighted loss.
...
Conclusion: The proposed method effectively improves conditional coverage in conformal prediction by directly minimizing the mean squared error of conditional coverage through refined quantile regression with density-weighted loss, offering both theoretical guarantees and empirical improvements.
...
TL;DR: Paired seed evaluation design improves statistical efficiency in ML comparisons by using identical random seeds across competing systems, reducing variance through seed-level correlation.
...
Conclusion: Paired seed evaluation is weakly dominant in practice - improves statistical reliability when correlation exists and reduces to independent evaluation without loss of validity when correlation is absent.
...
TL;DR: Neural network surrogates enable variance-reduced Monte Carlo uncertainty quantification for Vlasov-Poisson-Landau plasma systems, reducing computational costs while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Plasma kinetic equations are highly sensitive to microscopic perturbations, requiring reliable uncertainty quantification. Traditional methods face challenges from sampling costs, high-dimensional phase space, multiscale stiffness, and collision term complexity.

Method: Variance-reduced Monte Carlo framework using neural network surrogates for Landau collision term. Couples high-fidelity VPL solver with correlated VPFP and EP surrogates. Introduces tensor neural networks based on anisotropic micro-macro decomposition to reduce dimensionality and complexity.
...
Conclusion: The proposed framework successfully addresses computational challenges in plasma uncertainty quantification by combining neural network surrogates with variance reduction techniques, enabling efficient and accurate simulations.
...
TL;DR: Researchers developed four machine learning models optimized for wearable devices to predict sepsis onset using heart rate data, achieving promising results for early detection in non-ICU settings.


<details>
  <summary>Details</summary>
Motivation: Sepsis causes high mortality and healthcare costs, but existing prediction models focus mainly on ICU patients. There's a critical need for early sepsis detection methods suitable for non-ward settings using wearable technology.

Method: Developed four novel machine learning algorithms specifically designed for wearable devices using heart rate data. Used genetic algorithm to optimize model architecture for performance, computational complexity, and memory requirements. Initially designed for 1-hour prediction window, then extended to 4 hours via transfer learning.

Result: Models demonstrated encouraging performance metrics suggesting feasibility for implementation on wearable devices with accurate heart rate monitoring capabilities.

Conclusion: Wearable technology shows promising potential for early sepsis detection outside traditional ICU and ward environments, enabling timely intervention to reduce adverse outcomes.
...
TL;DR: SaM2B is a semantic-aware multi-modal beam prediction framework for UAV communications that uses reliability-aware dynamic weighting and cross-modal contrastive learning to adapt to varying modality importance and enhance generalization across scenarios.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal beam prediction methods use fixed/empirical weights assuming equal modality reliability, but modality importance fluctuates with UAV motion. Static weighting amplifies degraded modality impact, and modal mismatch/weak alignment hurt cross-scenario generalization.
...
Conclusion: SaM2B effectively addresses limitations of static weighting in multi-modal beam prediction by dynamically adapting to modality reliability fluctuations and enhancing generalization through semantic alignment, improving performance for low-altitude UAV communications.
...
TL;DR: TRL is a new Bayesian approximation method that models neural network posteriors as probabilistic tubes following low-loss valleys, using Riemannian geometry to separate different types of uncertainty, achieving ensemble-level calibration at much lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Standard Laplace approximations struggle with the anisotropic, curved loss surfaces and large symmetry groups in modern deep neural networks. There's a need for Bayesian methods that can adapt to this complex geometry while remaining computationally practical.

Method: Tubular Riemannian Laplace (TRL) explicitly models the posterior as a probabilistic tube that follows low-loss valleys induced by functional symmetries. It uses Fisher/Gauss-Newton metrics to separate prior-dominated tangential uncertainty (along symmetry directions) from data-dominated transverse uncertainty (across loss valleys). This is implemented as a scalable reparametrized Gaussian approximation using implicit curvature estimates.

Result: TRL achieves excellent calibration on ResNet-18 with CIFAR-10 and CIFAR-100, matching or exceeding Deep Ensembles in terms of Expected Calibration Error (ECE) while requiring only 1/5 of the training cost. It effectively bridges single-model efficiency with ensemble-grade reliability.

Conclusion: TRL provides a practical Bayesian approximation method that adapts to the complex geometry of neural network loss surfaces, offering ensemble-level calibration reliability at significantly reduced computational cost compared to traditional ensemble methods.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal reasoning systems rely too heavily on textual information, limiting their effectiveness in spatial tasks like visual navigation and geo-localization that require understanding of spatial dependencies and geometric relations.

Method: Proposes Geo-Consistent Visual Planning framework (ViReLoc) that performs planning and localization using only visual representations. Uses step-by-step visual inference encoding, reinforcement learning objectives, contrastive learning, and adaptive feature interaction to align cross-view perspectives and reduce viewpoint differences.

Result: Experiments across diverse navigation and localization scenarios show consistent improvements in spatial reasoning accuracy and cross-view retrieval performance compared to text-based approaches.

Conclusion: Visual reasoning serves as a strong complementary approach for navigation and localization tasks, enabling performance without real-time GPS data for more secure navigation solutions.
...
TL;DR: The paper develops a semiparametric framework for debiased inverse reinforcement learning that enables statistically efficient inference for reward-dependent functionals in maximum entropy IRL and Gumbel-shock DDC models, bridging machine learning flexibility with valid statistical inference.


<details>
  <summary>Details</summary>
Motivation: Current IRL methods using machine learning lack statistical inference guarantees, while classical DDC approaches impose restrictive parametric specifications and require computationally intensive dynamic programming. There's a need for a framework that combines the flexibility of machine learning with valid statistical inference for sequential decision-making models.

Method: Develops a semiparametric framework showing that the log-behavior policy acts as a pseudo-reward that identifies policy value differences and rewards. Formalizes targets as smooth functionals of behavior policy and transition kernel, establishes pathwise differentiability, derives efficient influence functions, and constructs automatic debiased machine-learning estimators.

Result: The framework yields √n-consistent, asymptotically normal, and semiparametric efficient estimators that allow flexible nonparametric estimation of nuisance components while maintaining valid statistical inference. Extends classical DDC inference to nonparametric rewards and modern machine-learning tools.

Conclusion: Provides a unified and computationally tractable approach to statistical inference in IRL that bridges the gap between flexible machine learning methods and classical econometric approaches, enabling valid inference while leveraging modern nonparametric estimation techniques.
...
TL;DR: Proposes sparse regularization methods for high-dimensional Positive-Confidence (Pconf) classification, using Lasso, SCAD, and MCP penalties to achieve near-optimal sparse recovery without needing negative samples.


<details>
  <summary>Details</summary>
Motivation: Existing Pconf methods are inadequate for high-dimensional settings where features exceed sample size. While sparse regularization works well in fully supervised learning, it remains underexplored for weak-supervision scenarios like Pconf classification that only use positive samples with confidence scores.

Method: Develops a sparse-penalization framework for high-dimensional Pconf classification using convex (Lasso) and non-convex (SCAD, MCP) penalties to address shrinkage bias. Proposes an efficient proximal gradient algorithm to solve the composite objective function.

Result: Establishes theoretical bounds showing L1-regularized Pconf estimator achieves near minimax-optimal sparse recovery rates under Restricted Strong Convexity. Simulations show proposed methods achieve predictive performance and variable selection accuracy comparable to fully supervised approaches.

Conclusion: The proposed framework effectively bridges the gap between weak supervision and high-dimensional statistics, enabling sparse regularization for Pconf classification with performance comparable to fully supervised methods despite using only positive samples with confidence scores.
...
TL;DR: A diagnostic-driven adaptive learning framework that decomposes error evolution into bias, noise, and alignment components for stable learning in dynamic environments.
...
Method: Proposes a diagnostic framework that decomposes error evolution into three components: bias (persistent drift), noise (stochastic variability), and alignment (repeated directional excitation leading to overshoot). These diagnostics are computed online from lightweight statistics of loss or temporal-difference error trajectories, independent of model architecture or task domain.

Result: The bias-noise-alignment decomposition provides a unifying control backbone for supervised optimization, actor-critic reinforcement learning, and learned optimizers. Diagnostic-driven instantiations include stabilized supervised optimizer, diagnostic-regulated actor-critic scheme, and diagnostic-conditioned learned optimizer, with proven bounded effective updates and stability properties.

Conclusion: This work elevates error evolution to a first-class object in adaptive learning, providing an interpretable, lightweight foundation for reliable learning in dynamic environments across multiple learning paradigms.
...
TL;DR: Proposes reframing chaotic system forecasting as joint generative modeling of lagged states over temporal windows, enabling improved prediction skill and uncertainty quantification without ground truth.


<details>
  <summary>Details</summary>
Motivation: Chaotic systems have fundamental forecasting limitations due to sensitivity to initial conditions and unresolved multiscale processes. Existing generative approaches focus on next-step conditional prediction rather than capturing underlying dynamic structure.

Method: Learn joint probability distribution of lagged system states over short temporal windows, then obtain forecasts through marginalization. Introduces model-agnostic training/inference framework with three uncertainty quantification metrics: ensemble variance, short-horizon autocorrelation, and cumulative Wasserstein drift.

Result: Joint generative models yield improved short-term predictive skill, preserve attractor geometry, and achieve substantially more accurate long-range statistical behavior than conventional conditional next-step models on Lorenz-63 and Kuramoto-Sivashinsky systems.

Conclusion: Reframing forecasting as joint generative modeling captures nonlinear temporal dependencies better than conditional approaches, enables robust uncertainty quantification without ground truth, and improves both short-term and long-range predictions for chaotic systems.
...
TL;DR: HOLOGRAPH is a framework that formalizes LLM-guided causal discovery using sheaf theory, representing local causal beliefs as sections of a presheaf and identifying coherent global structures through sheaf cohomology.


<details>
  <summary>Details</summary>
Motivation: Causal discovery from observational data faces fundamental identifiability constraints. While recent work uses LLMs as sources of prior causal knowledge, existing approaches rely on heuristic integration lacking theoretical grounding.

Method: Uses sheaf theory to represent local causal beliefs as sections of a presheaf over variable subsets. Coherent global causal structure corresponds to existence of a global section, with topological obstructions manifesting as non-vanishing sheaf cohomology. Introduces Algebraic Latent Projection for hidden confounders and Natural Gradient Descent on belief manifold for optimization.

Result: Experiments on synthetic and real-world benchmarks show competitive performance on causal discovery tasks with 50-100 variables. Sheaf-theoretic analysis reveals Identity, Transitivity, and Gluing axioms satisfied to numerical precision (<10^{-6}), but Locality axiom fails for larger graphs, suggesting fundamental non-local coupling in latent variable projections.

Conclusion: HOLOGRAPH provides rigorous mathematical foundations for LLM-guided causal discovery through sheaf theory, offering theoretical grounding while maintaining practical performance, though revealing fundamental limitations in locality assumptions for complex causal structures.
...
TL;DR: Small proxy models used to evaluate data recipes may give misleading results because optimal training hyperparameters are data-dependent; using reduced learning rates improves correlation with full-scale model performance.


<details>
  <summary>Details</summary>
Motivation: Data teams at AI companies use small proxy models to make critical decisions about pretraining data recipes, but there's limited understanding of whether conclusions from small-scale experiments reliably transfer to full-scale training. The standard "fair comparison" protocol using identical training configurations for all data recipes is problematic.

Method: The paper introduces a simple patch to the evaluation protocol: using reduced learning rates for proxy model training. This approach is theoretically analyzed using random-feature models and empirically validated across 23 data recipes covering four critical dimensions of data curation.

Result: The reduced learning rate approach yields relative performance that strongly correlates with that of fully tuned large-scale LLM pretraining runs. The method dramatically improves the reliability of small-scale experiments, with theoretical proof showing it preserves dataset ordering according to optimal achievable loss.

Conclusion: The objective of data recipe assessment should be to identify the recipe that yields the best performance under data-specific tuning, not under identical training configurations. The proposed reduced learning rate protocol provides a practical solution that improves transferability of conclusions from small-scale to full-scale training.
...
TL;DR: Extends E-prop to deep recurrent networks, enabling online learning across both time and depth without backpropagation through time.


<details>
  <summary>Details</summary>
Motivation: BPTT is biologically implausible, RTRL is computationally expensive, and existing E-prop only handles single-layer recurrent networks. Need online learning for deep recurrent networks that works across both temporal and hierarchical dimensions.

Method: Extends E-prop framework to arbitrarily deep networks by deriving a novel recursion relationship across depth that extends eligibility traces to deeper layers.

Result: Develops an online learning algorithm that can perform accurate credit assignment across both time and depth simultaneously.

Conclusion: Enables training of deep recurrent networks without backpropagation through time, providing a more biologically plausible learning mechanism for hierarchical temporal processing.
...
TL;DR: MDBF improves extreme low-bit LLM quantization by replacing DBF's single envelope with multiple rank-l envelopes while keeping binary sign bases, enhancing magnitude expressiveness without changing inference primitives.
...
Method: Proposes Multi-envelope DBF (MDBF) which retains shared 1-bit sign bases but replaces the single envelope with rank-l envelopes. Uses closed-form initialization and alternating refinement optimization while maintaining binary carrier structure.

Result: MDBF enhances perplexity and zero-shot accuracy over previous binary formats across LLaMA and Qwen families at matched bits per weight while preserving deployment-friendly inference primitives.

Conclusion: MDBF successfully addresses DBF's limitations by improving magnitude expressiveness through multiple envelopes while maintaining efficient binary inference, making it a superior approach for extreme low-bit LLM quantization.
...
TL;DR: HUMOR is a framework that enhances visual language models for humorous meme generation through hierarchical reasoning and group-wise human preference alignment.


<details>
  <summary>Details</summary>
Motivation: Generating humorous memes requires nuanced multimodal reasoning beyond simple image-to-caption supervision, involving visual content understanding, contextual cues, and subjective humor perception.

Method: Uses hierarchical multi-path Chain-of-Thought reasoning (template intent → diverse paths → context-specific anchoring) and trains a pairwise reward model for group-wise preference alignment, enabling reinforcement learning optimization.
...
Conclusion: The framework presents a general training paradigm for open-ended, human-aligned multimodal generation where success is guided by comparative judgment within coherent output groups.
...
TL;DR: DLCM introduces hierarchical language modeling that compresses tokens into variable-length concepts, enabling more efficient computation allocation and improved performance under fixed FLOPs.


<details>
  <summary>Details</summary>
Motivation: Current LLMs apply uniform computation to all tokens, wasting capacity on predictable spans while under-allocating computation to semantically critical transitions, due to language's non-uniform information density.

Method: DLCM learns semantic boundaries from latent representations to compress tokens into variable-length concepts, uses compression-aware scaling laws to allocate compute, and employs decoupled μP parametrization for stable training across compression regimes.

Result: At R=4 compression (4 tokens per concept), DLCM reallocates ~1/3 of inference compute to higher-capacity reasoning backbone, achieving +2.69% average improvement across 12 zero-shot benchmarks under matched inference FLOPs.

Conclusion: Hierarchical compression with dynamic concept formation enables more efficient compute allocation, fundamentally changing scaling behavior and improving language model performance without increasing computational cost.
...
TL;DR: AutoFed is a novel Personalized Federated Learning framework for traffic prediction that eliminates manual hyper-parameter tuning through prompt learning, achieving superior performance across diverse real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: Traffic prediction is crucial for Intelligent Transportation Systems, but privacy concerns lead to data silos. Standard Federated Learning struggles with non-IID data, and current Personalized Federated Learning frameworks require manual hyper-parameter tuning that's impractical for real-world deployment.

Method: AutoFed introduces a federated representor with client-aligned adapters that distill local data into compact, globally shared prompt matrices. These prompts condition personalized predictors, enabling cross-client knowledge sharing while maintaining local specificity without manual hyper-parameter tuning.

Result: Extensive experiments on real-world datasets demonstrate that AutoFed consistently achieves superior performance across diverse traffic prediction scenarios compared to existing methods.

Conclusion: AutoFed provides an effective, practical Personalized Federated Learning solution for traffic prediction that overcomes the limitations of manual hyper-parameter tuning, enabling better privacy-preserving collaborative training for real-world Intelligent Transportation Systems.
...
TL;DR: Large-scale logP prediction framework using 426K compounds achieves 0.765 R² with tree-based ensembles, showing molecular weight is most important predictor despite weak bivariate correlation.


<details>
  <summary>Details</summary>
Motivation: To develop a robust, scalable framework for lipophilicity (logP) prediction using well-curated data from multiple chemical databases, addressing data integration challenges and establishing competitive baselines with 2D descriptors.

Method: Created computational infrastructure with byte-offset indexing for 740x faster data processing; used 426,850 compounds from PubChem, ChEMBL, eMolecules intersection; evaluated linear models, Random Forest, XGBoost; employed SHAP analysis; implemented stratified modeling for drug-like vs extreme molecules.

Result: Tree-based ensembles achieved R²=0.765, RMSE=0.731; molecular weight identified as most important predictor via SHAP; linear models failed due to heteroskedasticity; stratified modeling achieved RMSE=0.838 for drug-like subset (91%) and R²=0.767 for extreme molecules (9%).

Conclusion: Well-curated descriptor-based ensemble models remain competitive with graph neural networks; molecular weight is globally most important predictor despite weak bivariate correlation; stratified modeling optimizes performance; framework provides actionable guidance for molecular design.
...
TL;DR: HeteroHBA: A generative backdoor attack framework for heterogeneous graph neural networks that injects trigger nodes with diverse features/connections while maintaining stealthiness through distribution alignment.


<details>
  <summary>Details</summary>
Motivation: Backdoor attacks on heterogeneous graphs remain understudied despite their practical risks. Existing methods may not effectively handle the complexity of heterogeneous graphs with diverse node types and connection patterns.

Method: 1) Saliency-based screening to select influential auxiliary neighbors for trigger attachment; 2) Synthesis of diverse trigger features and connection patterns to match local heterogeneous context; 3) Adaptive Instance Normalization (AdaIN) with MMD loss for feature distribution alignment; 4) Bilevel optimization for joint attack success and clean accuracy preservation.

Result: HeteroHBA consistently achieves higher attack success than prior backdoor baselines on multiple real-world heterogeneous graphs with representative HGNN architectures, with comparable or smaller impact on clean accuracy. The attack remains effective under heterogeneity-aware structural defense CSD.

Conclusion: The work highlights practical backdoor risks in heterogeneous graph learning and demonstrates the need for stronger defenses against such attacks that exploit the heterogeneous nature of graphs.
...
<details>
  <summary>Details</summary>
Motivation: DFL suffers from performance degradation due to limited connectivity and data heterogeneity in sparse networks. User mobility (natural or induced) can act as relays/bridges to enhance information flow, but its impact on DFL has been largely overlooked despite its potential.

Method: 1) Establish convergence of DFL in sparse networks under user mobility, theoretically showing random movement of fraction of users boosts performance. 2) Propose DFL framework utilizing mobile users with induced mobility patterns that exploit knowledge of data distribution to determine trajectories for enhanced information propagation.

Result: Theoretical analysis shows mobility improves DFL convergence; empirical experiments confirm findings, validate superiority over baselines, and provide comprehensive analysis of how network parameters influence DFL performance in mobile networks.

Conclusion: User mobility significantly enhances DFL performance in sparse networks; induced mobility patterns based on data distribution knowledge can further optimize information propagation and improve collaborative training outcomes.
...
<details>
  <summary>Details</summary>
Motivation: Current Language Models face fundamental challenges in continual learning, self-improvement, and finding effective solutions. Existing methods lack coherent frameworks for representing learning as multi-level optimization processes.

Method: Proposes Nested Learning (NL) paradigm with three contributions: 1) Expressive Optimizers as associative memory modules, 2) Self-Modifying Learning Module that learns its own update algorithm, and 3) Continuum Memory System generalizing traditional memory concepts.

Result: Developed Hope - a continual learning module combining self-modifying sequence model with continuum memory, showing promising results in language modeling, knowledge incorporation, few-shot generalization, continual learning, and long-context reasoning tasks.

Conclusion: NL provides a philosophical framework for designing more expressive learning algorithms with multiple levels, enabling higher-order in-context learning and potentially unlocking effective continual learning capabilities in large models.
...
TL;DR: DCL-DECOR: A precision-based pipeline for causal discovery in linear Gaussian systems with mixed latent confounding, separating pervasive global confounders from local dependencies to improve directed edge recovery.
...
Method: A modular pipeline that: 1) Decomposes observed precision matrix into structured and low-rank components to isolate pervasive latent effects, 2) Applies correlated-noise DAG learning to the deconfounded representation to recover directed edges while modeling remaining structured error correlations, 3) Performs reconciliation to enforce bow-freeness.

Result: Provides identifiability results characterizing recoverable causal targets under mixed confounding, reduces overall problem to well-studied subproblems with modular guarantees. Synthetic experiments show consistent improvements in directed edge recovery over applying correlated-noise DAG learning directly to confounded data.

Conclusion: DCL-DECOR effectively addresses the challenge of mixed latent confounding by separating pervasive from localized confounding effects, enabling more accurate causal discovery in practical settings where both global and local unobserved factors exist.
...
TL;DR: BandiK is a three-stage multi-task auxiliary task selection method using multi-bandits that efficiently identifies beneficial auxiliary tasks for each target task while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Negative transfer and computational costs hinder effective auxiliary task selection in multi-task learning. Current methods face challenges with exponential candidate sets and varying task complexities.

Method: Three-stage approach: 1) Estimate pairwise transfers between tasks, 2) Construct linear number of candidate auxiliary sets per target task, 3) Use Multi-Armed Bandit framework with novel multi-bandit structure exploiting semi-overlapping arms across tasks.

Result: BandiK significantly reduces computational costs by evaluating candidate auxiliary sets efficiently through multi-bandit optimization while maintaining effective knowledge transfer across tasks.

Conclusion: The proposed BandiK method provides an efficient solution to the auxiliary task selection problem in multi-task learning by leveraging multi-bandit optimization and exploiting shared neural network structures across tasks.
...
<details>
  <summary>Details</summary>
Motivation: LLMs have high computation and memory requirements that hinder deployment in resource-constrained environments, requiring efficient compression and acceleration techniques.

Method: Unified pipeline applying N:M structured pruning and 4-bit integer quantization, with optimized dequantization and matrix multiplication for multiple hardware platforms including CPUs, GPUs with Dense/Sparse Tensor Cores, and custom systolic-array-based FPGA accelerators.
...
Conclusion: Fine-grained N:M sparsity combined with quantization enables efficient LLM deployment, while FPGA accelerators offer flexible support for diverse sparsity patterns beyond fixed hardware constraints.
...
<details>
  <summary>Details</summary>
Motivation: Most existing research on autonomous taxi acceptance uses hypothetical scenarios, lacking investigation of actual user behavior based on operational autonomous vehicle services.

Method: Surveyed 336 actual users in Wuhan with realistic service attributes, used Structural Equation Modeling to analyze six latent psychological constructs and their influence on adoption frequency across ten scenarios.

Result: Cost Sensitivity and Behavioral Intention are strongest positive predictors of adoption; other constructs (Trust & Policy Support, Performance, Lifestyle, Education) play more nuanced roles; model shows strong goodness-of-fit.

Conclusion: Provides empirical evidence for policymaking, fare design, and public outreach strategies to scale autonomous taxi deployments in real urban settings.
...
TL;DR: Gradient descent on log-sum-exp objectives implicitly performs expectation-maximization, unifying mixture modeling, attention, and classification under a single mechanism where optimization and inference are the same process.


<details>
  <summary>Details</summary>
Motivation: To provide a direct mathematical explanation for why neural networks trained with standard objectives exhibit probabilistic inference behaviors (soft clustering, prototype specialization, Bayesian uncertainty tracking) across various architectures, rather than relying on loose analogies or post-hoc interpretations.

Method: Derives that for any objective with log-sum-exp structure over distances or energies, the gradient with respect to each distance equals the negative posterior responsibility of the corresponding component (∂L/∂d_j = -r_j). This algebraic identity shows gradient descent implicitly performs expectation-maximization.

Result: Demonstrates that responsibilities are not auxiliary variables but gradients to be applied, and no explicit inference algorithm is needed because inference is embedded in optimization. Unifies unsupervised mixture modeling, attention mechanisms, and cross-entropy classification under a single mechanism.

Conclusion: The Bayesian structure observed in trained transformers is not emergent but a necessary consequence of objective geometry. Optimization and inference are fundamentally the same process when using log-sum-exp objectives.
...
TL;DR: Proposes self-supervised learning for neural architecture search of multimodal DNNs, enabling architecture design from unlabeled data.


<details>
  <summary>Details</summary>
Motivation: Multimodal DNNs benefit from NAS due to structural complexity, but traditional NAS requires substantial labeled data for architecture construction.

Method: Applies self-supervised learning comprehensively for both architecture search and model pretraining processes of multimodal DNNs.

Result: Experimental results show the method successfully designed architectures for DNNs from unlabeled training data.

Conclusion: SSL enables effective neural architecture search for multimodal DNNs without requiring labeled data, addressing the data labeling bottleneck.
...
<details>
  <summary>Details</summary>
Motivation: Need for precise probabilistic information in DTI predictions to understand limitations and boost performance. Current methods lack proper uncertainty quantification and Bayesian operations like classification with rejection, top-K selection, and ranking.

Method: Deep kernel learning-based GP architecture with neural embedding module for compounds and proteins, plus GP module. Uses predictive distribution sampling to estimate Bayesian precedence matrix for fast selection/ranking operations.
...
Conclusion: The proposed DTI-GP framework provides scalable Bayesian inference for DTI predictions with uncertainty quantification, enabling novel operations that improve practical utility in drug discovery pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Standard preference modeling assumes transitivity, overlooking complex human population preferences. NLHF addresses non-transitive preferences via game theory, but existing algorithms use regularization causing bias in duality gap computation.

Method: Analyzes Optimistic Multiplicative Weights Update (OMWU) in Nash learning from human feedback, proving convergence guarantees without NE uniqueness assumption. Identifies novel marginal convergence behavior where rarely played actions grow exponentially from small values.

Result: OMWU achieves last-iterate linear convergence after burn-in phase when NE with full support exists, with instance-dependent linear convergence rate to original NE measured by duality gaps. Shows exponentially better dependence on instance-dependent constants than prior work.

Conclusion: OMWU provides theoretically sound approach for NLHF alignment without regularization bias, with practical potential for LLM applications as demonstrated in tabular and neural policy experiments.
...
TL;DR: The paper proposes a novel multi-agent option discovery method using joint-state abstraction and neural graph Laplacian estimation to discover strongly coordinated behaviors, addressing limitations of existing methods that produce loosely coupled or independent options.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent option discovery methods often sacrifice coordination by producing loosely coupled or fully independent behaviors due to the exponential growth of joint state space with number of agents. There's a need for methods that can discover strongly coordinated behaviors while managing state space complexity.

Method: 1) Proposes a joint-state abstraction that compresses state space while preserving coordination information. 2) Uses the inductive bias that synchronization over agent states provides natural foundation for coordination. 3) Approximates a fictitious "Fermat state" of maximal alignment with the team. 4) Defines "spreadness" measure capturing team-level misalignment on each state dimension. 5) Employs neural graph Laplacian estimator to derive options capturing state synchronization patterns between agents.
...
Conclusion: The proposed approach successfully addresses limitations of existing multi-agent option discovery methods by enabling discovery of strongly coordinated behaviors through joint-state abstraction and synchronization-based representation, demonstrating improved coordination capabilities in multi-agent settings.
...
Method: Proposes AODDiff with: 1) corruption-aware training to learn spatiotemporal AOD priors from naturally incomplete data, and 2) decoupled annealing posterior sampling to integrate heterogeneous observations as constraints for guided generation.
...
Conclusion: AODDiff provides a flexible, probabilistic reconstruction framework that addresses data scarcity issues, enables uncertainty quantification, and maintains high fidelity for atmospheric monitoring applications.
...
TL;DR: The paper proposes using learning curves over varying sample sizes to model transfer effects in multi-task learning, showing this approach is more fundamental than gradient-based methods and offers better power with broader applicability.


<details>
  <summary>Details</summary>
Motivation: Current approaches to understanding transfer effects focus on gradient updates during training, but the authors hypothesize that perturbing the dataset by including more samples provides a more fundamental characterization of transfer effects in both training and inductive inference.

Method: Develop multi-task learning curves that approximate inductive performance over varying sample sizes, analogous to Task Affinity Grouping method. Compare statistical and computational approaches to transfer, with evaluations on a drug-target interaction benchmark dataset.

Result: Learning curves better capture multi-task learning effects, and their multi-task extensions can delineate pairwise and contextual transfer effects in foundation models. The statistical approach shows considerably higher compute costs but better power and broader applicability than previous methods.

Conclusion: Modeling transfer effects through multi-task learning curves provides a more fundamental characterization than gradient-based approaches, offering better analytical power for understanding transfer in both training and inductive inference scenarios.
...
### [186] [PRISM: A hierarchical multiscale approach for time series forecasting](https://arxiv.org/abs/2512.24898)
*Zihao Chen,Alexandre Andre,Wenrui Ma,Ian Knight,Sergey Shuvaev,Eva Dyer*

Main category: cs.LG

TL;DR: PRISM introduces a hierarchical tree-based partitioning method for time series forecasting that captures multi-scale features through time-frequency analysis at different levels, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Real-world time series contain complex multi-scale features including global trends, local fine-grained structure, and intermediate-scale patterns, making accurate forecasting challenging despite existing progress in the field.
...
Result: Experiments across benchmark datasets show that PRISM outperforms state-of-the-art forecasting methods, demonstrating superior performance in capturing both global structure and local dynamics of time series.

Conclusion: The hierarchical tree-based approach provides a lightweight and flexible framework for multivariate time series forecasting that effectively handles multi-scale features, with code publicly available for further research and application.
...
### [187] [Spectral Graph Neural Networks for Cognitive Task Classification in fMRI Connectomes](https://arxiv.org/abs/2512.24901)
*Debasis Maji,Arghya Banerjee,Debaditya Barman*

Main category: cs.LG

TL;DR: SpectralBrainGNN uses graph neural networks with spectral convolution via graph Fourier transforms to classify cognitive tasks from fMRI connectomes, achieving 96.25% accuracy on HCPTask dataset.


<details>
  <summary>Details</summary>
Motivation: Current methods for cognitive task classification from neuroimaging data often miss complex topological dependencies and multi-scale interactions in brain networks. There's a need for better integration of machine learning with brain network analysis to extract meaningful connectivity patterns from fMRI data.

Method: Proposes SpectralBrainGNN, a spectral convolution framework based on graph Fourier transforms computed via normalized Laplacian eigendecomposition. Models brain regions as nodes and functional connections as edges in a graph neural network to capture topological dependencies and multi-scale interactions.

Result: Achieved 96.25% classification accuracy on the Human Connectome Project-Task (HCPTask) dataset, demonstrating superior performance in decoding cognitive states from fMRI connectomes.

Conclusion: SpectralBrainGNN effectively advances cognitive task classification by leveraging spectral graph theory and GNNs to capture complex brain network patterns, with publicly available implementation supporting reproducibility and future research.
...
### [188] [Frequent subgraph-based persistent homology for graph classification](https://arxiv.org/abs/2512.24917)
*Xinyang Chen,Amaël Broustet,Guoting Chen*

Main category: cs.LG

TL;DR: Proposes Frequent Subgraph Filtration (FSF) for persistent homology on graphs, extracting frequency-based topological features from recurring subgraphs, and integrates it into ML/GNN frameworks for improved graph classification.


<details>
  <summary>Details</summary>
Motivation: Current persistent homology methods on graphs use limited filtrations (degree/weight-based) that miss richer features like recurring patterns across datasets, restricting expressive power for topology-aware learning.

Method: Introduces Frequent Subgraph Filtration (FSF) derived from frequent subgraphs, producing stable frequency-based persistent homology (FPH) features. Presents FPH-ML (machine learning model) and FPH-GNN (hybrid with graph neural networks) frameworks.

Result: FPH-ML achieves competitive/superior accuracy vs kernel/degree-based filtration methods. FPH-GNN yields 0.4-21% relative gains, up to 8.2 percentage point improvements over GCN/GIN backbones across benchmarks.

Conclusion: FSF bridges frequent subgraph mining and topological data analysis, offering new perspective for topology-aware feature extraction with demonstrated performance improvements in graph classification tasks.
...
### [189] [MSACL: Multi-Step Actor-Critic Learning with Lyapunov Certificates for Exponentially Stabilizing Control](https://arxiv.org/abs/2512.24955)
*Yongwei Zhang,Yuanzhe Xing,Quan Quan,Zhikun She*

Main category: cs.LG

TL;DR: MSACL integrates exponential stability theory with maximum entropy RL using multi-step Lyapunov certificates to achieve provable stability in model-free RL without complex reward engineering.
...
Result: MSACL demonstrates superiority over state-of-the-art Lyapunov-based RL algorithms across six benchmarks including stabilization and nonlinear tracking tasks. It achieves exponential stability and rapid convergence under simple rewards, shows significant robustness to uncertainties, and generalizes well to unseen trajectories. Sensitivity analysis establishes multi-step horizon n=20 as a robust default.

Conclusion: MSACL provides a foundation for verifiably safe learning-based control by linking Lyapunov theory with off-policy actor-critic frameworks, enabling provable stability without complex reward engineering while maintaining learning efficiency.
...
TL;DR: The paper introduces Sequential Support Network Learning (SSNL) framework and semi-overlapping multi-bandit (SOMMAB) model for efficiently learning optimal support networks from sparse candidate lists through shared evaluations.


<details>
  <summary>Details</summary>
Motivation: Many AI/ML problems require evaluating partners' contributions through asymmetric, computationally intensive processes while selecting the most beneficial candidates. Sequential approaches to these problems need a unified framework for efficient learning of support networks.

Method: Proposes SSNL framework and SOMMAB model where single evaluations provide distinct feedback to multiple bandits due to structural overlap. Develops generalized GapE algorithm for SOMMABs with new exponential error bounds.

Result: Derives improved exponential error bounds that scale linearly with degree of overlap, showing significant sample-complexity gains from shared evaluations. Improves best known constant in exponent for multi-bandit best-arm identification.

Conclusion: Provides theoretical foundation and improved performance guarantees for sequential learning tools in identifying support networks from sparse candidates, applicable to multi-task learning, federated learning, and multi-agent systems.
...
TL;DR: DMSAEs distill a compact core of consistently useful features from SAEs and reuse it to train new SAEs, improving feature consistency and transferability across training runs and sparsity levels.
...
Method: Iterative distillation cycle: train Matryoshka SAE with shared core, use gradient X activation to measure feature contributions, keep smallest subset explaining fixed attribution fraction. Only core encoder weights transfer across cycles.

Result: On Gemma-2-2B, seven distillation cycles yielded distilled core of 197 consistently selected features. Training with distilled core improves SAEBench metrics.

Conclusion: DMSAEs demonstrate that consistent sets of latent features can be transferred across sparsity levels, addressing SAE feature inconsistency problems.
...
TL;DR: The paper proposes a method to predict task data efficiency (how many examples needed for fine-tuning) using gradient cosine similarity of low-confidence examples, eliminating costly annotation cycles.


<details>
  <summary>Details</summary>
Motivation: LLMs often require fine-tuning for specialized tasks, but it's unknown how many examples are needed, leading to expensive cycles of incremental annotation and retraining. The paper shows that performant LLMs struggle zero-shot on specialized tasks but improve with fine-tuning, creating a need to predict data efficiency without extensive annotation.

Method: Introduces a metric to quantify task data efficiency, then proposes using gradient cosine similarity of low-confidence examples to predict data efficiency based on a small number of labeled samples.

Result: Validated on 30 specialized tasks, achieving 8.6% error in overall data efficiency prediction, typically eliminating hundreds of unnecessary annotations per task.

Conclusion: The proposed method effectively predicts task data efficiency with minimal labeled data, reducing annotation costs and improving fine-tuning efficiency for specialized LLM tasks.
...
TL;DR: DLMs with chain-of-thought can simulate parallel sampling algorithms with optimal sequential steps, but revision/remasking is needed for optimal space complexity.


<details>
  <summary>Details</summary>
Motivation: To provide rigorous theoretical foundation for diffusion language models as efficient parallel samplers compared to autoregressive models, and to understand the computational advantages and limitations of different DLM variants.

Method: Formalize parallel sampling model, analyze DLMs with chain-of-thought, prove simulation capabilities, establish expressivity gaps between DLM variants with/without revision/remasking.

Result: DLMs with CoT can simulate any parallel sampling algorithm with optimal sequential steps; revision/remasking enables optimal space complexity; revision/remasking DLMs are strictly more expressive than those without.
...
TL;DR: ResponseRank is a method that learns preference strength from noisy proxy signals (like response times or annotator agreement) by comparing signals locally within strata, enabling robust utility learning without strong assumptions about the strength signal.


<details>
  <summary>Details</summary>
Motivation: Binary choices in RLHF only convey direction, not strength of preference. Strength is crucial for decision-making and generalization, but hard to measure reliably. Existing proxies (response times, inter-annotator agreement) are noisy and confounded.

Method: ResponseRank uses relative differences in proxy signals to rank responses by inferred preference strength. It compares signals only locally within carefully constructed strata to control for systemic variation, enabling robust learning of utility differences consistent with strength-derived rankings with minimal assumptions.

Result: The method shows improved sample efficiency and robustness across diverse tasks: synthetic preference learning (with simulated response times), language modeling (with annotator agreement), and RL control tasks (with simulated episode returns).

Conclusion: ResponseRank provides a novel approach to robustly learn preference strength from noisy signals, with empirical validation across multiple domains and introduction of the Pearson Distance Correlation metric to isolate cardinal utility learning from ordinal accuracy.
...
TL;DR: Generative classifiers using class-conditional generative models outperform discriminative classifiers on distribution shift by avoiding overreliance on spurious correlations.


<details>
  <summary>Details</summary>
Motivation: Discriminative classifiers often learn shortcuts and spurious correlations that fail under distribution shift, while generative models can avoid this by modeling all features.

Method: Use class-conditional generative models (diffusion-based and autoregressive) as classifiers, which are simple to train without specialized augmentations, regularization, or prior knowledge of spurious correlations.

Result: Achieve state-of-the-art performance on five standard image and text distribution shift benchmarks, and reduce spurious correlation impact in medical and satellite datasets.

Conclusion: Generative classifiers are effective for distribution shift robustness, with analysis in Gaussian toy settings revealing their inductive biases and conditions for outperforming discriminative classifiers.
...
TL;DR: Both uniform and learnable attention architectures implement the same modular addition algorithm with topologically equivalent representations, contrary to prior interpretations.


<details>
  <summary>Details</summary>
Motivation: To challenge the Clock and Pizza interpretations that claimed different architectural designs (uniform vs learnable attention) create distinct circuits for modular addition.

Method: Analyze learned representations as manifolds using topological tools, studying groups of neurons collectively rather than individual neurons/weights, and statistically analyze hundreds of circuits.

Result: Both uniform attention and trainable attention architectures implement the same algorithm with topologically and geometrically equivalent representations.

Conclusion: Different architectural designs don't create fundamentally different circuits for modular addition - they converge to the same underlying algorithm and representations.
...
TL;DR: B-Trans transforms standard LLMs into Bayesian Transformers using stochastic normalization offsets to sample diverse model instances from pre-trained weights, enabling population-level decision-making.


<details>
  <summary>Details</summary>
Motivation: Current transformers are trained as single deterministic systems, but intelligence may emerge from many minds. The authors propose creating Bayesian Transformers to support sampling diverse model instances from a single set of pre-trained weights.

Method: Introduces Bayesian posterior proxy by treating bias-like offsets in normalization layers as stochastic variables with Gaussian variational approximation. Samples diverse model instances while maintaining competence. Freezes sampled noise at sequence level for temporal consistency.

Result: B-Trans enables population-level decision-making where aggregating predictions across sampled individuals enhances exploration. Experiments show superior semantic diversity and better task performance in zero-shot generation, RLVR, and RL without explicit labels compared to deterministic baselines.

Conclusion: B-Trans effectively leverages the wisdom of crowds by transforming standard LLMs into Bayesian Transformers, yielding both better task performance and increased semantic diversity through population-level decision-making.
...
TL;DR: Researchers train language models for open-ended forecasting using automated news-based data generation, achieving state-of-the-art performance with their 8B model matching larger proprietary models.
...
Method: 1) Automatically synthesize forecasting questions from daily news events using a careful curation recipe; 2) Train Qwen3 thinking models on the OpenForesight dataset; 3) Use offline news corpus to prevent future information leakage; 4) Implement retrieval mechanisms and improved reward functions for reinforcement learning; 5) Evaluate on held-out test data from May-August 2025.

Result: OpenForecaster 8B matches performance of much larger proprietary models. Training improves accuracy, calibration, and consistency of predictions. Calibration improvements generalize across popular benchmarks. The model demonstrates benefits from retrieval and improved RL reward functions.

Conclusion: Specialized forecasting models can achieve state-of-the-art performance with proper data curation and training techniques. The research makes forecasting more accessible by open-sourcing models, code, and data, enabling broader research in language model forecasting capabilities.
...
### [199] [Leveraging Machine Learning for Early Detection of Lung Diseases](https://arxiv.org/abs/2512.23757)
...
<details>
  <summary>Details</summary>
Motivation: To address healthcare disparities by providing rapid, accurate, and non-invasive diagnostic solutions for respiratory diseases in areas with limited access to radiologists and healthcare resources.

Method: Combined traditional image processing with advanced neural networks; trained and validated multiple deep learning models including CNNs, VGG16, InceptionV3, and EfficientNetB0 on chest X-ray data.

Result: Models achieved high accuracy, precision, recall, and F1 scores, demonstrating reliability and potential for real-world diagnostic applications in detecting COVID-19, lung cancer, and pneumonia.

Conclusion: Deep learning approaches can significantly impact patient outcomes by enabling predictive and preventive healthcare, particularly benefiting underserved regions with limited medical resources.
...
### [200] [Targeted Semantic Segmentation of Himalayan Glacial Lakes Using Time-Series SAR: Towards Automated GLOF Early Warning](https://arxiv.org/abs/2512.24117)
...
<details>
  <summary>Details</summary>
Motivation: Existing GLOF monitoring approaches have limitations: they either prioritize spatial coverage for general models or rely on optical imagery affected by cloud coverage. There's a need for targeted, automated monitoring of high-risk Himalayan glacial lakes using SAR data that works regardless of weather conditions.

Method: End-to-end automated deep learning pipeline using time-series Sentinel-1 SAR data. Uses U-Net with EfficientNet-B3 backbone trained with "temporal-first" strategy on curated dataset of 4 high-risk Himalayan lakes (Tsho Rolpa, Chamlang Tsho, Tilicho, Gokyo Lake). Includes operational engineering architecture: Dockerized pipeline automating data ingestion via ASF Search API and RESTful endpoint for inference results.

Result: Model achieves IoU of 0.9130, validating the success and efficacy of the "temporal-first" training strategy. The system enables dynamic, automated monitoring suitable for transitioning to Early Warning Systems.

Conclusion: The proposed system shifts paradigm from static mapping to dynamic automated early warning, providing scalable architectural foundation for future Early Warning System development. Combines accurate deep learning model with operational engineering pipeline for practical GLOF monitoring.
...
### [201] [The OCR-PT-CT Project: Semi-Automatic Recognition of Ancient Egyptian Hieroglyphs Based on Metric Learning](https://arxiv.org/abs/2512.24197)
...
TL;DR: OCR-PT-CT project develops hieroglyph recognition using Deep Metric Learning with 97.70% accuracy, outperforming Mobilenet (93.87%) for Coffin and Pyramid Texts.


<details>
  <summary>Details</summary>
Motivation: Digital humanities are transforming Egyptology, but hieroglyph recognition faces challenges with class imbalance and limited data for underrepresented signs.

Method: Two approaches: 1) Mobilenet neural network trained on 140 hieroglyph classes; 2) Novel Deep Metric Learning approach for better flexibility with new or data-limited signs.

Result: Deep Metric Learning achieved 97.70% accuracy vs. Mobilenet's 93.87%, showing superior performance under class imbalance and recognizing more hieroglyphs.

Conclusion: Deep Metric Learning adopted as default classifier due to superior accuracy, flexibility for underrepresented classes, and adaptability to new signs.
...
### [202] [Generative Video Compression: Towards 0.01% Compression Rate for Video Transmission](https://arxiv.org/abs/2512.24300)
...
<details>
  <summary>Details</summary>
Motivation: To enable extreme video compression (as low as 0.01%) for bandwidth-constrained environments like emergency rescue, remote surveillance, and mobile edge computing, addressing the fundamental trade-off between compression rate and computation.

Method: Generative Video Compression (GVC) framework that encodes video into extremely compact representations and uses generative video models at the receiver to reconstruct content from minimal transmitted information, with a compression-computation trade-off strategy for practical deployment on consumer-grade GPUs.

Result: Achieves compression rates as low as 0.02% in some cases, demonstrating viability for extreme compression while maintaining perception-centric, task-oriented communication (Level C of Shannon-Weaver model).

Conclusion: GVC offers a new effective, efficient, scalable, and practical video communication paradigm that shifts computation burden from transmission to inference, opening possibilities for bandwidth- and resource-constrained environments.
...
### [203] [Automated Classification of First-Trimester Fetal Heart Views Using Ultrasound-Specific Self-Supervised Learning](https://arxiv.org/abs/2512.24492)
*Youssef Megahed,Aylin Erman,Robin Ducharme,Mark C. Walker,Steven Hawken,Adrian D. C. Chan*

Main category: eess.IV

TL;DR: USF-MAE, a self-supervised ultrasound foundation model pretrained on 370K+ unlabeled ultrasound images, outperforms supervised CNN and ViT baselines for first-trimester fetal heart view classification, achieving 90.57% accuracy without requiring aggressive preprocessing.


<details>
  <summary>Details</summary>
Motivation: First-trimester fetal echocardiography enables early detection of congenital heart disease but automated analysis is challenging due to small cardiac structures, low signal-to-noise ratio, and inter-operator variability. Current methods struggle with these limitations.

Method: USF-MAE uses masked autoencoding pretraining on 370,000+ unlabeled ultrasound images across 40+ anatomical regions. The pretrained Vision Transformer encoder is fine-tuned on 6,720 first-trimester fetal echocardiography images to classify five categories (aorta, atrioventricular flows, V sign, X sign, Other). Benchmarking against ResNet-18, ResNet-50, and ImageNet-pretrained ViT-B/16 with identical preprocessing and training protocols.

Result: USF-MAE achieved best performance: 90.57% accuracy, 91.15% precision, 90.57% recall, 90.71% F1-score. Outperformed strongest baseline (ResNet-18) by +2.03% accuracy and +1.98% F1-score. Demonstrated robust performance without aggressive preprocessing and improved discrimination of non-diagnostic frames.

Conclusion: Self-supervised ultrasound foundation models like USF-MAE offer superior performance for first-trimester fetal heart view classification compared to supervised approaches, potentially improving early detection of congenital heart disease while reducing reliance on manual preprocessing.
...
### [204] [An Adaptive, Disentangled Representation for Multidimensional MRI Reconstruction](https://arxiv.org/abs/2512.24674)
...
<details>
  <summary>Details</summary>
Motivation: To develop a learning-based approach for multidimensional MRI reconstruction that works well with limited training data by better exploiting feature correlations and incorporating pre-learned priors.

Method: Uses encoder-decoder network with style-based decoder for feature disentanglement, latent diffusion models for feature space constraints, zero-shot self-supervised learning adaptation, and subspace modeling integration.

Result: Improved performance over state-of-the-art methods on accelerated T1 and T2 parameter mapping, without requiring task-specific supervised training or fine-tuning.

Conclusion: Provides a new strategy for learning-based multidimensional image reconstruction that works effectively with limited problem-specific training data.
...
### [205] [Hierarchical Quasi-cyclic Codes from Reed-Solomon and Polynomial Evaluation Codes](https://arxiv.org/abs/2512.23872)
*Emily McMillon,Kathryn Haymaker*

Main category: cs.IT

TL;DR: First algebraic construction of hierarchical quasi-cyclic codes using Reed-Solomon codes and Kautz-Singleton superimposed codes, with hierarchy levels and index determined by field size.


<details>
  <summary>Details</summary>
Motivation: To provide an algebraic construction of hierarchical quasi-cyclic codes, moving beyond simulation-based approaches in existing literature, and to establish theoretical bounds on code parameters.

Method: Construct hierarchical quasi-cyclic codes from Reed-Solomon codes using Kautz and Singleton's 1964 superimposed code construction. Analyze polynomial evaluation codes and derive algebraic bounds on parameters like rank and distance.

Result: Hierarchical quasi-cyclic codes with hierarchy levels and index determined by field size; codes with dimension k=2 yield Tanner graphs of girth 6; some codes achieve best known minimum distance for binary codes while maintaining hierarchical quasi-cyclic structure.

Conclusion: Novel algebraic approach provides theoretical foundations for hierarchical quasi-cyclic codes, establishing parameter bounds and connecting to existing constructions while offering improved structural understanding.
...
### [206] [Continuous Angular Power Spectrum Recovery From Channel Covariance via Chebyshev Polynomials](https://arxiv.org/abs/2512.24039)
*Shengsong Luo,Ruilin Wu,Chongbin Xu,Junjie Ma,Xiaojun Yuan,Xin Wang*

Main category: cs.IT

TL;DR: Proposes Chebyshev polynomial expansion framework for recovering continuous angular power spectrum from channel covariance, enabling accurate APS reconstruction and reliable FDD downlink covariance prediction.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the ill-posed problem of angular power spectrum inversion from channel covariance in multi-antenna systems, particularly for frequency division duplex settings where downlink covariance prediction from uplink measurements is challenging.

Method: Uses Chebyshev polynomial expansion to transform APS recovery into finite-dimensional linear regression. Derives exact series representation of covariance, introduces semidefinite characterization of nonnegative APS, and adds derivative-based regularizer for smooth APS profiles while preserving cluster transitions.

Result: Simulation results show accurate APS reconstruction and reliable downlink covariance prediction from uplink measurements in FDD settings. The Chebyshev-based framework effectively exploits smoothness and nonnegativity properties.

Conclusion: Jointly exploiting smoothness and nonnegativity in Chebyshev domain provides effective tool for covariance-domain processing in multi-antenna systems, enabling accurate APS recovery and FDD downlink covariance prediction.
...
### [207] [Random Multiplexing](https://arxiv.org/abs/2512.24087)
*Lei Liu,Yuhao Chi,Shunqi Huang,Zhaoyang Zhang*

Main category: cs.IT

TL;DR: Random multiplexing technique decoupled from physical channels enables robust wireless communication in dynamic environments, achieving optimal BER performance with AMP-type detectors.


<details>
  <summary>Details</summary>
Motivation: Traditional multiplexing techniques (SC-FDE, OFDM, OTFS, AFDM) rely on specific channel structures and lack robustness in dynamic real-world environments. There's a need for techniques that work with arbitrary channel matrices.

Method: Proposes random multiplexing technique that constructs equivalent input-isotropic channel matrix in random transform domain, enabling application to arbitrary norm-bounded and spectrally convergent channel matrices. Uses cross-domain memory AMP (CD-MAMP) detector leveraging time-domain channel sparsity and equivalent channel randomness.

Result: Achieves statistical fading-channel ergodicity, guarantees asymptotic replica MAP BER optimality of AMP-type detectors, derives optimal power allocations to minimize BER and maximize constrained capacity, and demonstrates versatility in diverse wireless applications.

Conclusion: Random multiplexing provides a robust, channel-agnostic approach that overcomes limitations of structure-dependent multiplexing techniques, enabling reliable communication in dynamic wireless environments with optimal performance guarantees.
...
### [208] [When Wires Can't Keep Up: Reconfigurable AI Data Centers Empowered by Terahertz Wireless Communications](https://arxiv.org/abs/2512.24110)
*Chong Han,Mingjie Zhu,Wenqi Zhao,Ziming Yu,Guolong Huang,Guangjian Wang,Wen Tong,Wenjun Zhang*

Main category: cs.IT

TL;DR: THz wireless data centers proposed as solution for AI workloads, offering 1 Tbps links, sub-50 ns latency, and sub-10 pJ/bit efficiency over 20m, surpassing copper/optical in specific domains.


<details>
  <summary>Details</summary>
Motivation: Traditional copper and optical interconnects face fundamental challenges in latency, power consumption, and rigidity, constraining scalability of distributed AI clusters in modern data centers.

Method: Proposes Terahertz Wireless Data Center (THz-WDC) combining ultra-broadband capacity with one-hop low-latency communication. Key enabling technologies include digital-twin-based orchestration, low-complexity beam manipulation, all-silicon THz transceivers, and low-complexity analog baseband architectures.

Result: Numerical analysis identifies specific distance and throughput domains where THz links surpass conventional wired solutions. THz wireless provides flexible mechanism for interconnecting, testing, and reconfiguring quantum and chiplet-based modular architectures.

Conclusion: THz wireless links offer path toward wireless-defined, reconfigurable, and sustainable AI data centers, with roadmap for implementation and specific performance advantages over copper/optical interconnects.
...
### [209] [Efficient Decoding of Twisted GRS Codes and Roth--Lempel Codes](https://arxiv.org/abs/2512.24217)
*Runtian Zhu,Lingfei Jin*

Main category: cs.IT

TL;DR: Proposed efficient list and unique decoding algorithms for TGRS and Roth-Lempel codes with near-linear time complexity, extending beyond GRS codes and supporting many twists.


<details>
  <summary>Details</summary>
Motivation: Most known MDS codes are GRS codes, but non-GRS codes like TGRS and Roth-Lempel codes are less understood and practically important for cryptography where GRS structure is undesirable. While construction and analysis of these codes exist, efficient decoding algorithms remain underdeveloped.

Method: Developed list and unique decoding algorithms based on the Guruswami-Sudan algorithm for TGRS and Roth-Lempel codes. The approach achieves near-linear running time under suitable parameter conditions and supports TGRS codes with up to O(n²) twists. Incorporated AMD codes into list-decoding framework for message recovery.

Result: Achieved near-linear time complexity (improving from quadratic), extended TGRS decoding to support many twists (beyond single-twist), provided first efficient decoder for Roth-Lempel codes, surpassed classical unique-decoding radius for many parameters, and enabled correct message recovery with AMD codes.

Conclusion: The paper presents significant advances in decoding non-GRS MDS codes, providing efficient algorithms for TGRS and Roth-Lempel codes that overcome previous limitations and open new possibilities for practical applications in cryptography and beyond.
...
### [210] [SC-LDPC Codes Over $\mathbb{F}_q$: Minimum Distance, Decoding Analysis and Threshold Saturation](https://arxiv.org/abs/2512.24232)
*Jiaxin Lyu,Guanghui He*

Main category: cs.IT

TL;DR: This paper analyzes random spatially coupled LDPC codes over finite fields, proving asymptotic distance properties and establishing universal threshold saturation for belief propagation decoding on q-ary symmetric channels.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a comprehensive theoretical framework for analyzing spatially coupled LDPC codes over finite fields, particularly focusing on their distance properties and iterative decoding performance. The researchers aim to understand how different coupling strategies affect code performance and establish universal threshold saturation results.

Method: The authors define random Tanner graphs for coupled ensembles using multiple independent, uniformly random monomial maps under different variable-node edge-spreading rules. They analyze two main ensembles: standard and improved coupled ensembles. They introduce analytical tools including symmetric probability measures, reference measures on probability simplices, metric topologies, and degradation theory to study iterative decoding thresholds on q-ary input memoryless symmetric channels (QMSCs).

Result: Both coupled ensembles exhibit asymptotically good minimum distance and minimum stopping set size. The improved coupled ensemble achieves better distance performance than the standard coupled ensemble. The paper establishes a universal threshold saturation result: as coupling parameters increase, the belief-propagation threshold of coupled systems saturates to a well-defined threshold that depends only on the underlying ensemble and channel family.

Conclusion: Spatially coupled LDPC codes over finite fields provide asymptotically good distance properties and exhibit threshold saturation behavior under belief propagation decoding. The improved coupling strategy offers superior distance performance, and the developed analytical framework enables rigorous analysis of iterative decoding thresholds across various finite fields and symmetric channels.
...
### [211] [Infinite families of graphs and stable completion of arbitrary matrices, Part I](https://arxiv.org/abs/2512.24468)
*Augustin Cosse*

Main category: cs.IT

TL;DR: The paper studies deterministic graph constructions where low-rank matrix completion is generically possible regardless of entry values, relating completability to specific patterns in lattice graphs.


<details>
  <summary>Details</summary>
Motivation: To develop deterministic constructions of graphs that guarantee generic low-rank matrix completion, enabling exact and stable completion for fixed-rank matrices through sum-of-squares hierarchy.

Method: Relates matrix completability to presence of specific patterns (unions of self-avoiding walks) in the subgraph of lattice graphs generated from bi-adjacency matrix support. Uses these patterns to design infinite families of graphs.

Result: Develops construction methods for infinite families of graphs where exact and stable low-rank matrix completion is possible for every fixed rank matrix via sum-of-squares hierarchy.

Conclusion: Deterministic graph constructions with specific pattern properties enable guaranteed generic low-rank matrix completion, providing foundations for reliable matrix completion algorithms.
...
### [212] [Throughput Optimization in UAV-Mounted RIS under Jittering and Imperfect CSI via DRL](https://arxiv.org/abs/2512.24773)
*Anas K. Saeed,Mahmoud M. Salim,Ali Arshad Nasir,Ali H. Muqaibel*

Main category: cs.IT

TL;DR: DRL-based optimization for UAV-mounted RIS systems under jitter and imperfect CSI, outperforming conventional methods in challenging conditions with much faster inference.


<details>
  <summary>Details</summary>
Motivation: UAV-mounted RIS can reshape wireless propagation but suffers from performance sensitivity to UAV jitter and cascaded channel uncertainty, requiring robust optimization methods.

Method: Model-free DRL framework with contextual bandit formulation, using differentiable feasibility layer to handle constraints, instantiated with constrained DDPG and TD3 variants without target networks.

Result: Proposed DRL methods achieve higher throughput than AO-WMMSE baselines under severe jitter/low CSI, with 0-12% gap to SAA benchmark, and 0.6ms inference time vs 370-550ms for conventional solvers.

Conclusion: DRL provides efficient real-time optimization for UAV-RIS systems under practical impairments, balancing performance and computational efficiency for practical deployment.
...
### [213] [Overflow-Avoiding Memory AMP](https://arxiv.org/abs/2407.03898)
*Shunqi Huang,Lei Liu,Brian M. Kurkoski*

Main category: cs.IT

TL;DR: Two improvements to GD-MAMP algorithm: overflow-avoiding version to handle floating point issues, and complexity-reduced version that cuts matrix-vector products by 1/3 while maintaining convergence speed.


<details>
  <summary>Details</summary>
Motivation: GD-MAMP inherits strengths of AMP and OAMP/VAMP for signal recovery in high-dimensional noisy linear systems, but has issues with overflow from intermediate variables exceeding floating point range, and high computational complexity requiring 3 matrix-vector products per iteration.

Method: 1) Overflow-avoiding GD-MAMP (OA-GD-MAMP) addresses floating point overflow problems. 2) Complexity-reduced GD-MAMP (CR-GD-MAMP) reduces matrix-vector products from 3 to 2 per iteration while maintaining similar convergence performance.

Result: Two improved algorithms: OA-GD-MAMP solves overflow issues, and CR-GD-MAMP reduces computational complexity by 1/3 with minimal impact on convergence speed.

Conclusion: The proposed improvements make GD-MAMP more practical and efficient for real-world applications by addressing numerical stability issues and reducing computational overhead.
...
### [214] [Random Modulation: Achieving Asymptotic Replica Optimality over Arbitrary Norm-Bounded and Spectrally Convergent Channel Matrices](https://arxiv.org/abs/2508.08099)
*Lei Liu,Yuhao Chi,Shunqi Huang*

Main category: cs.IT

TL;DR: Random modulation technique decoupled from channel matrix enables AMP-type detectors to achieve replica MAP BER optimality for arbitrary norm-bounded channels, with 2-3 dB performance gains over existing OFDM/OTFS/AFDM systems.


<details>
  <summary>Details</summary>
Motivation: Existing modulation schemes like OFDM/OTFS/AFDM are coupled with specific channel structures, limiting their applicability to arbitrary channel matrices. There's a need for a modulation technique that works with arbitrary norm-bounded channels while enabling optimal detection performance.

Method: Proposes random modulation that constructs equivalent dense random channel matrix, ensuring sufficient statistical channel fading. Develops CD-MAMP detector leveraging time-domain channel sparsity and random transform-domain channel. Derives optimal power allocation schemes for replica MAP BER minimization and capacity maximization.

Result: Achieves asymptotic replica MAP BER optimality for AMP-type detectors with arbitrary norm-bounded channels. Numerical results show 2-3 dB BER/BLER performance gains over existing OFDM/OTFS/AFDM with 5G-NR LDPC codes under both average and optimized power allocation.

Conclusion: Random modulation provides a universal framework for arbitrary channel matrices, enabling optimal detection performance with significant practical gains over current modulation schemes, especially when combined with optimal power allocation.
...
### [215] [RUL-QMoE: Multiple Non-crossing Quantile Mixture-of-Experts for Probabilistic Remaining Useful Life Predictions of Varying Battery Materials](https://arxiv.org/abs/2512.23725)
*Sel Ly,Rufan Yang,Ninad Dixit,Hung Dinh Nguyen*

Main category: eess.SP

TL;DR: A material-based Mixture-of-Experts (MoE) approach called RUL-QMoE is proposed for predicting battery remaining useful life, addressing heterogeneous battery chemistries through specialized expert networks for different cathode materials.


<details>
  <summary>Details</summary>
Motivation: Existing RUL prediction models often overlook battery cathode materials and designs, ignoring their electrochemical effects on degradation. This lack of material consideration leads to poor generalizability, especially when battery specifications are unavailable or batteries use mixed materials.

Method: The paper proposes RUL-QMoE, a probabilistic Multiple Non-crossing Quantile Mixture-of-Experts framework. It integrates specialized expert networks for five battery types (LFP, NCA, NMC, LCO, NMC-LCO) with a gating mechanism that dynamically assigns relevance based on input features. Non-crossing quantile regression enables coherent predictive distributions.

Result: The RUL-QMoE model produces coherent and interpretable predictive distributions of battery RUL, enabling robust uncertainty quantification. It accommodates battery operational conditions and addresses heterogeneous battery chemistries through material-specific expert networks.

Conclusion: The proposed material-based MoE approach effectively tackles the challenges of heterogeneous battery chemistries in RUL prediction, providing a generalizable solution with uncertainty quantification that accounts for different cathode materials and designs.
...
### [216] [Beamforming for Massive MIMO Aerial Communications: A Robust and Scalable DRL Approach](https://arxiv.org/abs/2512.23902)
*Hesam Khoshkbari,Georges Kaddoum,Omid Abbasi,Bassant Selim,Halim Yanikomeroglu*

Main category: eess.SP

TL;DR: Distributed beamforming framework for massive MIMO NTN using entropy-based multi-agent DRL with Fourier Neural Operators for downlink sum-rate maximization under imperfect CSI.


<details>
  <summary>Details</summary>
Motivation: Need for scalable and robust beamforming in massive MIMO non-terrestrial networks with imperfect channel state information, supporting large-scale user deployments and aerial layers while handling CSI imperfections and user mobility.

Method: Novel entropy-based multi-agent DRL approach where each NTBS independently computes beamforming vectors using Fourier Neural Operators to capture frequency-domain dependencies, integrated with transfer learning via conjugate prior mechanism and low-rank decomposition for scalability.

Result: Superior performance over WMMSE, ZF, MRT, CNN-based DRL, and DDPG in average sum rate, robustness to CSI imperfection, user mobility, and scalability across varying network sizes and user densities, with significant computational efficiency and reduced communication overhead.

Conclusion: The proposed framework effectively addresses distributed beamforming challenges in massive MIMO NTNs, offering superior performance, robustness, and scalability while reducing computational and communication costs compared to existing methods.
...
### [217] [A multimodal Transformer for InSAR-based ground deformation forecasting with cross-site generalization across Europe](https://arxiv.org/abs/2512.23906)
*Wendong Yao,Binhua Huang,Soumyabrata Dev*

Main category: eess.SP

TL;DR: A multimodal patch-based Transformer model for nowcasting ground deformation from InSAR time series, achieving high accuracy in predicting next-epoch displacement maps.


<details>
  <summary>Details</summary>
Motivation: Near-real-time monitoring of ground deformation is needed for urban planning, infrastructure management, and hazard mitigation. While existing services like EGMS provide dense observations of past motion, predicting future deformation remains challenging due to complex superposition of long-term trends, seasonal cycles, and abrupt discontinuities with spatial heterogeneity.

Method: Proposes a multimodal patch-based Transformer for single-step, fixed-interval next-epoch nowcasting of displacement maps from EGMS time series. The model ingests recent displacement snapshots along with static kinematic indicators (mean velocity, acceleration, seasonal amplitude) computed from training window only, and harmonic day-of-year encodings. Evaluated on 64x64 grid over 100 km x 100 km tiles.

Result: On the eastern Ireland tile (E32N34), the multimodal Transformer outperforms CNN-LSTM, CNN-LSTM+Attn, and multimodal STGCN when all models receive the same multimodal inputs, achieving RMSE = 0.90 mm and R² = 0.97 on the test set with best threshold accuracies.

Conclusion: The multimodal patch-based Transformer effectively addresses the challenge of ground deformation nowcasting by leveraging both recent displacement patterns and static kinematic indicators, demonstrating superior performance over existing approaches for predicting next-epoch displacement maps.
...
### [218] [Movable Antenna Enhanced Multi-Region Beam Coverage: A Multi-Notch-Filter-Inspired Design](https://arxiv.org/abs/2512.24090)
*Dong Wang,Weidong Mei,Zhi Chen,Boyu Ning*

Main category: eess.SP

TL;DR: MA technology enhances wireless coverage by optimizing antenna positions; proposed algorithm maximizes minimum beam gain over multiple subregions using spatial multi-notch filter design and Gibbs sampling.


<details>
  <summary>Details</summary>
Motivation: Movable antennas offer new degrees of freedom for improving wireless communication performance, particularly for achieving wide beam coverage over multiple spatial subregions, which is challenging with fixed position antennas.

Method: Joint optimization of transmit beamforming and antenna position vector using spatial multi-notch filter design approach; constructs continuous amplitude/phase-shift profile, then uses sequential update algorithm with Gibbs sampling to select optimal MA positions while avoiding local optima.

Result: Proposed algorithm significantly outperforms conventional fixed position antennas and achieves comparable performance to alternating optimization algorithm with dramatically lower computational complexity.

Conclusion: MA technology with the proposed optimization algorithm enables efficient wide beam coverage over multiple subregions, offering substantial performance gains over traditional approaches with manageable complexity.
...
### [219] [The Wigner-Ville Transform as an Information Theoretic Tool in Radio-frequency Signal Analysis](https://arxiv.org/abs/2512.24488)
*Erik Lentz,Emily Ellwein,Bill Kay,Audun Myers,Cameron Mackenzie*

Main category: eess.SP

TL;DR: The paper presents Wigner-Ville transform as an information measurement tool using Tsallis' entropy, showing significant sensitivity advantages over energy-based methods for RF signal detection.


<details>
  <summary>Details</summary>
Motivation: To improve signal detection in noisy environments by using Wigner-Ville transform as an information measurement tool rather than just energy-based methods, especially for applications with limited observation resources and transient signals.

Method: Uses Wigner-Ville transform combined with Tsallis' entropy and information functionals to detect and localize information-laden signals in noisy backgrounds and measure their information volumes.

Result: Wigner-Ville-based detection provides significant sensitivity advantages (over 15 dB in some contexts) over energy-based measures without extensive training, particularly beneficial for resource-limited applications.

Conclusion: Wigner-Ville transform methods can improve sensing effectiveness by multiple orders of magnitude for applications with time/space integration pressures and transient signals, with potential for advancing several such applications.
...
### [220] [Discovering Optimal Robust Minimum Redundancy Arrays (RMRAs) through Exhaustive Search and Algebraic Formulation of a New Sub-Optimal RMRA](https://arxiv.org/abs/2512.24155)
*Ashish Patwari,Sanjeeva Reddy S,G Ramachandra Reddy*

Main category: eess.SP

TL;DR: This paper proposes methods to find optimal robust minimum redundancy arrays (RMRAs) for N>10 sensors and develops closed-form expressions for near-optimal RMRAs, extending known configurations and providing systematic design formulas.


<details>
  <summary>Details</summary>
Motivation: Current sparse arrays are vulnerable to single-sensor failures, while existing robust arrays (TFRSAs and RMRAs) provide redundancy but optimal configurations are only known for N=6 to 10 sensors. There's a need to extend optimal RMRAs to larger arrays and develop systematic design methods.

Method: Two-stage approach: (1) Using exhaustive search in MATLAB to find optimal RMRAs for N=11-14 and near/sub-optimal arrays for N=15-20; (2) Applying pattern mining and algebraic generalization to derive closed-form expressions for a new family of near-optimal RMRAs.

Result: Successfully found optimal RMRAs for N=11-14 and near/sub-optimal arrays for N=15-20. Developed closed-form expressions for sensor positions, aperture, and degrees of freedom that are valid for N≥8, creating a large catalogue of valid configurations.

Conclusion: The work extends the catalogue of known optimal RMRAs and formulates a sub-optimal RMRA family with closed-form expressions, providing systematic design tools for robust sparse arrays that are resilient to single-sensor failures.
...
### [221] [A Uniform Pilot and Data Payload Optimization Framework for OTFS-Based ISAC](https://arxiv.org/abs/2512.24624)
*Borui Du,Yumeng Zhang,Christos Masouros,Bruno Clerckx*

Main category: eess.SP

TL;DR: Optimal signal design for OTFS dual-functional radar and communication systems, balancing sensing and communication performance through pilot design and power allocation.


<details>
  <summary>Details</summary>
Motivation: OTFS is promising for high-mobility environments but long frame duration causes latency when radar and communication are separate. DFRC mode reduces latency by performing both simultaneously, but optimal signal design is insufficiently explored.

Method: Derived channel capacity lower bound for communication considering channel estimation errors, and integrated sidelobe level (ISL) for sensing accounting for data symbol randomness. Formulated optimization problem balancing radar/communication performance and solved using alternating optimization framework.

Result: OTFS DFRC requires deterministic pilot signal with concentrated DD domain peak for common sensing/channel estimation structure, with data multiplexed in other grids. Achieves at least 9.45 dB ISL suppression for sensing and 4.82 dB SINR gain for communication compared to conventional OTFS.

Conclusion: Proposed OTFS DFRC signal design unifies sensing and communication within single signal, expands performance region, and significantly improves both sensing and communication metrics compared to conventional approaches.
...
### [222] [Quantifying the advantage of vector over scalar magnetic sensor networks for undersea surveillance](https://arxiv.org/abs/2512.24250)
*Wenchao Li,Xuezhi Wang,Qiang Sun,Allison N. Kealy,Andrew D. Greentree*

Main category: eess.SP

TL;DR: Vector magnetometer networks outperform scalar networks for maritime target tracking using UKF approach


<details>
  <summary>Details</summary>
Motivation: Magnetic monitoring of maritime environments is crucial for shipping optimization and national security, with new quantum magnetometers enabling better sensor network evaluation

Method: Modeling comparison of scalar vs vector magnetometer network architectures, implementing unscented Kalman filter for target tracking

Result: Vector networks provide significant improvement in target tracking accuracy and resilience compared to scalar networks

Conclusion: Vector magnetometer networks are superior for maritime target identification and tracking applications
...
### [223] [OptiVote: Non-Coherent FSO Over-the-Air Majority Vote for Communication-Efficient Distributed Federated Learning in Space Data Centers](https://arxiv.org/abs/2512.24334)
*Anbang Zhang,Chenyuan Feng,Wai Ho Mow,Jia Ye,Shuaishuai Guo,Geyong Min,Tony Q. S. Quek*

Main category: eess.SP

TL;DR: OptiVote: A robust non-coherent FSO AirComp framework for federated learning in space data centers that eliminates phase synchronization requirements and handles dynamic space impairments.


<details>
  <summary>Details</summary>
Motivation: Mega-constellations enable space data centers but face challenges for distributed federated learning due to bandwidth/energy constraints and dynamic link conditions in space environments. Conventional coherent AirComp requires stringent phase alignment that's difficult to maintain due to satellite jitter and Doppler effects.

Method: Integrates signSGD with majority-vote aggregation and pulse-position modulation (PPM), where satellites convey gradient signs via orthogonal PPM time slots. Uses non-coherent energy accumulation for aggregation, eliminating phase synchronization needs. Includes CSI-free dynamic power control to mitigate aggregation bias from heterogeneous FSO channels.

Result: Theoretical analysis characterizes aggregate error probability under statistical FSO channels and establishes convergence guarantees for non-convex objectives. The framework transforms phase-sensitive field superposition into phase-agnostic optical intensity combining.

Conclusion: OptiVote provides a robust, communication-efficient solution for federated learning in space data centers by overcoming phase synchronization challenges and handling dynamic space impairments through non-coherent FSO AirComp with adaptive power control.
...
### [224] [Low-complexity spectral shaping method for OFDM signals with dynamically adaptive emission mask](https://arxiv.org/abs/2512.24412)
*Javier Giménez,José A. Cortés,Luis Díez*

Main category: eess.SP

TL;DR: A framework using preoptimized pulses for dynamic spectral shaping of OFDM signals in cognitive radio environments without complex optimization.


<details>
  <summary>Details</summary>
Motivation: OFDM signals with rectangular pulses have poor spectral confinement, which is problematic in overcrowded spectrum environments. Cognitive radio requires dynamic spectral shaping that can adapt to non-contiguous, time-varying spectrum availability.

Method: Proposes a framework using a reduced set of preoptimized pulses that combine active interference cancellation (AIC) and adaptive symbol transition (AST) terms. These pulses can be dynamically adapted to different spectral widths and locations through simple transformations, transparent to the receiver.

Result: Enables online adaptation of OFDM spectral shaping to changing spectrum conditions without solving NP-hard optimization problems required by existing methods.

Conclusion: Provides a practical solution for dynamic spectral shaping in cognitive radio systems by using transformable preoptimized pulses that eliminate the need for complex optimization while maintaining receiver transparency.
...
### [225] [Power Minimization in Pinching-Antenna Systems under Probabilistic LoS Blockage](https://arxiv.org/abs/2512.24573)
*Lei Li,Yanqing Xu,Tenghao Cai,Tsung-Hui Chang*

Main category: eess.SP

TL;DR: Power minimization for multi-pinching-antenna systems with probabilistic LoS blockage, achieving substantial power savings vs fixed antennas.


<details>
  <summary>Details</summary>
Motivation: Pinching antennas offer flexible positioning to mitigate large-scale attenuation in wireless networks, but optimal antenna positioning and beamforming design under probabilistic LoS blockage needs investigation.

Method: For single PA: prove convexity and find global optimum. For multiple PAs: derive closed-form beamforming structures and develop efficient first-order algorithm for high-quality local solutions.

Result: Extensive numerical validation shows proposed designs are effective and PA systems achieve substantial power savings compared to conventional fixed-antenna systems.

Conclusion: Pinching antenna systems with optimized AP-BF design significantly outperform fixed-antenna systems in power efficiency under probabilistic LoS blockage conditions.
...
### [226] [Resource Allocation via Backscatter-Aware Transmit Antenna Selection for Low-PAPR and Ultra-Reliable WSNs](https://arxiv.org/abs/2512.24583)
*Rahul Gulia,Ashish Sheikh,Feyisayo Favour Popoola,Serisha Vadlamudi*

Main category: eess.SP

TL;DR: BC-TAS framework optimizes multi-antenna illuminator selection in MD-OFDM systems to balance high-throughput communication with passive backscatter sensor power requirements, achieving dual-envelope regulation and improved energy efficiency.


<details>
  <summary>Details</summary>
Motivation: Addresses the fundamental conflict between high-throughput primary communication and stringent power envelope requirements of passive backscatter sensors in hybrid WSNs, where conventional SNR-centric approaches fail to meet both needs simultaneously.

Method: Proposes Backscatter-Constrained Transmit Antenna Selection (BC-TAS) framework using per-subcarrier antenna selection in MD-OFDM architecture with multi-objective cost function that jointly maximizes link reliability, stabilizes RF energy envelope at passive SAW sensors, and suppresses interference. Incorporates Kalman-based channel smoothing for robustness under imperfect CSI.

Result: Achieves orders-of-magnitude improvement in outage probability and significant energy efficiency gains compared to conventional MU-MIMO baselines, while ensuring spectral mask compliance under reduced power amplifier back-off. Enables dual-envelope regulation reducing both transmitter PAPR and Backscatter Crest Factor.

Conclusion: BC-TAS establishes an effective illuminator-side control mechanism for reliable and energy-stable sensing and communication coexistence in dense, power-constrained wireless environments, solving the fundamental physical layer conflict in hybrid WSNs.
...
### [227] [Beam-Squint-Aided Hierarchical Sensing for Integrated Sensing and Communications with Uniform Planar Arrays](https://arxiv.org/abs/2512.24727)
*Jaehong Jo,Jihun Park,Yo-Seb Jeon,H. Vincent Poor*

Main category: eess.SP

TL;DR: A hierarchical sensing framework for wideband ISAC with UPAs that leverages beam-squint effect for efficient 2D angle estimation through multi-stage sensing process.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient wideband integrated sensing and communications (ISAC) system that can perform accurate two-dimensional angle estimation while minimizing sensing power and computational complexity, addressing challenges in conventional sensing methods.

Method: Proposes a hierarchical sensing framework with structured multi-stage process: first searches elevation angles, then azimuth angles given estimated elevations. Uses true-time-delay lines and phase shifters in UPA to cover multiple grid points simultaneously across OFDM subcarriers. Formulates angle estimation as sparse signal recovery problem and develops modified matching pursuit algorithm tailored to hierarchical architecture. Designs power allocation strategies to minimize total transmit power while meeting both sensing and communication requirements.

Result: Numerical results demonstrate superior performance over conventional sensing methods with reduced sensing power, showing efficient and accurate target localization capabilities.

Conclusion: The proposed hierarchical sensing framework effectively leverages beam-squint effect in wideband OFDM systems to enable efficient 2D angle estimation with reduced power consumption, offering a promising solution for wideband ISAC systems with uniform planar arrays.
...
### [228] [Digitalizing Over-the-Air Computation via The Novel Complement Coded Modulation](https://arxiv.org/abs/2512.24788)
*Zhixu Wang,Jiacheng Yao,Wei Xu,Wei Shi,Kaibin Huang*

Main category: eess.SP

TL;DR: Proposes a two's complement-based coding scheme for digital over-the-air computation with compatible digital modulations, achieving asymptotic error-free computation with minimal codeword length.


<details>
  <summary>Details</summary>
Motivation: To overcome inherent limitations of analog signals in over-the-air computation (AirComp) by developing a digital approach that can work with existing digital modulation schemes.

Method: Quantized discrete values are encoded into binary sequences using two's complement and transmitted over multiple subcarriers. A decoder constructs functional mapping between superimposed digital signals and computational targets. Uses truncated inversion for pre-processing, derives optimal LMMSE detector, and proposes uneven power allocation across subcarriers.

Result: The scheme achieves asymptotic error-free computation with minimal codeword length. Numerical results show superiority over existing digital AirComp approaches, especially at low SNR regimes.

Conclusion: The proposed two's complement-based coding scheme effectively enables digital AirComp with compatible digital modulations, overcoming analog limitations while maintaining computational accuracy through optimized detection and power allocation strategies.
...
### [229] [Efficient Joint Resource Allocation for Wireless Powered ISAC with Target Localization](https://arxiv.org/abs/2512.24815)
*Boyao Li,Qinwei He,Boao Zhang,Xiaopeng Yuan,Anke Schmeink*

Main category: eess.SP

TL;DR: Wireless powered ISAC system with target localization faces tradeoff between energy supply, communication throughput, and sensing accuracy. The paper proposes joint time-power optimization with CRB constraints and develops SCA-based algorithm for max-min throughput.


<details>
  <summary>Details</summary>
Motivation: Wireless powered ISAC systems face fundamental tradeoffs between energy supply, communication throughput, and sensing accuracy. There's a need to balance these competing objectives while meeting target localization requirements, which is challenging due to the nonconvex nature of the optimization problem.

Method: Joint optimization of WPT duration, ISAC transmission time allocation, and transmit power with CRB constraints for localization accuracy. The nonconvex problem is reformulated using variable substitutions and exploiting monotonicity of logarithmic functions, then solved using a successive convex approximation (SCA)-based iterative algorithm.

Result: Simulation results demonstrate convergence of the proposed algorithm and significant performance gains over benchmark schemes. The coordinated time-power optimization effectively balances sensing accuracy and communication performance in wireless powered ISAC systems.

Conclusion: Coordinated time-power optimization is crucial for balancing sensing accuracy and communication performance in wireless powered ISAC systems. The proposed SCA-based algorithm provides an effective solution to the max-min throughput problem with CRB constraints, offering significant performance improvements over existing approaches.
...
### [230] [No Vision, No Wearables: 5G-based 2D Human Pose Recognition with Integrated Sensing and Communications](https://arxiv.org/abs/2512.24923)
*Haojin Li,Dongzhe Li,Anbang Zhang,Wenqi Zhang,Chen Sun,Haijun Zhang*

Main category: eess.SP

TL;DR: A 5G-based ISAC system using uplink SRS signals achieves superior 2D human pose recognition compared to vision/RF methods, addressing privacy, occlusion, and equipment limitations in indoor interactive applications.


<details>
  <summary>Details</summary>
Motivation: Current vision/RF-based HPR solutions face practical deployment challenges including privacy concerns, susceptibility to occlusion, dedicated equipment requirements, and limited sensing resolution/range. 5G ISAC technology offers a promising alternative by merging communication and sensing functions.

Method: Propose a 5G-based ISAC system that infers 2D HPR from uplink sounding reference signals (SRS). Extract rich features from multiple domains and use an encoder for unified alignment/representation in latent space. Fuse low-dimensional features to output human pose state.

Result: Experimental results show the proposed 5G-based ISAC HPR system significantly outperforms current mainstream baseline solutions in HPR performance in typical indoor environments.

Conclusion: The system provides a solid technical foundation for universal human-computer interaction by addressing key limitations of existing HPR solutions through 5G ISAC technology.
...
### [231] [Fundamental Limits for Near-Field Sensing -- Part I: Narrow-Band Systems](https://arxiv.org/abs/2512.24958)
*Tong Wei,Kumar Vijay Mishra,Bhavani Shankar M. R.,Björn Ottersten*

Main category: eess.SP

TL;DR: This paper develops fundamental limits for near-field sensing with extremely large antenna arrays (ELAAs) for 6G, deriving closed-form Cramér-Rao bounds for joint estimation of target position, velocity, and radar cross-section, with far-field and near-field approximations revealing scaling laws.


<details>
  <summary>Details</summary>
Motivation: Extremely large-scale antenna arrays (ELAAs) for 6G enable high-resolution sensing but operate in near-field regions where classical far-field models fail, requiring new theoretical foundations for fundamental estimation limits in near-field sensing systems.

Method: Develops unified narrow-band near-field signal model for joint parameter sensing of moving targets using ELAAs, applies Slepian-Bangs formulation to derive closed-form Cramér-Rao bounds (CRBs) for joint estimation of position, velocity, and RCS, and establishes explicit far-field and near-field approximations for interpretable insights.

Result: Derived closed-form CRBs reveal how estimation bounds scale with array aperture, target range, carrier wavelength, and integration length, exposing roles of self-information and cross terms, clarifying when Fresnel corrections become non-negligible. Simulations validate CRBs and approximations, demonstrating accurate agreement with analytical scaling laws.

Conclusion: The paper establishes fundamental theoretical limits for near-field sensing with ELAAs, providing beamformer and algorithm design guidelines for 6G sensing applications, with Part II presumably extending this theory further.
...
### [232] [Fundamental Limits for Near-Field Sensing -- Part II: Wide-Band Systems](https://arxiv.org/abs/2512.24962)
*Tong Wei,Kumar Vijay Mishra,Bhavani Shankar M. R.,Björn Ottersten*

Main category: eess.SP

TL;DR: Derives Cramér-Rao bounds for wide-band near-field sensing with ELAA arrays in 6G systems, showing how bandwidth, array size, and integration time affect estimation performance.


<details>
  <summary>Details</summary>
Motivation: 6G systems with extremely large antenna arrays will operate over broad bandwidths where delay, Doppler, and spatial effects become tightly coupled across frequency, requiring unified performance bounds for near-field wide-band sensing.

Method: Develops exact near-field wide-band signal model for OFDM systems, uses Slepian-Bangs formulation to derive Fisher information matrix and CRBs for joint position/velocity/RCS estimation, and provides tractable far-field/near-field approximations.

Result: Derives fundamental estimation limits showing how wide-band information aggregates across subcarriers, provides design insights into bandwidth, integration length, and array aperture effects, and validates bounds through simulations.

Conclusion: Establishes unified performance bounds for wide-band near-field sensing in 6G systems, clarifying when wide-band effects matter and providing analytical scaling laws for system design.
...
### [233] [The Drill-Down and Fabricate Test (DDFT): A Protocol for Measuring Epistemic Robustness in Language Models](https://arxiv.org/abs/2512.23850)
*Rahul Baxi*

Main category: cs.AI

TL;DR: The paper introduces DDFT, a new evaluation protocol that measures epistemic robustness - how well language models maintain factual accuracy under semantic compression and adversarial fabrication, revealing that robustness is orthogonal to model size/architecture and depends on verification mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current language model evaluations (like MMLU, TruthfulQA) only measure knowledge under ideal conditions, not how robustly models maintain factual accuracy when information degrades or when adversaries probe for weaknesses. There's a need to assess epistemic robustness before deployment in critical applications.

Method: Introduces the Drill-Down and Fabricate Test (DDFT) protocol that measures epistemic robustness through progressive semantic compression and adversarial fabrication. Proposes a two-system cognitive model: Semantic System (generates fluent text) and Epistemic Verifier (validates factual accuracy). Evaluates 9 frontier models across 8 knowledge domains at 5 compression levels (1,800 turn-level evaluations).
...
Conclusion: Epistemic robustness emerges from training methodology and verification mechanisms distinct from current approaches. The DDFT framework provides both theoretical foundation and practical tools for assessing epistemic robustness before deployment in critical applications, challenging assumptions about the relationship between model size and reliability.
...
### [234] [CASCADE: Cumulative Agentic Skill Creation through Autonomous Development and Evolution](https://arxiv.org/abs/2512.23880)
*Xu Huang,Junwu Chen,Yuxing Fei,Zhuohan Li,Philippe Schwaller,Gerbrand Ceder*

Main category: cs.AI

TL;DR: CASCADE is a self-evolving LLM agent framework that enables agents to master complex tools and acquire skills through continuous learning and self-reflection, achieving 93.3% success on scientific tasks.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents rely on predefined tools or brittle tool generation, limiting their capability and adaptability to complex scientific tasks. There's a need to transition from "LLM + tool use" to "LLM + skill acquisition" for scalable AI-assisted scientific research.

Method: CASCADE enables agents to master external tools and codify knowledge through two meta-skills: 1) continuous learning via web search and code extraction, and 2) self-reflection via introspection and knowledge graph exploration. The framework includes human-agent collaboration and memory consolidation.
...
Conclusion: CASCADE represents a transition toward scalable AI-assisted scientific research by accumulating executable skills that can be shared across agents and scientists, moving beyond traditional tool-based approaches to true skill acquisition.
...
### [235] [A Proof-of-Concept for Explainable Disease Diagnosis Using Large Language Models and Answer Set Programming](https://arxiv.org/abs/2512.23932)
*Ioanna Gemou,Evangelos Lamprou*

Main category: cs.AI

TL;DR: McCoy combines LLMs and Answer Set Programming to automate medical diagnosis by translating literature into ASP code and processing patient data.


<details>
  <summary>Details</summary>
Motivation: Symbolic AI in healthcare requires significant effort to build knowledge bases, limiting adoption despite the critical need for accurate disease prediction for timely intervention and treatment.

Method: McCoy uses LLMs to translate medical literature into Answer Set Programming (ASP) code, combines this with patient data, and processes it through an ASP solver to generate diagnoses.
...
Conclusion: The integration of LLMs with ASP overcomes the knowledge base construction barrier in symbolic AI for healthcare, yielding an interpretable diagnostic system.
...
### [236] [SPARK: Search Personalization via Agent-Driven Retrieval and Knowledge-sharing](https://arxiv.org/abs/2512.24008)
*Gaurab Chhetri,Subasish Das,Tausif Islam Chowdhury*

Main category: cs.AI

TL;DR: SPARK is a multi-agent LLM framework for personalized search that uses specialized persona-based agents with coordinated retrieval and knowledge-sharing to model users' evolving information needs.


<details>
  <summary>Details</summary>
Motivation: Personalized search systems struggle with static profiles and monolithic retrieval pipelines that cannot capture users' evolving, multi-dimensional information needs. Current approaches lack the ability to model the complexity, fluidity, and context sensitivity of human information-seeking behavior.

Method: SPARK uses coordinated persona-based LLM agents with formalized persona spaces (role, expertise, task context, domain). A Persona Coordinator dynamically interprets queries to activate relevant specialized agents. Each agent performs independent retrieval-augmented generation with dedicated memory stores and context-aware reasoning. Inter-agent collaboration uses structured communication protocols including shared memory repositories, iterative debate, and relay-style knowledge transfer.

Result: The framework yields testable predictions about coordination efficiency, personalization quality, and cognitive load distribution. It incorporates adaptive learning for continuous persona refinement and models how emergent personalization properties arise from distributed agent behaviors governed by minimal coordination rules.

Conclusion: SPARK provides insights for next-generation search systems capable of capturing the complexity of human information-seeking behavior by integrating fine-grained agent specialization with cooperative retrieval, bridging cognitive architectures, multi-agent coordination theory, and information retrieval principles.
...
### [237] [ROAD: Reflective Optimization via Automated Debugging for Zero-Shot Agent Alignment](https://arxiv.org/abs/2512.24040)
...
TL;DR: ROAD is a novel automatic prompt optimization framework that uses multi-agent debugging instead of labeled datasets, achieving significant performance improvements with minimal iterations.


<details>
  <summary>Details</summary>
Motivation: Current APO methods require large labeled development sets, which are unavailable during cold-start agent development in real-world software engineering where only messy production logs exist.
...
Result: ROAD achieved 5.6% increase in success rate (73.6% to 79.2%) and 3.8% increase in search accuracy within three iterations; improved retail reasoning tasks by ~19% relative to baseline.
...
### [238] [LoongFlow: Directed Evolutionary Search via a Cognitive Plan-Execute-Summarize Paradigm](https://arxiv.org/abs/2512.24077)
*Chunhui Wan,Xunan Dai,Zhuo Wang,Minglei Li,Yanpeng Wang,Yinan Mao,Yu Lan,Zhiwen Xiao*

Main category: cs.AI

TL;DR: LoongFlow is a self-evolving agent framework that integrates LLMs into evolutionary search using a cognitive "Plan-Execute-Summarize" paradigm, achieving state-of-the-art solution quality with 60% better efficiency than existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional evolutionary approaches for transitioning static LLMs to self-improving agents lack structured reasoning, suffer from premature convergence, and struggle with inefficient exploration in high-dimensional code spaces.

Method: Integrates LLMs into a cognitive "Plan-Execute-Summarize" paradigm, uses hybrid evolutionary memory system combining Multi-Island models with MAP-Elites and adaptive Boltzmann selection, and implements two agent types: General Agent for algorithmic discovery and ML Agent for pipeline optimization.

Result: Outperforms leading baselines (OpenEvolve, ShinkaEvolve) by up to 60% in evolutionary efficiency on AlphaEvolve benchmark and Kaggle competitions, discovers superior solutions with reduced computational overhead.

Conclusion: LoongFlow represents a substantial advancement in autonomous scientific discovery, enabling generation of expert-level solutions with significantly reduced computational costs through structured reasoning in evolutionary search.
...
### [239] [CogRec: A Cognitive Recommender Agent Fusing Large Language Models and Soar for Explainable Recommendation](https://arxiv.org/abs/2512.24113)
*Jiaxin Hu,Tao Wang,Bingsan Yang,Hongrun Wang*

Main category: cs.AI

TL;DR: CogRec is a cognitive recommender agent that combines LLMs' knowledge with Soar's symbolic reasoning to create an interpretable, online-learning recommendation system that addresses LLM limitations like black-box nature and hallucination.


<details>
  <summary>Details</summary>
Motivation: LLMs have shown promise for recommendation systems but suffer from black-box characteristics, knowledge hallucination, and limited online learning capacity, compromising trustworthiness. Meanwhile, cognitive architectures like Soar offer structured reasoning but require laborious knowledge acquisition. The paper aims to combine their strengths.

Method: CogRec integrates LLMs with the Soar cognitive architecture: uses Soar as symbolic reasoning engine, LLM for knowledge initialization to populate working memory with production rules. Operates on Perception-Cognition-Action cycle, dynamically queries LLM when encountering impasses, transforms LLM solutions into symbolic production rules via Soar's chunking mechanism for online learning.
...
Conclusion: The proposed CogRec agent successfully synergizes LLMs and cognitive architectures, creating an interpretable, continuously evolving recommendation system that overcomes limitations of both approaches while providing robust online learning capabilities.
...
### [240] [Graph-Based Exploration for ARC-AGI-3 Interactive Reasoning Tasks](https://arxiv.org/abs/2512.24156)
*Evgenii Rudakov,Jonathan Shock,Benjamin Ultan Cowley*

Main category: cs.AI

TL;DR: Training-free graph-based approach for ARC-AGI-3 interactive reasoning tasks that combines vision processing with systematic state-space exploration using graph representations, outperforming state-of-the-art LLMs.


<details>
  <summary>Details</summary>
Motivation: ARC-AGI-3 benchmark reveals that current state-of-the-art LLMs cannot reliably solve interactive reasoning tasks requiring hypothesis formation, testing, and mechanic tracking. There's a need for approaches that can handle sparse-feedback environments where LLMs fail to capture task dynamics.

Method: Combines vision-based frame processing with systematic state-space exploration using graph-structured representations. Segments visual frames into meaningful components, prioritizes actions based on visual salience, maintains a directed graph of explored states and transitions, and tracks visited states and tested actions to prioritize actions providing shortest path to untested state-action pairs.

Result: Solves median of 30 out of 52 levels across six games on ARC-AGI-3 Preview Challenge, ranking 3rd on private leaderboard. Substantially outperforms frontier LLM-based agents, demonstrating that explicit graph-structured exploration without learning can serve as strong baseline for interactive reasoning.

Conclusion: Explicit graph-structured exploration, even without learning, provides effective approach for interactive reasoning tasks. Highlights importance of systematic state tracking and action prioritization in sparse-feedback environments where current LLMs fail. The method serves as strong baseline for future research in this domain.
...
TL;DR: SCP (Science Context Protocol) is an open-source standard enabling autonomous scientific agents to discover, call, and compose scientific resources (tools, models, datasets, instruments) across platforms and institutions through protocol-level standardization and orchestrated experiment lifecycle management.


<details>
  <summary>Details</summary>
Motivation: To accelerate scientific discovery by enabling global collaboration between autonomous AI agents and human researchers, reducing integration overhead, enhancing reproducibility, and bridging computational and physical laboratories across institutional boundaries.

Method: SCP has two pillars: (1) Unified Resource Integration - a universal specification for describing and invoking scientific resources, and (2) Orchestrated Experiment Lifecycle Management - a secure service architecture with centralized SCP Hub and federated SCP Servers that manages registration, planning, execution, monitoring, and archival of experiments with fine-grained authentication.
...
Conclusion: SCP establishes essential infrastructure for scalable, multi-institution, agent-driven science by standardizing scientific context and tool orchestration at the protocol level, enabling autonomous scientific agents to accelerate discovery through global resource integration.
...
### [242] [Deep Reinforcement Learning for Solving the Fleet Size and Mix Vehicle Routing Problem](https://arxiv.org/abs/2512.24251)
*Pengfu Wan,Jiawei Chen,Gangyan Xu*

Main category: cs.AI

TL;DR: A deep reinforcement learning approach for solving Fleet Size and Mix Vehicle Routing Problem that integrates fleet composition and routing decisions using a novel policy network called FRIPN.


<details>
  <summary>Details</summary>
Motivation: FSMVRP is complex due to simultaneous fleet composition and routing decisions, especially challenging in large-scale, time-constrained real-world scenarios like vehicle rental and on-demand logistics.

Method: Formulates FSMVRP as Markov Decision Process and develops FRIPN policy network with specialized input embeddings including remaining graph embedding for vehicle employment decisions.
...
Conclusion: Approach demonstrates practical potential for real-world applications and provides inspiration for extending DRL techniques to other VRP variants.
...
### [243] [Constrained Language Model Policy Optimization via Risk-aware Stepwise Alignment](https://arxiv.org/abs/2512.24263)
*Lijun Zhang,Lin Li,Wei Wei,Yajie Qi,Huizhong Song,Jun Wang,Yaodong Yang,Jiye Liang*

Main category: cs.AI

TL;DR: RSA is a risk-aware alignment method that uses nested risk measures to control safety risks during LLM fine-tuning, addressing rare but catastrophic harmful behaviors that risk-neutral methods miss.


<details>
  <summary>Details</summary>
Motivation: Existing safety alignment methods (like Safe RLHF and SACPO) are risk-neutral and insufficient for addressing risks from policy deviations and rare catastrophic harmful behaviors. There's a need for explicit risk awareness in alignment to ensure safety and trustworthiness.

Method: Risk-aware Stepwise Alignment (RSA) formulates safety alignment as token-level risk-aware constrained policy optimization using nested risk measures. It employs a stepwise alignment procedure with token-level policy updates derived from these risk measures.

Result: RSA achieves high helpfulness while ensuring strong safety and significantly suppresses tail risks (low-probability, high-impact unsafe responses). Theoretical analysis shows policy optimality under mild assumptions.

Conclusion: RSA provides an effective risk-aware alignment approach that mitigates risks from excessive model shift and suppresses rare catastrophic harmful behaviors, offering improved safety control compared to risk-neutral methods.
...
### [244] [Align While Search: Belief-Guided Exploratory Inference for World-Grounded Embodied Agents](https://arxiv.org/abs/2512.24461)
*Seohui Bae,Jeonghye Kim,Youngchul Sung,Woohyung Lim*

Main category: cs.AI

TL;DR: Test-time adaptive agent that refines beliefs through posterior-guided exploration without gradient updates, using LLM-based information gain estimation and world alignment rewards.


<details>
  <summary>Details</summary>
Motivation: LLM agents operating under partial observability need better ways to align with latent world states without requiring gradient-based updates or additional training.

Method: Agent maintains external structured belief over environment state, updates it via action-conditioned observations, selects actions by maximizing predicted information gain using lightweight LLM-based surrogate, and assesses world alignment through consistency reward between posterior belief and ground-truth.
...
Conclusion: Posterior-guided belief refinement with information gain maximization enables effective test-time adaptation for LLM agents under partial observability without gradient updates.
...
### [245] [What Drives Success in Physical Planning with Joint-Embedding Predictive World Models?](https://arxiv.org/abs/2512.24497)
*Basile Terver,Tsung-Yen Yang,Jean Ponce,Adrien Bardes,Yann LeCun*

Main category: cs.AI

TL;DR: The paper investigates JEPA-based world models for efficient planning in representation space, analyzing key components to optimize performance in physical tasks.


<details>
  <summary>Details</summary>
Motivation: To develop AI agents capable of solving diverse physical tasks and generalizing to unseen environments by improving planning efficiency through representation-space optimization rather than input-space planning.

Method: Proposes comprehensive study of JEPA-WMs (Joint Embedding Predictive Architecture World Models), analyzing model architecture, training objectives, and planning algorithms. Experiments conducted in simulated environments and with real-world robotic data.

Result: The proposed model outperforms established baselines DINO-WM and V-JEPA-2-AC in both navigation and manipulation tasks.

Conclusion: JEPA-based world models with optimized components enable more efficient planning in representation space, advancing capabilities for physical task solving and generalization.
...
### [246] [Thinking on Maps: How Foundation Model Agents Explore, Remember, and Reason Map Environments](https://arxiv.org/abs/2512.24504)
*Zhiwei Wei,Yuxing Liu,Hua Liao,Wenjia Xu*

Main category: cs.AI

TL;DR: Interactive evaluation framework for foundation model agents' spatial understanding in symbolic map environments, analyzing exploration, memory, and reasoning components.
...
### [247] [Evaluating the Reasoning Abilities of LLMs on Underrepresented Mathematics Competition Problems](https://arxiv.org/abs/2512.24505)
*Samuel Golladay,Majid Bani-Yaghoub*

Main category: cs.AI

TL;DR: LLMs struggle with underrepresented math competition problems, especially Geometry, with DeepSeek-V3 performing best overall but showing distinct error patterns across models.


<details>
  <summary>Details</summary>
Motivation: Most LLM mathematical reasoning studies use the same benchmark datasets, limiting generalizability and failing to capture diverse mathematical challenges. This study aims to analyze LLM performance on underrepresented mathematics competition problems to better understand their limitations.

Method: Tested three leading LLMs (GPT-4o-mini, Gemini-2.0-Flash, DeepSeek-V3) on Missouri Collegiate Mathematics Competition problems across Calculus, Analytic Geometry, and Discrete Mathematics. Compared LLM responses to known correct solutions, analyzed reasoning patterns, and identified error types across problem domains.

Result: DeepSeek-V3 performed best across all three categories in both reasoning and final answers. All LLMs showed notably weak performance in Geometry. DeepSeek-V3 errors were mainly computational/logical, GPT-4o-mini had logical/approach errors, while Gemini struggled with incomplete reasoning and rushed conclusions.

Conclusion: Evaluating LLMs on underrepresented mathematics competition datasets reveals distinct error patterns and highlights ongoing challenges in structured reasoning, particularly in Geometry, providing deeper insights beyond standard benchmarks.
...
### [248] [From Building Blocks to Planning: Multi-Step Spatial Reasoning in LLMs with Reinforcement Learning](https://arxiv.org/abs/2512.24532)
*Amir Tahmasbi,Sadegh Majidi,Kazem Taram,Aniket Bera*

Main category: cs.AI

TL;DR: Two-stage approach for spatial reasoning in LLMs: first fine-tune on basic spatial transformations, then train lightweight adapters for multi-step planning in puzzle environments.


<details>
  <summary>Details</summary>
Motivation: LLMs have strong general language capabilities but struggle with spatial transformations and multi-step planning in structured environments, which is important for navigation and planning applications.
...
Result: Method consistently outperforms baselines (generic backbone, physics-aware model, end-to-end RL models) in both Dynamic and Static environments. Converges faster and exhibits more stable training than end-to-end RL from scratch.

Conclusion: The proposed two-stage decomposition approach effectively improves spatial reasoning in LLMs, with attention pattern analysis showing meaningful improvements in spatial understanding.
...
### [249] [MCPAgentBench: A Real-world Task Benchmark for Evaluating LLM Agent MCP Tool Use](https://arxiv.org/abs/2512.24565)
*Wenrui Liu,Zixiang Liu,Elsie Dai,Wenhan Yu,Lei Yu,Tong Yang*

Main category: cs.AI

TL;DR: MCPAgentBench is a new benchmark for evaluating LLM agents' tool-use capabilities using real-world MCP definitions, featuring authentic tasks, simulated tools, and dynamic sandbox testing with distractor tools.


<details>
  <summary>Details</summary>
Motivation: Current MCP evaluation sets have limitations: they rely on external MCP services and lack difficulty awareness. As LLMs increasingly serve as autonomous agents using tools via MCP, there's a need for better benchmarks to evaluate their tool-use capabilities.

Method: Created MCPAgentBench with dataset of authentic tasks and simulated MCP tools. Uses dynamic sandbox environment that presents agents with candidate tool lists containing distractors to test tool selection and discrimination. Introduces comprehensive metrics for both task completion rates and execution efficiency.
...
Conclusion: MCPAgentBench addresses limitations of current MCP evaluation methods and provides a robust benchmark for assessing LLM agents' tool-use capabilities in realistic scenarios with complex tool interactions.
...
### [250] [Recursive Language Models](https://arxiv.org/abs/2512.24601)
...
Method: Recursive Language Models (RLMs) - an inference strategy where LLMs treat long prompts as external environments, programmatically examine/decompose them, and recursively call themselves over prompt snippets.
...
Conclusion: RLMs provide an effective inference-time scaling approach for processing arbitrarily long prompts, offering superior quality and practical cost efficiency compared to existing methods.
...
### [251] [Reinforcement Learning-Augmented LLM Agents for Collaborative Decision Making and Performance Optimization](https://arxiv.org/abs/2512.24609)
*Dong Qiu,Duo Xu,Limengxi Yue*

Main category: cs.AI

TL;DR: A reinforcement learning framework that enhances LLM collaboration by treating cooperation as a Dec-POMDP problem with centralized training and decentralized execution, achieving significant speed and quality improvements in writing and coding tasks.


<details>
  <summary>Details</summary>
Motivation: LLMs perform well individually but lack collaborative awareness and struggle to optimize global performance in multi-agent settings, limiting their effectiveness in complex workflows requiring coordination.

Method: Formulates cooperation as a decentralized partially observable Markov decision process (Dec-POMDP) with centralized training and decentralized execution (CTDE). Introduces Group Relative Policy Optimization (GRPO) to jointly optimize agent policies using global signals during training, with a simplified joint reward balancing task quality, speed, and coordination cost.

Result: Achieves 3x increase in task processing speed over single-agent baselines, 98.7% structural/style consistency in collaborative writing, and 74.6% test pass rate in coding. Consistently outperforms strong multi-agent LLM baselines.

Conclusion: The framework provides a practical path toward reliable collaboration in complex workflows by effectively addressing LLM coordination challenges through reinforcement learning and structured multi-agent optimization.
...
### [252] [Group Deliberation Oriented Multi-Agent Conversational Model for Complex Reasoning](https://arxiv.org/abs/2512.24613)
*Zheyu Shi,Dong Qiu,Shanlong Yu*

Main category: cs.AI

TL;DR: A multi-agent conversational model with three-level role division (generation, verification, integration) improves complex reasoning through self-game mechanisms, retrieval enhancement, and composite rewards, achieving significant accuracy gains on multi-hop QA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Single large language models have limitations in complex reasoning tasks, requiring more sophisticated approaches to handle multi-hop reasoning, factual verification, and logical coherence.

Method: Three-level role architecture: opinion generation agent produces diverse perspectives, evidence verification agent retrieves external knowledge and quantifies factual support, consistency arbitration agent integrates coherent conclusions. Includes self-game mechanism for multi-path reasoning, retrieval enhancement module, composite reward function (factual consistency + logical coherence), and improved proximal policy optimization for collaborative training.

Result: Improves multi-hop reasoning accuracy by 16.8% on HotpotQA, 14.3% on 2WikiMultihopQA, and 19.2% on MeetingBank. Improves consistency by 21.5%. Achieves higher reasoning efficiency than mainstream multi-agent approaches.
...
### [253] [Youtu-Agent: Scaling Agent Productivity with Automated Generation and Hybrid Policy Optimization](https://arxiv.org/abs/2512.24615)
...
TL;DR: Youtu-Agent is a modular LLM agent framework that automates agent generation and enables continuous evolution through workflow/meta-agent modes and hybrid policy optimization (practice + RL).


<details>
  <summary>Details</summary>
Motivation: Existing LLM agent frameworks require high manual configuration costs and have static capabilities that can't adapt to dynamic environments without expensive fine-tuning.

Method: Features structured configuration system decoupling execution environments, toolkits, and context management. Offers two generation paradigms: Workflow mode for standard tasks and Meta-Agent mode for complex requirements. Includes hybrid policy optimization with Agent Practice (in-context optimization) and Agent RL (distributed reinforcement learning).

Result: Achieves SOTA on WebWalkerQA (71.47%) and GAIA (72.8%) with open-weight models. Automated generation achieves 81% tool synthesis success rate. Practice module improves AIME 2024/2025 by +2.7%/+5.4%. Agent RL achieves 40% speedup with 35%/21% improvements on coding/reasoning and searching tasks.

Conclusion: Youtu-Agent addresses configuration costs and static capability limitations through automated generation and continuous evolution, enabling flexible, high-performance LLM agents that adapt without expensive fine-tuning.
...
### [254] [Multi-modal cross-domain mixed fusion model with dual disentanglement for fault diagnosis under unseen working conditions](https://arxiv.org/abs/2512.24679)
*Pengcheng Xia,Yixiang Huang,Chengjin Qin,Chengliang Liu*

Main category: cs.AI

TL;DR: Multi-modal cross-domain mixed fusion model with dual disentanglement for fault diagnosis under unseen working conditions, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing fault diagnosis methods suffer performance decline under unseen working conditions, domain adaptation requires target domain samples, and single-modal approaches overlook complementary multi-modal information.

Method: Dual disentanglement framework to separate modality-invariant/specific features and domain-invariant/specific representations; cross-domain mixed fusion strategy for modality/domain diversity; triple-modal fusion mechanism for adaptive multi-modal integration.

Result: Extensive experiments on induction motor fault diagnosis show consistent outperformance over advanced methods under both unseen constant and time-varying working conditions; ablation studies verify component effectiveness.

Conclusion: Proposed multi-modal cross-domain mixed fusion model with dual disentanglement effectively addresses generalization challenges in fault diagnosis, leveraging complementary multi-modal information for robust performance under unseen conditions.
...
### [255] [BatteryAgent: Synergizing Physics-Informed Interpretation with LLM Reasoning for Intelligent Battery Fault Diagnosis](https://arxiv.org/abs/2512.24686)
*Songqi Zhou,Ruixue Liu,Boman Su,Jiazhou Wang,Yixing Wang,Benben Jiang*

Main category: cs.AI

TL;DR: BatteryAgent: A hierarchical LLM-based framework for interpretable battery fault diagnosis that combines physical features with AI reasoning to provide root cause analysis and maintenance recommendations.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for battery fault diagnosis have two key limitations: they lack interpretability (black-box nature) and are restricted to binary classification, preventing root cause analysis and maintenance recommendations.

Method: Proposes BatteryAgent with three core modules: 1) Physical Perception Layer using 10 mechanism-based electrochemical features, 2) Detection and Attribution Layer using Gradient Boosting Decision Trees and SHAP for feature contribution analysis, and 3) Reasoning and Diagnosis Layer using LLMs as agent core to create "numerical-semantic" bridge combining SHAP attributions with mechanism knowledge base.

Result: Achieves AUROC of 0.986, effectively correcting misclassifications on hard boundary samples and significantly outperforming state-of-the-art methods. Extends binary detection to multi-type interpretable diagnosis.

Conclusion: BatteryAgent represents a paradigm shift from "passive detection" to "intelligent diagnosis" for battery safety management, offering comprehensive fault reports with root cause analysis and maintenance suggestions.
...
### [256] [Explaining Why Things Go Where They Go: Interpretable Constructs of Human Organizational Preferences](https://arxiv.org/abs/2512.24829)
*Emmanuel Fashae,Michael Burke,Leimin Tian,Lingheng Meng,Pamela Carreno-Medrano*

Main category: cs.AI

TL;DR: The paper introduces four interpretable constructs for object arrangement preferences and validates them through a questionnaire study, then demonstrates their utility in robotic planning.


<details>
  <summary>Details</summary>
Motivation: Current robotic systems for household object rearrangement rely on latent preference models that lack interpretability, making it difficult to understand the factors guiding human decisions.

Method: 1) Designed and validated a self-report questionnaire through a 63-participant online study to capture four interpretable constructs. 2) Integrated these constructs into a Monte Carlo Tree Search (MCTS) planner for robotic arrangement generation.

Result: The study confirmed the psychological distinctiveness of the four constructs and their explanatory power across kitchen and living room scenarios. The MCTS planner guided by participant-derived preferences generated reasonable arrangements that closely aligned with human-generated arrangements.

Conclusion: The work contributes a compact, interpretable formulation of object arrangement preferences and demonstrates how it can be operationalized for robot planning, providing both theoretical understanding and practical application.
...
### [257] [GenZ: Foundational models as latent variable generators within traditional statistical models](https://arxiv.org/abs/2512.24834)
*Marko Jojic,Nebojsa Jojic*

Main category: cs.AI

TL;DR: GenZ is a hybrid model that combines LLMs with statistical modeling by discovering interpretable semantic features through an iterative error analysis process, outperforming pure LLM baselines on prediction tasks.


<details>
  <summary>Details</summary>
Motivation: Large language models have broad domain knowledge but often fail to capture dataset-specific patterns crucial for prediction tasks. There's a need to bridge foundational models with statistical modeling to leverage both general knowledge and specific data patterns.

Method: Uses an iterative process that contrasts groups of items identified via statistical modeling errors to discover semantic feature descriptions. Formulated as a generalized EM algorithm that jointly optimizes semantic feature descriptors and statistical model parameters. Prompts a frozen foundational model to classify items based on discovered features, treating these judgments as noisy observations of latent binary features that predict real-valued targets through learned statistical relationships.
...
Conclusion: The approach successfully bridges foundational models and statistical modeling, discovering dataset-specific patterns that diverge from the model's domain knowledge alone, and demonstrates substantial improvements over pure LLM baselines on real-world prediction tasks.
...
### [258] [A study on constraint extraction and exception exclusion in care worker scheduling](https://arxiv.org/abs/2512.24853)
*Koki Suenaga,Tomohiro Furuta,Satoshi Ono*

Main category: cs.AI

TL;DR: A method using constraint templates to extract facility-specific scheduling constraints for long-term care facilities, with mechanisms to exclude exceptional constraints, enabling automated schedule generation.


<details>
  <summary>Details</summary>
Motivation: Long-term care facilities have varying conditions between facilities, requiring facility-specific constraint extraction through manager interviews rather than generic scheduling solutions.

Method: Uses constraint templates to extract combinations of scheduling components (shift patterns, staff combinations) by adjusting parameters like number of days and staff members. Includes mechanisms to exclude exceptional constraints, unlike existing techniques.

Result: Successfully created schedules satisfying all hard constraints and reduced soft constraint violations by avoiding extraction of exceptional constraints.

Conclusion: The proposed constraint template approach with exceptional constraint exclusion effectively enables facility-specific automated scheduling for long-term care facilities.
...
### [259] [Let It Flow: Agentic Crafting on Rock and Roll, Building the ROME Model within an Open Agentic Learning Ecosystem](https://arxiv.org/abs/2512.24873)
...
TL;DR: ALE is an end-to-end ecosystem for developing agentic LLMs, featuring ROLL for weight optimization, ROCK for environment management, and iFlow CLI for context engineering. They release ROME, an open-source agent trained on 1M+ trajectories using novel data composition and IPA algorithm.
...
Method: ALE infrastructure with three components: ROLL (post-training weight optimization), ROCK (sandbox environment manager for trajectory generation), and iFlow CLI (agent framework for context engineering). Includes data composition protocols for complex behaviors and Interaction-based Policy Alignment (IPA) algorithm that assigns credit over semantic interaction chunks rather than individual tokens.

Result: Released ROME (ROME is Obviously an Agentic Model), an open-source agent trained on over one million trajectories. Introduced Terminal Bench Pro benchmark with improved scale and contamination control. ROME demonstrates strong performance across benchmarks like SWE-bench Verified and Terminal Bench.

Conclusion: ALE provides an effective foundational infrastructure for agent LLM development, with ROME proving the ecosystem's effectiveness through strong benchmark performance, addressing the gap in open-source agent development tools.
...
### [260] [Semi-Automated Data Annotation in Multisensor Datasets for Autonomous Vehicle Testing](https://arxiv.org/abs/2512.24896)
*Andrii Gamalii,Daniel Górniak,Robert Nowak,Bartłomiej Olber,Krystian Radlak,Jakub Winter*

Main category: cs.AI

TL;DR: A semi-automated data annotation pipeline for creating large-scale multimodal driving datasets in Polish conditions, using human-in-the-loop AI to reduce annotation costs and time.


<details>
  <summary>Details</summary>
Motivation: Manual annotation of heterogeneous driving scenario data is costly and time-consuming, creating a bottleneck for autonomous vehicle research in Poland.

Method: Human-in-the-loop approach combining AI with human expertise, using 3D object detection algorithms for initial annotations, iterative model retraining, data anonymization, and domain adaptation techniques.

Result: Substantial time savings while ensuring consistent, high-quality annotations across different sensor modalities, accelerating dataset preparation in standardized format.

Conclusion: The developed tools and methodology effectively support the DARTS project by strengthening Poland's technological base for autonomous vehicle research through efficient large-scale dataset creation.
...
### [261] [Iterative Deployment Improves Planning Skills in LLMs](https://arxiv.org/abs/2512.24940)
...
<details>
  <summary>Details</summary>
Motivation: To understand how iterative deployment of LLMs with user-curated fine-tuning data affects model properties and capabilities, particularly examining whether this process leads to emergent behaviors and improved performance over time.

Method: Testing iterative deployment mechanism across various planning domains, where each new model is fine-tuned on data carefully curated by users from previous models' deployments, then analyzing the resulting model properties and capabilities.

Result: Substantial improvements in planning skills observed, with later models displaying emergent generalization by discovering much longer plans than initial models. Theoretical analysis reveals iterative deployment effectively implements reinforcement learning with implicit reward function.

Conclusion: Iterative deployment creates an implicit RL training loop with important implications: 1) AI safety concerns due to undefined reward functions, and 2) alternative training regime to explicit RL that relies on data curation rather than explicit rewards.
...
### [262] [AMAP Agentic Planning Technical Report](https://arxiv.org/abs/2512.24957)
...
TL;DR: STAgent is a specialized LLM agent for spatio-temporal tasks like POI discovery and itinerary planning, featuring 10+ tools, hierarchical data curation, and cascaded training while preserving general capabilities.


<details>
  <summary>Details</summary>
Motivation: The paper aims to develop a specialized agentic LLM for complex spatio-temporal reasoning tasks that require exploration, verification, and refinement capabilities, while maintaining general language understanding abilities.

Method: Three key contributions: (1) stable tool environment with 10+ domain-specific tools for asynchronous rollout/training; (2) hierarchical data curation framework filtering high-quality queries at 1:10,000 ratio; (3) cascaded training recipe with seed SFT (guardian), second SFT on high-certainty queries, and RL on low-certainty data.

Result: STAgent shows promising performance on TravelBench while maintaining general capabilities across wide benchmarks, demonstrating effectiveness of the proposed agentic model approach.

Conclusion: STAgent successfully addresses complex spatio-temporal tasks through specialized tool interaction and training methodology while preserving general LLM capabilities, offering a viable approach for domain-specific agent development.
...
### [263] [Context-aware LLM-based AI Agents for Human-centered Energy Management Systems in Smart Buildings](https://arxiv.org/abs/2512.25055)
*Tianzhi He,Farrokh Jazizadeh*

Main category: cs.AI

TL;DR: LLM-based BEMS AI agents framework for smart building energy management using natural language interaction, achieving good accuracy in device control (86%) and energy analysis (77%), but lower accuracy (49%) in complex cost estimation tasks.


<details>
  <summary>Details</summary>
Motivation: To address limitations in existing building energy management systems by leveraging LLMs' autonomous data analytics capabilities for context-aware energy management through natural language interaction in smart buildings.

Method: Proposed a three-module framework (perception, central control, action) forming a closed feedback loop. Evaluated prototype using 120 user queries across four real-world residential energy datasets with metrics including latency, functionality, capability, accuracy, and cost-effectiveness. Used ANOVA tests to demonstrate generalizability.

Result: Promising performance: device control (86% accuracy), memory-related tasks (97%), scheduling/automation (74%), energy analysis (77%). Complex cost estimation tasks showed lower accuracy (49%). Demonstrated trade-off between response accuracy and computational efficiency.

Conclusion: The study formalizes assessment of LLM-based BEMS AI agents and identifies future research directions, highlighting the framework's potential for context-aware energy management while acknowledging areas for improvement in complex tasks like cost estimation.
