...
TL;DR: A geometry-informed computer vision pipeline automates overtaking event detection from a single bicycle-mounted camera, achieving 97.8% recall with zero false positives on real-world data, and enables calibration-free lateral distance estimation with 13-14 cm MAE.


<details>
  <summary>Details</summary>
Motivation: Manual frame-by-frame annotation of overtaking events from rear-facing bicycle video is a bottleneck that limits sample sizes and constrains naturalistic cycling safety research.

Method: RT-DETR object detection + ByteTrack multi-object tracking + three-stage geometric validation (bearing angle trend, apparent size growth, spatial confirmation) from perspective projection principles; calibration-free lateral distance estimation using bounding box geometric features.

Result: 97.8% recall with zero false positives on 315 manually annotated real-world events; mean 2.44s advance detection; 33.3% of passes below 5-ft threshold; 13-14 cm MAE for lateral distance estimation.

Conclusion: The system removes the primary annotation bottleneck, provides a scalable foundation for vehicle-bicycle interaction analysis, and demonstrates feasibility for active cyclist warning systems.
...
TL;DR: Sol Video Inference Engine is an agentic, training-free acceleration framework that automatically optimizes video diffusion models for specific model-hardware-configuration instances, achieving >2x speedup with near-lossless quality across 64B, 22B, and 2B models.


<details>
  <summary>Details</summary>
Motivation: The most effective acceleration strategies for video diffusion models are highly instance-specific (dependent on model architecture, hardware platform, and inference settings), creating a large tuning space that makes manual performance engineering costly and non-transferable.

Method: An agentic acceleration stack with five techniques (cache, sparse attention, token pruning, quantization, kernel fusion). Parallel skill agents optimize each technique for a specific deployment target, an agent integrator composes them into a global stack, and a human validator provides quality feedback.

Result: Applied to three models of varying sizes (64B Cosmos3-Super, 22B LTX-2.3, 2B SANA-Video), the framework achieves over 2x end-to-end acceleration while maintaining near-lossless VBench quality with minimal human effort.

Conclusion: The agentic, training-free framework effectively addresses instance-specific acceleration for video diffusion models, demonstrating generalizability across model sizes and architectures.
...
TL;DR: PV-TAM evaluates vision-language consistency in large VLMs using prompt-side semantics and attention peak distribution, avoiding distortions from decoding drift and structural tokens, and outperforms answer-side baselines.


<details>
  <summary>Details</summary>
Motivation: Traditional vision-language consistency assessment relies on answer-side attention distributions, which suffer from decoding drift (accumulated language priors mismatching visual attention) and distortion from structural tokens (e.g., modality boundary markers) that generate high attention to irrelevant areas.

Method: Proposes Prompt-Vision Token Activation Map (PV-TAM), which uses prompt-side semantics instead of answer-side tokens, includes a filter to remove systematic bias from modality boundary markers, and leverages peak distribution of attention rather than mask-only overlap to measure alignment between prompts and visual regions.

Result: PV-TAM consistently improves both attention-based and IoU-style localization metrics over answer-side baselines across various datasets.

Conclusion: Shifting from answer-side to prompt-side evaluation with attention peak distribution and structural token filtering provides more reliable vision-language consistency assessment for large VLMs.
...
TL;DR: The paper shifts small object detection from spatial to spectral domain, proposing a Frequency-Guided Feature Representation framework and lightweight DER operator (WDG, LGE, FDHead) that achieves superior performance with 1/6 the parameters of YOLOv11.


<details>
  <summary>Details</summary>
Motivation: Small object detection suffers from feature scarcity due to spatial-domain operations discarding high-frequency details, and recovering these cues in spatial domain is computationally expensive and amplifies noise.

Method: Proposes a frequency-guided paradigm with a Decompose–Enhance–Reconstruct (DER) operator using three lightweight modules: Wavelet-Difference Gate (WDG), Log-Gabor Enhancer (LGE), and Frequency-Driven Head (FDHead) to inject frequency-aware modulation into backbone, neck, and head of detectors.

Result: DERNet outperforms YOLOv11 models under the same scale using only 1/6 of the parameters, with consistent gains across VisDrone2019, UAVDT, TinyPerson, and DOTAv1 benchmarks.

Conclusion: Spectral-domain feature processing offers a more efficient and effective alternative to spatial-domain methods for small object detection, decoupling feature modeling from resolution reduction.
...
TL;DR: ABACUS is a unified vision-language model for counting and count-faithful image generation that achieves SOTA across seven benchmarks using density-aware zooming, boundary-aware GRPO, and cycle-consistent GRPO, without benchmark-specific training.


<details>
  <summary>Details</summary>
Motivation: To create a single model that handles multiple counting tasks (object counting, crowd counting, referring-expression counting, and count-faithful image generation) without needing benchmark-specific training, and to close the understanding-generation gap without external annotations.

Method: Built on a 3B-parameter unified foundation model with three innovations: (1) density-aware adaptive zooming with objectness maps for spatial grounding; (2) a boundary-aware count policy via GRPO to eliminate crop-boundary errors; (3) a cycle-consistent GRPO strategy where the understanding branch self-critiques generated outputs.
...
Conclusion: ABACUS demonstrates that a unified approach with innovations in adaptive zooming, boundary-aware policy, and cycle-consistent self-critique can excel across diverse counting tasks without specialized training, bridging the understanding-generation gap.
...
TL;DR: HANCLIP restructures VLM embedding spaces using hyperbolic and angular objectives to improve negation sensitivity, trained on only 20K quadruplets, and pluggable into existing CLIP variants without degrading standard benchmarks.


<details>
  <summary>Details</summary>
Motivation: VLMs are brittle to negation despite strong overall performance, and direct finetuning on negation data causes degradation on standard benchmarks.

Method: HANCLIP combines hyperbolic formulation (hierarchical semantic relations & asymmetries) with an angular triplet objective, trained on 20K image-text quadruplets to separate negated descriptions from positives.

Result: Consistent gains on NegBench, competitive/improved performance on standard classification and retrieval benchmarks.

Conclusion: A carefully designed geometric objective can substantially extend VLM reasoning capabilities using modest additional data, and the framework is model-agnostic across CLIP variants.
...
TL;DR: HeRA improves MLLMs by aligning individual attention heads' representations with an external vision encoder using MKNN-based contrastive learning, focusing on the least aligned heads.


<details>
  <summary>Details</summary>
Motivation: Existing representation alignment methods align a fixed layer of the language backbone, ignoring the fine-grained structure of Transformer models (individual attention heads).

Method: HeRA enforces cross-modal alignment at the attention head level using Mutual K-Nearest Neighbor (MKNN) as a metric and a differentiable contrastive objective to match local topological structures, applied to the least aligned heads selected by alignment score.

Result: HeRA consistently improves performance on vision-centric tasks across multiple MLLMs and 18 benchmarks, and effectively reduces visual hallucinations by curbing over-reliance on linguistic priors.

Conclusion: Head-wise alignment, especially on the least aligned heads, is an effective regularizer for multimodal LLMs, improving both performance and robustness.
...
TL;DR: REALM is the first unified red-teaming benchmark for physical-world VLMs, integrating 12 attack methods, 3 defenses, and 13 models under a standardized black-box threat model. It introduces agentic target-generation for fair comparison and finds text/typographic injection attacks most effective, multimodal co-optimization best for visual transfer, and that model scale alone doesn't ensure robustness.


<details>
  <summary>Details</summary>
Motivation: Existing red-teaming evaluations for VLMs are fragmented across datasets, metrics, and threat models, making direct comparison of attack methods impossible. Current benchmarks are chatbot-centric and fail to capture physically grounded functional failures in safety-critical embodied systems.

Method: REALM integrates 12 red-teaming methods, 3 model-agnostic defenses, and 13 VLMs under a unified black-box threat model. It introduces an agentic target-generation pipeline to construct shared, scenario-specific, physically grounded attack objectives across diverse attack families, enabling fair comparison.

Result: Text and typographic injection attacks induce the most failures; multimodal co-optimization yields the strongest visual-perturbation transfer; single-pass attacks approach iterative methods at much lower cost; model scale alone does not confer adversarial robustness.

Conclusion: REALM provides a standardized benchmark for systematically comparing diverse red-teaming methods on physical-world VLMs, revealing key findings about attack efficacy and model vulnerability that can inform future safety research.
...
TL;DR: TheProfessor extends PromptKD by distilling from two teachers (domain-finetuned + zero-shot) instead of one, achieving +1.77 HM improvement on average across four datasets, with largest gains on domain-shifted EuroSAT (+5.78).


<details>
  <summary>Details</summary>
Motivation: Single-teacher prompt distillation (PromptKD) may lack complementary supervision. The authors propose using a second, zero-shot teacher to provide additional knowledge that improves student generalization, especially under domain shift.

Method: Multi-teacher distillation with a fixed two-teacher ensemble: (1) a domain-finetuned PromptSRC ViT-L/14 teacher, and (2) a zero-shot EVA-CLIP-L/14 teacher with pre-computed logits. Two ensembling strategies tested: equal-probability averaging and confidence-weighted averaging.

Result: Confidence-weighted ensembling improves average HM from 87.52 to 89.28 (+1.77). Gains vary: negligible on Caltech-101 (+0.16), modest on UCF101 (+0.62), largest on EuroSAT (+5.78). Equal averaging yields +1.37 HM average.

Conclusion: Multi-teacher prompt distillation is most beneficial when the second teacher provides complementary supervision under domain shift; gains are dataset-dependent.
...
### [10] [Flood Mapping from RGB imagery using a Vision Foundation Model](https://arxiv.org/abs/2606.24120)
*Vladyslav Polushko,Tilman Bucher,Ronald Rösch,Thomas März,Markus Rauhut,Andreas Weinmann*

Main category: cs.CV

TL;DR: This paper investigates adapting a satellite-pretrained Earth observation foundation model (Prithvi-EO-2.0-600M Vision Transformer with UPerNet decoder) for centimeter-scale floodwater mapping from RGB aerial imagery. The model achieves state-of-the-art results on two flood datasets, shows better zero-shot transfer to new flood events than baselines, and demonstrates efficient fine-tuning with limited data.


<details>
  <summary>Details</summary>
Motivation: Timely, high-resolution flood maps around settlements are essential for emergency response, but deep learning models for water segmentation require substantial data for adaptation to new flood events. Vision foundation models for Earth observation exist but are pretrained on satellite data, which differs significantly from nadir RGB imagery in resolution, viewing geometry, and radiometry.

Method: The authors fine-tune Prithvi-2.0-UPN (Prithvi-EO-2.0-600M Vision Transformer + UPerNet decoder) for binary water segmentation on two RGB flood datasets (BlessemFlood21, NeuenahrFlood). Three experiments are conducted: (1) full training on both datasets, (2) zero-shot transfer from BlessemFlood21 to NeuenahrFlood, (3) fine-tuning with small portions of NeuenahrFlood training data.

Result: Prithvi-2.0-UPN achieves state-of-the-art results on both datasets when fully trained. It outperforms baseline models in zero-shot transfer to a new flood event. With small shares of target data, it improves fastest and nearly matches fully-trained performance.

Conclusion: A satellite-pretrained Earth observation foundation model can be effectively adapted to centimeter-scale flood mapping from RGB imagery, showing strong transfer capabilities and data efficiency. However, zero-shot performance still has room for improvement.
...
TL;DR: The paper proposes Forensic Knowledge Graphs (FKGs) that unify forensic evidence extraction, structured reasoning, and interpretable explanations for image forgery detection, outperforming both traditional detectors and vision-language models.


<details>
  <summary>Details</summary>
Motivation: Existing forensic detectors lack interpretability, and VLMs provide explanations but cannot exploit forensic traces for reliable detection. A unified framework combining both capabilities is needed.

Method: The authors introduce Forensic Knowledge Graphs (FKGs) that encode forensic traces, causal dependencies, and scene content. They develop a forensic authentication network and an Iterative Context Refinement strategy for faithful explanation generation, along with the FKG-50K dataset.

Result: FKG outperforms both forensic detectors and VLMs in detection accuracy, forgery identification, localization, and forensic justification.

Conclusion: FKGs provide a unified, interpretable framework for trustworthy image authentication, bridging the gap between forensic detection and explainable AI.
...
### [12] [High-Fidelity Synthetic Transmission Electron Microscopy Image Generation Using Diffusion Probabilistic Models for Data-Limited Semiconductor Metrology](https://arxiv.org/abs/2606.24817)
*Johannes Boehm,Bappaditya Dey*

Main category: cs.CV

TL;DR: A DDPM framework generates realistic synthetic TEM images from only 15 real samples, using progressive patch-based training and custom augmentations, achieving >0.98 MS-SSIM and enabling downstream ML for defect detection and segmentation.


<details>
  <summary>Details</summary>
Motivation: Advanced semiconductor nodes require TEM imaging, but destructive sample preparation, slow imaging, and high costs severely limit the availability of diverse datasets needed for machine learning applications.

Method: A Denoising Diffusion Probabilistic Model (DDPM) with progressive patch-based training (low-res to full images), TrivialAugment adaptation, cross-process domain transfer, classifier guidance, and RePaint-style inpainting, plus repurposing DDPM features for segmentation.

Result: Achieves MS-SSIM > 0.98 on synthetic images, with qualitative expert assessment consistent with structural similarity results, enabling downstream ML training for defect detection, segmentation, and metrology.

Conclusion: The DDPM framework successfully generates physically realistic TEM images under extreme data scarcity (only 15 samples), preserving statistical and physical realism for downstream semiconductor metrology tasks.
...
TL;DR: DivRL is a post-training framework that resolves the Identity-Diversity Paradox in subject-driven image generation by using disentangled visual features, a Negative Self-Similarity Measure (nSSM) for diversity, Visual Semantic Matching (VSM) for identity, and an Explore-and-Suppress strategy that treats identity as a gated constraint rather than a competing objective.


<details>
  <summary>Details</summary>
Motivation: Subject-driven image generation suffers from an "Identity-Diversity Paradox"—strong identity preservation leads to rigid, low-diversity outputs. The authors aim to jointly optimize both identity consistency and structural diversity.

Method: The framework (DivRL) introduces: (1) Negative Self-Similarity Measure (nSSM) to quantify structural diversity, (2) Visual Semantic Matching (VSM) for identity consistency, and (3) an "Explore-and-Suppress" strategy where VSM acts as a gated constraint—only samples violating identity threshold are penalized via quadratic hinge loss, converting identity preservation from competing objective to feasibility constraint.

Result: The method effectively generates both identity-consistent and structurally diverse images. It improves structural diversity while maintaining comparable identity consistency through the gated optimization formulation.

Conclusion: The Explore-and-Suppress strategy successfully resolves the Identity-Diversity Paradox by reformulating identity preservation as a feasibility constraint, enabling joint improvement of nSSM and VSM.
...
TL;DR: Introduces Token-to-Token alignment, a method that rephrases prompts into a structured format and aligns token embeddings semantically, enabling smooth linear interpolation for image blending and continuous editing in text-to-image models.


<details>
  <summary>Details</summary>
Motivation: Token sequences lack a consistent structure—semantically similar images can come from very different token sequences, while similar sequences can yield different semantics. This makes smooth transitions (blending, editing) difficult.

Method: Two-stage alignment: (1) Structural alignment rephrases prompts into a shared structured form; (2) Embedding-level alignment matches token representations across prompts based on semantic similarity.

Result: After alignment, simple linear interpolation produces smooth, coherent semantic transitions, enabling effective image blending and continuous editing without modifying the generative model.

Conclusion: Text embedding spaces in text-to-image models already encode continuous semantic structure, and proper alignment makes it accessible—semantic control can be achieved by organizing existing representations rather than modifying the model.
...
TL;DR: DriveStack-VLA enhances VLA driving models with dual visual modeling (BEV + Render-Teacher Alignment) and a self-critique module for trajectory refinement, achieving state-of-the-art results on NAVSIM and Bench2Drive benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA driving models lack driving-oriented spatial intelligence, relying on perspective image tokens and language priors while ignoring metric geometry, top-down scene structure, and safety-critical perceptual cues, making them vulnerable to weak geometry modeling.

Method: (1) Dual visual modeling: BEV representation injected into LLM decoder via DeepStack-style connection, plus Render-Teacher Alignment to align perceptual focus of real and rasterized images. (2) Head-based self-critique module for multimodal trajectory selection that ranks sampled trajectories and conditionally refines the best one.

Result: 91.6 PDMS on NAVSIMv1, 91.0 EPDMS on NAVSIMv2, and driving score 79.49 with 56.36% success rate on closed-loop Bench2Drive.

Conclusion: Strengthening spatial grounding in VLA driving models through BEV injection and perceptual alignment significantly improves driving performance and safety.
...
TL;DR: EPEdit is a user-friendly photo editor that combines zero-shot Stable Diffusion-based algorithms with a simple interface, enabling efficient image manipulation without costly fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing image editing tools (Photoshop, Capture One) require expertise, while generative AI solutions like Stable Diffusion need costly retraining. The goal is to provide a cost-effective, accessible alternative for non-experts.

Method: EPEdit integrates a backend framework with zero-shot image editing algorithms based on Stable Diffusion, eliminating fine-tuning. Users interact via text commands or masks for precise region-specific edits.

Result: User evaluations for image editing, thematic design, and overall system performance show EPEdit outperforms existing solutions in usability and cost-effectiveness.

Conclusion: EPEdit offers a user-friendly, cost-effective, and efficient solution for comprehensive image editing, accessible to non-technical users.
...
TL;DR: VisChronos bridges the semantic gap between images and language by using LLMs and dense captioning models to generate event-aware captions based on real-world historical knowledge, supported by a new EventCap dataset.


<details>
  <summary>Details</summary>
Motivation: Traditional image captioning methods fail to capture contextual narratives and semantic meaning behind visual content; there is a semantic gap between what is seen and what is understood in context.

Method: VisChronos framework combines large language models (LLMs) with dense captioning models to identify and describe real-life events from a single input image, leveraging historical real-world knowledge for caption generation. A new dataset, EventCap, is constructed using this framework.

Result: The framework generates detailed, context-aware, and event-focused descriptions. User study confirms accuracy, coherence, and relevance of generated captions.

Conclusion: VisChronos effectively enhances event-centric image understanding and sets a foundation for future research in context-rich, narrative-driven captioning.
...
TL;DR: This paper studies ingredient-level semantic segmentation on FoodSeg103, fine-tuning SegFormer-B0 and SegFormer-B1 models. B1 achieved 0.7929 pixel accuracy and 0.3204 mean IoU, outperforming B0. The system converts predictions into ingredient-area percentages for visual nutrition-awareness cues.


<details>
  <summary>Details</summary>
Motivation: Food images contain multiple visible ingredients, but assigning a single dish label loses important visual structure. The authors aim to capture ingredient-level details through pixel-wise semantic segmentation.

Method: Two SegFormer variants (B0 and B1) with ImageNet-pretrained MiT backbones and newly initialized 104-class output layers were fine-tuned and evaluated on FoodSeg103 test split (2,135 images). Predicted masks are converted into ingredient-area percentages.

Result: B0: 0.7709 pixel accuracy, 0.2521 mean IoU. B1: 0.7929 pixel accuracy, 0.3204 mean IoU. B1 showed a +0.0683 absolute gain in mean IoU over B0 across all metrics.

Conclusion: The system provides a visual composition summary for nutrition-awareness cues as an alternative to detailed food tracking, but is not a direct estimate of calories, macronutrients, or portion size.
...
TL;DR: ObsGraph is a hierarchical scene graph with room-view-object layers that unifies scene representation, retrieval, and exploration for embodied agents, using coarse-to-fine retrieval under bounded budgets to structure multi-scale exploration, achieving improved success and efficiency on embodied reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Robots operating in complex, unfamiliar environments need to identify and acquire necessary information through exploration, but existing methods lack a unified framework that tightly couples scene representation, evidence retrieval, and adaptive exploration strategies.

Method: ObsGraph organizes visual evidence into a room-view-object hierarchical scene graph: rooms provide semantic anchors, views preserve object covisibility contexts, and objects store fine-grained details. It performs coarse-to-fine hierarchical retrieval under bounded budgets and uses retrieval outcomes to adaptively activate room-level exploration, view refinement, or frontier exploration.

Result: Experiments on embodied reasoning and exploration benchmarks show improved success rates and efficiency, demonstrating the benefits of structured scene representation and targeted information gathering driven by identified evidence gaps.

Conclusion: Tightly coupling scene representation, retrieval, and adaptive multi-scale exploration through a hierarchical structure like ObsGraph significantly enhances robotic performance in complex environments by enabling more targeted information gathering.
...
TL;DR: This paper introduces the first comprehensive benchmark for fabric image demoiréing, addressing the underexplored problem of aliasing artifacts caused by textile-camera interaction. The authors propose a physically motivated synthesis framework, create a large-scale dataset (16,050 paired images), and provide a baseline model with strong generalization performance.


<details>
  <summary>Details</summary>
Motivation: Fabric moiré is fundamentally different from screen moiré due to the broadband, semi-periodic nature of textiles, making demoiréing more ill-posed. Existing models trained on screen moiré datasets fail to generalize, yet fabric demoiréing lacks standardized benchmarks and datasets.

Method: A physically motivated synthesis framework is developed to generate pixel-aligned real-world-like paired images. A large-scale dataset of 16,050 multi-resolution fabric images with controllable aliasing severity is constructed, and a customized baseline model is designed for fabric demoiréing.

Result: The proposed benchmark dataset and baseline model demonstrate promising demoiréing performance with strong generalization ability on fabric images.

Conclusion: This work provides the first standardized benchmark for fabric image demoiréing, filling a critical gap in the field and offering a foundation for future research.
...
TL;DR: EMRFormer is a spiking neural network (SNN) with spike-driven transformer architecture for automatic modulation recognition (AMR). It achieves state-of-the-art accuracy, reduces theoretical energy consumption by over 90%, and delivers up to 5x power reduction on neuromorphic hardware (KA200 chip) compared to GPU (3090) or Orin NX.


<details>
  <summary>Details</summary>
Motivation: Deep learning AMR methods suffer from high computational costs and power consumption, making them unsuitable for resource-constrained platforms. The authors aim to balance accuracy and energy efficiency for AMR on such devices.

Method: Proposes EMRFormer, an end-to-end SNN with: (1) adaptive spike encoder and Integer Leaky Integrate-and-Fire (LIF) neurons to preserve information and boost representational capacity; (2) spike-separable CNNs (SSCNN) integrated into Spike-Driven Transformers (SpikeFormer) to extract multi-scale temporal features from raw IQ waveforms.

Result: Achieves SOTA accuracy across mainstream datasets, maintains strong performance in low-SNR environments, reduces theoretical energy consumption by over 90%, and demonstrates up to 5x power reduction on KA200 neuromorphic chip vs. 3090 GPU or Orin NX.

Conclusion: EMRFormer offers a promising pathway for deploying AMR on resource-constrained devices by combining high accuracy with extreme energy efficiency via neuromorphic hardware.
...
TL;DR: PNAFusion proposes a Progressive Pixel-Neighborhood Deformable Cross-Attention framework for multispectral object detection that uses local spatial priors (PNCA module) and adaptive deformable alignment (ADA module) with iterative refinement to achieve efficient cross-modal fusion, reducing computational cost while achieving strong mAP results on FLIR, M3FD, and DroneVehicle datasets.


<details>
  <summary>Details</summary>
Motivation: Global cross-attention has quadratic complexity limiting deployment on resource-constrained platforms. Two key observations motivate the work: (1) misalignment between visible and thermal images is usually local, and (2) semantic correspondences across modalities follow non-linear spatial mappings that fixed receptive fields cannot model well.

Method: Proposes PNAFusion with two core modules: (1) Pixel-Neighborhood Cross-Attention (PNCA) that focuses on local neighborhoods rather than global matching to reduce redundancy and suppress background noise, and (2) Adaptive Deformable Alignment (ADA) that learns pixel-wise offsets to capture non-linear spatial correspondences. An iterative feedback mechanism progressively refines cross-modal feature alignment.

Result: Achieves 84.2, 90.5, and 85.5 mAP@0.5 on FLIR, M3FD, and DroneVehicle with YOLOv5; reaches 86.8 and 90.8 mAP@0.5 when transferred to Co-DETR. Reduces GPU memory by 33.0% vs ICAFusion and reduces FLOPs from 194.8G to 156.4G, though deformable sampling and iterative refinement add some latency.

Conclusion: PNAFusion demonstrates that local spatial priors and deformable alignment can effectively replace costly global attention for multispectral fusion, achieving strong accuracy-efficiency trade-offs suitable for resource-constrained deployment.
...
TL;DR: A universal image clustering framework using textual guidelines and LLM reasoning that works across diverse clustering scenarios without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Existing image clustering methods are task-specific and fail to generalize across different clustering scenarios (e.g., general vs. fine-grained, global vs. local criteria, balanced vs. long-tail distributions).

Method: Proposes a Guideline-Driven Image Clustering Agent with two key components: (1) Generative Concept Proxy Modeling for creating guideline-aware embeddings, and (2) LLM Traversal based on Minimum Spanning Tree for automatic cluster discovery using selective LLM reasoning.

Result: The framework consistently outperforms specialized methods across diverse clustering tasks spanning general to fine-grained categorization, global to local criteria, and balanced to long-tail distributions.

Conclusion: Textual guidelines can effectively bridge the gaps between different clustering scenarios, enabling a universal clustering framework without task-specific training.
...
TL;DR: This paper investigates which camera sensor properties most impact autonomous driving perception. Using a differentiable RAW-to-task pipeline, they find that learning spectral color-filter-array (CFA) weights significantly improves performance (+0.017-0.023 mIoU), while point-spread-function co-design actually hurts (-0.020 mIoU) due to data-processing inequality. They recommend a simple recipe: learn 2x2 CFA weights and keep identity PSF.


<details>
  <summary>Details</summary>
Motivation: While most autonomous driving research focuses on scaling models, backbones, and cooperative multi-agent fusion, this work explores an upstream question: what should the camera sensor itself measure to best benefit perception tasks?

Method: Using a differentiable RAW-to-task pipeline, the authors decompose and optimize various sensor degrees of freedom (CFA weights, PSF/optics, noise) to evaluate their impact on dense prediction tasks.

Result: Learning CFA weights is the most effective intervention (+0.017 mIoU on KITTI-360, +0.023 on ACDC). PSF co-design is net-negative (-0.020 mIoU). Noise co-optimization is marginal. Enlarging CFA tile beyond 2x2 consistently hurts performance.

Conclusion: The authors recommend a simple, model-agnostic recipe: learn the 2x2 CFA weights and keep an identity PSF. Gains are validated across fog, night, rain, and snow conditions.
...
TL;DR: DramaDirector is a geometry-grounded framework for generating multi-shot short dramas from plots, using depth-pose reference retrieval, schema-constrained training, and a new benchmark (DramaBoard) to improve faithfulness, consistency, and controllability over existing baselines.


<details>
  <summary>Details</summary>
Motivation: Short dramas pose unique challenges (rapid shot rhythms, dialogue-driven focus shifts, demanding cinematographic grounding) that prompt-level or text-only video generation pipelines struggle to handle. The paper aims to enable plot-to-short-drama generation with proper visual grounding.

Method: DramaDirector uses a planner that borrows cinematographic geometry from a gallery of real short-drama shots indexed by depth and pose. It decouples each shot into static visual and dynamic narrative conditions, trains the planner with schema-constrained SFT and GRPO under a learned text-visual alignment reward, and retrieves depth-pose references to guide first-frame generation and image-to-video synthesis.

Result: Experiments show DramaDirector improves over representative multi-agent and video generation baselines on faithfulness, consistency, and controllability.

Conclusion: The proposed geometry-grounded approach effectively addresses the challenges of short drama generation, and the DramaBoard benchmark provides a structured evaluation resource for future research.
...
TL;DR: This paper benchmarks nine hallucination detection methods on GI endoscopy VQA (Gut-VLM dataset) across five VLMs, finding that white-box method ReXTrust significantly outperforms all others (up to 93.0 AUC), with white-box access providing a consistent 19.5 AUC point advantage, while also identifying "confident confabulation" as a systemic failure mode for non-white-box methods.


<details>
  <summary>Details</summary>
Motivation: Hallucination detection in VLMs is critical for safe clinical deployment, but most existing evaluations focus on radiology (MIMIC-CXR, VQA-RAD), leaving gastrointestinal (GI) endoscopy largely unexplored.

Method: Nine hallucination detection methods (black-box: RadFlag, SelfCheckGPT-NLI; gray-box: AvgProb, AvgEnt, MaxProb, MaxEnt, Semantic Entropy, VASE; white-box: ReXTrust) are benchmarked on the Gut-VLM dataset (4,392 VQA pairs) across five VLMs of varying sizes (4B-32B).

Result: ReXTrust (white-box) achieves highest AUC across all five models (peak 93.0 on MedGemma-4B), outperforming alternatives by statistically significant margins. White-box access yields 19.5 average AUC points advantage. Token-level gray-box methods (MaxEnt, MaxProb) outperform clustering-based gray-box and black-box methods. Identified "confident confabulation" as a systemic failure for consistency/uncertainty-based methods.

Conclusion: White-box hidden-state access provides substantial and consistent advantages for hallucination detection in GI endoscopy VQA, while non-white-box methods remain vulnerable to confident confabulation, suggesting future work should prioritize white-box approaches or develop methods robust to this failure mode.
...
TL;DR: PreciseDoc is a Large Multimodal Model designed for precise element grounding in document images, using synthetic data pipelines and reinforcement learning to improve visual grounding and reasoning for Document VQA tasks.


<details>
  <summary>Details</summary>
Motivation: Existing LMMs exhibit poor grounding precision in text-rich document images, failing to accurately locate critical document elements needed for reliable reasoning.

Method: (1) Two data pipelines for mass-producing high-quality synthetic documents with fine-grained coordinate metadata, including camera-effect hand-filled documents. (2) A training paradigm where grounding and reasoning are jointly supervised with reinforcement learning.

Result: Comprehensive evaluation on various benchmarks demonstrates advantages in document spatial grounding and document understanding.

Conclusion: The proposed data and methods significantly improve grounding precision and can be optimized for Document VQA tasks, enabling real-world functions like locating personal information from CVs.
...
TL;DR: The paper introduces Bengal-HP_RU, the first publicly available head pose dataset focused on Bengali subjects, containing 12,894 labeled images with yaw, pitch, and roll annotations, collected from Wikimedia Commons and split into training/test sets by uploader identity.


<details>
  <summary>Details</summary>
Motivation: Existing head pose datasets lack representation of South Asian populations, especially Bengali individuals, creating a demographic gap in head pose estimation research.

Method: Images were collected from Wikimedia Commons under free licenses, processed through an automated pipeline with manual label correction, and partitioned by uploader identity to prevent data contamination.

Result: 12,894 labeled head images with continuous yaw/pitch/roll values; 10,494 training and 2,400 test images from 296 unique uploaders; diverse in age, gender, occlusion, illumination, and background.

Conclusion: Bengal-HP_RU addresses the underrepresentation of Bengali subjects in head pose datasets and is publicly available for further research.
...
TL;DR: Sat2City v2 generates explicit 3D textured city meshes from a single satellite image by adapting a pretrained 3D foundation model, using a real-world dataset of 16,241 satellite-mesh pairs across 9 cities, achieving state-of-the-art performance in geometry and appearance generation.


<details>
  <summary>Details</summary>
Motivation: The authors aim to move beyond rendering-oriented 3D proxies to generate reusable, controllable textured mesh assets from single satellite images, addressing limitations of the prior Sat2City framework (random appearance, synthetic training data, poor scalability to real-world noisy reconstructions).

Method: Sat2City v2 fine-tunes a pretrained native structured-latent 3D foundation model on a real-world dataset of 16,241 satellite-mesh pairs. It encodes meshes into a pretrained 3D latent space, uses satellite-conditioned geometry flow, then anchors satellite-conditioned texturing on the decoded shape, preserving a geometry-to-appearance cascade.

Result: Achieves best overall performance on metric-scale DSM reconstruction and generative city-asset benchmarks for both geometry and appearance compared to evaluated baselines. Introduces the first documented satellite-mesh paired dataset for this asset-level task.

Conclusion: Sat2City v2 successfully advances satellite-to-city generation to explicit textured mesh assets with controllable appearance, supported by a real-world paired dataset, outperforming prior methods.
...
TL;DR: A geometry-aware style transfer framework for 3D Gaussian splatting that simultaneously transfers appearance and geometric structures using decoupled optimization and geometry-aware contrastive feature matching (GCFM).


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS stylization methods focus on color-based stylization and overlook structural adaptation (geometry transformation), leading to incomplete style transfer.

Method: A decoupled optimization scheme that alternately updates color and geometry parameters, enabled by geometry-aware contrastive feature matching (GCFM) which integrates RGB, depth, and edge cues into a contrastive objective.

Result: Superior performance in both qualitative fidelity and quantitative metrics, significantly outperforming existing 3DGS-based stylization methods.

Conclusion: The proposed geometry-aware approach achieves stable and consistent scene-level geometry transformation, demonstrating the importance of incorporating structural adaptation in 3D style transfer.
...
TL;DR: This paper argues that video generation models learn only partial, implicit world models, not fully grounded ones, and proposes that autonomous video generation with counterfactual controllability is a pathway to self-evolving world models.


<details>
  <summary>Details</summary>
Motivation: To challenge the oversimplified claim that video generation equals world modeling, and to propose a more nuanced perspective emphasizing counterfactual controllability as the key criterion for genuine world models.

Method: Conceptual analysis and argumentation: distinguishing between predictive realism (mere video generation) and counterfactual controllability (the ability to test "what would happen under an action" and feed action knowledge back into generation).

Result: The paper identifies that current video generation lacks grounded knowledge of controllable variables, embodiment constraints, and intervention validity; it posits counterfactual controllability as the decisive criterion.

Conclusion: Autonomous video generation with counterfactual controllability is a promising direction for realizing self-evolving world models that go beyond predictive realism.
...
TL;DR: DU introduces Differential Unfolding, a heterogeneous framework for video Snapshot Compressive Imaging that replaces uniform repetition with dynamic evolution, achieving SOTA results with reduced computation.


<details>
  <summary>Details</summary>
Motivation: Existing DUNs waste computation on redundant, uniformly stacked priors that suffer from representation stagnation as optimization converges, leading to inefficient resource usage.

Method: DU uses a Differential Evolutionary Framework (DEF) with sparse high-parameter stages for structural anchoring and lightweight differential stages using Differential Representation Prior (DRP), Differential Representation Attention (DRA), and Differential Modulated FFN (DM-FFN) for efficient feature refinement.

Result: DU establishes new state-of-the-art results on video SCI while significantly reducing computational overhead compared to prior DUNs.

Conclusion: By focusing computation on dynamic evolution instead of static redundancy, DU achieves a superior accuracy-efficiency trade-off for video SCI reconstruction.
...
TL;DR: ProteusVPR is a two-stage retrieval-refinement framework for visual place recognition in maritime environments, combining standard VPR with geometric-visual estimation using temporal frames. A new XHZ dataset (8K panoramic ship-borne) is introduced. Results show over 60% reduction in mean localization error across multiple VPR backbones.


<details>
  <summary>Details</summary>
Motivation: Existing VPR methods designed for urban/indoor scenes fail in maritime environments due to cross-scene perceptual shifts between open decks (sparse textures, severe illumination) and enclosed cabins (repetitive structures, high visual ambiguity).

Method: A two-stage retrieval-refinement framework: Stage 1 uses any standard VPR model for initial retrieval; Stage 2 employs a geometric-visual estimation network fusing the retrieved image with two temporally preceding frames, incorporating geometric descriptors, a local affine coordinate system, and camera azimuth encoding.

Result: Consistent improvement in localization accuracy across multiple VPR backbones on the XHZ dataset, with over 60% average reduction in mean localization error.
...
TL;DR: PriorTR is a training-free token reduction method for MLLMs that corrects for model-induced attention bias by contrasting prior (instruction-agnostic) attention with task-conditioned attention using a null token probe, improving accuracy-efficiency trade-off especially under aggressive token budgets.


<details>
  <summary>Details</summary>
Motivation: Existing token reduction methods rely on text-visual attention scores, but these scores are dominated by a model-induced prior that focuses on task-agnostic regions even without textual instructions, causing instruction-conditioned tokens to be incorrectly suppressed and discarded during pruning.

Method: PriorTR introduces a null token that acts as an instruction-agnostic probe in the attention block to estimate the model-induced prior attention map. It then contrasts this prior with the task-conditioned attention distribution to measure the additional usable information per visual token, enabling better pruning decisions—all within a single forward pass.

Result: Extensive experiments across multiple multimodal benchmarks and MLLMs show PriorTR consistently improves the accuracy-efficiency trade-off over strong training-free baselines, particularly under aggressive token budgets.

Conclusion: PriorTR effectively addresses the attention bias problem in visual token pruning by decoupling task-conditioned attention from model-induced prior, achieving better performance without requiring additional training or multiple forward passes.
...
TL;DR: OpenReLoc is a camera relocalization system that uses structured object representations (semantics, layout, geometry) instead of low-level vision, leveraging foundation models for open-vocabulary 2D-3D object matching, DIOU-based reference frame selection, and dual-path 2D ICP loss for accurate pose estimation.


<details>
  <summary>Details</summary>
Motivation: Prior indoor relocalization methods rely on low-level vision schemes that fail to perceive scene semantics and compositions, limiting interpretability and applicability. The paper explores organizing rich object information into structured map representations to drive relocalization.

Method: 1) Multi-modal mechanism integrating open-vocabulary semantic knowledge for 2D-3D object matching using foundation models. 2) Object-oriented reference frames as position priors with DIOU-based selection strategy for scalable scenes. 3) Dual-path 2D Iterative Closest Pixel (ICP) loss guided by object shape for stable pose optimization.

Result: Superior relocalization recall and accuracy across various datasets compared to prior methods.

Conclusion: OpenReLoc demonstrates that leveraging structured object-level representations with foundation models significantly improves indoor visual relocalization performance, offering both better interpretability and accuracy.
...
TL;DR: A novel debiasing method combining diffusion-based bias concept mining (CBCM) and dual-branch cross-projection (DCD) to tackle spurious correlations in foundation models without requiring group labels, achieving SOTA worst-group accuracy with minimal parameter tuning.


<details>
  <summary>Details</summary>
Motivation: Foundation models suffer from spurious correlations in biased datasets, leading to poor generalization on minority groups. Existing methods struggle because (1) group-unsupervised approaches cannot reliably identify semantically meaningful spurious attributes, and (2) single-branch designs entangle target and spurious features.

Method: (1) CBCM: Uses diffusion-disentangled concept representations to mine semantically grounded spurious attributes without labels. (2) DCD: A dual-branch prompt-tuning framework that separates target/spurious representations and removes spurious information via cross null-space projection while preserving target semantics.

Result: Achieves state-of-the-art worst-group accuracy among group-unsupervised methods on four benchmarks, tuning only 0.22% of model parameters.

Conclusion: The proposed two-stage approach effectively addresses bias mitigation by first identifying meaningful spurious concepts via diffusion models, then explicitly disentangling and removing them in a dual-branch architecture.
...
TL;DR: CLSE is a training-free token pruning framework that uses cross-layer spectral evolution in the frequency domain to identify semantically important visual tokens, reducing redundancy in MLLMs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Existing token pruning methods rely on single-layer signals (attention scores, token similarities) that overlook cross-layer transformation dynamics and suffer from positional bias in multimodal sequences.

Method: CLSE quantifies token importance by measuring how token representations evolve across Transformer layers in the frequency domain, tracking the transition from high-frequency structural details to low-frequency semantic abstractions, and preserving tokens with stronger spectral redistribution.

Result: CLSE achieves superior efficiency-accuracy trade-off under aggressive token reduction across image and video benchmarks, reducing FLOPs, KV cache memory, and latency while maintaining competitive or improved performance across multiple MLLMs.

Conclusion: Cross-layer spectral evolution provides a stable, position-bias-free importance criterion for token pruning, enabling efficient MLLM inference without training.
...
TL;DR: PoinTriE is a unified framework for point cloud video understanding that achieves data, parameter, and memory efficiency through pre-training with synthetic pseudo-motion trajectories and multimodal contrastive learning, plus fine-tuning with a frozen backbone and lightweight Spatio-temporal Side Network using LoRA units and gradient flow masking.


<details>
  <summary>Details</summary>
Motivation: Existing PEFT methods for point cloud video suffer from high annotation costs and severe memory bottlenecks. The authors aim to mine richer supervision signals from existing data rather than scaling datasets, while drastically reducing memory footprint compared to full fine-tuning.

Method: (1) Pre-training: synthesizes pseudo-motion trajectories via rigid transformations, paired with text and 2D projections; proposes Geometric-Motion Duality Network with multimodal contrastive learning, rigid rotation prediction, and motion distribution divergence. (2) Fine-tuning: freezes pretrained backbone, updates only a lightweight Spatio-temporal Side Network built with LoRA units, equipped with gradient flow masking to reduce memory and parameters.

Result: Establishes new state-of-the-art results on action recognition and semantic segmentation tasks.

Conclusion: PoinTriE successfully unifies data, parameter, and memory efficiency for point cloud video understanding, outperforming prior methods on key benchmarks.
...
TL;DR: The paper reframes test-time canonicalization for affine-invariant classification as OOD detection, systematically evaluating ~20 OOD scores and 9 search algorithms, finding distance-based scores with random search and local refinement work best, and adding a gating mechanism to preserve in-distribution accuracy.


<details>
  <summary>Details</summary>
Motivation: Pretrained vision models fail on affine-transformed inputs (rotation, scaling, shearing). Existing solutions require architectural changes or retraining. Test-time canonicalization leaves the classifier untouched but prior work uses limited scoring functions and search procedures.

Method: Reframes canonicalization as OOD detection problem, enabling any OOD score to serve as the energy minimized over transformations. Systematically evaluates ~20 OOD scores and 9 search algorithms across multiple benchmarks. Adds a gated mechanism that canonicalizes only when OOD score indicates it's needed.

Result: Distance-based OOD scores paired with random search and local refinement perform best overall. The gated mechanism preserves most in-distribution accuracy while retaining robustness gains on transformed inputs.

Conclusion: The proposed approach broadens the design space for canonicalization, achieves strong robustness without model modification, and the gating mechanism effectively balances in-distribution and out-of-distribution performance.
...
TL;DR: A systematic review of 3D scene completion research from 2016-2026, covering evolution from voxel-based methods (SSCNet) to Gaussian splatting with diffusion priors, including a taxonomy, analysis of contributions, challenges, and a future research agenda.


<details>
  <summary>Details</summary>
Motivation: 3D scene completion is critical for applications like autonomous navigation and augmented reality, and the field has undergone rapid paradigm shifts over the past decade. This review aims to systematically organize and clarify the evolving landscape of representations—from voxels to Gaussians—to help researchers understand the progress and identify remaining challenges.

Method: A systematic literature review covering 2016-2026, compiling and categorizing research contributions across representation paradigms: voxel grids, point learning, implicit neural fields, transformers, diffusion networks, and 3D Gaussian primitives. A taxonomy is developed to structure the analysis.

Result: Identified six major representation paradigms that revolutionized the field. The latest paradigm combines generative diffusion priors with real-time rendering using Gaussian splatting. A comprehensive taxonomy was established, and key challenges still requiring attention were documented.

Conclusion: The study provides a structured understanding of a decade of 3D scene completion research and proposes a research agenda outlining future directions for developing next-generation systems.
...
TL;DR: AdaQ is a training-free, adaptive keyframe selection method for long video understanding that uses Quasi-Gaussian Sampling (inspired by the 3-σ rule) to dynamically adjust the sampling interval based on query type, achieving state-of-the-art performance with only 1 hyper-parameter.


<details>
  <summary>Details</summary>
Motivation: Long video understanding with MLLMs suffers from excessive computation and memory costs. Existing keyframe selection methods use hard sampling principles that lead to low flexibility and high noise, lacking adaptive frame selection.

Method: Formulates video frame selection as a Quasi-Gaussian Sampling problem. Inspired by the 3-σ rule, AdaQ adaptively determines the optimal 3-σ interval — smaller for local queries and larger for global queries — enabling robust, training-free, adaptive frame sampling with only one hyper-parameter.

Result: Applied to 4 MLLMs with 3 embedding models. AdaQ helps Qwen3-VL-8B outperform GPT4o by 15.8% on average using only 64 frames. It shows obvious performance gains over default MLLMs and SOTA keyframe selection methods, with superior robustness and efficiency.

Conclusion: AdaQ provides an effective, efficient, and robust solution for long-video understanding by adaptively sampling keyframes without training, requiring minimal hyper-parameter tuning.
...
TL;DR: The paper introduces CARE (Co-occurring Associated REtained concepts) and a metric (CARE score) to measure preservation of benign co-occurring concepts during unlearning in diffusion models. The proposed ReCARE framework automatically identifies and safeguards these concepts while erasing only the target concept, achieving state-of-the-art balance between erasure quality and concept preservation.


<details>
  <summary>Details</summary>
Motivation: Existing unlearning methods for diffusion models inadvertently suppress benign co-occurring concepts (e.g., unlearning nudity also suppresses the concept of "person"). The authors aim to identify and preserve these undesirably removed concepts while still effectively erasing the target concept.

Method: Defines CARE (Co-occurring Associated REtained concepts) and a CARE score metric. Proposes ReCARE framework that automatically constructs a CARE-set (curated vocabulary of benign co-occurring tokens) from target images and leverages it during training for stable unlearning.

Result: Extensive experiments across multiple target concepts (Nudity, Van Gogh style, Tench object) show ReCARE achieves state-of-the-art performance in balancing robust concept erasure, overall utility, and CARE preservation.

Conclusion: ReCARE effectively addresses the overlooked problem of collateral damage to co-occurring concepts during unlearning, providing both a metric and a practical framework for more precise concept erasure in diffusion models.
...
TL;DR: I2C-3D is a novel optimization-based method for generating multi-view consistent compositional 3D assets with physically plausible interactions between objects, addressing limitations in existing single-object 3D generation approaches.


<details>
  <summary>Details</summary>
Motivation: Existing 3D generation methods struggle with (1) generating multi-object compositional 3D assets due to lack of modeling for Gaussian primitive interactions, and (2) cross-view inconsistency caused by Score Distillation Sampling operating on single views independently.

Method: Two key components: (1) Inclusive Interactive Collisions strategy to guide Gaussian primitives toward reasonable interaction regions for physically plausible and visually coherent object interactions; (2) Multi-View Adaptive Score Distillation Sampling that distills multi-view consistency and layout priors by modulating attention maps of instance and spatial tokens across viewpoints.

Result: I2C-3D outperforms existing methods in generation quality and multi-view consistency, while also supporting flexible 3D editing for complex scene generation.

Conclusion: The proposed I2C-3D effectively generates high-fidelity, multi-view consistent compositional 3D assets with reasonable inter-object interactions, advancing beyond single-object 3D generation capabilities.
...
TL;DR: MorVess is a morphology-aware segmentation framework that integrates differentiable geometric priors with SAM adaptation to improve pulmonary vessel segmentation, achieving superior performance on two CT benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing deep segmentation models optimize binary masks without explicit geometric constraints, struggling to preserve topology integrity and recover continuous tubular morphology in sparse, tortuous, multi-scale pulmonary vessels.

Method: MorVess jointly predicts vessel masks, distance maps, and thickness maps for explicit geometric supervision; uses a lightweight 2.5D adapter to bridge 3D context with 2D SAM representations; and employs a global-local fusion block for multi-level semantics and geometric cues.

Result: MorVess achieves superior Dice, clDice, and HD95 scores on two challenging pulmonary CT benchmarks, with substantial improvements in small-vessel recovery and global connectivity.
...
TL;DR: GIVE is a geometry-instructed video editing framework that uses depth-box and orientation-box streams to specify object-level geometric edits (translate, rotate, scale, duplicate, remove) with temporal coherence and consistent secondary effects like shadows and reflections, trained on a scalable graphics-engine pipeline.


<details>
  <summary>Details</summary>
Motivation: Object-level geometric edits (translating, rotating, scaling, duplicating, or removing objects) are routine in digital content creation but remain unreliable in generative video editing due to difficulty in specifying 3D state changes unambiguously across viewpoint and time while maintaining consistent secondary effects (shadows, reflections).

Method: GIVE uses a unified object-state formulation with two video-aligned geometry streams: depth-box (coarse 3D placement/extent) and orientation-box (appearance-agnostic orientation cue). A scalable graphics-engine pipeline provides paired supervision by executing object-level edit programs and rendering controlled before/after pairs.

Result: GIVE produces faithful geometric edits with temporal coherence and consistent secondary effects across multiple operators in a unified framework, with promising transfer to in-the-wild videos.

Conclusion: The proposed geometry-instructed approach effectively addresses object-level geometric video editing by providing unambiguous 3D state specification and maintaining geometry-dependent secondary effects consistently.
...
TL;DR: FiCA is a feed-forward pipeline that generates photorealistic, drivable 3D Gaussian avatars from a single portrait image using human-centric vision models, a diffusion model, and a mesh refinement network, achieving superior visual quality without test-time optimization.


<details>
  <summary>Details</summary>
Motivation: Creating lifelike, drivable avatars from a single image is highly challenging due to limited visual information for inferring 3D head appearance and geometry.

Method: Combines human-centric vision foundation models with a diffusion model for generative mapping from partial observations to complete 3D mesh reconstruction, plus a feed-forward mesh refinement network and a universal prior that decodes mesh into 3D Gaussians.

Result: Generated avatars faithfully represent diverse identities and surpass the visual quality of recent competing methods.

Conclusion: FiCA enables real-time, photorealistic, drivable avatar generation from a single image without person-specific optimization.
...
TL;DR: EVA introduces a framework that generates continuous latent visual representations (Latent_slot tokens) instead of discrete outputs for tool invocation, enabling end-to-end training with text tokens. The authors propose D-GSPO to address policy deviation in the transition window and construct EVA-230K dataset for SFT, achieving performance gains with improved inference efficiency.


<details>
  <summary>Details</summary>
Motivation: Current large multimodal models rely on generating discrete outputs (code, box coordinates) to invoke external tools, which introduces rigid dependencies and substantial latency. The authors aim to overcome these limitations by enabling continuous latent visual representations as intermediate visual thoughts.

Method: EVA generates continuous latent visual representations (Latent_slot tokens) that are trained end-to-end with discrete text tokens. To address policy deviation in the 'transition window' after Latent_slot tokens, the authors develop D-GSPO (Decouple-GSPO) to decouple optimization of latent and discrete components. They also construct EVA-230K, a high-quality text-image interleaved CoT dataset.

Result: Extensive experiments across multiple benchmarks confirm that EVA achieves significant performance gains while enhancing inference efficiency.

Conclusion: The EVA framework effectively overcomes limitations of discrete-output-based visual integration in multimodal models by using continuous latent visual representations, achieving better performance and efficiency.
...
TL;DR: M^2C-EvDet proposes a multi-domain, multi-order cross-modal knowledge distillation framework using frequency learning and hypergraph computation to improve event-based object detection by transferring richer knowledge from frame-based detectors.


<details>
  <summary>Details</summary>
Motivation: Event-based object detection suffers from performance gaps compared to frame-based detection due to sparse representations and inadequate visual semantics. Prior knowledge distillation methods only address spatial or pairwise relational information, limiting performance in complex scenarios.

Method: M^2C-EvDet uses two modules: (1) Adaptive Frequency-Decoupled Feature Distillation (AF^2D^2) for frequency-domain learning, and (2) Multi-Order Relational Distillation (MORD) based on hypergraph computation to capture higher-order relational information.

Result: The proposed framework outperforms prior distillation methods by addressing both frequency-domain and multi-order relational information, reducing the gap between event-based and frame-based detection.

Conclusion: M^2C-EvDet effectively bridges the cross-modal discrepancy in event-based object detection through comprehensive multi-domain and multi-order knowledge transfer.
...
TL;DR: TuringViT introduces Turing Linear Attention (TLA), VISTA-Curation data strategy, and native dynamic-resolution pretraining to enable efficient, customizable ViT training with 90% less data while outperforming leading baselines in latency, VLM integration, and downstream performance.


<details>
  <summary>Details</summary>
Motivation: Current VLMs/VLAs rely on off-the-shelf ViTs (e.g., SigLIP2) that don't meet diverse downstream needs (latency, temporal modeling, VLM integration). Training customized SOTA ViTs is prohibitively expensive due to massive data requirements and the computational cost of softmax attention at high/dynamic resolutions, forcing low-resolution pretraining with post-hoc adaptation.

Method: Three key designs: (1) Turing Linear Attention (TLA) for efficient sequence modeling, (2) VISTA-Curation to construct supervision-rich image-video training data, (3) native dynamic-resolution pretraining supporting flexible inputs from the start with seamless transfer to downstream VLMs.

Result: Outperforms leading open-source ViT baselines using only 10% of the data, achieves stronger downstream VLM performance, and delivers substantially better latency scaling on high-resolution inputs. Scaling-law analysis shows predictable improvement with curated data scale, far from saturation.

Conclusion: TuringViT provides a reproducible, low-cost pipeline for the community to train, customize, and deploy SOTA-level ViTs, serving as a unified visual foundation across XPeng's AI systems and democratizing access to high-performance Vision Transformers.
...
TL;DR: The paper proposes a "Think with LLM, Move with Motion Skill" framework for text-driven 3D human-human interaction generation, where an LLM planner infers social structure (phases, roles, coordination) and a motion executor grounds this structure into realistic two-person motion using a pretrained solo motion model adapted with LoRA and partner-aware conditioning.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-motion generation excels at single-person motions but fails to handle human-human interaction (HHI) because HHI requires modeling social structure (phase progression, actor roles, inter-actor coordination), which existing methods lack.

Method: A planner-executor paradigm: (1) LLM planner decomposes interactions into phases, assigns partner-aware roles, and aligns them with motion sequences; (2) Motion executor adapts a pretrained solo motion model using LoRA, previous-phase self-conditioning, and ego-relative partner conditioning to generate coordinated two-person motion.

Result: The Solo-to-Social framework produces improved phase consistency, role alignment, and partner-aware coordination in 3D HHI generation.

Conclusion: LLMs can think about social structure but cannot directly generate motion; separating planning (LLM) from execution (motion skill) effectively bridges social organization and motion realization for text-driven human-human interaction generation.
...
TL;DR: This paper introduces Tailor-Bench, a benchmark for evaluating whether visual world models (image/video generation models) truly understand physical principles, revealing that they struggle with rare/unconventional physical interactions and primarily rely on superficial visual patterns rather than genuine physical reasoning.


<details>
  <summary>Details</summary>
Motivation: Physical interactions follow a long-tailed distribution, and current visual world models are primarily evaluated on common interactions. The key question is whether these models truly internalize physical principles or just memorize common patterns.

Method: The authors propose Tailor-Bench with three scenario modes (Regular, Unconventional, Impossible) and two settings (predictive generation and descriptive generation) to systematically test physical reasoning capabilities.

Result: Performance degrades from Regular to Unconventional to Impossible scenarios, revealing a clear long-tail gap; image models fail on state changes, and video models additionally suffer from temporal inconsistencies.

Conclusion: Current visual world models show limited generalization beyond common physical interactions and rely on superficial visual patterns rather than true physical understanding.
...
TL;DR: 3DCarGen is a single-view 3D car generation framework that synthesizes 3D-consistent multi-view images from one real-world input image, using multi-view diffusion, 3D Gaussian Splatting, and color-normal joint optimization to produce high-fidelity, geometrically consistent 3D vehicle models for autonomous driving simulation.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view diffusion methods for single-image 3D reconstruction produce limited viewpoints and suffer from cross-view geometric inconsistencies, reducing reconstruction fidelity in real-world autonomous driving scenarios.

Method: (1) Synthesize fixed-viewpoint images from a single input image; (2) Feed them into a feed-forward reconstruction model to obtain a coarse 3D Gaussian Splatting representation; (3) Use this explicit 3D prior to guide a multi-view diffusion model for generating 3D-consistent images from arbitrary viewpoints; (4) Apply a fast mesh reconstruction algorithm with color-normal joint optimization for detailed, coherent 3D models.

Result: Extensive experiments on synthetic and real-world datasets show robust geometric consistency and superior reconstruction fidelity compared to existing methods.

Conclusion: 3DCarGen provides a scalable, high-fidelity solution for generating 3D-consistent vehicle models from single real-world images, with code and models to be released.
...
TL;DR: This paper introduces the first generative model for spatio-temporal interpolation of tropical cyclone microwave satellite imagery, handling heterogeneous data from multiple instruments at irregular intervals, and demonstrates improved performance through self-supervised learning and multi-source data fusion.


<details>
  <summary>Details</summary>
Motivation: Microwave satellite imagery is critical for monitoring tropical cyclone precipitation and intensity, but suffers from long revisit times that can miss rapid storm evolution. There is a need for interpolation methods, but it's challenging due to high heterogeneity across different microwave instruments (varying characteristics, irregular intervals, geographic misalignment).

Method: A generative model that handles multiple geospatial sources across samples with irregular time intervals, geographic misalignment, and varying instrument characteristics. Trained with a self-supervised task where a random source is masked and reconstructed, combining infrared and microwave data.

Result: Self-supervised training significantly reduces Continuous Ranked Probability Score compared to supervised training. Combining infrared and microwave data improves performance over microwave-only. The generative model's ensemble mean matches a deterministic model, while producing a power spectrum much closer to true observations.

Conclusion: This is the first generative model capable of interpolating microwave images of cyclones by combining multiple microwave instruments and infrared observations at irregular time intervals.
...
TL;DR: This paper proposes a unified multimodal framework for RGB-D video interpolation that jointly exploits RGB, depth, and event camera cues to achieve high frame-rate RGB-D videos with better photometric and geometric quality.


<details>
  <summary>Details</summary>
Motivation: RGB-D cameras are limited to low frame rates, making it hard to capture rapid scene dynamics. Existing RGB interpolation methods fail on RGB-D data, producing blurry boundaries and geometric artifacts. Motion estimation from only two frames is under-constrained in complex scenes.

Method: A unified multimodal framework that: (1) extracts and fuses RGB, depth, and event cues; (2) estimates bidirectional flow with motion basis refinement for RGB and Z-axial refinement for depth; (3) synthesizes target frames via bidirectional warping and soft blending. A new RGB-D-Event dataset is also constructed.

Result: Superior photometric fidelity for RGB interpolation and stronger geometric accuracy for depth interpolation compared to existing methods, validated on a public benchmark and the proposed dataset.

Conclusion: Leveraging event cameras' ultra-high temporal resolution significantly improves RGB-D video interpolation quality, addressing both appearance and geometry challenges.
...
TL;DR: ActiveScope is a training-free framework that enhances Multimodal Large Language Models (MLLMs) for fine-grained high-resolution image perception by addressing two key issues: Contextual Dominance (distractors overwhelming target attention) and Semantic Bias (model fixating on salient concepts). It introduces Semantic Anchor Localization (SAL) and Interference-Suppressed Refinement (ISR) modules, achieving 96.34% accuracy on V* Bench and outperforming existing training-free methods.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs struggle with fine-grained perception in high-resolution images due to two issues: Contextual Dominance (salient distractors overwhelm target attention) and Semantic Bias (global semantics cause fixation on the most salient concept, leading to incomplete multi-object localization). Current training-free methods fail to handle these problems effectively.
...
Result: ActiveScope achieves 96.34% accuracy on V* Bench and outperforms existing training-free methods on high-resolution image understanding benchmarks, demonstrating the effectiveness of active search and self-correction.

Conclusion: The proposed active search and self-correction paradigm (ActiveScope) effectively addresses fine-grained perception limitations in MLLMs without requiring additional training, validating its superiority over existing methods.
...
TL;DR: DHANet is a Dual Hierarchical Aggregation Network that addresses semantic over-alignment and attribute over-alignment in cross-domain few-shot segmentation via hierarchical spatial and channel aggregation, plus an Online Probabilistic Semantic Bank for pseudo-prototype sampling.


<details>
  <summary>Details</summary>
Motivation: Existing CD-FSS methods only address style-based feature distribution shifts but ignore semantic granularity and discriminative attribute differences across domains, causing semantic over-alignment and attribute over-alignment in support-query matching.

Method: Three modules: (1) HSA - multi-scale region aggregation along spatial dim for semantic-enhanced features; (2) HCA - multi-scale attribute aggregation along channel dim for attribute-enhanced features; (3) OPSB - online probabilistic semantic bank that constructs/updates class probability distributions from query predictions and samples pseudo-prototypes as extra support.

Result: State-of-the-art results on four target-domain datasets.

Conclusion: Hierarchical dual aggregation (spatial + channel) and online pseudo-prototype generation effectively mitigate two types of over-alignment in CD-FSS.
...
TL;DR: A training-free framework for Cross-domain Few-shot Segmentation using DINOv2, achieving SOTA performance with SAFR, ASE, and HPM modules.


<details>
  <summary>Details</summary>
Motivation: Existing CD-FSS methods rely on training/fine-tuning which incurs high computational costs and risks overfitting; even powerful foundation models show marginal gains or degrade when fine-tuned.

Method: A training-free framework built on DINOv2 with three modules: SAFR (semantic feature re-fusion), ASE (adaptive support enhancement via query aggregation), and HPM (hybrid prototype matching for varying semantic complexity).

Result: Achieves state-of-the-art performance on four target domain datasets without any training.

Conclusion: Eliminating trainable parameters effectively avoids overfitting and training overhead while leveraging vision foundation models for superior CD-FSS performance.
...
TL;DR: MM-TRELLIS is a multimodal 3D vehicle generation method that integrates multi-view images and LiDAR point clouds from autonomous driving data into a native 3D generative model, producing high-quality meshes with geometric accuracy and cross-view consistency, outperforming existing methods on Waymo dataset.


<details>
  <summary>Details</summary>
Motivation: Existing vehicle generation methods fail to fully exploit multimodal sensors (multi-view images & LiDAR), relying on neural rendering that yields low-quality meshes. Meanwhile, native 3D generative models cannot handle arbitrary multi-view inputs from in-the-wild driving scenes.

Method: (1) Multi-view images serve as conditioning inputs for the 3D generative model. (2) LiDAR point clouds provide test-time guidance for geometric accuracy and cross-view consistency. (3) During denoising, guidance point cloud is first aligned with model priors, then consistency is enforced between generated geometry and guidance. (4) A voxel filtering strategy based on 3D Gaussian opacity suppresses floaters and produces clean meshes.

Result: Comprehensive experiments on Waymo dataset demonstrate the method outperforms existing methods in high-fidelity 3D vehicle generation.

Conclusion: MM-TRELLIS successfully integrates multimodal sensor data (LiDAR + images) into native 3D generative models for in-the-wild vehicle generation, achieving superior quality and clean mesh outputs.
...
TL;DR: This paper investigates how contrast normalization (CLAHE), data augmentation, and layer freezing affect TrOCR-based HTR on medieval manuscripts, achieving 8.03% CER on the Cortonese dataset and 7.84% CER without CLAHE, with analysis of freezing thresholds and cross-dataset transferability.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning transformer-based HTR models on medieval manuscripts is challenging due to domain shift from modern pre-training data; the authors aim to systematically understand how controllable fine-tuning choices affect recognition accuracy on small historical datasets.

Method: Controlled experiments varying contrast normalization (CLAHE), data augmentation, and layer freezing strategies, tested on a 13th-century Italian manuscript (Cortonese) and validated on the READ-16 benchmark; error analysis uses Grad-CAM and decoder cross-attention maps.

Result: Best configuration achieved 8.03% CER on Cortonese; removing CLAHE yielded 7.84% CER; freezing up to 3 encoder or 6 decoder layers caused no significant harm; decoder freezing thresholds transfer better across datasets than encoder thresholds.

Conclusion: Optimization can reduce reliance on image preprocessing; freezing strategies must be validated per dataset; combined freezing is not robustly transferable; attention-based diagnostics reveal failure modes.
...
TL;DR: REDI-Match introduces a Rotation-Equivariant Distillation (REDI) paradigm that distills semantic knowledge from Vision Foundation Models into a strictly rotation-equivariant encoder, paired with an entropy-driven spatial alignment module, achieving SOTA pose accuracy (13.89% improvement on SatAst) with 1.9x faster inference than RoMa v2 (~41 FPS on RTX 4090).


<details>
  <summary>Details</summary>
Motivation: Existing VFM-based dense matching methods struggle with severe in-plane rotation: data-driven approaches need costly parameter scaling to learn rotations implicitly, while strictly equivariant networks lack the rich semantic capacity of modern VFMs. Current practice freezes VFMs and pushes all rotation generalization burden onto the decoder, creating an architectural bottleneck.

Method: (1) Rotation-Equivariant Distillation (REDI): distills non-equivariant semantic representations from a frozen VFM into a lightweight, strictly rotation-equivariant encoder using an equivariant geometric architecture. (2) Entropy-driven spatial alignment module in the decoder that evaluates discrete rotation hypotheses to explicitly lock onto the canonical coordinate system, eliminating global ambiguity before continuous refinement.

Result: Achieves new SOTA across multiple benchmarks, with 13.89% absolute pose accuracy improvement on SatAst dataset, operates 1.9x faster than RoMa v2, enabling real-time inference at ~41 FPS on a single RTX 4090 GPU.

Conclusion: REDI-Match effectively breaks the rotation generalization bottleneck in VFM-based matching by combining equivariant distillation with explicit rotation hypothesis evaluation, delivering both superior accuracy and real-time efficiency.
...
TL;DR: UniTranslator is a unified multimodal framework for In-Image Machine Translation that addresses understanding-generation conflicts and spatial misalignment via an Understand-Generation Alignment Module (UGAM) and a Spatial Mask Decoder (SMD), achieving state-of-the-art performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing unified multimodal models suffer from (1) understanding-generation conflicts where translated text during understanding mismatches text supervision in generation, and (2) spatial position misalignment where rendered text fails to accurately match target text regions.

Method: UniTranslator introduces (1) Understand-Generation Alignment Module (UGAM) to bridge representation gaps between understanding and generation for semantic consistency, and (2) Spatial Mask Decoder (SMD) with pixel-level supervision over text regions for improved spatial grounding and layout controllability.

Result: Achieves state-of-the-art performance across diverse language directions and complex real-world layouts. Results demonstrate strong mutual reinforcement between translation understanding and image generation.

Conclusion: UniTranslator effectively resolves key IIMT challenges through unified multimodal learning, highlighting the advantage of tightly coupling translation understanding and text editing.
...
TL;DR: This paper proposes metric object-size estimation as an ill-posed diagnostic task to study evidence selection in VLMs, finding that even the largest models (up to 397B parameters) underperform text-only LLMs and fail to leverage scene geometry cues.


<details>
  <summary>Details</summary>
Motivation: Counterfactual analysis has limited diagnostic value on well-posed tasks because multiple cues independently support the same answer. The authors propose metric object-size estimation as an ill-posed setting where no single cue is sufficient, allowing more informative analysis of evidence selection in VLMs.

Method: The authors assemble Metric VQA dataset (10,813 queries from Objectron + 331 real-world scenes). They evaluate 12 open-weight VLMs (3B-397B parameters) using counterfactual analysis that decomposes six visual and language evidence channels, and analyze LoRA fine-tuning as an intervention.

Result: The largest VLMs (Qwen3-VL-235B, Qwen3.5-397B, InternVL3.5-241B) still trail a text-only frontier LLM on in-the-wild splits. Target identity is the most load-bearing cue; target pixels and local context help only some models; apparent size shifts predictions without directional readout; global scene geometry is largely unused.

Conclusion: While metric estimation is learnable via LoRA fine-tuning, models do not learn to effectively leverage scene geometry cues, indicating a fundamental gap in how VLMs process spatial/geometric information for ill-posed tasks.
...
TL;DR: TIGER is a tri-prior fusion framework (Identity, Geometry, Generative) for face video restoration that addresses identity shift, viewpoint entanglement, and perceptual realism, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: To overcome three key challenges in face video restoration: identity shift across frames, viewpoint-entangled guidance, and maintaining perceptual realism while being efficient.

Method: Proposes TIGER framework with: (1) Identity Prior via subject-discriminative embeddings, (2) Geometry Prior by lifting 2D cues to disentangled 3D parameter space, (3) Generative Prior via one-step rectified flow; plus a progressive three-stage training optimization strategy and a large-scale FVR dataset.

Result: State-of-the-art performance in identity fidelity and temporal stability, delivering high-quality, efficient, and identity-consistent face video restoration.

Conclusion: TIGER effectively solves identity shift, viewpoint entanglement, and realism trade-offs through structured tri-prior fusion, setting a new SOTA for FVR.
...
TL;DR: OVBEVSeg is a geometry-aware open-vocabulary BEV segmentation framework that uses Gaussian splatting-based unprojection with 3D geometric constraints to recognize novel categories in autonomous driving, achieving 15.3 mIoU improvement on unseen classes, 2.5x faster inference, and 0.22x memory vs. projection methods.


<details>
  <summary>Details</summary>
Motivation: Existing BEV perception methods are limited to closed-set scenarios and vulnerable to unpredictable real-world environments. The authors aim to achieve open-vocabulary BEV segmentation (OVBS) that can recognize categories beyond the training set while maintaining precise BEV perception and real-time efficiency.

Method: OVBEVSeg uses a geometry-aware framework with Gaussian splatting (GS)-based unprojection and three progressive stages: (1) 2D-to-BEV pseudo-labeling via reliable 3D projection for OV generalization; (2) joint 2D-BEV per-scene optimization with BEV structural constraints for 3D geometric consistency; and (3) 3D geometric distillation for online efficiency.

Result: On nuScenes, OVBEVSeg outperforms closed-set methods by 15.3 mIoU on unseen categories. It competes with self- and semi-supervised baselines trained with up to 40% ground-truth annotations despite having no novel-class labels. Achieves 2.5x faster inference with 0.22x memory consumption of projection-based methods.

Conclusion: OVBEVSeg demonstrates that geometry-aware 3D constraints effectively bridge the gap between 2D VLM semantics and BEV perception for open-vocabulary segmentation, achieving strong performance and efficiency. This work opens new possibilities for open-world autonomous driving perception.
...
TL;DR: SignNet-1M is a large-scale augmented sign language dataset spanning ASL, CSL, and DGS, created via 3D Gaussian Splatting, diffusion models, and post-rendering augmentations to improve model robustness under real-world distribution shifts across viewpoint, background, and signer identity.


<details>
  <summary>Details</summary>
Motivation: Existing sign language models trained on datasets captured under constrained conditions (limited viewpoint, background, and signer diversity) lack robustness to real-world distribution shifts.

Method: SignNet-1M synthesizes variations via (i) novel-view rendering (rotation/zoom) using 3DGS, (ii) scene/identity editing via diffusion models, and (iii) post-rendering augmentations emulating capture/compression artifacts.
...
Conclusion: SignNet-1M enhances sign language model robustness through systematic augmentation, providing a benchmark for translation and recognition tasks.
...
TL;DR: This paper proposes that learnable functions in convolutional KANs should be placed in the convolution's structure rather than on each edge, introducing three variants: SV-KAN (shared value function), AG-KAN (content-adaptive Gaussian gate), and RF-KAN (learnable filter shape using Morlet wavelet basis). RF-KAN and SV-KAN achieve top results on CIFAR-10/100 with ~0.4M parameters, outperforming plain convolutions and per-edge KANs at ~1/5 the parameters, with learned shape being the critical component.


<details>
  <summary>Details</summary>
Motivation: Existing convolutional KANs attach learnable functions to every kernel entry, which is parameter-heavy and prone to overfitting. The authors argue that learnable functions are better placed in the convolution's structure rather than on each edge.

Method: The authors organize the design space along whether the function acts on pixel values or filter shape. They propose three realizations: SV-KAN (shared univariate function on values, static spatial filter), AG-KAN (shared value function with content-adaptive Gaussian gate for spatial structure), and RF-KAN (learnable functions on filter shape using oriented ridge profiles expanded in localized Morlet wavelet basis with content-adaptive amplitudes).

Result: Under a matched four-layer protocol, RF-KAN and SV-KAN achieve 88.47±0.10% and 88.20±0.31% on CIFAR-10, and 64.40±0.19% and 64.57±0.30% on CIFAR-100, with ~0.4M parameters. They outperform plain convolution and per-edge KAN variants at roughly 1/5 the parameters. Ablation removing learned shape collapses accuracy by over 40 points.

Conclusion: At the matched scale, the shape model (RF-KAN) and simplest value model (SV-KAN) perform comparably at the top. The learned shape is identified as the load-bearing ingredient, with gains attributed to the intrinsically localized oscillatory basis and content adaptivity.
...
TL;DR: MATCH is a new multi-view anomaly detection method using Flow Matching that achieves state-of-the-art performance on anomaly detection and segmentation across multiple datasets while running on consumer hardware in real-time.


<details>
  <summary>Details</summary>
Motivation: Complex industrial objects require analysis from multiple viewpoints for effective anomaly detection, motivating the need for a multi-view approach that is both accurate and computationally efficient for real-time production scenarios.

Method: MATCH uses Flow Matching with an ODE formulation to estimate likelihoods for anomaly scoring. It transforms features of varying spatial sizes to a normal distribution through FM's architectural flexibility, and omits the costly divergence term for real-time usability.

Result: Achieves state-of-the-art performance on Real-IAD dataset and provides the first comprehensive evaluation on MANTA-Tiny dataset. Runs on consumer-level hardware in real-time production scenarios.

Conclusion: Flow Matching is an effective and efficient foundation for multi-view anomaly detection, enabling real-time deployment on consumer hardware without sacrificing accuracy.
...
TL;DR: The paper proposes a post-hoc hybrid detector for multi-modal out-of-distribution detection that leverages relationships between multi-modal and uni-modal predictions, combined with feature-space scores normalized by multi-modal logits, consistently outperforming state-of-the-art methods on MultiOOD benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: While multi-modal action recognition models benefit from additional modalities, their robustness for out-of-distribution detection remains underexplored. Existing methods apply off-the-shelf uni-modal OOD detectors during inference, discarding important multi-modal information.

Method: The authors discover a relationship between multi-modal and uni-modal predictions, then build a post-hoc detector that combines this signal with a feature-space score (detecting off-manifold samples in multi-modal space) normalized by multi-modal logits.

Result: Experiments on the MultiOOD benchmark show the proposed hybrid detector consistently outperforms state-of-the-art methods across a wide range of established datasets.

Conclusion: Explicitly considering different modalities at inference time is crucial for effective multi-modal OOD detection, and the proposed approach is compatible with existing training-time methods.
...
TL;DR: EgoSAT is a comprehensive benchmark for egocentric video reasoning in streaming settings, evaluating VLMs on retrospective, online, and prospective reasoning tasks across 1,997 videos and 4,800 QA pairs, revealing that existing models struggle with temporal reasoning and exhibit severe confidence miscalibration.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to evaluate VLMs on streaming egocentric video understanding where frames arrive sequentially and models must reason about past, present, and future under the constraint of only previously observed frames, while also lacking assessment of model confidence calibration.

Method: EgoSAT unifies retrospective (past events), online (ongoing activities), and prospective (future actions) reasoning tasks into a single streaming framework with 1,997 unique egocentric videos (165 hours) and ~4,800 QA pairs. Evaluation distinguishes answerability and performs diagnostic analysis on model confidence.

Result: Existing open-weight and closed-weight VLMs struggle with both prospective and retrospective modeling, and exhibit severe miscalibration where confidence fails to track inherent answerability, leading to "confidently wrong" behaviors.

Conclusion: VLMs are insufficient for streaming egocentric video reasoning, particularly in temporal understanding and confidence calibration, highlighting the need for better models that can reliably track answerability over time.
...
TL;DR: This paper evaluates how image transformations affect latent space embeddings from different encoder networks (Lunit, Bioptimus, Meta) in histopathology. It finds transformed embeddings are closer to originals than random embeddings, indicating partial robustness but not full invariance, explaining why data augmentation improves performance. Significant differences exist between general and histopathology-specific encoders.


<details>
  <summary>Details</summary>
Motivation: To understand whether encoder networks (general vs. histopathology-specific) achieve invariance to label-irrelevant image transformations, and to explain why transformation-based data augmentation remains beneficial for downstream histopathology classification tasks.

Method: The authors used encoder networks from Lunit Inc., Bioptimus, and Meta Research Team. They applied standard image transformations to hematoxylin/eosin-stained tiles from a colorectal tissue dataset and TCGA. They compared embedding distances between original and transformed images, and contrasted them with random unrelated embeddings to quantify invariance.

Result: Original and transformed image embeddings are closer to each other than to random embeddings, showing robustness but not full invariance. Significant differences were observed between general-purpose (Meta) and histopathology-specific (Lunit, Bioptimus) encoder networks.

Conclusion: Encoder networks partially neutralize transformation effects but do not achieve full invariance, which explains why transformation-based augmentation continues to improve model performance. Histopathology-specific encoders behave differently from general ones, highlighting the need to consider encoder choice in downstream tasks.
...
TL;DR: PCFM is a flow matching method for medical point cloud completion using a PTv3 backbone, achieving state-of-the-art generative performance with fewer sampling steps than diffusion models and up to 7× speed-up over PVCNN backbones.


<details>
  <summary>Details</summary>
Motivation: Generative modeling for medical point cloud completion is insufficiently studied, yet it is important for anatomical reconstruction and downstream clinical workflows.

Method: The authors propose PCFM, a continuous-time generative flow matching approach backed by PTv3 (Point Transformer v3), and compare it against deterministic PTv3 baselines and diffusion-based completion (PCDiff) denoised by both PVCNN and PTv3.

Result: PCFM with PTv3 is competitive with deterministic PTv3, achieves state-of-the-art generative performance across SkullFix, SkullBreak, and Mandibular Defect datasets, requires fewer sampling steps than diffusion, and delivers up to 7× throughput speed-up over PVCNN backbones.

Conclusion: Flow matching is an efficient and effective generative paradigm for medical point cloud completion, with consistent scaling benefits from higher point resolution and informative trade-offs across model sizes.
...
TL;DR: S1-Omni-Image is an open-weight multimodal model that unifies scientific image understanding, generation, and editing using a think-before-generate paradigm, achieving SOTA results on multiple benchmarks while preserving strong understanding capabilities.


<details>
  <summary>Details</summary>
Motivation: Scientific image tasks require high-fidelity synthesis plus robust understanding of scientific semantics, structural relations, domain knowledge, and task intent, which general-purpose image generation models fail to address.

Method: Builds on S1-VL-32B backbone; uses a think-before-generate paradigm where the model first produces a reasoning trace, textual answer, and task special token, whose hidden states condition the generation module.

Result: Outperforms open-source models on GenExam and TechImage-Bench; achieves SOTA on four editing benchmarks (MSD, cigRockSEM, SynthRAD2025, IXI); maintains stable scientific image understanding performance.

Conclusion: S1-Omni-Image substantially improves scientific image generation and editing while preserving understanding capabilities, validated by a newly constructed SciGenEdit-314K dataset.
...
TL;DR: P-MTP is a framework that uses Progressive Multi-Token Prediction with adaptive curriculum learning and confidence-gated dynamic drafting to achieve up to 5× inference speedup in VLM-based document parsing while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: VLMs for document parsing suffer from latency bottlenecks due to token-dense documents. Existing MTP approaches face optimization instability when scaling to deeper look-ahead depths, limiting their acceleration potential.

Method: Two key components: (1) Progressive Curriculum Loss that adaptively re-weights look-ahead depths using cumulative path reliability and retrospective target consistency, enabling easy-to-hard optimization; (2) Confidence-Gated Dynamic Drafting that adaptively calibrates speculative length during inference to maximize acceptance rate and minimize waste.

Result: Achieves up to 5× speedup with negligible accuracy loss across multiple benchmarks and architectures, providing the first successful validation of extensive look-ahead MTP in document parsing.

Conclusion: P-MTP successfully scales look-ahead depth for multi-token prediction in document parsing VLMs, offering significant inference acceleration while preserving output quality.
...
### [74] [EERLoss: A Novel Loss Function for Training Deep Biometric Models. A Case Study in Keystroke Dynamics](https://arxiv.org/abs/2606.24586)
*Nahuel Gonzalez,Marta Robledo-Moreno,Ivan DeAndres-Tame,Ruben Vera-Rodriguez,Ruben Tolosana*

Main category: cs.CV

TL;DR: This paper proposes EERLoss, a differentiable approximation of Equal Error Rate (EER) for direct optimization in deep biometric verification, achieving ~30% relative EER reduction on keystroke dynamics verification using the large-scale KVC-onGoing benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning biometric verification methods optimize indirect objectives that misalign with the primary evaluation metric (EER), creating a gap between training optimization and actual performance assessment.

Method: EERLoss is a subdifferentiable, arbitrarily accurate approximation to EER that can also be adapted to optimize specific operating points on the DET curve, enabling task-aligned training for biometric models.

Result: EERLoss outperforms existing state-of-the-art loss functions, converges substantially faster, and achieves ~30% relative EER reduction when applied to the KVC-winning architecture on the large-scale KVC-onGoing benchmark (185K+ subjects).

Conclusion: EERLoss is an effective task-aligned training objective particularly suited for high-variance biometric traits, validated on challenging keystroke dynamics verification.
...
### [75] [SENTRY: SAM2-Enhanced Neighbor-Aware and Temporally Reasoned Memory for Visual Tracking](https://arxiv.org/abs/2606.24449)
*Mohamad Alansari,Yonathan Michael,Hasan AlMarzouqi,Muzammal Naseer,Naoufel Werghi,Sajid Javed*

Main category: cs.CV

TL;DR: SENTRY is a training-free, plug-and-play module that improves SAM2-based visual object tracking by replacing confidence-only mask selection with consistency-validated memory updates, achieving new SOTA on multiple benchmarks at 32.8 FPS.


<details>
  <summary>Details</summary>
Motivation: The authors identify that confidence-only mask selection in SAM2-based trackers causes drift under occlusion, rapid motion, and distractors. They aim to fix this without retraining or modifying the base architecture.

Method: SENTRY is a refine-before-write module that: (1) aggregates diverse segmentation hypotheses per frame, (2) backtracks them into short tracklets, and (3) uses neighbor-aware cycle-consistent matching against recent trajectories to select temporally and geometrically consistent masks before memory updates.

Result: SENTRY achieves new zero-shot SOTA on LaSOT, LaSOT_ext, GOT-10k, VOT20, VOT22, and DiDi, runs at 32.8 FPS on A100, and adds only 0.4-0.6 GB VRAM. It delivers consistent gains across five strong baselines and nine benchmarks.

Conclusion: Enforcing temporal validity at write time stabilizes memory-augmented tracking without retraining. The paper also provides the first unified all-scale evaluation of SAM2-based trackers.
...
### [76] [Lite Any Stereo V2: Faster and Stronger Efficient Zero-Shot Stereo Matching](https://arxiv.org/abs/2606.24457)
...
TL;DR: LAS2 is an ultra-fast, efficient stereo matching model series that achieves strong zero-shot generalization through a 2D-only cost aggregation framework and a three-stage training strategy (synthetic supervision, self-distillation, real-world knowledge distillation), outperforming prior efficient methods in both accuracy and latency.


<details>
  <summary>Details</summary>
Motivation: Existing stereo matching models either achieve high accuracy with heavy computation or offer fast inference but poor zero-shot generalization. The authors aim to bridge this gap by designing an efficient model that can generalize well to unseen domains without sacrificing speed.

Method: 1) A 2D-only cost aggregation framework optimized for real inference latency. 2) A three-stage training strategy: synthetic supervision → self-distillation → real-world knowledge distillation. 3) Pseudo-label filtering and error-clamping to improve synthetic-to-real transfer.

Result: LAS2 achieves state-of-the-art accuracy among efficient stereo methods with significantly lower latency. LAS2-H outperforms Fast-FoundationStereo in zero-shot performance while being 1.8x faster on H200 and 2.7x faster on Orin.

Conclusion: Efficient stereo models can achieve strong zero-shot generalization without relying on large models or foundation-model priors, challenging the common assumption that efficiency comes at the cost of generalization ability.
...
### [77] [Boosting Text-Driven Video Segmentation via Geometry-Aware Distillation](https://arxiv.org/abs/2606.24464)
*Tianyu Zhu,Yingping Liang,Hesong Li,Ying Fu*

Main category: cs.CV

TL;DR: GeoLaV is a two-stage framework that enhances Referring Video Object Segmentation by distilling 3D geometric knowledge from images, achieving state-of-the-art performance through monocular geometry pretraining and geometry-aware distillation.


<details>
  <summary>Details</summary>
Motivation: Existing RVOS models are trained on 2D data with naive segmentation losses, overlooking geometric consistency across frames and leading to weak spatial understanding.

Method: A two-stage framework: (1) monocular geometry pretraining with novel-view synthesis for geometry-consistent visual representations, (2) geometry-aware distillation fine-tuning using a 3D prior model to transfer structural knowledge.

Result: Using only image segmentation data provides notable zero-shot generalization in RVOS; with video fine-tuning, achieves state-of-the-art across multiple RVOS benchmarks.

Conclusion: Incorporating 3D geometric knowledge into language-guided video segmentation significantly improves spatiotemporal coherence and language grounding, outperforming prior methods.
...
### [78] [video-SALMONN-R$^3$: Learning to ReWatch, ReAsk, and ReAnswer for Efficient Video Understanding](https://arxiv.org/abs/2606.24477)
*Yixuan Li,Guangzhi Sun,Yudong Yang,Wei Li,Zejun MA,Chao Zhang*

Main category: cs.CV

TL;DR: video-SALMONN-R³ is an end-to-end video-LLM that uses reinforcement learning to enable re-watching of video segments at higher fidelity, overcoming memory/computation limits while avoiding costly chain-of-thought data annotations.


<details>
  <summary>Details</summary>
Motivation: Video LLMs are constrained by computation and memory budgets, forcing reduced frame rates and spatial resolutions that cause them to miss critical information for question answering.

Method: Proposes a re-watch paradigm via reinforcement learning (no CoT cold-start), a re-answer strategy (direct answer first, refine after re-watch), and a re-ask mechanism (re-inject query when revisiting segments) to improve question adherence.

Result: Consistently outperforms both the base model and QA-SFT baseline, surpasses prior re-watch-based approaches with significantly lower computational cost.

Conclusion: video-SALMONN-R³ offers an efficient and effective solution for video QA by enabling selective re-watching through RL, without costly CoT annotations or degraded pretrained abilities.
...
### [79] [MambaRaw: Selective State Space Modeling for Efficient 4K Raw Image Reconstruction](https://arxiv.org/abs/2606.24479)
*Peize Li,Fanhu Zeng,Tongda Xu,Xingguo Xu,Xinjie Zhang,Xingtong Ge,Haotian Zhang,Yan Wang*

Main category: cs.CV

TL;DR: MambaRaw is a JPEG-conditioned raw image reconstruction framework using State Space Models (SSMs) to efficiently estimate entropy parameters, with a Spatial-Energy Coupled Context Modeling mechanism (TileMambaBlock + Energy-Aware Refinement) that achieves SOTA results on Sony, Olympus, and Samsung datasets while reducing latency by ~9%.


<details>
  <summary>Details</summary>
Motivation: Existing metadata-based raw reconstruction frameworks using attention mechanisms scale quadratically with feature maps, becoming computationally expensive for high-resolution (e.g., 4K) raw images, hindering practical deployment.

Method: MambaRaw uses State Space Models (SSMs) for efficient entropy parameter estimation, with two key modules: (1) TileMambaBlock - performs Mamba-style selective scanning only on information-dense tiles; (2) Energy-Aware Refinement (EAR) - an identity-initialized residual module that enhances feature representation to match the long-tail energy distribution of raw signals.

Result: Outperforms strong metadata-based baselines on Sony, Olympus, Samsung datasets. At low metadata bitrates, achieves 1.2-1.4 dB PSNR improvement and ~9% reduction in end-to-end coding latency. Sets new SOTA for JPEG-guided raw reconstruction.

Conclusion: MambaRaw demonstrates that SSM-based context modeling can replace quadratic-complexity attention mechanisms for efficient, high-quality raw image reconstruction from JPEG previews, offering practical benefits for real-world imaging pipelines.
...
### [80] [Advancing WordArt-Oriented Scene Text Recognition: Datasets and Methods](https://arxiv.org/abs/2606.24484)
*Xingsong Ye,Yongkun Du,Jiaxin Zhang,Haojie Zhang,Chong Sun,Chen Li,Jing Lyu,Zhineng Chen*

Main category: cs.CV

TL;DR: This paper tackles WordArt (artistic text) recognition by constructing a 2M synthetic dataset WATER-S and proposing WATERec, a model with arbitrary-shaped visual encoder and autoregressive decoder, achieving 90.40% accuracy on WordArt-Bench, significantly outperforming existing STR and VLM methods.


<details>
  <summary>Details</summary>
Motivation: Existing STR datasets and methods are built around regular scene text and struggle with highly customized artistic text (WordArt). There is a lack of large-scale, diverse, and controllable WordArt data, and current fixed-template models cannot handle the complex layouts of WordArt.

Method: (1) Data: Constructed WATER-S (2M samples) with two subsets — SynthWordArt (upgraded rendering pipeline for accurate synthetic data) and Qwen3-VL + Z-Image pipeline (for realistic/diverse image synthesis). Also reorganized real STR data into WATER-R. (2) Model: WATERec uses an arbitrary-shaped visual encoder + autoregressive decoder to break fixed-template bottlenecks.

Result: WATERec achieves 90.40% accuracy on WordArt-Bench, surpassing both prior STR methods and OCR-specialized/general-purpose vision-language models by a large margin.

Conclusion: The combination of large-scale high-quality synthetic data (WATER-S) and a flexible architecture (WATERec) effectively advances the WordArt-oriented scene text recognition task, setting a new strong baseline.
...
### [81] [RetiSEM: Generalising Causal Models for Fragmented Biomedical Data](https://arxiv.org/abs/2606.24488)
*Inam Ullah,Imran Razzak,Shoaib Jameel*

Main category: cs.CV

TL;DR: RetiSEM is a domain-constrained structural equation modeling framework for causal graph recovery and mediation analysis from fragmented multimodal biomedical data, outperforming unconstrained baselines on synthetic and real-world benchmarks.


<details>
  <summary>Details</summary>
Motivation: Learning causal models from fragmented biomedical data is challenging because clinical, molecular, and imaging variables are often incomplete or not jointly observed, limiting the ability to perform reliable causal inference under limited multimodal resources.

Method: RetiSEM organizes variables into biologically informed blocks, applies forbidden-edge constraints to enforce domain knowledge, and decomposes pathway-level effects into total effect (TE), natural direct effect (NDE), and natural indirect effect (NIE) for mediation analysis.

Result: Achieves lower structural error and higher causal accuracy than unconstrained baselines across ten synthetic benchmarks. In real-data analysis combining NHANES clinical variables with external retinal representations, retinal variables behave mainly as downstream biomarkers with small but detectable indirect effects.

Conclusion: The domain-constrained SEM strategy provides an interpretable framework for testing structured causal hypotheses in limited-resource biomedical AI settings.
...
TL;DR: VistaRef enhances Transformer-based visual models for deictic gesture grounding by introducing Local Hand Entity Modeling, Geometric Ray Modeling, and Orientation-Consistent Alignment Loss, achieving a 14-point absolute gain in grounding accuracy.


<details>
  <summary>Details</summary>
Motivation: Transformer-based visual models with global attention mechanisms neglect micro-geometric relationships, degrading orientation accuracy in pointing tasks - causing pointing drift and localization ambiguity for distant or densely packed objects.

Method: 1) Local Hand Entity Modeling (LHEM) with hand-pose embeddings for capturing subtle finger deviations; 2) Geometric Ray Modeling (GRM) transforming implicit orientation into explicit spatial features via multi-view geometry; 3) Orientation-Consistent Alignment Loss (OCAL) for joint supervision of hand presence and pointing consistency.

Result: 14-point absolute gain in grounding accuracy over baseline; qualitative results confirm effective modeling of hand-to-target geometric correlations.

Conclusion: VistaRef bridges the spatial perception gap in traditional Transformers for complex grounding scenarios, advancing AR and human-robot collaboration.
...
### [83] [GeoIMO: Geometry-Driven Independent Motion Classification for Event Cameras](https://arxiv.org/abs/2606.24499)
*Anil Bayram Gogebakan,Filippo Marostica,Alessio Caviglia,Alessandro Savino,Stefano Di Carlo*

Main category: cs.CV

TL;DR: A geometry-driven, annotation-free framework that classifies objects as static or moving by analyzing ego-motion directly from event streams, using Focus of Expansion with yaw compensation and residual-based motion deviation detection.


<details>
  <summary>Details</summary>
Motivation: Existing event datasets use appearance-based annotations from frame pipelines, which are poorly suited for motion-aware event perception, creating a need for a method that leverages the temporal nature of event data.

Method: A Focus of Expansion model with yaw compensation estimates global background motion; objects are labeled as moving when local motion deviates from this prediction via a scale-invariant residual. Temporal stabilization is applied across consecutive event windows.

Result: Consistent performance on MVSEC and Prophesee 1 Megapixel Automotive Detection datasets across diverse driving scenarios; yaw compensation improves results during turns.

Conclusion: The method is learning-free, requires no manual motion labels, works with any input bounding boxes, and offers a favorable accuracy-efficiency trade-off with a simple translational local model.
...
### [84] [What Do Flow-Based Inverse Solvers Approximate? A Posterior-Transport View](https://arxiv.org/abs/2606.24516)
*Jian Xu,Delu Zeng,John Paisley,Qibin Zhao*

Main category: cs.CV

TL;DR: This paper provides a theoretical framework showing that flow-based inverse problem solvers using trajectory guidance approximate posterior sampling poorly, while a simple source reweighting method yields exact posterior samples; they propose a principled velocity-correction solver that outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing training-free solvers repurpose pretrained flow-matching priors for inverse problems by adding measurement-guidance terms, but what these corrections actually approximate and how far they deviate from the true posterior has not been characterized.

Method: The authors derive that for deterministic flow priors, Bayesian conditioning equals source distribution reweighting (not drift correction); show existing solvers correspond to approximations of the true correction field; propose a principled velocity-correction solver.

Result: In controlled 2D experiments, source reweighting matches true posterior exactly while trajectory guidance incurs 200-800x larger error and collapses posterior modes; proposed solver is competitive across in-domain and out-of-distribution settings with diverse posterior samples.

Conclusion: Source reweighting is theoretically and empirically superior to trajectory-guidance solvers; the proposed velocity-correction solver offers a principled alternative that produces uncertainty-aware posterior samples.
...
TL;DR: VisCritic is a visual process reward framework that uses Siamese vision transformers to compare pre/post-action screenshots, providing step-level verification for GUI agents without requiring additional human labels.


<details>
  <summary>Details</summary>
Motivation: GUI agents fail in long-horizon tasks due to lack of step-level verification; existing process reward models rely only on textual reasoning and miss visual GUI state changes.

Method: (1) Siamese vision transformer for change-aware visual feature extraction from pre/post-action screenshots; (2) Action-Aware Critic Head evaluating action success, task progress, and error type; (3) Weakly supervised data pipeline from existing trajectories without extra human labels.

Result: Across five benchmarks, VisCritic improves metrics as a plug-and-play enhancement for diverse GUI agents and provides visual diagnostic cues.

Conclusion: Visual process reward modeling (via direct screenshot comparison) is more effective than text-only verification for GUI agents; weakly supervised critic training is feasible and generalizable.
...
TL;DR: ForensicsTok reformulates image manipulation localization as autoregressive sequence generation, using a Token Splatting Decoder and Hierarchical Expert Fusion to improve MLLM-based forensic detection without relying on separate segmentation decoders.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based forensic approaches use stitched segmentation decoders that create information bottlenecks, dilute spatial signals during backpropagation, and are limited by semantic priors of the segmentor, leading to suboptimal localization.

Method: (1) Reformulates image manipulation localization as autoregressive sequence generation for direct mask prediction. (2) Token Splatting Decoder (TSD) maps tokens to binary masks via codebook-aware code smoothing. (3) Hierarchical Expert Fusion (HEF) injects multi-scale features from a forensic expert model.

Result: Substantially improves over existing MLLM-based baselines and slightly improves over strong forensic expert baselines across six benchmarks, with stronger robustness to perturbations.

Conclusion: The unified architecture of ForensicsTok effectively compensates for the lack of forensic priors in standard MLLMs and achieves superior localization and robustness compared to prior methods.
...
TL;DR: PointVG-R is a Multi-modal Large Language Model that enhances pointing-based visual grounding by introducing geometric-aware reasoning, reinforcement learning, and a novel Chain-of-Thought dataset (EgoPoint-CoT), achieving 15.86 points improvement in mIoU over baselines.


<details>
  <summary>Details</summary>
Motivation: Traditional pointing-based visual grounding methods encode images into static features and reason primarily in the linguistic domain, overlooking rich perceptual cues and explicit spatial geometry in images. This causes cognitive vulnerability in interpreting gestural spatial relations.

Method: (1) Geometric reasoning pipeline simulating human iterative cognitive process for pointing gestures. (2) EgoPoint-CoT dataset with detailed reasoning trajectories for SFT and RL training. (3) Adaptive Importance Weighting based on Group Variance to dynamically adjust reward signals.

Result: PointVG-R achieves SOTA performance, outperforming baseline by 15.86 points in mIoU. Ablation studies validate the efficacy of each proposed module.

Conclusion: Integrating geometric-aware reasoning with RL and cold-start data effectively mitigates cognitive vulnerability in pointing-based visual grounding, as evidenced by the substantial performance gains.
...
TL;DR: This paper introduces CF-World, a benchmark to test whether text-to-image models can generate counterfactual images that contradict real-world priors. Results show all models fail significantly, revealing they rely on frequent visual co-occurrences rather than genuine causal understanding.


<details>
  <summary>Details</summary>
Motivation: To investigate whether T2I models possess genuine causal understanding or merely engage in sophisticated pattern matching by testing their ability to generate images under rules that systematically contradict real-world priors, inspired by Russell's inductivist turkey paradox.

Method: CF-World organizes scenarios into three levels: factual (ordinary world knowledge), explicit counterfactual (direct visual instructions), and implicit counterfactual (causal deduction from altered rules). They evaluate open-source and closed-source T2I models using a VLM-based evaluator (CF-Eval) with two metrics: Prior Resistance Rate (PRR) and Reasoning Retention Rate (RRR).

Result: All models exhibit sharp degradation from factual to counterfactual settings. Models fail to overcome entrenched real-world priors and struggle to maintain reasoning-dependent counterfactual generation without explicit visual cues.

Conclusion: Current T2I models encode world knowledge and visual appearances as tightly coupled patterns, leading to heavy reliance on frequent visual co-occurrences in training data, which forces them to default to familiar commonsense priors when generating counterfactual worlds.
...
TL;DR: SPOFA stabilizes heterogeneous knowledge distillation by decoupling feature norms from directions via LayerNorm and adaptively penalizing gradient conflicts via momentum-driven MEMA dynamic scaling, achieving SOTA accuracy with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Heterogeneous Knowledge Distillation (HKD) suffers from severe training instability due to two coupled challenges: massive feature norm discrepancies and severe gradient conflicts between primary and distillation objectives.

Method: Proposes SPOFA with two core components: (1) a LayerNorm-based decoupling projector that decouples feature magnitude from direction for stable semantic alignment; (2) a momentum-driven Exponential Moving Average (MEMA) dynamic scaler that evaluates gradient conflicts and adaptively penalizes harmful distillation signals.
...
Conclusion: Feature and gradient dual stabilization is effective and lightweight for addressing HKD instability; SPOFA enables stable cross-architecture knowledge transfer.
...
### [90] [Quantum CT via Dynamic Interval Encoding and Prior-Balanced QUBO Reconstruction](https://arxiv.org/abs/2606.24561)
*Ao Wang,Yikuang Yuluo,Yujie Liu,Shuangyang Zhong,Yuwen Zhang,Zihao Wang,Fenglin Liu,Andreas Maier,Haijun Yu,Yixing Huang*

Main category: cs.CV

TL;DR: A QUBO-based grayscale CT reconstruction framework using dynamic interval encoding and prior-balanced optimization that outperforms traditional methods and runs on hybrid quantum-classical hardware.


<details>
  <summary>Details</summary>
Motivation: Existing QUBO-based quantum CT is limited to binary images; grayscale encoding suffers from either excessive QUBO size with high bit-depth or quantization errors with low bit-depth.

Method: Dynamic interval encoding (refining only active pixels within local gray-level intervals per round) + boundary-hit-guided update rule + prior-balanced optimization (projection data consistency + edge-preserving quadratic prior).

Result: Outperforms analytic, iterative, variational, and representation-based baselines in sparse-view and limited-angle fan-beam CT; validated on D-Wave hybrid BQM solver.

Conclusion: Dynamic local encoding and stable data-fidelity-prior coupling are key to improvement; the framework is executable on real quantum-classical hybrid backends.
...
### [91] [PatternGSL: A Structured Specification Language for Template-Free and Simulation-Ready 3D Garments](https://arxiv.org/abs/2606.24564)
*Zhenyang Li,Lutao Jiang,Yizhou Zhao,Ying-Cong Chen,Xin Wang,Weikai Chen,Yifan Peng*

Main category: cs.CV

TL;DR: PatternGSL introduces a template-free, learnable specification language (GSL) that encodes complete sewing patterns from a single image, enabling simulation-ready garment reconstruction with explicit sewing structure, paired with a large-scale dataset (PatternGSLData) and a vision-language framework for direct prediction without optimization.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to bridge the gap between geometric reconstruction (no sewing structure) and programmatic systems (template-dependent). PatternGSL aims to provide a structured, template-free representation that is both simulation-ready and learnable from a single image.

Method: PatternGSL proposes (1) a template-free specification language encoding panel boundaries, parameterized seams, and stitch topology; (2) a vision-language framework predicting GSL from a single image; (3) PatternGSLData, a 300K image-GSL paired dataset for supervised VLM training; and (4) lightweight deterministic validity handling for decoding.

Result: Improved pattern accuracy over baselines, explicit sewing-structure recovery, reliable cloth simulation, and pattern-level editing via the same deterministic pipeline.

Conclusion: PatternGSL demonstrates that structured, simulation-ready garment reconstruction from a single image is achievable without template constraints or optimization-based refinement, offering a new paradigm for generative modeling of garments.
...
### [92] [Multilevel Stochastic Plug-and-Play for Sparse-View CT Reconstruction](https://arxiv.org/abs/2606.24567)
*Antoine De Paepe,Alexandre Bousse,Dimitris Visvikis*

Main category: cs.CV

TL;DR: ML-SPnP accelerates stochastic PnP reconstruction for sparse-view CT by using multiresolution analysis approximation spaces, which avoids costly fine-level prior gradient estimations and reduces runtime while maintaining reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Sparse-view CT reconstruction is ill-posed and prone to artifacts. Stochastic PnP methods improve robustness but require many iterations, limiting practical efficiency. The authors aim to accelerate these methods without sacrificing quality.

Method: The authors propose a multilevel stochastic PnP framework that operates in multiresolution analysis (MRA) approximation spaces. By leveraging wavelet decomposition structure, the prior-coherence correction vanishes in expectation, eliminating the need for costly fine-level stochastic prior gradient estimation in coarse-level corrections.

Result: On sparse-view CT reconstruction tasks, ML-SPnP achieves reconstruction quality comparable to state-of-the-art methods while substantially reducing runtime.

Conclusion: The multilevel approach in MRA spaces effectively accelerates stochastic PnP reconstruction for SVCT, offering a practical efficiency gain without compromising quality.
...
TL;DR: ConQuer improves CLIP-style 3D medical foundation models by learning multiple concept-specific cross-attention queries from radiology reports, enabling localized image-text alignment without segmentation masks, resulting in the state-of-the-art Jolia model for chest/abdominal CT.


<details>
  <summary>Details</summary>
Motivation: CLIP-style pretraining compresses entire medical images and long, structured radiology reports into single global tokens, losing important details across dozens of organs.

Method: ConQuer splits reports into concept-specific sections and learns cross-attention queries to pool matching image features per concept (e.g., organ/region), applying contrastive learning independently for each concept without spatial supervision.

Result: Jolia (trained with ConQuer on chest/abdominal CT) consistently outperforms CLIP baseline on findings classification, report generation, and cross-center transfer, achieving state-of-the-art across multiple public benchmarks.

Conclusion: Localized concept-level alignment via cross-attention queries is a effective alternative to global CLIP-style alignment for 3D medical vision-language pretraining, with built-in interpretability.
...
TL;DR: ViTexQA (a video-text QA dataset requiring cross-frame reasoning) and FrameThinker (a two-stage training framework with CoT-guided SFT and temporally-grounded RL) are proposed to address the limitation that current MLLMs can answer most video questions from single frames, achieving 6.3% ROUGE-L improvement over SOTA.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with video text understanding that requires integrating temporally distributed textual cues across multiple frames. Existing datasets fail because most questions can be answered from single frames, not reflecting real-world video text comprehension.

Method: (1) ViTexQA: a large-scale video-text QA dataset built via quality-controlled CoT annotation pipeline with temporal constraints, ensuring all QA pairs require cross-frame text fusion. (2) FrameThinker: two-stage training — CoT-Guided SFT generates frame-aware reasoning chains, then Temporally-grounded RL optimizes with multi-frame coherence rewards.

Result: FrameThinker outperforms SOTA baselines on ViTexQA, lifting ROUGE-L by 6.3%.

Conclusion: Explicit temporal modeling via CoT-guided SFT and temporally-grounded RL is effective for multi-frame video text reasoning, and temporally-constrained datasets are crucial for advancing multimodal video understanding.
...
TL;DR: A collaborative multi-agent framework (Planning Agent + Perception Agent) for zero-shot 3D understanding that iteratively plans viewpoints and builds a structured cognitive map, achieving SOTA on 6 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based 3D understanding methods are bottlenecked by finite video viewpoints and implicit 3D perception; the paper aims to overcome these limitations through explicit scene representation and dynamic viewpoint planning.

Method: A closed-loop multi-agent system: (1) Planning Agent selects/view supplements query-relevant viewpoints based on cognitive map analysis; (2) Perception Agent extracts object-level attributes with consistent identifiers across views, builds a structured holistic cognitive map, and provides feedback to filter mismatches; iterating until sufficient info is collected.

Result: SOTA results on 6 benchmarks: +11.1% Acc@0.5 on ScanRefer, +14.6 BLEU-1 on 3D-assisted dialog, +2.1 EM on SQA3D.

Conclusion: The collaborative multi-agent paradigm effectively addresses the viewpoint limitation and implicit perception bottlenecks in zero-shot 3D understanding, showing significant improvements across diverse 3D tasks.
...
TL;DR: This paper proposes a human-grounded evaluation framework for sparse autoencoders (SAEs) in vision models, introducing a matching procedure (FBMP) and a perturbation alignment score (TAPAScore) to quantify semantic correspondence between SAE latents and human-annotated concepts, using synthetic benchmarks synCUB and synCOCO.


<details>
  <summary>Details</summary>
Motivation: Existing SAE evaluation methods rely on proxy metrics or qualitative inspection rather than directly measuring semantic correspondence between learned latents and human-interpretable concepts.

Method: The authors construct synCUB and synCOCO synthetic benchmarks, introduce Fully-Binary Matching Pursuit (FBMP) for coalition-based many-to-one matching, and propose a Targeted Attribute Perturbation Alignment Score (TAPAScore) for functional validation via image-level attribute perturbations.

Result: FBMP and TAPAScore reliably distinguish trained SAEs from untrained ones, where other metrics fail. Increased overcompleteness in SAEs can reduce perturbation alignment, indicating reduced interpretability.

Conclusion: Moderate dictionary sizes provide the best interpretability trade-off for SAEs trained on CLIP and DINOv2 embeddings, and the proposed framework enables systematic evaluation of semantic correspondence.
...
TL;DR: This paper proposes Semantic Evidence Reward (SER), a new RL reward for video MLLMs that uses a referee VLM to evaluate spatio-temporal evidence claims based on relevance and localization quality rather than geometry-only IoU, achieving 49.6% mLGM (+3.0) on V-STAR.


<details>
  <summary>Details</summary>
Motivation: Existing video MLLMs struggle with fine-grained spatio-temporal reasoning, often producing correct answers from irrelevant frames/objects. Current RL frameworks rely on geometry-only IoU rewards that are sensitive to boundary perturbations and overlook semantic alignment.

Method: SER reformulates evidence grounding as a constrained verification task: a referee VLM evaluates model-generated evidence claims across relevance and localization quality dimensions, combined with a temporal penalty, replacing pixel-level overlap computation.

Result: On V-STAR benchmark, SER achieves 49.6% mLGM, improving by 3.0 points over the strong baseline Open-o3-Video.

Conclusion: SER demonstrates potential in enhancing both answer accuracy and evidence grounding, reducing reliance on dense box annotations, and enabling training directly on standard video QA data.
...
TL;DR: VSANet introduces a view-aware sparse attention network for light field image denoising, using locality-sensitive hashing to achieve efficient cross-view global feature interactions with linear complexity, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Light field denoising is challenging due to its high-dimensional structure; existing methods struggle to effectively exploit cross-view correlations while handling independent noise across sub-aperture images.

Method: Proposes a view-aware sparse attention (VSA) block that unifies 4D LF features into a spatial-angular token space and uses locality-sensitive hashing for global cross-view aggregation with linear complexity, plus a feature refinement (FR) block emphasizing informative features in spatial, angular, and epipolar subspaces.

Result: VSANet outperforms state-of-the-art LF denoising methods on experimental benchmarks.

Conclusion: The proposed VSA and FR blocks integrated in a sequential attention refinement module effectively leverage LF correlations for superior denoising performance.
...
TL;DR: BioMedVR is the first Visual Reprogramming framework for adapting vision-language models (like CLIP) to biomedical imaging with few-shot learning, introducing a Confusion Minimization Mechanism and Mixture-of-Prompt Experts to handle fine-grained class confusion. It achieves strong results across 18 datasets.


<details>
  <summary>Details</summary>
Motivation: Adapting VLMs (e.g., CLIP) to biomedical imaging is challenging due to high computational cost of full fine-tuning, scarce medical data, and subtle fine-grained inter-class differences. Existing VR methods focus on positive class prompts and overlook confusing negatives, causing miscalibrated predictions.

Method: BioMedVR uses compact learnable VR modules for few-shot adaptation. It introduces: (1) a Confusion Minimization Mechanism with LLM-generated confusion-aware attributes and a Confusion-Suppression Loss to reduce false-positive alignment; (2) a Mixture-of-Prompt Experts with a positive expert for main-class discrimination and a negative expert for confusion suppression, balanced via adaptive gating.

Result: Extensive experiments on 18 datasets (11 biomedical + 7 natural image benchmarks) show BioMedVR achieves superior accuracy and generalization compared to existing methods.

Conclusion: BioMedVR effectively bridges VR and VLMs in biomedical domains, offering a parameter-efficient solution for fine-grained medical image classification with few-shot learning.
...
TL;DR: The paper proposes Adaptive Hebbian Routing for few-shot Vision Transformers, using a lightweight MLP router to dynamically control Hebbian fast-weight memory (contribution, update strength, retention). Experiments across multiple backbones show small accuracy improvements (e.g., 96.74% → 96.94% on Omniglot) and reduced inference time compared to fixed Hebbian memory.


<details>
  <summary>Details</summary>
Motivation: Fixed Hebbian fast-weight memory behavior may not be optimal for every few-shot task, motivating the need for adaptive control over memory contribution, update strength, and retention.

Method: Introduces a lightweight MLP router to dynamically control three aspects of Hebbian memory: (1) contribution to model output, (2) plasticity (update strength), and (3) retention of previous memory. Three variants are studied: Adaptive Placement, Adaptive Plasticity, and Fully Adaptive Hebbian Routing.

Result: On Omniglot (5-way 1-shot, Swin-Tiny): Fixed Hebbian 96.74% → Adaptive Plasticity 96.92% → Fully Adaptive 96.94%. Inference time reduced from 16.51ms to 14.05ms. On CIFAR-FS, adaptive variants improve all three backbones. Gains persist in multi-shot settings.

Conclusion: Adaptive plasticity and adaptive memory activation can improve few-shot Transformer representations beyond fixed Hebbian behavior, achieving both higher accuracy and faster inference.
...
TL;DR: UniDrive is a unified visual-language and grounding framework for autonomous driving that combines temporal reasoning from multi-frame inputs with high-resolution spatial detail from the latest frame via gated cross-attention fusion, outperforming baselines in risk description, object grounding, and interpretability.
...
Method: UniDrive has a temporal reasoning branch (multi-frame video input) and a high-resolution perception branch (latest frame), integrated via a gated cross-attention fusion module, then jointly generates natural-language risk descriptions and grounded bounding-box outputs for risk objects.

Result: Outperforms image-based and video-based baselines on DRAMA-Reasoning benchmark; achieves best validation performance, advantages in small-object localization, zero-shot generalization to NuScenes and BDD100K, and higher human-rated interpretability and trustworthiness.

Conclusion: Explicitly combining temporal semantics with high-resolution perception provides a stronger foundation for interpretable and safety-oriented autonomous driving systems.
...
TL;DR: GradAudit is a gradient-based auditing framework that detects training data in VLLMs by analyzing internal optimization dynamics, outperforming existing methods on medical and general datasets, and revealing that current methods underestimate unauthorized data usage especially in advanced models.


<details>
  <summary>Details</summary>
Motivation: VLLMs trained on massive crawled data raise copyright and data-provenance concerns, especially in healthcare requiring privacy safeguards. Existing training data detection methods fail in cross-modal scenarios or lack sufficient discriminative power.

Method: GradAudit is a gradient-based framework that examines internal optimization dynamics rather than treating VLLMs as black boxes. It leverages the observation that gradients on training samples converge to stable, well-aligned regions, while non-training samples produce noisy, inconsistent gradients.

Result: Across medical and general-domain datasets, GradAudit substantially outperforms state-of-the-art baselines in both pretraining and fine-tuning VLLMs. The case study shows existing methods underestimate unauthorized data usage, with underestimation worsening for more recent/advanced models.

Conclusion: GradAudit effectively detects genuine image-text associations learned during training, not merely individual modality membership, providing stronger privacy auditing for VLLMs.
...
TL;DR: AerialFusionMapNet introduces a structured two-stage training strategy that enhances aerial feature integration for aerial-onboard fusion, achieving 54.7 mAP on nuScenes (+12.1% relative improvement over prior fusion baselines) without increasing architectural complexity.


<details>
  <summary>Details</summary>
Motivation: Prior aerial-onboard fusion methods for HD map construction do not fully exploit structural information contained in aerial representations under conventional end-to-end fusion.
...
Result: AerialFusionMapNet achieves up to 54.7 mAP on nuScenes geographic split, improving over prior aerial-onboard fusion baselines (48.8 mAP) by +5.9 absolute and +12.1% relative.

Conclusion: Structured training design, rather than increased architectural complexity, plays a more decisive role in unlocking the full potential of aerial imagery for online HD map construction.
...
TL;DR: This paper proposes a tree counting method from satellite imagery using Unbalanced Optimal Transport for spatial density matching, with a self-correction mechanism to handle noisy LiDAR-derived supervision. It introduces the TinyTrees benchmark (215M+ annotations across 23,000 sq.km) and outperforms detection, regression, and transport-based baselines.


<details>
  <summary>Details</summary>
Motivation: Individual tree counting from satellite imagery is underexplored due to: (1) ambiguous crown boundaries in dense forests making detection ill-defined, (2) prohibitively expensive manual annotations at scale, and (3) noisy supervision from LiDAR-derived annotations that are difficult to exploit effectively.

Method: Formulates tree counting as a spatial density matching problem supervised via Unbalanced Optimal Transport, which handles both precise localization of isolated trees and robust density estimation in dense forests. A self-correction mechanism uses transport residuals to progressively refine noisy supervision during training.

Result: The TinyTrees benchmark covers 3 continents, 3 satellite sensors, 215M+ tree annotations (773K manually verified), across 23,000 sq.km. The method consistently outperforms detection-based, regression-based, and transport-based distribution-matching baselines.

Conclusion: Unbalanced Optimal Transport with reliability-aware self-correction is effective for large-scale tree counting from satellite imagery, and the benchmark/resources are publicly released.
...
TL;DR: Pocket-SLAM proposes a rendering-area-aware pruning strategy to reduce memory consumption in 3DGS-SLAM for large-scale scenes like autonomous driving, achieving 60% memory reduction and 2x FPS improvement while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: 3DGS-SLAM faces critical memory consumption issues in large-scale scenes because Gaussian points accumulate continuously, leading to poor memory efficiency that limits applicability in autonomous driving.

Method: A rendering-area-aware pruning strategy that selectively removes Gaussians based on their contribution to the effective rendering area, rather than relying solely on opacity or gradient magnitude heuristics.

Result: Outperforms existing pruning approaches on EuRoC and KITTI datasets, achieving over 60% memory reduction and more than 2x FPS improvement while preserving localization and mapping accuracy.
...
TL;DR: This paper introduces EG-VQA, a benchmark that evaluates VideoQA models not just on answer correctness but also on grounding predictions in relevant video evidence. The authors propose EG-F1 metric and EG-Reasoner model, showing explicit evidence supervision improves reliability, especially on reasoning-intensive tasks, and that scaling alone is insufficient for robust video understanding.


<details>
  <summary>Details</summary>
Motivation: Existing Video-LLM benchmarks only evaluate answer correctness, ignoring whether predictions are grounded in relevant video evidence. This disconnect between answer generation and evidence understanding motivates building a benchmark that jointly evaluates reasoning and precise evidence localization.

Method: The authors construct EG-VQA with 2,067 videos and 11,838 QA pairs annotated with supporting temporal evidence. They introduce EG-F1 metric measuring temporal alignment and semantic consistency. They also propose EG-Reasoner, an evidence-grounded reasoning model trained with explicit supervision.

Result: Even strong proprietary models struggle to accurately ground their predictions. EG-Reasoner achieves state-of-the-art among open-source models and competitive results against proprietary systems, with pronounced gains on reasoning-intensive tasks like counterfactual questions.
...
TL;DR: OrbitForge converts text-generated videos into 3D Gaussian Splatting scenes by using frozen video priors and per-prompt reconstruction optimization, achieving full 360° orbit coverage without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Text-to-video models offer rich scene priors but fail to produce reliable 3D assets due to uncontrolled camera motion, partial view coverage, and temporal inconsistencies.

Method: 1) Generate initial video, 2) Reconstruct 3D via Deformable Gaussian Splatting with MedianGS proxy, 3) Detect missing viewpoints from prescribed orbit, 4) Use text-to-video model to complete only missing views, 5) Reconstruct into final Gaussian Splatting scene.

Result: Achieves 359.0° median span (near full orbit), improves Q10 ImageReward from 8.07 to 16.36 vs MedianGS-only, and remains competitive with VideoMV on coverage-quality tradeoff.

Conclusion: OrbitForge effectively bridges text-to-video generation and 3D reconstruction without task-specific fine-tuning or score-distillation, and demonstrates the need for coverage-aware evaluation metrics.
...
TL;DR: DDStereo is a real-time, open-set stereo 3D object detector using a dual-decoder transformer that achieves state-of-the-art accuracy and monocular-level inference speed.


<details>
  <summary>Details</summary>
Motivation: Existing stereo 3D detectors have high accuracy but low inference speed (not real-time), and stereo open-set detection is unexplored despite advances in monocular open-world detection.

Method: DDStereo uses a Dual-Decoder Stereo Transformer with two lightweight decoder branches: one for open-set foreground 2D detection and one for 3D attribute regression, sharing object-level queries for unified alignment. It also features a compact disparity feature extractor and streamlined decoder architecture.

Result: Achieves state-of-the-art accuracy on both closed-set and open-set stereo 3D benchmarks, surpasses existing stereo detectors in inference speed, and reaches real-time performance comparable to monocular methods.

Conclusion: DDStereo bridges the gap in stereo open-set 3D detection and solves the speed-accuracy trade-off, achieving real-time performance for the first time in stereo 3D detection.
...
TL;DR: GeoT2V-Bench is a reconstruction-based benchmark that evaluates whether camera-prompted text-to-video clips can support explicit rigid 3D reconstruction, using metrics like motion, trajectory, rendering error, and flow agreement across 3,840 reconstructions from 12 models.


<details>
  <summary>Details</summary>
Motivation: Visual plausibility alone is insufficient for camera-prompted T2V outputs; generated frames should also provide coherent multi-view evidence for a single static 3D scene, which existing benchmarks fail to assess.

Method: Pipeline estimates per-frame camera intrinsics/poses with VGGT-style geometry, fits DeformableGS, derives static MedianGS proxy via temporal-median aggregation, and renders along the estimated camera path. Reports a continuous reconstruction profile with 5 metrics.

Result: Visible motion, static rendering error, flow agreement, and flexible-vs-static behavior often disagree, revealing complementary failure modes in T2V models when tested as global static-scene acquisitions.

Conclusion: GeoT2V-Bench captures complementary failure modes that emerge when generated videos are tested as global static-scene acquisitions, providing a more nuanced evaluation than pass/fail or single-score metrics.
...
TL;DR: A one-step diffusion editing method (Riemannian Residual Line Search) that treats edit strength as a candidate-selection problem, using local time curvature estimation and CLIP-based selection to achieve SOTA on PIE-Bench++.


<details>
  <summary>Details</summary>
Motivation: One-step diffusion editors face a fundamental tension: a single transport update must be both aggressive enough to realize the target prompt and conservative enough to preserve the source image, and no fixed update strength works across all edit types.

Method: Riemannian Residual Line Search: (1) estimates local time curvature of the prompt-delta field to project corrected direction onto the original update norm; (2) forms a residual path from source to strong edit; (3) retains original first-order output as a candidate; (4) selects final image by maximizing target-prompt CLIP alignment.

Result: Achieves state-of-the-art (SOTA) performance among current one-step update algorithms on a 700-sample PIE-Bench++ evaluation across 10 edit type IDs.

Conclusion: The tension between edit strength and image preservation can be effectively resolved by framing it as a post-hoc candidate-selection problem on top of energy-field transport, rather than designing a new editing model.
...
TL;DR: This paper shows that spherical/omnidirectional stereo images can be effectively processed for disparity estimation by first converting them to equirectangular (ERP) projection, then applying a RAFT+EACS framework to extract one-dimensional disparity, achieving accurate results on synthetic fisheye datasets.


<details>
  <summary>Details</summary>
Motivation: Omnidirectional (360°) stereo images violate classical disparity estimation assumptions because epipolar lines follow curved great-circle paths, causing 2D displacements that cannot be treated as single-axis disparity without geometric rectification.

Method: Convert spherical stereo images to equirectangular (ERP) projection to straighten epipolar curves into 1D horizontal/vertical disparities, then apply RAFT dense optical flow + Epipolar-Aligned Channel Selection (EACS) to extract only the baseline-aligned flow component as disparity.
...
Conclusion: Established ERP preprocessing can be effectively combined with RAFT+EACS to enable practical, interpretable, and efficient disparity estimation from spherical stereo, offering a straightforward pathway to extend conventional stereo pipelines to 360° imaging.
...
TL;DR: IV-CoT introduces an Implicit Visual Chain-of-Thought framework that decomposes visual conditioning into structural and semantic queries to improve structure-aware image generation, achieving superior results without extra inference-time decoding.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with structure-aware prompt following (object counts, spatial relations, attribute bindings, layouts) due to the entanglement of structural planning and appearance rendering in a single conditioning stream.

Method: IV-CoT decomposes visual conditioning queries into a structural-to-semantic cascade: structural queries form a latent visual plan, semantic queries render appearance conditioned on that plan. Training-only sketch supervision guides structural queries to capture structure without inference-time decoding.

Result: Achieves superior results on GenEval and T2I-CompBench benchmarks. Visualizations confirm structural and semantic queries play complementary roles in structure-aware generation.

Conclusion: The proposed implicit CoT reasoning in a single forward pass effectively decouples structural planning from appearance rendering, significantly improving structure-aware generation quality in MLLMs.
...
TL;DR: FLUX3D proposes a scalable image-to-3D Gaussian Splatting framework that addresses two key bottlenecks (representation and cross-modal alignment) via Diffusion-Aligned Structured Latents (DA-SLAT), a decoder-only architecture, and a Sparse-structure Multimodal Diffusion Transformer (SMDiT) with Modal-Aware Rotary Positional Embedding (MARoPE), achieving SOTA results in high-fidelity 3DGS generation.


<details>
  <summary>Details</summary>
Motivation: Existing sparse voxel representation methods for image-to-3DGS suffer from two structural bottlenecks: (1) using discriminative 2D features designed for semantic abstraction suppresses reconstructive cues (representation bottleneck), and (2) standard diffusion transformers fail to align dense 2D image tokens with sparse 3D voxel latents (cross-modal correspondence bottleneck).

Method: FLUX3D introduces: (1) Diffusion-Aligned Structured Latents (DA-SLAT) coupled with a decoder-only architecture to improve 3DGS reconstruction fidelity; (2) Sparse-structure Multimodal Diffusion Transformer (SMDiT) and Modal-Aware Rotary Positional Embedding (MARoPE) for geometry-agnostic 2D-3D cross-modal alignment.

Result: FLUX3D yields substantial improvements in appearance fidelity and significantly outperforms all state-of-the-art (SOTA) methods in generating high-quality 3DGS assets on extensive benchmarks.

Conclusion: By addressing both the representation bottleneck and the cross-modal correspondence bottleneck, FLUX3D provides a scalable and effective framework for high-fidelity image-to-3DGS generation.
...
TL;DR: FLAT introduces a method to decode triangle splats directly from video diffusion latents for single-image 3D scene generation, achieving better geometric accuracy than 3D Gaussians while maintaining competitive visual quality, using a ray-centered rotation parameterization and novel product window function for improved gradient flow.


<details>
  <summary>Details</summary>
Motivation: Existing feedforward latent scene decoders output volumetric 3D Gaussians that lack well-defined surfaces, limiting their use in simulation and standard graphics pipelines. The authors aim to decode surface-aligned primitives (triangle splats) directly from compressed video diffusion latents in a single pass.

Method: FLAT uses (1) a ray-centered rotation parameterization for triangle regression and (2) a novel product window function that improves gradient flow during differentiable triangle rendering, enabling direct decoding of triangle splats from video diffusion latents.

Result: FLAT achieves significantly better geometric accuracy than state-of-the-art feedforward baselines while maintaining competitive visual quality. A test-time refinement step converts predicted triangle soup into game-engine-ready representation supporting real-time rendering.

Conclusion: The paper provides the first systematic analysis of representation tradeoffs (3DGS, 2DGS, triangle splatting) under identical training setup, demonstrating that triangle splats decoded from video diffusion latents are a viable surface-aligned primitive for feedforward scene generation.
...
TL;DR: This paper develops a large-scale open benchmark of 85,355 CT scans to evaluate 12 tumor-detection AI models across different tumor sizes, locations, patient subgroups, and imaging protocols, finding that models optimized for average accuracy perform poorly in rare or underrepresented subgroups (e.g., young, female African Americans).


<details>
  <summary>Details</summary>
Motivation: AI models for medical imaging often perform inconsistently across real-world clinical settings due to variations in patient demographics and imaging protocols, and these inconsistencies need to be systematically quantified.

Method: The authors built a benchmark of 85,355 CT scans evaluating 12 tumor-detection AI models across tumor size, location, patient subgroup, and imaging protocol, using LLMs to extract and organize subgroup information from clinical data.

Result: Current state-of-the-art AI models optimized for average accuracy perform poorly in rare or underrepresented subgroups such as young, female African Americans.

Conclusion: The benchmark provides a foundation for building more reliable AI models for tumor detection and highlights the need for rigorous subgroup-level evaluation in medical imaging and computer vision.
...
TL;DR: NanoGen is a unified DiT training/evaluation framework supporting both ImageNet and T2I generation. It shows that method rankings between the two tasks have weak to negative correlation (-0.377 to -0.580), proving ImageNet-only evaluation is insufficient. The authors propose DiffusionBench as a more holistic benchmark.


<details>
  <summary>Details</summary>
Motivation: Current DiT research relies almost exclusively on ImageNet class-conditional generation for evaluation, but it's unclear if improvements there translate to real progress in text-to-image generation. The authors argue T2I evaluation is now feasible and necessary.

Method: NanoGen framework supports RAE, VAE, pixel-space, and MeanFlow diffusion under both ImageNet and T2I setups with minimal configuration changes (12 lines). 21 latent diffusion models were trained under this unified framework.

Result: Pearson correlation between ImageNet and T2I method rankings ranges from -0.377 to -0.580 across three metrics, showing weak/negative correlation. A method improving ImageNet FID may not improve (or may even hurt) T2I performance.

Conclusion: The paper recommends replacing ImageNet-only evaluation with DiffusionBench (combining ImageNet and T2I results) for more meaningful progress assessment in DiT research.
...
TL;DR: SpaCE is the first theoretical framework for multi-frame spatial reasoning in MLLMs, proving information-theoretic bounds on accuracy, sample complexity of Θ(d_eff·K_max/(ε²·δ)), PAC-Bayes generalization guarantees, and characterizing the bias-variance trade-off between explicit 3D and implicit reasoning, validated on MultiSPA, CA-VQA, and SpatialRGPT benchmarks.


<details>
  <summary>Details</summary>
Motivation: While MLLMs show empirical success in spatial understanding through large-scale training on spatial VQA datasets, the theoretical foundations of multi-frame spatial reasoning remain completely unexplored. This paper aims to provide a principled theoretical understanding of when, why, and how multi-frame spatial reasoning succeeds.

Method: The paper develops SpaCE, a rigorous theoretical framework that (1) proves an information-theoretic upper bound on spatial reasoning accuracy via mutual information, (2) derives sample complexity bound of Θ(d_eff·K_max/(ε²·δ)), (3) provides a PAC-Bayes generalization bound under distribution shift, and (4) characterizes the bias-variance trade-off between explicit 3D representations and implicit reasoning approaches. Theoretical predictions are validated on three benchmarks.

Result: Four main theoretical results are established: information-theoretic accuracy bounds, sample complexity bounds, PAC-Bayes generalization guarantees under distribution shift, and formal characterization of bias-variance trade-offs between explicit 3D and implicit reasoning. Empirical validation shows bounds are tight and frame complementarity is the key driver of multi-frame spatial capacity.

Conclusion: SpaCE provides the first principled theoretical foundation for multi-frame spatial reasoning in MLLMs, offering provable guarantees and practical insights into when explicit vs. implicit spatial reasoning is preferable.
...
TL;DR: This paper integrates Entropy-Conserving Binarization (ECB) into a full CABAC implementation, comparing it against UEG, Huffman, and HuffmanPos on synthetic sources, DCT residuals, and the Kodak image suite. ECB beats single-context Huffman on Kodak but loses to HuffmanPos, revealing that context allocation (not bin count) drives rate gaps at low entropy. ECB's decoder latency is 7-10x higher due to O(N*m) complexity; a faster variant is sketched.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether the entropy-conserving binarization (ECB), which provably preserves entropy when mapping m-ary data to m-1 binary strings, performs effectively inside a production-style CABAC (binary arithmetic coder), and to understand which factors—binarization or context allocation—dominate compression rate differences.

Method: ECB was integrated into a from-scratch CABAC implementation alongside UEG, single-context Huffman, and HuffmanPos (Huffman with per-bin-position contexts), all sharing one M-coder backend. They were benchmarked on synthetic sources, DCT residuals from a procedural image, and the full 24-image Kodak suite (2,480 round-trip trials, bit-exact verified).

Result: On procedural images, ECB overtakes single-context Huffman at Q=8, reaching 27 percentage points below at Q=32. On Kodak, ECB beats single-context Huffman at all Q values, gap growing from 0.031 to 0.113 bits/symbol. HuffmanPos beats ECB on 12/15 source cells, losing by at most 0.56 percentage points on the other three. ECB's decoder latency is 7-10x higher on large alphabets due to O(N*m) complexity.

Conclusion: At low source entropy, the rate gap is driven primarily by context allocation over the bin stream, not the binarization's per-symbol bin count. ECB's rate efficiency is competitive but has a high decoder latency cost; an interleaved single-pass variant is proposed to close this gap.
...
TL;DR: ChameleonNet uses contrastive unpaired translation to synthesize non-contrast CT from contrast CT, enabling heart chamber segmentation from non-contrast scans without manual annotations. Achieves high Dice scores (0.91-0.94) on synthesized images but shows moderate volume errors on real non-contrast CT, especially for LV and RV.


<details>
  <summary>Details</summary>
Motivation: To enable heart chamber segmentation from non-contrast CT scans without requiring manual annotations on non-contrast images, leveraging existing contrast CT annotations through image translation.

Method: ChameleonNet: (1) CUT network with DCL loss to translate contrast CT to non-contrast CT; (2) nnU-Net with Hausdorff distance loss trained on synthesized non-contrast images using annotations from contrast scans.

Result: On synthesized non-contrast: DSC 0.91-0.94, HD95 3.63-5.74mm. On real non-contrast: Pearson correlation 0.82-0.93, MAPE 9.22%-20.79%, MPE -12.52% to 4.67%.

Conclusion: Feasible approach for non-contrast heart chamber segmentation, but volume errors (especially LV, RV) require further refinement before clinical deployment.
...
TL;DR: E-MRL is a cross-view aligned multimodal RL framework for 3D CT report generation that reduces hallucinations by forcing VLMs to identify and verify key evidence slices, using a novel cross-view consistency reward that validates semantic alignment between reports and local visual re-queries.


<details>
  <summary>Details</summary>
Motivation: Current VLMs for medical report generation suffer from visual hallucinations and rely on language priors rather than genuine visual perception, as SFT and RL methods only optimize text fidelity without grounding in actual 3D CT data.

Method: Formulates generation as a Markov Decision Process of "diagnosis-localization-verification", explicitly trains the model to identify a key evidence slice alongside the global report, and introduces a cross-view consistency reward that validates semantic alignment between the golden-standard report and a local visual re-query of the selected key slice.

Result: On large-scale 3D CT tumor datasets, E-MRL significantly reduces hallucinations and improves diagnostic accuracy compared to SFT and RL baselines.

Conclusion: E-MRL offers a clinically interpretable solution for visually-grounded and tumor analysis by grounding findings in verifiable visual evidence.
...
TL;DR: A dual-edge spatial-Jacobian image graph is proposed for interpretable DR grading, integrating vessel information, lesion evidence maps, contrastive embeddings, and morphometric biomarkers to achieve strong predictive performance while enabling explainability.


<details>
  <summary>Details</summary>
Motivation: Clinical interpretation of DR grading requires understanding how lesion evidence is distributed around retinal vessels and its relationship to quantitative vascular biomarkers, beyond just image-level labels.

Method: Each fundus image is represented as a graph node with four aligned evidence streams (vessel info, lesion maps, contrastive embedding, biomarkers), connected via spatial edge (vessel-lesion geometry) and Jacobian edge (embedding-biomarker sensitivity) branches, fused by lightweight two-token attention.

Result: On 2,910 APTOS images: 0.8076 accuracy, 0.8312 quadratic weighted kappa, 0.5915 macro-F1, 0.9330 adjacent-grade accuracy; referable DR achieves 0.9055 accuracy and 0.9711 AUROC.

Conclusion: The framework serves as an explainable representation-learning tool for lesion-biomarker hypothesis generation rather than a deployment-ready clinical classifier.
...
TL;DR: This study combines in-situ soil moisture sensors with multi-sensor SAR data (Sentinel-1, Sentinel-2) and ML methods (XGBoost, LightGBM, RF, etc.) to improve surface soil moisture retrieval over a limestone quarry in Finland. Best results (RMSE 0.037-0.050 m³/m³, R²=0.90) came from tree-based ensembles using comprehensive multi-source features, with sediment-specific calibration providing limited extra benefit when rich features were available.


<details>
  <summary>Details</summary>
Motivation: To assess and improve methodologies for retrieving surface soil moisture from SAR measurements over mining sites, where accurate ground moisture monitoring is crucial but challenging due to heterogeneous geomaterials.

Method: Combined in-situ IoT capacitance sensors with Sentinel-1 C-band SAR, Sentinel-2 optical data, topographic, and temperature predictors. Tested XGBoost, LightGBM, RFs, linear regression, and k-NN regression, with sediment-specific calibration.

Result: Best RMSE=0.037-0.050 m³/m³ (R²=0.90). Tree-based ensembles (LightGBM, RF, XGBoost) outperformed others. Lowest errors for clay/organic soil; highest for flotation sand/gravel. Sediment information improved Sentinel-1-only retrievals by >2 vol-% but little benefit with multi-source features.

Conclusion: Multi-source feature sets with tree-based ensemble methods enable high-accuracy soil moisture retrieval over mining sites. Sediment-specific calibration is beneficial when data is limited but less critical with rich multi-source inputs.
...
TL;DR: Female-RHINO is a real-time, scanner-integrated AI framework that automatically analyzes uterine MRI, generates structured reports, and provides quantitative biomarkers (segmentation, landmark detection, incidental finding quantification) within 70 seconds during ongoing scans, validated on 500+ multi-center datasets with robust performance.


<details>
  <summary>Details</summary>
Motivation: Standardized uterine MRI assessment is challenging due to anatomical variability, observer dependence, and the lack of workflow-integrated automated analysis tools. Existing methods require manual interaction and post-hoc processing, limiting clinical adoption and reproducibility.

Method: An end-to-end system integrating inline communication with MRI scanner and deep learning models (segmentation + anatomical landmark detection) trained on 500+ multi-center datasets. Performs volumetry, detects fibroids and Nabothian cysts, extracts six anatomical landmarks, and compiles structured reports with visualizations—all without manual interaction.

Result: Dice: uterus 0.82, fibroids 0.80, lower for Nabothian cysts. Landmark detection mean radial error: 3.7 mm. End-to-end processing under 70 seconds, enabling real-time availability. Prospective deployment showed standardized, reproducible analyses with inter-observer agreement.

Conclusion: The proposed system enables real-time scanner-integrated AI for automated uterine MRI analysis and reporting, with potential to improve standardization, efficiency, and clinical workflow in pelvic imaging.
...
### [124] [Systematic Exploration of 4-Expert Heterogeneous Mixture-of-Experts via Automated Pipeline Search](https://arxiv.org/abs/2606.23739)
*Yashkumar R Lukhi,Harsh Rameshbhai Moradiya,Radu Timofte,Dmitry Ignatov*

Main category: cs.LG

TL;DR: An automated pipeline for generating heterogeneous 4-Expert Mixture-of-Experts (MoE4) architectures was run for 28 days on an RTX 4090, producing 4,463 candidate models. A critical coverage bias was discovered: due to alphabetical enumeration via itertools.combinations, the entire 4.8% explored search space is anchored to a single family (AirNet). ShuffleNet+MobileNetV3 ensembles achieve highest accuracy (0.632), while FractalNet and MNASNet are low-yield. The pipeline and fixes are open-sourced via NNGPT.


<details>
  <summary>Details</summary>
Motivation: To automate the design of heterogeneous MoE architectures by replacing manual design with a systematic code-assembly generator, and to evaluate large-scale architectural combinations efficiently.

Method: A deterministic generator that combines 4 base architecture families from the LEMUR database into MoE4 ensembles, using convolutional gating networks with temperature scaling, mixup augmentation, and cosine-annealed learning rate scheduling. The pipeline ran for 28 days on an NVIDIA RTX 4090, exploring 4.8% of the theoretical 23,751 combinations.

Result: 1,021 models were successfully evaluated. AirNet families dominate due to itertools.combinations alphabetical ordering bias. ShuffleNet+MobileNetV3 ensembles achieve highest accuracy (0.632). FractalNet and MNASNet are low-yield. The coverage bias is precisely characterized and a stratified random sampling fix is proposed.

Conclusion: The pipeline reveals systematic search bias from combinatorial enumeration ordering, identifies high-yield architecture families for future MoE design, and provides an open-source corrected generator.
...
### [125] [Weight-Space Geometry of Offline Reasoning Training](https://arxiv.org/abs/2606.23740)
*Aleksandr Nikolich,Igor Kiselev,Vladimir Platonov,Karina Romanova*

Main category: cs.LG

TL;DR: Six offline RL losses for reasoning distillation (SFT, RFT, DFT, RIFT, Offline GRPO, DPO) are compared mechanistically. Most reward-weighted methods produce nearly colinear weight updates, while DFT diverges more, Offline GRPO adds orthogonal components, and DPO sits in a near-orthogonal subspace with the highest accuracy but also the largest update-norm gap due to a 10× smaller learning rate.


<details>
  <summary>Details</summary>
Motivation: To determine whether different offline RL losses used for distilling reasoning from large to small models are mechanistically distinct or converge to similar weight updates, rather than just comparing downstream accuracy.

Method: Trained six methods (SFT, RFT, DFT, RIFT, Offline GRPO, DPO) on identical math rollouts from Qwen3-4B with attention-only LoRA, then analyzed weight deltas using cosine similarity, principal-angle subspace analysis, linear mode connectivity, and CKA.

Result: (i) SFT, RFT, RIFT have nearly colinear weight deltas (cosine≥0.97) with comparable GSM8K accuracy (87-88%); (ii) DFT diverges more than reward-weighted methods; (iii) Offline GRPO adds orthogonal component (~67% globally) but stays in SFT loss basin; (iv) DPO is near-orthogonal, shows mode-connectivity barrier, collapses late-layer CKA (~0.46), but achieves highest accuracy (GSM8K 93.5%, AIME26 30.0%) with 10× smaller learning rate.

Conclusion: Methods that appear similar in accuracy can be mechanistically distinct; DPO stands out as uniquely different from all other methods, but its superiority may be partly due to optimizer choices rather than the loss function alone, warranting future learning-rate-matched comparisons.
...
### [126] [A Survey on Federated Causal Discovery and Inference](https://arxiv.org/abs/2606.23741)
*Xianjie Guo,Yuwei Wang,Guodu Xiang,Xiaoli Tang,Kui Yu,Han Yu,Qiang Yang*

Main category: cs.LG

TL;DR: This paper provides a comprehensive systematic review of federated causal discovery (FCD) and federated causal inference (FCI), organizing them through multi-dimensional taxonomies and proposing them as complementary stages of a unified federated causal reasoning pipeline.


<details>
  <summary>Details</summary>
Motivation: Causal reasoning is fundamental for data-driven decision making, but data needed for reliable causal analysis is often distributed across institutions and cannot be centralized due to privacy and communication constraints. The interdisciplinary nature of federated causal discovery and inference, combined with the absence of a comprehensive survey, creates barriers for researchers entering this rapidly growing field.

Method: The authors conduct a systematic review using multi-dimensional taxonomies. For FCD, they organize methods along three core design decisions (how structures are learned, how data are partitioned, what structural knowledge each party obtains) and three axes (methodological paradigm, federation topology, structural scope), examining practical dimensions like temporal dynamics, data heterogeneity, missing data, and non-identical variable sets. For FCI, they categorize methods by target estimand (average vs. individualized/conditional treatment effects) and estimation strategy (from classical weighting to deep generative architectures). They formalize FCD and FCI as complementary stages of a unified pipeline.

Result: The review systematically organizes the federated causal reasoning field through structured taxonomies, identifies key practical challenges, and formally connects FCD and FCI as complementary stages in a unified pipeline where FCD supplies structural knowledge for valid effect estimation in FCI.

Conclusion: The paper highlights shared concerns across FCD and FCI regarding privacy, communication efficiency, theoretical guarantees, and application domains, and identifies open challenges for future research in federated causal reasoning.
...
### [127] [Low-power analogue neural networks with trainable nonlinear connections for continuous control](https://arxiv.org/abs/2606.23742)
...
TL;DR: This paper introduces physical Kolmogorov-Arnold networks that place trainable nonlinear functions on connections rather than nodes, using analogue band-pass filters on field-programmable analogue arrays. They achieve parameter efficiency for smooth, continuous tasks (robotics, control, solar tracking) but not for classification, transfer reliably to hardware at ~30μW, and show the benefit comes from connection-based nonlinearity rather than specific devices.


<details>
  <summary>Details</summary>
Motivation: Physical neural networks typically force nonlinear device responses to act as scalar weights, limiting their expressiveness. The authors aim to leverage device physics more directly by placing trainable nonlinear functions on connections, inspired by Kolmogorov-Arnold networks, to improve parameter efficiency for certain tasks.

Method: The authors implement trainable nonlinear functions as analogue band-pass filters on field-programmable analogue arrays (FPAAs). They train these physical Kolmogorov-Arnold networks and transfer them to hardware across ~35,000 connections. They also simulate a memristive realization and project a dedicated CMOS implementation at ~30 microwatts.

Result: The networks achieve parameter efficiency on smooth, continuous-valued tasks (robotic kinematics, continuous control, photovoltaic tracking) using far fewer nodes and connections than MLPs. However, they offer no advantage on classification-like decision boundaries. Training transfers to hardware with quantified fidelity across ~35,000 connections.

Conclusion: The advantage of this approach comes from placing trainable nonlinearity on connections rather than from a specific device, as demonstrated by consistent behavior across FPAA, memristive, and CMOS implementations. This opens new directions for low-power (μW-scale) analogue neural computing suited to continuous control tasks.
...
### [128] [Synergizing Physically Constrained MCMC and Chemical-Informed Gaussian Processes for Reaction Network Discovery](https://arxiv.org/abs/2606.23757)
*Runzhe Liu,Zihao Wang,Wenbo Yang,Shengyang Tao*

Main category: cs.LG

TL;DR: PC-MCMC-CIGP is a gray-box workflow combining topology sampling, conservation screening, and Chemical-Informed Gaussian Processes to extract interpretable governing equations and optimize yields from sparse, noisy chemical time-series data.


<details>
  <summary>Details</summary>
Motivation: Extracting interpretable governing equations from sparse, noisy chemical time-series data is difficult because reaction topology and kinetic parameters are tightly coupled, and existing methods lack physically constrained uncertainty-aware integration.

Method: A gray-box workflow integrating spike-and-slab topology sampling, hard conservation and thermodynamic screening, and a Chemical-Informed Gaussian Process (CIGP) residual model for parameter calibration and experimental design.

Result: On H2+Br2 benchmark, the sampler distinguished elementary radical pathways from deceptive fits. On styrene epoxidation, CIGP loop improved yield by 12.5% over GP-BO baseline. Acquisition study showed PC-EI reduces low-yield suggestions while EI-style criteria give strongest final yield.

Conclusion: The proposed integration of physically constrained MCMC with CIGP provides a reproducible, uncertainty-aware framework for chemical equation discovery and optimization, with clear trade-offs in acquisition strategies.
...
### [129] [Exploring Dualistic Meta-Learning to Enhance Domain Generalization in Open Set Scenarios](https://arxiv.org/abs/2606.23758)
*Xiran Wang,Jian Zhang,Lei Qi,Yang Gao,Yinghuan Shi*

Main category: cs.LG

TL;DR: MEDIC uses meta-learning with dual task splits (domain and class) to handle open set domain generalization by balancing decision boundaries between known and unknown classes across domains.


<details>
  <summary>Details</summary>
Motivation: Existing open set domain generalization methods using one-vs-all classifiers suffer from class imbalance (few positives vs many negatives), causing over-rejection of out-of-distribution data even from known classes in unseen domains.

Method: Proposes MEDIC, a meta-learning strategy that jointly performs implicit gradient matching on inter-domain and inter-class task splits to find balanced decision boundaries for both domains and classes.

Result: MEDIC outperforms prior methods in open set scenarios while maintaining competitive close set generalization performance.

Conclusion: Joint domain-class matching via meta-learning effectively addresses the label mismatch and imbalance problems in open set domain generalization.
...
TL;DR: A same-hands re-evaluation of causal inference methods on the Tuebingen cause-effect pairs shows that published accuracies are inflated by inconsistent protocols (different subsets, weighting, model selection, and abstention). Under a strict common ruler (no tuning, forced decisions on all 102 pairs), methods cluster in the low-to-mid 70s, and a zero-parameter baseline (sorted-conditional compression with bz2) ties the strongest methods at 74.7% weighted accuracy.


<details>
  <summary>Details</summary>
Motivation: Published comparisons of causal inference methods on Tuebingen pairs are unreliable because each method is evaluated under its own protocol (different subsets, weightings, model selection, decision rates). The authors argue this is the wrong comparison and aim to run the right one — a fair, same-hands re-evaluation.

Method: The authors re-evaluate all methods on the identical 102 pairs with one strict rule: no tuning, forced decision on every pair. They introduce a minimal baseline called sorted-conditional compression: quantize, sort, first-difference data, then apply an off-the-shelf compressor (bz2) — zero fitted parameters.

Result: The ranking under the common ruler differs sharply from the literature. The baseline achieves 74.7% weighted accuracy (p=3.7e-7). On the same 100 pairs as SLOPE, it scores 76.0% — a 1.2-point gap (noise level). RECI re-run lands at 70.7% (inside original error bar, not the mis-copied 77.5%). SLOPE's published 82.4% is a decided-subset figure. Methods cluster in low-to-mid 70s; the zero-parameter compressor ties the strongest.

Conclusion: Published figures are inflated by test-set model selection and significance-gated abstention. The compression score magnitude serves as a model-free confounding flag (p=2.8e-68). A pre-registered falsification test fails instructively, bounding the method's theoretical interpretation. Code and data are released.
...
### [131] [Deciphering Fingerprints of 3D Molecular Surfaces for Accurate Epitope Prediction](https://arxiv.org/abs/2606.23830)
*Fang Wu,Weihao Xuan,Jure Leskovec,Yejin Choi,Li Erran Li*

Main category: cs.LG

TL;DR: SurfBind is a surface-centric deep learning framework for epitope prediction that directly models molecular surfaces using Transformers, cross-attention, and coarse-to-fine prediction, achieving state-of-the-art results on standard benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on sequences or backbone structures and fail to capture discontinuous, surface-driven epitopes, highlighting the need for direct molecular surface representation learning.

Method: SurfBind operates on molecular surface representations using a Transformer with patch-level surface modeling, binder-aware cross-attention, and a hierarchical coarse-to-fine prediction paradigm.

Result: SurfBind achieves state-of-the-art performance on SAbDab and DB5.5 benchmarks, with strong generalization across unseen antibodies and conformational states.

Conclusion: Interaction-aware surface modeling is valuable for understanding antibody-antigen recognition and protein-protein interaction mechanisms.
...
### [132] [Reconstructing GRACE Terrestrial Water Storage with Spatio-Temporal Graph Neural Networks: An Application to South America](https://arxiv.org/abs/2606.23833)
*Lukas Arzoumanidis,Lara Johannsen,Klara Middendorf,Annette Eicker,Youness Dehbi*

Main category: cs.LG

TL;DR: This paper presents a deep learning approach using a multi-variate time series graph neural network (MTGNN) to reconstruct GRACE-like terrestrial water storage anomalies back to 1940 from ERA5 meteorological data, achieving strong performance with fewer predictors than existing methods.


<details>
  <summary>Details</summary>
Motivation: The GRACE and GRACE-FO satellite missions only provide TWS data from 2002 onward, which is too short for climate-scale analyses. The authors aim to extend the TWS record back to 1940 using a reconstruction approach.

Method: The authors adapt a multi-variate time series graph neural network (MTGNN) originally developed for traffic forecasting. They use a hybrid adjacency matrix combining geodesic proximity and lagged correlations of climatic time series to encode spatial dependencies, and train the model on daily ERA5 meteorological forcing (precipitation, evapotranspiration, runoff) to predict monthly GRACE TWS anomalies.

Result: The reconstruction achieves a grid-cell Pearson correlation of 0.69, basin-mean correlation of 0.94, near-zero bias, and reproduces spatial patterns of El Niño/La Niña events. It is statistically competitive with established methods (GTWS-MLrec, RM-REC, GRAiCE) while using only half to a tenth of the predictors.

Conclusion: The graph-based model provides an efficient and effective way to reconstruct long-term TWS records, with particular advantages in predictor efficiency. The implementation is publicly available on GitHub.
...
TL;DR: This paper introduces a mathematically equivalent reformulation of the PPMH sampler for exact NML codelength computation that reduces per-step complexity from O(N³) to O(k³ + N²k) using Schur complement and Sylvester's determinant identity, achieving >14,000× speedup while maintaining numerical precision for Lasso, Sparse SVM, Elastic Net, and Group Lasso.


<details>
  <summary>Details</summary>
Motivation: Exact computation of NML codelength for regular non-smooth estimators like Lasso has been computationally prohibitive due to O(N³) bottlenecks from manifold-constrained projection (requiring inversion of (N+k)×(N+k) KKT matrices) and volume integration (requiring determinants of (N-k)×(N-k) Gram matrices).

Method: The authors reformulate the PPMH sampler using block Schur complement to avoid full KKT matrix inversion and Sylvester's determinant identity to bypass direct Gram matrix determinant calculation, reducing complexity from O(N³) to O(k³ + N²k) per step. The method is generalized to Sparse SVM, Elastic Net, and Group Lasso, with rigorous numerical stability analysis.

Result: The approach achieves a constant speedup exceeding 14,100× on high-dimensional datasets while maintaining double-precision numerical equivalence. Efficiency is measured via Effective Sample Size (ESS) per second.

Conclusion: Exact non-smooth NML estimation is rendered highly tractable for large-scale statistical inference, breaking the historical cubic scaling limitations.
...
### [134] [The Degeneracy Distillery](https://arxiv.org/abs/2606.23838)
*T. Lucas Makinen,Deaglan J. Bartlett,Niall Jeffrey,Benjamin D. Wandelt*

Main category: cs.LG

TL;DR: A method (degeneracy distillery) that automatically detects and resolves degenerate parameter combinations in models by analyzing the Fisher information matrix, requiring no actual data - just parameter-simulation pairs. It discovers symbolic coordinate transformations that flatten the Fisher information globally, reducing simulation needs by up to 10x for neural posterior estimation.


<details>
  <summary>Details</summary>
Motivation: Degeneracies (parameters/labels producing similar data) make label prediction and inverse problems difficult. The authors aim to automatically detect and resolve these degeneracies to improve model understanding and reduce computational costs in posterior estimation.

Method: The degeneracy distillery works by (1) estimating the Fisher information matrix from parameter-data/parameter-simulation pairs, (2) flattening it through symbolic coordinate transformations, and (3) exploring the information geometry of the likelihood. This is done automatically and symbolically without requiring realized data observations.

Result: Demonstrated on synthetic and real-world problems, the method discovers symbolic coordinate transformations that identify parameter combinations yielding independent effects. It flattens Fisher information globally (unlike posterior-based methods that flatten at a single point), and reduces simulation budget by up to 10x for neural posterior estimation at matched validation calibration.

Conclusion: Degeneracies are intrinsic properties of physical models, and the degeneracy distillery effectively identifies and resolves them via Fisher information analysis and symbolic coordinate transformations, providing both computational efficiency gains and physical insight into the system.
...
### [135] [Machine Learning Modeling for Real-Time Melt Pool Monitoring in Laser Powder Bed Fusion Additive Manufacturing: A Hybrid Approach](https://arxiv.org/abs/2606.23851)
*Inioluwa Emmanuel,Zhuo Yang,Ho Yeung,Xinyao Zhang*

Main category: cs.LG

TL;DR: This paper develops a binary image classification framework using AI/ML for real-time melt pool anomaly detection in laser powder bed fusion additive manufacturing. A hybrid approach combining EfficientNetB0 feature embeddings with Random Forest achieves the best performance (F1=0.9451, accuracy=0.9458, AUC=0.9904) with sub-millisecond inference (1.15 ms), outperforming pure deep learning models in both accuracy and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: To enable real-time monitoring and anomaly detection in LPBF additive manufacturing by developing a computationally efficient AI/ML framework that balances accuracy with hardware constraints of open-architecture machines, addressing data-limited environments in factory-floor settings.

Method: A binary image classification framework using a balanced dataset of 1,200 melt pool images (Nickel superalloy 625). Benchmarks three transfer learning architectures (ResNet50, EfficientNetB0, MobileNetV2) against two Random Forest approaches (hybrid using EfficientNetB0 embeddings, baseline using raw pixels). Models evaluated on accuracy, precision, recall, F1, AUC, training time, inference latency, and CPU/GPU usage.

Result: The hybrid EfficientNetB0-plus-Random Forest approach achieves the best performance: F1=0.9451, accuracy=0.9458, AUC=0.9904 with sub-millisecond inference (1.15 ms). Pure deep learning models show higher inference times with lower accuracy.

Conclusion: Combining pre-trained convolutional features with classical ensemble methods offers a robust, computationally efficient solution for real-time melt pool anomaly detection in data-limited additive manufacturing environments.
...
### [136] [Sesame: Structure-Aware Molecular Generation via Spatial Density-Map Conditioning](https://arxiv.org/abs/2606.23856)
*Konstantin Yatsenko,Arvind Thiagarajan*

Main category: cs.LG

TL;DR: Sesame is a diffusion-based molecular generation model that uses a spatial pairformer module to condition on both partial molecular structures and protein pockets via continuous spatial density maps, enabling both de novo generation and fragment-conditioned lead optimization, with joint denoising of atom types, bond types, and positions plus trajectory finetuning.


<details>
  <summary>Details</summary>
Motivation: Existing generative molecular models often lack the ability to incorporate partial molecular starting points (scaffolds/fragments) for lead optimization, treating this as secondary to de novo generation, whereas in practice medicinal chemists need to grow molecules from existing scaffolds conditioned on protein pockets.

Method: Sesame introduces: (1) a spatial pairformer module that conditions on partial molecular structure and protein pockets via continuous spatial density maps; (2) a diffusion framework for joint denoising of atom types, bond types, and positions; (3) a trajectory finetuning scheme that trains on the model's own sampling rollouts to improve generation quality.

Result: The model is trained on a large corpus of both ligand-only and protein-ligand datasets, supporting both de novo generation and fragment-conditioned lead optimization from scaffolds supplied by chemists.

Conclusion: Sesame demonstrates a unified approach to molecular generation that equally prioritizes both de novo design and scaffold-based lead optimization through a single conditioning mechanism, addressing a critical gap in computational drug design.
...
### [137] [Are Safety Guarantees in Neural Networks Safe? How to Compute Trustworthy Robustness Certifications](https://arxiv.org/abs/2606.23858)
*Merkouris Papamichail,Konstantinos Varsos,Giorgos Flouris,João Marques-Silva*

Main category: cs.LG

TL;DR: The paper introduces the apothem measure for computing optimal robustness certifications for neural networks, proving it can be computed in linear time (vs. intractable volume-optimal approaches), and presents the ParallelepipedoNN system achieving two-fold improvement on MNIST/Fashion MNIST.


<details>
  <summary>Details</summary>
Motivation: Adversarial examples are a major AI safety challenge. Existing robustness certifications focus on volume-optimal hyper-rectangles, but recent results show computing volume-optimal certifications is intractable in reasonable time.

Method: Introduces the apothem measure for robustness certifications. Proposes an oracle-based algorithm that computes apothem-optimal certifications with linear complexity (w.r.t. input domain diameter). Introduces dual certifications for apothem-minimum upper bounds. Implements the ParallelepipedoNN system.

Result: The apothem-optimal certification can be computed in a linear number of NN verifier calls. Proves no volume-optimal oracle-based algorithm exists (even ignoring oracle costs). ParallelepipedoNN achieves at least two-fold improvement in minimum edge length on MNIST and Fashion MNIST.

Conclusion: Apothem-optimality is a practical alternative to volume-optimality for NN robustness certifications, with provable computational efficiency and strong empirical performance.
...
### [138] [Federated Survival Analysis in Healthcare: A Multi-Model Evaluation on Cross-Institutional Heterogeneous Breast Cancer Data](https://arxiv.org/abs/2606.23871)
*Natalia Moreno-Blasco,Anusha Ihalapathirana,Pekka Siirtola,Miguel Fernandez-de-Retana*

Main category: cs.LG

TL;DR: A systematic evaluation of federated survival analysis on a cross-institutional breast cancer cohort, comparing three models (Cox, DeepSurv, RSF) and three FL strategies (FedAvg, FedProx, FedAdam). FL outperforms local training, RSF performs best overall, and FedAvg/FedProx are more stable than FedAdam. Practical guidelines for model selection are provided.


<details>
  <summary>Details</summary>
Motivation: Time-to-event models require large, diverse datasets rarely available at single institutions, but privacy regulations prevent centralizing patient data. FL offers a privacy-preserving alternative, but its effectiveness for survival modeling under realistic heterogeneous conditions was insufficiently understood.

Method: Evaluated three survival models (Cox PH, DeepSurv, RSF) under centralized, local, and federated training on a naturally heterogeneous cross-institutional breast cancer cohort. Compared three FL optimization strategies (FedAvg, FedProx, FedAdam) for gradient-based models.

Result: FL consistently outperforms local training and approaches or exceeds centralized performance. RSF offers the best discrimination, calibration, and robustness. FedAvg and FedProx are stronger and more stable than FedAdam. Performance depends on client distribution diversity.

Conclusion: Derived practical guidelines mapping data, privacy, interpretability, and resource constraints to recommended model and training-paradigm choices for federated survival modeling in healthcare.
...
### [139] [MGI: Member vs Generated Inference](https://arxiv.org/abs/2606.23872)
*Bihe Zhao,Michel Meintz,Juangui Xu,Franziska Boenisch,Adam Dziedzic*

Main category: cs.LG

TL;DR: This paper formalizes the Member vs Generated Inference (MGI) problem and proposes Data Circuit Breaker (DCB), a three-stage method that combines signals from a model's autoencoder and latent generator to distinguish training members from generated samples, outperforming existing membership inference and attribution methods.


<details>
  <summary>Details</summary>
Motivation: Existing membership inference methods systematically misclassify generated samples as training members, while attribution methods misclassify true members as generated, because both rely on likelihood signals that are elevated for both training examples and model outputs.

Method: DCB (Data Circuit Breaker) is a three-stage method that combines complementary signals from a generative model's autoencoder and latent generator to distinguish training members from generated samples.

Result: DCB consistently outperforms existing methods across image autoregressive and diffusion models, remains effective even with near-duplicate outputs, and generalizes to model derivative settings where new models are trained on generated data.

Conclusion: The proposed DCB method effectively addresses the MGI challenge by leveraging combined signals, overcoming the fundamental limitations of likelihood-based approaches used in prior work.
...
### [140] [GRACE: Gated Refinement for Accurate Causal Edge Discovery in High-Dimensional Time Series](https://arxiv.org/abs/2606.23880)
*Mohammad Fesanghary,Abhinav Havaldar*

Main category: cs.LG

TL;DR: GRACE refines constraint-based causal discovery using Hard Concrete gates with L0 regularization, producing clean bimodal edge scores that outperform L1/attention methods. It achieves 75x speedup over nonlinear CI tests and recovers 9/11 edges on a real river dataset with 99% fewer false positives.


<details>
  <summary>Details</summary>
Motivation: Modern high-dimensional time-series datasets (tens/hundreds of variables) make causal discovery difficult: constraint-based methods with nonlinear CI tests are infeasible at scale, while score-based methods require arbitrary thresholds on continuous scores that often have narrow, overlapping distributions.

Method: GRACE uses Hard Concrete gates with L0 regularization — each candidate edge has an independent gate that concentrates near 0 or 1, producing clean bimodal separation. A fast linear CI skeleton provides high-recall candidates, then a single gated model prunes false positives by learning which edges improve prediction, with automatic regularization adapted to problem dimensions.

Result: On synthetic benchmarks (scale-free, Erdős-Rényi, small-world, d≤100), GRACE substantially improves F1 over its base CI method while maintaining high precision, outperforms attention-based and score-based alternatives, and matches expensive nonlinear CI tests at 75× lower cost. On a real Elbe River dataset, it recovers 9/11 edges with F1=0.86, AUROC=0.99, reducing skeleton's 106 false positives by 99%.

Conclusion: GRACE provides an efficient and robust bridge between constraint-based and score-based causal discovery, achieving high accuracy with dramatically lower computational cost, especially valuable for real-world settings where standard assumptions (e.g., no confounding, stationarity) are violated.
...
### [141] [ARIA: Adaptive Region-Based Importance Allocation for Conditional Diffusion Distillation](https://arxiv.org/abs/2606.23898)
*Loay Mualem,Vinh Tong,Samir Darouich,Mathias Niepert*

Main category: cs.LG

TL;DR: ARIA is an adaptive framework for distilling conditional diffusion models that allocates training effort across regions of the conditioning space based on online estimates of teacher-student discrepancy, improving performance especially in unseen and underrepresented regimes.


<details>
  <summary>Details</summary>
Motivation: Effective distillation of conditional diffusion models requires exploring a large conditioning space, but paired image-condition data may be limited and generating synthetic images for all conditions is computationally infeasible. Existing methods (RC) address this by switching conditions during training but don't address how training effort should be allocated across conditions.

Method: ARIA adaptively allocates training effort across coarse regions of the conditioning space by maintaining online estimates of teacher-student discrepancy at the region level, focusing updates where misalignment persists while preserving the original distillation objective.

Result: ARIA improves over RC across most architectures and settings, with clearest gains in unseen and underrepresented regimes. Theoretical analysis shows the tracking mechanism follows evolving discrepancy under bounded variance and drift assumptions.

Conclusion: Adaptive allocation of training effort across conditioning regions is an effective strategy for knowledge distillation in conditional diffusion models, particularly beneficial for addressing distributional gaps in unseen or underrepresented conditions.
...
### [142] [Closing the Loop: Formally Verified Law as a Reward Signal for Self-Improving Legal AI](https://arxiv.org/abs/2606.23913)
*Armin Heydari,Torben Leowald*

Main category: cs.LG

TL;DR: This paper presents an architecture that creates a formally verifiable reward signal for training legal AI by adapting the "LLM proposes, verifier disposes" paradigm from math to law. It uses LLM-driven autoformalization into a formal legal calculus (extending Catala), a verification kernel, and explanation generation from formal proof traces. The approach provides provable correctness for computational law and structural guarantees for open-textured legal analysis. Demonstrated on German procedural deadlines, U.S. Commerce Clause analysis, and cross-jurisdictional sanction proportionality, the architecture closes the reinforcement-learning loop gap in legal AI by providing a deterministic external verifier.


<details>
  <summary>Details</summary>
Motivation: Legal AI lacks verifiable reward signals for training, creating a gap in reinforcement learning approaches. Mathematical AI has successfully used the "LLM proposes, verifier disposes" paradigm, but law's unique demands (including open-textured concepts) require an adapted solution.

Method: The architecture has three components: (1) LLM-driven autoformalization into a formal legal calculus extending Catala, (2) a verification kernel that checks correctness, and (3) explanation generation from formal proof traces.

Result: Demonstrated on three diverse legal domains: German procedural deadline calculations, U.S. Commerce Clause constitutional analysis, and cross-jurisdictional sanction proportionality. The architecture provides provable correctness for computational law and structural guarantees (mandatory stages addressed, correct argumentation placement, valid deductive links) for open-textured analysis. It also closes the RL training loop gap by providing a deterministic external verifier.

Conclusion: The proposed architecture successfully adapts formal verification methods from mathematics to law, enabling verifiable reward signals for legal AI training while accommodating both computational and open-textured aspects of legal reasoning.
...
### [143] [Catastrophic Compositional Generation: Why Vanilla Diffusion Models Fail to Extrapolate](https://arxiv.org/abs/2606.23920)
*Duncan Soiffer,Chandler Squires,Yuan Guan,Jason Hartford,Pradeep Ravikumar*

Main category: cs.LG

TL;DR: This paper argues that vanilla conditional diffusion models cannot efficiently perform compositional generation (producing samples from combined target distributions) when the target is out-of-distribution from training data, and shows that score estimation errors catastrophically degrade performance in such settings.


<details>
  <summary>Details</summary>
Motivation: To investigate whether conditional diffusion models can perform compositional generation—producing samples from geometrically combined target distributions when only trained on subset conditions—and to understand the fundamental limitations of inference-time techniques for this task.

Method: The authors use theory-guided generalization arguments combined with carefully-designed experiments on both synthetic and realistic data to test the feasibility of compositional generation. They compare approaches including Feynman-Kac correction against vanilla conditional diffusion models.

Result: Score estimation error has a catastrophic effect on compositional generation performance when the target distribution is out-of-distribution with respect to training sources. Even advanced inference-time methods like Feynman-Kac correction fail to overcome this limitation.

Conclusion: Compositional generation is often infeasible for vanilla conditional diffusion models in certain well-motivated settings, suggesting that fundamentally different approaches are needed rather than just improved inference-time corrections.
...
### [144] [KLip-PPO: A per-sample KL perspective on PPO-Clip](https://arxiv.org/abs/2606.23932)
*Riccardo Colletti,Robin Holzinger*

Main category: cs.LG

TL;DR: PPO's clipped surrogate gradient is exactly reproduced by a Kullback-Leibler penalty with per-sample adaptive coefficients, revealing clipped surrogate as a step-function penalty at trust region boundaries.


<details>
  <summary>Details</summary>
Motivation: To clarify the relationship between PPO's two forms (clipped surrogate and KL penalty) which have been treated as separate algorithms with distinct gradients, hyperparameters, and implementations, despite empirical comparisons being common.

Method: Mathematical derivation showing the clipped surrogate gradient equals a KL surrogate with per-sample varying coefficients, validated empirically on five MuJoCo continuous-control benchmarks.

Result: The gradient identity holds at every minibatch step across the entire inner loop; both losses produce indistinguishable training curves on MuJoCo benchmarks.

Conclusion: PPO-Clip's implicit per-sample penalty is a step function at trust region boundaries, and this coefficient shape is the natural design axis for generalizing the algorithm.
...
### [145] [DREG: A Layer-Wise Jacobian Regularization as a General-Purpose Penalty](https://arxiv.org/abs/2606.23942)
*Rowan Martnishn*

Main category: cs.LG

TL;DR: A large-scale study (960 experiments) shows Derivative Regularization (DREG) outperforms other regularizers overall and under GELU activation, excels in data-scarce settings, and works as a plug-and-play method without hyperparameter tuning.


<details>
  <summary>Details</summary>
Motivation: To systematically isolate and evaluate when, where, and why Derivative Regularization (DREG) works compared to other regularizers across diverse settings.

Method: 960 factorial experiments across 4 activations, 6 regularizers, 8 datasets, and 5 random seeds; DREG applied with fixed λ=10^{-2.5} without per-dataset tuning.

Result: DREG achieves highest overall/clean accuracy; ranks 2nd in noise robustness behind SN; best under GELU activation; most effective under data scarcity.

Conclusion: DREG is a plug-and-play regularizer that works by concentrating regularization pressure on layers with large activation derivatives, serving as a geometric inductive bias that substitutes for data volume.
...
### [146] [Learning the Koopman Operator using Attention Free Transformers](https://arxiv.org/abs/2606.23957)
*Mohammed Nagdi,Evangelos-Marios Nikolados,Alexey Yermakov,Mars Gao,Nathan Kutz,Filippo Menolascina*

Main category: cs.LG

TL;DR: The paper introduces attention-free latent memory (AFT) blocks and dynamic re-encoding mechanisms to make Koopman autoencoders more robust for long-horizon prediction, reducing drift and error accumulation on systems with switching, continuous spectra, or transients.


<details>
  <summary>Details</summary>
Motivation: Koopman operators with autoencoders enable linear latent-space prediction, but long-horizon rollouts drift off the learned manifold, causing phase/amplitude errors on systems with switching, continuous spectra, or strong transients.

Method: Two complementary components: (1) Attention-free latent memory (AFT) block that aggregates past latents for linear-time correction before each Koopman update; (2) Dynamic re-encoding using online change-point triggers (EWMA, CUSUM, sequential two-sample tests) to detect drift and project predictions back onto the manifold.

Result: On Duffing oscillator, Repressilator, and IRMA benchmarks, the model consistently reduces error accumulation vs. standard Koopman autoencoder and matched multi-head attention. Improves long-horizon prediction up to 1000 steps with lower inference latency.

Conclusion: A fast, compact predictor that stays on the learned manifold over long horizons, with AFT adding only ~30k parameters and linear-time complexity.
...
### [147] [Forget Without Compromise: Nexus Sampling for Streaming KV-Cache Eviction Under Fixed Budgets](https://arxiv.org/abs/2606.23961)
*Duc Duong,Hoang Anh Duy Le,Jianwen Xie,Anshumali Shrivastava,Zhaozhuo Xu*

Main category: cs.LG

TL;DR: Nexus Sampling is a training-free KV cache eviction method that uses iterative Nexus scoring (to surface bridge tokens) and weighted reservoir sampling (probabilistic retention instead of deterministic top-K), achieving dense-attention-level performance at 80% eviction and dominating deterministic methods in long-run preservation of subtly important tokens.


<details>
  <summary>Details</summary>
Motivation: Existing KV cache eviction methods use per-step direct attention scores followed by deterministic top-K selection, which permanently erases subtly important tokens that cannot be distinguished from noise by direct attention alone, especially problematic for long-context and agentic LLM workloads with continuous inference.

Method: Nexus Sampling combines (1) Nexus scoring, an iterative walk over direct attention scores to surface "bridge" tokens that are indirectly important, and (2) weighted reservoir sampling, which retains tokens probabilistically based on inclusion probability rather than deterministic top-K selection.
...
Conclusion: Probabilistic eviction via Nexus Sampling theoretically dominates deterministic top-K in long-run survival of subtly important tokens, offering a practical training-free solution for aggressive KV cache compression in continuous inference settings.
...
### [148] [3D Masked Autoencoders are Robust Learners of Volumetric and Multimodal Cellular Representations for Microscopy](https://arxiv.org/abs/2606.23964)
*Amirhossein Kardoost,Lion Gleiter,Tingying Peng,Carsten Marr*

Main category: cs.LG

TL;DR: This study compares 2D vs. 3D masked autoencoders on volumetric fluorescence microscopy data, showing that 3D modeling (MAE-3D) consistently outperforms 2D variants on downstream tasks. Cross-modal alignment with protein language model (ESM2) further boosts performance, achieving state-of-the-art results on protein-protein interaction (ROC-AUC 0.865) and protein localization (AUC_micro 0.952, F1_micro 0.742) tasks.


<details>
  <summary>Details</summary>
Motivation: Self-supervised learning in fluorescence microscopy typically relies on 2D projections despite cells being inherently 3D. This mismatch between data representation and biological reality motivates the need to investigate whether native 3D modeling improves representation learning.

Method: Systematic comparison of MAE-2D (max-projection and slice-based) vs. MAE-3D under matched architectures and training protocols. Visual representations are further aligned with a pretrained protein language model (ESM2). The model incorporates channel cross-attention and frequency-domain regularization to leverage 3D spatial context.

Result: MAE-3D consistently outperforms 2D variants. On protein-protein interaction: ROC-AUC 0.865 (+0.025 over prior). On protein localization: AUC_micro 0.952 (+0.003) and F1_micro 0.742 (+0.010), achieving state-of-the-art.

Conclusion: Native 3D modeling and multimodal alignment with protein language models offer clear advantages for representation learning in single-cell microscopy.
...
### [149] [A Comparative Study of Bayesian Contextual Bandits for Real-Time Warehouse Sorter Optimization](https://arxiv.org/abs/2606.23977)
*Tina Dongxu Li,Mouhacine Benosman,Ken Meszaros,Trevor Dardik*

Main category: cs.LG

TL;DR: This paper compares three hybrid ML frameworks (LR+GDO, XGB+BO, BCB) for real-time sorter diversion control in e-commerce warehouse MHS. BCB outperforms with 2.03% reward uplift over baseline, offering decisive time-optimal policy, online learning, and low inference latency.


<details>
  <summary>Details</summary>
Motivation: Current sorter diversion systems use static cost function weights that cannot adapt to dynamic warehouse contexts (volume, congestion, equipment status, dependencies), driving the need for adaptive real-time optimization.

Method: Comparative study of LR+GDO, XGBoost+Bayesian Optimization, and Bayesian Contextual Bandits using a high-fidelity physics-aware emulator for offline-to-online learning transition.

Result: BCB achieved 2.03% reward uplift over heuristic baseline, with decisive Bang-Bang control policy, continuous online learning, exploration-exploitation balance, and lower inference latency than tree-based models.

Conclusion: BCB framework shows strong potential for real-time control optimization in large-scale warehouse environments, motivating further operational deployment research.
...
### [150] [Offline Reinforcement Learning for Warehouse SLAM Throughput Control](https://arxiv.org/abs/2606.23978)
*Tina Dongxu Li,Mouhacine Benosman,Rajat Kumar,Kevin Tan,Ken Meszaros,Trevor Dardik*

Main category: cs.LG

TL;DR: This paper presents an offline RL framework for optimizing SLAM throughput control in warehouses, where the CQL policy improves system health by 22.97% and reduces throttling duration by 3.18%.


<details>
  <summary>Details</summary>
Motivation: To optimize SLAM throughput control in warehouse fulfillment environments to balance throughput maximization with downstream stability, addressing system congestion and operational efficiency.

Method: Uses an offline RL framework with history-informed state representation, action space abstraction for delayed-impact control, and a reward function capturing upstream/downstream metrics. Three offline RL algorithms (including CQL) are trained on historical operational logs and evaluated via model-free (regression, FQE) and model-based (Deep Koopman) methods.
...
Conclusion: Offline RL is a safe and scalable approach for warehouse throughput control optimization.
...
### [151] [Learning to Trigger: Reinforcement Learning at the Large Hadron Collider](https://arxiv.org/abs/2606.23993)
...
TL;DR: This paper presents a reinforcement learning approach for real-time optimization of trigger thresholds in high-energy physics experiments like the Large Hadron Collider, achieving significant improvements in signal efficiency and bandwidth compliance on both simulated and real collision data.


<details>
  <summary>Details</summary>
Motivation: High-throughput scientific facilities rely on static, hand-tuned trigger menus that become suboptimal over time as detector conditions, pileup, and background composition drift, leading to inefficiencies in bandwidth, latency, and storage usage.

Method: The authors formulate online threshold tuning as a sequential decision-making problem using reinforcement learning, adapting Group-Filtered Policy Optimization (GFPO) to streaming control with two variants (GFPO-F, GFPO-FR) that enforce background rate feasibility during training.

Result: On simulated streams, the agent improves in-tolerance time intervals by 48% (H_T trigger) and 28% (AD trigger), with up to 2% cumulative gain in signal efficiency. On real CMS Run 283408 data without fine-tuning, gains of 56% (H_T) and 28% (AD) in-tolerance improvement are achieved.

Conclusion: This is the first demonstration of RL-based trigger control on real LHC collision data, showing strong transferability from simulation to real-world deployment without fine-tuning.
...
### [152] [EMAgnet: Parameter-Space EMA Regularization for Policy Gradient Self-Play in Large Games](https://arxiv.org/abs/2606.23995)
*Tristan Maidment,JB Lanier,Chase McDonald,Nathan Tsang,Eugene Vinitsky,Roy Fox,Albert Wang,Wesley N. Kerr*

Main category: cs.LG

TL;DR: EMAgnet uses an exponential moving average of policy parameters as an adaptive regularization target in self-play, outperforming uniform-magnet PPO on most tested two-player zero-sum games, especially those with dominated strategies.


<details>
  <summary>Details</summary>
Motivation: Uniform regularization in self-play PPO treats all actions equally regardless of viability; the authors seek a more adaptive regularization target that evolves with the agent's improving strategy.

Method: EMAgnet regularizes policy gradients toward an exponential moving average (EMA) of the last-iterate policy parameters, creating a dynamic regularization target that adapts as the agent improves.

Result: EMAgnet achieves lower exploitability than PPO with uniform-magnet regularization (under both linear and power-law annealing schedules) in the majority of tested two-player zero-sum benchmarks, with consistent gains on games containing strictly dominated strategies.

Conclusion: Adaptive regularization via EMA of policy parameters is a more effective alternative to uniform regularization for self-play policy gradient methods in imperfect-information games.
...
### [153] [Cyclic Denoising Reveals Ultrastable Memories in Diffusion Models](https://arxiv.org/abs/2606.24000)
*Rishabh Sharma,Stefano Martiniani*

Main category: cs.LG

TL;DR: The paper introduces cyclic denoising, a technique that repeatedly applies forward and reverse diffusion at controlled noise levels to extract memorized training images from diffusion models, revealing ultrastable attractors corresponding to private or copyrighted data.


<details>
  <summary>Details</summary>
Motivation: To develop a method for extracting memorized training images from diffusion models that requires only sampler-level control, without gradients, weight access, prompts, or prior knowledge of training data, addressing privacy and copyright concerns.

Method: Cyclic denoising applies repeated forward (adding noise) and reverse (denoising) diffusion at controlled noise amplitudes, inspired by random organization in disordered solids. Samples are driven toward attractors; ultrastable attractors that survive thousands of noising-denoising cycles correspond to memorized training images.

Result: The attack successfully extracts memorized training images (stock photos, watermarks, web-crawl artifacts) from Stable Diffusion v1.4 and a pixel-space DDPM. A yielding-like transition occurs: low-amplitude cycling gives trivial fixed points, larger amplitudes produce structured memorized attractors. The method shows cross-initial-condition universality.

Conclusion: Cyclic denoising serves as both a physics-inspired probe of generative landscapes and a practical tool for memorization auditing, with implications for privacy, copyright compliance, and model fingerprinting.
...
### [154] [Fast and Slow Variational Continual Learning](https://arxiv.org/abs/2606.24007)
*Subarnaduti Paul,Yohan Jung,Mohammad Emtiyaz Khan,Siddharth Swaroop,Thomas Möllenhoff,Martin Mundt*

Main category: cs.LG

TL;DR: The paper introduces Continual IVON (CoVON), a simple optimizer that incorporates fast-slow adaptation into deep learning by merging past posteriors as priors in the VCL framework, showing consistent improvements over existing methods across domain-incremental learning, continual pre-training, and LLM fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Continual learning remains a major challenge for deep networks because common optimizers lack inherent mechanisms for continual adaptation, and while fast-slow adaptation is a natural biological mechanism, there is no consensus on how to incorporate it into optimizers.

Method: The authors incorporate slow adaptation via merging past posteriors (used as priors in VCL updates) to slow knowledge drift, then use the merged posterior as the prior in VCL for fast-weight updates, implemented seamlessly in the IVON optimizer (similar to Adam).

Result: CoVON consistently improves over existing VCL optimizers and performs better than other weight-regularization strategies across domain-incremental learning, continual pre-training, and fine-tuning of large language models.

Conclusion: The proposed CoVON optimizer offers an effective, simple, and low-cost solution to continual learning by naturally integrating fast-slow adaptation, with performance gains across multiple challenging settings.
...
### [155] [You Don't Need to Run Every Eval](https://arxiv.org/abs/2606.24020)
*Yuchen Zeng,Dimitris Papailiopoulos*

Main category: cs.LG

TL;DR: A score matrix of 84 models on 133 benchmarks is approximately rank-2, meaning model performance is captured by two latent factors. BenchPress, a rank-2 matrix completion method, recovers held-out scores to within ~4.6 points, and a subset of 5 benchmarks can predict the full scorecard with similar accuracy.


<details>
  <summary>Details</summary>
Motivation: To reduce the computational burden of running dozens of benchmarks repeatedly during model development by determining whether most evaluations are redundant.

Method: Compiled a public score matrix of 84 frontier models on 133 benchmarks (2,604 cells, 23.3% filled). Applied rank-2 matrix analysis and designed BenchPress, a logit-space rank-2 matrix completion method with a confidence layer for trustworthiness of predictions.

Result: The matrix is approximately rank-2: two factors explain over 90% of score variation. BenchPress recovers held-out scores within 4.6 points. A 5-benchmark subset (GPQA-D, HLE, Codeforces, MMLU-Pro, ARC-AGI-1) recovers full scorecards within 3.93 points.

Conclusion: Most evaluation benchmarks are redundant; a small set of carefully chosen benchmarks can accurately predict a model's performance across all 133 benchmarks, enabling substantial cost savings in model evaluation.
...
### [156] [Information-Theoretic Classifier-Free Guidance with Adaptive Schedule Optimization](https://arxiv.org/abs/2606.24025)
*Haobo Chen,Xiangxiang Xu,Yuheng Bu*

Main category: cs.LG

TL;DR: The paper proposes an information-theoretic framework to optimize classifier-free guidance (CFG) schedules in diffusion models, addressing the consistency-coverage trade-off by learning time-dependent guidance weights that outperform constant guidance on ImageNet and COCO benchmarks.


<details>
  <summary>Details</summary>
Motivation: CFG improves condition consistency but reduces diversity with stronger guidance; the trade-off is poorly understood across the reverse trajectory because the induced distribution differs from simple tilted distributions.

Method: An information-theoretic framework that uses a clean endpoint reference to specify the desired consistency-coverage trade-off and optimizes the actual sampler-induced distribution toward this reference using trajectory-level formulas estimated from samples and score evaluations.
...
Conclusion: The proposed optimization framework effectively controls the consistency-coverage trade-off in CFG by learning adaptive guidance schedules, outperforming constant guidance and providing a principled information-theoretic approach.
...
### [157] [RoPE-Aware Bit Allocation for KV-Cache Quantization](https://arxiv.org/abs/2606.24033)
*Fengfeng Liang,Yuechen Zhang,Jiaya Jia*

Main category: cs.LG

TL;DR: Block-GTQ is a RoPE-aware bit allocator for low-bit KV-cache quantization that treats key caching as a block-wise bit-allocation problem across 2D frequency blocks, achieving significant fidelity gains, downstream task improvements, and practical compression (3.24x) with faster inference and lower memory usage compared to fp16 baselines.


<details>
  <summary>Details</summary>
Motivation: Existing low-bit KV-cache quantizers treat each cached key as a flat vector, ignoring that under RoPE, a key's contribution to attention logits decomposes into a position-dependent sum over 2D frequency blocks. This oversight leads to suboptimal bit allocation.

Method: Block-GTQ computes a label-free energy score for each RoPE block per layer/KV head, then greedily allocates integer bit widths based on marginal gain, built on top of TurboQuant-MSE (TQ-MSE).

Result: Block-GTQ cuts per-layer MAE by 32-80% at 2 and 3 b/dim K-only quantization, wins all 367/367 layer comparisons against uniform TQ-MSE. On Llama-3.1-8B-Instruct at K2V2, NIAH average rises from 70.6 to 97.4, LongBench-EN from 36.87 to 53.31. On DeepSeek-R1-Distill-Qwen-7B at K3V2 (without fp16 recent-key buffer), AIME 2024/2025 scores hit 51.7/37.5 vs fp16's 54.2/37.9, while uniform TQ-MSE collapses to 0.0/0.0. Packed K3V3 on Qwen2.5-3B-Instruct achieves 3.24x compression, 1.34x speedup over fp16 FlashAttention2 at 128K context, and reduces peak memory from 56.31 GB to 19.85 GB.

Conclusion: Block-GTQ demonstrates that RoPE-aware block-wise bit allocation significantly outperforms uniform quantization for KV-cache compression, delivering both better fidelity and practical system-level benefits (higher throughput, lower memory), making long-context inference with low-bit KV-cache feasible where fp16 would run out of memory.
...
### [158] [Rapid FinFET Modelling Using an Autoencoder](https://arxiv.org/abs/2606.24046)
*Amit Sarkar Suman Sau,Swagata Mandal*

Main category: cs.LG

TL;DR: A machine learning framework using autoencoders for efficient FinFET modeling, which compresses I-V curves into latent space and reconstructs them to extract key device metrics like VTH, SS, and gm, with VDS explicitly incorporated as a feature.


<details>
  <summary>Details</summary>
Motivation: To develop a data-driven compact model for FinFET that can achieve high accuracy with minimal training data, enabling rapid device characterization and circuit simulation.

Method: Calibrated BSIM-CMG model to generate ID-VG dataset → Trained autoencoder to compress I-V curves into low-dimensional latent space → Explicitly incorporated VDS as input feature → Model reconstructs full I-V curves and extracts VTH, SS, and gm.

Result: The autoencoder-based model successfully reconstructs full I-V curves and directly extracts critical device metrics (VTH, SS, peak gm) with high accuracy using minimal training data.

Conclusion: Data-driven compact models built from actual characterization data can achieve high accuracy with minimal data, providing a powerful tool for rapid device characterization, modeling, and circuit-level simulation.
...
### [159] [RAVEN: A Regime-Aware Variable-context Expert Network for Financial Time Series Forecasting](https://arxiv.org/abs/2606.24062)
*Cheng He,Zhenyu Guan,Xijie Liang,Defu Lian,Jiajia Li,Enhong Chen,Patrick P. C. Lee,Geng Hu,Zehao Chen*

Main category: cs.LG

TL;DR: RAVEN is a Mixture-of-Experts framework that uses adaptive variable-length context windows and cumulative importance thresholding to outperform SOTA models on financial time series forecasting and traffic benchmarks.


<details>
  <summary>Details</summary>
Motivation: Financial time series have non-stationary log-returns, low SNR, and regime-dependent dependencies; fixed context windows fail to match the time-varying optimal look-back horizon of such processes.

Method: RAVEN constructs nested contiguous windows via Cumulative Importance Thresholding (CIT) on patch importance scores, routes each window to a scale-specialized expert, runs a Global Compressed Representation (GCR) branch in parallel, and aggregates expert outputs with Correlation-Aware Weighting (CAW) to penalize cosine similarity.

Result: RAVEN achieved SOTA results: +9.2% Pearson on HS300, +20.2% on S&P500, -18.2% MSE on fund sales, and best in 14/16 metrics on PEMS traffic benchmarks.

Conclusion: Adaptively determining temporal context per sample via nested window routing and correlation-aware aggregation significantly improves forecasting performance on non-stationary time series.
...
### [160] [Blockwise Policy-Drift Gating for On-Policy Distillation](https://arxiv.org/abs/2606.24084)
*Liwen Zheng,Haiyun Jiang*

Main category: cs.LG

TL;DR: Blockwise policy-drift gating improves on-policy distillation by using old-current log-probability shifts over fixed token blocks to reweight losses, boosting pass@8 from 0.4978 to 0.5160 on math reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Sampled-token on-policy distillation is fragile on long-horizon reasoning tasks, and existing local teacher-support matching repairs are insufficient; the paper aims to control policy drift under rollout reuse with a student-only mechanism.

Method: Blockwise policy-drift gating computes log-probability shifts between behavior and current student over fixed token blocks, applies detached mean-normalized gates to reweight OPD position losses, without altering teacher targets or rollout policy.

Result: On a Qwen3 math reasoning benchmark with six variants and 200-step training budget, Block64 gating improves sampled-token OPD mean pass@8 from 0.4978 to 0.5160 across AIME24, AIME25, MATH500, and AMC23, achieving best overall performance among trained students.

Conclusion: Local old-current policy drift is a practical control signal for reused OPD rollouts, and block-level gating is a simple, effective default for improving solve-rate robustness.
...
### [161] [NeuroSonic: Conditional Flow Matching for EEG-to-Speech Reconstruction](https://arxiv.org/abs/2606.24087)
*Wenhao Gao,Yifan Wang,Yijia Ma,Carl Yang,Wen Li,Chenyu You*

Main category: cs.LG

TL;DR: NeuroSonic is a conditional flow-matching framework that reconstructs speech from scalp EEG by learning a deterministic probability-flow velocity field, outperforming GAN, diffusion, and mean-flow baselines with up to 26.3% perceptual quality improvement on CineBrain and EAV benchmarks.


<details>
  <summary>Details</summary>
Motivation: EEG provides weak, diffuse, and variable cortical measurements, while speech has coherent harmonic structure. Direct waveform regression is unstable, and stochastic multi-step generation suffers from artifact-dependent conditioning and subject variability.

Method: NeuroSonic uses conditional flow-matching: it learns a deterministic probability-flow velocity field transporting noise-corrupted acoustic states to clean speech under EEG conditioning. EEG and audio are embedded into a shared token space and processed by a time-conditioned gated Transformer parameterizing a transport ODE, avoiding iterative stochastic sampling.

Result: On CineBrain and EAV benchmarks under cross-subject evaluation, NeuroSonic improves distributional realism, spectral fidelity, and perceptual quality over GAN, diffusion, and mean-flow baselines, with up to 26.3% gain in overall perceptual quality. The gap is largest in artifact-heavy segments.

Conclusion: Deterministic conditional transport provides a stable and effective formulation for EEG-driven speech reconstruction, addressing the fundamental mismatch between noisy EEG signals and coherent speech structure.
...
### [162] [FedUP: One-Shot Federated Unlearning via Centroid-Guided Plug-in Filters](https://arxiv.org/abs/2606.24113)
*Feihong Nan,Zhengyi Zhong,Pan Wang,Weidong Bao,Xiongtao Zhang,Quan Wen,Ji Wang*

Main category: cs.LG

TL;DR: FedUP is a one-shot federated unlearning framework using pluggable filters ("knowledge funnel") with DP-protected class centroids to achieve fast (seconds vs minutes), precise unlearning without multi-round communication, while enabling reversible unlearning by simply removing filters.


<details>
  <summary>Details</summary>
Motivation: Current federated unlearning methods suffer from a dilemma between non-target knowledge loss (forgetting too much) and high request latency (slow response time due to multi-round communication and retraining).

Method: FedUP freezes original model parameters and trains lightweight pluggable filters at the server side using differentially private (DP)-protected class centroid samples. These filters act as a "knowledge funnel" to screen out target data in a one-shot manner without client-server multi-round communication.
...
Conclusion: FedUP resolves the core dilemma in federated unlearning through a pluggable, reversible, one-shot framework that is both efficient and precise, with inherent reversibility for knowledge restoration.
...
### [163] [When Top-1 Fails: Calibrating LoRA Monitors for Masked Diffusion LMs](https://arxiv.org/abs/2606.24119)
*Lucky Verma,Pratik Yadav*

Main category: cs.LG

TL;DR: Top-1 argmax concentration is ineffective as a collapse warning for DLM LoRA fine-tuning (0/816 precision). Max LoRA gradient norm works better (precision 0.68, F1=0.79) but is limited to short-horizon DLM-LoRA inspection within the same model family.


<details>
  <summary>Details</summary>
Motivation: To test whether denoising-time confidence monitors (specifically top-1 argmax concentration) are reliable as collapse warnings in PEFT (LoRA) settings for discrete diffusion language models, since their meaning in this context was previously untested.

Method: Evaluated 816 LoRA/PEFT configurations across three DLM families, testing top-1 argmax concentration vs. max LoRA gradient norm as collapse detectors. Used pooled held-out LLaDA-family split with train-optimized thresholding.

Result: Top-1 argmax concentration fired warnings for all 816 configurations with zero actual collapses (precision=0). Max LoRA gradient norm achieved precision 0.68 and F1=0.79 on LLaDA-family split, above the all-positive baseline.

Conclusion: Drop top-1 as a PEFT alarm; use max-gradient early in training; calibrate thresholds per DLM family. Results are bounded to short-horizon DLM-LoRA inspection, not a universal collapse detector.
...
### [164] [Holistic Data Scheduler for LLM Pre-training via Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2606.24133)
*Chenhao Dang,Jing Ma,Mingjie Liao*

Main category: cs.LG

TL;DR: HDS is a novel online data mixing framework that uses reinforcement learning (SAC algorithm) with a multi-objective reward function to dynamically adjust data mixtures during LLM pre-training, achieving better efficiency and performance than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing Online Data Mixing (ODM) methods rely on a singular optimization perspective, which fails to account for the multi-dimensional dynamics needed in complex LLM pre-training.

Method: Formulates data scheduling as a continuous control RL problem using Soft Actor-Critic (SAC), with a multi-objective reward function combining data-driven (quality), loss-driven (inter-domain influence), and model-driven (weight norms) rewards.

Result: On The Pile, HDS reaches the next best method's validation perplexity with 44% fewer iterations; achieves 7.2% improvement on MMLU 0-shot with consistent gains on other benchmarks.

Conclusion: HDS demonstrates that multi-objective, holistic reward design in an RL-based data scheduling framework significantly improves both training efficiency and final LLM capabilities.
...
### [165] [A Time-Reparameterized Cumulative Intensity Extrapolation Sampler for Discrete Flow Matching](https://arxiv.org/abs/2606.24140)
*Feiyang Fu,Hehe Fan*

Main category: cs.LG

TL;DR: The paper proposes TR-CIE, a sampler for discrete flow matching that improves sampling quality under limited function evaluations via time reparameterization and cumulative-intensity extrapolation.


<details>
  <summary>Details</summary>
Motivation: Efficient sampling for discrete flow matching under a limited number of function evaluations (NFE) remains less studied, despite its practical importance.

Method: TR-CIE combines (1) schedule-based time reparameterization to rescale the time grid and mitigate stiffness, and (2) cumulative-intensity extrapolation that reuses cached model outputs to improve approximation on non-uniform time grids.

Result: Theoretical bounds on local approximation error and convergence are established. Experiments on synthetic tasks, text generation, and text-to-image benchmarks show improved sampling quality under limited NFE.

Conclusion: TR-CIE enhances sampling efficiency for DFM without additional model evaluations, offering practical gains under constrained function evaluations.
...
### [166] [AsyncOPD: How Stale Can On-Policy Distillation Be?](https://arxiv.org/abs/2606.24143)
...
TL;DR: This paper systematically studies the effects of stale data in asynchronous on-policy distillation (OPD) for LLM post-training, proposing AsyncOPD which achieves 1.6-3.8x throughput improvement over synchronous training while maintaining comparable accuracy.


<details>
  <summary>Details</summary>
Motivation: On-policy distillation (OPD) faces a systems bottleneck due to rollouts dominating training time. Asynchronous training can alleviate this but introduces stale-policy data, whose effects in OPD are underexplored. The authors aim to systematically study staleness in asynchronous OPD under practical constraints like expensive teacher logits.

Method: The authors study staleness in asynchronous OPD with local KL losses and finite teacher-score caches. They analyze KL direction effects, test asynchronous RL stabilization methods, examine bias-variance tradeoffs from finite caches, and propose multi-sample Monte Carlo for sparse reverse-KL estimators. They build and open-source AsyncOPD, a fully asynchronous training pipeline.

Result: Forward KL is more robust to stale rollouts than reverse KL. Existing asynchronous RL methods do not improve over a simpler OPD-specific surrogate (recomputing reverse-KL under current student). Multi-sample MC reduces variance while maintaining correctability. AsyncOPD achieves 1.6x to 3.8x throughput improvement with comparable accuracy.

Conclusion: Asynchronous OPD can significantly improve training throughput with minimal accuracy loss, but careful estimator design (KL direction, staleness handling, cache management) is crucial for effective deployment.
...
### [167] [Lightweight Transformer Models for On-Device Fault Detection: A Benchmark Study on Resource-Constrained Deployment](https://arxiv.org/abs/2606.24173)
*Disha Patel*

Main category: cs.LG

TL;DR: This paper benchmarks traditional ML (Random Forest, XGBoost, SVM, Logistic Regression) vs. lightweight transformers (DistilBERT, TinyBERT variants, MobileBERT) for on-device fault detection across three datasets, finding transformers match traditional ML on well-separated data but at 100x size and 9000x latency cost, while both methods fail on severely imbalanced datasets.


<details>
  <summary>Details</summary>
Motivation: To enable real-time fault detection on resource-constrained hardware without cloud dependency, systematic benchmarks comparing traditional ML and lightweight transformers are needed to understand tradeoffs between accuracy, latency, and model size.

Method: Benchmarked traditional ML (RF, XGBoost, SVM, LR) vs. lightweight transformers (DistilBERT, TinyBERT-6L, TinyBERT-4L, MobileBERT) on three public datasets (C-MAPSS, SECOM, UCI AI4I 2020) using F1-score, AUC, model size, and CPU latency. Also evaluated INT8 quantization and a two-stage adaptive inference pipeline.

Result: On C-MAPSS, lightweight transformers match traditional ML at 87.8% F1 but at 100x model size and 9000x latency. TinyBERT-4L is most deployment-friendly (55 MB, 18 ms). INT8 quantization reduces size 25% while preserving 86.9% F1. Adaptive pipeline achieves 87.6% F1 at 19.5 ms average latency. On imbalanced datasets (SECOM, UCI-PM), all methods struggle significantly.

Conclusion: Traditional ML remains far more practical for on-device fault detection than lightweight transformers in terms of size and latency. Neither approach handles extreme class imbalance well, indicating a fundamental limitation that needs further research.
...
### [168] [Project Ariadne: Prompt-Conditioned Route Generation for Synthesis Planning](https://arxiv.org/abs/2606.24184)
*Anton Morgunov,Victor S. Batista*

Main category: cs.LG

TL;DR: Ariadne is a decoder-only transformer that generates full retrosynthetic routes from a single prompt, supporting constraints like depth and required starting materials without specialized algorithms, achieving competitive results on benchmark tasks with faster inference than prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing retrosynthetic planning methods either require iterative search procedures with specialized algorithms, or use direct route generation but need separate models for different planning constraints. The authors aim to unify these into a single prompt-conditioned model.

Method: Ariadne is a decoder-only transformer that represents the target molecule, optional constraints (e.g., route depth, required starting materials), and the full retrosynthetic route as a single prompt-completion sequence, enabling conditional route generation without architectural changes.

Result: On RetroCast/PaRoutes benchmarks, Ariadne improves Solv-0 by 13.7 points (depth) and 31.2 points (required-leaf) over baselines. It outperforms DESP on required-leaf Top-10/Solv-0 in 24 GPU-minutes vs. 6.8 GPU-hours. Comparable to DMS Explorer XL with half the inference time.

Conclusion: Sequence generation can be extended from specialist retrosynthesis models to prompt-conditioned structural route generation. However, lack of Tier-1--3 route checkers remains the main bottleneck for practical adoption by experimental chemists.
...
### [169] [Managing Task Execution for Unknown Workloads in Batteryless IoT: A Hardware-Agnostic Evaluation](https://arxiv.org/abs/2606.24340)
*Samer Nasser,Henrique Duarte Moura,Ritesh Kumar Singh,Maarten Weyn,Jeroen Famaey*

Main category: cs.LG

TL;DR: This paper proposes two hardware-agnostic dynamic scheduling strategies (RL and Approximated Prediction) for batteryless IoT devices, evaluating them against existing methods under real-world energy conditions, revealing distinct trade-offs between throughput, survival-execution balance, and complexity based on capacitor size.


<details>
  <summary>Details</summary>
Motivation: Traditional energy-aware schedulers for batteryless IoT devices rely on static execution thresholds or pre-measured hardware-specific profiles, which struggle with unpredictable workloads and growing edge application complexity.

Method: Two hardware-agnostic dynamic scheduling strategies are proposed: (1) a model-free Reinforcement Learning (RL) agent, and (2) an on-the-fly Approximated Prediction (AP) method. Both treat applications as a "black box" requiring no prior energy information. Evaluation uses a custom simulation framework with real-world solar data and dynamic LoRa transmission profiles, comparing against AsTAR and optimized static thresholds.

Result: AP achieves lightweight, near-oracle task throughput; RL provides tunable survival-execution balancing; AsTAR excels at execution pacing across long energy gaps. Advanced strategies benefit severely constrained systems with small capacitors, while larger energy buffers can efficiently use simpler static policies.

Conclusion: No single strategy is universally superior; the choice depends on hardware constraints (capacitor size) and operational priorities. The findings guide scheduler selection for batteryless IoT systems based on energy buffer capacity.
...
### [170] [Parallel Manifold Steering: Efficient Adaptation of Large Associative Memories via Residual Energy Shaping](https://arxiv.org/abs/2606.24396)
*Kanishk Awadhiya*

Main category: cs.LG

TL;DR: H-Res (Hierarchical Residual Steering) is a method for adapting frozen Transformers to new tasks by learning state-dependent vector fields that steer token trajectories into task-specific attractor basins, avoiding catastrophic interference and computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing methods for adapting frozen Transformers face a Plasticity-Stability dilemma: weight-modification approaches (e.g., LoRA) risk catastrophic interference, while prompt-based methods (e.g., VPT) degrade associative capacity by clogging the retrieval buffer with static tokens.

Method: H-Res formulates adaptation as a control problem on the activation manifold, learning a state-dependent vector field that steers token trajectories into task-specific basins of attraction within the Transformer's energy landscape, without altering weights or expanding sequence length.

Result: H-Res outperforms global weight modification by 26% on associative retrieval tasks, eliminates computational overhead of prompt-based methods, scales effectively to structured domains, preserves attention entropy, and facilitates Neural Collapse.

Conclusion: Manifold steering via H-Res offers a principled and efficient alternative to both weight-tuning and prompt-based adaptation, resolving the Plasticity-Stability dilemma for frozen Transformer models.
...
### [171] [Natural Identifiers for Privacy and Data Audits in Large Language Models](https://arxiv.org/abs/2606.24408)
*Lorenzo Rossi,Bartłomiej Marek,Franziska Boenisch,Adam Dziedzic*

Main category: cs.LG

TL;DR: This paper introduces natural identifiers (NIDs)—structured random strings like hashes and URLs found in LLM training data—to enable post-hoc privacy auditing without retraining and dataset inference without requiring private held-out datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods for auditing differential privacy require inserting canary data during training, and dataset inference needs a private non-member held-out dataset from the same distribution—both impractical for already-trained models or real-world cases due to unavailability or difficulty of constructing such datasets.

Method: The authors propose using natural identifiers (NIDs), which are structured random strings (e.g., cryptographic hashes, shortened URLs) that naturally appear in LLM training data. Their format allows generating unlimited random strings from the same distribution, serving as alternative canaries for DP auditing and as same-distribution held-out data for dataset inference.

Result: Evaluation shows that NIDs can facilitate post-hoc differential privacy auditing without any retraining and enable dataset inference for any suspect dataset containing NIDs without needing a private non-member held-out dataset.

Conclusion: NIDs provide a scalable, practical solution for post-hoc privacy audits of LLMs, overcoming key limitations of existing methods.
...
### [172] [Data Augmentation: A Fourier Analysis Perspective](https://arxiv.org/abs/2606.24418)
*Behrooz Tahmasebi,Melanie Weber,Stefanie Jegelka*

Main category: cs.LG

TL;DR: This paper formally analyzes partial vs. full data augmentation for group-invariant learning, showing that partial augmentation with random subset sampling achieves the same minimax rates as full augmentation (with vanishing approximation error), while proving that exact invariance requires full group averaging when the hypothesis space is expressive.


<details>
  <summary>Details</summary>
Motivation: Full group-sized data augmentation is computationally infeasible for large groups, yet it's unclear whether partial augmentation retains the same statistical benefits. The authors aim to bridge this gap between computational efficiency and statistical optimality.

Method: The authors develop a Fourier analysis and representation theory framework for finite groups, analyzing partial augmentation via random subset sampling of group elements, and proving both achievability and impossibility results.

Result: Partial augmentation with random subset sampling achieves the same minimax rates as full augmentation, with approximation error that vanishes as subset size grows. Conversely, exact invariance via augmentation requires averaging over the entire group for sufficiently expressive hypothesis spaces.

Conclusion: Partial data augmentation provides a computationally feasible way to retain the statistical benefits of full symmetry enforcement, offering a principled trade-off between computational cost and approximation accuracy.
...
### [173] [An LLM-based Two-Stage Transformer Framework for Cross-Domain Bearing Fault Diagnosis with Limited Data](https://arxiv.org/abs/2606.24459)
*Jinghan Wang,Feng Cheng,Wentao Wu,Hang Li,Gaoliang Peng,Tianchen Liu*

Main category: cs.LG

TL;DR: A knowledge-guided two-stage transfer learning framework using a lightweight GPT-2-style Transformer is proposed for bearing fault diagnosis, achieving 92.61% accuracy with only 10% labeled data and outperforming SOTA by 17.24 percentage points.


<details>
  <summary>Details</summary>
Motivation: Bearing fault diagnosis faces critical challenges when dataset heterogeneity, operating condition variations, and limited labeled data occur simultaneously in industrial environments, but existing approaches address these issues in isolation and rely on implicit feature alignment.

Method: A knowledge-guided two-stage transfer learning framework with a lightweight GPT-2-style Transformer using causal self-attention for hierarchical feature extraction, employing multi-source pre-training, prototype-based knowledge modulation, and taxonomy-adaptive classification.

Result: 92.61% average accuracy with only 10% labeled target data, outperforming state-of-the-art methods by 17.24 percentage points on four real-world datasets.

Conclusion: The framework establishes a practical pathway toward cost-effective predictive maintenance in Industry 4.0 applications by addressing concurrent challenges of heterogeneity, condition variations, and limited labels.
...
### [174] [A Fair Evaluation of Graph Foundation Models for Node Property Prediction](https://arxiv.org/abs/2606.24509)
*Oleg Platonov,Gleb Bazhenov,Dmitry Eremeev,Liudmila Prokhorenkova*

Main category: cs.LG

TL;DR: This paper reevaluates 9 recent Graph Foundation Models (GFMs) for node property prediction and finds that only the most recent Prior-data Fitted Networks-based GFMs outperform well-tuned GNNs, but at higher inference cost.


<details>
  <summary>Details</summary>
Motivation: Graph Foundation Models for node property prediction have been rapidly emerging, but there is no unified evaluation standard, making it impossible to reliably compare different GFMs against each other or against strong baselines.

Method: The authors conduct a fair and rigorous reevaluation of 9 recent GFMs for node property prediction, comparing them against strong Graph Neural Network (GNN) baselines under a unified setting.

Result: Only the most recent GFMs based on the Prior-data Fitted Networks paradigm outperform well-tuned GNNs in predictive performance, albeit at a higher inference cost.

Conclusion: The field lacks standardized evaluation; most current GFMs do not surpass well-tuned GNNs, and the few that do come with a significant computational overhead.
...
### [175] [Reasoning as Attractor Dynamics: Latent Memory Retrieval via Gibbs-Weighted Energy Minimization](https://arxiv.org/abs/2606.24543)
*Kanishk Awadhiya*

Main category: cs.LG

TL;DR: The paper reinterprets LLMs as Dense Associative Memories where reasoning is an energy-based settling process. They propose a Gibbs-sampling retrieval mechanism that weights reasoning paths by spectral entropy, improving Phi-3.5 on GSM8K by 5.38%.


<details>
  <summary>Details</summary>
Motivation: Traditional autoregressive view of LLMs fails to explain hallucinations and reasoning robustness. The authors posit that viewing LLMs through the lens of collective computation and energy landscapes can provide a better theoretical foundation for understanding and improving reasoning.

Method: 1) Model reasoning as an energy landscape where correct chains are "flat minima" and hallucinations are "sharp local minima." 2) Introduce a Gibbs measure-based retrieval using spectral entropy of trajectories. 3) Sample multiple reasoning paths and weight by inverse energy (P ∝ e^{-βE}) to approximate equilibrium distribution.

Result: Applied to Microsoft Phi-3.5, the method achieves 90.1% on GSM8K, a 5.38% improvement over the baseline 84.7%.

Conclusion: Inference in LLMs is better understood as a dynamic settling process into attractor basins rather than greedy autoregressive prediction, and physics-inspired sampling mechanisms can significantly improve reasoning reliability.
...
### [176] [QC-SMOTE: Quality-Controlled SMOTE for Imbalanced Classification](https://arxiv.org/abs/2606.24625)
*Parth Upman,Shreyank N Gowda*

Main category: cs.LG

TL;DR: QC-SMOTE is a quality-controlled oversampling method that generates synthetic minority samples using neighborhood trustworthiness scores, adaptive interpolation, and graceful degradation to duplicates in noisy regions, outperforming existing methods on 30 imbalanced datasets.


<details>
  <summary>Details</summary>
Motivation: Class imbalance causes poor classification performance, and existing oversampling methods like SMOTE often produce low-quality synthetic samples in noisy or overlapping regions.

Method: QC-SMOTE uses a composite neighborhood trustworthiness score (density, safe-level, majority isolation) to evaluate minority sample reliability, generates candidates via IPQ-guided best-of-K strategy with midpoint purity checks, adapts interpolation to local geometry, and replaces low-quality synthetics with original duplicates under an adaptive threshold.

Result: On 30 imbalanced datasets with repeated stratified cross-validation, QC-SMOTE achieves the strongest average AUC-ROC and Macro F1 among compared oversampling methods, especially under moderate and severe imbalance.

Conclusion: Quality-aware, geometry-adaptive synthetic sampling is crucial for robust imbalanced classification, as demonstrated by QC-SMOTE's consistent superiority.
...
### [177] [FlowPipe: LLM-Enhanced Conditional Generative Flow Networks for Data Preparation Pipeline Construction](https://arxiv.org/abs/2606.24679)
*Kunyu Ni,Lei Cao,Jie He,Xiaotong Zhang,Jianfeng Jin,Junyu Dong,Yanwei Yu*

Main category: cs.LG

TL;DR: FlowPipe is a unified framework for automatic ML data pipeline synthesis that uses Conditional Generative Flow Networks with Trajectory Balance, LLM-based semantic modulation, and failure awareness, achieving 11.96% accuracy improvement and 12.5x faster convergence over SOTA baselines on 74 datasets.


<details>
  <summary>Details</summary>
Motivation: Automatically constructing data preparation pipelines is computationally difficult due to combinatorial operator sequences and expensive end-to-end evaluation. Existing SOTA Multi-DQN methods suffer from decoupled value estimators weakening long-horizon credit assignment, weak injection of dataset context into policy, and inefficient exploration in sparse search spaces with many invalid states.

Method: FlowPipe formulates pipeline synthesis as conditional probabilistic flow generation over a DAG using C-GFlowNets with Trajectory Balance objective to connect terminal validation rewards with early decisions. It introduces Deep Semantic Modulation via FiLM to inject LLM-derived logical priors conditioned on dataset semantics, and incorporates failure awareness into the flow objective to avoid invalid states and focus search.
...
Conclusion: FlowPipe effectively addresses key limitations of prior pipeline synthesis methods and demonstrates superior performance and efficiency. The source code is publicly available.
...
### [178] [Grad Detect: Gradient-Based Hallucination Detection in LLMs](https://arxiv.org/abs/2606.24790)
*Anand Kamat,Daniel Blake,Brent M. Werness*

Main category: cs.LG

TL;DR: Grad Detect is a gradient-based method that detects LLM hallucinations by analyzing layer-wise gradient patterns from a single forward-backward pass, outperforming existing methods and revealing that over 97% of discriminative signal concentrates in the final five layers.


<details>
  <summary>Details</summary>
Motivation: LLMs are prone to hallucinations, and detecting these errors is critical for reliable deployment in high-stakes applications. Existing methods relying on output-level signals are insufficient.

Method: Grad Detect predicts hallucinations by analyzing layer-wise gradient patterns obtained from a single forward-backward pass during inference, leveraging internal gradient structure rather than output-level signals alone.

Result: Outperforms confidence-based and sampling-based baselines across Q&A benchmarks for both hallucination detection and model abstention prediction. Layer ablation across 11 models shows the final 5 layers contain over 97% of discriminative gradient signal.

Conclusion: Grad Detect offers a unified framework for predicting multiple dimensions of LLM reliability with strong performance and interpretable insights into model failure origins, and can be deployed efficiently by focusing on final layers.
...
### [179] [Real vs. Complex Spectral Bases for Neural Operators: The Role of Green's Function Alignment](https://arxiv.org/abs/2606.24851)
*Jason Sulskis,Sathya Ravi*

Main category: cs.LG

TL;DR: This paper proposes the Hartley Neural Operator (HNO), a real-valued alternative to Fourier Neural Operators (FNO), showing that FNO is better for time-dependent PDEs (with phase) while HNO excels for self-adjoint elliptic PDEs, providing a predictive rule to choose the spectral basis based on operator symmetry.


<details>
  <summary>Details</summary>
Motivation: FNO uses complex FFT which carries representational redundancy (conjugate symmetry) for real-valued PDE solutions. This motivates a purely real alternative that may be more efficient or better suited for certain PDE classes.

Method: Replace the FFT with the Discrete Hartley Transform (purely real), learning one real multiplier per retained spectral mode instead of complex pairs. This makes HNO iso-parametric with FNO but differing in spectral basis.

Result: Self-adjoint elliptic operators (Poisson, biharmonic) favor HNO; time-dependent operators with phase (wave, advection, Burgers, Navier-Stokes) favor FNO; heat equation is the borderline case. The split is monotone in operator phase content.

Conclusion: There is no universal winner; the best spectral basis depends on operator symmetry—match the basis to the solution operator's properties for best performance.
...
### [180] [Verifiable Foundation Models for Robot Safety](https://arxiv.org/abs/2606.23754)
*Davide Corsi,Kyungmin Kim,Roy Fox*

Main category: cs.RO

TL;DR: FEARL separates robot control policies into a large perception/task Controller and a small formally-verifiable Safety module, enabling formal verification of safety properties without sacrificing expressive power.


<details>
  <summary>Details</summary>
Motivation: Foundation models used for robot control are expressive but opaque and difficult to formally verify, creating a tension between rich multimodal perception and safety assurance.

Method: FEARL decomposes policy into a large Controller (C) for high-dimensional perception and task reasoning, and a small Safety module (S) that uses low-dimensional safety sensor observations plus a bounded context embedding from C, enabling formal verification on S alone.

Result: Evaluated on three simulated robotic domains with multiple Controller backbones (including pretrained VLAs); successfully transferred a learned policy to a physical robot, demonstrating sim-to-real feasibility.

Conclusion: The modular decomposition makes formal verification tractable while preserving the Controller's expressive power, and supports practical sim-to-real transfer via the low-dimensional safety interface.
...
### [181] [Engineering Reliable Autonomous Systems: Challenges and Solutions](https://arxiv.org/abs/2606.23760)
...
TL;DR: This workshop report summarizes the ERAS 2024 workshop that brought together academia and industry to create a roadmap for engineering reliable autonomous systems, focusing on verification/validation, real-world engineering, and software architectures.


<details>
  <summary>Details</summary>
Motivation: As autonomous systems become more prevalent, there is an increasing need for easy-to-use techniques to build them reliably. The workshop aimed to bridge academia and industry to address these reliability challenges.

Method: The Lorentz Center Workshop "Engineering Reliable Autonomous Systems" (ERAS) was held June 10-14, 2024, co-organised by FMAS and AREA communities, bringing together academics, industry practitioners, and sector representatives. It focused on three research topics through collaborative discussions.

Result: A catalogue of challenges was produced, identifying which challenges can be addressed by existing academic techniques not yet adopted in practice, and which require further research.

Conclusion: The roadmap developed at the workshop is intended to support future research and industrial collaboration toward engineering reliable autonomous systems.
...
### [182] [Enforcing Human-like Kinematics in Dexterous Piano Playing via Adversarial Posture Regularization](https://arxiv.org/abs/2606.23848)
*Bin Qiu,Yanming Shao,Guanyu Cai,Yao Mu*

Main category: cs.RO

TL;DR: APR uses adversarial posture regularization with casual human playing data to produce more natural, human-like hand movements for reinforcement learning-based piano playing with dexterous hands.


<details>
  <summary>Details</summary>
Motivation: High-DoF dexterous hands trained with RL or IK for piano playing often produce unnatural postures and joint overextension; expert demonstrations are expensive and song-aligned data is hard to obtain.

Method: Proposes Adversarial Posture Regularization (APR) which matches policy posture distribution to human prior via adversarial objective using a small amount of casual human playing data collected with Meta Quest 3 and retargeted to Shadow Hand.

Result: Significantly better performance than prior methods on all three human-likeness metrics (cPSI, BSE, FAC) and superior visual quality.

Conclusion: APR effectively regularizes dexterous hand policies to produce more human-like postures without requiring expensive expert demonstrations, validated through quantitative metrics and visual quality.
...
### [183] [Topological Online Learning for Displacement-based Formation Control](https://arxiv.org/abs/2606.23901)
*Saksham Sharma,Shubhankar Gupta,Sumant A Gunagi,Suresh Sundaram*

Main category: cs.RO

TL;DR: TOLD is a real-time edge-level adaptation framework for robust formation control that updates interaction topology weights online, achieving significant distortion reduction in both simulations (1.2%-33.14%) and hardware experiments (up to 62%) compared to fixed-weight approaches.


<details>
  <summary>Details</summary>
Motivation: Conventional node-level robust controllers regulate individual robot inputs without modifying the interaction topology, leaving formation distortion unaddressed at the structural level.

Method: Two online learning strategies are proposed: Online Gradient Flow (OGF) with unconstrained weights and Online Exponential Gradient Flow (OExpGF) with non-negative convex weights, operating at the edge level to dynamically update interaction topology in real time.

Result: Simulations with 12 robots show 1.2%-33.14% median cumulative Root Mean Distortion Error reduction. Hardware experiments with Crazyflie 2.0 quadrotors show over 62% (OGF) and 31.4% (OExpGF) reduction in median formation distortion.

Conclusion: OExpGF guarantees asymptotic consensus for single-integrator agents over directed graphs, while OGF ensures bounded formation distortion, demonstrating that edge-level online adaptation significantly outperforms fixed-weight consensus.
...
### [184] [Sim-to-Real Betting on the E-Process: Bringing "simulators" to anytime-valid confidence sequences](https://arxiv.org/abs/2606.24038)
*Yujia Chen,Bowen Weng*

Main category: cs.RO

TL;DR: This note integrates sim-to-real performance estimates with betting-based inference and safe anytime-valid inference to produce efficient, reliable certificates for mean estimates in robot performance testing.


<details>
  <summary>Details</summary>
Motivation: To produce efficient, reliable certificates for mean estimates in robot performance testing using sim-to-real transfer.

Method: Integration of sim-to-real performance estimates with betting (from Chen et al.) and safe anytime-valid inference (from Ramdas et al.), using scaled simulators.

Result: The method produces efficient, reliable certificates for the mean estimate.

Conclusion: This primary account provides a self-contained construction; synthetic examples are available on GitHub for demonstration.
...
### [185] [TurboMPC: Fast, Scalable, and Differentiable Model Predictive Control on the GPU](https://arxiv.org/abs/2606.24039)
*Gabriel Bravo-Palacios,Jianghan Zhang,Zachary Pestrikov,Brian Plancher,Thomas Lew*

Main category: cs.RO

TL;DR: TurboMPC is a GPU-based differentiable MPC solver that achieves massive speedups (up to 58x) over state-of-the-art solvers, supporting complex constraints and deployed successfully on a real car for minimum-time racing.


<details>
  <summary>Details</summary>
Motivation: As robotics increasingly uses GPUs for simulation and learning, MPC solvers must run efficiently on GPU hardware while remaining fast, differentiable, and compatible with expressive robotics formulations.

Method: Combines SQP, ADMM inner solver, implicit differentiation, and a co-designed JAX-CUDA implementation to run entirely on GPU with support for inequality constraints, implicit integrators, cross-time costs, and slack variables.

Result: Achieves up to 15x and 58x speedups over CPU and GPU differentiable solvers; validated on constrained planning, humanoid imitation learning, reinforcement learning, and real-world minimum-time racing on a full-scale car; scales to 8000+ knot points.

Conclusion: TurboMPC enables efficient, differentiable GPU-based MPC for robotics, and batched GPU-accelerated Bayesian optimization yields better performance than hand-tuned baselines in real-world deployment.
...
### [186] [SPACE: Enabling Learning from Cross-Robot Data Toward Generalist Policies](https://arxiv.org/abs/2606.24049)
*Haeone Lee,Byeongguk Jeon,Suchae Jeong,Jian Kim,Kimin Lee*

Main category: cs.RO

TL;DR: SPACE proposes using Cartesian state delta as a universal action representation for robot learning, with an Action Adapter that handles dynamics variations across different embodiments, hardware units, and deployment conditions.


<details>
  <summary>Details</summary>
Motivation: Robot actions are tied to specific robot dynamics, so different robots may require different actions for the same motion, hindering policy generalization across diverse robots and deployment conditions.

Method: SPACE has two components: (i) a Cartesian state delta policy predicting geometric end-effector displacement, and (ii) Action Adapter converting predicted displacement into robot-specific control commands.

Result: Substantially outperforms policies that directly predict control commands when learning from data across different embodiments and hardware units, and remains robust under dynamics shifts at deployment.

Conclusion: Cartesian state delta as a universal action representation effectively handles robot dynamics variation across embodiments, hardware units, and deployment conditions.
...
### [187] [MinInter: Minimizing Trajectory Interpolation During Data Augmentation for Imitation Learning](https://arxiv.org/abs/2606.24078)
*Qingyang Wang,Xingang Liu,Changwei Yao,Zikai Ouyang,Junwei Liu,Haibo Lu,Wei Zhang*

Main category: cs.RO

TL;DR: MinInter is a trajectory selection method that minimizes interpolations when recombining demonstration segments from varied initial states, improving synthetic data quality and policy success rates across 12 manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Trajectory-level data augmentation for imitation learning often inserts non-expert interpolation segments between disjoint demonstration parts, degrading synthetic data quality. The authors aim to minimize such interpolations.

Method: For each sampled initial configuration, MinInter selects the source demonstration that requires the least interpolation to form a complete trajectory, explicitly minimizing non-expert segments during data generation.

Result: Tested on 12 manipulation tasks (26 variants) from MimicGen benchmark. MinInter consistently improves data generation success rates and policy success rates, with largest gains in contact-rich, long-horizon, and high-variance settings. Outperforms SkillGen despite simplicity.

Conclusion: Minimizing interpolation during trajectory-level data augmentation is a simple yet effective strategy that significantly enhances synthetic demonstration quality and downstream policy performance.
...
### [188] [DynaWM: Dynamics-Aware Distillation with World Model and Momentum Targets for Smooth Locomotion over Continuous Stairs](https://arxiv.org/abs/2606.24089)
*Haidong Hou,Zhangguo Yu,Hengbo Qi,Jianlin Zhang*

Main category: cs.RO

TL;DR: DynaWM is a dynamics-aware representation learning framework that uses a world model regularizer and momentum target encoder to improve bipedal-wheeled robot traversal of long staircases, achieving superior terrain adaptability and motion smoothness in both simulation and real hardware.


<details>
  <summary>Details</summary>
Motivation: Existing teacher-student frameworks for bipedal-wheeled robots struggle with long staircase traversal due to weakened dynamics-aware representations and incomplete terrain geometry encoding.

Method: DynaWM introduces (1) a world model as a regularizer to enforce forward-dynamics awareness and enable hierarchical encoding visualization, and (2) a momentum target encoder for consistent distillation targets to prevent dimensional collapse from non-stationary teacher updates.

Result: PCA visualization and quantitative metrics show hierarchical terrain geometry capture with higher encoding capability. Simulation and real hardware experiments demonstrate superior terrain adaptability and motion smoothness on diverse continuous stairs.

Conclusion: DynaWM effectively bridges the gap in long staircase traversal for bipedal-wheeled robots by enhancing dynamics-aware representations and terrain geometry encoding.
...
### [189] [NavWM: A Unified Navigation World Model for Foresight-Driven Planning](https://arxiv.org/abs/2606.24101)
*Yanghong Mei,Longteng Guo,Ming-Ming Yu,Guiyu Zhao,Xingjian He,Jing Liu*

Main category: cs.RO

TL;DR: NavWM is a unified navigation world model integrating latent world reasoning, multimodal action prediction, and controllable visual generation, achieving SOTA results in future state generation and zero-shot navigation.


<details>
  <summary>Details</summary>
Motivation: Conventional navigation policies suffer from myopic decision-making and mode collapse, while existing world model paradigms isolate perception, generation, and control, failing to capture shared spatio-temporal dynamics.

Method: NavWM uses latent world tokens to distill geometric/semantic priors, plus an anchor-based multimodal trajectory forecasting framework to generate diverse action spaces, enabling the world model to act as a closed-loop planner via visual foresight.

Result: Remarkable improvements in both high-fidelity future state generation and zero-shot navigation success across diverse robotics datasets, advancing the state-of-the-art.

Conclusion: Unified navigation world models that integrate reasoning, prediction, and generation can overcome myopic planning and mode collapse, providing robust zero-shot navigation capabilities.
...
### [190] [The Evaluation Cost of Task Specialization in Evolutionary Multi-Robot Systems](https://arxiv.org/abs/2606.24191)
*Paolo Leopardi,Heiko Hamann,Jonas Kuckling,Tanja Katharina Kaiser*

Main category: cs.RO

TL;DR: This paper analyzes the cost-benefit trade-off of evolving task-specialist vs. generalist robot controllers in multi-robot foraging, finding that as team size increases, specialists outperform generalists with a lower total evaluation budget.


<details>
  <summary>Details</summary>
Motivation: Previous work showed task specialization is more likely to evolve when subtask behaviors are available as building blocks, but it's unclear how the evaluation budget (which must be split across subtasks for specialists vs. fully available for a single generalist) affects this comparison.

Method: The authors conduct a cost-benefit analysis using a physics-based robotics simulator in a foraging scenario, studying the total evaluation budget required for evolving task-specialist vs. generalist behaviors across different MRS sizes.

Result: As the multi-robot system size increases, a lower total evaluation budget is sufficient to evolve task-specialist behaviors that outperform generalist behaviors.

Conclusion: Task specialization becomes more advantageous over generalist approaches as the number of robots grows, requiring less evaluation budget to achieve superior performance in larger teams.
...
### [191] [Grounding Generative Policies in Physics: Optimization-Guided Diffusion for Robot Control](https://arxiv.org/abs/2606.24208)
*Sabrina Bodmer,René Zurbrügg,Tifanny Portela,Hao Ma,Alexandre Didier,Marco Hutter,Colin Jones,Melanie Zeilinger*

Main category: cs.RO

TL;DR: This paper proposes an inference-time optimization framework that guides diffusion model sampling using constrained optimization to enforce physical feasibility (reachability, collision-avoidance, executability) for robot behavior generation, improving task success by up to 23pp without retraining.


<details>
  <summary>Details</summary>
Motivation: Diffusion models can generate diverse, high-quality robot behaviors in task space, but these outputs often violate embodiment-specific constraints (e.g., reachability, collision avoidance, closed-loop executability), creating an "embodiment gap" that prevents zero-shot deployment across different robots.

Method: The authors replace the standard sampling perturbation in the backward diffusion process with an optimized correction derived from a constrained optimization formulation. This allows hard constraints or soft penalties to be imposed during sampling while keeping samples close to the learned prior, all without retraining the diffusion model.

Result: The method improves task success by up to 20pp on dexterous grasping and 23pp on visuomotor manipulation over the best baseline. It matches projection- and gradient-guidance baselines in feasibility while better preserving grasp quality and improving executability.

Conclusion: Optimization-guided denoising effectively bridges the embodiment gap, enabling task-space diffusion policies to generalize across different robot embodiments with improved physical feasibility and task success, all at inference time without model retraining.
...
### [192] [RoBoSR: Structured Scene Representations for Embodied Robotic Reasoning](https://arxiv.org/abs/2606.24338)
*Kewei Hu,Wanchan Yu,Fangwen Chen,Jing Jiajian,Zimeng Li,Ying Wei,Tianhao Liu,Michael Zhang,Hanwen Kang*

Main category: cs.RO

TL;DR: RoBoSR introduces an object-centric scene graph representation for embodied reasoning, paired with a large dataset (Manip-Cognition-1.6M), enabling structured, causal long-horizon task planning that outperforms prompting-based and TAMP methods.


<details>
  <summary>Details</summary>
Motivation: Existing embodied reasoning methods rely on demonstration-driven sequential biases, which limit flexibility in open-ended and long-horizon tasks requiring structured reasoning over evolving states.

Method: RoBoSR uses intermediate structural representation (object-centric scene graphs) to model state transitions, disentangling high-level reasoning from raw inputs. A large dataset Manip-Cognition-1.6M is constructed to jointly supervise scene understanding, instruction interpretation, and subtask planning.

Result: Outperforms prompting-based methods and classical TAMP baselines in zero-shot generalization and long-horizon tasks across benchmarks and real-world demonstrations.

Conclusion: Structured intermediate representations serve as a critical inductive bias for scalable embodied reasoning in open-world environments.
...
### [193] [SlipSense: Multimodal Sensing for Online Slip Detection in Legged Robots](https://arxiv.org/abs/2606.24350)
*Iris Szu-Yao Liu,Chien Chern Cheah,Meng Yee Michael Chuah*

Main category: cs.RO

TL;DR: SlipSense is a force-based slip detection framework for legged robots that uses a custom sensorized foot and LSTM model to detect early-stage slips on slippery terrain, achieving 85.9% accuracy and detecting slips as small as 24.1mm—3.3x finer resolution than kinematic baselines.


<details>
  <summary>Details</summary>
Motivation: Existing slip detection methods rely on kinematics and proprioception, which lack sensitivity to detect early-stage slips before catastrophic instability occurs on variable terrains like slippery surfaces.

Method: A custom lightweight sensorized foot is designed for quadruped robots, integrating multimodal sensors with an LSTM-based model to infer ground reaction forces and detect slip-indicative anomalies during locomotion, deployed on a Unitree Go1 quadruped.

Result: Detects early-stage slips down to 24.1±6.4mm average displacement with 85.9% overall accuracy, achieving 3.3x finer detection resolution and 24% relative accuracy improvement over kinematic baseline using foot velocity from state estimation.

Conclusion: This work provides a foundation for force-aware gait adaptation in legged locomotion, enabling future controllers to estimate terrain friction and adjust constraints for improved system stability.
...
### [194] [PDS Joint: A Parametric Double-Spiral Joint Tailored for Dexterous Hands](https://arxiv.org/abs/2606.24377)
*Haoyang Li,Yibo Wen,Yixiang Fan,Yiheng Xu,Yufeng Yue*

Main category: cs.RO

TL;DR: This paper presents the PDS (parametric double-spiral) compliant joint for dexterous hands, enabling directional stiffness shaping across multiple deformation modes. It introduces Archimedean/logarithmic spiral templates with an asymmetry ratio for stiffness tuning, a learning-based calibration pipeline for proprioception using inductive sensing and MLP mapping, and demonstrates integration into an open-source hand for safe grasping and human interaction.


<details>
  <summary>Details</summary>
Motivation: Compliant joints offer safety and adaptability for dexterous hands, but existing designs struggle to achieve large-stroke anthropomorphic motion while maintaining joint-specific, direction-dependent stiffness and reliable proprioception.

Method: The paper proposes a parametric double-spiral (PDS) compliant joint with Archimedean and logarithmic spiral templates, an asymmetry ratio for stiffness tailoring, embedded inductive proprioception, and a learning-based calibration pipeline using ArUco-marker tracking and MLP mapping for joint-state estimation.

Result: Experiments revealed non-monotonic dependence of lateral support on asymmetry. The MLP-based joint-state estimation reduced error by 41.6% compared to conventional curve fitting for abduction/adduction. The joint was successfully integrated into an open-source dexterous hand that grasped nine everyday objects and performed safe human-involved interactions.

Conclusion: The PDS joint provides a principled approach to achieving direction-dependent stiffness and reliable proprioception for compliant dexterous hands, with demonstrated effectiveness in large-stroke motion and contact-rich tasks.
...
### [195] [RE4: Transformation-aware Imitation of Object Interactions Using Manipulation Modes](https://arxiv.org/abs/2606.24403)
*Arsh Chawla,Rahul Shome*

Main category: cs.RO

TL;DR: The paper proposes RE4, a framework that combines lightweight self-supervised pose estimation with mode-aware demonstration retrieval, transformation, and replanning to achieve interpretable and robust imitation learning for object interaction tasks, outperforming on Push-T and Robomimic benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between performance and interpretability in imitation learning for object interactions, where end-to-end methods (diffusion/flow-based) achieve high performance but sacrifice interpretability, while object-centric methods preserve interpretability but lag in performance.

Method: RE4 framework with four steps: (1) lightweight self-supervised pose estimation from image observations using demonstration data, (2) manipulation mode-aware retrieval of demonstrations, (3) mode-aware transformation, and (4) replanning to connect to retrieval points while preserving mode constraints, followed by rolling out transformed demonstrations.

Result: Evaluated on Push-T and Robomimic benchmarks (both state-based and image-based). Adversarial benchmark testing sparse data regions in image-based Push-T demonstrated robustness. Low-data regime experiments showed promising performance.

Conclusion: Simple interpretable building blocks can effectively learn manipulation skills while preserving both performance and interpretability, showing promise over complex end-to-end methods.
...
### [196] [Legible and Intuitive Multi-modal Robot State and Intent Communication Validated in Online and Real-world Studies](https://arxiv.org/abs/2606.24445)
*Tim Schreiter,Jens V. Rüppel,Andrey Rudenko,Martin Magnusson,Achim J. Lilienthal*

Main category: cs.RO

TL;DR: This paper systematically compares unimodal LED-based vs. highly expressive multimodal (gaze, gestures, voice) robot-to-human communication strategies across message types and settings (online vs. in-person), finding multimodal communication more legible and intuitive, and revealing a significant virtual-to-real gap where legibility and confidence decrease in real-world settings.


<details>
  <summary>Details</summary>
Motivation: Effective robot-to-human communication is crucial for transparency, trust, and safety, but designing such strategies is challenging due to varying robot modalities, diverse recipient interpretations, and the underexplored virtual-to-real gap.

Method: Systematic large-scale comparative validation of existing communication strategies (unimodal LED vs. multimodal gaze/gestures/voice) for a mobile non-humanoid robot, evaluating communication of five message types (turning intention, attention request, error status, stuck, functioning normally) in replicated online and in-person experiments.

Result: Highly expressive multimodal communication was perceived as more legible and intuitive than unimodal LED-based communication. Real-world legibility and confidence decreased notably compared to online settings, especially for LED signaling.

Conclusion: Multimodal strategies outperform unimodal ones for robot-to-human communication, but researchers should account for significant virtual-to-real gaps, as results from online studies may overestimate real-world legibility and user confidence.
...
### [197] [Supervise What Survives: Geometry-Guided VLA Adaptation from Synthetic Robot Videos](https://arxiv.org/abs/2606.24448)
*Danze Chen,Yanzhe Chen,Qiming Huang,Zhijun Cao,Chen Gao,Mike Zheng Shou*

Main category: cs.RO

TL;DR: This paper proposes GRA (Geometry-guided Representation Alignment), a method that uses generated robot videos to supervise only visual perception (via 2D waypoints) rather than recovering pseudo-actions, preserving the geometric content while leaving control to real demonstrations. It outperforms pseudo-action baselines and narrows the gap to policies trained with more real data.


<details>
  <summary>Details</summary>
Motivation: VLA models need large-scale video-action pairs but real teleoperation data is scarce. Generated robot videos are a scalable alternative, but existing methods incorrectly try to recover low-level control signals (actions) from generated visuals, creating a mismatch between what video generation preserves (geometry) and what they try to extract (control).

Method: GRA extracts geometric content as future 2D end-effector waypoints from human videos via pose estimation, retargeting, simulation, and calibrated projection. These waypoints are routed to the VLA vision backbone through an auxiliary 2D head, while the action head is trained only on real demonstrations. During fine-tuning, the waypoint loss acts as a spatial representation anchor to maintain geometric grounding.

Result: GRA outperforms pseudo-action baselines under matched data budgets and narrows the gap to policies trained with substantially more real demonstrations on real-robot tasks.

Conclusion: Correctly routed geometry bridges generated videos to robot policies more reliably than recovered actions, validating the Asymmetric Preservation Principle that generated videos should supervise perception (geometry) while real data should supervise control (actions).
...
### [198] [NoContactNoWorries: Estimating Contact through Vision and Proprioception for In-Hand Dexterous Manipulation](https://arxiv.org/abs/2606.24450)
*Soham Patil,Avirup Das,Sourabh Bhosale,Spandan Roy*

Main category: cs.RO

TL;DR: NoContactNoWorries is a transformer-based framework that infers binary hand-object contact states from RGB-D vision and robot proprioception, eliminating the need for dedicated tactile sensors. It supports downstream reinforcement learning for in-hand object reorientation and generalizes to novel objects in both simulation and real-world settings.


<details>
  <summary>Details</summary>
Motivation: Tactile sensors face practical challenges in cost, fragility, and integration. Inspired by humans' ability to infer contact from vision and proprioception, the authors aim to develop a scalable, hardware-free alternative for binary contact estimation in dexterous manipulation.

Method: A transformer-based multimodal framework (NoContactNoWorries) that fuses RGB-D vision with robot proprioception to predict binary contact states as pseudo-tactile signals. A single model is trained on multiple objects and used to provide contact feedback to downstream RL agents for in-hand reorientation tasks.

Result: The approach successfully predicts contact states from vision and proprioception, supports downstream RL for in-hand object reorientation, and generalizes to novel objects. Validation conducted in both simulation and on a real-world robot.
...
### [199] [Varying Bundle Size Reactive Multi-Task Assignment using Selective Cost Estimation for Multi-Agent Systems](https://arxiv.org/abs/2606.24462)
*Niklas Dahlquist,Shridhar Velhal,George Nikolakopoulos*

Main category: cs.RO

TL;DR: This paper proposes a scalable two-stage multi-fidelity bundle generation framework for multi-robot task allocation, using low-fidelity heuristics for rapid bundle exploration and high-fidelity path planning only for promising candidates, enabling reactive auction-based allocation with dynamic bundle sizes.


<details>
  <summary>Details</summary>
Motivation: Combinatorial auction-based multi-robot task allocation suffers from exponential complexity in bundle generation when accurate path planning is needed for cost estimation, making it intractable for real-time reactive applications.

Method: A distributed two-stage approach: (1) agents use a local search tree with low-fidelity heuristics (e.g., Euclidean distance) to explore bundle space rapidly, then apply high-fidelity path planning only to the most promising candidates in a best-first manner; (2) refined bids are submitted to a central coordinator solving a set packing problem for global feasibility and utility maximization.

Result: Simulation results across multiple environments show the framework improves the performance of reactive auction-based task allocation.

Conclusion: The framework enables reactive task allocation with dynamic bundle sizes without exposing agents' internal states or cost estimation models.
...
### [200] [FT-WBC: Learning Fault-Tolerant Whole-Body Control for Legged Loco-Manipulation](https://arxiv.org/abs/2606.24466)
*Yudong Zhong,Pengfei Mai,Sikai Guo,Jiahang Cao,Zhihai Bi,Qiuyue Liu,Ziyan Feng,Jinni Zhou,Jun Ma*

Main category: cs.RO

TL;DR: FT-WBC is a fault-tolerant loco-manipulation framework for legged manipulators that uses a Fault Estimator and Posture Adaptation Module to handle actuator failures, significantly improving survival rate and workspace under weakening or locked joint failures.


<details>
  <summary>Details</summary>
Motivation: Existing fault-tolerant control methods focus on locomotion alone, leaving the coupled problem of whole-body stability and arm reachability in fault-tolerant loco-manipulation largely unaddressed.

Method: FT-WBC uses a decoupled upper- and lower-body policy architecture with two key modules: (1) Fault Estimator (FE) that predicts faulty joints from lower-body proprioceptive histories, and (2) Posture Adaptation Module (PAM) that adapts base posture plans to convert unstable requests into safe commands.

Result: Simulation and real-world experiments show FT-WBC significantly improves survival rate and workspace under weakening or locked failures, and transfers zero-shot to a real legged manipulator.

Conclusion: The fault-aware posture adaptation mechanism enables compensatory gaits under actuator failures while preserving arm workspace and maintaining whole-body stability.
...
### [201] [G$^3$VLA: Geometric inductive bias for Vision-Language-Action Models](https://arxiv.org/abs/2606.24472)
*Yue Peng,Yongzhe Zhao,Artur Habuda,Khuyen Pham,Yanheng Zhu,Tran Nguyen Le,Fares Abu-Dakka,Li Guo*

Main category: cs.RO

TL;DR: G^3VLA injects camera-aware geometric information (ray embeddings, projective positional encoding, cross-view fusion) into pretrained VLAs, improving robot manipulation performance across multiple benchmarks and real-robot settings without needing depth sensors or manual annotations.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models use 2D image tokens without calibrated camera geometry, which is problematic for multi-camera setups where views are related via known intrinsics/extrinsics but processed independently, limiting spatial understanding.

Method: Three key components: (1) intrinsic-conditioned ray embeddings, (2) projective positional encoding (PRoPE), and (3) bidirectional cross-view fusion. Geometric supervision comes from ground-truth point maps or confidence-gated π^3X teacher predictions, requiring no depth sensors. Instantiated on π_0, π_{0.5}, and GR00T 1.5.

Result: Consistent gains across LIBERO suites, RoboCasa24, RoboTwin2.0, and real-robot settings. Largest improvements on spatially and object-sensitive tasks. Geometric transfer most effective when geometry-aware tokens directly access the action generation pathway.

Conclusion: Injecting calibrated geometric structure into VLA visual tokens improves manipulation performance, especially for spatial tasks, and works best when geometry-aware tokens connect directly to action generation.
...
### [202] [Decentralized Pose Graph Riemannian Optimization for Object-based Multi-Robot SLAM](https://arxiv.org/abs/2606.24489)
*Yixian Zhao,Yan Huang,Yang Xu,Liang Li,Jinming Xu*

Main category: cs.RO

TL;DR: A fully decentralized Riemannian optimization framework for object-based multi-robot PGO using consensus and approximate-Newton methods, enabling flexible communication topologies with reduced iteration count and communication overhead.


<details>
  <summary>Details</summary>
Motivation: Existing decentralized PGO solutions assume communication graphs closely match physical interaction topologies, which is restrictive in real-world deployments where communication is sparse, intermittent, or time-varying.

Method: Proposes a fully decentralized Riemannian optimization framework with consensus mechanism to decouple coupled estimation, plus a distributed approximate-Newton scheme exploiting local second-order information on the SE(d) manifold.

Result: Reduced iteration count and communication overhead without sacrificing accuracy; validated on public benchmarks, large-scale simulations, and real-world experiments showing improved accuracy, runtime efficiency, scalability, and robustness to communication failures.

Conclusion: The method achieves convergence to Riemannian first-order stationary points with theoretical guarantees, and the approximate second-order approach outperforms first-order Riemannian descent in terms of condition number and efficiency.
...
### [203] [Explaining Failures of Cyber-Physical Systems with Actual Causality](https://arxiv.org/abs/2606.24546)
*Khen Elimelech,Tom Yaacov,David A. Kelly,Hana Chockler,Moshe Y. Vardi*

Main category: cs.RO

TL;DR: This paper introduces actual causality for explaining failures in autonomous Cyber-Physical Systems (CPS), providing theoretical foundations and two practical algorithms for deriving explanations, demonstrated on a neural-network-controlled autonomous car.


<details>
  <summary>Details</summary>
Motivation: Unexpected failures in black-box autonomous CPS (e.g., self-driving cars) are inevitable and potentially catastrophic. Deriving explanations for these failures is crucial for building trust and enabling future mitigation, but existing causality frameworks were limited to simple systems like image classifiers.

Method: The paper extends the actual causality framework to the CPS domain by addressing theoretical gaps. It proposes two system-agnostic explanation derivation algorithms that prioritize either explanation optimality or efficiency.

Result: The approach is demonstrated on a neural-network-controlled autonomous car designed to avoid collisions, showing practical feasibility and trade-offs between optimality and efficiency.

Conclusion: Actual causality can be effectively applied to explain failures in complex CPS, offering both theoretical foundations and practical algorithms for failure explanation.
...
### [204] [Enabling Robust Cloth Manipulation via Inference-Time Simulator-in-the-Loop Refinement](https://arxiv.org/abs/2606.24552)
*Xin Liu,Yulin Li,Ziming Li,Pengyu Jing,Zhenhao Huang,Bingyang Zhou,Ziqiu Zeng,Siyuan Luo,Chenkun Qi,Fan Shi*

Main category: cs.RO

TL;DR: This paper brings simulator-in-the-loop optimization to real-world cloth manipulation from single RGB input, using a synthetic-data trained real-to-sim module and prior-guided MPPI with a deformable-object simulator (FLASH) as rollout backend, achieving higher success rates and robustness than baselines.


<details>
  <summary>Details</summary>
Motivation: Simulator-in-the-loop optimization has proven effective for rigid-body manipulation, but it remains unexplored for deformable objects like cloth, where state estimation and contact dynamics are more complex and challenging.

Method: Three pillars: (i) synthetic-data generation & rollout pipeline using FLASH simulator; (ii) real-to-sim module trained purely on synthetic data fusing pretrained visual features with learnable canonical tokens for single RGB to cloth state mapping; (iii) online planning via prior-guided MPPI with sparse-mesh rollout backend anchored at offline-distilled policy trajectory.

Result: Real-robot experiments demonstrate higher success rates and stronger robustness compared to baseline methods for cloth manipulation tasks.

Conclusion: Simulator-in-the-loop optimization can be effectively extended to deformable object manipulation (cloth) by combining synthetic-data training, real-to-sim bridging, and prior-guided online planning, outperforming existing approaches in both success rate and robustness.
...
### [205] [ArtiTwinSplat: Interactable Digital Twin Reconstruction via Gaussian Splatting from RGB-D videos](https://arxiv.org/abs/2606.24628)
*Pranjal Mishra,René Zurbrügg,Max Wilder-Smith,Marco Hutter,Marc Pollefeys,Zuria Bauer,Hermann Blum*

Main category: cs.RO

TL;DR: ArtiTwinSplat automatically creates articulated, photorealistic 3D digital twins of objects from RGB-D videos using 3D Gaussian Splatting and unsupervised articulation discovery, enabling real-time rendering and interactive manipulation for downstream robotic systems without CAD models or manual annotations.


<details>
  <summary>Details</summary>
Motivation: Deploying robots in unstructured environments requires accurate, interactive object models, but constructing these models at scale remains a critical bottleneck for robotic system integration, especially without CAD models or manual annotations.

Method: ArtiTwinSplat uses 3D Gaussian Splatting for geometric and photometric fidelity, coupled with an unsupervised articulation discovery pipeline that recovers part structure and joint kinematics purely from observed motion in RGB-D videos, followed by tracking and optimization stages.

Result: Produces stable, queryable digital twins that support real-time rendering, viewpoint control, and interactive manipulation, operating directly on real-world observations and usable by downstream robot planning and learning systems.

Conclusion: Offers a practical, scalable pathway for digital twin construction, lowering the integration barrier for articulated object manipulation in embodied AI and human-robot collaboration contexts.
...
### [206] [Optimization-based Safe Trajectory Planning for Autonomous Ground Vehicle in Multi-Floor Scenarios](https://arxiv.org/abs/2606.24631)
*Zishang Xiang,Runda Zhang,Runqi Chai,Kaiyuan Chen,Senchun Chai,Yuanqing Xia*

Main category: cs.RO

TL;DR: This paper proposes a trajectory planning framework for multi-floor autonomous ground vehicles, combining task planning (using GVD and multi-objective algorithms for exit selection) with trajectory planning (optimization-based with warm-started hierarchical planning and correlation constraint reduction), verified through simulations.


<details>
  <summary>Details</summary>
Motivation: To address the lack of trajectory planning strategies for autonomous ground vehicles operating in multi-floor environments.

Method: A two-module framework: (1) Task planning using generalized Voronoi diagrams (GVD) and multi-objective algorithms to select floor exits; (2) Trajectory planning using optimization-based methods with a warm-started hierarchical planning framework and correlation constraint calculation for handling complex obstacles.

Result: Simulation results demonstrate the feasibility and effectiveness of the proposed framework.

Conclusion: The proposed multi-floor trajectory planning framework is feasible and effective for AGVs in multi-floor intelligent transportation scenarios.
...
### [207] [Beyond Monotonic Progress: Retry-Supervised Value Learning for Robot Imitation](https://arxiv.org/abs/2606.24633)
*Xinyao Qin,Junjie Lu,Kaixin Wang,Chuheng Zhang,Sinjae Kang,Kimin Lee,Min Xu,Bin Liang,Jun Yang,Li Zhao*

Main category: cs.RO

TL;DR: ReTVL learns mistake-sensitive value functions from imperfect demonstrations by using retry events as supervision, then reweights demonstrations for better imitation learning.


<details>
  <summary>Details</summary>
Motivation: Existing reward/value models rely on monotonic progress assumptions, which overlook local execution errors and corrective behaviors in imperfect demonstrations.

Method: ReTVL combines global progress calibration with local pairwise preference learning induced by sparsely annotated retry keypoints to capture degradation-and-recovery structures around mistakes.

Result: ReTVL produces more fine-grained value estimates than progress-based baselines and improves imitation learning on real-robot manipulation tasks.

Conclusion: Retry events in imperfect demonstrations are valuable supervision signals for learning mistake-sensitive value functions that improve downstream imitation learning.
...
### [208] [TACTFUL: Tactile-Driven Exploration For Object Localization and Identification in Confined Environments](https://arxiv.org/abs/2606.24712)
*Shivani Kamtikar,Chung Hee Kim,Camilla Tabasso,Tye Brady,Joshua Migdal,Taskin Padir*

Main category: cs.RO

TL;DR: TACTFUL is a vision-free tactile exploration framework that enables multi-fingered robots to autonomously explore, discover, and identify objects through touch alone, achieving 77% success with 0.015m reconstruction error using a single policy trained entirely on real hardware.


<details>
  <summary>Details</summary>
Motivation: Humans can identify objects by touch alone, but robotic systems rely heavily on vision and struggle with autonomous tactile exploration and object identification without visual input.

Method: A vision-free tactile exploration framework using a dynamic reward schedule that balances global workspace exploration with local surface refinement, trained entirely on real hardware without simulation.

Result: 77% success rate, 0.015m average reconstruction error, outperforming baseline approaches on real-world objects.

Conclusion: Tactile sensing paired with structured learning can serve as an effective primary modality for object-level reasoning, even without vision or simulation-based training.
...
### [209] [World Value Models for Robotic Manipulation](https://arxiv.org/abs/2606.24742)
*Zhihao Wang,Jianxiong Li,Yu Cui,Yuan Gao,Xianyuan Zhan,Junzhi Yu,Xiao Ma*

Main category: cs.RO

TL;DR: The paper introduces World Value Model (WVM), a generalist robotic value model that combines world models with value estimation to better assess data quality and guide policy learning from mixed-quality data, achieving SOTA performance on both expert and suboptimal benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing robotic value models built on VLM backbones lack temporal modeling capabilities needed for accurate value estimation, as VLMs are pretrained on static or temporally sparse visual data. World models naturally excel at temporal modeling and future planning, making them better foundations for learning generalizable value functions.

Method: The authors marry world models with value estimation to construct a generalist robotic value model called World Value Model (WVM), which provides accurate task progressions to assess data quality. They also introduce Suboptimal-Value-Bench, a multi-embodiment benchmark with 800 suboptimal trajectories and human-labeled frame annotations.

Result: WVM achieves SOTA Value-Order Correlation (VOC) results on standard benchmarks. It maintains SOTA performance on the newly introduced Suboptimal-Value-Bench, demonstrating robustness in handling both expert and suboptimal data. When deployed for policy learning, WVM improves manipulation performance across various policy extraction approaches in both simulated and real-world settings.

Conclusion: World models are superior to VLMs as foundations for generalist robotic value models due to their inherent temporal modeling and future planning capabilities. WVM provides robust guidance for learning from mixed-quality data in robotic policy learning.
...
### [210] [Vision-Language Model Reasoning for Contextual Semantic Mapping in Intralogistics](https://arxiv.org/abs/2606.24814)
*Marvin Rüdt,Hao Pang,Constantin Enke,Zäzilia Seibold,Kai Furmans*

Main category: cs.RO

TL;DR: The paper presents a contextual semantic mapping pipeline for autonomous mobile robots that combines SLAM, SAM-based instance segmentation, clustering, and VLM multi-view reasoning to create maps encoding geometry, object class, and movability, achieving 98.93% mIoU for semantic classification and 89.17% mAcc for movability estimation without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Autonomous robots in intralogistics use geometric maps for localization/navigation but lack semantic understanding of objects and their contextual properties (e.g., movability), limiting their ability to operate in dynamic environments.

Method: A pipeline combining SLAM-based geometric mapping, SAM-based instance segmentation, instance clustering, and VLM multi-view reasoning in a zero-shot, open-vocabulary setting to infer contextual object properties like movability.

Result: 98.93% mIoU for semantic classification, 89.17% mAcc for object movability estimation; VLM reasoning identified as main bottleneck for contextual understanding, instance clustering as main limitation for panoptic performance.

Conclusion: The contextual semantic map supports context-aware filtering and robust navigation in dynamic intralogistics environments, with zero-shot generalization capabilities.
...
### [211] [InSight: Self-Guided Skill Acquisition via Steerable VLAs](https://arxiv.org/abs/2606.24884)
*Maggie Wang,Lars Osterberg,Stephen Tian,Ola Shorinwa,Jiajun Wu,Mac Schwager*

Main category: cs.RO

TL;DR: InSight enables VLAs to autonomously acquire new primitive-level skills (like pouring, lifting) via automated segmentation and a VLM-guided data flywheel, achieving zero-shot novel task learning without human demonstrations.


<details>
  <summary>Details</summary>
Motivation: VLA models are bounded by the skills present in their training data; they cannot autonomously acquire new manipulation skills without human demonstrations.

Method: Two-stage framework: (1) automated segmentation pipeline partitions demonstrations into labeled primitives using VLM plan decomposition and end-effector poses; (2) VLM-guided data flywheel identifies missing primitives, autonomously attempts demonstrations with VLM-proposed low-level control, and integrates successful ones into training.

Result: Successfully learned block flipping, drawer closing, sweeping, twisting, and pouring in simulation and real-world without any human demonstrations of those target skills; learned primitives can compose to novel long-horizon tasks.

Conclusion: Primitive steerability provides a practical foundation for continual skill acquisition in VLA policies, enabling autonomous expansion of manipulation capabilities.
...
### [212] [RIFT-Bench: Dynamic Red-teaming For Agentic AI Systems](https://arxiv.org/abs/2606.23927)
*Yarin Yerushalmi Levi,Roy Betser,Amit Giloni,Lidor Erez,Itay Gershon,Oren Rachmil,Sindhu Padakandla,Roman Vainshtein*

Main category: cs.AI

TL;DR: RIFT-Bench is a graph-based automated red-teaming framework for unified security evaluation of diverse agentic AI systems, operating via two phases (Discovery and Scanning) to dynamically probe vulnerabilities across 45 heterogeneous systems.


<details>
  <summary>Details</summary>
Motivation: Existing security evaluations for agentic AI systems are tied to specific implementations or domains, lacking a unified methodology for comparing security across heterogeneous agentic architectures.

Method: RIFT-Bench uses a graph representation-driven methodology with two automated phases: (1) Discovery - extracts system structure via hierarchical representation, and (2) Scanning - deploys adaptive adversarial attacks across diverse vectors and produces comprehensive evaluation reports.

Result: The approach generalizes effectively across 45 diverse agentic systems spanning heterogeneous architectures, and also supports direct evaluation of mitigation strategies.

Conclusion: RIFT-Bench provides a scalable, unified foundation for security evaluation of agentic AI systems, enabling cross-system comparison and mitigation assessment.
...
### [213] [Neuro-Symbolic Drive: Rule-Grounded Faithful Reasoning for Driving VLAs](https://arxiv.org/abs/2606.23938)
*Xiangbo Gao,Xiukun Huang,Boyu Lu,Junge Zhang,Mengjie Mao,Jiachen Li,Wei Xiong,Zhengzhong Tu*

Main category: cs.AI

TL;DR: Neuro-Symbolic Drive supervises a driving VLA model using rule-grounded reasoning traces extracted from classical rule-based planners, improving motion prediction accuracy (ADE@3s from 0.47→0.26, miss rate 8.30%→6.40%) by ensuring causal coupling between reasoning and planned motion.


<details>
  <summary>Details</summary>
Motivation: Current VLA driving models with CoT reasoning lack step-by-step decision semantics that causally connect rationales to planned motion.

Method: Captures internal decision traces from rule-based planners at each rule-evaluation step, serializes them into structured rule-grounded reasoning, and pairs with trajectories to fine-tune Qwen3.5-4B as a driving VLA.

Result: ADE@3s reduced from 0.47 to 0.26, miss rate from 8.30% to 6.40% (3 cameras); ADE@3s from 0.54 to 0.26, miss rate from 10.13% to 5.99% (8 cameras).

Conclusion: Neuro-symbolic planning logic can be converted into structured supervision for training driving VLAs with causally grounded reasoning traces.
...
### [214] [Critique of Agent Model](https://arxiv.org/abs/2606.23991)
*Eric Xing,Mingkai Deng,Jinyu Hou*

Main category: cs.AI

TL;DR: This paper distinguishes between "agentic" systems (competence via external scaffolding) and "agentive" systems (capabilities arising endogenously), proposes the Goal-Identity-Configurator (GIC) architecture for general-purpose agents, and discusses safety implications of genuine AI agency.


<details>
  <summary>Details</summary>
Motivation: To clarify the boundary between automation and genuine agency, addressing confusion around AI agent terminology driven by both commercial hype and existential concerns about AI autonomy and control.

Method: The authors draw on Descartes' philosophy and science fiction portrayals, survey current AI agent architectures, analyze them along five dimensions (goal, identity, decision-making, self-regulation, learning), and propose the GIC architecture combining hierarchical goal decomposition, identity evolution, simulative reasoning, learned self-regulation, and self-directed learning.

Result: A conceptual framework distinguishing agentic vs. agentive systems; the GIC architecture for endogenous agency; insights on auditability, controllability, and safety of autonomous agentive systems under human oversight.

Conclusion: Genuine agency requires internalized structures rather than external scaffolding; agentive systems with true autonomy can still be kept under human oversight, but their safety, auditability, and controllability must be carefully addressed.
...
### [215] [Safe and Generalizable Hierarchical Multi-Agent RL via Constraint Manifold Control](https://arxiv.org/abs/2606.24010)
*Zihao Guo,Jianing Zhao,Ling Li,Hao Liang,Giuseppe Loianno,Yali Du*

Main category: cs.AI

TL;DR: A hierarchical MARL framework that combines hard safety constraints via constraint manifolds at low level with high-level policy learning, providing theoretical safety guarantees and stationary learning dynamics for multi-agent coordination.


<details>
  <summary>Details</summary>
Motivation: Existing approaches face a trade-off between learning-based methods (strong performance, no safety guarantees) and control-theoretic methods (safety guarantees, overly conservative behavior).

Method: Hierarchical multi-agent RL framework with low-level hard safety constraints enforced via a constraint manifold and high-level policy learning for coordination.

Result: Competitive performance with nearly perfect safety rates; generalizes effectively to varying numbers of agents and obstacles.

Conclusion: The proposed method bridges the gap between learning-based and control-theoretic approaches by providing both theoretical safety guarantees and efficient multi-agent coordination.
...
### [216] [Reinforcement Learning Towards Broadly and Persistently Beneficial Models](https://arxiv.org/abs/2606.24014)
...
TL;DR: Training RL on beneficial behaviors (truthfulness, fairness, etc.) in realistic domains like health leads to broad, persistent alignment generalization: improvements on 80%+ of out-of-distribution benchmarks, transfer across domains, and increased resistance to adversarial prompting and harmful finetuning.


<details>
  <summary>Details</summary>
Motivation: AI systems are deployed in high-stakes, diverse settings, and RL can introduce unexpected misalignment (reward hacking, deception). We need alignment that generalizes beyond training distributions.

Method: Construct a dataset of realistic situations spanning health, science, and education to measure/train beneficial traits (truthfulness, fairness, risk awareness, corrigibility). Train models with RL on this dataset, then evaluate on 50+ independent alignment benchmarks.

Result: Beneficial trait RL improves performance on over 80% of out-of-distribution benchmarks. Training limited to the health domain transfers broadly to non-health alignment evaluations. Models show improved persistence (resistance to adversarial prompting and harmful finetuning).

Conclusion: RL to reinforce beneficial behavior in realistic domains can produce models that are more robustly aligned with human flourishing, with broad and persistent alignment generalization.
...
### [217] [Can Language Model Agents be Helpful Circuit Explainers in Mechanistic Interpretability?](https://arxiv.org/abs/2606.24026)
*Ayan Antik Khan,Harsh Kohli,Yuekun Yao,Huan Sun,Ziyu Yao*

Main category: cs.AI

TL;DR: This paper introduces AgenticInterpBench, a benchmark for circuit explanation in mechanistic interpretability, and HyVE (Hypothesize, Validate, Explain), an LM-agent-based method that iteratively explains circuit components. Results show agents can produce useful explanations, but reliable validation remains a major challenge.


<details>
  <summary>Details</summary>
Motivation: Mechanistic interpretability has automated circuit localization, but explaining what localized components do is still manual, labor-intensive, and hard to standardize.

Method: Proposes HyVE, an agentic explainer that iteratively observes, hypothesizes, and causally validates component behaviors. Evaluated on AgenticInterpBench (84 semi-synthetic circuits with 163 annotations) across four LM backbones.

Result: HyVE recovers useful component- and task-level explanations across backbones, but no backbone is uniformly best. Strong backbones form observation-grounded hypotheses; failures occur later in validation due to incomplete plans, code errors, or unresolved hypotheses. A case study on Llama-3-8B arithmetic circuits shows the approach extends beyond semi-synthetic settings.

Conclusion: LM agents are promising for circuit explanation, but reliable causal validation remains the key bottleneck to overcome.
...
### [218] [Breaking the Filter Bubble: A Semantic Pareto-DQN Framework for Multi-Objective Recommendation](https://arxiv.org/abs/2606.24042)
*Cláudio Lúcio Do Val Lopes,Lucca Machado da Silva,André de Oliveira Brandão*

Main category: cs.AI

TL;DR: Introduces a multi-objective reinforcement learning framework (Pareto-DQN with semantic embeddings) for recommender systems that jointly optimizes engagement, diversity, and fairness via hypervolume-based action selection, achieving societal gains with minimal engagement loss on MovieLens.


<details>
  <summary>Details</summary>
Motivation: Recommender systems optimized solely for engagement create filter bubbles and semantic homogenization, while standard single-objective models (e.g., DQN) cannot properly trade off between platform retention and societal values like information diversity and provider fairness.

Method: Formalizes recommendation as a semantic multi-objective MDP; integrates high-fidelity semantic embeddings with a Pareto-DQN agent that treats engagement, diversity, and fairness as distinct reward signals; uses hypervolume-based action selection to explore the Pareto frontier.

Result: On MovieLens small dataset, the Pareto-DQN disrupts semantic collapse feedback loops, sustains high state-trajectory variance, effectively maps the Pareto frontier, and achieves gains in diversity and fairness with only marginal engagement reduction.

Conclusion: Provides a path toward intrinsically aligned, responsible recommender systems that balance engagement with critical societal objectives.
...
### [219] [Ensemble Feature Selection and Harris Hawks Optimization for Explainable Mental Health Risk Prediction in Female Sex Workers](https://arxiv.org/abs/2606.24047)
*Ahnaf Atef Choudhury,Md. Parvej Hoque Palash,Shahriar Siddique Ayon,Ramkrishna Saha,Abdullah Al Mamun*

Main category: cs.AI

TL;DR: A hybrid ML model combining ANOVA-MI feature selection and Harris Hawks Optimization-tuned logistic regression achieves 95.78% accuracy in predicting depression among 3,005 female sex workers, with XAI identifying PTSD, client violence, and occupational factors as key contributors.


<details>
  <summary>Details</summary>
Motivation: Female sex workers face high depression risk due to violence, stigma, and economic hardship, but current ML models fail to capture complex risk patterns in this marginalized group.

Method: Hybrid model using ensemble feature selection (ANOVA + mutual information) + Harris Hawks Optimization-tuned logistic regression + explainable AI (XAI) for interpretability.

Result: Achieved 95.78% accuracy, 95.77% F1 score, 0.96 AUC on 3,005 FSWs, outperforming traditional classifiers. Key predictors: PTSD, client violence, occupational factors.

Conclusion: The XAI tool bridges conventional and ML approaches, enabling early assistance and targeted psychosocial care for vulnerable groups.
...
### [220] [Beyond Trajectory Imitation: Strategy-Guided Policy Optimization for LLM Reasoning](https://arxiv.org/abs/2606.24064)
*Tianyuan Shi,Canbin Huang,Bei Li,Xin Chen,Xiaojun Quan,Jingang Wang,Qifan Wang*

Main category: cs.AI

TL;DR: SGPO replaces trajectory imitation with strategy distillation for reasoning transfer, using forward-KL objectives and adaptive weighting, outperforming SFT and RL baselines on math benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning distillation methods imitate solution trajectories (what to answer) rather than reasoning processes (how to reason), leading to memorization of instance-specific steps and poor generalization to novel problems.

Method: SGPO extracts structured strategy descriptions from strong models, constructs both autonomous and strategy-guided trajectories, uses a token-level forward-KL objective with proximal constraints for distillation, and applies adaptive instance-level weighting to balance guidance based on model competence.

Result: SGPO consistently outperforms SFT, on-policy RL, and hybrid-policy baselines across four math benchmarks and two model families, improving average score by 2.2 points on Qwen2.5-7B-Instruct. Forward-KL provides better selective distillation than direct imitation, and strategy distillation scales complementarily with base model capability.

Conclusion: Distilling how to reason via reusable strategies (rather than what to answer via trajectories) improves generalization and performance in weak-to-strong reasoning transfer.
...
### [221] [Exploring Academic Influence of Algorithms by Co-occurrence Network Based on Full-text of Academic Papers](https://arxiv.org/abs/2606.24099)
*Yuzhuo Wang,Chengzhi Zhang,Min Song,Seong Deok Kim,Youngsoo Ko,Juhee Lee*

Main category: cs.AI

TL;DR: This study constructs large-scale algorithm co-occurrence networks in NLP using full-text academic papers, analyzing network structures and centrality measures to assess collective algorithm influence over four decades, revealing that algorithm networks exhibit complex network features with classic and bridging algorithms holding high influence.


<details>
  <summary>Details</summary>
Motivation: Existing studies evaluate individual algorithms in isolation and pay limited attention to the collective influence formed through their interconnections in the era of AI-driven research.

Method: Using deep learning models to extract algorithm entities from full-text academic papers, constructing overall, cumulative, and annual co-occurrence networks, and applying multiple centrality measures to analyze structural characteristics.

Result: Algorithm networks display typical complex network features with increasingly dense connections over ~20 years. Classic, high-performing algorithms and those at intersections of research periods have high popularity and balanced influence. When influence declines, algorithms first lose core network position, then weaker associations.

Conclusion: This is the first large-scale analysis of algorithm co-occurrence networks, providing temporal and structural views of algorithm influence across four decades, and offering a foundation for future research on networks linking algorithms, scholars, and tasks.
...
### [222] [ReMMD: Realistic Multilingual Multi-Image Agentic Verification for Multimodal Misinformation Detection](https://arxiv.org/abs/2606.24112)
*Chenhao Dang,Dantong Zhu,Jun Yang,Conghui He,Weijia Li*

Main category: cs.AI

TL;DR: ReMMD presents a realistic multilingual, multi-image agentic verification framework for multimodal misinformation detection, featuring a new benchmark (ReMMDBench) with 500 samples, 2,756 images, and diverse linguistic/veracity settings, along with a persistent-memory agent (ReMMD-Agent) that achieves best five-way veracity performance (41.80% accuracy) while reducing cost by 17.5-79.9%.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal misinformation detection benchmarks and methods poorly match real-world viral posts, which combine long multilingual narratives, multiple images, mixed provenance, and subtle text-image framing errors. Current approaches isolate short captions, single images, binary labels, or one manipulation source, and agentic verification remains costly under realistic evidence search.

Method: ReMMD includes (1) ReMMDBench — a benchmark with 500 samples, 2,756 images, five monolingual languages, two cross-lingual settings, three text-length tiers, multi-image posts, five-way veracity labels, eight distortion labels, evidence provenance, and rationales; (2) ReMMD-Agent — a persistent-memory verifier that decomposes posts into atomic points, builds a reusable evidence set, and predicts structured L1/L2/L3 outputs.

Result: ReMMD-Agent achieved the best five-way veracity performance with 41.80% accuracy and 39.12% macro-F1 using GPT-5.2, while reducing cost by 17.5% relative to MMD-Agent and 79.9% relative to T2-Agent.

Conclusion: ReMMD demonstrates that realistic evaluation settings (multi-image, multilingual, multi-granularity labels) are feasible and important for advancing multimodal misinformation detection, and that persistent-memory agent architectures can improve both accuracy and cost efficiency.
...
### [223] [VeryTrace: Verifying Reasoning Traces through Compilable Formalism and Structured Verification](https://arxiv.org/abs/2606.24124)
*Ninghan Zhong,Ahmet Ege Tanriverdi,Kaan Kale,Sriram Vishwanath*

Main category: cs.AI

TL;DR: VeryTrace is a zero-shot framework that converts natural language reasoning traces into a structured DSL for hybrid verification (deterministic + LLM audits), enabling step-level error localization and repair. It improves accuracy across math, robotics planning, and kinship reasoning without domain-specific training.


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought reasoning is fragile because early errors silently propagate, leading to confident but incorrect conclusions. Existing methods lack mechanisms to formally verify and repair reasoning traces.

Method: VeryTrace introduces a DSL that makes step dependencies explicit, mechanizes quantitative content as executable expressions, and structures inferences via deduction schemas. A hybrid verifier uses deterministic checks (computational correctness, dependency resolution, constraints) plus targeted LLM audits for non-mechanizable semantic judgments to localize and repair errors at the step level.

Result: Across AIME 2025 (math), LLM-BabyBench (robotics), and CLUTRR (kinship), VeryTrace improves accuracy over zero-shot baselines on state-of-the-art LLMs without domain-specific training or in-context examples.

Conclusion: Formalized trace verification achieves both precision (via deterministic checks) and generalization (across diverse domains) for multi-step reasoning.
...
### [224] [OmniPath: A Multi-Modal Agentic Framework for Auditing Wheelchair Accessibility](https://arxiv.org/abs/2606.24129)
*ASM Mobarak Hossain,Nadim Mahmud,Vaskar Raychoudhury,Md Osman Gani*

Main category: cs.AI

TL;DR: OmniPath is a system that fuses OpenStreetMap data with high-density aerial LiDAR to create a 3D pedestrian model, then virtually traverses paths in 0.5m increments to quantify physical friction points (slope, cross slope, vertical discontinuities) against ADA standards, validated against 200 ground truth surveys on the National Mall.


<details>
  <summary>Details</summary>
Motivation: Standard maps (even OSM) fail to convey how a path physically feels to wheelchair users; the "blue line on a map is often a broken promise" because it captures location but not surface conditions, creating problematic information barriers for wheelchair users.

Method: The framework fuses OSM network topology with USGS 3DEP aerial LiDAR data (submeter precision) to create a high-fidelity 3D model. An agent virtually traverses the network in 0.5m increments, quantifying running slope, cross slope, and vertical discontinuities against ADA compliance standards, and computes a weighted severity score (Mild to Critical). Validation uses 200 stratified random ground truth surveys on the National Mall.

Result: Strong diagnostic reliability for high-severity hazards: F1-scores of 0.60 (Severe) and 0.58 (Critical) categories based on 200 field surveys.

Conclusion: OmniPath automates micro-scale inspection to identify "invisible" barriers that standard maps miss, transforming static datasets into proactive accessibility data that anticipates challenges before the user leaves home.
...
### [225] [T2D-Bench: Evidence-Gated Evaluation of LLM Outputs for Type 2 Diabetes Using a Multi-Layer Clinical-Lifestyle Knowledge Graph](https://arxiv.org/abs/2606.24145)
*Saba A. Farahani,Hung Cao,Ramesh Jain,Amir M. Rahmani*

Main category: cs.AI

TL;DR: T2D-Bench is a benchmark and evaluation framework that tests whether LLM outputs satisfy evidence requirements for type 2 diabetes, using a multi-layer knowledge graph. Baseline LLMs failed evidence-path checks in 33-35% of cases, and the framework's evidence gate detects and corrects unsupported omissions.


<details>
  <summary>Details</summary>
Motivation: LLMs can generate clinically fluent but guideline-noncompliant recommendations for type 2 diabetes, making it necessary to systematically detect and measure unsupported clinical omissions.

Method: T2D-Bench combines a multi-layer clinical-lifestyle knowledge graph (UMLS, DrugBank, SIDER, ADA Standards of Care rules, lifestyle-laboratory mechanistic bridges) with an evidence-gated evaluation framework that detects unsupported omissions and applies constrained revision for compliance.

Result: GPT-4o-mini failed evidence-path checks in 35% of cases and GPT-4o in 33% across 100 vignettes spanning diagnosis, medication safety, and adversarial lifestyle conflicts.
...
### [226] [The Geometry Behind Diffusion and Flow Matching: Gradient Flows and Geodesics in Wasserstein Space](https://arxiv.org/abs/2606.24157)
*Yian Yao,Weiwei Zhang*

Main category: cs.AI

TL;DR: This paper unifies diffusion models (DDPM, DDIM, NCSN/SMLD, Energy Matching) and Flow Matching on the Wasserstein manifold: diffusion models solve free-energy gradient flows (Fokker-Planck via JKO scheme), while Flow Matching solves optimal-transport geodesics (Benamou-Brenier). Both reach the same target distribution via different paths—initial-value vs. boundary-value problems.


<details>
  <summary>Details</summary>
Motivation: To provide a unified geometric framework explaining the relationship between diffusion models and Flow Matching, showing they are not separate theories but different variational principles on the same Wasserstein manifold.

Method: The paper identifies $\mathcal{P}_2(\mathbb{R}^d)$ with the quadratic Wasserstein distance as a Riemannian manifold, then shows: (1) diffusion models correspond to gradient flow of KL divergence (Fokker-Planck equation, JKO scheme), and (2) Flow Matching corresponds to geodesic/optimal-transport paths (Benamou-Brenier formula).

Result: DDPM, DDIM, NCSN/SMLD, and Energy Matching are all unified as one JKO scheme. Flow Matching learns deterministic ODEs along Wasserstein geodesics, requiring fewer sampling steps.

Conclusion: Diffusion and Flow Matching are two different variational principles on the same Wasserstein geometry: an initial-value problem (gradient flow) vs. a boundary-value problem (geodesic), both reaching the same target distribution.
...
### [227] [An Introduction to Causal Reinforcement Learning](https://arxiv.org/abs/2606.24160)
*Elias Bareinboim,Junzhe Zhang,Sanghack Lee*

Main category: cs.AI

TL;DR: This paper bridges causal inference and reinforcement learning by framing RL environments as structural causal models, introducing a unified treatment of online, off-policy, and causal learning, and proposing novel CRL settings like generalized policy learning, imitation learning, and counterfactual learning.


<details>
  <summary>Details</summary>
Motivation: Causal inference and reinforcement learning have evolved independently despite both operating on counterfactual relations. The authors identify this gap and argue that explicitly acknowledging their connection unlocks novel learning opportunities.

Method: The paper decomposes any RL environment as a collection of autonomous mechanisms modeled as a structural causal model (SCM). This formalization unifies online, off-policy, and causal calculus learning, and introduces new learning modalities through a causal lens.

Result: Identifies that standard RL implicitly encodes an SCM. Introduces generalized policy learning, imitation learning under interventions, and counterfactual learning as new pervasive classes of learning settings.

Conclusion: Proposes causal reinforcement learning (CRL) as a broader framework that studies causal inference and RL side by side, opening new dimensions for analysis and learning beyond existing paradigms.
...
### [228] [Data Scale, Not Latency, Shapes Cross-Lingual Encoder Transfer in Streaming ASR](https://arxiv.org/abs/2606.24169)
*Nenad Banfic*

Main category: cs.AI

TL;DR: A controlled study of multilingual vs. English-only encoder warm starts for streaming speech recognition across 8 languages, 5 data scales, and 3 latency tiers shows multilingual initialization helps at low data (4.21 pp WER gap at 100h), but the advantage decays to near zero by 2500h, is stable across latency tiers, and quantization decisions are independent of the initialization choice.


<details>
  <summary>Details</summary>
Motivation: To resolve the unclear trade-off between multilingual and English-only encoder initialization for adapting streaming speech recognition to new languages, specifically examining how data scale, streaming latency, and quantization affect this choice.

Method: Controlled experiments with a 0.6B-parameter FastConformer transducer across 8 European languages, varying target-language data from 100h to 2500h, testing 3 streaming latency tiers plus offline decoding, and evaluating on up to 4 public test sets.

Result: Multilingual initialization's WER advantage decays sharply with data scale (4.21pp → 0.20pp), follows a power-law (advantage halves per data doubling), is stable across latency tiers, and 4-bit quantization adds ~0.5pp WER increase regardless of initialization choice.

Conclusion: Use multilingual initialization for low-data regimes; the choice becomes irrelevant at large data scales; latency and quantization decisions can be made independently of the warm-start strategy.
...
### [229] [Navigating User Behavior toward Personalized Multimodal Generation](https://arxiv.org/abs/2606.24196)
*Hengji Zhou,Yufeng Liu,Ye Liu,Yong Xu,Lianghao Xia,Liqiang Nie*

Main category: cs.AI

TL;DR: NaviGen turns user interaction history into executable instructions for AIGC by coupling collaborative and textual codes via dual identifiers, trained with two-stage SFT+RL pipeline, improving personalized image/video generation across product, game, and short-video domains.


<details>
  <summary>Details</summary>
Motivation: Users rarely articulate visual details, causing misalignment between AIGC output and user demand. The paper aims to enable personalized content generation by converting interaction history into executable instructions.

Method: NaviGen represents each item with a dual identifier coupling collaborative code (behavioral substrate) and textual code (semantic bridge). A two-stage SFT+RL pipeline first distills preference reasoning and instruction writing from evolutionarily searched supervision, then aligns generation with user intent via hierarchical and self-consistent rewards.

Result: Improved personalized image and video generation, strengthened next-item prediction, and yielded more specific, relevant, and visually generatable instructions across product, game, and short-video domains.

Conclusion: Encoding user behavior into a language-legible form and acquiring instruction-writing skill through SFT+RL effectively bridges the gap between user history and AIGC synthesis, enabling better personalization.
...
### [230] [Exploring the relationship between human-centric AI and firm idiosyncratic risks](https://arxiv.org/abs/2606.24224)
*Zhen-Yuan Ralph Liu,Yu-Ting Wang,Jia-Jia Yan,Shivam Gupta,Mihalis Giannakis*

Main category: cs.AI

TL;DR: This study examines how Human-centric AI (HCAI) reduces firms' idiosyncratic risks (IR) in Industry 5.0, using Chinese listed firm data (2015-2023). HCAI lowers IR by reducing ethical risks and fostering AI-human synergies. Digitalisation and executive shareholding strengthen this effect, while operational efficiency and IT-background CEOs weaken it.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored relationship between HCAI and firms' idiosyncratic risks, which is critical for firms navigating financial risks during the current technological revolution.

Method: Multi-source panel dataset of Chinese listed firms from 2015 to 2023, integrating situated AI theory with social-technical systems theory.
...
### [231] [FlowR2A: Learning Reward-to-Action Distribution for Multimodal Driving Planning](https://arxiv.org/abs/2606.24231)
*Xirui Li,Zhe Liu,Xiaoqing Ye,Wenhua Han,Yifeng Pan,Junyu Han,Hengshuang Zhao*

Main category: cs.AI

TL;DR: FlowR2A unifies scoring-based dense reward supervision with anchor-based generative proposal planning via reward-conditioned flow matching, achieving SOTA on NAVSIM benchmarks.


<details>
  <summary>Details</summary>
Motivation: To resolve the tension between scoring-based methods (dense supervision but fixed action vocabulary) and anchor-based methods (dynamic proposals but sparse supervision) in multimodal driving planning.

Method: Reframes simulation-based rewards as generative conditions using a flow-matching decoder to learn reward-conditioned action distributions from dense trajectory-reward pairs, with per-timestep reward conditioning and noise augmentation.

Result: State-of-the-art results on NAVSIM v1 and v2 benchmarks with substantially higher quality multimodal proposals than prior methods.

Conclusion: FlowR2A effectively bridges the gap between scoring and anchor-based paradigms by forcing the model to internalize action-outcome correlations across safety, progress, comfort, and rule compliance, with controllable test-time sampling via reward guidance.
...
### [232] [SP-Mind: An Autonomous Reasoning Agent for Spatial Proteomics Analysis](https://arxiv.org/abs/2606.24235)
*Yucheng Yuan,Yuanfeng Ji,Zhongxiao Li,Ruijiang Li*

Main category: cs.AI

TL;DR: SP-Mind is the first autonomous AI agent that unifies the spatial proteomics analysis pipeline, converting natural-language queries into end-to-end workflows, evaluated on SP-Bench with 102 tasks across 18 categories, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current spatial proteomics analysis workflows are fragmented, requiring manual orchestration of heterogeneous tools, which limits scalability and reproducibility.

Method: SP-Mind is an autonomous AI agent equipped with expert-curated biological analysis skills and specialized computational tools that converts natural-language queries into end-to-end analytical workflows without task-specific fine-tuning. SP-Bench benchmark includes 102 tasks across 18 categories spanning diverse tissue types.
...
Conclusion: SP-Mind successfully unifies and automates spatial proteomics analysis, improving scalability and reproducibility in this domain.
...
### [233] [Towards Federated Long-Tailed Graph Learning: An Energy-Guided Dual Decoupling Approach](https://arxiv.org/abs/2606.24237)
*Lianshuai Guo,Zhongzheng Yuan,Xunkai Li,Meixia Qu,Wenyu Wang*

Main category: cs.AI

TL;DR: FedEPD is a federated graph learning framework that uses dual decoupling (topological purification + semantic recalibration) to address long-tailed data distributions, achieving SOTA results with up to 4.97% Accuracy and 5.48% Macro-F1 improvements.


<details>
  <summary>Details</summary>
Motivation: Long-tailed data distributions in federated graph learning cause global model bias toward majority classes and structural isolation of minority nodes in heterophilic neighborhoods, while existing topology-agnostic methods overfit structural noise.

Method: FedEPD proposes (1) distribution-aware Dirichlet energy pruning to filter heterophilic edges, (2) robust global prototypes from topologically central nodes via spatial low-pass injection, and (3) two-stage alternating optimization to protect majority boundaries while improving minority accuracy.
...
Conclusion: The dual decoupling paradigm effectively separates topological purification from semantic recalibration, addressing long-tail degradation in federated graph learning.
...
### [234] [Probing the Misaligned Thinking Process of Language Models](https://arxiv.org/abs/2606.24251)
*Kaiwen Zhou,Constantin Venhoff,Jonathan Michala,Xin Eric Wang,William Saunders*

Main category: cs.AI

TL;DR: The paper proposes detecting misaligned behaviors in LLMs by decomposing them into 18 fine-grained cognitive indicators and training linear probes on internal model activations, achieving 0.935 AUROC on OOD benchmarks with low false positive rates.


<details>
  <summary>Details</summary>
Motivation: As LLMs exhibit increasingly sophisticated misaligned behaviors (strategic deception, sandbagging, self-preservation) and are deployed in high-stakes settings, there is a critical need for reliable detection methods to ensure safe and responsible use.

Method: Decomposes misalignment into 18 fine-grained cognitive indicators (taxonomy), uses an automated meta-plan-guided pipeline to generate multi-turn training conversations, trains linear probes on internal model activations to detect these indicators, and evaluates on an OOD suite combining automated elicitation, established benchmarks, and benign conversations.

Result: Linear probes match a strong LLM judge with 0.935 AUROC on out-of-distribution benchmarks while maintaining low false positive rates on benign traffic across 5 misaligned behaviors.

Conclusion: Monitoring internal representations via fine-grained cognitive indicators is an effective and reliable approach for detecting LLM misalignment, with strong generalization to unseen distributions.
...
### [235] [Tractable Reasoning and Conjunctive Query Answering for Defeasible DL-Lite under Rational Closure](https://arxiv.org/abs/2606.24279)
*Giovanni Casini,Umberto Straccia*

Main category: cs.AI

TL;DR: This paper shows how to efficiently apply Rational Closure (non-monotonic reasoning) to DL-Lite logics for instance checking and conjunctive query answering using a plug-in architecture that works with existing classical reasoners with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: To extend Rational Closure, a well-established non-monotonic formalism for handling defeasible knowledge in Description Logics, to the lightweight DL-Lite family (core and horn variants), and to provide efficient reasoning and query answering methods.

Method: The authors propose a plug-in architecture that leverages existing standard classical reasoners to handle reasoning under Rational Closure for DL-Lite, including both instance checking (entailment) and Conjunctive Query (CQ) answering.

Result: Reasoning and CQ answering under Rational Closure for DL-Lite can be performed efficiently with minimal computational overhead compared to classical reasoning.

Conclusion: The plug-in approach enables practical non-monotonic reasoning in lightweight DLs by building on existing classical reasoners, making Rational Closure feasible for DL-Lite without significant performance penalties.
...
### [236] [LemonHarness Technical Report](https://arxiv.org/abs/2606.24311)
...
TL;DR: LemonHarness is an execution framework for long-horizon LLM agents that constrains state-changing operations within a defined workspace, introduces a reusable rule knowledge base, and adds time-aware execution, achieving 86.52% accuracy on Terminal-Bench 2.0.


<details>
  <summary>Details</summary>
Motivation: LLM agents working on long tasks modify workspace state across many rounds, but state-changing operations (file writes, artifact generation) scatter changes across paths without explicit boundaries, making states difficult to track over time.

Method: LemonHarness establishes an explicit execution boundary by constraining state-changing operations within a defined workspace, brings model invocation/tool execution/rule knowledge under one boundary, uses structured tool interfaces for file writes/dependency installs/artifact creation, introduces a reusable rule knowledge base, and adds time-aware execution that exposes elapsed/remaining budget to the model.

Result: On Terminal-Bench 2.0, LemonHarness_GPT-5.3-CodeX reached 84.49% accuracy over 445 trials; with GPT-5.5 backbone, average accuracy rose to 86.52% across five jobs.
...
### [237] [Prob-BBDM: a Probabilistic Brownian Bridge Diffusion Model for MRI sequence image-to-image translation](https://arxiv.org/abs/2606.24313)
*Martin Valls,Pascal Bourdon,Christine Fernandez-Maloigne,Guillaume Herpe,David Helbert*

Main category: cs.AI

TL;DR: A novel Probabilistic-Brownian Bridge Diffusion Model (Prob-BBDM) for MRI sequence synthesis from 2D axial slices achieves high quality (88.46% SSIM, 26.09 dB PSNR) with only 4 diffusion steps, and preserves clinical utility (88.71% Dice score for tumor segmentation).


<details>
  <summary>Details</summary>
Motivation: Multi-modal MRI acquisition is resource-intensive and time-consuming for 3D imaging; there is a need for efficient, high-quality image-to-image translation to reduce clinical burden.

Method: Proposes Prob-BBDM integrating variational encoder-guided diffusion with Brownian Bridge Diffusion Model (BBDM) for 2D axial slice-to-MRI sequence synthesis.

Result: Achieves 88.46% SSIM and 26.09 dB PSNR on BraTS 2021; requires only 4 diffusion steps; maintains performance on external dataset; tumor segmentation Dice 88.71% and HD95 3.49 mm.

Conclusion: Prob-BBDM offers efficient, high-quality, generalizable MRI synthesis with preserved diagnostic value, promising for clinical medical image translation.
...
### [238] [MVG-KAN: Multi-View Geo-Wind Guided KAN for PM$_{2.5}$ Forecasting](https://arxiv.org/abs/2606.24347)
*Cheng Huang,Muyao Guan,Jairus Yougui Railey,Ning Xu,Honghui Xu,Changjiang Zhang,Zhen Zhang,Shiqing Zhang,Cong Bai*

Main category: cs.AI

TL;DR: MVG-KAN uses a multi-view approach (periodic, residual temporal, and wind-guided spatial) with a Geo-Wind Graph and temporal KAN to improve short-term PM2.5 forecasting.


<details>
  <summary>Details</summary>
Motivation: Accurate PM2.5 forecasting is critical for public health and environmental management, but existing spatio-temporal methods fail to capture heterogeneous driving factors like wind-dependent pollutant transport.

Method: MVG-KAN decomposes PM2.5 into periodic (daily/weekly) and residual components, uses a Geo-Wind Graph for wind-aware spatial propagation, and applies a temporal KAN residual head for nonlinear autoregressive correction.

Result: The paper does not report specific numerical results in the abstract, but claims the method outperforms existing approaches by comprehensively modeling periodic, residual, and wind-guided spatial dynamics.

Conclusion: MVG-KAN effectively addresses the limitations of existing spatio-temporal models for PM2.5 forecasting by integrating multi-view representations including wind-direction-dependent transport.
...
### [239] [Accelerating Disaggregated RL for Visual Generative LLMs with Diffusion-Based Parallelism and Trainer-Assisted Generation](https://arxiv.org/abs/2606.24369)
*Sijie Wang,Zhengyu Qing,Zhiqiang Tan,Yiming Yin,Yeqing Zhang,Yaoyuan Wang,Qiang Wang,Xiaowen Chu,Shaohuai Shi*

Main category: cs.AI

TL;DR: DigenRL is a disaggregated RL framework for diffusion-based generative LLMs that separates rollout and training resources, achieving 1.56-2.10x throughput improvements via generation-axis pipeline, time-step parallelism, elastic trainer-assisted generation, and tightly one-step constrained asynchrony.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion RL systems (e.g., veRL-Omni) rely on colocated execution that couples rollout and training resources, limiting heterogeneous deployment and independent scaling. Efficient RL systems for diffusion generative LLMs remain underexplored.

Method: DigenRL introduces three key techniques: (1) generation-axis pipeline (GAP) and time-step parallelism (TSP) for finer-grained pipelining, (2) elastic trainer-assisted generation (TAG) for dynamic GPU resource sharing, and (3) tightly one-step constrained asynchronous strategy to utilize tail pipeline bubbles.

Result: Experiments on 16-32 GPUs with HunyuanVideo-13B, Wan2.1-14B, FLUX.1-12B, and QwenImage-20B models show DigenRL achieves 1.56-2.10x throughput improvements over state-of-the-art diffusion RL systems (veRL-Omni and GenRL).

Conclusion: DigenRL effectively addresses the resource coupling problem in diffusion RL by disaggregating rollout and training, enabling flexible resource allocation, heterogeneous GPU support, and efficient task scheduling for diffusion-based generative LLMs.
...
### [240] [When Helpfulness Overrides Causal Caution: Context-Dependent Suppression and Recovery in LLMs](https://arxiv.org/abs/2606.24370)
*Hiroshi Okumura*

Main category: cs.AI

TL;DR: This paper finds that LLMs exhibit strong Causal Caution (refraining from causal judgment without sufficient evidence) in academic contexts (91.7-100%) but dramatically lose it in practical advisory contexts (6.7-18.3%). A simple self-correction prompt restores this caution to 71.4-100%, suggesting the issue is context-dependent expression rather than capability limitation.


<details>
  <summary>Details</summary>
Motivation: While prior research has evaluated LLMs' causal reasoning capabilities, the more fundamental epistemic dimension of Causal Caution—knowing when NOT to make causal judgments—has been overlooked, which is critical for responsible deployment in business and policy decision-support roles.

Method: Using an evaluation rubric inspired by Pearl's Causal Hierarchy (PCH score), the authors conducted 480 trials across four LLMs (Claude Sonnet 4.6, Claude Opus 4.7, GPT 5.5, Gemini 3.1 Pro), comparing Causal Caution maintenance rates in academic vs. practical advisory contexts, including a self-correction prompt intervention.

Result: Causal Caution rates dropped from 91.7-100% (academic) to 6.7-18.3% (practical). Only 0.5% of practical responses requesting concrete recommendations maintained caution. A self-correction prompt restored caution to 71.4-100%.

Conclusion: The suppression of Causal Caution is driven by helpfulness-oriented response patterns in practical contexts, not by capability limitations. This suggests multi-agent architectures separating proposal generation from causal auditing as a promising governance design for organizational use.
...
### [241] [PHANTOM: A Large-Scale Dataset of Multimodal Adversarial Attacks for Vision-Language Models](https://arxiv.org/abs/2606.24388)
*Simone Gallivanone,Hossein Khodadadi,Mauro Dore,Mauro Medda,Nicola Franco*

Main category: cs.AI

TL;DR: The paper introduces a large-scale, open-source dataset (47,524 samples) of pre-generated adversarial attacks across 10 high-level and 55 subcategories of harmful intents for vision-language models, designed to make adversarial research more accessible and reproducible.


<details>
  <summary>Details</summary>
Motivation: Adversarial attack generation for VLMs is computationally expensive and complex, creating a barrier for researchers. The authors aim to democratize access to diverse, pre-generated adversarial samples to lower this barrier and enable systematic, reproducible safety evaluations.

Method: The dataset is built by consolidating and extending prior benchmarks from multiple sources, resulting in 7,826 intents across 10 high-level and 55 subcategories. It includes 47,524 adversarial samples generated using state-of-the-art attack strategies from recent literature, plus an additional category to broaden coverage.

Result: A large-scale, open-source dataset of 47,524 pre-generated adversarial samples covering 10 high-level categories and 55 subcategories of harmful intents, derived from 7,826 intents.

Conclusion: This resource enables researchers to systematically evaluate VLM robustness and safety, fine-tune attack-generation models, and develop defensive guardrails, thereby fostering more reproducible, comprehensive, and comparable evaluations.
...
### [242] [Age of LLM: A Strategic 1v1 Benchmark for Reasoning, Diplomacy and Reliability of Large Language Models under Fog of War](https://arxiv.org/abs/2606.24391)
*Arnaud Ricci*

Main category: cs.AI

TL;DR: Age of LLM is a turn-based 1v1 benchmark where LLMs play a grid-based war game with fog of war, diplomacy, and strict JSON formatting. Over 54 matches, findings show nuclear rush dominates (~78%), diplomacy is prolific but rarely consummated, ~58% of illegal actions are belief-tracking errors, and a weak link between reliability and winning emerges. The main contribution is the corpus as a lens for studying LLM reasoning under adversarial uncertainty, not the ranking.


<details>
  <summary>Details</summary>
Motivation: To study how LLMs reason under adversarial uncertainty, belief-tracking, deception, and strategic decision-making in a complex, interactive environment with minimal contamination risk (private engine, random seeds, fresh opponents).

Method: A turn-based 1v1 game on a 13x7 grid with three stressors: fog of war, full diplomacy (messages, ceasefires, secret uranium), and strict JSON reliability enforcement. 15 reasoning models played 54 matches (5,258 actions). Prompts were rule-only with no build-order advice.

Result: (1) Nuclear rush dominates (78% v0.11+; 85% overall); (2) military conquest rare but faster; (3) diplomacy prolific but rarely consummated; (4) ~58% illegal actions are fog/state errors; (5) exploratory finding: weak link between reliability and winning.

Conclusion: The ranking is preliminary and not a contribution; the real value is the turn-by-turn corpus as a lens for studying LLM reasoning under adversarial uncertainty—belief-tracking, spontaneous deception, and cognitive personas—framed as future work. Data and viewer are released.
...
### [243] [ATRIA: Adaptive Traceable ECG Reporting with Iterative Agents](https://arxiv.org/abs/2606.24392)
*Donggyun Hong,Kyuhwan Lee,Junmyung Kwon,Yong-Yeon Jo*

Main category: cs.AI

TL;DR: ATRIA is a multi-agent ECG reporting system that decouples interpretation and reporting, supports iterative bidirectional editing, and binds claims to evidence, enabling clinicians to verify findings progressively rather than accept a single opaque output.


<details>
  <summary>Details</summary>
Motivation: Existing ECG report generation is tightly coupled (end-to-end fusion of interpretation and reporting, propagating errors without recourse) or agent-based but single-pass (never revisiting earlier outputs). Clinical ECG reporting is inherently iterative, requiring progressive context integration and bidirectional editing.

Method: ATRIA is a multi-agent system that: (1) binds every report claim to supporting evidence, (2) flags unsupported statements, (3) incorporates additional context mid-session, and (4) allows clinicians to verify and revise individual findings. It uses ECG analysis models already in clinical use and is deployed as a cloud-based web service.

Result: Demonstrated through four interaction cases with a live demo and video available. The system is ready for immediate deployment as a cloud web service using clinically trustworthy models.

Conclusion: ATRIA mirrors the clinician's iterative workflow, enabling progressive context integration and bidirectional editing, addressing limitations of both end-to-end and single-pass agent-based ECG report generation systems.
...
### [244] [Cycle-Consistent Neural Explanation of Formal Verification Certificates](https://arxiv.org/abs/2606.24414)
*Andoni Rodriguez,Alberto Pozanco,Daniel Borrajo*

Main category: cs.AI

TL;DR: A cycle-consistent neural architecture that generates faithful natural language explanations of formal verification certificates, achieving 90.0% cycle-verified soundness and 860x faster inference compared to multi-LLM baselines.


<details>
  <summary>Details</summary>
Motivation: Formal verification certificates are opaque to non-specialist stakeholders; existing methods lack faithful, efficient, and accessible natural language explanations of verification results.

Method: A cycle-consistent architecture with forward (NN1) and inverse (NN2) neural networks, a symbolic verifier for differentiable faithfulness proxy, and a pointer-generator mechanism for lexical grounding. Evaluated on 420 test certificates across six verification methods.

Result: Achieves 90.0% cycle-verified soundness, surpassing best multi-LLM baseline (76.1%) by 13.9 points; wins on 10 of 12 categories; provides 860x faster inference (185 ms vs. 160 s per certificate) with offline, deterministic, zero-cost operation.

Conclusion: Trained specialization outperforms general-purpose LLM prompting for structured certificate explanation, eliminating deployment constraints of cloud-based inference while delivering superior soundness and efficiency.
...
### [245] [Agentic AI for Bilevel Long-Term Optimization of Policy-Driven Physical Layer Systems](https://arxiv.org/abs/2606.24416)
*Bingnan Xiao,Chenhao Yang,Wei Ni,Xin Wang,Tony Q. S. Quek*

Main category: cs.AI

TL;DR: Agentic-LTPO is a nested bilevel optimization framework using agentic AI to adapt physical layer configurations dynamically, achieving 57.2% better long-term performance than traditional methods in cell-free MIMO beamforming.


<details>
  <summary>Details</summary>
Motivation: Existing methods with fixed objectives fail under changing operator policies, service requirements, and real-time constraints in network optimization.

Method: A nested bilevel optimization where the upper level uses multi-agent AI (with retrieval-augmented experience verification) to generate problem configurations, and the lower level solves configured problems for real-time physical-layer decisions.

Result: Agentic-LTPO achieves a 57.2% improvement in long-term performance over traditional methods, demonstrating strong adaptability to dynamic operator policies.

Conclusion: Agentic-LTPO effectively addresses dynamic network optimization by combining agentic AI with bilevel optimization, significantly outperforming fixed-objective approaches.
...
### [246] [Can Aggregate Invariants Accelerate Continuous Subgraph Matching? Limits, Laws, and a Dynamic Spectral Index](https://arxiv.org/abs/2606.24421)
*Minghao Chen,Jiale Zheng*

Main category: cs.AI

TL;DR: Spectral filtering (Laplacian interlacing) accelerates continuous subgraph matching over dynamic graphs by pruning candidates, but loses power under updates; exact maintenance is cheap for selective (small-neighborhood) vertices, removing up to 51% of candidates, skipping 47% of updates, and speeding up by 748× in favorable cases.


<details>
  <summary>Details</summary>
Motivation: To determine whether aggregate structural tests (spectral filtering), which work well for static subgraph matching, can also accelerate continuous subgraph matching (CSM) over dynamic graphs where the graph changes over time.

Method: The paper characterizes the tightest safe spectral pruning rule under perturbation relaxation, shows it loses power after few updates, proposes exact selective maintenance of local spectra for small-neighborhood vertices (hubs never prune), and integrates this into a decoupled CSM benchmark with an identical-minus-spectra control.

Result: Removes up to 51% of candidates, safely skips up to 47% of update enumerations; in a constructed radius-stratified workload, reduces intermediates by 99.9% and achieves 748× speedup; aggregate tests accelerate candidate-set operations (construction, list scans) but not adjacency-guided exploration.

Conclusion: Spectral filtering can accelerate CSM with selective exact maintenance; the paper distills an intermediate-invariance methodology for evaluating CSM filters and releases a reusable dynamic local-spectra index.
...
### [247] [ReM-MoA: Reasoning Memory Sustains Mixture-of-Agents Scaling](https://arxiv.org/abs/2606.24437)
*Heng Ping,Arijit Bhattacharjee,Peiyu Zhang,Shixuan Li,Wei Yang,Ali Jannesari,Nesreen Ahmed,Paul Bogdan*

Main category: cs.AI

TL;DR: ReM-MoA introduces memory-augmented Mixture-of-Agents architecture with ranked reasoning memory and curated diversified memory routing to sustain performance gains across deep multi-agent layers, outperforming prior MoA variants on five reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing MoA architectures suffer from degradation, plateauing, or saturation as depth increases, unable to sustain inference-time scaling gains.

Method: (1) Ranked Reasoning Memory storing/ranking reasoning traces via a Reviewer Agent; (2) Curated Diversified Memory Routing exposing agents to varied success/failure trace combinations; (3) optional multi-domain Reviewer distillation with frontier-model supervision.

Result: ReM-MoA consistently outperforms prior MoA variants across depth and width scaling on five reasoning benchmarks (math, logic, code, knowledge, commonsense), with advantages widening as depth increases.

Conclusion: Structured cross-layer reasoning memory is a key missing mechanism for achieving scalable multi-agent inference.
...
### [248] [Bayesian control for coding agents](https://arxiv.org/abs/2606.24453)
*Theodore Papamarkou,Vladislav Smirnov,Viktor Mazanov,Artem Vazhentsev,Preslav Nakov,Timothy Baldwin,Artem Shelmanov*

Main category: cs.AI

TL;DR: A Bayesian controller for coding agents dynamically decides whether to gather evidence, refine, verify, or stop, optimizing cost-sensitive sequential hypothesis testing across generators and benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing coding agents use fixed rules to decide when to use cheap diagnostics vs. expensive verifiers, ignoring uncertainty. The goal is to formulate orchestration as cost-sensitive sequential hypothesis testing to improve decision-making under uncertainty.

Method: A Bayesian controller maintains a belief over candidate correctness and dynamically decides whether to gather more evidence, refine the candidate, verify it, or stop, treating orchestration as sequential hypothesis testing.

Result: Bayesian control is most valuable when verification is costly and critics are informative but imperfect. The belief state also yields an interpretable correctness score outperforming token-probability and raw tool-success baselines for uncertainty quantification.

Conclusion: Bayesian control improves coding agent orchestration under costly verification and imperfect critics, and provides better uncertainty quantification than existing baselines.
...
### [249] [CompressKV: Semantic-Retrieval-Guided KV-Cache Compression for Resource-Efficient Long-Context LLM Inference](https://arxiv.org/abs/2606.24467)
*Xiaolin Lin,Jingcun Wang,Olga Kondrateva,Yiyu Shi,Bing Li,Grace Li Zhang*

Main category: cs.AI

TL;DR: CompressKV is a KV-cache compression framework for GQA-based LLMs that identifies Semantic Retrieval Heads (SRHs) to selectively retain critical tokens, achieving over 97% full-cache performance using only 3% KV cache on LongBench and 90% accuracy with 0.7% KV storage on Needle-in-a-Haystack.


<details>
  <summary>Details</summary>
Motivation: Long-context LLM inference is constrained by memory and decoding costs of KV caches, and existing eviction methods apply heuristic token scoring over all heads in GQA-based LLMs, ignoring different functionalities of attention heads, leading to eviction of critical tokens and degraded performance.

Method: CompressKV identifies Semantic Retrieval Heads (SRHs) that capture initial, final, and semantically important mid-context tokens, using them to select which KV pairs to retain. It also allocates cache budgets across layers based on offline estimates of layer-wise eviction error.

Result: On LongBench, it preserves over 97% full-cache performance using only 3% KV cache. On Needle-in-a-Haystack, it achieves 90% accuracy with just 0.7% KV storage, consistently outperforming existing methods across memory budgets.

Conclusion: CompressKV demonstrates an improved resource-performance trade-off for long-context LLM inference, enabling sustainable deployment on resource-constrained hardware through intelligent, head-aware KV cache compression.
...
### [250] [The Latent Bridge: A Continuous Slow-Fast Channel for Real-Time Game Agents](https://arxiv.org/abs/2606.24470)
*Bojie Li,Noah Shi*

Main category: cs.AI

TL;DR: A real-time agent couples a slow reasoning VLM (1.5s) with a fast reactive VLM (ms) using a learned Latent Bridge that projects slow-model residuals into fast-model embedding space, outperforming Text Bridge on planning-heavy tasks while matching it elsewhere.


<details>
  <summary>Details</summary>
Motivation: Real-time computer agents must balance fast reactive control (milliseconds) with deliberative planning (seconds). No single VLM achieves both, so coupling two models of complementary speeds is needed.

Method: Two frozen VLMs (9B fast reactive + 8B slow reasoning) are coupled via a learned Latent Bridge that projects slow model residuals into the fast model's embedding space (LLaVA-style), compared against Text Bridge and Fast-Only baselines.

Result: Latent Bridge matches or beats Text Bridge across 7 Atari games + MetaDrive, with significant gains in MsPacman (+57%) and RoadRunner (+28%). Dual-channel coupling causes destructive interference (-96% on RoadRunner). Bridge gains correlate with slow-reasoning advantage at r=0.93.

Conclusion: Latent Bridge is a safe drop-in replacement for Text Bridge, enabling low-latency deliberation. MetaDrive serves as a controlled negative where the bridge is inert because slow reasoning adds no value over fast reaction.
...
### [251] [On the Smallness of the Large Language Models Scaling Exponents](https://arxiv.org/abs/2606.24504)
*Sauro Succi,Peter V. Coveney,Alex Hansen*

Main category: cs.AI

TL;DR: This paper argues that the scaling exponents in current LLMs point to unsustainable energy consumption, and that the "pedestal effect" (nonzero loss floor) does not resolve this issue. It also draws an analogy between data smoothness/roughness and fluid turbulence to comment on scaling exponents.


<details>
  <summary>Details</summary>
Motivation: To highlight the unsustainability of current LLM scaling trends in terms of energy resources and to examine whether the pedestal effect or data smoothness can mitigate this concern.

Method: Analytical discussion of scaling exponents in LLMs, critique of the pedestal effect as a numerical bias, and a phenomenological analogy with fluid turbulence models to relate data roughness to scaling behavior.

Result: The pedestal effect does not resolve the unsustainability issue. Data smoothness/roughness, by analogy with turbulence, may influence scaling exponents but does not alter the fundamental energy concern.

Conclusion: Current LLM scaling trends are energetically unsustainable; neither the pedestal effect nor data smoothness considerations offer a viable escape from this problem.
...
### [252] [A specialized reasoning large language model for accelerating rare disease diagnosis: a randomized AI physician assistance trial](https://arxiv.org/abs/2606.24510)
...
TL;DR: RaDaR is an open-source 32B parameter reasoning LLM for rare disease diagnosis, trained on ~154K cases, that outperforms larger models (including 671B DeepSeek-R1) across benchmarks, achieves 61% earlier diagnosis than clinical suspicion, and improves physician diagnostic accuracy by 21.44 percentage points in a randomized trial.


<details>
  <summary>Details</summary>
Motivation: Rare diseases affect millions globally but face delayed diagnosis due to scarcity of specialized expertise; existing LLMs lack clinical deployability, grounded evidence, and sufficient training data for this domain.

Method: RaDaR is a 32B parameter reasoning LLM trained on 49,170 public free-text cases and 104,666 synthetic cases with reasoning-enhanced training; includes phenotype-anchored narrative generation for data augmentation and is validated on public benchmarks and four external centers.

Result: Outperforms all evaluated open-source models including 671B DeepSeek-R1; prioritizes final diagnosis 1.87 months before clinical suspicion (61% of cases); improves physician diagnostic accuracy by 21.44 percentage points over internet search alone; shows monotonic scaling with synthetic data volume.

Conclusion: RaDaR provides a deployable, open-source rare-disease diagnostic reasoning model and a reproducible development framework for building diagnostic AI under data-scarce conditions.
...
### [253] [Reinforcement Learning for Computer-Use Agents with Autonomous Evaluation](https://arxiv.org/abs/2606.24515)
*Marta Sumyk,Oleksandr Kosovan*

Main category: cs.AI

TL;DR: This paper proposes an RL fine-tuning framework for Computer-Use Agents that uses Vision-Language Models as autonomous evaluators to provide reward signals, with noise correction via Proximal Policy Optimization, achieving significant improvements across three GUI benchmarks.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning for Computer-Use Agents is difficult because open-ended desktop environments lack scalable, machine-readable reward signals; task success is visually grounded and hard to specify with handcrafted rewards or dense manual labels.

Method: The framework uses Vision-Language Models to judge task completion from final screenshots and instructions, modeling evaluator feedback as a noisy binary reward channel, then deriving a noise-corrected reward estimator for Proximal Policy Optimization.

Result: Experiments across macOSWorld, Windows Agent Arena, and OSWorld show corrected evaluator rewards improve success rates by 12.6 percentage points over zero-shot baselines and 5.1 points over raw evaluator fine-tuning.
...
### [254] [Governed Shared Memory for Multi-Agent LLM Systems](https://arxiv.org/abs/2606.24535)
*Yanki Margalit,Nurit Cohen-Inger,Erni Avram,Ran Taig,Oded Margalit*

Main category: cs.AI

TL;DR: This paper formalizes the fleet-memory problem in multi-agent LLM systems, identifies four failure modes, proposes systems-level primitives (scoped retrieval, temporal supersession, provenance tracking, policy-governed propagation), implements them in MemClaw, and evaluates via ArgusFleet. Key findings include successful provenance reconstruction, zero cross-fleet leakage, and discovery of real-world architectural issues like asymmetric scope enforcement and pipeline ordering conflicts.


<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM environments lack robust mechanisms for shared knowledge management, leading to critical failure modes (unauthorized leakage, stale propagation, contradiction persistence, provenance collapse) that undermine system reliability and security.

Method: The authors formalize the fleet-memory problem, define four systems-level primitives, implement them in MemClaw (a production multi-tenant memory service), and evaluate via ArgusFleet across four governance dimensions.

Result: Provenance: 100% reconstruction of depth-four derivation chains with correct writer identity at sub-second latency. Propagation: High intra-fleet visibility, zero cross-fleet leakage, write-to-visible latency optimized to single search round-trip. Discovered issues: asymmetric scope enforcement (sub-tenant bypass) and pipeline ordering conflict (near-duplicate gate rejecting contradictory writes prematurely).

Conclusion: Long-context retrieval alone is insufficient for production multi-agent memory. Governed shared memory requires explicit systems-level abstractions, and live evaluation is critical to expose enforcement and pipeline-ordering failures missed by design-only approaches.
...
### [255] [GUI vs. CLI: Execution Bottlenecks in Screen-Only and Skill-Mediated Computer-Use Agents](https://arxiv.org/abs/2606.24551)
*Xiao Zhou,Siyue Zhang,Yilun Zhao,Jinbiao Wei,Tingyu Song,Arman Cohan,Chen Zhao*

Main category: cs.AI

TL;DR: This paper creates a matched benchmark (440 desktop tasks) comparing GUI vs CLI computer-use agents with controlled conditions, finding GUI agents (59.1%) outperform original-skill CLI agents (48.2%), but skill-augmented CLI reaches 69.3%, revealing GUI limitations in grounded interaction and CLI limitations in skill coverage.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of computer-use agents confound interaction modality with differences in tasks, initial states, verifiers, and permitted actions, making it impossible to directly compare GUI and CLI approaches fairly.

Method: Created a matched execution-layer benchmark of 440 desktop tasks across 18 applications and 12 workflow categories, where screen-only GUI agents and skill-mediated CLI agents receive identical goals, states, and final-state verifiers while being restricted to modality-native actions.

Result: GUI agent: 59.1% full pass rate; strongest original-skill CLI: 48.2%; verifier-guided skill-augmented CLI: 69.3%.

Conclusion: GUI and CLI expose different execution bottlenecks: GUI agents are limited by reliable grounded interaction over long-horizon workflows, while CLI agents are limited by the coverage and scalability of their skill interfaces. Much of the CLI deficit comes from incomplete skill coverage rather than model capability alone.
...
### [256] [Quant Convergence: Bridging Classical Value Investing and Modern Factor Models for Systematic Equity Selection](https://arxiv.org/abs/2606.24575)
*Augusto Eiji Yamazaki,Hugo Garrido-Lestache Belinchon*

Main category: cs.AI

TL;DR: This paper tests whether Benjamin Graham's value investing rules can act as a "low-pass filter" to prevent complex AI models from overfitting to market noise. Using 20 years of S&P 500 data, the authors compare pure Graham features, modern market factors, and a combined set across XGBoost and AutoGluon models. Results show that the pure Graham Random Forest achieved 232.13% return with low risk (1.38 Calmar Ratio), outperforming the more complex AutoGluon (222.68% return, 39.78% drawdown). Graham's "margin of safety" effectively prevents AI from taking excessive risk.


<details>
  <summary>Details</summary>
Motivation: Modern machine learning models in finance tend to memorize short-term market noise instead of identifying companies with lasting value. The authors aim to test whether Graham's classic value investing rules can mathematically constrain these models to avoid excessive risk-taking.

Method: Three feature sets (pure Graham rules, modern market factors, mixed) were tested against XGBoost and AutoGluon models on 20 years of S&P 500 data. A strict buy-and-hold strategy was applied over a four-year test period (March 2022 - March 2026).

Result: Pure Graham Random Forest achieved highest return (232.13%) with low risk (1.38 Calmar Ratio). AutoGluon captured 222.68% but suffered 39.78% drop. Combined Random Forest achieved 202.91% return with the lowest maximum drawdown (34.53%).

Conclusion: Graham's "margin of safety" remains effective in the AI era. Value investing principles serve as a powerful risk-control mechanism for modern machine learning models, proving that complex algorithms don't always outperform simpler, principles-based approaches.
...
### [257] [LLMs Prompted for Legal Context Object More: Overrefusal from Small On-Premises LLMs in Criminal Legal Context](https://arxiv.org/abs/2606.24585)
*Anastasiia Kucherenko,François Brouchoud,Dimitri Percia David,Andrei Kucharavy*

Main category: cs.AI

TL;DR: Small LLMs used as on-device legal assistants show unstable refusal behavior, with authority-style prefixes (e.g., "supreme court assistant") increasing refusal rates 2-20x, potentially introducing bias in legal processing.


<details>
  <summary>Details</summary>
Motivation: Legal professionals already use personal LLMs for translation/reformulation, but even these seemingly innocuous uses can introduce biases if LLMs selectively refuse assistance on certain topics. The authors seek to anticipate such biases.

Method: The authors investigate several modern small LLMs (suitable for on-device deployment) and evaluate their refusal rates on legal prompts under different prefix conditions: no-prefix baseline, authority-style prefixes (e.g., "supreme court assistant", "defense lawyer"), and a known role-play jailbreak prefix.

Result: Authority-style prefixes systematically increase refusal rates by 2-20x compared to baseline. The role-play jailbreak prefix shows mixed effects—sharply increasing refusals in some models while barely affecting others.

Conclusion: Small on-premise deployable LLMs are unstable under contextual framings that real institutional users might naturally introduce, and further investigation is essential to minimize opportunities for bias.
...
### [258] [AdversaBench: Automated LLM Red-Teaming with Multi-Judge Confirmation and Cross-Model Transferability](https://arxiv.org/abs/2606.24589)
*Khanak Khandelwal*

Main category: cs.AI

TL;DR: AdversaBench is an end-to-end red-teaming pipeline that mutates seed prompts with five operators, queries target LLMs, and validates failures via a three-judge panel with meta-judge tiebreaker. Evaluated on 45 seeds across reasoning, instruction-following, and tool use, it found confirmed failures on every seed. Key findings: operator effectiveness varies by category, instruction-following requires more iterations, judge agreement is skewed, and adversarial prompts transfer zero-shot across model sizes.


<details>
  <summary>Details</summary>
Motivation: To develop a scalable, reliable method for adversarial evaluation of large language models that can generate hard inputs and confirm resulting failures are real.

Method: AdversaBench mutates seed prompts with five structured operators, queries a target model, and confirms failures through a three-judge panel with a meta-judge tiebreaker.

Result: Every seed produced a confirmed failure. Operator effectiveness varies by category (e.g., inject_distractor scores 0.00 on instruction-following vs 0.80-0.83 on others). Instruction-following requires 2.4 attacker iterations vs 1.1 for others. Pairwise judge agreement 80-87% but near-zero Cohen's kappa due to label skew. Adversarial prompts transfer zero-shot from Llama 3.1 8B to Llama 3.3 70B.

Conclusion: Adversarial mutations exploit general behavioral patterns rather than model-specific weaknesses, and category-level analysis reveals important nuances hidden by aggregate metrics like binary failure rates or Cohen's kappa.
...
### [259] [ASALT: Adaptive State Alignment for Lateral Transfer in Multi-agent Reinforcement Learning](https://arxiv.org/abs/2606.24601)
*Anurag Akula,Satheesh K. Perepu,Abhishek Sarkar,Kaushik Dey*

Main category: cs.AI

TL;DR: ASALT introduces observation-level and state-level adapters to enable transfer learning in MARL across domains with mismatched state-space dimensionalities, outperforming baselines in cooperative settings while mitigating negative transfer.


<details>
  <summary>Details</summary>
Motivation: Existing MARL transfer learning methods require identical observation and global state space dimensionalities across source and target domains, limiting their applicability to heterogeneous scenarios.

Method: ASALT employs both observation-level and state-level adapters that map target-domain observations and global states into a shared embedding space, enabling knowledge transfer across both actors and critics.

Result: ASALT surpasses existing baselines in sample efficiency and global return in cooperative settings, but effectiveness depends on the degree of domain mismatch; it also mitigates negative transfer.

Conclusion: The proposed adapter-based approach effectively enables cross-domain policy transfer in MARL without requiring identical state-space dimensionalities, offering robustness against negative transfer.
...
### [260] [Uncertainty-Aware Longitudinal Forecasting of Alzheimer's Disease Progression Using Deep Learning](https://arxiv.org/abs/2606.24604)
*Arya Hariharan,Shreyank N Gowda,Anala M R*

Main category: cs.AI

TL;DR: A probabilistic framework combining Temporal Fusion Transformer with CORAL ordinal output, Mixture Density Network for trajectory generation, and bootstrap ensemble uncertainty decomposition achieves strong Alzheimer's progression modelling on ADNI and OASIS-3.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches reduce Alzheimer's progression to single-step classification, treating diagnoses as flat categories and providing limited insight into how uncertainty accumulates across future visits.

Method: Temporal Fusion Transformer encoder with CORAL ordinal output layer, asymmetric loss weighting, and converter oversampling; autoregressive Mixture Density Network generates five-year probabilistic trajectories; aleatoric/epistemic uncertainty separated via analytic mixture variance and five-member bootstrap ensemble.

Result: Outperforms baselines on next-visit diagnosis, especially MCI-to-dementia discrimination; near-nominal 90% credible interval coverage; epistemic uncertainty higher for rare progression archetypes, MCI/dementia patients, and under external OASIS-3 evaluation where it correlates with prediction error.

Conclusion: The framework provides clinically useful longitudinal modelling with reliable uncertainty quantification, showing promise for patient-specific disease trajectory forecasting.
...
### [261] [ScaleToT: Generalizing Structured LLM Reasoning for Billion-Scale Low-Activity User Modeling](https://arxiv.org/abs/2606.24605)
*Tianbao Ma,Chang Xi,Yichuan Zou,Chengen Li,Linxun Chen,Zilong Lu,Yanan Niu,Zhaojie Liu,Han Li,Kun Gai*

Main category: cs.AI

TL;DR: ScaleToT uses a small LLM-processed subset to generate structured reasoning chains via Tree-of-Thought refinement, trains a student model to learn from sparse profiles, and transfers reasoning to a lightweight encoder for billion-scale user modeling without expensive LLM inference, demonstrated by 6.738% LT30 lift in advertising deployment.


<details>
  <summary>Details</summary>
Motivation: Billions of low-activity users lack rich interaction histories for accurate modeling; LLM reasoning on sparse profiles is unreliable and prohibitively expensive at scale.

Method: (1) Bounded entropy-guided Tree-of-Thought (ToT) refinement to construct typed user-state chains; (2) Teacher-student training via SFT and Outcome-Driven Segment-Aware Implicit Reward Policy Optimization (OSIPO); (3) Transfer reasoning representations to a lightweight profile encoder for population-wide coverage without LLM inference.

Result: Online A/B test showed 6.738% LT30 lift in billion-scale advertising; offline reasoning covered only 7.32% of the population, significantly reducing compute cost.

Conclusion: ScaleToT effectively scales LLM reasoning to billions of low-activity users at low cost, achieving strong performance gains in real-world deployment.
...
### [262] [AI Tokenomics: The Economics of Tokens, Computation, and Pricing in Foundation Models](https://arxiv.org/abs/2606.24616)
*Quanyan Zhu*

Main category: cs.AI

TL;DR: This paper introduces "AI tokenomics" as a framework for understanding how tokens—the fundamental unit in AI systems—relate to technical costs, economic value, production functions, resource allocation, and market design.


<details>
  <summary>Details</summary>
Motivation: Tokens have become the practical accounting unit for modern foundation model services, yet there is no unified framework connecting token-level technical costs to economic value, resource allocation, and market design.

Method: The paper develops a conceptual framework linking token-level technical costs to workflow-level production functions, enterprise resource allocation, measurement methods, and emerging market-design questions.

Result: The framework demonstrates that token expenditure and economic value are distinct—value depends on marginal productivity, workflow position, hidden reasoning activity, risk, and downstream propagation effects.
...
### [263] [Abstractions of Queries in Ontology-Based Data Access](https://arxiv.org/abs/2606.24618)
*Michel Leclère,Marie-Laure Mugnier,Guillaume Pérution-Kihli*

Main category: cs.AI

TL;DR: This paper studies query abstraction in ontology-based data access, focusing on minimally complete and maximally sound abstractions using an extension of UCQs with limited inequality and a special constant-marking predicate.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of query abstraction in OBDA settings based on existential rules, particularly when perfect abstractions may not exist.

Method: Extends UCQs with a limited form of inequality and a special predicate for marking database constants, and draws a connection between maximally sound abstractions and the concept of maximum recovery from data exchange.

Result: The extension can express minimally complete abstractions (and thus perfect abstractions when they exist) without increasing complexity, and maximally sound abstractions are characterized via maximum recovery.

Conclusion: The proposed extension is expressive enough for meaningful query abstraction while maintaining tractable complexity.
...
### [264] [When CQs Go Wrong: Challenges in CQ Verification with OE-Assist](https://arxiv.org/abs/2606.24619)
*Anna Sofia Lippolis,Mohammad Javad Saeedizade,Robin Keskisärkkä,Aldo Gangemi,Eva Blomqvist,Andrea Giovanni Nuzzolese*

Main category: cs.AI

TL;DR: This paper investigates how to improve Competency Question (CQ) verification in ontology engineering by identifying challenging CQ characteristics and testing LLM-assisted support with 19 participants, finding that tools for refining CQs before publication are needed to reduce ambiguity and complexity.


<details>
  <summary>Details</summary>
Motivation: CQ-verification is often time-consuming and error-prone due to linguistic ambiguities and complexity, leading to inconsistent modelling decisions and verification outcomes.
...
Conclusion: There is a need for tools that can refine CQs early to prevent ambiguity and complexity issues, thereby enhancing user performance in the CQ-verification process.
...
### [265] [Themis: An explainable AI-enabled framework for Reinforcement Learning with Human Feedback](https://arxiv.org/abs/2606.24622)
*Andreas Chouliaras,Luke Connolly,Dimitris Chatzpoulos*

Main category: cs.AI

TL;DR: Themis is a new XAI-enabled framework that combines explainability and Reinforcement Learning from Human Feedback (RLHF) to train safer RL systems, supporting 200+ environments and scalable human feedback collection.


<details>
  <summary>Details</summary>
Motivation: Current RL systems lack combined transparency (explainability) and alignment (human feedback) tools, making it difficult to avoid unwanted behaviors during training.

Method: Themis integrates XAI techniques with RLHF, offering a configurable framework for 200+ environments, along with a cloud-based platform for scalable human feedback collection and experiment management.

Result: Reward models trained with Themis match or outperform true reward signals; the platform supports up to 1000 concurrent users on a modest machine.

Conclusion: Themis successfully bridges explainability and alignment in RL, providing a practical, scalable open framework for safer RL training.
...
### [266] [SAFARI: Scaling Long Horizon Agentic Fault Attribution via Active Investigation](https://arxiv.org/abs/2606.24626)
...
TL;DR: SAFARI introduces a tool-augmented diagnostic loop with Short-Term Memory to replace linear context loading for diagnosing agent failures in long-horizon, multi-agent tasks, achieving 20% improvements over SOTA and maintaining high precision even beyond the model's native context window.


<details>
  <summary>Details</summary>
Motivation: Current methods for diagnosing agent failures load full trajectories into LLM context windows, which suffers from attention dilution and fails when agentic traces exceed context limits. As tasks grow more complex, trajectories scale beyond even the largest context windows.
...
Result: SAFARI outperforms SOTA by 20% on Who&When (1M token budget) and 19% on TRAIL GAIA subset (25K token budget). It maintains 0.58 precision even when the target fault resides 5x beyond the model's native context window, where traditional evaluators fail entirely.

Conclusion: SAFARI effectively decouples diagnostic accuracy from architectural context limits, enabling reliable agentic fault attribution in scenarios that exceed traditional context window constraints.
...
### [267] [CineCap: Structured Reasoning with Spatio-Temporal Anchors for Cinematographic Video Captioning](https://arxiv.org/abs/2606.24636)
*Xinyu Mao,Yuhui Zeng,Xiaokun Liu,Wenyu Qin,Meng Wang,Xin Tao,Pengfei Wan,Xiaohan Xing,Max Meng*

Main category: cs.AI

TL;DR: CineCap is a framework for cinematographic captioning that uses structured reasoning with spatio-temporal anchors and reinforcement learning to generate comprehensive and accurate descriptions of film-language concepts (camera movement, shot size, etc.), outperforming state-of-the-art baselines on a new benchmark (CineCap Bench).


<details>
  <summary>Details</summary>
Motivation: Cinematographic captioning is important for fine-grained video understanding and controllable video generation, but existing multimodal large language models fail to generate unified open-form descriptions over multiple cinematographic dimensions due to challenges in inferring professional concepts from subtle visual evidence and ensuring both comprehensiveness and accuracy.

Method: CineCap combines: (1) structured reasoning with spatio-temporal anchors to ground cinematographic descriptions in explicit visual evidence, organized into compact atomic reasoning for supervised fine-tuning; (2) reinforcement learning with comprehensiveness, accuracy, and gated coverage rewards to balance descriptive completeness and factual correctness. A benchmark (CineCap Bench) of 472 manually annotated video-caption pairs was also constructed.

Result: CineCap consistently outperforms strong proprietary and open-source baselines, establishing a new state of the art for cinematographic captioning on the CineCap Bench benchmark.

Conclusion: The proposed CineCap framework effectively addresses the underexplored task of cinematographic captioning, and its code, model checkpoint, and benchmark are publicly available for future research.
...
### [268] [LaGO: Latent Action Guidance for Online Reinforcement Learning](https://arxiv.org/abs/2606.24669)
*Kuan-Yen Liu,Ren-Jyun Huang,Ti-Rong Wu*

Main category: cs.AI

TL;DR: LaGO uses a pretrained LLM as a latent action prior to softly guide online RL (PPO), improving success rates from 15.1%→27.2% on CLEVR-Robot and 2.7%→15.2% on Meta-World, without using the LLM as a direct controller.


<details>
  <summary>Details</summary>
Motivation: LLMs have potential for planning/decision-making, but using them as direct controllers is unreliable due to precise action generation requirements. The authors propose soft guidance instead of explicit control.

Method: LaGO leverages a pretrained LLM as a latent action prior to softly guide online policy optimization (PPO), avoiding direct LLM action generation.

Result: LaGO consistently improves reward and success rate over Vanilla PPO on both CLEVR-Robot (15.1%→27.2%) and Meta-World (2.7%→15.2%). Stronger LLMs provide more effective guidance.

Conclusion: LLM knowledge can improve planning and online decision-making when used as a soft prior rather than a direct controller.
...
### [269] [Cost-Optimal Decision Diagrams for Stochastic Boolean Function Evaluation](https://arxiv.org/abs/2606.24672)
*Xia Zong,Tuomo Lehtonen,Jussi Rintanen*

Main category: cs.AI

TL;DR: Proposes the first practical exact algorithm for minimizing expected evaluation cost of propositional formulas under variable-specific costs and probabilistic truth assignments, combining branch-and-bound with heuristics, pruning, and caching.


<details>
  <summary>Details</summary>
Motivation: In many real-world decision-making scenarios, acquiring information incurs different costs (e.g., medical tests). Existing methods lacked practical exact algorithms for minimizing expected evaluation cost under variable costs and probability distributions.
...
Result: Experiments on random instances show scalability; structured heart-disease diagnosis case study demonstrates real-world applicability; theoretical results show the problem is #P-hard and in PSPACE.

Conclusion: This is the first practical exact algorithm for evaluation strategy optimization under general variable costs and probabilities, with both theoretical hardness results and empirical validation.
...
### [270] [Decentralised AI Training and Inference with BlockTrain](https://arxiv.org/abs/2606.24722)
*Peter Toth*

Main category: cs.AI

TL;DR: Spheroid BlockTrain is a decentralized training protocol that partitions models into independently trainable blocks, achieving near-Transformer quality (cross-entropy 1.359 vs 1.32 baseline) while each worker trains only one block. It demonstrates real distributed training over public networks with 15.22 GB data transfer, and inference on up to 75.8B parameter models outperforming autoregressive pipelines by emitting full sequences per traversal.


<details>
  <summary>Details</summary>
Motivation: To address the structural advantage of hyperscalers in AI training by enabling decentralized, open training without dependence on dense centralized accelerator clusters, scarce capital, or privileged data-center geography.

Method: Spheroid BlockTrain partitions a model into independently trainable blocks, each optimized on a local objective derived from the same global target, composed at inference. Uses HTTP/TCP transport for serialized checkpoints and updates with same-block averaging across workers.

Result: CE 1.359 (perplexity 3.89) on byte-level WikiText, within 0.04 CE of end-to-end Transformer baseline. Six-worker run achieves CE 1.385. Public-IP three-host run improves CE from 5.580 to 1.811 moving 15.22 GB. Inference supports up to 75.80B-parameter logical fp16 shape, outperforming plain-autoregressive TCP pipeline.

Conclusion: BlockTrain demonstrates that decentralized model training is viable and can approach centralized quality while avoiding full-model optimizer state per worker, with efficient inference over WAN.
...
### [271] [Scaling Laws for Task-Specific LLM Distillation](https://arxiv.org/abs/2606.24747)
*Lavinia Ghita,Dhruv Desai,Ioana Boier*

Main category: cs.AI

TL;DR: This paper derives empirical scaling laws for domain-specific LLM compression in quantitative finance, showing that while in-domain performance degrades predictably under compression, general knowledge collapses earlier, and chain-of-thought supervision can recover erased general knowledge.


<details>
  <summary>Details</summary>
Motivation: LLMs are too large for deployment in latency/cost-constrained applications; there is a need to understand how compression affects domain-specific vs. general knowledge performance systematically.

Method: The authors compare logit-based and LoRA-based distillation under iterative structural pruning, introduce a blended chain-of-thought supervision loss that stabilizes KL-divergence distillation over reasoning traces, and derive scaling laws across dataset size, compression ratio, supervision format, and pruning schedule.

Result: In-domain task quality degrades predictably under compression; general-knowledge benchmarks collapse well before the same point; supervision format is the key driver of this tradeoff; chain-of-thought supervision actively recovers general knowledge erased by pruning.

Conclusion: The paper provides a reusable framework (including the FinHeadlineMix dataset and scaling laws) for making domain-specific compression decisions.
...
### [272] [Can Scale Save Us From Plasticity Loss in Large Language Models?](https://arxiv.org/abs/2606.24752)
*J. Fernando Hernandez-Garcia,Tomás Figliolia,Beren Millidge*

Main category: cs.AI

TL;DR: This paper shows that GPT-style Transformer language models (5M-314M params) lose plasticity—the ability to learn new information—during multilingual continual learning, with the onset following a predictable scaling law. Plasticity loss also occurs under stationary training, not just in continual learning scenarios.


<details>
  <summary>Details</summary>
Motivation: Loss of plasticity has been studied mostly in older, small architectures and rarely in natural-language domains. The authors want to determine whether it remains a problem in modern transformer-based LLMs.

Method: GPT-style Transformer models (5M-314M non-embedding parameters) are trained on a multilingual continual learning problem. Plasticity loss is measured by deterioration on a held-out Vietnamese probing task.

Result: Evidence of plasticity loss is found across all model sizes. The onset follows a predictable scaling law growing sublinearly with model size. Plasticity loss also occurs under stationary multilingual training.

Conclusion: Larger models may delay but not prevent plasticity loss. Increasing parameter count alone is insufficient. Even large Transformer LMs trained on natural language will eventually lose the ability to efficiently adapt to new data.
...
### [273] [BluTrain: A C++/CUDA Framework for AI Systems](https://arxiv.org/abs/2606.24780)
...
TL;DR: BluTrain is a lightweight, architecture-general deep learning training framework built from scratch in C++ and CUDA that outperforms PyTorch in throughput (407K vs 395K tokens/s) and memory efficiency (22% reduction) on GPT-2 124M training, while preserving numerical fidelity.


<details>
  <summary>Details</summary>
Motivation: Deep learning progress at scale depends more on systems engineering than modeling, but existing frameworks lack full hardware-level control and introduce repetitive orchestration complexity. BluTrain was designed to eliminate this gap.

Method: BluTrain is implemented natively in standard C++ and core CUDA, featuring a typed tensor module with reverse-mode autograd, linear algebra library, caching allocator, multi-mode distributed execution, and an MLIR-based deep learning compiler—all from first principles.

Result: On 8-GPU 6000 Ada with GPT-2 124M (FP32), BluTrain achieves 407K tokens/s throughput (vs PyTorch's 395K), up to 22% memory reduction, and marginally lower final validation loss with exact numerical fidelity.

Conclusion: By exposing every layer to native tuning, BluTrain sets its own performance ceiling, demonstrating that a systems-first approach can surpass industry-standard frameworks in both efficiency and flexibility.
...
### [274] [Assessing Distribution Shift in Human Activity Recognition for Domain Generalization](https://arxiv.org/abs/2606.24781)
*Rebecca Adaimi,Edison Thomaz*

Main category: cs.AI

TL;DR: This paper systematically evaluates four types of distribution shifts (device type, sensor placement, sampling rate, user behavior) in Human Activity Recognition (HAR), finding that diversity shifts dominate and current domain generalization methods barely outperform simple baselines. It introduces a uniform HAR benchmark with 28 domain generalization methods and open-source resources.


<details>
  <summary>Details</summary>
Motivation: While data diversity in HAR is well-acknowledged, there is a gap in understanding the effect of various distribution shifts on HAR models and the domain generalization problem that arises. The authors aim to fill this gap.

Method: The paper systematically evaluates 4 types of distribution shifts (device type, sensor placement, sampling rate, user behavior), quantifies their effects, introduces a uniform HAR-based distribution shift benchmark, and conducts a comprehensive evaluation of up to 28 domain generalization methods.

Result: Diversity shifts predominantly define all types of shifts, indicating unique features not shared across domains. Current domain generalization algorithms marginally outperform the empirical risk minimization baseline, exposing limitations in achieving model generalizability.

Conclusion: This is the first systematic exploration of domain generalization and adaptation concerning specific distribution shifts in sensor-based HAR, offering an open-source benchmark platform and datasets to spur further research.
...
### [275] [Solving Inverse Problems of Chaotic Systems with Bidirectional Conditional Flow Matching](https://arxiv.org/abs/2606.24824)
*Peiyan Hu,Jian Zhang,Jiashu Pan,Ruiqi Feng,Tao Zhang,Zhi-Ming Ma,Yuan-Sen Ting,Gongjie Li,Tailin Wu*

Main category: cs.AI

TL;DR: Bi-CFM (and its conservation-constrained variant CBi-CFM) solves the inverse problem of inferring initial states from final states in chaotic systems using bidirectional conditional flow matching, achieving superior accuracy, speed (>100x faster), and conservation law adherence across Lorenz, Circuit, Lorenz 96, three-body, and real globular cluster systems.


<details>
  <summary>Details</summary>
Motivation: Inverse problems in chaotic dynamics (inferring initial conditions from final states) are largely unsolved due to ill-posedness, non-uniqueness, instability, and chaotic time-reverse dynamics, despite the importance of modeling chaotic systems.

Method: Bidirectional Conditional Flow Matching (Bi-CFM) learns bidirectional mappings between distributions of initial and final states to capture chaotic evolution stochasticity and mitigate exponential error accumulation. For systems with conservation laws, Conservation-constrained Bi-CFM (CBi-CFM) is introduced.

Result: Bi-CFM outperforms baselines on five distribution-level metrics across Lorenz, Circuit, and Lorenz 96 systems, with >100x speedup. CBi-CFM achieves conservation errors comparable to ground truth in three-body planet-planet scattering. On real globular cluster observations (10 Gyr evolution), the method advances accuracy.

Conclusion: The method establishes a scalable route to solving inverse problems of long-timescale real-world chaotic dynamics, representing a significant advance for chaotic systems modeling.
...
### [276] [Difference-Making without Making a Difference](https://arxiv.org/abs/2606.24832)
*Sander Beckers*

Main category: cs.AI

TL;DR: This paper demonstrates that Andreas & Günther's seven definitions of actual causation, which they classified into three distinct types, actually collapse into each other, with their newest factual difference-making definition instantiating all three types. This undermines all seven accounts when tested on crucial examples.


<details>
  <summary>Details</summary>
Motivation: To show that the three types of causal accounts (factual difference-making, counterfactual difference-making, and regularity-based) that Andreas & Günther claim are competing and distinct are actually "distinctions without a difference."

Method: The author demonstrates that Andreas & Günther's most recent factual difference-making definition instantiates all three types of accounts, then compares this novel account to the other six accounts on several crucial examples to reveal inconsistencies.

Result: The factual difference-making definition contains elements of all three types, proving they are not genuinely distinct. When tested on crucial examples, this undermines all seven of Andreas & Günther's accounts.

Conclusion: The claimed distinctions between the three types of causal accounts are illusory, and the entire framework of seven definitions proposed by Andreas & Günther is fundamentally flawed.
...
### [277] [Accuracy and Satisfaction in Multi-Turn LLM Dialogues for NFR Assessment](https://arxiv.org/abs/2606.24834)
*Ali Pourghasemi Fatideh,Wilder Baldwin,Maria Dhakal,Collin McMillan,Sepideh Ghanavati*

Main category: cs.AI

TL;DR: This study evaluates multi-turn conversations between developers and GitHub Copilot for assessing HIPAA Non-Functional Requirements, finding that developers tend to agree with LLM assessments despite low accuracy against expert ground truth, and that longer responses reduce user satisfaction while proactive interactions improve it.


<details>
  <summary>Details</summary>
Motivation: Current evaluation benchmarks for LLM-based dialogue assistants focus solely on functional correctness, ignoring the critical gap in assessing quality and accuracy when handling Non-Functional Requirements (NFRs), which are vague, context-dependent, and involve multiple program parts.

Method: 49 programmers interacted with GitHub Copilot to assess 148 HIPAA-derived NFRs against the iTrust codebase across three dimensions: requirement satisfaction level, reasoning, and code localization.

Result: Developers tend to agree with LLM assessments, but accuracy against expert ground truth is low. Longer system responses and more information-providing turns negatively affect satisfaction, while proactive interactions positively affect it.

Conclusion: The findings provide insights for designing LLM-based dialogue systems that better support NFR assessment, highlighting the need to balance response length and proactive engagement.
...
### [278] [Grading the Grader: Lessons from Evaluating an Agentic Data Analysis System](https://arxiv.org/abs/2606.24839)
*Tian Zheng,Kai-Tai Hsu*

Main category: cs.AI

TL;DR: This paper develops and evaluates a three-layer human-AI grading cascade for agentic data analysis systems, achieving 100% precision and 97% recall on 153 QRData tasks.


<details>
  <summary>Details</summary>
Motivation: Agentic data analysis systems produce complex, multi-modal outputs (code, numbers, text), making evaluation challenging. There's a need to distinguish genuine errors from grading artifacts.

Method: A three-layer grading cascade: strict regex matching, LLM-based lenient grading, and snippet-based human inspection. Uses LAMBDA system on 153 QRData tasks from DSGym. Includes keyword-anchored extraction and iterative nudge mechanism.

Result: Both automated graders achieve 100% observed precision. Lenient grader achieves 97% recall. Keyword-anchored extraction improves strict grader's recall by 60 percentage points. Nudge raises grading run success from 36% to 97%.

Conclusion: The three-layer cascade effectively evaluates agentic outputs; nudge mechanisms work as answer template cues (not requiring question re-injection); variable type is the most influential task metadata for grading dynamics.
...
### [279] [Matching Tasks to Objectives: Fine-Tuning and Prompt-Tuning Strategies for Encoder-Decoder Pre-trained Language Models](https://arxiv.org/abs/2606.24841)
*Ahmad Pouramini,Hesham Faili*

Main category: cs.AI

TL;DR: This paper introduces the MTO (Match Task to Objective) framework that aligns pre-training objectives with downstream tasks to significantly boost encoder-decoder PLM performance, achieving over 120% improvement in few-shot settings and outperforming baselines even in full-dataset scenarios.


<details>
  <summary>Details</summary>
Motivation: To explore how diverse pre-training objectives affect encoder-decoder PLMs across generation and QA tasks, and to provide systematic guidance for selecting and aligning objectives to maximize task performance.

Method: Proposes MTO framework with: (1) automated methods to identify the appropriate objective for a given task, (2) unsupervised task-specific data preparation, (3) novel templates aligned with pre-training/adaptation objectives, and (4) extension to prompt-tuning for soft prompt optimization.

Result: Over 120% performance gain in few-shot settings compared to conventional methods; significantly outperforms related works in few-shot and exceeds baselines in full-dataset scenarios; also enhances prompt-tuning performance.

Conclusion: Aligning pre-training objectives with downstream tasks through the MTO framework provides substantial improvements, offering precise guidance for model selection and optimization tailored to specific tasks.
...
### [280] [World Models in Pieces: Structural Certification for General Agents](https://arxiv.org/abs/2606.24842)
*Yikai Lu,Yifei Wu,Xinyu Lu,Tongxin Li*

Main category: cs.AI

TL;DR: This paper proves that general agents cannot be universally capable, and introduces a structural certification framework that provides localized, transition-level guarantees on an agent's world model, achieving tight error bounds for reliable long-horizon planning.


<details>
  <summary>Details</summary>
Motivation: Standard worst-case analysis fails to distinguish between critical bottlenecks and irrelevant failures in the big-world regime where agents must specialize across a fragmented world model.

Method: The authors first formalize the limitation by proving general agents are not universal. They then introduce structural certification, a transition-local framework with algorithms that filter transitions using deep compositional goals, proving a 𝒪(1/n) + 𝒪(δ) error bound on the agent's structural world model.

Result: The proposed framework achieves a tight 𝒪(1/n) + 𝒪(δ) error bound in the small-δ regime, with existence explicitly guaranteed by the certification. This enables certifiable deployment by localizing reliable transitions for long-horizon planning.

Conclusion: Structural certification provides a principled way to deploy general agents with certified reliability by focusing on localized transition guarantees rather than uninformative uniform bounds.
...
### [281] [OpenThoughts-Agent: Data Recipes for Agentic Models](https://arxiv.org/abs/2606.24855)
...
TL;DR: OpenThoughts-Agent (OT-Agent) introduces a fully open data curation pipeline for training agentic language models, with 100+ ablation experiments showing the importance of task diversity. Their 100K-example training set improves Qwen3-32B to 44.8% average accuracy across 7 benchmarks (+3.9% over prior best), with strong scaling properties. All resources are publicly released.


<details>
  <summary>Details</summary>
Motivation: Existing open agentic training datasets (SWE-Smith, SERA, Nemotron-Terminal) target single benchmarks, lacking generalization across diverse agentic tasks. No publicly known systematic method exists for curating training data for broadly capable agentic models.

Method: (1) A fully open data curation pipeline with systematic investigation via 100+ controlled ablation experiments. (2) Assembly of a 100K-example training set. (3) Fine-tuning Qwen3-32B on this dataset. (4) Compute-controlled comparisons against alternative open datasets.

Result: OT-Agent achieves 44.8% average accuracy across seven agentic benchmarks, outperforming Nemotron-Terminal-32B (40.9%) by 3.9 percentage points. The training data shows strong scaling properties, outperforming alternative open datasets at every training set size in compute-controlled comparisons.

Conclusion: Open data curation pipelines with diverse task sources are critical for training generalizable agentic models. The OT-Agent pipeline, datasets, experimental data, and models are publicly released to support future open research.
...
### [282] [FEM-Based Dispersion and Mode Analysis of Rectangular, Circular, and Ridge Waveguide Geometries](https://arxiv.org/abs/2606.23703)
*Sabrina Saima*

Main category: eess.SP

TL;DR: This paper develops a 2D FEM solver for modal analysis of hollow metallic waveguides, validated on empty guides and applied to ridged waveguides, showing accurate cutoff frequencies and field distributions.


<details>
  <summary>Details</summary>
Motivation: To provide a robust numerical tool for analyzing complex waveguide geometries where analytical solutions are unavailable, particularly for ridged waveguides.

Method: 2D FEM solver solving scalar Helmholtz eigenvalue formulations from source-free Maxwell equations over the waveguide cross-section, with PEC boundary conditions for TE and TM modes.

Result: High accuracy in cutoff wavenumbers, dispersion curves, and field maps for empty rectangular and circular waveguides; ridged waveguides show redistributed modal fields and significantly lowered dominant-mode cutoff frequency.

Conclusion: The generalized eigenvalue FEM formulation is a robust and adaptable tool for analyzing complex waveguide geometries where exact analytical solutions are unavailable.
...
### [283] [Zero-Shot Neural Priors for Generalizable Cross-Subject and Cross-Task EEG Decoding](https://arxiv.org/abs/2606.23706)
*Baimam Boukar Jean Jacques,Brandone Fonya,Nchofon Tagha Ghogomu,Pauline Nyaboe,Kipngeno Koech*

Main category: eess.SP

TL;DR: The paper proposes a zero-shot cross-subject EEG decoding framework using a Transformer with progressive unfreezing, achieving improved generalization (nRMSE 0.9799 vs 0.9991 baseline) on the large-scale Healthy Brain Network dataset for computational psychiatry.


<details>
  <summary>Details</summary>
Motivation: Existing EEG decoding models suffer from poor cross-subject and cross-task generalization due to high inter-subject variability and non-stationary neural signals, limiting their practical use in BCI and mental health applications.

Method: A zero-shot cross-subject decoding framework is built on the large-scale Healthy Brain Network dataset, comparing CNN baseline, hybrid LSTM, and Transformer. A novel progressive unfreezing strategy is proposed to adapt the Transformer for regression while preventing catastrophic forgetting.

Result: The baseline CNN achieved nRMSE of 0.9991, while the fine-tuned Transformer with progressive unfreezing achieved 0.9799 on unseen subjects, demonstrating improved cross-subject generalization.

Conclusion: The proposed approach advances scalable, calibration-free EEG decoding, showing promise for computational psychiatry and behavioral prediction applications.
...
### [284] [Coordinate-Queryable Neural Field Reconstruction for EEG Spatial Super-Resolution with Unseen-Electrode Generation](https://arxiv.org/abs/2606.23707)
*Hongjun Liu,Leyu Zhou,Zijianghao Yang,Chao Yao*

Main category: eess.SP

TL;DR: This paper proposes a conditional implicit neural representation framework for EEG spatial super-resolution that handles variable missing channels by learning a shared conditional scalp field from partially observed support channels, achieving robust performance across diverse missing-channel scenarios.


<details>
  <summary>Details</summary>
Motivation: Real-world EEG deployments suffer from random channel missingness, unstable electrode quality, and changing visible-channel patterns, while existing EEGSR methods learn fixed channel mappings that fail under varying missing-channel conditions during inference.

Method: A position-guided encoder summarizes observed EEG channels and their coordinates into a latent condition, then a conditional implicit neural representation decoder reconstructs target EEG signals by querying this condition at desired electrode coordinates. A fidelity-preserving channel corruption training strategy under mixed electrode states is introduced to strengthen latent representation constraints.

Result: On the strict held-out-electrode setting with AAD, the method reduces NMSE by 37.5% and improves SNR by 2.12 dB over the strongest baseline. Extensive experiments across multiple EEG datasets demonstrate effectiveness for both random missing-channel reconstruction and unseen-electrode signal generation.

Conclusion: The proposed framework successfully reformulates EEGSR as learning a shared conditional scalp field, enabling robust signal reconstruction under diverse and unpredictable channel missingness patterns, including electrode positions never seen during training.
...
### [285] [RadioRange: An Open-Source Digital Twin-based Ranging Simulator for UWB, Wi-Fi, and 5G](https://arxiv.org/abs/2606.23708)
*Zhen Lyu,Yidi Chen,Li-Ta Hsu,Guohao Zhang*

Main category: eess.SP

TL;DR: RadioRange is an open-source digital twin for RF-based ranging that unifies UWB, Wi-Fi, and 5G NR on ray-traced channels, models 11 hardware impairments, provides ranging detectors and multipath algorithms, and is validated against real-world measurements.


<details>
  <summary>Details</summary>
Motivation: No open platform exists for fair, reproducible comparison of RF-based ranging protocols under realistic hardware impairments; existing simulators focus on communication-layer metrics and lack ranging algorithms and positioning-specific evaluation.

Method: Builds a positioning-first digital twin unifying UWB, Wi-Fi, and 5G NR on identical ray-traced physical channels. Models 11 toggleable hardware impairments across three injection stages (antenna-level offsets, RF circuit non-idealities, post-compensation CSI residuals). Provides five first-path ranging detectors and three multipath identification algorithms within a protocol-specific evaluation framework.

Result: Validated against real-world UWB and Wi-Fi measurements; the channel model captures geometry-dependent multipath bias. Enables controlled Monte Carlo benchmarking and systematic ablation studies.

Conclusion: RadioRange fills the gap for reproducible, fair ranging evaluation across protocols, and is publicly released to the research community.
...
### [286] [WiFi-Based People Counting Using Beam-Steerable Antennas: A Test-bed Study](https://arxiv.org/abs/2606.23710)
*Riccardo Bersan,Anay Ajit Deshpande,Sanaz Kianoush,Daniele Piazza,Stefano Savazzi*

Main category: eess.SP

TL;DR: This paper discusses using RF signals (especially Wi-Fi) for ubiquitous sensing, highlighting CSI extraction, OFDM subcarriers, MIMO systems, and the potential of IEEE 802.11be (Wi-Fi 7) for tracking human movements and re-identification.


<details>
  <summary>Details</summary>
Motivation: To leverage ubiquitous RF signals (Wi-Fi/cellular) for personalized sensing services like smart living, remote healthcare, automated logistics, and gesture interaction, without requiring dedicated hardware.

Method: Utilizes CSI extraction from Wi-Fi signals (phase/amplitude across subcarriers), MIMO antenna arrangements, OFDM subcarriers, and the enhanced 160-320MHz bandwidth of IEEE 802.11be (Wi-Fi 7) for radar-like sensing and human behavior analysis.

Result: Wi-Fi signals can track daily human movements/behaviors and enable person re-identification based on signal variation patterns across individuals.

Conclusion: Future Wi-Fi standards (Wi-Fi 7) with wider bandwidth and dedicated sensing features offer a promising platform for ubiquitous RF-based perception, supporting applications from gesture recognition to re-identification.
...
### [287] [Optical Ground Stations for Space Communications:Systems Engineering, Availability, and Service Economics Through 2030](https://arxiv.org/abs/2606.23711)
*Slava G. Turyshev*

Main category: eess.SP

TL;DR: This paper analyzes the optical ground station (OGS) sector as of June 2026, finding that the bottleneck has shifted from peak line rate to repeatable service under weather, acquisition, scheduling, and operations constraints. Under a normalized planning scenario, the fixed-cost component is approximately $3,000-$4,000 USD/TB, and the defensible 2030 baseline is hybrid optical+radio-frequency.


<details>
  <summary>Details</summary>
Motivation: To assess the systems-engineering and service-economics of the OGS sector, moving beyond raw optical line rates to consider service availability and utilization as key adoption factors.

Method: Combines public flight demonstrations and operational records with scalar link-budget, availability, and cost-normalization models to evaluate the OGS sector.

Result: The bottleneck has shifted from peak line rate to repeatable service. Under a 10 Gbps near-Earth station scenario with $2M/year cost and 0.5h/day scheduled contact time, the fixed-cost component is ~$3,000-$4,000 USD/TB.

Conclusion: The defensible 2030 baseline is hybrid optical+RF: optical for throughput and spectrum relief; RF for continuity, contingency, and assured command paths. The sector is best viewed as a stratified value chain, not a single vendor ranking.
...
### [288] [Audio-visual Contrastive Alignment for Diffusion-based Visual-conditioned Speech Enhancement](https://arxiv.org/abs/2606.23712)
*Colombe Mboungou,Mostafa Sadeghi,Jean-Eudes Ayilo,Romain Serizel*

Main category: eess.SP

TL;DR: This paper improves unsupervised audio-visual speech enhancement by adding a contrastive audio-visual loss to the diffusion model training, which enhances cross-modal alignment and leads to better performance, especially in low SNR conditions.


<details>
  <summary>Details</summary>
Motivation: The impact of explicitly enforcing cross-modal alignment in the fusion of audio and visual features for diffusion-based AVSE remains unclear, motivating the authors to investigate and propose a method to encourage stronger use of visual information.

Method: The authors augment the diffusion training objective with a contrastive audio-visual loss to encourage better cross-modal alignment, while keeping the posterior sampling framework unchanged.

Result: Experiments on matched and mismatched test data show consistent improvements in interference suppression, signal reconstruction, and perceptual quality, with the largest gains at low SNRs.

Conclusion: Adding contrastive audio-visual loss to diffusion-based AVSE improves cross-modal alignment and enhances performance, particularly in challenging low-SNR environments.
...
### [289] [Unifying Adaptive Fourier and Möbius-Based Models for Efficient and Interpretable Biomedical Signal Decomposition](https://arxiv.org/abs/2606.23713)
*Christian Canedo,Rocío Carratalá-Sáez,Cristina Rueda*

Main category: eess.SP

TL;DR: This paper proves mathematical equivalence between Adaptive Fourier Decomposition (AFD) and Frequency-Modulated Mobius (FMM) model for biomedical signal analysis, showing they solve the same optimization problem, and demonstrates practical implications for EEG and ECG signals.


<details>
  <summary>Details</summary>
Motivation: To bridge two independently developed finite-order decomposition frameworks (AFD and FMM) for oscillatory biomedical signals (ECG, EEG), enabling combined computational efficiency and interpretability.

Method: Formal proof of mathematical equivalence between AFD (orthonormal Takenaka-Malmquist expansions) and FMM (Mobius-transform-based parametric decomposition), under mild regularity assumptions and independent Gaussian noise, extended to multi-channel signals.

Result: Established equivalence shows AFD and FMM solve the same underlying optimization problem; demonstrated via EEG (approximation behavior over component count) and ECG (five-component comparisons of unlabeled AFD vs. physiologically identified FMM components).

Conclusion: The equivalence provides a principled basis to combine AFD's computational advantages (including FFT-based implementations) with FMM's interpretability for biomedical signal analysis.
...
### [290] [Physics-Informed Path-Parametric Learning for Efficient and Lightweight CSI Feedback](https://arxiv.org/abs/2606.23730)
*Chunyu Ling,Jiajia Guo,Yiming Cui,Chao-Kai Wen,Shi Jin,Shuangfeng Han,Xiaoyun Wang*

Main category: eess.SP

TL;DR: HS-PINNnet is a physics-informed neural network for CSI feedback that reformulates high-dimensional CSI reconstruction as low-dimensional multipath parameter estimation, achieving 92.8% FLOPs reduction and orders of magnitude lower latency than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for CSI feedback treat data as visual images in a "black-box" manner, lacking interpretability and producing CSI inconsistent with multipath propagation principles. The paper aims to address these limitations by integrating physical channel models into the network.

Method: HS-PINNnet uses a hierarchical sensing encoder to produce compact multipath representations, a heterogeneous decoder with dedicated branches for parameter-specific CSI reconstruction (amplitude, angle, etc.), and a PCD module to adaptively estimate dominant paths. It also employs subchannel-wise shared encoding and parallel decoding to decompose high-dimensional processing.

Result: Outperforms state-of-the-art under various configurations, achieves 92.8% reduction in FLOPs, and exhibits two orders of magnitude lower FPGA simulation latency.

Conclusion: The physics-informed approach effectively bridges channel modeling and deep learning for CSI feedback, offering high efficiency, interpretability, and scalability for future XL-MIMO systems.
...
### [291] [Integrated Sensing and Communications for Real-time Avatar Control in XR over 5G](https://arxiv.org/abs/2606.23771)
*Nabeel Nisar Bhat,Javad Sameri,Rreze Halili,Rafael Berkvens,Maria Torres Vega,Jeroen Famaey*

Main category: eess.SP

TL;DR: This paper proposes a multimodal sensing architecture for XR that combines 5G mmWave ISAC for coarse body-level gesture recognition and sEMG sensors for fine-grained finger-level gestures, enabling multi-scale gesture recognition without handheld controllers or cameras.


<details>
  <summary>Details</summary>
Motivation: Current XR interaction solutions rely on handheld controllers and cameras that cannot capture full-body poses, inhibit free hand use, and require clear line-of-sight. There is a need for a seamless, controller-free multimodal sensing approach for immersive XR experiences.

Method: The architecture combines two modalities: (1) 5G mmWave ISAC signals using PPBP from standard 5G NR beam management for coarse body-level gestures, and (2) lightweight sEMG sensors on forearm for fine-grained finger-level gestures.

Result: PPBP-based body-level sensing achieves 82.2±5.9% accuracy on unseen users. sEMG shows strong discriminative information for fine-grained finger interactions across different movement settings.

Conclusion: Combining 5G mmWave ISAC and sEMG enables multi-scale gesture recognition (body-level via 5G signals, finger-level via sEMG), forming a complete controller-free XR interaction framework.
...
### [292] [Generative Modeling for Physiological Signals](https://arxiv.org/abs/2606.23864)
*Xinqi Bao,Ernest Kamavuako,Saikat Chatterjee*

Main category: eess.SP

TL;DR: This review comprehensively summarizes generative modeling approaches (GANs, VAEs, diffusion models, autoregressive models, hybrids) applied to physiological signals (cardiovascular, neural, muscular, peripheral, specialized), addressing challenges like limited labeled data, class imbalance, noise, and privacy. It also proposes a hierarchical evaluation framework covering signal-level similarity, dataset distribution, physiological validity, task utility, and generalization/robustness.


<details>
  <summary>Details</summary>
Motivation: Physiological signal applications face significant barriers including limited labeled data, class imbalance, noisy/incomplete recordings, heterogeneous acquisition settings, and privacy restrictions. Generative modeling offers potential solutions to these challenges, motivating the need for a structured review of existing approaches.

Method: The paper conducts a literature review organized by: (1) signal types (cardiovascular, neural, muscular, peripheral, specialized), (2) model families (GANs, AEs/VAEs, diffusion models, autoregressive models, hybrids), and (3) evaluation practices structured into a hierarchical framework with five levels (signal similarity, dataset distribution, physiological validity, task-oriented utility, generalization/robustness).

Result: Identifies that generative models are used for data augmentation, signal restoration, modality translation, and conditional waveform synthesis across physiological signal domains. Proposes a hierarchical evaluation framework to standardize assessment practices in this field.

Conclusion: By linking signal-specific constraints, generative roles, model families, and evaluation evidence, this review provides structured guidance for future use and evaluation of generative models in physiological-signal research.
...
### [293] [Unlocking Realism and Interpretability in Wireless Channel Synthesis: A Physics-Guided Generative Approach](https://arxiv.org/abs/2606.23868)
*Satyavrat Wagle,Akshay Malhotra,Shahab Hamidi-Rad,Aditya Sant,David J. Love,Christopher G. Brinton*

Main category: eess.SP

TL;DR: This paper proposes integrating a physics-based geometric channel model with generative methods to produce realistic, physically-viable wireless channel data. It addresses three key issues: outputs may not correspond to physically valid channels, lack environmental insights, and need for labeled data. The authors introduce a linearized reformulation of the PPGC model with tensor decomposition for smooth training and parameter flexibility.


<details>
  <summary>Details</summary>
Motivation: ML methods for wireless communication need large amounts of realistic channel data, but OTA sampling is resource-intensive and cannot fully represent real-world channels. Existing generative models produce outputs that may not be physically viable, lack environmental insights, and often require labeled data for training.

Method: (1) Integration of parametric physics-based geometric channel (PPGC) modeling with generative methods. (2) Linearized reformulation of the PPGC model to overcome non-convex optimization issues. (3) Tensor decomposition framework for flexible channel parameters. (4) Compatibility demonstration with parameter extraction tasks.

Result: The model is evaluated against prior baselines by comparing generated scenario-specific samples to true channels in terms of similarity and utility in downstream compression tasks.

Conclusion: The proposed approach produces physically viable, interpretable channel data with smooth training dynamics and flexible parameter handling, offering advantages over prior generative methods for wireless channel synthesis.
...
### [294] [A Kalman Filter-Based Tracking Loop Design for Real-Time Aerospace GNSS Applications with Minimum Pull-Out Probability](https://arxiv.org/abs/2606.23925)
*J. Fermín Llorente,Javier A. Smidt,Pedro A. Roncagliolo,Ramón López La Valle*

Main category: eess.SP

TL;DR: The paper introduces a Modified Kalman filter (mKF) for GNSS tracking that accounts for processing delays in hardware-based architectures, achieving better performance than traditional KF and one-delay DPLL under high dynamics.


<details>
  <summary>Details</summary>
Motivation: Existing KF-based tracking loops neglect processing delays inherent in real-time systems with hardware correlators, which compromises their theoretical optimality in practical implementations.

Method: The authors reformulate the measurement update to be consistent with processing delays, propose a systematic tuning method for process noise covariance and correlation time based on pull-out probability (POP), and establish a direct equivalence between the mKF and a one-delay DPLL in the digital domain.

Result: At equal noise bandwidth, the mKF matches DPLL's phase error variance while achieving lower error in higher-order states. The mKF sustains lock at bandwidths inaccessible to the optimal one-delay DPLL under the same dynamic stress.

Conclusion: The mKF is a robust and noise-efficient solution for high-dynamic aerospace GNSS applications, validated via Monte Carlo simulation, post-processing, and real-time GPS receiver testing.
...
### [295] [Low-rank Updates in Slowly Time-varying Graphs for Spatial-Temporal Signal Interpolation](https://arxiv.org/abs/2606.24011)
*Saghar Bagheri,Gene Cheung,Tim Eadie,Antonio Ortega*

Main category: eess.SP

TL;DR: This paper proposes a joint optimization framework for interpolating signals and estimating time-varying graph adjacency matrices by modeling the change between consecutive graphs as low-rank, solved via alternating optimization with OMP-based proximal mapping and unrolled into a lightweight neural network.


<details>
  <summary>Details</summary>
Motivation: Static spatial graphs are insufficient for spatial-temporal data where node-to-node similarities evolve over time. The authors aim to model slowly time-varying pairwise relationships in graphs.

Method: The change between two consecutive adjacency matrices is modeled as a low-rank matrix. Alternating optimization is used: fixing W(2) to interpolate x2 via linear system, and fixing x2 to update W(2) via proximal gradient descent, where the proximal mapping of the rank term is approximated using fast OMP with a dictionary from W(1)'s eigenvectors. Iterations are unrolled into a lightweight neural network.

Result: The joint optimization achieves better signal interpolation compared to existing time-varying graph models.

Conclusion: The proposed approach effectively combines graph signal smoothness and low-rank priors for time-varying graphs, and the unrolled network enables limited data-driven parameter tuning.
...
### [296] [A Conditional Timing Protection Level: Holdover-Limited Undetected Time Error Under GNSS Spoofing](https://arxiv.org/abs/2606.24210)
*Chakshu Baweja*

Main category: eess.SP

TL;DR: This paper addresses GNSS timing receiver vulnerabilities under spoofing attacks. Through field measurements, it shows a 20,000x gap between actual time error (~1.01 ms) and reported error (51 ns). It proves no finite unconditional bound exists for undetected time error under a single self-referential clock monitor. It proposes a Timing Protection Level (TPL) — a conditional bound combining a model-free monitor's detectability floor and oscillator coast — that provides protection orders of magnitude better than clock-aided sequential tests alone.


<details>
  <summary>Details</summary>
Motivation: GNSS timing receivers under spoofing lack nominal-geometry faults that position-domain RAIM can bound. The threat is a slow, common-mode clock time pull that the receiver's own time-accuracy flag may not reveal. The authors aim to characterize this vulnerability and propose a method to bound undetected time errors.

Method: The paper makes three contributions: (1) Field measurement using raw L1 pseudoranges and broadcast ephemeris from a recorded over-the-air spoofing attack (JammerTest 2024 campaign) on a u-blox ZED-F9P; (2) Impossibility proof showing no finite unconditional bound exists under a single self-referential clock-aided monitor; (3) Proposal of the Timing Protection Level (TPL), a conditional bound combining a model-free monitor's static detectability floor plus oscillator coast over detection latency, verified in the open Kshana simulator.

Result: The recorded spoofing attack pulled the receiver's served time by ~1.01 ms while it reported at most 51 ns — a ~20,000x gap. The TPL provides 114 ns at one-second recovery and 458 ns at 60-second coast, thousands of times below the 1.01 ms attack. A clock-aided sequential test alone alarms only near ~1 ms capture, while the model-free monitor alarms during the ramp.

Conclusion: The bound is calibrated, not field-validated; carries no integrity-risk budget; and is reported as a band at long coast. The simulator, bound, and calibration example are open source under AGPL-3.0. The work explicitly acknowledges limitations while providing practical protection against slow-ramp spoofing attacks.
...
### [297] [PROTECT-90: A Fault Dataset for Power System Protection](https://arxiv.org/abs/2606.24298)
*Julian Oelhaf,Georg Kordowich,Christian Bergler,Andreas Maier,Johann Jäger,Siming Bayer*

Main category: eess.SP

TL;DR: The PROTECT-90 dataset provides 9,022 open EMT-simulated fault waveforms on a standardized 90 kV double-line topology with systematic documentation, enabling reproducible benchmarking for power system protection methods.


<details>
  <summary>Details</summary>
Motivation: Current data-driven approaches for power system protection lack standardized, publicly available high-voltage waveform datasets for transparent and reproducible evaluation.

Method: Creates an open EMT-simulated reference benchmark (PROTECT-90) with 9,022 physically consistent short-circuit episodes on a 90 kV double-line topology, using systematic domain randomization of operating points, line parameters, and fault conditions, with synchronized three-phase measurements at eight locations.

Result: A publicly available dataset of 9,022 fault episodes with structured metadata, documented modeling assumptions, and balanced scenario coverage for reproducible benchmarking.

Conclusion: PROTECT-90 establishes a standardized, open foundation for reproducible comparison of protection-oriented signal processing and learning-based methods in power systems.
...
### [298] [Explainable AI for Next-Generation Wireless Physical Layer: Basics, State-of-the-Art, and Open Challenges](https://arxiv.org/abs/2606.24424)
*Bingnan Xiao,Shuyan Hu,Xiaojing Chen,Zhiyuan Zhai,Bingcong Li,Wei Ni,Xin Wang,Ekram Hossain*

Main category: eess.SP

TL;DR: This survey explores explainable AI (XAI) for next-generation wireless PHY layers, proposing a responsibility-oriented goal framework, a taxonomy of explainability methods, and practical deployment criteria, while identifying key challenges such as explainability-performance tradeoffs, cross-layer consistency, and the need for XAI in LLM- and agentic-AI-driven systems.


<details>
  <summary>Details</summary>
Motivation: As wireless systems become "AI-native" with NNs embedded throughout the PHY layer, deep learning models' opacity raises concerns about reliability, safety, and privacy in complex, time-varying environments, motivating the need for explainable AI to ensure responsible deployment.

Method: The paper formalizes responsibility-oriented goals for wireless XAI, develops a systematic taxonomy of explainability approaches, distills practical deployment criteria, and provides a comprehensive review connecting learning paradigms to explanation techniques, evaluation metrics, and deployment considerations across the PHY layer.

Result: A structured taxonomy and mapping of XAI techniques to PHY-layer communication scenarios, along with practical criteria for deploying explanations in wireless systems.

Conclusion: Open challenges remain, including explainability-performance tradeoffs, explainability-aware data processing, customized XAI for communication-specific structures, cross-layer explanation consistency, and emerging needs for explaining LLM- and Agentic-AI-driven PHY layers.
...
### [299] [WiWorld-RealData: A Real-World Multi-Modal Dataset for 6G Wireless World Models](https://arxiv.org/abs/2606.24476)
*Yinyin Jiao,Huixin Xu,Jianhua Zhang,Yuelong Qiu,Jingjing Wang,Shaoyi Liu,Yuxiang Zhang,Li Yu,Xuebin Sun,Guangyi Liu*

Main category: eess.SP

TL;DR: WiWorld-RealData is a large-scale, real-world outdoor multi-band channel and multi-modal sensing dataset (3.7 GHz & 6.775 GHz) with aligned CIRs, images, LiDAR, mmWave radar, and GNSS data for 6G digital twin channel research, achieving 2.02 dB MAE in environment-assisted path-loss prediction.


<details>
  <summary>Details</summary>
Motivation: To enable wireless world models that jointly understand physical environments and channel responses, there is a need for real-world datasets that capture both measured wireless responses and corresponding environment states under realistic propagation conditions.

Method: The dataset was collected along campus mobile routes, providing channel impulse responses at 3.7 GHz and 6.775 GHz, paired with multi-view images, panoramic images, LiDAR point clouds, mmWave radar records, and GNSS trajectories. Unified file organization and metadata manifests establish sample-level correspondences among all modalities.

Result: 10 TB-level multi-modal field data collected; public release provides one representative dual-band route with complete channel-environment alignment. A case study on environment-assisted path-loss prediction achieves MAE of 2.02 dB and RMSE of 2.69 dB.

Conclusion: The dataset demonstrates that aligned environment observations contain predictive information for channel variations. The acquisition framework supports extension to more frequency bands and scenarios, supporting 6G digital twin channel research.
...
### [300] [Adaptive Machine Learning Framework for UAV Trajectory Optimization in O-RAN](https://arxiv.org/abs/2606.24483)
*Chenrui Sun,Swarna Bindu Chetty,Gianluca Fontanesi,Mahnaz Arvaneh,Walid Saad,Hamed Ahmadi*

Main category: eess.SP

TL;DR: This paper proposes a UAV trajectory optimization framework for 6G systems that uses continual transfer learning within O-RAN architecture, leveraging a library of pre-trained models and a selection mechanism to reduce adaptation time. Simulations show 44-56% faster convergence than retraining from scratch.


<details>
  <summary>Details</summary>
Motivation: Deploying UAVs as O-RUs in 6G systems offers scalable network coverage, but optimizing their trajectories in dynamic, unfamiliar environments is challenging due to extensive retraining requirements in each new scenario.

Method: The framework integrates enhanced continual transfer learning within O-RAN architecture, maintaining a library of pre-trained models with a selection mechanism to transfer knowledge from relevant environments. A fallback model with continuous refinements handles cases with no sufficiently similar model. Real-world city maps and ray tracing techniques are used for training.

Result: The model selection-based transfer learning approach reduces convergence time by 44% to 56% compared to retraining from scratch, and up to 40% compared to traditional transfer learning without model selection.

Conclusion: The proposed framework significantly improves UAV trajectory optimization efficiency in 6G O-RAN systems by reducing adaptation time through intelligent knowledge transfer, enabling scalable and adaptive network coverage.
...
### [301] [Degeneracy-Aware Resilient Resource Allocation in Cell-Free Cache-Aided MU-MIMO Networks](https://arxiv.org/abs/2606.24611)
*Sayanti Ghosh,Indrakshi Dey,Nicola Marchetti*

Main category: eess.SP

TL;DR: This paper develops a degeneracy-aware resilient framework for cell-free cache-aided MIMO networks, introducing three attack-aware metrics (DWPR^{att}, FSS^{trust}, D_k^{rob}) and a decentralized consensus-based agent framework (DC-ABM). Five theoretical results are established, and simulations show 1.8×-3× faster convergence than baselines with throughput maintained up to f_max≈0.19.


<details>
  <summary>Details</summary>
Motivation: Cell-free cache-aided MU-MIMO networks improve spectral efficiency but their distributed architecture introduces vulnerabilities to jamming, cache-aware eavesdropping, Byzantine corruption, and pilot-contamination attacks. Existing methods lack a unified framework addressing these diverse attack modes.

Method: The paper develops a degeneracy-aware resilient framework with four vulnerability-mode partitions (subfile, edge node, multicast stream, user) and three structural metrics: DWPR^{att} (path robustness), FSS^{trust} (trust-aware substitution), D_k^{rob} (robust degeneracy index). These are incorporated into a decentralized consensus-based agent model (DC-ABM) using trust-weighted trimmed-mean aggregation and adaptive trust evolution.

Result: Five theoretical results established: (i) tight top-mass concentration lemma, (ii) matching memory-rate-resilience bounds, (iii) robust-degeneracy bound with outage characterization, (iv) secrecy-cache coupling theorem, (v) Byzantine-robust convergence with breakdown threshold f_max. Simulations show 1.8×-3× faster convergence than ADMM, MARL/GNN-based control, and Su-Vaidya consensus, maintaining throughput up to f_max≈0.19.

Conclusion: The proposed framework effectively unifies resilience against multiple attack types in cell-free cache-aided MIMO networks, with strong theoretical guarantees and practical convergence improvements over existing methods.
...
### [302] [Sonus Health: Calibrated Heart-Murmur Detection from Smartphone-Based Veterinary Auscultation](https://arxiv.org/abs/2606.24885)
*Aswin Jose,Roeland P-J E. Decorte,Laurent Locquet*

Main category: eess.SP

TL;DR: Sonus Health is a smartphone-based screening system that detects heart murmurs in dogs and cats from ~30-second auscultation recordings, achieving 95.9% accuracy in high-confidence cases (30% of recordings) while routing uncertain cases to human review.


<details>
  <summary>Details</summary>
Motivation: Heart murmurs are early signs of cardiac disease in pets but are often missed in general-practice exams. Definitive diagnosis requires specialists or echocardiograms, which are costly and have scheduling constraints. There's a need for accessible, low-cost screening tools.

Method: The system analyzes smartphone-recorded auscultation audio (≥30 seconds) using machine learning, returning tiered results. High-confidence cases get automated classification; uncertain cases are routed to veterinary review. Evaluated on 322 veterinary-labelled recordings using cross-validation.

Result: For high-confidence tier (30% of cases): 95.9% accuracy, 94.0% sensitivity, 97.9% specificity. Results stable across cross-validation methods, held-out test split, and multiple random seeds. Also estimates heart rate and heart rate variability.

Conclusion: Sonus Health provides a scalable, accessible screening, triage, and monitoring tool for companion-animal cardiac care, bridging the gap between general practice and specialist cardiac assessment.
...
### [303] [Low-Complexity Hybrid Precoding for Cell-Free Massive MU-MIMO ISAC Systems](https://arxiv.org/abs/2606.23709)
*Jun Zhu,Yin Xu,Aimin Tang,Ruomeng Wang,Dazhi He,Yunfeng Guan,Wenjun Zhang*

Main category: cs.IT

TL;DR: This paper proposes a low-complexity hybrid precoding framework for cell-free massive MU-MIMO ISAC systems with partially-connected architectures, achieving high sum-rate and accurate sensing while reducing computational complexity by 87%.


<details>
  <summary>Details</summary>
Motivation: Centralized coordination among distributed APs in CF massive MU-MIMO ISAC systems incurs substantial fronthaul overhead and computation complexity, motivating the need for a low-complexity solution.

Method: Proposes a hybrid precoding framework with partially-connected AP architectures, converting high-dimensional channel info into low-dimensional effective channel. Uses alternating optimization (AO) with PEB constraints reformulated into convex constraints, and a multi-branch rate-splitting MMSE-THP update algorithm for dynamic user topology.

Result: Achieves high ergodic sum-rate and accurate multi-target sensing while reducing computational complexity by 87.02% compared to conventional baselines.

Conclusion: The proposed hybrid precoding framework effectively balances communication performance and sensing accuracy with significantly lower complexity, making it suitable for practical CF massive MU-MIMO ISAC systems.
...
### [304] [Beyond Mutual Information: Extension Profiles and Shape Functions of Random Variable Pairs](https://arxiv.org/abs/2606.23849)
*Rostislav Matveev,Andrei Romashchenko*

Main category: cs.IT

TL;DR: This paper studies the "extension profile" of jointly distributed random variables (X,Y), defined as all possible triples (H(X|W), H(Y|W), I(X:Y|W)) achievable by adding an auxiliary variable W. The authors introduce a "shape function" via Legendre-Fenchel transform, derive bounds using information-theoretic quantities, and connect results to spectral graph theory for uniform-on-support pairs. Applications to non-Shannon inequalities and Gács-Körner common information are discussed.


<details>
  <summary>Details</summary>
Motivation: To capture structural properties of joint distributions beyond simple entropy and mutual information values, and to understand what constraints extensions by auxiliary variables impose on the information-theoretic characterization of pairs (X,Y).

Method: Defines the extension profile and its boundary shape function via Legendre-Fenchel transform. Derives upper/lower bounds using classical information measures. For uniform-on-support pairs, interprets the support as a biregular bipartite graph and relates bounds to the graph's second-largest eigenvalue.

Result: Establishes general bounds on the shape function. Shows that for uniform-on-support pairs, the shape function is bounded by spectral properties of the associated bipartite graph—specifically, a small second eigenvalue restricts possible extensions.

Conclusion: Provides new connections among non-Shannon-type inequalities, Gács-Körner common information, and spectral graph theory, offering a fresh perspective on the information-theoretic structure of joint distributions.
...
### [305] [How Many RF Chains Does a Microwave Linear Analog Computer (MiLAC) Need to Match the Fully-Digital Cramér-Rao Bound?](https://arxiv.org/abs/2606.23986)
*Yuchen Zhang,Yu Ge,Bruno Clerckx,Tareq Y. Al-Naffouri*

Main category: cs.IT

TL;DR: The paper analyzes direction-of-arrival estimation using a Microwave Linear Analog Computer (MiLAC) combiner, showing it can achieve the digital Cramér-Rao bound with only 2 RF chains per target, and a stem-connected MiLAC attains this bound asymptotically with linear scaling.


<details>
  <summary>Details</summary>
Motivation: While lossless reciprocal MiLACs have been studied for wireless communications beamforming, their sensing performance (specifically direction-of-arrival estimation) remains largely unexplored.

Method: The authors analyze the Fisher information matrix for DOA estimation with K far-field targets using a tunable receive-side lossless reciprocal MiLAC combiner, showing it depends only on the orthogonal projector onto the combiner's row space. They use dimension-counting arguments to bound the number of tunable components needed to achieve the digital CRB.

Result: The Fisher information of MiLAC never exceeds that of a fully digital receiver. Equality requires the row space to contain the 2K-dimensional joint steering-derivative subspace (2 RF chains per target). The stem-connected MiLAC achieves the digital CRB asymptotically with linear scaling in antenna and target counts, unlike phase-shifter front ends.

Conclusion: MiLAC can exactly attain the fully digital Cramér-Rao bound for DOA estimation with a zero-gap threshold of two RF chains per target, offering significant hardware savings over fully digital receivers while outperforming phase-shifter-based front ends.
...
### [306] [Minimal additive codes and additive strong blocking sets](https://arxiv.org/abs/2606.24262)
*Gianira N. Alfarano,Marine Le Meur*

Main category: cs.IT

TL;DR: This paper introduces minimal additive codes over finite fields, establishes their geometric equivalence to additive strong blocking sets, compares them with outer strong blocking sets, and provides constructions and bounds on their minimum length.


<details>
  <summary>Details</summary>
Motivation: To generalize linear codes by relaxing linearity over the alphabet while retaining linearity over a subfield, and to initiate the study of minimal additive codes from a geometric perspective.

Method: The paper defines additive strong blocking sets (h-projective systems) and establishes a one-to-one correspondence between nondegenerate minimal additive codes and additive strong blocking sets. It compares this framework with outer strong blocking sets.

Result: Established equivalence between minimal additive codes and additive strong blocking sets; showed outer strong blocking sets as a special case; provided constructions, existence results, and upper/lower/asymptotic bounds on minimum length.

Conclusion: The geometric framework via additive strong blocking sets effectively characterizes minimal additive codes, offering new bounds and constructions while unifying existing theories.
...
### [307] [Discrepancy for Random Linear Codes](https://arxiv.org/abs/2606.24471)
*Dean Doron,Tal Leonov,Jonathan Mosheiff,Henrique Navas,Nicolas Resch,João Ribeiro*

Main category: cs.IT

TL;DR: Random linear codes achieve nearly optimal discrepancy properties, matching unstructured random codes for list-decoding above capacity and zero-error list-recovery over prime fields, with applications to secret sharing schemes resilient to local leakage.


<details>
  <summary>Details</summary>
Motivation: To establish that random linear codes, which have algebraic structure, can nonetheless achieve nearly optimal discrepancy properties comparable to unstructured random codes, particularly for list-decoding from errors above capacity and zero-error list-recovery.

Method: The proofs use a refined second-moment analysis that tracks intersection sizes as random generators are iteratively added to the linear code C.

Result: Random linear codes match unstructured codes for: (1) list-decoding above capacity, controlling all translates of a fixed test; (2) zero-error list-recovery above capacity over prime fields, controlling large families of Fourier-pseudorandom tests.

Conclusion: Random linear codes have nearly optimal discrepancy in broad regimes, with implications for list-decoding, list-recovery, and construction of n-party linear ramp secret sharing schemes with thresholds of n/(2 log q) and 5n/(2 log q) resilient to balanced local leakage.
...
### [308] [Importance Sampling for Event Discovery via Guesswork](https://arxiv.org/abs/2606.24537)
*Asaf Cohen*

Main category: cs.IT

TL;DR: This paper formalizes Quality of Discovery for importance sampling, shifting focus from minimizing variance to minimizing the description length (surprisal) of discovered rare trajectories. It proves discovery-optimality is achieved by minimizing the guesswork exponent $H(Q)+D(Q\|p)$, not classical IS objective $D(Q\|p)$, and serves as a lexicographic tie-breaker under budget constraints.


<details>
  <summary>Details</summary>
Motivation: Traditional importance sampling minimizes estimator variance for rare-event probability estimation, but many applications prioritize rapid discovery — generating a trajectory within a rare set. This requires a new design principle centered on hitting time rather than ensemble estimation.

Method: Formalizes Quality of Discovery by minimizing description length (surprisal) of discovered trajectories under nominal model $p$. Proves equivalence to minimizing nominal rank exponent $J_{\mathrm{rank}}(q_n)$, linked to guesswork $G_n(x^n)$. For i.i.d. models and type-defined rare sets, derives optimal distribution $Q_{\mathrm{GW}}^*$ minimizing $H(Q)+D(Q\|p)$.

Result: Discovery-optimal distribution minimizes $H(Q)+D(Q\|p)$ (guesswork exponent) rather than classical IS objective $D(Q\|p)$. This ensures the discovered trajectory is the "least surprising" representative. The guesswork exponent also serves as a lexicographic tie-breaker when hitting-time minimizer is not unique.

Conclusion: Establishes $H(Q)+D(Q\|p)$ as the natural objective for discovery-based importance sampling, bridging randomized sampling and systematic search through a formal framework.
...
### [309] [Time-varying Wireless Channel Tracking with Online Parameter Learning via the Birth-Death-Drift Model](https://arxiv.org/abs/2606.24727)
*Tiancheng Gao,Mohamed Akrout,Faouzi Bellili,Amine Mezghani*

Main category: cs.IT

TL;DR: The paper proposes BDD-VAMP-EM, a novel algorithm for massive MIMO CSI acquisition that overcomes three key limitations of the existing AMP-SI method by unifying the birth-death-drift model, vector AMP, and expectation-maximization into a fully automated framework, achieving superior performance especially under parameter mismatch.


<details>
  <summary>Details</summary>
Motivation: Accurate massive MIMO CSI acquisition with low pilot overhead is critical in dynamic environments. Existing methods like AMP-SI have practical limitations: reliance on i.i.d. Gaussian sensing matrices, need for perfect BDD parameter knowledge, and statistically approximate temporal information treatment.

Method: The authors introduce BDD-VAMP-EM, a fully automated algorithm that unifies the birth-death-drift (BDD) model, vector AMP (VAMP), and expectation-maximization (EM) in a single framework.

Result: Simulations show BDD-VAMP-EM consistently outperforms existing benchmarks, particularly under model parameter mismatch, confirming its practical viability.

Conclusion: The proposed BDD-VAMP-EM addresses all three limitations of AMP-SI and demonstrates robust practical deployment potential for dynamic massive MIMO environments.
