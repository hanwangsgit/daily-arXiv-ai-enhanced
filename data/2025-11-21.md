...
TL;DR: Proposes SAM2S, a surgical video segmentation foundation model that enhances SAM2 with long-term tracking, temporal semantic learning, and ambiguity-resilient learning, achieving 80.42 average J&F score on the new SA-SV benchmark.


<details>
  <summary>Details</summary>
Motivation: Surgical video segmentation is crucial for computer-assisted surgery, but existing iVOS models like SAM2 face challenges in surgical scenarios due to domain gap and limited long-term tracking capabilities.

Method: Built SA-SV benchmark (largest surgical iVOS dataset), then developed SAM2S with three key components: DiveMem for long-term tracking, temporal semantic learning for instrument understanding, and ambiguity-resilient learning for multi-source data consistency.

Result: Fine-tuning on SA-SV improved SAM2 by 12.99 average J&F. SAM2S achieved 80.42 average J&F, surpassing vanilla and fine-tuned SAM2 by 17.10 and 4.11 points respectively, while maintaining 68 FPS real-time inference.

Conclusion: SAM2S effectively addresses surgical iVOS challenges, demonstrating substantial performance gains and strong zero-shot generalization, making it suitable for real-time computer-assisted surgery applications.
...
Method: Proposes MLLM-Guided Semantic Alignment Module (MGSA) with learnable queries and semantic alignment loss, plus a two-stage progressive training strategy with self-synthesis pipeline.
...
Conclusion: The framework effectively bridges semantic gaps and learns from limited data, providing a universal solution for complex virtual try-on scenarios.
...
TL;DR: AGU is an efficient guided upsampling method that optimizes multiple image quality characteristics like noise reduction and sharpness enhancement for low-light images through multi-parameter optimization.


<details>
  <summary>Details</summary>
Motivation: Existing guided image methods fail with low-light images due to high noise and low brightness, resulting in suboptimal upscaling results.
...
Conclusion: The proposed method effectively addresses the limitations of guided methods for low-light image upscaling through learned multi-parameter optimization.
...
TL;DR: EfficientSAM3 is a family of efficient models that enables on-device concept segmentation and tracking through Progressive Hierarchical Distillation, achieving strong performance-efficiency trade-offs.


<details>
  <summary>Details</summary>
Motivation: SAM3's unified architecture is prohibitive for on-device use, requiring efficient alternatives that maintain high fidelity to teacher behavior while enabling deployment on resource-constrained devices.

Method: Progressive Hierarchical Distillation (PHD) in three stages: (1) Encoder Distillation with prompt-in-the-loop training, (2) Temporal Memory Distillation using compact Perceiver-based module, and (3) End-to-End Fine-Tuning on SAM3 PCS data.

Result: Produces a spectrum of student variants using lightweight backbones (RepViT, TinyViT, EfficientViT) that maintain high performance while enabling on-device deployment, with strong benchmarks on VOS datasets.

Conclusion: EfficientSAM3 successfully transfers SAM3's capabilities to efficient models through progressive distillation, achieving practical on-device concept segmentation and tracking while preserving performance.
...
TL;DR: Proposes four novel extensions for model-based 6D pose estimation to handle occlusion: dynamic sampling, multi-hypothesis inference, iterative refinement, and occlusion-focused training, achieving significant accuracy improvements and faster inference.


<details>
  <summary>Details</summary>
Motivation: Existing multi-stage pose estimation pipelines are prone to errors under occlusion that propagate through sequential processing, degrading performance for unseen objects.

Method: Four extensions: dynamic non-uniform dense sampling on visible regions, multi-hypothesis inference with confidence-ranked candidates, iterative refinement, and occlusion-focused training augmentations.

Result: Achieves >5% accuracy improvement on ICBIN and >2% on BOP benchmarks, with approximately 3x faster inference speed.

Conclusion: The proposed approach effectively addresses occlusion challenges in 6D pose estimation, improving both accuracy and efficiency while introducing a new weighted evaluation metric for occlusion scenarios.
...
TL;DR: Proposes automated and manual methods to generate synthetic historical maps by transferring cartographic styles onto vector data, addressing data scarcity for deep learning in historical document analysis.


<details>
  <summary>Details</summary>
Motivation: Historical maps lack sufficient annotated training data for deep learning, and existing synthetic data often lacks realism and diversity needed for effective learning.

Method: Uses deep generative approach and manual stochastic degradation to emulate visual uncertainty in historical map scans, transferring cartographic style onto vector data to create synthetic maps.
...
Conclusion: The approach effectively addresses data scarcity in historical map analysis by creating realistic synthetic training data that enables successful domain-adaptive semantic segmentation.
...
<details>
  <summary>Details</summary>
Motivation: Address the limitations of existing 6D pose estimation methods: model-based methods require exact CAD models and transfer poorly, model-free methods fail under challenging conditions, and category-level approaches ignore environment/object priors in industrial settings.

Method: Uses single RGB-D observation, infers box dimensions via fast binary search, estimates poses using category CAD template (not instance-specific), employs depth-based plausibility filter and early-stopping strategy to reject implausible hypotheses.

Result: Competitive or superior 6D pose precision compared to existing methods while reducing inference time by approximately 76% in real-world storage scenarios and public benchmarks.

Conclusion: Box6D effectively balances flexibility and accuracy for warehouse applications by leveraging category-level approach with environment-specific priors, offering practical solution for industrial 6D pose estimation.
...
<details>
  <summary>Details</summary>
Motivation: VLMs struggle with domain-specific video classification due to a rationale gap - sparse domain data can't bridge the semantic distance between complex spatio-temporal content and abstract labels.

Method: Two-stage approach: 1) Prompt VLMs to generate detailed textual rationales for videos, then fine-tune on these self-generated rationales; 2) Perform conventional supervised fine-tuning on task labels after domain reasoning is acquired.
...
TL;DR: MGLL is a contrastive learning framework that improves multi-label and cross-granularity alignment in medical imaging by leveraging structured multi-label supervision and cross-granularity consistency.


<details>
  <summary>Details</summary>
Motivation: CLIP's single-label, single-granularity alignment is insufficient for medical imaging where images often have multiple high-level labels across different annotation granularities.

Method: Proposes Multi-Granular Language Learning (MGLL) with structured multi-label supervision, cross-granularity textual integration, soft-label supervision with point-wise constraints, and smooth KL divergence for consistency.
...
Conclusion: MGLL effectively addresses multi-label and cross-granularity alignment challenges in medical imaging through its contrastive learning framework.
...
<details>
  <summary>Details</summary>
Motivation: 3D echocardiography offers better image quality but physicians are accustomed to interpreting 2D views. This bridges the gap by allowing clinicians to work with familiar 2D formats while benefiting from 3D acquisition speed.

Method: Combines deep learning view classifier with anatomical landmark-based heuristics and cardiologist-provided rules to reconstruct standard echocardiography views from 3D volumes.

Result: Validated with 96% accuracy on 1,600 videos from 2 hospitals by three cardiologists. Extracted 2D videos preserve spatial calibration and diagnostic features, enabling accurate cardiac abnormality detection and clinical-grade measurements.

Conclusion: The approach successfully bridges 3D and 2D echocardiography, allowing clinicians to obtain accurate interpretations from 3D volumes while maintaining their familiar workflow with standard 2D views.
...
<details>
  <summary>Details</summary>
Motivation: Current VSGG systems are closed, feed-forward pipelines that cannot incorporate human guidance, while promptable segmentation models like SAM2 lack semantic or relational reasoning capabilities.

Method: The framework uses a Dynamic Interaction Discovery Module to generate subject-conditioned object prompts and a Semantic Classification Head for joint entity and predicate reasoning. It segments and tracks subjects across time, discovers interacting objects, and predicts triplets to form temporally consistent scene graphs.
...
Method: Proposes InfoCLIP with two mutual information objectives: compressing pixel-text modality alignment from pretrained CLIP to reduce noise, and maximizing mutual information between pretrained and fine-tuned model alignment knowledge.

Result: Extensive evaluations across various benchmarks validate effectiveness in enhancing CLIP fine-tuning for open-vocabulary semantic segmentation.
...
TL;DR: A novel consistency regularization approach using BI-RADS-inspired morphological features improves breast ultrasound tumor segmentation generalization in multi-task learning by reducing destructive interference between segmentation and classification tasks.


<details>
  <summary>Details</summary>
Motivation: Multi-task learning often suffers from destructive task interference that limits generalization performance, particularly in breast ultrasound-based tumor segmentation where joint training can underperform single-task baselines.
...
Result: Statistically significant improvements (p<0.001) in segmentation generalization across three external datasets: UDIAT (Dice=0.81 vs 0.59), BUSI (0.66 vs 0.56), BUS-UCLM (0.69 vs 0.49). Achieved state-of-the-art segmentation performance on UDIAT dataset.

Conclusion: The proposed consistency regularization approach effectively mitigates destructive task interference in multi-task learning, significantly improving generalization performance for breast ultrasound tumor segmentation across diverse external datasets.
...
<details>
  <summary>Details</summary>
Motivation: Current approaches relying on global similarity struggle with fine-grained category distinctions and attribute diversity in large-scale e-commerce scenarios, requiring a unified framework for object detection, category prediction, and attribute recognition.

Method: Extract refined ROI-level features from detected objects and use a BART-based generator to produce semantic tokens in coarse-to-fine sequence covering category hierarchies and property-value pairs, supporting property-conditioned attribute recognition.
...
Conclusion: The proposed framework achieves stronger fine-grained recognition and more coherent unified inference for visual semantic understanding tasks.
...
TL;DR: FADS is a fairness-aware demonstration selection method for multimodal LLMs that uses clustering-based sampling to create demographically balanced exemplars, reducing disparities in medical image reasoning without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing debiasing methods require large labeled datasets or fine-tuning, which are impractical for foundation-scale models. In-context learning offers a lightweight alternative, but conventional demonstration selection fails to ensure fairness due to demographic imbalance.

Method: Proposed Fairness-Aware Demonstration Selection (FADS) that builds demographically balanced and semantically relevant demonstrations through clustering-based sampling from available exemplars.
...
Conclusion: Fairness-aware in-context learning offers a scalable and data-efficient solution for equitable medical image reasoning, providing an efficient path toward fair medical AI without requiring model fine-tuning.
...
TL;DR: A graph-based approach using GCNs and Gaussianization improves OOD detection in long-tailed vision datasets by enhancing inter-sample relationships and addressing tail-class performance issues.


<details>
  <summary>Details</summary>
Motivation: OOD detection is challenging with long-tailed ID datasets, leading to high false positive rates and poor tail-class classification accuracy. Existing methods struggle with tail-class performance.

Method: Use pre-trained model features to initialize graph structure, apply Gaussianization to normalize activations, then refine with GCNs to create optimal feature space for long-tailed OOD detection.

Result: Outperforms state-of-the-art methods by large margins on CIFAR10-LT, CIFAR100-LT, and ImageNet-LT benchmarks in terms of FPR and tail-class ID classification accuracy.

Conclusion: Graph-based representation with GCNs and Gaussianization effectively addresses long-tailed OOD detection challenges, significantly improving performance over existing approaches.
...
TL;DR: A sequence-level optimization framework generates printable adversarial textures for clothing that maintain concealment across entire walking videos, addressing motion and deformation challenges in both digital and physical settings.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks for human detection are vulnerable to adversarial attacks, creating safety and privacy risks. Existing wearable attack approaches fail to maintain concealment across long video sequences with motion, pose changes, and garment deformation.

Method: Product images are mapped to UV space and converted to compact palette/control-point parameterization with ICC color locking. A physically-based human-garment pipeline simulates motion, multi-angle viewpoints, cloth dynamics, and illumination variation. Sequence-level optimization with temporal weighting minimizes detection confidence across entire video sequences.
...
Conclusion: The framework successfully generates natural, printable adversarial textures that remain effective throughout entire walking sequences, confirming real-world feasibility for wearable adversarial attacks.
...
TL;DR: The paper proposes a Mixture-of-Ranks (MoR) architecture for single-step image super-resolution that integrates sparse Mixture-of-Experts into Real-ISR, using fine-grained expert partitioning and degradation-aware routing to adaptively handle complex real-world degraded images.


<details>
  <summary>Details</summary>
Motivation: Existing dense Real-ISR models using LoRA fine-tuning lack adaptive capability to capture heterogeneous characteristics of complex real-world degraded samples and enable efficient knowledge sharing under equivalent computational budgets.
...
Result: Comprehensive experiments validate the framework's effectiveness and demonstrate state-of-the-art performance in real-world image super-resolution tasks.

Conclusion: The MoR architecture successfully addresses limitations of dense Real-ISR models by enabling adaptive knowledge recombination and optimal computational resource allocation through sparse expert activation guided by degradation severity.
...
TL;DR: A deep learning-based method using CNN to classify materials from laser speckle patterns for safer laser cutting, achieving high accuracy even with different laser colors.


<details>
  <summary>Details</summary>
Motivation: Laser cutting generates hazardous dust and smoke, requiring real-time material monitoring. Existing speckle sensing methods struggle with laser color changes.

Method: Train a convolutional neural network on laser speckle pattern dataset to recognize material types for process monitoring and control.

Result: 98.30% training accuracy, 96.88% validation accuracy, and 0.9643 F1-score on 3000 new images across 30 materials.

Conclusion: The method provides robust and accurate material classification for material-aware laser cutting using speckle sensing.
...
TL;DR: CuriGS is a curriculum-guided framework that improves 3D Gaussian Splatting for sparse-view reconstruction by generating pseudo-views with progressive perturbation levels and selective quality-based promotion.
...
Method: Introduces student views (pseudo-views) around ground-truth poses with different perturbation levels, uses curriculum training with progressive unlocking, and applies depth-correlation and co-regularization with multi-signal quality evaluation.

Result: Outperforms state-of-the-art baselines in rendering fidelity and geometric consistency across various synthetic and real sparse-view scenes.

Conclusion: CuriGS effectively addresses sparse-view 3D reconstruction challenges through curriculum-guided training and selective view augmentation.
...
TL;DR: Cross-modal learning enriches high-resolution satellite imagery with UAV-level detail for crop canopy trait estimation, bridging the gap between satellite and UAV sensing in agriculture.


<details>
  <summary>Details</summary>
Motivation: Satellite missions are limited by spatial resolution for micro-plot management, while UAVs provide high performance but have different operational constraints. The goal is to leverage the benefits of both modalities.

Method: A cross-modal learning strategy that learns fine-grained spectral-spatial correspondences between satellite and UAV imagery using co-registered image pairs from 84 hybrid maize varieties across five locations.

Result: Generated UAV-like representations from satellite inputs consistently outperform real satellite imagery on downstream tasks like yield and nitrogen prediction.
...
TL;DR: A framework using large language models (LLMs) to address food recognition challenges including domain shift, long-tailed distribution, and fine-grained classification by generating food titles/ingredients and aligning text-image features.
...
Method: Leverage LLMs to parse food images and generate food titles/ingredients, then project generated texts and images to shared embedding space to maximize pair similarities, and use aligned multimodal features for recognition.
...
Conclusion: Using LLMs to generate textual descriptions and align multimodal features provides an effective solution for challenging food recognition problems including domain shift, long-tailed distribution, and fine-grained classification.
...
<details>
  <summary>Details</summary>
Motivation: KV caching for next-scale prediction in visual autoregressive models faces excessive memory growth with increasing scales, severely limiting scalability. Current KV caching designs are largely unexplored for this paradigm.

Method: AMS-KV prioritizes storing KVs from condensed and local scales, preserves relevant tokens, and optimizes cache utilization through inter-scale similarity analysis of cache-demanding layers.

Result: Reduces KV cache usage by up to 84.83%, self-attention latency by 60.48%, and enables stable scaling to batch size 256 where baseline fails at 128.

Conclusion: AMS-KV effectively addresses KV memory bottlenecks in VAR models through scale-adaptive caching, significantly improving scalability and computational efficiency while maintaining generation quality.
...
TL;DR: LiSTAR is a generative world model for synthesizing high-fidelity 4D LiDAR data using a Hybrid-Cylindrical-Spherical representation and Spatio-Temporal Attention with Ray-Centric Transformer to address challenges in autonomous driving simulations.
...
Result: State-of-the-art performance with 76% reduction in generation MMD, 32% improvement in reconstruction IoU, and 50% reduction in prediction L1 Med error.

Conclusion: LiSTAR provides a powerful foundation for creating realistic and controllable autonomous systems simulations with superior 4D LiDAR data synthesis capabilities.
...
TL;DR: VideoSeg-R1 is the first framework to use reinforcement learning for video reasoning segmentation, featuring a decoupled architecture with hierarchical frame sampling, explicit reasoning chains, and adaptive difficulty-aware reasoning length control.
...
Method: Uses a decoupled architecture with three stages: hierarchical text-guided frame sampling, reasoning model with explicit reasoning chains, and segmentation-propagation using SAM2 and XMem. Incorporates reinforcement learning and adaptive difficulty-aware reasoning length control.

Result: Achieves state-of-the-art performance on multiple benchmarks for complex video reasoning and segmentation tasks.

Conclusion: VideoSeg-R1 successfully introduces reinforcement learning into video reasoning segmentation, demonstrating improved generalization and explicit reasoning capabilities compared to traditional supervised approaches.
...
TL;DR: SpectralTrain is a universal training framework that uses curriculum learning and PCA-based spectral downsampling to reduce computational costs for hyperspectral image classification while maintaining performance.
...
Result: Achieves 2-7x training speedups with small-to-moderate accuracy differences across three benchmark datasets, demonstrating strong generalization across spatial scales and spectral characteristics.
...
TL;DR: Rad-GS is a 4D radar-camera SLAM system that uses 3D Gaussian representation for large-scale outdoor environments, achieving robust mapping comparable to camera/LiDAR methods while reducing memory usage.


<details>
  <summary>Details</summary>
Motivation: To enable robust outdoor mapping at kilometer scale using 4D mmWave radar, overcoming limitations of traditional sensors and addressing dynamic object artifacts in large environments.

Method: Combines radar point clouds with Doppler information for dynamic object masking, uses unsynchronized images for global refinement, and employs octree structure with Gaussian primitive management to reduce noise and memory consumption.

Result: Achieves performance comparable to traditional 3D Gaussian methods using camera/LiDAR inputs, with successful kilometer-scale real-world reconstruction and improved localization accuracy.

Conclusion: Demonstrates the feasibility of robust outdoor mapping using 4D mmWave radar, with potential for large-scale scene reconstruction applications.
...
TL;DR: The paper proposes T2T-VICL, a pipeline for cross-task visual in-context learning that enables vision-language models to perform tasks when visual prompts and target images come from different vision tasks.


<details>
  <summary>Details</summary>
Motivation: To investigate whether vision-language models can enable visual in-context learning when the demonstration examples and target images originate from different visual tasks, expanding the boundaries of cross-task VICL.

Method: Developed a collaborative pipeline with text prompt generation/selection mechanism to describe task differences, created first cross-task VICL dataset, and proposed inference framework combining perceptual score-based reasoning with traditional metrics.
...
Conclusion: Successfully unlocked the boundaries of cross-task visual in-context learning within vision-language models, showing their potential to handle diverse visual tasks through unified in-context learning approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing 4DGS methods struggle with ambiguous pixel correspondences and inadequate densification in dynamic regions, leading to poor reconstruction accuracy.
...
Result: Significantly improves temporal consistency and achieves state-of-the-art perceptual rendering quality, with 0.39dB PSNR improvement on Technicolor Light Field dataset. Visualization shows improved alignment between splats and dynamic objects.

Conclusion: The proposed method effectively addresses limitations of existing 4DGS approaches for dynamic scenes through targeted error correction and improved splat-object mapping consistency.
...
TL;DR: DCS-LDM decouples information complexity from scale in visual generation by creating a hierarchical, scale-independent latent space that supports flexible resolution/frame rate decoding and progressive coarse-to-fine generation.


<details>
  <summary>Details</summary>
Motivation: Existing latent diffusion models couple scale with content complexity, using more tokens for higher-resolution/frame-rate content, but latent capacity should primarily depend on content complexity rather than scale.

Method: Constructs a hierarchical, scale-independent latent space with multi-level tokens that models sample complexity and supports decoding to arbitrary resolutions/frame rates within fixed latent representation.
...
Conclusion: DCS-LDM provides a novel paradigm that successfully decouples information complexity from scale, enabling progressive coarse-to-fine generation and flexible quality-scale adaptation.
...
<details>
  <summary>Details</summary>
Motivation: Current VFI methods use bilinear upsampling for high-resolution flows, causing blur/mosaic at edges and failing to capture fine pixel motion, leading to ghosting and discontinuities in interpolated frames.

Method: Two-stage approach: 1) Guided Flow Upsampling (GFU) uses input frames as guidance to refine bilinear-upsampled flows, and 2) Texture Mapping creates an intermediate proxy frame and maps clear texture blocks from input frames onto it via reconstruction.

Result: Extensive experiments show VTinker achieves state-of-the-art performance in video frame interpolation, with clearer flow edges and reduced ghosting/discontinuities.

Conclusion: VTinker's GFU and Texture Mapping components effectively address limitations of traditional flow-based VFI methods, producing higher quality interpolated frames with improved visual fidelity.
...
TL;DR: PiN-CLIP improves AI-generated image detection by injecting positive-incentive noise into feature space to suppress spurious shortcuts and amplify stable forensic cues, achieving state-of-the-art performance.
...
Method: Jointly trains noise generator and detection network using variational positive-incentive principle, constructing noise via cross-attention fusion of visual and categorical semantic features to fine-tune visual encoder.
...
Conclusion: Positive-incentive noise injection effectively suppresses shortcut-sensitive directions while amplifying stable forensic cues, enabling more robust and generalized artifact representations for AI-generated image detection.
...
<details>
  <summary>Details</summary>
Motivation: Existing QECV methods rely on known QPs and use uniform architectures, limiting real-world applicability where QPs may be unknown and computational demands vary with compression levels.

Method: Uses pretrained Degradation Representation Learning (DRL) module to extract high-dimensional multiscale degradation representations, and hierarchical termination mechanism to dynamically adjust artifact reduction stages based on compression level.
...
Conclusion: The proposed approach effectively addresses limitations of existing blind QECV methods by providing detailed spatial degradation guidance and adaptive computational allocation.
...
Method: Two key components: 1) Spatial-prioritized optimal transport assignment that dynamically emphasizes well-located and spatially reliable samples, 2) Rank-aware adaptive self-distillation scheme that injects ranking perception via self-distillation paradigm.
...
Conclusion: SR3D effectively bridges the training-inference gap in 3D object detection and achieves state-of-the-art performance for indoor point cloud detection tasks.
...
TL;DR: Proposes HSRW-CD dataset for high-resolution water body change detection and SSCP attention module to enhance spatial semantics and structural information in deep features.
...
Method: Introduces HSRW-CD dataset with >3m spatial resolution and SSCP attention module with three components: Multi-Semantic spatial Attention (MSA), Structural Relation-aware Global Attention (SRGA), and Channel-wise Self-Attention (CSA).
...
Conclusion: The proposed SSCP module effectively enhances spatial semantics and structure in water body change detection networks and can be integrated as plug-and-play component into existing models.
...
TL;DR: LEGO-SLAM is the first real-time, open-vocabulary SLAM system using 3D Gaussian Splatting that integrates language features through a scene-adaptive encoder-decoder, enabling semantic understanding while maintaining 15 FPS performance.


<details>
  <summary>Details</summary>
Motivation: Current 3DGS-based SLAM systems lack semantic understanding for robotic interaction, and existing language integration methods either require excessive memory or lack adaptability to novel environments.

Method: Uses a scene-adaptive encoder-decoder to distill high-dimensional language embeddings into compact 16D features, enabling real-time performance. Includes language-guided pruning to reduce Gaussian count by 60% and language-based loop detection using mapping features.
...
Conclusion: LEGO-SLAM successfully integrates language understanding into real-time SLAM, enabling semantic robotic interaction in novel environments with efficient memory usage and rendering performance.
...
TL;DR: RGE enhances multimodal embeddings by incorporating explicit reasoning from MLLMs, improving retrieval performance by 4.9% through structured rationale generation coupled with contrastive training.
...
Method: Proposes Reasoning Guided Embeddings (RGE) that performs structured rationale generation conditioned on instructions, then extracts representations after reasoning unfolds, coupled with contrastive training.

Result: On MMEB benchmark, reasoning-guided conditioning improves multimodal retrieval performance by 4.9% over non-reasoning baseline.
...
TL;DR: PPCL is a pluggable pruning framework for Diffusion Transformers that reduces parameters by 50% with minimal performance degradation through contiguous layer distillation.
...
Method: Uses linear probing and similarity trend analysis to identify redundant layers, then applies plug-and-play teacher-student alternating distillation for depth-wise and width-wise pruning in a single training phase.
...
Conclusion: PPCL provides an effective compression solution for DiT models, making them suitable for resource-constrained environments with high compression ratios.
...
<details>
  <summary>Details</summary>
Motivation: Current grid-based map methods for spatial understanding in MLLMs rely on discretized raster representations that limit fine-grained spatial reasoning capabilities.

Method: Two-stage approach: 1) Supervised fine-tuning using high-quality dataset from AI2THOR simulator to learn mapping from visual inputs to precise boundary coordinates; 2) Reinforcement fine-tuning to enhance real-world generalization. Uses continuous object boundary coordinates to quantify inter-object distances and sizes.

Result: V2LO-7B model achieves 4.92% average improvement over grid map-trained models on QVS-Bench and mainstream spatial reasoning benchmarks, demonstrating superior spatial reasoning capabilities.

Conclusion: The proposed Video2Layout framework with continuous coordinate representation effectively overcomes limitations of discretized grid maps and enables more accurate spatial reasoning in multimodal language models.
...
TL;DR: Simba reformulates point cloud completion by replacing direct regression with distribution learning using diffusion models, integrating symmetry priors to avoid overfitting and improve robustness against noise.


<details>
  <summary>Details</summary>
Motivation: Address limitations of regression-based methods that overfit to instance-specific transformations and are sensitive to input noise, preventing preservation of both fine details and global structure.

Method: Reformulates point-wise transformation regression as distribution learning, integrates symmetry priors with diffusion models for robust geometric structures, and uses hierarchical Mamba-based architecture for upsampling.

Result: Achieves state-of-the-art performance on PCN, ShapeNet, and KITTI benchmarks, demonstrating superior completion quality and robustness.

Conclusion: Simba successfully overcomes limitations of regression-based approaches by combining distribution learning with symmetry priors, enabling better preservation of geometric details while maintaining structural integrity.
...
TL;DR: LNG-SWR improves medical image segmentation robustness by injecting noise during training to learn frequency-bias priors, then using selective wavelet reconstruction to suppress noise-sensitive bands while enhancing structures.


<details>
  <summary>Details</summary>
Motivation: Address the clean-robustness trade-off and high training costs of adversarial training in medical imaging by developing a more scalable and maintainable robustness solution.

Method: Inject small zero-mean noise at multiple layers during training to learn frequency-bias priors, then apply prior-guided selective wavelet reconstruction on input/feature branches for frequency adaptation.

Result: Consistent gains on clean Dice/IoU, significant reduction in performance drop under strong attacks, and additive gains when combined with adversarial training on CT and ultrasound datasets.

Conclusion: LNG-SWR provides a simple, effective, and engineering-friendly path to robust medical image segmentation that works with both adversarial and standard training regimes.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of existing methods that fail to directly maximize output token length as an explicit optimization objective, lacking stability and controllability in inducing verbose outputs from VLMs.

Method: Two-stage framework: 1) Adversarial prompt search using reinforcement learning to identify prompts that induce verbose outputs, 2) Vision-aligned perturbation optimization to craft adversarial images that maximize similarity with adversarial prompt embeddings.
...
Conclusion: The proposed VTIA method successfully induces verbose text generation in VLMs through imperceptible adversarial perturbations, highlighting security vulnerabilities in multimodal AI systems.
...
TL;DR: EvoVLA is a self-supervised Vision-Language-Action framework that addresses stage hallucination in long-horizon robotic manipulation through stage-aligned rewards, pose-based object exploration, and long-horizon memory, achieving significant improvements in task success and sample efficiency.


<details>
  <summary>Details</summary>
Motivation: Current VLA models suffer from stage hallucination where agents exploit coarse evaluation signals to shortcut multi-step tasks without truly completing them, making long-horizon robotic manipulation challenging.
...
Conclusion: EvoVLA effectively addresses stage hallucination in long-horizon manipulation, demonstrates strong sim-to-real transfer and generalization, and significantly outperforms existing VLA models in both simulation and real-world settings.
...
TL;DR: RF-CLIP improves open-vocabulary semantic segmentation by addressing CLIP's attention distraction to irrelevant tokens, using a training-free approach that refocuses attention on target regions.


<details>
  <summary>Details</summary>
Motivation: Existing methods rarely investigate CLIP's performance boundaries for dense prediction from an interpretability perspective, particularly how CLIP diverts attention from target regions to irrelevant tokens.

Method: Propose ReFocusing CLIP (RF-CLIP), a training-free approach that identifies and filters dimension-specific over-activated distraction tokens, then redirects attention back to target regions to improve multimodal alignment.

Result: Achieves state-of-the-art performance on eight benchmarks while maintaining high inference efficiency.

Conclusion: By addressing CLIP's attention distraction mechanism through a human-inspired refocusing approach, RF-CLIP significantly enhances pixel-level multimodal alignment for open-vocabulary semantic segmentation.
...
TL;DR: Mantis is a VLA framework with Disentangled Visual Foresight that decouples visual prediction from the backbone using meta queries and DiT, achieving state-of-the-art performance on robotics tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models face issues with high-dimensional visual state prediction distributing model capacity, training costs, information bottlenecks from compression, and poor comprehension due to neglect of language supervision.

Method: Uses Disentangled Visual Foresight with meta queries and diffusion Transformer head, decoupling visual foresight from backbone via residual connections and next-state prediction to capture latent actions.

Result: Achieves 96.7% success rate on LIBERO benchmark after fine-tuning, outperforms baselines with high convergence speed. Real-world evaluations show superior instruction-following, generalization, and reasoning compared to π₀.₅.

Conclusion: Mantis effectively addresses VLA limitations by disentangling visual foresight, maintaining comprehension through language supervision, and demonstrating superior performance in robotics tasks.
...
TL;DR: Proposes DSLGA, a two-stage unsupervised domain adaptation method for Visible-Infrared person Re-ID to handle inter-domain and intra-domain modality discrepancies without requiring target domain annotations.


<details>
  <summary>Details</summary>
Motivation: Existing VI-ReID algorithms struggle in real-world applications due to discrepancies between public datasets and real-world data, requiring a solution that transfers knowledge without compromising accuracy or needing new annotations.

Method: Two-stage approach: 1) Pre-training with Domain-Shared Learning Strategy (DSLS) to mitigate inter-domain discrepancies, 2) Fine-tuning with Gradual Alignment Strategy (GAS) for cross-modality alignment through cluster-to-holistic alignment. Also introduces CMDA-XD testing framework.
...
Conclusion: DSLGA effectively addresses modality discrepancies in UDA-VI-ReID through shared learning and gradual alignment strategies, demonstrating superior performance compared to existing approaches.
...
TL;DR: PrIntMesh is a template-based framework that reconstructs organs as unified systems while preserving topology and internal boundaries, achieving high geometric accuracy and structural consistency.


<details>
  <summary>Details</summary>
Motivation: Current deep-learning approaches treat organ substructures independently, leading to anatomically implausible reconstructions that don't account for geometric and spatial relationships between interconnected parts.
...
Conclusion: PrIntMesh provides a data-efficient, topology-preserving solution suitable for clinical use that treats organs as unified systems rather than independent parts.
...
<details>
  <summary>Details</summary>
Motivation: Despite impressive progress in VLAs for embodied environments, their adversarial robustness remains largely unexplored, especially under realistic multimodal and black-box conditions where cross-modal misalignment fundamentally affects reasoning.

Method: VLA-Fool unifies three attack levels: textual perturbations (gradient/prompt-based), visual perturbations (patch/noise), and cross-modal misalignment attacks. It incorporates a VLA-aware semantic space into linguistic prompts for automated semantic-guided prompting.

Result: Experiments on LIBERO benchmark using fine-tuned OpenVLA show that even minor multimodal perturbations cause significant behavioral deviations, revealing the fragility of embodied multimodal alignment.

Conclusion: The study demonstrates the vulnerability of VLA models to multimodal adversarial attacks and highlights the critical need for robust multimodal alignment in embodied AI systems.
...
<details>
  <summary>Details</summary>
Motivation: To bridge the performance gap between unsupervised and supervised image classification by improving clustering methods in the era of foundational models, which have shifted focus away from representation learning.

Method: Multi-head clustering on frozen backbone, adaptive nearest neighbor selection, cluster ensembling to consolidate diverse clusterings, and training classifier using consensus clustering as pseudo-labels.

Result: Achieved 99.3% accuracy on CIFAR10, 89% on CIFAR100, and 70.4% on ImageNet - the first unsupervised method to exceed 70% accuracy on ImageNet.

Conclusion: ICCE demonstrates that advanced clustering strategies can significantly narrow the performance gap between unsupervised and supervised image classification methods.
...
TL;DR: MLLMs lack human-like social reasoning to detect deception. A new MIDA task and dataset reveal models struggle with truth/falsehood distinction, showing poor grounding in social cues and inability to model others' mental states.
...
Result: Benchmark revealed significant performance gap - even powerful models like GPT-4o struggle to reliably distinguish truth from falsehood. Models fail to ground language in multimodal social cues and cannot model others' knowledge, beliefs, or intentions.

Conclusion: The proposed SoCoT and DSEM framework shows performance improvement, offering a promising path toward building MLLMs with genuine human-like social reasoning capabilities for more perceptive and trustworthy AI systems.
...
TL;DR: SwiTrack is a novel cross-modal object tracking framework that uses three specialized streams (RGB, NIR, and trajectory prediction) to handle modality switching and prevent object drift, achieving state-of-the-art performance with real-time tracking.


<details>
  <summary>Details</summary>
Motivation: Existing CMOT methods with parallel RGB-NIR branches fail to fully extract modality-specific features and struggle with object drift from unreliable inputs, especially when modalities switch.

Method: Uses three streams: RGB processed by visual encoder, NIR refined via gated adapter with visual encoder, and trajectory prediction module for invalid modalities. Also includes dynamic template reconstruction and similarity alignment loss.

Result: Achieves state-of-the-art performance with 7.2% precision and 4.3% success rate gains over existing methods, while maintaining real-time tracking at 65 FPS.

Conclusion: SwiTrack's state-switching framework with specialized streams effectively handles cross-modal tracking challenges, prevents object drift, and delivers superior performance with real-time efficiency.
...
TL;DR: Proposes a novel MLP-based method with Memory-Block for full-body tracking from sparse sensor inputs, achieving state-of-the-art performance with 72 FPS on mobile HMDs.
...
Method: Uses multi-layer perceptron (MLP) backbone enhanced with residual connections and a novel Memory-Block component that represents missing sensor data with trainable code-vectors, combined with sparse signals from previous time instances for temporal consistency. Formulated as multi-task learning.
...
Conclusion: The proposed method effectively addresses the challenge of full-body tracking from sparse inputs, providing both high accuracy and real-time performance suitable for immersive AR/VR applications.
...
TL;DR: TetraSDF is a precise analytic meshing framework for neural SDFs using ReLU MLPs with tetrahedral positional encoding, enabling exact mesh extraction without discretization error.
...
Method: Uses ReLU MLP composed with multi-resolution tetrahedral positional encoder, tracks ReLU linear regions within encoder-induced polyhedral complex, and applies fixed analytic input preconditioner to reduce directional bias.

Result: Matches or surpasses grid-based encoders in SDF reconstruction accuracy, produces highly self-consistent meshes faithful to learned isosurfaces, with practical runtime and memory efficiency.

Conclusion: TetraSDF provides a precise analytic meshing solution that overcomes limitations of previous approaches, enabling exact mesh extraction from neural SDFs with positional encoders.
...
TL;DR: Fast spatio-temporal scene understanding framework using Vision Gated Generative Transformers (VGGT) for efficient, near real-time assistive navigation applications.


<details>
  <summary>Details</summary>
Motivation: To enable efficient, close to real-time scene understanding for assistive navigation applications while overcoming VGGT's high memory demands.

Method: Process image flow with sliding window and align submaps; use VGGT tracking head to aggregate 2D semantic instance masks into 3D objects; store timestamps and instance-level identities for temporal consistency.
...
Conclusion: The framework successfully enables efficient spatio-temporal scene understanding with temporal consistency and change detection capabilities for assistive navigation.
...
TL;DR: Hybrid deep learning framework combining CNNs, Vision Transformers, and GNNs for robust weed detection with 99.33% accuracy, using GAN-based augmentation and self-supervised pre-training for limited data scenarios.
...
Method: Hybrid framework using CNNs for local features, Vision Transformers for global features, and GNNs for relational features, with GAN-based augmentation and self-supervised contrastive pre-training.
...
Conclusion: The framework provides robust, interpretable weed detection suitable for real-time edge deployment, reducing herbicide overuse and enabling scalable sustainable farming.
...
TL;DR: Texture3dgs is an optimized mobile GPU implementation of 3D Gaussian Splatting that achieves up to 4.1× sorting speedup and 1.7× overall reconstruction speedup while reducing memory usage by 1.6× through novel texture cache-aware sorting and memory optimizations.


<details>
  <summary>Details</summary>
Motivation: To enable efficient 3D scene reconstruction on mobile devices for benefits like data privacy, offline operation, and faster responses, while addressing the computational challenges of 3DGS on mobile GPU hardware.

Method: Developed a novel sorting algorithm optimized for 2D texture cache with optimized data movement and placement, plus improved variable layout design and other optimizations for the 3DGS pipeline.

Result: Achieved up to 4.1× speedup for sorting and 1.7× overall speedup for 3D scene reconstruction, while reducing memory usage by up to 1.6× compared to baseline implementations.

Conclusion: Texture3dgs demonstrates effective mobile GPU optimization for 3D Gaussian Splatting through texture cache-aware algorithms and memory optimizations, enabling efficient mobile 3D scene reconstruction.
...
TL;DR: Upsample Anything is a test-time optimization framework that upscales low-resolution features from Vision Foundation Models to high-resolution outputs using a learned anisotropic Gaussian kernel, achieving state-of-the-art performance without training.


<details>
  <summary>Details</summary>
Motivation: Vision Foundation Models produce downsampled representations (14x/16x) that limit pixel-level applications, and existing upsampling methods require dataset-specific retraining or heavy optimization, reducing scalability.

Method: Per-image optimization learns an anisotropic Gaussian kernel combining spatial and range cues, bridging Gaussian Splatting and Joint Bilateral Upsampling to create a universal, edge-aware operator.

Result: Achieves state-of-the-art performance on semantic segmentation, depth estimation, and upsampling tasks, running in ≈0.419s per 224x224 image with seamless transfer across architectures and modalities.

Conclusion: Upsample Anything provides an efficient, training-free solution for high-resolution reconstruction of features, depth, and probability maps, overcoming limitations of existing upsampling approaches.
...
Method: Extend Latent Dirichlet Allocation to embedding spaces, derive SAE objective as MAP estimator, and develop SAE-TM framework that trains SAEs to learn topic atoms interpretable as word distributions.

Result: SAE-TM produces more coherent topics than strong baselines while maintaining diversity, and enables thematic analysis of image datasets and tracking topic changes over time.
...
TL;DR: BioBench is a new ecology vision benchmark that addresses ImageNet-1K's limitations in predicting performance on scientific imagery, providing better evaluation for computer vision models in ecological applications.
...
Result: The benchmark provides new signal for computer vision in ecology and serves as a template for building reliable AI-for-science benchmarks in any domain, with ViT-L models evaluating in 6 hours on an A6000 GPU.

Conclusion: BioBench captures what ImageNet misses and offers a standardized approach for evaluating vision models on ecological tasks, with code and results publicly available.
...
TL;DR: NaTex is a native 3D texture generation framework that predicts texture color directly in 3D space, avoiding limitations of traditional multi-view diffusion approaches by treating texture as a dense color point cloud.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of MVD pipelines including difficulties with occluded regions, mesh-texture alignment, and cross-view consistency in both content and color intensity.

Method: Uses latent color diffusion with geometry-aware color point cloud VAE and multi-control diffusion transformer (DiT), trained from scratch with 3D data. Features native geometry control via positional embeddings and geometry latents for precise alignment.

Result: Demonstrates strong performance, significantly outperforming previous methods in texture coherence and alignment. Shows strong generalization capabilities for various downstream applications.

Conclusion: NaTex presents an effective native 3D texture generation approach that addresses fundamental limitations of existing methods and enables precise texture generation with strong generalization to various applications.
...
<details>
  <summary>Details</summary>
Motivation: Recent hybrid UIE approaches achieve strong performance but incur high computational costs, limiting their practicality in real-time scenarios on resource-limited platforms.

Method: Integrates three interpretable priors: adaptive white balance to correct color attenuation, wavelet-based enhancement block for multi-band decomposition, and gradient-aware module with Sobel operators to preserve edge structures.
...
TL;DR: ChangeDINO is a Siamese framework for building change detection that combines a lightweight backbone with frozen DINOv3 features, uses a spatial-spectral differential transformer decoder, and includes a learnable morphology module for boundary refinement.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods rely too much on change-map annotations and underuse semantic information in non-changing regions, limiting robustness under illumination variation, off-nadir views, and scarce labels.

Method: End-to-end multiscale Siamese framework with: 1) Lightweight backbone fused with frozen DINOv3 features, 2) Spatial-spectral differential transformer decoder using multi-scale absolute differences as change priors, 3) Learnable morphology module for boundary refinement.

Result: Outperforms recent state-of-the-art methods in IoU and F1 on four public benchmarks, with ablation studies confirming each component's effectiveness.

Conclusion: ChangeDINO provides robust building change detection by effectively leveraging semantic information and multi-scale features while suppressing irrelevant responses through its novel architecture.
...
<details>
  <summary>Details</summary>
Motivation: Existing FSR methods are limited by fixed up-sampling scales and sensitivity to input size variations, which restricts their practical applications.

Method: Uses 2D deep features, local relative coordinates, and up-sampling scale ratios to predict RGB values; incorporates local frequency estimation module to capture high-frequency textures and global coordinate modulation module to leverage facial structure priors.

Result: Demonstrates robustness over state-of-the-art methods across various input sizes and up-sampling scales in both quantitative and qualitative evaluations.

Conclusion: ARASFSR effectively addresses limitations of existing FSR methods by enabling arbitrary-scale super-resolution with improved texture preservation and facial structure adaptation.
...
TL;DR: Proposes teacher-student architecture with key frame selection for weakly supervised terrain classification in UAV remote sensing, achieving improved mIoU and temporal consistency using only 30% labeled data.
...
Method: Uses teacher-student architecture with key frame selection and updating algorithms for weakly supervised learning and temporal consistency knowledge distillation.

Result: Method achieves improved mIoU and temporal consistency using only 30% labeled data, enabling stable terrain object localization in aerial positioning tasks.
...
TL;DR: Real-time camera localization using pre-captured LiDAR point clouds via neural rendering to bridge synthetic-real domain gap, achieving drift-free metric-scale tracking.
...
Method: Render synthetic views from colored LiDAR point clouds to establish 2D-3D correspondences, using neural rendering to reduce domain gap between synthetic and real images for improved feature matching. Two variants: Online Render and Match, and Prebuild and Localize.
...
Conclusion: The method enables reliable, real-time camera localization without drift or scale ambiguity by leveraging accurate LiDAR point clouds and neural rendering for improved feature matching.
...
TL;DR: MOMNet is an alignment-free depth super-resolution framework that handles misaligned RGB-D data through multi-order matching and aggregation mechanisms.
...
Method: Proposes Multi-Order Matching Network (MOMNet) with multi-order matching (zero-order, first-order, second-order) to identify relevant RGB information, and multi-order aggregation with structure detectors for selective feature transfer.
...
Conclusion: MOMNet effectively addresses the misalignment problem in real-world RGB-D scenarios through adaptive multi-order matching and aggregation, providing robust depth super-resolution without requiring strict spatial alignment.
...
TL;DR: DetailSemNet is a new model for offline signature verification that focuses on local structure matching and fine-grained differences, outperforming previous methods and achieving state-of-the-art results with improved interpretability and generalization.


<details>
  <summary>Details</summary>
Motivation: Previous methods for offline signature verification rely on holistic features for pair comparisons, which may not capture fine-grained differences needed for robust verification. Transformer-based backbones can naturally obscure local details, adversely impacting performance.

Method: Proposes DetailSemNet with a Detail Semantics Integrator that uses feature disentanglement and re-entanglement to enhance intricate details while expanding discriminative semantics. The model emphasizes local structural matching between signature images.

Result: Consistently outperforms recent methods on leading benchmarks, achieving state-of-the-art results with clear margins. Demonstrates remarkable generalization capabilities in cross-dataset testing scenarios.

Conclusion: The emphasis on local structure matching improves performance, enhances interpretability, and provides strong generalization capabilities, making DetailSemNet highly suitable for real-world offline signature verification applications.
...
TL;DR: CAMS proposes a CLIP-based compositional zero-shot learning method that uses gated cross-attention and multi-space disentanglement to better separate attribute and object semantics for improved recognition of unseen compositions.
...
Method: Uses Gated Cross-Attention to extract fine-grained semantic features from CLIP's image encoder blocks via latent units while suppressing irrelevant information, followed by Multi-Space Disentanglement to separate attribute and object semantics.
...
### [69] [End-to-End Motion Capture from Rigid Body Markers with Geodesic Loss](https://arxiv.org/abs/2511.16418)
*Hai Lan,Zongyan Li,Jianmin Hu,Jialing Yang,Houde Dai*

Main category: cs.CV

TL;DR: Introduces Rigid Body Markers (RBMs) for sparse 6-DoF motion capture, enabling real-time SMPL pose estimation with geodesic loss that matches optimization methods but with 10x less computation.
...
Method: Uses sparse Rigid Body Markers providing unambiguous 6-DoF data, with deep learning regression model estimating SMPL parameters under geodesic loss, trained on synthesized AMASS data.
...
TL;DR: A geometry-guided method for self-supervised surround-view depth estimation that ensures cross-view consistency by projecting 3D points onto a shared cylinder and using spatial attention across images.


<details>
  <summary>Details</summary>
Motivation: Existing methods for surround-view depth estimation suffer from inconsistent depth estimates between overlapping images, limiting their practical utility for 360° 3D perception.

Method: Predict initial depth maps per image, project 3D points onto shared unit cylinder to establish cross-view neighborhood relations, then apply explicit spatial attention across images based on cylinder distances to refine depth maps.
...
Conclusion: The proposed geometry-guided method with cylinder projection and spatial attention effectively addresses cross-view consistency issues in surround-view depth estimation.
...
TL;DR: Graph-based segmentation models combining Vision Transformers with Graph Neural Networks improve surgical anatomy identification by enhancing spatial relationships and handling occlusions, achieving 7-8% mIoU improvement over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Accurate identification of hepatocystic anatomy is critical to preventing surgical complications during laparoscopic cholecystectomy. Deep learning models struggle with occlusions, long-range dependencies, and capturing fine-scale geometry of rare structures.

Method: Two segmentation models integrating Vision Transformer (ViT) feature encoders with Graph Neural Networks (GNNs): (1) static k-NN graph with GCNII for stable long-range information propagation, and (2) dynamic Differentiable Graph Generator (DGG) with Graph Attention Network (GAT) for adaptive topology learning.

Result: 7-8% improvement in Mean Intersection over Union (mIoU) and 6% improvement in Mean Dice (mDice) scores over state-of-the-art baselines. Produces anatomically coherent predictions, particularly on thin, rare and safety-critical structures.

Conclusion: Graph-based segmentation methods enhance both performance and anatomical consistency in surgical scene segmentation. Combining ViT-based global context with graph-based relational reasoning improves interpretability and reliability for safer laparoscopic and robot-assisted surgery.
...
TL;DR: LDAG uses language descriptions from LLMs instead of support images to provide unbiased meta guidance for few-shot segmentation, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing FSS methods rely on support images which suffer from intra-class variations and cannot provide accurate guidance for untrained classes. The key insight is that support should provide unbiased meta guidance rather than visual references.

Method: Proposes Language-Driven Attribute Generalization (LDAG) with two modules: Multi-attribute Enhancement (MaE) uses LLMs to generate multiple detailed attribute descriptions, and Multi-modal Attribute Alignment (MaA) enables cross-modal interaction between attribute texts and visual features.
...
Conclusion: Using language descriptions from LLMs as support strategy is more effective than visual references from support images for few-shot segmentation, providing unbiased meta guidance across both trained and untrained classes.
...
TL;DR: StreetView-Waste dataset addresses urban waste container monitoring with detection, tracking, and overflow segmentation tasks, featuring improved tracking heuristics and geometry-aware segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing litter detection datasets lack annotations for container tracking and are captured in static environments, limiting real-world utility for smart city waste management.

Method: Created StreetView-Waste dataset with three evaluation tasks, benchmarked state-of-the-art models, and proposed heuristic-based tracking and geometry-aware segmentation strategies.

Result: Fine-tuned detectors achieved reasonable container detection, but tracking struggled; proposed heuristics reduced counting error by 79.6%, and geometry-aware strategy improved segmentation mAP@0.5 by 27%.

Conclusion: StreetView-Waste provides a challenging benchmark for real-world urban waste management perception systems, demonstrating value of multimodal inputs and specialized strategies.
...
TL;DR: VLA-Pruner is a token pruning method for Vision-Language-Action models that addresses the limitations of existing VLM-specific pruning methods by considering both semantic understanding and action execution needs.


<details>
  <summary>Details</summary>
Motivation: Existing token pruning methods for VLMs focus only on semantic salience, ignoring VLA's dual-system nature of high-level semantic understanding and low-level action execution, leading to degraded performance in action generation tasks.

Method: VLA-Pruner uses a dual-level importance criterion: vision-language prefill attention for semantic relevance and action decode attention (estimated via temporal smoothing) for action importance. It adaptively selects tokens for both semantic understanding and action execution under computational constraints.

Result: VLA-Pruner achieves state-of-the-art performance across multiple VLA architectures and diverse robotic tasks.

Conclusion: The proposed dual-level token pruning approach effectively addresses the specific needs of VLA models by balancing semantic understanding and action execution requirements, enabling efficient real-time deployment.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of limited 3D training data by leveraging abundant 2D datasets, enabling 3D scene understanding without expensive 3D data collection or model fine-tuning.

Method: Creates omnidirectional visual representations of objects from multi-view 2D images using intermediate 3D reconstruction, inspired by Cubist painting techniques that show multiple viewpoints simultaneously.

Result: Outperforms previous 2D-based vision-language models on 3D VQA and 3D language grounding tasks, demonstrating effective 3D scene understanding from 2D inputs.

Conclusion: LLaVA³ provides an effective alternative to direct 3D training by leveraging multi-view 2D images and Cubist-inspired representations, enabling robust 3D scene understanding without fine-tuning.
...
### [76] [PUP 3D-GS: Principled Uncertainty Pruning for 3D Gaussian Splatting](https://arxiv.org/abs/2406.10219)
*Alex Hanson,Allen Tu,Vasu Singla,Mayuka Jayawardhana,Matthias Zwicker,Tom Goldstein*

Main category: cs.CV

TL;DR: Proposes PUP 3D-GS, a principled pruning method for 3D Gaussian Splatting that achieves 90% compression while maintaining visual quality and foreground details through sensitivity-based pruning and multi-round refinement.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting models use millions of Gaussians, causing high storage/memory requirements that limit deployment on resource-constrained devices. Current pruning methods suffer from visual degradation and loss of foreground details at high compression ratios.

Method: Uses a principled sensitivity pruning score based on second-order approximation of reconstruction error, plus a multi-round prune-refine pipeline that works with any pretrained 3D-GS model without changing training.

Result: After pruning 90% of Gaussians, achieves 3.56× faster rendering while retaining more foreground information and higher image quality metrics than existing methods on Mip-NeRF 360, Tanks & Temples, and Deep Blending datasets.

Conclusion: The proposed sensitivity pruning approach enables significantly higher compression ratios than existing methods while preserving visual fidelity and foreground details, making 3D-GS more viable for resource-constrained devices.
...
<details>
  <summary>Details</summary>
Motivation: Despite extensive research on corpus callosum segmentation, few publicly available tools provide comprehensive automated analysis pipelines for this important brain structure used in aging, neurological disease research, and clinical trials.

Method: FastSurfer-CC automatically identifies mid-sagittal slices, segments corpus callosum and fornix, localizes anterior/posterior commissures for head standardization, generates thickness profiles and subdivisions, and extracts eight shape metrics.

Result: The method outperforms existing specialized tools across individual tasks and reveals statistically significant differences between Huntington's disease patients and healthy controls not detected by current state-of-the-art approaches.

Conclusion: FastSurfer-CC provides an efficient, fully automated framework for comprehensive corpus callosum analysis that offers superior performance and enhanced sensitivity for detecting disease-related morphological changes.
...
TL;DR: This paper presents the first systematic study on occluded video prediction, showing that incorporating point-flow and depth-map information improves prediction performance in occluded scenarios and background motion.


<details>
  <summary>Details</summary>
Motivation: Occlusion remains a fundamental challenge in video prediction despite general models performing well in standard scenarios. The authors hypothesize that explicit motion (point-flow) and geometric structure (depth-maps) information can help models handle occlusion better.

Method: Used a standard multi-object latent transformer architecture for video prediction, modified to incorporate depth and point-flow information. Evaluated on both synthetic and real-world datasets using appearance-based metrics and Wasserstein distances on object masks.
...
Conclusion: Explicit motion and geometric structure information (point-flow and depth-maps) significantly improve video prediction performance in challenging occluded scenarios.
...
TL;DR: Physics-informed GAN framework synthesizes high-fidelity microscope images for microrobot pose estimation by integrating wave optics-based rendering and depth alignment, achieving near-real-data performance without expensive real data collection.
...
Method: Novel physics-informed deep generative learning framework that integrates wave optics-based physical rendering and depth alignment into a GAN to synthesize high-fidelity microscope images efficiently.

Result: Improves SSIM by 35.6% vs purely AI-driven methods with real-time rendering (0.022 s/frame). Pose estimator trained on synthetic data achieves 93.9%/91.9% (pitch/roll) accuracy, only 5.0%/5.4% below real-data training. Generalizes to unseen poses without additional training.

Conclusion: The framework enables robust microrobot pose estimation with synthetic data, reducing dependency on expensive real data collection while maintaining high accuracy and generalization capabilities.
...
TL;DR: A tumor segmentation method for breast DCE-MRI that uses acquisition time information via FiLM layers to improve segmentation accuracy and generalization across different protocols.
...
Conclusion: Using acquisition time information through FiLM layers enhances breast tumor segmentation in DCE-MRI, addressing challenges from protocol variations and improving model generalization across different datasets.
...
TL;DR: A novel method for jointly mapping indoor scenes and registering ceiling-mounted cameras using a mobile agent with RGB-D camera, enabling collaborative scene mapping and camera registration in a unified framework.


<details>
  <summary>Details</summary>
Motivation: Manual registration of ceiling-mounted cameras is inefficient and costly, while automatic visual localization may fail due to visual ambiguity. There's a need for a reliable solution that can handle both scene mapping and camera registration simultaneously.

Method: Use a mobile agent with head-mounted RGB-D camera to traverse the scene while ceiling cameras capture the agent. Correlate trajectories using timestamps, then use factor graph optimization to jointly optimize ego-camera poses, scene layout, and ceiling camera poses.

Result: The method effectively accomplishes both scene mapping and camera registration tasks within a unified framework, with joint optimization enhancing performance for both tasks. A new benchmark dataset was created for collaborative scene mapping and camera registration.

Conclusion: Provides a reliable tool for position-aware applications by enabling efficient joint mapping and registration of ceiling-mounted cameras in indoor environments.
...
TL;DR: A comprehensive boxing punch dataset with 6,915 manually annotated clips across 6 punch types, extracted from YouTube sparring sessions to support real-time action recognition research.


<details>
  <summary>Details</summary>
Motivation: To address the bottleneck in robust datasets for combat sports analysis due to dynamic, unstructured actions and varying recording environments, enabling better computer vision applications in boxing.

Method: Created dataset by extracting punch clips from 20 publicly available YouTube sparring sessions involving 18 athletes, with manual segmentation and labeling of 6 distinct punch types to ensure precise temporal boundaries and class consistency.

Result: Produced a well-annotated dataset capturing diverse motion styles, camera angles, and athlete physiques, specifically curated for real-time vision-based action recognition in low-resource environments.

Conclusion: This dataset aims to accelerate progress in movement analysis, automated coaching, and performance assessment within boxing and related combat sports domains by providing a rich benchmark with diverse punch examples.
...
TL;DR: SemCLIP improves CLIP's robustness to semantic transformations by using a new contrastive loss and LLM-generated training triples with original, paraphrased, and negated captions.


<details>
  <summary>Details</summary>
Motivation: CLIP struggles with negated or paraphrased text because negation changes meaning with minimal lexical changes while paraphrasing creates different expressions with same meaning, posing challenges for vision-language alignment.

Method: Proposes SemCLIP with new CLIP contrastive loss accounting for paraphrasing and negation, using LLM-generated training triples (original, paraphrased, negated captions) in CLIP-like training.
...
TL;DR: Robust multi-camera gymnast tracking using domain knowledge and cascaded data association for gymnastics judging applications.


<details>
  <summary>Details</summary>
Motivation: Track gymnasts in international competitions despite limited cameras and challenging conditions like lighting variations, occlusions, and detection failures in some views.

Method: Cascaded data association combining triangulation (when sufficient cross-view detections) and ray-plane intersection (when insufficient detections), leveraging domain knowledge that gymnasts move within predefined vertical planes.

Result: Superior performance over existing methods in challenging scenarios, successfully applied at Gymnastics World Championships with recognition from International Gymnastics Federation.

Conclusion: Incorporating gymnastics domain knowledge and flexible data association strategies enables robust tracking under limited camera setups and challenging conditions.
...
TL;DR: Analysis of local (Lucas-Kanade) and global (Horn-Schunck) optical flow methods, with implementation of a multiresolution Horn-Schunck version using bilinear interpolation and prolongation.


<details>
  <summary>Details</summary>
Motivation: To explore and compare the theoretical and practical aspects of local and global optical flow computation methods, particularly focusing on improving motion estimation accuracy under varying image conditions.

Method: Implemented a multiresolution version of the Horn-Schunck algorithm using bilinear interpolation and prolongation techniques, while also analyzing local approaches like Lucas-Kanade method.

Result: The study investigated the effectiveness of combined strategies (local and global methods with multiresolution approach) for motion estimation between frames.

Conclusion: Multiresolution Horn-Schunck with bilinear interpolation and prolongation improves optical flow computation accuracy and convergence in varying image conditions.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of detecting AI-generated images as new models emerge rapidly, making traditional detection approaches that require frequent retraining impractical.
...
Result: 91.3% average detection accuracy with only 150 images per class, 5.2% improvement over existing methods. For source attribution: 14.70% AUC and 4.27% OSCR improvements in open set classification.

Conclusion: The framework enables robust, scalable forensic attribution that adapts to evolving generative AI without exhaustive retraining, marking significant advancement in synthetic image detection.
...
TL;DR: EOGS++ extends 3D Gaussian Splatting for satellite imagery, operating directly on raw panchromatic data with embedded bundle adjustment via optical flow, achieving state-of-the-art reconstruction quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: To improve 3D Gaussian Splatting for Earth observation by eliminating preprocessing requirements, enhancing camera pose estimation, and achieving better geometric accuracy for satellite imagery.

Method: Extends EOGS framework with direct operation on raw high-resolution panchromatic data, embeds bundle adjustment using optical flow during training, and adds early stopping and TSDF post-processing.

Result: Achieves state-of-the-art performance on IARPA 2016 and DFC2019 datasets, with MAE errors on buildings improving from 1.33 to 1.19 compared to original EOGS, while maintaining computational efficiency.
...
TL;DR: VARiant introduces a novel approach to reduce memory overhead in Visual Auto-Regressive models by leveraging scale-depth asymmetric dependency, enabling flexible depth adjustment through weight-shared subnets and achieving significant memory reduction with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Progressive multi-scale generation in VAR models incurs substantial memory overhead due to cumulative KV caching, limiting practical deployment despite their inference efficiency.

Method: Proposed VARiant with equidistant sampling to select multiple subnets from the original 30-layer network, using full network for early scales and subnets for later scales with weight sharing, plus progressive training strategy to resolve optimization conflicts.

Result: VARiant-d16 and VARiant-d8 achieve nearly equivalent quality (FID 2.05/2.12) to VAR-d30 (FID 1.95) while reducing memory by 40-65%. VARiant-d2 achieves 3.5x speedup and 80% memory reduction at moderate quality cost (FID 2.97).

Conclusion: VARiant's single-model architecture enables zero-cost runtime depth switching and provides flexible deployment options from high quality to extreme efficiency, catering to diverse application scenarios.
...
Method: Designed compact yet expressive backbone, hybrid cost aggregation module, and three-stage training strategy on million-scale data to bridge sim-to-real gap.

Result: Achieves state-of-the-art zero-shot performance across four real-world benchmarks, with accuracy comparable to or exceeding non-prior-based methods while using <1% computational cost.
...
TL;DR: NutriScreener is a retrieval-augmented multi-pose graph attention network that uses CLIP-based visual embeddings and context awareness for robust malnutrition detection and anthropometric prediction from children's images, achieving high accuracy and efficiency in clinical settings.
...
Method: Uses retrieval-augmented multi-pose graph attention network combining CLIP-based visual embeddings, class-boosted knowledge retrieval, and context awareness for malnutrition detection and anthropometric prediction from children's images.

Result: Achieves 0.79 recall, 0.82 AUC, significantly lower anthropometric RMSEs, and cross-dataset results show up to 25% recall gain and up to 3.5 cm RMSE reduction. Doctors rated it 4.3/5 for accuracy and 4.6/5 for efficiency.

Conclusion: NutriScreener offers a scalable and accurate solution for early malnutrition detection in low-resource environments, demonstrating deployment readiness and reliable measurement in unconstrained pediatric settings.
...
TL;DR: POMA-3D is the first self-supervised 3D representation model that learns from point maps, enabling transfer of 2D foundation model priors to 3D understanding without requiring RGB data.


<details>
  <summary>Details</summary>
Motivation: Addresses the scarcity of pretrained priors and limited data in 3D representation learning by leveraging point maps that preserve 3D geometry while being compatible with 2D foundation models.

Method: Uses point maps on structured 2D grids with view-to-scene alignment and POMA-JEPA architecture for geometrically consistent features across multiple views. Trained on ScenePoint dataset from 6.5K room-level RGB-D scenes and 1M 2D image scenes.

Result: POMA-3D serves as strong backbone for diverse 3D tasks including 3D question answering, embodied navigation, scene retrieval, and embodied localization using only geometric inputs (3D coordinates).

Conclusion: POMA-3D successfully explores point maps as an effective approach for 3D scene understanding, bridging 2D foundation models with 3D geometric representation learning.
...
<details>
  <summary>Details</summary>
Motivation: Selective knowledge removal from medical segmentation networks is important for privacy compliance, ethical deployment, and continual dataset revision, addressing the need for targeted forgetting without full retraining.

Method: Teacher-student distillation paradigm with Low-Rank Adaptation (LoRA) constrained subspace updates, featuring strong unlearning phase with adversarial optimization to contradict teacher predictions on forget subset, followed by gentle restoration phase with head-only supervised refinement.

Result: For ISIC segmentation: forget-set IoU reduced from 0.875 to 0.509 while maintaining performance on retain/validation (0.647 to 0.677 IoU). For ISIC classification: forget accuracy decreased from 87.0% to 64.1% while retain accuracy improved from 83.9% to 90.6%. Cross-domain CHASE dataset shows consistent performance.

Conclusion: LoRA-based subspace unlearning provides a practical pathway for responsible, controllable, and reversible unlearning in medical image analysis, enabling models to forget sensitive samples while preserving performance where it matters most.
...
### [93] [Solving Spatial Supersensing Without Spatial Supersensing](https://arxiv.org/abs/2511.16655)
*Vishaal Udandarao,Shyamgopal Karthik,Surabhi S. Nath,Andreas Hochlehnert,Matthias Bethge,Ameya Prabhu*

Main category: cs.CV

TL;DR: The paper critically analyzes Cambrian-S's spatial supersensing benchmarks and methods, showing they can be solved without spatial cognition through simple baselines and are vulnerable to shortcut exploitation.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether Cambrian-S's benchmarks and methods genuinely measure spatial supersensing capabilities in video world models, or if they can be solved through simpler approaches and shortcut exploitation.

Method: Introduces NoSense baseline (bag-of-words SigLIP model without temporal structure) and VSC-Repeat sanity check (concatenating videos with themselves) to test benchmark robustness and method reliability.

Result: NoSense achieves 95% accuracy on VSR benchmark without spatial cognition. VSC-Repeat collapses Cambrian-S's accuracy from 42% to 0%, revealing reliance on benchmark shortcuts rather than robust spatial supersensing.

Conclusion: Current VSI-Super benchmarks don't reliably measure spatial supersensing, and Cambrian-S's inference methods improve performance by exploiting shortcuts rather than demonstrating genuine spatial supersensing capabilities.
...
TL;DR: TimeViper is a hybrid Mamba-Transformer model for long video understanding that introduces TransV to compress vision tokens into instruction tokens, enabling processing of hour-long videos with 10,000+ frames while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Long video understanding requires efficient architectures and effective temporal context handling. The paper addresses vision token redundancy in multimodal models where information flows from vision to text tokens across LLM depth.

Method: Hybrid Mamba-Transformer backbone combining state-space model efficiency with attention expressivity. Introduces TransV module to transfer and compress vision tokens into instruction tokens while preserving multimodal understanding.

Result: TimeViper processes hour-long videos exceeding 10,000 frames and competes with state-of-the-art models across multiple benchmarks while extending frame processing capabilities. Attention behavior analysis provides insights into hybrid model interpretability.

Conclusion: This work represents an initial step in developing, interpreting, and compressing hybrid Mamba-Transformer architectures for long video understanding, demonstrating effective token compression without sacrificing multimodal capabilities.
...
TL;DR: Dataset distillation method that optimizes synthetic images to match linear classifier gradients from real data when using pre-trained vision models, enabling efficient training of linear probes.
...
Conclusion: The method provides efficient dataset distillation for pre-trained models, enabling cross-model generalization and offering tools for analyzing model representations and biases.
...
TL;DR: Using generative AI to create synthetic smoke datasets for wildfire detection, with domain adaptation and style transfer to bridge synthetic-real data gap.


<details>
  <summary>Details</summary>
Motivation: Early wildfire detection is crucial but limited by scarce annotated smoke datasets, hindering deep learning model performance.
...
Result: Developed comprehensive annotated smoke dataset and improved domain adaptation methods for better synthetic-to-real data alignment.

Conclusion: Generative AI and domain adaptation techniques enable more accurate and scalable wildfire detection by overcoming data scarcity limitations.
...
TL;DR: This paper addresses long-tailed object detection using LVISv1 dataset, achieving state-of-the-art performance with enhanced Balanced Group Softmax and metric learning approaches.


<details>
  <summary>Details</summary>
Motivation: Real-world object detection faces class imbalance issues where rare categories have few instances, causing models to bias towards frequent classes and degrade performance on rare categories.

Method: Two-stage Faster R-CNN architecture with enhanced Balanced Group Softmax framework, plus metric learning for feature embeddings and k-NN for inference to improve rare class classification.

Result: Achieved new state-of-the-art performance with 24.5% mAP on LVISv1 dataset, surpassing previous benchmark of 24.0%.

Conclusion: The proposed methods effectively mitigate class imbalance in long-tailed object detection and demonstrate significant performance improvements, especially for rare categories.
...
TL;DR: SAM 3D is a generative model for 3D object reconstruction from single images, handling geometry, texture, and layout, with strong performance on natural images with occlusion and scene clutter.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of 3D object reconstruction from single images in real-world scenarios where occlusion, scene clutter, and contextual cues are common, overcoming the limited availability of 3D training data.
...
Conclusion: SAM 3D successfully enables high-quality 3D reconstruction from single images in challenging real-world conditions, with plans to release code, models, demo, and a new benchmark for in-the-wild 3D reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: Existing survival analysis methods lack transparency for clinical adoption, and recent pathology agents face limitations in multimodal data integration, region-of-interest exploration, and experiential learning from historical cases for survival prediction.
...
Result: Outperforms state-of-the-art baselines (EPG, EPSD) at high sparsity levels and offers more efficient alternative to iterative pruning schemes like COLT across multiple image classification benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian diffusion models are slow due to massive Gaussian primitives requiring intensive denoising and post-processing, limiting scalability and generation speed.

Method: Proposes TRIM with temporal trimming (lightweight selector for early trajectory reduction) and spatial trimming (instance mask denoising to prune redundant background Gaussians).
...
<details>
  <summary>Details</summary>
Motivation: Previous 3DHS methods overlook two key challenges: multi-hierarchy conflicts in cross-hierarchy optimization and class imbalance across multiple hierarchies that dominates model performance.
...
Conclusion: The proposed late-decoupled framework effectively mitigates multi-hierarchy conflicts and class imbalance problems in 3D hierarchical semantic segmentation.
...
TL;DR: TwiG is the first interleaved framework that enables co-evolving textual reasoning throughout visual generation, producing more context-aware outputs through dynamic interplay between reasoning and generation.


<details>
  <summary>Details</summary>
Motivation: Current visual generation methods incorporate reasoning either before or after generation, but lack on-the-fly multimodal interaction during the generation process itself.

Method: Three strategies: zero-shot prompting, supervised fine-tuning on TwiG-50K dataset, and reinforcement learning via customized TwiG-GRPO strategy for interleaved reasoning during generation.

Result: The framework enables dynamic interplay between reasoning and generation, producing more context-aware and semantically rich visual outputs.

Conclusion: This work inspires further research into interleaving textual reasoning for enhanced visual generation, with code to be released publicly.
...
TL;DR: PartUV is a part-based UV unwrapping pipeline that generates fewer, part-aligned charts with low distortion for AI-generated meshes, outperforming existing methods in chart count and seam length.


<details>
  <summary>Details</summary>
Motivation: Existing UV unwrapping methods struggle with AI-generated meshes that are noisy, bumpy, and poorly conditioned, often producing fragmented charts and suboptimal boundaries that introduce artifacts.

Method: Combines semantic part decomposition from PartField with geometric heuristics in a top-down recursive framework, ensuring distortion thresholds while minimizing chart count, with parallelization and handling of non-manifold meshes.

Result: Outperforms existing tools and neural methods across four datasets in chart count and seam length, achieves comparable distortion, high success rates on challenging meshes, and enables part-specific multi-tiles packing.

Conclusion: PartUV provides an effective solution for UV unwrapping challenging AI-generated meshes by leveraging part-based decomposition and geometric optimization, enabling better downstream applications.
...
TL;DR: TriDiff-4D is a diffusion-based 4D avatar generation pipeline that creates high-quality, temporally coherent 4D avatars from text using triplane re-posing and auto-regressive generation.
...
Conclusion: TriDiff-4D enables efficient skeleton-driven 4D generation with superior temporal consistency, motion accuracy, computational efficiency, and visual fidelity compared to existing approaches.
...
TL;DR: SceneDesigner enables precise 9D pose control (location, size, orientation) for multiple objects in image generation, addressing limitations of existing methods through a branched network architecture, CNOCS map representation, and specialized training strategies.


<details>
  <summary>Details</summary>
Motivation: Existing methods for controllable image generation lack comprehensive control over 9D poses of multiple objects simultaneously, suffering from limited controllability and degraded quality when handling complex multi-object scenes.

Method: Proposes SceneDesigner with branched network architecture, CNOCS map representation for 9D pose encoding, two-stage training with reinforcement learning to address data imbalance, and Disentangled Object Sampling for inference.

Result: Significantly outperforms existing approaches in both controllability and quality, enabling accurate multi-object 9D pose manipulation while maintaining high image quality.

Conclusion: SceneDesigner provides an effective solution for comprehensive multi-object 9D pose control in image generation, with strong geometric interpretation properties and flexible customization capabilities.
...
TL;DR: V-ReasonBench is a new benchmark for evaluating video reasoning across structured problem-solving, spatial cognition, pattern-based inference, and physical dynamics, revealing performance gaps in current video models.
...
Method: Built from synthetic and real-world image sequences, providing diverse answer-verifiable tasks that are reproducible, scalable, and unambiguous across four reasoning dimensions.

Result: Evaluations of six state-of-the-art video models show clear dimension-wise performance differences and strong variation in reasoning abilities. Video models were compared with image models, hallucination behaviors analyzed, and video duration effects on Chain-of-Frames reasoning studied.

Conclusion: V-ReasonBench provides a unified, reproducible framework for measuring video reasoning to support development of models with more reliable, human-aligned reasoning skills.
...
TL;DR: VANS introduces Video-Next-Event Prediction (VNEP) as a new task requiring video responses instead of text, and proposes a reinforcement learning approach to align Vision-Language and Video Diffusion Models for generating consistent video predictions.


<details>
  <summary>Details</summary>
Motivation: Video's ability to demonstrate physical-world information that's hard to convey through text alone, particularly for procedural learning and creative exploration where showing is more intuitive than telling.
...
Result: VANS achieves state-of-the-art performance on procedural and predictive benchmarks for both video event prediction and visualization, demonstrating the effectiveness of the proposed approach.

Conclusion: The VNEP task represents a significant shift from text-based to video-based next-event prediction, and VANS successfully addresses the challenges through reinforcement learning alignment of multimodal models.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLMs pursue lengthy reasoning chains for all tasks, leading to excessive computational costs, unlike human cognitive resource allocation where simple tasks get quick thinking and complex tasks get deeper analysis.
...
Conclusion: The proposed simple RL approach successfully enables VLMs to mimic human-like cognitive resource allocation, balancing performance and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To enable autonomous improvement of large multimodal models' reasoning abilities without relying on human-curated data or external reward models, addressing limitations in scalability and autonomy of current training approaches.

Method: Proposes a self-evolving framework with two agents from a single backbone model: a Proposer that generates diverse image-grounded questions, and a Solver that solves them through internal consistency, creating a continuous self-rewarding learning process.

Result: Achieves consistent gains of up to ~3% on multimodal math-reasoning benchmarks (ChartQA, MathVista, MathVision) using only raw training images, with Qwen2.5-VL as the base model.

Conclusion: EvoLMM provides a simple yet effective baseline for fully unsupervised self-improvement of large multimodal models, demonstrating the potential for autonomous reasoning capability enhancement without external supervision.
...
TL;DR: NoPo-Avatar reconstructs animatable 3D human avatars from single/sparse images without requiring human pose inputs, overcoming limitations of pose-dependent methods that degrade with noisy pose estimates.


<details>
  <summary>Details</summary>
Motivation: Prior methods rely on accurate ground-truth camera poses and human poses as input, but reconstruction quality degrades significantly when pose estimates are noisy, limiting practical applicability.

Method: NoPo-Avatar reconstructs avatars solely from images without any pose input, removing dependence on test-time human pose estimation to avoid degradation from noisy poses.
...
Conclusion: By eliminating pose dependence, NoPo-Avatar provides more robust and widely applicable 3D avatar reconstruction that is not affected by noisy human pose estimates.
...
### [112] [UniUltra: Interactive Parameter-Efficient SAM2 for Universal Ultrasound Segmentation](https://arxiv.org/abs/2511.15771)
...
TL;DR: UniUltra adapts SAM2 for universal ultrasound segmentation using parameter-efficient fine-tuning and knowledge distillation to overcome domain disparities and enable clinical deployment.


<details>
  <summary>Details</summary>
Motivation: SAM2 performs poorly on ultrasound images due to domain gaps, requiring efficient adaptation methods that maintain parameter efficiency and enable deployment in resource-constrained clinical settings.

Method: Proposes context-edge hybrid adapter (CH-Adapter) for fine-grained perception across ultrasound modalities, and deep-supervised knowledge distillation (DSKD) to transfer knowledge from large encoder to lightweight encoder.
...
Conclusion: UniUltra enables efficient adaptation of SAM2 for ultrasound segmentation while significantly reducing computational requirements, making it highly suitable for practical clinical deployment.
...
### [113] [Weakly Supervised Segmentation and Classification of Alpha-Synuclein Aggregates in Brightfield Midbrain Images](https://arxiv.org/abs/2511.16268)
*Erwan Dereure,Robin Louiset,Laura Parkkinen,David A Menassa,David Holcman*

Main category: eess.IV

TL;DR: Automated pipeline using deep learning to segment and classify alpha-synuclein aggregates in Parkinson's disease histopathology images, achieving 80% balanced accuracy for identifying Lewy bodies and neurites.


<details>
  <summary>Details</summary>
Motivation: To better understand the spatial organization of alpha-synuclein aggregates in Parkinson's disease through automated analysis of immunohistochemistry images, overcoming challenges of immunohistochemical labeling variability.

Method: Developed an automated image processing pipeline using weakly supervised segmentation and ResNet50 classifier to analyze whole-slide images of midbrain tissue from PD and incidental Lewy Body Disease cases.
...
Conclusion: This framework enables large-scale characterization of alpha-synuclein aggregate spatial distribution and heterogeneity, and investigation of their relationships with surrounding cells like microglia and astrocytes.
...
### [114] [Rapid and Accurate Changepoint Detection of Power System Forced Oscillations](https://arxiv.org/abs/2511.15812)
*Luke Dosiek,Akaash Karn,Frank Liu*

Main category: eess.SP

TL;DR: A new changepoint detection method for estimating forced oscillation start/stop times in power systems that uses manual penalty parameter tuning to reduce computation time by 98% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing changepoint detection methods for forced oscillation analysis require automatic penalty parameter tuning, which is computationally expensive. The paper aims to develop a more efficient approach that reduces computation time while maintaining accuracy.

Method: Uses pruned exact linear time (PELT) algorithm with manual penalty parameter tuning instead of automatic tuning. Also provides a data-driven approach for setting minimum forced oscillation segment length and tests using a low-order ARMAX representation of the minniWECC model.

Result: Achieved 98% reduction in computation time compared to previous methods while maintaining high estimation accuracy for forced oscillation start and stop times.

Conclusion: The manual penalty parameter tuning approach for changepoint detection significantly improves computational efficiency for forced oscillation analysis without sacrificing accuracy, making it more practical for real-time power system monitoring.
...
### [115] [EEG Emotion Recognition Through Deep Learning](https://arxiv.org/abs/2511.15902)
*Roman Dolgopolyi,Antonis Chatzipanagiotou*

Main category: eess.SP

TL;DR: CNN-Transformer model achieves 91% accuracy in classifying positive, neutral, and negative emotions from EEG signals using only 5 electrodes, enabling affordable consumer-grade deployment.


<details>
  <summary>Details</summary>
Motivation: To enable accessible at-home emotion monitoring and address underresearched areas like media-induced mood changes, particularly in clinical settings where traditional behavioral cues are limited.

Method: Combined CNN-Transformer architecture trained on merged SEED datasets (1,455 samples) using only 5 out of 62 EEG electrodes for reduced hardware requirements.

Result: 91% testing accuracy outperforming SVM, DNN, and Logistic Regression; demonstrated feasibility of consumer-grade EEG headsets with reduced computational power.

Conclusion: Sets groundwork for continuous passive emotional monitoring in medical, wellness, and home-health platforms, potentially transforming mental health diagnostics and interventions.
...
### [116] [Integrated Coexistence for Satellite and Terrestrial Networks with Multistatic ISAC](https://arxiv.org/abs/2511.15947)
*Jeongju Jee,Jeffrey G. Andrews*

Main category: eess.SP

TL;DR: A cooperation framework for integrated coexistence between LEO satellite communications and terrestrial ISAC networks, addressing interference through practical CSI utilization and co-design of beamforming and power allocation.


<details>
  <summary>Details</summary>
Motivation: Spectrum sharing between satellite and terrestrial networks in 6G era may cause severe interference, requiring effective coexistence solutions.

Method: Proposes a practical two-stage structure (pre-optimization and refinement) leveraging predictable satellite CSI, co-designs terrestrial beamforming and satellite power allocation using weighted minimum mean-squared error algorithm, and introduces target-radar association for multistatic ISAC.

Result: Simulation results show significant performance enhancement for integrated networks, with overall performance approaching interference-free benchmark as spot beams and radar receivers increase.

Conclusion: The approach demonstrates feasibility of spectral coexistence between satellite and terrestrial networks through practical cooperation framework.
...
### [117] [Neural Positioning Without External Reference](https://arxiv.org/abs/2511.16352)
*Till-Yannic Müller,Frederik Zumegen,Reinhard Wiesmayr,Emre Gönültaş,Christoph Studer*

Main category: eess.SP

TL;DR: Proposes a neural positioning pipeline that trains positioning networks using only CSI measurements and relative displacement commands from COTS robots, eliminating need for external reference positioning systems.


<details>
  <summary>Details</summary>
Motivation: Existing neural positioning methods require costly ground-truth position labels from external reference systems, making label acquisition difficult and expensive for large areas.

Method: Trains positioning network using CSI acquired off-device and relative displacement commands from commercial robot platforms (e.g., robotic vacuum cleaners) without external position labels.

Result: Achieves UE localization accuracies close to state-of-the-art methods that require externally acquired high-precision ground-truth position labels, validated in real-world LoS and NLoS scenarios using Wi-Fi and 5G NR systems.

Conclusion: Enables inexpensive training of accurate neural positioning functions over large areas without costly external reference positioning systems.
...
### [118] [Joint Admission Control and Power Minimization in IRS-assisted Networks](https://arxiv.org/abs/2511.16000)
*Weijie Xiong,Jingran Lin,Zhiling Xiao,Qiang Li,Yuhan Zhang*

Main category: eess.SP

TL;DR: Proposes sigmoid-based l0-norm approximation and PDD algorithm for joint admission control and power minimization in IRS-assisted networks, reducing complexity and improving performance.


<details>
  <summary>Details</summary>
Motivation: Traditional methods using l1-norm approximations and alternating optimization suffer from high computational complexity and lack robust convergence guarantees.

Method: Sigmoid-based approximation of l0-norm AC indicator combined with penalty dual decomposition (PDD) algorithm to jointly optimize beamforming and admission control.

Result: Achieves lower power consumption, accommodates more users, reduces computational time, and ensures convergence to stationary solution.

Conclusion: The proposed approach outperforms existing methods by providing more efficient and tractable solution with distributed implementation capability.
...
### [119] [UT-OSANet: A Multimodal Deep Learning model for Evaluating and Classifying Obstructive Sleep Apnea](https://arxiv.org/abs/2511.16169)
*Zijian Wang,Xiaoyu Bao,Chenhao Zhao,Jihui Zhang,Sizhi Ai,Yuanqing Li*

Main category: eess.SP

TL;DR: UT OSANet is a deep learning model for high-resolution, event-level diagnosis of obstructive sleep apnea, capable of detecting apnea, hypopnea, oxygen desaturation, and arousal events across multiple scenarios with flexible input modalities.


<details>
  <summary>Details</summary>
Motivation: Existing OSA diagnostic approaches lack precision for event-level diagnosis and cannot provide comprehensive, high-resolution detection of various respiratory events associated with sleep apnea.

Method: Deep learning model using random masked modality combination training with flexible input modalities (EEG, airflow, SpO2) trained on 9,021 polysomnography recordings from five independent datasets.

Result: Achieved sensitivities up to 0.93 and macro F1 scores of 0.84-0.85 across home, clinical, and research scenarios, demonstrating consistent performance across varying modality conditions.

Conclusion: UT OSANet serves as an effective event-level, multi-scenario diagnostic tool for real-world OSA applications and provides a means to better understand respiratory processes in sleep disorders.
...
### [120] [Low-Complexity Rydberg Array Reuse: Modeling and Receiver Design for Sparse Channels](https://arxiv.org/abs/2511.16260)
*Hao Wu,Shanchi Wu,Xinyuan Yao,Rui Ni,Chen Gong*

Main category: eess.SP

TL;DR: This paper proposes a multiplexed Rydberg array architecture inspired by hybrid analog-digital beamforming to overcome the bulkiness of current stacked single-antenna designs in quantum RF receivers.


<details>
  <summary>Details</summary>
Motivation: Current Rydberg array antennas rely on simple stacking of single-antenna units, leading to substantial system bulkiness due to multiple laser setups, making them impractical for real-world applications and challenging to fabricate.

Method: Drawing inspiration from hybrid analog-digital beamforming in conventional RF arrays, the authors conduct a systematic study on design principles, equivalent modeling, and precoding strategies for low-complexity multiplexed Rydberg arrays.

Result: The paper develops a framework for multiplexed Rydberg sensor array architectures that reduces hardware complexity while maintaining performance, addressing the scalability limitations of current designs.

Conclusion: This work is crucial for enabling practical and scalable quantum-enhanced communication systems by overcoming the bulkiness and fabrication challenges of existing Rydberg array implementations.
...
### [121] [Dynamic Multiple-Parameter Joint Time-Vertex Fractional Fourier Transform and its Intelligent Filtering Methods](https://arxiv.org/abs/2511.16277)
*Manjun Cui,Ziqi Yan,Yangfan He,Zhichao Zhang*

Main category: eess.SP

TL;DR: Proposes a dynamic multiple-parameter joint time-vertex fractional Fourier transform (DMPJFRFT) with time-varying fractional parameters for adaptive spectral modeling of dynamic graph signals, outperforming existing methods in denoising and deblurring tasks.


<details>
  <summary>Details</summary>
Motivation: Existing joint time-vertex transforms use fixed fractional orders for spatial and temporal domains, limiting their ability to model complex, continuously varying dynamics of graph signals.

Method: Developed DMPJFRFT framework with time-varying fractional parameters, analyzed its theoretical properties, and created two filtering approaches: gradient descent-based and neural network-based methods for dynamic signal restoration.

Result: Experimental results on dynamic graph and video datasets show the framework effectively captures temporal topology variations and achieves superior performance in denoising and deblurring compared to state-of-the-art methods.

Conclusion: The proposed DMPJFRFT framework enables dynamic and flexible representation of spatio-temporal signal evolution, providing adaptive spectral modeling for dynamic graph structures with improved restoration performance.
...
### [122] [Revealing computation-communication trade-off in Segmented Pinching Antenna System (PASS)](https://arxiv.org/abs/2511.16327)
*Deqiao Gan,Xiaoxia Xu,Xiaohu Ge,Yuanwei Liu*

Main category: eess.SP

TL;DR: Proposes a joint communication and computation framework using segmented pinching antenna system (PASS) with three protocols (SS, SA, SM) that simultaneously transmits communication and computation data via uplink, achieving significant MSE reduction and WSR improvement over conventional systems.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of large-scale path loss and in-waveguide loss in wireless systems while enabling simultaneous transmission of communication bit streams and computation data via uplink communications.

Method: Uses segmented PASS design with three operating protocols (SS, SA, SM). Develops AO-MMSE algorithm for MSE minimization in computation-oriented cases and AO-WMMSE algorithm for WSR maximization in communication-oriented cases, both using alternating optimization with closed-form solutions.

Result: Achieves up to 70.65% and 45.32% reductions in MSE compared to conventional MIMO and PASS, and 87.70% and 51.35% improvements in WSR compared to conventional MIMO and PASS respectively.

Conclusion: The segmented JCC-PASS framework significantly outperforms conventional systems in both computation and communication performance metrics, providing an effective solution for joint communication and computation applications.
...
### [123] [VersaPants: A Loose-Fitting Textile Capacitive Sensing System for Lower-Body Motion Capture](https://arxiv.org/abs/2511.16346)
*Deniz Kasap,Taraneh Aminosharieh Najafi,Jérôme Paul Rémy Thevenot,Jonathan Dan,Stefano Albini,David Atienza*

Main category: eess.SP

TL;DR: VersaPants is a textile-based capacitive sensing system for lower-body motion capture using conductive patches and a Transformer model, achieving real-time performance on edge devices with minimal computational requirements.


<details>
  <summary>Details</summary>
Motivation: To create a comfortable, privacy-preserving motion capture system that doesn't require user-specific fitting adjustments like IMU-based systems or compromise privacy like camera-based methods.

Method: Custom smart garment with 6 capacitive channels per leg, using conductive textile patches and a lightweight Transformer-based deep learning model to map capacitance signals to joint angles for embedded implementation.

Result: Achieved MPJPE of 11.96 cm and MPJAE of 12.3 degrees across hip, knee, and ankle joints, with 22x fewer parameters and 18x fewer FLOPs than existing methods, enabling 42 FPS real-time inference on commercial smartwatch.

Conclusion: VersaPants represents a scalable, comfortable embedded motion-capture solution suitable for fitness, healthcare, and wellbeing applications with competitive performance and minimal computational requirements.
...
### [124] [Reasoning Meets Representation: Envisioning Neuro-Symbolic Wireless Foundation Models](https://arxiv.org/abs/2511.16369)
*Jaron Fontaine,Mohammad Cheraghinia,John Strassner,Adnan Shahid,Eli De Poorter*

Main category: eess.SP

TL;DR: The paper proposes a neuro-symbolic framework combining neural networks with symbolic reasoning to address limitations in Wireless Physical Layer Foundation Models and enable trustworthy AI for 6G networks.


<details>
  <summary>Details</summary>
Motivation: Current WPFMs lack explainability, robustness, adaptability, and regulatory compliance verification. The vision for AI-native 6G requires deeply embedded, trustworthy intelligence that existing deep learning approaches cannot provide.

Method: A neuro-symbolic framework integrating universal RF embeddings with symbolic knowledge graphs and differentiable logic layers, combining data-driven learning with rule-based reasoning.

Result: The proposed approach enables models to learn from large datasets while reasoning over explicit domain knowledge, creating a hybrid system that bridges neural and symbolic AI.

Conclusion: Neuro-symbolic AI is essential for developing trustworthy, generalizable, and efficient wireless AI that can meet the demands of future 6G networks by addressing critical limitations of pure data-driven approaches.
...
### [125] [3-20 GHz Wideband Tightly-Coupled Dual-Polarized Vivaldi Antenna Array](https://arxiv.org/abs/2511.16472)
*Niko Lindvall,Mikko Heino,Mikko Valkama*

Main category: eess.SP

TL;DR: A novel tightly-coupled dual-polarized antipodal Vivaldi antenna (TC-AVA) achieves 3-20 GHz bandwidth with improved lower band edge through overlapping Vivaldi leaves.


<details>
  <summary>Details</summary>
Motivation: Very wideband apertures are needed for positioning, sensing, spectrum monitoring, and frequency hopping systems. Vivaldi antennas are suitable but their lower band edge can be improved using tight coupling techniques previously demonstrated with dipole elements but not with dual-polarized Vivaldi antennas.

Method: The paper presents a tightly-coupled dual-polarized antipodal Vivaldi antenna (TC-AVA) design where tight coupling is achieved by overlapping the Vivaldi leaves between adjacent elements.

Result: The TC-AVA achieves -6 dB impedance bandwidth of 3 to 20 GHz. The tight coupling extends the lower band edge from 3.75 GHz to 3 GHz and 2.75 GHz for both polarizations, representing 20% to 25% improvement compared to an isolated antipodal Vivaldi element.

Conclusion: Tight coupling through overlapping Vivaldi leaves successfully extends the lower band edge of dual-polarized Vivaldi antennas, demonstrating significant bandwidth improvement for wideband applications.
...
### [126] [TFCDiff: Robust ECG Denoising via Time-Frequency Complementary Diffusion](https://arxiv.org/abs/2511.16627)
*Pengxin Li,Yimin Zhou,Jie Min,Yirong Wang,Wei Liang,Wang Li*

Main category: eess.SP

TL;DR: TFCDiff is a novel ECG denoising method that uses time-frequency complementary diffusion in the DCT domain with temporal feature enhancement to remove mixed noise from ambulatory ECG signals, achieving state-of-the-art performance and superior generalization.


<details>
  <summary>Details</summary>
Motivation: Ambulatory ECG readings are prone to mixed noise from physical activities (baseline wander, muscle artifact, electrode motion artifact), which affects diagnostic accuracy. Current methods struggle with denoising multi-beat ECG segments and complex noise patterns.

Method: Proposes Time-Frequency Complementary Diffusion (TFCDiff) operating in DCT domain, using DCT coefficients of noisy signals as conditioning input. Incorporates Temporal Feature Enhancement Mechanism (TFEM) to reinforce temporal representations and preserve physiological information.

Result: Achieves state-of-the-art performance across five evaluation metrics on synthesized dataset. Shows superior generalization on unseen SimEMG Database, outperforming all benchmark models. Processes raw 10-second sequences and maintains robustness under flexible random mixed noise.

Conclusion: TFCDiff enables plug-and-play deployment in wearable ECG monitors for high-motion scenarios, providing effective denoising of complex mixed noise while preserving diagnostic information in ambulatory ECG signals.
...
<details>
  <summary>Details</summary>
Motivation: Foundation flow-matching models currently underperform compared to domain-specific or untrained priors in inverse problems, despite their potential as universal priors.

Method: FMPlug combines instance-guided time-dependent warm-start strategy with sharp Gaussianity regularization to add problem-specific guidance while preserving Gaussian structures.

Result: The framework achieves significant performance boost across image restoration and scientific inverse problems.
...
TL;DR: The paper introduces 3D test-time scaling, a unified framework that combines context-length, batch, and turn scaling to enhance reasoning performance beyond traditional test-time scaling limitations.


<details>
  <summary>Details</summary>
Motivation: Test-time scaling in reasoning reinforcement learning is fundamentally limited by the context length of base models, which is much smaller than training tokens. The authors aim to overcome this limitation by exploring additional dimensions of test-time enhancement.

Method: Proposed 3D test-time scaling framework that integrates three dimensions: context scaling (increasing reasoning context length), batch scaling (improving accuracy through parallel sampling), and turn scaling (enhancing reasoning quality via iterative self-refinement).

Result: Each scaling dimension shows test-time scaling effects with bounded capacity. Combining all three dimensions substantially improves reasoning performance on challenging testbeds (IOI, IMO, CPHO) and benefits from human preference feedback. The framework also extends to embodied learning for humanoid control behavior design.

Conclusion: 3D test-time scaling effectively extends reasoning capacity beyond traditional context-length limitations, demonstrating significant performance improvements across multiple domains and enabling applications in embodied learning with human-in-the-loop frameworks.
...
### [129] [Attention-Based Feature Online Conformal Prediction for Time Series](https://arxiv.org/abs/2511.15838)
*Meiyi Zhu,Caili Guo,Chunyan Feng,Osvaldo Simeone*

Main category: cs.LG

TL;DR: AFOCP improves online conformal prediction by using feature-space calibration and attention-based weighting to create more compact prediction sets while maintaining coverage guarantees.


<details>
  <summary>Details</summary>
Motivation: Standard OCP has limitations: it operates in output space with simple nonconformity scores and treats all historical data uniformly, failing to handle temporal dependencies and distribution shifts effectively.
...
Conclusion: AFOCP successfully addresses OCP limitations through feature-space calibration and attention-based weighting, providing more compact prediction sets with maintained coverage guarantees in non-stationary environments.
...
### [130] [Connecting the Dots: A Machine Learning Ready Dataset for Ionospheric Forecasting Models](https://arxiv.org/abs/2511.15743)
...
TL;DR: The paper presents a curated, open-access dataset integrating diverse ionospheric and heliospheric measurements for machine learning-ready ionospheric forecasting, and benchmarks spatiotemporal ML models for TEC prediction.


<details>
  <summary>Details</summary>
Motivation: To address critical space weather forecasting challenges for GNSS, communications, aviation safety, and satellite operations by overcoming sparse observations and complex geospatial coupling issues.

Method: Integrated diverse data sources (solar observations, solar wind parameters, geomagnetic indices, TEC maps) into a coherent, modular data structure, then trained and benchmarked spatiotemporal machine learning architectures for TEC forecasting.

Result: Developed a novel heterogeneous dataset that supports both physical and data-driven modeling, enabling exploration of ionospheric dynamics and Sun-Earth interactions.

Conclusion: The work provides an extensive dataset and modeling pipeline that supports both scientific inquiry and operational forecasting efforts for ionospheric prediction.
...
TL;DR: TB or not TB is an LLM-based framework for automated hardware verification stimulus generation using Coverage-Driven Direct Preference Optimization (CD-DPO) that achieves significant coverage improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: Design verification is the most time-consuming phase in hardware development, and generating effective stimuli for design under test is critical but labor-intensive. LLMs offer potential for automation but need guidance to generate high-quality verification stimuli.

Method: Proposes Coverage-Driven Direct Preference Optimization (CD-DPO) that integrates quantitative coverage feedback into LLM optimization. Uses PairaNet dataset with high/low-quality testbench pairs labeled by simulation coverage metrics, and fine-tunes LLMs to generate stimuli maximizing verification coverage.

Result: On CVDP CID12 benchmark, TB or not TB outperforms both open-source and commercial baselines, achieving up to 77.27% improvement in code coverage compared to existing methods.

Conclusion: Coverage-driven preference optimization is highly effective for LLM-based hardware verification, demonstrating significant improvements in verification coverage and automation capabilities.
...
### [132] [TopoReformer: Mitigating Adversarial Attacks Using Topological Purification in OCR Models](https://arxiv.org/abs/2511.15807)
...
<details>
  <summary>Details</summary>
Motivation: Adversarial perturbations in text images can fool OCR systems with invisible changes, creating security risks in document processing and license plate recognition. Existing defenses are model-specific, computationally expensive, and vulnerable to adaptive attacks while degrading performance on clean inputs.

Method: Leverages topological features (connectivity, holes, loops) that remain unchanged under continuous deformations. Uses a topological autoencoder to enforce manifold-level consistency in latent space for robustness without explicit gradient regularization.

Result: Benchmarked on EMNIST and MNIST datasets against standard attacks (FGSM, PGD, Carlini-Wagner), adaptive attacks (EOT, BDPA), and OCR-specific watermark attack (FAWA), showing effective mitigation of adversarial perturbations.

Conclusion: TopoReformer provides a model-agnostic defense that preserves text structural integrity while being robust to various adversarial attacks, addressing limitations of existing methods.
...
### [133] [Beyond Tsybakov: Model Margin Noise and $\mathcal{H}$-Consistency Bounds](https://arxiv.org/abs/2511.15816)
*Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: Introduces Model Margin Noise (MM noise), a weaker alternative to Tsybakov noise that enables enhanced consistency bounds for classification by focusing on hypothesis-Bayes classifier discrepancy rather than distributional minimal margin.
...
Method: Defines the Model Margin Noise assumption and derives enhanced H-consistency bounds for binary and multi-class classification, extending previous work with the same favorable exponents but under weaker assumptions.

Result: Obtains enhanced consistency bounds that interpolate between linear and square-root regimes for intermediate noise levels, with instantiation for common surrogate loss families.

Conclusion: MM noise provides a more flexible noise condition than Tsybakov noise, enabling stronger theoretical guarantees for classification consistency across various noise levels and loss functions.
...
### [134] [Transparent Early ICU Mortality Prediction with Clinical Transformer and Per-Case Modality Attribution](https://arxiv.org/abs/2511.15847)
*Alexander Bakumenko,Janine Hoelscher,Hudson Smith*

Main category: cs.LG

TL;DR: A transparent multimodal ensemble combining physiological time-series and clinical notes improves ICU mortality prediction with interpretable decision-making.


<details>
  <summary>Details</summary>
Motivation: Existing machine learning approaches for ICU mortality prediction lack transparency and robustness, limiting clinical adoption despite high performance.

Method: Late-fusion ensemble using logistic regression to combine predictions from bidirectional LSTM for vitals and finetuned ClinicalModernBERT for clinical notes from first 48 hours of ICU stay.

Result: Improved discrimination over best single model (AUPRC 0.565 vs 0.526; AUROC 0.891 vs 0.876) with well-calibrated predictions and robust performance with missing modalities.

Conclusion: The system demonstrates competitive performance with reliable, auditable risk estimates and transparent operation crucial for clinical use.
...
TL;DR: A Python package for efficient discrete approximation of Gaussian mixture distributions with Wasserstein distance guarantees, implementing advanced quantization methods and supporting custom schemes for cyber-physical systems.


<details>
  <summary>Details</summary>
Motivation: To provide efficient and accurate discrete approximations of Gaussian mixture distributions with guaranteed error bounds, particularly for applications in control and verification of cyber-physical systems.

Method: Implements state-of-the-art quantization methods for Gaussian mixture models, extends them for improved scalability, and integrates complementary strategies like sigma-point methods with a modular interface for custom schemes.

Result: The package produces accurate approximations at low computational cost, demonstrated through benchmarking on high-dimensional, large, and degenerate Gaussian mixtures.

Conclusion: discretize_distributions is an effective tool for constructing discrete approximations of Gaussian mixture distributions with performance guarantees, suitable for integration into control and verification pipelines.
...
### [136] [GLOBE: Accurate and Generalizable PDE Surrogates using Domain-Inspired Architectures and Equivariances](https://arxiv.org/abs/2511.15856)
*Peter Sharpe*

Main category: cs.LG

TL;DR: GLOBE is a neural surrogate for homogeneous PDEs that combines boundary-element methods with equivariant ML, achieving 200x lower error than baselines on AirFRANS dataset while being compact and discretization-invariant.


<details>
  <summary>Details</summary>
Motivation: To create more accurate, generalizable, and practical ML-based PDE surrogates for industrial CAE by incorporating rigorous physics- and domain-inspired inductive biases.

Method: Represents solutions as superpositions of learnable Green's-function-like kernels from boundary faces to targets, with translation-, rotation-, and parity-equivariant architecture, explicit far-field decay, boundary-to-boundary hyperlayer communication, and rigorous nondimensionalization.

Result: On AirFRANS dataset, achieves 200x lower MSE than reference baselines and 50x lower than next-best model; 100x lower error on velocity/pressure fields and 600x lower on surface pressure in scarce data setting; maintains accuracy with modest extrapolation in Reynolds number and angle of attack.

Conclusion: Rigorous physics-inspired inductive biases enable substantial gains in accuracy, generalizability, and practicality for ML-based PDE surrogates in industrial applications.
...
### [137] [Global Resolution: Optimal Multi-Draft Speculative Sampling via Convex Minimization](https://arxiv.org/abs/2511.15898)
*Rahul Krishna Thomas,Arka Pal*

Main category: cs.LG

TL;DR: The paper presents an efficient algorithm for multi-draft speculative sampling that achieves 90% acceptance rates with minimal overhead by reformulating the optimal transport problem as a max-flow problem and reducing it to a convex optimization problem.


<details>
  <summary>Details</summary>
Motivation: To improve latency in autoregressive decoding for large language models by enhancing multi-draft speculative sampling, which currently faces computational challenges due to the exponentially large optimal transport linear program.

Method: Reformulate the optimal transport problem as a max-flow problem using polymatroid theory, then reduce it to a convex optimization problem in at most V variables, enabling efficient solution for n-draft speculative sampling.

Result: Achieved 90% acceptance rates with under 100 ms overhead per token and negligible deviation from target model distribution, providing the first practical multi-draft algorithm with such performance.

Conclusion: The proposed method successfully addresses computational bottlenecks in multi-draft speculative sampling, enabling efficient decoding with high acceptance rates and minimal overhead while maintaining target model distribution fidelity.
...
### [138] [Unified all-atom molecule generation with neural fields](https://arxiv.org/abs/2511.15906)
...
TL;DR: FuncBind is a modality-agnostic framework for structure-based drug design that generates target-conditioned all-atom molecules using neural fields and score-based generative models adapted from computer vision.


<details>
  <summary>Details</summary>
Motivation: Existing generative models for structure-based drug design are limited to specific modalities, restricting their broader applicability across different molecular systems.

Method: Uses neural fields to represent molecules as continuous atomic densities and employs score-based generative models with modern computer vision architectures. The modality-agnostic representation allows training on diverse atomic systems with variable atom/residue counts.

Result: Achieved competitive in silico performance in generating small molecules, macrocyclic peptides, and antibody CDR loops. Successfully generated novel antibody binders via de novo redesign of CDR H3 loops, validated in vitro. Also introduced a new dataset and benchmark for structure-conditioned macrocyclic peptide generation.

Conclusion: FuncBind provides a unified framework for structure-based drug design that works across multiple molecular modalities, demonstrating practical utility through successful in vitro validation of generated antibody binders.
...
### [139] [AccelOpt: A Self-Improving LLM Agentic System for AI Accelerator Kernel Optimization](https://arxiv.org/abs/2511.15915)
*Genghan Zhang,Shaowei Zhu,Anjiang Wei,Zhenyu Song,Allen Nie,Zhen Jia,Nandita Vijaykumar,Yida Wang,Kunle Olukotun*

Main category: cs.LG

TL;DR: AccelOpt is a self-improving LLM agent that autonomously optimizes AI accelerator kernels without expert knowledge, achieving 49-61% peak throughput on Trainium accelerators while being 26x cheaper than alternatives.


<details>
  <summary>Details</summary>
Motivation: To eliminate the need for expert-provided hardware-specific optimization knowledge for emerging AI accelerators, enabling autonomous kernel optimization.

Method: Uses iterative generation with an optimization memory that curates experiences from slow-fast kernel pairs, evaluated on NKIBench (AWS Trainium kernel benchmark suite from real LLM workloads).

Result: Improved average peak throughput from 49% to 61% on Trainium 1 and 45% to 59% on Trainium 2; matches Claude Sonnet 4 performance while being 26x cheaper with open-source models.

Conclusion: AccelOpt demonstrates effective self-improving capability for autonomous kernel optimization on emerging AI accelerators with significant cost efficiency.
...
### [140] [Breaking the Bottleneck with DiffuApriel: High-Throughput Diffusion LMs with Mamba Backbone](https://arxiv.org/abs/2511.15927)
...
<details>
  <summary>Details</summary>
Motivation: To overcome the inference efficiency limitations of Transformer-based diffusion models, which suffer from quadratic attention and KV-cache overhead.

Method: Built on bidirectional Mamba backbone combining diffusion objective with linear-time sequence modeling, with a hybrid variant (DiffuApriel-H) interleaving attention and Mamba layers.
...
### [141] [iLTM: Integrated Large Tabular Model](https://arxiv.org/abs/2511.15941)
*David Bonet,Marçal Comajoan Cara,Alvaro Calafell,Daniel Mas Montserrat,Alexander G. Ioannidis*

Main category: cs.LG

TL;DR: iLTM is an integrated Large Tabular Model that combines tree embeddings, dimensionality-agnostic representations, meta-trained hypernetworks, MLPs, and retrieval to outperform GBDTs and deep tabular models across classification and regression tasks with less tuning.


<details>
  <summary>Details</summary>
Motivation: Deep learning advances haven't fully transferred to tabular data, where GBDTs remain dominant despite their limitations in adaptability and scalability.
...
Result: Consistently superior performance across tabular classification and regression tasks; matches or surpasses strong baselines after light fine-tuning; outperforms well-tuned GBDTs and leading deep tabular models.

Conclusion: iLTM bridges the gap between tree-based and neural methods, offering a robust, adaptable, and scalable framework for tabular foundation models.
...
### [142] [L-JacobiNet and S-JacobiNet: An Analysis of Adaptive Generalization, Stabilization, and Spectral Domain Trade-offs in GNNs](https://arxiv.org/abs/2511.16081)
*Huseyin Goksu*

Main category: cs.LG

TL;DR: AOPF class addresses limitations of spectral GNNs like ChebyNet. Two Jacobi-based models (L-JacobiNet adaptive, S-JacobiNet static) reveal domain trade-offs: [0,∞) better for heterophily, [-1,1] more stable at high K. Key finding: ChebyNet's issue is stabilization, not static nature.


<details>
  <summary>Details</summary>
Motivation: Spectral GNNs like ChebyNet suffer from heterophily and over-smoothing due to static low-pass filter design. Need to investigate Adaptive Orthogonal Polynomial Filter (AOPF) class as solution.

Method: Introduce two models in [-1,1] domain: L-JacobiNet (adaptive generalization of ChebyNet with learnable alpha, beta parameters) and S-JacobiNet (LayerNorm-stabilized static ChebyNet baseline). Compare against AOPFs in [0,∞) domain like LaguerreNet.

Result: Domain trade-offs discovered: [0,∞) domain superior for modeling heterophily, [-1,1] domain provides better numerical stability at high K (K>20). Static S-JacobiNet outperforms adaptive L-JacobiNet on 4/5 benchmark datasets.

Conclusion: ChebyNet's main flaw is stabilization issue, not its static nature. S-JacobiNet identified as powerful overlooked baseline. Adaptation in [-1,1] domain can lead to overfitting.
...
### [143] [Self-supervised and Multi-fidelity Learning for Extended Predictive Soil Spectroscopy](https://arxiv.org/abs/2511.15965)
...
TL;DR: Self-supervised ML framework for multi-fidelity soil spectroscopy using latent space embeddings from MIR spectra, enabling NIR-to-MIR conversion to leverage large MIR datasets for improved soil property prediction.


<details>
  <summary>Details</summary>
Motivation: To leverage large MIR spectral libraries for soil property prediction while enabling use of low-cost portable NIR scanners, overcoming limitations of smaller NIR datasets.

Method: Pretrained Variational Autoencoder on MIR spectra for latent embeddings, then used frozen MIR decoder with NIR encoder for spectrum conversion, followed by downstream ML models for soil property prediction.
...
Conclusion: Unified spectral latent space effectively leverages large MIR datasets to improve soil property predictions, particularly for properties not well represented in NIR libraries.
...
### [144] [HybSpecNet: A Critical Analysis of Architectural Instability in Hybrid-Domain Spectral GNNs](https://arxiv.org/abs/2511.16101)
*Huseyin Goksu*

Main category: cs.LG

TL;DR: HybSpecNet resolves the stability-adaptivity trade-off in spectral GNNs by combining stable ChebyNet and adaptive KrawtchoukNet branches with late fusion to prevent gradient instability.
...
Method: Proposed HybSpecNet with hybrid-domain architecture combining stable ChebyNet branch and adaptive KrawtchoukNet branch. Identified instability poisoning in naive concatenation fusion and developed late fusion architecture to isolate gradient pathways.

Result: Naive hybrid architecture achieved strong performance at low K but catastrophically collapsed at K=25 due to instability poisoning. Late fusion architecture remained perfectly stable up to K=30 while maintaining SOTA performance across all graph types.

Conclusion: HybSpecNet with late fusion successfully resolves the stability-adaptivity trade-off, providing a robust architectural solution that prevents gradient instability while achieving state-of-the-art performance on both homophilic and heterophilic graphs.
...
### [145] [Machine Learning Epidemic Predictions Using Agent-based Wireless Sensor Network Models](https://arxiv.org/abs/2511.15982)
*Chukwunonso Henry Nwokoye,Blessing Oluchi,Sharna Waldron,Peace Ezzeh*

Main category: cs.LG

TL;DR: This paper develops an agent-based SEIRV model to predict malware spread in wireless sensor networks using machine learning algorithms, with Random Forest, XGBoost, Decision Trees, and k-NN achieving the best performance.


<details>
  <summary>Details</summary>
Motivation: The lack of epidemiological data in WSNs makes it difficult to build robust models for forecasting and mitigating malware threats like viruses and worms.
...
Result: Predictions performed well with low error metrics and high R² values (0.997-1.000) on training set. Validation scores were lower (0.992-0.999) but still strong. Random Forest, XGBoost, Decision Trees, and k-nearest neighbors were top performers.

Conclusion: The agent-based SEIRV model combined with ML algorithms effectively predicts malware spread in WSNs, with ensemble methods and tree-based algorithms showing superior performance compared to linear regression variants.
...
TL;DR: This paper provides theoretical guarantees for stochastic machine unlearning algorithms R2D and D2D, proving (ε, δ) certified unlearning for strongly convex, convex, and nonconvex loss functions by analyzing them as disturbed gradient systems.
...
Method: Analyze unlearning through disturbed gradient systems, use optimal coupling of unlearning and retraining trajectories to create probabilistic sensitivity bounds, and combine with a novel relaxed Gaussian mechanism.
...
Conclusion: Both algorithms can achieve certified unlearning, with D2D superior for strongly convex functions due to contraction to unique minimum, and R2D more versatile for convex/nonconvex cases by reversing accumulated disturbances.
...
TL;DR: A novel framework combining Sub-treatment Group Alignment (SGA) and Random Temporal Masking (RTM) for improved counterfactual outcome estimation from time-series data, achieving state-of-the-art performance through synergistic deconfounding and temporal generalization.


<details>
  <summary>Details</summary>
Motivation: Counterfactual outcome estimation from time-series observations is challenging due to unobserved counterfactual trajectories and time-evolving confounders that distort estimation at every step.

Method: Proposes SGA for fine-grained sub-treatment group alignment to improve deconfounding, and RTM for temporal generalization by randomly replacing covariates with Gaussian noise during training to encourage reliance on stable historical patterns.
...
Conclusion: The framework successfully addresses time-series counterfactual estimation challenges through complementary approaches: RTM enhances temporal generalization across time steps, while SGA improves deconfounding at each specific time point.
...
### [148] [Physics-Guided Inductive Spatiotemporal Kriging for PM2.5 with Satellite Gradient Constraints](https://arxiv.org/abs/2511.16013)
...
TL;DR: SPIN is a novel physics-guided deep learning framework for high-resolution PM2.5 mapping that overcomes satellite data gaps by modeling physical processes and using AOD as gradient constraints rather than direct inputs.


<details>
  <summary>Details</summary>
Motivation: Traditional PM2.5 mapping methods suffer from severe data missingness in satellite AOD data due to cloud cover and nighttime limitations, and are affected by inversion biases, creating gaps in pollution monitoring.

Method: Proposes SPIN framework with parallel graph kernels to explicitly model physical advection and diffusion processes, and introduces a training strategy that uses AOD as spatial gradient constraints in the loss function instead of direct inputs.

Result: Achieved state-of-the-art performance with MAE of 9.52 ug/m³ in Beijing-Tianjin-Hebei region, generating continuous pollution fields even in unmonitored areas with physical plausibility.

Conclusion: SPIN provides a robust, low-cost, all-weather solution for fine-grained environmental management by overcoming data voids through physics-guided learning and innovative use of satellite data.
...
### [149] [CARE: Turning LLMs Into Causal Reasoning Expert](https://arxiv.org/abs/2511.16016)
...
<details>
  <summary>Details</summary>
Motivation: LLMs lack causal reasoning capabilities and tend to ignore observational data, relying instead on semantic meaning of variable names. Current approaches using algorithm outputs as prompts actually decrease performance.

Method: Proposed CARE framework: supervised fine-tuning of LLMs using outputs from established causal discovery algorithms to teach effective utilization of both internal knowledge and external algorithmic clues.

Result: Fine-tuned Qwen2.5-1.5B model significantly outperforms traditional causal discovery algorithms and state-of-the-art LLMs with over 1000x more parameters.

Conclusion: Supervised fine-tuning enables LLMs to effectively combine their knowledge with external causal discovery algorithm outputs, achieving superior causal reasoning performance despite smaller model size.
...
### [150] [HGCN2SP: Hierarchical Graph Convolutional Network for Two-Stage Stochastic Programming](https://arxiv.org/abs/2511.16027)
*Yang Wu,Yifan Zhang,Zhenxing Liang,Jian Cheng*

Main category: cs.LG

TL;DR: HGCN2SP is a novel hierarchical graph-based model that uses reinforcement learning to select representative scenarios in optimal order for solving two-stage stochastic programming problems efficiently.
...
Method: Developed HGCN2SP with hierarchical graph design to encode scenarios and model their relationships hierarchically. Trained in reinforcement learning paradigm using solver feedback, with hierarchical graph convolutional network for feature encoding and attention-based decoder for scenario selection in proper order.

Result: Evaluation on two classic 2SP problems shows HGCN2SP provides high-quality decisions in short computational time and exhibits remarkable generalization capabilities for large-scale instances with many variables or unseen scenarios.

Conclusion: HGCN2SP effectively addresses limitations of current scenario selection methods by deeply integrating scenario information and considering scenario order, enabling efficient solution of large-scale two-stage stochastic programming problems.
...
### [151] [Agent0: Unleashing Self-Evolving Agents from Zero Data via Tool-Integrated Reasoning](https://arxiv.org/abs/2511.16043)
*Peng Xia,Kaide Zeng,Jiaqi Liu,Can Qin,Fang Wu,Yiyang Zhou,Caiming Xiong,Huaxiu Yao*

Main category: cs.LG

TL;DR: Agent0 is a fully autonomous framework that evolves high-performing LLM agents through multi-step co-evolution and tool integration, eliminating dependency on human data and achieving significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agent frameworks are limited by human-curated data dependency and single-round interactions, hindering the development of complex curricula involving tool use and dynamic reasoning.

Method: Agent0 establishes symbiotic competition between two agents from the same base LLM: a curriculum agent that proposes challenging tasks, and an executor agent that solves them with tool integration, creating a self-reinforcing cycle.

Result: Agent0 substantially boosts reasoning capabilities, improving the Qwen3-8B-Base model by 18% on mathematical reasoning and 24% on general reasoning benchmarks.

Conclusion: Agent0 demonstrates that fully autonomous co-evolution with tool integration can effectively evolve high-performing agents without external data, establishing a scalable self-improvement framework.
...
### [152] [Change-of-Basis Pruning via Rotational Invariance](https://arxiv.org/abs/2511.16061)
*Alex Ning,Vainateya Rangaraju*

Main category: cs.LG

TL;DR: The paper introduces two-subspace radial activations (TSRAs) to enable change-of-basis pruning in deep learning architectures by providing rotational invariance, allowing CoB transformations to be merged into weights without extra parameters.


<details>
  <summary>Details</summary>
Motivation: Structured pruning effectiveness depends on importance distribution, and CoB pruning addresses this by concentrating importance, but standard architectures lack invariance to such transformations.

Method: Propose TSRAs - activation functions invariant to orthogonal linear transformations within two activation subspaces, enabling CoB transformations to merge into weights without additional parameters.

Result: CoB+TSRA framework shows: 4.52% accuracy drop vs ReLU baseline; extends reliable pruning frontier from 30% to 70% parameters without fine-tuning; prunes 90-96% parameters with 1-6% accuracy drop after fine-tuning.

Conclusion: Rotationally invariant architectures offer a promising path for CoB pruning, enabling more effective structured pruning while maintaining accuracy.
...
### [153] [Gauge-Equivariant Graph Networks via Self-Interference Cancellation](https://arxiv.org/abs/2511.16062)
*Yoonhyuk Choi,Chong-Kwon Kim*

Main category: cs.LG

TL;DR: GESC is a novel graph neural network that addresses heterophily by using gauge-equivariant connections and self-interference cancellation instead of traditional additive aggregation.


<details>
  <summary>Details</summary>
Motivation: Standard GNNs perform well on homophilous graphs but struggle with heterophily due to self-reinforcing and phase-inconsistent signals that degrade performance.

Method: Replaces additive aggregation with projection-based interference mechanism using U(1) phase connections and rank-1 projection to attenuate self-parallel components before attention, with sign- and phase-aware gating.

Result: Consistently outperforms recent state-of-the-art models across diverse graph benchmarks.

Conclusion: GESC provides a unified, interference-aware view of message passing that effectively handles heterophily in graph learning.
...
### [154] [ILoRA: Federated Learning with Low-Rank Adaptation for Heterogeneous Client Aggregation](https://arxiv.org/abs/2511.16069)
*Junchao Zhou,Junkang Liu,Fanhua Shang*

Main category: cs.LG

TL;DR: ILoRA addresses federated learning challenges with LoRA under client heterogeneity through orthonormal initialization, concatenated QR aggregation for mixed ranks, and AdamW with control variates to prevent client drift.


<details>
  <summary>Details</summary>
Motivation: Federated Learning with LoRA faces three critical challenges: initialization-induced instability from random subspace misalignment, rank incompatibility and aggregation errors when averaging different rank parameters, and exacerbated client drift under non-IID data.

Method: ILoRA integrates three innovations: QR-based orthonormal initialization for coherent subspace starting points, concatenated QR aggregation to fuse heterogeneous-rank updates via concatenation and decomposition, and AdamW optimizer with rank-aware control variates to correct local updates.

Result: Extensive experiments on vision and NLP benchmarks demonstrate that ILoRA consistently achieves superior accuracy and convergence stability compared to existing federated LoRA methods.

Conclusion: ILoRA provides a unified framework with theoretical convergence guarantees that effectively addresses the three critical challenges in federated learning with LoRA under client heterogeneity.
...
### [155] [A Mathematical Framework for Custom Reward Functions in Job Application Evaluation using Reinforcement Learning](https://arxiv.org/abs/2511.16073)
*Shreyansh Jain,Madhav Singhvi,Shreya Rahul Jain,Pranav S,Dishaa Lokesh,Naren Chittibabu,Akash Anandhan*

Main category: cs.LG

TL;DR: A two-step fine-tuning process using SFT and GRPO-based RL with a refined reward function creates a small language model that achieves 91% accuracy in resume evaluation, overcoming traditional ATS limitations.


<details>
  <summary>Details</summary>
Motivation: Traditional ATS systems are inflexible keyword-matchers that reject qualified candidates due to minor semantic mismatches, requiring a more refined approach to resume evaluation.

Method: Two-step process: 1) Supervised Fine-Tuning (SFT) to build baseline model, 2) Reinforcement Learning using GRPO with a multi-component reward function that holistically assesses candidates beyond keyword matching.

Result: Final model achieves 91% accuracy on unseen test data with 0.85 recall for 'SELECTED' class and 1.0 precision. Overcame initial reward hacking issues through refined reward function and hyperparameter tuning.

Conclusion: A properly executed two-step fine-tuning procedure can effectively refine small language models for human-like candidate scoring, overcoming drawbacks of traditional ATS and naive RL usage.
...
### [156] [AssayMatch: Learning to Select Data for Molecular Activity Models](https://arxiv.org/abs/2511.16087)
*Vincent Fan,Regina Barzilay*

Main category: cs.LG

TL;DR: AssayMatch is a framework that uses data attribution methods to select homogeneous training data compatible with target test sets, improving model performance in drug discovery by filtering out noisy experiments.


<details>
  <summary>Details</summary>
Motivation: Machine learning models in drug discovery suffer from noisy training data due to aggregating bioactivity data from diverse sources with variable experimental protocols, which limits model performance.

Method: Leverages data attribution methods to quantify training assay contributions, finetunes language embeddings of assay descriptions to capture compatibility, and enables data selection for test sets with unknown labels.

Result: Models trained on AssayMatch-selected data surpass performance of models trained on complete datasets, with increased prediction capability for 9/12 model-target pairs over language-only baseline.

Conclusion: AssayMatch provides a data-driven mechanism to curate higher-quality datasets, reducing noise from incompatible experiments and improving predictive power and data efficiency in drug discovery.
...
### [157] [Mitigating Estimation Bias with Representation Learning in TD Error-Driven Regularization](https://arxiv.org/abs/2511.16090)
*Haohui Chen,Zhiyong Chen,Aoxiang Liu,Wentuo Fang*

Main category: cs.LG

TL;DR: Enhanced double actor-critic framework with tunable bias control and improved representations for continuous control, outperforming benchmarks by flexibly managing overestimation and underestimation biases.


<details>
  <summary>Details</summary>
Motivation: Deterministic policy gradient algorithms suffer from value estimation biases that degrade performance, and the exploration potential of double actors remains underexplored despite double critics reducing biases.

Method: Proposes three convex combination strategies (symmetric and asymmetric) that balance pessimistic estimates to mitigate overestimation and optimistic exploration via double actors to alleviate underestimation, controlled by a single hyperparameter. Integrates augmented state and action representations into actor and critic networks.
...
Conclusion: The framework achieves flexible bias control and stronger representation learning, showing that tunable bias mechanisms and enhanced representations significantly improve performance in continuous control tasks.
...
### [158] [Pathlet Variational Auto-Encoder for Robust Trajectory Generation](https://arxiv.org/abs/2511.16105)
*Yuanbo Tang,Yan Tang,Zixuan Zhang,Zihui Zhao,Yang Li*

Main category: cs.LG

TL;DR: Proposes a robust and interpretable deep generative model for trajectory generation using pathlet representations, achieving significant improvements in performance and efficiency over baselines.


<details>
  <summary>Details</summary>
Motivation: Address limitations in robustness and interpretability of existing trajectory generation models, which restrict their application on noisy real-world data and trustworthiness in downstream tasks.

Method: Uses pathlet representation to encode trajectories as binary vectors with a learned dictionary of trajectory segments. Combines Variational Autoencoder (VAE) with linear decoder in a probabilistic graphical model to simultaneously learn latent embeddings and pathlet dictionary capturing mobility patterns.

Result: Achieves 35.4% and 26.3% relative improvements over baselines on two real-world datasets, with 64.8% time savings and 56.5% GPU memory reduction. Enables trajectory prediction and data denoising.

Conclusion: The proposed model effectively handles noisy data, provides interpretable trajectory generation, and offers efficiency advantages for practical applications in urban mobility studies.
...
### [159] [An Interpretability-Guided Framework for Responsible Synthetic Data Generation in Emotional Text](https://arxiv.org/abs/2511.16132)
*Paula Joy B. Martinez,Jose Marie Antonio Miñoza,Sebastian C. Ibañez*

Main category: cs.LG

TL;DR: SHAP-guided LLM framework generates synthetic emotion data that matches real data performance for classification but shows reduced linguistic richness compared to authentic social media posts.


<details>
  <summary>Details</summary>
Motivation: Rising API costs and platform restrictions make accessing training data for emotion recognition from social media prohibitively expensive.

Method: Interpretability-guided framework using Shapley Additive Explanations (SHAP) to provide principled guidance for LLM-based synthetic data generation.

Result: SHAP-guided approach matches real data performance, outperforms naive generation, and improves classification for underrepresented emotion classes, but synthetic text shows reduced vocabulary richness and fewer personal/temporal expressions.
...
### [160] [Labels Matter More Than Models: Quantifying the Benefit of Supervised Time Series Anomaly Detection](https://arxiv.org/abs/2511.16145)
...
Method: Introduced STAND, a streamlined supervised baseline, and conducted the first methodical comparison between supervised and unsupervised paradigms on five public datasets.

Result: Under limited labeling budget, simple supervised models significantly outperform complex state-of-the-art unsupervised methods; minimal supervision yields higher returns than architectural innovations; STAND shows superior prediction consistency and anomaly localization.
...
### [161] [Enhancing Nuclear Reactor Core Simulation through Data-Based Surrogate Models](https://arxiv.org/abs/2511.16148)
*Perceval Beja-Battais,Alain Grossetête,Nicolas Vayatis*

Main category: cs.LG

TL;DR: This paper introduces two surrogate models (data-driven and physics-informed) to enhance nuclear reactor core simulation for improving Model Predictive Control in nuclear power plants, achieving up to 1000x computational time reduction.
...
Method: Developed two surrogate models from nonlinear stiff ODEs: data-driven and physics-informed models as alternative simulation schemes to improve nuclear reactor core simulation.
...
Conclusion: Surrogate models provide efficient simulation schemes that significantly enhance computational performance for nuclear reactor core simulation in MPC applications.
...
### [162] [Achieving Skilled and Reliable Daily Probabilistic Forecasts of Wind Power at Subseasonal-to-Seasonal Timescales over France](https://arxiv.org/abs/2511.16164)
*Eloi Lindas,Yannig Goude,Philippe Ciais*

Main category: cs.LG

TL;DR: A forecasting pipeline that transforms ECMWF subseasonal-to-seasonal weather forecasts into wind power predictions for 1-46 day horizons, with post-processing to address forecast biases and improve calibration.


<details>
  <summary>Details</summary>
Motivation: Need for accurate long-term wind power forecasts (beyond short-term horizons) for grid stability, supply-demand balancing, and market risk management, as existing methods typically rely on temporal and spatial aggregation.

Method: Developed a forecasting pipeline that converts ECMWF subseasonal-to-seasonal weather forecasts into wind power predictions at daily resolution, including post-processing to correct biases and improve ensemble dispersion.

Result: Outperformed climatological baseline by 50% in Continuous Ranked Probability Skill Score and Ensemble Mean Squared Error, with near-perfect calibration for 15-46 day lead times.

Conclusion: The proposed framework successfully enables reliable wind power forecasting at subseasonal-to-seasonal time scales with significant skill improvements over baseline methods.
...
### [163] [CausalMamba: Interpretable State Space Modeling for Temporal Rumor Causality](https://arxiv.org/abs/2511.16191)
*Xiaotong Zhan,Xi Cheng*

Main category: cs.LG

TL;DR: CausalMamba integrates Mamba-based sequence modeling, GCNs, and causal discovery via NOTEARS to detect rumors on social media while providing interpretable causal insights.


<details>
  <summary>Details</summary>
Motivation: Existing rumor detection models lack interpretability and fail to reveal causal mechanisms behind misinformation spread, limiting their practical utility.

Method: Combines Mamba-based sequence modeling for temporal tweet sequences, GCNs for reply structures, and differentiable causal discovery (NOTEARS) to learn joint representations and uncover latent causal graphs.

Result: Achieves competitive classification performance on Twitter15 dataset and enables counterfactual intervention analysis, showing that removing top causal nodes significantly alters graph connectivity.

Conclusion: Provides a unified framework for rumor classification and influence analysis, offering more explainable and actionable misinformation detection systems.
...
### [164] [A Switching Framework for Online Interval Scheduling with Predictions](https://arxiv.org/abs/2511.16194)
*Antonios Antoniadis,Ali Shahheidar,Golnoosh Shahkarami,Abolfazl Soltani*

Main category: cs.LG

TL;DR: Online interval scheduling with predictions: SemiTrust-and-Switch framework combines prediction-based and classical algorithms to balance consistency and robustness, with tight lower bounds and smooth performance degradation.


<details>
  <summary>Details</summary>
Motivation: To improve online interval scheduling performance using machine-learned predictions while maintaining robustness against prediction errors in the irrevocable decision setting.

Method: Developed SemiTrust-and-Switch framework that unifies prediction-based and classical algorithms, plus a randomized algorithm that smoothly interpolates between prediction-based and robust approaches.

Result: The framework provides trade-off between consistency (good predictions) and robustness (adversarial inputs), with proven tight lower bounds. The randomized algorithm achieves smooth performance degradation with prediction quality.

Conclusion: Learning-augmented approach successfully enhances online interval scheduling with predictions, providing both improved performance with good predictions and robust guarantees against errors.
...
### [165] [Causal Synthetic Data Generation in Recruitment](https://arxiv.org/abs/2511.16204)
*Andrea Iommi,Antonio Mastropietro,Riccardo Guidotti,Anna Monreale,Salvatore Ruggieri*

Main category: cs.LG

TL;DR: This paper presents a specialized synthetic data generation method using causal generative models to create synthetic recruitment datasets that preserve causal relationships, enabling fairness evaluation in candidate ranking algorithms.


<details>
  <summary>Details</summary>
Motivation: Recruitment domains face data scarcity due to privacy constraints, hindering development of fair ML models. Synthetic data generation is needed to overcome poor data quality and limited access to sensitive CV information.

Method: Uses two causal generative models (CGMs) - one for job offers and one for curricula - structured according to domain-informed causal graphs to generate synthetic datasets and evaluate candidate ranking fairness under controlled bias scenarios.

Result: The method enables generation of synthetic recruitment datasets that preserve underlying causal relationships, providing control over fairness and interpretability in data generation.

Conclusion: Causal generative models offer a promising solution for synthetic data generation in recruitment domains, allowing fairness evaluation of ranking algorithms under controlled bias conditions.
...
### [166] [Towards Overcoming Data Scarcity in Nuclear Energy: A Study on Critical Heat Flux with Physics-consistent Conditional Diffusion Model](https://arxiv.org/abs/2511.16207)
*Farah Alsafadi,Alexandra Akins,Xu Wu*

Main category: cs.LG

TL;DR: This paper investigates using diffusion models to overcome data scarcity in nuclear energy applications, specifically for critical heat flux (CHF) data augmentation.


<details>
  <summary>Details</summary>
Motivation: Deep generative modeling addresses data scarcity in energy applications where experimental data are limited, costly, or difficult to obtain, enabling improved robustness of downstream machine learning models.

Method: Developed both vanilla and conditional diffusion models using a public CHF dataset, with the conditional DM capable of generating targeted CHF data under user-specified thermal-hydraulic conditions.

Result: Both DM and conditional DM successfully generated realistic and physics-consistent CHF data, with conditional DM being highly effective for data augmentation while maintaining acceptable uncertainty levels.

Conclusion: Diffusion models, particularly conditional variants, provide an effective pathway for overcoming data scarcity in nuclear energy applications by generating high-fidelity synthetic data that preserves physical consistency.
...
### [167] [Mind the Gap: Bridging Prior Shift in Realistic Few-Shot Crop-Type Classification](https://arxiv.org/abs/2511.16218)
*Joana Reuss,Ekaterina Gikalo,Marco Körner*

Main category: cs.LG

TL;DR: Proposes Dirichlet Prior Augmentation (DirPA) to address class imbalance in agricultural crop classification by simulating real-world label distribution skew during few-shot learning training.


<details>
  <summary>Details</summary>
Motivation: Real-world agricultural data has severe class imbalance (long-tailed distribution) and limited labeled datasets. Current few-shot learning methods use artificially balanced training sets that don't reflect real conditions, causing distribution shift and poor generalization.

Method: Dirichlet Prior Augmentation (DirPA) models real-world label distribution as Dirichlet-distributed random variables and performs prior augmentation during few-shot learning to simulate target domain label skew.

Result: DirPA successfully shifts decision boundaries and stabilizes training by acting as a dynamic feature regularizer, improving real-world generalization.

Conclusion: DirPA effectively addresses the training-test distribution mismatch in imbalanced agricultural classification by proactively simulating real-world label skew during model training.
...
### [168] [Real-Time Inference for Distributed Multimodal Systems under Communication Delay Uncertainty](https://arxiv.org/abs/2511.16225)
*Victor Croisfelt,João Henrique Inacio de Souza,Shashi Raj Pandey,Beatriz Soret,Petar Popovski*

Main category: cs.LG

TL;DR: Proposes a neuro-inspired non-blocking inference paradigm using adaptive temporal windows of integration to handle stochastic delays across data streams, eliminating the need for reference modality and offline profiling.


<details>
  <summary>Details</summary>
Motivation: Address challenges of uncertain communication delays in connected cyber-physical systems that perform real-time inference from multiple data streams, overcoming limitations of state-of-the-art methods that require reference modality and costly offline profiling.

Method: Uses adaptive temporal windows of integration (TWIs) to dynamically adjust to stochastic delay patterns across heterogeneous streams, relaxing the reference-modality requirement in a communication-delay-aware framework.

Result: Achieves robust real-time inference with finer-grained control over accuracy-latency tradeoff, demonstrating superior adaptability to network dynamics compared to state-of-the-art approaches on audio-visual event localization tasks.

Conclusion: The proposed neuro-inspired non-blocking inference paradigm effectively handles stochastic delays in multi-stream systems without requiring reference modality or offline profiling, providing better adaptability to network dynamics.
...
### [169] [Deep SOR Minimax Q-learning for Two-player Zero-sum Game](https://arxiv.org/abs/2511.16226)
...
Method: Developed a deep successive over-relaxation minimax Q-learning algorithm that incorporates deep neural networks as function approximators, extending the approach to handle high-dimensional state-action spaces in two-player zero-sum games.

Result: Proved finite-time convergence of the proposed algorithm and showed through numerical experiments that it outperforms existing Q-learning methods. Ablation studies revealed the impact of different successive over-relaxation parameter values.

Conclusion: The proposed deep successive over-relaxation minimax Q-learning algorithm effectively handles high-dimensional two-player zero-sum games with function approximation, offering improved performance over traditional methods.
...
### [170] [Pass@k Metric for RLVR: A Diagnostic Tool of Exploration, But Not an Objective](https://arxiv.org/abs/2511.16231)
*Yang Yu*

Main category: cs.LG

TL;DR: The pass@k metric, while useful for evaluation, is an unsuitable direct optimization objective for LLMs due to vanishing learning signals in critical exploration regimes and exploration collapse dynamics.


<details>
  <summary>Details</summary>
Motivation: To analyze the pass@k metric's limitations as a direct optimization objective in reinforcement learning for LLMs, given its widespread adoption despite potential issues with exploration.

Method: Analyzed the pass@k objective mathematically, derived its gradient, and demonstrated it's essentially a per-example positive reweighting of pass@1. Examined exploration collapse dynamics where policy concentration diminishes the gap between pass@k and pass@1.

Result: The pass@k objective provides vanishing learning signals when exploration is most needed. As policies concentrate probability mass, the distinction between pass@k and pass@1 diminishes, leading to exploration collapse.

Conclusion: pass@k should be used as a diagnostic tool rather than a direct optimization objective. Explicit exploration mechanisms are needed for effective reinforcement learning in reasoning tasks.
...
### [171] [GeoPTH: A Lightweight Approach to Category-Based Trajectory Retrieval via Geometric Prototype Trajectory Hashing](https://arxiv.org/abs/2511.16258)
*Yang Xu,Zuliang Yang,Kai Ming Ting*

Main category: cs.LG

TL;DR: GeoPTH is a lightweight, non-learning framework for efficient trajectory similarity retrieval using geometric prototypes as hash anchors, achieving competitive accuracy and superior efficiency compared to traditional and learning-based methods.


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing trajectory similarity methods: traditional metrics are computationally expensive, while learning-based methods have high training costs and instability.

Method: Constructs data-dependent hash functions using representative trajectory prototypes (small point sets preserving geometric characteristics) as anchors, mapping trajectories to closest prototypes via Hausdorff metric.

Result: Highly competitive retrieval accuracy with traditional metrics and state-of-the-art learning methods, significantly outperforms binary codes from learned embeddings, and consistently superior efficiency over all competitors.

Conclusion: Lightweight prototype-centric approach offers practical and powerful alternative for trajectory retrieval, achieving exceptional performance and computational efficiency without learning overhead.
...
### [172] [Graph Diffusion Counterfactual Explanation](https://arxiv.org/abs/2511.16287)
*David Bechtoldt,Sidney Bender*

Main category: cs.LG

TL;DR: A novel framework for generating counterfactual explanations on graph data using discrete diffusion models and classifier-free guidance, addressing the challenge of explaining graph-based ML predictions.
...
Result: The method reliably generates in-distribution and minimally structurally different counterfactuals for both discrete classification and continuous property predictions.

Conclusion: The proposed Graph Diffusion Counterfactual Explanation framework effectively addresses the interpretability gap in graph-based machine learning models.
...
### [173] [Optimizing Operation Recipes with Reinforcement Learning for Safe and Interpretable Control of Chemical Processes](https://arxiv.org/abs/2511.16297)
*Dean Brandner,Sergio Lucia*

Main category: cs.LG

TL;DR: A novel RL approach that optimizes chemical process operation by tuning existing operation recipes and linear controllers, requiring less data and handling constraints better than traditional RL methods.


<details>
  <summary>Details</summary>
Motivation: Traditional RL struggles with hard constraints and data requirements in chemical processes, while optimal control methods face computational complexity. Current manual recipes with linear controllers lead to suboptimal performance.

Method: Use reinforcement learning to optimize parameters of existing operation recipes and their underlying linear controllers, leveraging embedded expert knowledge.

Result: Simulation on industrial batch polymerization reactor shows approach can achieve near-optimal controller performance while being data-efficient and constraint-compliant.

Conclusion: The method provides an interpretable, data-efficient alternative to traditional RL and optimal control for chemical process optimization, leveraging existing expert knowledge in operation recipes.
...
### [174] [Learning-Enhanced Observer for Linear Time-Invariant Systems with Parametric Uncertainty](https://arxiv.org/abs/2511.16318)
*Hao Shu*

Main category: cs.LG

TL;DR: A learning-enhanced observer (LEO) framework that refines system matrices through gradient-based optimization of output discrepancy, improving state estimation accuracy by over 15% for uncertain linear systems.


<details>
  <summary>Details</summary>
Motivation: To address parameter uncertainty in linear time-invariant systems and improve state estimation accuracy beyond what nominal models can achieve.

Method: Treats system matrices as optimizable variables and refines them through gradient-based minimization of steady-state output discrepancy loss, creating data-informed surrogate models while preserving classical observer structure.

Result: Extensive Monte Carlo studies show systematic and statistically significant reductions exceeding 15% in normalized estimation error for both open-loop and Luenberger observers across diverse system dimensions.

Conclusion: Modern learning mechanisms can effectively complement traditional observer design, yielding more accurate and robust state estimation in uncertain systems while maintaining classical design structures.
...
### [175] [Beyond Generative AI: World Models for Clinical Prediction, Counterfactuals, and Planning](https://arxiv.org/abs/2511.16333)
*Mohammad Areeb Qazi,Maryam Nadeem,Mohammad Yaqub*

Main category: cs.LG

TL;DR: This paper reviews World Models for healthcare, analyzing their capabilities across medical imaging, disease progression, and surgical domains using a 4-level rubric, and identifies research gaps for clinical reliability.


<details>
  <summary>Details</summary>
Motivation: Healthcare needs AI that is predictive, reliable, and data-efficient, but current generative models lack physical foundation and temporal reasoning required for clinical decision support.

Method: Survey of recent work across three healthcare domains: medical imaging/diagnostics, disease progression modeling from EHRs, and robotic surgery/surgical planning, using a 4-level capability rubric (L1-L4).

Result: Most reviewed systems achieve L1 (temporal prediction) and L2 (action-conditioned prediction), with fewer instances of L3 (counterfactual rollouts) and rare L4 (planning/control). Key gaps identified include under-specified action spaces, weak interventional validation, and incomplete multimodal state construction.

Conclusion: Outlines a research agenda for clinically robust prediction-first world models that integrate generative backbones with causal/mechanical foundation for safe decision support in healthcare.
...
### [176] [Improving Iterative Gaussian Processes via Warm Starting Sequential Posteriors](https://arxiv.org/abs/2511.16340)
*Alan Yufei Dong,Jihao Andreas Lin,José Miguel Hernández-Lobato*

Main category: cs.LG

TL;DR: Proposes a method to improve convergence of iterative Gaussian process solvers by leveraging solutions from smaller systems, achieving speed-ups in sequential decision-making tasks.


<details>
  <summary>Details</summary>
Motivation: Scalable Gaussian process inference is crucial for sequential decision-making but remains challenging; iterative GPs using linear solvers need improved convergence.

Method: Leverages known solutions from smaller linear systems to improve convergence of iterative linear solvers (conjugate gradients, SGD, alternative projections) for larger systems.

Result: Achieves speed-ups when solving to tolerance and improved Bayesian optimization performance under fixed compute budget for incremental data addition tasks.

Conclusion: The proposed technique effectively improves solver convergence in iterative GPs, providing practical benefits for sequential decision-making applications.
...
### [177] [Are Foundation Models Useful for Bankruptcy Prediction?](https://arxiv.org/abs/2511.16375)
*Marcin Kostrzewa,Oleksii Furman,Roman Furman,Sebastian Tomczak,Maciej Zięba*

Main category: cs.LG

TL;DR: Foundation models (Llama-3.3-70B-Instruct and TabPFN) underperform classical ML methods like XGBoost and CatBoost for corporate bankruptcy prediction on large imbalanced datasets, with LLMs showing unreliable probability estimates and TabPFN having unjustified computational costs.
...
Method: Used Llama-3.3-70B-Instruct and TabPFN foundation models, compared against classical ML baselines (XGBoost, CatBoost) on large, highly imbalanced datasets of over one million company records from the Visegrád Group.

Result: XGBoost and CatBoost consistently outperformed foundation models across all prediction horizons. LLMs showed unreliable probability estimates, while TabPFN was competitive with simpler baselines but required substantial computational resources not justified by performance gains.

Conclusion: Current foundation models remain less effective than specialized methods for bankruptcy forecasting, despite their generality, highlighting limitations in risk-sensitive financial applications.
...
### [178] [Optimal Fairness under Local Differential Privacy](https://arxiv.org/abs/2511.16377)
*Hrad Ghoukasian,Shahab Asoodeh*

Main category: cs.LG

TL;DR: The paper proposes optimal local differential privacy mechanisms that reduce data unfairness and improve fairness in downstream classification, showing both theoretical guarantees and empirical superiority over existing methods.


<details>
  <summary>Details</summary>
Motivation: To address how local differential privacy can be used as a pre-processing technique to reduce data unfairness and improve classification fairness while maintaining privacy of sensitive attributes.

Method: Derived closed-form optimal LDP mechanisms for binary sensitive attributes and developed tractable optimization framework for multi-valued attributes, with theoretical analysis linking data unfairness reduction to classification fairness improvement.

Result: The proposed approach consistently outperforms existing LDP mechanisms in reducing data unfairness across diverse datasets and fairness metrics, while maintaining accuracy close to non-private models and achieving better accuracy-fairness trade-off than leading fairness methods.

Conclusion: Local differential privacy serves as a principled and effective pre-processing fairness intervention technique that simultaneously preserves privacy and improves classification fairness.
...
### [179] [Collaborative Management for Chronic Diseases and Depression: A Double Heterogeneity-based Multi-Task Learning Method](https://arxiv.org/abs/2511.16398)
*Yidong Chai,Haoxin Liu,Jiaheng Xie,Chaopeng Wang,Xiao Fang*

Main category: cs.LG

TL;DR: Proposes ADH-MTL method for joint assessment of comorbid physical chronic diseases and depression using wearable sensor data, addressing double heterogeneity through group-level modeling, decomposition strategy, and Bayesian network.


<details>
  <summary>Details</summary>
Motivation: Most health sensing studies focus only on physical chronic diseases, overlooking the critical need for joint assessment of comorbid physical diseases and depression for collaborative chronic care.

Method: Multi-task learning approach with ADH-MTL method featuring three innovations: group-level modeling for new patient predictions, decomposition strategy to reduce complexity, and Bayesian network to capture dependencies while balancing similarities/differences.

Result: Empirical evaluations on real-world wearable sensor data show ADH-MTL significantly outperforms existing baselines, with each innovation proving effective.

Conclusion: Provides computational solution for integrated physical and mental healthcare and design principles for advancing collaborative chronic disease management across treatment phases.
...
### [180] [FreqFlow: Long-term forecasting using lightweight flow matching](https://arxiv.org/abs/2511.16426)
*Seyed Mohamad Moghadas,Bruno Cornelis,Adrian Munteanu*

Main category: cs.LG

TL;DR: FreqFlow introduces a frequency-domain flow matching framework for multivariate time-series forecasting that achieves state-of-the-art performance with significantly reduced computational overhead compared to diffusion models.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based generative models for MTS forecasting suffer from computational inefficiency due to iterative sampling and struggle with high-dimensional, non-stationary, multi-scale periodic patterns in real-world sensor data.

Method: Transforms forecasting to spectral domain using conditional flow matching, learns amplitude and phase shifts through a single complex-valued linear layer, decomposes signals into trend/seasonal/residual components, and uses ODE integration for deterministic sampling.

Result: Achieves 7% average RMSE improvement on real-world traffic datasets, uses only 89k parameters (order of magnitude smaller than diffusion models), and enables significantly faster inference through single-pass deterministic sampling.

Conclusion: FreqFlow provides an efficient and effective alternative to diffusion models for MTS forecasting by leveraging frequency-domain representations and flow matching, achieving superior performance with minimal computational requirements.
...
### [181] [Generative Modeling of Clinical Time Series via Latent Stochastic Differential Equations](https://arxiv.org/abs/2511.16427)
*Muhammad Aslanimoghanloo,Ahmed ElGazzar,Marcel van Gerven*

Main category: cs.LG

TL;DR: A generative modeling framework using latent neural stochastic differential equations (SDEs) for clinical time series analysis, handling irregular sampling and uncertainty in medical data.
...
Method: Proposes latent neural SDEs with modality-dependent emission models, using variational inference for state estimation and parameter learning to handle irregular observations and capture stochasticity.

Result: Outperforms ODE and LSTM baselines in accuracy and uncertainty estimation on simulated PKPD lung cancer treatment effect estimation and real ICU data forecasting tasks.

Conclusion: The framework enables precise, uncertainty-aware predictions to support clinical decision-making by handling complex medical time series data effectively.
...
### [182] [A Comparison Between Decision Transformers and Traditional Offline Reinforcement Learning Algorithms](https://arxiv.org/abs/2511.16475)
*Ali Murtaza Caunhye,Asad Jeewa*

Main category: cs.LG

TL;DR: Comparative study shows Decision Transformers outperform traditional offline RL methods in sparse reward scenarios and are less sensitive to reward density, while value-based methods excel in dense reward settings with high-quality data.


<details>
  <summary>Details</summary>
Motivation: To evaluate how Decision Transformers compare against traditional offline RL algorithms (CQL, IQL) in handling different reward structures (dense vs sparse) in continuous control environments.

Method: Empirical analysis comparing Decision Transformers with CQL and IQL algorithms in the ANT continuous control environment, testing performance across dense and sparse reward settings with varying dataset qualities.

Result: DTs showed less sensitivity to varying reward density, excelled with medium-expert datasets in sparse rewards, while IQL performed better in dense rewards with high-quality data. CQL offered balanced performance across data qualities. DTs had lower variance but higher computational costs.

Conclusion: Sequence modelling approaches like DTs are more suitable for uncertain reward structures or mixed-quality data, while value-based methods remain competitive in dense reward settings with high-quality demonstrations.
...
### [183] [Limitations of Scalarisation in MORL: A Comparative Study in Discrete Environments](https://arxiv.org/abs/2511.16476)
*Muhammad Sa'ood Shah,Asad Jeewa*

Main category: cs.LG

TL;DR: Scalarisation functions in MORL struggle to approximate Pareto fronts accurately in complex environments, while inner-loop multi-policy algorithms like Pareto Q-Learning offer more robust alternatives.


<details>
  <summary>Details</summary>
Motivation: To investigate limitations of scalarisation approaches for decision-making in multi-objective settings and compare performance between single-policy and multi-policy MORL algorithms.

Method: Used outer-loop multi-policy methodology to assess MO Q-Learning with linear and Chebyshev scalarisation functions, and explored Pareto Q-Learning as an inner-loop multi-policy alternative.

Result: Scalarisation functions' performance depends heavily on environment and Pareto front shape, often failing to retain solutions and favoring certain solution space regions. Finding appropriate weight configurations is complex.

Conclusion: Inner-loop multi-policy algorithms provide more sustainable and generalizable approaches for intelligent decision-making in dynamic and uncertain environments compared to scalarisation-based methods.
...
### [184] [Correlation-Aware Feature Attribution Based Explainable AI](https://arxiv.org/abs/2511.16482)
...
<details>
  <summary>Details</summary>
Motivation: Existing global attribution methods have high computational costs, lack stability with correlated inputs, and don't scale well to large or heterogeneous datasets.

Method: ExCIR quantifies sign-aligned co-movement between features and model outputs after robust centering. BlockCIR extends this to groupwise attribution for correlated feature sets, preventing double-counting in collinear clusters.

Result: ExCIR shows trustworthy agreement with established baselines, delivers consistent top-k rankings across diverse datasets (text, tabular, signal, image), and reduces runtime via lightweight evaluation on data subsets.
...
<details>
  <summary>Details</summary>
Motivation: Designing rewards for autonomous cyber attack and defense learning agents in complex, dynamic environments is challenging for subject matter experts, requiring automated approaches.

Method: Used LLM-guided reward design with multiple attack/defense personas, provided contextual cyber simulation environment information to LLM, then utilized reward structures in DRL-driven attack-defense simulation to learn defense policy ensembles.

Result: LLM-guided reward designs can lead to effective defense strategies against diverse adversarial behaviors in cyber attack-defense scenarios.

Conclusion: LLM-based reward design approach shows promise for generating autonomous cyber defense policies that can adapt to heterogeneous adversarial behaviors in complex simulation environments.
...
TL;DR: ODE-ViT: A Vision Transformer reformulated as an ODE system for stable, interpretable performance with fewer parameters, enhanced by teacher-student framework.


<details>
  <summary>Details</summary>
Motivation: Large vision models demand high computational resources and lack interpretability; ODE formulation addresses stability and understanding of model decisions.

Method: Reformulate Vision Transformer as ODE system using residual network-ODE connection, add teacher-student framework where discrete ViT guides continuous ODE trajectory.

Result: Achieves stable, interpretable performance on CIFAR-10/100 with 10x fewer parameters, surpassing prior ODE-based Transformers; teacher-student improves performance by 10+% over scratch training.

Conclusion: ODE-ViT provides efficient, interpretable alternative to large vision models with competitive performance and enhanced training via teacher guidance.
...
<details>
  <summary>Details</summary>
Motivation: Methods for detecting label errors need models robust to label errors, but training on corrupted data is challenging. Focal Loss's emphasis on difficult samples inspired the development of loss functions that instead de-weight or ignore such samples.
...
Conclusion: The proposed loss functions that de-weight or ignore difficult samples show promise for improving label error detection in training data.
...
### [188] [Dynamic Participation in Federated Learning: Benchmarks and a Knowledge Pool Plugin](https://arxiv.org/abs/2511.16523)
*Ming-Lun Lee,Fu-Shiang Yang,Cheng-Kuan Lin,Yan-Ann Chen,Chih-Yu Lin,Yu-Chee Tseng*

Main category: cs.LG

TL;DR: First open-source framework for benchmarking FL under dynamic client participation, revealing significant performance degradation and proposing KPFL solution.


<details>
  <summary>Details</summary>
Motivation: Existing FL research assumes consistent client participation, ignoring practical dynamic scenarios where clients intermittently join/leave during training.

Method: Proposed KPFL with shared knowledge pool, dual-age and data-bias weighting, and generative knowledge distillation to maintain stability and prevent knowledge loss.

Result: Benchmarking revealed substantial performance degradation under dynamic participation; KPFL significantly improved model robustness and generalization.

Conclusion: Dynamic participation severely impacts FL performance; KPFL effectively addresses these challenges through knowledge sharing and stabilization mechanisms.
...
### [189] [FairLRF: Achieving Fairness through Sparse Low Rank Factorization](https://arxiv.org/abs/2511.16549)
*Yuanbo Guo,Jun Xia,Yiyu Shi*

Main category: cs.LG

TL;DR: Proposes FairLRF, a fairness-oriented low rank factorization framework using SVD to enhance DL model fairness by selectively removing bias-inducing elements from unitary matrices, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing bias-mitigation methods are computationally expensive or cause significant accuracy drops, limiting practicality in resource-constrained settings like medical diagnosis.

Method: Uses SVD to analyze weight matrices, identifies that elements in unitary matrices contribute unequally to bias across sensitive groups, and selectively removes bias-inducing elements to reduce group disparities.

Result: Extensive experiments show FairLRF outperforms conventional LRF methods and state-of-the-art fairness-enhancing techniques, with ablation studies examining hyper-parameter influence.

Conclusion: This is the first work to use SVD primarily for fairness enhancement rather than compression, demonstrating SVD's effectiveness as a fairness tool.
...
### [190] [Broad stochastic configuration residual learning system for norm-convergent universal approximation](https://arxiv.org/abs/2511.16550)
*Han Su,Zhongyan Li,Wanquan Liu*

Main category: cs.LG

TL;DR: The paper identifies limitations in randomized learning networks' universal approximation property and proposes BSCRLS with a supervisory mechanism to ensure norm convergence.


<details>
  <summary>Details</summary>
Motivation: Randomized learning networks like BRLS have universal approximation property based on probability measure convergence rather than norm convergence, making them sensitive to random parameter selection.

Method: Proposed BSCRLS algorithm with novel supervisory mechanism that adaptively constrains random parameter ranges within BRLS framework, and developed three incremental versions for different network update scenarios.

Result: Theoretically proved BSCRLS achieves universal approximation based on norm convergence, and experimental results on solar panel dust detection show superiority over 13 deep and broad learning algorithms.

Conclusion: BSCRLS effectively addresses the norm convergence limitation in randomized learning networks and demonstrates practical effectiveness in real-world applications.
...
### [191] [Toward Valid Generative Clinical Trial Data with Survival Endpoints](https://arxiv.org/abs/2511.16551)
*Perrine Chassat,Van Tuan Nguyen,Lucas Ducrot,Emilie Lanoy,Agathe Guilloux*

Main category: cs.LG

TL;DR: A VAE-based method for generating synthetic control arms with time-to-event outcomes, addressing challenges in clinical trials like small sample sizes and censoring, outperforming GAN baselines on fidelity, utility, and privacy metrics.


<details>
  <summary>Details</summary>
Motivation: Clinical trials face challenges with fragmented populations, slow enrollment, and high costs, especially in oncology and rare diseases. External control arms from real-world data are limited, and existing generative methods are data-hungry and make strong assumptions.

Method: A variational autoencoder (VAE) that jointly generates mixed-type covariates and survival outcomes within a unified latent variable framework, without assuming independent censoring. Evaluated in data sharing and control-arm augmentation scenarios.

Result: Outperforms GAN baselines on fidelity, utility, and privacy metrics across synthetic and real trial datasets. Reveals systematic miscalibration of type I error and power, addressed with a post-generation selection procedure.

Conclusion: The method shows progress in generative survival modeling for clinical trials but highlights open challenges, particularly in calibration, which can be improved with post-generation selection.
...
### [192] [Boosting Predictive Performance on Tabular Data through Data Augmentation with Latent-Space Flow-Based Diffusion](https://arxiv.org/abs/2511.16571)
*Md. Tawfique Ihsan,Md. Rakibul Hasan Rafi,Ahmed Shoyeb Raihan,Imtiaz Ahmed,Abdullahil Azeem*

Main category: cs.LG

TL;DR: A family of latent-space, tree-driven diffusion methods for minority oversampling in imbalanced tabular data, using conditional flow matching with gradient-boosted trees as vector-field learners.


<details>
  <summary>Details</summary>
Motivation: Address severe class imbalance in real-world tabular learning where rare minority classes are essential but existing generative methods struggle with tabular heterogeneity, training stability, and privacy concerns.

Method: Three variants: PCAForest (linear PCA embedding), EmbedForest (learned nonlinear embedding), and AttentionForest (attention-augmented embedding), each coupling GBT-based flow with decoder back to original feature space.

Result: Across 11 datasets, AttentionForest achieves best average minority recall while maintaining competitive precision, calibration, and distributional similarity. PCAForest and EmbedForest offer faster generation with similar utility.

Conclusion: Latent-space, tree-driven diffusion provides efficient and privacy-aware approach to high-fidelity tabular data augmentation under severe class imbalance, with smaller embeddings improving minority recall and aggressive learning rates harming stability.
...
### [193] [ECPv2: Fast, Efficient, and Scalable Global Optimization of Lipschitz Functions](https://arxiv.org/abs/2511.16575)
*Fares Fourati,Mohamed-Slim Alouini,Vaneet Aggarwal*

Main category: cs.LG

TL;DR: ECPv2 is an improved global optimization algorithm for Lipschitz-continuous functions that addresses computational limitations of ECP while maintaining theoretical guarantees and achieving better performance.


<details>
  <summary>Details</summary>
Motivation: To overcome key limitations of the ECP framework, including high computational cost and overly conservative early behavior in global optimization of Lipschitz-continuous functions with unknown Lipschitz constants.

Method: Introduces three innovations: adaptive lower bound to avoid vacuous acceptance regions, Worst-m memory mechanism restricting comparisons to fixed-size subsets, and fixed random projection for accelerated distance computations in high dimensions.

Result: ECPv2 retains ECP's no-regret guarantees with optimal finite-time bounds, expands acceptance regions with high probability, and consistently matches or outperforms state-of-the-art optimizers across benchmarks while significantly reducing wall-clock time.

Conclusion: ECPv2 provides a scalable, theoretically grounded optimization algorithm that addresses computational bottlenecks while maintaining strong performance guarantees across high-dimensional, non-convex problems.
...
### [194] [Almost Sure Convergence Analysis of Differentially Private Stochastic Gradient Methods](https://arxiv.org/abs/2511.16587)
*Amartya Mukherjee,Jun Liu*

Main category: cs.LG

TL;DR: DP-SGD converges almost surely under smoothness assumptions with decaying step sizes, extending to momentum variants like DP-SHB and DP-NAG.


<details>
  <summary>Details</summary>
Motivation: Existing analyses of DP-SGD focus on convergence in expectation or high probability, lacking understanding of almost sure convergence for single trajectories.

Method: Prove almost sure convergence for DP-SGD under standard smoothness assumptions with decaying step sizes, extending analysis to momentum variants using careful energy constructions.

Result: DP-SGD converges almost surely in both nonconvex and strongly convex settings, with similar guarantees for momentum variants DP-SHB and DP-NAG.

Conclusion: The work provides stronger theoretical foundations for differentially private optimization, showing pathwise stability despite privacy-induced distortions in both convex and nonconvex regimes.
...
### [195] [gfnx: Fast and Scalable Library for Generative Flow Networks in JAX](https://arxiv.org/abs/2511.16592)
*Daniil Tiapkin,Artem Agarkov,Nikita Morozov,Ian Maksimov,Askar Tsyganov,Timofei Gritsaev,Sergey Samsonov*

Main category: cs.LG

TL;DR: gfnx is a fast JAX-based library for training Generative Flow Networks (GFlowNets) that provides extensive environments, metrics, and achieves significant speedups (up to 80x) compared to PyTorch implementations.


<details>
  <summary>Details</summary>
Motivation: To provide a fast and scalable package for GFlowNet research and applications, addressing the need for standardized benchmarking and accelerated training.

Method: Developed a JAX-based library with single-file implementations of core GFlowNet objectives, supporting diverse environments including hypergrids, sequence generation, molecular generation, phylogenetic trees, Bayesian structure learning, and Ising model sampling.

Result: Achieved significant speedups: up to 55x on CPU-based sequence generation and up to 80x on GPU-based Bayesian network structure learning compared to PyTorch benchmarks.

Conclusion: gfnx provides standardized benchmarks and accelerates GFlowNet research with substantial performance improvements, available as an open-source library.
...
### [196] [Toward Artificial Palpation: Representation Learning of Touch on Soft Bodies](https://arxiv.org/abs/2511.16596)
*Zohar Rimon,Elisei Shafer,Tal Tepper,Efrat Shimron,Aviv Tamar*

Main category: cs.LG

TL;DR: Self-supervised learning enables artificial palpation through encoder-decoder framework that learns tactile representations for medical imaging and change detection.


<details>
  <summary>Details</summary>
Motivation: Human palpation is currently the standard for medical touch examination, but artificial methods could provide more objective and detailed analysis.

Method: Developed simulation environment and collected real-world dataset using robot with tactile sensor, trained encoder-decoder model to predict sensory readings at different positions.

Result: Learned representation captures intricate tactile patterns beyond simple force maps, enabling successful tactile imaging and change detection applications.

Conclusion: Self-supervised learning provides promising foundation for artificial palpation systems that can perform medical imaging and detect changes in soft tissues.
...
### [197] [Stabilizing Policy Gradient Methods via Reward Profiling](https://arxiv.org/abs/2511.16629)
*Shihab Ahmed,El Houcine Bergou,Aritra Dutta,Yue Wang*

Main category: cs.LG

TL;DR: A universal reward profiling framework that integrates with policy gradient methods to selectively update policies based on high-confidence performance estimations, improving convergence speed and reducing variance.
...
Method: Proposes a reward profiling framework that can be integrated with any policy gradient algorithm, where policy updates are selectively performed based on high-confidence performance estimations rather than updating at every step.

Result: Empirical results on eight continuous-control benchmarks show up to 1.5x faster convergence to near-optimal returns and up to 1.75x reduction in return variance, while theoretical analysis confirms no slowdown in convergence.

Conclusion: The profiling approach provides a general, theoretically grounded path to more reliable and efficient policy learning in complex environments by addressing the variance issues in policy gradient methods.
...
### [198] [Evolution Strategies at the Hyperscale](https://arxiv.org/abs/2511.16652)
...
TL;DR: EGGROLL is an evolution strategies algorithm that uses low-rank matrix perturbations to scale backprop-free optimization to large neural networks with billions of parameters, reducing computational and memory costs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Evolution strategies (ES) are powerful for non-differentiable or noisy objectives but become prohibitively expensive at scale due to computational and memory costs of generating full-rank matrix perturbations for large neural networks.

Method: EGGROLL generates low-rank matrix perturbations using random matrices A and B with r << min(m,n) to form A·B⊤ instead of full-rank perturbations, reducing storage from O(mn) to O(r(m+n)) and computational cost from O(mn) to O(r(m+n)).

Result: EGGROLL converges to full-rank ES at O(1/r) rate, maintains ES performance in RL settings, is competitive with GRPO for LLM reasoning improvement, and enables stable pre-training of integer-only nonlinear recurrent language models.

Conclusion: EGGROLL successfully scales evolution strategies to large neural networks through low-rank learning, achieving significant computational savings without compromising performance across various applications.
...
### [199] [Taming the Long-Tail: Efficient Reasoning RL Training with Adaptive Drafter](https://arxiv.org/abs/2511.16665)
*Qinghao Hu,Shang Yang,Junxian Guo,Xiaozhe Yao,Yujun Lin,Yuxian Gu,Han Cai,Chuang Gan,Ana Klimovic,Song Han*

Main category: cs.LG

TL;DR: TLT accelerates reasoning RL training for LLMs using adaptive speculative decoding, achieving 1.7x speedup without accuracy loss by training draft models during idle GPU time and optimizing rollout strategies.


<details>
  <summary>Details</summary>
Motivation: Training reasoning LLMs with RL suffers from efficiency bottlenecks due to long-tail response generation where few very long responses dominate execution time, wasting resources and increasing costs.

Method: TLT integrates adaptive speculative decoding with two components: (1) Adaptive Drafter - lightweight draft model trained continuously on idle GPUs during long-tail generation, (2) Adaptive Rollout Engine - maintains memory-efficient CUDAGraph pool and adaptively selects SD strategies per batch.

Result: TLT achieves over 1.7x end-to-end RL training speedup over state-of-the-art systems while preserving model accuracy, and produces a high-quality draft model suitable for efficient deployment as a free byproduct.

Conclusion: TLT successfully addresses RL training efficiency bottlenecks through adaptive speculative decoding, providing significant speedup without accuracy loss and yielding deployable draft models.
...
TL;DR: An ensemble framework (eLLM) using multiple large language models significantly improves text categorization performance by 65% in F1-score over single models, achieving near human-expert accuracy.


<details>
  <summary>Details</summary>
Motivation: To address weaknesses in individual LLM systems including inconsistency, hallucination, category inflation, and misclassification in unstructured text categorization.

Method: Integrated multiple LLMs into an ensemble framework using mathematical collective decision-making with principled aggregation criteria, evaluated on IAB hierarchical taxonomy with 10 state-of-the-art LLMs in zero-shot conditions on 8,660 human-annotated samples.

Result: eLLM achieved up to 65% F1-score improvement over the strongest single model, with individual models plateauing due to text compression issues while eLLM improved both robustness and accuracy.

Conclusion: The ensemble framework offers a scalable and reliable solution for taxonomy-based classification that can significantly reduce dependence on human expert labeling, achieving near human-expert-level performance.
...
TL;DR: Graph-Memoized Reasoning is a framework that stores and reuses reasoning workflows as graph-structured memory to avoid recomputing similar reasoning steps across tasks, improving computational efficiency and reproducibility.


<details>
  <summary>Details</summary>
Motivation: Modern LLM-based reasoning systems waste resources by recomputing similar reasoning steps across different tasks, leading to inefficient computation, higher inference latency, and limited reproducibility.

Method: The approach encodes past decision graphs and retrieves them through structural and semantic similarity, enabling compositional reuse of subgraphs across new reasoning tasks. It formulates an optimization objective that minimizes reasoning cost while regularizing inconsistency between stored and generated workflows.
...
Conclusion: Graph-Memoized Reasoning offers a step toward persistent memory in large-scale agentic systems, enabling more efficient and reproducible reasoning workflows.
...
TL;DR: MACIE is a framework that explains multi-agent reinforcement learning systems using causal models, counterfactuals, and Shapley values to attribute outcomes, quantify emergent behaviors, and provide natural language explanations.


<details>
  <summary>Details</summary>
Motivation: As MARL systems are used in safety-critical applications, understanding why agents make decisions and how they achieve collective behavior is crucial. Existing explainable AI methods struggle in multi-agent settings by failing to attribute collective outcomes to individuals, quantify emergent behaviors, or capture complex interactions.

Method: MACIE combines structural causal models, interventional counterfactuals, and Shapley values to provide comprehensive explanations. It addresses three questions: individual agent causal contributions, system-level emergent intelligence through synergy metrics, and actionable explanations using natural language narratives.

Result: Evaluation across four MARL scenarios (cooperative, competitive, mixed motive) showed accurate outcome attribution (mean phi_i = 5.07, std < 0.05), detection of positive emergence in cooperative tasks (synergy index up to 0.461), and efficient computation (0.79 seconds per dataset on CPU).
...
TL;DR: The paper analyzes how different text and image modalities affect perception and reasoning in ARC-AGI tasks, finding that structured text captures coordinates well while images preserve 2D shapes, with combined modalities improving execution accuracy.


<details>
  <summary>Details</summary>
Motivation: To understand how different encoding modalities (text vs. images) shape model perception in ARC-AGI tasks and separate instruction errors from execution errors, addressing the lack of principled analysis of perceptual bottlenecks.

Method: Used a two-stage reasoning pipeline with weighted set-disagreement metric to isolate perception from reasoning across nine text and image modalities, testing how different representations affect feature perception.
...
TL;DR: ToolMind is a large-scale, high-quality tool-agentic dataset with 160k synthetic and 200k augmented data instances, featuring turn-level validation to prevent error propagation in LLM agent training.


<details>
  <summary>Details</summary>
Motivation: The scarcity of high-quality trajectories hinders LLM agent development, and existing validation methods overlook turn-level errors that propagate during training, degrading model performance.

Method: Constructed function graph based on parameter correlations, used multi-agent framework for realistic user-assistant-tool interactions, and employed fine-grained turn-level filtering to remove erroneous steps while preserving self-corrective reasoning signals.

Result: Models fine-tuned on ToolMind show significant improvements over baselines on several benchmarks.

Conclusion: ToolMind addresses limitations in existing datasets by providing high-quality, turn-validated trajectories that mitigate error amplification and enable robust tool-use learning in LLM agents.
...
TL;DR: Proposes Chain of Summaries (CoS) method inspired by Hegel's dialectical method to generate information-dense summaries of web content for LLMs, improving Q&A performance while reducing token usage.


<details>
  <summary>Details</summary>
Motivation: Address the problem that much web content is not easily digestible by LLMs due to unfriendly formats and context length limitations.

Method: Chain of Summaries (CoS) iteratively refines an initial summary (thesis) by identifying limitations through questioning (antithesis), leading to a general-purpose summary (synthesis) that satisfies current and anticipates future information needs.

Result: Outperforms zero-shot LLM baselines by up to 66% and specialized summarization methods (BRIO, PEGASUS) by up to 27% on TriviaQA, TruthfulQA, and SQUAD datasets. CoS summaries yield higher Q&A performance than source content with substantially fewer tokens.

Conclusion: CoS provides an appealing option for website maintainers to make content more accessible for LLMs while retaining human oversight capabilities.
...
TL;DR: This thesis proposes a multimodal AI framework combining text and image analysis to identify construction safety hazards, evaluating both proprietary LLMs (GPT-4o) and open-source VLMs (Molmo 7B, Qwen2 VL 2B) through case studies on OSHA reports and construction site imagery.


<details>
  <summary>Details</summary>
Motivation: Construction safety data exists in multiple formats (text reports, inspection records, site imagery), making hazard synthesis challenging with traditional approaches. A multimodal framework is needed to better identify safety hazards by combining textual and visual analysis.

Method: Two case studies: 1) Hybrid pipeline using GPT-4o and GPT-4o mini to extract insights from 28,000 OSHA accident reports (2000-2025); 2) Evaluation of lightweight open-source VLMs (Molmo 7B, Qwen2 VL 2B) on ConstructionSite10k dataset for rule-level safety violation detection using natural language prompts.

Result: Despite their smaller size, Molmo 7B and Qwen2 VL 2B showed competitive performance in certain prompt configurations, demonstrating feasibility of low-resource multimodal systems for rule-aware safety monitoring. The framework successfully identified safety hazards through combined text and image analysis.

Conclusion: Multimodal AI frameworks combining text and image analysis are feasible and effective for construction safety hazard identification, with lightweight open-source models showing competitive performance against proprietary alternatives, enabling cost-effective safety monitoring at scale.
...
TL;DR: Survey paper proposing a cognitive-based taxonomy for spatial reasoning in MLLMs, organizing tasks by reasoning complexity and cognitive functions rather than input modality.


<details>
  <summary>Details</summary>
Motivation: Existing surveys categorize spatial reasoning progress by input modality, but spatial ability is not solely determined by input format. The paper aims to provide a more principled cognitive perspective for cross-task comparisons.

Method: Introduces a taxonomy organizing spatial intelligence from cognitive aspect, divides tasks by reasoning complexity, links to cognitive functions, maps benchmarks across text/vision/embodied settings, and analyzes training/reasoning-based improvement methods.

Result: The cognitive perspective reveals critical gaps between current model capabilities and human-like reasoning, enables principled cross-task comparisons, and clarifies complementary mechanisms between different improvement approaches.

Conclusion: Provides comprehensive understanding of spatial reasoning field with actionable research directions, emphasizing the need for cognitive-based evaluation and development of spatial intelligence in MLLMs.
...
TL;DR: Thesis proposes consistency-guided cross-modal transfer for uncertainty-resilient multimodal learning, using cross-modal semantic consistency to improve robustness against noise, imperfect supervision, and modality gaps.


<details>
  <summary>Details</summary>
Motivation: Address uncertainty in multimodal learning from noisy data, low-quality labels, and heterogeneous modality characteristics, especially critical in human-computer interaction settings with varying data quality and annotation consistency.

Method: Projects heterogeneous modalities into shared latent space, uses cross-modal semantic consistency for robust representation learning, and develops strategies to enhance semantic robustness and data efficiency without large high-quality annotations.

Result: Experiments on multimodal affect-recognition benchmarks show significant improvements in model stability, discriminative ability, and robustness to noisy/incomplete supervision. Latent space analyses reveal reliable cross-modal structure capture.

Conclusion: Provides unified perspective on resilient multimodal learning by integrating uncertainty modeling, semantic alignment, and data-efficient supervision, offering practical insights for reliable brain-computer interface systems.
...
TL;DR: This paper proposes using Retrieval-Augmented Generation (RAG) and Multi-Agent Systems (MAS) to enhance LLM performance in biomechanics education, addressing knowledge gaps and complex reasoning challenges.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with domain-specific knowledge in biomechanics and decline in performance on complex multi-step reasoning tasks requiring calculations and analysis.

Method: Dual-module framework: 1) RAG for conceptual true/false questions to improve specificity and logical consistency, 2) MAS for calculation problems involving multi-step reasoning and code execution. Evaluated on Qwen-1.0-32B, Qwen-2.5-32B, and Llama-70B using 100 biomechanics questions.

Result: RAG significantly enhanced LLM performance and stability on conceptual questions, surpassing vanilla models. MAS successfully performed multi-step reasoning, equation derivation, code execution, and generated explainable solutions for calculation tasks.
...
TL;DR: Multi-agent orchestration achieves 100% actionable recommendations vs 1.7% for single-agent LLMs, with 80x better specificity and 140x better correctness, enabling production SLAs impossible with single-agent approaches.


<details>
  <summary>Details</summary>
Motivation: Single-agent LLMs generate vague, unusable recommendations for incident response, limiting their practical deployment in production systems despite their potential to accelerate response times.

Method: MyAntFarm.ai framework with 348 controlled trials comparing single-agent copilot vs multi-agent systems on identical incident scenarios, using containerized reproducible environment and introducing Decision Quality (DQ) metric.

Result: Multi-agent orchestration achieved 100% actionable recommendation rate (vs 1.7% single-agent), 80x improvement in action specificity, 140x improvement in solution correctness, and zero quality variance across all trials.
...
<details>
  <summary>Details</summary>
Motivation: There's a significant gap in assessing supply chain risks in AI systems, particularly problematic for critical applications like healthcare, utilities, and transport, where current AI governance falls short of actionable risk management.

Method: The authors survey current AI risk assessment practices and develop a taxonomy specifically for categorizing AI supply chain entities to help stakeholders systematically identify dependencies.

Result: The proposed taxonomy enables stakeholders, including those without extensive AI expertise, to systematically inventory dependencies across their organization's AI systems and ask the right questions about supply chain risks.

Conclusion: This work bridges the gap between current AI governance and the urgent need for actionable risk assessment and management of AI use in critical applications by providing a systematic approach to AI supply chain risk evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of extracting structured medical insights from unstructured clinical text in non-English contexts where NLP resources are scarce.

Method: Comparative analysis of low-compute rule-based methods and Large Language Models for information extraction from Polish EHRs, evaluating effects of text normalization and translation.

Result: Rule-based methods provide higher accuracy for patient demographics (age/sex), while LLMs excel in drug name recognition and offer greater adaptability. Translation from Polish to English caused information loss.

Conclusion: Hybrid approaches combining rule-based precision with LLM adaptability offer the most practical path for reliable and resource-efficient clinical NLP in real-world hospital settings.
...
TL;DR: IMACT-CXR is an interactive multi-agent conversational tutor for chest X-ray interpretation that combines spatial annotation, gaze analysis, knowledge retrieval, and image-grounded reasoning using AutoGen workflow with specialized agents and Bayesian Knowledge Tracing.


<details>
  <summary>Details</summary>
Motivation: To help medical trainees improve their chest X-ray interpretation skills by providing interactive tutoring that unifies multiple analysis methods in a single workflow with precise control over answer disclosure.

Method: Uses AutoGen-based multi-agent workflow with specialized agents for localization evaluation, Socratic coaching, PubMed evidence retrieval, similar case suggestion, and vision-language reasoning. Incorporates Bayesian Knowledge Tracing for skill mastery tracking, lung-lobe segmentation for gaze feedback, and safety prompts to prevent premature label disclosure.

Result: The system demonstrates responsive tutoring with bounded latency, precise control over answer leakage, and extensibility for live residency deployment. Preliminary evaluation shows improved localization and diagnostic reasoning compared to baseline methods.

Conclusion: IMACT-CXR successfully integrates multiple tutoring components into a unified system that enhances chest X-ray interpretation training while maintaining control over educational pacing and answer disclosure, showing promise for medical education applications.
...
TL;DR: Mini Amusement Parks (MAPs) is a simulator that evaluates AI agents' holistic decision-making abilities in complex business management scenarios, revealing significant performance gaps between humans and current LLM agents.


<details>
  <summary>Details</summary>
Motivation: Current AI systems struggle with interconnected real-world decision-making challenges like long-horizon planning, spatial reasoning, and active learning, while existing benchmarks isolate these capabilities rather than assessing them holistically.

Method: Developed MAPs amusement-park simulator that requires agents to model environments, anticipate long-term consequences under uncertainty, and strategically operate a complex business. Evaluated human baselines and state-of-the-art LLM agents.

Result: Humans significantly outperform LLM agents by 6.5x on easy mode and 9.8x on medium mode. Analysis reveals persistent weaknesses in long-horizon optimization, sample-efficient learning, spatial reasoning, and world modeling.

Conclusion: MAPs provides a unified foundation for benchmarking agents capable of adaptable decision making by integrating multiple real-world challenges within a single environment.
...
### [215] [Step-Audio-R1 Technical Report](https://arxiv.org/abs/2511.15848)
...
TL;DR: Step-Audio-R1 is the first successful audio reasoning model that enables meaningful reasoning in audio domains through modality-grounded reasoning chains, outperforming Gemini 2.5 Pro and matching Gemini 3 Pro.


<details>
  <summary>Details</summary>
Motivation: Audio language models paradoxically perform better with minimal reasoning, raising questions about whether audio intelligence can truly benefit from deliberate thinking like text and vision models.
...
Result: Step-Audio-R1 surpasses Gemini 2.5 Pro and achieves performance comparable to Gemini 3 Pro across comprehensive audio understanding benchmarks spanning speech, environmental sounds, and music.
...
### [216] [Decomposing Theory of Mind: How Emotional Processing Mediates ToM Abilities in LLMs](https://arxiv.org/abs/2511.15895)
*Ivan Chulo,Ananya Joshi*

Main category: cs.AI

TL;DR: Activation steering improves LLMs' Theory of Mind by enhancing emotional processing while suppressing analytical reasoning.


<details>
  <summary>Details</summary>
Motivation: To understand what internal changes occur when activation steering improves Theory of Mind in language models, as previous work showed steering helps but mechanisms remained unclear.

Method: Decomposed ToM by comparing steered vs baseline LLMs using linear probes on 45 cognitive actions, applied Contrastive Activation Addition steering to Gemma-3-4B, and evaluated on 1,000 BigToM forward belief scenarios.

Result: Improved belief attribution accuracy (32.5% to 46.7%) was mediated by enhanced emotional processing (emotion perception +2.23, emotion valuing +2.20) while suppressing analytical processes (questioning -0.78, convergent thinking -1.59).
...
### [217] [Thinking, Faithful and Stable: Mitigating Hallucinations in LLMs](https://arxiv.org/abs/2511.15921)
*Chelsea Zou,Yiheng Yao,Basant Khalil*

Main category: cs.AI

TL;DR: A self-correcting framework for LLMs that detects hallucinations during multi-step reasoning using fine-grained uncertainty signals and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: To improve LLMs' reasoning reliability by detecting and mitigating hallucinations in real-time during multi-step reasoning processes, rather than just focusing on final answer correctness.

Method: Uses fine-grained uncertainty signals (self-assessed confidence alignment and token-level entropy spikes) to detect unreliable reasoning, and applies reinforcement learning with a composite reward function that penalizes unjustified high confidence and entropy spikes while encouraging stable reasoning.

Result: The method improves both final answer accuracy and reasoning calibration, with ablations confirming the individual contributions of each uncertainty signal.

Conclusion: The self-correcting framework successfully makes LLMs more introspective and improves the coherence and faithfulness of intermediate reasoning steps through confidence-aware reward feedback.
...
### [218] [JudgeBoard: Benchmarking and Enhancing Small Language Models for Reasoning Evaluation](https://arxiv.org/abs/2511.15958)
*Zhenyu Bi,Gaurav Srivastava,Yang Li,Meng Lu,Swastik Roy,Morteza Ziyadi,Xuan Wang*

Main category: cs.AI

TL;DR: JudgeBoard is a novel evaluation pipeline that directly queries models to assess answer correctness without requiring answer comparisons, using multi-agent judging (MAJ) to improve SLM judgment performance.


<details>
  <summary>Details</summary>
Motivation: Current LLM-as-a-judge frameworks rely on indirect answer comparisons with predefined metrics, which are difficult to automate and offer limited support for fine-grained evaluation of reasoning outputs.

Method: Proposed JudgeBoard pipeline with MAJ framework that uses multiple interacting SLMs with distinct reasoning profiles for collaborative deliberation, evaluated across mathematical and science/commonsense reasoning domains using accuracy-based ranking and Elo-based rating.

Result: Significant performance gap between SLMs and LLMs in isolated judging tasks, but MAJ substantially improves SLM reliability and consistency. On MATH dataset, MAJ with smaller models performs comparably or better than larger counterparts.

Conclusion: Multi-agent SLM systems can potentially match or exceed LLM performance in judgment tasks, enabling scalable and efficient assessment without requiring large models.
...
### [219] [KRAL: Knowledge and Reasoning Augmented Learning for LLM-assisted Clinical Antimicrobial Therapy](https://arxiv.org/abs/2511.15974)
*Zhe Li,Yehan Qiu,Yujie Chen,Xiang Zhou*

Main category: cs.AI

TL;DR: KRAL is a knowledge and reasoning augmented learning paradigm that enhances LLMs for clinical antimicrobial therapy decisions through teacher-model reasoning, heuristic learning, and agentic reinforcement learning, achieving better performance than RAG and SFT at lower costs.


<details>
  <summary>Details</summary>
Motivation: Address limitations of LLMs in high-stakes clinical decision-making including knowledge gaps, data privacy concerns, high deployment costs, and limited reasoning capabilities.

Method: Leverages teacher-model reasoning for knowledge distillation via answer-to-question reverse generation, heuristic learning for semi-supervised data augmentation (reducing manual annotation by ~80%), and agentic reinforcement learning to enhance medical knowledge and reasoning while optimizing computational efficiency.

Result: Outperforms traditional RAG and SFT methods: improves knowledge QA capability (Accuracy@1 on MEDQA increased by 1.8% vs. SFT and 3.6% vs. RAG) and reasoning capability (Pass@1 on PUMCH Antimicrobial increased by 27% vs. SFT and 27.2% vs. RAG) at ~20% of SFT's long-term training costs.
...
### [220] [Detecting Sleeper Agents in Large Language Models via Semantic Drift Analysis](https://arxiv.org/abs/2511.15992)
*Shahin Zanbaghi,Ryan Rostampour,Farhan Abid,Salim Al Jarmakani*

Main category: cs.AI

TL;DR: A dual-method detection system combining semantic drift analysis and canary baseline comparison achieves 92.5% accuracy in identifying backdoored LLMs in real-time.


<details>
  <summary>Details</summary>
Motivation: LLMs can be backdoored as "sleeper agents" that appear safe during training but exhibit malicious behavior during deployment, with no existing practical detection methods.
...
Result: Achieved 92.5% accuracy with 100% precision (zero false positives) and 85% recall on the dolphin-llama3-8B sleeper agent model, operating in real-time (<1s per query).

Conclusion: Provides the first practical solution to LLM backdoor detection, addressing a critical security gap without requiring model modifications or sacrificing deployment efficiency.
...
### [221] [CARE-RAG - Clinical Assessment and Reasoning in RAG](https://arxiv.org/abs/2511.15994)
*Deepthi Potluri,Aby Mammen Mathew,Jeffrey B DeWitt,Alexander L. Rasgon,Yide Hao,Junyuan Hong,Ying Ding*

Main category: cs.AI

TL;DR: LLMs struggle to reason correctly with retrieved evidence, especially in clinical settings. Using WET guidelines, the study shows persistent errors even with authoritative sources and proposes an evaluation framework for accuracy, consistency, and fidelity.


<details>
  <summary>Details</summary>
Motivation: There's a critical gap between retrieval and reasoning in LLMs, particularly concerning in clinical settings where outputs must follow structured protocols to ensure patient safety and treatment effectiveness.

Method: Used Written Exposure Therapy (WET) guidelines as a testbed, evaluated model responses to clinician-vetted questions, and proposed an evaluation framework measuring accuracy, consistency, and fidelity of reasoning.

Result: Found that errors persist even when authoritative passages are provided, showing that retrieval-augmented generation (RAG) can constrain outputs but safe deployment requires rigorous reasoning assessment.

Conclusion: Safe deployment of LLMs in clinical settings requires assessing reasoning as rigorously as retrieval, highlighting both the potential and risks of RAG systems.
...
### [222] [Sensorium Arc: AI Agent System for Oceanic Data Exploration and Interactive Eco-Art](https://arxiv.org/abs/2511.15997)
...
<details>
  <summary>Details</summary>
Motivation: To reimagine ocean data as a living narrative rather than abstract datasets, inspired by eco-aesthetic philosophy and aiming to provide affective, intuitive access to environmental data through conversational AI.

Method: Built on a modular multi-agent system and retrieval-augmented LLM framework with keyword detection and semantic parsing to dynamically trigger data visualizations and audiovisual content based on dialogue cues like time, location, and themes.

Result: Developed a real-time multimodal interactive system that enables natural spoken conversations with AI agents embodying the ocean's perspective, blending scientific insight with ecological poetics.
...
### [223] [MUSEKG: A Knowledge Graph Over Museum Collections](https://arxiv.org/abs/2511.16014)
*Jinhao Li,Jianzhong Qi,Soyeon Caren Han,Eun-Jung Holden*

Main category: cs.AI

TL;DR: MuseKG is a knowledge-graph framework that integrates structured and unstructured museum data through symbolic-neural integration, enabling natural language queries and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Digital transformation in cultural heritage has created fragmented artefact data collections, and existing museum information systems struggle to integrate heterogeneous metadata, unstructured documents, and multimodal artefacts into a coherent queryable form.
...
### [224] [SpellForger: Prompting Custom Spell Properties In-Game using BERT supervised-trained model](https://arxiv.org/abs/2511.16018)
...
<details>
  <summary>Details</summary>
Motivation: To explore AI as a core gameplay co-creation tool for dynamic content generation and player creativity, moving beyond traditional AI applications in games.

Method: Uses a supervised-trained BERT model to interpret natural language prompts and map them to spell prefabs with balanced parameters (damage, cost, effects), implemented in Unity with Python backend.

Result: Expected to deliver a functional prototype demonstrating real-time spell generation within an engaging gameplay loop that centers player creativity.

Conclusion: Validates the use of AI as a direct gameplay mechanic for personalized spell creation and enhanced player agency in game design.
...
### [225] [An Aligned Constraint Programming Model For Serial Batch Scheduling With Minimum Batch Size](https://arxiv.org/abs/2511.16045)
...
<details>
  <summary>Details</summary>
Motivation: Existing CP models for serial batch scheduling with minimum batch sizes rely on predefined virtual batches that suffer from dimensionality issues and complexity, especially in practical applications like semiconductor manufacturing.

Method: Proposes a CP model with key alignment parameters to reason directly on same-family job sequences, enhanced with tailored search phases and strengthened constraint propagator inference levels.

Result: The model outperforms existing methods (MIP, tabu search, CP) on small-to-medium instances (up to 100 jobs) and finds solutions up to 25% better on large-scale instances (up to 500 jobs, 10 families, 10 machines).

Conclusion: The novel CP approach provides a more compact formulation that effectively handles serial batch scheduling with minimum batch sizes, demonstrating significant performance improvements over state-of-the-art methods.
...
### [226] [Artificial Intelligence and Accounting Research: A Framework and Agenda](https://arxiv.org/abs/2511.16055)
*Theophanis C. Stratopoulos,Victor Xiaoqi Wang*

Main category: cs.AI

TL;DR: This paper proposes a framework to classify AI-accounting research and analyzes how generative AI transforms accounting research, creating both opportunities and competitive threats for scholars.


<details>
  <summary>Details</summary>
Motivation: Recent advances in generative AI and large language models are fundamentally transforming accounting research, creating both opportunities and competitive threats that need systematic analysis.

Method: The authors develop a classification framework with two dimensions (research focus: accounting-centric vs AI-centric; methodological approach: AI-based vs traditional methods) and apply it to analyze papers from IJAIS special issue and leading accounting journals.

Result: The framework reveals strategic positioning opportunities for accounting researchers and shows that while GenAI democratizes certain research capabilities, it intensifies competition by raising expectations for higher-order contributions where human judgment remains valuable.

Conclusion: These shifts call for reforming doctoral education to cultivate comparative advantages while building AI fluency, as human creativity and theoretical depth remain valuable in the AI-transformed research landscape.
...
### [227] [A Hybrid Proactive And Predictive Framework For Edge Cloud Resource Management](https://arxiv.org/abs/2511.16075)
*Hrikshesh Kumar,Anika Garg,Anshul Gupta,Yashika Agarwal*

Main category: cs.AI

TL;DR: Proactive cloud-edge workload management system combining CNN-LSTM forecasting with multi-agent DRL for optimized resource allocation, outperforming traditional reactive methods.


<details>
  <summary>Details</summary>
Motivation: Traditional reactive resource management with static thresholds leads to overspending or performance issues. Need proactive solutions that anticipate problems rather than reacting to them.

Method: Hybrid architecture embedding CNN-LSTM time series forecasting directly into multi-agent Deep Reinforcement Learning state space, enabling the AI manager to see future workload patterns.

Result: System significantly outperforms traditional methods, effectively handling complex multi-objective optimization (cost, performance, reliability) and finding optimal long-term resource allocation strategies.

Conclusion: The novel integration of predictive forecasting with DRL creates a smarter AI manager that anticipates future needs, enabling smooth, optimized resource management rather than reactive problem-solving.
...
### [228] [SkyRL-Agent: Efficient RL Training for Multi-turn LLM Agent](https://arxiv.org/abs/2511.16108)
...
TL;DR: SkyRL-Agent is a framework for efficient multi-turn agent training with async dispatching and tool integration. It trained SA-SWE-32B, a software engineering agent that achieves 39.4% Pass@1 on SWE-Bench with 2x cost reduction.


<details>
  <summary>Details</summary>
Motivation: To create an efficient framework for training and evaluating multi-turn, long-horizon agents with better performance and lower costs than existing approaches.

Method: Uses optimized asynchronous pipeline dispatcher (1.55x speedup), tool-enhanced training with AST-based search for code navigation, and flexible backend interoperability with RL frameworks.

Result: SA-SWE-32B achieves 39.4% Pass@1 on SWE-Bench Verified with >2x cost reduction, and generalizes well to other tasks like Terminal-Bench, BrowseComp-Plus, and WebArena despite being trained only on SWE tasks.

Conclusion: SkyRL-Agent enables efficient agent training with significant performance improvements and cost reductions, while demonstrating strong generalization capabilities and extensibility across different agent types.
...
### [229] [Multidimensional Rubric-oriented Reward Model Learning via Geometric Projection Reference Constraints](https://arxiv.org/abs/2511.16139)
*Yongnan Jin,Xurui Li,Feng Cao,Liucun Gao,Juanjuan Yao*

Main category: cs.AI

TL;DR: MR-RML is a novel alignment framework that addresses LLM limitations in medical practice through structured medical standards integration, multi-dimensional reward modeling, and geometric constraints for improved clinical reasoning.


<details>
  <summary>Details</summary>
Motivation: Current LLMs face critical alignment challenges in medical practice: disconnect from dynamic clinical needs, difficulty adapting to evolving medical standards, and inability of conventional reward models to capture nuanced medical quality criteria.
...
Result: Achieves substantial performance gains over base Qwen-32B (45% on full subset, 85% on Hard subset), SOTA among open-source LLMs with scores of 62.7 (full) and 44.7 (hard), outperforming most closed-source models on Healthbench benchmark.

Conclusion: The framework successfully bridges the gap between LLMs and clinical needs by integrating medical standards throughout training, enabling more consistent and cost-effective alignment with nuanced medical quality criteria.
...
### [230] [FOOTPASS: A Multi-Modal Multi-Agent Tactical Context Dataset for Play-by-Play Action Spotting in Soccer Broadcast Videos](https://arxiv.org/abs/2511.16183)
*Jeremie Ochin,Raphael Chekroun,Bogdan Stanciulescu,Sotiris Manitsaris*

Main category: cs.AI

TL;DR: FOOTPASS is the first benchmark for play-by-play action spotting in soccer that combines computer vision outputs with tactical knowledge to generate reliable play-by-play data.
...
Method: Leverages tactical knowledge as prior to support computer-vision-based predictions, integrating both STAD and MOT in a multi-modal, multi-agent tactical context.

Result: Created FOOTPASS dataset enabling development of player-centric action spotting methods that exploit computer vision outputs and soccer tactical regularities.

Conclusion: FOOTPASS enables more automated and reliable extraction of play-by-play data streams essential for data-driven sports analytics.
...
### [231] [From Performance to Understanding: A Vision for Explainable Automated Algorithm Design](https://arxiv.org/abs/2511.16201)
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based automated algorithm design focuses on performance but lacks transparency - it doesn't reveal why algorithms work, which components matter, or how design choices relate to problem structures.
...
Conclusion: This integration will accelerate progress while producing reusable scientific insight into when and why optimization strategies succeed, moving beyond performance-driven automation to understanding-driven design.
...
### [232] [Multi-Agent Collaborative Reward Design for Enhancing Reasoning in Reinforcement Learning](https://arxiv.org/abs/2511.16202)
*Pei Yang,Ke Zhang,Ji Wang,Xiao Chen,Yuxin Tang,Eric Yang,Lynn Ai,Bill Shi*

Main category: cs.AI

TL;DR: CRM replaces single reward models with a team of specialist evaluators for more robust and interpretable RLHF, using multi-agent collaboration to handle conflicting preferences.


<details>
  <summary>Details</summary>
Motivation: Conventional reward models struggle with multiple conflicting preference dimensions and lack transparency in scoring decisions.

Method: Decomposes preference evaluation into domain-specific agents with partial signals, uses global evaluators, and fuses signals through a centralized aggregator with advantage-based policy optimization.

Result: Enables multi-perspective reward shaping without additional human annotations, providing more transparent reward modeling and stable optimization.

Conclusion: CRM and rewardBench offer a practical, modular approach to improve reward modeling transparency and optimization stability in RLHF.
...
### [233] [ChemLabs on ChemO: A Multi-Agent System for Multimodal Reasoning on IChO 2025](https://arxiv.org/abs/2511.16205)
*Xu Qiang,Shengyuan Bai,Leqing Chen,Zijing Liu,Yu Li*

Main category: cs.AI

TL;DR: ChemO is a new benchmark from the International Chemistry Olympiad 2025 that addresses chemistry's multimodal challenges through Assessment-Equivalent Reformulation (AER) for automated evaluation and Structured Visual Enhancement (SVE) to separate visual perception from chemical reasoning.
...
Method: Proposed ChemLabs, a hierarchical multi-agent framework with specialized agents for problem decomposition, perception, reasoning, and auditing, mimicking human expert collaboration.

Result: Combining SVE with the multi-agent system yields dramatic performance gains, achieving 93.6/100 score that surpasses estimated human gold medal threshold and establishes new state-of-the-art in automated chemical problem-solving.

Conclusion: ChemO benchmark and ChemLabs framework successfully address the challenge of automated chemical reasoning, demonstrating that specialized multi-agent systems can achieve human-expert-level performance in complex chemistry problem-solving.
...
### [234] [FlipVQA-Miner: Cross-Page Visual Question-Answer Mining from Textbooks](https://arxiv.org/abs/2511.16216)
*Zhen Hao Wong,Jingwen Deng,Hao Liang,Runming He,Chengyu Shen,Wentao Zhang*

Main category: cs.AI

TL;DR: Automated pipeline extracts high-quality QA/VQA pairs from educational documents using layout-aware OCR and LLM-based semantic parsing, enabling scalable use of real-world educational content for LLM training.


<details>
  <summary>Details</summary>
Motivation: Existing instruction-tuning and RL datasets are costly and rely on synthetic samples with hallucination and limited diversity, while textbooks contain abundant high-quality QA content that's difficult to extract from raw PDFs.

Method: Combines layout-aware OCR with LLM-based semantic parsing to automatically extract well-formed QA and visual-QA pairs from educational documents.
...
Conclusion: Provides a practical alternative to synthetic data generation for improving reasoning-oriented LLM training, with all code and data-processing pipelines open-sourced.
...
### [235] [Revisiting Fairness-aware Interactive Recommendation: Item Lifecycle as a Control Knob](https://arxiv.org/abs/2511.16248)
...
<details>
  <summary>Details</summary>
Motivation: To address fairness in interactive recommendation by incorporating item lifecycle patterns, which were found to follow a compressed three-phase pattern (rapid growth, transient stability, sharp decay) rather than the classical four-stage model.

Method: LHRL framework with two components: PhaseFormer (lightweight encoder combining STL decomposition and attention for phase detection) and a two-level HRL agent (high-level policy imposes phase-aware fairness constraints, low-level policy optimizes user engagement).

Result: Experiments on multiple real-world datasets show LHRL significantly improves both fairness and user engagement, and integrating lifecycle-aware rewards into existing RL models consistently yields performance gains.

Conclusion: The approach effectively reconciles long-term equity and short-term utility in interactive recommendation, demonstrating generalizability and practical value through lifecycle-aware optimization.
...
### [236] [MuISQA: Multi-Intent Retrieval-Augmented Generation for Scientific Question Answering](https://arxiv.org/abs/2511.16283)
...
Result: Experiments on MuISQA and other RAG datasets show the method consistently outperforms conventional approaches, particularly in retrieval accuracy and evidence coverage.

Conclusion: The proposed intent-aware retrieval framework effectively addresses the limitations of single-intent RAG systems by improving evidence coverage across multiple sub-questions in complex scientific QA tasks.
...
### [237] [Distributed Agent Reasoning Across Independent Systems With Strict Data Locality](https://arxiv.org/abs/2511.16292)
*Daniel Vaughan,Kateřina Vaughan*

Main category: cs.AI

TL;DR: Proof-of-concept for agent-to-agent communication across distributed systems using natural language messages without shared identifiers, structured schemas, or centralized data exchange.


<details>
  <summary>Details</summary>
Motivation: To demonstrate how multiple organizations can cooperate securely via pseudonymised tokens and local data lookups while maintaining privacy and operational boundaries.

Method: Uses Orpius platform for multi-agent orchestration with OperationRelay calls for natural-language communication. Agents operate on local data with HMAC-based pseudonymous tokens, coverage rule evaluation, and specialist consultations.

Result: Successfully demonstrated feasibility of distributed reasoning among specialized agents while keeping data local to each organization, though no clinical validation was performed.

Conclusion: The prototype highlights architectural patterns and privacy considerations for decentralized multi-agent systems, outlining opportunities for future research and more rigorous evaluation.
...
### [238] [OpenMMReasoner: Pushing the Frontiers for Multimodal Reasoning with an Open and General Recipe](https://arxiv.org/abs/2511.16334)
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of transparent and reproducible data curation and training strategies in multimodal reasoning research, which hinders scalable progress despite recent advancements.

Method: Two-stage approach: 1) SFT stage with 874K-sample cold-start dataset featuring step-by-step validation, 2) RL stage with 74K-sample dataset across diverse domains to sharpen and stabilize reasoning abilities.
...
Conclusion: The work establishes a solid empirical foundation for large-scale multimodal reasoning research and highlights the critical importance of data quality and training design, with all code, pipeline, and data open-sourced.
...
### [239] [Reducing Instability in Synthetic Data Evaluation with a Super-Metric in MalDataGen](https://arxiv.org/abs/2511.16373)
*Anna Luiza Gomes da Silva,Diego Kreutz,Angelo Diniz,Rodrigo Mansilha,Celso Nobre da Fonseca*

Main category: cs.AI

TL;DR: Proposes a Super-Metric for evaluating synthetic Android malware data quality by aggregating eight metrics across four fidelity dimensions into a single weighted score.


<details>
  <summary>Details</summary>
Motivation: Existing metrics for synthetic Android malware data evaluation are unstable and lack standardization, making quality assessment challenging.
...
Conclusion: The Super-Metric provides a more reliable and standardized approach for evaluating synthetic Android malware data quality compared to traditional individual metrics.
...
### [240] [An Agent-Based Framework for the Automatic Validation of Mathematical Optimization Models](https://arxiv.org/abs/2511.16383)
*Alexander Zadorojniy,Segev Wasserkrug,Eitan Farchi*

Main category: cs.AI

TL;DR: Proposes an agent-based method for automatic validation of optimization models generated by LLMs, extending software testing techniques to optimization modeling.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of validating that optimization models generated from natural language descriptions are correct and meet requirements, as manual validation is difficult.

Method: Uses multiple agents to: 1) generate problem-level testing API, 2) create tests using this API, and 3) generate optimization-specific mutations to assess test suite quality.

Result: The framework provides high-quality validation measured by mutation coverage, a standard software testing metric.

Conclusion: The proposed agent-based validation method effectively addresses the verification challenge for LLM-generated optimization models using established software testing principles.
...
### [241] [CorrectHDL: Agentic HDL Design with LLMs Leveraging High-Level Synthesis as Reference](https://arxiv.org/abs/2511.16395)
...
<details>
  <summary>Details</summary>
Motivation: Address LLM hallucination issues that introduce functional errors in generated HDL designs, while leveraging LLMs' potential for hardware design.

Method: Uses C/C++ program input, LLM-generated HDL with RAG for syntax repair, iterative functional improvement by comparing with HLS reference design simulation.

Result: Generated circuits achieve significantly better area/power efficiency than conventional HLS designs, approaching human-engineered circuit quality while maintaining correctness.

Conclusion: Demonstrates effective agentic HDL design combining LLM generative capabilities with traditional correctness-driven IC design flows.
...
TL;DR: Bauplan is an agent-first lakehouse design that addresses trust issues in agentic workflows by implementing data and compute isolation through transactional infrastructure, enabling trustworthy agent operations on production data.


<details>
  <summary>Details</summary>
Motivation: Most enterprises don't trust AI agents with production data due to infrastructure limitations, as traditional lakehouses are not suited for agent access patterns.

Method: Proposed Bauplan - an agent-first lakehouse design that reimplements data and compute isolation, drawing operational analogies to MVCC but adapted for decoupled, multi-language environments.
...
Conclusion: The path to trustworthy agentic workflows begins with solving the infrastructure problem first, as designing around transactions enables proper governance to follow.
...
<details>
  <summary>Details</summary>
Motivation: ESG reports present significant challenges for large-scale understanding due to chaotic reading order from irregular layouts and implicit hierarchies in lengthy, weakly structured content, which hinders effective ESG integration in financial governance.

Method: The framework integrates reading-order modeling based on layout flow, hierarchy-aware segmentation guided by table-of-contents anchors, and multimodal aggregation that transforms visual elements into coherent natural language with ESG, GRI, and sentiment labels.

Result: Extensive experiments show Pharos-ESG consistently outperforms both dedicated document parsing systems and general-purpose multimodal models. The authors also release Aurora-ESG, the first large-scale public ESG report dataset spanning multiple markets.

Conclusion: Pharos-ESG successfully addresses the challenges of ESG report analysis and provides structured representations that better support ESG integration in financial governance and decision-making, with the released dataset further advancing the field.
...
TL;DR: TOFA is a training-free one-shot federated adaptation framework for Vision-Language Models that addresses communication costs and data heterogeneity without requiring additional client/server training resources.


<details>
  <summary>Details</summary>
Motivation: To overcome high communication costs and security vulnerabilities in iterative federated VLM adaptation, while addressing challenges in multimodal information exploitation, data heterogeneity handling, and resource requirements in existing one-shot methods.

Method: Uses dual visual and textual pipelines: hierarchical Bayesian model for personalized visual prototypes, global alignment of local text prompts, and adaptive weight calibration to balance personalization and robustness.
...
Conclusion: TOFA provides an efficient, training-free solution for federated VLM adaptation that handles data heterogeneity while minimizing communication overhead and resource requirements.
...
TL;DR: The paper argues that generative AI principles from ML should be investigated for their relevance to brain function and cognitive neuroscience, highlighting five key ML concepts that neuroscience could learn from.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between machine learning advances and neuroscience by exploring how generative principles and ML characterizations of neural information processing could inform our understanding of brain function.

Method: The authors discuss and analyze five specific examples from ML research: shortcomings of world modelling, generation of thought processes, attention mechanisms, neural scaling laws, and quantization techniques.

Result: The paper identifies concrete ML concepts and generative principles that have proven successful in AI applications and suggests their potential relevance for understanding brain function.

Conclusion: Neuroscience could significantly benefit from studying ML research findings, particularly generative principles and neural information processing characterizations, to advance our understanding of cognitive processes in the brain.
...
TL;DR: PersonaDrift is a synthetic benchmark for evaluating ML methods to detect progressive communication changes in dementia patients, simulating 60-day interaction logs with caregiver-informed personas.


<details>
  <summary>Details</summary>
Motivation: Current computational tools don't track gradual behavioral drift in dementia patients' communication, which caregivers notice informally but can't quantify systematically.

Method: Created synthetic benchmark with 60-day interaction logs using caregiver-informed personas, injected progressive changes (flattened sentiment and semantic drift), and evaluated various detection methods including statistical models, sequence models, and classifiers.
...
TL;DR: CLOZE is a zero-shot framework that uses LLMs to automatically extract medical entities from clinical notes and integrate them into hierarchical medical ontologies, providing accurate, scalable, and privacy-preserving ontology extension.


<details>
  <summary>Details</summary>
Motivation: Clinical notes contain valuable medical insights but are underutilized for ontology extension. The goal is to enhance existing ontologies' coverage and utility for biomedical research and clinical applications by leveraging unstructured clinical data.

Method: Uses large language models (LLMs) to automatically extract medical entities from clinical notes and integrate them into hierarchical ontologies. The zero-shot approach requires no additional training or labeled data and includes automated PHI removal for privacy protection.

Result: Experimental results show CLOZE provides accurate, scalable, and privacy-preserving ontology extension. It effectively identifies disease-related concepts and captures complex hierarchical relationships using LLMs' language understanding and biomedical knowledge.

Conclusion: CLOZE demonstrates strong potential for supporting downstream applications in biomedical research and clinical informatics, offering a cost-efficient solution for ontology extension from clinical notes without compromising patient privacy.
...
TL;DR: A taxonomical framework for classifying challenges to digital AI consciousness, distinguishing levels of granularity (Marr's levels) and degrees of force from functionalism challenges to strict impossibility claims.


<details>
  <summary>Details</summary>
Motivation: To provide structure for analyzing debates about AI consciousness by disambiguating between challenges to computational functionalism and challenges to digital consciousness, and between different ways of parsing such challenges.

Method: Developed a framework with levels of granularity (based on Marr's levels) and three degrees of force, then applied it to 14 examples from scientific and philosophical literature.

Result: Created a systematic tool for classifying consciousness challenges that distinguishes between functionalism critiques (degree 1), practical improbability claims (degree 2), and strict impossibility arguments (degree 3).

Conclusion: The framework provides structure for the AI consciousness debate without taking sides, enabling clearer disambiguation between different types of challenges and their intended targets.
...
TL;DR: The paper addresses limitations in case-based reasoning networks' explanations by proposing Abductive Latent Explanations (ALEs), which provide formal guarantees for interpretability by combining case-based reasoning with formal XAI methods.


<details>
  <summary>Details</summary>
Motivation: Case-based reasoning networks provide explanations by pointing to prototypes, but these explanations can be misleading and lack formal guarantees, which limits their usefulness in safety-critical applications.

Method: Proposed Abductive Latent Explanations (ALEs) - a formalism that expresses sufficient conditions on latent representations that imply predictions. Developed a solver-free, scalable algorithm for generating ALEs using three distinct paradigms.

Result: Demonstrated the feasibility of ALEs on diverse datasets for both standard and fine-grained image classification, showing improved explanation reliability compared to traditional case-based reasoning explanations.

Conclusion: ALE combines the interpretability of case-based reasoning with formal XAI guarantees, providing more reliable explanations that are particularly valuable for safety-critical applications.
...
TL;DR: D-GARA is a dynamic benchmarking framework that evaluates Android GUI agents' robustness against real-world anomalies like permission dialogs and battery warnings, showing significant performance degradation in current agents.
...
Result: State-of-the-art GUI agents show substantial performance degradation when exposed to anomaly-rich environments, demonstrating the need for robustness-aware learning.

Conclusion: D-GARA is a modular, extensible framework that can integrate new tasks and anomaly types to better evaluate GUI agent robustness in real-world scenarios.
...
Method: YOFO uses a structured requirement template and produces binary yes/no decisions for each requirement in one inference step by reading the logits of the final token associated with each requirement.

Result: Extensive experiments show YOFO achieves state-of-the-art results on standard recommendation datasets, supports dependency-aware analysis, and benefits from post-hoc CoT.

Conclusion: YOFO provides an efficient and interpretable solution for MLLM-based judgment that overcomes the speed-accuracy trade-off in existing approaches.
...
TL;DR: DPPO is a metacognitive training framework that alternates between supervised fine-tuning and reinforcement learning to efficiently learn from sparse data, achieving 20.3% performance improvement over base models.


<details>
  <summary>Details</summary>
Motivation: Address the embodied data bottleneck (scarce real-world data) and algorithmic inefficiency of existing methods that are resource-prohibitive.

Method: Deliberate Practice Policy Optimization (DPPO) - a metacognitive Metaloop framework that dynamically alternates between supervised fine-tuning (competence expansion) and reinforcement learning (skill refinement) for automatic weakness identification and targeted resource allocation.

Result: Training Pelican-VL 1.0 with DPPO yields 20.3% performance improvement over base model and surpasses open-source 100B-parameter models by 10.6%.

Conclusion: DPPO provides the first systematic framework that alleviates data and resource bottlenecks, enabling efficient development of versatile embodied agents.
...
TL;DR: MedBayes-Lite is a lightweight Bayesian enhancement for clinical transformers that adds uncertainty quantification without retraining, improving calibration and reducing diagnostic errors by flagging uncertain predictions.
...
Result: Reduces overconfidence by 32-48%, improves calibration across biomedical QA benchmarks (MedQA, PubMedQA, MIMIC-III), and prevents up to 41% of diagnostic errors by flagging uncertain predictions.

Conclusion: MedBayes-Lite effectively enables reliable uncertainty propagation and improves interpretability in medical AI systems with minimal parameter overhead.
...
TL;DR: Implementation of AI-based algorithmic trading system for EUR-USD Forex pair using fundamental and technical analysis features, with comparative evaluation of feature effectiveness.


<details>
  <summary>Details</summary>
Motivation: To develop an advanced AI trading system for high-frequency Forex trading and determine which type of analysis (fundamental vs technical) provides better predictive capacity for profitable signals.

Method: Integrates macroeconomic fundamental variables (GDP, Unemployment Rate from Euro Zone and US) with technical indicators (oscillators, Fibonacci levels, price divergences). Uses machine learning metrics for predictive accuracy and backtesting simulations on historical data.

Result: Performance evaluated using standard ML metrics and backtesting simulations to assess trading profitability and risk.

Conclusion: Comparative analysis reveals which input feature class (fundamental or technical) provides greater and more reliable predictive capacity for generating profitable trading signals.
...
TL;DR: This paper analyzes differences between human and LLM reasoning by developing a cognitive taxonomy and evaluation framework, revealing systematic structural differences and proposing test-time guidance that improves model performance by up to 60%.


<details>
  <summary>Details</summary>
Motivation: To understand why large language models fail on simpler problems despite solving complex ones, suggesting they use fundamentally different reasoning mechanisms than humans, and to bridge cognitive science with LLM research.

Method: Developed a taxonomy of 28 cognitive elements, analyzed 170K reasoning traces from 17 models across text/vision/audio modalities plus 54 human think-aloud traces, and conducted meta-analysis of 1,598 LLM reasoning papers.

Result: Found systematic structural differences: humans use hierarchical nesting and meta-cognitive monitoring while models rely on shallow forward chaining. Research community focuses on easily quantifiable behaviors while neglecting meta-cognitive controls that correlate with success.

Conclusion: Models have behavioral repertoires associated with success but don't deploy them spontaneously. Test-time reasoning guidance that scaffolds successful structures improves performance by up to 60%, establishing foundation for models that reason through principled cognitive mechanisms.
...
### [256] [Fluid Reconfigurable Intelligent Surface (FRIS) Enabling Secure Wireless Communications](https://arxiv.org/abs/2511.15860)
*Xusheng Zhu,Kai-Kit Wong,Boyi Tang,Wen Chen,Chan-Byoung Chae*

Main category: cs.IT

TL;DR: FRIS enhances physical-layer security by enabling dynamic element positioning from candidate locations, outperforming conventional RIS through joint optimization of beamforming, element selection, and phase shifts.


<details>
  <summary>Details</summary>
Motivation: To improve physical-layer security in wireless communications where conventional RIS with fixed elements has limitations, by introducing fluid reconfigurability that provides additional degrees of freedom.

Method: Proposes an alternating optimization framework combining generalized eigenvalue method for beamforming and cross-entropy optimization for joint element selection and discrete phase design.

Result: Simulation results show FRIS significantly outperforms conventional RIS and other baselines, demonstrating substantial security gains from element positioning as a new degree of freedom.

Conclusion: FRIS provides enhanced physical-layer security capabilities compared to conventional RIS by leveraging dynamic element positioning as an additional optimization dimension.
