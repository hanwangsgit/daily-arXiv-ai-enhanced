...
Motivation: Existing Vision-Language Models struggle with evidence localization in long visual documents, leading to poor performance and model hallucination due to inability to retrieve relevant pages and fine-grained details.

Method: Proposes DocLens framework that effectively "zooms in" on evidence by navigating from full document to specific visual elements on relevant pages, then uses sampling-adjudication mechanism to generate reliable answers.

Result: Paired with Gemini-2.5-Pro, achieves state-of-the-art performance on MMLongBench-Doc and FinRAGBench-V, surpassing human experts, particularly on vision-centric and unanswerable queries.

Conclusion: DocLens demonstrates superior evidence localization capabilities through its multi-agent framework, showing particular strength in handling vision-centric and unanswerable queries in long visual document comprehension.
...
Method: LARM extends LVSM (novel view synthesis for static objects) to articulated objects by jointly reasoning over camera pose and articulation variation using transformer-based architecture. It generates auxiliary outputs like depth maps and part masks for explicit 3D mesh extraction and joint estimation.
...
Conclusion: The framework eliminates the need for dense supervision and enables scalable, high-fidelity reconstruction of 3D articulated objects from sparse-view images.
...
### [3] [Detection in Bistatic ISAC with Deterministic Sensing and Gaussian Information Signals](https://arxiv.org/abs/2511.10897)
*Xianxin Song,Xianghao Yu,Jie Xu,Derrick Wing Kwan Ng*

Main category: eess.SP

TL;DR: This paper proposes a Neyman-Pearson detector for bistatic ISAC systems that leverages both deterministic sensing and random communication signals, with optimized beamforming to maximize detection probability under communication constraints.


<details>
  <summary>Details</summary>
Motivation: Integrated sensing and communications (ISAC) is crucial for 6G networks, but existing approaches often treat communication signals as interference for sensing, limiting performance. The paper aims to develop a detector that effectively utilizes both signal types.

Method: Developed NP-based detector using both deterministic sensing and Gaussian communication signals. Used semi-definite relaxation and successive convex approximation to optimize transmit beamforming under SINR and power constraints.

Result: Simulations show the proposed NP detector outperforms benchmarks that treat information signals as interference. Higher communication-rate thresholds reduce deterministic signal power and weaken detection performance.

Conclusion: Both deterministic sensing and random communication signals contribute to improved detection performance in ISAC systems. Optimal beamforming balances communication and sensing requirements, but communication constraints can limit sensing capabilities.
...
TL;DR: A blockage-aware hierarchical beamforming framework for movable antenna systems that improves energy efficiency and reduces beam training overhead through adaptive codebook design and two-stage optimization.


<details>
  <summary>Details</summary>
Motivation: Existing MA systems neglect specialized codebook design and fail to address increased energy consumption and latency issues, limiting their practical deployment potential.

Method: Integrated blockage detection into codebook design using Gerchberg-Saxton algorithm, and used two-stage approach for joint beamforming and RIS optimization to reduce complexity.

Result: Simulations show improved Energy Efficiency (EE) and reduced beam training overhead, substantially boosting practical deployment potential.

Conclusion: The proposed framework successfully addresses key limitations of MA systems and enhances their viability for future wireless networks.
...
### [5] [Sensitivity of Finite Element Models to Relationship Between T2 Relaxation and Modulus in Articular Cartilage](https://arxiv.org/abs/2511.10947)
*Alexander A. Donabedian,Deva D. Chan*

Main category: eess.IV

TL;DR: Evaluating how uncertainty in MRI-based cartilage property relationships affects finite element model predictions, showing modulus shifts significantly impact stress/strain while slope changes have minimal effect.


<details>
  <summary>Details</summary>
Motivation: To understand the sensitivity of finite element models to uncertainty in subject-specific relationships between MRI biomarkers (T2) and cartilage material properties (dynamic modulus).

Method: Shifted the slope and intercept of a linear T2-dynamic modulus relationship used to define cartilage properties in finite element models.

Result: Modulus shifts led to notable percent changes in the top 1% of calculated stress and strain, while modulating slope had a negligible impact.

Conclusion: Supports the use of physiologically relevant moduli ranges in subject-specific finite element models of articular cartilage.
...
TL;DR: LAD-BNet is a neural network for real-time energy forecasting on edge devices that combines temporal lag exploitation with dilated TCNs, achieving 14.49% MAPE with 18ms inference time on Edge TPU.


<details>
  <summary>Details</summary>
Motivation: Real-time energy forecasting on edge devices is challenging but crucial for smart grid optimization and intelligent buildings, requiring efficient models suitable for embedded constraints.

Method: Hybrid architecture combining a branch for explicit temporal lag exploitation with a Temporal Convolutional Network using dilated convolutions to capture both short and long-term dependencies.

Result: Achieved 14.49% MAPE at 1-hour horizon with 18ms inference time on Edge TPU (8-12x faster than CPU), 2.39% improvement over LSTM and 3.04% over pure TCN, with 180MB memory footprint.

Conclusion: The model enables industrial applications in real-time energy optimization, demand management, and operational planning with performance suitable for embedded device constraints.
