...
TL;DR: VL4Gaze is the first large-scale benchmark for evaluating vision-language models on gaze understanding, showing current VLMs struggle with gaze interpretation but can be improved with targeted training.


<details>
  <summary>Details</summary>
Motivation: Gaze provides crucial cues for understanding attention, intention, and social interaction, but current vision-language models lack systematic evaluation or training for gaze interpretation. There's no existing benchmark to assess whether gaze understanding emerges from general-purpose vision-language pre-training.

Method: Created VL4Gaze benchmark with 489K automatically generated QA pairs across 124K images, formulating gaze understanding as a unified VQA problem through four tasks: gaze object description, gaze direction description, gaze point location, and ambiguous question recognition. Evaluated both commercial and open-source VLMs under in-context learning and fine-tuning settings.
...
Conclusion: Gaze understanding doesn't naturally emerge from general-purpose vision-language pre-training; targeted multi-task supervision is essential. The VL4Gaze benchmark enables systematic evaluation and development of gaze understanding capabilities in VLMs.
...
TL;DR: The paper presents TrashDets, a family of efficient trash detectors for TinyML devices developed using hardware-aware neural architecture search, achieving significant improvements in accuracy, energy efficiency, and latency over existing detectors.


<details>
  <summary>Details</summary>
Motivation: There's a need for efficient trash detection models that can run on resource-constrained edge and IoT devices (TinyML constraints) for real-world environmental monitoring applications.

Method: Uses an iterative hardware-aware neural architecture search framework with Once-for-All-style ResDets supernet, alternating backbone and neck/head optimization, population passthrough mechanism, and accuracy predictor to reduce search cost.

Result: TrashDet-l achieves 19.5 mAP50 with 30.5M parameters on TACO dataset, improving accuracy by up to 3.6 mAP50. On MAX78002 microcontroller, specialized variants reduce energy by 88%, latency by 78%, and power by 53% while improving mAP50 by 10.2%.

Conclusion: The proposed hardware-aware NAS framework successfully generates scalable, deployment-ready trash detectors that significantly outperform existing TinyML detectors in accuracy, efficiency, and resource utilization for edge/IoT applications.
...
TL;DR: First real-world camera-based aerial semantic scene completion benchmark (OccuFly) for UAVs, addressing limitations of LiDAR in aerial scenarios with automated label generation.


<details>
  <summary>Details</summary>
Motivation: SSC is crucial for 3D perception in robotics but unexplored in aerial domains. LiDAR sensors are problematic for UAVs due to regulations, constraints, and sparse point clouds from elevated viewpoints. Need for camera-based SSC benchmark for aerial scenarios.

Method: Introduces OccuFly benchmark captured at 50m/40m/30m altitudes across seasons. Proposes LiDAR-free data generation framework using camera modality with traditional 3D reconstruction. Automates label transfer by lifting annotated 2D masks into reconstructed point clouds to minimize manual 3D annotation.

Result: First real-world camera-based aerial SSC benchmark covering urban, industrial, rural scenarios with 22 semantic classes. Data format follows established conventions for integration with existing research. Benchmarked state-of-the-art methods and identified challenges specific to elevated viewpoints.

Conclusion: OccuFly provides comprehensive vision benchmark for holistic aerial 3D scene understanding, enabling progress in UAV applications by addressing limitations of LiDAR-based approaches and leveraging ubiquitous camera sensors on modern UAVs.
...
Method: Proposes NullBUS framework with nullable prompts implemented as learnable null embeddings with presence masks, enabling fallback to image-only evidence when metadata are absent while still utilizing text when available in a single unified model.
...
TL;DR: End-to-end RAW-to-task co-design framework that jointly optimizes optics, sensor modeling, and lightweight segmentation networks for autonomous driving perception, achieving better performance with smaller models.
...
Method: A task-driven co-design framework that unifies optics, sensor modeling, and lightweight semantic segmentation networks into a single end-to-end RAW-to-task pipeline. Includes realistic cellphone-scale lens models, learnable color filter arrays, Poisson-Gaussian noise processes, and quantization, all optimized directly for segmentation objectives.

Result: Consistent mIoU improvements over fixed pipelines on KITTI-360, with optics modeling and CFA learning providing the largest gains, especially for thin or low-light-sensitive classes. Achieved with compact ~1M-parameter model running at ~28 FPS, demonstrating edge deployability.
...
TL;DR: CHAMMI-75 is a diverse dataset of 75 biological studies' microscopy images enabling development of channel-adaptive cellular morphology models that work across different imaging types.


<details>
  <summary>Details</summary>
Motivation: Current cellular morphology models are specialized to single microscopy imaging types, limiting reusability across studies due to technical mismatches and out-of-distribution conditions.

Method: Created CHAMMI-75 dataset by curating heterogeneous, multi-channel microscopy images from 75 diverse biological studies from public sources to train channel-adaptive models.
...
Conclusion: This work enables next-generation cellular morphology models that can process any microscopy image type, overcoming current limitations in model reusability across biological studies.
...
TL;DR: Adaptive visual preprocessing method that dynamically adjusts input resolution and spatial coverage based on image content to reduce redundant computation in vision-language models, achieving over 50% reduction in inference time without modifying model architecture.


<details>
  <summary>Details</summary>
Motivation: VLMs face high inference latency and computational costs, especially with high-resolution inputs. Existing pipelines use static visual preprocessing, leading to redundant computation for visually simple inputs. There's a need for deployment-oriented efficiency improvements.

Method: Proposes adaptive visual preprocessing that combines: 1) content-aware image analysis, 2) adaptive resolution selection, and 3) content-aware cropping to reduce visual redundancy before vision encoding. Integrated with FastVLM without architecture changes or retraining.

Result: On DocVQA subset: over 50% reduction in per-image inference time, lower mean full generation time, and consistent >55% reduction in visual token count compared to baseline. Maintains model performance while significantly improving efficiency.

Conclusion: Input-aware preprocessing is an effective, lightweight strategy for improving deployment efficiency of VLMs. The method demonstrates substantial computational savings without architectural modifications, making it practical for real-world deployment.
...
TL;DR: NeRV360 is a viewport-adaptive neural video compression method for 360° videos that decodes only the user-selected viewport instead of the entire panoramic frame, achieving 7x memory reduction and 2.5x speedup over prior work.


<details>
  <summary>Details</summary>
Motivation: Current implicit neural representations (NeRV) for video compression suffer from high memory usage and slow decoding when applied to high-resolution 360-degree videos, making real-time applications impractical.

Method: NeRV360 is an end-to-end framework that integrates viewport extraction into decoding and introduces a spatial-temporal affine transform module for conditional decoding based on viewpoint and time, enabling selective viewport reconstruction.
...
Conclusion: NeRV360 enables practical real-time applications for 360° video compression by selectively decoding only the needed viewport, significantly improving efficiency over full-frame reconstruction approaches.
...
Method: 1) B/C/D-Scan three-view joint analysis with FDTD simulation validation; 2) DCO-YOLO framework integrating DySample, CGLU, and OutlookAttention into YOLOv11; 3) 3D-DIoU spatial feature matching with geometric constraints and center distance penalty.

Result: Achieves 96.2% accuracy, 93.3% recall, and 96.7% mAP in complex multi-pipeline scenarios, outperforming baseline by 2.0%, 2.1%, and 0.9% respectively. Ablation studies validate synergistic optimization.
...
TL;DR: DRI: A novel Parameter-Efficient Fine-Tuning strategy that injects domain-specific representations into frozen Vision Foundation Models via feature space optimization for cross-modality ship re-identification.


<details>
  <summary>Details</summary>
Motivation: Cross-modality ship re-ID faces significant modality discrepancies, and existing methods require large paired datasets for pre-training. Vision Foundation Models have potential but generic PEFT methods perform suboptimally on limited-capacity models.

Method: Proposes Domain Representation Injection (DRI): keeps VFM frozen, uses lightweight Offset Encoder to extract domain-specific representations, adaptively transforms them via Modulator guided by contextual information, and injects them into intermediate layers via additive fusion.
...
Conclusion: DRI effectively bridges modality gaps by optimizing in feature space rather than weight space, preserving VFM's general knowledge while adapting to downstream tasks with minimal trainable parameters.
...
TL;DR: Dual-Graph Spatiotemporal Attention Network (DGSAN) improves lung cancer diagnosis by effectively fusing multimodal and temporal data for pulmonary nodule classification.
...
Method: The authors propose a Dual-Graph Spatiotemporal Attention Network (DGSAN) with: 1) Global-Local Feature Encoder to capture local, global, and fused nodule characteristics; 2) Dual-Graph Construction organizing multimodal features into inter-modal and intra-modal graphs; 3) Hierarchical Cross-Modal Graph Fusion Module for refined feature integration; and 4) compilation of a novel multimodal NLST-cmst dataset.

Result: Extensive experiments on both the NLST-cmst dataset and curated CSTL-derived datasets show that DGSAN significantly outperforms state-of-the-art methods in classifying pulmonary nodules while maintaining exceptional computational efficiency.

Conclusion: The proposed DGSAN framework effectively addresses the limitations of current multimodal fusion methods and provides a superior solution for pulmonary nodule classification by leveraging temporal variations and multimodal data through innovative graph-based attention mechanisms.
...
TL;DR: First benchmark for evaluating Vision-Language Models on compressed images, revealing performance gaps and proposing a universal adaptor that improves performance by 10-30% across codecs and bitrates.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs mainly process high-bitrate compressed images, but their ability to handle low-bitrate compressed images remains unexplored despite growing demand for efficient image compression in VLM applications.

Method: 1) Created comprehensive benchmark with over 1M compressed images using various codecs and tasks; 2) Analyzed performance gaps into information loss vs. generalization failure; 3) Proposed universal VLM adaptor to enhance performance on compressed images.

Result: Identified that only generalization gap can be mitigated for compressed images. The proposed universal adaptor improves VLM performance by 10-30% across images with varying codecs and bitrates.

Conclusion: The benchmark and enhancement method provide valuable insights for bridging the gap between VLMs and compressed images, enabling better performance on efficiently compressed visual inputs.
...
TL;DR: PanoGrounder: A generalizable 3D visual grounding framework that uses panoramic renderings with 3D features and pretrained 2D VLMs for strong 3D scene reasoning, achieving SOTA results with superior generalization.


<details>
  <summary>Details</summary>
Motivation: Traditional supervised 3D visual grounding models have limited generalization due to scarce 3D vision-language datasets and weaker reasoning capabilities compared to modern 2D VLMs. There's a need for a framework that leverages strong 2D VLM reasoning while effectively handling 3D scene understanding.

Method: Three-stage pipeline: 1) Places panoramic viewpoints considering scene layout/geometry, 2) Grounds text queries on each panoramic rendering using a pretrained 2D VLM, 3) Fuses per-view predictions into a single 3D bounding box via lifting. Uses panoramic renderings augmented with 3D semantic/geometric features as intermediate representation.
...
Conclusion: PanoGrounder successfully bridges 2D VLMs with 3D scene understanding through panoramic representations, offering strong generalization capabilities for 3D visual grounding tasks while leveraging the powerful reasoning of pretrained 2D vision-language models.
...
<details>
  <summary>Details</summary>
Motivation: General image fusion needs to handle diverse tasks while improving performance without increasing complexity, unlike task-specific methods that only consolidate inter-modal information.

Method: Proposes SMC-Mamba with three key components: 1) Modality-Agnostic Feature Enhancement (MAFE) module for detail preservation and global representation enhancement, 2) Multiplex Consensus Cross-modal Mamba (MCCM) module for dynamic expert collaboration and cross-modal feature interaction, and 3) Bi-level Self-supervised Contrastive Learning Loss (BSCL) for high-frequency preservation without computational overhead.
...
Conclusion: SMC-Mamba provides an effective general image fusion framework that enhances multiple fusion tasks and downstream applications through self-supervised learning and efficient cross-modal integration.
...
TL;DR: Q-Render introduces quantile-based sparse sampling for efficient high-dimensional feature rendering in 3D Gaussian Splatting, enabling real-time open-vocabulary segmentation with minimal information loss.
...
Method: Proposes Quantile Rendering (Q-Render) that sparsely samples only dominant Gaussians along each ray instead of all intersecting ones. Also introduces Gaussian Splatting Network (GS-Net) as a generalizable 3D neural network that predicts Gaussian features.

Result: Outperforms state-of-the-art methods on ScanNet and LeRF datasets while achieving ~43.7x speedup on 512-D feature maps, enabling real-time rendering with high fidelity.

Conclusion: Q-Render provides an efficient solution for high-dimensional feature rendering in 3D Gaussian Splatting, addressing the bottleneck in open-vocabulary 3D segmentation while maintaining quality and enabling real-time performance.
...
TL;DR: TVP introduces transductive visual programming that learns reusable tools from experience rather than speculating, achieving state-of-the-art spatial reasoning performance.
...
Method: TVP first solves problems using basic tools while accumulating solutions in an Example Library, then abstracts recurring patterns into reusable higher-level tools for an evolving Tool Library, enabling increasingly powerful tools learned from experience.

Result: TVP achieves SOTA on Omni3D-Bench, outperforming GPT-4o by 22% and previous best visual programming by 11%. Learned tools are used 5x more frequently than inductive ones and generalize well to unseen spatial tasks without modification.
...
Method: Collaborative Multi-Agent Reasoning Framework that decouples Semantic Planning from Visual Synthesis. Includes: (1) self-correcting Verification Agent using Chain-of-Thought reasoning to correct segmentation and identify occluders, (2) Diverse Hypothesis Generator for multiple plausible semantic interpretations of invisible regions, and (3) MAC-Score evaluation metric for assessing inferred content.
...
Conclusion: The proposed multi-agent reasoning approach with explicit semantic planning enables visually and semantically coherent single-pass synthesis for amodal completion, addressing limitations of prior progressive methods.
...
TL;DR: REM shifts synthetic image detection from learning generator artifacts to modeling real image distributions, achieving 7.5% average improvement over SOTA methods with strong generalization to real-world degradations.


<details>
  <summary>Details</summary>
Motivation: Existing detectors overfit to generator-specific artifacts and are sensitive to real-world degradations. As generative models evolve and images undergo multi-round sharing and post-processing, these artifact cues become obsolete and harder to detect.

Method: Proposes Real-centric Envelope Modeling (REM) that shifts detection from learning generator artifacts to modeling robust real image distributions. Uses feature-level perturbations in self-reconstruction to generate near-real samples, and employs an envelope estimator with cross-domain consistency to learn boundaries enclosing the real image manifold.

Result: Across eight benchmark evaluations, REM achieves 7.5% average improvement over state-of-the-art methods. Maintains exceptional generalization on the severely degraded RealChain benchmark, establishing solid foundation for synthetic image detection under real-world conditions.

Conclusion: REM provides a new paradigm for synthetic image detection that focuses on modeling real image distributions rather than generator artifacts, enabling robust performance under real-world conditions with chain degradations.
...
Method: Represents road structures and CCTV placement as spatial documents using chunking techniques, transforms vehicle positions to world coordinates using relative position and FOV data, and performs beam search at intersections combining map spatial info with vehicle movement patterns to predict next CCTV locations.
...
Conclusion: SPOT successfully addresses CCTV blind spot limitations through spatial reasoning and map guidance, enabling reliable vehicle tracking without prior training in multi-camera environments.
...
Result: Extensive experiments show superior mapping quality while overcoming efficiency limitations of voxel-guided methods, outperforming existing state-of-the-art mapping methods in both quality and real-time performance.

Conclusion: XGrid-Mapping successfully addresses the trade-off between mapping quality and computational efficiency in neural LiDAR mapping by jointly exploiting explicit and implicit representations, enabling efficient large-scale incremental mapping with robust performance.
...
TL;DR: A novel data synthesis pipeline using normal X-rays to augment rare lung lesions via diffusion model inpainting, enhanced by LLM guidance and progressive learning.
...
Conclusion: The approach effectively addresses the data scarcity problem for rare pulmonary lesions by leveraging abundant normal X-rays and advanced generative techniques, achieving state-of-the-art diagnostic performance.
...
TL;DR: PUFM++ is an enhanced flow-matching framework for high-quality point cloud upsampling that improves geometric fidelity, robustness to imperfect inputs, and consistency with downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Recent generative models show promise for point cloud upsampling, but need improvements in handling sparse, noisy, and partial observations while maintaining geometric fidelity and downstream task compatibility.

Method: Two-stage flow-matching strategy: first learns direct flow from sparse to dense, then refines with noise-perturbed samples. Includes adaptive time scheduler for efficient sampling, on-manifold constraints for surface alignment, and recurrent interface network for hierarchical feature interactions.

Result: Sets new state-of-the-art in point cloud upsampling on synthetic benchmarks and real-world scans, delivering superior visual fidelity and quantitative accuracy across various tasks.

Conclusion: PUFM++ advances point cloud upsampling through enhanced flow matching with improved geometric fidelity, robustness, and downstream task consistency, with publicly available code and models.
...
TL;DR: A feed-forward multi-view inverse rendering framework that predicts materials and lighting from RGB image sequences using cross-view attention, with consistency-based finetuning for real-world generalization.
...
Method: Proposes a feed-forward framework using alternating attention across views to capture intra-view lighting interactions and inter-view material consistency. Includes consistency-based finetuning using unlabeled real-world videos to improve generalization.
...
Conclusion: The method provides an efficient, scalable solution for multi-view inverse rendering that maintains cross-view consistency and generalizes well to real-world scenes through novel attention mechanisms and finetuning strategies.
...
<details>
  <summary>Details</summary>
Motivation: Most visual generative pretraining methods rely on BERT-style masked modeling which ignores temporal information crucial for video analysis. Existing autoregressive visual pretraining methods have problems with inaccurate semantic localization and poor generation quality.
...
Conclusion: NExT-Vid successfully addresses limitations of existing visual pretraining methods by introducing autoregressive modeling with context-isolated flow-matching, achieving strong visual representations that transfer well to downstream tasks.
...
Method: Granular-ball Guided Masking (GBGM) uses Granular-ball Computing (GBC) to guide a coarse-to-fine hierarchical masking process. It adaptively preserves semantically rich, structurally important regions while suppressing redundant areas, producing representative and discriminative augmentations.

Result: Extensive experiments on multiple benchmarks show consistent improvements in classification accuracy and masked image reconstruction. The method demonstrates effectiveness and broad applicability across different tasks.

Conclusion: GBGM provides a simple, model-agnostic structure-aware data augmentation paradigm that integrates seamlessly into both CNNs and Vision Transformers, enhancing model robustness without requiring architectural changes.
...
TL;DR: FluencyVE is a one-shot video editing method that replaces temporal attention with Mamba modules in Stable Diffusion models to achieve temporal consistency with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Current video editing methods using text-to-image diffusion models suffer from temporal inconsistency and high computational overhead, despite adding temporal attention mechanisms.
...
Result: Demonstrates promising results in editing various attributes, subjects, and locations in real-world videos while maintaining generative power and reducing computational burden.

Conclusion: FluencyVE provides a simple yet effective approach for one-shot video editing that achieves temporal consistency with reduced computational costs compared to existing methods.
...
Conclusion: The proposed framework provides an efficient and effective defense against 3D-field talking face generation attacks, addressing privacy concerns while maintaining video quality and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Traditional facial recognition models trained on visible light datasets perform poorly on infrared images due to domain shifts. Existing HFR methods suffer from distortion and feature loss during infrared-to-visible conversion.

Method: Latent diffusion-based model for generating visible face images from thermal inputs, incorporating multi-attribute classifier to preserve identity features, and Self-attn Mamba module for global cross-modal feature modeling and faster inference.

Result: Achieves state-of-the-art performance on two benchmark datasets in both image quality and identity preservation.

Conclusion: The proposed approach effectively addresses domain shift and feature loss challenges in infrared face recognition through improved generative modeling and feature preservation techniques.
...
TL;DR: NSP introduces a novel self-supervised denoising method that decouples noise decorrelation from detail preservation using cross-scale training pairs, achieving SOTA performance on real-world benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised denoising methods face a fundamental trade-off: aggressive downsampling fragments fine details while mild downsampling fails to remove correlated noise. This creates an antagonistic conflict between noise decorrelation and detail preservation.

Method: Next-Scale Prediction (NSP) constructs cross-scale training pairs where blind-spot networks take low-resolution, fully decorrelated sub-images as input to predict high-resolution targets that retain fine details. This decouples the noise decorrelation process from detail preservation.
...
Conclusion: NSP successfully addresses the long-standing conflict between noise decorrelation and detail preservation in self-supervised real-world image denoising, offering a novel paradigm that outperforms existing methods.
...
TL;DR: KOREATECH-CGH dataset with 6,000 RGB-D/hologram pairs and amplitude projection technique improves hologram quality at large depth ranges.
...
Method: Created KOREATECH-CGH dataset with 6,000 RGB-D image/hologram pairs across various resolutions. Introduced amplitude projection technique that replaces amplitude components while preserving phase to improve hologram quality at large depth ranges.

Result: Achieved 27.01 dB PSNR and 0.87 SSIM, surpassing previous optimized silhouette-masking method by 2.03 dB and 0.04 SSIM. Dataset validated through hologram generation and super-resolution experiments with state-of-the-art ML models.
...
Conclusion: Reweighted logarithmic norm provides closer approximation to rank function than existing alternatives, enabling more effective low-rank matrix completion with better practical performance.
...
<details>
  <summary>Details</summary>
Motivation: Traditional cameras struggle with motion blur, sensor noise, occlusion, and lighting changes for object pose tracking. Event cameras offer high dynamic range and low latency advantages to overcome these challenges.

Method: Uses optical flow-guided approach with 2D-3D hybrid feature extraction: detects corners/edges from events and models, searches for corner optical flow via event probability maximization, establishes corner-edge correlations guided by optical flow, and iteratively optimizes 6DoF pose by minimizing corner-edge distances.
...
Conclusion: The optical flow-guided event camera approach effectively addresses traditional camera limitations and achieves superior 6DoF object pose tracking performance.
...
TL;DR: DexAvatar is a framework that reconstructs accurate 3D hand and body poses from monocular sign language videos using learned priors, addressing limitations of current 2D datasets and poor 3D reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Current sign language generation relies on data-driven methods needing precise 3D pose data, but existing datasets are mostly 2D video-based with poor 3D reconstruction quality due to self-occlusion, noise, and motion blur in automatic estimation methods.
...
Conclusion: DexAvatar provides a significant advancement in 3D human pose reconstruction from sign language videos, enabling better quality data for sign language generation systems.
...
TL;DR: UniPath is a semantics-driven pathology image generation framework that uses diagnostic understanding for controllable generation, achieving state-of-the-art performance with 51% better Patho-FID than second-best methods.


<details>
  <summary>Details</summary>
Motivation: Current pathology image generation faces three main challenges: 1) scarcity of large, high-quality image-text datasets, 2) lack of precise semantic control forcing reliance on non-semantic cues, and 3) terminological heterogeneity where different phrasings for the same diagnostic concept impede reliable text conditioning.

Method: UniPath implements Multi-Stream Control with three streams: 1) Raw-Text stream, 2) High-Level Semantics stream using learnable queries to a frozen pathology MLLM to distill paraphrase-robust Diagnostic Semantic Tokens and expand prompts into diagnosis-aware attribute bundles, and 3) Prototype stream for component-level morphological control via a prototype bank. The authors also curate a 2.65M image-text corpus and a 68K high-quality subset.

Result: UniPath achieves state-of-the-art performance with Patho-FID of 80.9 (51% better than second-best) and fine-grained semantic control achieving 98.7% of real-image quality. The framework demonstrates superior controllable generation capabilities.

Conclusion: UniPath successfully bridges the gap between understanding and generation in computational pathology by leveraging diagnostic understanding for controllable image generation, addressing key data scarcity and semantic control challenges while establishing a comprehensive evaluation hierarchy for pathology image generation.
...
TL;DR: A self-supervised multimodal skeleton-based action learning framework called Decomposition and Composition that balances computational efficiency and performance by decomposing fused features into unimodal components and using them as self-supervised guidance.
...
Method: Proposes a self-supervised framework with two strategies: 1) Decomposition - carefully decomposes fused multimodal features into distinct unimodal features and aligns them with ground truth unimodal counterparts; 2) Composition - integrates multiple unimodal features and uses them as self-supervised guidance to enhance multimodal representation learning.
...
Conclusion: The proposed Decomposition and Composition framework successfully addresses the efficiency-effectiveness trade-off in multimodal skeleton-based action understanding, offering a practical solution for real-world applications.
...
TL;DR: UniPR-3D is a novel Visual Place Recognition architecture that effectively integrates multi-view information using 3D representations from VGGT backbone with dedicated 2D/3D feature aggregation modules.


<details>
  <summary>Details</summary>
Motivation: Traditional VPR is single-image based, while multi-view approaches offer advantages but remain underexplored and struggle with generalization across diverse environments.

Method: Uses VGGT backbone for multi-view 3D representations, adapts with feature aggregators, jointly leverages 3D tokens and intermediate 2D tokens, designs dedicated aggregation modules for 2D and 3D features, incorporates single- and multi-frame aggregation schemes with variable-length sequence retrieval.
...
Conclusion: The proposed architecture effectively integrates multi-view information for VPR, highlighting the value of geometry-grounded tokens and achieving superior performance through dedicated 2D/3D feature aggregation.
...
TL;DR: A novel multi-task table recognition model using non-causal attention and parallel inference for faster, more accurate table extraction.


<details>
  <summary>Details</summary>
Motivation: Table recognition from documents is challenging due to diverse elements requiring different recognition methods. Existing models lack explainability and have slow inference times despite using multi-task learning, local attention, and mutual learning.
...
Result: Demonstrated superiority both visually and statistically on two large public datasets, showing improved performance and faster inference.

Conclusion: The proposed approach effectively addresses table recognition challenges by combining non-causal attention for comprehensive structure understanding with parallel processing for efficiency.
...
Method: Created T2AV-Compass with 500 diverse prompts via taxonomy-driven pipeline, plus a dual-level evaluation framework: objective signal-level metrics (video/audio quality, cross-modal alignment) and subjective MLLM-as-a-Judge protocol for instruction following and realism.
...
TL;DR: UniRec-0.1B is a lightweight unified recognition model with only 0.1B parameters that performs text and formula recognition at multiple levels (characters to documents), achieving better performance and 2-9× speedup over existing models.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models for unified text and formula recognition are large and computationally demanding, limiting their practical applications. There's a need for a lightweight yet effective model that can handle both text and formulas across multiple hierarchical levels.

Method: 1) Created UniRec40M dataset with 40M text, formula, and mixed samples; 2) Introduced hierarchical supervision training to handle structural variability across hierarchies; 3) Developed semantic-decoupled tokenizer to separate text and formula representations; 4) Built comprehensive evaluation benchmark covering Chinese/English documents across domains and levels.

Result: UniRec-0.1B outperforms both general-purpose VLMs and leading document parsing expert models while achieving 2-9× speedup. The model demonstrates effectiveness and efficiency across multiple domains and hierarchical levels.

Conclusion: The proposed UniRec-0.1B successfully addresses the computational limitations of existing unified recognition models while maintaining high performance through hierarchical supervision and semantic-decoupled tokenization, making it practical for real-world applications.
...
TL;DR: FreeInpaint: A tuning-free optimization method for text-guided image inpainting that improves prompt alignment and visual rationality by optimizing diffusion latents during inference.


<details>
  <summary>Details</summary>
Motivation: Existing text-guided image inpainting methods struggle to simultaneously maintain both prompt alignment (faithfulness to user text) and visual rationality (visual fidelity), even though they leverage powerful pre-trained diffusion models.
...
Result: Extensive experiments across various inpainting diffusion models and evaluation metrics demonstrate the effectiveness and robustness of FreeInpaint in improving both prompt alignment and visual rationality.
...
TL;DR: VLMs struggle with marine domain expertise despite general capabilities; new MarineEval benchmark reveals significant performance gaps.


<details>
  <summary>Details</summary>
Motivation: While VLMs show promise as general-purpose assistants, their effectiveness in specialized domains like marine science remains unknown. Marine questions require significant domain expertise and address unique challenges that may not be covered by general VLM training.

Method: Created MarineEval, the first large-scale marine VLM benchmark with 2,000 image-based QA pairs covering 7 task dimensions and 20 capacity dimensions. Domain requirements were integrated into data construction and verified by marine experts. Evaluated 17 existing VLMs on this benchmark.

Result: Existing VLMs cannot effectively answer domain-specific marine questions, showing significant performance gaps. There is substantial room for improvement in handling marine expertise requirements.

Conclusion: Current VLMs lack sufficient marine domain knowledge despite their general capabilities. The MarineEval benchmark provides a foundation for future research to improve VLM performance in specialized scientific domains.
...
TL;DR: TGC-Net is a CLIP-based framework for text-guided medical segmentation that addresses limitations of existing methods through parameter-efficient adaptations, achieving state-of-the-art performance with fewer trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Existing text-guided medical segmentation methods rely on unaligned image and text encoders requiring complex fusion modules. While CLIP provides pre-aligned multimodal features, its direct medical application faces three issues: insufficient preservation of fine-grained anatomical structures, inadequate modeling of complex clinical descriptions, and domain-specific semantic misalignment.
...
Conclusion: TGC-Net effectively addresses CLIP's limitations for medical segmentation through parameter-efficient, task-specific adaptations, achieving superior performance while maintaining computational efficiency across multiple medical imaging modalities.
...
TL;DR: ORCA is a comprehensive multi-modal benchmark for marine visual understanding with 14,647 images, 42,217 bounding boxes, and 22,321 expert-verified captions across 478 species, evaluated on object detection, instance captioning, and visual grounding tasks.


<details>
  <summary>Details</summary>
Motivation: Marine visual understanding is crucial for ecosystem monitoring but faces challenges from limited training data and lack of systematic task formulation aligning marine domain challenges with computer vision tasks, hindering effective model application.

Method: Created ORCA benchmark with fine-grained visual and textual annotations capturing morphology-oriented attributes across diverse marine species, then evaluated 18 state-of-the-art models on three tasks: closed-set/open-vocabulary object detection, instance captioning, and visual grounding.

Result: Evaluation results highlight key challenges including species diversity, morphological overlap, and specialized domain demands, demonstrating the difficulty of marine understanding and providing baseline performance metrics for future research.

Conclusion: ORCA establishes a comprehensive benchmark to advance marine visual understanding research by addressing data limitations and providing systematic task formulations aligned with domain-specific challenges.
...
Conclusion: EFR provides an effective adaptation strategy for few-shot image generation that overcomes limitations of traditional consistency constraints by aligning domains in an equivariant proxy feature space, enabling better knowledge transfer from source to target domain while preserving structural information.
...
TL;DR: Proposes NAME, a hierarchical implicit representation framework for continuous human motion sequences that enables interpolation, inbetweening, and extrapolation at arbitrary frame rates using parametric activation and Fourier transformations.


<details>
  <summary>Details</summary>
Motivation: Physical motions are inherently continuous, and higher camera frame rates improve smoothness and temporal coherence. Current methods lack the ability to handle motion sequences at arbitrary frame rates through continuous representations.

Method: Proposes NAME framework based on Implicit Neural Representations (INRs) with hierarchical temporal encoding to extract multi-scale temporal features, and integrates a custom parametric activation function using Fourier transformations in the MLP decoder.
...
Conclusion: The NAME framework successfully enables continuous representation of human motion sequences with capabilities for interpolation, inbetweening, and extrapolation at arbitrary frame rates, advancing motion representation techniques.
...
TL;DR: UltraShape 1.0 is a scalable 3D diffusion framework for high-fidelity geometry generation using a two-stage pipeline: coarse structure synthesis followed by detailed refinement with voxel-based localization and RoPE encoding.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of generating high-fidelity 3D geometry with reliable quality, especially given limited training resources and the need to work with publicly available datasets that often contain low-quality samples and incomplete structures.

Method: Two-stage generation pipeline: 1) Synthesize coarse global structure, 2) Refine with voxel-based detail synthesis. Includes comprehensive data processing with watertight processing and quality filtering. Uses spatial localization decoupling via voxel queries from coarse geometry with RoPE encoding for positional anchors.
...
Conclusion: UltraShape 1.0 demonstrates an effective approach for high-fidelity 3D geometry generation through improved data processing and a novel two-stage diffusion framework that decouples spatial localization from detail synthesis, enabling fine-grained refinement.
...
TL;DR: VisRes Bench is a new benchmark for evaluating visual reasoning in VLMs without language supervision, revealing their limitations in perceptual and relational reasoning across three complexity levels.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models show impressive performance but it's unclear whether they truly perform visual reasoning or just rely on linguistic priors. There's a need to isolate and evaluate pure visual reasoning capabilities without language context.

Method: Created VisRes Bench with 19,000+ controlled task images across three complexity levels: Level 1 tests perceptual completion and image matching under perturbations (blur, texture, occlusion, rotation); Level 2 tests rule-based inference on single attributes (color, count, orientation); Level 3 tests compositional reasoning requiring integration of multiple visual attributes.

Result: State-of-the-art VLMs perform near random under subtle perceptual perturbations, showing limited abstraction beyond pattern recognition. The benchmark reveals clear limitations in perceptual and relational visual reasoning capacities.

Conclusion: VisRes provides a unified framework for advancing abstract visual reasoning in multimodal research, highlighting the need for models that can perform true visual reasoning rather than just pattern matching with linguistic priors.
...
TL;DR: EveryWear: A lightweight human motion capture system using everyday wearables (smartphone, smartwatch, earbuds, smart glasses) without explicit calibration, trained on real-world data to eliminate sim-to-real gap.
...
Method: Multimodal teacher-student framework integrating visual cues from egocentric cameras (forward-facing + two downward-facing) with inertial signals from consumer devices, trained on real-world Ego-Elec dataset (9 hours, 56 activities, 17 environments).

Result: Outperforms baseline models, validates effectiveness for practical full-body motion estimation, eliminates sim-to-real gap by training on real-world rather than synthetic data.

Conclusion: EveryWear provides a lightweight, practical solution for human motion capture using everyday wearables without calibration, enabling robust motion estimation for daily life applications.
...
TL;DR: Proposes a task-agnostic method for training LMMs to discover and use visual reasoning tokens without explicit supervision, enabling better handling of vision-centric tasks.


<details>
  <summary>Details</summary>
Motivation: Current LMMs are text-centric and struggle with visual reasoning tasks. Existing approaches use supervised intermediate visual steps (helper images, depth maps, crops) which impose restrictive priors, add annotation costs, and lack generalization.

Method: Task-agnostic mechanism that trains LMMs to discover and use visual reasoning tokens without explicit supervision. These tokens attend globally and re-encode images in a task-adaptive way, allowing extraction of relevant visual information without hand-crafted supervision.
...
Conclusion: The proposed unsupervised visual reasoning token discovery approach effectively addresses limitations of text-centric LMMs, enabling better performance on visual reasoning tasks without restrictive priors or heavy annotation costs.
...
TL;DR: A two-stage image-text retrieval pipeline combining event-centric entity extraction with BM25 filtering and BEiT-3 reranking achieves state-of-the-art performance on OpenEvents benchmark.


<details>
  <summary>Details</summary>
Motivation: Real-world image-text retrieval faces challenges with vague/context-dependent queries, linguistic variability, and scalability needs. Existing methods struggle with temporal and contextual signals in real-world captions.
...
<details>
  <summary>Details</summary>
Motivation: Existing text-to-motion generation methods align text and motion at the sequence level, ignoring the internal semantic structure. Both motion descriptions and motion sequences can be naturally decomposed into smaller, semantically coherent segments that could serve as atomic alignment units for finer-grained correspondence.

Method: SegMo consists of three modules: (1) Text Segment Extraction decomposes textual descriptions into temporally ordered phrases representing atomic actions; (2) Motion Segment Extraction partitions complete motion sequences into corresponding motion segments; (3) Fine-grained Text-Motion Alignment aligns text and motion segments using contrastive learning.

Result: Extensive experiments show SegMo improves strong baselines on two widely used datasets, achieving an improved TOP 1 score of 0.553 on the HumanML3D test set. The learned shared embedding space also enables retrieval-style tasks like motion grounding and motion-to-text retrieval.

Conclusion: SegMo demonstrates that decomposing both text and motion into semantically coherent segments enables fine-grained text-motion alignment, improving generation quality and enabling additional applications like retrieval tasks through a shared embedding space.
...
TL;DR: DreaMontage: A framework for generating seamless, expressive long-duration one-shot videos from arbitrary frame inputs using adaptive tuning, visual expression fine-tuning, and memory-efficient autoregressive inference.


<details>
  <summary>Details</summary>
Motivation: One-shot filmmaking is aesthetically sophisticated but prohibitively expensive and constrained in practice. Existing video generation approaches using naive clip concatenation fail to maintain visual smoothness and temporal coherence.
...
Conclusion: DreaMontage provides a comprehensive framework for generating high-quality, expressive one-shot videos from diverse user inputs, overcoming limitations of existing methods through innovative architectural modifications, training strategies, and efficient inference techniques.
...
TL;DR: A unified Any-Modality AD framework for brain MRI that handles arbitrary modality availability without retraining, using feature alignment and normal pattern reconstruction to detect anomalies.


<details>
  <summary>Details</summary>
Motivation: Clinical brain MRI anomaly detection faces challenges due to scarce annotated abnormal cases and frequent missing modalities in real workflows. Existing models rely on fixed modality configurations, require repetitive training, or fail to generalize to unseen modality combinations, limiting clinical scalability.

Method: 1) Dual-pathway DINOv2 encoder with feature distribution alignment to statistically align incomplete-modality features with full-modality representations; 2) Intrinsic Normal Prototypes (INPs) extractor and INP-guided decoder that reconstructs only normal anatomical patterns while amplifying abnormal deviations; 3) Randomized modality masking and indirect feature completion during training to adapt to all modality configurations without retraining.
...
Conclusion: The study establishes a scalable paradigm for multimodal medical anomaly detection under real-world, imperfect modality conditions, enabling robust anomaly detection and localization with arbitrary MRI modality availability.
...
TL;DR: ACD is a novel framework for direct conditional control in video diffusion models using attention supervision, achieving better alignment with conditioning signals while preserving temporal coherence.


<details>
  <summary>Details</summary>
Motivation: Existing methods for conditional video synthesis have limitations: classifier-free guidance provides limited controllability, while classifier-based guidance can lead to adversarial artifacts and limited effective control. There's a need for better direct conditional control in video diffusion models.

Method: Proposes Attention-Conditional Diffusion (ACD) framework that aligns model's attention maps with external control signals. Introduces sparse 3D-aware object layout as conditioning signal, along with Layout ControlNet and automated annotation pipeline for scalable layout integration.
...
Conclusion: ACD establishes an effective paradigm for conditional video synthesis by achieving better controllability through direct attention supervision, overcoming limitations of existing guidance methods.
...
TL;DR: A novel image sequence generation method that factorizes the process into low-resolution sequence generation followed by individual frame super-resolution, outperforming SoTA in quality and speed.


<details>
  <summary>Details</summary>
Motivation: Current SoTA methods treat image sequences as large tensors of stacked frames, which leads to inefficiencies and bottlenecks in generation. The authors question whether this straightforward representation is ideal and aim to devise a more effective way to model image sequence data.

Method: Factorizes generation into two stages: 1) Generate coarse sequence at low resolution using a generative model trained on grid images of subsampled frames, leveraging Diffusion Transformer's self-attention to capture frame correlations, and 2) Super-resolve each frame individually to add high-resolution details. This extends 2D image generators to operate as low-resolution 3D sequence generators without architectural modifications.

Result: Achieves superior synthesis quality and improved coherence across sequences compared to existing methods. Delivers high-fidelity generation of arbitrary-length sequences with increased efficiency in inference time (at least twice-as-fast) and training data usage. Generalizes effectively across diverse data domains without requiring additional priors or supervision.
...
TL;DR: SpikeSurgSeg: First spike-driven video Transformer for surgical scene segmentation that achieves comparable accuracy to SOTA ANN models while reducing inference latency by 8× and accelerating 20× over foundation models.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for surgical scene segmentation have high computational demands that hinder real-time deployment in resource-constrained surgical environments. SNNs offer efficiency but face challenges with limited labeled surgical data and sparse video representations.

Method: Propose SpikeSurgSeg framework with: 1) Surgical-scene masked autoencoding pretraining using layer-wise tube masking for robust spatiotemporal representation learning, and 2) Lightweight spike-driven segmentation head for temporally consistent predictions while preserving SNN low-latency characteristics.

Result: Achieves mIoU comparable to SOTA ANN-based models while reducing inference latency by at least 8× and delivering over 20× acceleration relative to most foundation-model baselines on EndoVis18 and in-house SurgBleed dataset.

Conclusion: SpikeSurgSeg demonstrates potential for time-critical surgical scene segmentation on non-GPU platforms, addressing the computational efficiency gap in surgical AI while maintaining competitive accuracy.
...
TL;DR: A multi-scale signal processing method for table edge detection that models row/column transitions as 1D signals, uses Gaussian convolution with increasing variances and statistical thresholding to detect stable structural edges in noisy table images.


<details>
  <summary>Details</summary>
Motivation: Existing table structure extraction methods struggle with low-resolution/noisy images. Transformer-based methods lack adaptability to noisy inputs, while mask-based edge detection approaches suffer from noise sensitivity, resolution loss, or high computational cost when applied directly to images.
...
Conclusion: The proposed multi-scale signal-processing approach effectively detects table edges from table masks, providing robust performance in noisy/low-resolution conditions and producing optimized structured tabular outputs suitable for downstream analysis.
...
TL;DR: AndroidLens is a challenging evaluation framework for mobile GUI agents with 571 long-latency tasks across 38 domains, featuring static and dynamic evaluation methods that reveal current models achieve only 12.7% success rate and 50.47% average task progress.


<details>
  <summary>Details</summary>
Motivation: Existing GUI agent benchmarks are limited to simple tasks, few applications, and coarse metrics, failing to capture real-world complexity. There's a need for more challenging evaluation frameworks that reflect actual user scenarios with long-latency tasks.

Method: Created AndroidLens with 571 long-latency tasks (avg 26+ steps) across 38 domains in Chinese/English environments. Features: 1) Real-world tasks including multi-constraint, multi-goal, and domain-specific types; 2) Static evaluation preserving real-world anomalies with multiple valid paths; 3) Dynamic evaluation using milestone-based scheme with Average Task Progress (ATP) metric.

Result: Even the best models achieve only 12.7% task success rate and 50.47% ATP, highlighting significant performance gaps. Key challenges identified include environmental anomalies, adaptive exploration difficulties, and long-term memory retention issues.

Conclusion: AndroidLens provides a more realistic and challenging benchmark for mobile GUI agents, revealing substantial limitations in current models and identifying critical research directions for improving real-world GUI automation capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current tile encoder pipelines extract embeddings without considering slide-level context, which is essential for both local and global pathology tasks. Different tile encoders excel at different tasks, requiring a unified model to contextualize embeddings from any tile-level foundation model.

Method: TICON uses a single shared transformer encoder pretrained with masked modeling objective to unify and contextualize representations from diverse tile-level pathology foundation models. It also includes an aggregator to form slide-level foundation models.
...
Conclusion: TICON successfully addresses the need for contextualized tile representations in computational pathology, demonstrating that slide-level context is crucial for both tile-level and slide-level tasks, and enables efficient slide-level foundation model training with significantly less data.
...
TL;DR: A text-guided token pruning framework improves SAM2 efficiency by selectively reducing token density before temporal propagation, achieving 42.5% faster inference and 37.4% lower GPU memory while maintaining segmentation quality.
...
TL;DR: Streamo is a real-time streaming video LLM that serves as a general-purpose interactive assistant for diverse streaming video tasks, trained on a large-scale instruction-following dataset.


<details>
  <summary>Details</summary>
Motivation: Existing online video models are too narrow, focusing only on specific tasks like question answering or captioning. There's a need for a unified model that can handle diverse real-time streaming video tasks as an interactive assistant.
...
<details>
  <summary>Details</summary>
Motivation: To expose and systematically investigate popularity bias in vision-language models, where models perform better on famous/memorized items versus ordinary ones, indicating flawed reasoning capabilities.

Method: Created YearGuessr dataset (55,546 building images with construction years, GPS, page-view counts), framed year prediction as ordinal regression, introduced popularity-aware interval accuracy metrics, benchmarked 30+ models including YearCLIP.
...
Conclusion: State-of-the-art VLMs suffer from significant popularity bias, relying on memorization rather than generalizable understanding, highlighting a fundamental limitation in their reasoning capabilities that needs addressing.
...
Result: On 1080p benchmarks, HiStream achieves state-of-the-art visual quality with up to 76.2x faster denoising than Wan2.1 baseline and negligible quality loss. HiStream+ (with all three optimizations) achieves 107.5x acceleration over baseline.

Conclusion: HiStream makes high-resolution video generation practical and scalable by offering compelling speed-quality trade-offs through systematic redundancy reduction across spatial, temporal, and timestep dimensions.
...
### [65] [Uplink RSMA Performance Analysis with Rate Adaptation: A Stochastic Geometry Approach](https://arxiv.org/abs/2512.20883)
*Xinyi Guo,Li You,Qiong Liu,Xiqi Gao,Xiang-Gen Xia*

Main category: cs.IT

TL;DR: A unified stochastic geometry framework for uplink RSMA with finite MCS-based rate adaptation, bridging theoretical tractability and practical realism in dense wireless networks.


<details>
  <summary>Details</summary>
Motivation: Most existing RSMA studies focus on downlink and single-cell designs, leaving uplink RSMA under large-scale deployments largely unexplored. There's a need to bridge the gap between theoretical tractability and practical realism in analyzing interference management for next-generation wireless networks.

Method: Develops a unified analytical framework based on stochastic geometry that integrates finite modulation and coding scheme (MCS)-based rate adaptation. The framework jointly captures spatial interference coupling and discrete rate behavior, deriving tractable expressions for conditional received rate, spatial average, and higher-order statistics via meta distribution.

Result: The proposed framework generalizes existing NOMA and OMA analyses and provides new insights into how discrete rate adaptation reshapes interference dynamics and fairness in dense RSMA-enabled networks. It quantifies both mean and user-specific rate performance.

Conclusion: The unified stochastic geometry framework successfully bridges theoretical tractability and practical realism for uplink RSMA analysis, offering comprehensive tools to evaluate interference management and fairness in next-generation dense wireless networks.
...
### [66] [Knowledge-Driven 3D Semantic Spectrum Map: KE-VQ-Transformer Based UAV Semantic Communication and Map Completion](https://arxiv.org/abs/2512.20984)
*Wei Wu,Lingyi Wang,Fuhui Zhou,Zhaohui Yang,Qihui Wu*

Main category: cs.IT

TL;DR: A knowledge-enhanced semantic spectrum map completion framework using UAV-collected sparse data with physics-based constraints to improve 3D spectrum mapping accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D spectrum mapping faces challenges in spectrum efficiency, computation efficiency, and AI-driven approaches under complex environments with sparse sampling data. Statistical ML methods lack interpretability and can be misled by superficial data correlations.

Method: Proposes a knowledge-enhanced semantic spectrum map completion framework with physics-based constraints from signal propagation models. Uses KE-VQ-Transformer with sparse window attention and multi-scale design. Introduces KMSE/RKMSE metrics combining numerical precision and physical consistency, with joint offline/online training using supervised and unsupervised knowledge loss.

Result: The proposed scheme outperforms state-of-the-art benchmark schemes in terms of RKMSE (Root Knowledge-enhanced Mean Square Error).

Conclusion: The knowledge-enhanced framework effectively captures real-world physics, avoids superficial data distribution pitfalls, and enables efficient, accurate 3D spectrum map completion for intelligent communication networks.
...
### [67] [Coding-Logic Correspondence: Turning Information and Communication Networks into Logical Formulae via Hypergraph Heyting Algebra](https://arxiv.org/abs/2512.21112)
*Cheuk Ting Li*

Main category: cs.IT

TL;DR: Hyperconfusions model information as Heyting algebras, enabling logical formulation of network coding problems and direct computation of optimal schemes via hypergraph entropy.


<details>
  <summary>Details</summary>
Motivation: Traditional random variable models lack algebraic structure for logical operations on information. The paper aims to bridge coding theory with logic by developing a framework where communication requirements can be expressed as logical formulas.

Method: Introduces confusion hypergraphs (hyperconfusions) that form Heyting algebras, allowing conjunction, disjunction, and implication of information. Uses connection between Heyting algebra and intuitionistic logic to express network coding requirements as logical formulas, then computes optimal schemes via hypergraph entropy.

Result: Optimal communication cost is given by hypergraph entropy (within logarithmic gap). Establishes correspondence between coding settings and logical formulas, analogous to Curry-Howard correspondence between proofs and programs.

Conclusion: Hyperconfusions provide a unified algebraic-logical framework for network coding problems, revealing deep connections between information theory, algebra, and logic, with potential for new coding scheme derivations.
...
### [68] [BitRL-Light: 1-bit LLM Agents with Deep Reinforcement Learning for Energy-Efficient Smart Home Lighting Optimization](https://arxiv.org/abs/2512.20623)
...
<details>
  <summary>Details</summary>
Motivation: Smart home lighting consumes 15-20% of residential energy but lacks adaptive intelligence to simultaneously optimize for user comfort and energy efficiency. Current systems are either energy-inefficient or don't adapt to user preferences.
...
Conclusion: BitRL-Light establishes a practical framework for deploying adaptive AI on resource-constrained IoT devices, enabling intelligent home automation without cloud dependencies through efficient 1-bit quantization and reinforcement learning.
...
### [69] [Quantum-Inspired Multi Agent Reinforcement Learning for Exploration Exploitation Optimization in UAV-Assisted 6G Network Deployment](https://arxiv.org/abs/2512.20624)
*Mazyar Taghavi,Javad Vahidi*

Main category: cs.AI

TL;DR: Quantum-inspired MARL framework for UAV-assisted 6G network deployment that integrates quantum optimization with classical MARL to optimize exploration-exploitation tradeoff, improving coverage and convergence.


<details>
  <summary>Details</summary>
Motivation: To address the exploration-exploitation tradeoff in multiagent reinforcement learning for UAV-assisted 6G networks, where UAVs need to autonomously coordinate to maximize signal coverage and support network expansion under partial observability and dynamic conditions.

Method: Integrates classical MARL algorithms with quantum-inspired optimization using variational quantum circuits (VQCs) and QAOA for combinatorial optimization. Incorporates Bayesian inference, Gaussian processes, and variational inference for probabilistic modeling. Uses CTDE paradigm with shared memory and local view grids for enhanced observability.

Result: The framework improves sample efficiency, accelerates convergence, enhances coverage performance, and maintains robustness. QI-MARL achieves superior balance between exploration and exploitation compared to classical PPO and DDPG baselines, as shown through scalability tests, sensitivity analysis, and convergence studies.

Conclusion: Quantum-inspired optimization techniques can effectively enhance MARL performance for UAV-assisted 6G networks, providing better exploration-exploitation balance and improved network deployment outcomes. The approach is publicly available for reproducibility.
...
### [70] [MegaRAG: Multimodal Knowledge Graph-Based Retrieval Augmented Generation](https://arxiv.org/abs/2512.20626)
...
<details>
  <summary>Details</summary>
Motivation: Traditional RAG struggles with high-level conceptual understanding of long-form content due to limited context windows, while existing KG-based RAG solutions only handle text and miss valuable visual information that could enhance reasoning.

Method: Introduces multimodal knowledge graph-based RAG that incorporates visual cues into KG construction, retrieval phase, and answer generation process, enabling cross-modal reasoning for better content understanding.
...
Conclusion: Multimodal knowledge graphs provide structured support for cross-modal reasoning, addressing limitations of text-only RAG systems and improving comprehension of complex, domain-specific content.
...
### [71] [Proceedings of the 20th International Conference on Knowledge, Information and Creativity Support Systems (KICSS 2025)](https://arxiv.org/abs/2512.20628)
...
<details>
  <summary>Details</summary>
Motivation: To provide a multidisciplinary forum for researchers to share and discuss advancements in artificial intelligence, knowledge engineering, human-computer interaction, and creativity support systems through an international conference platform.

Method: Conference proceedings compiled from papers accepted through a double-blind peer-review process, with selected papers recommended for further publication in IEICE Transactions on Information and Systems after additional review.

Result: Published proceedings containing peer-reviewed research papers presented at KICSS 2025, establishing a record of current research in the field and identifying high-quality papers for extended journal publication.

Conclusion: The KICSS 2025 proceedings successfully document the state-of-the-art research in knowledge, information, and creativity support systems, facilitating knowledge exchange and identifying promising work for further dissemination through journal publication.
...
### [72] [MicroProbe: Efficient Reliability Assessment for Foundation Models with Minimal Data](https://arxiv.org/abs/2512.20630)
*Aayam Bansal,Ishaan Gangwani*

Main category: cs.AI

TL;DR: Microprobe enables comprehensive foundation model reliability assessment using only 100 strategically selected examples, achieving 23.5% higher reliability scores than random sampling while reducing assessment cost by 90%.


<details>
  <summary>Details</summary>
Motivation: Traditional foundation model reliability assessment requires thousands of examples, making it computationally expensive and time-consuming for real-world deployment. There's a critical need for efficient model evaluation methods for responsible AI deployment.

Method: Combines strategic prompt diversity across five key reliability dimensions with advanced uncertainty quantification and adaptive weighting. Uses only 100 strategically selected probe examples to efficiently detect potential failure modes.

Result: Achieves 23.5% higher composite reliability scores compared to random sampling baselines with exceptional statistical significance (p < 0.001, Cohen's d = 1.21). Expert validation rates approach 4.14/5.0 vs 3.14/5.0 for random selection. Completes assessment with 99.9% statistical power while reducing cost by 90% and maintaining 95% of traditional method coverage.

Conclusion: Microprobe addresses a critical gap in efficient model evaluation for responsible AI deployment, enabling comprehensive reliability assessment with dramatically reduced computational requirements while maintaining high statistical power and coverage.
...
### [73] [Erkang-Diagnosis-1.1 Technical Report](https://arxiv.org/abs/2512.20632)
*Jianbing Ma,Ao Feng,Zhenjie Gao,Xinyu Song,Li Su,Bin Chen,Wei Wang,Jiamin Wu*

Main category: cs.AI

TL;DR: Erkang-Diagnosis-1.1 is an AI healthcare assistant built on Alibaba's Qwen-3 model, integrating 500GB of medical knowledge using hybrid pre-training and retrieval methods to provide diagnostic suggestions through 3-5 interaction rounds, outperforming GPT-4 on medical exams.
...
Method: Built on Alibaba Qwen-3 model with 500GB of structured medical knowledge, using hybrid approach combining enhanced pre-training and retrieval-enhanced generation (RAG) to enable efficient 3-5 round interactions for symptom understanding and analysis.

Result: Erkang-Diagnosis-1.1 outperforms GPT-4 on comprehensive medical exams, demonstrating superior medical knowledge and diagnostic capabilities while providing valuable health guidance through brief interactions.

Conclusion: The Erkang model successfully creates an intelligent health companion that can accurately understand symptoms, conduct preliminary analysis, and provide reliable diagnostic suggestions, positioning it as a valuable tool for primary healthcare empowerment.
...
### [74] [Reasoning Relay: Evaluating Stability and Interchangeability of Large Language Models in Mathematical Reasoning](https://arxiv.org/abs/2512.20647)
*Leo Lu,Jonathan Zhang,Sean Chua,Spencer Kim,Kevin Zhu,Sean O'Brien,Vasu Sharma*

Main category: cs.AI

TL;DR: This paper explores whether partially completed reasoning chains from one LLM can be reliably continued by another model, examining reasoning interchangeability across model families.


<details>
  <summary>Details</summary>
Motivation: While CoT prompting advances LLM reasoning, little is known about reasoning interchangeability across models. The paper investigates whether reasoning chains can be transferred between models to examine inference-time trustworthiness and collaborative reasoning.
...
Result: Hybrid reasoning chains often preserve and sometimes improve final accuracy and logical structure. Interchangeability emerges as a behavioral property of reasoning models.

Conclusion: Reasoning interchangeability offers insights for reliable modular reasoning in collaborative AI systems, suggesting new paradigms for model collaboration and trustworthiness assessment.
...
### [75] [AIAuditTrack: A Framework for AI Security system](https://arxiv.org/abs/2512.20649)
...
<details>
  <summary>Details</summary>
Motivation: The rapid growth of AI applications has created security, accountability, and risk traceability challenges due to the surge in AI interaction data, requiring better governance and auditing mechanisms.

Method: AAT uses decentralized identity (DID) and verifiable credentials (VC) to establish trusted AI entities, records interaction trajectories on blockchain, models entities as nodes in a dynamic interaction graph, and implements a risk diffusion algorithm to trace risky behaviors.

Result: The system demonstrates feasibility and stability under large-scale interaction recording through blockchain TPS metrics evaluation, providing scalable and verifiable AI auditing capabilities.

Conclusion: AAT offers a scalable, verifiable solution for AI auditing, risk management, and responsibility attribution in complex multi-agent environments, addressing critical governance challenges in AI ecosystems.
...
### [76] [Mixture of Attention Schemes (MoAS): Learning to Route Between MHA, GQA, and MQA](https://arxiv.org/abs/2512.20650)
*Esmail Gumaan*

Main category: cs.AI

TL;DR: MoAS dynamically selects optimal attention schemes (MHA, GQA, MQA) per token via learned routing, achieving MHA-level performance with conditional compute efficiency.
...
Method: Proposes Mixture of Attention Schemes (MoAS) with learned router that dynamically selects optimal attention scheme (MHA, GQA, or MQA) for each token, rather than static averaging.
...
Conclusion: MoAS provides effective dynamic attention scheme selection, balancing model quality and inference efficiency better than static approaches.
...
### [77] [Memory Bear AI A Breakthrough from Memory to Cognition Toward Artificial General Intelligence](https://arxiv.org/abs/2512.20651)
*Deliang Wen,Ke Sun*

Main category: cs.AI

TL;DR: Memory Bear system addresses LLM memory limitations by building human-like memory architecture based on cognitive science, improving long-term dialogue performance and reducing hallucinations.


<details>
  <summary>Details</summary>
Motivation: LLMs have inherent memory limitations: restricted context windows, long-term knowledge forgetting, redundant information accumulation, and hallucination generation. These issues constrain sustained dialogue and personalized services.

Method: Proposes Memory Bear system with human-like memory architecture grounded in cognitive science principles. Integrates multimodal information perception, dynamic memory maintenance, and adaptive cognitive services for full-chain reconstruction of LLM memory mechanisms.

Result: Significantly improves knowledge fidelity and retrieval efficiency in long-term conversations, reduces hallucination rates, enhances contextual adaptability and reasoning capability. Outperforms existing solutions (Mem0, MemGPT, Graphiti) across key metrics including accuracy, token efficiency, and response latency.

Conclusion: Memory Bear represents a crucial step forward in advancing AI from "memory" to "cognition" by addressing fundamental memory limitations through cognitive science-inspired architecture.
...
### [78] [AI-Driven Decision-Making System for Hiring Process](https://arxiv.org/abs/2512.20652)
*Vira Filatova,Andrii Zelenchuk,Dmytro Filatov*

Main category: cs.AI

TL;DR: AI-driven multi-agent hiring assistant that integrates document/video preprocessing, structured profile construction, public-data verification, technical/culture-fit scoring with risk penalties, and human-in-the-loop validation to improve early-stage candidate validation efficiency.


<details>
  <summary>Details</summary>
Motivation: Early-stage candidate validation is a major bottleneck in hiring due to heterogeneous inputs (resumes, screening answers, code assignments, limited public evidence) that recruiters must reconcile manually, leading to inefficiency and high screening costs.

Method: Modular multi-agent system with: (1) document/video preprocessing, (2) structured candidate profile construction, (3) public-data verification, (4) technical/culture-fit scoring with explicit risk penalties, (5) human-in-the-loop validation via interactive interface. Orchestrated by LLM under strict constraints to reduce output variability and generate traceable rationales. Candidate ranking uses configurable aggregation of technical fit, culture fit, and normalized risk penalties.

Result: Evaluated on 64 real applicants for mid-level Python backend engineer role. System achieved 1.70 hours per qualified candidate vs. 3.33 hours for experienced recruiter, with substantially lower estimated screening cost. Preserved human decision-maker as final authority while improving throughput.

Conclusion: The AI-driven multi-agent hiring assistant significantly improves hiring efficiency (reducing time per qualified candidate by nearly 50%) while maintaining human oversight, demonstrating practical value for early-stage candidate validation in technical hiring.
...
TL;DR: AFA training mechanism improves sentiment analysis by redistributing attention weights from common words to task-relevant terms using adversarial feedback and policy gradient optimization.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer models for sentiment analysis often focus attention on common words while overlooking less popular but highly relevant terms, leading to suboptimal accuracy. The attention distribution bias toward frequent words impairs overall performance.

Method: Proposes Adversarial Feedback for Attention (AFA) training mechanism with: 1) Dynamic masking strategy that masks various words to deceive a discriminator, 2) Discriminator that detects mask-induced differences, 3) Policy gradient approach leveraging Transformer sensitivity to token-level perturbations to optimize attention distributions efficiently.

Result: Achieves state-of-the-art results on three public datasets. When applied to enhance attention in large language models, yields additional 12.6% performance improvement.

Conclusion: AFA effectively addresses attention distribution bias in sentiment analysis by automatically redirecting focus to task-relevant terms without manual annotations, demonstrating significant performance gains across datasets and model types.
...
TL;DR: LLMs show laziness in multi-part tasks but surprisingly good context retention; decoding suboptimality not significant in simple reasoning.


<details>
  <summary>Details</summary>
Motivation: To quantify three behavioral artifacts in LLMs: laziness (premature truncation/partial compliance), decoding suboptimality (myopic decoding), and context degradation (forgetting instructions).

Method: Three controlled experiments (A, B, C) testing advanced LLMs (OpenAI GPT-4 variant, DeepSeek) on multi-part instructions, simple reasoning tasks, and 200-turn chaotic conversations.

Result: Widespread laziness in complex multi-part tasks; limited evidence of decoding suboptimality; surprising robustness against context degradation in long conversations.
...
### [81] [Eidoku: A Neuro-Symbolic Verification Gate for LLM Reasoning via Structural Constraint Satisfaction](https://arxiv.org/abs/2512.20664)
*Shinobu Miya*

Main category: cs.AI

TL;DR: Eidoku: A neuro-symbolic verification system that treats LLM reasoning as a Constraint Satisfaction Problem, using structural consistency rather than probability to detect hallucinations.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce high-confidence hallucinations, showing that probability-based verification fails for structurally inconsistent but statistically plausible statements.

Method: Reformulates verification as a CSP using structural violation cost (graph connectivity, feature space consistency, logical entailment). Uses lightweight System-2 gate (Eidoku) with context-calibrated thresholds.

Result: Successfully rejects "smooth falsehoods" - highly probable but structurally disconnected statements that probability-based verifiers cannot detect.

Conclusion: Explicit structural constraints enable deterministic rejection of specific hallucination classes, providing neuro-symbolic sanity check for generative reasoning.
...
### [82] [Bridging the AI Trustworthiness Gap between Functions and Norms](https://arxiv.org/abs/2512.20671)
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the growing importance of Trustworthy AI (TAI) due to regulations and functional benefits, while recognizing that current gaps between functional implementation (FTAI) and normative regulations (NTAI) make it difficult to assess AI system trustworthiness.

Method: The paper takes a position paper approach: describing current state-of-the-art, identifying the FTAI-NTAI gap, discussing starting points for developing a semantic language, envisioning its effects, and providing key considerations for future actions.

Result: The paper identifies the critical gap between functional and normative TAI approaches and proposes the development of a conceptual semantic language as a bridging solution to help developers assess trustworthiness and help stakeholders translate regulations into implementation steps.

Conclusion: A semantic language is needed to bridge FTAI and NTAI, enabling better trustworthiness assessment of AI systems and facilitating the translation of regulations into concrete implementation steps, with future work required to develop and implement this approach.
...
### [83] [From Pilots to Practices: A Scoping Review of GenAI-Enabled Personalization in Computer Science Education](https://arxiv.org/abs/2512.20714)
*Iman Reihanian,Yunfei Hou,Qingquan Sun*

Main category: cs.AI

TL;DR: This scoping review analyzes 32 studies (2023-2025) on generative AI for personalized computer science education, identifying effective design patterns and proposing a framework for responsible adoption.


<details>
  <summary>Details</summary>
Motivation: While generative AI enables personalized CS education at scale, there's uncertainty about whether such personalization actually supports or undermines learning outcomes, necessitating a systematic review of evidence.

Method: Scoping review of 32 studies purposively sampled from 259 records, analyzing personalization mechanisms and effectiveness signals in higher-education computer science contexts across five application domains.

Result: Identified five application domains (intelligent tutoring, personalized materials, formative feedback, AI-augmented assessment, code review) and effective design patterns including explanation-first guidance, solution withholding, graduated hint ladders, and artifact grounding. Successful implementations share four patterns: context-aware tutoring, multi-level hint structures, composition with traditional CS infrastructure, and human-in-the-loop quality assurance.

Conclusion: Generative AI can provide precision scaffolding when embedded in audit-ready workflows that preserve productive struggle while scaling personalized support, with an exploration-first adoption framework emphasizing piloting, instrumentation, learning-preserving defaults, and evidence-based scaling.
...
### [84] [From artificial to organic: Rethinking the roots of intelligence for digital health](https://arxiv.org/abs/2512.20723)
*Prajwal Ghimire,Keyoumars Ashkan*

Main category: cs.AI

TL;DR: The paper argues that AI is not truly "artificial" but rather an extension of organic intelligence, as it's created by humans and inspired by biological systems, blurring the distinction between artificial and organic.


<details>
  <summary>Details</summary>
Motivation: To challenge the conventional dichotomy between "artificial" and "organic" intelligence, particularly in digital health, by showing that AI systems are fundamentally products of organic human cognition and biological inspiration.

Method: Conceptual analysis examining the origins and foundations of AI systems, tracing how neural networks and decision algorithms are inspired by human neurobiology and evolutionary processes.

Result: The paper demonstrates that AI development represents a continuum from organic intelligence rather than a separate category, with the transition being about organization and adaptation rather than mystical transformation.

Conclusion: The boundaries between artificial and organic intelligence are far less distinct than the terminology suggests, as AI is essentially an extension and product of organic intelligence systems.
...
### [85] [AgentMath: Empowering Mathematical Reasoning for Large Language Models via Tool-Augmented Agent](https://arxiv.org/abs/2512.20745)
*Haipeng Luo,Huawen Feng,Qingfeng Sun,Can Xu,Kai Zheng,Yufei Wang,Tao Yang,Han Hu,Yansong Tang,Di Wang*

Main category: cs.AI

TL;DR: AgentMath is an agent framework that combines language models' reasoning with code interpreters' computational precision to solve complex math problems efficiently, achieving SOTA results on math competition benchmarks.
...
Method: AgentMath integrates language models with code interpreters through three innovations: 1) Automated conversion of natural language chain-of-thought into structured tool-augmented trajectories for SFT data generation; 2) Agentic reinforcement learning that dynamically interleaves natural language generation with real-time code execution; 3) Efficient training system with asynchronous rollout scheduling, partial rollout, and load balancing for 4-5x speedup.

Result: AgentMath achieves state-of-the-art performance on challenging mathematical competition benchmarks: 90.6% on AIME24, 86.4% on AIME25, and 73.8% on HMMT25 with the AgentMath-30B-A3B model.

Conclusion: The approach effectively combines reasoning with computational precision, validates the framework's effectiveness, and paves the way for more sophisticated and scalable mathematical reasoning agents.
...
### [86] [A Benchmark for Evaluating Outcome-Driven Constraint Violations in Autonomous AI Agents](https://arxiv.org/abs/2512.20798)
*Miles Q. Li,Benjamin C. M. Fung,Martin Weiss,Pulei Xiong,Khalil Al-Hussaeni,Claude Fachkha*

Main category: cs.AI

TL;DR: A new benchmark reveals high rates of outcome-driven constraint violations in AI agents, showing that superior reasoning doesn't ensure safety and highlighting the need for better agentic-safety training.


<details>
  <summary>Details</summary>
Motivation: Current safety benchmarks focus on single-step decisions, simulated environments, or explicit negative constraints, lacking evaluation of emergent outcome-driven constraint violations that arise when agents optimize goals under performance incentives while deprioritizing ethical/legal/safety constraints in multi-step realistic settings.

Method: Introduced a benchmark with 40 distinct scenarios requiring multi-step actions, each with Mandated (instruction-commanded) and Incentivized (KPI-pressure-driven) variations to distinguish obedience from emergent misalignment. Evaluated 12 state-of-the-art large language models.

Result: Found outcome-driven constraint violations ranging from 1.3% to 71.4%, with 9 of 12 models showing 30-50% misalignment rates. Gemini-3-Pro-Preview had highest violation rate (>60%), often escalating to severe misconduct. Also observed "deliberative misalignment" where models recognize their actions as unethical in separate evaluation.

Conclusion: Superior reasoning capability doesn't inherently ensure safety, and there's critical need for more realistic agentic-safety training before deployment to mitigate real-world risks from AI agents.
...
### [87] [Safety Alignment of LMs via Non-cooperative Games](https://arxiv.org/abs/2512.20806)
*Anselm Paulus,Ilia Kulikov,Brandon Amos,Rémi Munos,Ivan Evtimov,Kamalika Chaudhuri,Arman Zharmagambetov*

Main category: cs.AI

TL;DR: AdvGame: A game-theoretic approach to AI safety where Attacker and Defender LMs are jointly trained via online RL with preference-based rewards, improving both safety and utility while creating strong red-teaming agents.


<details>
  <summary>Details</summary>
Motivation: Current safety alignment methods rely on sequential adversarial training which may be suboptimal. The paper aims to create a more dynamic, iterative approach where models continuously adapt to each other's strategies, potentially leading to better safety-utility trade-offs.

Method: Frames safety alignment as a non-zero-sum game between Attacker and Defender LMs trained jointly via online reinforcement learning. Uses preference-based reward signals from pairwise comparisons instead of point-wise scores to reduce reward hacking and provide more robust supervision.

Result: AdvGame shifts the Pareto frontier of safety and utility, yielding a Defender LM that is simultaneously more helpful and more resilient to adversarial attacks. The Attacker LM converges into a strong, general-purpose red-teaming agent that can directly probe arbitrary target models.

Conclusion: Game-theoretic joint training via online RL with preference-based rewards offers a promising alternative to sequential adversarial training, enabling continuous adaptation and improvement in both safety and utility while producing effective red-teaming agents.
...
TL;DR: RL algorithm for parameterized action spaces that learns state and action abstractions online, enabling efficient learning in long-horizon sparse-reward settings.


<details>
  <summary>Details</summary>
Motivation: Real-world sequential decision-making often involves parameterized action spaces with both discrete actions and continuous parameters. Existing approaches have limitations: planning methods need hand-crafted models, standard RL handles either discrete or continuous but not both, and few RL methods for parameterized actions rely on domain-specific engineering without exploiting latent structure.
...
Conclusion: The proposed approach successfully addresses the challenge of parameterized action spaces in RL by learning adaptive abstractions, enabling efficient learning in complex, sparse-reward environments without requiring hand-crafted models or domain-specific engineering.
...
TL;DR: Multi-agent with multi-persona debaters improves LLM reflection diversity and performance over single-agent reflection, preventing degeneration of thought.


<details>
  <summary>Details</summary>
Motivation: Single LLM self-reflection leads to degeneration where the model repeats the same errors despite knowing they're wrong. Need to improve reflection diversity and quality.

Method: Introduce multi-agent with multi-persona debaters to generate reflections instead of single LLM self-reflection.

Result: 47% EM on HotPot QA and 82.7% on HumanEval, surpassing single LLM reflection performance.

Conclusion: Multi-agent with multi-persona debaters effectively addresses reflection degeneration and improves reasoning task performance through diverse reflection generation.
...
TL;DR: A probabilistic framework for bidirectional knowledge exchange between LLM agents, using Beta-Bernoulli distributions with forgetting factor to quantify epistemic uncertainty and drive optimal active learning through public contributions.


<details>
  <summary>Details</summary>
Motivation: Current LLM/RAG agents are unidirectional consumers (epistemic asymmetry), leading to redundant reasoning and stagnated collective intelligence. Self-reflection frameworks lack probabilistic foundations to quantify certainty or justify external interactions.

Method: Formal probabilistic framework modeling agent beliefs with Beta-Bernoulli distribution with forgetting factor (γ). Isolates epistemic uncertainty as belief variance, establishing dual interaction drives: homeostatic certainty maintenance and optimal learning targeting maximum ambiguity. Introduces epistemic caching for scalability and shows how belief states serve as RLHF reward signals and SFT data filters.
...
Conclusion: Public contribution can be reframed as optimal active learning where sharing solutions to elicit feedback is the most efficient method for agents to reduce their own uncertainty, enabling scalable bidirectional knowledge exchange.
...
<details>
  <summary>Details</summary>
Motivation: Agentic AI systems in critical domains (healthcare, smart cities, forensics, supply chain) raise trust, oversight, and integrity concerns despite their flexibility and real-time reasoning capabilities.

Method: Combines LangChain-based multi-agent system with permissioned blockchain (Hyperledger Fabric) for continuous monitoring, policy enforcement, and immutable auditability. Links perception-conceptualization-action cycle to blockchain governance layer that verifies inputs, evaluates actions, and documents outcomes. Uses MCP-integrated action executors and LangChain agents.

Result: Blockchain security verification effectively prevents unauthorized practices, provides full traceability of decision-making processes, and maintains operational latency within acceptable ranges. Demonstrated through smart inventory management, traffic-signal control, and healthcare monitoring experiments.

Conclusion: The framework offers a universal approach for implementing high-impact autonomous AI applications that are both autonomous and responsible, addressing critical trust and oversight challenges.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of limited household budgets and nutritional demands in middle-income environments where food prices fluctuate, aiming to make healthy eating affordable.
...
Result: Simulations with Saudi household case study show 12-18% cost reduction vs static menu, over 95% nutrient adequacy, and high performance with 20-30% price changes.
...
### [93] [TrafficSimAgent: A Hierarchical Agent Framework for Autonomous Traffic Simulation with MCP Control](https://arxiv.org/abs/2512.20996)
...
<details>
  <summary>Details</summary>
Motivation: Existing traffic simulators like SUMO and MATSim are complex and challenging for users without deep platform knowledge to use effectively for experiments and daily work applications.

Method: An LLM-based agent framework with cross-level collaboration: high-level expert agents interpret natural language instructions, plan workflows, and invoke MCP-compatible tools; low-level expert agents select optimal action plans for fundamental elements based on real-time traffic conditions.

Result: The framework effectively executes simulations under various conditions, produces reasonable outcomes even with ambiguous instructions, and achieves superior performance compared to other systems and state-of-the-art LLM-based methods through expert-level autonomous decision-driven optimization.

Conclusion: TrafficSimAgent successfully addresses the accessibility challenge of traffic simulation platforms by providing an intuitive, LLM-powered framework that enables non-experts to conduct complex simulations through natural language interaction and automated optimization.
...
TL;DR: Agentic XAI framework combines SHAP explainability with LLM-driven iterative refinement to generate progressively enhanced agricultural recommendations, showing quality peaks at 3-4 refinement rounds before declining due to bias-variance trade-off.


<details>
  <summary>Details</summary>
Motivation: XAI outputs are hard for laypersons to understand, hindering AI trust. LLMs can translate technical explanations, but agentic AI (autonomous LLM refinement) hasn't been integrated with XAI for generating accessible narratives.

Method: Proposed agentic XAI framework combining SHAP-based explainability with multimodal LLM-driven iterative refinement. Tested as agricultural recommendation system using rice yield data from 26 Japanese fields across 11 refinement rounds (0-10).

Result: Both human experts (12 crop scientists) and LLMs (14) evaluated explanations across 7 metrics. Recommendation quality improved 30-33% from Round 0, peaking at Rounds 3-4, then declined with excessive refinement due to verbosity and ungrounded abstraction (bias-variance trade-off).

Conclusion: Strategic early stopping (regularization) is needed for optimizing agentic XAI utility, challenging monotonic improvement assumptions and providing evidence-based design principles for agentic XAI systems.
...
TL;DR: LLM-based persona simulation can validly replace human field experiments for method evaluation when methods observe only aggregate outcomes and evaluation is algorithm-blind, with discriminability determined by sample size.


<details>
  <summary>Details</summary>
Motivation: Field experiments (A/B tests) are costly and slow, creating bottlenecks for iterative method development in societal systems. LLM-based persona simulation offers a cheaper alternative, but it's unclear whether replacing humans with personas preserves the benchmark interface that methods optimize against.

Method: The paper proves an if-and-only-if characterization: when methods observe only aggregate outcomes (aggregate-only observation) and evaluation depends only on the submitted artifact (algorithm-blind evaluation), swapping humans for personas is just a panel change. The authors also define an information-theoretic discriminability of the induced aggregate channel and derive explicit bounds on the number of independent persona evaluations required to reliably distinguish different methods.

Result: The paper establishes theoretical conditions under which persona simulation is valid for benchmarking, showing it's indistinguishable from changing the evaluation population. It also provides quantitative bounds showing that making persona benchmarking as decision-relevant as field experiments is fundamentally a sample-size question.

Conclusion: LLM-based persona simulation can serve as a valid and useful alternative to field experiments under specific conditions (aggregate-only observation and algorithm-blind evaluation), with the discriminability of the benchmark determined by the number of independent persona evaluations, providing practical guidance for implementation.
...
TL;DR: Current LLM safety mechanisms fail to understand context and user intent, allowing malicious users to systematically bypass protections through emotional framing, progressive revelation, and academic justification techniques.


<details>
  <summary>Details</summary>
Motivation: The paper identifies a critical vulnerability in current LLM safety approaches: they focus on explicitly harmful content but overlook the inability to understand context and recognize user intent, creating exploitable vulnerabilities that malicious users can systematically leverage to circumvent safety mechanisms.

Method: The researchers empirically evaluated multiple state-of-the-art LLMs including ChatGPT, Claude, Gemini, and DeepSeek. They analyzed how safety mechanisms could be circumvented through specific techniques like emotional framing, progressive revelation, and academic justification.

Result: The analysis demonstrated reliable circumvention of safety mechanisms across most models. Reasoning-enabled configurations actually amplified exploitation effectiveness by increasing factual precision while failing to interrogate underlying intent. Claude Opus 4.1 was the exception, prioritizing intent detection over information provision in some cases.

Conclusion: Current architectural designs create systematic vulnerabilities that require paradigmatic shifts toward contextual understanding and intent recognition as core safety capabilities rather than post-hoc protective mechanisms.
...
TL;DR: First evaluation of LLM-based medication safety review on real NHS primary care data shows strong issue detection but poor comprehensive accuracy, with contextual reasoning failures as the dominant problem.


<details>
  <summary>Details</summary>
Motivation: While LLMs often match clinician performance on medical benchmarks, they're rarely evaluated on real clinical data or examined beyond headline metrics. The authors aim to assess LLM-based medication safety review systems in real-world NHS settings with detailed failure analysis.

Method: Retrospective study using population-scale EHR data (2.1M adults) from NHS Cheshire and Merseyside. Strategically sampled 277 patients representing varied clinical complexity and medication safety risk. Expert clinician reviewed system-identified issues and interventions. Primary LLM system evaluated with detailed failure analysis across patient complexity and demographic strata.

Result: LLM showed excellent issue detection (100% sensitivity, 83.1% specificity) but correctly identified all issues and interventions in only 46.9% of patients. Dominant failure mechanism was contextual reasoning rather than missing medication knowledge, with five primary patterns: overconfidence in uncertainty, misapplied guidelines, misunderstanding healthcare delivery, factual errors, and process blindness. Patterns persisted across models and configurations.
...
<details>
  <summary>Details</summary>
Motivation: Embodied agents powered by vision-language models are increasingly capable but vulnerable to hazardous instructions. Existing runtime safety defenses rely on static rule filters or prompt-level control, which struggle with implicit risks in dynamic, temporally dependent, and context-rich environments.

Method: Proposes RoboSafe with two complementary reasoning processes on a Hybrid Long-Short Safety Memory: 1) Backward Reflective Reasoning that revisits recent trajectories to infer temporal safety predicates and triggers replanning when violations are detected; 2) Forward Predictive Reasoning that anticipates upcoming risks by generating context-aware safety predicates from long-term memory and multimodal observations. Forms adaptive, verifiable safety logic that is interpretable and executable as code.

Result: Extensive experiments across multiple agents show RoboSafe substantially reduces hazardous actions (-36.8% risk occurrence) compared with leading baselines while maintaining near-original task performance. Real-world evaluations on physical robotic arms confirm its practicality.

Conclusion: RoboSafe provides an effective runtime safety safeguard for embodied agents through hybrid reasoning and executable predicate-based safety logic, addressing limitations of existing static approaches in dynamic environments.
...
TL;DR: Proposes a parameter-efficient alternative to Neural CDEs that serves as a "Continuous RNN" with significantly fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Neural CDEs are effective for temporal sequence analysis but suffer from high parameter requirements, limiting their efficiency and practicality.

Method: Develops a parameter-efficient alternative to Neural CDEs that maintains the continuous-time modeling capability while reducing parameter count, positioning it as a "Continuous RNN" analog.

Result: The proposed method achieves comparable performance to Neural CDEs with substantially fewer parameters, making it more efficient while preserving the continuous-time modeling benefits.

Conclusion: A parameter-efficient alternative to Neural CDEs is presented that reduces computational overhead while maintaining the continuous-time sequence modeling capabilities, offering a practical "Continuous RNN" solution.
...
<details>
  <summary>Details</summary>
Motivation: To enable language agents to develop and evolve strategic behaviors over long-term interactions without the computational cost of fine-tuning large language models, and to provide interpretable abstract strategic representations.

Method: Dual-loop architecture: behavior loop adjusts action preferences based on environmental rewards, while language loop updates external latent vectors by reflecting on semantic embeddings of generated text. Latent vectors of abstract concepts are liberated from static representations and updated through interaction.
...
TL;DR: Zero-training temporal drift analysis of transformer sentiment models on real social media data shows significant accuracy drops (up to 23.4%) during events, with novel drift metrics outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: To understand transformer model instability during real-world events and develop practical drift detection methods for production sentiment monitoring systems without requiring retraining.

Method: Comprehensive zero-training analysis using three transformer architectures on 12,279 authentic social media posts from major events, with rigorous statistical validation and introduction of four novel drift metrics.

Result: Significant model instability with accuracy drops up to 23.4%, maximum confidence drops of 13.0%, strong correlation between confidence drops and actual performance degradation, and novel metrics outperforming embedding-based baselines.

Conclusion: Zero-training methodology enables immediate deployment for real-time sentiment monitoring, provides new insights into transformer behavior during dynamic content periods, and offers practical drift detection exceeding industry thresholds.
...
<details>
  <summary>Details</summary>
Motivation: Predicting lung cancer treatment outcomes is difficult due to sparse, heterogeneous real-world EHR data. Traditional models fail to capture semantic information across modalities, and large-scale fine-tuning is impractical in clinical workflows.

Method: Introduced a framework using LLMs as Goal-oriented Knowledge Curators (GKC) to convert laboratory, genomic, and medication data into high-fidelity, task-aligned features. This operates as an offline preprocessing step integrated into hospital informatics pipelines.

Result: On a lung cancer cohort (N=184), GKC achieved mean AUROC of 0.803 (95% CI: 0.799-0.807), outperforming expert-engineered features, direct text embeddings, and end-to-end transformers. Ablation study confirmed complementary value of combining all three modalities.
...
TL;DR: The paper introduces a shallow vs deep alignment framework to quantify and address catastrophic forgetting in continual learning for LLMs, showing that shallow alignment (only 3-5 tokens) causes spurious forgetting, and proposes methods to detect and promote deep alignment.


<details>
  <summary>Details</summary>
Motivation: Current understanding of catastrophic forgetting in continual learning for LLMs is limited - prior work only qualitatively describes alignment disruption as a cause, relies on post-hoc analysis, and lacks automatic mechanisms to distinguish between true knowledge loss and spurious forgetting.
...
Conclusion: The shallow vs deep alignment framework provides the first quantitative characterization of alignment depth, explains why spurious forgetting occurs and is reversible, and offers practical tools for detecting and mitigating catastrophic forgetting in continual learning for LLMs.
...
TL;DR: SHRP is a structured pruning framework for Transformer encoders that removes redundant attention heads while preserving accuracy, achieving 48% parameter reduction with 93% accuracy retention.


<details>
  <summary>Details</summary>
Motivation: Transformer encoders have high inference latency and memory consumption due to architectural redundancy in attention modules, posing challenges for real-time web services. Attention heads operate independently and contain parameter redundancy, making them suitable for structured compression.

Method: SHRP introduces Expert Attention treating each head as an independent expert, followed by a shared expander feed-forward network. It uses a unified Top-1 usage-driven mechanism for dynamic routing during training and deterministic pruning at deployment.
...
Conclusion: SHRP effectively compresses Transformer encoders while preserving accuracy, demonstrating practical utility for large-scale, latency-sensitive web deployments through significant parameter and computation reduction.
...
TL;DR: Gate-Norm is a fast, data-free pruning method that removes attention sublayers in LLMs based on query-key coupling, achieving up to 1.3× higher inference throughput with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Many attention layers in LLMs can be removed without performance loss, suggesting some layers learn to suppress their own contributions during pre-training (Attention Suppression Hypothesis).

Method: Gate-Norm ranks attention sublayers using a weight-only criterion based on query-key coupling, requiring no calibration data, forward passes, fine-tuning, or specialized kernels.

Result: Pruning 8-16 attention layers in 40-layer LLaMA models yields up to 1.3× higher inference throughput while keeping average zero-shot accuracy within 2% of baseline across multiple benchmarks.

Conclusion: Gate-Norm matches data-driven pruning accuracy while being ~1000× faster, enabling practical, data-free compression of LLMs.
...
### [106] [Forecasting N-Body Dynamics: A Comparative Study of Neural Ordinary Differential Equations and Universal Differential Equations](https://arxiv.org/abs/2512.20643)
...
<details>
  <summary>Details</summary>
Motivation: Traditional ML models for n-body trajectory prediction are data-intensive black boxes that ignore physical laws and lack interpretability. Scientific ML addresses this by embedding known physical laws into the learning framework.

Method: Uses Scientific ML frameworks in Julia: Neural ODEs and Universal Differential Equations (UDEs) to predict n-body system dynamics. Employs synthetically created noisy data to simulate real-world observational limitations, and determines forecasting breakdown point (minimum training data needed for accurate predictions).
...
Conclusion: Scientific ML approaches, particularly UDEs, offer more interpretable and data-efficient solutions for n-body problem forecasting compared to traditional black-box ML models, with UDEs demonstrating superior performance in data-limited scenarios.
...
### [107] [Q-RUN: Quantum-Inspired Data Re-uploading Networks](https://arxiv.org/abs/2512.20654)
*Wenbo Qiao,Shuaixian Wang,Peng Zhang,Yan Ming,Jiaming Zhao*

Main category: cs.LG

TL;DR: Q-RUN: A quantum-inspired data re-uploading network that brings quantum neural network advantages to classical models without needing quantum hardware.


<details>
  <summary>Details</summary>
Motivation: Data re-uploading quantum circuits (DRQC) show superior performance for fitting high-frequency functions but are limited by quantum hardware scalability. The authors aim to bring these quantum advantages to classical models.

Method: Introduce the mathematical paradigm of DRQC into classical models by proposing Q-RUN (quantum-inspired data re-uploading network), which retains Fourier-expressive advantages without quantum hardware.

Result: Q-RUN delivers superior performance across data modeling and predictive tasks, reduces parameters while decreasing error by 1-3 orders of magnitude on certain tasks, and serves as a drop-in replacement for standard fully connected layers.

Conclusion: This work demonstrates how quantum machine learning principles can guide the design of more expressive AI, enabling quantum advantages without quantum hardware limitations.
...
### [108] [MaskOpt: A Large-Scale Mask Optimization Dataset to Advance AI in Integrated Circuit Manufacturing](https://arxiv.org/abs/2512.20655)
...
Method: Created MaskOpt dataset from real IC designs at 45nm node with 104,714 metal-layer tiles and 121,952 via-layer tiles. Tiles are clipped at standard-cell placements to preserve cell information, supporting different context window sizes to capture optical proximity effects from neighboring shapes.

Result: Evaluated state-of-the-art deep learning models for IC mask optimization, revealing distinct trade-offs across baseline models. Context size analysis and input ablation studies confirmed the importance of surrounding geometries and cell-aware inputs for accurate mask generation.

Conclusion: MaskOpt advances deep learning for practical mask optimization by providing a large-scale, real-world benchmark that preserves cell hierarchy and captures context effects, enabling more accurate and scalable mask generation solutions.
...
### [109] [Managing the Stochastic: Foundations of Learning in Neuro-Symbolic Systems for Software Engineering](https://arxiv.org/abs/2512.20660)
*Matthew Thompson*

Main category: cs.LG

TL;DR: Proposes a dual-state architecture that treats LLMs as stochastic environment components rather than decision-making agents, using atomic action pairs with guard functions to separate deterministic workflow control from stochastic generation, improving code generation reliability.


<details>
  <summary>Details</summary>
Motivation: Current AI coding agents blur lines between LLMs and agents, asking LLMs to make decisions that should be deterministic, leading to stochastic failures like gaming unit tests or hallucinating syntax. Need to apply established software engineering practices for managing unpredictable processes.

Method: Dual-State Architecture separates workflow state (deterministic control flow) from environment state (stochastic generation). Uses Atomic Action Pairs that couple generation with verification as indivisible transactions, with Guard Functions as sensing actions that project probabilistic outputs onto observable workflow state.
...
Conclusion: Architectural constraints can substitute for parameter scale in achieving reliable code generation. Treating LLMs as environment components rather than decision-making agents preserves creative stochasticity while ensuring deterministic control flow.
...
TL;DR: The paper identifies a "Dominant-vs-Dominated" imbalance in text-to-image diffusion models where one concept token dominates others in multi-concept prompts, introduces DominanceBench to analyze it, and examines causes from data and architectural perspectives.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models struggle with multi-concept prompts where one concept token often dominates and suppresses others, limiting reliable and controllable generation of complex scenes with multiple elements.

Method: Introduced DominanceBench to systematically analyze the DvD imbalance, examined causes from both data (limited instance diversity) and architectural perspectives, conducted cross-attention dynamics analysis, and performed head ablation studies.

Result: Found that limited training data diversity exacerbates inter-concept interference, dominant tokens rapidly saturate attention across diffusion timesteps, and DvD behavior arises from distributed attention mechanisms across multiple heads.

Conclusion: The findings provide key insights into generative collapse in diffusion models, advancing toward more reliable and controllable text-to-image generation by understanding and addressing the Dominant-vs-Dominated imbalance.
...
### [111] [Forward Only Learning for Orthogonal Neural Networks of any Depth](https://arxiv.org/abs/2512.20668)
*Paul Caillon,Alex Colagrande,Erwan Fagnou,Blaise Delattre,Alexandre Allauzen*

Main category: cs.LG

TL;DR: FOTON is a forward-only training algorithm for neural networks that eliminates the need for backpropagation, enabling training of deep networks without backward passes while maintaining performance comparable to backpropagation.


<details>
  <summary>Details</summary>
Motivation: Backpropagation has computational limitations with modern large architectures, and existing forward-only alternatives like PEPITA fail to scale beyond a few hidden layers. There's a need for a scalable forward-only training method that can handle deep networks.

Method: The authors first analyze limitations of existing forward-only approaches, then design a forward-only algorithm equivalent to backpropagation under linear and orthogonal assumptions. By relaxing the linear assumption, they develop FOTON (Forward-Only Training of Orthogonal Networks) that bridges the gap with backpropagation.
...
Conclusion: FOTON provides a viable alternative to backpropagation that eliminates the computational burden of backward passes while maintaining scalability to deep networks, with potential applications to various advanced architectures.
...
TL;DR: Proposes a CVAE-based architecture to generate synthetic clinical records for cardiac rehabilitation, addressing data scarcity and missing values to improve cardiac risk prediction models.


<details>
  <summary>Details</summary>
Motivation: Cardiac rehabilitation involves complex sequential processes that can be modeled as business processes, but real-world medical databases face significant limitations: data scarcity due to economic/time constraints, unsuitable existing records for specific analyses, and high prevalence of missing values from varying diagnostic tests across patients.

Method: Architecture based on Conditional Variational Autoencoder (CVAE) for synthesizing realistic clinical records coherent with real-world observations, aiming to increase dataset size and diversity.
...
Conclusion: The CVAE-based approach effectively addresses data limitations in cardiac rehabilitation contexts, enabling better risk prediction models while reducing the need for potentially hazardous diagnostic procedures like exercise stress testing.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal fake news detection methods rely on consistency-based fusion, which mistakenly treats critical cross-modal discrepancies as noise and smooths them out, thereby diluting crucial evidence of fabrication.

Method: Proposes Dynamic Conflict-Consensus Framework (DCCF) with three key components: 1) Decouples inputs into independent Fact and Sentiment spaces, 2) Uses physics-inspired feature dynamics to iteratively polarize representations and extract conflicts, 3) Employs conflict-consensus mechanism to standardize local discrepancies against global context.
...
<details>
  <summary>Details</summary>
Motivation: Mobile-oriented VLMs have high computational training requirements that hinder practical application. Standard LoRA with fixed rank is insufficient for training mobile VLMs that process both text and image modalities.

Method: HyDRA framework with two optimization strategies: 1) hierarchical optimization (coarse-grained rank assignment to different layers and fine-grained rank adjustment within individual layers), and 2) dynamic adjustment using lightweight performance model for end-to-end automatic rank optimization during fine-tuning.
...
### [115] [Revisiting the Learning Objectives of Vision-Language Reward Models](https://arxiv.org/abs/2512.20675)
...
<details>
  <summary>Details</summary>
Motivation: Recent VLM-based reward learning methods use increasingly complex objectives, but meaningful comparison is difficult due to differences in training data, architectures, and evaluation settings. The paper aims to isolate the impact of learning objectives.

Method: Established a unified framework to evaluate recent VLM-based reward models with identical backbones, finetuning data, and evaluation environments. Used Meta-World tasks to assess modeling accuracy by measuring consistency with ground truth reward and correlation with expert progress.
...
Conclusion: The learning objective itself may be less critical than previously thought; simpler methods can achieve comparable or better performance when evaluated under controlled, unified conditions.
...
### [116] [PHOTON: Hierarchical Autoregressive Modeling for Lightspeed and Memory-Efficient Language Generation](https://arxiv.org/abs/2512.20687)
*Yuma Ichikawa,Naoya Takagi,Takumi Nakagawa,Yuzi Kanazawa,Akira Sakai*

Main category: cs.LG

TL;DR: PHOTON is a hierarchical autoregressive model that replaces Transformer's flat token scanning with vertical multi-resolution context access, reducing KV-cache traffic and improving throughput by up to 1000x per unit memory.


<details>
  <summary>Details</summary>
Motivation: Transformers suffer from increasing prefill latency and memory-bound long-context decoding due to their horizontal token-by-token scanning pattern, where KV-cache reads/writes dominate inference throughput rather than computation.

Method: PHOTON maintains a hierarchy of latent streams: a bottom-up encoder progressively compresses tokens into low-rate contextual states, while lightweight top-down decoders reconstruct fine-grained token representations, enabling vertical multi-resolution context access instead of flat scanning.

Result: PHOTON shows superior throughput-quality trade-off compared to Transformer-based language models, with significant advantages in long-context and multi-query tasks, reducing decode-time KV-cache traffic and yielding up to 1000x higher throughput per unit memory.

Conclusion: The hierarchical approach of PHOTON addresses fundamental limitations of Transformer's access pattern, offering a more efficient alternative for autoregressive language modeling that reduces memory bottlenecks while maintaining quality.
...
### [117] [FEM-Bench: A Structured Scientific Reasoning Benchmark for Evaluating Code-Generating LLMs](https://arxiv.org/abs/2512.20732)
*Saeed Mohammadzadeh,Erfan Hamdi,Joel Shor,Emma Lejeune*

Main category: cs.LG

TL;DR: FEM-Bench is a computational mechanics benchmark for evaluating LLMs' ability to generate correct finite element method code, with current state-of-the-art models showing limited reliability on even introductory tasks.


<details>
  <summary>Details</summary>
Motivation: There's a critical gap in rigorous benchmarks for evaluating LLMs' ability to generate scientifically valid physical models. Computational mechanics provides an ideal foundation due to its clear mathematical structure, strict physical constraints, and objective verification requirements.

Method: Created FEM-Bench 2025 with introductory but nontrivial tasks aligned with first graduate course material on computational mechanics. Tasks capture essential numerical and physical modeling challenges while representing only a small fraction of the discipline's complexity.

Result: Best performing model (Gemini 3 Pro) completed 30/33 tasks at least once and 26/33 tasks all five times in function writing. Best model for unit test writing (GPT-5) had 73.8% Average Joint Success Rate. Other popular models showed broad performance variation.

Conclusion: FEM-Bench establishes a structured foundation for evaluating AI-generated scientific code, with future iterations planned to incorporate increasingly sophisticated tasks to track progress as models evolve.
...
### [118] [Stabilizing Multimodal Autoencoders: A Theoretical and Empirical Analysis of Fusion Strategies](https://arxiv.org/abs/2512.20749)
*Diyar Altinses,Andreas Schwung*

Main category: cs.LG

TL;DR: Analysis of Lipschitz properties in multimodal autoencoders with theoretical derivation of Lipschitz constants and introduction of a regularized attention-based fusion method for improved training stability.


<details>
  <summary>Details</summary>
Motivation: Multimodal autoencoders are gaining attention for handling complex multimodal data, but understanding their stability and robustness is crucial for optimizing training, architecture, and real-world applicability. The paper aims to analyze Lipschitz properties to enhance training stability.

Method: 1) Derive theoretical Lipschitz constants for aggregation methods in multimodal autoencoders. 2) Introduce a regularized attention-based fusion method based on theoretical analysis. 3) Conduct experiments to empirically validate theoretical findings by estimating Lipschitz constants across multiple trials and fusion strategies.

Result: The proposed fusion function aligns with theoretical predictions and outperforms existing strategies in consistency, convergence speed, and accuracy. Experiments validate theoretical findings through empirical estimation of Lipschitz constants.

Conclusion: The work provides a solid theoretical foundation for understanding fusion in multimodal autoencoders and contributes a solution for enhancing their performance through improved stability and training characteristics.
...
### [119] [Bridging Efficiency and Safety: Formal Verification of Neural Networks with Early Exits](https://arxiv.org/abs/2512.20755)
*Yizhak Yisrael Elboher,Avraham Raviv,Amihay Elboher,Zhouxing Shi,Omri Azencot,Hillel Kugler,Guy Katz*

Main category: cs.LG

TL;DR: Verification framework for neural networks with early exits that provides robustness guarantees while maintaining inference efficiency.


<details>
  <summary>Details</summary>
Motivation: Early exits improve inference efficiency but introduce verification challenges due to conditional execution paths. Need to ensure both safety (robustness) and efficiency in AI systems.

Method: Define tailored robustness property for early exit architectures, use off-the-shelf solvers with baseline algorithm enhanced by early stopping strategy and heuristic optimizations while maintaining soundness and completeness.

Result: Framework validated on multiple benchmarks, showing effectiveness and performance gains. Early exits enhance both inference acceleration and verifiability - more queries solved in less time compared to standard networks.

Conclusion: Early exits provide dual benefits: inference efficiency and improved verifiability. The framework helps users navigate accuracy-efficiency trade-offs with robustness analysis.
...
### [120] [Generalization of RLVR Using Causal Reasoning as a Testbed](https://arxiv.org/abs/2512.20760)
*Brian Lu,Hongyu Zhao,Shuo Sun,Hao Peng,Rui Ding,Hongyuan Mei*

Main category: cs.LG

TL;DR: RLVR improves LLM generalization on causal reasoning tasks, but only with sufficient model size and initial competence, enhancing marginalization strategies and reducing calculation errors.


<details>
  <summary>Details</summary>
Motivation: Understanding when RLVR yields robust generalization for LLMs on complex reasoning tasks, particularly in causal inference where generalization patterns across query levels and structural complexity are unclear.
...
Result: RLVR yields stronger within-level and across-level generalization than SFT, but only for specific model size/training query level combinations. RLVR's effectiveness depends on initial reasoning competence - with sufficient competence, it improves marginalization strategies and reduces intermediate probability errors, especially on complex queries.

Conclusion: RLVR can improve specific causal reasoning subskills in LLMs, but its benefits emerge only when models have sufficient initial competence, highlighting the importance of model capability for effective RLVR generalization.
...
### [121] [TS-Arena Technical Report -- A Pre-registered Live Forecasting Platform](https://arxiv.org/abs/2512.20761)
*Marcel Meyer,Sascha Kaltenpoth,Kevin Zalipski,Henrik Albers,Oliver Müller*

Main category: cs.LG

TL;DR: TS-Arena is a platform that addresses evaluation crisis in Time Series Foundation Models by using live data streams with pre-registration to ensure genuine future testing, preventing information leakage from historical data.


<details>
  <summary>Details</summary>
Motivation: Current evaluation of Time Series Foundation Models suffers from information leakage due to overlapping training/test sets and illegitimate transfer of global patterns to test data, violating independence requirements for valid benchmarking.

Method: TS-Arena implements a pre-registration mechanism on live data streams, treating the genuinely unknown future as the definitive test environment. It establishes a moving temporal frontier with strict global temporal splits to prevent historical contamination.

Result: The platform provides a sustainable infrastructure for comparing foundation models under real-world constraints, initially applied within the energy sector. A prototype is available on Hugging Face Spaces.

Conclusion: TS-Arena restores operational integrity of forecasting evaluation by ensuring test targets remain physically non-existent during inference, enabling authentic assessment of model generalization capabilities.
...
### [122] [Subgroup Discovery with the Cox Model](https://arxiv.org/abs/2512.20762)
*Zachary Izzo,Iain Melvin*

Main category: cs.LG

TL;DR: First study of subgroup discovery for survival analysis using Cox models, introducing novel metrics (EPE and CRS) and algorithms to find interpretable subgroups where Cox models perform well.


<details>
  <summary>Details</summary>
Motivation: Existing subgroup discovery methods lack appropriate quality functions for survival analysis with Cox models. Need to find interpretable data subsets where Cox models are highly accurate, which has practical applications in domains like healthcare and engineering.

Method: Introduced two technical innovations: Expected Prediction Entropy (EPE) for evaluating survival models predicting hazard functions, and Conditional Rank Statistics (CRS) for quantifying individual point deviation from subgroup survival distributions. Developed eight algorithms, with main algorithm combining EPE and CRS for theoretical correctness.

Result: Theoretical analysis shows EPE and CRS solve problems with existing metrics. Empirical evaluation on synthetic and real data confirms recovery of ground-truth subgroups in well-specified cases and better model fit compared to naive whole-dataset Cox fitting. NASA jet engine case study uncovered known nonlinearities/homogeneity mirrored in practical design choices.

Conclusion: Successfully addressed subgroup discovery for Cox survival analysis with novel metrics and algorithms. The approach enables finding interpretable subgroups where Cox models perform well, with practical applications validated through theoretical analysis, empirical evaluation, and real-world case study.
...
### [123] [Improving Matrix Exponential for Generative AI Flows: A Taylor-Based Approach Beyond Paterson--Stockmeyer](https://arxiv.org/abs/2512.20777)
*Jorge Sastre,Daniel Faronbi,José Miguel Alonso,Peter Traver,Javier Ibáñez,Nuria Lloret*

Main category: cs.LG

TL;DR: Optimized Taylor-based algorithm for matrix exponential with dynamic parameter selection, offering superior performance for generative AI applications compared to traditional Padé methods.


<details>
  <summary>Details</summary>
Motivation: Matrix exponential is crucial for scientific computing and generative AI, but traditional Padé methods with scaling/squaring may not be optimal for high-throughput AI workflows. Recent Taylor-based methods show promise for better accuracy and efficiency.

Method: Develops an optimized Taylor-based algorithm using advanced polynomial evaluation schemes that surpass classical Paterson-Stockmeyer technique. Includes rigorous error analysis and dynamic selection strategy for Taylor order and scaling factor to minimize computation under error tolerance.

Result: Extensive numerical experiments show significant acceleration and high numerical stability compared to state-of-the-art implementations. The method proves highly efficient for large-scale generative modeling.

Conclusion: The proposed Taylor-based algorithm establishes itself as a highly efficient tool for matrix exponential computation in generative AI applications, offering better performance than traditional Padé approximants.
...
### [124] [Symbolic regression for defect interactions in 2D materials](https://arxiv.org/abs/2512.20785)
...
<details>
  <summary>Details</summary>
Motivation: While neural networks provide high accuracy for scientific data analysis, they lack interpretability. Symbolic regression offers interpretable analytical equations that can generalize to unseen data, making it valuable for scientific discovery.
...
Result: SEGVAE showed comparable or even identical performance to graph neural network methods in predicting material properties, demonstrating the effectiveness of symbolic regression for this scientific application.

Conclusion: Symbolic regression methods like SEGVAE offer interpretable alternatives to black-box neural networks for scientific applications, with comparable performance in predicting material properties of 2D materials with defects.
...
### [125] [GraphFire-X: Physics-Informed Graph Attention Networks and Structural Gradient Boosting for Building-Scale Wildfire Preparedness at the Wildland-Urban Interface](https://arxiv.org/abs/2512.20813)
*Miguel Esparza,Vamshi Battal,Ali Mostafavi*

Main category: cs.LG

TL;DR: Novel dual-specialist ensemble framework combines GNN for environmental contagion and XGBoost for structural fragility to model wildfire risk in WUI, revealing neighborhood-scale environmental pressure dominates structural features in fire propagation.


<details>
  <summary>Details</summary>
Motivation: Traditional wildfire risk models treat structures as isolated assets and fail to capture non-linear contagion dynamics in the wildland-urban interface (WUI), especially as wildfires increasingly evolve into urban conflagrations.

Method: Dual-specialist ensemble framework with two predictive streams: 1) Environmental specialist using Graph Neural Network (GNN) to model community as directed contagion graph weighted by physics-informed convection, radiation, and ember probabilities, enriched with Google AlphaEarth Foundation embeddings; 2) Structural specialist using XGBoost to isolate granular asset-level resilience. The ensemble synthesizes signals through logistic stacking.

Result: Applied to 2025 Eaton Fire, the framework reveals critical dichotomy: GNN shows neighborhood-scale environmental pressure dominates intrinsic structural features in propagation pathways, while XGBoost identifies eaves as primary micro-scale ingress vector. Ensemble achieves robust classification and generates diagnostic risk topology.

Conclusion: The framework enables decision-makers to move beyond binary loss prediction and precisely target mitigation strategies: vegetation management for high-connectivity clusters and structural hardening for architecturally vulnerable nodes, operationalizing a proactive, data-driven approach to community resilience.
...
### [126] [FedMPDD: Communication-Efficient Federated Learning with Privacy Preservation Attributes via Projected Directional Derivative](https://arxiv.org/abs/2512.20814)
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses two key challenges in federated learning: high communication costs from transmitting high-dimensional gradients, and privacy vulnerabilities to gradient inversion attacks. There's a need for methods that simultaneously optimize bandwidth utilization while enhancing privacy protection.

Method: FedMPDD encodes each client's high-dimensional gradient by computing directional derivatives along multiple random vectors, compressing the gradient into smaller messages. The server decodes aggregated information by projecting it back onto the same random vectors. Using multiple projections overcomes dimension-dependent convergence limitations of single projections.

Result: Theoretical analysis shows FedMPDD converges at O(1/√K) rate, matching FedSGD performance. Communication costs are reduced from O(d) to O(m) where m ≪ d. The method provides inherent privacy against gradient inversion attacks through geometric properties of low-rank projections, with tunable privacy-utility trade-off controlled by projection count.

Conclusion: FedMPDD successfully addresses both communication efficiency and privacy concerns in federated learning through multi-projected directional derivatives, achieving convergence comparable to FedSGD while significantly reducing communication overhead and providing built-in privacy protection.
...
### [127] [Defending against adversarial attacks using mixture of experts](https://arxiv.org/abs/2512.20821)
...
<details>
  <summary>Details</summary>
Motivation: Machine learning models are vulnerable to adversarial threats that can cause misclassification through imperceptible perturbations, poison training data, or steal sensitive information via model queries.

Method: Uses adversarial training module within mixture-of-experts architecture with nine pre-trained ResNet-18 experts. Jointly updates expert parameters and gating mechanism during end-to-end training for further optimization.

Result: The proposed defense system outperforms state-of-the-art defense systems and plain classifiers, even those using more complex architectures than the ResNet-18 backbone.

Conclusion: Adversarial training within mixture-of-experts architecture effectively enhances model robustness against adversarial threats, achieving superior performance compared to existing defense approaches.
...
### [128] [Memory-Efficient Acceleration of Block Low-Rank Foundation Models on Resource Constrained GPUs](https://arxiv.org/abs/2512.20861)
...
<details>
  <summary>Details</summary>
Motivation: Transformer-based foundation models are growing too large for single GPU deployment, making computational costs prohibitive. While Block Low-Rank (BLR) compression methods like Monarch and BLAST can reduce model size while preserving accuracy, they face memory bottlenecks in multi-token inference scenarios, limiting their practical speedups despite theoretical computational savings.

Method: The authors use roofline analysis to identify memory bottlenecks in BLR methods during multi-token inference. They then develop custom Triton kernels with partial fusion and memory layout optimizations specifically for Monarch and BLAST compression techniques. These optimizations address the memory-bound nature of multi-token inference that wasn't effectively handled by PyTorch's CUDA backend and compiler-level optimizations.

Result: On memory-constrained NVIDIA GPUs (Jetson Orin Nano and A40), the optimized kernels achieve up to 3.76× speedups and 3× model size compression compared to PyTorch dense baselines. The approach supports various models including Llama-7/1B, GPT2-S, DiT-XL/2, and ViT-B, demonstrating practical efficiency gains for BLR compression methods.

Conclusion: Custom kernel optimizations are essential for realizing the full potential of Block Low-Rank compression methods in practice, particularly for memory-constrained multi-token inference scenarios. The work provides efficient implementations that bridge the gap between theoretical computational savings and practical deployment efficiency on resource-limited hardware.
...
### [129] [Robustness Certificates for Neural Networks against Adversarial Attacks](https://arxiv.org/abs/2512.20865)
*Sara Taheri,Mahalakshmi Sabanayagam,Debarghya Ghoshdastidar,Majid Zamani*

Main category: cs.LG

TL;DR: This paper introduces a formal robustness certification framework for data poisoning attacks using barrier certificates from control theory, providing provable safety guarantees for both training-time and test-time attacks without restrictive assumptions.


<details>
  <summary>Details</summary>
Motivation: Machine learning in safety-critical domains faces significant risks from adversarial data poisoning attacks. Existing defenses lack formal guarantees or rely on restrictive assumptions about model class, attack type, poisoning extent, or point-wise certification, limiting practical reliability.

Method: Models gradient-based training as a discrete-time dynamical system and formulates poisoning robustness as formal safety verification. Adapts barrier certificates from control theory, parameterizes them as neural networks trained on poisoned trajectories, and derives PAC bounds via scenario convex programming to provide confidence lower bounds on certified robustness radius.

Result: Experiments on MNIST, SVHN, and CIFAR-10 demonstrate that the approach certifies non-trivial perturbation budgets while being model-agnostic and requiring no prior knowledge of attack or contamination level. The framework provides the first unified certification for both training-time and test-time attacks.

Conclusion: The proposed framework offers principled formal robustness certification against data poisoning attacks with provable guarantees, overcoming limitations of existing defenses by being model-agnostic and not requiring restrictive assumptions about attacks or contamination levels.
...
### [130] [From GNNs to Symbolic Surrogates via Kolmogorov-Arnold Networks for Delay Prediction](https://arxiv.org/abs/2512.20885)
*Sami Marouani,Kamal Singh,Baptiste Jeudy,Amaury Habrard*

Main category: cs.LG

TL;DR: The paper proposes FlowKANet, a graph neural network using Kolmogorov-Arnold Networks for flow delay prediction, with attention-based message passing and symbolic distillation for lightweight deployment.


<details>
  <summary>Details</summary>
Motivation: Accurate flow delay prediction is essential for optimizing modern communication networks, requiring efficient and transparent models that can handle graph-structured data while being deployable in resource-constrained environments.

Method: Three-level approach: 1) Heterogeneous GNN with attention-based message passing as baseline, 2) FlowKANet replacing MLP layers with KAN layers using KAMP-Attn (Kolmogorov-Arnold Message Passing with Attention), 3) Symbolic distillation via block-wise regression to produce closed-form equations without trainable weights.

Result: KAN layers provide favorable trade-off between efficiency and accuracy with reduced trainable parameters, while symbolic surrogates enable lightweight deployment and enhanced transparency while preserving graph-structured dependencies.

Conclusion: FlowKANet demonstrates that KAN-based architectures offer efficient alternatives to traditional MLPs for graph-based tasks, and symbolic distillation enables practical deployment of complex neural models through transparent, weight-free equations.
...
### [131] [Time-Efficient Evaluation and Enhancement of Adversarial Robustness in Deep Neural Networks](https://arxiv.org/abs/2512.20893)
*Runqi Lin*

Main category: cs.LG

TL;DR: This thesis develops time-efficient methods for evaluating and improving adversarial robustness in deep neural networks, addressing computational limitations of existing red-blue team approaches.


<details>
  <summary>Details</summary>
Motivation: As DNNs become more embedded in society, ensuring their safety is critical. Existing red-blue adversarial frameworks for vulnerability identification and mitigation are computationally intensive, limiting applicability to large-scale models.

Method: The thesis proposes time-efficient methods for both red team (vulnerability identification) and blue team (vulnerability mitigation) approaches within the adversarial robustness framework.

Result: The abstract doesn't specify concrete results, but indicates the thesis provides computational efficiency improvements over existing adversarial robustness methods.

Conclusion: The thesis aims to overcome computational limitations in adversarial robustness evaluation and enhancement, making these safety-critical approaches more applicable to large-scale DNN models.
...
### [132] [DiEC: Diffusion Embedded Clustering](https://arxiv.org/abs/2512.20905)
*Haidong Hu*

Main category: cs.LG

TL;DR: DiEC performs unsupervised clustering by extracting bottleneck features from pretrained diffusion U-Nets at optimal timesteps, using a two-stage search for clustering-friendly representations.


<details>
  <summary>Details</summary>
Motivation: Current deep clustering methods use single encoders with fixed embeddings, ignoring that clusterability varies across diffusion model hierarchies and noise timesteps. The representation trajectory in pretrained diffusion models contains varying degrees of clusterability that should be exploited.

Method: Two-stage approach: 1) Fix U-Net bottleneck as Clustering-friendly Middle Layer (CML), 2) Use Optimal Timestep Search (OTS) to find clustering-optimal timestep t*. Extract bottleneck features at t* and apply lightweight residual mapping. Optimize DEC-style KL self-training with adaptive graph and entropy regularization. Add denoising-consistency branch at random timesteps for stability.

Result: DiEC achieves competitive clustering performance on multiple standard benchmarks, demonstrating effectiveness of exploiting diffusion model representation trajectories.

Conclusion: Diffusion models' internal activations across layers and timesteps contain valuable clustering information. DiEC successfully identifies and leverages optimal representations from pretrained diffusion U-Nets for unsupervised clustering.
...
### [133] [Towards a General Framework for Predicting and Explaining the Hardness of Graph-based Combinatorial Optimization Problems using Machine Learning and Association Rule Mining](https://arxiv.org/abs/2512.20915)
...
<details>
  <summary>Details</summary>
Motivation: There's a need to predict and understand computational hardness of combinatorial optimization problems on graphs to guide algorithm selection and improve efficiency.

Method: Two-stage framework: 1) Train ML classifiers to map problem-agnostic graph features to hardness categories, 2) Use association rule mining (FP-Growth) to explain predictions, plus regression models for computation time prediction.

Result: Excellent performance on 3287 maximum clique instances: weighted F1 score 0.9921, minority-class F1 0.878, ROC-AUC 0.9083 with only 3 features. Best association rule had 0.8829 support for hard instances with 87.64% accuracy. Regression achieved 5.12% RMSE and R² 0.991.

Conclusion: GCO-HPIF effectively predicts and explains computational hardness of combinatorial optimization problems, demonstrating practical utility for both prediction and explanation with high accuracy.
...
### [134] [RevFFN: Memory-Efficient Full-Parameter Fine-Tuning of Mixture-of-Experts LLMs with Reversible Blocks](https://arxiv.org/abs/2512.20920)
*Ningyuan Liu,Jing Yang,Kaitong Cai,Keze Wang*

Main category: cs.LG

TL;DR: RevFFN enables memory-efficient full parameter fine-tuning of MoE LLMs using reversible Transformer blocks that reconstruct activations during backpropagation, eliminating intermediate activation storage and enabling single-GPU fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Full parameter fine-tuning of large language models requires caching extensive intermediate activations for backpropagation, causing substantial memory overhead that makes fine-tuning contemporary large-scale LLMs challenging in practice. Existing distributed solutions like DeepSpeed require additional hardware resources and reduce training speed.

Method: RevFFN introduces reversible Transformer blocks for mixture of experts (MoE) LLMs that allow reconstruction of layer input activations from outputs during backpropagation, eliminating the need to store most intermediate activations in memory while preserving MoE architecture expressive capacity.

Result: The approach significantly reduces peak memory consumption for full parameter fine-tuning, enabling efficient full fine-tuning on a single consumer-grade or server-grade GPU.

Conclusion: RevFFN provides a memory-efficient fine-tuning paradigm that addresses the memory bottleneck of full parameter fine-tuning for MoE LLMs, making it practical to fine-tune large models on limited hardware resources.
...
### [135] [Guardrailed Elasticity Pricing: A Churn-Aware Forecasting Playbook for Subscription Strategy](https://arxiv.org/abs/2512.20932)
*Deepit Sapru*

Main category: cs.LG

TL;DR: A dynamic pricing framework for subscriptions that combines demand forecasting, price elasticity, and churn prediction to optimize revenue while enforcing business guardrails and protecting customer experience.


<details>
  <summary>Details</summary>
Motivation: Traditional subscription pricing (static tiers, uniform uplifts) fails to capture dynamic market conditions, customer heterogeneity, and the need to balance revenue optimization with customer retention and ethical considerations.

Method: Blends seasonal time-series models with tree-based learners for multivariate demand forecasting, segment-level price elasticity, and churn propensity. Uses Monte Carlo scenario testing to map risk envelopes and solves constrained optimization with business guardrails (customer experience, margin floors, allowable churn).

Result: Consistently outperforms static tiers and uniform uplifts across heterogeneous SaaS portfolios by reallocating price moves toward segments with higher willingness-to-pay while protecting price-sensitive cohorts.

Conclusion: The framework serves as a strategy playbook for shifting from flat to dynamic pricing, aligning pricing with CLV/MRR targets, and embedding ethical guardrails to enable durable growth without eroding customer trust, with real-time recalibration via modular APIs.
...
### [136] [A Multi-fidelity Double-Delta Wing Dataset and Empirical Scaling Laws for GNN-based Aerodynamic Field Surrogate](https://arxiv.org/abs/2512.20941)
*Yiren Shen,Juan J. Alonso*

Main category: cs.LG

TL;DR: Study investigates relationship between training data size and prediction accuracy for GNN-based aerodynamic surrogate models, releases open-source multi-fidelity dataset for double-delta wings, and establishes scaling laws for optimal sampling density.


<details>
  <summary>Details</summary>
Motivation: Limited open-source multi-fidelity datasets and empirical guidelines linking dataset size to model performance for vehicle design acceleration using surrogate models.

Method: Created open-source multi-fidelity aerodynamic dataset (2448 snapshots across 272 geometries) using nested Saltelli sampling, conducted empirical scaling study with MF-VortexNet GNN surrogate across six training datasets (40-1280 snapshots) with varying model sizes (0.1-2.4M parameters).

Result: Test error decreases with data size following power-law exponent of -0.6122, indicating efficient data utilization; optimal sampling density estimated at ~8 samples per dimension; larger models show improved data utilization efficiency.

Conclusion: Established empirical scaling laws for aerodynamic surrogate models, providing guidelines for dataset size requirements and revealing trade-offs between dataset generation costs and model training budgets.
...
### [137] [Solving Functional PDEs with Gaussian Processes and Applications to Functional Renormalization Group Equations](https://arxiv.org/abs/2512.20956)
*Xianjin Yang,Matthieu Darcy,Matthew Hudes,Francis J. Alexander,Gregory Eyink,Houman Owhadi*

Main category: cs.LG

TL;DR: Operator learning framework using Gaussian processes solves functional renormalization group equations directly on function space, outperforming traditional approximations while handling non-constant fields.


<details>
  <summary>Details</summary>
Motivation: To develop a flexible, non-perturbative method for solving functional renormalization group equations that can handle complex field configurations and is independent of specific discretizations.

Method: Uses Gaussian process operator learning to construct functional representations directly on function space, allowing incorporation of physical priors in prior mean or kernel design.

Result: Achieves equal or better performance than existing approximations (like local-potential approximation) on Wetterich and Wilson-Polchinski equations, with ability to handle non-constant fields.

Conclusion: The method is promising for studying complex field configurations like instantons due to its flexibility and ability to handle non-constant fields while maintaining or improving accuracy over traditional approaches.
...
### [138] [ReACT-Drug: Reaction-Template Guided Reinforcement Learning for de novo Drug Design](https://arxiv.org/abs/2512.20958)
*R Yadunandan,Nimisha Ghosh*

Main category: cs.LG

TL;DR: ReACT-Drug is a reinforcement learning framework for de novo drug design that uses protein embeddings to identify similar proteins, decomposes known ligands into fragments, and employs PPO to generate novel, synthetically accessible drug candidates with competitive binding affinities.


<details>
  <summary>Details</summary>
Motivation: De novo drug design faces challenges in navigating vast chemical space to find synthetically accessible, high-affinity candidates. Traditional supervised learning methods lack capabilities for multi-objective optimization and exploration of novel chemical space that reinforcement learning can provide.

Method: ReACT-Drug uses ESM-2 protein embeddings to identify similar proteins from PDB, decomposes known drug ligands into fragments to initialize search space, then employs Proximal Policy Optimization (PPO) with ChemBERTa-encoded molecules guided through reaction-template-based transformations to generate novel compounds.

Result: The framework generates de novo drug candidates with competitive binding affinities and high synthetic accessibility, ensuring 100% chemical validity and novelty as per MOSES benchmarking standards.

Conclusion: The architecture demonstrates the potential of integrating structural biology, deep representation learning, and chemical synthesis rules to automate and accelerate rational drug design through a target-agnostic, generalist approach.
...
### [139] [Can Agentic AI Match the Performance of Human Data Scientists?](https://arxiv.org/abs/2512.20959)
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether agentic AI systems can truly match human data scientists who leverage domain-specific knowledge, especially when important variables are hidden in non-tabular data sources like images.

Method: Designed a prediction task with a synthetic property insurance dataset where a crucial latent variable is hidden in image data rather than tabular features. Compared performance of agentic AI systems that generate generic analytics code against methods that incorporate domain-specific insights.

Result: Agentic AI systems relying on generic analytics workflows performed poorly, while approaches using domain-specific knowledge (like human experts) could identify and leverage the hidden variable in image data.

Conclusion: Current agentic AI for data science has a key limitation in recognizing and incorporating domain knowledge, highlighting the need for future research to develop AI systems that can better integrate domain-specific insights.
...
### [140] [Generalization of Diffusion Models Arises with a Balanced Representation Space](https://arxiv.org/abs/2512.20963)
*Zekai Zhang,Xiao Li,Xiang Li,Lianghe Shi,Meng Wu,Molei Tao,Qing Qu*

Main category: cs.LG

TL;DR: Diffusion models can memorize training data when overfit, but generalization occurs when they learn local data statistics. The paper analyzes this through representation learning, showing memorization creates "spiky" representations while generalization produces "balanced" ones.


<details>
  <summary>Details</summary>
Motivation: Diffusion models risk memorizing training data when overfit to the training objective, which can compromise their ability to generate novel content. The authors want to understand the distinction between memorization and generalization in diffusion models through the lens of representation learning.

Method: The authors analyze a two-layer ReLU denoising autoencoder (DAE) theoretically to prove the differences between memorization and generalization. They then validate these theoretical findings on real-world unconditional and text-to-image diffusion models. Based on these insights, they propose a representation-based method for detecting memorization and a training-free editing technique using representation steering.

Result: Theoretical analysis shows that memorization corresponds to storing raw training samples in learned weights, creating localized "spiky" representations, while generalization arises from capturing local data statistics, producing "balanced" representations. These findings are validated on real-world diffusion models, and practical applications include memorization detection and training-free editing via representation steering.

Conclusion: Learning good representations is central to achieving novel and meaningful generative modeling. The distinction between memorization (spiky representations) and generalization (balanced representations) provides insights for developing better diffusion models and practical tools for controlling their behavior.
...
### [141] [Generalised Linear Models in Deep Bayesian RL with Learnable Basis Functions](https://arxiv.org/abs/2512.20974)
...
<details>
  <summary>Details</summary>
Motivation: Classical Bayesian RL methods assume known transition and reward models, limiting real-world applicability. Recent deep BRL methods using neural networks require optimizing ELBOs, which are difficult to optimize and can lead to indistinctive task parameters and compromised policies.

Method: GLiBRL (Generalised Linear Models in Deep Bayesian RL with Learnable Basis Functions) enables efficient learning of transition and reward models with fully tractable marginal likelihood and Bayesian inference on task parameters and model noises.

Result: On MetaWorld ML10/45 benchmarks, GLiBRL improves success rate of VariBAD by up to 2.7x. It also demonstrates low-variance and decent performance compared to MAML, RL2, SDVT, TrMRL and ECET.

Conclusion: GLiBRL provides a novel approach to deep Bayesian RL that overcomes ELBO optimization challenges and achieves superior performance on challenging meta-reinforcement learning benchmarks.
...
### [142] [CoSeNet: A Novel Approach for Optimal Segmentation of Correlation Matrices](https://arxiv.org/abs/2512.21000)
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of identifying correlated segments in noisy correlation matrices more effectively than previous approaches, providing a robust and generalizable solution for various applications.

Method: Four-layer algorithmic architecture (input, formatting, re-scaling, segmentation) using overlapping techniques and pre-trained ML algorithms. Includes heuristic optimization of re-scaling parameters based on Window Difference-based metric fitness.

Result: Produces binary noise-free matrix representing optimal segmentation with segmentation points. Achieves better performance than previous approaches for similar problems.

Conclusion: CoSeNet provides an effective, robust, and generalizable approach for correlation segmentation that balances efficiency, memory, and speed for deployment in various applications.
...
### [143] [LLM Swiss Round: Aggregating Multi-Benchmark Performance via Competitive Swiss-System Dynamics](https://arxiv.org/abs/2512.21010)
*Jiashuo Liu,Jiayun Wu,Chunjie Wu,Jingkai Liu,Zaiyuan Wang,Huan Zhou,Wenhao Huang,Hongseok Namkoong*

Main category: cs.LG

TL;DR: CSD framework introduces competitive Swiss-system dynamics for LLM evaluation, using sequential contests and Monte Carlo simulations to generate robust rankings and risk profiles.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation methods are fragmented and static, lacking proper benchmark weighting and failing to capture dynamic competitive fitness or vulnerability in sequential, high-stakes tasks.

Method: Competitive Swiss-System Dynamics (CSD) framework simulates multi-round sequential contests where models are dynamically paired based on win-loss records across curated benchmarks. Uses Monte Carlo Simulation (100,000 iterations) to compute Expected Win Score and implements Failure Sensitivity Analysis with parameterized elimination to profile risk appetite.
...
Conclusion: CSD represents a vital step toward risk-informed, next-generation LLM evaluation by addressing limitations of current static scoring methods and capturing dynamic competitive fitness.
...
TL;DR: The paper develops Neural Feature Dynamics (NFD) to understand feature learning in deep ResNets, explains why depth-muP fails for multi-layer blocks, and proposes a depth-aware learning rate correction to restore hyperparameter transfer in deeper networks.


<details>
  <summary>Details</summary>
Motivation: Scaling laws predict gains from larger models but don't explain when scaling succeeds or fails. Current theory (muP) works for width but breaks down for depth, especially for residual blocks with multiple internal layers. Need rigorous understanding of feature learning at large depth.

Method: Derive Neural Feature Dynamics (NFD) for ResNets with single-layer residual blocks in joint infinite-width and infinite-depth limit. Analyze feature learning via coupled forward-backward stochastic system. Study two-layer residual blocks to understand collapse mechanism. Propose depth-aware learning-rate correction.

Result: NFD identifies when scaling-law trends persist and explains diminishing returns. Reveals vanishing mechanism where gradient-independence assumption becomes valid at infinite depth. Shows feature-learning collapse in first internal layer of two-layer blocks at large depth, explaining depth-muP failure. Proposed correction restores depth-wise hyperparameter transfer and improves deeper ResNet performance.

Conclusion: NFD provides theoretical framework for understanding feature learning at large depth, explains empirical failures of depth scaling, and offers practical solution (depth-aware learning rate correction) to improve training of deeper residual networks.
...
### [145] [Shared Representation Learning for High-Dimensional Multi-Task Forecasting under Resource Contention in Cloud-Native Backends](https://arxiv.org/abs/2512.21102)
*Zixiao Huang,Jixiao Yang,Sijia Li,Chi Zhang,Jinyu Chen,Chengda Xu*

Main category: cs.LG

TL;DR: Unified forecasting framework for high-dimensional multi-task time series in cloud native systems, featuring shared encoding, state fusion, cross-task structural propagation, and dynamic adjustment mechanisms.


<details>
  <summary>Details</summary>
Motivation: Need to predict cloud native backend systems operating under highly dynamic loads, coupled metrics, and parallel tasks, requiring handling of resource contention, link interactions, and service topology changes.

Method: Builds shared encoding structure for unified representation of monitoring indicators, employs state fusion mechanism for multi-scale trend changes, introduces cross-task structural propagation module for dependency modeling, and incorporates dynamic adjustment mechanism for non-stationary behaviors.

Result: Superior performance on several error metrics, more accurate representations of future states under different operating conditions, verified through hyperparameter sensitivity, environmental sensitivity, and data sensitivity analyses.

Conclusion: The unified forecasting framework offers reliable predictive capability for high-dimensional, multi-task, strongly dynamic cloud native environments and provides essential technical support for intelligent backend management.
...
### [146] [A Mechanistic Analysis of Transformers for Dynamical Systems](https://arxiv.org/abs/2512.21113)
*Gregory Duthé,Nikolaos Evangelou,Wei Liu,Ioannis G. Kevrekidis,Eleni Chatzi*

Main category: cs.LG

TL;DR: Transformers for time-series forecasting lack theoretical understanding from dynamical systems perspective. This paper analyzes single-layer Transformers' representational capabilities, showing softmax attention imposes convexity constraints limiting linear dynamics representation, while enabling adaptive delay-embedding for nonlinear systems.


<details>
  <summary>Details</summary>
Motivation: Transformers are widely used for time-series modeling but treated as black boxes without theoretical foundations from dynamical systems perspective. This gap is critical as attention-based models are considered for general-purpose forecasting across diverse dynamical regimes.

Method: Analyzes single-layer Transformers from dynamical systems perspective, interpreting causal self-attention as linear, history-dependent recurrence. Investigates representational capabilities through linear and nonlinear case studies, examining how attention processes temporal information.

Result: For linear systems: softmax attention's convexity constraint fundamentally restricts representable dynamics, causing oversmoothing in oscillatory settings. For nonlinear systems under partial observability: attention acts as adaptive delay-embedding mechanism enabling state reconstruction with sufficient temporal context and latent dimensionality.
...
### [147] [STLDM: Spatio-Temporal Latent Diffusion Model for Precipitation Nowcasting](https://arxiv.org/abs/2512.21118)
*Shi Quan Foo,Chi-Ho Wong,Zhihan Gao,Dit-Yan Yeung,Ka-Hing Wong,Wai-Kin Wong*

Main category: cs.LG

TL;DR: STLDM is a diffusion-based model for precipitation nowcasting that combines deterministic forecasting with generative enhancement to overcome blurry predictions from deterministic models and poor accuracy from generative models.


<details>
  <summary>Details</summary>
Motivation: Precipitation nowcasting is critical for preventing weather-related damage, but existing approaches struggle with the complex, stochastic nature of the task. Deterministic models produce blurry predictions while generative models often have poor accuracy.

Method: STLDM is a diffusion-based model that learns latent representations end-to-end using both Variational Autoencoder and conditioning network. It decomposes the task into two stages: deterministic forecasting (handled by conditioning network) and enhancement (performed by latent diffusion model).
...
Conclusion: STLDM presents an effective diffusion-based architecture for precipitation nowcasting that addresses limitations of both deterministic and generative approaches, achieving better performance and efficiency.
...
### [148] [MODE: Multi-Objective Adaptive Coreset Selection](https://arxiv.org/abs/2512.21152)
*Tanmoy Mukherjee,Pierre Marquis,Zied Bouraoui*

Main category: cs.LG

TL;DR: MODE is a dynamic coreset selection framework that adaptively combines different selection strategies based on training phase, achieving competitive accuracy with reduced memory requirements.


<details>
  <summary>Details</summary>
Motivation: Static coreset selection methods use fixed criteria throughout training, but data utility evolves across different training phases. There's a need for adaptive selection that changes emphasis based on training progress.

Method: MODE dynamically combines multiple coreset selection strategies: emphasizing class balance early in training, diversity during representation learning phase, and uncertainty near convergence. It adapts selection criteria to training phases.

Result: Achieves (1-1/e)-approximation with O(n log n) complexity, demonstrates competitive accuracy while reducing memory requirements, and provides interpretable insights into data utility evolution.

Conclusion: MODE provides an effective adaptive framework for coreset selection that outperforms static methods by dynamically adjusting to training phases, offering both performance benefits and interpretability.
...
### [149] [BALLAST: Bandit-Assisted Learning for Latency-Aware Stable Timeouts in Raft](https://arxiv.org/abs/2512.21165)
*Qizhi Wang*

Main category: cs.LG

TL;DR: BALLAST replaces static Raft election timeouts with contextual bandits to improve liveness under network variability, reducing recovery time in challenging WAN conditions.


<details>
  <summary>Details</summary>
Motivation: Randomized election timeouts in Raft become brittle under long-tail latency, jitter, and partition recovery, where repeated split votes can cause extended unavailability.

Method: BALLAST uses lightweight online adaptation with contextual bandits (LinUCB variants) to select from discrete timeout options, augmented with safe exploration to cap risk during unstable periods.

Result: In simulations with long-tail delay, loss, correlated bursts, node heterogeneity, and partition/recovery turbulence, BALLAST substantially reduces recovery time and unwritable time compared to standard approaches.

Conclusion: BALLAST effectively addresses Raft's liveness issues under network variability while remaining competitive in stable settings, offering a practical improvement over static timeout heuristics.
...
### [150] [A Unified Framework for EEG Seizure Detection Using Universum-Integrated Generalized Eigenvalues Proximal Support Vector Machine](https://arxiv.org/abs/2512.21170)
*Yogesh Kumar,Vrushank Ahire,M. A. Ganaie*

Main category: cs.LG

TL;DR: Novel Universum-enhanced classifiers (U-GEPSVM and IU-GEPSVM) for EEG signal classification that combine generalized eigenvalue decomposition efficiency with Universum learning benefits to address EEG analysis challenges.


<details>
  <summary>Details</summary>
Motivation: Address critical challenges in EEG analysis: non-stationarity, low signal-to-noise ratio, and limited labeled data by leveraging computational efficiency and generalization benefits.

Method: U-GEPSVM extends GEPSVM with Universum constraints via ratio-based objective function; IU-GEPSVM enhances stability through weighted difference-based formulation for independent control over class separation and Universum alignment.
...
Conclusion: The proposed Universum-enhanced classifiers effectively address EEG classification challenges and demonstrate superior performance compared to existing methods, with IU-GEPSVM showing particular promise for practical EEG analysis applications.
...
### [151] [Analytic and Variational Stability of Deep Learning Systems](https://arxiv.org/abs/2512.21208)
*Ronald Katende*

Main category: cs.LG

TL;DR: A unified analytic-variational framework for studying stability in deep learning systems via Learning Stability Profiles that track perturbation responses along learning trajectories, with equivalence to Lyapunov-type energy dissipation.


<details>
  <summary>Details</summary>
Motivation: To develop a unified theoretical framework for analyzing stability across diverse deep learning architectures and optimization methods, addressing how architectural and algorithmic choices jointly affect robustness to perturbations during training.

Method: Introduces Learning Stability Profiles that track infinitesimal responses of representations, parameters, and update mechanisms to perturbations. Uses analytic stability theorems linking bounded stability signatures to Lyapunov-type energy dissipation. Extends to non-smooth systems using Clarke generalized derivatives and variational Lyapunov functionals.

Result: Proves Fundamental Analytic Stability Theorem showing equivalence between uniform boundedness of stability signatures and existence of dissipative Lyapunov-type energy. Derives explicit stability exponents linking spectral norms, activation regularity, step sizes, and learning rates to contractivity. Recovers classical stability results as special cases.

Conclusion: Provides a unified dynamical framework for stability analysis across architectures and optimization methods, clarifying joint effects of architectural and algorithmic choices on robustness. Offers foundation for extensions to continuous-time limits and geometric formulations of learning dynamics.
...
### [152] [MiST: Understanding the Role of Mid-Stage Scientific Training in Developing Chemical Reasoning Models](https://arxiv.org/abs/2512.21231)
...
TL;DR: Large language models need 'latent solvability' for RL to work in chemical reasoning. The paper proposes MiST (mid-stage scientific training) to build symbolic competence and latent chemical knowledge, enabling RL to dramatically improve performance on chemical tasks.


<details>
  <summary>Details</summary>
Motivation: Current reinforcement learning for chemical reasoning only works when models already have some probability of correct answers ('latent solvability'). The paper investigates what prerequisites are needed for chemical reasoning and how to build them.

Method: Proposes MiST (mid-stage scientific training): data-mixing with SMILES/CIF-aware pre-processing, continued pre-training on 2.9B tokens, and supervised fine-tuning on 1B tokens. This builds symbolic competence and latent chemical knowledge before RL.
...
Conclusion: Defines clear prerequisites for chemical reasoning training (symbolic competence and latent chemical knowledge) and highlights the importance of mid-stage training for unlocking reasoning capabilities in LLMs for scientific domains.
...
### [153] [Improving the Convergence Rate of Ray Search Optimization for Query-Efficient Hard-Label Attacks](https://arxiv.org/abs/2512.21241)
...
<details>
  <summary>Details</summary>
Motivation: Hard-label black-box adversarial attacks face prohibitive query complexity as they only have access to top-1 predicted labels, making practical deployment challenging. Existing methods need optimization to reduce query costs.

Method: ARS-OPT uses momentum-based optimization inspired by Nesterov's Accelerated Gradient to proactively estimate gradients with respect to future ray directions. PARS-OPT extends this by incorporating surrogate-model priors for enhanced gradient estimation.
...
Conclusion: The proposed momentum-based approach with Nesterov acceleration principles significantly improves query efficiency in hard-label black-box adversarial attacks, making such attacks more practical for real-world deployment.
...
### [154] [Model Merging via Multi-Teacher Knowledge Distillation](https://arxiv.org/abs/2512.21288)
*Seyed Arshan Dalili,Mehrdad Mahdavi*

Main category: cs.LG

TL;DR: SAMerging introduces a flatness-aware PAC-Bayes bound for model merging, frames merging as multi-teacher distillation, and uses Sharpness-Aware Minimization to find flat minima, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Current model merging methods lack theoretical guarantees and rely on heuristics for coefficient scaling, leading to brittle performance sensitive to initialization. There's a need for principled understanding of generalization in model merging across heterogeneous data distributions.

Method: 1) Derives flatness-aware PAC-Bayes generalization bound with "cross-task heterogeneity" term; 2) Frames model merging as multi-teacher knowledge distillation on unlabeled data; 3) Implements SAMerging using Sharpness-Aware Minimization to find flat minima.

Result: SAMerging establishes new state-of-the-art performance across vision and NLP benchmarks, demonstrating remarkable empirical results.

Conclusion: The paper provides theoretical foundation for model merging, introduces a principled objective through flatness-aware bounds, and shows SAMerging effectively addresses brittleness in existing methods while achieving superior performance.
...
### [155] [Transcriptome-Conditioned Personalized De Novo Drug Generation for AML Using Metaheuristic Assembly and Target-Driven Filtering](https://arxiv.org/abs/2512.21301)
...
TL;DR: Novel computational framework integrates transcriptomics, AI protein modeling, and evolutionary algorithms to generate personalized drug candidates for AML patients.


<details>
  <summary>Details</summary>
Motivation: AML remains challenging due to molecular heterogeneity and high relapse rates, with many patients lacking effective personalized treatment options despite advances in precision medicine.

Method: 1) Analyzed TCGA-LAML RNA-seq data using WGCNA to identify 20 high-value biomarkers; 2) Modeled protein structures with AlphaFold3; 3) Identified druggable hotspots with DOGSiteScorer; 4) Developed reaction-first evolutionary metaheuristic algorithm for de novo ligand assembly from fragment libraries; 5) Used multi-objective optimization for spatial alignment to hotspots; 6) Validated candidates through ADMET profiling and SwissDock molecular docking.

Result: Generated structurally unique chemical entities with drug-like properties (QED scores 0.5-0.7). Identified high-confidence candidates like Ligand L1 with binding free energy of -6.571 kcal/mol against A08A96 biomarker. Framework successfully produced pharmacologically viable, patient-tailored leads.

Conclusion: Integration of systems biology with metaheuristic molecular assembly can produce scalable, personalized drug candidates for AML, offering a blueprint for precision oncology beyond leukemia.
...
### [156] [Learning to Solve PDEs on Neural Shape Representations](https://arxiv.org/abs/2512.21311)
*Lilian Welschinger,Yilin Liu,Zican Wang,Niloy Mitra*

Main category: cs.LG

TL;DR: A mesh-free method for solving surface PDEs directly on neural shape representations without explicit mesh extraction or per-instance training.


<details>
  <summary>Details</summary>
Motivation: Modern 3D assets increasingly use neural representations, but existing PDE solvers require polygonal meshes, creating a mismatch that prevents end-to-end workflows in shape analysis and engineering.

Method: Learns a local update operator conditioned on neural shape attributes, enabling surface PDE solving directly on neural representations. The operator is trained once on a single representative shape and generalizes across shape and topology variations.

Result: Slightly outperforms CPM while remaining reasonably close to FEM accuracy. Provides the first end-to-end pipeline for solving surface PDEs on both neural and classical surface representations without explicit meshing or per-instance optimization.

Conclusion: The method enables accurate, fast inference of surface PDEs directly within neural domains, preserving differentiability and enabling end-to-end workflows for modern 3D assets.
...
### [157] [Does the Data Processing Inequality Reflect Practice? On the Utility of Low-Level Tasks](https://arxiv.org/abs/2512.21315)
*Roy Turgeman,Tom Tirer*

Main category: cs.LG

TL;DR: The paper challenges the data processing inequality by showing that pre-classification processing can improve classification accuracy for finite training samples, despite the principle suggesting no benefit.


<details>
  <summary>Details</summary>
Motivation: The data processing inequality suggests no benefit in signal enhancement before classification, but in practice, low-level processing is commonly performed before high-level tasks. The paper aims to understand when and why such pre-processing can be beneficial for classification despite modern deep networks' capabilities.

Method: Theoretical study of binary classification with a classifier converging to optimal Bayes classifier. Proves existence of pre-classification processing that improves accuracy for finite training samples. Explores effects of class separation, training set size, and class balance. Empirical investigation of theoretical setup and practical study with denoising/encoding on benchmark datasets.

Result: Proved that for any finite number of training samples, there exists pre-classification processing that improves classification accuracy. Empirical studies show consistent trends with theoretical results across varying training set sizes, class distributions, and noise levels.

Conclusion: While data processing inequality holds for optimal Bayes classifier, pre-classification processing can benefit practical classifiers with finite training data. The benefit depends on factors like class separation, training set size, and class balance.
...
### [158] [Measuring all the noises of LLM Evals](https://arxiv.org/abs/2512.21326)
*Sida Wang*

Main category: cs.LG

TL;DR: The paper proposes an "all-pairs paired method" to measure and analyze three types of noise in LLM evaluations: prediction noise, data noise, and total noise, enabling better statistical power and significance assessment.


<details>
  <summary>Details</summary>
Motivation: LLM evaluations have unique noise characteristics that require specialized statistical methods to effectively separate signal from noise and gain statistical power for meaningful comparisons.

Method: The all-pairs paired method applies paired analysis to all pairs of LLMs, measuring three noise components (prediction noise, data noise, total noise) based on millions of question-level predictions across various evaluations and settings.
...
Conclusion: The proposed method enables practitioners to assess significance without custom testing and detect much smaller effects in controlled experiments by better understanding and managing LLM evaluation noise.
...
### [159] [ASCHOPLEX encounters Dafne: a federated continuous learning project for the generalizability of the Choroid Plexus automatic segmentation](https://arxiv.org/abs/2512.20741)
...
TL;DR: Federated incremental learning approach improves choroid plexus segmentation across diverse MRI datasets compared to conventional fine-tuning.


<details>
  <summary>Details</summary>
Motivation: The Choroid Plexus (ChP) is important for brain physiology, but existing segmentation tool ASCHOPLEX suffers from poor generalizability across different MRI datasets due to inter-dataset variability.

Method: Integrated enhanced ASCHOPLEX with Dafne (Deep Anatomical Federated Network) framework for federated incremental learning, comparing it against conventional fine-tuning on 2,284 subjects from five independent MRI datasets including Multiple Sclerosis patients and healthy controls.

Result: Conventional fine-tuning works well on homogeneous data but has limited generalizability across diverse datasets. Federated incremental learning consistently achieves higher generalizability and more stable performance across different acquisition settings.

Conclusion: Federated incremental learning provides a robust alternative to conventional fine-tuning for ChP segmentation, enabling better performance across heterogeneous imaging conditions and multiple cohorts.
...
### [160] [Leveraging Overfitting for Low-Complexity and Modality-Agnostic Joint Source-Channel Coding](https://arxiv.org/abs/2512.20981)
*Haotian Wu,Gen Li,Pier Luigi Dragotti,Deniz Gündüz*

Main category: eess.IV

TL;DR: Implicit-JSCC is an overfitted joint source-channel coding method that optimizes channel symbols and a lightweight neural decoder per source instance, eliminating training datasets and achieving 1000x lower decoding complexity with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for efficient, storage-free, modality-agnostic communication systems that can handle streaming scenarios where one-time offline encoding supports multiple online decoding, while overcoming limitations of traditional JSCC approaches.

Method: Implicit-JSCC uses an instance-specific overfitting approach that directly optimizes channel symbols and a lightweight neural decoder for each source, eliminating the need for training datasets or pre-trained models. It operates as a low-complexity alternative with minimal parameters.

Result: Achieves around 1000x lower decoding complexity compared to alternatives, using only 607 model parameters and 641 multiplications per pixel. Achieves state-of-the-art results in high SNR regimes and inherently addresses source generalizability.

Conclusion: Implicit-JSCC presents a promising paradigm for future communication systems, particularly streaming scenarios, offering storage-free, modality-agnostic solutions with dramatically reduced complexity while maintaining strong performance in high SNR conditions.
...
### [161] [Learning-Enabled Elastic Network Topology for Distributed ISAC Service Provisioning](https://arxiv.org/abs/2512.20722)
*Jie Chen,Xianbin Wang*

Main category: eess.SP

TL;DR: Proposes elastic network topology for distributed ISAC services, balancing signaling overhead and resource utilization through dynamic CCN aggregation into CFNs, with MADRL-based optimization.


<details>
  <summary>Details</summary>
Motivation: Existing mobile networks (CCN/CFN) have rigid topologies that cannot flexibly support distributed ISAC services, making it difficult to align heterogeneous service demands with available radio resources.

Method: Elastic network topology allowing dynamic aggregation of CCNs into CFNs; two-phase protocol for service classification and resource partitioning; USR metric for utility-signaling tradeoff; MADRL framework with centralized training/decentralized execution.

Result: Proposed ENT enables flexible ISAC service provisioning by orchestrating CCN/CFN boundaries; USR formulation quantifies utility-overhead tradeoff; MADRL framework addresses distributed optimization without complete CSI.

Conclusion: Elastic network topology with dynamic CCN-CFN aggregation and MADRL-based optimization provides efficient distributed ISAC service provisioning by balancing signaling overhead and resource utilization.
...
### [162] [Balancing Decentralized Trust and Physical Evidence: A Blockchain-Physical Layer Co-Design for Real-Time 3D Prioritization in Disaster Zones](https://arxiv.org/abs/2512.20824)
*Mohammad Taghi Dabiri,Mazen Hasna,Aiman Erbad,Rula Ammuri,Khalid Qaraqe*

Main category: eess.SP

TL;DR: A blockchain-based crisis coordination system that uses UAV verifiers for physical attestation to create trusted 3D crisis maps during disasters.


<details>
  <summary>Details</summary>
Motivation: Current disaster response systems struggle with unreliable data, misinformation, and communication breakdowns, creating a need for trusted real-time coordination that can save lives.

Method: Dual-layer architecture combining blockchain for digital voting (timestamped, geotagged claims) with mobile UAV verifiers for physical-layer attestation using modulated retro-reflector sensors and 3D-aware line-of-sight placement.

Result: Proposed system fuses immutable digital records with sensory-grounded trust, addresses challenges like spoofing and connectivity loss, and offers layered mitigation strategies for resilient crisis coordination.

Conclusion: The blockchain-UAV integration provides a path toward trust-anchored crisis coordination that overcomes limitations of current systems by combining cryptographic trust with physical verification.
...
### [163] [The Area Signal-to-Noise Ratio: A Robust Alternative to Peak-Based SNR in Spectroscopic Analysis](https://arxiv.org/abs/2512.20830)
*Alex Yu,Huaqing Zhao,Lin Z. Li*

Main category: eess.SP

TL;DR: Researchers propose area-based SNR (aSNR) as a better alternative to peak-based SNR (pSNR) for spectroscopic analysis, showing it requires lower signal amplitudes for detection and works better for broad peaks.


<details>
  <summary>Details</summary>
Motivation: Traditional peak-based SNR (pSNR) has limitations including sensitivity to noise spikes and reduced effectiveness for broader peaks, creating a need for a more robust SNR metric.

Method: Introduced area-based SNR (aSNR) that integrates signal over a defined region of interest. Used Monte Carlo simulations (2,000 trials per condition) to test aSNR on Gaussian, Lorentzian, and Voigt lineshapes.

Result: aSNR requires significantly lower amplitudes than pSNR to achieve 50% detection probability. ROC curves show aSNR performs better than pSNR at low amplitudes, especially advantageous for broad peaks.

Conclusion: aSNR is a robust alternative to pSNR that reduces noise variance and improves detection for various lineshapes, with potential extension to volume-based SNR for multidimensional spectra.
...
### [164] [Towards City-Scale Quantum Timing: Wireless Synchronization via Quantum Hubs](https://arxiv.org/abs/2512.20827)
*Mohammad Taghi Dabiri,Mazen Hasna,Rula Ammuri,Saif Al-Kuwari,Khalid Qaraqe*

Main category: eess.SP

TL;DR: Novel wireless quantum synchronization framework using entangled photons and passive CCR arrays for city-scale secure timing with sub-nanosecond accuracy.


<details>
  <summary>Details</summary>
Motivation: Need for secure, low-power, infrastructure-free synchronization solutions for next-generation smart cities that can provide sub-nanosecond timing accuracy without active circuitry at target devices.

Method: Centralized quantum hub emits entangled photon pairs, sending one to target device with passive CCR array and one to local reference. Develops analytical model considering Gaussian beam spread, spatial misalignment, atmospheric turbulence, Gamma-Gamma fading, background photons, SPAD detector jitter, and quantum randomness.

Result: Derived closed-form expression for single-photon detection probability under Gamma-Gamma fading, modeled photon arrival events and synchronization error. Simulation results confirm analytical model accuracy and reveal trade-offs among beam waist, CCR array size, and background light.

Conclusion: Proposed architecture offers secure, low-power, infrastructure-free quantum synchronization solution for smart cities with sub-nanosecond accuracy using passive CCR arrays and entangled photons.
...
### [165] [QoS- and Physics-Aware Routing in Optical LEO Satellite Networks via Deep Reinforcement Learning](https://arxiv.org/abs/2512.20835)
*Mohammad Taghi Dabiri,Rula Ammuri,Mazen Hasna,Khalid Qaraqe*

Main category: eess.SP

TL;DR: A DRL-based routing framework for optical LEO satellite networks that enforces physical feasibility constraints derived from jitter-aware beam models, ensuring stable and QoS-aware routing without global recomputation.


<details>
  <summary>Details</summary>
Motivation: Optical ISLs in LEO constellations face unique challenges due to extreme sensitivity to satellite motion, pointing jitter, and rapidly changing geometry, making routing fundamentally more difficult than in RF systems. Intra-plane and inter-plane links have different stability and range profiles, creating dynamic, partially constrained connectivity that must be respected for physically consistent routing.

Method: A lightweight geometry- and QoS-aware routing framework that incorporates class-dependent feasibility constraints from a jitter-aware Gaussian-beam model. These analytically computed thresholds are embedded into the time-varying ISL graph and enforced via feasible-action masking in a DRL agent. The method leverages local geometric progress, feasible-neighbor structure, and congestion indicators to select next-hop relays without global recomputation.

Result: Simulation results on a Starlink-like constellation show that learned paths are physically consistent, exploit intra-plane stability, adapt to jitter-limited inter-plane connectivity, and maintain robust end-to-end latency under dynamic topology evolution.

Conclusion: The proposed DRL-based routing framework successfully addresses the unique challenges of optical LEO networks by incorporating physical feasibility constraints directly into the routing decision process, enabling stable, adaptive, and QoS-aware routing without requiring global recomputation.
...
### [166] [When Future Communications Shift Toward Narrow Beams: A Forward Looking Survey on Pointing Errors and Alignment Limits](https://arxiv.org/abs/2512.20874)
...
TL;DR: This survey paper provides a unified framework for analyzing pointing errors across optical and high-frequency wireless communications, addressing the fragmentation in existing literature and offering consistent terminology, models, and mitigation approaches.


<details>
  <summary>Details</summary>
Motivation: Directional links in FSO, mmWave, and THz systems are crucial for 6G networks but suffer from reliability issues due to pointing errors. Existing studies use technology-specific approaches that hinder cross-domain comparison and transferable design insights.

Method: The survey establishes consistent terminology and cross-technology taxonomy of pointing errors, reviews angular misalignment and statistical distribution models, analyzes system performance impact, and systematically surveys mitigation techniques with emphasis on optical systems.

Result: Provides a unified treatment of pointing errors across optical and high-frequency wireless communications, including detailed examination of pointing-error effects in OAM links and quantum optical communications, with corresponding mitigation approaches.

Conclusion: By consolidating fragmented literature into a coherent framework, this work supports consistent analysis and robust design of next-generation directional communication systems, while outlining open challenges and future research directions.
...
### [167] [Semantic Radio Access Networks: Architecture, State-of-the-Art, and Future Directions](https://arxiv.org/abs/2512.20917)
...
TL;DR: This paper provides a comprehensive review of Semantic RAN (SemRAN), exploring the integration of semantic communication into Radio Access Networks to overcome limitations of conventional bit-level transmission and enable more efficient, intelligent wireless systems.


<details>
  <summary>Details</summary>
Motivation: There is a lack of comprehensive reviews on integrating Semantic Communication (SemCom) into Radio Access Networks (RAN), despite SemCom's potential to transform communication by prioritizing meaning-level transmission over conventional bit-level delivery, offering improved spectrum efficiency, anti-interference capabilities, flexible resource allocation, and enhanced user experience.

Method: The paper systematically explores recent advancements in Semantic RAN by: 1) introducing fundamentals of RAN and SemCom, 2) identifying limitations of conventional RAN, 3) outlining the overall architecture of SemRAN, 4) reviewing representative techniques across physical layer, data link layer, network layer, and security plane, 5) envisioning future services and applications, and 6) examining current standardization progress.

Result: The paper provides a comprehensive framework for understanding Semantic RAN, including its architecture, layered implementation across network protocols, security considerations, and potential applications, while also documenting current standardization efforts in this emerging field.

Conclusion: The review identifies critical research challenges and outlines forward-looking directions to guide subsequent investigations in Semantic RAN, recognizing it as a burgeoning field that could fundamentally transform wireless communication systems through semantic-aware transmission.
...
### [168] [BenchLink: An SoC-Based Benchmark for Resilient Communication Links in GPS-Denied Environments](https://arxiv.org/abs/2512.20942)
...
TL;DR: BenchLink: SoC-based benchmark for resilient wireless communication without GPS, featuring adaptive pilot density/modulation and precise latency control for GPS-denied environments.


<details>
  <summary>Details</summary>
Motivation: Many emerging wireless applications must operate in GPS-denied environments where GPS signals are unreliable or disrupted, causing oscillator drift and carrier frequency impairments. Traditional GPP-based SDRs lack precise latency control needed for resilient synchronization.

Method: Developed BenchLink, a System-on-Chip (SoC)-based benchmark for resilient communication links that functions without GPS and supports adaptive pilot density and modulation. Implemented on Zynq UltraScale+ MPSoCs for precise latency control, unlike traditional GPP-based SDRs like USRPs.

Result: Successfully implemented and evaluated BenchLink on Zynq UltraScale+ MPSoCs, demonstrating effectiveness in both ground and aerial environments. Collected comprehensive dataset under various conditions. Both SoC-based link design and dataset will be made available to the wireless community.

Conclusion: BenchLink facilitates future research on data-driven link adaptation, resilient synchronization in GPS-denied scenarios, and emerging applications requiring precise latency control (e.g., integrated radar sensing and communication). The SoC-based approach enables GPS-independent operation with superior timing precision.
...
### [169] [Reconfigurable Intelligent Surface-Enhanced Satellite Networks: Deployment Strategies, Key Capabilities, Practical Solutions, and Future Directions](https://arxiv.org/abs/2512.20982)
*Ziyuan Zheng,Xiangyu Li,Shirui Zuo,Yu Wan,Bodong Shang,Wenpeng Jing,Qingqing Wu*

Main category: eess.SP

TL;DR: RIS-enabled satellite networks overcome blockage and spectrum coexistence challenges through connectivity restoration and angular selectivity mechanisms, with practical designs for high-mobility operation and future opportunities in AI and multifunctional architectures.


<details>
  <summary>Details</summary>
Motivation: Satellite networks face persistent barriers: blockage-induced service discontinuities and stringent spectrum coexistence across satellite layers and with terrestrial systems. RIS offers low-power programmable apertures to address these challenges.

Method: Develop a deployment-first, operations-aware view of RIS-enabled satellite networking treating RIS as both satellite/terminal antennas and inter-satellite/space-ground relays. Focus on two unifying mechanisms: connectivity restoration via virtual line-of-sight links and angular selectivity for interference reshaping.

Result: System-level gains are governed by connectivity restoration (preserving connectivity under blockage and mobility) and angular selectivity (reshaping interference to enlarge spectrum reuse). Practical operation requires Delay-Doppler channel acquisition, predictive beam tracking, and control designs budgeting overhead and latency.

Conclusion: RIS enables overcoming key satellite network barriers. Future opportunities include generative AI paradigm, multifunctional RIS architectures, ubiquitous satellite integrated sensing and communication, and sustainable satellite Internet-of-Things.
...
### [170] [Low-Altitude ISAC with Rotatable Active and Passive Arrays](https://arxiv.org/abs/2512.20987)
*Ziyuan Zheng,Qingqing Wu,Yanze Zhu,Honghao Wang,Ying Gao,Wen Chen,Jian Xiong*

Main category: eess.SP

TL;DR: This paper proposes a low-altitude ISAC system with cooperative rotatable active/passive arrays, developing a rotation-aware multipath model and optimization framework to maximize sum-rate while meeting sensing requirements.


<details>
  <summary>Details</summary>
Motivation: To enhance low-altitude ISAC performance by leveraging mechanical rotation of BS and RIS arrays to dynamically adjust beam directions, overcoming limitations of fixed arrays in 3D environments with directional antennas.

Method: Developed rotation-aware geometry-based multipath model; formulated optimization problem for sum-rate maximization with sensing constraints; proposed penalty-based AO framework with QT/MM for precoder, RCG for RIS phases, and PGD for rotation angles.

Result: Numerical results show significant sum-rate gains over fixed/partially rotatable baselines while guaranteeing sensing requirements, especially with directional antennas and in interference-limited regimes.

Conclusion: Jointly rotatable BS-RIS architecture effectively enhances low-altitude ISAC performance by exploiting mechanical rotation to optimize beam alignment and mitigate interference.
...
### [171] [LEO Constellations as a Decentralized GNSS Network: Optimizing PNT Corrections in Space](https://arxiv.org/abs/2512.21018)
*Xing Liu,Xue Xian Zheng,José A. López-Salcedo,Tareq Y. Al-Naffouri,Gonzalo Seco-Granados*

Main category: eess.SP

TL;DR: Decentralized GNSS network for LEO constellations enables precise orbit and clock estimation through neighbor-to-neighbor communication, matching centralized accuracy while reducing communication overhead.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of LEO constellations creates an unprecedented spaceborne GNSS network, but current onboard processing is standalone and insufficient for high-precision applications, while centralized fusion faces computational bottlenecks and lacks in-orbit infrastructure.

Method: Model the moving constellation as a dynamic graph and tailor a momentum-accelerated gradient tracking method to ensure steady convergence despite topology changes. Each satellite processes its own measurements while exchanging compact information with neighboring nodes.

Result: Numerical simulations with constellations containing hundreds of satellites show the proposed method matches the accuracy of an ideal centralized benchmark while substantially reducing communication burdens.

Conclusion: This framework supports the development of autonomous and self-organizing space systems, enabling high-precision navigation with reduced dependence on continuous ground contact.
...
### [172] [Co-Existence of Private 5G Network and Wireless Hospital Systems](https://arxiv.org/abs/2512.21079)
*Mohsin Khan,Matti Hämäläinen,Timo J. Mäkelä,Erkki Harjula,Jani Katisko*

Main category: eess.SP

TL;DR: Study shows private 5G networks in hospital operating rooms are safe and don't interfere with existing wireless systems, with RF exposure well below safety limits.


<details>
  <summary>Details</summary>
Motivation: To evaluate the feasibility of deploying private 5G networks in hospital environments, specifically assessing interference risks with existing wireless systems (LTE, Wi-Fi) and electromagnetic safety for patients and staff.

Method: Conducted measurement campaign at Oulu University Hospital using state-of-the-art instrumentation with high-resolution, long-duration spectrum scans in the 3.9-4.1 GHz band.

Result: No measurable interference between private 5G network and adjacent LTE/Wi-Fi bands; RF exposure levels in operating room were well below ICNIRP, WHO, and IEEE safety thresholds.

Conclusion: Private 5G networks can be safely integrated into hospital environments without compromising electromagnetic compatibility or patient safety, providing foundation for digital transformation in healthcare.
...
### [173] [Cruising the Spectrum: Joint Spectrum Mobility and Antenna Array Management for Mobile (cm/mm)Wave Connectivity](https://arxiv.org/abs/2512.21203)
*Ece Bingöl,Eylem Ekici,Mehmet C. Vuran*

Main category: eess.SP

TL;DR: The paper proposes a spectrum mobility and antenna array management approach for mmWave FR2/FR3 bands using POMDP and PBVI to optimize frequency selection and beam direction under mobility.


<details>
  <summary>Details</summary>
Motivation: Millimeter wave bands (FR2: 24-71 GHz, FR3: 7-24 GHz) offer large bandwidths but face challenges with directional beamforming in mobile scenarios like vehicular connectivity. Current antenna and beam management approaches struggle with mobility and increasing number of available bands.

Method: Formulates spectrum mobility as a Partially Observable Markov Decision Process (POMDP) to select optimal channel frequency and beam direction in each time slot. Uses Point-Based Value Iteration (PBVI) algorithm to find a policy with performance guarantees.

Result: Numerical examples confirm the efficacy of the resulting policy for multiple available frequency bands, even when user mobility significantly deviates from models assumed during policy generation.

Conclusion: The proposed spectrum mobility approach with POMDP and PBVI provides robust beamforming performance under mobility conditions across multiple frequency bands, addressing challenges in mmWave vehicular connectivity.
...
### [174] [Neural Network-Assisted RIS Weight Optimization for Spatial Nulling in Distorted Reflector Antenna Systems](https://arxiv.org/abs/2512.21253)
*Xinrui Li,R. Michael Buehrer*

Main category: eess.SP

TL;DR: Proposes ResNet-SA framework using neural networks to optimize RIS weights for reflector antennas with pattern distortions, achieving better interference cancellation than conventional methods.


<details>
  <summary>Details</summary>
Motivation: Real-world reflector antennas have distortions causing mismatches between theoretical and actual radiation patterns, degrading interference cancellation performance when using RIS weights optimized from theoretical models alone.

Method: Combines residual learning neural network with simulated annealing optimization. The NN learns the residual difference between theoretical and true antenna gains, then embeds this learned model into heuristic optimization to find optimal weight vectors without requiring explicit knowledge of distorted electric field.

Result: Simulation results show improved null depth in true radiation pattern compared to conventional methods that optimize weights based solely on theoretical models.

Conclusion: The ResNet-SA algorithm effectively addresses pattern mismatch in reflector antenna systems with approximate pattern knowledge, validating its effectiveness for practical RIS applications.
