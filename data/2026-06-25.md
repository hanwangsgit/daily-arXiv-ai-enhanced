...
TL;DR: GenSeC-PC is a channel-adaptive cross-modal generative semantic communication framework for point cloud transmission, using images as side information and diffusion models for reconstruction, enabling efficient real-time analog transmission under diverse conditions.


<details>
  <summary>Details</summary>
Motivation: With the rapid growth of autonomous driving and extended reality, efficient point cloud transmission is critical but faces challenges in bandwidth, noise robustness, and real-time performance. Existing methods lack cross-modal synergy and require error-free side information.

Method: Proposes a cross-modal semantic encoder fusing images (side info) with point clouds, a decoder based on PointDif backbone, an asymmetric channel-adaptive joint semantic-channel coding using SNR/bandwidth feedback, rectified denoising diffusion implicit models for fast decoding, and fully analog transmission.

Result: Simulations confirm effectiveness of cross-modal semantic extraction and dual-metric guided fine-tuning, showing robustness under low SNR, bandwidth limits, varying image counts, and unseen objects, with millisecond-level decoding speed.

Conclusion: GenSeC-PC achieves efficient, robust, and real-time point cloud communication via cross-modal generative SemCom, outperforming prior methods in compression and reconstruction quality across diverse transmission conditions.
...
TL;DR: This paper evaluates whether PE risk stratification can be accurately performed using only medical records and CTPA-derived biomarkers, without blood tests. The study finds that global features (medical records and cardiac biomarkers) are most predictive, while vascular biomarkers and even GNNs on vascular graphs do not improve performance, suggesting vascular graphs may lack discriminative information for PE risk stratification.


<details>
  <summary>Details</summary>
Motivation: Blood tests are often missing in routine clinical practice, yet current PE risk stratification guidelines rely on them. The authors aim to determine if accurate risk stratification is possible using only medical records and CTPA-derived biomarkers.

Method: The authors benchmark various approaches combining medical records, cardiac biomarkers, and vascular biomarkers using tabular models and GNNs on vascular tree graph representations, evaluated on a private dataset (n=353) with complete PE risk stratification data.

Result: Medical records and cardiac biomarkers are the most significant predictors; vascular biomarkers do not improve stratification. GNNs on vascular graphs fail to outperform strong tabular baselines using global features.

Conclusion: Vascular graphs may hold no discriminative information for PE risk stratification, counter to expectations.
...
TL;DR: Reflective VLA enhances vision-language-action models by conditioning decisions on observation-action-consequence triplets, enabling deployment-specific adaptation and improving generalization under distribution shifts by 4.2-5.4 percentage points.


<details>
  <summary>Details</summary>
Motivation: Reactive VLA models assume current observations fully specify action-relevant states, but in embodied control, embodiment-specific factors (e.g., camera-to-robot geometry, calibration, actuation bias) are hard to identify from a single observation, leading to overfitting and poor generalization under distribution shifts.

Method: Reflective VLA conditions decisions on context of observation-action-consequence triplets, recording what the robot observed, executed, and how the scene changed afterward. All observation modalities are routed through VLM under shared attention, with a block-causal mask enabling parallel multi-frame training without leakage and KV-cached real-time inference.

Result: Preserves strong in-distribution performance on LIBERO and SimplerEnv-Bridge. Under distribution shift (LIBERO-Plus and LIBERO-Plus-Hard), improves average success rate by 5.4 and 4.2 percentage points over reactive baseline. Ablations show action consequences, not just extra context length, drive cross-environment generalization.

Conclusion: Modeling action consequences is key to generalizable embodied control, and Reflective VLA provides an effective architecture for integrating this reflective reasoning into real-time VLA systems.
...
### [4] [An iterative energy-based multimodal transformer for joint retrieval of wheat soil moisture, leaf area index, and plant height from Sentinel-1 and Sentinel-2 time series](https://arxiv.org/abs/2606.25174)
*Shubham Kumar Singh,Peilei Fan,Suraj A. Yadav,Rajendra Prasad,Prashant K Srivastava*

Main category: cs.LG

TL;DR: iEBT is an iterative energy-based transformer that jointly retrieves soil moisture, leaf area index, and plant height from Sentinel-1/2 data by minimizing a learned energy function, achieving strong performance (mean R²=0.854) on field data from India, with the energy value serving as a built-in quality diagnostic.


<details>
  <summary>Details</summary>
Motivation: Field-scale retrieval of soil moisture, LAI, and plant height is essential for precision agriculture but remains an ill-posed inverse problem due to concurrent variations in soil and canopy parameters causing ambiguities in radar and optical responses, reducing effectiveness of traditional regression models in heterogeneous smallholder systems.

Method: The Iterative Energy-Based Transformer (iEBT) embeds multi-modal predictors (Sentinel-1 SAR and Sentinel-2 multispectral) within a shared sequence, produces an initial state estimate, and iteratively updates the target [SM, LAI, PH] vector through normalized gradient descent to minimize a learned scalar compatibility energy function.

Result: Using 700 field measurements from Varanasi, India, iEBT achieved mean R²=0.854±0.012 across four seeds (R²_SM=0.841, R²_LAI=0.905, R²_PH=0.821). Sentinel-1 drives SM retrieval, Sentinel-2 dominates LAI, while PH relies on combined signatures. The terminal energy serves as an uncalibrated quality diagnostic; screening the 10% highest-energy samples reduced RMSE.

Conclusion: Compatibility-guided multimodal fusion via iEBT offers a structured self-diagnostic path toward reliable biophysical parameter estimation, though leave-one-campaign-out validation highlights persistent cross-season domain shift challenges due to localized management variations.
...
### [5] [When Do Conservation Laws Survive Learned Representations? Certified Horizons for Latent World Models](https://arxiv.org/abs/2606.24945)
*Hongbo Wang*

Main category: cs.LG

TL;DR: This paper studies how conservation laws in physical systems remain certifiably valid after learning latent representations. It introduces "shell-horizon certificates" that bound rollout error by decomposing into representation, readout, and dynamics defects, showing that decoded physical invariants—not latent Hamiltonians—are the robust object for certification across different observation types.


<details>
  <summary>Details</summary>
Motivation: To understand when conservation laws (e.g., energy, momentum) remain verifiably preserved after a model learns a compressed latent representation of physical dynamics, and to provide formal guarantees on rollout fidelity despite representation learning errors.

Method: Defines "decoded physical invariants" (decoding latent state then evaluating known invariant) as the certified object. Derives "shell-horizon certificates" that decompose certification budgets into representation defects, readout defects, and latent-dynamics defects. Uses a monotone alignment bridge where a soft learned witness yields a certified horizon. Tests across state, learned-lift, and pixel observations.

Result: Hard canonical symplectic structure yields longest horizons but fails in learned charts. Controlled-Lipschitz-aligned soft invariants survive learned-representation settings. Pixel certification is recovered on a readout-stable sub-tube. The Kepler problem reveals a geometric boundary where certification breaks down.

Conclusion: The central object for certification is not a latent Hamiltonian but a decoded physical invariant, whose robustness to representation learning can be measured, certified, and empirically falsified. Not all geometric priors transfer equally to learned representations.
...
### [6] [Conformal Orbit-Valid Trust Horizons for Equivariant World Models](https://arxiv.org/abs/2606.24946)
*Hongbo Wang*

Main category: cs.LG

TL;DR: This paper proposes a method for certifying trust horizons of latent world models with known group symmetries, using split-conformal calibration to produce conservative, orbit-constant horizon certificates. Experiments on 2D and 3D domains show equivariant models achieve valid, non-vacuous certificates while non-equivariant baselines require more calibration sectors or incur violations.


<details>
  <summary>Details</summary>
Motivation: Learned world models are only useful when their rollout error remains bounded over time, but existing methods lack rigorous certification of how far ahead predictions can be trusted, especially when model symmetries can be exploited for generalization.

Method: The authors formulate a trust-horizon certificate using a raw horizon curve from one-step residual and finite-time expansion estimates, then calibrate it with split-conformal multiplicative factors. They prove that under exact equivariance/invariance conditions, rollout errors and trust horizons are orbit-constant, enabling single-sector calibration to generalize across group orbits.

Result: The conformal factor is 1.0 (raw certificate already conservative); zero anti-conservative violations across 50 audits (95% upper bound 5.8% violation rate). Equivariant models show small orbit-transport residuals (median 1.1%). Certificate is non-vacuous (median certified-to-measured ratio 0.67). On 2D substrate, all models are orbit-valid from single sector; on 3D yaw audit, only equivariant model achieves safe, non-vacuous one-sector certificate.

Conclusion: Equivariant latent world models enable efficient, calibrated trust-horizon certification that generalizes across group orbits, reducing calibration cost compared to non-equivariant baselines. The certificate is a conservative distributional audit, not a global guarantee, and subgoal spacing guidance from the certificate is not yet confirmed in the 3D MPC layer.
...
### [7] [Swarm-Inspired Generation of Collective Behaviors in Graph Dynamical Systems](https://arxiv.org/abs/2606.24958)
*Ji Chen,Song Chen,Chengzhang Gong,Li Fan,Chao Xu*

Main category: cs.LG

TL;DR: SIES is a graph-dynamical framework that learns local interaction rules to produce desired global collective organization, achieving strong results in synchronization control, robot locomotion, and heterophilous graph learning.


<details>
  <summary>Details</summary>
Motivation: To design local interaction rules that can produce desired global organization and generalize across graphs, dynamics, and tasks—addressing the challenge of not just explaining but controlling collective behavior.

Method: SIES uses signed source-target-conditioned attention as adaptive coupling inside an explicit evolution model, combining a dynamical engine with local agent intelligence akin to biological swarms.

Result: SIES generalizes across untrained network scales, target phase relations, and intrinsic dynamics for synchronization; outperforms oscillator baselines in gait-related modes; generalizes to simulated/real hexapod robots after leg disablement; achieves top performance on heterophilous node-classification benchmarks.

Conclusion: SIES is a generalizable, learnable graph-dynamical framework promising for synchronization control, adaptive robot coordination, and heterophilous graph representation learning.
...
### [8] [Memory-Efficient Policy Libraries with Low-Rank Adaptation in Reinforcement Learning](https://arxiv.org/abs/2606.25700)
*Samuel Valland Lyngset,Tor Viljen Raanaas,Gard Sveipe,Eirik Møller Nilsen,Jim Torresen,Kai Olav Ellefsen,Tobias Lømo*

Main category: cs.LG

TL;DR: This paper applies LoRA (a PEFT method from NLP) to multi-task robotics with PPO, showing memory reductions of 20-160x and 90-95% storage savings for policy libraries without sacrificing task success rates.


<details>
  <summary>Details</summary>
Motivation: To explore whether Parameter-Efficient Fine-Tuning (PEFT) methods like LoRA, successful in LLMs, can be transferred to robotics and Reinforcement Learning to reduce memory usage and improve computational efficiency, especially for multi-task policy libraries where memory efficiency is critical.

Method: Applied LoRA fine-tuning within a Proximal Policy Optimization (PPO) algorithm to adapt a baseline model to various robotics tasks, creating a library of specialist policies and comparing memory usage and success rates against full fine-tuning.

Result: LoRA reduces memory usage by 20-160x compared to full fine-tuning, achieving 90-95% storage savings for a library of 10-50 specialized policies, with no significant difference in success rates between LoRA and full fine-tuning.

Conclusion: LoRA is effectively transferable to robotics RL, enabling substantial memory savings in multi-task policy libraries without performance degradation, which is critical for practical deployment where memory is constrained.
...
### [9] [ROAD-VLA: Robust Online Adaptation via Self-Distillation for Vision-Language-Action Models](https://arxiv.org/abs/2606.25800)
*Kejing Wang,Toan Nguyen,Minh Hoang Nguyen,Simon Khan,Flora D. Salim*

Main category: cs.LG

TL;DR: ROAD-VLA introduces an advantage-guided self-distillation framework that bridges the modality gap in VLA model adaptation by creating a proximal teacher in action space using perturbed action-token logits with calibrated advantage estimates, outperforming PPO across seven robotic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Online adaptation of VLA models is difficult due to sparse reward signals providing weak supervision for high-dimensional action policies, and text-based privileged teachers suffer from a modality gap between symbolic guidance and low-level robot actions.

Method: ROAD-VLA uses an advantage-guided self-distillation framework that constructs a proximal teacher in action space by perturbing action-token logits with calibrated advantage estimates, converting sparse rewards into dense token-level supervision while keeping the teacher close to the current policy, supported by a policy-improvement lower bound.

Result: Outperforms PPO in nearly all settings across seven robotic manipulation environments with both in-distribution and out-of-distribution shifts, demonstrating robust online VLA adaptation.

Conclusion: Action-space proximal teachers with calibrated advantage estimates effectively bridge the modality gap and enable robust online VLA adaptation where text-based privileged teachers fail.
...
### [10] [Information from coincidences](https://arxiv.org/abs/2606.25042)
*Akshay Balsubramani*

Main category: cs.IT

TL;DR: This paper proves a single algebraic mixed coincidence identity that unifies many information-theoretic variational results (Sanov's theorem, Chernoff information, Donsker-Varadhan, PAC-Bayes, etc.) as special cases, generalizing Renyi entropy/divergence to multiple priors, and demonstrates applications in language modeling and genomics.


<details>
  <summary>Details</summary>
Motivation: To unify a broad class of information-theoretic variational results (concentration, hypothesis testing, change-of-measure, rare-pattern laws) under a single algebraic identity, and to extend Renyi entropy/divergence variational formulas from 1-2 priors to W-prior simplex.

Method: Proves a mixed coincidence identity involving the log of mixed count E_x∼ν[∏ π_i^{α_i}(x)], showing it simultaneously acts as Boltzmann coincidence weight, exponential-family normalizer, maximum-entropy value, and KL-barycenter optimum. The identity generalizes to unnormalized and continuum-indexed priors.

Result: Unified derivation of Sanov-type decompositions, Chernoff information, Donsker-Varadhan inequality, PAC-Bayes bounds, Erdos-Renyi run-length, iterative guesswork, rate-distortion, and birthday thresholds. Yields exact multi-prior PAC-Bayes penalty and asymptotic MAP error exponent for W-ary hypothesis testing.

Conclusion: Demonstrates applicability on language-model next-token prediction (recovering contrastive decoding) and human genomic regulatory sequence (separating correlated from diverse prior families via sliding-window trace).
...
### [11] [Wideband Near-Field Channel Estimation Under Hybrid Compression: Cross-Subcarrier KL Covariance Fitting With OFDM Fresnel Model](https://arxiv.org/abs/2606.25101)
*Rıfat Volkan Şenyuva*

Main category: cs.IT

TL;DR: The paper proposes a wideband channel estimation method (WB-CL-KL) for XL-MIMO arrays using hybrid analog-digital compression, addressing subcarrier-dependent near-field effects. It jointly estimates angle and range from compressed sample covariance via KL divergence, achieving near-CRB performance (range RMSE ratio 0.996 at SNR=10 dB, 0.959 at median deployment SNR).


<details>
  <summary>Details</summary>
Motivation: Standard narrowband polar-domain models fail for wideband XL-MIMO at 28 GHz with hundreds of MHz bandwidth because the near-field Fresnel curvature becomes subcarrier-dependent, and compressed observations destroy per-subcarrier spatial covariance structure that narrowband methods rely on.

Method: Proposes the Wideband Cross-subcarrier Kullback--Leibler (WB-CL-KL) estimator that jointly estimates angle and range from compressed sample covariance across OFDM subcarriers by fitting a structured Fresnel covariance model via cross-subcarrier KL divergence, without full-array reconstruction. Also derives the compressed-domain CRB using the Slepian-Bangs formula.

Result: The wideband compressed-domain CRB decomposes into a data-diversity component (+27.093 dB) and a geometric-diversity component (+0.701 dB), totaling +27.793 dB at B=400 MHz. WB-CL-KL achieves range RMSE of 19.8 mm vs 19.9 mm bound (ratio 0.996) at SNR=10 dB, and 0.959 ratio at median deployment SNR of 9.6 dB under 3GPP UMi model.

Conclusion: WB-CL-KL achieves near-CRB operation in the single-path LOS regime, demonstrating that joint subcarrier processing can effectively overcome wideband near-field challenges in compressed XL-MIMO systems without requiring full-array reconstruction.
...
### [12] [Inner and Outer Bounds on the Secrecy Capacity of Degraded Broadcast Channels with RMSI and Transmitter CSI](https://arxiv.org/abs/2606.25351)
*Saeid Pakravan,Mohsen Ahmadzadeh,Ghosheh Abed Hodtani*

Main category: cs.IT

TL;DR: This paper derives secrecy capacity regions for degraded broadcast channels with confidential messages, external eavesdroppers, channel state information (CSI), and receiver message side information (RMSI), considering both complementary and non-complementary RMSI scenarios.


<details>
  <summary>Details</summary>
Motivation: To characterize the fundamental limits of secure communication in multiuser broadcast settings where both CSI at the transmitter and prior message knowledge at receivers (RMSI) are available, addressing gaps left by prior work that did not jointly consider these factors under secrecy constraints.

Method: The authors derive novel inner bounds (achievable secrecy rate regions) and tight outer bounds for degraded broadcast channels under two RMSI scenarios (complementary and non-complementary), then extend analysis to Gaussian degraded broadcast channels.

Result: Complete characterization of the secrecy capacity region for the considered degraded channel settings; demonstration that combining RMSI and CSI can expand the secrecy capacity region.

Conclusion: CSI plays a pivotal role in enhancing secure transmission, and the strategic combination of RMSI and CSI offers new design insights for secure multiuser communication systems.
...
### [13] [OAMP-Aided Joint Channel Estimation and Data Detection for ODDM Systems](https://arxiv.org/abs/2606.25420)
*Kehan Huang,Min Qiu,Akram Shafie,Jinhong Yuan*

Main category: cs.IT

TL;DR: This paper proposes an OAMP-aided JED receiver for ODDM systems that alternately performs channel estimation and data detection with closed-form variance updates, achieving near-perfect-CSI performance.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of joint channel estimation and data detection (JED) for ODDM in doubly selective channels, where conventional separate approaches suffer from error propagation.

Method: A bilinear cross-domain JED model is formulated and linearized into two subproblems. Two OAMP modules are alternately executed for channel estimation and data detection, coupled via a variational noise term. Closed-form scalar-variance updates leverage OAMP's error orthogonality for principled soft information exchange.

Result: For both uncoded and coded ODDM, OAMP-JED achieves lower BER than benchmark schemes, and its BER closely approaches that of OAMP with perfect CSI.

Conclusion: The proposed OAMP-JED receiver effectively mitigates error propagation in JED and achieves near-optimal performance without requiring perfect channel state information.
...
### [14] [Center-Fed Pinching Antenna System for Uplink Environment Sensing](https://arxiv.org/abs/2606.25423)
*Cong Yu,Xu Gan,Bo Ai,Fellow,Yuanwei Liu,Wei Chen*

Main category: cs.IT

TL;DR: A center-fed pinching antenna system (C-PASS) is proposed for uplink environment sensing, achieving double the degrees of freedom and lower reconstruction error bounds compared to conventional end-fed PASS.


<details>
  <summary>Details</summary>
Motivation: To improve environment sensing performance by overcoming the limitations of conventional end-fed pinching antenna systems (PASS), particularly in terms of degrees of freedom and reconstruction error.

Method: A center-fed pinching antenna system (C-PASS) framework is designed with a linear inverse model for environment reconstruction. Closed-form feed-point distance for signal separation is derived, and Ziv-Zakai bounds (ZZB) for mean-squared reconstruction error are derived for both C-PASS and end-fed PASS.

Result: C-PASS achieves doubled degrees of freedom and strictly lower ZZB reconstruction error bounds than conventional end-fed PASS. Numerical results validate the ZZB accuracy and confirm more stable signal separation and consistent performance advantages.

Conclusion: C-PASS is proven superior to conventional PASS for uplink environment sensing, offering better signal separation and lower reconstruction error.
...
### [15] [A Path-Survival Analytical Framework for SCL Decoding of Polar Code](https://arxiv.org/abs/2606.25522)
*Xianbin Wang,Zhichao Liu,Yuan Li,Huazi Zhang,Jiajie Tong,Jun Wang,and Wen Tong*

Main category: cs.IT

TL;DR: This paper proposes an analytical framework for CA-SCL decoding of polar codes, using a path-survival model to predict performance without exhaustive simulations.


<details>
  <summary>Details</summary>
Motivation: Despite widespread practical adoption of CA-SCL decoding for polar codes, there is no theoretical analysis framework comparable to density evolution for LDPC codes, due to the complex path-pruning mechanism.

Method: A path-survival model that captures the evolution of the correct path's rank during CA-SCL decoding, enabling efficient performance prediction.

Result: The framework effectively predicts CA-SCL decoding performance across various code lengths, rates, list sizes, and channel models, as validated by extensive numerical evaluations.

Conclusion: The proposed analytical framework fills the theoretical gap for CA-SCL decoding analysis, offering a practical tool for performance prediction without exhaustive Monte Carlo simulations.
...
### [16] [Interplay between VAoI, Packet Error Rate, and Delay for Energy-Efficient Remote Monitoring](https://arxiv.org/abs/2606.25566)
*Yasaman Khorsandmanesh,Zinat Behdad,Emil Bjornson,Cicek Cavdar*

Main category: cs.IT

TL;DR: This paper studies energy optimization for short-packet transmission in event-triggered remote monitoring, using VAoI as a freshness metric, and proposes a low-complexity method to minimize long-term energy under VAoI constraints.


<details>
  <summary>Details</summary>
Motivation: To optimize energy consumption in wireless sensor networks with event-triggered updates, considering freshness (VAoI) and finite-blocklength transmission constraints.

Method: Models VAoI evolution as a Markov chain coupled with packet error rate and delay; formulates a transmit-power allocation problem solved via low-complexity search.

Result: Update arrival probability and blocklength significantly impact the energy–VAoI tradeoff; optimizing long-term energy outperforms minimizing per-transmission energy.

Conclusion: The proposed approach substantially reduces energy consumption under VAoI constraints, highlighting the importance of long-term optimization over transmission-level approaches.
...
### [17] [The Condition for Structured Coding to Improve Random Coding in the Binary Modulo-sum Problem](https://arxiv.org/abs/2606.25570)
*Yohsuke Tsujino,Shun Watanabe*

Main category: cs.IT

TL;DR: This paper analytically characterizes conditions where multi-letter extended Ahlswede-Han (AH) coding improves upon Slepian-Wolf (SW) coding for the modulo-sum problem, using a method-of-types approach to convert multi-letter divergence comparisons into single-letter ones.


<details>
  <summary>Details</summary>
Motivation: Previous work showed numerically that multi-letter AH coding can beat SW coding for the modulo-sum problem, but the exact conditions were unknown. This paper aims to provide an analytical characterization of when such improvement occurs.

Method: The authors apply the method of types to reduce multi-letter expression evaluation to a comparison of single-letter divergences, enabling analytical characterization of the improvement conditions.

Result: They derive tight analytical conditions under which multi-letter extended AH coding outperforms SW coding, which coincide with the complement of the known sufficient condition for SW optimality.

Conclusion: The conditions are tight and analytically characterized, resolving a gap left by prior numerical approaches. The single-letter divergence reduction technique may be independently useful for other problems.
...
### [18] [Performance Analysis for Heterogeneous Air-Ground ISAC in Coordinated Multipoint Networks](https://arxiv.org/abs/2606.25574)
*Yihang Jiang,Xiaoyang Li,Guangxu Zhu,Changsheng You,Xiaowen Cao,Dingzhu Wen,Bingpeng Zhou,Xinyi Wang,Rui Zhang*

Main category: cs.IT

TL;DR: This paper proposes a heterogeneous air-ground ISAC network architecture using coordinated multipoint (CoMP) with hybrid mono/bi-static sensing to support both communication and sensing in low-altitude economy scenarios, analyzing trade-offs via simulations.


<details>
  <summary>Details</summary>
Motivation: Existing ISAC research focuses on link-level or single-cell terrestrial architectures, leaving network-level cooperative air-ground ISAC unexplored. The emergence of low-altitude economy (LAE) demands highly integrated communication and sensing systems.

Method: A heterogeneous air-ground ISAC network architecture based on CoMP is proposed, using two-tier BS deployment (master BSs in hexagonal lattice, slave BSs via Poisson point process) with a cooperative hybrid mono/bi-static sensing scheme.

Result: Simulation results reveal inherent trade-offs between communication and sensing performance, particularly under multi-BS cooperation and varying network density.

Conclusion: These findings provide practical guidance for deploying scalable and efficient ISAC networks in low-altitude economy scenarios.
...
### [19] [The MDS or NMDS for Modified GRS codes with flexible hull dimensions and lengths](https://arxiv.org/abs/2606.25662)
*Zhonghao Liang,Qunying Liao,Jun Zhang,Xiaoping Li*

Main category: cs.IT

TL;DR: This paper studies modified generalized Reed-Solomon (MGRS) and extended MGRS (EMGRS) codes, proving they are either MDS or NMDS, determining their weight distributions, constructing LCD and hull codes, and showing their inequivalence to elliptic curve codes.


<details>
  <summary>Details</summary>
Motivation: Non-GRS type MDS, NMDS, and LCD codes have important applications in cryptography and coding theory, motivating the study of MGRS and EMGRS codes as a specific class of non-GRS codes.

Method: The authors analyze MGRS/EMGRS codes theoretically: prove MDS/NMDS conditions, compute weight distributions (for one class), construct Euclidean LCD and one-dimensional hull codes, and demonstrate linear inequivalence via Schur product.

Result: - MGRS and EMGRS codes are either MDS or NMDS (with necessary/sufficient conditions given).<br>- Weight distributions for one class of NMDS MGRS/EMGRS codes are fully determined.<br>- Four classes of Euclidean LCD or one-dimensional Euclidean hull MGRS codes are constructed.<br>- MGRS codes with flexible Hermitian hull dimensions and lengths exist.<br>- NMDS MGRS codes are linearly inequivalent to elliptic curve NMDS codes.

Conclusion: MGRS and EMGRS codes are versatile non-GRS codes with rich MDS/NMDS structure, LCD and hull properties, and are distinct from elliptic curve codes.
...
### [20] [MAP-Based Task-Oriented Precoding for Multiuser Communication](https://arxiv.org/abs/2606.25722)
*Mohammad Javad Ahmadi,Rafael F. Schaefer,H. Vincent Poor*

Main category: cs.IT

TL;DR: A MAP-driven task-oriented multiuser wireless communication framework for distributed classification is proposed, which achieves higher classification accuracy with lower computational complexity by using a tractable class-mean separation objective to jointly design feature extraction and precoding, avoiding costly covariance inversions and eigen-decompositions.


<details>
  <summary>Details</summary>
Motivation: To improve distributed classification accuracy under wireless channel impairments while reducing computational complexity, avoiding the heavy overhead of existing covariance-based and reconstruction-oriented methods that require repeated covariance inversions and eigen-decomposition operations.

Method: Derives a tractable class-mean separation objective for MAP-driven system design, enabling low-complexity joint optimization of learning-based feature extraction and precoding strategies that directly enhance class separability after channel distortion.

Result: Achieves higher classification accuracy than existing schemes while simultaneously reducing computational complexity, avoiding repeated covariance inversions and eigen-decompositions.

Conclusion: The proposed task-oriented, MAP-driven framework effectively balances accuracy and efficiency for distributed classification under wireless channel impairments, outperforming covariance-based and reconstruction-oriented approaches.
...
TL;DR: BFMs struggle with time-varying motion tracking. The paper proposes Latent Sequence Optimization (LSO) that uses simulation rollouts and policy gradient to optimize latent sequences, enabling precise motion tracking without reward engineering.


<details>
  <summary>Details</summary>
Motivation: Behavioral Foundation Models (BFMs) cannot handle time-varying objectives like motion sequence tracking. Existing heuristics (moving-window-averaging) fail for highly dynamic motions.

Method: Latent Sequence Optimization (LSO): combines simulation rollouts with policy gradient updates over a sequence of latents; uses temporally correlated noise to model smooth latent trajectories.

Result: Validated on dense tracking, sparse keyframing, and direct deployment on a real humanoid robot.

Conclusion: LSO extends BFMs to precise motion tracking without reward engineering, outperforming heuristic baselines on dynamic motion tasks.
...
TL;DR: The paper introduces a relative L-extended pose Lie group representation for IMU-instrumented articulated rigid-body systems, enabling invariant extended Kalman filtering with joint kinematic constraints. The proposed iterated IEKF (IterIEKF) achieves 50%+ RMSE reduction over baselines on UR5e robot and human leg validation.


<details>
  <summary>Details</summary>
Motivation: Extending IEKF's convergence and consistency guarantees from single rigid bodies to articulated systems is challenging because inter-body pose coupling prevents direct application, and incorporating joint kinematic constraints within the invariant framework remains unsolved.

Method: Proposes relative L-extended pose Lie group representation for kinematic-tree systems; uses one IMU per body to achieve group-affine dynamics; incorporates joint constraints as noise-free pseudo-measurements within an iterated IEKF (IterIEKF).

Result: IterIEKF outperforms all baselines (EKF, IterEKF, absolute-pose IterIEKF) on UR5e robot and human leg data; converges faster, lower variability, and achieves at least 50% RMSE reduction over the second-best filter.

Conclusion: The proposed relative L-extended pose formulation successfully extends invariant filtering guarantees to articulated systems with joint constraints, demonstrating significant performance improvements in pose estimation.
...
TL;DR: ADM-Fusion is an end-to-end deep learning multi-sensor fusion method that uses an adaptive mixture-of-experts with content-aware routing to dynamically weight sensors, plus separate translation/rotation branches with cross-task attention, achieving robust ego-motion estimation under sensor degradation.


<details>
  <summary>Details</summary>
Motivation: Multi-sensor fusion in autonomous systems needs to handle varying sensor reliability in degraded environments; existing fixed-weight fusion methods fail when different modalities degrade unpredictably, motivating an adaptive, content-aware approach.

Method: An adaptive sensor mixture-of-experts framework with content-aware routing for dynamic sensor weighting, plus separate translation and rotation branches coupled via cross-task attention, trained on CARLA-LOC and fine-tuned on KITTI.

Result: Robust under degraded conditions while maintaining competitive performance against existing methods; successful simulation-to-real transfer demonstrated.

Conclusion: ADM-Fusion effectively adapts to sensor degradation and environmental changes, providing a promising adaptive fusion paradigm for reliable ego-motion estimation in autonomous systems.
...
TL;DR: This paper introduces SurveilNav, a collaborative navigation framework that integrates robot perception with multi-view surveillance cameras, achieving state-of-the-art performance on a new multi-floor indoor dataset built on Habitat-Sim.


<details>
  <summary>Details</summary>
Motivation: Existing surveillance-robot integration methods focus on single-robot scenarios and cannot handle multi-view collaboration in large-scale environments effectively.

Method: SurveilNav framework integrates active camera scheduling, joint 2D/3D mapping, VLM-based value estimation, and collaborative target verification to synergize robot's dynamic local perception with static global surveillance views.

Result: SurveilNav achieves state-of-the-art performance on the HM3D dataset, outperforming existing methods in both exploration efficiency and navigation success rate.

Conclusion: The proposed approach effectively overcomes limited perception range of single agents and blind spots of fixed cameras, showing strong potential for large-scale search, home environments, and rescue missions.
...
TL;DR: AeroCast is a probabilistic trajectory prediction framework using a Transformer encoder with Mixture Density Network to predict multi-modal future positions of non-cooperative aerial obstacles, achieving ~50% error reduction over baselines with 0.1ms inference suitable for real-time deployment.


<details>
  <summary>Details</summary>
Motivation: Non-cooperative obstacles (birds, uncontrolled drones, debris) exhibit multi-modal motion that deterministic predictors cannot adequately represent, and existing methods struggle with long-range kinematic precursors or fail to provide distributional information needed for downstream planning.

Method: Combines a Transformer encoder (to capture long-range dependencies) with a Mixture Density Network output head (to predict per-timestep Gaussian mixture distributions over future 3D displacements); uses translation-invariant consecutive displacement encoding and calibration-oriented training objective to address input design and mode-degeneracy challenges.

Result: On a hybrid real-and-synthetic quadrotor dataset (9 motion categories), AeroCast reduces ADE and FDE by ~50% over baselines at 5-second horizon, achieves lowest NLL and CRPS among compared methods, and runs at 0.1ms per sample (compatible with 100Hz real-time deployment).

Conclusion: AeroCast effectively addresses multi-modal aerial trajectory prediction for non-cooperative obstacles with high accuracy and real-time feasibility, with velocity input and model capacity being primary contributors to quality and positional encoding essential for long-horizon coherence.
...
TL;DR: This paper introduces an RL-guided whole-body Model Predictive Path Integral (MPPI) framework that acts as an add-on feedback controller on top of a pretrained RL policy for humanoid robots. The RL policy serves as a sampling prior to bias MPPI rollouts toward feasible behaviors, while MPPI corrects the RL prior online to satisfy task-specific cost objectives without retraining. Simulations on a Unitree G1 humanoid show improved precision over pure RL baselines at stable 280 Hz control.


<details>
  <summary>Details</summary>
Motivation: Humanoid robots need whole-body controllers that are both robust (achievable via RL) and precise (difficult to add new feedback objectives to pretrained RL policies without retraining). The authors aim to decouple robustness from precision by combining RL with an online optimization-based controller.

Method: The authors propose an RL-guided MPPI framework where a pretrained RL policy serves as a sampling prior for MPPI. MPPI generates rollouts biased by the RL prior and uses modular cost terms to specify task objectives, then continuously corrects the RL prior online to satisfy these objectives—all without retraining the RL policy.

Result: Simulations on a 29-DoF Unitree G1 humanoid in MuJoCo achieve stable high-rate control (average 280 Hz). The method improves task-level precision over a pure RL baseline under the same command interface, correcting systematic drift during straight walking and tracking additional whole-body reference signals.

Conclusion: The RL-guided MPPI framework effectively combines the robustness of RL with the precision and flexibility of online optimization (MPPI), enabling task-level improvements without retraining the policy. This modular approach allows adding new feedback objectives through cost terms, enhancing controller adaptability for contact-rich humanoid locomotion.
...
TL;DR: This paper extends Parametric-CBF to multi-robot interactions using causality inference to model inter-vehicle influence, enabling adaptive safety-critical control that improves task efficiency without being overly conservative.


<details>
  <summary>Details</summary>
Motivation: Existing safety-critical control methods for autonomous driving either struggle to model inter-vehicle influence in multi-vehicle interactions or become overly conservative due to worst-case analysis.

Method: Extends Parametric-Control Barrier Function (Parametric-CBF) with embedded causality inference to explicitly reason over inter-vehicle influence, then uses the learned Causality-based Parametric-CBF in an adaptive safety-critical controller.

Result: The approach improves task efficiency in various interaction-intensive scenarios by leveraging motion flexibility among multi-vehicle systems.

Conclusion: Causality-based Parametric-CBF enables safer and less conservative multi-vehicle interaction control compared to prior methods.
...
TL;DR: HALO is a visuomotor policy with attention-based memory retrieval that addresses spurious correlations and error accumulation in long-horizon robot control by using VLM-distilled question-answering and sparse attention over up to 8 minutes of history.


<details>
  <summary>Details</summary>
Motivation: General-purpose robots in homes need versatile memory retrieval from past experience, but existing methods face spurious correlations in imitation learning and error accumulation during closed-loop control.

Method: HALO uses (1) VLM-distilled question-answering objectives to steer attention toward task-relevant information, and (2) sparse attention mechanisms restricting retrieval to the most relevant history parts.

Result: The system achieves reliable long-horizon control by retrieving task-relevant information from up to 8 minutes of past experience.

Conclusion: Combining VLM priors with sparse attention enables more robust memory-guided policy learning for long-horizon robot tasks.
...
### [29] [1000 Rallies: An Event-Camera Dataset and Real-Time Learned Ball-State Estimation for Robotic Table Tennis](https://arxiv.org/abs/2606.25620)
*Raphaela Kreiser,Asude Aydin,Yin Bi,Claudio Fanconi,Peter Dürr,Naoya Takahashi*

Main category: cs.RO

TL;DR: This paper introduces the first large-scale event-camera dataset for table tennis (1000+ rallies) and demonstrates that event-based ball state estimation, combined with a CNN and Kalman filter, reduces bounce-point prediction error by 36% and enables real-time human-robot table tennis rallies.


<details>
  <summary>Details</summary>
Motivation: Traditional frame-based cameras face a trade-off between temporal blind spots (low frame rates) and high computational cost (high frame rates). Event cameras offer microsecond temporal resolution and no motion blur, but the community lacks large-scale datasets for event-based perception in realistic sports scenarios.

Method: Collected a dataset of 1000+ rallies with event streams and 14 synchronized 200 FPS cameras (producing 1 kHz pseudo ground-truth). Trained a CNN robust to background motion to jointly estimate ball position and velocity from events. Used predicted velocity as an additional measurement in a Kalman filter to improve trajectory prediction.

Result: Bounce-point prediction error reduced by 36% compared to a position-only baseline. Successfully closed the perception-action loop by integrating with a Stäubli robotic arm for the first real-time human-robot table tennis rallies driven by event-based perception.

Conclusion: Event cameras are a promising alternative for high-speed robotic perception in sports. The proposed method and dataset enable accurate, low-latency ball state estimation and real-time human-robot interaction in table tennis.
...
### [30] [SwarmFly: A simulation platform for UAV swarm experiment design and validation](https://arxiv.org/abs/2606.25146)
*Abhishek Phadke,Karthik Kumar Vasudeva,Abhishek Joshi*

Main category: cs.RO

TL;DR: SwarmFly is a MATLAB-based, modular simulation platform for multi-UAV swarms featuring real-time operational maps, four coordination modes, plugin architecture, and eight plugins, validated through eight experiments measuring formation accuracy, wind tolerance, fault recovery, energy endurance, and airspace compliance.


<details>
  <summary>Details</summary>
Motivation: Existing open-source UAV swarm simulators are often unmaintained, have steep learning curves, or are limited to single fixed scenarios, creating a recognized research gap for a comprehensive, modular simulation platform.

Method: The authors developed SwarmFly, a MATLAB-based platform combining a real-time operational map, four swarm coordination modes (leader-follower, decentralized, heterogeneous relay, heterogeneous speed), simulated IMU telemetry, IP-based geolocation, and a plugin architecture allowing researchers to add behaviors, fault models, and analysis tools. Eight bundled plugins extend the base simulator.

Result: Eight experiments verified and characterized each subsystem, measuring formation accuracy, wind tolerance, fault recovery, energy endurance, and airspace compliance. The platform runs entirely in MATLAB and is open-source.

Conclusion: SwarmFly's modular design supports straightforward extension toward hardware-in-the-loop testing, larger swarms, and higher-fidelity dynamics, addressing the research gap in UAV swarm simulation tools.
...
### [31] [Toward Low-Latency Vision-Language Models with Doubly-Correct Predictions in Egocentric Visual Understanding](https://arxiv.org/abs/2606.25160)
*Qitong Wang,Fan Du,Pranav Maneriker,Jihui Jin,Christopher Rasmussen*

Main category: cs.RO

TL;DR: This paper proposes a rationale-informed pruning strategy for Vision-Language Models that preserves both prediction accuracy and evidence localization (doubly-correct predictions) in human-robot collaboration tasks, outperforming existing pruning methods on egocentric video benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing VLM pruning methods often preserve correct evidence localization but undermine correct prediction, failing to achieve doubly-correct predictions (both accurate and evidentially grounded) necessary for safe human-robot interaction.

Method: The authors propose a rationale-informed pruning strategy that better aligns evidence with decisions, specifically designed to preserve doubly-correct predictions in VLMs for egocentric visual understanding in HRC tasks.

Result: On egocentric video datasets, the proposed method achieves the highest prediction accuracy and outperforms existing approaches in attaining doubly-correct predictions.

Conclusion: Rationale-informed pruning is effective for maintaining both accuracy and evidential grounding in VLMs, promoting transparency, auditability, and safety for responsible human-robot interaction and embodied intelligence.
...
### [32] [fARfetch: Enabling Collocated AR-HRC in Large Visually Diverse Environments with VLM-Driven AR Content Adaptation](https://arxiv.org/abs/2606.25162)
*Christian Fronk,Hanting Ye,David Hunt,Miroslav Pajic,Maria Gorlatova*

Main category: cs.RO

TL;DR: fARfetch is an AR-HRC system for large outdoor environments that uses shared semantic mapping, world-in-miniature path authoring, and VLM-driven adaptive view management to improve robot collaboration, showing 66% faster task completion and significantly reduced workload vs. non-AR baseline in a 30.5m outdoor study (N=13).


<details>
  <summary>Details</summary>
Motivation: AR improves collocated human-robot collaboration by visualizing robot state/intent, but large visually diverse outdoor environments (long distances, beyond line of sight) challenge interaction and content legibility.

Method: fARfetch integrates (i) shared semantic environment mapping for landmark-grounded go-to commands, (ii) context-aware world-in-miniature for path authoring, and (iii) VLM-driven AR view management that jointly adapts virtual content color, size, and orientation for legibility. Implemented with Meta Quest 3 and Unitree Go2 robot; evaluated via within-subjects study (N=13) on a 30.5m outdoor inspection task.

Result: 66% faster completion time than non-AR baseline; significantly lower workload: mental demand (-43%), temporal demand (-34%), frustration (-66%). Legibility survey confirmed effectiveness in large outdoor environments.

Conclusion: fARfetch demonstrates that combining shared semantic mapping, world-in-miniature, and adaptive VLM-driven view management effectively enables AR-HRC in large outdoor environments, improving task efficiency and reducing user workload.
...
### [33] [Learning Perceptive Platform Adaptive Locomotion Controllers for Quadrupedal Robots](https://arxiv.org/abs/2606.25179)
*David Rytz,Kim Tien Ly,Ioannis Havoutis*

Main category: cs.RO

TL;DR: This paper investigates how to integrate perception into morphology-aware reinforcement learning for quadrupedal locomotion, proposing and comparing blind, critic-perceptive (MorAL+), and fully perceptive (PPAL) policies across multiple robot morphologies. Results show critic-only perception (MorAL+) offers the best trade-off between robustness and terrain awareness.


<details>
  <summary>Details</summary>
Motivation: Universal quadrupedal locomotion is limited by poor cross-embodiment generalization, as existing controllers rely on single-robot training or blind policies that omit real-time perception. Fully perceptive policies are sensitive to noise, while blind controllers lack terrain awareness.

Method: Building on MorAL, the authors train morphology-specialized universal controllers on multiple reference quadrupeds using adaptive terrain curricula. They compare three variants: a blind baseline, a critic-perceptive variant (MorAL+), and a fully perceptive actor-critic (PPAL).

Result: Critic-only perception (MorAL+) improves robustness and tracking consistency over blind baselines while remaining more stable than fully perceptive policies under perception noise. Policies were evaluated in simulation on flat/rough terrains and deployed on ANYmal hardware.

Conclusion: Perception placement and curriculum design are key factors for scalable, morphology-aware locomotion. Critic-only perception offers the best trade-off for deployable quadrupedal control.
...
### [34] [RAVEN: Long-Horizon Reasoning & Navigation with a Visuo-Spatio-Temporal Memory](https://arxiv.org/abs/2606.25206)
*Yixun Hu,Zhicheng Zheng,Lihan Zha,Chunwei Xing,Rajdeep Singh,Omar Hossain,Antonio Loquercio,Dhruv Shah*

Main category: cs.RO

TL;DR: RAVEN is an agentic memory system for long-horizon robotic QA and navigation that stores visual embeddings with pose/time in a vector database and grounds retrieval in a spatial map, outperforming caption-based systems at 10x lower cost.


<details>
  <summary>Details</summary>
Motivation: Long-term robot deployment requires a compact, scalable memory that preserves fine-grained visual semantics, grounds observations spatially and temporally, and enables efficient storage and retrieval without lossy image-to-text captioning.

Method: RAVEN stores visual embeddings with pose and time in a vector database, grounds retrieval in a spatial map, and operates directly on visual embeddings to answer queries and navigate to goals.

Result: RAVEN consistently surpasses caption-based memory systems and matches frontier VLMs on long-horizon tasks at 10x lower retrieval cost, with successful real-world deployment on a Unitree Go1 robot.

Conclusion: RAVEN offers an efficient, scalable, and accurate alternative to caption-based memory for long-horizon robotic question answering and navigation, validated in both simulated and real-world environments.
...
### [35] [Wear-Clearance-Impact Coupling in the Jansen Linkage: A Gait-Durability-Optimized Design Slows Joint Loosening](https://arxiv.org/abs/2606.25208)
*Jichao Wang*

Main category: cs.RO

TL;DR: This paper extends a prior study of the Theo Jansen walking linkage by replacing ideal, clearance-free revolute joints with realistic clearance joints using a forward-dynamic model with Lankarani-Flores contact, Ambrosio friction, and Archard wear. Key findings: (1) clearance models show ~2x higher peak forces than ideal joints, up to ~426 N with multiple clearance joints; (2) the system is chaos-sensitive, requiring statistical comparisons over ensembles—optimized designs show 1.7-9x lower wear; (3) wear is highly non-uniform (concentrated on a ~10° arc), and assuming uniform clearance growth underestimates local growth by ~36x. The optimized design's durability advantage holds in the ensemble mean despite chaotic, multi-joint, non-uniform coupling.


<details>
  <summary>Details</summary>
Motivation: To relax the idealization of clearance-free revolute joints in prior Jansen linkage durability studies and develop a more realistic model that accounts for wear, impact, and clearance feedback, validating whether the optimized "holy numbers" design retains its durability advantage under real-world joint degradation.

Method: Forward-dynamic model of the Jansen leg with revolute clearance joints using Lankarani-Flores hysteresis-damped normal contact, Ambrosio friction, and constraint-stabilized differential-algebraic equations, coupled with Archard wear law in a wear→clearance→impact feedback loop. Statistical ensemble analysis over 16 randomized phases.

Result: Peak contact force ~104 N vs ~48 N for ideal joints (~2x amplification), rising to ~426 N with two clearance joints. Wear is non-uniform (~10° arc), uniform clearance assumption underestimates local growth by ~36x. Optimized design has 1.7-9x lower wear (p<0.01) in ensemble mean.

Conclusion: The clearance-free durability advantage of the optimized Jansen linkage design survives under chaotic, multi-joint, non-uniformly-worn conditions in the ensemble mean. The paper delivers the first clearance-coupled forward-dynamic Jansen leg model and a falsifiable protocol for testing predictions.
...
### [36] [RigPI: Dynamic Parameter Identification of Rigid Body via VLM-Seeded Differentiable Simulation](https://arxiv.org/abs/2606.25212)
*Xincheng He,Rongrong Zhang,Wei Jiang,Wenqiang Xu*

Main category: cs.RO

TL;DR: RigPI is a differentiable simulation framework that combines vision-language priors with gradient-based optimization to accurately identify inertial and frictional parameters of rigid bodies during robot-object interaction.


<details>
  <summary>Details</summary>
Motivation: Accurate physical parameter identification (inertial and frictional properties) is essential for robotic manipulation and digital twins, but remains challenging due to sensing noise, modeling errors, and limited prior knowledge.

Method: RigPI integrates vision-language model (VLM) priors for informed initialization and constrained search space with differentiable physics simulation for gradient-based parameter refinement, using a two-stage optimization strategy to handle noise and avoid implausible solutions.

Result: Real-world experiments on objects with revolute and prismatic joints show accurate and stable parameter estimates, and successful reproduction of manipulation trajectories with predictive validity.

Conclusion: RigPI is an effective and robust framework for real-world robotic system identification tasks.
...
### [37] [Swazure: Swarm Measurement of Pose for Flying Light Specks](https://arxiv.org/abs/2606.25222)
*Hamed Alimohammadzadeh,Shahram Ghandeharizadeh*

Main category: cs.RO

TL;DR: Swazure is a novel technique enabling Flying Light Specks (miniature drones with lights) to track each other's pose even when outside their sensors' optimal range, using cooperation and physical data independence, with heuristics to handle obstructions from larger FLS sizes.


<details>
  <summary>Details</summary>
Motivation: FLSs need to accurately measure relative pose using sensors like cameras, but these sensors have limited sweet ranges. When point cloud data requires FLSs to operate outside this sweet range, sensor data is missing—posing a key challenge for 3D multimedia displays.

Method: Swazure employs cooperation among FLSs to fill missing sensor data, implementing physical data independence to abstract sensor hardware characteristics. Two heuristics (Move Obstructing and Move Source) address obstructions caused by larger FLS sizes.

Result: With medium-sized FLSs, Swazure positions 100% of neighbors. The Move Obstructing heuristic resolves ~30% of obstructions in worst-case scenarios, outperforming Move Source.

Conclusion: Swazure effectively addresses the sweet range limitation via cooperative sensing, with size-dependent performance. Move Obstructing is the recommended heuristic for handling obstructions.
...
TL;DR: The paper proposes an imitation learning model with a temporal reasoning module that uses nearest neighbor retrieval and cross-attention to enable adaptive coaching for complex motor tasks, validated on both semi-synthetic and real-world driving datasets.


<details>
  <summary>Details</summary>
Motivation: Existing automated coaching systems lack adaptability because they rely only on local, context-dependent reasoning and fail to account for the long-term temporal nature of student learning and cumulative impact of repeated teacher-student interactions.

Method: An imitation learning model with a temporal reasoning module using nearest neighbor retrieval and cross-attention prior, combined with an encoder-decoder based concurrent teaching model, designed for low-data regimes.

Result: The adaptive teaching model with nearest neighbor retrieval and cross-attention prior consistently outperforms non-adaptive baselines and other adaptive models with different priors and temporal fusion mechanisms.

Conclusion: Incorporating temporal reasoning over interaction history via retrieval-based mechanisms significantly improves adaptive coaching performance in complex motor tasks.
...
TL;DR: GRAFT is a geometry-aware framework for zero-shot robotic manipulation transfer that uses part-based graphs, requiring only one demonstration per object, to retrieve similar objects and propagate contact points via part-level and vertex-level descriptors.


<details>
  <summary>Details</summary>
Motivation: Generalizing robotic manipulation to unseen objects is challenging because learning-based approaches require many demonstrations and fail in few-shot settings, and prior work using semantic retrieval neglects geometric similarity which is critical for manipulation.

Method: Objects are represented as part-based graphs with part-level descriptors for global instance retrieval and part correspondence, and vertex-level descriptors for fine-grained contact point matching. For unseen objects, the method retrieves the most functionally and geometrically similar instance from a demonstration buffer and propagates contact points through point-wise correspondence.

Result: Zero-shot manipulation transfer using only one demonstration per object, enabling generalization to unseen objects.

Conclusion: GRAFT achieves zero-shot manipulation transfer by leveraging geometry-aware correspondences through part-based graph representations, outperforming semantic-only retrieval approaches.
...
TL;DR: A hardware-software integrated optoelectronic architecture using DMD-based optical convolutional layers, compressed sensing, and CLIP-guided attention to achieve low-data, annotation-efficient defect detection in robotic visual inspection.


<details>
  <summary>Details</summary>
Motivation: To address data overload and inefficient manual shape-level annotation in robotic visual inspection, particularly for transparent material defect detection.

Method: (1) DMD-based physical optical convolutional layer for photonic-domain feature extraction; (2) Block-based compressed sensing to reduce data redundancy; (3) CLIP-based natural language guidance for defect shape localization without manual annotation; (4) LAA metric for quantifying localization accuracy.

Result: Maintains equivalent accuracy while reducing data volume by 90% (Vision Transformers) and computational workload by 60% (CNNs), validated on transparent material defect detection.

Conclusion: The low-data paradigm offers an efficient solution for industrial automation with massive data streams, high acquisition costs, or constrained edge resources.
...
TL;DR: This paper proposes a dynamic mobile manipulation framework that couples an anchor-based diffusion model for short-horizon grasp trajectory prediction with a whole-body reinforcement learning policy, achieving strong performance in both simulated and real-world dynamic grasping tasks.
...
Method: The framework uses an anchor-based diffusion model to generate temporally consistent short-horizon grasp trajectories conditioned on historical observations. These trajectories are encoded as compact features and fed to a whole-body RL policy. An anticipation-guided reward is introduced to equip the policy with an anticipatory grasping horizon by adaptively shifting the target from current observation to predicted trajectory.

Result: Strong performance in mobile manipulation of dynamic objects across diverse settings and grasping metrics in Isaac Gym simulation, with demonstrated generalizability in real-world experiments.

Conclusion: The proposed method effectively couples grasp trajectory prediction with whole-body control for dynamic mobile manipulation, showing promising results and generalization capability.
...
TL;DR: This paper introduces a low-cost passive wheeled-legged robot with standard casters, using an asymmetric actor-critic control method and posture adjustment strategy to achieve omnidirectional mobility, reducing transport cost by up to 89.1% compared to legged motion.


<details>
  <summary>Details</summary>
Motivation: Conventional actuated wheeled robots require specialized hardware and electrical design; the authors propose a simpler, low-cost alternative using passive casters.

Method: Uses an asymmetric actor-critic structure (leveraging privileged caster angle/velocity info), a caster base posture adjustment strategy based on velocity commands, and multiple propulsion modes to convert caster twisting into propulsive force.

Result: Slalom test and mode switch experiment show omnidirectional movement capability and up to 89.1% reduction in cost of transport (COT) versus legged motion.

Conclusion: Passive wheeled-legged robots can achieve versatile omnidirectional mobility with significantly higher efficiency, offering a low-cost alternative for long-distance transport.
...
### [43] [AI Coaching for Accelerating Human Skill Development with Reinforcement Learning](https://arxiv.org/abs/2606.25337)
*Wei Wang,Enlin Gu,Antonio Loquercio,Haimin Hu,Rahul Mangharam*

Main category: cs.RO

TL;DR: This paper proposes a reinforcement learning framework for embodied AI coaching that strategically balances assistance and autonomy to accelerate human motor-skill development, validated through a drone racing user study showing significant learning improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: AI copilots can boost performance but cause over-reliance and skill atrophy. The authors argue for strategic coaching that aligns assistance with learner capability and allows productive failures for effective skill development.

Method: Formalizes AI coaching as a non-cooperative dynamic game; develops an RL framework combining adaptive shared control with probabilistic models of causal influence on skill evolution.

Result: User study (N=33) on first-person-view drone racing shows significant gains in human learning outcomes over state-of-the-art AI coaching baselines.

Conclusion: Strategic scaffolding and stepping back, aligned with learner capability, can accelerate motor-skill development without inducing over-reliance.
...
### [44] [Self Capacitive Tactile Sensor System designed for Companion Robots](https://arxiv.org/abs/2606.25348)
*Mohsin Ali,Hidenobu Sumioka,Shuhei Ikemoto*

Main category: cs.RO

TL;DR: This paper presents a simple, scalable tactile sensing system for humanoid robots using self-capacitance with a single conductive fabric layer. A 100-point sensor array was fabricated on FPC, and tests showed 100Hz+ sampling reliably distinguishes touch types. A decision-tree classifier on FPGA offloads inference from the main processor, meeting full-body sensing needs for the HIRO-chan robot.


<details>
  <summary>Details</summary>
Motivation: Tactile sensing is critical for humanoid robots, but existing designs suffer from complex multilayer structures, intricate wiring, high cost, and poor scalability, making full-body sensing with real-time low-latency detection difficult to achieve.

Method: The system uses self-capacitance sensing with a single conductive fabric layer and conductive fabric wire architecture, avoiding intricate electrode patterning. A 100-point sensor array was fabricated on FPC. A decision-tree classifier runs directly on FPGA for real-time inference offloading.

Result: Evaluation across sampling frequencies showed 10Hz is insufficient, while 100Hz and 1000Hz reliably capture all interaction types (gentle touch, slow tapping, fast tapping, hitting). The FPGA-based classifier adds minimal latency and negligible power overhead.

Conclusion: The design fully meets the tactile sensing requirements of the HIRO-chan robot and is well-suited for full-body tactile sensing in companion robots, offering simplicity, scalability, and hardware-friendliness.
...
### [45] [Decoupling Semantics and Geometric Grounding: Spatial Visual Prompts for Language-Conditioned Imitation Learning](https://arxiv.org/abs/2606.25360)
*Yanzhe Tang,Xinyu Shao,Yuxuan Hu,Siyu Chen,Bowen Yang,Yajun Gao,Tongtong Cao,Xiu Li,Long Zeng*

Main category: cs.RO

TL;DR: SVP-IL decouples spatial grounding from action generation in VLA models, using vision-language foundation models to create zero-shot spatial visual prompts that guide action generation via lightweight feature fusion, achieving significant improvements in low-data regimes (50-100 demos) for robotic manipulation.


<details>
  <summary>Details</summary>
Motivation: End-to-end VLA models couple semantic reasoning and spatial control, creating an alignment bottleneck that limits precise target disambiguation in data-constrained imitation learning.

Method: Decoupled architecture: (1) Extract spatial visual prompts (SVP) from instructions via vision-language foundation models as zero-shot geometric masks; (2) Inject SVP priors into a continuous action generator through lightweight direct feature-level fusion for uncorrupted spatial gradient guidance.

Result: Outperforms SOTA VLAs and visuomotor baselines; trained on 50-100 demos, improves success rates from 24.0% to 39.5% on ambiguous tasks, achieves 67.8% on standard benchmarks; validated in real-world unstructured environments.

Conclusion: Decoupling spatial grounding from action generation is an effective strategy for improving data efficiency and robustness in vision-language-action models, particularly under low-data imitation learning settings.
...
### [46] [Reliability-Asymmetric Spacecraft Autonomy: Co-Designing a Capable Learned GNC Stack with a Verified, Adaptation-Aware Runtime Shield](https://arxiv.org/abs/2606.25366)
*Alireza Shojaei*

Main category: cs.RO

TL;DR: AMPLE-GNC is a three-tier autonomy stack for deep-space missions combining a foundation-model commander (natural language to PDDL+), constraint-screening verifier, fault-adaptive controller (Rapid Motor Adaptation recovering 97.8% actuator faults), and a runtime shield with machine-checked LTL invariants. Tested on 6-DOF Basilisk, it achieves 84% executable actions, strong fault recovery, and 94.5% autonomy via adaptive safe-hold engagement.


<details>
  <summary>Details</summary>
Motivation: Deep-space missions need onboard autonomy that is both capable (like learned approaches) and certifiable (like rule-based systems). Existing solutions trade off between brittleness and unverifiability.

Method: Three-tier GNC stack: (1) small foundation-model commander with grammar-constrained decoding for natural language to PDDL+, (2) constraint-screening verifier, (3) fault-adaptive controller using Rapid Motor Adaptation. All bounded by a runtime shield with 9 LTL invariants verified by Kind 2 model checker. Split-conformal recovery-deadline certificate for safe-hold engagement.

Result: Fine-tuned 360M model achieves 84% executable actions; fault-adaptive controller recovers 97.8% actuator-sign faults and 94.4% continuous-gain faults; split-conformal safe-hold achieves 94.5% autonomy; randomization breadth (not data volume) drives generalization; robustness flat under star-tracker noise to 0.005.

Conclusion: AMPLE-GNC demonstrates that learned autonomy can be made certifiable through runtime shielding and formal verification, achieving high fault recovery and autonomy rates while maintaining safety guarantees for deep-space missions.
...
### [47] [Commerge: Communication-Efficient, Robust, and Fast LiDAR Map Merging Framework for Multi-Robot Coordination in Resource-Constrained Scenarios](https://arxiv.org/abs/2606.25386)
*Hogyun Kim,Jiwon Choi,Juwon Kim,Geonmo Yang,Seokhwan Jeong,Hyungtae Lim,Younggun Cho*

Main category: cs.RO

TL;DR: Commerge is a communication-efficient multi-robot LiDAR map merging framework that selects only a small subset of keyframes via graph-theoretic optimization, achieving up to 5,000x bandwidth reduction (99.98% data reduction) while maintaining alignment accuracy.


<details>
  <summary>Details</summary>
Motivation: Multi-robot LiDAR map merging requires massive sensor data exchange between server and robots, making communication a bottleneck in bandwidth-constrained environments. Existing approaches either transmit whole scans (GB-scale) or use naive downsampling, both inefficient.

Method: Formulates scan selection as a three-stage cascaded optimization on an exchange graph (vertices=keyframes, edges=candidate loops). The three stages select: (1) sequentially overlapped scans, (2) balanced-transmission-cost subsets, and (3) geometrically-perceptually optimal subsets to preserve alignment quality while minimizing communication.

Result: Up to 5,000x reduction in inter-robot communication (99.98% data reduction, e.g., 7,000MB to 1.3MB on HeLiPR dataset) while maintaining alignment accuracy. Evaluated on five public and four in-house datasets across cave, planetary-analog, indoor, and outdoor environments.

Conclusion: Graph-theoretic selective data exchange enables MB-scale map merging with alignment accuracy comparable to GB-scale full-data approaches, making multi-robot SLAM practical in communication-constrained settings.
...
### [48] [Large-Scale Tunnel Air--Ground Collaboration With FLISP: Fast LiDAR-IMU Synchronized Path Planne](https://arxiv.org/abs/2606.25393)
*Fenghe Guo,Runjie Shen,Chenyang Sun,Junrui Zhang,Quanxi Zhan,Yongchun Wang,Junjie Zhang*

Main category: cs.RO

TL;DR: FLISP introduces a mapless planning framework for cooperative UGV-UAV inspection of hydropower tunnels, using a single LiDAR-IMU suite to synchronously plan paths for both robots. It achieves 100% success rate with ~7 ms latency, outperforming traditional map-based methods with massive speedups.


<details>
  <summary>Details</summary>
Motivation: Hydropower tunnel inspection using manual methods is inefficient and hazardous. Existing map-based robotic approaches suffer from structural bottlenecks like rasterization overhead and sampling instability.

Method: FLISP uses (1) a single UGV-mounted LiDAR-IMU suite for synchronized path generation, (2) an enhanced Firefly Algorithm for UGV obstacle avoidance and a dynamic iterative optimizer for UAV flight, and (3) hierarchical refinement ensuring kinematic feasibility without state estimation drift.

Result: In a 1.2 km operational tunnel, FLISP achieves 100% success rate with ~7 ms latency, 7× speedup over grid-based (Fast-LIO2 + A*) and three-orders-of-magnitude improvement over sampling-based (LIO-SAM + RRT*) baselines.

Conclusion: FLISP offers a scalable, mapless solution for robotic inspection in feature-degraded linear infrastructure, validated in real operational hydropower tunnels.
...
TL;DR: MAPL uses LLMs to generate multi-objective preference comparisons for robot locomotion, training policies without manual reward engineering, achieving performance comparable to expert-designed rewards.


<details>
  <summary>Details</summary>
Motivation: Reward design is a major bottleneck in RL for robot locomotion, requiring carefully tuned task-specific functions. Existing LLM-based preference methods capture only single overall judgments, failing to handle multiple competing objectives.

Method: MAPL prompts an LLM to compare trajectories along semantically meaningful, terrain-invariant criteria, trains a multi-head preference scoring model from these objective-wise preferences, and aggregates scores into a scalar reward for policy optimization.

Result: Across four quadruped locomotion environments, MAPL trains policies using only LLM-generated preferences, achieving performance comparable to or better than expert-designed rewards.

Conclusion: MAPL eliminates task-specific reward engineering while producing high-quality locomotion policies, demonstrating that LLM-informed multi-objective preferences can effectively replace manual reward design.
...
### [50] [HEART: Coordination of Heterogeneous Expert Agents for Physically Grounded Robotic Task Planning](https://arxiv.org/abs/2606.25404)
*Junho Lee,Seabin Lee,Wonjong Lee,Nayoung Kim,Moonjeong Kang,Changjoo Nam*

Main category: cs.RO

TL;DR: HEART is a heterogeneous multi-LLM framework that decomposes instructions into atomic reasoning tasks for role-specialized agents, enabling physically executable robotic task plans under token and resource constraints.


<details>
  <summary>Details</summary>
Motivation: LLMs fail to satisfy physical/spatial constraints for robotic task planning, producing invalid or incomplete plans due to lack of structured reasoning about feasibility, reachability, and logical order.

Method: A heterogeneous multi-LLM framework that decomposes instructions into atomic reasoning tasks, assigns them to role-specialized expert agents under token budgets, then performs constraint-driven plan synthesis after validating capability, reachability, and constraints.

Result: HEART consistently improves plan success compared to single-LLM and rule-based planners across different household benchmarks.

Conclusion: Heterogeneous LLM collaboration enables robust and scalable robotic task planning under resource constraints.
...
### [51] [Delta-Position Estimation-Based IMU Odometry: A Comparison of MLP and Kolmogorov-Arnold Networks](https://arxiv.org/abs/2606.25454)
*Osman Tokluoglu,Emin Keresteci*

Main category: cs.RO

TL;DR: This study compares MLP and KAN for learning-based inertial odometry using IMU data. KAN, despite having 6.9x fewer parameters, achieves 44% lower cumulative drift (9.61m vs 17.23m) and more stable long-term error accumulation.


<details>
  <summary>Details</summary>
Motivation: To investigate learning-based inertial odometry using raw IMU measurements, and to compare standard MLP with KAN using learnable B-spline activations for this task.

Method: Models are trained to estimate incremental displacement (Δp) over a fixed 50 ms sliding window, with full trajectory reconstructed via numerical integration. MLP is compared against KAN with learnable B-spline activations.

Result: KAN (8,444 parameters) achieves 44% lower final cumulative drift (9.61m) than MLP (57,859 parameters, 17.23m drift), with more stable long-term error accumulation and lower P_50/P_90 drift values.

Conclusion: Learnable B-spline-based activations in KAN have the potential to reduce error accumulation in the inertial odometry problem compared to standard MLP architectures.
...
### [52] [Generative AI for Safe and Photorealistic Drone Light Shows](https://arxiv.org/abs/2606.25458)
*Pascal Reinhold,Alexander Gräfe,Sebastian Trimpe*

Main category: cs.RO

TL;DR: SWAN is an end-to-end pipeline that generates photorealistic, collision-free drone choreographies from text prompts, demonstrated on up to 2,000 simulated drones and 49 real quadcopters using consumer hardware.
...
Method: SWAN converts text prompts to realistic reference videos, translates pixel dynamics to swarm kinematics via an adaptive point-tracking algorithm that maintains spatial coherence through occlusions and topological shifts, allocates trajectories to drones, and applies a safety filter for collision-free execution.

Result: Scalability demonstrated with 2,000-drone simulated formations and physical feasibility validated on a real swarm of 49 quadcopters, all running on standard consumer hardware.

Conclusion: Generative AI can effectively automate multi-robot choreography design, offering an accessible framework for drone light shows.
...
### [53] [SAGE-Nav: Leveraging LLM Planning and Alignment Fusion for Hierarchical Scene Graph-Guided Navigation](https://arxiv.org/abs/2606.25497)
*Hao Su,Yuehao Huang,Yukai Ma,Yong Liu,Jiajun Lv*

Main category: cs.RO

TL;DR: SAGE-Nav is a hierarchical LLM-driven framework for Object-Goal Navigation that decouples global semantic planning from reactive control, using a Hierarchical Scene Graph Encoder (HSGE) and Goal-aware Alignment-Fusion Network (GAFN) to achieve state-of-the-art results in i-THOR and RoboTHOR environments.


<details>
  <summary>Details</summary>
Motivation: Existing monolithic ObjNav methods struggle with long-horizon reasoning and generalize poorly to novel environments, motivating the need for a hierarchical approach with better planning and generalization capabilities.

Method: Proposes SAGE-Nav with: (1) LLM as global planner decomposing instructions into semantic waypoints; (2) Hierarchical Scene Graph Encoder (HSGE) using relational graph convolutions for structure-aware embeddings; (3) Goal-aware Alignment-Fusion Network (GAFN) with adaptive gating for dynamic fusion of perception and structural priors.

Result: Achieves state-of-the-art performance in i-THOR and RoboTHOR environments, with substantial gains in navigation efficiency and zero-shot generalization while maintaining low control latency suitable for physical robotic deployment.

Conclusion: The hierarchical decoupling of global semantic planning from reactive control, combined with graph-structured scene encoding and adaptive fusion, effectively addresses long-horizon reasoning and generalization challenges in ObjNav.
...
### [54] [AISPO: Enhancing Depth Reliability for Robotic Manipulation of Non-Lambertian Objects via Affine-Invariant Shape Prior](https://arxiv.org/abs/2606.25503)
*Zhiming Chen,Linfang Zheng,Kun Zhang,Hyung Jin Chang,Wei Zhang,Hongyu Yu,Hua Chen*

Main category: cs.RO

TL;DR: AISPO is a depth completion framework combining multi-scale RGB-D feature fusion with an affine-invariant shape prior to improve depth reliability for robotic manipulation of non-Lambertian objects (transparent/specular surfaces), enhancing physical plausibility and structural integrity, with strong benchmark performance and improved real-world grasping success.


<details>
  <summary>Details</summary>
Motivation: Reliable depth perception is critical for robotic manipulation, but non-Lambertian objects (transparent/highly specular) cause corrupted or missing depth measurements, which propagate to motion planning failures and invalid grasp poses.

Method: AISPO combines multi-scale RGB-D feature fusion with an affine-invariant shape prior to enforce geometric consistency and mitigate catastrophic depth failures.

Result: Competitive benchmark performance with strong generalization to unseen objects and novel scenes; real-world grasping experiments show significantly improved manipulation success rates, especially for transparent objects.

Conclusion: Enhancing depth reliability through physically plausible and structurally consistent depth completion significantly improves robotic manipulation success in challenging sensing conditions.
...
### [55] [GROVE: Grounded Pedestrian Simulation via Natural Language for Interactive Social Robot Navigation](https://arxiv.org/abs/2606.25504)
*Duc Tai Nguyen,Volodymyr Shcherbyna,Anh Do Duc,Zhengcheng Shen,Teham Buiyan,Linh Kästner*

Main category: cs.RO

TL;DR: GROVE is a text-to-scenario pedestrian simulation framework that generates realistic, socially challenging scenarios for social robot navigation by combining multiple SotA modules (long-horizon human behavior, medium-horizon navigation, short-horizon interactions) tuned via user prompts, integrated into Isaac Sim, Gazebo, and RViz.


<details>
  <summary>Details</summary>
Motivation: Pedestrian simulation for social robot navigation is rigid and requires manual data generation for even simple scenarios, lacking flexibility and adaptability.

Method: GROVE combines three modules (long-horizon human behavior, medium-horizon pedestrian navigation, short-horizon robot/social interactions) dynamically selected via prompts, with presets (emergency, queuing, normal) and free-form text input, integrated into common simulators.

Result: Qualitative validation across residential, hospital, and office environments shows GROVE produces high-fidelity pedestrian simulations that challenge social robot navigation with complex, diverse, realistic human behaviors.

Conclusion: GROVE narrows the sim2real gap by capturing situational nuances of pedestrian behavior through dynamic module selection, enabling customizable and realistic simulation scenarios for social robot navigation.
...
### [56] [ASSCG: Just-Right Gating over Chattering for Fast-Slow LLM Planning in Autonomous Driving](https://arxiv.org/abs/2606.25509)
*Sining Ang,Yuan Chen,Liu Haiyan,Xuanyao Mao,Jason Bao,Xuliang,Bingchuan Sun,Yan Wang*

Main category: cs.RO

TL;DR: Proposes ASSCG, an adaptive gate that decides when to query a slow LLM planner vs. cache or drop guidance, improving autonomous driving performance while reducing latency by 60%.


<details>
  <summary>Details</summary>
Motivation: LLMs improve driving planning but are costly to query; existing fast-slow planners use rigid hand-designed rules that either over-call the slow system or call it at suboptimal times.

Method: Formulates slow-system invocation as a resource-aware sequential decision problem. ASSCG uses an RWKV backbone for efficient long-horizon gating and is trained with supervised fine-tuning + GRPO-style compute-aware reinforcement fine-tuning, making frame-level Query/Cache/Drop decisions.

Result: On nuPlan Hard20 (AsyncDriver): score 67.28 (+2.28), 60% latency reduction. On NAVSIM (RecogDrive-based): 91.4 PDMS (+0.6), 25% speed increase.

Conclusion: ASSCG effectively replaces hand-designed triggering rules with a learned gating policy, improving both performance and efficiency in fast-slow autonomous driving planners.
...
### [57] [One Body, Two Minds: Variable Autonomy Approach for a Co-embodied Robotic Hand](https://arxiv.org/abs/2606.25575)
*Piotr Koczy,Yuchong Zhang,Danica Kragic,Michael C. Welle*

Main category: cs.RO

TL;DR: This paper presents a co-embodied wearable robotic hand with variable autonomy, where human and robot share a single physical body and operate at different autonomy levels across task phases. The system uses a learning-from-demonstration visuomotor diffusion policy for autonomous grasping, followed by human-dominant control via head gestures. A user study with 44 participants showed 23.3% improvement in completion times, 93.6% task success rate, and high acceptance ratings (5.70/7).


<details>
  <summary>Details</summary>
Motivation: Assistive robotic systems face a fundamental trade-off between full autonomy (lack of user agency) and full user control (continuous cognitive effort). Existing shared autonomy approaches blend human and robot commands but are mostly deployed in separate physical bodies.

Method: The authors propose co-embodiment with variable autonomy where human and robot share a single physical body (wearable robotic hand). A learning-from-demonstration visuomotor diffusion policy enables autonomous grasping near known objects. After grasping, the system signals completion and the human actuates tools via hands-free head gestures. The user retains veto authority through a release gesture. This realizes a "one body, two minds" paradigm.

Result: 44 participants performed five bimanual tasks. Completion times improved by 23.3% across trials (p < 0.001, Cohen's d = 0.94). Best policy variant achieved 93.6% task success rate. User acceptance was high with 5.70/7 overall impression and 5.52/7 daily use willingness.

Conclusion: Co-embodiment with variable autonomy is established as a viable approach for assistive robotics, enabling effective human-robot collaboration through co-embodiment.
...
### [58] [WOLF-VLA: Whole-Body Humanoid Optimal Locomotion Framework for Vision-Language-Action Learning](https://arxiv.org/abs/2606.25591)
*Melya Boukheddimi,Omar Adjali,Daniel Sontag,Frank Kirchner*

Main category: cs.RO

TL;DR: WOLF-VLA integrates whole-body optimal-control motion synthesis with multi-modal datasets to train Vision-Language-Action models that generate humanoid locomotion policies from natural language instructions, demonstrating strong reasoning, robustness, and competitive performance across six locomotion task families.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models show strong generalization in robotic manipulation but are underexplored for whole-body, contact-rich humanoid locomotion due to data scarcity, lack of dynamically consistent demonstrations, and difficulty encoding optimality/safety in learning-based pipelines.

Method: (1) Constructs a comprehensive dataset of dynamically feasible humanoid trajectories across six locomotion task families using whole-body optimal-control motion synthesis; (2) Trains a VLA model on joint trajectories, ego-centric visual observations, and natural language instructions.

Result: The trained policy exhibits strong reasoning, robustness to initial-condition variability, and competitive performance across various tasks and environment settings. Ablation studies show impact of each modality on model performance.

Conclusion: The open-sourced dataset, model checkpoints, and benchmarking suite establish a reproducible dynamically consistent benchmark for whole-body humanoid locomotion VLA control, enabling future research in scalable transfer of instruction-driven locomotion policies.
...
TL;DR: EAMP is an efficient VLM-based navigation framework that uses a selective semantic trigger, distilled vision-language model, and semantic model predictive control to handle unforeseen semantic events in safety-critical robot navigation, improving safety while maintaining real-time performance.


<details>
  <summary>Details</summary>
Motivation: Robot navigation in safety-critical scenarios faces collisions from unpredictable dynamic agent behaviors rather than static obstacles, and using large VLMs continuously causes severe computational latency that destabilizes physical execution.

Method: The EAMP framework has three key components: (1) a prompt-configurable semantic event trigger (PC-SET) that selectively activates based on behavioral anomaly detection, (2) an event-triggered distilled SemNav-VLM fine-tuned via physically verified semantic distillation for strategy-level decisions, and (3) a semantic model predictive control (SMPC) module that translates strategies into dynamic reconfigurations of objectives and geometric references.

Result: EAMP effectively aligns high-level reasoning with low-level control, significantly improves dynamic safety margins over existing baselines, and preserves real-time efficiency in safety-critical logistics scenarios.

Conclusion: The proposed event-adaptive approach addresses the latency challenge of VLM-based navigation by selectively invoking semantic reasoning only when needed, achieving both safety and efficiency in dynamic environments.
...
TL;DR: This paper proposes a multi-camera extrinsic calibration method using pure rotational motion and a single static calibration board, achieving competitive accuracy without specialized hardware by leveraging a latent turntable frame and global SE(3) optimization.


<details>
  <summary>Details</summary>
Motivation: Existing multi-camera calibration methods with non-overlapping FOVs suffer from either high setup overhead (large targets, pre-measured poses) or drift/scale ambiguity (motion-based approaches). The authors aim to bridge accuracy and usability.

Method: The method uses pure rotational motion to let all cameras sequentially observe the same static calibration target under a shared geometric reference. A latent turntable frame is introduced, and the problem is formulated as a global optimization with 3D error on SE(3).

Result: Validated on a controlled camera rig and a full-scale vehicle with heterogeneous cameras. Achieves competitive accuracy without precision hardware, and remains robust under non-ideal turntable motion.

Conclusion: The approach is strongly suitable for realistic on-site deployments, balancing accuracy and usability without requiring specialized hardware.
...
TL;DR: Reptile-D-learning uses meta-learning to train Lyapunov controllers that quickly adapt to new system parameters without full retraining.


<details>
  <summary>Details</summary>
Motivation: LLC requires accurate system models and fails under parameter variations, while D-learning is limited to single-task dynamics and degrades under large parameter shifts – both need costly retraining.

Method: Combines Reptile meta-learning with D-learning to learn a generalizable Lyapunov network initialization and controller across systems with different parameters.

Result: On multiple nonlinear control systems, the method significantly improves generalization and rapid adaptation to unseen parameter configurations.

Conclusion: Reptile-D-learning effectively captures shared dynamical structures, enabling fast adaptation and reducing the need for expensive retraining under parameter uncertainty.
...
TL;DR: This paper proposes using a constrained Markov decision process with PPO-Lagrangian to enforce an explicit power budget (in physical units) for energy-efficient underwater vehicle control, achieving 14-65% power reduction across three vehicles and four tasks without manual weight tuning.


<details>
  <summary>Details</summary>
Motivation: Underwater vehicles have limited onboard energy, and standard RL controllers optimized solely for task accuracy tend to produce oscillatory, energy-wasting actuation. Existing reward-penalty approaches require manual weight tuning with no physical interpretability and can even increase power draw if mismatched.

Method: Formulates energy-efficient control as a constrained Markov decision process (CMDP) with an explicit average thruster power budget, solved via PPO-Lagrangian where a single dual variable is updated online to enforce the power constraint for each vehicle and task without manual weight search.

Result: Across 3 vehicles and 4 tasks in MarineGym, the energy-constrained policy reduced power by 14-65% (up to 64.9%) over a task-only baseline, outperformed an energy-reward baseline everywhere, remained the smoothest in 10/12 settings, and preserved task accuracy in all but one deliberately power-limited regime.

Conclusion: Imposing energy as an explicit constraint provides a tuning-free, physically interpretable approach to energy-efficient underwater control that eliminates per-vehicle, per-task weight search.
...
TL;DR: SA-LIVO proposes a subspace-aware information fusion (SAIF) framework that eigendecomposes the joint LiDAR-visual information matrix and applies direction-selective soft gating, enabling visual information to only compensate where LiDAR is deficient, achieving competitive accuracy with lower latency and memory.


<details>
  <summary>Details</summary>
Motivation: LiDAR and visual sensors have complementary but independent failure modes (LiDAR degenerates under under-constrained geometry, visual fails under poor lighting/texture). Existing methods address degeneracy at the modality level (binary detection, covariance inflation, quality gating) but ignore the direction-dependent structure of the joint information matrix, causing visual residuals to enter well-constrained LiDAR directions and visual compensation to disperse instead of concentrating where needed.

Method: SA-LIVO uses the Subspace-Aware Information Fusion (SAIF) framework: it eigendecomposes the joint LiDAR-visual information matrix, applies a linear-clamp soft gate per eigendirection to attenuate degenerate directions while preserving well-observable ones, jointly optimizes residuals in one InEKF loop at a shared linearization point, and reuses photometric Jacobians across iterations to reduce per-iteration cost.

Result: Tested on 29 sequences across HILTI'22, New College, Oxford Spires benchmarks and concurrent-degradation scenarios. Achieves accuracy competitive with strongest baselines and bounded drift where others diverge. Runs at 12.3 ms/frame on laptop CPU and 26.8 ms on embedded ARM without GPU, with 3.6-6.3x lower peak memory.

Conclusion: SA-LIVO demonstrates that direction-selective information fusion effectively handles degenerate scenarios in LIVO, improving efficiency (lower latency and memory) while maintaining state-of-the-art accuracy, and will be open-sourced.
...
TL;DR: A framework for asynchronous upper-body task-space tracking in humanoids, using teacher-student distillation, sliding-window reward, and MPC-based post-training to handle temporal and structural gaps between sparse planners and high-frequency controllers.


<details>
  <summary>Details</summary>
Motivation: Address temporal asynchrony (low-rate sparse planning vs. high-rate whole-body control) and structural incompleteness (task-space plans lacking full-body references) in humanoid control.

Method: Teacher-student distillation initializes a policy conditioned on cached future trajectory and execution index; sliding-window global reward reduces frame drift; MPC module and self-guidance (action & FK level) enable task-specific post-training.

Result: Improved tracking under low update rates, outperforms synchronous and decoupled baselines, safer OOD motion adaptation; validated in simulation and on Unitree G1 hardware.

Conclusion: The asynchronous tracking framework effectively bridges sparse planning and high-frequency control, enabling robust and safe humanoid whole-body motion execution.
...
TL;DR: SRDP improves robotic polishing by using stage-aware diffusion policy and roughness-constrained sampling for better surface quality and process consistency.


<details>
  <summary>Details</summary>
Motivation: Current robotic imitation learning for polishing suffers from long-horizon dependencies, uncertain stage transitions, and difficulty in modeling coupled process parameters.

Method: Proposes Stage-Aware and Roughness-Constrained Diffusion Policy (SRDP) that infers process-stage posteriors from multimodal observation histories, conditions shared reverse denoising, and uses roughness-oriented constrained diffusion sampling with stage-wise preset spindle speeds.

Result: SRDP improves stage-transition stability, process-parameter consistency, and final surface quality across spacecraft cabin coating and inner-cavity structural surface polishing scenarios.

Conclusion: Stage-aware and roughness-constrained diffusion policy effectively addresses key limitations in robotic polishing imitation learning, demonstrating superior performance in real-robot validations.
...
TL;DR: StairMaster is a reinforcement learning framework enabling quadruped robots to climb steep hollow stairs (up to 55°) via cross-attention, spatial-temporal memory, and realistic depth sensor simulation, deployed zero-shot on a Unitree Go2.


<details>
  <summary>Details</summary>
Motivation: Hollow stair climbing is extremely challenging for quadrupeds due to leg trapping risks, sparse depth data, and noise. Existing RL methods fail on such extreme discontinuous terrains.

Method: Three-stage RL framework with Cross-Attention for structural feature extraction from noisy depth, Spatial-aware Recurrent Unit (SRU) for spatio-temporal memory, high-fidelity depth sensor sim-to-real pipeline, 3D waypoint-guided active perception reward, and kinematic/gap penalties for precise foothold placement.

Result: Achieved zero-shot deployment on Unitree Go2, climbing hollow stairs up to 55° incline — the first RL policy to accomplish such steep hollow stair climbing in real-world environments.

Conclusion: StairMaster demonstrates that combining attention mechanisms, spatial-temporal memory, and accurate sensor simulation can bridge the sim-to-real gap for extreme terrain locomotion, setting a new benchmark for hollow stair climbing.
...
TL;DR: The paper proposes MIL-LC, a multimodal localization framework fusing magnetometer, inertial sensors, and LiDAR, designed for challenging environments (GNSS-denied, textureless, repetitive scenes). It addresses the gap in applying ambient magnetic field-based fusion to autonomous mobile robots (AMRs). Results show robust and accurate localization in both simulation and real-world tests.


<details>
  <summary>Details</summary>
Motivation: Existing single-modality and multimodal localization methods fail in GNSS-denied, repetitive, or textureless environments, and most fusion approaches still depend on geometric/texture features or expensive infrastructure. Ambient magnetic field (AMF)-based localization offers a promising infrastructure-free and feature-independent complementary modality, but its application in AMR systems is unexplored.

Method: The authors propose MIL-LC, a magnetometer-inertial-LiDAR fused multimodal localization framework with a custom-designed sensor suite, which fuses ambient magnetic field, inertial, and LiDAR data to maintain reliable localization even when LiDAR suffers from geometric degeneration or magnetic maps change over time.

Result: Extensive simulation and real-world experiments demonstrate robust and accurate localization performance of the proposed MIL-LC framework in challenging environments.

Conclusion: The MIL-LC framework effectively bridges the gap of AMF-based multimodal localization for AMRs, providing a reliable solution for challenging environments without relying on geometric/texture features or additional infrastructure.
...
TL;DR: The paper exploits vehicle chassis-induced radar shadows as a geometric cue to estimate the 3D inclination of nearby vertical objects using 2D rotating FMCW radar, validated in simulation and real-world experiments.


<details>
  <summary>Details</summary>
Motivation: Reliable perception in adverse conditions (poor lighting/weather) is challenging for autonomous systems because cameras and LiDAR degrade, while FMCW radar is robust but suffers from elevation collapse (lack of 3D information). The authors aim to recover 3D geometric information from 2D radar by leveraging a previously overlooked geometric cue.

Method: The method uses the observation that vehicle chassis blocks radar rays, creating a consistent geometric shadow. By analyzing the radar return boundaries of slender vertical objects intersecting this shadow, a closed-form analytical mapping computes the object's in-plane inclination without scene assumptions. Validation uses simulation and a Navtech CTS350-X radar.

Result: Inclinations can be estimated under practical conditions; the main bottleneck is segmentation of the object in the radar scan. The approach works in both simulation and real-world experiments.

Conclusion: Chassis shadows are introduced as a novel geometric cue for 2D rotating radar, extending its capability beyond localization toward 3D scene reconstruction.
...
TL;DR: TacVerse is a multi-sensor tactile dataset (106,800 images from 7 VBTSs) for cross-sensor perception tasks. While within-sensor performance is strong, direct cross-sensor transfer degrades significantly. Few-shot adaptation helps but doesn't match within-sensor bounds. MAE pretraining offers the most consistent improvements across tasks and sensors.


<details>
  <summary>Details</summary>
Motivation: Generalization across different vision-based tactile sensor designs remains poorly understood, limiting the reusability of models trained on one sensor for another.

Method: The authors collected 106,800 tactile images from seven different VBTSs, supporting shape classification, grating classification, and force regression. They evaluated under within-sensor, zero-shot cross-sensor transfer, and few-shot adaptation settings, and investigated MAE pretraining for representation learning.

Result: Within-sensor performance is strong across tasks. Zero-shot cross-sensor transfer degrades substantially—shape classification is most robust, while grating classification and force regression are more sensitive. Few-shot adaptation improves force regression but doesn't reach within-sensor upper bounds. MAE pretraining yields the most consistent gains.

Conclusion: TacVerse serves as a controlled testbed for studying sensor shift, data-efficient adaptation, and self-supervised learning in tactile perception, revealing that cross-sensor generalization remains a significant challenge.
...
TL;DR: This paper introduces a 3D-printable dataset of mathematically defined textures for tactile sensing, enabling fair comparison across different sensors and printers by controlling surface geometry via parametric functions.


<details>
  <summary>Details</summary>
Motivation: Existing tactile texture datasets are sensor-specific and lack reproducibility, hindering fair sensor comparisons and reproducible research in tactile sensing.

Method: Six parametric surface patterns were designed using sine-wave and Fourier-based functions. These were 3D-printed across three printers and multiple filaments, then evaluated using an optical TacTip sensor under controlled conditions.

Result: Higher-end printers produced more consistent tactile signatures; within-printer generalization was strong, but cross-printer generalization remains difficult due to geometric inconsistencies.

Conclusion: This work establishes the first openly available, physically reproducible 3D-printed texture benchmark, providing a foundation for fair tactile sensor comparison.
...
TL;DR: DeformGen is a dynamics-based data augmentation framework that addresses two key challenges in deformable manipulation—high-dimensional state constraints and non-equivariant trajectory transfer—by applying localized physical disturbances with forward simulation for state diversity and using deformation-field warping for consistent trajectory transfer, significantly improving policy learning.


<details>
  <summary>Details</summary>
Motivation: Existing demonstration augmentation methods fail in deformable manipulation due to (1) the high-dimensional, physics-constrained state space where simple pose perturbations cannot produce valid configurations, and (2) the non-equivariant nature of trajectory transfer under deformation, where material points do not move rigidly together.

Method: DeformGen augments data via (1) localized physical disturbances + forward dynamics simulation to generate physically plausible, topology-coherent deformable states, and (2) deformation-field warping that lifts per-particle displacements into continuous spatial functions to adapt end-effector trajectories consistently with deformed geometry.

Result: Experiments on high-fidelity deformable manipulation benchmarks show DeformGen generally improves policy learning over training on original demonstrations alone and over rigid-style augmentation baselines.

Conclusion: Dynamics-based augmentation with deformation-aware trajectory warping is an effective approach for achieving topological diversity and improving policy learning in deformable object manipulation.
...
TL;DR: DSP-SLAM++ enables real-time, multi-class object-aware SLAM with high-fidelity object models by using an asynchronous mapping pipeline and fisheye-LiDAR fusion, cutting object processing latency by up to 70%.


<details>
  <summary>Details</summary>
Motivation: Existing object-aware SLAM systems trade off real-time performance, multi-class support, and high-fidelity semantically coherent object model generation, making them impractical for real-world applications.

Method: Extends DSP-SLAM with an asynchronous mapping pipeline for real-time performance and dedicated sensor fusion adaptations for a monocular fisheye-LiDAR sensor suite.

Result: Generates fine-grained, geometrically-complete shapes for multiple object classes, reduces maximum object processing latency by up to 70% compared to the state-of-the-art baseline, and achieves robust real-time performance on 25 Hz multi-class datasets.

Conclusion: This work makes high-fidelity, multi-class object SLAM more practical for real-world applications such as autonomous driving and robotic manipulation, especially on platforms with common fisheye-LiDAR sensor setups.
...
TL;DR: This paper proposes drone-based quantum magnetometry for locating survivors in collapsed buildings, using simulation pipelines with Unreal Engine and Bayesian active sampling to show feasibility of sub-pT magnetic sensing from above rubble.


<details>
  <summary>Details</summary>
Motivation: Locating survivors within the critical first 72 hours after building collapses remains extremely challenging, as existing sensing modalities provide only partial structural information beneath rubble.

Method: The authors build a simulation pipeline using Unreal Engine to generate a steel-reinforced concrete parking-garage collapse, compute induced magnetic fields via per-triangle dipole approximation, and feed sparse multi-sensor samples into Gaussian Process Regression with Bayesian active sampling.

Result: Meaningful magnetic structure is recoverable in the sub-pT to sub-nT range from ~1 m above roofline; a three-sensor array optimally balances gradient resolution and UAV payload; active sampling reaches peak structural correlation in ~100 samples.

Conclusion: Quantum-grade magnetometry could become a useful tool for drone-based structural analysis and void detection in collapsed buildings.
...
TL;DR: This paper proposes a fault-aware modular control architecture for legged robots that uses fault-diagnosis information to activate specialized control experts for different actuator failure modes, outperforming monolithic policies especially under compute-constrained conditions.


<details>
  <summary>Details</summary>
Motivation: Legged robots in planetary exploration need reliable locomotion despite actuator failures, but monolithic reinforcement learning policies struggle to efficiently represent diverse control strategies for different fault conditions.

Method: A fault-aware modular control architecture that leverages fault-diagnosis information to activate specialized control experts associated with distinct actuator failure modes.

Result: Explicit fault-conditioned modular policies consistently outperform monolithic policies of comparable size across failure scenarios, and retain competitive performance even under significantly reduced network capacity.

Conclusion: The modular architecture is more suitable for compute-constrained robotic platforms (e.g., space applications) than monolithic policies, and code is open-sourced.
...
TL;DR: This paper demonstrates that magnetic plantar sensing can be embedded inside a 3D-printed lattice footplate of a semi-active prosthetic foot, enabling plantar-force estimation and feedforward damping control without external insole layers.


<details>
  <summary>Details</summary>
Motivation: To determine if magnetic plantar sensing can be integrated directly into the load-bearing compliant element of a low-cost semi-active prosthetic foot, eliminating the need for external insole sensor layers.

Method: The authors built a prototype with a sensorised 3D-printed lattice footplate, servo-adjustable hydraulic damper, and reduced-order ankle model. They conducted controlled compression tests, cyclic loading tests, static-posture trials, and implemented a feedforward damping schedule.

Result: Embedded sensor tracked reference force accurately; forefoot/rearfoot loading distributions were separable across stance configurations; feedforward damping approximated dorsiflexion trends but lacked active push-off capability.

Conclusion: Sensing can be embedded inside the load-bearing compliant element of a prosthetic foot and used to drive semi-active damping, though a purely dissipative mechanism cannot generate active push-off.
...
TL;DR: ACNet is a lightweight delay-aware adapter that improves asynchronous robot control by using executed motion suffixes as residual conditions, reducing handoff discontinuities and action jitter without full retraining.


<details>
  <summary>Details</summary>
Motivation: Inference latency in VLA models hinders stable high-frequency robot control. Asynchronous execution causes stale observations and handoff discontinuities. Existing fixes require retraining or architecture-specific logic.

Method: ACNet is a delay-aware adapter that uses the executed motion suffix as a residual condition for a mostly frozen action head, leaving the pretrained backbone unchanged with few additional trainable parameters.

Result: On Kinetix, Meta-World MT50, and real-world SO-ARM101, ACNet improves robustness under inference delay and produces smoother trajectories than direct chunk stitching, while being more lightweight than full delay-conditioned retraining.

Conclusion: ACNet offers a practical, lightweight solution for asynchronous VLA control that is compatible with generative action heads and requires minimal architectural changes.
...
TL;DR: FORCE is a 3-stage RL fine-tuning framework for VLA models that addresses catastrophic unlearning and inefficient policy updates through Value-Calibrated Warm-Up and Q-function-based action filtering, achieving 79% success rate improvement, 10% gain over prior RL methods, and 32.5% faster training without human intervention.


<details>
  <summary>Details</summary>
Motivation: VLA models suffer from imitation ceilings due to sub-optimal data, and RL fine-tuning is sample inefficient due to catastrophic unlearning from unstable Q-functions and inefficient policy updates from low-quality exploration data, often requiring costly human interventions.

Method: FORCE is a 3-stage framework: (1) Value-Calibrated Warm-Up using on-policy rollouts to stabilize the Q-function and mitigate distributional shift; (2) Online stage where the calibrated Q-function filters both the policy's own action proposals and expert data; (3) Only high-value actions are used for policy updates.

Result: 79% absolute improvement in success rates, 10% outperformance over prior RL methods, 32.5% training acceleration, and mitigation of success rate drop without human intervention across simulation and real-world tasks.

Conclusion: FORCE enables stable and efficient RL fine-tuning for VLA models without human intervention, representing progress toward capable and autonomous robotic agents.
...
TL;DR: EMCAR is a no-code tool that uses artistic practices (puppetry, drawing) to enable non-technical users to program robot motion and expand creative HRI applications.


<details>
  <summary>Details</summary>
Motivation: To lower the technical barrier of robot programming by leveraging artistic affordances, enabling artists and non-experts to engage in HRI research and creative applications.

Method: A no-code software platform (EMCAR) that integrates puppetry and drawing-based interaction paradigms for programming collaborative robot motion without traditional coding.

Result: Expands creative applications for collaborative robots and provides an inclusive environment for individuals with little/no technical background to participate in robotics research.

Conclusion: Artistic practices like puppetry and drawing can effectively serve as intuitive interfaces for robot programming, democratizing access to HRI.
...
TL;DR: FAR-LIO is a CUDA-accelerated LiDAR-inertial odometry framework that achieves fast, accurate, and robust performance by using a voxel hashmap-based parallelized nearest-neighbor search and sparsity-aware GICP with adaptive thresholding, combined with an EKF backend that fuses LiDAR odometry with high-frequency IMU data via upsampling and delay compensation.


<details>
  <summary>Details</summary>
Motivation: Robust and accurate odometry estimation is essential in robotics, especially in environments with highly dynamic motion and sensor noise like autonomous racing. Minimizing odometry latency is critical for stable closed-loop control, motivating the need for a faster and more robust LiDAR-inertial odometry framework.

Method: 1) CUDA-based voxel hashmap for parallelized nearest-neighbor search and efficient map updates. 2) Sparsity-aware Generalized Iterative Closest Point (GICP) algorithm with adaptive thresholding on top of the voxel hashmap with adaptive density. 3) Extended Kalman Filter (EKF) backend with upsampling and delay compensation to fuse LiDAR odometry with high-frequency IMU data.

Result: Evaluated across four sensor setups using public datasets and autonomous racecars at speeds up to 250 km/h. Achieved 6.9% reduction in positional error and 38.4% lower runtime compared to state-of-the-art baselines with a single parameter set.

Conclusion: FAR-LIO demonstrates computational efficiency and broad applicability for high-speed autonomous racing. Code is open-source at https://github.com/TUMFTM/FAR-LIO.
...
TL;DR: G2DP is a diffusion-based planner for autonomous driving that uses differentiable spatio-temporal cost volumes (fusing occupancy distributions with route-progress maps) to inject dense gradients during denoising, achieving state-of-the-art closed-loop performance on nuPlan (+7.2 reactive score) and strong zero-shot transfer results.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based planners rely on sparse entity-level geometric queries or post-hoc refinement for guidance, which leads to limited situational awareness and fragile performance in dense, interactive traffic scenarios.

Method: G2DP constructs a differentiable spatio-temporal cost volume by fusing probabilistic future occupancy distributions with a route-progress map, formulated as a continuous safety energy functional. This injects dense gradients directly into the denoising loop to steer trajectory generation toward collision-free and progress-optimal regions.

Result: Outperforms the strongest IL baseline by +7.2 points in reactive score on nuPlan; achieves top scores in zero-shot transfers to interPlan and DeepScenario; collision avoidance improves by +10.15 over unguided approach on interPlan.

Conclusion: Spatio-temporal cost grids serve as an effective representation for providing robust, dense guidance in diffusion-based planning, enabling safer and more route-adherent closed-loop driving.
...
TL;DR: ICWM enables robot policies to adapt to novel configurations (e.g., camera viewpoints, robot morphologies) by inferring system dynamics from a short history of task-agnostic interactions, without requiring parameter updates.


<details>
  <summary>Details</summary>
Motivation: Standard VLA models fail to generalize to novel setups (altered camera viewpoints or robot morphologies) because they assume a fixed execution context and require data-intensive fine-tuning for new environments.

Method: ICWM treats system identification as an in-context adaptation problem. It uses a short history of self-generated, task-agnostic interactions processed in the context window to implicitly capture world dynamics of the current system.

Result: Extensive simulation and real-world experiments show ICWM significantly outperforms standard VLA baselines on novel camera viewpoints.

Conclusion: ICWM offers a parameter-free adaptation method that improves generalization to novel configurations by leveraging in-context world modeling.
...
TL;DR: RoboAtlas is a contextual Active SLAM framework that balances geometric exploration and semantic reasoning using a 3D semantic mapping system (OpenRoboVox). It achieves 100% task success in real-world tests and state-of-the-art 90.6% SR on GOAT-Bench, demonstrating that grounding foundation models with large-scale 3D semantic maps significantly improves performance.


<details>
  <summary>Details</summary>
Motivation: To address the need for an Active SLAM system that can adaptively balance geometric exploration and semantic reasoning in large-scale environments, moving beyond purely geometric approaches.

Method: Integrates frontier exploration, global semantic-map reasoning, and egocentric VLM-based reasoning using a contextual multi-armed bandit that transitions from exploration to semantically guided navigation as scene understanding improves.

Result: Achieves 100% task success on Unitree Go2 in real-world environments (1800 m², ~30k semantic instances). On GOAT-Bench "Val Unseen": 90.6% SR with GPT-4o (17.8 pp improvement), and 88.8% SR with Qwen2.5-VL-7B, outperforming all baselines using GPT-4o.

Conclusion: Grounding foundation models with large-scale 3D semantic maps enables robust and efficient contextual Active SLAM, and the semantic mapping framework itself contributes more to performance than simply replacing the underlying VLM.
...
TL;DR: iCrowdNav uses visual attention and pose-based intention inference for robot crowd navigation, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Existing DRL navigation methods treat pedestrians as simple 2D points, ignoring rich visual cues from humans and environments.

Method: Two components: (1) spatio-temporal encoder for occupancy features, (2) Intent-Interact Former (I² Former) for encoding human poses to infer motion intentions. Integrated into DRL policy training.

Result: Superior performance over baselines in extensive experiments; successful real-world deployment.

Conclusion: Visual scene representations and intention-aware encoding improve crowd navigation in DRL policies.
...
### [84] [Deep Reinforcement Learning-Enhanced Event-Triggered Data-Driven Predictive Control for a 3D Cable-Driven Soft Robotic Arm](https://arxiv.org/abs/2606.26048)
*Cheng Ouyang,Moeen Ul Islam,Kaixiang Zhang,Zhaojian Li,Xiaobo Tan,Dong Chen*

Main category: cs.RO

TL;DR: The paper proposes RL-ET-DeePC, a reinforcement-learning-based event-triggered data-enabled predictive control framework that reduces optimization frequency by up to 66% in simulation and 34% in hardware while maintaining tracking accuracy for soft robotic control.


<details>
  <summary>Details</summary>
Motivation: Soft robots are hard to control due to nonlinear/time-varying dynamics, and standard DeePC is computationally expensive for real-time deployment on resource-limited platforms.

Method: An RL policy is trained to decide when to trigger the DeePC optimizer based on system state, reducing unnecessary optimization calls.

Result: 66% reduction in optimization frequency in simulation; 34% reduction on a real 3D cable-driven soft robotic arm with comparable tracking accuracy.

Conclusion: RL-ET-DeePC effectively reduces computational burden while maintaining performance, and transfers zero-shot to real hardware.
...
### [85] [ForceBand: Learning Forceful Manipulation with sEMG](https://arxiv.org/abs/2606.26093)
...
TL;DR: ForceBand is a low-cost wrist-worn sEMG system that predicts per-finger forces from muscle activity, enabling force-enriched robot demonstrations from human video data.


<details>
  <summary>Details</summary>
Motivation: Human demonstrations used for robot learning lack contact force information crucial for force-sensitive manipulation tasks.

Method: Collected 10-hour multimodal dataset (video, sEMG, IMU, fingertip forces) → pre-trained EMG2Force model predicting per-finger forces → user-specific calibration → force-augmented demonstrations for robot policy learning.

Result: Outperforms vision-based baselines by >50% lower force prediction error; achieves 87% success rate on force-sensitive pick/squeeze/place tasks across diverse objects.

Conclusion: ForceBand effectively bridges the force gap in human demonstrations for robot learning using low-cost wearable sEMG sensing.
...
### [86] [Learning Action Priors for Cross-embodiment Robot Manipulation](https://arxiv.org/abs/2606.26095)
*Dong Jing,Tianqi Zhang,Jiaqi Liu,Jinman Zhao,Zelong Sun,Li Erran Li,Zhiwu Lu,Mingyu Ding*

Main category: cs.RO

TL;DR: This paper proposes a two-stage training framework where the action module is first pretrained with motion priors using flow-matching on unconditioned action trajectories, then transferred to Vision-Language-Action (VLA) training via decoder reuse and latent distillation, achieving faster convergence and stronger cross-embodiment performance.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models inherit strong visual and linguistic priors from VLMs but require the action module to learn motion dynamics from scratch, lacking explicit motion priors. This is especially problematic in cross-embodiment settings where temporal action dynamics and cross-modal alignment must be learned simultaneously.

Method: A two-stage training framework: Stage 1 pretrains a lightweight flow-matching encoder-decoder action module on unconditioned action trajectories to learn temporal motion structure; Stage 2 transfers this prior to VLA training via decoder reuse and early-stage latent distillation, with the encoder serving as a history compressor.

Result: Experiments across 13 cross-embodiment tasks (simulated and real-world) show faster convergence, higher success rates, stronger data-scarce performance, and scaling benefits from more action pretraining data.

Conclusion: Pretraining action modules with motion priors significantly improves VLA policy learning, enabling more efficient cross-embodiment generalization and better real-world performance with limited data.
...
### [87] [Reasonable Motion: A General ASP Foundation for Environment Constrained Movement Trajectory Computation](https://arxiv.org/abs/2606.25626)
*Julius Monsen,Jakob Suchan,Mehul Bhatt,Lars Karlsson*

Main category: cs.AI

TL;DR: A general ASP-based hybrid quantitative-qualitative method for computing constrained branching trajectory modes for moving objects, demonstrated on autonomous driving with Argoverse 2, offering verifiable interpretability through stable model traceability.


<details>
  <summary>Details</summary>
Motivation: To develop a verifiably interpretable trajectory computation method that combines quantitative and qualitative reasoning, overcoming the lack of explainability in purely learned approaches for moving object trajectory analysis.

Method: Answer set programming (ASP) based hybrid quantitative-qualitative method that performs constrained traversal of an environment graph, enumerating geometrically admissible motion behaviors as stable models, incorporating domain-dependent (event sequences, map topology, domain norms) and domain-independent factors.

Result: Demonstrated applicability on autonomous driving with Argoverse 2 benchmark; computed trajectories are traceable to their underlying stable models, enabling verifiable interpretability.

Conclusion: The hybrid trajectory computation method is generally applicable across diverse dynamic domains with moving objects and provides verifiable interpretability that purely learned approaches cannot offer.
...
### [88] [A Leakage-Aware Comparative Benchmark of Machine Learning, Deep Learning, and Transformer Models for Reliable Leukemia Detection](https://arxiv.org/abs/2606.24944)
*Nisreen Albzour*

Main category: eess.IV

TL;DR: This paper reveals that inflated performance on ALL classification from the C-NMC 2019 dataset is caused by patient-level data leakage from random image-level partitioning, and provides a reproducible, calibration-aware benchmark using a strict subject-disjoint protocol.


<details>
  <summary>Details</summary>
Motivation: To expose the inflation of near-perfect classification results on the C-NMC 2019 dataset due to patient-level data leakage, and to establish a more honest, calibration-aware benchmark under a subject-disjoint evaluation protocol.

Method: The authors use a subject-disjoint (patient-level) data partitioning with 73 subjects divided into three folds, and evaluate on an external test set of 1,867 images from 28 unseen subjects. They compare LightGBM, RBF-SVM, EfficientNet-B0/B1, and ViT-Tiny, assessing both discrimination (AUROC, sensitivity, specificity) and calibration (ECE, Brier score, temperature scaling). An ablation study compares random vs. subject-disjoint splitting.

Result: Under honest evaluation, EfficientNet-B1 performs best (AUROC 0.913, sensitivity 0.87, specificity 0.80, calibrated ECE 0.024). Frozen-feature classifiers and ViT-Tiny show high sensitivity but poor specificity (over-predicting malignancy). Random splitting inflates AUROC by ~0.04 even in the conservative frozen-feature setting.

Conclusion: Image-level evaluation on C-NMC 2019 is misleading due to patient leakage; future work should use subject-disjoint splits and consider calibration metrics. The authors provide a reproducible benchmark to guide honest performance assessment.
...
### [89] [Benchmarking the Alignment of Data-Quality Metrics, Human Judgment and Land-Cover Segmentation Performance for Earth Observation](https://arxiv.org/abs/2606.25128)
*Ümit Mert Çağlar,Alptekin Temizel*

Main category: eess.IV

TL;DR: This paper demonstrates that common image quality metrics (FID, KID, etc.) are unreliable for evaluating synthetic Earth observation data, as they misalign with human perception and downstream task performance.


<details>
  <summary>Details</summary>
Motivation: Quality metrics like FID focus on visual fidelity but fail to reflect downstream utility and can diverge from human perception, especially for geospatial data.

Method: Systematic evaluation of real and synthetic Earth observation datasets using automatic metrics, human perception tests, and downstream semantic segmentation tasks.

Result: Semantics-preserving perturbations (e.g., rotation) drastically alter metric scores but not human recognition; poorly-scored synthetic samples achieve comparable/higher perceived realism and can improve downstream performance when mixed with real data.

Conclusion: Automatic quality evaluation of synthetic geospatial datasets should be grounded in downstream task performance and human evaluation rather than ImageNet-pretrained feature space metrics.
...
### [90] [Dimension expansion for simulation-efficient nanophotonic neural networks](https://arxiv.org/abs/2606.25226)
*Shuo Huang,Mahsa Torfeh,Lujia Zhong,Michelle L. Povinelli,Owen D. Miller,Chia Wei Hsu*

Main category: eess.IV

TL;DR: DEN is an unsupervised neural network framework that converts low-dimensional design targets into high-dimensional representations for efficient nanophotonic inverse design, achieving strong results on metalens and Y-splitter tasks while reducing simulation costs by ~50% without needing precomputed datasets.


<details>
  <summary>Details</summary>
Motivation: Nanophotonic inverse design is hindered by large design spaces, nonlinear structure-response relationships, and high computational costs. Existing deep-learning methods require large precomputed datasets or libraries of optimized structures, limiting scalability to continuous and complex tasks.

Method: The Dimension Expansion Network (DEN) transforms compact low-dimensional target parameters into structured high-dimensional conditioning representations before performing inverse design. The model is trained end-to-end using differentiable electromagnetic simulations, eliminating the need for pre-generated datasets.

Result: DEN achieves focal intensities comparable to adjoint-based optimization while reducing simulation cost by ~50% for metalens design. For Y-splitter design, it produces arbitrary power-splitting ratios using only 21 training targets with robust broadband performance. Ablation studies show dimension expansion improves sensitivity to target variations and reduces mode-collapse-like behavior.

Conclusion: DEN provides a scalable conditioning strategy for inverse design with low-dimensional objectives, enabling efficient photonic design across large continuous target spaces without requiring precomputed datasets.
...
### [91] [Dual Agreement Consistency Learning for Semi-Supervised Fetal Ultrasound Segmentation](https://arxiv.org/abs/2606.25254)
*Fangyijie Wang,Guénolé Silvestre,Ziyang Wang,Kathleen M. Curran*

Main category: eess.IV

TL;DR: DACL is a semi-supervised framework for fetal ultrasound segmentation that jointly trains a lightweight CNN and a Transformer using dual-agreement consistency loss and mixup, achieving superior performance with only 5% labeled data.


<details>
  <summary>Details</summary>
Motivation: Pixel-level annotations for fetal US images are scarce and expensive to obtain, limiting automated segmentation performance.

Method: DACL combines a lightweight CNN (1.47M params) and a Transformer via CPS, with a dual-agreement consistency loss coupling pixel-wise probabilistic divergence and entropy-guided confidence alignment, plus mixup-based interpolation for unlabeled samples.

Result: Under 5% labeled data, DACL improves Dice by 2.77% and reduces HD95 by 14.69mm over recent semi-supervised methods on fetal head and abdomen datasets.

Conclusion: Dual-agreement consistency learning effectively addresses annotation scarcity for robust fetal US segmentation.
...
### [92] [Blasto-Net: An Explainable Multi-Task Learning for Blastocyst Segmentation, Grading, and Implantation Prediction](https://arxiv.org/abs/2606.25463)
*Zahra Asghari Varzaneh,Reza Khoshkangini,Magnus Johnsson,Thomas Ebner,Lars Johansson*

Main category: eess.IV

TL;DR: Blasto-Net is a multi-task deep learning model that simultaneously performs blastocyst compartment segmentation (ZP, TE, ICM), morphological grading, and implantation outcome prediction in a single forward pass, achieving high Dice scores and an 80% implantation F1-score.


<details>
  <summary>Details</summary>
Motivation: Accurate blastocyst analysis in IVF is challenging because compartments have similar textures but very different structures, and existing methods often handle tasks separately rather than jointly.

Method: Blasto-Net uses an EfficientNet-B3 encoder with a UNet-style decoder enhanced by CBAM and a novel Edge-Aware Attention Module (EAAM), along with specialized segmentation heads, a composite region- and boundary-based loss, and Grad-CAM++ for interpretability.

Result: Dice scores: ICM 94.93%, ZP 91.60%, TE 88.82%; implantation F1-score: 80.0% on a public HMC blastocyst dataset.

Conclusion: Blasto-Net provides an accurate, interpretable, and efficient solution for automated blastocyst assessment with strong potential to support clinical decision-making in IVF.
...
### [93] [Cross-Attention Multimodal Learning for Predicting Response to Neoadjuvant Imatinib in Gastrointestinal Stromal Tumors: A Multicenter Retrospective Study](https://arxiv.org/abs/2606.25579)
...
TL;DR: A multimodal deep learning framework integrating CT imaging and clinical variables was developed to predict neoadjuvant imatinib response in GIST patients. Cross-attention models showed high internal (AUC up to 0.99) but limited external performance (AUC 0.60-0.63), with explainability revealing key feature differences between responders and non-responders.


<details>
  <summary>Details</summary>
Motivation: Response to neoadjuvant imatinib in GIST is highly variable and cannot be reliably predicted using current clinical or molecular markers.

Method: Retrospective study with 935 pretraining and 213 prediction patients from four centers. Developed a cross-attention framework integrating clinical variables and CT imaging, tested self-supervised pretraining vs. training from scratch, with SMAC3 hyperparameter optimization and ablation/explainability analyses.

Result: Cross-attention models achieved AUC up to 0.99 internally but only 0.60-0.63 externally. Clinical-only AUC was 0.66, imaging-only AUC 0.56-0.66. Explainability identified CD117, BRAF, PDGFRA, age, sex, disease status, and comorbidities as significant differentiators (FDR-adjusted P≤0.036).

Conclusion: The cross-attention framework shows potential for improving imatinib response prediction in GIST with interpretable insights, but external generalizability remains limited, indicating need for further refinement.
...
### [94] [Improving Richardson--Lucy Deconvolution with Diffusion Priors for Fluorescence Microscopy](https://arxiv.org/abs/2606.25924)
*Hao Chen,Scott S. Howard*

Main category: eess.IV

TL;DR: This paper integrates score-based diffusion priors into Richardson-Lucy deconvolution to improve fluorescence microscopy reconstruction by reducing noise amplification while preserving fine structures in low-photon regimes.


<details>
  <summary>Details</summary>
Motivation: RL deconvolution is ill-posed and amplifies noise in low-photon conditions; regularizers like TV cause oversmoothing. There is a need for structural guidance that preserves fine biological features.

Method: A score-based diffusion prior is integrated into a decoupled inverse-problem framework, where the diffusion prior guides RL optimization iterations while RL maintains Poisson data consistency.

Result: The approach reduces RL noise amplification and improves preservation of weak filamentous and punctate structures under low photon counts across diverse biological samples.

Conclusion: Learned generative priors via diffusion models effectively guide RL deconvolution, offering better stability and structural preservation than conventional regularizers.
