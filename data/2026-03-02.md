...
TL;DR: DesignSense-10k: A large-scale human-annotated preference dataset and reward model for graphic layout evaluation that outperforms existing models and improves layout generation quality.


<details>
  <summary>Details</summary>
Motivation: Current layout generation models often fail to align with human aesthetic judgment, and existing preference datasets/reward models from text-to-image generation don't generalize to layout evaluation where spatial arrangement is crucial.
...
Result: DesignSense outperforms existing open-source and proprietary models by 54.6% in Macro F1 score. Frontier VLMs fail catastrophically on four-class layout evaluation. Using DesignSense improves RL-based training win rate by ~3% and inference-time scaling by 3.6%.

Conclusion: Specialized, layout-aware preference modeling is essential for improving graphic layout generation quality, as demonstrated by the significant performance gains of DesignSense over general-purpose models.
...
TL;DR: ANTShapes is a Unity-based simulator for generating synthetic neuromorphic vision datasets with configurable 3D scenes and statistical anomaly labeling.


<details>
  <summary>Details</summary>
Motivation: Limited availability of Dynamic Vision Sensors (DVS) and existing datasets with insufficient samples/scenarios hinders neuromorphic computer vision research.

Method: Built in Unity engine, simulates abstract 3D scenes with objects having randomly-generated behaviors (motion, rotation). Uses statistical process following central limit theorem principles for behavior sampling and anomaly labeling.

Result: Creates datasets with arbitrary number of samples, exportable with labels and frame data through parameter adjustment. Enables simulation of bespoke datasets for object recognition, localization, and anomaly detection.

Conclusion: ANTShapes addresses neuromorphic vision data scarcity by providing flexible dataset simulation framework for event-based computer vision research.
...
TL;DR: A unified proactive forensics framework called LIDMark that jointly addresses deepfake detection, tampering localization, and source tracing using a 152-dimensional landmark-identity watermark and specialized decoder architecture.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake forensics methods treat detection, localization, and source tracing as separate tasks, lacking a unified framework. With deepfake technology advancing rapidly and posing threats to privacy and security, there's a need for an integrated solution.

Method: Proposes LIDMark framework with: 1) 152-dimensional landmark-identity watermark that structurally combines facial landmarks with source identifier; 2) Factorized-Head Decoder (FHD) with specialized regression and classification heads to robustly extract landmarks and identifier even under distortion/tampering.

Result: Extensive experiments show the framework provides unified, robust, and imperceptible solution for deepfake detection, localization, and tracing. The regression head enables detection/localization via "intrinsic-extrinsic" consistency check, while classification head decodes source identifier for tracing.

Conclusion: LIDMark offers an "all-in-one" trifunctional forensic solution that addresses the limitations of existing fragmented approaches, providing comprehensive protection against malicious face manipulations through a unified watermark-based framework.
...
TL;DR: SVG2 is a massive panoptic video scene graph dataset with 636K videos, and TRaSER is a new model that generates spatio-temporal scene graphs from videos, achieving significant improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: Prior spatio-temporal scene graph datasets lack scale and diversity, limiting progress in video understanding. There's a need for larger datasets and better models to generate explicit spatio-temporal scene graphs as intermediate representations for video reasoning tasks.

Method: Created SVG2 using automated pipeline with multi-scale panoptic segmentation, trajectory tracking with new-object discovery, semantic parsing, and GPT-5-based relation inference. TRaSER model augments VLMs with trajectory-aligned token arrangement, object-trajectory resampler, and temporal-window resampler to generate scene graphs in single forward pass.

Result: TRaSER improves relation detection by +15-20%, object prediction by +30-40% over baselines and +13% over GPT-5, and attribute prediction by +15%. When used for VQA, it provides +1.5 to 4.6% accuracy gain over video-only or Qwen2.5-VL scene graphs.

Conclusion: SVG2 enables training of high-quality video scene graph generation models like TRaSER, which demonstrates the utility of explicit spatio-temporal scene graphs as intermediate representations for improving video understanding and reasoning tasks.
...
TL;DR: LE-NeuS is a latency-efficient neuro-symbolic framework for long-form video QA that reduces inference latency from 90x to 10x slower than base VLM prompting while maintaining accuracy gains.


<details>
  <summary>Details</summary>
Motivation: Existing neuro-symbolic approaches to LVQA have prohibitive latency overheads (up to 90x slower than base VLM prompting), making them impractical for latency-sensitive edge deployments, despite their accuracy benefits.

Method: Two optimizations: (1) CLIP-guided two-stage adaptive sampling to skip semantically similar frames while preserving temporal boundaries, and (2) batched proposition detection that parallelizes VLM inference across temporal windows. Also provides theoretical latency bounds analysis.

Result: On LongVideoBench and Video-MME benchmarks (NVIDIA H100 GPUs), LE-NeuS reduces latency gap from 90x to ~10x while maintaining >10% accuracy gains on temporally complex queries.

Conclusion: LE-NeuS demonstrates that neuro-symbolic temporal reasoning can be made latency-efficient through principled optimizations, enabling practical deployment while preserving accuracy benefits for complex temporal queries.
...
TL;DR: First study of cross-sensor view synthesis across different modalities, proposing a match-densify-consolidate method to eliminate cumbersome calibration for RGB-X data collection.


<details>
  <summary>Details</summary>
Motivation: Existing RGB-X work assumes aligned data pairs exist and focuses on modality fusion, but calibration requires huge engineering effort. The paper aims to remove cumbersome calibration for various RGB-X sensors and advance cross-sensor learning popularity.

Method: Match-densify-consolidate approach: 1) RGB-X image matching with guided point densification using confidence-aware densification and self-matching filtering, 2) Consolidation in 3D Gaussian Splatting (3DGS). Uses no 3D priors for X-sensor and only assumes nearly no-cost COLMAP for RGB.

Result: Method attains better view synthesis and provides a scalable solution that breaks through the bottleneck in large-scale real-world RGB-X data collection.

Conclusion: Proposes a practical solution to eliminate calibration requirements for RGB-X sensors, enabling scalable cross-sensor learning and advancing the field by overcoming data collection bottlenecks.
...
TL;DR: Evidential Neural Radiance Fields (ENeRF) is a probabilistic approach that quantifies both aleatoric and epistemic uncertainty in 3D scene modeling using a single forward pass, achieving state-of-the-art reconstruction fidelity and uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: Current NeRF methods lack proper uncertainty estimation, which limits their deployment in safety-critical applications. Existing uncertainty quantification methods either fail to capture both types of uncertainty (aleatoric and epistemic), compromise rendering quality, or require significant computational overhead.

Method: Evidential Neural Radiance Fields - a probabilistic approach that integrates seamlessly with the NeRF rendering process and enables direct quantification of both aleatoric and epistemic uncertainty from a single forward pass.

Result: State-of-the-art scene reconstruction fidelity and uncertainty estimation quality on three standardized benchmarks, outperforming existing uncertainty quantification methods.

Conclusion: The proposed Evidential Neural Radiance Fields successfully addresses the limitations of existing NeRF uncertainty methods by providing efficient, high-quality uncertainty quantification without compromising rendering performance, enabling more trustworthy 3D scene modeling for safety-critical applications.
...
TL;DR: CycleBEV: A regularization framework using cycle consistency to enhance bird's-eye-view semantic segmentation by adding an inverse view transformation network during training.


<details>
  <summary>Details</summary>
Motivation: Transforming image features from perspective view to bird's-eye-view in autonomous driving is challenging due to depth ambiguity and occlusion. Existing view transformation paradigms still struggle with these issues, requiring better methods to capture semantic and geometric information.

Method: Proposes CycleBEV framework with an inverse view transformation network that maps BEV segmentation maps back to PV segmentation maps. Uses cycle consistency losses to regularize VT networks during training. Introduces two novel extensions of cycle consistency into geometric and representation spaces to further exploit IVT network capacity.

Result: Consistent improvements across four representative VT models on nuScenes dataset: gains up to 0.74, 4.86, and 3.74 mIoU for drivable area, vehicle, and pedestrian classes respectively. No inference complexity increase since IVT network is only used during training.

Conclusion: CycleBEV effectively enhances existing view transformation models for BEV semantic segmentation through cycle consistency regularization, improving performance without adding inference overhead. The framework demonstrates broad applicability across different VT paradigms.
...
TL;DR: HDFLIM aligns frozen vision and language models using hyperdimensional computing without parameter updates, achieving comparable performance to fine-tuning methods.


<details>
  <summary>Details</summary>
Motivation: Traditional multimodal alignment requires computationally intensive fine-tuning that perturbs pretrained representations. The authors question whether cross-modal alignment can be achieved without modifying models, given evidence that independently trained foundation models may already have latent semantic compatibility.

Method: Projects unimodal embeddings into a shared hyperdimensional space and uses lightweight symbolic operations (binding, bundling, similarity-based retrieval) to construct associative cross-modal representations in a single pass. Caption generation emerges from high-dimensional memory retrieval rather than iterative gradient-based optimization.
...
Conclusion: Semantic mapping across foundation models can be realized through symbolic operations on hyperdimensional encodings, pointing toward an alternative paradigm where frozen models are integrated through structured representational mappings rather than large-scale retraining.
...
TL;DR: A new training paradigm using pseudo contrastive samples from synthetic diagrams improves vision-language models' diagram understanding by enhancing sensitivity to fine-grained structural variations.


<details>
  <summary>Details</summary>
Motivation: Current multimodal models like CLIP struggle with domains requiring sensitivity to small visual differences with large semantic significance, such as diagram understanding, due to limited ability to capture fine-grained structural variations.

Method: Introduces pseudo contrastive samples generated by a diagram renderer that creates synthetic diagrams using randomly picked text elements, highlighting structural differences without modifying original data. These samples are incorporated into the training objective to help models learn more precise and semantically consistent diagram structures.
...
Conclusion: The approach demonstrates the value of domain-specific training strategies for advancing diagrammatic understanding within vision-language learning, showing that targeted training with synthetic contrastive samples can significantly improve performance on diagram comprehension tasks.
...
TL;DR: An incremental dimension reduction algorithm that processes features in batches to enable anomaly detection on large datasets with thousands of images while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks extract high-dimensional features for visual anomaly detection, but this high dimensionality makes it difficult to process large datasets with thousands of images due to memory constraints.

Method: An incremental algorithm that groups feature vectors into batches, computes truncated SVD for each batch, updates global singular values/vectors incrementally, reduces batch features using their own SVD for storage, and finally re-transforms batch-wise singular vectors to the global feature space.
...
Conclusion: The proposed incremental dimension reduction method enables efficient processing of large-scale visual anomaly detection datasets by overcoming memory limitations while preserving detection accuracy.
...
TL;DR: HART is a framework that enables Large Multimodal Models to focus on key regions in high-resolution images without needing costly human annotations, using self-verification and policy optimization techniques.


<details>
  <summary>Details</summary>
Motivation: Current LMMs struggle with high-resolution visual inputs due to quadratic token growth, introducing redundancy. Existing methods require expensive human annotations for visual grounding, and there's a need for annotation-free approaches to enhance grounding abilities for reasoning.

Method: HART uses a closed-loop framework with self-verification of key regions and Advantage Preference Group Relative Policy Optimization (AP-GRPO) for post-training to encourage accurate localization without external visual supervision.

Result: HART improves performance across high-resolution visual tasks, outperforming strong baselines. When applied to Qwen2.5-VL-7B, it surpasses larger models like Qwen2.5-VL-72B and LLaVA-OneVision-72B on high-resolution, vision-centric benchmarks.

Conclusion: HART provides an effective annotation-free approach for enhancing LMMs' high-resolution reasoning capabilities through self-verification and policy optimization, achieving state-of-the-art performance without costly human annotations.
...
TL;DR: The paper introduces Eva-3M, a large-scale egocentric visibility-aware human pose estimation dataset with 3M+ frames and 435K visibility annotations, and proposes EvaPose, a novel method that explicitly incorporates visibility information to improve pose estimation accuracy.


<details>
  <summary>Details</summary>
Motivation: Egocentric human pose estimation faces challenges due to keypoint invisibility, but existing datasets lack visibility annotations and methods treat visible/invisible keypoints indiscriminately, compromising their accuracy.

Method: The authors present Eva-3M dataset with visibility annotations, augment EMHI dataset with similar annotations, and propose EvaPose method that explicitly incorporates visibility information to enhance pose estimation.
...
Conclusion: The work addresses the keypoint visibility problem in egocentric HPE by providing annotated datasets and a visibility-aware method that significantly improves pose estimation accuracy.
...
TL;DR: The paper introduces DeepLookEditBench (DLEBench), the first benchmark for evaluating Instruction-based Image Editing Models (IIEMs) on small-scale object editing, revealing significant performance gaps in this challenging task.


<details>
  <summary>Details</summary>
Motivation: Current IIEMs show strong reasoning on existing benchmarks but their ability to edit small objects remains underexplored, despite its importance for precise local editing and detail refinement in both real and generated images.

Method: Created DLEBench with 1889 samples across 7 instruction types where target objects occupy only 1%-10% of image area, covering complex scenarios like partial occlusion and multi-object editing. Proposed an evaluation protocol with refined score rubrics and dual-mode framework (Tool-driven and Oracle-guided Modes) to address misalignment between LMM-as-a-Judge and human judgments.

Result: Empirical evaluation of 10 IIEMs revealed significant performance gaps in small-scale object editing, demonstrating the need for specialized benchmarks to advance this capability.

Conclusion: DLEBench addresses a critical gap in evaluating IIEMs for small-scale object editing, providing a challenging testbed and robust evaluation protocol that reveals current model limitations and highlights the need for further research in this important area.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of structured 3D building reconstruction from point clouds with diverse distributions (airborne LiDAR, Structure-from-Motion) in a highly underconstrained setting, aiming to recover artist-created building abstractions.

Method: The method uses a Loosely Cascaded Diffusion Transformer (Loca-DiT) with two stages: 1) distribution recovery from noisy/sparse points using conditional latent diffusion models, and 2) autoregressive mesh generation using a decoder-only transformer conditioned on the recovered point clouds.

Result: The method shows substantial qualitative and quantitative improvements over prior building abstraction methods, and the recovered point clouds perform strongly on building point cloud completion benchmarks with improved surface accuracy and distribution uniformity.

Conclusion: BuildAnyPoint effectively addresses structured 3D building reconstruction from diverse point clouds by leveraging explicit 3D generative priors through a cascaded diffusion-transformer approach, demonstrating superior performance in both building abstraction and point cloud completion tasks.
...
TL;DR: MedMAP is a Medical Modality-Aware Pretraining framework that enhances vision-language representation learning for 3D MRI, addressing modality-specific alignment and cross-modal fusion challenges in multi-organ medical imaging.


<details>
  <summary>Details</summary>
Motivation: Applying vision-language models to multi-organ medical imaging faces two main challenges: modality-specific vision-language alignment and cross-modal feature fusion, which need to be addressed for effective diagnostic applications.

Method: MedMAP uses a two-stage approach: (1) modality-aware vision-language alignment pre-training where modality-aware encoders capture joint modality distributions, and (2) fine-tuning pre-trained vision encoders (with frozen text encoder) for downstream multi-organ abnormality detection tasks.

Result: Extensive experiments on MedMoM-MRI3D dataset (7,392 3D MRI volume-report pairs) show MedMAP significantly outperforms existing VLMs in 3D MRI-based multi-organ abnormality detection.

Conclusion: MedMAP effectively addresses modality-specific alignment and cross-modal fusion challenges in medical VLMs, demonstrating strong performance for multi-organ abnormality detection in 3D MRI and providing a valuable dataset (MedMoM-MRI3D) for 3D medical analysis tasks.
...
TL;DR: ProtoDCS: A robust open-set test-time adaptation framework for vision-language models that uses probabilistic verification and prototype-level updates to safely separate covariate-shifted in-distribution and out-of-distribution samples.


<details>
  <summary>Details</summary>
Motivation: Current VLM-based test-time adaptation methods fail in open-set scenarios where test streams contain both covariate-shifted in-distribution (csID) and out-of-distribution (csOOD) data. Existing open-set TTA methods use brittle hard thresholds and entropy minimization, leading to misclassification of ambiguous samples and overconfident predictions, with computationally prohibitive parameter updates for VLMs.

Method: ProtoDCS features: (1) a double-check separation mechanism using probabilistic Gaussian Mixture Model verification instead of brittle thresholding, and (2) an evidence-driven adaptation strategy with uncertainty-aware loss and efficient prototype-level updates to mitigate overconfidence and reduce computational overhead.
...
Conclusion: ProtoDCS effectively addresses the limitations of existing open-set TTA methods for VLMs by providing robust separation of csID and csOOD samples, enabling safe and efficient adaptation while maintaining computational feasibility.
...
TL;DR: Training-free inference-time control framework (SDLS) reduces prior-comparison hallucinations in radiology report generation by using LLM-driven semantic decomposition and QR orthogonalization to create semantic-free intervention vectors.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models for radiology report generation suffer from "prior-comparison hallucination" - generating historical findings not supported by current imaging studies, which poses clinical risks.

Method: Semantically Decoupled Latent Steering (SDLS): Uses LLM-driven semantic decomposition followed by QR-based orthogonalization to create semantic-free intervention vectors that target only the "historical comparison" axis without affecting clinical accuracy.

Result: Significantly reduces historical hallucinations (FilBERT score from 0.2373 to 0.1889) and improves clinical label fidelity (CheXpert macro-F1 from 0.2242 to 0.3208) on MIMIC-CXR, with robust zero-shot transfer to CheXpert Plus and IU-Xray.

Conclusion: SDLS effectively addresses prior-comparison hallucination in radiology report generation without compromising clinical accuracy, offering a training-free solution that maintains narrative integrity while improving reliability.
...
TL;DR: VL-WS is a vision-language framework for crop-weed segmentation that uses CLIP embeddings and natural language conditioning to improve generalization across diverse agricultural environments.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models for crop-weed segmentation struggle to generalize across heterogeneous agricultural environments due to reliance on dataset-specific visual features. There's a need for models that can work across different crop types, weed species, growth stages, and sensing conditions.

Method: Proposes Vision-Language Weed Segmentation (VL-WS) with dual-encoder design: frozen CLIP embeddings and task-specific spatial features fused via FiLM layers conditioned on natural language captions. Trained on unified corpus including both ground and UAV imagery covering diverse agricultural conditions.

Result: Achieves mean Dice score of 91.64%, outperforming CNN baseline by 4.98%. Largest gains on challenging weed class: 80.45% Dice score vs 65.03% for best baseline (15.42% improvement). Maintains stable performance under limited target-domain supervision.
...
TL;DR: rs-embed is a Python library that provides a unified interface for accessing and comparing remote sensing foundation model embeddings across different models, platforms, and data formats with minimal code.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of remote sensing foundation models has created heterogeneity in model release formats, platforms, interfaces, and input data specifications, making practical adoption and fair comparison difficult and increasing costs for obtaining, using, and benchmarking embeddings.

Method: Developed a Python library with a unified, region-of-interest (ROI) centric interface that allows users to retrieve embeddings from any supported model for any location and time range with a single line of code, plus efficient batch processing for large-scale embedding generation and evaluation.

Result: Created rs-embed library that simplifies access to remote sensing foundation model embeddings, reduces barriers to adoption and comparison, and enables large-scale embedding generation and evaluation through efficient batch processing.

Conclusion: rs-embed addresses the fragmentation in remote sensing foundation models by providing a standardized interface that lowers the cost and complexity of using and comparing different models, facilitating broader adoption and more equitable benchmarking in the field.
...
<details>
  <summary>Details</summary>
Motivation: Existing object swapping methods either require slow per-object finetuning or rely on paired data showing the same object across contexts, which forces models to use background cues rather than learning true cross-object alignment.

Method: Proposes SourceSwap with frequency-separated perturbation in initial-noise space to synthesize pseudo pairs that alter appearance while preserving pose, shape, and scene layout. Uses dual U-Net with full-source conditioning and noise-free reference encoder for direct inter-object alignment. Also introduces SourceBench benchmark with higher resolution, more categories, and richer interactions.

Result: SourceSwap achieves superior fidelity, stronger scene preservation, and more natural harmony compared to existing methods. It enables zero-shot inference without per-object finetuning and transfers well to other edits like subject-driven refinement and face swapping.

Conclusion: SourceSwap provides an effective self-supervised framework for object swapping that learns cross-object alignment through synthetic pseudo pairs, eliminating the need for paired data or per-object finetuning while achieving better performance and broader applicability.
...
TL;DR: HiDrop is a framework that reduces vision token computational cost in MLLMs by 90% while maintaining performance, using hierarchical token pruning aligned with layer functions.


<details>
  <summary>Details</summary>
Motivation: Current progressive vision token pruning methods misinterpret shallow layer functions and use rigid schedules, failing to unlock full efficiency potential in MLLMs. The quadratic computational cost of processing vision tokens hinders widespread adoption of MLLMs.

Method: HiDrop aligns token pruning with true hierarchical function of MLLM layers through: (1) Late Injection - bypasses passive shallow layers to introduce visual tokens where active fusion begins; (2) Concave Pyramid Pruning with Early Exit - dynamically adjusts pruning rates across middle and deep layers using inter-layer similarity measure and differentiable top-k operator; plus persistent positional encoding, FlashAttention-compatible token selection, and parallel decoupling of vision computation.
...
Conclusion: HiDrop not only achieves superior efficiency but also provides valuable insights into the hierarchical nature of multimodal fusion, offering a practical solution to computational bottlenecks in MLLMs.
...
TL;DR: EgoGraph is a training-free knowledge-graph framework that captures long-term dependencies in multi-day egocentric videos for better video understanding.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with ultra-long egocentric videos spanning multiple days due to fragmented local processing and limited temporal modeling, restricting their ability to reason over extended sequences.

Method: EgoGraph uses a novel egocentric schema to extract and abstract core entities (people, objects, locations, events) and structurally reason about their attributes/interactions. It employs temporal relational modeling to capture cross-entity dependencies and accumulate stable long-term memory over multiple days.

Result: Extensive experiments on EgoLifeQA and EgoR1-bench benchmarks show EgoGraph achieves state-of-the-art performance on long-term video question answering.

Conclusion: EgoGraph provides an effective new paradigm for ultra-long egocentric video understanding by explicitly encoding long-term, cross-entity dependencies through dynamic knowledge-graph construction.
...
TL;DR: U-MLLMs fail to maintain semantic equivalence across modalities - they reason well in text but collapse when generating visual answers, despite good rendering capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations assess multimodal understanding and generation separately, overlooking semantic equivalence - whether models can produce consistent reasoning results regardless of output modality. The paper investigates if current Unified Multimodal LLMs truly satisfy this premise.

Method: Introduces VGUBench framework with three diagnostic tasks: 1) Textual Generative Understanding (text reasoning baseline), 2) Visual Generative Understanding (visual answer generation), and 3) Visual Rendering control task (direct rendering without reasoning). This decouples reasoning logic from generation fidelity.

Result: Significant disparity found: U-MLLMs show strong textual reasoning and visual rendering capabilities, but performance collapses when generating visual answers to questions. Negligible correlation between visual answering performance and basic rendering quality, suggesting failure stems from cross-modal semantic alignment breakdown, not generation fidelity.

Conclusion: Current U-MLLMs lack true semantic equivalence across modalities. The failure is due to breakdown in cross-modal semantic alignment rather than insufficient generation capabilities. Provides diagnostic insights for future unified generation and understanding models.
...
TL;DR: Proposes a difference-in-difference method using second-order reconstruction error differences instead of first-order errors to improve AI-generated image detection as images become more realistic.
...
Method: Instead of using direct reconstruction error (first-order difference), the method computes the difference in reconstruction error (second-order difference) for variance reduction and improved detection accuracy.
...
Conclusion: The proposed difference-in-difference method effectively addresses the challenge of detecting increasingly realistic AI-generated images by using second-order error differences rather than first-order reconstruction errors.
...
TL;DR: UTPTrack is a unified token pruning framework for Transformer-based trackers that jointly compresses search region, dynamic template, and static template tokens using attention-guided strategy, achieving state-of-the-art efficiency-accuracy trade-off for both RGB and multimodal tracking.


<details>
  <summary>Details</summary>
Motivation: Current one-stream Transformer trackers have high computational overhead that prevents real-time deployment. Existing token pruning methods are fragmented - they prune different components (search region, dynamic template, static template) in isolation, ignoring inter-component dependencies, leading to suboptimal pruning and accuracy degradation.

Method: UTPTrack introduces a unified token pruning framework that jointly compresses all three components (search region, dynamic template, static template) using an attention-guided, token type-aware strategy to holistically model redundancy. The design supports unified tracking across multimodal and language-guided tasks within a single model.

Result: Extensive evaluations on 10 benchmarks show UTPTrack achieves state-of-the-art accuracy-efficiency trade-off: prunes 65.4% of vision tokens in RGB-based tracking and 67.5% in unified tracking while preserving 99.7% and 100.5% of baseline performance respectively. Strong performance across both RGB and multimodal scenarios.

Conclusion: UTPTrack provides a robust foundation for future research in efficient visual tracking by offering a simple yet effective unified token pruning framework that addresses the limitations of fragmented pruning approaches and maintains strong performance across diverse tracking scenarios.
...
TL;DR: U-Mind is the first unified system for real-time multimodal dialogue that jointly models language, speech, motion, and video synthesis in a single interactive loop, achieving state-of-the-art performance through unified alignment and reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing systems for multimodal interaction are limited to unimodal generation or suffer from degraded reasoning and poor cross-modal alignment, preventing coherent and perceptually grounded interactions needed for intelligent embodied agents.

Method: U-Mind implements a Unified Alignment and Reasoning Framework with segment-wise alignment for cross-modal synchronization and Rehearsal-Driven Learning to preserve reasoning abilities. It uses text-first decoding with internal chain-of-thought planning followed by temporally synchronized generation across modalities, plus real-time video rendering conditioned on pose and speech.

Result: U-Mind achieves state-of-the-art performance on various multimodal interaction tasks including question answering, instruction following, and motion generation, demonstrating superior cross-modal synchronization and reasoning capabilities.

Conclusion: U-Mind represents a significant advancement toward intelligent, immersive conversational agents by enabling real-time, coherent multimodal interactions through unified modeling of language, speech, motion, and video synthesis.
...
TL;DR: Selfment: A fully self-supervised framework for object segmentation without any manual annotations, pretrained models, or post-processing, achieving state-of-the-art results across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: The core challenge in computer vision is accurately segmenting objects without manual annotations. Current methods often rely on human labels, pretrained models, or post-processing, which limits their applicability and scalability.

Method: Selfment constructs patch-level affinity graphs from self-supervised features and applies Normalized Cut (NCut) for initial foreground-background separation. It then introduces Iterative Patch Optimization (IPO) for feature-space refinement through iterative patch clustering. Finally, refined masks train a lightweight segmentation head with contrastive and region-consistency objectives.

Result: Achieves substantial improvements on F_max over previous unsupervised methods: +4.0% on ECSSD, +4.6% on HKUIS, +5.7% on PASCAL-S. Demonstrates remarkable zero-shot generalization to camouflaged object detection (0.910 S_m on CHAMELEON, 0.792 F_β^ω on CAMO), outperforming all unsupervised approaches and rivaling state-of-the-art supervised methods.

Conclusion: Selfment presents a simple yet effective fully self-supervised framework that achieves state-of-the-art segmentation performance without any manual supervision, demonstrating strong generalization capabilities to challenging tasks like camouflaged object detection.
...
TL;DR: Diffusion Probe: A framework that predicts final image quality from early cross-attention distributions in text-to-image diffusion models, enabling early quality assessment to reduce computational waste.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models lack efficient early quality assessment mechanisms, leading to costly trial-and-error in multi-generation scenarios like prompt iteration and agent-based generation.

Method: Leverages internal cross-attention maps as predictive signals, using a lightweight predictor that maps statistical properties of early-stage cross-attention from initial denoising steps to final image quality.

Result: Achieves strong correlation (PCC > 0.7) and high classification performance (AUC-ROC > 0.9) across multiple T2I models, early denoising windows, resolutions, and quality metrics.

Conclusion: Diffusion Probe is model-agnostic, efficient, and broadly applicable, offering practical solution for improving T2I generation efficiency through early quality prediction, enabling early quality-aware decisions in workflows.
...
TL;DR: Fourier Angle Alignment (FAA) method addresses directional incoherence and task conflict in rotated object detection using Fourier rotation equivariance, achieving state-of-the-art results on remote sensing datasets.


<details>
  <summary>Details</summary>
Motivation: Mainstream rotated object detection methods suffer from two key bottlenecks: directional incoherence at the detector neck (where features from different levels have inconsistent orientations) and task conflict at the detection head (where classification and regression tasks interfere with each other due to angle variations).

Method: Proposes Fourier Angle Alignment (FAA) that analyzes angle information through frequency spectrum and aligns main directions to certain orientations. Introduces two plug-and-play modules: 1) FAAFusion at detector neck - aligns higher-level features to lower-level features and fuses them, 2) FAA Head - pre-aligns RoI features to canonical angle and adds to original features before classification and regression.

Result: Achieves state-of-the-art results: 78.72% mAP on DOTA-v1.0 and 72.28% mAP on DOTA-v1.5 datasets with single scale training/testing. Also tested on HRSC2016. Method significantly improves previous work and validates efficacy in remote sensing object detection.

Conclusion: Fourier Angle Alignment effectively addresses directional incoherence and task conflict in rotated object detection through Fourier rotation equivariance. The plug-and-play modules (FAAFusion and FAA Head) provide practical solutions that can be integrated into existing detectors to improve performance on remote sensing datasets.
...
TL;DR: Sea² adapts perception model deployment via pose-control agent instead of fine-tuning, using frozen perception modules and scalar feedback to navigate toward informative viewpoints.


<details>
  <summary>Details</summary>
Motivation: Pre-trained perception models degrade in novel environments like indoor scenes. Fine-tuning causes catastrophic forgetting and requires expensive scene-specific annotations. Need a way to adapt deployment rather than the models themselves.

Method: Sea² transforms a vision-language model into a low-level pose controller through two-stage training: 1) fine-tuning on rule-based exploration trajectories, 2) refining via unsupervised RL using perception module outputs and confidence as rewards. Keeps all perception modules frozen, requires no downstream labels.
...
Conclusion: Sea² enables direct use of off-the-shelf perception models without retraining by adapting deployment through intelligent pose control, achieving significant performance gains while avoiding catastrophic forgetting and annotation costs.
...
TL;DR: A framework for bimanual manipulation that uses a pre-trained 3D geometric foundation model to predict both actions and future 3D scene geometry from RGB images, outperforming 2D and point-cloud baselines.


<details>
  <summary>Details</summary>
Motivation: Bimanual manipulation requires 3D geometric reasoning and coordinated motions, but existing methods rely on 2D features with limited spatial awareness or require explicit point clouds that are difficult to obtain reliably in real-world settings.

Method: Leverages a pre-trained 3D geometric foundation model to reconstruct 3D structure from RGB images. The policy fuses geometry-aware latents, 2D semantic features, and proprioception into a unified state representation, using a diffusion model to jointly predict future action chunks and future 3D latents that decode into dense pointmaps.

Result: Outperforms 2D-based and point-cloud-based baselines in both simulation (RoboTwin benchmark) and real-world robot executions, achieving state-of-the-art performance in manipulation success, inter-arm coordination, and 3D spatial prediction accuracy.

Conclusion: By explicitly predicting how 3D scenes evolve together with action sequences, the policy gains strong spatial understanding and predictive capability using only RGB observations, enabling effective bimanual manipulation without requiring explicit point clouds.
...
TL;DR: A continual learning framework for pathology report generation from whole-slide images that avoids catastrophic forgetting without storing past data, using compact domain footprints and generative replay.


<details>
  <summary>Details</summary>
Motivation: Clinical deployment of pathology report generation faces challenges with evolving data (new organs, institutions, reporting conventions) over time, and sequential fine-tuning causes catastrophic forgetting. Current approaches assume static training with all data available simultaneously, which doesn't match real-world clinical settings.

Method: Introduces an exemplar-free continual learning framework using compact domain footprints: codebooks of representative morphology tokens, slide-level co-occurrence summaries, and patch-count priors in frozen patch-embedding space. Uses generative replay with synthesized pseudo-WSI representations and teacher snapshots for pseudo-reports. Distills domain-specific linguistic characteristics into style descriptors for steering generation, with inference identifying compatible descriptors from slide signals.

Result: Outperforms exemplar-free and limited-buffer rehearsal baselines across multiple public continual learning benchmarks, demonstrating effective continual learning without storing raw data.

Conclusion: Footprint-based generative replay provides a practical solution for deploying pathology report generation in evolving clinical settings, addressing catastrophic forgetting while maintaining data privacy by avoiding storage of raw slides or exemplars.
...
TL;DR: CPN-YOLO improves YOLOv8 for SAR ship detection with denoising, attention enhancement, and Gaussian similarity loss, achieving state-of-the-art performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: SAR ship detection faces challenges in complex scenes due to clutter, speckle noise causing false alarms, and difficulty detecting small targets. Existing methods need improvement for robust performance.

Method: Three improvements to YOLOv8: 1) Learnable large-kernel denoising module for input pre-processing, 2) PPA attention mechanism for feature extraction enhancement and multi-scale modeling, 3) Gaussian similarity loss based on normalized Wasserstein distance for better bounding-box similarity measurement.

Result: On SSDD dataset: 97.0% precision, 95.1% recall, 98.9% mAP, surpassing YOLOv8 baseline and outperforming other deep-learning detectors. Also validated on HRSID dataset.

Conclusion: CPN-YOLO effectively addresses SAR ship detection challenges in complex scenes through targeted architectural improvements, demonstrating superior performance and generalization capability.
...
TL;DR: APPO is an Attention-guided Perception Policy Optimization algorithm that improves video reasoning by enhancing fine-grained perception through token-level dense rewards, outperforming existing methods by 0.5%-4% across benchmarks.


<details>
  <summary>Details</summary>
Motivation: The paper identifies that complex video reasoning relies more on fine-grained perception than expert-level reasoning. When perception ability is fixed, improving reasoning yields minimal gains (0.7%), while enhancing perception provides substantial improvements (1.4%). The goal is to enhance perception through reasoning without expensive fine-grained annotations.

Method: APPO (Attention-guided Perception Policy Optimization) uses token-level dense rewards to optimize intra-group perception tokens - tokens from different responses that focus on the same crucial video frame. The algorithm leverages attention mechanisms to identify and enhance these perception-focused tokens.

Result: APPO consistently outperforms GRPO and DAPO by 0.5% to 4% across diverse video benchmarks and models of different scales (3B/7B). The approach demonstrates that perception enhancement through reasoning is more effective than pure reasoning improvement.

Conclusion: APPO provides a promising low-cost approach to enhance model perception abilities through reasoning, serving diverse scenarios and demands. The work highlights the critical importance of perception over reasoning in video understanding tasks.
...
TL;DR: Multi-modal multi-task model for detecting AI-generated images and identifying their source models, achieving 5th place in competition with 83.16% and 48.88% F1 scores.


<details>
  <summary>Details</summary>
Motivation: To detect AI-generated images and identify the specific AI models that created them, addressing the growing need for AI-generated content detection in real-world scenarios.

Method: Uses pre-trained BERT for text features and CLIP Vision encoder for image features, with cross-modal feature fusion and tailored multi-task loss. Employs pseudo-labeling-based data augmentation to expand training dataset with high-confidence samples.

Result: Achieved 5th place in both Tasks A (detection) and B (model identification) of the CT2 competition, with F1 scores of 83.16% for detection and 48.88% for model identification.

Conclusion: The proposed architecture is effective for AI-generated content detection and has potential for real-world applications. Source code is publicly available on GitHub.
...
TL;DR: Altitude-adaptive visual place recognition using ground feature density analysis for aerial platforms under significant altitude variations.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of aerial visual place recognition (VPR) under significant altitude variations, especially for small- and medium-sized airborne platforms that may lack additional hardware like barometric altimeters or ToF sensors.

Method: Estimates relative altitude by analyzing ground feature density in images, applies altitude-based cropping to generate canonical query images, and uses classification-based VPR strategy for localization.

Result: Boosts average R@1 and R@5 by 29.85% and 60.20% respectively compared to VPR alone; reduces mean error by 202.1m compared to MMDE methods; achieves 31.4% and 44% additional improvements in R@1 and R@5.

Conclusion: Establishes a robust, vision-only framework for 3D visual place recognition that offers a practical, scalable solution for accurate airborne platform localization under large altitude variations without additional hardware.
...
TL;DR: ReSeg-CLIP: Training-free Open-Vocabulary Semantic Segmentation for remote sensing using SAM masks and multi-model composition.


<details>
  <summary>Details</summary>
Motivation: To address limitations of vision-language models like CLIP in semantic segmentation, particularly inappropriate interactions within self-attention layers when applied to remote sensing data.

Method: 1) Hierarchical scheme using SAM-generated masks to constrain self-attention interactions at multiple scales; 2) Model composition averaging parameters of multiple RS-specific CLIP variants with new weighting scheme based on representational quality using varying text prompts.

Result: Achieves state-of-the-art results across three remote sensing benchmarks without additional training.

Conclusion: ReSeg-CLIP provides an effective training-free solution for open-vocabulary semantic segmentation in remote sensing by addressing CLIP's attention limitations through hierarchical mask constraints and optimized model composition.
...
TL;DR: The paper proposes AoE (Always-on Egocentric), a system that uses humans with smartphones to collect large-scale, low-cost real-world interaction data for embodied AI training, addressing data scarcity through distributed collection and automated processing.


<details>
  <summary>Details</summary>
Motivation: Current embodied AI models need large-scale real-world interaction data, but existing collection methods are expensive, hardware-dependent, and limited in scope. Humans are ideal embodied agents, and using their smartphones offers a low-cost, sustainable solution for scalable data collection.

Method: 1) Neck-mounted smartphone holder for ergonomic egocentric data collection; 2) Cloud-edge collaborative architecture with mobile app for real-time on-device processing; 3) Cloud-based automated labeling and filtering pipelines to transform raw videos into training data; 4) Distributed collection system accessible to anyone, anytime, anywhere.

Result: The AoE system enables high-quality egocentric data collection at scale. Evaluation shows that data collected through this approach significantly improves real-world generalization in downstream tasks compared to existing methods.

Conclusion: The AoE system successfully addresses the data scarcity problem for embodied foundation models by leveraging humans as embodied agents with smartphones, enabling low-cost, scalable, and scene-agnostic real-world interaction data collection that enhances model generalization.
...
TL;DR: Hybrid V2X offloading system for autonomous driving that splits BEV transformer processing between vehicle and cloud, using dynamic optimization to balance latency and accuracy under varying network conditions.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles face latency challenges in real-time obstacle detection due to limited onboard compute resources, especially in complex urban environments where processing demands are high.

Method: Uses V2X communication to offload partial processing to cloud. Employs transformer-based models to fuse multi-camera data into BEV representation for 3D object detection. Dynamically splits computation between vehicle and cloud based on layers processed locally and feature quantization. Applies feature vector clipping and compression to reduce network load.

Result: 72% reduction in end-to-end latency compared to traditional onboard solution. Dynamic optimization improves accuracy by up to 20% over static parameterization while maintaining same latency performance under realistic bandwidth variability.

Conclusion: Hybrid V2X offloading with dynamic optimization effectively addresses latency constraints in autonomous driving while maintaining high detection accuracy, adapting to fluctuating network conditions for robust real-time performance.
...
TL;DR: RAM-based real embedding extractor with degradation selection and Mamba-based network for real-world image super-resolution


<details>
  <summary>Details</summary>
Motivation: Multimodal large models have limited ability in handling degraded images for real-world super-resolution tasks, despite their success with language conditions.

Method: 1) Analyze RAM's degraded image capabilities via text similarity; 2) Propose Real Embedding Extractor (REE) with degradation selection strategy using contrastive learning; 3) Use Conditional Feature Modulator (CFM) to incorporate REE's high-level info into Mamba-based network for texture restoration.

Result: REE significantly improves recognition on degraded images and helps super-resolution networks balance fidelity and perceptual quality, demonstrating Mamba's potential in real-world applications.

Conclusion: The proposed approach effectively addresses degraded image super-resolution by combining REE with Mamba-based architecture, achieving visually pleasing results with good fidelity-perception balance.
...
TL;DR: Self-supervised 3D occupancy flow estimation without human annotations or external flow supervision, using disentangled static/dynamic SDFs and temporal aggregation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D occupancy and motion estimation require expensive annotations (3D occupancy/flow labels, velocity from bounding boxes, or pretrained optical flow models), which limits scalability and practicality for autonomous driving applications.

Method: Proposes a self-supervised approach that: 1) Disentangles scene into separate static and dynamic signed distance fields (SDFs), 2) Learns motion implicitly through temporal aggregation, 3) Introduces a strong self-supervised flow cue using features' cosine similarities.

Result: Demonstrates efficacy on three major autonomous driving datasets: SemanticKITTI, KITTI-MOT, and nuScenes, showing the method works without human annotations or external flow supervision.

Conclusion: The proposed self-supervised method successfully estimates 3D occupancy flow without expensive annotations, making it more scalable and practical for autonomous driving applications while maintaining competitive performance on standard benchmarks.
...
TL;DR: Ref-Adv is a new challenging benchmark for Referring Expression Comprehension that suppresses shortcuts by using linguistically complex expressions with only necessary information and hard distractors, revealing weaknesses in current multimodal LLMs' visual reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Standard REC benchmarks (RefCOCO, RefCOCO+, RefCOCOg) are weak tests of visual reasoning because they contain short expressions, few distractors, and redundant descriptors that enable shortcut solutions, failing to evaluate genuine text understanding and visual reasoning.

Method: Created Ref-Adv benchmark with linguistically nontrivial expressions paired with only necessary information to uniquely identify targets, using real images with hard distractors, annotated with reasoning facets including negation, and conducted comprehensive ablations (word order perturbations and descriptor deletion sufficiency).

Result: Despite strong performance on standard benchmarks, contemporary multimodal LLMs show significant performance drops on Ref-Adv, revealing reliance on shortcuts and gaps in visual reasoning and grounding capabilities.

Conclusion: Ref-Adv exposes limitations in current MLLMs' visual reasoning and provides a more challenging benchmark to guide future work on improving genuine text understanding and visual grounding in multimodal systems.
...
TL;DR: BUSD-Agent is a two-stage multi-agent framework for breast ultrasound screening and diagnosis that uses experience-guided decision-making to reduce unnecessary diagnostic escalations and biopsy referrals by learning from past cases.


<details>
  <summary>Details</summary>
Motivation: To address the problem of excessive diagnostic escalation and unnecessary biopsy referrals in breast ultrasound screening, which leads to patient anxiety, healthcare costs, and resource strain.

Method: A cascaded two-agent framework: 1) Screening clinic agent uses classification models to filter low-risk cases; 2) Diagnostic clinic agent with richer tools handles escalated cases. Both agents use a memory bank of past decision trajectories for retrieval-conditioned in-context adaptation without parameter updates.

Result: Evaluation across 10 datasets shows significant reductions: diagnostic escalation from 84.95% to 58.72%, biopsy referrals from 59.50% to 37.08%, while improving screening specificity by 68.48% and diagnostic specificity by 6.33%.

Conclusion: The experience-guided BUSD-Agent framework effectively reduces unnecessary diagnostic procedures while maintaining diagnostic accuracy, demonstrating the value of retrieval-conditioned adaptation in medical decision-making systems.
...
TL;DR: SegMate is an efficient 2.5D medical image segmentation framework that reduces computational requirements by 2.5x while improving accuracy by ~1% through asymmetric architectures, attention mechanisms, and multi-scale fusion.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art medical image segmentation models require substantial computational resources, limiting deployment in resource-constrained clinical settings where efficiency is crucial.

Method: A 2.5D framework integrating asymmetric architectures, attention mechanisms, multi-scale feature fusion, slice-based positional conditioning, and multi-task optimization across three backbones (EfficientNetV2-M, MambaOut-Tiny, FastViT-T12).

Result: Reduces computation by up to 2.5x (GFLOPs) and memory by up to 2.1x (VRAM) while gaining ~1% performance improvement. Achieves 93.51% Dice on TotalSegmentator with only 295MB GPU memory, with strong zero-shot generalization on SegTHOR (86.85%) and AMOS22 (89.35%).

Conclusion: SegMate provides an efficient solution for medical image segmentation that maintains high accuracy while significantly reducing computational requirements, enabling deployment in resource-constrained clinical environments.
...
TL;DR: CLIP models often incorrectly increase similarity scores when wrong details are added to correct descriptions (half-truths). CS-CLIP addresses this by decomposing captions into components and fine-tuning to ground individual entities and relations.


<details>
  <summary>Details</summary>
Motivation: CLIP-style models show a vulnerability where adding plausible but incorrect details to correct descriptions actually increases similarity scores, violating the intuition that wrong details should decrease similarity. This reveals weak supervision on caption parts during contrastive training.

Method: Proposes CS-CLIP which: 1) decomposes captions into entity and relation units, 2) constructs minimally edited foils for each unit, 3) fine-tunes the model to score correct units above their foils while preserving standard dual-encoder inference.

Result: On COCO, CLIP prefers correct shorter descriptions only 40.6% of the time (32.9% for relations). CS-CLIP raises half-truth accuracy to 69.3% and improves average performance on established compositional benchmarks by 5.7 points.

Conclusion: Reducing half-truth errors through component-level supervision aligns with broader gains in compositional understanding, suggesting that explicit grounding of individual caption components improves model robustness.
...
TL;DR: Topology-Driven Transferability Estimation framework for selecting optimal medical foundation models for segmentation tasks without fine-tuning, outperforming existing methods by 31%.


<details>
  <summary>Details</summary>
Motivation: Existing Transferability Estimation metrics for medical foundation models are designed for classification tasks and fail to capture the topological complexity needed for dense prediction tasks like segmentation. There's a need for efficient, training-free methods to select optimal models for specific medical segmentation tasks.

Method: Proposes a Topology-Driven Transferability Estimation framework with three components: 1) Global Representation Topology Divergence (GRTD) using Minimum Spanning Trees to quantify feature-label structural isomorphism, 2) Local Boundary-Aware Topological Consistency (LBTC) assessing manifold separability at anatomical boundaries, and 3) Task-Adaptive Fusion that dynamically integrates global and local metrics based on task semantic cardinality.

Result: Validated on the large-scale OpenMind benchmark across diverse anatomical targets and SSL foundation models, the approach significantly outperforms state-of-the-art baselines by around 31% relative improvement in weighted Kendall correlation.

Conclusion: The proposed framework provides a robust, training-free proxy for efficient medical foundation model selection for segmentation tasks without the computational cost of fine-tuning, addressing the limitations of existing classification-focused transferability estimation methods.
...
TL;DR: GPU-SDF improves neural implicit surface reconstruction by explicitly estimating geometric prior uncertainty and using it to modulate prior influence, plus adding edge distance field and multi-view consistency constraints for better fine detail recovery.


<details>
  <summary>Details</summary>
Motivation: Existing neural implicit surface reconstruction methods struggle with fine details like thin structures and complex geometries due to unreliable/noisy geometric priors. Current approaches use implicit uncertainty filtering which is indirect and inefficient, and masking supervision in high-uncertainty regions leads to under-constrained optimization.
...
Conclusion: GPU-SDF addresses limitations of existing neural implicit surface reconstruction methods by explicitly handling geometric prior uncertainty and adding complementary constraints, leading to better recovery of fine details in indoor scenes.
...
TL;DR: PointCoT introduces explicit Chain-of-Thought reasoning for 3D point cloud understanding in MLLMs to address geometric hallucinations in current implicit approaches.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with 3D point cloud understanding as they treat geometric reasoning as implicit mapping, leading to geometric hallucinations where models generate plausible but structurally inaccurate responses.

Method: Proposes PointCoT framework with "Look, Think, then Answer" paradigm using explicit Chain-of-Thought reasoning. Creates Point-Reason-Instruct benchmark (86k samples) with hierarchical CoT annotations and employs dual-stream multimodal architecture combining semantic appearance with geometric truth.

Result: Extensive experiments show PointCoT achieves state-of-the-art performance on complex reasoning tasks for 3D point cloud understanding.

Conclusion: Explicit geometric reasoning through Chain-of-Thought approach effectively addresses geometric hallucinations in MLLMs for 3D perception, enabling more accurate and grounded 3D understanding.
...
TL;DR: A dual-branch micro-expression recognition network with parallel attention achieves 74.67% accuracy on CASME II, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Micro-expressions are transient and subtle, making them challenging for existing optical flow-based recognition methods to detect accurately.

Method: Proposes a dual-branch feature extraction network with parallel attention, including: 1) residual network to prevent gradient vanishing, 2) Inception network to enhance representation and suppress irrelevant regions, and 3) adaptive feature fusion module to integrate dual-branch features.
...
TL;DR: AHAP reconstructs 3D humans from arbitrary camera perspectives without camera calibration by fusing multi-view geometry for identity association, SMPL prediction, and triangulation-based localization.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view 3D human reconstruction methods require pre-calibration (checkerboards or MVS algorithms), which limits scalability and applicability in diverse real-world scenarios.

Method: A feed-forward framework with: 1) Cross-View Identity Association using learnable person queries and soft assignment with contrastive learning; 2) Human Head for SMPL prediction with cross-view reprojection losses; 3) Multi-view triangulation for precise 3D localization.

Result: AHAP achieves competitive performance on world-space human reconstruction and camera pose estimation on EgoHumans and EgoExo4D datasets, while being 180× faster than optimization-based approaches.

Conclusion: AHAP enables calibration-free 3D human reconstruction from arbitrary perspectives by effectively fusing multi-view geometry, offering scalability and practical applicability in real-world scenarios.
...
TL;DR: CC-VQA is a training-free method that addresses knowledge conflicts in KB-VQA by analyzing visual-semantic conflicts and using correlation-guided encoding/decoding, achieving SOTA performance with 3.3-6.4% accuracy improvements.


<details>
  <summary>Details</summary>
Motivation: Current KB-VQA methods suffer from conflicts between static parametric knowledge in VLMs and dynamically retrieved information, leading to outputs that either ignore retrieved contexts or show inconsistent integration. Existing conflict mitigation methods adapted from language-based approaches neglect visual information and suffer from redundant retrieved contexts.

Method: CC-VQA has two core components: (1) Vision-Centric Contextual Conflict Reasoning for visual-semantic conflict analysis across internal and external knowledge contexts, and (2) Correlation-Guided Encoding and Decoding featuring positional encoding compression for low-correlation statements and adaptive decoding using correlation-weighted conflict scoring.
...
Conclusion: CC-VQA effectively addresses knowledge conflicts in KB-VQA by incorporating visual information into conflict analysis and using correlation-aware mechanisms, demonstrating superior performance without requiring additional training.
...
TL;DR: GDA-YOLO11 amodal segmentation model improves robotic fruit harvesting by detecting occluded fruits, achieving better precision and recall than YOLO11n, with harvesting success rates up to 92.59% under varying occlusion levels.


<details>
  <summary>Details</summary>
Motivation: Occlusion in robotic fruit harvesting leads to substantial crop losses when fruits are undetected or inaccurately localized. Current methods struggle with occluded fruits, creating a need for more robust perception systems.

Method: Proposed GDA-YOLO11 amodal segmentation model with architectural improvements and asymmetric mask loss, trained on modified citrus dataset. Full fruit masks (including invisible regions) are inferred, picking points estimated via Euclidean distance transform, then projected to 3D coordinates for robotic execution.

Result: GDA-YOLO11 achieves precision 0.844, recall 0.846, mAP@50 0.914, mAP@50:95 0.636, outperforming YOLO11n by 5.1%, 1.3%, and 1.0% respectively. Harvesting success rates: 92.59% (zero occlusion), 85.18% (low), 48.14% (medium), 22.22% (high), with 3.5% improvement under medium/high occlusion.

Conclusion: GDA-YOLO11 enhances occlusion-robust segmentation and streamlines perception-to-action integration, providing the first practical demonstration of amodal instance segmentation in robotic fruit harvesting and paving the way for more reliable autonomous agricultural systems.
...
TL;DR: SwitchCraft is a training-free framework for multi-event video generation that uses Event-Aligned Query Steering and Auto-Balance Strength Solver to improve prompt alignment and scene consistency.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video diffusion models are optimized for single-event generation and struggle with multi-event prompts, producing blended or collapsed scenes that break the intended narrative.

Method: SwitchCraft introduces Event-Aligned Query Steering (EAQS) to steer frame-level attention to align with relevant event prompts, and Auto-Balance Strength Solver (ABSS) to adaptively balance steering strength for temporal consistency and visual fidelity.

Result: Extensive experiments show SwitchCraft substantially improves prompt alignment, event clarity, and scene consistency compared with existing baselines.

Conclusion: SwitchCraft offers a simple yet effective training-free solution for multi-event video generation by addressing the limitations of uniform prompt injection across time.
...
TL;DR: NV-CoT enables multimodal LLMs to perform visual reasoning using continuous numerical coordinates instead of textified coordinates or fixed patches, improving localization precision and answer accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs use either textified coordinates (causing modality mismatch and semantic fragmentation) or fixed-granularity patches (limiting precise region selection and requiring architectural changes), which are suboptimal for region-grounded reasoning.

Method: Proposes Numerical Visual Chain-of-Thought (NV-CoT) that expands MLLM action space to continuous Euclidean space, allowing direct bounding-box coordinate generation with minimal architectural changes. Uses Gaussian/Laplace policies with reparameterized sampling, compatible with GRPO-style policy optimization via supervised fine-tuning and reinforcement learning.
...
Conclusion: NV-CoT validates the effectiveness of continuous-action visual reasoning in MLLMs, providing a framework that overcomes limitations of existing approaches for region-grounded reasoning.
...
TL;DR: SpikeTrack is a novel spike-driven framework for energy-efficient RGB object tracking that uses asymmetric design and memory-retrieval modules to achieve state-of-the-art performance among SNN trackers while consuming minimal energy.
...
Method: SpikeTrack employs an asymmetric design with asymmetric timestep expansion and unidirectional information flow, plus a memory-retrieval module that recurrently queries compact memory initialized by templates to retrieve target cues and sharpen perception over time.

Result: SpikeTrack achieves state-of-the-art among SNN-based trackers and remains competitive with advanced ANN trackers, surpassing TransT on LaSOT dataset while consuming only 1/26 of its energy.

Conclusion: SpikeTrack is the first spike-driven framework to make RGB tracking both accurate and energy efficient, demonstrating that SNNs can achieve competitive performance in visual tracking while maintaining significant energy advantages.
...
TL;DR: AesGuide introduces the first large-scale aesthetic guidance dataset and Venus framework that empowers MLLMs to provide actionable aesthetic feedback and achieve SOTA performance in aesthetic cropping.


<details>
  <summary>Details</summary>
Motivation: There's a gap between ordinary users and professional photographers who can identify aesthetic issues and provide actionable shooting guidance during capture. Existing MLLMs offer overly positive feedback without identifying issues or providing actionable guidance, lacking aesthetic guidance capability.

Method: Introduces AesGuide dataset (10,748 photos with aesthetic scores, analyses, guidance) and Venus framework with two stages: 1) empowers MLLMs with AG capability through progressively complex aesthetic questions, 2) activates aesthetic cropping power via CoT-based rationales.

Result: Venus substantially improves aesthetic guidance capability and achieves state-of-the-art performance in aesthetic cropping, enabling interpretable and interactive aesthetic refinement across both photo creation stages.

Conclusion: The work addresses the underexplored domain of aesthetic guidance, providing both dataset and framework that bridge the gap between ordinary users and professional photographers through actionable feedback and cropping capabilities.
...
TL;DR: MIGM-Shortcut accelerates masked image generation models by learning a lightweight model that predicts feature evolution using previous features and sampled tokens, achieving 4x speedup without quality loss.


<details>
  <summary>Details</summary>
Motivation: Masked Image Generation Models (MIGMs) suffer from computational inefficiency due to multiple bi-directional attention steps. Existing feature caching methods have high approximation errors at aggressive acceleration rates because they lack expressivity and ignore sampling information.

Method: Proposes MIGM-Shortcut: a lightweight model that incorporates both previous features and sampled tokens to regress the average velocity field of feature evolution. The model has moderate complexity to capture subtle dynamics while remaining lightweight compared to the original base model.

Result: Applied to two representative MIGM architectures and tasks. On state-of-the-art Lumina-DiMOO, achieves over 4x acceleration of text-to-image generation while maintaining quality, significantly pushing the Pareto frontier of masked image generation.

Conclusion: MIGM-Shortcut effectively addresses computational redundancy in MIGMs by learning feature evolution dynamics, enabling substantial acceleration without compromising generation quality.
...
TL;DR: An ordinal latent diffusion model for generating diabetic retinopathy fundus images that captures continuous disease progression instead of treating stages as independent classes.


<details>
  <summary>Details</summary>
Motivation: Current generative models treat disease stages as independent classes, ignoring the continuous nature of disease progression. This is problematic in medical imaging where pathological processes are continuous but observed through coarse, discrete ordered labels.

Method: Proposed an ordinal latent diffusion model using scalar disease representation instead of categorical conditioning, enabling smooth transitions between adjacent DR severity stages. Evaluated on EyePACS dataset using visual realism metrics and classification-based clinical consistency analysis.

Result: Model reduced Fréchet inception distance for 4 of 5 DR stages and increased quadratic weighted κ from 0.79 to 0.87 compared to standard conditional diffusion model. Interpolation experiments showed the model captured continuous spectrum of disease progression.

Conclusion: The ordinal latent diffusion model successfully incorporates ordered disease structure into image generation, better capturing continuous disease progression from coarse ordered labels, with improved performance over standard categorical approaches.
...
TL;DR: DeBiasLens is an interpretable, model-agnostic framework that uses sparse autoencoders to localize social attribute neurons in Vision-Language Models and mitigate bias by selectively deactivating them.


<details>
  <summary>Details</summary>
Motivation: Vision-Language models have black-box reasoning processes that could lead to unintended social bias. Current debiasing approaches only address surface-level bias signals without exploring internal model dynamics.

Method: Uses sparse autoencoders (SAEs) applied to multimodal encoders to localize social attribute neurons. Trains SAEs on facial image or caption datasets without social attribute labels to uncover neurons responsive to specific demographics. Selectively deactivates social neurons most strongly tied to bias for each group.
...
Conclusion: Lays groundwork for future auditing tools that prioritize social fairness in real-world AI systems by providing an interpretable approach to bias mitigation that preserves model performance.
...
TL;DR: SR3R: A feed-forward framework that directly predicts high-resolution 3D Gaussian Splatting from sparse low-resolution views, enabling robust cross-scene generalization without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Existing 3D super-resolution methods rely on dense inputs and per-scene optimization, limiting reconstruction fidelity, generalization, and real-time usability. They inherit high-frequency priors from 2D super-resolution models rather than learning 3D-specific geometry and appearance.

Method: SR3R reformulates 3D super-resolution as a direct feed-forward mapping from sparse LR views to HR 3DGS representations. It uses a learned mapping network with Gaussian offset learning and feature refinement to stabilize reconstruction and sharpen details. The framework is plug-and-play with any feed-forward 3DGS backbone.

Result: Extensive experiments across three 3D benchmarks show SR3R surpasses state-of-the-art 3DSR methods and achieves strong zero-shot generalization, even outperforming per-scene optimization methods on unseen scenes.

Conclusion: SR3R fundamentally changes how 3D super-resolution acquires high-frequency knowledge by learning 3D-specific priors from large-scale data, enabling robust generalization and real-time usability without per-scene optimization.
...
TL;DR: SteerVAD is a novel intervention framework that actively steers frozen multi-modal large language models (MLLMs) for video anomaly detection, achieving SOTA performance with only 1% training data through targeted representation rectification.


<details>
  <summary>Details</summary>
Motivation: Traditional VAD methods require expensive labeled data and full training, while existing MLLM-based approaches inherit pre-training biases and cannot adapt to specific video contexts, struggling with subtle or ambiguous anomalies.

Method: Uses gradient-free representational separability analysis (RSA) to identify top attention heads as latent anomaly experts (LAEs), then employs a hierarchical meta-controller (HMC) to generate dynamic rectification signals that perform anisotropic scaling on LAE representation manifolds to amplify anomaly-relevant dimensions while suppressing biases.

Result: Achieves state-of-the-art performance on mainstream benchmarks among tuning-free approaches, requiring only 1% of training data.

Conclusion: SteerVAD establishes a powerful new direction for video anomaly detection by shifting from passive reading to active steering of MLLM representations, enabling effective anomaly detection with minimal training data.
...
TL;DR: GuardAlign is a training-free defense framework that improves safety in large vision-language models by combining optimal transport-based malicious region detection with cross-modal attention calibration to maintain safety signals during generation.


<details>
  <summary>Details</summary>
Motivation: Current input-side defenses for LVLMs using CLIP detection and safety prefixes suffer from inaccurate detection in complex scenes and unstable safety signals during decoding, creating safety vulnerabilities.
...
Result: Reduces unsafe response rates by up to 39% on SPA-VL benchmark while preserving utility, achieving improvement on VQAv2 from 78.51% to 79.21% across six representative MLLMs.

Conclusion: GuardAlign provides an effective training-free defense framework that addresses both detection accuracy and safety signal stability issues in LVLM safety, demonstrating significant safety improvements without compromising model utility.
...
<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from hallucination where generated content deviates from visual evidence. Existing solutions are either costly (require training supervision) or introduce inference latency. Current vision enhancement methods inject all visual tokens indiscriminately, causing interference from background regions and distracting from critical cues.

Method: AIR consists of two components: 1) Prototype-based token reduction - condenses large pool of visual tokens into compact subset to suppress redundancy; 2) OT-guided patch reinforcement - quantifies alignment between hidden states and patch embeddings to selectively integrate the most consistent patches into feed-forward layers.
...
Conclusion: AIR is a training-free framework that enhances model's reliance on salient visual information and effectively mitigates hallucination in multimodal large language models, providing a practical solution without additional training costs or inference latency.
...
TL;DR: A unified framework for high-fidelity 3D garment reconstruction from monocular images/videos using implicit sewing patterns and diffusion models, achieving accurate reconstruction of both tight and loose clothing with temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Reconstructing 3D clothed humans from monocular images/videos is fundamental for virtual try-on, avatar creation, and mixed reality, but accurately reconstructing garment geometry (especially loose-fitting clothing) remains challenging.

Method: Combines Implicit Sewing Patterns (ISP) with generative diffusion models to learn garment shape priors in 2D UV space. Introduces a mapping model connecting image pixels, UV pattern coordinates, and 3D geometry. For video sequences, uses spatio-temporal diffusion with test-time guidance for temporal consistency and analytic projection-based constraints for geometry preservation.

Result: Method generalizes well to real-world imagery despite being trained only on synthetic cloth data, outperforms existing approaches on both tight- and loose-fitting garments, preserves fine geometric detail, and exhibits realistic dynamic motion.

Conclusion: The framework enables high-fidelity 3D garment reconstruction from monocular inputs, supporting downstream applications like texture editing, garment retargeting, and animation while addressing the challenge of loose clothing reconstruction.
...
TL;DR: QE introduces token-aware adaptive error compensation with mixture-of-experts for VLM quantization, handling both token-independent and token-dependent important channels to improve PTQ performance.


<details>
  <summary>Details</summary>
Motivation: Existing PTQ methods for VLMs use static identification and global compensation of sensitive/outlier channels, but overlook distributional differences of these channels across inputs and modalities, leading to unsatisfactory quantization results.

Method: Quant Experts (QE) divides important channels into token-independent and token-dependent groups. For token-independent channels, uses shared expert with low-rank adapter for global error compensation. For token-dependent channels, uses routed experts with multiple low-rank adapters for local error compensation specific to tokens.

Result: QE consistently enhances task accuracy across various quantization settings and model scales (2B to 70B parameters) while maintaining performance comparable to full-precision models.

Conclusion: The proposed token-aware adaptive error compensation with mixture-of-experts effectively addresses distributional differences in important channels across modalities and tokens, significantly improving VLM quantization performance.
...
TL;DR: EvalMVX is a new real-world dataset for benchmarking multiview 3D reconstruction techniques (MVS, MVPS, MVSfP) with 25 objects, 20 views, 17 lighting conditions, and ground-truth meshes.


<details>
  <summary>Details</summary>
Motivation: Current datasets focus on RGB-based multiview stereo (MVS), but multiview photometric stereo (MVPS) and multiview shape from polarization (MVSfP) are important for high-fidelity reconstruction and sparse inputs, yet lack quantitative assessment alongside MVS.

Method: Created EvalMVX dataset with 25 objects captured using polarized camera under 20 views and 17 lighting conditions (OLAT and natural illumination), totaling 8,500 images with aligned ground-truth 3D meshes for quantitative benchmarking.

Result: Evaluated 13 recent MVX methods, identified best-performing approaches, and revealed open problems across different geometric details and reflectance types.

Conclusion: EvalMVX enables comprehensive benchmarking of multiview 3D reconstruction techniques and aims to inspire future research in this field.
...
TL;DR: FoV-Net is a rotation-invariant B-rep learning framework that uses Local Reference Frame UV-grids and Field-of-View grids to capture both local geometry and global context, achieving SOTA performance on CAD analysis tasks while being robust to arbitrary rotations.


<details>
  <summary>Details</summary>
Motivation: Current B-rep learning methods rely on absolute coordinates and normals, making them highly sensitive to rotations. Models achieving 95% accuracy on aligned benchmarks can collapse to 10% under arbitrary SO(3) rotations, highlighting the need for rotation-invariant approaches.

Method: FoV-Net represents each face with: 1) Local Reference Frame (LRF) UV-grids encoding local surface geometry, and 2) Field-of-View (FoV) grids capturing surrounding 3D context by casting rays and recording intersections with neighboring faces. Lightweight CNNs extract per-face features, which are then propagated over the B-rep graph using a graph attention network.

Result: FoV-Net achieves state-of-the-art performance on B-rep classification and segmentation benchmarks. It demonstrates robustness to arbitrary rotations while requiring less training data to achieve strong results compared to previous methods.

Conclusion: FoV-Net successfully addresses the rotation sensitivity problem in B-rep learning by introducing a rotation-invariant framework that captures both local geometry and global structural context, enabling more robust 3D CAD analysis in real-world applications.
...
TL;DR: DiffusionHarmonizer is an online generative enhancement framework that improves neural reconstruction-based simulations by transforming imperfect renderings into temporally consistent, realistic outputs for autonomous robot development.


<details>
  <summary>Details</summary>
Motivation: Neural reconstruction methods like NeRF and 3D Gaussian Splatting enable scalable simulation from real-world data but suffer from artifacts in novel views and unrealistic integration of inserted dynamic objects from different scenes.

Method: A single-step temporally-conditioned enhancer converted from a pretrained multi-step image diffusion model, trained using a custom data curation pipeline that creates synthetic-real pairs focusing on appearance harmonization, artifact correction, and lighting realism.

Result: A scalable system capable of running in online simulators on a single GPU that significantly elevates simulation fidelity for both research and production environments.

Conclusion: DiffusionHarmonizer overcomes limitations of current neural reconstruction methods by providing temporally consistent, realistic enhancement of simulation renderings, advancing autonomous robot development through improved simulation quality.
...
TL;DR: A neurosymbolic verification framework that audits VLM-generated radiology reports for logical consistency using SMT solvers and clinical knowledge bases.


<details>
  <summary>Details</summary>
Motivation: Current VLMs for radiology report generation suffer from logical inconsistencies (unsupported diagnostic impressions, missing conclusions) and standard metrics fail to capture these deductive failures in reference-free settings.

Method: Neurosymbolic verification framework that autoformalizes free-text radiographic findings into structured propositional evidence, then uses SMT solver (Z3) and clinical knowledge base to verify diagnostic claims as mathematically entailed, hallucinated, or omitted.

Result: Evaluated 7 VLMs across 5 chest X-ray benchmarks, exposing distinct reasoning failure modes (conservative observation, stochastic hallucination) invisible to traditional metrics. Solver-backed entailment eliminates unsupported hallucinations, significantly increasing diagnostic soundness and precision.

Conclusion: The neurosymbolic verification framework provides rigorous post-hoc guarantees for clinical reasoning in VLM-generated radiology reports, systematically improving diagnostic soundness and precision in generative clinical assistants.
...
<details>
  <summary>Details</summary>
Motivation: Existing two-stage motion-based 3D tracking methods suffer from error accumulation due to decoupled optimization (foreground segmentation before motion estimation) and computational bottlenecks from sequential processing.

Method: Proposes FocusTrack with two core innovations: 1) Inter-frame Motion Modeling (IMM) using temporal-difference siamese encoder to capture global motion patterns, and 2) Focus-and-Suppress Attention that enhances foreground semantics via motion-salient feature gating and suppresses background noise using temporal-aware motion context without explicit segmentation.

Result: Achieves new state-of-the-art performance on prominent 3D tracking benchmarks (KITTI, nuScenes, Waymo) while running at 105 FPS, demonstrating both accuracy and efficiency.

Conclusion: FocusTrack provides an effective one-stage solution that addresses limitations of existing two-stage approaches through unified motion-semantics co-modeling, enabling end-to-end training with compact pipeline and superior performance.
...
TL;DR: AgenticOCR is a query-driven OCR system that dynamically extracts only relevant document regions instead of processing entire pages, improving efficiency and accuracy in multimodal RAG systems for visual documents.


<details>
  <summary>Details</summary>
Motivation: Traditional page-level chunking in multimodal RAG systems introduces excessive extraneous context that overloads generators' attention mechanisms and dilutes salient evidence. Compressing information-rich pages into limited visual token budgets increases hallucination risks.

Method: AgenticOCR transforms OCR from static full-text processing into a query-driven, on-demand extraction system. It autonomously analyzes document layout "thinking with images" to identify and selectively recognize regions of interest, performing on-demand decompression of visual tokens where needed.
...
Conclusion: AgenticOCR serves as a "third building block" for visual document RAG stacks alongside standard Embedding and Reranking modules, offering a dynamic parsing paradigm that addresses bottlenecks in multimodal document processing.
...
TL;DR: A method for compressing 3D Gaussian Splatting models by 90% through reconstruction-aware pruning and new 3D Difference-of-Gaussians primitives while maintaining or improving visual quality.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting achieves real-time photorealistic rendering but requires many primitives, leading to redundancy, high resource consumption, and limited scalability for complex scenes. More efficient representations are needed for practical deployment.

Method: Two key innovations: 1) An integrated reconstruction-aware pruning strategy that adaptively determines pruning timing and refining intervals based on reconstruction quality, and 2) A 3D Difference-of-Gaussians primitive that jointly models both positive and negative densities in a single primitive to improve expressiveness under compact configurations.

Result: Achieves up to 90% reduction in Gaussian count while delivering visual quality similar to or better than state-of-the-art methods. The method significantly improves model compactness.

Conclusion: The proposed approach enables more efficient 3D scene representation by reducing redundancy while preserving visual quality, making 3D Gaussian Splatting more scalable and practical for complex scenes. Code will be publicly released.
...
TL;DR: TASOT is an unsupervised method for surgical phase/step recognition that uses multimodal optimal transport with visual and text features, eliminating need for heavy surgical pre-training while outperforming zero-shot methods.


<details>
  <summary>Details</summary>
Motivation: Current surgical phase recognition methods rely on expensive large-scale pre-training on thousands of labeled surgical videos, which incurs substantial computational and data collection costs. The authors question whether such heavy pre-training is truly necessary.

Method: TASOT extends Action Segmentation Optimal Transport (ASOT) by incorporating textual information generated from videos. It formulates temporal action segmentation as a multimodal optimal transport problem with weighted visual and text-based costs, regularized through temporally consistent unbalanced Gromov-Wasserstein formulation.
...
Conclusion: Fine-grained surgical understanding can be achieved by exploiting information already present in standard visual and textual representations without complex pre-training pipelines, demonstrating that heavy surgical-specific pre-training may not be necessary.
...
TL;DR: RETA improves decoupled dataset distillation by using dynamic retrieval of real patches and persistent topology alignment to address fit-complexity gap and pull-to-anchor effect, achieving state-of-the-art performance on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Current decoupled dataset distillation methods suffer from fit-complexity gap and pull-to-anchor effect due to reliance on static real patches, which reduces intra-class diversity and hurts generalization performance.

Method: Two main components: 1) Dynamic Retrieval Connection (DRC) selects optimal real patches from a pool by minimizing fit-complexity score in teacher feature space, 2) Persistent Topology Alignment (PTA) uses persistent homology to regularize synthesis by comparing topology of real and synthetic feature graphs via persistence images.

Result: Consistently outperforms baselines on CIFAR-100, Tiny-ImageNet, ImageNet-1K and subsets. Achieves 64.3% top-1 accuracy on ImageNet-1K with ResNet-18 at 50 images per class, +3.1% improvement over previous best.

Conclusion: RETA effectively addresses key limitations in decoupled dataset distillation through dynamic patch retrieval and topological regularization, demonstrating superior performance across diverse datasets with comparable computational resources.
...
TL;DR: HumanOrbit generates 360° orbit videos around a person from a single image using video diffusion models, producing consistent multi-view frames and reconstructing high-quality 3D textured meshes.


<details>
  <summary>Details</summary>
Motivation: Existing image-based diffusion models for multi-view synthesis produce inconsistent results across views and fail to preserve original identity. Video diffusion models show promise for photorealistic, consistent generation, inspiring a video-based approach for human orbit generation.

Method: HumanOrbit uses a video diffusion model specifically designed for multi-view human image generation. It synthesizes continuous camera rotations around subjects while maintaining geometric consistency and preserving appearance/identity. The method includes a reconstruction pipeline that converts generated multi-view frames into textured 3D meshes.

Result: The method effectively generates consistent multi-view images from single inputs. Reconstructed 3D models show superior completeness and fidelity compared to state-of-the-art baselines, demonstrating the approach's effectiveness for both image generation and 3D reconstruction.

Conclusion: HumanOrbit successfully addresses limitations of existing multi-view synthesis methods by leveraging video diffusion models, enabling high-quality 360° orbit video generation and superior 3D reconstruction from single images.
...
TL;DR: RAViT: Multi-branch vision transformer with early exit mechanism that reduces computational cost by 30% while maintaining accuracy through processing images at different resolutions.


<details>
  <summary>Details</summary>
Motivation: Vision transformers achieve excellent performance but have very high computational costs compared to alternatives like CNNs. Need to reduce computational burden while preserving accuracy.

Method: Proposes RAViT framework with multi-branch network processing same image at different resolutions. Uses early exit mechanism for adaptive trade-off between accuracy and computation. Two-branch example: resize image, predict with first transformer, reuse prediction with original image for final prediction on second transformer with less computation.

Result: Achieved equivalent accuracy to classical Vision transformer with only ~70% of FLOPs on CIFAR-10, Tiny ImageNet, and ImageNet datasets.

Conclusion: RAViT successfully reduces computational cost of vision transformers by 30% while maintaining accuracy through multi-resolution processing and early exit mechanism, making vision transformers more practical for real-world applications.
...
TL;DR: A superpixel hierarchy method for high-dimensional images that considers both attribute space and spatial layout, enabling consistent exploration in both image and attribute spaces.


<details>
  <summary>Details</summary>
Motivation: Current hierarchical embedding techniques for high-dimensional images ignore spatial layout, creating a disconnect between image regions and attribute abstractions, which hinders effective exploration of large datasets with millions of pixels.

Method: Develops a superpixel hierarchy that incorporates both high-dimensional attribute information and spatial layout during construction, enabling consistent exploration across image and attribute spaces.

Result: The method enables effective exploration of high-dimensional images by maintaining congruence between image regions and attribute abstractions, as demonstrated through comparison with classical hierarchical embedding approaches in two use cases.

Conclusion: The proposed image-guided superpixel hierarchy successfully addresses the limitation of existing methods by integrating spatial information, providing a more effective framework for exploring large high-dimensional image datasets.
...
TL;DR: A novel framework using geometry-aware diffusion to create photorealistic, animatable 4D head avatars from single portrait images with accurate 3D geometry and real-time rendering.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reconstructing 4D head avatars from single images rely on 2D priors and struggle with consistent 3D geometry, creating a need for better geometry-aware approaches.

Method: Uses geometry-aware diffusion to jointly synthesize portrait images and surface normals, with pose-free expression encoder for implicit expressions, integrated into 3D Gaussian-based avatars.
...
Conclusion: The proposed framework successfully addresses the challenge of reconstructing photorealistic, animatable 4D head avatars with accurate geometry from single portrait images.
...
TL;DR: The paper proposes Omnivorous Vision Encoder, a framework that learns modality-agnostic feature representations by aligning different modalities (RGB, depth, segmentation) of the same scene while distilling knowledge from frozen pre-trained vision encoders like DINOv2.


<details>
  <summary>Details</summary>
Motivation: Pre-trained vision encoders like DINOv2 perform well on unimodal tasks but their feature representations are poorly aligned across different modalities. For example, RGB images and their corresponding depth maps of the same scene have feature similarity nearly identical to unrelated random images, indicating poor cross-modal alignment.

Method: The framework trains an encoder with dual objectives: 1) maximizing feature alignment between different modalities of the same scene, and 2) a distillation objective that anchors learned representations to the output of a frozen teacher model (e.g., DINOv2). This creates a modality-agnostic feature space.

Result: The resulting student encoder becomes "omnivorous" by producing consistent, powerful embeddings for a given scene regardless of input modality (RGB, Depth, Segmentation, etc.), enabling robust cross-modal understanding while retaining discriminative semantics from the foundation model.

Conclusion: The proposed approach addresses the cross-modal alignment problem in vision encoders, creating a versatile encoder that maintains strong performance across different input modalities while preserving the semantic understanding capabilities of pre-trained foundation models.
...
TL;DR: First automated multimodal auditing framework for medical image classifiers that extends slice discovery methods to multimodal representations, showing improved failure discovery and explanation capabilities.


<details>
  <summary>Details</summary>
Motivation: Despite advances in ML-based medical image classifiers, safety and reliability remain major concerns. Existing auditing approaches rely on unimodal features or metadata-based subgroup analyses, which are limited in interpretability and often fail to capture hidden systematic failures.

Method: Introduces the first automated auditing framework that extends slice discovery methods to multimodal representations specifically for medical applications. Uses MIMIC-CXR-JPG dataset for comprehensive experiments under common failure scenarios.

Result: Demonstrates strong capability in both failure discovery and explanation generation. Shows that multimodal information generally allows more comprehensive and effective auditing of classifiers, while unimodal variants beyond image-only inputs exhibit strong potential in resource-constrained scenarios.

Conclusion: Multimodal auditing framework provides more comprehensive and effective auditing of medical image classifiers compared to unimodal approaches, with practical applications even in resource-constrained settings using unimodal variants.
...
TL;DR: SenCache is a sensitivity-aware caching framework that accelerates diffusion model inference by dynamically selecting caching timesteps based on model output sensitivity analysis, outperforming heuristic caching methods.


<details>
  <summary>Details</summary>
Motivation: Diffusion models have excellent video generation quality but suffer from expensive inference due to many sequential denoising steps. Existing caching methods use heuristic criteria requiring extensive tuning, lacking a principled approach.

Method: Formalizes caching error through analysis of model output sensitivity to perturbations in denoising inputs (noisy latent and timestep). Proposes SenCache - a dynamic caching policy that adaptively selects caching timesteps per-sample based on sensitivity analysis.

Result: SenCache achieves better visual quality than existing caching methods under similar computational budgets on Wan 2.1, CogVideoX, and LTX-Video benchmarks.

Conclusion: Provides theoretical basis for adaptive caching, explains effectiveness of prior heuristics, and extends them to dynamic, sample-specific approach for efficient diffusion model inference.
...
TL;DR: MuViT is a transformer architecture that fuses multi-resolution microscopy images by embedding patches in a shared world-coordinate system, enabling attention to integrate wide-field context with high-resolution detail.


<details>
  <summary>Details</summary>
Motivation: Modern microscopy produces gigapixel images with structures across multiple spatial scales, but existing vision models operate at single resolutions or derive multi-scale features from one view, limiting their ability to exploit the inherently multi-resolution nature of microscopy data.

Method: MuViT embeds all image patches into a shared world-coordinate system and extends rotary positional embeddings to these coordinates, enabling attention mechanisms to integrate wide-field context with high-resolution detail within a single encoder. The approach also includes multi-resolution MAE pretraining.

Result: Across synthetic benchmarks, kidney histopathology, and high-resolution mouse-brain microscopy, MuViT delivers consistent improvements over strong ViT and CNN baselines. Multi-resolution MAE pretraining produces scale-consistent representations that enhance downstream tasks.

Conclusion: Explicit world-coordinate modeling provides a simple yet powerful mechanism for leveraging multi-resolution information in large-scale microscopy analysis, enabling better integration of wide-field context with high-resolution detail.
...
TL;DR: A new method improves spatial understanding in text-to-image generation by creating SpatialReward-Dataset with 80k+ preference pairs and SpatialScore reward model, enabling better spatial relationship evaluation and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image generation models struggle with complex spatial relationships, requiring multiple sampling attempts to achieve satisfactory results with intricate spatial prompts.

Method: Construct SpatialReward-Dataset with over 80k preference pairs, then build SpatialScore reward model to evaluate spatial relationship accuracy. Use this reward model to enable online reinforcement learning for spatial generation.

Result: SpatialScore outperforms leading proprietary models on spatial evaluation. Extensive experiments show significant and consistent gains in spatial understanding for image generation across multiple benchmarks.

Conclusion: The specialized reward model effectively strengthens spatial understanding in text-to-image generation models, addressing the challenge of complex spatial relationship encoding.
...
TL;DR: GTASR is a lightweight consistency training method for real-world image super-resolution that addresses consistency drift and geometric decoupling issues through trajectory alignment and structural rectification.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based Real-ISR has high computational costs from iterative sampling. Existing distillation approaches are parameter-heavy and limited by teacher models, while consistency models suffer from consistency drift accumulation and geometric decoupling (pixel alignment without structural coherence).

Method: GTASR introduces two key components: 1) Trajectory Alignment (TA) strategy to rectify the tangent vector field via full-path projection, and 2) Dual-Reference Structural Rectification (DRSR) mechanism to enforce strict structural constraints.

Result: Extensive experiments show GTASR delivers superior performance over representative baselines while maintaining minimal latency, achieving efficient real-world image super-resolution.

Conclusion: GTASR provides a simple yet effective consistency training paradigm that addresses critical limitations of existing approaches, offering lightweight, high-quality real-world image super-resolution with efficient inference.
...
TL;DR: The paper shows that compositional generalization requires representations to have linear, orthogonal structure, providing theoretical grounding for the Linear Representation Hypothesis.


<details>
  <summary>Details</summary>
Motivation: Modern models trained on massive datasets still cover only a tiny fraction of combinatorial input space, raising the question of what representational structure is needed to support generalization to unseen combinations.

Method: Formalize three desiderata for compositional generalization (divisibility, transferability, stability), derive geometric constraints, and empirically evaluate predictions across vision models (CLIP, SigLIP, DINO).
...
Conclusion: The linear structure observed in neural representations is a necessary consequence of compositional generalization, predicting the representational geometry models may converge to as they scale.
...
TL;DR: HAL model uses hierarchical causal structure with varying timescales for weakly-supervised action segmentation, outperforming existing methods by better identifying high-level action variables.


<details>
  <summary>Details</summary>
Motivation: Humans perceive actions through hierarchical key transitions while machines over-segment using visual features. The observation that low-level visual features change rapidly while high-level action variables evolve slowly enables better hierarchical reasoning in video understanding.

Method: Proposes Hierarchical Action Learning (HAL) model with hierarchical causal data generation process where high-level latent actions govern low-level visual dynamics. Uses deterministic processes to align latent variables over time, hierarchical pyramid transformer to capture both visual features and latent variables, and sparse transition constraint to enforce slower high-level dynamics.

Result: Proves strict identifiability of latent action variables under mild assumptions. Experimental results on several benchmarks show HAL significantly outperforms existing methods for weakly-supervised action segmentation.

Conclusion: HAL model effectively addresses hierarchical reasoning in video understanding by modeling varying timescales between visual and action variables, demonstrating practical effectiveness in real-world applications through superior weakly-supervised action segmentation performance.
...
TL;DR: A training paradigm called "Mode Seeking meets Mean Seeking" that decouples local fidelity from long-term coherence for minute-scale video generation, using a Decoupled Diffusion Transformer with separate heads for global flow matching and local distribution matching.


<details>
  <summary>Details</summary>
Motivation: Scaling video generation from seconds to minutes faces a critical bottleneck: abundant short-video data exists with high fidelity, but coherent long-form data is scarce and limited to narrow domains.

Method: Proposes a unified representation via Decoupled Diffusion Transformer with two heads: 1) Global Flow Matching head trained via supervised learning on long videos to capture narrative structure, and 2) Local Distribution Matching head that aligns sliding windows to a frozen short-video teacher via mode-seeking reverse-KL divergence.
...
Conclusion: The method effectively improves local sharpness, motion, and long-range consistency in minute-scale video generation by decoupling local fidelity from long-term coherence through the Mode Seeking meets Mean Seeking paradigm.
...
TL;DR: UFO-4D is a unified feedforward framework that reconstructs dense 4D representations from just two unposed images by directly estimating dynamic 3D Gaussian Splats, enabling joint estimation of geometry, motion, and camera pose.


<details>
  <summary>Details</summary>
Motivation: Current methods for dense 4D reconstruction from unposed images rely on slow test-time optimization or fragmented, task-specific feedforward models, creating a need for a unified, efficient solution.

Method: Uses a unified feedforward framework with dynamic 3D Gaussian Splats representation. Differentiably renders multiple signals (appearance, depth, motion) from a single representation, enabling self-supervised image synthesis loss that tightly couples all modalities through shared geometric primitives.

Result: Outperforms prior work by up to 3 times in joint geometry, motion, and camera pose estimation. Enables high-fidelity 4D interpolation across novel views and time, overcoming data scarcity through synergistic supervision.

Conclusion: UFO-4D demonstrates that differentiably rendering multiple signals from a single Dynamic 3D Gaussian representation offers major training advantages, enabling unified, feedforward 4D reconstruction from minimal input while achieving state-of-the-art performance.
...
### [90] [Multi-Band Sensing in FR3 with Background Dense Multipath Components](https://arxiv.org/abs/2602.23539)
*Dexin Wang,Roberto Bomfin,Ahmad Bazzi,Marwa Chafii*

Main category: eess.SP

TL;DR: Novel ISAC channel analysis for multi-band sensing with DMC modeling, CRB-based trade-off analysis, and scalable multi-band estimator that outperforms single-band approaches.


<details>
  <summary>Details</summary>
Motivation: Multi-band sensing is crucial for 6G ISAC, especially with FR3's non-contiguous sub-bands. Frequency-dependent propagation effects (DMCs like clutter, diffraction, diffuse scattering) need careful consideration for accurate sensing.

Method: Proposes ISAC channel analysis using background DMC model, assesses sensing trade-offs via CRB analysis, and develops scalable multi-band estimator to resolve angular ambiguities from grating lobes.

Result: Multi-band estimator achieves 37.41% and 17.04% reduction in delay estimation RMSE compared to single-band estimators at 8.75 GHz and 21.7 GHz respectively, with substantial gains in accuracy and reduced false alarm rates.

Conclusion: Multi-band sensing with proper DMC modeling and joint estimation across sub-bands significantly outperforms single-band approaches, enabling more accurate ISAC systems for 6G networks.
...
### [91] [Massive MIMO Channel-aware Decision Fusion Aided by Reconfigurable Intelligent Surfaces](https://arxiv.org/abs/2602.23733)
*Domenico Ciuonzo,Alessio Zappone,Marco Di Renzo,Linlong Wu*

Main category: eess.SP

TL;DR: Massive MIMO + RIS integration for improved decision fusion performance, with analysis of propagation properties, design of simple fusion rules for large arrays, and RIS optimization using long-term channel statistics.


<details>
  <summary>Details</summary>
Motivation: To improve goal-oriented decision fusion performance by integrating massive MIMO systems with reconfigurable intelligent surfaces (RIS), addressing unique propagation challenges introduced by this combination.

Method: 1) Investigate traditional favorable propagation properties in RIS-aided Massive MIMO decision fusion context. 2) Design three sub-optimal simple fusion rules for large-array regime. 3) Devise RIS reflection coefficient optimization based on long-term channel statistics.

Result: Simulation results confirm the appeal of the presented design, showing improved decision fusion performance through the integrated massive MIMO-RIS approach.

Conclusion: The integration of massive MIMO with RIS enables improved channel-aware decision fusion, with practical fusion rules and RIS optimization methods that work effectively in large-array scenarios.
...
### [92] [From Continuous sEMG Signals to Discrete Muscle State Tokens: A Robust and Interpretable Representation Framework](https://arxiv.org/abs/2602.23738)
*Yuepeng Chen,Kaili Zheng,Ji Wu,Zhuangzhuang Li,Ye Ma,Dongwei Liu,Chenyi Guo,Xiangling Fu*

Main category: eess.SP

TL;DR: A physiology-informed tokenization framework converts sEMG signals into discrete muscle-state tokens, achieving high inter-subject consistency and outperforming raw signals in action recognition despite 96% dimensionality reduction.


<details>
  <summary>Details</summary>
Motivation: sEMG signals suffer from substantial inter-subject variability and noise sensitivity, making robust and interpretable decoding challenging. Current approaches lack physiological interpretability and generalizability across subjects.

Method: Proposes a discrete sEMG representation using physiology-informed tokenization: sliding windows aligned with minimal muscle contraction cycles isolate activation events, extract 10 time-frequency features (RMS, MDF, etc.), and apply K-means clustering to create muscle-state tokens. Also introduces ActionEMG-43 dataset with 43 actions and 16 muscle groups.

Result: Token representation shows high inter-subject consistency (Cohen's Kappa = 0.82±0.09). In action recognition: ViT achieves 75.5% Top-1 accuracy, SVM 67.9%, outperforming raw-signal baselines (72.8% and 64.4%) despite 96% input reduction. Tokens provide interpretable patterns for movement quality assessment.

Conclusion: Tokenized sEMG representations offer a compact, generalizable, and physiologically meaningful feature space effective for rehabilitation, human-machine interaction, and motor function analysis, addressing key limitations of traditional sEMG processing.
...
### [93] [Joint Optimization of Flexible Antenna Array Shape and Beamforming for Secure Communication](https://arxiv.org/abs/2602.23769)
*Zhen Xu,Gaojie Chen,Jing Zhu,Weiwei Zhao,Yonghui Li,Rahim Tafazolli,Wei Huang*

Main category: eess.SP

TL;DR: This paper proposes a joint optimization of flexible antenna array geometry control and transmit beamforming for physical layer security enhancement, showing significant performance gains over conventional approaches.


<details>
  <summary>Details</summary>
Motivation: Flexible antenna arrays (FAAs) can physically reshape their geometry to add spatial degrees of freedom, while transmit beamforming electronically steers radiation patterns. The paper aims to explore how integrating both FAA geometry control and beamforming can enhance physical layer security against eavesdroppers.
...
Result: Simulation results confirm that the proposed joint optimization framework significantly outperforms conventional fixed shape or beamforming only schemes, demonstrating substantial performance gains for secure wireless communications.

Conclusion: The integration of FAA geometry control with beamforming offers promising potential for enhancing physical layer security, leveraging FAA-enabled reconfigurability to significantly improve secure wireless communication performance against eavesdroppers.
...
### [94] [Explicit Formulas for the Inversion of the Convolution of Polynomials and Arbitrary Functions with Schwartz Kernels](https://arxiv.org/abs/2602.23780)
*Alfredo González-Calvin*

Main category: eess.SP

TL;DR: Convolution with even Schwartz functions acts as automorphism on polynomial spaces, with explicit inverse formulas. Deconvolution extended to broader function classes including L¹, L², Schwartz space, and tempered distributions, with new formula for inverse Weierstrass Transform.


<details>
  <summary>Details</summary>
Motivation: Convolution operations model many engineering and physical phenomena (weighted averages, blurring, integral equations). Understanding polynomial behavior under convolution is important since polynomials are inherently smooth and convolution serves as regularization. Need to investigate deconvolution/inverse operations for practical applications.

Method: Show convolution with even Schwartz functions acts as automorphism on finite-order polynomial spaces. Derive explicit equations for inverse convolution operation. Extend deconvolution to broader function classes (L¹, L², Schwartz space, tempered distributions). Establish explicit rigorous formula for deconvolution after convolution with Schwartz functions. For Weierstrass Transform, show recovery via limit of sequence of linear combinations of recursive convolutions.

Result: Proved convolution with even Schwartz functions is automorphism on polynomial spaces with explicit inverse formulas. Extended deconvolution framework to wide function classes. Developed new formula for inverse Weierstrass Transform that can be numerically implemented, allowing recovery of Schwartz functions and tempered distributions in their respective topologies.

Conclusion: Convolution operations on polynomials and broader function spaces have well-defined inverses under mild conditions. The developed deconvolution formulas, particularly for the Weierstrass Transform, provide practical numerical methods for recovering original functions from their convolutions, with applications in engineering and physics modeling.
...
### [95] [Quantized Precoding for Maximizing Sum Rate in MU-MIMO Systems with Constrained Fronthaul](https://arxiv.org/abs/2602.23856)
*Yasaman Khorsandmanesh,Alva Kosasih,Emil Björnson,Joakim Jaldén*

Main category: eess.SP

TL;DR: This paper proposes novel quantization-aware precoding methods for MU-MIMO systems with limited-capacity fronthaul, outperforming conventional infinite-resolution precoding followed by quantization.


<details>
  <summary>Details</summary>
Motivation: In downlink MU-MIMO systems, the limited bit resolution of digital fronthaul links introduces quantization effects that degrade performance when using conventional precoding approaches that ignore these constraints.

Method: Proposes a sum rate maximization framework incorporating quantizer constraints directly into precoding design. Uses WMMSE-inspired iterative algorithm, reformulates precoding as integer least-squares solved with novel sphere decoding, and introduces low-complexity expectation propagation method for MU-mMIMO systems.

Result: Numerical evaluations show proposed schemes outperform conventional infinite-resolution precoding followed by quantization. The EP-based approach offers near-optimal performance with substantial complexity reduction.

Conclusion: The proposed quantization-aware precoding methods, particularly the EP-based approach, provide superior performance with practical complexity, making them suitable for real-time MU-mMIMO applications with limited-capacity fronthaul.
...
### [96] [Secure OFDM Waveform Design for ISAC: Artificial Phase-Doppler Shifts Against Passive Sensing](https://arxiv.org/abs/2602.23861)
...
<details>
  <summary>Details</summary>
Motivation: To enhance security in integrated sensing and communication systems by designing waveforms that are difficult for eavesdroppers to intercept while maintaining full functionality for legitimate users.

Method: Introduces controlled artificial phase and Doppler shifts into OFDM waveforms that are unknown to eavesdroppers, disrupting their passive radar processing and intercept attempts. Legitimate receivers can fully compensate for these impairments.

Result: Measurement results with 1 GHz bandwidth at 27 GHz demonstrate effectiveness of different impairment introduction approaches, showing no impact on cooperative system performance while limiting eavesdropper compensation capabilities.

Conclusion: The proposed LPI waveform design successfully enhances security in OFDM-based ISAC systems by exploiting artificial impairments that disrupt eavesdroppers while being fully compensable by legitimate receivers.
...
### [97] [Optimization-Based Behavioral Modeling of Mixers for Frequency Comb OFDM Radar Processing](https://arxiv.org/abs/2602.23889)
...
TL;DR: Optimization-based behavioral model for mixers with multi-tone LO signals, validated for frequency comb OFDM radar applications.


<details>
  <summary>Details</summary>
Motivation: Traditional mixer models are inadequate for multi-tone LO excitations used in frequency comb OFDM radar systems. There's a need for accurate behavioral models that can replace computationally expensive circuit-level simulations for system-level performance estimation and waveform optimization.

Method: Uses polynomial nonlinearities for both IF and LO ports with spectrum-domain fitting that selectively emphasizes strong intermodulation products. Introduces a polynomial block to capture input power-dependent phase nonlinearity. Model is validated using circuit-level simulations and measurements.

Result: The model successfully replicates distortive effects in radar processing simulations. It enables rapid system-level performance estimations and waveform optimization, effectively replacing computationally expensive circuit-level simulations.

Conclusion: The proposed optimization-based behavioral model provides an accurate and efficient alternative to circuit-level simulations for mixers driven by multi-tone LO signals in frequency comb OFDM radar applications, facilitating system-level design and optimization.
...
### [98] [Design of a Hands-Free Short-Range Intercommunication Device Using LoRa for Secure Field Communication](https://arxiv.org/abs/2602.23924)
*Ayush Kumar Agrawal,Soumendu Das,Jayendra Kumar*

Main category: eess.SP

TL;DR: Design of a miniature, LoRa-based encrypted intercommunication device for secure field communication over 1-1.5km range under line-of-sight conditions.


<details>
  <summary>Details</summary>
Motivation: Need for short-range reliable and secure communication in tactical, military, and disaster response settings where traditional infrastructure is unavailable or vulnerable to interception. Current VHF/UHF radios are too large and power-hungry for lightweight wearable applications.

Method: System design includes voice-activated acquisition, digital audio compression, embedded microcontroller processing, AES-128 encryption, and low-power transmission via LoRa protocol using chirp spread spectrum modulation. Theoretical analysis uses link-budget calculations to justify communication range.

Result: Proposed system achieves reliable communications with low power consumption and low electromagnetic footprint over 1-1.5km range under line-of-sight conditions, demonstrating LoRa's potential beyond traditional IoT telemetry.

Conclusion: The architecture enables infrastructural agnosticism, peer-to-peer security, and wearable ergonomics, showing promise for secure tactical voice communication platforms and extending LoRa technology applications.
...
### [99] [Hypercomplex Phase Retrieval](https://arxiv.org/abs/2602.23946)
*Kumar Vijay Mishra,Henry Arguello,Brian M. Sadler*

Main category: eess.SP

TL;DR: Survey of hypercomplex phase retrieval (HPR) methods and applications, focusing on optical imaging systems that use quaternion/octonion representations to exploit inter-dimensional correlations.
...
Method: Survey methodology examining emerging HPR techniques that extend classical phase retrieval to hypercomplex domains. These formulations involve measurements obtained via complex, hypercomplex, Fourier, or other structured sensing operators, opening new avenues for HSP-based algorithms and theoretical frameworks.

Result: The chapter provides a comprehensive overview of hypercomplex phase retrieval methodologies and their applications, with particular emphasis on optical imaging systems where these techniques show significant promise.

Conclusion: Hypercomplex phase retrieval represents an important advancement in signal processing, offering enhanced capabilities for multidimensional signal analysis in optical imaging and computational sensing applications through the exploitation of inter-dimensional correlations in quaternion/octonion representations.
...
### [100] [From Signals to Causes: A Causal Signal Processing Framework for Robust and Interpretable Clinical Risk Prediction](https://arxiv.org/abs/2602.23977)
*Surajit Das,Maxine Tan*

Main category: eess.SP

TL;DR: The paper advocates for a causal signal processing approach in medical AI, treating biomedical signals as effects of latent generative mechanisms rather than isolated predictive inputs, to improve robustness and interpretability across clinical settings.


<details>
  <summary>Details</summary>
Motivation: Current learning-based medical systems rely on statistical correlations that are unstable across different acquisition settings, patient populations, and institutional practices, limiting robustness, interpretability, and clinical trust.
...
### [101] [Formation Control for CRLB-Optimal Cooperative Sensing in Low-Altitude Wireless Networks](https://arxiv.org/abs/2602.23990)
*Jun Wu,Haijia Jin,Nanchi Su,Jinna Li,Haoyuan Pan,Tse-Tin Chan*

Main category: eess.SP

TL;DR: This paper develops optimal formation design and distributed control for UAV cooperative sensing, achieving CRLB-optimal target localization through regular polygon geometries with elevation angle optimization.


<details>
  <summary>Details</summary>
Motivation: Cooperative sensing with UAVs is crucial for low-altitude wireless networks, but sensing accuracy heavily depends on UAV spatial configuration. Current approaches lack systematic methods for designing formations that optimize sensing performance metrics like Cramer-Rao lower bound.

Method: The authors first establish a sensing performance model based on range measurements and derive the Fisher information matrix for target location. Using the A-optimality criterion, they analytically characterize the formation geometry that minimizes CRLB. They then develop a distributed formation control strategy that steers UAVs from arbitrary initial deployments toward the optimal configuration while maintaining formation motion and obstacle avoidance.

Result: The optimal formation exhibits isotropic Fisher information in the horizontal plane, resulting in a regular polygon geometry with an elevation angle determined by the tradeoff between path loss and geometric diversity. Numerical results show the proposed scheme consistently outperforms benchmark formations in terms of CRLB and achieves reliable convergence under practical constraints.

Conclusion: The paper provides a systematic framework for CRLB-optimal cooperative target sensing with UAVs, combining analytical formation design with practical distributed control that enables reliable convergence to optimal sensing configurations while addressing real-world constraints like obstacle avoidance.
...
### [102] [Master-Assisted Channel Estimation for Cell-Free Massive MIMO Networks](https://arxiv.org/abs/2602.24018)
*Andreas Angelou,Marc Moonen*

Main category: eess.SP

TL;DR: MACE improves CFmMIMO channel estimation by leveraging inter-AP signal correlation through partial centralization, outperforming local estimation.


<details>
  <summary>Details</summary>
Motivation: Current CFmMIMO channel models assume no inter-AP channel correlation, missing potential benefits of centralized processing. However, careful pilot design can create inter-AP signal correlation, enabling improved channel estimation through centralization.

Method: Master-Assisted Channel Estimation (MACE): A subset of APs fuse and forward received pilot signals to a master AP, which performs channel estimation using both fused signals and its local signals, balancing local and fully centralized processing.

Result: Numerical experiments show MACE consistently outperforms local channel estimation that neglects inter-AP signal correlation, while reducing fronthaul signaling and computational complexity.

Conclusion: MACE effectively leverages inter-AP signal correlation through partial centralization to improve channel estimation performance in CFmMIMO networks, offering a practical balance between local and fully centralized approaches.
...
### [103] [Channel Estimation for Beyond Diagonal RIS Exploiting Core Tensor Sparsity](https://arxiv.org/abs/2602.24150)
...
<details>
  <summary>Details</summary>
Motivation: BD-RIS enhances wave manipulation but faces channel estimation challenges due to cascaded channels and block-Kronecker structures, requiring efficient estimation methods for practical deployment in next-generation mmWave/sub-THz networks.

Method: Proposes compressive sensing framework exploiting sparse Tucker decomposition of measurement tensor and Kronecker rank-one structure of channel components. Develops two algorithms: STORM (uses OMP for greedy support recovery) and STAR (leverages subspace-based projection for noise robustness). Both perform joint sparse support identification followed by Kronecker rank-one factorization via SVD.
...
Conclusion: The proposed compressive sensing framework with STAR algorithm provides efficient channel estimation for BD-RIS, addressing key challenges and enabling practical deployment in next-generation wireless networks.
...
### [104] [Joint Subcarrier Phase Recovery for Nonlinearity Mitigation](https://arxiv.org/abs/2602.24158)
*Marco Secondini,Stella Civelli*

Main category: eess.SP

TL;DR: Low-complexity phase recovery scheme that simultaneously mitigates laser phase noise and fiber nonlinearity across multiple subcarriers, achieving 0.9 dB gain with only 99 real multiplications per complex symbol in Raman-amplified single-span links.
...
Result: Achieves 0.9 dB gain in a long single-span link with Raman amplification while maintaining very low computational complexity (99 real multiplications per complex symbol).

Conclusion: The proposed scheme effectively addresses both phase noise and nonlinearity with minimal computational overhead, making it suitable for practical implementation in long-haul optical communication systems with Raman amplification.
...
### [105] [Cybersecurity of Teleoperated Quadruped Robots: A Systematic Survey of Vulnerabilities, Threats, and Open Defense Gaps](https://arxiv.org/abs/2602.23404)
*Mohammad Sabouri*

Main category: cs.RO

TL;DR: Survey paper analyzing cybersecurity threats and defenses for teleoperated quadruped robots, covering attack taxonomies, platform comparisons, and research gaps.


<details>
  <summary>Details</summary>
Motivation: Quadruped robots are increasingly deployed in safety-critical missions but their communication and control security remains insufficiently characterized, with unique challenges from dynamic stability constraints, gait vulnerabilities, high kinetic energy, and operator cognitive load.

Method: Synthesizes peer-reviewed literature and vulnerability disclosures (2019-2025) to create: six-layer attack taxonomy, systematic attack-to-consequence mapping, Technology Readiness Level classification, comparative security analysis of six commercial platforms, deployment guidance, and prioritized research gaps.

Result: Identifies critical maturity gaps between field-deployed communication protections (TRL 7-9) and experimental perception/operator-layer defenses (TRL 3-5), provides comparative platform security analysis, and outlines eight prioritized research gaps with implementation roadmaps.

Conclusion: Teleoperated quadruped robots face significant cybersecurity challenges requiring coordinated research and development efforts, with particular urgency in bridging the gap between mature communication protections and emerging perception/operator-layer defenses for safety-critical deployments.
...
### [106] [Demystifying Action Space Design for Robotic Manipulation Policies](https://arxiv.org/abs/2602.23408)
*Yuchun Feng,Jinliang Zheng,Zhihao Wang,Dongxiu Liu,Jianxiong Li,Jiangmiao Pang,Tai Wang,Xianyuan Zhan*

Main category: cs.RO

TL;DR: Action space design significantly impacts robotic policy learning; delta actions improve performance, while joint-space vs task-space offer complementary stability vs generalization benefits.


<details>
  <summary>Details</summary>
Motivation: Action space specification is crucial for imitation-based robotic manipulation policy learning but remains guided by ad-hoc heuristics, creating ambiguity in robotic policy design philosophies.

Method: Conducted large-scale empirical study analyzing action design space along temporal (absolute vs delta) and spatial (joint-space vs task-space) axes. Evaluated 500+ trained models over four scenarios with 13,000+ real-world rollouts on a bimanual robot.

Result: Delta action representations consistently improve performance. Joint-space parameterizations favor control stability while task-space representations favor generalization, offering complementary strengths.

Conclusion: Action space design significantly impacts robotic policy learning, with delta actions being preferable and joint-space/task-space representations providing complementary benefits for stability and generalization respectively.
...
### [107] [Printed helicoids with embedded air channels make sensorized segments for soft continuum robots](https://arxiv.org/abs/2602.23457)
*Annan Zhang,Hanna Matusik,Miguel Flores-Acton,Emily R. Sologuren,Joshua Jacob,Daniela Rus*

Main category: cs.RO

TL;DR: A fabrication method for embedding sensors into helicoid-based soft continuum robots using multi-material 3D printing, enabling distributed deformation sensing in large-scale soft robotic systems.
...
Method: Embedding air channels into helicoid-based soft continuum robots via vision-controlled jetting in a single print, interfacing with PCBs housing miniature pressure sensors and IMUs for distributed deformation sensing.

Result: Characterized mechanical properties of four helicoid designs, validated sensor response to deformation modes, and demonstrated scalability with a meter-scale, 14-DoF cable-driven soft arm capable of open-loop trajectory tracking, object grasping, and tactile-based stiffness detection.

Conclusion: Establishes a scalable fabrication strategy for sensorized architected materials in large-scale soft robotic systems, addressing the sensing and control challenges of highly deformable soft robots.
...
### [108] [Refining Almost-Safe Value Functions on the Fly](https://arxiv.org/abs/2602.23478)
*Sander Tonkens,Sosuke Kojima,Chenhao Liu,Judy Masri,Sylvia Herbert*

Main category: cs.RO

TL;DR: A framework for real-time adaptation of Control Barrier Functions using Hamilton-Jacobi reachability to refine approximate CBFs and ensure safety in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Existing methods for ensuring robotic safety have limitations: CBFs are hard to design/learn for complex systems, while Hamilton-Jacobi reachability scales poorly and is typically performed offline, making it unsuitable for dynamic environments.

Method: Two methods: refineCBF for refining approximate CBFs via warm-started HJ reachability, and HJ-Patch which accelerates this through localized updates. Both guarantee recovery of safe value functions and monotonic safety improvements during adaptation.

Result: Successful real-time adaptation to sudden environmental changes (new obstacles, unmodeled wind disturbances) in simulation and on physical hardware with ground vehicles and quadcopters.

Conclusion: The framework bridges offline synthesis and online adaptation, providing a practical path toward deploying formally guaranteed safety in real-world dynamic settings.
...
### [109] [TaCarla: A comprehensive benchmarking dataset for end-to-end autonomous driving](https://arxiv.org/abs/2602.23499)
*Tugrul Gorgulu,Atakan Dag,M. Esat Kalfaoglu,Halil Ibrahim Kuru,Baris Can Cam,Ozsel Kilinc*

Main category: cs.RO

TL;DR: A new CARLA-based autonomous driving dataset with 2.85M frames for diverse Leaderboard 2.0 scenarios, supporting multiple tasks including planning, detection, prediction, and VLMs, with rarity scoring.


<details>
  <summary>Details</summary>
Motivation: Existing autonomous driving datasets are incomplete - perception datasets lack planning data, planning datasets have limited behavioral diversity, and real datasets lack proper closed-loop evaluation. CARLA Leaderboard 2.0 addresses long-tail problems but existing datasets have limited sensor configurations.

Method: Collected over 2.85 million frames using CARLA simulation environment for diverse Leaderboard 2.0 challenge scenarios, designed to support multiple tasks including planning, object detection, lane/centerline detection, traffic light recognition, prediction, and visual language action models.

Result: Created a comprehensive dataset that addresses limitations of existing datasets, demonstrated versatility by training various models, and provided numerical rarity scores to understand state occurrence frequency.

Conclusion: The new dataset supports end-to-end autonomous driving research by providing comprehensive data for multiple tasks in both open-loop and closed-loop evaluation setups, with added value through rarity scoring for better understanding of scenario distribution.
...
### [110] [V-MORALS: Visual Morse Graph-Aided Estimation of Regions of Attraction in a Learned Latent Space](https://arxiv.org/abs/2602.23524)
*Faiz Aladin,Ashwin Balasubramanian,Lars Lindemann,Daniel Seita*

Main category: cs.RO

TL;DR: V-MORALS extends MORALS to estimate Regions of Attraction from image-based trajectories using learned latent spaces, eliminating the need for full state information.
...
Method: V-MORALS learns a latent space from image-based trajectories of a system under a given controller, then generates Morse Graphs in this learned latent space to compute Regions of Attraction without relying on state knowledge.

Result: V-MORALS provides capabilities similar to the original MORALS architecture but works with only high-level sensor data (images) instead of requiring full state information.

Conclusion: V-MORALS enables reachability analysis for systems where only sensor measurements are available, extending the applicability of topological methods for safety analysis in robotics.
...
### [111] [Tilt-X: Enabling Compliant Aerial Manipulation through a Tiltable-Extensible Continuum Manipulator](https://arxiv.org/abs/2602.23576)
...
<details>
  <summary>Details</summary>
Motivation: Existing aerial manipulators are limited to manipulation below UAVs, restricting deployability in multiple directions and through clutter, and are sensitive to propeller downwash.

Method: Developed Tilt-X with integrated tilting mechanism, telescopic stage, and cable-driven continuum section; presented design, kinematic model, and validated through flight demonstrations.

Result: Achieved volumetric workspace with up to 75 mm extension and planar orientations between 0° to 90°; experiments showed stabilization of end effector pose as manipulator extends out of propeller influence zone.

Conclusion: Tilt-X addresses limitations of existing aerial manipulators by enabling multi-directional deployment and reducing downwash interference, providing critical evidence for reliable aerial manipulator design and control.
...
### [112] [VCA: Vision-Click-Action Framework for Precise Manipulation of Segmented Objects in Target Ambiguous Environments](https://arxiv.org/abs/2602.23583)
*Donggeon Kim,Seungwon Jan,Hyeonjun Park,Daegyu Lim*

Main category: cs.RO

TL;DR: VCA replaces language commands with click-based visual interaction for robotic manipulation, reducing ambiguity and cognitive load when dealing with multiple similar objects.


<details>
  <summary>Details</summary>
Motivation: Language-based VLA models suffer from ambiguity, cognitive overhead, and difficulties in precise object identification, especially with multiple visually similar objects. Textual commands create interpretation errors and are impractical for real-world robotic manipulation.

Method: Proposes Vision-Click-Action (VCA) framework that replaces textual commands with direct click-based visual interaction using pretrained segmentation models. Operators specify target objects by clicking on them in the robot's 2D camera view.

Result: Experimental results validate that VCA achieves effective instance-level manipulation of specified target objects. The framework reduces interpretation errors and lowers cognitive load compared to language-driven interfaces.

Conclusion: VCA provides a practical and scalable alternative to language-driven interfaces for real-world robotic manipulation by enabling clear visual specification of target objects through direct interaction.
...
### [113] [KEEP: A KV-Cache-Centric Memory Management System for Efficient Embodied Planning](https://arxiv.org/abs/2602.23592)
*Zebin Yang,Tong Xie,Baotong Lu,Shaoshan Liu,Bo Yu,Meng Li*

Main category: cs.RO

TL;DR: KEEP is a KV-cache-centric memory management system for efficient embodied planning in LLMs, achieving 2.68x speedup with minimal accuracy loss by optimizing memory storage and computation.


<details>
  <summary>Details</summary>
Motivation: Existing memory-augmented LLMs for embodied planning store memory as raw text, causing long prompts and high prefill latency. While KV cache reuse is possible, frequent updates undermine efficiency benefits.

Method: KEEP introduces three innovations: (1) Static-Dynamic Memory Construction with mixed-granularity memory groups to reduce KV cache recomputation, (2) Multi-hop Memory Re-computation that dynamically identifies important cross-attention and reconstructs memory interactions iteratively, and (3) Layer-balanced Memory Loading to eliminate unbalanced KV cache loading and cross-attention computation across layers.

Result: KEEP achieves 2.68x speedup with negligible accuracy loss compared to text-based memory methods on ALFRED dataset. Compared to CacheBlend, it shows 4.13% success rate improvement and 1.90x time-to-first-token reduction.

Conclusion: KEEP provides an efficient KV-cache-centric memory management system for embodied planning that significantly improves performance while maintaining accuracy, addressing the limitations of existing text-based and KV cache approaches.
...
### [114] [MicroPush: A Simulator and Benchmark for Contact-Rich Cell Pushing and Assembly with a Magnetic Rolling Microrobot](https://arxiv.org/abs/2602.23607)
*Yanda Yang,Sambeeta Das*

Main category: cs.RO

TL;DR: MicroPush: Open-source simulator and benchmark suite for magnetic rolling microrobots in cluttered 2D environments, enabling reproducible evaluation of contact-rich micromanipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Current magnetic rolling microrobots lack autonomy for contact-rich behaviors like cell pushing and multi-target assembly, and there's difficulty in developing and evaluating such capabilities reproducibly.

Method: Combines overdamped interaction model with contact-aware stick-slip effects, near-field damping, optional Poiseuille flow, and calibrated actuation mapping. Provides modular planning-control stack with two-phase strategy for contact establishment and goal-directed pushing, plus deterministic benchmark protocol with fixed tasks and unified logging.

Result: Controller stability dominates performance under flow disturbances, while planner choice affects command smoothness over long-horizon sequences via waypoint progression. Success, time, tracking metrics, and actuation-variation measure E_Δω are reported.

Conclusion: MicroPush enables reproducible comparison and ablation of planning, control, and learning methods for microscale contact-rich micromanipulation, addressing the need for standardized evaluation in this domain.
...
### [115] [FAVLA: A Force-Adaptive Fast-Slow VLA model for Contact-Rich Robotic Manipulation](https://arxiv.org/abs/2602.23648)
...
Method: FAVLA uses a slow VLM at fixed low frequency to encode modalities, produce latent representations, and predict near-future force variation. A fast action expert executes at variable high frequency, conditioning on latest force sequence data. It includes a force adapter that injects high-frequency force features into multiple AE layers, and adaptively schedules AE execution frequency based on VLM's predicted force variation.
...
Conclusion: Decoupling slow perception planning from fast contact-aware control with adaptive frequency scheduling enables better reactivity to contact events, addressing limitations of unified-frequency fusion in VLA models for contact-rich manipulation.
...
### [116] [SpikingTac: A Miniaturized Neuromorphic Visuotactile Sensor for High-Precision Dynamic Tactile Imprint Tracking](https://arxiv.org/abs/2602.23654)
*Tianyu Jiang,Chaofan Zhang,Shaolin Zhang,Shaowei Cui,Shuo Wang*

Main category: cs.RO

TL;DR: SpikingTac is a low-cost, miniaturized neuromorphic tactile sensor with custom event camera that achieves high-speed tracking (1000Hz perception, 350Hz tracking) and exceptional precision in dynamic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing event-driven tactile sensors are limited by bulky standard event cameras, hindering integration into robotic systems for human-like dynamic manipulation.

Method: Developed SpikingTac with custom standalone event camera module (<$150 cost), global dynamic state map with unsupervised denoising network, and hysteresis-aware incremental update law with spatial gain damping to address silicone elastomer viscoelastic hysteresis.

Result: Achieved 100% return-to-origin success rate with 0.8039 pixel mean bias, 6.2mm obstacle-avoidance overshoot (5x better than frame-based sensors), and sub-millimeter accuracy (0.0952mm localization RMSE, 0.0452mm radius RMSE).

Conclusion: SpikingTac demonstrates that miniaturized, low-cost neuromorphic tactile sensors can achieve high-speed, precise tracking for dynamic manipulation, overcoming limitations of conventional frame-based sensors.
...
### [117] [Physics-Embedded Neural ODEs for Learning Antagonistic Pneumatic Artificial Muscle Dynamics](https://arxiv.org/abs/2602.23670)
*Xinyao Wang,Jonathan Realmuto*

Main category: cs.RO

TL;DR: Hybrid Neural ODE framework embeds physical structure into learned model of antagonistic pneumatic artificial muscle dynamics for improved stiffness control.


<details>
  <summary>Details</summary>
Motivation: Antagonistic pneumatic artificial muscles (PAMs) provide variable impedance through co-contraction but exhibit complex coupled, nonlinear, and hysteretic dynamics that are challenging to model and control effectively.

Method: Hybrid neural ordinary differential equation (Neural ODE) framework combining parametric joint mechanics and pneumatic state dynamics with a neural network force component to capture antagonistic coupling and rate-dependent hysteresis.

Result: Forward model predicts joint motion and chamber pressures with mean R² of 0.88 across 225 co-contraction conditions. Inverse formulation computes pressure commands for desired motion/stiffness profiles, achieving reliable stiffness control across 126-176 N/mm and consistent impedance across velocities.

Conclusion: The hybrid Neural ODE approach successfully models antagonistic PAM dynamics and enables effective stiffness control, outperforming static models which degrade at higher velocities.
...
### [118] [Interpretable Multimodal Gesture Recognition for Drone and Mobile Robot Teleoperation via Log-Likelihood Ratio Fusion](https://arxiv.org/abs/2602.23694)
*Seungyeol Baek,Jaspreet Singh,Lala Shakti Swarup Ray,Hymalai Bello,Paul Lukowicz,Sungho Suh*

Main category: cs.RO

TL;DR: Multimodal gesture recognition framework using Apple Watch IMU data and capacitive gloves for hands-free robot teleoperation, achieving comparable performance to vision-based methods with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Current vision-based gesture recognition for hands-free robot teleoperation suffers from performance degradation under occlusions, lighting variations, and cluttered backgrounds, limiting real-world applicability in hazardous environments.

Method: Proposes multimodal gesture recognition integrating inertial data (accelerometer, gyroscope, orientation) from Apple Watches on both wrists with capacitive sensing from custom gloves, using late fusion based on log-likelihood ratio (LLR) for interpretable modality combination.

Result: Framework achieves performance comparable to state-of-the-art vision-based baseline while significantly reducing computational cost, model size, and training time. Introduces new dataset of 20 aircraft marshalling-inspired gestures with synchronized RGB video, IMU, and capacitive sensor data.

Conclusion: Sensor-based multimodal fusion provides robust, interpretable solution for gesture-driven mobile robot and drone teleoperation, suitable for real-time control in hazardous environments.
...
### [119] [A Reliable Indoor Navigation System for Humans Using AR-based Technique](https://arxiv.org/abs/2602.23706)
*Vijay U. Rathod,Manav S. Sharma,Shambhavi Verma,Aadi Joshi,Sachin Aage,Sujal Shahane*

Main category: cs.RO

TL;DR: AR-based indoor navigation system using Vuforia Area Target for environment modeling and A* algorithm for pathfinding, showing 2-3x faster performance than Dijkstra's algorithm with improved accuracy and user experience.


<details>
  <summary>Details</summary>
Motivation: Indoor navigation systems are unreliable, forcing users to rely on confusing static signage or floor maps. There's a need for intuitive, real-time navigation solutions for campus and small indoor areas.

Method: Uses Vuforia Area Target for environment modeling, AI navigation's NavMesh component for navigation, and A* algorithm for shortest path calculation within the NavMesh component.

Result: A* algorithm reaches solutions 2-3 times faster than Dijkstra's algorithm for smaller search spaces. Experimental results show significantly improved navigation accuracy, better user experience, and greater efficiency compared to traditional methods.

Conclusion: AR technology integrated with existing pathfinding algorithms is feasible and scalable for indoor navigation, though further NavMesh optimization is needed for large or highly dynamic environments.
...
### [120] [SAGE-LLM: Towards Safe and Generalizable LLM Controller with Fuzzy-CBF Verification and Graph-Structured Knowledge Retrieval for UAV Decision](https://arxiv.org/abs/2602.23719)
*Wenzhe Zhao,Yang Zhao,Ganchao Liu,Zhiyu Jiang,Dandan Ma,Zihao Li,Xuelong Li*

Main category: cs.RO

TL;DR: A train-free two-layer UAV decision architecture using LLMs with safety verification and scene adaptation, enhancing safety and generalization without online training.


<details>
  <summary>Details</summary>
Motivation: LLMs offer semantic understanding for UAV decision-making but lack domain-specific control knowledge and formal safety assurances, limiting their direct application in dynamic, hazardous environments.

Method: Two-layer architecture: high-level safety planning with fuzzy Control Barrier Function verification for LLM outputs, plus low-level precise control. Uses star-hierarchical graph-based retrieval-augmented generation for efficient scene adaptation.

Result: Validated in pursuit-evasion scenarios with unknown obstacles and emergent threats. SAGE-LLM maintains performance while significantly enhancing safety and generalization without online training.

Conclusion: The framework demonstrates strong extensibility and potential for generalization to broader embodied intelligence systems and safety-critical control domains.
...
### [121] [StemVLA:An Open-Source Vision-Language-Action Model with Future 3D Spatial Geometry Knowledge and 4D Historical Representation](https://arxiv.org/abs/2602.23721)
*Jiasong Xiao,Yutao She,Kai Li,Yuyang Sha,Ziang Cheng,Ziang Tong*

Main category: cs.RO

TL;DR: StemVLA is a vision-language-action model that incorporates 3D future spatial forecasting and 4D historical spatiotemporal representations to improve robot manipulation in dynamic environments.
...
Method: StemVLA incorporates: 1) Forecasting structured 3D future spatial-geometric world knowledge to anticipate scene geometry, 2) Using a pretrained video-geometry transformer backbone to extract implicit 3D world representations from historical frames, and 3) Aggregating these across time with a temporal attention module (VideoFormer) to form unified 4D historical spatiotemporal representations.

Result: Extensive simulation experiments show StemVLA significantly improves long-horizon task success and achieves state-of-the-art performance on the CALVIN ABC-D benchmark with an average sequence length of XXX.

Conclusion: By jointly modeling 2D observations, predicted 3D future structure, and aggregated 4D temporal dynamics, StemVLA enables more comprehensive world understanding for robot manipulation compared to existing approaches.
...
### [122] [Acceleration-Based Control of Fixed-Wing UAVs for Guidance Applications](https://arxiv.org/abs/2602.23821)
*Jixiang Wang,Siyuan Yang,Ziyi Wu,Siqi Wei,Ashay Wakode,Agata Barcis,Hung Nguyen,Shaoming He*

Main category: cs.RO

TL;DR: This paper presents an acceleration-level outer-loop control framework that converts commanded tangential and normal accelerations into executable body-rate and normalized thrust commands for fixed-wing UAVs, enabling practical implementation of acceleration-commanded guidance laws like proportional navigation.


<details>
  <summary>Details</summary>
Motivation: Acceleration-commanded guidance laws (e.g., proportional navigation) are attractive for high-level decision making but challenging to deploy directly on fixed-wing UAVs because accelerations are not directly actuated and must be realized through attitude and thrust under flight-envelope constraints.

Method: The paper presents an acceleration-level outer-loop control framework that converts commanded accelerations into body-rate and normalized thrust commands. For normal channel: engineering mapping from desired normal acceleration to roll- and pitch-rate commands that regulate lift vector direction/magnitude under small-angle assumptions. For tangential channel: energy-based formulation inspired by total energy control with empirical thrust-energy acceleration relationship from flight data, avoiding explicit propulsion modeling or thrust bench calibration. Also discusses priority handling between normal/tangential accelerations under saturation and non-level maneuvers.
...
Conclusion: The proposed framework successfully bridges the gap between acceleration-commanded guidance laws and practical fixed-wing UAV control, providing a practical solution for implementing proportional navigation and similar guidance strategies on platforms with standard autopilot interfaces.
...
### [123] [OmniTrack: General Motion Tracking via Physics-Consistent Reference](https://arxiv.org/abs/2602.23832)
*Yuhan Li,Peiyuan Zhi,Yunshen Wang,Tengyu Liu,Sixu Yan,Wenyu Liu,Xinggang Wang,Baoxiong Jia,Siyuan Huang*

Main category: cs.RO

TL;DR: OmniTrack is a two-stage framework for humanoid robot motion tracking that decouples physical feasibility from general motion tracking to handle discrepancies between human motion data and robot morphology/dynamics.


<details>
  <summary>Details</summary>
Motivation: Human motion data contains physically infeasible artifacts (floating, penetration) due to morphology/dynamics differences between humans and robots, plus data noise. These artifacts create conflicts between following inaccurate references and maintaining robot stability, hindering generalizable motion tracking policies.

Method: Two-stage approach: 1) Privileged generalist policy generates physically plausible motions via trajectory rollout in simulation, strictly adhering to robot dynamics. 2) General control policy is trained to track these physically feasible motions for stable transfer to real robot.

Result: Improved tracking accuracy and strong generalization to unseen motions. Real-world tests achieved hour-long consistent stable tracking including complex acrobatics (flips, cartwheels). Supports human-style stable dynamic online teleoperation with robustness to varying user inputs.

Conclusion: OmniTrack successfully addresses physical feasibility challenges in humanoid motion tracking by decoupling feasibility from tracking, enabling stable real-world performance and generalization to diverse motions including complex acrobatics.
...
### [124] [OmniXtreme: Breaking the Generality Barrier in High-Dynamic Humanoid Control](https://arxiv.org/abs/2602.23843)
*Yunshen Wang,Shaohang Zhu,Peiyuan Zhi,Yuhan Li,Jiaxin Li,Yong-Lu Li,Yuchen Xiao,Xingxing Wang,Baoxiong Jia,Siyuan Huang*

Main category: cs.RO

TL;DR: OmniXtreme breaks the fidelity-scalability trade-off in humanoid motion tracking by decoupling general motor skill learning from sim-to-real physical refinement.


<details>
  <summary>Details</summary>
Motivation: Current motion tracking policies face a "generality barrier" where tracking fidelity collapses as motion libraries scale in diversity, especially for high-dynamic motions in real-world deployment. This is caused by learning bottlenecks in multi-motion optimization and physical executability constraints.

Method: OmniXtreme uses a two-phase approach: 1) A flow-matching policy with high-capacity architectures to scale representation capacity without interference-intensive multi-motion RL optimization, and 2) An actuation-aware refinement phase that ensures robust performance on physical hardware.

Result: The framework maintains high-fidelity tracking across diverse, high-difficulty datasets and successfully executes multiple extreme motions on real robots, breaking the long-standing fidelity-scalability trade-off in high-dynamic humanoid control.

Conclusion: OmniXtreme effectively overcomes the generality barrier in motion tracking by separating general motor skill learning from physical skill refinement, enabling scalable high-fidelity motion execution on real hardware.
...
### [125] [Hybrid Offline-Online Reinforcement Learning for Sensorless, High-Precision Force Regulation in Surgical Robotic Grasping](https://arxiv.org/abs/2602.23870)
*Edoardo Fazzari,Omar Mohamed,Khalfan Hableel,Hamdan Alhadhrami,Cesare Stefanini*

Main category: cs.RO

TL;DR: Sensorless control framework using physics modeling and hybrid RL achieves precise distal force regulation in surgical instruments without distal sensing.


<details>
  <summary>Details</summary>
Motivation: Precise grasp force regulation in tendon-driven surgical instruments is limited by nonlinear coupling between motor dynamics, transmission compliance, friction, and distal mechanics. Existing solutions require distal force sensing or analytical compensation, increasing hardware complexity or degrading performance under dynamic motion.

Method: Combines physics-consistent modeling and hybrid reinforcement learning. Develops first-principles digital twin of da Vinci Xi grasping mechanism capturing coupled electrical, transmission, and jaw dynamics. Uses three-stage pipeline: (1) receding-horizon CMA-ES oracle for expert trajectories, (2) fully offline policy learning via Implicit Q-Learning for stable initialization, (3) online refinement using TD3 for adaptation to on-policy dynamics.

Result: In simulation, controller maintains grasp force within 1% of desired reference during multi-harmonic jaw motion. Hardware experiments show average force errors below 4% across diverse trajectories. Learned policy has ~71k parameters and executes at kHz rates for real-time deployment.

Conclusion: High-fidelity modeling combined with structured offline-online RL can recover precise distal force behavior without additional sensing, offering scalable and mechanically compatible solution for surgical robotic manipulation.
...
### [126] [TSC: Topology-Conditioned Stackelberg Coordination for Multi-Agent Reinforcement Learning in Interactive Driving](https://arxiv.org/abs/2602.23896)
*Xiaotong Zhang,Gang Xiong,Yuanjing Wang,Siyu Teng,Alois Knoll,Long Chen*

Main category: cs.RO

TL;DR: TSC is a decentralized multi-agent reinforcement learning framework for autonomous driving that uses topology-based Stackelberg coordination to handle dense traffic interactions without communication.


<details>
  <summary>Details</summary>
Motivation: Existing MARL approaches for autonomous driving either use synchronous decision-making (which worsens non-stationarity) or centralized sequencing (which doesn't scale well with traffic density), leading to unstable behaviors like oscillatory yielding or unsafe commitments in dense traffic scenarios.

Method: TSC extracts a time-varying directed priority graph from braid-inspired weaving relations between trajectories, defining local leader-follower dependencies without global ordering. It factorizes interactions into graph-local Stackelberg subgames, uses CTDE to learn sequential coordination policies where leaders are anticipated via action prediction and followers are trained through action-conditioned value learning to approximate local best responses.
...
Conclusion: TSC provides a scalable, communication-free approach to decentralized interactive driving that improves training stability and safety in dense traffic through topology-conditioned Stackelberg coordination.
...
### [127] [ABPolicy: Asynchronous B-Spline Flow Policy for Real-Time and Smooth Robotic Manipulation](https://arxiv.org/abs/2602.23901)
*Fan Yang,Peiguang Jing,Kaihua Qu,Ningyuan Zhao,Yuting Su*

Main category: cs.RO

TL;DR: ABPolicy is an asynchronous flow-matching policy that uses B-spline control-point action space to achieve smooth robotic manipulation by eliminating jitter, discontinuities, and stop-and-go execution.
...
Method: ABPolicy operates in B-spline control-point action space for intra-chunk smoothness, uses bidirectional action prediction with refitting optimization for inter-chunk continuity, and leverages asynchronous inference for real-time continuous updates.

Result: Evaluation across seven tasks shows ABPolicy reduces trajectory jerk, leading to smoother motion and improved performance in both static and dynamic settings with moving objects.

Conclusion: ABPolicy effectively addresses smoothness and responsiveness challenges in robotic manipulation through B-spline representation, bidirectional prediction, and asynchronous inference.
...
### [128] [Teleoperated Omni-directional Dual Arm Mobile Manipulation Robotic System with Shared Control for Retail Store](https://arxiv.org/abs/2602.23923)
*Rolif Lima,Somdeb Saha,Nijil George,Vismay Vakharia,Shubham Parab,Sahil Gaonkar,Vighnesh Vatsal,Kaushik Das*

Main category: cs.RO

TL;DR: Researchers developed a dual-arm mobile robot with VR tele-operation for retail automation, enabling shared control between human operators and robots to handle diverse retail items.


<details>
  <summary>Details</summary>
Motivation: Retail sector is adopting AI-powered robots but faces challenges with dynamic product environments and autonomous operation in novel situations, requiring more adaptable robotic systems.

Method: Developed an omni-directional dual-arm mobile robot with heterogeneous grippers for different item types, plus a VR motion capture tele-operation system for shared human-robot control in retail settings.

Result: System successfully manipulated various retail items using both single and dual-arm coordinated techniques in a mockup retail environment, demonstrating practical viability.

Conclusion: The proposed tele-operated dual-arm robot system effectively addresses retail automation challenges by combining robot autonomy with human oversight for handling diverse products.
...
### [129] [Learning to Build: Autonomous Robotic Assembly of Stable Structures Without Predefined Plans](https://arxiv.org/abs/2602.23934)
*Jingwen Wang,Johannes Kirschner,Paul Rolland,Luis Salamanca,Stefana Parascho*

Main category: cs.RO

TL;DR: Autonomous robotic assembly framework using RL for blueprint-free construction, tested on 2D block tasks with real-world validation.


<details>
  <summary>Details</summary>
Motivation: To create more adaptable robotic construction systems that don't rely on predefined blueprints, enabling flexible adaptation to environmental uncertainty and variations during building processes.

Method: Reinforcement learning policy trained with deep Q-learning and successor features for decision-making; construction tasks defined through targets and obstacles rather than fixed plans; evaluated on 15 2D robotic assembly tasks with discrete block construction.

Result: Real-world closed-loop robotic experiments demonstrate feasibility and ability to handle construction noise; framework shows promise for adaptable and robust robotic construction in real environments.

Conclusion: The blueprint-free robotic assembly framework using RL offers a promising direction for more adaptable and robust construction systems capable of handling real-world environmental uncertainty and variations.
...
### [130] [Enhancing Vision-Language Navigation with Multimodal Event Knowledge from Real-World Indoor Tour Videos](https://arxiv.org/abs/2602.23937)
*Haoxuan Xu,Tianfu Li,Wenbo Chen,Yi Liu,Xingxing Zuo,Yaoxian Song,Haoang Li*

Main category: cs.RO

TL;DR: Proposes YE-KG, a large-scale multimodal spatiotemporal knowledge graph from indoor videos, and STE-VLN framework that integrates event knowledge via hierarchical retrieval to improve VLN performance on coarse-grained instructions and long-horizon reasoning.


<details>
  <summary>Details</summary>
Motivation: VLN agents struggle with long-horizon reasoning in unseen environments, especially with ambiguous, coarse-grained instructions. Current knowledge graph approaches don't fully leverage multimodal event knowledge inspired by human episodic memory.

Method: 1) Construct YE-KG with 86k nodes/83k edges from real-world indoor videos using multimodal LLMs (LLaVa, GPT4) to extract structured semantic-action-effect events. 2) Develop STE-VLN framework with Coarse-to-Fine Hierarchical Retrieval mechanism to integrate event knowledge and fuse with visual observations.

Result: Outperforms state-of-the-art approaches on REVERIE, R2R, and R2R-CE benchmarks across diverse action spaces, demonstrating efficiency of event-centric strategy.

Conclusion: Event-centric knowledge enhancement through multimodal spatiotemporal knowledge graphs effectively addresses coarse-grained instruction and long-horizon reasoning challenges in VLN, with publicly available data and code.
...
### [131] [Learning Robust Control Policies for Inverted Pose on Miniature Blimp Robots](https://arxiv.org/abs/2602.23972)
*Yuanlin Yang,Lin Hong,Fumin Zhang*

Main category: cs.RO

TL;DR: A framework for learning robust control policies to achieve inverted poses on miniature blimp robots using simulation-to-real transfer with domain randomization and modified TD3 algorithm.


<details>
  <summary>Details</summary>
Motivation: Inverted poses are essential for maximizing miniature blimp robot agility, but control development is challenging due to complex, underactuated dynamics. There's a need for reliable control methods for these aerial systems.

Method: Three-stage framework: 1) Build high-fidelity 3D simulation calibrated with real-world motion data, 2) Train robust policy using domain randomization and modified TD3 algorithm, 3) Design mapping layer to bridge sim-to-real gap for deployment.

Result: Simulation evaluation shows learned policy achieves higher success rate than energy-shaping controller. Real-world experiments confirm the policy with mapping layer enables MBR to achieve and maintain fully upside-down pose.

Conclusion: The proposed framework successfully enables robust inverted pose control for miniature blimp robots through simulation-based learning with effective sim-to-real transfer, demonstrating practical real-world application.
...
### [132] [Autonomous Inspection of Power Line Insulators with UAV on an Unmapped Transmission Tower](https://arxiv.org/abs/2602.24011)
*Václav Riss,Vít Krátký,Robert Pěnička,Martin Saska*

Main category: cs.RO

TL;DR: Online UAV inspection algorithm for transmission towers using camera-LiDAR fusion to detect and localize insulators without prior maps, achieving significant time savings and high localization accuracy.


<details>
  <summary>Details</summary>
Motivation: Autonomous inspection of transmission towers typically requires prior maps or multiple flights. The paper aims to enable single-flight inspection without prior knowledge of the tower structure, improving efficiency and reducing inspection time.
...
Result: Simulation shows 24% time savings compared to two-flight strategies. Real-world experiments achieve mean horizontal/vertical localization errors of 0.16m with low variance, outperforming existing methods by an order of magnitude in horizontal error variance.

Conclusion: The proposed online inspection algorithm enables efficient single-flight transmission tower inspection without prior maps, demonstrating practical viability through simulation and real-world validation with improved accuracy and reduced variance.
...
### [133] [Curriculum Reinforcement Learning for Quadrotor Racing with Random Obstacles](https://arxiv.org/abs/2602.24030)
*Fangyu Sun,Fanxing Li,Yu Hu,Linzuo Zhang,Yueqian Liu,Wenxian Yu,Danping Zou*

Main category: cs.RO

TL;DR: Vision-based curriculum reinforcement learning framework for drone racing with obstacles, achieving faster lap times and higher success rates than existing approaches.
...
Method: Combines multi-stage curriculum learning, domain randomization, and multi-scene updating strategy to address conflicting challenges of obstacle avoidance and gate traversal. Uses end-to-end control policy implemented as a single network.
...
Conclusion: The proposed framework effectively advances drone racing in obstacle-rich environments by addressing the previously underexplored challenges of obstacle perception and dynamic navigation.
...
### [134] [Geometry-based pneumatic actuators for soft robotics](https://arxiv.org/abs/2602.24104)
*Rui Chen,Daniele Leonardis,Domenico Chiaradia,Antonio Frisoli*

Main category: cs.RO

TL;DR: Soft pneumatic actuators with geometry-based design using CNC heat-sealed chambers achieve complex actuation patterns, near-zero bending radii, and multi-state capabilities for wearable robotics and haptic systems.


<details>
  <summary>Details</summary>
Motivation: Traditional soft pneumatic actuators have design limitations including minimum bending radii, limited multi-state capabilities, and structural stability issues, preventing complex actuation patterns needed for advanced applications.

Method: Geometry-based pneumatic actuators (GPAs) using constraint layers with configurable CNC heat-sealed chambers. Mathematical modeling predicts linear angle transformations and nonlinear torque-angle relationships.

Result: GPAs achieve predictable deformation, near-zero bending radii, multi-state actuation, and customizable complex geometries. Demonstrated with: 49g wrist exoskeleton reducing muscle activity by 51%, 30.8g haptic interface delivering 8N force with fast response, and 208g bipedal robot achieving multi-gait locomotion.

Conclusion: GPAs establish a configurable platform for next-generation wearable robotics, haptic systems, and soft locomotion devices, overcoming traditional soft pneumatic actuator limitations.
...
### [135] [Planning from Observation and Interaction](https://arxiv.org/abs/2602.24121)
*Tyler Han,Siyang Shen,Rohan Baijal,Harine Ravichandiran,Bat Nemekhbold,Kevin Huang,Sanghun Jung,Byron Boots*

Main category: cs.RO

TL;DR: Proposes a planning-based Inverse Reinforcement Learning algorithm for robot learning from observation alone, enabling real-world manipulation tasks to be learned from scratch in under an hour without hand-designed rewards or demonstrator actions.


<details>
  <summary>Details</summary>
Motivation: Observational learning in robotics typically requires hand-designed rewards or demonstrator actions, which are often unavailable or impractical. The paper aims to enable robots to learn manipulation tasks solely from observations without these assumptions.

Method: Develops a planning-based Inverse Reinforcement Learning algorithm that learns world models from observation and interaction alone. The approach combines world modeling with planning to infer reward functions and policies without access to demonstrator actions or pre-designed rewards.

Result: The method successfully learns image-based manipulation tasks from scratch in under an hour in real-world experiments. It demonstrates superior sample efficiency and success rates compared to existing IRL, RL, and Behavior Cloning approaches, and shows capability for online transfer learning.

Conclusion: The proposed approach provides a practical path for online world modeling and planning from observation and interaction, overcoming limitations of existing methods that require more restrictive assumptions about rewards and demonstrator actions.
...
### [136] [Robust Skills, Brittle Grounding: Diagnosing Restricted Generalization in Vision-Language Action Policies via Multi-Object Picking](https://arxiv.org/abs/2602.24143)
*David Emukpere,Romain Deffayet,Jean-Michel Renders*

Main category: cs.RO

TL;DR: VLA policies show strong benchmark performance but may rely on object-location correlations rather than true language grounding. A controlled study reveals manipulation skill acquisition is decoupled from instruction following.
...
Method: Conducted controlled multi-object picking study with progressive workspace randomization, evaluating held-out object-location pairings to break familiar associations without increasing spatial difficulty. Tested representative VLA policies including SmolVLA and π₀.₅ across stress tests and data scaling.
...
### [137] [Humanoid Robots as First Assistants in Endoscopic Surgery](https://arxiv.org/abs/2602.24156)
...
TL;DR: First demonstration of a humanoid robot (Unitree G1) assisting in actual surgical procedure - providing endoscopic visualization during a cadaveric sphenoidectomy via teleoperation.


<details>
  <summary>Details</summary>
Motivation: Despite mainstream claims about humanoid robots' surgical capabilities, there's no empirical evidence of humanoids assisting in actual procedures. This work aims to provide the first proof-of-concept demonstration.

Method: Used a teleoperated Unitree G1 humanoid robot to provide endoscopic visualization while an attending otolaryngologist performed a cadaveric sphenoidectomy. Teleoperation allowed assessment of physical demands while cognitive demands were handled by the operator.

Result: Procedure completed successfully with stable visualization maintained throughout. Demonstrated form-factor feasibility for surgical assistance and identified engineering targets for clinical translation and near-term opportunities like autonomous diagnostic scoping.

Conclusion: Establishes feasibility of humanoid form factor for surgical assistance while identifying challenges for continued development, marking the first empirical demonstration of humanoid robot assistance in an actual surgical procedure.
...
### [138] [How IMU Drift Influences Multi-Radar Inertial Odometry for Ground Robots in Subterranean Terrains](https://arxiv.org/abs/2602.24192)
*Moumita Mukherjee,Magnus Norén,Anton Koval,Avijit Banerjee,George Nikolakopoulos*

Main category: cs.RO

TL;DR: MRIO is a two-stage radar-inertial odometry framework that addresses IMU bias drift in GPS-denied subterranean environments, combining IMU bias estimation with radar ego-velocity for robust localization and mapping in smoke/dust conditions.


<details>
  <summary>Details</summary>
Motivation: Cost-effective IMUs like Pixhawk suffer from bias drift in extreme subterranean conditions, while radar returns are sparse and noisy. LiDAR fails under smoke/dust, but FMCW radars remain robust, compact, and cost-effective in these environments.

Method: Two-stage MRIO framework: 1) Radar-based ego-velocity estimation via least-squares approach incorporated into EKF for online IMU bias correction, 2) Fusing corrected IMU accelerations with heterogeneous measurements from multiple radars and IMU to refine odometry. Supports radar-only mapping using estimated displacements.

Result: In subterranean field trials, MRIO outperforms EKF-RIO, maintaining accuracy across cost-efficient FMCW radar setups and different IMUs (Pixhawk and higher-grade VectorNav), showing robust localization and mapping in GPS-denied environments affected by smoke.

Conclusion: MRIO provides a resilient solution for subterranean navigation where LiDAR fails, offering robust localization and mapping through effective IMU bias correction and radar-inertial fusion. The framework will be open-sourced to benefit the robotics community.
...
### [139] [Evaluating Accuracy of Vine Robot Shape Sensing with Distributed Inertial Measurement Units](https://arxiv.org/abs/2602.24202)
*Alexis E. Laudenslager,Antonio Alvarez Valdivia,Nathaniel Hanson,Margaret McGuinness*

Main category: cs.RO

TL;DR: Experimental evaluation of distributed IMU-based shape sensing for vine robots, quantifying accuracy under passive/active steering, varying lengths, and sensor spacing.


<details>
  <summary>Details</summary>
Motivation: Vine robots need full-body shape sensing for sensor localization and environmental mapping, but distributed IMU-based methods lack systematic accuracy quantification under realistic conditions like active steering and varying lengths.

Method: Experimental evaluation using distributed IMUs along vine robot body, measuring orientation drift, and testing accuracy under passive/active steering, varying robot lengths (30-175 cm), and different sensor spacings.

Result: Average orientation drift: 1.33°/min; tip position error: 11% of length (passive), 16% (active); 8% error across lengths with positive trend; intermediate sensor spacings minimize error for single-curvature shapes.

Conclusion: Distributed IMU-based shape sensing is feasible for vine robots but has limitations; improved modeling and algorithmic integration needed for field deployment.
...
### [140] [SafeGen-LLM: Enhancing Safety Generalization in Task Planning for Robotic Systems](https://arxiv.org/abs/2602.24235)
*Jialiang Fan,Weizhe Xu,Mengyu Liu,Oleg Sokolsky,Insup Lee,Fangxin Kong*

Main category: cs.RO

TL;DR: SafeGen-LLM is a safety-generalizable LLM for robotic task planning that combines SFT and GRPO training to ensure safety compliance and generalization across domains.


<details>
  <summary>Details</summary>
Motivation: Classical planners have scalability issues, RL methods generalize poorly, and base LLMs cannot guarantee safety in safety-critical robotic task planning.

Method: Two-stage post-training: 1) SFT on constraint-compliant planning dataset to learn planning syntax/semantics, 2) GRPO guided by fine-grained reward machines from formal verification for safety alignment, plus curriculum learning for complex tasks.

Result: SafeGen-LLM achieves strong safety generalization and outperforms frontier proprietary baselines across multi-domain planning tasks with multiple input formats (PDDLs and natural language).

Conclusion: SafeGen-LLM effectively addresses safety-critical planning challenges by combining LLM capabilities with formal verification methods for safety-guaranteed generalization.
...
### [141] [Detoxifying LLMs via Representation Erasure-Based Preference Optimization](https://arxiv.org/abs/2602.23391)
*Nazanin Mohammadi Sepahvand,Eleni Triantafillou,Hugo Larochelle,Doina Precup,Daniel M. Roy,Gintare Karolina Dziugaite*

Main category: cs.LG

TL;DR: REPO is a new method for detoxifying LLMs that erases toxic representations at the token level, achieving robust protection against adversarial attacks where previous methods fail.


<details>
  <summary>Details</summary>
Motivation: Existing LLM detoxification methods (DPO, NPO, etc.) are vulnerable to adversarial prompting and fine-tuning attacks because they only make superficial changes - toxic "directions" remain in the model's representations.

Method: REPO (Representation Erasure-based Preference Optimization) reformulates detoxification as token-level preference optimization. It uses a novel objective with preference data to force toxic continuation representations to converge toward benign counterparts, making deep, localized edits to toxicity-encoding neurons.

Result: REPO achieves state-of-the-art robustness, stopping sophisticated threats including relearning attacks and enhanced GCG jailbreaks where existing representation- and output-based methods fail. Mechanistic analysis shows it induces deep, localized edits while preserving general model utility.

Conclusion: REPO provides a fundamentally more robust approach to LLM detoxification by addressing the problem at the representation level rather than just output behavior, making it resistant to adversarial attacks that bypass previous methods.
...
### [142] [U-CAN: Utility-Aware Contrastive Attenuation for Efficient Unlearning in Generative Recommendation](https://arxiv.org/abs/2602.23400)
*Zezheng Wu,Rui Wang,Xinghe Cheng,Yang Shao,Qing Yang,Jiapu Wang,Jingwei Zhang*

Main category: cs.LG

TL;DR: U-CAN is a precision unlearning framework for generative recommendation LLMs that selectively attenuates sensitive parameters in LoRA adapters while preserving utility, addressing the polysemy dilemma where neurons encode both sensitive and general reasoning patterns.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning LLMs for generative recommendation encodes sensitive user attributes into model parameters, creating privacy risks. Existing machine unlearning methods cause catastrophic utility loss due to the polysemy dilemma - where neurons superimpose sensitive data with general reasoning patterns.

Method: Utility-aware Contrastive Attenuation (U-CAN) operates on low-rank adapters (LoRA). It quantifies risk by contrasting activations between forgetting and retention sets, identifies neurons with asymmetric responses, and uses utility-aware calibration combining weight magnitudes with retention-set activation norms. Instead of binary pruning, it applies adaptive soft attenuation with a differentiable decay function to selectively down-scale high-risk parameters while preserving network connectivity.
...
Conclusion: U-CAN provides an effective solution for privacy-preserving generative recommendation by enabling precise unlearning of sensitive information while maintaining model utility through soft attenuation on LoRA adapters, overcoming the limitations of traditional gradient or pruning methods.
...
### [143] [Long Range Frequency Tuning for QML](https://arxiv.org/abs/2602.23409)
*Michael Poppel,Jonas Stein,Sebastian Wölckert,Markus Baumann,Claudia Linnhoff-Popien*

Main category: cs.LG

TL;DR: Trainable-frequency quantum models struggle with limited frequency reachability during optimization, but ternary grid initialization overcomes this by ensuring target frequencies are within reachable range.


<details>
  <summary>Details</summary>
Motivation: While trainable-frequency quantum models theoretically offer efficient function approximation by matching target spectrum size, their practical effectiveness depends on gradient optimization driving prefactors to arbitrary target values, which may not be achievable in practice.

Method: Proposed grid-based initialization using ternary encodings that generate dense integer frequency spectra, requiring O(log_3(omega_max)) encoding gates. This ensures target frequencies lie within the locally reachable range during optimization.

Result: On synthetic targets with three shifted high frequencies, ternary grid initialization achieved median R² of 0.9969 vs 0.1841 for trainable-frequency baseline. For Flight Passengers dataset, ternary grid achieved median R² of 0.9671 (22.8% improvement over trainable-frequency's 0.7876).

Conclusion: Ternary grid initialization effectively overcomes the frequency reachability limitation in trainable-frequency quantum models, providing practical improvements over theoretical trainable-frequency approaches while maintaining exponential efficiency gains over fixed-frequency methods.
...
### [144] [Brain-OF: An Omnifunctional Foundation Model for fMRI, EEG and MEG](https://arxiv.org/abs/2602.23410)
*Hanning Guo,Farah Abdellatif,Hanwen Bi,Andrei Galbenus,Jon. N. Shah,Abigail Morrison,Jürgen Dammers*

Main category: cs.LG

TL;DR: Brain-OF is the first omnifunctional brain foundation model jointly pretrained on fMRI, EEG, and MEG data, using novel techniques to handle multimodal integration and dual-domain pretraining for superior performance across neuroscience tasks.


<details>
  <summary>Details</summary>
Motivation: Most existing brain foundation models are limited to single functional modalities, missing opportunities to exploit complementary spatiotemporal dynamics and collective data scale across different brain imaging techniques (fMRI, EEG, MEG).

Method: 1) Any-Resolution Neural Signal Sampler projects diverse brain signals into shared semantic space; 2) DINT attention with Sparse Mixture of Experts backbone manages semantic shifts; 3) Masked Temporal-Frequency Modeling dual-domain pretraining objective reconstructs signals in both time and frequency domains.

Result: Brain-OF demonstrates superior performance across diverse downstream tasks after pretraining on a large-scale corpus of around 40 datasets, highlighting benefits of joint multimodal integration and dual-domain pretraining.

Conclusion: Brain-OF successfully addresses limitations of single-modality brain foundation models by enabling unified multimodal processing through innovative architectural components and pretraining strategies, advancing the field of brain foundation models.
...
### [145] [EvoX: Meta-Evolution for Automated Discovery](https://arxiv.org/abs/2602.23413)
...
TL;DR: EvoX is an adaptive evolution method that jointly evolves candidate solutions and search strategies, outperforming existing AI-driven evolutionary methods across 200+ real-world optimization tasks.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-driven evolutionary methods use fixed search strategies with static parameters that don't adapt across tasks or as search spaces change over time, limiting their effectiveness.

Method: EvoX jointly evolves both candidate solutions and the search strategies used to generate them, enabling continuous adaptation of how prior solutions are selected and varied based on progress.
...
Conclusion: EvoX demonstrates that adaptive evolution of search strategies alongside solutions enables more effective optimization across diverse tasks by dynamically shifting strategies during the optimization process.
...
### [146] [Human Supervision as an Information Bottleneck: A Unified Theory of Error Floors in Human-Guided Learning](https://arxiv.org/abs/2602.23446)
*Alejandro Rodriguez Dominguez*

Main category: cs.LG

TL;DR: Human supervision creates fundamental limits on AI performance due to information loss in human feedback channels, which scaling alone cannot overcome.


<details>
  <summary>Details</summary>
Motivation: Large language models trained on human data exhibit persistent errors despite scaling, suggesting fundamental limitations in human supervision channels rather than model capacity.

Method: Developed unified theory across six frameworks (operator theory, PAC-Bayes, information theory, causal inference, category theory, game theory) showing human supervision acts as information-reducing channel, creating positive excess-risk floor.

Result: Theory explains why scaling can't eliminate human-aligned errors; experiments confirm human-only supervision has persistent error floor while auxiliary non-human signals reduce/eliminate excess error.

Conclusion: Human supervision has inherent information limits; overcoming persistent errors requires auxiliary non-human signals to restore information about latent evaluation targets.
...
### [147] [Global Interpretability via Automated Preprocessing: A Framework Inspired by Psychiatric Questionnaires](https://arxiv.org/abs/2602.23459)
*Eric V. Strobl*

Main category: cs.LG

TL;DR: REFINE is a two-stage method that uses nonlinear preprocessing to stabilize questionnaire data followed by linear prediction, achieving better accuracy than interpretable approaches while maintaining global interpretability through coefficient matrices.


<details>
  <summary>Details</summary>
Motivation: Psychiatric questionnaires are context-sensitive and weakly predictive of future symptoms. While nonlinear models can improve accuracy, their lack of interpretability undermines clinical trust. The paper aims to develop a method that combines predictive power with interpretability.

Method: REFINE decouples preprocessing from prediction: 1) A nonlinear preprocessing module estimates stable item values from questionnaire data, 2) A linear model maps these stabilized items to future symptom severity. This concentrates nonlinearity in preprocessing while keeping the prognostic relationship transparently linear.

Result: REFINE outperforms other interpretable approaches while preserving clear global attribution of prognostic factors across psychiatric and non-psychiatric longitudinal prediction tasks. It provides interpretability through a coefficient matrix rather than post hoc local attributions.

Conclusion: The REFINE framework successfully addresses the trade-off between predictive accuracy and interpretability in psychiatric questionnaire analysis by separating nonlinear preprocessing from linear prediction, enabling both improved performance and clinical trust through transparent coefficient-based interpretation.
...
### [148] [Uncertainty-aware Language Guidance for Concept Bottleneck Models](https://arxiv.org/abs/2602.23495)
*Yangyi Li,Mengdi Huai*

Main category: cs.LG

TL;DR: The paper proposes an uncertainty-aware Concept Bottleneck Model (CBM) that quantifies uncertainty in LLM-annotated concepts with distribution-free guarantees and incorporates this uncertainty into CBM training to handle varying reliability of LLM annotations.


<details>
  <summary>Details</summary>
Motivation: Traditional CBMs require expensive expert annotations for concepts, limiting their adoption. While some works use LLMs to generate concepts, they overlook the uncertainty in LLM annotations and lack mechanisms to quantify or incorporate this uncertainty, increasing error risks from LLM hallucinations.

Method: Proposes a novel uncertainty-aware CBM method that: 1) rigorously quantifies uncertainty of LLM-annotated concept labels with valid distribution-free guarantees, and 2) incorporates quantified concept uncertainty into CBM training to account for varying reliability across LLM-annotated concepts. The method includes theoretical analysis.

Result: Extensive experiments on real-world datasets validate the desired properties of the proposed methods, showing improved handling of uncertain LLM annotations.

Conclusion: The proposed uncertainty-aware CBM addresses limitations of existing LLM-based concept annotation methods by providing rigorous uncertainty quantification and incorporating this uncertainty into model training, making CBMs more practical by reducing reliance on expensive expert annotations while managing LLM uncertainty.
...
### [149] [FedDAG: Clustered Federated Learning via Global Data and Gradient Integration for Heterogeneous Environments](https://arxiv.org/abs/2602.23504)
...
Conclusion: FedDAG addresses key limitations in clustered FL by providing a more comprehensive similarity metric and enabling beneficial cross-cluster knowledge transfer while maintaining cluster specialization, leading to improved performance in heterogeneous federated learning settings.
...
### [150] [Sample Size Calculations for Developing Clinical Prediction Models: Overview and pmsims R package](https://arxiv.org/abs/2602.23507)
*Diana Shamsutdinova,Felix Zimmer,Oyebayo Ridwan Olaniran,Sarah Markham,Daniel Stahl,Gordon Forbes,Ewan Carr*

Main category: cs.LG

TL;DR: The paper proposes a novel simulation-based approach using learning curves and Gaussian Process optimization to determine minimum sample sizes for clinical prediction models, implemented in an R package called pmsims.


<details>
  <summary>Details</summary>
Motivation: Current methods for determining sample size in clinical prediction modeling are inadequate, leading to overfitting, poor generalizability, and biased predictions. Existing approaches vary in flexibility and accuracy, especially for complex data structures and machine learning models.

Method: The authors propose a simulation-based approach that integrates learning curves, Gaussian Process optimization, and assurance principles to identify sample sizes that achieve target performance with high probability. This is implemented in pmsims, an open-source, model-agnostic R package.

Result: Case studies show that sample size estimates vary substantially across methods, performance metrics, and modeling strategies. pmsims provides flexible, efficient, and interpretable solutions that accommodate diverse models and user-defined metrics while accounting for variability in model performance.

Conclusion: The framework and software advance sample size methodology for clinical prediction modeling by combining flexibility with computational efficiency. Future work should extend these methods to hierarchical and multimodal data, incorporate fairness and stability metrics, and address challenges like missing data and complex dependencies.
...
TL;DR: Neural operators can learn to cluster infinite-dimensional functional data, even for non-convex/disconnected classes, with theoretical guarantees and practical applications to ODE trajectory analysis.


<details>
  <summary>Details</summary>
Motivation: While neural operators are well-understood for regression tasks, there's limited understanding of their capabilities for classification and clustering problems, especially for functional data in scientific computing applications.

Method: Prove universal clustering theorem showing neural operators can approximate any finite collection of classes in RKHS. Develop practical pipeline: discretized ODE trajectories → pre-trained encoder → lightweight trainable head → soft assignments.

Result: Theoretical: Neural operators can learn any K closed classes in infinite-dimensional RKHS under mild kernel sampling assumptions. Practical: SNO (practical implementation) successfully recovers latent dynamical structure in ODE benchmarks where classical methods fail.

Conclusion: Neural operators provide a powerful framework for functional data clustering with theoretical guarantees, enabling discovery of latent structure in scientific data like ODE trajectories where traditional methods are inadequate.
...
### [152] [Active Value Querying to Minimize Additive Error in Subadditive Set Function Learning](https://arxiv.org/abs/2602.23529)
*Martin Černý,David Sychrovský,Filip Úradník,Jakub Černý*

Main category: cs.LG

TL;DR: This paper studies how to approximate unknown subadditive set functions with additive error when some values are missing, focusing on minimizing the gap between minimal and maximal completions through offline and online value disclosure methods.


<details>
  <summary>Details</summary>
Motivation: Subadditive set functions are crucial in economics, optimization, and AI, but specifying them requires exponentially many values. Missing values create ambiguity that becomes problematic when optimizing incomplete functions, especially since deterministic multiplicative approximation is known to be hard.

Method: The paper explores minimal and maximal completions of set functions with missing values, analyzes their distance, and develops methods to minimize this distance by disclosing additional subset values. Approaches include both offline and online strategies for value disclosure.

Result: The research provides theoretical analysis of completion distances for different function classes and develops algorithms to efficiently reduce this distance. Empirical demonstrations show the algorithms perform well in practical scenarios.

Conclusion: The work offers a practical approach to handling incomplete subadditive set functions by focusing on additive error approximation and developing efficient methods to close the gap between minimal and maximal completions through strategic value disclosure.
...
### [153] [Rudder: Steering Prefetching in Distributed GNN Training using LLM Agents](https://arxiv.org/abs/2602.23556)
*Aishwarya Sarkar,Sayan Ghosh,Nathan Tallent,Aman Chadha,Tanya Roosta,Ali Jannesari*

Main category: cs.LG

TL;DR: Rudder is an adaptive prefetching module for distributed GNN training that uses LLM-based reasoning to dynamically prefetch remote nodes, achieving up to 91% performance improvement over baselines.


<details>
  <summary>Details</summary>
Motivation: Distributed GNN training suffers from frequent irregular communication stalls due to neighbor sampling. Static prefetching methods cannot adapt to dynamic conditions like varying graph structures, distributions, sampling parameters, and caching policies.

Method: Rudder integrates into AWS DistDGL framework and uses LLM-based generative AI with In-Context Learning capabilities for zero-shot adaptive control. It autonomously prefetches remote nodes by leveraging LLMs' emergent reasoning properties for dynamic decision-making.

Result: Evaluations on NERSC Perlmutter supercomputer show up to 91% improvement in end-to-end training performance over baseline DistDGL (no prefetching), 82% improvement over static prefetching, and over 50% reduction in communication.

Conclusion: LLM-based adaptive prefetching (Rudder) effectively addresses communication bottlenecks in distributed GNN training by dynamically adapting to changing conditions, significantly outperforming both static prefetching and no-prefetching approaches.
...
TL;DR: Learners in multi-platform settings can get stuck in "overspecialization traps" where they only serve their existing user base, leading to poor global performance. The paper proposes using knowledge distillation to probe peer models and escape these traps.


<details>
  <summary>Details</summary>
Motivation: In multi-platform ML deployments, each platform learns from its own user base, but existing algorithms can converge to models with arbitrarily poor global performance despite good models existing. This happens through "overspecialization traps" where platforms only optimize for their existing users.

Method: Proposes an algorithm using knowledge distillation to allow learners to "probe" predictions of peer models, enabling them to learn about users who don't select them. This helps escape overspecialization traps by accessing information about the broader user population.

Result: The algorithm converges almost surely to a stationary point with bounded full-population risk when probing sources are sufficiently informative (e.g., market leader or majority of peers with good global performance). Verified with semi-synthetic experiments on MovieLens, Census, and Amazon Sentiment datasets.

Conclusion: Knowledge distillation-based probing can help ML systems in multi-platform settings escape overspecialization traps and achieve better global performance by learning from peer models about users outside their immediate user base.
...
TL;DR: ULW-SleepNet: An ultra-lightweight multimodal sleep stage scoring framework that achieves competitive accuracy with only 13.3K parameters and 7.89M FLOPs, enabling real-time sleep monitoring on wearable/IoT devices.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models for sleep stage scoring are computationally demanding and designed for single-channel EEG, limiting their practicality for multimodal polysomnography (PSG) data in real-world applications.

Method: Proposes ULW-SleepNet with novel Dual-Stream Separable Convolution (DSSC) Block, depthwise separable convolutions, channel-wise parameter sharing, and global average pooling to efficiently integrate multiple physiological signals while reducing computational overhead.

Result: Achieves 86.9% accuracy on Sleep-EDF-20 and 81.4% on Sleep-EDF-78 datasets with only 13.3K parameters and 7.89M FLOPs, reducing parameters by up to 98.6% compared to state-of-the-art methods with marginal performance loss.

Conclusion: ULW-SleepNet demonstrates strong potential for real-time sleep monitoring on wearable and IoT devices due to its ultra-lightweight design and competitive performance, with source code publicly available.
...
### [156] [Flowette: Flow Matching with Graphette Priors for Graph Generation](https://arxiv.org/abs/2602.23566)
*Asiri Wijesinghe,Sevvandi Kandanaarachchi,Daniel M. Steinberg,Cheng Soon Ong*

Main category: cs.LG

TL;DR: Flowette: A continuous flow matching framework for graph generation that incorporates structural priors (graphettes) to model recurring subgraph motifs, combining flow-based training with domain-driven structural knowledge.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of generative modeling of graphs with recurring subgraph motifs, which are common in real-world graph structures like molecular graphs with specific functional groups or social networks with characteristic connection patterns.

Method: Flowette uses continuous flow matching with a graph neural network transformer to learn velocity fields over graph representations. It preserves topology through optimal transport coupling and long-range dependencies through regularization. Introduces "graphettes" - a probabilistic family of graph structure models that generalize graphons via controlled structural edits for motifs like rings, stars, and trees.

Result: Theoretical analysis of coupling, invariance, and structural properties. Empirical evaluation on synthetic and small-molecule graph generation tasks shows consistent improvements over baseline methods.

Conclusion: Combining structural priors (graphettes) with flow-based training is effective for modeling complex graph distributions, demonstrating the importance of incorporating domain knowledge about recurring motifs in graph generation tasks.
...
### [157] [InfoNCE Induces Gaussian Distribution](https://arxiv.org/abs/2602.24012)
*Roy Betser,Eyal Gofer,Meir Yossef Levi,Guy Gilboa*

Main category: cs.LG

TL;DR: InfoNCE contrastive learning induces Gaussian structure in learned representations, proven under alignment/concentration assumptions and with vanishing regularization.


<details>
  <summary>Details</summary>
Motivation: Contrastive learning is fundamental for modern representation learning, but the statistical properties of representations learned via InfoNCE loss are not well understood. The paper aims to explain the commonly observed Gaussianity in contrastive representations.

Method: Two complementary theoretical approaches: 1) Under alignment and concentration assumptions, show high-dimensional representations asymptotically approach multivariate Gaussian distributions. 2) Under less strict assumptions, add vanishing regularization promoting low feature norm and high entropy to achieve similar Gaussian results. Experimental validation on synthetic and CIFAR-10 datasets across multiple encoder architectures.

Result: Theoretical analysis proves InfoNCE induces Gaussian structure in representations. Experiments confirm consistent Gaussian behavior across different architectures and datasets. The Gaussian model enables principled analytical treatment of learned representations.

Conclusion: InfoNCE contrastive learning naturally produces Gaussian representations, providing a principled explanation for observed empirical patterns. The resulting Gaussian model supports analytical treatment and enables various applications in contrastive learning.
...
### [158] [Hybrid Quantum Temporal Convolutional Networks](https://arxiv.org/abs/2602.23578)
*Junghoon Justin Park,Maria Pak,Sebin Lee,Samuel Yen-Chi Chen,Shinjae Yoo,Huan-Hsin Tseng,Jiook Cha*

Main category: cs.LG

TL;DR: HQTCN combines classical temporal windowing with quantum CNN for multivariate time-series, achieving competitive performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Quantum ML models for sequential data face scalability challenges with complex multivariate signals, needing parameter-efficient approaches.

Method: Hybrid Quantum Temporal Convolutional Network (HQTCN) combines classical temporal windowing with quantum convolutional neural network core, applying shared quantum circuit across temporal windows.

Result: HQTCN performs competitively with classical baselines on univariate data and outperforms all baselines on multivariate tasks, especially strong under data-limited conditions with fewer parameters.

Conclusion: HQTCN establishes as a parameter-efficient approach for multivariate time-series analysis, capturing long-range dependencies while achieving significant parameter reduction.
...
### [159] [SDMixer: Sparse Dual-Mixer for Time Series Forecasting](https://arxiv.org/abs/2602.23581)
*Xiang Ao*

Main category: cs.LG

TL;DR: Proposes a dual-stream sparse Mixer framework for multivariate time series forecasting that extracts global trends and local dynamics in frequency/time domains using sparsity to filter noise and improve cross-variable dependency modeling.


<details>
  <summary>Details</summary>
Motivation: Multivariate time series forecasting faces challenges with multi-scale characteristics, weak correlations, and noise interference that limit existing models' predictive performance.

Method: Dual-stream sparse Mixer framework that extracts global trends and local dynamic features from sequences in both frequency and time domains, employing a sparsity mechanism to filter out invalid information.
...
### [160] [Normalisation and Initialisation Strategies for Graph Neural Networks in Blockchain Anomaly Detection](https://arxiv.org/abs/2602.23599)
*Dang Sy Duy,Nguyen Duy Chien,Kapil Dev,Jeff Nijsse*

Main category: cs.LG

TL;DR: Systematic study shows GNN performance on financial fraud detection depends on architecture-specific initialization and normalization strategies, with GraphSAGE working best with Xavier initialization, GAT benefiting from GraphNorm + Xavier, and GCN being less sensitive.


<details>
  <summary>Details</summary>
Motivation: While GNNs offer principled approaches to financial fraud detection, their effectiveness on real-world AML benchmarks depends critically on underexplored training practices like weight initialization and normalization strategies.

Method: Systematic ablation study of initialization and normalization strategies across three GNN architectures (GCN, GAT, GraphSAGE) on the Elliptic Bitcoin dataset, using temporal data splits, seeded runs, and reproducible experimental framework.

Result: Initialization and normalization effects are architecture-dependent: GraphSAGE achieves strongest performance with Xavier initialization alone, GAT benefits most from combining GraphNorm with Xavier initialization, while GCN shows limited sensitivity to these modifications.

Conclusion: The findings provide practical, architecture-specific guidance for deploying GNNs in AML pipelines, especially for datasets with severe class imbalance, and the authors release a reproducible experimental framework for further research.
...
### [161] [When Does Multimodal Learning Help in Healthcare? A Benchmark on EHR and Chest X-Ray Fusion](https://arxiv.org/abs/2602.23614)
*Kejing Yin,Haizhou Xu,Wenfang Yao,Chen Liu,Zijie Chen,Yui Haang Cheung,William K. Cheung,Jing Qin*

Main category: cs.LG

TL;DR: Systematic benchmark of multimodal fusion (EHR + chest X-rays) reveals when multimodal learning helps clinical prediction, how fusion strategies compare, robustness to missing modalities, and fairness implications.


<details>
  <summary>Details</summary>
Motivation: To understand when multimodal learning truly helps in clinical practice, especially under modality missingness and fairness constraints, by systematically benchmarking EHR and chest X-ray fusion.
...
Result: Multimodal fusion improves performance with complete modalities, especially for diseases needing complementary EHR/CXR info. Benefits degrade with missing modalities unless explicitly handled. Cross-modal learning captures meaningful dependencies but EHR temporal structure creates modality imbalance. Multimodal fusion doesn't inherently improve fairness, with disparities from unequal sensitivity across demographics.

Conclusion: Provides actionable guidance on when multimodal learning helps/fails, lays foundation for clinically deployable multimodal systems, and releases open-source benchmarking toolkit (CareBench) for reproducible evaluation.
...
### [162] [BTTackler: A Diagnosis-based Framework for Efficient Deep Learning Hyperparameter Optimization](https://arxiv.org/abs/2602.23630)
...
<details>
  <summary>Details</summary>
Motivation: Current automated HPO methods waste significant computation resources on bad trials that encounter training problems like vanishing gradients or insufficient convergence, which aren't reflected in accuracy metrics during early training stages, leading to inefficient optimization.

Method: BTTackler introduces training diagnosis using carefully designed quantified indicators to detect training problems automatically, triggering early termination of bad trials to free up computational resources for better hyperparameter configurations.

Result: BTTackler reduces time consumption by 40.33% to achieve comparable accuracy to baseline methods, and conducts 44.5% more top-10 trials within given time budgets across evaluations with three DNNs and four HPO methods.

Conclusion: BTTackler effectively tackles the problem of wasted computation on bad trials in HPO through automated training diagnosis, significantly improving optimization efficiency while maintaining comparable accuracy, with an open-source library available for easy integration.
...
### [163] [On the Convergence of Single-Loop Stochastic Bilevel Optimization with Approximate Implicit Differentiation](https://arxiv.org/abs/2602.23633)
*Yubo Zhou,Luo Luo,Guang Dai,Haishan Ye*

Main category: cs.LG

TL;DR: SSAID algorithm achieves optimal O(ε⁻²) convergence rate for stochastic bilevel optimization with explicit κ⁷ dependence, matching multi-loop methods while maintaining single-loop efficiency.


<details>
  <summary>Details</summary>
Motivation: Single-loop algorithms for stochastic bilevel optimization are widely used in practice (meta-learning, hyperparameter optimization) but lack rigorous theoretical understanding compared to multi-loop methods. Existing analyses have suboptimal rates and obscure dependence on lower-level condition number κ.

Method: Single-loop Stochastic Approximate Implicit Differentiation (SSAID) algorithm that concurrently updates lower and upper variables in a single loop, providing refined convergence analysis with explicit characterization of κ dependence.

Result: SSAID achieves ε-stationary point with oracle complexity O(κ⁷ ε⁻²), matching optimal O(ε⁻²) rate of state-of-the-art multi-loop methods while maintaining single-loop computational efficiency.

Conclusion: SSAID is not just a heuristic but has rigorous theoretical foundation with convergence guarantees competitive with multi-loop frameworks, providing first explicit fine-grained characterization of κ-dependence for stochastic AID-based single-loop methods.
...
### [164] [FlexGuard: Continuous Risk Scoring for Strictness-Adaptive LLM Content Moderation](https://arxiv.org/abs/2602.23636)
*Zhihao Ding,Jinming Li,Ze Lu,Jieming Shi*

Main category: cs.LG

TL;DR: FlexGuard: A flexible LLM moderation system that outputs calibrated risk scores instead of binary classifications, enabling adaptation to varying enforcement strictness across platforms and over time.


<details>
  <summary>Details</summary>
Motivation: Existing LLM moderation models use fixed binary classification, assuming a fixed definition of harmfulness. In practice, enforcement strictness varies across platforms and evolves over time, making binary moderators brittle under shifting requirements.

Method: Proposes FlexGuard, an LLM-based moderator that outputs calibrated continuous risk scores reflecting risk severity. Uses risk-alignment optimization to improve score-severity consistency and provides threshold selection strategies to adapt to target strictness at deployment.

Result: Experiments on FlexBench (a new strictness-adaptive benchmark) and public benchmarks show FlexGuard achieves higher moderation accuracy and substantially improved robustness under varying strictness compared to existing moderators.

Conclusion: FlexGuard addresses the brittleness of binary moderators by providing flexible, calibrated risk scoring that can adapt to different enforcement strictness requirements, making LLM moderation more practical for real-world deployment.
...
### [165] [FedRot-LoRA: Mitigating Rotational Misalignment in Federated LoRA](https://arxiv.org/abs/2602.23638)
*Haoran Zhang,Dongjun Kim,Seohyeon Cha,Haris Vikalo*

Main category: cs.LG

TL;DR: FedRot-LoRA addresses rotational misalignment in federated LoRA by aligning client updates via orthogonal transformations before aggregation, improving stability and performance without extra communication cost.


<details>
  <summary>Details</summary>
Motivation: Standard federated LoRA suffers from aggregation errors due to rotational misalignment - semantically equivalent updates can be represented in different latent subspaces across clients, causing destructive interference when factors are averaged directly.

Method: FedRot-LoRA aligns client updates via orthogonal transformations prior to aggregation, preserving semantic updates while reducing cross-client subspace mismatch without increasing communication cost or restricting model expressivity.
...
### [166] [Selective Denoising Diffusion Model for Time Series Anomaly Detection](https://arxiv.org/abs/2602.23662)
*Kohei Obata,Zheng Chen,Yasuko Matsubara,Lingwei Zhu,Yasushi Sakurai*

Main category: cs.LG

TL;DR: AnomalyFilter is a novel diffusion-based method for time series anomaly detection that selectively denoises only anomaly parts while preserving normal parts, achieving better performance than existing conditional diffusion approaches.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based TSAD methods use conditional strategies that reconstruct entire instances from noise, which leads to inaccurate reconstruction of normal parts and suboptimal detection performance. The authors aim to create a more effective approach by focusing only on anomaly parts.

Method: Proposes AnomalyFilter, a diffusion-based method that acts as a selective filter. During training, they mask Gaussian noise and conduct denoising without adding noise to instances. The approach only denoises anomaly parts while retaining normal parts intact.

Result: Extensive experiments on five datasets show that AnomalyFilter achieves notably low reconstruction error on normal parts, demonstrating its effectiveness in anomaly detection. The synergy of simple components greatly enhances performance over naive diffusion models.

Conclusion: AnomalyFilter represents a pioneering approach that focuses on noise design in diffusion models specifically tailored for TSAD, offering a more effective solution than existing conditional diffusion methods by selectively filtering only anomaly parts.
...
### [167] [Disentangled Mode-Specific Representations for Tensor Time Series via Contrastive Learning](https://arxiv.org/abs/2602.23663)
*Kohei Obata,Taichi Murayama,Zheng Chen,Yasuko Matsubara,Yasushi Sakurai*

Main category: cs.LG

TL;DR: MoST is a novel representation learning method for multi-mode tensor time series that uses tensor slicing to reduce complexity and learns disentangled representations capturing both mode-specific and mode-invariant features through contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Multi-mode tensor time series (TTS) are common in domains like search engines and environmental monitoring, but learning effective representations is challenging due to the complexities inherent in tensor structures.

Method: MoST uses tensor slicing to reduce TTS complexity and learns disentangled representations for individual non-temporal modes. It employs contrastive learning with a two-part loss function: one for mode-specific features (relationships within same mode) and another for mode-invariant features (common across different modes), using disentangled representations as augmentations.
...
Conclusion: MoST effectively addresses the challenges of TTS representation learning by disentangling mode-specific and mode-invariant features through tensor slicing and contrastive learning, achieving superior performance in downstream tasks.
...
### [168] [Optimizer-Induced Low-Dimensional Drift and Transverse Dynamics in Transformer Training](https://arxiv.org/abs/2602.23696)
*Yongzhong Xu*

Main category: cs.LG

TL;DR: Training trajectories in small transformers show dominant drift direction with residual oscillations; optimizer choice (AdamW vs SGD) shapes trajectory geometry beyond loss values.


<details>
  <summary>Details</summary>
Motivation: To understand how different optimizers shape the geometry of training trajectories in transformer models beyond just loss values, examining the structure of parameter updates and their relationship to learning dynamics.

Method: Used uncentered, row-normalized trajectory PCA to analyze training trajectories in small transformer models. Compared AdamW with SGD variants at matched loss levels, and performed reheating experiments to perturb transverse components.

Result: A single dominant drift direction captures most parameter movement early in training, with residual components encoding oscillatory probe performance. AdamW develops multi-dimensional drift structure while SGD produces nearly colinear evolution. Reheating selectively affects transverse components without disrupting the dominant drift.

Conclusion: Optimizer choice fundamentally shapes the effective dimensionality and structure of learning trajectories, with AdamW producing richer multi-dimensional dynamics compared to SGD's simpler colinear evolution, revealing that trajectory geometry contains information beyond what loss values alone can capture.
...
### [169] [Bridging Dynamics Gaps via Diffusion Schrödinger Bridge for Cross-Domain Reinforcement Learning](https://arxiv.org/abs/2602.23737)
*Hanping Zhang,Yuhong Guo*

Main category: cs.LG

TL;DR: BDGxRL is a cross-domain RL framework that uses Diffusion Schrödinger Bridge to align source transitions with target dynamics from offline demonstrations, enabling target-oriented policy learning without target environment interaction or reward supervision.


<details>
  <summary>Details</summary>
Motivation: Cross-domain RL faces challenges due to dynamics shifts between source and target domains, lack of target-domain environment interaction, and absence of reward supervision, preventing direct policy learning in target domains.

Method: Proposes BDGxRL framework that leverages Diffusion Schrödinger Bridge to align source transitions with target-domain dynamics from offline demonstrations, plus a reward modulation mechanism that estimates rewards based on state transitions for DSB-aligned samples.

Result: Experiments on MuJoCo cross-domain benchmarks show BDGxRL outperforms state-of-the-art baselines and demonstrates strong adaptability under transition dynamics shifts.

Conclusion: BDGxRL enables effective target-oriented policy learning entirely within the source domain without requiring access to the target environment or its rewards, addressing key challenges in cross-domain RL.
...
### [170] [OPTIAGENT: A Physics-Driven Agentic Framework for Automated Optical Design](https://arxiv.org/abs/2602.23761)
*Yuyu Geng,Lei Sun,Yao Gao,Xinxin Hu,Zhonghua Yi,Xiaolong Qian,Weijian Hu,Jian Bai,Kaiwei Wang*

Main category: cs.LG

TL;DR: First LLM-based optical design system that enables non-experts to create functional lens systems using hybrid training and physics-aligned optimization.


<details>
  <summary>Details</summary>
Motivation: Optical design is complex, non-convex, and requires expert knowledge. LLMs have optical knowledge but can't effectively apply it to lens design. Need to bridge expertise gap for non-experts.

Method: Created OptiDesignQA dataset with classical and novel lens configurations. Used hybrid objective of full-system synthesis and lens completion. Applied DrGRPO with Optical Lexicographic Reward for physics alignment. Integrated with optical optimization routines for fine-tuning.
...
Conclusion: Successfully demonstrated first LLM application to optical design, enabling non-experts to create functional lens systems through physics-aligned training and optimization integration.
...
### [171] [MAGE: Multi-scale Autoregressive Generation for Offline Reinforcement Learning](https://arxiv.org/abs/2602.23770)
*Chenxing Lin,Xinhui Gao,Haipeng Zhang,Xinran Li,Haitao Wang,Songzhu Mei,Chenglu Wen,Weiquan Liu,Siqi Shen,Cheng Wang*

Main category: cs.LG

TL;DR: MAGE is a multi-scale autoregressive generation method for offline RL that captures hierarchical trajectory structures to improve performance on long-horizon sparse-reward tasks.


<details>
  <summary>Details</summary>
Motivation: Existing generation-based offline RL methods struggle with long-horizon sparse-reward tasks. While hierarchical approaches help by decomposing problems, they often overlook the multi-scale temporal structure inherent in trajectories, leading to suboptimal performance.

Method: MAGE uses a condition-guided multi-scale autoencoder to learn hierarchical trajectory representations, combined with a multi-scale transformer that autoregressively generates trajectory representations from coarse to fine temporal scales. It also employs a condition-guided decoder for precise control over short-term behaviors.
...
Conclusion: MAGE effectively addresses the limitations of existing generation-based offline RL methods by capturing multi-scale temporal dependencies and enabling precise behavioral control, making it particularly suitable for challenging long-horizon sparse-reward tasks.
...
### [172] [TradeFM: A Generative Foundation Model for Trade-flow and Market Microstructure](https://arxiv.org/abs/2602.23784)
*Maxime Kawawa-Beaudan,Srijan Sood,Kassiani Papasotiriou,Daniel Borrajo,Manuela Veloso*

Main category: cs.LG

TL;DR: TradeFM is a 524M-parameter generative Transformer that learns from billions of trade events across 9K+ equities to model market microstructure, achieving better performance than baselines and zero-shot generalization to new markets.


<details>
  <summary>Details</summary>
Motivation: Foundation models have revolutionized domains like language and genomics by learning general-purpose representations from large-scale data. The authors aim to bring this paradigm to market microstructure, which has heterogeneous, multi-modal event streams that are challenging to model.

Method: Developed scale-invariant features and universal tokenization scheme to map heterogeneous trade events into unified discrete sequences, eliminating asset-specific calibration. Integrated with deterministic market simulator. Built 524M-parameter generative Transformer trained on billions of trade events across 9K+ equities.

Result: TradeFM-generated rollouts reproduce key stylized facts of financial returns (heavy tails, volatility clustering, no return autocorrelation). Achieves 2-3x lower distributional error than Compound Hawkes baselines. Generalizes zero-shot to geographically out-of-distribution APAC markets with moderate perplexity degradation.

Conclusion: Scale-invariant trade representations capture transferable structure in market microstructure, opening paths toward synthetic data generation, stress testing, and learning-based trading agents.
...
### [173] [Provable Subspace Identification of Nonlinear Multi-view CCA](https://arxiv.org/abs/2602.23785)
*Zhiwei Han,Stefan Matthes,Hao Shen*

Main category: cs.LG

TL;DR: The paper proves that multi-view nonlinear CCA can identify correlated signal subspaces up to orthogonal ambiguities, with exact recovery for N≥3 views under certain conditions.


<details>
  <summary>Details</summary>
Motivation: Traditional nonlinear CCA unmixing is ill-posed; the paper aims to reframe it as a basis-invariant subspace identification problem to achieve identifiable recovery of correlated subspaces.

Method: Multi-view CCA is reformulated as subspace identification rather than exact unmixing. Theoretical analysis uses spectral separation conditions and latent priors to prove subspace recovery up to orthogonal ambiguity.

Result: For N≥3 views, multi-view CCA isolates jointly correlated subspaces across all views while eliminating view-private variations. Finite-sample consistency guarantees are established via spectral perturbation theory.

Conclusion: Multi-view CCA can reliably recover correlated subspaces under appropriate conditions, with experiments validating theoretical findings and confirming necessity of assumptions.
...
### [174] [UPath: Universal Planner Across Topological Heterogeneity For Grid-Based Pathfinding](https://arxiv.org/abs/2602.23789)
*Aleksandr Ananikian,Daniil Drozdov,Konstantin Yakovlev*

Main category: cs.LG

TL;DR: Universal heuristic predictor for A* pathfinding that generalizes across unseen grid maps, reducing computational effort by up to 2.2x while maintaining near-optimal solutions.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based heuristics for A* pathfinding only work well on training-like maps and fail on out-of-distribution tasks, limiting practical use where universal solvers are needed.

Method: Design a universal heuristic predictor trained once but capable of generalizing across completely different problem instances, using deep neural networks to approximate informed heuristics.

Result: The approach halves A*'s computational effort by up to factor 2.2 while providing solutions within 3% of optimal cost on average, even on tasks completely different from training data.

Conclusion: First learnable solver to achieve milestone of generalizing across unseen tasks, enabling practical universal pathfinding solvers that maintain efficiency and near-optimality.
...
### [175] [GRAIL: Post-hoc Compensation by Linear Reconstruction for Compressed Networks](https://arxiv.org/abs/2602.23795)
*Wenwu Tang,Dong Wang,Lothar Thiele,Olga Saukh*

Main category: cs.LG

TL;DR: GRAIL is a zero-finetuning method that compensates for accuracy degradation in structured model compression by reconstructing original hidden representations from compressed ones using ridge regression on Gram matrices, then absorbing the reconstruction into downstream weights.


<details>
  <summary>Details</summary>
Motivation: Structured compression methods reduce memory/inference costs but cause accuracy degradation under aggressive compression. Post-compression finetuning is often impractical due to missing labeled data or high training costs, creating a need for zero-finetuning solutions.

Method: Post-hoc blockwise compensation using Gram matrices to summarize hidden activations, then applying ridge regression to linearly reconstruct original hidden representations from compressed ones. The reconstruction map is absorbed into downstream projection weights while upstream layers are compressed.

Result: GRAIL consistently improves accuracy/perplexity over data-free and data-aware pruning/folding baselines across ResNets, ViTs, and decoder-only LLMs in practical compression regimes, with manageable overhead and no backpropagation.

Conclusion: GRAIL provides an effective zero-finetuning solution for structured model compression that is selector-agnostic, data-aware (requires only forward passes without gradients/labels), and recovers classic methods when inter-channel correlations are weak.
...
### [176] [MPU: Towards Secure and Privacy-Preserving Knowledge Unlearning for Large Language Models](https://arxiv.org/abs/2602.23798)
*Tiantong Wang,Xinyu Yan,Tiantong Wu,Yurong Hao,Yong Jiang,Fei Huang,Wei Yang Bryan Lim*

Main category: cs.LG

TL;DR: MPU is a privacy-preserving framework for machine unlearning in LLMs that addresses dual non-disclosure constraints by using multiple perturbed copies, allowing clients to perform unlearning locally without accessing server parameters while maintaining performance comparable to noise-free baselines.


<details>
  <summary>Details</summary>
Motivation: Address the privacy dilemma in machine unlearning where strict constraints prohibit sharing either the server's parameters or the client's forget set (dual non-disclosure constraint).

Method: Proposes MPU framework with two server-side modules: Pre-Process for generating multiple perturbed and reparameterized model instances, and Post-Process for inverting reparameterization and aggregating updates with harmonic denoising to mitigate perturbation effects.

Result: Experiments with seven unlearning algorithms show MPU achieves comparable performance to noise-free baselines, with most algorithms' average degradation below 1% under 10% noise, and can even outperform noise-free baseline for some algorithms under 1% noise.

Conclusion: MPU effectively addresses dual non-disclosure constraints in machine unlearning while maintaining competitive unlearning performance through its perturbed copies approach and denoising aggregation.
...
### [177] [Actor-Critic Pretraining for Proximal Policy Optimization](https://arxiv.org/abs/2602.23804)
*Andreas Kernbach,Amr Elsheikh,Nicolas Grupp,René Nagel,Marco F. Huber*

Main category: cs.LG

TL;DR: Actor-critic pretraining using expert demonstrations improves RL sample efficiency by 86.1% over no pretraining and 30.9% over actor-only pretraining.


<details>
  <summary>Details</summary>
Motivation: RL actor-critic algorithms require many environment interactions, limiting robotics applications. While actor pretraining with expert data is common, critic initialization is often neglected despite its crucial role in policy optimization.
...
Result: Evaluated on 15 simulated robotic manipulation and locomotion tasks. Actor-critic pretraining improves sample efficiency by 86.1% average over no pretraining and 30.9% over actor-only pretraining.

Conclusion: Pretraining both actor and critic networks with expert demonstrations significantly enhances RL sample efficiency, making it more practical for robotics applications where environment interactions are costly.
...
### [178] [Beyond State-Wise Mirror Descent: Offline Policy Optimization with Parameteric Policies](https://arxiv.org/abs/2602.23811)
*Xiang Li,Nan Jiang,Yuheng Zhang*

Main category: cs.LG

TL;DR: Extends offline RL theory to parameterized policies over large/continuous action spaces, addressing computational tractability and standalone policy parameterization limitations of prior work.


<details>
  <summary>Details</summary>
Motivation: Prior offline RL algorithms with theoretical guarantees are computationally tractable only for finite/small action spaces, rely on state-wise mirror descent, and require actors to be implicitly induced from critics rather than supporting standalone policy parameterization common in practice.

Method: Extends mirror descent to parameterized policies by addressing contextual coupling difficulty, connecting mirror descent to natural policy gradient, and developing novel analyses and algorithmic insights.

Result: Theoretical guarantees extended to parameterized policy classes over large or continuous action spaces, with surprising unification between offline RL and imitation learning.

Conclusion: Successfully addresses limitations of prior offline RL theory by enabling computationally tractable algorithms for parameterized policies over large/continuous action spaces, with novel connections to natural policy gradient and imitation learning.
...
### [179] [Learning to maintain safety through expert demonstrations in settings with unknown constraints: A Q-learning perspective](https://arxiv.org/abs/2602.23816)
*George Papadopoulos,George A. Vouros*

Main category: cs.LG

TL;DR: SafeQIL algorithm learns safe policies from demonstrations in constrained MDPs with unknown constraints by balancing reward maximization and safety using Q-values that incorporate both task rewards and safety assessments.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of learning safe policies from demonstrations in constrained MDPs where constraints are unknown and costs are non-observable, but rewards are observable. The goal is to find policies that maximize the likelihood of demonstrated trajectories while balancing between being conservative (safe) and pursuing high-reward trajectories that may contain unsafe steps.

Method: The authors formulate the "promise" of state-action pairs using Q-values that incorporate both task-specific rewards and safety assessments. They develop Safe Q Inverse Constrained Reinforcement Learning (SafeQIL), which takes a safe Q-learning perspective on the inverse learning problem under constraints. The method learns from demonstrations to maximize the probability of the most promising trajectories.

Result: SafeQIL is compared to state-of-the-art inverse constraint reinforcement learning algorithms on a set of challenging benchmark tasks, demonstrating its merits and effectiveness in learning safe policies from demonstrations.

Conclusion: The paper presents SafeQIL as an effective approach for learning safe policies from demonstrations in constrained MDPs with unknown constraints, successfully balancing reward maximization and safety through a Q-learning framework that incorporates both reward expectations and safety assessments.
...
### [180] [Inferring Chronic Treatment Onset from ePrescription Data: A Renewal Process Approach](https://arxiv.org/abs/2602.23824)
*Pavlin G. Poličar,Dalibor Stanimirović,Blaž Zupan*

Main category: cs.LG

TL;DR: A probabilistic framework using prescription renewal patterns to infer chronic disease onset, overcoming left-censoring issues in EHR data by detecting transitions from sporadic to sustained therapy.


<details>
  <summary>Details</summary>
Motivation: Longitudinal EHR data often suffers from left-censoring, making diagnosis records incomplete and unreliable for determining disease onset. Prescription trajectories provide continuous signals of disease management that can overcome this limitation.

Method: Probabilistic framework modeling prescription dynamics as a renewal process, detecting transitions from sporadic to sustained therapy via change-point detection between baseline Poisson (sporadic) and regime-specific Weibull (sustained therapy) renewal models.

Result: Using nationwide ePrescription data of 2.4M individuals, the approach yields more temporally plausible onset estimates than naive rule-based triggering, substantially reducing implausible early detections under strong left censoring. Performance varies by disease and is strongly associated with prescription density.

Conclusion: The proposed treatment-based onset inference framework effectively addresses left-censoring issues in EHR data, though detection performance depends on prescription patterns, highlighting both the strengths and limitations of using prescription data for disease onset inference.
...
### [181] [FedNSAM:Consistency of Local and Global Flatness for Federated Learning](https://arxiv.org/abs/2602.23827)
*Junkang Liu,Fanhua Shang,Yuxuan Tian,Hongying Liu,Yuanyuan Liu*

Main category: cs.LG

TL;DR: FedNSAM improves federated learning by using global Nesterov momentum to align local and global model flatness, addressing the limitation of standard SAM in heterogeneous data settings.


<details>
  <summary>Details</summary>
Motivation: In federated learning, multi-step local updates and data heterogeneity cause sharp global minima that degrade model performance. While SAM helps find flatter minima, local flatness doesn't guarantee global flatness in heterogeneous data settings, limiting SAM's effectiveness in FL.
...
Conclusion: FedNSAM effectively addresses the flatness inconsistency problem in federated learning with heterogeneous data, improving generalization ability through better alignment of local and global model flatness via Nesterov momentum.
...
### [182] [A Theory of Random Graph Shift in Truncated-Spectrum vRKHS](https://arxiv.org/abs/2602.23880)
*Zhang Wan,Tingting Mu,Samuel Kaski*

Main category: cs.LG

TL;DR: The paper develops a theory for graph classification under domain shift using random graph models, deriving a generalization bound with three factorizable components.


<details>
  <summary>Details</summary>
Motivation: Existing domain adaptation theories don't adequately handle the structured nature of graph data, and the non-Euclidean properties of graphs complicate fine-grained analysis of distribution shifts in graph learning.

Method: Proposes a theory using random graph models as data generative process, connecting to hypothesis complexity in function space. Builds on vector-valued reproducing kernel Hilbert space (vRKHS) formulation to derive a generalization bound.

Result: Derives a generalization bound whose shift penalty factorizes into: (1) domain discrepancy term, (2) spectral-geometry term from truncated spectrum, and (3) amplitude term aggregating convergence and construction-stability effects.

Conclusion: The theory provides a fine-grained analysis of graph distribution shifts, with empirical verification on real data and simulations showing insights into the three factorizable terms.
...
### [183] [LK Losses: Direct Acceptance Rate Optimization for Speculative Decoding](https://arxiv.org/abs/2602.23881)
*Alexander Samarin,Sergei Krutikov,Anton Shevtsov,Sergei Skvortsov,Filipp Fisin,Alexander Golubev*

Main category: cs.LG

TL;DR: LK losses improve speculative decoding by directly optimizing acceptance rate instead of KL divergence, achieving 8-10% gains in average acceptance length across various model sizes and domains.


<details>
  <summary>Details</summary>
Motivation: Standard draft model training minimizes KL divergence as a proxy for acceptance rate, but small draft models with limited capacity converge to suboptimal solutions where minimizing KL doesn't guarantee maximizing acceptance rate, limiting speculative decoding speedup.

Method: Proposes LK losses - special training objectives that directly target acceptance rate instead of using KL divergence as a proxy. These losses are easy to implement, introduce no computational overhead, and can be integrated into any existing speculator training framework.
...
Conclusion: LK losses provide a compelling alternative to standard KL-based training for draft models in speculative decoding, offering direct optimization of acceptance rate with consistent performance improvements across diverse configurations.
...
### [184] [Hierarchical Concept-based Interpretable Models](https://arxiv.org/abs/2602.23947)
*Oscar Hill,Mateo Espinosa Zarlenga,Mateja Jamnik*

Main category: cs.LG

TL;DR: HiCEMs introduce hierarchical concept structures to model inter-concept relationships, with Concept Splitting automatically discovering finer sub-concepts without extra annotations, enabling granular explanations and interventions.


<details>
  <summary>Details</summary>
Motivation: Existing Concept Embedding Models (CEMs) fail to represent inter-concept relationships and require concept annotations at different granularities during training, limiting their applicability for model interpretation, debugging, and debiasing.

Method: Hierarchical Concept Embedding Models (HiCEMs) explicitly model concept relationships through hierarchical structures. Concept Splitting automatically discovers finer-grained sub-concepts from a pretrained CEM's embedding space without requiring additional annotations.

Result: Concept Splitting discovers human-interpretable sub-concepts absent during training that can train accurate HiCEMs. HiCEMs enable powerful test-time concept interventions at different granularities, leading to improved task accuracy across multiple datasets including PseudoKitchens.

Conclusion: HiCEMs with Concept Splitting provide hierarchical concept representations that reduce annotation burdens while enabling fine-grained explanations and interventions, advancing interpretable deep learning.
...
### [185] [Learning Generation Orders for Masked Discrete Diffusion Models via Variational Inference](https://arxiv.org/abs/2602.23968)
*David Fox,Sam Bowyer,Song Liu,Laurence Aitchison,Raul Santos-Rodriguez,Mengyue Yang*

Main category: cs.LG

TL;DR: A variational inference framework for learning parallel generation orders in masked discrete diffusion models, achieving competitive accuracy with fewer generation steps compared to heuristic methods.


<details>
  <summary>Details</summary>
Motivation: Masked discrete diffusion models enable parallel token generation but struggle to balance efficiency and quality. Current approaches use fixed heuristic methods, and learning-based approaches lack proper variational inference formulation.

Method: Proposes a variational inference framework for learning parallel generation orders, with a parameterization for approximate posterior of generation orders that enables parallelism and efficient sampling during training.

Result: On GSM8K dataset, achieves 33.1% accuracy with only 4 generation steps, outperforming standard methods (23.7-29.0% accuracy) in same step count.

Conclusion: The method shows promise for parallel generation with MDMs, and further experiments could provide valuable insights into balancing parallel generation and sample quality.
...
### [186] [Intrinsic Lorentz Neural Network](https://arxiv.org/abs/2602.23981)
*Xianglong Shi,Ziheng Chen,Yunhan Jiang,Nicu Sebe*

Main category: cs.LG

TL;DR: ILNN is a fully intrinsic hyperbolic neural network using Lorentz geometry with novel point-to-hyperplane fully connected layers and intrinsic modules for state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Real-world data often has hierarchical structures naturally represented by hyperbolic geometry, but existing hyperbolic neural networks are partially intrinsic, mixing Euclidean operations or relying on extrinsic parameterizations.

Method: Proposes Intrinsic Lorentz Neural Network (ILNN) with all computations within Lorentz model. Core innovation is point-to-hyperplane fully connected layer using hyperbolic distances to learned Lorentz hyperplanes. Includes GyroLBN (Lorentz batch normalization), gyro-additive bias, Lorentz patch-concatenation operator with digamma-based scale, and Lorentz dropout layer.

Result: ILNN achieves state-of-the-art performance and computational cost among hyperbolic models on CIFAR-10/100 and genomic benchmarks (TEB and GUE), consistently surpassing strong Euclidean baselines.

Conclusion: ILNN demonstrates that fully intrinsic hyperbolic architectures can outperform both existing hyperbolic models and Euclidean baselines while maintaining computational efficiency, providing a complete framework for hyperbolic deep learning.
...
TL;DR: MINT transfers MRI biomarker knowledge to speech encoder for Alzheimer's screening without requiring MRI at inference, achieving comparable performance to speech-only methods while providing biologically grounded decision boundaries.


<details>
  <summary>Details</summary>
Motivation: MRI provides reliable biomarkers for Alzheimer's disease but is expensive and inaccessible at population scale. Speech analysis offers a non-invasive alternative but lacks biological grounding and reliability for subtle cognitive impairment detection.

Method: Three-stage cross-modal framework: 1) Train MRI teacher on 1,228 subjects for CN-vs-MCI classification, 2) Use residual projection head to align speech representations to frozen MRI embedding space via geometric loss, 3) Apply frozen MRI classifier to aligned speech embeddings at inference without requiring MRI scanner.

Result: Aligned speech achieves AUC 0.720 vs 0.711 for speech-only baselines, comparable performance while requiring no imaging at inference. Multimodal fusion improves over MRI alone (0.973 vs 0.958). Dropout regularization and self-supervised pretraining identified as critical design decisions.
...
TL;DR: Foundation world models: persistent compositional representations that unify RL, program synthesis, and abstraction for adaptive agents in open worlds.


<details>
  <summary>Details</summary>
Motivation: Current autonomous agents are limited by fixed tasks/environments with little novelty, preventing world models from supporting policy evolution as conditions change. Need for agents that can adapt reliably in open worlds.

Method: Four-component agenda: (1) learnable reward models from specifications, (2) adaptive formal verification integrated throughout learning, (3) online abstraction calibration to quantify prediction reliability, (4) test-time synthesis and world-model generation guided by verifiers.

Result: Enables agents to synthesize verifiable programs, derive new policies from few interactions, maintain correctness while adapting to novelty, and provide explainable/justifiable behavior.

Conclusion: Foundation world models serve as substrate for learning, reasoning, and adaptation, laying groundwork for agents that act well and can explain/justify their behavior in open worlds.
...
TL;DR: RewardUQ introduces a unified framework to systematically evaluate uncertainty quantification methods for reward models in LLM alignment, finding that model size and initialization matter most.


<details>
  <summary>Details</summary>
Motivation: Current reward models for LLM alignment rely on pointwise estimates that ignore epistemic uncertainty from limited human feedback. While uncertainty-aware models show promise for reducing annotation costs and mitigating reward overoptimization, they lack systematic comparison and are poorly understood.

Method: Developed RewardUQ, a unified framework to systematically evaluate uncertainty quantification methods for reward models. Compared common methods using standard accuracy and calibration metrics, and proposed a new ranking strategy combining both dimensions for simplified comparison.
...
Conclusion: The work provides a systematic evaluation framework (RewardUQ) for uncertainty quantification in reward models, releases it as an open-source Python package to foster development and aid deployment in downstream applications.
...
TL;DR: pathsig is a PyTorch-native library for computing path signatures with high GPU throughput and minimal memory, offering 10-30x speedups over existing libraries and supporting flexible signature projections.


<details>
  <summary>Details</summary>
Motivation: Existing path signature libraries lack scalability for large-scale, gradient-based learning, creating a gap between the theoretical promise of signatures and practical implementation in modern machine learning workflows.

Method: Develops a PyTorch-native library that computes path signatures directly in the word basis using CUDA kernels to update signature coefficients in parallel over prefix-closed word sets, achieving high GPU throughput and near-minimal peak memory.

Result: pathsig achieves 10-30x speedups for truncated signature computation and up to 4-10x speedups in training requiring backpropagation through signatures, while supporting flexible projections and anisotropic truncation for more compact representations.

Conclusion: pathsig bridges the gap between path signature theory and practical implementation, enabling scalable, efficient signature computation for modern gradient-based learning while offering flexible representation options to reduce dimensionality and computational cost.
...
TL;DR: COVE is a high-dimensional, explainable node embedding method that uses co-occurrence on random walks for similarity, performs similarly to Louvain when reduced with UMAP and clustered with HDBSCAN.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of low-dimensional node embeddings by creating an explainable high-dimensional embedding that maintains performance while being more interpretable.

Method: COVE uses non-linear dimension reduction and is inspired by neural embedding methods that leverage co-occurrence on random walks as similarity indicators, relating to diffusion processes. The method involves creating high-dimensional embeddings then reducing them with UMAP for downstream tasks.

Result: COVE embeddings, when reduced to low dimension with UMAP, slightly increase performance on clustering and link prediction tasks. The COVE-UMAP-HDBSCAN pipeline performs similarly to the popular Louvain algorithm on community detection benchmarks.

Conclusion: High-dimensional, explainable embeddings like COVE can achieve comparable performance to established methods like Louvain while offering better interpretability through their connection to diffusion processes and random walk co-occurrence patterns.
...
### [192] [Adaptive Correlation-Weighted Intrinsic Rewards for Reinforcement Learning](https://arxiv.org/abs/2602.24081)
*Viet Bac Nguyen,Phuong Thai Nguyen*

Main category: cs.LG

TL;DR: ACWI is an adaptive intrinsic reward scaling framework that dynamically balances intrinsic and extrinsic rewards for better exploration in sparse reward RL, replacing manual tuning with learned state-dependent scaling.


<details>
  <summary>Details</summary>
Motivation: Conventional approaches use manually tuned scalar coefficients to balance intrinsic and extrinsic rewards, which often leads to unstable or suboptimal performance across different tasks. There's a need for an adaptive method that can dynamically adjust this balance during training.

Method: ACWI introduces a lightweight Beta Network that predicts intrinsic reward weights directly from agent states using an encoder-based architecture. It optimizes scaling through a correlation-based objective that aligns weighted intrinsic rewards with discounted future extrinsic returns, enabling task-adaptive exploration incentives.

Result: ACWI consistently improves sample efficiency and learning stability compared to fixed intrinsic reward baselines on sparse reward environments in MiniGrid, achieving superior performance with minimal computational overhead.

Conclusion: ACWI provides an effective adaptive framework for balancing intrinsic and extrinsic rewards in sparse reward RL, offering better exploration, improved stability, and task-adaptive performance without the need for manual coefficient tuning.
...
### [193] [Neural Diffusion Intensity Models for Point Process Data](https://arxiv.org/abs/2602.24083)
*Xinlong Du,Harsha Honnappa,Vinayak Rao*

Main category: cs.LG

TL;DR: Neural Diffusion Intensity Models: A variational framework using neural SDEs for tractable Cox process inference with guaranteed posterior inclusion and amortized inference.


<details>
  <summary>Details</summary>
Motivation: Cox processes are useful for modeling overdispersed point process data but suffer from intractable nonparametric estimation and expensive MCMC-based posterior inference, creating a need for more efficient methods.

Method: Proposes Neural Diffusion Intensity Models using neural SDEs with a variational framework. Key theoretical result shows conditioning on point process observations preserves diffusion structure with explicit drift correction. Uses amortized encoder architecture that maps event sequences to posterior intensity paths via drift-corrected SDE simulation.
...
Conclusion: The framework provides tractable variational inference for Cox processes with theoretical guarantees, replacing expensive MCMC with efficient amortized inference while maintaining accuracy.
...
### [194] [Agentic AI-RAN: Enabling Intent-Driven, Explainable and Self-Evolving Open RAN Intelligence](https://arxiv.org/abs/2602.24115)
*Zhizhou He,Yang Luo,Xinkai Liu,Mahdi Boloursaz Mashhadi,Mohammad Shojafar,Merouane Debbah,Rahim Tafazolli*

Main category: cs.LG

TL;DR: This paper proposes using agentic AI controllers in O-RAN architecture to improve multi-tenant, multi-objective RAN operations, showing 8.83% average resource reduction across network slices.


<details>
  <summary>Details</summary>
Motivation: O-RAN exposes rich control interfaces but makes it harder to operate multi-tenant, multi-objective RANs safely and auditably. Agentic AI systems with planning, tool use, memory, and self-management offer a natural way to structure long-lived control loops for these complex operations.

Method: The paper introduces agentic primitives (Plan-Act-Observe-Reflect, skills as tool use, memory and evidence, and self-management gates) and applies them to O-RAN architecture. It organizes tasks around three clusters: network slice life-cycle, radio resource management closed loops, and cross-cutting security/privacy/compliance. The approach is evaluated in a multi-cell O-RAN simulation comparing against conventional baselines and ablations.

Result: The agentic controller framework achieves an average 8.83% reduction in resource usage across three classic network slices compared to conventional baselines. The paper also demonstrates how removing individual agentic primitives reduces performance, showing the value of the complete framework.

Conclusion: Agentic AI controllers offer a promising approach for managing complex O-RAN operations, improving performance while addressing security, privacy, and compliance challenges. The framework provides a structured way to implement long-lived control loops in multi-tenant RAN environments, though standards-aligned deployments still face open challenges.
...
### [195] [Learning with a Budget: Identifying the Best Arm with Resource Constraints](https://arxiv.org/abs/2602.24146)
*Zitian Li,Wang Chi Cheung*

Main category: cs.LG

TL;DR: Proposes SH-RR algorithm for Best Arm Identification with Resource Constraints, unifying stochastic and deterministic consumption settings with effective consumption measure.


<details>
  <summary>Details</summary>
Motivation: Many real-world applications involve evaluating alternatives with varying costs/resource usage, requiring algorithms that can identify the best option under heterogeneous resource constraints.

Method: Successive Halving with Resource Rationing (SH-RR) algorithm integrates resource-aware allocation into classical successive halving framework for best arm identification.

Result: SH-RR provides unified theoretical analysis for both stochastic and deterministic consumption settings using a new effective consumption measure.

Conclusion: The proposed algorithm effectively addresses resource-constrained best arm identification problems with theoretical guarantees across different consumption models.
...
### [196] [What You Read is What You Classify: Highlighting Attributions to Text and Text-Like Inputs](https://arxiv.org/abs/2602.24149)
*Daniel S. Berman,Brian Merritt,Stanley Ta,Dana Udwin,Amanda Ernlund,Jeremy Ratcliff,Vijay Narayan*

Main category: cs.LG

TL;DR: A new explainable AI method for discrete token inputs (like text) that uses mask-based explanations, generalizing image explanation techniques to token sequences by training an Explainer network to identify important segments.


<details>
  <summary>Details</summary>
Motivation: Current explainable AI methods don't work well for discrete token inputs like text because they either identify disparate important tokens or assign low importance to too many tokens. Transformers focus on global connections, making local explanations difficult.

Method: Generalizes mask-based explainable AI from images to tokens. Uses an Explainer neural network trained to create masks that hide irrelevant information for classification. Takes Hadamard product of mask and classifier's embedding layer values, changing magnitude but keeping orientation unchanged.

Result: The method was trained on a taxonomic classifier for nucleotide sequences, showing that masked segments are less relevant to classification than unmasked ones. Produces human-readable explanations by focusing on token segments as a whole.

Conclusion: The proposed method successfully addresses explainable AI for token-based classifiers by providing segment-level explanations that are more interpretable than existing approaches, bridging the gap between image and text explanation techniques.
...
### [197] [Sandwiching Polynomials for Geometric Concepts with Low Intrinsic Dimension](https://arxiv.org/abs/2602.24178)
*Adam R. Klivans,Konstantinos Stavropoulos,Arsen Vasilyan*

Main category: cs.LG

TL;DR: New method constructs low-degree sandwiching polynomials with exponentially improved degree bounds for fundamental function classes like k halfspaces under Gaussian distribution.


<details>
  <summary>Details</summary>
Motivation: Low-degree sandwiching polynomials have shown surprising power in challenging learning settings (distribution shift, testable learning, contamination), but prior constructions had exponential degree bounds. The paper aims to develop better sandwiching polynomial constructions.

Method: New approach uses smoothness of target function's boundary to construct sandwiching Lipschitz functions, which are then amenable to high-dimensional approximation theory results. The proof is simpler than prior work and avoids complex techniques like FT-mollification.

Result: Achieves poly(k) degree sandwiching polynomials for functions of k halfspaces under Gaussian distribution (exponential improvement over prior 2^O(k) bound). For low-dimensional polynomial threshold functions (PTFs) with respect to Gaussians, obtains doubly exponential improvements.

Conclusion: The new method provides significantly better sandwiching polynomial constructions for fundamental function classes, with simpler proofs and broader applicability to low-dimensional function classes with smooth boundaries.
...
### [198] [Multi-Objective Reinforcement Learning for Large-Scale Tote Allocation in Human-Robot Collaborative Fulfillment Centers](https://arxiv.org/abs/2602.24182)
*Sikata Sengupta,Guangyi Liu,Omer Gottesman,Joseph W Durham,Michael Kearns,Aaron Roth,Michael Caldara*

Main category: cs.LG

TL;DR: The paper presents a Multi-Objective Reinforcement Learning approach for optimizing container consolidation in fulfillment centers, balancing processing speed, resource usage, and space utilization while satisfying operational constraints.


<details>
  <summary>Details</summary>
Motivation: Container-based fulfillment centers need to optimize consolidation processes that involve moving items between containers using human and robotic workstations. This requires balancing competing objectives like processing speed, resource usage, and space utilization while adhering to real-world operational constraints.

Method: Formulates the problem as large-scale MORL with high-dimensional state spaces. Builds on theoretical advances in constrained RL via best-response and no-regret dynamics in zero-sum games for principled minimax policy learning. Introduces theoretical framework to handle error cancellation and oscillatory behavior in time-averaged solutions.

Result: Policy evaluation on realistic warehouse simulations shows effective trade-offs between objectives. Empirically learns a single policy that simultaneously satisfies all constraints. Theoretical framework returns a single iterate with Lagrangian value close to the minimax value of the game.

Conclusion: Demonstrates the promise of MORL for solving complex, high-impact decision-making problems in large-scale industrial systems like fulfillment center optimization.
...
### [199] [Flow-Based Density Ratio Estimation for Intractable Distributions with Applications in Genomics](https://arxiv.org/abs/2602.24201)
*Egor Antipov,Alessandro Palma,Lorenzo Consoli,Stephan Günnemann,Andrea Dittadi,Fabian J. Theis*

Main category: cs.LG

TL;DR: A flow-based method for efficient density ratio estimation between intractable distributions using condition-aware flow matching, applied to single-cell genomics analysis.


<details>
  <summary>Details</summary>
Motivation: Density ratio estimation is crucial for comparing sample likelihoods under different data-generating processes, but existing flow-based methods are computationally expensive due to separate likelihood integrals for each distribution.

Method: Leverages condition-aware flow matching to derive a single dynamical formulation for tracking density ratios along generative trajectories, avoiding separate costly likelihood computations.

Result: Competitive performance on simulated benchmarks for closed-form ratio estimation, and versatile application to single-cell genomics for treatment effect estimation and batch correction evaluation.

Conclusion: The proposed method provides an efficient approach to density ratio estimation that enables principled comparisons across conditions and covariates, particularly valuable in biological data analysis.
...
### [200] [The Stability of Online Algorithms in Performative Prediction](https://arxiv.org/abs/2602.24207)
*Gabriele Farina,Juan Carlos Perdomo*

Main category: cs.LG

TL;DR: No-regret algorithms in performative prediction converge to performatively stable equilibria without restrictive assumptions, using martingale arguments and randomization to avoid hardness results.


<details>
  <summary>Details</summary>
Motivation: Algorithmic predictions in decision-making create feedback loops where deployed models influence data distributions used for retraining, raising concerns about stability and convergence in performative prediction settings.

Method: Uses an unconditional reduction showing any no-regret algorithm converges to performatively stable equilibria, employing martingale arguments and allowing randomization to avoid restrictive assumptions about model influence on distributions.

Result: Demonstrates convergence to (mixed) performatively stable equilibria without strong restrictions on how models influence distributions, sidestepping recent hardness results for finding stable models.

Conclusion: Provides a general framework connecting online optimization and performativity, explaining why common algorithms like gradient descent are naturally stabilizing and preventing runaway feedback loops, enabling future technical transfer between these fields.
...
### [201] [An Efficient Unsupervised Federated Learning Approach for Anomaly Detection in Heterogeneous IoT Networks](https://arxiv.org/abs/2602.24209)
*Mohsen Tajgardan,Atena Shiranzaei,Mahdi Rabbani,Reza Khoshkangini,Mahtab Jamali*

Main category: cs.LG

TL;DR: Proposes an unsupervised federated learning framework for IoT anomaly detection that leverages shared features from complementary datasets while preserving dataset-specific features, using explainable AI for interpretability.


<details>
  <summary>Details</summary>
Motivation: Federated learning is promising for IoT anomaly detection but faces challenges from data heterogeneity across devices (different capabilities, formats, constraints) which affects both model performance and privacy preservation in unsupervised settings.

Method: An unsupervised FL framework that extracts and leverages shared features from two IoT datasets (anomaly detection + device identification) while preserving dataset-specific features. Uses explainable AI techniques like SHAP to identify key features influencing local model decisions for transparency.
...
Conclusion: Demonstrates the potential of using shared features from complementary datasets to optimize unsupervised federated learning for superior anomaly detection in decentralized IoT environments.
...
### [202] [Comparing Classical and Quantum Variational Classifiers on the XOR Problem](https://arxiv.org/abs/2602.24220)
*Miras Seilkhan,Adilbek Taizhanov*

Main category: cs.LG

TL;DR: Quantum and classical models compared on XOR problem: deeper quantum circuits match neural network accuracy but show no clear advantage in robustness or efficiency.


<details>
  <summary>Details</summary>
Motivation: To compare classical machine learning models with variational quantum classifiers on the XOR problem, examining how quantum principles like superposition and entanglement affect model expressivity and performance.

Method: Evaluated logistic regression, one-hidden-layer multilayer perceptron, and two-qubit variational quantum classifiers (circuit depths 1 and 2) on synthetic XOR datasets with varying Gaussian noise and sample sizes. Used accuracy and binary cross-entropy metrics, with robustness analyses across noise levels, dataset sizes, and random seeds.

Result: Performance primarily determined by model expressivity. Logistic regression and depth-1 quantum circuit failed to represent XOR reliably, while multilayer perceptron and depth-2 quantum circuit achieved perfect test accuracy under representative conditions. Despite matching accuracy, multilayer perceptron achieved lower binary cross-entropy and substantially shorter training time. Hardware execution preserved global XOR structure but introduced structured deviations.

Conclusion: Deeper variational quantum classifiers can match classical neural networks in accuracy on low-dimensional XOR benchmarks, but no clear empirical advantage in robustness or efficiency was observed in the examined settings.
...
### [203] [Adaptive Combinatorial Experimental Design: Pareto Optimality for Decision-Making and Inference](https://arxiv.org/abs/2602.24231)
*Hongrui Xie,Junyu Cao,Kan Xu*

Main category: cs.LG

TL;DR: First investigation of adaptive combinatorial experimental design in CMAB, balancing regret minimization and statistical power through Pareto optimality with algorithms for full-bandit and semi-bandit feedback.


<details>
  <summary>Details</summary>
Motivation: Address the fundamental trade-off between regret minimization (exploitation) and statistical power (exploration) in combinatorial multi-armed bandits, which hasn't been formally investigated before.

Method: Formalize the trade-off through Pareto optimality concept. Propose two algorithms: MixCombKL for full-bandit feedback and MixCombUCB for semi-bandit feedback, with theoretical guarantees for both regret and estimation error.

Result: Both algorithms are Pareto optimal with finite-time guarantees on regret and arm gap estimation. Richer feedback (semi-bandit) significantly tightens the attainable Pareto frontier, primarily improving estimation accuracy.

Conclusion: Establishes a principled framework for adaptive combinatorial experimentation in multi-objective decision-making, showing how different feedback structures affect the trade-off between learning and optimization.
...
### [204] [HumanMCP: A Human-Like Query Dataset for Evaluating MCP Tool Retrieval Performance](https://arxiv.org/abs/2602.23367)
...
Method: Developed on MCP Zero dataset, created large-scale MCP dataset with diverse user queries matching 2800 tools across 308 MCP servers. Each tool paired with multiple unique user personas capturing varying levels of user intent from precise task requests to ambiguous, exploratory commands.

Result: Introduced first large-scale MCP dataset featuring diverse, high-quality user queries that reflect real-world interaction patterns and complexity.

Conclusion: This dataset addresses critical gap in evaluating MCP server tool usage ecosystems by providing realistic user queries that better represent how different users actually interact with tools.
...
### [205] [An Agentic LLM Framework for Adverse Media Screening in AML Compliance](https://arxiv.org/abs/2602.23373)
...
<details>
  <summary>Details</summary>
Motivation: Traditional adverse media screening for AML/KYC compliance relies on keyword-based searches that generate high false-positive rates and require extensive manual review, creating inefficiencies in financial institutions' compliance processes.

Method: A multi-step agentic system leveraging LLMs with Retrieval-Augmented Generation (RAG) that searches the web, retrieves and processes relevant documents, and computes an Adverse Media Index (AMI) score for each subject.

Result: The system was evaluated using multiple LLM backends on a dataset including PEPs, regulatory watchlist persons, sanctioned individuals from OpenSanctions, and clean names from academic sources, demonstrating ability to distinguish between high-risk and low-risk individuals.

Conclusion: The proposed agentic LLM-RAG system provides an effective automated solution for adverse media screening that improves accuracy and reduces manual workload in AML/KYC compliance processes.
...
### [206] [Causal Identification from Counterfactual Data: Completeness and Bounding Results](https://arxiv.org/abs/2602.23541)
*Arvind Raghavan,Elias Bareinboim*

Main category: cs.AI

TL;DR: The paper develops CTFIDU+, a complete algorithm for identifying counterfactual queries from Layer 3 distributions, establishes theoretical limits of counterfactual identification from physically realizable distributions, and derives analytic bounds for non-identifiable counterfactuals.


<details>
  <summary>Details</summary>
Motivation: Recent work showed that some counterfactual distributions (Layer 3) can be directly estimated via experiments (counterfactual realizability), raising the question of what additional counterfactual quantities become identifiable given access to Layer 3 data.

Method: Developed the CTFIDU+ algorithm for identifying counterfactual queries from arbitrary Layer 3 distributions, proved its completeness, established theoretical limits of counterfactual identification from physically realizable distributions, and derived novel analytic bounds for non-identifiable quantities.

Result: CTFIDU+ is complete for identifying counterfactual queries from Layer 3 distributions, revealing the fundamental limit to exact causal inference in non-parametric settings, and counterfactual data helps tighten bounds for non-identifiable quantities in practice.

Conclusion: The work provides a complete solution for counterfactual identification from realizable Layer 3 data, establishes theoretical limits of causal inference, and offers practical bounds for quantities that remain non-identifiable.
...
### [207] [Planning under Distribution Shifts with Causal POMDPs](https://arxiv.org/abs/2602.23545)
*Matteo Ceriscioli,Karthika Mohan*

Main category: cs.AI

TL;DR: A theoretical framework for planning under distribution shifts using causal POMDPs, where environmental changes are modeled as interventions, enabling plan evaluation and domain identification while preserving tractable planning.


<details>
  <summary>Details</summary>
Motivation: Real-world planning faces challenges from distribution shifts where environment models become invalid as conditions change, causing previously learned strategies to fail. There's a need for planning methods that can handle such shifts in partially observable environments.

Method: Proposes using Partially Observable Markov Decision Processes (POMDPs) formulated with causal knowledge. Environmental shifts are represented as interventions on this causal POMDP. The framework maintains and updates beliefs over both latent states and the underlying domain, enabling evaluation of plans under hypothesized changes and active identification of altered environment components.

Result: Proves that the value function remains piecewise linear and convex (PWLC) in the augmented belief space that includes both state and domain beliefs. This preservation of PWLC structure maintains the tractability of planning using α-vector-based POMDP methods even under distribution shifts.

Conclusion: The causal POMDP framework provides a theoretically sound approach for planning under distribution shifts in partially observable environments, maintaining computational tractability while enabling adaptation to changing conditions through causal intervention modeling.
...
### [208] [Construct, Merge, Solve & Adapt with Reinforcement Learning for the min-max Multiple Traveling Salesman Problem](https://arxiv.org/abs/2602.23579)
*Guillem Rodríguez-Corominas,Maria J. Blesa,Christian Blum*

Main category: cs.AI

TL;DR: RL-CMSA: A hybrid reinforcement learning approach for min-max mTSP that combines probabilistic clustering with learned q-values, route merging, exact MILP solving, and local refinement to balance workload across multiple salesmen.


<details>
  <summary>Details</summary>
Motivation: The min-max mTSP aims to minimize the longest tour to achieve workload balance among multiple salesmen, which is important for practical applications like delivery routing and workforce management. Existing methods need improvement for larger instances and more salesmen.
...
Conclusion: The combination of exact optimization and reinforcement-guided construction effectively balances exploration and exploitation for the min-max mTSP, demonstrating superior performance over existing methods for larger and more complex instances.
...
### [209] [SleepLM: Natural-Language Intelligence for Human Sleep](https://arxiv.org/abs/2602.23605)
*Zongzhe Xu,Zitao Shuai,Eideen Mozaffari,Ravi S. Aysola,Rajesh Kumar,Yuzhe Yang*

Main category: cs.AI

TL;DR: SleepLM is a sleep-language foundation model that aligns natural language with multimodal sleep data, enabling language-based sleep analysis, interpretation, and interaction beyond traditional closed-label systems.


<details>
  <summary>Details</summary>
Motivation: Current learning-based sleep analysis systems are limited to predefined labels (stages/events) and cannot describe, query, or generalize to novel sleep phenomena. There's a need for more flexible, language-grounded approaches to sleep analysis.

Method: 1) Developed multilevel sleep caption generation pipeline to create first large-scale sleep-text dataset (100K+ hours from 10K+ individuals); 2) Created unified pretraining objective combining contrastive alignment, caption generation, and signal reconstruction; 3) Built SleepLM family of models bridging natural language and multimodal polysomnography.

Result: SleepLM outperforms state-of-the-art in zero-shot/few-shot learning, cross-modal retrieval, and sleep captioning. Exhibits novel capabilities: language-guided event localization, targeted insight generation, and zero-shot generalization to unseen tasks.

Conclusion: SleepLM successfully bridges natural language with sleep physiology, enabling more flexible and interpretable sleep analysis. The model demonstrates strong performance on real-world tasks and novel capabilities for sleep understanding. All code and data will be open-sourced.
...
### [210] [MMKG-RDS: Reasoning Data Synthesis via Deep Mining of Multimodal Knowledge Graphs](https://arxiv.org/abs/2602.23632)
*Lun Zhan,Feng Xiong,Huanyong Liu,Feng Zhang,Yuhui Yin*

Main category: cs.AI

TL;DR: MMKG-RDS is a flexible framework for synthesizing high-quality reasoning training data using multimodal knowledge graphs, addressing limitations in existing methods for long-tail knowledge coverage, effectiveness verification, and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing methods for synthesizing training data to enhance domain models' reasoning abilities have limitations in long-tail knowledge coverage, effectiveness verification, and interpretability. Knowledge-graph-based approaches also fall short in functionality, granularity, customizability, and evaluation.

Method: Proposes MMKG-RDS, a flexible framework for reasoning data synthesis leveraging multimodal knowledge graphs. It supports fine-grained knowledge extraction, customizable path sampling, and multidimensional data quality scoring.

Result: Validated with MMKG-RDS-Bench dataset covering 5 domains, 17 task types, and 14,950 samples. Fine-tuning Qwen3 models (0.6B/8B/32B) on synthesized samples improved reasoning accuracy by 9.2%. Framework generates challenging data for tasks involving tables and formulas, useful for complex benchmark construction.

Conclusion: MMKG-RDS effectively addresses limitations of existing methods and provides a practical framework for synthesizing high-quality reasoning training data using multimodal knowledge graphs, with demonstrated improvements in model performance and utility for benchmark construction.
...
### [211] [AI Must Embrace Specialization via Superhuman Adaptable Intelligence](https://arxiv.org/abs/2602.23643)
*Judah Goldfeder,Philippe Wyder,Yann LeCun,Ravid Shwartz Ziv*

Main category: cs.AI

TL;DR: The paper critiques the flawed concept of AGI (Artificial General Intelligence) and proposes SAI (Superhuman Adaptable Intelligence) as a more useful framework for AI development, emphasizing specialization over generality.


<details>
  <summary>Details</summary>
Motivation: Current discussions about AGI are confused by overloaded and inconsistent definitions. The human-centric "can do everything a human can do" definition is problematic because humans themselves are not truly general. The paper aims to clarify AI discourse by addressing what's wrong with AGI concepts and proposing a better framework.

Method: The authors analyze the coherence and usefulness of widely accepted AGI definitions, questioning whether they are plausible, useful, and truly general. They then introduce the concept of SAI (Superhuman Adaptable Intelligence) as an alternative framework that emphasizes specialization and superhuman performance.

Result: The paper concludes that AGI is a flawed concept for describing AI's future. Instead, they propose SAI as a more coherent framework that focuses on specialized intelligence that can exceed human capabilities in important domains and fill skill gaps where humans are incapable.

Conclusion: AI should embrace specialization rather than strive for generality, aiming for superhuman performance in specific domains. SAI provides a clearer framework for discussing AI's future and guiding development, replacing the overloaded and problematic AGI concept.
...
### [212] [PseudoAct: Leveraging Pseudocode Synthesis for Flexible Planning and Action Control in Large Language Model Agents](https://arxiv.org/abs/2602.23668)
*Yihan,Wen,Xin Chen*

Main category: cs.AI

TL;DR: PseudoAct introduces a pseudocode-based planning framework for LLM agents that replaces reactive decision-making with structured pseudocode synthesis for more efficient long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents use reactive decision-making (like ReAct) which leads to redundant tool usage, unstable reasoning, and high token consumption in complex long-horizon tasks involving branching, iteration, or multi-tool coordination.

Method: PseudoAct synthesizes structured pseudocode plans that decompose tasks into subtasks with explicit control flow (sequencing, conditionals, loops, parallel composition). Actions are then executed by following this global plan, making decision logic explicit and temporally coherent.

Result: Significantly outperforms existing reactive agent approaches with 20.93% absolute gain in success rate on FEVER and sets new state-of-the-art on HotpotQA benchmark datasets.

Conclusion: PseudoAct's pseudocode-based planning framework enables consistent and efficient long-horizon decision-making by reducing redundant actions, preventing infinite loops, and avoiding uninformative alternative exploration in LLM agents.
...
### [213] [ODAR: Principled Adaptive Routing for LLM Reasoning via Active Inference](https://arxiv.org/abs/2602.23681)
...
<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning approaches rely on uniform brute-force sampling (like best-of-N or self-consistency) which is computationally expensive, hard to attribute, and suffers from diminishing returns due to overthinking. There's a need for more efficient adaptive resource allocation.
...
Result: Extensive evaluation across 23 benchmarks shows strong gains: 98.2% accuracy on MATH and 54.8% on Humanity's Last Exam (HLE), while improving compute-accuracy frontier. On open-source stack (Llama 4 + DeepSeek), ODAR surpasses homogeneous sampling strategies while reducing computational costs by 82%.

Conclusion: Optimal scaling for LLM reasoning requires adaptive resource allocation with free-energy-based decision-making rather than simply increasing test-time compute. ODAR provides a principled framework for efficient and accurate reasoning.
...
### [214] [From Flat Logs to Causal Graphs: Hierarchical Failure Attribution for LLM-based Multi-Agent Systems](https://arxiv.org/abs/2602.23701)
*Yawen Wang,Wenjie Wu,Junjie Wang,Qing Wang*

Main category: cs.AI

TL;DR: CHIEF is a framework that transforms chaotic multi-agent system trajectories into hierarchical causal graphs for better failure attribution, outperforming existing methods on accuracy.


<details>
  <summary>Details</summary>
Motivation: LLM-powered Multi-Agent Systems are fragile with opaque failure mechanisms. Existing failure attribution methods treat execution logs as flat sequences, failing to capture complex causal relationships and leading to weak observability and ambiguous responsibility boundaries.

Method: CHIEF transforms chaotic trajectories into structured hierarchical causal graphs, uses hierarchical oracle-guided backtracking to prune search space via synthesized virtual oracles, and implements counterfactual attribution through progressive causal screening to distinguish root causes from propagated symptoms.
...
Conclusion: CHIEF provides a structured approach to failure attribution in multi-agent systems by capturing hierarchical causal relationships, enabling more accurate identification of root causes versus propagated symptoms.
...
### [215] [ProductResearch: Training E-Commerce Deep Research Agents via Multi-Agent Synthetic Trajectory Distillation](https://arxiv.org/abs/2602.23716)
*Jiangyuan Wang,Kejun Xiao,Huaipeng Zhao,Tao Luo,Xiaoyi Zeng*

Main category: cs.AI

TL;DR: ProductResearch is a multi-agent framework that generates synthetic training data for e-commerce shopping agents, enabling LLMs to perform complex product research through fine-tuning on filtered multi-agent trajectories.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based e-commerce agents lack depth for complex product research, while deep research methods from web search don't transfer well to e-commerce domains, creating a need for specialized training data and methods.

Method: Multi-agent framework with User Agent (infers shopping intents), Supervisor Agent (orchestrates collaboration), and Research Agent to generate synthetic tool-use trajectories. These are filtered and distilled into single-role training examples through reflective internalization.

Result: A compact MoE model fine-tuned on the synthetic data shows substantial improvements in response comprehensiveness, research depth, and user-perceived utility, approaching performance of proprietary deep research systems.

Conclusion: Multi-agent synthetic trajectory training is an effective and scalable paradigm for enhancing LLM-based shopping assistance, enabling robust e-commerce agents through specialized fine-tuning data generation.
...
### [216] [The Auton Agentic AI Framework](https://arxiv.org/abs/2602.23720)
*Sheng Cao,Zhao Chang,Chang Li,Hannan Li,Liyao Fu,Ji Tang*

Main category: cs.AI

TL;DR: The Auton Agentic AI Framework provides a standardized architecture for creating, executing, and governing autonomous AI agents, addressing the mismatch between stochastic LLM outputs and deterministic backend systems through a separation between declarative Cognitive Blueprints and runtime execution engines.


<details>
  <summary>Details</summary>
Motivation: The transition from Generative AI to Agentic AI exposes a fundamental architectural mismatch: LLMs produce stochastic, unstructured outputs, while backend infrastructure (databases, APIs, cloud services) requires deterministic, schema-conformant inputs. Current systems lack standardization for creating, executing, and governing autonomous agents.

Method: The framework introduces a strict separation between Cognitive Blueprint (declarative, language-agnostic specification of agent identity/capabilities) and Runtime Engine (platform-specific execution substrate). It formalizes agent execution as an augmented POMDP with latent reasoning, implements hierarchical memory consolidation inspired by biological systems, uses constraint manifolds for safety enforcement via policy projection, provides a three-level self-evolution framework, and includes runtime optimizations like parallel graph execution and speculative inference.

Result: The framework enables cross-language portability, formal auditability, modular tool integration via Model Context Protocol (MCP), and reduces end-to-end latency for multi-step agent workflows through various runtime optimizations.

Conclusion: The Auton Agentic AI Framework provides a principled architecture that addresses the fundamental mismatch between stochastic LLMs and deterministic backend systems, offering standardization, safety, and performance for autonomous agent development and deployment.
...
### [217] [Unlocking Cognitive Capabilities and Analyzing the Perception-Logic Trade-off](https://arxiv.org/abs/2602.23730)
...
TL;DR: MERaLiON2-Omni (Alpha) is a 10B-parameter multilingual MLLM for Southeast Asia that uses a progressive training pipeline to decouple and integrate perception (System 1) and reasoning (System 2) capabilities, revealing an efficiency-stability paradox where reasoning boosts abstract tasks but destabilizes sensory processing.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack robust integration of sensory grounding with complex reasoning, especially for underrepresented regions like Southeast Asia, where region-specific audio-visual cues and multilingual capabilities are needed.

Method: 1) Build Perception Backbone by aligning region-specific audio-visual cues with multilingual LLM via orthogonal modality adaptation. 2) Use Generate-Judge-Refine pipeline with Super-LLM to filter hallucinations and synthesize high-quality silver data for transferring textual Chain-of-Thought to multimodal scenarios.

Result: Evaluation on SEA-Omni Benchmark reveals Efficiency-Stability Paradox: reasoning amplifies abstract tasks (math, instruction-following) but introduces instability in sensory processing - Temporal Drift in audio (desynchronization) and Visual Over-interpretation (logic overrides pixel reality).

Conclusion: The paper presents a novel approach to integrating perception and reasoning in MLLMs for underrepresented regions, highlighting important trade-offs between robust sensory processing and structured reasoning capabilities that need to be addressed in future multimodal AI systems.
...
### [218] [Reasoning-Driven Multimodal LLM for Domain Generalization](https://arxiv.org/abs/2602.23777)
*Zhipeng Xu,Zilong Wang,Xinyang Jiang,Dongsheng Li,De Cheng,Nannan Wang*

Main category: cs.AI

TL;DR: RD-MLDG uses multimodal LLMs with reasoning chains for domain generalization, addressing optimization challenges through multi-task training and self-aligned regularization to achieve SOTA results.


<details>
  <summary>Details</summary>
Motivation: Most DG methods focus on visual feature invariance, but this paper explores leveraging MLLMs' reasoning capabilities to construct reasoning chains for more robust predictions under domain shift.

Method: Proposes RD-MLDG with two components: (1) MTCT (Multi-Task Cross-Training) adds direct classification pathway to guide reasoning supervision, and (2) SARR (Self-Aligned Reasoning Regularization) preserves semantic richness while mitigating reasoning-pattern mismatches via iterative self-labeling.
...
Conclusion: Reasoning chains from MLLMs provide valuable complementary signals for domain generalization, and the proposed RD-MLDG framework effectively addresses optimization challenges to achieve robust performance across domains.
...
### [219] [EMO-R3: Reflective Reinforcement Learning for Emotional Reasoning in Multimodal Large Language Models](https://arxiv.org/abs/2602.23802)
*Yiyang Fang,Wenke Huang,Pei Fu,Yihao Yang,Kehua Su,Zhenbo Luo,Jian Luan,Mang Ye*

Main category: cs.AI

TL;DR: EMO-R3: A reflective reinforcement learning framework that enhances multimodal LLMs' emotional reasoning through structured thinking and reflective rewards.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with complex human emotions - supervised fine-tuning has limited generalization and poor interpretability, while RL methods like GRPO fail to align with emotional cognition characteristics.

Method: Proposes EMO-R3 framework with two key components: 1) Structured Emotional Thinking for step-by-step interpretable reasoning, and 2) Reflective Emotional Reward that enables model to re-evaluate reasoning based on visual-text consistency and emotional coherence.
...
Conclusion: EMO-R3 successfully addresses limitations of existing approaches by combining structured reasoning with reflective reinforcement learning, enhancing MLLMs' ability to handle the complexity and subjectivity of human emotions.
...
### [220] [RUMAD: Reinforcement-Unifying Multi-Agent Debate](https://arxiv.org/abs/2602.23864)
*Chao Wang,Han Lin,Huaze Tang,Huijing Lin,Wenbo Ding*

Main category: cs.AI

TL;DR: RUMAD is a reinforcement learning framework that dynamically optimizes communication topology in multi-agent debate systems, achieving 80%+ token cost reduction while improving reasoning accuracy across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent debate systems struggle to balance accuracy, consensus formation, and computational efficiency. Static topology methods lack adaptability to task complexity, while external LLM-based coordination risks introducing privileged knowledge that compromises debate neutrality.

Method: RUMAD formulates dynamic communication topology control as a reinforcement learning problem. It uses a content-agnostic observation scheme capturing high-level debate dynamics, a multi-objective reward for solution quality, cohesion and efficiency, and a PPO-trained controller that dynamically adjusts edge weights in the communication graph with dual-threshold mechanism for agent activation and information visibility control.

Result: RUMAD achieves over 80% reduction in token costs while improving reasoning accuracy compared to single LLM and multiple MAD baselines across MMLU, GSM8K, and GPQA benchmarks. It shows robust zero-shot generalization to out-of-domain tasks when trained only on MMLU.
...
### [221] [RF-Agent: Automated Reward Function Design via Language Agent Tree Search](https://arxiv.org/abs/2602.23876)
...
<details>
  <summary>Details</summary>
Motivation: Designing efficient reward functions for low-level control is challenging. Current LLM-based methods have poor historical feedback utilization and inefficient search, limiting improvements in complex control tasks.

Method: Treats LLMs as language agents, frames reward function design as sequential decision-making, and integrates Monte Carlo Tree Search (MCTS) to manage reward design and optimization with multi-stage contextual reasoning.

Result: Outstanding experimental results in 17 diverse low-level control tasks demonstrate effectiveness. Source code is publicly available.

Conclusion: RF-Agent enhances optimization through better contextual reasoning, improves historical information utilization and search efficiency for identifying promising reward functions in complex control tasks.
...
### [222] [Pessimistic Auxiliary Policy for Offline Reinforcement Learning](https://arxiv.org/abs/2602.23974)
*Fan Zhang,Baoru Huang,Xin Zhang*

Main category: cs.AI

TL;DR: PAPER proposes a pessimistic auxiliary policy for offline RL to sample reliable actions and reduce approximation errors from out-of-distribution actions.


<details>
  <summary>Details</summary>
Motivation: Offline RL learns from pre-collected datasets but suffers from approximation errors when accessing out-of-distribution actions, leading to error accumulation and overestimation.

Method: Constructs a pessimistic auxiliary policy by maximizing the lower confidence bound of the Q-function, which samples actions with high value and low uncertainty near the learned policy.

Result: Extensive experiments on offline RL benchmarks show the pessimistic auxiliary strategy effectively improves performance of other offline RL approaches.

Conclusion: The proposed pessimistic auxiliary policy reduces approximation errors and error accumulation in offline RL, enhancing overall efficacy.
...
### [223] [Portfolio Reinforcement Learning with Scenario-Context Rollout](https://arxiv.org/abs/2602.24037)
*Vanya Priscillia Bendatu,Yao Lu*

Main category: cs.AI

TL;DR: Proposes macro-conditioned scenario-context rollout (SCR) for portfolio rebalancing under market stress, addresses reward-transition mismatch in RL, and improves Sharpe ratio by up to 76% with 53% lower drawdown.


<details>
  <summary>Details</summary>
Motivation: Market regime shifts cause distribution shifts that degrade portfolio rebalancing performance. Historical data doesn't show what would have happened differently under stress events, creating challenges for generating plausible scenarios.

Method: Macro-conditioned scenario-context rollout (SCR) generates plausible next-day multivariate return scenarios under stress. Addresses reward-transition mismatch by constructing counterfactual next states using rollout-implied continuations and augmenting critic agent's bootstrap target.

Result: Out-of-sample evaluations across 31 distinct universes of U.S. equity and ETF portfolios show Sharpe ratio improvements up to 76% and maximum drawdown reductions up to 53% compared to classic and RL-based baselines.

Conclusion: The proposed method stabilizes RL learning by addressing reward-transition inconsistency, provides viable bias-variance tradeoff, and significantly improves portfolio performance under market stress conditions.
...
### [224] [CIRCLE: A Framework for Evaluating AI from a Real-World Lens](https://arxiv.org/abs/2602.24055)
...
<details>
  <summary>Details</summary>
Motivation: Existing frameworks like MLOps focus on system stability, while benchmarks measure abstract capabilities, leaving decision-makers without systematic evidence about AI behavior under real-world user variability and constraints.

Method: CIRCLE operationalizes the Validation phase of TEVV by formalizing stakeholder concerns into measurable signals, integrating field testing, red teaming, and longitudinal studies into a coordinated six-stage pipeline.

Result: CIRCLE produces systematic knowledge that is comparable across sites yet sensitive to local context, enabling evidence-based governance of AI's materialized downstream effects.

Conclusion: CIRCLE provides a structured, prospective protocol for linking context-sensitive qualitative insights to scalable quantitative metrics, enabling governance based on actual outcomes rather than theoretical capabilities.
...
### [225] [Human or Machine? A Preliminary Turing Test for Speech-to-Speech Interaction](https://arxiv.org/abs/2602.24080)
...
<details>
  <summary>Details</summary>
Motivation: To determine whether modern speech-to-speech (S2S) conversational AI systems can truly converse like humans, moving beyond the traditional Turing test framework to provide detailed diagnostic insights.

Method: Conducted first Turing test for S2S systems with 2,968 human judgments on dialogues between 9 state-of-the-art S2S systems and 28 human participants. Developed fine-grained taxonomy of 18 human-likeness dimensions and crowd-annotated dialogues accordingly.
...
Conclusion: Establishes first human-likeness evaluation for S2S systems, proposes interpretable model for accurate human-vs-machine discrimination using fine-grained ratings, enabling detailed diagnostic insights for improving conversational AI systems.
...
### [226] [Bi-level RL-Heuristic Optimization for Real-world Winter Road Maintenance](https://arxiv.org/abs/2602.24097)
*Yue Xie,Zizhen Xu,William Beazley,Fumiya Iida*

Main category: cs.AI

TL;DR: A bi-level optimization framework using RL for network partitioning and multi-objective VRP for winter road maintenance, validated on UK road networks with improved efficiency and reduced emissions.


<details>
  <summary>Details</summary>
Motivation: Existing winter road maintenance methods struggle with large-scale routing problems and rely heavily on human decision-making, lacking efficiency and scalability for real-world networks.

Method: A bi-level optimization framework: upper level uses RL to partition road networks into clusters and allocate resources from multiple depots; lower level solves multi-objective VRP within each cluster minimizing maximum travel time and carbon emissions.
...
Conclusion: Advanced AI-driven bi-level optimization can directly enhance operational decision-making in real-world transportation and logistics for winter road maintenance.
...
### [227] [Artificial Agency Program: Curiosity, compression, and communication in agents](https://arxiv.org/abs/2602.24100)
*Richard Csaky*

Main category: cs.AI

TL;DR: AAP proposes building AI as resource-bounded agents driven by curiosity-as-learning-progress, treating AI as part of extended human-tool systems to enhance capabilities while reducing interface friction.


<details>
  <summary>Details</summary>
Motivation: To develop AI systems that are reality-embedded and resource-bounded, functioning as part of extended human-tool systems to increase sensing, understanding, and actuation capabilities while minimizing interface friction between people, tools, and environments.

Method: Unifies predictive compression, intrinsic motivation, empowerment/control, interface quality, and language/self-communication as selective information bottlenecks. Formulates as falsifiable program with explicit costs, staged experiments, and multimodal tokenized testbed where agents allocate limited budget among observation, action, and deliberation.

Result: Proposes a conceptual and experimental framework connecting intrinsic motivation, information theory, thermodynamics, bounded rationality, and modern reasoning systems through the AAP research agenda.

Conclusion: AAP provides a comprehensive position and research agenda for developing AI as resource-bounded agents driven by curiosity-as-learning-progress, offering a unified framework that bridges multiple theoretical domains with practical experimental testbeds.
...
### [228] [Recycling Failures: Salvaging Exploration in RLVR via Fine-Grained Off-Policy Guidance](https://arxiv.org/abs/2602.24110)
...
Method: SCOPE uses Process Reward Models to identify the first erroneous step in suboptimal rollouts and applies fine-grained, step-wise off-policy rectification to salvage partially correct trajectories, maintaining exploration diversity.

Result: SCOPE increases diversity score by 13.5%, achieves average accuracy of 46.6% on math reasoning, and shows robust generalization with 53.4% accuracy on out-of-distribution reasoning tasks, establishing new SOTA results.

Conclusion: Step-wise correction using Process Reward Models effectively salvages partially correct trajectories, sustains broad exploration space, and significantly improves reasoning performance in RLVR frameworks.
...
### [229] [LemmaBench: A Live, Research-Level Benchmark to Evaluate LLM Capabilities in Mathematics](https://arxiv.org/abs/2602.24173)
*Antoine Peyronnet,Fabian Gloeckle,Amaury Hayat*

Main category: cs.AI

TL;DR: A new benchmark for evaluating LLMs on research-level mathematics using automatically extracted lemmas from arXiv, updated regularly to avoid contamination from training data.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks use static, hand-curated contest/textbook problems as proxies for mathematical research, which don't reflect actual research-level mathematics and can become contaminated when used for training.

Method: Automatic pipeline that extracts lemmas from arXiv papers and rewrites them into self-contained statements by making all assumptions and definitions explicit, creating an updatable benchmark.

Result: Current state-of-the-art LLMs achieve only 10-15% accuracy (pass@1) in theorem proving, showing large gap between current LLM capabilities and human-level research mathematics.

Conclusion: There's significant room for improvement before LLMs reach human-level proving capabilities in research mathematics, and the proposed benchmark provides a sustainable evaluation framework that avoids data contamination issues.
...
### [230] [Learning Flexible Job Shop Scheduling under Limited Buffers and Material Kitting Constraints](https://arxiv.org/abs/2602.24180)
*Shishun Zhang,Juzhan Xu,Yidan Fan,Chenyang Zhu,Ruizhen Hu,Yongjun Wang,Kai Xu*

Main category: cs.AI

TL;DR: Proposes a deep reinforcement learning approach with heterogeneous graph networks for Flexible Job Shop Scheduling with Limited Buffers and Material Kitting, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Current FJSP studies often ignore practical constraints like limited buffers, which significantly impact production efficiency. The paper addresses the gap between idealized academic models and real-world production scenarios.

Method: Uses deep reinforcement learning with heterogeneous graph networks to model global state. Constructs efficient message passing among machines, operations, and buffers to avoid decisions causing frequent pallet changes during long-sequence scheduling.

Result: Outperforms traditional heuristics and advanced DRL methods on both synthetic and real production line datasets in terms of makespan and pallet changes. Achieves good balance between solution quality and computational cost.

Conclusion: The proposed heterogeneous graph network within DRL framework effectively handles complex dependencies and long-term constraints in practical FJSP with limited buffers, improving buffer utilization and decision quality for real production scenarios.
...
### [231] [Uncertainty Quantification for Multimodal Large Language Models with Incoherence-adjusted Semantic Volume](https://arxiv.org/abs/2602.24195)
*Gregory Kang Ruey Lau,Hieu Dao,Nicole Kan Hui Lin,Bryan Kian Hsiang Low*

Main category: cs.AI

TL;DR: UMPIRE is a training-free uncertainty quantification framework for Multimodal Large Language Models that measures response uncertainty by analyzing semantic diversity and internal confidence across various modalities without external tools.


<details>
  <summary>Details</summary>
Motivation: MLLMs often produce plausible but erroneous outputs, making reliable deployment challenging. Existing uncertainty metrics have limitations: they're modality-specific, require external tools, or are computationally expensive.
...
Conclusion: UMPIRE provides an effective, training-free uncertainty quantification framework for MLLMs that works across various modalities without external tools, enabling more reliable deployment through better uncertainty estimation.
...
### [232] [A Minimal Agent for Automated Theorem Proving](https://arxiv.org/abs/2602.24273)
*Borja Requena Pozo,Austin Letson,Krystian Nowakowski,Izan Beltran Ferreiro,Leopoldo Sarra*

Main category: cs.AI

TL;DR: A minimal agentic baseline for AI theorem proving that enables systematic comparison of architectures, featuring iterative proof refinement, library search, and context management, with competitive performance using simpler design.


<details>
  <summary>Details</summary>
Motivation: To create a standardized baseline for comparing different AI-based theorem prover architectures, addressing the need for systematic evaluation across diverse systems and design choices in the field.
...
Conclusion: The minimal baseline provides a valuable reference for future research and an accessible prover for the community, with open-source implementation released to facilitate systematic comparison and advancement in AI theorem proving.
...
### [233] [DARE-bench: Evaluating Modeling and Instruction Fidelity of LLMs in Data Science](https://arxiv.org/abs/2602.24288)
*Fan Shu,Yite Wang,Ruofan Wu,Boyi Liu,Zhewei Yao,Yuxiong He,Feng Yan*

Main category: cs.AI

TL;DR: DARE-bench is a new benchmark for evaluating LLMs on data science tasks with verifiable ground truth, addressing gaps in existing benchmarks through standardized evaluation and providing large-scale training data.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack standardized process-aware evaluation and have scarce accurately labeled training data for complex multi-step data science tasks using LLMs.

Method: Created DARE-bench with 6,300 Kaggle-derived tasks featuring verifiable ground truth, covering broad data science tasks and supporting agentic tools, providing both training and evaluation datasets.

Result: Even top models like GPT-4 struggle, especially on ML modeling tasks. Fine-tuning with DARE-bench data significantly improves performance: Qwen3-32B accuracy increased 1.83x with supervised fine-tuning, and Qwen3-4B accuracy improved 8x+ with reinforcement learning.

Conclusion: DARE-bench serves as both an accurate evaluation benchmark and critical training data source for improving LLM performance on data science tasks, addressing key gaps in existing benchmarking approaches.
...
### [234] [SALIENT: Frequency-Aware Paired Diffusion for Controllable Long-Tail CT Detection](https://arxiv.org/abs/2602.23447)
*Yifan Li,Mehrdad Salimitari,Taiyu Zhang,Guang Li,David Dreizin*

Main category: eess.IV

TL;DR: SALIENT: A mask-conditioned wavelet-domain diffusion framework for synthesizing paired lesion-masking volumes to address extreme class imbalance in whole-body CT detection through controllable augmentation.


<details>
  <summary>Details</summary>
Motivation: Rare lesion detection in whole-body CT suffers from extreme class imbalance and low target-to-volume ratios, causing precision collapse despite high AUROC. Existing diffusion approaches are computationally expensive and lack controllable attribute-level regulation and paired supervision.

Method: SALIENT performs structured diffusion over discrete wavelet coefficients instead of pixel space, separating low-frequency brightness from high-frequency structural detail. Uses learnable frequency-aware objectives to disentangle target/background attributes, 3D VAE for volumetric lesion masks, and semi-supervised teacher for paired pseudo-labels.

Result: Improves generative realism with higher MS-SSIM (0.63 to 0.83) and lower FID (118.4 to 46.5). Augmented training improves long-tail detection performance with disproportionate AUPRC gains across low prevalences. Optimal synthetic ratios shift from 2x to 4x as labeled seed size decreases.

Conclusion: Frequency-aware diffusion enables controllable, computationally efficient precision rescue in long-tail CT detection, demonstrating a seed-dependent augmentation regime under low-label conditions.
...
### [235] [SGDC: Structurally-Guided Dynamic Convolution for Medical Image Segmentation](https://arxiv.org/abs/2602.23496)
*Bo Shi,Wei-ping Zhu,M. N. S. Swamy*

Main category: eess.IV

TL;DR: SGDC replaces average pooling in dynamic convolution with structure-guided feature modulation to preserve fine-grained clinical structures in medical segmentation.


<details>
  <summary>Details</summary>
Motivation: Current dynamic convolution methods in medical segmentation use average pooling which collapses high-frequency spatial details, leading to over-smoothed predictions that degrade fine-grained clinical structure fidelity.

Method: Proposes Structure-Guided Dynamic Convolution (SGDC) with an explicitly supervised structure-extraction branch that guides dynamic kernel generation and gating signals for structure-aware feature modulation, fusing high-fidelity boundary information with semantic features.

Result: Achieves state-of-the-art performance on ISIC 2016, PH2, ISIC 2018, and CoNIC datasets, reducing Hausdorff Distance (HD95) by 2.05 and providing consistent IoU gains of 0.99%-1.49% over pooling-based baselines.

Conclusion: SGDC effectively prevents information loss from average pooling, delivers superior boundary fidelity, and shows strong potential for extension to other fine-grained, structure-sensitive vision tasks like small-object detection.
...
### [236] [SegReg: Latent Space Regularization for Improved Medical Image Segmentation](https://arxiv.org/abs/2602.23509)
*Puru Vaish,Amin Ranem,Felix Meister,Tobias Heimann,Christoph Brune,Jelmer M. Wolterink*

Main category: eess.IV

TL;DR: SegReg is a latent-space regularization framework for medical image segmentation that improves domain generalization and continual learning by structuring feature embeddings in U-Net models.


<details>
  <summary>Details</summary>
Motivation: Current medical image segmentation models use voxel-wise losses that only constrain predictions in output space, leaving latent feature representations largely unconstrained. This limitation potentially restricts model generalization across different domains.

Method: SegReg is a latent-space regularization framework that operates on feature maps of U-Net models to encourage structured embeddings. It remains fully compatible with standard segmentation losses and is integrated with the nnU-Net framework.

Result: Evaluation on prostate, cardiac, and hippocampus segmentation shows consistent improvements in domain generalization. Additionally, explicit latent regularization improves continual learning by reducing task drift and enhancing forward transfer across sequential tasks without adding memory or extra parameters.
...
### [237] [Few-Shot Continual Learning for 3D Brain MRI with Frozen Foundation Models](https://arxiv.org/abs/2602.23533)
*Chi-Sheng Chen,Xinyu Zhang,Guan-Ying Chen,Qiuzhe Xie,Fan Zhang,En-Jui Kuo*

Main category: eess.IV

TL;DR: Frozen foundation models with task-specific LoRA adapters enable few-shot continual learning for 3D brain MRI tasks without catastrophic forgetting, using <0.1% trainable parameters per task.


<details>
  <summary>Details</summary>
Motivation: Foundation models pretrained on large-scale 3D medical imaging data face challenges when adapted to multiple downstream tasks under continual learning with limited labeled data, particularly avoiding catastrophic forgetting while maintaining performance across tasks.

Method: Combine frozen pretrained backbone with task-specific Low-Rank Adaptation (LoRA) modules. Each task (tumor segmentation and brain age estimation) receives dedicated LoRA adapter; only adapter and task-specific head are trained while backbone remains frozen, eliminating catastrophic forgetting by design.

Result: LoRA achieves best balanced performance: T1 Dice 0.62±0.07, T2 MAE 0.16±0.05 with zero forgetting (BWT=0) and <0.1% trainable parameters per task. Sequential full fine-tuning suffers severe forgetting (T1 Dice drops from 0.80 to 0.16), while sequential linear probing fails on T2 (MAE 1.45).

Conclusion: Frozen foundation models with task-specific LoRA adapters offer practical solution for few-shot continual learning when both tasks must be maintained, though systematic age underestimation in T2 remains a limitation.
...
### [238] [Hierarchical Multi-Scale Graph Learning with Knowledge-Guided Attention for Whole-Slide Image Survival Analysis](https://arxiv.org/abs/2602.23557)
*Bin Xu,Yufei Zhou,Boling Song,Jingwen Sun,Yang Bian,Cheng Lu,Ye Wu,Jianfei Tu,Xiangxue Wang*

Main category: eess.IV

TL;DR: HMKGN is a hierarchical multi-scale graph network for cancer prognosis from whole-slide images that models spatial hierarchies and multi-scale interactions, outperforming existing MIL methods.


<details>
  <summary>Details</summary>
Motivation: Current attention-based MIL ignores spatial organization, while graph-based MIL relies on static handcrafted graphs. There's a need to capture hierarchical spatial relationships and multi-scale interactions within WSIs for better cancer prognostication.

Method: HMKGN enforces hierarchical structure with spatial locality constraints: local cellular-level dynamic graphs aggregate spatially proximate patches within each ROI, and global slide-level dynamic graph integrates ROI-level features. Multi-scale integration combines coarse contextual features from broader views with fine-grained structural representations from local patch-graph aggregation.

Result: Evaluated on four TCGA cohorts (KIRC, LGG, PAAD, STAD; N=513, 487, 138, 370). Consistently outperformed existing MIL-based models with 10.85% better concordance indices and statistically significant stratification of patient survival risk (log-rank p < 0.05).

Conclusion: HMKGN effectively models hierarchical spatial relationships and multi-scale interactions in WSIs, providing superior cancer prognosis performance compared to existing MIL approaches.
...
### [239] [Unsupervised Causal Prototypical Networks for De-biased Interpretable Dermoscopy Diagnosis](https://arxiv.org/abs/2602.23752)
*Junhao Jia,Yueyi Wu,Huangwei Chen,Haodong Jing,Haishuai Wang,Jiajun Bu,Lei Wu*

Main category: eess.IV

TL;DR: CausalProto is an unsupervised causal prototypical network that purifies visual evidence in dermoscopy analysis by disentangling pathological features from environmental confounders, improving both diagnostic accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Deep learning's black-box nature hinders clinical trust in dermoscopy analysis, and prototypical networks can suffer from shortcut learning where environmental confounders are encoded as predictive prototypes, generating misleading visual evidence.

Method: Uses Structural Causal Model framework with Information Bottleneck-constrained encoder for unsupervised orthogonal disentanglement of pathological features from environmental confounders. Maps decoupled representations to independent prototypical spaces and uses learned spurious dictionary for backdoor adjustment via do-calculus, transforming causal interventions into efficient expectation pooling.

Result: Extensive experiments on multiple dermoscopy datasets show CausalProto achieves superior diagnostic performance, consistently outperforms standard black box models, and provides transparent, high-purity visual interpretability without accuracy compromise.

Conclusion: CausalProto effectively mitigates confounding effects in dermoscopy analysis by purifying the visual evidence chain through causal disentanglement, offering both improved accuracy and trustworthy interpretability for clinical decision-making.
...
### [240] [VideoPulse: Neonatal heart rate and peripheral capillary oxygen saturation (SpO2) estimation from contact free video](https://arxiv.org/abs/2602.23771)
...
TL;DR: VideoPulse: A neonatal dataset and pipeline for contact-free heart rate and SpO2 estimation from facial video using 3D CNNs with artifact-aware supervision.


<details>
  <summary>Details</summary>
Motivation: Conventional vital sign monitoring for neonates requires adhesive probes that can irritate fragile skin and increase infection risk. Remote photoplethysmography (rPPG) offers contact-free monitoring but needs specialized neonatal datasets and methods.

Method: End-to-end pipeline with face alignment, artifact-aware supervision using denoised pulse oximeter signals, 3D CNN backbones for heart rate and SpO2 regression, label distribution smoothing, and weighted regression for SpO2. Predictions in 2-second windows.

Result: Heart rate MAE: 2.97 bpm (2-second windows), 2.80 bpm (6-second windows) on NBHR dataset. SpO2 MAE: 1.69%. Cross-dataset evaluation shows 5.34 bpm MAE for heart rate on VideoPulse, and fine-tuned SpO2 model achieves 1.68% MAE.

Conclusion: Short unaligned neonatal video segments can support accurate heart rate and SpO2 estimation, enabling low-cost, non-invasive monitoring in neonatal intensive care units.
...
### [241] [Breaking the Data Barrier: Robust Few-Shot 3D Vessel Segmentation using Foundation Models](https://arxiv.org/abs/2602.23782)
*Kirato Yoshihara,Yohei Sugawara,Yuta Tokuoka,Lihang Hong*

Main category: eess.IV

TL;DR: A novel framework using pre-trained Vision Foundation Model (DINOv3) adapted for volumetric vessel segmentation with 3D adapter, multi-scale aggregator, and Z-channel embedding, achieving significant improvements in few-shot and out-of-distribution settings.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art vessel segmentation methods require large annotated datasets and suffer from performance degradation under domain shifts, which is impractical in clinical settings where acquiring extensive annotations for every new scanner/protocol is unfeasible.

Method: Proposed framework leverages pre-trained Vision Foundation Model (DINOv3) adapted for volumetric vessel segmentation with: 1) lightweight 3D Adapter for volumetric consistency, 2) multi-scale 3D Aggregator for hierarchical feature fusion, and 3) Z-channel embedding to bridge the gap between 2D pre-training and 3D medical modalities.

Result: In extreme few-shot regime (5 training samples): achieved Dice score of 43.42% (30% relative improvement over nnU-Net's 33.41%, outperforming Transformer baselines by up to 45%). In out-of-distribution setting: achieved 21.37% vs nnU-Net's 14.22% (50% relative improvement). Ablation studies confirmed 3D adaptation and multi-scale aggregation are critical.

Conclusion: Foundation models offer a viable cold-start solution for vessel segmentation, improving clinical reliability under data scarcity or domain shifts, with the proposed 3D adaptation mechanisms being critical for capturing continuous vascular structures from limited data.
...
### [242] [FluoCLIP: Stain-Aware Focus Quality Assessment in Fluorescence Microscopy](https://arxiv.org/abs/2602.23791)
*Hyejin Park,Jiwon Yoon,Sumin Park,Suree Kim,Sinae Jang,Eunsoo Lee,Dongmin Kang,Dongbo Min*

Main category: eess.IV

TL;DR: This paper introduces stain-aware focus quality assessment (FQA) for fluorescence microscopy, proposing FluoMix dataset and FluoCLIP framework to address stain-dependent focus variations.


<details>
  <summary>Details</summary>
Motivation: Current FQA methods treat focus quality as stain-agnostic, but fluorescent dyes cause abrupt and heterogeneous focus shifts that vary by stain type, requiring stain-aware modeling.

Method: Proposes FluoMix dataset (multiple tissues, stains, focus variations) and FluoCLIP framework - a two-stage vision-language model using CLIP alignment: stain-grounding phase learns general stain representations, stain-guided ranking phase optimizes stain-specific rank prompts for ordinal focus prediction.

Result: Demonstrates that focus-rank relationships vary substantially across stains; FluoCLIP achieves strong generalization across diverse fluorescence microscopy conditions, establishing first foundation for stain-aware FQA.

Conclusion: The paper formulates stain-aware FQA as essential for fluorescence microscopy, provides first dataset and framework for this task, showing that modeling focus behavior as function of staining characteristics significantly improves performance and generalization.
...
### [243] [BiM-GeoAttn-Net: Linear-Time Depth Modeling with Geometry-Aware Attention for 3D Aortic Dissection CTA Segmentation](https://arxiv.org/abs/2602.23803)
*Yuan Zhang,Lei Liu,Jialin Zhang,Ya-Nan Zhang,Ling Wang,Nan Mu*

Main category: eess.IV

TL;DR: BiM-GeoAttn-Net: A lightweight framework for 3D aortic dissection segmentation using bidirectional depth mamba for cross-slice dependencies and geometry-aware attention for tubular structure refinement.


<details>
  <summary>Details</summary>
Motivation: Accurate 3D segmentation of aortic dissection lumens in CT angiography is crucial for clinical assessment but challenging due to limited long-range context modeling (poor inter-slice coherence) and insufficient structural discrimination under low-contrast conditions.

Method: Proposes BiM-GeoAttn-Net integrating Bidirectional Depth Mamba (BiM) for efficient cross-slice dependency capture and Geometry-Aware Vessel Attention (GeoAttn) module using orientation-sensitive anisotropic filtering to refine tubular structures and sharpen ambiguous boundaries.
...
### [244] [Revisiting Integration of Image and Metadata for DICOM Series Classification: Cross-Attention and Dictionary Learning](https://arxiv.org/abs/2602.23833)
*Tuan Truong,Melanie Dohmen,Sara Lorio,Matthias Lenga*

Main category: eess.IV

TL;DR: End-to-end multimodal framework for DICOM series classification that jointly models image content and metadata while handling heterogeneous slice content, variable series length, and incomplete metadata without imputation.


<details>
  <summary>Details</summary>
Motivation: Automated DICOM series identification is crucial for large-scale medical image analysis, quality control, and reliable downstream processing, but remains challenging due to heterogeneous slice content, variable series length, and inconsistent/incomplete DICOM metadata.

Method: Multimodal framework with: (1) modality-aware image and metadata encoding with bi-directional cross-modal attention, (2) sparse, missingness-aware metadata encoder using learnable feature dictionaries and value-conditioned modulation (no imputation required), (3) 2.5D visual encoder with attention on equidistantly sampled slices to handle variable series length and image dimensions.

Result: Outperforms image-only, metadata-only, and multimodal 2D/3D baselines on Duke Liver MRI dataset and large multi-institutional in-house cohort, demonstrating consistent performance across in-domain and out-of-domain generalization settings.

Conclusion: Explicitly modeling metadata sparsity and cross-modal interactions improves robustness for DICOM series classification, enabling reliable automated identification essential for large-scale medical image analysis.
...
### [245] [Polarization Uncertainty-Guided Diffusion Model for Color Polarization Image Demosaicking](https://arxiv.org/abs/2602.23847)
*Chenggong Li,Yidong Luo,Junchao Zhang,Degui Yang*

Main category: eess.IV

TL;DR: Proposes using diffusion model priors to improve color polarization demosaicking, addressing limitations of network-based methods in reconstructing polarization characteristics by modeling uncertainty to guide diffusion recovery.


<details>
  <summary>Details</summary>
Motivation: Existing network-based CPDM methods effectively recover scene intensity but have significant errors in reconstructing polarization characteristics (DOP and AOP) due to challenges in predicting numerous missing pixels and scarcity of high-quality training data.

Method: Introduces image diffusion priors from text-to-image models to overcome network limitations, explicitly models polarization uncertainty during reconstruction, and uses uncertainty to guide the diffusion model in recovering high error regions.
...
Conclusion: The proposed approach effectively addresses the performance bottleneck of network-based CPDM methods by leveraging diffusion priors and uncertainty modeling to achieve superior polarization characteristic reconstruction.
...
### [246] [Clinically-aligned ischemic stroke segmentation and ASPECTS scoring on NCCT imaging using a slice-gated loss on foundation representations](https://arxiv.org/abs/2602.23961)
*Hiba Azeem,Behraj Khan,Tahir Qasim Syed*

Main category: eess.IV

TL;DR: A deep learning framework for stroke segmentation on NCCT that incorporates anatomical priors from ASPECTS scoring, using a frozen DINOv3 backbone with a lightweight decoder and Territory-Aware Gated Loss to enforce consistency between basal ganglia and supraganglionic levels.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for infarct assessment on non-contrast CT perform pixel-wise segmentation without modeling the structured anatomical reasoning underlying ASPECTS scoring, where basal ganglia and supraganglionic levels are clinically interpreted in a coupled manner.

Method: Proposes a clinically aligned framework combining a frozen DINOv3 backbone with a lightweight decoder and introduces a Territory-Aware Gated Loss (TAGL) to enforce BG-SG consistency during training, adding no inference-time complexity.

Result: Achieves Dice score of 0.6385 on AISD dataset, outperforming prior CNN and foundation-model baselines. On a proprietary ASPECTS dataset, TAGL improves mean Dice from 0.698 to 0.767.

Conclusion: Integrating foundation representations with structured clinical priors improves NCCT stroke segmentation and ASPECTS delineation.
...
### [247] [Extending 2D foundational DINOv3 representations to 3D segmentation of neonatal brain MR images](https://arxiv.org/abs/2602.23962)
*Annayah Usman,Behraj Khan,Tahir Qasim Syed*

Main category: eess.IV

TL;DR: A volumetric segmentation method that uses frozen 2D foundation model features with a window-based disassembly-reassembly mechanism for 3D hippocampal segmentation in infant brains.


<details>
  <summary>Details</summary>
Motivation: Precise hippocampal volumetric delineation is crucial for quantifying neurodevelopmental trajectories in infants, but existing 2D foundation models are limited for 3D brain anatomy. There's a need to bridge the gap between 2D foundation representations and 3D medical imaging.

Method: Proposes a structured window-based disassembly-reassembly mechanism: decomposes 3D MRI volumes into non-overlapping sub-cubes (windows), processes each with separate decoding arms using frozen 2D foundation features, then reassembles them with a dense-prediction head while maintaining anatomical consistency.

Result: Achieves Dice score of 0.65 for single 3D window segmentation on ALBERT dataset for hippocampal segmentation, demonstrating volumetric structure can be recovered from frozen 2D foundation representations.

Conclusion: The method successfully bridges 2D foundation models to 3D medical applications through structured compositional decoding, offering a principled and generalizable extension for volumetric segmentation tasks.
...
### [248] [Zero-Incoherence Capacity of Interactive Encoding Systems: Achievability, Converse, and Side Information Bounds](https://arxiv.org/abs/2602.23520)
*Tristan Simas*

Main category: cs.IT

TL;DR: The paper introduces zero-incoherence capacity for interactive multi-location encoding systems, proving exact capacity C₀=1, tight side-information bounds, and rate-complexity separation.


<details>
  <summary>Details</summary>
Motivation: To establish fundamental limits on encoding systems that guarantee zero probability of disagreement among replicated encodings, addressing the problem of incoherence in distributed encoding systems.

Method: Frames encoding locations as terminals in multi-terminal source coding; uses information-theoretic analysis with achievability/converse proofs, formalizes confusability/incoherence graph connection, and provides constructive instantiations with programming patterns and software case study.

Result: Exact capacity theorem (C₀=1), tight side-information lower bound (≥log₂k bits for k-way incoherence), rate-complexity separation (O(1) modification cost at capacity vs Ω(n) above), with machine-checked proofs in Lean 4.

Conclusion: Only complete derivation (one independent source) achieves zero incoherence; theoretical results are complemented by practical instantiations, providing both fundamental limits and constructive implementations for zero-incoherence encoding systems.
...
### [249] [Are Stacked Intelligent Metasurfaces (SIMs) Better than Single-layer Reconfigurable Intelligent Surfaces (RISs) for Wideband Multi-user MIMO Communication Systems?](https://arxiv.org/abs/2602.23534)
*Muhammad Ibrahim,Amine Mezghani,Ekram Hossain*

Main category: cs.IT

TL;DR: SIMs outperform single-layer RISs in wideband multi-user MIMO by mitigating coupling effects and maintaining consistent phase response across frequencies, with partially reconfigurable architectures achieving near-optimal performance with reduced complexity.


<details>
  <summary>Details</summary>
Motivation: Single-layer RISs have physical limitations in wideband wireless communication. SIMs can overcome these limitations by providing additional degrees of freedom for beamforming and better electromagnetic wave manipulation in complex environments.

Method: Proposes a coupling-aware, wideband, circuit-based framework capturing frequency-dependent mutual coupling and wideband channel responses. Formulates joint active and passive beamforming design optimizing base-station precoder for carrier aggregation across frequency-selective subbands together with metasurface phase shifts.

Result: SIMs significantly outperform single-layer RISs under wideband multi-user conditions by mitigating coupling-induced distortion and maintaining consistent phase response across frequencies. Single-layer RISs remain favorable in narrowband/low-SNR regimes. Partially reconfigurable SIM architectures achieve near-optimal performance with reduced complexity.

Conclusion: SIMs offer a scalable solution for future-generation wideband multi-user MIMO systems, providing design trade-offs between structural simplicity and wideband adaptability. The framework reveals the importance of accounting for coupling and wideband effects in performance optimization.
...
### [250] [The Distance Spectrum of IEEE 802.11 Binary Convolutional Codes](https://arxiv.org/abs/2602.23651)
*Rethna Pulikkoonattu*

Main category: cs.IT

TL;DR: This paper analyzes the binary convolutional coding (BCC) used in IEEE 802.11 standards, explaining how to compute the exact distance spectrum for the rate 1/2 mother code and its punctured derivatives, with performance analysis for various modulation schemes.


<details>
  <summary>Details</summary>
Motivation: BCC remains essential in IEEE 802.11 standards despite LDPC codes dominating high-throughput applications. It's mandatory for backward compatibility, used in bandwidth-constrained devices, IoT nodes, and packet preambles. The new Enhanced Long Range format in 802.11bn/UHR mandates rate-1/2 BCC, reinforcing its continued importance.

Method: The paper explains how to compute the exact distance spectrum for the IEEE 802.11 mother code (rate 1/2, K=7, generators octal 133/171) and its three standard punctured derivatives (rates 2/3, 3/4, 5/6) obtained via rate-compatible puncturing. Union-bound BEP and FER curves are derived for AWGN with BPSK/QPSK and Gray-coded M-QAM modulation.

Result: The analysis provides exact distance spectrum computations and performance curves for BCC under Viterbi decoding. The results are validated against Monte Carlo simulation, and implementations are provided in Python, Julia, and C++.

Conclusion: BCC remains a critical component of IEEE 802.11 standards, and understanding its distance spectrum is essential for performance analysis. The paper provides tools and methods for exact computation of BCC performance metrics, supporting continued use in current and future wireless deployments.
...
### [251] [On the Uniqueness of Solutions in GPS Source Localization: Distance and Squared-Distance Minimization under Limited Measurements in Two and Three Dimensions](https://arxiv.org/abs/2602.23741)
*Kiwoon Kwon*

Main category: cs.IT

TL;DR: Analysis of source localization minimization problems for GPS applications, examining distance vs squared distance error formulations in 2D/3D space and solution uniqueness with limited measurements.


<details>
  <summary>Details</summary>
Motivation: Source localization is fundamental to GPS applications where solution uniqueness is critical for reliability and accuracy in navigation and military operations.

Method: Examines two minimization problems: distance error minimization and squared distance error minimization, analyzed in both 3D space (standard scenario) and 2D space (simplified case). Also discusses solution possibilities when measurements are fewer than three.

Result: The paper explores theoretical aspects of solution uniqueness and number of possible source solutions under different error formulations and dimensional constraints with limited measurements.

Conclusion: Understanding the mathematical properties of source localization minimization problems is essential for ensuring reliable GPS applications, particularly regarding solution uniqueness with limited measurement data.
...
### [252] [Antenna Coding Optimization for Pixel Antenna Empowered Wireless Communication Using Deep Learning with Heterogeneous Multi-Head Selection](https://arxiv.org/abs/2602.23831)
*Binzhou Zuo,Shanpu Shen,Hongyu Li*

Main category: cs.IT

TL;DR: Deep learning algorithm optimizes pixel antenna coding with 81-297x faster computation than traditional methods while maintaining 98% performance.


<details>
  <summary>Details</summary>
Motivation: Pixel antennas enable flexible radiation adjustment through antenna coding, but traditional heuristic search algorithms have high computational complexity that limits practical implementation.

Method: Proposes a deep learning-based optimization algorithm using heterogeneous multi-head selection mechanism - trains multiple neural networks on different coding schemes and selects the best performing one.

Result: Achieves 98% of traditional search algorithm performance with 81x faster computation for SISO systems and 297x faster for MIMO systems.

Conclusion: The algorithm demonstrates pixel antennas' practical potential for wireless enhancement through high performance and low computational complexity.
...
### [253] [Weighted Unequal Error Protection over a Rayleigh Fading Channel](https://arxiv.org/abs/2602.24225)
*Adeel Mahmood*

Main category: cs.IT

TL;DR: This paper studies unequal error protection in quasi-static Rayleigh fading channels, comparing power-domain superposition (PDS) and orthogonal resource allocation (ORA) schemes for maximizing weighted sum of per-block decoding success probabilities.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address unequal error protection in wireless communications where different message blocks may have different importance levels, and to analyze performance under practical channel conditions (quasi-static Rayleigh fading with receiver-only CSI).

Method: Two achievability schemes are analyzed: power-domain superposition (PDS) and orthogonal resource allocation (ORA/time-sharing). The paper derives upper bounds on optimal number of blocks, provides algorithms for optimal power/time splits, and develops simplified algorithms for locally optimal solutions.

Result: PDS outperforms ORA but the performance difference is less than 2% in both asymptotic and finite blocklength regimes. The gap between asymptotic and finite blocklength performance is bounded by approximately 10% for n=1000 and 3% for n=5000.

Conclusion: Both PDS and ORA are effective for unequal error protection, with PDS having slight performance advantage. The small performance gap suggests ORA may be preferable in practice due to simpler implementation, while the bounded finite blocklength performance gap provides practical design guidelines.
...
### [254] [Finite Block Length Rate-Distortion Theory for the Bernoulli Source with Hamming Distortion: A Tutorial](https://arxiv.org/abs/2602.24243)
*Bhaskar Krishnamachari*

Main category: cs.IT

TL;DR: A tutorial on rate-distortion theory for Bernoulli sources with Hamming distortion, covering classical results, computational methods, and finite block length refinements including rate-distortion dispersion.


<details>
  <summary>Details</summary>
Motivation: Shannon's rate-distortion theory provides fundamental compression limits but assumes infinite block lengths, which are impractical. There's a need to understand how compression performance approaches these theoretical limits with finite block lengths in practice.

Method: Derives classical rate-distortion function for Bernoulli(p) sources with Hamming distortion from first principles, illustrates computation via Blahut-Arimoto algorithm, and develops finite block length refinements using rate-distortion dispersion V(D).

Result: Presents the classical RD = Hp - HD formula, shows how to compute it algorithmically, and characterizes the O(1/√n) penalty for finite block lengths through rate-distortion dispersion analysis.

Conclusion: Provides a comprehensive tutorial bridging classical rate-distortion theory with practical finite block length considerations, with numerical examples and Python code to illustrate the concepts.
