...
TL;DR: SKETCHGAIT introduces a new visual modality (sketch) for gait recognition that captures dense structural cues without semantic labels, complementing existing silhouette and parsing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing gait recognition methods rely on silhouettes (sparse, miss internal details) or parsing (unstable due to upstream parser dependencies). There's an underexplored paradigm of dense part-level structure without explicit semantic labels.

Method: Proposes SKETCH as a new visual modality that extracts high-frequency structural cues (limb articulations, self-occlusion contours) via edge-based detectors from RGB images without labels. Introduces SKETCHGAIT framework with two independent streams for modality-specific learning and lightweight early-stage fusion for structural complementarity.

Result: Achieves 92.9% Rank-1 on SUSTech1K and 93.1% mean Rank-1 on CCPG datasets, validating the effectiveness of the sketch modality and proposed framework.

Conclusion: Sketch provides a valuable new modality for gait recognition that captures structural details without semantic labels, complementing existing approaches. The disentangled multi-modal framework effectively leverages the complementary strengths of different representations.
...
TL;DR: Thinking with Spatial Code transforms RGB video into explicit 3D representations for physical-world visual question answering, enabling LLMs to reason over spatial variables and achieving SOTA on VSI-Bench.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models lack explicit 3D spatial understanding for physical-world visual question answering. The paper aims to bridge this gap by creating temporally coherent 3D representations that enable LLMs to reason directly over spatial variables.

Method: Proposes a spatial encoder that unifies 6D object parsing and tracking with geometric prediction to transform RGB video into structured spatial code with 3D oriented bounding boxes and semantic labels. Finetunes LLMs with reinforcement learning using a spatial rubric reward for perspective-aware, geometrically grounded inference.

Result: The model outperforms proprietary vision-language models on VSI-Bench, setting a new state-of-the-art for physical-world visual question answering.

Conclusion: The Thinking with Spatial Code framework successfully enables LLMs to reason over explicit 3D spatial representations, demonstrating superior performance on spatial reasoning tasks compared to existing vision-language models.
...
TL;DR: First coupled robustness verification framework for heatmap-based keypoint detectors that bounds joint deviation across all keypoints, using MILP formulation with sound guarantees.


<details>
  <summary>Details</summary>
Motivation: Keypoint detection is crucial for many vision tasks but modern neural models are vulnerable to small perturbations. Formal robustness verification for keypoint detectors is largely unexplored due to high-dimensional inputs and continuous coordinate outputs, with existing approaches being decoupled and conservative.

Method: Proposes a coupled verification framework using mixed-integer linear programming (MILP) that combines reachable heatmap sets with a polytope encoding joint deviation constraints. Formulates verification as a falsification problem where infeasibility certifies robustness and feasibility provides counterexamples.

Result: The coupled approach achieves high verified rates and remains effective under strict error thresholds where decoupled methods fail. The method is proven to be sound - if it certifies the model as robust, then the keypoint detection model is guaranteed to be robust.

Conclusion: This work presents the first coupled robustness verification framework for keypoint detectors that captures interdependencies between keypoints, providing stronger guarantees than decoupled approaches and enabling more reliable verification for safety-critical applications.
...
TL;DR: DreamCAD is a multi-modal generative framework that produces editable BReps from point-level supervision without CAD annotations, using parametric patches and differentiable tessellation, achieving SOTA performance on CAD benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing CAD generative methods are limited by small annotated datasets with explicit design histories or BRep labels, while millions of unannotated 3D meshes remain unused, hindering scalable CAD generation.

Method: DreamCAD represents BReps as parametric patches (e.g., Bézier surfaces) and uses differentiable tessellation to generate meshes, enabling large-scale training on 3D datasets while reconstructing connected, editable surfaces. Also introduces CADCap-1M dataset with 1M+ GPT-5 generated descriptions.
...
Conclusion: DreamCAD enables scalable CAD generation without CAD-specific annotations, and the release of CADCap-1M dataset advances text-to-CAD research, with code and dataset to be publicly available.
...
TL;DR: ABRA is a novel domain generalization method that mitigates biological batch effects in cell painting images through adversarial representation augmentation with geometric margin constraints.


<details>
  <summary>Details</summary>
Motivation: High-content screening generates massive cell painting images, but technical variations create biological batch effects that degrade deep learning model generalization on unseen data. Existing methods either require additional prior knowledge or struggle with unseen batches.

Method: ABRA frames batch mitigation as domain generalization, parameterizing feature statistics as structured uncertainties. It uses min-max optimization to synthesize worst-case bio-batch perturbations in representation space with angular geometric margin constraints to preserve class discriminability, plus a synergistic distribution alignment objective to prevent representation collapse.

Result: Extensive evaluations on large-scale RxRx1 and RxRx1-WILDS benchmarks show ABRA establishes new state-of-the-art performance for siRNA perturbation classification.

Conclusion: ABRA effectively addresses biological batch effects in cell painting images through adversarial representation augmentation, achieving superior generalization without requiring additional prior knowledge.
...
TL;DR: PFS is a lightweight post-fusion stabilizer module that improves robustness of BEV fusion detectors against domain shift and sensor failures without architectural changes.
...
Method: PFS operates on intermediate BEV representations of existing detectors, producing refined feature maps for original detection heads. It stabilizes feature statistics under domain shift, suppresses spatial regions affected by sensor degradation, and adaptively restores weakened cues through residual correction as a near-identity transformation.

Result: On nuScenes benchmark, PFS achieves SOTA results in several failure modes: +1.2% mAP improvement for camera dropout robustness, +4.4% mAP improvement for low-light performance, while maintaining only 3.3M parameters.

Conclusion: PFS provides a lightweight, plug-and-play solution to enhance robustness of existing BEV fusion detectors against domain shifts and sensor failures without requiring architectural modifications or retraining.
...
TL;DR: CBM-Suite addresses four key limitations of Concept Bottleneck Models: no metric for concept relevance evaluation, the linearity problem causing bypass of concept bottleneck, accuracy gap compared to opaque models, and lack of systematic study on visual backbones/VLMs.


<details>
  <summary>Details</summary>
Motivation: Current Concept Bottleneck Models (CBMs) have fundamental limitations: 1) No metric to pre-evaluate concept relevance before training, 2) "Linearity problem" where recent CBMs bypass the concept bottleneck entirely, 3) Accuracy gap compared to opaque models, and 4) Lack of systematic study on how different visual backbones and vision-language models impact CBMs.

Method: CBM-Suite introduces four methodological solutions: 1) Entropy-based metric to quantify concept set suitability, 2) Non-linear layer between concept activations and classifier to resolve linearity problem, 3) Distillation loss guided by linear teacher probe to narrow accuracy gap, and 4) Comprehensive analyses of vision encoders, VLMs, and concept sets interactions.

Result: Extensive evaluations show CBM-Suite yields more accurate models and provides insights for improving concept-based interpretability. The framework addresses the fundamental limitations of CBMs while maintaining interpretability.

Conclusion: CBM-Suite provides a systematic framework to overcome key challenges in Concept Bottleneck Models, enabling more accurate and interpretable models while offering methodological insights for concept-based interpretability research.
...
TL;DR: Proposes iFID, a variant of reconstruction FID that correlates strongly with generation FID of latent diffusion models by interpolating nearest neighbor latents.


<details>
  <summary>Details</summary>
Motivation: Reconstruction FID (rFID) of VAEs poorly correlates with generation FID (gFID) of latent diffusion models, creating a need for better predictive metrics.

Method: For each dataset element, retrieve its nearest neighbor in latent space, interpolate their latent representations, decode the interpolated latent, and compute FID between decoded samples and original dataset.

Result: iFID achieves strong correlation with diffusion gFID (Pearson ~0.85, Spearman ~0.85), the first metric to demonstrate such correlation. Also refines understanding: rFID correlates with diffusion refinement phase quality, while iFID correlates with navigation phase quality.

Conclusion: iFID provides a simple, effective metric that strongly predicts diffusion generation quality, addressing the disconnect between reconstruction and generation metrics, with theoretical connections to diffusion generalization and hallucination.
...
TL;DR: The paper proposes Implicit Error Counting (IEC) for reinforcement learning in reference-free settings where multiple valid outputs exist, addressing limitations of rubric-based methods that require ideal reference answers.


<details>
  <summary>Details</summary>
Motivation: Current post-training methods like RLVR and RaR work well when ideal reference answers exist, but many real-world tasks have multiple valid outputs without single ideal references, creating a gap in evaluation methods.

Method: IEC enumerates what responses get wrong rather than what they get right, using severity-weighted scores across task-relevant axes with implicit score emission and group calibration to create stable rewards. Cascaded Error Counting (CEC) is introduced as an evaluation metric.

Result: IEC outperforms RaR on MDressBench (CEC: 5.31 vs. 5.60 on flat references; 5.20 vs. 5.53 on non-flat) and matches/surpasses six baselines on 6 of 8 perceptual metrics on VITON-HD and DressCode. CEC tracks human preferences well (60% top-1 vs. 30% others).

Conclusion: When ideal answers are unavailable, counting errors provides a stronger signal than constructing rubrics, making IEC effective for reference-free domains like virtual try-on where multiple valid outputs exist.
...
TL;DR: This paper analyzes surgical instrument manufacturing defects in Pakistan using deep learning models (YOLOv8, ResNet-152, EfficientNet-b4) on a dataset of 4,414 images to improve quality assurance and patient safety.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address critical safety concerns in surgical instrument manufacturing, where even minor defects can have fatal consequences. The paper aims to prevent financial losses for manufacturers while safeguarding patient lives through improved quality control.

Method: The method involves collaboration with industry leaders Daddy D Pro and Dr. Frigz International, using a newly developed dataset of 4,414 high-resolution images of Pakistani-made surgical instruments. The study employs deep learning architectures including YOLOv8, ResNet-152, and EfficientNet-b4 for automated defect detection through Automated Optical Inspection (AOI) tools.

Result: The research provides insights into identifying and analyzing various manufacturing defects such as cracks, rust, and structural irregularities in surgical instruments. The automated defect detection methodologies contribute to enhancing quality standards in the manufacturing process.

Conclusion: The paper concludes that integrating deep learning-based automated defect detection can significantly improve surgical instrument quality assurance, ensuring safer healthcare environments through industry expertise and cutting-edge technology. The findings serve as a valuable resource for manufacturers, healthcare professionals, and regulatory bodies.
...
TL;DR: SemVID is a training-free visual token pruning framework for Video Temporal Grounding that achieves strong accuracy-efficiency trade-offs by selectively preserving boundary-sensitive evidence and maintaining cross-frame connectivity.


<details>
  <summary>Details</summary>
Motivation: Video Temporal Grounding requires processing long untrimmed videos, making video-language-model pipelines prohibitively expensive. Existing training-free visual token pruning methods designed for video QA degrade significantly when applied to VTG because VTG depends on boundary-sensitive evidence and cross-frame reasoning chains.

Method: SemVID identifies two VTG-specific pruning principles: Evidence Retention (keeps query-critical patches around event boundaries) and Connectivity Strength (preserves token-level cross-frame connectivity). The framework allocates per-frame token budgets by balancing query relevance and inter-frame variation, then selects three complementary token types: object tokens for query-critical evidence, motion tokens for transitions and cross-frame relays, and context tokens for scene continuity.

Result: Extensive experiments on VTG benchmarks show SemVID retains up to 95.4% mIoU with only 12.5% visual tokens and delivers up to 5.8x prefill speedup, consistently outperforming prior methods under the same token budgets.

Conclusion: SemVID effectively addresses the unique challenges of VTG through principled token pruning that preserves boundary-sensitive evidence and maintains cross-frame connectivity, achieving superior accuracy-efficiency trade-offs without requiring training.
...
TL;DR: NOVA proposes an open-vocabulary 3D tracking paradigm using LLM-based autoregressive sequence completion to generalize across unknown objects by modeling trajectories as spatio-temporal semantic sequences.


<details>
  <summary>Details</summary>
Motivation: Existing 3D Multi-Object Tracking pipelines are limited by closed-set assumptions and "semantic-blind" heuristics, preventing generalization to unknown targets in open-world perception scenarios.

Method: Reformulates 3D trajectories as structured spatio-temporal semantic sequences and uses LLM autoregressive capabilities to transform tracking into next-step sequence completion, encoding both physical motion continuity and linguistic priors.

Result: Achieves 22.41% AMOTA for Novel categories on nuScenes (20.21% absolute improvement over baseline), with strong performance on V2X-Seq-SPD and KITTI using only a 0.5B parameter model.

Conclusion: NOVA successfully shifts 3D tracking from traditional distance-based matching to generative spatio-temporal semantic modeling, enabling open-vocabulary generalization through language-space reasoning and LLM capabilities.
...
TL;DR: OWL is a perception function that uses visual motion cues (looming and rotation) to enable scaled 3D mapping and camera heading without requiring explicit measurements of depth or motion parameters.


<details>
  <summary>Details</summary>
Motivation: To address complex 3D perception challenges during motion by developing a unified analytical approach that works directly from visual motion cues without needing prior knowledge of environment, objects, or camera motion.

Method: Derives OWL values from two fundamental visual motion cues: perceived local visual looming near fixation points and perceived rotation of rigid objects relative to fixation points. Uses minimalistic pixel-based parallel computations on raw visual motion image sequences.

Result: OWL achieves geometric constancy of 3D objects over time and enables scaled 3D scene reconstruction from visual motion cues alone. Simulations demonstrate it can work without prior knowledge of stationary environments, moving objects, or camera motion.

Conclusion: OWL bridges theory and practice in robotics/autonomous navigation, offers real-time 3D representation, and provides an alternative perspective on machine perception with implications for natural perception, behavioral psychology, and neural functionality.
...
TL;DR: The paper introduces OccNL, the first benchmark for 3D occupancy prediction under label noise, and proposes DPR-Occ, a noise-robust framework that outperforms existing methods even at 90% noise levels.


<details>
  <summary>Details</summary>
Motivation: Real-world 3D voxel annotations are corrupted by structural artifacts and dynamic trailing effects, creating unreliable supervision for autonomous systems. Current 2D label noise learning methods fail catastrophically in sparse 3D voxel spaces, exposing critical vulnerabilities in existing paradigms.

Method: Proposes DPR-Occ, a label noise-robust framework that constructs reliable supervision through dual-source partial label reasoning. It synergizes temporal model memory with representation-level structural affinity to dynamically expand and prune candidate label sets, preserving true semantics while suppressing noise propagation.

Result: Extensive experiments on SemanticKITTI show DPR-Occ prevents geometric and semantic collapse under extreme corruption. At 90% label noise, it achieves significant performance gains (up to 2.57% mIoU and 13.91% IoU) over existing label noise learning baselines adapted to 3D occupancy prediction.

Conclusion: The work bridges label noise learning and 3D perception, providing a reliable foundation for safety-critical robotic perception in dynamic environments through the OccNL benchmark and DPR-Occ framework.
...
TL;DR: MultiHaystack benchmark reveals multimodal retrieval as major bottleneck for MLLMs, showing sharp performance drops when models must retrieve evidence from large heterogeneous corpora rather than being given evidence directly.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate MLLMs on text, image, or video understanding separately but don't assess real-world requirement of retrieving relevant evidence from large, heterogeneous multimodal corpora before reasoning. Current benchmarks restrict retrieval to small, single-modality candidate sets, oversimplifying search space and overstating reliability.

Method: Introduced MultiHaystack benchmark with 46,000+ multimodal retrieval candidates across documents, images, and videos, plus 747 open yet verifiable questions. Each question is grounded in unique validated evidence item requiring evidence localization across modalities and fine-grained reasoning.

Result: Models perform competitively when given corresponding evidence directly, but performance drops sharply when required to retrieve evidence from full corpus. Best retriever E5-V achieves only 40.8% Recall@1. GPT-5 reasoning accuracy drops from 80.86% (with evidence) to 51.4% under top-5 retrieval.

Conclusion: Multimodal retrieval over heterogeneous pools remains primary bottleneck for MLLMs. MultiHaystack serves as valuable testbed highlighting limitations obscured by small-scale evaluations and promotes retrieval-centric advances in multimodal systems.
...
TL;DR: Audiovisual Geolocation framework combines audio and visual cues for precise global localization, outperforming unimodal approaches through interpretable acoustic perception and multimodal reasoning.


<details>
  <summary>Details</summary>
Motivation: Multimodal Large Language Models have improved image-based localization, but precise global geolocation remains challenging due to visual ambiguity and underutilized auditory cues. The paper aims to resolve geographic ambiguity by leveraging both visual and auditory information.

Method: Three-stage framework: (1) Perception stage uses mixture-autoregressive sparse autoencoder to decompose noisy audio into "acoustic atoms"; (2) Multimodal Reasoning stage employs MLLM finetuned via Group Relative Policy Optimization to synthesize acoustic atoms with visual features; (3) Precision Prediction stage uses Riemannian Flow Matching on the S² manifold for accurate localization.

Result: The framework significantly outperforms unimodal baselines. The AVG benchmark includes 20,000 curated video clips across 1,000 distinct locations. Results show that interpretable soundscape perception provides critical orthogonal signals for high-precision global localization.

Conclusion: Interpretable perception of soundscapes combined with multimodal reasoning enables high-precision global geolocation, demonstrating that audio provides a critical complementary signal to visual information for resolving geographic ambiguity.
...
TL;DR: Any2Full is a one-stage depth completion framework that uses scale-prompting adaptation of pretrained monocular depth estimation models to handle various sparse depth patterns without domain-specific training.


<details>
  <summary>Details</summary>
Motivation: Existing RGBD-fused depth completion methods have limited domain generalization and robustness to different depth patterns. Current two-stage approaches using MDE models require explicit alignment that adds computation and causes structured distortions.

Method: Reformulates depth completion as scale-prompting adaptation of pretrained MDE models. Uses Scale-Aware Prompt Encoder to distill scale cues from sparse inputs into unified scale prompts, guiding MDE toward globally scale-consistent predictions while preserving geometric priors.

Result: Outperforms OMNI-DC by 32.2% in average AbsREL and delivers 1.4× speedup over PriorDA with same MDE backbone. Achieves superior robustness and efficiency across various depth patterns.

Conclusion: Any2Full establishes a new paradigm for universal depth completion that is one-stage, domain-general, and pattern-agnostic, offering better generalization and efficiency than existing methods.
...
TL;DR: Automated pipeline converts ImageNet training set to multi-label dataset using self-supervised ViTs for object discovery, improving classification accuracy and transfer learning.


<details>
  <summary>Details</summary>
Motivation: ImageNet's single-label assumption creates label noise and limits learning signal since real images often contain multiple objects. Multi-label annotations better reflect real-world scenes and enable richer representation learning.

Method: Use self-supervised Vision Transformers for unsupervised object discovery, select regions aligned with original labels to train lightweight classifier, apply classifier to all regions to generate coherent multi-label annotations across dataset.

Result: Models trained with multi-label supervision achieve up to +2.0 top-1 accuracy on ReaL and +1.5 on ImageNet-V2, with stronger transferability (+4.2 mAP on COCO, +2.3 mAP on VOC). Labels align well with human judgment.

Conclusion: Multi-label annotations significantly enhance both classification performance and representation learning, demonstrating importance of accurate multi-label supervision for visual recognition tasks.
...
TL;DR: Automatic surgical timeline generation from video using CLIP-based multimodal alignment between video frames and textual gesture descriptions.


<details>
  <summary>Details</summary>
Motivation: Current methods for creating surgical timelines are either too vague (post-op reports) or too time-consuming (manual video annotation), creating a need for an automated solution.

Method: CLIP-based multimodal framework that aligns surgical video frames with textual gesture descriptions using visual and text encoders in shared embedding space, fine-tuned for surgical domain.

Result: Model predicts gestures and phases for video frames, enabling automatic construction of structured surgical timelines from raw surgical videos.

Conclusion: Proposed approach bridges visual gestures and textual narratives using pretrained multimodal representations, reducing manual video review and annotation workload for surgeons.
...
TL;DR: Icarus is a deep learning sky-model that generates photorealistic, high-resolution environment maps with accurate solar region modeling and user-controlled positioning of sun and clouds.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning sky-models fail to accurately recreate natural skies, especially at higher resolutions where they struggle with the 14EV+ class-imbalanced solar region, resulting in poor visual quality, skewed lighting, shadows, and tones.

Method: Icarus is an all-weather sky-model that learns the exposure range of Full Dynamic Range (FDR) physically captured outdoor imagery, enabling conditional generation of environment maps with intuitive user-positioning of solar and cloud formations, plus user-controlled texturing of atmospheric formations.

Result: Icarus demonstrates interchangeability with FDR physically captured outdoor imagery or parametric sky-models, and illuminates scenes with unprecedented accuracy, photorealism, lighting directionality (shadows), and tones in Image Based Lighting (IBL).

Conclusion: Icarus represents a significant advancement in sky-modeling, providing a fast, flexible alternative to physical capture while overcoming the limitations of previous deep learning approaches for photorealistic environment map generation.
...
TL;DR: LayerBind is a training-free plug-and-play method for precise region and occlusion control in text-to-image generation by modeling regional generation as distinct layers and binding them during generation.


<details>
  <summary>Details</summary>
Motivation: Existing region-instructed layout control methods have limitations: training-based approaches inherit data bias and degrade image quality, while current techniques struggle with occlusion order, limiting real-world usability. The authors observed that spatial layout and occlusion are established at early denoising stages, suggesting rearranging early latent structure can modify final output.

Method: Two-phase approach: (1) Layer-wise Instance Initialization creates per-instance branches that attend to their own regions while anchoring to shared background, then fuses them according to layer order at early denoising step. (2) Layer-wise Semantic Nursing reinforces regional details and maintains occlusion order via sequential layered attention path alongside standard global path, with updates composited under layer-transparency scheduler.

Result: LayerBind enables precise regional and occlusion controllability without training, serving as plug-and-play controller across Diffusion Transformers. It natively supports editable workflows for flexible modifications like changing instances or rearranging visible orders.

Conclusion: LayerBind effectively addresses limitations of existing methods, offering training-free precise layout control with occlusion handling, demonstrating strong potential for creative applications through both qualitative and quantitative results.
...
TL;DR: BM25-V applies text retrieval's BM25 scoring to visual-word activations from Vision Transformers, creating an efficient, interpretable sparse retrieval method that achieves high recall for dense reranking pipelines.


<details>
  <summary>Details</summary>
Motivation: Dense image retrieval has accuracy but lacks interpretability/attribution and is computationally expensive at scale. The authors want a more efficient, interpretable alternative that maintains high performance.

Method: Apply Okapi BM25 scoring to sparse visual-word activations from a Sparse Auto-Encoder (SAE) on Vision Transformer patch features. Use BM25's IDF weighting to suppress common visual words and emphasize rare discriminative ones. Retrieve via sparse inverted-index operations as first-stage retriever for dense reranking.

Result: Achieves Recall@200 ≥ 0.993 across seven benchmarks, enabling two-stage pipeline that reranks only 200 candidates per query and recovers near-dense accuracy within 0.2% on average. SAE trained on ImageNet-1K transfers zero-shot to seven fine-grained benchmarks without fine-tuning. Retrieval decisions are attributable to specific visual words with quantified IDF contributions.

Conclusion: BM25-V provides an efficient, interpretable sparse retrieval method that achieves high recall for dense reranking pipelines, addressing limitations of dense retrieval while maintaining competitive accuracy.
...
TL;DR: Learnable upsampling methods for 3D reconstruction often focus on spatial detail but may harm spectral consistency, which is more critical for reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Current learnable upsampling methods in 2D-to-3D pipelines aim to enhance spatial details but their impact on 3D awareness remains underexplored. The authors want to understand how different upsampling methods affect 3D reconstruction quality through spectral analysis.

Method: Introduces a spectral diagnostic framework with six complementary metrics to analyze upsampling methods: amplitude redistribution, structural spectral alignment, and directional stability metrics. Tests classical interpolation and learnable upsampling methods on CLIP and DINO backbones.

Result: Three key findings: 1) Structural spectral consistency is the strongest predictor of novel view synthesis quality, while high-frequency emphasis often correlates negatively with reconstruction; 2) Geometry and texture respond to different spectral properties; 3) Learnable upsamplers often produce sharper features but rarely outperform classical interpolation in reconstruction quality.

Conclusion: Reconstruction quality depends more on preserving spectral structure than enhancing spatial detail, highlighting spectral consistency as a key principle for designing upsampling strategies in 2D-to-3D pipelines.
...
TL;DR: EventGeM is a state-of-the-art event-based Visual Place Recognition system that combines global and local feature fusion with depth estimation for robust localization across various lighting conditions in real-time.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer sparse activation and high-temporal resolution ideal for robotic navigation, but existing methods need improvement for accurate positioning in challenging conditions like varying lighting.

Method: Uses ViT-S/16 for global feature extraction from event histogram images, MaxViT for local keypoint detection with 2D-homography re-ranking via RANSAC, and vision foundation model for depth estimation-based structural similarity refinement.

Result: Achieves state-of-the-art localization performance compared to existing event-based place recognition methods across multiple benchmark datasets and lighting conditions, with real-time capability on various compute architectures.

Conclusion: EventGeM demonstrates effective global-to-local feature fusion for robust event-based Visual Place Recognition, enabling real-world robotic deployment with event camera streams for online localization.
...
TL;DR: LIPAR framework speeds up video generation by pruning redundant latent patches and recovering attention values, achieving 1.45× throughput increase without quality loss.


<details>
  <summary>Details</summary>
Motivation: Current video generation models have high computational latency that makes real-time applications prohibitively expensive. The paper aims to address this limitation by exploiting temporal redundancy in video latent patches.

Method: Proposes Latent Inter-frame Pruning with Attention Recovery (LIPAR) framework that detects and skips recomputing duplicated latent patches. Introduces novel Attention Recovery mechanism to approximate attention values of pruned tokens and remove visual artifacts from naive pruning.

Result: Increases video editing throughput by 1.45× on average, achieving 12.2 FPS on NVIDIA A6000 compared to baseline 8.4 FPS. Method maintains generation quality and can be integrated without additional training.

Conclusion: The approach effectively bridges the gap between traditional compression algorithms and modern generative pipelines, providing efficient video generation without compromising quality.
...
TL;DR: MaCS is a regularization framework that improves classifier calibration and robustness by enforcing logit-space margins and local prediction consistency, without architectural changes or inference overhead.


<details>
  <summary>Details</summary>
Motivation: Deep vision classifiers often achieve high accuracy but suffer from poor calibration and fragility under small distribution shifts, indicating overconfidence and lack of robustness.

Method: MaCS augments cross-entropy with two components: (1) hinge-squared margin penalty enforcing target logit gap between correct class and strongest competitor, and (2) consistency regularizer minimizing KL divergence between predictions on clean inputs and mildly perturbed views.

Result: Across multiple image classification benchmarks and backbones (CNNs and Vision Transformers), MaCS consistently improves calibration (lower ECE and NLL) and robustness to common corruptions while preserving or improving top-1 accuracy.

Conclusion: MaCS provides an effective drop-in replacement for standard training objectives that improves both calibration and robustness without additional data, architectural changes, or inference overhead, supported by theoretical guarantees on generalization and robustness.
...
TL;DR: A fusion model combining CNNs and Vision Transformers with ensemble learning achieves state-of-the-art accuracy on remote sensing image classification datasets.


<details>
  <summary>Details</summary>
Motivation: Remote sensing image classification requires accurate techniques, but CNNs struggle with global context while ViTs capture long-range dependencies. Combining both architectures should improve performance, but simple integration creates bottlenecks from redundant features.

Method: Proposes a fusion approach that trains four independent models combining CNN and ViT backbones, then ensembles their outputs at the final prediction stage to overcome performance bottlenecks from redundant feature representations.
...
Conclusion: The ensemble-based fusion of CNN and ViT backbones effectively combines local feature extraction with global context modeling, achieving superior remote sensing classification performance with efficient computational resource utilization.
...
TL;DR: Cog2Gen3D: A 3D cognition-guided diffusion framework that uses semantic and geometric representations structured into a 3D cognition graph to guide 3D Gaussian generation, addressing scale inconsistency and spatial awareness limitations in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing 3D generation methods struggle with spatial geometry constraints, often only modeling relative relationships and suffering from scale inconsistency of absolute geometry. The authors argue that both semantic information and absolute geometry are crucial for 3D cognition to enable controllable 3D generation for the physical world.

Method: Three key designs: 1) Cognitive Feature Embeddings - encode different modalities into semantic and geometric representations plus logical representations; 2) 3D Latent Cognition Graph - structure representations into dual-stream semantic-geometric graphs fused via common-based cross-attention; 3) Cognition-Guided Latent Diffusion - use the fused 3D cognition graph to guide latent diffusion process for 3D Gaussian generation.

Result: Extensive experiments show Cog2Gen3D significantly outperforms existing methods in both semantic fidelity and geometric plausibility. The model was validated using a subset based on Marble World Labs.

Conclusion: The proposed unified framework with 3D cognition graph ensures physical plausibility and structural rationality of 3D generation, addressing limitations of existing methods through combined semantic and geometric guidance.
...
TL;DR: VS3R is a video stabilization framework that combines 3D reconstruction with generative video diffusion to achieve full-frame stabilization without aggressive cropping, outperforming state-of-the-art methods in robustness and visual quality.


<details>
  <summary>Details</summary>
Motivation: Video stabilization faces a fundamental trade-off: 2D methods suffer from aggressive cropping, while 3D techniques often fail under extreme motions due to fragile optimization pipelines. There's a need to bridge this gap between geometric robustness and full-frame consistency.

Method: VS3R synergizes feed-forward 3D reconstruction with generative video diffusion. It jointly estimates camera parameters, depth, and masks for reliability, uses a Hybrid Stabilized Rendering module to fuse semantic and geometric cues for dynamic consistency, and employs a Dual-Stream Video Diffusion Model to restore disoccluded regions and rectify artifacts by combining structural guidance with semantic anchors.

Result: VS3R achieves high-fidelity, full-frame stabilization across diverse camera models and significantly outperforms state-of-the-art methods in both robustness and visual quality.

Conclusion: The proposed VS3R framework successfully bridges the gap between geometric robustness and full-frame consistency in video stabilization by combining 3D reconstruction with generative video diffusion, delivering superior performance compared to existing methods.
...
TL;DR: TumorChain: A multimodal reasoning framework for clinical tumor analysis using 3D CT scans with Chain-of-Thought reasoning to improve diagnostic accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Accurate tumor analysis is crucial for clinical radiology and precision oncology, but current methods lack interpretability and traceability. Chain-of-Thought reasoning can enable step-by-step interpretation from imaging findings to clinical conclusions, reducing diagnostic errors.

Method: Created TumorCoT dataset (1.5M CoT-labeled VQA instructions with 3D CT scans) and proposed TumorChain framework with multimodal interleaved reasoning. Combines 3D imaging encoders, clinical text understanding, organ-level vision-language alignment, and iterative self-refinement through cross-modal alignment.

Result: Consistent improvements over strong baselines in lesion detection, impression generation, and pathology classification. Strong generalization demonstrated on DeepTumorVQA benchmark.

Conclusion: Multimodal reasoning with Chain-of-Thought approach enables reliable and interpretable tumor analysis in clinical practice, improving traceability and reducing hallucination risk.
...
TL;DR: PatchCue: A patch-based visual cue paradigm that enhances VLMs' reasoning by using patch-level visual cues instead of pixel-level localization, improving performance across diverse multimodal tasks.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning paradigms like Chain-of-Thought rely only on text and underutilize visual cues. Prior pixel-level visual cue approaches require precise spatial localization which adds learning complexity. There's a need for a more effective visual reasoning paradigm that better aligns with human perception and modern VLM architectures.

Method: Proposes PatchCue: partitions images into patches and represents cues at patch level. Uses two-stage training: 1) cold-start supervised fine-tuning to output patch-level cues, 2) reinforcement learning with process-supervised cue reward to guide intermediate visual reasoning steps.

Result: Extensive experiments on multiple VLMs and diverse benchmarks (general VQA, complex reasoning, document understanding) show PatchCue consistently improves overall model performance. Patch-level cues outperform both pixel-level bounding boxes and point-based cues.

Conclusion: PatchCue provides a more effective and cognitively aligned visual reasoning paradigm that better leverages visual information while reducing learning complexity compared to pixel-level approaches.
...
TL;DR: MemSeg-Agent is a memory-augmented segmentation agent that enables few-shot learning, federated learning, and test-time adaptation for medical image segmentation by shifting adaptation from weight space to memory space.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation models trained on single datasets fail to generalize across institutions, scanners, or patient populations. Vision foundation models require task-specific fine-tuning with substantial communication overhead in federated learning and prevent continuous knowledge evolution during deployment.

Method: Proposes MemSeg-Agent that conditions a fixed backbone with lightweight static, few-shot, and test-time working memories dynamically composed by an agentic controller. In federated settings, updates compact memory units instead of model parameters to reduce communication overhead.

Result: Experiments on four public datasets show strong performance and robustness to domain shift. Static memory alone matches or surpasses strong supervised baselines with high parameter efficiency. Test-time working memory further improves in-domain and cross-domain performance without fine-tuning.

Conclusion: MemSeg-Agent introduces a new paradigm for scalable and adaptive medical image segmentation in the era of agentic AI, enabling unified architecture for few-shot learning, federated supervised learning, and test-time adaptation.
...
TL;DR: Synthetic novel views improve Video Place Recognition (VPR) performance when added in small quantities, with dataset characteristics and view count being more important than viewpoint magnitude for larger additions.


<details>
  <summary>Details</summary>
Motivation: Synthetic novel views can enhance robot navigation by enabling cross-platform guidance (ground-to-air and air-to-ground) and improving place recognition across different viewpoints in Video Place Recognition systems.

Method: Systematic evaluation using five public VPR image databases and seven typical image similarity methods to assess the impact of synthetic novel views on VPR performance.

Result: Small synthetic additions improve VPR recognition statistics; for larger additions, the number of views added and dataset imagery type matter more than viewpoint change magnitude.

Conclusion: Synthetic novel views are beneficial for VPR, with optimal benefits achieved through strategic addition based on dataset characteristics rather than maximizing viewpoint variation.
...
TL;DR: CylinderSplat introduces a feed-forward framework for panoramic 3D Gaussian Splatting using a cylindrical Triplane representation to better handle 360° scenes and sparse-view scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods struggle with panoramic imagery due to multi-view cost volumes that fail with occlusions in sparse views, and standard Cartesian Triplanes cause distortion and aliasing in 360° scenes.

Method: Proposes CylinderSplat with a cylindrical Triplane representation aligned with panoramic geometry and Manhattan-world structures. Uses dual-branch architecture: pixel-based branch for well-observed regions and volume-based branch with cylindrical Triplane for occluded/sparse areas. Handles variable input views from single to multiple panoramas.

Result: Achieves state-of-the-art results in both single-view and multi-view panoramic novel view synthesis, outperforming previous methods in reconstruction quality and geometric accuracy.

Conclusion: CylinderSplat effectively addresses limitations of existing 3DGS methods for panoramic scenes through cylindrical representation and dual-branch architecture, enabling high-quality novel view synthesis for 360° imagery.
...
TL;DR: PixARMesh is a single-image 3D scene reconstruction method that generates complete indoor scene meshes autoregressively in one forward pass, producing artist-ready meshes without post-hoc optimization.


<details>
  <summary>Details</summary>
Motivation: Prior methods for 3D scene reconstruction from single images rely on implicit signed distance fields and require post-hoc layout optimization, which can be inefficient and produce less coherent results. There's a need for a unified approach that jointly predicts object layout and geometry while producing production-ready meshes.

Method: PixARMesh uses a point-cloud encoder augmented with pixel-aligned image features and global scene context via cross-attention. It generates scenes autoregressively from a unified token stream containing context, pose, and mesh information, enabling joint prediction of object layout and geometry in a single forward pass.

Result: Experiments on synthetic and real-world datasets show PixARMesh achieves state-of-the-art reconstruction quality while producing lightweight, high-quality meshes. The method generates compact meshes with high-fidelity geometry that are ready for downstream applications.

Conclusion: PixARMesh represents a significant advance in single-image 3D scene reconstruction by producing complete, coherent, artist-ready meshes in a unified autoregressive framework, outperforming prior methods that require separate layout optimization steps.
...
TL;DR: InnoAds-Composer: A single-stage diffusion framework for e-commerce poster generation with efficient tri-conditional control over subject, text, and style, addressing issues in multi-stage pipelines.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models for product poster generation rely on multi-stage pipelines with poor subject fidelity, inaccurate text rendering, and inconsistent style control. There's a need for simultaneous control over all three elements (subject, text, style) in a more efficient framework.

Method: Proposes InnoAds-Composer, a single-stage framework with tri-conditional control tokens. Uses importance analysis over layers and timesteps to route each condition only to most responsive positions, reducing token sequence length. Includes Text Feature Enhancement Module (TFEM) for better Chinese text rendering by integrating glyph image and crop features.

Result: Significantly outperforms existing product poster methods without increasing inference latency. Also contributes a new high-quality e-commerce poster dataset with subject, text, and style conditions - the first of its kind.

Conclusion: InnoAds-Composer effectively addresses the limitations of multi-stage pipelines by enabling efficient simultaneous control over subject, text, and style in product poster generation, while maintaining computational efficiency.
...
### [37] [Mitigating Bias in Concept Bottleneck Models for Fair and Interpretable Image Classification](https://arxiv.org/abs/2603.05899)
*Schrasing Tong,Antoine Salaun,Vincent Yuan,Annabel Adeyeri,Lalana Kagal*

Main category: cs.CV

TL;DR: Three bias mitigation techniques for concept bottleneck models improve fairness in image classification while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: Concept bottleneck models (CBMs) theoretically support fairness by masking sensitive attributes, but in practice they leak information and show only marginal bias reduction. There's a need to improve fairness in CBMs while preserving their interpretability benefits.

Method: Three bias mitigation techniques: 1) Top-k concept filter to decrease information leakage, 2) Removal of biased concepts, and 3) Adversarial debiasing. These are applied to concept bottleneck models for fair image classification.

Result: Outperforms prior work in fairness-performance tradeoffs, providing significant improvements over previous CBMs that showed only marginal gender bias reduction on datasets like ImSitu.

Conclusion: The proposed debiased CBM represents a significant step toward achieving both fair and interpretable image classification systems.
...
TL;DR: CollabOD is a lightweight collaborative detection framework for small object detection in UAV imagery that preserves structural details and aligns features before fusion to address scale variation and detail degradation issues.


<details>
  <summary>Details</summary>
Motivation: Small object detection in UAV imagery faces challenges from scale variation, structural detail degradation, and limited computational resources. In high-altitude scenarios, fine-grained features are weakened during downsampling and cross-scale fusion, leading to unstable localization and reduced robustness.

Method: Proposes CollabOD framework with three key strategies: Structural Detail Preservation, Cross-Path Feature Alignment, and Localization-Aware Lightweight Design. Integrates these approaches to optimize conventional UAV perception models from image processing, channel structure, and lightweight design perspectives. Includes a unified detail-aware detection head for improved regression robustness.

Result: The framework enhances representation stability while maintaining efficient inference. The detail-aware detection head improves regression robustness without adding deployment overhead. Code is publicly available on GitHub.

Conclusion: CollabOD effectively addresses small object detection challenges in UAV imagery by preserving structural details, aligning heterogeneous features, and maintaining lightweight design for practical deployment in resource-constrained scenarios.
...
TL;DR: Art3D proposes artistic disparity synthesis for 3D conversion, shifting from geometric accuracy to artistic coherence by learning from professional 3D films to replicate cinematic depth effects.


<details>
  <summary>Details</summary>
Motivation: Current 2D-to-3D conversion methods achieve geometric accuracy but lack artistic quality, failing to replicate the immersive and emotionally resonant experience of professional 3D cinema. They mistake deliberate artistic intent (like zero-plane shifts for pop-out effects) for data noise or ambiguity.

Method: Art3D uses a dual-path architecture to decouple global depth parameters (macro-intent) from local artistic effects (visual brushstrokes), learning from professional 3D film data via indirect supervision. It introduces Artistic Disparity Synthesis paradigm instead of physically accurate disparity estimation.

Result: Experiments show Art3D demonstrates potential in replicating key local out-of-screen effects and aligning with the global depth styles of cinematic 3D content, laying groundwork for artistically-driven conversion tools.

Conclusion: The paper argues for a paradigm shift from geometric reconstruction to artistic disparity synthesis, proposing Art3D as a preliminary framework that shows promise in creating more artistically coherent 3D conversions that capture professional cinematic intent.
...
TL;DR: Pano3DComposer is a feed-forward framework that efficiently generates 360-degree 3D scenes from panoramic images by decoupling object generation from layout estimation using an Object-World Transformation Predictor.


<details>
  <summary>Details</summary>
Motivation: Current compositional image-to-3D scene generation methods are either too slow (iterative optimization) or inflexible (joint object-layout generation), and most rely on limited field-of-view perspective images, preventing creation of complete 360-degree environments.

Method: Proposes Pano3DComposer with: 1) Object-World Transformation Predictor that converts 3D objects from off-the-shelf models from local to world coordinates using Alignment-VGGT architecture; 2) Pseudo-geometric supervision to handle shape discrepancies; 3) Coarse-to-Fine alignment mechanism for unseen domains that iteratively refines geometric consistency.

Result: Achieves superior geometric accuracy for image/text-to-3D tasks on synthetic and real-world datasets. Can generate high-fidelity 3D scenes in approximately 20 seconds on an RTX 4090 GPU.

Conclusion: Pano3DComposer provides an efficient, feed-forward solution for panoramic 3D scene generation that addresses limitations of existing methods through decoupled object-layout processing and robust transformation prediction.
...
### [41] [CORE-Seg: Reasoning-Driven Segmentation for Complex Lesions via Reinforcement Learning](https://arxiv.org/abs/2603.05911)
*Yuxin Xie,Yuming Chen,Yishan Yang,Yi Zhou,Tao Zhou,Zhen Zhao,Jiacheng Liu,Huazhu Fu*

Main category: cs.CV

TL;DR: CORE-Seg is a reasoning-driven framework for complex lesion segmentation that integrates Chain-of-Thought reasoning with segmentation, achieving state-of-the-art performance with 37.06% mean Dice score.
...
Method: Proposes CORE-Seg, an end-to-end framework with Semantic-Guided Prompt Adapter, progressive training from SFT to GRPO, and adaptive dual-granularity reward mechanism to mitigate reward sparsity.

Result: Achieves state-of-the-art results with mean Dice of 37.06% (14.89% higher than second-best baseline) and reduces failure rate to 18.42% on the ComLesion-14K benchmark.

Conclusion: The framework successfully integrates reasoning with segmentation for complex lesions, demonstrating significant improvements over existing methods and addressing the gap between general MLLMs and specialized segmentation models.
...
### [42] [BlackMirror: Black-Box Backdoor Detection for Text-to-Image Models via Instruction-Response Deviation](https://arxiv.org/abs/2603.05921)
*Feiran Li,Qianqian Xu,Shilong Bao,Zhiyong Yang,Xilin Zhao,Xiaochun Cao,Qingming Huang*

Main category: cs.CV

TL;DR: BlackMirror is a novel black-box detection framework for backdoored text-to-image models that identifies semantic pattern deviations rather than relying on image similarity, achieving accurate detection across diverse backdoor attacks.


<details>
  <summary>Details</summary>
Motivation: Existing detection methods fail against emerging backdoor attacks where backdoored generations appear visually diverse, as they rely on image-level similarity analysis assuming consistent outputs. The key insight is that only partial semantic patterns are steadily manipulated in backdoor attacks while other content remains diverse.

Method: BlackMirror consists of two components: MirrorMatch aligns visual patterns with instructions to detect semantic deviations, and MirrorVerify evaluates the stability of these deviations across varied prompts to distinguish true backdoor behavior from benign responses. It's a training-free, plug-and-play framework for MaaS applications.

Result: Comprehensive experiments demonstrate that BlackMirror achieves accurate detection across a wide range of backdoor attacks, outperforming existing approaches that struggle with visually diverse backdoored generations.

Conclusion: BlackMirror provides an effective solution for detecting backdoored text-to-image models in black-box settings by focusing on semantic pattern deviations rather than image similarity, making it generalizable to emerging attacks with diverse visual outputs.
...
### [43] [RAC: Rectified Flow Auto Coder](https://arxiv.org/abs/2603.05925)
*Sen Fang,Yalin Feng,Yanxin Zhang,Dimitris N. Metaxas*

Main category: cs.CV

TL;DR: RAC replaces VAE with rectified flow-based autoencoder, achieving straight/correctable decoding, bidirectional inference, and better quality with lower computation.


<details>
  <summary>Details</summary>
Motivation: To address limitations of traditional VAEs: non-straight decoding paths, separate encoder/decoder parameters, and the reconstruction-generation gap.

Method: Rectified Flow Auto Coder (RAC) uses flow timesteps for multi-step decoding, enables bidirectional inference (decoder serves as encoder via time reversal), and allows correction of latent variables along the path.

Result: RAC surpasses SOTA VAEs in both reconstruction and generation with ~70% lower computational cost and ~41% parameter reduction.

Conclusion: RAC provides a more efficient and effective alternative to VAEs with straight/correctable decoding, bidirectional inference, and improved generation quality.
...
### [44] [Towards Driver Behavior Understanding: Weakly-Supervised Risk Perception in Driving Scenes](https://arxiv.org/abs/2603.05926)
*Nakul Agarwal,Yi-Ting Chen,Behzad Dariush*

Main category: cs.CV

TL;DR: RAID dataset for driver risk perception research with 4,691 annotated video clips and a weakly supervised risk object identification framework that outperforms SOTA by 20-23%.


<details>
  <summary>Details</summary>
Motivation: Zero-collision mobility requires understanding driver risk perception, which is complex and shaped by driver responses to external stimuli and attentiveness of surrounding road users.

Method: Introduce RAID dataset with diverse traffic scenario annotations, and propose a weakly supervised risk object identification framework that models relationship between driver's intended maneuver and responses to identify risk sources.

Result: Method achieves 20.6% and 23.1% performance gains over prior state-of-the-art approaches on RAID and HDDS datasets respectively.

Conclusion: RAID dataset enables research on driver risk perception and contextual risk assessment, with proposed framework demonstrating significant improvements in risk object identification.
...
TL;DR: TAR-ViTPose is a temporal Vision Transformer for video-based 2D human pose estimation that aggregates temporal cues across frames to improve robustness and accuracy over single-frame methods.


<details>
  <summary>Details</summary>
Motivation: Existing ViT-based pose estimators process each frame independently, ignoring temporal coherence in videos, which leads to unstable predictions in challenging scenes with motion blur, occlusion, or defocus.

Method: Proposes TAR-ViTPose with two key components: 1) Joint-centric Temporal Aggregation (JTA) that uses learnable query tokens for each joint to selectively attend to corresponding regions in neighboring frames, and 2) Global Restoring Attention (GRA) to restore aggregated temporal features back into the current frame's token sequence while preserving global context.

Result: Achieves +2.3 mAP gain over single-frame baseline ViTPose on PoseTrack2017 benchmark, outperforms existing state-of-the-art video-based methods, and achieves higher runtime frame rate in real-world applications.

Conclusion: TAR-ViTPose effectively leverages temporal information in videos to enhance pose estimation robustness and accuracy while maintaining computational efficiency, demonstrating the value of temporal modeling for video-based human pose estimation.
...
### [46] [Facial Expression Recognition Using Residual Masking Network](https://arxiv.org/abs/2603.05937)
*Luan Pham,The Huynh Vu,Tuan Anh Tran*

Main category: cs.CV

TL;DR: A novel Residual Masking Network using attention mechanism with segmentation to refine CNN feature maps for facial expression recognition, achieving SOTA accuracy on FER2013 and VEMO datasets.


<details>
  <summary>Details</summary>
Motivation: To improve facial expression recognition (FER) performance by enhancing deep architectures with attention mechanisms, specifically focusing on making CNNs better at identifying relevant facial features for accurate expression classification.

Method: Proposes a Masking idea using a segmentation network to refine CNN feature maps. Combines Deep Residual Network with Unet-like architecture to create Residual Masking Network, enabling the network to focus on relevant facial information for correct decisions.

Result: Achieves state-of-the-art accuracy on both the public FER2013 dataset and a private VEMO dataset, demonstrating superior performance compared to existing methods.

Conclusion: The Residual Masking Network effectively improves facial expression recognition by leveraging attention mechanisms through segmentation-based feature refinement, establishing a new SOTA benchmark for FER tasks.
...
TL;DR: A feed-forward framework for generating triangle primitives from multi-view images, enabling simulation-ready 3D reconstruction without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Current 3D reconstruction methods like NeRF and 3DGS require time-consuming per-scene optimization, limiting real-time deployment. Feed-forward Gaussian splatting methods improve efficiency but lack explicit manifold geometry needed for direct simulation in robotics and graphics applications.

Method: Proposes a feed-forward framework that directly predicts continuous triangle surfaces from calibrated multi-view images. Uses a pixel-aligned triangle generation module and incorporates relative 3D point cloud supervision to enhance geometric learning stability and consistency.

Result: The method achieves efficient reconstruction while maintaining seamless compatibility with standard graphics and robotic simulators. Produces simulation-ready models in a single forward pass without per-scene optimization or post-processing.

Conclusion: The proposed framework addresses limitations of existing 3D reconstruction methods by providing efficient, simulation-ready triangle primitive generation that bridges the gap between neural rendering and practical robotics/graphics applications.
...
### [48] [Energy-Driven Adaptive Visual Token Pruning for Efficient Vision-Language Models](https://arxiv.org/abs/2603.05950)
*Jialuo He,Huangxun Chen*

Main category: cs.CV

TL;DR: E-AdaPrune: Energy-driven adaptive pruning for VLMs that dynamically allocates token budgets based on image information density using singular value spectrum analysis.


<details>
  <summary>Details</summary>
Motivation: Existing visual token reduction methods use fixed budgets for all images, ignoring the significant variation in image information density across different scenes.

Method: Energy-driven adaptive pruning framework that determines token budget from singular value spectrum of visual features. Preserves certain proportion of spectral energy, allocating more tokens to information-dense scenes while aggressively compressing redundant ones. Uses randomized SVD for efficiency.

Result: Evaluated on 9 benchmarks with 3 VLM backbones (LLaVA-1.5-7B, LLaVA-1.5-13B, LLaVA-NeXT-8B). Under matched average token budgets, achieves average improvement up to 0.6%, including +5.1% relative boost on MMVet reasoning task. Additional latency limited to 8ms per image.

Conclusion: E-AdaPrune provides effective adaptive token pruning for VLMs without additional learnable parameters, significantly improving performance on reasoning tasks while maintaining low computational overhead.
...
TL;DR: OD-RASE is a framework that detects accident-prone road structures and generates infrastructure improvement proposals using ontology and large visual language models to enhance autonomous driving safety.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving systems struggle with rare situations and complex road structures designed for humans. Safety improvements are typically reactive (after accidents), but autonomous systems need proactive risk mitigation. There's a need to connect accident analysis with infrastructure development.

Method: 1) Formalize ontology based on road traffic domain knowledge; 2) Generate infrastructure improvement proposals using large-scale visual language model (LVLM); 3) Use ontology-driven data filtering to enhance reliability; 4) Automatically annotate improvement proposals on pre-accident images to build dataset; 5) Develop OD-RASE model combining LVLM and diffusion model to produce both improvement proposals and generated images of improved roads.

Result: Ontology-driven data filtering enables highly accurate prediction of accident-causing road structures and corresponding improvement plans. The framework successfully connects accident analysis with infrastructure development proposals.

Conclusion: OD-RASE contributes to traffic environment safety and represents an important step toward broader adoption of autonomous driving systems by enabling proactive infrastructure improvements based on accident analysis.
...
### [50] [Skeleton-to-Image Encoding: Enabling Skeleton Representation Learning via Vision-Pretrained Models](https://arxiv.org/abs/2603.05963)
*Siyuan Yang,Jun Liu,Hao Cheng,Chong Wang,Shijian Lu,Hedvig Kjellstrom,Weisi Lin,Alex C. Kot*

Main category: cs.CV

TL;DR: S2I transforms 3D skeleton sequences into image-like data to enable vision-pretrained models for self-supervised skeleton representation learning, handling heterogeneous skeleton formats.


<details>
  <summary>Details</summary>
Motivation: Large-scale pretrained vision models can't be directly applied to 3D skeleton data due to format differences. There's also a need to incorporate skeleton data into multi-modal action recognition without adding model branches, and existing methods don't handle structural heterogeneity from diverse data sources.

Method: Skeleton-to-Image Encoding (S2I) transforms skeleton sequences into image-like data by partitioning and arranging joints based on body-part semantics, then resizing to standardized image dimensions.

Result: Extensive experiments on NTU-60, NTU-120, and PKU-MMD demonstrate effectiveness and generalizability for self-supervised skeleton representation learning, including challenging cross-format evaluation settings.

Conclusion: S2I provides a unified image-like format that enables vision-pretrained models for skeleton analysis, effectively transferring visual-domain knowledge while accommodating heterogeneous skeleton data.
...
TL;DR: ProCap reformulates change captioning from static image comparison to dynamic procedure modeling using a two-stage framework that learns latent procedural dynamics from keyframes and generates descriptions through learnable procedure queries.


<details>
  <summary>Details</summary>
Motivation: Existing change captioning methods only work on static image pairs, ignoring the rich temporal dynamics of how changes occur. Understanding not just what changed but how it changed requires modeling the change procedure dynamically.

Method: Two-stage framework: 1) Train procedure encoder to learn change dynamics from sparse keyframes via caption-conditioned masked reconstruction; 2) Integrate encoder with decoder using learnable procedure queries to infer latent procedure representations for caption generation, trained end-to-end.

Result: Experiments on three datasets demonstrate ProCap's effectiveness in generating change descriptions that capture procedural dynamics.

Conclusion: ProCap successfully shifts change modeling from static comparison to dynamic procedure modeling, enabling richer descriptions that capture not only what changed but how changes occurred through temporal dynamics.
...
TL;DR: SLER-IR is a unified image restoration framework using spherical layer-wise expert routing to dynamically activate specialized experts across network layers, achieving state-of-the-art performance on multi-task benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing unified all-in-one image restoration frameworks suffer from feature interference and insufficient expert specialization when handling diverse degradations, limiting their effectiveness across multiple restoration tasks.

Method: Proposes SLER-IR with three key components: 1) Spherical layer-wise expert routing that dynamically activates specialized experts across network layers, 2) Spherical Uniform Degradation Embedding with contrastive learning to map degradation representations onto a hypersphere and eliminate geometry bias, 3) Global-Local Granularity Fusion module to integrate global semantics and local degradation cues for spatially non-uniform degradations.

Result: Experiments on three-task and five-task benchmarks demonstrate consistent improvements over state-of-the-art methods in both PSNR and SSIM metrics.

Conclusion: SLER-IR effectively addresses feature interference and expert specialization issues in unified image restoration frameworks through spherical expert routing and granularity fusion, achieving superior performance across multiple degradation types.
...
TL;DR: A novel skew estimation method using Adaptive Radial Projection on Fourier spectrum, plus a new dataset DISE-2021 for evaluation, showing superior performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Skew estimation is crucial for document processing systems, especially scanned documents, as it directly impacts subsequent processing steps. With the rise of digitization, there's a need for robust and reliable skew estimation methods.

Method: Proposes a novel skew estimation method that extracts dominant skew angle by applying Adaptive Radial Projection on the 2D Discrete Fourier Magnitude spectrum of document images.

Result: The proposed method is robust, reliable, and outperforms all compared methods. A new high-quality dataset DISE-2021 is introduced for performance assessment.

Conclusion: The research presents an effective Fourier-based skew estimation approach with comprehensive analyses, provides a benchmark dataset, and demonstrates superior performance over existing methods.
...
TL;DR: LucidNFT is a multi-reward RL framework for flow-matching Real-ISR that addresses hallucination issues through degradation-robust faithfulness evaluation, decoupled advantage normalization, and large-scale real degradation datasets.


<details>
  <summary>Details</summary>
Motivation: Generative Real-ISR suffers from semantic/structural hallucination where outputs look sharp but are unfaithful to LR evidence. Current methods lack degradation-robust faithfulness signals, have optimization bottlenecks in rollout groups, and limited real degradation coverage.

Method: 1) LucidConsistency: degradation-robust semantic evaluator for LR-anchored faithfulness measurement; 2) Decoupled advantage normalization: preserves objective-wise contrasts within rollout groups before fusion; 3) LucidLR: large-scale real-world degraded image collection for robust RL fine-tuning.

Result: LucidNFT consistently improves strong flow-based Real-ISR baselines, achieving better perceptual-faithfulness trade-offs with stable optimization dynamics across diverse real-world scenarios.

Conclusion: The proposed framework effectively addresses key challenges in Real-ISR alignment by making faithfulness measurable, preventing advantage collapse, and providing comprehensive real degradation coverage for robust RL optimization.
...
TL;DR: MM-ISTS is a multimodal framework using vision-text LLMs for irregularly sampled time series forecasting, combining temporal, visual, and textual modalities through cross-modal encoding and adaptive feature extraction.


<details>
  <summary>Details</summary>
Motivation: Existing irregularly sampled time series (ISTS) forecasting methods only use historical observations and fail to capture contextual semantics and fine-grained temporal patterns, limiting their effectiveness in real-world applications.

Method: Two-stage encoding: 1) Cross-modal vision-text encoding generates informative images/text for temporal patterns with MLLMs, 2) ISTS encoding extracts enriched temporal features. Includes adaptive query-based feature extractor to compress MLLM tokens and multimodal alignment with modality-aware gating.
...
Conclusion: MM-ISTS successfully bridges temporal, visual, and textual modalities for ISTS forecasting, capturing intricate patterns and contextual understanding while reducing computational costs through adaptive feature extraction.
...
TL;DR: VINE is a few-shot segmentation framework that addresses structural misalignment and cross-view inconsistency by jointly modeling structural consistency and foreground discrimination to refine class-specific prototypes.


<details>
  <summary>Details</summary>
Motivation: Few-shot segmentation struggles with structural misalignment and cross-view inconsistency under large appearance or viewpoint variations, which limits generalization to novel classes with limited supervision.

Method: VINE introduces a spatial-view graph on backbone features (spatial graph for local geometry, view graph for cross-perspective connections), uses support-query feature discrepancy to derive discriminative prior for foreground enhancement, and progressively integrates SAM and ResNet features through masked cross-attention to generate class-consistent prototypes as adaptive prompts for SAM decoder.

Result: Extensive experiments on multiple FSS benchmarks validate VINE's effectiveness and robustness, particularly under challenging scenarios with viewpoint shifts and complex structures.

Conclusion: VINE provides a unified framework that successfully addresses structural consistency and foreground discrimination challenges in few-shot segmentation, demonstrating strong performance across various benchmarks.
...
TL;DR: RepKAN is a novel interpretable architecture for remote sensing image classification that combines CNNs' structural efficiency with KANs' non-linear power, enabling discovery of class-specific spectral fingerprints and physical interaction manifolds while outperforming SOTA models.


<details>
  <summary>Details</summary>
Motivation: Standard CNNs and Transformers for remote sensing image classification function as uninterpretable black-boxes, lacking transparency in decision-making processes essential for Earth observation applications.

Method: RepKAN integrates CNN structural efficiency with KANs' non-linear representational power using a dual-path design: Spatial Linear path and Spectral Non-linear path, enabling autonomous discovery of class-specific spectral fingerprints and physical interaction manifolds.

Result: Experimental results on EuroSAT and NWPU-RESISC45 datasets show RepKAN provides explicit physically interpretable reasoning while outperforming state-of-the-art models.

Conclusion: RepKAN holds significant potential to serve as the backbone for future interpretable visual foundation models in remote sensing and Earth observation.
...
### [58] [OVGGT: O(1) Constant-Cost Streaming Visual Geometry Transformer](https://arxiv.org/abs/2603.05959)
*Si-Yu Lu,Po-Ting Chen,Hui-Che Hsu,Sin-Ye Jhong,Wen-Huang Cheng,Yung-Yao Chen*

Main category: cs.CV

TL;DR: OVGGT enables streaming 3D reconstruction from arbitrarily long videos with constant memory/compute budget via selective KV cache compression and anchor protection.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D reconstruction from video either use quadratic-cost attention (limited to short sequences) or causal attention with unbounded memory growth (exhausts GPU within hundreds of frames). Need a solution that can handle long-horizon streaming inference with fixed resource budget.

Method: Training-free framework combining: 1) Self-Selective Caching - uses FFN residual magnitudes to compress KV cache while maintaining FlashAttention compatibility; 2) Dynamic Anchor Protection - shields coordinate-critical tokens from eviction to prevent geometric drift over long sequences.

Result: OVGGT processes arbitrarily long videos within constant VRAM envelope while achieving state-of-the-art 3D geometric accuracy on indoor, outdoor, and ultra-long sequence benchmarks.

Conclusion: OVGGT enables practical long-horizon streaming 3D reconstruction by bounding both memory and compute to fixed budgets, overcoming limitations of previous methods that either had quadratic complexity or unbounded memory growth.
...
TL;DR: VLMs fail on simple driving-relevant visual questions; study examines linear encoding of visual concepts in activations to identify perceptual vs. cognitive failures.


<details>
  <summary>Details</summary>
Motivation: VLMs are increasingly used in automated driving but fail on simple visual questions relevant to driving. The reasons for these failures are poorly understood, motivating investigation into how visual information flows through these models.

Method: Create counterfactual image sets differing only in targeted visual concepts. Train linear probes on activations of four SOTA VLMs to assess linear encoding of concepts. Analyze when concepts are explicitly vs. implicitly encoded.

Result: Object presence is explicitly linearly encoded, but spatial concepts like orientation are only implicitly encoded via vision encoder's spatial structure. Two failure modes identified: perceptual (visual info not linearly encoded) and cognitive (info present but misaligned with language). Object distance degrades linear separability.

Conclusion: Study improves understanding of VLM failures on simple driving-relevant tasks by analyzing linear encoding of visual concepts, identifying perceptual vs. cognitive failure modes, and showing degradation with object distance.
...
### [60] [Exploring Open-Vocabulary Object Recognition in Images using CLIP](https://arxiv.org/abs/2603.05962)
*Wei Yu Chen,Ying Dai*

Main category: cs.CV

TL;DR: Proposes a two-stage open-vocabulary object recognition framework using segmentation then recognition, with CLIP-based encoding and CNN/MLP alternatives, achieving SOTA performance without training.


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing open-vocabulary methods: high system complexity, substantial training costs, and limited generalization capabilities.

Method: Two-stage strategy: object segmentation followed by recognition. Uses CLIP for object-level image and category-level text embeddings. Also introduces CNN/MLP-based encoding to reduce CLIP reliance. Concatenates embeddings and processes via SVD for shared representation space. Recognition via embedding similarity matching.

Result: Training-free CLIP-based encoding without SVD achieves highest average AP on COCO, Pascal VOC, and ADE20K, outperforming current state-of-the-art methods. CNN/MLP-based encoding shows potential for OVOR.

Conclusion: Proposed framework eliminates complex retraining and labor-intensive annotation while achieving superior performance, demonstrating effectiveness of streamlined two-stage approach for open-vocabulary object recognition.
...
TL;DR: TempoSyncDiff is a reference-conditioned latent diffusion framework for efficient audio-driven talking-head generation that uses teacher-student distillation to reduce inference steps while maintaining quality, with identity anchoring and temporal regularization to address flicker and identity drift.
...
Method: The approach uses a teacher-student distillation framework where a diffusion teacher trained with standard noise prediction guides a lightweight student denoiser for few-step inference. It incorporates identity anchoring and temporal regularization to mitigate identity drift and flicker, and uses viseme-based audio conditioning for coarse lip motion control.

Result: Experiments on LRS3 dataset show that distilled diffusion models retain much of the reconstruction behavior of the stronger teacher while enabling substantially lower latency inference, with preliminary measurements showing feasibility for edge deployment.

Conclusion: The study demonstrates that distilled diffusion models can achieve practical talking-head generation under constrained computational settings, representing an initial step toward efficient diffusion-based synthesis for edge deployment.
...
TL;DR: Proposes Emotionally Continuous Talking Face Generation (EC-TFG) task and TIE-TFG model to generate talking faces with smooth, natural emotional transitions rather than fixed emotions.


<details>
  <summary>Details</summary>
Motivation: Previous TFG methods create faces with fixed target emotions but lack ability to show continuously changing, natural emotional expressions like humans do when conveying information.

Method: Introduces Temporal-Intensive Emotion Fluctuation Modeling in TIE-TFG model to manage dynamic emotional variations, generating emotion variation sequences that drive continuous facial expression changes.
...
Conclusion: The proposed EC-TFG task and TIE-TFG model successfully address the limitation of fixed emotional expressions in TFG, enabling realistic, emotionally continuous talking face generation.
...
TL;DR: CR-QAT framework combines curriculum quantization-aware training with relational knowledge distillation to preserve vision-language alignment in extreme low-bit (4-bit) open-vocabulary object detection models.


<details>
  <summary>Details</summary>
Motivation: Open-vocabulary object detection models are too large for resource-constrained devices, but naive extreme low-bit quantization severely degrades vision-language alignment and relational structures.

Method: CR-QAT integrates curriculum QAT (progressive quantization with error isolation) and text-centric relational KD (transferring teacher's multi-dimensional relational knowledge via text-anchored similarity matrices).

Result: Outperforms existing QAT baselines on LVIS and COCO zero-shot benchmarks with relative AP improvements up to 38.9% and 40.9% under aggressive low-bit settings.

Conclusion: CR-QAT effectively preserves vision-language alignment in quantized OVOD models, enabling deployment on resource-constrained devices without sacrificing detection performance.
...
TL;DR: DynUAV is a new benchmark for dynamic UAV-perspective multi-object tracking featuring intense ego-motion, complex trajectories, and challenging conditions like scale/viewpoint changes and motion blur.


<details>
  <summary>Details</summary>
Motivation: Existing UAV-perspective MOT benchmarks lack the complexity of real-world scenarios with rapid UAV movements and agile maneuvers, featuring mostly predictable camera dynamics and linear motion patterns.

Method: Created DynUAV benchmark with 42 video sequences containing over 1.7 million bounding box annotations covering vehicles, pedestrians, and industrial categories (excavators, bulldozers, cranes).

Result: Comprehensive evaluations show state-of-the-art trackers have limitations in managing detection and association challenges under dynamic conditions with ego-motion, scale/viewpoint changes, and motion blur.

Conclusion: DynUAV establishes a rigorous benchmark that will serve as a demanding testbed to spur progress in real-world UAV-perspective MOT, with all resources made publicly available.
...
TL;DR: A novel sketch colorization framework that directly addresses distribution shift between training and test data using a dual-branch approach with Gram Regularization Loss, achieving state-of-the-art quality and controllability.


<details>
  <summary>Details</summary>
Motivation: Previous sketch colorization methods only mitigated artifacts from distribution shift between semantically aligned training data and diverse test data, rather than fundamentally solving the problem. The authors aim to directly minimize this distribution shift for better quality, resolution, and controllability.

Method: Proposes a dual-branch framework with semantic-aligned and semantic-misaligned branches to model training and inference distributions. Uses Gram Regularization Loss across feature maps for cross-domain coherence. Incorporates anime-specific Tagger Network for fine-grained attribution extraction and SDXL conditional encoder modulation, plus a plugin module for texture transfer enhancement.

Result: Quantitative and qualitative comparisons with user studies confirm the method effectively overcomes distribution shift challenge, establishing State-of-the-Art performance in both quality and controllability metrics. Ablation studies validate each component's contribution.

Conclusion: The framework successfully minimizes distribution shift in sketch colorization, achieving superior results through explicit modeling of training/inference distributions, cross-domain regularization, and precise control mechanisms, representing a fundamental improvement over previous approaches.
...
TL;DR: Place-it-R1 is an MLLM-guided framework for physically plausible video object insertion using Chain-of-Thought reasoning and closed-loop refinement.


<details>
  <summary>Details</summary>
Motivation: Current video editing techniques focus on visual fidelity but ignore physical causality, resulting in physically inconsistent object insertions that don't interact properly with their environment.

Method: Uses MLLM Chain-of-Thought reasoning to orchestrate video diffusion in a Think-then-Place paradigm. Key innovations: 1) MLLM performs physical scene understanding and generates environment-aware tokens/insertion regions, 2) MLLM-guided Spatial DPO for visual naturalness scoring, 3) Closed-loop refinement with iterative MLLM-triggered adjustments. Offers two modes: plausibility-oriented (allows environment modifications) and fidelity-oriented (preserves scene integrity).

Result: Extensive experiments show Place-it-R1 achieves physically-coherent video object insertion superior to state-of-the-art solutions and commercial models.

Conclusion: The framework successfully bridges cognitive reasoning and generative execution for physically plausible video editing, offering users control over the plausibility-fidelity trade-off through dual operating modes.
...
TL;DR: VLM-RobustBench evaluates vision-language models under 49 real-world image distortions across 133 settings, revealing VLMs are semantically strong but spatially fragile, with geometric distortions causing largest performance drops.


<details>
  <summary>Details</summary>
Motivation: While VLMs perform well on standard datasets, their robustness to real-world image distortions is not well understood. The paper aims to systematically evaluate how VLMs handle various types of image corruptions encountered in practical applications.

Method: Created VLM-RobustBench with 49 augmentation types across noise, blur, weather, digital, and geometric perturbations, evaluated under graded severities (low/mid/high) and binary transforms (133 settings total). Evaluated VLMs from four families (Qwen, InternVL, Molmo, Gemma) on MMBench (visually grounded) and MMMU-Pro (reasoning-oriented) benchmarks.

Result: Visual severity is weak predictor of difficulty - low-severity spatial perturbations degrade performance more than visually severe photometric corruptions. Low-severity glass_blur reduces MMBench accuracy by ~8pp average. Largest drops from resampling and geometric distortions (upsample, elastic_transform) up to 34pp. Current VLMs are semantically strong but spatially fragile.

Conclusion: VLMs need improved robustness to spatial distortions. Findings motivate novel robustness evaluation protocols and training regimes emphasizing resampling and geometric invariances for more robust vision-language models.
...
TL;DR: RePer-360 is a distortion-aware self-modulation framework that adapts depth foundation models to panoramic images while preserving pretrained perspective priors, achieving strong performance with minimal training data.


<details>
  <summary>Details</summary>
Motivation: Depth foundation models trained on perspective images generalize poorly to 360° panoramic images due to geometric discrepancies, and full fine-tuning requires large amounts of panoramic data.

Method: Uses a lightweight geometry-aligned guidance module with ERP and CP projections to generate modulation signals, Self-Conditioned AdaLN-Zero for pixel-wise scaling to reduce feature distribution gaps, and cubemap-domain consistency loss for training stability.

Result: Surpasses standard fine-tuning methods using only 1% of training data, achieves ~20% RMSE improvement under in-domain training settings, and effectively adapts to panoramic domain while preserving perspective priors.

Conclusion: RePer-360 successfully adapts depth foundation models to panoramic depth estimation with minimal data requirements by focusing on domain adaptation while preserving pretrained knowledge, offering an efficient solution for 360° depth estimation.
...
### [69] [Reflective Flow Sampling Enhancement](https://arxiv.org/abs/2603.06165)
*Zikai Zhou,Muyao Wang,Shitong Shao,Lichen Bai,Haoyi Xiong,Bo Han,Zeke Xie*

Main category: cs.CV

TL;DR: RF-Sampling is a training-free inference enhancement framework for flow-based text-to-image models (like FLUX) that improves generation quality and prompt alignment through reflective sampling and flow inversion.


<details>
  <summary>Details</summary>
Motivation: Existing inference-time enhancement strategies work well for conventional diffusion models but fail on flow models, creating a gap that needs to be addressed for flow-based text-to-image generation systems.

Method: RF-Sampling uses a theoretically-grounded approach with formal derivation showing it performs gradient ascent on text-image alignment score. It combines textual representations with flow inversion to explore noise spaces more consistent with input prompts.

Result: Extensive experiments show RF-Sampling consistently improves generation quality and prompt alignment across multiple benchmarks, and is the first inference enhancement method to exhibit test-time scaling ability on FLUX.

Conclusion: RF-Sampling successfully bridges the gap for inference enhancement in flow models, providing a training-free solution that improves performance and shows promising scaling properties.
...
TL;DR: EffectMaker is a unified reasoning-generation framework for reference-based visual effects customization that uses multimodal LLM reasoning and diffusion transformers for in-context learning, eliminating per-effect fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current VFX production requires expert knowledge and costly pipelines, while existing AIGC systems struggle with VFX generation due to data scarcity, difficulty modeling supernatural effects, and reliance on per-effect fine-tuning that limits scalability.

Method: A dual-path framework: 1) multimodal LLM interprets high-level effect semantics and reasons about adaptation to target subjects, 2) diffusion transformer uses in-context learning to capture fine-grained visual cues from reference videos, creating semantic-visual guidance without per-effect fine-tuning.

Result: EffectMaker achieves superior visual quality and effect consistency over SOTA baselines, enabled by EffectData - a 130k video dataset across 3k VFX categories that improves generalization and scalability.

Conclusion: EffectMaker offers a scalable, flexible paradigm for customized VFX generation that eliminates the need for per-effect fine-tuning while maintaining accurate, controllable, and effect-consistent synthesis.
...
### [71] [Contrastive-to-Self-Supervised: A Two-Stage Framework for Script Similarity Learning](https://arxiv.org/abs/2603.06180)
*Claire Roman,Philippe Meyer*

Main category: cs.CV

TL;DR: Two-stage framework for learning glyph similarities: supervised contrastive learning on invented alphabets, then unsupervised teacher-student distillation on historical scripts to discover cross-script relationships without requiring ground-truth evolutionary labels.


<details>
  <summary>Details</summary>
Motivation: Learning similarity metrics for glyphs and writing systems is challenging because while individual graphemes in invented alphabets can be reliably labeled, historical relationships between different scripts remain uncertain and contested. There's a need to bridge supervised learning with unsupervised discovery of potential historical influences.

Method: Two-stage framework: 1) Train encoder with contrastive loss on labeled invented alphabets to create teacher model with robust discriminative features. 2) Extend to historically attested scripts through teacher-student distillation, where student learns unsupervised representations guided by teacher's knowledge but free to discover latent cross-script similarities. Asymmetric setup enables deformation-invariant embeddings while inheriting discriminative structure.

Result: Experiments on diverse writing systems demonstrate effective few-shot glyph recognition and meaningful script clustering without requiring ground-truth evolutionary relationships. The approach enables both hard boundaries between distinct systems and soft similarities reflecting potential historical influences.

Conclusion: The proposed framework successfully bridges supervised contrastive learning and unsupervised discovery, addressing the epistemological constraint of uncertain historical relationships between scripts while enabling meaningful similarity learning and clustering of writing systems.
...
### [72] [MOSIV: Multi-Object System Identification from Videos](https://arxiv.org/abs/2603.06022)
...
TL;DR: MOSIV is a new framework for multi-object system identification from videos that optimizes continuous per-object material parameters using differentiable simulation guided by geometric objectives, outperforming adapted baselines on a new synthetic benchmark.


<details>
  <summary>Details</summary>
Motivation: Prior methods are ill-suited for multi-object system identification as they focus on single-object scenes or discrete material classification with fixed material prototypes, creating a gap for handling complex multi-object interactions.

Method: MOSIV directly optimizes continuous, per-object material parameters using a differentiable simulator guided by geometric objectives derived from video data, enabling fine-grained material estimation in multi-object settings.

Result: On a new synthetic benchmark with contact-rich, multi-object interactions, MOSIV substantially improves grounding accuracy and long-horizon simulation fidelity over adapted baselines, establishing it as a strong baseline for this new task.

Conclusion: Object-level fine-grained supervision and geometry-aligned objectives are critical for stable optimization in complex multi-object settings, and MOSIV provides an effective framework for multi-object system identification from videos.
...
TL;DR: Curious-VLA addresses the narrow policy limitation in autonomous VLA models by introducing a two-stage framework that enhances exploration diversity in IL and improves RL feedback sensitivity, achieving state-of-the-art results on Navsim benchmark.


<details>
  <summary>Details</summary>
Motivation: The paper identifies a fundamental limitation in autonomous VLA models where driving Imitation Learning tends to collapse exploration, limiting subsequent Reinforcement Learning stages that saturate prematurely due to insufficient feedback diversity.

Method: Two-stage framework: 1) IL stage with Feasible Trajectory Expansion (FTE) to generate multiple physically valid trajectories and step-wise normalized trajectory representation; 2) RL stage with Adaptive Diversity-Aware Sampling (ADAS) prioritizing high-diversity samples and Spanning Driving Reward (SDR) with focal style weighting to amplify reward value span.

Result: Achieves state-of-the-art results on Navsim benchmark: PDMS 90.3, EPDMS 85.4, and Best-of-N PDMS of 94.8, demonstrating effectiveness in unlocking exploratory potential of VLA models.

Conclusion: Curious-VLA successfully addresses the exploit-explore dilemma in autonomous VLA models through its two-stage design, enhancing exploration diversity and improving RL feedback sensitivity, leading to superior performance on driving benchmarks.
...
TL;DR: CoE is a training-free multimodal summarization framework that uses Chain-of-Events reasoning guided by a Hierarchical Event Graph to address challenges in cross-modal grounding and temporal modeling.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal summarization approaches suffer from three main challenges: reliance on domain-specific supervision, implicit fusion with weak cross-modal grounding, and flat temporal modeling without event transitions.

Method: CoE performs structured reasoning through a Chain-of-Events guided by a Hierarchical Event Graph (HEG). The HEG encodes textual semantics into an explicit event hierarchy that scaffolds cross-modal grounding and temporal reasoning. The framework localizes key visual cues, models event evolution and causal transitions, and refines outputs via lightweight style adaptation.

Result: CoE consistently outperforms state-of-the-art video CoT baselines across eight diverse datasets, achieving average gains of +3.04 ROUGE, +9.51 CIDEr, and +1.88 BERTScore.

Conclusion: CoE demonstrates robustness, interpretability, and cross-domain generalization in multimodal summarization through its training-free, structured reasoning approach with explicit event hierarchy modeling.
...
TL;DR: StruVis is a novel framework that enhances text-to-image generation by using text-based structured visual representations instead of intermediate image generation, enabling more efficient and effective reasoning without computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning frameworks for text-to-image generation have limitations: text-only reasoning lacks visual context and misses spatial elements, while text-image interleaved reasoning is computationally expensive and constrained by generator limitations. There's a need for a more efficient approach that maintains visual grounding without computational overhead.

Method: StruVis employs text-based structured visual representations as intermediate reasoning states, allowing MLLMs to "perceive" visual structure within a purely text-based reasoning process. It's a generator-agnostic framework that uses structured-vision-guided reasoning to unlock MLLM's reasoning potential for T2I generation.

Result: Extensive experiments show significant performance improvements: 4.61% gain on T2I-ReasonBench and 4% gain on WISE benchmarks. The framework can be seamlessly integrated with diverse T2I generators to efficiently enhance their performance.

Conclusion: StruVis provides an effective solution to the limitations of existing reasoning frameworks by using structured visual representations in text form, achieving better performance while being computationally efficient and generator-agnostic.
...
### [76] [Transforming Omnidirectional RGB-LiDAR data into 3D Gaussian Splatting](https://arxiv.org/abs/2603.06061)
*Semin Bae,Hansol Lim,Jongseong Brad Choi*

Main category: cs.CV

TL;DR: A pipeline to repurpose archived omnidirectional RGB-LiDAR logs into 3D Gaussian Splatting initialization assets for digital twins, overcoming distortion and computational bottlenecks.


<details>
  <summary>Details</summary>
Motivation: Large-scale digital twins require expensive data collection, while deployed platforms routinely collect extensive RGB-LiDAR logs that are discarded due to transmission constraints and lack of scalable reuse pipelines.

Method: Omnidirectional RGB-LiDAR reuse pipeline with ERP-to-cubemap conversion for spatial anchoring, PRISM color stratified downsampling, FPFH-based global registration, and ICP to bridge multi-modal inputs.

Result: Pipeline successfully repurposes discarded data into usable SfM geometry and enhances 3DGS rendering fidelity in complex scenes compared to vision-only baselines.

Conclusion: Provides a deterministic workflow for creating simulation-grade digital twins from standard archived sensor logs, enabling scalable reuse of underutilized data.
...
TL;DR: TaPD is a plug-and-play framework for trajectory prediction that handles variable observation lengths through progressive knowledge distillation and temporal backfilling.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory predictors assume fixed-length histories and degrade with variable or extremely short observations in real-world scenarios (occlusions, limited sensing range).

Method: Two cooperative modules: 1) Observation-Adaptive Forecaster (OAF) using progressive knowledge distillation to transfer motion patterns from long to short observations, 2) Temporal Backfilling Module (TBM) that reconstructs missing historical segments. Uses cosine-annealed distillation weighting and decoupled pretrain-reconstruct-finetune protocol.

Result: Outperforms strong baselines across all observation lengths on Argoverse 1 and 2, especially large gains with very short inputs. Works as plug-and-play enhancement for other predictors like HiVT.

Conclusion: TaPD provides a unified solution for observation-adaptive trajectory forecasting that handles variable history lengths effectively, improving real-world applicability where observations are often incomplete.
...
### [78] [Occlusion-Aware SORT: Observing Occlusion for Robust Multi-Object Tracking](https://arxiv.org/abs/2603.06034)
*Chunjiang Li,Jianbo Ma,Li Shen,Yanru Chen,Liangyin Chen*

Main category: cs.CV

TL;DR: OA-SORT is a plug-and-play, training-free multi-object tracking framework that addresses occlusion challenges through three novel components: Occlusion-Aware Module, Occlusion-Aware Offset, and Bias-Aware Momentum.


<details>
  <summary>Details</summary>
Motivation: 2D multi-object tracking faces challenges from positional cost confusion caused by partial occlusion, which degrades tracking performance and object counting accuracy in video sequences.

Method: Proposes OA-SORT framework with three components: 1) Occlusion-Aware Module (OAM) analyzes occlusion status using Gaussian Map to reduce background influence, 2) Occlusion-Aware Offset (OAO) mitigates cost confusion, and 3) Bias-Aware Momentum (BAM) suppresses estimation instability.

Result: Achieves 63.1% HOTA and 64.2% IDF1 on DanceTrack test set. When integrated into four additional trackers, improves HOTA and IDF1 by average of 2.08% and 3.05% respectively, demonstrating strong reusability.

Conclusion: The occlusion-aware framework effectively addresses occlusion challenges in MOT, showing significant performance improvements and demonstrating the importance of occlusion handling through plug-and-play, training-free components.
...
TL;DR: VG3S improves 3D semantic occupancy prediction by injecting geometric priors from Vision Foundation Models into Gaussian splatting, achieving significant performance gains on nuScenes benchmark.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian splatting for occupancy prediction relies on accurate geometric cues that are insufficient in vision-only systems. There's a need to leverage the strong geometric grounding capabilities of Vision Foundation Models to improve occupancy prediction quality.

Method: Proposes Visual Geometry Grounded Gaussian Splatting (VG3S) framework with a plug-and-play hierarchical geometric feature adapter. This adapter transforms generic VFM tokens through feature aggregation, task-specific alignment, and multi-scale restructuring to inject 3D geometric priors into Gaussian-based occupancy prediction.

Result: VG3S achieves remarkable improvements of 12.6% in IoU and 7.5% in mIoU over baseline on nuScenes occupancy benchmark. The framework generalizes well across diverse VFMs, consistently enhancing occupancy prediction accuracy.

Conclusion: Integrating priors from pre-trained geometry-grounded Vision Foundation Models provides immense value for 3D semantic occupancy prediction, demonstrating the effectiveness of VG3S framework in bridging the geometric gap in vision-centric paradigms.
...
### [80] [GazeMoE: Perception of Gaze Target with Mixture-of-Experts](https://arxiv.org/abs/2603.06256)
*Zhuangzhuang Dai,Zhongxi Lu,Vincent G. Zakka,Luis J. Manso,Jose M Alcaraz Calero,Chen Li*

Main category: cs.CV

TL;DR: GazeMoE: A novel end-to-end framework using Mixture-of-Experts modules to selectively leverage gaze-target-related cues from frozen foundation models, achieving state-of-the-art performance on gaze estimation tasks.


<details>
  <summary>Details</summary>
Motivation: Estimating human gaze targets from visible images is critical for robots to understand human attention, but developing generalizable neural architectures and training paradigms remains challenging. While pre-trained vision foundation models offer promising avenues, integrating multi-modal cues (eyes, head poses, gestures, context) requires adaptive and efficient decoding mechanisms.

Method: Proposes GazeMoE, an end-to-end framework inspired by Mixture-of-Experts (MoE) that selectively leverages gaze-target-related cues from a frozen foundation model through MoE modules. Incorporates class-balancing auxiliary loss to address class imbalance (in-frame vs. out-of-frame) and uses strategic data augmentations including region-specific cropping and photometric transformations.
...
Conclusion: GazeMoE effectively addresses gaze estimation challenges by adaptively integrating multi-modal cues through MoE modules while handling class imbalance and enhancing robustness through strategic techniques. The framework shows superior performance and the code/models are publicly available.
...
### [81] [Ensemble Learning with Sparse Hypercolumns](https://arxiv.org/abs/2603.06036)
*Julia Dietlmeier,Vayangi Ganepola,Oluwabukola G. Adegboro,Mayug Maniparambil,Claudia Mazo,Noel E. O'Connor*

Main category: cs.CV

TL;DR: Hypercolumns with stratified subsampling and ensemble learning achieve 24.53% improvement over UNet baseline for brain tumor segmentation, especially effective in low-shot learning scenarios (N≤20).


<details>
  <summary>Details</summary>
Motivation: Hypercolumns are biologically-inspired feature vectors for pixel classification, but their computational complexity grows linearly with training set size, limiting practical use. The paper aims to address this computational challenge.

Method: Applied stratified subsampling to VGG16-based hypercolumns to reduce computational complexity. Investigated ensemble learning (stacking and voting) on sparse hypercolumns. Compared with Logistic Regression for extreme low-shot cases.

Result: For 10% stratified subsampling rate with N=20, best average Dice score is 0.66. This represents a statistically significant 24.53% improvement over standard multi-scale UNet baseline (p-value = 3.07e-11). In extreme low-shot cases (N≤20), simple Logistic Regression outperforms ensemble methods.

Conclusion: Stratified subsampling of hypercolumns combined with appropriate classifier selection (ensembles for moderate data, Logistic Regression for extreme low-shot) provides effective brain tumor segmentation with significant improvements over baseline methods while addressing computational complexity.
...
### [82] [FontUse: A Data-Centric Approach to Style- and Use-Case-Conditioned In-Image Typography](https://arxiv.org/abs/2603.06038)
*Xia Xin,Yuki Endo,Yoshihiro Kanamori*

Main category: cs.CV

TL;DR: The paper introduces FontUse, a data-centric approach to improve typography control in text-to-image models by creating a large-scale typography-focused dataset with structured annotations and fine-tuning existing generators.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with controlling typography - requested typographic appearance is often ignored or weakly followed, limiting their practical applications where precise typography matters.

Method: 1) Created FontUse dataset (~70K images) with automated annotations using segmentation models and MLLMs; 2) Structured prompts combining font styles and use cases; 3) Fine-tuned existing image generators with these annotations; 4) Introduced Long-CLIP-based metric for evaluation.

Result: Models trained with FontUse pipeline produce text renderings more consistent with prompts than competitive baselines across diverse prompts and layouts, achieving better typography control without architectural modifications.

Conclusion: The data-centric approach with structured typography annotations enables text-to-image models to better interpret style and use-case conditions, providing intuitive typography control for users while maintaining model architecture compatibility.
...
TL;DR: HiPP-Prune is a hierarchical preference-conditioned pruning framework for vision-language models that treats pruning as conditional resource allocation, enabling controllable trade-offs between task utility, hallucination robustness, and compression.


<details>
  <summary>Details</summary>
Motivation: Pruning VLMs is challenging because compression affects both task utility and visual grounding, often amplifying object hallucinations even at the same sparsity level. Current methods don't adequately address VLM-specific failure modes like hallucination amplification.

Method: Hierarchical preference-conditioned structured pruning framework that makes plan-level decisions: a single policy outputs global pruning blueprint by factorizing decisions into overall sparsity budget and layer-wise allocation. Integrates visual sensitivity signal from attention flow between vision tokens and language hidden states. Optimizes with plan-level Group Relative Policy Optimization (GRPO) under multi-objective return combining task utility, hallucination robustness (POPE), compression, and synaptic-flow-inspired stability proxy.

Result: Experiments on LLaVA with POPE and ScienceQA show that HiPP-Prune discovers diverse non-dominated pruning plans and provides controllable robustness-utility trade-offs under matched sparsity budgets.

Conclusion: HiPP-Prune effectively addresses VLM-specific pruning challenges by enabling preference-conditioned resource allocation that balances multiple objectives including hallucination mitigation, offering a framework for controllable compression of vision-language models.
...
### [84] [Learning to Generate via Understanding: Understanding-Driven Intrinsic Rewarding for Unified Multimodal Models](https://arxiv.org/abs/2603.06043)
*Jiadong Pan,Liang Li,Yuxin Peng,Yu-Ming Tang,Shuohuan Wang,Yu Sun,Hua Wu,Qingming Huang,Haifeng Wang*

Main category: cs.CV

TL;DR: UMMs have strong visual understanding but weak generation. Proposed GvU uses understanding branch as intrinsic reward to guide generation via self-supervised RL, closing the capability gap.


<details>
  <summary>Details</summary>
Motivation: Unified multimodal models show promise but have a persistent capability gap: strong visual understanding but comparatively weak generative capabilities. This decoupling between understanding and generation processes limits their effectiveness in complex text-to-image tasks.

Method: Proposed GvU - a token-level intrinsic text-image alignment reward mechanism that enables UMMs to act as both teacher and student. Uses understanding branch to evaluate outputs and guide generation. Built a self-supervised reinforcement learning framework for iterative improvement without external supervision.

Result: Method substantially boosts UMMs' generation capabilities, which in turn strengthens their fine-grained visual understanding. Successfully narrows the capability gap between visual understanding and generation in unified multimodal models.

Conclusion: Leveraging UMMs' internal understanding capability through intrinsic reward mechanisms and self-supervised RL effectively bridges the understanding-generation gap, enhancing both capabilities simultaneously without external supervision.
...
### [85] [GenHOI: Towards Object-Consistent Hand-Object Interaction with Temporally Balanced and Spatially Selective Object Injection](https://arxiv.org/abs/2603.06048)
...
TL;DR: GenHOI is a lightweight augmentation for pretrained video generation models that improves hand-object interaction synthesis by injecting reference-object information with temporal balancing and spatial selectivity.


<details>
  <summary>Details</summary>
Motivation: Current HOI reenactment methods fail to generalize to complex in-the-wild scenarios, while all-in-one video editing models struggle with HOI-specific issues like inconsistent object appearance across frames.

Method: Two key components: 1) Head-Sliding RoPE for temporal balancing - assigns head-specific temporal offsets to reference tokens to distribute influence evenly across frames and mitigate 3D RoPE temporal decay; 2) Two-level spatial attention gate for spatial selectivity - concentrates object-conditioned attention on HOI regions and adaptively scales its strength.

Result: Extensive evaluations on unseen, in-the-wild scenes show GenHOI significantly outperforms state-of-the-art HOI reenactment and all-in-one video editing methods in both qualitative and quantitative assessments.

Conclusion: GenHOI effectively addresses HOI challenges in video synthesis by combining temporal balancing and spatial selectivity, enabling robust generalization to complex real-world scenarios while maintaining background realism and interaction fidelity.
...
TL;DR: A spatial calibration method for diffuse direct time-of-flight LiDARs that estimates per-pixel footprints and spatial sensitivity in RGB images, enabling cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Diffuse direct time-of-flight LiDARs violate the single-ray assumption of standard LiDAR-RGB calibration because they aggregate photon returns over wide fields of view, creating a mismatch between LiDAR pixels and corresponding RGB image regions.

Method: Using a scanned retroreflective patch with background subtraction to recover per-pixel response maps that capture each LiDAR pixel's footprint (effective support region) and relative spatial sensitivity in the co-located RGB image plane.

Result: Demonstrated the calibration method on the ams OSRAM TMF8828 LiDAR, successfully recovering explicit LiDAR-to-RGB correspondence for cross-modal alignment and fusion.

Conclusion: The proposed spatial calibration procedure enables accurate alignment between diffuse LiDAR and RGB data by explicitly modeling the spatial characteristics of each LiDAR pixel, overcoming limitations of traditional single-ray calibration assumptions.
...
TL;DR: BEVLM connects spatially consistent BEV representations with LLMs to improve autonomous driving reasoning by 46% accuracy and end-to-end driving by 29%.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based autonomous driving methods process multi-view images independently, causing redundant computation and poor spatial consistency. BEV representations provide spatial structure but lack semantic richness from foundation vision encoders.
...
Result: BEVLM improves LLM reasoning accuracy in cross-view driving scenes by 46% and enhances closed-loop end-to-end driving performance by 29% in safety-critical scenarios.

Conclusion: BEVLM effectively bridges the gap between spatial consistency and semantic richness, enabling more effective LLM reasoning for autonomous driving while improving overall driving performance.
...
### [88] [DEX-AR: A Dynamic Explainability Method for Autoregressive Vision-Language Models](https://arxiv.org/abs/2603.06302)
*Walid Bousselham,Angie Boggust,Hendrik Strobelt,Hilde Kuehne*

Main category: cs.CV

TL;DR: DEX-AR is a new explainability method for autoregressive vision-language models that generates 2D heatmaps showing important image regions for each token and overall sequence, addressing challenges of traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional explainability methods designed for classification tasks struggle with modern autoregressive VLMs due to their complex token-by-token generation process and intricate visual-textual modality interactions.

Method: DEX-AR computes layer-wise gradients with respect to attention maps during token generation, using dynamic head filtering to identify attention heads focused on visual information, and sequence-level filtering to aggregate per-token explanations while distinguishing visually-grounded from linguistic tokens.

Result: Evaluation on ImageNet, VQAv2, and PascalVOC shows consistent improvement in both perturbation-based metrics (using novel normalized perplexity measure) and segmentation-based metrics.

Conclusion: DEX-AR effectively addresses explainability challenges for autoregressive VLMs by providing both per-token and sequence-level visual explanations, outperforming traditional methods across multiple benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Large biomedical VLMs adapted to high-end imaging (CT) fail to transfer to frontline low-end modalities (radiography) due to modality-specific shortcuts, creating a deployment gap for medical AI.

Method: K-MaT factorizes prompts, anchors them to clinical text descriptions, and aligns low-end prompt manifolds to visually-grounded high-end space using Fused Gromov-Wasserstein optimal transport, enabling zero-shot cross-modal transfer.

Result: Achieves SOTA on four cross-modal benchmarks, improving average harmonic mean accuracy to 44.1% (from 42.0%) and macro-F1 to 36.2%. Mitigates catastrophic forgetting seen in standard methods like CoOp (which drops to 27.0% accuracy).

Conclusion: Aligning prompt manifolds via optimal transport provides an effective route for zero-shot cross-modal deployment of medical VLMs, enabling robust performance across different imaging modalities without low-end training data.
...
### [90] [Dynamic Chunking Diffusion Transformer](https://arxiv.org/abs/2603.06351)
*Akash Haridas,Utkarsh Saxena,Parsa Ashrafi Fashi,Mehdi Rezagholizadeh,Vikram Appia,Emad Barsoum*

Main category: cs.CV

TL;DR: DC-DiT introduces adaptive token compression for diffusion transformers, using fewer tokens for uniform regions and more for detail-rich areas, improving efficiency and quality over standard DiT.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion transformers use fixed-length token sequences from patchify operations, spending uniform compute on all image regions regardless of varying detail levels and ignoring that denoising progresses from coarse to fine structure.

Method: Augments DiT backbone with learned encoder-router-decoder scaffold that adaptively compresses 2D input into shorter token sequences using chunking mechanism learned end-to-end with diffusion training. Learns to compress uniform regions into fewer tokens and detail-rich regions into more tokens, and adapts compression across diffusion timesteps.

Result: Consistently improves FID and Inception Score over parameter-matched and FLOP-matched DiT baselines on class-conditional ImageNet 256×256 across 4× and 16× compression. Shows meaningful visual segmentations emerge without explicit supervision.

Conclusion: DC-DiT is a promising technique for adaptive compute allocation in diffusion models, practical for upcycling from pretrained checkpoints with minimal post-training compute, and composable with other dynamic computation methods for further FLOP reduction.
...
### [91] [Lyapunov Probes for Hallucination Detection in Large Foundation Models](https://arxiv.org/abs/2603.06081)
*Bozhi Luan,Gen Li,Yalan Qin,Jifeng Guo,Yun Zhou,Faguo Wu,Hongwei Zheng,Wenjun Wu,Zhaoxin Fan*

Main category: cs.CV

TL;DR: The paper proposes Lyapunov Probes for hallucination detection in LLMs/MLLMs by framing them as dynamical systems where factual knowledge corresponds to stable equilibrium points, and hallucinations occur at knowledge-transition boundaries.


<details>
  <summary>Details</summary>
Motivation: Current approaches treat hallucination detection as a simple classification task, but the authors argue that hallucinations in (M)LLMs should be understood through dynamical systems theory, where factual knowledge forms stable regions in representation space and hallucinations emerge at boundaries between stable and unstable zones.

Method: Propose Lyapunov Probes - lightweight networks trained with derivative-based stability constraints that enforce monotonic confidence decay under input perturbations. Use systematic perturbation analysis and two-stage training to distinguish stable factual regions from unstable hallucination-prone regions.

Result: Experiments on diverse datasets and models show consistent improvements over existing baselines for hallucination detection.

Conclusion: Framing (M)LLMs as dynamical systems and using stability theory provides an effective approach to hallucination detection, with Lyapunov Probes offering a practical solution that outperforms previous methods.
...
TL;DR: DeepSight is the first dedicated depth MLLM that enhances 3D scene understanding by leveraging depth images instead of RGB, addressing limitations in depth perception through novel datasets and encoder modifications.


<details>
  <summary>Details</summary>
Motivation: Current multimodal LLMs struggle with accurate depth interpretation in visual data despite good performance on tasks like image captioning and VQA. There's a need for better 3D scene understanding capabilities.

Method: 1) Uses depth images (single-channel grayscale) instead of RGB for better spatial reasoning; 2) Creates novel depth image-text pair dataset and depth instruction dataset using GLPN for depth map generation and GPT-4 for instruction curation; 3) Modifies ViT encoder in CLIP to incorporate local object information for capturing subtle depth variations.

Result: DeepSight significantly enhances depth perception and downstream task performance, marking substantial progress in multimodal 3D understanding as demonstrated on a comprehensive depth QA benchmark.

Conclusion: The work introduces the first dedicated depth MLLM that effectively addresses depth interpretation challenges in multimodal models, providing a foundation for improved 3D scene understanding through specialized depth-focused architecture and datasets.
...
### [93] [Prompt Group-Aware Training for Robust Text-Guided Nuclei Segmentation](https://arxiv.org/abs/2603.06384)
*Yonghuang Wu,Zhenyang Liang,Wenwen Zeng,Xuan Xie,Jinhua Yu*

Main category: cs.CV

TL;DR: A method to reduce prompt sensitivity in text-guided medical image segmentation by treating semantically equivalent prompts as groups and applying consistency constraints during training.


<details>
  <summary>Details</summary>
Motivation: Foundation models like SAM3 enable text-guided medical image segmentation but are highly sensitive to prompt formulation, where even semantically equivalent descriptions can produce inconsistent masks, limiting reliability in clinical workflows.

Method: Reformulates prompt sensitivity as a group-wise consistency problem: organizes semantically related prompts into prompt groups sharing the same ground-truth mask, then uses quality-guided group regularization (using segmentation loss as implicit ranking signal) and logit-level consistency constraint with stop-gradient strategy to align predictions within each group.

Result: Extensive experiments on multi-dataset nuclei benchmarks show consistent gains under textual prompting and markedly reduced performance variance across prompt quality levels. On six zero-shot cross-dataset tasks, improves Dice by an average of 2.16 points.

Conclusion: The method demonstrates improved robustness and generalization for vision-language segmentation in computational pathology without requiring architectural modifications or changing inference procedures.
...
TL;DR: FedARKS addresses limitations in federated domain generalization for person re-identification by introducing robust knowledge selection and integration mechanisms to capture local details and handle client heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Existing FedDG-ReID methods have two key limitations: 1) reliance on global features fails to capture domain-invariant local details like accessories/textures, and 2) uniform parameter averaging ignores client heterogeneity in feature extraction capabilities, diluting contributions from high-quality clients.

Method: Proposes FedARKS framework with two mechanisms: RK (Robust Knowledge) for capturing local details and KS (Knowledge Selection) for selective aggregation based on client quality, addressing both feature granularity and client heterogeneity issues.

Result: The abstract doesn't provide experimental results, but the proposed method aims to enhance model generalization to unseen domains while protecting data privacy through improved feature representation and client-aware aggregation.

Conclusion: FedARKS represents a novel approach to federated domain generalization in person re-identification that addresses critical limitations in existing methods by focusing on local feature capture and quality-aware client aggregation.
...
TL;DR: PSIVG integrates a physical simulator into video diffusion to enforce physical laws like gravity and collision, improving motion realism while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based video generation produces visually realistic results but often violates basic physical laws (gravity, inertia, collision), leading to inconsistent motion and implausible dynamics that limit realism and reliability.

Method: PSIVG reconstructs 4D scenes and object meshes from diffusion-generated template videos, initializes them in a physical simulator to generate physically consistent trajectories, then uses these trajectories to guide video generation. Also includes Test-Time Texture Consistency Optimization (TTCO) for texture consistency during movement.
...
Conclusion: Integrating physical simulation into video diffusion pipelines effectively addresses physical law violations in generated videos, enhancing realism through physically coherent motion guidance.
...
TL;DR: RMD is a diffusion distillation framework that bridges cross-resolution distribution gaps to enable high-fidelity, few-step multi-resolution cascaded inference with significant speedups.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion distillation methods are limited by denoising process saturation, and partial timestep low-resolution generation suffers quality degradation due to cross-resolution distribution gaps.

Method: RMD divides timestep intervals per resolution using logSNR curves, introduces logSNR-based mapping to compensate for resolution shifts, performs distribution matching along resolution trajectories, and incorporates predicted-noise re-injection during upsampling.

Result: RMD achieves up to 33.4X speedup on SDXL and 25.6X on Wan2.1-14B while preserving high visual fidelity, outperforming existing methods.

Conclusion: RMD effectively bridges cross-resolution distribution gaps for accelerated, high-fidelity multi-resolution diffusion inference through novel distribution matching and noise re-injection techniques.
...
TL;DR: CLoPA: Continual adaptation strategy that tunes a small fraction of nnInteractive's parameters on annotation cache to rapidly elevate zero-shot medical segmentation to expert-level performance across diverse tasks.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot models like nnInteractive fail to consistently reach expert-level performance across diverse medical imaging tasks, and annotation campaigns produce growing streams of task-specific data that could be leveraged for online adaptation.

Method: CLoPA uses continual adaptation strategy that tunes a small fraction of nnInteractive's parameters on the annotation cache, triggered by lightweight episode scheduling. It requires no new parameters or changes to inference pipeline and operates within existing annotation workflow.

Result: Across eight Medical Segmentation Decathlon tasks, CLoPA rapidly elevates performance to expert-level, even for tasks where nnInteractive previously failed, with majority of gains realized after a single training episode. Benefits of tuning different parameter groups depend on task characteristics and data regimes.

Conclusion: CLoPA effectively bridges the gap between zero-shot inference and expert-level segmentation through continual adaptation, though for targets with complex geometries, deeper feature-representation alignment may be needed beyond instance normalization and low-level feature tuning.
...
### [98] [Do Foundation Models Know Geometry? Probing Frozen Features for Continuous Physical Measurement](https://arxiv.org/abs/2603.06459)
*Yakov Pyotr Shkolnikov*

Main category: cs.CV

TL;DR: Vision-language models encode geometric information that their text pathways can't express well, creating a 3.3x bottleneck between visual features and text output. Training objective matters more than architecture for geometric accuracy, and lightweight probes can extract this information without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: The paper investigates why vision-language models struggle to express geometric information through their text pathways despite encoding it in their visual features. There's a significant gap between what the models can represent geometrically and what they can express verbally.

Method: Used linear probes (6,000 parameters) to extract hand joint angles from frozen vision-language model features. Compared different architectures (self-supervised, contrastive, hybrid) and conducted layer-wise analysis. Tested LoRA fine-tuning with 2,000 images. Analyzed representational similarity using CKA and functional convergence.

Result: Linear probes achieved 6.1° MAE for hand joint angles vs. 20.0° via text output (3.3x bottleneck). LoRA fine-tuning reduced gap to 6.5°. Different architectures converged to similar accuracy (R²≈0.55) despite low representational similarity (CKA=0.41). Autoregressive generation damages geometric fidelity, but LLM layers can actually improve accuracy. Mid-network layers (18-22) carry most geometric signal.

Conclusion: Vision-language models have a pathway-training deficit, not a representational deficit. Training objective determines geometric accuracy more than architecture. Lightweight probes can extract geometric information from frozen backbones without fine-tuning, enabling multi-task geometric sensing without text generation.
...
### [99] [Spatial Colour Mixing Illusions as a Perception Stress Test for Vision-Language Models](https://arxiv.org/abs/2603.06141)
*Nicoleta-Nina Basoc,Adrian Cosma,Emilian Radoi*

Main category: cs.CV

TL;DR: VLMs fail on structured color distortions that humans easily handle, but simple preprocessing can help recover performance.


<details>
  <summary>Details</summary>
Motivation: Vision-language models show strong benchmark performance but have systematic perceptual weaknesses - they make confident wrong predictions on structured color distortions that don't fool humans, revealing a gap between VLM and human perception.

Method: Introduced Spatial Colour Mixing framework with 8 variants of structured color distortions in RGB and Ostwald color systems. Evaluated 9 VLMs across 3 families on 4 datasets. Conducted human study with 61 participants on animal recognition dataset. Tested simple human-inspired preprocessing.

Result: VLM accuracy degrades sharply with increasing distortion, scaling language models doesn't reliably help, humans substantially outperform VLMs on same distortions, and simple preprocessing recovers meaningful performance for several distortion types.

Conclusion: VLMs have systematic perceptual weaknesses to structured color distortions, revealing a gap with human perception. Perception-aware preprocessing and tool-use are practical strategies for improving VLM robustness.
...
### [100] [Longitudinal NSCLC Treatment Progression via Multimodal Generative Models](https://arxiv.org/abs/2603.06147)
...
TL;DR: A Virtual Treatment framework predicts NSCLC tumor evolution during radiotherapy using dose-aware multimodal image translation to synthesize follow-up CT scans from baseline images and clinical data.


<details>
  <summary>Details</summary>
Motivation: Predicting tumor evolution during radiotherapy is clinically critical but challenging due to anatomical changes driven by both anatomy and treatment effects, especially for non-small cell lung cancer patients undergoing radiation therapy.

Method: Formulates NSCLC progression as dose-aware multimodal conditional image-to-image translation. Uses CT scans, baseline clinical variables, and radiation dose increments to synthesize follow-up CT images. Benchmarks GAN-based and diffusion-based models across 2D and 2.5D configurations on 222 stage III NSCLC patients with 895 CT scans.

Result: Diffusion-based models benefit more consistently from multimodal, dose-aware conditioning and produce more stable and anatomically plausible tumor evolution trajectories than GAN-based baselines.

Conclusion: The Virtual Treatment framework shows potential as a tool for in-silico treatment monitoring and adaptive radiotherapy research in NSCLC, with diffusion models demonstrating superior performance for this task.
...
TL;DR: 3D dental implant inpainting using perpendicular score-based diffusion models to reduce CBCT artefacts by modeling correlations across projections.


<details>
  <summary>Details</summary>
Motivation: CBCT imaging suffers from artefacts from high-density dental implants, compromising diagnostic accuracy. Current diffusion-based methods treat 2D projections independently, ignoring inter-projection correlations and causing reconstruction inconsistencies.

Method: Proposes a 3D dental implant inpainting approach using perpendicular score-based diffusion models trained in two different planes (projection domain). Combines two 2D score-based diffusion models in sampling scheme to model 3D distribution of projection series.

Result: Method effectively produces high-quality, artefact-reduced 3D CBCT images, demonstrating promising clinical imaging improvements.

Conclusion: The 3D approach using perpendicular diffusion models successfully addresses limitations of independent 2D projection inpainting, offering a promising solution for clinical CBCT artefact reduction.
...
TL;DR: AI system trained on 45k+ ultrasound images achieves >93% sensitivity/95% specificity for fetal orofacial cleft detection, matching senior radiologists and boosting junior radiologists' performance by 6% when used as copilot.


<details>
  <summary>Details</summary>
Motivation: Orofacial clefts are common congenital abnormalities but prenatal detection is challenging due to scarcity of experienced specialists and condition rarity. Early diagnosis is crucial for timely intervention and reduced morbidity.

Method: Developed an AI system trained on 45,139 ultrasound images from 9,215 fetuses across 22 hospitals. Tested as medical copilot to assist radiologists, and conducted pilot study with 24 radiologists/trainees to evaluate expertise development.

Result: AI system achieved sensitivity >93% and specificity >95%, matching senior radiologists' performance and substantially outperforming junior radiologists. As copilot, raised junior radiologists' sensitivity by >6%. Pilot study showed system accelerates clinical expertise development for rare conditions.

Conclusion: The dual-purpose AI system offers scalable solution for improving both diagnostic accuracy and specialist training in settings with scarce experienced radiologists, addressing challenges in prenatal detection of orofacial clefts.
...
TL;DR: FreeOcc is a training-free pipeline that uses pretrained foundation models for semantic and panoptic occupancy prediction from multi-view images, achieving competitive performance without 3D supervision or domain-specific training.


<details>
  <summary>Details</summary>
Motivation: Current camera-only occupancy prediction methods require costly dense 3D supervision or domain-specific training, limiting deployment in unseen environments. There's a need for training-free approaches that can generalize across domains.

Method: Uses pretrained foundation models: 1) promptable segmentation model for per-view panoptic priors, 2) reconstruction model for metric 3D points. Combines depth- and confidence-aware filtering to lift 2D labels to 3D, with temporal fusion and voxelization. For panoptic occupancy, recovers instances by fitting and merging 3D box candidates.

Result: On Occ3D-nuScenes: 16.9 mIoU and 16.5 RayIoU train-free (comparable to SOTA weakly supervised methods). As pseudo-label generator: 21.1 RayIoU (surpasses previous SOTA). Sets new baselines for train-free (3.1 RayPQ) and weakly supervised (3.9 RayPQ) panoptic occupancy prediction.

Conclusion: Foundation-model-driven perception provides a practical route to training-free 3D scene understanding, enabling competitive occupancy prediction without costly 3D supervision or domain-specific training.
...
TL;DR: SUREON is a large-scale surgical video QA dataset created from academic lecture narrations to teach AI surgical reasoning about intent, rationale, and anticipation.


<details>
  <summary>Details</summary>
Motivation: Current surgical AI lacks reasoning capabilities (understanding why instruments are chosen, risks, next steps) because annotated training data for surgical reasoning is difficult to obtain at scale. Surgical video lectures contain expert explanations of intent and rationale that could provide this missing training signal.

Method: Created SUREON dataset by harvesting surgical academic video narrations using a multi-agent pipeline to extract and structure supervision. Defined 12 question categories covering safety assessment, decision rationale, and forecasting. Also introduced two models: SureonVLM (vision-language model fine-tuned on SUREON) and SureonVLM-R1 (reasoning model trained with Group Relative Policy Optimization).

Result: SUREON contains 134.7K clips across 170 procedure types, yielding 206.8k QA pairs with expert-validated benchmark of 354 examples. Both models substantially outperform larger general-domain models, exceeding 84% accuracy on SUREON benchmark while also outperforming on standard surgical perception tasks. SureonVLM-R1 shows explicit reasoning behavior like inferring operative intent from visual context.

Conclusion: Surgical video lectures provide valuable training signal for teaching AI surgical reasoning capabilities. The SUREON dataset and models demonstrate that structured supervision from expert narrations can enable AI to answer complex questions about surgery, bridging the gap between perception and reasoning in surgical AI.
...
### [105] [A Semi-Supervised Framework for Breast Ultrasound Segmentation with Training-Free Pseudo-Label Generation and Label Refinement](https://arxiv.org/abs/2603.06167)
*Ruili Li,Jiayi Ding,Ruiyu Li,Yilun Jin,Shiwen Ge,Yuwen Zeng,Xiaoyong Zhang,Eichi Takaya,Jan Vrba,Noriyasu Homma*

Main category: cs.CV

TL;DR: A semi-supervised framework for breast ultrasound segmentation that uses vision-language models with simple appearance descriptions to generate training-free pseudo labels, achieving near-fully-supervised performance with only 2.5% labeled data.


<details>
  <summary>Details</summary>
Motivation: Current semi-supervised learning for breast ultrasound segmentation suffers from unstable pseudo labels under extremely limited annotations, and existing vision-language models struggle with domain-specific prompts for medical images.

Method: Proposes training-free pseudo-label generation using simple appearance-based descriptions (e.g., "dark oval") for cross-domain transfer, combined with a static teacher for global structural priors and an exponential moving average teacher with uncertainty entropy weighted fusion and adaptive uncertainty-guided reverse contrastive learning.

Result: Achieves performance comparable to fully supervised models with only 2.5% labeled data on four BUS datasets, significantly outperforming existing SSL approaches.

Conclusion: The framework is extensible to other imaging modalities/diseases requiring only global appearance descriptions for reliable pseudo supervision, enabling scalable semi-supervised medical image segmentation under limited annotations.
...
### [106] [JOPP-3D: Joint Open Vocabulary Semantic Segmentation on Point Clouds and Panoramas](https://arxiv.org/abs/2603.06168)
*Sandeep Inuganti,Hideaki Kanayama,Kanta Shimizu,Mahdi Chamseddine,Soichiro Yokota,Didier Stricker,Jason Rambach*

Main category: cs.CV

TL;DR: JOPP-3D is an open-vocabulary semantic segmentation framework that jointly uses panoramic images and 3D point clouds for language-driven scene understanding, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Semantic segmentation across visual modalities like 3D point clouds and panoramic images is challenging due to scarce annotated data and limited adaptability of fixed-label models.

Method: Converts RGB-D panoramic images to tangential perspective images and 3D point clouds, extracts and aligns foundational vision-language features from both modalities, enabling natural language querying to generate semantic masks.

Result: Achieves significant improvement over SOTA in open and closed vocabulary 2D and 3D semantic segmentation on Stanford-2D-3D-s and ToF-360 datasets, producing coherent and semantically meaningful segmentations.

Conclusion: JOPP-3D successfully enables language-driven scene understanding across panoramic and 3D domains through joint processing of multiple visual modalities.
...
### [107] [Optimizing 3D Diffusion Models for Medical Imaging via Multi-Scale Reward Learning](https://arxiv.org/abs/2603.06173)
*Yueying Tian,Xudong Han,Meng Zhou,Rodrigo Aviles-Espinosa,Rupert Young,Philip Birch*

Main category: cs.CV

TL;DR: RL-enhanced 3D diffusion models for medical imaging with multi-scale feedback improve both image quality and downstream task utility.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion model training objectives don't align well with clinical relevance in 3D medical image generation, creating a gap between technical metrics and practical utility.

Method: Two-stage approach: 1) Pretrain 3D diffusion model on MRI volumes, 2) Fine-tune with PPO using novel reward system combining 2D slice-wise and 3D volumetric assessments to optimize both local details and global structure.

Result: Significant improvements in FID scores, and synthetic data shows enhanced utility in downstream tumor and disease classification tasks on BraTS 2019 and OASIS-1 datasets.

Conclusion: RL with multi-scale feedback effectively bridges the gap between standard training objectives and clinical relevance in 3D medical image generation, producing higher quality distributions with better downstream task performance.
...
### [108] [Making Training-Free Diffusion Segmentors Scale with the Generative Power](https://arxiv.org/abs/2603.06178)
*Benyuan Meng,Qianqian Xu,Zitai Wang,Xiaochun Cao,Longtao Huang,Qingming Huang*

Main category: cs.CV

TL;DR: Training-free diffusion segmentation using cross-attention maps fails to scale with stronger generative models due to attention map discrepancies and score imbalances; proposed auto aggregation and per-pixel rescaling techniques improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing training-free diffusion segmentation methods that use cross-attention maps don't scale well with more powerful generative models, despite the expectation that stronger generative capability should lead to better segmentation performance.

Method: Two techniques: (1) auto aggregation to address discrepancies between individual attention maps across multiple heads/layers and unified global representation, and (2) per-pixel rescaling to handle score imbalances among different text tokens that prevent accurate semantic correlation for segmentation.

Result: Improved performance on standard semantic segmentation benchmarks and broader applicability when integrated into generative techniques, enabling training-free segmentation to better leverage generative capability.

Conclusion: The proposed auto aggregation and per-pixel rescaling techniques successfully bridge the gaps in existing training-free diffusion segmentation methods, allowing them to better scale with more powerful generative models and achieve improved segmentation performance.
...
### [109] [Towards Motion Turing Test: Evaluating Human-Likeness in Humanoid Robots](https://arxiv.org/abs/2603.06181)
...
TL;DR: Proposes Motion Turing Test framework and HHMotion dataset to evaluate human-likeness of humanoid robot poses using only kinematic data, revealing current limitations in robot motion and developing automated evaluation methods.


<details>
  <summary>Details</summary>
Motivation: Inspired by the Turing Test, the paper aims to create a systematic framework to evaluate how human-like humanoid robot motions appear to human observers, addressing the gap in objective assessment of motion naturalness despite visual appearance improvements.

Method: 1) Proposes Motion Turing Test framework; 2) Creates HHMotion dataset with 1,000 motion sequences across 15 action categories from 11 humanoid models and 10 humans in SMPL-X format; 3) Collects 500+ hours of human annotations (30 annotators rating 0-5 scale); 4) Develops baseline model for automated human-likeness prediction.

Result: Humanoid motions still show noticeable deviations from human movements, especially in dynamic actions (jumping, boxing, running). Current multimodal LLMs are inadequate for motion human-likeness assessment, while the proposed baseline outperforms LLM-based methods.

Conclusion: The Motion Turing Test and HHMotion dataset provide valuable tools for evaluating and improving humanoid motion generation. Significant gaps remain in dynamic motion human-likeness, and automated evaluation methods need further development. Public release of dataset and benchmark will support community research.
...
TL;DR: SCOPE is a plug-and-play framework for 3D incremental few-shot segmentation that enriches prototypes using unlabeled background instances from base training scenes to improve novel class learning with minimal forgetting.


<details>
  <summary>Details</summary>
Motivation: 3D incremental few-shot segmentation is underexplored, with existing methods suffering from catastrophic forgetting or poor discriminative prototypes under sparse supervision. Novel categories often appear as unlabeled background in base-training scenes, providing valuable cues that are currently overlooked.

Method: SCOPE uses a class-agnostic segmentation model after base training to extract high-confidence pseudo-instances from background regions, building a prototype pool. When novel classes arrive with few samples, relevant background prototypes are retrieved and fused with few-shot prototypes to create enriched representations without retraining the backbone or adding parameters.

Result: On ScanNet and S3DIS datasets, SCOPE achieves state-of-the-art performance, improving novel-class IoU by up to 6.98% and 3.61%, and mean IoU by 2.25% and 1.70% respectively, while maintaining low forgetting.

Conclusion: SCOPE effectively leverages background information from base training scenes to enhance few-shot learning in 3D segmentation, achieving superior performance with minimal forgetting and no additional parameters.
...
### [111] [SpaCRD: Multimodal Deep Fusion of Histology and Spatial Transcriptomics for Cancer Region Detection](https://arxiv.org/abs/2603.06186)
*Shuailin Xue,Jun Wan,Lihua Zhang,Wenwen Min*

Main category: cs.CV

TL;DR: SpaCRD is a transfer learning method that integrates histology images with spatial transcriptomics data for accurate cancer tissue region detection across diverse samples, platforms, and batches.


<details>
  <summary>Details</summary>
Motivation: Traditional cancer tissue region detection methods relying on histology images alone suffer from high false positives due to morphological similarities. Spatial transcriptomics provides detailed cellular phenotypes and spatial information but current methods can't effectively integrate both data types, especially in cross-sample and cross-platform settings.

Method: SpaCRD uses a category-regularized variational reconstruction-guided bidirectional cross-attention fusion network that adaptively captures latent co-expression patterns between histological features and gene expression from multiple perspectives. It's a transfer learning approach that can generalize to new samples from different platforms and batches.

Result: Extensive benchmark analysis on 23 matched histology-ST datasets across various disease types, platforms, and batches shows SpaCRD consistently outperforms eight state-of-the-art methods in cancer tissue region detection.

Conclusion: SpaCRD enables reliable cancer tissue region detection by effectively integrating histology images with spatial transcriptomics data, demonstrating superior performance across diverse samples, platforms, and batches compared to existing methods.
...
### [112] [Adaptive Language-Aware Image Reflection Removal Network](https://arxiv.org/abs/2603.06200)
*Siyan Fang,Yuntao Wang,Jinpu Zhang,Ziwen Li,Yuehuan Wang*

Main category: cs.CV

TL;DR: ALANet is a language-aware network that removes complex reflections from images even when language descriptions are inaccurate, using filtering and optimization strategies with a new CRLAV dataset.


<details>
  <summary>Details</summary>
Motivation: Existing reflection removal methods struggle with complex reflections. Language descriptions can help understand image content, but inaccurate descriptions from machines due to reflection distortions harm language-guided approaches.

Method: Proposes Adaptive Language-Aware Network (ALANet) with filtering strategy to reduce negative effects of inaccurate language while preserving benefits, optimization strategy to enhance language-visual feature alignment, and uses language cues to decouple layer content from feature maps.

Result: ALANet surpasses state-of-the-art methods for image reflection removal, demonstrated through experiments on the new CRLAV dataset which contains complex reflections with varying language accuracy levels.

Conclusion: ALANet effectively handles complex reflections even with inaccurate language inputs, with code and dataset publicly available for further research.
...
### [113] [Point-Supervised Skeleton-Based Human Action Segmentation](https://arxiv.org/abs/2603.06201)
*Hongsong Wang,Yiqin Shen,Pengbo Yan,Jie Gui*

Main category: cs.CV

TL;DR: Point-supervised skeleton action segmentation using only one labeled frame per action segment, leveraging multimodal skeleton data and pseudo-label integration to achieve competitive performance with reduced annotation cost.


<details>
  <summary>Details</summary>
Motivation: Fully-supervised methods for skeleton-based temporal action segmentation require costly frame-level annotations and are sensitive to ambiguous action boundaries. There's a need for methods that reduce annotation effort while maintaining performance.

Method: Proposes a point-supervised framework using only one labeled frame per action segment. Leverages multimodal skeleton data (joint, bone, motion) encoded via pretrained unified model. Introduces prototype similarity method combined with energy function and constrained K-Medoids clustering for pseudo-label generation. Uses multimodal pseudo-label integration to enhance reliability and guide training.

Result: Establishes new benchmarks on PKU-MMD (X-Sub and X-View), MCFS-22, and MCFS-130 datasets. Achieves competitive performance, even surpassing some fully-supervised methods while significantly reducing annotation effort.

Conclusion: The proposed point-supervised framework effectively reduces annotation costs for skeleton-based action segmentation while maintaining competitive performance, demonstrating the viability of using minimal supervision for this challenging task.
...
### [114] [EntON: Eigenentropy-Optimized Neighborhood Densification in 3D Gaussian Splatting](https://arxiv.org/abs/2603.06216)
*Miriam Jäger,Boris Jutzi*

Main category: cs.CV

TL;DR: EntON introduces an Eigenentropy-optimized neighborhood densification strategy for 3D Gaussian Splatting that improves geometric accuracy and rendering quality while reducing computational cost.


<details>
  <summary>Details</summary>
Motivation: Standard 3DGS produces Gaussians poorly aligned with object geometry, while surface-focused methods sacrifice photometric accuracy. There's a need for better geometric accuracy without compromising rendering quality.

Method: Uses Eigenentropy (from covariance matrix eigenvalues) to quantify local structural order. Implements alternating optimization: standard gradient-based densification + Eigenentropy-aware densification that splits/prunes based on local order (low-Eigenentropy for flat surfaces, high-Eigenentropy for spherical regions).

Result: Improves geometric accuracy by up to 33%, rendering quality by up to 7%, reduces Gaussians by up to 50%, and training time by up to 23% on DTU and TUM2TWIN datasets.

Conclusion: EntON achieves favorable balance between geometric accuracy, rendering quality, and efficiency by avoiding unnecessary scene expansion through geometry-aware densification.
...
### [115] [Word-Anchored Temporal Forgery Localization](https://arxiv.org/abs/2603.06220)
*Tianyi Wang,Xi Shao,Harry Cheng,Yinglong Wang,Mohan Kankanhalli*

Main category: cs.CV

TL;DR: WAFL transforms temporal forgery localization from temporal regression to discrete word-level binary classification using linguistic boundaries, forensic feature realignment, and artifact-centric asymmetric loss.


<details>
  <summary>Details</summary>
Motivation: Current TFL approaches suffer from feature granularity misalignment and costly computation due to reliance on temporal boundary regression or continuous frame-level anomaly detection.

Method: 1) Shift to word-level binary classification using linguistic boundaries; 2) Forensic Feature Realignment (FFR) module to map pre-trained features to forensic manifold; 3) Artifact-Centric Asymmetric (ACA) loss to handle class imbalance by prioritizing forensic artifacts.

Result: WAFL significantly outperforms state-of-the-art approaches in localization performance under both in- and cross-dataset settings, while requiring substantially fewer parameters and operating at high computational efficiency.

Conclusion: The word-anchored paradigm provides a more effective and efficient approach to temporal forgery localization by aligning with natural linguistic units and addressing feature granularity issues.
...
### [116] [Low-latency Event-based Object Detection with Spatially-Sparse Linear Attention](https://arxiv.org/abs/2603.06228)
*Haiqing Hao,Zhipeng Sui,Rong Zou,Zijia Dai,Nikola Zubić,Davide Scaramuzza,Wenhui Wang*

Main category: cs.CV

TL;DR: SSLA-Det introduces spatially-sparse linear attention for event-based object detection, achieving SOTA accuracy with 20x less computation than prior async methods.


<details>
  <summary>Details</summary>
Motivation: Existing async event-based networks have bottlenecks: recurrent architectures are hard to train on long sequences, and accuracy improvements increase per-event computation/latency. Linear attention supports parallel training but has poor accuracy-efficiency trade-off due to global state updates.

Method: Proposes Spatially-Sparse Linear Attention (SSLA) with mixture-of-spaces state decomposition and scatter-compute-gather training procedure, enabling state-level sparsity while preserving training parallelism. Built SSLA-Det as end-to-end async linear attention model for event-based detection.

Result: Achieves state-of-the-art accuracy among async methods: 0.375 mAP on Gen1 and 0.515 mAP on N-Caltech101. Reduces per-event computation by >20x compared to strongest prior async baseline.

Conclusion: SSLA-Det demonstrates the potential of linear attention for low-latency event-based vision by solving key bottlenecks through spatially-sparse state activation that exploits event sparsity while maintaining efficient parallel training.
...
### [117] [Hierarchical Collaborative Fusion for 3D Instance-aware Referring Expression Segmentation](https://arxiv.org/abs/2603.06250)
*Keshen Zhou,Runnan Chen,Mingming Gong,Tongliang Liu*

Main category: cs.CV

TL;DR: HCF-RES improves 3D referring expression segmentation by using hierarchical visual semantics from SAM and CLIP with progressive multi-level fusion, achieving SOTA on ScanRefer and Multi3DRefer.


<details>
  <summary>Details</summary>
Motivation: Existing 3D referring expression segmentation methods rely only on sparse point clouds, lacking rich visual semantics needed for fine-grained descriptions, especially when language matches multiple or zero targets.

Method: Proposes HCF-RES with two innovations: 1) Hierarchical Visual Semantic Decomposition using SAM instance masks to guide CLIP encoding at pixel-level and instance-level features, preserving object boundaries during 2D-to-3D projection; 2) Progressive Multi-level Fusion integrating representations through intra-modal collaboration, cross-modal adaptive weighting between 2D semantic and 3D geometric features, and language-guided refinement.

Result: Achieves state-of-the-art results on both ScanRefer and Multi3DRefer benchmarks for 3D referring expression segmentation.

Conclusion: HCF-RES effectively addresses the limitations of sparse point cloud methods by incorporating rich visual semantics through hierarchical decomposition and progressive fusion, enabling better handling of ambiguous language descriptions in 3D scenes.
...
TL;DR: Real-time drone detection system using spinning event camera for moving carriers, achieving 360° FOV and bearing estimation without motion compensation.


<details>
  <summary>Details</summary>
Motivation: Need for drone detection systems that work on moving platforms (like robots/vehicles) in challenging conditions where conventional frame cameras fail, especially with fast-moving targets and adverse lighting.

Method: Uses spinning event-based camera for 360° horizontal field of view. Develops novel image-like event representation without motion compensation and lightweight neural network for spatiotemporal learning. Implemented on Jetson Orin NX for real-time operation.

Result: System operates in real-time with reliable detection and mean angular error below 2° under challenging outdoor conditions. Suitable for real-world surveillance applications.

Conclusion: Event-based approach enables effective drone detection on moving carriers in challenging conditions, overcoming limitations of conventional cameras. Complete pipeline will be open-sourced.
...
TL;DR: StrSR is a novel one-step adversarial distillation framework for diffusion transformer-based real-world image super-resolution that addresses trajectory mismatch and periodic artifacts through spectral and trajectory regularization.


<details>
  <summary>Details</summary>
Motivation: Diffusion transformer architectures show great potential for real-world image super-resolution, but their computationally expensive iterative sampling requires one-step distillation. Existing one-step distillation methods struggle with Real-ISR on DiT due to fundamental trajectory mismatch and severe grid-like periodic artifacts.

Method: StrSR proposes a novel one-step adversarial distillation framework with spectral and trajectory regularization. It uses an asymmetric discriminative distillation architecture to bridge the trajectory gap and a frequency distribution matching strategy to suppress DiT-specific periodic artifacts caused by high-frequency spectral leakage.

Result: Extensive experiments demonstrate that StrSR achieves state-of-the-art performance in Real-ISR across both quantitative metrics and visual perception.

Conclusion: StrSR effectively addresses the challenges of one-step distillation for DiT-based Real-ISR through its innovative spectral and trajectory regularization approach, achieving superior performance while eliminating the computational burden of iterative sampling.
...
TL;DR: ADiVA addresses two key challenges in generative zero-shot learning: class-instance gap and semantic-visual domain gap through attribute distribution modeling and semantic-visual alignment.


<details>
  <summary>Details</summary>
Motivation: Generative ZSL synthesizes features for unseen classes but faces two intrinsic challenges: (1) class-level attributes fail to capture instance-specific visual appearances due to intra-class variability (class-instance gap), and (2) mismatch between semantic and visual feature distributions due to inter-class correlations (semantic-visual domain gap).

Method: Proposes ADiVA approach with two modules: Attribute Distribution Modeling (ADM) learns transferable attribute distributions for each class and samples instance-level attributes for unseen classes; Visual-Guided Alignment (VGA) refines semantic representations to better reflect visual structures.

Result: Significantly outperforms state-of-the-art methods on three benchmark datasets, achieving gains of 4.7% on AWA2 and 6.1% on SUN. The approach can also serve as a plugin to enhance existing generative ZSL methods.

Conclusion: ADiVA effectively addresses the class-instance gap and semantic-visual domain gap in generative ZSL through joint attribute distribution modeling and semantic-visual alignment, demonstrating superior performance and flexibility as a plugin enhancement.
...
TL;DR: FlowMotion is a training-free video motion transfer framework that uses flow-based T2V model predictions for efficient motion guidance, avoiding heavy computational overhead of existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing training-free approaches for video motion transfer rely on intermediate outputs of pre-trained T2V models, which causes heavy computational overhead and limited flexibility. The authors aim to create a more efficient and flexible solution.

Method: FlowMotion leverages early latent predictions from flow-based T2V models that inherently encode rich temporal information. It introduces flow guidance to extract motion representations from these predictions to align motion patterns between source and generated videos, plus a velocity regularization strategy to stabilize optimization and ensure smooth motion evolution.

Result: FlowMotion achieves superior time and resource efficiency while maintaining competitive performance compared to state-of-the-art methods, by operating purely on model predictions without additional training.

Conclusion: The proposed FlowMotion framework demonstrates that direct utilization of flow-based T2V model predictions enables efficient and flexible video motion transfer, offering a practical solution that balances performance with computational efficiency.
...
TL;DR: LTA is a transfer-based adversarial attack that optimizes perturbations in Stable Diffusion's latent space instead of pixel space, producing more robust and coherent attacks that transfer better across models.


<details>
  <summary>Details</summary>
Motivation: Pixel-space adversarial attacks produce high-frequency, texture-like noise that is brittle to preprocessing and transfers poorly across architectures. The authors want to create more robust attacks that survive common input transformations and transfer better between models.

Method: LTA encodes clean images into Stable Diffusion VAE's latent space, optimizes perturbations there to maximize surrogate classifier loss, while enforcing pixel-space ℓ∞ constraints after decoding. Uses Expectation Over Transformations (randomized resizing, interpolation, cropping) and periodic latent Gaussian smoothing for robustness.

Result: LTA achieves strong transfer attack success across CNN and vision-transformer targets, producing spatially coherent, low-frequency perturbations that differ from pixel-space baselines and occupy a distinct point in the transfer-quality trade-off.

Conclusion: Pretrained generative latent spaces are effective domains for adversarial optimization, bridging robustness evaluation with modern generative priors and producing more robust attacks.
...
TL;DR: Wavelet-enhanced mixture-of-experts prompt learning for zero-shot anomaly detection that combines wavelet decomposition for multi-frequency features with adaptive prompt learning for better anomaly detection across diverse patterns.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models for zero-shot anomaly detection rely on fixed textual prompts that struggle with complex semantics and focus only on spatial-domain features, limiting their ability to detect subtle anomalies.

Method: Proposes wavelet-enhanced mixture-of-experts prompt learning: uses variational autoencoder to model global semantic representations for adaptive prompts, wavelet decomposition for multi-frequency image features, cross-modal interactions to refine textual embeddings, and semantic-aware mixture-of-experts module to aggregate contextual information.

Result: Extensive experiments on 14 industrial and medical datasets demonstrate the effectiveness of the proposed method.

Conclusion: The proposed approach addresses limitations of fixed prompts and spatial-only features in zero-shot anomaly detection by integrating wavelet decomposition and adaptive prompt learning, showing strong performance across diverse datasets.
...
TL;DR: P-SLCR: A prototype library-driven unsupervised point cloud semantic segmentation method using Structure Learning and Consistent Reasoning, achieving state-of-the-art performance on major datasets.


<details>
  <summary>Details</summary>
Motivation: Current point cloud semantic segmentation heavily relies on manual labeling, while unsupervised methods are underdeveloped. The absence of annotations and pre-training poses significant challenges, requiring effective strategies for unsupervised learning.

Method: 1) Consistent Structure Learning: establishes structural feature learning between consistent points and prototype library by selecting high-quality features. 2) Semantic Relation Consistent Reasoning: constructs prototype inter-relation matrix between consistent and ambiguous prototype libraries separately, preserving semantic consistency through constraints.

Result: Achieved best performance among unsupervised methods on S3DIS, SemanticKITTI, and Scannet datasets. Specifically achieved 47.1% mIoU for Area-5 of S3DIS dataset, surpassing classical fully supervised method PointNet by 2.5%.

Conclusion: The proposed P-SLCR framework effectively addresses unsupervised point cloud semantic segmentation challenges through prototype library-driven learning with structure learning and consistent reasoning, demonstrating superior performance over existing methods.
...
TL;DR: WorldCache accelerates diffusion-based world models by addressing token heterogeneity and non-uniform temporal dynamics through curvature-guided token prediction and chaotic-prioritized adaptive skipping, achieving 3.7× speedups with 98% rollout quality.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based world models are computationally expensive for interactive use and long-horizon rollouts due to iterative denoising. Existing feature caching methods don't work well because policies designed for single-modal diffusion fail to address world-model-specific challenges: token heterogeneity (multi-modal coupling and spatial variation) and non-uniform temporal dynamics (where a small set of hard tokens drives error growth).

Method: WorldCache introduces two key techniques: 1) Curvature-guided Heterogeneous Token Prediction - uses physics-grounded curvature scores to estimate token predictability and applies Hermite-guided damped predictors for chaotic tokens with abrupt direction changes; 2) Chaotic-prioritized Adaptive Skipping - accumulates a curvature-normalized, dimensionless drift signal and recomputes only when bottleneck tokens begin to drift.

Result: Experiments on diffusion world models show WorldCache delivers up to 3.7× end-to-end speedups while maintaining 98% rollout quality, demonstrating practical advantages for resource-constrained scenarios.

Conclusion: WorldCache provides an effective caching framework tailored to diffusion world models that addresses their unique challenges, enabling significant acceleration without compromising rollout quality, making diffusion-based world simulation more practical for interactive applications.
...
TL;DR: LATO is a novel topology-preserving latent representation for scalable flow matching-based synthesis of explicit 3D meshes, using a Vertex Displacement Field anchored on surface with sparse voxel VAE compression and direct connectivity prediction.


<details>
  <summary>Details</summary>
Motivation: Current 3D mesh generation methods using isosurface extraction or heuristic meshing struggle with complex geometry and topology preservation, while being inefficient in inference. There's a need for scalable, topology-aware mesh synthesis that can generate complex geometry with well-formed topology.

Method: LATO represents meshes as Vertex Displacement Fields anchored on surface, compressed via sparse voxel VAE into structured topology-aware voxel latents. The decoder progressively subdivides/prunes latent voxels to instantiate vertices. A connection head predicts edge connectivity directly from voxel latents. For generation, uses two-stage flow matching: first synthesizes structure voxels, then refines voxel-wise topology features.
...
Conclusion: LATO provides a scalable, topology-preserving approach for explicit 3D mesh synthesis that avoids isosurface extraction and heuristic meshing, enabling efficient generation of complex, topologically sound meshes through flow matching.
...
TL;DR: Computer vision methods for estimating invertebrate dry mass from images using area and sinking speed predictors, achieving 10-20% median error.


<details>
  <summary>Details</summary>
Motivation: To scale up biodiversity monitoring by automating dry mass estimation, eliminating manual weighing which is time-consuming and destructive to specimens.

Method: Two approaches: 1) Linear model with novel predictors (area and sinking speed) calculated from BIODISCOVER dual-camera system, 2) End-to-end deep neural networks with single-view, multi-view, and metadata-aware architectures trained on image sequence and dry mass pairs.

Result: Methods successfully estimate dry mass for diverse specimen morphologies with 10-20% median percentage error for individuals when combined with taxonomic classification.

Conclusion: Computer vision enables accurate, non-destructive biomass estimation; recommends using both percentage and absolute error metrics; explores optimization losses, data augmentation, and architectures for deep learning models.
...
TL;DR: OralGPT-Plus is an agentic vision-language model for panoramic dental radiograph analysis that uses iterative, symmetry-aware reasoning with reinforcement learning and a new dataset/benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models use static single-pass paradigms that lack fine-grained spatial reasoning, bilateral symmetry understanding, and multi-step diagnostic verification needed for reliable clinical panoramic dental analysis.

Method: 1) Created DentalProbe dataset (5,000 images) with expert-curated diagnostic trajectories for localized inspection and contralateral comparison. 2) Developed Reinspection-driven reinforcement learning framework with rubric-based and diagnostic-driven rewards for stable long-horizon reasoning. 3) Built MMOral-X benchmark (300 open-ended questions with region-level annotations across difficulty levels).

Result: OralGPT-Plus shows consistent and reliable improvements over strong baselines on both MMOral-X benchmark and established panoramic benchmarks, demonstrating effectiveness of interactive and symmetry-informed reasoning.

Conclusion: Agentic modeling is valuable for dental imaging, providing a foundation for clinically aligned panoramic radiograph analysis with iterative, symmetry-aware diagnostic reasoning.
...
TL;DR: Rewis3d uses 3D reconstruction from 2D videos to improve weakly supervised semantic segmentation, achieving SOTA performance with sparse annotations.


<details>
  <summary>Details</summary>
Motivation: Dense pixel-level annotations for semantic segmentation are expensive and time-consuming to obtain. While sparse annotations offer a weakly-supervised alternative, they still suffer from a performance gap compared to fully supervised methods.

Method: The framework leverages 3D scene reconstruction as auxiliary supervision. It uses a dual student-teacher architecture that enforces semantic consistency between 2D images and reconstructed 3D point clouds. State-of-the-art feed-forward 3D reconstruction provides reliable geometric supervision to propagate sparse annotations across entire scenes.
...
Conclusion: 3D geometric structure recovered from 2D videos provides strong cues for propagating sparse annotations, enabling more effective weakly supervised semantic segmentation without additional annotation costs.
...
TL;DR: MoEMambaMIL is a structure-aware State Space Model framework for whole-slide image analysis that organizes patch tokens into region-aware sequences and uses mixture-of-experts modeling to capture hierarchical multi-resolution patterns.


<details>
  <summary>Details</summary>
Motivation: Existing MIL approaches treat WSIs as unordered patch collections, failing to capture structured dependencies between global tissue organization and local cellular patterns. Current SSMs lack proper structuring of WSI tokens to exploit spatial hierarchy.

Method: Proposes MoEMambaMIL with region-nested selective scanning and MoE modeling. Organizes patch tokens into region-aware sequences preserving spatial containment across resolutions. Uses static resolution-specific experts and dynamic sparse experts with learned routing for resolution-aware encoding and region-adaptive contextual modeling.

Result: Achieves best performance across 9 downstream tasks, demonstrating effectiveness of structure-aware SSM framework for WSI analysis.

Conclusion: MoEMambaMIL successfully addresses the challenge of modeling hierarchical multi-resolution structure in WSIs by integrating structured token organization with efficient long-sequence modeling through expert specialization.
...
TL;DR: CHMv2 is a new global canopy height map at meter resolution derived from optical satellite imagery using DINOv3-based depth estimation, trained on expanded ALS data with improved accuracy over existing products.


<details>
  <summary>Details</summary>
Motivation: Accurate canopy height measurements are essential for forest carbon quantification, restoration monitoring, and habitat assessment, but high-quality ALS data remains unevenly available globally.

Method: Uses high-resolution optical satellite imagery with a depth-estimation model built on DINOv3, trained against ALS canopy height models with expanded geographically diverse training data, automated data curation/registration, and tailored loss formulation and data sampling strategies.

Result: CHMv2 substantially improves accuracy, reduces bias in tall forests, better preserves fine-scale structure (canopy edges and gaps), and demonstrates consistent performance across major forest biomes when validated against independent ALS, GEDI, and ICESat-2 data.

Conclusion: CHMv2 provides a globally consistent, high-resolution canopy height map that addresses limitations of existing products and enables better forest monitoring and carbon assessment worldwide.
...
TL;DR: REACT++ is a real-time Scene Graph Generation model that balances inference speed, relation prediction accuracy, and object detection performance, achieving 20% faster inference with 10% better relation accuracy than previous REACT.


<details>
  <summary>Details</summary>
Motivation: Current SGG methods focus on either relation prediction accuracy, object detection accuracy, or latency reduction, but not all three simultaneously. There's a need for models that balance performance and speed for real-time applications like embodied agent reasoning.

Method: Builds on REACT architecture with efficient feature extraction and subject-to-object cross-attention within prototype space to balance latency and representational power.

Result: Achieves highest inference speed among existing SGG models, improves relation prediction accuracy without sacrificing object detection performance. 20% faster than previous REACT with 10% gain in relation prediction accuracy on average.

Conclusion: REACT++ represents a new state-of-the-art for real-time SGG that successfully balances the trade-off between performance and inference speed, enabling practical applications.
...
TL;DR: Human-in-the-loop framework for archaeological artifact reassembly that combines automatic puzzle solving with interactive human guidance to handle large-scale, complex real-world fragments.


<details>
  <summary>Details</summary>
Motivation: Traditional jigsaw puzzle solvers fail with real-world archaeological artifacts due to erosion, missing regions, irregular shapes, and large-scale ambiguity, especially when dealing with thousands of fragments as in the RePAIR benchmark.

Method: Integrates automatic relaxation-labeling solver with interactive human guidance, featuring two interaction strategies: Iterative Anchoring (locking verified placements) and Continuous Interactive Refinement (correcting errors and guiding assemblies).

Result: Outperforms both fully automatic and manual baselines in accuracy and efficiency on several RePAIR groups, demonstrating practical effectiveness for large-scale artifact reassembly.

Conclusion: The hybrid human-in-the-loop approach offers a practical solution for expert-in-the-loop cultural heritage reconstruction, effectively handling the complexity and scale of real-world archaeological puzzles.
...
TL;DR: DiffInf uses self-influence scoring and diffusion-based image editing to correct annotation inconsistencies in facial attribute datasets, improving classification without discarding data.


<details>
  <summary>Details</summary>
Motivation: Facial attribute datasets have annotation inconsistencies due to subjective labeling and visual confounders (pose, illumination, expression, demographic variation), which create supervision errors that impair representation learning and degrade downstream prediction performance.

Method: 1. Train baseline classifier and compute self-influence scores using first-order approximation to identify influential training samples with annotation inconsistencies. 2. Apply targeted generative correction via latent diffusion autoencoder to align visual content with assigned labels while preserving identity and realism. 3. Train lightweight predictor of high-influence membership as surrogate influence regularizer for differentiable guidance during correction. 4. Replace original samples with edited ones to create influence-refined dataset of same size.

Result: DiffInf consistently improves generalization across multi-class facial attribute classification compared to: standard noisy-label training, robust optimization baselines, and influence-based filtering methods.

Conclusion: Repairing influential annotation inconsistencies at the image level using generative correction enhances downstream facial attribute classification without sacrificing distributional coverage, demonstrating the effectiveness of targeted image editing over discarding problematic samples.
...
TL;DR: BEiT vision transformers show a bias toward convex completions in perceptual ambiguity tasks, with specific attention heads (like L0H9) governing this preference.


<details>
  <summary>Details</summary>
Motivation: To understand where Gestalt perceptual priors (specifically convexity) are realized within vision transformer architectures, and to identify the specific components that govern figure-ground organization decisions.

Method: Used controlled perceptual conflict with synthetic dart shapes that equally admit concave or convex completions. Projected internal activations into discrete visual codebook space via logit attribution to trace decision-making. Analyzed attention heads' direct effects to identify functional units.

Result: BEiT reliably favors convex completion under competition. Figure-ground organization remains ambiguous through early/intermediate layers but resolves abruptly in later layers. Head L0H9 acts as an early seed introducing weak convexity bias. Downscaling this single head shifts decisions toward concave completions.

Conclusion: Perceptual priors like convexity are governed by identifiable functional units within transformer substructures, with specific attention heads playing crucial roles in figure-ground organization decisions that can be manipulated to shift perceptual outcomes.
...
TL;DR: Non-invasive refraction-aware stereo vision method for measuring fish length in aquariums using YOLOv11-Pose for keypoint detection and refraction-aware 3D triangulation.


<details>
  <summary>Details</summary>
Motivation: Monitoring fish growth is important for health assessment in aquaculture and home aquariums, but traditional methods are invasive or impractical. Fish are small and subject to strong refractive distortions in aquarium environments, making accurate measurement challenging.

Method: Uses YOLOv11-Pose network to detect fish and predict anatomical keypoints in stereo images. Implements refraction-aware epipolar constraint to account for air-glass-water interfaces for robust matching, filters unreliable detections with learned quality score, and performs refraction-aware 3D triangulation to recover 3D keypoints for length measurement.

Result: Validated on a new stereo dataset of endangered Sulawesi ricefish under aquarium-like conditions. Demonstrated that filtering low-quality detections is essential for accurate length estimation.

Conclusion: The proposed system offers a simple, practical solution for non-invasive growth monitoring that can be easily applied in home aquariums, addressing the challenges of refractive distortions in aquatic environments.
...
TL;DR: WanderDream is a large-scale dataset for mental exploration simulation that enables AI models to reason about spatial what-if questions without physical exploration, using panoramic videos and QA pairs across real scenes.


<details>
  <summary>Details</summary>
Motivation: Real-world active exploration is often infeasible due to robot physical constraints or safety concerns for visually impaired users, creating a need for mental simulation of future trajectories to answer spatial what-if questions.

Method: Created WanderDream dataset with two components: WanderDream-Gen (15.8K panoramic videos across 1,088 real scenes showing imagined trajectories) and WanderDream-QA (158K question-answer pairs covering starting states, paths, and end states).

Result: Experiments show: (1) mental exploration is essential for situated reasoning, (2) world models perform well on WanderDream-Gen, (3) imagination significantly improves reasoning on WanderDream-QA, and (4) the data transfers effectively to real-world scenarios.

Conclusion: WanderDream enables AI models to perform emulative mental exploration without physical constraints, demonstrating the importance of imagination for spatial reasoning and providing a valuable benchmark for future research.
...
TL;DR: CaTok is a 1D causal image tokenizer with MeanFlow decoder that enables autoregressive modeling of images by learning causal 1D representations, supporting both fast one-step generation and high-fidelity multi-step sampling.


<details>
  <summary>Details</summary>
Motivation: Current visual tokenizers fail to properly align with autoregressive "next-token prediction" patterns - they either flatten 2D patches into non-causal sequences or use heuristic orderings. Diffusion autoencoders also lack proper causality or introduce imbalance through nested dropout.

Method: CaTok selects tokens over time intervals and binds them to a MeanFlow objective to learn causal 1D representations. It also uses REPA-A regularization to align encoder features with Vision Foundation Models for training stability and acceleration.

Result: Achieves state-of-the-art ImageNet reconstruction results: 0.75 FID, 22.53 PSNR, and 0.674 SSIM with fewer training epochs. The AR model attains performance comparable to leading approaches.

Conclusion: CaTok successfully addresses the challenge of extending autoregressive language modeling to vision by creating a proper causal image tokenizer that supports both efficient generation and high-quality sampling while capturing diverse visual concepts.
...
TL;DR: Pinterest Canvas is a specialized image generation system using task-specific fine-tuning of a foundational diffusion model to meet strict product requirements, achieving significant engagement lifts in online experiments.


<details>
  <summary>Details</summary>
Motivation: While recent image generation models are flexible, they're hard to control via prompting alone and unsuitable for use cases with strict product requirements at Pinterest.

Method: Train a foundational diffusion model on diverse multimodal data, then rapidly fine-tune task-specific variants on specialized datasets for individual use cases like background enhancement and aspect-ratio outpainting.

Result: Online A/B experiments show 18.0% and 12.5% engagement lift for enhanced images; models outperform third-party solutions and human raters validate quality.

Conclusion: The task-specific fine-tuning approach effectively addresses strict product requirements and generalizes to various downstream tasks including multi-image synthesis and image-to-video generation.
...
TL;DR: Systematic analysis of training objectives for denoising-based generative models, focusing on loss weighting, output parameterization, and their interactions with data geometry, architecture, and dataset size.


<details>
  <summary>Details</summary>
Motivation: To understand how different training choices (loss weighting, output parameterization) affect denoising-based generative models, and disentangle factors that matter when training flow matching models for practical guidance.

Method: Systematic numerical study comparing training objectives across synthetic datasets with controlled geometry and image data, analyzing interactions with intrinsic data dimensionality, model architecture, and dataset size.

Result: Analysis using quantitative metrics for denoising accuracy (PSNR across noise levels) and generative quality (FID) to evaluate different training formulations (noise-, clean image-, and velocity-based).

Conclusion: Provides practical insights on design choices for flow matching models by systematically analyzing how training objectives interact with various factors, rather than proposing new methods.
...
TL;DR: GreenRFM is a resource-efficient radiology foundation model that achieves SOTA performance with dramatically reduced computational requirements through principled supervision design rather than brute-force scaling.


<details>
  <summary>Details</summary>
Motivation: Current radiology foundation models rely on brute-force scaling, directly translating methods from natural images that prioritize scale over precision, leading to brittle and expensive models unsuitable for clinical practice.

Method: A resource-efficient pre-training framework with principled supervision design called MUST (More distilled, Ubiquitous, Semantic-enforcing, and Task-aligning) supervision that maximizes supervisory signals rather than just increasing training data quantity.

Result: Two configurations: (1) performant model achieves SOTA using single 24GB GPU in 24 hours; (2) lightweight model matches existing benchmarks with 6GB VRAM in 4 hours. Superior performance on chest/abdominal CT datasets across public/private benchmarks, and transfers to musculoskeletal MRI.

Conclusion: GreenRFM challenges the "scale is all you need" dogma and democratizes equitable development of state-of-the-art radiology foundation models for clinicians, even on consumer hardware like laptops.
...
TL;DR: Match4Annotate: A lightweight framework for propagating point and mask annotations within and across videos using implicit neural representations of DINOv3 features and smooth deformation fields.


<details>
  <summary>Details</summary>
Motivation: Per-frame video annotation is expensive and slow, especially in specialized domains like medical imaging. Existing propagation methods have limitations: trackers require per-video initialization and don't generalize across videos; correspondence methods struggle with low-texture scenes; dense matching lacks spatiotemporal smoothness and unified support for both point and mask annotations.

Method: 1) Fits a SIREN-based implicit neural representation to DINOv3 features at test time to produce continuous, high-resolution spatiotemporal feature fields. 2) Learns smooth implicit deformation fields between frame pairs to guide correspondence matching. 3) Supports both intra-video and inter-video propagation of point and mask annotations.

Result: Achieves state-of-the-art inter-video propagation on three challenging clinical ultrasound datasets, outperforming feature matching and one-shot segmentation baselines. Remains competitive with specialized trackers for intra-video propagation.

Conclusion: Lightweight, test-time-optimized feature matching pipelines like Match4Annotate offer an efficient and accessible solution for scalable annotation workflows in specialized domains like medical imaging.
...
TL;DR: Self-Flow is a self-supervised flow matching method that learns strong semantic representations alongside generative capabilities through dual-timestep scheduling, eliminating dependence on external models.


<details>
  <summary>Details</summary>
Motivation: Current diffusion/flow models rely on external models for semantic representations, which require separate training, have misaligned objectives, and show unexpected scaling behavior. This dependence stems from the denoising objective providing little incentive to learn semantic representations internally.

Method: Introduces Self-Flow with Dual-Timestep Scheduling that applies heterogeneous noise levels across tokens, creating information asymmetry that forces the model to infer missing information from corrupted inputs, driving representation learning within the generative framework.

Result: The method generalizes across modalities (image, video, audio), enables multi-modal training, follows expected scaling laws, and achieves superior generation quality compared to approaches relying on external models.

Conclusion: Self-Flow successfully integrates representation learning within generative models through self-supervised flow matching, eliminating the need for external models while improving convergence, generation quality, and scaling behavior across multiple modalities.
...
TL;DR: SCAN is a universal XAI framework that generates high-resolution self-confidence maps for both CNNs and Transformers using AutoEncoder-based feature reconstruction guided by Information Bottleneck principle.


<details>
  <summary>Details</summary>
Motivation: Current XAI methods face a trade-off between fidelity (architecture-specific methods) and applicability (universal methods), resulting in abstract/fragmented explanations and difficulty comparing explanatory power across different model families like CNNs and Transformers.

Method: SCAN uses an AutoEncoder-based approach to reconstruct features from intermediate layers, guided by Information Bottleneck principle to generate high-resolution Self-Confidence Maps that identify information-rich regions. Works for both CNNs and Transformers.

Result: Extensive experiments show SCAN consistently achieves outstanding performance on quantitative metrics (AUC-D, Negative AUC, Drop%, Win%) and produces significantly clearer, object-focused explanations than existing methods.

Conclusion: SCAN provides a unified framework that is both architecturally universal and highly faithful, enhancing model transparency and offering a more reliable tool for understanding complex neural network decision-making processes.
...
TL;DR: AV-Unified is a unified framework for joint learning across diverse audio-visual scene understanding tasks by standardizing inputs/outputs into token sequences and using multi-scale spatiotemporal perception with cross-modal guidance.


<details>
  <summary>Details</summary>
Motivation: Current audio-visual scene understanding tasks (event localization, parsing, segmentation, QA) are explored individually, making it hard to comprehensively understand complex scenes and explore inter-task relationships. There's a need for a unified approach.

Method: 1) Standardizes diverse task inputs/outputs into sequences of discrete tokens for shared representation; 2) Multi-scale temporal perception module for varying event granularity; 3) Cross-modal guidance-based spatial perception module to overcome lack of auditory supervision; 4) Task-specific text prompts for adaptability.

Result: Extensive experiments on benchmark datasets (AVE, LLP, MUSIC-AVQA, VGG-SS, AVS) demonstrate effectiveness across temporal, spatial, and spatiotemporal audio-visual understanding tasks.

Conclusion: AV-Unified provides a comprehensive framework for joint learning across diverse audio-visual scene understanding tasks, enabling better exploration of inter-task relationships and more holistic scene understanding.
...
TL;DR: A training-free method for modeling linguistic negation in diffusion models by treating it as structured feasibility constraints on semantic guidance, enabling robust negation compliance without retraining.


<details>
  <summary>Details</summary>
Motivation: Negation is a fundamental linguistic operator that remains inadequately modeled in diffusion-based generative systems, creating a gap in handling diverse negation phenomena like object absence, graded semantics, and scope-sensitive disambiguation.

Method: Reinterpret classifier-free guidance as defining semantic update directions and enforce negation by projecting updates onto convex constraint sets derived from linguistic structure, creating a unified framework without heuristics or parameter retraining.

Result: The method achieves robust negation compliance while preserving visual fidelity and structural coherence, works with pretrained diffusion backbones, and extends from images to video trajectories. A structured negation benchmark suite is also introduced.

Conclusion: This establishes the first unified formulation of linguistic negation in diffusion-based generative models beyond representation-level evaluation, providing a principled approach to handle diverse negation phenomena in a training-free manner.
...
TL;DR: SurgFormer is a transformer-based model for real-time soft tissue simulation on volumetric meshes, handling both standard deformation and cut-conditioned cases with XFEM supervision.


<details>
  <summary>Details</summary>
Motivation: High-fidelity biomechanical solvers are too slow for interactive surgical simulation, requiring a data-driven approach that can predict tissue deformation at near real-time rates while handling both standard deformation and surgical cuts.

Method: Uses a multiresolution gated transformer architecture with fixed mesh hierarchy, multibranch blocks combining local message passing, coarse global self-attention, and feedforward updates, fused by learned gates. For cuts, encodes resection information as learned embeddings.

Result: Achieves strong accuracy with favorable efficiency across diverse baselines, making it practical for both standard deformation prediction and topology-altering surgical cases.

Conclusion: SurgFormer provides a scalable, unified backbone for real-time surgical simulation that handles both standard deformation and cut-conditioned cases within the same volumetric pipeline, enabled by novel surgical datasets with XFEM supervision.
...
TL;DR: This paper analyzes data redundancy as a quality factor in autonomous vehicle datasets, showing that removing redundant labels from overlapping camera views and multimodal sensors can improve object detection performance.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicle research has focused on algorithm design while overlooking data quality issues like redundancy. The paper aims to study redundancy as a fundamental but underexplored data quality factor in AV datasets.

Method: The researchers model and measure redundancy in multisource camera data and multimodal image-LiDAR data using nuScenes and Argoverse 2 datasets. They evaluate how removing redundant labels affects YOLOv8 object detection performance.

Result: Selectively removing redundant multisource image object labels improves detection: in nuScenes, mAP50 gains from 0.66 to 0.70, 0.64 to 0.67, and 0.53 to 0.55 on three overlap regions. In AV2, 4.1-8.6% of labels are removed while mAP50 stays near baseline. Multimodal analysis reveals substantial redundancy between image and LiDAR data.

Conclusion: Redundancy is a measurable and actionable data quality factor with direct implications for AV performance. The work highlights redundancy's role in AV perception and motivates a data-centric perspective for evaluating and improving AV datasets.
...
TL;DR: EgoReasoner: A two-stage framework for egocentric 4D reasoning that adapts reasoning scaffolds and reward signals to different cognitive tasks, achieving state-of-the-art performance on HD-EPIC benchmark.


<details>
  <summary>Details</summary>
Motivation: Egocentric video understanding is complex due to dynamic 4D environments where camera motion and object displacements require continuous spatial re-evaluation. Existing task-agnostic approaches (generic Chain-of-Thought and uniform reinforcement learning) are insufficient for diverse cognitive tasks like spatial anchoring, temporal tracking, and duration reasoning.

Method: Two-stage framework: 1) Task-Adaptive Thinking Templates guide structured CoT traces for supervised fine-tuning, teaching adaptive reasoning across task types. 2) Task-aware reward functions verify entity grounding, temporal alignment, and logical consistency for reinforcement fine-tuning with GRPO, selectively strengthening each reasoning pathway.

Result: The 3B-parameter model trained on only 16K samples achieves 37.5% average accuracy on the challenging HD-EPIC benchmark, surpassing Qwen2.5-VL-7B (25.7%) by over 10 points.

Conclusion: Aligning both reasoning scaffolds and reward signals to each task's cognitive structure enables effective egocentric 4D reasoning, demonstrating that task-aware adaptation outperforms generic approaches for complex spatial-temporal reasoning tasks.
...
TL;DR: Penguin-VL is a compact Vision Language Model that replaces contrastive-pretrained vision encoders with a text-only LLM initialization, achieving superior performance on dense captioning and complex reasoning tasks while being efficient for mobile/edge deployment.


<details>
  <summary>Details</summary>
Motivation: Current VLMs rely on scaling model size and contrastive-pretrained vision encoders (CLIP/SigLIP), which are inefficient for mobile/edge devices and suppress fine-grained visual cues needed for dense captioning and complex reasoning.

Method: Penguin-VL initializes its vision encoder from a text-only LLM instead of contrastive-pretrained encoders, addressing the objective mismatch where contrastive learning enforces coarse category-level invariances that suppress fine-grained visual details.

Result: Penguin-VL achieves performance comparable to leading VLMs (Qwen3-VL) in mathematical reasoning and surpasses them in document understanding, visual knowledge, and multi-perspective video understanding, despite using lightweight architecture.

Conclusion: Improved visual representation rather than model scaling is the primary driver of VLM performance. Penguin-Encoder serves as a superior drop-in alternative to contrastive-pretrained encoders, enabling high performance in resource-constrained settings while preserving fine-grained spatial and temporal cues.
...
TL;DR: Omni-Diffusion is the first any-to-any multimodal language model built entirely on mask-based discrete diffusion models, unifying understanding and generation across text, speech, and images.


<details>
  <summary>Details</summary>
Motivation: Current multimodal LLMs mainly use autoregressive architectures, leaving room for exploring more effective and efficient alternatives. Discrete diffusion models have shown promise in visual understanding and generation, suggesting they could be a strong backbone for multimodal systems.

Method: Uses a unified mask-based discrete diffusion model to directly capture the joint distribution over discrete multimodal tokens. This approach supports bimodal tasks and more complex multi-modal scenarios.

Result: Outperforms or performs on par with existing multimodal systems that process two or more modalities across diverse benchmarks.

Conclusion: Demonstrates significant promise of diffusion models in powering next-generation multimodal foundation models, offering an effective alternative to conventional autoregressive architectures.
...
TL;DR: MLLM classification performance gaps with supervised models are largely due to flawed evaluation protocols and noisy ground truth, not genuine model deficiencies. Fixing these issues narrows the performance gap significantly.


<details>
  <summary>Details</summary>
Motivation: Conflicting conclusions in MLLM vs supervised model comparisons stem from evaluation protocols that either inflate or underestimate performance, and from poor ground truth quality. The research aims to identify and fix these issues to get accurate performance assessments.

Method: Identified and fixed key issues in common evaluation protocols: handling model outputs outside class lists, addressing weak multiple-choice distractors, and improving output mapping. Quantified impact of overlooked design choices (batch size, image ordering, text encoder). Evaluated on ReGT, a multilabel reannotation of 625 ImageNet-1k classes. Conducted controlled case study on MLLM-assisted human annotation.

Result: MLLMs benefit most from corrected labels (up to +10.8% improvement), substantially narrowing the perceived gap with supervised models. Models less reliant on supervised training are most sensitive to annotation quality. MLLMs can assist human annotators: in difficult cases, annotators confirmed or integrated MLLM predictions in ~50% of cases.

Conclusion: Much of the reported MLLM underperformance on classification is an artifact of noisy ground truth and flawed evaluation protocols rather than genuine model deficiency. MLLMs show potential for large-scale dataset curation through human-in-the-loop annotation assistance.
...
TL;DR: Traversal-as-Policy distills LLM execution logs into Gated Behavior Trees (GBTs) where tree traversal replaces unconstrained generation, improving success rates while eliminating safety violations through deterministic gating mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current autonomous LLM agents fail because long-horizon policies remain implicit in model weights and transcripts, and safety measures are retrofitted post hoc rather than being built into the control policy from the start.

Method: Distill sandboxed execution logs into executable Gated Behavior Trees (GBTs) where each node encodes state-conditioned action macros mined from successful trajectories. Unsafe traces attach deterministic pre-execution gates over structured tool context and bounded history. At runtime, a lightweight traverser matches intent to child macros, executes one macro at a time under gating, and performs risk-aware shortest-path recovery when stalled.

Result: GBT improves success rates while driving violations toward zero and reduces costs. On SWE-bench Verified (500 issues), GBT-SE raises success from 34.6% to 73.6%, reduces violations from 2.8% to 0.2%, and cuts token usage from 208k to 126k. With distilled trees, 8B executors more than double success on SWE-bench Verified (14.0%→58.8%) and WebArena (9.1%→37.3%).

Conclusion: Traversal-as-Policy with Gated Behavior Trees provides a structured approach to LLM agent control that improves reliability, safety, and efficiency by replacing unconstrained generation with deterministic tree traversal and safety gating mechanisms.
...
TL;DR: JAWS is a probabilistic regularization method for neural operator learning that adaptively modulates regularization strength based on local physical complexity, improving stability while preserving gradients near singular features.


<details>
  <summary>Details</summary>
Motivation: Autoregressive surrogate models for dynamical systems suffer from instability and spectral blow-up. Global regularization techniques uniformly damp high-frequency features (contraction-dissipation dilemma), and long-horizon trajectory optimization methods are bottlenecked by memory constraints.
...
Result: JAWS serves as an effective spectral pre-conditioner, reducing the base operator's burden of handling high-frequency instabilities. This enables memory-efficient, short-horizon trajectory optimization to match or exceed long-term accuracy of long-horizon baselines. On 1D viscous Burgers' equation, it improves long-term stability, shock fidelity, and out-of-distribution generalization while reducing training costs.

Conclusion: The spatially-adaptive prior approach effectively addresses the contraction-dissipation dilemma in neural operator learning, enabling more stable and efficient surrogate models for dynamical systems with improved handling of singular features.
...
TL;DR: VDCook is a self-evolving video data operating system that enables researchers to create specialized video datasets through natural language queries, combining real video retrieval with controlled synthesis, with continuous updates and comprehensive metadata.


<details>
  <summary>Details</summary>
Motivation: Traditional video datasets are static and one-time-built, making it difficult for researchers and domain teams to create specialized, up-to-date training data. There's a need for a system that lowers the barrier to constructing domain-specific video datasets while supporting continuous updates and community contributions.

Method: Users make natural language queries with adjustable parameters (scale, retrieval-synthesis ratio, quality threshold). The system performs query optimization and concurrently runs real video retrieval and controlled synthesis modules. It uses MCP (Model Context Protocol) for automated data ingestion and provides multi-dimensional metadata annotation including scene segmentation, motion scoring, OCR ratio, and automatic captioning.

Result: The system generates in-domain data packages with complete provenance and metadata, along with reproducible Notebooks. It transforms datasets into dynamically evolving open ecosystems that support continuous updates and domain expansion through community contributions.

Conclusion: VDCook significantly lowers the barrier to constructing specialized video training datasets through infrastructure-level solutions, supporting a governance-enabled data expansion paradigm that moves beyond traditional static datasets to create evolving data ecosystems.
...
TL;DR: IntSeqBERT: Dual-stream Transformer for OEIS integer sequences using magnitude and modulo embeddings, achieving 95.85% magnitude accuracy and 50.38% modulo accuracy, with 7.4x improvement in next-term prediction over tokenized baselines.


<details>
  <summary>Details</summary>
Motivation: Standard tokenized models struggle with OEIS sequences due to out-of-vocabulary values and inability to exploit periodic arithmetic structure in sequences spanning from small constants to astronomical values.

Method: Dual-stream Transformer encoder with continuous log-scale magnitude embeddings and sin/cos modulo embeddings for 100 residues (moduli 2-101), fused via FiLM. Three prediction heads: magnitude regression, sign classification, and modulo prediction for 100 moduli.

Result: 95.85% magnitude accuracy and 50.38% Mean Modulo Accuracy (MMA), outperforming tokenized Transformer by +8.9pt and +4.5pt. CRT-based Solver yields 7.4x improvement in next-term prediction (19.09% vs 2.59%). Strong correlation between information gain and Euler's totient ratio.

Conclusion: IntSeqBERT effectively models OEIS sequences by combining magnitude and modular arithmetic representations, with modulo stream providing significant performance gains. Composite moduli capture arithmetic structure more efficiently via CRT aggregation.
...
TL;DR: Photonic chaotic decision-making systems show autocorrelation-dependent performance in multi-armed bandit problems, with optimal autocorrelation sign depending on reward environment richness.


<details>
  <summary>Details</summary>
Motivation: To understand whether the observed autocorrelation benefits in photonic chaotic decision-making systems can be explained by a minimal mathematical model, and to clarify the relationship between autocorrelation properties and decision accuracy in different reward environments.

Method: Analysis of a stochastic-process model using the tug-of-war principle for solving two-armed bandit problems, where threshold and two-valued Markov signal evolve jointly. Numerical investigation of how autocorrelation affects decision performance.

Result: Reveals environment-dependent structure: negative autocorrelation is optimal in reward-rich environments (sum of winning probabilities > 1), positive autocorrelation is optimal in reward-poor environments (sum < 1), and performance is independent of autocorrelation when sum equals 1.

Conclusion: The study provides mathematical clarification of autocorrelation effects in decision-making systems, paving the way for improved reinforcement learning applications in wireless communications and robotics by optimizing temporal correlation properties.
...
TL;DR: CT-KAE is a lightweight Koopman autoencoder for ocean forecasting that uses linear latent dynamics for stable long-term predictions with bounded errors, outperforming Transformers and being much faster than numerical solvers.
...
Method: Continuous-Time Koopman Autoencoder (CT-KAE) projects nonlinear ocean dynamics into a latent space governed by a linear ODE, enabling resolution-invariant forecasting via matrix exponential formulation for the two-layer quasi-geostrophic system.

Result: CT-KAE shows bounded error growth and stable large-scale statistics over 2083-day rollouts, maintains consistent bulk energy spectra, enstrophy evolution, and autocorrelation structure, while being orders-of-magnitude faster than numerical solvers.

Conclusion: Continuous-time Koopman surrogates offer a promising backbone for efficient and stable hybrid physical-machine learning climate models, balancing accuracy, stability, and computational efficiency for long-term ocean forecasting.
...
TL;DR: AI equalizes individual skill differences but may increase aggregate inequality by shifting value to complementary assets, with effects depending on technology structure and labor markets.


<details>
  <summary>Details</summary>
Motivation: To resolve the paradox where AI reduces individual skill differences yet potentially increases overall inequality, by examining how AI shifts economic value toward concentrated complementary assets.

Method: Develop a task-based model with endogenous education, employer screening, and heterogeneous firms; use Method of Simulated Moments with six empirical targets for calibration; conduct sensitivity analysis; test with occupation-level regressions using BLS OEWS data.

Result: Model yields two regimes depending on AI's technology structure (proprietary vs. commodity) and labor market institutions; calibration shows aggregate inequality sign depends on specific parameters, with AI's technology structure independently crossing the boundary between regimes.

Conclusion: The main contribution is identifying the mechanism (not determining the sign of inequality effect); current occupation-level data cannot test task-level predictions, requiring within-occupation, within-task panel data that doesn't yet exist at scale.
...
TL;DR: CDDS is a novel cross-modal alignment method that uses constrained decoupling and distribution sampling to achieve better semantic consistency between vision and language by separating semantic from non-semantic information.


<details>
  <summary>Details</summary>
Motivation: Traditional cross-modal alignment methods focus on embedding consistency but ignore non-semantic information in embeddings. Simply decoupling embeddings into semantic and modality components introduces challenges: no clear standard for distinguishing semantic vs modal information, and modality gaps can cause semantic alignment deviation or information loss.

Method: CDDS (Constrained Decoupling and Distribution Sampling) uses: (1) A dual-path UNet to adaptively decouple embeddings with multiple constraints for effective separation, and (2) A distribution sampling method to bridge the modality gap and ensure alignment rationality.

Result: Extensive experiments on various benchmarks and model backbones show CDDS outperforms state-of-the-art methods by 6.6% to 14.2%.

Conclusion: CDDS effectively addresses the limitations of traditional cross-modal alignment by properly separating semantic from non-semantic information and bridging modality gaps, achieving superior semantic consistency between vision and language.
...
<details>
  <summary>Details</summary>
Motivation: Existing dual-target drug design approaches use staged pipelines that either decouple binding poses via conditional-independence assumptions or enforce overly rigid correlations, failing to jointly generate two target-specific binding modes needed for effective polypharmacological therapies.

Method: FuseDiff uses an end-to-end diffusion model with a message-passing backbone featuring Dual-target Local Context Fusion (DLCF) that fuses each ligand atom's local context from both pockets, enabling expressive joint modeling while preserving symmetries. It includes explicit bond generation to enforce topological consistency across poses while allowing target-specific geometric adaptation.
...
Conclusion: FuseDiff successfully addresses limitations of existing approaches by jointly generating ligand molecular graphs and pocket-specific binding poses, advancing dual-target structure-based drug design for polypharmacological therapies with improved efficacy and reduced resistance.
...
TL;DR: The paper analyzes expressivity bounds of scalable sequence models using Lie-algebraic control theory, showing that error diminishes exponentially with depth.


<details>
  <summary>Details</summary>
Motivation: To understand the fundamental limits of scalable sequence models (Transformers, structured state-space models) that trade expressivity for parallel training efficiency, and to characterize their error behavior when operating outside expressivity regimes.

Method: Uses Lie-algebraic control theory to formulate correspondence between model depth and Lie algebra extensions, characterizes Lie-algebraic class of constant-depth sequence models, and analytically derives approximation error bounds.

Result: Shows that approximation error diminishes exponentially as model depth increases, consistent with empirical performance. Validates theory with experiments on symbolic word and continuous-valued state-tracking problems.

Conclusion: Lie-algebraic framework provides theoretical foundation for understanding expressivity bounds of scalable sequence models, explaining their strong empirical performance despite architectural constraints for parallel training.
...
TL;DR: A hybrid heuristic-reinforcement learning framework for solving railcar shunting problems with two-sided track access using two locomotives.


<details>
  <summary>Details</summary>
Motivation: Railcar shunting in freight railyards is complex, especially with two-sided classification tracks that allow FIFO access. Traditional planning is combinatorially challenging, requiring efficient solutions for assembling multiple outbound trains with limited resources.

Method: Decompose the two-sided, two-locomotive problem into two single-sided subproblems. Develop a Hybrid Heuristic-Reinforcement Learning (HHRL) framework integrating railway-specific heuristics with Q-learning. Use methods to reduce state-action space and guide exploration during reinforcement learning.

Result: Numerical experiments show the HHRL algorithm is efficient and produces high-quality solutions for both one-sided access (single locomotive) and two-sided access (two locomotive) problems.

Conclusion: The HHRL framework effectively addresses complex railcar shunting problems by combining domain-specific heuristics with reinforcement learning, demonstrating practical applicability for real-world railyard operations.
...
TL;DR: GeoAI Hybrid framework integrates MGWR, Random Forest, and ST-GCN to model multimodal traffic flow patterns and land use interactions, achieving superior predictive performance and identifying key urban traffic typologies.


<details>
  <summary>Details</summary>
Motivation: Conventional models fail to capture the complex, nonlinear interactions between land use configuration and spatiotemporally heterogeneous mobility demand across multiple travel modes simultaneously.

Method: Sequential integration of Multiscale Geographically Weighted Regression (MGWR), Random Forest (RF), and Spatio-Temporal Graph Convolutional Networks (ST-GCN) to model spatiotemporal heterogeneity of traffic flow across motor vehicle, public transit, and active transport modes.

Result: Achieved RMSE of 0.119 and R² of 0.891 (outperforming benchmarks by 23-62%); identified land use mix as strongest predictor for motor vehicle flows and transit stop density for public transit; discovered five distinct urban traffic typologies; showed 72% reduction in spatial autocorrelation residuals.

Conclusion: The GeoAI Hybrid framework provides an interpretable, scalable toolkit for evidence-based multimodal mobility management and land use policy design, with strong within-cluster transferability but limited cross-cluster generalizability due to urban morphological context.
...
TL;DR: BISE extracts bias-free subnetworks from standard trained models via pruning, enabling debiasing without retraining or additional data.


<details>
  <summary>Details</summary>
Motivation: Existing debiasing techniques often require complex training procedures or dataset manipulation. The paper explores whether fair, bias-agnostic subnetworks can be extracted from standard vanilla-trained models without needing unbiased training data.

Method: Bias-Invariant Subnetwork Extraction (BISE) identifies and isolates "bias-free" subnetworks that already exist within conventionally trained models through pruning techniques, without retraining or finetuning original parameters.

Result: The approach successfully extracts subnetworks that rely less on biased features while maintaining robust performance. Extensive experiments on common benchmarks show advantages in both performance and computational efficiency.

Conclusion: BISE enables efficient bias mitigation through structural adaptation of pre-trained neural networks via parameter removal, offering a cost-effective alternative to data-centric approaches or full model retraining.
...
TL;DR: Tokeniser pretraining improves physics emulation accuracy and efficiency, with domain-aligned pretraining providing the largest gains.


<details>
  <summary>Details</summary>
Motivation: Physics foundation models need to learn both data compression and physical dynamics simultaneously, which can be inefficient when trained from scratch. There's a need to improve training efficiency and accuracy for physics emulation tasks.
...
Result: In-domain pretraining reduces VRMSE by 64% after 10,500 training steps compared to training from scratch. Domain-aligned pretraining yields largest improvements, while cross-domain pretraining provides moderate gains.

Conclusion: Tokeniser pretraining significantly enhances physics emulation efficiency, with strategic pretraining data selection being crucial. This provides practical guidance for training efficient physics emulators.
...
TL;DR: A framework for identifying attacker characteristics from observed attacks in ML systems, addressing the non-identifiability problem and enabling better defense strategies.


<details>
  <summary>Details</summary>
Motivation: ML models in automated decision-making systems are vulnerable to data-manipulation attacks. Current defenses focus on the attack or model, but identifying the attacker could improve defense effectiveness and enable better mitigation strategies.

Method: Proposes a domain-agnostic framework for identifying the most probable attacker from observed attacks. Addresses the non-identifiability problem (multiple attackers could perform same attack) and provides methodology applicable across various ML learners.

Result: Demonstrates that attackers are non-identifiable without additional knowledge, but the proposed framework can identify the most probable attacker. Shows applicability through specific instantiations on various learners.

Conclusion: Identifying attacker characteristics enables two defense improvements: 1) exogenous mitigation by altering system design or limiting attacker capabilities, and 2) better performance of adversarial regularization when attacker-specific knowledge is incorporated.
...
TL;DR: Using knowledge graph embeddings improves NBA salary prediction by capturing complex on/off-court relationships that traditional tabular data misses.


<details>
  <summary>Details</summary>
Motivation: Traditional supervised learning on tabular data fails to accurately predict NBA player salaries, especially for veterans and high-earners, due to the complex interplay of performance and contextual factors that aren't captured in simple statistics.

Method: Build a knowledge graph with on-court and off-court data, embed the graph into vector space using various graph embedding algorithms, and incorporate these embeddings as features in supervised learning models for salary prediction.

Result: Knowledge graph embeddings significantly improve salary prediction accuracy compared to traditional tabular approaches, with different embedding algorithms showing varying effectiveness in capturing the complex factors affecting NBA salaries.

Conclusion: Incorporating knowledge graph embeddings is essential for accurate NBA salary prediction as it enables models to understand the complex landscape of factors influencing player compensation beyond simple performance statistics.
...
TL;DR: Researchers use reinforcement learning to find power network parameters with many equilibrium points, discovering configurations with more solutions than average baselines.


<details>
  <summary>Details</summary>
Motivation: Current computational algebra methods can't handle power flow equations for large networks. The paper aims to find network parameters with many equilibrium points (real solutions) to understand system stability and complexity.

Method: Design a probabilistic reward function approximating root count, create state-space mimicking power flow equations, derive average root count for Gaussian model as baseline, and use reinforcement learning agents to explore the space.

Result: RL agents discover instances of power flow equations with significantly more solutions than the average baseline, demonstrating RL's effectiveness for this problem.

Conclusion: Reinforcement learning shows potential for power-flow network design/analysis and can meaningfully contribute to complex non-linear algebra/geometry problems that traditional computational methods struggle with.
...
TL;DR: Weak-to-strong generalization can substantially improve scaling laws - a strong student trained on imperfect labels from a weak teacher can achieve better scaling than the teacher, sometimes reaching minimax optimal rates even when the teacher's error doesn't decay.


<details>
  <summary>Details</summary>
Motivation: To understand the theoretical potential of weak-to-strong generalization where strong models are trained on labels from weaker models, and to quantify how much this two-stage procedure can improve scaling laws in machine learning.

Method: Analyze weak-to-strong generalization using random feature ridge regression (RFRR) framework. Derive a deterministic equivalent for the excess test error of the student trained on teacher-generated labels, enabling theoretical analysis of scaling laws.

Result: The student can achieve improved scaling laws compared to the teacher in both bias-dominated and variance-dominated regimes. Remarkably, the student may attain minimax optimal rates even when the teacher's test error doesn't decay with sample size.

Conclusion: Weak-to-strong generalization offers substantial theoretical benefits, enabling strong students to surpass teacher performance and achieve optimal scaling laws despite being trained on imperfect labels, with implications for practical machine learning pipelines.
...
TL;DR: Moore machines can be exactly realized as state-space models (SSMs), establishing a formal correspondence between symbolic automata and continuous ML architectures, but SSMs require much more data and fail to learn state structure compared to symbolic methods.


<details>
  <summary>Details</summary>
Motivation: To establish a formal connection between symbolic automata theory and continuous machine learning architectures (SSMs), enabling principled exploitation of symbolic structure in continuous domains for more efficient learning of complex systems.

Method: Prove that Moore machines can be exactly realized as SSMs, compare SSM training with passive/active automata learning, recover automata from SYNTCOMP benchmark, and combine automata learning with SSMs by initializing SSMs with symbolically-learned approximations.

Result: SSMs require orders of magnitude more data than symbolic methods and fail to learn state structure; initializing SSMs with symbolically-learned approximations leads to 2-5 times faster convergence and better overall model accuracies on test data.

Conclusion: Symbolic structure provides a strong inductive bias for learning these systems, and combining automata learning with SSMs enables more efficient learning of complex systems by lifting automata learning out of purely discrete spaces.
...
TL;DR: UDA improves radioisotope classifier generalization from synthetic to real data using unlabeled target data and MMD minimization.


<details>
  <summary>Details</summary>
Motivation: Radioisotope identification using gamma spectroscopy is challenging due to difficulty acquiring large labeled experimental datasets. Models trained on simulated data often fail when deployed in real-world environments due to domain shift.

Method: First pretrain a spectral classifier using labeled synthetic data, then use unlabeled target data to align feature representations between source and target domains via unsupervised domain adaptation. Compare various UDA techniques, finding MMD minimization between source and target feature vectors works best.

Result: MMD-based UDA improved testing accuracy from 0.754 ± 0.014 to 0.904 ± 0.022 on experimental LaBr3 test set using a custom transformer-based neural network.

Conclusion: UDA can effectively adapt radioisotope classifiers trained on synthetic data for real-world deployment by leveraging unlabeled target data to bridge the domain gap.
...
TL;DR: BoN sampling is statistically optimal for win-rate (not expected reward), explaining its practical success. A simple variant eliminates reward hacking while maintaining optimality.


<details>
  <summary>Details</summary>
Motivation: Prior theoretical work suggests BoN sampling is statistically suboptimal and vulnerable to reward hacking, but these analyses focus on expected true reward rather than win-rate, which better reflects how reward models are actually trained and evaluated in practice.

Method: Re-analyze BoN sampling under more realistic assumptions focusing on win-rate (pairwise comparison metric). Show that properly tuned BoN is optimal for win-rate under minimal quality conditions. Propose a simple variant that eliminates reward hacking while maintaining optimal statistical performance.

Result: BoN sampling is both computationally and statistically optimal for achieving high win-rate when properly tuned, explaining its widespread practical success. The proposed variant provably eliminates reward hacking while maintaining optimal statistical performance. Prior approaches are provably suboptimal when considering win-rate.

Conclusion: BoN sampling's practical success is explained by its optimality for win-rate (not expected reward). The choice of objective (win-rate vs expected reward) is crucial when analyzing inference-time alignment methods. A simple variant can eliminate reward hacking while preserving optimality.
...
TL;DR: MIRACL is a hierarchical Meta-MORL framework for few-shot generalization across diverse multi-objective tasks, achieving better performance than conventional MORL methods.


<details>
  <summary>Details</summary>
Motivation: Multi-objective reinforcement learning (MORL) is effective for supply chain optimization but faces deployment challenges in dynamic environments due to task-specific retraining requirements and high computational costs.

Method: MIRACL decomposes tasks into structured subproblems for efficient policy adaptation and meta-learns a global policy across tasks using Pareto-based adaptation strategy to encourage diversity in meta-training and fine-tuning.

Result: MIRACL outperforms conventional MORL baselines in simple to moderate tasks, achieving up to 10% higher hypervolume and 5% better expected utility.

Conclusion: MIRACL demonstrates potential for robust, efficient adaptation in multi-objective problems and is theoretically domain-agnostic for broader dynamic multi-objective decision-making applications.
...
TL;DR: SGPP is a unified framework for controlling Rectified Flow models that bridges deterministic optimization and stochastic sampling, enabling precise tasks like semantic editing and image recovery without geometric locking or computational instability.


<details>
  <summary>Details</summary>
Motivation: Current approaches for controlling RF models have limitations: inversion-based guidance suffers from "geometric locking" by rigidly adhering to source trajectories, while posterior sampling approximations (like DPS) are computationally expensive and unstable. There's a need for a better framework that can handle precise tasks like semantic editing and blind image recovery.

Method: Proposes Score-Guided Proximal Projection (SGPP), which reformulates recovery tasks as proximal optimization problems. Defines an energy landscape balancing input fidelity with realism from pre-trained score fields. Theoretically proves this induces normal contraction property, geometrically guaranteeing out-of-distribution inputs are snapped onto the data manifold. Generalizes state-of-the-art editing methods and enables "soft guidance" through proximal variance relaxation.

Result: SGPP bridges the gap between deterministic optimization and stochastic sampling, avoids geometric locking, reduces computational expense and instability compared to posterior sampling methods, and provides a continuous training-free trade-off between strict identity preservation and generative freedom.

Conclusion: SGPP offers a unified framework for controlling RF models that overcomes limitations of existing methods, enabling precise editing and recovery tasks with theoretical guarantees and practical flexibility through soft guidance mechanisms.
...
TL;DR: TML-Bench is a tabular benchmark for evaluating data science agents on Kaggle-style tasks, testing 10 OSS LLMs across four competitions with varying time budgets, measuring success rates and performance variability.


<details>
  <summary>Details</summary>
Motivation: While autonomous coding agents can quickly produce strong tabular baselines, their practical value depends on end-to-end correctness and reliability under time constraints, necessitating a benchmark to evaluate these capabilities systematically.

Method: The authors introduce TML-Bench, evaluating 10 open-source LLMs on four Kaggle competitions with three time budgets (240s, 600s, 1200s). Each model runs five times per task and budget, with success defined as producing a valid submission and achieving a private-holdout score on hidden labels.

Result: MiniMax-M2.1 achieves the best aggregate performance across all four competitions. Average performance improves with larger time budgets, though scaling shows noise for some individual models at the current run count.

Conclusion: TML-Bench provides a valuable framework for evaluating data science agents, demonstrating that performance generally improves with more time, but reliability and consistency remain important considerations for practical deployment.
...
TL;DR: SCORE method improves domain generalization in model merging by resolving subspace conflicts through shared orthogonal basis projection.


<details>
  <summary>Details</summary>
Motivation: Domain generalization in model merging is underexplored, and merging models fine-tuned on distinct domains creates stronger subspace conflicts than traditional multi-task settings, which hurts generalization to unseen domains.

Method: SCORE (Subspace COnflict-Resolving mErging) finds a shared orthogonal basis by computing principal components of concatenated leading singular vectors, then projects each task matrix into this basis and prunes off-diagonal components to remove conflicting singular directions.

Result: SCORE consistently outperforms existing model merging approaches in domain generalization settings across various architectures and model scales, demonstrating effectiveness and scalability.

Conclusion: SCORE effectively addresses subspace conflicts in domain generalization model merging, providing a scalable solution that improves performance on unseen domains.
...
TL;DR: Novel federated learning algorithm for distributed stochastic minimax optimization with constraints achieves O(ε⁻⁴) complexity, handles partial client participation, and provides stable optimization of worst-case client performance.


<details>
  <summary>Details</summary>
Motivation: Address distributed stochastic minimax optimization with stochastic constraints in federated learning, overcoming hyperparameter sensitivity and convergence oscillations in traditional primal-dual or penalty-based approaches.

Method: Propose Softmax-Weighted Switching Gradient method - a single-loop primal-only switching mechanism for federated learning, with theoretical analysis for both full and partial client participation regimes.

Result: Achieves O(ε⁻⁴) oracle complexity for ε-optimality and feasibility, establishes tighter lower bound for softmax hyperparameter, provides O(log(1/δ)) high-probability convergence guarantee.

Conclusion: The framework demonstrates stable optimization of worst-case client performance, effectively bypassing hyperparameter sensitivity issues, validated on NP classification and fair classification tasks.
...
TL;DR: MoE models learn fewer, more specialized features compared to dense models with equal active parameters, showing different feature organization patterns.


<details>
  <summary>Details</summary>
Motivation: Mixture of Experts (MoE) models achieve parameter efficiency through sparse routing, but their internal representations are poorly understood compared to dense models. The paper aims to systematically compare MoE and dense model internals to understand their feature organization differences.

Method: Used crosscoders (sparse autoencoder variant) to jointly model multiple activation spaces. Trained 5-layer dense and MoE models (with equal active parameters) on 1B tokens across three domains: code, scientific text, and English stories. Employed BatchTopK crosscoders with explicitly designated shared features to analyze feature organization.

Result: Achieved ~87% fractional variance explained. MoE learns significantly fewer unique features than dense model. MoE-specific features show higher activation density than shared features, while dense-specific features show lower density. MoEs develop more specialized, focused representations, while dense models distribute information across broader, more general-purpose features.

Conclusion: MoE models develop more specialized, focused representations with fewer unique features, while dense models use broader, more general-purpose features. This reveals fundamental differences in how these architectures organize information internally.
...
TL;DR: MoE models rely heavily on a few specialized experts rather than balanced use of all experts, enabling potential inference optimization through expert pruning with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Despite MoE models enabling parameter-efficient scaling, their inference and memory costs remain challenging to optimize due to limited understanding of how experts specialize and contribute to model outputs.

Method: Two complementary approaches: 1) analyzing domain-specific routing patterns, and 2) using an early decoding framework to track expert contributions to output representations. Analysis conducted on DeepSeekMoE model with 64 routed experts (6 active per layer).

Result: MoE models predominantly rely on few specialized experts - top-weighted expert's output closely approximates full ensemble prediction. Few experts handle over 50% of routing decisions across domains. Hidden state similarity between single and ensemble experts is extremely high (cosine similarity up to 0.95), with only 5% perplexity increase when using single expert across domains.

Conclusion: MoE models exhibit concentrated expertise, highlighting opportunities for inference optimization through targeted expert pruning while maintaining performance, and opening avenues for studying localization of learned knowledge in these models.
...
TL;DR: SEA-PEFT automates adapter configuration for few-shot 3D medical image segmentation by treating it as an online allocation problem with search-audit-allocate loop, outperforming fixed-topology PEFT methods.


<details>
  <summary>Details</summary>
Motivation: Adapting foundation models to new clinical sites is challenging due to domain shift, scarce annotations, and lack of AI engineering expertise. Existing PEFT methods require manual configuration or computationally expensive searches that are infeasible for few-shot 3D medical imaging.

Method: SEA-PEFT treats adapter configuration as an online allocation problem solved during fine-tuning. It uses a search-audit-allocate loop: trains active adapters, estimates each adapter's Dice utility by momentarily toggling it off, then reselects the active set under a parameter budget using greedy knapsack allocator. Exponential Moving Average, Interquartile Range smoothing, and Finite-State Ranking controller stabilize the loop.
...
Conclusion: SEA-PEFT successfully automates adapter configuration for few-shot 3D medical image segmentation, addressing practical challenges of domain adaptation in clinical settings where AI engineering expertise is limited.
...
TL;DR: Many-shot prompting for test-time adaptation in LLMs has practical limits - works well for structured tasks with high information gain but is sensitive to selection strategy and limited for open-ended generation.


<details>
  <summary>Details</summary>
Motivation: To understand the reliability and limits of many-shot prompting as a test-time adaptation mechanism for LLMs, particularly for open-source models, since current understanding of this update mechanism is poor despite its common use.

Method: Empirical study across tasks and model backbones analyzing performance variation with update magnitude, example ordering, and selection policy. Also studied Dynamic and Reinforced ICL as alternative test-time update strategies.
...
Conclusion: Characterizes practical limits of prompt-based test-time adaptation and outlines when input-space updates are beneficial versus harmful, providing guidance on effective use of many-shot prompting.
...
TL;DR: STEP reformulates temporal link prediction as sequential forecasting using temporal motifs and Poisson processes, achieving better accuracy and efficiency than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current temporal link prediction methods treat the task as binary classification with negative sampling, which ignores the sequential and correlated nature of real-world interactions in networks.

Method: STEP models event dynamics through discrete temporal motif transitions governed by Poisson processes, maintaining open motif instances that evolve with new interactions. It decides whether to initiate new motifs or extend existing ones using Bayesian scoring of temporal likelihoods and structural priors.

Result: Experiments on five real-world datasets show up to 21% average precision gains over state-of-the-art baselines in classification, 0.99 precision in next k sequential forecasting, and consistently lower runtime than competing motif-aware methods.

Conclusion: STEP provides an effective framework for temporal link prediction by modeling sequential event dynamics through temporal motifs, offering both improved accuracy and computational efficiency while producing compact feature representations compatible with existing graph neural networks.
...
TL;DR: RePO is a new method for molecular optimization that combines reinforcement learning with reference guidance to overcome limitations of SFT and RLVR when only single optimized molecules are available without step-by-step trajectories.


<details>
  <summary>Details</summary>
Motivation: Standard SFT and RLVR methods perform poorly in instruction-based molecular optimization where only single optimized reference molecules are available without step-by-step trajectories. SFT collapses reasoning and RLVR provides sparse feedback due to lack of effective exploration.

Method: Reference-guided Policy Optimization (RePO) combines RL with reference guidance: samples candidate molecules with reasoning trajectories, uses verifiable rewards for property satisfaction under similarity constraints (RL term), and applies reference guidance by keeping policy's intermediate reasoning as context while training only the answer in supervised manner.

Result: RePO consistently outperforms SFT and RLVR baselines (including GRPO) across molecular optimization benchmarks, achieving improvements on Success Rate × Similarity metric, better balance across competing objectives, and better generalization to unseen instruction styles.

Conclusion: RePO effectively addresses the exploration-exploitation trade-off in molecular optimization by combining RL exploration with reference guidance, overcoming limitations of existing methods when only single optimized molecules are available without trajectories.
...
TL;DR: Integrated framework combining node transformer architecture with BERT-based sentiment analysis for stock price forecasting, achieving 0.80% MAPE for one-day-ahead predictions with 65% directional accuracy.
...
Method: Proposes an integrated framework representing stock market as a graph structure (stocks as nodes, relationships as edges). Uses fine-tuned BERT for sentiment analysis from social media, combined with quantitative features via attention-based fusion. Node transformer processes historical data capturing both temporal evolution and cross-sectional dependencies.

Result: Achieves 0.80% MAPE for one-day-ahead predictions vs 1.20% for ARIMA and 1.00% for LSTM. Sentiment analysis reduces error by 10% overall (25% during earnings). Graph-based modeling adds 15% improvement. Directional accuracy reaches 65%. Statistical validation confirms improvements (p < 0.05). Maintains MAPE below 1.5% during high-volatility periods.

Conclusion: The integrated framework combining graph-based modeling with sentiment analysis significantly improves stock price forecasting accuracy, particularly during volatile periods and earnings announcements, demonstrating the value of capturing both structural dependencies and behavioral factors.
...
TL;DR: Artificial Replay (AR) is a new experimental design for comparing bandit algorithms that reuses recorded rewards from one policy to evaluate another, reducing experimental cost and variance.


<details>
  <summary>Details</summary>
Motivation: Comparing multi-armed bandit algorithms requires many independent restarts due to dependent trajectories, making experimentation costly and delaying deployment decisions. Standard A/B testing approaches are inefficient for bandit algorithms.

Method: AR runs one policy and records its trajectory. When evaluating a second policy, it reuses recorded rewards when the second policy selects actions already taken by the first policy, and only queries the real environment for new actions.

Result: The AR estimator is unbiased, requires only T + o(T) user interactions instead of 2T (nearly halving experimental cost), and has sub-linearly growing variance compared to linearly-growing variance in naive designs.

Conclusion: Artificial Replay provides an efficient experimental design for comparing bandit algorithms with theoretical guarantees and practical benefits, as demonstrated with UCB, Thompson Sampling, and ε-greedy policies.
...
TL;DR: SIGReg repurposed from self-supervised learning to stabilize supervised optimization by constraining representations towards isotropic Gaussian, preventing collapse in low-bias architectures like ViTs.


<details>
  <summary>Details</summary>
Motivation: Modern neural networks rely on architectural priors (BatchNorm, Residual connections) for stable training. Without these, or in low-data regimes with aggressive augmentation, low-bias architectures like Vision Transformers suffer from optimization collapse.

Method: Adopts Sketched Isotropic Gaussian Regularization (SIGReg) from LeJEPA framework and repurposes it for supervised learning. Derives computationally efficient Weak-SIGReg variant that targets covariance matrix via random sketching. Views representation collapse as stochastic drift and constrains representation density towards isotropic Gaussian.

Result: SIGRec recovers ViT training on CIFAR-100 from collapsed 20.73% to 72.02% accuracy without architectural hacks. Significantly improves convergence of deep vanilla MLPs trained with pure SGD.

Conclusion: SIGReg serves as an effective general optimization stabilizer for supervised learning, particularly beneficial for low-bias architectures and challenging training regimes where traditional architectural priors are insufficient.
...
TL;DR: OMGD is a memory-efficient optimization method for large language models that achieves better theoretical convergence (Õ(ε⁻³)) than standard methods (O(ε⁻⁴)) while being lightweight and plug-and-play.


<details>
  <summary>Details</summary>
Motivation: Current memory-efficient optimization methods for large language models either lack convergence guarantees or only achieve suboptimal O(ε⁻⁴) iteration complexity, creating a need for methods with both theoretical guarantees and practical efficiency.

Method: Omni-Masked Gradient Descent (OMGD) uses mask traversal for memory-efficient training, allowing it to be integrated as a lightweight, plug-and-play component into most mainstream optimizers.

Result: OMGD achieves a strictly improved iteration complexity of Õ(ε⁻³) for finding ε-approximate stationary points, and empirically shows consistent improvements over competitive baselines in both fine-tuning and pre-training tasks.

Conclusion: OMGD provides a theoretically sound and practically effective solution for memory-efficient training of large language models, offering better convergence guarantees while maintaining ease of integration with existing optimizers.
...
TL;DR: EvoESAP: Evolutionary search framework using ESAP metric to optimize non-uniform expert pruning allocations in SMoE models, improving generation quality while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Current SMoE pruning methods focus on which experts to prune within layers but use uniform layer-wise sparsity allocation, even though allocation strongly affects performance. Need better allocation strategies to reduce memory/throughput costs while preserving model quality.

Method: Decouple pruning into within-layer expert ranking and across-layer budget allocation. Introduce ESAP (Expected Speculative Acceptance Proxy) metric to measure how well pruned models match full models. Use EvoESAP evolutionary search framework to optimize non-uniform layer-wise sparsity allocations under fixed global budget.

Result: Across 7B-30B SMoE LLMs at 25% and 50% sparsity, EvoESAP discovers non-uniform allocations that improve open-ended generation (up to +19.6% on MATH-500 at 50% sparsity) while preserving competitive multiple-choice accuracy compared to uniform pruning.

Conclusion: EvoESAP provides effective plug-and-play method for optimizing expert pruning allocations, demonstrating that non-uniform sparsity allocation significantly improves SMoE model performance while maintaining efficiency benefits.
...
TL;DR: PPO performance plateaus stem from noisy gradient estimates, not exploration or optimization issues. Increasing parallel environments reduces noise and enables scaling to 1M+ environments with monotonic improvement up to 1T transitions.


<details>
  <summary>Details</summary>
Motivation: Address performance plateaus in deep on-policy RL (specifically PPO) that occur not from known challenges like exploration or capacity, but from sample-based loss estimates becoming poor proxies for the true objective during training.

Method: Model PPO's outer loop as stochastic optimization where step size is controlled by regularization strength and gradient noise by samples between updates. Validate model predictions, analyze hyperparameter effects, propose co-scaling recipe for parallelization, and scale PPO to over 1M parallel environments.

Result: Increasing parallel environments reduces both step size and update noise. Proper co-scaling of hyperparameters with parallelization is crucial to avoid performance degradation. Achieved state-of-the-art performance by scaling PPO to >1M parallel environments, enabling monotonic improvement up to 1 trillion transitions.

Conclusion: PPO plateaus arise from noisy gradient estimates, which can be addressed by increasing parallel environments and properly co-scaling hyperparameters. This approach enables unprecedented scaling and performance in complex open-ended domains.
...
TL;DR: The paper improves the degree bound for L1-approximation of concept classes with bounded Gaussian surface area from O(Γ²/ε⁴) to Õ(Γ²/ε²), yielding near-optimal bounds for agnostically learning polynomial threshold functions in the statistical query model.


<details>
  <summary>Details</summary>
Motivation: Previous work by Klivans et al. (2008) established that degree d = O(Γ²/ε⁴) suffices for ε-approximation of concept classes with Gaussian surface area Γ. This note aims to improve this bound to achieve better complexity results for learning various concept classes, particularly in light of recent lower bounds by Diakonikolas et al. (2021).

Method: The proof uses a direct analogue of a construction by Feldman et al. (2020) for L1-approximation on the Boolean hypercube, adapting their approach to the Gaussian setting. The method focuses on improving the polynomial degree bound for approximating concept classes with bounded Gaussian surface area.

Result: The main result shows that degree d = Õ(Γ²/ε²) is sufficient for ε-approximation, improving the previous O(Γ²/ε⁴) bound. This yields near-optimal bounds on the complexity of agnostically learning polynomial threshold functions in the statistical query model.

Conclusion: The improved analysis provides tighter bounds on the complexity of learning concept classes under Gaussian marginals, achieving near-optimal results in light of known lower bounds. The construction adapts techniques from Boolean hypercube approximation to the Gaussian setting.
...
TL;DR: Langevin dynamics with iterate averaging achieves optimal sample complexity n ≳ d^{k*/2} for recovering planted directions in high-dimensional models, matching smoothed gradient descent performance without explicit smoothing.


<details>
  <summary>Details</summary>
Motivation: Previous work showed gradient descent requires n ≳ d^{max(1, k*-1)} samples to recover planted directions, while smoothed gradient descent achieves optimal n ≳ d^{max(1, k*/2)}. The paper investigates whether this optimal rate can be achieved without explicit smoothing.

Method: Uses Langevin dynamics (noise-injected gradient descent) with iterate averaging rather than last iterate. Shows that noise-injection combined with averaging emulates landscape smoothing effects.

Result: Proves Langevin dynamics with iterate averaging succeeds with n ≳ d^{k*/2} samples, achieving the same optimal rate as smoothed gradient descent. Applies results to tensor PCA and single-index models.

Conclusion: Noise-injection (Langevin dynamics) combined with iterate averaging can achieve optimal sample complexity without explicit smoothing. Conjectures minibatch SGD may also achieve same rate without additional noise.
...
TL;DR: IR-GeoDiff: A latent diffusion model that generates 3D molecular geometries from infrared spectra by integrating spectral information into molecular node and edge representations.


<details>
  <summary>Details</summary>
Motivation: Existing methods for recovering molecular structures from IR spectra use 1D SMILES or 2D graphs, which fail to capture the relationship between spectral features and 3D molecular geometry. While diffusion models have improved 3D molecular generation, no model has explored the distribution of 3D geometries corresponding to a single IR spectrum.

Method: IR-GeoDiff is a latent diffusion model that recovers 3D molecular geometries from IR spectra by integrating spectral information into both node and edge representations of molecular structures. The model uses attention mechanisms to focus on characteristic functional group regions in spectra.

Result: The model successfully recovers molecular distributions corresponding to given IR spectra, evaluated from both spectral and structural perspectives. Attention-based analysis shows the model focuses on characteristic functional group regions in IR spectra, consistent with chemical interpretation practices.

Conclusion: IR-GeoDiff represents a significant advancement in molecular structure determination from IR spectra by enabling 3D geometry recovery, bridging the gap between spectral features and molecular spatial arrangements through diffusion modeling.
...
TL;DR: SGDF: SGD with Filter optimizer uses optimal linear filtering to compute time-varying gains for gradient updates, achieving better bias-variance trade-off than fixed momentum methods.


<details>
  <summary>Details</summary>
Motivation: Current SGD and momentum optimizers have fixed momentum coefficients that distort the bias-variance balance, leading to suboptimal parameter updates. The underlying gradient dynamics are not well understood from a signal processing perspective.

Method: Proposes SGDF (SGD with Filter), inspired by Optimal Linear Filtering principles. It computes an online, time-varying gain to dynamically refine gradient estimation by minimizing mean-squared error, achieving optimal noise suppression and signal preservation trade-off.

Result: Extensive experiments across diverse architectures and benchmarks show SGDF surpasses conventional momentum methods and achieves performance on par with or surpassing state-of-the-art optimizers.

Conclusion: SGDF provides a principled approach to gradient optimization through signal processing techniques, offering broad applicability to optimization frameworks beyond just SGD variants.
...
TL;DR: The paper proposes a new evaluation metric for time series anomaly detection that addresses limitations in existing metrics through semantic partitioning of anomaly regions and threshold-agnostic assessment.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation metrics for time series anomaly detection have several critical limitations: bias toward point-level coverage, insensitivity to near-miss detections, inadequate penalization of false alarms, and inconsistency from threshold selection. These issues produce unreliable results that hinder objective progress in the field.

Method: The authors propose a novel metric based on detection semantics. They first introduce a partitioning strategy that decomposes each anomaly's local temporal region into three functionally distinct subregions. They then design finer-grained scoring mechanisms for each subregion and aggregate detection qualities across the full threshold spectrum to eliminate evaluation inconsistency from threshold selection.

Result: Extensive experiments on synthetic and real-world data demonstrate that the proposed metric provides stable, discriminative, and interpretable evaluation. It achieves robust assessment compared with ten widely used existing metrics.

Conclusion: The proposed semantic-based evaluation metric addresses fundamental limitations in current time series anomaly detection assessment practices, offering more reliable, interpretable, and threshold-agnostic evaluation that better reflects detection performance.
...
TL;DR: The paper proposes a novel approach to model policy structure in policy gradients by optimizing subsets of future rewards, where simpler policies (smaller reward subsets) are more reliably learned due to more accurate gradient estimates.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning requires learning to act in unknown environments, but policy gradients can be unreliable due to inaccurate empirical gradient estimates. The authors aim to develop a framework that can model different policy structures and compare their performance systematically.

Method: The approach optimizes subsets of future rewards rather than the full reward sequence. Smaller subsets represent simpler policies with more accurate gradient estimates. The framework models various policy classes including full planning, greedy, K-step lookahead, and segment policies.

Result: Empirical evaluation on multiple persona-alignment conversational problems shows that different policies excel in different problems, reflecting their distinct characteristics and demonstrating the importance of policy class selection.

Conclusion: The proposed framework successfully models and compares different policy structures, showing that policy class selection significantly impacts performance in RL problems, with different policies being optimal for different problem types.
...
TL;DR: PCGs generalize predictive coding networks and are shown to mathematically contain feedforward neural networks (MLPs), bridging neuroscience-inspired models with mainstream ML.


<details>
  <summary>Details</summary>
Motivation: To strengthen the connection between neuroscience-inspired predictive coding networks and contemporary machine learning by showing their mathematical relationship with standard feedforward neural networks.

Method: Mathematical proof demonstrating that predictive coding graphs (PCGs) form a superset of feedforward artificial neural networks (multilayer perceptrons).

Result: PCGs are proven to mathematically contain MLPs, positioning predictive coding networks more firmly within machine learning and supporting exploration of non-hierarchical neural network topologies.

Conclusion: This work establishes PCGs as a mathematical generalization that bridges neuroscience-inspired models with standard ML architectures, opening avenues for studying diverse network topologies in machine learning.
...
TL;DR: GNN ensemble learning for SST forecasting using input perturbation strategies shows that spatially coherent noise (Perlin/fractal) yields better uncertainty calibration than random Gaussian noise without compromising deterministic skill.


<details>
  <summary>Details</summary>
Motivation: Regional ocean forecasting needs computationally efficient models that can represent predictive uncertainty. Ensemble methods are promising but require effective strategies to introduce diversity without expensive retraining.

Method: Adapted GNN architecture for Canary Islands region, implemented homogeneous ensemble with bagging-inspired approach using input perturbations (Gaussian, Perlin, fractal Perlin noise) on initial ocean states. Varied noise intensity and spatial structure, evaluated over 15-day horizon.

Result: Deterministic skill comparable to single-model, but perturbation type strongly affects uncertainty representation. Spatially coherent perturbations (low-res Perlin noise) achieved better calibration and lower CRPS than random Gaussian noise, especially at longer lead times.

Conclusion: Noise structure and scale are critical in ensemble GNN design. Carefully constructed input perturbations can produce well-calibrated probabilistic forecasts without additional training cost, supporting feasibility of ensemble GNNs for operational regional ocean prediction.
...
TL;DR: TDA with persistent homology features (Betti curves, persistence landscapes, silhouettes) applied to foot clearance time series improves differential diagnosis of Parkinsonian syndromes (IPD vs VaP) with 83% accuracy and AUC=0.89.


<details>
  <summary>Details</summary>
Motivation: Differential diagnosis among parkinsonian syndromes is clinically challenging due to overlapping motor symptoms and subtle gait abnormalities. Conventional gait analysis methods often overlook hidden nonlinear and structural features in foot clearance patterns, necessitating more sophisticated analytical approaches.

Method: Applied Topological Data Analysis (TDA) using persistent homology to extract features (Betti curves, persistence landscapes, silhouettes) from foot clearance time series. Used Random Forest classifier with leave-one-out cross-validation on dataset of 15 controls, 15 idiopathic Parkinson's disease, and 14 vascular Parkinsonism patients.

Result: Betti-curve descriptors performed best, achieving 83% accuracy and AUC=0.89 for IPD vs VaP classification in medicated state using foot clearance variables. Performance improved in medicated state and further when considering both medication states, showing sensitivity to levodopa-related gait changes.

Conclusion: Integrating TDA with machine learning enhances clinical gait analysis and aids differential diagnosis across parkinsonian disorders by capturing subtle structural features in gait patterns that conventional methods miss.
...
TL;DR: A distributed XGBoost protocol for wearable sensor-based human activity recognition that matches centralized performance within 1% gap while preserving XGBoost's structural advantages.


<details>
  <summary>Details</summary>
Motivation: Wearable sensors can enable early detection of health threats in spinal cord injury patients (pressure injuries, blood pressure instability), but distributed processing is needed for practical remote monitoring scenarios with data fragmentation.

Method: Novel distributed machine learning protocol based on gradient-boosted decision trees (XGBoost), inspired by Party-Adaptive XGBoost (PAX) but preserving XGBoost's histogram-based split construction and tree-ensemble dynamics.

Result: Theoretical analysis shows convergence to centralized XGBoost solutions under appropriate conditions. Empirical evaluation on wearable-sensor HAR dataset shows performance gap under 1% compared to centralized XGBoost, outperforming IBM PAX.

Conclusion: The distributed protocol successfully maintains XGBoost's structural advantages while enabling practical deployment in fragmented wearable sensor environments for health monitoring applications.
...
TL;DR: DC-Merge is a model merging method that maintains directional consistency of singular spaces to preserve task knowledge when integrating multiple task-adapted models.


<details>
  <summary>Details</summary>
Motivation: Current model merging approaches often fail to preserve task knowledge due to two issues: 1) imbalanced energy distribution in task vectors where dominant singular values overshadow weaker but semantically important components, and 2) geometric inconsistency of task vectors in parameter space that distorts directional geometry during merging.

Method: DC-Merge first balances energy distribution by smoothing singular values of each task vector. Then it projects energy-balanced vectors onto a shared orthogonal subspace to align their directional geometries with minimal reconstruction error. Finally, aligned vectors are aggregated in the shared subspace and projected back to original parameter space.
...
Conclusion: DC-Merge effectively addresses directional consistency issues in model merging through energy balancing and geometric alignment, enabling better knowledge retention across tasks.
...
TL;DR: Gradient flow in value-softmax models (core of transformers) inherently drives optimization toward low-entropy solutions, explaining phenomena like attention sinks.


<details>
  <summary>Details</summary>
Motivation: To understand the non-convex training dynamics of softmax-based models, particularly transformers, by analyzing the gradient flow dynamics of the value-softmax model which is the core building block of self-attention.

Method: Analyze gradient flow dynamics of the value-softmax model defined as L(Vσ(a)), where V is a learnable value matrix and a is an attention vector. Study how gradient flow optimization behaves on this structure.

Result: Gradient flow inherently drives optimization toward solutions characterized by low-entropy outputs. This polarizing effect is universal across various objectives including logistic and square loss.

Conclusion: The analysis provides theoretical insight into transformer training dynamics and offers a formal mechanism explaining empirical phenomena like attention sinks and massive activations through the inherent low-entropy optimization tendency.
...
TL;DR: Synthetic Monitoring Environments (SMEs) provide configurable continuous control tasks with known optimal policies for precise RL algorithm diagnostics and regret calculation.


<details>
  <summary>Details</summary>
Motivation: Current RL benchmarks lack precise diagnostics, entangle complexity factors, and don't provide ground-truth optimality metrics, making it difficult to isolate why algorithms fail.

Method: Introduce Synthetic Monitoring Environments (SMEs) - an infinite suite of continuous control tasks with fully configurable task characteristics, known optimal policies, rigorous geometric state space bounds, and exact regret calculation capabilities.

Result: Demonstrated through multidimensional ablations of PPO, TD3, and SAC, revealing how specific environmental properties (action/state space size, reward sparsity, optimal policy complexity) impact within-distribution and out-of-distribution performance.

Conclusion: SMEs offer a standardized, transparent testbed for transitioning RL evaluation from empirical benchmarking toward rigorous scientific analysis with precise diagnostics.
...
TL;DR: DeCoST is a learning-based two-stage method that decouples discrete and continuous variables in OPTWVP, achieving superior solution quality and computational efficiency compared to state-of-the-art approaches.


<details>
  <summary>Details</summary>
Motivation: Current approaches fail to develop efficient solvers for the OPTWVP problem which involves both discrete and continuous variables, limiting practical applications in real-world scenarios.

Method: Two-stage approach: Stage 1 uses parallel decoding to predict path and initial service time allocation; Stage 2 optimizes service times via linear programming with long-horizon learning of structure estimation.
...
Conclusion: The proposed framework effectively decouples discrete and continuous variables in OPTWVP, is compatible with various constructive solvers, and consistently enhances solution quality while maintaining computational efficiency.
...
TL;DR: Agentic retrieval-augmented reasoning improves LLM reliability in clinical decision support by reducing decision dispersion and increasing cross-model robustness, though high agreement doesn't guarantee correctness.


<details>
  <summary>Details</summary>
Motivation: To understand how agentic retrieval-augmented reasoning pipelines affect LLM reliability in clinical decision support under model variability, beyond just accuracy metrics.

Method: Evaluated 34 LLMs on 169 expert-curated radiology questions, comparing zero-shot inference with a radiology-specific multi-step agentic retrieval condition where all models received identical structured evidence reports from curated radiology knowledge.

Result: Agentic inference reduced inter-model decision dispersion (median entropy 0.48 vs. 0.13), increased robustness of correctness across models (mean 0.74 vs. 0.81), and increased majority consensus. However, high agreement didn't guarantee correctness, and 72% of incorrect outputs had moderate/high clinical severity.

Conclusion: Agentic retrieval improves decision concentration, consensus, and cross-model robustness, but evaluating these systems requires complementary analyses of stability, cross-model robustness, and clinical impact beyond just accuracy or agreement metrics.
...
TL;DR: Stem is a novel sparse attention module that reduces quadratic computational complexity in LLMs by aligning sparsity with information flow patterns in causal attention, using position-dependent token selection and output-aware metrics.


<details>
  <summary>Details</summary>
Motivation: The quadratic computational complexity of self-attention, especially during pre-filling phase, limits scaling LLMs to long contexts. Existing sparse attention methods use uniform token selection across positions, ignoring the cumulative dependency patterns inherent in causal attention architectures.

Method: Stem introduces two key strategies: 1) Token Position-Decay strategy applies position-dependent top-k selection within each layer to retain initial tokens for recursive dependencies, and 2) Output-Aware Metric prioritizes high-impact tokens based on approximate output magnitude to preserve information-rich tokens.

Result: Extensive evaluations show Stem achieves superior accuracy with reduced computation and pre-filling latency compared to existing methods.

Conclusion: Stem provides an effective plug-and-play sparsity module that aligns with information flow patterns in causal attention, offering a practical solution to reduce computational complexity while maintaining accuracy in long-context LLMs.
...
TL;DR: PolaDCA is a novel graph neural network framework for machinery fault diagnosis that uses polarized direct cross-attention to dynamically construct graphs and adaptively pass messages, achieving superior noise robustness and state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Conventional GNNs for machinery fault diagnosis have limitations in modeling complex dynamic interactions due to their reliance on predefined static graph structures and homogeneous aggregation schemes, which hinders accurate and robust fault diagnosis in safety-critical industrial systems.

Method: Introduces polarized direct cross-attention (PolaDCA), a relational learning framework that enables adaptive message passing through data-driven graph construction. Uses a direct cross-attention mechanism that dynamically infers attention weights from three semantically distinct node features (individual characteristics, neighborhood consensus, and neighborhood diversity) without fixed adjacency matrices.

Result: Theoretical analysis shows PolaDCA has superior noise robustness over conventional GNNs. Extensive experiments on three industrial datasets (XJTUSuprgear, CWRUBearing, Three-Phase Flow Facility) demonstrate state-of-the-art diagnostic accuracy and enhanced generalization under varying noise conditions, outperforming seven competitive baseline methods.

Conclusion: PolaDCA provides an effective solution for safety-critical industrial applications by overcoming limitations of conventional GNNs through adaptive graph construction and robust relational learning.
...
TL;DR: LLMs can be post-trained to provide calibrated, interpretable uncertainty estimates for their responses through a three-stage pipeline involving entropy-based scoring, calibration, and reinforcement learning alignment.


<details>
  <summary>Details</summary>
Motivation: Current post-hoc uncertainty estimation methods for LLMs are often computationally expensive (sampling-based) or lack calibration, which is problematic for high-stakes applications where reliable uncertainty estimates are crucial.

Method: Three-stage pipeline: 1) Compute fine-grained entropy-based uncertainty scores on training data capturing output variability in embedding space; 2) Calibrate scores via Platt scaling for reliable, interpretable uncertainty signals; 3) Post-train target LLM via reinforcement learning to align its policy with calibrated uncertainty signals using a verifiable reward function.

Result: Models trained with this pipeline achieve better calibration than baselines and generalize to unseen tasks without further processing, indicating they learn robust uncertainty reasoning behavior.

Conclusion: The proposed approach enables LLMs to provide interpretable and computationally efficient uncertainty estimates at test time, unlike post-hoc methods, making them more suitable for high-stakes domains requiring reliable uncertainty quantification.
...
TL;DR: FS-HNN improves Hamiltonian neural networks by separating fast/slow dynamics using multiple networks trained at different timescales, enhancing multiscale modeling for ODEs and PDEs.


<details>
  <summary>Details</summary>
Motivation: Hamiltonian neural networks struggle with complex multiscale temporal dynamics due to spectral bias favoring low-frequency patterns, and existing methods either fail to fully capture multiscale dynamics or require strong domain assumptions.

Method: FS-HNN parameterizes system Hamiltonian using multiple networks, each governed by Hamiltonian dynamics and trained on data sampled at distinct timescales. Extends to PDEs by learning state- and boundary-conditioned symplectic operators.
...
Conclusion: By explicitly separating fast and slow modes in Hamiltonian decomposition, FS-HNN overcomes spectral bias limitations and enables better modeling of complex multiscale dynamics in both ODEs and PDEs.
...
TL;DR: NCD-based kernel methods enable effective client-side ML with small data, despite NCD not being a formal metric.


<details>
  <summary>Details</summary>
Motivation: Conflict between privacy needs (client-side processing) and ML requirements (large labeled data, heavy computation). Need for effective ML that works with single-user data on limited hardware.

Method: Use normalized compression distance (NCD) in kernel methods for complex data modeling; develop techniques to improve training time; demonstrate NCD works despite lacking formal metric properties.

Result: NCD performs as well or better than other metrics/kernels with marginally more computation; enables simple models with remarkable accuracy using very few samples; suitable for client-side deployment.

Conclusion: NCD-based kernel methods provide practical solution for privacy-preserving client-side ML, balancing accuracy, computational efficiency, and small data requirements.
...
TL;DR: CLAIRE is a hybrid end-to-end learning framework combining unsupervised deep representation learning with supervised classification for fault detection in high-dimensional industrial data, outperforming conventional classifiers and offering interpretability.


<details>
  <summary>Details</summary>
Motivation: Accurate fault detection in high-dimensional industrial environments is challenging due to complexity, noise, and redundancy in sensor data. There's a need for robust intelligent quality control systems in smart manufacturing.

Method: Hybrid framework integrating unsupervised deep representation learning (optimized deep autoencoder) with supervised classification. The autoencoder transforms raw input into compact latent space, capturing intrinsic data structure while suppressing noise. Learned representations feed into downstream binary classifier. Includes post hoc interpretability using game-theory-based technique to analyze latent space and identify informative input features.

Result: CLAIRE significantly outperforms conventional classifiers trained directly on raw features on high-dimensional datasets. The framework successfully integrates explainable AI with feature-aware regularization for robust fault detection.

Conclusion: The modular and interpretable framework shows potential for integrating explainable AI with feature-aware regularization. It's highly adaptable for other domains with complex high-dimensional data like healthcare, finance, and environmental monitoring.
...
TL;DR: ALFCG is an adaptive projection-free method for stochastic composite nonconvex optimization that doesn't need global smoothness constants or line search, using historical iterate differences to estimate local smoothness.


<details>
  <summary>Details</summary>
Motivation: Existing conditional gradient methods require global smoothness constants, line search, or conservative Lipschitz constants, which can be impractical or computationally expensive for stochastic nonconvex problems.

Method: ALFCG uses a self-normalized accumulator of historical iterate differences to estimate local smoothness and minimizes a quadratic surrogate model at each step. Three variants: ALFCG-FS for finite-sum problems with SPIDER estimator, and ALFCG-MVR1/2 for stochastic expectation problems with momentum-based variance reduction.

Result: ALFCG-FS achieves O(N + √Nε⁻²) iteration complexity, while ALFCG-MVR1 and ALFCG-MVR2 achieve Õ(σ²ε⁻⁴ + ε⁻²) and Õ(σε⁻³ + ε⁻²) respectively. As noise level σ→0, rates reduce to optimal Õ(ε⁻²). Experiments show ALFCG outperforms state-of-the-art conditional gradient baselines.

Conclusion: ALFCG provides the first adaptive projection-free framework for stochastic composite nonconvex optimization that adapts to unknown geometry without requiring global smoothness constants or line search, achieving improved convergence rates.
...
TL;DR: M-CMAB is a multi-modal LLM inference scheduling framework that uses contextual bandits to optimize task allocation across heterogeneous backends under multi-dimensional budget constraints.


<details>
  <summary>Details</summary>
Motivation: Online MLLM task scheduling is challenging due to sharp variations in request modality composition and reasoning difficulty, coupled with time-varying backend costs from system jitter and network variation. Existing homogeneous single-backend approaches cannot achieve optimal response quality under practical heterogeneous budgets.

Method: Three-component framework: (1) CLS-attentive frozen-backbone Predictor extracts compact task representations using lightweight adapters; (2) primal-dual Constrainer maintains Lagrange multipliers for long-horizon constraint enforcement; (3) two-phase Scheduler balances exploration-exploitation under irreversible budgets using contextual multi-armed bandit approach.

Result: Achieves up to 14.18% higher reward than state-of-the-art baselines across budget regimes on composite multimodal benchmark with heterogeneous backends, closely tracking oracle-aided upper bound. Provides theoretical regret guarantee under multi-dimensional knapsack constraints.

Conclusion: M-CMAB enables effective MLLM inference scheduling under practical heterogeneous budgets, addressing coupled uncertainties in task characteristics and backend costs through a principled multi-constraint contextual bandit framework with theoretical guarantees.
...
TL;DR: The paper proposes a Correlation-Complexity Map with QCLI and CCI indicators to identify when real-world data aligns with IQP quantum models, validates it on turbulence data, and shows IQP models achieve competitive performance with fewer resources.


<details>
  <summary>Details</summary>
Motivation: To develop practical diagnostic tools for determining when real-world data distributions are structurally compatible with IQP-type quantum generative models, enabling efficient identification of quantum-advantage domains.

Method: Proposes a Correlation-Complexity Map with two indicators: QCLI (quantum correlation-likeness from Walsh-Hadamard power spectrum) and CCI (classical correlation-complexity from Chow-Liu tree approximation). Uses turbulence data with invertible float-to-bitstring representation and latent-parameter adaptation scheme for temporal sequences.

Result: Identifies classical turbulence data as both IQP-compatible and classically complex. IQP approach achieves competitive distributional alignment against RBM and DCGAN while using substantially fewer training snapshots and smaller latent blocks.

Conclusion: The QCLI/CCI indicators serve as practical tools for locating IQP-aligned domains, supporting generative quantum utility with resource-efficient quantum models that can compete with classical approaches.
...
TL;DR: NOBLE adds nonlinear low-rank branches to transformer linear layers for pretraining, achieving faster convergence with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Current PEFT methods like LoRA are designed for fine-tuning on frozen weights, but there's a need for architectural improvements that work during pretraining from scratch to improve training efficiency.

Method: NOBLE adds permanent nonlinear low-rank branches to transformer linear layers: σ(xWdown)Wup where σ is a learnable nonlinearity. Best performance with CosNet - a two-layer cosine nonlinearity with learnable frequency and phase.

Result: Achieves up to 1.47x step speedup (32% fewer training steps) with only 4% additional parameters and 7% step time overhead, resulting in up to 1.22x net wallclock speedup. Works across LLMs (250M-1.5B), BERT, VQGAN, and ViT.

Conclusion: NOBLE provides an effective architectural augmentation for pretraining transformers, improving training efficiency with minimal overhead, though may conflict with certain regularization techniques like Mixup/CutMix.
...
TL;DR: COLD-Steer is a training-free framework that steers LLM behavior by approximating gradient descent effects on activations using minimal examples, achieving 95% effectiveness with 50x fewer samples than baselines.


<details>
  <summary>Details</summary>
Motivation: Current activation steering methods face a trade-off: sample-efficient methods poorly capture steering signals, while effective methods require hundreds/thousands of examples. Need for inference-time control without extensive demonstration data.

Method: Approximates representational changes from gradient descent on in-context examples without actual parameter updates. Uses two approaches: (1) unit kernel approximation updating activations directly with normalized gradients, (2) finite-difference approximation requiring only two forward passes regardless of example count.

Result: Achieves up to 95% steering effectiveness while using 50 times fewer samples compared to best baselines. Validated across various steering tasks and benchmarks, including pluralistic alignment tasks.

Conclusion: COLD-Steer enables adaptive, context-aware model control without extensive data, opening new possibilities for flexible alignment with diverse human preferences through principled approximation of learning dynamics rather than specialized training.
...
TL;DR: The paper discovers "Backdoor Modality Collapse" in multimodal diffusion models, where backdoor attacks degenerate to rely on only a subset of modalities despite attacking multiple modalities simultaneously, challenging the assumption of complementary effects.


<details>
  <summary>Details</summary>
Motivation: While diffusion models have revolutionized visual content generation, their rapid adoption has created critical vulnerabilities to backdoor attacks. The natural assumption is that attacking multiple modalities simultaneously (e.g., text and image) would yield complementary effects and strengthen backdoors, but this paper challenges that assumption.

Method: The authors introduce two novel metrics to quantify backdoor modality collapse: Trigger Modality Attribution (TMA) and Cross-Trigger Interaction (CTI). They conduct extensive experiments across diverse training configurations in multimodal conditional diffusion models to analyze backdoor behavior.

Result: The experiments consistently reveal a "winner-takes-all" dynamic where backdoor attacks collapse into subset-modality dominance. Cross-modal interaction is found to be negligible or even negative, contradicting the intuition of synergistic vulnerability. High attack success rates often mask fundamental reliance on a subset of modalities.

Conclusion: The findings highlight a critical blind spot in current security assessments of multimodal diffusion models. The discovery of Backdoor Modality Collapse establishes a principled foundation for mechanistic analysis and future defense development against backdoor attacks.
...
TL;DR: Novel interpretability method for hierarchical probabilistic time-series forecasting that provides explanations for industrial supply chain scenarios, addressing hierarchical structures and uncertainty while improving explainability accuracy.


<details>
  <summary>Details</summary>
Motivation: While machine learning models have achieved good accuracy in hierarchical time-series forecasting for demand prediction, their predictions lack interpretability which is crucial for real-world industrial applications like supply chain management.

Method: Adapts generic interpretability techniques to address hierarchical structures and uncertainty in probabilistic forecasting. The method explains: 1) significance of time-series within hierarchy and external variables at specific times, 2) impact of variables on forecast uncertainty, and 3) forecast changes due to training data modifications.

Result: Evaluated on semi-synthetic datasets based on real-world hierarchical demands for over 10,000 products at a chemical company. The method successfully explained state-of-the-art industrial forecasting methods with significantly higher explainability accuracy. Real-world case studies showed efficacy in identifying important patterns and explanations.

Conclusion: The interpretability method helps build trust and confidence among users, enables better understanding of forecasts, facilitates identification of key demand drivers, and leads to better adoption/utilization of hierarchical forecasting models in practice.
...
TL;DR: CODEC decomposes neural network behavior into sparse motifs of hidden-neuron contributions, revealing causal processes that activation analysis alone cannot determine.


<details>
  <summary>Details</summary>
Motivation: Existing approaches analyze neural networks by identifying activation patterns correlated with human concepts, but this doesn't reveal how neurons actually drive outputs. There's a need for direct examination of how hidden neurons causally contribute to network behavior.

Method: CODEC (Contribution Decomposition) uses sparse autoencoders to decompose network behavior into sparse motifs of hidden-neuron contributions, revealing causal processes that cannot be determined by analyzing activations alone.

Result: Applied to image-classification networks, contributions grow in sparsity and dimensionality across layers, and unexpectedly decorrelate positive/negative effects on outputs. CODEC enables greater control and interpretation of intermediate layers, supporting causal manipulations and human-interpretable visualizations. In retina models, it uncovers combinatorial interneuron actions and identifies dynamic receptive field sources.

Conclusion: CODEC provides a rich, interpretable framework for understanding nonlinear computations across hierarchical layers, establishing contribution modes as an informative unit of analysis for mechanistic insights into artificial neural networks.
...
TL;DR: AllScAIP is a scalable attention-based MLIP that uses all-to-all node attention to capture long-range interactions, achieving SOTA accuracy on molecular systems while enabling stable MD simulations.


<details>
  <summary>Details</summary>
Motivation: Current MLIPs struggle with long-range interactions in large systems like biomolecules and electrolytes, forcing reliance on explicit physics-based terms that may limit scalability and accuracy.

Method: Proposes AllScAIP - an attention-based, energy-conserving MLIP with all-to-all node attention component that is data-driven, scaling to O(100 million) training samples without explicit physics-based long-range terms.

Result: Achieves state-of-the-art energy/force accuracy on molecular systems (OMol25), competitive performance on materials (OMat24) and catalysts (OC20), and enables stable long-timescale MD simulations that recover experimental observables like density and heat of vaporization.

Conclusion: As data and model size scale, inductive biases become less beneficial while all-to-all attention remains critical for capturing long-range interactions, suggesting a shift toward more data-driven approaches in large-scale MLIP development.
...
TL;DR: ProFocus is a training-free progressive framework for Vision-and-Language Navigation that combines proactive perception (identifying missing visual info) with focused reasoning (selective historical context use) using LLMs and VLMs, achieving SOTA zero-shot performance.


<details>
  <summary>Details</summary>
Motivation: Existing VLN methods have inefficient perception (passively processing redundant visual inputs) and unfocused reasoning (treating all historical contexts equally), leading to suboptimal navigation performance.

Method: ProFocus framework with two components: 1) Proactive perception using structured ego-centric semantic maps to identify missing visual info and generate targeted queries, 2) Focused reasoning using Branch-Diverse Monte Carlo Tree Search (BD-MCTS) to select top-k high-value waypoints and focus reasoning on their associated historical contexts.

Result: Achieves state-of-the-art performance among zero-shot methods on R2R and REVERIE benchmarks, validating the effectiveness of the approach.

Conclusion: ProFocus successfully addresses VLN challenges by unifying proactive perception and focused reasoning through LLM-VLM collaboration, demonstrating that training-free progressive frameworks can achieve competitive performance without extensive training.
...
TL;DR: Digital twin-driven V2X trajectory prediction pipeline using Bi-LSTM generator with twin loss to ensure safety and rule compliance at signalized intersections.


<details>
  <summary>Details</summary>
Motivation: Need for accurate and safety-conscious trajectory prediction in V2X-enabled urban environments with complex multi-agent interactions, especially at signalized intersections where traffic rules and safety are critical.

Method: Digital twin-driven V2X trajectory prediction pipeline combining Bi-LSTM-based generator with structured training objective: standard MSE loss + novel twin loss. Twin loss encodes infrastructure constraints, collision avoidance, diversity across predicted modes, and rule-based priors from digital twin.

Result: Significantly reduces critical violations (infrastructure and rule violations) while maintaining comparable prediction accuracy (ADE, FDE) and real-time performance. Introduces ITS-relevant safety indicators beyond standard trajectory metrics.

Conclusion: Digital twin-driven multi-loss learning shows strong potential for V2X-enabled intelligent transportation systems by ensuring scene-consistent and safety-compliant predictions while maintaining accuracy.
...
TL;DR: TEGA is a closed-loop assistive teleoperation system that combines EMG-based force intent inference with visuotactile sensing and vibrotactile feedback to enable intuitive grasp force modulation for users with upper limb disabilities.


<details>
  <summary>Details</summary>
Motivation: Current teleoperation systems focus on finger positioning but overlook grasp force modulation, which is critical for handling objects with varying hardness, texture, and shape. This is especially challenging for users with upper limb disabilities who lack natural tactile feedback.

Method: TEGA integrates EMG-based intent-to-force inference with visuotactile sensing, mapped to real-time vibrotactile feedback via a wearable haptic vest, creating a closed-loop assistive teleoperation framework for proportional force adjustment.

Result: User studies confirm that the system substantially improves grasp stability and task success, demonstrating its effectiveness for assistive robotic applications.

Conclusion: TEGA addresses the critical gap in grasp force modulation for assistive teleoperation, enabling intuitive force adjustment through multimodal sensing and haptic feedback, with promising results for users with upper limb disabilities.
...
TL;DR: PRISM is an instruction-conditioned refinement method that combines imitation learning and reinforcement learning to adapt generic robotic manipulation policies to new fine-grained behaviors using natural language instructions and human feedback.


<details>
  <summary>Details</summary>
Motivation: To bridge imitation learning and reinforcement learning for robotic manipulation, enabling reuse of imitation policies for new tasks without extensive retraining, improving data efficiency and robustness.

Method: Combines IL and RL in a pipeline: 1) Train imitation policy on generic task from demonstrations, 2) Refine using RL with reward functions generated from natural language descriptions (Eureka paradigm), 3) Add human feedback correction on intermediate rollouts to adapt to new goals/constraints.

Result: Outperforms policies without human feedback in simulated pick-and-place tasks, improves robustness on deployment, and reduces computational burden.

Conclusion: PRISM enables efficient policy refinement and reuse through the combination of imitation learning, reinforcement learning with language-generated rewards, and human feedback, achieving better performance with less computation.
...
TL;DR: A novel joint learning approach for robot skill generalization that combines forward and inverse task representations, enabling accurate zero-shot transfer to novel conditions without direct supervision.


<details>
  <summary>Details</summary>
Motivation: Imitation learning methods fail outside training regions, while transfer learning approaches are data-hungry and lack accuracy in zero-shot generalization. There's a need for methods that can efficiently generalize skills to novel conditions.

Method: Frames the problem as task inversion learning, constructs a common representation of forward and inverse tasks, and leverages auxiliary forward demonstrations from novel configurations to execute corresponding inverse tasks without direct supervision.

Result: Outperforms diffusion-based alternatives in simulated and real-world environments requiring complex manipulation skills with diverse objects and tools, demonstrating strong extrapolation capabilities.

Conclusion: The proposed joint learning approach enables accurate and efficient knowledge transfer for robot skill generalization to novel conditions, addressing limitations of both imitation learning and traditional transfer learning methods.
...
TL;DR: RACAS is a robot-agnostic control system using three LLM/VLM modules that communicate via natural language to control diverse robots without platform-specific retraining.


<details>
  <summary>Details</summary>
Motivation: Existing robot control systems require complicated pipelines and either need retraining for each new robot or only work on structurally similar platforms, creating barriers to prototyping robotic solutions.

Method: RACAS uses three cooperative agentic modules (Monitors, Controller, and Memory Curator) based on LLM/VLM that communicate exclusively through natural language. It only requires a natural language description of the robot, available actions definition, and task specification - no code, model weights, or reward function modifications.
...
Conclusion: RACAS demonstrates the potential of agentic AI to substantially reduce the barrier to prototyping robotic solutions by providing robot-agnostic control that works across diverse platforms without platform-specific modifications.
...
TL;DR: A control framework for underactuated soft robots with bounded inputs that enforces rapid exponential stabilization via control Lyapunov functions while satisfying dynamics and actuator constraints.


<details>
  <summary>Details</summary>
Motivation: Soft and hybrid soft-rigid robots are underactuated with tight actuator limits, making stable task-space control challenging. Existing nonlinear control methods often assume full actuation without limits, which doesn't match real-world constraints.

Method: Proposes a control framework that enforces a rapidly exponentially stabilizing control Lyapunov function as a convex inequality constraint while simultaneously satisfying underactuated full-body dynamics and actuator bounds.

Result: Validated in simulation on three platforms with increasing underactuation: tendon-driven finger, trimmed helicoid manipulator, and highly underactuated spiral robot. Shows improved task-space accuracy and consistent Lyapunov convergence under input limits compared to baseline methods.

Conclusion: The framework achieves superior set-point and trajectory-tracking performance for underactuated soft robots with bounded inputs, addressing the gap between theoretical control methods and practical actuator limitations.
...
TL;DR: RFM-HRI Dataset captures human responses to robot failures in medical settings, showing failures degrade valence and control while increasing confusion, annoyance, and frustration.


<details>
  <summary>Details</summary>
Motivation: Understanding user responses to robot interaction failures is under-explored in HRI, especially in healthcare settings where failures affect task performance and user trust.

Method: Wizard-of-Oz studies with 41 participants across lab and hospital settings, systematically inducing four failure types (speech, timing, comprehension, search) during crash-cart item retrieval tasks, recording multimodal data including facial action units, head pose, speech transcripts, and self-reports.

Result: Failures significantly degrade affective valence and reduce perceived control compared to successful interactions; failures associate with confusion, annoyance, frustration while successes associate with surprise, relief, confidence; emotional responses evolve with repeated failures (confusion decreases, frustration increases).

Conclusion: Provides RFM-HRI dataset, analysis of user responses to failures and recovery strategies, and crash-cart scenario enabling systematic comparison of recovery strategies for safety-critical failure recovery, forming foundation for failure detection and recovery methods in embodied HRI.
...
TL;DR: SCOUT is a new method for open-world interactive object search that uses 3D scene graphs with learned utility scores based on relational heuristics, achieving LLM-level performance with computational efficiency through offline procedural distillation.


<details>
  <summary>Details</summary>
Motivation: Current methods for interactive object search either use vision-language embeddings that don't capture task-relevant relational semantics well, or use LLMs that are too slow and costly for real-time deployment. There's a need for a method that can understand semantic relationships between objects and context efficiently.
...
Result: SCOUT outperforms embedding similarity-based methods and matches LLM-level performance while remaining computationally efficient. Real-world experiments show effective transfer to physical environments under realistic sensing and navigation constraints.

Conclusion: SCOUT enables efficient open-world interactive object search by combining scene graph-based exploration with learned utility from LLM-distilled knowledge, bridging the gap between semantic reasoning and real-time deployment in household environments.
...
TL;DR: TransMASK is a self-supervised method that learns a mask to identify relevant state components for imitation learning, improving policy generalization by ignoring irrelevant environmental factors.


<details>
  <summary>Details</summary>
Motivation: Robots trained in one environment need to generalize to new environments. Humans intuitively know which state aspects are task-relevant vs. irrelevant (like table color or background clutter), but robots need explicit mechanisms to learn this invariance for better generalization.

Method: TransMASK learns a mask that transforms observed states into latent representations biased toward relevant elements. It's trained simultaneously with the policy using gradients from policy updates, forcing the mask to align with the Jacobian of the expert policy. The method normalizes mask row magnitudes to identify relevant states (large magnitudes) vs. irrelevant states (near-zero magnitudes).

Result: The approach can be combined with various imitation learning frameworks (like diffusion policies) without additional labels or loss function modifications. It enables robots to focus on task-relevant state components while ignoring irrelevant ones.

Conclusion: TransMASK provides a self-supervised way to learn state relevance masks that improve policy generalization by making robots invariant to irrelevant environmental variations, enhancing robustness to environment changes.
...
TL;DR: CGP enables contact-rich dexterous manipulation by grounding multi-point contacts through robot state and tactile feedback predictions, using learned contact-consistency mapping to generate controller targets.


<details>
  <summary>Details</summary>
Motivation: To enable fine-grained, contact-rich dexterous manipulation that requires handling multiple contact points and complex interactions, which is challenging for traditional control methods.

Method: Uses contact-grounded policy that predicts robot state and tactile feedback, learns contact-consistency mapping to convert predictions into controller-executable targets, works with both tactile arrays and vision-based tactile sensors, collects demonstrations via teleoperation in simulation and physical robot.

Result: Evaluated across multiple dexterous manipulation tasks, demonstrating capability for contact-rich manipulation with different sensor modalities.

Conclusion: CGP provides an effective approach for fine-grained dexterous manipulation by properly grounding contact information and converting it into executable control targets for compliance controllers.
...
TL;DR: Researchers created a high-precision dataset (NC-tALC) to study how transitional autonomous vehicles interact with other vehicles during lane changes, addressing the lack of data on tAV behavior in complex traffic maneuvers.


<details>
  <summary>Details</summary>
Motivation: There's a growing presence of transitional autonomous vehicles (tAVs) on roads that operate between partial and full autonomy, but there's a scarcity of high-resolution trajectory data capturing how these vehicles interact with human-driven vehicles during complex maneuvers like lane changes, which is crucial for understanding traffic stability and safety implications.

Method: The study introduces the NC-tALC Dataset collected through two controlled experimental series: 1) tAV lane-changing experiments where a tAV performs lane changes near ACC-equipped target vehicles, and 2) tAV responding experiments where two tAVs respond to cut-in maneuvers by another tAV. The dataset includes 152 trials (72 lane-changing, 80 responding) sampled at 20 Hz with centimeter-level RTK-GPS accuracy.

Result: The NC-tALC Dataset provides a high-fidelity trajectory dataset with 152 trials that captures detailed tAV interactions during mandatory lane-changing scenarios, offering centimeter-level precision for analyzing both lane-changing execution and follower response dynamics.

Conclusion: The NC-tALC dataset establishes a rigorous empirical foundation for evaluating tAV decision-making and interaction dynamics in controlled mandatory lane-changing scenarios, addressing critical data gaps in understanding how transitional autonomous vehicles behave during complex traffic maneuvers.
...
TL;DR: Proposes an environment-aware path generation framework for robotic additive manufacturing using four path planning algorithms to handle dynamic construction environments with obstacles.


<details>
  <summary>Details</summary>
Motivation: Current AM design methods use pre-conceived offline toolpaths, but dynamic construction environments (terrestrial/extraterrestrial with obstacles) require online path generation methods.

Method: Environment-aware path generation framework (PGF) utilizing four path planning algorithms (two search-based, two sampling-based) for online structure design. Performance evaluated using structural metrics (roughness, turns, offset, RMSE, deviation) and computational metrics (run time) in challenging obstacle arrangements.

Result: Evaluated capability of four path planners in finding feasible paths in dense obstacle environments, assessed effectiveness of structural performance metrics, identified most essential metrics for toolpath evaluation, and determined most promising path planners for challenging environments.

Conclusion: Proposed framework enables online path generation for robotic AM in dynamic environments, identifies best-performing path planning algorithms for challenging conditions, and prescribes essential metrics for evaluating resulting structures.
...
TL;DR: Privacy-preserving vision-language framework for medical interpreter robots that detects speech acts (consent/instruction) and generates corresponding robotic gestures using local open-source models.


<details>
  <summary>Details</summary>
Motivation: Effective communication in healthcare across language barriers requires non-verbal cues and gestures, but privacy concerns limit cloud-based solutions.

Method: Locally deployed open-source models with LLM few-shot prompting for intent detection, plus novel dataset of clinical conversations annotated for speech acts and gesture clips.

Result: Achieved 0.90 accuracy, 0.93 weighted precision, and 0.91 weighted F1-Score for identification; improved computational efficiency and outperformed baseline in human-likeness while maintaining appropriateness.

Conclusion: The privacy-preserving framework effectively enables medical interpreter robots to generate appropriate gestures for speech acts, enhancing cross-language healthcare communication.
...
TL;DR: Safe-Night VLA integrates thermal perception with safety constraints for robust robot manipulation in unstructured environments, outperforming RGB-only models.


<details>
  <summary>Details</summary>
Motivation: Current VLA models rely only on RGB perception, missing important modalities like thermal signals, and lack explicit safety constraints, making them fragile in novel scenarios and when encountering obstacles.

Method: Integrates long-wave infrared thermal perception into pre-trained vision-language backbone for semantic reasoning based on thermodynamic properties, and incorporates safety filter via control barrier functions for deterministic workspace constraint enforcement.

Result: Outperforms RGB-only baselines in real-world experiments on Franka manipulator, demonstrating effective use of non-visible physical modalities for robust manipulation while maintaining safety constraints.

Conclusion: Foundation models can effectively leverage non-visible physical modalities like thermal perception for robust manipulation when combined with rigorous safety constraints, enabling robots to operate safely in unstructured environments.
...
TL;DR: VGM-VLM framework improves robotic manipulation by using vision-language models to extract compositional constraints that guide video generative model rollouts and optimize robot trajectories, achieving 43.3% higher success rates without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Video generative models can produce coherent rollout videos for robotic manipulation but often generate physically implausible motions, and converting them to robot actions introduces cumulative errors from imperfect depth estimation and keypoint tracking.

Method: Method uses vision-language models to automatically extract compositional constraints from language instructions, then applies them in two stages: constraint-guided rollout selection to filter VGM outputs for physical plausibility, and constraint-based trajectory optimization to refine robot trajectories and correct retargeting errors.

Result: The framework improves overall success rate by 43.3% points over the strongest baseline on six real-robot manipulation tasks requiring precise, constraint-sensitive execution, without any task-specific training data.

Conclusion: Combining VGMs' motion generation capabilities with VLMs' structured spatial reasoning enables data-free, constraint-aware robotic manipulation that significantly outperforms existing methods by ensuring physical plausibility and correcting geometric retargeting errors.
...
TL;DR: A two-stage framework for multi-robot motion planning under STL specifications with kinodynamic constraints, combining constrained Bayesian Optimization-based Tree search for single-robot planning and STL-enhanced Kinodynamic Conflict-Based Search for multi-robot coordination.


<details>
  <summary>Details</summary>
Motivation: Existing approaches face scalability bottlenecks (exact methods) or require excessive samples (sampling-based methods) for multi-robot motion planning under Signal Temporal Logic specifications with kinodynamic constraints.

Method: Two-stage framework: 1) Single-robot level uses constrained Bayesian Optimization-based Tree search (cBOT) with Gaussian process surrogate model to learn local cost maps and feasibility constraints; 2) Multi-robot level uses STL-enhanced Kinodynamic Conflict-Based Search (STL-KCBS) that incorporates STL monitoring into conflict detection and resolution.

Result: Demonstrates improved trajectory efficiency and safety over existing methods in benchmarking, with real-world validation using autonomous surface vehicles showing robustness and practical applicability in uncertain environments.

Conclusion: The proposed framework effectively addresses scalability and sample efficiency challenges in multi-robot motion planning under STL specifications, with open-source release (STLcBOT Planner) and practical validation in real-world scenarios.
...
TL;DR: TDGC: Hierarchical policy for quadruped navigation with high-level task decisions and low-level gait control, enabling robust sim-to-real transfer and adaptation to terrain variations.


<details>
  <summary>Details</summary>
Motivation: Address scale mismatch between navigation decisions and gait execution, and instability under out-of-distribution environmental changes that challenge sim-to-real transfer and cause falls in quadruped robots.

Method: Two-level hierarchical architecture: low-level RL-trained policy for gait-conditioned locomotion with controllable behavior parameters; high-level policy making task decisions from sparse terrain cues. Includes structured curriculum with performance-driven progression.

Result: Higher task success rates on mixed terrains and out-of-distribution tests compared to end-to-end approaches.

Conclusion: TDGC provides explicit interfaces for deployment tuning, fault diagnosis, and policy refinement, enabling robust quadruped navigation with traceable decision pipeline without dense maps.
...
TL;DR: CDF-Glove: A lightweight, low-cost cable-driven force-feedback glove for dexterous teleoperation that improves imitation learning demonstration quality through haptic feedback.


<details>
  <summary>Details</summary>
Motivation: High-quality teleoperated demonstrations are a bottleneck for imitation learning in dexterous manipulation. Existing teleoperation platforms are expensive, bulky, and lack haptic feedback, which is crucial for real-time finger adjustments during manipulation tasks.

Method: Developed CDF-Glove, a cable-driven force-feedback glove with 20 finger degrees of freedom sensing (16 direct, 4 inferred). Created kinematic model and control stack validated across multiple robotic hands. Collected bimanual teleoperation datasets and trained Diffusion Policy baselines.

Result: Achieved 0.4° distal joint repeatability, ~200ms force feedback latency, 4x improvement in task success rate vs no-feedback teleoperation. Policies trained on teleoperated demonstrations increased success rate by 55% and reduced completion time by 15.2 seconds (47.2% reduction) compared to kinesthetic teaching.

Conclusion: CDF-Glove provides affordable (~$230), effective haptic feedback for dexterous teleoperation, significantly improving demonstration quality for imitation learning while being open-source and compatible with various robotic hands.
...
TL;DR: HiLAM is a hierarchical latent action model that discovers high-level skills from actionless videos by modeling long-term temporal dependencies, improving over baseline methods.


<details>
  <summary>Details</summary>
Motivation: Existing Latent Action Models (LAMs) focus on short-horizon frame transitions and capture only low-level motion, while actionless videos often contain temporally extended, high-level skills that current methods overlook.

Method: HiLAM uses a hierarchical architecture with a pretrained LAM as a low-level extractor to capture latent action sequences, then aggregates these sequences into high-level latent skills by modeling long-term temporal information.

Result: Experiments show HiLAM improves over baseline methods and exhibits robust dynamic skill discovery from actionless video data.

Conclusion: HiLAM successfully discovers high-level latent skills by modeling long-term temporal dependencies in actionless videos, addressing limitations of existing LAMs that only capture short-horizon, low-level motion patterns.
...
TL;DR: A sample-efficient RL framework for legged manipulators to robustly open heterogeneous articulated objects using compact geometric representations and adaptive sensor fusion.


<details>
  <summary>Details</summary>
Motivation: Legged manipulators struggle with robust interaction across diverse articulated objects (doors, drawers, cabinets) due to varying articulation types and complex robot dynamics. Existing RL approaches are sample-inefficient due to high-dimensional sensory inputs.

Method: Proposes SAFE (Sampling-based Abstracted Feature Extraction) to encode handle/panel geometry into low-dimensional representations for better generalization, and ArtIEst (Articulation Information Estimator) to adaptively mix proprioception and exteroception for estimating opening direction and range of motion.

Result: The framework was successfully deployed to manipulate various heterogeneous articulated objects in both simulation and real-world robot systems, demonstrating robust performance.

Conclusion: The proposed approach enables robust and sample-efficient manipulation of diverse articulated objects by legged manipulators through compact geometric representations and adaptive sensor fusion, addressing key challenges in real-world deployment.
...
TL;DR: Small-scale robots struggle with real-world terrain adaptation. SILA Bot uses lizard-inspired design and proprioceptive sensing to adapt body movement patterns based on granular depth, achieving 95% depth estimation accuracy and improved locomotion via simple linear feedback control.


<details>
  <summary>Details</summary>
Motivation: Small-scale robots are largely confined to labs due to poor terrain adaptation. As robot size decreases, robot-terrain interactions change fundamentally, but there's no systematic understanding of what sensory information small robots need or how they should respond to complex natural terrains.

Method: Developed SILA Bot (Small-scale, Intelligent, Lizard-inspired, Adaptive Robot) that adapts to diverse substrates. Used granular media of varying depths as controlled terrain paradigm. Parameterized optimal body movement patterns as linear function of granular depth. Used proprioceptive signals (joint torque) with K-Nearest Neighbors classifier to estimate granular depth. Designed simple linear feedback controller to modulate body phase based on depth estimation.

Result: Achieved 95% accuracy in granular depth estimation using proprioceptive signals. Showed optimal body movement patterns range from standing-wave bending on flat ground to traveling-wave undulation in deep granular media. The linear feedback controller substantially improved locomotion performance on terrains with unknown depth.

Conclusion: Established a principled framework for perception and control in small-scale locomotion. Enables effective terrain-adaptive locomotion while maintaining low computational complexity, bridging the gap between lab research and real-world deployment of small-scale robots.
...
TL;DR: TraD-RL: A reinforcement learning method for autonomous racing that combines expert trajectory guidance, vehicle dynamics constraints via control barrier functions, and multi-stage curriculum learning to improve both speed and safety.


<details>
  <summary>Details</summary>
Motivation: Standard RL methods suffer from training instability and unsafe actions in high-dynamics autonomous racing environments with strong nonlinearities, necessitating a more robust approach that integrates domain knowledge and safety constraints.

Method: 1) Uses prior expert racing line for augmented state representation and reward shaping; 2) Embeds vehicle dynamics priors via control barrier functions for safety constraints; 3) Implements multi-stage curriculum learning from expert guidance to autonomous exploration.

Result: TraD-RL effectively improves both lap speed and driving stability in high-fidelity Tempelhof Airport Street Circuit simulation, achieving synergistic optimization of racing performance and safety.

Conclusion: The proposed TraD-RL method successfully addresses RL limitations in autonomous racing by integrating domain knowledge, safety constraints, and progressive learning, enabling policies to surpass expert-level performance while maintaining safety.
...
TL;DR: DexEMG: A lightweight teleoperation system using surface EMG signals to control dexterous robotic hands with high precision and strong generalization.


<details>
  <summary>Details</summary>
Motivation: Existing teleoperation systems face trade-offs between performance and portability - vision-based systems have cost/line-of-sight constraints, while mechanical exoskeletons are bulky and restrictive. There's a need for a scalable, intuitive interface for robotic manipulation and assistive technologies.

Method: 1) Collect synchronized sEMG signals and hand poses via MoCap glove; 2) Train EMG2Pose neural network to predict hand kinematics from muscle activity; 3) Develop robust hand retargeting algorithm to map predicted poses onto multi-fingered dexterous hand in real-time.

Result: DexEMG achieves high precision in diverse teleoperation tasks, exhibits strong generalization across novel objects and complex environments, and requires minimal individual-specific recalibration.

Conclusion: DexEMG provides a scalable, intuitive teleoperation interface that bridges human intent and robotic execution, offering a cost-effective solution for both general-purpose robotic manipulation and assistive technologies.
...
TL;DR: Zero-shot camera adaptation framework for robot manipulation that virtually adjusts test-time camera observations to match training configuration without additional data or fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models for robot manipulation are sensitive to camera viewpoint changes in unstructured environments, requiring fine-tuning that is data-intensive and doesn't generalize well to new viewpoints.

Method: Uses feed-forward novel view synthesis model to virtually adjust test-time camera observations in real-time to match training camera configuration, handling both extrinsic and intrinsic parameters. This is a plug-and-play approach that preserves pre-trained VLA capabilities.

Result: Consistently outperforms baselines using data augmentation for policy fine-tuning or 3D-aware features on LIBERO benchmark. Enhances viewpoint robustness in real-world scenarios with varying camera extrinsics, intrinsics, and handheld cameras.

Conclusion: Proposed zero-shot camera adaptation framework effectively addresses viewpoint sensitivity in robot manipulation without requiring additional demonstration data, policy fine-tuning, or architectural changes, improving generalization to diverse camera configurations.
...
TL;DR: Simple slope compensation method for hopping robots using body angle adjustment and corrective torque to cancel unwanted rotation on inclined terrain.


<details>
  <summary>Details</summary>
Motivation: Hopping robots lose balance on slopes because tilted ground creates unwanted rotation at landing, which destabilizes the robot and prevents stable hopping on uneven terrain.

Method: Analyzed the effect using a simple spring mass model, then introduced two fixes: adjusting body touchdown angle based on slope and applying small corrective torque before takeoff to cancel unwanted rotation.

Result: Simulation results confirm that even small slope-aware adjustments dramatically improve landing stability, allowing robots to land smoothly and maintain stable hopping even on steep slopes.

Conclusion: The approach provides a practical path toward reliable hopping on uneven terrain, suitable for low-cost robotic platforms without complex sensing/computation, enabling future autonomous field robots to navigate natural environments like hills and rubble.
...
TL;DR: A novel iterative convex MPC-DCBF framework for polytopic obstacle avoidance using exact polytope distances and linearized dynamics for fast real-time control.


<details>
  <summary>Details</summary>
Motivation: Existing methods for polytopic obstacle avoidance either use smooth geometric approximations (which distort true geometry) or exact polytope distances in nonconvex MPC (which limits real-time performance). There's a need for a method that preserves exact geometry while maintaining computational efficiency for real-time control.

Method: Construct linear discrete-time control barrier function (DCBF) constraints using supporting hyperplanes from exact closest-point computations between convex polytopes. Propose an iterative convex MPC-DCBF framework with local linearization of system dynamics and robot geometry to ensure convexity at each iteration.

Result: The framework reduces computational complexity and enables fast online implementation (millisecond-level solve times) for safety-critical control. It extends to multi-robot and 3D environments, and numerical experiments demonstrate collision-free navigation in cluttered maze scenarios.

Conclusion: The proposed iterative convex MPC-DCBF framework successfully addresses the challenge of polytopic obstacle avoidance by combining exact geometry preservation with computational efficiency, enabling real-time safety-critical control for general nonlinear dynamics in complex environments.
...
TL;DR: Swooper: A DRL-based approach for high-speed aerial grasping using a single neural network policy, trained in under 60 minutes and deployed zero-shot on lightweight hardware achieving 84% success rate at speeds up to 1.5 m/s.


<details>
  <summary>Details</summary>
Motivation: High-speed aerial grasping requires precise flight control and coordinated gripper manipulation, which is challenging to achieve with traditional methods. The goal is to develop a robust control policy that seamlessly integrates both flight and grasping capabilities.

Method: Two-stage deep reinforcement learning approach: first pre-train a flight control policy, then fine-tune it to acquire grasping skills. Uses carefully designed reward functions and training framework. Deployed zero-shot on lightweight quadrotor platform with Raspberry Pi 4B.

Result: Training completes in under 60 minutes on standard desktop with RTX 3060 GPU. In 25 real-world trials: 84% grasp success rate, speeds up to 1.5 m/s, inference takes only 1.0 ms on Raspberry Pi 4B. Matches state-of-the-art classical systems with sophisticated grippers.

Conclusion: DRL can learn robust control policies that seamlessly integrate high-speed flight and grasping, achieving performance comparable to sophisticated classical systems using simple off-the-shelf hardware and minimal training time.
...
TL;DR: The paper presents a dynamics model for the Crazyflie Brushless quadcopter, validates it through simulations and hardware, demonstrates its use for reinforcement learning controllers (including a backflip maneuver), and investigates domain randomization for sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: The Crazyflie Brushless platform offers improved thrust capabilities but lacks a validated dynamics model for research. The authors aim to provide an accurate simulation model to support researchers in developing and testing controllers, particularly for reinforcement learning applications.

Method: Developed a dynamics model for the Crazyflie Brushless quadcopter, identified key parameters, validated through simulations and hardware analyses. Demonstrated model utility by training an end-to-end neural network position controller and a backflip controller using reinforcement learning. Investigated domain randomization effects on sim-to-real transfer.

Result: Successfully created and validated an accurate dynamics model. Demonstrated model's effectiveness by training controllers that successfully transfer from simulation to hardware, including a backflip controller achieving two complete rotations within 1.8 meters vertical movement. Provided insights on domain randomization for bridging sim-to-real gap.

Conclusion: The presented dynamics model enables effective development and testing of controllers for the Crazyflie Brushless platform, particularly for reinforcement learning applications. The open-sourced project provides researchers with a validated simulation platform for rapid controller implementation and testing.
...
TL;DR: PROBE is a learning-free LiDAR place recognition descriptor that models BEV cell occupancy as Bernoulli random variables with continuous translation marginalization via polar Jacobian, achieving state-of-the-art handcrafted performance across diverse LiDAR sensors.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for robust LiDAR place recognition that works across different sensor types without dataset-specific tuning, overcoming limitations of discrete point-cloud perturbations and enabling cross-sensor generalization.

Method: PROBE models each BEV cell occupancy as Bernoulli random variables, analytically marginalizes over continuous Cartesian translations using polar Jacobian (yielding distance-adaptive angular uncertainty σ_θ = σ_t/r), and uses Bernoulli-KL Jaccard similarity with exponential uncertainty gating and FFT-based height cosine similarity for rotation alignment.

Result: Evaluated on four datasets with diverse LiDAR types, PROBE achieves highest accuracy among handcrafted descriptors in multi-session evaluation and competitive single-session performance against both handcrafted and supervised baselines.

Conclusion: PROBE provides a sensor-independent, learning-free approach to LiDAR place recognition that generalizes across different LiDAR types without per-dataset tuning, offering physical interpretability through the σ_t parameter and efficient computation in O(R×S) time.
...
TL;DR: Shape estimation for tensegrity manipulators using only strut inclination angles from IMUs, achieving 2.1% accuracy on a 5-layer manipulator.


<details>
  <summary>Details</summary>
Motivation: Shape estimation is crucial for controlling continuously bending tensegrity manipulators. While exteroceptive sensors are straightforward but costly and environment-limited, proprioceptive approaches avoid these limitations. Current methods lack proven examples for large-scale tensegrity manipulators.

Method: Proposes using only inclination angle information relative to gravity for each strut, obtained by attaching an IMU to each strut. This intrinsic sensory data enables shape estimation of the entire tensegrity manipulator without external sensors.

Result: Experiments on a five-layer tensegrity manipulator with 20 struts (total length 1160 mm) show the method can estimate shape with 2.1% accuracy of total manipulator length. Works from arbitrary initial conditions under static conditions and maintains stable estimation under external disturbances.

Conclusion: Demonstrates successful shape estimation for large-scale tensegrity manipulators using only proprioceptive inclination angle data, providing a practical, cost-effective solution that works in various conditions and environments.
...
TL;DR: First study transferring vision-language-action policies to real greenhouse strawberry harvesting using VR teleoperated demonstrations and fine-tuning methods, achieving 74% success rate with 32.6 seconds per pick.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of transferring VLA policies to real-world long-horizon, unstructured tasks like greenhouse strawberry harvesting, which involves occlusion and specular reflections, while avoiding complex depth sensing and calibration.

Method: Built end-to-end closed-loop system on HarvestFlex platform using three-view RGB sensing (two fixed scene views plus wrist-mounted view), collected 3.71 hours of VR teleoperated demonstrations (227 episodes), and fine-tuned pi_0, pi_0.5, and WALL-OSS models with both full fine-tuning and LoRA approaches.

Result: Under 50-trial real-greenhouse protocol, pi_0.5 with full fine-tuning achieved 74.0% success rate, 32.6 seconds per pick, and 4.1% damage rate. Asynchronous inference-control decoupling further improved performance over synchronous deployment.

Conclusion: Demonstrated non-trivial closed-loop picking with less than four hours of real data, showing feasibility of VLA policy transfer to complex real-world tasks, though limitations remain in close-range observability loss and contact-dynamics mismatch.
...
TL;DR: First universal open-source simulation platform for designing, benchmarking, and optimizing magnetically navigated medical robots, validated with phantom/ex vivo experiments.


<details>
  <summary>Details</summary>
Motivation: Current magnetic navigation system design relies heavily on experimental prototyping (time-consuming, costly) and lacks consistent experimental environments for benchmarking hardware/algorithms across different systems.

Method: Developed an intuitive GUI-based simulator that can simulate both magnetic actuation and tracking tasks for rigid/soft robots, supports third-party anatomical models, and allows customization of hardware/algorithms.

Result: Simulator validated using phantom/ex vivo experiments with continuum and capsule robots; demonstrated functionality through three medical use cases (bronchoscopy, endovascular intervention, GI endoscopy); enables flexible design/optimization of magnetic navigation systems.

Conclusion: The universal open-source simulation platform addresses key challenges in magnetic navigation research by providing a consistent benchmarking environment and reducing reliance on expensive experimental prototyping.
...
TL;DR: MTC is a VR-based framework for collecting scene-aware humanoid locomotion data in cluttered 3D environments, addressing the gap between current humanoid capabilities in open spaces and real-world cluttered settings.


<details>
  <summary>Details</summary>
Motivation: Current humanoid locomotion research focuses on open, flat environments, but real-world settings (homes, offices) are densely cluttered and geometrically constrained, requiring scene-aware whole-body coordination. There's no public dataset coupling full-body human locomotion with scene geometry.

Method: Developed Moving Through Clutter (MTC): a VR-based data collection framework that procedurally generates cluttered scenes with controllable clutter levels, captures whole-body human motion through immersive VR navigation, and automatically retargets motion to humanoid robot models.

Result: Created a dataset of 348 trajectories across 145 diverse 3D cluttered scenes, with benchmarks quantifying environment clutter level and locomotion performance (stability and collision safety).

Conclusion: MTC provides a foundation for studying geometry-induced adaptation in humanoid locomotion and developing scene-aware planning and control methods, addressing the underexplored challenge of humanoid navigation in cluttered 3D environments.
...
TL;DR: TADPO: A novel policy gradient method extending PPO for off-road autonomous driving, combining off-policy teacher guidance with on-policy student exploration for effective long-horizon planning.


<details>
  <summary>Details</summary>
Motivation: Off-road autonomous driving faces challenges with unmapped, variable terrain and uncertain dynamics requiring long-horizon planning. Standard RL methods struggle due to long-horizon tasks with low-signal rewards.

Method: TADPO extends PPO by leveraging off-policy trajectories for teacher guidance and on-policy trajectories for student exploration. Developed a vision-based, end-to-end RL system for high-speed off-road driving.

Result: Demonstrated performance in simulation and achieved zero-shot sim-to-real transfer on a full-scale off-road vehicle, navigating extreme slopes and obstacle-rich terrain.

Conclusion: First deployment of RL-based policies on a full-scale off-road platform, showing TADPO's effectiveness for challenging off-road autonomous driving tasks.
...
TL;DR: VLA models suffer from "linguistic blindness" - they prioritize visual cues over contradictory language instructions, executing plausible but wrong actions. The paper introduces ICBench to diagnose this and proposes IGAR, a train-free attention recalibration method to fix it.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action (VLA) models are becoming foundation models for robotic policies, but their reliability under Out-of-Distribution (OOD) instructions is underexplored. The paper identifies a critical failure mode where VLA policies execute visually plausible actions even when language instructions contradict the scene, calling this "linguistic blindness."

Method: 1) Introduces ICBench, a diagnostic benchmark from LIBERO dataset that probes language-action coupling by injecting controlled OOD instruction contradictions while keeping visual environment unchanged. 2) Proposes Instruction-Guided Attention Recalibration (IGAR), a train-free inference-time mechanism that rebalances attention distributions to restore language instruction influence without retraining or architectural modifications.

Result: Evaluations on three VLA architectures (Pi0, Pi0.5, OpenVLA OFT) show models frequently succeed at tasks despite logically impossible instructions, revealing strong visual bias. IGAR substantially reduces erroneous execution under OOD contradictory instructions while preserving baseline task performance across 30 LIBERO tasks. Real-world validation on Franka robotic arm shows IGAR effectively prevents manipulation triggered by inconsistent instructions.

Conclusion: VLA models exhibit linguistic blindness where visual priors dominate instruction semantics. The proposed IGAR method effectively mitigates this issue through attention recalibration at inference time, improving reliability under contradictory instructions without compromising baseline performance or requiring model retraining.
...
TL;DR: RODEO is a blockchain-based framework that enables service robots to operate as Decentralized Autonomous Organizations (DAOs) with trust, accountability, and auditable actions through smart contracts and proof-verification mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current robot autonomy lacks auditable actions, transparent decision processes, and proper human-robot interaction models needed for extended autonomy. There's a need for trust and accountability mechanisms in robotic systems.

Method: Proposed RODEO framework with: 1) ROS-ETH bridge connecting robots to DAOs, 2) templates for organizations to integrate service robots, 3) proof-verification mechanisms for auditable robot actions. Tested with mobile trash collector robot using smart bin and blockchain-based compensation system.

Result: In 3-day experiment: robot doubled its income, reinvested funds for battery charging, achieved ~1 minute proof validation times, funded up to 88 hours of future autonomous operation through accumulated income.

Conclusion: Blockchain-based DAOs enable robots and organizations to coordinate tasks and payments with auditable execution proofs and on-chain settlement, providing trust, accountability, and extended autonomy for service robots.
...
TL;DR: Uni-Walker: A lifelong embodied navigation framework using Decoder Extension LoRA to decouple navigation knowledge into task-shared and task-specific components, enabling continual learning without catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Current embodied navigation agents powered by large language models perform well on individual tasks but suffer from catastrophic forgetting when learning new navigation skills sequentially, limiting their ability to continually acquire new knowledge across multiple scenes and instruction styles.

Method: Proposes Uni-Walker framework with Decoder Extension LoRA (DE-LoRA) to decouple knowledge into task-shared and task-specific components. Uses knowledge inheritance and experts co-activation strategies for shared knowledge, and expert subspace orthogonality constraint with navigation-specific chain-of-thought reasoning for specific knowledge.

Result: Extensive experiments demonstrate Uni-Walker's superiority in building universal navigation agents with lifelong learning capabilities, effectively addressing catastrophic forgetting in sequential navigation task learning.

Conclusion: Uni-Walker successfully tackles the lifelong embodied navigation learning challenge by separating shared and specific knowledge components, enabling agents to continually adapt to new navigation tasks while retaining previously learned knowledge.
...
TL;DR: Fine-tuning vision-language models to generate executable behavior trees for robotic task planning from visual observations and instructions, achieving 87% success rate with 4B-parameter model.


<details>
  <summary>Details</summary>
Motivation: Combine language models for robotic planning with vision-language models' visual understanding capabilities. Need to address the lack of dataset linking visual observations/instructions to executable behavior trees.

Method: Construct dataset from existing robotic episodes (Open X-Embodiment) using large model as teacher in multi-stage generation pipeline. Fine-tune VLMs (500M-4B parameters) via parameter-efficient fine-tuning (PEFT) to generate BehaviorTree.CPP-compatible trees.

Result: Fine-tuned 4B-parameter VLM approaches performance of state-of-the-art closed-source models, achieving 87% success rate on household tasks in embodied simulator while requiring only fraction of computational resources.

Conclusion: Compact open-source multimodal models can effectively generate executable behavior trees for robotic task planning, bridging visual understanding with planning capabilities while being computationally efficient.
...
TL;DR: Object-centric gaze grounding framework with sticky-glance algorithm for robust intent recognition in multi-object environments, achieving high tracking and selection accuracy for impaired users.


<details>
  <summary>Details</summary>
Motivation: Gaze is valuable for impaired people with limited motor capabilities, but robust gaze-based intent recognition is challenging due to gaze noise, micro-saccades, viewpoint changes, and dynamic objects.

Method: Propose object-centric gaze grounding framework with sticky-glance algorithm that jointly models geometric distance and direction trends to stabilize intent, plus continuous shared control and multi-modal interaction paradigm.

Result: Achieves 0.94 tracking rate for dynamic targets and 0.98 selection accuracy for static targets with minimal 3 gaze samples, reduces task duration by nearly 10%, and demonstrates improved robustness, efficiency, and reduced workload.

Conclusion: The framework enables robust gaze-based intent recognition for impaired users in complex environments through object-centric grounding and sticky-glance stabilization, supporting high-readiness control with human-in-loop feedback.
...
TL;DR: A robotics safety framework combining classical risk engineering with ML pipelines through asset declaration, vulnerability analysis, and synthetic data generation for safety envelope learning.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between traditional safety engineering approaches and modern machine learning methods in robotics, creating a more systematic and formalized approach to physical safety.

Method: Structured framework with explicit asset declaration, systematic vulnerability enumeration, hazard-driven synthetic data generation, and formalized hazard ontology for safety envelope learning.

Result: A framework that aligns classical safety engineering, digital twin simulation, synthetic data generation, and machine learning model training into a cohesive safety system.

Conclusion: The framework successfully integrates traditional risk engineering with modern ML pipelines to enable systematic robotics physical safety through formalized hazard analysis and synthetic data generation.
...
TL;DR: Event-driven dual-agent reinforcement learning for 6-DoF upper-limb rehabilitation robots that decomposes reaching tasks into axes, with human controlling primary direction and robot managing orthogonal corrections, reducing oscillations and improving task success.


<details>
  <summary>Details</summary>
Motivation: Traditional fixed-frequency control in upper-limb rehabilitation robots causes trajectory oscillations due to variable inverse-kinematics execution times, which hinders smooth reaching movements and reduces rehabilitation effectiveness.

Method: Proposes event-driven progression with admission spheres around waypoints, and Dual Agent Multiple Model Reinforcement Learning (DAMMRL) where human selects sphere radius (speed-accuracy trade-off) and robot adjusts 3D Cartesian step magnitudes based on user's cognitive state.

Result: The approach effectively suppresses waypoint chatter, balances spatial precision with temporal efficiency, and significantly improves success rates in object acquisition tasks when trained in simulation and deployed across mixed environments.

Conclusion: Event-triggered DAMMRL enables safe and efficient human-robot co-adaptation for upper-limb rehabilitation by decomposing complex reaching tasks and optimizing the interaction between human control and robotic assistance.
...
TL;DR: DreamToNav uses generative video models to translate natural language commands into robot motion by first refining prompts into visual descriptions, generating synthetic videos of the robot performing tasks, then extracting executable kinematic paths from those videos.


<details>
  <summary>Details</summary>
Motivation: To enable intuitive human-in-the-loop control of robots using natural language instead of rigid waypoint navigation, allowing users to provide vague instructions like "Follow the person carefully" that the system can translate into executable motion.

Method: Uses Qwen 2.5-VL-7B-Instruct to refine vague user instructions into precise visual descriptions, then conditions NVIDIA Cosmos 2.5 video foundation model to synthesize physically consistent videos of robot performing tasks. From synthetic videos, extracts kinematic paths using visual pose estimation, robot detection, and trajectory recovery.

Result: Achieves 76.7% success rate on indoor navigation tasks with wheeled mobile robot and quadruped robot. Final goal errors typically within 0.05-0.10 m and trajectory tracking errors below 0.15 m.

Conclusion: Trajectories extracted from generative video predictions can be reliably executed on physical robots across different locomotion platforms, providing a unified framework for obstacle avoidance and goal-directed navigation without task-specific engineering.
...
TL;DR: KISS-IMU is a self-supervised inertial odometry framework that eliminates ground truth dependency by using LiDAR-based ICP registration and pose graph optimization as supervisory signals, with motion-aware balanced training and uncertainty-driven adaptive weighting.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for inertial odometry heavily rely on ground truth data during training, which fundamentally limits scalability and generalization to unseen and diverse environments. There's a need for methods that can learn robust odometry without ground truth supervision.

Method: The framework uses LiDAR-based ICP registration and pose graph optimization as a lightweight supervisory signal instead of ground truth. It employs two key principles: 1) keeping the IMU stable through motion-aware balanced training, and 2) keeping the IMU strong through uncertainty-driven adaptive weighting during inference. The IMU network is trained in a self-supervised manner with LiDAR serving only as supervision, not requiring additional learnable processes.

Result: Comprehensive experiments were conducted on various real-world platforms including quadruped robots across diverse motion patterns and scenarios. The framework demonstrates robustness without relying on joint multi-modal learning or ground truth supervision.

Conclusion: KISS-IMU provides a novel self-supervised approach to inertial odometry that eliminates ground truth dependency, enabling better scalability and generalization to diverse environments while maintaining robustness through motion-aware training and uncertainty-driven inference.
...
TL;DR: A few-shot real-to-sim approach combines analytical physics with GNNs to create differentiable simulators that require minimal real-world data, improving simulation fidelity and policy learning efficiency.


<details>
  <summary>Details</summary>
Motivation: Analytical simulators fail to capture complex contact dynamics, while learning-based simulators require large amounts of costly real-world data. There's a need to bridge this gap with a method that combines physical consistency with representational capacity using minimal real data.

Method: Proposes a few-shot real-to-sim approach that: 1) calibrates analytical simulators with small real-world data to generate large synthetic datasets, 2) introduces a mesh-based GNN that implicitly models rigid-body forward dynamics, and 3) derives surrogate gradients for collision detection to achieve full differentiability.

Result: The approach enables learning-based simulators to outperform differentiable baselines in replicating real-world trajectories. The differentiable design supports gradient-based optimization, validated through simulation-based policy learning in multi-object interaction scenarios.

Conclusion: Differentiable simulation with few-shot real-world grounding provides a powerful direction for advancing robotic manipulation and control, improving simulation fidelity with minimal supervision and increasing policy learning efficiency.
...
TL;DR: Two-stage approach combining satellite-derived APA index for initial SAV detection with USV-mounted multibeam SONAR for precise bathymetric mapping to support targeted weed harvesting in artificial lakes.


<details>
  <summary>Details</summary>
Motivation: Artificial Water Bodies (AWBs) require continuous monitoring and maintenance due to their artificial biological processes. Rapid underwater vegetation growth must be harvested to preserve ecological balance, but current methods are labor-intensive for operators.

Method: Two-step approach: 1) Initial detection of Submerged Aquatic Vegetation (SAV) using satellite-derived Aquatic Plants and Algae (APA) index to identify areas of interest; 2) High-resolution bathymetric mapping using Unmanned Surface Vehicle (USV) equipped with multibeam SONAR for precise location and quantification of aquatic vegetation.

Result: Preliminary results demonstrate feasibility of integrating satellite imagery and underwater acoustic sensing for improved vegetation management. The approach enables effective human-robot collaboration, narrows search space, and reduces manual workload for operators.

Conclusion: The proposed two-stage approach combining satellite data and USV-mounted SONAR offers an effective solution for targeted weed harvesting in artificial lakes, making vegetation management less labor-intensive while maintaining ecological balance.
...
TL;DR: SuperSuit is a bimodal data acquisition framework for mobile manipulators that supports both teleoperation and active demonstration using a shared kinematic interface, enabling scalable collection of long-horizon demonstrations with 2.6× higher throughput in active mode.


<details>
  <summary>Details</summary>
Motivation: Acquiring high-quality, long-horizon demonstrations for wheeled mobile manipulators is a fundamental bottleneck. Existing teleoperation and wearable interfaces struggle with continuous coordination between SE(2) locomotion and precise manipulation in these tightly coupled systems.

Method: SuperSuit uses a bimodal framework with shared kinematic interface for both robot-in-the-loop teleoperation and active demonstration. It maps human stepping to continuous planar base velocities for locomotion, employs isomorphic wearable arm for manipulation, and uses shift-invariant delta-joint representation for policy training to handle calibration offsets without inverse kinematics.

Result: Real-world experiments show 2.6× higher demonstration throughput in active mode vs teleoperation baseline, comparable policy performance when substituting teleoperation data with active demonstrations at fixed dataset size, and monotonic performance improvement as active data volume increases.

Conclusion: Consistent kinematic representations across collection modalities enable scalable data acquisition for long-horizon mobile manipulation, with active demonstration providing higher throughput while maintaining policy performance comparable to teleoperation.
...
TL;DR: Researchers propose using 1:10 scaled autonomous mini-cars with ROS2 and OScar (ETSI C-ITS compliant stack) as an affordable intermediate platform between simulations and full-scale field tests for cooperative vehicle algorithm development.


<details>
  <summary>Details</summary>
Motivation: The automotive industry's transition to fully automated, connected, and cooperative vehicles requires testing new algorithms, but moving from simulations to full-scale practical experiments is prohibitively expensive and creates barriers to research and development.

Method: Developed 1:10 scaled mini-cars equipped with Jetson Orin board running ROS2 for autonomous operations, sensors for navigation, and Raspberry Pi with OScar (Open Stack for Car) implementing full ETSI C-ITS compliant connectivity stack.

Result: Successfully demonstrated platform feasibility by implementing and validating the Day-1 intersection collision warning (ICW) application, showing the scaled platform can effectively test cooperative intelligent transport systems.

Conclusion: The proposed scaled mini-car platform provides an affordable, practical intermediate step between simulations and full-scale vehicle testing, enabling more accessible research and development of cooperative autonomous vehicle technologies.
...
TL;DR: Distributed control framework for multi-manipulator cooperative transport with safety guarantees using hierarchical event-triggered CBFs and consensus coordination.


<details>
  <summary>Details</summary>
Motivation: Need for coordinated formation tracking of multiple manipulators transporting heavy payloads while enforcing strict safety constraints in varying environments with limited communication and real-time computation budgets.

Method: Three-level hierarchical event-triggered safety architecture with CBFs, integrated with consensus-based coordination protocol using local neighbor information, plus risk-aware leader selection and smooth switching strategy.

Result: Validated through real-world hardware experiments with two Franka manipulators and comprehensive simulations; achieves higher precision cooperation under strict safety constraints with substantially reduced computational cost and communication frequency.

Conclusion: Proposed distributed control framework successfully enables safe, coordinated multi-manipulator transport with reduced computation and communication requirements.
...
TL;DR: Cross-embodiment transfer for robot policies benefits more from structured data analogies (paired demonstrations) than from simply scaling unpaired heterogeneous data, especially for morphology shifts.


<details>
  <summary>Details</summary>
Motivation: To understand what form of demonstration data is most useful for enabling transfer across different robot set-ups, given that generalist robot policies are trained on heterogeneous data from various robots, scenes, and viewpoints.

Method: Conducted controlled experiments varying end-effector morphology, robot platform appearance, and camera perspective. Compared effects of scaling demonstration numbers versus systematically broadening diversity in different ways. Used simulation experiments to analyze different types of shifts.

Result: Perceptual shifts (viewpoint) benefit most from broad diversity, while morphology shifts benefit far less from unstructured diversity and instead see largest gains from data analogies (paired demonstrations that align scenes, tasks, and/or trajectories across different embodiments). Real-world cross-embodiment transfer success improved by average 22.5% over large-scale unpaired datasets by changing only data composition.

Conclusion: Data composition matters more than simply scaling data volume for cross-embodiment transfer. Structured data analogies (paired demonstrations) are particularly effective for morphology shifts, while perceptual shifts benefit more from broad diversity.
...
TL;DR: Training-free spatio-temporal vision token pruning framework for Vision-Language-Action models to enable efficient real-time navigation while preserving accuracy.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action models have strong navigation performance but high computational cost limits real-time deployment due to latency issues.

Method: Proposes training-free spatio-temporal vision token pruning: spatial token selection for current view + spatio-temporal compression for historical memories, using attention-based token importance and query-guided filtering without retraining pretrained models.

Result: Significantly outperforms existing pruning strategies, preserves navigation accuracy under extreme pruning, maintains competitive inference efficiency, and validates reliable low-latency navigation on real quadruped robot.

Conclusion: Bridges gap between large-scale multimodal modeling and efficient real-time embodied deployment in robotic navigation systems through plug-and-play pruning framework.
...
TL;DR: The paper introduces Control Barrier Corridors (CBCs) that unify control barrier functions and safe motion corridors for safe autonomy, enabling persistent goal selection in unknown environments.


<details>
  <summary>Details</summary>
Motivation: Current safety methods for robots in unstructured environments use either functional approaches (control barrier functions) or geometric approaches (safe motion corridors), but these are technically distinct. There's a need to unify these approaches for more effective safe motion planning and control.

Method: The paper introduces Control Barrier Corridors (CBCs) as a new concept that converts control barrier functions into local safe goal regions for reference goal selection in feedback control systems. This unifies the functional and geometric safety approaches.

Result: The authors demonstrate that individual state safety can be extended locally over control barrier corridors for convex barrier functions, provided the control convergence rate matches the barrier decay rate. This reveals a trade-off between safety and reactiveness. The approach enables safely reachable persistent goal selection over continuously changing barrier corridors during system motion.

Conclusion: Control Barrier Corridors provide a unified framework for safe autonomy that enables verifiably safe and persistent path following in autonomous exploration of unknown environments, bridging the gap between functional and geometric safety methods.
...
TL;DR: A unified design embedding for soft robots that jointly optimizes shape, material, and actuation using smooth, low-dimensional parameterization with shared basis functions.


<details>
  <summary>Details</summary>
Motivation: Soft robot design requires joint consideration of geometry, material composition, and actuation due to their tight coupling, but current approaches face computational challenges from nonlinear mechanics, contact handling, and non-smooth state transitions that limit gradient-based optimization.

Method: Introduces a smooth, low-dimensional design embedding using continuous deformation maps for shape variation and spatial fields for material properties, both constructed from shared basis functions. This unified parameter space enables joint co-optimization of all three aspects.

Result: The method shows that design expressiveness increases with basis function count (unlike neural network encodings), joint co-optimization outperforms sequential strategies, and the approach works with black-box simulators while using fewer parameters than neural network and voxel-based baselines.

Conclusion: Structuring the design space itself enables efficient co-design of soft robots, with the unified embedding providing superior performance across dynamic tasks while maintaining compatibility with existing simulation pipelines.
...
TL;DR: CFEAR-TR is a teach-and-repeat localization system using a single spinning radar that achieves lidar-level accuracy (0.117m, 0.096°) in adverse weather conditions, running at 29 Hz.


<details>
  <summary>Details</summary>
Motivation: Reliable localization under adverse weather conditions where optical sensors fail, requiring robust navigation using radar which can operate in poor visibility.

Method: Joint alignment of live radar scans to both stored teach-pass scans and recent live keyframes using sparse oriented surface points from Doppler-compensated measurements, stored in a pose graph.

Result: Achieves localization accuracy of 0.117 m and 0.096° on Boreas dataset, improving state-of-the-art by up to 63%, narrowing gap to lidar-level performance, particularly in heading estimation.

Conclusion: CFEAR-TR provides lightweight, robust radar-based localization for adverse weather navigation, with open-source C++ implementation available.
...
TL;DR: SG-DOR is a relational framework that uses scene graphs with direction-conditioned occlusion reasoning to identify which plant organs obstruct target fruits from specific approach directions, improving robotic harvesting in dense crop canopies.


<details>
  <summary>Details</summary>
Motivation: Robotic harvesting in dense crop canopies requires understanding not just geometry but also explicit direction-conditioned relations between plant organs to determine which leaves obstruct specific fruits from different approach angles.

Method: SG-DOR infers scene graphs from instance-segmented organ point clouds, encoding physical attachments and direction-conditioned occlusion. It introduces occlusion ranking for candidate leaves and uses a direction-aware graph neural network with per-fruit leaf-set attention and union-level aggregation.

Result: Experiments on synthetic pepper dataset show improved occlusion prediction (F1=0.73, NDCG@3=0.85) and attachment inference (edge F1=0.83) over strong ablations, providing structured relational signals for intervention planning.

Conclusion: SG-DOR successfully models direction-conditioned occlusion relations between plant organs, enabling better understanding of crop canopy structure for robotic harvesting interventions in dense environments.
...
TL;DR: A two-wheel monopedal robot for low-gravity exploration that combines rolling, jumping, and self-righting with only three actuators.


<details>
  <summary>Details</summary>
Motivation: To develop a lightweight, compact robot capable of multimodal locomotion (rolling and jumping) for exploration on low-gravity celestial bodies like asteroids or moons, where traditional wheeled rovers face challenges.

Method: Uses only two reaction wheels and three actuators total. Implements two controllers: 1) balancing controller for ground-based jump direction aiming, and 2) aerial reorientation controller for leg positioning during flight. Simple switching between locomotion modes enables adaptation to different terrains.

Result: Successfully demonstrated rolling, targeted jumping and landing, and self-righting capabilities. The system remains compact at 0.33m and 1.25kg, showing effective multimodal locomotion with minimal actuation.

Conclusion: The underactuated monopedal robot with only three actuators can perform complex multimodal locomotion suitable for low-gravity exploration, offering a lightweight alternative to traditional rovers for navigating challenging extraterrestrial terrains.
...
TL;DR: A method for learning both symbolic (Allen relations) and subsymbolic (timing parameters) temporal constraints from human demonstrations to generate executable bimanual manipulation plans.


<details>
  <summary>Details</summary>
Motivation: Existing approaches address high-level symbolic temporal relations and low-level timing parameters in isolation, creating a gap between task planning and movement synchronization for bimanual manipulation.

Method: Three-part approach: (1) 3D timing representation using multivariate Gaussian Mixture Models for subsymbolic temporal relationships, (2) DPLL-based algorithm to find contradiction-free Allen relation assignments for symbolic constraints, (3) optimization-based planning combining both to generate temporally parametrized plans.

Result: Evaluation shows the method generates temporally parametrized plans closer to human demonstrations than baseline approaches using the most characteristic demonstration.

Conclusion: The approach successfully bridges the gap between high-level symbolic reasoning and low-level execution timing for bimanual manipulation by learning both types of constraints from demonstrations.
...
TL;DR: Novel uncertainty-aware adaptive dynamics model for underwater vehicle-manipulator systems that enforces physical consistency constraints during online estimation, achieving accurate predictions with quantified uncertainty.


<details>
  <summary>Details</summary>
Motivation: Underwater vehicle-manipulator systems require accurate dynamic models that can adapt to time-varying hydrodynamic effects and changing parameters during operation, while maintaining physical plausibility.

Method: Moving horizon estimation framework that stacks horizon regressors, enforces convex physical consistency constraints (realizable inertia, damping, friction, hydrostatics), and quantifies uncertainty from parameter evolution. Model remains linear in lumped vehicle and manipulator parameters.

Result: Experiments on BlueROV2 Heavy with 4-DOF manipulator show rapid convergence and calibrated predictions: manipulator fits achieve R² = 0.88-0.98 with slopes near unity, vehicle surge/heave/roll reproduced with good fidelity. Median solver time ~0.023s per update enables online feasibility. Compared to fixed parameter model, consistent reductions in MAE and RMSE across degrees of freedom.

Conclusion: The framework produces physically plausible parameters with near 100% coverage confidence intervals, enabling reliable feedforward control and simulation in underwater environments through uncertainty-aware adaptive modeling.
...
TL;DR: Fly360 enables panoramic drones to perform omnidirectional obstacle avoidance using a two-stage perception-decision pipeline with depth map conversion and lightweight policy network.


<details>
  <summary>Details</summary>
Motivation: Current UAV obstacle avoidance methods rely on limited field-of-view sensors and are unsuitable for scenarios requiring full-spatial awareness when movement direction differs from UAV heading, especially for panoramic drones.

Method: Two-stage perception-decision pipeline: 1) Convert panoramic RGB observations to depth maps as robust intermediate representation, 2) Use lightweight policy network to output body-frame velocity commands from depth inputs. Fixed random-yaw training strategy is employed.

Result: Fly360 achieves stable omnidirectional obstacle avoidance and outperforms forward-view baselines across all three benchmark flight tasks in both simulation and real-world experiments.

Conclusion: The proposed Fly360 system successfully addresses the challenge of omnidirectional obstacle avoidance for panoramic drones, demonstrating superior performance over traditional forward-view approaches through comprehensive benchmark evaluation.
...
### [280] [Beamforming Optimization for Extremely Large-Scale RIS-Aided Near-Field Secure Communications](https://arxiv.org/abs/2603.05922)
*Xiaotong Xu,Qian Zhang,Yunxiao Li,Xuejun Cheng,Meihui Liu,Ju Liu*

Main category: cs.IT

TL;DR: This paper proposes an optimization algorithm for XL-RIS-aided near-field PLS systems to maximize secrecy rate by jointly optimizing BS precoding and RIS reflection coefficients, with artificial jamming for enhanced security.


<details>
  <summary>Details</summary>
Motivation: The paper addresses physical layer security challenges in near-field communications where eavesdroppers may be located close to legitimate users in the same direction, requiring robust security mechanisms for XL-RIS-aided systems.

Method: The authors use alternate optimization to decompose the non-convex secrecy rate problem into two sub-problems: 1) BS transmit beamformer optimization using WMMSE and SCA algorithms, and 2) XL-RIS phase-shifting matrix optimization using ADMM-based algorithm that supports discrete phase optimization.

Result: Simulation results show the proposed scheme ensures secure communication even when eavesdroppers are in the same direction as legitimate users and located closer to the XL-RIS, demonstrating robust security performance.

Conclusion: The proposed algorithm effectively enhances physical layer security in XL-RIS-aided near-field communications through joint optimization of BS precoding and RIS reflection coefficients with artificial jamming, accommodating practical discrete phase constraints.
...
### [281] [STAR Beyond Diagonal RISs with Amplification: Modeling and Optimization](https://arxiv.org/abs/2603.06020)
*Chandan Kumar Sheemar,Giovanni Iacovelli,Wali Ullah Khan,George C. Alexandropoulos,Stefano Tomasin,Symeon Chatzinotas*

Main category: cs.IT

TL;DR: This paper develops a signal model for STAR BD-RIS with per-element amplification and lossless power splitting, proposes an alternating optimization framework for downlink sum-rate maximization with closed-form updates, and demonstrates substantial performance gains over conventional passive BD-RIS.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of conventional passive beyond-diagonal RIS (BD-RIS) by incorporating per-element amplification and lossless power splitting capabilities, enabling simultaneous transmission and reflection with practical hardware constraints like per-element emission caps and aggregate power budgets.

Method: Develops a physically consistent signal model that decouples amplification, element-wise reflection/transmission splitting, and passive beyond-diagonal coupling. Casts downlink sum-rate maximization as a WMMSE problem and proposes an alternating optimization framework with: closed-form updates for MMSE combiners/weights, waterfilling-like beamformer updates, per-element amplification updates satisfying emission constraints, STAR power-splitting updates via cyclic coordinate descent, and Riemannian gradient steps on complex Stiefel manifold for beyond-diagonal coupling matrices.

Result: Numerical results demonstrate substantial sum-rate gains compared to conventional passive BD-RIS. The proposed approach enables efficient distributed implementation by decoupling optimization of reflective and transmissive responses.

Conclusion: The paper successfully develops a comprehensive framework for STAR BD-RIS with amplification and power splitting, providing practical optimization methods with provable convergence that significantly outperform passive BD-RIS while respecting hardware constraints.
...
### [282] [Distributed Semantic Alignment over Interference Channels: A Game-Theoretic Approach](https://arxiv.org/abs/2603.06077)
*Giuseppe Di Poce,Mattia Merluzzi,Emilio Calvanese Strinati,Paolo Di Lorenzo*

Main category: cs.IT

TL;DR: This paper proposes a game-theoretic approach for optimizing MIMO transceivers in semantic communication systems to address semantic mismatches and multi-user interference in interference channels.


<details>
  <summary>Details</summary>
Motivation: Semantic communication systems face two key challenges: 1) semantic mismatches when devices use different internal representations, hindering mutual understanding, and 2) performance degradation from multi-user interference in interference channel environments where multiple devices coexist.

Method: Formulates joint optimization of linear MIMO transceivers as a distributed non-cooperative game, enabling closed-form solutions. Derives sufficient conditions for Nash Equilibrium existence in multiple point-to-point MIMO channels, with users modeled as selfish players optimizing transmission and semantic alignment strategies.

Result: Numerical results demonstrate the approach's effectiveness in goal-oriented semantic communication, highlighting trade-offs between information compression, interference mitigation, semantic alignment, and task performance.

Conclusion: The game-theoretic framework provides a viable solution for addressing semantic coexistence and latent space misalignment in interference channel environments, enabling effective semantic communication despite device heterogeneity and interference challenges.
...
### [283] [On the Secrecy Performance of Continuous-Aperture Arrays Over Fading Channels](https://arxiv.org/abs/2603.06171)
*Xuan Yang,Chongjun Ouyang,Dongming Li,Yuanwei Liu*

Main category: cs.IT

TL;DR: CAPA wiretap channels achieve better secrecy than discrete arrays with same high-SNR slope/diversity order across all eavesdropper scenarios, but collaborative eavesdroppers degrade performance most.


<details>
  <summary>Details</summary>
Motivation: To analyze the secrecy performance of continuous-aperture array (CAPA) systems in wiretap channels, comparing them against traditional spatially-discrete arrays, and understanding how different eavesdropper scenarios affect security metrics.

Method: Established CAPA system models with maximum-ratio transmission under Rayleigh fading, derived approximate PDFs for legitimate and eavesdropper SNRs using Mercer's and Landau's theorems, analyzed three eavesdropper scenarios (single, multiple independent, multiple collaborative), derived secrecy rate and SOP expressions, and obtained high-SNR performance metrics.

Result: CAPA systems achieve same high-SNR slope and diversity order (equal to spatial degrees of freedom) across all scenarios, outperform discrete arrays with half-wavelength spacing, with single eavesdropper scenario showing best performance (smallest offset, highest array gain) and collaborative eavesdroppers showing worst performance (largest offset, lowest array gain).

Conclusion: CAPA-based wiretap channels provide superior secrecy performance compared to discrete arrays, maintaining consistent fundamental performance limits across different eavesdropper scenarios while offering practical advantages in real-world security applications.
...
### [284] [Belief-Adaptive MAP Detection for Molecular ISI Channels with Heteroscedastic Noise](https://arxiv.org/abs/2603.06304)
*Erencem Ozbey,H. Birkan Yilmaz,Chan-Byoung Chae*

Main category: cs.IT

TL;DR: Novel BA-MAP and Soft BA-MAP detectors for molecular communication that account for state-dependent noise variance in ISI, outperforming conventional methods with up to 100% throughput improvement.


<details>
  <summary>Details</summary>
Motivation: Prior detector designs for molecular communication via diffusion (MCvD) have not systematically considered the heteroscedastic (state-dependent) noise variance across inter-symbol interference (ISI) states, which is a defining feature of MCvD systems.

Method: Two decoding mechanisms: 1) BA-MAP (Belief-Adaptive Maximum A Posteriori) derives per-symbol adaptive MAP thresholds based on receiver's current state beliefs; 2) Soft BA-MAP computes mixture log-likelihood ratios by weighting all possible ISI states. Both explicitly incorporate state-dependent means and variances of molecular count channel.

Result: Simulation and information-theoretic analyses confirm the proposed detectors outperform conventional equalization and fixed-threshold methods, achieving up to 100% throughput improvement under realistic MCvD settings.

Conclusion: Explicitly modeling state-dependent noise variance in ISI is crucial for optimal detection in molecular communication systems, and the proposed BA-MAP and Soft BA-MAP detectors provide significant performance gains over existing approaches.
...
### [285] [Quasi-twisted codes and their connection with additive constacyclic codes over finite fields](https://arxiv.org/abs/2603.06309)
*Kanat Abdukhalikov,Gyanendra K. Verma*

Main category: cs.IT

TL;DR: This paper establishes connections between quasi-twisted codes and additive constacyclic codes using polynomial methods, characterizing duals and self-orthogonality conditions.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between quasi-twisted codes and additive constacyclic codes by developing a unified polynomial framework that reveals their structural relationships and enables dual code analysis.

Method: Uses polynomial characterization of quasi-twisted codes, establishes one-to-one correspondence between quasi-twisted codes (length lm, index l) and additive constacyclic codes (length m), and analyzes relationships between different inner products (Euclidean, Hermitian, symplectic, trace).

Result: Derived polynomial characterization of quasi-twisted codes, determined duals for index 2 codes, established equivalence between trace duals of additive constacyclic codes and Euclidean/symplectic duals of corresponding quasi-twisted codes, and provided self-orthogonality conditions.

Conclusion: The paper successfully connects quasi-twisted and additive constacyclic codes through polynomial methods, showing that analyzing trace duals of additive constacyclic codes reduces to analyzing Euclidean/symplectic duals of corresponding quasi-twisted codes, providing a unified framework for code analysis.
...
### [286] [The DNA Coverage Depth Problem: Duality, Weight Distributions, and Applications](https://arxiv.org/abs/2603.06489)
*Matteo Bertuzzo,Alberto Ravagnani,Eitan Yaakobi*

Main category: cs.IT

TL;DR: The paper develops combinatorial methods to compute DNA coverage depth for linear codes using duality and extended weight enumerators, deriving formulas for specific code families.


<details>
  <summary>Details</summary>
Motivation: DNA data storage requires computing expected reads needed to recover all encoded strands, which equals expected random column draws for full rank. While MDS codes are optimal over large fields, practical DNA storage often uses structured codes over small fields, necessitating analysis tools for these specific code families.

Method: Develops combinatorial tools based on duality arguments and extended weight enumerators to solve DNA coverage depth problem for linear codes. Uses these methods to derive closed formulas for specific code families.

Result: Derives closed formulas for coverage depth of simplex, Hamming, ternary Golay, extended ternary Golay, and first-order Reed-Muller codes. Presents general expression for coverage depth of any linear code in terms of weight distributions of its higher-field extensions.

Conclusion: Provides systematic combinatorial approach to DNA coverage depth problem for linear codes over small fields, with practical formulas for important code families and general theoretical framework connecting coverage depth to extended weight enumerators.
...
### [287] [Asymmetric Stream Allocation and Linear Decodability in MIMO Coded Caching](https://arxiv.org/abs/2603.06534)
*Mohammad NaseriTehrani,MohammadJavad Salehi,Antti Tölli*

Main category: cs.IT

TL;DR: The paper proposes a flexible scheduling framework for MIMO coded caching systems that enables asymmetric stream allocation while maintaining linear decodability, expanding the achievable DoF region beyond symmetric-constrained designs.


<details>
  <summary>Details</summary>
Motivation: Existing MIMO coded caching systems largely assume symmetric stream allocations across users, which limits practical implementation with linear receivers and doesn't fully exploit gains across all SNR regimes. Flexible scheduling is needed to enable practical linear receivers while maximizing DoF benefits.

Method: The authors extend their prior work by deriving a simple criterion based on per-user stream allocation that guarantees linear decodability for both symmetric and non-symmetric bit-level coded caching schemes. They then propose a heuristic MIMO-CC delivery and scheduling framework that enables asymmetric stream allocation while adhering to this linear decodability criterion.

Result: The proposed framework expands the feasibility region of achievable Degrees of Freedom compared to symmetric-constrained designs, enabling more flexible and practical MIMO coded caching implementations.

Conclusion: The paper provides a practical solution for implementing MIMO coded caching with linear receivers by enabling asymmetric stream allocation through a flexible scheduling framework that maintains linear decodability, overcoming limitations of symmetric-constrained designs.
...
### [288] [Clinical-Injection Transformer with Domain-Adapted MAE for Lupus Nephritis Prognosis Prediction](https://arxiv.org/abs/2603.05535)
*Yuewen Huang,Zhitao Ye,Guangnan Feng,Fudan Zheng,Xia Gao,Yutong Lu*

Main category: eess.IV

TL;DR: First multimodal computational pathology framework for pediatric lupus nephritis prognosis using only PAS-stained biopsies and clinical data, achieving 90.1% accuracy for three-class treatment response prediction.


<details>
  <summary>Details</summary>
Motivation: Pediatric lupus nephritis has worse outcomes than adult cases, but computational pathology for prognosis prediction is unexplored. Existing methods require multiple expensive stains and don't integrate clinical data.

Method: Multimodal framework with Clinical-Injection Transformer embedding clinical features into patch-level attention, plus decoupled representation-knowledge adaptation using domain-adapted Masked Autoencoder. Multi-granularity morphological type injection bridges classification knowledge with prognostic predictions.

Result: Achieved 90.1% three-class accuracy and 89.4% AUC on cohort of 71 pediatric LN patients with KDIGO-standardized labels, outperforming existing approaches.

Conclusion: Proposed framework demonstrates potential as highly accurate and cost-effective prognostic tool for pediatric lupus nephritis using only routine PAS staining and clinical data.
...
### [289] [Gabor Primitives for Accelerated Cardiac Cine MRI Reconstruction](https://arxiv.org/abs/2603.05681)
...
TL;DR: Gabor primitives outperform existing methods for accelerated cardiac cine MRI reconstruction by providing explicit, interpretable representations with better high-frequency capture.


<details>
  <summary>Details</summary>
Motivation: Current methods for accelerated cardiac cine MRI reconstruction have limitations: implicit neural representations (INRs) lack interpretability, while Gaussian primitives can't capture high-frequency details due to limited spectral support.

Method: Propose Gabor primitives that modulate Gaussian envelopes with complex exponentials to place spectral support at arbitrary k-space locations. For cardiac cine, decompose temporal variation into low-rank geometry basis (cardiac motion) and signal-intensity basis (contrast changes).

Result: Gabor primitives consistently outperform compressed sensing, Gaussian primitives, and hash-grid INR baselines on cardiac cine data with both Cartesian and radial trajectories.

Conclusion: Gabor primitives provide a superior solution for cardiac cine MRI reconstruction, offering compact continuous-resolution representations with physically meaningful parameters while maintaining interpretability.
...
### [290] [Longitudinal Lesion Inpainting in Brain MRI via 3D Region Aware Diffusion](https://arxiv.org/abs/2603.05693)
*Zahra Karimaghaloo,Dumitru Fetco,Haz-Edine Assemlal,Hassan Rivaz,Douglas L. Arnold*

Main category: eess.IV

TL;DR: Novel pseudo-3D longitudinal inpainting framework using DDPM with multi-channel conditioning and region-aware diffusion for brain MRI lesion repair, achieving superior perceptual quality, temporal stability, and 10x speedup over baselines.


<details>
  <summary>Details</summary>
Motivation: Evolving brain lesions in longitudinal MRI scans bias automated neuroimaging pipelines, requiring accurate inpainting methods that maintain 3D anatomical continuity and leverage longitudinal context for progressive neurodegenerative disease studies.

Method: Pseudo-3D longitudinal inpainting framework based on Denoising Diffusion Probabilistic Models (DDPM) with multi-channel conditioning from distinct visits (t_1, t_2), extending Region-Aware Diffusion (RAD) to medical domain for focused generative process on pathological regions without altering healthy tissue.

Result: Significantly outperforms FastSurfer-LIT baseline: reduces LPIPS distance from 0.07 to 0.03, achieves Temporal Fidelity Index of 1.024 (ideal=1.0 vs LIT's 1.22), and processes volumes in 2.53 min (10x faster than LIT's 24.30 min).

Conclusion: The framework provides highly reliable and efficient preprocessing for progressive neurodegenerative disease studies by leveraging longitudinal priors and region-specific denoising, with code and derivative dataset to be released upon acceptance.
...
### [291] [Interpretable Motion Artificat Detection in structural Brain MRI](https://arxiv.org/abs/2603.05726)
*Naveetha Nithianandam,Prabhjot Kaur,Anil Kumar Sao*

Main category: eess.IV

TL;DR: Lightweight 3D DHoGM framework for motion artifact detection in T1 brain MRI using combined 2D/3D features with minimal parameters (209), achieving high accuracy (94.34% in-domain, 89% cross-site) while avoiding false acceptance of poor-quality scans.


<details>
  <summary>Details</summary>
Motivation: Automated MRI quality assessment is crucial but challenging due to motion artifacts and poor generalization across sites. Existing methods require extensive preprocessing (high computational cost) or suffer from poor generalization to unseen data.

Method: Extends Discriminative Histogram of Gradient Magnitude (DHoGM) to 3D space, integrating complementary slice-level (2D) and volume-level (3D) DHoGM features through parallel decision strategy. Uses overlapping 3D cuboids for volumetric analysis. Employs simple threshold-based classifier and low-parameter multilayer perceptron (only 209 trainable parameters).

Result: Achieved 94.34% accuracy in-domain evaluation and 89% accuracy on unseen sites, while almost completely avoiding false acceptance of poor-quality scans. Ablation studies confirmed complementary benefits of combining 2D and 3D features.

Conclusion: Proposed approach offers effective, efficient, and robust solution for automated MRI quality check with strong potential for integration into large-scale clinical and research workflows due to its lightweight, interpretable nature and strong generalization capabilities.
...
### [292] [Uni-LVC: A Unified Method for Intra- and Inter-Mode Learned Video Compression](https://arxiv.org/abs/2603.05756)
*Yichi Zhang,Ruoyu Yang,Fengqing Zhu*

Main category: eess.IV

TL;DR: Uni-LVC is a unified learned video compression method that supports both intra and inter coding with low-delay and random-access modes in a single model, overcoming limitations of existing LVCs that require separate models and degrade with unreliable temporal references.


<details>
  <summary>Details</summary>
Motivation: Existing learned video compression methods have limitations: they require separate models for intra and inter coding modes, and their performance degrades when temporal references are unreliable. There's a need for a unified approach that can handle both coding modes effectively in various scenarios.

Method: Uni-LVC builds on a strong intra-codec and formulates inter-coding as intra-coding conditioned on temporal information from reference frames. It uses an efficient cross-attention adaptation module to integrate temporal cues for both unidirectional (low-delay) and bidirectional (random-access) prediction. A reliability-aware classifier selectively scales temporal cues to behave closer to intra coding when references are unreliable, with a multistage training strategy for adaptive learning across coding modes.

Result: Extensive experiments show Uni-LVC achieves superior rate-distortion performance in both intra and inter configurations while maintaining comparable computational efficiency.

Conclusion: Uni-LVC provides a unified solution for learned video compression that effectively handles both intra and inter coding modes with adaptive temporal reference handling, offering improved performance over existing approaches.
...
### [293] [Architectural Unification for Polarimetric Imaging Across Multiple Degradations](https://arxiv.org/abs/2603.05834)
*Chu Zhou,Yufei Han,Junda Liao,Linrui Dai,Wangze Xu,Art Subpa-Asa,Heng Guo,Boxin Shi,Imari Sato*

Main category: eess.IV

TL;DR: A unified architectural framework for polarimetric imaging that handles multiple degradation types (low-light noise, motion blur, mosaicing) with consistent architecture, performing joint image-Stokes processing to maintain physical consistency.


<details>
  <summary>Details</summary>
Motivation: Existing polarimetric imaging methods have limitations: they use task-specific architectures for each degradation type, suffer from error accumulation in multi-stage pipelines, and operate in single domains (image or Stokes) without exploiting their physical relationships. This makes accurate retrieval of physically consistent polarimetric parameters from degraded measurements challenging.

Method: Proposes a unified architectural framework with consistent design across multiple degradation scenarios. The model performs single-stage joint image-Stokes processing (avoiding error accumulation) and explicitly preserves physical consistency. It maintains the same architecture but is trained separately for different degradations (low-light denoising, motion deblurring, demosaicing).

Result: Extensive experiments show state-of-the-art performance across all three tasks (low-light denoising, motion deblurring, demosaicing). The unified design consistently achieves superior results when trained for specific degradation types.

Conclusion: The proposed framework establishes a versatile and physically grounded solution for degraded polarimetric imaging, overcoming limitations of existing approaches by providing a unified architecture that maintains physical consistency while achieving top performance across multiple restoration scenarios.
...
### [294] [Enhancing Neural Video Compression of Static Scenes with Positive-Incentive Noise](https://arxiv.org/abs/2603.06095)
*Cheng Yuan,Zhenyu Jia,Jiawei Shao,Xuelong Li*

Main category: eess.IV

TL;DR: Proposes adding positive-incentive noise to neural video compression for static scenes, achieving 73% BD-rate savings while maintaining pixel-level fidelity.


<details>
  <summary>Details</summary>
Motivation: Static scene videos (surveillance, videotelephony) consume massive storage/bandwidth but current codecs struggle with temporal redundancy. Neural video compression suffers from training-test distribution gaps, while generative methods introduce unacceptable hallucinations for authenticity-critical applications.

Method: Incorporate positive-incentive noise into NVC for static scenes, where short-term temporal changes are reinterpreted as positive-incentive noise to facilitate model finetuning. Disentangle transient variations from persistent background, internalizing structured prior information in the compression model.

Result: Preliminary experiments show 73% Bjøntegaard delta (BD) rate saving compared to general NVC models. The method reduces data transmission while maintaining pixel-level fidelity.

Conclusion: Provides effective solution to trade computation for bandwidth, enabling robust video transmission under adverse network conditions and economic long-term retention of surveillance footage.
...
### [295] [AI End-to-End Radiation Treatment Planning Under One Second](https://arxiv.org/abs/2603.06338)
...
TL;DR: AIRT is an end-to-end deep learning framework that generates deliverable prostate VMAT treatment plans from CT images in under 1 second, achieving non-inferior quality compared to traditional planning methods.


<details>
  <summary>Details</summary>
Motivation: To reduce planning time and inter-planner variability in radiation therapy workflows, addressing the limitations of existing automated approaches that take several minutes and require multiple dose evaluations/corrections.

Method: End-to-end deep learning framework with differentiable dose feedback, adversarial fluence map shaping, and plan generation augmentation. Trained on 10,000+ intact prostate cases, generates single-arc VMAT plans directly from CT images and structure contours.

Result: Generates plans in under 1 second on Nvidia A100 GPU. Demonstrates non-inferiority to RapidPlan Eclipse in target coverage and OAR sparing metrics. Target homogeneity (HI = 0.10 ± 0.01) and OAR sparing similar to reference plans when evaluated with AcurosXB.

Conclusion: AIRT represents a significant advancement toward ultra-fast standardized RT planning and streamlined clinical workflows, potentially revolutionizing radiation therapy planning efficiency.
...
### [296] [RoboLayout: Differentiable 3D Scene Generation for Embodied Agents](https://arxiv.org/abs/2603.05522)
*Ali Shamsaddinlou*

Main category: cs.AI

TL;DR: RoboLayout extends LayoutVLM with agent-aware reasoning and improved optimization stability for generating physically feasible indoor layouts for diverse embodied agents.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs for spatial reasoning generate semantically coherent layouts but lack consideration for physical feasibility and agent interaction capabilities, especially in constrained indoor environments.

Method: Extends LayoutVLM with: 1) explicit reachability constraints in differentiable layout optimization, 2) agent abstraction for diverse entities (robots, humans, animals), 3) local refinement stage for selective reoptimization of problematic object placements.

Result: RoboLayout preserves semantic alignment and physical plausibility while enhancing applicability to agent-centric indoor scene generation, demonstrated across diverse scene configurations.

Conclusion: RoboLayout successfully bridges the gap between semantic scene understanding and physical feasibility for embodied agents, enabling tailored environment design for diverse agent capabilities.
...
### [297] [Real-Time AI Service Economy: A Framework for Agentic Computing Across the Continuum](https://arxiv.org/abs/2603.05614)
*Lauri Lovén,Alaa Saleh,Reza Farahani,Ilir Murturi,Miguel Bordallo López,Praveen Kumar Donta,Schahram Dustdar*

Main category: cs.AI

TL;DR: Dependency graph topology determines whether decentralized price-based resource allocation works reliably for AI services across device-edge-cloud continuum. Hierarchical graphs enable stable prices and optimal allocations, while complex graphs cause instability. Hybrid architecture with cross-domain integrators improves stability.


<details>
  <summary>Details</summary>
Motivation: Real-time AI services operate across device-edge-cloud continuum with latency-sensitive workloads, multi-stage processing pipelines, and shared resource competition under policy constraints. Need to understand when decentralized, price-based resource allocation can work reliably at scale.

Method: Model service-dependency graphs as DAGs where nodes represent compute stages and edges encode execution ordering. Analyze how graph topology affects price stability and allocation efficiency. Propose hybrid management architecture where cross-domain integrators encapsulate complex sub-graphs into resource slices with simpler interfaces.

Result: Hierarchical (tree or series-parallel) dependency graphs enable price convergence to stable equilibria, efficient optimal allocation computation, and truthful bidding incentives. Complex graphs with cross-cutting ties cause price oscillation and allocation degradation. Hybrid architecture reduces price volatility by 70-75% without sacrificing throughput. Governance constraints create efficiency-compliance trade-offs dependent on topology and load.

Conclusion: Dependency-graph topology is a primary determinant of decentralized resource allocation reliability. Hybrid architecture with cross-domain integrators bridges the gap between hierarchical and complex graphs, enabling stable decentralized coordination that matches centralized allocation quality under truthful bidding.
...
### [298] [Reasoning Models Struggle to Control their Chains of Thought](https://arxiv.org/abs/2603.05706)
*Chen Yueh-Han,Robert McCarthy,Bruce W. Lee,He He,Ian Kivlichan,Bowen Baker,Micah Carroll,Tomek Korbak*

Main category: cs.AI

TL;DR: Models struggle to control what they verbalize in their chain-of-thought reasoning, maintaining CoT monitorability despite attempts to evade detection.


<details>
  <summary>Details</summary>
Motivation: To assess whether models can control their chain-of-thought verbalizations, which could undermine the monitorability of reasoning models if they can hide misbehaviors in their internal reasoning process.

Method: Introduced CoT-Control evaluation suite with tasks requiring models to solve problems while adhering to CoT instructions (e.g., reasoning without using specific words). Measured controllability across different models, training regimes, and conditions including incentives and adversarial prompting.

Result: Models show significantly lower CoT controllability (2.7% for Claude Sonnet 4.5) than output controllability (61.9%). Controllability increases with model size but decreases with RL training, test-time compute, and problem difficulty. Adversarial prompting doesn't meaningfully increase controllability.

Conclusion: Cautiously optimistic that CoT controllability is currently unlikely to undermine CoT monitorability, but the mechanism behind low controllability isn't well understood. Recommends frontier labs track CoT controllability in future models.
...
### [299] [Evolving Medical Imaging Agents via Experience-driven Self-skill Discovery](https://arxiv.org/abs/2603.05860)
*Lin Fan,Pengyu Dai,Zhipeng Deng,Haolin Wang,Xun Gong,Yefeng Zheng,Yafei Ou*

Main category: cs.AI

TL;DR: MACRO is a self-evolving medical AI agent that autonomously discovers and synthesizes effective multi-step tool sequences into reusable composite tools, enabling adaptive clinical image interpretation without manual redesign.


<details>
  <summary>Details</summary>
Motivation: Clinical image interpretation requires iterative, multi-step tool use, but existing LLM-based agents have static tool sets that degrade under real-world domain shifts, tasks, and evolving diagnostic requirements, demanding costly manual re-design.

Method: MACRO autonomously identifies effective multi-step tool sequences from verified execution trajectories, synthesizes them into reusable composite tools, and registers these as new high-level primitives. It uses image-feature memory for visual-clinical context and a GRPO-like training loop to reinforce reliable composite tool invocation.

Result: Extensive experiments across diverse medical imaging datasets show that autonomous composite tool discovery consistently improves multi-step orchestration accuracy and cross-domain generalization over strong baselines and state-of-the-art agentic methods.

Conclusion: MACRO bridges the gap between brittle static tool use and adaptive, context-aware clinical AI assistance by enabling closed-loop self-improvement with minimal supervision through experience-driven tool discovery.
...
### [300] [The World Won't Stay Still: Programmable Evolution for Agent Benchmarks](https://arxiv.org/abs/2603.05910)
...
TL;DR: ProEvolve is a graph-based framework for programmatically evolving agent environments to better evaluate LLM agents' adaptability to real-world dynamics.
...
Result: The authors validated ProEvolve by evolving a single environment into 200 environments and 3,000 task sandboxes, and benchmarked representative agents accordingly.

Conclusion: ProEvolve provides a programmable framework for scalable and controllable environment evolution, enabling better evaluation of LLM agents' adaptability to real-world dynamics through systematic environment transformations.
...
### [301] [DeepFact: Co-Evolving Benchmarks and Agents for Deep Research Factuality](https://arxiv.org/abs/2603.05912)
*Yukun Huang,Leonardo F. R. Ribeiro,Momchil Hardalov,Bhuwan Dhingra,Markus Dreyer,Venkatesh Saligrama*

Main category: cs.AI

TL;DR: Researchers propose Evolving Benchmarking via Audit-then-Score (AtS) to create reliable benchmarks for verifying factuality in deep research reports, showing that expert auditors are more reliable than one-shot labelers.


<details>
  <summary>Details</summary>
Motivation: Existing fact-checkers are designed for general-domain atomic claims and don't transfer well to deep research reports (DRRs). Building reliable benchmarks for DRR factuality verification is difficult because static expert-labeled benchmarks are brittle, with experts achieving only 60.8% accuracy in controlled studies.

Method: Proposed Evolving Benchmarking via Audit-then-Score (AtS) where benchmark labels and rationales are explicitly revisable. When a verifier disagrees with current benchmark, it submits evidence; an auditor adjudicates the dispute; accepted revisions update the benchmark before models are scored. Instantiated as DeepFact-Bench (versioned DRR factuality benchmark) and DeepFact-Eval (document-level verification agent).

Result: Across four AtS rounds, expert micro-gold accuracy rose from 60.8% to 90.9%, showing experts are substantially more reliable as auditors than as one-shot labelers. DeepFact-Eval outperforms existing verifiers on DeepFact-Bench and transfers well to external factuality datasets.

Conclusion: The Audit-then-Score approach creates more reliable benchmarks for DRR factuality verification by leveraging experts as auditors rather than one-shot labelers, with the DeepFact system demonstrating strong performance and transferability.
...
### [302] [An Interactive Multi-Agent System for Evaluation of New Product Concepts](https://arxiv.org/abs/2603.05980)
*Bin Xuan,Ruo Ai,Hakyeon Lee*

Main category: cs.AI

TL;DR: This paper proposes an LLM-based multi-agent system for automated product concept evaluation, using 8 virtual domain experts to assess technical and market feasibility through RAG and real-time search, achieving expert-level consistency.


<details>
  <summary>Details</summary>
Motivation: Traditional expert-led product concept evaluation suffers from subjective bias, high time/cost requirements, and limited scalability, creating a need for more objective, efficient automated approaches.

Method: Developed an LLM-based multi-agent system with 8 virtual agents representing specialized domains (R&D, marketing, etc.), using retrieval-augmented generation (RAG) and real-time search tools for evidence gathering, with agents fine-tuned on professional product review data.

Result: System evaluation rankings for professional display monitor concepts showed consistency with senior industry experts, demonstrating the approach's practical usability for product development decisions.

Conclusion: The proposed multi-agent-based evaluation approach provides an effective automated solution for product concept evaluation that can support strategic resource allocation and decision-making in enterprises.
...
### [303] [Agentic LLM Planning via Step-Wise PDDL Simulation: An Empirical Characterisation](https://arxiv.org/abs/2603.06064)
*Kai Göbel,Pierrick Lorang,Patrik Zips,Tobias Glück*

Main category: cs.AI

TL;DR: LLMs show modest planning capability via agentic approach with PDDL engine, but performance lags classical planners and appears driven by training data recall rather than generalizable reasoning.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs can serve as viable task planners alongside classical symbolic methods, and to evaluate interactive agentic approaches vs. direct planning.

Method: PyPDDLEngine - an open-source PDDL simulation engine exposing planning operations as LLM tool calls via MCP interface. LLM acts as interactive search policy selecting one action at a time, observing resulting states, with reset/retry capability.

Result: Classical planner (Fast Downward) achieved 85.3% success. Direct LLM planning: 63.7%. Agentic LLM planning: 66.7% (modest 3% advantage at 5.7× higher token cost). LLMs produced shorter plans than seq-sat-lama-2011 but likely due to training-data recall.

Conclusion: Agentic gains depend on environmental feedback nature. Coding agents benefit from externally grounded signals (compiler errors, test failures), while PDDL step feedback is self-assessed without external verification, limiting LLM planning effectiveness.
...
### [304] [Aggregative Semantics for Quantitative Bipolar Argumentation Frameworks](https://arxiv.org/abs/2603.06067)
*Yann Munro,Isabelle Bloch,Marie-Jeanne Lesot*

Main category: cs.AI

TL;DR: Introduces aggregative semantics for Quantitative Bipolar Argumentation Frameworks (QBAF) that separately aggregates attackers and supporters before combining with intrinsic argument weights, offering more parametrizable and interpretable gradual semantics.


<details>
  <summary>Details</summary>
Motivation: Existing gradual semantics in QBAF often treat attackers and supporters symmetrically, but real-world scenarios require handling them asymmetrically. There's a need for more interpretable and parametrizable semantics that better preserve bipolarity throughout computation.

Method: Proposes aggregative semantics with three-stage computation: 1) compute global weight for attackers, 2) compute global weight for supporters, 3) aggregate these two values with the intrinsic weight of the argument. Discusses properties for the three aggregation functions and their relationship with classical principles.

Result: The approach leads to more parametrizable and understandable gradual semantics that keep bipolarity one step further than existing literature. Tested 500 aggregative semantics on examples, demonstrating a wide range of possible behaviors while maintaining interpretability.

Conclusion: Aggregative semantics provide a flexible, interpretable framework for QBAF that handles asymmetric roles of attackers and supporters through separate aggregation, offering better parametrization and preserving bipolarity throughout the computation process.
...
### [305] [Offline Materials Optimization with CliqueFlowmer](https://arxiv.org/abs/2603.06082)
...
<details>
  <summary>Details</summary>
Motivation: Current generative modeling methods for materials discovery are ineffective at exploring optimal regions of materials space due to their maximum likelihood training, which doesn't directly optimize for target properties.

Method: CliqueFlowmer integrates clique-based model-based optimization into transformer and flow generation, enabling direct optimization of target material properties during the generation process.

Result: CliqueFlowmer produces materials that strongly outperform those generated by baseline generative methods, demonstrating superior optimization capabilities.

Conclusion: The proposed offline model-based optimization approach offers an effective alternative to traditional generative methods for computational materials discovery, with code made publicly available to support interdisciplinary research.
...
### [306] [Conversational Demand Response: Bidirectional Aggregator-Prosumer Coordination through Agentic AI](https://arxiv.org/abs/2603.06217)
*Reda El Makroum,Sebastian Zwickl-Bernhard,Lukas Kranzl,Hans Auer*

Main category: cs.AI

TL;DR: CDR uses bidirectional natural language conversations between aggregators and prosumers via agentic AI for more transparent and participatory demand response coordination.


<details>
  <summary>Details</summary>
Motivation: Current demand response systems lack meaningful prosumer participation - they're either fully automated or offer limited one-way communication that prevents informed decision-making.

Method: Two-tier multi-agent architecture with aggregator agents dispatching flexibility requests and prosumer HEMS agents assessing deliverability via optimization tools, enabling bidirectional natural language conversations.

Result: Proof-of-concept shows interactions complete in under 12 seconds, demonstrating how agentic AI can bridge the aggregator-prosumer coordination gap while maintaining transparency and user agency.

Conclusion: CDR provides the scalability of automated demand response while preserving the transparency, explainability, and user agency necessary for sustained prosumer participation, with all components released as open source.
...
### [307] [Artificial Intelligence for Climate Adaptation: Reinforcement Learning for Climate Change-Resilient Transport](https://arxiv.org/abs/2603.06278)
...
TL;DR: RL-based framework for long-term urban flood adaptation planning outperforms traditional optimization by discovering coordinated spatial-temporal pathways and learning cost-impact trade-offs under climate uncertainty.


<details>
  <summary>Details</summary>
Motivation: Climate change intensifies rainfall and pluvial flooding, increasing urban transportation disruptions. Designing adaptation strategies is challenging due to long-term sequential investments, deep climate uncertainty, and complex flooding-infrastructure-mobility interactions.
...
Result: Framework evaluated on Copenhagen case study (2024-2100) with multiple adaptation options and climate scenarios. Outperforms traditional optimization by discovering coordinated spatial-temporal adaptation pathways and learning trade-offs between impact reduction and investment.

Conclusion: RL shows potential as a flexible decision-support tool for adaptive infrastructure planning under climate uncertainty, yielding more resilient strategies through coordinated adaptation pathways.
...
### [308] [The EpisTwin: A Knowledge Graph-Grounded Neuro-Symbolic Architecture for Personal AI](https://arxiv.org/abs/2603.06290)
...
TL;DR: EpisTwin is a neuro-symbolic framework that creates Personal Knowledge Graphs from fragmented user data to enable trustworthy personal AI with verifiable reasoning.


<details>
  <summary>Details</summary>
Motivation: Current personal AI suffers from data fragmentation across isolated silos, and existing RAG approaches fail to capture semantic topology and temporal dependencies needed for holistic sensemaking.

Method: Uses multimodal language models to convert heterogeneous data into semantic triples, builds a Personal Knowledge Graph, and enables complex reasoning via an agentic coordinator combining Graph RAG with Online Deep Visual Refinement.

Result: Demonstrates robust performance across state-of-the-art judge models on PersonalQA-71-100 benchmark, showing promising results for trustworthy Personal AI.

Conclusion: EpisTwin offers a neuro-symbolic approach to ground generative reasoning in verifiable personal knowledge graphs, addressing data fragmentation and enabling trustworthy personal AI.
...
### [309] [SAHOO: Safeguarded Alignment for High-Order Optimization Objectives in Recursive Self-Improvement](https://arxiv.org/abs/2603.06333)
...
<details>
  <summary>Details</summary>
Motivation: As recursive self-improvement moves from theory to practice, iterative self-modification risks subtle alignment drift where systems may gradually deviate from their intended goals and constraints.

Method: Three safeguards: (1) Goal Drift Index - learned multi-signal detector combining semantic, lexical, structural, and distributional measures; (2) constraint preservation checks for safety-critical invariants; (3) regression-risk quantification to flag improvement cycles that undo prior gains.

Result: Across 189 tasks in code generation, mathematical reasoning, and truthfulness, SAHOO produces substantial quality gains (18.3% improvement in code tasks, 16.8% in reasoning) while preserving constraints and maintaining low violations in truthfulness.

Conclusion: SAHOO makes alignment preservation during recursive self-improvement measurable, deployable, and systematically validated at scale, mapping the capability-alignment frontier and exposing domain-specific tensions.
...
### [310] [Talk Freely, Execute Strictly: Schema-Gated Agentic AI for Flexible and Reproducible Scientific Workflows](https://arxiv.org/abs/2603.06394)
...
TL;DR: Schema-gated orchestration separates conversational flexibility from deterministic execution in LLM-driven scientific workflows, addressing the trade-off between flexibility and determinism through machine-checkable specifications.


<details>
  <summary>Details</summary>
Motivation: Scientific workflows require determinism, provenance, and governance that LLMs struggle to guarantee when making execution decisions, creating tension between deterministic execution and conversational flexibility.

Method: Proposed schema-gated orchestration where schemas serve as mandatory execution boundaries; analyzed 20 systems across 5 architectural groups using multi-model LLM scoring (15 sessions across 3 LLM families) to assess execution determinism and conversational flexibility.

Result: Found empirical Pareto front showing no system achieves both high flexibility and high determinism, but identified convergence zone; multi-model LLM scoring showed substantial agreement (α=0.80 for ED, α=0.98 for CF), validating it as alternative to human expert panels.

Conclusion: Schema-gated architecture can decouple the flexibility-determinism trade-off by separating conversational from execution authority, guided by three principles: clarification-before-execution, constrained plan-act orchestration, and tool-to-workflow-level gating.
...
### [311] [Boosting deep Reinforcement Learning using pretraining with Logical Options](https://arxiv.org/abs/2603.06565)
*Zihan Ye,Phil Chau,Raban Emunds,Jannis Blüml,Cedric Derstroff,Quentin Delfosse,Oleg Arenz,Kristian Kersting*

Main category: cs.AI

TL;DR: H²RL: Hybrid Hierarchical RL combines symbolic planning with neural RL to improve long-horizon decision-making and avoid short-term reward exploitation.


<details>
  <summary>Details</summary>
Motivation: Deep RL agents often misalign by over-exploiting early reward signals. Purely symbolic approaches address this but are hard to scale and apply to continuous settings. Humans acquire skills through hybrid reasoning, inspiring a need for approaches that combine symbolic structure with neural flexibility.

Method: Two-stage hybrid framework: 1) Logical option-based pretraining injects symbolic structure to steer learning away from short-term reward loops toward goal-directed behavior. 2) Final policy refinement via standard environment interaction maintains deep policy expressivity.

Result: Empirically shows consistent improvement in long-horizon decision-making. Outperforms strong neural, symbolic, and neuro-symbolic baselines.

Conclusion: H²RL successfully combines symbolic structure with neural RL, avoiding the limitations of purely symbolic approaches while addressing deep RL's misalignment problems, resulting in better goal-directed agents.
...
### [312] [A Survey on Stacked Intelligent Metasurfaces: Fundamentals, Recent Advances, and Challenges](https://arxiv.org/abs/2603.05633)
*Chandan Kumar Sheemar,Wali Ullah Khan,Sourabh Solanki,George C. Alexandropoulos,Symeon Chatzinotas*

Main category: eess.SP

TL;DR: This survey paper provides a comprehensive overview of Stacked Intelligent Metasurfaces (SIMs) - advanced programmable antenna front-end architectures using multiple metasurface layers for wave-domain signal processing in 6G wireless systems.


<details>
  <summary>Details</summary>
Motivation: Traditional single-layer reconfigurable intelligent surfaces (RISs) have limited electromagnetic transformation capabilities. SIMs address this limitation by enabling richer and more flexible wave manipulation through cascaded metasurface layers, expanding the design space for programmable wireless systems.

Method: The survey examines SIMs from electromagnetic processing perspective, covering physical principles, modeling frameworks (cascaded operators, multiport impedance formulations, network parameter representations), hardware realizations, and emerging architectural designs.

Result: SIMs enable advanced communication functionalities including performance optimization, near-field/wideband transmission, learning-driven control, integrated sensing and communications, and support for emerging architectures like cell-free and non-terrestrial networks.

Conclusion: SIMs represent a promising direction for 6G wireless systems as fully programmable electromagnetic processors, with open research challenges in physical modeling, scalability, hardware-algorithm co-design, and network integration that need to be addressed.
...
### [313] [Frequency Response of Windowed DFT Phasor Estimation: Impact on Oscillation Observability](https://arxiv.org/abs/2603.05746)
*Jiahui Yang,Yuru Wu,Haozong Wang,Yu Liu,Biao Sun,Yilu Liu,Clifton Black*

Main category: eess.SP

TL;DR: The paper analyzes how windowed DFT phasor estimation in PMUs affects sub-synchronous oscillation monitoring, deriving the complete complex-valued frequency response and proposing a recovery method to restore true oscillation parameters.


<details>
  <summary>Details</summary>
Motivation: PMUs are widely used for sub-synchronous oscillation monitoring, but the effect of windowed DFT-based phasor estimation on oscillation observability is not fully characterized. There's a need to understand how the estimation window affects oscillation measurements.

Method: The authors derive the complete complex-valued frequency response of the windowed DFT phasor estimator under both magnitude and phase modulation. They analyze how the estimation window introduces frequency-dependent magnitude attenuation and phase shift governed by complex gain, and propose a simple recovery method using the analytically known complex gain.

Result: The analysis reveals that windowed DFT introduces both magnitude attenuation and phase shift to oscillation components, which are frequency-dependent. The proposed recovery method can restore true oscillation amplitude and phase from PMU data. Results are validated through time-domain simulations.

Conclusion: The study provides guidance for industry practitioners on interpreting PMU-based oscillation measurements and selecting appropriate window lengths. The derived frequency response and recovery method help improve the accuracy of sub-synchronous oscillation monitoring using PMUs.
...
### [314] [Rethinking Next-Generation Signal Waveform: Integration of Orthogonality and Non-Orthogonality](https://arxiv.org/abs/2603.05752)
*Tongyang Xu,Shuangyang Li,Zhongxiang Wei,Gan Zheng,Izzat Darwazeh*

Main category: eess.SP

TL;DR: This paper explores sustainable waveform options for 6G communications, comparing SC-OFDM and SC-NOFS with 1D/2D precoding, finding SC-NOFS(2D) superior for future 6G requirements.


<details>
  <summary>Details</summary>
Motivation: As 6G communications advance, there's increasing demand for new services and capabilities while maintaining compatibility with existing standards. The paper aims to develop sustainable physical layer waveforms that balance backward compatibility with forward innovation.

Method: The paper investigates two signal formats: SC-OFDM (1D,2D) and SC-NOFS (1D,2D), both using 1D frequency and 2D time-frequency precoding. It conducts comparative analysis of these waveform options, focusing on their ability to provide enhanced frequency/time diversity, simplified processing, and resilience to delay-Doppler effects.

Result: SC-NOFS(2D) demonstrates superior performance with improved spectral efficiency, reduced latency, enhanced frequency and time diversity, simplified processing, and better resilience to delay-Doppler effects compared to SC-OFDM variants.

Conclusion: SC-NOFS(2D) provides a broader range of capabilities for 6G applications, particularly those requiring high data rate, high mobility, low-latency communication, sustainability, and interoperability, positioning it as a versatile solution for next-generation 6G communication.
...
### [315] [In-Wave Computation Aided Stacked Intelligent Metasurfaces in Next-Generation Networks: Challenges and Opportunities](https://arxiv.org/abs/2603.05892)
*Mengbing Liu,Chau Yuen,Dusit Niyato,Bruno Clerckx,Lajos Hanzo*

Main category: eess.SP

TL;DR: Stacked intelligent metasurfaces (SIMs) enable in-wave computation during signal propagation, offering potential for reduced energy, lower latency, and improved joint communication-sensing-computation tasks, though still facing scalability and robustness challenges.


<details>
  <summary>Details</summary>
Motivation: To enable signal processing in the wave domain during propagation rather than solely after reception, expanding controllable degrees of freedom and supporting joint design of communication, sensing, and computation with potential benefits in energy efficiency, latency reduction, and task execution.

Method: Review of state-of-the-art SIM research including applications, functions, and characteristics, with demonstration through case studies on neural-like analog inference and communication enhancement.

Result: SIMs show promise as a new signal processing paradigm for in-wave computation, but research is still at early stage with challenges in scalability, controllability, nonlinearity, and robustness that need to be addressed.

Conclusion: SIMs represent a promising new paradigm for next-generation networks, but require further research to overcome current challenges and establish them as viable in-wave computation technology.
...
### [316] [Ill-Posedness Analysis of CSI-Based Electromagnetic Inverse Scattering for Material Reconstruction in ISAC Systems](https://arxiv.org/abs/2603.05918)
*Yubin Luo,Li Yu,Takumi Takahashi,Shaoyi Liu,Yuxiang Zhang,Jianhua Zhang,Hideki Ochiai*

Main category: eess.SP

TL;DR: The paper analyzes electromagnetic inverse scattering for material reconstruction in ISAC systems, identifies ill-conditioning causes, proposes ROI-constrained optimization to improve conditioning and reduce complexity.


<details>
  <summary>Details</summary>
Motivation: CSI-based electromagnetic inverse scattering enables material-aware digital twins but suffers from severe ill-conditioning. Need to understand the origin of ill-posedness and develop methods to improve conditioning and computational efficiency.

Method: Analyzes mathematical properties of electromagnetic inverse problem and ISAC scattering matrix structure. Proves ROI restriction reduces condition number and CRLB. Implements ROI-constrained QP framework with linear sampling for coarse ROI delineation and QP update in reduced subspace.

Result: Background-related matrix columns are highly coherent causing near rank deficiency, while scatterer-related columns are weakly correlated with coherence decreasing with probing frequencies. ROI restriction yields provable condition-number reduction and tightened CRLB. FDTD simulations show pronounced conditioning improvement, substantial complexity savings, and improved robustness.

Conclusion: The analysis reveals structural causes of ill-conditioning in ISAC scattering matrices. ROI-constrained optimization effectively addresses these issues, providing conditioning improvement, complexity reduction, and robustness enhancement for material reconstruction in ISAC systems.
...
### [317] [Channel Estimation for Reconfigurable Intelligent Surface Assisted Upper Mid-Band MIMO Systems](https://arxiv.org/abs/2603.06017)
*Jeongjae Lee,Chanwon Kim,Songnam Hong*

Main category: eess.SP

TL;DR: Proposes a conditioning-aware channel estimation framework for RIS-assisted UMB communications that addresses ill-conditioned LS problems through greedy column grouping and piecewise RIS phase design.


<details>
  <summary>Details</summary>
Motivation: RIS-assisted UMB communications face severe channel estimation challenges due to near-field propagation and transitional scattering, which cause strong spatial correlation and ill-conditioned LS formulations that degrade estimation performance.

Method: Transforms ill-conditioned high-dimensional problem into multiple well-conditioned subproblems via greedy column grouping. Uses piecewise RIS phase design to separate highly correlated RIS elements into distinct sub-blocks, improving Gram matrix conditioning and enabling stable piecewise LS reconstruction without sparsity assumptions.

Result: Significantly outperforms conventional LS and OMP-based estimators in pilot-limited and transitional UMB regimes, achieving robust performance with low computational complexity.

Conclusion: The proposed conditioning-aware framework effectively addresses channel estimation challenges in RIS-assisted UMB communications by improving Gram matrix conditioning through systematic correlation management, offering a practical solution for 6G systems.
...
### [318] [Reinforcement Learning for Secrecy Optimization in Underwater Energy Harvesting Relay Network](https://arxiv.org/abs/2603.06046)
*Shalini Tripathi,Ankur Bansal,Chinmoy Kundu*

Main category: eess.SP

TL;DR: RL-based optimal power allocation for secure underwater hybrid optical-acoustic relay networks with energy harvesting, outperforming greedy and naive approaches.


<details>
  <summary>Details</summary>
Motivation: Secure communication in underwater energy-harvesting relay networks faces challenges: optical links can be blocked, acoustic links are vulnerable to eavesdropping, and power allocation must balance energy harvesting constraints with security needs.

Method: Formulate relay power allocation as infinite-horizon MDP, propose model-based RL-driven optimal power allocation strategy, and develop lower-complexity alternatives: Greedy Algorithm and Naive Algorithm.

Result: RL-based OPA adapts effectively to battery dynamics, varying channel conditions, and optical link availability, achieving highest secure data transmission; GA performs reasonably; NA performs poorly due to short-sighted decisions.

Conclusion: Model-based RL provides effective power allocation for secure underwater hybrid communication, balancing energy harvesting constraints with security requirements while outperforming simpler heuristic approaches.
...
### [319] [A Retrieval-Assisted Framework for Wireless Localization](https://arxiv.org/abs/2603.06158)
*Haoyu Huang,Guangjin Pan,Kaixuan Huang,Shunqing Zhang,Yuhao Zhang,Musa Furkan Keskin,Zheng Xing,Henk Wymeersch*

Main category: eess.SP

TL;DR: A unified retrieval-assisted fingerprinting framework combining similarity-based and learning-based approaches for accurate wireless localization using CSI, leveraging channel charting for dimensionality reduction and graph attention networks for correlation modeling.


<details>
  <summary>Details</summary>
Motivation: Traditional fingerprint-based localization methods face challenges: similarity-based approaches have high computational complexity and limited scalability in high-dimensional CSI spaces, while purely learning-based methods fail to explicitly exploit correlations among reference fingerprints during inference. There's a need to integrate both paradigms for better performance.

Method: Proposes a unified retrieval-assisted fingerprinting framework that integrates similarity-based and learning-based paradigms. Uses channel charting to project high-dimensional CSI into low-dimensional latent space for efficient retrieval of locally correlated reference points. Then employs a graph attention network (GAT) to model inter-sample correlations between query CSI and retrieved references, enabling adaptive geometry-aware feature aggregation for position estimation.

Result: Extensive experiments on real-world indoor and ray-tracing simulated outdoor scenarios demonstrate that the proposed method consistently outperforms state-of-the-art similarity-based and learning-based localization approaches.

Conclusion: The proposed unified framework successfully addresses limitations of both similarity-based and learning-based fingerprinting methods by combining their strengths through retrieval-assisted architecture, achieving superior localization performance across diverse scenarios.
...
### [320] [MAD: A Multimodal and Multi-perspective Affective Dataset with Hierarchical Annotations](https://arxiv.org/abs/2603.06206)
*Shengwei Guo,Yunqing Qiao,Wenzhan Zhang,Bo Liu,Yong Wang,Guobing Sun*

Main category: eess.SP

TL;DR: MAD is a multimodal emotion dataset with synchronized physiological signals (EEG, ECG, EOG, EMG, PPG, BCG) and tri-view RGB-D facial videos from 18 participants, featuring a three-level emotion annotation framework for comprehensive affective computing research.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive multimodal emotion dataset that enables observation of emotional dynamics from neural, physiological, and behavioral perspectives simultaneously, addressing the need for temporally aligned data across multiple modalities for affective computing and neurophysiological modeling.

Method: Built dataset through synchronous collection of diverse physiological signals (EEG, ECG, EOG, EMG, PPG, BCG) with tri-view RGB-D facial videos from 18 participants. Implemented a three-level emotion annotation framework covering stimulus elicitation, subjective cognition, and behavioral expression for joint modeling of the full emotion process.

Result: Systematic benchmark experiments show consistent and comparable performance across intra-subject EEG emotion recognition, cross-subject EEG transfer learning, cardiac signal classification, multimodal physiological fusion, and multi-view facial emotion recognition, establishing MAD as a reliable benchmark.

Conclusion: MAD provides a valuable resource for emotion recognition, cross-modal affective analysis, and studying emotion mechanisms across multiple levels, with synchronized multimodal data supporting comprehensive affective computing research.
...
### [321] [Set-Prediction-Based J-Peak Detection for Pillow-Based Ballistocardiography](https://arxiv.org/abs/2603.06221)
*Shengwei Guo,Guobing Sun*

Main category: eess.SP

TL;DR: A set-prediction-based framework for BCG J-peak detection that directly models peaks as discrete events, outperforming segmentation approaches with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Existing BCG J-peak detection methods use dense segmentation with heuristic post-processing, leading to redundant models and sensitivity to parameters. There's a need for more efficient and direct peak detection approaches.

Method: Proposed a set-prediction-based J-peak detection framework that directly models peaks as discrete temporal events, eliminating segmentation heads and explicit peak suppression. Created and released a pillow-based BCG-ECG dataset with multi-subject, multi-night sleep recordings and manual annotations.

Result: The proposed method achieves superior detection performance compared to U-Net-based segmentation baseline while substantially reducing model parameters and computational complexity.

Conclusion: Event-level set prediction provides a concise and efficient modeling paradigm for BCG J-peak detection in sleep monitoring, offering better performance with reduced complexity.
...
### [322] [U6G XL-MIMO Radiomap Prediction: Multi-Config Dataset and Beam Map Approach](https://arxiv.org/abs/2603.06401)
*Xiaojie Li,Yu Han,Zhizheng Lu,Shi Jin,Chao-Kai Wen*

Main category: eess.SP

TL;DR: First XL-MIMO radiomap dataset with 78,400 radiomaps across 800 urban scenes, 5 frequency bands, and 9 array configurations up to 32x32 directional arrays, plus a physics-informed beam map method that improves generalization by 60% for unseen configurations.


<details>
  <summary>Details</summary>
Motivation: Existing datasets only support small-scale arrays (up to 8x8) with isotropic antennas, while 6G requires 1024-element directional arrays. Current methods encode array configurations as scalar parameters, forcing neural networks to extrapolate radiation patterns, which fails for configurations absent from training data.

Method: 1) Constructed first XL-MIMO radiomap dataset with 78,400 radiomaps across 800 urban scenes, 5 frequency bands (1.8-6.7 GHz), and 9 array configurations up to 32x32 uniform planar arrays with directional elements. 2) Established comprehensive benchmark framework for practical scenarios. 3) Proposed beam map - a physics-informed spatial feature that analytically computes array-specific coverage patterns, decoupling deterministic array radiation from learned multipath propagation.

Result: Beam map integration reduces mean absolute error by up to 60.0% when generalizing to unseen configurations and up to 50.5% when transferring to unseen environments. Complete dataset and code are publicly available.

Conclusion: The paper addresses both data scarcity and generalization limitations for XL-MIMO radiomap prediction by providing the first large-scale dataset and introducing physics-informed beam maps that enable generalization to arbitrary beam configurations without retraining, shifting from neural network extrapolation to physics-based computation.
...
### [323] [The DCT Model as a Novel Regression Framework within a Lagrangian Formulation](https://arxiv.org/abs/2603.06418)
*Marc Martinez-Gost,Ana I. Perez Neira,Miguel Angel Lagunas*

Main category: eess.SP

TL;DR: A unified regression framework using Lagrangian formalism shows polynomial and logistic regression share common structure, with DCT-based model emerging as novel approach offering computational advantages.


<details>
  <summary>Details</summary>
Motivation: To create a unified mathematical framework that reveals the common variational structure underlying different regression methods (polynomial and logistic), and to explore novel approaches that emerge naturally from this framework.

Method: Develops a Lagrangian formalism-based regression framework where DCT (Discrete Cosine Transform) is used as constraints. Leverages the nearly orthogonal and bounded nature of cosine basis functions to create DCT-based regression models.

Result: The DCT-based model emerges naturally from the unified framework and demonstrates computational advantages and improved convergence properties compared to traditional polynomial regression methods.

Conclusion: The DCT-based neuron shows strong potential as a powerful tool for regression analysis and related learning tasks, offering benefits over conventional approaches within the unified Lagrangian framework.
...
### [324] [A Unified Multicarrier Waveform Framework for Next-generation Wireless Networks: Principles, Performance, and Challenges](https://arxiv.org/abs/2603.06515)
...
TL;DR: A unified multicarrier waveform framework for 6G networks that systematically analyzes and compares existing waveforms (OFDM, AFDM, OTFS) to guide waveform selection for diverse channel conditions.


<details>
  <summary>Details</summary>
Motivation: Next-generation wireless networks need enhanced flexibility, efficiency, and reliability in physical layer waveform design to handle heterogeneous channel conditions and stringent QoS demands for 6G and beyond.

Method: Proposes a unified multicarrier waveform framework that examines design principles of state-of-the-art waveforms categorized into one-dimensional (OFDM, AFDM) and two-dimensional (OTFS) modulation waveforms, studies their resilience against channel interference, and compares their KPIs.

Result: The analysis reveals distinct suitability of different waveforms for diverse channel conditions, provides practical implementation guidelines, and explores applications of advanced waveforms.

Conclusion: This work serves as a pivotal reference for waveform adoption in future 6G standardization and network deployment, facilitating informed waveform selection based on channel conditions and application requirements.
...
### [325] [Codebook Design and Baseband Precoding for Pragmatic Array-Fed RIS Hybrid Multiuser MIMO](https://arxiv.org/abs/2603.06541)
*Krishan Kumar Tiwari,Giuseppe Caire*

Main category: eess.SP

TL;DR: Extends previous HDA MU-MIMO architecture with phase-only flat-top beam design for AMAF-RIS modules and evaluates performance under realistic mmWave multipath channels with 3GPP-compliant beam acquisition and precoding.


<details>
  <summary>Details</summary>
Motivation: Previous work only evaluated performance under pure LOS conditions, lacking realistic channel modeling and practical beam design methods for wide angular coverage with low ripple and sidelobes.

Method: Proposes phase-only flat-top beam design for AMAF-RIS modules enabling hierarchical beamforming codebooks. Develops low-complexity HDA MU-MIMO framework with user-beam association, dynamic user grouping, 3GPP-compliant channel estimation, and zero-forcing precoding under per-antenna power constraints.

Result: Achieves high spectral efficiency and multiplexing gain while preserving hardware simplicity and power efficiency. The approach is fully compliant with 3GPP 5GNR beam acquisition and sounding reference signaling mechanisms.

Conclusion: The extended HDA MU-MIMO architecture with practical beam design and realistic channel modeling demonstrates robust performance in mmWave multipath environments while maintaining hardware and power efficiency.
...
### [326] [LiveSense: A Real-Time Wi-Fi Sensing Platform for Range-Doppler on COTS Laptop](https://arxiv.org/abs/2603.06545)
*Jessica Sanson,Rahul C. Shah,Maximilian Pinaroc,Cagri Tanriover,Valerio Frascolla*

Main category: eess.SP

TL;DR: LiveSense transforms COTS Wi-Fi NICs into centimeter-level Range-Doppler sensors while maintaining simultaneous communication, enabling real-time detection of distance, velocity, respiration, and gestures using commercial Wi-Fi hardware.


<details>
  <summary>Details</summary>
Motivation: To enable accurate sensing capabilities using commercial Wi-Fi hardware without specialized equipment, allowing for practical deployment of Wi-Fi sensing applications while preserving existing communication functions.

Method: Uses COTS Intel Wi-Fi 6E/7 NICs (AX211/BE201) with a cross-platform system that extracts synchronized CSI at ≥40Hz, performs time-phase alignment and self-interference cancellation on-device, and provides real-time data streams to a Python/Qt GUI.

Result: Demonstrates centimeter-level range sensing despite limited 160MHz bandwidth, capable of detecting: distance/radial velocity of nearby attendees, respiration micro-motion, and hand-gesture ranging in real-time.

Conclusion: LiveSense represents the first demonstration of accurate range sensing from commercial Wi-Fi hardware, enabling practical Wi-Fi sensing applications using existing laptop hardware while maintaining communication capabilities.
