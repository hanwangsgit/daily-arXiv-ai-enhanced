...
TL;DR: VLMMs struggle with temporal event ordering despite good performance on existing benchmarks; new benchmark VECTOR reveals this weakness; proposed MECOT method improves temporal understanding through event-by-event training and chain-of-thought prompting.


<details>
  <summary>Details</summary>
Motivation: Current Video Large Multimodal Models (VLMMs) perform well on existing benchmarks but may not actually understand temporal order of events, instead relying on prior knowledge of typical scenarios. The authors discovered this by showing models perform well even when video frames are scrambled, indicating a gap in temporal understanding capabilities.

Method: 1) Proposed VECTOR benchmark to explicitly assess temporal order understanding; 2) Developed MECOT (Multi-Event instruction fine-tuning with Chain-of-Thought) which trains models on detailed event-by-event video descriptions and uses chain-of-thought prompts at inference to enhance temporal awareness.

Result: Various VLMMs often fail to understand event orders on VECTOR benchmark. MECOT outperforms prior methods on VECTOR and also improves performance on existing video benchmarks, demonstrating effectiveness in enhancing temporal understanding.

Conclusion: Temporal understanding is a critical but underexplored capability in VLMMs. The proposed VECTOR benchmark reveals current limitations, while MECOT provides an effective solution through event-focused training and reasoning prompts, advancing video understanding capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current open-source VLM-based agents are limited to single-image inputs, failing to handle real-world multi-image QA tasks. There's a need for agents that can effectively process and reason across multiple images while maintaining visual attention during deeper reasoning steps.

Method: 1) Generate MIFG-QA dataset (10k samples) using multi-agent system for challenging multi-image QA pairs; 2) Develop specialized tools for visual reflection and confirmation to maintain visual attention; 3) Use end-to-end RL training with action-trajectory two-level mask strategy for stable tool use without supervised fine-tuning.

Result: IMAgent maintains strong performance on existing single-image benchmarks while achieving substantial improvements on the proposed multi-image dataset. The method provides stable tool use behavior through pure RL training.

Conclusion: IMAgent successfully addresses multi-image QA challenges through specialized visual tools and RL training, offering an effective open-source alternative to proprietary systems like OpenAI O3 for complex visual reasoning tasks.
...
TL;DR: UTIE reduces demographic bias in face recognition by enriching facial embeddings with text-derived demographic features from other groups using vision-language models, achieving fairer verification performance.
...
Method: Proposes Unified Text-Image Embedding (UTIE) that leverages Vision-Language Models (CLIP, OpenCLIP, SigLIP) to enrich facial embeddings of each demographic group with text-derived demographic features from other groups, encouraging demographic ambiguity and emphasizing identity-relevant features.
...
Conclusion: UTIE effectively addresses demographic bias in face recognition by inducing demographic ambiguity in embeddings through cross-modal enrichment, promoting fairer verification performance without compromising accuracy.
...
TL;DR: Consist-Retinex adapts consistency models to Retinex-based low-light enhancement, enabling one-step sampling with state-of-the-art performance while reducing computational costs.
...
Conclusion: Consist-Retinex successfully adapts consistency modeling to Retinex-based low-light enhancement, enabling efficient one-step generation while maintaining high-quality results, bridging the gap between unconditional synthesis and conditional enhancement tasks.
...
TL;DR: HSCP: Hierarchical Spectral Clustering Pruning framework for UAV RFFI that combines layer and channel pruning to achieve extreme compression while improving accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Traditional UAV identification methods struggle with feature extraction and real-time requirements in complex environments. Deep learning RFFI approaches have accuracy but face deployment challenges on resource-constrained edge devices due to large model sizes and high computational demands. Existing pruning techniques fail to optimize compression, hardware acceleration, and accuracy simultaneously.

Method: Two-stage hierarchical pruning: 1) Spectral clustering guided by Centered Kernel Alignment (CKA) to identify and remove redundant layers, 2) Same strategy applied to channel dimension for finer redundancy elimination. Followed by noise-robust fine-tuning for robustness.
...
Conclusion: HSCP outperforms existing channel and layer pruning methods, achieving extreme compression, high performance, and efficient inference for UAV RFFI on resource-constrained edge devices.
...
Method: Computes lightweight statistical descriptors, retrieves similar samples from vector database, uses contextual evidence for LLM-based activity identification. Enhanced with prompt optimization and LLM-based activity descriptors for context-enriched databases.

Result: Achieves state-of-the-art performance across six diverse HAR benchmarks. Enables recognition and meaningful labeling of multiple unseen human activities without requiring training or fine-tuning.

Conclusion: RAG-HAR offers a robust, training-free framework for HAR that moves beyond known behaviors, emphasizing practical applicability and eliminating computational resource demands of traditional approaches.
...
TL;DR: Proposes Verifier-Threshold method for efficient test-time compute allocation in diffusion/flow models, achieving 2-4x speedup over SOTA on GenEval benchmark.


<details>
  <summary>Details</summary>
Motivation: Current methods for allocating non-uniform inference-compute budgets across denoising steps in diffusion/flow models use greedy algorithms that allocate compute ineffectively, wasting computational resources.

Method: Verifier-Threshold method that automatically reallocates test-time compute across different denoising steps more efficiently than greedy approaches.

Result: Achieves 2-4x reduction in computational time over state-of-the-art methods while maintaining same performance on the GenEval benchmark.

Conclusion: The proposed method effectively optimizes test-time compute allocation for image generation models, delivering substantial efficiency improvements without sacrificing quality.
...
TL;DR: Proposed a quality-control framework for diabetic retinopathy datasets that filters inadequate images, enhances them, provides annotation assistance, and evaluates annotator agreement to ensure high-quality data for AI training.


<details>
  <summary>Details</summary>
Motivation: Diabetic retinopathy requires early diagnosis to prevent vision loss, and AI can help clinicians identify lesions, but current models need high-quality annotated datasets. Manual annotation suffers from errors due to complex retinal structures and image acquisition issues.

Method: Three-stage framework: 1) Explainable feature-based classifier filters inadequate images using features from image processing and contrastive learning; 2) Image enhancement and deep-learning-assisted annotation; 3) Agreement calculation between annotators using derived formulas to determine annotation usability.

Result: The framework ensures only high-standard data is used for evaluation and AI training by systematically filtering poor images, enhancing quality, assisting annotation, and validating annotator agreement.

Conclusion: The proposed quality-control framework addresses data quality issues in diabetic retinopathy datasets, enabling more reliable AI model development for clinical applications by ensuring high-quality annotated training data.
...
TL;DR: 3DID framework enables true 3D inverse design from scratch by combining continuous latent representation with physics-aware optimization, outperforming existing methods that rely on 2D projections or fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current inverse design methods for 3D domains either use 2D projections or fine-tune existing 3D shapes, which sacrifices volumetric detail and constrains design exploration, preventing true 3D design from scratch.

Method: Proposes a 3D Inverse Design (3DID) framework that learns a unified physics-geometry embedding in continuous latent space, then uses a two-stage optimization: 1) gradient-guided diffusion sampler explores global latent manifold, 2) objective-driven, topology-preserving refinement sculpts candidates toward target objectives.

Result: 3DID generates high-fidelity 3D geometries and outperforms existing methods in both solution quality and design versatility.

Conclusion: The proposed framework enables true 3D inverse design from scratch by directly navigating the 3D design space through continuous latent representation and physics-aware optimization.
...
<details>
  <summary>Details</summary>
Motivation: Existing HSI transfer methods are limited by assumptions of homogeneous domains or co-occurring categories, and they fail to handle target-private information when label spaces don't overlap, restricting effective knowledge transfer across heterogeneous scenes.

Method: CKI framework with three components: 1) ASC for spectral discrepancy reduction via domain-agnostic projection, 2) CKSP with SSM for semantic mismatch resolution, and 3) CII for maximizing target-specific complementary information utilization.
...
TL;DR: LiM-YOLO is a specialized ship detector for satellite imagery that addresses scale disparity and morphological anisotropy by shifting detection heads to shallower pyramid levels and adding normalization for training stability.


<details>
  <summary>Details</summary>
Motivation: General-purpose object detectors struggle with ship detection in satellite imagery due to extreme scale differences and anisotropic shapes of ships. Standard architectures with deep layers (P5) fail to resolve narrow vessels, causing spatial feature dilution.

Method: Based on statistical analysis of ship scales, introduces Pyramid Level Shift Strategy to reconfigure detection heads from P5 to P2-P4 layers. Also incorporates Group Normalized Convolutional Block for Linear Projection (GN-CBLinear) to stabilize training on high-resolution inputs in micro-batch settings.
...
Conclusion: LiM-YOLO effectively resolves domain-specific conflicts in ship detection from satellite imagery through pyramid level adaptation and training stabilization techniques, achieving better performance than existing methods.
...
TL;DR: Proposes Deterministic World Model (DWM) for vision-based control verification, eliminating stochastic latent variables to reduce overapproximation error and improve precision.


<details>
  <summary>Details</summary>
Motivation: Verifying closed-loop vision-based control systems is challenging due to high-dimensional images and difficulty modeling visual environments. Current generative models as camera surrogates rely on stochastic latent variables, introducing unnecessary overapproximation error.

Method: Develops Deterministic World Model (DWM) that maps system states directly to generative images, eliminating uninterpretable latent variables. Uses dual-objective loss combining pixel-level reconstruction accuracy with control difference loss for behavioral consistency. Integrates DWM with Star-based reachability analysis (StarV) and employs conformal prediction for statistical bounds on trajectory deviation.

Result: Experiments on standard benchmarks show DWM yields significantly tighter reachable sets and better verification performance than latent-variable baselines.

Conclusion: DWM addresses the bottleneck of stochastic latent variables in vision-based control verification, enabling more precise input bounds and improved verification performance through deterministic mapping from system states to images.
...
TL;DR: A generative model predicts 3D radiotherapy dose distributions using customizable user preferences instead of fixed reference plans, enabling personalized planning and outperforming Varian RapidPlan in some scenarios.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for 3D dose prediction rely on reference plans as ground truth, which biases models toward specific institutional planning styles and preferences, limiting flexibility and personalization.

Method: A novel generative model that predicts 3D dose distributions based solely on user-defined preference flavors, allowing planners to customize trade-offs between organs-at-risk (OARs) and planning target volumes (PTVs). Designed for clinical integration.

Result: The method demonstrates superior adaptability and plan quality compared to Varian RapidPlan in some scenarios, enabling efficient generation of high-quality personalized radiotherapy plans.

Conclusion: The approach offers a flexible, preference-driven alternative to reference-based dose prediction models, providing greater personalization and clinical utility in radiotherapy planning.
...
TL;DR: A diffusion model regularized implicit neural representation framework for metal artifact reduction in CT images that combines physical constraints with learned priors.


<details>
  <summary>Details</summary>
Motivation: Existing supervised MAR methods rely on limited paired metal-clean data, causing performance instability. Unsupervised methods fail to effectively incorporate CT physical geometry and traditional regularization can't capture rich prior knowledge.

Method: Proposes a framework combining implicit neural representation (integrates physical constraints and data fidelity) with pre-trained diffusion models (provides prior knowledge regularization).
...
Conclusion: The proposed framework successfully addresses limitations of existing MAR methods by integrating physical constraints with learned priors, offering promising clinical applicability.
...
TL;DR: Novel method generates large-scale, physically valid lithography defect datasets with pixel-level annotations using physics-constrained morphology operations and DMD-based fabrication, achieving ~34-42% AP@0.5 improvement over baseline models.


<details>
  <summary>Details</summary>
Motivation: AI in micro/nano manufacturing faces data scarcity for defect inspection, especially in semiconductor lithography where defect data is rarely accessible for research, creating a bottleneck for robust AI-based measurement/inspection systems.

Method: 1) Ab initio defect synthesis using physics-constrained mathematical morphology (erosion/dilation) on design layouts; 2) Physical fabrication via DMD-based lithography; 3) Optical micrograph comparison for pixel-accurate annotation; 4) Dataset construction with 3,530 images and 13,365 defect instances across four classes.

Result: Mask R-CNN achieves AP@0.5 of 0.980, 0.965, 0.971 for bridge, burr, pinch classes (vs 0.740, 0.719, 0.717 for Faster R-CNN), representing ~34% mean improvement. For contamination class, Mask R-CNN achieves ~42% higher AP@0.5 than Faster R-CNN.

Conclusion: The methodology successfully generates physically valid defect datasets with pixel-level annotations, enabling robust AI-based measurement/inspection in semiconductor fabrication with significant performance improvements over baseline approaches.
...
TL;DR: This paper is a comprehensive survey reviewing body and face motion generation, covering concepts, representations, generative approaches, datasets, and evaluation metrics, with a focus on enhancing avatar realism in dyadic communication.


<details>
  <summary>Details</summary>
Motivation: Body and face motion are crucial for communication but generating expressive and coherent motion remains challenging due to complex verbal/non-verbal cue interplay and individual personality traits. There's a need for a comprehensive review covering both body and face motion generation.

Method: The paper conducts a systematic survey covering core concepts, representation techniques, generative approaches (including generative modeling and multi-modal learning), datasets, and evaluation metrics for body and face motion generation.

Result: The survey provides the first comprehensive review covering both body and face motion generation, organizing existing research and highlighting current challenges in generating realistic, coherent, and expressive avatar motions.

Conclusion: The paper identifies future directions to enhance realism, coherence, and expressiveness of avatars in dyadic settings, and provides detailed resources through a dedicated website for the research community.
...
TL;DR: LUVC is a training-free framework that compresses visual tokens in VLMs through iterative merging in the visual encoder and spectrum pruning in LLM layers, achieving 2× speedup with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Visual language models suffer from computational inefficiency due to redundant token representations in high-resolution images/videos. Existing compression methods have position bias, class imbalance issues, and don't generalize well to shallow LLM layers with weaker cross-modal interactions.

Method: 1) Extends token compression to visual encoder via iterative merging orthogonal in spatial axes. 2) Integrates spectrum pruning unit into LLM using attention/similarity-free low-pass filter that gradually prunes redundant visual tokens (FlashAttention compatible). 3) Proposes LUVC framework that systematically compresses visual tokens until complete elimination at final LLM layer, gradually fusing high-dimensional visual features into multimodal queries.
...
Conclusion: LUVC effectively addresses computational efficiency challenges in VLMs through systematic visual token compression that maintains accuracy while significantly accelerating inference, with the advantage of being immediately deployable without retraining.
...
TL;DR: A deep learning approach for detecting specific individuals in video sequences using supervised learning algorithms, with applications for national security systems in Angola.


<details>
  <summary>Details</summary>
Motivation: To develop a system for searching and detecting specific entities (individuals) in video sequences, addressing the complexity of object detection in video data and aiming to enhance national security systems.

Method: Uses deep learning with artificial neural networks and supervised learning algorithms structured for computer vision tasks, focusing on simple characteristics of target characters for detection.

Result: The approach successfully locates wanted individuals efficiently from image databases, demonstrating effectiveness compared to state-of-the-art methods. Specifically enables detection from Angola's Integrated Public Security Centre (CISP) video sequences.

Conclusion: The proposed classifier provides a practical solution for reinforcing national security systems by enabling efficient detection of target individuals (disappeared persons, criminals) from video surveillance data.
...
TL;DR: E-Deflare is the first framework for removing lens flare from event camera data, featuring a physics-based forward model, a comprehensive benchmark with simulated and real datasets, and a neural network that achieves state-of-the-art restoration performance.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer high temporal resolution and dynamic range but remain vulnerable to lens flare, a fundamental optical artifact that causes severe degradation in event streams. This complex spatio-temporal distortion has been largely overlooked in event camera research, creating a need for systematic flare removal methods.

Method: The authors first derive a physics-grounded forward model of the non-linear suppression mechanism for lens flare in event cameras. They then create the E-Deflare Benchmark with two components: E-Flare-2.7K (large-scale simulated training set) and E-Flare-R (first-ever paired real-world test set captured using a novel optical system). Based on this benchmark, they design E-DeflareNet for flare removal.

Result: E-DeflareNet achieves state-of-the-art restoration performance for lens flare removal from event camera data. Extensive experiments validate the approach and demonstrate clear benefits for downstream tasks. The code and datasets are publicly available.

Conclusion: E-Deflare represents the first systematic framework for addressing lens flare in event camera data, providing both theoretical foundations through physics-based modeling and practical solutions through comprehensive benchmarks and effective neural network architecture, enabling improved event-based vision systems.
...
TL;DR: ConceptPose: A training-free, model-free framework for object pose estimation using vision-language models to create 3D concept maps and establish 3D-3D correspondences for precise 6DoF pose estimation.


<details>
  <summary>Details</summary>
Motivation: Most object pose estimation methods require extensive dataset-specific training, while vision-language models show remarkable zero-shot capabilities. The paper aims to bridge these two worlds by creating a training-free approach to pose estimation.

Method: Uses vision-language models to create open-vocabulary 3D concept maps where each point is tagged with concept vectors from saliency maps. Establishes robust 3D-3D correspondences across concept maps to estimate 6DoF relative pose.

Result: Achieves state-of-the-art results on zero-shot relative pose estimation benchmarks, outperforming existing methods by over 62% in ADD(-S) score, including methods that use extensive dataset-specific training.

Conclusion: ConceptPose demonstrates that training-free, model-free pose estimation is possible by leveraging vision-language models, achieving superior performance without object or dataset-specific training.
...
Method: Created SIP dataset using terrestrial LiDAR scanner with indoor/outdoor construction scenes, annotated at point level using construction-specific taxonomy (Built Environment, Construction Operations, Site Surroundings). Includes structural components and slender temporary objects like scaffolding, MEP piping, and scissor lifts.

Result: SIP dataset captures real-world sensing characteristics including radial density decay, fragmented geometry, and view-dependent visibility. It's openly available with Git repository and adaptable class configurations for modern 3D deep learning frameworks.

Conclusion: SIP enables robust benchmarking for construction-oriented 3D vision tasks by providing field data that retain real-world sensing characteristics, addressing the gap between ideal lab conditions and practical construction site constraints.
...
TL;DR: KD-OCT is a knowledge distillation framework that compresses a large ConvNeXtV2-Large teacher model into a lightweight EfficientNet-B2 student for efficient AMD/CNV classification from OCT images, enabling clinical deployment.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for AMD/CNV detection from OCT images are computationally demanding, hindering clinical deployment. There's a need for efficient models that maintain high diagnostic performance while enabling real-time use in clinical settings.

Method: Proposes KD-OCT knowledge distillation framework: uses ConvNeXtV2-Large teacher (enhanced with advanced augmentations, stochastic weight averaging, focal loss) to train lightweight EfficientNet-B2 student via real-time distillation with combined loss balancing soft teacher knowledge and hard ground-truth supervision.

Result: KD-OCT achieves near-teacher performance with substantial reductions in model size and inference time, outperforming comparable multi-scale/feature-fusion OCT classifiers in efficiency-accuracy balance on the NEH dataset using patient-level cross-validation.

Conclusion: The compressed student model exceeds most existing frameworks, facilitating edge deployment for AMD screening while maintaining diagnostic performance comparable to state-of-the-art large models.
...
<details>
  <summary>Details</summary>
Motivation: Current VPR systems rely on manually set thresholds for determining good image matches, which is difficult to maintain across different visual scenarios (seasonal changes, weather, illumination, structural changes, traffic). There's a need for automatic threshold selection that works across various image databases and descriptors.

Method: The approach analyzes 'negative' Gaussian mixture statistics for a place - specifically, image statistics indicating "not this place." This statistical analysis of negative examples enables automatic threshold determination.

Result: The method successfully selects thresholds that work well for a variety of image databases and image descriptors, demonstrating robustness across different visual scenarios.

Conclusion: Automatic threshold selection for VPR is achievable through statistical analysis of negative place examples, providing a more practical and robust solution than manual threshold setting for real-world robot implementations.
...
TL;DR: AgentComp is a framework that improves text-to-image model compositionality by using LLM agents to create compositional datasets and applying agentic preference optimization, achieving SOTA results without compromising image quality.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with compositionality - accurately capturing object relationships, attribute bindings, and fine-grained details. They lack explicit training to differentiate between compositionally similar prompts, leading to outputs that deviate in subtle details.

Method: AgentComp leverages LLMs with image generation, editing, and VQA tools to autonomously construct compositional datasets. It then applies agentic preference optimization to fine-tune text-to-image models, enabling better differentiation between compositionally similar samples.

Result: AgentComp achieves state-of-the-art results on compositionality benchmarks like T2I-CompBench without compromising image quality. It also generalizes to other capabilities not explicitly trained for, such as text rendering.

Conclusion: The framework successfully addresses compositionality limitations in text-to-image models by explicitly training them to differentiate compositional variations, resulting in stronger compositional generation ability while maintaining image quality.
...
<details>
  <summary>Details</summary>
Motivation: Underground mining disasters create darkness, dust, and collapses that obscure vision, making situational awareness difficult for both humans and conventional systems, necessitating automated scene understanding solutions.
...
TL;DR: FoCULR improves food image generation for multi-noun categories by incorporating domain knowledge and layout refinement to prevent misinterpretation of compound food names.
...
Method: Proposes FoCULR (Food Category Understanding and Layout Refinement) which incorporates food domain knowledge and introduces core concepts early in the generation process to address text encoder limitations and spatial layout misinterpretations.

Result: Experimental results show that integrating these techniques improves image generation performance in the food domain, particularly for challenging multi-noun categories.

Conclusion: The proposed FoCULR approach effectively addresses the challenges of generating realistic food images for multi-noun categories by incorporating domain-specific knowledge and refining spatial layouts during generation.
...
TL;DR: GimbalDiffusion enables precise camera control in text-to-video generation using physical-world coordinates and gravity as reference, allowing absolute camera trajectory specification rather than relative motion.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-video generation lacks fine-grained control over camera motion and orientation, using ambiguous relative representations that limit explicit geometric control.

Method: Uses absolute coordinate system with gravity reference, leverages panoramic 360-degree videos for diverse trajectories, introduces null-pitch conditioning to reduce text-camera conflicts, and establishes a rebalanced benchmark for evaluation.

Result: Enables precise, interpretable camera parameter control without initial reference frames, supports wide camera pitch variation, and improves model robustness when camera specifications conflict with text content.

Conclusion: GimbalDiffusion advances text-to-video controllability by enabling gravity-aligned camera manipulation with physical-world coordinate grounding, moving beyond conventional forward-facing trajectories.
...
TL;DR: SuperF is a test-time optimization approach for multi-image super-resolution (MISR) that uses neural fields to enhance image resolution from multiple shifted low-resolution frames without hallucinating structures.


<details>
  <summary>Details</summary>
Motivation: High-resolution imagery faces limitations from sensor technology, atmospheric conditions, and costs. Single-image super-resolution methods often hallucinate structures, while multi-image super-resolution (MISR) can improve resolution using multiple views with sub-pixel shifts but needs better approaches.

Method: Uses coordinate-based neural networks (neural fields) with an implicit neural representation (INR) shared across multiple shifted low-resolution frames. Jointly optimizes frame alignment and INR representation, parameterizing sub-pixel alignment as optimizable affine transformations and optimizing via a super-sampled coordinate grid matching output resolution.

Result: Compelling results on simulated bursts of satellite imagery and ground-level images from handheld cameras with upsampling factors up to 8x. Key advantage: doesn't rely on any high-resolution training data.

Conclusion: SuperF provides an effective test-time optimization approach for MISR using neural fields, avoiding hallucinated structures while achieving high-quality super-resolution without requiring high-resolution training data.
...
TL;DR: AngioAI-QFR is an end-to-end angiography pipeline combining deep learning stenosis detection, lumen segmentation, and automated QFR computation with virtual PCI planning, achieving strong correlation with invasive FFR and fast processing times.


<details>
  <summary>Details</summary>
Motivation: Current angiography-based assessment of coronary artery disease has limitations: visual grading is variable and moderately related to ischemia, wire-based FFR is underutilized, and existing angiography-derived tools are workflow-intensive and separate from anatomy analysis and virtual PCI planning.

Method: Developed AngioAI-QFR, an end-to-end angiography-only pipeline combining deep learning stenosis detection, lumen segmentation, centerline and diameter extraction, per-millimeter Relative Flow Capacity profiling, and virtual stenting with automatic recomputation of angiography-derived QFR.

Result: Strong correlation with invasive FFR (r=0.89, MAE 0.045), excellent diagnostic performance (AUC 0.93, sensitivity 0.88, specificity 0.86), fast processing (median 41 seconds), and 93% fully automatic completion. RFC profiling distinguished focal from diffuse disease, and virtual stenting predicted larger QFR gain in focal disease.
...
<details>
  <summary>Details</summary>
Motivation: Gaussian Splatting enables photorealistic head avatar reconstruction but lacks intuitive editability compared to traditional mesh-based methods with UV texture mapping.
...
Conclusion: Successfully combines Gaussian Splatting's fidelity with UV texture mapping's editability for practical head avatar applications in visual effects, videoconferencing, and VR.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of continual adaptation in compositional zero-shot learning where models need to learn new attribute-object compositions over time while preventing catastrophic forgetting of prior knowledge, which is more complex than classical continual learning due to shared attributes/objects across sessions.

Method: Built on frozen VLM backbone with recency-weighted multi-teacher distillation. Uses session-aware compositional prompts for new compositions and session-agnostic attribute/object prompts for global consistency. Includes Cosine Anchor Loss (CAL) for stability, Orthogonal Projection Loss (OPL) to prevent overlap with previous embeddings, and Intra-Session Diversity Loss (IDL) for richer representations.
...
Conclusion: PromptCCZSL effectively addresses continual compositional zero-shot learning by balancing adaptation to new compositions with preservation of prior knowledge through innovative prompt engineering and specialized loss functions, establishing a strong foundation for future CCZSL research.
...
Method: Proposes Δ-LFM framework with two key components: 1) Uses Flow Matching (FM) to treat disease dynamics as a velocity field, aligning temporal evolution of patient data, and 2) Learns patient-specific latent alignment that enforces patient trajectories to lie along a specific axis with magnitude increasing monotonically with disease severity, creating a consistent semantic latent space.

Result: Demonstrates strong empirical performance across three longitudinal MRI benchmarks, and provides a new framework for interpreting and visualizing disease dynamics with semantically meaningful progression trajectories.

Conclusion: Δ-LFM offers an interpretable approach to disease progression modeling by combining Flow Matching with patient-specific latent alignment, addressing key limitations of existing methods and providing clinically meaningful representations of disease dynamics.
...
TL;DR: Proposes View-on-Graph (VoG), a new zero-shot 3D visual grounding method that externalizes 3D spatial information into a multi-modal scene graph, allowing VLMs to actively explore and retrieve only needed cues during reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot 3DVG approaches use VLM + SI paradigm that yields entangled visual representations, forcing VLMs to process entire cluttered cues and making it hard to exploit spatial semantic relationships effectively.

Method: Introduces VLM x SI paradigm that externalizes 3D spatial information into a form enabling incremental retrieval. Instantiates with View-on-Graph (VoG) method that organizes scenes into multi-modal, multi-layer scene graphs, allowing VLM to operate as an active agent that selectively accesses necessary cues while traversing the scene.

Result: VoG achieves state-of-the-art zero-shot performance, establishing structured scene exploration as a promising strategy for advancing zero-shot 3DVG.

Conclusion: The proposed VLM x SI paradigm with VoG method offers two advantages: (1) lowers VLM's reasoning difficulty by structuring 3D context into coherent scene graphs rather than entangled visual inputs, and (2) produces transparent, step-by-step traces for interpretable 3DVG through active scene graph exploration.
...
TL;DR: The paper introduces the Feature Coding for Machines (FCM) standard by MPEG-AI, which enables efficient compression and transmission of neural network features for AI applications, reducing bitrate by 75.9% compared to remote inference while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: As consumer devices become more intelligent and interconnected, there's a growing need for efficient data transfer solutions for machine tasks. Low-powered devices need to leverage large deep learning models, but face computational limitations.
...
Conclusion: The FCM standard provides an effective solution for AI-driven applications on resource-constrained devices by enabling efficient feature coding and transmission, significantly reducing bandwidth requirements while preserving model accuracy.
...
TL;DR: Proposed Z-score normalization method for feature compression in split-inference AI systems reduces bitrate by 17.09% on average across tasks and up to 65.69% for object tracking without accuracy loss.
...
Method: Uses Z-score normalization to efficiently recover compressed feature data at decoder side; integrated into MPEG's FCM codec standard; also proposes simplified method for further overhead reduction in certain circumstances.

Result: Method reduces overhead bits and improves end-task accuracy; shows 17.09% average bitrate reduction across different tasks and up to 65.69% for object tracking without sacrificing accuracy; supersedes existing scaling method in current standard.

Conclusion: Z-score normalization effectively compresses feature data in split-inference AI systems, significantly reducing bitrate while maintaining or improving task accuracy, making it superior to existing methods in the FCM codec standard.
...
<details>
  <summary>Details</summary>
Motivation: CKD is a major global health burden with high morbidity/mortality, requiring reliable early diagnostic approaches for timely clinical management.

Method: Deep convolutional neural network (CNN) for CKD detection from CT images, using SMOTE for class balancing and Grad-CAM for interpretability, trained on CT KIDNEY DATASET with 12,446 images across cyst, normal, stone, and tumor classes.

Result: The model achieved 100% accuracy in early CKD detection, demonstrating exceptional classification performance.

Conclusion: The approach shows strong potential for addressing clinical diagnostic challenges and enhancing early medical intervention strategies for CKD.
...
TL;DR: OmniPSD is a diffusion framework for generating and decomposing layered PSD files with transparency, using in-context learning and spatial attention to handle multiple layers.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models excel at image generation/editing but struggle with generating or reconstructing layered PSD files with transparent alpha channels, which are essential for professional design workflows.

Method: Built on Flux ecosystem, uses in-context learning for both text-to-PSD generation (spatial arrangement of layers with attention) and image-to-PSD decomposition (iterative in-context editing). Employs RGBA-VAE to preserve transparency without affecting structure learning.

Result: Extensive experiments on new RGBA-layered dataset show high-fidelity generation, structural consistency, and transparency awareness, outperforming existing approaches.

Conclusion: OmniPSD offers a new paradigm for layered design generation and decomposition using diffusion transformers, enabling professional-grade PSD file creation and editing workflows.
...
TL;DR: GLACIA is a novel glacial lake segmentation framework that integrates large language models with segmentation to produce both accurate masks and spatial reasoning outputs, outperforming existing CNN, ViT, and geo-foundation models.


<details>
  <summary>Details</summary>
Motivation: Existing glacial lake segmentation methods based on CNNs and Vision Transformers are limited to pixel-level predictions without high-level global scene semantics and human-interpretable reasoning needed for disaster preparedness and policy-making.

Method: GLACIA integrates large language models with segmentation capabilities, creating the Glacial Lake Position Reasoning (GLake-Pos) dataset pipeline with diverse, spatially grounded question-answer pairs to address the lack of instance-aware positional reasoning data in remote sensing.
...
Conclusion: The framework enables intuitive disaster preparedness and informed policy-making in rapidly changing glacial environments by facilitating natural language interaction, supporting more efficient and interpretable decision-making.
...
TL;DR: ROI-Packing is a specialized image compression method for machine vision that prioritizes and efficiently packs regions of interest while discarding less relevant data, achieving significant compression gains without retraining end-task models.


<details>
  <summary>Details</summary>
Motivation: Traditional image compression methods are optimized for human perception, not machine vision tasks. There's a need for compression methods that maintain end-task accuracy while achieving better compression efficiency for machine vision applications.
...
Result: Evaluations across five datasets and two tasks (object detection and instance segmentation) show up to 44.10% reduction in bitrate without compromising accuracy, and 8.88% improvement in accuracy at the same bitrate compared to VVC codec.

Conclusion: ROI-Packing provides an effective compression solution for machine vision that significantly outperforms state-of-the-art general-purpose codecs by focusing compression resources on task-critical regions.
...
TL;DR: MoRel: A 4D Gaussian Splatting framework with Anchor Relay-based Bidirectional Blending for memory-efficient, temporally consistent modeling of long-range dynamic scenes, addressing memory explosion and flickering issues in existing 4DGS methods.


<details>
  <summary>Details</summary>
Motivation: Existing 4D Gaussian Splatting methods struggle with long-range dynamic videos, suffering from severe memory explosion, temporal flickering, and failure to handle appearing/disappearing occlusions over time.

Method: Proposes MoRel with Anchor Relay-based Bidirectional Blending (ARBB) mechanism that constructs locally canonical anchor spaces at key-frames and models inter-frame deformations at anchor level. Includes Feature-variance-guided Hierarchical Densification (FHD) scheme and introduces SelfCap_LR dataset for evaluation.

Result: Achieves temporally coherent and flicker-free long-range 4D reconstruction while maintaining bounded memory usage, demonstrating scalability and efficiency in dynamic Gaussian-based representations.

Conclusion: MoRel effectively addresses key challenges in 4DGS for long-range dynamic scenes through anchor-based bidirectional blending and hierarchical densification, enabling practical real-time rendering of complex temporal dynamics.
...
TL;DR: LongT2IBench: A benchmark with 14K long text-image pairs and graph-structured annotations for evaluating text-to-image alignment in long prompt scenarios, plus LongT2IExpert, an MLLM-based evaluator with hierarchical alignment chain-of-thought.
...
Method: 1) Created LongT2IBench with 14K long text-image pairs using Generate-Refine-Qualify protocol to convert prompts into textual graph structures (entities, attributes, relations). 2) Proposed LongT2IExpert, an MLLM instruction-tuned with Hierarchical Alignment Chain-of-Thought to provide both scores and structured interpretations.

Result: Established comprehensive benchmark with fine-grained graph annotations; LongT2IExpert demonstrates superiority in alignment evaluation and interpretation compared to existing methods.

Conclusion: LongT2IBench addresses the gap in long prompt evaluation, providing interpretable graph-structured annotations, while LongT2IExpert offers effective quantitative and qualitative assessment of long text-image alignment.
...
<details>
  <summary>Details</summary>
Motivation: Parkinson's disease significantly impacts patients' daily functioning and social interactions. Current diagnostic approaches need to be more efficient and accessible. The paper aims to facilitate PD diagnosis by targeting hypomimia, a characteristic clinical symptom, through non-invasive facial expression analysis.
...
Result: Achieves 93.1% accuracy in PD diagnosis, outperforming other in-vitro PD diagnostic approaches. The method demonstrates effectiveness in detecting hypomimia symptoms through facial expression analysis.

Conclusion: The proposed dynamic facial expression analysis method offers a more convenient, non-invasive detection approach for potential PD patients, improving diagnostic experience and accessibility while maintaining high accuracy comparable to traditional methods.
...
TL;DR: LoGoColor is a novel 3D colorization method that preserves color diversity by eliminating guidance averaging and using a Local-Global approach with multi-view diffusion models, achieving better consistency and color variety in complex 360° scenes.


<details>
  <summary>Details</summary>
Motivation: Existing 3D colorization methods that distill 2D image colorization models suffer from inconsistency issues, causing colors to be averaged during training and resulting in monotonous, oversimplified outputs, especially in complex 360° scenes.

Method: LoGoColor uses a 'Local-Global' approach: partitions scenes into subscenes and addresses both inter-subscene and intra-subscene consistency using a fine-tuned multi-view diffusion model, generating consistently colorized training views to bypass the averaging process.
...
Conclusion: LoGoColor successfully preserves color diversity in 3D colorization by eliminating guidance averaging and ensuring multi-view consistency through a Local-Global approach, outperforming previous methods on complex 360° scenes.
...
TL;DR: FoundIR-v2 is a diffusion-based image restoration foundation model that uses data equilibrium scheduling and MoE-driven diffusion priors to handle over 50 restoration tasks with balanced performance.


<details>
  <summary>Details</summary>
Motivation: The authors identify that data mixture proportions from different restoration tasks are critical for all-in-one image restoration models, and current approaches may have imbalanced training data distributions that limit overall performance across diverse tasks.

Method: Proposes FoundIR-v2 with two key innovations: 1) Data equilibrium scheduling paradigm that dynamically optimizes mixed training dataset proportions using data mixing laws, and 2) Mixture-of-Experts-driven scheduler for generative pre-training that allocates task-adaptive diffusion priors based on each task's distinct degradation forms and levels.

Result: The method addresses over 50 sub-tasks across broader real-world scenarios and achieves favorable performance against state-of-the-art approaches, demonstrating consistent generalization and comprehensive performance across diverse restoration tasks.

Conclusion: Balancing data mixture proportions and using adaptive diffusion priors through MoE scheduling are crucial for building effective all-in-one image restoration foundation models that can handle diverse real-world restoration scenarios.
...
TL;DR: MelanomaNet: An explainable deep learning system for skin lesion classification that combines high accuracy with four interpretability mechanisms to address the black box problem in clinical adoption.


<details>
  <summary>Details</summary>
Motivation: Despite high accuracy in automated skin lesion classification using deep learning, clinical adoption remains limited due to the "black box" nature of these models, which lack transparency and interpretability for clinicians.
...
Conclusion: High classification performance can be achieved alongside comprehensive interpretability, potentially facilitating greater trust and adoption in clinical dermatology workflows by addressing the black box problem through multiple complementary explanation mechanisms.
...
TL;DR: Improved YOLOv8n-SPTS model enhances small traffic target detection in autonomous driving through SPD-Conv, SPPFCSPC, and TSFP modules, achieving state-of-the-art performance on VisDrone2019-DET dataset.
...
Method: Three key innovations: 1) Replace traditional convolution with SPD-Conv modules to retain fine-grained information; 2) Introduce SPPFCSPC module to enhance multi-scale feature fusion; 3) Design Triple-Stage Feature Pyramid (TSFP) structure with dedicated 160*160 small target detection head while removing redundant large target heads.

Result: On VisDrone2019-DET dataset: achieves 61.9% precision, 48.3% recall, 52.6% mAP@0.5, and 32.6% mAP@0.5:0.95 - ranking first in all metrics. Visualization shows significantly reduced miss rate for pedestrians and bicycles in occluded and dense scenes.

Conclusion: The proposed YOLOv8n-SPTS model effectively addresses small target detection challenges in autonomous driving through architectural improvements that enhance feature extraction, fusion, and dedicated small target detection capabilities.
...
TL;DR: VABench is a comprehensive benchmark framework for evaluating synchronous audio-video generation models across multiple tasks and dimensions, addressing the lack of proper evaluation in existing video generation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing video generation benchmarks focus on visual quality but lack convincing evaluations for audio-video generation, especially for models that aim to generate synchronized audio-video outputs. There's a gap in systematic assessment of synchronous audio-video generation capabilities.

Method: VABench introduces a multi-dimensional benchmark framework covering three primary task types: text-to-audio-video (T2AV), image-to-audio-video (I2AV), and stereo audio-video generation. It establishes two major evaluation modules with 15 dimensions assessing pairwise similarities (text-video, text-audio, video-audio), audio-video synchronization, lip-speech consistency, and curated audio/video QA pairs.

Result: The benchmark covers seven major content categories (animals, human sounds, music, environmental sounds, synchronous physical sounds, complex scenes, virtual worlds) and provides systematic analysis and visualization of evaluation results.
...
TL;DR: Polyp-DiFoM: A distillation framework that transfers knowledge from large vision foundation models to lightweight polyp segmentation models, achieving better performance with much lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Lightweight polyp segmentation models (U-Net, U-Net++, PraNet) have deployment advantages but struggle with polyp variations and camouflage. Large vision foundation models (SAM, DINOv2, etc.) have strong generalization but don't transfer well to medical imaging due to dataset scarcity and domain gap.

Method: Proposes Polyp-DiFoM distillation framework that infuses semantic priors from foundation models into lightweight architectures (U-Net, U-Net++) and uses frequency domain encoding for enhanced knowledge transfer.

Result: Outperforms baseline models and state-of-the-art on five benchmark datasets (Kvasir-SEG, CVC-ClinicDB, ETIS, ColonDB, CVC-300) with nearly 9 times reduced computation overhead.

Conclusion: Polyp-DiFoM successfully bridges the gap between large foundation models and lightweight medical segmentation, enabling efficient and accurate polyp segmentation for clinical deployment.
...
TL;DR: This paper introduces USE50k, a large-scale suspiciousness analysis dataset with 65,500 images from diverse public environments, and DeepUSEvision, a lightweight vision framework for real-time suspiciousness detection using object detection, facial/body language analysis, and multimodal fusion.


<details>
  <summary>Details</summary>
Motivation: Suspiciousness estimation is critical for proactive threat detection and public safety in complex environments, but existing approaches lack comprehensive datasets and efficient real-time frameworks.

Method: DeepUSEvision framework with three components: 1) Suspicious Object Detector based on enhanced YOLOv12, 2) dual DCNNs for facial expression and body-language recognition using image and landmark features, and 3) transformer-based Discriminator Network for adaptive multimodal fusion.

Result: Extensive experiments confirm superior accuracy, robustness, and interpretability compared to state-of-the-art approaches. The USE50k dataset contains 65,500 images from diverse public environments covering weapons, fire, crowd density, abnormal expressions, and unusual postures.

Conclusion: The USE50k dataset and DeepUSEvision framework establish a strong, scalable foundation for intelligent surveillance and real-time risk assessment in safety-critical applications.
...
TL;DR: NeRF trained in log RGB color space instead of sRGB improves rendering quality, robustness, and low-light performance by better separating illumination and reflectance components.


<details>
  <summary>Details</summary>
Motivation: NeRF typically uses sRGB images for supervision, but little attention has been paid to the color space for learning radiance field representations. Inspired by the BiIlluminant Dichromatic Reflection (BIDR) model which suggests logarithmic transformation simplifies separation of illumination and reflectance, the authors hypothesize that log RGB space enables NeRF to learn more compact and effective scene appearance representations.
...
Result: Quantitative and qualitative evaluations show log RGB color space consistently improves rendering quality, exhibits greater robustness across scenes, and performs particularly well in low light conditions while using same bit-depth input images. Further analysis across different network sizes and NeRF variants confirms generalization and stability of log space advantage.

Conclusion: Log RGB color space enables NeRF to learn more effective radiance field representations, improving rendering quality, robustness, and low-light performance by better separating illumination and reflectance components as suggested by the BIDR model.
...
TL;DR: LNMBench is a comprehensive benchmark for evaluating label noise robustness in medical imaging, testing 10 methods across 7 datasets, 6 modalities, and 3 noise patterns, revealing performance degradation under high/real noise and proposing improvements.


<details>
  <summary>Details</summary>
Motivation: Medical image annotation suffers from expert variability and noise, but existing label noise learning methods haven't been systematically assessed for robustness in medical imaging contexts.

Method: Created LNMBench benchmark with 10 representative methods evaluated across 7 datasets, 6 imaging modalities, and 3 noise patterns, establishing unified reproducible framework for robustness evaluation.

Result: Existing LNL methods degrade substantially under high and real-world noise, highlighting challenges of class imbalance and domain variability in medical data.

Conclusion: Proposed simple yet effective improvement for robustness, released codebase to facilitate standardized evaluation and promote noise-resilient algorithm development for medical applications.
...
TL;DR: UniLS: First end-to-end framework for generating unified speak-listen facial expressions using only dual-track audio, solving the listener modeling challenge through a two-stage training approach.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with modeling listener behavior in conversational avatars because listener motion follows internal motion priors rather than being directly audio-driven, leading to stiff, unnatural listening expressions. Most prior work focuses only on speaking generation, and the only joint generation approach requires extra speaker motion input, making it non-end-to-end and impractical for real-time applications.

Method: Two-stage training paradigm: Stage 1 learns internal motion prior by training an audio-free autoregressive generator to capture natural facial dynamics. Stage 2 introduces dual-track audio and fine-tunes the generator to modulate the learned motion prior based on external speech cues, creating a unified end-to-end framework.
...
Conclusion: UniLS provides the first practical, high-fidelity audio-driven solution for interactive digital humans by enabling end-to-end generation of unified speak-listen expressions using only dual-track audio, overcoming the fundamental challenge of modeling listener behavior in conversational avatars.
...
<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Model (VLM) methods for class-incremental learning still struggle with differentiating classes across learning tasks, leading to confusion between old and new classes.

Method: 1) Task-specific adapters added to frozen pre-trained image encoder; 2) Cross-task representation calibration using mixture of light-weight projectors; 3) Uncertainty-guided inference strategy for feature selection.
...
Conclusion: The proposed framework effectively addresses class confusion in continual learning by combining task-specific adaptation, cross-task calibration, and uncertainty-aware inference.
...
<details>
  <summary>Details</summary>
Motivation: Existing NeRF and 3DGS methods for human avatar modeling often produce unsatisfactory results with insufficient geometrical details related to body motion (like clothing wrinkles). There's a need for better photo-realistic human avatars that can handle pose-variant deformation and relighting.

Method: Proposes a 3DGS-based framework with dynamic skinning weights that define articulation based on pose while learning additional motion-induced deformations. Introduces novel regularization to capture fine geometric details under sparse visual cues, and creates a new multi-view dataset with varied lighting conditions for evaluation.

Result: Achieves state-of-the-art performance in novel view synthesis, novel pose rendering, and relighting. Enables realistic rendering of novel poses and views with photo-realistic lighting effects under arbitrary lighting conditions.

Conclusion: RnD-Avatar successfully addresses the limitations of previous methods by providing accurate pose-variant deformation for high-fidelity geometrical details while supporting relightable rendering, making significant advances in human avatar modeling from monocular video.
...
TL;DR: First computational framework for detecting Cytoplasmic Strings (CS) in human IVF embryos using deep learning with novel loss function for imbalanced data.


<details>
  <summary>Details</summary>
Motivation: Embryo selection is critical in IVF but current automated methods overlook important biomarkers like Cytoplasmic Strings. Manual CS assessment is labor-intensive, subjective, and affected by subtle visual appearance, creating need for computational analysis.

Method: Two-stage deep learning framework: (1) frame-level CS classification using Novel Uncertainty-aware Contractive Embedding (NUCE) loss to handle severe imbalance, (2) RF-DETR-based localization for CS regions in positive cases. Built on human-in-the-loop annotated dataset of 13,568 frames.

Result: NUCE loss consistently improves F1-score across five transformer backbones. RF-DETR achieves state-of-the-art detection performance for thin, low-contrast CS structures. Framework enables automated CS analysis for embryo viability assessment.

Conclusion: First computational framework for CS analysis in human IVF embryos successfully addresses challenges of severe imbalance and subtle visual features, providing automated tool for embryo selection that could improve IVF outcomes.
...
TL;DR: TextGuider: A training-free method that improves text rendering in diffusion models by aligning text tokens with image regions during early denoising steps.


<details>
  <summary>Details</summary>
Motivation: Despite advances in text-to-image diffusion models, they still struggle with accurate text rendering, particularly with text omission (partial or complete missing text). Existing methods focus on fine-tuning or refinement but overlook the critical issue of text omission.

Method: TextGuider analyzes attention patterns in MM-DiT models for text-related tokens, then applies latent guidance during early denoising steps using two novel loss functions that align textual content tokens with text regions in the image.
...
Conclusion: TextGuider effectively addresses text omission in diffusion models through training-free latent guidance, improving both completeness and accuracy of rendered text.
...
TL;DR: First comprehensive validation of a privacy-preserving AI vision framework on real industrial data, showing effective privacy-utility balance across three production use cases.


<details>
  <summary>Details</summary>
Motivation: AI computer vision adoption in industry faces tension between operational utility and worker privacy protection, requiring practical solutions that balance both concerns.

Method: Uses learned visual transformations that obscure sensitive/task-irrelevant information while retaining essential features; validated on real-world data from industrial partners across woodworking monitoring, AGV navigation, and ergonomic risk assessment.

Result: Task-specific obfuscation enables effective monitoring with reduced privacy risks; framework demonstrates readiness for real-world adoption with positive feedback from industrial partners.

Conclusion: The privacy-preserving framework is ready for real-world industrial adoption and provides cross-domain recommendations for responsible, human-centric AI deployment.
...
### [59] [Video-QTR: Query-Driven Temporal Reasoning Framework for Lightweight Video Understanding](https://arxiv.org/abs/2512.09354)
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal large-language models (MLLMs) are computationally intensive for long-video understanding due to dense frame encoding, which generates excessive visual tokens leading to high memory consumption, redundant computation, and limited scalability. The traditional "process-then-reason" paradigm is inefficient as it analyzes visual streams exhaustively before semantic reasoning.

Method: Video-QTR redefines video comprehension as a query-guided reasoning process. Instead of encoding every frame, it dynamically allocates perceptual resources based on the semantic intent of the query, creating an adaptive feedback loop between reasoning and perception. This query-driven temporal reasoning approach selectively processes relevant video content.
...
Conclusion: Query-driven temporal reasoning provides an efficient and scalable solution for video understanding, addressing the computational inefficiencies of traditional MLLM approaches while maintaining high performance.
...
<details>
  <summary>Details</summary>
Motivation: Cloud cover in multispectral imagery corrupts spectral information, hindering early-season crop mapping. Existing ViT-based methods use coarse temporal embeddings that aggregate entire sequences, causing information loss and reduced reconstruction accuracy.

Method: Proposes a Video Vision Transformer (ViViT)-based framework with temporal-spatial fusion embedding. Extracts non-overlapping tubelets via 3D convolution with constrained temporal span (t=2) to ensure local temporal coherence while reducing cross-day information degradation. Tests both MSI-only and SAR-MSI fusion scenarios.

Result: On 2020 Traill County data: MTS-ViViT achieves 2.23% MSE reduction compared to MTS-ViT baseline; SMTS-ViViT achieves 10.33% improvement with SAR integration over SMTS-ViT baseline. Framework effectively enhances spectral reconstruction quality.

Conclusion: The proposed ViViT-based framework with temporal-spatial fusion embedding significantly improves cloud-covered MSI reconstruction for robust agricultural monitoring, outperforming existing ViT-based approaches.
...
Method: The framework repurposes a pretrained video generator for monocular-to-stereo conversion, using joint conditioning on monocular video input with explicit geometry-aware regularization for 3D structural fidelity, plus spatio-temporal tiling for efficient high-resolution synthesis.

Result: The method substantially outperforms prior approaches, generating stereo videos with superior visual fidelity and geometric consistency, supported by a curated high-definition stereo video dataset of over 11M frames aligned to natural human IPD.

Conclusion: StereoWorld provides an effective solution for high-quality stereo video generation that addresses the cost and artifact challenges of current production methods, enabling more accessible stereo content creation for XR applications.
...
<details>
  <summary>Details</summary>
Motivation: Despite remarkable visual fidelity in diffusion-based generative models, there's limited understanding of how specific perceptual attributes like color and shape are internally represented. The paper aims to explore how color is encoded in generative models through systematic analysis of latent representations.

Method: Systematic analysis of latent representations in Stable Diffusion using controlled synthetic datasets, principal component analysis (PCA), and similarity metrics to examine how color information is encoded.
...
Conclusion: The findings reveal an interpretable structure in Stable Diffusion's latent space that follows efficient coding principles. This provides a foundation for future work in model understanding, editing applications, and designing more disentangled generative frameworks.
...
TL;DR: ASSIST-3D is a novel 3D scene synthesis pipeline that generates diverse synthetic training data to enhance generalization in class-agnostic 3D instance segmentation, outperforming existing methods on major benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current class-agnostic 3D instance segmentation methods struggle with generalization due to limited annotated 3D data or noisy 2D segmentations. Existing 3D scene synthesis approaches fail to simultaneously achieve geometry diversity, context complexity, and layout reasonability needed for effective training data.

Method: ASSIST-3D features three key innovations: 1) Heterogeneous Object Selection with random sampling from CAD collections for geometric diversity, 2) Scene Layout Generation using LLM-guided spatial reasoning with depth-first search for reasonable placements, and 3) Realistic Point Cloud Construction via multi-view RGB-D rendering and fusion to mimic real sensor data.
...
TL;DR: FUSER is a feed-forward transformer for multiview point cloud registration that directly predicts global poses without pairwise matching, using superpoint encoding and geometric attention with 2D foundation model priors, plus a diffusion refinement framework (FUSER-DF) for pose correction.


<details>
  <summary>Details</summary>
Motivation: Traditional multiview registration relies on expensive pairwise matching to build pose graphs, which is computationally intensive and ill-posed without holistic geometric constraints. There's a need for more efficient, direct global registration methods.

Method: FUSER encodes scans into low-resolution superpoint features via sparse 3D CNN, performs intra- and inter-scan reasoning through Geometric Alternating Attention module, and transfers 2D attention priors from foundation models. FUSER-DF adds an SE(3)^N diffusion refinement framework that uses FUSER as a surrogate model to construct a denoiser for pose correction.

Result: Extensive experiments on 3DMatch, ScanNet, and ArkitScenes demonstrate superior registration accuracy and outstanding computational efficiency compared to existing methods.

Conclusion: FUSER presents the first feed-forward transformer for multiview point cloud registration that bypasses pairwise matching, achieving both high accuracy and efficiency through unified latent space processing and diffusion-based refinement.
...
<details>
  <summary>Details</summary>
Motivation: CNNs struggle with global image context understanding due to their focus on local patterns, while Vision Transformers use self-attention mechanisms to capture relationships across entire images, potentially offering better performance for tasks requiring global understanding.

Method: Comprehensive comparison of different ViT types (pure, hierarchical, hybrid) against traditional CNNs across multiple tasks: object recognition, detection, and medical image classification. Experiments conducted on standard datasets (ImageNet, COCO) and medical dataset (ChestX-ray14) with data augmentation techniques.

Result: Hybrid and hierarchical transformers (especially Swin and CvT) provide optimal balance between accuracy and computational efficiency. Data augmentation significantly improves medical image performance, particularly with Swin Transformer. ViTs generally outperform CNNs, especially in global context understanding tasks like medical imaging.

Conclusion: Vision Transformers are competitive with and often superior to traditional CNNs, particularly for tasks requiring global visual context understanding. Hybrid/hierarchical architectures offer practical advantages, and data augmentation enhances their performance in specialized domains like medical imaging.
...
TL;DR: The paper proposes a novel white balance correction framework using a perception-inspired Learnable HSI color space with Mamba-based network, outperforming existing methods on benchmark datasets.
...
Conclusion: The work highlights the potential of perception-inspired color space design in computational photography and provides open-source implementation for the research community.
...
TL;DR: The paper challenges the necessity of lengthy Chain-of-Thought reasoning for video understanding, proposing that concise reasoning with compressed visual tokens can be equally effective while being more efficient.
...
Method: Design and validate an efficient post-training and inference framework that enables video MLLMs to operate on compressed visual tokens and generate brief reasoning traces before answering, without requiring manual CoT annotations or supervised fine-tuning.

Result: The resulting models achieve substantially improved inference efficiency while delivering competitive performance across diverse benchmarks, showing that concise reasoning can be both effective and efficient.

Conclusion: Long, human-like Chain-of-Thought reasoning may not be necessary for general video reasoning; concise reasoning with compressed visual tokens can provide effective and efficient video understanding.
...
### [68] [Detection and Localization of Subdural Hematoma Using Deep Learning on Computed Tomography](https://arxiv.org/abs/2512.09393)
...
<details>
  <summary>Details</summary>
Motivation: Subdural hematoma is a common neurosurgical emergency requiring rapid identification, but existing automated tools lack interpretability and spatial localization. There's a need for transparent systems that integrate multimodal clinical and imaging information for real-time decision support.

Method: Developed multimodal deep learning framework integrating: 1) structured clinical variables (demographics, comorbidities, medications, labs), 2) 3D CNN trained on CT volumes, and 3) transformer-enhanced 2D segmentation model for SDH localization. Used 25,315 head CT studies (3,774 with SDH) from 2015-2024. Employed greedy ensemble strategy to combine complementary predictors.
...
Conclusion: The multimodal interpretable framework provides rapid, accurate SDH detection and localization with transparent outputs. Integration into radiology workflows could streamline triage, reduce time to intervention, and improve consistency in SDH management.
...
TL;DR: WAH framework uses hyperbolic representations with Wasserstein alignment for multi-view clustering, achieving state-of-the-art performance by addressing view-specific noise and ensuring global semantic consistency.


<details>
  <summary>Details</summary>
Motivation: Existing hyperbolic MVC methods focus on instance-level alignment but neglect global semantic consistency, making them vulnerable to view-specific noise and cross-view discrepancies.

Method: Uses view-specific hyperbolic encoders to embed features into Lorentz manifold, introduces hyperbolic sliced-Wasserstein distance for global semantic alignment, and employs soft cluster assignments for cross-view consistency.

Result: Extensive experiments on multiple benchmarking datasets show the method achieves state-of-the-art clustering performance.

Conclusion: The WAH framework effectively addresses limitations of existing hyperbolic MVC methods by incorporating global semantic alignment through Wasserstein distance, leading to superior clustering results.
...
TL;DR: A new 3D registration method called Generative Point Cloud Registration that uses 2D generative models to create consistent image pairs from point clouds, enabling better matching through geometry-color feature fusion.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between advanced 2D generative models and 3D matching tasks, aiming to enhance registration performance by leveraging both geometric and color/texture information through generated image pairs.

Method: Introduces Match-ControlNet, a controllable 2D generative model that ensures 2D-3D geometric consistency by using depth maps from point clouds, and promotes cross-view texture consistency through coupled conditional denoising and prompt guidance.

Result: Extensive experiments on 3DMatch and ScanNet datasets verify the effectiveness of the approach, showing improved registration performance that can be integrated into various existing registration methods.

Conclusion: The proposed Generative Point Cloud Registration paradigm successfully bridges 2D generative models with 3D matching, providing a general framework that enhances registration performance through geometry-color feature fusion.
...
Method: 1) Create HeadSwapBench dataset using video editing model to synthesize fake swapping pairs; 2) DirectSwap framework extends image U-Net to video diffusion with motion module; 3) MEAR loss reweights diffusion loss using frame-difference and facial-landmark proximity for better motion/expression coherence.
...
Conclusion: The proposed paired dataset and mask-free framework with motion-aware training significantly improves video head swapping quality, with plans to release code and dataset for future research.
...
TL;DR: CHEM method quantifies hallucination artifacts in image reconstruction models using wavelet/shearlet representations and conformal quantile regression, with theoretical analysis of why U-shaped networks hallucinate.


<details>
  <summary>Details</summary>
Motivation: U-shaped architectures like U-Net generate unrealistic artifacts/hallucinations in safety-critical scenarios, requiring trustworthy quantification methods for computer vision models.

Method: Conformal Hallucination Estimation Metric (CHEM) uses wavelet and shearlet representations to extract hallucination features and conformalized quantile regression for distribution-free assessment.

Result: Tested on CANDELS astronomical dataset with U-Net, SwinUNet, and Learnlets, providing new perspectives on hallucination in deep learning-based image processing.

Conclusion: CHEM enables efficient identification/quantification of hallucination artifacts in any image reconstruction model, with theoretical insights into why U-shaped networks are prone to hallucinations.
...
TL;DR: MCDM is a label-free latent diffusion model that synthesizes realistic echocardiography videos using self-supervised motion features, addressing data scarcity in cardiac ultrasound analysis.


<details>
  <summary>Details</summary>
Motivation: Echocardiography suffers from limited labeled data due to privacy restrictions and complex expert annotation, hindering deep learning applications in cardiac function assessment.
...
TL;DR: Bind & Compose is a one-shot visual concept composition method that binds visual concepts to prompt tokens and composes them from various sources using hierarchical binders and specialized mechanisms for improved accuracy and compatibility.
...
Method: Uses hierarchical binder structure in Diffusion Transformers to encode visual concepts into prompt tokens. Introduces Diversify-and-Absorb Mechanism to improve binding accuracy by using absorbent tokens to filter irrelevant details. Implements Temporal Disentanglement Strategy with dual-branch binder for video concept training.
...
Conclusion: Bind & Compose effectively addresses limitations in visual concept composition through innovative binding mechanisms and temporal strategies, demonstrating strong performance in combining concepts from diverse visual sources.
...
TL;DR: A novel method for distilling echocardiographic video datasets using motion feature extraction and graph-based selection to create compact synthetic subsets that preserve clinical features.


<details>
  <summary>Details</summary>
Motivation: Echocardiography generates large-scale video data that creates storage, computation, and training efficiency challenges. Dataset distillation offers a solution by creating compact synthetic subsets that retain key clinical features.

Method: Leverages motion feature extraction to capture temporal dynamics, followed by class-wise graph construction and representative sample selection using the Infomap algorithm to select diverse and informative synthetic videos.
...
Conclusion: The proposed approach effectively distills echocardiographic video datasets into compact synthetic subsets that preserve essential characteristics, offering a scalable solution for medical video data management and analysis.
...
TL;DR: MedForget is a hierarchical multimodal unlearning testbed for medical AI that enables fine-grained assessment of data removal across eight organizational levels, revealing that current unlearning methods struggle with complete forgetting while maintaining diagnostic performance.


<details>
  <summary>Details</summary>
Motivation: Medical AI systems using pretrained MLLMs face privacy and compliance challenges under regulations like HIPAA and GDPR, which require the "right to be forgotten." Current unlearning methods for selectively removing training data influence are underexplored in complex medical settings with hierarchical data structures.

Method: The authors introduce MedForget, a Hierarchy-Aware Multimodal Unlearning Testbed that models hospital data as a nested hierarchy (Institution → Patient → Study → Section) with explicit retain/forget splits. The benchmark contains 3840 multimodal instances across eight organizational levels. They test four state-of-the-art unlearning methods on three tasks (generation, classification, cloze) and introduce a reconstruction attack that progressively adds hierarchical context to test unlearning completeness.

Result: Existing unlearning methods struggle to achieve complete, hierarchy-aware forgetting without reducing diagnostic performance. Coarse-grained unlearning shows strong resistance to reconstruction attacks, while fine-grained unlearning leaves models vulnerable to hierarchical reconstruction. The testbed reveals the tension between complete forgetting and maintaining model utility.

Conclusion: MedForget provides a practical, HIPAA-aligned testbed for building compliant medical AI systems, highlighting the need for improved unlearning methods that can handle complex hierarchical medical data while balancing privacy requirements with diagnostic performance.
...
TL;DR: FunPhase is a functional periodic autoencoder that learns phase manifolds for motion, enabling smooth trajectories at arbitrary temporal resolutions and unifying motion prediction/generation.
...
Method: Introduces FunPhase, a functional periodic autoencoder that learns phase manifolds for motion and replaces discrete temporal decoding with a function-space formulation, enabling smooth trajectories that can be sampled at arbitrary temporal resolutions.
...
Conclusion: FunPhase provides a scalable, interpretable phase manifold approach that generalizes across skeletons and datasets, unifying motion prediction and generation while enabling a broader range of applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing part-level 3D generation methods have limitations: they either use implicit part segmentation with poor granularity control or rely on external segmenters trained on large annotated datasets. The authors observe that part awareness naturally emerges during whole-object geometry learning, suggesting a more integrated approach is possible.

Method: Proposes Geom-Seg VecSet, a unified geometry-segmentation latent representation that jointly encodes object geometry and part-level structure. Then introduces UniPart, a two-stage latent diffusion framework: 1) first stage performs joint geometry generation and latent part segmentation, 2) second stage conditions part-level diffusion on both whole-object and part-specific latents. Uses dual-space generation scheme predicting part latents in both global and canonical spaces for enhanced geometric fidelity.

Result: Extensive experiments show UniPart achieves superior segmentation controllability and part-level geometric quality compared with existing approaches.

Conclusion: The proposed unified representation and two-stage diffusion framework effectively enables high-quality, controllable part-level 3D generation without relying on external segmentation models, demonstrating the natural emergence of part awareness during geometry learning.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs like CLIP neglect fine-grained anomaly details (e.g., "hole", "cut", "scratch") which are crucial for understanding root causes and implementing targeted corrective measures. Handcrafted prompts for each defect type are time-consuming and biased.

Method: DAPO (Defect-aware Prompt Optimization) uses progressive tuning to learn hybrid defect-aware prompts with fixed textual anchors and learnable token embeddings, aligning anomaly-relevant image features with corresponding text semantics for zero-shot multi-type and binary anomaly detection.

Result: DAPO achieves 3.7% average improvement in AUROC and average precision at image level under distribution shift, and 6.5% average improvement in localizing novel anomaly types under zero-shot settings across public benchmarks (MPDD, VisA, MVTec-AD, MAD, Real-IAD) and internal dataset.

Conclusion: Learning fine-grained defect semantics through optimized prompts significantly improves anomaly detection performance, enabling better understanding of anomaly types and more targeted corrective actions in manufacturing settings.
...
### [80] [Controlling Steering Angle for Cooperative Self-driving Vehicles utilizing CNN and LSTM-based Deep Networks](https://arxiv.org/abs/1904.04375)
*Rodolfo Valiente,Mahdi Zaman,Sedat Ozer,Yaser P. Fallah*

Main category: cs.CV

TL;DR: A deep learning architecture using CNN, LSTM, and FC layers that incorporates temporal dependencies and V2V-shared future images to predict steering angles with state-of-the-art accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing autonomous vehicle steering angle prediction methods ignore temporal dependencies between image frames and don't leverage multi-vehicle image sharing via V2V communication, which could improve accuracy.

Method: Proposes an end-to-end deep architecture combining CNN, LSTM, and fully connected layers that uses both present and future images (shared via V2V communication from a vehicle ahead) to predict steering angles while considering temporal dependencies.

Result: The proposed model achieves the lowest error compared to other existing approaches in the literature for steering angle prediction.

Conclusion: Incorporating temporal dependencies and leveraging V2V-shared future images through an LSTM-based deep architecture significantly improves steering angle prediction accuracy in autonomous vehicles.
...
### [81] [MODA: The First Challenging Benchmark for Multispectral Object Detection in Aerial Images](https://arxiv.org/abs/2512.09489)
...
<details>
  <summary>Details</summary>
Motivation: RGB-based aerial object detectors struggle with small objects and background interference due to limited discriminative information. Multispectral images offer additional spectral cues but lack sufficient training data, creating a bottleneck for exploiting their potential.

Method: OSSDet framework integrates spectral and spatial information with object-aware cues: 1) Cascaded spectral-spatial modulation structure for target perception optimization, 2) Spectral similarity aggregation to reinforce intra-object correlations, 3) Object-aware masking to suppress irrelevant background, and 4) Cross-spectral attention with explicit object-aware guidance.

Result: Extensive experiments show OSSDet outperforms existing methods with comparable parameters and efficiency. The MODA dataset provides 14,041 multispectral images and 330,191 annotations across diverse challenging scenarios.

Conclusion: The paper addresses the data bottleneck in multispectral aerial object detection by introducing the first large-scale dataset (MODA) and proposing OSSDet, which effectively integrates spectral-spatial information with object-aware mechanisms to overcome challenges of small objects and background interference.
...
### [82] [StateSpace-SSL: Linear-Time Self-supervised Learning for Plant Disease Detectio](https://arxiv.org/abs/2512.09492)
*Abdullah Al Mamun,Miaohua Zhang,David Ahmedt-Aristizabal,Zeeshan Hayder,Mohammad Awrangjeb*

Main category: cs.CV

TL;DR: StateSpace-SSL: A linear-time self-supervised learning framework using Vision Mamba state-space encoder for plant disease detection, outperforming CNN- and transformer-based SSL methods by better capturing continuous disease patterns along leaf structures.


<details>
  <summary>Details</summary>
Motivation: Existing SSL methods for plant disease detection have limitations: CNN-based SSL struggles to capture continuously evolving disease patterns along leaf structures, while transformer-based SSL suffers from quadratic attention costs from high-resolution patches. There's a need for SSL methods better matched to agricultural imagery characteristics.

Method: Proposes StateSpace-SSL framework with Vision Mamba state-space encoder that models long-range lesion continuity through directional scanning across leaf surfaces. Uses prototype-driven teacher-student objective to align representations across multiple views, encouraging stable and lesion-aware features from labeled data.

Result: Outperforms CNN- and transformer-based SSL baselines on three publicly available plant disease datasets across various evaluation metrics. Qualitative analyses confirm it learns compact, lesion-focused feature maps.

Conclusion: StateSpace-SSL demonstrates the advantage of linear state-space modeling for self-supervised plant disease representation learning, effectively capturing continuous disease patterns while maintaining computational efficiency.
...
### [83] [Gradient-Guided Learning Network for Infrared Small Target Detection](https://arxiv.org/abs/2512.09497)
...
### [84] [Masked Registration and Autoencoding of CT Images for Predictive Tibia Reconstruction](https://arxiv.org/abs/2512.09525)
...
TL;DR: A neural approach combining registration and autoencoder models to predict patient-specific healthy tibia reconstruction from fractured CT scans.


<details>
  <summary>Details</summary>
Motivation: Surgical planning for complex tibial fractures is challenging due to difficulty imagining 3D bone alignment; need to predict reconstruction targets from fractured CT scans.

Method: Combines modified spatial transformer network (STN) for CT registration to standardized coordinates with various autoencoder architectures to model healthy tibial variations, both designed to handle masked inputs from fractured bones.

Result: Developed a system that can predict patient-specific healthy bone structures in standard coordinates from fractured CT scans using robust registration and generative modeling.

Conclusion: The approach enables predictive generation of healthy bone structures for surgical planning, with contributions in 3D-adapted STN, comparative AE analysis, and masked-input handling.
...
### [85] [A Dual-Domain Convolutional Network for Hyperspectral Single-Image Super-Resolution](https://arxiv.org/abs/2512.09546)
...
Conclusion: DDSRNet successfully integrates spatial- and frequency-domain learning through DWT decomposition and reconstruction, providing an efficient solution for hyperspectral image super-resolution with good performance and low computational requirements.
...
### [86] [Building Reasonable Inference for Vision-Language Models in Blind Image Quality Assessment](https://arxiv.org/abs/2512.09555)
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs for BIQA often produce contradictory textual descriptions and unstable quality predictions that don't align with human reasoning, indicating that predictions aren't properly grounded in visual features and have weak logical connections.

Method: A two-stage tuning method: first stage learns visual features, second stage infers quality solely from these features. This explicitly separates visual perception from quality inference to encourage human-like reasoning.

Result: Reduces prediction instability from 22.00% to 12.39%, achieves average gains of 0.3124/0.3507 in SRCC/PLCC across multiple datasets (LIVE, CSIQ, SPAQ, KONIQ) compared to baseline. Improves both stability and reliability of inference process.

Conclusion: The proposed two-stage tuning method successfully addresses instability and contradictory reasoning in VLMs for BIQA by enforcing proper grounding of quality predictions in visual features, leading to more human-like and reliable quality assessment.
...
### [87] [From Graphs to Gates: DNS-HyXNet, A Lightweight and Deployable Sequential Model for Real-Time DNS Tunnel Detection](https://arxiv.org/abs/2512.09565)
*Faraz Ali,Muhammad Afaq,Mahmood Niazi,Muzammil Behzad*

Main category: cs.CV

TL;DR: DNS-HyXNet: Lightweight xLSTM framework for real-time DNS tunnel detection with 99.99% accuracy and 0.041ms latency, replacing computationally expensive graph-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based DNS tunnel detection methods (like GraphTunnel) have high latency and computational overhead from recursive parsing and graph construction, making them unsuitable for real-time deployment. Need for efficient, real-time detection on commodity hardware.

Method: DNS-HyXNet uses a hybrid xLSTM framework that integrates tokenized domain embeddings with normalized numerical DNS features. Processes through two-layer xLSTM network to learn temporal dependencies directly from packet sequences, eliminating graph reconstruction. Single-stage multi-class classification.
...
### [88] [Investigate the Low-level Visual Perception in Vision-Language based Image Quality Assessment](https://arxiv.org/abs/2512.09573)
*Yuan Li,Zitang Sun,Yen-Ju Chen,Shin'ya Nishida*

Main category: cs.CV

TL;DR: MLLM-based IQA systems struggle with low-level distortion detection due to vision-language misalignment, but improving vision encoder alignment dramatically enhances distortion recognition from 14.92% to 84.43% accuracy.


<details>
  <summary>Details</summary>
Motivation: Despite MLLMs' strong visual perception, they fail to reliably detect basic low-level distortions (blur, noise, compression) and produce inconsistent evaluations, raising questions about whether they truly perceive relevant visual features for IQA.

Method: Introduce low-level distortion perception task requiring distortion type classification; conduct component-wise analysis; compute semantic distance between visual features and semantic tokens before/after component-wise fine-tuning; improve vision encoder alignment.

Result: MLLMs structurally capable of representing distortions but overfit training templates, causing quality scoring biases; improving vision encoder alignment dramatically enhances distortion recognition from 14.92% to 84.43% accuracy.

Conclusion: Incorporating dedicated constraints on vision encoder strengthens text-explainable visual representations, enabling MLLM-based pipelines to produce more coherent and interpretable reasoning in vision-centric tasks.
...
### [89] [Seeing Soil from Space: Towards Robust and Scalable Remote Soil Nutrient Analysis](https://arxiv.org/abs/2512.09576)
*David Seu,Nicolas Longepe,Gabriel Cioltea,Erik Maidik,Calin Andrei*

Main category: cs.CV

TL;DR: A scalable soil property estimation system using remote sensing and environmental data achieves high accuracy for soil organic carbon and nitrogen across European croplands.


<details>
  <summary>Details</summary>
Motivation: Environmental factors increasingly impact agricultural decisions, but accessible, scalable soil assessment tools are limited. Current methods need improvement for accurate, large-scale soil property estimation.

Method: Hybrid modeling combining indirect proxy/driver methods with direct spectral modeling, using physics-informed covariates from radiative transfer models and nonlinear embeddings from foundation models. Validated on harmonized European cropland data with strict spatial blocking and stratified splits.

Result: Highest accuracy for SOC (MAE: 5.12 g/kg, CCC: 0.77) and N (MAE: 0.44 g/kg, CCC: 0.77). Performance maintained across unseen locations in spatial cross-validation and independent testing. Uncertainty assessment achieved 90% coverage at target confidence level.

Conclusion: The system advances digital agriculture through scalable, data-driven soil analysis frameworks applicable to carbon markets and other domains requiring quantitative soil evaluation.
...
TL;DR: CA-ATP: Content-Adaptive image retouching method using Attribute-based Text Representation for adaptive color adjustments and user style preferences.


<details>
  <summary>Details</summary>
Motivation: Existing image retouching methods use uniform pixel-wise color mapping across entire images, ignoring color variations from image content and lacking ability to adapt to diverse color distributions and user-defined style preferences.

Method: Proposes CA-ATP with two modules: 1) Content-adaptive curve mapping module using basis curves and learned weight maps for content-aware color adjustments, 2) Attribute text prediction module generating text representations from image attributes to represent user style preferences, integrated with visual features via multimodal model.

Result: Extensive experiments on several public datasets demonstrate state-of-the-art performance.

Conclusion: CA-ATP effectively addresses limitations of existing approaches by enabling content-aware color adjustments and incorporating user-defined style preferences through attribute-based text representations.
...
Method: Uses frozen vision transformer encoder for multi-scale features, lightweight head for highlight localization, and token-level inpainting module. Introduces Virtual Highlight Synthesis pipeline for training without paired data by rendering physically plausible specularities using monocular geometry and Fresnel-aware shading.

Result: Achieves competitive performance with state-of-the-art results on several benchmarks and generalizes across natural and surgical domains despite non-Lambertian surfaces and non-uniform lighting.

Conclusion: UnReflectAnything effectively removes specular highlights from single RGB images, enabling better texture recovery and geometric reasoning across diverse domains through a novel training approach without requiring paired supervision.
...
TL;DR: OxEnsemble is a data-efficient ensemble method for fair classification in low-data, unbalanced medical imaging settings that aggregates predictions from fairness-constrained ensemble members while carefully reusing held-out data.


<details>
  <summary>Details</summary>
Motivation: Address fair classification in scarce, unbalanced data regimes common in medical imaging where false negatives can be fatal, requiring methods that work with limited data while ensuring fairness across demographic groups.

Method: Proposes OxEnsemble that trains ensembles where each member satisfies fairness constraints, aggregates predictions across members, carefully reuses held-out data for fairness enforcement, and is designed to be both data-efficient and compute-efficient.

Result: Theoretical guarantees for the approach; experimental validation shows more consistent outcomes and stronger fairness-accuracy trade-offs than existing methods across multiple challenging medical imaging classification datasets.

Conclusion: OxEnsemble provides an effective solution for fair classification in low-data medical imaging settings, balancing fairness and accuracy while being computationally efficient and theoretically sound.
...
### [93] [CS3D: An Efficient Facial Expression Recognition via Event Vision](https://arxiv.org/abs/2512.09592)
...
<details>
  <summary>Details</summary>
Motivation: Event cameras offer advantages for facial expression recognition (high temporal resolution, low latency, efficiency, low-light robustness), but existing deep learning methods are too energy-intensive for edge deployment, especially for high-frequency event vision approaches.

Method: Proposed CS3D framework that decomposes Convolutional 3D (C3D) to reduce computational complexity, uses soft spiking neurons to enhance information retention, and incorporates spatial-temporal attention mechanism to improve detection accuracy.
...
Conclusion: The CS3D framework successfully addresses the energy efficiency challenge for event-based facial expression recognition, making it suitable for deployment on edge computing devices in human-robot interaction applications.
...
### [94] [FROMAT: Multiview Material Appearance Transfer via Few-Shot Self-Attention Adaptation](https://arxiv.org/abs/2512.09617)
*Hubert Kompanowski,Varun Jampani,Aaryaman Vasishta,Binh-Son Hua*

Main category: cs.CV

TL;DR: Lightweight adaptation technique for appearance transfer in multiview diffusion models that learns to combine object identity from input image with appearance cues from reference image, enabling multi-view-consistent output with desired materials, textures, or styles.


<details>
  <summary>Details</summary>
Motivation: Existing multiview diffusion models offer limited appearance manipulation (material, texture, style) compared to explicit geometry representations like meshes or radiance fields, despite providing spatial consistency across viewpoints.

Method: Uses three diffusion denoising processes (original object, reference, target) and performs reverse sampling to aggregate a small subset of layer-wise self-attention features from object and reference to influence target generation. Requires only few training examples to introduce appearance awareness to pretrained multiview models.

Result: Enables explicit specification of appearance parameters at generation time while preserving underlying object geometry and view coherence. Provides simple yet effective way toward multiview generation with diverse appearance.

Conclusion: Method advocates adoption of implicit generative 3D representations in practice by enabling appearance-aware multiview generation with minimal training data.
...
### [95] [Beyond Sequences: A Benchmark for Atomic Hand-Object Interaction Using a Static RNN Encoder](https://arxiv.org/abs/2512.09626)
*Yousef Azizi Movahed,Fatemeh Ziaeetabar*

Main category: cs.CV

TL;DR: A novel approach achieves 97.60% accuracy in classifying atomic hand-object interaction states ('approaching', 'grabbing', 'holding') by converting raw videos into statistical-kinematic features and using a Bidirectional RNN with sequence length=1 as a high-capacity static encoder.


<details>
  <summary>Details</summary>
Motivation: Reliably predicting human intent in hand-object interactions remains an open challenge in computer vision. The research focuses on the fundamental sub-problem of fine-grained classification of atomic interaction states to advance understanding of human-object manipulation.

Method: Developed a structured data engineering process converting raw MANIAC dataset videos into 27,476 statistical-kinematic feature vectors capturing relational and dynamic properties. Compared static classifiers (MLPs) against temporal models (RNNs), with the key innovation being a Bidirectional RNN with sequence length=1, which functions as a high-capacity static feature encoder.

Result: Achieved 97.60% accuracy in classifying interaction states, with particular success in overcoming the challenging transitional 'grabbing' class (balanced F1-score of 0.90). The approach outperformed expectations by showing that sequential modeling wasn't critical for this task.

Conclusion: The research establishes a new benchmark for low-level hand-object interaction recognition using structured, interpretable features and lightweight architectures. The counter-intuitive finding that a temporal model with sequence length=1 functions best as a static encoder provides valuable insights for future work in human-object interaction analysis.
...
### [96] [Benchmarking SAM2-based Trackers on FMOX](https://arxiv.org/abs/2512.09633)
...
<details>
  <summary>Details</summary>
Motivation: To understand current limitations in state-of-the-art object trackers by benchmarking SAM2-based tracking pipelines on challenging fast-moving object datasets.

Method: Benchmarking several high-performing SAM2-based trackers (SAM2, EfficientTAM, DAM4SAM, SAMURAI) on datasets containing fast moving objects (FMO) specifically designed to be challenging for tracking approaches.

Result: DAM4SAM and SAMURAI perform well on more challenging sequences compared to other SAM2-based trackers.

Conclusion: The study provides detailed insights into tracker behavior and identifies which trackers perform best on challenging fast-moving object scenarios, helping understand current limitations in state-of-the-art tracking approaches.
...
### [97] [Kaapana: A Comprehensive Open-Source Platform for Integrating AI in Medical Imaging Research Environments](https://arxiv.org/abs/2512.09644)
...
TL;DR: Kaapana is an open-source platform for medical imaging research that addresses challenges in multi-center studies by providing a modular framework for data ingestion, cohort curation, workflow orchestration, and result inspection, enabling distributed experimentation while keeping sensitive data local.


<details>
  <summary>Details</summary>
Motivation: Medical imaging research faces challenges with multi-center studies due to regulatory constraints, fragmented software infrastructure, and difficulties in collaboration between clinicians and data scientists, leading to non-reproducible, single-institution projects.

Method: Kaapana provides a modular, extensible framework that unifies data ingestion, cohort curation, processing workflows, and result inspection under a common UI. It brings algorithms to the data, enabling institutions to maintain data control while participating in distributed research.

Result: The platform reduces technical overhead, improves reproducibility, and enables large-scale collaborative multi-center imaging studies, supporting diverse use cases from local prototyping to nation-wide research networks.

Conclusion: Kaapana bridges the gap in medical imaging research by providing a comprehensive open-source platform that addresses infrastructure fragmentation and regulatory constraints, facilitating reproducible, scalable, and collaborative research across institutions.
...
### [98] [VHOI: Controllable Video Generation of Human-Object Interactions from Sparse Trajectories via Motion Densification](https://arxiv.org/abs/2512.09646)
...
<details>
  <summary>Details</summary>
Motivation: Existing controllable video generation approaches face a trade-off: sparse controls (keypoints) lack instance-awareness while dense signals (optical flow, 3D meshes) are costly to obtain. Realistic human-object interaction synthesis needs both controllability and understanding of complex instance-specific dynamics.

Method: Two-stage framework: 1) Densify sparse trajectories into HOI mask sequences, 2) Fine-tune video diffusion model conditioned on these dense masks. Introduces HOI-aware motion representation using color encodings to distinguish human/object motion and body-part-specific dynamics.

Result: Achieves state-of-the-art results in controllable HOI video generation. Can generate not only interaction-only scenarios but also full human navigation leading up to object interactions end-to-end.

Conclusion: VHOI successfully addresses the controllability vs. cost trade-off in HOI video generation by transforming sparse trajectories into informative dense masks with HOI-aware motion representations, enabling realistic and controllable human-object interaction synthesis.
...
### [99] [IF-Bench: Benchmarking and Enhancing MLLMs for Infrared Images with Generative Visual Prompting](https://arxiv.org/abs/2512.09663)
*Tao Zhang,Yuyang Hong,Yang Xia,Kun Ding,Zeyu Zhang,Ying Wang,Shiming Xiang,Chunhong Pan*

Main category: cs.CV

TL;DR: IF-Bench is the first benchmark for evaluating multimodal LLMs on infrared image understanding, with 499 images and 680 QA pairs across 10 dimensions. The paper evaluates 40+ MLLMs and proposes GenViP, a training-free method that converts infrared to RGB images to improve performance.


<details>
  <summary>Details</summary>
Motivation: While MLLMs have advanced significantly, their capability in understanding infrared images remains unexplored. There's a need for a specialized benchmark to evaluate and improve MLLM performance on this important domain.

Method: Created IF-Bench with 499 infrared images from 23 datasets and 680 QA pairs across 10 understanding dimensions. Evaluated 40+ MLLMs using cyclic evaluation, bilingual assessment, and hybrid judgment. Proposed GenViP method that uses image editing models to translate infrared to RGB images without training.

Result: Systematic evaluation revealed how model scale, architecture, and inference paradigms affect infrared comprehension. GenViP consistently improved performance across various MLLMs by mitigating domain distribution shifts between infrared and RGB domains.

Conclusion: IF-Bench fills an important gap in evaluating MLLMs on infrared images. The proposed GenViP method effectively addresses domain shift issues and improves performance. The benchmark and code are publicly available to advance research in this area.
...
### [100] [An Automated Tip-and-Cue Framework for Optimized Satellite Tasking and Visual Intelligence](https://arxiv.org/abs/2512.09670)
*Gil Weissman,Amir Ivry,Israel Cohen*

Main category: cs.CV

TL;DR: Automated Tip-and-Cue framework for satellite tasking/scheduling using AI models for maritime vessel tracking with AIS data.


<details>
  <summary>Details</summary>
Motivation: Satellite constellations with reduced latency and diverse sensors create opportunities for automated Earth observation, requiring efficient tasking/scheduling systems.

Method: Fully automated Tip-and-Cue framework: tips from external data/prior imagery identify targets; cues generate imaging tasks with sensor constraints. Uses continuous utility functions for multi-satellite scheduling and AI models (object detectors, vision-language models) for image processing.

Result: Demonstrated efficacy through maritime vessel tracking scenario using AIS data for trajectory prediction, targeted observations, and actionable outputs.

Conclusion: System is extensible to broader applications like smart-city monitoring and disaster response where timely tasking and automated analysis are critical.
...
### [101] [Unconsciously Forget: Mitigating Memorization; Without Knowing What is being Memorized](https://arxiv.org/abs/2512.09687)
*Er Jin,Yang Zhang,Yongli Mou,Yanfei Dong,Stefan Decker,Kenji Kawaguchi,Johannes Stegmaier*

Main category: cs.CV

TL;DR: UniForget proposes model pruning to suppress copyrighted content generation in diffusion models without targeting specific concepts, offering a scalable alternative to existing memorization mitigation methods.


<details>
  <summary>Details</summary>
Motivation: Generative models increasingly memorize training data, leading to copyright infringement and legal challenges. Existing methods have computational overhead or limited scalability, requiring targeted unlearning of specific concepts.

Method: Identifies specific model parts responsible for copyrighted content generation and applies model pruning to suppress generation of copyrighted content while preserving general generative capabilities.

Result: Effectively reduces probability of generating copyrighted content without targeting specific concepts, and is orthogonal/complementary to existing unlearning methods.

Conclusion: Model pruning offers a scalable approach to mitigate memorization in generative models, addressing root causes rather than symptoms, with potential to enhance current de-memorization techniques.
...
TL;DR: SMA is a new meta-dataset for OOD generalization research with 24 datasets (12 content + 12 stylized) created via style transfer, enabling flexible control over groups/classes for diverse benchmark scenarios.


<details>
  <summary>Details</summary>
Motivation: To address practical constraints in capturing extensive group diversity in real-world data collection, and to enable more comprehensive evaluation of OOD generalization, fairness, and domain adaptation under diverse group and domain configurations.

Method: Created using style transfer techniques on 12 subject classification datasets, resulting in 4800 groups combining various subjects with multiple styles, allowing flexible control over styles, subject classes, and domains.

Result: Two benchmarks demonstrated: (1) OOD generalization/fairness benchmark showing increased group diversity significantly impacts fairness and alters algorithm rankings, with Top-M worst group accuracy proposed as new tuning metric; (2) UDA benchmark with 73% and 28% lower error bars in closed-set and UniDA settings respectively.

Conclusion: SMA enables more comprehensive evaluation of model performance across diverse group/domain configurations, significantly impacting conventional benchmark outcomes and opening new methodological directions for fairness, robustness, and adaptation research.
...
Method: Proposes FastPose-ViT, a Vision Transformer architecture that directly regresses 6DoF pose from cropped object bounding boxes. Introduces novel mathematical formalism based on projective geometry and "apparent rotation" concept to map localized predictions back to full-image scale, where the model predicts an apparent rotation matrix that is corrected to find true orientation.

Result: Outperforms other non-PnP strategies and achieves performance competitive with state-of-the-art PnP-based techniques on the SPEED dataset. When quantized and deployed on NVIDIA Jetson Orin Nano, achieves ~75 ms latency per frame sequentially and up to 33 FPS throughput with concurrent scheduling.
...
Result: Consistently outperforms strong semi-supervised and multi-modal baselines on BraTS 2019 (HGG subset) under 1%, 5%, and 10% labeled data settings, with significant improvements in both Dice and Sensitivity scores. Ablation studies confirm complementary effects of MEM and CIF.

Conclusion: The proposed framework effectively bridges cross-modality discrepancies and improves segmentation robustness under scarce supervision through explicit modality-specific enhancement and adaptive cross-modal fusion, demonstrating superior performance for semi-supervised multi-modal medical image segmentation.
...
TL;DR: DynaIP is a novel plugin for personalized text-to-image generation that improves concept preservation, prompt following, and multi-subject scalability without test-time fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current personalized text-to-image methods struggle with balancing concept preservation vs prompt following, retaining fine-grained details, and scaling to multiple subjects. There's a need for zero-shot PT2I solutions that don't require test-time fine-tuning.

Method: DynaIP introduces two key innovations: 1) Dynamic Decoupling Strategy that removes concept-agnostic information during inference to improve CP-PF balance and multi-subject scalability, and 2) Hierarchical Mixture-of-Experts Feature Fusion Module that leverages CLIP's hierarchical features for better fine-grained concept fidelity.

Result: Extensive experiments show DynaIP outperforms existing approaches on both single- and multi-subject PT2I tasks, demonstrating superior concept preservation, prompt following, and scalability.

Conclusion: DynaIP represents a significant advancement in personalized text-to-image generation, effectively addressing key challenges in concept preservation, prompt following, fine-grained detail retention, and multi-subject scalability without requiring test-time fine-tuning.
...
TL;DR: The paper presents online struggle detection and anticipation models for real-time assistive systems, achieving 70-80% mAP for detection and comparable performance for 2-second anticipation with real-time processing capabilities.


<details>
  <summary>Details</summary>
Motivation: Real-time assistive systems need to detect and anticipate user struggles during skill performance, but prior work only focused on offline classification and localization, lacking online capabilities.

Method: Reformulated struggle localization as online detection task and extended to anticipation. Adapted two off-the-shelf models as baselines for online struggle detection and anticipation, examining generalization across tasks and activities, and analyzing skill evolution impact.

Result: Online struggle detection achieved 70-80% per-frame mAP, while struggle anticipation up to 2 seconds ahead yielded comparable performance with slight drops. Models generalized across tasks and activities, outperforming random baselines by 4-20% despite domain gaps. Feature-based models run at 143 FPS, with full pipeline at ~20 FPS.

Conclusion: The proposed online struggle detection and anticipation models are effective for real-time assistive applications, with sufficient performance and processing speed to support timely interventions.
...
TL;DR: UniUGP is a unified framework that combines scene reasoning, future video generation, and trajectory planning to improve autonomous driving in long-tail scenarios through visual dynamics and semantic reasoning.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving systems fail in long-tail scenarios due to limited world knowledge and weak visual dynamic modeling. Existing VLA methods can't use unlabeled videos for visual causal learning, while world model methods lack reasoning capabilities from large language models.

Method: Propose UniUGP framework with hybrid expert architecture integrating pre-trained VLMs and video generation models. Uses multi-frame observations and language instructions to produce reasoning, trajectories, and future videos. Four-stage training strategy across multiple AD datasets with specialized reasoning/planning datasets.
...
Conclusion: UniUGP synergizes scene reasoning, future video generation, and trajectory planning to enhance autonomous driving performance in complex scenarios through integrated visual dynamics and semantic reasoning.
...
TL;DR: DPS4Un is a novel semiblind hyperspectral unmixing method using diffusion posterior sampling with superpixel-based endmember bundles and data fidelity constraints.


<details>
  <summary>Details</summary>
Motivation: Address key challenges in linear spectral mixture models: modeling spectral prior distribution and spectral variability, while avoiding bias from existing spectral libraries.

Method: Uses diffusion posterior sampler with four key features: (1) pretrained conditional spectrum diffusion model as posterior sampler, (2) image-based endmember bundles within superpixels for prior learning, (3) superpixel-based data fidelity term, (4) iterative Gaussian noise initialization and update of abundance and endmembers.

Result: Outperforms state-of-the-art hyperspectral unmixing methods on three real-world benchmark datasets.

Conclusion: DPS4Un effectively addresses spectral variability and prior modeling challenges in hyperspectral unmixing through diffusion-based posterior sampling with superpixel-based constraints.
...
TL;DR: Novel benchmarking framework for evaluating PDF formula parsing using synthetic PDFs with LaTeX ground truth and LLM-as-a-judge semantic evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing PDF parsing benchmarks either exclude formulas or lack semantically-aware evaluation metrics, creating a critical gap for training LLMs and building scientific knowledge bases from academic literature.

Method: 1) Synthetic PDF generation with precise LaTeX ground truth for systematic control; 2) LLM-as-a-judge for semantic formula assessment; 3) Two-stage matching pipeline to handle parser output inconsistencies; 4) Human validation with 250 formula pairs (750 ratings from 30 evaluators).
...
Conclusion: Provides crucial insights for practitioners selecting parsers for downstream applications and establishes a robust, scalable methodology for reproducible evaluation of PDF formula extraction quality.
...
TL;DR: The paper introduces Visual Action Reasoning task and VisualActBench benchmark to evaluate VLMs' ability to reason and act proactively from visual inputs without textual prompts, revealing significant gaps in human-aligned reasoning.


<details>
  <summary>Details</summary>
Motivation: Current VLMs excel at perceiving and describing visual environments but lack proactive reasoning and action capabilities based solely on visual inputs without explicit textual prompts. This gap limits their real-world readiness as autonomous agents.

Method: Proposed Visual Action Reasoning task and created VisualActBench benchmark with 1,074 videos and 3,733 human-annotated actions across four real-world scenarios. Actions are labeled with Action Prioritization Level (APL) and proactive-reactive types to assess human-aligned reasoning and value sensitivity. Evaluated 29 VLMs on this benchmark.

Result: While frontier models like GPT4o show relatively strong performance, significant gaps remain compared to human-level reasoning, especially in generating proactive, high-priority actions. Current VLMs struggle with interpreting complex context, anticipating outcomes, and aligning with human decision-making frameworks.

Conclusion: VisualActBench establishes a comprehensive foundation for assessing and improving proactive, vision-centric AI agents' real-world readiness. The benchmark reveals critical limitations in current VLMs' action reasoning capabilities and provides a roadmap for developing more human-aligned, proactive vision-language models.
...
TL;DR: NordFKB is a fine-grained benchmark dataset for geospatial AI in Norway, featuring high-resolution orthophotos with detailed annotations for 36 semantic classes, collected from seven diverse geographic areas with expert-reviewed quality control.


<details>
  <summary>Details</summary>
Motivation: To provide a robust foundation for advancing AI methods in mapping, land administration, and spatial planning in Norway by creating a high-quality, authoritative benchmark dataset that addresses the need for fine-grained geospatial AI evaluation with accurate annotations.

Method: Dataset derived from Norway's authoritative Felles KartdataBase (FKB), containing high-resolution orthophotos paired with detailed annotations for 36 semantic classes. Data collected from seven geographically diverse areas to ensure variation in climate, topography, and urbanization. Only tiles with annotated objects included, with training/validation splits created through random sampling across areas. Human expert review and quality control ensures annotation accuracy.

Result: Created NordFKB dataset with per-class binary segmentation masks in GeoTIFF format and COCO-style bounding box annotations. Released benchmarking repository with standardized evaluation protocols and tools for semantic segmentation and object detection, enabling reproducible and comparable research.

Conclusion: NordFKB provides a comprehensive benchmark for geospatial AI in Norway, paving the way for future expansions in coverage, temporal scope, and data modalities while supporting advancements in mapping, land administration, and spatial planning applications.
...
TL;DR: Splatent is a diffusion-based enhancement framework that improves 3D Gaussian Splatting in VAE latent space by recovering fine-grained details in 2D through multi-view attention, preserving VAE reconstruction quality while achieving state-of-the-art detail recovery.
...
Method: Splatent operates on top of 3D Gaussian Splatting in VAE latent space, but instead of reconstructing details in 3D, it recovers them in 2D from input views using multi-view attention mechanisms. This preserves pretrained VAE quality while achieving faithful detail recovery.
...
Conclusion: The 2D detail recovery approach through multi-view attention provides a better solution than conventional 3D-centric methods, preserving VAE reconstruction quality while achieving superior detail recovery without hallucinations.
...
TL;DR: RVE introduces reason-informed video editing, addressing the gap between video models' reasoning and editing capabilities through a new benchmark and self-reflective framework.


<details>
  <summary>Details</summary>
Motivation: Existing video unified models have strong understanding/generation capabilities but struggle with reason-informed visual editing due to inadequate datasets and disconnect between reasoning and editing capabilities.

Method: Proposes RVE-Bench benchmark with two subsets (Reasoning-Informed Video Editing and In-Context Video Generation), and ReViSE framework with Self-Reflective Reasoning that unifies generation and evaluation using internal VLM feedback.

Result: ReViSE achieves 32% improvement in Overall score on reasoning-informed video editing subset over state-of-the-art methods, enhancing both editing accuracy and visual fidelity.

Conclusion: The paper successfully bridges the reasoning-editing gap in video models through systematic benchmarking and a novel self-reflective framework that leverages internal VLMs for feedback-driven refinement.
...
TL;DR: GAINS is a two-stage Gaussian-based inverse rendering framework that uses learning-based priors to improve material recovery from sparse multi-view captures, addressing the ambiguity problem in geometry, reflectance, and lighting estimation.
...
Conclusion: GAINS effectively addresses the sparse-view inverse rendering challenge by leveraging learning-based priors to stabilize geometry and material estimation, enabling high-quality material recovery from limited observations.
...
TL;DR: Bayesian ATM improves on standard ATM by replacing Q-learning with Kalman filter updates for more stable, uncertainty-aware Q-value estimation in mHealth RL with costly measurements.


<details>
  <summary>Details</summary>
Motivation: Standard ATM algorithm uses Q-learning which is unstable in sparse/noisy mHealth environments. Need more stable, sample-efficient method that maintains uncertainty awareness when measurements are costly.

Method: Propose Bayesian extension to ATM that replaces standard Q-learning with Kalman filter-style Bayesian update, maintaining uncertainty-aware Q-value estimates.

Result: In small tabular environments: Bayesian ATM achieves comparable/improved returns with lower variance and more stable policies. In complex mHealth settings: Both ATM variants perform poorly, suggesting ATM assumptions mismatch real-world challenges.

Conclusion: Uncertainty-aware methods valuable in low-data settings, but need new RL algorithms that explicitly model causal structure, continuous states, and delayed feedback under observation cost constraints.
...
TL;DR: Researchers created a virtual humanoid simulator to train conversational agents for depression/PTSD screening without physical hardware, achieving better social timing and completeness with TD3 reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Physical humanoid robot testing is slow, causes wear, and limits iteration. Screening agents need to master complex conversational skills (timing, prosody, backchannels, facial attention) for mental health assessment, but most simulators ignore nonverbal dynamics and prioritize task accuracy over trust, pacing, and rapport.

Method: Created an agent-centered simulator pipeline converting interview data into 276 Unreal Engine MetaHuman patients with synchronized speech, gaze/face, and head-torso poses. Used a perception-fusion-policy loop with counterfactual replay (bounded nonverbal perturbations) and uncertainty-aware turn management. Compared three controllers: custom TD3 (Twin Delayed DDPG), PPO, and CEM.

Result: Custom TD3 outperformed PPO and CEM, achieving near-ceiling coverage with steadier pace at comparable rewards. Showed negligible turn overlap, aligned cut timing, fewer clarification prompts, and shorter waits. Performance remained stable under modality dropout and renderer swap, with rankings holding on held-out patient split.

Conclusion: The simulation-first approach enables safe, scalable training of humanoid conversational agents for mental health screening. TD3-based controller excels at social timing and completeness, providing a foundation for clinician-supervised humanoid pilots without physical hardware burden.
...
TL;DR: Foundation models show promise for ECG analysis, achieving 80% top performance rate compared to traditional time-series deep learning models, though limitations remain.


<details>
  <summary>Details</summary>
Motivation: ECG analysis requires domain expertise, creating a barrier for AI applications in healthcare. Foundation models offer potential to acquire domain knowledge without heavy reliance on human expertise, but their performance on ECG tasks hasn't been comprehensively analyzed.

Method: Benchmark evaluation comparing language models, general time-series foundation models, and ECG-specific foundation models against traditional time-series deep learning models on ECG analysis tasks.

Result: General time-series and ECG foundation models achieve up to 80% top performance rate, demonstrating effectiveness in ECG analysis. The study provides comprehensive experimental results and insights into model performance.

Conclusion: Foundation models show potential for advancing physiological waveform analysis like ECG, though limitations exist. The study provides a public benchmark for further research in this area.
...
TL;DR: LLM4XCE is a novel XL-MIMO channel estimation framework that leverages large language models' semantic modeling capabilities to handle hybrid-field channels, outperforming existing methods through fine-tuning only top transformer layers.


<details>
  <summary>Details</summary>
Motivation: XL-MIMO is crucial for 6G networks but faces challenges with hybrid-field channels (near-field + far-field effects). Traditional methods struggle with generalization, while LLMs have shown strong semantic understanding capabilities that could be applied to channel estimation.

Method: Proposes LLM4XCE framework with: 1) Carefully designed embedding module with Parallel Feature-Spatial Attention for deep fusion of pilot features and spatial structures, 2) Fine-tuning only the top two Transformer layers of LLMs to capture latent dependencies while maintaining training efficiency.
...
Conclusion: LLM4XCE successfully leverages LLMs' semantic modeling capabilities for XL-MIMO channel estimation, providing an effective solution for hybrid-field challenges with high efficiency and superior performance compared to existing approaches.
...
Method: Proposes Double Weighted KNN (DW-KNN) that integrates exponential distance weighting with neighbor validity assessment for more robust predictions.

Result: Achieves 0.8988 average accuracy, ranks 2nd among six methods, has lowest cross-validation variance (0.0156), and shows statistically significant improvements over other KNN variants.

Conclusion: DW-KNN provides a simple yet effective alternative to complex adaptive schemes, offering transparent and robust predictions valuable for high-stakes applications requiring explainability.
...
<details>
  <summary>Details</summary>
Motivation: Traditional user behavior prediction approaches rely on task-specific models and domain-specific feature engineering, which is time-consuming, computationally expensive, requires domain expertise, and doesn't scale well for online B2C platforms.

Method: LUMOS uses a transformer-based architecture with: 1) novel cross-attention mechanism that conditions predictions on future known events (holidays, sales), 2) multi-modal tokenization combining user transactions, event context, and static demographic attributes through specialized embedding pathways, 3) joint learning of multiple tasks from raw user activity data.

Result: On production dataset of 275B user activity tokens from 250M users: average improvement of 0.025 in ROC-AUC for binary classification tasks and 4.6% reduction in MAPE for regression tasks across 5 tasks. Online A/B testing showed 3.15% increase in Daily Active Users.

Conclusion: LUMOS demonstrates superior performance over traditional task-specific models by eliminating manual feature engineering and enabling scalable user behavior prediction through joint multi-task learning with future event conditioning.
...
TL;DR: A unified benchmarking framework for evaluating EEG-based foundation models across 11 clinical diagnostic tasks using 14 public datasets, showing foundation models perform well but simpler models remain competitive under clinical distribution shifts.
...
Method: Created a unified benchmarking framework with minimal preprocessing, standardized evaluation protocols across 11 diagnostic tasks using 14 publicly available EEG datasets covering various neurological and psychiatric conditions.

Result: Foundation models achieve strong performance in certain settings, but simpler models often remain competitive, especially under clinical distribution shifts. The framework enables reproducible side-by-side comparisons.

Conclusion: The benchmark provides an accessible, extensible framework for evaluating EEG foundation models in clinical applications, revealing that while advanced models show promise, simpler approaches remain relevant, particularly for real-world clinical scenarios with distribution shifts.
...
TL;DR: PS-LoRA addresses catastrophic forgetting in Continual Learning by aligning updates to prevent antagonistic gradient conflicts and using magnitude-based merging of adapters.


<details>
  <summary>Details</summary>
Motivation: Low-Rank Adaptation (LoRA) enables efficient Continual Learning but suffers from catastrophic forgetting due to destructive interference between tasks, primarily driven by antagonistic directional updates where new task gradients oppose historical weight trajectories.

Method: PS-LoRA uses a dual-regularization objective that penalizes conflicting directions and constrains magnitude deviations to ensure consistency with prior knowledge. It also implements a magnitude-based merging strategy to consolidate sequential adapters into a robust representation without retraining.

Result: Experiments on NLP and Vision benchmarks show that PS-LoRA outperforms state-of-the-art methods by preserving the stability of learned representations while efficiently adapting to new domains.

Conclusion: PS-LoRA effectively resolves gradient conflicts in Continual Learning with LoRA, preventing catastrophic forgetting through aligned updates and efficient adapter consolidation.
...
TL;DR: SEA is a new family of adversarial attacks on GNNs that uses spectral robustness evaluation to guide structural perturbations by identifying fragile directions in the input manifold.


<details>
  <summary>Details</summary>
Motivation: GNNs are vulnerable to structural perturbations, but existing attacks rely on gradient-based heuristics or local patterns and treat edges as equally important. There's a need for attacks that explicitly leverage spectral properties to identify the most vulnerable structural components.

Method: Compute spectral embeddings capturing fragile directions of the input manifold, assign robustness scores to edges/non-edges, then implement two attack variants: 1) Spade-guided deletion attack removing most spectrally robust edges, and 2) Spade-guided addition attack inserting edges between maximally incompatible nodes in fragile spectral space.

Result: The attacks operate at graph level, are model-aware but conceptually simple, and can be plugged into existing GNN architectures without requiring gradients.

Conclusion: SEA provides a new spectral-based approach for adversarial attacks on GNNs that explicitly leverages spectral robustness evaluation to guide structural perturbations more effectively than existing methods.
...
### [124] [Rates and architectures for learning geometrically non-trivial operators](https://arxiv.org/abs/2512.09376)
*T. Mitchell Roddenberry,Leo Tzou,Ivan Dokmanić,Maarten V. de Hoop,Richard G. Baraniuk*

Main category: cs.LG

TL;DR: The paper extends operator learning theory to geometric integral operators (double fibration transforms) that propagate singularities, proving they avoid the curse of dimensionality and achieve superalgebraic error decay with few training samples.


<details>
  <summary>Details</summary>
Motivation: Current operator learning theory focuses on elliptic operators with simple geometry that don't propagate singularities, but scientific ML often deals with problems involving singularity propagation (waves, advection, fluid dynamics). There's a need to expand theory to more complex operators.

Method: Extends learning theory to double fibration transforms (geometric integral operators including generalized Radon and geodesic ray transforms). Proves these operators avoid curse of dimensionality. Investigates architectures that explicitly encode geometry, using cross-attention based on levelset methods.

Result: Proves double fibration transforms don't suffer from curse of dimensionality - error decays superalgebraically (faster than any fixed power of reciprocal of training samples). Shows proposed architecture is universal, stable, and learns these transforms from very few training examples.

Conclusion: Expands operator learning theory to include geometric integral operators that propagate singularities, providing theoretical foundation for data-efficient learning of complex operators in scientific ML applications involving wave propagation, advection, and fluid dynamics.
...
### [125] [Financial Instruction Following Evaluation (FIFE)](https://arxiv.org/abs/2512.08965)
*Glenn Matlin,Siddharth,Anirudh JM,Aditya Shukla,Yahya Hassan,Sudheer Chava*

Main category: cs.LG

TL;DR: FIFE is a new benchmark for evaluating language models on complex financial analysis tasks, showing open-weight models outperform proprietary ones but all struggle with perfect compliance.


<details>
  <summary>Details</summary>
Motivation: Language models struggle with complex, interdependent instructions in high-stakes domains like finance where precision is critical, necessitating better evaluation methods.

Method: Created FIFE benchmark with 88 human-authored financial analysis prompts and verification system using chainable, verifiable constraints for fine-grained reward signals; evaluated 53 models (proprietary, open-weight, open-source) in zero-shot setting.

Result: Open-weight models (76.1 strict/79.5 loose) outperform proprietary systems (65.9/70.5), with open-source models lagging significantly (45.5/48.9). All models struggle with complex requirements and fail to achieve perfect compliance.

Conclusion: FIFE reveals performance hierarchy and limitations in current LMs for financial tasks; benchmark released as open-source to promote RL research in finance domain.
...
### [126] [CluCERT: Certifying LLM Robustness via Clustering-Guided Denoising Smoothing](https://arxiv.org/abs/2512.08967)
...
<details>
  <summary>Details</summary>
Motivation: LLMs are vulnerable to adversarial attacks through minor meaning-preserving changes like synonym substitutions, making robustness certification vital. Existing certification methods have loose bounds due to lack of semantic validation and suffer from high computational costs from repeated sampling.

Method: CluCERT uses clustering-guided denoising smoothing with: (1) semantic clustering filter to reduce noisy samples and retain meaningful perturbations, (2) refine module to extract core semantics, and (3) fast synonym substitution strategy to accelerate denoising.
...
Conclusion: CluCERT provides an effective framework for certifying LLM robustness with tighter bounds and improved efficiency, addressing key limitations of existing certification methods.
...
### [127] [StructuredDNA: A Bio-Physical Framework for Energy-Aware Transformer Routing](https://arxiv.org/abs/2512.08968)
*Mustapha Hamdi*

Main category: cs.LG

TL;DR: StructuredDNA introduces a bio-inspired sparse Transformer architecture using energy-guided routing to dramatically reduce computational costs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Address the critical increase in energy and compute costs from scaling large models by drawing inspiration from biological systems where structure emerges from low-energy configurations.

Method: Replaces dense Mixture-of-Experts routing with bio-physical, energy-guided routing based on semantic energy minimization. Inputs are grouped into semantic codons, and routing selects experts by minimizing a global energy functional combining cohesion, uncertainty, and computational cost.

Result: On BioASQ (K=50): 97.7% reduction in Energy Utilization Density (EUD) and Semantic Stability Index (SSI) of 0.998. On WikiText-103: demonstrates Semantic Scaling Law, scaling to K=2048 experts while maintaining >99% energy efficiency.

Conclusion: Establishes a robust, domain-agnostic paradigm for sparse computational frameworks, linking bio-physical principles to sparse expert routing. Points toward future energy-aware, modular, and scalable systems.
...
### [128] [TinyDéjàVu: Smaller Memory Footprint & Faster Inference on Sensor Data Streams with Always-On Microcontrollers](https://arxiv.org/abs/2512.09786)
*Zhaolan Huang,Emmanuel Baccelli*

Main category: cs.LG

TL;DR: TinyDéjàVu is a framework that drastically reduces RAM footprint for tiny neural networks on microcontrollers by optimizing data flows across layers, achieving >60% RAM savings and eliminating up to 90% of redundant computation on overlapping sliding windows.


<details>
  <summary>Details</summary>
Motivation: Always-on sensors with battery constraints need to run tiny neural networks on microcontrollers with limited memory (e.g., 128kB RAM). Optimizing data flows across neural network layers is crucial for meeting lifetime and energy requirements.

Method: Introduces TinyDéjàVu framework with novel algorithms designed to reduce RAM footprint for inference on sensor data time-series. The approach optimizes data flows across neural network layers and handles overlapping sliding window inputs efficiently.

Result: TinyDéjàVu saves more than 60% of RAM usage and eliminates up to 90% of redundant compute on overlapping sliding window inputs. Implementation is open source with reproducible benchmarks on hardware.

Conclusion: TinyDéjàVu provides an effective solution for memory-constrained microcontroller deployments of tiny ML models, significantly reducing RAM requirements and computational redundancy for sensor data time-series inference.
...
### [129] [Learning Robust Representations for Malicious Content Detection via Contrastive Sampling and Uncertainty Estimation](https://arxiv.org/abs/2512.08969)
*Elias Hossain,Umesh Biswas,Charan Gudla,Sai Phani Parsa*

Main category: cs.LG

TL;DR: UCF is a Positive-Unlabeled representation learning framework that uses uncertainty-aware contrastive loss, adaptive temperature scaling, and self-attention LSTM to improve classification under noisy, imbalanced conditions, achieving >93% accuracy in malicious content detection.


<details>
  <summary>Details</summary>
Motivation: Address classification challenges in noisy and imbalanced datasets where traditional methods struggle, particularly in high-stakes domains like cybersecurity and biomedical text mining where accurate positive instance identification is critical.

Method: UCF integrates uncertainty-aware contrastive loss (dynamically weighting samples by confidence), adaptive temperature scaling (adjusting to batch variability), self-attention-guided LSTM encoder, and positive anchors for training stabilization in PU learning setting.

Result: Achieved >93.38% accuracy, >0.93 precision, near-perfect recall with minimal false negatives, competitive ROC-AUC scores for malicious content classification. Visual analyses show clear separation between positive and unlabeled instances.

Conclusion: UCF is a robust, scalable PU learning solution that produces calibrated, discriminative embeddings suitable for high-stakes applications, effectively handling noisy, imbalanced data conditions.
...
### [130] [Peek-a-Boo Reasoning: Contrastive Region Masking in MLLMs](https://arxiv.org/abs/2512.08976)
...
<details>
  <summary>Details</summary>
Motivation: Prior approaches only evaluate final answers or attention maps, lacking causal, step-level attribution of how MLLMs use visual information during reasoning. There's a need to understand model dependencies on visual regions and evaluate reasoning faithfulness beyond just answer correctness.

Method: CRM systematically masks annotated visual regions and contrasts the resulting reasoning traces with unmasked baselines. It provides causal attribution by analyzing how masking specific regions affects each step of chain-of-thought reasoning.

Result: Applied to VisArgs datasets, CRM reveals distinct failure modes: some models preserve reasoning structure but hallucinate when evidence is missing, while others ground tightly to visual cues but collapse under perturbations.

Conclusion: CRM reframes visual benchmarks as diagnostic tools, highlighting the need for multimodal evaluation frameworks that measure not just performance, but also robustness and fidelity of reasoning, shifting focus from answer correctness to reasoning faithfulness.
...
### [131] [Graph Deep Learning for Intracranial Aneurysm Blood Flow Simulation and Risk Assessment](https://arxiv.org/abs/2512.09013)
...
<details>
  <summary>Details</summary>
Motivation: Intracranial aneurysm rupture risk depends on local hemodynamics, but current methods are either too slow/complex (CFD simulations) or have insufficient resolution (4D Flow MRI). There's a need for fast, accurate hemodynamic predictions that can be integrated into clinical workflows.

Method: Graph neural network surrogate model combining graph transformers with autoregressive predictions, trained on comprehensive high-fidelity CFD simulations of patient-specific aneurysms. The model takes vascular geometries as input and predicts hemodynamic parameters without mesh-specific calibration.

Result: The model reproduces full-field hemodynamics in less than one minute per cardiac cycle, generalizes across unseen patient geometries and inflow conditions, and enables near real-time inference integrated with existing imaging pipelines.

Conclusion: This work transforms high-fidelity simulations into a deployable decision support system, delivering high-resolution hemodynamic predictions within minutes of patient imaging without requiring computational specialists, enabling real-time bedside aneurysm analysis.
...
### [132] [Improving Multi-Class Calibration through Normalization-Aware Isotonic Techniques](https://arxiv.org/abs/2512.09054)
*Alon Arad,Saharon Rosset*

Main category: cs.LG

TL;DR: Novel isotonic normalization-aware techniques for multiclass calibration that outperform previous methods by accounting for probability normalization.


<details>
  <summary>Details</summary>
Motivation: Isotonic regression works well for binary calibration but its extension to multi-class problems via one-vs-rest produces suboptimal results compared to parametric methods, limiting practical adoption. There's a need for better multi-class calibration methods that practitioners find intuitive.

Method: Proposes two novel isotonic normalization-aware techniques: 1) NA-FIR (Normalization-Aware Fractional Isotonic Regression) that incorporates normalization directly into the optimization process, and 2) SCIR (Smooth Cumulative Isotonic Regression) that models the problem as cumulative bivariate isotonic regression.
...
Conclusion: The proposed isotonic normalization-aware techniques provide effective multi-class calibration solutions that are both theoretically grounded and practically useful, addressing limitations of previous isotonic regression extensions while maintaining intuitive assumptions expected by practitioners.
...
### [133] [A Diffusion-Based Framework for High-Resolution Precipitation Forecasting over CONUS](https://arxiv.org/abs/2512.09059)
*Marina Vicens-Miquel,Amy McGovern,Aaron J. Hill,Efi Foufoula-Georgiou,Clement Guilloteau,Samuel S. P. Shen*

Main category: cs.LG

TL;DR: A diffusion-based deep learning framework compares three precipitation forecasting strategies using different data sources (observations-only, forecasts-only, hybrid), showing the hybrid model excels at short lead times while forecasts-only correction works best for longer predictions up to 12 hours.


<details>
  <summary>Details</summary>
Motivation: Accurate precipitation forecasting is crucial for hydrometeorological risk management, especially for extreme rainfall events that cause flash flooding and infrastructure damage. There's a need to understand how different data sources (observations vs. numerical weather predictions) contribute to predictive skill.

Method: A diffusion-based deep learning framework systematically compares three residual prediction strategies: (1) fully data-driven using past MRMS observations, (2) corrective model using HRRR forecasts only, and (3) hybrid model integrating both MRMS and selected HRRR variables. Evaluation uses 1-km resolution forecasts from 1 to 12 hours via autoregressive rollouts across CONUS with calibrated uncertainty quantification.

Result: The DL framework consistently outperforms HRRR baseline across all lead times. Hybrid model performs best at shortest lead times, while HRRR-corrective model excels at longer lead times (up to 12 hours). All models show improved skill at extreme rainfall thresholds with reliable uncertainty quantification.

Conclusion: The study advances DL-based precipitation forecasting by demonstrating how different data sources contribute to predictive skill at various lead times. The findings are critical for emergency preparedness, where modest forecast horizon improvements can enhance decision-making for extreme weather events.
...
### [134] [Contrast transfer functions help quantify neural network out-of-distribution generalization in HRTEM](https://arxiv.org/abs/2512.09067)
*Luis Rangel DaCosta,Mary C. Scott*

Main category: cs.LG

TL;DR: Neural networks for HRTEM nanoparticle segmentation show predictable OOD performance degradation as imaging conditions shift from training data, with a framework developed to quantify domain shifts.


<details>
  <summary>Details</summary>
Motivation: Understanding neural network OOD generalization is crucial for experimental deployment, especially when ground-truth knowledge is hard to establish or experimental conditions vary significantly. Simulation-based data curation enables precise investigation of OOD behavior.

Method: Trained and measured OOD generalization of over 12,000 neural networks using synthetic data generated via random structure sampling and multislice simulation. Developed a framework using HRTEM contrast transfer function to compare information content and quantify OOD domain shifts.

Result: Neural network segmentation models show significant performance stability but smoothly and predictably worsen as imaging conditions shift from the training distribution. The framework effectively quantifies OOD domain shifts for imaging conditions.

Conclusion: The approach provides insights into OOD generalization for imaging condition shifts but has limitations for explaining other OOD shifts (e.g., atomic structures). Complementary techniques are needed for understanding generalization in broader settings.
...
### [135] [Modular Deep-Learning-Based Early Warning System for Deadly Heatwave Prediction](https://arxiv.org/abs/2512.09074)
*Shangqing Xu,Zhiyuan Zhao,Megha Sharma,José María Martín-Olalla,Alexander Rodríguez,Gregory A. Wellenius,B. Aditya Prakash*

Main category: cs.LG

TL;DR: DeepTherm is a deep learning-based early warning system that predicts deadly heatwaves without requiring historical heat-related mortality data, using a dual-prediction pipeline to disentangle baseline mortality from heatwave effects.


<details>
  <summary>Details</summary>
Motivation: Severe urban heatwaves threaten public health, but predicting incoming deadly heatwaves is challenging due to difficulty in defining/estimating heat-related mortality. Early warning systems need data availability, spatial/temporal robustness, and decision cost considerations.

Method: DeepTherm uses a modular deep learning approach with dual-prediction pipeline: (1) predicts baseline mortality in absence of heatwaves/irregular events, (2) disentangles this from all-cause mortality to identify heatwave impacts without requiring heat-related mortality history.

Result: Evaluation on real-world Spanish data shows consistent, robust, and accurate performance across diverse regions, time periods, and population groups. System allows trade-off between missed alarms and false alarms.

Conclusion: DeepTherm provides a flexible early warning solution for deadly heatwave prediction that addresses key challenges in data requirements, robustness, and decision-making trade-offs for public health protection.
...
### [136] [Beyond the Hype: Comparing Lightweight and Deep Learning Models for Air Quality Forecasting](https://arxiv.org/abs/2512.09076)
*Moazzam Umer Gondal,Hamad ul Qudous,Asma Ahmad Farhan*

Main category: cs.LG

TL;DR: Lightweight additive models (Facebook Prophet, NeuralProphet) outperform complex DL models and traditional statistical methods for urban air pollution forecasting in Beijing, offering competitive accuracy with better interpretability.


<details>
  <summary>Details</summary>
Motivation: Deep Learning and hybrid models for air pollution forecasting are complex and lack interpretability, hindering operational use. The study investigates whether simpler, interpretable additive models can deliver competitive forecasting performance.

Method: Used Facebook Prophet (FBP) and NeuralProphet (NP) with multi-year pollutant and meteorological data from Beijing. Applied systematic feature selection (correlation, mutual information, mRMR), leakage-safe scaling, chronological data splits. Compared against LSTM, LightGBM, and SARIMAX baselines.

Result: FBP consistently outperformed NP, SARIMAX, and learning-based baselines, achieving test R² above 0.94 for both PM2.5 and PM10 pollutants on a 7-day holdout evaluation.

Conclusion: Interpretable additive models remain competitive with both traditional and complex approaches, offering a practical balance of accuracy, transparency, and ease of deployment for urban air pollution forecasting.
...
### [137] [GS-KAN: Parameter-Efficient Kolmogorov-Arnold Networks via Sprecher-Type Shared Basis Functions](https://arxiv.org/abs/2512.09084)
*Oscar Eliasson*

Main category: cs.LG

TL;DR: GS-KAN is a lightweight KAN variant that uses shared parent functions with learnable linear transformations per layer, achieving better parameter efficiency than standard KANs while maintaining strong performance.


<details>
  <summary>Details</summary>
Motivation: Standard Kolmogorov-Arnold Networks (KANs) suffer from parameter inefficiency due to unique parameterizations for every network edge, making them impractical for high-dimensional regimes with parameter constraints.
...
Result: GS-KAN outperforms MLPs and standard KANs on continuous function approximation tasks with superior parameter efficiency, achieves competitive performance on tabular regression, and outperforms MLPs on high-dimensional classification tasks.

Conclusion: GS-KAN enables deployment of KAN-based architectures in high-dimensional regimes under strict parameter constraints where standard implementations are infeasible due to parameter explosion.
...
### [138] [Natural Geometry of Robust Data Attribution: From Convex Models to Deep Networks](https://arxiv.org/abs/2512.09103)
*Shihao Li,Jiachen Li,Dongmei Chen*

Main category: cs.LG

TL;DR: The paper introduces certified robust attribution methods that work for both convex models and deep networks, addressing the fragility of existing attribution methods to distributional perturbations.


<details>
  <summary>Details</summary>
Motivation: Current data attribution methods are sensitive to distributional perturbations, undermining their practical reliability. Standard attribution scores like TRAK, while accurate point estimates, are geometrically fragile and yield vacuous certification bounds when analyzed with naive Euclidean robustness analysis.

Method: 1) For convex models: Wasserstein-Robust Influence Functions (W-RIF) with provable coverage guarantees. 2) For deep networks: Natural Wasserstein metric that measures perturbations in the geometry induced by the model's own feature covariance, eliminating spectral amplification. This leads to Natural W-TRAK for neural networks.

Result: 1) Natural W-TRAK certifies 68.7% of ranking pairs on CIFAR-10 with ResNet-18 vs 0% for Euclidean baselines. 2) Self-Influence achieves 0.970 AUROC for label noise detection, identifying 94.1% of corrupted labels by examining top 20% of training data. 3) Reduces worst-case sensitivity by 76× and stabilizes attribution estimates.

Conclusion: The paper presents the first non-vacuous certified bounds for neural network attribution by addressing spectral amplification through the Natural Wasserstein metric. The framework provides theoretical grounding for attribution stability and enables practical reliable data attribution across model types.
...
### [139] [Learning Unmasking Policies for Diffusion Language Models](https://arxiv.org/abs/2512.09106)
...
TL;DR: This paper proposes using reinforcement learning to train sampling procedures for masked discrete diffusion language models, replacing heuristic strategies with learned policies that can generalize across models and sequence lengths.


<details>
  <summary>Details</summary>
Motivation: Current heuristic sampling strategies for masked discrete diffusion language models require manual tuning, degrade with larger buffer sizes, and have limitations in balancing quality and efficiency. There's a need for more systematic, learnable sampling procedures.

Method: Formalize masked diffusion sampling as a Markov decision process where the dLLM serves as the environment. Train lightweight transformer-based policies that map token confidences to unmasking decisions using reinforcement learning.

Result: Trained policies match state-of-the-art heuristic performance with semi-autoregressive generation and outperform them in full diffusion settings. Policies show transferability to new dLLMs and longer sequences, but degrade on out-of-domain data and face challenges with fine-grained accuracy-efficiency tuning.

Conclusion: Reinforcement learning can effectively train sampling policies for dLLMs that generalize across models and sequence lengths, offering a systematic alternative to heuristic approaches, though challenges remain with domain transfer and precise trade-off tuning.
...
### [140] [Spectral Embedding via Chebyshev Bases for Robust DeepONet Approximation](https://arxiv.org/abs/2512.09165)
*Muhammad Abid,Omer San*

Main category: cs.LG

TL;DR: SEDONet improves DeepONets by replacing coordinate-based trunks with Chebyshev spectral embeddings, achieving 30-40% better accuracy on non-periodic PDE problems.


<details>
  <summary>Details</summary>
Motivation: Standard DeepONet trunks using raw coordinates struggle with sharp gradients, boundary layers, and non-periodic structures in PDEs on bounded domains with Dirichlet/Neumann conditions.

Method: Introduces Spectral-Embedded DeepONet (SEDONet) with trunk driven by fixed Chebyshev spectral dictionary instead of coordinate inputs, providing non-periodic spectral embedding tailored to bounded domains.

Result: SEDONet achieves lowest relative L2 errors across all tested PDE benchmarks (2D Poisson, 1D Burgers, advection-diffusion, Allen-Cahn, Lorenz-96), with 30-40% average improvement over baseline DeepONet and meaningful gains over Fourier-embedded variants on non-periodic geometries.

Conclusion: Chebyshev spectral embeddings provide a simple, parameter-neutral modification to DeepONets that offers robust and efficient spectral framework for surrogate modeling of PDEs on bounded domains, better preserving high-frequency and boundary-localized features.
...
### [141] [Understanding the Failure Modes of Transformers through the Lens of Graph Neural Networks](https://arxiv.org/abs/2512.09182)
*Hunjae Lee*
...
<details>
  <summary>Details</summary>
Motivation: Transformers have surprising failure modes and asymmetric performance degradation, but lack theoretical understanding. The paper aims to bridge this gap by applying GNN theory to analyze transformer failure modes systematically.

Method: The study uses graph neural network theory as a lens to analyze transformers, viewing deep learning as learnable information mixing/propagation. It examines information propagation bottlenecks in decoder-only transformers and their geometric properties.

Result: Many transformer issues parallel GNN problems. Causal decoder-only transformers create specific geometric properties in information propagation leading to predictable failure modes. Existing ad-hoc solutions can be unified under this theoretical perspective.

Conclusion: The paper provides a theoretical framework for understanding transformer failure modes through GNN theory, offering insights into why existing solutions work and how to improve them to target specific bottlenecks in information propagation.
...
TL;DR: A data-driven clinical support tool for selecting optimal transcatheter heart valves in TAVR to minimize permanent pacemaker implantation risk, showing 26% and 16% PPI reduction in US and Greek cohorts respectively.


<details>
  <summary>Details</summary>
Motivation: Current guidelines for transcatheter heart valve selection in TAVR are debated, and permanent pacemaker implantation is a major postoperative complication that needs to be minimized.

Method: Developed a data-driven clinical support tool using a novel synthesized dataset combining US and Greek patient populations with three data sources (demographics, CT scans, echocardiograms). Introduced leaf-level analysis to leverage population heterogeneity and avoid uncertain counterfactual risk estimates.
...
Conclusion: This represents the first unified, personalized prescription strategy for THV selection in TAVR, demonstrating significant reduction in pacemaker implantation risk across different patient populations.
...
TL;DR: LLMs show reliability challenges in analog circuit design tasks, with sensitivity to data formats, instability in generated designs, and limited generalization to unseen configurations.


<details>
  <summary>Details</summary>
Motivation: While LLMs have shown impressive capabilities in natural language tasks, their reliability and robustness in real-world engineering domains like analog circuit design remain unexplored, limiting practical utility in human-centric workflows.

Method: Investigated LLMs for analog circuit design tasks, studying how different data representations influence model behavior and comparing smaller models (T5, GPT-2) with larger foundation models (Mistral-7B, GPT-oss-20B) under varying training conditions.

Result: Key reliability challenges identified: sensitivity to data format, instability in generated designs, and limited generalization to unseen circuit configurations.

Conclusion: Findings provide early evidence on the limits and potential of LLMs as tools to enhance human capabilities in complex engineering tasks, offering insights for designing reliable, deployable foundation models for structured, real-world applications.
...
### [144] [Tensor-Compressed and Fully-Quantized Training of Neural PDE Solvers](https://arxiv.org/abs/2512.09202)
*Jinming Lu,Jiayi Tian,Yequan Zhao,Hai Li,Zheng Zhang*

Main category: cs.LG

TL;DR: A framework for efficient PINN training on edge devices using quantization, Stein's estimator, and tensor-train decomposition, achieving significant speedups and energy savings.


<details>
  <summary>Details</summary>
Motivation: PINNs are promising for solving PDEs but face computational/memory challenges on resource-constrained platforms due to high-order differentiation, tensor operations, and full-precision arithmetic.

Method: Integrates fully quantized training, Stein's estimator-based residual loss computation, and tensor-train decomposition for weight compression. Key innovations: mixed-precision training with SMX format, difference-based quantization for Stein's estimator, and partial-reconstruction scheme for TT-Layers.

Result: Achieves comparable/better accuracy than full-precision baselines on 2-D Poisson, 20-D HJB, and 100-D Heat equations, with 5.5x-83.5x speedups and 159.6x-2324.1x energy savings.
...
### [145] [Contrastive Learning for Semi-Supervised Deep Regression with Generalized Ordinal Rankings from Spectral Seriation](https://arxiv.org/abs/2512.09267)
*Ce Wang,Weihang Dai,Hanru Bai,Xiaomeng Li*

Main category: cs.LG

TL;DR: Semi-supervised contrastive regression method that uses spectral seriation to recover ordinal relationships from unlabeled data, reducing annotation dependence while improving representation learning.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive learning methods for regression heavily depend on label information to establish ordinal relationships, limiting their application in semi-supervised settings where labeled data is scarce and expensive to obtain.

Method: Construct feature similarity matrix with both labeled and unlabeled samples; use spectral seriation to recover ordinal rankings of unlabeled samples; regularize with labeled data; employ dynamic programming for robust feature selection; use recovered ordinal relationships for contrastive learning and as supervision signal.

Result: The method surpasses existing state-of-the-art semi-supervised deep regression methods on various datasets, with theoretical guarantees and empirical verification provided.

Conclusion: The proposed approach successfully extends contrastive regression to semi-supervised settings, reducing annotation dependence while achieving more robust results through effective utilization of both labeled and unlabeled data.
...
### [146] [Goal inference with Rao-Blackwellized Particle Filters](https://arxiv.org/abs/2512.09269)
*Yixuan Wang,Dan P. Guralnik,Warren E. Dixon*

Main category: cs.LG

TL;DR: The paper proposes a Rao-Blackwellized Particle Filter approach for intent inference of mobile agents, with information-theoretic analysis of estimation quality and bounds on estimator performance.


<details>
  <summary>Details</summary>
Motivation: Inferring goals of mobile agents from noisy trajectory observations is a fundamental estimation problem with applications in surveillance, security, and human-robot interaction.

Method: Uses a variant of Rao-Blackwellized Particle Filter (RBPF) that analytically marginalizes linear-Gaussian substructure, improving sample efficiency. Introduces two estimators: Gaussian mixture model using RBPF weights and a reduced version confined to effective sample.

Result: Provides computable lower bounds on KL divergence between true intent distribution and RBPF estimates via Gaussian-mixture KL bounds. Shows reduced estimator performs almost as well as complete one. Experiments demonstrate fast and accurate intent recovery.

Conclusion: The RBPF approach enables efficient intent inference with provable performance bounds, motivating future work on designing controllers that can obfuscate intent from adversaries.
...
### [147] [Hetero-SplitEE: Split Learning of Neural Networks with Early Exits for Heterogeneous IoT Devices](https://arxiv.org/abs/2512.09313)
*Yuki Oda,Yuta Ono,Hiroshi Nakamura,Hideki Takase*

Main category: cs.LG

TL;DR: Hetero-SplitEE enables heterogeneous IoT devices to collaboratively train deep neural networks by allowing each client to select different split points based on computational capacity, using either sequential or parallel training strategies.


<details>
  <summary>Details</summary>
Motivation: Existing Split Learning approaches assume client homogeneity and uniform split points, which limits applicability to real-world IoT systems where devices have heterogeneous computational resources. There's a need for methods that can accommodate diverse device capabilities while maintaining collaborative training efficiency.

Method: Proposes Hetero-SplitEE which integrates heterogeneous early exits into hierarchical training, allowing each client to select distinct split points (cut layers) based on computational capacity. Two cooperative training strategies: 1) Sequential strategy trains clients sequentially with shared server model to reduce computational overhead, 2) Averaging strategy enables parallel client training with periodic cross-layer aggregation.

Result: Extensive experiments on CIFAR-10, CIFAR-100, and STL-10 datasets using ResNet-18 demonstrate that the method maintains competitive accuracy while efficiently supporting diverse computational constraints, enabling practical deployment in heterogeneous IoT ecosystems.

Conclusion: Hetero-SplitEE successfully addresses the limitation of existing Split Learning approaches by accommodating device heterogeneity, making collaborative deep learning practical for real-world IoT systems with diverse computational capabilities.
...
### [148] [Self-Supervised Learning with Gaussian Processes](https://arxiv.org/abs/2512.09322)
*Yunshan Duan,Sinead Williamson*

Main category: cs.LG

TL;DR: GPSSL is a novel self-supervised learning method using Gaussian processes that provides uncertainty quantification and avoids explicit positive pair generation.


<details>
  <summary>Details</summary>
Motivation: Traditional SSL methods require explicit generation of similar observation pairs, which can be challenging for many data types, and lack uncertainty quantification, performing poorly in out-of-sample prediction settings.

Method: Uses Gaussian processes with GP priors on representations, minimizing a loss function that encourages informative representations. The GP covariance function naturally pulls similar representations together as an alternative to explicit positive samples.

Result: GPSSL outperforms traditional methods in accuracy, uncertainty quantification, and error control across various datasets for classification and regression tasks.

Conclusion: GPSSL addresses key limitations of existing SSL methods by providing uncertainty quantification without requiring explicit positive pair generation, while maintaining connections to established methods like kernel PCA and VICReg.
...
### [149] [Self Distillation Fine-Tuning of Protein Language Models Improves Versatility in Protein Design](https://arxiv.org/abs/2512.09329)
*Amin Tavakoli,Raswanth Murugan,Ozan Gokdemir,Arvind Ramanathan,Frances Arnold,Anima Anandkumar*

Main category: cs.LG

TL;DR: A simple recipe for fast supervised fine-tuning of protein language models that uses the PLM itself to generate high-quality training data via domain-specific filters, improving generated protein sequence fidelity, reliability, and novelty.


<details>
  <summary>Details</summary>
Motivation: Supervised fine-tuning for protein language models is currently ad hoc due to the difficulty of obtaining high-quality annotated protein data compared to natural language. Existing approaches require costly precompiled experimental datasets.

Method: Leverages the PLM itself with a lightweight curation pipeline and domain-specific filters to construct high-quality training data. The filters can refine PLM output and identify candidates for in vitro evaluation. The approach is agnostic to PLM choice and protein system.

Result: Demonstrated with GenSLM applied to tryptophan synthase enzyme family. The fine-tuned model generates sequences that are more novel and display improved characteristics across both targeted design constraints and emergent protein property measures.

Conclusion: The method enables PLMs to generate more stable and functional enzymes while expanding exploration into protein sequence space beyond natural variants, addressing the data scarcity problem in protein sequence modeling.
...
### [150] [Improved Physics-Driven Neural Network to Solve Inverse Scattering Problems](https://arxiv.org/abs/2512.09333)
*Yutong Du,Zicheng Liu,Bo Wu,Jingwei Kou,Hang Li,Changyou Li,Yali Zong,Bo Qi*

Main category: cs.LG

TL;DR: IPDNN framework with GLOW activation, dynamic scatter identification, and transfer learning for electromagnetic inverse scattering achieves better accuracy, robustness, and efficiency than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: To solve electromagnetic inverse scattering problems more effectively by combining physical interpretability of iterative algorithms with real-time inference capability of neural networks, while addressing convergence stability and computational efficiency issues.

Method: Improved physics-driven neural network (IPDNN) framework with: 1) Gaussian-localized oscillation-suppressing window (GLOW) activation function for stable convergence and lightweight architecture, 2) Dynamic scatter subregion identification strategy for adaptive domain refinement, 3) Transfer learning for practical scenario applicability.

Result: Superior reconstruction accuracy, robustness, and efficiency compared with existing state-of-the-art methods, demonstrated through both numerical simulations and experimental results.

Conclusion: The proposed IPDNN framework successfully integrates physical interpretability with neural network efficiency, providing an effective solution for electromagnetic inverse scattering problems with practical applicability.
...
### [151] [Branching Strategies Based on Subgraph GNNs: A Study on Theoretical Promise versus Practical Reality](https://arxiv.org/abs/2512.09355)
...
<details>
  <summary>Details</summary>
Motivation: There's a gap between theoretical expressivity and practical efficiency in GNNs for MILP branching. While higher-order GNNs are expressive but computationally prohibitive, and standard MPNNs are efficient but less expressive, the paper investigates Subgraph GNNs as a middle ground to understand the trade-off between expressivity and efficiency.

Method: The paper uses node-anchored Subgraph GNNs, which have expressive power strictly lower than 3-WL but higher than standard MPNNs. It proves theoretical sufficiency for approximating Strong Branching scores and conducts extensive empirical evaluation on four benchmark datasets to compare practical performance.

Result: Theoretical analysis shows node-anchored Subgraph GNNs can approximate Strong Branching scores. However, empirical evaluation reveals significant memory bottlenecks and slower solving times due to O(n) complexity overhead, making them less practical than MPNNs and heuristics despite theoretical advantages.

Conclusion: For MILP branching, the computational cost of expressive GNNs currently outweighs their gains in decision quality. Future research should focus on efficiency-preserving expressivity rather than pursuing maximum theoretical expressivity alone.
...
### [152] [A Granular Framework for Construction Material Price Forecasting: Econometric and Machine-Learning Approaches](https://arxiv.org/abs/2512.09360)
*Boge Lyu,Qianye Yin,Iris Denise Tommelein,Hanyang Liu,Karnamohit Ranka,Karthik Yeluripati,Junzhe Shi*

Main category: cs.LG

TL;DR: A forecasting framework using CSI MasterFormat structure with LSTM, ARIMA, VECM, and Chronos-Bolt models predicts construction material prices at six-digit section level, showing LSTM with explanatory variables achieves best accuracy.


<details>
  <summary>Details</summary>
Motivation: Construction material price volatility creates significant risks for cost estimation, budgeting, and project delivery, requiring granular and scalable forecasting methods to improve reliability.

Method: Developed forecasting framework using CSI MasterFormat as target data structure for six-digit section level predictions. Integrated explanatory variables (raw material prices, commodity indexes, macroeconomic indicators). Evaluated four time-series models: LSTM, ARIMA, VECM, and Chronos-Bolt in baseline (CSI only) and extended (with explanatory variables) configurations.

Result: Explanatory variables significantly improved predictive performance across all models. LSTM achieved highest accuracy with RMSE 1.390 and MAPE 0.957, representing up to 59% improvement over traditional ARIMA. Framework validated across multiple CSI divisions, with Division 06 (Wood, Plastics, Composites) as detailed demonstration case.

Conclusion: Research provides robust methodology enabling owners and contractors to improve budgeting practices and achieve more reliable Definitive-level cost estimation through granular construction material price forecasting.
...
### [153] [KGOT: Unified Knowledge Graph and Optimal Transport Pseudo-Labeling for Molecule-Protein Interaction Prediction](https://arxiv.org/abs/2512.09365)
*Jiayu Qin,Zhengquan Luo,Guy Tadmor,Changyou Chen,David Zeevi,Zhiqiang Xu*

Main category: cs.LG

TL;DR: A framework that addresses molecule-protein interaction prediction challenges by aggregating diverse biological data and using optimal transport for pseudo-labeling, improving accuracy and zero-shot ability.


<details>
  <summary>Details</summary>
Motivation: Two main challenges in molecule-protein interaction (MPI) prediction: 1) scarcity of labeled molecule-protein pairs limiting model performance, and 2) existing methods ignore broader biological context (genes, pathways, functional annotations) that could provide complementary information.

Method: First aggregates diverse biological datasets (molecular, protein, genes, pathway-level interactions), then develops an optimal transport-based approach to generate high-quality pseudo-labels for unlabeled molecule-protein pairs, leveraging known interaction distributions to guide label assignment.
...
### [154] [CFLight: Enhancing Safety with Traffic Signal Control through Counterfactual Learning](https://arxiv.org/abs/2512.09368)
*Mingyuan Li,Chunyu Liu,Zhuojun Li,Xiao Liu,Guangsheng Yu,Bo Du,Jun Shen,Qiang Wu*

Main category: cs.LG

TL;DR: CFLight: A counterfactual learning framework that enhances safety in traffic signal control by reducing collisions while maintaining traffic efficiency.


<details>
  <summary>Details</summary>
Motivation: Current RL-based traffic signal control methods prioritize efficiency over safety and lack interpretability, failing to address the critical balance needed to reduce intersection accidents.

Method: Proposes a counterfactual learning framework with a structural causal model to predict outcomes of alternative actions, asking "what if" scenarios when unsafe events occur, and integrates this with additional modules for safe RL practices.

Result: CFLight reduces collisions and improves overall traffic performance compared to conventional RL methods and recent safe RL models, achieving near-zero collision control through extensive experiments on real-world and synthetic datasets.

Conclusion: The framework provides a generalized and safe approach for RL methods that can be applied to other domains, successfully balancing safety and efficiency in traffic signal control.
...
### [155] [Are Hypervectors Enough? Single-Call LLM Reasoning over Knowledge Graphs](https://arxiv.org/abs/2512.09369)
*Yezi Liu,William Youngwoo Chung,Hanning Chen,Calvin Yeung,Mohsen Imani*

Main category: cs.LG

TL;DR: PathHD: A lightweight KG reasoning framework using hyperdimensional computing instead of neural encoders, with single LLM call per query for efficient, interpretable reasoning.


<details>
  <summary>Details</summary>
Motivation: Current KG-LLM pipelines use heavy neural encoders or repeated LLM calls, leading to high latency, GPU cost, and opaque decisions that hinder faithful, scalable deployment.

Method: Replaces neural path scoring with hyperdimensional computing: encodes relation paths into block-diagonal GHRR hypervectors, ranks candidates with blockwise cosine similarity and Top-K pruning, then performs one-shot LLM adjudication with cited supporting paths.

Result: Achieves comparable/better Hits@1 than neural baselines with single LLM call; reduces latency by 40-60% and GPU memory by 3-5×; delivers faithful, path-grounded rationales for better error diagnosis and controllability.

Conclusion: Carefully designed HDC representations provide practical substrate for efficient KG-LLM reasoning, offering favorable accuracy-efficiency-interpretability trade-off.
...
TL;DR: Proposes a federated distillation-assisted vehicle edge caching scheme using lightweight denoising diffusion probabilistic model to reduce communication overhead and improve cache hit rate while protecting privacy.


<details>
  <summary>Details</summary>
Motivation: Vehicle edge caching reduces latency but requires accurate content prediction without exposing user privacy. Traditional federated learning protects privacy but has high communication overhead and fails when vehicles leave RSU coverage before training completes.

Method: Uses federated distillation-assisted vehicle edge caching scheme based on lightweight denoising diffusion probabilistic model (LDPM). Combines federated distillation to reduce communication overhead with LDPM for efficient content prediction.

Result: The proposed scheme demonstrates good robustness to vehicle speed variations, significantly reduces communication overhead, and improves cache hit percentage compared to traditional approaches.

Conclusion: The federated distillation-assisted LDPM approach effectively addresses privacy concerns, reduces communication overhead, and improves caching performance in vehicle edge networks, making it suitable for dynamic vehicular environments.
...
TL;DR: ConFormer is a novel traffic prediction framework using conditional transformers with graph propagation and guided normalization, outperforming state-of-the-art models while being more efficient.


<details>
  <summary>Details</summary>
Motivation: Traffic prediction is challenging due to complex external factors like accidents and regulations that existing models often overlook due to limited data integration.

Method: Proposed ConFormer (Conditional Transformer) integrates graph propagation with guided normalization layer, dynamically adjusting spatial and temporal node relationships based on historical patterns. Also introduced enriched traffic datasets from Tokyo and California with accident and regulation data.

Result: ConFormer surpasses state-of-the-art STAEFormer in both predictive performance and efficiency, achieving lower computational costs and reduced parameter demands. Consistently outperforms mainstream spatio-temporal baselines across multiple metrics.

Conclusion: ConFormer demonstrates potential to advance traffic prediction research by effectively integrating external factors and achieving superior performance with improved efficiency.
...
TL;DR: Black-box distillation attack can cheaply replicate medical LLM capabilities while stripping safety mechanisms, exposing alignment vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: As medical LLMs integrate into clinical workflows, concerns about alignment robustness and safety are escalating. Prior work focused on classification models or memorization leakage, leaving safety-aligned generative medical LLMs' vulnerabilities underexplored.

Method: Black-box distillation attack using 48,000 instruction queries to Meditron-7B, collecting 25,000 benign instruction-response pairs, then fine-tuning LLaMA3 8B surrogate via LoRA with zero-alignment supervision. Also developed dynamic adversarial evaluation framework with Generative Query-based harmful prompt generation, verifier filtering, and adaptive Random Search jailbreak attacks.

Result: With $12 cost, surrogate achieves strong fidelity on benign inputs while producing unsafe completions for 86% of adversarial prompts (vs. Meditron-7B's 66% and base model's 46%). Reveals functional-ethical gap where task utility transfers but alignment collapses.

Conclusion: Benign-only black-box distillation exposes practical threat: adversaries can cheaply replicate medical LLM capabilities while stripping safety mechanisms, highlighting need for extraction-aware safety monitoring and layered defense systems.
...
TL;DR: Proposes a Cauchy-Schwarz fairness regularizer that consistently improves group fairness metrics while maintaining competitive accuracy, with more stable utility-fairness trade-offs than prior methods.
...
Method: Organizes existing methods into three families, identifies desirable properties for distance measures, and proposes a Cauchy-Schwarz fairness regularizer that penalizes empirical CS divergence between prediction distributions conditioned on sensitive groups.

Result: CS divergence yields tighter bounds than KL divergence, MMD, and mean disparity; extensive experiments on 4 tabular benchmarks and 1 image dataset show CS regularizer consistently improves Demographic Parity and Equal Opportunity metrics while maintaining competitive accuracy.

Conclusion: The proposed CS regularizer achieves more stable utility-fairness trade-offs across hyperparameter settings compared to prior regularizers, providing a principled approach to group fairness regularization.
...
TL;DR: The paper challenges the assumption that balanced subgroup representation optimizes model performance, showing that imbalanced data can sometimes improve subgroup performance. It introduces the "latent separation hypothesis" linking subgroup performance sensitivity to the degree of separation between subgroups in pre-trained model's latent space.


<details>
  <summary>Details</summary>
Motivation: Standard practice assumes balancing subgroup representation optimizes model generalization across populations, but recent empirical results contradict this assumption. The paper aims to systematically study how subgroup allocation affects performance and understand when and why imbalanced data distributions can be beneficial.

Method: Conducted systematic study across four vision and language models, varying training data composition to characterize sensitivity of subgroup performance to data balance. Proposed and formalized the latent separation hypothesis, provided theoretical analysis, and validated it empirically. Applied the framework to foundation model fine-tuning.

Result: Found that subgroup performance sensitivity to data representation depends on latent space separation between subgroups. When subgroups are well-separated in latent space, performance is less dependent on balanced representation. Quantitative analysis of latent subgroup separation can inform data collection and balancing decisions.

Conclusion: The latent separation hypothesis provides a principled framework for understanding when balanced data matters. Rather than always balancing subgroups, practitioners should analyze latent space separation to make informed decisions about data collection and balancing for optimal subgroup performance.
...
TL;DR: The paper studies contextual dynamic pricing with heterogeneous buyers, developing algorithms with optimal regret bounds for both contextual and non-contextual settings.


<details>
  <summary>Details</summary>
Motivation: Prior work assumes homogeneous buyer types, but real-world buyers have heterogeneous valuations. The paper addresses this gap by studying contextual pricing where buyer valuation types are drawn from an unknown distribution with finite support size K⋆.

Method: Develops a contextual pricing algorithm based on optimistic posterior sampling for the general case, and a variance-aware zooming algorithm for the non-contextual case.

Result: Achieves regret Õ(K⋆√dT) for contextual pricing (tight in d and T up to logarithmic terms), and optimal dependence on K⋆ for non-contextual pricing.

Conclusion: The paper provides the first study of contextual dynamic pricing with heterogeneous buyer populations, establishing optimal regret bounds and introducing specialized algorithms for both contextual and non-contextual settings.
...
TL;DR: QuanvNeXt is a fully quanvolutional model for EEG-based depression diagnosis that achieves state-of-the-art performance with 93.1% accuracy and 97.2% AUC-ROC, featuring novel Cross Residual blocks for better feature learning and demonstrating robust uncertainty calibration.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient and reliable end-to-end model for EEG-based depression diagnosis that addresses feature homogeneity issues in existing approaches while maintaining parameter efficiency.

Method: QuanvNeXt incorporates novel Cross Residual blocks that reduce feature homogeneity and strengthen cross-feature relationships. The model was evaluated on two open-source EEG datasets and compared against state-of-the-art baselines like InceptionTime. Uncertainty analysis was conducted across Gaussian noise levels, and post-hoc explainable AI analysis was performed to validate learned patterns.

Result: Achieved average accuracy of 93.1% and average AUC-ROC of 97.2%, outperforming InceptionTime (91.7% accuracy, 95.9% AUC-ROC). Uncertainty analysis showed well-calibrated predictions with low ECE scores (0.0436 to 0.1159) even at high noise levels (ε = 0.1). Explainable AI confirmed the model effectively identifies spectrotemporal patterns distinguishing healthy controls from major depressive disorder.

Conclusion: QuanvNeXt establishes an efficient and reliable approach for EEG-based depression diagnosis, demonstrating superior performance, robust uncertainty calibration, and meaningful pattern learning for clinical applications.
...
TL;DR: A VAE with Gaussian Process latent dynamics transfers sequential structure from observations to latent space while keeping parallel decoding, showing temporal structure can emerge from latent geometry rather than explicit neural operations.


<details>
  <summary>Details</summary>
Motivation: To explore whether temporal structure in language modeling can be supported by the probabilistic geometry of latent space rather than explicit neural autoregressive operations, allowing for parallel generation while maintaining sequential dynamics.

Method: Combines Variational Autoencoder with Gaussian Process latent dynamics: causal GP prior for sequential structure, structured amortized posterior, non-autoregressive parallel decoder, and training with regularized ELBO.

Result: Model trains stably in proof-of-concept framework; sequential and parallel sampling variants show consistent behavior; latent space geometry can capture temporal structure.

Conclusion: Part of temporal structure in language models can be supported by latent space probabilistic geometry rather than explicit neural operations, enabling parallel generation with sequential dynamics.
...
TL;DR: Stanford Sleep Bench is a large-scale PSG dataset with 17,467 recordings and 13 clinical tasks, enabling systematic evaluation of self-supervised representation learning methods for sleep analysis.


<details>
  <summary>Details</summary>
Motivation: PSG generates vast multimodal clinical data suitable for self-supervised learning, but progress in sleep foundation models is hindered by: 1) lack of shared dataset/benchmark with diverse tasks, and 2) absence of systematic evaluation of SSRL approaches across sleep tasks.

Method: Introduced Stanford Sleep Bench dataset (17,467 PSG recordings, 163,000+ hours) with 13 clinical disease prediction tasks plus canonical sleep tasks. Systematically evaluated SSRL pre-training methods across four downstream tasks: sleep staging, apnea diagnosis, age estimation, and disease/mortality prediction.

Result: Multiple pretraining methods achieve comparable performance for sleep staging, apnea diagnosis, and age estimation. However, contrastive learning significantly outperforms other approaches for mortality and disease prediction while also converging faster during pretraining.

Conclusion: The Stanford Sleep Bench dataset enables systematic evaluation of SSRL methods for sleep analysis. Contrastive learning shows particular promise for clinical prediction tasks. The authors will release the dataset, pretrained models, training pipelines, and evaluation code to advance sleep research.
...
TL;DR: TCSC framework combines semantic communication with VEC for IoV highway scenarios, using MAPPO-PDN for semantic symbol optimization and LP for offloading ratio to minimize latency and symbols.


<details>
  <summary>Details</summary>
Motivation: To address efficient edge task processing in Internet of Vehicles by combining semantic communication with vehicular edge computing, specifically focusing on highway scenarios where vehicles need to offload tasks via V2I and V2V communications while optimizing latency and resource usage.

Method: Proposes Tripartite Cooperative Semantic Communication (TCSC) framework with two subproblem approach: 1) MAPPO-PDN (multi-agent proximal policy optimization with parametric distribution noise) for semantic symbol optimization, 2) Linear programming for offloading ratio optimization, solving a MINLP problem for task latency and semantic symbol minimization.

Result: Simulations demonstrate superior performance compared to other algorithms, showing effective optimization of both semantic symbol usage and task offloading in vehicular edge computing scenarios.

Conclusion: The TCSC framework successfully integrates semantic communication with VEC for IoV, providing an efficient solution for task offloading optimization in highway scenarios through innovative MAPPO-PDN and LP methods.
...
TL;DR: Membership inference attacks on generative audio models have limited effectiveness at scale, but dataset inference (aggregating evidence across multiple samples) successfully detects if an artist's collection was used in training, offering practical copyright protection.


<details>
  <summary>Details</summary>
Motivation: As generative audio models advance using large training corpora of artistic works, copyright concerns arise about verifying whether specific artists' materials were included in training, creating a need for practical verification mechanisms for copyright holders.

Method: Investigates membership inference attacks (MIA) on open-source generative audio models to detect if specific audio samples were in training data, then extends to dataset inference (DI) which aggregates membership evidence across multiple samples from an artist's collection.

Result: Membership inference alone is ineffective at scale due to weak per-sample signals in large diverse datasets, but dataset inference successfully detects whether an artist's works contributed to model training by aggregating evidence across multiple samples.

Conclusion: Dataset inference offers a practical mechanism for copyright protection and dataset accountability in large audio generative models, providing a promising direction for artists and media owners to verify training data usage.
...
TL;DR: Equivariant networks have symmetry bias but limited expressivity; increasing model size compensates while maintaining lower complexity for better generalization.


<details>
  <summary>Details</summary>
Motivation: To understand the expressive power of equivariant neural networks, which encode symmetry as inductive bias but whose limitations are not well understood.

Method: Analyze 2-layer ReLU networks, examine boundary hyperplanes and channel vectors, construct examples showing expressivity limitations, and demonstrate compensation through model size enlargement.

Result: Equivariance constraints strictly limit expressive power, but this can be compensated by increasing model size; despite larger size, equivariant networks have lower hypothesis space complexity.

Conclusion: Equivariant networks trade off expressivity for symmetry bias, but can achieve both through model scaling while maintaining superior generalizability due to lower complexity.
...
TL;DR: A data-driven approach that integrates design features with manufacturing process data using machine learning to enable automated design improvements and support sustainable product development.


<details>
  <summary>Details</summary>
Motivation: Current data-driven methods are siloed within specific domains (design or manufacturing), limiting the ability to leverage the relationship between design decisions and manufacturing outcomes. IIoT enables real-time manufacturing data collection, but the lack of integration between design features and process data restricts data-driven product design improvements.

Method: Develops a comprehensive system architecture for continuous data collection and integration. Creates a linkage between design features and manufacturing process data, then uses this as the basis for a machine learning model that provides automated design improvement suggestions.

Result: The approach enables automated design improvement suggestions by analyzing relationships between design features and manufacturing outcomes. It integrates manufacturing process data with sustainability metrics, opening new possibilities for sustainable product development.

Conclusion: The proposed data-driven approach successfully bridges the gap between design and manufacturing domains, leveraging IIoT data to enable automated design improvements and support sustainable product development through integrated analysis of design features and manufacturing process data.
...
TL;DR: CrossAgent is a unified AI agent that can dynamically switch between different action spaces (APIs, GUI events, robotic commands) to adapt to varying task requirements, achieving SOTA performance in Minecraft.


<details>
  <summary>Details</summary>
Motivation: Current AI agents are limited to static, predefined action spaces (e.g., only APIs or only GUI events), which restricts their adaptability in dynamic environments where optimal interaction granularity varies contextually.

Method: Proposes CrossAgent with a training pipeline combining cold-start supervised fine-tuning and Multi-Turn Group Relative Policy Optimization (GRPO) to learn adaptive action switching without human-specified rules.

Result: Extensive experiments on over 800 tasks in Minecraft show CrossAgent achieves state-of-the-art performance, significantly outperforming fixed-action baselines with superior generalization and efficiency in long-horizon reasoning.

Conclusion: CrossAgent demonstrates that unified agents mastering heterogeneous action spaces with autonomous interface selection can overcome the limitations of rigid, single-action-space agents, enabling more adaptive and efficient AI systems.
...
TL;DR: MoLKV improves upon MoLE by introducing context-aware expert selection through key-value pairs instead of token-id-based expert activation.
...
Method: Proposes Mixture of Lookup Key-Value Experts (MoLKV) where each expert is structured as a key-value pair. Input-derived queries interact with cached key-value experts from the current sequence to generate context-aware expert outputs.

Result: MoLKV achieves significantly lower validation loss in small-scale evaluations compared to MoLE.

Conclusion: MoLKV addresses MoLE's context-awareness limitation while maintaining suitability for resource-constrained devices through efficient key-value expert architecture.
...
TL;DR: Transformers can generate valid chemical structures, but their internal mechanisms are poorly understood. This paper provides a mechanistic analysis of autoregressive transformers trained on drug-like molecules, revealing computational patterns at multiple abstraction levels.


<details>
  <summary>Details</summary>
Motivation: While transformers can generate valid and diverse chemical structures, there is limited understanding of the internal mechanisms that allow these models to capture the complex rules of molecular representation. The authors aim to uncover how these models learn and apply chemical knowledge.

Method: The authors perform a mechanistic analysis of autoregressive transformers trained on drug-like small molecules. They use sparse autoencoders (SAEs) to extract feature dictionaries associated with chemically relevant activation patterns, examining computational patterns across multiple levels of abstraction.

Result: The analysis identifies computational patterns consistent with both low-level syntactic parsing and more abstract chemical validity constraints. The extracted feature dictionaries reveal chemically meaningful activation patterns, and these mechanistic insights translate to improved predictive performance in various downstream tasks.

Conclusion: Mechanistic analysis of chemical transformers reveals structured computational patterns that capture molecular representation rules at multiple abstraction levels. These insights not only explain model behavior but also enhance practical predictive performance in chemical applications.
...
Method: Embed detailed three-phase grid information (phase voltages, unbalanced loads, BESS states) into heterogeneous graph nodes, then use diverse GNN architectures (GCN, GAT, GraphSAGE, GPS) for joint prediction. Incorporate physics-informed loss function with soft penalties for critical battery constraints (SoC and C-rate limits) during training.

Result: Experimental validation on CIGRE 18-bus distribution system shows low prediction errors: bus voltage MSEs of 6.92e-07 (GCN), 1.21e-06 (GAT), 3.29e-05 (GPS), and 9.04e-07 (SAGE). Physics-informed method ensures nearly zero SoC and C-rate constraint violations.

Conclusion: The embedding-loss approach achieves high accuracy in predicting network state variables while ensuring constraint-compliant dispatch, confirming its effectiveness for reliable BESS operation in three-phase unbalanced distribution grids.
...
TL;DR: Deep learning model predicts polymer solubility (wt%) from SMILES strings of polymers and solvents, achieving high accuracy on both simulation and experimental data.


<details>
  <summary>Details</summary>
Motivation: Understanding polymer solubility is crucial for applications like recycling, pharmaceutical formulation, and materials design, but traditional methods are limited. There's a need for scalable, accurate prediction methods that can handle diverse polymer-solvent combinations.

Method: Developed a deep learning framework using SMILES representations of polymers and solvents. Created dataset of 8,049 polymer-solvent pairs from molecular dynamics simulations. Combined molecular descriptors and fingerprints into 2,394 features per sample. Trained a fully connected neural network with six hidden layers using Adam optimizer and MSE loss.

Result: Model achieved strong agreement between predicted and actual solubility values. Demonstrated generalizability on 25 unseen polymer-solvent combinations from experimental Materials Genome Project data, maintaining high accuracy.
...
### [174] [Knowledge Diversion for Efficient Morphology Control and Policy Transfer](https://arxiv.org/abs/2512.09796)
*Fu Feng,Ruixiao Shi,Yucheng Xie,Jianlu Shen,Jing Wang,Xin Geng*

Main category: cs.LG

TL;DR: DivMorph: A modular training paradigm using knowledge diversion with SVD factorization and dynamic gating to create decomposable controllers for universal morphology control, achieving better cross-task generalization with reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based controllers for universal morphology control have high computational costs and limited cross-task generalization, requiring training from scratch for each new task.

Method: DivMorph factorizes randomly initialized Transformer weights into factor units via SVD before training, uses dynamic soft gating with task/morphology embeddings to separate knowledge into shared "learngenes" and specific "tailors," enabling selective activation of relevant components.

Result: Achieves state-of-the-art performance with 3× improvement in sample efficiency for cross-task transfer and 17× reduction in model size for single-agent deployment compared to direct fine-tuning.

Conclusion: DivMorph enables scalable and efficient policy deployment while supporting effective policy transfer to novel tasks through knowledge disentanglement and modular architecture.
...
### [175] [Ariel-ML: Computing Parallelization with Embedded Rust for Neural Networks on Heterogeneous Multi-core Microcontrollers](https://arxiv.org/abs/2512.09800)
*Zhaolan Huang,Kaspar Schleiser,Gyungmin Myung,Emmanuel Baccelli*

Main category: cs.LG

TL;DR: Ariel-ML: A Rust-based toolkit for parallel TinyML inference on multi-core MCUs, outperforming prior art in latency while maintaining competitive memory footprint.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the gap in Rust embedded software for automated parallelization of TinyML inference on multi-core MCUs. With the shift from single-core to multi-core MCU architectures and increasing adoption of Rust in embedded systems, there's a need for tools that can leverage multi-core capabilities for edge AI deployment while supporting field retrofitting of neural networks.

Method: The authors introduce Ariel-ML, a toolkit combining a generic TinyML pipeline with an embedded Rust software platform. The system is designed to automatically parallelize inference computation across multi-core 32-bit microcontrollers (Arm Cortex-M, RISC-V, ESP-32) and supports arbitrary TinyML models. The implementation is fully open source.

Result: Benchmarks using various TinyML models show that Ariel-ML outperforms prior art in inference latency as expected. Compared to existing C/C++ toolkits, it achieves comparable memory footprints. The toolkit demonstrates effective utilization of multi-core capabilities for edge AI applications.

Conclusion: Ariel-ML successfully fills the gap for Rust-based TinyML inference on multi-core MCUs, providing a useful foundation for both TinyML practitioners and resource-constrained embedded Rust developers. It enables efficient parallel execution of neural networks while maintaining competitive resource usage.
...
### [176] [Incorporating Fairness in Neighborhood Graphs for Fair Spectral Clustering](https://arxiv.org/abs/2512.09810)
*Adithya K Moorthy,V Vijaya Saradhi,Bhanu Prasad*

Main category: cs.LG

TL;DR: Novel fair graph construction methods (fair kNN and fair epsilon-neighborhood) that enforce demographic parity during graph formation to achieve equitable spectral clustering outcomes without modifying the clustering algorithm itself.


<details>
  <summary>Details</summary>
Motivation: Traditional graph clustering methods perpetuate bias through unfair graph constructions that underrepresent some groups, leading to biased clustering results. Current approaches propagate edge-based disparate impact on sensitive groups.

Method: Introduces fair k-nearest neighbor (kNN) and fair epsilon-neighborhood graph construction methods that incorporate fairness constraints during neighborhood selection. These methods enforce proportional representation of sensitive features in local graph structure while maintaining geometric consistency.

Result: The fair graph construction methods surpass current baselines in graph clustering tasks across three synthetic datasets, seven real-world tabular datasets, and three real-world image datasets. Topological fairness in graph construction leads to fairer spectral clustering results.

Conclusion: Topological fairness in graph construction is essential for achieving equitable clustering outcomes. Fair graph construction inherently facilitates fairer spectral clustering without needing to modify the clustering algorithm itself, addressing a critical gap in pre-processing for fair spectral clustering.
...
### [177] [Predicting the Containment Time of California Wildfires Using Machine Learning](https://arxiv.org/abs/2512.09835)
*Shashank Bhardwaj*

Main category: cs.LG

TL;DR: This research develops machine learning models to predict wildfire containment duration in California as a regression task, comparing XGBoost, Random Forest, and LSTM models using FRAP datasets.


<details>
  <summary>Details</summary>
Motivation: California's worsening wildfire seasons overwhelm emergency response teams, causing massive destruction. There's a growing need for accurate predictions to assist with resource allocation for wildfire managers, addressing a gap in literature that typically focuses on risk/spread prediction or uses broader categorical duration predictions rather than continuous measures.

Method: Built machine learning models treating wildfire duration prediction as a regression task. Combined three publicly available datasets from California Department of Forestry and Fire Protection's FRAP program. Compared baseline ensemble regressor, Random Forest, XGBoost, and LSTM neural network models.

Result: XGBoost slightly outperformed Random Forest due to better handling of static features. LSTM performed worse than ensemble models because the dataset lacked temporal features. The study demonstrates that depending on feature availability, appropriate models can be selected for accurate duration prediction.

Conclusion: Wildfire managers can select appropriate machine learning models based on feature availability to accurately predict containment duration and allocate resources effectively. The regression approach provides more detailed and precise forecasts than broader categorical predictions used in prior work.
...
### [178] [Conformal Bandits: Bringing statistical validity and reward efficiency to the small-gap regime](https://arxiv.org/abs/2512.09850)
*Simone Cuonzo,Nina Deliu*

Main category: cs.LG

TL;DR: Conformal Bandits integrates Conformal Prediction into bandit problems to provide finite-time statistical guarantees alongside regret minimization, particularly effective in small-gap regimes like portfolio allocation.


<details>
  <summary>Details</summary>
Motivation: Traditional bandit strategies (Thompson Sampling, UCB) rely on distributional assumptions or asymptotic guarantees, focus mainly on regret, and lack statistical properties. There's a need to bridge regret-minimizing policies with statistical guarantees in finite time.

Method: Integrates Conformal Prediction (CP) into bandit framework to provide finite-time prediction coverage guarantees. For portfolio allocation application, incorporates hidden Markov models to capture regime-switching behavior of financial markets, enhancing exploration-exploitation trade-off.

Result: Demonstrates practical advantage in small-gap settings (like portfolio allocation) where classical policies fail to achieve optimal regret bounds. Shows improved regret performance while achieving nominal coverage guarantees where classical UCB policies fail. Integration with hidden Markov models leads to higher risk-adjusted returns while preserving coverage guarantees.

Conclusion: Conformal Bandits successfully bridges regret minimization with statistical guarantees, offering a practical framework that outperforms classical bandit policies in small-gap regimes while providing finite-time coverage guarantees, particularly valuable in applications like portfolio allocation.
...
### [179] [HPM-KD: Hierarchical Progressive Multi-Teacher Framework for Knowledge Distillation and Efficient Model Compression](https://arxiv.org/abs/2512.09886)
*Gustavo Coelho Haase,Paulo Henrique Dourado da Silva*

Main category: cs.LG

TL;DR: HPM-KD is an automated knowledge distillation framework that eliminates manual hyperparameter tuning, handles large teacher-small student gaps via progressive distillation, coordinates multi-teacher ensembles, and improves computational efficiency through parallel processing.


<details>
  <summary>Details</summary>
Motivation: Knowledge distillation has limitations: sensitivity to hyperparameters requiring manual tuning, capacity gaps between large teachers and small students, poor coordination in multi-teacher scenarios, and inefficient computational resource usage.

Method: HPM-KD integrates six components: 1) Adaptive Configuration Manager via meta-learning, 2) Progressive Distillation Chain with automatic intermediate models, 3) Attention-Weighted Multi-Teacher Ensemble with dynamic per-sample weights, 4) Meta-Learned Temperature Scheduler, 5) Parallel Processing Pipeline with load balancing, and 6) Shared Optimization Memory for cross-experiment reuse.

Result: Achieves 10x-15x compression while maintaining 85% accuracy retention, eliminates manual tuning, reduces training time by 30-40% via parallelization, and ablation studies show independent contributions of each component (0.10-0.98 pp improvements).

Conclusion: HPM-KD provides a comprehensive solution to key knowledge distillation challenges, offering automated tuning, efficient compression, and computational optimization, available as part of the open-source DeepBridge library.
...
### [180] [Analysis of Dirichlet Energies as Over-smoothing Measures](https://arxiv.org/abs/2512.09890)
*Anna Bison,Alessandro Sperduti*

Main category: cs.LG

TL;DR: Analysis shows normalized graph Laplacian Dirichlet energy fails as node-similarity measure, highlighting critical distinctions between unnormalized vs normalized Laplacian energies for GNN over-smoothing monitoring.


<details>
  <summary>Details</summary>
Motivation: To resolve ambiguities in monitoring GNN dynamics by clarifying distinctions between two commonly used over-smoothing measures: Dirichlet energies from unnormalized vs normalized graph Laplacians.

Method: Analyze axiomatic properties of both functionals, demonstrate normalized Laplacian energy fails Rusch et al.'s node-similarity measure definition, formalize fundamental spectral properties of both definitions.

Result: Normalized graph Laplacian Dirichlet energy does not satisfy axiomatic node-similarity measure criteria, revealing critical spectral distinctions that affect compatibility with GNN architectures.

Conclusion: Proper selection between unnormalized and normalized Laplacian energies is essential for spectrally compatible over-smoothing monitoring in GNNs, resolving previous ambiguities in dynamics tracking.
...
### [181] [Provably Learning from Modern Language Models via Low Logit Rank](https://arxiv.org/abs/2512.09892)
*Noah Golowich,Allen Liu,Abhishek Shetty*

Main category: cs.LG

TL;DR: The paper presents an efficient algorithm for learning approximately low logit rank models from queries, providing the first end-to-end learning guarantee for generative models that plausibly capture modern language models.


<details>
  <summary>Details</summary>
Motivation: Modern language models are complex, but recent work shows they exhibit approximately low logit rank structure. The authors want to understand how this structure can be exploited algorithmically for provable learning guarantees, especially since low logit rank models can encode hard-to-learn distributions like noisy parities.

Method: The authors study a query learning model with logit queries that reflects the access model for common APIs. They develop an efficient algorithm for learning any approximately low logit rank model from queries, leveraging the low rank structure observed in modern language models.

Result: The main result is an efficient algorithm for learning any approximately low logit rank model from queries. This provides the first end-to-end learning guarantee for a generative model that plausibly captures the behavior of modern language models.

Conclusion: The paper demonstrates that the empirically observed low logit rank structure in modern language models can be exploited algorithmically to obtain provable learning guarantees, bridging the gap between empirical observations and theoretical learning theory.
...
### [182] [STACHE: Local Black-Box Explanations for Reinforcement Learning Policies](https://arxiv.org/abs/2512.09909)
*Andrew Elashkin,Orna Grumberg*

Main category: cs.LG

TL;DR: STACHE is a framework for generating local, black-box explanations of RL agent actions using Robustness Regions and Minimal Counterfactuals in discrete Markov games.
...
Method: STACHE produces Composite Explanations with two components: (1) Robustness Region - connected neighborhood where action remains invariant, and (2) Minimal Counterfactuals - smallest state perturbations that change the decision. Uses exact, search-based algorithm exploiting factored state spaces to avoid surrogate model fidelity gaps.

Result: Empirical validation on Gymnasium environments shows the framework explains policy actions and captures evolution of policy logic during training - from erratic to optimized strategies - providing insights into agent sensitivity and decision boundaries.

Conclusion: STACHE provides a comprehensive framework for generating actionable explanations of RL agent decisions, addressing the critical need for debugging and verification tools in safety-critical applications.
...
TL;DR: PLM architecture affects antibody feature capture; antibody-specific models naturally focus on CDRs while general protein models need explicit CDR training.


<details>
  <summary>Details</summary>
Motivation: To systematically investigate how architectural choices in protein language models influence their ability to comprehend antibody-specific biological properties and functions, as this relationship remains unexplored despite advances in PLMs.

Method: Evaluated three state-of-the-art PLMs (AntiBERTa, BioBERT, ESM2) against GPT-2 baseline on antibody target specificity prediction tasks, using attention attribution analysis to examine feature capture patterns.

Result: All PLMs achieved high classification accuracy but exhibited distinct biases in capturing biological features (V gene usage, somatic hypermutation patterns, isotype information). Antibody-specific models like AntiBERTa naturally learned to focus on CDRs, while general protein models benefited significantly from explicit CDR-focused training strategies.

Conclusion: The findings provide insights into the relationship between model architecture and biological feature extraction, offering valuable guidance for future PLM development in computational antibody design, particularly regarding the importance of domain-specific architectural choices.
...
### [184] [FALCON: Few-step Accurate Likelihoods for Continuous Flows](https://arxiv.org/abs/2512.09914)
...
<details>
  <summary>Details</summary>
Motivation: Current Boltzmann Generators using continuous normalizing flows (CNFs) have extremely costly likelihood calculations requiring thousands of function evaluations per sample, severely limiting their adoption for molecular state sampling.
...
Result: FALCON outperforms state-of-the-art normalizing flow models for molecular Boltzmann sampling and is two orders of magnitude faster than equivalently performing CNF models.

Conclusion: FALCON addresses the computational bottleneck of CNFs in Boltzmann Generators, making scalable molecular state sampling practical by enabling efficient few-step sampling with accurate likelihoods.
...
### [185] [Closing the Train-Test Gap in World Models for Gradient-Based Planning](https://arxiv.org/abs/2512.09929)
*Arjun Parthasarathy,Nimit Kalra,Rohun Agrawal,Yann LeCun,Oumayma Bounou,Pavel Izmailov,Micah Goldblum*

Main category: cs.LG

TL;DR: Improved world model training methods for gradient-based planning that close the train-test gap, achieving comparable performance to CEM in 10% of the time.


<details>
  <summary>Details</summary>
Motivation: World models with MPC can be trained offline on expert data but gradient-based planning has underperformed compared to other methods. There's a train-test gap where models are trained for next-state prediction but used for action sequence estimation at test time.

Method: Propose train-time data synthesis techniques to improve gradient-based planning with existing world models. Focus on closing the train-test gap by better aligning training objectives with inference-time usage.
...
Conclusion: Improved world model training methods enable efficient gradient-based planning that closes the performance gap with traditional methods while being significantly faster.
...
### [186] [SURA: Secure Unsourced Random Access](https://arxiv.org/abs/2512.09104)
*Mohammad Javad Ahmadi,Rafael F. Schaefer,H. Vincent Poor*

Main category: cs.IT

TL;DR: Wiretap-inspired physical layer security for unsourced random access (URA) that opportunistically uses existing feedback signals for encryption and artificial noise generation without modifying URA's structure.


<details>
  <summary>Details</summary>
Motivation: To provide confidential communication in URA systems while preserving their low-cost advantages (low delay, minimal signaling overhead) and without altering the original URA structure or adding overhead.

Method: Users generate secret keys and artificial noise from BS feedback signals (which are channel-dependent and private per user). Secure transmission involves: 1) encrypting data with secret key, 2) sending only LDPC parity bits of the secret key for legitimate receiver recovery, and 3) masking parity bits with artificial noise. Includes receiver algorithm design and leakage analysis.

Result: Simulation shows meaningful secrecy achieved in URA without structural modifications and with negligible impact on standard performance.

Conclusion: Physical layer security can be effectively integrated into URA systems using existing feedback mechanisms, providing confidentiality while maintaining URA's efficiency advantages.
...
### [187] [$t$-Fold $s$-Blocking Sets and $s$-Minimal Codes](https://arxiv.org/abs/2512.09457)
*Hao Chen,Xu Pan,Conghui Xie*

Main category: cs.IT

TL;DR: The paper establishes new lower bounds for t-fold s-blocking sets and projective s-minimal codes, generalizes the Ashikhmin-Barg condition to s-minimal codes, and constructs families of such codes with examples showing distinctions between minimal and 2-minimal codes.


<details>
  <summary>Details</summary>
Motivation: To advance the study of blocking sets and minimal codes in projective geometry and coding theory by removing restrictive conditions (like t ≤ q) from classical results and extending theoretical frameworks to s-minimal codes.

Method: Develops new lower bounds for t-fold s-blocking sets without the t ≤ q condition, extends results to projective s-minimal codes, proves relationships between (s+1)-minimal and s-minimal codes, generalizes the Ashikhmin-Barg condition, and constructs infinite families of s-minimal codes.

Result: Obtains stronger lower bounds than Beutelspacher's 1983 result, establishes lower bounds on lengths of projective s-minimal codes, proves (s+1)-minimal codes are s-minimal, generalizes Ashikhmin-Barg condition, constructs infinite families of s-minimal codes, and provides examples distinguishing minimal from 2-minimal codes.

Conclusion: The paper significantly advances the theory of blocking sets and minimal codes by removing limitations of classical results, establishing new bounds and relationships, and providing concrete constructions that both satisfy and violate generalized conditions, with practical examples showing the distinction between different levels of minimality.
...
### [188] [Binary and Non-Binary Self-Dual Sequences and Maximum Period Single-Track Gray Codes](https://arxiv.org/abs/2512.09655)
*Tuvi Etzion*

Main category: cs.IT

TL;DR: The paper analyzes binary and non-binary self-dual sequences, connects them to single-track Gray codes, and constructs infinite families of maximum period non-binary single-track Gray codes.


<details>
  <summary>Details</summary>
Motivation: Motivated by construction methods for single-track Gray codes, the authors aim to examine the structure and recursive constructions for binary and non-binary self-dual sequences, and explore their connections to maximum period single-track codes.

Method: The paper examines the structure and recursive constructions for binary and non-binary self-dual sequences, discusses feedback shift registers that generate such sequences, and explores connections between these sequences and maximum period single-track codes.

Result: The authors construct maximum period non-binary single-track Gray codes of length p^t and period p^{p^t}, which represent the first infinite families of maximum period codes presented in the literature.

Conclusion: The paper establishes connections between self-dual sequences and single-track Gray codes, provides recursive constructions, and presents the first infinite families of maximum period non-binary single-track Gray codes, advancing the theoretical understanding and practical applications of these sequences.
...
### [189] [Typical Solutions of Multi-User Linearly-Decomposable Distributed Computing](https://arxiv.org/abs/2512.09858)
*Ali Khalesi,Mohammad Reza Deylam Salehi*

Main category: cs.IT

TL;DR: The paper solves the multi-sender linearly-decomposable distributed computing problem using real-valued encoders/decoders, analyzing structural fidelity via thresholded graph edit distance between demand support and computed product's two-hop support.


<details>
  <summary>Details</summary>
Motivation: To address the distributed computing problem introduced by tessellated distributed computing, particularly focusing on real-world applications with real-valued data and structural fidelity requirements in network communications.

Method: Model real-valued encoders/decoders and demand matrices, assess structural fidelity using thresholded graph edit distance between demand support and two-hop support of computed product, analyze using spike-and-slab ensembles, Gaussian surrogates, and concentration bounds.

Result: Obtained closed-form second-moment risk, deterministic links between thresholded GED and norm error, Gaussian surrogate with sub-exponential tails revealing explicit recall lines, concentration of GED and operator-norm control, and compute-capped design with visible knee.

Conclusion: The approach successfully solves the distributed computing problem with structural fidelity guarantees and demonstrates practical applicability to aeronautical and satellite networks.
...
### [190] [Calibrated Trust in Dealing with LLM Hallucinations: A Qualitative Study](https://arxiv.org/abs/2512.09088)
*Adrian Ryser,Florian Allwein,Tim Schlippe*

Main category: cs.AI

TL;DR: Hallucinations in LLMs lead to context-sensitive trust calibration rather than blanket mistrust, with intuition identified as a new user-related trust factor for hallucination detection.


<details>
  <summary>Details</summary>
Motivation: To understand how LLM hallucinations influence user trust and interaction patterns in everyday use, moving beyond simple mistrust to nuanced trust calibration.

Method: Qualitative study with 192 participants, building on existing trust models (Lee & See, Afroogh et al., Blöbaum) to analyze trust factors and dynamics.

Result: Hallucinations lead to context-sensitive trust calibration; confirmed existing trust factors (expectancy, prior experience, expertise) and identified intuition as new factor; validated recursive trust calibration process with intuition added.

Conclusion: Practical recommendations for responsible LLM use based on nuanced understanding of trust dynamics, emphasizing the importance of intuition in hallucination detection and context-sensitive trust calibration.
...
### [191] [AI TIPS 2.0: A Comprehensive Framework for Operationalizing AI Governance](https://arxiv.org/abs/2512.09114)
*Pamela Gupta*

Main category: cs.AI

TL;DR: AI TIPS 2.0 framework addresses three governance gaps: inadequate use-case risk assessment, lack of actionable controls in existing frameworks, and missing operationalization mechanisms for trustworthy AI at scale.


<details>
  <summary>Details</summary>
Motivation: Current AI governance frameworks fail to address critical challenges: inadequate risk assessment at use-case level (e.g., Humana lawsuit), lack of actionable controls in frameworks like ISO 42001 and NIST AI RMF, and absence of mechanisms to operationalize governance at scale across organizations.

Method: AI TIPS (Artificial Intelligence Trust-Integrated Pillars for Sustainability 2.0) - an updated comprehensive operational framework originally developed in 2019, four years before NIST's AI RMF, designed to directly address the identified governance gaps.

Result: The framework provides a systematic approach to embed trustworthy AI practices throughout the development lifecycle, enable quantitative compliance measurement, and offer role-appropriate visibility from boards to data scientists.

Conclusion: AI TIPS 2.0 offers a practical solution to bridge the gap between high-level AI governance principles and actionable implementation, addressing critical operational challenges that current frameworks fail to solve.
...
### [192] [A Categorical Analysis of Large Language Models and Why LLMs Circumvent the Symbol Grounding Problem](https://arxiv.org/abs/2512.09117)
*Luciano Floridi,Yiyang Jia,Fernando Tohmé*

Main category: cs.AI

TL;DR: LLMs circumvent rather than solve the symbol grounding problem through a categorical framework analyzing how humans and LLMs transform content into truth-evaluated propositions about possible worlds.


<details>
  <summary>Details</summary>
Motivation: To provide a formal, categorical analysis of how humans and LLMs process content into truth-evaluated propositions about possible worlds, specifically addressing the symbol grounding problem in AI and cognitive science.

Method: Develops a formal categorical framework that analyzes the transformation of content into truth-evaluated propositions about a state space of possible worlds W, comparing human and LLM approaches to this process.

Result: The analysis demonstrates that LLMs do not actually solve the symbol grounding problem but instead circumvent it through their approach to proposition formation and truth evaluation.

Conclusion: LLMs achieve their capabilities by bypassing rather than genuinely solving the fundamental symbol grounding problem, which has important implications for understanding their limitations and the nature of meaning in AI systems.
...
### [193] [SDialog: A Python Toolkit for End-to-End Agent Building, User Simulation, Dialog Generation, and Evaluation](https://arxiv.org/abs/2512.09142)
...
<details>
  <summary>Details</summary>
Motivation: To create a unified framework that addresses the fragmentation in conversational AI research by combining dialog generation, evaluation, and interpretability tools into a single system, enabling more systematic development and analysis of conversational agents.

Method: Built around a standardized Dialog representation, SDialog provides: 1) persona-driven multi-agent simulation with composable orchestration, 2) comprehensive evaluation with linguistic metrics, LLM-as-a-judge, and functional correctness validators, 3) mechanistic interpretability tools for activation inspection and steering, and 4) audio generation with full acoustic simulation including 3D room modeling.

Result: SDialog integrates with all major LLM backends through a unified API, enabling mixed-backend experiments and providing researchers with a comprehensive toolkit for building, benchmarking, and understanding conversational systems.

Conclusion: SDialog offers a dialog-centric architecture that couples generation, evaluation, and interpretability, enabling more systematic research and development of conversational AI systems through its open-source, MIT-licensed framework.
...
### [194] [Visual Categorization Across Minds and Models: Cognitive Analysis of Human Labeling and Neuro-Symbolic Integration](https://arxiv.org/abs/2512.09340)
*Chethana Prasad Kabgere*

Main category: cs.AI

TL;DR: This paper compares how humans and AI systems interpret ambiguous, low-resolution images, analyzing differences in perception, reasoning, and decision-making strategies between biological and artificial systems.


<details>
  <summary>Details</summary>
Motivation: To understand fundamental differences in how humans and AI systems perceive and interpret ambiguous visual stimuli, providing insights into perception, reasoning, and decision-making processes across biological and artificial systems.

Method: Comparative analysis of image labeling performance using low-resolution, perceptually degraded stimuli. Combines computational cognitive science approaches (ACT-R, Soar) with deep neural network analysis (Grad-CAM visualizations). Grounded in theoretical frameworks including Marr's tri-level hypothesis, Simon's bounded rationality, and Thagard's representation/emotion frameworks.

Result: Reveals key parallels and divergences between human and AI systems in representation, inference, and confidence calibration. Humans use analogical reasoning, shape-based recognition, and confidence modulation, while AI relies on feature-based processing.

Conclusion: The analysis supports developing neuro-symbolic architectures that unify structured symbolic reasoning with connectionist representations, informed by principles of embodiment, explainability, and cognitive alignment for more interpretable and cognitively grounded AI systems.
...
### [195] [Architectures for Building Agentic AI](https://arxiv.org/abs/2512.09458)
*Sławomir Nowaczyk*

Main category: cs.AI

TL;DR: AI reliability is an architectural property emerging from principled componentization, disciplined interfaces, and explicit control loops in agentic systems.


<details>
  <summary>Details</summary>
Motivation: To establish that reliability in agentic and generative AI systems is fundamentally an architectural concern rather than just a functional requirement, and to provide systematic design principles for building reliable AI agents.

Method: Proposes an architectural framework with principled componentization (goal manager, planner, tool-router, executor, memory, verifiers, safety monitor, telemetry), disciplined interfaces (schema-constrained, validated, least-privilege tool calls), and explicit control/assurance loops. Develops a taxonomy of agent types and analyzes reliability patterns for each.

Result: Provides a comprehensive architectural approach to AI reliability with design guidance on typed schemas, idempotency, permissioning, transactional semantics, memory provenance, runtime governance, and simulate-before-actuate safeguards.

Conclusion: Reliability in agentic AI systems can be systematically engineered through proper architectural design patterns, componentization, and control mechanisms, enabling more trustworthy and dependable AI systems across various agent types and deployment scenarios.
...
### [196] [Toward Closed-loop Molecular Discovery via Language Model, Property Alignment and Strategic Search](https://arxiv.org/abs/2512.09566)
*Junkai Ji,Zhangfan Yang,Dong Xu,Ruibin Bai,Jianqiang Li,Tingjun Hou,Zexuan Zhu*

Main category: cs.AI

TL;DR: Trio is a molecular generation framework combining fragment-based language modeling, reinforcement learning, and Monte Carlo tree search for targeted drug design with improved binding affinity, drug-likeness, and synthetic accessibility.


<details>
  <summary>Details</summary>
Motivation: Traditional drug discovery methods (high-throughput screening, docking) are inefficient and limited. Current generative models have poor generalization, limited interpretability, and focus too much on binding affinity while neglecting other important pharmacological properties.

Method: Trio integrates three components: fragment-based molecular language modeling, reinforcement learning, and Monte Carlo tree search. This enables context-aware fragment assembly, enforces physicochemical/synthetic feasibility, and balances exploration of novel chemotypes with exploitation of promising intermediates.

Result: Trio outperforms state-of-the-art approaches with improved binding affinity (+7.85%), drug-likeness (+11.10%), and synthetic accessibility (+12.05%), while expanding molecular diversity more than fourfold.

Conclusion: Trio provides an effective and interpretable closed-loop framework for targeted molecular design that addresses limitations of current generative models and produces chemically valid, pharmacologically enhanced ligands.
...
### [197] [An End-to-end Planning Framework with Agentic LLMs and PDDL](https://arxiv.org/abs/2512.09629)
*Emanuele La Malfa,Ping Zhu,Samuele Marro,Sara Bernardini,Michael Wooldridge*

Main category: cs.AI

TL;DR: End-to-end LLM-powered framework that converts natural language specifications into PDDL planning models, refines them through verification agents, generates plans with external planners, and translates results back to natural language.


<details>
  <summary>Details</summary>
Motivation: To create an automated planning system that can handle human natural language specifications without human intervention, addressing common planning requirements like time constraints, optimality, and resolving ambiguities/contradictions in specifications.

Method: Uses an orchestrator with LLM-powered agents to convert natural language to PDDL, iteratively refine domain/problem definitions, validate with external verifiers, generate plans with planning engines, and translate results back to natural language.

Result: Demonstrated flexibility and effectiveness across various domains including Google NaturalPlan benchmark, PlanBench, Blocksworld, and Tower of Hanoi problems where LLMs typically struggle. Framework integrates with multiple PDDL engines (Fast Downward, LPG, POPF, VAL, uVAL).

Conclusion: Represents a significant step toward end-to-end planning aided by LLMs, showing that LLM-powered systems can handle complex planning tasks without human intervention while maintaining correctness through verification.
...
### [198] [Gaussian Process Aggregation for Root-Parallel Monte Carlo Tree Search with Continuous Actions](https://arxiv.org/abs/2512.09727)
*Junlin Xiao,Victor-Alexandru Darvariu,Bruno Lacerda,Nick Hawes*

Main category: cs.AI

TL;DR: Gaussian Process Regression improves Monte Carlo Tree Search in continuous action spaces by aggregating statistics across parallel threads more effectively.


<details>
  <summary>Details</summary>
Motivation: In continuous action spaces, how to best aggregate statistics from different parallel threads in Monte Carlo Tree Search is an important but underexplored problem, especially when wall clock time is limited but best performance is desired.

Method: The paper introduces a method that uses Gaussian Process Regression to obtain value estimates for promising actions that were not trialed in the environment, improving aggregation of statistics across parallel threads.

Result: Systematic evaluation across 6 different domains shows that the approach outperforms existing aggregation strategies while requiring only a modest increase in inference time.

Conclusion: Gaussian Process Regression is an effective method for improving Monte Carlo Tree Search performance in continuous action spaces by enabling better aggregation of statistics from parallel threads.
...
### [199] [Analyzing Planner Design Trade-offs for MAPF under Realistic Simulation](https://arxiv.org/abs/2512.09736)
*Jingtian Yan,Zhifei Li,William Kang,Stephen F. Smith,Jiaoyang Li*

Main category: cs.AI

TL;DR: This paper analyzes how key MAPF planner design choices affect performance in realistic execution settings, examining solution optimality, kinodynamic modeling accuracy, and their interactions.


<details>
  <summary>Details</summary>
Motivation: There's a gap between simplified MAPF benchmarks and real-world performance. While recent frameworks like SMART enable realistic evaluation with kinodynamic modeling, it's unclear how planner design choices actually impact practical deployment in industrial settings.
...
Result: The paper provides empirical insights into how design choices affect performance in realistic settings, though specific quantitative results aren't detailed in the abstract. The analysis reveals important relationships between theoretical optimality and practical execution.

Conclusion: Highlights open challenges and research directions to guide the MAPF community toward practical, real-world deployment, emphasizing the need to bridge the gap between algorithmic benchmarks and actual performance in industrial applications.
...
### [200] [RIFT: A Scalable Methodology for LLM Accelerator Fault Assessment using Reinforcement Learning](https://arxiv.org/abs/2512.09829)
*Khurram Khalil,Muhammad Mahad Khaliq,Khaza Anuarul Hoque*

Main category: cs.AI

TL;DR: RIFT uses reinforcement learning to automate discovery of minimal high-impact fault scenarios for AI accelerator fault assessment, achieving 2.2× speedup over evolutionary methods and 99% test reduction while improving coverage.


<details>
  <summary>Details</summary>
Motivation: Traditional fault assessment methods for modern AI accelerators face prohibitive computational costs and poor coverage of critical failure modes due to massive scale.

Method: RIFT transforms fault search into sequential decision-making problem, combining hybrid sensitivity analysis for search space pruning with reinforcement learning to generate minimal high-impact test suites.

Result: Evaluated on billion-parameter LLM workloads using NVIDIA A100 GPUs: 2.2× speedup over evolutionary methods, 99% test reduction vs random fault injection, superior fault coverage, and 12.8× improvement in cost-effectiveness for selective error correction.

Conclusion: RIFT provides scalable fault assessment framework that generates actionable verification artifacts for commercial RTL workflows and enables intelligent hardware protection strategies.
...
### [201] [Interpretation as Linear Transformation: A Cognitive-Geometric Model of Belief and Meaning](https://arxiv.org/abs/2512.09831)
*Chainarong Amornbunchornvej*

Main category: cs.AI

TL;DR: A geometric framework models belief transmission between cognitively heterogeneous agents using vector spaces and linear maps, showing how beliefs survive or die based on structural compatibility rather than shared information.


<details>
  <summary>Details</summary>
Motivation: To develop a unified geometric framework for understanding belief transmission, influence, and meaning preservation across cognitively diverse agents (both human and artificial), addressing limitations of traditional approaches based on shared information or rationality.

Method: Each agent is represented by a personalized value space (vector space) encoding internal dimensions for interpreting meaning. Beliefs are formalized as structured vectors ("abstract beings") transmitted via linear interpretation maps. Belief survival depends on avoiding null spaces of these maps.

Result: The framework explains belief distortion, motivational drift, counterfactual evaluation, and limits of mutual understanding as algebraic constraints. Key result: "No-Null-Space Leadership Condition" characterizes leadership as representational reachability rather than persuasion/authority.

Conclusion: The cognitive-geometric perspective grounds meaning preservation in structural compatibility rather than shared information or rationality, clarifying epistemic boundaries of influence and providing a general foundation for analyzing belief dynamics across heterogeneous agents.
...
### [202] [Comparing AI Agents to Cybersecurity Professionals in Real-World Penetration Testing](https://arxiv.org/abs/2512.09882)
...
TL;DR: AI agent ARTEMIS outperformed 9 of 10 human cybersecurity professionals in live enterprise testing, finding 9 valid vulnerabilities with 82% accuracy at lower cost, but struggles with false positives and GUI tasks.
...
Method: Evaluated 10 human cybersecurity professionals, 6 existing AI agents, and ARTEMIS (a new multi-agent framework with dynamic prompt generation, arbitrary sub-agents, and automatic vulnerability triaging) on a large university network with ~8,000 hosts across 12 subnets.

Result: ARTEMIS placed second overall, discovered 9 valid vulnerabilities with 82% valid submission rate, outperforming 9 of 10 human participants. AI agents showed advantages in systematic enumeration, parallel exploitation, and cost ($18/hour vs $60/hour for humans), but had higher false-positive rates and struggled with GUI-based tasks.

Conclusion: AI agents like ARTEMIS demonstrate technical sophistication comparable to top human cybersecurity professionals and offer cost advantages, but need improvement in false-positive reduction and GUI interaction capabilities to become fully practical alternatives.
...
### [203] [Human-in-the-Loop and AI: Crowdsourcing Metadata Vocabulary for Materials Science](https://arxiv.org/abs/2512.09895)
...
TL;DR: MatSci-YAMZ is an AI-human-in-the-loop platform that accelerates metadata vocabulary development through collaborative AI refinement and crowdsourcing, demonstrated in materials science.
...
Method: Integrated AI with human-in-the-loop (HILT) including crowdsourcing; conducted proof-of-concept with 6 materials science researchers who contributed term definitions and provided examples for AI-definition refinement.

Result: Successfully generated 19 AI definitions through iterative feedback loops; demonstrated feasibility of AI-HILT refinement for vocabulary development with alignment to FAIR/open-science principles.

Conclusion: MatSci-YAMZ model enhances semantic transparency, reduces consensus-building time, and shows potential for scalable cross-domain metadata vocabulary development.
...
### [204] [SCOPE: Language Models as One-Time Teacher for Hierarchical Planning in Text Environments](https://arxiv.org/abs/2512.09897)
*Haoye Lu,Pavan Seshadri,Kaheer Suleman*

Main category: cs.AI

TL;DR: SCOPE is a one-shot hierarchical planner that uses LLM-generated subgoals only at initialization to pretrain a lightweight student model, achieving better performance and much faster inference than LLM-based approaches.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based planning approaches are computationally expensive (require repeated LLM queries during training/inference) and use fixed LLM parameters that don't adapt to target tasks.

Method: SCOPE uses LLM-generated subgoals only at initialization to pretrain a lightweight student model, deriving subgoals directly from example trajectories rather than repeated LLM prompting.

Result: Achieves 0.56 success rate vs 0.52 for ADaPT, while reducing inference time from 164.4 seconds to just 3.0 seconds on TextCraft environment.

Conclusion: LLM-generated subgoals can serve as strong starting points for hierarchical planning, and one-shot distillation enables efficient planning with minimal LLM usage, though at the cost of reduced explainability.
...
### [205] [Bayesian Networks, Markov Networks, Moralisation, Triangulation: a Categorical Perspective](https://arxiv.org/abs/2512.09908)
*Antonio Lorenzin,Fabio Zanasi*

Main category: cs.AI

TL;DR: A categorical framework models moralisation and triangulation as functors between Bayesian and Markov networks, with variable elimination reinterpreted as a functor splitting triangulation into syntactic and semantic parts.


<details>
  <summary>Details</summary>
Motivation: To provide a categorical framework for understanding transformations between Bayesian networks (directed models) and Markov networks (undirected models), specifically moralisation and triangulation, and to distinguish between syntactic and semantic aspects of these transformations.

Method: Represent Bayesian and Markov networks as functors from syntax to semantics domains. Model moralisation and triangulation as functors between categories of these networks. Define transformations inductively via functor pre-composition. Reinterpret variable elimination as a functor that splits triangulation into syntactic and semantic components.

Result: Developed a categorical framework where moralisation is fully syntactic while triangulation relies on semantics. Showed that variable elimination can be viewed as a functor that separates triangulation into purely syntactic and purely semantic parts, highlighting the distinction between these two types of modifications.

Conclusion: The paper introduces a functorial perspective into probabilistic graphical models theory, providing a structured way to understand transformations between different graphical model representations and emphasizing the important distinction between syntactic and semantic modifications in these transformations.
...
### [206] [Agreement Disagreement Guided Knowledge Transfer for Cross-Scene Hyperspectral Imaging](https://arxiv.org/abs/2512.08990)
*Lu Huo,Haimin Zhang,Min Xu*

Main category: eess.IV

TL;DR: ADGKT framework integrates agreement and disagreement mechanisms to address gradient conflicts and dominant gradients in cross-scene hyperspectral imaging knowledge transfer, improving feature capture and transfer robustness.


<details>
  <summary>Details</summary>
Motivation: Existing cross-scene HSI transfer methods overlook gradient conflicts and dominant gradients during shared parameter optimization, and fail to simultaneously capture both agreement and disagreement information, missing diverse target scene patterns.

Method: Proposes Agreement Disagreement Guided Knowledge Transfer (ADGKT) framework with two components: 1) Agreement component includes GradVac for gradient direction alignment to mitigate conflicts and LogitNorm for logit magnitude regulation to prevent gradient domination; 2) Disagreement component includes Disagreement Restriction (DiR) and ensemble strategy to capture diverse predictive target features and preserve critical information.

Result: Extensive experiments demonstrate the effectiveness and superiority of the proposed method in achieving robust and balanced knowledge transfer across heterogeneous HSI scenes.

Conclusion: The ADGKT framework successfully addresses gradient optimization challenges in cross-scene HSI transfer by integrating both agreement and disagreement mechanisms, leading to improved knowledge transfer performance across diverse hyperspectral imaging scenarios.
...
### [207] [Enhanced Chest Disease Classification Using an Improved CheXNet Framework with EfficientNetV2-M and Optimization-Driven Learning](https://arxiv.org/abs/2512.08992)
*Ali M. Bahram,Saman Muhammad Omer,Hardi M. Mohammed,Sirwan Abdolwahed Aula*

Main category: eess.IV

TL;DR: Improved chest X-ray classification using EfficientNetV2-M with advanced training techniques achieves 96.45% accuracy for detecting 5 key diseases including COVID-19 and tuberculosis.


<details>
  <summary>Details</summary>
Motivation: Addressing the shortage of radiologists in resource-limited settings and improving upon CheXNet's computational inefficiency and poor single-label classification performance for chest X-ray interpretation.

Method: Proposed framework uses EfficientNetV2-M backbone with advanced training techniques: Automatic Mixed Precision training, AdamW optimizer, Cosine Annealing learning rate scheduling, and Exponential Moving Average regularization. Dataset includes 18,080 chest X-ray images across 5 disease categories from three authoritative sources.

Result: Achieved mean test accuracy of 96.45% (vs 95.30% baseline, p<0.001) and macro-averaged F1-score of 91.08% (p<0.001). Near-perfect detection for critical diseases: COVID-19 (99.95% accuracy) and Tuberculosis (99.97% accuracy). Despite 6.8× more parameters, training time reduced by 11.4% and performance stability increased by 22.7%.

Conclusion: The framework serves as an effective decision-support tool for pandemic response, tuberculosis screening, and routine thoracic disease assessment in various healthcare settings, particularly where radiologist resources are limited.
...
### [208] [DermETAS-SNA LLM: A Dermatology Focused Evolutionary Transformer Architecture Search with StackNet Augmented LLM Assistant](https://arxiv.org/abs/2512.08998)
*Nitya Phani Santosh Oruganty,Keerthi Vemula Murali,Chun-Kit Ngan,Paulo Bandeira Pinho*

Main category: eess.IV

TL;DR: DermETAS-SNA LLM Assistant combines evolutionary transformer architecture search with StackNet-augmented LLM for skin disease classification and medical explanation generation, outperforming SkinGPT-4 by 16.06% in F1-score.


<details>
  <summary>Details</summary>
Motivation: To create an AI assistant that can both accurately classify skin diseases and provide medically informed explanations to facilitate clinician-patient communication in dermatology.

Method: 1) ETAS framework on SKINCON dataset to optimize ViT for dermatological features, fine-tuned on DermNet's 23 disease categories; 2) StackNet architecture integrating multiple binary ViT classifiers; 3) RAG pipeline with Google Gemini 2.5 Pro LLM for diagnostic explanations; 4) Extensive evaluation on 23 skin diseases; 5) Domain-expert evaluation by 8 medical doctors.

Result: Achieved 56.30% overall F1-score, surpassing SkinGPT-4 (48.51%) by 16.06% improvement. Domain-expert evaluation showed 92% agreement rate with AI assistant assessments. Created proof-of-concept prototype for clinical/educational applications.

Conclusion: The DermETAS-SNA LLM Assistant demonstrates superior performance in skin disease classification and generates clinically useful explanations, showing practical feasibility for real-world dermatology applications.
...
### [209] [Causal Attribution of Model Performance Gaps in Medical Imaging Under Distribution Shifts](https://arxiv.org/abs/2512.09094)
*Pedro M. Gordaliza,Nataliia Molchanova,Jaume Banus,Thomas Sanchez,Meritxell Bach Cuadra*

Main category: eess.IV

TL;DR: Causal attribution framework for medical image segmentation performance drops due to distribution shifts, using Shapley values to quantify contributions of acquisition protocols vs annotation variability.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for medical image segmentation suffer significant performance drops due to distribution shifts, but the causal mechanisms behind these drops remain poorly understood. Need to understand how acquisition protocols and annotation variability independently contribute to performance degradation.

Method: Extend causal attribution frameworks to high-dimensional segmentation tasks. Model the data-generating process through a causal graph and employ Shapley values to fairly attribute performance changes to individual mechanisms. Address unique challenges: high-dimensional outputs, limited samples, and complex mechanism interactions.
...
Conclusion: This mechanism-specific quantification enables practitioners to prioritize targeted interventions based on deployment context, allowing for more effective mitigation strategies for distribution shifts in medical image segmentation.
...
### [210] [SITP: A High-Reliability Semantic Information Transport Protocol Without Retransmission for Semantic Communication](https://arxiv.org/abs/2512.09291)
*Yunhao Wang,Shuai Ma,Youlong Wu,Guangming Shi,Xiang Cheng,Yuxuan Liu,Pengfei He*

Main category: eess.IV

TL;DR: SITP is a new transport protocol for 6G that achieves TCP-level reliability with UDP-level latency by verifying only packet headers while keeping corrupted payloads for semantic decoding, plus cross-layer modeling and cross-image feature interleaving for burst loss mitigation.


<details>
  <summary>Details</summary>
Motivation: 6G networks need high reliability and low latency, but conventional transport protocols (TCP/UDP) fail to meet semantic robustness requirements - TCP has high latency for bit-level reliability, UDP lacks reliability.

Method: 1) Semantic Information Transport Protocol (SITP) that verifies only packet headers while retaining potentially corrupted payloads for semantic decoding; 2) Cross-layer analytical model linking SNR to packet-loss probability across all layers; 3) Cross-image feature interleaving mechanism to redistribute semantic features across multiple correlated images to mitigate burst losses.
...
Conclusion: SITP successfully bridges the gap between TCP reliability and UDP latency for semantic communication in 6G networks, with cross-layer modeling providing theoretical foundation and cross-image interleaving enhancing robustness against burst losses.
...
### [211] [NOC4SC: A Bandwidth-Efficient Multi-User Semantic Communication Framework for Interference-Resilient Transmission](https://arxiv.org/abs/2512.09356)
*Yunhao Wang,Shuai Ma,Pengfei He,Dahua Gao,Guangming Shi,Xiang Cheng*

Main category: eess.IV

TL;DR: NOC4SC is a semantic communication framework using Swin Transformer and adaptive modulation to enable simultaneous same-frequency transmission without spectrum spreading, effectively reducing inter-user interference in massive access scenarios.


<details>
  <summary>Details</summary>
Motivation: Current wireless networks face unprecedented demands for massive user access, where inter-user interference becomes a critical challenge to maintaining high QoS in multi-user communication systems.
...
Result: Extensive experiments show NOC4SC achieves comparable performance to DeepJSCC-PNOMA and outperforms other multi-user semantic communication baseline methods.

Conclusion: NOC4SC provides a bandwidth-efficient semantic communication paradigm that enables simultaneous same-frequency transmission without spectrum spreading while effectively mitigating inter-user interference through adaptive modulation and orthogonal feature generation.
...
### [212] [QSMnet-INR: Single-Orientation Quantitative Susceptibility Mapping via Implicit Neural Representation in k-Space](https://arxiv.org/abs/2512.09425)
*Xuan Cai,Ruo-Mi Guo,Xiao-Wen Luo,Jing Zhao,Silun Wang,Tao Tan,Yue Liu,Hongbin Han,Mengting Liu*

Main category: eess.IV

TL;DR: QSMnet-INR: A deep physics-informed framework integrating Implicit Neural Representation to solve ill-posed single-orientation QSM inversion by completing cone-null regions in k-space.


<details>
  <summary>Details</summary>
Motivation: Single-orientation QSM inversion is highly ill-posed due to the dipole kernel's cone-null region in Fourier domain, causing streaking artifacts and structural loss. Current methods struggle with this limitation.

Method: Proposes QSMnet-INR integrating Implicit Neural Representation (INR) into k-space domain to continuously model multi-directional dipole responses and explicitly complete cone-null regions. Uses frequency-domain residual-weighted Dipole Loss for physical consistency. Combines 3D U-Net-based QSMnet backbone with INR module through alternating optimization for end-to-end joint training.

Result: Outperforms conventional and recent deep-learning approaches across multiple quantitative metrics on 2016 QSM Reconstruction Challenge, multi-orientation GRE dataset, and clinical data. Shows advantages in structural recovery within cone-null regions and artifact suppression. Ablation studies confirm complementary contributions of INR module and Dipole Loss.

Conclusion: QSMnet-INR effectively alleviates ill-posedness of single-orientation QSM without requiring multi-orientation acquisition, achieving high accuracy, robustness, and strong cross-scenario generalization with potential for clinical translation.
...
### [213] [PathCo-LatticE: Pathology-Constrained Lattice-Of Experts Framework for Fully-supervised Few-Shot Cardiac MRI Segmentation](https://arxiv.org/abs/2512.09779)
*Mohamed Elbayumi,Mohammed S. M. Elbaz*

Main category: eess.IV

TL;DR: PathCo-LatticE is a fully supervised few-shot learning framework for cardiac MRI segmentation that uses pathology-guided synthetic supervision instead of unlabeled data, achieving near-fully-supervised performance with minimal labeled anchors and strong zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot learning methods for cardiac MRI segmentation rely on semi-supervised techniques that are sensitive to domain shifts and validation bias, limiting their zero-shot generalizability to unseen data without target-domain fine-tuning.

Method: Three key components: 1) Virtual Patient Engine models continuous latent disease trajectories from sparse clinical anchors to synthesize fully labeled 3D cohorts; 2) Self-Reinforcing Interleaved Validation provides leakage-free online evaluation with progressively challenging synthetic samples; 3) dynamic Lattice-of-Experts organizes specialized networks in pathology-aware topology and activates relevant experts per input.

Result: Outperforms four state-of-the-art FSL methods by 4.2-11% Dice with only 7 labeled anchors, approaches fully supervised performance (within 1% Dice) with only 19 anchors, shows superior harmonization across four vendors and generalization to unseen pathologies in strict OOD setting.

Conclusion: PathCo-LatticE demonstrates that pathology-guided synthetic supervision can enable robust zero-shot generalization in few-shot learning for medical imaging, eliminating the need for real validation data and target-domain fine-tuning while achieving near-fully-supervised performance with minimal labeled data.
...
### [214] [New Algorithm for Structured OFDM Channel Estimation using Subgroup Duality](https://arxiv.org/abs/2512.08947)
*Demerson N. Gonçalves,João T. Dias*

Main category: eess.SP

TL;DR: Group-theoretic framework for structured OFDM channel estimation using subgroup nulling and annihilator constraints, with low-complexity energy concentration detection.


<details>
  <summary>Details</summary>
Motivation: To develop a structured channel estimation approach for OFDM systems that leverages mathematical group theory to impose constraints on channel impulse response, enabling improved estimation performance with lower complexity compared to conventional methods.

Method: Models OFDM subcarriers as cyclic group Z_N, shows that nulling a subgroup H ⊆ Z_N constrains channel impulse response to its annihilator H⊥ in dual domain. Proposes low-complexity estimator that detects structure by evaluating energy concentration across candidate annihilators.

Result: Simulations demonstrate consistent gains in mean squared error, bit error rate, and throughput compared with least-squares and linear minimum mean square error baselines. Achieves competitive performance with substantially lower complexity and preserved interpretability.

Conclusion: The group-theoretic framework provides an effective structured approach to OFDM channel estimation that offers performance improvements over conventional methods while maintaining low computational complexity and mathematical interpretability through subgroup-annihilator relationships.
...
### [215] [A New Particle Filter for Target Tracking in MIMO OFDM Integrated Sensing and Communications](https://arxiv.org/abs/2512.09098)
*Shixiong Wang,Wei Dai,Geoffrey Ye Li*

Main category: eess.SP

TL;DR: A new particle filtering framework for MIMO pulse-Doppler radar target tracking that replaces explicit likelihood models with tailored cost functions, addressing computational and statistical challenges while enabling ISAC capabilities in MIMO-OFDM systems.


<details>
  <summary>Details</summary>
Motivation: Three key challenges in particle filtering for MIMO pulse-Doppler radar target tracking: 1) lack of reliable likelihood models for raw radar data, 2) complications from augmenting nuisance parameters into state vectors, and 3) prohibitive computational burden of extracting measurements from snapshots. The paper is motivated by an optimization-centric interpretation of Bayes' rule.

Method: Proposes a new particle filtering framework that evaluates each hypothesized state using a tailored cost function instead of relying on explicit likelihood relations. The method is implemented in MIMO-OFDM systems to enable integrated sensing and communications (ISAC) capabilities.

Result: The framework yields substantial reductions in both running time and tracking error compared to existing schemes. Experiments suggest MIMO-OFDM with pulse-Doppler processing holds considerable promise for ISAC, particularly when utilizing wide bandwidth, extended on-target time, and large antenna aperture.

Conclusion: The proposed cost-function-based particle filtering approach effectively addresses long-standing obstacles in MIMO pulse-Doppler radar tracking while enabling practical ISAC implementation in modern communication infrastructure through MIMO-OFDM systems.
...
### [216] [A Hybrid Residue Floating Numerical Architecture for High Precision Arithmetic on FPGAs](https://arxiv.org/abs/2512.09155)
*Mostafa Darvishi*

Main category: eess.SP

TL;DR: HRFNA is a hybrid arithmetic system combining residue channels with floating scaling that achieves 2.1× throughput and 38-52% LUT reduction vs IEEE 754 single precision on FPGAs.


<details>
  <summary>Details</summary>
Motivation: Floating point arithmetic is expensive on FPGAs due to wide datapaths and normalization logic, creating need for alternative representations that preserve dynamic range at lower cost.

Method: Developed Hybrid Residue Floating Numerical Architecture (HRFNA) combining carry-free residue channels with lightweight floating point scaling factor. Created mathematical framework, bounded error normalization rules, and FPGA-optimized microarchitectures for modular multiplication, exponent management, and hybrid reconstruction.

Result: Implemented on Xilinx ZCU104 with Vitis simulation, RTL synthesis, and on-chip ILA traces confirming cycle-accurate correctness. Achieved over 2.1× throughput improvement and 38-52% LUT reduction compared to IEEE 754 single precision baselines while maintaining numerical stability across long iterative sequences.

Conclusion: HRFNA offers an efficient and scalable alternative to floating point computation on modern FPGA devices, demonstrating significant performance and resource improvements while preserving numerical stability.
...
### [217] [Secure Wireless Communication Using Distributed Coherent Transmission and Spatial Signal Decomposition](https://arxiv.org/abs/2512.09194)
*Anton Schlegel,Jason M/ Merlo,Samuel Wagner,John B. Lancaster,Jeffrey A. Nanzer*

Main category: eess.SP

TL;DR: A secure wireless communication method using coherent distributed transmission where signals are spatially decomposed between two transmitters, creating a confined secure region where only the intended receiver can recover information.


<details>
  <summary>Details</summary>
Motivation: To enhance wireless communication security by creating spatially confined secure regions where information is only recoverable at designated destinations, preventing eavesdropping at other locations.

Method: Uses a two-element coherent distributed phased array where each symbol is decomposed into two pseudo-random signal vectors transmitted separately from two transmitters. These signals combine coherently only at the intended receiver via distributed beamforming.

Result: Experimental validation shows low SER (0.0082) at the intended broadside location and high SER (>0.25) at all other locations, compared to traditional beamforming which achieves SER=0 everywhere.

Conclusion: The approach successfully creates spatially confined secure communication regions, offering enhanced security over traditional beamforming by making information unrecoverable outside the designated area.
...
### [218] [Flexible Reconfigurable Intelligent Surface-Aided Covert Communications in UAV Networks](https://arxiv.org/abs/2512.09714)
*Chong Huang,Gaojie Chen,Zhuoao Xu,Jing Zhu,Taisong Pan,Rahim Tafazolli,Wei Huang*

Main category: eess.SP

TL;DR: F-RIS enhanced UAV covert communication framework using DRL for joint optimization of trajectories, RIS parameters, and power allocation to maximize covert rate while meeting security constraints.


<details>
  <summary>Details</summary>
Motivation: UAVs are crucial for wireless networks but face security/privacy issues due to open communications. Traditional RIS lacks flexibility for curved surfaces, limiting covert communication enhancement in dynamic UAV environments.

Method: Proposed F-RIS (flexible RIS) with electromagnetic modeling, fitted reflection model, and DRL-based joint optimization of UAV trajectories, F-RIS reflection vectors, incident angles, and NOMA power allocation to maximize covert rate under constraints.

Result: Simulation shows proposed framework significantly outperforms traditional benchmarks, demonstrating F-RIS advantages in enhancing covert communication performance within UAV networks.

Conclusion: F-RIS provides superior deployment flexibility for UAV covert communications, and the DRL-based joint optimization effectively addresses the complex non-convex problem, achieving substantial performance gains over conventional approaches.
...
### [219] [Joint Channel Estimation and Localization in Pinching-Antenna OFDM Systems: The Blessing of Multipath](https://arxiv.org/abs/2512.09432)
*Min Liu,Yue Xiao,Shuaixin Yang,Gang Wu,Xianfu Lei,Wei Xiang*

Main category: eess.SP

TL;DR: A hybrid inference framework for joint localization and channel estimation in uplink pinching-antenna systems (PASS) using OFDM, achieving near-CRLB performance with reduced hardware complexity.


<details>
  <summary>Details</summary>
Motivation: Pinching-antenna systems (PASS) offer flexible reconfiguration of large-scale wireless channels, motivating investigation of joint localization and channel estimation for uplink PASS in multipath environments to leverage this potential.

Method: Established comprehensive multi-user OFDM uplink PASS model using cyclic prefix to transform multipath dispersion into frequency-domain sinusoids. Proposed hybrid inference framework: expectation propagation for multi-user interference mitigation, OMP or BP-VI for path delay extraction, iterative localization with geometric information refinement, and recursive feedback to EP.

Result: Simulation results show the proposed framework closely approaches the Cramer-Rao lower bound (CRLB), achieving performance comparable to cooperative multi-base station localization but with significantly fewer RF chains and reduced hardware complexity.

Conclusion: The hybrid inference framework effectively solves joint localization and channel estimation for uplink PASS, demonstrating near-optimal performance with practical hardware advantages over traditional cooperative localization approaches.
...
### [220] [Analytical and DNN-Aided Performance Evaluation of IRS-Assisted THz Communication Systems](https://arxiv.org/abs/2512.09515)
*Soumendu Das,Nagendra Kumar,Dharmendra Dixit*

Main category: eess.SP

TL;DR: IRS-assisted THz communication system analysis with α-μ fading, beam misalignment, and deep learning-based performance prediction.


<details>
  <summary>Details</summary>
Motivation: THz communication faces challenges like atmospheric attenuation, fading, and beam misalignment. IRS can enable connectivity when direct paths are unavailable, but comprehensive performance analysis under practical impairments is needed.

Method: Model IRS links with α-μ fading and beam misalignment errors. Use Laguerre series expansion for IRS characterization. Derive closed-form expressions for OP, ACC, and ASER for RQAM/HQAM. Develop DNN frameworks for fast performance prediction. Conduct asymptotic high-SNR analysis and Monte Carlo validation.

Result: Derived analytical expressions for key performance metrics. Developed accurate DNN models for fast system evaluation. Validated theoretical results through simulations. Analyzed impact of phase errors and quantization on system performance.

Conclusion: IRS effectively enables THz communication in non-line-of-sight scenarios. Theoretical analysis provides design insights, while DNN frameworks offer computationally efficient performance evaluation. System performance is affected by phase errors and fading characteristics.
...
### [221] [CKM-Enabled Joint Spatial-Doppler Domain Clutter Suppression for Low-Altitude UAV ISAC](https://arxiv.org/abs/2512.09560)
*Zihan Xu,Zhiwen Zhou,Di Wu,Xiaoli Xu,Yong Zeng*

Main category: eess.SP

TL;DR: Proposes a clutter suppression technique for OFDM ISAC systems using a novel Clutter Angle Map (CLAM) to enhance sensing of slow-moving low-altitude UAV targets in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Traditional clutter suppression methods based on Doppler difference or signal strength are inadequate for low-altitude UAV sensing in complex dynamic environments with slow-moving targets and dynamic clutter.

Method: Introduces CLAM (Clutter Angle Map) as a site-specific database of primary clutter angles, enabling spatial domain clutter suppression. Also proposes a two-step joint spatial-Doppler domain algorithm for scenarios where targets and clutter are in close directions.

Result: Simulation results demonstrate effective clutter suppression and enhanced target sensing performance, achieving accurate parameter estimation for slow-moving low-altitude UAV targets.

Conclusion: The proposed CLAM-based technique provides an effective solution for clutter suppression in OFDM ISAC systems, significantly improving sensing performance for low-altitude UAV targets in complex dynamic environments.
...
### [222] [Temporal Windows of Integration for Multisensory Wireless Systems as Enablers of Physical AI](https://arxiv.org/abs/2512.09589)
*Anup Mishra,João Henrique Inacio de Souza,Petar Popovski*

Main category: eess.SP

TL;DR: The paper proposes a timing-aware network design framework for Physical AI systems that minimizes the validity horizon (usefulness window) while ensuring delivery reliability, optimizing delay-reliability budgets for sensors within finite spatial footprints.


<details>
  <summary>Details</summary>
Motivation: Physical AI systems interact with the physical world in real-time using multimodal sensor data, but face challenges with heterogeneous and unreliable network paths. Traditional throughput/latency guarantees don't ensure correct decision-making due to misaligned, misordered, or stale inputs that can lead to wrong inferences.

Method: Focuses on the "usefulness" timing primitive using Temporal Window of Integration (TWI)-Causality. Models end-to-end path delay as sum of sensing/propagation, computation, and access/transmission latencies. Formulates network design as convex optimization problem to minimize validity horizon under delivery reliability constraints, solving for minimal horizon and per-path reliability allocation.

Result: Proposes an optimal solution for calibrating delay-reliability budgets that outperforms a benchmark uniform-after-threshold allocation approach. The framework enables timing-aware Physical AI systems to operate effectively over sensors within finite spatial footprints.

Conclusion: The study contributes to timing-aware Physical AI in next-generation networks by providing a systematic approach to ensure decision-time coherence through optimized timing primitives, particularly focusing on usefulness while assuming upstream handling of simultaneity and causality.
...
### [223] [On the Ambiguity Function of OFDM-based ISAC Signals Under Non-Ideal Power Amplifiers](https://arxiv.org/abs/2512.09803)
*Eya Gourar,Yahia Medjahdi,Laurent Clavier,Abdul Karim Gizzini,Patrick Sondi*

Main category: eess.SP

TL;DR: PA nonlinearity degrades OFDM-based ISAC performance, diminishing PSK's sensing advantage over QAM and imposing performance ceilings for both constellations.


<details>
  <summary>Details</summary>
Motivation: While OFDM is promising for ISAC, its high PAPR requires linear PAs that conflict with radar's need for high transmit power. The sensing superiority of PSK over QAM under PA nonlinearity remains unclear, especially since distortion artifacts could reshape ambiguity functions and degrade performance.

Method: Analyzed PA-induced distortion effects on PSK and QAM constellations for OFDM-based sensing. Introduced Signal-to-Distortion Ratio (SDR) to quantify distortion limitations on ranging. Combined simulations with theoretical characterization of ambiguity functions to examine distortion artifacts in zero-Doppler and zero-delay sidelobes.

Result: PA distortions impose performance ceilings for both constellations, reshape ambiguity functions, reduce detection probability, diminish PSK's theoretical advantage over QAM, and further compromise OFDM sensing with non-uniform envelope signals.

Conclusion: PA nonlinearity significantly impacts ISAC performance, undermining the theoretical benefits of unimodular signaling (PSK) and highlighting the need to consider hardware impairments in waveform design for practical ISAC systems.
...
### [224] [Energy-Efficient Federated Learning with Relay-Assisted Aggregation in IIoT Networks](https://arxiv.org/abs/2512.09827)
*Hamid Reza Hashempour,Mostafa Nozari,Gilberto Berardinelli,Yanjiao Li,Jie Zhang,Hien Quoc Ngo,Shashi Raj Pandey*

Main category: eess.SP

TL;DR: Energy-efficient relay-assisted FL framework for IIoT with partial aggregation, SN grouping, relay selection, and SPCA optimization to enhance convergence, reduce latency/energy, and improve reliability.


<details>
  <summary>Details</summary>
Motivation: FL in IIoT faces strict latency and energy constraints. Direct uploads from machinery subnetworks (SNs) to edge servers are inefficient. Need to improve communication efficiency, reduce training latency, and address energy efficiency challenges while ensuring reliability under practical IIoT conditions.

Method: Relay-assisted FL with partial aggregation at relays; decomposition of non-convex EE optimization into computation/communication sub-problems; SN grouping algorithm for single/two-hop categorization; relay selection; SPCA for joint parameter configuration; extension to imperfect CSI.

Result: Significant convergence speed enhancement; outage probability reduced from 10^-2 (single-hop) to 10^-6; substantial energy savings: SPCA reduces energy consumption by at least 2x vs unaggregated cooperation and up to 6x vs single-hop transmission.

Conclusion: Proposed framework effectively addresses EE challenges in IIoT FL, improves reliability through broader participation, and demonstrates superior performance in convergence, energy efficiency, and outage reduction compared to baseline approaches.
...
### [225] [A Speculative GLRT-Backed Approach for Adversarial Resilience on Deep Learning-Based Array Processing](https://arxiv.org/abs/2512.09893)
*Nian-Cin Wang,Rajeev Sahay*

Main category: eess.SP

TL;DR: Proposes a hybrid DL-GLRT framework for array processing that combines fast DL inference with statistically-grounded GLRT validation to achieve both low latency and adversarial robustness.


<details>
  <summary>Details</summary>
Motivation: Classical GLRT methods are statistically sound but computationally expensive for low-latency applications. DL offers fast inference but lacks statistical guarantees and is vulnerable to adversarial attacks in wireless environments.

Method: A speculative framework with DL classifier for fast inference and GLRT validator for confirmation. Leverages spatial invariance of second-order statistics to L-p bounded adversarial perturbations for robustness.

Result: Theoretical analysis shows spatial invariance properties provide adversarial robustness. Empirical evaluations demonstrate superior performance across various L-p bounds, perturbation designs, and magnitudes compared to state-of-the-art baselines.

Conclusion: The hybrid DL-GLRT framework successfully addresses the trade-off between computational efficiency and statistical reliability, providing both low-latency inference and adversarial robustness for array processing tasks.
