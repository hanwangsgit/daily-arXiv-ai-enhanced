...
Motivation: Full fine-tuning of CNNs is infeasible on edge devices due to strict memory, compute, and energy constraints, especially for handling domain shifts in applications like Human Activity Recognition.
...
Conclusion: LoRA-Edge enables practical, structure-aligned, parameter-efficient on-device CNN adaptation for edge platforms with minimal accuracy loss.
...
<details>
  <summary>Details</summary>
Motivation: Existing annotation tools either support behavioral labeling without individual localization or localization without interaction capture, creating a gap for analyzing social and individualized animal behavior.

Method: Developed SILVI - an open-source labeling software that integrates both behavioral annotation and individual localization functionalities within video data.
...
Conclusion: SILVI bridges behavioral ecology with computer vision and could be broadly useful for annotating interactions in various video contexts beyond animal behavior.
...
<details>
  <summary>Details</summary>
Motivation: DL models for COVID-19 detection from CXRs fail to generalize to OOD data due to learning source-specific shortcuts rather than meaningful biomarkers.

Method: Used fundamental noise injection techniques (Gaussian, Speckle, Poisson, and Salt and Pepper) during model training.

Result: Significantly reduced ID-OOD performance gap from 0.10-0.20 to 0.01-0.06 across AUC, F1, accuracy, recall and specificity metrics.

Conclusion: Noise injection is an effective technique to improve model robustness against distribution shifts in medical imaging tasks.
...
TL;DR: Imitation learning policies can successfully guide cannula insertion in spine procedures using bi-planar X-rays, achieving 68.5% first-attempt success rate with safe trajectories, though entry point precision needs improvement.


<details>
  <summary>Details</summary>
Motivation: To examine whether imitation learning applies to X-ray-guided spine procedures like cannula insertion, given the complexity of interpreting multi-view X-rays in medical robotics.
...
Result: Policy succeeded on first attempt in 68.5% of cases, maintained safe intra-pedicular trajectories across diverse vertebral levels, generalized to complex anatomy including fractures, and remained robust to varied initializations. Rollouts on real bi-planar X-rays showed plausible trajectories despite simulation-only training.

Conclusion: Promising preliminary results show imitation learning can work for X-ray-guided spine procedures, but limitations in entry point precision need addressing. With robust priors and domain knowledge, this approach could enable lightweight, CT-free robotic spinal navigation.
...
TL;DR: Proposes an enhanced real-time object detection framework using pruned YOLOv12 with Self-Adversarial Training and specialized data augmentation for waste detection in desert environments, achieving high accuracy and efficiency suitable for aerial drones.


<details>
  <summary>Details</summary>
Motivation: Address the global waste crisis and limitations of traditional waste collection methods, especially in remote/harsh environments like deserts, where current computer vision research focuses mainly on urban settings and recyclable materials.

Method: Developed a lightweight version of YOLOv12 integrated with Self-Adversarial Training (SAT) and specialized data augmentation strategies, using the DroneTrashNet dataset for training and evaluation.

Result: Achieved significant improvements in precision, recall, and mean average precision (mAP) with low latency and compact model size suitable for deployment on resource-constrained aerial drones, outperforming state-of-the-art lightweight YOLO variants.

Conclusion: Validates the effectiveness of combining data-centric and model-centric enhancements for robust, real-time waste detection in desert environments, providing an optimal balance of accuracy and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of small, imbalanced datasets and poor image quality that cause high false prediction rates in medical deep learning applications.

Method: Create Composite Input Images (CoImg) by fusing multiple images of the same class into 3x1 layout composites, enhancing intra-class variance and information density per training sample.

Result: Achieved near-perfect accuracy (99.6%), F1-score (0.995), and AUC (0.9996) on OCTDL dataset, significantly outperforming baseline model trained on raw data with lower false prediction rates.

Conclusion: The method effectively handles class imbalance and small sample sizes, producing high-quality predictions even with weak datasets in medical imaging applications.
...
TL;DR: Using synthetic and real Sentinel-1 satellite data with YOLOv10 models improves offshore platform detection, achieving F1 scores up to 0.90 and demonstrating geographic transferability across unseen regions.
...
Method: Trained YOLOv10 object detection models using synthetic and real Sentinel-1 satellite imagery from four regions, then tested geographic transferability on three unseen regions using region-holdout evaluation.

Result: Detected 3,529 offshore platforms across three regions with F1 score of 0.85 (improving to 0.90 with synthetic data), demonstrating successful generalization beyond training areas.

Conclusion: Synthetic data generation effectively addresses dataset imbalance challenges in remote sensing and enables globally transferable offshore infrastructure detection, highlighting deep learning's potential for scalable monitoring.
...
Conclusion: The framework is effective and efficient for real-world medical imaging applications with scarce labels, steadily refining normality without generative reconstruction or replay buffers.
...
<details>
  <summary>Details</summary>
Motivation: Current temporal action localization methods apply uniform computation despite significant variations in boundary detection difficulty, leading to inefficient resource usage.

Method: Two complementary approaches: Boundary Distance Regression (BDR) for optimal localization via signed-distance regression, and Adaptive Temporal Refinement (ATR) for computation allocation via continuous depth selection with differentiable optimization.

Result: BDR achieves 43% sharper boundary peaks and 1.8-3.1% mAP@0.7 improvements across architectures. ATR achieves 56.5% mAP@0.7 at 162G FLOPs vs 53.6% at 198G for uniform processing - 2.9% improvement with 18% less compute. Gains scale to 4.2% for short actions.

Conclusion: The proposed methods provide significant improvements in temporal action localization efficiency and performance, with BDR offering easy integration and ATR enabling adaptive computation allocation without reinforcement learning.
...
Method: Simultaneous optimization of mesh geometry (vertex positions and faces) and vertex colors via Gaussian-guided mesh differentiable rendering, leveraging photometric consistency and geometric regularization from normal and depth maps.
...
Conclusion: The proposed unified treatment of geometry and appearance optimization enables seamless Gaussian-mesh joint optimization for improved 3D reconstruction quality and editing capabilities.
...
Result: Experimental results on both physical and simulated data verified the performance, and the model enables faster simulation of raw light field images for deep learning methods.

Conclusion: The proposed method effectively calibrates light field cameras and enables faster image simulation for data-driven approaches, with code available from author's website.
...
TL;DR: A synthetic dataset called Room Envelopes is introduced to help predict structural scene elements like walls, floors, and ceilings, which are often missing in traditional 3D reconstructions due to occlusions.


<details>
  <summary>Details</summary>
Motivation: Traditional scene reconstruction methods fail to capture occluded surfaces, especially structural elements. These elements are planar, repetitive, and simple, making them easier to predict with less complex approaches.

Method: The Room Envelopes dataset provides RGB images with two pointmaps per image: one for visible surfaces and one for structural layout surfaces (after removing fixtures). This enables direct supervision for monocular geometry estimators.

Result: The dataset facilitates training models to predict both visible surfaces and structural layout surfaces, improving understanding of scene extent and object shapes/locations.

Conclusion: The Room Envelopes dataset supports progress in predicting structural scene elements, offering a practical solution for more complete scene reconstructions.
...
<details>
  <summary>Details</summary>
Motivation: PPG signals offer non-invasive, cost-effective biometric authentication with inherent liveness detection, but face challenges from motion artifacts and physiological variability.

Method: Preprocess PPG signals with baseline removal, PCA artifact suppression, filtering, and normalization; convert to time-frequency scalograms via CWT; use hybrid CVT-ConvMixer-LSTM model for spatial-temporal feature learning.

Result: 98% authentication accuracy on 46 subjects, demonstrating robustness to noise and inter-subject variability.

Conclusion: The proposed system is efficient, scalable, and suitable for real-world mobile biometric security applications due to its liveness detection capability.
...
### [14] [Simple 3D Pose Features Support Human and Machine Social Scene Understanding](https://arxiv.org/abs/2511.03988)
*Wenshuo Qin,Leyla Isik*

Main category: cs.CV

TL;DR: 3D pose information outperforms AI vision models in predicting human social interaction judgments, with face position and direction being key features.


<details>
  <summary>Details</summary>
Motivation: To understand why humans excel at social interaction recognition while AI struggles, and test if 3D visuospatial pose information is the missing element.

Method: Combined pose and depth estimation to extract 3D joint positions from videos, compared with AI vision models, and derived minimal 3D social pose features focusing on face position/direction.

Result: 3D joint positions outperformed most AI models; minimal face position/direction features matched full joint set performance and improved AI models when combined; model performance correlated with 3D pose feature representation.
...
### [15] [CaRF: Enhancing Multi-View Consistency in Referring 3D Gaussian Splatting Segmentation](https://arxiv.org/abs/2511.03992)
*Yuwen Tao,Kanglei Zhou,Xin Tan,Yuan Xie*

Main category: cs.CV

TL;DR: CaRF is a camera-aware framework for 3D Gaussian segmentation that achieves multi-view consistency by incorporating camera geometry into Gaussian-text interactions and aligning per-Gaussian logits across calibrated views during training.
...
### [16] [PhysCorr: Dual-Reward DPO for Physics-Constrained Text-to-Video Generation with Automated Preference Selection](https://arxiv.org/abs/2511.03997)
*Peiyao Wang,Weining Wang,Qi Li*

Main category: cs.CV

TL;DR: PhysCorr is a framework that improves physical consistency in text-to-video generation by introducing PhysicsRM (a dual-dimensional reward model) and PhyDPO (a direct preference optimization pipeline) to address implausible object dynamics and interactions.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video generation models produce high perceptual quality but often violate physical plausibility principles, limiting their use in embodied AI, robotics, and simulation domains.

Method: Proposes PhysicsRM to quantify intra-object stability and inter-object interactions, and develops PhyDPO with contrastive feedback and physics-aware reweighting for direct preference optimization.
...
### [17] [GNN-MoE: Context-Aware Patch Routing using GNNs for Parameter-Efficient Domain Generalization](https://arxiv.org/abs/2511.04008)
*Mahmoud Soliman,Omar Abdelaziz,Ahmed Radwan,Anand,Mohamed Shehata*

Main category: cs.CV

TL;DR: GNN-MoE enhances parameter-efficient fine-tuning for domain generalization in Vision Transformers using a Mixture-of-Experts framework with graph neural network routing.


<details>
  <summary>Details</summary>
Motivation: Standard fine-tuning of pretrained Vision Transformers for domain generalization is computationally expensive and can impair generalization performance on unseen domains.

Method: Proposes GNN-MoE with efficient Kronecker adapters and a novel GNN router (GCN, GAT, SAGE) that operates on inter-patch graphs to dynamically assign patches to specialized experts, leveraging inter-patch relationships for context-aware routing.
...
Conclusion: Graph-based contextual routing enables robust and lightweight domain generalization, demonstrating the utility of GNN routing for adapting Vision Transformers to domain shifts.
...
TL;DR: MedDChest is a Vision Transformer model pre-trained from scratch on 1.2M thoracic medical images, using novel Guided Random Resized Crops augmentation, achieving superior performance over ImageNet-pretrained models on various diagnostic tasks.


<details>
  <summary>Details</summary>
Motivation: To address the domain gap between natural images and medical imaging that hinders vision model performance in thoracic diagnostics, by creating a domain-specific foundation model.

Method: Pre-trained ViT from scratch on 1.2M multimodal thoracic images (X-ray, CT) from 10 public sources, using novel Guided Random Resized Crops augmentation that biases sampling towards anatomically relevant regions.

Result: MedDChest significantly outperforms strong ImageNet-pretrained models across diverse downstream diagnostic tasks, demonstrating the superiority of large-scale in-domain pre-training with domain-specific augmentation.

Conclusion: MedDChest provides a powerful, robust feature extractor that serves as a superior starting point for thoracic diagnostic tasks, establishing the value of domain-specific pre-training and augmentation in medical imaging.
...
### [19] [Near-Lossless 3D Voxel Representation Free from Iso-surface](https://arxiv.org/abs/2511.04029)
*Yihao Luo,Xianglong He,Chuanyu Pan,Yiwen Chen,Jiaqi Wu,Yangguang Li,Wanli Ouyang,Yuanming Hu,Guang Yang,ChoonHwai Yap*

Main category: cs.CV

TL;DR: Faithful Contouring is a sparse voxelized representation for 3D meshes that achieves near-lossless fidelity at 2048+ resolutions without requiring water-tightening or isosurface extraction, enabling superior accuracy in 3D reconstruction and generation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing voxelized representations based on iso-surface methods compromise geometric fidelity by requiring water-tightening or rendering optimization, which fail to preserve sharp features and internal structures of arbitrary meshes.

Method: Proposes Faithful Contouring - a sparse voxelized representation that avoids converting meshes to field functions or extracting isosurface during remeshing, and designs a dual-mode autoencoder for scalable shape reconstruction while preserving details.

Result: Achieves distance errors at 10^{-5} level for direct representation, and for mesh reconstruction yields 93% reduction in Chamfer Distance and 35% improvement in F-score over strong baselines, demonstrating superior geometric fidelity.

Conclusion: Faithful Contouring surpasses existing methods in both accuracy and efficiency for 3D representation and reconstruction, confirming its superior fidelity as a representation for 3D learning tasks.
...
TL;DR: Bratrix is a multimodal framework that aligns visual and brain representations with language embeddings to improve interpretability and robustness in decoding neural signals like EEG, MEG, and fMRI.


<details>
  <summary>Details</summary>
Motivation: Existing methods align neural activity directly with visual embeddings, but visual-only representations fail to capture semantic dimensions, limiting interpretability and robustness due to subject variability and entangled visual features.

Method: Bratrix decouples visual stimuli into hierarchical visual and linguistic components, projects them into a shared latent space, and uses uncertainty-aware weighting. It employs learnable language-anchored semantic matrices and a two-stage training strategy (single-modality pretraining followed by multimodal fine-tuning).

Result: Extensive experiments on EEG, MEG, and fMRI benchmarks show Bratrix improves retrieval, reconstruction, and captioning performance, surpassing state-of-the-art methods by 14.3% in 200-way EEG retrieval.

Conclusion: Bratrix effectively addresses limitations of visual-only representations by incorporating multimodal language-anchored alignment, enhancing cross-modal correlations and handling noisy neural signals through uncertainty perception.
...
<details>
  <summary>Details</summary>
Motivation: To address CT image denoising in scenarios where paired clean-noisy data is unavailable, requiring unpaired and self-supervised approaches.

Method: Evaluated two paradigms: CycleGAN-based residual translator and Noise2Score score-matching denoiser. Used standard U-Net backbone with optimized hyperparameters (lambda_cycle=30, lambda_iden=2, ngf=ndf=64) for CycleGAN.

Result: CycleGAN improved noisy input from 34.66 dB/0.9234 SSIM to 38.913 dB/0.971 SSIM, with estimated score of 1.9441 and unseen-set score of 1.9343. Noise2Score achieved large gains on very noisy inputs despite slightly lower absolute PSNR/SSIM.

Conclusion: CycleGAN offers strongest final image quality, while Noise2Score provides robust pair-free alternative with competitive performance, making both valuable for different scenarios.
...
Method: Proposed UKAST architecture that integrates rational-function based KANs into Swin Transformer encoders, using Group Rational KANs (GR-KANs) from KAT to improve efficiency over vanilla spline-based KANs.
...
Conclusion: KAN-enhanced Transformers show strong potential for advancing data-efficient medical image segmentation, addressing the data-hungry limitations of standard Vision Transformers.
...
TL;DR: SpatialLock is a novel framework that uses perception signals and grounding information to achieve precise object localization in text-to-image generation, addressing limitations in existing methods that fail to fully utilize positional information.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image synthesis methods struggle with precise control over object localization and spatial layouts, leading to inadequate understanding of object positioning in generated images.
...
Conclusion: The proposed SpatialLock framework effectively addresses object localization challenges in text-to-image synthesis by jointly leveraging perception signals and grounding information, enabling precise spatial arrangements of objects.
...
TL;DR: THG is a training-free diffusion sampling acceleration method that reduces function evaluations by 30% while maintaining image quality, using a multirate ODE system where noise estimation and guidance terms are computed at different frequencies.


<details>
  <summary>Details</summary>
Motivation: To address the computational inefficiency in diffusion models where conventional solvers treat all components equally, despite different sensitivity to numerical errors.

Method: Reformulates classifier-free guidance ODE as multirate system, with noise estimate on fine grid and guidance term on coarse grid, plus adaptive timestep sampler and guidance-scale scheduler.
...
Method: Proposes a framework that incorporates reference features as auxiliary information with linear smoothing and style-content guidance, avoiding overwriting self-attention matrices. Extends to multi-style generation using a joint AdaIN module.

Result: Achieves high-quality sketch generation with accurate style alignment, especially effective when reference and target sketches have low structural similarity. Reduces content leakage and improves synthesis quality.

Conclusion: The proposed M3S framework provides improved flexibility in style control for sketch generation while maintaining high quality and accurate style alignment without requiring additional training.
...
TL;DR: Automated tennis match analysis pipeline using deep learning for player/ball detection, tracking, and court keypoint detection to provide detailed performance analytics.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive automated system for tennis match analysis that provides actionable insights for coaches, broadcasters, and players without manual intervention.

Method: Integrated deep learning pipeline using YOLOv8 for player detection, custom YOLOv5 for ball tracking, and ResNet50-based architecture for court keypoint detection to analyze player movement, ball speed, shot accuracy, and reaction times.

Result: Robust performance demonstrated in varying court conditions and match scenarios, with system outputting annotated videos and detailed performance metrics.

Conclusion: The complete pipeline successfully automates tennis match analysis, providing valuable insights into game dynamics through integrated deep learning models.
...
<details>
  <summary>Details</summary>
Motivation: Complex maritime environments cause camera motion and visual degradation, posing significant challenges to multi-object tracking for vessel navigation and maritime surveillance.

Method: Proposes a parallel tracker with affine compensation: (1) object detection and ReID branch with Reversible Columnar Detection Network and lightweight Transformer-based appearance extractor, (2) camera motion estimation branch that decouples platform and target motion, and (3) clustering-optimized feature fusion module.
...
Conclusion: DMSORT provides an efficient solution for maritime MOT that handles camera motion and visual degradation through its dual-branch architecture and motion compensation techniques.
...
<details>
  <summary>Details</summary>
Motivation: Computer-use agents lag behind humans in tasks requiring domain-specific procedural knowledge. Humans learn from video tutorials by selectively imitating relevant segments, but current agents lack this capability.

Method: Proposes a framework that: 1) retrieves and filters tutorial videos, 2) converts them into structured demonstration trajectories using a VLM to infer UI actions and segment videos, 3) uses two-stage selection to dynamically choose the most helpful trajectory as in-context guidance at each execution step.

Result: Experiments on two benchmarks show the framework consistently outperforms base agents and variants using only textual tutorials or transcripts. Key factors include trajectory segmentation/selection, action filtering, and visual information.

Conclusion: Online videos can be systematically distilled into actionable guidance that improves computer-use agents at inference time, bridging the gap between human and agent performance on complex workflows.
...
Method: Introduces ORB benchmark and a fast rotation classification pipeline built on Phi-3.5-Vision model with dynamic image cropping, fine-tuned for 4-class rotation classification.

Result: Achieves 96% accuracy on ORB-En (English) and 92% on ORB-Indic (11 Indic languages). Boosts OCR performance: closed-source models by up to 14% and open-weights models by up to 4x.

Conclusion: The proposed rotation classification module is critical for improving OCR performance in real-world document understanding scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Accurate registration of images from different modalities is essential in digital pathology for applications like biomarker analysis and tissue reconstruction, but existing methods may not adequately handle the color differences between H&E stained images and non-linear multimodal images.

Method: Used 20 tissue sample pairs with various preprocessing steps including different color transformations (CycleGAN, Macenko, Reinhard, Vahadane), inversion, contrast adjustment, intensity normalization, and denoising. Registered images using VALIS method with rigid followed by non-rigid registration, evaluated using relative Target Registration Error (rTRE).
...
Conclusion: Applying color transformation before registration improves alignment between images from different modalities and supports more reliable analysis in digital pathology, with CycleGAN being the most effective method.
...
TL;DR: Covariance descriptors from pre-trained vision encoders outperform handcrafted features for medical image classification, with SPDNet achieving state-of-the-art results when combined with DINOv2 features.
...
Method: Construct covariance descriptors from features extracted by pre-trained general vision encoders (DINOv2 and MedSAM), compare with handcrafted descriptors, and evaluate using SPDNet on eleven datasets from MedMNSIT benchmark.

Result: Covariance descriptors from GVE features consistently outperform handcrafted features, and SPDNet with DINOv2 features yields superior performance to state-of-the-art methods.
...
TL;DR: Proposes Adaptive Statistics Fusor (AStF) with skewness and kurtosis for motion style transfer, outperforming state-of-the-art methods by better capturing complex dynamic patterns.


<details>
  <summary>Details</summary>
Motivation: Traditional style transfer methods using only mean and variance are insufficient for motion data due to fundamental differences between images and motion, requiring more comprehensive statistical analysis.

Method: Developed AStF with Style Disentanglement Module and High-Order Multi-Statistics Attention, trained with Motion Consistency Regularization discriminator to capture spatiotemporal statistical patterns.

Result: Experimental results show AStF achieves superior performance in motion style transfer compared to state-of-the-art methods by modeling spatiotemporal statistical patterns more comprehensively.

Conclusion: Incorporating higher-order statistics (skewness and kurtosis) alongside mean and variance significantly improves motion style transfer by better capturing complex dynamic patterns and spatiotemporal coherence.
...
TL;DR: MedSapiens adapts human-centric foundation models (Sapiens) for anatomical landmark detection in medical imaging, achieving state-of-the-art performance across multiple datasets with significant improvements over both generalist and specialist models.
...
Method: Adaptation of Sapiens (human-centric foundation model) through multi-dataset pretraining to create MedSapiens, specifically designed for medical imaging landmark detection.
...
Conclusion: Human-centric foundation models provide strong priors for anatomical landmark detection and can be effectively adapted for medical imaging tasks, establishing new state-of-the-art performance while requiring fewer annotations for novel downstream tasks.
...
<details>
  <summary>Details</summary>
Motivation: The growing sophistication of synthetic image and deepfake generation models has created critical challenges for source attribution and authenticity verification in computer vision systems.

Method: The framework operates in the latent domain of diffusion models, re-simulating partial forward diffusion to expose generator-specific cues. It uses a temporal attention encoder to aggregate multi-step latent features and a feature-weighted prototype head to structure the embedding space.

Result: Proto-LeakNet achieves a Macro AUC of 98.13%, learns robust latent geometry under post-processing, surpasses state-of-the-art methods, and achieves strong separability between known and unseen generators.
...
TL;DR: DinoGRL framework uses DINOv2 to learn gait features for cross-modal video person re-identification, combining silhouette learning with appearance cues through progressive multi-granularity enhancement.


<details>
  <summary>Details</summary>
Motivation: Existing VVI-ReID methods focus on modality-invariant visual features but overlook gait features, which are both modality-invariant and rich in temporal dynamics, limiting their spatiotemporal consistency modeling for cross-modal video matching.

Method: Proposed DinoGRL framework with Semantic-Aware Silhouette and Gait Learning (SASGL) to generate silhouette representations using DINOv2 priors, and Progressive Bidirectional Multi-Granularity Enhancement (PBMGE) module for bidirectional interactions between gait and appearance streams across multiple spatial granularities.

Result: Extensive experiments on HITSZ-VCM and BUPT datasets show superior performance, significantly outperforming existing state-of-the-art methods.
...
TL;DR: FastGS is a novel acceleration framework for 3D Gaussian splatting that uses multi-view consistency to regulate Gaussian density, achieving 3.32×-15.45× training acceleration while maintaining comparable rendering quality.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS acceleration methods fail to properly regulate Gaussian numbers during training, causing redundant computational overhead and inefficiency.
...
Result: Achieves 3.32× training acceleration vs DashGaussian on Mip-NeRF 360 and 15.45× acceleration vs vanilla 3DGS on Deep Blending, with strong generality across multiple tasks (2-7× acceleration).

Conclusion: FastGS provides significant training speed improvements while maintaining rendering quality, demonstrating strong generality across various 3D reconstruction tasks.
...
<details>
  <summary>Details</summary>
Motivation: General-purpose vision models have limited performance in agriculture where fine-grained distinctions between plant species and herbicide damage types are critical, requiring domain-specific adaptation.
...
Result: Significant improvements over general-purpose models: species identification F1 from 0.91 to 0.94, damage classification from 0.26 to 0.33. Under unseen conditions, gains were even greater (species from 0.56 to 0.66, damage from 0.17 to 0.27). Achieved 5.4% higher F1 with 80% fewer labeled samples.

Conclusion: Domain-specific foundation models offer superior generalization capabilities and can significantly reduce manual annotation efforts, providing a scalable automated solution for herbicide trial analysis.
...
<details>
  <summary>Details</summary>
Motivation: Existing text-to-video diffusion models fail to maintain video quality with concise prompts due to limited textual semantics understanding, and cannot rephrase prompts online to better align with user intentions, limiting scalability and usability.

Method: Proposes RISE-T2V framework with a Rephrasing Adapter that enables diffusion models to use text hidden states during LLM's next token prediction as video generation condition, implicitly rephrasing basic prompts into comprehensive representations.

Result: Extensive experiments show RISE-T2V is versatile across different video diffusion model architectures, significantly enhancing T2V models' ability to generate high-quality videos that align with user intent.

Conclusion: RISE-T2V provides a universal framework that can be applied to various pre-trained LLMs and video diffusion models, improving their capabilities for text-to-video tasks by better understanding and rephrasing user prompts.
...
TL;DR: A novel two-stage method using voxel sparsification and submanifold sparse convolutional networks for automated 3D tumor segmentation in CT scans, achieving state-of-the-art accuracy with significant computational efficiency improvements.


<details>
  <summary>Details</summary>
Motivation: Manual tumor delineation in radiological images is time-consuming and specialized, creating a bottleneck for quantitative clinical analysis. Current 3D segmentation methods face impracticality due to large voxel volumes, requiring downsampling or patch-based approaches.

Method: Two-stage approach combining voxel sparsification with submanifold sparse convolutional networks, enabling high-resolution 3D segmentation without traditional computational limitations.

Result: Achieved competitive results on KiTS23 renal cancer dataset: 95.8% Dice for kidneys+masses, 85.7% for tumors+cysts, 80.3% for tumors alone. Significant computational improvements: 60% reduction in inference time and 75% reduction in VRAM usage compared to dense architectures.

Conclusion: The proposed method enables high-accuracy 3D tumor segmentation with substantially reduced computational requirements, making automated quantitative analysis more feasible for clinical applications.
...
TL;DR: Evaluation of 9 CNN architectures for binary classification of horses/motorcycles in VOC 2008 dataset, addressing class imbalance with data augmentation.


<details>
  <summary>Details</summary>
Motivation: To address significant class imbalance problem in binary classification tasks and compare performance of modern architectures.

Method: Implemented minority-class augmentation techniques and compared ResNet-50, ConvNeXt-Tiny, DenseNet-121, Vision Transformer across multiple metrics.

Result: ConvNeXt-Tiny achieved highest AP (95.53% for horses, 89.12% for motorcycles). Data augmentation significantly improved minority class detection, especially for deeper architectures.

Conclusion: Provides insights for architecture selection in imbalanced binary classification and quantifies data augmentation's impact in mitigating class imbalance.
...
TL;DR: Study investigates sensor occlusion effects on 3D object detection using BEVFusion architecture. Camera occlusions cause 41.3% mAP drop, LiDAR drops 47.3% under heavy occlusion. Fusion shows stronger reliance on LiDAR - camera occlusion causes minor 4.1% drop while LiDAR occlusion causes 26.8% drop.


<details>
  <summary>Details</summary>
Motivation: To understand how sensor occlusions (from fog, haze, obstructions) affect 3D detection accuracy in BEV-based fusion systems, as this remains underexplored despite BEV's strong performance in multimodal integration.

Method: Used BEVFusion architecture evaluated on nuScenes dataset, measuring detection performance with mAP and NDS metrics. Analyzed individual sensor and fused performance under camera and LiDAR occlusion conditions.

Result: Camera-only detection drops 41.3% mAP under moderate occlusion. LiDAR drops 47.3% mAP under heavy occlusion. In fusion: camera occlusion causes minor 4.1% drop, LiDAR occlusion causes 26.8% drop, showing stronger reliance on LiDAR for 3D detection.

Conclusion: BEVFusion relies more heavily on LiDAR for 3D object detection. Future research needed for occlusion-aware evaluation methods and improved sensor fusion techniques to maintain accuracy under adverse conditions and partial sensor failure.
...
TL;DR: A step-by-step tutorial for applying deep learning to extract spatial features from imaging data in analytical chemistry, focusing on using existing models rather than training new ones.
...
Method: Provides MATLAB code demonstrations for processing various imaging modalities using existing open-source deep learning models to extract deep features.

Result: Enables analytical chemists to extract multiscale spatial features from imaging data that are challenging to capture with traditional chemometric methods.

Conclusion: This tutorial bridges the gap between deep learning capabilities and analytical chemistry applications by providing practical implementation guidance for spatial feature extraction.
...
<details>
  <summary>Details</summary>
Motivation: To develop a comprehensive medical VQA system that can simultaneously provide accurate answers, generate explanations, and perform visual grounding for better interpretability in medical applications.

Method: Uses LoRA-tuned Florence-2 model with three curated datasets: Kvasir-VQA-x1 for Q&A, synthetic explanation dataset for medical reasoning, and text-to-region pairs for visual grounding with segmentation masks.

Result: Substantially improves over single-task baselines in both answer accuracy and visual localization, demonstrating effectiveness of grounded multi-task learning.

Conclusion: Multi-task learning with visual grounding enables joint learning of visual grounding, reasoning, and interpretation, producing more accurate and interpretable responses for medical VQA applications.
...
<details>
  <summary>Details</summary>
Motivation: To address poor depth estimation performance and blurred object boundaries in existing monocular depth estimation methods on embedded systems, leveraging the low-cost advantage of monocular approaches.
...
Result: The model significantly outperforms previous lightweight models on multiple challenging datasets, achieves 50.7 FPS on NVIDIA Jetson Orin, and provides improved boundary quality in depth maps.

Conclusion: BoRe-Depth demonstrates effective lightweight depth estimation with enhanced boundary quality suitable for embedded systems, with code publicly available for further research and deployment.
...
TL;DR: HideAndSeg is a minimally supervised AI tool that combines SAM2 and YOLOv11 to automatically segment octopuses in videos, using unsupervised metrics for quality evaluation without ground-truth data.


<details>
  <summary>Details</summary>
Motivation: Analyzing octopuses in natural habitats is difficult due to camouflage, rapid skin changes, non-rigid deformations, occlusions, and variable underwater conditions, compounded by lack of large annotated datasets.

Method: Integrates SAM2 with custom-trained YOLOv11 detector. Starts with user-provided points for initial SAM2 masks, then trains YOLO model and automates pipeline with bounding box prompts. Uses unsupervised metrics (temporal consistency DICE_t and new component count NC_t) for evaluation.

Result: Achieves satisfactory performance, reduces segmentation noise compared to manual prompting, and successfully re-identifies octopuses after complete occlusion where manual methods fail.
...
TL;DR: This paper introduces a computational solver for convex partition jigsaw puzzles, expanding beyond traditional square puzzles by using geometric and pictorial compatibilities with a greedy algorithm approach.


<details>
  <summary>Details</summary>
Motivation: Most existing jigsaw puzzle solvers only work with square puzzles, which severely limits their practical applications. The authors aim to expand computational puzzle solving to handle convex partitions - a major subset of polygonal puzzles with convex pieces.
...
Result: The paper reports several performance measures and introduces the first benchmark dataset for convex partition jigsaw puzzles, expanding the types of puzzles that can be handled computationally.

Conclusion: This work significantly expands the practical applicability of computational jigsaw puzzle solvers by addressing convex partition puzzles, which represent a major subset of polygonal puzzles beyond the traditional square format limitations.
...
TL;DR: V-Thinker is a multimodal reasoning assistant that enables interactive, vision-centric thinking through end-to-end reinforcement learning, outperforming existing LMM baselines.
...
Method: Uses a Data Evolution Flywheel for automatic dataset synthesis and verification, combined with Visual Progressive Training Curriculum that includes perception alignment and two-stage reinforcement learning.
...
Conclusion: V-Thinker provides valuable insights for advancing image-interactive reasoning applications and represents significant progress in vision-centric multimodal reasoning.
...
TL;DR: GeoFMs like Prithvi-EO-2.0 outperform traditional models for landslide mapping across sensors, regions, and limited data conditions through a three-axis framework of sensor, label, and domain adaptation.
...
Result: Consistently outperforms task-specific CNNs (U-Net, U-Net++), vision transformers (Segformer, SwinV2-B), and other GeoFMs (TerraMind, SatMAE), showing resilience to spectral variation and maintaining accuracy with limited labels.
...
TL;DR: Proposes a comprehensive 8-metric evaluation framework for talking head video generation, addressing limitations in current assessment methods by focusing on quality, naturalness, and synchronization dimensions.


<details>
  <summary>Details</summary>
Motivation: Current video generation advances have outpaced evaluation metrics development, with limited metrics primarily assessing general quality and lip sync, lacking comprehensive assessment of talking head generation.

Method: Developed an evaluation framework with 8 metrics across three dimensions: quality, naturalness, and synchronization, emphasizing efficiency and human preference alignment. Analyzed fine-grained dynamics of head, mouth, eyebrows, and face quality.
...
Conclusion: The proposed benchmark framework aims to properly evaluate generative method improvements, with plans for public release of code, dataset, and leaderboards to track field progress.
...
TL;DR: STC-Net is a novel framework for automated surgical complexity estimation in Laparoscopic Cholecystectomy using the Parkland Grading Scale, operating on full videos with weak temporal supervision.
...
Method: STC-Net performs joint temporal localization and grading through localization, window proposal, and grading modules. It uses a novel loss combining hard/soft localization objectives and background-aware grading supervision, operating directly on full videos.
...
Conclusion: STC-Net provides a scalable and effective approach for automated PGS-based surgical complexity estimation from full LC videos, suitable for post-operative analysis and surgical training.
...
Method: Developed Video Thinking Benchmark (VideoThinkBench) with vision-centric tasks (e.g., Eyeballing Puzzles) and text-centric tasks (e.g., subsets of GSM8K, MMMU). Evaluated Sora-2 video generation model as a reasoner using self-consistency and in-context learning techniques.

Result: Sora-2 performs comparably to SOTA VLMs on vision-centric tasks, surpasses them on Eyeballing Games, achieves 92% accuracy on MATH and 75.53% on MMMU for text-centric tasks. Self-consistency and in-context learning improve performance.
...
Method: Constructs 3D latent scaffold using pretrained foundation models, introduces efficient fusion mechanism for spatio-temporal alignment, and uses dual-branch decoder with point-anchored refinement and voxel-based generation for dynamic-aware Gaussians with persistent static memory.
...
Conclusion: UniSplat demonstrates effective dynamic scene reconstruction through unified latent spatio-temporal fusion, enabling complete and detailed reconstructions in challenging autonomous driving scenarios.
...
TL;DR: PixCLIP enhances CLIP's fine-grained vision-language alignment by incorporating visual prompts and processing long textual descriptions, achieving state-of-the-art performance through pixel-text alignment.


<details>
  <summary>Details</summary>
Motivation: CLIP has limitations in fine-grained image-text alignment due to token length constraints in its text encoder, preventing it from processing detailed textual descriptions that could improve alignment.

Method: Proposes PixCLIP framework with automated annotation pipeline for pixel-level localized long-form descriptions, constructs LongGRIT dataset, replaces CLIP's text encoder with LLM, and implements three-branch pixel-text alignment learning.
...
Conclusion: The proposed PixCLIP framework successfully addresses CLIP's limitations by synergistically enhancing both visual and textual granularity processing, enabling superior fine-grained vision-language alignment capabilities.
...
TL;DR: The paper introduces an automated framework to evaluate virtual IHC stain quality using stain accuracy metrics instead of traditional image fidelity metrics, showing that paired models perform best and WSI-level evaluation is crucial.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for virtual IHC stains focus on image fidelity rather than staining accuracy, creating a need for metrics that directly quantify correct pixel-level labeling without requiring expert annotations.

Method: Uses color deconvolution to generate masks of IHC-positive pixels from virtual IHC models, then computes stain accuracy metrics (Dice, IoU, Hausdorff distance) by comparing segmented masks of real and virtual IHC.

Result: Conventional metrics (FID, PSNR, SSIM) correlate poorly with stain accuracy and pathologist assessment. Paired models (PyramidPix2Pix, AdaptiveNCE) achieve highest accuracy, while unpaired diffusion- and GAN-based models are less reliable. WSI evaluations reveal performance declines not visible in patch-based assessments.

Conclusion: The framework provides a reproducible approach for assessing virtual IHC model quality, which is critical for accelerating clinical translation and routine use by pathologists.
...
TL;DR: A scalable streaming VQA model that is no-reference and opinion-unaware, using temporal modeling to predict FR metrics from degraded video without references.
...
Method: Leverage synthetic degradations of DAVIS dataset to train temporal-aware convolutional architecture that predicts FR metrics (LPIPS, PSNR, SSIM) directly from degraded video without references.

Result: Streaming approach outperforms image-based baseline and generalizes across diverse degradations. Achieves higher correlation with FR metrics than BRISQUE baseline.

Conclusion: Temporal modeling is valuable for scalable VQA in real-world vision systems, and opinion-unaware approach with temporal awareness is effective.
...
TL;DR: Polarization-enabled eye tracking (PET) uses polarization-resolved near-infrared imaging to improve gaze tracking accuracy by revealing trackable features on ocular tissues that are invisible in intensity-only images.


<details>
  <summary>Details</summary>
Motivation: To enhance eye tracking by leveraging polarization contrast mechanisms that provide additional optical information beyond intensity, enabling more robust gaze tracking in various conditions.

Method: A PET system composed of a polarization-filter-array camera with linearly polarized near-infrared illuminator, combined with convolutional neural network models trained on data from 346 participants.
...
Conclusion: Polarization effects provide practical improvements for human-computer interaction and position PET as a simple, robust sensing modality for future wearable devices.
...
TL;DR: The paper proposes a diagnostic framework to identify and mitigate non-visual biases in multimodal benchmarks, using Test-set Stress-Test methodology and Iterative Bias Pruning to create more robust evaluations.


<details>
  <summary>Details</summary>
Motivation: Current multimodal benchmarks can be gamed without strong visual understanding due to biases, linguistic priors, and superficial patterns, undermining their reliability for evaluating true visual reasoning capabilities.

Method: Two-component framework: 1) Test-set Stress-Test (TsT) using k-fold cross-validation on textual inputs to reveal shortcuts and assign bias scores, plus Random Forest diagnostic; 2) Iterative Bias Pruning (IBP) to filter high-bias samples.

Result: Applied to four benchmarks (VSI-Bench, CV-Bench, MMMU, VideoMME), revealed pervasive non-visual biases. Created VSI-Bench-Debiased showing reduced non-visual solvability and wider vision-blind performance gap.

Conclusion: Benchmark designers should proactively game their own benchmarks using diagnostic procedures to identify and mitigate non-visual biases, ensuring more reliable evaluation of multimodal models' true visual understanding capabilities.
...
Result: Identified three key question categories (metric measurement, perspective-dependent reasoning, temporal tracking) that enable efficient training; 7B-parameter model fine-tuned on 25K simulated examples outperforms 72B baseline and achieves competitive performance with proprietary models on real-world spatial reasoning benchmarks.

Conclusion: Simulated data can effectively develop transferable spatial intelligence, enabling robust generalization while maintaining performance on general video understanding and showing substantial improvements on embodied and real-world spatial tasks.
...
TL;DR: The paper proposes a shift from reactive multimodal systems to 'supersensing' - a broader paradigm involving semantic perception, streaming event cognition, implicit 3D spatial cognition, and predictive world modeling. It introduces VSI-SUPER benchmark and shows that scale alone is insufficient, proposing predictive sensing as a solution.


<details>
  <summary>Details</summary>
Motivation: Current multimodal systems are reactive and task-driven with limited spatial cognition capabilities. Progress requires moving beyond linguistic-only understanding to true spatial supersensing that involves anticipating, selecting, and organizing experiences.

Method: Introduces VSI-SUPER benchmark with two tasks: VSR (long-horizon visual spatial recall) and VSC (continual visual spatial counting). Tests data scaling limits with VSI-590K dataset and trains Cambrian-S model. Proposes predictive sensing using self-supervised next-latent-frame predictor that leverages prediction error for memory and event segmentation.

Result: Achieved +30% absolute improvement on VSI-Bench without sacrificing general capabilities, but performance on VSI-SUPER remains limited. Predictive sensing approach substantially outperforms leading proprietary baselines on VSI-SUPER.

Conclusion: Scale alone is insufficient for spatial supersensing. True progress requires models that not only see but also anticipate, select, and organize experience through predictive sensing approaches.
...
TL;DR: InfinityStar is a unified spacetime autoregressive framework for high-resolution image and dynamic video synthesis that outperforms existing autoregressive models and some diffusion competitors while being 10x faster.
...
TL;DR: Track Any State: A zero-shot system called TubeletGraph that tracks objects through transformations while detecting and describing state changes, achieving SOTA performance on the new VOST-TAS benchmark.
...
Method: TubeletGraph identifies overlooked tracks and integrates them using semantic and proximity priors, then reasons about added tracks to generate state graphs describing transformations.
...
Conclusion: TubeletGraph effectively addresses the limitation of tracking objects through transformations and provides a comprehensive framework for understanding object state evolution over time.
...
TL;DR: This paper addresses the problem of generating multiple distinct aesthetic crops from images, motivated by social media applications. It introduces a dataset of 277 images with human labels and evaluates single-crop models with image partitioning as preprocessing.


<details>
  <summary>Details</summary>
Motivation: Modern social media applications require multiple distinct crops with aesthetic appeal, but existing work focuses mainly on producing singular crops rather than multiple distinct ones.

Method: The authors evaluate several single-crop models combined with an image partitioning algorithm as a preprocessing step to generate multiple crops.

Result: The paper introduces a dataset of 277 relevant images with human labels, which is made publicly available for research purposes.

Conclusion: The work provides a foundation for addressing the problem of multiple aesthetic image cropping and makes available a dataset to support further research in this area.
...
TL;DR: Deep learning models, particularly temporal convolutional networks, outperform traditional mechanistic models for forecasting Perennial Ryegrass growth, achieving RMSE of 2.74 and MAE of 3.46 using historical grass height data.


<details>
  <summary>Details</summary>
Motivation: Current grass growth forecasting relies on impractical mechanistic models, while the Irish dairy sector faces profitability and sustainability challenges. Grasslands are crucial carbon sinks and biodiversity habitats.

Method: Proposed deep learning models tailored for univariate datasets, with temporal convolutional networks specifically designed for forecasting Perennial Ryegrass growth using historical grass height data.

Result: The temporal convolutional network achieved high performance with RMSE of 2.74 and MAE of 3.46, validated across 1,757 weeks over 34 years, providing insights into optimal model configurations.

Conclusion: Deep learning models enhance reliability in grass growth forecasting, improve understanding of model behavior, and contribute to advancing sustainable dairy farming practices.
...
TL;DR: Federated learning framework for ECG classification using GAF image transformation achieves 95.18% accuracy while preserving privacy across IoT devices.


<details>
  <summary>Details</summary>
Motivation: To enable privacy-preserving ECG classification in IoT healthcare environments where sensitive medical data must remain local to devices rather than being centralized.
...
Result: Achieved 95.18% classification accuracy in multi-client setup, significantly outperforming single-client baseline in both accuracy and training time, while maintaining efficient resource utilization despite GAF computational complexity.
...
TL;DR: LOLGORITHM is a modular multi-agent system for generating stylistically diverse and context-aware comments on short-video platforms, supporting six comment styles and achieving over 90% preference rates.
...
Method: A modular multi-agent system integrating video segmentation, contextual/affective analysis, and style-aware prompt construction, powered by a multimodal LLM and using explicit prompt markers with few-shot examples.

Result: Outperforms baseline models with preference rates over 90% on Douyin and 87.55% on YouTube, as validated by both automated metrics and human evaluation involving 105 participants.

Conclusion: Provides a scalable and culturally adaptive framework for stylized comment generation that can enhance user engagement and creative interaction on short-video platforms.
...
TL;DR: Common-O benchmark reveals major gaps in multimodal models' scene reasoning despite strong perception performance, with best models achieving only 35% accuracy on basic reasoning and 1% on complex scenes.


<details>
  <summary>Details</summary>
Motivation: To address the gap between multimodal models' strong performance on perception benchmarks and their poor real-world reasoning abilities, particularly the problem of hallucinations when reasoning across scenes.

Method: Built Common-O benchmark with 10.5k examples using new images not in web training data, designed to probe reasoning across scenes by asking "what's in common?", inspired by cognitive tests for humans.

Result: Models perform well on single-image perception but struggle with cross-scene reasoning - best model achieves only 35% on Common-O and 1% on Common-O Complex. Models hallucinate more when similar objects are present, suggesting reliance on training co-occurrence patterns.

Conclusion: Current multimodal models have significant limitations in cross-scene reasoning despite perception capabilities. Multi-image training shows promise for improvement, and the benchmark is released to spur research on reducing hallucinations in scene reasoning.
...
TL;DR: Proposes a novel detection method using multi-modal semantic perturbation to identify test-set leakage in Vision-Language Models, showing contaminated models fail to generalize under controlled perturbations.


<details>
  <summary>Details</summary>
Motivation: Address the underexplored problem of detecting test-set leakage in VLMs, as existing detection approaches fail or behave inconsistently when models are contaminated with benchmark data.

Method: Deliberately contaminate open-source VLMs on popular benchmarks, then develop a detection method based on multi-modal semantic perturbation that reveals contaminated models' inability to generalize under controlled perturbations.

Result: The proposed detection method effectively identifies contaminated VLMs across multiple realistic contamination strategies, demonstrating robustness and effectiveness.

Conclusion: The multi-modal semantic perturbation approach provides a reliable way to detect test-set leakage in VLMs, addressing a critical gap in ensuring model evaluation integrity.
...
<details>
  <summary>Details</summary>
Motivation: Traditional DP-SGD applies privacy protection to all features equally, causing excessive noise injection and utility degradation. In real scenarios, only specific sensitive features (like demographic data) need privacy protection while other features (like lab results) are less sensitive.

Method: 1) Use large foundation models to impute sensitive features from non-sensitive features as external priors without accessing true sensitive values during training. 2) Modified DP-SGD algorithm that trains models on both original and imputed features while formally preserving privacy of original sensitive features.
...
Conclusion: FusionDP demonstrates the potential of foundation model-driven imputation to enhance the privacy-utility trade-off for various modalities, showing significant utility improvements while maintaining feature-level privacy protection.
...
### [70] [Complexity as Advantage: A Regret-Based Perspective on Emergent Structure](https://arxiv.org/abs/2511.04590)
*Oshri Naparstek*

Main category: cs.LG

TL;DR: CAA framework defines complexity as predictive regret differences across observers, showing systems are complex when some observers find them easy to model while others struggle, creating information advantages.


<details>
  <summary>Details</summary>
Motivation: To provide a unified framework for understanding complexity not as an intrinsic property but as observer-dependent, explaining why complex systems can be functionally valuable by creating differentiated predictive capabilities.

Method: Define complexity relative to observer families through predictive regret analysis, demonstrate with simple dynamical models, and connect to concepts like multiscale entropy and predictive information.

Result: Shows that complexity emerges from differentiated predictive regret across observers, unifying various notions of emergent behavior and providing quantitative grounding for complexity's functional value.

Conclusion: Complex systems gain advantage by creating information asymmetries where some observers can predict them better than others, with implications for learning, evolution, and artificial intelligence.
...
### [71] [Fair and Explainable Credit-Scoring under Concept Drift: Adaptive Explanation Frameworks for Evolving Populations](https://arxiv.org/abs/2511.03807)
*Shivogo John*

Main category: cs.LG

TL;DR: This paper develops adaptive explanation frameworks to maintain interpretability and fairness in credit-scoring systems under concept drift, showing improved stability and reduced disparate impact compared to static methods.


<details>
  <summary>Details</summary>
Motivation: Conventional explainability techniques like SHAP assume static data distributions, making their explanations unstable and potentially unfair when concept drift occurs in evolving credit models.

Method: Integrates XGBoost with three adaptive SHAP variants: per-slice explanation reweighting, drift-aware SHAP rebaselining with sliding-window samples, and online surrogate calibration using incremental Ridge regression.

Result: Adaptive methods, particularly rebaselined and surrogate-based explanations, substantially improve temporal stability and reduce disparate impact across demographic groups without degrading predictive accuracy.

Conclusion: Adaptive explainability provides a practical mechanism for sustaining transparency, accountability, and ethical reliability in evolving data-driven systems like credit scoring.
...
TL;DR: A routing method that assigns problems to the smallest suitable reasoning model, reducing compute while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Reasoning language models are effective but expensive due to their large size and long reasoning processes.

Method: Using intermediate representations from s1.1-32B to train lightweight predictors that estimate problem difficulty or model correctness, enabling intelligent routing across a pool of reasoning models.
...
### [73] [One Size Does Not Fit All: Architecture-Aware Adaptive Batch Scheduling with DEBA](https://arxiv.org/abs/2511.03809)
*François Belias,Naser Ezzati-Jivan,Foutse Khomh*

Main category: cs.LG

TL;DR: DEBA is an adaptive batch scheduler that monitors gradient variance, gradient norm variation and loss variation to guide batch size adaptations. It shows that architecture fundamentally determines adaptation efficacy, with lightweight architectures achieving 45-62% speedup and accuracy improvements, while deep networks show high variance.


<details>
  <summary>Details</summary>
Motivation: Existing adaptive batch size methods apply identical strategies across all architectures, assuming a one-size-fits-all solution, but this work challenges that assumption by showing architecture fundamentally determines adaptation efficacy.
...
Result: Lightweight architectures achieve 45-62% training speedup with 1-7% accuracy improvements. ResNet-18 shows +2.4-4.0% accuracy and 36-43% speedup. ViT-B16 shows minimal speedup (6%). Deep networks like ResNet-50 exhibit high variance and occasional degradation.

Conclusion: Adaptive batch size methods require architecture-aware design, challenging the prevailing assumption that they generalize across architectures. A baseline characterization framework using gradient stability metrics can predict which architectures will benefit from adaptive scheduling.
...
### [74] [Sketch-Augmented Features Improve Learning Long-Range Dependencies in Graph Neural Networks](https://arxiv.org/abs/2511.03824)
*Ryien Hosseini,Filippo Simini,Venkatram Vishwanath,Rebecca Willett,Henry Hoffmann*

Main category: cs.LG

TL;DR: Injecting randomized global embeddings called Sketched Random Features into GNNs helps capture long-range dependencies and addresses oversquashing, oversmoothing, and limited expressive power issues.
...
Method: Inject Sketched Random Features - randomized global embeddings of node features that are unique, distance-sensitive, and topology-agnostic - into standard GNNs.

Result: Experimental results show consistent performance improvements over baseline GNNs on real-world graph learning tasks, working both as standalone solution and complementary enhancement to existing techniques.

Conclusion: Sketched Random Features effectively alleviate GNN limitations by enabling efficient capture of long-range dependencies while maintaining the benefits of local message passing.
...
TL;DR: A score-based CUSUM method for quickest change detection in high-dimensional Markov processes with unknown transition kernels, using conditional score learning and truncated statistics to ensure theoretical guarantees.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of change detection in Markov processes with unknown transition kernels, especially in high-dimensional settings where explicit likelihood evaluation is impractical.

Method: Learn conditional scores ∇_y log p(y|x) directly from sample pairs, develop a score-based CUSUM procedure using conditional Hyvarinen score differences, and propose a truncated statistic for bounded increments.
...
Conclusion: The approach provides both theoretical guarantees and practical feasibility for score-based change detection in high-dimensional Markov models without requiring explicit likelihood evaluation.
...
TL;DR: StratDiff uses diffusion models and energy-based functions to stratify training samples into offline-like and online-like subsets, enabling smoother offline-to-online RL transitions by applying appropriate learning strategies to each subset.


<details>
  <summary>Details</summary>
Motivation: Address distributional shifts between offline datasets and evolving online policies in RL, and bridge the gap in adapting learning strategies to different sample types.

Method: Energy-Guided Diffusion Stratification (StratDiff) uses diffusion models to learn from offline data, refines with energy functions, computes KL divergence to stratify samples, and applies offline/online objectives accordingly.
...
Conclusion: StratDiff effectively facilitates smoother offline-to-online RL transitions by leveraging sample stratification and appropriate learning strategies.
...
TL;DR: Extends causal additive models to include higher-order interactions using directed acyclic hypergraphs, providing identifiability results and a greedy algorithm for learning these more complex structures.
...
Method: Extends causal additive models to handle higher-order interactions, introduces directed acyclic hypergraphs as representation, provides theoretical identifiability results, and develops a greedy CAM algorithm extension for hyper DAG search.

Result: Shows that learning hypergraph structure can lead to better empirical results, with more restrictive assumptions corresponding to easier-to-learn hyper DAGs and improved finite sample complexity.

Conclusion: The framework successfully extends causal structure learning to handle higher-order interactions, with theoretical guarantees and practical algorithms that demonstrate improved performance in synthetic experiments.
...
<details>
  <summary>Details</summary>
Motivation: DQN suffers from high variance due to using next states from potentially suboptimal past policies, which provides poor learning signals when transitions are misaligned with current policy.

Method: Proposes Successor-state Aggregation Deep Q-Network (SADQ) that uses a stochastic transition model to integrate successor-state distributions into Q-value estimation, enabling more stable updates and efficient action selection.

Result: SADQ consistently outperforms DQN variants in stability and learning efficiency across standard RL benchmarks and real-world vector-based control tasks, with theoretical guarantees of unbiased value estimates and reduced variance.

Conclusion: Explicitly modeling environment dynamics through successor-state distributions provides more stable and policy-aligned value updates, addressing key limitations of standard DQN approaches.
...
<details>
  <summary>Details</summary>
Motivation: Social platforms show patterns where early interactions (views, likes) predict later high-impact outcomes (citations, sales), but this hasn't been treated as a unified forecasting problem due to lack of standardized datasets.

Method: Created two benchmark datasets: arXiv (2.3M papers, accesses -> citations) and GitHub (3M repositories, pushes/stars -> forks), with detailed data curation and cleaning. Verified lead-lag dynamics statistically and benchmarked parametric/non-parametric regression baselines.

Result: Established LLF as a novel forecasting paradigm with ideal testbeds capturing long-horizon dynamics across years, spanning full outcome spectrum, and avoiding survivorship bias. Statistical tests confirmed presence of lead-lag patterns.

Conclusion: The study formalizes LLF as a distinct forecasting problem and provides empirical foundation for systematic exploration in social and usage data, with available data portal for further research.
...
<details>
  <summary>Details</summary>
Motivation: We bring this idea to hyperdimensional computing (HDC), where footprint cuts usually shrink the feature axis and erode concentration and robustness.

Method: We introduce DecoHD, which learns directly in a decomposed HDC parameterization: a small, shared set of per-layer channels with multiplicative binding across layers and bundling at the end, yielding a large representational space from compact factors.

Result: In evaluation, DecoHD attains extreme memory savings with only minor accuracy degradation under tight deployment budgets.

Conclusion: DecoHD is more robust to random bit-flip noise, reaches its accuracy plateau with up to ~97% fewer trainable parameters, and -- in hardware -- delivers roughly 277x/35x energy/speed gains over a CPU (AMD Ryzen 9 9950X), 13.5x/3.7x over a GPU (NVIDIA RTX 4090), and 2.0x/2.4x over a baseline HDC ASIC.
...
TL;DR: This paper presents a framework for predicting sociodemographic attributes from mobility data using behaviorally-grounded mobility descriptors, uncertainty quantification tools, and multitask learning to improve accuracy, interpretability, and generalization.
...
Method: Three approaches: 1) Behaviorally-grounded higher-order mobility descriptors based on directed mobility graphs capturing trip sequences, travel modes, and social co-travel; 2) Metrics and visual diagnostic tools for uncertainty quantification; 3) Multitask learning framework that jointly predicts multiple attributes from shared representations.

Result: The proposed features significantly improve prediction of age, gender, income, and household structure over baseline features. The multitask approach outperforms single-task models, especially with limited training data or when test distribution differs from training.

Conclusion: The framework addresses key challenges in sociodemographic inference from mobility data by improving predictive accuracy while maintaining interpretability, enabling uncertainty quantification, and enhancing generalization across contexts through multitask learning.
...
TL;DR: Introduces SynQuE framework for ranking synthetic datasets by expected real-world task performance using limited unannotated real data, with novel LENS proxy metric that outperforms others on complex tasks.


<details>
  <summary>Details</summary>
Motivation: Addresses critical challenge of data scarcity due to collection costs or privacy constraints, enabling better synthetic data selection when real data is limited.

Method: Establishes benchmarks with proxy metrics including distribution/diversity-based measures adapted via embedding models, and proposes LENS - a novel proxy leveraging large language model reasoning for complex tasks.

Result: SynQuE proxies correlate with real task performance across diverse tasks (sentiment analysis, Text2SQL, web navigation, image classification). LENS consistently outperforms others on complex tasks, e.g., raising Text2SQL accuracy from 30.4% to 38.4% (+8.1%) when selecting top-3 datasets.
...
TL;DR: Nemotron Nano V2 VL is an enhanced vision-language model for document understanding and video comprehension, featuring hybrid Mamba-Transformer architecture and token reduction techniques for improved performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: To create a more capable vision-language model that excels in real-world document understanding, long video comprehension, and reasoning tasks, building upon previous work to deliver significant improvements across all vision and text domains.

Method: Builds on Nemotron Nano V2 (hybrid Mamba-Transformer LLM) with innovative token reduction techniques, enhanced model architecture, improved datasets, and optimized training recipes.

Result: Delivers significant improvements over previous model (Llama-3.1-Nemotron-Nano-VL-8B) across all vision and text domains, achieving higher inference throughput in long document and video scenarios.

Conclusion: The model represents a major advancement in vision-language capabilities and is being released with model checkpoints in multiple formats (BF16, FP8, FP4) along with datasets, recipes, and training code.
...
TL;DR: LogHD reduces HDC memory from O(CD) to O(D log C) by replacing per-class prototypes with logarithmic bundle hypervectors, maintaining accuracy while improving robustness and efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard HDC requires O(CD) memory which is inefficient for constrained systems, and prior compaction methods reduce dimensionality at the cost of robustness.

Method: Uses logarithmic class-axis reduction with bundle hypervectors, capacity-aware codebook, profile-based decoding, and composes with feature-axis sparsification.

Result: Achieves competitive accuracy with smaller models, higher resilience at matched memory, 2.5-3.0x higher bit-flip tolerance, and significant ASIC efficiency gains (498x energy efficiency, 62.6x speedup vs CPU).

Conclusion: LogHD provides superior memory efficiency and robustness compared to feature-axis compression, making HDC more practical for resource-constrained systems.
...
TL;DR: This survey paper synthesizes the new frontier of alignment research beyond traditional RLHF, focusing on multi-modal alignment, cultural fairness, and low-latency optimization.


<details>
  <summary>Details</summary>
Motivation: To address critical gaps in current alignment research as RLHF progresses beyond canonical text-based methods, providing a roadmap for more robust, efficient, and equitable AI systems.

Method: Systematically reviews foundational algorithms (PPO, DPO, GRPO) and presents detailed analysis of latest innovations through comparative synthesis of alignment techniques.

Result: Provides a comprehensive synthesis of alignment techniques and identifies open challenges in the field.

Conclusion: This work serves as an essential roadmap for researchers working on multi-modal alignment, cultural fairness, and low-latency optimization in AI systems.
...
### [86] [PrivacyCD: Hierarchical Unlearning for Protecting Student Privacy in Cognitive Diagnosis](https://arxiv.org/abs/2511.03966)
*Mingliang Hou,Yinuo Wang,Teng Guo,Zitao Liu,Wenzhou Dou,Jiaqi Zheng,Renqiang Luo,Mi Tian,Weiqi Luo*

Main category: cs.LG

TL;DR: First systematic study of data unlearning for cognitive diagnosis models, proposing HIF algorithm that uses hierarchical importance guidance to efficiently remove specific student data while maintaining model utility.


<details>
  <summary>Details</summary>
Motivation: Address the growing need for data removal in CD models due to users' 'right to be forgotten', as existing models lack privacy considerations and effective unlearning mechanisms.

Method: Proposed HIF (hierarchical importance-guided forgetting) algorithm that leverages layer-wise parameter importance characteristics through a smoothing mechanism combining individual and layer-level importance.

Result: Experiments on three real-world datasets show HIF significantly outperforms baselines on key metrics, providing effective data removal while maintaining model performance.

Conclusion: HIF offers the first effective solution for CD models to respond to user data removal requests and enables deployment of privacy-preserving AI systems.
...
### [87] [Non-Asymptotic Optimization and Generalization Bounds for Stochastic Gauss-Newton in Overparameterized Models](https://arxiv.org/abs/2511.03972)
*Semih Cayci*
...
<details>
  <summary>Details</summary>
Motivation: To understand how higher-order optimization methods affect generalization in deep learning, specifically analyzing stochastic Gauss-Newton with Levenberg-Marquardt damping for overparameterized networks.

Method: Stochastic Gauss-Newton method with Levenberg-Marquardt damping and mini-batch sampling for training overparameterized deep neural networks with smooth activations in regression setting.

Result: Established finite-time convergence bounds via variable-metric analysis and derived non-asymptotic generalization bounds using uniform stability in overparameterized regime.
...
### [88] [PETRA: Pretrained Evolutionary Transformer for SARS-CoV-2 Mutation Prediction](https://arxiv.org/abs/2511.03976)
*Xu Zou*

Main category: cs.LG

TL;DR: PETRA is a novel transformer model that uses evolutionary trajectories from phylogenetic trees instead of raw RNA sequences to predict SARS-CoV-2 mutations, achieving significantly better performance than baselines.


<details>
  <summary>Details</summary>
Motivation: SARS-CoV-2's rapid and unpredictable evolution with immune-evasive variants poses challenges to public health and vaccine development. Direct application of large-scale GPT models to noisy viral genomic sequences is limited.

Method: PETRA uses evolutionary trajectories derived from phylogenetic trees rather than raw RNA sequences, with a weighted training framework to address geographical and temporal imbalances in global sequence data.

Result: PETRA achieved weighted recall@1 of 9.45% for nucleotide mutations and 17.10% for spike amino-acid mutations, compared to 0.49% and 6.64% respectively for the best baseline. It also demonstrated real-time mutation prediction capability for major clades.

Conclusion: PETRA effectively mitigates sequencing noise and captures hierarchical viral evolution structure, providing superior mutation prediction performance for SARS-CoV-2 evolution tracking.
...
### [89] [Structural Priors and Modular Adapters in the Composable Fine-Tuning Algorithm of Large-Scale Models](https://arxiv.org/abs/2511.03981)
*Yuxiao Wang,Di Wu,Feng Liu,Zhimin Qiu,Chenrui Hu*

Main category: cs.LG

TL;DR: A composable fine-tuning method using graph structural priors and modular adapters to improve multi-task adaptation efficiency and stability for large pre-trained models.


<details>
  <summary>Details</summary>
Motivation: Address high computational cost and structural instability in multi-task adaptation of large-scale pre-trained models.

Method: Integrates graph structural priors via relation matrix to model task dependencies, with modular adapters using low-rank mapping and pluggable mechanism for efficient cross-task composition.
...
Conclusion: Demonstrates synergistic advantages of graph priors and modular mechanisms in composable fine-tuning, with verified consistency across various sensitivity analyses.
...
### [90] [TwIST: Rigging the Lottery in Transformers with Independent Subnetwork Training](https://arxiv.org/abs/2511.03983)
*Michael Menezes,Barbara Su,Xinze Feng,Yehya Farhat,Hamza Shili,Anastasios Kyrillidis*

Main category: cs.LG

TL;DR: TwIST is a distributed training framework that identifies high-quality sparse subnetworks during training, enabling zero-cost pruning at deployment without post-processing.


<details>
  <summary>Details</summary>
Motivation: To enable efficient deployment of sparse LLMs without the overhead of post-training procedures like calibration or Hessian-based recovery.

Method: Trains multiple subnetworks in parallel, periodically aggregates their parameters, and resamples new subnetworks during training to identify optimal sparse structures.

Result: Achieves competitive perplexity with state-of-the-art methods, especially under aggressive sparsity (50%+), with 23.14 PPL vs 31.64 for prior approaches, while producing structured dense matrices for practical inference speedups.

Conclusion: TwIST provides an efficient training-time path to deployable sparse LLMs that work well on commodity hardware without additional fine-tuning or recovery overhead.
...
### [91] [Use of Continuous Glucose Monitoring with Machine Learning to Identify Metabolic Subphenotypes and Inform Precision Lifestyle Changes](https://arxiv.org/abs/2511.03986)
*Ahmed A. Metwally,Heyjun Park,Yue Wu,Tracey McLaughlin,Michael P. Snyder*

Main category: cs.LG

TL;DR: CGM and wearables enable dynamic metabolic phenotyping to identify distinct dysglycemia subtypes beyond traditional diabetes classifications, allowing personalized prevention strategies.
...
Method: Use continuous glucose monitoring and wearable technologies with machine learning models to analyze high-resolution glucose data from at-home CGM-enabled oral glucose tolerance tests and real-world nutrition responses.

Result: Machine learning can predict gold-standard measures of muscle IR and beta-cell function; postprandial glycemic responses serve as biomarkers for metabolic subtypes; wearable data reveals lifestyle patterns associated with specific metabolic dysfunctions.

Conclusion: CGM can deconstruct early dysglycemia complexity into actionable subphenotypes, enabling targeted nutritional, behavioral, and pharmacological strategies for precision diabetes prevention.
...
### [92] [Multiscale Astrocyte Network Calcium Dynamics for Biologically Plausible Intelligence in Anomaly Detection](https://arxiv.org/abs/2511.03993)
*Berk Iskar,Michael Taynnan Barros*

Main category: cs.LG

TL;DR: A biologically-inspired Ca²⁺-modulated learning framework for network anomaly detection that addresses concept drift and new threats by simulating astrocytic Ca²⁺ signaling dynamics.


<details>
  <summary>Details</summary>
Motivation: Traditional offline-trained network anomaly detectors are vulnerable to concept drift and new threats like zero-day attacks, requiring adaptive solutions.

Method: Couples a multicellular astrocyte dynamics simulator with a DNN, modeling Ca²⁺ dynamics through IP₃-mediated release, SERCA pump uptake, and gap junction diffusion.

Result: Achieves ~98% accuracy on CTU-13 network traffic data, outperforming baseline DNN with reduced false positives/negatives and negligible runtime overhead after precomputation.

Conclusion: The Ca²⁺-modulated framework provides a generic solution for streaming detection tasks requiring rapid, biologically grounded adaptation to evolving data patterns.
...
### [93] [Towards Scalable Meta-Learning of near-optimal Interpretable Models via Synthetic Model Generations](https://arxiv.org/abs/2511.04000)
*Kyaw Hpone Myint,Zhe Wu,Alexandre G. R. Day,Giri Iyengar*

Main category: cs.LG

TL;DR: Efficient synthetic data generation method for meta-learning decision trees that achieves comparable performance to real data pre-training with significantly lower computational costs.


<details>
  <summary>Details</summary>
Motivation: Decision trees are crucial in high-stakes domains for interpretability, but meta-learning them requires large datasets which are computationally expensive to obtain using optimal decision trees.
...
### [94] [Accelerating scientific discovery with the common task framework](https://arxiv.org/abs/2511.04001)
...
TL;DR: A common task framework (CTF) is introduced to evaluate ML/AI algorithms in science and engineering using standardized challenge datasets and metrics for objectives like forecasting, state reconstruction, generalization, and control.


<details>
  <summary>Details</summary>
Motivation: There is a critical need for objective metrics to compare diverse ML/AI algorithms being rapidly developed and deployed across science and engineering, especially given limited data scenarios and noisy measurements.

Method: Introduces a common task framework (CTF) featuring a growing collection of challenge data sets with diverse practical objectives, similar to frameworks used in traditional ML applications like speech recognition and computer vision.

Result: The CTF provides standardized evaluation metrics and datasets to enable fair comparison of ML/AI algorithms across different scientific domains and objectives.

Conclusion: A common task framework is critically needed to advance ML/AI applications in science and engineering by providing objective evaluation standards similar to those that enabled progress in traditional ML domains.
...
### [95] [Memory- and Latency-Constrained Inference of Large Language Models via Adaptive Split Computing](https://arxiv.org/abs/2511.04002)
*Mingyu Sung,Vikas Palakonda,Suhwan Im,Sunghwan Moon,Il-Min Kim,Sangseok Yun,Jae-Mo Kang*

Main category: cs.LG

TL;DR: This paper introduces an autoregressive-aware split computing framework for deploying large language models on edge devices, addressing memory and communication challenges through strategic model partitioning and compression techniques.


<details>
  <summary>Details</summary>
Motivation: LLMs achieve near-human reasoning performance but are impractical for resource-constrained IoT devices due to massive parameter footprints and memory-intensive autoregressive decoding. Existing split computing approaches fail to address the unique challenges of autoregressive inference.

Method: Proposes three key contributions: 1) One-point split compression (OPSC) for mixed-precision quantization and strategic model partitioning, 2) Two-stage intermediate compression pipeline combining threshold splitting and token-wise adaptive bit quantization, 3) Unified optimization framework for joint selection of split points, quantization settings, and sequence lengths.
...
Conclusion: The proposed autoregressive-aware split computing framework enables practical deployment of LLMs on edge devices by effectively addressing memory constraints and communication bottlenecks through optimized partitioning and compression strategies.
...
### [96] [Enhancing Multimodal Protein Function Prediction Through Dual-Branch Dynamic Selection with Reconstructive Pre-Training](https://arxiv.org/abs/2511.04040)
...
Method: Uses reconstructive pre-training for fine-grained information, Bidirectional Interaction Module for multimodal feature learning, and Dynamic Selection Module for hierarchical multi-label classification.

Result: Significant improvements in BPO, MFO, and CCO on human datasets, outperforming other benchmark models.
...
TL;DR: DartQuant is an efficient distribution-aware rotational calibration method that accelerates quantization of large models by constraining activation distributions after rotation, achieving 47x speedup and 10x memory savings on 70B models.
...
Method: Proposes DartQuant with distribution-aware rotational calibration that reduces rotational optimization complexity by constraining activation distributions, and introduces QR-Orth optimization scheme to replace expensive alternating optimization.

Result: Achieves 47x acceleration and 10x memory savings for rotational optimization on 70B models, and successfully completes rotational calibration for 70B model on single 3090 GPU - the first method to do so.

Conclusion: DartQuant makes quantization of large language models feasible in resource-constrained environments by providing superior performance with significantly reduced computational requirements.
...
Method: Fine-tuned ResNet architecture on Regensburg Pediatric Appendicitis Dataset with ultrasound scans, using preprocessing (normalization, resizing, augmentation) for classification between appendicitis and non-appendicitis cases.

Result: Model achieved 93.44% accuracy, 91.53% precision, and 89.8% recall, demonstrating strong performance in identifying appendicitis across heterogeneous ultrasound views.

Conclusion: The ResNet model effectively learned discriminative spatial features and overcame challenges of low contrast, speckle noise, and anatomical variability in pediatric ultrasound imaging.
...
TL;DR: The study applied nnU-Net framework to automatically segment left atrium from cardiac MRI, achieving high accuracy (93.5 Dice score) and outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Manual LA segmentation is time-consuming and observer-dependent, making it impractical for clinical workflows. Deep learning methods offer superior performance for medical image segmentation.

Method: Used nnU-Net framework on Left Atrial Segmentation Challenge 2013 dataset (30 MRI scans with expert masks). The model automatically adapted preprocessing, network configuration, and training pipeline to MRI data characteristics.

Result: Achieved mean Dice score of 93.5, showing high overlap with expert annotations and outperforming traditional segmentation approaches. Demonstrated robust generalization across variations in LA shape, contrast, and image quality.

Conclusion: nnU-Net provides accurate and automated LA segmentation from cardiac MRI, offering a practical solution for clinical applications like AF ablation guidance and biophysical model construction.
...
TL;DR: Proposes a learned approach for filtered approximate nearest neighbor search that adapts distance functions to data distributions instead of using fixed penalties, improving accuracy by 5-10%.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based methods use fixed, data-independent penalties for filter awareness, which fail to generalize across datasets with diverse label and vector distributions.

Method: Formulates filtered ANN search as a constrained linear optimization problem to learn optimal trade-offs between vector distance and filter match directly from data, using learned weights to guide search and index construction.

Result: Experiments show the method significantly improves accuracy by 5-10% over fixed-penalty methods across diverse datasets.

Conclusion: Learning the optimal trade-off between vector distance and filter match from data provides a more flexible and generalizable framework for filtered ANN search.
...
TL;DR: DeNoise is a robust unsupervised graph-level anomaly detection framework that handles contaminated training data by using adversarial learning, encoder anchor-alignment denoising, and contrastive learning to create noise-resistant graph embeddings.


<details>
  <summary>Details</summary>
Motivation: Most Graph Neural Network approaches assume clean training data with only normal graphs, but real-world datasets often contain anomalous graphs that can distort learned representations and degrade performance.

Method: Joint optimization of graph-level encoder, attribute decoder, and structure decoder via adversarial objective; encoder anchor-alignment denoising mechanism that fuses node embeddings from normal graphs; contrastive learning to compact normal embeddings and repel anomalous ones.
...
TL;DR: The paper introduces KoTaP, a Korean Tax Avoidance Panel dataset covering 2011-2024 with 12,653 firm-year observations from 1,754 non-financial firms, designed to study corporate tax avoidance as a predictor variable across multiple business domains.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive panel dataset that treats corporate tax avoidance as a predictor variable and links it to various business domains including earnings management, profitability, stability, growth, and governance, while reflecting unique Korean institutional features.

Method: Constructed a balanced panel dataset by collecting data from non-financial firms listed on KOSPI and KOSDAQ (2011-2024), excluding financial firms, non-December fiscal year ends, capital impairment, and negative pre-tax income. Tax avoidance is measured using CETR, GETR, TSTA, and TSDA indicators with adjustments for interpretability.

Result: Created KoTaP with 12,653 firm-year observations from 1,754 firms, featuring standardized variables, international comparability, and distinctive Korean institutional characteristics like concentrated ownership, high foreign shareholding, and elevated liquidity ratios.
...
<details>
  <summary>Details</summary>
Motivation: Most symbolic regression methods prioritize prediction error over identifying governing equations, often producing overly complex or inaccurate expressions that lack interpretability.

Method: Uses Multi-Set Transformer to generate univariate symbolic skeletons, evaluates them with GA-based selection, merges them via GP-based cascade while preserving structure, and optimizes coefficients with GA.
...
Conclusion: The proposed decomposable SR method successfully generates interpretable multivariate expressions that accurately capture underlying mathematical relationships while maintaining structural fidelity.
...
TL;DR: This paper explores using Large Language Models as end-to-end compilers (LaaC), finding they have basic compilation capabilities but low success rates, which can be improved through prompt optimization, model scaling, and reasoning methods.


<details>
  <summary>Details</summary>
Motivation: While LLMs have been used to assist compiler development, their potential as complete end-to-end compilers remains largely unexplored despite their advantages in various domains.

Method: Created CompilerEval dataset and framework to evaluate LLMs' source code comprehension and assembly generation capabilities, analyzed various errors, and tested multiple improvement methods including prompt optimization, model scaling, and reasoning techniques.

Result: LLMs demonstrate basic compilation capabilities but currently achieve low compilation success rates. However, quality of generated assembly code can be significantly enhanced through optimization techniques.

Conclusion: The authors maintain an optimistic outlook for LaaC, proposing that with targeted training, knowledge-rich prompts, and specialized infrastructure, LLMs have potential to generate high-quality assembly code and drive paradigm shift in compilation field.
...
TL;DR: EPO is a safe RL framework for semi-infinite constraint problems that iteratively solves subproblems with finite constraint sets and adaptively adjusts active constraints through expansion and deletion.


<details>
  <summary>Details</summary>
Motivation: Many practical safe RL applications involve infinite constraints across continuous parameter spaces, requiring methods that can handle semi-infinite constraints while maintaining safety bounds.

Method: EPO iteratively solves safe RL subproblems with finite constraint sets, adaptively adding constraints that violate tolerance and removing those with zero Lagrange multipliers after policy updates.

Result: Theoretical analysis shows EPO-trained strategies achieve performance comparable to optimal solutions while keeping global constraint violations strictly within prescribed bounds.

Conclusion: EPO provides an effective algorithmic framework for semi-infinite safe RL that maintains optimal performance with deterministic bounded safety through adaptive constraint management.
...
<details>
  <summary>Details</summary>
Motivation: Many secondary and regional airports face severe trajectory data scarcity, limiting machine learning applications and simulation capabilities in Air Traffic Management.
...
Result: Diffusion-based models achieve competitive performance with 5% of Dublin data and reach baseline performance at 20%, consistently outperforming models trained from scratch across metrics.

Conclusion: Transfer learning can substantially reduce data requirements for trajectory generation in ATM, enabling realistic synthetic data generation even in data-scarce environments.
...
TL;DR: Transformer-based method for clinical risk classification using heterogeneous EHR data with irregular temporal patterns, achieving superior performance over traditional models.
...
Method: Uses feature embedding for unified representation, learnable temporal encoding for uneven sampling, multi-head self-attention for global dependency modeling, semantic-weighted pooling for adaptive importance assignment, and linear mapping for risk scores.
...
Conclusion: Provides stable and precise risk identification in multi-source heterogeneous EHR environments and offers an efficient framework for clinical intelligent decision-making.
...
<details>
  <summary>Details</summary>
Motivation: To investigate the benefits of jointly tuning ensemble components (weight decay, temperature scaling, early stopping) rather than tuning them individually, and to address the practical challenge of data usage for joint evaluation.

Method: Proposes joint tuning of ensemble hyperparameters and introduces a partially overlapping holdout strategy that allows joint evaluation while maximizing training data usage.

Result: Joint tuning generally matches or improves performance compared to individual tuning, with effect sizes varying across tasks and metrics. The overlapping holdout strategy provides a practical compromise solution.

Conclusion: Joint optimization of deep ensembles offers performance benefits, and the overlapping holdout strategy presents an attractive practical approach for ensemble training optimization.
...
TL;DR: ScaleDL is a runtime prediction framework for deep neural networks that combines nonlinear layer-wise modeling with GNN-based cross-layer interactions, achieving 6x lower MRE and 5x lower RMSE than baselines while reducing data collection costs.


<details>
  <summary>Details</summary>
Motivation: As DNN models grow larger and more complex, accurate runtime prediction becomes crucial for optimizing development and resource allocation. Traditional methods have limited accuracy and generalizability, while graph-enhanced methods increase data collection costs significantly.

Method: Proposes ScaleDL framework that combines nonlinear layer-wise modeling with graph neural network (GNN)-based cross-layer interaction mechanism, and employs D-optimal method to reduce data collection costs.

Result: Experiments on five popular DNN models show ScaleDL achieves 6x lower Mean Relative Error (MRE) and 5x lower Root Mean Square Error (RMSE) compared to baseline models, with improved generalizability across different network architectures.

Conclusion: ScaleDL effectively balances accuracy, generalizability, and data collection costs for DNN runtime prediction, providing a practical solution for optimizing distributed computing resource allocation in modern AI services.
...
TL;DR: This paper analyzes post-training quantization methods for LLMs using the new MXFP4 format, finding that rotation-based methods are incompatible while GPTQ performs well. The authors propose a block rotation strategy to adapt rotation methods to MXFP4.


<details>
  <summary>Details</summary>
Motivation: Large language models are growing rapidly in scale, creating prohibitive costs in memory, computation, and energy. While post-training quantization offers efficient deployment, achieving accurate W4A4 quantization remains challenging, especially with the emergence of new formats like MXFP4 that have hardware support from major vendors.

Method: The authors establish a comprehensive benchmark of PTQ methods under MXFP4 format, systematically evaluate existing approaches, analyze the incompatibility between rotation-based methods and MXFP4's PoT block scaling, and propose a simple block rotation strategy to adapt rotation-based methods to MXFP4.

Result: GPTQ consistently delivers strong performance with MXFP4, while rotation-based approaches suffer severe incompatibility. The proposed block rotation strategy leads to substantial accuracy improvements across diverse LLMs when adapting rotation-based methods to MXFP4.

Conclusion: The findings provide clear guidance for practitioners and set a foundation for advancing PTQ research under emerging low-precision formats. The analysis reveals a fundamental mismatch between MXFP4's PoT block scaling and the redistribution of outlier energy via global rotation, which the proposed block rotation strategy effectively addresses.
...
TL;DR: This paper proves the strong lottery ticket hypothesis for transformer architectures, specifically for multi-head attention mechanisms, showing that high-performing subnetworks exist in randomly initialized transformers.


<details>
  <summary>Details</summary>
Motivation: The strong lottery ticket hypothesis (SLTH) has been established for various neural architectures but lacks theoretical understanding for transformers, particularly the multi-head attention mechanism which is a core component.

Method: Theoretical analysis proving that randomly initialized multi-head attention with sufficient hidden dimension contains strong lottery tickets that can approximate arbitrary MHAs. Extended this to transformers without normalization layers.

Result: Proved that with hidden dimension O(d log(Hd^{3/2})) for key and value, a randomly initialized MHA contains an SLT that approximates an arbitrary MHA with high probability. Empirical validation shows exponential decrease in approximation error with increased hidden dimension.

Conclusion: The strong lottery ticket hypothesis holds for transformer architectures, providing theoretical foundation for the existence of high-performing subnetworks in randomly initialized transformers with multi-head attention.
...
TL;DR: seqme is a Python library for evaluating biological sequence design methods with model-agnostic metrics across sequence, embedding, and property dimensions.


<details>
  <summary>Details</summary>
Motivation: Lack of a unified software library implementing metrics to evaluate computational methods for designing biological sequences, despite recent advances in the field.

Method: Developed seqme - a modular, extendable open-source Python library containing three groups of metrics: sequence-based, embedding-based, and property-based metrics applicable to various biological sequences.

Result: Created a comprehensive library that offers embedding models, property models, diagnostics, and visualization functions, supporting evaluation of both one-shot and iterative design methods.

Conclusion: seqme provides a unified framework for evaluating biological sequence design methods, addressing the gap in available evaluation tools for this rapidly advancing field.
...
<details>
  <summary>Details</summary>
Motivation: Time series classification is critical in safety-critical applications but current deep learning methods are black-box and lack interpretability, making it hard to understand prediction rationales.

Method: Introduces a novel STL-inspired kernel that maps raw time series to their alignment with predefined STL formulae, jointly optimizing accuracy and interpretability through neuro-symbolic embedding.

Result: STELLE achieves competitive accuracy while providing logically faithful explanations, validated on diverse real-world benchmarks.

Conclusion: The framework successfully unifies classification and explanation through temporal logic embedding, offering both local (individual prediction) and global (class-characterizing) human-readable explanations.
...
TL;DR: A hybrid framework combining RLHF's scalability with PBO's sample efficiency through active querying for preference learning.


<details>
  <summary>Details</summary>
Motivation: Collecting human preference data is costly and time-consuming, creating need for more efficient learning paradigms that leverage complementary advantages of existing approaches.

Method: Integrates an acquisition-driven module into RLHF pipeline to enable active and sample-efficient preference gathering, unifying RLHF's scalability with PBO's query efficiency.

Result: Validated on high-dimensional preference optimization and LLM fine-tuning, showing consistent improvements in both sample efficiency and overall performance.

Conclusion: Proposed hybrid framework successfully combines strengths of RLHF and PBO for more efficient preference learning from human judgments.
...
TL;DR: DP-ICL framework with privacy-aware nearest neighbor search outperforms existing methods by better privacy-utility trade-offs across text classification and QA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing DP-ICL approaches overlook privacy risks in similarity search for context retrieval, which is critical in modern LLM pipelines.

Method: Nearest neighbor retrieval from context database with privacy filter that tracks cumulative privacy cost to ensure central DP budget adherence.

Result: Outperforms existing baselines by substantial margin across all evaluated benchmarks with more favorable privacy-utility trade-offs.

Conclusion: The proposed DP framework with privacy-aware nearest neighbor search shows clear advantage over existing methods in text classification and document question answering.
...
TL;DR: A novel Gibbs sampling method for learning Dynamic Bayesian Networks from incomplete clinical data, providing principled imputation and uncertainty estimation with superior accuracy compared to standard methods.


<details>
  <summary>Details</summary>
Motivation: Existing missing data approaches for longitudinal clinical data fail to properly account for temporal relationships, limiting uncertainty quantification over time which is critical for clinical decision-making in settings like intensive care.

Method: Gibbs sampling-based approach that treats missing values as unknown parameters following Gaussian distributions, sampling unobserved values from their full conditional distributions at each iteration.

Result: Superior reconstruction accuracy and convergence properties compared to standard model-agnostic techniques like MICE, demonstrated on both simulated datasets and real-world intensive care data.

Conclusion: The Bayesian approach provides more reliable imputations and deeper insight into model behavior, supporting safer clinical decision-making in settings with frequent missing data.
...
TL;DR: SCER is a novel embedding regularization method that suppresses spurious correlations to improve worst-group robustness by theoretically constraining feature representations to focus on core features rather than spurious patterns.


<details>
  <summary>Details</summary>
Motivation: Deep learning models often rely on spurious correlations, making them vulnerable to distribution shifts, especially in subpopulation shift scenarios where models struggle with underrepresented groups. Existing methods lack rigorous theoretical frameworks connecting embedding space representations with worst-group error.

Method: SCER directly regularizes feature representations to suppress spurious cues by identifying core and spurious directions from group-wise mean embedding differences across domains and classes, then imposing theoretical constraints at the embedding level.
...
Conclusion: By theoretically constraining embeddings to reduce reliance on spurious patterns and focus on core features, SCER effectively improves model robustness to subpopulation shifts and achieves superior worst-group performance.
...
TL;DR: Current LLM uncertainty quantification methods fail on ambiguous data, performing close to random despite working well on unambiguous tasks. The authors introduce ambiguous QA datasets and show theoretical limitations of existing approaches.


<details>
  <summary>Details</summary>
Motivation: Real-world language is inherently ambiguous, but existing uncertainty quantification methods are only tested on unambiguous tasks, creating a gap between research and practical deployment needs.

Method: Introduced MAQA* and AmbigQA* datasets with ground-truth answer distributions from factual co-occurrence. Tested three estimation paradigms: predictive distribution, internal representations, and model ensembles on ambiguous data.

Result: All uncertainty estimators degraded to near-random performance on ambiguous data, revealing consistent failure across different estimation approaches. Theoretical analysis showed fundamental limitations of predictive-distribution and ensemble-based methods under ambiguity.

Conclusion: Current UQ methods have critical shortcomings for handling ambiguity in LLMs, necessitating a rethinking of modeling paradigms to address real-world language uncertainty.
...
TL;DR: This paper establishes a formal equivalence between regression and classification, showing that regression with M samples on a hyperplane is equivalent to a linearly separable classification task with 2M samples, leading to new regression formulations and applications.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between regression and classification by establishing a formal mathematical equivalence, as the connection has been tenuous despite shared concepts like margin maximization in support vector regression.

Method: The authors demonstrate that regression problems with M samples on a hyperplane have a one-to-one equivalence with linearly separable classification tasks with 2M samples, and use this equivalence to derive new regression formulations and applications.

Result: The equivalence enables margin maximization on the classification task to yield different regression formulations, introduces a 'regressability' measure to estimate dataset difficulty without model training, and allows training neural networks to learn linearizing maps.

Conclusion: The paper provides a formal mathematical foundation connecting regression and classification, enabling new regression formulations, dataset difficulty assessment, and neural network training approaches for linearization.
...
TL;DR: ForecastGAN is a novel decomposition-based adversarial framework for multi-horizon time series forecasting that outperforms transformers in short-term predictions while remaining competitive for long-term horizons.


<details>
  <summary>Details</summary>
Motivation: Transformer models excel in long-term forecasting but often underperform in short-term scenarios and typically ignore categorical features, creating a need for a more balanced and comprehensive approach.

Method: Three integrated modules: Decomposition Module (extracts seasonality/trend), Model Selection Module (identifies optimal neural network configurations), and Adversarial Training Module (enhances robustness through Conditional GAN training).

Result: Outperforms state-of-the-art transformer models on 11 benchmark datasets for short-term forecasting while remaining competitive for long-term horizons, effectively integrating both numerical and categorical features.
...
<details>
  <summary>Details</summary>
Motivation: Heavy-tailed noise is more realistic than standard bounded variance assumptions in stochastic optimization, especially for federated learning applications where gradient noise distributions may be heavy-tailed.

Method: Fed-NSGDA-M uses normalized gradients while FedMuon-DA leverages the Muon optimizer for local updates. Both are designed specifically to handle heavy-tailed noise in federated minimax optimization.

Result: Both algorithms achieve theoretical convergence rate of O(1/(TNp)^((s-1)/(2s))) under heavy-tailed noise conditions. Extensive experiments validate their effectiveness.

Conclusion: These are the first federated minimax optimization algorithms with rigorous theoretical guarantees under heavy-tailed noise, addressing a more realistic noise assumption in federated learning.
...
Result: Validated on synthetic autoregressive models inspired by Ornstein-Uhlenbeck process, achieving superior counterfactual probability estimation with L1 distances of 0.03-0.10 compared to ground truth.
...
TL;DR: SynthKGQA is a framework for generating synthetic Knowledge Graph Question Answering datasets from any Knowledge Graph, enabling better benchmarking of KG retrievers and training improved models.
...
Method: Developed SynthKGQA framework that generates synthetic KGQA datasets from any Knowledge Graph, providing full ground-truth facts for reasoning over each question.

Result: Created GTSQA dataset from Wikidata to test zero-shot generalization of KG retrievers for unseen graph structures and relation types, and benchmarked popular KG-augmented LLM solutions.

Conclusion: SynthKGQA enables more informative benchmarking of KG retrievers and helps train better models by providing high-quality synthetic datasets with complete ground-truth information.
...
Method: Proposes Quadratic Reweighted Rank Regularizer (Q3R) inspired by iteratively reweighted least squares framework, using a quadratic regularizer that majorizes a smoothed log determinant serving as rank surrogate objective.

Result: Achieves comparable predictive performance as dense models with small computational overhead, demonstrated by truncating 60-80% of ViT-Tiny parameters with only 1.3-4% accuracy drop on CIFAR-10. Effective across Transformers for both image and language tasks.

Conclusion: Q3R enables training weight matrices with prescribed low target ranks while maintaining performance, remaining fully compatible with existing architectures and effective for both pre-training and fine-tuning.
...
TL;DR: Data-informed neural network compression using tensorization with covariance-based norms that minimize function space error instead of weight space error, achieving competitive accuracy without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Neural networks demand high computing power, and existing compression methods use isotropic norms in weight space that don't account for actual data distribution, often requiring post-compression fine-tuning.

Method: Propose alternating least square algorithms for Tucker-2 and CPD tensor decompositions that minimize $\lVert (W - \widetilde{W}) \Sigma^{1/2}\rVert_F$ where $\Sigma^{1/2}$ is the square root of input covariance matrix, directly optimizing function space error.

Result: Achieves competitive accuracy without fine-tuning, works across multiple CNN architectures (ResNet-18/50, GoogLeNet) and datasets (ImageNet, FGVC-Aircraft, Cifar10/100), and covariance norms transfer between datasets with minor accuracy drop.

Conclusion: Data-informed compression with covariance-based norms provides effective neural network compression without requiring fine-tuning or original training data, enabling practical deployment of compressed models.
...
TL;DR: The paper proposes a modified group fairness approach for criminal justice that minimizes weighted error loss while keeping false negative rate differences within tolerance, and provides a practical deployment framework.


<details>
  <summary>Details</summary>
Motivation: Algorithmic fairness concepts remain unsettled in criminal justice, with conflicts between different fairness definitions. There's a need for practical, deployable fairness solutions that balance competing objectives.

Method: Modified group fairness approach that minimizes weighted error loss while constraining false negative rate differences within a small tolerance. Also proposes a deployment framework with three pillars: need-based decisions, transparency/accountability, and narrowly tailored solutions.

Result: The proposed method makes solutions easier to find, can improve predictive accuracy, and explicitly surfaces ethical choices about error costs. It addresses critiques about biased data, latent affirmative action, and subgroup constraint explosion.

Conclusion: The paper provides a practical framework linking technical design to legitimacy in public decision systems, offering actionable guidance for agencies using risk assessment tools with balanced fairness approaches.
...
TL;DR: This paper studies linear mode connectivity (LMC) under data shifts, showing that small learning rates and large batch sizes can mitigate data shift effects by reducing gradient noise, influencing whether models converge to similar or different loss basins.


<details>
  <summary>Details</summary>
Motivation: To understand how linear mode connectivity relates to deep learning aspects like training stability, generalization, and model diversity, and specifically to investigate LMC's behavior under data shift conditions.

Method: Experimental study of LMC under data shifts, interpreting data shifts as gradient noise sources, and analyzing the effects of learning rates and batch sizes on model convergence patterns.

Result: Small learning rates and large batch sizes reduce the impact of data shifts by decreasing gradient noise, affecting whether models converge to the same local minimum or to regions with different smoothness and generalization properties.

Conclusion: LMC provides a trade-off between training efficiency and ensemble diversity benefits, with models connected via LMC making more similar errors but offering computational advantages over larger ensembles.
...
TL;DR: Benchmarking study comparing B-EPGP surrogate with CN-FEM for 2D wave equation, showing B-EPGP achieves ~2 orders of magnitude better accuracy under matched degrees of freedom.


<details>
  <summary>Details</summary>
Motivation: To compare the performance of a novel boundary-constrained Ehrenpreis-Palamodov Gaussian Process surrogate against classical finite element methods for solving the two-dimensional wave equation with homogeneous Dirichlet boundary conditions.
...
Result: Under matched DoF, B-EPGP consistently achieves lower space-time L²-error and maximum-in-time L²-error in space than CN-FEM, improving accuracy by approximately two orders of magnitude.

Conclusion: The B-EPGP surrogate demonstrates superior performance over classical CN-FEM for solving the 2D wave equation, offering significantly higher accuracy while maintaining computational fairness through DoF matching.
...
TL;DR: The paper demonstrates that reinforcement learning-based Koopman surrogate models scale effectively to large-scale air separation units for economic nonlinear model predictive control, achieving similar economic performance as system identification-based approaches while avoiding constraint violations.


<details>
  <summary>Details</summary>
Motivation: To extend the previously proposed reinforcement learning method for Koopman surrogate models from small-scale case studies to more challenging large-scale industrial applications, specifically demand response scenarios with limited observable variables.
...
Result: The method scales well to the large-scale case study, delivering similar economic performance as system identification-based Koopman eNMPC while successfully avoiding constraint violations that frequently occurred with the purely system identification-based approach.

Conclusion: Reinforcement learning-based Koopman surrogate models are effective for large-scale industrial eNMPC applications, providing robust constraint satisfaction while maintaining economic performance, even with limited observability of plant variables.
...
### [130] [Uncertainty Quantification for Reduced-Order Surrogate Models Applied to Cloud Microphysics](https://arxiv.org/abs/2511.04534)
...
Result: Demonstrated on a latent space dynamical model for cloud microphysics, accurately predicting droplet-size distribution evolution and quantifying uncertainty across the ROM pipeline.
...
### [131] [Integrating Temporal and Structural Context in Graph Transformers for Relational Deep Learning](https://arxiv.org/abs/2511.04557)
*Divyansha Lachi,Mahmoud Mohammadi,Joe Meyer,Vinam Arora,Tom Palczewski,Eva L. Dyer*

Main category: cs.LG

TL;DR: RGP is a graph transformer that integrates temporal and structural contexts using cross-attention, supporting multi-task learning on relational data.


<details>
  <summary>Details</summary>
Motivation: Existing graph models focus on spatial structure and treat temporal data as filtering constraints, lacking support for multi-task prediction and global context integration.

Method: Temporal subgraph sampler for capturing long-range dependencies; Relational Graph Perceiver (RGP) with cross-attention latent bottleneck and flexible decoder for multi-task learning.

Result: State-of-the-art performance on RelBench, SALT, and CTU benchmarks, demonstrating scalability and effectiveness across diverse tasks.

Conclusion: RGP provides a general, scalable solution for relational deep learning that effectively integrates temporal dynamics and supports multiple predictive tasks.
...
### [132] [ARETE: an R package for Automated REtrieval from TExt with large language models](https://arxiv.org/abs/2511.04573)
...
<details>
  <summary>Details</summary>
Motivation: Lack of machine-readable species occurrence data and the need for faster data processing due to human impact on biodiversity.

Method: Developed ARETE R package using chatGPT API for automated data extraction, including OCR, outlier detection, and tabular output, validated against human annotation.

Result: Extracted data expanded known species ranges by 3 orders of magnitude for 100 spider species, revealing new historical occurrence areas.

Conclusion: ARETE enables faster access to untapped occurrence data, improving resource prioritization and project planning for conservation initiatives.
...
<details>
  <summary>Details</summary>
Motivation: Multi-agent systems are crucial for applications like swarm robotics and traffic routing, but learning in decentralized multi-agent stochastic shortest path problems remains largely unexplored compared to single-agent settings.

Method: Study decentralized multi-agent SSPs under linear function approximation, applying novel symmetry-based arguments to identify optimal policy structure and constructing hard-to-learn instances.

Result: Established the first regret lower bound of Ω(√K) over K episodes for any number of agents, demonstrating inherent learning difficulty in Dec-MASSPs.

Conclusion: The results clarify learning complexity of decentralized control and provide guidance for designing efficient learning algorithms in multi-agent systems.
...
### [134] [Addressing divergent representations from causal interventions on neural networks](https://arxiv.org/abs/2511.04638)
*Satchel Grant,Simon Jerome Han,Alexa Tartaglini,Christopher Potts*

Main category: cs.LG

TL;DR: Causal interventions in mechanistic interpretability often create out-of-distribution representations, potentially compromising explanation faithfulness. The paper identifies harmless vs pernicious divergences and proposes a modified regularization method to mitigate harmful effects.
...
Result: Common causal interventions do shift internal representations away from natural distributions. The modified CL loss reduces harmful divergences while preserving interpretive power.
...
### [135] [Environment Agnostic Goal-Conditioning, A Study of Reward-Free Autonomous Learning](https://arxiv.org/abs/2511.04598)
*Hampus Åström,Elin Anna Topp,Jacek Malec*

Main category: cs.LG

TL;DR: Transforming RL environments into goal-conditioned setups enables autonomous, reward-free learning where agents select their own goals, achieving training times comparable to externally guided RL.


<details>
  <summary>Details</summary>
Motivation: To enable agents to learn tasks autonomously without external rewards by creating goal-conditioned environments where agents can self-select goals.

Method: Transform regular RL environments into goal-conditioned environments, allowing agents to select their own goals in an environment-agnostic way, independent of the underlying off-policy learning algorithm.

Result: Agents achieve comparable training times to externally guided RL, with average goal success rate improving and stabilizing, though individual goal performance may be unstable due to equal goal valuation.

Conclusion: This approach enables generic agent training prior to specific use cases, allowing agents to be instructed to seek any observations in the environment.
...
<details>
  <summary>Details</summary>
Motivation: To develop efficient probabilistic forecasting methods for dynamical systems described by PDEs (like Navier-Stokes equations) by reducing the number of sampling steps required in flow matching approaches.

Method: Investigates and compares multiple flow matching extensions: direct distillation, progressive distillation, adversarial diffusion distillation, Wasserstein GANs, and rectified flows. Experiments conducted on challenging systems including 2D slices of large-scale 3D simulations.

Result: The paper provides comparative analysis of different flow matching techniques for efficient sampling in PDE-based forecasting.

Conclusion: The study paves the way for efficient inflow generation for solvers by enabling direct prediction of 2D slices from large-scale 3D simulations using accelerated flow matching methods.
...
TL;DR: This paper analyzes the parallel sampling capabilities of masked diffusion language models, providing exact characterization of sampling divergence and novel bounds based on information-theoretic properties.


<details>
  <summary>Details</summary>
Motivation: Standard auto-regressive LLMs have sequential inference causing long inference times. Diffusion language models like MDM can sample tokens in parallel, but there's limited understanding of how much parallel sampling is possible without performance degradation.

Method: The authors develop an exact characterization of expected divergence between true and sampled distributions for any distribution and unmasking schedule, connecting it to univariate function approximation theory. They derive novel lower and upper bounds using information-theoretic properties.

Result: The paper shows that optimal unmasking schedules require strong a priori knowledge of the distribution, but also demonstrates that in natural settings, one can sample in O(log n) steps without visible performance loss, where n is sequence length.

Conclusion: The work provides rigorous theoretical understanding of parallel sampling limits in diffusion language models, establishing connections to function approximation and information theory, with practical implications for efficient inference.
...
TL;DR: The paper proposes adaptive model pruning for time-triggered federated learning to reduce communication overhead while maintaining model performance.


<details>
  <summary>Details</summary>
Motivation: Federated learning faces challenges with growing user devices having limited wireless bandwidth, leading to stragglers and high communication overhead.

Method: Introduces adaptive model pruning to TT-Fed systems, jointly optimizes pruning ratio and bandwidth allocation using convergence analysis and KKT conditions.

Result: Simulation results show model pruning reduces communication cost by 40% while maintaining model performance.

Conclusion: Adaptive model pruning effectively addresses communication bottlenecks in federated learning systems without sacrificing model quality.
...
TL;DR: A 3D gray-box nowcasting framework that combines physical constraints with data-driven learning to forecast extreme precipitation with high accuracy up to 3 hours ahead, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing approaches (NWP, deep-learning emulations, 2D radar methods) have limitations: too slow/coarse for convection, error accumulation, excessive smoothing, and loss of crucial vertical information needed for accurate height-dependent dynamics.

Method: Hybrid framework processing volumetric radar reflectivity with physically constrained neural operators. Learns 3D advection fields, parameterizes diffusion, adds stochastic term for unresolved motions, includes residual branch for small-scale features, and diffusion-based uncertainty estimation.
...
Conclusion: The framework successfully restores full 3D dynamics with physical consistency, providing a scalable and robust pathway for skillful and reliable extreme precipitation nowcasting.
...
TL;DR: A unified theory of forgetting as lack of self-consistency in predictive distributions, with a general measure for algorithm forgetting propensity, validated across multiple learning settings.


<details>
  <summary>Details</summary>
Motivation: Address the fundamental challenge of catastrophic forgetting in general learning algorithms and provide a principled understanding of forgetting dynamics, as no unified definition currently exists.

Method: Proposed an algorithm- and task-agnostic theory characterizing forgetting as loss of predictive information due to lack of self-consistency in predictive distributions. Designed comprehensive experiments across classification, regression, generative modeling, and reinforcement learning.

Result: Empirical demonstration that forgetting is present across all learning settings and significantly impacts learning efficiency. The theory yields a general measure of algorithm forgetting propensity.

Conclusion: Establishes a principled understanding of forgetting and provides foundation for analyzing and improving information retention capabilities of general learning algorithms.
...
TL;DR: A multi-method analysis of a 40-item math placement exam reveals excellent discrimination for most items, identifies Question 6 as the strongest discriminator, shows high ML prediction accuracy (97.5%), and suggests lowering the placement threshold from 55% to 42.5% to reduce overclassification into remedial categories.


<details>
  <summary>Details</summary>
Motivation: To optimize mathematics placement examinations by evaluating item quality, identifying effective discriminators, and establishing evidence-based placement thresholds using a comprehensive multi-method framework.

Method: Combined Classical Test Theory, machine learning (Random Forest, Gradient Boosting), and unsupervised clustering (K-means) to analyze a 40-item placement exam administered to 198 students.

Result: 55% of items showed excellent discrimination, Question 6 was the strongest discriminator, ML algorithms achieved 96-97.5% accuracy, and clustering revealed a natural competency boundary at 42.5% (vs. institutional 55%), suggesting potential overclassification.

Conclusion: Multi-method integration provides robust evidence for placement optimization, recommending item replacement, two-stage assessment, and ML integration with transparency to improve mathematics placement decisions.
...
### [142] [Environment Division Multiple Access (EDMA): A Feasibility Study via Pinching Antennas](https://arxiv.org/abs/2511.03820)
*Zhiguo Ding,Robert Schober,H. V. Poor*

Main category: cs.IT

TL;DR: EDMA is a new multiple access technique that reconfigures wireless propagation environments using pinching antennas to improve signal strength and suppress interference without complex signal processing.


<details>
  <summary>Details</summary>
Motivation: To develop a multiple access technique that leverages dynamic wireless propagation environments to improve multi-user communications without requiring complex signal processing like precoding or beamforming.

Method: Uses pinching antennas placed at specific locations to reconfigure line-of-sight links, intentionally blocking interference links. Analyzes ergodic sum-rate gain and develops low-complexity optimization algorithms for pinching antenna placement.

Result: Derived closed-form expressions for ergodic sum-rate gain over conventional multiple access and probability of achieving larger instantaneous sum rates. Developed optimization algorithms that show near-optimal performance compared to exhaustive searches.

Conclusion: EDMA demonstrates significant potential for supporting multi-user communications by intelligently reconfiguring propagation environments, offering performance gains without complex signal processing requirements.
...
### [143] [Which Similarity-Sensitive Entropy?](https://arxiv.org/abs/2511.03849)
*Phuc Nguyen,Josiah Couch,Rahul Bansal,Alexandra Morgan,Chris Tam,Miao Li,Rima Arnaout,Ramy Arnaout*

Main category: cs.IT

TL;DR: Comparison of two similarity-sensitive entropy measures: Leinster-Cobbold-Reeve (LCR) method vs Vendi score (VS), showing they capture complementary information and differ significantly in magnitude.


<details>
  <summary>Details</summary>
Motivation: To understand how LCR and VS compare as similarity-sensitive entropy measures, determine which is preferable, and analyze their conceptual and practical differences.

Method: Conceptual, analytical, and experimental comparison using 53 machine-learning datasets, examining how both measures depend on similarity scaling via 'half distance' parameterization.

Result: LCR and VS differ by orders of magnitude and capture complementary information except in limiting cases; VS provides an upper bound on LCR for several Rényi-Hill order parameters.

Conclusion: VS is preferable only for systems with linear combinations of 'ur-elements' or quantum-mechanical character; LCR is favored for general similarity information capture, though both can complement each other at certain half-distances.
...
### [144] [Efficient and rate-optimal list-decoding in the presence of minimal feedback: Weldon and Slepian-Wolf in sheep's clothing](https://arxiv.org/abs/2511.04088)
*Pranav Joshi,Daniel McMorrow,Yihan Zhang,Amitalok J. Budkuley,Sidharth Jaggi*

Main category: cs.IT

TL;DR: First schemes achieving near-optimal rates for adversarial channels with any q≥2 using low-rate feedback, with efficient encoding/decoding and small list-size guarantees.


<details>
  <summary>Details</summary>
Motivation: To enable reliable communication at information-theoretically optimal rates over adversarial channels with arbitrary alphabet size q≥2, where previous constructions only worked for large q or lacked computational tractability.

Method: Develop minimal feedback schemes using vanishing feedback rate O(1/log n), with parameters optimized for any q≥2 and corruption fraction ρ∈(0,1-1/q).

Result: Achieves rate 1-H_q(ρ)-ε (near-optimal), list-size exp(O(ε^{-3/2}log²(1/ε))), encoding/decoding complexity n^{O(ε^{-1}log(1/ε))}, storage O(n^{η+1}log n), error probability O(n^{-η}), and feedback rate O(1/log n).

Conclusion: The proposed schemes demonstrate that near-capacity communication is achievable for any alphabet size q≥2 using minimal feedback, with practical computational and storage requirements.
...
### [145] [List Decoding of Folded Reed-Solomon Codes Over Galois Ring](https://arxiv.org/abs/2511.04135)
*Chen Yuan,Ruiqi Zhu*

Main category: cs.IT

TL;DR: Extends list decoding of Reed-Solomon codes to Galois rings, achieving radius 1-√r for RS codes and reaching Singleton bound for folded RS codes with improved list size.


<details>
  <summary>Details</summary>
Motivation: Growing demand from zero knowledge systems to study codes over Galois rings, as current understanding of RS codes over these rings is limited and hinders development of ring-based arithmetic circuit proof systems.

Method: Extends Guruswami-Sudan list decoding procedure to Reed-Solomon codes over Galois rings, then investigates folded RS codes over Galois rings and improves list size using techniques from recent work.

Result: RS codes with rate r can be list decoded up to radius 1-√r over Galois rings; folded RS codes achieve Singleton bound decoding radius; list size improved to O(1/ε²).

Conclusion: Successfully extends list decoding theory to Galois rings, providing important tools for zero knowledge proof systems and bridging the gap between finite field and ring-based coding theory.
...
### [146] [Affine Frequency Division Multiplexing: From Communication to Sensing](https://arxiv.org/abs/2511.04471)
*Ali Bemani,Nassar Ksairi,Marios Kountouris*

Main category: cs.IT

TL;DR: AFDM waveform enables low-complexity ISAC systems with efficient self-interference cancellation, sub-Nyquist sampling, and flexible resource assignment for multiradar interference mitigation.


<details>
  <summary>Details</summary>
Motivation: Address two key challenges in integrated sensing and communication (ISAC) systems: maintaining low receiver complexity/energy consumption with large bandwidths, and mitigating interference in multiradar environments.

Method: Leverage AFDM waveform features including compatibility with low-complexity self-interference cancellation schemes, analog dechirping for reduced sampling rates, sub-Nyquist sampling without hardware modifications, and flexible resource assignment via discrete affine Fourier transform (DAFT).

Result: AFDM-based ISAC receivers can achieve efficient monostatic sensing with direct transmitter-receiver leakage cancellation, bistatic sensing with preserved delay resolution using sub-Nyquist sampling, and multiradar interference mitigation through flexible resource assignment.

Conclusion: AFDM provides effective solutions for ISAC system challenges through its unique waveform properties, enabling practical implementation of integrated sensing and communication with acceptable complexity and interference management.
...
### [147] [Age of Job Completion Minimization with Stable Queues](https://arxiv.org/abs/2511.04630)
*Stavros Mitrolaris,Subhankar Banerjee,Sennur Ulukus*

Main category: cs.IT

TL;DR: This paper studies a job-assignment system with a Markov machine that changes states, proposing policies to minimize job completion age and sampling cost while maintaining queue stability.


<details>
  <summary>Details</summary>
Motivation: To maximize the number of jobs completed per unit time in a system with a central server, multiple users, and a Markov machine that alternates between free and busy states, while considering sampling costs.

Method: Proposed two policies that schedule users and submit jobs when the machine is in free state, introducing a new metric called 'age of job completion' to optimize performance.

Result: Numerically evaluated the performance of the proposed policies and found sufficient conditions under which the job queues remain stable for both policies.

Conclusion: The proposed policies effectively minimize job completion age and sampling cost while ensuring queue stability in the Markov machine job-assignment system.
...
### [148] [Correlation and Temporal Consistency Analysis of Mono-static and Bi-static ISAC Channels](https://arxiv.org/abs/2511.03837)
*Saúl Fenollosa,Narcis Cardona,Wenfei Yang,Jian Li*

Main category: eess.SP

TL;DR: Empirical 79-GHz measurements reveal mono-static and bi-static sensing channels have low instantaneous correlation but unified temporal consistency in dynamic urban environments.


<details>
  <summary>Details</summary>
Motivation: Existing channel models lack comprehensive characterization of ISAC-specific dynamics, particularly the relationship between mono-static and bi-static sensing configurations in future wireless networks like 6G.

Method: Empirical measurements using a 79-GHz FMCW channel sounder in dynamic urban microcell (UMi) environments across seven real-world scenarios with moving targets/transceivers.

Result: Two key findings: (1) mono-static and bi-static channels exhibit consistently low instantaneous correlation due to divergent propagation geometries; (2) both channels share unified temporal consistency, evolving predictably under environmental kinematics.

Conclusion: These insights inform robust ISAC system design and future standardization for efficient spectrum and hardware utilization in 6G networks.
...
### [149] [Adaptive Phase Shift Information Compression for IRS Systems: A Prompt Conditioned Variable Rate Framework](https://arxiv.org/abs/2511.03923)
*Xianhua Yu,Dong Li,Bowen Gu,Liuqing Yang,Sumei Sun,George K. Karagiannidis*

Main category: eess.SP

TL;DR: A prompt-conditioned PSI compression system for IRS that integrates prompt learning to enable adaptive encoding across diverse SNRs, fading types, and compression ratios using a single model.


<details>
  <summary>Details</summary>
Motivation: Current deep learning-based compression methods for IRS phase shift information suffer from high decoder complexity, inadequate flexibility to dynamic channels, and static compression ratios, hindering practical implementation.

Method: Proposes hybrid prompt technique combining soft prompt concatenation with FiLM, variable rate technique with latent masking for compression ratios, and lightweight depthwise convolutional gating decoder for minimal complexity.

Result: Significantly reduces NMSE compared to traditional autoencoder baselines while ensuring robustness across various channel conditions and accommodating variable compression ratios within a single model.

Conclusion: The framework shows promise as a scalable and efficient solution for real-time IRS control in next-generation wireless networks.
...
### [150] [Score-Based Quickest Change Detection and Fault Identification for Multi-Stream Signals](https://arxiv.org/abs/2511.03967)
*Wuxia Chen,Sean Moushegian,Vahid Tarokh,Taposh Banerjee*

Main category: eess.SP

TL;DR: Proposes min-SCUSUM, a Hyvarinen score-based method for multi-stream quickest change detection and fault isolation, replacing traditional likelihood ratio calculations with score function differences to handle complex models.


<details>
  <summary>Details</summary>
Motivation: Traditional change detection algorithms require explicit pre- and post-change distributions for likelihood ratio calculations, which is computationally expensive for high-dimensional data and infeasible for complex ML models.

Method: min-SCUSUM algorithm uses Hyvarinen score-based approach that computes differences of score functions instead of log-likelihood ratios for change detection and fault isolation in multi-stream systems.

Result: Asymptotic performance depends on Fisher divergence between pre- and post-change distributions, with established upper bound on probability of fault misidentification when distinguishing affected streams.

Conclusion: The proposed score-based approach provides an effective alternative to traditional change detection methods, enabling efficient detection and isolation in complex statistical models without explicit distribution knowledge.
...
### [151] [Joint Beamforming and Position Design for Movable Antenna Assisted LEO ISAC Systems](https://arxiv.org/abs/2511.03984)
*Hanfu Zhang,Erwu Liu*

Main category: eess.SP

TL;DR: This paper proposes a movable antenna (MA)-assisted LEO satellite ISAC system to overcome signal attenuation and power limitations, achieving better communication-sensing trade-offs with at least 25% sensing performance gain.


<details>
  <summary>Details</summary>
Motivation: LEO satellite ISAC systems face performance degradation due to severe signal attenuation and limited transmit power, requiring innovative solutions to improve both communication and sensing capabilities.

Method: Joint optimization of transmit beamforming and MA positions using semidefinite relaxation (SDR) and alternating optimization (AO) algorithm to minimize sensing error under communication quality constraints.

Result: The proposed algorithm achieves convergence and effectiveness, demonstrating better trade-off between communication and sensing performance with at least 25% gain in sensing performance compared to benchmarks.

Conclusion: MA-assisted LEO ISAC systems with optimized beamforming and antenna positioning can significantly enhance ISAC performance, overcoming power and attenuation limitations in satellite communications.
...
### [152] [Optimal RIS Placement in a Multi-User MISO System with User Randomness](https://arxiv.org/abs/2511.03998)
*Abhishek Rajasekaran,Mehdi Karbalayghareh,Xiaoyan Ma,David J. Love,Christopher G. Brinton*

Main category: eess.SP

TL;DR: Optimizing RIS placement for maximum expected minimum SINR considering user randomness and obstacle configuration using a recursive coarse-to-fine search method.


<details>
  <summary>Details</summary>
Motivation: Previous RIS placement methods assume known user locations, but in practice only user density and obstacle information is available prior to deployment, requiring a more realistic optimization approach.

Method: Proposed a recursive coarse-to-fine methodology that constructs candidate RIS locations based on obstacle configuration and evaluates them over multiple user distribution instantiations, with recursive refinement in optimal regions.

Result: Numerical results validate the effectiveness of the proposed approach for determining optimal RIS deployment locations.

Conclusion: The method successfully addresses the practical challenge of RIS placement optimization when only statistical user information and obstacle data are available, providing an effective solution for real-world deployment scenarios.
...
### [153] [A Survey on Noise-Based Communication](https://arxiv.org/abs/2511.04011)
*Higo T. P. Da Silva,Hugerles S. Silva,Felipe A. P. Figueiredo,Andre A. Dos Anjos,Rausley A. A. Souza*

Main category: eess.SP

TL;DR: Survey of noise-based communication technologies for 6G and IoT, covering thermal noise modulation, noise modulation variants, and KLJN secure key exchange, with applications in SWIPT and NOMA.


<details>
  <summary>Details</summary>
Motivation: 6G networks and massive IoT demand ultra-low-power, secure, and covert wireless communication technologies that traditional deterministic carriers cannot provide.

Method: Comprehensive synthesis of noise-based communication principles and methodologies including thermal noise modulation (TherMod), noise modulation (NoiseMod) variants, and Kirchhoff-law-Johnson-noise (KLJN) secure key exchange.

Result: Noise-based systems offer unparalleled advantages in energy efficiency and covertness, with practical applications in simultaneous wireless information and power transfer (SWIPT) and non-orthogonal multiple access (NOMA).

Conclusion: Noise-based communication represents a transformative paradigm for enabling next-generation autonomous and secure wireless networks, with future research needed to realize its full potential.
...
### [154] [Tiny-WiFo: A Lightweight Wireless Foundation Model for Channel Prediction via Multi-Component Adaptive Knowledge Distillation](https://arxiv.org/abs/2511.04015)
*Haotian Zhang,Shijian Gao,Xiang Cheng*

Main category: eess.SP

TL;DR: MCAKD framework enables efficient deployment of wireless foundation models on edge devices by selectively distilling critical features and balancing knowledge transfer with independent learning.


<details>
  <summary>Details</summary>
Motivation: The massive scale of wireless foundation models hinders their real-time deployment on edge devices, requiring efficient compression methods.

Method: Multi-Component Adaptive Knowledge Distillation with Cross-Attention-Based Knowledge Selection and Autonomous Learning-Passive Learning strategy.

Result: Tiny-WiFo model with 5.5M parameters achieves 1.6 ms inference time while retaining over 98% of WiFo's performance and zero-shot generalization capability.

Conclusion: MCAKD makes real-time foundation model deployment viable on edge hardware through efficient knowledge distillation.
...
### [155] [Ambiguity Function Analysis of AFDM Under Pulse-Shaped Random ISAC Signaling](https://arxiv.org/abs/2511.04200)
*Yuanhan Ni,Fan Liu,Haoran Yin,Yanqun Tang,Zulin Wang*

Main category: eess.SP

TL;DR: Analysis of AFDM waveform's ambiguity function for ISAC, showing AFDM can flexibly control sidelobe depressions via parameter c1, unlike OFDM and OCDM, and pulse shaping improves mainlobe and sidelobe characteristics.


<details>
  <summary>Details</summary>
Motivation: To investigate AFDM's ambiguity function for ISAC applications and understand how its parameters affect sensing performance compared to OFDM and OCDM waveforms.

Method: Derived closed-form expressions for average squared discrete period AF of AFDM with/without pulse shaping, compared AF characteristics of AFDM, OFDM, and OCDM, and proposed parameter design to mitigate depression effects.

Result: AFDM's AF depends on parameter c1 and data kurtosis but not c2; all waveforms show regular sidelobe depressions affecting weak target detection; AFDM can flexibly control depression positions via c1; pulse shaping creates shaped mainlobe and rapid roll-off sidelobes.

Conclusion: AFDM offers flexible control over AF characteristics through parameter c1, enabling better weak target detection in ISAC systems, with pulse shaping further enhancing performance through improved mainlobe and sidelobe properties.
...
### [156] [BTTDA: Block-Term Tensor Discriminant Analysis for Brain-Computer Interfacing](https://arxiv.org/abs/2511.04292)
*Arne Van Den Kerchove,Hakim Si-Mohammed,François Cabestaing,Marc M. Van Hulle*

Main category: eess.SP

TL;DR: BTTDA is a novel tensor-based supervised feature extraction method that outperforms traditional approaches in EEG-based brain-computer interfaces, achieving state-of-the-art performance in ERP decoding (91.25% ROC-AUC).


<details>
  <summary>Details</summary>
Motivation: Current tensor-based approaches like Tucker and PARAFAC decompositions lack flexibility to fully capture EEG data complexity, necessitating more adaptable methods for effective classification of brain signals.

Method: Extends Higher Order Discriminant Analysis (HODA) with a novel forward model and deflation scheme to iteratively extract discriminant block terms, providing flexible multilinear dimensionality reduction.

Result: BTTDA significantly outperforms HODA in both ERP (91.25% ROC-AUC vs traditional methods) and MI decoding (64.52% vs 61.00%), achieving state-of-the-art performance in ERP classification.

Conclusion: BTTDA offers interpretable and efficient dimensionality reduction without compromising discriminative power, providing a promising adaptable approach for BCI and neuroimaging applications.
...
### [157] [RCMCL: A Unified Contrastive Learning Framework for Robust Multi-Modal (RGB-D, Skeleton, Point Cloud) Action Understanding](https://arxiv.org/abs/2511.04351)
*Hasan Akgul,Mari Eplik,Javier Rojas,Akira Yamamoto,Rajesh Kumar,Maya Singh*

Main category: eess.SP

TL;DR: RCMCL is a self-supervised framework for robust human action recognition that handles modality failures through cross-modal contrastive learning, self-distillation, and degradation simulation, achieving state-of-the-art performance with minimal degradation under severe modality dropout.


<details>
  <summary>Details</summary>
Motivation: Traditional multi-modal HAR systems rely on large labeled datasets and suffer significant performance degradation when sensors fail or produce noisy data, limiting real-world deployment reliability.

Method: Joint optimization of three objectives: cross-modal contrastive learning for modality alignment, intra-modal self-distillation for view-invariance, and degradation simulation for recovery training. Uses Adaptive Modality Gating for robust fusion.

Result: Achieves state-of-the-art accuracy on NTU RGB+D 120 and UWA3D-II datasets. Under severe dual-modality dropout, shows only 11.5% degradation, significantly outperforming supervised fusion baselines.

Conclusion: Self-supervised cross-modal alignment combined with explicit degradation modeling and adaptive fusion enables deployable multi-modal HAR systems that remain reliable under real-world sensor failures.
...
### [158] [High-Resolution Forest Mapping from L-Band Interferometric SAR Time Series using Deep Learning over Northern Spain](https://arxiv.org/abs/2511.04362)
*Chiara Telli,Oleg Antropov,Anne Lönnqvist,Marco Lavalle*

Main category: eess.SP

TL;DR: High-resolution forest mapping using L-band SAR interferometric time series and deep learning models improves forest height retrieval accuracy compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of L-band interferometric SAR time series combined with deep learning for accurate forest height mapping, suitable for future satellite missions like NISAR and ROSE-L.

Method: Used ALOS-2 PALSAR-2 dual-pol SAR time series with airborne laser scanning reference data. Tested various UNet-family models (Vanilla UNet, SeU-Net with attention, nested UNet) with different combinations of polarimetric and interferometric features including model-based InSAR features and coherence layers.

Result: Adding model-based InSAR features or coherence layers improved accuracy over backscatter intensity alone. Attention mechanisms and nested connections outperformed Vanilla UNet and traditional ML. Best RMSE was 1.95 m at 60m resolution using all features, compared to 2.2 m with intensity only.

Conclusion: Hybrid approach combining L-band SAR interferometric time series with advanced deep learning models provides superior forest height retrieval performance and is recommended for future missions.
...
### [159] [A Lightweight Framework for Integrated Sensing and Communications with RIS](https://arxiv.org/abs/2511.04448)
*Chu Li,Kevin Weinberger,Aydin Sezgin*

Main category: eess.SP

TL;DR: A lightweight RIS phase design framework for ISAC systems that provides closed-form solutions with low computational complexity, balancing communication and sensing performance trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing RIS optimization methods for ISAC systems suffer from high computational complexity (SDR) or suboptimal solutions (iterative algorithms), especially with large numbers of RIS elements.

Method: Partition RIS configuration into two parts: first part maximizes communication performance, second part introduces small perturbations to generate multiple beams for multi-target sensing, explicitly accounting for communication-sensing trade-off.

Result: Simulation results show the approach achieves performance comparable to SDR but with significantly lower computational complexity.

Conclusion: The proposed lightweight framework provides an efficient closed-form solution for RIS phase design in ISAC systems, effectively balancing communication and sensing requirements with low computational overhead.
...
### [160] [An Area-Efficient 20-100-GHz Phase-Invariant Switch-Type Attenuator Achieving 0.1-dB Tuning Step in 65-nm CMOS](https://arxiv.org/abs/2511.04635)
*Qingbin Li,Jian Pang*

Main category: eess.SP

TL;DR: A 20-100 GHz switch-type attenuator using capacitive compensation and metal line resistors achieves 7.5 dB attenuation range with low RMS errors (0.15 dB amplitude, 1.6° phase) in 65nm CMOS.


<details>
  <summary>Details</summary>
Motivation: To develop a wideband attenuator (20-100 GHz) with minimized amplitude and phase errors while reducing chip area, addressing the challenges of parasitic capacitance in high-frequency applications.

Method: Uses capacitive compensation technique to reduce phase error, implements small resistors with metal lines to minimize parasitic capacitance, employs continuous tuning attenuation unit for improved accuracy, fabricated in standard 65nm CMOS process.

Result: Achieves 7.5 dB relative attenuation range with continuous tuning from 20-100 GHz, insertion loss of 1.6-3.8 dB, return losses better than 11.5 dB, RMS amplitude error <0.15 dB, RMS phase error <1.6°.

Conclusion: The proposed attenuator successfully demonstrates wideband operation with excellent amplitude and phase accuracy using metal line resistors and capacitive compensation, making it suitable for high-frequency applications in standard CMOS technology.
...
### [161] [Scaling Agent Learning via Experience Synthesis](https://arxiv.org/abs/2511.03773)
...
TL;DR: DreamGym is a unified framework that synthesizes diverse experiences for scalable online RL training of autonomous agents, using reasoning-based experience models instead of expensive real-environment rollouts.


<details>
  <summary>Details</summary>
Motivation: Practical adoption of RL for LLM agents is challenging due to costly rollouts, limited task diversity, unreliable rewards, and infrastructure complexity, which obstruct scalable experience data collection.
...
Result: DreamGym substantially improves RL training across diverse environments, outperforming baselines by over 30% on non-RL-ready tasks like WebArena, and matches GRPO/PPO performance using only synthetic interactions in RL-ready settings.

Conclusion: DreamGym provides a scalable warm-start strategy for general-purpose RL, enabling significant performance gains with far fewer real-world interactions through synthetic experience training.
...
### [162] [How Different Tokenization Algorithms Impact LLMs and Transformer Models for Binary Code Analysis](https://arxiv.org/abs/2511.03825)
...
<details>
  <summary>Details</summary>
Motivation: Tokenization is fundamental in assembly code analysis but remains underexplored, creating a gap in understanding how NLP tokenization models perform with low-level code characteristics.

Method: Systematic study of various tokenization models with intrinsic evaluations (tokenization efficiency, vocabulary compression, representational fidelity) using pre-trained models like Llama 3.2, BERT, and BART across multiple performance metrics.

Result: Tokenizer choice significantly influences downstream performance, with intrinsic metrics providing partial but incomplete predictability of extrinsic evaluation outcomes, revealing complex trade-offs between tokenizer properties and practical utility.
...
### [163] [To See or To Read: User Behavior Reasoning in Multimodal LLMs](https://arxiv.org/abs/2511.03845)
*Tianning Dong,Luyi Ma,Varun Vasudevan,Jason Cho,Sushant Kumar,Kannan Achan*

Main category: cs.AI

TL;DR: BehaviorLens framework compares text vs image representations of user behavior data for MLLMs, finding 87.5% accuracy improvement with image representations.


<details>
  <summary>Details</summary>
Motivation: To determine whether textual or image representations of user behavior data are more effective for maximizing Multimodal Large Language Model performance in reasoning tasks.

Method: Developed BehaviorLens benchmarking framework to assess modality trade-offs across six MLLMs by representing transaction data as text paragraphs, scatter plots, and flowcharts using real-world purchase-sequence data.

Result: When data is represented as images, MLLMs achieved 87.5% improvement in next-purchase prediction accuracy compared to equivalent textual representations, without additional computational cost.

Conclusion: Image representations of user behavior data significantly outperform textual representations for MLLM reasoning tasks, particularly in next-purchase prediction.
...
### [164] [KnowThyself: An Agentic Assistant for LLM Interpretability](https://arxiv.org/abs/2511.03878)
*Suraj Prasai,Mengnan Du,Ying Zhang,Fan Yang*

Main category: cs.AI

TL;DR: KnowThyself is a chat-based LLM interpretability tool that consolidates fragmented analysis capabilities into an accessible interface with natural language queries and interactive visualizations.
...
Method: Uses an orchestrator LLM to reformulate user queries, an agent router to direct them to specialized modules, and contextualizes outputs into coherent explanations within a conversational workflow.

Result: Creates an extensible platform that lowers technical barriers for LLM inspection through interactive visualizations and guided explanations.

Conclusion: KnowThyself provides a robust foundation for accessible LLM interpretability by embedding the entire analysis process into a conversational interface.
...
### [165] [Extracting Causal Relations in Deep Knowledge Tracing](https://arxiv.org/abs/2511.03948)
...
<details>
  <summary>Details</summary>
Motivation: To challenge the prevailing explanation that DKT's performance gains stem from modeling bidirectional relationships between KCs, and instead demonstrate that its strength lies in modeling prerequisite relationships as causal structures.
...
Conclusion: DKT's effectiveness is largely driven by its capacity to model causal dependencies between knowledge components as prerequisite relationships, challenging the conventional understanding of bidirectional relationships.
...
### [166] [LLMs and Cultural Values: the Impact of Prompt Language and Explicit Cultural Framing](https://arxiv.org/abs/2511.03980)
*Bram Bulté,Ayla Rigouts Terryn*

Main category: cs.AI

TL;DR: LLMs show cultural bias despite prompt variations, consistently favoring values from Netherlands, Germany, US, and Japan while being responsive but anchored to specific cultural defaults.


<details>
  <summary>Details</summary>
Motivation: To examine whether LLMs can represent cultural diversity given documented imbalances in training data and optimization objectives, and how prompt language and cultural framing influence model alignment with human values across different countries.
...
Conclusion: LLMs occupy an uncomfortable middle ground - responsive enough to produce variation but too firmly anchored to specific cultural defaults to adequately represent cultural diversity.
...
### [167] [ArchPilot: A Proxy-Guided Multi-Agent Approach for Machine Learning Engineering](https://arxiv.org/abs/2511.03985)
*Zhuowen Yuan,Tao Liu,Yang Yang,Yang Wang,Feng Qi,Kaushik Rangadurai,Bo Li,Shuang Yang*

Main category: cs.AI

TL;DR: ArchPilot is a multi-agent system that uses proxy-based evaluation and adaptive search to efficiently generate and evaluate ML architectures, reducing reliance on expensive full training runs.
...
Method: Three-agent system: orchestration agent coordinates search using MCTS-inspired algorithm with restart mechanism; generation agent creates and improves architectures; evaluation agent uses proxy training and fidelity-aware metrics.

Result: Outperforms SOTA baselines like AIDE and ML-Master on MLE-Bench, demonstrating effective multi-agent collaboration for efficient ML engineering.

Conclusion: ArchPilot enables efficient ML architecture search under limited budgets by prioritizing high-potential candidates through proxy-based evaluation and multi-agent coordination.
...
### [168] [Detecting Silent Failures in Multi-Agentic AI Trajectories](https://arxiv.org/abs/2511.04032)
*Divya Pathak,Harshit Kumar,Anuska Roy,Felix George,Mudit Verma,Pratibha Moogi*

Main category: cs.AI

TL;DR: Introduces anomaly detection for multi-agent AI systems to identify silent failures like drift and cycles, with curated datasets and benchmarking showing supervised methods achieve up to 98% accuracy.


<details>
  <summary>Details</summary>
Motivation: Multi-agent AI systems powered by LLMs are non-deterministic and prone to silent failures that are difficult to detect, requiring systematic approaches for anomaly identification.

Method: Developed a dataset curation pipeline capturing user behavior, agent non-determinism, and LLM variation; created two benchmark datasets with labeled trajectories; benchmarked supervised (XGBoost) and semi-supervised (SVDD) anomaly detection methods.

Result: Supervised (XGBoost) and semi-supervised (SVDD) approaches performed comparably, achieving accuracies up to 98% and 96% respectively on the curated datasets.

Conclusion: Provides the first systematic study of anomaly detection in multi-agent AI systems, offering datasets, benchmarks, and insights to guide future research in detecting silent failures.
...
### [169] [Interpreting Multi-Attribute Confounding through Numerical Attributes in Large Language Models](https://arxiv.org/abs/2511.04053)
*Hirohane Takagi,Gouki Minegishi,Shota Kizawa,Issey Sukeda,Hitomi Yanaka*

Main category: cs.AI

TL;DR: LLMs encode real-world numerical correlations but systematically amplify them, and irrelevant context causes consistent shifts in magnitude representations with varying downstream effects by model size.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs internally integrate multiple numerical attributes of entities and how irrelevant numerical context affects these representations and outputs.
...
Result: LLMs encode real-world numerical correlations but amplify them; irrelevant context induces consistent shifts in magnitude representations with downstream effects varying by model size.

Conclusion: Reveals vulnerability in LLM decision-making and lays groundwork for fairer, representation-aware control under multi-attribute entanglement.
...
### [170] [Agentmandering: A Game-Theoretic Framework for Fair Redistricting via Large Language Model Agents](https://arxiv.org/abs/2511.04076)
*Hao Li,Haotian Chen,Ruoyuan Gong,Juanjuan Wang,Hao Jiang*

Main category: cs.AI

TL;DR: Agentmandering is a game-theoretic redistricting framework using LLM agents that reduces partisan bias and achieves much lower variance than traditional methods.


<details>
  <summary>Details</summary>
Motivation: Existing redistricting methods focus on generating legally valid maps but ignore strategic selection processes, allowing partisan actors to cherry-pick politically advantageous maps despite technical compliance.

Method: A turn-based negotiation framework between two LLM agents representing opposing political interests, using a Choose-and-Freeze protocol where agents alternately select and freeze districts from candidate maps.

Result: Significantly reduces partisan bias and unfairness, achieves 2-3 orders of magnitude lower variance than standard baselines, and demonstrates fairness and stability especially in swing-state scenarios.

Conclusion: Embedding strategic interaction into redistricting through LLM agents can produce fairer and more stable outcomes than traditional ensemble-based approaches.
...
### [171] [KGFR: A Foundation Retriever for Generalized Knowledge Graph Question Answering](https://arxiv.org/abs/2511.04093)
*Yuanning Cui,Zequn Sun,Wei Hu,Zhangjie Fu*

Main category: cs.AI

TL;DR: LLM-KGFR framework combines LLMs with a Knowledge Graph Foundation Retriever for knowledge-intensive reasoning, enabling zero-shot generalization to unseen graphs through structured retrieval and progressive expansion.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with knowledge-intensive questions due to limited context and parametric knowledge, while existing methods have limitations in dataset-specific tuning and scalability on large or unseen graphs.

Method: Uses LLM-KGFR collaborative framework with KGFR that encodes relations using LLM-generated descriptions and initializes entities based on question roles. Employs Asymmetric Progressive Propagation for efficient large graph handling and provides node-, edge-, and path-level interfaces for iterative reasoning.
...
Conclusion: LLM-KGFR framework offers an effective approach for knowledge graph-augmented reasoning with good scalability and zero-shot generalization capabilities.
...
### [172] [Testing the Testers: Human-Driven Quality Assessment of Voice AI Testing Platforms](https://arxiv.org/abs/2511.04133)
*Miguel E. Andres,Vadim Fedorov,Rida Sadek,Enric Spagnolo-Arrizabalaga,Nadescha Trudel*

Main category: cs.AI

TL;DR: First systematic framework for evaluating voice AI testing quality through human-centered benchmarking, addressing both simulation quality (generating realistic conversations) and evaluation quality (accurately assessing agent responses).


<details>
  <summary>Details</summary>
Motivation: Voice AI agents are scaling to billions of daily interactions but lack systematic methods for ensuring testing reliability, creating a critical measurement gap that prevents organizations from objectively assessing testing approaches.

Method: Combines psychometric techniques (pairwise comparisons with Elo ratings, bootstrap confidence intervals, permutation tests) with rigorous statistical validation to provide reproducible metrics applicable to any testing platform.

Result: Empirical evaluation of three commercial platforms using 21,600 human judgments revealed statistically significant performance differences: Evalion achieved 0.92 evaluation quality (f1-score) vs 0.73 for others, and 0.61 simulation quality vs 0.43 for other platforms.
...
### [173] [When Empowerment Disempowers](https://arxiv.org/abs/2511.04177)
...
<details>
  <summary>Details</summary>
Motivation: To study how empowerment-based assistance, which works well for single humans, behaves in multi-human settings where it may negatively impact other humans' control and rewards.

Method: Created Disempower-Grid test suite and empirically tested assistive RL agents optimizing for single human's empowerment in multi-human gridworld environments.

Result: Agents optimizing for one human's empowerment significantly reduced another human's environmental influence and rewards (disempowerment). Joint empowerment mitigated disempowerment but reduced user rewards.

Conclusion: Goal-agnostic objectives that appear aligned in single-agent settings can become misaligned in multi-agent contexts, presenting broader challenges for AI alignment.
...
### [174] [Opus: A Quantitative Framework for Workflow Evaluation](https://arxiv.org/abs/2511.04220)
...
Result: A unified framework that supports automated workflow assessment, ranking, and optimization within modern automation systems like Opus.

Conclusion: The proposed framework enables direct workflow comparison and optimization through joint Reward-Penalty trade-offs, and can be integrated into Reinforcement Learning loops for workflow discovery and refinement.
...
### [175] [Shared Spatial Memory Through Predictive Coding](https://arxiv.org/abs/2511.04235)
*Zhengru Fang,Yu Guo,Jingjing Wang,Yuang Zhang,Haonan An,Yinhai Wang,Yuguang Fang*

Main category: cs.AI

TL;DR: A multi-agent predictive coding framework that minimizes mutual uncertainty among agents, enabling bandwidth-efficient communication and emergence of social place cells for robust coordination under limited bandwidth constraints.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of sharing consistent spatial memory in multi-agent systems where partial observability and limited bandwidth cause coordination failures.

Method: Multi-agent predictive coding with information bottleneck objective, self-supervised motion prediction for grid-cell-like spatial coding, hierarchical reinforcement learning policy for active exploration.
...
### [176] [RLoop: An Self-Improving Framework for Reinforcement Learning with Iterative Policy Initialization](https://arxiv.org/abs/2511.04285)
*Zeng Zhiyuan,Jiashuo Liu,Zhangyue Yin,Ge Zhang,Wenhao Huang,Xipeng Qiu*

Main category: cs.AI

TL;DR: RLoop introduces iterative policy initialization to combat RL overfitting in reasoning models, converting transient policy diversity into robust performance gains through exploration-exploitation cycles.


<details>
  <summary>Details</summary>
Motivation: Address RL overfitting where models gain training rewards but lose generalization due to policy over-specialization and catastrophic forgetting of diverse solutions.

Method: Self-improving framework with iterative policy initialization: RL explores solution space, successful trajectories are filtered into expert dataset, then RFT refines initial policy for next iteration.

Result: Mitigates forgetting and improves generalization, boosting average accuracy by 9% and pass@32 by over 15% compared to vanilla RL.

Conclusion: RLoop effectively converts policy variations into performance gains through iterative re-initialization, substantially improving generalization in RL training.
...
### [177] [GUI-360: A Comprehensive Dataset and Benchmark for Computer-Using Agents](https://arxiv.org/abs/2511.04307)
...
<details>
  <summary>Details</summary>
Motivation: To overcome three persistent gaps in CUA research: scarcity of real-world tasks, lack of automated multi-modal trajectory collection pipelines, and absence of unified benchmarks for GUI grounding, screen parsing, and action prediction.
...
### [178] [Probing the Probes: Methods and Metrics for Concept Alignment](https://arxiv.org/abs/2511.04312)
*Jacob Lysnæs-Larsen,Marte Eggen,Inga Strümke*

Main category: cs.AI

TL;DR: Probe accuracy alone is unreliable for measuring concept alignment in CAVs; misaligned probes can achieve similar accuracy. A new spatial linear attribution method and three alignment metrics are proposed to better assess concept representation.


<details>
  <summary>Details</summary>
Motivation: Current CAV evaluation relies too heavily on probe accuracy, which doesn't guarantee the CAV actually captures the intended concept due to spurious correlations.

Method: Introduced spatial linear attribution for concept localization, compared with feature visualization, and proposed three alignment metrics: hard accuracy, segmentation scores, and augmentation robustness.

Result: Misaligned probes exploiting spurious correlations achieve accuracy close to standard probes. Probes with translation invariance and spatial alignment consistently improve concept alignment.

Conclusion: Probe accuracy alone is insufficient; alignment-based metrics are needed, and probes should be tailored to model architecture and concept nature.
...
### [179] [AdversariaLLM: A Unified and Modular Toolbox for LLM Robustness Research](https://arxiv.org/abs/2511.04316)
...
<details>
  <summary>Details</summary>
Motivation: The rapid expansion of LLM safety research has created a fragmented, buggy ecosystem that makes reproducibility and comparability challenging, hindering meaningful progress in the field.

Method: Developed AdversariaLLM toolbox with focus on reproducibility, correctness, and extensibility. Includes 12 adversarial attack algorithms, 7 benchmark datasets covering harmfulness, over-refusal, and utility evaluation, integration with Hugging Face LLMs, and features like compute-resource tracking and deterministic results.

Result: Created a comprehensive framework that establishes foundation for transparent, comparable, and reproducible LLM safety research, with integration to JudgeZoo for independent judging capabilities.

Conclusion: AdversariaLLM provides a robust solution to the fragmentation problem in LLM safety research, enabling more reliable and comparable studies through its reproducible and extensible framework.
...
### [180] [RxSafeBench: Identifying Medication Safety Issues of Large Language Models in Simulated Consultation](https://arxiv.org/abs/2511.04328)
*Jiahao Zhao,Luxin Xu,Minghuan Tan,Lichao Zhang,Ahmadreza Argha,Hamid Alinejad-Rokny,Min Yang*

Main category: cs.AI

TL;DR: RxSafeBench is a comprehensive benchmark for evaluating medication safety in LLMs, featuring 2,443 clinical consultation scenarios with embedded medication risks. Current LLMs struggle with integrating contraindication and interaction knowledge, especially for implied risks.


<details>
  <summary>Details</summary>
Motivation: Limited research exists on LLM medication safety due to lack of real-world datasets and privacy constraints. Evaluation in realistic clinical consultation settings is underexplored, creating a gap in assessing LLM reliability for medication-related decisions.

Method: Proposed framework simulates clinical consultations with embedded medication risks. Created RxRisk DB with 6,725 contraindications, 28,781 drug interactions, and 14,906 indication-drug pairs. Used two-stage filtering for clinical realism, resulting in RxSafeBench with 2,443 scenarios. Evaluated LLMs using structured multiple choice questions.

Result: Current LLMs struggle to integrate contraindication and interaction knowledge effectively. Performance is particularly poor when medication risks are implied rather than explicit. Both open-source and proprietary models show limitations in medication safety capabilities.

Conclusion: RxSafeBench provides the first comprehensive benchmark for medication safety evaluation in LLMs. Findings highlight key challenges and offer insights for improving reliability through better prompting and task-specific tuning, advancing safer AI-driven clinical decision support.
...
### [181] [Monitor-Generate-Verify (MGV):Formalising Metacognitive Theory for Language Model Reasoning](https://arxiv.org/abs/2511.04341)
*Nick Oh,Fernand Gobet*

Main category: cs.AI

TL;DR: The paper proposes the Monitor-Generate-Verify (MGV) framework to address the prefix dominance trap in test-time reasoning architectures, where models commit early to suboptimal reasoning paths and struggle to recover, causing ~20% accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Current Generate-Verify paradigm architectures prioritize generation and verification but lack monitoring processes that determine when and how reasoning should begin, leading to the prefix dominance trap where models get stuck on early suboptimal reasoning paths.

Method: Formalize Flavell's and Nelson and Narens' metacognitive theories into computational specifications, extending the Generate-Verify paradigm by adding explicit monitoring that captures metacognitive experiences before generation begins and refines future monitoring through verification feedback.

Result: No empirical validation presented, but the work provides the first systematic computational translation of foundational metacognitive theories.

Conclusion: The MGV framework offers a principled vocabulary for understanding reasoning system failures and suggests specific architectural interventions for future test-time reasoning designs.
...
### [182] [Post-Training LLMs as Better Decision-Making Agents: A Regret-Minimization Approach](https://arxiv.org/abs/2511.04393)
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with basic online decision-making problems, failing to achieve low regret or effective exploration-exploitation tradeoffs, despite being increasingly deployed as agents in interactive environments.

Method: Iterative RMFT repeatedly distills low-regret decision trajectories back into the base model. At each iteration, the model rolls out multiple trajectories, selects the k-lowest regret ones, and fine-tunes itself on them using natural-language training signals.

Result: Iterative RMFT improves LLMs' decision-making performance across diverse models (Transformers, open-weight LLMs, GPT-4o mini) and enables generalization across tasks with varying horizons, action spaces, reward processes, and natural-language contexts.
...
### [183] [The Peril of Preference: Why GRPO fails on Ordinal Rewards](https://arxiv.org/abs/2511.04439)
*Anisha Garg,Ganesh Venkatesh*

Main category: cs.AI

TL;DR: CoRPO improves upon GRPO by using an adaptive baseline that prevents positive reinforcement of failed trajectories and transitions to relative preference mode once quality threshold is met, enabling better learning from ordinal rewards.
...
Method: Introduces Correctness Relative Policy Optimization (CoRPO) with adaptive baseline that enforces minimum quality threshold and transitions to relative preference mode after threshold is consistently met.
...
Conclusion: CoRPO represents a critical step in enabling LLMs to learn from rich, multi-dimensional feedback, progressing from binary to ordinal rewards toward denser per-step supervision.
...
### [184] [Beyond Shortest Path: Agentic Vehicular Routing with Semantic Context](https://arxiv.org/abs/2511.04464)
*Carnot Braun,Rafael O. Jarczewski,Gabriel U. Talasso,Leandro A. Villas,Allan M. de Souza*

Main category: cs.AI

TL;DR: PAVe combines classical routing algorithms with LLM-based semantic reasoning to create personalized vehicle routing that considers complex human contexts like multi-step tasks and preferences.


<details>
  <summary>Details</summary>
Motivation: Traditional routing systems only optimize singular metrics and cannot handle complex human contexts like multi-step tasks, situational constraints, or urgent needs.

Method: Uses a hybrid approach: multi-objective Dijkstra algorithm generates candidate routes, then an LLM agent evaluates them against user tasks, preferences, and avoidance rules using geospatial POI cache.
...
### [185] [Promoting Sustainable Web Agents: Benchmarking and Estimating Energy Consumption through Empirical and Theoretical Analysis](https://arxiv.org/abs/2511.04481)
*Lars Krupp,Daniel Geißler,Vishal Banwari,Paul Lukowicz,Jakob Karolus*

Main category: cs.AI

TL;DR: This paper explores the sustainability issues of web agents, analyzing their energy and CO2 costs through theoretical estimation and empirical benchmarking, revealing that different design approaches significantly impact energy consumption without necessarily improving results.


<details>
  <summary>Details</summary>
Motivation: Web agents are powerful AI systems that autonomously interact with the internet, but their sustainability impacts remain largely unexplored despite growing research in this field.

Method: The study uses both theoretical estimation and empirical benchmarking approaches to analyze energy and CO2 costs of web agents, comparing different design philosophies.

Result: Results show that different web agent design philosophies severely impact energy consumption, and higher energy use doesn't necessarily lead to better performance. The study also identifies lack of transparency in model parameters as a limiting factor for accurate energy estimation.

Conclusion: The work advocates for changing how web agents are evaluated by including dedicated metrics for energy consumption in benchmarks, highlighting the need for sustainability considerations in web agent development.
...
### [186] [Large language models replicate and predict human cooperation across experiments in game theory](https://arxiv.org/abs/2511.04500)
*Andrea Cera Palatsi,Samuel Martin-Gutierrez,Ana S. Cardenal,Max Pellert*

Main category: cs.AI

TL;DR: LLMs can replicate human decision-making in game theory experiments, with Llama closely matching human cooperation patterns and Qwen aligning with Nash equilibrium, enabling new social science research approaches.


<details>
  <summary>Details</summary>
Motivation: To understand how closely LLMs mirror actual human decision-making, as misalignment could cause harm in practical applications and failure to replicate human behavior makes LLMs ineffective for social simulations.

Method: Developed a digital twin of game-theoretic experiments with systematic prompting and probing framework, testing three open-source models (Llama, Mistral, Qwen) without persona-based prompting.

Result: Llama reproduced human cooperation patterns with high fidelity, capturing human deviations from rational choice theory, while Qwen aligned closely with Nash equilibrium predictions. Achieved population-level behavioral replication and generated testable hypotheses for novel game configurations.
...
### [187] [Optimizing Sensor Placement in Urban Storm Sewers: A Data-Driven Sparse Sensing Approach](https://arxiv.org/abs/2511.04556)
*Zihang Ding,Kun Zhang*

Main category: cs.AI

TL;DR: A data-driven sparse sensing framework optimizes sensor placement in stormwater systems to reconstruct peak flowrates with minimal sensors, achieving high accuracy (NSE 0.92-0.95) using just 3 sensors among 77 nodes.


<details>
  <summary>Details</summary>
Motivation: Urban surface water flooding is increasing but high-resolution monitoring is constrained by time, budget, and technology. Need efficient methods to monitor drainage networks under resource limitations.

Method: Integrated EPA-SWMM with data-driven sparse sensing using singular value decomposition for dimensionality reduction and QR factorization for optimal sensor placement. Validated with Woodland Avenue catchment case study.

Result: Three optimally placed sensors achieved excellent reconstruction performance (NSE 0.92-0.95). Framework showed good robustness to measurement uncertainty and location-dependent robustness to sensor failures.

Conclusion: The DSS framework balances computational efficiency and physical interpretability, enabling accurate flow reconstruction with minimal sensors. Can be integrated with predictive models for flood early warning under limited monitoring resources.
...
### [188] [Jr. AI Scientist and Its Risk Report: Autonomous Scientific Exploration from a Baseline Paper](https://arxiv.org/abs/2511.04583)
*Atsuyuki Miyai,Mashiro Toyooka,Takashi Otonari,Zaiying Zhao,Kiyoharu Aizawa*

Main category: cs.AI

TL;DR: Jr. AI Scientist is a state-of-the-art autonomous AI system that mimics a novice researcher's workflow to analyze papers, formulate hypotheses, conduct experiments, and write papers, demonstrating improved performance over fully automated systems while revealing important limitations and risks.


<details>
  <summary>Details</summary>
Motivation: To understand current capabilities and risks of AI Scientist systems for ensuring trustworthy AI-driven scientific progress and preserving academic integrity.

Method: Developed Jr. AI Scientist that follows a novice researcher workflow: analyzes paper limitations, formulates hypotheses, validates through experiments, and writes papers using modern coding agents for complex implementations.

Result: Jr. AI Scientist generates papers with higher review scores than existing fully automated systems, but author evaluations and Agents4Science reviews reveal important limitations and risks.

Conclusion: Current AI Scientist systems have limitations and risks that need addressing, and the findings provide insights for understanding progress and challenges in AI Scientist development.
...
### [189] [Are We Asking the Right Questions? On Ambiguity in Natural Language Queries for Tabular Data Analysis](https://arxiv.org/abs/2511.04584)
*Daniel Gomm,Cornelius Wolff,Madelon Hulsebos*

Main category: cs.AI

TL;DR: The paper reframes ambiguity in natural language queries as cooperative interaction, develops a framework to distinguish cooperative vs uncooperative queries, and analyzes 15 datasets to show current evaluations mix query types inadequately.


<details>
  <summary>Details</summary>
Motivation: Natural language interfaces to tabular data must handle query ambiguities, but current approaches treat ambiguity as a deficiency rather than a feature of cooperative interaction.

Method: Developed a principled framework distinguishing cooperative queries (resolvable) from uncooperative queries (unresolvable), and applied it to analyze queries in 15 popular tabular QA datasets.

Result: Found uncontrolled mixing of query types in current datasets that is inadequate for evaluating both execution accuracy and interpretation capabilities of systems.
...
### [190] [Question the Questions: Auditing Representation in Online Deliberative Processes](https://arxiv.org/abs/2511.04588)
*Soham De,Lodewijk Gelauff,Ashish Goel,Smitha Milli,Ariel Procaccia,Alice Siu*

Main category: cs.AI

TL;DR: The paper introduces an auditing framework for measuring question representation in deliberative processes using justified representation (JR) concepts, with efficient algorithms and applications to real-world deliberations.
...
Method: Developed auditing framework based on justified representation (JR) with efficient algorithms (O(mn log n) runtime), applied to historical deliberations comparing moderator-chosen questions, ILP-selected questions, and LLM-generated summary questions.

Result: Applied methods to historical deliberations, revealing both promise and limitations of LLMs in supporting deliberative processes. Integrated framework into an online deliberation platform used globally.

Conclusion: The auditing framework enables practitioners to measure and improve representation in deliberative processes, with practical implementation in real-world deliberation platforms across multiple countries.
...
### [191] [DR. WELL: Dynamic Reasoning and Learning with Symbolic World Model for Embodied LLM-Based Multi-Agent Collaboration](https://arxiv.org/abs/2511.04646)
...
Method: Two-phase negotiation protocol: agents propose candidate roles with reasoning, then commit to joint allocation under consensus and environment constraints. Each agent independently generates symbolic plans for their role, grounded via a shared dynamic world model.

Result: Experiments on cooperative block-push tasks show improved task completion rates and efficiency through negotiation and self-refinement, with the dynamic world model capturing reusable patterns.

Conclusion: Symbolic planning enables higher-level operations that are reusable, synchronizable, and interpretable, avoiding brittle step-level alignment while trading time overhead for evolving collaboration strategies.
...
### [192] [VeriCoT: Neuro-symbolic Chain-of-Thought Validation via Logical Consistency Checks](https://arxiv.org/abs/2511.04662)
...
Method: Extracts CoT reasoning steps into first-order logic, identifies premises grounded in source context/commonsense/prior steps, uses symbolic representation for automated verification, and applies verification signal for self-reflection, supervised fine-tuning, and preference fine-tuning.

Result: Experiments on ProofWriter, LegalBench, and BioASQ show VeriCoT effectively identifies flawed reasoning and serves as strong predictor of final answer correctness.

Conclusion: VeriCoT improves reasoning validity and accuracy through verification-based approaches including self-reflection, supervised fine-tuning, and preference fine-tuning.
...
### [193] [Reconstruction-free segmentation from undersampled k-space using transformers](https://arxiv.org/abs/2511.03762)
*Yundi Zhang,Nil Stolt-Ansó,Jiazhen Pan,Wenqi Huang,Kerstin Hammernik,Daniel Rueckert*

Main category: eess.IV

TL;DR: Direct cardiac segmentation from undersampled k-space measurements using transformers, bypassing intermediate image reconstruction for better performance at high acceleration factors.


<details>
  <summary>Details</summary>
Motivation: High acceleration factors limit MRI reconstruction quality, which subsequently affects segmentation models when treated as separate processes.

Method: Transformer architecture encodes global k-space information into latent features that condition queried coordinates during decoding to generate segmentation probabilities.

Result: Produces better segmentations across high acceleration factors than image-based segmentation baselines.

Conclusion: Direct k-space segmentation circumvents intermediate reconstruction, enabling myocardial structure assessment at higher acceleration factors than image-based methods.
...
### [194] [Computed Tomography (CT)-derived Cardiovascular Flow Estimation Using Physics-Informed Neural Networks Improves with Sinogram-based Training: A Simulation Study](https://arxiv.org/abs/2511.03876)
*Jinyuxuan Guo,Gurnoor Singh Khurana,Alejandro Gonzalo Grande,Juan C. del Alamo,Francisco Contijoch*

Main category: eess.IV

TL;DR: SinoFlow uses sinogram data directly for CT-based blood flow estimation, outperforming image-based methods by avoiding reconstruction errors and working better with various CT settings.


<details>
  <summary>Details</summary>
Motivation: CT imaging is widely used for cardiovascular assessment but lacks direct methods to estimate blood flow velocity from contrast evolution movies. Current approaches suffer from reconstruction errors.

Method: Generated pulsatile flow in 2D vessel bifurcation using CFD, simulated CT scans with varying parameters, and compared PINN-based flow estimation using reconstructed images (ImageFlow) vs. direct sinogram data (SinoFlow).

Result: SinoFlow significantly improved flow estimation by avoiding filtered backprojection errors, was robust across gantry speeds, produced lower MSE and velocity errors, and worked well with pulsed-mode imaging.

Conclusion: SinoFlow demonstrates strong potential for CT-based flow estimation, providing a more accurate non-invasive approach for blood flow assessment with reasonable acquisition parameters.
...
### [195] [Shape Deformation Networks for Automated Aortic Valve Finite Element Meshing from 3D CT Images](https://arxiv.org/abs/2511.03890)
*Linchen Qian,Jiasong Chen,Ruonan Gong,Wei Sun,Minliang Liu,Liang Liang*

Main category: eess.IV

TL;DR: A template-fitting pipeline with deep neural networks generates structured quadrilateral meshes from 3D CT images for aortic valve modeling, ensuring consistent correspondence across patients and simplifying training with fewer regularization terms.


<details>
  <summary>Details</summary>
Motivation: Traditional approaches produce irregular triangular meshes with poor element quality and inconsistent correspondence due to inter-patient anatomical variation, making biomechanical analysis and patient-specific simulations challenging.

Method: A template-fitting pipeline with deep neural networks that remeshes aortic valves using a common quad mesh template, employing a simplified loss function with only geometry reconstruction and smoothness regularization terms.

Result: The approach produces high-quality aortic valve surface meshes with improved smoothness and shape quality, requiring fewer explicit regularization terms compared to traditional methods.

Conclusion: Using structured quad meshes for template and neural network training ensures mesh correspondence and quality while simplifying the training process, enhancing the effectiveness and efficiency of aortic valve modeling.
...
### [196] [DeepFixel: Crossing white matter fiber identification through spherical convolutional neural networks](https://arxiv.org/abs/2511.03893)
...
TL;DR: DeepFixel is a spherical CNN that efficiently separates crossing white matter fibers in diffusion MRI data, outperforming traditional optimization methods in speed while maintaining high accuracy.


<details>
  <summary>Details</summary>
Motivation: Crossing white matter fibers in voxels complicate analysis and cause errors in downstream tasks like tractography, requiring efficient methods to separate fiber orientation distribution functions.

Method: DeepFixel uses a spherical convolutional neural network to approximate nonlinear optimization, modeling fiber probability distributions on a spherical mesh with higher angular resolution than spherical harmonics.

Result: DeepFixel achieves median angular correlation coefficient of 0.973 (vs 1.0 for optimization and 0.988 for fixel-based method) but is 3150x faster than optimization (0.32 ms vs 1.01x10^6 ms per voxel).
...
### [197] [$μ$NeuFMT: Optical-Property-Adaptive Fluorescence Molecular Tomography via Implicit Neural Representation](https://arxiv.org/abs/2511.04510)
